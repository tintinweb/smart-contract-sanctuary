...
contract DoodleRooms is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    modifier maxOGSaleDoodleRoomsPerWallet(uint256 numberOfTokens) {
        require(
            balanceOf(msg.sender) + numberOfTokens <= MAX_OG_DOODLE_ROOMS_PER_WALLET,
            "Max DoodleRooms for OGs to mint is ten"
        );
        _;
    }

    modifier maxPresaleDoodleRoomsPerWallet(uint256 numberOfTokens) {
        require(
            balanceOf(msg.sender) + numberOfTokens <= MAX_PRESALE_DOODLE_ROOMS_PER_WALLET,
            "Max DoodleRooms for presale to mint is five"
        );
        _;
    }
    ...
    modifier canMintPresaleDoodleRooms(uint256 numberOfTokens) {
        require(
            tokenCounter.current() + numberOfTokens <= maxPresaleDoodleRooms,
            "Not enough DoodleRooms remaining to mint"
        );
        _;
    }

    modifier canMintOGDoodleRooms(uint256 numberOfTokens) {
        require(
            tokenCounter.current() + numberOfTokens <= maxOGSaleDoodleRooms,
            "Not enough DoodleRooms remaining to mint"
        );
        _;
    }
    ...
    modifier canGiftDoodleRooms(uint256 num) {
        ...
        require(
            tokenCounter.current() + num <= DOODLE_ROOMS_SUPPLY,
            "Not enough DoodleRooms remaining to mint"
        );
        _;
    }
    ...
    constructor(
        ...
        // avoid redundant SSTORE (zero initialzation of tokenCounter) for the first mint
        // saves some gas
    ...
    nonReentrant
    presaleActive
    canMintPresaleDoodleRooms(numberOfTokens)
    maxPresaleDoodleRoomsPerWallet(numberOfTokens)
    isCorrectPayment(numberOfTokens)
    ...
    nonReentrant
    ogSaleActive
    canMintOGDoodleRooms(numberOfTokens)
    maxOGSaleDoodleRoomsPerWallet(numberOfTokens)
    ...
    //    canGiftDoodleRooms(1)
    //    {
    //        require(!claimed[msg.sender], "Doodle Room already claimed by this wallet");
    //
    //        claimed[msg.sender] = true;
    //        numGiftedDoodleRooms += 1;
    //
    ...
    canGiftDoodleRooms(addresses.length)
    {
        ...
        for (uint256 i = 0; i < numToGift; i++) {
            _safeMint(addresses[i], tokenCounter.current());
            tokenCounter.increment();
        }
    }
    ...
    //
    //    function reserveDoodleRooms(address[] calldata addresses)
    //    external
    //    nonReentrant
    //    onlyOwner
    //    {
    ...
    //            "All DoodleRooms are already reserved"
    //        );
    //
    //        for (uint256 i = 0; i < addresses.length; i++) {
    //            presaleMintCounts[addresses[i]] += 1;
    //            // use mint rather than _safeMint here to reduce gas costs
    //            // and prevent this from failing in case of grief attempts
    //            _mint(addresses[i], tokenCounter.current());
    //            tokenCounter.increment();
    //        }
    //    }

    function _batchMint(address _addr, uint256 _numberOfTokens) private {
        for (uint256 i = 0; i < _numberOfTokens; i++) {
            _safeMint(_addr, tokenCounter.current());
            tokenCounter.increment();
        }
    }
