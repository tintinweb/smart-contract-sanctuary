...
contract Gateway is Ownable {
    ...
    function initialize(IPriceOracle _priceOracle, address _orgWallet, address _teamWallet) public initializer {
        ...
        maxVariableFeeInUsd = 500e18; // $500
        serviceFeePercentWhenNoPrice = 1e17; // 10%
    ...
    function transferFromAnotherChain(
        ...
        require(tokens.length > tokenId, "Gateway: token does't exist");
    ...
    function setFeeParameters(uint128 _minVariableFeeInUsd, uint128 _maxVariableFeeInUsd, uint128 _serviceFeePercentWhenNoPrice, uint128 _serviceFeePercent, uint256 _feeShareToTheTeam) public onlyOwner {
        require(minVariableFeeInUsd < maxVariableFeeInUsd, "Gateway: minVariableFeeInUsd should be < maxVariableFeeInUsd");
        require(feeShareToTheTeam <= 1e18, "Gateway: Invalid feeShareToTheTeam");
        require(serviceFeePercentWhenNoPrice <= 1e18, "Gateway: Invalid serviceFeePercentWhenNoPrice");
        require(serviceFeePercent <= 1e18, "Gateway: Invalid serviceFeePercent");
        require(feeShareToTheTeam <= 1e18, "Gateway: Invalid feeShareToTheTeam");
    ...
    function feeCalculation(address token, uint256 tokenAmount) public view returns(uint256 fee) {
        uint256 usdcAmount = address(priceOracle) != address(0) ? priceOracle.priceOf(token, tokenAmount) : 0;
        if (usdcAmount == 0) {
            return tokenAmount.mul(serviceFeePercentWhenNoPrice).div(1e18);
        }
        
        uint256 feeInUsdc = usdcAmount.mul(serviceFeePercent).div(1e18);

        if (feeInUsdc < minVariableFeeInUsd) {
            return tokenAmount.mul(minVariableFeeInUsd).div(usdcAmount);
        } else if(feeInUsdc > maxVariableFeeInUsd) {
            return tokenAmount.mul(maxVariableFeeInUsd).div(usdcAmount);
        }
...
interface IPriceOracle {
    ...
    function priceOf(address token, uint256 amount) external view returns (uint256 usdcAmount);
}
