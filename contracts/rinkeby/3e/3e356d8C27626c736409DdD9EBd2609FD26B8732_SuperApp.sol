...
contract SuperApp is SuperAppBase {
    ...
    // function startStream(address lockAddress) external {
    //     IPublicLock newLock = IPublicLock(lockAddress);
                
    //     if(newLock.getTokenIdFor(msg.sender) != 0) {
    //         revert("Content already unlocked");
    //     }

    //     if(userToLockMapping[msg.sender] != address(0)) {
    //         IPublicLock prevLock = IPublicLock(userToLockMapping[msg.sender]);
    //         prevLock.expireAndRefundFor(msg.sender, 0);
    //     }

    //     (,int96 userToAppFlowRate,,) = _cfa.getFlow(_acceptedToken, msg.sender, address(this));
    //     if(userToAppFlowRate >= _requiredFlowRate) {
    //         // @dev get the information about the previous stream the user was watching.
    //         address prevCreator = IPublicLock(userToLockMapping[msg.sender]).beneficiary();

            
    //         (,int96 userToPrevCreatorFlowRate,,) = _cfa.getFlow(_acceptedToken, address(this), prevCreator);
    //         int96 userToPrevCreatorNewFlowRate = userToPrevCreatorFlowRate - _requiredFlowRate;
            
    //         // @dev there is a possibility that the previous streamer had only one viewers so the flow has to be deleted else updated.
    //         updateOrDeleteFlow(userToPrevCreatorNewFlowRate, prevCreator);

    //         address newCreator = newLock.beneficiary();
    //         (,int96 userToNewCreatorFlowRate,,) = _cfa.getFlow(_acceptedToken, address(this), newCreator);
    //         int96 userToNewCreatorNewFlowRate = userToNewCreatorFlowRate + _requiredFlowRate;

    //         // @dev there is a possibility that the new streamer gets the first viewer in that case the flow has to be created else updated.
    //         updateOrCreateFlow(userToNewCreatorNewFlowRate, newCreator);

    //         // @dev assign the new key to user.
    //         address[] memory receipients = new address[](1);
    //         uint256[] memory timestamp = new uint256[](1);
    //         address[] memory keyManagers = new address[](1);
    //         receipients[0] = msg.sender;
    //         timestamp[0] = block.timestamp + 10 * 60;
    //         keyManagers[0] = address(this);

    //         newLock.grantKeys(receipients, timestamp, keyManagers);

    //         // @dev now change the userToLock mapping
    //         userToLockMapping[msg.sender] = lockAddress;

    //         // @dev emit some event here.
    //     } else if(userToAppFlowRate == 0) {
    //         address newCreator = newLock.beneficiary();
    //         (,int96 userToNewCreatorFlowRate,,) = _cfa.getFlow(_acceptedToken, address(this), newCreator);
    //         int96 userToNewCreatorNewFlowRate = userToNewCreatorFlowRate + _requiredFlowRate;
            
    //         (bool success, bytes memory data) = address(_cfa).delegatecall(abi.encodeWithSelector(_cfa.createFlow.selector, "_acceptedToken", "address(this)", "_requiredFlowRate", ""));
    ...
    // function updateOrDeleteFlow(int96 flowRate, address receiver) internal {
    //     if(flowRate < 0) {
    //         (bool success, bytes memory data) = address(_cfa).delegatecall(abi.encodeWithSelector(_cfa.deleteFlow.selector, "_acceptedToken", "address(this)", "receiver", ""));
    //     } else {
    //         (bool success, bytes memory data) = address(_cfa).delegatecall(abi.encodeWithSelector(_cfa.updateFlow.selector, "_acceptedToken", "receiver", "flowRate", ""));
    //     }
    // }

    // function updateOrCreateFlow(int96 flowRate, address receiver) internal {
    //     if(flowRate == _requiredFlowRate) {
    //         (bool success, bytes memory data) = address(_cfa).delegatecall(abi.encodeWithSelector(_cfa.createFlow.selector, "_acceptedToken", "receiver", "flowRate", ""));
    //     } else {
    //         (bool success, bytes memory data) = address(_cfa).delegatecall(abi.encodeWithSelector(_cfa.updateFlow.selector, "_acceptedToken", "receiver", "flowRate", ""));
    //     }
    // }
        ...
        returns (bytes memory newCtx)
    {
        ...
        _expirationTimestamps[0] = block.timestamp + 10 * 60;
        _keyManagers[0] = address(this);

        IPublicLock(lockAddress).grantKeys(_recipients, _expirationTimestamps, _keyManagers);
