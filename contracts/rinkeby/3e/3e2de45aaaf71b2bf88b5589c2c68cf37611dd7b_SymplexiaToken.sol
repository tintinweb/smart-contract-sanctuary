...
abstract contract AutoLiquidity is BaseToken, Adjustable {
    ...
    uint256 private _liquidityDeviation;
    ...
    function _tokenLiquify(address sender, uint256 amountToTransfer) internal {    
        ...
        if ( autoLiquidity && !inSwapAndLiquify ) {
            ...
            if (sender == liquidityPair && isAdjustable ) {
                uint256 _stabilityRatio = (balanceOf(liquidityPair) * 10) / 100;
                ...
                if (_liquidityDeviation >= _stabilityRatio && isAdjustable) {
                    Wallet[regulatoryFunds].balance -= _liquidityDeviation;      
                    Wallet[address(this)].balance   += _liquidityDeviation;
                    _tokenToLiquidity                = _liquidityDeviation;
                    _liquidityDeviation              = 0;
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor ()  ERC20("BETA15", "BTG15") BaseToken ( 
