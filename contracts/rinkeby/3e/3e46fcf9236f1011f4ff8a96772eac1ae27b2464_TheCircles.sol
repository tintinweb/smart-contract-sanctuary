...
// File: contracts/TheCircles.sol

/**
 * @title TheCircles contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract TheCircles is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public MAX_CIRCLE_SUPPLY;
    ...
    constructor() ERC721("The Circles", "CIRCLE") {
        MAX_CIRCLE_SUPPLY = 10000;
        REVEAL_TIMESTAMP = block.timestamp + (86400 * 7);
        mintPrice = 69000000000000000; // 0.069 ETH
        maxToMint = 30;
        saleIsActive = false;
        wallet = 0x24C4A1bD89CaD93Fb1b5DAa6aa72ED3B6bE0dbA6;
    }
    ...
    /**
     * Mint Circles by owner
     */
    function reserveCircles(address _to, uint256 _numberOfTokens) external onlyOwner {
    ...
    * Mints tokens
    */
    function mintCircles(uint256 numberOfTokens) external payable {
        require(saleIsActive, "Sale must be active to mint");
        require(numberOfTokens <= maxToMint, "Invalid amount to mint per once");
        require(totalSupply().add(numberOfTokens) <= MAX_CIRCLE_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < MAX_CIRCLE_SUPPLY) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_CIRCLE_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() external {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_CIRCLE_SUPPLY;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % MAX_CIRCLE_SUPPLY;
        }
