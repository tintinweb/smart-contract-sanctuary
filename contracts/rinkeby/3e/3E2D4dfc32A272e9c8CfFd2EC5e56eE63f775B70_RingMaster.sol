...
contract RingMaster is NFTExtension, Metadata {
  ...
  uint256 private reservedSupply = 70;
  ...
  // Mint Function to be called from staking contract
  function evolve_mint(address _user) external override onlyAdmin returns (uint256) {
    uint256 _currentSupply = _tokenIds.current();
    require(_currentSupply + 1 <= maxSupply - reservedSupply, 'Not enough Tokens left.');
    _tokenIds.increment();
    uint256 newItemId = _tokenIds.current();
    mintListPurchases[_user]++;
  ...
  function sendReserve(address _receiver, uint256 _nbTokens) public onlyAdmin {
    uint256 _currentSupploy = _tokenIds.current();
    require(_currentSupploy + _nbTokens <= maxSupply - reservedSupply, 'Not enough supply left');
  ...
  function setReservedSupply(uint256 _newReservedSupply) external onlyOwner {
    require(_tokenIds.current() < 1, 'Reserved Supply can not be changed after first mint.');
