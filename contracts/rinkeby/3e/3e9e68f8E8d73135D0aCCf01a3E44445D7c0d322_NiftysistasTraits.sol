...
contract NiftysistasTraits is ERC1155, Ownable, Pausable {
    ...
    uint[] private specialTraits = [746, 730, 725, 560, 284, 171, 142, 100, 682, 683];
    uint private rn = 0;
    ...
    constructor () ERC1155("https://niftysistas.com/traits/metadata/") {
        _pause();
    }

    function mint(uint[] calldata traits, address to) external payable {
        require(mintingContract == msg.sender, "Only allowed from minting contract.");
    ...
    function setMintingContract(address _address) external onlyOwner {
        mintingContract = _address;
    }

    function burn(address account, uint256 tokenId) external {
        require(balanceOf(msg.sender, tokenId) >= 1 || mintingContract == msg.sender, "Burnable: caller is not owner nor approved");
        _burn(account, tokenId, 1);
    }    

    function claimSpecialTrait(uint[] calldata dudeIds) external {
        require(!paused(), "claiming is paused");
        //require(block.timestamp < 1623092400, "claiming period expired");
        ...
        for (uint i=0; i < dudeIds.length; i++) {
            ...
            _mint(msg.sender, specialTraits[(dudeIds[i]+rn)%10], 1, "");
        }
    }
