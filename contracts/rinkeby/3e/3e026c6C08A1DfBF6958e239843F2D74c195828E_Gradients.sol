...
/**
 * @title Gradients ERC-721 token contract.
 * @author Josh Stow (https://github.com/jshstw)
 */
contract Gradients is ERC721Enumerable, Ownable, Pausable {
  using SafeMath for uint256;
  using Address for address payable;

  uint256 public constant GRAD_SUPPLY = 9999;
  uint256 public constant GRAD_SUPPLY_PRESALE = 2500;
  uint256 public constant GRAD_PER_WALLET = 20;
  uint256 public constant GRAD_PER_WALLET_PRESALE = 5;
  uint256 public constant GRAD_PRICE = 0.06 ether;

  uint256 private _totalWhitelisted;

  mapping(address => bool) private whitelist;

  mapping(address => uint256) public addressToReserved;
  ...
  /**
   * @dev Reserves number of tokens specified during presale.
   * @param quantity uint256 Number of tokens to be reserved
   */
  function presaleBuy(uint256 quantity) external payable whenNotPaused {
    require(presaleLive, "Gradients: Presale not currently live");
    require(!saleLive, "Gradients: Sale is no longer in the presale stage");
    require(whitelist[msg.sender], "Gradients: Caller is not eligible for presale");
    require(quantity <= (GRAD_PER_WALLET_PRESALE - addressToReserved[msg.sender]), "Gradients: Wallet cannot reserve any new tokens");
    require(quantity != 0, "Gradients: Cannot buy zero tokens");
    require(totalSupply() <= (GRAD_SUPPLY_PRESALE - quantity), "Gradients: Quantity exceeds remaining tokens");
    require(msg.value >= (quantity * GRAD_PRICE), "Gradients: Insufficient funds");

    addressToReserved[msg.sender] += quantity;
  }

  /**
   * @dev Reserves number of tokens specified.
   * @param quantity uint256 Number of tokens to be reserved
   */
  function buy(uint256 quantity) external payable whenNotPaused {
    require(!presaleLive, "Gradients: Sale is currently in the presale stage");
    require(saleLive, "Gradients: Sale is not currently live");
    require(quantity <= (GRAD_PER_WALLET - addressToReserved[msg.sender]), "Gradients: Wallet cannot reserve any new tokens");
    require(quantity != 0, "Gradients: Cannot buy zero tokens");
    require(totalSupply() <= (GRAD_SUPPLY - quantity), "Gradients: Quantity exceeds remaining tokens");
    require(msg.value >= (quantity * GRAD_PRICE), "Gradients: Insufficient funds");
  ...
  function mint() external whenNotPaused {
    require(!presaleLive, "Gradients: Sale is currently in the presale stage");
    require(!saleLive, "Gradients: Sale is currently ongoing");
    require(mintingLive, "Gradients: Minting is not open yet");
    require(addressToReserved[msg.sender] != 0, "Gradients: Caller has no tokens reserved");

    uint256 quantity = addressToReserved[msg.sender];
    delete addressToReserved[msg.sender];
  ...
  function addToWhitelist(address[] calldata wallets) external onlyOwner {
    for(uint256 i = 0; i < wallets.length; i++) {
      ...
      require(wallet != address(0), "Gradients: Cannot add zero address");
      require(!whitelist[wallet], "Gradients: Duplicate address");
  ...
  /**
   * @dev Toggles status of token minting. Only callable by owner.
   */
