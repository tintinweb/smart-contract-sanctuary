...
// File: contracts/AlmanackLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title Almanack Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract AlmanackLottery is ReentrancyGuard, IAlmanackLottery, Ownable {
    ...
    uint256 public maxPriceTicketInAlmanack = 50 ether;
    uint256 public minPriceTicketInAlmanack = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInAlmanack;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] almanackPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInAlmanack;
        uint32 finalNumber;
    }
     ...
     * @param _almanackTokenAddress: address of the ALMANACK token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _almanackTokenAddress, address _randomGeneratorAddress) {
        almanackToken = IERC20(_almanackTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ALMANACK to this contract
        uint256 amountAlmanackToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInAlmanack,
            _ticketNumbers.length
        );

        // Transfer almanack tokens to this contract
        almanackToken.safeTransferFrom(address(msg.sender), address(this), amountAlmanackToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInAlmanack += amountAlmanackToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInAlmanackToTransfer
        uint256 rewardInAlmanackToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInAlmanackToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        almanackToken.safeTransfer(msg.sender, rewardInAlmanackToTransfer);

        emit TicketsClaim(msg.sender, rewardInAlmanackToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        // Calculate prizes in ALMANACK for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].almanackPerBracket[j] =
                ...
                // A. No ALMANACK to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].almanackPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInAlmanack - amountToShareToWinners);

        // Transfer ALMANACK to treasury address
        almanackToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        almanackToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInAlmanack += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInAlmanack >= minPriceTicketInAlmanack) && (_priceTicketInAlmanack <= maxPriceTicketInAlmanack),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInAlmanack: _priceTicketInAlmanack,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            almanackPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInAlmanack: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(almanackToken), "Cannot be ALMANACK token");
    ...
    /**
     * @notice Set ALMANACK price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInAlmanack: minimum price of a ticket in ALMANACK
     * @param _maxPriceTicketInAlmanack: maximum price of a ticket in ALMANACK
     */
    function setMinAndMaxTicketPriceInAlmanack(uint256 _minPriceTicketInAlmanack, uint256 _maxPriceTicketInAlmanack)
        external
        onlyOwner
    {
        require(_minPriceTicketInAlmanack <= _maxPriceTicketInAlmanack, "minPrice must be < maxPrice");

        minPriceTicketInAlmanack = _minPriceTicketInAlmanack;
        maxPriceTicketInAlmanack = _maxPriceTicketInAlmanack;
    }
     ...
     * @param _priceTicket price of a ticket (in ALMANACK)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].almanackPerBracket[_bracket];
        } else {
            return 0;
        }
    }
