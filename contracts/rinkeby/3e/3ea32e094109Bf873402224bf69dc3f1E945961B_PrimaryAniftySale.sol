...
contract PrimaryAniftySale is ERC1155Holder {
    ...
    modifier onlyWhitelisted(address _artist) {
        require(
            admin == msg.sender ||
                owners[msg.sender] ||
    ...
    modifier onlyArtistOrAuthority(address _artist) {
        require(
            _artist == msg.sender || admin == msg.sender || owners[msg.sender],
            "PrimaryMP: !AritistOrAuth"
        );
        _;
    }

    modifier onlyLister(address _lister) {
        require(_lister == msg.sender, "PrimaryMP: !lister");
        _;
    }
        ...
        returns (Sale[] memory)
    {
        ...
        for (uint256 i = 0; i < _saleIds.length; i++) {
            Sale memory sale = Sale(
                sales[_saleIds[i]].lister,
                sales[_saleIds[i]].artist,
    ...
    function mintBatchAndList(
        ...
        uint256[] memory saleIds = new uint256[](_tokenIds.length);
    ...
    ) external onlyWhitelisted(_artist) {
        require(_price > 0, "PrimaryMP: Price > 0");
        require(_amount > 0, "PrimaryMP: Amount > 0");
    ...
    function cancelSale(uint256 _saleId)
        external
        onlyArtistOrAuthority(sales[_saleId].artist)
    {
        require(
            sales[_saleId].endTimestamp == 0,
            "PrimaryMP: Sale ended/cancelled"
        );
    ...
    function buyFromSale(uint256 _saleId, uint256 _amount) external payable {
        ...
        require(sale.artist != address(0), "PrimaryMP: !sale");
        require(_amount > 0, "PrimaryMP: Amount > 0");
        require(sale.endTimestamp == 0, "PrimaryMP: Sale ended");
        require(_amount <= sale.amount, "PrimaryMP: !amount");
        ...
        if (sale.token == address(0)) {
            ...
            if (artistInfo[sale.artist].hasDiscount) {
                commissionAmount = commissionAmount.sub(
                    paymentAmount.mul(discount).div(PRECISION)
                );
            }
            // Transfer artist the price - commission fee
            sale.artist.transfer(paymentAmount.sub(commissionAmount));
            ...
            token.transferFrom(
                msg.sender,
                sale.artist,
                paymentAmount.sub(commissionAmount)
            );
            // Transfer commission wallet the commission fee
    ...
    function setIsArtist(address[] memory _artists, bool _set)
        external
        onlyAdminOrOwner
    {
    ...
    function setHasDiscount(address[] memory _artists, bool _set)
        external
        onlyAdminOrOwner
    {
        for (uint256 i = 0; i < _artists.length; i++) {
