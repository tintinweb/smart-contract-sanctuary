...
contract PrivateVestingMock is Ownable {
        ...
        returns (uint256 lockedBalance, uint256 unlockedBalance)
    {
        ...
        return (deposited[_addr].sub(unlock).sub(rewardsPaid[_addr]), unlock);
    }
    ...
    function setWhitelist(
        ...
        for (uint256 index = 0; index < _addr.length; index++) {
            address user = _addr[index];
            if (deposited[user] > 0) {
                _totalDeposited = _totalDeposited.sub(deposited[user]);
            } else {
                _totalDeposited = _totalDeposited.add(_tokenAmount[index]);
            }
            deposited[user] = _tokenAmount[index];
            require(
    ...
    function burnUnsoldToken() external onlyOwner {
        ...
        _rewardToken.burn(_totalSupply.sub(_totalDeposited));
        isBurned = true;
    }
