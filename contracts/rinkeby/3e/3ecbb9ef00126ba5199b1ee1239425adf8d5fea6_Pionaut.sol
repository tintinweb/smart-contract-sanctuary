...
contract Pionaut is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function withdraw() public onlyOwner {
        ...
        payable(royaltyWallet1).transfer((balance * royaltyFeePercent1) / 100);
        payable(royaltyWallet2).transfer((balance * royaltyFeePercent2) / 100);
    }
    ...
    function setPreSaleMaxSupply(uint256 newAmount) public onlyOwner {
        presaleMaxSupply = newAmount;
    }
    ...
    function mint(uint256 amount) public payable {
        ...
        if (!saleOpen) {
            ...
            require(presaleAmount + amount <= presaleMaxSupply, 'presale MaxSupply lmited');
        ...
        require(MintAmountPerPublicAddress[msg.sender] + amount <= publicSaleLimitPerAddress, 'only limited amount per transaction allowed');
        require(publicSaleAmount + amount <= publicMaxSupply, 'max public sale amount is limited.');
        for (uint256 i = 0; i < amount; i++) internalMint(msg.sender);
