...
contract INGREDIENTS is ERC1155, Ownable, ERC1155Burnable {
    ...
    string cURI = "http://j48baforms.io/ingredient/";
    uint256 tokenId = 0;
    uint256 limitAmount = 20000;
    uint256 private tokenPrice = 40000000000000000; // 0.04 ETH

    string name_ = "SUDOBURGER: INGREDIENTS";
    string symbol_ = "INGREDIENT";
    ...
    constructor() ERC1155("") {
        authorizedContract[0x64aFE07Bd1df9A4f439179D664825d44aF829bE3] = 721; // SUDOBURGER: FRYCOOKS
        authorizedContract[0x650831612494c8B6622A100edC0412C7A47d44CC] = 1155; // sudoPASS
    }
    ...
    function cookRedeem(address[] calldata contractIds, uint256[] calldata cookIds) public returns(uint256) {
        require(marketOpen == true, "Sales have not started");
        require(contractIds.length > 0, "No cook contract detected");
        ...
        for(uint256 i = 0; i < contractIds.length; i++) {
            ...
            if(authorizedContract[contractIds[i]] == 721) {
                ...
                require(contractAddress.balanceOf(msg.sender, cookIds[i]) > 0, "Doesn't own the cook");
            } else {
        ...
        uint256 toMint = cookIds.length.mul(3);
        for (uint256 i = 0; i < toMint ; i++) {
            tokenId++;
            require(prevTokenId <= limitAmount, "Limit reached");
            _mint(msg.sender, tokenId, 1, "");
        }
        return prevTokenId;
    }

    //the only way to get burgers without a FRYCOOK is through the dumpster :(
    function dumpsterDive(uint256 trashAmmt) public payable {
        require(marketOpen == true, "Sales have not started");
        require(msg.value >= tokenPrice.mul(trashAmmt), "Not enough money");
        uint256 prevTokenId = tokenId;
    ...
    //ownerBalance replicating the 721 style
    ...
    //returns all tokens of owner, 721 style
    ...
    //airdrop ingredients to users
    function airdrop(address[] calldata to) public onlyOwner {
        uint256 prevTokenId = tokenId;
        for(uint256 i = 0; i < to.length; i++) {
            tokenId++;
            require(prevTokenId <= limitAmount, "All ingredients have been minted.");
            _mint(to[i], tokenId, 1, "");
        }
    }
    // cook your SUDOBURGER INGREDIENTS into a custom SUDOBURGER! Truly delectable
    function grillBurger(uint256[] calldata id) public returns(uint256){
        require(kitchenOpen == true, "The kitchen isn't open right now.");
        require(id.length >= 3, "All burgers must be made of a minimum 3 ingredients.");
    ...
    // Check if token has been redeemed for an INGREDIENT
    ...
    //set dynamic URI root // will be changed to a ipfs hash once all ingredients are minted
