...
contract Cyberchads is ERC721, Ownable, AccessControl {
    ...
    uint256 public presaleTokensMinted; // Tokens minted in presale
    ...
    uint256 public constant MAX_CHAD_SUPPLY_PRESALE = 5453; // Max chad supply in presale
    ...
    */
    function getChadMaxAmount() public view returns (uint256) {
        ...
        require(presaleTokensMinted < MAX_CHAD_SUPPLY_PRESALE, "Sale has already ended, no more Chads left to sell.");

        if (presaleTokensMinted >= 1220) {
            return 20; // After 1220, 20 Chads/tx.
        } else if (presaleTokensMinted >= 233) {
    ...
    */
    function mintChads(uint256 numberOfChads) public payable {
        // Error handling.
        require(presaleTokensMinted < MAX_CHAD_SUPPLY_PRESALE, "Sale has already ended.");
        ...
        require(SafeMath.add(presaleTokensMinted, numberOfChads) <= MAX_CHAD_SUPPLY_PRESALE, "Exceeds maximum Chad supply. Please try to mint less Chads.");
        ...
        for (uint i = 0; i < numberOfChads; i++) {
            presaleTokensMinted = presaleTokensMinted + 1;
            uint256 newItemId = totalSupply();
