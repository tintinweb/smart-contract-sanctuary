...
contract Mizuki is ERC721, ERC721Enumerable, VRFConsumerBase, Ownable {
    ...
    function generateMetadata(uint64[maxSupply] memory traits)
        public
        onlyOwner
    {
        require(randomNumber > 0, "Random number is not ready yet");

        uint256[] memory expandedRandomNumbers = expandRandomNumber(maxSupply);

        // Again shuffle metadata array
        for (uint256 i = 0; i < maxSupply; i++) {
            uint256 n = i + (expandedRandomNumbers[i] % (maxSupply - i));
            uint64 temp = traits[n];
            traits[n] = metadata[i];
            metadata[i] = temp;
        }
    }
