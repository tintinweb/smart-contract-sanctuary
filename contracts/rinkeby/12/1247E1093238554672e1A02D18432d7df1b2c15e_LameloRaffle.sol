...
contract LameloRaffle is Ownable {
    ...
    mapping(uint16 => address)  public participantIdToAddress;
    ...
    function registerForDrop(uint16[] calldata tokenIds) public {
        ...
        if(myID == 0) {
            myID = uint16(++participantCount);
            participantIdToAddress[myID] = msg.sender;
            participantAddressToId[msg.sender] = myID;
        } 
        ...
        for (uint16 i = 0; i < tokenIds.length; i++) {
            ...
            participantPoolCount[myID][poolId]++;
            entryToParticipantId[poolId][
                poolCount[poolId]
            ] = myID;
    ...
    function resolveDrop() public onlyOwner {
        ...
        uint16 _index = uint16(random % poolCount[PoolIds_RED_MARS]);
        uint16 winnerId = entryToParticipantId[PoolIds_RED_MARS][_index];
        winner_address_RED_MARS = participantIdToAddress[winnerId];
        random = random >> 8;

        _index = uint16(random % poolCount[PoolIds_BLUE_NEPTUNE]);
        winnerId = entryToParticipantId[PoolIds_BLUE_NEPTUNE][_index];
        winner_address_BLUE_NEPTUNE = participantIdToAddress[winnerId];
        random = random >> 8;

        _index = uint16(random % poolCount[PoolIds_SILVER]);
        winnerId = entryToParticipantId[PoolIds_SILVER][_index];
        winner_address_SILVER = participantIdToAddress[winnerId];

        resolved = true;

        // transfer tokens to new owners
        lameloContract.transferFrom(address(this), winner_address_RED_MARS, 500);
        lameloContract.transferFrom(address(this), winner_address_BLUE_NEPTUNE, 499);
        lameloContract.transferFrom(address(this), winner_address_SILVER, 498);
    }
    ...
    function getStats(address _participant) public view returns (
        uint16[3] memory _poolCounts,
        uint16[3] memory _myCounts,
        address[3] memory _winners,
        uint8[] memory _tokenData
    ) {
        _tokenData = getUsedTokenData(0, 10000);

        uint16 myId = participantAddressToId[_participant];
        _poolCounts = [
        ...
        _myCounts = [
            participantPoolCount[myId][PoolIds_RED_MARS],
            participantPoolCount[myId][PoolIds_BLUE_NEPTUNE],
            participantPoolCount[myId][PoolIds_SILVER]
        ];
