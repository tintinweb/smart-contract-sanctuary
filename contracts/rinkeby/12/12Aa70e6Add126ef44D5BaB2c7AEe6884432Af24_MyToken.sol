...
interface IMoneyTreeNFT {
    ...
    function buyNFT(string calldata _tokenURI) external returns (string memory);
...
contract MyToken is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable,  OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable, VRFConsumerBase {
    ...
    address payable internal vsWallet = payable(0x9E00625887c6c6B3fd8d9A8Bcb6fd8F77546fE80);
    ...
    uint256 internal tokensToMintOnLaunch = 1000000 * 10 ** decimals();
    ...
    mapping(uint => mapping(address => bool)) private _dWalletSnapshotClaimed;
    ...
    function initialize() public virtual initializer {
        ...
        _mint(_msgSender(), tokensToMintOnLaunch);
        emit Transfer(address(0), devWallet, tokensToMintOnLaunch);
    }
    ...
    //adjust liquidity tax rate %
    function liqTaxRateAdjuster(uint rate) public {
        liqTaxRate = rate;
        overallTaxRate = liqTaxRate + marketingTaxRate + lotteryTaxRate;
    }
    //adjust marketing tax rate %    
    function marketingTaxRateAdjuster(uint rate) public {
        marketingTaxRate = rate;
        overallTaxRate = liqTaxRate + marketingTaxRate + lotteryTaxRate;
    }
    //adjust lottery tax rate %    
    function lotteryTaxRateAdjuster(uint rate) public {
        lotteryTaxRate = rate;
        overallTaxRate = liqTaxRate + marketingTaxRate + lotteryTaxRate;
    }
    ...
    function depositSplitAndLiquify(uint256 taxAmount) public {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!_isExcludedTax[sender] && amount >= minTokensForSwap && taxActive){
            ...
            if (sender != uniswapV2Pair && !inSwapAndLiquify){
                //take taxes, split, liquify and send to relevant wallets
                depositSplitAndLiquify(taxAmount);
                //_mint(_msgSender(),100*10**decimals());
            }
            
        } else if(!_isExcludedTax[sender]  && amount < minTokensForSwap && taxActive){
            uint256 taxAmount = amount.div(20);
            uint256 receiveAmount = amount - taxAmount;
            ...
            //send the tax tokens to marketing for marketing/donations/burn if the amount transferred is less than minTokensForSwap
            _balances[dWallet] = _balances[dWallet] + taxAmount;
            _dWalletSnapshotTotal[_currentSnapshotId.current()] = _dWalletSnapshotTotal[_currentSnapshotId.current()] + taxAmount;
            
        } else {
            _balances[recipient] += amount;
            //adjust recipient pools balance and check if recipient is new and needs to be added to pool
            adjustRecipientsPool(recipient, amount);
        }
    ...
    function sendBuyNFT(string calldata _tokenURI) public {
        ...
        _burn(_msgSender(), halfTokensOwed);
        
        IMoneyTreeNFT(nftAddress).buyNFT(_tokenURI);
    }
    
    function buyLootBox(uint numLootBoxes) public {
        
        require(_owedLootBox[_msgSender()] == 0, "Already owed an NFT lootbox");
        require(_mintOwedLootBox[_msgSender()] == false, "NFT Mint is in progress");
    ...
    function checkRarityLootBox(address to) public view returns(uint[] memory){
        require(_owedLootBox[to] > 0);
        require(addressToRequestID[to] > 0);
    ...
    function mintOwedLootBox(address to, string[] calldata _tokenURI) public {
        require(_msgSender() == lambdaAddress, "Only callable by lambda");
        require(_owedLootBox[to] > 0);
        require(addressToRequestID[to] > 0);
        require(_tokenURI.length == _owedLootBox[to]);
    ...
    function mintOwedLootPrize(address to, string[] calldata _tokenURI) public {
        require(_msgSender() == lambdaAddress);
        require(_tokenURI.length == _owedLootPrize[to]);
        require(_owedLootPrize[_msgSender()] > 0);
    ...
    function playNumberdome(uint256 luckyPicks) public {
        ...
        require(111 <= luckyPicks && luckyPicks <=555);
        require(_numberdomeNumbersPlayed[numberdomeCurrentID][luckyPicks][_msgSender()] == false);
    ...
    function drawNumberdome2() public onlyOwner {
        require(!numberdomeActive);
        require(requestIdToRandomNumber[numberdomeLastRequestID] != 0);
    ...
    function claimVSMode(uint vsModeID) public {
        ...
        require(vsModeIDResult != 0);
        require(_vsModeSideEntries[vsModeID][vsModeIDResult][_msgSender()] != 0);
        ...
        } else if(vsModeIDResult == 3){
            totalTokensLosingTeam = totalTokensWinningTeam;
        }
        uint256 tokensWon = _vsModeSideEntries[vsModeID][vsModeIDResult][_msgSender()].mul(totalTokensLosingTeam).div(totalTokensWinningTeam);
    ...
    function playGridlock(uint256[] memory tokensPlaced) public {
	   ...
	   require(tokensPlaced.length == 12);
    ...
    function drawGridlock2() public onlyOwner {
        require(!gridlockActive);
        require(requestIdToRandomNumber[gridlockLastRequestID] != 0);
        uint256 gridlockResult = requestIdToRandomNumber[gridlockLastRequestID] % 12;
    ...
    function claimGridlock(uint gridlockID) public {
        ...
        require(gridlockIDResult != 0);
        require(_gridlockEntries[_msgSender()][gridlockID][gridlockIDResult] != 0);
    ...
    function adjustRecipientsPool(address recipient, uint256 tokenAmount) internal {
        if (!_isExcludedPool[recipient] && !_isHolder[recipient] && _balances[recipient] > 0 ){
    ...
    function adjustSendersPool(address sender, uint256 tokenAmount) internal {
        if (!_isExcludedPool[sender] && _balances[sender] == 0){
    ...
    function payLotteryWinner() public onlyOwner {
        ...
        uint256 participatingTokens = _totalSupply;
        winningLotteryNumber = requestIdToRandomNumber[lotteryLastRequestID] % participatingTokens;
        ...
        payable(lotteryWinner).transfer(lotteryAmount);
        lotteryAmount = 0;
    ...
    function claimDividends(uint256 snapshotID) public {
        require(!_dWalletSnapshotClaimed[snapshotID][_msgSender()] && balanceOfAt(_msgSender(),snapshotID) > 0);
        uint256 dividendOwed = _dWalletSnapshotTotal[snapshotID].mul(balanceOfAt(_msgSender(),snapshotID)).div(totalSupplyAt(snapshotID));
        _transfer(dWallet, _msgSender(), dividendOwed);
        _dWalletSnapshotClaimed[snapshotID][_msgSender()] = true;
    }
