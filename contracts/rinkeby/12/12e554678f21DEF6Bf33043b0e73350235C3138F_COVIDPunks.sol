...
contract COVIDPunks is Ownable, ERC721Enumerable, ReentrancyGuard {
  ...
  string public imageHash; // Variable for the hash of the image

  bool public isSaleOn = false; // Boolean variable for sale

  bool public saleHasBeenStarted = false; // Boolean variable is sale has started

  uint256 public constant MAX_MINTABLE_AT_ONCE = 20; // Maximum amount to be minted by one wallet

  uint256 private _price = 0.03 ether; // 30000000000000000 - Variable for price of each NFT
  
  string public punkcontractURI; // Variable for URI
  
  constructor() ERC721("COVIDPunks", "PUNK-19") {} // Define token name and symbol

  
  
  // Members wallets
  address member1 = 0x15776c1F16C3B766A8a4af06DdB83dAD1512C9E9;
  address member2 = 0xD9Eb7Ddd1d5f9081BA745dcf0e56B39788b91f94;
  
  
  
  // Members percentages
  uint256 constant m1_per = 50;
  uint256 constant m2_per = 50;
  
  
  
  // Define max supply, remaining tokens and free rolls given
  ...
  // To a certain address assigns a number of free rolls
...
// Minting related functions
  ...
  // For an address to mint for free
  ...
  // Assign number of free rolls for an address
  ...
  // Chooses IDs for tokens to mint
...
// Show some units-and-global-variables
...
// Base URI functions
  ...
  string private _baseTokenURI; // Private variable fot base token URI
...
// onlyOwner functions
  ...
  // Max number of free mints we can give
  ...
  // Withdraw funds from contract. Stablish percentage for all members. _memberx is the amount of ETH for each member
    function withdrawTeam() public onlyOwner {
    //uint256 _each = address(this).balance / 4;
    // uint256 _sixp = .06;
    uint256 _balance = address(this).balance;
    uint256 _member1 = _balance / 100 * m1_per;
    uint256 _member2 = _balance / 100 * m2_per;
    require(payable(member1).send(_member1));
    require(payable(member2).send(_member2));
  }
...
// Whitelist stuff
  ...
  // For a whitelisted address to mint
  function whitelistMint(uint256 _numToMint) public payable nonReentrant() {
      require(whitelist[msg.sender] = true, "You haven't been whitelisted");
      
      uint256 totalSupply = totalSupply();
      
      require(totalSupply + _numToMint <= numTotalPunks(),"There aren't this many punks left.");
      
      uint256 costForMintingPunks = _price * _numToMint;
      
      require(msg.value >= costForMintingPunks, "Too little sent, please send more eth.");
