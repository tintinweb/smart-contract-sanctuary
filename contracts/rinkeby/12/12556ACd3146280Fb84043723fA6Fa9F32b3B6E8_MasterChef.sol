...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 rewards;
        //
        // We do some fancy math here. Basically, any point in time, the amount of STANDs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accStandPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accStandPerShare` (and `lastRewardBlock`) gets updated.
    ...
    IMigrationFarm public migrator;

    // The STAND TOKEN!
    TokenStand public stand;
    // xSTAND token!
    xStand public xSTAND;
    // Dev address.
    ...
    uint256 private constant ACC_STAND_PRECISION = 1e12;
    ...
    function setMigrator(IMigrationFarm _migrator) public onlyOwner {
        migrator = _migrator;
    }
        ...
        returns (uint256 pending)
    {
        ...
        if (block.number > farm.lastRewardBlock && lpSupply != 0) {
            ...
            accStandPerShare = accStandPerShare.add(
                standReward.mul(ACC_STAND_PRECISION).div(lpSupply)
            );
        }
        pending = user
        .amount
        .mul(accStandPerShare)
        .div(ACC_STAND_PRECISION)
        .sub(user.rewardDebt);
    }
        ...
        if (account == address(0)) {
            ...
            if (block.number > farm.lastRewardBlock) {
                ...
                if (lpSupply > 0) {
                    ...
                    stand.mint(address(xSTAND), standReward);
    ...
    function stake(
        ...
        if (user.amount > 0) {
            uint256 pendingStand = user.amount.mul(farm.accStandPerShare).div(ACC_STAND_PRECISION).sub(user.rewardDebt);
            uint256 pendingGift = user.rewards;
            if (pendingStand > 0) {
                safeStandTransfer(msg.sender, pendingStand);
            }
            if (pendingGift > 0) {
                user.rewards = 0;
                farm.gift.safeTransfer(msg.sender, pendingGift);
            }
        }
        if (amount > 0) {
            lpToken[fid].safeTransferFrom(msg.sender, address(this), amount);

            // Effects
            user.amount = user.amount.add(amount);
            user.rewardDebt = user.rewardDebt.add(
                amount.mul(farm.accStandPerShare).div(ACC_STAND_PRECISION)
            );
        }
        emit Staked(msg.sender, fid, amount);
    }
    ...
    function withdraw(
        ...
        FarmInfo memory farm = farmInfo[fid];
        UserInfo storage user = userInfo[fid][msg.sender];
        require(user.amount >= amount, "TokenStand Farming: amount not enough to withdraw");
        ...
        if (amount > 0) {
            ...
            lpToken[fid].safeTransfer(msg.sender, amount);
        }
        user.rewardDebt = user.amount.mul(farm.accStandPerShare).div(ACC_STAND_PRECISION);
        emit Withdrawn(msg.sender, fid, amount, pendingStand, pendingGift);
    }

    /// @notice Harvest proceeds for transaction sender to msg.sender.
    /// @param fid The index of the farm. See `farmInfo`.
    function harvest(uint256 fid) public {
        FarmInfo memory farm = farmInfo[fid];
        UserInfo storage user = userInfo[fid][msg.sender];

        updateFarm(fid, msg.sender);

        uint256 accumulateStand = user.amount.mul(farm.accStandPerShare).div(
            ACC_STAND_PRECISION
        );
        uint256 pendingStand = accumulateStand.sub(user.rewardDebt);
        uint256 pendingGift = user.rewards;

        // Effects
        user.rewardDebt = accumulateStand;
        ...
        emit Harvest(msg.sender, fid, pendingStand, pendingGift);
    }
    ...
    function emergencyWithdraw(uint256 fid, address to) public {
        UserInfo storage user = userInfo[fid][msg.sender];
        ...
        lpToken[fid].safeTransfer(to, amount);
    ...
    function safeStandTransfer(address _to, uint256 _amount) internal {
        xSTAND.safeStandTransfer(_to, _amount);
    }
