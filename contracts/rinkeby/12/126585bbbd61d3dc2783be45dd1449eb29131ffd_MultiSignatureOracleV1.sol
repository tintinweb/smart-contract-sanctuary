 ...
 */
contract MultiSignatureOracleV1 is AccessControl, Initializable, OwnableUpgradeable {
    ...
    bytes32 constant TXTYPE_HASH_ERC20 = keccak256("ReleaseRequest(address receiverAddress,address sourceNetworkTokenAddress,uint256 amount,uint256 depositChainId,uint256 depositNumber)");
    bytes32 constant TXTYPE_HASH_ERC721 = keccak256("ReleaseRequest(address receiverAddress,address sourceNetworkTokenAddress,uint256 tokenId,uint256 depositChainId,uint256 depositNumber)");
    ...
    function initialize() public initializer {
        ...
        //@TODO: should we initiate multiSignatureThreshold with a default value?
    }
    ...
    function setMultiSignatureThreshold(uint256 threshold) external {
        require(hasRole(MULTISIG_THRESHOLD_ROLE, msg.sender), "MultiSignatureOracle: must have MULTISIG_THRESHOLD_ROLE role to execute this method");
        require(threshold > 0, "MultiSignatureOracle: Invalid threshold value");
    ...
    function addMultiSignatureOracle(address oracleAddress) external {
        require(oracleAddress != address(0), "MultiSignatureOracle: Address of oracle is zero-address");
        // check if sender has sufficient permission
        require(hasRole(MANAGE_ORACLES_ROLE, msg.sender), "MultiSignatureOracle: must have MANAGE_ORACLES_ROLE role to execute this method");
        // add address to list of oracles
    ...
    function signaturesCheckERC20(
        ...
        require(sigV.length >= multiSignatureThreshold, "MultiSignatureOracle: Not enough signatures");
        require(sigR.length == sigS.length && sigR.length == sigV.length, "MultiSignatureOracle: Inconsistent signature input");
        ...
        bool verified = _verifySignatures(sigV, sigR, sigS, txInputHash, depositChainId, depositNumber);
        require(verified, "MultiSignatureOracle: Release not permitted.");
    ...
    function signaturesCheckERC721(
        ...
        // signatures verified - emit event  
        emit SignaturesCheckPassedERC721(tokenAddress, tokenId, receiverAddress, depositChainId, depositNumber);
        return verified;
    }
    ...
    function _verifySignatures(
        ...
        for (uint i = 0; i < sigV.length; i++) {
            ...
            require(recovered > lastAdd, "MultiSignatureOracle: Oracles not sorted");
     ...
     */
    function removePermittedOracleAddress(address oracleAddress) public {
        require(hasRole(MANAGE_ORACLES_ROLE, msg.sender), "MultiSignatureOracle: must have MANAGE_ORACLES_ROLE role to execute this method");
