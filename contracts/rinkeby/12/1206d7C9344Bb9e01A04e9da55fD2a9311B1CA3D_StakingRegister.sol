...
contract StakingRegister is IStakingRegister, IStakingFactory, Managed, EIP712 {
    using EnumerableSet for EnumerableSet.AddressSet;

    address public factorySigner;
    address public proxyOwner;
    address public managedOwner;
    address public genesisStakeImplementation;
    address public liquidityMiningImplementation;
    mapping(address => uint256) public nonces;

    uint256 internal _defaultFeePercentage;
    EnumerableSet.AddressSet internal _stakes;
    mapping(address => EnumerableSet.AddressSet) internal _ownerStakes;

    constructor(
        address management_,
        address genesisStakeImplementation_,
        address liquidityMiningImplementation_,
        uint256 defaultFeePercentage_
    ) Managed(management_) EIP712("StakingFarmingFactory", "v1") {
        genesisStakeImplementation = genesisStakeImplementation_;
        liquidityMiningImplementation = liquidityMiningImplementation_;
        require(
            defaultFeePercentage_ < MAX_FEE_PERCENTAGE,
            ERROR_INCORRECT_FEE
        );
        _defaultFeePercentage = defaultFeePercentage_;
        _setDependency();
    }
    ...
    function isOwner(address _owner, address _staking)
        external
        view
        override
        returns (bool)
    {
        return _ownerStakes[_owner].contains(_staking);
    }

    function isRegistered(address _stakingAddress)
        external
        view
        override
        returns (bool)
    {
        return _stakes.contains(_stakingAddress);
    }
    ...
    {
        uint256 to = (_offset + _limit) < _stakes.length()
            ? (_offset + _limit)
            : _stakes.length();
    ...
    function listByUser(
        ...
        EnumerableSet.AddressSet storage set = _ownerStakes[_user];
        uint256 to = (_offset + _limit) < set.length()
            ? (_offset + _limit)
            : set.length();
        result = new address[](to - _offset);
        for (uint256 i = _offset; i < to; i++) {
    ...
    {
        require(_amount < MAX_FEE_PERCENTAGE, ERROR_INCORRECT_FEE);
        _defaultFeePercentage = _amount;
    ...
    function setGenesisStakingImplementation(
        address _genesisStakeImplementation
    ) external override requirePermission(ROLE_ADMIN) {
        require(
            _genesisStakeImplementation != address(0),
            ERROR_WRONG_IMPLEMENT_ADDRESS
        );
        genesisStakeImplementation = _genesisStakeImplementation;
    }

    function setLiquidityMiningImplementation(
        address _liquidityMiningImplementation
    ) external override requirePermission(ROLE_ADMIN) {
        require(
            _liquidityMiningImplementation != address(0),
            ERROR_WRONG_IMPLEMENT_ADDRESS
        );
        liquidityMiningImplementation = _liquidityMiningImplementation;
    }
    ...
    function createGenesisStaking(
        ...
        bool isAdmin = _hasPermission(_msgSender(), ROLE_ADMIN);
        require(
            isAdmin || _hasPermission(_msgSender(), ROLE_REGULAR),
            ERROR_ACCESS_DENIED
        );
        if (!isAdmin) {
            bytes32 structHash = keccak256(
                abi.encode(
                    _GENESIS_CONTAINER_TYPEHASE,
            ...
            require(
                ECDSA.recover(_hashTypedDataV4(structHash), v_, r_, s_) ==
                    factorySigner,
                ERROR_INVALIG_SIGNER
            );
        }
        bytes memory bytesData = abi.encodeWithSignature(
            "initialize(address,string,bool,bool,bool,address,uint256,uint256,uint256)",
            address(management),
            name_,
            isETHStake_,
            isPrivate_,
        ...
        proxy = address(
            new TransparentUpgradeableProxy(
                genesisStakeImplementation,
                proxyOwner,
                bytesData
            )
        );
        Ownable(proxy).transferOwnership(managedOwner);
        _add(_msgSender(), proxy, false);
    }
    ...
    function createLiquidityMining(
        ...
        if (!isAdmin) {
            bytes32 structHash = keccak256(
                abi.encode(
                    _LIQUIDITY_MINING_CONTAINER_TYPEHASE,
                    keccak256(bytes(name_)),
                    isPrivate_,
                    isCanTakeReward_,
                    stakedToken_,
        ...
        bytes memory bytesData = abi.encodeWithSignature(
            "initialize(address,string,bool,bool,address,uint256,uint256,uint256)",
            address(management),
            name_,
            isPrivate_,
            isCanTakeReward_,
        ...
        proxy = address(
            new TransparentUpgradeableProxy(
                liquidityMiningImplementation,
                proxyOwner,
                bytesData
            )
        );
        Ownable(proxy).transferOwnership(managedOwner);
        _add(_msgSender(), proxy, true);
    }
    ...
    ) external override requirePermission(REGISTER_CAN_ADD_STAKING) {
        _add(_owner, _stakingAddress, _isLiquidityMining);
    }

    function remove(address _owner, address _stakingAddress)
        external
        override
        requirePermission(REGISTER_CAN_REMOVE_STAKING)
    {
        require(_stakes.remove(_stakingAddress), ERROR_NOT_FOUND);
        require(_ownerStakes[_owner].remove(_stakingAddress), ERROR_NOT_FOUND);
        emit RemoveStaking(_msgSender(), _owner, _stakingAddress);
    }
    ...
    function _add(
        ...
        require(_stakes.add(_stakingAddress), ERROR_IS_EXISTS);
        require(_ownerStakes[_owner].add(_stakingAddress), ERROR_IS_EXISTS);
        management.setPoolOwner(_stakingAddress, _owner, true);
        emit CreateStaking(
            _msgSender(),
            _owner,
            _stakingAddress,
    ...
    function _setDependency() internal virtual {
        factorySigner = management.contractRegistry(ADDRESS_FACTORY_SIGNER);
        proxyOwner = management.contractRegistry(ADDRESS_PROXY_OWNER);
        managedOwner = management.contractRegistry(ADDRESS_MANAGED_OWNER);
    }
...
contract Managed is Ownable {
    ...
    modifier canCallOnlyRegisteredContract(uint256 _key) {
        require(
            _msgSender() == management.contractRegistry(_key),
            ERROR_ACCESS_DENIED
        );
        _;
    }
