...
contract Token is Context, IERC20, Ownable {
    ...
    {
        (uint256 tTransferAmount, uint256 tMarketingAndBurn) = _getTValues(
            tAmount
        );

        return (tTransferAmount, tMarketingAndBurn);
    }
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 tTransferAmount = tAmount.sub(tMarketingAndBurn);
        return (tTransferAmount, tMarketingAndBurn);
    }
    ...
    function _takeMarketingAndBurnToken(
        ...
        if (_marketingFee + _burnFee > 0) {
            ...
            _balances[burnWallet] = _balances[burnWallet].add(tBurn);
    ...
    function _transferBothExcluded(
        ...
        (uint256 tTransferAmount, uint256 tMarketingAndBurn) = _getValues(
            tAmount
        );
