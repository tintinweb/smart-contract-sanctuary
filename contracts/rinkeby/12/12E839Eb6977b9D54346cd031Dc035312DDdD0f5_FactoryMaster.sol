...
import "../token/ERC20/ERC20Test.sol";
...
contract FactoryMaster {
    ERC20Test[] public childrenErc20;
    ...
    event ChildCreatedERC20(address childAddress,uint256 _initialSuplly , string name, string symbol);
    ...
    function createChild(
        ...
        if (types == Types.erc20) {
            ...
            ERC20Test child = new ERC20Test( _initialSuplly, name, symbol);
            childrenErc20.push(child);
            emit ChildCreatedERC20(address(child), _initialSuplly, name, symbol);
        }
        if (types == Types.erc20Burn) {
            ...
            ERC20Burn child = new ERC20Burn(totalSupply_, name, symbol);
            childrenErc20Burn.push(child);
            emit ChildCreatedERC20Burnable(address(child),totalSupply_, name, symbol);
        }
...
uint256 private initialSupply_;
     ...
     * construction.
     */
    constructor(uint256 _initialSuplly, string memory name_, string memory symbol_) {
        _name = name_;
        _symbol = symbol_;
        initialSupply_ = _initialSuplly;
    } 
    ...
    function initialSupply(uint256 totalSupply_) internal  {
...
contract ERC20Burn is ERC20Burnable {
    ...
    constructor(uint256 totalSupply, string memory name, string memory symbol) ERC20Standard(totalSupply, name, symbol) {
...
contract ERC20Mintable is ERC20 {
    ...
    constructor(uint256 cap_, string memory name, string memory symbol) 
    ERC20(name, symbol) {
        _name = name;
        _symbol = symbol;
        _cap = cap_;
...
contract ERC20MintableBurnable is ERC20Burnable {
    ...
    constructor(uint256 totalSupply, uint256 cap_,string memory name, string memory symbol) ERC20Standard(totalSupply, name, symbol) {
        _name = name;
        _symbol = symbol;
        _cap = cap_;
        _totalSupply = totalSupply;
  ...
  function totalSupply_() public view virtual  returns (uint256) {
        return _totalSupply;
    }
