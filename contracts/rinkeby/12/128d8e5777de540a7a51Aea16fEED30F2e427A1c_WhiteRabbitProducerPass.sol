...
contract WhiteRabbitProducerPass is ERC1155, ERC1155Supply, Ownable {
    ...
    function getEpisodeToProducerPass(uint256 episodeID) external view returns (ProducerPass memory) {
    ...
    function setProducerPass(
        ...
        external onlyOwner {
            episodeToProducerPass[episodeID] = ProducerPass(price, episodeID, totalSupply, episodeName);
    }

    function mintProducerPass(
        uint256 episodeID, 
        uint256 amount) 
        external payable {
        ProducerPass memory pass = episodeToProducerPass[episodeID];
        require(totalSupply(episodeID) < pass.maxSupply, "sold out");
        require(totalSupply(episodeID) + amount <= pass.maxSupply, "cannot mint that many");
    ...
    function getNumberOfProdocerPassAvailableForEpisode(
        uint256 episodeID) 
        public view 
        returns (uint256 numberOfPass) {
        require(exists(episodeID), "producer pass does not exist yet");
        ProducerPass memory pass = episodeToProducerPass[episodeID];
        require (pass.maxSupply > totalSupply(episodeID), "");

        return pass.maxSupply - totalSupply(episodeID);
    }
