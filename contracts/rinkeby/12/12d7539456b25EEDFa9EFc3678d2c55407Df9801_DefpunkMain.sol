...
contract DefpunkMain is IDefpunkMain, Ownable, ReentrancyGuard, Pausable {
    ...
    */
    function _mint(uint256 amount) internal {
        ...
        if(earlyAccessMintSale && whitelist[_msgSender()].mintClaimed == 0) { 
            require(amount - 1 * mintPrice == msg.value, "Invalid payment amount"); // Whitelisters get 1 free mint so the msg value has to be the amount - 1. Because they do not have to pay for it.
        } else {
    ...
    */
    function earlyAccessMint(uint256 _amount, bytes memory signature) external payable whenNotPaused {
        ...
        if(whitelist[_msgSender()].mintClaimed == 0) {
            whitelist[_msgSender()].mintClaimed = 1;
            _mint(_amount + 1);
        } else {
            _mint(_amount);
        }
