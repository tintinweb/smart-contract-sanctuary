...
contract LaunchPad6 is ERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        ...
        if(isExcluded[msg.sender]) {
            ...
            if(!isExcluded[sender] && !isExcluded[recipient] && (sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair))) {
                ...
                if(sender == address(_UniswapV2Pair) && block.number < startBlock + globalStart) {
                    require(isWhitelisted[recipient], "LPAD6: Not whitelisted");
                    require(block.number >= (startBlock += whitelistStartBlock[recipient]), "LPAD6: Whitelist is not activated for this block");
                    nextBuyBlock[recipient] = startBlock + globalStart;
                }
    ...
    function setGlobalStart(uint256 newValue) external onlyOwner {
