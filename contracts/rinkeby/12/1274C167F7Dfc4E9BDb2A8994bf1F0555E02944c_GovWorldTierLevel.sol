...
contract GovWorldTierLevel is IGovWorldTierLevel, Ownable {
    ...
    */
    ...
    {
        //TODO add max nft ttier check
        //check if user oowns one of the nft in nft tier data mapping
        for (uint256 i = 0; i < nftTierLevelsKeys.length; i++) {
            if (IERC721(nftTierLevelsKeys[i]).balanceOf(_wallet) > 0) {
                //user owns nft balannce
                return nftTierLevels[nftTierLevelsKeys[i]];
            }
        }
        return nftTierLevels[address(0)];
    }
    ...
    function isCreateLoanTokenUnderTier(
        ...
        if (tierData.govHoldings > 0) {
            ...
            if (
                _loanAmount >
                this.getMaxLoanAmount(_collateralInBorrowed, _wallet)
            ) {
                //allowed ltv
                return 3;
            }
            ...
            if (nftTier.isTraditional) {
                ...
                uint256 maxLoanAmount = (_collateralInBorrowed *
                    nftSpTier.ltv) / 100;
        ...
        if (tierData.govHoldings > 0) {
            ...
            if (nftTier.isTraditional) {
                ...
                for (uint256 x = 0; x < nftTier.allowedNfts.length; x++) {
                    if (_stakedCollateralTokens[0] == nftTier.allowedNfts[x]) {
                        //collateral can not be other then sp token
                        found = true;
                    }
                }
