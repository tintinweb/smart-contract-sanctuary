...
contract LuckyCatz     is  ERC721Enumerable 
{
    ...
    uint256 private     totalTokens        = 30;//30;//7777;
    uint256 private     totalMintedTokens  = 0;
    uint256 private     maxMintPerWallet   = 15;
    ...
    {
        address minterWallet = msg.sender;
        ...
        require(walletMintCounts[minterWallet] < maxMintPerWallet, "You cannot claim more tokens");

        require(availableTokens.length>0, "No tokens left to be claimed");

        _mint(minterWallet, getTokenToBeClaimed());
    ...
    {
        require(ownerWallet!=address(0), "Withdraw impossible to the Blackhole address");

        address  fromAddr = address(this);
        uint256  balance  = fromAddr.balance;
        
        require(balance>0, "The balance is empty. Cannot withdraw");
