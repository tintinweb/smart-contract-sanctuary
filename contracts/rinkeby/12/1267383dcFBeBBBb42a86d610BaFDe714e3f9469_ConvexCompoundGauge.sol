...
interface ICompoundBooster {
    ...
    function getBorrowRatePerBlock(uint256 _pid)
        ...
        view
        returns (uint256);
    function getExchangeRateStored(uint256 _pid) external view returns (uint256);
    function getBlocksPerYears(uint256 _pid,bool isSplit) external view returns(uint256);
    function getUtilizationRate(uint256 _pid) external view returns(uint256);
    function getCollateralFactorMantissa(uint256 _pid) external view returns(uint256);
}
...
contract ConvexCompoundGauge {
    ...
    function _borrow(uint256 _pid,PoolInfo storage pool,uint256 supportPid,int128 curveCoinId,uint256 _borrowNumber,uint256 _token0) internal {
        (address swapAddress, ,address lpToken, , ) = IConvexBooster(convexBooster).poolInfo(
            pool.convexPid
        );
        ...
        (uint256 lendingAmount,uint256 collateralAmount,uint256 interestValue) = getLendingAmount(
            supportPid, 
            token0Price,
    ...
    // function _updateUserLendings(uint256 _pid,address _sender,bytes16 lendingId,uint256 _token0,uint256 token0Price,uint256 lendingAmount,uint256 _supportPid,int128 _curveCoinId,uint256 interestValue,uint256 _borrowNumber,uint256 borrowIndex) internal {
    //     userLendings[msg.sender].push(
    //         UserLending({
    //             lendingId: lendingId,
    //             token0Value: _token0,
    //             token0Price: token0Price,
    //             lendingAmount: lendingAmount,
    //             supportPid: _supportPid,
    //             curveCoinId: _curveCoinId,
    //             interestValue: interestValue,
    //             borrowNumbers: _borrowNumber,
    //             startedBlock: block.number,
    //             state: UserLendingState.LENDING
    ...
    //         userLendingId: userLendings[_sender].length - 1
    //     });
    // }

    function _updateStatistics(uint256 _pid,uint256 token0Price,uint256 lendingAmount,uint256 _token0) internal {
    ...
    function getLendingAmount(uint256 _compoundPid, uint256 _token0Price, uint256 _lendingThreshold,uint256 _liquidateThreshold,uint256 _borrowBlocks) public view returns(uint256,uint256,uint256) {
        // 借款数 = 抵押品价值 * 0.85 / (1 + LF区块利率 * 借款规格)
        uint256 collateralFactorMantissa = ICompoundBooster(compoundBooster).getCollateralFactorMantissa(_compoundPid);
        uint256 utilizationRate = ICompoundBooster(compoundBooster).getUtilizationRate(_compoundPid);
        uint256 lendingAmount =  _token0Price*(1000-_lendingThreshold-_liquidateThreshold)/(1e18+utilizationRate*_borrowBlocks)/1000;
        uint256 collateralAmount = lendingAmount / collateralFactorMantissa;
        uint256 interestValue = getBorrowInterestRate(utilizationRate, _compoundPid);


        return (lendingAmount,collateralAmount,interestValue.mul(_borrowBlocks));
    }

    function getBorrowInterestRate(uint256 _utilizationRate, uint256 _supportPid)
        public
        view
        returns (uint256)
    {
