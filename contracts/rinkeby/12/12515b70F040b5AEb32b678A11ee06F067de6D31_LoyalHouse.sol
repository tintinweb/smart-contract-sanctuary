...
/**
 * @title LoyalHouse contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract LoyalHouse is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256[] public luckyIds;
    address[] public luckyAddresses;
    uint256 public numForOneLucky = 50;

    address wallet1;
    address wallet2;

    event LuckyReward(uint256 tokenId, address receiver, uint256 amount);

    constructor(string memory name, string memory symbol, uint256 maxNftSupply, uint256 revealTime, uint256 _mintPrice, address _rewardToken) ERC721(name, symbol) {
        MAX_DOGE_SUPPLY = maxNftSupply;
        REVEAL_TIMESTAMP = revealTime;
        mintPrice = _mintPrice;
        maxToMint = 30;
        saleIsActive = false;
        wallet1 = 0x165C435985a44fb4369da64973730A11EA2031BE;
        wallet2 = 0x5d409661e9d3Aa80523a89bADAb548b709F74091;
    ...
    */
    function mintDoges(uint256 numberOfTokens) external payable {
        ...
        for(uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if ((mintIndex + 1) % numForOneLucky == 0 || (mintIndex + 1) == MAX_DOGE_SUPPLY) {
                uint256 id = getLuckyId(mintIndex);
                luckyIds.push(id);
                luckyAddresses.push(ownerOf(id));
                ...
                rewardToken.transfer(ownerOf(id), amount);
                emit LuckyReward(id, ownerOf(id), amount);
            }
        }
    ...
    function getLuckyId(uint256 endId) view internal returns (uint256) {
        uint256 id = uint256(blockhash(block.number - 1)) % (endId + 1);
        return id;
    }
    ...
    function withdraw() external{
        require(msg.sender == address(wallet1) || msg.sender == address(wallet2), "Not admin!");
