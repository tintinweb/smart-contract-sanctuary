...
interface ITreasury {
    ...
    function getReserveToken() external returns ( address );
    function getPrincipleToken() external returns ( address );
}
...
contract OlympusBondDepository is Ownable {
    ...
    function setBondTerms( 
        ...
        DAOShare = DAOShare_;
        liquidityPremium = liquidityPremium_;
        return true;
    }
    ...
    internal returns ( bool ) {
        ...
        bool isReserve = token_ == ITreasury( treasury ).getReserveToken();
        bool isPrinciple = token_ == ITreasury( treasury ).getPrincipleToken();
        
        uint value; // ohm that can be minted against asset
        uint premium; // to incentivize lp over reserve
        uint markdown; // markdown for lp from market to risk-free value
        if ( isReserve ) {
            value = amount_.div( 1e9 );
            premium = 1e4; // minimum value
            markdown = 1e9; // minimum value
        } else if ( isPrinciple ) { // adjust for LP token
            value = IBondCalculator( bondCalculator ).valuation( token_ , amount_ );
            premium = liquidityPremium;
            markdown = IBondCalculator( bondCalculator ).markdown( token_ );
        } else { return false; }

        uint payout = calculateBondInterest( value.mul( markdown ).div( 1e9 ) ).mul( premium ).div( 1e4 );
        ...
        if ( isReserve ) {
            ITreasury( treasury ).depositReserves( amount_, token_ );
        } else if ( isPrinciple ) {
            ITreasury( treasury ).depositPrinciple( amount_, token_ );
        }
        ...
        for ( uint i = firstIndex[ msg.sender ]; i < lastIndex[ msg.sender ]; i++ ) {
            ...
            if ( !info.vested ) {
                ...
                if ( percentVested >= 10000 ) { // if fully vested
                    ...
                    depositorInfo[ msg.sender ][ i ].vested = true;
                } else {
     ...
     */
    function calculateBondInterest( uint value_ ) public view returns ( uint _interestDue ) {
