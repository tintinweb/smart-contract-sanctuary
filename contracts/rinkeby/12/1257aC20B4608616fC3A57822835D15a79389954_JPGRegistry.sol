...
contract JPGRegistry is Ownable, SignedMessage {
    ...
    event TokensListedSubregistry(NFT[] tokens);
    event TokenListedForSale(NFT token, uint256 price, address owner);
    ...
    function listForSale(
        ...
        emit TokenListedForSale(token, price, address(msg.sender));
    }
    ...
    function getPrice(
        ...
        address curator,
        address owner
    ) public returns (ListingPrice memory) {
        ...
        uint256 artistTake = 0;
        address artist = address(0);

        if (supportsRoyalties(token.tokenContract)) {
            (address recipient, uint256 royalty, ) =
                IERC2981(token.tokenContract).royaltyInfo(token.tokenId, priceInternal.sellerTake, bytes("0"));
            artistTake = royalty;
            artist = recipient;
            totalPrice = totalPrice.add(royalty);
        }
    ...
    function buyFixedPrice(
        ...
        balances[price.royaltyAddress] += price.artistTake;
     ...
     * @param token An instance of an NFT struct
     */
    function addToSubregistry(NFT calldata token) public {
        ...
        if (mainRegistry[token.tokenContract][token.tokenId] || _addToMainRegistry(token)) {
            subRegistry[msg.sender].nfts[token.tokenContract][token.tokenId] = true;
            // TODO: event
        }
    }
    ...
    function supportsRoyalties(address _contract) internal view returns (bool) {
        return ERC165(_contract).supportsInterface(_INTERFACE_ID_ERC2981);
    }
