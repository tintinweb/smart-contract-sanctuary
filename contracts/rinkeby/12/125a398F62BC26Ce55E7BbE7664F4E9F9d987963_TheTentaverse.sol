...
contract TheTentaverse is Delegated, ERC721EnumerableB {
  ...
  function setWhitelist( address[] calldata wallets, uint[] calldata quantities ) external onlyDelegates {
    require(wallets.length == quantities.length, "Must provide equal qallets and quantities" );
    for( uint i; i < wallets.length; ++i ){
      _whitelist[ wallets[i] ] = quantities[i];
    }
  }
