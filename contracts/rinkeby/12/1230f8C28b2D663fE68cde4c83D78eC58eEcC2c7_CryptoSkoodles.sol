...
contract CryptoSkoodles is Ownable, ERC721Enumerable {
    ...
    function mint(uint256 num) public payable {
        ...
        require(msg.value >= mintPrice(num) * num, "Not enough");
    ...
    function mintPrice(uint256 amount) public view returns (uint256) {
        if (_freeMints >= amount) {
            return 0;
        } else {
            return (amount - _freeMints) * _price;
        }
    }
    ...
    function getFreeMints() public view returns (uint256) {
        return _freeMints;
    }

    function setFreeMints(uint256 _newFreeMints) public onlyOwner {
        _freeMints = _newFreeMints;
    }
