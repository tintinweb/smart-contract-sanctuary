...
////// lib/cl-contracts/contracts/src/v0.8/VRFRequestIDBase.sol
...
////// lib/cl-contracts/contracts/src/v0.8/interfaces/LinkTokenInterface.sol
/* pragma solidity ^0.8.0; */
...
////// lib/cl-contracts/contracts/src/v0.8/VRFConsumerBase.sol
/* pragma solidity ^0.8.0; */
...
////// lib/oz-contracts/contracts/token/ERC20/IERC20.sol
...
contract ThorGuards is Ownable, ERC721, ERC721Enumerable, VRFConsumerBase {
    using Strings for uint256;

    bytes32 internal immutable KEY_HASH;
    uint256 internal constant LINK_FEE = 2e18;
    uint256 public constant MINT_COST = 0.08 ether;
    uint256 public immutable AVAILABLE_SUPPLY;
    IERC20 public immutable LINK_TOKEN;
    uint256 public immutable RAFFLE_PRE_START_TIME;
    ...
    uint256 public preRaffleCount = 0;
    uint256 public ownerRaffleCount = 0;
    ...
    event PreRaffleEntered(address indexed user, uint256 entries);
    ...
    constructor(
        ...
        bytes32 _LINK_VRF_KEY_HASH,
        address _LINK_VRF_COORDINATOR_ADDRESS,
        address _LINK_TOKEN,
        uint256 _AVAILABLE_SUPPLY,
        uint256 _RAFFLE_PRE_START_TIME,
        uint256 _RAFFLE_START_TIME,
        uint256 _RAFFLE_END_TIME,
        uint256 _ownerRaffleCount
    )
        VRFConsumerBase(_LINK_VRF_COORDINATOR_ADDRESS, _LINK_TOKEN)
        ERC721(_NFT_NAME, _NFT_SYMBOL)
        Ownable()
    {
        KEY_HASH = _LINK_VRF_KEY_HASH;
        LINK_TOKEN = IERC20(_LINK_TOKEN);
        AVAILABLE_SUPPLY = _AVAILABLE_SUPPLY;
        RAFFLE_PRE_START_TIME = _RAFFLE_PRE_START_TIME;
        RAFFLE_START_TIME = _RAFFLE_START_TIME;
        RAFFLE_END_TIME = _RAFFLE_END_TIME;

        ownerRaffleCount = _ownerRaffleCount;
        shuffledCount = _ownerRaffleCount;
        preRaffleCount += _ownerRaffleCount;
        for (uint256 i = 0; i < _ownerRaffleCount; i++) {
    ...
    function entriesList(uint256 page, uint256 pageSize)
        external
        view
        returns (address[] memory)
    {
        ...
        for (
            ...
            i < (page + 1) * pageSize && i < raffleEntries.length;
            i++
        ) {
            list[i - (page * pageSize)] = raffleEntries[i];
        }
        return list;
    }

    function raffleEntriesLength() public view returns (uint256) {
    ...
    function enterPreRaffle(uint256 numTickets) external payable {
        require(
            block.timestamp >= RAFFLE_PRE_START_TIME,
            "Pre-Raffle not active"
        );
        require(block.timestamp < RAFFLE_START_TIME, "Pre-Raffle ended");
        require(
            whitelistPre[msg.sender] >= numTickets,
            "Max mints for address reached"
        );
        require(msg.value == numTickets * MINT_COST, "Incorrect payment");

        shuffledCount += numTickets;
        preRaffleCount += numTickets;
        whitelistPre[msg.sender] -= numTickets;
        entriesPerAddressPre[msg.sender] += numTickets;

        for (uint256 i = 0; i < numTickets; i++) {
            raffleEntries.push(msg.sender);
        }
        emit PreRaffleEntered(msg.sender, numTickets);
    }
    ...
    function enterRaffle(uint256 numTickets) external payable {
        ...
        require(
            whitelist[msg.sender] >= numTickets,
            "Max mints for address reached"
        );
        require(msg.value == numTickets * MINT_COST, "Incorrect payment");
    ...
    function claimRaffle(uint256[] calldata tickets, address to) external {
        require(block.timestamp >= RAFFLE_END_TIME, "Raffle has not ended");
        require(clearingEntropySet, "No entropy to claim raffle");
        require(
            (raffleEntries.length < AVAILABLE_SUPPLY) ||
                (shuffledCount == AVAILABLE_SUPPLY),
        ...
        for (uint256 i = 0; i < tickets.length; i++) {
            ...
            require(
                raffleEntries[tickets[i]] == msg.sender,
    ...
    function setClearingEntropy() external returns (bytes32 requestId) {
        ...
        require(
            LINK_TOKEN.balanceOf(address(this)) >= LINK_FEE,
            "Insufficient LINK"
        );
    ...
    function withdrawRaffleProceeds() external onlyOwner {
        ...
        proceeds -= ownerRaffleCount * MINT_COST;

        bool sent = payable(msg.sender).send(proceeds);
