 ...
 */
contract TestNFTs is ERC721Tradable {
    using Address for address;
    ...
    uint256 public constant maxPerTx = 4;

    // TODO change
    /// @notice Max number of tokens available during first round
    uint256 public constant roundOneSupply = 5;

    // TODO change
    /// @notice Max number of tokens available during second round
    uint256 public constant roundTwoSupply = 10;
    ...
    ) ERC721Tradable("Test NFTs", "TSTNFTs", _proxyRegistryAddress) {
    ...
    function baseTokenURI() public pure override returns (string memory) {
        ...
        return "https://gateway.pinata.cloud/ipfs/QmYWu9ERaZKNcUZm8pQ8T8ESGxY47ESSkKTTbkyfGMkdFK/";
    }
    ...
    function turboBoost(uint256 amount) view internal returns (uint) {
     uint256 globalAmount = amount;
        if (tokenIdCount.current() < roundOneSupply){
            globalAmount = uint(globalAmount * 150 / 100);
        } 

        else if (tokenIdCount.current() < roundTwoSupply){
    ...
    function governorMintMultiple(address[] memory to) external onlyOwner {
