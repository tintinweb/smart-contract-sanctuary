...
contract WaifuWorld is ERC721Enumerable, Ownable {
    ...
    // Define if sale is active
    bool public isConsulationActive = false;
     ...
     * Mint DRNA
     */
    function mintDRNA(uint256 numberOfTokens) public payable {
        ...
        require(
            ...
            "Purchase would exceed max supply of DRNA"
        );
        require(
    ...
    function getConsulationToken() public 
    {
       require(isConsulationActive,"Consulation Not Active");
        ...
        for(uint256 i =0;i<userBalance;i++)
          {
           ...
           if(tokenId<=500)
           {
               uint256 consoluationTokenId = 3000+tokenId;
                   if(!_exists(consoluationTokenId)){
                        uint256 first_encounter = block.timestamp;
                        _safeMint(msg.sender, consoluationTokenId);
                        _waifuWorld[consoluationTokenId] = WaifuWorldDetail(first_encounter);
                        emit TokenMinted(consoluationTokenId, msg.sender, first_encounter); 
                    }
           }
