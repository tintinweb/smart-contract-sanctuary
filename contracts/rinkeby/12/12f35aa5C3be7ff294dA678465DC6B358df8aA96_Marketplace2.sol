...
contract Marketplace2 {
  ...
  modifier HasTransferApproval(address sellerAddress, address tokenAddress) {
    IERC721 tokenContract = IERC721(tokenAddress);
    require(tokenContract.isApprovedForAll(sellerAddress, tokenAddress));
    _;
  }
  ...
  // HasTransferApproval(msg.sender, tokenAddress)

  function addItemToMarket(
    address tokenAddress,
    uint256 tokenId,
    uint256 askingPrice
  ) external OnlyItemOwner(tokenAddress, tokenId) returns (uint256) {
    require(
  ...
  function buyItem(uint256 id)
    ...
    ItemExists(id)
    IsForSale(id)
    HasTransferApproval(itemsForSale[id].seller, itemsForSale[id].tokenAddress)
  {
