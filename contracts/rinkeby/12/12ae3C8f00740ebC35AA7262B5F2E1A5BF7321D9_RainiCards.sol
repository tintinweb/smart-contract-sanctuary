...
contract RainiCards is ERC1155, AccessControl {
  ...
  function initCards(uint256[] memory _cardId, uint16[] memory _maxSupply, string[] memory _pathUri, address[] memory _subContract, uint256[][] memory _tokenIds, bool[][] memory _isGrouped, uint256[][] memory _maxStamina)
    external onlyOwner() {
      ...
      for (uint256 i; i < _cardId.length; i++) {
        ...
        cardPathUri[_cardId[i]] = _pathUri[i];
        ...
        for (uint256 j = 0; j < _tokenIds[i].length; j++) {
          ...
          cardLevels[_cardId[i]].push(CardLevel({
            conversionRate: uint64(_tokenIds[i][j]),
            numberMinted: 0,
