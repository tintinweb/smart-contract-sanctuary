...
contract NamelessTokenData is INamelessTokenData, AccessControl, Initializable {
  ...
  function enableExtensions(uint256 newExtensions) public onlyRole(DEFAULT_ADMIN_ROLE) onlyUnsealed {
    ...
    if (newExtensions & TOKEN_REDEEMABLE_EXTENSION != 0) {
      initializeTokenRedeemableExtension();
    }
  }
  ...
  function initializeTokenRedeemableExtension() internal {
    uint256[1] storage storageMetadata;
    uint256 metadataSlot = TOKEN_REDEMPTION_EXTENSION_COUNT_SLOT;
    // solhint-disable-next-line no-inline-assembly
    assembly {
      storageMetadata.slot := metadataSlot
    }
  ...
  function redeem(uint256 tokenId) public onlyFrontend override {
    require(extensions & TOKEN_REDEEMABLE_EXTENSION != 0, 'Token is not redeemable' );

    uint256[65535] storage redemptionCount;
    uint256 redemptionCountSlot = TOKEN_REDEMPTION_EXTENSION_COUNT_SLOT + 1;
    ...
    redemptionCount[tokenId] = redemptionCount[tokenId] + 1;
  }
...

{
  ...
  "libraries": {
    "/home/bart/git/InfinityTokens-contract/contracts/nameless/NamelessMetadataURIV1.sol": {
      "NamelessMetadataURIV1": "0x1564c1E98d3c0134EB3A941b2b412737b1B29227"
    }
  },
  "outputSelection": {
    "*": {
