...
contract NameTag is ERC721, Ownable, StringUpper, DenyList, Random {
    ...
    function _buyTokens() internal returns(uint256[] memory) {
        uint256 tokensAmount = _getTokenAmount(msg.value);
        _preValidatePurchase(tokensAmount);
        return _processPurchase(msg.sender, tokensAmount);
    }

    function buyNamedTokens(string[] memory _names) external payable returns (uint256[] memory, bool[] memory) {
        uint256[] memory tokens = _buyTokens();
