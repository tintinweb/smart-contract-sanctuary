...
// Inspired/Copied from Chubbies
contract Ethersparks is ERC721, Ownable {
    using SafeMath for uint256;
    uint256 public constant MAX_ETHERSPARKS = 10;
    ...
    constructor() ERC721("Ethersparks", "ETHERSPARKS") {
        setBaseURI(
            "https://gateway.pinata.cloud/ipfs/QmRG9Gn8iKoP9eEypLEC1hAXKkYuvo1GkQnhxhym2V8t3Z/"
        );
    }
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        if (currentSupply == 9) {
            return 1000000000000000000; // 10: 1.00 ETH
        } else if (currentSupply >= 8) {
            return 640000000000000000; // 8-9:  0.64 ETH
        } else if (currentSupply >= 6) {
            return 320000000000000000; // 6-7:  0.32 ETH
        } else if (currentSupply >= 4) {
            return 160000000000000000; // 4-5:  0.16 ETH
        } else if (currentSupply >= 2) {
            return 80000000000000000; // 2-3:  0.08 ETH
        } else if (currentSupply >= 0) {
            return 40000000000000000; // 0-1:   0.04 ETH
        }
    }
    ...
    function adoptEthersparks(uint256 numEthersparks) public payable {
        require(totalSupply() < MAX_ETHERSPARKS, "Sale has already ended");
        require(
            numEthersparks > 0 && numEthersparks <= 2,
            "You can adopt minimum 1, maximum 2 Ethersparks"
        );
        require(
        ...
        for (uint256 i = 0; i < numEthersparks; i++) {
