...
contract RebaseToken is AddressPrivileges {
    ...
    constructor (string memory _name, string memory _symbol,address token) public{
    ...
    function newErc20(uint256 leftAmount) external onlyMinter {
  ...
  /**
    ...
    /**
     * @notice to update ratio
     * @dev new totalSupply replace old supply
     */
    function calRebaseRatio(uint256 newTotalSupply) public onlyMinter {
    ...
    /**
     * @dev Return the amount of tokens owned by `account`.
     */
    function balanceOf(address account) external view returns (uint256){
        Erc20Info storage info = Erc20InfoList[Erc20InfoList.length-1];
