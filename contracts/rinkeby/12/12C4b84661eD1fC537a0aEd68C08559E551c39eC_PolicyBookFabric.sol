...
contract PolicyBookFabric is IPolicyBookFabric, AbstractDependant {
    ...
    function create(
        ...
        policyBookRegistry.add(_insuranceContract, _contractType, address(_policyBookProxy));

        emit Created(_insuranceContract, _contractType, address(_policyBookProxy));
        ...
        if (_initialDeposit > 0) {
            ...
            // ShieldMiningModel shieldMiningModel = new ShieldMiningModel(policyBookRegistry);
...
/* is ShieldMiningOperator */
contract PolicyBookFacade {
    ...
    modifier updateShieldMiningReward() {
        _;
        address insuredContract = IPolicyBook(address(pbProxy)).insuranceContractAddress();
        // // address rewardToken = shieldMiningView.getDefaultRewardToken(insuredContract);
        // (, , uint256 rewardPerTokenFromStart, , , , uint256 rewardTokensOwedToUser) =
        //     shieldMiningView.getRewardData(insuredContract, rewardToken);

        // shieldMiningController.updateRewardPerToken(
        //     insuredContract,
        //     rewardToken,
        //     rewardPerTokenFromStart
        // );

        // shieldMiningController.setRewardTokensOwedToUser(
        //     insuredContract,
        //     rewardToken,
    ...
    ) external updateShieldMiningReward {
        IPolicyBook(address(pbProxy)).buyPolicyFor(_buyer, _epochsNumber, _coverTokens);
    }

    /// @notice Let user to buy policy by supplying stable coin, access: ANY
    /// @param _durationSeconds is number of seconds to cover
    /// @param _coverTokens is number of tokens to cover
    function buyPolicy(uint256 _durationSeconds, uint256 _coverTokens)
        external
        updateShieldMiningReward
    {
        IPolicyBook(address(pbProxy)).buyPolicy(_durationSeconds, _coverTokens);
    }

    /// @notice Let user to add liquidity by supplying stable coin, access: ANY
    /// @param _liquidityAmount is amount of stable coin tokens to secure
    function addLiquidity(uint256 _liquidityAmount) external updateShieldMiningReward {
        IPolicyBook(address(pbProxy)).addLiquidity(_liquidityAmount);
    }
    ...
    function addLiquidityFor(address _liquidityHolderAddr, uint256 _liquidityAmount)
        ...
        updateShieldMiningReward
    {
        IPolicyBook(address(pbProxy)).addLiquidityFor(_liquidityHolderAddr, _liquidityAmount);
    }
    ...
    function addLiquidityAndStake(uint256 _liquidityAmount, uint256 _stakeSTBLAmount)
        ...
        updateShieldMiningReward
    {
        IPolicyBook(address(pbProxy)).addLiquidityAndStake(_liquidityAmount, _stakeSTBLAmount);
    }

    /// @notice Let user to withdraw deposited liqiudity, access: ANY
    function withdrawLiquidity() external updateShieldMiningReward {
        IPolicyBook(address(pbProxy)).withdrawLiquidity();
    }
}
