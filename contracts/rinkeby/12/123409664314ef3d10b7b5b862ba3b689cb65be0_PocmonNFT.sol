...
// Part: OpenZeppelin/[email protected]/EnumerableMapUpgradeable
...
// Part: OpenZeppelin/[email protected]/IERC721EnumerableUpgradeable
...
abstract contract NFT721Mint is Initializable, AccountMigration, ERC721Upgradeable, NFT721Creator, NFT721Market, NFT721Metadata {
   ...
   */
  function _initializeNFT721Mint(MintRequirementsChoice _mintRequirements) internal initializer {
    // Use ID 1 for the first NFT tokenId
    mintRequirements = _mintRequirements;
    nextTokenId = 1;
  }

  function setMintRequirements(MintRequirementsChoice _mintRequirements) public onlyWhitelistOperator {
    require(_mintRequirements >= MintRequirementsChoice.Everybody && _mintRequirements <= MintRequirementsChoice.Nobody,
  ...
  function mintAndTransfer(string memory _tokenIPFSPath, address creator, bytes memory signature) public returns (uint256) {
    require(mintRequirements == MintRequirementsChoice.Everybody,
  ...
  function mint(string memory _tokenIPFSPath, uint256 timestamp, bytes memory signature) public returns (uint256) {
    require(mintRequirements == MintRequirementsChoice.Limited || mintRequirements == MintRequirementsChoice.OnlyOperator,
  ...
  function mint(string memory tokenIPFSPath) public returns (uint256 tokenId) {
    if(mintRequirements == MintRequirementsChoice.Limited) {
    ...
    } else if(mintRequirements == MintRequirementsChoice.OnlyOperator) {
  ...
  function _mint(address to, string memory tokenIPFSPath) internal returns (uint256 tokenId) {
    require(mintRequirements != MintRequirementsChoice.Nobody, "NFT721Mint: Minting is not permitted");
    tokenId = nextTokenId++;
    _mint(to, tokenId);
