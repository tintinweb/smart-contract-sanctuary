...
// xBMF
contract XBMFToken is ERC20("xBMF-TEST-3", "XBMF-TEST-3"), Ownable {
    ...
    uint256 constant private REWARD_PER_DAY_FOR_LEVEL_ONE = 13 ether;//check (curr 25%)
    uint256 constant private REWARD_PER_DAY_FOR_LEVEL_TWO = 17 ether;//check (curr 50%)
    uint256 constant private REWARD_PER_DAY_FOR_LEVEL_THREE = 25 ether;//check (curr 100%)
    uint256 constant private SECS_IN_DAY = 86400;
    uint256 constant private REWARD_DENOMINATOR = 1000000000000000000;
    
    address constant private BLOOTMF_ADDRESS = 0x0EF32052eB9349C360503894729c13715745FBD8;
    ...
    event RewardPaidForBlootMF(address indexed owner, uint256 reward, uint256 nftId);//todo emit this
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the assigned minter.
