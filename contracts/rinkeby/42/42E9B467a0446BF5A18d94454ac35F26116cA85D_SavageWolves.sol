...
contract SavageWolves is ERC721Enumerable, Ownable {
    ...
    mapping(address=>mapping(uint256=>bool)) isMinter;
    ...
    function mint(uint256 _mintAmount) public payable {
        require(isPresaleStart == true || isPublicStart == true, "Neither of the sale is start yet!");
    ...
    function calculateTokens(address _user) public view returns(uint256){
        ...
        for(uint8 i=0; i < walletOfOwner(_user).length; i++){
            if(isMinter[_user][walletOfOwner(_user)[i]] == true){
                if(_user == ownerOf(walletOfOwner(_user)[i])){
                    tokens+=((block.timestamp - userPerIdMintingTime[_user][walletOfOwner(_user)[i]]).div(1 minutes)).mul(5);
                }
            } 
        }
       return tokens;
    ...
    function calculateTokensId(address _user, uint256 _id) public view returns(uint256){
        ...
        if(_user == ownerOf(_id)){
            if(isMinter[_user][_id] == true){
            tokens+= ((block.timestamp - userPerIdMintingTime[_user][_id])/(1 minutes))*5;
            }
        }
