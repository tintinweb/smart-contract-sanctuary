...
contract blahtest3 is ERC721A, Ownable {
    ...
    constructor() ERC721A("blahtest3", "blahtest3", MAX_MINT_PER_TX) {}
    ...
    modifier isPaymentSufficient(uint256 quantity) {
        if (totalSupply() + quantity > 10 && totalSupply() < 10) {
            require(
                PRICE * (totalSupply() + quantity - 10) <= msg.value,
                "There was not enough/extra ETH transferred to mint an NFT."
            );
        } else if (totalSupply() + quantity > 10 && totalSupply() > 10) {
            require(
