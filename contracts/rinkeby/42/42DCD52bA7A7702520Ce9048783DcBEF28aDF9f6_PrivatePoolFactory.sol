...
contract PrivatePoolFactory is IPoolFactory, Managed {
    ...
    function create(
        ...
        IPoolRegister(management.contractRegistry(CONTRACT_POOL_REGISTER)).add(
            address(poolInstant),
            true
        );
    }
...
interface IPoolFactory {
    event CreatePool(address indexed sender, address pool, bool isPrivate);
...
interface IPoolRegister {
    ...
    function isPrivate(address _pool) external view returns (bool);
