...
contract CoolBabiesClub is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant MAX_MINT = 10; // Upper Limit is 10 in CrowdSale
    uint256 private constant MAX_MINT_PRESALE = 5; // Upper Limit is 5 in PreSale
    uint256 private constant MAX_MINT_PREMINT = 28; // Upper Limit per Mint is 28

    // First Stage
    uint256 private constant PRICE_PRESALE = 4 * 10**16; // 0.04ETH Per CBC
    uint256 private constant PRICE_PREMINT = 0; // Free Per CBC
    
    uint256 private constant MAX_ELEMENTS_PRESALE = 990; // 990 CBCs in PreSale
    uint256 private constant MAX_ELEMENTS_FIRST_PREMINT = 20; // 20 CBCs for First GiveAway.


    // Second Stage
    uint256 private constant PRICE = 7 * 10**16; // 0.07ETH Per CBC

    uint256 private constant MAX_ELEMENTS = 10000; // 10000 CBCs for Max Supply
    uint256 private constant MAX_ELEMENTS_PUBLICSALE = 8883; // 8883 CBCs in CrowdSale
    uint256 private constant MAX_ELEMENTS_SECOND_PREMINT = 107; // 107 CBCs for GiveAway.


    address private constant ownerAddress = 0xd7b01f6cD01D648F21D646f5334129fB8fED7097; // Wallet Address of Owner
    address public constant developerAddress = 0x09964eAB43f1bd45Ab98416Dd94d16C8b76AB5Aa; // Wallet Address of the Developer
    ...
    event CreateCoolBabiesClub(address to, uint256 indexed id);

    modifier saleIsOpen {
        if (_msgSender() != owner()) {
            require(SALE_OPEN == true, "SALES: Please wait a big longer before buying CBCs ;)");
        }
    ...
    constructor (string memory baseURI) ERC721("CoolBabiesClub", "CBC") {
        setBaseURI(baseURI);

        _price = PRICE_PREMINT;
        _maxElements = MAX_ELEMENTS_FIRST_PREMINT;
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        require(total <= _maxElements, "MINT: Please go to the Opensea to buy CBC.");
        require(_ids.length <= _maxMint, "MINT: Current count exceeds maximum mint count.");
        
         if (_to != owner()) {
            require(msg.value >= price(_ids.length), "MINT: Current value is below the sales price of CBC");
        }
    ...
    function _mintAnElement(address payable _to, uint256 _id) private {
        ...
        emit CreateCoolBabiesClub(_to, _id);
    }
    ...
    function startPreSale() public onlyOwner {
        ...
        _maxElements = MAX_ELEMENTS_PRESALE + MAX_ELEMENTS_FIRST_PREMINT;
        _maxMint = MAX_MINT_PRESALE;
    }

    function startSecondPremint() public onlyOwner {
        _isPresale = false;

        SALE_OPEN = true;

        _price = PRICE_PREMINT;
        _maxElements = MAX_ELEMENTS_PRESALE + MAX_ELEMENTS_FIRST_PREMINT + MAX_ELEMENTS_SECOND_PREMINT;
        _maxMint = MAX_MINT_PREMINT;
    }
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        _widthdraw(developerAddress, balance.mul(4).div(100));
