...
contract SomeTestNFT is ERC721, Ownable {
    ...
    string private _apiURI = "https://test-api-something2.herokuapp.com/token/";
    ...
    uint public presaleTokensPerMintLimit = 5; // Up to 5 AniCats per transaction during the presale.
    uint public presaleTokensPerWalletLimit = 5; // Up to 5 AniCats per unique wallet during the presale.
    ...
    IERC721 public friendContract;

    constructor() ERC721("SomeTestNFT", "STNFT") {
        mintNFTs(5); // auto mint to the team
    }
    ...
    function presaleMint(uint amount) external payable {
        ...
        require(amount > 0 && amount <= presaleTokensPerMintLimit, "Can only mint 20 tokens at a time");
        ...
        require(balanceOf(msg.sender) + amount <= presaleTokensPerWalletLimit, "Can only mint 10 tokens during the presale per wallet");
