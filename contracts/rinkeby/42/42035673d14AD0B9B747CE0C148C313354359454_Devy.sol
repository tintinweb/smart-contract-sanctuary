...
contract Devy is ERC20, EIP712, Ownable {
    ...
    // for team
    uint256 public constant AMOUNT_TEAM = INITIAL_MAX_SUPPLY / 100 * 15;
    address public constant ADDR_TEAM = 0x9c3154807Df6a66FEa8518580DF68d468ffc0155;

    // for advisors
    uint256 public constant AMOUNT_ADVISORS = INITIAL_MAX_SUPPLY / 100 * 5;
    address public constant ADDR_ADVISORS = 0x6c35BBAD5254765BbE7E3bce116531b3BDAC7927;

    // for free claim
    uint256 public constant AMOUNT_AIRDROP = INITIAL_MAX_SUPPLY - (AMOUNT_STAKING + AMOUNT_LP + AMOUNT_TEAM + AMOUNT_ADVISORS);
    ...
    ) ERC20(_name, _symbol, stakingFeeAddress_, LPFeeAddress_, DAOFeeAddess_, operationsFeeAddress_) EIP712("Devy", "1") {
        _mint(ADDR_STAKING, AMOUNT_STAKING);
        _mint(ADDR_LP, AMOUNT_LP);
        _mint(ADDR_TEAM, AMOUNT_TEAM);
        _mint(ADDR_ADVISORS, AMOUNT_ADVISORS);
    ...
    */
    function claim(uint256 amountV, bytes32 r, bytes32 s) external {
        require(claimLive, "Devy: claim is not live");

        uint256 amount = uint248(amountV);
        uint8 v = uint8(amountV >> 248);
        uint256 total = totalSupply() + amount;
        require(total <= maxSupply, "Devy: > max supply");
        require(!claimed[msg.sender], "Devy: Already claimed");
        require(signerValid(v, r, s, msg.sender, amount), "Devy: Invalid signer");
    ...
    */
    function decreaseMaxSupply(uint256 newSupply) external onlyOwner {
        require(newSupply >= totalSupply(), "Devy: < total supply");
        require(newSupply < maxSupply, "Devy: >= max supply");
...
/**
 * @dev Implementation of the {IERC20} interface with Devy fees 
