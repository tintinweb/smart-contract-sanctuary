...
// File contracts/RichPunksDNA.sol


pragma solidity ^0.8.0;

contract RichPunksDNA {
...
contract RichPunks is
    ERC721("RichPunks", "RICHP"),
    ERC721Enumerable,
    RichPunksDNA
{
    ...
    modifier limitableSupply {
        uint256 current = _idCounter.current();
        require(current <= maxSupply, "No RichPunks left");

        _;
    }

    function mint() public limitableSupply {
        uint256 current = _idCounter.current();
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory){
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "RichPunks #',
                tokenId.toString(),
                '", "description": "Dapp RichPunks by Richard Lozada",',
                image,
                '"}'
            )
        );
    ...
    function _paramsURI(uint256 _dna) internal view returns (string memory){
        ...
        return string(abi.encodePacked(params, "&skinColor=", getSkinColor(uint8(_dna))));
    }
    ...
    function imageByDNA(uint256 _dna) public view returns (string memory) {
        ...
        return string(abi.encodePacked('"image": "', baseURI, "?", paramsURI));
    }
