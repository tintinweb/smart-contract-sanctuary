...
contract SplitProxy is SplitStorage, IERC721Receiver {
    ...
    function mintNFT(
        ...
        bytes memory payload = abi.encode("mint(MediaData memory data, IMarket.BidShares memory bidShares)", data, bidShares);
        (bool sent, bytes memory data) = zoraMediaRinkeby.call(payload);
        require(sent, "Failed to mint NFT");
    }
    // function mintNFT(
    //     MediaData calldata data, 
    //     BidShares calldata bidShares
