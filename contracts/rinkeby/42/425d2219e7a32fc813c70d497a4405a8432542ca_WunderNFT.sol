...
contract WunderNFT is ERC721, VRFConsumerBase, AccessControl {
        ...
        ERC721("WunderPassNFT", "WPN")
    {
        ...
        patterns = ["Waves", "Grid", "Zigzag", "Lines", "Horizontal", "Wunderpass"];
    ...
    function mintForUser(string memory _edition, address _owner) public onlyRole(ADMIN_ROLE) returns(uint) {
    ...
    function mint(string memory _edition) public payable returns(uint) {
        require(msg.value == publicPrice);
        return mintForUser(_edition, msg.sender);
    }

    function determineStatus() internal view returns(string memory) {
    ...
    function determineWonder(uint randomNumber) internal view returns(string memory) {
        uint modNumber = (randomNumber % wonders.length);
        return wonders[modNumber];
    }

    function determinePattern(uint randomNumber) internal view returns(string memory) {
        uint modNumber = (randomNumber % patterns.length);
    ...
    // Modify Public Price
    function setPublicPrice(uint _newEthPrice) external onlyRole(OWNER_ROLE) {
        publicPrice = _newEthPrice * 10 ** 18;
    }
