...
contract SWFGameCR is ISafezoneGame, Ownable, ReentrancyGuard, Pausable {
  ...
  function mintCommit(uint256 amount, bool stake) external payable whenNotPaused nonReentrant {
    ...
    // mint price
	  uint256 MINT_PRICE = .040000 ether;
  ...
  function mintCost(uint256 tokenId, uint256 maxTokens) public view returns (uint256) {
    if (tokenId <= maxTokens * 8 / 20) return 0;
    if (tokenId <= maxTokens * 11 / 20) return 24000 ether;
    if (tokenId <= maxTokens * 14 / 20) return 36000 ether;
    if (tokenId <= maxTokens * 17 / 20) return 48000 ether; 
