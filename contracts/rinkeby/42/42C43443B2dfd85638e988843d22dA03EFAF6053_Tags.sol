...
abstract contract ERC721Buyable is Context, ERC721 {
    using SafeERC20 for IERC20;
    ...
    function buy(
        ...
        payable 
    {
        buyFor(_tokenId, _paid, _newPrice, _msgSender());
    }
    ...
    function withdrawProceeds(address _token, uint96 _amount) external {
        ...
        uint96 available = owed[sender][_token];
        require(available > 0, "!owed");
        uint96 withdrawn = _amount >= available ? available : _amount;
        owed[sender][_token] -= withdrawn;
        Utils.pushToken(sender, _token, withdrawn);
    }
}
