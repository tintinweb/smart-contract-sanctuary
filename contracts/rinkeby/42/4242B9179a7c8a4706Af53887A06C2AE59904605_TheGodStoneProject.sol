...
contract TheGodStoneProject is ERC721, ERC721Enumerable, Ownable {
    ...
    */
    ...
    Used to track the property of each stone that have been minted.
    */
    ...
    /**
    Mapping of Stone[].
    They are for the purpose of assigning tokenIds only and not edited after minting.
    mapping[0] => Stone[] (CommonStones)
    mapping[1] => Stone[] (YellowStones)
    mapping[2] => Stone[] (OrangeStones)
    mapping[3] => Stone[] (AquaStones)
    mapping[4] => Stone[] (GreenStones)
    mapping[5] => Stone[] (RedStones)
    mapping[6] => Stone[] (PurpleStones)
    mapping[7] => Stone[] (TheGodStones)
    */
    struct StoneClass{
        string name;
        uint256 upperIndex; // Required to assign tokenId to the different stone types
        uint256 maxSupply; 
        uint256 stonesRequired; // Number of immediate predecessor stones required to mint this class of stone
        Stone[] arrayOfStones;
    }
    ...
    function mintCommonStone(uint256 amount) external payable {
        ...
        for (uint256 i = 1; i <= amount; i++) {
            //uint256 newTokenId = mapStoneCollections[0].length;
    ...
    */
    ...
    //     (bool canMint, uint[] memory requiredStones) = ownerCanMintStone(msg.sender, 1);
    ...
    function mintYellowStone(uint256 amount, uint256 stoneClassIndex) external payable {
        require( saleActive, "Sale is not yet active." );
        require( amount == 1, "Only 1 Yellow Stone can be minted per transaction." );
        require( msg.value == 0, "The amount of ether sent should be 0." );
        ...
        require(canMint, "Not enough Common Stones");
        //require((_allYellowStones.length + amount) <= (_orangeStoneUpperIndex - _yellowStoneUpperIndex), "There are nore more Yellow Stones");
        require((mapStoneClass[stoneClassIndex].arrayOfStones.length.add(amount)) <= mapStoneClass[stoneClassIndex].maxSupply, "There are no more stones");
    ...
    */
    ...
    function ownerCanMintStone(address _addr, uint256 stoneClassIndex) public view returns (bool, uint[] memory) {
        return ownerHasRequiredStones(_addr, mapStoneClass[stoneClassIndex].arrayOfStones, mapStoneClass[stoneClassIndex].stonesRequired);
    }
