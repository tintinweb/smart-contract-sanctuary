...
import "contracts4/token/ERC20/IERC20.sol";
import "contracts4/token/ERC20/utils/SafeERC20.sol";
...
contract TaskManager is IIterableFunctions, StorageAccessor, FeeManager {
    ...
    function getFunctions() override external pure returns (bytes4[] memory){
        ...
        functions[7] = this.submitSolution.selector;
        functions[8] = this.acceptSolution.selector;
        functions[9] = this.rejectSolution.selector;
        return functions;
    }
    ...
    function getConfigStorage() override virtual internal view returns (IConfigStorage) {
        return IConfigStorage(getTaskStorage().configStorage());
    }
    ...
    function createTask(
        ...
        if (msg.value > 0) {
            ...
            require(
                ...
                "TM: payment in Hyve but commission not");
            if (taskParams.isHyveCommission) {
        ...
        getTaskStorage().addTaskCreatedBy(_msgSender(), taskId);
        ...
        //interactions
        if (taskParams.isHyveCommission) {
            splitTax(_msgSender(), isEth, tax, IERC20Upgradeable(getConfigStorage().getHyveERC()));
        } else {
            splitTax(_msgSender(), isEth, tax, IERC20Upgradeable(taskParams.tokenAddress));
        }

        if (isEth) {
            payable(getConfigStorage().getEscrow()).call{value : taskParams.amount}(abi.encodeWithSelector(
                    IEscrow(getConfigStorage().getEscrow()).createTaskDepositETH.selector, taskId, _msgSender(), taskParams.amount));
        } else {
            IEscrow(getConfigStorage().getEscrow()).createTaskDeposit(address(this), taskId, _msgSender(), taskParams.tokenAddress, taskParams.amount);
        }

        emit TaskCreated(taskParams.externalId, taskId);
        uint256 applicationId = getTaskStorage().getNewApplicationId();
        getTaskStorage().addNewApplication(applicationId, TaskStorage.Application(taskId, applicant));
    ...
    function cancelTask(uint256 taskId) external {
        ...
        require(task.deadline < block.timestamp, "TM: deadline not reached!");
        require(task.solutionId == 0, "TM: solution already accepted!");
        getTaskStorage().cancelTask(taskId);
        IEscrow(getConfigStorage().getEscrow()).redeemDeposit(taskId);
        emit TaskCanceled(taskId);
    }

    function submitSolution(uint256 applicationId, string memory solutionUri, string memory solutionHash) external {
        require(applicationId <= getTaskStorage().getLastApplicationId(), "TM: no application");
        TaskStorage.Application memory application = getTaskStorage().getApplication(applicationId);
        require(_msgSender() == application.worker, "TM: someone else's application");
        uint256 solutionId = getTaskStorage().getNewSolutionId();
        getTaskStorage().addNewSolution(solutionId, TaskStorage.Solution(applicationId, solutionUri, solutionHash, ""));
        emit TaskSolutionSubmitted(application.taskId, applicationId, solutionId, _msgSender());
    }

    function acceptSolution(uint256 solutionId) external {
        require(solutionId <= getTaskStorage().getLastSolutionId(), NO_SOLUTION);
        TaskStorage.Application memory application = getTaskStorage().getApplication(getTaskStorage().getSolution(solutionId).applicationId);
        require(_msgSender() == getTaskStorage().getTask(application.taskId).creator, NOT_CREATOR_REASON);
        getTaskStorage().acceptSolution(solutionId);
        IEscrow(getConfigStorage().getEscrow()).setBeneficiary(application.taskId, application.worker);
        IEscrow(getConfigStorage().getEscrow()).executePayment(application.taskId);
        emit TaskSolutionAccepted(application.taskId, solutionId);
    }

    function rejectSolution(uint256 solutionId, string memory reason) external {
        require(solutionId <= getTaskStorage().getLastSolutionId(), NO_SOLUTION);
        TaskStorage.Application memory application = getTaskStorage().getApplication(getTaskStorage().getSolution(solutionId).applicationId);
        require(_msgSender() == getTaskStorage().getTask(application.taskId).creator, NOT_CREATOR_REASON);
        getTaskStorage().rejectSolution(solutionId, reason);
        emit TaskSolutionRejected(application.taskId, solutionId);
    }

    function checkUserTokenBalance(address user, address token, uint256 minimumBalance) internal view {
        require(
            IERC20(token).balanceOf(user) >= minimumBalance &&
            IERC20(token).allowance(user, address(this)) >= minimumBalance,
            "TM: Too few funds");
    }
}
...
contract TaskStorage is UtilityStorage {
    ...
    mapping(uint256 => uint256[]) private _taskApplications;
    mapping(uint256 => Application) private _applications;

    uint256 private _solutionId;
    mapping(uint256 => uint256[]) private _taskSolutions;
    ...
    function addTaskCreatedBy(address creator, uint256 taskId) external onlyRole(OPERATOR_ROLE) {
        _creatorTasks[creator].push(taskId);
    }
    ...
    function getTaskApplications(uint256 taskId) external view returns (uint256[] memory) {
        return _taskApplications[taskId];
    }
    ...
    function addNewApplication(uint256 applicationId, Application memory application) external onlyRole(OPERATOR_ROLE) {
        ...
        uint256 taskId = application.taskId;
        _taskApplications[taskId].push(applicationId);
    }

    function acceptApplication(uint applicationId) external onlyRole(OPERATOR_ROLE) {
        _tasks[_applications[applicationId].taskId].applicationId = applicationId;
    }
    ...
    function getTaskSolutions(uint256 applicationId) external view returns (uint256[] memory) {
        return _taskSolutions[applicationId];
    }
    ...
    function addNewSolution(uint256 solutionId, Solution memory solution) external onlyRole(OPERATOR_ROLE) {
        ...
        _taskSolutions[solution.applicationId].push(solutionId);
    }

    function acceptSolution(uint256 solutionId) external onlyRole(OPERATOR_ROLE) {
        _tasks[_applications[_solutions[solutionId].applicationId].taskId].solutionId = solutionId;
    }
...
interface IEscrow {
    ...
    function createTaskDeposit(address mediator, uint256 taskId, address from, address tokenAddress, uint256 amount) external;

    function createTaskDepositETH(uint256 taskId, address from, uint256 amount) external payable;

    function setBeneficiary(uint256 taskId, address beneficiary) external;

    function executePayment(uint256 taskId) external;

    function getEscrowDeposit(uint256 taskId) external view returns (EscrowDeposit memory);

    function redeemDeposit(uint256 taskId) external;

    function splitDeposit(uint256 taskId, uint256 paymentPercentage) external;
}
...
contract Escrow is IEscrow, AccessLevel, ERC20Taker {
    ...
    event DepositCreated(uint256 taskId, address from, address token, uint256 amount);
    event DepositWithdrawal(uint256 taskId, address beneficiary, uint256 amount);
    ...
    function createTaskDeposit(address mediator, uint256 taskId, address from, address tokenAddress, uint256 amount) override external {
        require(tokenAddress != address(0), "Escrow: Token address is 0");
        require(!depositExistsForTask(taskId), "Escrow: A deposit has already been created");

        createNewDeposit(taskId, from, tokenAddress, amount, false);

        takeERC20(ERC20Sender(mediator), from, IERC20Upgradeable(tokenAddress), amount);
        emit DepositCreated(taskId, from, tokenAddress, amount);
    }

    function createTaskDepositETH(uint256 taskId, address from, uint256 amount) override external payable {
        require(amount == msg.value, "Escrow: Sent amount not equal to amount as parameter");
        require(!depositExistsForTask(taskId), "Escrow: A deposit has already been created");
        createNewDeposit(taskId, from, address(0), msg.value, true);
        emit DepositCreated(taskId, from, address(0), amount);
    }

    function setBeneficiary(uint256 taskId, address beneficiary) override external onlyRole(OPERATOR_ROLE) {
        require(beneficiary != address(0), "Escrow: Beneficiary address should not be 0");
        require(depositExistsForTask(taskId), "Escrow: Deposit should already exist!");
        _deposits[taskId].beneficiary = beneficiary;
    }

    function depositExistsForTask(uint256 taskId) private view returns (bool) {
        return _deposits[taskId].initialDeposit != 0;
    }
    ...
    function createNewDeposit(
        ...
        require(taskId >= 0, "Escrow: Task id is not valid");
        require(amount > 0, "Escrow: Deposit amount should be more than 0");

        _deposits[taskId] = EscrowDeposit(from, token, amount, address(0));
    }

    function executePayment(uint256 taskId) override external onlyRole(OPERATOR_ROLE) {
        require(depositExistsForTask(taskId), "Escrow: No deposit has been created for given task id!");
        transferDeposit(taskId, _deposits[taskId].beneficiary, _deposits[taskId].initialDeposit);

        emit DepositWithdrawal(taskId, _deposits[taskId].beneficiary, _deposits[taskId].initialDeposit);
    }

    function getEscrowDeposit(uint256 taskId) override external view returns (EscrowDeposit memory) {
        require(taskId >= 0, "Escrow: Task id is not valid");
        return _deposits[taskId];
    }

    function splitDeposit(uint256 taskId, uint256 paymentPercentage) override external onlyRole(OPERATOR_ROLE) {
        require(depositExistsForTask(taskId), "Escrow: No deposit has been created for given task id!");

        uint256 depositedAmount = _deposits[taskId].initialDeposit;
        ...
        transferDeposit(taskId, _deposits[taskId].beneficiary, amountForApplicant);
        emit DepositWithdrawal(taskId, _deposits[taskId].beneficiary, amountForApplicant);
        transferDeposit(taskId, _deposits[taskId].from, amountForOwner);
        emit DepositWithdrawal(taskId, _deposits[taskId].from, amountForOwner);
    }

    function redeemDeposit(uint256 taskId) override external onlyRole(OPERATOR_ROLE) {
        require(depositExistsForTask(taskId), "Escrow: No deposit has been created for given task id!");

        transferDeposit(taskId, _deposits[taskId].from, _deposits[taskId].initialDeposit);
        emit DepositWithdrawal(taskId, _deposits[taskId].from, _deposits[taskId].initialDeposit);
    }

    function transferDeposit(uint256 taskId, address beneficiary, uint256 amount) private {
        require(amount <= _deposits[taskId].initialDeposit, "Escrow: Amount too large");
        require(amount > 0, "Escrow not enough funds");

        if (_deposits[taskId].tokenAddress == address(0)) {
            //@dev - please make sure deposit amount is zeroed before transferring ETH to avoid re-entrancy attack
            delete _deposits[taskId];
            payable(beneficiary).call{value: amount}("");
        } else {
            address tokenAddress = _deposits[taskId].tokenAddress;
            delete _deposits[taskId];
