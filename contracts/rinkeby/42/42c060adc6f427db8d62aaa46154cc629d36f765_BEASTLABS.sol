...
contract BEASTLABS is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public cost = 0.199 ether;
  uint256 public presalecost =  0.155 ether;
  uint256 public bullcost =  0.099 ether;
  uint256 public maxSupply = 10888;
  uint256 public maxMintAmount = 20;
  uint256 public maxbirdAmount = 5;
  ...
  uint256 public royaltyFee = 9000;
  address public royaltyWallet = msg.sender;

  constructor(
    string memory _initBaseURI,
    string memory _initNotRevealedUri
  ) ERC721("BEAST LABS", "BEASTLABS") {
  ...
  function Presalemint(address _to, uint256 _mintAmount) public payable {
    ...
    require(tokensPerWallet[msg.sender] <= 5, "BEASTLABS: Presale mint is five token only.");
    require(supply + _mintAmount <= 4000);
    require(supply >= 1000);
  ...
  function EARLYBULLSALE(address _to, uint256 _mintAmount) public payable {
      uint256 supply = totalSupply();
      require(!paused);
      require(_mintAmount > 0);
      require(_mintAmount <= maxbirdAmount);
      require(tokensPerWallet[msg.sender] + _mintAmount <= 5, "BEASTLABS: You can't mint more than 5 tokens!");
      require(supply + _mintAmount <= 1000);
      tokensPerWallet[msg.sender] += _mintAmount;


      if (msg.sender != owner()) {
          require(msg.value >= bullcost * _mintAmount);
      }
      for (uint256 i = 1; i <= _mintAmount; i++) {
          _safeMint(_to, supply + i);
      }
  ...
  function PublicMint(address _to, uint256 _mintAmount) public payable {
      ...
      require(_mintAmount > 0);
      require(_mintAmount <= maxMintAmount);
      require(supply + _mintAmount <= 10500);
      require(supply >= 4000);
  ...
  function FreeMint(address _to, uint256 _mintAmount) public onlyOwner {
      uint256 supply = totalSupply();
      require(_mintAmount > 0);
      for (uint256 i = 1; i <= _mintAmount; i++) {
          _safeMint(_to, supply + i);
  ...
  function setBullCost(uint256 _newbullCost) public onlyOwner {
    bullcost = _newbullCost;
  }
...
function batchWhitelist(address[] memory _user) public onlyOwner {
