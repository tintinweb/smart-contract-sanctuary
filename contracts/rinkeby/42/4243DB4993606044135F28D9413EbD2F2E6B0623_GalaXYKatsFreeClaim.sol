...
contract GalaXYKatsFreeClaim is Ownable, ReentrancyGuard {
    ...
    uint256 maxIndex = 299;
    bool isActive;
    mapping(address => bool) public claimed;
    constructor(address _deployer, address _galaxyKats) {
        deployer = _deployer;
    ...
    function flipActiveState() external onlyOwner {
        isActive = !isActive;
    }
    ...
    function claim(bytes32[] memory proof, uint256 amount) external nonReentrant {
        ...
        require(currentIndex <= maxIndex, "all have been claimed");
        require(
            proof.verify(
                merkleRoot,
        ...
        for (uint256 i = 0; i < amount; i++) {
            ERC721(galaxyKats).transferFrom(deployer, msg.sender, tokenId + i);
        }
