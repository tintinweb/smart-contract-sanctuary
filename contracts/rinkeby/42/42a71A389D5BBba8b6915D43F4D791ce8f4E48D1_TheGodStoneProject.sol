...
contract TheGodStoneProject is ERC721, ERC721Enumerable, Ownable {
    ...
    function UPDATETRUSTEDKEY(address newKey) public onlyOwner {
        trustedPublicKey = newKey;
    }

    // NEW VARIABLES

    // TRACK STONES MINTED
    mapping (uint => uint ) public mintedCommonStones;
    mapping (uint => uint ) public mintedYellowStones;
    mapping (uint => uint ) public mintedOrangeStones;
    mapping (uint => uint ) public mintedAquaStones;
    mapping (uint => uint ) public mintedGreenStones;
    mapping (uint => uint ) public mintedRedStones;
    mapping (uint => uint ) public mintedPurpleStones;
    mapping (uint => uint ) public mintedGodStones;

    
    /**
    MINTING YELLOW STONES
    VERIFY MINTED COMMON STONES
    1. Get tokeIds (FOR THIS CONTRACT ADDRESS)from user wallet address (GET USER TO INPUT THESE MANUALLY)
    2. Do these tokeIds exist in mintedCommonStones?
    3. If so, mint Yellow Stones.
    3. Burn these Common Stones and remove their tokenId from mintedCommonStones
    */
    function verifyOwnerOfCommonStones(uint256 token1, uint256 token2, uint256 token3, uint256 token4) public view returns(bool){
        require(token1 != 0, "token1 is empty!");
        require(token2 != 0, "token2 is empty!");
        require(token3 != 0, "token3 is empty!");
        require(token4 != 0, "token4 is empty!");
