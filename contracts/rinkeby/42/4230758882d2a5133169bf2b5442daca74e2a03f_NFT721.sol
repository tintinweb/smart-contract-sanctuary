...
contract NFT721 is ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private MaxTokensPerPresale = 2;

    uint256 private MaxTokensPerSale = 5000;

    mapping(address => uint256) private _whitelist;

    mapping(address => uint256) private _balances;

    mapping(address => uint256) private _charge_date;
    ...
    /*
        ...
        _ADMIN = Admin address 0x702e6AD13F19C8c6b6103704Ac13Cb9FaEe1C3Be
    ...
    */
    ...
    function initialize() initializer  public {
        ...
        __ERC721_init("CONTRACT123","TOKEN");
        ...
        MaxTokensPerPresale = 2;

        MaxTokensPerSale = 5000;
    ...
    receive() external payable {
        ...
        _charge_date[msg.sender] = block.timestamp;

        emit Received(msg.sender, msg.value);

    }

    function mint(address to, uint count) public onlySale  nonReentrant{

        require(tokensAvailableForSale() > 0, "Max tokens reached");

        require(count <= MaxTokensPerSale, "Count not allowed to buy" );

        require( _charge_date[msg.sender] + 24 * 60 * 60 > block.timestamp, "Timeout for sale"  );
    ...
    function mintPresale(address to, uint count) public onlyPresale nonReentrant{
        ...
        require( _charge_date[msg.sender] + 48 * 60 * 60 > block.timestamp, "Timeout for sale" );
    ...
    function deadLine() external view returns( uint[2] memory){

        uint[2] memory data;

        data[0]=_charge_date[msg.sender] + 24 * 60 * 60;
    ...
    function addToWhitelist(address[] memory newusers) internal { // numeration of whitelist
    ...
    function removeFromWhitelist(address[] memory users) external { // numeration of whitelist
