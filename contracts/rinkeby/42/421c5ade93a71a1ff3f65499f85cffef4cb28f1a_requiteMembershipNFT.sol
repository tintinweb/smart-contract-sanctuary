 ...
 * {ERC721Enumerable}.
 */
contract requiteMembershipNFT is ERC721 {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory name = string(abi.encodePacked(merbershipType, ' Anchor Certificate #',toString(merberships[tokenId].nr)));
    ...
    function generateImage(uint256 tokenId) public view returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                paletteSection,
                skyscraper,
                '<text x="215" y="80" class="small">METAVERSE</text>',
    ...
    function generatePaletteSection(uint256 tokenId, uint256 pIndex) internal view returns (string memory) {
        return string(abi.encodePacked(
                ...
                '<circle cx="255" cy="30" r="15" stroke="white" fill="transparent" stroke-width="4" opacity="0.7" />',
                '<text x="250" y="35" class="medium" stroke="',palette[pIndex][3],'" stroke-width="3">R</text>'
            )
        );
    }
    ...
    function mintMembershipNFT() public payable returns (uint256 tokenId) {
