...
contract Staking is Deposit,ReentrancyGuard{
    ...
    modifier checkUser (uint _depositId) {
      require(msg.sender == signDeposit[_depositId].Depositer,'Reject extraction');
      _;
    }
    ...
    function _safeDeposit(address _in,uint _amount) private {
        ...
        require(newFromBalance == oldFromBalance + _amount,'deposit fail');
        require(oldSenderFromBalance == newSenderFromBalance + _amount,'deposit fail');
    }
    ...
    function extractionCapital(uint _cycleId,uint _depositId) external nonReentrant checkUser(_depositId) existCycle(_cycleId){
        require(!signDeposit[_depositId].extractStatu,'Extracted');
        uint _amount;
