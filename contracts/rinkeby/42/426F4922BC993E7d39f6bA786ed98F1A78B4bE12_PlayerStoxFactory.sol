...
interface IPlayerStoxToken {
    ...
    function initialize(string memory name_, string memory symbol_, uint8 decimals_, address factoryAddress_, address USDT_) external;
...
contract PlayerStoxFactory {
    ...
    function createPlayerStox(string memory name_, string memory symbol_, uint8 decimals_) external onlyAdmin {
        ...
        IPlayerStoxToken(address(playerStox)).initialize(name_, symbol_, decimals_, address(this), USDT);
