...
contract PreIDOToken is ReentrancyGuard, ERC20, Ownable {
    ...
    function exchangeBoughtAmount(
        ...
        require(sigs.length >= 1, "PreIDOToken: insufficient number of signatures");
        {
            ...
            require(muon.verify(reqId, uint(hash), sigs), "PreIDOToken: not verified");
        }
        ...
        returns (uint, uint)
    {
        ...
        returnAmount = _bancorCalculateSaleReturn(supply + _shiftSupply, collateralReserve, cw, idoAmount);
    ...
    function _bancorCalculateSaleAmountIn(
        ...
        require(
            ...
            _connectorWeight > 0 && 
            _connectorWeight <= scale && 
            _collateralAmount <= _supply, 
            "PreIDOToken: _bancorCalculateSaleAmountIn Error"
        );
        // special case for 0 sell amount
        ...
        returns (uint, uint)
    {
        ...
        uint returnAmount = _bancorCalculateSaleAmountIn(supply + _shiftSupply, collateralReserve, cw, newCollateralAmount);
    ...
    function _bancorCalculatePurchaseReturn(
        ...
        require(
            ...
            _connectorWeight > 0 &&
            _connectorWeight <= scale,
            "PreIDOToken: _bancorCalculatePurchaseReturn() Error"
        );
        ...
        returns (uint, uint)
    {
        ...
        return (
            _bancorCalculatePurchaseReturn(supply + _shiftSupply, collateralReserve, cw, collateralAmount),
            feeAmount
        );
    ...
    function adjustParameters(address adjuster, uint p1, address p2, address p3, address p4) external onlyOwner returns (bool) {
        (bool success,) = adjuster.delegatecall(abi.encodeWithSignature("adjustment(uint256,address,address)", p1, p2, p3, p4));
        return success;
    }
