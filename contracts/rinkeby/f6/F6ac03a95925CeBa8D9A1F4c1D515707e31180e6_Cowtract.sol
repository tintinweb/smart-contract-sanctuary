...
contract Cowtract is ERC721Enumerable, Ownable {
  using Strings for uint256;
  using Address for address;
  ...
  function mintPresale(uint256 _amount) public payable {
    ...
    uint256 reservedAmt = whitelist[msg.sender];
    ...
    whitelist[msg.sender] = reservedAmt - _amount;
  ...
  // edit whitelist
  function editWhiteList(address[] memory _a) public onlyOwner {
    for (uint256 i; i < _a.length; i++) {
      // gives each whitelisted address 10 mints
      whitelist[_a[i]] = 10;
    }
  }
  ...
  // Withdraw funds from contract to the team wallet
