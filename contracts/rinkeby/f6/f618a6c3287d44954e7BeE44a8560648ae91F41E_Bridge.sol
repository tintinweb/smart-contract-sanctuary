...
// 
interface IFungibleBridgeBase is IAccessControl {
    event DepositErc20(string indexed sourceChainName, address indexed sourceContractAddress, uint256 indexed id, uint256 value, address toAddress);
    event WithdrawErc20(string indexed sourceChainName, address indexed sourceContractAddress, uint256 indexed id, uint256 value, address toAddress, address destinationContractAddress);

    function addSupportedErc20Contract(address erc20Contract) external;
    function removeSupportedErc20Contract(address erc20Contract) external;
}

// 
interface INonFungibleBridgeBase is IAccessControl {
    event DepositErc721(string indexed sourceChainName, address indexed sourceContractAddress, uint256 indexed id, uint256 tokenId, address toAddress);
    event WithdrawErc721(string indexed sourceChainName, address indexed sourceContractAddress, uint256 indexed id, uint256 tokenId, address toAddress, address destinationContractAddress);

    function addSupportedErc721Contract(address erc721Contract) external;
    function removeSupportedErc721Contract(address erc721Contract) external;
}
...
// 
abstract contract BridgeReceiver is IFungibleBridgeBase, INonFungibleBridgeBase, AccessControl {
    ...
    mapping(string => mapping(address => mapping(uint256 => bool))) alreadyClaimed;

    /**
    * @dev Verifies ETH signature and checks if 'account' signed it.
    */
    ...
    /**
    * @dev Prepares bytes32 hash from message parameters.
    */
    function _createMessageHash(string memory sourceChainName, address sourceErc20, uint256 id, uint256 amount, address sendTo, address destinationErc20) internal virtual view returns (bytes32);

    function claimMigratedErc20Tokens(string memory sourceChainName, address sourceErc20, uint256 id, uint256 amount, address destinationErc20, bytes memory signature, address whiteListedSigner) external {
        require(hasRole(SIGNER_ROLE, whiteListedSigner), "Only white-listed signers");
        require(alreadyClaimed[sourceChainName][sourceErc20][id] == false, "User already claimed");

        bytes32 messageHash = _createMessageHash(sourceChainName, sourceErc20, id, amount, msg.sender, destinationErc20);
        require(_verifySignature(messageHash, signature, whiteListedSigner), "");
        alreadyClaimed[sourceChainName][sourceErc20][id] = true;

        ERC20Mintable(destinationErc20).mintTo(msg.sender, amount);

        emit WithdrawErc20(sourceChainName, sourceErc20, id, amount, msg.sender, destinationErc20);
    }
}
...
contract ERC721Mintable is ERC721Enumerable, Ownable {
     ...
     */
    function mintTo(address _to, uint256 tokenId) external onlyOwner {
        _mint(_to, tokenId);
    }
...
contract Bridge is BridgeReceiver {
    ...
    */
    ...
    function depositErc20AndWrap(uint256 amount, address erc20, address recipient) internal {
        require(allowedErc20[erc20], "Contract not supported");
        require(ERC20(erc20).transferFrom(msg.sender, address(this), amount) == true, "transferFrom failed for unknown reason");
    ...
    function mintErc20(string memory sourceChainName, address sourceContractAddress, uint256 id, uint256 value,
        address toAddress, address destinationContractAddress) external {
        ...
        ERC20Mintable(destinationContractAddress).mintTo(toAddress, value);
    ...
    function depositErc721AndWrap(uint256 id, address erc721, address recipient) internal {
        require(allowedErc721[erc721], "Contract not supported");

        ERC721(erc721).transferFrom(msg.sender, address(this), id);
    ...
    function mintErc721(string memory sourceChainName, address sourceContractAddress, uint256 id, uint256 tokenId,
        ...
        claimed[sourceChainName][sourceContractAddress][id] = true;
        ERC721Mintable(destinationContractAddress).mintTo(toAddress, tokenId);
        emit WithdrawErc20(sourceChainName, sourceContractAddress, id, tokenId, toAddress, destinationContractAddress);
    }
    ...
    function isErc721Allowed(address erc721) public view returns(bool) {
