...
contract LimitTokenSale is Ownable, ReentrancyGuard {
   ...
   */
  function getRound(uint256 _index) external view returns(SaleLib.State, uint256, uint256, uint256, uint256, uint256, uint8) {
    return (
      rounds[_index].state,
      rounds[_index].price,
      rounds[_index].vesting,
      rounds[_index].totalSupply,
      rounds[_index].totalBalance,
      rounds[_index].maxInvestment,
      rounds[_index].pid
    );
  }
  ...
  function claimToken(uint256 _index, uint256 _resubmit, uint256 _refund) external nonReentrant {
    ...
    if(tokenTotal > rounds[_index].totalSupply) {
      ...
      saleBalance = saleBalance.add(amount);
    } else {
      ...
      saleBalance = saleBalance.add(balances[_index][_msgSender()]);
    }
    ...
    if(_resubmit > 0) {
      resubmitInternal(_index, _index++, _resubmit);
    }
  ...
  function claimLeftoverInternal(uint256[] memory _indexes, uint256[] memory _refunds) internal {
    ...
    for(uint256 i = 0; i < _indexes.length; i++) {
      ...
      totalRefund = totalRefund.add(_refunds[i]);

      emit LeftoverClaimed(_indexes[i], _msgSender(), _refunds[i]);
    }

    collateralToken.safeTransfer(_msgSender(), totalRefund);
  }
