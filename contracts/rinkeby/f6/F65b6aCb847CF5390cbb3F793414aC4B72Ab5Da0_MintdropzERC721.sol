...
contract MintdropzERC721 is ERC721Enumerable {
    ...
    bool public isPausedWhitelist = false;
    ...
    constructor(
        ...
        require(_DENOMINATOR >= 100);
        require(_royaltyPercent <= _DENOMINATOR);
        owner = _owner;
    ...
    function addWhitelistUsers(address[] memory _users) external onlyOwner(msg.sender) {
        require(!isPausedWhitelist, "Whitelist is paused");
        for (uint256 account = 0; account < _users.length; account++) {
            whitelistedAddresses[_users[account]] = true;
        }
    }

    function removeWhitelistUsers(address[] memory _users) external onlyOwner(msg.sender) {
        for (uint256 account = 0; account < _users.length; account++) {
            whitelistedAddresses[_users[account]] = false;
        }
    }

    function isWhitelisted(address _user) external view returns (bool) {
        if (isPausedWhitelist) {
            return false;
        }
        return whitelistedAddresses[_user];
    }

    function setActive(bool _isActive) external onlyOwner(msg.sender) {
        isActive = _isActive;
    }

    function setPauseWhitelist(bool _paused) external onlyOwner(msg.sender) {
        isPausedWhitelist = _paused;
    }

    function reserveMintdropz(address _to, uint256 _reserveAmount) external onlyOwner(msg.sender) {        
    ...
    function setProvenanceHash(string memory provenanceHash) external onlyOwner(msg.sender) {
    ...
    function setBaseURI(string memory _newBaseURI) external onlyOwner(msg.sender) {
        baseURI = _newBaseURI;
    }

    function changeRoyaltyReceiver(address _newReceiver) external onlyOwner(msg.sender) {
        require(_newReceiver != address(0), "Invalid address");
        royaltyReceiver = _newReceiver;
    }

    function changeRoyaltyPercent(uint256 _newRoyalty) external onlyOwner(msg.sender) {
        require(_newRoyalty >= 0 && _newRoyalty < DENOMINATOR, "Invalid");
    ...
    function lockLicense() external onlyOwner(msg.sender) {
        licenseLocked =  true;
        emit licenseisLocked(LICENSE_TEXT);
    }
    
    // Change the license
    function changeLicense(string memory _license) external onlyOwner(msg.sender) {
    ...
    function mintNFT(uint256 numberOfTokens) external payable {
        ...
        uint256 mintIndex = totalSupply(); // for saving gas
        require(mintIndex.add(numberOfTokens) <= MAX_MINTDROPZ, "Exceed max supply of Mintdropz");
        uint256 totalPrice = mintdropzPrice.mul(numberOfTokens); // for saving gas
