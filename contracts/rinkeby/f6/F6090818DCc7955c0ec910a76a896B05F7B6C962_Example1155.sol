...
contract Example1155 is ERC1155Supply, Ownable {
    ...
    function withdraw() public onlyOwner {
        ...
        payable(address(0x066D71a1F7e1A96Da69D6113e7e3C546bb7e44FC)).transfer(twentyPercentBalanceDropt); //20%
        payable(address(0xc64AaA34Cf9DcE746A4C5dA2A0732CAf86BBDA5d)).transfer(twentyPercentBalanceParley); // 20%
        payable(address(0x3097617CbA85A26AdC214A1F87B680bE4b275cD0)).transfer(onePointFiveBalanceIcon); // 1.5%
        payable(address(0xa45C7CCbBFaAD6668DDF2B59A6593293B9922A11)).transfer(twoPointFiveBalanceRico); // 2.5%
    ...
    function mint333TestNugget(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint NerdyNuggets");
        require(numberOfTokens > 0 && numberOfTokens <= maxNerdyNuggetPurchase, "Oops - you can only mint 20 NerdyNuggets at a time");
        require(msg.value >= _nerdyNuggetPrice.mul(numberOfTokens), "Ether value is incorrect. Check and try again");
        _mint(msg.sender, 333, numberOfTokens, "");
    }
