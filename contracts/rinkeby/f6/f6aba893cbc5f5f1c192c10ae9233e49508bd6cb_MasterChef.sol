...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function unlockAvailable() public view returns(uint256) {
        if(_currentBlock < startUnlockTime) return 0;

        LockedInfo storage user = lockedInfo[msg.sender];
        if(user.lockedAmount <= 0) return 0;

        uint256 pastMonth = (_currentBlock - startUnlockTime) / 30 days;
    ...
    function unlock() external nonReentrant {
        require(_currentBlock > startUnlockTime, "No available to unlock");
        ...
        user.unlockedCount = (_currentBlock - startUnlockTime) / 30 days;
    }
    ...
    function updateStartUnlockTime(uint256 _time) external onlyOwner {
        require(_currentBlock < startUnlockTime);
        require(_time > _currentBlock, "time should be greater than current timestamp");
