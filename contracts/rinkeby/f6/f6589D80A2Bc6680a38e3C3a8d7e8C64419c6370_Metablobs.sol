...
contract Metablobs is ERC721, Ownable, ReentrancyGuard {
  ...
  ERC721Interface bayc = ERC721Interface(0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D); // blootelves unverified on etherscan
 ...
 */
  ...
  function testAlternative(address tokenholder) public view returns(uint){
    ERC721Interface lot1 = ERC721Interface(0xFdF92f93070c3a9dF073E57D389310199473AE9b); 
    ERC721Interface randsvg = ERC721Interface(0x53D5001FEe2EDbbb194b278621e8934888626c8E); 
    ERC721Interface lot2 = ERC721Interface(0x69Cc647333F7cf359bc06BAce626b9c27A554182); 
    uint allowance = 1;
    ERC721Interface[3] memory nftprojects = [lot1,randsvg, lot2];
  ...
  // here have some mfkn if statements
  function getAddressBeneficiaryProjectsMetablobsTokenAllowance(address tokenholder) public view returns(uint){
    uint allowance = 1;
    /* if(flbc.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(coolcats.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(bayc.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(turtletanks.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(creatures.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(animetas.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(dogepound.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(uunicorns.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(mekaverse.balanceOf(tokenholder) > 0){
      allowance += 1;
    }
    if(neotokyocitadelIdentity.balanceOf(tokenholder) > 0){
      allowance += 1;
    } */
    return allowance;
  }
  ...
  function mintMetablobs(address to) external onlyEOA nonReentrant {
    ...
    if(to == msg.sender){
      require(tokensPerAddress[msg.sender] < testAlternative(msg.sender), "you minted enough, pal");
  ...
  // in case of accidental eth sent to contract, break g(l)as(s)
  function withdrawAccidentalEth() public onlyOwner {
    payable(owner()).transfer(address(this).balance);
  }

  // if you dont know these two, learn solidity 101
