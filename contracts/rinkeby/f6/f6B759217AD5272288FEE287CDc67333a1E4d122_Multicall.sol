...
contract Multicall is IMulticall, MulticallView {
  ...
  function aggregate(Call[] memory calls) public override returns (bytes[] memory returnData) {
    returnData = new bytes[](calls.length);
    for (uint256 i = 0; i < calls.length; i++) {
      (address target, bytes memory callData) = (calls[i].target, calls[i].callData);
      bytes memory ret = target.functionCall(callData, 'Multicall aggregate: call failed');
      returnData[i] = ret;
    }
  }
  ...
  function aggregateETH(CallETH[] memory calls)
    ...
    payable
    override
    returns (bytes[] memory returnData, uint256 valueLeft)
  {
    uint256 msgValue = msg.value;
    uint256 valueInit = thisEthBalance().sub(msgValue);

    returnData = new bytes[](calls.length);
    for (uint256 i = 0; i < calls.length; i++) {
      (address target, uint256 val, bytes memory callData) = (calls[i].target, calls[i].value, calls[i].callData);
      require(msgValue >= val, 'Not enough ETH');
      msgValue = msgValue.sub(val);
      bytes memory ret = target.functionCallWithValue(callData, val, 'Multicall aggregate: call failed');
      returnData[i] = ret;
    }

    valueLeft = thisEthBalance().sub(valueInit);
    if (valueLeft > 0) {
...
interface IMulticall is IMulticallView {
  ...
  function aggregateETH(CallETH[] memory calls) external payable returns (bytes[] memory returnData, uint256 valueLeft);
}
...
contract MulticallView is IMulticallView {
  ...
  function thisEthBalance() public view override returns (uint256 balance) {
    balance = getEthBalance(address(this));
  }
...
interface IMulticallView {
  ...
  function thisEthBalance() external view returns (uint256 balance);
