...
contract Toadz is ERC721, Ownable {
    ...
    uint256 public startingBlockPresale =999999999;
    ...
    {
        require(
             block.number >= startingBlockPresale,
             "Presale hasn't started yet!"
        );
        require(
             onWhiteList(msg.sender),
             "Not Whitelisted!"
        );
        require(
            quantity <= maxMintsPerTx,
    ...
    function onWhiteList(address caller)
        public
        view
        returns (bool)
    {
        return preSaleWhitelist[caller];
    }

    function addWhiteList(address [] memory _participants) 
        external
        onlyOwner 
    {
        for (uint256 i = 0; i < _participants.length; i++) {
            preSaleWhitelist[_participants[i]]=true;
        }    
    }
    ...
    function setStartingBlockPresale(uint256 _startingBlock)
        public
        onlyOwner
    {
        require (block.number<startingBlockPresale,
        "Sale has already started"); //dev may not change the starting block once it has been passed
        startingBlockPresale=_startingBlock;
    }
