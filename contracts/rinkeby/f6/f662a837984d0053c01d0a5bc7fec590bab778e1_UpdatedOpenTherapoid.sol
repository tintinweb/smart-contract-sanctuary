...
// File: contracts/UpdatedOpenTherapoid.sol

pragma solidity ^0.8.0;

/// @title UpdatedOpenTherapoid
/// @notice ERC-20 implementation of OpenTherapoid token
contract UpdatedOpenTherapoid is ERC20, Ownable {
    ...
    event LogBulkMint(address indexed to, uint256 amount, bytes32 activity);

    /**
     * @dev Sets the values for {name = OpenTherapoidCoin}, {totalSupply = 210000} and {symbol = OPSCI}.
     *
     * All two of these values are immutable: they can only be set once during
     * construction.
     */
    constructor(uint256 initialSupply) ERC20("ScienceCoin", "OPSCI") {
        tokenDecimals = 18;
    ...
    function bulkMint(
        ...
        require(
            (accounts.length == amounts.length) &&
                (accounts.length == activities.length),
            "bulkMint: Unequal params"
        );
        for (uint256 i = 0; i < accounts.length; i++) {
            _mint(accounts[i], amounts[i]);
            emit LogBulkMint(accounts[i], amounts[i], activities[i]);
        }
    }
