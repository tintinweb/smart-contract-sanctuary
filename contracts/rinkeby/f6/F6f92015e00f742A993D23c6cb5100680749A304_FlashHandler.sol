...
import "./interfaces/IFlashPool.sol";
...
contract FlashHandler is AccessController, IHandler {
    ...
    function calculateLPToken(
        ...
        uint256 totalSupply = IFlashPool(lpToken).totalSupply();
        uint256 flashReserve = IFlashPool(lpToken).reserveAltAmount(); 
        uint256 altReserve = IFlashPool(lpToken).reserveFlashAmount();

        uint256 lpAmountA = (tokenFeesA * totalSupply) / flashReserve;
        uint256 lpAmountB = (tokenFeesB * totalSupply) / altReserve;
        ...
        )
    {
        ...
        // Calculate fee for flash & altToken
        ...
        )
    {
        ...
        flashAmount = (lpAmount * flashReserve) / totalSupply;
        altAmount = (lpAmount * altReserve) / totalSupply;
        
        rootK = Math.sqrt(flashAmount * altAmount);
    }
    ...
    function getPairTokens(address lpToken) public view returns (address, address) {
        ...
        return (IFlashPool(lpToken).FLASH_TOKEN(), IFlashPool(lpToken).token());
    }
}
...
interface IFlashPool {
    ...
    function FLASH_PROTOCOL() external view returns(address);
...
contract AccessController is IAccessController, Ownable {
    ...
    address public FLASH_PROTOCOL;
    address public AQUA_PRIMARY;
    address public FLASH_TOKEN;
    ...
    constructor(address factory, address primary) {
        FLASH_PROTOCOL = factory;
        AQUA_PRIMARY = primary;
        FLASH_TOKEN = IFlashProtocol(FLASH_PROTOCOL).FLASH_TOKEN();
    }

    function addPools(
        address[] memory tokenA, // should be Flash token
        address[] memory tokenB, // should be Alt address for which the pool already exists 
        ...
        for (uint8 i = 0; i < tokenA.length; i++) {
            require(tokenA[i] == FLASH_TOKEN , "Flash handler :: Not flash address.");
            // address pool = IUniswapV2Factory(FLASH_PROTOCOL).getPair(tokenA[i], tokenB[i]);
            address pool = IFlashProtocol(FLASH_PROTOCOL).pools(tokenB[i]);
            require(pool != address(0), "Flash handler :: Pool does not exist");
...
interface IFlashProtocol {
    ...
    function pools(address alt) external view returns(address);
