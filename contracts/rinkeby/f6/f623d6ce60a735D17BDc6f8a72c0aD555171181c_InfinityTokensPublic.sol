...
contract InfinityTokensPublic is ERC721, OwnableClone, HasSecondarySaleFees, Saleable {
  ...
  mapping(uint256 => bool[100]) internal artworkSlotFilled;

  mapping(uint256 => string[100]) internal hashIPFSMemory;
  mapping(uint256 => string[100]) internal hashArweaveMemory;
  mapping(uint256 => string[100]) internal artworkTypeMemory;

  mapping(uint256 => string) internal mouldName;
  mapping(uint256 => string) internal mouldDescription;
  mapping(uint256 => Attributes[]) internal mouldAttributes;

  mapping(uint256 => uint256) internal editionSizeMemory;
  mapping(uint256 => uint256) internal editionNumberMemory;
  mapping(uint256 => uint256[]) internal tokensByMouldMemory;
  mapping(uint256 => uint256) internal redemptionMouldMemory;
  mapping(uint256 => uint64) internal redemptionExpirationMouldMemory;
  mapping(uint256 => uint256) internal redemptionMemory;
  mapping(uint256 => uint256) internal evolutionLevelMemory;

  mapping(uint256 => uint256) internal totalCreated;
  mapping(uint256 => uint256) internal totalMinted;

  mapping(uint256 => bool) internal mintingActive;

  string public fileIPFSReferenceURL;
  string public fileArweaveReferenceURL;

  string public multiURI1;
  string public multiURI2;
  string public multiURI3;
  ...
  function _init(address factoryAddress, address contractCreator) internal {
    ...
    updateMultiURI(
      'https://infinitytokensmulti.azurewebsites.net/api/HttpTrigger?artContract=',
      '&id=',
      '&artworkIndex='
    );
  ...
  modifier authorised() {
    require(authorisedCaller[msg.sender] || msg.sender == owner(), 'Not authorised');
    _;
  }
  ...
  function addFile(
    uint256 mouldToUpdate,
    string memory hashIPFS,
    string memory hashArweave,
    string memory artworkType,
    uint256 artworkIndex
  ) public onlyOwner {
    require(artworkSlotFilled[mouldToUpdate][artworkIndex] == false, 'File Already Set');
  ...
  function _mintInternal(uint256 nftId, address _recipient) internal {
    ...
    tokensByMouldMemory[nftId].push(nftId);
  ...
  function getFileData(uint256 tokenId, uint256 index)
    public
    view
    returns (
      string memory hashIPFS,
  ...
  function NFTMouldFileData(uint256 nftId, uint256 index)
    public
    view
    returns (
      string memory hashIPFS,
  ...
  // solhint-disable-next-line func-name-mixedcase
  function NFTMouldMetadata(uint256 nftId)
    public
    view
    returns (
      string memory name,
  ...
  function updateMultiURI(
    string memory newURI1,
    string memory newURI2,
    string memory newURI3
  ) public onlyOwner {
    multiURI1 = newURI1;
    multiURI2 = newURI2;
    multiURI3 = newURI3;
  ...
  function multiURI(uint256 tokenId, uint256 artworkIndex) external view returns (string memory uri) {
    require(_exists(tokenId), 'ERC721Metadata: no such token');
    uri = string(
      abi.encodePacked(
        multiURI1,
        addressToString(address(this)),
        ...
        multiURI3,
        Strings.toString(artworkIndex)
      )
    );
  }

  function getTokensbyMould( uint256 nftId, uint256 start, uint256 length ) public view returns (uint256[] memory tokens) {
    uint256[] storage mouldTokens = tokensByMouldMemory[nftId];

    require(start < mouldTokens.length, 'out of bounds');
    uint256 remaining = mouldTokens.length - start;
    uint256 actualLength = remaining < length ? remaining : length;
    tokens = new uint256[](actualLength);

    for (uint256 idx = 0; idx < actualLength; idx++) {
      tokens[idx] = mouldTokens[idx + start];
    }
  }
...
contract ERC721 is Context, ERC165Storage, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  function balanceOf(address owner) public view override returns (uint256) {
    require(owner != address(0), 'ERC721: queried null address');
  ...
  function ownerOf(uint256 tokenId) public view override returns (address) {
    return _tokenOwners.get(tokenId, 'ERC721: no such token');
  }
  ...
  function approve(address to, uint256 tokenId) public virtual override {
    ...
    require(to != owner, 'ERC721: approving self');
  ...
  function getApproved(uint256 tokenId) public view override returns (address) {
    require(_exists(tokenId), 'ERC721: no such token');
  ...
  function _safeTransfer(
    ...
    _transfer(from, to, tokenId);
    require(_checkOnERC721Received(from, to, tokenId, _data), 'ERC721: not an ERC721Receiver');
  }
  ...
  function _safeMint(
    ...
    _mint(to, tokenId);
    require(_checkOnERC721Received(address(0), to, tokenId, _data), 'ERC721: not an ERC721Receiver');
  }
  ...
  function _transfer(
    ...
    require(ownerOf(tokenId) == from, 'ERC721: not authorized');
    require(to != address(0), 'ERC721: to cannot be null');
