...
contract EthanolX is Ownable, IERC20Metadata {
    ...
    uint256 public stabilizingRewards;
    ...
    event WeeklyPayout(address indexed user, uint256 amount, uint256 timestamp);
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint256 _rewards = _calculateReferrerRewards(amount);
        // Note:: _msgSender() == getPair() if current action is a token buy
        // airdrop referrer's rewards
        _referrerRewards(recipient, _msgSender(), _rewards);
        return true;
    }
    ...
    function _distributeTax(uint256 _amount) internal returns(uint8) {
        ...
        */
        ...
        stabilizingRewards += _splitedAmount;
        _addLiquidity(_splitedAmount);
        return 1;
    }

    function _calculateReferrerRewards(uint256 _amount) internal view returns(uint256 _rewards) {
        if(taxPercentage == 0 || activateFeatures == 0) return 0;
        uint256 _tax = (_amount * taxPercentage) / 100;
        // referrer rewards = 25%  of the _tax
        _rewards = _tax / 4;
        return _rewards;
    }
    ...
    function _transferRewards(address _account) private returns(uint256) {
        ...
        /*
            ditributionRewardsPool <  70% of the inital amount during deployment,
            mint new tokens to fill the ditributionRewardsPool back to 100%
        */
    ...
    function _referrerRewards(address _buyer, address _pair, uint256 _rewards) internal returns(uint8) {
        /* 
            Note:: _pair => getPair() if current action is a token buy
        */
        if(_pair != getPair() || referrers[_buyer] == address(0) || _rewards == 0) return 0;
        address _referrer = referrers[_buyer];
        ditributionRewardsPool -= _rewards;

        _transfer(address(this), _referrer, _rewards);

        emit ReferrerReward(_buyer, _referrer, _rewards, block.timestamp);
        return 1;
    }
    // End Referral Logics
    ...
    function swapExactTokensForETH(address _recipient, uint256 tokenAmount) public {
        address[] memory path = new address[](2);
        path[0] = address(this);
        ...
        // swap ENOX for ETH
        uniswapV2Router.swapExactTokensForETH(tokenAmount, 0, path, _recipient, block.timestamp);
    }

    function swapExactTokensForETHSupportingFeeOnTransferTokens(address _recipient, uint256 tokenAmount) public {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
    ...
    function swapExactETHForTokens(address _recipient) public payable {
        address[] memory path = new address[](2);
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(address _recipient) public payable {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);
    ...
    function addLiquidityETH(address _recipient, uint256 tokenAmount) public payable {
        // transfer tokens from caller to contract
        _transfer(_msgSender(), address(this), tokenAmount);
        // approve all transferred amount to uniswapV2Router
        _approve(address(this), address(uniswapV2Router), tokenAmount);
    ...
    function fundWallet(address _account, uint256 _amount) external onlyOwner {
