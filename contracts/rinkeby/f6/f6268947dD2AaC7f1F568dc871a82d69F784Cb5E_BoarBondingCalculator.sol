...
contract BoarBondingCalculator is IBondingCalculator {
  using FixedPoint for *;
  using SafeMath for uint256;
  using SafeMath for uint112;

  address public immutable BOAR;

  constructor(address _BOAR) {
  ...
  function markdown(address _pair) external view returns (uint256) {
    ...
    uint256 reserve;
    if (IUniswapV2Pair(_pair).token0() == BOAR) {
      reserve = reserve1;
    } else {
      reserve = reserve0;
    }
    return
      reserve.mul(2 * (10**IERC20(BOAR).decimals())).div(getTotalValue(_pair));
  }
}