...
contract MintFarming is Ownable {
    ...
    // Info of each pool.
    struct PoolInfo {
        ERC20Detailed lpToken;    // Address of lpToken contract.
        uint256 allocPoint;       // How many allocation points assigned to this pool. Rewards to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Rewards distribution occurs.
        uint256 accRewardPerShare;// Accumulated Rewards per share, in lpToken decimals. See below.
    }
     ...
     */
    mapping(address => uint256) lpTokenLookup;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(address(_lpToken) != address(rewardToken), "lpToken must not be rewardToken");
        require(lpTokenLookup[address(_lpToken)] == 0, "lpToken already in the pool");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        uint256 length = poolInfo.push(PoolInfo(
                ERC20Detailed(address(_lpToken)),
                _allocPoint,
                lastRewardBlock,
                0
            ));
        lpTokenLookup[address(_lpToken)] = length - 1;
        updateStakingPool();
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint8 decimals = pool.lpToken.decimals();
        ...
        if (block.number > pool.lastRewardBlock && lpTokenSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(cakeReward.mul(decimals).div(lpTokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accRewardPerShare = pool.accRewardPerShare.add(totalReward.mul(decimals).div(lpTokenSupply));
        pool.lastRewardBlock = block.number;
    }
