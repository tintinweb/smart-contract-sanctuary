...
contract OnChainGenArtist is ERC721Enumerable, Ownable {
    ...
    event MintGenArt(address indexed _to, uint256 indexed _tokenId, uint256 indexed _projectId, uint256 _invocations);
    ...
    function mint(uint256 _projectId, uint256 numberOfTokens) external payable {
        require(projects[_projectId].isSaleActive, 'Sale must be active to mint tokens');
        require(numberOfTokens <= MAX_PUBLIC_MINT, 'Exceeded max token purchase');
        require(
            totalSupply() + numberOfTokens <= projects[_projectId].maxInvocations,
