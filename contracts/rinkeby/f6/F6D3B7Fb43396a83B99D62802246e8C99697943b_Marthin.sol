...
contract Marthin is EIP712, ERC721Enumerable, Ownable {
    ...
    uint256 public constant TOTAL_NUMBER_OF_MARTHIN = 10000;
    ...
    modifier whenMintNotPaused() {
        require(!paused_mint, "Marthin: mint is paused");
        _;
    }

    modifier whenPreSaleNotPaused() {
        require(!paused_pre_sale, "Marthin: pre sale is paused");
        _;
    }
    ...
    function presaleBuy(uint256 tokenQuantity, bytes memory signature) external payable whenPreSaleNotPaused(){
        ...
        require( supply + tokenQuantity <= TOTAL_NUMBER_OF_MARTHIN - giveaway_reserved, "You can not mint exceeds maximum NFT" );
    ...
    function mint(uint256 num) public payable whenMintNotPaused(){
        uint256 supply = totalSupply();
        require(!paused_mint, "Marthin: mint is paused");
        require( num <= 20,                                                             "Marthin You can mint a maximum of 20 NFT" );
        require( supply + num <= TOTAL_NUMBER_OF_MARTHIN - giveaway_reserved,           "You can not mint exceeds maximum NFT" );
        require( msg.value >= PRICE * num,                                              "Insufficient ETH sent" );
    ...
    function giveAway(address _to, uint256 _reserveAmount) external onlyOwner {
        require(giveaway_reserved > 0 && _reserveAmount <= giveaway_reserved, "Marthin Exceeds giveaway reserved Galaxy Eggs supply" );
        uint256 supply = totalSupply();
    ...
    function withdraw() public onlyOwner {
        ...
        require(balance > 0, "Marthin: withdraw all call without balance");
        require(payable(msg.sender).send(balance), "Marthin: FAILED withdraw all call");
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "Marthin URI query for nonexistent token");
