...
contract Faucet {
    ...
    function updateTokenAddress(address _tokenAddress) external onlyOwner {
        require(
            ...
            "updateTokenAddress: zero address"
        );
        tokenAddress = _tokenAddress;
    }

    function updateOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "updateOwner: zero address");
        owner = _owner;
    }
    ...
    function request(address user, uint256 amount) external onlyOwner {
        ...
        if (block.timestamp.sub(lastWithdrawnAt[user]) <= 86400) {
            require(
            ...
            lastWithdrawnAt[user] = block.timestamp;
        }
        ICoin(tokenAddress).mint(user, amount);
