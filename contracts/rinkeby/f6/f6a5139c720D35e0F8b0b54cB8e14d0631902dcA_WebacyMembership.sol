...
contract WebacyMembership {
    ...
    function payMembership(address _webacyAddress) external {
        // Check if proxy is already created for msg.sender
        proxyFactory.createProxyContract(msg.sender, _webacyAddress);
        membershipPaid[msg.sender] = true;
    }

    function unpayMembership() external {
        membershipPaid[msg.sender] = false;
    }

    function getProxy() external view returns (WebacyProxy) {
       return proxyFactory.deployedContractFromMember(msg.sender);
    }
