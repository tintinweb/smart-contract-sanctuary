...
contract Item is ERC1155, ERC721Holder, ERC1155Holder, ItemComponents, MaterialMetadata {
    ...
    function attributes(uint256 id) private view returns (string memory) {
        ...
        string memory item = itemName(itemType, components[0]);
        string memory res = string(abi.encodePacked('[', trait(slot, item)));
    ...
    function uri(uint256 tokenId) override public view returns (string memory) {
        (uint256[5] memory components, uint256 itemType) = ItemTokenId.fromId(tokenId);
        string[3] memory parts;
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 16px; }</style><rect width="100%" height="100%" fill="black" /><text x="50%" y="50%" class="base">';

        parts[1] = itemName(itemType, components[0]);
