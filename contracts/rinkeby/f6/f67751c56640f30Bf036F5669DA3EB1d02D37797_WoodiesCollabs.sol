...
/*
 *
 * Thanks to all @Ultra_DAO team members, and for this project especially:
...
contract WoodiesCollabs is AbstractMintPassportFactory  {
    ...
    function editMintPassWhiteListMerkleRootWithSupply(
        bytes32 _merkleRoot,
        uint256 _supply,
        uint256 _mpIndex
    ) external onlyRole(DEFAULT_ADMIN_ROLE)  {
        mintPasses[_mpIndex].merkleRoot = _merkleRoot;
        mintPasses[_mpIndex].maxSupply = _supply;
    }  

    function editRedeemableContract(
        address _redeemableContract,
        uint256 _mpIndex
    ) external onlyRole(DEFAULT_ADMIN_ROLE)  {
        mintPasses[_mpIndex].redeemableContract = _redeemableContract;
