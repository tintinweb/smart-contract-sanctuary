...
contract NEXT is Context, IERC20, Ownable, Pausable {
    ...
    uint256 private liquiditySellLimit = 100 * 10**18;
    uint256 private marketingSellLimit = 100 * 10**18;
    ...
    function _transfer(
        ...
        bool takeFee = false;
        if(_enableFee && (from == uniswapV2Pair || to == uniswapV2Pair)){
            _swapAndLiquify(from);
            takeFee = true;
        }
        //if any account belongs to _isExcludedFromFee account then remove the fee
        if(_enableFee && (_isExcludedFromFee[from] || _isExcludedFromFee[to])){
            takeFee = false;
        }
