...
contract FLYZTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SFLYZ }
    ...
    uint public sFLYZQueue; // Delays change to sFLYZ address
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sFLYZ ).balanceOf( msg.sender ); // Can only borrow against sFLYZ held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithFLYZ( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = 1000 * _amount.mul( 10 ** IERC20( FLYZ ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SFLYZ ) { // 9
