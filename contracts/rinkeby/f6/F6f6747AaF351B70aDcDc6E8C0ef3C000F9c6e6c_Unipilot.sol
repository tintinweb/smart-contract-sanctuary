...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    function addExchangeAddress(address exchangeAddress) external onlyGovernance {
        require(exchangeAddress != address(0), "UNIPILOT:: ZERO_ADDRESS");
    ...
    function _getAndIncrementNonce(uint256 tokenId)
        ...
        returns (uint256)
    {
        (uint256 nonce, , , , , , ) = liquidityManager.positions(tokenId);
        return nonce++;
    }
    ...
    // ) external payable returns (address pool, uint256 mintedTokenId) {
    ...
    // function deposit(IncreaseLiquidityParams memory params)
    //     public
    //     payable
    ...
    function deposit(
        IHandler.DepositParams calldata params,
        bytes calldata data
    ) external payable returns (uint256 tokenId) {
        // require(
        //     exchangeWhitelist[params.exchangeAddress] == true,
        //     "UNIPILOT:: INVALID_EXCHANGE"
        // );
        require(
            params.amount0Desired > 0 || params.amount1Desired > 0,
            "UNIPILOT::INSUFFICIENT_AMOUNTS"
        ...
        pay(params.token0, msg.sender, params.exchangeAddress, params.amount0Desired);
        pay(params.token1, msg.sender, params.exchangeAddress, params.amount1Desired);

        if (msg.value > 0) TransferHelper.safeTransferETH(params.exchangeAddress, msg.value);
        ...
        tokenId = IHandler(params.exchangeAddress).deposit(
            params.token0,
            params.token1,
            amount0,
            amount1,
    ...
    //     // require(
    //     //     exchangeWhitelist[exchangeAddress] == true,
    //     //     "UNIPILOT:: EXCHANGE_NOT_WHITELISTED"
    //     // );
    //     liquidityManager.collect(pilotToken, wethToken, recipient, tokenId, data);
    // }
    ...
    //         IUniswapLiquidityManager.WithdrawParams({
    ...
    //     //     "UNIPILOT:: INVALID_EXCHANGE"
    //     // );
    //     liquidityManager.withdrawForETH(
    //         wethToken,
    //         pilotToken,
    //         recipient,
    //         tokenId,
    ...
    // function collectFeesForWETHPairs(
    //     bool pilotToken,
    //     bool wethToken,
    //     address recipient,
    //     uint256 tokenId
    ...
    //     //     "UNIPILOT:: INVALID_EXCHANGE"
    //     // );
    //     liquidityManager.collectFeesForWETHPairs(
    //         pilotToken,
    //         wethToken,
    //         recipient,
    //         tokenId,
...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    address internal constant ORACLE = 0x02d4D49B5042703747042df43CECdd5301de50d0;
    address private constant INDEX_FUND = 0x00a0e981dbf19c96658e2e7c1BbEf46D10C2A54B;
    address internal constant UNI_STRATEGY = 0xe3870a4A37F9c506EdB2eA279f71d13B77aaa4Bd;
    address internal constant ULM_STATE = 0xCb1d82785e8dcEf926965a6b3CDdBc59D4661291;
    ...
    mapping(uint256 => Position) public override positions; // uniplot mappings
    ...
    function readjustFrequencyStatus(address pool) public returns (bool status) {
        if (block.timestamp - readjustFrequency[pool].timestamp > 600) {
    ...
    function _verifyCallback(
        ...
        require(msg.sender == getPoolAddress(token0, token1, fee), "INCB");
    }
    ...
    //     if (block.timestamp - readjustFrequency[b.poolAddress].timestamp > 600) {
    ...
    //     readjustFrequency[b.poolAddress].timestamp = block.timestamp;
    ...
    //     if (oracle.checkPoolValidation(params.token0, params.token1, amount0, amount1)) {
    ...
    //     uint256 shares,
    //     uint256 tokenId
    // ) public payable override returns (uint256 mintedTokenId) {
    //     // IncreaseLiquidityParams memory decoded = abi.decode(data, (IncreaseLiquidityParams));
    //     address pool = getPoolAddress(token0, token1, fee);
    //     LiquidityPosition storage poolPosition = liquidityPositions[pool];

    //     addLiquidityInManager(sender, pool, amount0Desired, amount1Desired, shares);
    ...
    //         // emit Deposited(decoded.tokenId, pool, amount0, amount1, shares);
    //     } else {
    ...
    //     // );
    //     Position storage position = positions[tokenId];
    //     require(position.liquidity > 0, "NLL");
    //     collectPositionFees(position.pool);
    ...
    // function checkUserFees(bytes calldata data)
    //     external
    //     onlyUnipilot
    //     returns (uint256 fees0, uint256 fees1)
    // {
    //     uint256 tokenId = abi.decode(abi.encodePacked(data), (uint256));
    //     Position storage position = positions[tokenId];
    //     collectPositionFees(position.pool);
    // (uint256 tokensOwed0, uint256 tokensOwed1, , ) = IULMState(ULM_STATE).getTokensOwedAmount(
    //     address(this),
    //     position.pool,
    //     position.liquidity,
    //     position.feeGrowth0,
    ...
    function _distributeFeesInTokens(
        ...
        if (_token0 == WETH || _token1 == WETH) {
            // hure
            if (!wethToken) {
                IWETH9(WETH9).withdraw(_tokensOwed0);
                TransferHelper.safeTransferETH(INDEX_FUND, _indexAmount0);
                TransferHelper.safeTransferETH(_recipient, _userBalance0); // transfer userFees & liquidity

                TransferHelper.safeTransfer(_token1, INDEX_FUND, _indexAmount1); // transfer indexAmount
                TransferHelper.safeTransfer(_token1, _recipient, _userBalance1); // remaining alt amount will be transfered to user
            }
        }
    ...
    function distributeFeesAndLiquidity(DistributeFeesParams memory params)
        public
        returns (uint256 indexAmount0, uint256 indexAmount1)
    {
        // only unipilot
        LiquidityPosition storage position = liquidityPositions[params.pool];
        (address token0, address token1, , , , , ) = IULMState(ULM_STATE).getPoolDetails(
            params.pool
        );
        ...
        if (token0 == WETH || token1 == WETH) {
            ...
            address ethTokenPair = oracle.checkWethPairsAndLiquidity(tokenAlt);
            ...
            address ethToken0Pair = oracle.checkWethPairsAndLiquidity(token0);
            address ethToken1Pair = oracle.checkWethPairsAndLiquidity(token1);
    ...
    function withdraw(WithdrawParams calldata params, bytes calldata data)
        external
        override
    {
        // only unipilot
        // (address recipient, uint256 tokenId, uint256 liquidity) = abi.decode(
        //     abi.encodePacked(data),
        //     (address, uint256, uint256)
        // );
        Position storage position = positions[params.tokenId];
        require(position.liquidity >= params.liquidity, "IL");

        (uint256 amount0Removed, uint256 amount1Removed) = removeLiquidityWithFees(
            false,
            position.pool,
            params.recipient,
            params.liquidity
        ...
        (uint256 indexAmount0, uint256 indexAmount1) = distributeFeesAndLiquidity(
            DistributeFeesParams({
                pilotToken: params.pilotToken,
                wethToken: params.wethToken,
                pool: position.pool,
                recipient: params.recipient,
                _tokensOwed0: position.tokensOwed0,
                _tokensOwed1: position.tokensOwed1,
                amount0Removed: amount0Removed,
                amount1Removed: amount1Removed
            })
        );
        ...
        position.liquidity = position.liquidity.sub(params.liquidity);
        position.tokensOwed0 = 0;
        position.tokensOwed1 = 0;
    }
    ...
    //     removeLiquidityWithFees(false, position.pool, recipient, liquidity);
    ...
    // function distributeFeesForETH(DistributeFeesParams memory params) public {
    //     LiquidityPosition storage position = liquidityPositions[params.pool];
    //     (address token0, address token1, , , , , ) = IULMState(ULM_STATE).getPoolDetails(
    //         params.pool
    //     );
    ...
    //             TransferHelper.safeTransfer(tokenWeth, INDEX_FUND, wethAmount);
    //         if (params._tokensOwed1 > 0)
    //             TransferHelper.safeTransfer(tokenAlt, INDEX_FUND, altAmount);
    ...
    //             sweepToken(tokenAlt, 0, params.recipient);
    //         } else { // hire
    //             (uint256 _indexAmount0, uint256 _indexAmount1, , ) = IULMState(ULM_STATE)
    //                 .getUserAndIndexShares(params._tokensOwed0, params._tokensOwed1);

    //             uint256 balanceWETH9 = IWETH9(WETH9).balanceOf(address(this)); // contains fees plus userLiquidity
    ...
    {
        (uint24 fee, uint256 tokenId) = abi.decode(abi.encodePacked(data),(uint24, uint256));
        address pool = getPoolAddress(token0,token1,fee);
        (,,totalAmount0,totalAmount1,totalLiquidity) = getTotalAmounts(pool);
    }
    ...
    function deposit(
        ...
        // IncreaseLiquidityParams memory decoded = abi.decode(data, (IncreaseLiquidityParams));
        (address sender, uint24 fee, uint256 tokenId) = abi.decode(data,(address, uint24, uint256));
        address pool = getPoolAddress(token0, token1, fee);
        LiquidityPosition storage poolPosition = liquidityPositions[pool];
    ...
    // function withdraw(bool pilotToken, bytes calldata data)
    //     external
    //     override
    //     onlyUnipilot
    // {
    ...
    //     removeLiquidityWithFees(true, position.pool, recipient, liquidity);
    ...
    // function collect(bool pilotToken, bytes calldata data)
    //     external
    //     override
    //     onlyUnipilot
    // {
...
interface IUniswapLiquidityManager is IULMEvents {
    ...
    function withdraw(WithdrawParams calldata params, bytes calldata data) external;
...
interface IHandler {
    ...
    // function withdraw(bool pilotToken, bytes calldata data) external;
    ...
    // function collect(bool pilotToken, bytes calldata data) external;
