...
contract SotaMarketV2 is Manager, ERC1155Holder, ERC721Holder, ReentrancyGuard {
	...
	function _match (
		...
		if (_addr[3] != address(0)) {
			...
			uint256 amountToBuyerRef = _data[3].mul(xUser).mul(yRefRate).div(ZOOM_FEE ** 2);	// 1.025
			_paid(_addr[1], _addr[3], amountToBuyerRef);

			if (discountForBuyer > 0) {
				// amountToBuyer = orderAmount * xUser * discountForBuyer / ZOOM_FEE ** 2
				_paid(_addr[1], _addr[0], _data[3].mul(xUser).mul(discountForBuyer).div(ZOOM_FEE ** 2));
			}
		}

		// If the order have the retailer
		if (order.retailer != address(0)) {
			uint256 amountToRetailer = _data[3].mul(order.retailFee).div(ZOOM_FEE);
		...
		if (isSOTANFTs[order.tokenAddress]) {
			...
			if (amountFirstSale[order.tokenAddress][order.tokenId] > 0 && (creator == order.owner)) {
				...
				if (_addr[2] != address(0)) {
					...
					if (amountFirstSale[order.tokenAddress][order.tokenId] >= _data[1]) {
						// amountToSellerRef = orderAmount * xCreator * yRefRate / ZOOM_FEE ** 2
						amountToSellerRef = _data[3].mul(xCreator).mul(yRefRate).div(ZOOM_FEE ** 2);
						// amountToSeller = 
						amountToSeller = amountToSeller.sub(_data[3].mul(xCreator).div(ZOOM_FEE));
						amountFirstSale[order.tokenAddress][order.tokenId] = amountFirstSale[order.tokenAddress][order.tokenId].sub(_data[1]);
					} else {
						uint256 remainedQuantity = _data[1].sub(amountFirstSale[order.tokenAddress][order.tokenId]);
						uint256 paybackAmount = remainedQuantity.mul(_data[2]);
						amountToSeller = amountToSeller.sub(paybackAmount);
						paybackAmount = paybackAmount.add(remainedQuantity.mul(_data[2]).mul(xCreator).div(ZOOM_FEE));

						amountToSellerRef = amountToSeller.mul(xCreator).mul(yRefRate).div(ZOOM_FEE ** 2);
						amountToSeller = amountToSeller.sub(amountToSeller.mul(xCreator).div(ZOOM_FEE));
				...
				if (isFarmingNFTs[order.tokenAddress] && (order.owner != creator) && farmingAmount[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] > 0) {
					...
					if (a >= _data[1]) {	// If the supply is greater than _quantity

						_paid(_addr[1], creator, _data[3].mul(zProfitToCreator).div(ZOOM_FEE));
						// amountToSeller = amountToSeller.sub(orderAmount.mul(ZOOM_FEE - zProfitToCreator).div(ZOOM_FEE));
						_paid(_addr[1], order.owner, _data[3].mul(ZOOM_FEE - zProfitToCreator).div(ZOOM_FEE));
						farmingAmount[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] = a.sub(_data[1]);
					} else {
						{
							...
							// 	);
							_paid(_addr[1], creator, a.mul(_data[2]).mul(zProfitToCreator).div(ZOOM_FEE));
							_paid(_addr[1], order.owner, a.mul(_data[2]).mul(ZOOM_FEE.sub(zProfitToCreator)).div(ZOOM_FEE));
					...
					amountToSeller = amountToSeller.sub(_data[3].mul(xUser).div(ZOOM_FEE));
					if (_addr[2] != address(0)) {
						// If the seller has referred
						// -> amountToSellerRef = amount * xUser * refRate / ZOOM_FEE ** 2
						_paid(_addr[1], _addr[2], _data[3].mul(xUser).mul(yRefRate).div(ZOOM_FEE**2));
					}
					
					// If 
					if (order.owner == creator) {	// -> no royalty fee
						_paid(_addr[1], order.owner, amountToSeller);
					} else { // -> have to calculate the royalty fee
						...
						_paid(_addr[1], order.owner, amountToSeller.sub(amountToCreator));
					}
				}
			}
		} else {
...
// 		return _updateOrder(_buyer, _paymentToken, _orderId, _quantity, _price, keccak256(abi.encodePacked(order.tokenAddress, order.tokenId)));
		...
		return _updateOrder(_addr[0], _addr[1], _data[0], _data[1], _data[2], keccak256(abi.encodePacked(order.tokenAddress, order.tokenId)));
	}
	...
	function buy(
		...
		if (_paymentToken != order.paymentToken) {	// If user doesn't use USDT to pay
		...
		// 	exactPaymentAmount = orderAmount.mul(ZOOM_FEE + xUser).div(ZOOM_FEE);
		// }
		if (_paymentToken == sota && discountForSota > 0) {		// If user pay with WAY token -> discount 50% system fee -> 1.25% system fee

			// exact_amount = exact_amount - (order_amount * (xUser * discountForSota) / ZOOM_FEE ** 2)
			exactPaymentAmount = exactPaymentAmount.sub(orderAmount.mul(discountForSota).mul(xUser).div(ZOOM_FEE ** 2));
		}

		if (_paymentToken == address(0) && msg.value > 0) {	// If use pay with ETh
			require(msg.value >= exactPaymentAmount, "The value should be greater than the exact payment amount");
		} else {
		...
		return _match([msg.sender, _paymentToken, payable(getRefData(order.owner)), payable(getRefData(msg.sender))], [_orderId, _quantity, estimateToken(_paymentToken, order.price), orderAmount]);
	}
	...
	function acceptBid(uint256 _bidId, uint256 _quantity) external whenNotPaused returns (bool) {
		...
		_match([bid.bidder, bid.paymentToken, payable(getRefData(msg.sender)), payable(getRefData(bid.bidder))], [_orderId, _quantity, bid.bidPrice, orderAmount]);
