...
contract StakingERC20 {
    ...
    function claimStake() external {
        ...
        require(IERC20(stakeToken).balanceOf(address(this)) >= StakeHolders[msg.sender].rewards, "Invalid Balance");
        ...
        updateClaimInfo();
        emit Claimed(msg.sender, StakeHolders[msg.sender].amount, StakeHolders[msg.sender].rewards);
    }
    ...
    function safeWithdrawalAfter() external onlyManager {
        require(currentBlockNumber() > stakingEndBlock, "Can't withdraw early");
        uint256 totalStakingIncludingRewards = stakingPool.add(stakingPoolRewards);
