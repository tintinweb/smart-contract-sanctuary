...
contract TST_CONTRACT111 is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 private RANGE_MIN_TOKEN_FOR_FREE_MINT = 10;
    uint256 private RANGE_MAX_TOKEN_FOR_FREE_MINT = 60;
   ...
   function atokensByOwnerBBBB(address _owner) external view returns(uint256) {
    ...
    function atokensByOwnerDDDD(address _owner) external view returns(uint256) {
        uint256 tokenCount = balanceOf(_owner);
            uint256 index;
            uint256 count;
            for (index = 0; index < tokenCount && index <= RANGE_MAX_TOKEN_FOR_FREE_MINT; index++) {
   ...
   function _totalSupply() internal view returns (uint) {
    ...
    function safeMintTWO(address to) public onlyOwner {
        uint256 totalSupply = totalSupply();
        require(totalSupply < MAX_TOKENS + 1, "Exceeds maximum tokens available for purchase");
        _safeMint(to, totalSupply);
    }
