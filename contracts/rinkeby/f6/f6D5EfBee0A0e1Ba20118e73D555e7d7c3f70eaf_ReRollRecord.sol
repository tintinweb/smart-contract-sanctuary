...
\:::_:\ \    \:\\:\\:\ \\:::_ \ \\:::_ \ \ \:::_ \ \\::::_\/_
  ...
  \::_:\ \    \:\\:\\:\ \\:\ \ \ \\: __ `\ \\:\ \ \ \\_::._\:\
  /___\:\ '    \:\\:\\:\ \\:\_\ \ \\ \ `\ \ \\:\/.:| | /____\:\
__\______/  ___ \_______\/_\_____\/ \_\/ \_\/ \____/_/_\_____\/   ___
...
\:(_) ) )_\:\/___/\ /______/\\:(_) ) )_\:\ \ \ \\:\ \    \:\ \
 \: __ `\ \\::___\/_\__::::\/ \: __ `\ \\:\ \ \ \\:\ \____\:\ \____
  \ \ `\ \ \\:\____/\          \ \ `\ \ \\:\_\ \ \\:\/___/\\:\/___/\
___\_\/ \_\/_\_____\/   __    __\_\/_\_\/_\_____\/ \_____\/ \_____\/  _________
/_____/\ /_____/\ /__/\ /__/\ /________/\/_____/\  /_______/\ /_____/\/________/\
\:::__\/ \:::_ \ \\::\_\\  \ \\__.::.__\/\:::_ \ \ \::: _  \ \\:::__\/\__.::.__\/
\:\ \  __\:\ \ \ \\:. `-\  \ \  \::\ \   \:(_) ) )_\::(_)  \ \\:\ \  __ \::\ \
 \:\ \/_/\\:\ \ \ \\:. _    \ \  \::\ \   \: __ `\ \\:: __  \ \\:\ \/_/\ \::\ \
  \:\_\ \ \\:\_\ \ \\. \`-\  \ \  \::\ \   \ \ `\ \ \\:.\ \  \ \\:\_\ \ \ \::\ \
   \_____\/ \_____\/ \__\/ \__\/   \__\/    \_\/ \_\/ \__\/\__\/ \_____\/  \__\/
*/
   ...
   ERC721
{
   ...
   uint256 PRICE = 0 * 10**15; // 0 ETH for test
   uint256 ROYALTY_PRICE = 0; // 0% of PRICE
...
contract ReRollRecord {
   ...
   ThreeWords ThreewordsContract = ThreeWords(0x9F04dE3764E3762f2332A844Cd14519Be3Aeb288);

   mapping(uint256 => bool) private _isTokenLocked;
   mapping(address => uint256) private whoLocked;
   mapping(uint256 => bytes32) private _tokenIdToPhraseId;
   mapping(uint256 => string[]) private _tokenIdToWords;
   mapping(uint256 => string[]) private lastWord1;
   mapping(uint256 => string[]) private lastWord2;
   mapping(uint256 => string[]) private lastWord3;


   //constructor
   constructor(address payable _multisig) {
       multisig = _multisig;
   // fill the lastWords with the previous contracts words
   //maybe do the same for the phraseIDs?
   }

   function lock(uint256 tokenId) public payable {
       address caller = msg.sender;
       require(!_isTokenLocked[tokenId]);
       require(ThreewordsContract.ownerOf(tokenId) == caller);
       // maybe pay to lock?
       whoLocked[caller] = tokenId;
       _isTokenLocked[tokenId] = true;
   }

   function ReRoll(uint256 tokenId, string memory _word1, string memory _word2, string memory _word3) public payable {
       address caller = msg.sender;
       require(!_isTokenLocked[tokenId]);
       require(ThreewordsContract.ownerOf(tokenId) == caller);
       uint256 w1 = compareStrings(_word1, lastWord1[tokenId][1]);
       uint256 w2 = compareStrings(_word2, lastWord2[tokenId][2]);
       uint256 w3 = compareStrings(_word3, lastWord3[tokenId][3]);

       uint256 totalPrice = 333 * 10**15*(w1+w2+w3);
       uint256 dribFee = ROYALTY_PRICE*(w1+w2+w3);
       require (msg.value >= totalPrice, "must pay reroll fee");

       uint256 multisigFee = msg.value - dribFee;
       DRIBNET.call{value: dribFee}("");
       multisig.call{value: multisigFee}("");


       bytes32 phraseId = ThreewordsContract.wordsToPhraseId(_word1, _word2, _word3);
       if (phraseId == _tokenIdToPhraseId[tokenId]) {
           emit ReRollEvent(phraseId, tokenId, _word1, _word2, _word3, true);
       }
       else{
           emit ReRollEvent(phraseId, tokenId, _word1, _word2, _word3, false);
       }
   }
...
function compareStrings(string memory a, string memory b) view returns(uint256) {
