...
contract TheGauntlet is ERC721Enumerable, CollaborativeOwnable {
  using SafeMath for uint256;
  using Address for address;
  using Strings for uint256;

  // event MintTheGauntlet(address indexed sender, uint256 startWith, uint256 quantity);
  ...
  uint256 public constant survivorCutPercentage = 6;
  ...
  address withdrawAddress0 = address(0);
  address withdrawAddress1 = address(0);
  ...
  function mint(uint256 quantity) public payable {
    ...
    // emit MintTheGauntlet(_msgSender(), ts, quantity);
  ...
  function startGauntlet() external onlyCollaborator {
    ...
    survivorCut = balance.mul(survivorCutPercentage).div(100);
  }
