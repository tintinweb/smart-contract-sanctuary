...
contract Momento is IERC20Metadata, Ownable {
  ...
  function _transfer(
    ...
    if (from != address(this) && to != address(this)) {
      ...
      if (
        ...
        // don't enforce max transfer if excluded or adding/removing liquidity
        if (
          !(_isExcludedFromFee[from] || _isExcludedFromFee[to]) &&
          to != address(uniswapV2Router) &&
          from != address(uniswapV2Router)
        ) {
          _enforceMaxTransferSellPerDay(amount);
        }
  ...
  function _enforceMaxTransferSellPerDay(uint256 amount) private {
