...
contract NFT is ERC721, PaymentSplitter {
  ...
  uint public constant reservedAmount = 100; // TODO set final value
  ...
  uint public saleStartDate = block.timestamp + 1 hours; // TODO change to correct duration
  
  string public baseUri = ""; 
  string public baseExtension = ".json"; 
  
  mapping(address => uint) public mintedTokens;
  ...
  function mintSale(uint amount) external payable onlyEOA {
    ...
    require(mintedTokens[msg.sender] + amount <= maxMintsPerAddress, "Address already minted");
    require(msg.value == calculateTokenPriceDA() * amount , "Incorrect presalePrice sent");
  ...
  function mintWhitelist(uint amount, bytes32[] calldata proof) external payable onlyEOA {
    ...
    require(mintedTokens[msg.sender] + amount <= maxMintsPerAddress, "already minted max amount");
