...
contract WLD is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor(
		...
		for (uint256 i = 1; i < 518; i++) {
			_isOccupiedId[i] = true;
			_occupiedList.push(i);
		}
    }
    ...
    function reserveTokens(uint256[] memory _ids) public {
		require(_msgSender() == owner() || _msgSender() == devAddress, "Owner could call reserveTokens function!");
		require(totalSupply() + _ids.length <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
    ...
    function mintToken( uint256[] memory _ids) public payable {
        ...
        require(_ids.length <= maxTokenPurchase, "Exceeded max token purchase");
        require(_numberOfWallets[msg.sender] + _ids.length <= maxTokenPerWallet, "Exceeded max token purchase per wallet");
        require(totalSupply() + _ids.length <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
        require(price * _ids.length <= msg.value, "Ether value sent is not correct");
        ...
        _numberOfWallets[msg.sender] = _numberOfWallets[msg.sender] + _ids.length;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        payable(msg.sender).transfer(balance.div(100).mul(88));
