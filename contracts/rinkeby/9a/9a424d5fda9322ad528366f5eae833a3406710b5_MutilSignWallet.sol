...
contract MutilSignWallet { 
    ...
    constructor(string memory _walletName, uint _maxOners, uint _maxApprover,address _tokenAddress) {
    ...
    function applyAddManagers(address[] memory _owners) public payable {
    ...
    function approveAddOwners(address addNonce) public payable{
    ...
    function applyARemoveOwners(address [] memory _removeOwners) public payable{
    ...
    function applyTransaction(address _to, uint256 _value) public {
    ...
    function approveTransactions() public {
        require(owners[msg.sender], "Your are not right to apply send transaction");
        approveTransOwners[applyTransOwner].push(msg.sender);
