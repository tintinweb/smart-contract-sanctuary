...
//payment splitter 
contract Bear is IBear, ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
  ...
  constructor(address _honey, address _traits, uint256 _maxTokens) ERC721("Bear Game", 'BGAME')  { 
    honey = HONEY(_honey);
 ...
 //   uint256 public Price = 8e16; //0.08 ETH
    ...
    // body
    rarities[1] = [255, 255, 255, 255, 255, 20, 80, 20, 255, 255, 255, 255, 80, 255, 255, 255];
    aliases[1] = [0, 1, 2, 3, 4, 3, 10, 11, 8, 9, 10, 11, 13, 13, 14, 15];
    // ears
    rarities[2] =  [255];
    aliases[2] = [0];
    // eyes
    rarities[3] = [245, 255, 249, 251, 252, 255, 78, 83, 255];
    aliases[3] = [1, 2, 5, 8, 1, 8, 1, 4, 5];
    // face
    rarities[4] = [253, 255, 79, 250, 246, 247, 244, 253, 250, 255, 239, 254, 21, 255, 255, 24];
    aliases[4] = [0, 1, 3, 4, 5, 6, 7, 8, 9, 10, 11, 13, 1, 1, 13, 14];
    // head
    rarities[5] = [31, 78, 227, 228, 255, 28, 255, 245, 81, 243, 233, 255, 229, 254, 243, 255, 251, 225, 227, 228, 235, 23];
    aliases[5] = [3, 2, 4, 7, 6, 4, 10, 11, 12, 13, 14, 6, 16, 11, 18, 19, 19, 11, 20, 9, 3, 6];
    // mouth
    rarities[6] = [243, 255, 230, 229, 255, 243, 255, 251, 225, 244, 71, 68, 245, 255, 245];
    aliases[6] = [0, 1, 1, 3, 4, 5, 6, 4, 8, 9, 9, 12, 13, 13, 14];
    // shoulder(wing or neck)
    rarities[7] = [255, 233, 230, 229, 224, 84, 29, 255, 240];
    aliases[7] = [1, 0, 5, 0, 1, 7, 1, 3, 7];
    // basebody
    rarities[8] = [255];
    aliases[8] = [0];
    // senseIndex, void for bees
    rarities[9] = [255];
    aliases[9] = [0];
    ...
    // body
    rarities[11] = [210, 255, 240, 31, 245, 244, 239, 84, 255, 29, 91];
    aliases[11] = [0, 1, 2, 1, 4, 4, 6, 8, 0, 5, 1];
    // ears
    rarities[12] = [24, 30, 33, 255];
    aliases[12] = [3, 3, 3, 3];
    // eyes
	rarities[13] = [235, 247, 239, 241, 76, 225, 235, 255, 234, 239, 29];
    aliases[13] = [1, 2, 3, 0, 5, 6, 7, 7, 7, 8, 9];
    // face
    rarities[14] = [243, 64, 251, 73, 236, 26, 233, 99];
    aliases[14] = [0, 0, 0, 2, 2, 2, 4, 6];
    // head
    rarities[15] = [75, 25, 74, 238, 28, 255, 235, 230, 89, 81, 73, 241];
    aliases[15] = [5, 5, 5, 11, 11, 3, 3, 5, 6, 6, 7, 7];
    // mouth
    rarities[16] = [25, 237, 72, 246, 34, 255, 255, 239, 245, 235, 245, 239, 89, 230];
    aliases[16] = [6, 6, 6, 5, 5, 5, 4, 10, 10, 13, 13, 1, 1, 7];
    // shoulder(neck or wing) 
    rarities[17] = [78, 76, 255, 19]; 
    aliases[17] = [2, 2, 2, 2];
    // basebody
    rarities[18] = [255];
    aliases[18] = [0];
...
//add the logic honey tokens to mint cubs based on price
  ...
  function selectTraits(uint256 seed) internal view returns (BeesCubs memory t) {    
    ...
    uint8 shift = t.isBee ? 0 : 10;
    seed >>= 16;
    t.background = selectTrait(uint16(seed & 0xFFFF), 0 + shift);
    seed >>= 16;
    t.baseBody = selectTrait(uint16(seed & 0xFFFF), 8 + shift);
    ...
    seed >>= 16;
    t.face = selectTrait(uint16(seed & 0xFFFF), 4 + shift);
    seed >>= 16;
    t.head = selectTrait(uint16(seed & 0xFFFF), 5 + shift);
    seed >>= 16;
    t.mouth = selectTrait(uint16(seed & 0xFFFF), 6 + shift);
    seed >>= 16;
    t.shoulder = selectTrait(uint16(seed & 0xFFFF), 7 + shift);
    seed >>= 16;
    t.senseIndex = selectTrait(uint16(seed & 0xFFFF), 9 + shift);
  }
...
contract HONEY is ERC20, Ownable, ReentrancyGuard {
...
//only ever called once for community surprise
  function Surprisemint(bool _didWeGetTheReserves) public onlyOwner nonReentrant {
