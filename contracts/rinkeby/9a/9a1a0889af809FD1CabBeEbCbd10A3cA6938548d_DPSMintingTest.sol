...
contract DPSMintingTest is ReentrancyGuard, Ownable {
  bytes32 merkleRoot;
  ...
  uint256 public PRICE = 0.001 ether; // 1 MOVR
  ...
  function claim(bytes32 _leaf, bytes32[] calldata _merkleProof) external payable nonReentrant {
    ...
    require(DPSI(DPS).totalSupply() <= DPSI(DPS).getMaxSupply(), "Cap Reached");
    ...
    mintedAlready++;
    uint256 id = mintedAlready;
    DPSI(DPS).mint(msg.sender, id);
    require(id > 0 && id <= DPSI(DPS).getMaxSupply(), "Minting failed, try again please");
    payable(devAddr1).transfer((msg.value * 30) / 1000);
    payable(devAddr2).transfer((msg.value * 20) / 1000);
  ...
  function eligibleThisRound(address _collector) external view returns (bool) {
    if (DPSI(DPS).tokensOfOwner(_collector).length >= 5) return false;
    if (block.timestamp < lastDayStart) return false;
    uint256 timeElapsed = block.timestamp - lastDayStart;
    uint256 daysElapsed = timeElapsed / ROUND;

    return lastMintTimePerHolder[_collector] < lastDayStart + (daysElapsed * ROUND);
  }
