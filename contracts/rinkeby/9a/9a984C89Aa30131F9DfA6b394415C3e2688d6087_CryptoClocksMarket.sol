...
contract CryptoClocksMarket {

    // You can use this hash to verify the image file containing all the punks
    string public imageHash = "97f4182ed9392db048b75318d33d7508cd82d9e146267c14abe68220de968072";

    address owner;

    string baseURI;
    string public standard = "CryptoClocks";
    string public name;
  ...
  constructor(
    ...
    function PayCryptoClocks() public payable {
        //        balanceOf[msg.sender] = initialSupply;              // Give the creator all initial tokens
        owner = msg.sender;
        totalSupply = 10000;                        // Update total supply
        punksRemainingToAssign = totalSupply;
        name = "CryptoClocks";                                   // Set the name for display purposes
        symbol = "CCX";                               // Set the symbol for display purposes
        decimals = 0;                                       // Amount of decimals for display purposes
    }

    function _setInitialOwner(address to, uint punkIndex) private {
    ...
    function setInitialOwners(address[] memory addresses) view public {
        require(msg.sender != owner);

        uint length = addresses.length;

        for (uint i = 0; i < length; i++) {
   //         _setInitialOwner(addresses[i], data[i]);
        }
    }
