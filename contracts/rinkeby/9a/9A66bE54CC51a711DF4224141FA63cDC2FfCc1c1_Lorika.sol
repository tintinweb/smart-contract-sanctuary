...
contract Lorika is ERC721Enumerable, Ownable, ReentrancyGuard {

	uint256 public constant LR_MAX_AMOUNT = 3_333;
	uint256 public LR_PER_TX = 10;
	uint256 public PRICE = 0.1 ether;
	uint256 public constant TIME_FOR_GROWN = 7 days;
	address public founder = 0x8941DC4A6E85EcF5Cda3eBe364B1C8E657fa40CB;
	...
	function mintPublic(uint256 amount) external payable nonReentrant() {
		...
		require(amount <= LR_PER_TX, "You can't mint that many tokens");
		require(totalSupply() + amount <= LR_MAX_AMOUNT, "Mint would exceed max supply of tokens");
	...
	function enableClaimFoundingMembers(address[] memory members, uint256[] memory amounts) external onlyOwner {
		...
		for (uint256 i = 0; i < members.length; i++) {
			...
			require(amounts[i] <= LR_PER_TX, "amount is more than the allowable");
	...
	function setIpfsHash(string[] memory ipfsHashs, uint256[] memory idTokens) external onlyOwner {
		...
		for (uint256 i = 0; i < idTokens.length; i++) {
			...
			require(idTokens[i] <= LR_MAX_AMOUNT, "id is more than the allowable");
	...
	function makePresaleMemberMint(uint256 tokenAmount, uint256 etherAmount) private {
		...
		require(totalSupply() + tokenAmount <= LR_MAX_AMOUNT, "Mint would exceed max supply of tokens");
		require(mintedPresaleCount + tokenAmount <= LR_MAX_AMOUNT, "Mint would exceed max pre-sale tokens");
	...
	function setEntityBirthdaysPerTx(uint256 newPerTx) public onlyOwner {
		...
		emit SetNewValue("LR_PER_TX", LR_PER_TX, newPerTx);
		LR_PER_TX = newPerTx;
	}
