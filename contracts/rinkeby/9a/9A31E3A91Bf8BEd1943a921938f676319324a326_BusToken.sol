...
contract BusToken is ERC721, Ownable, Pausable, ReentrancyGuard {
    ...
    uint public priceInEth = 0.05 ether; // TODO: change price
    uint public minPriceInEth = 0.007 ether; // TODO: change price
    uint public priceInPenny = 50000 ether; // TODO: change price
    uint public startSaleDate;

    mapping(address => bool) public approvedManagers;
    mapping(uint16 => uint) public phasePrice;
    mapping(uint16 => bool) public bigCapacityMap;
    IPenny public penny;
    ...
    constructor() ERC721("BusToken", "BusTest") {
        _pause();
        ...
        // Initialization
        fillTokens(0, 100);
        fillPaidTokens(101, 200);
        
        // Init random with some values
    ...
    function isBigCapacity(uint16 id) public view returns (bool) {
        return bigCapacityMap[id];
    }
    ...
    // The price drops from 0.5 up to 0.07
    function mintPrice(uint _amount) public view returns (uint) {
        uint diff = block.timestamp - startSaleDate;
        if (diff > 1 days) return priceInEth * _amount; // Base price might be updated by devs after 1 day

        // Every 10 minutes the price changes MINUS 0.003 ether
        // uint price = priceInEth - (diff / 10 minutes) * 0.003 ether;
  
        uint changingPart = (diff / 120 seconds) * 0.003 ether;
        if (changingPart >= priceInEth) return minPriceInEth * _amount;

        return (priceInEth - changingPart) * _amount;
    }

    function mintPriceForPenny(uint _amount) public view returns (uint) {
        return _amount * priceInPenny;
    }
    ...
    function changePriceInPenny(uint _weiPrice) public onlyOwner {
        priceInPenny = _weiPrice;
    }
    ...
    function shuffleSeeds(uint _seed, uint _max) external onlyOwner {
        uint shuffleCount = random(_seed, _max);
