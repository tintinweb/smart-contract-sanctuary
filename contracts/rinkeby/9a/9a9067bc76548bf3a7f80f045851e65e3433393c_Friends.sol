...
contract FriendsBase is ERC20Permit, Configurable, __AdminUpgradeabilityProxy0__ {
...
contract Friends is FriendsBase {
    ...
    //function totalSupply_() public view returns(uint) {
    ...
    function balanceOf_(address who) public view returns(uint bal) {
        bal = _balances[who];
    ...
    function transferFrom(address from, address to, uint256 amt) public extend virtual override returns(bool) {
    ...
    function setCmpd(bool isCmpd) public extend {
    ...
    function APY() public view returns(uint y) {
        (, uint r, uint period) = calcRebaseProfit(address(0));
        r = r.add(1e18);
        y = 1e18;
    ...
    function calcRebaseProfit(address who) public view returns(uint profit, uint ratio, uint period) {
        period = config[_rebasePeriod_];
        profit = totalProfit.mul(period).div(config[_rebaseSpan_]);
        profit = profit.sub(profit.mul(config[_ecoRatio_]).div(1e18));
        ratio = profit.mul(1e18).div(_totalSupply.sub(flatSupply));
        if(who != address(0))
            profit = profit.mul(balanceOf(who)).div(_totalSupply.sub(flatSupply));
    }
    
    function rebase() public extend {
    ...
    modifier compound(bytes32 referrer) {
     ...
     function verify(address sender, bytes32 referrer, uint nonce, Twitter[] memory twitters, Signature[] memory signatures) public extend {
        require(twitters.length > 0, "missing twitters");
        require(getConfig(_denyVerify_) == 0, "denyVerify");
    ...
    function airClaim(bytes32 referrer, uint nonce, Twitter[] memory twitters, Signature[] memory signatures) external payable extend {
        require(getConfig(_denyAirClaim_) == 0, "denyAirClaim");
        rebase();
    ...
    function buyMore(bytes32 referrer, uint nonce, Twitter[] memory twitters, Signature[] memory signatures) external payable compound(referrer) extend {
        address sender = _msgSender();
        verify(sender, referrer, nonce, twitters, signatures);
    ...
    function buy(bytes32 referrer) external payable compound(referrer) extend {
    ...
    function calcOut(address sender, Twitter[] memory twitters, uint value) public view returns(uint a) {
        a = moreQuotaOf(sender, twitters);
        uint r1 = calcRatio1();
    ...
    function _buy(address sender, uint value) internal {
        require(getConfig(_denyBuy_) == 0, "denyBuy");
        if(value == 0)
            return;
...
contract FriendsEx is FriendsBase {

    function __Friends_init() public initializer {
    ...
    function __Friends_init_unchained() public initializer {
        index = 1e18;
    ...
    function setBuf_(uint buf, uint price) external governance {
        if(price == 0)
            price = Friends(address(this)).price1At(0);
        require(buf <= uint112(-1), "buyBuf OVERFLOW");
        (_buyBuf, _lastUpdateBuf) = (uint112(buf), uint32(now));
        config[_factorPrice_] = price.mul(Friends(address(this)).buyTotal()).div(buf);
    }
    ...
    function buyback_(uint value) external governance {
        ...
        address pair = IUniswapV2Factory(config[_swapFactory_]).getPair(WETH, address(this));
        require(config[_buybackAnytime_] > 0 || _totalSupply.mul(Friends(address(this)).price2()).div(1e18) < address(this).balance.add(IERC20(WETH).balanceOf(pair).mul(2)), "price2 should below net value");
        address[] memory path = new address[](2);
