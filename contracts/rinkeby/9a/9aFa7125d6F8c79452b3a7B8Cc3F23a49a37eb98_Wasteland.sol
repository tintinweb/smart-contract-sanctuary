...
contract Wasteland is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    modifier isApprovedForAll() {
        require(WastelandCactusCrew.isApprovedForAll(msg.sender, address(this)));
        _;
    }

    function getClaimable(address account) public view returns (uint256) {
        return rewards[account] + _getPending(account);
    }

    function _getPending(address account) internal view returns (uint256) {
        return owned[account].length
        * BASE_RATE
    ...
    function _update(address account) internal {
        rewards[account] += _getPending(account);
    ...
    function _claim(address account) internal whenNotPaused {
        ...
        _mint(account, getClaimable(account));
        rewards[account] = 0;
        lastUpdate[account] = block.timestamp;
    }
    ...
    function unstake(uint256 tokenId, bool claimTokens) external nonReentrant {
        require(owners[tokenId] == msg.sender, "Caller is not token owner");
        ...
        WastelandCactusCrew.transferFrom(address(this), msg.sender, tokenId);
    }

    function unstakeMany(uint256[] calldata tokenIds, bool claimTokens) external nonReentrant {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(owners[tokenIds[i]] == msg.sender, "Caller is not token owner");
        }
        ...
        uint256[] storage ownedTokens = owned[msg.sender];
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ownedTokens[index[tokenIds[i]]] = ownedTokens[ownedTokens.length - 1];
            delete ownedTokens[ownedTokens.length - 1];
            owners[tokenIds[i]] = address(0);
    ...
    function unstakeAll() external nonReentrant {
        if (!paused()) _claim(msg.sender);
        else _update(msg.sender);

        uint256[] storage ownedTokens = owned[msg.sender];
        for (uint256 i = 0; i < ownedTokens.length; i++) {
            uint256 currentToken = ownedTokens[ownedTokens.length - 1];
            ownedTokens.pop();
            owners[currentToken] = address(0);

            WastelandCactusCrew.transferFrom(address(this), msg.sender, currentToken);
        }
    }
    ...
    function recoveryTransfer(address from, address to, uint256 tokenId) external onlyOwner {
        if (from == address(WastelandCactusCrew)) {
            require(owners[tokenId] == address(0), "Token was not transferred accidentally");
        }

        IERC721(from).transferFrom(address(this), to, tokenId);
    }
}
