...
contract NFT is ERC721Enumerable, Ownable {
  ...
  uint256 public maxSupplyPresale = 100;
  uint256 public balancePresale = 100;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(supply + _mintAmount <= maxSupply + balancePresale, "max NFT limit exceeded");
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      ...
      _safeMint(msg.sender, supply + i);
        if(onlyWhitelisted == true) {
            balancePresale = balancePresale - 1;
        }
    }
  }
