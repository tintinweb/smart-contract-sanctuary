...
contract CurseWords is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        uint256 randPrimSize = seed + 28 % 128;
        uint256 randSecondarySize = seed + 4 % 28;
        ...
        parts[1] = 'px}</style><rect width="100%" height="100%" fill="black" />';

        parts[2] = string(abi.encodePacked('<text x="', randomX, '" y="20" class="base">'));

        parts[3] = getCurseWord(tokenId);

        parts[4] = string(abi.encodePacked('</text><text x="10" y="', randomY, '" class="aux">'));

        parts[5] = getSecondaryCurseWord(tokenId);
