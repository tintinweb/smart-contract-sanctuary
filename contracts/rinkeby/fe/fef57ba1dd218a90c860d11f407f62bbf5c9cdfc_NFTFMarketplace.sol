...
error InvalidTokenAddress(address contractAddress);
    ...
    IERC1155ReceiverUpgradeable
{
    ...
    mapping(uint256 => uint256) private _tokenReservedAmounts;

    mapping(uint256 => uint256) private _tokenPricesInWei;
    ...
    function _getTokenPresentAmount(uint256 tokenId, bool reserved)
        private
        view
        returns (uint256)
    {
        uint256 tokenReservedAmount = _tokenReservedAmounts[tokenId];
    ...
    function _getTokenPriceInWei(uint256 tokenId)
        ...
        returns (uint256)
    {
        return _tokenPricesInWei[tokenId];
    }
    ...
    /// @dev Checks if `minimumPresentAmount` tokens present (listed or reserved)
    modifier isTokensPresentAtLeast(
        uint256 tokenId,
        uint256 minimumPresentAmount,
        bool reserved
    ) {
        uint256 presentAmount = _getTokenPresentAmount(tokenId, reserved);

        if (presentAmount < minimumPresentAmount)
            revert InsufficientTokenAmount(presentAmount, minimumPresentAmount);
    ...
    ) private isTokensPresentAtLeast(tokenId, amount, transferReserved) {
    ...
    function _reserveToken(uint256 tokenId, uint256 amount)
        private
        isTokensPresentAtLeast(tokenId, amount, false)
    {
        _tokenReservedAmounts[tokenId] += amount;
    }

    function _unreserveToken(uint256 tokenId, uint256 amount)
        private
        isTokensPresentAtLeast(tokenId, amount, true)
    {
        _tokenReservedAmounts[tokenId] -= amount;
    }
    ...
    function setTokenPrice(uint256 tokenId, uint256 priceInWei)
        ...
        onlyOwner
    {
        _tokenPricesInWei[tokenId] = priceInWei;
    }
        ...
        )
    {
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            tokenListedAmounts[index] = _getTokenPresentAmount(tokenId, false);
