...
contract Token is ERC20 {
  ...
  string[] memory _students = getContractInterface(studentContract).getStudentsList();
  uint _length = _students.length;
  return _length;
  }  

  receive() external payable  {
    int256 num = priceConsumerV3Inreface(priceContract).getLatestPrice();
    uint num1;

    num1 = uint256(num);

    _mint(msg.sender, num1*10**18);
  }
