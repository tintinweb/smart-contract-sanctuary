...
contract FineYachtsHarbourClub is ERC721, Ownable, ReentrancyGuard {
    ...
    mapping (address => uint256) private presaleLimitPerWallet;
    mapping (address => uint256) private limitPerWallet;
    ...
    function mint(uint256 amount) public payable {
        ...
        if(limitPerWallet[msg.sender] + amount > PUBLIC_MAX_TX) revert WalletLimitReached(PUBLIC_MAX_TX);
        if(msg.value < PRICE * amount) revert InvalidPriceSentForAmount(msg.value, PRICE * amount);

        for(uint256 i = 0; i < amount; i++) {
            _mint(msg.sender, totalSupply);
        }
        limitPerWallet[msg.sender] += amount;
    }
    ...
    function presaleMint(uint256 amount, bytes memory signature) public payable {
        ...
        if(presaleLimitPerWallet[msg.sender] + amount > PRESALE_MAX_TX) revert WalletLimitReached(PRESALE_MAX_TX);
        if(msg.value < PRICE * amount) revert InvalidPriceSentForAmount(msg.value, PRICE * amount);

        for(uint256 i = 0; i < amount; i++) {
            _mint(msg.sender, totalSupply);
        }
        presaleLimitPerWallet[msg.sender] += amount;
    }
