...
contract WooInuV2 is ERC20("Nature boy inu", "WOOINU"), Ownable, IWooInuV2 {
    /// @dev Burn address and max number
    address internal constant BURN = 0x000000000000000000000000000000000000dEaD;
    uint256 internal constant MAX =
        0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;

    /// @dev The woo inu v1 token, for migrations
    IERC20 internal immutable v1Token;

    /// @dev The current fees
    Fees private _currentFees;

    /// @dev Store the amount that hasn't been distributed yet
    StoredFees private _feesStored;
    ...
    constructor() {
        v1Token = IERC20(0xB3Dd9764Ab0906176781554196C7BC8aB136511f);
        ...
        _currentFees = Fees(
            ...
            3, // Sell dev fee
            3, // Sell liquidty fee
            1, // Max sell percentage (from tokens in uni pair)
            1000000000000000e9, // Max transfer (number)
            100000000000000e9, // Threshold to distribute
            address(0x6fB1FE07C51ab844Ddd6d4C8f04186e92fe3961F), // Dev wallet
            address(0x6fB1FE07C51ab844Ddd6d4C8f04186e92fe3961F) // PPV Wallet
        );
    }
    ...
    /**
     * @dev Distributes the fees in the contract; Adds liquidity, and transfers dev and PPV fees
     */
    function distributeFees() public override {
        StoredFees memory feesToDistribute = _feesStored;
        /// @dev Reset to 0
        delete _feesStored;
        ...
        transferNoFee(
            address(this),
            _currentFees.devWallet,
            feesToDistribute.toDevWallet
        );
        transferNoFee(
            address(this),
            _currentFees.ppvWallet,
            feesToDistribute.toPpvWallet
        );
        WooInuUtils.swapAndLiquify(
            uniswapV2Router,
            feesToDistribute.toLiquidity,
            BURN
        );
    ...
    function _takeFees(
        ...
        if (liquidityFee != 0) {
            ...
            _feesStored.toLiquidity += toLiquidity;
            feeTotal += toLiquidity;
        }
        transferNoFee(from, address(this), feeTotal);

        /// @dev If enough tokens to distribute fees, and it can distribute fee, distribute them
        if (
            from != uniswapV2Pair &&
            balanceOf(address(this)) >= _currentFees.thresholdToAutoDistribute
        ) distributeFees();
    ...
    function takeFees(
        ...
        } else if (to == address(uniswapV2Router) || to == uniswapV2Pair) {
            /// @dev Is a sell
    ...
    /**
     * @dev Take a fee before a transfer
     */
    function _transfer(
        address sender,
        address recipient,
    ...
    /**
     * @dev Transfers tokens without taking a fee
     */
...
library WooInuUtils {
     ...
     */
    function addLiquidityETH(IuniswapV2Router uniswapV2Router, uint256 amountToken, uint256 amountETH, address to) internal {
     ...
     * @param uniswapV2Router: The uniswap router to conduct the swap + liquidity add through
    ...
    function swapAndLiquify(
        ...
        swapForETH(uniswapV2Router, half, address(this));
        addLiquidityETH(uniswapV2Router, half, address(this).balance, to);
    }
}
...
interface IWooInuV2 {
    ...
    event FeesDistributed(uint256 liquidityFee, uint256 devFee, uint256 ppvFee);
     ...
     * @param thresholdToAutoDistribute: The amount of tokens needed in the contract before it distributes them
    ...
    struct Fees {
        ...
        uint256 maxSell;
        uint256 maxTransfer;
        uint256 thresholdToAutoDistribute;
        address devWallet;
        address ppvWallet;
    }

    /**
     * @dev Stores the amount of fees to be taken in the contract, that can be distributed
     * @param toDevWallet: The amount of fees to be sent to the dev wallet
     * @param toPpvWallet: The amount of fees to be sent to the PPV wallet
     * @param toLiquidity: The amount of fees to be sent to liquidity
     */
    struct StoredFees {
        uint256 toDevWallet;
        uint256 toPpvWallet;
