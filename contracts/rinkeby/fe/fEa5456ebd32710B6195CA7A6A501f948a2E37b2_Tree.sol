...
contract Tree is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  mapping(uint256 => uint256) growth;

  uint256 public maxViewLevel = 17;
  uint256 maxCountLevel = 17;

  uint256 public price = 0.003 ether;

  function setMaxViewLevel(uint256 _maxViewLevel) public {
  ...
  function grow(address friend) public payable {
    require(msg.value >= price * 100, 'PRICE_NOT_MET');
    uint256 tokenId = tokenOfOwnerByIndex(friend, 0);
    growth[tokenId] += 1;
  }
  ...
  function getTreeLevel(uint256 tokenId) public view returns (uint256) {
    return treeSprouts[tokenId] + growth[tokenId];
  }

  string[] colors = ['28FFD5', '27DAFF', 'FFD2FA', 'FC00B4', '2600FC'];

  function levelXml(uint256 level) public view returns (string memory) {
    return
      string(
        abi.encodePacked(
          "<g id='l",
          ...
          "'><use xlink:href='#l",
          toString(level - 1),
          "' transform='translate(0, -1) rotate(-45) scale(.7)'></use><use xlink:href='#l",
          toString(level - 1),
          "' transform='translate(0, -1) rotate(+22) scale(.7)'></use><use xlink:href='#stem' stroke='#",
