...
abstract contract UnifarmV3 is Admin {
    ...
    function viewStakingDetails(address _user) external virtual  view returns (address[] memory,
        address[] memory, bool[] memory, uint8[] memory, uint256[] memory, uint256[] memory, uint256[] memory);
}
...
contract unifarmV3Fixed is Ownable {
    ...
    constructor(address v3Address, address oldPaid, address newPaid) Ownable(msg.sender)   {
    ...
    function transferFromContractV3(address tokenAddress) internal {
        uint bal = IERC20(tokenAddress).balanceOf(address(UniV3));
        
        if(bal > 0)
            UniV3.safeWithdraw(tokenAddress, bal);
    }
    ...
    function getOneDayReward(uint256 stakedAmount,address stakedToken,address rewardToken,uint256 totalStake) public view returns (uint256 reward) {
        reward = (stakedAmount.mul(UniV3.tokenDailyDistribution(stakedToken,rewardToken))).div(totalStake);
    ...
    function unStake(address user, uint256 stakeId) external  {
        ...
        require(msg.sender == user || msg.sender == _owner, "UNSTAKE: Invalid User Entry");
        
        
        (,address[] memory tokenAddress,bool[] memory activeStatus,uint8[] memory stakeOption, ,
            uint256[] memory stakedAmount,uint256[] memory startTime) = (UniV3.viewStakingDetails(user));
            
        ( ,,,,, uint256 lockableDays, bool optionableStatus) = UniV3.tokenDetails(tokenAddress[stakeId]);
        ...
        if(optionableStatus) {
            ...
            if(stakeOption[stakeId] == 1)
                require(startTime[stakeId].add(stakeDuration) <= block.timestamp, 
                "Locked in optional lock");
            else if(stakeOption[stakeId] == 2)
                require(poolStartTime.add(stakeDuration) <= block.timestamp, 
                "Locked in optional lock");
        ...
        // Checks
         ...
         (address stakedToken, uint totalStaking) = swapPaidToken(tokenAddress[stakeId]);
        

        // Balance check
        require(
            IERC20(stakedToken).balanceOf(
                address(this)
        ...
        // State updation
        totalUnStakingB[stakedToken] = totalUnStakingB[stakedToken].add(stakedAmount[stakeId]);
    ...
    function claimRewards(address user, uint256 stakeId, uint256 totalStaking) internal {
        
        
        (address[] memory referrerAddress,address[] memory tokenAddress, ,uint8[] memory stakeOption, ,
            uint256[] memory stakedAmount,uint256[] memory startTime) = (UniV3.viewStakingDetails(user));
        ...
        if(stakeOption[stakeId] == 1)  {
            interval = startTime[stakeId].add(stakeDuration);
            if (interval > block.timestamp)
                endOfProfit = block.timestamp;
            else    
                endOfProfit = startTime[stakeId].add(stakeDuration);
        ...
        interval = endOfProfit.sub(startTime[stakeId]);
         ...
         // Reward calculation
        if (interval >= HOURS)
            _rewardCalculation(user, referrerAddress[stakeId], tokenAddress[stakeId], stakedAmount[stakeId], interval, totalStaking);
    }
    
     function _rewardCalculation(address user, address referrerAddress, address stakedToken, uint256 stakedAmount, uint256 interval, uint256 totalStake
    ) internal {
        ...
        if (referrerAddress != address(0)) {
            ...
            if(stakedToken != oldPaidAddress) {

                require(IERC20(stakedToken).transfer(
                    referrerAddress,
                ...
                require(IERC20(newPaidAddress).transfer(
                    referrerAddress,
                    refEarned) == true, "Transfer Failed");
    ...
    function swapPaidToken(address tokenAddress) internal  returns(address, uint256) {
        transferFromContractV3(tokenAddress);
        if(tokenAddress == oldPaidAddress) {
            uint256 totalStaking = UniV3.totalStaking(tokenAddress).add(UniV3.totalStaking(newPaidAddress));
            uint256 totalUnstakingB1 = totalUnStakingB[tokenAddress].add(totalUnStakingB[newPaidAddress]);
            uint totalStaking1 = totalStaking.sub(totalUnstakingB1);
            ...
            uint totalStaking1 = totalStaking.sub(totalUnStakingB[tokenAddress]);
    ...
    function updateV3Address(address v3Address) external onlyOwner returns(bool) {
        UniV3 = UnifarmV3(v3Address);
        return true;
    }
    
    function updateOldPaidAddress(address oldPaid) external onlyOwner returns(bool) {
        oldPaidAddress = oldPaid;
        return true;
    }
    
    function updateNewPaidAddress(address newPaid) external onlyOwner returns(bool) {
        newPaidAddress = newPaid;
        return true;
    }
    ...
    function emergencyUnstake(uint256 stakeId, address userAddress, address[] memory rewardtokens, uint256[] memory amount) external onlyOwner {
        ...
        (address[] memory referrerAddress,address[] memory tokenAddress,bool[] memory activeStatus, , ,
            uint256[] memory stakedAmount,) = (UniV3.viewStakingDetails(userAddress)); 
        ...
        transferFromContractV3(tokenAddress[stakeId]);
            ...
            require(IERC20(rewardtokens[i]).balanceOf(address(this)) >= amount[i],
                ...
                uint rewardsEarned = amount[i];
                 ...
                 if (referrerAddress[stakeId] != address(0)) {
                        ...
                        require(IERC20(rewardtokens[i]).transfer(referrerAddress[stakeId], refEarned), "EMERGENCY : Transfer Failed");
