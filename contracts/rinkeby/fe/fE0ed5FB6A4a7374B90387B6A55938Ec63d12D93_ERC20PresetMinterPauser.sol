...
contract SampleERC20 is ERC20PresetMinterPauser("Sample ERC20", "SAM") {
    function empower() public {
        empower(msg.sender);
        mint(msg.sender, 10 ether);
    }

    function empower(address subject) public {
        grantRole(DEFAULT_ADMIN_ROLE, subject);
        grantRole(MINTER_ROLE, subject);
        grantRole(PAUSER_ROLE, subject);
    }
}
...
contract Jukebox is Ownable, IJukebox, IERC721Receiver {
        ...
        returns (bytes32 salt)
    {
        ...
        emit OrderCreated(order, maxFills, address(order.erc20), order.nftBeneficiary, salt);
    }
...
contract IdValidator is IERC721Validator, Ownable {
    ...
    function initialize(
        address _owner,
        IJukebox.ERC721Grouping[] calldata tokenGroupings
    ) external {
        require(owner() == address(0));
        _transferOwnership(_owner);
    ...
    function setIds(IJukebox.ERC721Grouping[] calldata tokenGroupings, bool set)
        external
        onlyOwner
    {
...
contract SampleERC721 is ERC721Enumerable {
    ...
    constructor() ERC721("Sample ERC721", "SAM") {}
