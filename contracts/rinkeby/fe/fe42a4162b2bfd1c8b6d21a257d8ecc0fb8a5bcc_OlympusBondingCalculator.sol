...
contract OlympusBondingCalculator is IBondingCalculator {
  ...
  event BondPremium( uint debtRatio_, uint bondScalingFactor, uint premium_ );
  event PrincipleValuation( uint k_, uint amountDeposited_, uint totalSupplyOfTokenDeposited_, uint principleValuation_  );
  event BondInterest( uint k_, uint amountDeposited_, uint totalSupplyOfTokenDeposited_, uint pendingDebtDue_, uint managedTokenTotalSupply_, uint bondScalingFactor, uint interestDue_ );
  ...
  function _calcBondPremium( uint debtRatio_, uint bondScalingFactor ) internal pure returns ( uint premium_ ) {
    // premium_ = uint( uint(1).mul( 1e9 ).add( debtRatio_ ) ** bondScalingFactor);
    premium_ = bondScalingFactor.mul( (debtRatio_) ).add( uint(1010000000) ).div( 1e7 );
  }

  function calcBondPremium( uint debtRatio_, uint bondScalingFactor ) external pure override returns ( uint premium_ ) {
    premium_ = _calcBondPremium( debtRatio_, bondScalingFactor );
  }
  ...
  function _calculateBondInterest( uint k_, uint amountDeposited_, uint totalSupplyOfTokenDeposited_, uint pendingDebtDue_, uint managedTokenTotalSupply_, uint bondScalingFactor ) internal returns ( uint interestDue_ ) {
    ...
    uint premium_ = _calcBondPremium( debtRatio_, bondScalingFactor );
    ...
    .div(1e7);
    emit BondInterest( k_, amountDeposited_, totalSupplyOfTokenDeposited_, pendingDebtDue_, managedTokenTotalSupply_, bondScalingFactor, interestDue_ );
  }


  function calculateBondInterest( address treasury_, address principleTokenAddress_, uint amountDeposited_, uint bondScalingFactor ) external override returns ( uint interestDue_ ) {
    ...
    interestDue_ = _calculateBondInterest( k_, amountDeposited_, principleTokenTotalSuply_, outstandingDebtAmount_, managedTokenTotalSuply_, bondScalingFactor );
  }
