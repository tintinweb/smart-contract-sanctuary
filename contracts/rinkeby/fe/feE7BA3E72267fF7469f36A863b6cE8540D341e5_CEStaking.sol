...
contract CEStaking is ReentrancyGuard, Ownable {
  ...
  function withdraw(uint256 _amountToWithdraw) external nonReentrant onlyOpen {
    ...
    require(
      ...
      "Stake: CEG Allowance not enough"
    );
    ...
    require(CE.balanceOf(address(this)) > total, "Contract not Funded for CE");
  ...
  function checkCurrentRewards(address owner) public view returns (uint256 rewards_) {
    rewards_ =
      ((((block.timestamp - timeStaked[owner]) / 86400) * 10**3) / 365) *
      amountStaked[owner];
  }

  function checkStake(address owner) public view returns (uint256 stake_) {
    stake_ = amountStaked[owner];
  }
