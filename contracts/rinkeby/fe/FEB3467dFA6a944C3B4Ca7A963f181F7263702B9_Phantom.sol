...
contract Phantom is ERC721, Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    function PublicMint(string memory tokenURI) public payable noReentrant {
    ...
    // Mint token for whitelist accounts 
    
    function WhitelistMint() public payable noReentrant {
        uint id = totalSupply();
        require(totalSupply()<= MAX_PHANTOMS);
        require(whitelist[msg.sender].hasMinted < 2,"You can mint only two token");
        require(isWhitelisted(msg.sender),"Your account is not whitelisted");
