...
contract CA is ERC721, Ownable, PaymentSplitter {
    ...
    event MintCA (address indexed buyer, uint256 startWith, uint256 batch);

    constructor(
        address[] memory payees,
        uint256[] memory shares,
        string memory baseURI
    ) ERC721("CA", "CA") PaymentSplitter(payees, shares) {
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        require(totalMinted + numberOfTokens <= MAX_TOKENS - reservedTokens, "Purchase would exceed max available tokens");
        require(mintPrice * numberOfTokens <= msg.value, "Ether value sent is not correct");

        emit MintCA(msg.sender, totalMinted+1, numberOfTokens);
    ...
    function reservedMint() external payable {
        ...
        emit MintCA(msg.sender, totalMinted + 1, 1);
