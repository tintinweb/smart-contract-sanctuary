...
contract TheBirdHouse is ERC721Enumerable, Ownable {
    ...
    //strings
    string currentContractURI = "https://mint.thebirdhouse.app/api/contract";
    string baseURI = "https://mint.thebirdhouse.app/api/bird/";
    ...
    function mintBird() public payable returns (uint256) {
        _tokenIds.increment();
        ...
        require(
            newItemId >= 0 && newItemId < 6000,
            "TokenId must be between 0 and 6000"
        );
        require(totalSupply() < maxSupply, "Maximum birds already minted!");
        require(_exists(newItemId) == false, "Bird already minted.");
    ...
    function mintBirds(uint256 birdCount)
        public
        payable
        returns (uint256[] memory)
    {
        require(block.timestamp >= saleStartTime, "Sale has not started");
        require(
            birdCount > 0 && birdCount < 11,
            "Can only mint between 1 and 10 birds!"
        );
        require(msg.value >= (currentPrice * birdCount));

        uint256[] memory returnTokenIds = new uint256[](birdCount);

        for (uint256 iter = 0; iter < birdCount; iter++) {
            _tokenIds.increment();
            uint256 newItemId = _tokenIds.current();
    ...
    function ownerMintBirds(uint256 birdCount)
        public
        returns (uint256[] memory)
    {
