...
contract CANDYV is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
	...
	function _mintAnElement(address _to) private {
	   uint randomNumber = random(MAX_NFT - _tokenIdTracker.current());
	...
	function random(uint maxValue) internal view returns (uint) {
        return uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, _tokenIdTracker.current()))) % maxValue;
    }
}