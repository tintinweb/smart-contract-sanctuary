...
contract Factory {
    ...
    function transferReceiverFunds(address[] memory _receivers, address[] memory _tokens, uint256[] memory _amounts, address _receiver) external
    {
        require(msg.sender == owner);
        ...
        {
            Receiver(_receivers[i]).transferFunds(_tokens[i], _amounts[i], _receiver);
        }
    }
    
    function transferFunds(address[] memory _tokens, uint256[] memory _amounts, address _receiver) public {
        require(msg.sender == owner);
