...
contract CrownyVesting is Ownable {
    ...
    mapping (address => uint256[]) internal __amounts;
    mapping (address => uint256[]) internal __junctures;
    ...
    uint256 internal __balanceAllocated;
    ...
    function setVesting(address _beneficiary, uint256[] memory _junctures, uint256[] memory _amounts) external onlyOwner {
        require (_junctures.length > 0, "CrownyVesting: junctures and amounts cannot be empty");
        require (_junctures.length == _amounts.length, "CrownyVesting: junctures and amounts should match");
        require (__vesters[_beneficiary] == false, "CrownyVesting: address already vesting");

        uint256 totalAmount = _amounts[_amounts.length - 1];

        require (totalAmount <= __token.balanceOf(address(this)).sub(__balanceAllocated), "CrownyVesting: contract doesn't have enough tokens (yet)");


        __balanceAllocated             = __balanceAllocated.add(totalAmount);
        __junctures[_beneficiary]      = _junctures;
        __amounts[_beneficiary]        = _amounts;
        __vestingAmounts[_beneficiary] = totalAmount;
        __vesters[_beneficiary]        = true;

        emit Vesting(_beneficiary, totalAmount);
    }
    ...
    function allowedToClaim(address _beneficiary) public view returns (uint256) {
        ...
        uint256 vestedTokens = __amounts[_beneficiary][__getJunctureIndex(_beneficiary)];
    ...
    function __handleRevoke(address _beneficiary) internal {
        ...
        __amounts[_beneficiary]        = [__amounts[_beneficiary][junctureIndex]];
