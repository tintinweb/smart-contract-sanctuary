...
contract Bridge is Ownable {
    ...
    /// @return length  of availableTokens
    function addIRC20Token(address _token)
        public
        onlyOwner
    ...
    function transferToken(
        ...
        require(
            processNonce[_nonce] == false,
            "This nonce is already in proccess "
        );
        require(
        ...
        bytes32 _secretData = keccak256(
            abi.encodePacked(((block.timestamp + _amount + _nonce)))
        );

        irc20Token.transferFrom(from, address(this), _amount);
        processNonce[_nonce] = true;
        emit TransferToken(from, address(this), _amount, _nonce, _secretData);
    }
    ...
    function RedeemToken(
        ...
        require(processNonce[nonce] == false, "This nonce Already process ");
        require(
            ecrecover(messageHash, v, r, s) == signer,
            "The amount or vrs is not correct "
        );
        irc20Token.transfer(to, amount);
        emit WithdrawToken(from, to, amount, nonce, secretData);
        processNonce[nonce] = true;
        processSecretData[secretData] = true;
    }
}