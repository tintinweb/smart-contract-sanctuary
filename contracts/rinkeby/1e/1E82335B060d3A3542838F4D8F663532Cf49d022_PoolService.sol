...
contract VaultService is PriceModel, Ownable {
    ...
    function removeLiquidity(uint256 amount) external {
        ...
        if (_balanceLiquidity == 0) {
            ERC20(_underlyingTokenAddress).transfer(msg.sender, ERC20(_underlyingTokenAddress).balanceOf(address(this)));
            _profit = 0;
            _totalLiquidity = 0;
            _availableLiquidity = 0;
            _rate = S_1;
        } else {
            ERC20(_underlyingTokenAddress).transfer(msg.sender, amountUnderlyingToken);
            _profit = _profit.add(amount).sub(amountUnderlyingToken);
            _totalLiquidity = _totalLiquidity.sub(amountUnderlyingToken);
            _availableLiquidity = _availableLiquidity.sub(amountUnderlyingToken);
        }
...
contract PoolService is Ownable {
    ...
    event CloseLeverage(address indexed sender, uint256 collateral, uint256 debt, uint256 currentBalance, uint256 apr, uint256 apy);
    event LiquidateLeverage(address indexed sender, address indexed liquidator, uint256 collateral, uint256 debt, uint256 currentBalance, uint256 apr, uint256 apy);
    ...
    function closePosition() external {
        (uint256 collateral, uint256 debt, uint256 currentBalance, uint256 apr, uint256 apy) = _infoClosePosition(msg.sender);

        _closePosition(msg.sender, address(0));
        emit CloseLeverage(msg.sender, collateral, debt, currentBalance, apr, apy);
    }
    ...
    function liquidatePosition(address holder) external onlyLiquidatePosition {
        (uint256 collateral, uint256 debt, uint256 currentBalance, uint256 apr, uint256 apy) = _infoClosePosition(holder);

        _closePosition(holder, msg.sender);
        emit LiquidateLeverage(holder, msg.sender, collateral, debt, currentBalance, apr, apy);
    }

    function _infoClosePosition(address holder) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 apy = _vaultService.getAPY(holder);

        return (collateral, debt, currentBalance, apr, apy);
    }
