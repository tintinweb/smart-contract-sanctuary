...
contract Genegeosis is ERC721Tradable {
    ...
    uint256 public constant MAX_GEOS = 930;
    ...
    function mint() external payable {
        ...
        require(supply < MAX_GEOS, 'Sold out!');
        require(price == msg.value, 'Price must be equal to the price.');
        require(balanceOf(msg.sender) + 1 <= 10, 'There is a minting limit of 10 per wallet.');
    ...
    function mintMany(uint256 _amount) external payable {
        uint256 supply = totalSupply();
        require(!locked, 'Not unlocked yet.');
        require(supply * _amount < MAX_GEOS, 'Sold out!');
        require(price * _amount == msg.value, 'Price must be equal to the price.');
        // require(_amount <= 10, 'You cannot mint more than 10 tokens at once.');
        require(balanceOf(msg.sender) + _amount <= 10, 'There is a minting limit of 10 per wallet.');
    ...
    function mintManyTo(address _to, uint256 _amount) external onlyOwner {
        uint256 supply = totalSupply();
        require(supply * _amount < MAX_GEOS, 'Sold out!');
    ...
    function forwardPayment() public payable {
    ...
    function changeArtistWallet(address _artistWallet) public onlyOwner {
        artistWallet = _artistWallet;
    }

    function changeDevWallet(address _devWallet) public onlyOwner {
        devWallet = _devWallet;
    }
