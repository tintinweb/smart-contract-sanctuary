...
contract MemeLimitedCollectionsV2 is WhitelistAdminRole, Ownable, Pausable {
	...
	struct Token {
		address token;
        uint256 decimals;
		uint256 max; // 500000000
		uint256 rate; // 11574074074000, 1 point per day per staked MEME
	}
	...
	mapping(address => uint256) public pendingWithdrawals; // revisit
	mapping(uint256 => Pool) public pools;

	// Info of each user that deposits tokens for a pool
	...
	event Staked(address indexed user, uint256 poolId, uint256 tokenId, uint256 amount);
	event Withdrawn(address indexed user, uint256 poolId, uint256 tokenId, uint256 amount);
	event Redeemed(address indexed user, uint256 poolId, uint256 amount);

	modifier updateReward(address account, uint256 id) {
		if (account != address(0)) {
			userInfo[id][account].points = earned(account, id);
			userInfo[id][account].lastUpdateTime = block.timestamp;
		}
		_;
	}
	...
	constructor(address _controller, IERC1155 _memeLtdAddress) public {
	...
	function earned(address account, uint256 pool) public view returns (uint256) {
		UserInfo storage u = userInfo[pool][account];
		uint256 pineapples = u.points;

		for (uint256 i = 0; i < tokens.length; i++) {
			if (u.balances[i] > 0) {
                pineapples.add(
                    u.balances[i].mul(block.timestamp.sub(u.lastUpdateTime).mul(tokens[i].rate)).div(tokens[i].decimals)
                );
            }
		}
		return pineapples;
    ...
    function balanceOfPool(uint256 pool, uint256 token) public view returns (uint256) {
		return pools[pool].balances[token];
	}

	function balanceOf(address account, uint256 pool, uint256 token) public view returns (uint256) {
		return userInfo[pool][account].balances[token];
	}    
	...
	function stake(
		...
		uint256 tokenId,
		uint256 amount
	) public poolExists(pool) updateReward(msg.sender, pool) whenNotPaused() {
		UserInfo storage u = userInfo[pool][msg.sender];
		Pool storage p = pools[pool];
		Token memory t = tokens[tokenId];

		require(block.timestamp >= p.periodStart, "pool not open");
		require(amount.add(u.balances[tokenId]) <= t.max, "stake exceeds max");

		p.balances[tokenId] = p.balances[tokenId].add(amount);
		u.balances[tokenId] = u.balances[tokenId].add(amount);

		require(IERC20(t.token).transferFrom(msg.sender, address(this), amount), "ERC20 transfer failed");
	...
	function withdraw(
		...
		uint256 tokenId,
		uint256 amount
	) public poolExists(pool) updateReward(msg.sender, pool) {
		require(amount > 0, "cannot withdraw 0");
		UserInfo storage u = userInfo[pool][msg.sender];
		Pool storage p = pools[pool];
		Token memory t = tokens[tokenId];

		require(u.balances[tokenId] >= amount, "cannot withdraw more than deposited");
		p.balances[tokenId] = p.balances[tokenId].sub(amount);
		u.balances[tokenId] = u.balances[tokenId].sub(amount);

		require(IERC20(t.token).transfer(msg.sender, amount), "ERC20 transfer failed");
		emit Withdrawn(msg.sender, pool, tokenId, amount);
	}

	function exit(uint256 pool, uint256 tokenId) external {
		UserInfo storage u = userInfo[pool][msg.sender];
		withdraw(pool, tokenId, u.balances[tokenId]);
	}

	function redeem(uint256 pool, uint256 card) public payable 
        poolExists(pool)
        ...
        updateReward(msg.sender, pool) 
    {
		...
		require(u.points >= c.points, "not enough pineapples");
    ...
    returns (uint256) {
		...
		uint256 earnedPoints = u.points;
		p.spentPineapples = p.spentPineapples.add(earnedPoints);
		u.points = 0;

		for (uint256 i = 0; i < tokens.length; i++) {
			if (u.balances[i] > 0) {
				uint256 amount = u.balances[i];
				p.balances[i] = p.balances[i].sub(amount);
				u.balances[i] = u.balances[i].sub(amount);
				require(IERC20(tokens[i].token).transfer(msg.sender, amount), "ERC20 transfer failed");
			}
		}
	...
	function addCard(
		...
		uint256 mintFee,
		uint256 releaseTime
	) public onlyWhitelistAdmin poolExists(pool) {
	...
	function createCard(
		...
		uint256 mintFee,
		uint256 releaseTime
	) public onlyWhitelistAdmin poolExists(pool) returns (uint256) {
		uint256 tokenId = memeLtd.create(supply, 0, "", "");
		require(tokenId > 0, "ERC1155 create did not succeed");
        addCard(pool, tokenId, points, mintFee, releaseTime);
		return tokenId;
	}
    ...
    function addToken(address _token, uint256 _decimals, uint256 _max, uint256 _rate) public onlyOwner {
		tokens.push(Token({ token: _token, decimals: _decimals, max: _max, rate: _rate }));
	}
}