...
contract CanvasShopBase {
  ...
  function _buy(uint256 _tokenId, uint256 _sentAmount) internal returns (uint256) {
    ...
    if (sale.price > 0) {
      uint256 saleCut = _calculateCut(sale.price);
      uint256 sellerGains = sale.price - saleCut;
  ...
  function _calculateCut(uint256 _price) internal view returns (uint256) {
...
contract Picasso is SquareShop {
  ...
  function _paint(uint _tokenId, Colors[25] newArt) internal view {
    Square memory square = squares[_tokenId];
