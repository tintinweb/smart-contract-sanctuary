...
contract VolatilityChainlinkOracle is ChainlinkClient {
    uint256 private constant GRACE_PERIOD = 2 hours;
    ...
    function updateVolatility() external returns (bytes32 requestId) {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId,
            address(this),
            this.fulfillVolatility.selector
        );

        Chainlink.add(request, "path", "volatility");
     ...
     */
    function fulfillVolatility(bytes32 _requestId, uint256 __volatility)
        public
        recordChainlinkFulfillment(_requestId)
    {
        _volatility = __volatility;
    }

    /**
     * @dev returns the lastest volatility report
     */
    function latestVolatility() external view returns (uint256) {
        // after the grace period you need to update the volatility again
        require(_lastUpdate + GRACE_PERIOD > block.timestamp, "Volatility update required"); // solhint-disable-line
