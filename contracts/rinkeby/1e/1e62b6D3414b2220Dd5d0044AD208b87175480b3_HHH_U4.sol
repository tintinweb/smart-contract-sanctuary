...
contract HHH_U1 is HHHmainnet {
    //added function that does not modify the state
    function randomF() external virtual pure returns (uint256) {
        return 2;
    }
}

contract HHH_U2 is HHH_U1 {
    //changed existing function that modifies the state
...
contract HHH_U3 is HHH_U2 {
    //add function that modifies the state
    uint256 public version;

    function addversion(uint256 newVersion) public virtual {
        require(newVersion > version, "New version must be higher than existing");
        version = newVersion;
    }
}

contract HHH_U4 is HHH_U3 {
    function getVersion() public virtual view returns(uint) {
        return version;
    }

    function getTransferCount() public virtual view returns(uint) {
        return transferCount;
    }
}

contract HHH_U5 is HHH_U4 {
    function randomF() external virtual override pure returns (uint256) {
        return 0;
    }
}
