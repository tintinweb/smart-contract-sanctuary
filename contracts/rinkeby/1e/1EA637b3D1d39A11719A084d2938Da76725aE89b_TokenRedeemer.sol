...
contract TokenRedeemer is Ownable, ReentrancyGuard {
  ...
  */
  constructor(ISuper1155 _super1155, address _burnTarget, bool _customBurn) {
  ...
  */
  function _redeemToken(uint256 _tokenId, address _tokenOut) internal {
    ...
    uint256 mintCount = super1155.groupMintCount(groupIdOut);
    ...
    if (config.burnOnRedemption) {
      if (customBurn) {
        super1155.safeTransferFrom(_msgSender(), burnAddress, _tokenId, 1, "");
      } else {
        super1155.burnBatch(_msgSender(), _asSingletonArray(_tokenId), _asSingletonArray(1));
      }
    }

    super1155.mintBatch(_msgSender(), ids, amounts, "");

    emit TokenRedemption(_msgSender(), _tokenId, _tokenOut, ids);
  }
