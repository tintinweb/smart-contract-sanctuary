...
contract RichIsland is ContextMixin, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint256 TOKEN_PRICE_IN_WEI = 0;
    uint256 TOKEN_PRICE_IN_ETH = 0;
    ...
    modifier onlyWhitelisted() {
        require(whitelist[msg.sender], "Alert: You're not on the whitelist.");
        _;
    }

    // For RandomlyAssigned: Max supply is 10000; id start from 1 (instead of 0)
    constructor( ) ERC721 ("2RichIsland", "2RI") {
        baseTokenURI = "ipfs:/QmZ45mmdi7A3cCcaDmtqthXCTKZPqCKLjM57w41rHEdo99";
        notRevealedURI = "ipfs://QmengZJPhpUi3gN4URXvxUqJBYMH6stKaB2jQiYY8JdAYP";
        _setEthPrice(0.04 ether);
    }
    ...
    function _setEthPrice(uint _etherPrice) internal onlyOwner {
        TOKEN_PRICE_IN_ETH = _etherPrice;
        TOKEN_PRICE_IN_WEI = 1 ether * 2 / _etherPrice / 100;
    }

    function setEthPrice(uint _etherPrice) external onlyOwner {
        _setEthPrice(_etherPrice);
    }
    ...
    function pledge(uint8 _num) external payable onlyWhitelisted {
        ...
        require(
            (_num + pledgeNum[msg.sender] + claimed[msg.sender]) <= 5,
            "Alert: Each address can only purchase up to 5 Tikis."
        );
        require(
            totalMint + uint256(_num) <= MAX_SUPPLY - totalPledge - 10,
            "Alert: Sorry, all Tikis have been sold."
        );
        require(
            msg.value >= uint256(_num) * TOKEN_PRICE_IN_WEI,
            "Alert: You need to pay at least the required cost."
        );
        pledgeNum[msg.sender] = pledgeNum[msg.sender] + _num;
    ...
    function pauseSetTrue() external onlyOwner {
        paused = true;
    }

    function pauseSetFalse() external onlyOwner {
        paused = false;
    }
