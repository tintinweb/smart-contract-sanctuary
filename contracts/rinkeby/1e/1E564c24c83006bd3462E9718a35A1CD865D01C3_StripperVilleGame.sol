...
contract StripperVilleGame is Administration {
    ...
    mapping(uint => mapping (uint => Club)) private _poolClub;
    mapping(uint => uint) public stripperWearable;
    mapping(uint => Worker[]) private _poolThieves;
    mapping(uint => Worker[]) private _poolCustomersOld;
    mapping(uint => mapping (uint => address)) private _poolCustomers;
    ...
    function _worker(uint gameId, address to, bool thief) internal {
        ...
        if(thief){
            ...
            require(_poolCustomersOld[gameId].length < games[gameId].maxCustomers, "MAX CUSTOMERS REACHED");
            _poolCustomersOld[gameId].push(Worker(to, 0));
            // TODO: need to track # of unallocated workers per address so they can allocate them
        }
    ...
    function _workerAction(uint gameId, uint tokenId, bool thief) internal gameOn(gameId) {
        ...
        if(thief){
            ...
            _poolCustomersOld[gameId][index].tokenId = tokenId;
            _poolCustomers[gameId][tokenId] = _msgSender();
        }
    ...
    function _getWorker(uint gameId, address owner, bool thief) internal view returns (Worker memory,uint) {
        ...
        uint index = 9999;
        Worker[] memory workers = thief ? _poolThieves[gameId] : _poolCustomersOld[gameId];
    ...
    function pauseGame(uint gameIndex) public onlyAdmin {
        games[gameIndex].paused = true;
    }
    
    function unpauseGame(uint gameIndex) public onlyAdmin {
        games[gameIndex].paused = false;
    }
    ...
    function setEarn(uint256 stripperId, uint256 earn) external onlyOwner {
        _stripperEarnings[stripperId] = earn;
    }

    function getEarnValue(uint256 stripperId) internal view returns (uint256) {
        return _stripperEarnings[stripperId];
    }
    
    function work(uint stripperId, uint clubId, uint gameId) public ownerOf(stripperId) {
        require(_poolStripClub[gameId][stripperId] < 100000, "CLUB: already set for this game");
        ...
        uint earn = getEarnValue(stripperId);
        address stripperAddr = _poolCustomers[gameId][stripperId];
        if(stripperAddr != address(0)) {
            // If the stripper has a customer assigned to them, increase the earn by
            //  the customer's multiplier
            earn = earn * game.customerMultiplier;
        }
        _poolStripClub[gameId][stripperId] = clubId;
        Club memory club = _poolClub[gameId][clubId];
        club.earn += earn;
        club.stripperCount += 1;
        _poolClub[gameId][clubId] = club;
        emit Work(stripperId, gameId);
    }
    
    function workOld(uint stripperId, uint clubId, uint gameId) public ownerOf(stripperId) {
        require(_poolStripClub[gameId][stripperId] < 100000, "CLUB: already set for this game");
        ...
        Worker[] memory workers = _poolCustomersOld[gameId];
        for(uint i=0; i< workers.length; i++){
    ...
    function getClubStrippersCount(uint gameId, uint clubId) public view returns (uint) {
        ...
        return  _poolClub[gameId][clubId].stripperCount;
    }
    ...
    function closeGame(uint index) public onlyAdmin {
        ...
        for(uint i=0; i < clubIds.length; i++){
            ...
            uint totalOne = thievesOne > 0 ? thievesOne > 9 ? 0 : (_poolClub[index][one].earn / 10) * (10 - thievesOne) : _poolClub[index][one].earn;
            if(totalOne > 0){
                for(uint j=0; j < clubIds.length; j++){
                    uint two = clubIds[j];
                    if(one != two){
                        uint thievesTwo = _getThievesByClubId(index, two);
                        uint totalTwo = thievesTwo > 0 ? thievesTwo > 9 ? 0 : (_poolClub[index][two].earn / 10) * (10 - thievesTwo) : _poolClub[index][two].earn;
        ...
        if(asset.tokenType == 0){
            ...
            for(uint i=0;i<games.length;i++){
                ...
                if(games[i].endDate > 0 && baseEarn > 0){
                    ...
                    earn += ((gameEarn / _poolClub[i][_poolStripClub[i][tokenId]].stripperCount / 100) * (totalEarn / 10 ** 18) * getCustomerMultiply(i, tokenId));
                }
            }
