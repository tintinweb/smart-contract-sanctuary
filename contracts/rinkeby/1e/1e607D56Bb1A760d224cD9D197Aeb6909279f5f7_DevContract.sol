...
contract DevContract is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  uint256 CHARITY_SHARES = 10;
  uint256 DEV_SHARES = 45;
  ...
  function selectGoldenPickle() public onlyOwner {
      require(OG_GANGSTA_HAMSTA == 0, "OG Gangsta Hamsta already minted and selected");
      uint256 max = MAX_GANGSTA_HAMSTA - 1;
      ...
      if (randomHash >= 5 && randomHash <= sell){
          OG_GANGSTA_HAMSTA =  randomHash;
      }
  }

  function getOGGangstaHamsta() public view returns (uint256){ 
    require( OG_GANGSTA_HAMSTA != 0, "OG Gangsta Hamsta is not chosen yet.");
    return OG_GANGSTA_HAMSTA;
  }
