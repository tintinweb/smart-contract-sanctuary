...
contract FunkiFoxes is ERC721, Ownable {
    ...
    function mintReserves(uint256[] memory ids) public onlyOwner {
        require(ids.length <= 20, "Too many tokens");
        for(uint256 i = 0; i < ids.length; i++) {
            uint256 _mintIndex = ids[i];
            require(_mintIndex > currentSupply, "Disabled token Id to reserve");
            require(!isReserved[_mintIndex], "Token is reserved already.");
    ...
    function mintFox(uint256 _numberOfTokens) public payable {
        ...
        for (uint256 i = 1; i <= _numberOfTokens; i++) {
            uint256 _mintIndex = _foxIndex.add(i).add(skip);
            while (isReserved[_mintIndex]) {
                _mintIndex ++;
                currentSupply ++;
                skip ++;
            }
