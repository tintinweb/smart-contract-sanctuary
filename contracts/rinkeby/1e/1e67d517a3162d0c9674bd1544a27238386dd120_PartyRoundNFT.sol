...
contract PartyRoundNFT is ERC1155Supply, Ownable {

  address private adminSigner;
  bool private metadataLocked;
  uint256 private lastFrozenMetadataTokenId;
  string frozenMetadataUri;

  constructor(address _adminSigner)
    ERC1155("https://api.partyround.com/api/metadata/{id}.json")
  {
  ...
  function _recoverSigner(address _addr, uint256 _tokenId, bytes memory _sig) private pure returns (address) {
    ...
    return ecrecover(fullHash, v, r, s);
  }
  ...
  /**
     * @dev The dev mint function

     * Params:
     * _tokenIds: the token ids to be minted
     * _addresses: the addresses to send the tokens to
     */
  function devMint(uint256[] memory _tokenIds, address[] memory _addresses) external {
    ...
    for (uint i = 0; i < _tokenIds.length; i++) {
      ...
      _mint(_addresses[i], _tokenIds[i], 1, "");
    }
  }
}
