...
contract NotRandom is ERC721Enumerable {
  ...
  mapping(bytes32 => bool) internal hashExists;

  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;

  constructor() ERC721("NotRandom", "NR") {}

  function mint(string memory tokenName) public {
    bytes32 tokenHash = keccak256(abi.encodePacked(tokenName));
    require(!hashExists[tokenHash], "This hash already exists and cannot be minted again");
    hashExists[tokenHash] = true;
  ...
  function burn(uint256 tokenId) public {
    require(msg.sender == ownerOf(tokenId), "Only the token owner can burn their token");
    hashExists[tokens[tokenId].hash] = false;
    delete tokens[tokenId];
    _burn(tokenId);
  }
  
  // This is from Loot Project (extended to include linewrapping via textPaths) - its a clean look
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    string[5] memory parts;
    parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><defs><path id="path1" d="M10,20 H330 M10,40 H330 M10,60 H330 M10,80 H330 M10,100 H330 M10,120 H330 M10,140 H330 M10,160 H330 M10,180 H330 M10,200 H330 M10,220 H330"></path><path id="path2" d="M10,280 H330 M10,300 H330"></path></defs><use xlink:href="#path1" x="0" y="20"/><use xlink:href="#path2" x="0" y="20"/><text transform="translate(0,20)" class="base"><textPath xlink:href="#path1">';

    parts[1] = tokens[tokenId].name;

    parts[2] = '</textPath><textPath xlink:href="#path2">';

    parts[3] = toHexString(tokens[tokenId].hash);

    parts[4] = '</textPath></text></svg>';

    string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4]));
    
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', tokens[tokenId].name, '", "description": "NotRandom provides a keccak256 hash of the token name which is chosen during mint. This hash can be used for anything including any of your not random generative needs but is built to be used with SOLVER_CODE to provide initial values to create a three body simulation.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
