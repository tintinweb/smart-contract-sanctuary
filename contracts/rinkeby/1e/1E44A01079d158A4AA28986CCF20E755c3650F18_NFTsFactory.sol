...
contract NFTsFactory is CloneFactory {
    ...
    )
  {
    if (nft.supportsInterface(ERC721)) {
      balance = IERC721(nft).balanceOf(owner);
  ...
  {
    balances = new uint256[](_implementations.length);
    name = new string[](_implementations.length);
    symbol = new string[](_implementations.length);
    for (uint256 i = 0; i < _implementations.length; i += 1) {
      (balances[i], name[i], symbol[i]) = balanceOf(_implementations[i], owner);
    }
  }
}
