...
contract Ornament is Ownable, ERC721Enumerable {
        ...
        returns(string memory, string memory, string memory, string memory)
    {
        ...
        for (uint256 i = 0; i < svgBytes.length; i+=4) {
            // Convert first 3 bytes to uint256 to get `length` and `offset`
            string memory firstChars = string(abi.encodePacked(svgBytes[i], svgBytes[i+1], svgBytes[i+2]));
            uint256 offset = strToUint(firstChars); 
            uint256 length = 800 - (2 * offset);
            ...
            // If this is the last bevel, set it as frameWidth
            if (svgBytes.length - 4 == i) {
                frameWidth = offset;
            }
        }

        // Calculate attributes
        uint256 totalBevels = svgBytes.length / 4;
        // Solidity will truncate/round this result
        string memory lightness = (frameWidth / totalBevels).toString();
        string memory style;

        if (frameWidth < 120) {
            style = 'sharp';
        } else if (120 <= frameWidth && frameWidth < 300) {
            style = 'robust';
