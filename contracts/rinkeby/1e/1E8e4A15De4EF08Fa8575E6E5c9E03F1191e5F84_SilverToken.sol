...
contract SilverToken is AccessControl, ERC20 {
    ...
    bytes32 public constant REWARD_REDEEMER_ROLE =
        keccak256('REWARD_REDEEMER_ROLE');
    ...
    constructor(ISilverBackNFT _silverbackNFT) ERC20('Silver', 'SILVER') {
    ...
    /**
     * @dev Helper to verifiy msg.sender is a reward distributor
     */
    modifier onlyRewardDistributors() {
        _checkRole(REWARD_DISTRIBUTOR_ROLE, msg.sender);
        _;
    }

    /**
     * @dev Helper to verifiy msg.sender is a reward redeemer
     */
    modifier onlyRewardRedeemers() {
        _checkRole(REWARD_REDEEMER_ROLE, msg.sender);
        _;
    }

    /**
     * @dev Helper to verifiy msg.sender is an admin
     */
    modifier onlyAdmin() {
     ...
     */
    function getRewards(address _for, address _to)
        external
        onlyRewardDistributors
    {
        ...
        if (reward > 0) {
            ...
            emit RewardsPaid(msg.sender, _for, _to, reward);
        }
    }

    /**
     * @dev Called from a reward redeemer to burn rewards
     */
    function burn(address _from, uint256 _amount) external onlyRewardRedeemers {
        _burn(_from, _amount);
    }
    ...
    /**
     * @dev Called configure address as a reward redeemer
     */
    function configureRewardRedeemer(
        address _rewardRedeemer,
        bool _isRewardRedeemer
    ) external onlyAdmin {
        if (_isRewardRedeemer) {
            grantRole(REWARD_REDEEMER_ROLE, _rewardRedeemer);
        } else {
            revokeRole(REWARD_REDEEMER_ROLE, _rewardRedeemer);
        }
    }
