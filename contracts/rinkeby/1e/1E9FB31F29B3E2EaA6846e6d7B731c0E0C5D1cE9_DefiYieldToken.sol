    ...
    ERC20Votes
{
    ...
    constructor() ERC20("DefiYield", "DFY") ERC20Permit("DefiYield") {
    ...
    function mint(address to_, uint256 amount_) external onlyRole(MINTER_ROLE) {
        require(
            ...
            "DFY::mint: minting not allowed yet"
        );
        require(
            to_ != address(0),
            "DFY::mint: cannot transfer to the zero address"
        );
        require(
            amount_ <= ((totalSupply() * MINT_CAP) / 100),
            "DFY::mint: exceeded mint cap"
        );
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !blacklist[from_],
            "DFY::_beforeTokenTransfer: from address is blacklisted"
        );
        require(
            !blacklist[to_],
            "DFY::_beforeTokenTransfer: to address is blacklisted"
        );
