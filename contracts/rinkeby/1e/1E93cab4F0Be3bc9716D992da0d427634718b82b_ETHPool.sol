...
contract ETHPool is Ownable {
    ...
    function getPoolInfo(uint256 poolId) external view returns(
        ...
        return (pool.poolAmount, pool.rewards, pool.rewardRecipients, pool.teamDeposited);
    }
    ...
    function getUserDepositAmount(uint256 poolId, address user) external view returns(uint256) {
    ...
    function getUserEligibility(uint256 poolId, address user) external view returns(bool){
    ...
    function depositRewards(uint256 poolID) external payable onlyTeam {
        require(!pools_[poolID].teamDeposited, "Pool has already been used");
        ...
        for(uint256 i = 0; i < pools_[poolID].rewardRecipients.length; i++){
            ...
            uint256 rewardPercent = userDeposit_[user][poolID] * PRECISION;
            rewardPercent = rewardPercent / pools_[poolID].poolAmount;
            uint256 userReward = pools_[poolID].rewards * rewardPercent;
            userReward = userReward / PRECISION;
    ...
    //Emergency withdrawal function for when the team wants to retrieve the rewards
    function withdrawReward(uint256 poolID) external onlyTeam {
        require(pools_[poolID].poolAmount == 0 && pools_[poolID].rewardRecipients.length == 0, "Can't withdraw if a user has deposits in the pool");
        require(pools_[poolID].rewards > 0, "There are no rewards in this pool");
        uint256 withdrawlAmount = pools_[poolID].rewards;
        pools_[poolID].rewards = 0;
        payable(address(team_)).transfer(withdrawlAmount);
        emit RewardWithdrawal(poolID, withdrawlAmount);
    }
    ...
    function withdrawETH(uint256 poolID) external {
        require(userDeposit_[address(msg.sender)][poolID] > 0, "User has no deposits in this pool");
        uint256 deposit = userDeposit_[address(msg.sender)][poolID];
        uint256 reward = userRewards_[address(msg.sender)][poolID];
        uint256 withdrawlAmount = deposit + reward;
        if(userRewards_[address(msg.sender)][poolID] != 0) {
            //Decrement amount in the pool
            pools_[poolID].poolAmount -= userDeposit_[address(msg.sender)][poolID];
            pools_[poolID].rewards -= userRewards_[address(msg.sender)][poolID];
        }
        //Since user deposit and/or rewards have been taken out, the mappings are set to zero
        userDeposit_[address(msg.sender)][poolID] = 0;
        userRewards_[address(msg.sender)][poolID] = 0;
        payable(address(msg.sender)).transfer(withdrawlAmount);
        emit UserWithdrawl(poolID, address(msg.sender), withdrawlAmount, reward);
    }
