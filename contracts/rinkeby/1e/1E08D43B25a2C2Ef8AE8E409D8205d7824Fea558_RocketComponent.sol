...
contract RocketComponent is ERC721, Ownable {
    ...
    // tokenId -> stickerId
    mapping(uint256 => uint256) componentStickers;
    ...
    constructor() ERC721("Tom Sachs Rocket Components", "TSRC") {
        ...
        // Pre-mint all components
        // This will be changed once we know what are we gonna mint
        componentModels.push(ComponentModel("NASA", 0, 665));
        componentModels.push(ComponentModel("Makita", 666, 1332));
        componentModels.push(
            ComponentModel("United States Postal Service", 1333, 1999)
        );
    }
     ...
     * users using the claim function.
     */
    function removeFromAvailableComponents(uint16 tokenId) external onlyOwner {
     ...
     * users using the claim function.
     */
    function resetAvailableComponents() external onlyOwner {
     ...
     */
    function setRocketFactoryContract(address _address) external onlyOwner {
     ...
     */
    function addStickers(string[] memory _stickers) external onlyOwner {
    ...
    function claim() external payable {
        ...
        require(msg.value >= claimPrice, "Insufficient Ether to claim");
        ...
        returns (ComponentView memory)
    {
        for (uint256 i; i < componentModels.length; i++) {
            ...
            (uint256 total, uint256 edition) =
                _totalComponentEditions(_tokenId, i);
            ...
            return
                ComponentView(
                    ...
                    string(
                        abi.encodePacked(
                            ipfsBaseURI,
                            utils.toSlug(componentModels[i].brand),
                            "-",
                            componentType,
                            stickerComponentPath,
                            ".png"
                        )
                    ),
                    hasSticker,
     ...
     */
    function _getRandomNumber(uint256 _upper) private returns (uint256) {
        seed = uint256(
            keccak256(
                abi.encodePacked(
                    seed,
...
contract Utils {
     ...
     */
    function toSlug(string memory _base) public pure returns (string memory) {
