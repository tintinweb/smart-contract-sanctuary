...
contract SugarVillage is ERC721Enumerable, Ownable, Pausable, Revealable {
    ...
    uint256[] private _mintingOrder;
    mapping(uint256 => bool) private _mintingOrderValidator;
    ...
    constructor(string memory baseURI) ERC721("SugarVillage", "SHOP"){
        ...
        _devReserveSize = 9 * 3 + 100 + 173;
        // AuctionReserve + SalesGiveaway * 3 Rounds + Team Reserve + ContinuousGiveaway
        _salesRoundToMintLimitMap["PRESALE"] = _devReserveSize + 1111;
        _salesRoundToMintLimitMap["EARLY"] = _devReserveSize + 1111 + 3333;
    ...
    function mintForDevReserve() public onlyOwner {
        _mint(_devReserveSize, owner());
    }

    function generateMintingOrder(uint256 size) public onlyOwner {
        require(!_mintingOrderGenerated, "Minting order is already completely generated!");
        uint256 current = _mintingOrder.length + 1;
        uint256 currentMax = Math.min(current + size, 9999);
        for (uint256 i = current; i <= currentMax; i++) {
            _mintingOrder.push(i);
        }
    }

    function shuffle(uint256 start, uint256 end) public onlyOwner {
        require(!_mintingOrderGenerated, "Minting order is already completely generated!");
        for (uint256 i = start; i < end; i++) {
            uint256 n = i + uint256(keccak256(abi.encodePacked(block.timestamp))) % (_mintingOrder.length - i);
    ...
    function mint(uint256 _count) public payable saleIsOpen {
        ...
        require(total + _count <= _salesRoundToMintLimitMap[_currentSalesRound], "Reached max minting limit for current round!");
        require(msg.value >= _price(_count), "Sent value is below the price!");

        _mint(_count, _msgSender());
    }

    function _mint(uint256 _count, address wallet) private saleIsOpen {
        for (uint8 i = 0; i < _count; i++) {
            _mintAnElement(wallet, _mintingOrder[totalToken()]);
        }
    }
    ...
    function _price(uint256 _count) private view returns (uint256) {
        return _salesRoundToPriceMap[_currentSalesRound] * _count;
    }
