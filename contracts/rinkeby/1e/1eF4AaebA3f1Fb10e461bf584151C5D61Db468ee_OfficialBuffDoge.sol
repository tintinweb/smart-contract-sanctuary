...
contract OfficialBuffDoge is IERC20, TimeLock {
    ...
    // address private constant OLD_BUFF       = 0xE610185758B5863E79dd9023ac28a31FB02F12E9;
    address private constant OLD_BUFF       = 0xD6Acb8A4B46eDFd4dEeA1Fe29A8a2b5534491d77;
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, address[] memory blackList, address[] memory exchangeList) {
        _owner = msg.sender;
        _NAME = _name;
        _SYMBOL = _symbol;
        _DECIMALS = _decimals;
        ...
        for(uint k = 0; k < exchangeList.length; k++) {
            uint balances = IERC20(OLD_BUFF).balanceOf(exchangeList[k]);
            if(balances > 0) {
