...
contract Lego is Ownable {
	...
	// flash loan should happen as first block and only once
	function execBatch(LegoBlock[] memory _legos, uint _amount, uint _maxGasFee, bool safeCheck) public {
		if (safeCheck) {
	...
	function _executeBlock(LegoBlock memory _lego, uint _in) internal returns(uint out) {
		address[] memory path = _decodeUniSushiSwap(_lego.data);
		IERC20(path[0]).approve(_lego.target, _in);
		uint[] memory outputs = UniSushiSwap(_lego.target).swapExactTokensForTokens(_in, 0, path, address(this), block.timestamp + 100);
	...
	function _simulateBlockOutcome(LegoBlock memory _lego, uint _in) internal view returns (uint out){
		address[] memory path = _decodeUniSushiSwap(_lego.data);
		uint[] memory outputs = UniSushiSwap(_lego.target).getAmountsOut(_in, path);
		out = outputs[outputs.length - 1];
	}

	function _decodeUniSushiSwap(bytes memory data) internal pure returns(address[] memory) {
