...
contract SCRedeem is Ownable {
    ...
    function claimSC(uint16 tokenId) public {
        ...
        randomNumberRequests[hash] = randomNumberRequest(tokenId, 0);
    ...
    function process(uint256 _random, bytes32 _requestHash) public {
        ...
        // offset collection
        mint_from_collection+= 13;

        // mint the charm NFT to the current owner of the token
        uint16 tokenId = randomNumberRequests[_requestHash].tokenId;
        ...
        randomNumberRequests[_requestHash].mintedTokenId = uint16(mintedTokenId);
    }

    function getStats(bytes32 hash) external view returns (uint16 tokenId, uint16 mintedTokenId)  {
