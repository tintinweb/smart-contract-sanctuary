...
contract CollectionFree is ERC1155, AccessControl {
    ...
    mapping(address => uint256) userNFTAmount;
    ...
    constructor(CollectionData memory collecData) ERC1155(collecData.uri) {
        ...
        minumumAmount = collecData.minumumAmount;
        ...
        emit Initialize(total, startTime, endTime, minumumAmount, NFTLimit);
    }
    ...
    modifier checkBalance(address buyer) {
        bool isTrue;
        if (Ifarm.farmed(buyer) + token.balanceOf(buyer) >= minumumAmount) {
            isTrue = true;
        } else {
            isTrue = false;
        }
