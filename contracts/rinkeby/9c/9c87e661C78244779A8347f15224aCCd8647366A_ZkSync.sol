...
/// @author Matter Labs
contract ZkSync is UpgradeableMaster, ReentrancyGuard, Executor, PublicInterface {
    ...
    function initialize(bytes calldata initializationParameters) external {
        ...
        ) = abi.decode(initializationParameters, (address, address, bytes32, bytes32));
        ...
        StoredBlockInfo memory storedBlockZero =
            StoredBlockInfo(0, 0, EMPTY_STRING_KECCAK, 0, _genesisStateHash, _genesisZkPorterHash, bytes32(0));
    ...
    function upgrade(bytes calldata upgradeParameters) external nonReentrant {
        ...
        require(totalPriorityRequests == totalCommittedPriorityRequests, "3"); // no uncommitted priority requests
    ...
    function cancelOutstandingDepositsForExodusMode(uint64 _n, bytes[] memory _depositsPubdata) external nonReentrant {
        ...
        uint64 totalOpenPriorityRequests = totalPriorityRequests - firstPriorityRequestId;
        ...
        for (uint64 id = firstPriorityRequestId; id < firstPriorityRequestId + toProcess; id++) {
            if (priorityRequests[id].opType == Operations.OpType.Deposit) {
                ...
                (, Operations.Deposit memory op) = Operations.readDepositPubdata(depositPubdata, 0);
                bytes32 packedBalanceKey = packAddressAndToken(op.owner, op.zkSyncTokenAddress);
...
contract Storage {
    ...
    /// @dev Root-chain balances (per owner and token id, see packAddressAndToken) to withdraw
    mapping(bytes32 => PendingBalance) internal pendingBalances;
    ...
    mapping(uint32 => mapping(address => bool)) public performedExodus;
    ...
    /// Uses keccack256 hash internally.
    function packAddressAndToken(address _address, address _zkSyncTokenAddress) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(_address, _zkSyncTokenAddress));
    }

    /// @Rollup block stored data
    /// @member blockNumber Rollup block number
    /// @member lastProcessedPriorityOp Serial ID of the last priority operation processed by the smart contract.
    ...
    /// @member stateRoot Root hash of the rollup state.
    /// @member zkPorterHash Root hash of zkPorter subtree.
    /// @member commitment Verified input for the zkSync circuit.
    struct StoredBlockInfo {
        uint32 blockNumber;
        uint64 lastProcessedPriorityOp;
...
/// @title zkSync Executor contract capable of processing events emitted in the rollup protocol
/// @author Matter Labs
contract Executor is BaseInterface, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeMathUInt128 for uint128;
    ...
    /// @param lastProcessedPriorityOp Serial ID of the last priority operation processed by the smart contract.
    /// @param onchainOperationsData Data for operations that should be processed by the smart contract.
    /// @param deployedSmartContracts Bytecode of deployed smart contracts.
    ...
    struct CommitBlockInfo {
        ...
        uint256 timestamp;
        uint64 lastProcessedPriorityOp;
        bytes32 unpriorityOpsOnchainDataHash;
        bytes priorityOperationsData;
    ...
    /// @dev NOTE: Does not change storage
        ...
        returns (StoredBlockInfo memory storedNewBlock)
    {
        ...
        (bytes32 processableOnchainOperationsHash, uint64 priorityOperationsProcessed) =
            collectPriorityQueueOnchainOps(_newBlock, _previousBlock.lastProcessedPriorityOp);

        require(
            _previousBlock.lastProcessedPriorityOp + priorityOperationsProcessed == _newBlock.lastProcessedPriorityOp,
            "F"
        ); // Check that priority operations are processed correctly.
        ...
        return
            StoredBlockInfo(
                ...
                _newBlock.lastProcessedPriorityOp,
                processableOnchainOperationsHash,
                _newBlock.timestamp,
    ...
    /// @dev processableOnchainOperationsHash - hash of the all operations that needs to be executed (Withdraw, ForcedWithdraw)
    /// @dev priorityOperationsProcessed - number of priority operations processed in this block (Deposits, AddToken, L1Withdraw, L1ForcedWithdraw)
    function collectPriorityQueueOnchainOps(CommitBlockInfo memory _newBlockData, uint64 _lastProcessedPriorityOp)
        internal
        view
        returns (bytes32 processableOnchainOperationsHash, uint64 priorityOperationsProcessed)
    {
        bytes memory priorityOperationsData = _newBlockData.priorityOperationsData;
        uint64 uncommittedPriorityRequestsOffset = _lastProcessedPriorityOp;
        uint256 offset = 0;

        priorityOperationsProcessed = 0;
        processableOnchainOperationsHash = _newBlockData.unpriorityOpsOnchainDataHash;

        while (offset != priorityOperationsData.length) {
            Operations.OpType opType = Operations.OpType(uint8(priorityOperationsData[offset]));

            if (opType == Operations.OpType.Deposit) {
                Operations.Deposit memory op;
                (offset, op) = Operations.readDepositPubdata(priorityOperationsData, offset);
                checkPriorityOperation(op, uncommittedPriorityRequestsOffset + priorityOperationsProcessed);
            } else if (opType == Operations.OpType.AddToken) {
                Operations.AddToken memory op;
                (offset, op) = Operations.readAddTokenPubdata(priorityOperationsData, offset);
                checkPriorityOperation(op, uncommittedPriorityRequestsOffset + priorityOperationsProcessed);
            } else if (opType == Operations.OpType.MigrateToPorter) {
                Operations.MigrateToPorter memory op;
                (offset, op) = Operations.readMigrateToPorterPubdata(priorityOperationsData, offset);
                checkPriorityOperation(op, uncommittedPriorityRequestsOffset + priorityOperationsProcessed);
            } else {
                ...
                if (opType == Operations.OpType.Withdraw) {
                    ...
                    (offset, op) = Operations.readWithdrawPubdata(priorityOperationsData, offset);
                    checkPriorityOperation(op, uncommittedPriorityRequestsOffset + priorityOperationsProcessed);

                    opPubData = Operations.writeWithdrawPubdataForPriorityQueue(op);
                } else if (opType == Operations.OpType.ForcedWithdraw) {
                    Operations.ForcedWithdraw memory op;
                    (offset, op) = Operations.readForcedWithdrawPubdata(priorityOperationsData, offset);
                    checkPriorityOperation(op, uncommittedPriorityRequestsOffset + priorityOperationsProcessed);

                    opPubData = Operations.writeForcedWithdrawPubdataForPriorityQueue(op);
                } else {
                    revert("E"); // unsupported op in collect onchain ops
                }

                // Read one additional byte that is responsible for the success of the operation.
                uint8 statusByte;
                (offset, statusByte) = Bytes.readUint8(priorityOperationsData, offset);
                if (statusByte == 0x01) {
                    processableOnchainOperationsHash = Utils.concatHash(processableOnchainOperationsHash, opPubData);
                }
            }
        ...
        nonReentrant
    {
        ...
        for (uint32 i = 0; i < _newBlocksData.length; ++i) {
            ...
            totalCommittedPriorityRequests = _lastCommittedBlockData.lastProcessedPriorityOp;
        ...
        require(totalCommittedPriorityRequests <= totalPriorityRequests, "j");
    }
    ...
    function withdrawOrStore(
        ...
        bytes32 packedBalanceKey = packAddressAndToken(_recipient, _tokenAddress);
        ...
        if (_tokenAddress == ZKSYNC_ETH_ADDRESS) {
            ...
            try this._transferERC20{gas: WITHDRAWAL_GAS_LIMIT}(IERC20(_tokenAddress), _recipient, _amount, _amount) {
                sent = true;
            } catch {
                sent = false;
            }
        ...
        if (sent) {
            emit Withdrawal(_tokenAddress, _amount);
        } else {
    ...
    function executeOneBlock(ExecuteBlockInfo memory _blockExecuteData, uint32 _executedBlockIdx) internal {
        ...
        bytes32 processablePriorityOperationsHash = EMPTY_STRING_KECCAK;
        bytes memory pubdata = _blockExecuteData.pendingOnchainOpsPubdata;
        uint256 offset = 0;
        while (offset != pubdata.length) {
            ...
            if (opType == Operations.OpType.Withdraw) {
                Operations.Withdraw memory op;
                (offset, op) = Operations.readWithdrawPubdata(pubdata, offset);
                withdrawOrStore(op.zkSyncTokenAddress, op.to, op.amount);

                opPubData = Operations.writeWithdrawPubdataForPriorityQueue(op);
            } else if (opType == Operations.OpType.ForcedWithdraw) {
                Operations.ForcedWithdraw memory op;
                (offset, op) = Operations.readForcedWithdrawPubdata(pubdata, offset);
                withdrawOrStore(op.zkSyncTokenAddress, op.target, op.amount);
            ...
            processablePriorityOperationsHash = Utils.concatHash(processablePriorityOperationsHash, opPubData);
        }

        require(processablePriorityOperationsHash == _blockExecuteData.storedBlock.pendingOnchainOperationsHash, "m"); // incorrect onchain ops executed
    }
    ...
    function executeBlocks(ExecuteBlockInfo[] memory _blocksData) external nonReentrant {
        ...
        for (uint32 i = 0; i < nBlocks; ++i) {
            ...
            priorityRequestsExecuted = _blocksData[i].storedBlock.lastProcessedPriorityOp;
    ...
    function revertBlocks(StoredBlockInfo[] memory _blocksToRevert) external nonReentrant {
        ...
        uint64 resultLastProcessedPriorityRequest = totalCommittedPriorityRequests;
        ...
        for (uint32 i = 0; i < blocksToRevert; ++i) {
            ...
            --blocksCommitted;
            resultLastProcessedPriorityRequest = storedBlockInfo.lastProcessedPriorityOp;
        }

        totalBlocksCommitted = blocksCommitted;
        totalCommittedPriorityRequests = resultLastProcessedPriorityRequest;
        ...
        returns (bytes32 commitment)
    {
        ...
        hash = sha256(abi.encodePacked(hash, _newBlockData.lastProcessedPriorityOp));
        hash = sha256(abi.encodePacked(hash, _newBlockData.unpriorityOpsOnchainDataHash));

        bytes memory pubdata =
            abi.encodePacked(
                _newBlockData.priorityOperationsData,
                _newBlockData.deployedContracts,
                _newBlockData.storageUpdateLogs
            );
    ...
    /// @notice Checks that add token operation is same as operation in priority queue
    /// @param _addToken AddToken data
    /// @param _priorityRequestId Operation's id in priority queue
    function checkPriorityOperation(Operations.AddToken memory _addToken, uint64 _priorityRequestId) internal view {
        Operations.OpType priorReqType = priorityRequests[_priorityRequestId].opType;
        require(priorReqType == Operations.OpType.AddToken, "H1"); // incorrect priority op type

        bytes20 hashedPubdata = priorityRequests[_priorityRequestId].hashedPubData;
        require(Operations.checkAddTokenInPriorityQueue(_addToken, hashedPubdata), "I1");
    }
    ...
    function checkPriorityOperation(Operations.Deposit memory _deposit, uint64 _priorityRequestId) internal view {
        Operations.OpType priorReqType = priorityRequests[_priorityRequestId].opType;
        require(priorReqType == Operations.OpType.Deposit, "H2"); // incorrect priority op type

        bytes20 hashedPubdata = priorityRequests[_priorityRequestId].hashedPubData;
        require(Operations.checkDepositInPriorityQueue(_deposit, hashedPubdata), "I2");
    }

    /// @notice Checks that add token operation is same as operation in priority queue
    /// @param _withdraw Withdraw data
    /// @param _priorityRequestId Operation's id in priority queue
    function checkPriorityOperation(Operations.Withdraw memory _withdraw, uint64 _priorityRequestId) internal view {
        Operations.OpType priorReqType = priorityRequests[_priorityRequestId].opType;
        require(priorReqType == Operations.OpType.Withdraw, "H3"); // incorrect priority op type

        bytes20 hashedPubdata = priorityRequests[_priorityRequestId].hashedPubData;
        require(Operations.checkWithdrawInPriorityQueue(_withdraw, hashedPubdata), "I3");
    }

    /// @notice Checks that add token operation is same as operation in priority queue
    /// @param _forcedWithdraw ForcedWithdraw data
    /// @param _priorityRequestId Operation's id in priority queue
    function checkPriorityOperation(Operations.ForcedWithdraw memory _forcedWithdraw, uint64 _priorityRequestId)
        internal
        view
    {
        Operations.OpType priorReqType = priorityRequests[_priorityRequestId].opType;
        require(priorReqType == Operations.OpType.ForcedWithdraw, "H4"); // incorrect priority op type

        bytes20 hashedPubdata = priorityRequests[_priorityRequestId].hashedPubData;
        require(Operations.checkForcedWithdrawInPriorityQueue(_forcedWithdraw, hashedPubdata), "I4");
    }

    /// @notice Checks that migrate to porter operation is same as operation in priority queue
    /// @param _migrateToPorter MigrateToPorter data
    /// @param _priorityRequestId Operation's id in priority queue
    function checkPriorityOperation(Operations.MigrateToPorter memory _migrateToPorter, uint64 _priorityRequestId)
        internal
        view
    {
        Operations.OpType priorReqType = priorityRequests[_priorityRequestId].opType;
        require(priorReqType == Operations.OpType.MigrateToPorter, "H5"); // incorrect priority op type

        bytes20 hashedPubdata = priorityRequests[_priorityRequestId].hashedPubData;
        require(Operations.checkMigrateToPorterInPriorityQueue(_migrateToPorter, hashedPubdata), "I5");
    }
...
/// @title zkSync public interface contract containing functions accessible to the wide audience.
/// @author Matter Labs
contract PublicInterface is BaseInterface, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function addToken(address _token) external {
        requireActive();
        if (governance.isGovernor(msg.sender) == false) {
            ...
            erc20.balanceOf(self);
            uint256 allowance = erc20.allowance(msg.sender, self);
            erc20.transferFrom(msg.sender, self, allowance);
            erc20.transfer(msg.sender, allowance);
        }

        // Check that token address is not yet added and add it.
        require(addedTokens[_token] == false, "1e"); // token exists
        addedTokens[_token] = true;
        registerAddToken(_token);
    }
    ...
    function depositETH(address _zkSyncAddress) external payable {
        requireActive();
        registerDeposit(ZKSYNC_ETH_ADDRESS, msg.value, _zkSyncAddress);
    }
    ...
    function depositERC20(
        ...
        require(Utils.transferFromERC20(_token, msg.sender, address(this), _amount), "c"); // token transfer failed deposit
        uint256 balanceAfter = _token.balanceOf(address(this));
        uint256 depositAmount = balanceAfter.sub(balanceBefore);

        registerDeposit(address(_token), depositAmount, _zkSyncAddress);
    }
    ...
    function getPendingBalance(address _address, address _token) public view returns (uint256) {
        // Translate ETH address to its zkSync address.
        if (_token == address(0)) {
            _token = ZKSYNC_ETH_ADDRESS;
        }
        return pendingBalances[packAddressAndToken(_address, _token)].balanceToWithdraw;
    }
    ...
    function withdrawPendingBalance(
        ...
        if (_token == address(0)) {
            registerWithdrawal(ZKSYNC_ETH_ADDRESS, _amount, _owner);
            (bool success, ) = _owner.call{value: _amount}("");
            require(success, "d"); // ETH withdraw failed
        } else {
            bytes32 packedBalanceKey = packAddressAndToken(_owner, _token);
            uint256 balance = pendingBalances[packedBalanceKey].balanceToWithdraw;
            // We will allow withdrawals of `value` such that:
            // `value` <= user pending balance
            // `value` can be bigger then `_amount` requested if token takes fee from sender in addition to `_amount` requested
            uint256 withdrawnAmount = this._transferERC20(IERC20(_token), _owner, _amount, balance);
            registerWithdrawal(_token, withdrawnAmount, _owner);
        }
    }
    ...
    /// @param _proof Proof
    /// @param _zkSyncTokenAddress Verified token id
    ...
    function performExodus(
        ...
        bytes32 packedBalanceKey = packAddressAndToken(_owner, _zkSyncTokenAddress);
        require(exodusMode, "s"); // must be in exodus mode
        require(!performedExodus[_accountId][_zkSyncTokenAddress], "t"); // already exited
        ...
        increaseBalanceToWithdraw(packedBalanceKey, _amount);
        performedExodus[_accountId][_zkSyncTokenAddress] = true;
    }

    /// @notice Register AddToken request - pack pubdata, add priority request and emit OnchainDeposit event
    /// @param _tokenAddress Address of token
    function registerAddToken(address _tokenAddress) internal {
        // Priority Queue request
        Operations.AddToken memory op = Operations.AddToken({tokenAddress: _tokenAddress});
        bytes memory pubData = Operations.writeAddTokenPubdataForPriorityQueue(op);
        addPriorityRequest(Operations.OpType.AddToken, pubData);
    }

    /// @notice Register deposit request - pack pubdata, add priority request and emit OnchainDeposit event
    /// @param _zkSyncTokenAddress Address of token in zkSync network
    /// @param _amount Token amount
    ...
    function registerDeposit(
        ...
        // Priority Queue request
        Operations.Deposit memory op =
            Operations.Deposit({owner: _owner, zkSyncTokenAddress: _zkSyncTokenAddress, amount: _amount});
        bytes memory pubData = Operations.writeDepositPubdataForPriorityQueue(op);
        addPriorityRequest(Operations.OpType.Deposit, pubData);
        emit Deposit(_zkSyncTokenAddress, _amount);
    }

    /// @notice Register withdraw request - pack pubdata, add priority request
    /// @param _token Token address, 0 address for ether
    /// @param _amount Amount of funds to be withdrawn
    /// @param _to Recipients for the withdrawn funds
    function requestWithdraw(
        ...
        // Priority Queue request
        Operations.Withdraw memory op =
            Operations.Withdraw({zkSyncTokenAddress: zkSyncTokenAddress, to: _to, amount: _amount});
        bytes memory pubData = Operations.writeWithdrawPubdataForPriorityQueue(op);
        addPriorityRequest(Operations.OpType.Withdraw, pubData);

        // User must fill storage slot of balancesToWithdraw(_to, zkSyncTokenAddress) with nonzero value
        // In this case operator should just overwrite this slot during confirming withdrawal
        bytes32 packedBalanceKey = packAddressAndToken(_to, zkSyncTokenAddress);
        pendingBalances[packedBalanceKey].gasReserveValue = FILLED_GAS_RESERVE_VALUE;
    }

    /// @notice Register MigrateToPorter request - pack pubdata, add priority request
    function requestMigrateToPorter() public nonReentrant {
        requireActive();

        // Priority Queue request
        Operations.MigrateToPorter memory op = Operations.MigrateToPorter({accountAddress: address(msg.sender)});
        bytes memory pubData = Operations.writeMigrateToPorterPubdataForPriorityQueue(op);
        addPriorityRequest(Operations.OpType.MigrateToPorter, pubData);
    }
    ...
    function registerWithdrawal(
        ...
        bytes32 packedBalanceKey = packAddressAndToken(_to, _token);
        uint256 balance = pendingBalances[packedBalanceKey].balanceToWithdraw;
    ...
    function addPriorityRequest(Operations.OpType _opType, bytes memory _pubData) internal {
        ...
        uint64 nextPriorityRequestId = totalPriorityRequests;
...
contract Verifier is KeysWithPlonkVerifier, KeysWithPlonkVerifierOld, Config {
    ...
    function verifyExitProof(
        ...
        bytes32 commitment = sha256(abi.encodePacked(_rootHash, _accountId, _owner, _zkSyncTokenAddress, _amount));
...
/// @title zkSync Base contract with common functions used alongside the codebase.
/// @author Matter Labs
contract BaseInterface is Storage, Config, Events {
    using SafeMath for uint256;
    ...
    function increaseBalanceToWithdraw(bytes32 _packedBalanceKey, uint256 _amount) internal {
        uint256 balance = pendingBalances[_packedBalanceKey].balanceToWithdraw;
