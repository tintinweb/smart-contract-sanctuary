...
contract AdvancedCollectible is ERC721, VRFConsumerBase, Ownable {
    ...
    string[] public metadata = new string[](maxSupply);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        ...
        uint256 randomized = randomNumber % (maxSupply-tokenCounter);
        string memory chosenOne= metadata[randomized]; 
        remove(randomized);
        _setTokenURI(newItemId, chosenOne);
        /* tokenIdToBreed[newItemId] = chosenOne; */
        requestIdToTokenURI[requestId] = chosenOne;
...
// pass METADATA array to contract so it can be picked up randomly during mint
    function fillMetadata(string[] memory _m) public onlyOwner() {
      metadata = _m;
    }
    function emptyMedata() public onlyOwner() {
      delete metadata;
    }
    ...
    function withdraw() public payable onlyOwner {
     // TODO: implement a transfer from smart contract msg.value / 2 to msig and to artist. I should keep track of tips in MINT function to I can send the tipcs to DAO.
