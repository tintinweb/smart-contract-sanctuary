...
interface IRandom {
  function Random(uint256 xyz) external returns (uint256 seedGenerator);
}
...
contract stakingContract is OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(address _erc721contract, address _erc20contract, address _seed, address _headdaostaking) initializer public {
  ...
  function _claimHeads(uint256 tokenId, bool unstake, bool payKings) private returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      if (seedGenerator.Random(tokenId) & 1 == 1) {                                         
        _payHuntersTax(owed);
        owed = 0;  
      }
  ...
  function setSeedGenerator(address _seed) external onlyOwner {
  ...
  function setInit(address _erc721contract, address _erc20contract, address _seed) external onlyOwner{
    erc721Contract = IMint(_erc721contract);                                          
    erc20Contract = IHead(_erc20contract);  
    seedGenerator = IRandom(_seed);                                      
  ...
  function setPaymentContract(address _new) external onlyOwner {
    paymentContract = _new;
