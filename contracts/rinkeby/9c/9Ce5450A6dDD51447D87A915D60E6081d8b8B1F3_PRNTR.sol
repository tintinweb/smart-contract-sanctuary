...
/**
 * This contract is the one of $PRNTR ERC20 token.
 * by PaprPrintr
 *
 * _____                     _____        _         _         
 *|  __ \                   |  __ \      (_)       | |        
...
contract PRNTR is ERC20Burnable, DoubleOperator {
    ...
    function addSenderExcludedAddresses(address newAddress) external onlyDoubleOperator returns(bool) {
    ...
    function removeSenderExcludedAddresses(address oldAddress) external onlyDoubleOperator returns(bool) {
    ...
    // Add an address to the exluded mapping
    function addrecipientExcludedAddresses(address newAddress) external onlyDoubleOperator returns(bool) {
    ...
    // Removes an address from the excluded mapping
    function removeRecipientExcludedAddresses(address oldAddress) external onlyDoubleOperator returns(bool) {
    ...
    // Get fees (index) :
    function getfeesTaxRate() public view returns(uint256) {
        return feesTaxRate;
    }

    // Set fees (index) :
    function setfeesTaxRate(uint256 fees) external onlyDoubleOperator {
        require(fees <= 45);
        feesTaxRate = fees;
    }
