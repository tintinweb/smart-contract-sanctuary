 ...
 * @dev This contract implements Counters for ERC1155 rather than ERC721
 *
...
contract ERC1155Metadata is IERC1155Metadata, ERC165 {
    ...
    function uri(uint256 _id) public override view returns (string memory) {
        ...
        // If both are set, concatenate the baseMetadataURI and _tokenURI via abi.encodePacked()
        if (bytes(_tokenURI).length > 0) {
            return string(abi.encodePacked(baseMetadataURI, _tokenURI));
        }

        // If there is a baseMetadataURI but no _tokenURI, concatenate _id to the base URI
        return string(abi.encodePacked(baseMetadataURI, _uint2str(_id)));
    }

    /**
     * @dev Sets '_tokenURI' as the new tokenURI of '_id'
     */
    function _setTokenURI(uint256 _id, string memory _tokenURI) internal virtual {
    ...
    function _logURIs(uint256[] memory _tokenIds) internal {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            tokenURI = string(abi.encodePacked(baseURI, _uint2str(_tokenIds[i])));
...
contract CorporateVerifier is ERC1155, ERC1155Metadata, ERC1155MintBurn, Ownable {
    // Contract name
    string public name;
    // Contract symbol
    ...
    function mint(address _to, uint256 _amount, string memory _tokenURI, bytes memory _data) external onlyOwner {
        _tokenIds.increment();
