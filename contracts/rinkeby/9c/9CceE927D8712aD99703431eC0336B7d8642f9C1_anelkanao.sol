...
contract anelkanao is ERC721PresetMinterPauserAutoId, VRFConsumerBase, Data  {
    ...
    address internal tokenAddress = 0x0CC8c52D259aCb911E0Eec1Ac920fAA727b840e7; // change this to ;
    ...
    constructor(
        ...
        ERC721PresetMinterPauserAutoId("maklwb", "beas", "ipfs://") 
    {
    ...
    function batchPlay(uint256[] memory tokenIds) public {
        for (uint i = 0; i < tokenIds.length; i++) {
            play(tokenIds[i]);
        }
    }
    ...
    function prestige(uint256 tokenId) public {
        ...
        if (device.color == 1) { device.name = names[0]; }
        if (device.color == 2) { device.name = names[10]; }
        if (device.color == 3) { device.name = names[20]; }
        if (device.color == 4) { device.name = names[30]; }
        if (device.color == 5) { device.name = names[40]; }
    ...
    //     uint256 rewards_, uint256 tokensToRevive_, uint256 color_ ) {
    ...
    //         device.rewards, device.tokensToRevive, device.color
    //     );
    // }
...
contract Data is Ownable {
    ...
    string internal EGG_IPFS;
    string internal DEAD_IPFS;
    
    string[50] internal names;
    string[50] internal metadata;
    ...
    /* ERROR MESSAGESS FROM REQUIREMENTS
        ...
        !e = not egg
        !g = either generation 0 or generation 11
