...
contract GenesisStakeFactory is IGenesisStakeFactory, Managed, EIP712 {
    ...
    function createGenesisStaking(
        ...
        IStakingRegister(management.contractRegistry(CONTRACT_STAKING_REGISTER))
            .add(msg.sender, address(staking), false);
    }
...
contract GenesisStaking is IStaking, Managed {
    ...
    function stakeETH() external payable canStake(isETHStake) {
    ...
    {
        address factory = management.contractRegistry(CONTRACT_STAKING_REGISTER);
        ...
        canCallOnlyRegisteredContract(CONTRACT_STAKING_REGISTER)
    {
        ...
        if (rewardTokenAddressIndex[_token] == 0) {
            ...
            uint256 notUseRewards = _amount.sub(_getRewardMustBePaid(_token));
            erc20.transfer(_recipient, notUseRewards);
        }
        ...
        updateRewards(_addr)
    {
        ...
        if (depositFee > 0) {
            ...
            if (isETHStake) {
                tresuary.transfer(fee);
            } else {
                IERC20(stakedToken).transferFrom(_addr, tresuary, fee);
            }
        }

        if (!isETHStake) {
            IERC20(stakedToken).transferFrom(_addr, address(this), value);
        }
