...
// solhint-disable state-visibility, const-name-snakecase
contract Factory is IFactory, Ownable, ReentrancyGuard {
    ...
    string constant internal baseMetadataURI = "URI of marketplace";
    uint256 constant UINT256_MAX = ~uint256(0);

    uint256 constant SUPPLY_PER_TOKEN_ID = UINT256_MAX;

    /**
     * Three different options for minting Collectibles
     */
    enum Option {
        Basic,
        Premium,
    ...
    function canMint(uint256 _optionId, uint256 _amount) external view override returns (bool) {
        return _canMint(_msgSender(), Option(_optionId), _amount);
    }
    ...
    function balanceOf(address _owner, uint256 _optionId) public view override returns (uint256) {
        ...
        Collection nftContract = Collection(nftAddress);
    ...
    function _mint(
        ...
        require(_canMint(_msgSender(), _option, _amount), "Factory: cannot mint more");
...
contract Collection is ERC1155Custom {
    constructor(
        string memory _collectionName,
        string memory _collectionSymbol,
        string memory _baseURI,
        address _proxyRegistryAddress
    ) ERC1155Custom(_collectionName, _collectionSymbol, _proxyRegistryAddress)
    {
...
import "../token/ERC1155/ERC1155Metadata.sol";
import "../utils/libraries/Strings.sol";
import "../utils/math/SafeMath.sol";
import "../market/ProxyRegistry.sol";

contract ERC1155Custom is ERC1155, ERC1155Metadata, Ownable {
    using Strings for string;
    using SafeMath for uint256;
    ...
    modifier creatorOnly(uint256 _id) {
        require(creators[_id] == _msgSender(), "ERC1155Tradable: only creator allowed");
        _;
    }

    modifier ownersOnly(uint256 _id) {
        require(_balances[_id][_msgSender()] > 0, "ERC1155Tradable: only owners allowed");
        _;
    }
    ...
    function uri(uint256 _id) public view override(ERC1155, ERC1155Metadata) returns (string memory) {
        require(_exists(_id), "ERC1155Tradable: non-existent token");
...
contract ERC1155Metadata {
    ...
    /**
     * @notice Will update the base URI of token's URI
     * @param _newBaseMetadataURI New base URL of token's URI
     */
