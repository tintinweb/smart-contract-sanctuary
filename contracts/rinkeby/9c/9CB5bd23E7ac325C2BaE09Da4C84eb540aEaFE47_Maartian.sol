...
contract Maartian is IMaartian, ERC721Enumerable, Ownable, Pausable {
  ...
  mapping(uint256 => MinerMartian) public tokenTraits;
  // mapping from hashed(tokenTrait) to the tokenId it's associated with
  // used to ensure there are no duplicates
  mapping(uint256 => uint256) public existingCombinations;

  // list of probabilities for each trait type
  // 0 - 9 are associated with Miner, 10 - 18 are associated with Wolves
  uint8[][18] public rarities;
  // list of aliases for Walker's Alias algorithm
  // 0 - 9 are associated with Miner, 10 - 18 are associated with Wolves
  uint8[][18] public aliases;

  // reference to the Barn for choosing random Martian thieves
  IBarn public barn;
  // reference to $INGOT for burning on mint
  Ingot public ingot;
  // reference to Traits
   ...
   */
  constructor(address _ingot, address _traits, uint256 _maxTokens) ERC721("Miners Vs. Martians", 'MGAME') { 
    ingot = Ingot(_ingot);
  ...
  /** 
   * mint a token - 90% Miner, 10% Wolves
   * The first 20% are free to claim, the remaining cost $INGOT
   */
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      totalIngotCost += mintCost(minted);
    }
    
    if (totalIngotCost > 0) ingot.burn(_msgSender(), totalIngotCost);
    if (stake) barn.addManyToBarnAndPack(_msgSender(), tokenIds);
  }

  /** 
   * the first 20% are paid in ETH
   * the next 20% are 20000 $INGOT
   * the next 40% are 40000 $INGOT
   * the final 20% are 80000 $INGOT
   ...
   */
  function generate(uint256 tokenId, uint256 seed) internal returns (MinerMartian memory t) {
    t = selectTraits(seed);
   ...
   * the remaining 80% have a 10% chance to be given to a random staked martian
   * @param seed a random value to select a recipient from
   * @return the address of the recipient (either the minter or the Martian thief's owner)
   */
  function selectRecipient(uint256 seed) internal view returns (address) {
    if (minted <= PAID_TOKENS || ((seed >> 245) % 10) != 0) return _msgSender(); // top 10 bits haven't been used
    address thief = barn.randomMartianOwner(seed >> 144); // 144 bits reserved for trait selection
   ...
   */
  function selectTraits(uint256 seed) internal view returns (MinerMartian memory t) {    
    t.isMiner = (seed & 0xFFFF) % 10 != 0;
    uint8 shift = t.isMiner ? 0 : 9;
    seed >>= 16;
   ...
   */
  function structToHash(MinerMartian memory s) internal pure returns (uint256) {
  ...
  function getTokenTraits(uint256 tokenId) external view override returns (MinerMartian memory) {
  ...
  /**
   * called after deployment so that the contract can get random martian thieves
...
interface IBarn {
  ...
  function randomMartianOwner(uint256 seed) external view returns (address);
}
...
contract Ingot is ERC20, Ownable {

  // a mapping from an address to whether or not it can mint / burn
  mapping(address => bool) controllers;
  
  constructor() ERC20("Ingot", "INGOT") { }

  /**
   * mints $INGOT to a recipient
   * @param to the recipient of the $INGOT
   * @param amount the amount of $INGOT to mint
   */
  ...
  /**
   * burns $INGOT from a holder
   * @param from the holder of the $INGOT
   * @param amount the amount of $INGOT to burn
   */
