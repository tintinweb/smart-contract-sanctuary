...
contract BlootWheels is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getWave(uint256 tokenId) public view returns (string memory) {
        ...
        for(uint j = 1; j < 5; j++)
        {
            ...
            wave = string(abi.encodePacked(wave, '<path id="gentle-wave', toString(j), '" d="M-310 310c30 0 58-', h1, ' 88-', h1, 's 58 ', h1, ' 88 ', h1 ));
            ...
            wave = string(abi.encodePacked(wave, ' v',toString(41- j * 2) ,'h-1000z"/>'));
        }

        wave = string(abi.encodePacked(wave, '</defs><g class="parallax"><use xlink:href="#gentle-wave1" x="48" y="1" fill="rgba(', color, ' ,0.7)" /><use xlink:href="#gentle-wave2" x="48" y="3" fill="rgba(', color, ', 0.5)" /><use xlink:href="#gentle-wave3" x="48" y="5" fill="rgba(', color, ',0.3)" /><use xlink:href="#gentle-wave4" x="48" y="7" fill="rgba(', color, ', 0.4)" /></g></svg>'));
