...
contract FroyoCatMachine is Ownable {
    event Claim(address user, address to, uint256 nftId, uint256 time);

    FroyoCat froyoCat;

    constructor(FroyoCat _froyoCat) {
        froyoCat = _froyoCat;
    }

    function updateFroyoCat(FroyoCat _froyoCat) external onlyOwner {
        froyoCat = _froyoCat;
    }
    ...
    function adminClaim(uint256 _nftId) external onlyOwner {
        address nftOwner;
        bool claimed;
        (nftOwner, claimed, ) = froyoCat.get(_nftId);
...
contract FroyoCat is ERC721, ERC721Enumerable, Ownable, ERC721Pausable {
    ...
    FroyoCatMachine froyoCatMachine;
    ...
    function updateFroyoCatMachine(FroyoCatMachine _froyoCatMachine)
        external
        onlyOwner
    {
