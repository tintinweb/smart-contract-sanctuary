...
// every mint money to be forwarded 
...
contract ShogunNFT is ERC721Enumerable, Ownable {
  ...
  address payable public treasury; // treasury wallet that money flows to
  ...
  uint256 public presaleStartTime = 1634342400; // 16th October, Saturday, 0800 SGT
  uint256 public publicSaleStartTime = 1634443200; // 17th October, Sunday 1200
  ...
  mapping(address => uint256) public publicAddressMintedAmount; // number of NFT minted for each wallet during public sale
  ...
  function devMint(uint256 _mintAmount) public payable onlyEOA onlyOwner{
      ...
      require(supply + _mintAmount <= maxSupply, "total mint amount exceeded supply, try lowering amount");
  ...
  function publicMint(uint256 _mintAmount) public payable onlyEOA{
    ...
    uint256 ownerMintedCount = publicAddressMintedAmount[msg.sender];
    require(ownerMintedCount + _mintAmount <= nftPerAddressLimitPublic, "You have exceeded max amount of mints");
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintPerTxn, "exceeded max min amount per transaction");
    require(supply + _mintAmount <= maxSupply, "total mint amount exceeded supply, try lowering amount");
    ...
    require(msg.value >= cost * _mintAmount, "not enough ether sent for mint amount");
    for (uint256 i = 1; i <= _mintAmount; i++) {
  ...
  function presaleMint(uint256 _mintAmount) public payable onlyEOA{
      ...
      uint256 supply = totalSupply();
      uint256 ownerMintedCount = presaleAddressMintedAmount[msg.sender];
      require(ownerMintedCount + _mintAmount <= nftPerAddressLimitPresale, "you can only mint a maximum of two nft during presale");
      require(msg.value >= cost * _mintAmount, "not enought ethere sent for mint amount");
  ...
  function setmaxMintPerTxn(uint256 _newmaxMintAmount) public onlyOwner() {
    maxMintPerTxn = _newmaxMintAmount;
  }
  ...
  function whitelistUsers(address[] calldata _users) public onlyOwner {
    // need a way to clear the mapping in case we are updating from a logner whitelist to a shorter whitelist
  ...
  function withdrawToTreasury() public payable onlyOwner {
    ...
    require(success); // check if successful, need to make use of variable
  }
