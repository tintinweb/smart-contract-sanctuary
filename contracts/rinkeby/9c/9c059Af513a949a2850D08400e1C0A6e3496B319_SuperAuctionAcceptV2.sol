...
/**
  ...
  winner.
*/
contract SuperAuctionAcceptV2 is Ownable, ReentrancyGuard {
  ...
  */
  ...
  function auctionData() external view returns (address, address, uint, uint, address, uint, uint, uint, uint) {
    return (beneficiary, address(item), groupId, highestBid, highestBidder, highestBidTime, auctionEndTime, minimumBid, bidHistory.length);
  }
  ...
  */
  function accept() public nonReentrant onlyOwner returns (bool) {
    ...
    (bool result, ) = beneficiary.call{ value: address(this).balance }("");
  ...
  */
  function remainder() external onlyOwner returns (bool){
    require(block.timestamp >= auctionEndTime.add(receiptBuffer), "Cannot claim remainder until auction has ended.");
    (bool redeemSuccess, ) = payable(msg.sender).call{ value: address(this).balance }("");
