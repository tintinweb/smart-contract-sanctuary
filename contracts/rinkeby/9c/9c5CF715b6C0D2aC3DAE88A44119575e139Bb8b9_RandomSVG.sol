...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    event requestedRandomSVG(bytes32 indexed requestId); 
    ...
    ERC721("RandomSVG", "rsNFT")
    {
        ...
        size = 500;
        pathCommands = ["M", "L"];
        colors = ["red", "blue", "green", "yellow", "black", "white", "transparent"];
    }
    ...
    // use this to do some manual testing if you like...
    // but ideally you use the mock VRF coordinator
    // function kickOff() public {
    //     bytes32 requestId = requestRandomness(keyHash, fee);
    //     requestIdToSender[requestId] = msg.sender;
    //     emit requestedRandomSVG(requestId);
    //     fulfillRandomness(requestId, 123);
    // }

    function finishMint(uint256 tokenId) public {
        require(bytes(tokenURI(tokenId)).length >= 0, "tokenURI is already set!"); 
        uint256 randomNumber = tokenIdToRandomNumber[tokenCounter];
        require(randomNumber > 0, "This tokenId is not minted yet!");
        string memory svg = generateSVG(randomNumber);
        string memory imageURI = svgToImageURI(svg);
        _setTokenURI(tokenCounter, formatTokenURI(imageURI));
        emit CreatedRandomSVG(tokenCounter, svg);
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) override internal {
        address nftOwner = requestIdToSender[requestId];
        _safeMint(nftOwner, tokenCounter);
        tokenIdToRandomNumber[tokenCounter] = randomNumber;
