...
contract FundamentaWrappedToken is SecureContract, ERC20
{
    ...
    event TransferBackingTokenTo(address indexed user, uint256 amount, uint256 fee);
    ...
    IERC20 private FMTA_;
    uint8 private decimals_;
    uint256 private heldAmtReq_;
    uint256 private fee_;
    uint256 private accumulatedFee_;

    constructor(address backingTokenContract, string memory name, string memory ticker, 
        uint8 tokenDecimals, uint256 heldAmtReq, uint256 fee, address fmtaAddress) ERC20(name, ticker)
    {
        require(backingTokenContract != address(0), "WrappedToken: Backing token not configured");
        require(fmtaAddress != address(0), "WrappedToken: FMTA token not configured");

        backingToken_ = IERC20(backingTokenContract);
        FMTA_ = IERC20(fmtaAddress);
        ...
        heldAmtReq_ = heldAmtReq;
        fee_ = fee;
        accumulatedFee_ = 0;
    }
    
    function setHeldAmountReq(uint256 newAmt) public isAdmin { heldAmtReq_ = newAmt; }

    function setFee(uint256 newFee) public isAdmin
    {
        require(newFee <= 10000, "WrappedToken: Fee exceeds 100%");
        fee_ = newFee;
    }

    function decimals() public override view returns (uint8) { return decimals_; }

    function queryHeldAmountRequired() public view returns (uint256) { return heldAmtReq_; }

    function queryFee() public view returns (uint256) { return fee_; }

    function queryAccumulatedFee() public view returns (uint256) { return accumulatedFee_; }

    function queryBackingToken() public view returns (IERC20) { return backingToken_; }

    function queryUserFmtaBalance(address user) internal view returns (uint256) { return FMTA_.balanceOf(user); }

    function calculateFee(uint256 amount) public view returns (uint256) { return (amount / 10000) * fee_; }

    function wrap(uint256 amount) public pause
    {
        require(queryUserFmtaBalance(msg.sender) >= heldAmtReq_, "WrappedToken: Must hold required amount of FMTA");
    ...
    function unwrap(uint256 amount) public pause
    {
        ...
        backingToken_.safeTransfer(msg.sender, amount - fee);
        accumulatedFee_ += fee;

        emit Unwrap(msg.sender, amount, fee);
    }

    function transferBackingTokenFrom(address user, uint256 amount) public pause
    {
        require(hasRole(_BRIDGE_TX, msg.sender), "WrappedToken: transferBackingTokenFrom - Permission denied");
        require(queryUserFmtaBalance(user) >= heldAmtReq_, "WrappedToken: Must hold required amount of FMTA");

        backingToken_.safeTransferFrom(user, address(this), amount);
        emit TransferBackingTokenFrom(user, amount);
    }

    function transferBackingTokenTo(address user, uint256 amount, uint256 fee) public pause
    {
        require(hasRole(_BRIDGE_TX, msg.sender), "WrappedToken: transferBackingTokenTo - Permission denied");
        require(queryUserFmtaBalance(user) >= heldAmtReq_, "WrappedToken: Must hold required amount of FMTA");

        backingToken_.safeTransfer(user, amount - fee);
        accumulatedFee_ += fee;

        emit TransferBackingTokenTo(user, amount, fee);
    }
    ...
    {
        require(accumulatedFee_ > 0, "WrappedToken: Accumulated fee = 0");
        backingToken_.safeTransfer(to, accumulatedFee_);
        uint256 temp = accumulatedFee_;
        accumulatedFee_ = 0;
