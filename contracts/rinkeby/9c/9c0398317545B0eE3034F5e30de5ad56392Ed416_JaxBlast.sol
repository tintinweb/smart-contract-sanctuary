...
contract JaxBlast is ERC721URIStorage, Ownable {
  ...
  function batchTransfer(address[] memory to, uint256[][] memory amount) public {
    require(isApprovedForAll(msg.sender, address(this)), "Not approved for batch");
    for (uint256 i = 0; i < to.length; i++) {
      for (uint256 j = 0; j < amount[i].length; j++) {
        safeTransferFrom(msg.sender, to[i], amount[i][j]);
      }
    }
  }
