...
contract IndexTokenFactory is Ownable{
    ...
    function createIndexToken(string memory name, string memory symbol, address treasury) external onlyOwner{
        require (_getToken[symbol] == address(0), "Token already exists!");
        IndexToken token = new IndexToken(name, symbol, treasury);
    ...
    function getSymol(address token) public view returns(string memory){
