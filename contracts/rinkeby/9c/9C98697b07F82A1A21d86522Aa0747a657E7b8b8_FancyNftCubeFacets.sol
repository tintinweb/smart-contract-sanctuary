...
import "./FancyNftCubeFacetsSale.sol";
import "./TrustedContacts.sol";
import "./RecoverableErc20ByOwner.sol";
import "./interfaces/IFancyNftCubeFacets.sol";

/// @custom:security-contact [email protected]
contract FancyNftCubeFacets is
    FancyNftCubeFacetsSale,
    TrustedContacts,
    RecoverableErc20ByOwner,
    IFancyNftCubeFacets
{
    address private constant _tokenBurnAddress =
        0x000000000000000000000000000000000000dEaD;
    address public immutable token;
    address public designers;

    uint8 public constant override totalColors = 4; // values: 1-4
    uint8 public constant override totalFacets = 3; // values: 0-2
    uint8 public constant override totalPlaces = 9; // values: 0-8
    uint8 public constant override totalCells = 27; // values: 0-26
    ...
    constructor(
        ...
        FancyNftBase(contractURI_, baseTokenURI_, proxyRegistry_, "item")
        FancyNftCubeFacetsSale(MAX_SUPPLY)
    {
        token = token_;
        designers = designers_;
    }

    function mintAirdrop(uint256 count) external {
    ...
    function mintPrivsale(uint256 count) external {
        ...
        uint256 value = privsaleValue(count);
        IERC20(token).transferFrom(_msgSender(), address(this), value);
    ...
    function mintSale(uint256 count) external {
        require(isSaleActive(), "Sale is not active");
        require(!isSaleLimit(_msgSender(), count), "Limits exceeded");
        uint256 value = saleValue(count);
        IERC20(token).transferFrom(_msgSender(), address(this), value);
    ...
    function _distribute(uint256 value) private {
        uint256 toBurn = (value * 2) / 3;
        IERC20(token).transfer(_tokenBurnAddress, toBurn);
        IERC20(token).transfer(designers, value - toBurn);
    }
    ...
    function getCellId(uint256 tokenId) public pure override returns (uint8) {
        return uint8(tokenId % totalCells);
    }

    function getPlaceId(uint256 tokenId) public pure override returns (uint8) {
        return uint8(tokenId % totalPlaces);
    }

    function getFacetId(uint256 tokenId) public pure override returns (uint8) {
        if (tokenId % totalCells < totalPlaces) return 0;
        else if (tokenId % totalCells < totalPlaces * 2) return 1;
        else return 2;
    }

    function getColorId(uint256 tokenId) public pure returns (uint8) {
        return uint8(tokenId % totalColors) + 1;
    }

    //  designers
    function setDesigners(address designers_) external onlyOwner {
        designers = designers_;
    }
...
interface IFancyNftCubeFacets {
    function totalColors() external view returns (uint8);

    function totalFacets() external view returns (uint8);

    function totalPlaces() external view returns (uint8);

    function totalCells() external view returns (uint8);
    ...
    function getCellId(uint256 tokenId) external view returns (uint8);

    function getPlaceId(uint256 tokenId) external view returns (uint8);

    function getFacetId(uint256 tokenId) external view returns (uint8);

    function getColorId(uint256 tokenId) external view returns (uint8);
}
...
/// @custom:security-contact [email protected]
abstract contract FancyNftCubeFacetsSale is FancyNftBase, Ownable {
    // Airdrop
    uint256 public airdropStartTime; // TODO
    uint256 public airdropMintsLimit = 3; // TODO
    ...
    // Presale
    uint256 public presalePrice = 400000000 ether; // TODO
    uint256 public presaleStartTime; // TODO
    uint256 public presaleMintsLimit = 10; // TODO
    ...
    // Privsale
    uint256 public privsalePrice = 500000000 ether; // TODO
    uint256 public privsaleStartTime; // TODO
    uint256 public privsaleMintsLimit = 50; // TODO
    ...
    // Sale
    uint256 public salePrice = 500000000 ether; // TODO
    ...
    function presaleMintsOf(address account) external view returns (uint256) {
