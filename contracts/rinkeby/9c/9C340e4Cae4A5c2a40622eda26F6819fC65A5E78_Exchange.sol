...
contract Exchange is Ownable {
    ...
    struct Order {
        ...
        bool isSystem;  
        bytes orderId;
        uint8 orderType; // true sell , false buy 
    }
    ...
    event Redeem(address sender, address token, uint256 amount, uint256 vaultId);
    ...
    function placeBuyOrder(
        ...
        uint256 expiry
         ...
         uint64 currentId = userCounter[msg.sender] + 1;

        userCounter[msg.sender] = currentId;

        bytes memory orderId = calcOrderId(msg.sender, currentId);
        require(now < expiry, "expiry illegal");
        require(ERC20(makerToken).balanceOf(msg.sender) >= makerAmount, "placeBuyOrder: EX11");
        require(ERC20(makerToken).allowance(msg.sender, address(this)) >= makerAmount, "placeBuyOrder: EX12");
        ...
        ERC20(makerToken).transferFrom(msg.sender, address(this), makerAmount);
        return order.orderId;
    }


    function settlement(bytes memory buyOrderId, uint buyMakerAmount,uint buyTakerAmount, bytes memory sellOrderId) public{
        Order memory buyOrder = orderMap[buyOrderId];
        Order memory sellOrder = orderMap[sellOrderId];
        
        require(buyOrder.maker != address(0) && sellOrder.maker != address(0), "settlement:EX1");
        require(buyOrder.status == OrderStatus.Init || buyOrder.status == OrderStatus.PartFilled, "settlement buy: EX9");
        require(sellOrder.status == OrderStatus.Init || sellOrder.status == OrderStatus.PartFilled, "settlement sell: EX9");

        require(now < sellOrder.expiry && now < sellOrder.expiry,"settlement:EX8");
        require(buyOrder.makerToken == sellOrder.takerToken && buyOrder.takerToken == sellOrder.makerToken,"settlement:EX13");
        uint buyFillableMakerAmount = SafeMath.sub(buyOrder.makerAmount, buyOrder.FilledMakerAmount);
        uint buyFillableTakerAmount = SafeMath.sub(buyOrder.takerAmount, buyOrder.FilledTakerAmount);

        uint sellFillableMakerAmount = SafeMath.sub(sellOrder.makerAmount, sellOrder.FilledMakerAmount);
        uint sellFillableTakerAmount = SafeMath.sub(sellOrder.takerAmount, sellOrder.FilledTakerAmount);

        require(buyMakerAmount <= buyFillableMakerAmount && buyTakerAmount <= buyFillableTakerAmount,"settlement:EX14");

        require(buyMakerAmount <= sellFillableTakerAmount && buyTakerAmount <= sellFillableMakerAmount,"settlement:EX15");

        buyOrder.FilledMakerAmount += buyMakerAmount;
        buyOrder.FilledTakerAmount += buyTakerAmount;

        sellOrder.FilledMakerAmount += buyTakerAmount;
        sellOrder.FilledTakerAmount += buyMakerAmount;

        // transfer from ex to buyer  buyerOrder' takerToken
        ERC20(buyOrder.takerToken).transfer(buyOrder.maker, buyTakerAmount); 
        // transfer from ex to seller buyerOrder' makeToken
        ERC20(buyOrder.makerToken).transfer(sellOrder.maker, buyMakerAmount);
    ...
    function _place(
        ...
        uint64 currentId = userCounter[seller] + 1;
        ...
        returns (uint256 takerTokenFilledAmount, uint256 makerTokenFilledAmount)
    {
        ...
        // just system seller order need record sellerToken
        if(order.isSystem == true){
            sellerToken[order.maker][order.makerToken] = SafeMath.add(
                sellerToken[order.maker][order.makerToken],
                fillMakerAmount
            );
        }
    ...
    function cancelOrder(bytes memory orderId, uint256 vaultId) external returns (bool) {
        ...
        require(remainingMakerAmount > 0, "cancelOrder: EX10");
        ...
        if (order.isSystem) {
            ...
            uint256 strikePrice = Token(order.makerToken).strikePrice();
            uint256 collateralAmount;
            if(Token(order.makerToken).isPut()){
                collateralAmount = SafeMath
                        .mul(strikePrice, remainingMakerAmount)
                        .mul(10**uint256(ERC20(Token(order.makerToken).collateralAsset()).decimals()))
                        .div(10**uint256(ERC20(order.makerToken).decimals()))
                        .div(10**uint256(STRIKE_PRICE_DIGITS));

            }else{
                collateralAmount = SafeMath
                        .mul(remainingMakerAmount,10**uint256(ERC20(Token(order.makerToken).collateralAsset()).decimals()))
                        .div(10**uint256(ERC20(order.makerToken).decimals()));
            }
            ...
            _actions[1] = Actions.ActionArgs({
                ...
                asset: Token(order.makerToken).collateralAsset(),
                vaultId: vaultId,
     ...
     */
    function settleCollateral(address maker, address token) external view returns (uint256) {
    ...
    function redeem(
        ...
        require(sellerToken[msg.sender][oToken] >= amount, "sellerToken balacnce not enough");
        require(ERC20(oToken).balanceOf(msg.sender) >= amount, "oToken balance of not enough");

        Actions.ActionArgs[] memory _actions = new Actions.ActionArgs[](2);

        uint256 strikePrice = Token(oToken).strikePrice();

        // eg: STRIKE_PRICE_DIGITS 1e8
        // strikePrice = 4000* STRIKE_PRICE_DIGITS  collateralAsset USDC = 1e6
        // oToken decimals = 1e8 , amount x*1e8
        // collateralAmount = 4000* 1e8 * x*1e8 * 1e6 / 1e8/1e8
        //                  = x*4000*1e6
        uint256 collateralAmount;

        if(Token(oToken).isPut()){
            collateralAmount = SafeMath
                    .mul(strikePrice, amount)
                    .mul(10**uint256(ERC20(Token(oToken).collateralAsset()).decimals()))
                    .div(10**uint256(ERC20(oToken).decimals()))
                    .div(10**uint256(STRIKE_PRICE_DIGITS));

        }else{
            collateralAmount = SafeMath
                    .mul(amount,10**uint256(ERC20(Token(oToken).collateralAsset()).decimals()))
                    .div(10**uint256(ERC20(oToken).decimals()));
        }
        ...
        _actions[1] = Actions.ActionArgs({
            ...
            owner: msg.sender,
            secondAddress: msg.sender,
            asset: Token(oToken).collateralAsset(),
            vaultId: vaultId,
            amount: collateralAmount,
            index: 0,
            data: new bytes(1)
        ...
        sellerToken[msg.sender][oToken] = SafeMath.sub(sellerToken[msg.sender][oToken], amount);

        emit Redeem(msg.sender, oToken, amount, vaultId);
    }

    /**
     * when not sell all oToken by manual place order 
     * can invoke this claim oToken income
     */
    function manualClaim(address oToken) public {
        uint sellAmount = userToken[msg.sender][oToken];
        require(sellAmount > 0,"manualClaim:EX16");
        ...
        Controller(controller).operate(_actions);
        userToken[msg.sender][oToken] = 0;
    }

    function calcOrderId(address addr, uint64 x) public pure returns (bytes memory b) {
        b = new bytes(32);
        assembly {
            mstore(add(b, 32), x)
        }
