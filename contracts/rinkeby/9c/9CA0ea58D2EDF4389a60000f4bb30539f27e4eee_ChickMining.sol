...
contract ChickMining is Ownable, ReentrancyGuard {
    ...
    bool public isEnableWhite = true;

    mapping(address => bool) public miningAddress;

    uint256 public player;

    uint256 public whiteQuota;
    ...
    constructor(address _mining_token, uint256 _mining_start_block) public {
        ...
        phase1TokenPerBlock = 267 * 1e16;

        whiteQuota = uint256(1888).mul(busdBaseUnit);
    }

    function updatawWhiteQuota(uint256 quotaAmount) public onlyOwner{
        whiteQuota = quotaAmount.mul(busdBaseUnit);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant onlyWhitelisted {
        require(_amount <= whiteQuota, "single account investment not more than 1888 busd！");
        require(_pid < poolInfo.length, "invalid _pid");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if(!miningAddress[msg.sender]){
            miningAddress[msg.sender] = true;
            player++;
        }
    }
    ...
    function updateWhiteSwitch(bool _isEnableWhite) public onlyOwner {
        isEnableWhite = _isEnableWhite;
    }
}