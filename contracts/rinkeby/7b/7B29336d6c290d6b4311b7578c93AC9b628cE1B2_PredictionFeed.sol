...
contract PredictionFeed is Ownable, ChainlinkClient {
    ...
    function initialize(
        ...
        jobId_ = "60f206a5128040fab7a4cd543456c8c0";
        fee_ = 0.1 * (10**18);
    ...
    modifier isValidTimes(uint256 _startTime, uint256 _endTime) {
        require(_startTime > now, "PredictionFeed: Start time is already gone");
        require(_endTime > now, "PredictionFeed: End time is already gone");
        require(
        ...
        for (uint256 i = 0; i < participants[_poolId].length; i++) {
            if (participants[_poolId][i].vote == _poolVote) {
                ...
                require(
                    ...
                    "PredictionFeed: Transfer failed"
                );
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        pools[poolId].initialPrice = getPoolPrice(poolId);
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        pools[poolId].finalPrice = getPoolPrice(poolId);
        ...
        emit PoolEnded(poolId, pools[poolId].finalPrice, winner);
    }
    ...
    //         emit PoolEnded(_poolId, pools[_poolId].finalPrice, "UP");
    //     } else if (pools[_poolId].finalPrice < pools[_poolId].initialPrice) {
    //         _presentAward(_poolId, PoolVote.DOWN);
    //         emit PoolEnded(_poolId, pools[_poolId].finalPrice, "DOWN");
    //     } else {
    //         _presentAward(_poolId, PoolVote.SAME);
    //         emit PoolEnded(_poolId, pools[_poolId].finalPrice, "SAME");
    //     }
    // }
