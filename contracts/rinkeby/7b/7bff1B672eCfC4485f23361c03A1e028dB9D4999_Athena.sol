...
contract Athena is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 5 ether;
    // Initial distribution for the day 2-5 ATH-WFTM LP -> ATH pool
    uint256 public constant INITIAL_ATH_POOL_DISTRIBUTION = 100 ether;
    // Distribution for airdrops wallet
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 10 ether;
    ...
    /**
     * @notice Constructs the ATH ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) ERC20("ATH", "ATH") {
        // Mints 1 ATH to contract creator for initial pool setup
    ...
    function _getAthenaPrice() internal view returns (uint256 _athenaPrice) {
        try IOracle(athenaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            ...
            revert("Athena: failed to fetch ATH price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _athenaPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_athenaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setAthenaOracle(address _athenaOracle) public onlyOperatorOrTaxOffice {
    ...
    /**
     * @notice Operator mints ATH to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ATH to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentAthenaPrice = _getAthenaPrice();
            currentTaxRate = _updateTaxRate(currentAthenaPrice);
            if (currentAthenaPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        _mint(_athenaPool, INITIAL_ATH_POOL_DISTRIBUTION);
