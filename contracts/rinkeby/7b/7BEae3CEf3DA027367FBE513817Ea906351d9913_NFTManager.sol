...
contract NFTManager is ReentrancyGuard, Ownable {
    ...
    event Create(uint256 tokenId, uint256 max_supply, string indexed name);
    ...
    function create(uint256 _max_supply, string calldata name) external nonReentrant returns (uint256) {
        if (onlyWhitelist) {
