 ...
 * @author Pedro Bergamini, reviewed by Basil Gorin
 */
contract XYZPool is XYZPoolBase {
    ...
    constructor(
        ...
        uint32 _weight,
        uint64 _endBlock
    ) XYZPoolBase(_ilv, _silv, _factory, _poolToken, _xyz, _initBlock, _weight) {
 ...
 * @author Pedro Bergamini, reviewed by Basil Gorin
 */
abstract contract XYZPoolBase is IPool, IlluviumAware, ReentrancyGuard {
    /// @dev Data structure representing token holder using a pool
    struct User {
        // @dev Total staked amount
        uint256 tokenAmount;
    ...
    /// @dev Link to the xyz token instance, for example ILV or ILV/ETH pair
    ...
    function _stake(
        ...
        // read the current balance
        uint256 previousBalance = IERC20(xyz).balanceOf(address(this));
        // transfer `_amount`; note: some tokens may get burnt here
        transferPoolTokenFrom(address(msg.sender), address(this), _amount);
        // read new balance, usually this is just the difference `previousBalance - _amount`
        uint256 newBalance = IERC20(xyz).balanceOf(address(this));
        // calculate real amount taking into account deflation
        uint256 addedAmount = newBalance - previousBalance;
    ...
    function transferPoolToken(address _to, uint256 _value) internal nonReentrant {
        // just delegate call to the target
        SafeERC20.safeTransfer(IERC20(xyz), _to, _value);
    }
    ...
    function transferPoolTokenFrom(
        ...
        // just delegate call to the target
        SafeERC20.safeTransferFrom(IERC20(xyz), _from, _to, _value);
    }
}
