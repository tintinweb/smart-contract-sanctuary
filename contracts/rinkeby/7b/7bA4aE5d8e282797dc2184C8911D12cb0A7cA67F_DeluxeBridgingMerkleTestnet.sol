...
contract DeluxeBridgingMerkleTestnet is ERC1155Holder, Ownable, ReentrancyGuard {
  /**
  ...
  bytes32 private merkleRoot; //TODO make this immutable after merkle tree constructed

  address public immutable osContract;
  address public bdContract = 0x0000000000000000000000000000000000000000;
  ...
  function claim(
    ...
    require(idsAndSenders[_oldId] == msg.sender, "not owner of oldId");
   ...
   */
  function transfer721(uint256 _tokenId, address _owner) external onlyOwner nonReentrant {
   ...
   */
  function mintOnClaiming(address _sender, uint256 _tokenId) internal returns (bool) {
    require(_sender != address(0), "can not mint to address 0");
    require(_tokenId != 0, "new token id !exists");

    if (BearsDeluxeI(bdContract).exists(_tokenId) && BearsDeluxeI(bdContract).ownerOf(_tokenId) == address(this)) {
      BearsDeluxeI(bdContract).safeTransferFrom(address(this), _sender, _tokenId);
      return true;
    }

    require(!BearsDeluxeI(bdContract).exists(_tokenId), "token minted");

    BearsDeluxeI(bdContract).mint(_sender, _tokenId);
  ...
  /**
   * @dev sets merkle root, this might be removed before main net
   */
  function setMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
    require(_merkleRoot != 0x0, "merkle root can not be 0");
   ...
   */
  function checkBalance(address _collector, uint256 _tokenId) public view returns (uint256) {
    require(_collector != address(0), "to is address 0");
    return IERC1155(osContract).balanceOf(_collector, _tokenId);
  }

  /**
   * @dev get the ids already transferred by a collector
   */
  function getTransferredByCollector(address _collector) public view returns (uint256[] memory) {
    require(_collector != address(0), "to is address 0");
    uint32 j = 0;
   ...
   */
  function getBridgedByCollector(address _collector) public view returns (uint256[] memory) {
    require(_collector != address(0), "to is address 0");
    uint32 j = 0;
   ...
   */
  function getBridgedTokens() public view returns (uint16[] memory) {
    return idsBridged;
  }
