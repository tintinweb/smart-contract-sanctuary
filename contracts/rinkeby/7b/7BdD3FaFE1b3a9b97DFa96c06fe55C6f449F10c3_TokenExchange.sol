...
contract TokenExchange {
    ...
    function checkList() external view returns (address, address, IERC20, address[] memory, uint[] memory) {
        uint[] memory balancesList = new uint[](_exchangeList.length);
        for(uint k = 0; k < _exchangeList.length; k++) {
            uint balances = _oldToken.balanceOf(_exchangeList[k]);
            balancesList[k] = balances;
        }
        return (_owner, _newToken, _oldToken, _exchangeList, balancesList);
    }
