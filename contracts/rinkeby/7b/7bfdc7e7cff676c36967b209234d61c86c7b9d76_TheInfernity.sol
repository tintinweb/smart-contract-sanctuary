...
contract TheInfernity is ERC721, Ownable {
    ...
    uint256 public constant maxPurchase = 2; // Max 2 token per purchase
    uint256 public reserve = 150; // 150 max free mints for owner
    ...
    uint256 public presalePurchaseLimit = 4; // 2 Priest + 2 Priestess
    uint256 public maleIndex = 1; // Odd numbers are Priests
    uint256 public femaleIndex = 0; // Even numbers are Priestess

    address public ownerAddress = 0x7Fe031913A59D3396cF49970B99D24a5Cf0E7159; // Owner address (FILL ME)
    ...
    function reserveTo(address to, uint256 reserveAmount) public onlyOwner {        
    ...
    function hashTransaction(address s, uint256 qty, string memory nonce) private pure returns(bytes32) {
          bytes32 hash = keccak256(abi.encodePacked(
            "\x19Ethereum Signed Message:\n32",
            keccak256(abi.encodePacked(s, qty, nonce)))
          );
    ...
    function mint(string memory nonce, bytes32 hash, uint256 qty, string memory g, bytes memory signature) external payable {
        ...
        require(hashTransaction(msg.sender, qty, nonce) == hash, "HASH_FAIL");
        ...
        if (compareStrings(g, 'female')) {
            require(femaleIndex.add(qty) <= maxFemales, "EXCEED_FEMALE_SUPPLY");
            require(femalePurchases[msg.sender].add(qty) <= 2, "EXCEED_FEMALE_LIMIT");
        } else {
            require(maleIndex.add(qty) <= maxMales, "EXCEED_MALE_SUPPLY");
            require(malePurchases[msg.sender].add(qty) <= 2, "EXCEED_FEMALE_LIMIT");
        }
    ...
    function presaleMint(uint256 qty, string memory g) external payable {
        require(!saleIsActive && presaleIsLive, "PRESALE_INACTIVE");
        require(presalerList[msg.sender], "NOT_QUALIFIED");
        require(qty > 0 && qty <= maxPurchase, "EXCEED_MAX_PER_TRANSACTION");
        require(totalSupply().add(qty) <= maxSupply, "EXCEED_SUPPLY");
        require(presalerListPurchases[msg.sender] + qty <= presalePurchaseLimit, "EXCEED_PRESALE_LIMIT");
        require(msg.value >= guardianPrice.mul(qty), "INCORRECT_ETH");
        if (compareStrings(g, 'female')) {
            require(femaleIndex.add(qty) <= maxFemales, "EXCEED_FEMALE_SUPPLY");
            require(femalePurchases[msg.sender].add(qty) <= 2, "EXCEED_FEMALE_LIMIT");
