...
// File: contracts\FairspinTokensale.sol





contract FairspinTokensale is OwnableUpgradeable, PausableUpgradeable {
     ...
     * EVENTS
     **/
    event TFSPurchased(address indexed user, address indexed purchaseToken, uint256 TFSAmount);
    event TokensClaimed(address indexed user, uint256 AmoTFSunt);
    ...
    uint256 public PUBLIC_SALE_END;

    uint256 public PRE_SALE_TFS_POOL; 
    uint256 public PUBLIC_SALE_TFS_POOL;
    ...
    modifier supportedCoin(address _token) {
        require(_token == USDTToken || _token == USDCToken, "Token not supported");
        _;
    }
    ...
    */
     ...
     * @param TFS TFSToken mainnet address
     */
    function initialize(address treasury, address TFS, uint maxAmount, 
        uint pre_start, uint pre_end, uint pub_start, uint pub_end, uint withdrawal_period, 
        uint pre_pool, uint public_pool, uint vest_duration) public initializer {
        require(treasury != address(0), "Zero address");
        require(TFS != address(0), "Zero address");
        ...
        PRE_SALE_START = pre_start;
        PRE_SALE_END = pre_end;

        PUBLIC_SALE_START = pub_start;
        PUBLIC_SALE_END = pub_end;

        PRE_SALE_TFS_POOL = pre_pool * (10 ** 18);
        PUBLIC_SALE_TFS_POOL = public_pool * (10 ** 18);
    ...
    * @param _maxTFS New max TFS amount
    */
    function adminSetMaxTFS(uint256 _maxTFS) external onlyOwner {
        maxTokensAmount = _maxTFS;
    }
    ...
    */
     ...
     * @param _publicPool New public pool TFS maximum value
     */
    function adminSetPublicPool(uint256 _publicPool) external onlyOwner {
        PUBLIC_SALE_TFS_POOL = _publicPool;
    }
    ...
    */
     ...
     */
    function purchaseTFSwithERC20(address ERC20token, uint256 ERC20amount) external onlySale supportedCoin(ERC20token) whenNotPaused correctGas {
        ...
        if (_isPreSale()) {
            require(purchasedPreSale.add(purchaseAmount) <= PRE_SALE_TFS_POOL, "Not enough TFS in presale pool");
            purchasedPreSale = purchasedPreSale.add(purchaseAmount);
        } else {
            require(purchaseAmount <= publicSaleAvailableTFS(), "Not enough TFS in sale pool");
        ...
        emit TFSPurchased(_msgSender(), ERC20token, purchaseAmount);
    }

    /**
     * @notice For purchase with ETH. ETH is left on the contract until withdrawn to treasury
     */
    function purchaseTFSwithETH() external payable onlySale whenNotPaused {
        require(msg.value > 0, "No ETH sent");
        _purchaseTFSwithETH();
    }

    function _purchaseTFSwithETH() correctGas private {
        ...
        emit TFSPurchased(_msgSender(), address(0), purchaseAmount);
    }

    /**
     * @notice Function to get available on public sale amount of TFS
     * @notice Unsold NUX pool and pre-sale pool go to public sale
     * @return The amount of the token released.
     */
    function publicSaleAvailableTFS() public view returns(uint256) {
        return PUBLIC_SALE_TFS_POOL.sub(purchasedPublicSale);
    }


    /**
     * @notice Function for the administrator to withdraw token (except TFS)
    ...
    function adminWithdrawERC20(address ERC20token) external onlyOwner notOnSale {
        require(ERC20token != TFSToken || _canWithdrawTFS(), "TFS withdrawal is forbidden");
    ...
    /**
     * @notice Returns TFS amount for 1 external token
    ...
    function claim() external {
        ...
        IERC20Upgradeable(TFSToken).safeTransfer(_msgSender(), unclaimed);
     ...
     * @param _user Address of the user who purchased tokens
     * @return Amount of TFS already vested
     */
    function _vestedAmount(address _user) private view returns (uint256) {
        if (block.timestamp >= vestingStart.add(vestingDuration)) {
            ...
            uint allowed = purchased[_user].div(5);
     ...
     * @return True is public sale is over
     */
    function _canWithdrawTFS() virtual internal view returns (bool) {
        return (block.timestamp >= vestingStart.add(WITHDRAWAL_PERIOD) );
    }

    /**
     * @dev Calculates TFS amount based on rate and token.
     * @param _token Supported ERC20 token
     * @param _amount Token amount to convert to TFS
     * @return TFS amount
     */
    ...
    /**
     * @dev Calculates TFS amount based on rate and ETH amount.
     * @param _amount ETH amount to convert to TFS
     * @return TFS amount
     */
     ...
     * @param purchaseAmount TFS tokens currently purchased
     */
...
// File: contracts\Test\FairspinTokensaleMock.sol

contract FairspinTokensaleMock is FairspinTokensale {
    ...
    function initialize(
        ...
        uint pub_end, uint withdrawal_period, uint pre_pool, uint public_pool, uint vest_duration) 
        public initializer
    {
        FairspinTokensale.initialize(_treasury, tfs, maxAmount, pre_start, pre_end, pub_start,
