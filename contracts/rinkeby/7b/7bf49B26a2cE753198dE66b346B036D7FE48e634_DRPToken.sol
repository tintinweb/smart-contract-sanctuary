...
contract DRPToken is ERC721Enumerable, Ownable {
  ...
  uint8 public constant MAX_NORMAL_PER_ACCOUNT = 10;
  uint8 public constant MAX_TETHER_PER_ACCOUNT = 10;
  ...
  string public baseURI_A1;
  string public baseURI_A2;
  string public baseURI_A3;
  string public baseURI_B;
  string public baseURI_C;

  constructor() ERC721('DRPToken', 'DRP') {
    DROP_1 = DRP_TOKEN(0x74A72C36CBf3e404BdfCcA82a995C5bA44bc06C4);
  }
  ...
  function setTokenURI(string calldata _uri_A1, string calldata _uri_A2, string calldata _uri_A3, string calldata _uri_B, string calldata _uri_C) external onlyOwner {
    baseURI_A1 = _uri_A1;
    baseURI_A2 = _uri_A2;
    baseURI_A3 = _uri_A3;
    baseURI_B = _uri_B;
  ...
  function claim() external payable {
    ...
    require(boundary >= 1, "Claim: All normal tokens are sold out.");
    ...
    uint16 index = uint16(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, block.coinbase, msg.sender, totalSupply(), address(this)))) % boundary); // 0 -> 87
  ...
  function isEligibleTether() public view returns (bool, uint256) {
    ...
    for (uint256 i = 0; i < balance; i++) {
      ...
      if (id <= 87 && !claimedTetherToken[id]) {
        hasNormalToken = true;
        tokenId = id;
        break;
      }
  ...
  function claimTether() external payable {
    ...
    require(tokenId <= 87, "Claim: Not allowed.");
  ...
  function isEligibleFree() public view returns (bool) {
    return DROP_1.balanceOf(msg.sender) > 10;
  }

  function claimFree() external {
    require(salesActive, "Claim is not active");
    require(tx.origin == msg.sender, "Claim cannot be made from a contract");
    require(isEligibleFree(), "Claim: Not allowed");
    require(freeTokenIdx < MAX_FREE_SUPPLY, "Claim: All free tokens are sold out.");

    _safeMint(msg.sender, freeTokenIdx);
    freeTokenIdx += 1;
  }
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory){
    ...
    if (_tokenId <= 43) {
      return string(abi.encodePacked(baseURI_A1, Strings.toString(_tokenId)));
    }
    if (_tokenId <= 65) {
      return string(abi.encodePacked(baseURI_A2, Strings.toString(_tokenId)));
    }
    if (_tokenId <= 87) {
      return string(abi.encodePacked(baseURI_A3, Strings.toString(_tokenId)));
    }
    if (_tokenId <= 175) {
      return string(abi.encodePacked(baseURI_B, Strings.toString(_tokenId)));
    }
    return string(abi.encodePacked(baseURI_C, Strings.toString(_tokenId)));
  }
...
interface DRP_TOKEN is IERC721 {}
