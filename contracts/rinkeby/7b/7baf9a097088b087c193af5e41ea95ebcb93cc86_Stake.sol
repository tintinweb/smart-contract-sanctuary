    ...
    AccessControlEnumerableUpgradeable
{
    ...
    function stake(
        ...
        if (count == 0) {
            EnumerableSetUpgradeable.AddressSet storage tmpSet = NFTOwner[_nftAdr];
    ...
    function recordStake(
        ...
        uint256 count = userNum[from][_nftAdr];
        userNum[from][_nftAdr] = count + 1;
        if (count == 0) {
            EnumerableSetUpgradeable.AddressSet storage tmpSet = NFTOwner[_nftAdr];
            tmpSet.add(from);
        }
    ...
    function withdraw(uint256 _stakeId) external whenNotPaused {
        ...
        if (count == 1) {
            EnumerableSetUpgradeable.AddressSet storage tmpSet = NFTOwner[userStake.nftAddress];
