...
contract UndeadDetectiveAgency is ERC721, Ownable {
    using SafeMath for uint256;
    using SafeMath for uint16;

    string public PROVENANCE = "";

    uint256 public nftPrice = 66600000000000000; // 0.0666 ETH
    uint256 public maxPurchase = 10;
    uint256 public maxWhitelistTotalPurchase = 3;
    uint256 public constant MAX_NFTS = 6666;
    ...
    event nftNameChange(address _by, uint256 _tokenId, string _name);
    event nftDescriptionChange(address _by, uint256 _tokenId, string _name);

    constructor() ERC721("Undead Detective Agency", "UNDEAD") {}
    ...
    function mintWhitelistNFT(uint256 numberOfTokens, bytes memory signature)
        public
        payable
    {
        bytes32 hash;
        ...
        // require(!saleIsActive, "Public sale has already started");
        // require(presaleIsActive, "Presale must be active to mint");
        // require(
        //     numberOfTokens <= whitelist[msg.sender],
        //     "You are not on the whitelist or can only purchase a maximum of 3 during presale."
        // );
        // require(
    ...
    function mintWhitelistNFT2(uint256 numberOfTokens, bytes memory signature)
        public
        payable
    {
        bytes32 hash;
    ...
    function changeNFTDescription(uint256 _tokenId, string memory _description)
        public
    {
        require(
            ownerOf(_tokenId) == msg.sender,
        ...
        require(
            sha256(bytes(_description)) !=
                sha256(bytes(nftDescriptions[_tokenId])),
            "New description is same as the current one"
        );
        nftDescriptions[_tokenId] = _description;

        emit nftDescriptionChange(msg.sender, _tokenId, _description);
    }

    function viewNFTName(uint256 _tokenId) public view returns (string memory) {
        require(_tokenId < totalSupply(), "Choose an NFT within range");
        return nftNames[_tokenId];
    }

    function viewNFTDescription(uint256 _tokenId)
        public
        view
        returns (string memory)
    {
    ...
    function addWhitelist(address[] memory _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            whitelist[_addresses[i]] = maxWhitelistTotalPurchase;
        }
    }

    function setMaxWhitelistTotalPurchase(uint256 _limit) public onlyOwner {
        maxWhitelistTotalPurchase = _limit;
    }

    function setMaxPurchase(uint256 _limit) public onlyOwner {
        maxPurchase = _limit;
    }
