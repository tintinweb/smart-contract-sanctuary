...
contract GenesisManaTest is ERC721Enumerable, ReentrancyGuard, Ownable {
  using strings for string;
  using strings for strings.slice;

    LootInterface private _lootContract = LootInterface(0x79E2d470f950f2Cf78eeF41720E8ff2cf4B3CD78);
    // 0x79E2d470f950f2Cf78eeF41720E8ff2cf4B3CD78); // Rinkeby Loot
    // 0xFF9C1b15B16263C61d017ee9F65C50e4AE0113D7; // Mainnext
    ...
    // Mapping is: _bagRefs[tokenId]
    mapping(uint256 => BagRef) private _bagRefs;
   ...
   mapping(uint8 => mapping(uint8 => uint8)) private _currentLostGenesisMana;
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
      require(
        !(_bagRefs[tokenId].lootTokenId == 0 && _bagRefs[tokenId].suffixId == 0 && _bagRefs[tokenId].inventoryId == 0 && _compareStrings(_bagRefs[tokenId].itemName,"")),
        "TOKEN_ID_NOT_MINTED"
      );
        ...
        parts[1] = _suffices[_bagRefs[tokenId].suffixId][1];
        parts[2] = '" /><text x="10" y="20" class="base">Order: ';
        parts[3] = _suffices[_bagRefs[tokenId].suffixId][0];
        parts[4] = '</text><text x="10" y="40" class="base">Equipment Type: ';
        parts[5] = _inventoryNames[_bagRefs[tokenId].inventoryId];
        parts[6] = '</text><text x="10" y="320" class="base italic">Distilled from ';
        if (_bagRefs[tokenId].lootTokenId == 0) {
          parts[7] = "";
        } else {
          parts[7] = string(abi.encodePacked('Loot Bag #',Strings.toString(_bagRefs[tokenId].lootTokenId)));
        }
        parts[8] = '</text><text x="10" y="340" class="base italic">';
        parts[9] = _bagRefs[tokenId].itemName;
        parts[10] = '</text></svg>';
      ...
      external
    {
        ...
        require(
            ...
            "GMANA_ALREADY_CLAIMED_FOR_TOKEN_ID_AND_INVENTORY_ID"
        );

        _claim(lootTokenId, inventoryId, 0, _msgSender());
    }

    function claimByOwner(uint8 inventoryId, uint8 suffixId)
      external
    {
        // Check that the inventory ID is in range
        require(
    ...
    function _claim(uint256 lootTokenId, uint8 inventoryId,  uint8 suffixId, address tokenOwner) internal {
        ...
        if (lootTokenId > 0) {
          ...
          if(inventoryId == 0) {
            itemName = string(abi.encodePacked("Lost Weapon of ", _suffices[suffixId][0]));
          } else if(inventoryId == 1) {
            itemName = string(abi.encodePacked("Lost Chest Armor of ", _suffices[suffixId][0]));
          } else if(inventoryId == 2) {
            itemName = string(abi.encodePacked("Lost Head Armor of ", _suffices[suffixId][0]));
          } else if(inventoryId == 3) {
            itemName = string(abi.encodePacked("Lost Waist Armor of ", _suffices[suffixId][0]));
          } else if(inventoryId == 4) {
            itemName = string(abi.encodePacked("Lost Foot Armor of ", _suffices[suffixId][0]));
          } else if(inventoryId == 5) {
            itemName = string(abi.encodePacked("Lost Hand Armor of ", _suffices[suffixId][0]));
          } else if(inventoryId == 6) {
            itemName = string(abi.encodePacked("Lost Neck Armor of ", _suffices[suffixId][0]));
          } else if(inventoryId == 7) {
            itemName = string(abi.encodePacked("Lost Ring of ", _suffices[suffixId][0]));
          }
        }

        claimedLootTokenId[lootTokenId][inventoryId] = true;
        uint256 newTokenId = _getNextTokenId();
        _bagRefs[newTokenId].lootTokenId = lootTokenId;
        _bagRefs[newTokenId].itemName = itemName;
        _bagRefs[newTokenId].suffixId = suffixId;
        _bagRefs[newTokenId].inventoryId = inventoryId;
    ...
    // Check there are genesis objects
    function _genesisItem (string memory item) private pure contains ("of", item) returns (bool) {
        return true;
    }
