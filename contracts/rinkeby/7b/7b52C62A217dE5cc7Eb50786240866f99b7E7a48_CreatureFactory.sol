...
contract CreatureFactory is FactoryERC721, Ownable {
    ...
    event testMint(uint256 _optionId, address _toAddress, uint id);
    uint public testMintId;
    function mint(uint256 _optionId, address _toAddress) override public {
        testMintId++;
        emit testMint(_optionId,_toAddress, testMintId);
    ...
    event AfterToken(address from,address to,  uint256 tokenId_);
    function _afterTokenTransfer(address from, address to,uint256 tokenId) internal  {
        emit AfterToken( from, to,  tokenId);
    }
...
contract CreatureLootBox is ERC721Tradable {
    ...
    constructor(address _proxyRegistryAddress, address _factoryAddress)
        ERC721Tradable("CEC_test_lut4", "EGS", _proxyRegistryAddress)
    {
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _afterTokenTransfer(
        ...
        emit testAfterTransfer(from, to, tokenId, msg.sender); 
        _burn(tokenId); 
        
        }
        uint a=1;
        function gavgav() public {
            a=2;
        }
