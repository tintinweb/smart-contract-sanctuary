...
contract EnchantedGame is IEnchantedGame, Ownable, ReentrancyGuard, Pausable {
    ...
    // reference to the protectedIsland for choosing random Dragon thieves
     ...
     */
    constructor(address _whitelistSigner, IERC721 _oldGoatContract) {
    ...
    modifier requireContractsSet() {
        require(
                address(egg) != address(0) &&
                address(traits) != address(0) &&
                address(goatNFT) != address(0) &&
                address(protectedIsland) != address(0) &&
                address(oldGoatContract) != address(0) &&
    ...
    function setContracts(
        ...
        address _traits,
        address _goat,
        address _protectedIsland,
        address _randomizer,
        address _oldGoatContract
    ) external onlyOwner {
        egg = IEGG(_egg);
        ...
        oldGoatContract = IERC721(_oldGoatContract);
    }
    ...
    */
    function _mint(uint256 amount, bool stake) internal {
        ...
        uint16 minted = goatNFT.minted();
        uint256 maxTokens = goatNFT.getMaxTokens();
        uint256 paidTokens = goatNFT.getPaidTokens();
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            if (!stake || recipient != _msgSender()) {
                goatNFT.mint(recipient, seed);
            } else {
                goatNFT.mint(address(protectedIsland), seed);
            }
            // check token traits for tortoise or goat and increase mint amount
            totalEggCost += mintCost(minted, maxTokens, paidTokens);
        }
       
        goatNFT.updateOriginAccess(tokenIds);
        if (totalEggCost > 0) {
            egg.burn(_msgSender(), totalEggCost);
            egg.updateOriginAccess();
        }
