...
contract ETHWalkersMinting is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint16 public maxEthWalkers = 10000;
    uint16 public constant MAXIMUM_RAISE_WALKERS_LIMIT = 12000; // If updating maximum, cannot update past 12,000
    uint private _eTHWalkersReserve = 100;
    address payable public payoutsAddress = payable(address(0x515E8EFAEb7544e150D9e6023Bc362009BAABebA)); //Update with address from ETHWalkersPayments.sol

    uint public presaleNuggetStart = 1638738000; // December 5th, 4PM EST
    uint public presaleApeStart = 1638824400;   // December 6th, 4PM EST
    uint public publicSale = 1638910800;   // December 7th 4PM EST

    //address boredApesAddress = 0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D; // LIVE contract ToDo: Uncomment before mainnet deploy
    address boredApesAddress = 0x5283419AC661651beB0d162BB87F384A1433C4a4; // ToDo: TEST ADDRESS ONLY - remove before prod
    //address nerdyNuggetsAddress = 0xb45F2ba6b25b8f326f0562905338b3Aa00D07640; // LIVE contract ToDo: Uncomment before mainnet deploy
    ...
    mapping(uint => string) public eTHWalkersNames;
    ...
    function reserveETHWalkers(address _to, uint256 _reserveAmount) public onlyOwner {
        ...
        require(_reserveAmount < maxEthWalkers, "No more tokens left to mint");
    ...
    function mintPreSaleRedeemSixPackNuggets(uint256[] memory ids) external nonReentrant {
        ...
        require(totalSupply().add(ids.length) <= maxEthWalkers, "Purchase exceeds max supply of ETHWalkers");
    ...
    function mintETHWalkers(uint numberOfTokens, uint id) external payable nonReentrant {
        require(saleState >= 1, "Pre-sale must be active to mint");
        ...
        if(block.timestamp >= publicSale) {
            ...
            require(totalSupply().add(numberOfTokens) <= maxEthWalkers, "Purchase exceeds max supply of ETHWalkers");
            if(boredApes.ownerOf(id) == msg.sender){
                require(boredApes.ownerOf(id) == msg.sender, "Must own a Bored Ape or Nerdy Nugget to mint ETH Walkers in 2nd presale");
            } else {
                require(nerdyNuggets.ownerOf(id) == msg.sender, "Must own a Bored Ape or Nerdy Nugget to mint ETH Walkers in 2nd presale");
            }
        }else if(block.timestamp >= presaleNuggetStart) {
            require(totalSupply().add(numberOfTokens) <= maxEthWalkers, "Purchase exceeds max supply of ETHWalkers");
            require(nerdyNuggets.ownerOf(id) == msg.sender, "Must own Nerdy Nugget to mint ETH Walkers in 1st presale");
        } else {
            require(false, "Must own NN or BAYC to mint in presale");
        }
    ...
    function eTHWalkersNamesOfOwner(address _owner, uint index) external view returns(string memory ) {
        return eTHWalkersNames[tokenOfOwnerByIndex(_owner, index)];
    }
