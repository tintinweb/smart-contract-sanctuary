...
contract Metarare is ERC721, ContextMixin, NativeMetaTransaction, Ownable, FactoryERC721 {
     ...
     */
    constructor(string memory newName, string memory newSymbol, string memory baseURI, address proxyRegistryAddress, uint256 newOptions, uint256 newMaxOptions) ERC721(newName, newSymbol) {

        setBaseURI(baseURI);

        _proxyRegistryAddress = proxyRegistryAddress;

        _numOptions = newOptions;
    ...
    function setNumOptions(uint newNumOptions) public onlyOwner {
        ...
        require(newNumOptions <= _maxOptions, "newNumOptions must be less or equal than maxOptions");
    ...
    function setMaxOptions(uint newMaxOptions) public onlyOwner {
        require(newMaxOptions >= _numOptions, "newMaxOptions must be greater or equal numOptions");

        _maxOptions = newMaxOptions;
    }

    function maxOptions() external view  returns (uint256) {
        return _maxOptions;
    }
    ...
    function mint(uint256 tokenId, address toAddress) override public {
        ...
        require(exists(tokenId) == false,"MetaRare Sale: Token has already been minted");

        uint nextTokenIdOnSale = _numOptions; 
        bool newOptionAllowed = _numOptions < _maxOptions;
        ...
        if (newOptionAllowed) {
            ...
            emit Transfer(address(0), owner(), nextTokenIdOnSale);
        }    
