...
contract GeneRoom is ERC721Enumerable, Ownable {
    ...
    constructor(address _treasureAddress, string memory base1, string memory base2) ERC721("Gene-Room", "GR") {
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(quantity <= MAX_PER_MINT, "exceed max mint quantity");
        require(_nextTokenId.add(quantity - 1) <= MAX_ELEMENTS, "sold out");
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory uri) {
        if (tokenId >= 1 && tokenId <= 5000) {
            return string(abi.encodePacked(baseURI1, tokenId.toString()));
        } else if (tokenId > 5000 && tokenId <= MAX_ELEMENTS) {
            return string(abi.encodePacked(baseURI2, tokenId.toString()));
        }
    }

    function setBaseURI(string memory base1, string memory base2) external onlyOwner {
        baseURI1 = base1;
        baseURI2 = base2;
    }
