...
contract KittyPartyFactory {
    ...
    function createKitty(address[] memory memberAddress, uint256 _amountPerRound, uint8 _roundDuration)
    external
    returns (address kittyAddress) {
        address kitty = Clones.clone(tomCatContract);
        KittyParty(kitty).initialize( memberAddress, _amountPerRound, _roundDuration);
