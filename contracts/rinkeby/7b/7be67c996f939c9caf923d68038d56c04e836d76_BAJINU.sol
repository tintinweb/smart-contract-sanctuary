...
contract BAJINU is Context, IERC20, Ownable {
    ...
    address public _charity = 0x73bAAFa24c486b432af47fd55F308aB46128Bee0;
    ...
    uint256 public _charityFee = 6; // 6% Charity fee
    ...
    event Charity(address _charity, uint256 charity);
    ...
    function _sendCharity(uint256 tAmount) private {
        uint256 tCharity = calculateCharityFee(tAmount);
        if(!_isExcludedFromFee[tx.origin]){
            _tOwned[_charity] = _tOwned[_charity].add(tCharity);
            _rOwned[_charity] = _rOwned[_charity].add(tCharity);
        }
        emit Charity(_charity,tCharity);
    }
