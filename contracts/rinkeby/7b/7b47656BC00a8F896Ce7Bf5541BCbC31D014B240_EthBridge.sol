...
// File: contracts\bridge\test\ITokenMock.sol
...
// File: contracts\bridge\BaseBridge.sol


abstract contract BaseBridge is Ownable {
    /** 
       @dev Interface for DAO token
        Since tokens are going to be minted and burnde on BSC,
     ...
     */
    mapping (address => uint) public transactionId;
     ...
     */
    function GetTransactionId() external view returns(uint) {
        return transactionId[_msgSender()];
    }

    // function setOtherBridge(address _other) external onlyOwner {
    //     otherChainBridgeAddress = _other;
    // }
}

// File: contracts\bridge\eth-bridge\EthBridge.sol
...
contract EthBridge is BaseBridge {
    ...
    /**
        ...
        @notice If function call is successfull, an event is emmited and bridge API
        sends tokens on BSC
        ...
        @param signature A message, signed by sender, required in order to make transaction
        secure
     */
    function sendTokensToBSC(address _to, uint _amount, bytes memory signature)
     external
    {
    ...
    /**
        ...
        @notice Once an event is emmited on BSC, pointing that transaction is requested,
        some amount of tokens are burned on BSC and unlocked on Ethereum
        @param _from An address on BSC which sent a transcation. Required in order to check
        ...
        secure
     */
    function unlockTokens(address _from, address _to, uint _amount, 
                        uint BSCTransactionId, bytes memory _signature)
     external onlyOwner 
    {
        require(!processedTransactions[_from][BSCTransactionId],
        ...
        require(MyECDSA.recoverAddress(message, _signature) == _from, "Signature mismatch");
