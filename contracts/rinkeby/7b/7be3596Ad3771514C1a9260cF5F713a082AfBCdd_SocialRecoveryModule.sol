...
contract SocialRecoveryModule is Module {
    ...
    /// @dev A Friend can call this method after the required deposit to start the recover delay
    /// @param prevOwner Owner that pointed to the owner to be replaced in the linked list
    /// @param oldOwner Owner address to be replaced.
    /// @param newOwner New owner address.
    function confirmDeposit(address prevOwner, address oldOwner, address newOwner)
        public
        onlyFriend
    {
        bytes32 swapHash = getTransactionHash(prevOwner, oldOwner, newOwner);
        require(readyToRecover[swapHash] == 0, "Recover already in progress");
        require(ERC20AlreadyDepositedAmount > 0, "recoverAccess function should be called first");
        IERC20 toDepositToken = IERC20(ERC20DepositAddress);
