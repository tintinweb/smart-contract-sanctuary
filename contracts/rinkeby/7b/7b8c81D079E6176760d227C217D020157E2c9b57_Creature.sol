...
contract Creature is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    function mintAllNFT() internal {
        ERC721._balances[address(this)] = MAX_NFT_SUPPLY;
        
    }

    function ownerOf(uint256 tokenId) public view override returns (address) {
        address owner = ERC721._owners[tokenId];
        if (!_distributed[tokenId] && tokenId > 0 && tokenId <= MAX_NFT_SUPPLY) {
            owner = address(this);
        }
    ...
    function _exists(uint256 tokenId) internal view override returns (bool) {
        if (!_distributed[tokenId]) {
            return tokenId <= MAX_NFT_SUPPLY && tokenId > 0;
        } else {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view override returns (uint256) {
        ...
        if (owner == address(this)) {
            uint256 preMintedTokenId = index + 1;
            if (!_distributed[preMintedTokenId]) {
                return preMintedTokenId;
            }
        }
        return ERC721Enumerable._ownedTokens[owner][index];
    }
