...
contract LuckyCatz     is ERC721 
{
    ...
    uint256 private     salesPrice         = 0.05 ether;

    uint256 private     presalesMaxCount   = 400;
    uint256 private     presalesSoldCount  = 0;

    uint256 private     totalTokens        = 30;//7777;
    ...
    constructor() ERC721("CTZ09","CETEZED Token")
    {
	...
	function	changeOwnerWallet(address newOwnerWallet) external onlyOwner
	{
    ...
    function    batchMint(uint256 qty) external payable callerIsUser //claimStarted
    {
        if (block.timestamp>=salesDate)
        {
        ...
        require((walletMintCounts[msg.sender]+qty)<=maxMintPerWallet, "You cannot claim more tokens");
        require(availableTokens.length>=qty,                                  "No tokens left to be claimed");

        uint256[] memory tokenIds = new uint256[](qty);

        walletMintCounts[msg.sender] += qty;
    ...
    function    getMintCount(address walletAddress) external view returns(uint256)
    {
        ...
        {
            require(walletAddress==msg.sender, "Only the owner of the wallet can view the quantity");
        }
