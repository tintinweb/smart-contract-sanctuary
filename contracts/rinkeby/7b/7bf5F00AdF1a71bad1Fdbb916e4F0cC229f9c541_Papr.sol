...
contract Papr is ERC20Burnable, DoubleOperator {
    ...
    mapping(uint256 => uint256) public feesTaxRate;

    // Oracle
    address public paprOracle;
    address public papr;

    // List of address which exclude fees
    ...
    constructor() public ERC20('PAPR', 'PAPR') {
        ...
        // Dont't forget to change Index 0 to 45 after injecting liquidity
    ...
    function isExcludedAddresses(address adress) public view returns(bool isIndeed) {
        return excludedAddresses[adress];
    }

    // Add an address to the exluded mapping
    function addExcludedAddresses(address newAddress) external onlyDoubleOperator returns(bool success) {
    ...
    function removeExcludedAddresses(address oldAddress) external onlyDoubleOperator returns(bool success) {
    ...
    function setfeesTaxRate(uint256 index, uint256 fees) external onlyDoubleOperator {
        require(fees >= 0 && fees <= 45);
        if (index >= 6) {
            require(fees >= 0 && fees <= 10);
        }
