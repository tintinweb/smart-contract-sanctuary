...
contract WIFnft is ERC721JsonMetadata, Ownable, ReentrancyGuard {
    ...
    mapping(TokenType=>SaleInfo) internal saleProperties;
    uint256 saleStart;
    ...
    function sale(TokenType ttype, uint256 nftAmount) external payable nonReentrant {
        require(saleStart > 0 && saleStart <= block.timestamp, "Sale not started");
    ...
    function setupSale(uint256[] calldata prices, uint256[] calldata caps, uint256 _saleStart) external onlyOwner {
        require((prices.length == caps.length) && (caps.length == TOKEN_TYPE_COUNT), "Wrong array lengths");
        for(uint256 i=0; i < TOKEN_TYPE_COUNT; i++){
            SaleInfo storage si = saleProperties[TokenType(i)];
    ...
    function setupSale(TokenType ttype, uint256 price, uint256 cap) external onlyOwner {
    ...
    function saleState() external view returns(uint256[] memory price, uint256[] memory minted, uint256[] memory available) {
        price = new uint256[](TOKEN_TYPE_COUNT);
        minted = new uint256[](TOKEN_TYPE_COUNT);
        available = new uint256[](TOKEN_TYPE_COUNT);
        for(uint256 i=0; i < TOKEN_TYPE_COUNT; i++){
            SaleInfo storage saleInfo = saleProperties[TokenType(i)];
    ...
    function withdrawERC1155(IERC1155 token, uint256 id, uint256 amount, bytes calldata data) external onlyOwner {
