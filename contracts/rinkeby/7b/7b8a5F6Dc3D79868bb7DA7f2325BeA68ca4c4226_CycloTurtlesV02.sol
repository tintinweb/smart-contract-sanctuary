...
contract CycloTurtlesV02 is ERC721URIStorage, Ownable { 
    ...
    VITSaleConfig public vitSaleConfig;
    PresaleConfig public presaleConfig;
    SaleConfig public saleConfig;

    uint256 public maxTotalSupply = 7777;
    uint public maxGiftSupply = 155;
    uint256 public maxVITsuppy = 1111;
    uint256 public maxPresaleSupply = 2222;

    uint256 public vitPrice = 80000000000000000; // 0.08 ETH
    uint256 public presalePrice = 100000000000000000; // 0.1 ETH
    uint256 public salePrice = 150000000000000000; // 0.15 ETH

    uint256 public giftCount;
    uint256 public vitCount;
    ...
    event VITSaleActive(bool _isActive);
    event PresaleActive(bool _isActive);
    event PublicSaleActive(bool _isActive);
    event UpdatedWorkflowStatus(WorkflowStatus previousStatus, WorkflowStatus newStatus);
    event UpdatedIsBurnEnabled(bool _isBurnEnabled);
    event VITMint(address _minter, uint256 _amount);
    event PresaleMint(address _minter, uint256 _amount);
    event SaleMint(address _minter, uint256 _ammount);

    constructor() ERC721("TestNFT", "TEST") { // "CycloTurtles", "TURTLES"
        // Maybe remove this
        vitSaleConfig = VITSaleConfig(false, 2);
        presaleConfig = PresaleConfig(false, 5);
        saleConfig = SaleConfig(false, 10);
    }
        ...
        onlyOwner
    {
        for(uint i = 0; i < _addresses.length; i++) {
            require(
                ...
                "CycloTurtles: Can't add a 0 address"
            );
    ...
    function isOnVITList (address _address) external view returns (bool) {
    ...
    function addToPresale(address[] calldata _addresses)
        external
        onlyOwner
    {
        for (uint i = 0; i < _addresses.length; i++) {
        ...
        onlyOwner
    {
        for (uint i = 0; i < _addresses.length; i++) {
            require (
                _addresses[i] != address(0),
                "CycloTurtles: can't remove a zero address"
            );
    ...
    function pauseSales() external onlyOwner {
        WorkflowStatus _oldWorkflow = workflow;
        workflow = WorkflowStatus.Paused;
        emit UpdatedWorkflowStatus(_oldWorkflow, workflow);
        
        vitSaleConfig.isActive = false;
        emit VITSaleActive(false);
        
        presaleConfig.isActive = false;
        emit PresaleActive(false);

        saleConfig.isActive = false;
        emit PublicSaleActive(false);

        console.log("Sales Paused");
    }

    function startVITSale() external onlyOwner {
        require(
            workflow == WorkflowStatus.Paused,
            "CycloTurtles: Unautgorized Transaction - attempting to setup VIT Sale workflow when not in initial status check workflow"
        );
        vitSaleConfig.isActive = true;
        emit VITSaleActive(true);

        workflow = WorkflowStatus.VITSale;
        emit UpdatedWorkflowStatus(
            WorkflowStatus.Paused,
            WorkflowStatus.VITSale
        );
    }
    ...
    function startPresale() external onlyOwner {
        require(
            workflow == WorkflowStatus.VITSale,
            "CycloTurtles: Unauthorized Transaction - attempting to setup presale workflow when not in VIT sale workflow"
        );
        vitSaleConfig.isActive = false;
        emit VITSaleActive(false);
        presaleConfig.isActive = true;
        emit PresaleActive(true);
    ...
    function startPublicSale() external onlyOwner {
        require(
            workflow == WorkflowStatus.Presale,
            "CycloTurtles: Unauthorized Transaction - attempting to setup public sale workflow when not in presale workflow"
        );
        presaleConfig.isActive = false;
        emit PresaleActive(false);
        saleConfig.isActive = true;
        emit PublicSaleActive(true);
    ...
    {
        require(
            totalSupply + _addresses.length <= maxTotalSupply,
        ...
        for (uint i = 0; i < _addresses.length; i++) {
            ...
            _giftClaimed[_addresses[i]] = _giftClaimed[_addresses[i]] + 1;
            _totalClaimed[_addresses[i]] = _totalClaimed[_addresses[i]] + 1;
    ...
    function vitMint(uint256 _amount) external payable {
        ...
        require(
            _vitClaimed[msg.sender] + _amount < vitSaleConfig.maxCount,
        ...
        for(uint i = 0; i < _amount; i++) {
            ...
            _setTokenURI(_newTokenId, baseURI);
            _vitClaimed[msg.sender] = _vitClaimed[msg.sender] + 1;
            _totalClaimed[msg.sender] = _totalClaimed[msg.sender] + 1;
            totalSupply = totalSupply + 1;
            vitCount = vitCount + 1;
        }
        emit VITMint(msg.sender, _amount);
    }
    ...
    function presaleMint (uint256 _amount) external payable {
        ...
        require(
            _presaleClaimed[msg.sender] + _amount < presaleConfig.maxCount,
            "CycloTurtles: Can only mint 2 tokens"
        );
        require(
            presaleCount + _amount < maxPresaleSupply,
    ...
    function saleMint(uint256 _amount) external payable {
        ...
        require(
            _saleClaimed[msg.sender] + _amount < saleConfig.maxCount,
            "CycloTurtles: Can only mint 10 tokens"
        );
        require(
            totalSupply + _amount < maxTotalSupply,
            "CycloTurtles: Cannot exceed max total supply"
