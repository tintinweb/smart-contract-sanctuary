 ...
 */
 ...
 //
contract Dorkis is ERC721, Ownable, ERC721Enumerable {
    using SafeMath for uint256;

    string public Dorks_PROVENANCE = "";
    ...
    bool public paybackIsActive = false;
    ...
    function withdraw() public onlyOwner {
        require(address(this).balance != 0, "PaymentSplitter: balance is 0");
        if (_payees.length !=0){
            // contract wallet gets 5%
            uint256 payout = address(this).balance;
            payable(owner()).transfer(payout/20);
            emit PaymentReleased(owner(), payout/20);
            // DAO wallet gets 35%
            Address.sendValue(payable(daoAddress), (payout/100)*35);
            emit PaymentReleased(daoAddress, (payout/100)*35);
            // team splits the rest
            payout = address(this).balance / (_payees.length+1);
            for (uint i = 0; i < _payees.length; i++) {
                Address.sendValue(payable(_payees[i]), payout);
                emit PaymentReleased(_payees[i], payout);
            }
        }else{
     ...
     * 
     */
    function addPayee(address account) public onlyOwner {
        require(account != address(0), "PaymentSplitter: account is the zero address");
        require(_indexOf(account)<0 , "PaymentSplitter: account already added.");
        _payees.push(account);
        emit PayeeAdded(account);
    }
    
    /**
     * @dev set the DAO adress to the contract
     * @param account The address of the DAO to set.
     * 
     */
    ...
    function removePayee(address account) public onlyOwner {
        require(account != address(0), "PaymentSplitter: account is the zero address");
        require(_indexOf(account)>=0 , "PaymentSplitter: account not found.");
        int index=_indexOf(account);
        if (index>=0){
            _removeFromArray(uint(index));
            emit PayeeRemoved(account); 
        }
    }
    
    function _indexOf(address account) internal view returns (int) {
        for (uint i = 0; i < _payees.length; i++) {
    ...
    function _removeFromArray(uint index) internal {
    ...
    function reserveDorkis() public onlyOwner {    
        require(totalSupply().add(maxPurchase) <= MAX_DORKS, "Reserve would exceed max supply of Dorkis");
    ...
    */
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        Dorks_PROVENANCE = provenanceHash;
    }
   ...
   */
    ...
    */
    ...
    function transferFrom(
        ...
        if (paybackIsActive && to == address(0x000000000000000000000000000000000000dEaD)){
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (ownerOf(tokenId) == address(0x000000000000000000000000000000000000dEaD)){
            return "";
        }else{
    ...
    function multiBurn(uint256[] memory tokenId) public {
         for (uint i = 0; i < tokenId.length; i++) {
             if (_isApprovedOrOwner(_msgSender(), tokenId[i])){
                transferFrom(_msgSender(),address(0x000000000000000000000000000000000000dEaD),tokenId[i]);
             }
         }
    }
