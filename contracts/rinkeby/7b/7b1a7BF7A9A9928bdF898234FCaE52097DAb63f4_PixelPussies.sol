...
contract PixelPussies is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	address private owner1 = 0x192F4B11c60DB39BF08AdDCb5cFF52711e11BDc2;
	...
	function mintPresale(uint8 TOKENS_TO_MINT) public payable isRealUser nonReentrant {
		...
		require(TOKENS_TO_MINT <= 3, "Sorry, you are trying to mint too many tokens at one time");
		...
		require((_stock[msg.sender] + TOKENS_TO_MINT) <= 3, "You are trying to mint too many tokens.");
	...
	function mintPublic(uint8 TOKENS_TO_MINT) public payable isRealUser nonReentrant {
		...
		require((totalSupply() + TOKENS_TO_MINT) <= MAX_VOLUME_MINTABLE , "Exceeding max supply");
		require(TOKENS_TO_MINT <= 5, "Sorry, you are trying to mint too many tokens at one time");
		require(TOKENS_TO_MINT > 0, "You need to mint at least one token");
		require(PRICE*TOKENS_TO_MINT <= msg.value, "Sorry, you did not sent the required amount of ETH");
		require((_stock[msg.sender] + TOKENS_TO_MINT) <= 8, "You are trying to mint too many tokens.");
