...
contract ShroomHeads is EIP712, AccessControl, ReentrancyGuard, ERC1155Supply
{
    ...
    address public WETH; // made varible for more convinient testing
    ...
    constructor(address _owner, string memory uri_, address[] memory _team, uint256[] memory _shares, address wETH, address _signerAddress) ERC1155(uri_) EIP712("ShroomHeads", "1") {
        ...
        // signer address. Not using setSignerAddress as msgSender doenst have TEAM role
    ...
    */
    function privateMint(uint256 tokenId, bytes memory _signature) external nonReentrant payable {
        ...
        require(signerAddress == keccak256(abi.encode("Shroomheads private mint_", tokenId, "_", _msgSender())).toEthSignedMessageHash().recover(_signature), "Not authorized. You need to be whitelisted on Discord");
    ...
    */
    function auctionMint(uint256 tokenId, address winningBidder, uint256 winningBid, uint256 ts, bytes memory _signature) external onlyRole(TEAM) {
        // check signature
    ...
    */
     ...
     * @dev Read NFTs info
     */
    function getNftsInfo(uint256[] calldata tokenIds) external view returns (
            bytes1[] memory,
            uint256[] memory,
            uint8[] memory,
            uint8[] memory,
            uint256[] memory) 
        {
        bytes1[] memory traits = new bytes1[](tokenIds.length);
        uint256[] memory price = new uint256[](tokenIds.length);
        uint8[] memory editions = new uint8[](tokenIds.length);
        uint8[] memory saleType = new uint8[](tokenIds.length);
        uint256[] memory tSupply = new uint256[](tokenIds.length);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] < Nfts.length, "Such NFT doesn't exist");
            traits[i] = Nfts[tokenIds[i]].traits;
            price[i] = Nfts[tokenIds[i]].price;
            editions[i] = Nfts[tokenIds[i]].editions;
            saleType[i] = Nfts[tokenIds[i]].saleType;
            tSupply[i] =  totalSupply(tokenIds[i]);
        }
        return (traits, price, editions, saleType, tSupply);
    }
