...
contract OlympicsStaking {
    ...
    address [] public stakerList0 ;
    address [] public stakerList1 ;
    ...
    function claim() public whenLocked canClaim {
        if (winner == 0) {
            uint userCanWithdraw = reward.mul(ledger0[msg.sender]).div(TVL0) ;
            ledger0[msg.sender] = 0 ;
            msg.sender.transfer(userCanWithdraw) ;
        } 
        else if (winner == 1) {
            uint userCanWithdraw = reward.mul(ledger1[msg.sender]).div(TVL1) ;
            ledger1[msg.sender] = 0 ;
            msg.sender.transfer(userCanWithdraw) ;
        } 
        else {
            uint userCanWithdraw = (ledger0[msg.sender].mul(39)).div(40) + (ledger1[msg.sender].mul(39)).div(40) ;
            ledger0[msg.sender] = 0 ;
            ledger1[msg.sender] = 0 ;
            msg.sender.transfer(userCanWithdraw) ;
        }
