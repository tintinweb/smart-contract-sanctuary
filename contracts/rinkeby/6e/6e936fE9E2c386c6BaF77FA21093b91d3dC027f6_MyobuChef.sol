...
contract MyobuChef is Ownable, ReentrancyGuard, ERC721Holder {
	...
	// This has to be set to true through the deployRewardsToken function
...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
    ...
    function deposit(uint256 _pid, uint112 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (_amount > 0) {
			...
			updateUserWeightedBalance(_pid, msg.sender); // @dev exploitable but fuck it
        }
    ...
    function reinvestETHRewards(address router) public nonReentrant {
