 ...
 */
contract BridgeLeveragedPositionController is AbstractController {
    ...
    function stake(uint256 _amount) external override onlyIndex {
        ...
        _checkTetherApprovals(address(stblToken), address(_uniswapRouter), _amount);
    ...
    function withdrawRewards() external override {
        ...
        //TODO withdraw rewards and shield

        //TODO send BMI (and shield rewards?) to feeInfo.feeRecipient

        //TODO swap rewards into base

        //deposit base into index
        index.depositInternal(
            0 /*_baseAmount*/
        );
    }
    ...
    function productsInfo() external view override returns (ProductInfo[] memory _infos) {
        ...
        for (uint256 i = 0; i < allProducts.length; i++) {
            ...
            _infos[i].unstakingPending = _pooledStaking.stakerContractPendingUnstakeTotal(
                address(this),
                allProducts[i]
            );
        }*/
...
abstract contract AbstractController is OwnableUpgradeable, IPositionController {
    ...
    function _checkTetherApprovals(address _token, address _spender, uint256 _amount) internal {
