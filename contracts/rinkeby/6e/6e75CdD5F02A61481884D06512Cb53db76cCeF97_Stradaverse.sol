...
/// @title Stradaverse
/// @author Will Holley
/// @notice "Hello World" test contract
contract Stradaverse is ERC721, Ownable, ReentrancyGuard {
  ...
  constructor(address proxyRegistryAddress_, string memory viewerBaseUrl_)
    ERC721("Stradaverse", "STRADA")
  {
  ...
  function mint(
    ...
    // Convert id to int
    uint256 idInt = uint256(id);

    // Save traits struct
    _traitMaps[idInt] = Traits({
      headgearId: headgearId_,
      earlobeId: earlobeId_,
  ...
  function tokenURI(uint256 id_) public view override returns (string memory) {
    ...
    string memory pageUrl = string(
      abi.encodePacked(viewerBaseUrl, '/', id_.toString())
    );

    Traits memory traits = _traitMaps[id_];

    // TODO: Use a map to names for attributes
    ...
    string memory traitsA = string(
      abi.encodePacked(
        "[",
        _makeAttribute("Headgear", traits.headgearId.toString()),
        ",",
        _makeAttribute("Earlobes", traits.earlobeId.toString()),
        ",",
        _makeAttribute("Face Mask", traits.faceMaskId.toString()),
        ",",
        _makeAttribute("Eyes", traits.eyeId.toString()),
        ",",
        _makeAttribute("Corneas", traits.corneaId.toString()),
        ",",
        _makeAttribute("Pupils", traits.pupilId.toString()),
        ",",
        _makeAttribute("Footwear", traits.footwearId.toString()),
        ",",
        _makeAttribute("Clothing", traits.clothingId.toString()),
        ","
      )
    );
    ...
    string memory traitsString = string(
      abi.encodePacked(
        traitsA,
        _makeAttribute("Background", traits.backgroundId.toString()),
        ",",
        _makeAttribute("Skin Color", traits.skinColorId.toString()),
        "]"
      )
    );
