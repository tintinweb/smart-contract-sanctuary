    ...
    WithLimitedSupply
{
    ...
    function mint(address _to, uint256 _tokenId, string memory _cid)
        public
        ensureAvailability
        onlyAuctionContractOrOwner
    {
        nextToken();
    ...
    /// @dev Restrict auction to owner / auction contract
    modifier onlyAuctionContractOrOwner {
        require(
            (_msgSender() == auctionContract()) ||  // solhint-disable-line avoid-tx-origin
            (_msgSender() == owner()),
            "Not allowed to mint"
        );
