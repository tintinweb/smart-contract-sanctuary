...
contract MarketPlace is OwnableUpgradeable, Payments {
    ...
    function buy(address _originContract, uint256 _tokenId) public payable {
        ...
        uint256 paymentPayee = (sp.amount).sub(paymentMarketPlace).sub(100);       //sub 100 to ensure that available balance is always > sending balance
    ...
    function acceptBid(address _originContract, uint256 _tokenId) public {
        ...
        uint256 paymentPayee = (currentBid.amount).sub(paymentMarketPlace).sub(100);       //sub 100 to ensure that available balance is always > sending balance
