...
contract KoiFishCollective is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
    // Provenance hash for all Koi
    string public constant KOI_PROVENANCE = "43fb437b18ec1c27eed81a62dcd0915781ac907f1513f41e8a0842ab21c1c6c";
    
    uint256 public startingIndexBlock;
    uint256 public startingIndex;
    
    uint256 public constant koiPrice = 80000000000000000; // .08 Eth
    
    uint256 public constant maxKoiPurchase = 20;
    ...
    function mintKoi(uint256 numTokens) external whenNotPaused payable {
        require(numTokens <= maxKoiPurchase, "Mint exceeds max purchase limit");
        require(totalSupply() + numTokens <= MAX_KOI, "Mint exceeds max tokens");
        require(koiPrice * numTokens <= msg.value,"Insufficient funds for mint");
        ...
        if (
            ...
            (totalSupply() == MAX_KOI || block.timestamp >= revealTimestamp)
        ) {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_KOI;
    
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number - startingIndexBlock > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % MAX_KOI;
        }
