    ...
    ERC721Burnable
{
    ...
    address pointsContractAddr = address(0x0);

    uint256 public pointsPerToken = 1;
    ...
    function setPointsContractAddress(address addr) public onlyOwner {
        pointsContractAddr = addr;
    }

    function setPointsPerToken(uint256 points) public onlyOwner {
        pointsPerToken = points;
    }
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override onceOnly(tokenId) {
        require(
    ...
    // Call Points contract when redeeming
    function redeemToken(address from, uint256 tokenId)
        public
        notRedeemed(tokenId)
    {
        require(
            pointsContractAddr != address(0x0),
            "Points contract address not set."
        );
        //setApprovalForAll(address(this), true);
        // burn the token
        //super.transferFrom(from, address(0xdead), tokenId);
        _burn(tokenId);
        // Send a point to the redeemer
        ITDCPoints(pointsContractAddr).mint(from, pointsPerToken);
