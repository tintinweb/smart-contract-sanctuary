...
contract GenesisAdventurer is ERC721EnumerableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    // 7 - ring
    // 8 - order id
    mapping(uint256 => uint256[9]) private _detailsByToken;
    ...
    // constructor(address _gmAddress, address _lootAddress)
    // ERC721("Genesis Adventurer", "GA")
    // Ownable() {
    //   _genesisMana = GMInterface(_gmAddress);
    //   // 0x03aFaC9EED5C594739381878e6E74556d3604AA5// Rinkeby
    //   // 0xf4B6040A4b1B30f1d1691699a8F3BF957b03e463 // Mainnet
    //
    ...
    function initialize(address _gmAddress, address _lootAddress) initializer public {
    ...
    // 402899 -> 394269 -> 3822489 -> 349253 -> 349517
    function claim(
      ...
      nonReentrant
    {
      uint256[2][9] memory items = [
        [weaponTokenId, uint256(0)],
        [chestTokenId, uint256(0)],
        [headTokenId,uint256(0)],
        [waistTokenId,uint256(0)],
        [footTokenId,uint256(0)],
        [handTokenId,uint256(0)],
        [neckTokenId,uint256(0)],
        [ringTokenId,uint256(0)],
        [uint256(0),uint256(0)]    //suffixId
      ];
      ...
      for (uint8 i = 0; i < 8; i++) {
        ...
        require(
            !itemUsedByGMID[items[i][0]],
            "ITEM_USED"
        );

        require(
            _msgSender() == _genesisMana.ownerOf(items[i][0]),
            "MUST_OWN"
        );
        details = _genesisMana.detailsByToken(items[i][0]);
        ...
        //to save memory override GM TokenId with Loot TokenId
        items[i][1] = details.lootTokenId;
      }

      _detailsByToken[_getNextTokenId()] = [items[0][1],items[1][1],items[2][1],items[3][1],items[4][1],items[5][1],items[6][1],items[7][1],items[8][0]];
      _safeMint(_msgSender(), _getNextTokenId());
      _incrementTokenId();

      for (uint8 i = 0; i < 8; i++) {
        itemUsedByGMID[items[i][0]] = true;
      }
    }
