...
interface AnftToken {
  ...
  function checkrepay(uint256 _tokenId) external view returns (uint amount,address repaytoken,uint expiry);
}
...
contract TBRegister is  Ownable {
    ...
    address public TBnft =  address(0x9666B387471490eE2Fe9f2944c1431e5d27652C7);
    ...
    address public BuyOrderToken = 0x0000000000000000000000000000000000000000;
    uint256 public BuyOrderAmount = 10000000000000000; // 0.01 ETH
    ...
    event ReclaimToken(uint256 tokenid, address fromaddr, uint256 amount, address tokenaddr);
    ...
    constructor( address bondsNFT, address _address ) public {
    ...
    function setBuyOrder( uint256 _amount, address _buytoken ) external onlyOwner {
        BuyOrderToken = _buytoken;
        BuyOrderAmount = _amount;
    }
    ...
    function registerNFT() public payable {
        ...
        if( BuyOrderToken == address(0x0000000000000000000000000000000000000000) )
        {
                require(BuyOrderAmount <= msg.value, "Ether too small");
        }
        else{
                ERC20Token _erctoken =  ERC20Token(BuyOrderToken);
                require( _erctoken.balanceOf(msg.sender) >= BuyOrderAmount, "Balance not enough");
                _erctoken.transferFrom(msg.sender, address(this), BuyOrderAmount);
        }
        ...
        emit RegisteredNFT( id, msg.sender);
    }
    
    /*
    function check_expiry( uint256 tokenId ) public view returns (uint256 exipry, uint256 current) {
    
        AnftToken _anftx =  AnftToken(TBnft);
        (uint256 amount, address tokenaddress, uint256 expiry) = _anftx.checkrepay(tokenId);
    ...
    */
    ...
    function reclaim( uint256 tokenId ) external {
            ...
            require( _anftx.ownerOf(tokenId ) == msg.sender);
            _anftx.safeTransferFrom(  msg.sender, address(this), tokenId );
            require( _anftx.ownerOf( tokenId ) == address(this));
            
            ERC20Token repaytoken = ERC20Token(tokenaddress);
            repaytoken.transfer(msg.sender,amount);
             
            emit ReclaimToken( tokenId, msg.sender, amount, tokenaddress);
    }
   
}

contract RegFactory {
    event showAddress(address _address);
    ...
    function deployBond(
        ...
        returns (uint)
    {
         bytes32 hash = keccak256(abi.encodePacked(_name, this));
         require(records[hash] == false , "Invalid name"); 
         records[hash] = true;
        
        TBRegister childContract = new TBRegister( bondsNFT, address(msg.sender));
        newnftaddr[hash] = address(childContract);
        emit showAddress(address(childContract));

        return (uint256(uint160(address(childContract))));
    }
    
    function derive( string memory _name) public view returns (address addr)
    {
       bytes32 hash = keccak256(abi.encodePacked(_name, this));
