...
contract HoneyFarmQueenDeluxe is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function claimBeesHoney(uint16[] calldata _beesIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _beesIds.length; i++) {
            ...
            if (lastReward > 0 && lastReward > STARTING_POINT && bee.staked == 0) {
                uint256 timeLeft = bee.epochsLeft * EPOCH_LENGTH;
                uint256 cutoff = bee.lastTimeFed + timeLeft;

                if (block.timestamp > cutoff) {
                    amount += HONEY_UNSTAKED_BEE_REWARDS_PER_EPOCH * ((cutoff - bee.lastTimeFed) / EPOCH_LENGTH);
                    bee.active = 0;
                    bee.becameInactiveTime = block.timestamp;
                } else {
                    uint256 epochsToReward = ((block.timestamp - bee.lastTimeFed) / EPOCH_LENGTH);
                    amount += HONEY_UNSTAKED_BEE_REWARDS_PER_EPOCH * epochsToReward;
                    bee.epochsLeft -= uint16(epochsToReward);
                }

            } else if (lastReward > 0 && lastReward > STARTING_POINT) {
                // solhint-disable-next-line
                //we get whole numbers for example if someone claims after 1 round and a half, he should be rewarded for 1 round.
                ...
                // if no rewards claimed so far, then he gets rewards from when the rewards started.
                amount += HONEY_STAKED_BEE_REWARDS_PER_EPOCH * ((block.timestamp - STARTING_POINT) / EPOCH_LENGTH);
            }
    ...
    function keepBeesActive(uint16[] calldata _beesIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _beesIds.length; i++) {
            ...
            uint16 honeyCostMultiplier = (bee.epochsLeft + EPOCHS_BEFORE_INACTIVE_BEE) / EPOCHS_BEFORE_INACTIVE_BEE;

            // if user has already feed bee before claiming, then add the multipler
            if (honeyCostMultiplier > 1) {
                honeyCostMultiplier = honeyCostMultiplier + 20;
            }
        ...
        for (uint16 i = 0; i < beesIds.length; i++) {
            ...
            if (bee.staked == 0) {
                ...
                if (block.timestamp > cutoff) {
                    ...
                    amount += HONEY_UNSTAKED_BEE_REWARDS_PER_EPOCH * ((block.timestamp - bee.lastTimeFed) / EPOCH_LENGTH);
                }
