...
contract TestTokenAirDrop is Ownable {
    // Public variables
    TestToken public tt;
    bool public airDropActive = false;
    uint256 public airDropMinimumToInvest = 50;
    mapping(address => bool) public userClaimedAirDrop;
    address[] public airDropUsers;
 
    /*
    constructor() {
        tt = new TestToken(10000000000, address(this));
    }
    */
 
    function setToken(address tokenAddress) public {
        tt = TestToken(tokenAddress);
    }
 
    function getUserBalance() public view returns (uint256) {
        return tt.balanceOf(msg.sender);
    }
    ...
    function getTokenAddress() public view returns (address) {
        return tt.getContractAddress();
    }
 
    // It's free real estate.
    function getTokens(uint256 amount) public {
        tt.transfer(msg.sender, amount);
    }
    ...
    function openAirDrop() public onlyOwner {
        require(!airDropActive, "There is already an ongoing air drop.");
        airDropActive = true;
    ...
    // Sends 10% of current balance + more for how long it's been held
    function claimAirDrop() public {
        address claimAddress = msg.sender;
        require(tt.balanceOf(claimAddress) >= airDropMinimumToInvest, "You need to have more invested to receive rewards from air drops!");
        ...
        require(tt.airDropInvestTime(claimAddress) > 0, "Your investing time needs to be greater than 0.");
        require(tt.airDropInvestTime(claimAddress) > tt.initialTimeStamp(), "Your investing time needs to be greater than the timestamp of the contract deployment.");
        ...
        // Initial 10%
        uint256 initialAmount = tt.balanceOf(claimAddress) / 10;

        // Complicated math to determine how much is added on to 10% for length tokens were held
        // If you invest super early, your finalAmount approaches 20%.
        // If you invest super late (close to the current time), your finalAmount approaches 10% but will be slightly greater.
        uint256 currTimeStamp = block.timestamp;
        uint256 userTimeStamp = tt.airDropInvestTime(claimAddress);
        uint256 finalAmount = initialAmount + (initialAmount * (1 + (((currTimeStamp - userTimeStamp) * currTimeStamp) / (currTimeStamp - tt.initialTimeStamp())))) / currTimeStamp;

        // Transfers the tokens to the user claiming them.
        tt.transfer(claimAddress, finalAmount);
   }
}
...
contract TestToken is ERC20 {
    ...
    // TODO: include realEstateWalletAddress and developerMarketingWalletAddress as constructor parameters.
    constructor(uint256 initialSupply, address _airDropContractAddress, address _preSaleContractAddress) ERC20("TestToken", "TT") {
        initialTimeStamp = block.timestamp;
        airDropContractAddress = _airDropContractAddress;
        realEstateWalletAddress = address(this);
        preSaleContractAddress = _preSaleContractAddress;
        developerMarketingWalletAddress = address(this);
        excludedFromFees[realEstateWalletAddress] = true;
        // _mint(msg.sender, initialSupply);
        _mint(airDropContractAddress, initialSupply / 2);
        _mint(preSaleContractAddress, initialSupply / 2);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 value) internal virtual override {
        ...
        uint256 realEstateFee = (value * realEstateTransactionFeePercent) / 100;
 
        // transfer(realEstateWalletAddress, realEstateFee);
 
        uint256 valueAfterFees = value - realEstateFee;
        super._beforeTokenTransfer(from, to, valueAfterFees);
    }
    */
