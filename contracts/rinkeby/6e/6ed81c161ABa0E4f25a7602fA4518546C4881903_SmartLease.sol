...
contract SmartLease {
    ...
    struct Leasing {
        ...
        uint256 lease_termination_date;
        uint256 lease_rate;
        ...
        returns (bool)
    {
        if (msg.sender != lessee) {
            ...
            activate_contract();
            lease_conditions.lease_termination_date =
                block.timestamp +
                lease_conditions.lease_duration_in_months;
            return true;
        }
        ...
        returns (bool)
    {
        ...
        } else if (get_contract_status() != CONTRACT_STATUS.ACTIVE) {
            revert(
        ...
        returns (bool)
    {
        uint256 total_monthly_lease_payment = monthly_payment() *
            lease_conditions.lease_duration_in_months;
        if (get_contract_status() != CONTRACT_STATUS.ACTIVE) {
            revert(
