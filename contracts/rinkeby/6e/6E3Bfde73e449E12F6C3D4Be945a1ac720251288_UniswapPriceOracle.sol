...
library Address {
     ...
     */
    function functionStaticCall(address target, bytes memory data) internal view returns (bool) {
    ...
    function _verifyCallResult(bool success, bytes memory returndata) public pure returns (bool) {
...
contract FactoryErrorReporter {
    ...
    enum FailureInfo {
        ...
        CREATE_PETH_POOL,
        CREATE_PPIE_POOL,
        DEFICIENCY_LIQUIDITY_OR_PAIR_IS_NOT_EXIST,
...
contract PTokenFactory is FactoryErrorReporter {
    ...
    function createPToken(address underlying_) external returns (uint) {
        uint underlyingType_ = oracle.getUnderlyingType(underlying_);

        if (!checkPair(underlying_, underlyingType_)) {
            return fail(Error.INVALID_UNDERLYING, FailureInfo.DEFICIENCY_LIQUIDITY_OR_PAIR_IS_NOT_EXIST);
        }
    ...
    function checkPair(address underlying_, uint underlyingType_) public view returns (bool) {
        if (underlyingType_ == uint(UniswapPriceOracle.UnderlyingType.RegularAsset)) {
            (address pair, uint112 ethEquivalentReserves) = oracle.searchPair(underlying_);

            return bool(pair != address(0) && ethEquivalentReserves >= minUniswapLiquidity);
        } else if (underlyingType_ == uint(UniswapPriceOracle.UnderlyingType.UniswapV2LP)) {
            uint112 totalReserve = oracle.getPoolLiquidity(underlying_);

            return bool(totalReserve >= minUniswapLiquidity);
        } else {
            return false;
        }
    }
    ...
    function _createPTokenNameAndSymbol(address underlying_, uint underlyingType_) internal view returns (string memory, string memory) {
        if (underlyingType_ == uint(UniswapPriceOracle.UnderlyingType.RegularAsset)) {
            return _createNameAndSymbolForAsset(underlying_);
        } else if (underlyingType_ == uint(UniswapPriceOracle.UnderlyingType.UniswapV2LP)) {
            return _createNameAndSymbolForUniswapV2LP(underlying_);
        } else {
            return ("","");
        }
    }

    function _createNameAndSymbolForAsset(address underlying_) internal view returns (string memory, string memory) {
    ...
    function _createNameAndSymbolForUniswapV2LP(address underlying_) internal view returns (string memory, string memory) {
...
contract UniswapPriceOracle is UniswapPriceOracleStorageV1, PriceOracle, OracleErrorReporter {
    ...
    /// @notice Support underlying type
    enum UnderlyingType {
        BadUnderlying,
        RegularAsset,
        UniswapV2LP
    ...
    function update(address asset) public returns (uint) {
        ...
        uint underlyingType = getUnderlyingType(asset);
        if (underlyingType == uint(UnderlyingType.RegularAsset)) {
            return updateRegular(asset);
        } else if (underlyingType == uint(UnderlyingType.UniswapV2LP)) {
            ...
            result1 = updateRegular(token0);
            result2 = updateRegular(token1);

            if (result1 == uint(Error.UPDATE_PRICE) || result2 == uint(Error.UPDATE_PRICE)) {
                return fail(Error.UPDATE_PRICE, FailureInfo.UPDATE_DATA);
            }

            uint power = EIP20Interface(asset).decimals();
            uint amountIn = 10**power;

            (uint token0AmountIn, uint token1AmountIn1) = getEquivalentTokensAmount(asset, amountIn);

            uint token0AmountOut = getETHAmountInternal(token0, token0AmountIn);
            uint token1AmountOut = getETHAmountInternal(token1, token1AmountIn1);

            averagePrices[asset] = (token0AmountOut + token1AmountOut);
    ...
    function getEquivalentTokensAmount(address uniswapPair, uint lpTokenAmount) public view returns (uint, uint) {
        address _token0 = IUniswapV2Pair(uniswapPair).token0();    // gas savings
        address _token1 = IUniswapV2Pair(uniswapPair).token1();    // gas savings
        uint balance0 = EIP20Interface(_token0).balanceOf(uniswapPair);
        uint balance1 = EIP20Interface(_token1).balanceOf(uniswapPair);
        uint liquidity = EIP20Interface(uniswapPair).balanceOf(uniswapPair) + lpTokenAmount;

        uint _totalSupply = EIP20Interface(uniswapPair).totalSupply();
        uint token0Amount = liquidity.mul(balance0) / _totalSupply;
        uint token1Amount = liquidity.mul(balance1) / _totalSupply;
        require(token0Amount > 0 && token1Amount > 0, 'Oracle: INSUFFICIENT_LIQUIDITY_BURNED');
        return (token0Amount, token1Amount);
    }

    function updateRegular(address asset) public returns (uint) {
        uint112 reserve0;
        uint112 reserve1;
        ...
        if (isNewAsset(asset)) {
            ...
            if (!isPeriodElapsed(asset)) {
                return fail(Error.NO_ERROR, FailureInfo.PERIOD_NOT_ELAPSED);
            }
    ...
    function calcCourseInETH(address asset) public view returns (uint) {
        ...
        return getETHAmountInternal(asset, amountIn);
    }
    ...
    function getETHAmountInternal(address asset, uint amountIn) public view returns (uint) {
    ...
    function searchPair(address asset) public view returns (address, uint112) {
        ...
        for (uint i = 0; i < poolFactories.length; i++) {
            (tempPair, ETHReserves) = searchPairWithFactory(i, asset);

            if (address(tempPair) != address(0) && ETHReserves > maxReserves) {
    ...
    function searchPairWithFactory(uint poolId, address asset) public view returns (address, uint112) {
        address pair;
        uint112 maxReserves;
        ...
        tempPair = IUniswapV2Pair(getPoolPair(asset, poolId));
        ...
        for (uint j = 0; j < stableCoins.length; j++) {
            tempPair = IUniswapV2Pair(getPoolPairWithStableCoin(asset, poolId, j));
    ...
    function factoryIsAdded(address poolFactory_) public view returns (bool) {
        for (uint i = 0; i < poolFactories.length; i++) {
    ...
    function getPoolFactoryID(address poolFactory_) public view returns (uint) {
        require(factoryIsAdded(poolFactory_));
        for (uint i = 0; i < poolFactories.length; i++) {
            if (poolFactories[i] == poolFactory_) {
                return i;
            }
    ...
    function getPoolLiquidity(address pool) public view returns (uint112) {
        address factory = IUniswapV2Pair(pool).factory();

        if (!factoryIsAdded(factory)) {
            return 0;
        }
        ...
        if (token0 == WETHToken || token1 == WETHToken) {
            ...
            if (token0 == WETHToken) {
                (, liquidity, ) = IUniswapV2Pair(pool).getReserves();
            } else {
                (liquidity, , ) = IUniswapV2Pair(pool).getReserves();
            }
            ...
            uint poolId = getPoolFactoryID(factory);

            (, uint112 ethEquivalentReserves0) = searchPairWithFactory(poolId, token0);
            (, uint112 ethEquivalentReserves1) = searchPairWithFactory(poolId, token1);

            return ethEquivalentReserves0 + ethEquivalentReserves1;
        }
    }
    ...
    function getUnderlyingType(address underlying_) public view returns (uint) {
        if (checkUniswapLP(underlying_)) {
            return uint(UnderlyingType.UniswapV2LP);
        } else if (checkAsset(underlying_)) {
            return uint(UnderlyingType.RegularAsset);
        } else {
            return uint(UnderlyingType.BadUnderlying);
        }
    }

    function checkAsset(address underlying_) public view returns (bool) {
        (address _pair,) = searchPair(underlying_);

        return bool(_pair != address(0));
    }

    function checkUniswapLP(address pool) public view returns (bool) {
        bool token0IsExist = _callOptionalReturn(pool, abi.encodeWithSelector(IUniswapV2Pair(pool).token0.selector));
        bool token1IsExist = _callOptionalReturn(pool, abi.encodeWithSelector(IUniswapV2Pair(pool).token1.selector));
        bool factoryIsExist = _callOptionalReturn(pool, abi.encodeWithSelector(IUniswapV2Pair(pool).factory.selector));

        if (!token0IsExist || !token1IsExist || !factoryIsExist) {
            return false;
        }
    ...
    function _callOptionalReturn(address pool, bytes memory data) private view returns (bool) {
        return pool.functionStaticCall(data);
    }
