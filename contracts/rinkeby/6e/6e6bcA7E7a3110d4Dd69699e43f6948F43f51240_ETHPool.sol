...
contract ETHPool is AccessControl {
    ...
    */
     ...
     * In other forms we can Steal money from our users, so users
     * have to trust us.
     * Also to accept an error range when pass earned interts, we
     * can accept and ERROR by 99% (uint num = 99). Because at the
     * same time we deposit rewards, ETHPool can generate rewards.
    */
    ...
    returns (bool success) {
        poolValue += _amount;
        uint8 fee = 99;
        uint256 thisBalance = address(this).balance * fee;
        uint256 balanceWithFee = thisBalance / 100;
        require(poolValue == balanceWithFee, "PoolValue is not equal to The POOL VALUE!");
