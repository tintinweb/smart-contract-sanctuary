...
contract InsuracePositionController is AbstractController {
    ...
    function apy() external view override returns (uint256,uint256 ) {
        ...
        uint256 _insurInOneStaking = _priceFeed.howManyTokensAinB(
            ...
            stakingAsset,
            swapRewardsVia,
            1 * 10**_stakingDecimals + 1
        );
        uint256 _poolTVLInRewardsAsset = _poolTVLInStakingAsset.mul(_insurInOneStaking).div(
            _stakingDecimals
        );
        _rewardsPerYear = _rewardsPerYear.mul(PRECISION_5_PERCENTAGE_100);
        return (_rewardsPerYear, _poolTVLInRewardsAsset);
    }
...
interface IPositionController {
    ...
    function apy() external view returns (uint256,uint256);
