...
contract Rat is IRat, ERC721Enumerable, Ownable, Pausable {
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    if (minted < PAID_TOKENS_FIRST) { // will allow tokens 1 - 1,000 to be minted
      require(minted + amount <= PAID_TOKENS_FIRST, "Not enough mints left for 0.05ETH");
      require(amount * MINT_PRICE_FIRST == msg.value, "Invalid payment amount");
    }
    else if (minted < PAID_TOKENS_SECOND) { // will allow tokens 1,001 - 3,000 to be minted
      require(minted + amount <= PAID_TOKENS_SECOND, "Not enough mints left for 0.055ETH");
      require(amount * MINT_PRICE_SECOND == msg.value, "Invalid payment amount");
    }
    else if (minted < PAID_TOKENS_THIRD) { // will allow tokens 3,001 - 6,000 to be minted
      require(minted + amount <= PAID_TOKENS_THIRD, "Not enough mints left for 0.06ETH");
      require(amount * MINT_PRICE_THIRD == msg.value, "Invalid payment amount");
    }
    else if (minted < PAID_TOKENS_TOTAL) { // will allow tokens 6,001 - 10,000 to be minted
      require(minted + amount <= PAID_TOKENS_TOTAL, "Not enough mints left for 0.069420ETH");
      require(amount * MINT_PRICE_FOURTH == msg.value, "Invalid payment amount");
    }
    else {
      require(msg.value == 0);
    }
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      if (!stake || recipient != _msgSender()) {
        ...
        _safeMint(address(Race), minted);
        tokenIds[i] = minted;
      }
      totalCHEESECost += mintCost(minted);
    }
   ...
   */
  function updateTraits(uint256 tokenId, SewerFat memory s) public {    
  ...
  function selectRecipient(uint256 seed) internal view returns (address) {
    if (minted <= PAID_TOKENS_TOTAL || ((seed >> 245) % 10) != 0) return _msgSender(); // top 10 bits haven't been used
