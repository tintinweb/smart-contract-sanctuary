    ...
    Ownable
{
    ...
    function _getMetadataIndex() private view returns (uint256) {
        uint256 randNum = _rand();
        // 19 total choices
        if (randNum < 56) {
            // common, 56%, 8 choices
            return randNum % 8;
        } else if (randNum >= 56 && randNum < 86) {
            // rare, 30%, 5 choices
            return (randNum % 5) + 8;
        } else if (randNum >= 86 && randNum < 95) {
            // very rare, 9%, 3 choices
            return (randNum % 3) + 13;
        } else if (randNum >= 95 && randNum < 99) {
            // ultra rare, 4%, 2 choices
            return (randNum % 2) + 16;
        } else {
            // super ultra rare, 1%, 1 choice
            return 18;
        }
    }
    ...
    function mintWhitelistGeneralMembership(bytes32[] calldata proof) external {
        ...
        require(!bought[msg.sender], "Address can only recieve 1 token");
        require(
            !Address.isContract(msg.sender),
            "Caller must not be a contract"
        );
    ...
    // @dev Returns if a sender is whitelisted & hasn't claimed
    // @param proof Merkel tree proof
    function isEligible(bytes32[] calldata proof) public view returns (bool) {
        if (
            MerkleProof.verify(
                proof,
                root,
