...
contract StakingRegister is IStakingRegister, Managed {
    using SafeMath for uint256;
    ...
    uint256 public defaultFeePercentage = 100; // 1%

    EnumerableSet.AddressSet private stakes;
    mapping(address => EnumerableSet.AddressSet) private ownerStakes;

    modifier canChange(address _stakingAddress) {
        require(
            hasPermission(_msgSender(), ROLE_ADMIN) ||
                (hasPermission(_msgSender(), ROLE_REGULAR) &&
                    ownerStakes[msg.sender].contains(_stakingAddress)),
            "Register: You don't have permission's"
        );
        _;
    }
    ...
    function isRegistered(address _stakingAddress) external view returns(bool){
        return stakes.contains(_stakingAddress);
    }

    function getDefaultFeePercentage()
        external
        view
        override
        returns (uint256)
    ...
    function list(uint256 _offset, uint256 _limit)
        ...
        returns (address[] memory result)
    {
        uint256 to = (_offset.add(_limit)).min(stakes.length()).max(_offset);

        result = new address[](to - _offset);

        for (uint256 i = _offset; i < to; i++) {
            result[i - _offset] = stakes.at(i);
        }
    }
    ...
    function listByUser(
        ...
        EnumerableSet.AddressSet storage set = ownerStakes[_user];

        uint256 to = (_offset.add(_limit)).min(set.length()).max(_offset);

        result = new address[](to - _offset);

        for (uint256 i = _offset; i < to; i++) {
            result[i - _offset] = set.at(i);
        }
    }

    function harvestAll(address[] calldata _stakingsAddress) external override {
        for (uint256 i = 0; i < _stakingsAddress.length; i++) {
            IStaking(_stakingsAddress[i]).harvestFor(msg.sender);
        }
    }
    ...
    function isOwner(address owner, address staking)
        ...
        returns (bool)
    {
        EnumerableSet.AddressSet storage set = ownerStakes[owner];

        return set.contains(staking);
    }
    ...
    function add(
        ...
        address _stakingAddress,
        bool _isLiquidityMining
    ) external override requirePermission(CAN_ADD_STAKING) {
        require(!stakes.contains(_stakingAddress), "Register: Alreade exists");

        stakes.add(_stakingAddress);

        EnumerableSet.AddressSet storage set = ownerStakes[_owner];
        set.add(_stakingAddress);
    ...
    function remove(address _owner, address _stakingAddress)
        external
        override
        requirePermission(ROLE_ADMIN)
    {
        stakes.remove(_stakingAddress);
        EnumerableSet.AddressSet storage set = ownerStakes[_owner];
        set.remove(_stakingAddress);

        emit RemoveStaking(msg.sender, _owner, _stakingAddress);
    }

    function withdrawExtraTokens(address _stakingAddress, address _tokenAddress)
        external
        override
        canChange(_stakingAddress)
    {
        IStaking(_stakingAddress).withdrawExtraTokens(
            _tokenAddress,
            msg.sender
        );
        emit WithdrawExtraTokensFromStaking(
            msg.sender,
            _stakingAddress,
            _tokenAddress
        );
    ...
    function setDefaultDepositFeePercentage(uint256 _amount)
        external
        override
        requirePermission(ROLE_ADMIN)
    {
        defaultFeePercentage = _amount;
        emit SetDefaultDepositFee(_msgSender(), _amount);
    }
    ...
    function setDepositeFee(address _addr, uint256 _amount)
        ...
        requirePermission(ROLE_ADMIN)
    {
        IStaking(_addr).setDepositeFee(_amount);
        emit SetDepositFee(_msgSender(), _addr, _amount);
    }
    ...
    function setCanTakeReward(address _addr, bool _value)
        ...
        canChange(_addr)
    {
        IStaking(_addr).setCanTakeReward(_value);
        emit SetCanTakeReward(_msgSender(), _addr, _value);
    }
    ...
    function setPrivate(address _addr, bool _value)
        ...
        canChange(_addr)
    {
        IStaking(_addr).setPrivate(_value);
        emit SetPrivate(_msgSender(), _addr, _value);
    }
    ...
    function setRewardSettingLiquidityMining(
        ...
        uint256[] calldata _rewardPerBlock,
        uint256[][] calldata _rewardApprovedNFTId
    ) external override canChange(_stakingAddress) {
        _setRewardSetting(
            _stakingAddress,
            _rewardTokens,
            _rewardPerBlock,
            _rewardApprovedNFTId
        );
    }
    ...
    function setRewardSettingGenesisStaking(
        ...
        uint256[][] memory arr = new uint256[][](_rewardTokens.length);
        _setRewardSetting(_stakingAddress, _rewardTokens, _rewardPerBlock, arr);
    }
    ...
    function addDurationGenesisStaking(
        ...
        uint256[][] memory approvedNFTId = new uint256[][](0);

        _addDurartion(_stakingAddress, _blockAmount, tokens, approvedNFTId);
    }
    ...
    function _setRewardSetting(
        ...
        uint256[][] memory _rewardApprovedNFTId
    ) internal {
        require(
            (_rewardTokens.length == _rewardPerBlock.length) &&
                (_rewardTokens.length == _rewardApprovedNFTId.length),
            "incorrect reward"
        );

        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            uint256 needTransfer =
                calculateNumberTokensNeedTransfer(
                    _stakingAddress,
                    _rewardTokens[i],
                    _rewardPerBlock[i]
                );

            if (_rewardTokens[i].supportsInterface(InterfaceId_ERC721)) {
                uint256 countNFT = needTransfer.div(UNITS).add(1);
                require(
                    countNFT <= _rewardApprovedNFTId[i].length,
                    "insufficient of NFT"
                );
                IERC721 erc721 = IERC721(_rewardTokens[i]);
                for (uint256 index = 0; index < countNFT; index++) {
                    erc721.safeTransferFrom(
                        msg.sender,
                        _stakingAddress,
                        _rewardApprovedNFTId[i][index]
                    );
                }
            } else {
        ...
        IStaking(_stakingAddress).setRewardSetting(
            _rewardTokens,
            _rewardPerBlock
        );
    }

    function calculateNumberTokensNeedTransfer(
        ...
        (startBlock, rewardEndBlock) = staking.getTimePoint();
        ...
        if (staking.getRewardsPerBlockInfo(rewardToken) >= newRewardPerBlock)
            return 0;

        uint256 requiredAmount =
            rewardEndBlock.sub(startBlock).mul(
                newRewardPerBlock.sub(
                    staking.getRewardsPerBlockInfo(rewardToken)
                )
            );

        uint256 balance = 0;

        if (rewardToken.supportsInterface(InterfaceId_ERC721)) {
            balance = IERC721(rewardToken)
                .balanceOf(stakingAddress)
                .mul(UNITS)
                .sub(staking.getMustBePaid(rewardToken));
        } else {
            balance = IERC20(rewardToken).balanceOf(stakingAddress).sub(
                staking.getMustBePaid(rewardToken)
            );
        }
        ...
        return requiredAmount.sub(balance);
    }

    function _addDurartion(
        address _stakingAddress,
        uint256 _blockAmount,
        address[] memory _rewardTokenAddress,
        uint256[][] memory _approvedNFTid
    ) internal {
        require(_blockAmount > 0, "Register: block amount can't be 0");

        require(
            _rewardTokenAddress.length == _approvedNFTid.length,
            "Register: incorrect value"
        );

        IStaking staking = IStaking(_stakingAddress);

        uint256 endBlock;
        uint256 transferAmount;

        uint256[] memory rewardPerBlock;
        address[] memory tokensRewards;

        (, endBlock) = staking.getTimePoint();

        (tokensRewards, rewardPerBlock) = staking.getRewardsPerBlockInfos();

        for (uint256 i = 0; i < tokensRewards.length; i++) {
            if (rewardPerBlock[i] > 0) {
                transferAmount = rewardPerBlock[i].mul(_blockAmount);
                if (!tokensRewards[i].supportsInterface(InterfaceId_ERC721)) {
                    IERC20(tokensRewards[i]).safeTransferFrom(
                        msg.sender,
                        _stakingAddress,
                        transferAmount
                    );
        ...
        staking.setRewardEndBlock(endBlock.add(_blockAmount));

        emit AddDuration(msg.sender, _stakingAddress, _blockAmount);
    }

    function _calculateAndTransferNFT(
        address _stakingAddress,
        uint256 _blockAmount,
        address[] memory _rewardTokenAddress,
        uint256[][] memory _approvedNFTid
        ...
        for (uint256 i = 0; i < _rewardTokenAddress.length; i++) {
            uint256 rewardPerBlock =
                IStaking(_stakingAddress).getRewardsPerBlockInfo(
                    _rewardTokenAddress[i]
                );

            uint256 transferAmount =
                rewardPerBlock.mul(_blockAmount).div(UNITS).add(1);

            require(
                transferAmount <= _approvedNFTid[i].length,
                "Register: Not enough amount of approve NFT id"
            );

            IERC721 erc721 = IERC721(_rewardTokenAddress[i]);
            for (uint256 index = 0; index < transferAmount; index++) {
                erc721.safeTransferFrom(
                    msg.sender,
                    _stakingAddress,
                    _approvedNFTid[i][index]
                );
            }
        }
    }
