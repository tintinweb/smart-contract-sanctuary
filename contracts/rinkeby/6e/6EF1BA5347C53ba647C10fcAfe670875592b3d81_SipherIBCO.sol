...
contract SipherIBCO is Ownable {
    ...
    uint256 public constant START = 1637988300;  // Monday, December 6, 2021 12:00:00 AM GMT+07:00
    uint256 public constant END = START + 3 days;
    uint256 public constant TOTAL_DISTRIBUTE_AMOUNT = 40000 * 10**18;
    uint256 public constant MINIMAL_PROVIDE_AMOUNT = 4 ether;
    ...
    function deposit() external payable {
        ...
        if (provided[msg.sender] > accumulated[msg.sender]){
            accumulated[msg.sender] = provided[msg.sender];
        }
     ...
     */
    function _getWithdrawableAmount(address _user) internal view returns (uint) {
     ...
     */
    function _getEstReceivedToken(address _user) internal view returns (uint) {
        uint userShare = provided[_user];
        return (TOTAL_DISTRIBUTE_AMOUNT * userShare) / Math.max(totalProvided, MINIMAL_PROVIDE_AMOUNT);
    }
    
    /**
     * @dev Calculate locked ether from accumulated Ether
     */
    function getLockedInvestment(uint256 userAccumulated) external pure returns (uint256) {
     ...
     */
     ...
     // provided[user] - amount > getLockedAmount(user)
    function withdraw(uint256 amount) external {
        ...
        emit Claim(msg.sender, amount, 0);
    }
     ...
     */
    function getEstReceivedToken(address _user) external view returns (uint) {
     ...
     */
    function getLockedAmount(address _user) external view returns (uint) {
        return _getLockedAmount(_user);
    }

    function _getLockedAmount(address _user) internal view returns (uint) {
        uint userAccumulated = accumulated[_user];
        return userAccumulated - _withdrawCap(userAccumulated);
    }
