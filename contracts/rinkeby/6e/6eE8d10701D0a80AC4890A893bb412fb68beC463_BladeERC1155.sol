...
contract BladeERC1155 is ERC1155, ERC1155Burnable, Ownable {
    ...
    constructor(address signer) ERC1155("https://nifty-island-palm-drop-public.s3.us-east-2.amazonaws.com/") {
        _signer = signer;
    }
    ...
    function mint(address to, uint256 id, uint256 nonce, bytes calldata signature)
        external {
        require(usedNonces[nonce] == false, "can't use the same signature twice");
        usedNonces[nonce] = true;

        bytes32 hash = ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(id, to, nonce)));
