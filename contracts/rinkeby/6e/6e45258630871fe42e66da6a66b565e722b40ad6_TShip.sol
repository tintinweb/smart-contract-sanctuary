...
contract TShip is ERC721, Ownable {
    ...
    uint256 public immutable amountFlee; // 10240
    ...
    function pledge(uint256 shipId) public {
        ...
        update(msg.sender,shipId);
        uint256 powerNow  = getAllShipIdsPowerPle(msg.sender);
        TokenMint token = TokenMint(tokenAddress);
    ...
    function mintShipWork(uint256 tpe) public payable {
        ...
        if (tpe == 1) {//ship one star
            ...
            require(amountShipTwoTotal().add(1) <= amountShip[2], "Purchase would exceed max supply of Martial");
            mintIndex = amountShipTwoTotal().add(1);
            price = priceShip[2];
            _amountShipTwoSet(msg.sender, mintIndex);
        } else if (tpe == 3) {//ship three star
            require(amountShipThreeTotal().add(1) <= amountShip[3], "Purchase would exceed max supply of Martial");
            mintIndex = amountShipThreeTotal().add(1);
            price = priceShip[3];
            _amountShipThreeSet(msg.sender, mintIndex);
        } else if (tpe == 4) {//work one star
            require(amountWorkOneTotal().add(1) <= amountWork[1], "Purchase would exceed max supply of Martial");
            mintIndex = amountWorkOneTotal().add(1);
            price = priceWork[1];
            _amountWorkOneSet(msg.sender, mintIndex);
        } else if (tpe == 5) {//work two star
            require(amountWorkTwoTotal().add(1) <= amountWork[2], "Purchase would exceed max supply of Martial");
            mintIndex = amountWorkTwoTotal().add(1);
            price = priceWork[2];
            _amountWorkTwoSet(msg.sender, mintIndex);
        } else if (tpe == 6) {//work three star
            require(amountWorkThreeTotal().add(1) <= amountWork[3], "Purchase would exceed max supply of Martial");
            mintIndex = amountWorkThreeTotal().add(1);
            price = priceWork[3];
    ...
    function mintFlee(uint256 shipTokenId, uint256[] memory workTokenIds) public {
        ...
        uint256 powerAll = 0;
        for (uint j = 0; j < workTokenIds.length; j++) {
            if (workTokenIds[j] >= 301 && workTokenIds[j] <= 400) {
                powerAll += powerWork[1] ;
            }
            if (workTokenIds[j] >= 401 && workTokenIds[j] <= 500) {
                powerAll += powerWork[2] ;
            }
            if (workTokenIds[j] >= 501 && workTokenIds[j] <= 600) {
...
library shipMapping
{
    ...
    struct itmap
    {
        ...
        mapping(uint256 => bool) worksStatus; // workId status for ple
        uint size;
    }
    ...
    function recharge(itmap storage self, address key, uint256 shipTokenId, uint256 tokenAmount) internal returns (bool)
    {
        if (self.data[key][shipTokenId].isVaild) {
            self.data[key][shipTokenId].isPledge = true;
            //充值
            self.data[key][shipTokenId].energy = self.data[key][shipTokenId].energy + tokenAmount * 7200;
            self.data[key][shipTokenId].overTime = self.data[key][shipTokenId].energy + block.timestamp;
            //充值
            return true;
        } else {
