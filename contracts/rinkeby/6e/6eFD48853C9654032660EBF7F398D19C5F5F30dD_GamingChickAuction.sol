...
contract GamingChickAuction is OwnableContract, IERC721Receiver, Initializable{
    ...
    function removeWinnerRecords() public onlyAdmin {
        ...
        for (uint256 i = 0; i < winnerList[id].length; i++) {
            ...
            // delete userInfo[0][addr];
            // delete userInfo[1][addr];
            // delete userInfo[2][addr];
            // delete userInfo[3][addr];

            // UserInfo memory userDetail;
            winnerInfo.push();
            winnerInfo.push();
            winnerInfo.push();
            winnerInfo.push();

            userInfo[0][addr] = winnerInfo[winnerInfo.length - 1];
            userInfo[1][addr] = winnerInfo[winnerInfo.length - 2];
            userInfo[2][addr] = winnerInfo[winnerInfo.length - 3];
            userInfo[3][addr] = winnerInfo[winnerInfo.length - 4];
