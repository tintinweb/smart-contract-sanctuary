...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    {        
        ...
        // (uint256 reserveA0, uint256 reserveB0, ) = IUniswapV2Pair(pair).getReserves();
        // uint256 balanceA0 = IERC20(tokenA).balanceOf(pair);
        // uint256 balanceB0 = IERC20(tokenB).balanceOf(pair);

        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = IUniswapV2Pair(pair).mint(to);                  
        
        // uint256 balanceA1 = IERC20(tokenA).balanceOf(pair);
        // uint256 balanceB1 = IERC20(tokenB).balanceOf(pair);
        // require(false, string(abi.encodePacked('444 : ' , uint2str(amountA), ' : ', uint2str(amountB), ' : ', uint2str(liquidity), ' : ',uint2str(balanceA0), ' : ', uint2str(balanceB0), ' : ',uint2str(balanceA1), ' : ', uint2str(balanceB1))));
        // require(false, string(abi.encodePacked('444 : ', uint2str(balanceA0), ' : ', uint2str(balanceB0), ' : ',uint2str(balanceA1), ' : ', uint2str(balanceB1))));
        // require(balanceB0 + amountB == balanceB1, string(abi.encodePacked('tokenB is deflationary token : ', uint2str(balanceB0), ':', uint2str(amountB), ':', uint2str(balanceB1))));
        // require(balanceA0 + amountA == balanceA1, string(abi.encodePacked('tokenA is deflationary token : ', uint2str(balanceA0), ':', uint2str(amountA), ':', uint2str(balanceA1))));
    }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = UniswapV2Library.getAmountsOut(factory, amountIn*995/1000, path);
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex'a02f729b5c0b7c5a3b95e898143010169185bd4e779182f6bd4a908c58d38618' // init code hash of factory
                    )
                )
            )
        );
