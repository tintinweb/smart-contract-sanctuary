...
contract CompoundStrategy is BaseRhoStrategy, PausableUpgradeable {
    ...
    bytes public encodedPathForUniV3;
    ...
    function initialize(
        ...
        uint256 _lockDurationInBlock,
        uint256 _blocksPerYear,
        bytes memory _encodedPathForUniV3,
        uint256 _slippage
    ) public initializer {
        ...
        swapRouter = ISwapRouter(_uniswapV3RouterAddr);

        encodedPathForUniV3 = _encodedPathForUniV3;
        slippage = _slippage;
        // uint24 poolFee1 = 3000;
        // uint24 poolFee2 = 500;
        // address wethAddr = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
        //encodedPathForUniV3 = abi.encodePacked(address(comp), poolFee1, wethAddr, poolFee2, address(underlying));
        NAME = "Compound";
    }
    ...
    function collectRewardToken() external override onlyRole(VAULT_ROLE) returns (bool) {
        ...
        uint256 minUnderlyingQty =
            (((compQty * compPrice * 10**underlying.decimals()) / 10**comp.decimals() / 10**compPriceDec) * slippage) /
                1000;

        if (swapExactInputMultihop(address(comp), msg.sender, compQty, minUnderlyingQty, encodedPathForUniV3) > 0) {
            return true;
        } else {
            return false;
        }
    ...
    function shouldCollectReward(uint256 rewardCollectThreshold) external view override returns (bool) {
        ...
        //uint256 compQty = comptroller.compAccrued(address(this));
        uint256 compQty = comp.balanceOf(address(this)) + comptroller.compAccrued(address(this));

        uint256 minUnderlyingQty =
            (((compQty * compPrice * 10**underlying.decimals()) / 10**comp.decimals() / 10**compPriceDec) * 990) / 1000;

        if (minUnderlyingQty > rewardCollectThreshold) {
            return true;
        } else {
            return false;
        }
