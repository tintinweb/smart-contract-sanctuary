...
contract TestingContract2 is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    
    string private _baseTigerURI;
    uint256 public MAX_TOKENS;
    uint256 public constant _price = 10000000000000000;
    bool public IsSalesActive = true;
    uint256 public constant _reservedCount = 100;
    
    constructor (uint256 maxTokens, string memory baseTigerURI) ERC721 ("Test Tigers", "TT") {
        MAX_TOKENS = maxTokens;
        setBaseURI(baseTigerURI);
    }
    ...
    function mint(uint256 count) public payable onlyOwner {
        require(count <= 1000);
        require(totalSupply().add(count) <= MAX_TOKENS);
        require(_price.mul(count) <= count * _price);
