...
contract Synthesis is RelayRecipientUpgradeable {
  ...
  function metaBurnSyntheticToken(
  ...
  ) external payable whenNotPaused returns (bytes32 txID, bytes memory out) {
    require(msg.value == nativeTokenPrice, "Symb: Not enough money");

    ISyntFabric(fabric).unsynthesize(
      _msgSender(),
      _metaBurnTransaction.amount,
      _metaBurnTransaction.path[0]
    );
    ...
    path[0] = rtoken;
    path[1] = _metaBurnTransaction.path[1];

    out = abi.encodeWithSelector(
      bytes4(
        keccak256(
          bytes(
            "metaUnsynthesize(bytes32,address,uint256,uint256,address[],uint256,address,bytes)"
          )
        )
      ),
      txID,
      ...
      _metaBurnTransaction.amount,
      _metaBurnTransaction.slippage,
      path,
      _metaBurnTransaction.deadline,
      _metaBurnTransaction.dex,
    ...
    requests[txID] = TxState({
      ...
      token: rtoken,
      stoken: _metaBurnTransaction.path[0],
