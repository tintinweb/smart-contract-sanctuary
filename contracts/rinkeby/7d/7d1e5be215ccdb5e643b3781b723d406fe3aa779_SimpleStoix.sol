...
contract SimpleStoix is ERC721, ERC721Enumerable, Ownable {
    ...
    address private _splitterAddress;
    string public provenance = "";
    string public baseURI =
        "https://abtrum80la.execute-api.us-east-1.amazonaws.com/";

    constructor(address splitterAddress, address ownersAddress)
        ERC721("Simple Stoix", "STOIX")
    {
        _splitterAddress = splitterAddress;
        _ownersAddress = ownersAddress;
    }

    // - Minting
    function _mintStoix(uint256 numberOfTokens, address sender) internal {
    ...
    function mintStoix(uint256 numberOfTokens) public payable {
        require(saleIsActive, "sale is not active");
        require(numberOfTokens <= 10, "can only mint 10");
        require(
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 splitAmount = contractBalance.mul(50).div(100);

        require(payable(_splitterAddress).send(splitAmount));
        require(payable(_ownersAddress).send(contractBalance.sub(splitAmount)));
    }

    function claimReserved(uint16 number, address receiver)
        external
        onlyOwner
    {
