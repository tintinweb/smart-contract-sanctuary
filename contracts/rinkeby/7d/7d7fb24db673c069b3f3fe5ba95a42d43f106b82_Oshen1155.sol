...
contract Oshen1155 is ERC1155 {
    ...
    function addMinter(address _minter) public onlyOwner {
        ...
        require(minters[_minter],"Minter: minter already exists");
    ...
    function removeMinter(address _minter) public onlyOwner {
        require(_minter != address(0),"Minter: minter is the zero address");
        require(!minters[_minter],"Minter: minter not exists");   
    ...
    function getMinters() public view onlyOwner returns(address[] memory) {
        address[] memory listOfMinters = new address[] (mintersCount);
        for(uint256 i = 0; i < _minters.length ; i++){
            if(minters[_minters[i]]){
                listOfMinters[i] = _minters[i];
            }
        }
        return ( listOfMinters);
    } 

    function mint(string memory uri, uint256 supply, uint256 fee)  public {
        require(minters[msg.sender],"ERC1155: caller is not minter");        
