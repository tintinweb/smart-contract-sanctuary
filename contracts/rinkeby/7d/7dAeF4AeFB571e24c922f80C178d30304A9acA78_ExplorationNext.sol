...
contract  ExplorationNext is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private constant _tTotal = 387000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "ExplorationNext";
    string private _symbol = "ENEXT";
    uint8 private _decimals = 18;

    uint256 public reflectionFee = 3;
    uint256 private previousReflectionFee = reflectionFee;

    uint256 public marketingWalletFee = 2;
    ...
    uint256 public minLiquidityToken = 50 * 10**18;

    event FeeEnable(bool enableFee);
    event SetMaxTxPercent(uint256 maxPercent);
    event SetTaxFeePercent(uint256 taxFeePercent);
    event ExternalTokenTransfered(address externalAddress,address toAddress,uint256 amount);
   ...
   function approve(address spender, uint256 amount) external virtual override returns (bool) {
    ...
    function takeMarketingWalletFee(uint256 rMarketingWalletFee, uint256 tMarketingWalletFee) internal {
        if (_isExcluded[devAddress]) {
            _rOwned[devAddress] = _rOwned[devAddress].add(rMarketingWalletFee);
        } else {
            _rOwned[devAddress] = _rOwned[devAddress].add(rMarketingWalletFee);
            _tOwned[devAddress] = _tOwned[devAddress].add(tMarketingWalletFee);
        }
    ...
    function calculateMarketingWalletFee(uint256 _amount) internal view returns (uint256) {
        return _amount.mul(marketingWalletFee).div(10**2);
    }
    ...
    function removeAllFee() internal {
        if (reflectionFee == 0 && marketingWalletFee == 0 && liquidityFee == 0) return;
    ...
    function _transferStandard(address sender,address recipient,uint256 tAmount) internal {
        (uint256 tTransferAmount,uint256 tFee,uint256 tMarketingWalletFee,uint256 tliquidityFee) = getTValues(tAmount);
        (uint256 rAmount,uint256 rTransferAmount,uint256 rFee,uint256 rdevFee,uint256 rliquidityFee) = getRValues(tAmount, tFee, tMarketingWalletFee, tliquidityFee);
        {
            address from = sender;
        ...
        takeMarketingWalletFee(rdevFee, tMarketingWalletFee);
    ...
    function _transferToExcluded( address sender,address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount,uint256 tFee,uint256 tMarketingWalletFee,uint256 tliquidityFee) = getTValues(tAmount);
        (uint256 rAmount,uint256 rTransferAmount,uint256 rFee,uint256 rMarketingWalletFee,uint256 rliquidityFee) = getRValues(tAmount, tFee, tMarketingWalletFee, tliquidityFee);
        {
            address from = sender;
            _rOwned[from] = _rOwned[from].sub(rAmount);
        }
