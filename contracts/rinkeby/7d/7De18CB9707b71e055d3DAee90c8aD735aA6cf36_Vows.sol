...
contract Vows is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function accept(address proposer) public nonReentrant {
        require(
            proposals[proposer] == msg.sender,
            "no proposal to you from them"
        );
        ...
        uint256 accepterTokenId = uint256(
            keccak256(abi.encodePacked(msg.sender, proposer))
        );
        uint256 proposerTokenId = uint256(
            keccak256(abi.encodePacked(proposer, msg.sender))
        );
    ...
    function annul() public nonReentrant {
        ...
        uint256 senderTokenId = uint256(
            keccak256(abi.encodePacked(msg.sender, marriedTo))
        );
        uint256 spouseTokenId = uint256(
            keccak256(abi.encodePacked(marriedTo, msg.sender))
        );
        ...
        returns (string memory)
    {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: black; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="white" /><text x="10" y="20" class="base">In the block number ';
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "A Vow", "description": "Vows are eternal commitments from one address to another.", "image": "data:image/svg+xml;base64,',
