 ...
 *
 */
contract MintableSaleWithLimitAndApproval is Ownable {
  // ----- SLOT.1 (256/256)
  /**
  ...
  /**
   * Copied from https://docs.soliditylang.org/en/v0.8.9/solidity-by-example.html?highlight=ecrecover#recovering-the-message-signer-in-solidity
   */
  ...
  function validSignature(bytes memory signature) public view returns (bool) {
    bytes32 message = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n", "40", toAsciiString(msg.sender)));

    return recoverSigner(message, signature) == backend;
  }
   ...
   */
  function buy(uint32 _amount, bytes memory signature) public payable {
    // delegate to `buyTo` with the transaction sender set to be a recipient
    buyTo(msg.sender, _amount, signature);
  }
   ...
   * @param _amount amount of tokens to create, two or more
   */
  function buyTo(address _to, uint32 _amount, bytes memory signature) public payable {
    // verify the inputs
   ...
   *      Accepts ETH as payment and mints a token
   */
  function buySingle(bytes memory signature) public payable {
    // delegate to `buySingleTo` with the transaction sender set to be a recipient
    buySingleTo(msg.sender, signature);
  }
   ...
   */
  function buySingleTo(address _to, bytes memory signature) public payable {
  ...
  /**
   * Taken from https://ethereum.stackexchange.com/questions/8346/convert-address-to-string
   */
