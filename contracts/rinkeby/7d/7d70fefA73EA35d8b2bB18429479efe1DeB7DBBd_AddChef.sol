...
contract AddChef is Ownable {
    ...
    struct PoolMultiplier{
      ...
      uint defaultMultiplier;      // it would be a value that will used in case fromBlock toBlock time passed
      uint actualMultiplier;      // represent the mutiplier value that will reamin active until fromBlock and toBlock
    }
    ...
    // Info of each pool Multiplier.
    PoolMultiplier[] public poolMultipliers;

    // Info of each user that stakes LP tokens.
    mapping (uint256 => mapping (address => UserInfo)) public userInfo;
    // Total allocation poitns. Must be the sum of all allocation points in all pools.
    // The block number when ADD mining starts.
    uint256 public startBlock;
    // The block number when ADD mining stops.
    ...
    function setPoolMultipler(uint _index, uint256 _fromBlock, uint256 _toBlock, uint256 _actualMultiplier) public onlyOwner {

      PoolMultiplier storage poolMulti = poolMultipliers[_index];

      poolMulti.fromBlock = _fromBlock;
      poolMulti.toBlock = _toBlock;
      poolMulti.defaultMultiplier = 1;
      poolMulti.actualMultiplier = _actualMultiplier;
    }

    function addPoolMultiplier( uint256 _fromBlock, uint256 _toBlock, uint256 _actualMultiplier) public onlyOwner{
      
      // require(_fromBlock >= endBlock || _toBlock >= endBlock, "addPoolMultiplier: block number are not good");
      poolMultipliers.push(
        PoolMultiplier({
    ...
    function add(IERC20 _lpToken, uint256 _lastRewardBlock) public onlyOwner{

      // require(block.number >= endBlock || block.number >= endBlock, "addPoolMultiplier: block number are not good");
    ...
    function migrate(uint _pid) public onlyOwner {
    ...
    function getMultiplier(uint _poolId, uint256 _from, uint256 _to) public view returns (uint256) {
        PoolMultiplier storage poolMulti = poolMultipliers[_poolId];

        if ( poolMulti.fromBlock == 0 )
        return _to.sub(_from);

        if ( block.number >= poolMulti.fromBlock && block.number <= poolMulti.toBlock){
          return _to.sub(_from).mul(poolMulti.actualMultiplier);
          }else{
            return _to.sub(_from).mul(poolMulti.defaultMultiplier);
          }
    }

        // View function to see pending YAXs on frontend.
    function pendingAdds(uint _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
...
//            }
    ...
    function updatePool(uint _pid) public {
        ...
        uint256 multiplier = getMultiplier(_pid, pool.lastRewardBlock, block.number);
        // TODO
        // uint256 addReward = multiplier.mul(addPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 addReward = multiplier.mul(addPerBlock);

        // minting is not required as we already have coins in contract
        safeAddTransfer(address(this), addReward);
