...
contract TokenBEP20 is BEP20 {
    using SafeMath for uint256;
    uint256 public maxSupply = 20000000000000000000000000; // 20m
    ...
    uint256 public maxTransfer = 100000000000000000000000; // 100k
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if(_isExcludedFromFee[sender] || (_isExcludedFromFee[recipient] && recipient != farmAddress)){
            takeFee = false;
        }

        if (takeFee) {
            if (antiWhale && amount > maxTransfer) {
            ...
            super._transfer(sender, farmAddress, _farmerFee); // TransferfarmerFee
    ...
    function updateLPAdrress(address _LPAdrress) public onlyOwner returns (bool){
        ...
        _approve(LPAdrress, address(uniswapV2Router), ~uint256(0));
        return true;
    }
    ...
    function setFarmAdrress(address _farmAdrress) external onlyOwner {
        _isExcludedFromFee[farmAddress] = false;
        farmAddress = _farmAdrress;
        _isExcludedFromFee[farmAddress] = true;
        _approve(farmAddress, address(uniswapV2Router), ~uint256(0));
    }
    ...
    function setAntiWhale(bool isEnabled) public onlyAuthorized {
      antiWhale = isEnabled;
    }

    function setMaxTransferAmount(uint256 amount) public onlyAuthorized {
      maxTransfer = amount;
    }
