...
interface full721 is IERC721, IERC721Enumerable {
...
contract allocator is Ownable {
    ...
    event ImmediateResolution(address requestor,IERC721 token,address vault,address recipient,uint256 tokenId);
    event RandomCardRequested(address requestor,IERC721 token,address vault,address recipient, bytes32 requestHash);
    ...
    function getMeRandomCardsWithCallback(full721 token, address[] memory vaults, address recipient) external noCallbacks onlyAuth {
        getMeRandomCards(token, vaults, recipient, true);
    }

    function getMeRandomCardsWithoutCallback(full721 token, address[] memory vaults, address recipient) external noCallbacks onlyAuth {
        getMeRandomCards(token, vaults, recipient, false);
    }

    function getMeRandomCards(full721 token, address[] memory vaults, address recipient, bool withCallback ) internal {
    ...
    function my_process(uint256 random, bytes32 _requestId) internal {
        ...
        for (uint j = 0; j < len; j++) {
            ...
            req.token.safeTransferFrom(vault,req.recipient,tokenId);
            vaultRequests[vault]--;
            random = random >> 32;
        }
    ...
    function checkAccess(full721 token, address vault) external view returns (bool) {
