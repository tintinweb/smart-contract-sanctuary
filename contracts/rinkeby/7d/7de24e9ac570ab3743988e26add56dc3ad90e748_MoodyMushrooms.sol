    ...
    */
    function mintFlower(uint numberOfTokens) public payable {
        ...
        if(presaleIsActive && !saleIsActive){
            ...
            require(_tokenSupply.current().add(numberOfTokens) <= MAX_PRESALE_SHROOM_SUPPLY, "Purchase would exceed pre-sale max supply of Moody Mushrooms");
        }
        ...
        require(_tokenSupply.current().add(numberOfTokens) <= MAX_SHROOM_SUPPLY, "Purchase would exceed max supply of Moody Mushrooms");
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            if (_tokenSupply.current() <= MAX_SHROOM_SUPPLY) {
        ...
        if (startingIndexBlock == 0 && (_tokenSupply.current() == MAX_SHROOM_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
            startingIndexBlock = block.number;
        } 
    }
    /*
     * Replacement for totalSupply() to save gas
     */
