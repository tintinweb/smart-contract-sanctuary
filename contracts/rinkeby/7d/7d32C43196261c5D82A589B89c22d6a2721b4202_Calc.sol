...
contract Calc is FractionalExponents {
	...
	function earned(uint256 depositEpoch, uint256 currentEpoch, uint256 amount) public view returns (uint256 rewardRateNoCoefficient, uint256 rewardRate, uint256 earnedAmount) {
        ...
        rewardRateNoCoefficient = rewardRateMantissa.mul(uint256(1 ether)).div(uint256(1) << uint256(rewardRateExponent)); // convert to realAmount*1e18
        
        rewardRate = rewardRateNoCoefficient.mul(rootN).div(rootD).div(86400); // multiply by (1.35/86400)
