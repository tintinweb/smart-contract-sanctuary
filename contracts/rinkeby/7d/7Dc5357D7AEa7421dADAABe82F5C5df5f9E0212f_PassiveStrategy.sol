...
contract PassiveStrategy {
    ...
    function rebalance() external {
        ...
        require(block.timestamp >= lastTimestamp.add(period), "period");

        (, int24 tick, , , , , ) = pool.slot0();
        int24 tickMove = tick > lastTick ? tick - lastTick : lastTick - tick;
        require(tickMove >= minTickMove, "tickMove");

        int24 twap = getTwap();
        int24 twapDeviation = tick > twap ? tick - twap : twap - tick;
        require(twapDeviation <= maxTwapDeviation, "twapDeviation");

        int24 maxThreshold = baseThreshold > limitThreshold ? baseThreshold : limitThreshold;
        require(tick >= TickMath.MIN_TICK + maxThreshold + tickSpacing, "too low");
        require(tick <= TickMath.MAX_TICK - maxThreshold - tickSpacing, "too high");
