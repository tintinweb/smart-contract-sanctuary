...
contract FlashMints is ERC721, Ownable, ReentrancyGuard  {
    ...
    IERC721 public mintPassNFTAddress;

    constructor() ERC721("Flash Mints", "FLSHMNTS") {
		mintPassNFTAddress = IERC721(0x9cfe4a98f5A4Cc2D38F737AbB14F23f6343C0eA5);
	}
    ...
    function getSenderFirstMintPass(address _owner) public view returns (uint256) {
         uint counter = 0;
         uint tokenId;
        for (uint i = 0; i < 500; i++) {
            if (mintPassNFTAddress.ownerOf(i) == _owner) {
                tokenId = i;
                counter++;
                break;
            }
    ...
    function mint(uint256 qty) external payable {
        ...
        require(
			mintPassNFTAddress.balanceOf(msg.sender) > 0,
			"you have to own this cat with this id"
		);
        uint mintPassToken = getSenderFirstMintPass(msg.sender);
        for (uint256 i = 0; i < qty; i++) {
            uint256 tokenId = nonce;
            _safeMint(msg.sender, tokenId);
            mintPassNFTAddress.safeTransferFrom(msg.sender, address(0), mintPassToken);
            // mintPassNFTAddress._clearApproval(owner, tokenId);
            // mintPassNFTAddress._removeTokenFrom(owner, tokenId);
            nonce++;
        }
    }
