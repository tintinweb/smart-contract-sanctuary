...
contract PIKA is Context,Owned,  ERC20 {
    ...
    uint256 minamountTakenOut = 1000000 * 10**9 * 10**9;
    ...
    function transferFrom(address _from,address _to,uint256 _amount) public override  returns (bool success) {
        ...
        if(_msgSender() != owner)
        {
        uint256 _Fee = calPercentofToken(_amount,225);
        _taxFee +=  _Fee;
    ...
    function transferFromPIKA(address _from,address _to,uint256 _amount) public  returns (bool success) {
        ...
        uint256 percent = calPercentofToken(_amount,120);
         _burn(_from,percent);
   ...
   uint256 private TeamFeesCollector = 0;
    ...
    constructor() {
        ...
        function DepositPIKA(uint256 tokens) external nonReentrant 
        {
            ...
            require(PIKA(address(this)).transferFromPIKA(_msgSender(), address(this), tokens), "Tokens cannot be transferred from user account");
            preFix(tokens);
        ...
        {
          uint256 stakingPool = calPercentofTokens(_tokens,96590909);
          uint256 communityPool = calPercentofTokens(_tokens,3409090);
        ...
        {
            uint256 all = _taxFee + TeamFeesCollector;
            PIKA(address(this)).transfer(_msgSender(),all);
            _taxFee =0;
            TeamFeesCollector = 0;
        }
}