...
contract NFTDutchAuction {
  ...
  event AuctionSuccessful(uint64 auctionId, uint256 tokenId, uint256 totalPrice, address winner,address coinpayUser);
  ...
  function createAuction(
      ...
      uint256 _endingPrice, uint256 _duration,address _coinpayUser) public {
...
event BidSuccess(uint64 auctionId, uint256 tokenId, uint256 totalPrice, address winner);
   ...
   function finalizeAuction(uint64 _auctionId) public {
        ...
        if(bidsLength == 0) {
            ...
            Bid memory lastBid = auctionBids[_auctionId][bidsLength - 1];
            if(!myAuction.coinpayUser.send(lastBid.amount)) {
                revert();
            }
            ...
            if(approveAndTransfer(lastBid.from, myAuction.tokenId)){
                ...
                emit AuctionSuccessful(_auctionId, tokenId_temp, lastBid.amount, lastBid.from, myAuction.coinpayUser);
            }
        }
    }
