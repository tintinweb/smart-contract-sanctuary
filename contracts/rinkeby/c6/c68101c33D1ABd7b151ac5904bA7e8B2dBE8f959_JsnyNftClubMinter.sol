...
// whitelist presale at 0.8 eth
// Doge Pound WL: 3000
// JRNY private WL: 1400
// total 4400 presale available 


// public dutch auction
//     Starting price: 3 ETH
//      minimum price: 0.8 ETH
// price decline over 8 hours, and then remains at 0.8 for 16 hours. rest is reserved


    constructor(address _nftAddress) {
        MAX_SUPPLY = 10000;
        startingPrice = 3000000000000000000; // 3 ETH
        ending_price = 800000000000000000; // 0.8 ETH
        nftAddress = _nftAddress;
        public_minting = false;


        decrease_interval = 600;
        decrease_per_interval = 45840000000000000;
        counter = 0;
    }

    function getCurrentPrice() public view returns(uint256) {
        uint256 passed_intervals = (block.timestamp.sub(starting_block)).div(decrease_interval);
        if (passed_intervals >= 48) {
            return ending_price;
        } else {
    ...
    function ownerMint(address _toAddress, uint256 amount) external onlyOwner {
        require(counter + amount <= 500, "Purchase would exceed max supply for owner");
        JrnyNftClub jrnyClub = JrnyNftClub(nftAddress);
