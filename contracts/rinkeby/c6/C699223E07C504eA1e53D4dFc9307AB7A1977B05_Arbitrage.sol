...
contract Arbitrage is Ownable {
    ...
    )    
  {
	...
	require(_token != address(0), "Arbitrage: token address must not be zero");
  ...
  function increaseAllowance(uint256 value) public onlyOwner {
	  IERC20(token).safeIncreaseAllowance(swapRouter, value);
  }
  
  function decreaseAllowance(uint256 value) public onlyOwner {
	  IERC20(token).safeDecreaseAllowance(swapRouter, value);
  }
  
  function buy(uint amountOutMin, address[] calldata path, uint deadline) external payable onlyAgent returns (uint[] memory amounts) {	  
	  return IUniswapV2Router01(swapRouter).swapExactETHForTokens(amountOutMin, path, address(this), deadline);  	  
  }
  ...
  function refundToken(address _token) public onlyOwner {
    uint256 balance = IERC20(_token).balanceOf(address(this));
    IERC20(_token).safeTransfer(owner(), balance);    
  }
  
  function refundBaseToken() public onlyOwner {
