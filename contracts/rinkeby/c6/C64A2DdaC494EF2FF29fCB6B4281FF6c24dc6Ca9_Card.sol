...
contract Card is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private rareCreature = ["Berserker", "Skin-Walker", "Griffin", "Fairy", "Minotaur", "Hydra"];

    string[] private legendaryCreature = ["Orge", "Giant", "Kraken", "Cyclops", "Oni"];
    ...
    string private _tokenBaseURI = "https://cardyourownadventure.com/";
    ...
    function getCardTitle(uint256 tokenId, uint256 offset) public view returns (string memory) {
        ...
        if (cardRand % 10 < 5) {
            ...
            return pluckSpell(tokenId, "ARTIFACTS", artifactNames, offset, cardQuality);
        } else if (cardRand % 10 < 8) {
            return pluckSpell(tokenId, "SPELL", spellNames, offset, cardQuality);
        } else {
            return pluckSpell(tokenId, "ENCHANTMENT", enchantmentNames, offset, cardQuality);
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        output = string(
            abi.encodePacked(
                output,
                "<text x='43' y='290' class='appen'>and 35 additional cards: ",
                toString(creatureCount),
                " monsters, ",
                toString(enchantmentCount),
                " enchantments,</text><text x='43' y='303' class='appen'>",
                toString(spellCount),
                " spells, ",
        ...
        output = string(
            abi.encodePacked(
                '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 935"> <defs> <style>@import url("https://fonts.googleapis.com/css?family=Arvo:400,400i,700,700i"); </style> </defs> <style>.base{fill: white; font-family: Arvo, serif; font-size: 12px;}</style> <rect width="100%" height="100%" fill="black"/> <rect width="100%" height="100%" fill="#fe2761" rx="15"/> <rect x="10" y="10" width="330" height="915" fill="black" rx="10"/>',
                output,
                "</svg>"
            )
        );
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Metaverse Decks #',
                        toString(tokenId),
                        '", "description": "Decks are collectable card game based on crypto primatives. The cards can be unbundled and swapped for better games.", "image": "data:image/svg+xml;base64,',
    ...
    uint256 public totalPublicIssued = 0;
    ...
    function mintPublic() public {
        ...
        require(totalPublicIssued < 7778, "There are no more NFTs for public minting.");
        require(totalPublicIssued < temporaryPublicMax, "There are no more NFTs for public minting at this time.");

        _mintedList[msg.sender] += 1;

        uint256 tokenId = totalPublicIssued + 1;
    ...
    function setTemporaryPublicMax(uint256 _temporaryPublicMax) public onlyOwner {
        require(_temporaryPublicMax <= MAX_MINT, "You cannot set the temporary max above the absolute total.");
