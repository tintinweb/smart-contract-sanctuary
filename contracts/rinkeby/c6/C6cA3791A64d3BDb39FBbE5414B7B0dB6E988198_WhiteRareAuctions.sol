...
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
   ...
   function getAuctionCurrentNumber() public view returns (uint256) {
    ...
    /**
     * @notice Create a fixed bid on a token, with a given amount.
     * @dev If provided a valid bid, transfers the provided amount to this contract.
     * If the auction is run in native ETH, the ETH is wrapped so it can be identically to other
     * auction currencies in this contract.
     */
    function _handleFixedBid(uint256 auctionId, uint256 amount) internal {
        // marker = msg.sender;
        address payable lastBidder = auctions[auctionId].bidder;
    ...
    // function to purchase an artwork in fixedSale
    function flatSale(uint256 auctionId, uint256 amount) external payable auctionExists(auctionId){
        require(auctions[auctionId].approved, "Auction must be approved by curator");
        require(
            amount == auctions[auctionId].reservePrice,
        ...
        _handleFixedBid(auctionId, amount);
    ...
    // function to purchase an artwork in fixedSale
    function flatSaleTest(uint256 auctionId, uint256 amount) external payable auctionExists(auctionId){
    ...
    // function to purchase an artwork in fixedSale
    function flatSaleTestTest(uint256 auctionId, uint256 amount) external payable auctionExists(auctionId){
        _handleFixedBid(auctionId, amount);
        _handleFixedSale(msg.sender, amount, auctions[auctionId].auctionCurrency);
        endAuctionMinimal(auctionId, msg.sender);
        delete auctions[auctionId];
