...
contract Thirsty_Chickens is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    string public baseExtension = ".json";
     ...
     //  This structure allows 200 giveaway mints which are consumed by the Public Sale
    uint public cost = 0.06 ether;

    uint public maxGiveawaySupply = 200;
    uint public maxPrioritySupply = 7500;
    uint public maxPublicSupply = 2375;
    uint public maxReserveSupply = 25;
    uint public reserveMinted = 0;
    uint public priorityMaxMintAmount = 2;
    ...
    function reserveMint() external payable onlyOwner {
    ...
    function giveawayMint() external payable nonReentrant() {
    ...
    function priorityMint(address _to, uint _priorityMintAmount) external payable nonReentrant() {
        ...
        for (uint i = 1; i <= _priorityMintAmount; i++){
            _safeMint(_to, prioritySupply + i);
        }
    }

    function mint(address _to, uint _mintAmount) external payable nonReentrant() {
    ...
    // Contract Owner Specific Functions
    ...
    function giveawayPause(bool _switch) public onlyOwner {
        giveawayPaused = _switch;
    }
    
    function priorityMintPause(bool _state) public onlyOwner {
        priorityMintPaused = _state;
    }

    function publicMintPause(bool _flag) public onlyOwner {
    ...
    function initGiveawayWhitelistWallets(address[] memory walletList) external onlyOwner{
        require(giveawayMapLength + walletList.length <= maxGiveawaySupply);
