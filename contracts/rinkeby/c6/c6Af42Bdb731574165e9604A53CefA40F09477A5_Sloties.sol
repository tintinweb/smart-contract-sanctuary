...
contract Sloties is AccessControl, Ownable, ERC721URIStorage {
    using SafeMath for uint256;
    using Strings for uint256;

    /** ADDRESSES */
    address public openseaProxyRegistryAddress;
    address public slotiesSales;
    ...
    /** NFT DATA */
    string public baseURIString = "https://gateway.pinata.cloud/ipfs/QmdDe8KAZsdCjkSXa5AyXxhkcJGHQ5yidm8rX8nxYbB2uo/";
    string public preRevealBaseURIString = "https://gateway.pinata.cloud/ipfs/QmTEe7YYDvtH8TpVCGsd3Fsx9YUC7Smv7h715newoND7J5";
    uint256 public nextTokenId = 0;
    uint256 public revealDate = 1638903600 + 86400 * 2; //1638903600 + 86400 * 2 9 dec (1 day after public sale which is 8 dec)
    ...
    event SetSlotiesSales(address _slotiesSales);
    ...
    function setSlotiesSales(address _slotiesSales) external onlyRole(DEFAULT_ADMIN_ROLE) {
        slotiesSales = _slotiesSales;
        emit SetSlotiesSales(_slotiesSales);
    }
    ...
    function setBreedingContract(address _breedingContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
        breedingContract = _breedingContract;
        emit SetBreedingContract(_breedingContract);
    }

    function setWATTSContract(address _WATTS) external onlyRole(DEFAULT_ADMIN_ROLE) {
        WATTS = _WATTS;
        emit SetWATTS(_WATTS);
    }

    function setLotteryContract(address _lotteryContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
        lotteryContract = _lotteryContract;
        emit SetLotteryContract(_lotteryContract);
    }

    function setPayoutContract(address _payoutContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    * @dev function to change the baseURI of the metadata
    */
    function setRevealDate(uint256 _revealDate) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    */
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (block.timestamp >= revealDate) {
            ...
            return preRevealBaseURIString; //string(abi.encodePacked(preRevealBaseURIString, tokenId.toString()));
        }        
    }
