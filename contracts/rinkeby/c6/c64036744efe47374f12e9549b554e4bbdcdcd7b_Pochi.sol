...
contract Pochi is ReentrancyGuard, Ownable, ERC165, IERC721Enumerable, IERC721Metadata {
    ...
    uint256 private constant HALF_HOUR = 1800;
    uint256 private constant HOUR = 3600;
    uint256 private constant DAY = 24 * HOUR;

    uint256 public constant MAX_NFT_SUPPLY = 1234;
    uint256 public constant MAX_EARLY_ACCESS_SUPPLY = 400;

    // A hash of all tokenHash and the generative scripts. This will be set after the collection is fully minted
    string public PROVENANCE = "";

    uint256 public SALE_START_TIMESTAMP = 1630508400;
    uint256 public EARLY_ACCESS_START_TIMESTAMP = SALE_START_TIMESTAMP - HOUR;

    // uint256 public EARLY_ACCESS_START_TIMESTAMP = 1629090000;
    // uint256 public SALE_START_TIMESTAMP = EARLY_ACCESS_START_TIMESTAMP + 1 * DAY;
    // The unit here is per half hour. Price decreases in a step function every half hour
    uint256 public SALE_DURATION = 12;
    uint256 public SALE_DURATION_SEC_PER_STEP = HALF_HOUR;
    ...
    /**
     * @dev Mint one token and refund any excessive amount of ETH sent in
     */
    function mintAndRefundExcess() external payable nonReentrant {
        // Checks
        require(!contractSealed, "Contract sealed");
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started");
        require(totalSupply() + 1 <= MAX_NFT_SUPPLY, "Sold out");

        uint256 price = getNFTPrice();
        require(msg.value >= price, "Ether value sent is insufficient");

        // Effects
        _safeMintWithHash(msg.sender, totalSupply());
    ...
    /**
     * @dev Mint one early access token
     */
    function mintEarlyAccess() external payable nonReentrant {
        // Checks
        require(!contractSealed, "Contract sealed");
        ...
        require(earlyAccessList[msg.sender], "Sender is not on the list");
        require(totalSupply() + 1 <= MAX_NFT_SUPPLY, "Sold out");
        require(earlyAccessMinted + 1 <= MAX_EARLY_ACCESS_SUPPLY, "No more early access tokens left");
        require(msg.value == EARLY_ACCESS_MINT_FEE, "Ether value sent is incorrect");

        // Effects
        earlyAccessList[msg.sender] = false;
        earlyAccessMinted = earlyAccessMinted + 1;
        _safeMintWithHash(msg.sender, totalSupply());
    }
    ...
    /**
     * @dev Safely mint one token, and assign tokenHash to the new token
     *
     * Emits a {NewTokenHash} event.
     */
    function _safeMintWithHash(address to, uint256 tokenId) internal virtual {
        bytes32 hash = keccak256(abi.encodePacked(tokenId, block.number, block.coinbase, block.timestamp, blockhash(block.number - 1), msg.sender));
     ...
     */
    function deployerAddEarlyAccess(address[] calldata recipients) external onlyDeployer {
        require(!contractSealed, "Contract sealed");

        for (uint256 i = 0; i < recipients.length; i++) {
            require(recipients[i] != address(0), "Can't add the null address");

            earlyAccessList[recipients[i]] = true;
        }
    }
    ...
    function deployerRemoveEarlyAccess(address[] calldata recipients) external onlyDeployer {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            earlyAccessList[recipients[i]] = false;
        }
    }
     ...
     * @dev Reserve tokens
     */
    function deployerMint(uint256 numberOfNfts, address recipient) external payable onlyDeployer {
        require(!contractSealed, "Contract sealed");
        ...
        for (uint256 i = 0; i < numberOfNfts; i++) {
            _safeMintWithHash(recipient, totalSupply());
        }
    }

    /**
     * @dev Air drop tokens to craft ham holders
     */
    function deployerMintMultiple(address[] calldata recipients) external payable onlyDeployer {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            _safeMintWithHash(recipients[i], totalSupply());
        }
    }
