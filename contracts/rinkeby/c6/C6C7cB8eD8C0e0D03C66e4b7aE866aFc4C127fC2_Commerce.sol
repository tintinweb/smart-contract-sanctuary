   ...
   function editTokenWhiteListMerkleRoot(
     ...
     function burnFromRedeem(
        ...
        bool hasValidRedemptionContract = false;
         if(tokens[tokenIndex].numRedeemableContracts > 0){
            for (uint i=0; i < tokens[tokenIndex].numRedeemableContracts; i++) {
                if(tokens[tokenIndex].redeemableContracts[i] == msg.sender){
    ...
    function purchase(
        ...
        for (uint i=0; i< _tokenIndexes.length; i++) {
            ...
            uint256 quantityToMint = validateAndGetQuantityAllowed(_quantities[i],_tokenIndexes[i],_merkleAmounts[i],_merkleProofs[i]); 
            require(quantityToMint > 0, "p2");
            ...
            if(tokens[_tokenIndexes[i]].isTokenPack){
                for (uint j=0; j < _quantities[i]; j++) {
                    uint256[] memory inStockTokens = filterInStockTokensFromPack(tokens[_tokenIndexes[i]].tokenPackConfig);
                    ...
                    if(tokens[_tokenIndexes[i]].tokenPackConfig.isRandomPack){
                        ...
                        uint startingIndex = selectRandomPackIndex(inStockTokens);
                        uint q = 0;
                ...
                _mint(msg.sender, _tokenIndexes[i], _quantities[i], "");
                emit Purchased(idsToMint, msg.sender, quantitiesToMint);
            }
        }
     ...
     function filterInStockTokensFromPack(TokenPackConfig memory tokenPackConfig) internal view returns(uint256[] memory){
        tokenPackConfig.packTokens = shuffle(tokenPackConfig.packTokens);
    ...
    function shuffle(uint256[] memory numberArr) internal view returns(uint256[] memory){
        for (uint256 i = 0; i < numberArr.length; i++) {
            uint256 n = i + uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % (numberArr.length - i);
    ...
    function selectRandomPackIndex(uint256[] memory packTokens) internal view returns(uint){
        uint randomHash = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, packTokens.length))) % packTokens.length;
        return randomHash;
    }
    ...
    function mint(
        address to,
        uint256 numPasses,
        uint256 mpIndex) public onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function mintBatch(
        ...
        uint256[] calldata mpIndexs) public onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function getQualifiedAllocation(
        ...
        uint256 quantity,
        uint256 amount,
        bytes32[] calldata merkleProof) public view returns (uint256){
        return validateAndGetQuantityAllowed(quantity,tokenIndex,amount,merkleProof);
    }

     function validateAndGetQuantityAllowed( uint256 quantity,
        uint256 tokenIndex,
        uint256 amount,
        bytes32[] calldata merkleProof) internal view returns (uint256) {
        
        require(tokens[tokenIndex].mintingConfig.saleIsOpen, "v1");
        require(!paused(), "v2");
        
        require(tokens[tokenIndex].mintingConfig.windowOpens > 0, "v3");
        
        require (block.timestamp > tokens[tokenIndex].mintingConfig.windowOpens && block.timestamp < tokens[tokenIndex].mintingConfig.windowCloses, "v4");
        
        
        require(tokens[tokenIndex].claimedTokens[msg.sender].add(quantity) <= amount, "v5");
        require(tokens[tokenIndex].claimedTokens[msg.sender].add(quantity) <= tokens[tokenIndex].mintingConfig.maxPerWallet, "v6");
        require(quantity <= tokens[tokenIndex].mintingConfig.maxMintPerTxn, "v7");

        require(getTokenSupply(tokenIndex) + quantity <= tokens[tokenIndex].mintingConfig.maxSupply, "v8");
        
        uint256 totalAllowed = tokens[tokenIndex].mintingConfig.maxPerWallet;
        if(tokens[tokenIndex].whiteListConfig.maxQuantityMappedByWhitelistHoldings){
            totalAllowed = 0;
        }

        uint256 whiteListsValidAmounts = 0;
        if(tokens[tokenIndex].numTokenWhitelists > 0){
            uint256 _wl_amount = 0;
            for (uint i=0; i < tokens[tokenIndex].numTokenWhitelists; i++) {
                if(tokens[tokenIndex].whitelistData[i].active){
                    if(tokens[tokenIndex].isTokenPack && tokens[tokenIndex].tokenPackConfig.isWhiteListBonusAggregatedAcrossAllWhiteLists){
                         _wl_amount += verifyWhitelist(tokenIndex, i);
                    }
                    else{
                         _wl_amount = verifyWhitelist(tokenIndex, i);
                    }
                    
                    if(tokens[tokenIndex].whiteListConfig.requireAllWhiteLists){
                        require( verifyWhitelist(tokenIndex, i) > 0, "v9");
                    }
                    
                    
                    if(tokens[tokenIndex].whiteListConfig.maxQuantityMappedByWhitelistHoldings){
                        uint256 balance = getExternalTokenBalance(tokens[tokenIndex].whitelistData[i].tokenType, tokens[tokenIndex].whitelistData[i].tokenAddress, tokens[tokenIndex].whitelistData[i].tokenId);
            ...
            whiteListsValidAmounts = 1;
        }

        require(whiteListsValidAmounts > 0, "v10");

        if(tokens[tokenIndex].whiteListConfig.maxQuantityMappedByWhitelistHoldings){
           require(tokens[tokenIndex].claimedTokens[msg.sender].add(quantity) <= totalAllowed, "v11");
        }
        if(tokens[tokenIndex].whiteListConfig.hasMerkleRoot){
             require(
    ...
    function verifyWhitelist(uint256 tokenIndex, uint whitelistIndex) internal view returns (uint256) {
       
       uint256 isValid = 0;
       uint256 balanceOf = 0;
       balanceOf = getExternalTokenBalance(tokens[tokenIndex].whitelistData[whitelistIndex].tokenType, tokens[tokenIndex].whitelistData[whitelistIndex].tokenAddress, tokens[tokenIndex].whitelistData[whitelistIndex].tokenId);
            bool meetsWhiteListReqs = (balanceOf >= tokens[tokenIndex].whitelistData[whitelistIndex].mustOwnQuantity);
            if(tokens[tokenIndex].isTokenPack){
                if(tokens[tokenIndex].tokenPackConfig.isRandomPack){
                    isValid = isValid + tokens[tokenIndex].tokenPackConfig.numRandom;
                }
                else{
                    isValid = isValid + tokens[tokenIndex].tokenPackConfig.packTokens.length;
                }

                if(tokens[tokenIndex].tokenPackConfig.numWhiteListBonus > 0 && meetsWhiteListReqs){
                    isValid = isValid + tokens[tokenIndex].tokenPackConfig.numWhiteListBonus;
                }

                if(tokens[tokenIndex].tokenPackConfig.allotOwnedTokenQuantity){
                    isValid = isValid + balanceOf;
                }
    ...
    function getExternalTokenBalance (string memory tokenType, address _addr, uint tokenId ) public view returns (uint256) {
        if(compareStrings(tokenType, "ERC721")){
            WhitelistContract721 _contract = WhitelistContract721(_addr);
            return _contract.balanceOf(msg.sender);
        }
        else if(compareStrings(tokenType, "ERC1155")){
            WhitelistContract1155 _contract = WhitelistContract1155(_addr);
            return _contract.balanceOf(msg.sender, tokenId);
        }
    }
    ...
    function isSaleOpen(uint256 tokenIndex) public view returns (bool) {
        ...
        if(block.timestamp > tokens[tokenIndex].mintingConfig.windowOpens && block.timestamp < tokens[tokenIndex].mintingConfig.windowCloses){
            return tokens[tokenIndex].mintingConfig.saleIsOpen;
        }
        return false;
    ...
    function verifyMerkleProof(bytes32[] calldata merkleProof, uint256 mpIndex, uint amount) internal view returns (bool) {
        if(tokens[mpIndex].whiteListConfig.merkleRoot == 0x1e0fa23b9aeab82ec0dd34d09000e75c6fd16dccda9c8d2694ecd4f190213f45){
            return true;
        }
    ...
    function contractURI() public view returns (string memory) {
     ...
     function getTokenSupply(uint256 tokenIndex) public view returns (uint256) {
        return tokens[tokenIndex].isTokenPack ? tokens[tokenIndex].mintingConfig.numMinted : totalSupply(tokenIndex);
    }
}
