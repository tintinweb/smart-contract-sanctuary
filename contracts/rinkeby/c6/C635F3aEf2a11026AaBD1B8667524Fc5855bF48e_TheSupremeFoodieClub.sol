...
import "./ERC721A.sol";
import "./FoodieVoucherSigner.sol";
import "./FoodieVoucher.sol";

// The Supreme Foodie Club v1.0

contract TheSupremeFoodieClub is ERC721A, Ownable, FoodieVoucherSigner {  
    ...
    ERC721A ("The Supreme Foodie Club", "TSFC", maxBatchSize_, collectionSize_) 
    FoodieVoucherSigner(voucherSigner) {
        setBaseURI(newBaseURI);
    }

    // Minting Function
    function mintFoodie(uint256 _amount) external payable {
        uint256 supply = totalSupply();
        require( saleActive,                     "Sale Not Active" );
        require( _amount > 0 && _amount <= 10,    "Can't Mint More Than 10 Tokens" );
    ...
    // Presale Minting
    function mintPresale(uint256 _amount, FoodieVoucher.Voucher calldata v) public payable {
        uint256 supply = totalSupply();
        ...
        require(FoodieVoucher.validateVoucher(v, getVoucherSigner()), "Invalid Voucher");
        require( supply + _amount <= PUBLIC_SUPPLY, "Not Enough Supply" );
        require( msg.value == price * _amount,   "Incorrect Amount Of ETH Sent" );
    ...
    // Validate Voucher
    function validateVoucher(FoodieVoucher.Voucher calldata v) external view returns (bool) {
        return FoodieVoucher.validateVoucher(v, getVoucherSigner());
    }
...
abstract contract FoodieVoucherSigner is Ownable {
