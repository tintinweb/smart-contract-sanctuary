...
contract EverPee is Context, IERC20, Ownable {
    ...
    string private _name = "Everpee";
    string private _symbol = "EVERPEE";
    ...
    event Min(uint256 minTransferAmountToBuyBack);
    event SwapRate(uint256 swapRate);
    event LowerBuyBack(uint256 lowerBuyBack);
    event SoldAmount(uint256 soldAmount);
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            ...
            if (buyBackEnabled && balance > uint256(1 * 10**18)) {
              ...
              //test
              emit SwapRate(tokenSwapRate);
              emit Min(minTransferAmountToBuyBack);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> EVERMOON swap when swap+liquify is triggered
