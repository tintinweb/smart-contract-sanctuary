...
contract Dispenser is Ownable {
    ...
    uint public constant GOLD_HUNTER_GGOLD_RATE = 10000 ether;
    uint public constant PIRATE_GGOLD_RATE = 20000 ether;
    ...
    function setGold(address _address) external onlyOwner {
        penny = IPenny(_address);
    }


    function claimGGold() external {
        // Get user stake
        ...
        for (uint i = 0; i < pirateStakes.length; i++) {
            ...
            claimedFromTokenId[pirateStakes[i].tokenId] = true; // Set as claimed one
        }
        ...
        for (uint i = 0; i < minerStakes.length; i++) {
            ...
            claimedFromTokenId[minerStakes[i].tokenId] = true; // Set as claimed one
        }
        if (owed == 0) return;
