...
contract SimpleCollectible is ERC721, Ownable {
    ...
    ERC721 public LITTLE_LEMON_FRIENDS = ERC721(0x0B22fE0a2995C5389AC093400e52471DCa8BB48a);

    address public charity;
    address public FE_DEV;
    address public BE_DEV;
    address public MORGAN;

    uint256 public _presalePrice = 100000000000000000; //.1 ETH
    uint256 public _salePrice = 150000000000000000; // .15 ETH

    uint256 public _maxPerTx = 124; // Set to one higher than actual, to save gas on <= checks.
    uint256 public _maxPerWallet = 124; // Set to one higher than actual, to save gas on <= checks.
    uint256 public _maxPerWalletInPresale = 6; // Set to one higher than actual, to save gas on <= checks.
    ...
    constructor (address _charity, address feDev, address beDev, address morgan) ERC721 ("The Awesome Apples","APPLE")  {
    ...
    function mintPresaleCollectibles(uint256 _count) public payable {
        ...
        require(minted[msg.sender] + _count < _maxPerWalletInPresale, "wallet limit reached");
        require(isWalletInFruitList(msg.sender), "Wallet isnt in presale! The owner needs to addWalletToPresale.");
	...
	function ownerMint(address _user, uint256 _count) public onlyOwner {
        require((_count + tokenCounter) <= _totalSupply, "Ran out of NFTs for sale! Sry!");
    ...
    function isWalletInFruitList(address _address) public view returns (bool) {
        return addressToPreSaleEntry[_address] || (LITTLE_LEMON_FRIENDS.balanceOf(_address) > 0);
    }
    ...
    function addWalletToFruitList(address _address) public onlyOwner {
    ...
    function setBEDev(address beDev) public onlyOwner {
        BE_DEV = beDev;
    }
    function setFEDev(address feDev) public onlyOwner {
        FE_DEV = feDev;
    }
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        require(FE_DEV != address(0), "Frontend dev not set");
        require(BE_DEV != address(0), "Backend dev not set");
        uint256 balance = address(this).balance;
        require(payable(charity).send(balance * 51 / 100));
        require(payable(FE_DEV).send(balance * 3 / 100));
        require(payable(BE_DEV).send(balance / 10));
