...
contract LCX_ISSUER {
    ...
    EVENTS
    ******************/
    event ABTCreated(address indexed tokenAddress, address indexed wallet);
    event ISCCreated(address indexed tokenAddress, address indexed wallet);
    ...
    ******************/
    function createToken(
        ...
        uint256 _degradationPeriod
    ) public payable {
        require(
            (keccak256(abi.encodePacked(_tokenType)) == keccak256(abi.encodePacked("ABT")) ||
                keccak256(abi.encodePacked(_tokenType)) == keccak256(abi.encodePacked("ISC")) ||
                keccak256(abi.encodePacked(_tokenType)) == keccak256(abi.encodePacked("CYC"))),
            "LCX_Issuer: Token type not supported"
        );
        LescovexERC20 token;
        if (keccak256(abi.encodePacked(_tokenType)) == keccak256(abi.encodePacked("ABT"))) {
            token = _createABTToken(
                _name,
                _tokenSymbol,
                _description,
                _initialSupply,
        ...
        } else if (keccak256(abi.encodePacked(_tokenType)) == keccak256(abi.encodePacked("ISC"))) {
            token = _createISCToken(
                _name,
                _tokenSymbol,
                _description,
                _initialSupply,
        ...
        } else if (keccak256(abi.encodePacked(_tokenType)) == keccak256(abi.encodePacked("CYC"))) {
            token = _createCYCToken(
                _name,
                _tokenSymbol,
                _description,
                _initialSupply,
    ...
    PRIVATE FUNCTIONS
    ******************/
    function _createABTToken(
        string memory _name,
        string memory _tokenSymbol,
        string memory _description,
        uint256 _initialSupply,
        uint256 _degradationPeriod
    ) private returns (Lescovex_ABT) {
        return new Lescovex_ABT(
                _initialSupply,
                _name,
                _tokenSymbol,
                msg.sender,
    ...
    function _createISCToken(
        string memory _name,
        string memory _tokenSymbol,
        string memory _description,
        uint256 _initialSupply,
        uint256 _degradationPeriod
    ) private returns (Lescovex_ISC) {
        return new Lescovex_ISC(_initialSupply, _name, _tokenSymbol, 0, msg.sender);
    }

    function _createCYCToken(
        string memory _name,
        string memory _tokenSymbol,
        string memory _description,
        uint256 _initialSupply,
        uint256 _degradationPeriod
    ) private returns (Lescovex_CYC) {
        return new Lescovex_CYC(_initialSupply, _name, _tokenSymbol, msg.sender);
    }
}