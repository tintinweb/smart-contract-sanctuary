...
contract VRFDP is VRFConsumerBase , AccessControl{
    ...
    bytes32 public constant NFT_ROLE = keccak256("NFT_CONTRACT_ROLE");

    uint fee = 0.1 * 10 ** 18 ;

    uint256 public randID;

    constructor(address _nftAddr)   VRFConsumerBase(VFRC_address, LINK_address)  {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRoleAdmin(NFT_ROLE, DEFAULT_ADMIN_ROLE);
        grantRole(NFT_ROLE,_nftAddr);
    }
    ...
    function shuffleRandom() public returns (bytes32 requestId) {
        require(hasRole(NFT_ROLE, msg.sender));
        require(LINK.balanceOf(address(this)) > fee, "Error, not enough LINK - fill contract with faucet");
        return requestRandomness(keyHash, fee);
    }

    function getRandomNumber(uint256 _tokenId,address recipient, uint256 seed) public view returns(uint256 _randomNumber){
       require(hasRole(NFT_ROLE, msg.sender));
       require(tx.origin == recipient, "Only EOA");
    ...
    function configNFT(address _addrNFT) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
         grantRole(NFT_ROLE, _addrNFT);
    }
