...
contract PlantValue is ERC721Pausable, AccessControl, Ownable, IPlantValue {
    ...
    function mint(address _to, uint256 _createPoolPlantAmount) external onlyOwner {
        uint256 supply = totalSupply();
        uint256 tokenId = nextTokenId;
        uint256 index = _randomPlantId();
        uint256 plantId = rangeOfId[index];
        require(rangeOfId.length > 0, "INSUFFFICIENT PLANT ID");
        require(_createPoolPlantAmount > 0 );
        require(_createPoolPlantAmount <= maxMintAmount);
        require(supply + _createPoolPlantAmount <= maxSupply);

        if (msg.sender != owner()){
        }
        for (uint256 i = 1; i <= 400; i++) {
            _mintPlant(_to, tokenId, supply + i);
        }
