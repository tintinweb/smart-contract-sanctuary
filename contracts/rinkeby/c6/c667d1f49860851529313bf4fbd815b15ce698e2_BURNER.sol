...
contract BURNER is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
	...
	function layerURIByCoordinates(uint layer, uint index) public view returns (string memory) {
		uint i = (7 * (layer - 1)) + index;
		return _layersURI[i];
	}

	function layerURICSV() public view returns (string memory) {
		bytes memory temp;
		uint i;

		for(i = 0; i < 62; i++) {
			temp = abi.encodePacked(temp, _layersURI[i]);
			temp = abi.encodePacked(temp, ',');
		}

		temp = abi.encodePacked(temp, _layersURI[62]);
	...
	function setLayerImageURIPerCoordinates(uint layer, uint index, string memory uri) public {
		require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Not authorized!");
		uint i = (7 * (layer - 1)) + index;
		_layersURI[i] = uri;
	}
