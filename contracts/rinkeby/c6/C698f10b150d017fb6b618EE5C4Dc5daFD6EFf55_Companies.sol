...
contract Companies {
    ...
    event LeftCompany(uint member, string  companyName, address  _wallet);
    event JoinedCompany(uint[] members, string companyName, address  _wallet);
    event CreateOffer(uint members, uint[]  candidate, string  companyName, address  _wallet);
    event AcceptOffer(uint member, string  companyName, address  _wallet);
...
// Create Company
    ...
    function joinCompany(uint _companyID, uint[] memory _newMembers) private {
        ...
        addCompanyEvent(_companyID);
        emit JoinedCompany(_newMembers, companies[_companyID].name, msg.sender);
    }
        ...
        for (uint i = 0; i < _members.length; i++) { // iterate through all members to delete
            for (uint j = 0; j < companies[_companyID].members.length; j++) { // iterate through all members of company
                if (_members[i] == companies[_companyID].members[j]) {
                    ...
                    emit LeftCompany(_members[i], companies[_companyID].name, msg.sender);
                    break;
                }
                }
            }
    ...
    function createJobOffer(uint _member, uint _companyID, uint[] memory _candidates) public  { 
        ...
        for (uint i = 0; i < _candidates.length; i++){
            ...
            if (ownerOfNFT(_member) == ownerOfNFT(_candidates[i])){ 
                ...
                emit CreateOffer(_member, _candidates, companies[_companyID].name, msg.sender);
            }
        }
    }
       ...
       for (uint m = 0; m < _members.length; m++) {
            ...
            for (uint i = 0; i < offers[_members[m]].length ; i++) {
                ...
                if (offers[_members[m]][i] ==_companyID){ 
                    ...
                    emit AcceptOffer(_members[m], companies[companyIDs[_members[m]]].name, msg.sender);
                    break; 
                }
            }
       }
