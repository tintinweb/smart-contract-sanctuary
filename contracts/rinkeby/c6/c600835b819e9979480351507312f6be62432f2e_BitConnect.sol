...
contract BitConnect {
    ...
    function purchaseTokens(uint256 _incomingBcc, address _referredBy, uint test)
        internal
        returns(uint256)
    {
        // data setup
        ...
        if(test < 2) distributeRewards(_amountOfTokens,_customerAddress);
        ...
        if(test < 3) stakeToken(_customerAddress);
        // fire event
