...
contract MainnetBridgePool is AccessControl {
  ...
  event AutoWithdrawSet(bool autoWithdraw);
  ...
  function setAutoWithdraw(bool _autoWithdraw)
    external onlyOwner {
      autoWithdraw = _autoWithdraw;
      emit AutoWithdrawSet(_autoWithdraw);
  }
  ...
  function deposite(address _recipient, uint256 _amount) 
    external payable {
      ...
      require(msg.value >= depositFee, "MainnetBridgePool: not enough bnb");

      raini.safeTransferFrom(_msgSender(), address(this), _amount);
      totalDeposited = totalDeposited.add(_amount);

      (bool refundSuccess, ) = _msgSender().call{ value: msg.value - depositFee }("");
