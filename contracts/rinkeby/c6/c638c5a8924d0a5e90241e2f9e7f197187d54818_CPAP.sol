...
contract CPAP is Context, IERC20, Ownable {
    ...
    address private _CPAPCharityWallet = 0x4bCa8e532F351f7Eb21554406fA30b57FE6D08D0;
    ...
    function _takeCharity(uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[_CPAPCharityWallet] = _rOwned[_CPAPCharityWallet].add(rCharity);
        if(_isExcluded[_CPAPCharityWallet])
            _tOwned[_CPAPCharityWallet] = _tOwned[_CPAPCharityWallet].add(tCharity);
    }
