...
contract ProtoPixels is ERC721Enumerable, Ownable {
    ...
    uint256 private pixelsReserved = 100;
    uint256 private pixelPrice = 0.0169 ether;
    ...
    function mintPixel(uint256 pixelQuantity) public payable {
        ...
        require( supply + pixelQuantity < maxPixels - pixelsReserved,"Exceeds maximum supply" );
    ...
    function reservePixels(address _to, uint256 pixelAmount) external onlyOwner() {
        require( pixelAmount <= pixelsReserved, "Too many!" );

        uint256 supply = totalSupply();
        for(uint256 i; i < pixelAmount; i++){
            _safeMint( _to, supply + i );
        }

        pixelsReserved -= pixelAmount;
    }
