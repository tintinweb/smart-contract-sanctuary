...
contract RhoTokenRewards is IRhoTokenRewards, BaseRewards {
    ...
    function lastBlockApplicable(address rhoToken) internal view isSupportedRhoToken(rhoToken) returns (uint256) {
    ...
    function updateRewardInternal(address rhoToken) internal isSupportedRhoToken(rhoToken) {
        RhoTokenInfo storage _rhoToken = rhoTokenInfo[rhoToken];
    ...
    function getRewardsEndBlock(address rhoToken)
        external
        view
        override
        isSupportedRhoToken(rhoToken)
    ...
    function endRewards(address rhoToken) external override onlyRole(DEFAULT_ADMIN_ROLE) isSupportedRhoToken(rhoToken) {
        RhoTokenInfo storage _rhoToken = rhoTokenInfo[rhoToken];
    ...
    function claimRewardInternal(address user, address rhoToken) internal nonReentrant isSupportedRhoToken(rhoToken) {
        updateReward(user, rhoToken);
        UserInfo storage _user = userInfo[rhoToken][user];
        if (_user.reward > 0) {
            _user.reward = flurryStakingRewards.claimRhoTokenReward(user, _user.reward);
        }
    }
