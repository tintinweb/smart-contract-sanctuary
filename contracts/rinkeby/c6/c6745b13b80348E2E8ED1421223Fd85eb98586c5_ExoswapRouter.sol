...
contract ExoswapRouter is IUniswapV2Router02, Initializable, ContextUpgradeSafe, OwnableUpgradeSafe {
    using SafeMath for uint;
    ...
    function __ExoswapRouter_init(address _factory, address _WETH) public initializer {
        __ExoswapRouter_init_unchained(_factory, _WETH);
    }

    function __ExoswapRouter_init_unchained(address _factory, address _WETH) internal initializer {
        __Ownable_init_unchained();
        factory = _factory;
        WETH = _WETH;
    }
    ...
    function addLiquidity(
        ...
        (amountADesired, amountBDesired) = convert(tokenA, tokenB, amountADesired, amountBDesired);
    ...
    function addLiquidityETH(
        ...
        (amountTokenDesired, amountEthDesired) = convert(token, WETH, amountTokenDesired, amountEthDesired);
        ...
        if (amountEthDesired > amountETH) {
            IWETH(WETH).withdraw(amountEthDesired - amountETH);
            TransferHelper.safeTransferETH(msg.sender, amountEthDesired - amountETH);
        }
    ...
    function convert(
        ...
        if ((amountADesired > 0 && amountBDesired > 0) || (amountADesired <= 0 && amountBDesired <= 0) 
            || address(pair) == address(0) || reserve0 == 0 || reserve1 == 0) {
            return (amountADesired, amountBDesired);
        } else if (amountADesired > 0) {
            token = tokenA;
            amountIn = amountADesired.div(2);
            amountA = amountADesired.sub(amountIn);
        } else if (amountBDesired > 0) {
            token = tokenB;
            amountIn = amountBDesired.div(2);
            amountB = amountBDesired.sub(amountIn);
        }
        ...
        TransferHelper.safeTransferFrom(token, address(this), address(pair), amountIn);
        pair.swap(amount0Out, amount1Out, address(this), new bytes(0));
        (amountA, amountB) = amountA == 0 ? (amountOut, amountB) : (amountA, amountOut);
    }
