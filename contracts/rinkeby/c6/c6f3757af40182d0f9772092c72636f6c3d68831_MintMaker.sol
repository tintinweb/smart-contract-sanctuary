...
interface IUniswapV2Factory {
    ...
    function makerFeeTo() external view returns (address);
    function deflationFeeTo() external view returns (address);
    ...
    function setMakerFeeTo(address) external;
    function setDeflationFeeTo(address) external;
...
// 灏嗘墜缁垂杞Щ鍒板钩鍙板垎绾ar鍚堢害 

contract MintMaker is Ownable {
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != mts && token != weth && token != bridge,
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "MintMaker: must use EOA");
        _;
    }

    // F1 - F10: OK
    // F3: _convert is separate to save gas by only checking the 'onlyEOA' modifier once in case of convertMultiple
    // F6: There is an exploit to add lots of MTS to the bar, run convert, then remove the MTS again.
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == mts) {
                IERC20(mts).safeTransfer(bar, amount);
                mtsOut = amount;
            } else if (token0 == weth) {
                mtsOut = _toMTS(weth, amount);
            } else {
            ...
            // eg. MTS - ETH
            IERC20(mts).safeTransfer(bar, amount0);
            mtsOut = _toMTS(token1, amount1).add(amount0);
        } else if (token1 == mts) {
            // eg. USDT - MTS
            IERC20(mts).safeTransfer(bar, amount1);
    ...
    {
        // X1 - X5: OK
        amountOut = _swap(token, mts, amountIn, bar);
    }
}