...
/**
 * @title BillboardClub contract.
 * @author The BillboardClub team.
 *
...
contract CryptoBBC is ERC721, Ownable {
    ...
    event SetBillboardToken(uint256 tokenId);
    ...
    uint256 public constant AUCTION_PRICE_START = 0.005 ether;
    uint256 public constant AUCTION_PRICE_END = 0.001 ether;
    ...
    /// @notice BillboardClub token to linked metadata mapping.
    mapping (uint256 => Metadata) internal _tokenMap;
    function tokenMap(uint256 tokenId) public view virtual  returns (Metadata memory) {
        require(_exists(tokenId), "CryptoBBC: TokenMap query for nonexistent token");
        return _tokenMap[tokenId];
    }
     ...
     */
    function _mintInner(uint256 numToMint, MintEntry[] memory _entries, bool isPresale, uint256 voucherAmount) internal {
        uint256 startingSupply = _totalSupply;
        ...
        for (uint256 i = 0; i < numToMint; i++) {
            ...
            setBillboardToken(startingSupply + i, _entries[i].adContractAddress, _entries[i].adTokenId);
        }
    ...
    function mintPresale(
        ...
        _mintInner(numToMint, _entries, true, remainingVoucherAmount);
    }

    /**
     * @notice Called by users to mint from the main sale.
     */
    function mint(uint256 numToMint, MintEntry[] memory _entries) external payable {
        require(
            _saleIsActive,
            "Sale not active"
        );
        ...
        // Mint tokens, checking for sufficient payment and supply.
        _mintInner(numToMint, _entries, false, 0);
    }
    ...
    function tokensOfOwner(address _owner, uint startId, uint endId) external view returns(uint256[] memory ) {
      ...
      if (tokensOwned == 0) {
        ...
        uint256[] memory result = new uint256[](tokensOwned);
        uint256 index = 0;
        for (uint256 tokenId = startId; tokenId < endId; tokenId++) {
            if (index == tokensOwned) break;
    ...
    function walletOfOwner(address _owner) external view returns(uint256[] memory ) {
      return this.tokensOfOwner(_owner, 0, _totalSupply);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return _tokenMap[tokenId].uri;
    }

    function linkedTokenAddress(uint256 tokenId) public view virtual returns (address) {
        require(_exists(tokenId), "CryptoBBC: Flex TokenAddress query for nonexistent token");
        return _tokenMap[tokenId].contractAddress;
    }

    function linkedTokenId(uint256 tokenId) public view virtual returns (uint256) {
        require(_exists(tokenId), "CryptoBBC: Flex TokenId query for nonexistent token");
        return _tokenMap[tokenId].tokenId;
    }
    ...
    function setBillboardToken(uint256 _tokenId,
        ...
        _tokenMap[_tokenId].uri = _retrieveAdURI(_adContractAddress, _adTokenId);
        _tokenMap[_tokenId].tokenId = _adTokenId;
        _tokenMap[_tokenId].contractAddress = _adContractAddress;
        emit SetBillboardToken(_tokenId);
    }
...
/**
 *  By interacting with this contract you agree that CryptoBBC will not be held responsible for your actions 
 */
