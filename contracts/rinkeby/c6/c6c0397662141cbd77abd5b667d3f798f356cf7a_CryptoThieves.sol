    ...
    Ownable
{
    ...
    uint256 public trxLimit = 10;
    bool public SALE_STATE = false;
    address public ERC20_CONTRACT;
    mapping(address => uint256) private _minted;
    string private _baseURIextended;

    constructor(string memory baseTokenURI) ERC721('Crypto Thieves', 'CT') {
    ...
    function minted(address owner) external view returns (uint256) {
        require(owner != address(0), "ERC721: balance query for the zero address");
    ...
    function reserveTokens(uint numberOfTokens) public onlyOwner {
        ...
        _minted[msg.sender] = _minted[msg.sender] + mintIndex - supply;
    }
    ...
    function hire(uint numberOfTokens) public payable {
        uint supply = totalSupply();
        require(SALE_STATE, "Hiring not available");
        require(numberOfTokens <= trxLimit, "Exceeded max transaction limit");
