...
contract NFTMarketAuction {
    ...
    function approveAndTransfer(address _from, address _to, uint256 _tokenId) internal returns(bool) {
        ...
        itemToken.approve(_to, _tokenId);
        itemToken.transferFrom(_from, _to, _tokenId);
        return true;
    }
    ...
    function bidAuction(uint256 _tokenId) public payable {
        ...
        if (msg.value > highestBid) {
            uint256 tempAmount = auction.highestBid;
            Bidder memory lastBid;
            ...
            if( bidsLength > 0 ) {
                lastBid = tokenIdToBidder[_tokenId][bidsLength - 1];
            ...
            if( bidsLength > 0 ) {
                if(!lastBid.addr.send(lastBid.amount)) {
                    revert();
                }  
            }
    ...
    function finishAuction(uint256 _tokenId) public payable{
        ...
        if(bidsLength == 0) {
            ...
            Bidder memory awarder = tokenIdToBidder[_tokenId][bidsLength - 1];
            ...
            approveAndTransfer(address(this), _bider, _tokenId);
   ...
   function cancelAuction(uint256 _tokenId)
    public
    {
        ...
        uint bidsLength= tokenIdToBidder[_tokenId].length;
        approveAndTransfer(address(this), auction.seller, auction.tokenId);
