// FOR TESTING WITHOUT ORACLE FUNCTIONALITY
...
contract GasTest is ChainlinkClient {
  using Chainlink for Chainlink.Request;

  // the payment amount for the oracle requests
  uint256 public payment = 10**18;
  ...
  function requestWithString(bytes32 _jobId, uint256[] calldata _IDs) external returns(Chainlink.Request memory) {

    string memory stringID;

    for (uint256 i = 0; i < _IDs.length; i++) {
      stringID = append3String(stringID, uintToString(_IDs[i]), ";");
    }
    string memory payload = append3String("{ 'id_array' :", stringID, " } ");

    Chainlink.Request memory req = buildChainlinkRequest(_jobId, address(this), this.fulfillRequest.selector);
  ...
  function requestWithBytes(bytes32 _jobId, uint256[] calldata _IDs) external returns(Chainlink.Request memory) {

    bytes memory idArray;
    for (uint i = 0; i < _IDs.length; i++){
      idArray = abi.encodePacked(idArray, toBytes(_IDs[i]), ";");
    }
    Chainlink.Request memory req = buildChainlinkRequest(_jobId, address(this), this.fulfillRequest.selector);
    req.addBytes("idArray:", idArray);
    bytes32 _requestId = sendChainlinkRequest(req, payment);

    return req;
  }
  
  function requestWithStringArray(bytes32 _jobId, uint256[] calldata _IDs) external returns(Chainlink.Request memory) {

    Chainlink.Request memory req = buildChainlinkRequest(_jobId, address(this), this.fulfillRequest.selector);
    req.addStringArray("idArray:", uintArrToStringArr(_IDs));
  ...
  function uintArrToStringArr(uint256[] memory arr) public returns(string[] memory){
    for (uint i = 0; i < arr.length; i++) {
      str.push(uintToString(arr[i]));
    }
    return str;
  }
  ...
  function append3String(string memory a, string memory b, string memory c) public pure returns (string memory) {
  ...
  function fulfillRequest(bytes32 requestId, uint256[] memory price) external {}
