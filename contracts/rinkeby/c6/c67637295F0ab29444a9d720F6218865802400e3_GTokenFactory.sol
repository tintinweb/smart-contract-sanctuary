...
contract GTokenFactory is Ownable, IGTokenFactory {
    ...
    function deployGToken(address _spToken, address _owner)
        external
        override
        returns (address _gToken)
    {
        ...
        _gToken = address(new GToken(gTokenName, gTokenSymbol, _spToken, _owner));
        GToken(_gToken).transferOwnership(_owner);
        return _gToken;
    }
}
...
contract GToken is Ownable, ERC20Burnable {
    ...
    address private liquidatorContract;
    ...
    function mint(address account, uint256 amount) external onlyOwner {
        ...
        require(liquidatorContract != address(0x0), "GToken: liqudator null address error");
        super._mint(account, amount);
        //always keep apprroval with token market to burn in case of liquidation
        super._approve(account, liquidatorContract, amount);
    }
    ...
    function _afterTokenTransfer(
        ...
        super._approve(to, liquidatorContract, amount);
    }
...
interface IGTokenFactory {
    function deployGToken(address, address) external returns (address);
}
