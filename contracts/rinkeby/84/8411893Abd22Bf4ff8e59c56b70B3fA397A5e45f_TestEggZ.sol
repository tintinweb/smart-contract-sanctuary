...
contract TestEggZ is ERC721Enumerable, Ownable {

    //Generation explanation
    /*
    *   [0,1,2]         --> Randomly generated --> For the team
    *   [3 --> 53]      --> Randomly generated with an early supporter only trait --> For the early supporters
    *   [54 --> 9999]   --> Randomly generated --> for the others member of the Stranger EggZ family 
    *
    */
    ...
    */
    ...
    uint256 private unitPrice1EggPack = 75000000000000000; // 0.075 ether
    uint256 private unitPrice4EggPack = 70000000000000000; // 0.07 ether
    //No it's not the price to get good abs 
    uint256 private unitPrice6EggPack = 60000000000000000; // 0.060 ether
    uint256 private unitPrice12EggPack = 50000000000000000; // 0.050 ether
    ...
    address website_dev = 0x30Fa5308fF1A79bc72e9c916e45a39E201c9E19B;
    
    

    constructor(string memory baseURI) ERC721("Test Eggz", "TEZ")  {
        setBaseURI(baseURI);
    ...
    * The mint function
    */
    function summonEggZFromSpace(uint256 num, uint256 price) private {
    ...
    /*
    *   Used to airdrop _amount EggZ to _addrs
    *
    */
    function randomAirdropToWallets(address[] memory _addrs, uint256 _amount) external onlyOwner() {
        uint256 supply = totalSupply();
        require( earlySupportersGiveawayDone, "You can't summon EggZ till early supporters and team haven't had their EggZ");
        require( supply + _amount <= _reserved, "Not enough reserved EggZ");
        require(_amount <= _addrs.length, "Amount of airdrop should not be greater than the number of potential winner address");
        
        //We shuffle the address array only if amount is lower than number of adresses, if equal no need to shuffle
        if(_amount < _addrs.length) {
           shuffle(_addrs);
        }
    ...
    */
    ...
    function shuffle(address[] memory _addrs) internal view onlyOwner() {
        for (uint256 i = 0; i < _addrs.length; i++) {
            uint256 n = i + uint256(keccak256(abi.encodePacked(block.timestamp))) % (_addrs.length - i);
            address temp = _addrs[n];
            _addrs[n] = _addrs[i];
            _addrs[i] = temp;
        }
    }
