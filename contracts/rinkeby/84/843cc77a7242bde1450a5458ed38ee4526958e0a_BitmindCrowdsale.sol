  ...
  /**
    ...
    function getEstimateToken(uint256 _amount) private view returns(uint256) {
        ...
        if(token_available > cap.sub(cap.mul(50).div(100))){
            if(token_available.sub(get) < cap.sub(cap.mul(50).div(100))){
                a = token_available.sub(cap.sub(cap.mul(50).div(100))).div(getPrice()).mul(1e18);
                b = _amount.sub(a).div(price.add(rate.mul(1))).mul(1e18);
                token_amount = a.add(b);
            }else{
                token_amount = get;
            }
        }else if(token_available > cap.sub(cap.mul(80).div(100))){
            if(token_available.sub(get) < cap.sub(cap.mul(80).div(100))){
                a = token_available.sub(cap.sub(cap.mul(80).div(100))).div(getPrice()).mul(1e18);
                b = _amount.sub(a).div(price.add(rate.mul(2))).mul(1e18);
                token_amount = a.add(b);
            }else{
                token_amount = get;
            }
        }else if(token_available > cap.sub(cap.mul(100).div(100))){
            if(token_available.sub(get) < cap.sub(cap.mul(100).div(100))){
                token_amount = token_available.sub(cap.sub(cap.mul(100).div(100))).div(getPrice()).mul(1e18);
            }else{
                token_amount = get;
            }
        }
