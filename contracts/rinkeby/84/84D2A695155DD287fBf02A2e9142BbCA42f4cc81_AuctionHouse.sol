...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    constructor(address _media, address _weth) public {
        ...
        media = _media;
        wethAddress = _weth;
        timeBuffer = 1; // extend 15 minutes after every bid made in last 15 minutes
    ...
    function _handleZoraAuctionSettlement(uint256 auctionId) internal returns (bool, uint256) {
        ...
        // address marketAddress = IMediaExtended(media).marketContract();
        // IMarket.BidShares memory bidShares = IMarket(marketAddress).bidSharesForToken(auctions[auctionId].tokenId);
        // uint creatorShare = IMarket(marketAddress).splitShare(bidShares.creator, bid.amount);
        // _handleOutgoingBid(IMedia(media).getTreasurerAddress(), creatorShare, auctions[auctionId].auctionCurrency);
