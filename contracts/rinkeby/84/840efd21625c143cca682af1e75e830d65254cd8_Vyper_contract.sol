...
@view
@external
def add_liquidity(old_balances:uint256[3], token_supply:uint256, amp:uint256, amounts: uint256[3], min_mint_amount: uint256) -> (uint256,uint256[3],uint256,uint256,uint256,uint256[3],uint256[3]):
    # old balance 18 me h
    # amounts bhi 18 me
    ...
    assert D1 > D0
    idl2: uint256[3] = empty(uint256[3])
    bal2: uint256[3] = empty(uint256[3])
    D2: uint256 = D1
    if token_supply > 0:
        # Only account for fees if we are not the first to deposit
        for i in range(N_COINS):
            ideal_balance: uint256 = D1 * old_balances[i] / D0
            idl2[i] = ideal_balance
            ...
            bal2[i] = new_balances[i] - (fees[i] * _admin_fee / FEE_DENOMINATOR)
    ...
    return mint_amount,new_balances,D0,D1,D2,idl2,bal2
