...
contract Controller is AccessControl,Ownable {
  ...
  modifier isAllowed(address addr) {
      require(hasRole(ADMIN_ROLE, addr) || hasRole(DEFAULT_ADMIN_ROLE, addr) || owner() == addr,
              "Sender is not admin");
      _;
  }
  function allowed(address addr) public view returns(bool){
    return(hasRole(ADMIN_ROLE, addr) || hasRole(DEFAULT_ADMIN_ROLE, addr) || owner() == addr);
  }
}
...
contract DSSRegistry is Controller,ERC1155Base {
    string public name;
    ...
    constructor(string memory _name,
                ...
                address[]memory _devs) ERC1155Base(contractURI, tokenURIPrefix) public {
        name = _name;
        symbol = _symbol;
    ...
    function mint(bytes32 mNHash,string memory uri,string memory label,uint256 royalites) public payable {
      ...
      if(!this.allowed(_msgSender()) && registry[uint256(nHash)].isExtension){
        charge = true;
      }
      ...
      address royReceiver = owner();
      if(registry[uint256(nHash)].isDomain && 
          _msgSender() != registry[registry[uint256(nHash)].motherId].owner){
               
            royReceiver = registry[registry[uint256(nHash)].motherId].owner;
      }
      Fee[4] memory _fees = [
        Fee({
      ...
      if(charge){
        ...
        require(msg.value == basePrice.mul(10**18),"Wrong msg.value");
    ...
    function setResolver(address _resolver) public isAllowed(_msgSender()) {
