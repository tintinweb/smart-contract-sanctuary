...
contract MIDT is Initializable, ERC20Upgradeable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    function initialize(string memory _name, string memory _symbol, uint256 _fees,address _admin) public virtual payable initializer {
    ...
    function buy(uint256 amount) public payable whenNotPaused {
        uint256 value;
        if(totalSupply() == 0){
             require(msg.value.mul(initialRatio) == amount,'Exact Value?');
            _mint(msg.sender,msg.value.mul(initialRatio));
        }
        else{
             value = amount.mul(price());
            require(msg.value == value,'Exact Value?');
    ...
    function sell(uint256 amount) public payable whenNotPaused {
        ...
        payable(msg.sender).transfer(_value.sub(fees));  
    ...
    function price() public view returns (uint256) {
        require(totalSupply() == 0,'No Supply. Buy at initialRatio');
        return quotient(
