...
interface IPerpetualTradeManager {
    ...
    //     referrerAddr  address of rabstracteferrer
    //     flags         trade flags
    struct Order {
    ...
    function trade(Order memory _order) external returns (int128);
...
contract PerpetualTradeManager is PerpetualRebalanceFunctions, IFunctionList, IPerpetualTradeManager {
    using ABDKMath64x64 for int128;
    ...
    bytes32 private constant TRADE_ORDER_TYPEHASH =
        keccak256(
            "Order(bytes32 iPerpetualId,address traderAddr,int128 fAmount,int128 fLimitPrice,uint256 iDeadline,address referrerAddr,uint32 flags,uint256 createdTimestamp)"
        );

    /**
     *
     */
    function trade(Order memory _order) external override returns (int128) {
        require(_order.traderAddr == msgSender(), "sender should be set in an order");
        return _trade(_order);
    }

    function tradeBySig(Order memory _order, bytes memory signature) external override returns (int128) {
        /*
    ...
    function _trade(Order memory _order) internal updateFundingAndPrices(_getPoolIdFromPerpetual(_order.iPerpetualId)) returns (int128) {
        ...
        //amount is the trader position (base currency)
        int128 fDeltaLockedValue;
        bool isClose;
        int128 fTradeAmount;
        int128 fPrice;
        {
            // shrink trade-amount if close-only trade and amount too large
            (fPrice, fTradeAmount) = _getTradeLogic().preTrade(perpetual.id, _order.traderAddr, _order.fAmount, _order.fLimitPrice, _order.flags);
            fDeltaLockedValue = fTradeAmount.mul(fPrice);
            ...
            int128 fTraderPos = _getTraderPosition(perpetual, _order.traderAddr);
            int128 fNewPos = fTraderPos.add(fTradeAmount);
            isClose = !_hasOpenedPosition(fNewPos, fTradeAmount);
            if (!isClose) {
                ...
                int128 minPosDelta;
                int128 maxPosDelta;
                (minPosDelta, maxPosDelta) = _getTradeLogic().getMaxTradeSizesForPos(
                    fTraderPos,
                    perpetual.fkStar,
                    perpetual.fCurrentTraderExposureEMA,
                    perpetual.fMaximalTradeSizeBumpUp
                );
                require(fTradeAmount >= minPosDelta && fTradeAmount <= maxPosDelta, "Trade amount exceeds maximal trade amount for trader and AMM state");
            }
            _getTradeLogic().executeTrade(perpetual.id, _order.traderAddr, fTraderPos, fTradeAmount, fDeltaLockedValue, isClose);
            require(perpetual.fOpenInterest >= 0, "negative open interest");
            // update target pool size before we distribute fees
        ...
        _getTradeLogic().distributeFees(perpetual.id, _order.traderAddr, _order.referrerAddr, fDeltaLockedValueCC, !isClose);

        _updateMarkPrice(perpetual, block.timestamp);
        require(_isTraderMarginSafe(perpetual, _order.traderAddr, !isClose), "trader margin unsafe");

        emit Trade(perpetual.id, _order.traderAddr, _order.flags, fTradeAmount, fPrice, block.timestamp);

        return fDeltaLockedValueCC;
    }
    ...
    function _getStructHash(Order memory order) internal returns (bytes32) {
    ...
    function getFunctionList() external view virtual override returns (bytes4[] memory) {
        ...
        functionList[1] = this.tradeBySig.selector;
        return functionList;
    }
}
