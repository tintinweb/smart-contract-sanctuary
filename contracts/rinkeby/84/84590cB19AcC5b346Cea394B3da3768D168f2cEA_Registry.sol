...
contract Registry is Initializable, AccessControlEnumerableUpgradeable {
    ...
    mapping(uint256 => address) public projecIdToAddress;
    ...
    {
        return projects[project].status;
    }
    ...
    function getProjectMaxSupply(address project)
        ...
        returns(uint256)
    {
        return projects[project].maxSupply;
    }

    function getProjectId(address project)
        public
        view
        returns(uint256)
    {
    ...
    function createProject(Project memory project) public onlyDefaultAdmin
    {
        ...
        projectContract.initialize(project.name, project.symbol, nextProjectId);


        projects[address(projectContract)].name = project.name;
        projects[address(projectContract)].symbol = project.symbol;
        projects[address(projectContract)].artist = project.artist;
        projects[address(projectContract)].description = project.description;
        projects[address(projectContract)].website = project.website;
        projects[address(projectContract)].license = project.license;
        projects[address(projectContract)].projectImage = project.projectImage;
        projects[address(projectContract)].price = project.price;
        projects[address(projectContract)].projectId = nextProjectId;
        projects[address(projectContract)].maxSupply = project.maxSupply;
        projects[address(projectContract)].status = project.status;
        ...
        projecIdToAddress[nextProjectId] = address(projectContract);
        nextProjectId++;
        ...
        onlyDefaultAdmin
    {
        ...
        projects[project].minter = address(minterContract);
        ...
        onlyDefaultAdmin
    {
        ...
        // Minter Admin
        address minter = projects[project].minter;
    ...
    function setMaxSupply(address project, uint256 supply)
        ...
        onlyRegistryAdminOrProjectAdmin(project)
    {
        projects[project].maxSupply = supply;
    }
