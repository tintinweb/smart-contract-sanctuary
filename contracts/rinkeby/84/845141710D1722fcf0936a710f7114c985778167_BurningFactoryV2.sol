...
import "./BurningV2.sol";
import "./Transfer.sol";

/**
 * BurningFactoryV2 is the contract for management of ZUSD/GYEN.
 * Create Burning Contract with create2.
 */
contract BurningFactoryV2 {
    ...
    function burn(address _tokenAddress, uint256 _amount, bytes32 _salt) public onlyBurner {
        ...
        bytes memory bytecode = type(BurningV2).creationCode;
        assembly {
    ...
    function predictBurningAddress(bytes32 _salt) public view returns(address){
        return address(uint(keccak256(abi.encodePacked(
            byte(0xff),
            address(this),
            _salt,
            keccak256(abi.encodePacked(type(BurningV2).creationCode))
        ))));
    }

    function predictTransferAddress(bytes32 _salt) public view returns(address){
        return address(uint(keccak256(abi.encodePacked(
            byte(0xff),
            address(this),
            _salt,
            keccak256(abi.encodePacked(type(Transfer).creationCode))
        ))));
    }

    function getBurningInitCode() public pure returns(bytes32){
        return keccak256(abi.encodePacked(type(BurningV2).creationCode));
    }

    function getTransferInitCode() public pure returns(bytes32){
        return keccak256(abi.encodePacked(type(Transfer).creationCode));
    }
}

pragma solidity 0.5.8;

import "./BurningFactoryV2.sol";
import "./Token_v2.sol";

/**
 * BurningV2 is the contract for management of ZUSD/GYEN.
 */
contract BurningV2 {

    constructor() public {
        address factory = msg.sender;
        address token = BurningFactoryV2(factory).tokenAddress();
        uint256 amount = BurningFactoryV2(factory).amount();

        Token_v2(token).burn(amount);
...
contract Transfer {
    ...
    constructor() public {
        address factory = msg.sender;
        address token = BurningFactoryV2(factory).tokenAddress();
        address recipient = BurningFactoryV2(factory).recipient();
        uint256 amount = BurningFactoryV2(factory).amount();

        require(Token_v2(token).transfer(recipient, amount), "Transfer is failed");
        selfdestruct(address(0));
    }
}
