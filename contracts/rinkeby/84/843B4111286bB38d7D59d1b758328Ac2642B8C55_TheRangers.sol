 ...
 * {ERC721Enumerable}.
 */
contract TheRangers is Context, ERC165, IERC721, IERC721Metadata {
    using Address for address;
    using Strings for uint256;
    using SafeMath for uint256;

    // Token name
    string private _name = 'The Rangers';

    // Token symbol
    string private _symbol = 'RANGERS';
    ...
    // Token Price
    uint256 public constant tokenPrice = 70000000000000000;
    ...
    uint256 private constant ROYALTY_SIZE = 500;
    ...
    */
    ...
    function buyTokensOnPresale(uint256 tokensNumber, bytes calldata signature) public payable {
      ...
      require(_mintedTokens[msg.sender].add(tokensNumber) <= 5, 'You cannot purchase more then 3 tokens on presale');
      require((tokensNumber.mul(tokenPrice)) == msg.value, "Received value doesnt match the requested tokens");
      ...
      require(signer != address(0) && signer == _contractOwner, 'claim: Invalid signature!');
    ...
    function buyTokens(uint256 tokensNumber) public payable {
        ...
        require(tokensNumber <= 15, "You cannot purchase more than 15 tokens at once");
        require((tokensNumber.mul(tokenPrice)) == msg.value, "Received value doesnt match the requested tokens");
        require((currentSupply.add(tokensNumber)) <= _totalSupply, "You try to mint more tokens than totalSupply");
    ...
    function mintForAirdrop(uint256 tokensNumber) public onlyOwner {
      ...
      require(tokensNumber <= 10, "You cannot claim more than 10 tokens at once");
      require((currentSupply.add(tokensNumber)) <= _totalSupply, "You try to mint more tokens than totalSupply");
