...
contract Skullys001 is ERC721, IERC2981, ReentrancyGuard, Ownable {
    ...
    function setSaleStartTime(uint256 saleStartTime) public onlyOwner {
        require(saleStartTime > block.timestamp, "Provided sale start time can't be earlier than current time");
    ...
    function allowedEarlyPresaleCount(address minter) public view returns (uint256) {
        if (tokensMinted[minter] > EARLY_PRESALE_LIMIT_PER_ADDRESS) {
            return 0;
        } else {
     ...
     **/
    function reserveSkullys(uint256 numberToMint) public onlyOwner nonReentrant{
        require(
     ...
     **/
    function mintCustomSkullys() public onlyOwner nonReentrant{
    ...
    function royaltyInfo(uint256, uint256 salePrice) external view override(IERC2981) returns (address receiver, uint256 royaltyAmount) {
        return (royaltyAddress, (salePrice * 700) / 10000);
    }
