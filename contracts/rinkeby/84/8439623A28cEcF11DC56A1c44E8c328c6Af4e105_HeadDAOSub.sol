...
contract HeadDAOSub is Ownable {
	...
	event SubscriptionCreated(address subscriber,uint256 date);
  	event PaymentSent(address from, uint256 amount, uint256 planId,uint256 date, uint256 expiry);


	// Subscription Plans
	uint256 public  initalPlanID = 0;
	uint256 public licneses = 4000;
	...
	event PlanCreated(uint256 planId,uint256 date);
	...
	function pay(uint planId) public payable basics{
		...
		emit PaymentSent(msg.sender, plan.amount, planId, block.timestamp, block.timestamp + plan.frequency);	
		totalBurnt += plan.amount;
	}
