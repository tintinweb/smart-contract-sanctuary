...
// Timeless Ape Club has 3 sale stages:
//   1. Whitelisted Pre-sale  - 0.7 ether
//   2. Non-whitelisted Pre-Sale - 0.7 ether
//   3. Public Sale - 0.1 ether
contract TimelessApe is ERC721A, Ownable {
    using Strings for uint256;

    uint256 public PRE_SALE_PRICE = 0.07 ether;
    uint256 public PUBLIC_MINT_PRICE = 0.1 ether;
    uint256 public PUBLIC_MINT_MAX_PER_TX = 20;
    ...
    bool public presaleToPublicOpen = false;
    ...
    constructor() ERC721A("Timeless Ape Club", "TAC", PUBLIC_MINT_MAX_PER_TX, MAX_SUPPLY) {}
    ...
    function presaleMint(
        ...
        require(totalSupply() + quantity <= PRESALE_SUPPLY);
        ...
        require(_preSaleWallets[msg.sender] + quantity <= maxClaimQuantity, "quantity of tokens cannot exceed max mint");
        require(totalSupply() + quantity <= MAX_SUPPLY, "The mint would exceed max supply of apes");
    ...
    function mintApe(uint256 quantity) external payable callerIsUser {
        require(publicSaleOpen, "Public Sale is not open");
        require(quantity > 0, "quantity of tokens cannot be less than or equal to 0");
        require(quantity <= PUBLIC_MINT_MAX_PER_TX, "quantity of tokens cannot exceed max limit per mint");
        require(totalSupply() + quantity <= MAX_SUPPLY + reserved, "the purchase would exceed max supply of apes");
        require(msg.value >= PUBLIC_MINT_PRICE * quantity, "insufficient ether value");
        _safeMint(msg.sender, quantity);
    }

    // need to add json to this
    ...
    function changePresaleLimit(uint256 newLimit) external onlyOwner {
    ...
    function togglePresaleToPublic() external onlyOwner {
        presaleToPublicOpen = !presaleToPublicOpen;
    }
    ...
    function giveAway(address to, uint256 quantity) external onlyOwner {
        ...
        require(totalSupply() + quantity <= MAX_SUPPLY, "the giveaway would exceed max supply of tokens");
        reserved -= quantity;
