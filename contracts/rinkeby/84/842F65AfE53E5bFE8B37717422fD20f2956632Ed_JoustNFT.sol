...
contract JoustNFT is Ownable,  ERC721 {
    uint256 public tokenCounter;

    // Optional mapping for token URIs
    // tokenURI is a address to the NFT json metadata stored in webserver or IPFS server.
    ...
    constructor () ERC721 ("JoustNFT", "JNFT"){
        tokenCounter = 0;
        _totalSupply = 1000;
    }

    //creates the nft asset 
    ...
    // It is internal function to set tokenURI for any particular tokenId.
    function _setTokenURI(uint256 tokenId, string memory _tokenURI) internal virtual {
        require(_exists(tokenId), "NFTMetadata: URI set of nonexistent token");
        require(ownerOf(tokenId) == msg.sender, "Only owners allowed to set tokenURI");
        _tokenURIs[tokenId] = _tokenURI;
    }

    // public function to return the tokenURI for any tokenId.
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
    ...
    // Set baseURI public function to be called by owner of the contract
