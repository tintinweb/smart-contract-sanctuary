...
import "./lib/ERC721B.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

contract GivingBulls is ERC721B, Ownable {
    
    using Strings for uint256;
    using SafeMath for uint256;

    uint public _price = 0.025 ether;
    uint public _max = 200;
    ...
    event NewTokensMinted(address sender, uint[] tokenId);

    constructor() ERC721B("GivingBulls", "{{TokenSymbol}}") {
    ...
    function mint(uint total) public payable {
        ...
        if(_presaleIsActive) {
            require(_presaleAddresses[msg.sender], "You are not on the presale list.  Please wait for public mint.");
        } else {
            require(_saleIsActive, "The sale must be active to mint");
        }
