...
contract CryptoPioneers is NFT {
  using Math for uint256;
  using SafeMath for uint256;

  uint256 public secInMin = 60; // 24 * 60 * 60
  uint256 public intervalMin = 3; // 3 min
  uint256 public intervalSec = intervalMin * secInMin; // 5 min
  ...
  function mint(uint256 qty) external payable override {
    // ...
  ...
  function _mint(uint256 quantity, address to) internal override {
    ...
    for (uint256 i = 0; i < quantity; i++) {
      ...
      uint256 lastTime = expiredTime[to] >= block.timestamp
        ? expiredTime[to]
        : block.timestamp;
      expiredTime[to] = lastTime + intervalSec;

      emit Mint(tokenId, to);
    }
  }

  function remainingDay(address owner) public view returns (uint256) {
    if (expiredTime[owner] <= block.timestamp) {
      return 0;
    }

    uint256 diff = expiredTime[owner] - block.timestamp;

    return diff.ceilDiv(secInMin);
  }

  function balanceOf(address owner) public view virtual override returns (uint256) {
    return remainingDay(owner).ceilDiv(intervalMin);
  }
}
