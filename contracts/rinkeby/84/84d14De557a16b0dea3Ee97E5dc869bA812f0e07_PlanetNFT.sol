...
contract PlanetNFT is ERC721Tradable {
    ...
    function batch_create(
        ...
        uint16[] calldata size,
        int16[][] calldata position,
        uint16[][] calldata rotation)  public onlyAdmin {

        for (uint256 i = 0; i < _seed.length; i++) {
            create(_tokenId[i], _seed[i], color_[i], size[i], position[i], rotation[i]);
        }
    }

    function batch_mint_create(
        uint8[] calldata _tokenId,
        string[] calldata _seed,
        string[] calldata color_,
        uint16[] calldata size,
        ...
        for (uint256 i = 0; i < _seed.length; i++) {
            mintTo(msg.sender);
            create(mintTokenId, _seed[i], color_[i], size[i], position[i], rotation[i]);
        }
    }
    ...
    function validateParams(
        ...
        uint16 size,
        int16[] calldata position, //x y
        uint16[] calldata roation //x y angle duration
    ) internal pure {
