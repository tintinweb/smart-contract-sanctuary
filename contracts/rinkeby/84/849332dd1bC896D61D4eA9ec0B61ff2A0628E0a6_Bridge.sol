...
interface IPapr {
    ...
    function getfeesTaxRate(uint256 index) external view returns (uint256 tax);
    function getPaprPrice()  external view returns (uint256 price);
}
...
contract Bridge is Trustable {
    ...
    function convertAmount(ERC20 token, uint amount, uint decimals) view internal returns (uint) {
     ...
     function setPaprToken(address _papr) external onlyOwner {
       papr = _papr;
    }
    ...
    function _countTimesOverCents() internal view returns (uint256) {
        uint256 paprPrice = IPapr(papr).getPaprPrice();
        if (paprPrice >= maxPriceTaxRates) {
            ...
            uint256 count;
            paprPrice = paprPrice - minPriceTaxRates;
            count = paprPrice / paprMultiplierStepExpansion;
            return count + 1;
        }
    }
    ...
    uint paprTaxAmount = IPapr(papr).getfeesTaxRate(_countTimesOverCents());
    return paprTaxAmount;
    ...
    function create(uint16 tokenId, uint amount, uint8 destination, address target) external {
        ...
        uint paprTaxAmount = amount * IPapr(papr).getfeesTaxRate(_countTimesOverCents()) / 100;

        amount = amount - feeAmount - paprTaxAmount;
