...
import "./interfaces/IMaxwells.sol";
...
contract Depictives is ERC721, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        address _maxwellsContractAddress) 
        ERC721(
            name,
            symbol
        ) {
            ...
            maxwells = IMaxwells(_maxwellsContractAddress);
        }
    ...
    bytes32 public merkleRoot = 0x937c3dbb6a335a60256710c589810ad39597a4c7001b35e5f140c00e3c82c94a;
    ...
    uint256 private preSaleLaunchTime = 1639591200;
    uint256 private publicSaleLaunchTime = 1639598400;
    ...
    mapping(address => uint256) preSaleMints;

    uint256[] private apostleIDs = [1,2,3,4];

    IMaxwells public immutable maxwells;

    string public baseTokenURI = "https://arweave.net/asd/";

    function _genMerkleLeaf(address account) internal pure returns (bytes32) {
    ...
    function mint(uint256 numberOfMints, bytes memory data) public payable nonReentrant {
        ...
        bool preSaleEligible = _preSaleEligible(msg.sender, numberOfMints);
        ...
        if (isPreSaleActive() && !isPublicSaleActive()) {
            ...
            require(MerkleProof.verify(proof, merkleRoot, _genMerkleLeaf(msg.sender)), "INVALID_PROOF");
            require(addr == msg.sender, "INVALID_SENDER");
            require(numberOfMints + preSaleMints[msg.sender] <= mintAllocation, "TOO_LARGE_PER_TX_OR_LIMIT");
        }

        require(isPublicSaleActive() || (isPreSaleActive() && preSaleEligible), "SALE_NOT_ACTIVE");
        require(isPublicSaleActive() && numberOfMints <= MAX_MULTI_MINT_AMOUNT, "TOO_LARGE_PER_TX_OR_LIMIT");
        ...
        if (isPreSaleActive() && !isPublicSaleActive()) {
            preSaleMints[msg.sender] += numberOfMints;
        }
    }
...
interface IMaxwells is IERC721, IERC721Metadata {
