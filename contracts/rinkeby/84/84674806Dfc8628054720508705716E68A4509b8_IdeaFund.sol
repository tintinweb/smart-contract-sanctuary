...
contract IdeaFund is Operator, ContractGuard {
    ...
    modifier checkMigration {
        require(!migrated, 'Idea Fund: migrated');
        _;
    }
    ...
    function getHaifSupply() external view returns (uint256)
    {
        return _haifSupply;
    }

    //external pricing function for Treasury, Boardroom
    function controlPrice() external view hasOracle returns (uint256)
    {
        return _getControlPrice();
    }
    ...
    function _getControlPrice() internal view hasOracle returns(uint256) {

        //calculate value        
        uint256 totalValue = _haifSupply.mul(IOracle(theOracle).priceOf(hedge)).div(2) + IERC20(wbtc).balanceOf(address(this)).mul(IOracle(theOracle).wbtcPriceOne());
        return totalValue.div(variableReduction).div(IERC20(control).totalSupply());
    }
    ...
    //TODO - we should consider a TAX on this transaction - if not today,  maybe a variable we set in the future.  LPs charge .3% ++
    ...
    //TODO: we could extend this to allow payout options of lfBTC or LIFT.  Right now it is just wBTC
        ...
        ctrlRedeemable
    {
        ...
        require(
            IERC20(wbtc).balanceOf(address(this)).mul(IOracle(theOracle).wbtcPriceOne()) >= amount.mul(_getControlPrice()),
            'Idea Fund: Treasury does not currently hold enough wBTC to purchase'
        );

        require(amount <= IERC20(control).allowance(msg.sender, address(this)), 
            'Idea Fund: is not approved to redeem your CTRL');

        IBasisAsset(control).burnFrom(msg.sender, amount);
        IERC20(wbtc).safeTransfer(msg.sender, amount.mul(_getControlPrice()).div(IOracle(theOracle).wbtcPriceOne()));
    ...
    //Selling Functions for Protocol Tokens - LFBTC / LIFT
    // we are either selling lfbtc or lift
    function ideaFundSellingToken(address token, uint256 numTokens) public checkMigration onlyOperator
    {
        require(numTokens > 0, 'Idea Fund: cannot sell token with zero amount');
        
        require(
            numTokens <= IERC20(token).balanceOf(address(this)),
            'Idea Fund: Not enough token to sell'
        );

        address[] memory pathTo;
        IERC20(token).approve(address(router), numTokens);
        ...
        if (token == peg) {
            ...
            require(false, 'Idea Fund: we cant exchange that token yet');
        }
        ...
        emit IdeaFundSoldlfBTC(msg.sender, numTokens);
    }

    // tokenA (lfbtc or lift) is what we are buying, with numTokens of tokenB (wbtc, lfbtc, lift)
    function ideaFundBuyingTokenAwithTokenB(address tokenA, address tokenB, uint256 numTokens) public checkMigration onlyOperator
    {
    ...
    function migrate(address target) public onlyOperator checkMigration {
        ...
        IERC20(peg).transfer(
            target,
            IERC20(peg).balanceOf(address(this))
        );

        IERC20(share).transfer(
            target,
            IERC20(share).balanceOf(address(this))
        );

        IERC20(control).transfer(
            target,
            IERC20(control).balanceOf(address(this))
        );

        IERC20(hedge).transfer(target,
            IERC20(hedge).balanceOf(address(this))
        );
    ...
    function cleanUpDust(uint256 amount, address tokenAddress, address sendTo) onlyOperator public  {     
        require(tokenAddress != wbtc ||
                tokenAddress != peg ||
                tokenAddress != share ||
                tokenAddress != control ||
                tokenAddress != hedge, 
                'If you need to withdrawl primary tokens use the DAO to migrate to a new contract');
