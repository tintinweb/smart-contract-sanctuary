 ...
 */
contract ERC1155CollectionImplementation is
    ERC1155Upgradeable,
    ERC1155CollectionBase,
    AdminControlUpgradeable
{
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        virtual
        override(ERC1155Upgradeable, AdminControlUpgradeable)
        view
        returns (bool)
    {
        return
     ...
     * @dev See {IERC1155Collection-premint}.
     */
    function premint(uint16[] calldata amounts, address[] calldata addresses)
        external
        override
        adminRequired
    {
    ...
    function _mint(
        ...
        return ERC1155Upgradeable._mint(to, tokenId, amount, data);
    }
}
...
abstract contract ERC1155CollectionBase is CollectionBase, IERC1155Collection {
    ...
    function purchase(
        ...
        if (block.timestamp - startTime < presaleInterval) {
            require(
                balance == 0 && amount <= presalePurchaseLimit,
                "Too many requested"
            );
            _validatePresalePrice(amount);
        } else {
            require(amount <= (purchaseLimit - balance), "Too many requested");
    ...
    /**
     * @dev See {IERC1155Collection-tokenRemaining}.
     */
    function tokenRemaining() public virtual override view returns (uint16) {
     ...
     */
    function _premint(uint16[] calldata amounts, address[] calldata addresses)
        internal
        virtual
    {
        require(!active, "Already active");
    ...
    /**
     * @dev A _mint function is required
     */
    function _mint(
        address to,
        uint256 tokenId,
    ...
    function _validatePresalePrice(uint16 amount) internal virtual {
        require(
            msg.value == amount * presaleTokenPrice,
            "Invalid purchase amount sent"
        );
    }
...
interface IERC1155Collection is ICollectionBase {
    ...
    /**
     * @dev Premint tokens to the list of addresses. Sale must not be active.
    ...
    function getRoyalties(uint256)
        external
        view
        returns (address payable recipient, uint256 bps);
