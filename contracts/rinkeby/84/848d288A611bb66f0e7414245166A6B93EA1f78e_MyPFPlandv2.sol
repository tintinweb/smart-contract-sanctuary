...
contract MyPFPlandv2 is ERC721Upgradeable {
    ...
    mapping(address => mapping(uint256 => uint256)) landBalanceOf;
    mapping(uint256 => address) landOwnerOf;
    mapping(uint256 => address) collectionAddressByIndex;
    mapping(uint256 => Rectangle) collectionRectByIndex;
    ...
    function claimLand(uint256 x, uint256 y) external {
        bool availability;
        uint256 collectionID;
        (availability, collectionID) = checkLandAvailability(msg.sender, x, y);
        require(availability == true, "not avaiable to claim");
        uint256 assetID = _encodeTokenId(x, y);
        landOwnerOf[assetID] = msg.sender;
        landBalanceOf[msg.sender][collectionID] ++;
    }

    // return two values
    // availability denotes land availability for the claimer
    // collectionID starts from zero
    function checkLandAvailability(address claimer, uint256 x, uint256 y) public view returns(bool availability, uint256 collectionID) {
        ...
        for (uint256 i = 0; i < totalCollection; i++) {
            ...
            if (isInsideCollectionRect(pt, area)) {
                ...
                uint256 claimable = collection.balanceOf(claimer);
                if (claimable == 0)
                    return (false, 0);
                if (landBalanceOf[claimer][i] < claimable)
                    return (true, i);
            }
        }
    }

    function isInsideCollectionRect(Point memory point, Rectangle memory area) internal pure returns(bool) {
        if (point.x == area.rightTop.x && point.y == area.rightTop.y)
            return true;
        else
            return false;
    }

    function setCollectionRect(uint256 leftBottomX, uint256 leftBottomY, uint256 rightTopX, uint256 rightTopY, address collectionAddress) external onlyOwner {
        Rectangle memory area;
    ...
    function _encodeTokenId(uint256 x, uint256 y) internal pure returns (uint256) {
        return ((uint256(x) * factor) & clearLow) | (uint256(y) & clearHigh);
    }

    function _decodeTokenId(uint256 value) internal pure returns (uint256 x, uint256 y) {
