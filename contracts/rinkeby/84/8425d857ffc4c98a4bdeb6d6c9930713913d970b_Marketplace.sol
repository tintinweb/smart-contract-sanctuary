...
contract Marketplace is Ownable, Pausable, IMarketplace, ERC721Holder {
     ...
     */
    function createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, bool _isEth) public whenNotPaused {
        _createOrder(_nftAddress, _assetId, _priceInWei, _expiresAt, _isEth);
    }
    ...
    function safeExecuteOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public payable whenNotPaused {
        ...
        if(order.currency == 0x0000000000000000000000000000000000000000) {
            ...
            // Transfer token amount to seller
            IERC20(address(order.currency)).transferFrom(msg.sender, order.seller, order.price);
        }
    ...
    function acceptBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public whenNotPaused {
        ...
        if(order.currency == 0x0000000000000000000000000000000000000000) {
            ...
            IERC20(address(order.currency)).transfer(order.seller, bid.price);
        }
     ...
     */
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, bool _isEth) internal {
        // Check nft registry
        ...
        orderByAssetId[_nftAddress][_assetId] = Order({
            ...
            currency: _isEth ? address(0x0000000000000000000000000000000000000000) : address(Shib),
            price: _priceInWei,
