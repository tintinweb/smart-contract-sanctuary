...
contract BaseFixedPriceAuction is ERC721Sequential, ReentrancyGuard, Ownable {
    ...
    function _nonReservedMintHelper(uint256 numberOfTokens) internal {
        require(msg.value >= numberOfTokens * price, "Not enough funds.");
    ...
    function mintReserved(uint256 numberOfTokens) external onlyOwner {
        ...
        require(reservedMinted + numberOfTokens <= reservedMax, 'Not enough tokens.');
    ...
    function publicMint(uint256 count) public payable {
        uint256 totalSupply = _owners.length;
        require(totalSupply + count < max, "Excedes max supply.");
        require(count < 1, "Exceeds max per transaction.");
