...
// Deployed Address: 0x1AbfF47EA67c008f41Ac22Cc4C6B867Bf08c77E9 
...
contract Spots is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    event TokenBonded(address nftContract, uint nftIndex, uint spotIndex);
    event TokenUnbonded(uint spotIndex);
    event BondedNFTModelURISet(uint spotIndex, string uri);
    event OwnerLinkSet(uint spotIndex, string link);
    event BondedNFTCreatorLinkSet(uint spotIndex, string link);
    ...
    requireSpotOwner(spotIndex) {
        ...
        emit TokenBonded(nftContract, nftIndex, spotIndex);
    }
    ...
    function genericERC1155TokenURI(address nftContract, uint tokenId) 
    public view
    returns (string memory) {
        string memory uri = IERC1155MetadataURI(nftContract).uri(tokenId);
        if (!uriSuffixExists(uri)) {
            return uri;
        }

        uint length = bytes(uri).length;
        string memory uriPrefix = substring(uri, 0, length - 4);
        string memory hexidecimal = uint2hexstr(tokenId);

        return concat(uriPrefix, hexidecimal, "", "", "");
    }
    ...
    returns(string memory) {
        ...
        if (hasIERC1155MetadataURISupport) {
            return genericERC1155TokenURI(nftContract, tokenIndex);
        }
        return "";
    }

    function uint2hexstr(uint i) 
    internal pure 
    returns (string memory) {
        if (i == 0) return "0";
        uint j = i;
    ...
    function uriSuffixExists(string memory uri) 
    internal pure
    returns (bool) {
        uint length = bytes(uri).length;
        string memory suffix = substring(uri, length - 4, length);
        
        return stringsEqual(suffix, "{id}");
    }
