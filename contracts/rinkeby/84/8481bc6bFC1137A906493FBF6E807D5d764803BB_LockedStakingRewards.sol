...
contract LockedStakingRewards is Ownable {
    ...
    function updatePool(uint256 _pool) external {
        ...
        require(block.timestamp < pool[_pool].startOfDeposit + pool[_pool].cycleDuration, "can only update before startOfDeposit");
    ...
    function balanceOf(address _user) external view returns(uint256) {
        uint256 sum = stakeToken.balanceOf(_user);
        for(uint i = 0; i < 5; i++) {
