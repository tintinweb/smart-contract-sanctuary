...
//    string public baseURI = "https://my-hgassembly.herokuapp.com/api/token/";
    ...
    uint256 FIVE_OPTION = 1;
    uint256 TEN_OPTION = 2;
    uint256 NUM_IN_FIVE_OPTION = 5;
    uint256 NUM_IN_TEN_OPTION = 10;

    uint assetPrice = 40; // milli ether
    bool saleStarted = false;
    string public HBMS_PROVENANCE = "";

    constructor() public ERC721("High Bunch Munch Station", "HBMS") {}
    ...
    function baseTokenURI() public pure returns (string memory) {
        return "https://my-hgassembly.herokuapp.com/api/token/";
    }

    function setProvenanceHash(string memory _hash) external onlyOwner {
        HBMS_PROVENANCE = _hash;
    }
    ...
    function mint(uint256 _optionId, address _toAddress) public whenSaleStarted {
        ...
        require(canBuy(_optionId, _msgSender()), "can_buy_failed");
        require(canMint(_optionId), "can_mint_failed");
    ...
    function canBuy(uint _optionId, address _sender) public view returns (bool){
        uint qty = 0;
        if (_optionId == SINGLE_OPTION) {
            qty = 1;
        } else if (_optionId == FIVE_OPTION) {
        ...
        if (_sender != owner()) {
            return addressToBalance[_sender] >= ((1 ether * assetPrice / 1000) * qty);
        } else {
            return true;
        }
    }
    ...
    function deductBalance(uint _optionId) internal {
        ...
        uint value = 0;
        if (_optionId == SINGLE_OPTION) {
            value = (1000000000000000 * assetPrice);
        } else if (_optionId == FIVE_OPTION) {
            value = (1000000000000000 * assetPrice) * NUM_IN_FIVE_OPTION;
        } else if (_optionId == TEN_OPTION) {
            value = (1000000000000000 * assetPrice) * NUM_IN_TEN_OPTION;
        }
