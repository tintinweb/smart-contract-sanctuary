...
contract BuissnesHourManager {
    ...
    function setBuissnesHours(
        ...
        require(weekDayType <= uint8(WeekDayType.sunday), "WRONG_WEEKDAY_INPUT");
        buissnesDays[key][weekDayType].startHour = startHour;
        buissnesDays[key][weekDayType].endHour = endHour;
    }

    function getBuissnesHours(
        bytes32 key,
        uint8 weekDayType
    ) external view returns (uint8 start, uint8 end){
        require(weekDayType <= uint8(WeekDayType.sunday), "WRONG_WEEKDAY_INPUT");
        return (buissnesDays[key][weekDayType].startHour,
    ...
    function initializeBuissnesHours(bytes32 key) internal {
        for(uint8 i = 0; i <= uint8(WeekDayType.sunday); i++)
            buissnesDays[key][i] = BuissnesDay(WeekDayType(i), uint8(0), uint8(3600*24));
    }
}
...
contract DateTimeAPI {
        ...
        function getMinute(uint timestamp) pure public returns (uint8);
        function getSecond(uint timestamp) pure public returns (uint8);
        function getWeekday(uint timestamp) pure public returns (uint8);
        function toTimestamp(uint16 year, uint8 month, uint8 day) pure public returns (uint timestamp);
        function toTimestamp(uint16 year, uint8 month, uint8 day, uint8 hour) pure public returns (uint timestamp);
        function toTimestamp(uint16 year, uint8 month, uint8 day, uint8 hour, uint8 minute) pure public returns (uint timestamp);
        function toTimestamp(uint16 year, uint8 month, uint8 day, uint8 hour, uint8 minute, uint8 second) pure public returns (uint timestamp);
}
