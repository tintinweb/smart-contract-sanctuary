...
contract SamotToken is ERC20, ERC721Holder, Ownable {
    ...
    function removeStake(address _stakeholder, uint256 _tokenId) internal {
        ...
        if (_isStaked) {
            ...
            stakedtokens.pop();
            for (uint256 i = 0; i < stakes[_stakeholder].tokens.length; i += 1) {
                if (_tokenId == stakes[_stakeholder].tokens[i]) {
                  stakes[_stakeholder].tokens[i] = stakes[_stakeholder].tokens[stakes[_stakeholder].tokens.length - 1];
    ...
    function unstakeNFTs(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(stakes[msg.sender].tokens.length > 0, "You don't have any NFTs staked.");
            require(
        ...
        if (stakes[msg.sender].tokens.length == 0) removeStakeholder(msg.sender);
    }

    function stakeOf(address _stakeholder) public view returns (uint256[] memory) {
    ...
    function totalStakes() public view returns (uint256) {
        ...
        for (uint256 s = 0; s < stakeholders.length; s += 1) {
            _totalStakes = _totalStakes.add(stakes[stakeholders[s]].tokens.length);
        }
        return _totalStakes;
    }
