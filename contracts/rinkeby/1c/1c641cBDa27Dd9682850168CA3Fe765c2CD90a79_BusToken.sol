...
contract BusToken is ERC721, Ownable, Pausable, ReentrancyGuard {
    ...
    function mintPrice(uint _amount) public view returns (uint) {
        ...
        if (diff > 1 days) return priceInEth; // Base price might be updated by devs after 1 day

        // Every 60 seconds the price changes MINUS 0.003 ether
        // uint price = priceInEth - (diff / 10 minutes) * 0.003 ether;
        uint price = priceInEth - (diff / 120 seconds) * 0.003 ether;
