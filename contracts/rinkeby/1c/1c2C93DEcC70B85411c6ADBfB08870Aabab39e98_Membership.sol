...
import "./interfaces/IMembershipMetadata.sol";

contract Membership is ERC721Enumerable, Ownable, IMembership, IMembershipMetadata, ReentrancyGuard {
  ...
  function mint()
    ...
    nonReentrant
  {
    require(_mintList[msg.sender], "You are not on the Allow List");
    require(mintIndex + 1 <= TOTAL_LIMIT, "out of membership limit");        
    require(mintPrice <= msg.value, "Ether value sent is not correct");
    require(_mintList[msg.sender], "Address not in the mint list");    
...
interface IMembership {
  ...
  function isOnMintList(address addr) external returns (bool);
