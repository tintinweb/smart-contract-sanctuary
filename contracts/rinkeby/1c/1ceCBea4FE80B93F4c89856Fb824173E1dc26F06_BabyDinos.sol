...
contract BabyDinos is ERC721Enumerable, Ownable {
  string public baseTokenURI;
  string public provenanceHash = "";

  uint256 private _price = 0.05001 ether;
  ...
  address private _verifier = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266;

  constructor(string memory baseURI) ERC721("BabyDinos", "BABYDINOS") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 _num, bytes calldata _signature) external payable {
    ...
    require(totalSupply + _num <= 5500 - _reserved, "Exceeds maximum supply");
    require(walletCount[_msgSender()] + _num < 5, "Max mint per account is 4");
  ...
  function whitelistMint(uint256 _num, bytes32[] calldata _merkleProof)
    external
    payable
  {
    uint256 totalSupply = totalSupply();

    require(!whitelistMintPaused, "Minting paused");
    require(
      tx.origin == msg.sender,
      "Purchase cannot be called from another contract"
    );
    require(totalSupply + _num <= 5500 - _reserved, "Exceeds maximum supply");
    require(walletCount[_msgSender()] + _num < 3, "Max mint per account is 2");
    require(msg.value >= _price * _num, "Ether sent is not correct");
  ...
  function giveAway(address _to, uint256 _amount) external payable onlyOwner {
  ...
  function emergencySetSeed() public onlyOwner {
