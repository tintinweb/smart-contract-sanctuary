...
contract RainiLpStakingPool is AccessControl, ReentrancyGuard {
  ...
  event RewardSet(uint256 rewardRate, uint256 minRewardStake, uint256 rewardDecimals);
  event BonusesSet(uint256 maxBonus, uint256 bonusDuration, uint256 bonusDecimals);
  ...
  modifier balanceUpdate(address _owner) {
    ...
    lastBonus[_owner] = lastBonus[_owner].add(bonusRate.mul(duration));
    _;
  }
  ...
  function setReward(uint256 _rewardRate, uint256 _minRewardStake, uint256 _rewardDecimals)
    external onlyOwner {
      rewardRate = _rewardRate;
      minRewardStake = _minRewardStake;
      rewardDecimals = _rewardDecimals;

      emit RewardSet(rewardRate, minRewardStake, rewardDecimals);
  }
  ...
  function setBonus(uint256 _maxBonus, uint256 _bonusDuration, uint256 _bonusDecimals)
    external onlyOwner {
      maxBonus = _maxBonus;
      bonusDuration = _bonusDuration;
      bonusRate = _maxBonus.div(_bonusDuration);
      bonusDecimals = _bonusDecimals;

      emit BonusesSet(_maxBonus, _bonusDuration, _bonusDecimals);
  }
  ...
  function calculateBonus(address _owner, uint256 _amount, uint256 _duration)
    private view returns(uint256) {
      uint256 currentBonus = Math.min(maxBonus, lastBonus[_owner].add(bonusRate.mul(_duration)));
      uint256 avgBonus = currentBonus.add(lastBonus[_owner]).div(2);
      return _amount.add(_amount.mul(avgBonus).mul(_duration).div(10 ** bonusDecimals));
  }
}
