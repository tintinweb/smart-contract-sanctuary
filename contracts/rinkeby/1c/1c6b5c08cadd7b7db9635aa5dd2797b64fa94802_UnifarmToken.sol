...
contract MultiBeneficiaryTokenVesting is Ownable {
  ...
  modifier onlyBeneficiaries {
    require(msg.sender == _owner || shares[msg.sender] > 0, "You cannot release tokens!");
    _;
  }
...
contract UnifarmToken is Ownable {
    ...
    uint public totalSupply = 1000000000e18; // 1 billion UNIFARM
    ...
    function vestingAllocate(address ownerAdd, uint256 _start, uint256 _cliff, uint256 _duration, uint vestToken, uint8 status) public onlyOwner {
        address poolAddress;
        poolAddress = address(new MultiBeneficiaryTokenVesting(ownerAdd, address(this), _start, _cliff, _duration, status));
        vestingList.push(poolAddress);
        
        _transferTokens(msg.sender, poolAddress, vestToken);
        
        emit Vested(poolAddress, block.timestamp);
    }
}