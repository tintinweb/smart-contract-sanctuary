...
contract Sector5slums is VRFConsumerBase {
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes calldata _data) public  {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        _safeTransfer(from, to, tokenId, _data);
    }
    function _safeTransfer(address from, address to, uint256 tokenId, bytes calldata _data) internal {
        _transfer(from, to, tokenId);
    ...
    function _checkOnERC721Received(address from, address to, uint256 tokenId, bytes calldata _data)
        internal view returns (bool)
    {
        if (!to.isContract()) {
            return true;
        }
        bytes4 retval = onERC721Received(_msgSender(), from, tokenId, _data);
        return (retval == _ERC721_RECEIVED);
    }
    function onERC721Received(address, address, uint256, bytes calldata) internal pure returns (bytes4) {
        return bytes4(keccak256("onERC721Received(address,address,uint256,bytes)")); 
    }
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal {}


    function requestNewRandomAerith(string memory name) public returns (bytes32) {
        ...
        requestToAerithName[requestId] = name;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        uint256 newId = aeriths.length; uint256 Strength = (randomNumber % 999); uint256 Magic = (randomNumber % 999); uint256 Vitality = (randomNumber % 999); 
        uint256 Spirit = (randomNumber % 999); uint256 Luck = (randomNumber % 999); uint256 Speed = (randomNumber % 999); uint256 Level = 0;
        aeriths.push(Aerith(Strength, Magic, Vitality, Spirit, Luck, Speed, Level, requestToAerithName[requestId]));
