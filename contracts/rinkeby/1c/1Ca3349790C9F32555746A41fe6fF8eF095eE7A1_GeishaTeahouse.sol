...
contract GeishaTeahouse is ERC721Enumerable, ERC721Pausable, Ownable, ERC721Burnable {
    ...
    uint256 public preSaleCost = 0.0777 ether;
    uint256 public cost = 0.0888 ether;
    uint256 public maxSupply = 9999;
    uint256 public maxMintAmountVipPresale = 3;
    uint256 public maxMintCount = 7;
    enum Sale{NONE, WHITELIST, PRESALE, SALE}
    Sale public sale;
    ...
    function mint(uint256 _mintAmount) public payable whenNotPaused {
        ...
        uint256 actualCost = getCurrentCost();
        require(_mintAmount > 0, "mint amount > 0");
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
        if (msg.sender != owner()) {
            require(sale != Sale.NONE, "Sale has not started yet");
            require(_mintAmount <= maxMintCount, "Too many items minted");
            if (sale == Sale.WHITELIST) {
                require(isWhitelisted(msg.sender), "user is not whitelisted");
                uint256 ownerMintedCount = addressMintedBalance[msg.sender];
                require(
                    ownerMintedCount + _mintAmount <= maxMintAmountVipPresale,
    ...
    function getCurrentCost() public view returns (uint256) {
        return (sale == Sale.WHITELIST || sale == Sale.PRESALE) ? preSaleCost : cost;
    }
    ...
    function setMaxMintAmountVipPreSale(uint256 _amount) public onlyOwner {
        maxMintAmountVipPresale = _amount;
    }
    ...
    function withdraw() public payable onlyOwner {
        (bool hs,) = payable(commission).call{value : address(this).balance * 5 / 100}("");
        require(hs, "WITHDRAW ERROR COMMISSION");
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721Enumerable, ERC721Pausable, ERC721) whenNotPaused {
