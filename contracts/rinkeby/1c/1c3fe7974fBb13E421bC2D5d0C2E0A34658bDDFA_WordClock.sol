...
contract WordClock is ERC721Enumerable, Ownable, ERC165Storage {
    ...
    mapping(uint256 => ClockConfig) internal clockConfig;

    constructor(address fontAddress) payable ERC721("WordClock", "OCWC") {
        _registerInterface(_INTERFACE_ID_ERC2981);
        font = ICorruptionsFont(fontAddress);
    }

    function claimGift(int256 offset, string calldata onHex) public {
        _mint(msg.sender, nextTokenId);
        clockConfig[nextTokenId] = ClockConfig({hourOffset: offset * 1 hours, onHex: onHex});
        nextTokenId += 1;
    }
    ...
    function updateConfig(
        uint256 tokenId,
        int256 offset,
        string calldata onHex
    ) public {
        require(msg.sender == ownerOf(tokenId), "WC:uC:401");

        clockConfig[tokenId] = ClockConfig({hourOffset: offset * 1 hours, onHex: onHex});
    }

    /* solhint-disable quotes */

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "WC:tU:404");
                ...
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"WordClock',
                                '", "description":"WordClock is an on-chain SVG clock.", "image": "',
    ...
    function svgBase64Data(uint256 tokenId) public view returns (string memory) {
        return string(abi.encodePacked("data:image/svg+xml;base64,", Base64.encode(svgRaw(tokenId, 0))));
    }
    ...
    function svgRaw(uint256 tokenId, uint256 timestamp) public view returns (bytes memory) {
        ...
        uint256 _timestamp = timestamp == 0 ? uint256(int256(block.timestamp) + clock.hourOffset) : timestamp;
    ...
    string[14][] internal ROWS = [
        ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"],
        ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL"],
        ["AUG", "SEP", "OCT", "NOV", "DEC", "001", "002"],
        ["003", "004", "005", "006", "007", "008", "009"],
        ["010", "011", "012", "013", "014", "015", "016"],
        ["017", "018", "019", "020", "021", "022", "023"],
        ["024", "025", "026", "027", "028", "029", "030"],
        ["031", "012", "001", "002", "003", "004", "005"],
        ["006", "007", "008", "009", "010", "011", "000"],
    ...
    function renderRow(uint256 rowNum) internal view returns (bytes memory) {
        ...
        string[14] memory rowLabels = ROWS[rowNum - 1];
    ...
    function animationUrl(uint256 tokenId) internal view returns (bytes memory) {
        ...
        return
            abi.encodePacked(
                "ipfs://QmcuNGMHDTGVxF1Bqrxb4tTm2ksPgVKnNT23FEB3z41DrC?h=",
                bytes32(bytes(clock.onHex)),
                "&tz=",
                uint256(12 + clock.hourOffset).toString()
            );
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC165Storage, ERC721Enumerable)
        returns (bool)
    {
 ...
 * account. The ineffecient totalSupply is replaced with a simple counter.
 */
