...
import "./mixins/MomentsNftNode.sol";
...
/**
 * @title A market for NFTs on Moments.
 * @dev This top level file holds no data directly to ease future upgrades.
 */

contract MomentsMarket is
    MomentsNftNode,
    MomentsTreasuryNode,
    MomentsAdminRole,
    ...
    NFTMarketReserveAuction
{
     ...
     */
    function initialize(address payable treasury, address payable nftContract)
        public
        initializer
    {
        MomentsTreasuryNode._initializeMomentsTreasuryNode(treasury);
        MomentsNftNode._initializeMomentsNftNode(nftContract);
        NFTMarketAuction._initializeNFTMarketAuction();
        NFTMarketReserveAuction._initializeNFTMarketReserveAuction();
    }

    /**
     * @notice Allows Moments to update the market configuration.
     */
    function adminUpdateConfig(
        uint256 minPercentIncrementInBasisPoints,
        uint256 duration
    ) public onlyMomentsAdmin {
...
abstract contract MomentsTreasuryNode is Initializable {
     ...
     */
    function _initializeMomentsTreasuryNode(address payable _treasury)
        internal
        initializer
    {
        require(
     ...
     */
    function getMomentsTreasury() public view returns (address payable) {
        return treasury;
    }
...
/**
 * @notice A mixin that stores a reference to the Moments Nft contract.
 */
abstract contract MomentsNftNode is Initializable {
    using AddressUpgradeable for address payable;

    address payable private nftContract;

    /**
     * @dev Called once after the initial deployment to set the Moments Nft address.
     */
    function _initializeMomentsNftNode(address payable _nftContract)
        internal
        initializer
    {
        require(
            _nftContract.isContract(),
            "MomentsNftNode: Address is not a contract"
        );
        nftContract = _nftContract;
    }

    /**
     * @notice Returns the address of the Moments Nft.
     */
    function getMomentsNft() public view returns (address payable) {
        return nftContract;
    }
...
/**
 * @notice Allows a contract to leverage an admin role defined by the Moments contract.
 */
abstract contract MomentsAdminRole is MomentsTreasuryNode {
    ...
    modifier onlyMomentsAdmin() {
        require(
            IAdminRole(getMomentsTreasury()).isAdmin(msg.sender),
    ...
    NFTMarketAuction
{
    ...
    function createReserveAuction(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            IERC721Upgradeable(getMomentsNft()).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
    ...
    /**
     * @notice Allows Moments to cancel an auction, refunding the bidder and returning the NFT to the seller.
