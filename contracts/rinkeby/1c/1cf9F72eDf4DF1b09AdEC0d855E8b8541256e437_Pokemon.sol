...
contract Pokemon is ERC721, VRFConsumerBase, Ownable, PokemonBaseStatData {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        ...
        setIvs(RNGNumbers, pokemonName, newUniquePokemonID);
        setBattleStats(pokemonName, newUniquePokemonID);
        emit PokemonCreated(tokenCounter);
        tokenCounter = tokenCounter + 1;
    }

    function getTypeFromName(string memory pokemonName) public returns (string memory, string memory ){
        PokemonBaseStats memory baseStats = pokemonNameToPokemonBaseStats[pokemonName];
