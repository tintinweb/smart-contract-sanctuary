...
contract NounsToken is ERC721Tradable, INounsToken, ReentrancyGuard {
    ...
    ) ERC721Tradable('Nouns', 'NOUN', _proxyRegistryAddr) {
        treasury = _treasury;
    ...
    /**
     * toggle on/off the public sale switch
     */
    function setPublicSaleStatus(bool active) external onlyOwner {
        isPublicSaleStarted = active;
    }

    /**
     * @notice Mint a Noun to the messenger sender
     */
    function mint(uint256 _count) external payable nonReentrant override {
        require(isPublicSaleStarted, "public sale not started yet");
        require(_count > 0 && _count <= MAX_PER_MINT, "Invalid number to mint");
        require(_currentNounId + _count <= MAX_SUPPLY, "Not enough supply");
        require(_count * MINT_PRICE == msg.value, "Incorrect amount of ether sent");

        for (uint256 i = 0; i < _count; i++) {
            _currentNounId ++;
            INounsSeeder.Seed memory seed = seeds[_currentNounId] = seeder.generateSeed(_currentNounId, descriptor);
            _safeMint(_msgSender(), _currentNounId);
            emit NounCreated(_msgSender(), _currentNounId, seed);
        }
    }
...
interface INounsToken is IERC721 {
    event NounCreated(address indexed creator, uint256 indexed tokenId, INounsSeeder.Seed seed);
