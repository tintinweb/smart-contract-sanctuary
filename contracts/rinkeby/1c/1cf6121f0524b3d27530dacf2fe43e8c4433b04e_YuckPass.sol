...
contract YuckPass is ERC1155, Ownable {
  ...
  uint public constant GALAXY = 0;
  uint public constant YUCKY = 1;
  uint public constant GOLD = 2;
  uint public constant SILVER = 3;
  uint public constant BRONZE = 4;

  uint public constant MAX_TOKENS = 3333;
  uint public numMinted = 0;

  uint public galaxyPriceWei = 1 ether;
  uint public yuckyPriceWei = 0.5 ether;
  uint public goldPriceWei = 0.1 ether;
  uint public silverPriceWei = 0.05 ether;
  uint public bronzePriceWei = 0.03 ether;
  ...
  function _internalMint(uint passType, uint amount) internal {
    ...
    if (passType == GALAXY) {
      checkPayment(galaxyPriceWei * amount);
    } 
    else if (passType == YUCKY) {
      checkPayment(yuckyPriceWei * amount);
    }
  ...
  function ownerMint(uint passType, uint amount) public onlyOwner {
    incrementNumMinted(amount);
    require(passType == GALAXY || passType == YUCKY || passType == GOLD || passType == SILVER || passType == BRONZE, "Invalid passType");
    _mint(msg.sender, passType, amount, "");
  }
  ...
  function setGalaxyPriceWei(uint newPrice) public onlyOwner {
    galaxyPriceWei = newPrice;
  }

   function setYuckyPriceWei(uint newPrice) public onlyOwner {
    yuckyPriceWei = newPrice;
  }
