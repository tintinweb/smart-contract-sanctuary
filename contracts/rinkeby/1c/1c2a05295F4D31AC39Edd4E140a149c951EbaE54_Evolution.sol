...
// @notice This contract is heavily inspired by Dom Hofmann's Loot Project, zeth's Settlements with game design from Sid Meirs Civilization & Spore.
...
contract Evolution is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private _industries = ['Steam Power', 'Railroad', 'Company', 'Finance', 'Flight', 'Rocket', 'Fission', 'Aerospace'];
    string[] private _informations = ['Computer', 'Information Theory', 'Cryptography', 'Operating System', 'TCP/IP', 'Web', 'Search Engine', 'Social Network', 'Mobile', 'GPU', 'Chip'];
    ...
    function tokenURI(uint256 tokenId) virtual override public view returns (string memory) {
        ...
        atrrOutput = string(abi.encodePacked(attributesParts[16], attributesParts[17], attributesParts[18], attributesParts[19], attributesParts[20]));
    ...
    function randomiseAttributes(uint256 tokenId) internal {
        attrIndex[tokenId].atom = generateAttribute(_getRandomSeed(tokenId, 'size'), _atoms);
        attrIndex[tokenId].molecular = generateAttribute(_getRandomSeed(tokenId, 'spirit'), _moleculars);
        attrIndex[tokenId].cell = generateAttribute(_getRandomSeed(tokenId, 'age'), _cells);
        attrIndex[tokenId].organism = generateAttribute(_getRandomSeed(tokenId, 'resource'), _organisms);
        attrIndex[tokenId].language = generateAttribute(_getRandomSeed(tokenId, 'morale'), _languages);
        attrIndex[tokenId].civilization = generateAttribute(_getRandomSeed(tokenId, 'government'), _civilizations);
        attrIndex[tokenId].science = generateAttribute(_getRandomSeed(tokenId, 'government'), _sciences);
        attrIndex[tokenId].industry = generateAttribute(_getRandomSeed(tokenId, 'morale'), _industries);
        attrIndex[tokenId].information = generateAttribute(_getRandomSeed(tokenId, 'government'), _informations);
        attrIndex[tokenId].metaverse = generateAttribute(_getRandomSeed(tokenId, 'government'), _metaverses);
    }
