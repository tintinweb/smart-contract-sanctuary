...
contract TestToken is ERC721, Ownable {
    ...
    bool private _paused = false; // TODO: set to true by default
    uint16[] public _availableTokens;
    ...
    constructor() ERC721("TestToken", "TEST") {
        ...
        phasePrice[2] = 2 ether; // 2 PENNY
        phasePrice[3] = 4 ether; // 4 PENNY
        phasePrice[4] = 8 ether; // 8 PENNY
    ...
    function addAvailableTokens(uint16 _from, uint16 _to) public {
    ...
    function switchToSalePhase(uint16 _phase, bool _setTokens) public onlyOwner {
        ...
        // if (phase == 1)
        // {
        //     addAvailableTokens(1, 10000);
        // } else if (phase == 2) {
        //     addAvailableTokens(10001, 20000);
        // } else if (phase == 3) {
        //     addAvailableTokens(20001, 40000);
        // } else if (phase == 4) {
        //     addAvailableTokens(40001, 50000);
        // }

        if (_phase == 1) {
            addAvailableTokens(1, 50);
        } else if (_phase == 2) {
            addAvailableTokens(51, 100);
        } else if (_phase == 3) {
            addAvailableTokens(21, 40);
        } else if (_phase == 4) {
            addAvailableTokens(41, 50);
        }
    }
    ...
    function mint(uint _amount, bool _stake) public payable whenNotPaused {
        ...
        for (uint i = 0; i < _amount; i++) {
            ...
            if (recipient != msg.sender) {
                isPirate(tokenId) ? pirateStolen += 1 : pirateStolen += 1;
    ...
    function selectRecipient() internal returns (address) {
        ...
        // 30% chance to steal NTF
        if (getSomeRandomNumber(pirateMinted, 100) >= 30) {
            return msg.sender;
        }

        address thief = manager.randomPirateOwner();
    ...
    function setSpeciality(uint16 id, bool special) external onlyOwner {
        _isPirate[id] = special;
    }

    function setSpeciality(uint16[] calldata ids) external onlyOwner {
    ...
    function withdraw(address to) external onlyOwner {
        ...
        payable(address(0x389A9Dc99A592B122c2A21c9fF4b44f588D54c8b)).transfer(share);
