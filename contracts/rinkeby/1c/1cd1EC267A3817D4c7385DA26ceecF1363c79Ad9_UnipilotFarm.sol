...
contract UnipilotFarm is IUnipilotFarm, IERC721Receiver {
    ...
    // mapping farming data --> user add => pool add => bool
    mapping(address => mapping(address => bool)) public farmingActive;

    //mapping pool address => bool (Exist in whitelist or not)
    mapping(address => bool) public poolWhitelist;

    address internal constant ULM = 0x0510Fdd44eE837998a3e36b7567E057C3A30994F;
    address internal constant UNIPILOT = 0x8e4Ccb58F96845be157a5DC7279A1db529A84E98;
    address internal constant PILOT_TOKEN = 0xc9F2964c57Bb34FaF41392f914d629933FAF80e0;
    ...
    function initializer(address[] memory pools) internal onlyAdmin {
        for (uint256 i = 0; i < pools.length; i++) {
            poolWhitelist[pools[i]] = true;
            poolInfo[pools[i]].startBlock = block.number;
            poolInfo[pools[i]].lastRewardBlock = block.number;
        }
    }

    function insertPool(address poolAddress) external override onlyAdmin {
        require(poolWhitelist[poolAddress] == false);
        poolWhitelist[poolAddress] = true;
    }

    modifier onlyAdmin() {
        require(_admin == msg.sender, "OAC");
        _;
    }
    ...
    function updatePilotPerBlock(address poolAddress, uint256 value)
        ...
        returns (uint256)
    {
        poolInfo[poolAddress].rewardPerBlock = value * 1e18;
    ...
    function updateMultiplier(address poolAddress, uint256 value)
        ...
        returns (uint256)
    {
        poolInfo[poolAddress].rewardMultiplier = value * 1e18;
    ...
    function toggleBooster(uint256 tokenId)
        external
        override
        onlyAdmin
        returns (bool currentStatus)
    {
        userInfo[tokenId].booster = !userInfo[tokenId].booster;
        currentStatus = userInfo[tokenId].booster;
    }

    function nftStatus(uint256 tokenId) external view override returns (bool) {
        return
            farmingActive[userInfo[tokenId].userAddress][userInfo[tokenId].poolAddress];
    }

    function depositNFT(address poolAddress, uint256 tokenId)
        external
        override
        onlyOwner(tokenId)
        returns (bool)
    {
        require(poolWhitelist[poolAddress] == true, "Pool not exist");
        (, address pool1, uint256 liquidity, , , , ) = IUniswapLiquidityManager(ULM)
            .positions(tokenId);

        require(poolAddress == pool1, "PNE");
        ...
        poolInfo[poolAddress] = PoolInfo({
            ...
            startBlock: poolInfo[poolAddress].startBlock,
            globalReward: poolInfo[poolAddress].globalReward,
            lastRewardBlock: poolInfo[poolAddress].lastRewardBlock,
            totalSupply: poolInfo[poolAddress].totalSupply,
            ...
            lastAltRewardBlock: poolInfo[poolAddress].lastAltRewardBlock,
        ...
        IERC721(UNIPILOT).safeTransferFrom(msg.sender, address(this), tokenId);

        userToPoolToTokenIds[msg.sender][poolAddress].push(userInfo[tokenId].nftId);

        farmingActive[msg.sender][pool1] = true;

        globalFarmingReward += poolInfo[poolAddress].globalReward;
        ...
        return farmingActive[msg.sender][pool1];
    }
    ...
    function getGlobalReward(
        ...
        uint256 temp = FullMath.mulDiv(reward, multiplier, 1e18);

        globalReward = FullMath
            .mulDiv(blockDifference * temp, 1e18, poolInfo[poolAddress].totalSupply)
            .add(_globalReward);
    }

    function currentReward(address poolAddress, uint256 tokenId)
        public
        view
        override
    ...
    function emergencyNFTWithdraw(uint256 tokenId) external {
        ...
        uint256 amountLock = IERC20(PILOT_TOKEN).balanceOf(address(this));
        IERC20(PILOT_TOKEN).safeTransfer(_admin, amountLock);

        emit WithdrawNFT(
            userInfo[tokenId].poolAddress,
            userInfo[tokenId].userAddress,
            userInfo[tokenId].nftId,
            poolInfo[userInfo[tokenId].poolAddress].totalSupply
        );
    ...
    function withdrawNFT(address poolAddress, uint256 tokenId)
        external
        override
        returns (bool isLeft)
    {
        withdrawReward(poolAddress, tokenId);
        (poolAddress, tokenId);
        ...
        farmingActive[msg.sender][poolAddress] = false;
    ...
    function withdrawReward(address poolAddress, uint256 tokenId)
        public
        override
        returns (bool isSent)
    {
        require(userInfo[tokenId].userAddress == msg.sender, "NOO");
        (uint256 pilotReward, uint256 globalReward) = currentReward(poolAddress, tokenId);
        require(
            IERC20(PILOT_TOKEN).balanceOf(address(this)) >= pilotReward,
            "UnipilotFarm:: INSUFFICIENT FUND"
        );
        ...
        if (poolInfo[poolAddress].isAltActive == true) {
            altWithdraw(poolAddress, tokenId);
        }
    ...
    function migrateFundData(address newContract) external onlyAdmin {
        require(newContract != address(0), "Contract not exist");
        IERC20(PILOT_TOKEN).safeTransfer(
            newContract,
            IERC20(PILOT_TOKEN).balanceOf(address(this))
        );

        uint256 countNFT = IERC721(UNIPILOT).balanceOf(address(this));
        if (countNFT > 0) {
            for (uint256 i = 0; i < countNFT; i++) {
                uint256 tokenId = IERC721Enumerable(UNIPILOT).tokenOfOwnerByIndex(
                    _admin,
                    i
                );
                IERC721(UNIPILOT).safeTransferFrom(address(this), newContract, tokenId);
            }
        }
    }

    // Alt Reward Functions

    function altDeposit(address poolAddress, uint256 tokenId) internal {
        if (poolInfo[poolAddress].lastAltRewardBlock == 0) {
    ...
    // function getAltGlobalReward(address poolAddress)
    //     public
    //     view
    //     returns (uint256 altGlobalReward)
    // {
    ...
    //     altGlobalReward = FullMath
    //         .mulDiv(blockDifference * temp, 1e18, poolInfo[poolAddress].totalSupply)
    //         .add(poolInfo[poolAddress].altGlobalReward);
    // }

    function currentAltReward(address poolAddress, uint256 tokenId)
        public
        view
    ...
    function altWithdraw(address poolAddress, uint256 tokenId) internal {
        (uint256 altReward, uint256 altGlobalReward) = currentAltReward(
            poolAddress,
            tokenId
        );

        address altToken = poolInfo[poolAddress].altToken;
        require(
            IERC20(altToken).balanceOf(address(this)) >= altReward,
            "UnipilotFarm:: INSUFFICIENT FUND"
        );
        poolInfo[poolAddress].altGlobalReward = altGlobalReward;
        userInfo[tokenId].altReward = altGlobalReward;
        IERC20(altToken).safeTransfer(userInfo[tokenId].userAddress, altReward);
    }
    ...
    function updateAltPerBlock(address poolAddress, uint256 value)
        ...
        returns (uint256)
    {
        poolInfo[poolAddress].altRewardPerBlock = value * 1e18;
    ...
    function updateAltMultiplier(address poolAddress, uint256 value)
        ...
        returns (uint256)
    {
        poolInfo[poolAddress].altRewardMultiplier = value * 1e18;
        return poolInfo[poolAddress].altRewardMultiplier;
    }

    function toggleActiveAlt(address poolAddress)
        external
        onlyAdmin
        returns (bool currentStatus)
    {
        poolInfo[poolAddress].isAltActive = !poolInfo[poolAddress].isAltActive;
        currentStatus = poolInfo[poolAddress].isAltActive;
    }
}
...
// start block = pool last block
    ...
    function insertPool(address poolAddress) external;
    ...
    function toggleBooster(uint256 tokenId) external returns (bool currentStatus);

    function nftStatus(uint256 tokenId) external view returns (bool);

    function depositNFT(address poolAddress, uint256 tokenId) external returns (bool);
