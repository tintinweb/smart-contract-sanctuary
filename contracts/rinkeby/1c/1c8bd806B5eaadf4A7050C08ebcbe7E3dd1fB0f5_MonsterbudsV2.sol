...
contract MonsterbudsV2 is MonsterBudHolders, ERC721URIStorageUpgradeable, IMonsterBuds {
    ...
    mapping(uint256 => uint256) private listPrices;
    ...
    */
     ...
     *
     * Requirements:
     * - only owner of token can list and update token price.
    */

    function sellList(uint256 tokenId, uint256 price) external returns (bool){
        require(ownerOf(tokenId) == msg.sender, "$MONSTERBUDS: You are not owner of token");
    ...
    */
    ...
    function buyCollectiable(uint256 token_id) external override payable returns(bool){
        ...
        require(listPrices[token_id] > 0 && listPrices[token_id] == msg.value,"$MONSTERBUDS: Token is not listed or enter price correctly");
