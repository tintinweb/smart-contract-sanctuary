...
contract PhantomFounders is PhantomStorageMixin, IPhantomFounders {
    ...
    function registerFounder(address founder, uint256 amount) external override onlyRegisteredContracts nonReentrant {
        PhantomStorage().setUint(keccak256(abi.encodePacked("phantom.founder.vesting.initialAmount", founder)), amount);
        PhantomStorage().setUint(
            keccak256(abi.encodePacked("phantom.founder.vesting.remainingAmount", founder)),
            amount
        );
    }
    ...
    function claim(address founder) external override onlyRegisteredContracts nonReentrant {
        uint256 amount = PhantomStorage().getUint(
            keccak256(abi.encodePacked("phantom.founder.vesting.initialAmount", founder))
        );
    ...
    function exercise(address founder) external override onlyRegisteredContracts nonReentrant {
        ...
        require(remainingAmount > 0, "Founder as no unvested fPHM");

        // Do the math to figure out how many tokens have vested.
        // 25% vest immediately, remaining 75% vest over the next 12 months
        uint256 unclaimedAmount = remainingAmount -
            (initialAmount.div(4) +
                initialAmount.div(4).mul(3).mul((block.timestamp - vestingStarts()).div(SECONDS_PER_YEAR)));
        // Burn and mint
        fPHM().burn(founder, unclaimedAmount);
        gPHM().mint(founder, unclaimedAmount);
