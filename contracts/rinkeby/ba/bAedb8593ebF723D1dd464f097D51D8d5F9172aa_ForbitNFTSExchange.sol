...
contract ForbitNFTSExchange is Exchange {
    ...
    string public constant codename = "Forbitswap";

    /**
     * @dev Initialize a ForbitNFTSExchange instance
...
import "../utils/libraries/Market.sol";
import "../utils/libraries/SaleKind.sol";
...
contract ExchangeCore is ReentrancyGuard, Ownable {
        ...
        returns (bool)
    {
        ...
        if (
            !SaleKind.validateParameters(order.saleKind, order.expirationTime)
        ) {
            return false;
        }
        ...
        if (
            order.feeMethod == Market.FeeMethod.SplitFee &&
     ...
     */
    function sizeOf(Market.Order memory order) internal pure returns (uint256) {
        return ((0x14 * 7) +
            (0x20 * 9) +
            4 +
        ...
        returns (bool)
    {
        ...
        (buy.side == Market.Side.Buy && sell.side == Market.Side.Sell) &&
            ...
            SaleKind.canSettleOrder(buy.listingTime, buy.expirationTime) &&
            /* Sell-side order must be settleable. */
            SaleKind.canSettleOrder(sell.listingTime, sell.expirationTime));
    }
    ...
    function executeFundsTransfer(
        ...
        This is done prior to fee payments to that a seller will have tokens before being charged fees. */
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            (receiveAmount, requiredAmount) = transferFundOnSellSide(
                buy,
                sell,
                price,
                receiveAmount,
            ...
            transferFundOnBuySide(buy, sell, price);
        }
    ...
    function transferFundOnSellSide(
        Market.Order memory buy,
        Market.Order memory sell,
        uint256 price,
        ...
        if (sell.feeMethod == Market.FeeMethod.SplitFee) {
            ...
            if (sell.makerRelayerFee > 0) {
                uint256 makerRelayerFee = sell.makerRelayerFee.mul(price).div(
                    INVERSE_BASIS_POINT
                );
            ...
            if (sell.takerRelayerFee > 0) {
                uint256 takerRelayerFee = sell.takerRelayerFee.mul(price).div(
                    INVERSE_BASIS_POINT
                );
            ...
            if (sell.makerProtocolFee > 0) {
                uint256 makerProtocolFee = sell.makerProtocolFee.mul(price).div(
                    INVERSE_BASIS_POINT
                );
            ...
            if (sell.takerProtocolFee > 0) {
                uint256 takerProtocolFee = sell.takerProtocolFee.mul(price).div(
                    INVERSE_BASIS_POINT
                );
        ...
        return (receiveAmount, requiredAmount);
    }

    function transferFundOnBuySide(
        Market.Order memory buy,
        Market.Order memory sell,
        uint256 price
    ) internal {
        ...
        if (sell.feeMethod == Market.FeeMethod.SplitFee) {
            ...
            if (buy.makerRelayerFee > 0) {
                uint256 makerRelayerFee = buy.makerRelayerFee.mul(price).div(
                    INVERSE_BASIS_POINT
                );
                transferTokens(
                    sell.paymentToken,
            ...
            if (buy.takerRelayerFee > 0) {
                uint256 takerRelayerFee = buy.takerRelayerFee.mul(price).div(
                    INVERSE_BASIS_POINT
                );
                transferTokens(
                    sell.paymentToken,
            ...
            if (buy.makerProtocolFee > 0) {
                uint256 makerProtocolFee = buy.makerProtocolFee.mul(price).div(
                    INVERSE_BASIS_POINT
                );
                transferTokens(
                    sell.paymentToken,
            ...
            if (buy.takerProtocolFee > 0) {
                uint256 takerProtocolFee = buy.takerProtocolFee.mul(price).div(
                    INVERSE_BASIS_POINT
                );
                transferTokens(
                    sell.paymentToken,
    ...
    function calculateMatchPrice(
        ...
        uint256 sellPrice = SaleKind.calculateFinalPrice(
            sell.saleKind,
            sell.basePrice
        );

        /* Calculate buy price. */
        uint256 buyPrice = SaleKind.calculateFinalPrice(
            buy.saleKind,
            buy.basePrice
        );
...
library SaleKind {
     ...
     */
    function calculateFinalPrice(Market.SaleKind saleKind, uint256 basePrice)
        internal
        pure
