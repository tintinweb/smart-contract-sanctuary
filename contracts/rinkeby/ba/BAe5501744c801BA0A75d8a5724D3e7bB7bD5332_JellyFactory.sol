...
contract JellyFactory is SafeTransfer {
     ...
     */
    function getContractsByTemplateId(bytes32 _templateId) external view returns (address[] memory, uint256) {
        address[] memory _contracts = new address[](contracts.length);
        uint256 i = 0;
        for (;i < contracts.length; i++) {
            address _contract = contracts[i]; 
            if(contractInfo[_contract].templateId == _templateId) {
                _contracts[i] = _contract;
            }
        }
        return (_contracts, i);
    }
