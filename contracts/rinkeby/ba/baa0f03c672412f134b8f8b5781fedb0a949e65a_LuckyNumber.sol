 ...
 */
contract LuckyNumber is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant numbersPrice = 100000000000000000; //0.1 BNB

    uint public constant maxnumbersPurchase = 20;

    uint256 public MAX_NUMBERS;

    bool public saleIsActive = false;


    constructor(string memory name, string memory symbol, uint256 maxNftSupply) ERC721(name, symbol) {
        MAX_NUMBERS = maxNftSupply;
    ...
    /**
     * Set some Numbers aside
     */
    function reserveNumbers() public onlyOwner {        
    ...
    */
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % MAX_NUMBERS;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % MAX_NUMBERS;
        }
