...
import "./interfaces/IFLYZERC20.sol";
...
contract FlyzTreasury is Ownable {
    ...
    function deposit(
        ...
        uint256 value = reserveValueOf(_token, _amount);
    ...
    function auditReserves() external onlyOwner {
        ...
        for (uint256 i = 0; i < reserveTokens.length; i++) {
            reserves = reserves.add(
                reserveValueOf(reserveTokens[i], IERC20(reserveTokens[i]).balanceOf(address(this)))
            );
        }
        for (uint256 i = 0; i < liquidityTokens.length; i++) {
            reserves = reserves.add(
                reserveValueOf(liquidityTokens[i], IERC20(liquidityTokens[i]).balanceOf(address(this)))
            );
        }
    ...
    /**
        ...
        @return value_ uint
     */
    function reserveValueOf(address _token, uint256 _amount) public view returns (uint256 value_) {
        if (isReserveToken[_token]) {
            // convert amount to match FLYZ decimals
            // we want 1 FLYZ = 0.001 reserve token
            value_ = 1000 * _amount.mul(10**IERC20Metadata(FLYZ).decimals()).div(10**IERC20Metadata(_token).decimals());
