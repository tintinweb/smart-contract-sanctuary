...
/**
 * @title Token contract for ABDUCTED JOEZ NFTs
 * @dev This contract allows the distribution of
 * ABDUCTED JOEZ NFTs in the form of a presale and main sale.
 *
 *
 *
 *
  ...
  ReentrancyGuard
{
  ...
  // Presale
  mapping(address => uint256) private saleNonce;
  ...
  constructor(
    uint256 totalSupply,
    uint256 maxMint
  ) ERC721("ABDUCTED JOEZ NFT", "AJOEZ") {
  ...
  function setMintPrice(uint256 cost) external onlyOwner {
    ...
    // @param maxMint The max tokens a user can mint from the presale
    ...
    function mintSale(
        ...
        require(tokenCount <= maxMint, "Token count exceeds limit");
        require((mintPrice * tokenCount) == msg.value, "ETH sent does not match required payment");
        require(maxMint <= maxMintPerTransaction, "The max mint is over required amount");
        
        // Verify signature
        bytes32 message = getSaleSigningHash(msg.sender, tokenCount, maxMint).toEthSignedMessageHash();
        require(ECDSA.recover(message, sig) == signVerifier, "Permission to call this function failed");

        saleNonce[msg.sender]++;
  ...
  function getSaleSigningHash(
    ...
    return
      keccak256(
        abi.encodePacked(sender, tokenCount, presaleMaxMint, saleNonce[sender])
      );
  }
