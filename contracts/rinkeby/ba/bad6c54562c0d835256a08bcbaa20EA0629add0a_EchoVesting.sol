 ...
 */
contract EchoVesting is TokenVesting {
    constructor(
        address beneficiary,
        uint256 start,
    ...
    TokenVesting(beneficiary, start, cliffDuration, duration, revocable, token) {}
}
...
contract TokenVesting is Ownable {
    ...
    constructor (
        ...
        uint256 __duration,
        bool __revocable,
        IERC20Upgradeable __token
    ) {
        require(__beneficiary != address(0), "TokenVesting: beneficiary is the zero address");
        require(address(__token) != address(0), "TokenVesting: token is the zero address");
    ...
    function release() public {
        // require(hasRole(TEAM_MEMBER, _msgSender()), "Caller is not the Team Account");
     ...
     * @param __token ERC20 token which is being vested
     */
    function _releasableAmount(IERC20Upgradeable __token) private view returns (uint256) {
        return _vestedAmount(__token).sub(_released[address(__token)]);
    }
     ...
     * @param __token ERC20 token which is being vested
     */
    function _vestedAmount(IERC20Upgradeable __token) private view returns (uint256) {
        uint256 currentBalance = __token.balanceOf(address(this));
        uint256 totalBalance = currentBalance.add(_released[address(__token)]);

        if (block.timestamp < _cliff) {
            return 0;
        } else if (block.timestamp >= _start.add(_duration) || _revoked[address(__token)]) {
            return totalBalance;
        } else {
