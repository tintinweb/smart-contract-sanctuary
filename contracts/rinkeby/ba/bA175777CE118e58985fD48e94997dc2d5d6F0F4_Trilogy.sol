...
contract Trilogy is ERC721TradableTrilogy {
    ...
    function check_trilogy(uint256 trilogy_id) public {
        ...
        while (verified_julias < 2) {
            ...
            if (owned%(250*5)>0) {
                owned_reduced = owned%(250*5);
            } else {
                owned_reduced = 250*5;
            }

            if      (owned<250*5*1 ) owned_trilogy = uint256(ITrilogyFinder(FINDER_1_ADDRESS).get_trilogy_id(owned_reduced));
            else if (owned<250*5*2 ) owned_trilogy = uint256(ITrilogyFinder(FINDER_2_ADDRESS).get_trilogy_id(owned_reduced));
            else if (owned<250*5*3 ) owned_trilogy = uint256(ITrilogyFinder(FINDER_3_ADDRESS).get_trilogy_id(owned_reduced));
            else if (owned<250*5*4 ) owned_trilogy = uint256(ITrilogyFinder(FINDER_4_ADDRESS).get_trilogy_id(owned_reduced));
            
            if (owned_trilogy==trilogy_id) verified_julias++;
            j++;
        }
