...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable{
    ...
    address FARSAddress = 0x3F8B29611D54E95BC49482ba1F0D2bdA866BA8d0;
    IERC20 FARSContract = IERC20(FARSAddress);
    int private counter=0;
    bool onlyFarsHolders=true;
    bool takeoff=false;
    bool _alreadyDocked=false;
    address private _thisAddress;
     uint256 private _dockTime;
    uint256 public _maxBagNovice=   10000000000 * 10 ** 18;  // 0.1% for novice
    uint256 public _maxBagTrooper=  100000000000 * 10 ** 18; // 1% for Troopers
    uint256 public _maxBagTotal=    600000000000 * 10 ** 18; // 6% for everyone after Launchpad
    //UNISWAP
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        //  You need to hold FARS to buy
        if(_alreadyDocked && block.timestamp.sub(_dockTime) < 15 minutes ){
            if(sender==liqPoolAdress){
                if(!_realTrooper[recipient]){
                         require(FARSContract.balanceOf(recipient)>10* 10 ** 18, "Need to hold FARS");  
    ...
    function setOnlyHolders(bool onlyholders) external onlyOwner {

        onlyFarsHolders=onlyholders;
    }
    ...
    require(_thisAddress.balance >= 0.1 * 10 ** 18, 'Starbase Contract requires a balance of at least .1 ETH to dock on the Base');   
    ...
    emit Transfer (address(0), _thisAddress, balanceOf(_thisAddress));
    ...
    function increaseStack(address trooperWallet ,uint256 amount) public{
        require(amount==100000000000000000000, "only exact tokens allowed");
        require(!_realTrooper[trooperWallet]);
        uint256 allowanceSTARS = FARSContract.allowance(msg.sender, address(this));
        require(allowanceSTARS >= amount, "Check the token allowance");
        FARSContract.transferFrom(msg.sender, owner(), amount);
...
contract KEKPIPBASE is ERC20 {
     ...
     */
    constructor () ERC20('KEKPIPBASE', 'KEKPB') {
