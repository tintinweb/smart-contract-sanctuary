...
contract MasterChef is IMasterChef, ReentrancyGuard {
    ...
    uint256 ACC_UNO_PRECISION = 1e12;
    ...
    function leaveFromPoolInPending(address _to, uint256 _amount) external override nonReentrant {
        ...
        require(((amount * lpPrice) / 1e18 - pendingAmount) > _amount, "UnoRe: withdraw amount overflow");
        ...
        rewardDebt[_to] = accumulatedUno - (((_amount * 1e18 * uint256(poolInfo.accUnoPerShare)) / lpPrice) / ACC_UNO_PRECISION);
    ...
    function getStakedAmountPerUser(address _to) external view returns (uint256 unoAmount, uint256 lpAmount) {
        ...
        uint256 lpPrice = IRiskPool(riskPool).lpPrice();
        unoAmount = (lpAmount * lpPrice) / 1e18;
    }
        ...
        )
    {
        ...
        pendingAmountInUno = (pendingAmount * lpPrice) / 1e18;
