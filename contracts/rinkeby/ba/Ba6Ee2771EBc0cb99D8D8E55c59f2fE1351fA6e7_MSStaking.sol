...
contract MSStaking is Context, AccessControlEnumerable, Pausable {
    ...
    event Stake(address staker, uint256 value, uint256 timestamp);
    ...
    function withdraw(bytes32[] calldata _proof, uint256 _order)
        public
        whenNotPaused
    {
        require(!withdrawnList[_msgSender()], "MSStaking: User has withdrawn");
        require(
            userData[_msgSender()].stakedValue >= _order * nftPrice,
            "MSSTaking: Order is not matched with stake value"
        );
        require(
            _verifyProof(_proof, _order),
            "MSStaking: Proof or order qty is wrong"
        );
        ...
        uint256 finalOrder = _getFinalOrder(_order);
        uint256 spentValue = finalOrder * nftPrice;
        ...
        for (uint256 i = 0; i < finalOrder; i++) {
    ...
    function stake(uint256 _order) public payable whenNotPaused {
        require(
            _order <= maxOrderLimit,
            "MSStaking: Order cannot exceed maxOrderLimit"
        );
        require(
            userData[_msgSender()].stakedValue + msg.value >= _order * nftPrice,
            "MSStaking: Stake value didn't match order"
        );
        require(
            userData[_msgSender()].stakedValue + msg.value >= minStakeValue,
            "MSStaking: Stake value is too low"
        );
        if (userData[_msgSender()].stakedValue == 0) {
            totalStaker += 1;
        }
        userData[_msgSender()].order = _order;
    ...
    function _getLeaf(uint256 _order) internal view returns (bytes32) {
        return
            keccak256(abi.encode(keccak256(abi.encode(_msgSender(), _order))));
    }

    function _verifyProof(bytes32[] calldata _proof, uint256 _order)
        internal
        view
        returns (bool)
    {
        bytes32 leaf = _getLeaf(_order);
        return MerkleProof.verify(_proof, root, leaf);
    }

    function _getFinalOrder(uint256 _order) internal view returns (uint256) {
        if (_order >= userData[_msgSender()].order) {
            return userData[_msgSender()].order;
        } else {
            return _order;
        }
    }
