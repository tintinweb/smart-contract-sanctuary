...
contract GoodOneNFT is ERC721, ERC721Burnable, Ownable {
  ...
  constructor() public ERC721("GoodOne", "GOOD") {
		admin = msg.sender;
        delegateHistory.push(admin);
      	_setBaseURI("https://goodonenft.herokuapp.com/api/posts/?m=");
  }
	 ...
	 * Delegate APIs
	 */
	function mint(address _to, uint256 _tokenId, string memory _tokenURI) public onlyDelegates {
		_mint(_to, _tokenId);
		_setTokenURI(_tokenId, _tokenURI);
	}

	function mintMany(address[] memory _tos, uint256[] memory _tokenIds, string[] memory _tokenURIs) public onlyDelegates {
	    uint256 numMinting = _tos.length;
		require(numMinting == _tokenIds.length && numMinting == _tokenURIs.length, "Invalid arguments");
		for (uint i = 0; i < numMinting; i++) {
	...
	/**
	 * @dev admin function to set minting delegate.
	 *
	 * @param delegate address the address to set minting privileges of
	 * @param privileged bool whether or not this address can mint nftweets
	 */
