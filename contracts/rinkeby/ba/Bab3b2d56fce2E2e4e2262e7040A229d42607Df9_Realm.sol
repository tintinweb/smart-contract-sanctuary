...
contract Realm is ERC721, ERC721Enumerable, IERC721Receiver, ReentrancyGuard, Ownable {
	...
	function increaseSupplyAndMint(address to, uint256 count) external nonReentrant onlyOwner {
...
library Util {
	function _tokenURI(uint256 tokenId, string memory _tt) public pure returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Cell #', toString(tokenId), '", "description": "This is the REALM. It exists to build upon.", "image_data": ', Base64.encode(bytes(output)), '"}'))));
...

{
  ...
  "libraries": {
    "/contracts/Realm.sol": {
      "Util": "0xE45498751A3DE8188341696eB770d277307c49aC"
    }
  },
  "outputSelection": {
    "*": {
