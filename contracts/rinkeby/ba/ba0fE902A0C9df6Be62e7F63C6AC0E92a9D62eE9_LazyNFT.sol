...
abstract contract ERC2981 is IERC165, IERC2981 {
  ...
  function royaltyInfo(uint256 tokenId, uint256 salePrice) external view override returns (address receiver, uint256 royaltyAmount) {
    ...
    return (royalty.recipient, (salePrice * royalty.value) / 1000);
  }
}

contract LazyNFT is ERC721URIStorage, Ownable, ERC2981 {
  ...
  constructor() ERC721("Lazy", "LAZY") {}

  function supportsInterface(bytes4 interfaceId) public view virtual override (ERC721, ERC2981) returns (bool) {
    return ERC721.supportsInterface(interfaceId) || ERC2981.supportsInterface(interfaceId);
  }

  function mint(string memory _uri, uint256 royaltyValue) public returns (uint256) {
    ...
    if (royaltyValue > 0) {
      _setTokenRoyalty(id, msg.sender, royaltyValue);
    }
