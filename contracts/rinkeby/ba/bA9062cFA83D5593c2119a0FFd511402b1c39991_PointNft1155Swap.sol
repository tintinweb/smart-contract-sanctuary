...
contract PointNft1155Swap is Ownable, ReentrancyGuard, IERC1155Receiver {
  ...
  function swapByRainbow(uint256 _id, uint256 _amount)
    external payable nonReentrant {
      ...
      if (msg.value.mul(rainbowToEth) < _totalPrice) {
        rainbowPool.burn(_msgSender(), _totalPrice.sub(msg.value.mul(rainbowToEth)));
      }
  ...
  function withdraw(uint256 _amount)
    external onlyOwner {
      require(_amount <= address(this).balance, "PointNft1155Swap: not enough balance");
