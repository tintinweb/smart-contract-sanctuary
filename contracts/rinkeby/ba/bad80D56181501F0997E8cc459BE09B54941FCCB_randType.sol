...
contract randType {
    ...
    function _randType() external {
        ...
        for (uint32 i = 0; i < availableWeightIndex -1; i++) {
            probStep = maxUInt32 * uint40(availableWeight[i]) / uint40(sumWeight);
            if (i == 0) {
        ...
        probRange[availableWeightIndex - 1] = uint32(maxUInt32);

        // Find the index of probRange where the randValue is in
        uint32 probRangeIndex = 0;
        uint32 typeIndex = 0;
        uint32 randValue = rand();
        for (typeIndex = 0; typeIndex < availableType.length; typeIndex++) {
            if (!availableType[typeIndex]) {
                continue;
            }
            else {
                ...
                probRangeIndex = probRangeIndex + 1;
            }
        }
