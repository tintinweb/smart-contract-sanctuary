...
function setClaimAmount(address user) internal returns (uint256){
  ...
  uint owned=0;
    ...
    owned=getOwned(user,owned);
    if(owned<777){
    amount=amount+(block.number-lastClaim[owned])*3*RewardTokenPerBlock;
    }else{
    amount=amount+(block.number-lastClaim[owned])*apeTypes[owned]*RewardTokenPerBlock;
        }
     lastClaim[owned]=block.number;
    }
...
function getClaimAmount(address user) internal returns (uint256){
  require(hidden==false,"Apes Not Revealed Yet");
  uint256 amount;
  uint owned=0;
    for(uint i = 0; i < balanceOf(user); i++){
