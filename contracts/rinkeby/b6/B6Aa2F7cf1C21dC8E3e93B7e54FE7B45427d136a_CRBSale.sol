...
contract CRBSale is Context, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor (
        ...
        require(address(_usdt) != address(0), "CRBSale: usdt cannot be zero address");
        require(address(_token) != address(0), "CRBSale: token cannot be zero address");
    ...
    function buyWithUSDT(uint256 _weiAmount) public {
        require(_weiAmount >= minUsdtInvestment, "CRBSale: weiAmount not valid");
     ...
     */
    function _buyTokens(uint256 _weiAmount, BuyType _buyType) internal nonReentrant {
    ...
    function _preValidatePurchase(address _beneficiary, uint256 _weiAmount) internal view {
        require(_beneficiary != address(0), "CRBSale: beneficiary is the zero address");
        require(_weiAmount != 0, "CRBSale: weiAmount is 0");
        require(
            block.timestamp >= startDate && block.timestamp <= endDate,
            "CRBSale: Sale period is ended"
        );
    }
