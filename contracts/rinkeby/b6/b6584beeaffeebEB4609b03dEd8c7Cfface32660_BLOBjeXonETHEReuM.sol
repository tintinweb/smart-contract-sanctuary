...
contract BLOBjeXonETHEReuM is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
for(uint i = 0; i < _mintAmount; i++) {
            uint mintIndex = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % 3300;
            mintIndex = mintIndex + 33;
