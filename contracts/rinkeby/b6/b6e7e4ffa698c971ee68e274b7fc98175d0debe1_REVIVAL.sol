...
contract REVIVAL is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "REVIVAL #', toString(tokenId), '", "description": "REVIVAL is randomized PSYOPS generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use REVIVAL in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        require(totalSupply() + numberOfTokens <= 7778 , "Purchase would exceed max supply of tokens");
