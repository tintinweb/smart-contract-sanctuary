...
contract NFTSimple is ERC721, ChainlinkYoutube, Ownable{
    ...
    constructor () public ERC721 ("Champions", "WWE"){
        tokenCounter = 1;
    }
    ...
    function createCollectible(string calldata _uri) public onlyOwner returns (uint256) {
        ...
        data.uri = _uri;
        _safeMint(msg.sender, newItemId,_uri);
        tokenCounter = tokenCounter + 1;
        return newItemId;
    }
    
    function updateCollectiables(uint256 _tokenID,uint256 _dynamicRating, string memory _username, string memory _id) onlyOwner public{
        uriDetails storage data = dataInfo[_tokenID];
        data.dynamicRating = _dynamicRating;
         updateFollowerCount(_tokenID, _username, _id);
 ...
 */
 contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     * @dev See {IERC721-safeTransferFrom}.
     */
    function safeTransferFrom(address from, address to, uint256 tokenId, string memory _data) public virtual override{
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
     ...
     */
    function _safeTransfer(address from, address to, uint256 tokenId, string memory _data) internal virtual {
        _transfer(from, to, tokenId);
     ...
     */
    function _checkOnERC721Received(address from, address to, uint256 tokenId, string memory _data)
...
interface IERC721 is IERC165 {
    ...
    /**
      ...
      * Emits a {Transfer} event.
      */
    function safeTransferFrom(address from, address to, uint256 tokenId, string calldata data) external;
}
...
interface IERC721Receiver {
     ...
     */
    function onERC721Received(address operator, address from, uint256 tokenId, string calldata data) external returns (bytes4);
}
