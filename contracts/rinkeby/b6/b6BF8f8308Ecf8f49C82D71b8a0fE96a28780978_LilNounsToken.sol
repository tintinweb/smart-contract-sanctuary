...
import {ILilNounsToken} from "./interfaces/ILilNounsToken.sol";
    ...
    Ownable
{
    ...
    bool public dynamicPriceEnabled = false;
    ...
    mapping(uint256 => uint256) private _lilnounForId;
    ...
    // Uses blockhash and tokenId as inputs for random() for new LilNouns, or returns existing LilNouns
    function getLilNounType(uint256 tokenId) private view returns (uint256) {
        if (_lilnounForId[tokenId] == 0) {
            uint256 rand = random(
                string(
                    abi.encodePacked(
            ...
            uint256 lilnounType = rand % 19;

            return lilnounType;
        } else {
            return _lilnounForId[tokenId] - 1;
        }
    }
    ...
    function fetchLilNoun(uint256 tokenId) public view returns (string memory) {
        require(isNounValid(tokenId), "invalid noun");
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Lil Noun ',
                        toString(tokenId),
                        '", "description": "Bleep Blop Boop LilNoun ',
                        toString(tokenId),
    ...
    // Mint function that saves the LilNoun Type (for future tokenURI reads), mints the supplied tokenId and increments counters
    function mint(address destination, uint256 tokenId) private {
        require(
            isNounValid(tokenId),
            "This noun does not exist (yet). So its Lil Noun cannot exist either."
        );

        _lilnounForId[tokenId] = getLilNounType(tokenId) + 1;
    ...
    function publicMint(uint256 tokenId) public payable virtual {
        ...
        require(
            tokenId % 10 != 0,
            "Every 10th Lil Noun is reserved for Noun holders and Nounders."
        );
    ...
    // Allows Noun holders to mint any Lil Noun for a supplied tokenId, including every 10th LilNoun
        ...
        onlyOwner
    {
        ...
        //require(reservedTokenURI(tokenId) == _msgSender(), "Addr not reserved tokens owner.");
    ...
    function getCurrentPrice() public view returns (uint256 dynamicPrice) {
        ...
        // Otherwise, price = priceIncrement added to itself, once for every 100 LilNoun mints
    ...
    // Sets maximum number of Lil Nouns allowed to minted by one address
