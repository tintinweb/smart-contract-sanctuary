...
contract NFTMarket is OrderState, TransferSafe {
	...
	INFTMintableStore private _NFTMintableStore;
	address public weth;
		...
		if (
			...
			if (order.orderType == 1 || order.orderType == 3) {
				if (!isStore) {
					...
					require(
						order.key.sellAsset.token == address(_NFTMintableStore),
						"invalid sell asset"
					);
					erc1155safeMintTransferFrom(
						INFTMintableStore(order.key.sellAsset.token),
						order.key.owner,
						buyer,
						order.key.sellAsset.tokenId,
						1,
				...
				if (!isStore) {
					...
					require(
						order.key.buyAsset.token == address(_NFTMintableStore),
						"invalid buy asset"
					);
					erc1155safeMintTransferFrom(
						INFTMintableStore(order.key.buyAsset.token),
						buyer,
						order.key.owner,
						order.key.buyAsset.tokenId,
						1,
	...
	function setMintableStore(address newMintableStore) external onlyOwner {
		...
		_NFTMintableStore = INFTMintableStore(newMintableStore);
	...
	function NFTMintableStore() external view returns (address) {
		return address(_NFTMintableStore);
	}
}