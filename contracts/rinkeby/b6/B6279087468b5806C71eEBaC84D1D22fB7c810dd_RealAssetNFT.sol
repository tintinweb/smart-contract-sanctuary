    ...
    ReentrancyGuard
{
    ...
    uint256 public immutable preSaleDuration;// PreSale duration of 24hours in seconds.
    uint256 public constant publicSaleBufferDuration=30;//After end of preSale the public sale will start after 30 second .
    uint256 public constant maxTokenPerMint = 5; //Maximum No of token can be purchased by user in single tx in public sale.
    ...
    //mintCost 50000000000000000 wei (0.05 ether)
    //preSaleDuration 86400 sec ie 24 hours 
    //SignerAddress
    //MaxSupply 8888
    ...
    function mint(uint256 tokenQuantity) external nonReentrant() payable saleNotEnded saleStarted preSaleEnded {
        ...
        require((totalMint + tokenQuantity) <= MaxSupply,"Sorry, all tokens are sold out.");
        require((mintCost * tokenQuantity) <= msg.value, "you need to pay the minimum token price.");
        require(msg.sender != address(0),"ERC1155: mint to the zero address");
