...
contract ticket204 is ERC721, Ownable, PaymentSplitter {
  using Strings for uint256;
  ...
  uint256 public LooksCost = 18.8 ether; 
  ...
  //public
  function avaliableFreeMint(uint256 _mintAmount) public view returns (bool) {
    if (_mintAmount + mintBalances[msg.sender] <= maxFreeMintAmount)
     return false;
  ...
  function freeMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {    
    ...
    require(avaliableFreeMint(_mintAmount), "You don't own FREE NFTs");
  ...
  function presaleMint(bool islooks, uint256 _mintAmount, bytes32 _hash, bytes memory _signature) public payable mintCompliance(_mintAmount) {
    ...
    require(Presalesupply.current() + _mintAmount <= maxPresale, "Invalid mint amount!");
    require(_checkHash(_hash, _signature, SIGNER), "Address is not on Presale List");
    _mintCost(islooks,_mintAmount);
    _mintLoop(msg.sender, _mintAmount);
    Presalesupply._value += _mintAmount;
  }
