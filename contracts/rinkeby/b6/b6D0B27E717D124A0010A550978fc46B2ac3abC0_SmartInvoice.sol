...
contract SmartInvoice is ISmartInvoice, IArbitrable, Initializable, Context, ReentrancyGuard {
  ...
  function release(uint256 _milestone) external override initialized nonReentrant {
  ...
  function releaseTokens(address _token) external override initialized nonReentrant {
    if (_token == token) {
      _release();
    } else {
      require(_msgSender() == client, "!client");
  ...
  function withdrawTokens(address _token) external override initialized nonReentrant {
    if (_token == token) {
      _withdraw();
    } else {
      require(block.timestamp > terminationTime, "!terminated");
  ...
  function lock(bytes32 _details) external payable override initialized nonReentrant {
    require(!locked, "locked");
    uint256 balance = IERC20(token).balanceOf(address(this));
    require(balance > 0, "balance is 0");
    require(block.timestamp < terminationTime, "terminated");
