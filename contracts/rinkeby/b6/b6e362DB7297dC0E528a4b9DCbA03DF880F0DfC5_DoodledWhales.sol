...
contract DoodledWhales is Ownable, Pausable, ERC721Enumerable {
    using Strings for uint256;

    string public baseURI;
    string public unrevealedURI;
    string public baseExtension;
    uint256 public baseCost = .0069 ether;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            require(
                _mintAmount <= maxMintPerTransaction,
                "max mint amount per transaction exceeded"
            );
            require(
                _mintAmount + addressMintedBalance[msg.sender] <= maxMintAmount,
                "max mint amount by address exceeded"
            );
            require(
                msg.value == baseCost * _mintAmount,
