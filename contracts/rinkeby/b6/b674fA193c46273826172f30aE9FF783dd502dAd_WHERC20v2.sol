...
interface IWhiteUSDCPool is ILiquidityPool {
    function lock(uint id, uint256 amountToLock, uint256 premium, uint256 settlementFee) external;
...
abstract contract WHAssetv2 is ERC721, IWHAssetv2, Ownable {
    ...
    function wrapCost(uint amount, uint period) view external returns (uint cost){
        ...
        (cost, , ,) = whiteOptionsPricer.getOptionPrice(period, amount, strike);
    }
    ...
    function _wrap(uint amount, uint period, address to, bool receiveAsset, bool _mintToken, uint minPremiumUSDC) internal returns (uint newTokenId){
        ...
        (uint total, uint settlementFee, , ) = whiteOptionsPricer.getOptionPrice(period, amount, strike);

        // receive asset + cost of hedge
        if(receiveAsset) _receiveAsset(msg.sender, amount, total);
        // buy option
        _createHedge(newTokenId, total, settlementFee, period, amount, strike, to, minPremiumUSDC);
     ...
     * @param settlementFee part of the fee that goes to the stakers
     ...
     */
    function _createHedge(uint tokenId, uint totalFee, uint settlementFee, uint period, uint amount, uint strike, address owner, uint minPremiumUSDC) internal {
        ...
        uint _totalFee = totalFee;
        uint premiumStablecoin;
        uint settlementFeeStablecoin;
        
        {
            uint premiumPercentage = _totalFee.sub(settlementFee).mul(1000).div(_totalFee);
            ...
            premiumStablecoin = premiumPercentage.mul(totalStablecoin).div(1000);
            settlementFeeStablecoin = totalStablecoin.sub(premiumStablecoin);
        }

        pool.lock(tokenId, collateral, premiumStablecoin, settlementFeeStablecoin);
    }
