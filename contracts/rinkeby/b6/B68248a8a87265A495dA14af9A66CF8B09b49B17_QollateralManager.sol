...
contract QollateralManager is Initializable, IQollateralManager {
  ...
  uint private _maintenanceLiquidityRatio;
  ...
  uint private _initialLiquidityRatio;
  ...
  function initialize(address admin_, address qontrollerAddress_) public initializer {
    ...
    // Set initial values for liquidity ratios
    _maintenanceLiquidityRatio = 1e8;
    _initialLiquidityRatio = 1.1e8;
  }
  ...
  function _setInitialLiquidityRatio(uint initialLiquidityRatio_) external {

    // Only `admin` may call this function
    require(msg.sender == _admin, "unauthorized");

    // `_initialLiquidityRatio` cannot be below `_maintenanceLiquidityRatio`
    require(initialLiquidityRatio_ >= _maintenanceLiquidityRatio, "must be greater than maintenance");
    
    _initialLiquidityRatio = initialLiquidityRatio_;
  }
  ...
  /// @notice Check the liquidity ratio including a request for new borrow in
  /// some `Market`. This is calculated as:
  ...
  /// @param account User account
  /// @param requestedMarketAddress Address of the `Market` user is requesting to borrow
  /// @param requestedAmount Amount user is requesting to borrow
  /// @return uint Current liquidity ratio
  function checkLiquidityRatio(
                               address account,
                               address requestedMarketAddress,
                               uint requestedAmount
                               ) external view returns(uint){
    return _checkLiquidityRatio(account, requestedMarketAddress, requestedAmount);
  }
  ...
  function liquidityRatio(address account) external view returns(uint){
    return _checkLiquidityRatio(account, address(0), 0);
  }
  ...
  /// @notice Get the current net value borrowed (i.e. borrows - lends) in USD for a
  /// particular Market
  ...
  function borrowValue(
    ...
    return _checkBorrowValue(marketAddress, account, 0);
  }

  /// @notice Get the current net value borrowed (i.e. borrows - lends) in USD for all
  /// Markets participated in by the user
  /// @param account Account to query
  /// @return uint Borrow value of account in USD
  function totalBorrowValue(address account) external view returns(uint){
    return _checkTotalBorrowValue(account, address(0), 0);
  }

  /// @notice Check the net value borrowed (i.e. borrows - lends) including a
  /// request for new borrow in a particular market, in USD for all Markets
  /// participated in by the user.
  /// @param account Account to query
  /// @param requestedMarketAddress Address of the Market user is requesting to borrow
  /// @param requestedAmount Amount user is requesting to borrow
  /// @return uint Borrow value of account in USD
  function checkTotalBorrowValue(
                                 address account,
                                 address requestedMarketAddress,
                                 uint requestedAmount
                                 ) external view returns(uint){
    return _checkTotalBorrowValue(account, requestedMarketAddress, requestedAmount);
  }
  ...
  function _checkLiquidityRatio(
    ...
    // The denominator is the net total borrow value plus new requested amount
    uint denom = _checkTotalBorrowValue(account, requestedMarketAddress, requestedAmount);
  ...
  /// @param requestedAmount Amount user is requesting to borrow
  /// @return uint Borrow value of account in USD
  function _checkTotalBorrowValue(
                                  address account,
                                  address requestedMarketAddress,
                                  uint requestedAmount
                                  ) internal view returns(uint){
    ...
    for(uint i=0; i<_iterableAccountMarkets[account].length; i++){
      ...
      if(marketAddress == requestedMarketAddress){
        // User requesting to borrow more in this `Market`, add the amount to borrows
        totalValueUSD += _checkBorrowValue(marketAddress, account, requestedAmount);
      }else{
        // User not requesting to borrow more in this `Market`, just get current value
        totalValueUSD += _checkBorrowValue(marketAddress, account, 0);
      }
    }
    return totalValueUSD;    
  }
  ...
  /// @notice Check the net value borrowed (i.e. borrows - lends) including a
  /// request for new borrow in USD for a particular market
  /// @param account Account to query
  /// @param requestedAmount Amount user is requesting to borrow
  /// @return uint Borrow value of account in USD
  function _checkBorrowValue(
                             address marketAddress,
                             address account,
                             uint requestedAmount
                             ) internal view returns(uint){
    ...
    // Total `borrowsInLocal` should be current borrow plus `requestedAmount`
    uint borrowsInLocal = market.accountBorrows(account) + requestedAmount;

    // Total `lendsInLocal` is just the user's balance of qTokens
