...
contract ETHRegistrarControllerWrapper is Ownable {
    ...
    bytes4 constant private COMMITMENT_WITH_CONFIG_CONTROLLER_ID = bytes4(
        keccak256("registerWithConfig(string,address,uint256,bytes32,address)") ^
    ...
    uint96 constant ETH2LD_FUSE = 1 | 32;
    ...
    function makeCommitment(string memory name, address owner, bytes32 secret) pure public returns(bytes32) {
        return makeCommitmentWithConfig(name, owner, secret, address(0));
    }
    ...
    function register(string calldata name, address owner, uint duration, bytes32 secret) external payable {
      registerWithConfig(name, owner, duration, secret, address(0));
    }
    ...
    function registerWithConfig(string memory name, address owner, uint duration, bytes32 secret, address resolver) public payable {
        ...
        bytes32 label = keccak256(bytes(name));
        uint expires = wrapper.registerAndWrapETH2LD(name, owner, duration, resolver, ETH2LD_FUSE);
