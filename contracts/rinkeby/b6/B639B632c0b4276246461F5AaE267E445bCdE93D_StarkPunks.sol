...
// File contracts/StarkPunksDNA.sol



pragma solidity ^0.8.0;

contract StarkPunksDNA {
...
// File contracts/StarkPunks.sol


pragma solidity ^0.8.0;
contract StarkPunks is ERC721, ERC721Enumerable, StarkPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("StarkPunks", "SPKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No StarkPunks left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "StarkPunks #',
                tokenId.toString(),
                '", "description": "Stark Punks are randomized avatars stored on chain.", "image": "',
                image,
                '"}'
            )
        );
