...
contract P is ERC721, Ownable {
    ...
    function claim() external {
        ...
        require(pSupply.current() + 1 <= maxSupply, "Purchase would exceed max supply of tokens.");
		require(addressClaimed[msg.sender] < 1, "Only one claim per wallet.");
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "P #', toString(tokenId),'","attributes": [ { "trait_type": "Not ', unicode"🅿️" ,'", "value": "','False','" }]',', "description": "If you own this NFT, you',"'" ,'re ', unicode"🅿️.",'", "image": "data:image/svg+xml;base64,',Base64.encode(bytes(output)),'"}'))));
