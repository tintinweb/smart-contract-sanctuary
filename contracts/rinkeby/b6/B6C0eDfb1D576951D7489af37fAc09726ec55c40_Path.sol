...
contract Path is ERC721Enumerable, ERC721URIStorage, VRFConsumerBase, Ownable, PathHelper {
  ...
  function whitelistMintPath(uint256 quantity) public payable {
    ...
    for (uint256 i = 0; i < quantity; i++) {
      ...
      uint pathCount = generatePaths(pathAttrStr);
      uint windTurbulence = generateWind(pathAttrStr);
      uint template = checkTemplate(pathAttrStr);
      string memory pathSeed = getPathSeed(pathAttrStr);
      string memory windSeed = getWindSeed(pathAttrStr);
      
      tokenIdAttr[mintIndex] = PathToken(mintIndex, template, pathCount, windTurbulence, pathSeed, windSeed);
...
contract PathHelper {
    ...
    uint32 [] pathWeights = [10,20,40,80,150,400,700,900,1000,1700,1700,1000,900,700,400,150,80,40,20,10];
    uint32 [] windWeights = [10,20,40,80,150,400,700,900,1000,1700,1700,1000,900,700,400,150,80,40,20,10];
    
    function getPathWeightsLength() internal view returns (uint){
        return pathWeights.length;
    }
    
    function getWindWeightsLength() internal view returns (uint){
        return windWeights.length;
    }
    ...
    function checkTemplate(string memory _pathAttr) internal pure returns (uint256 template) {
        string memory legendStr = getSubstring(2,5,_pathAttr);
        uint256 legendAttr = str2Uint(legendStr);
        if(legendAttr >= 9990){ //chance of getting lengendary
            return 2;
        }
        else if(legendAttr >= 9000){ //ascended chance
            return 1;
        }
        else return 0;
    }
    
    function generatePaths(string memory _pathAttr) internal view returns (uint256 pathCount) {
        string memory pathAttrStr = getSubstring(6,9,_pathAttr);
        uint256 pathAttr = str2Uint(pathAttrStr);
        for(uint i=0; i < getPathWeightsLength(); i++){
    ...
    function generateWind(string memory _pathAttr) internal view returns (uint256 windStrength) {
        string memory windAttrStr = getSubstring(10,13,_pathAttr);
        uint256 windAttr = str2Uint(windAttrStr);
        for(uint i=0; i < getWindWeightsLength(); i++){
    ...
    function getPathSeed(string memory _pathAttr) internal pure returns (string memory pathSeed) {
        return getSubstring(15,21,_pathAttr);
    }
    
    function getWindSeed(string memory _pathAttr) internal pure returns (string memory windSeed) {
        return getSubstring(22,28,_pathAttr);
    }
}
