...
contract BearXNFTStaking is OwnableUpgradeable{
    ...
    function initialize() public initializer{
        __Ownable_init();
        BearXNFTAddress = 0x9229C81078Ce77aB81658D4F504691ceA2aD2724;
        ROOTxTokenAddress = 0x4f37dd2A0c87FE6Ae9b14A93C4C76165bD092aC8;
        SROOTxTokenAddress = 0x355e68d6D79516735156120A451B5A8A48cF39F0;
        totalStakes = 0; 
        MaxSROOTXrate = 100*10**18;
        MinSROOTXrate = 50*10**18;
        MaxRate = 11050*10**18;
        ...
        if(sumofgenesisbear >= 5){
            for(uint256 i = 0; i < stakes[_addr].length; i++){
                if(isSpecialBear(stakes[_addr][i].nft_id)){
                    ...
                    finalOfSROOTxToken = finalOfSROOTxToken.add(200 * dd);
                     status = true;
                }
            }
            if(flag && stakes[_addr].length != 0){
            ...
            for(uint256 i = 0; i < stakes[_addr].length; i++){
                if(isSpecialBear(stakes[_addr][i].nft_id)){
                    finalOfSROOTxToken = finalOfSROOTxToken.add(200);
                     status = true;
                }
            }   
        }
         ...
         if(sumofgenesisbear >= 5){
            for(uint256 i = 0; i < stakes[_addr].length; i++){
                if(isSpecialBear(stakes[_addr][i].nft_id)){
                    ...
                    finalOfWETH = finalOfWETH.add(200 * dd);
                    status = true;
