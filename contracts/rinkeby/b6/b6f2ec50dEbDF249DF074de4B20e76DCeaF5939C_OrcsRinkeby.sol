...
import "../../EtherOrcs-PostMint.sol";

contract OrcsRinkeby is EtherOrcs {

    function mint(uint256 id) public {
        _mint(msg.sender, id);
    }

    function updateOrc(uint256 id, uint8 body, uint8 helm, uint8 mainhand, uint8 offhand, uint16 level, uint16 zugModifier, uint16 lvlProgress) external {
        orcs[id].body = body;
        orcs[id].helm = helm;
    ...
    function setRaidsAddress(address r_) public {
        raids = r_;
    }
    
    function setZug(address z_) public {
        zug = ERC20(z_);
    }
}
