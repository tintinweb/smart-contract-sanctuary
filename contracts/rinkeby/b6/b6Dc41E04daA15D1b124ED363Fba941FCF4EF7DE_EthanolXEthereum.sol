...
contract EthanolXEthereum is ERC20, ERC20Burnable, ERC20Permit, AccessControl {
    address public marketingWallet;

    // Tax system state variables
    uint256 public taxPercentage;
    uint256 public taxDivisor;

    mapping(address /* account */ => bool /* excluded status*/) public excluded;

    event Excluded(address indexed admin, address indexed account, bool indexed status);
    event Tax(uint256 indexed taxPercentage, uint256 indexed taxDivisor);

    constructor(address _marketingWallet) ERC20("EthanolX", "ENOL") ERC20Permit("EthanolX") {
    ...
    function _transfer(
        ...
        uint256 _taxAmount = _calculateTaxAmount(sender, recipient, amount);
        super._transfer(sender, recipient, amount - _taxAmount);
        
        if(_taxAmount > 0) {
            uint256 _splittedAmount = _taxAmount / 2;
            super._transfer(sender, marketingWallet, _splittedAmount);
            _burn(sender, _splittedAmount);
        }
    }

    function _calculateTaxAmount(address _sender, address _recipient, uint256 _amount) private view returns(uint256) {
        if(excluded[_sender] || excluded[_recipient] || taxPercentage == 0 || taxDivisor == 0) return 0;
        return (_amount * taxPercentage) / taxDivisor;
    }

    function setTax(uint256 _taxPercentage, uint256 _taxDivisor) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "EthanolX: Caller doesn't have ADMIN role");
        emit Tax(_taxPercentage, _taxDivisor);
    ...
    function setExcluded(address _account, bool _status) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "EthanolX: Caller doesn't have ADMIN role");
        emit Excluded(_msgSender(), _account, _status);
