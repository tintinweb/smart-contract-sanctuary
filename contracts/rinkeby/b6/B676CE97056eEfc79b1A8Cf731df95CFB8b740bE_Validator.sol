...
contract Validator is Ownable {
  ...
  bytes32 internal constant BUYYER = "BUYER";
  ...
  event AddCase(uint256 key,string txId, address seller, address buyer, uint256 amount);
  event ClaimReward(uint256 key, address user, bool result);
  event EvaluateResult(uint256 key, bytes32 result,uint256 buyerAmount,uint256 sellerAmount);
  ...
  function addCase(
    ...
    emit AddCase(key,_txId, _seller, _buyer, _amount);
  }
  ...
  function evaluate(uint256 _key) external onlyAdmin returns (bytes32,uint256,uint256 ) {
    CaseInfo storage caseInfo = casesInfo[_key];
    ...
    for (uint256 i = 0; i < caseInfo.users.length; i++) {
      ...
      bytes32 answer = keccak256(abi.encodePacked(userReplyAnswer.answer, _key,userAddress));
      if (answer == BUYYER) {
