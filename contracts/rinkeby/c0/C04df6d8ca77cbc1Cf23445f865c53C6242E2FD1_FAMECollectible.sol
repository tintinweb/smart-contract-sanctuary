...
contract FAMECollectible is ERC1155Tradable {
  ...
  function legacyConversion() public
  {
    ...
    uint256 mintAmount = convertingBalance / (legacy_zeros*10);
  ...
  function convertBaseCredits(uint256 targetID, uint256 amount) public
  {
    ...
    if(targetID==2) 
    {
      receiveAmount = amount * 1000;  //1000 FAME Tokens per Founder Credit
      burnAmount = amount;
    }
    if(targetID==3) 
    {
      receiveAmount = amount / (10);  //0.1 Senate Seat per Founder Credit
