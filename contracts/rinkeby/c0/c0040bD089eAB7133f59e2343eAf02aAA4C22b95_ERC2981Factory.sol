...
contract ERC2981Factory {
    ...
    address private immutable niftyRegistryContract;
    ...
    function createGlobalRoyaltyInfo(address _recipient, uint256 _value, address _tokenAddress) public onlyValidSender returns (address) {
        address clone = Clones.clone(tokenImplementation);
        ERC2981(clone).initialize(_recipient, _value, _tokenAddress, true);
    ...
    function createTokenRoyaltyInfo(address _tokenAddress) public onlyValidSender returns (address) {
        address clone = Clones.clone(tokenImplementation);
        ERC2981(clone).initialize(address(0), 0, _tokenAddress, false);
        emit ERC2981Created(clone);
        return clone;
    }
...
contract ERC2981 is ERC165, IERC2981, Initializable {
    ...
    function initialize(
        ...
        bool _globalRoyaltiesEnabled
    ) public initializer {
        tokenAddress = _tokenAddress;
        contractEnabled = true;

        if (_globalRoyaltiesEnabled) {
    ...
    function _setTokenRoyalty(
        ...
        _royalties[tokenId] = RoyaltyInfo(uint24(value), recipient);
    }
    ...
    function setGlobalRoyalties(address recipient, uint256 value)
        public
        onlyValidSender
    {
        require(contractEnabled, "Contract disabled");
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        if (globalInfo.enabled) {
            ...
            receiver = _royalties[_tokenId].recipient;
            basis = _royalties[_tokenId].amount;
        }
    ...
    function setGlobalRoyaltiesEnabled(bool _globalRoyaltiesEnabled)
        public
        onlyValidSender
    {
