...
contract Squirrels is ERC721Enumerable, Ownable {
  ...
  (((@%(((((#######(##(((((((((((((((((((((####((((((((((((%#####(#(((((((@#@
    ...
    */
    ...
    uint256 private constant maxPresaleSupply = 2000; // max amount that presale users can mint during presale
    uint256 private constant maxSupply = 5555; // max amount of squirrels

    // togglables
    bool public presaleActive = false;
    bool public publicSaleActive = false;
    bool public locked = false; // is the metadata frozen/locked

    // Team Addresses
    address private constant addr1 = 0x7e94A79DCf8CF174827986115196e4C400D15d72;
    address private constant addr2 = 0x417b8FBCc14d4c22464f433C4b40925710bfE096;
    address private constant addr3 = 0xb01A161e4F966D0EB581e451f38A58a5B5660E90;
    address private constant addr4 = 0xce2eEcd330171BCFa5d67c64F76ea75bb8C222Cc;
    ...
    constructor() public ERC721("Sneaky Squirrels Test", "TSQUIRRELS") {} // setup contract, first value is the name, second is the symbol or shorthand

    // Add array of addresses to presale
    function addToPresaleList(address[] calldata addresses) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Address is Blackhole");
            require(
    ...
    // Remove array of addresses from presale
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Address is Blackhole");
            require(whitelisted[addresses[i]], "Address is not whitelisted");
    ...
    function presaleMint(uint256 amount) public payable {
        ...
        require(
            presaleMintedPerUser[msg.sender].add(amount) <=
                maxCountForPresaleUser,
        ...
        presaleMintedPerUser[msg.sender] = presaleMintedPerUser[msg.sender].add(
            amount
        );
    ...
    function mintForOwner(uint256 amount) public onlyOwner {
        ...
        // loop 10 times, if the total supply is under the max, mint with id INDEX+1
        for (uint256 i = 0; i < amount; i++) {
            uint256 mintIndex = totalSupply().add(1);
            if (totalSupply() < maxSupply) {
                _safeMint(msg.sender, mintIndex);
    ...
    function withdraw() public onlyOwner {
        ...
        // 39.5%
        require(
            payable(addr1).send(balance.mul(445).div(1000)),
            "addr1 failed"
        );
        // 39.5%
        require(
            payable(addr2).send(balance.mul(445).div(1000)),
            "addr2 failed"
        );
        // 10%
        require(payable(addr3).send(balance.mul(10).div(100)), "addr3 failed");
        // 1%
        require(payable(addr4).send(balance.mul(1).div(100)), "addr4 failed");
    }
    ...
    // Does some magic with 0 cost contract reads, basically this gives the url of where the metadata is stored
    ...
    function setBaseURI(string memory uri) external onlyOwner {
        require(!locked, "Metadata is locked.");
        baseURI = uri;
    }
