...
contract AlbumFactory {
    ...
    function createAlbum(
        ...
        address sale = createAndSetupTokenSale(
            token,
            tokenSaleParams,
            ensSubNode
        );
    ...
    function getEnsSubnode(string memory name)
        internal
        view
        returns (bytes32 subnode)
    {
        bytes32 nameHash = keccak256(abi.encodePacked(name));
        subnode = keccak256(abi.encodePacked(BASE_ENS_NODE, nameHash));
    }

    function predictSafeAddress(string memory name)
        public
        view
        returns (address)
    {
        return FACTORY_SAFE_HELPER.predictSafeAddress(getEnsSubnode(name));
    }

    function predictRealityModuleAddress(string memory name)
        public
        view
        returns (address)
    {
        return
            FACTORY_SAFE_HELPER.predictRealityModuleAddress(
                getEnsSubnode(name)
            );
    }

    function predictSaleAddress(string memory name)
        public
        view
        returns (address predicted)
    {
        predicted = Clones.predictDeterministicAddress(
            TOKEN_SALE_MASTER_COPY,
            getEnsSubnode(name)
        );
    }

    function predictTokenAddress(string memory name, string memory symbol)
        public
        view
        returns (address predicted)
    {
        ...
        predicted = Create2.computeAddress("", keccak256(bytecode));
    }
