...
import "./PudgyApesInterface.sol";

contract Fridge is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.UintSet;
    using SafeMath for uint256;

    uint public rewardPerInterval = 3 * 10**18;

    // TODO Change to days for final deployment
    ...
    //ERC721 Interface
    PudgyApesInterface public pudgyApes;

    //ERC20 Interface
    SnackInterface public snack;

    constructor(address _pudgyApesAddress, address _snackAddress) {
        pudgyApes = PudgyApesInterface(_pudgyApesAddress);
        snack = SnackInterface(_snackAddress);
    }
    ...
    function stake(uint[] calldata _tokens) external whenNotPaused {
        for(uint i = 0; i < _tokens.length; i++) {
            pudgyApes.safeTransferFrom(msg.sender, address(this), _tokens[i]);
    ...
    function unstake(uint[] calldata _tokens) external whenNotPaused {
        ...
        for(uint i = 0; i < _tokens.length; i++) {
            ...
            pudgyApes.safeTransferFrom(address(this), msg.sender, _tokens[i]);
    ...
    function claim(uint[] calldata _tokens) public whenNotPaused nonReentrant {
        ...
        snack.transfer(msg.sender, rewards);
    }

    function calculateReward(uint _token) public view returns (uint) {
        uint reward = block.timestamp.sub(stakedTokensLastClaimByAddress[msg.sender][_token]).mul(rewardPerInterval).div(interval);
        return reward;
    }
    ...
    function tokensStaked(address _wallet) public view returns (uint[] memory _tokens) {
        uint tokenAmount = stakedTokensByAddress[_wallet].length();

        uint[] memory tokens = new uint[](tokenAmount);
