...
contract MyToken is ERC721 {
    ...
    function buySmallPack() external payable {
        ...
        uint balance = IERC721(token).balanceOf(msg.sender);
        if(balance > 0 || _whitelisted[msg.sender]){
            require(msg.value == whitelistPrice * 5, "Incorrect value!");
            for(uint8 i=0; i<5; i++){
                _safeMint(msg.sender, counter++);
            }
            _maxMintedPerWallet[msg.sender]+= 5;
        }else{ 
            require(msg.value == salePrice * 5, "Incorrect value!");
            for(uint8 i=0; i<5; i++){
                _safeMint(msg.sender, counter++);
            }
            _maxMintedPerWallet[msg.sender]+= 5;
    ...
    function buyBigPack() external payable {
        ...
        uint balance = IERC721(token).balanceOf(msg.sender);
        if(balance > 0 || _whitelisted[msg.sender]){
            require(msg.value == whitelistPrice * 10, "Incorrect value!");
            ...
            require(msg.value == salePrice * 10, "Incorrect value!");
            for(uint8 i=0; i<10; i++){
                _safeMint(msg.sender, counter++);
            }
            _maxMintedPerWallet[msg.sender]+= 10;
