...
contract Monkes is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  function mintPreSale(bytes32[] memory _proof) external payable nonReentrant {
    ...
    require(supply + wlAmount <= 3333 - _reserved, "Exceeds maximum supply");
    require(msg.value >= _price * wlAmount, "Ether sent is not correct");

    for (uint256 i = 1; i <= wlAmount; i++) {
      _safeMint(msg.sender, supply + i);
    }
 
    whitelistClaimed[msg.sender] = true;
    walletCount[msg.sender] += wlAmount;
  }
  ...
  function setwlAmount(uint256 _newWL) public onlyOwner {
    wlAmount = _newWL;
    }
