...
contract Portal is RelayRecipient {
  ...
  modifier onlyBridge {
    ...
    function synthesizeNative(address _chain2address, address _receiveSide, address _oppositeBridge, uint _chainID) whenNotPaused payable external returns (bytes32 txID) {
        ...
        balanceOf[wrapper] = balanceOf[wrapper] + amount;

        txID = keccak256(abi.encodePacked(this, requestCount));

        bytes memory out  = abi.encodeWithSelector(bytes4(keccak256(bytes('mintSyntheticToken(bytes32,address,uint256,uint256,address)'))), txID, wrapper, block.chainid, amount, _chain2address);
        IBridge(bridge).transmitRequestV2(out,_receiveSide, _oppositeBridge, _chainID);
        TxState storage txState = requests[txID];
        txState.recipient    = _msgSender();
        txState.chain2address    = _chain2address;
        txState.rtoken     = wrapper;
        txState.amount     = amount;
        txState.state = RequestState.Sent;

        requestCount +=1;
        emit SynthesizeRequest(txID, _msgSender(), _chain2address, amount, wrapper);
    }
    ...
    function emergencyUnsynthesize(bytes32 _txID) onlyBridge whenNotPaused external{
        ...
        if (txState.rtoken == wrapper) {
            IWrapper(wrapper).withdraw(txState.amount);
            TransferHelper.safeTransferETH(txState.recipient, txState.amount);
        } else {
            TransferHelper.safeTransfer(txState.rtoken, txState.recipient, txState.amount);
        }
    ...
    function emergencyUnburnRequest(bytes32 _txID, address _receiveSide, address _oppositeBridge, uint _chainId) whenNotPaused payable external {
