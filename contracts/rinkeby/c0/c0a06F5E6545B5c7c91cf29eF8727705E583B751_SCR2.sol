    ...
    ERC2981ContractWideRoyalties
{
    ...
    uint256 public totalTokenRequested;
    ...
    // mapping(uint256 => bool) public mapTokenIdToURISet;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _maxTokens; i++) {
            tokenPool.push(i); // for random selection
        }
    }
    ...
    function requestRandomIndex() public payable nonReentrant whenNotPaused {
        ...
        require(tokenPool.length > 0, "Token pool empty!");
    ...
    function fulfillRandomness(bytes32 requestIndex, uint256 randomNumber)
        ...
        override
    {
        uint256 newRandomIndex = (randomNumber % tokenPool.length); // eg if tokenPool.length is 10, newRandomIndex will be 0-9
        require(
            newRandomIndex < tokenPool.length,
        ...
        _setTokenURI(
            ...
            string(
                abi.encodePacked(
                    frontTokenName(newTokenId.toString()),
                    newTokenId.toString(),
                    ".json"
                )
            )
    ...
    // {
    //     /** Current Situation:
    //     since setTokenURI is called after requestRandomIndex,
    //     maybe grey out "button" until full process complete !! / how to deal with page reload? dont reload setTokenURI in JS?
    //     if still cut process halfway, manually call setTokenURI to set!!
    //      */
    //     require(
    //         _isApprovedOrOwner(_msgSender(), _tokenId),
    //         "ERC721: caller is not owner nor approved."
    //     ); // prevents random account from setting URI
    //     // require(_exists(_tokenId), "Token nonexistent."); // _isApprovedOrOwner calls this!!!
    //     require(!mapTokenIdToURISet[_tokenId], "Token URI already set.");

    //     _setTokenURI(_tokenId, _tokenURI);
    //     mapTokenIdToURISet[_tokenId] = true;
    // }
    ...
    function frontTokenName(string memory _token)
        internal
        pure
