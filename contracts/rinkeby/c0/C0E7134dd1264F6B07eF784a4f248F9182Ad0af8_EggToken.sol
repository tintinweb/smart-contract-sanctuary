...
contract EggToken is ERC721, BaseAccessControl {
    ...
    Counters.Counter private _definiteCounter;
    ...
    constructor(
        uint totalEggSupply,
        uint randomLegendaryDragonSupply, 
        uint randomEpic20DragonSupply, 
        uint randomCommonDragonSupply, 
        uint htchTime,
        string memory defaultCid,
        address accessControl,
        address dragonCreator) ERC721("CryptoDragons Eggs", "CDE") BaseAccessControl(accessControl) {
        
        _totalSupply = totalEggSupply;
        _totalDefiniteSupply = 
            totalEggSupply - randomLegendaryDragonSupply - randomEpic20DragonSupply - randomCommonDragonSupply;
        
        _randomDragonSupply[DragonInfo.Types.Legendary] = randomLegendaryDragonSupply;
        _randomDragonSupply[DragonInfo.Types.Epic20] = randomEpic20DragonSupply;
        _randomDragonSupply[DragonInfo.Types.Common] = randomCommonDragonSupply;
    ...
    function mint(address to, DragonInfo.Types _dragonType) external returns (uint) {
        ...
        if (_dragonType != DragonInfo.Types.Unknown) {
            require(_definiteCounter.current() < _totalDefiniteSupply, "EggToken: definite supply is exceeded");
            _definiteCounter.increment();
        }
...
contract DragonCreator is BaseAccessControl {
    ...
    function setGiveBirthCallers(address[] calldata callers, bool value) external onlyRole(CEO_ROLE) {
        for (uint i = 0; i < callers.length; i++) {
            ...
            emit BoolValueChanged(abi.encodePacked("giveBirthCallers.", callers[i]), previousValue, value);
        }
    }
