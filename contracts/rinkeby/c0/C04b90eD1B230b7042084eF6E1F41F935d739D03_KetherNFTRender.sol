...
contract KetherNFTRender is ITokenRenderer {
  ...
  function _renderIdx(IKetherHomepage instance, uint256 tokenId) public view returns (string memory) {
  }
...
<svg width="1000" height="1050" viewBox="0 0 1000 1050" xmlns="http://www.w3.org/2000/svg" style="background:#4a90e2;">
    ...
    <svg width="1000" height="1000" viewBox="0 -50 1000 1050" fill="white">
    ...
    */
    ...
    bytes memory buf;
    uint idx;
    if (tokenId > renderNum) idx = tokenId - renderNum;
    ...
    for (uint i=0; i<renderNum; i++) {
      ...
      (,uint x, uint y, uint width, uint height,,,,,) = instance.ads(idx);
      if (idx == tokenId) {
        buf = abi.encodePacked(buf, '<rect x="',x.toString(),'" y="',y.toString(),'" width="',width.toString(),'" height="',height.toString(),'" fill="rgb(66,185,131)"></rect>');
      } else {
    ...
    return Base64.encode(bytes(abi.encodePacked(
      '<svg width="1000" height="1050" viewBox="0 0 1000 1050" xmlns="http://www.w3.org/2000/svg" style="background:#4a90e2">',
        '<text x="5" y="34" style="font:30px sans-serif;fill:rgba(255,255,255,0.8);">The Thousand Ether Homepage</text>',
        '<svg width="1000" height="1000" viewBox="0 -5 105 105" fill="white">',
          '<rect width="100%" height="100%" fill="white"></rect>',
          buf,
        '</svg>',
      '</svg>')));
