...
contract MeltToken is DynamicArweaveHash {
    ...
    uint256 private _cadence = 86400;
    uint256 private _baseTokenId;
    uint public constant _dayBasePrice = 10000000000000000; // 0.01 ETH

    EnumerableSet.UintSet private _daysMinted;
    mapping(uint256 => string) private _tokenHash;
    mapping(uint256 => uint256) private _tokenDay;
    ...
    function mint(address to) public virtual override onlyOwner returns(uint256) {
        ...
        _baseTokenId = super.mint(to);
        return _baseTokenId;
    }

    function mintDay() public payable {
        require(IERC721(_creator).ownerOf(_baseTokenId) == msg.sender, "Only token owner can mint day");
        uint256 dayNumber = _getDay();
        uint dayPrice;
        if (dayNumber == 333) {
            dayPrice = 10 * _dayBasePrice * dayNumber;
        } else {
            dayPrice = _dayBasePrice * dayNumber;
        }
        require(!EnumerableSet.contains(_daysMinted, dayNumber), "Day already minted");
        require(msg.value >= dayPrice, "Must pay more");
        EnumerableSet.add(_daysMinted, dayNumber);
        uint256 dayTokenId = IERC721CreatorCore(_creator).mintExtension(msg.sender);
        _tokenDay[dayTokenId] = dayNumber;
        _tokenHash[dayTokenId] = _getImageHash(dayTokenId);
    }
    ...
    function _getStaticData() internal pure returns(string memory) {
        return string(abi.encodePacked(
            ',',_wrapTrait("Artist","Jake"),
            ',',_wrapTrait("Commissioned by", "Beets DAO"),
            ',',_wrapTrait("Contract by","manifoldxyz"),
            ',',_wrapTrait("FileType","MP4"),
            ',',_wrapTrait("Resolution","4096x3112")
        ));
    }

    function _getChangingData(uint256 tokenId) internal view returns(string memory) {
        return string(abi.encodePacked(
            _wrapTrait("Day",_getDay().toString()),
            ',',_wrapTrait("Melted", _getMelted(tokenId)),
            ',',_wrapTrait("Background Color",_getBackground(tokenId))
        ));
    }

    function changeCadence(uint256 cadence) external onlyOwner {
        _cadence = cadence;
    }

    function _getDay() internal view returns(uint256) {
        return (((block.timestamp - _creationTimestamp)/_cadence) % 365) + 1;
    }

    function _getDay(uint256 tokenId) internal view returns(uint256) {
        if (_tokenDay[tokenId] != 0) return _tokenDay[tokenId];
        return _getDay();
    }

    function _getMelted(uint256 tokenId) internal view returns(string memory) {
        uint256 day = _getDay(tokenId);
        if (day < 98) {
            return "Solid";
        } else if (day < 292) {
            return "Melted";
    ...
    function _getBackground(uint256 tokenId) internal view returns(string memory) {
        uint256 day = _getDay(tokenId);
        if (day < 333) {
            return "Sky Blue";
        } else if (day == 333) {
    ...
    function _getImageHash(uint256 tokenId) internal view override returns(string memory) {
        return arweaveHashes[_getDay(tokenId)];
    }
