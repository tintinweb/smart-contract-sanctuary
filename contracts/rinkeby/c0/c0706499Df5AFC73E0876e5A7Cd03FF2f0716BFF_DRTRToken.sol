...
contract DRTRToken is ERC1155, Ownable {
    ...
    function mintCollection(address beneficiary, uint256 itemsCount) external onlyOwner {
        ...
        for(uint256 i=0; i<itemsCount; i++){
            ids[i] = getId(collection, i);
            amounts[i] = 1;
        }
        _mintBatch(beneficiary, ids, amounts, '');
    }

    function setProperty(bytes32 propertieHash, uint256[] calldata ids, bytes[] calldata values) external onlyOwner {
        require(ids.length == values.length, "arrays length mismatch");
        for(uint256 i=0; i < ids.length; i++){
            properties[ids[i]][propertieHash] = values[i];
        }
    }

    function getProperty(uint256 id, bytes32 propertieHash) external view returns(bytes memory){
        return properties[id][propertieHash];
    }
    ...
    function getId(uint256 collection, uint256 num) public pure returns(uint256) {
        return uint256(keccak256(abi.encodePacked(collection, num)));
    }
}
