...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
  ...
  mapping(bytes32 => address) internal requestIdToMinter;
  mapping(bytes32 => uint256) internal requestIdToTokenId;
  mapping(uint256 => uint256) internal tokenIdToRandomNumber;

  // SVG params
  uint256 internal maxNumberOfPaths = 10;
  uint256 internal maxNumberOfPathCommands = 5;
  uint256 internal size = 500;
  string[] internal pathCommands = ["M", "L", "H", "V", "C"];
  string[] internal colors = ["red", "blue", "green", "yellow", "black"];
  ...
  function create() external returns (bytes32) {
    ...
    counter++;
    requestIdToTokenId[requestId] = counter;

    emit RequestedRandomSVG(requestId, counter);

    return requestId;
  }

  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    address nftOwner = requestIdToMinter[requestId];
  ...
  function finishMint(uint256 _tokenId) external {
    ...
    require(bytes(tokenURI(_tokenId)).length == 0, "Token URI already exists");
  ...
  function _generateSVG(uint256 _randomNumber) internal view returns (string memory) {
    uint256 numberOfPaths = (_randomNumber % maxNumberOfPaths) + 1;
    string memory finalSVG = string(abi.encodePacked(
      "<svg xmlns='https://www.w3.org/2000/svg' height='", uint2str(210), "' width='", uint2str(400), "'>"
    ));

    for (uint256 i = 0; i < numberOfPaths; i++) {
      uint256 newRandomNumber = uint256(keccak256(abi.encodePacked(_randomNumber, i)));
      string memory pathSVG = _generatePath(newRandomNumber);
  ...
  function _generatePath(uint256 _randomNumber) internal view returns (string memory) {
    ...
    for (uint256 i = 0; i < numberOfPathCommands; i++) {
      uint256 newRandomNumber = uint256(keccak256(abi.encodePacked(_randomNumber, i)));
      string memory pathCommand = _generatePathCommand(newRandomNumber);
  ...
  function _generatePathCommand(uint256 _randomNumber) internal view returns (string memory) {
    string memory pathCommand = pathCommands[_randomNumber % pathCommands.length];
    ...
    pathCommand = string(abi.encodePacked(pathCommand, " ", uint2str(paramOne), " ", uint2str(paramTwo)));
  ...
  function _formatTokenURI(string memory _imageURI) internal pure returns (string memory) {
    ...
    string memory tokenURIBase64Encoded = Base64.encode(bytes(abi.encodePacked(
      '{ "name": "SVG NFT"',
      '"description": "An NFT based on SVG"',
      '"attributes": ""',
      '"image": "', _imageURI, '" }'
    )));
    string memory tokenURI = string(abi.encodePacked(baseURI, tokenURIBase64Encoded));
