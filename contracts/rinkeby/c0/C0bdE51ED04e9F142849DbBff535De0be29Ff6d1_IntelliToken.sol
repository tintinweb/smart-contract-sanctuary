...
/**
 * @title Intelligent Token (iNFT)
 *
 * @notice Intelligent Token (Intelligent NFT, iNFT) represents an enhancement to an existing NFT (a target),
 *      it binds some data to the target to embed intelligence, is controlled and belongs to the owner of the target.
 *
 * @notice iNFT has limited ERC721 compatibility: it doesn't store token ownership information, it relies on the
 *      target ownership information instead
 *
 * // TODO: describe ERC721 properties, limitations, additional features and improvements
 *
 * @author Basil Gorin
 */
// TODO: consider implementing ERC721 interface(s) only and not inheriting from any impl
// TODO: use AccessControl instead of Ownable
// TODO: shadow ERC721 ownership-related functions
contract IntelliToken is ERC721Enumerable, IERC721Receiver, Ownable {
	// TODO: possibly just use Zeppelin SafeERC20 for safe ERC20 transfers
	 ...
	 */
	// TODO: consider the compression of IntelliBinding from 4 to 2(3) storage slots
	struct IntelliBinding {
		 ...
		 */
		// TODO: consider storing lower 96 of the targetId to save on storage for short target NFT IDs
		uint256 targetId;
		 ...
		 */
		// TODO: consider compressing that to uint64
		uint256 podId;
		 ...
		 */
		// TODO: taking into account 10 bil max supply uint96 is enough
		uint256 aliValue;
	}
	...
	string private baseURI = "https://alethea.ai/iNFT/";
	 ...
	 * @param tokenId ID of the iNFT to mint (create, bind)
	...
	event Minted(
		address indexed _by,
		// TODO: consider adding _to - owner of the NFT
		uint256 tokenId,
		address payer,
		uint256 podId,
		uint256 aliValue,
	 ...
	 * @param tokenId ID of the iNFT to burn (destroy, unbind)
	 * @param recipient and address which receives unlocked AI Pod and ALI tokens
	...
	/**
	 * @dev Creates/deploys an iNFT instance bound to already deployed AI Pod instance
	 *
	 * @param _pod address of the deployed AI Pod instance the iNFT is bound to
	 */
	// TODO: approve token name and symbol with Alethea
	constructor(address _pod, address _ali) ERC721("Alethea Intelligent NFT", "ALN") {
		// verify inputs are set
		require(_pod != address(0));
		require(_ali != address(0));

		// TODO: verify _pod and _ali are valid
	 ...
	 *
	 * @param _baseURI new _baseURI to set
	 */
	...
	function onERC721Received(address operator, address from, uint256 tokenId, bytes calldata data) external override returns (bytes4) {
		// TODO: implement function logic, handle NFT receipt
	...
	/**
	 * @dev Since iNFT owner is defined as an underlying NFT owner, it is not possible
	 *      to track balance since underlying NFTs can be transferred silently for the iNFT
	 *
	 * @dev Not supported. Always throws.
	 *
	 * @inheritdoc IERC721
	 */
	function balanceOf(address owner) public view override returns (uint256) {
		...
		// to silence compilation warnings pretend we return a value
		return 0;
	}
	...
	function ownerOf(uint256 tokenId) public view override returns (address) {
		...
		IntelliBinding memory binding = bindings[tokenId];

		// verify the binding exists and throw standard Zeppelin message if not
		require(binding.targetContract != address(0), "ERC721: owner query for nonexistent token");
	 ...
	 *      payer must authorize the transfer operation before the mint is called
	 * @dev Transfers specified amount of ALI token into iNFT smart contract for locking;
	 *      payer must authorize the transfer operation before the mint is called
	 * @dev Target NFT existence is not verified (TODO: consider fixing)
	 *
	 * // TODO: eventually access to this function goes to iNFT Linker
	 *
	 * @param tokenId ID of the iNFT to mint (create, bind)
	 * @param payer and address which funds the creation (supplies AI Pod and ALI tokens)
	 * @param podId ID of the AI Pod to bind (transfer) to newly created iNFT
	...
	function mint(
		...
		IERC721(podContract).transferFrom(payer, address(this), podId);

		// transfer the ALI tokens from the specified address `_from`
		// using unsafe transfer to avoid unnecessary callback triggering
		require(IERC20(aliContract).transferFrom(payer, address(this), aliValue));

		// TODO: consider verifying target NFT exists
		// TODO: consider verifying target NFT belongs to the same address `_from` which funds minting

		// bind AI Pod transferred and ALI ERC20 value transferred to an NFT specified
		bindings[tokenId] = IntelliBinding({
		...
		// emit an event
		emit Minted(msg.sender, tokenId, payer, podId, aliValue, targetContract, targetId);
	}
	 ...
	 *      transaction executor via // TODO: specify safe ERC20 transfer interface
	 ...
	 */
	function burn(uint256 tokenId, uint256 aliFee) public onlyOwner {
		// read the token binding
		IntelliBinding memory binding = bindings[tokenId];
		...
		if(aliFee > 0) {
			// TODO: this probably should be done via safe transfer
			require(IERC20(aliContract).transfer(msg.sender, aliFee));
		}
		...
		if(binding.aliValue > aliFee) {
			// TODO: this probably should be done via safe transfer
			require(IERC20(aliContract).transfer(owner, binding.aliValue - aliFee));
		}

		// emit an event
		emit Burnt(msg.sender, tokenId, owner, binding.podId, binding.aliValue, aliFee, binding.targetContract, binding.targetId);
	}
	...
	function _exists(uint256 tokenId) internal view override returns (bool) {
		...
		return bindings[tokenId].targetContract != address(0);
	}
