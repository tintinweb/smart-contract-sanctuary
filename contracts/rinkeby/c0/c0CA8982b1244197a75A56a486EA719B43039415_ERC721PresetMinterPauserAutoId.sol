...
contract ERC721PresetMinterPauserAutoId is Context, VRFConsumerBase, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ERC721URIStorage {
    ...
    function fulfillRandomness(
        ...
        if(requestToType[requestId] > 0) {
            tokenPower[requestToTokenId[requestId]] = (randomness == 0 || randomness % 5 == 0) ? 6 : (randomness % 8 < 6 ? 7 : randomness % 8);
        } else {
            tokenPower[requestToTokenId[requestId]] = (randomness == 0 || randomness % 5 == 0) ? 1 : randomness % 5;
        }
    }
...
interface IERC721Receiver {
    ...
    function onERC721ReceivedBatch(address operator, address from, uint256[] memory tokenId, bytes[] calldata data) external returns (bytes4);
}
