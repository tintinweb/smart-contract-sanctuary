...
contract PolkaBridgeFarm is Ownable {
        ...
        returns (uint256)
    {
        ...
        // require(false, string(abi.encodePacked(uint2str(pool.lastRewardBlock), ' : ', uint2str(block.number))));
    ...
    function updatePool(uint256 _pid) public {
        ...
        // polkaBridge.mint(address(this), PBRReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            IERC20(address(polkaBridge)).safeTransfer(msg.sender, pending);
        }
    ...
    function safePBRTransfer(address _to, uint256 _amount) internal {
        uint256 PBRBal = polkaBridge.balanceOf(address(this));
        if (_amount > PBRBal) {
            polkaBridge.transfer(_to, PBRBal);
        } else {
