...
contract FixedSale is Ownable, ReentrancyGuard {
    ...
    /// @param _tokenIdFrom Token Start ID of NFT
    /// @param _tokenIdTo Token End ID of NFT
    /// @param _quantityPerToken token amount to list (needed for ERC-1155 NFTs, set as 1 for ERC-721)
    ...
    function listAllItems(
        ...
        for(uint256 _tokenId=_tokenIdFrom; _tokenId<=_tokenIdTo; _tokenId++) {            
            if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
                IERC721 nft = IERC721(_nftAddress);
                ...
                IERC1155 nft = IERC1155(_nftAddress);
                require(nft.balanceOf(_msgSender(), _tokenId) >= _quantityPerToken, "Must hold enough NFTs.");
            }
        }
