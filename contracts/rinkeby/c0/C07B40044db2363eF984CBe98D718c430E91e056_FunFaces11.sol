/*
______ _   _ _   _  ______ ___  _____  _____ _____ 
|  ___| | | | \ | | |  ___/ _ \/  __ \|  ___/  ___|
| |_  | | | |  \| | | |_ / /_\ \ /  \/| |__ \ `--. 
|  _| | | | | . ` | |  _||  _  | |    |  __| `--. \
| |   | |_| | |\  | | |  | | | | \__/\| |___/\__/ /
\_|    \___/\_| \_/ \_|  \_| |_/\____/\____/\____/                                                
                                                                                                                                                                                                                                                                                                                                                                                                                                       
*/

/**
 * @title  Smart Contract for Fun Faces
 * @author SteelBalls
 * @notice NFT Minting
 */
...
contract FunFaces11 is ERC721, PaymentSplitter, Ownable {

    using Counters for Counters.Counter;
    Counters.Counter private _tokenSupply;

    // Variables
    uint256 public maxTokens = 125;           // Total token supply
    uint256 public tokenPrice = 0.05 ether;   // 0.05 ETH Token price
    uint256 public maxTokenPurchase = 1;      // Maximum number of tokens that can be purchased in a single transaction
    uint256 public tokenReserve = 15;         // Reserved up front for the teams effort and for giveaways/promotional activity
    ...
    */
    ...
    mapping(address => bool) public whitelistClaimed; // Record whitelist addresses that have claimed
    
    // Withdraw Addresses
    address[] private teamWallets = [
        0xE6C4a47A90Dc0C2C8962440C94f59dec2a40cf73, // Project
        0x918638b3cc1813778333700e6f8eAcD9834555E9, // Founder
        0xb335A50810923f30C21eC4Eb142eB4D962160E02, // Partner
    ...
    */
    function mintToken(uint _numberOfTokens) external payable {
        require(saleIsActive, "Fun Faces are not for sale yet!");
        require(!whitelistIsActive, "Public mint is not currently available");
        require(_numberOfTokens > 0 && _numberOfTokens <= maxTokenPurchase, "You have exceeded the maximum tokens that can be minted");
        require(_tokenSupply.current() <= maxTokens, "Purchase would exceed max supply of Tokens");
    ...
    */
    function whitelistMintToken(uint256 _numberOfTokens, bytes32[] calldata _merkleProof) external payable {
        require(saleIsActive, "Fun Faces are not for sale yet!");
        require(whitelistIsActive, "MINT_NOT_STARTED");

        require(!whitelistClaimed[msg.sender], "ADDRESS_ALREADY_CLAIMED");
        ...
        require(_numberOfTokens > 0 && ownerTokenCount + _numberOfTokens <= maxTokenPurchase, "COUNT_INVALID");
    ...
    *
    function whitelistMintToken(uint256 _numberOfTokens, bytes32[] memory _proof) external payable {
        require(saleIsActive, "Fun Faces are not for sale yet!");
        require(whitelistIsActive, "MINT_NOT_STARTED");
        require(verifyWhitelist(_leaf(msg.sender), _proof) == true, "ADDRESS_INVALID");
