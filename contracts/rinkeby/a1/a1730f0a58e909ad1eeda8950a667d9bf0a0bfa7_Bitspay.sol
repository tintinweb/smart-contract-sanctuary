...
contract Bitspay {
  ...
  function deposit(uint tariffPlan, address referer) external payable {
        ...
        investor[msg.sender].deposits.push(Deposit(tariffPlan, amount, block.timestamp,false));

        emit DepositAt(msg.sender, tariffPlan, amount);

 }
 
 function withdrawableUser(address user) public view returns (uint amount) {
