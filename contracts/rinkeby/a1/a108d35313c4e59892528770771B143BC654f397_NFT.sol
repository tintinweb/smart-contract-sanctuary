...
contract NFT is ERC721, Ownable {
    ...
    constructor(uint256 allowmint, uint256 airdrop_available, string memory name, string memory symbol, uint256 price, uint256 max_supply, uint16 networkid) ERC721(name, symbol) {
        ...
        if (_networkid == 1) {
            _baseURI = string(abi.encodePacked("https://crypto-native.luquitared.repl.co/api/v1/nfts/"));
        } else {
            _baseURI = string(abi.encodePacked("https://crypto-native.luquitared.repl.co/api/v1/nfts/"));
        }
    ...
    function update(uint256 price, uint256 max_supply) public onlyOwner {
        _max_supply = max_supply; 
        _price = price;
