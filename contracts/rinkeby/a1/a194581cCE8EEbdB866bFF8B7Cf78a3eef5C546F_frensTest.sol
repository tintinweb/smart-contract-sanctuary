...
contract frensTest is ERC721EnumerableB, Ownable, ReentrancyGuard {  
    using Address for address;
    using Strings for uint256;

    enum preSaleClaimStatus { Invalid, Claimed }
    ...
    string public _baseTokenURI = "ipfs://QmfVnrNZ4fzJwzP6nkUVNdndf8VNqW8NxFq5Ky1inTf2wi/";

    mapping (address => preSaleClaimStatus) private _preSaleClaims;
    mapping (address => uint256) private _preSaleTokens; 
    bytes32 public merkleRoot;

    // withdraw addresses
    
    address a1 = 0xeAC61e3ea94738FF17ad8C37b0D250E63228F72E;
    address a2 = 0x7f1086B3AEA172d38F51e7fa466eec27Ea458558;
    address a3 = 0xb8E3F2897325Fa6c9333C0A588b3f3Aeb02A8184;

    constructor()
        ERC721B("Frens Test", "FRT") {
        // team gets the first 2
        _safeMint( a1, 0);
        _safeMint( a2, 1);        
    }
    ...
    function mintToken(uint256 numberOfTokens) external payable nonReentrant() {
        ...
        require(numberOfTokens <= maxMint, "Can only mint a maximum of 5 tokens at once");
        require(price * numberOfTokens <= msg.value, "Wrong amout of ETH sent");

        uint256 supply = totalSupply();
        require(supply + numberOfTokens <= MAX_SUPPLY, "Can't mint more than max supply");
        for(uint256 i = 0; i < numberOfTokens; i++) {
           _safeMint( msg.sender, supply++);
        }
    } 

  function mintPreSale(uint256 numberOfTokens, bytes32[] calldata proof) external payable nonReentrant() {
        require(preSaleActive, "Pre sale must be active");
        require(MerkleProof.verify(proof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "You are not on the Presale List");
        require(_preSaleClaims[msg.sender] != preSaleClaimStatus.Claimed, "Already Claimed");
        require(numberOfTokens <= maxMint, "Check the max tokens per transaction");
        require(price * numberOfTokens <= msg.value, "Wrong amout of ETH sent");
        require(_preSaleTokens[msg.sender] + numberOfTokens <= maxMint, "Can only mint a maximum of 3 tokens");

        uint256 supply = totalSupply();
        require(supply + numberOfTokens <= MAX_SUPPLY, "Max supply reached" );

        _preSaleTokens[msg.sender] += numberOfTokens;
        if(_preSaleTokens[msg.sender] == maxMint) {
    ...
    function mintReserved(uint256 numberOfTokens) public onlyOwner {
        ...
        require( numberOfTokens <= reserved, "Can't reserve more than set amount" );
