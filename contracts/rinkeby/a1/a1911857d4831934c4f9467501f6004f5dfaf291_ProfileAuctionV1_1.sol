...
contract ProfileAuctionV1_1 is Initializable,
    ...
    OwnableUpgradeable
{
    ...
    */
    ...
    /**
     ...
     @param _action 0 = submitBidFor (new or existing), 1 = remove bid, 2 = mint profile ford
    */
    function findEffectDeleteArray(
        ...
        for (uint256 i = 0; i < bids.length; i++) {
            if (compareString(bids[i]._profileURI, _profileURI)) {
                if (_action == 0) {
                    // add to existing bid
                    require(_bids[msg.sender][i]._blockMinted == 0);
                    ...
                    // remove bid
                    require(_bids[msg.sender][i]._blockMinted == 0, "must claim");

                    emit RemoveBid(
                        msg.sender,
                        _profileURI,
                        _bids[msg.sender][i]._nftTokens.mul(9950).div(10000)
                    );
                    ...
                    _bids[msg.sender][i]._blockMinted = block.number;
                    ...
                    require(_bids[msg.sender][i]._blockMinted != 0, "no claim");
    ...
    */
    function removeProfileBid(
        ...
        // return fee
        payable(msg.sender).transfer(profileFee);
    }
