...
import "./interfaces/ILittleLemons.sol";
import "./interfaces/ILittleLemonsMetadata.sol";

contract LittleLemons is ERC721Enumerable, Ownable, ILittleLemons, ILittleLemonsMetadata {
    using Strings for uint256;

    uint256 public constant LEMONS_RESERVED = 200;
    uint256 public constant LEMONS_PUBLIC = 9799;
    uint256 public constant LEMONS_MAX = LEMONS_RESERVED + LEMONS_PUBLIC;
    ...
    function purchase(uint256 numberOfTokens) external payable override {
        ...
        require(totalSupply() < LEMONS_MAX, "All tokens have been minted");
        require(
            numberOfTokens <= PURCHASE_LIMIT,
            "Would exceed PURCHASE_LIMIT"
        );
        require(
            totalPublicSupply < LEMONS_PUBLIC,
            "Purchase would exceed LEMONS_PUBLIC"
        );
        require(
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            if (totalPublicSupply < LEMONS_PUBLIC) {
                uint256 tokenId = LEMONS_RESERVED + totalPublicSupply + 1;
    ...
    function purchasePreSale(uint256 numberOfTokens) external payable override {
        ...
        require(
            totalPublicSupply + numberOfTokens <= LEMONS_PUBLIC,
    ...
    function reserve(address to, uint256 amount) external override onlyOwner {
        require(totalSupply() < LEMONS_MAX, "All tokens have been minted");
        require(totalReservedSupply <= LEMONS_RESERVED,"Not enough tokens left to reserve");
...
interface ILittleLemonsMetadata {
