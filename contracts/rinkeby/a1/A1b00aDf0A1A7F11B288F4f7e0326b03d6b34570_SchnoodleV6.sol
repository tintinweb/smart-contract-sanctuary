...
contract SchnoodleV6 is ERC777PresetFixedSupplyUpgradeable, OwnableUpgradeable {
    ...
    uint256 private _sellLowerLimit;
    uint256 private _sellUpperLimit;
    ...
    function initialize(uint256 initialTokens, address serviceAccount) public initializer {
        ...
        _sellLowerLimit=1000000000*10**decimals();
        _sellUpperLimit=5000000000*10**decimals();
    }
    ...
    function _payFee(address sender, address recipient, uint256 amount, uint256 reflectedAmount) private {
        ...
        // Prevent large sells by limiting daily sells to the liquidity token
        //if (to == address(0x0F6b0960d2569f505126341085ED7f0342b67DAe))
        if (recipient == _pairToken && _sellLowerLimit > 0 && _sellsTrackers[sender].amount > _sellLowerLimit)
        {
            feeRate += feeRate * (balanceOf(sender) - _sellLowerLimit) / _sellLowerLimit;
        }
    ...
    function changeSellLimits(uint256 lowerLimit, uint256 upperLimit) public onlyOwner {
        _sellLowerLimit = lowerLimit;
        _sellUpperLimit = upperLimit;
        emit SellLimitsChanged(lowerLimit, upperLimit);
    }
    ...
    function _beforeTokenTransfer(address operator, address from, address to, uint256 amount) internal virtual override {
        // Prevent large sells by limiting daily sells to the liquidity token
        //if (to == address(0x0F6b0960d2569f505126341085ED7f0342b67DAe))
        if (to == _pairToken && _sellUpperLimit > 0)
        {
            ...
            require(sellsTrackerFrom.amount <= _sellUpperLimit, "Schnoodle: Transfer amount exceeds daily sell upper limit");
    ...
    event SellLimitsChanged(uint256 lowerLimit, uint256 upperLimit);
}
