 ...
 * were staken for
 */
contract TokenRewardStaking is ERC721Holder {
   ...
   */
  IMintableERC20 immutable public erc20;
   ...
   */
  uint256 immutable public dailyRewards;
   ...
   */
  constructor(address _erc20Address, address _erc721Address, uint256 _dailyRewards) {
   ...
   *         the user for each NFT if he was to unstake it at
   *         the current block
   *
   * @dev This is an auxiliary function to help with integration
   *      and is not used anywhere in the smart contract login
   *
   * @param _tokenIds token IDs of NFTs rewards are to be calculated for
   * @return an array of amounts of rewards for every NFT that was input
   */
  function currentRewardsOf(uint256[] memory _tokenIds) external view returns (uint256[] memory) {
    // Create an empty array to store the result
    // Should be the same length as the amount of token
    // IDs input
    uint256[] memory rewards = new uint256[](_tokenIds.length);

    // Enumerate provided token IDs
    for(uint256 i = 0; i < _tokenIds.length; i++) {
      ...
      uint256 stakingTime = block.timestamp - stakedAt[tokenId];
      ...
      // This wil yield the (rounded down) staking
      // time in days
    ...
    // Return calculated rewards array
    return rewards;
  }
  ...
  function unstake(uint256[] memory _tokenIds) public {
    ...
    for(uint256 i = 0; i < _tokenIds.length; i++) {
      ...
      // Calculate rewards for token by multiplying
      // rounded down number of staked days by daily
      // rewards variable
      uint256 rewards = stakingDays * dailyRewards;
