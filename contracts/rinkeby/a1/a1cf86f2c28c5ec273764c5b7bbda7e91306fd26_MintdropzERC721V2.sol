...
contract MintdropzERC721V2 is ERC721Upgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function removeWhitelistUsers(address[] memory _users) external onlyOwner {
        for (uint256 account = 0; account < _users.length; account++) {
    ...
    function setPauseWhitelist(bool _paused) external onlyOwner {
    ...
    function mintNFT(uint256 numberOfTokens) external payable {
        ...
        uint256 price = mintdropzPrice.mul(numberOfTokens); // for saving gas
        uint256 royaltyFee = price.mul(royaltyPercent).div(DENOMINATOR);
        uint256 totalPrice = price.add(royaltyFee);
        require(msg.value >= totalPrice, "Invalid amount");

        uint256 restAmount = msg.value.sub(totalPrice);
