...
contract BaseContract is Ownable {
    ...
    // delegate call to CollectionContract from BaseContract
    // Should change CollectionContract to ICollectionContract (interface in the future)
    function executeArbitration(address payable collectionAddress, uint256 _nonce, string memory side) public isArbitrator {
        CollectionContract(collectionAddress).executeArbitration(_nonce, side);
    }
}
