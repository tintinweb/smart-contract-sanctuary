...
contract CasperInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("CasperInu_Dividend_Tracker", "CasperInu_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "CasperInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CasperInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CasperInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CasperInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "CasperInu_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CasperInu is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    CasperInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Casper Inu", "CasperInu") {
        ETHRewardsFee = 4;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 10;
        charityFee = 6;
        marketingWallet = payable(0xF7470fBA6DF24D6Ee1bAD2c85A2d041C2C154875);
        charityWallet = payable(0x258d098cb28846537E0cA1dA6d9A4A471e4d10a1);

        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee).add(charityFee); // total fee transfer and buy

    	dividendTracker = new CasperInuDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CasperInu: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CasperInu: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CasperInu: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CasperInu: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CasperInu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CasperInu: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(ETHToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(charityFee)).mul(marketingFee);
        marketingWallet.transfer(marketingAmount);
        
         uint256 buybackAmount = deltaBalance.sub(ETHToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(marketingFee)).mul(charityFee);
    ...
    function swapAndSendETHToBuybak(uint256 tokenAmount) private {
