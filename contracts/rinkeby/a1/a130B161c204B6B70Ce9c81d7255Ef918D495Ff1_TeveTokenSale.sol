...
contract TeveTokenSale is Ownable {
    using SafeMath for uint256;

    event PurchaseSale(address buyer);
    ...
    */
    ...
    uint256 public startingPrice = 7 ether;
    // DSP
    ...
    uint256 public maxDecreaseNFTs = 100;

    address public presigner;

    address constant tokenAddress = 0x2F309CB5184dcf8490C6AC3d525e8937d4A30490;

    function currentPrice() public view returns (uint256) {
        uint256 gap = block.timestamp - sales_start;
        uint256 counts = gap / (30 minutes);
        if (gap >= sales_duration) {
            return 2 ether;
        }
        return startingPrice - (counts * 0.2 ether);
    }
    ...
    function purchase() public payable {
        ...
        require(decreaseRemainingTokens() >= 1, "sold out");
        maxDecreaseSold += 1;

        ITokenContract(tokenAddress).mint(msg.sender);

        emit PurchaseSale(msg.sender);
    }
    ...
    function tellEverything() external view returns (saleContractInfo memory) {
        return
            saleContractInfo(
                startingPrice,
                sales_start,
