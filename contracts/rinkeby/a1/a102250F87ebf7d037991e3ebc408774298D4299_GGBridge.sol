...
contract GGBridge is EIP712, Ownable {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < tokensToSupport_.length; i++) {
            ...
            require(info.collectedFee == 0, "CollectedFee is incorrect");
    ...
    function collectedFee(address tokenAddress_) external onlyOwner {
        uint256 collected = tokensInfo[tokenAddress_].collectedFee;

        require(collected > 0, "Collected fee is 0");

        tokensInfo[tokenAddress_].collectedFee = 0;

        if (NATIVE == tokenAddress_) {
            payable(msg.sender).sendValue(collected);
        } else {
            IERC20(tokenAddress_).safeTransfer(msg.sender, collected);
        }
    }
        ...
        returns (bool success)
    {
        ...
        require(info_.collectedFee == 0, "CollectedFee is incorrect");
    ...
    function deposite(
        ...
        if (tokenAddress_ != NATIVE) {
            if (info.tokenType == TokenType.ERC20_MINT_BURN_V2) {
                IERC20Mintable(tokenAddress_).burnFrom(msg.sender, amount);
            } else if (info.tokenType == TokenType.ERC20_MINT_BURN) {
        ...
        if (info.fee > 0) {
            uint256 fee = (amount * info.fee) / MAX_INITIAL_PERCENTAGE;
            amount -= fee;
            info.collectedFee += fee;
        }
