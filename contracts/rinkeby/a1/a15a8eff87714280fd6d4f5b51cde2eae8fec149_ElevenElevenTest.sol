...
contract ElevenElevenTest is Ownable {
    ...
    function setSupply(uint256 quantity) public returns (uint256) {
    ...
    function getNFTPrice() public view returns (uint256, string memory) {
        ...
        if (currentSupply >= 16381) {
            SalePrice memory price = SalePrice(100000000000000000000, "SMOL"); // 16381 - 16383 100 ETH
            return (price.amount, price.currency);
        } else if (currentSupply >= 16000) {
            SalePrice memory price = SalePrice(3000000000000000000, "ETH"); // 16000 - 16380 3.0 ETH
            return (price.amount, price.currency);
        } else if (currentSupply >= 15000) {
            SalePrice memory price = SalePrice(1700000000000000000, "SMOL"); // 15000  - 15999 1.7 ETH
            return (price.amount, price.currency);
        } else if (currentSupply >= 11000) {
            SalePrice memory price = SalePrice(900000000000000000, "ETH"); // 11000 - 14999 0.9 ETH
            return (price.amount, price.currency);
        } else if (currentSupply >= 7000) {
            SalePrice memory price = SalePrice(500000000000000000, "SMOL"); // 7000 - 10999 0.5 ETH
            return (price.amount, price.currency);
        } else if (currentSupply >= 3000) {
            SalePrice memory price = SalePrice(300000000000000000, "ETH"); // 3000 - 6999 0.3 ETH
            return (price.amount, price.currency);
        } else {
            SalePrice memory price = SalePrice(100000000000000000, "SMOL"); // 0 - 2999 0.1 ETH 
            return (price.amount, price.currency);
        }
    }
