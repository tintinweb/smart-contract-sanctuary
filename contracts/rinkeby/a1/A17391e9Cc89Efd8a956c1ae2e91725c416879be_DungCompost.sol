...
contract DungCompost is LPTokenWrapper, Ownable {
    ...
    */
    function apyEth() external view returns (uint) {
        ...
        if (lptoken.token0() == address(dung)) {
            ...
            ETHperLP = 1 ether* reserve1/totalSupplyLP; // DUNG value + ETH value in 1 LP
        } else {
            dungPrice = (uint256)(reserve0) * 1 ether/reserve1;
            ETHperLP = 1 ether* reserve0/totalSupplyLP; // DUNG value + ETH value in 1 LP
        }
        ...
        return earnerEthPerYearForStakedETH;
    }
    ...
    */
    ...
    function apyDung() external view returns (uint, uint, uint, uint) {
        ...
        uint DungPerLP; // 1 LP price in ETH currency
        if (lptoken.token0() == address(dung)) {
            DungPerLP = 1 ether * (uint256)(reserve0)/totalSupplyLP; // DUNG value + ETH value in 1 LP (wei)
        } else {
            DungPerLP = 1 ether * (uint256)(reserve1)/totalSupplyLP; // DUNG value + ETH value in 1 LP (wei)
        }
