...
contract FixStaking is AccessControl {
    ...
    {   require(_minStake > 0,"setMinMaxStake: minumum amount cannot be ZERO");
        require(_maxStake > _minStake, "setMinMaxStake: The maximum amount cannot be less than the minimum amount.");
...

    {
        ...
        emit PoolFunded(_fundingAmount);
    }
     ...
     */
    function deposit(uint256 _amount, uint256 _poolIndex) external isPoolExist(_poolIndex){
    ...
    function withdraw(uint256 _stakerIndex) external {
        ...
        require(pool.reserve >= totalReward ,"withdraw: This pool has no enough reward reserve.");

        stakerAmount[msg.sender][staker.poolIndex] -= staker.amount;
        pool.reserve -= (reward - (mainPen + subPen));
