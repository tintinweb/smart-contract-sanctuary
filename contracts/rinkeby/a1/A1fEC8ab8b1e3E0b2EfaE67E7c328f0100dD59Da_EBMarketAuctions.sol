...
contract EBMarketAuctions is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    ...
    function create(address[] calldata _nftAddresses,uint[] calldata _nftIds,uint32[] calldata _nftTypes,uint _valability,uint _price,uint _tax, address _currency) external {
        require(_price > 0 && _tax > 0 && _valability > 0 && _tax.mul(2) < _price, "Input values are not valid");
    ...
    function cancel(uint auctionId) external isAccesible(auctionId) nonReentrant {
        ...
        auctions[auctionId].owner = address(0);
    ...
    function bid(uint auctionId) external payable isAccesible(auctionId) nonReentrant {
        ...
        if ( auctions[auctionId].currency != address(0) ){
            ...
            auctions[auctionId].price = auctions[auctionId].price.add(auctions[auctionId].tax);
