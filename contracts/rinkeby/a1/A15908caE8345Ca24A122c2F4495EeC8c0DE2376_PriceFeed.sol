...
contract PriceFeed is Ownable, BaseMath, IPriceFeed {
    ...
    */
    ...
    uint public dumpForDetectingBothUntrustedFlow = 0;
    uint public dumpForDetectingGasEater = 0;
    ChainlinkResponse[] public dumpForChainLinkNow;
    ChainlinkResponse[] public dumpForChainLinkBefore;
    TellorResponse[] public dumpForTellor;
    ...
    */
    function fetchPrice() external override returns (uint) {
        ...
        dumpForDetectingGasEater = 1;
        ChainlinkResponse memory chainlinkResponse = _getCurrentChainlinkResponse();
        dumpForChainLinkNow.push(chainlinkResponse);
        ChainlinkResponse memory prevChainlinkResponse = _getPrevChainlinkResponse(chainlinkResponse.roundId, chainlinkResponse.decimals, chainlinkResponse.subAnswer, chainlinkResponse.subDecimal);
        dumpForChainLinkBefore.push(prevChainlinkResponse);
        TellorResponse memory tellorResponse = _getCurrentTellorResponse();
        dumpForTellor.push(tellorResponse);
        ...
        if (status == Status.chainlinkWorking) {
            ...
            if (_chainlinkIsBroken(chainlinkResponse, prevChainlinkResponse)) {
                ...
                if (_tellorIsBroken(tellorResponse)) {
                    dumpForDetectingBothUntrustedFlow = 1;
            ...
            if (_chainlinkIsFrozen(chainlinkResponse)) {          
                ...
                if( _tellorIsFrozen(tellorResponse)) {
                    dumpForDetectingBothUntrustedFlow = 2;
                    _changeStatus(Status.bothOraclesUntrusted);
                    return lastGoodPrice;
                }
            ...
            if (_chainlinkPriceChangeAboveMax(chainlinkResponse, prevChainlinkResponse)) {
                // If Tellor is broken, both oracles are untrusted, and return last good price
                 if (_tellorIsBroken(tellorResponse)) {
                    dumpForDetectingBothUntrustedFlow = 3;
                    _changeStatus(Status.bothOraclesUntrusted);
                    return lastGoodPrice;     
                }
        ...
        if (status == Status.usingTellorChainlinkUntrusted) { 
            ...
            if (_tellorIsBroken(tellorResponse)) {
                    dumpForDetectingBothUntrustedFlow = 4;
                _changeStatus(Status.bothOraclesUntrusted);
                return lastGoodPrice; 
            }
        ...
        if (status == Status.usingTellorChainlinkFrozen) {
            if (_chainlinkIsBroken(chainlinkResponse, prevChainlinkResponse)) {
                ...
                if (_tellorIsBroken(tellorResponse)) {
                    dumpForDetectingBothUntrustedFlow = 5;
                    _changeStatus(Status.bothOraclesUntrusted);
                    return lastGoodPrice;
                }
        ...
        // --- CASE 5: Using Chainlink, Tellor is untrusted ---
         if (status == Status.usingChainlinkTellorUntrusted) {
            ...
            if (_chainlinkIsBroken(chainlinkResponse, prevChainlinkResponse)) {
                    dumpForDetectingBothUntrustedFlow = 6;
                _changeStatus(Status.bothOraclesUntrusted);
                return lastGoodPrice;
            }
            ...
            if (_chainlinkPriceChangeAboveMax(chainlinkResponse, prevChainlinkResponse)) {
                    dumpForDetectingBothUntrustedFlow = 7;
                _changeStatus(Status.bothOraclesUntrusted);
                return lastGoodPrice;
            }
    ...
    */
    ...
    function _getCurrentChainlinkResponse() internal returns (ChainlinkResponse memory chainlinkResponse) {
        ...
        )
        {
            ...
            dumpForDetectingGasEater = 12;
            // If call to Chainlink aggregator reverts, return a zero response with success = false
            return chainlinkResponse;
        }
        dumpForDetectingGasEater = 13;
        dumpForChainLinkNow.push(ethChainlinkResponseInUSD);
        ...
        )
        {
            ...
            dumpForDetectingGasEater = 15;
            // If call to Chainlink aggregator reverts, return a zero response with success = false
            return chainlinkResponse;
        }
        dumpForChainLinkNow.push(jpyChainlinkResponseInUSD);
        dumpForDetectingGasEater = 16;
    ...
    function _getPrevChainlinkResponse(uint80 _currentRoundId, uint8 _currentDecimals, int256 _jpyInUSD, uint8 _jpyOracleDecimals) internal returns (ChainlinkResponse memory prevChainlinkResponse) {
        /*
