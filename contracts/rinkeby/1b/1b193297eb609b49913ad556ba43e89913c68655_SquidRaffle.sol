...
contract SquidRaffle is VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    uint16 constant tokenPerRound = 5;
    ...
    //uint256 public withdrawTime = block.timestamp + 30 days;
    uint256 public withdrawTime = block.timestamp + 180 seconds;
    ...
    mapping(uint => ethPrize) public ethClaims; // tokendId => paid address
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        bool picked = pickWinner(tokenId, randomness);
    ...
    function withdraw() public onlyOwner { 
        require(withdrawTime < block.timestamp, 'withdrawTime not passed');
    ...
    function pickWinner(uint _roundId, uint _randSeed) internal nonReentrant returns (bool) {
        ISquidNFT NFT = ISquidNFT(nftAddress);    
        uint startIdx = tokenPerRound * (_roundId - 1) + 1;
        uint pickIdx = _randSeed  % tokenPerRound;
        ...
        ethClaims[tokenId] = ethPrize(ethAmount, address(0));
        return true;
    }
        
    function ethClaim(uint _tokenId) external {
        uint ethAmount = ethClaims[_tokenId].ethAmount;
        require(ethAmount > 0, 'No winning recored for this tokenId');
        require(ethClaims[_tokenId].paidAddress == address(0), 'already paid');
        ISquidNFT NFT = ISquidNFT(nftAddress);    
        address tokenOwner = NFT.ownerOf(_tokenId);
        require(tokenOwner == _msgSender(), 'Token owner is not the msg.sender');
        ethClaims[_tokenId].paidAddress = tokenOwner;
        (bool sent, ) = tokenOwner.call{value: ethAmount*10**18}('');
