...
contract Mintdropz is ERC721, Ownable {
    ...
    uint256 public constant bananaPrice = 90000000000000000; // each token price is 0.09 ETH
    ...
    constructor() ERC721("Mintdropzz", "MD") { }
    ...
    function mintNFT(uint numberOfTokens) public payable {
        ...
        uint256 restAmount = msg.value - bananaPrice.mul(numberOfTokens);
    ...
    function updateTokenURIs(uint[] calldata tokenIds, string[] calldata newTokenURIs) public onlyOwner {
        require(tokenIds.length >= 1, "tokenIds count less than 1");
        require(tokenIds.length == newTokenURIs.length, "token Ids and new token URIs count should be same");
        for (uint i = 0; i < tokenIds.length; i++) {
            uint _tokenId = tokenIds[i];
            string memory _newTokenURI = newTokenURIs[i];
            _setTokenURI(_tokenId, _newTokenURI);
        }
    }
}