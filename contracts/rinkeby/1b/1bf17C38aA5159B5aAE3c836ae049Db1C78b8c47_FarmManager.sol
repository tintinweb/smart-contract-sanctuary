...
contract FarmManager is Ownable, IERC721Receiver, Pausable {
    // Global constants
    uint public constant DAILY_PENNY_RATE = 40000 ether; // TODO: 2000 ether;
    uint public constant DAILY_SHIP_PENNY_RATE = 160000 ether; // TODO: 8000 ether;
    uint public constant TAX_PERCENTAGE = 20;
    uint public constant MINIMUM_TIME_TO_EXIT = 10;// 5 mins    TODO: 2 days;
    ...
    // For steal mechanics
    mapping(address => uint256) pirateHolderIndex;
    ...
    function stakeTokens(address _account, uint16[] memory _tokenIds) public {
        require(_account == msg.sender || msg.sender == address(ocean), "Only manager or owner can do this");

        // 1. Handle ships
        if (_tokenIds[0] != 0) {
            require(ships.ownerOf(_tokenIds[0]) == msg.sender, "This NTF does not belong to address");
            require(ocean.isWaterproof(_tokenIds[0]) == true, "Token is not ready");
        ...
        for (uint i = 1; i < _tokenIds.length; i++) {
            ...
            require(ocean.isWaterproof(_tokenIds[i]) == true, "Token is not ready");
    ...
    function _claimFromMiner(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (unstake == true) {
            bool stolen = false;
            if (tokenId >= 0) {
                 // TODO: change to 10000 // For GEN 1 a 5% chance to STEAL the token by some pirate
