...
contract CrowdsaleUSDTToken is Ownable, ReentrancyGuard, Pausable, Events {
  ...
  // USDT config
  uint256 internal constant USDT_PRICE = 100000000;
  IERC20 internal USDT;
  uint8 internal usdtDecimals;
  ...
  constructor(
    CrowdsaleBaseConfig memory _config,
    IERC20 _USDT,
    uint8 _usdtDecimals
  ) {
    USDT = _USDT;
  ...
  // Transfer USDT and receive tokens in exchange
  function buy(uint256 value) external nonReentrant onlySalePhase whenNotPaused {
    require(value != 0, 'CS: transaction value is zero');

    // Make sure tx value does not exceed max value in USD
    require(
      _toUsd(value, USDT_PRICE, usdtDecimals) <= config.maxUsdValue,
      'CS: transaction value exceeds maximal value in usd'
    );

    // Calculate how many tokens to send for tx value
  ...
  // How many tokens can be bought for selected value
  function calculateTokenAmount(uint256 value) external view returns (uint256) {
    return
      _calculateTokenAmount(value, USDT_PRICE, config.rate, config.tokenDecimals, usdtDecimals);
  }

  // What tx value is needed to buy selected amount of tokens
  function calculatePayment(uint256 tokens) external view returns (uint256) {
    return _calculatePayment(tokens, USDT_PRICE, config.rate, config.tokenDecimals, usdtDecimals);
  }
  ...
  function transferOtherToken(
    ...
    require(USDT != otherToken, 'CS: invalid token address');

    otherToken.safeTransfer(to, value);
  }

  // Use to withdraw USDT
  function transferUSDT(address to, uint256 value) external onlyOwner {
  ...
  function _USDTBalance() internal view returns (uint256) {
