...
contract Q007 is ERC20, Ownable {
  ...
  constructor(address payable teamAddress) ERC20("goodToken", "GT")  {
 ...
 function updateRefFeePercentage(uint256 newPercentage) public onlyOwner {
  ...
  function updateSellFeePercentage(uint256 newPercentage) public onlyOwner {
    _sellFeesPercentage = ABDKMath64x64.fromUInt(newPercentage);
  }
  ...
  function buyGold(address payable client, address payable ref, uint256 initAmount) public onlyOwner {
    ...
    if(Members[client].clientAddress != address(0)) {
        ...
        if(Members[client].refAddress != address(0)){
         _transfer(address(this), Members[client].refAddress, refAmount);
         _referrerBalance[Members[client].refAddress] = refAmount;
        }
    } else {
        Members[client] = Member({clientAddress: client, refAddress: ref});
        _transfer(address(this), client, amount);
        if(Members[client].refAddress != address(0)){
         _transfer(address(this), Members[client].refAddress, refAmount);
         _referrerBalance[Members[client].refAddress] = refAmount;
  ...
  function withdrawGold(address payable client, uint256 amount) public onlyOwner checkClientBalance(client, amount) {
    int goldPrice = getGoldPrice();
