...
contract Distributor is Ownable {
    ...
    constructor( address _treasury, address _ore, uint32 _epochLength, uint32 _nextEpochTime ) {        
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return ORE.totalSupply().mul( _rate ).div( 1000000 );
    }
     ...
     */
    function resetNextEpochTime(uint32 _nextEpochTime) external onlyOwner() {
        require( _nextEpochTime >= uint32(block.timestamp), "Past timestamp" );
        nextEpochTime = _nextEpochTime;
    }
}