...
contract AnyTokenEscrowV1 {
    ...
    modifier inState(State _state) {
        require(
            transactions[msg.sender].state == _state,
            "Invalid state."
        );
        _;
    }
    ...
    function create(address _seller, address _token, uint _amount) public inState(State.Inactive)
    {
    ...
    // until confirmed
    function accept(address _buyer) public inState(State.Created)
    {
