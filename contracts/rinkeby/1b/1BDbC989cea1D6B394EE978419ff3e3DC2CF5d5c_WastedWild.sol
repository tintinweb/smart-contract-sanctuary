...
contract WastedWild is ERC721Enumerable, Ownable {
    ...
    uint256 public _startSaleTimestamp = 1635692400;
    ...
    uint256 public _fourthWindow = 0; //
    ...
    function mint(uint256 tokensToMint) external payable {
        ...
        if(block.timestamp < _startSaleTimestamp + _firstWindow + _secondWindow){
            require(
                _arfw.balanceOf(msg.sender) > 0 || _arfes.balanceOf(msg.sender) > 0,
            ...
            require(
                _arbo1.balanceOf(msg.sender) > 0 || _arbo2.balanceOf(msg.sender) > 0,
    ...
    function setStartingIndex() external {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % _maxSupply;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % _maxSupply;
        }
    ...
    /*function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, ((tokenId + startingIndex) % _maxSupply).toString())) : "";
    }
    */
