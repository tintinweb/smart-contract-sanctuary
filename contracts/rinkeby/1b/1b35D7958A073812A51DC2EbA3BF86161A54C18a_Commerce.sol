   ...
   function editTokenWhiteListMerkleRoot(
    ...
    function purchase(
        ...
        for (uint i=0; i< _tokenIndexes.length; i++) {
            ...
            uint256 quantityToMint = getQualifiedAllocation(msg.sender,_tokenIndexes[i], _quantities[i],_merkleAmounts[i],_merkleProofs[i]); 
            ...
            if(tokens[_tokenIndexes[i]].isTokenPack){
                for (uint j=0; j < _quantities[i]; j++) {
                    ...
                    if(tokens[_tokenIndexes[i]].tokenPackConfig.isRandomPack){
                        ...
                        uint startingIndex = shuffle(inStockTokens, true)[0];
                        uint q = 0;
    ...
    function mint(
        address to,
        uint256 qty,
        uint256 token) public onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _mint(to, token, qty, "");
    }
            ...
            for (uint i=0; i < token.numTokenWhitelists; i++) {
                if(token.whitelistData[i].active){
                    if(token.isTokenPack && token.tokenPackConfig.isWhiteListBonusAggregatedAcrossAllWhiteLists){
                         _wl_amount += verifyWhitelist(sender, tokenIndex, i);
                    }
                    else{
                         _wl_amount = verifyWhitelist(sender, tokenIndex, i);
                    }
                    ...
                    if(token.whiteListConfig.requireAllWhiteLists){
                        require( verifyWhitelist(sender, tokenIndex, i) > 0, "v9");
                    }
    ...
    function verifyWhitelist(address sender, uint256 tokenIndex, uint whitelistIndex) internal view returns (uint256) {
       ...
       balanceOf = getExternalTokenBalance(sender, balanceRequest);
            ...
            if(token.isTokenPack){
                ...
                if(token.tokenPackConfig.allotOwnedTokenQuantity){
