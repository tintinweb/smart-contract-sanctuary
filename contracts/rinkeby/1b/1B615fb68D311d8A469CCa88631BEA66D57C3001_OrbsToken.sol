...
contract OrbsToken is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 count) public payable {
        ...
        require(msg.value == price * count, 'Wrong ether value');
    ...
    function claimFree() public {
        require(totalSupply() + 1 <= MAX, 'Not enough left');
        require(!claimedFree[_msgSender()], 'Already claimed free');
        require(geomes.balanceOf(_msgSender()) >= 2, 'Hodl at least 2 GEOMEs');

        uint256 mintIndex = totalSupply() + 1;
        creationDates[mintIndex] = block.number;
        claimedFree[_msgSender()] = true;
        _safeMint(msg.sender, mintIndex);
    }
