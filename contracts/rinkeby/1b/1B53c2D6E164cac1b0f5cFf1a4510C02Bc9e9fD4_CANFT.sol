...
contract CANFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 public constant CA_GIFT = 350;
    uint256 public constant CA_PUBLIC = 4650;
    uint256 public constant CA_MAX = CA_PUBLIC + CA_GIFT;
    uint256 public constant CA_PRICE = 0.02 ether;
    uint256 public constant CA_PER_MINT = 5;
    uint256 public constant CA_PER_MINT_PRIVATE = 2;
    ...
    constructor() ERC721("Cool Apes", "CA") {}
   ...
   function matchAddresSigner(bytes32 hash, bytes memory signature, bool isPrivate) private view returns(bool) {
    ...
    function buy(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity) external payable {
      ...
      require(totalSupply() < CA_MAX, "OUT_OF_STOCK");
      require(publicAmountMinted + tokenQuantity <= CA_PUBLIC, "EXCEED_PUBLIC");
    ...
    function privateBuy(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity) external payable {
      ...
      require(totalSupply() < CA_MAX, "OUT_OF_STOCK");
      require(publicAmountMinted + tokenQuantity <= CA_PUBLIC, "EXCEED_PUBLIC");
      require(tokenQuantity <= CA_PER_MINT_PRIVATE, "EXCEED_CA_PER_MINT");
      require(CA_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
      require(_usedAddresses[msg.sender] + tokenQuantity <= CA_PER_MINT_PRIVATE, "EXCEED_ALLOC");
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= CA_MAX, "MAX_MINT");
        require(giftedAmount + receivers.length <= CA_GIFT, "GIFTS_EMPTY");
