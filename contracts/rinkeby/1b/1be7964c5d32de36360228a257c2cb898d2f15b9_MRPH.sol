...
contract MRPH is Controllable, Pausable, iERC20 {
	...
	string public constant symbol = "MRPH";
	...
	function transferToken(address tokenAddress, uint256 amount) public onlyController {
		iERC20(tokenAddress).safeTransfer(msg.sender,amount);
	}

	function flushToken(address tokenAddress) public onlyController {
		uint256 amount = iERC20(tokenAddress).balanceOf(address(this));
		iERC20(tokenAddress).safeTransfer(msg.sender,amount);
	}

	function burn(uint256 _value) public onlyController returns (bool) {
	...
	function freeze(address _address, bool _state) public onlyController returns (bool) {
