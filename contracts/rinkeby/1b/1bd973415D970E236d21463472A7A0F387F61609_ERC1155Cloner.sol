...
contract ERC1155Cloner {
  address immutable tokenImplementation;

  constructor(){
    tokenImplementation = address(new MOMENTS());
  }

  function createToken() external returns (address) {
    address clone = Clones.clone(tokenImplementation);
    MOMENTS(clone).approveAdmin(msg.sender);
    return clone;
  }
}
...
contract MOMENTS is ERC1155Creator {
    ...
    external
    {
        ...
        mintBaseNew(address[] calldata to, uint256[] calldata amounts, string[] calldata uris)
