...
contract FarmManager is Ownable, IERC721Receiver, Pausable {
    ...
    function stakeTokensToLand(address account, uint16 shipId, uint16[] memory tokenIds) public {
        // 1. Handle ships
        if (shipId != 0) {
            require(busToken.ownerOf(shipId) == msg.sender ||
                    busToken.ownerOf(shipId) == address(manager), "This NTF does not belong to address");
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            require(testToken.ownerOf(tokenIds[i]) == msg.sender ||
                    testToken.ownerOf(tokenIds[i]) == address(manager), "This NTF does not belong to address");
