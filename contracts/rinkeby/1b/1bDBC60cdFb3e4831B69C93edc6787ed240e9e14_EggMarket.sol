...
contract EggMarket is BaseAccessControl {
    ...
    event TokenBought(
        address indexed buyer, address indexed to, uint tokenId, DragonInfo.Types dragonType);
    ...
    event PublicSaleStarted(
        address operator, uint legendaryPrice, uint epicPrice, uint randomPrice);
    ...
    function allowPresale(uint totalAllowedPresaleCnt, uint psPrice) external onlyRole(COO_ROLE) {
        require(!publicSaleStarted(), "EggMarket: unable to allow presale");
        _totalAllowedPresaleCount = totalAllowedPresaleCnt;
        _presalePrice = psPrice;
        _currentPresaleCount = 0;
        emit PresaleAllowed(_msgSender(), totalAllowedPresaleCnt, psPrice);
    }
    ...
    function setPublicPrice(DragonInfo.Types drgType, uint newValue) external onlyRole(CFO_ROLE) {
        ...
        emit ValueChanged(abi.encodePacked("publicPrices.", drgType), previousValue, newValue);
    }
    ...
    function togglePublicSaleStarted(
        uint legendaryPrice, 
        uint epicPrice, 
        uint randomPrice) external onlyRole(COO_ROLE) {
    ...
    function buyDefinite(address to, DragonInfo.Types drgType) external payable {
        require(!Address.isContract(to), "EggMarket: address cannot be contract");
        require(publicSaleStarted(), "EggMarket: unable to buy a definite egg");
        require(msg.value >= publicPrice(drgType), "EggMarket: incorrect amount sent to the contract");
    ...
    function claim(address to, uint count) external {
        ...
        for (uint i = 0; i < count; i++) {
            uint tokenId = eggt.mint(to, DragonInfo.Types.Unknown);
    ...
    function _buy(address to, uint count) internal {
        ...
        require(msg.value >= count * publicPrice(DragonInfo.Types.Unknown), 
            "EggMarket: incorrect amount sent to the contract");
        EggToken eggt = EggToken(tokenAddress());
        for (uint i = 0; i < count; i++) {
            uint tokenId = eggt.mint(to, DragonInfo.Types.Unknown);
            emit TokenBought(_msgSender(), to, tokenId, DragonInfo.Types.Unknown);
        }
    }

    function _presaleBuy(address to, uint count) internal {
        require(currentPresaleCount() + count <= totalAllowedPresaleCount(), "EggMarket: exceed total allowed presale count");
