...
contract CompoundRewardPool {
    ...
    function stake(address _for, uint256 amount) public updateReward(_for) {
        require(amount > 0, "Cannot stake 0");

        emit Staked(_for, amount);
    }

    function withdraw(address _for, uint256 amount) public updateReward(_for) {
...
contract CompoundBooster {
    ...
    function deposit(
        ...
        IRewardPool(pool.rewardCompPool).stake(msg.sender, _amount);
        IRewardPool(pool.rewardInterestPool).stake(msg.sender, _amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public returns (bool) {
        ...
        uint256 depositAmount = IVirtualBalanceWrapper(pool.virtualBalance).balanceOf(
            msg.sender
        );
        ...
        IRewardPool(pool.rewardCompPool).withdraw(msg.sender, _amount);
        IRewardPool(pool.rewardInterestPool).withdraw(msg.sender, _amount);
    ...
    function claimComp() external returns (bool) {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            ...
            if (claimed) {
                IRewardPool(poolInfo[i].rewardCompPool).notifyRewardAmount(
                    rewards
                );
            }
        }
    ...
    function borrow(
        ...
        if (pool.isErc20) {
            ...
            if (bal > 0) {
                ...
                IRewardPool(pool.rewardInterestPool).notifyRewardAmount(
                    exchangeReward
                );

                IRewardPool(pool.rewardVeLendFlarePool).notifyRewardAmount(
                    lendflareDeposterReward
                );
            }
        } else {
    ...
    function _repayBorrow(
        ...
        if (_isErc20) {
            ...
            if (bal > 0) {
                ...
                IRewardPool(pool.rewardVeLendFlarePool).notifyRewardAmount(bal);
            }
        } else {
            uint256 bal = ICompoundProxyUserTemplate(lendingInfo.proxyUser)
...
contract CompoundProxyUserTemplate {
    ...
    function autoClaimComp(address _asset) internal {
        if (claimComp) {
            ...
            IRewardPool(compReward).notifyRewardAmount(bal);
        }
    }
