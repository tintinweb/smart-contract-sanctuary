...
contract nft_drops is ERC721, Ownable, VRFConsumerBase {
    ...
    PrimeSetted public prime;

    event statusChanged(MintStatus sta);
    event BaseURIChanged(string baseURI);
    event PrimeNumberSetted(PrimeSetted pst);
    ...
    function drawingLots(
        ...
        for (uint256 tokenId = startTokenId; tokenId <= endTokenId; tokenId++) {
            ...
            nft.safeTransferFrom(
                msg.sender,
                this.ownerOf(RANDOM_NUMBER % nextTokenId),
                tokenId
            );
        }
    }

    function ethLottery(uint256 winners, uint256 amount) external onlyOwner {
        require(address(this).balance > winners * amount, "Not enough ETH");
        ...
        for (uint256 i = 1; i <= winners; i++) {
            RANDOM_NUMBER = linearCongruential(RANDOM_NUMBER);
            (bool success, ) = (this.ownerOf(RANDOM_NUMBER % nextTokenId)).call{
                value: amount
            }("");
            require(success, "Withdraw unsuccessful");
        }
    ...
    // PrimeOne will be random 20 digits prime number
    ...
    function setPrimeNumbers(
        ...
        require(prime == PrimeSetted.NO, "Prime numbers are already setted");
        primeOne = _primeOne;
        primeTwo = _primeTwo;
        primeThree = _primeThree;
        prime = PrimeSetted.YES;
        emit PrimeNumberSetted(prime);
    }
