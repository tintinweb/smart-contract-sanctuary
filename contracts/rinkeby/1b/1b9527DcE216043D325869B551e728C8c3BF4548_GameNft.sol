...
contract GameNft is IGameNft, ERC721Enumerable, Ownable, Pausable {
  struct LastWrite {
    uint64 time;
    uint64 blockNum;
  }

  event TemplarMinted(uint256 indexed tokenId);
  event DemonMinted(uint256 indexed tokenId);
  event TemplarStolen(uint256 indexed tokenId);
  event DemonStolen(uint256 indexed tokenId);
  event TemplarBurned(uint256 indexed tokenId);
  ...
  mapping(uint256 => TemplarDemon) private tokenTraits;
  ...
  // 0 - 9 are associated with Templar, 10 - 18 are associated with Demons
  uint8[][18] public rarities;
  // list of aliases for Walker's Alias algorithm
  // 0 - 9 are associated with Templar, 10 - 18 are associated with Demons
  uint8[][18] public aliases;

  // reference to the Temple contract to allow transfers to it without approval
  ...
  constructor(uint256 _maxTokens) ERC721('Templars & Demons Game', 'WnD') {
  ...
  modifier blockIfChangingAddress() {
    ...
    require(
      admins[_msgSender()] || lastWriteAddress[tx.origin].blockNum < block.number,
      'hmmmm what doing?'
    );
    _;
  }
  ...
  function mint(address recipient, uint256 seed) external override whenNotPaused {
    ...
    if (tx.origin != recipient && recipient != address(temple)) {
      // Stolen!
      if (tokenTraits[minted].isTemplar) {
        emit TemplarStolen(minted);
      } else {
  ...
  function burn(uint256 tokenId) external override whenNotPaused {
    ...
    if (tokenTraits[tokenId].isTemplar) {
      emit TemplarBurned(tokenId);
    } else {
  ...
  function generate(
    ...
    uint256 seed,
    LastWrite memory lw
  ) internal returns (TemplarDemon memory t) {
    t = selectTraits(seed);
    if (existingCombinations[structToHash(t)] == 0) {
      tokenTraits[tokenId] = t;
      existingCombinations[structToHash(t)] = tokenId;
      if (t.isTemplar) {
        emit TemplarMinted(tokenId);
      } else {
   ...
   */
  function selectTraits(uint256 seed) internal view returns (TemplarDemon memory t) {
    t.isTemplar = (seed & 0xFFFF) % 10 != 0;
    uint8 shift = t.isTemplar ? 0 : 9;
    seed >>= 16;
   ...
   */
  function structToHash(TemplarDemon memory s) internal pure returns (uint256) {
    return
      uint256(
        keccak256(
          abi.encodePacked(
  ...
  /**
   * checks if a token is a Templars
   * @param tokenId the ID of the token to check
   * @return templar - whether or not a token is a Templars
   */
  function isTemplar(uint256 tokenId)
    external
    view
    override
    ...
    returns (bool)
  {
    // Sneaky demons will be slain if they try to peep this after mint. Nice try.
    IGameNft.TemplarDemon memory s = tokenTraits[tokenId];
    return s.isTemplar;
  }
