...
contract ArielBeckerArtCore is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
	...
	struct TokenStruc {
		bytes data; // Contenido en sí (imagen, texto, video, audio, etc).
		uint256 date; // Fecha de creación
		uint256 documentType; //Tipo de documento
		string json_attributes; //Metadatos (guardar acá dentro la URI del contenido renderizado; ver https://opensea-creatures-api.herokuapp.com/api/creature/3
	}
	...
	string private _baseTokenURI = "https://arielbecker.com/render/token.php";
	...
	function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
		// La URL base debe tener ya la ruta para el token, por ejemplo "https://arielbecker.com/render/token.php"
	...
	function mint(bytes memory _data, uint256 _documentType, string memory _json) public virtual {
		...
		beckers.push(TokenStruc(_data, block.timestamp, _documentType, _json));

		_mint(_msgSender(), _tokenIdTracker.current());
		_tokenIdTracker.increment();
	}

	// This function returns only the “media” content (image, text, whatever you stored as primary data)
	function retrieveMedia(uint256 _tokenId) public view returns (bytes memory) {
		require(_tokenId >= 0, "Token ID cannot be less than zero!");
		require(_tokenId < totalSupply(), "Token ID out of bounds!");
		return beckers[_tokenId].data;
	}

	// This function returns only the metadata content.
	// Please refer to https://opensea-creatures-api.herokuapp.com/api/creature/3 as an example on the JSON format and content.
	function retrieveMetadata(uint256 _tokenId) public view returns (string memory) {
		require(_tokenId >= 0, "Token ID cannot be less than zero!");
		require(_tokenId < totalSupply(), "Token ID out of bounds!");
		return beckers[_tokenId].json_attributes;
	}

	// This function returns only the data type.
	function retrieveDataType(uint256 _tokenId) public view returns (uint256) {
		require(_tokenId >= 0, "Token ID cannot be less than zero!");
		require(_tokenId < totalSupply(), "Token ID out of bounds!");
		return beckers[_tokenId].documentType;
	}

	// This function returns media, metadata, and media type.
	function retrieveContents(uint256 _tokenId) public view returns (bytes memory, string memory, uint256) {
		require(_tokenId >= 0, "Token ID cannot be less than zero!");
		require(_tokenId < totalSupply(), "Token ID out of bounds!");
		return (beckers[_tokenId].data, beckers[_tokenId].json_attributes, beckers[_tokenId].documentType);
	}
