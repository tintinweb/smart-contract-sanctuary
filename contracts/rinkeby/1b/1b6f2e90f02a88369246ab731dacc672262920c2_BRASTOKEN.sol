...
*/
interface IbladeRunnerapes {
    function balanceApes(address owner) external view returns(uint256);
}

/**
 * @dev BRAS token reward
*/
     
contract BRASTOKEN is ERC20, Ownable {
    ...
    address public  bladeRunnerapesAddress;
    address public  cityAddress;
    
    constructor() ERC20("BRAS", "BRAS") {
   ...
   function setbladeRunnerapesContractAddress(address contractAddress) public onlyOwner {
        bladeRunnerapesAddress = contractAddress;
   }
    

    function updateReward(address from, address to) external {
        require(msg.sender == bladeRunnerapesAddress);
        if(from != address(0)){
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[user], "Address needs to aprpove burn first");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        IbladeRunnerapes bladeRunnerapes = IbladeRunnerapes(bladeRunnerapesAddress);
        return bladeRunnerapes.balanceApes(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
    }
