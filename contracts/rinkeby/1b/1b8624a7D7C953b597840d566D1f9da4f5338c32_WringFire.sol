...
contract WringFire is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, Pausable {
  ...
  constructor() ERC721("Wring Fire", "WRING") Ownable() {}
  ...
  function reserveTokens(address _to, uint256 _reserveAmount) public onlyOwner {
    ...
    function shuffleStringArray(string[] memory sourceArray) internal view returns (string[] memory) {
      ...
      // for (string i = 0; i < sourceArray.length; i++) {
      //   result[i] = i + 1;
      // }
      ...
      for (uint i = 1; i < sourceArray.length - 1; i++) {
        ...
        string memory aux = result[last_item];
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Lootbox #', toString(tokenId), '", "description": "Altered Acoustics description.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
