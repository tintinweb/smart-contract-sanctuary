...
contract HashBridge is Ownable {
	...
	event OfferAdd(uint indexed offerId, address indexed token, address indexed payToken, address ownerAddress, address payAddress, uint amount, uint rate, bool active);
	event OfferUpdate(uint indexed offerId, address payAddress, uint amount, uint rate, bool active);
	...
	function addOffer(address _token, uint _amount, address _payToken, uint _rate, address _payAddress) public {
		require(_amount > 0, "Amount must be greater than 0");
		TransferHelper.safeTransferFrom(_token, msg.sender, address(this), _amount);
		uint offerId = offers.length;
		offers.push(Offer(_token, _amount, _payToken, _rate, msg.sender, _payAddress, true));
		emit OfferAdd(offerId, _token, _payToken, msg.sender, _payAddress, _amount, _rate, true);
	}
	
	function updateOffer(uint _offerId, uint _amount, uint _rate, address _payAddress) public {
		...
		if (_amount > offers[_offerId].amount) {
			...
			TransferHelper.safeTransferFrom(offers[_offerId].token, address(this), msg.sender, offers[_offerId].amount - _amount);
		}
		offers[_offerId].amount = _amount;
		offers[_offerId].rate = _rate;
		offers[_offerId].payAddress = _payAddress;
		emit OfferUpdate(_offerId, _payAddress, _amount, _rate, offers[_offerId].active);
	}
	...
	function activateOffer(uint _offerId) public {
		...
		emit OfferUpdate(_offerId, offers[_offerId].payAddress, offers[_offerId].amount, offers[_offerId].rate, true);
	}
	...
	function deactivateOffer(uint _offerId) public {
		...
		emit OfferUpdate(_offerId, offers[_offerId].payAddress, offers[_offerId].amount, offers[_offerId].rate, false);
	}
	...
	function withdrawTokens(uint _orderId, bytes calldata _sign) public {
		...
		//ToDo: проверить/отрефакторить сверку подписи
		...
		TransferHelper.safeTransferFrom(offers[offerId].token, address(this), orders[_orderId].withdrawAddress, amount);
