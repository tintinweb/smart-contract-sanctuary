...
contract SushiToken is IERC20, Context, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!antibotPaused) {
            if(sender != owner() && recipient != owner()) {
                require(amount <= totalSupply().mul(maxTxPercent).div(1000), "Overflow max transfer amount");
            }
            require(!blackList[sender], "Blacklisted seller");
            if(lastTxTimestamp[recipient] != 0 && recipient != uniswapRouter) {
                require(lastTxTimestamp[recipient].add(transferDelay) >= block.timestamp, "Can not transfer within limit period");
            }
