    ...
    AccessControlUpgradeable
{
        ...
        returns (uint256 returnedCollateral)
    {
        ...
        _mint(msg.sender, returnedCollateral);
    ...
    function redeem(uint256 _amount) external whenNotPaused {
        ...
        require(receipt[0] != 1, "iCOMP : Insuficient collaterals");
        require(
            balanceOf(msg.sender) > receipt[1] + receipt[2],
            "iCOMP : Insufucient collaterals!"
        );
        require(
            allowance(msg.sender, address(this)) >=
                receipt[1] + receipt[2] + receipt[1],
            "iCOMP : Insuducient allowance!"
        );

        if(receipt[1] > 0){
            require(
                transferFrom(msg.sender, address(STRATEGIST), receipt[1]),
                "iCOMP : Fees tx failed!"
            );
        }

        shares[msg.sender] -= receipt[3];
        if (farming == ETHER) {
            uint256 transferedAmount = sellETHPosition(receipt[2]);
            payable(msg.sender).transfer(transferedAmount);
        } else if (farming != ETHER) {
            IERC20Upgradeable asset = IERC20Upgradeable(farming);
            uint256 returned = sellERC20Position(receipt[2]);
            asset.transfer(msg.sender, returned);
        }
        _burn(msg.sender, receipt[2]);
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 underlyingMax = (rate * balanceOf(msg.sender)) / 1e18;
        ...
        if (underlyingMax >= _amount) {
            ...
            if (amountWithdraw <= balanceOf(msg.sender)) {
                txData[0] = 0; // 0 error
            ...
            } else if (amountWithdraw > balanceOf(msg.sender)) {
                ...
                deductedFees = _getCollateralAmount(
                    (((gainQuotient * shares[msg.sender]) / feeBase) * fees) /
                        1e18
                );
                deductedFees = _getCollateralAmount(
                    ((((gainQuotient * shares[msg.sender]) / 1e18) -
                        shares[msg.sender]) / feeBase) * fees
                );
                shareConsumed = (rate * balanceOf(msg.sender)) / 1e18;
        ...
        returns (uint256 collateralAmount)
    {
        collateralAmount =
            (_amount * 1e18) /
            CErc20(collateral).exchangeRateStored();
    ...
    /// @dev decimals to match Compound asset
