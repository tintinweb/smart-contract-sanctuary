...
contract TMCTestWLFinal  is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;


    uint public  maxSupply =3500;
    uint private whiteListCounter = 3;
    ...
    constructor(string memory baseURI) ERC721("TMCTest", "TMCTest2")  {
        setBaseURI(baseURI);
    }
    ...
    function reserveTokens(uint quantity) public onlyOwner {
     ...
     require(isPreSalePaused == false, "Sale is not active at the moment");
      require(totalSupply()<maxSupply,"Quantity is greater than remaining Supply");
        ...
        require(myaddress.counter>=1,"The Amount Selected is Greater than the Remaining Amount of the Token at this Address");
    ...
    function mint(uint quantity) public payable {
        ...
        require(totalSupply() + quantity<=maxSupply,"Quantity must be lesser then MaxSupply");
        require(price * quantity== msg.value, "Sent ether value is incorrect");
