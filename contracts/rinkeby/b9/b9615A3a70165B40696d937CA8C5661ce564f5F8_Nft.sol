...
contract Nft is Ownable, ERC721Enumerable, ERC721Referralable, ToggleableSale, WhitelistVerifier {
  ...
  constructor() ERC721('NFT Test Name', 'NFTSYM') {
    //TEST VALUES
    MAX_ITEMS = 10000;
    ...
    isSaleActive = true;
    // test - acc3 mm
    charityDonationAddress = 0x7957bF3A045b0FD356FF6DC33B44E15089549d04;
    singleReferalValue = 0.01 ether;
  }
  ...
  function mint(uint256 amount, uint256 referralTokenId) external payable whenSaleActive {
    ...
    sendReferrals(amount, referralTokenId);
  }

  function presaleMint(uint256 amount, uint256 referralTokenId) external payable whenPresaleActive {
    require(presaleMinted < MAX_PRESALE_ITEMS, 'Presale sold out');
    require(
      presaleMinted + amount <= MAX_PRESALE_ITEMS,
      'Purchase would exceed presale supply cap'
    );
...
contract ToggleableSale is Ownable {
  ...
  function setPresaleState(bool _isActive) external onlyOwner {
