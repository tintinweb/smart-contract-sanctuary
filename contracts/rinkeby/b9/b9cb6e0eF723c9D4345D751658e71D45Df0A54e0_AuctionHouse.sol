...
contract AuctionHouse is Ownable {
	...
	uint256 minSaleTime = 86400; // 1 day
	...
	//maxSaleDurationInSeconds - if you go over it, the sale is canceled
	function putForSale(
		address nftContract,
		uint256 nftID,
		uint256 price,
		uint256 maxSaleDurationInSeconds
	) public {
		require(price >= 0.01 ether, "price should be >= 0.01 ether");
		require(nftContract != address(0), "tokenContract != address(0)");
		require(maxSaleDurationInSeconds >= minSaleTime, "sale time less than minimum");
		...
		SellListing memory sl =
			SellListing(
				...
				nftContract,
				block.timestamp,
				block.timestamp + maxSaleDurationInSeconds,
				price
			);

		sellListings[sellsCount] = sl;
		sellsCount = sellsCount + 1;
		emit OnSale(nftContract, nftID, price, block.timestamp + maxSaleDurationInSeconds);
	}

	//unsold items are retured to the owner
	function claimUnsold(uint256 saleID) public {
		SellListing storage sl = sellListings[saleID];
		require(block.timestamp > sl.endTime, "sale period is active");

		//transfer the tokens
		if (isERC721(sl.nftContract)) {
			IERC721 auctionToken = IERC721(sl.nftContract);
			auctionToken.safeTransferFrom(address(this), sl.seller, sl.nftID);
		} else if (isERC1155(sl.nftContract)) {
			IERC1155 auctionToken = IERC1155(sl.nftContract);
			auctionToken.safeTransferFrom(address(this), sl.seller, sl.nftID, 1, "");
		} else {
			revert("only ERC721 and ERC1155 types of tokens are supported for claiming");
		}
	}
