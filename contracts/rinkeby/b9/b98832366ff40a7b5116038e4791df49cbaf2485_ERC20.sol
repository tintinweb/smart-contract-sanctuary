...
contract ERC20 is Context, IERC20, Ownable, HoldersStorage {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 fees = amount / 100 * feesPercent();
        amount = amount - fees;
     ...
     */
    function feesPercent() public view virtual returns (uint8) {
     ...
     */
    function totalFeesBalance() public view virtual returns (uint256) {
     ...
     */
    function totalHoldersBalance() public view virtual returns (uint256) {
        uint256 _totalBalanceOfAllHolders = 0;
        for (uint256 s = 0; s < _holders.length; s++) {
            _totalBalanceOfAllHolders = _totalBalanceOfAllHolders + balanceOf(_holders[s]);
        }
    ...
    function rewardBalanceOf(address account) public view virtual returns (uint256) {
        return totalFeesBalance() / totalHoldersBalance() * balanceOf(account);
    }

    function destributeFeeReward(uint256 amount) internal virtual onlyOwner { 
        uint256 _totalBalanceOfAllHolders = totalHoldersBalance();
        _transferFeesBalance = _transferFeesBalance.sub(amount, "ERC20: Destributed amount exceeds fee address balance");
        for (uint256 s = 0; s < _holders.length; s++) {
            _balances[_holders[s]] = _balances[_holders[s]].add(amount / _totalBalanceOfAllHolders * balanceOf(_holders[s]));
        }
    }
}