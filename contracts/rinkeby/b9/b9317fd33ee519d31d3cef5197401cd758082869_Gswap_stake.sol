...
contract Gswap_stake is Authorizable{
    ...
    function drawToken(uint _tokenId) public {
        uint _value = drawableValue(_tokenId);
    ...
    // 是否能领取token
    function drawableValue(uint _tokenId) public view returns (uint) {
        if(tokenUserDrawMap[_tokenId][msg.sender]){
            return 0;
        }
        IToken token = IToken( tokenErc20Map[_tokenId]);
        return token.totalSupply().mul(tokenOtherKRate).div(1000).mul(tokenUserMap[_tokenId][msg.sender]).div(goalMap[_tokenId]);
    }
    ...
    function fallbackValue(uint _tokenId, address _to) public view returns (uint){
        ...
        return userStaked(_to,_tokenId);
    ...
    function setRate(uint _usdg2OwnerKRate,uint _token2OwnerKRate,uint _token2OtherKRate ) onlyOwner public {
    ...
    function authedSendToken(address _token, address _to, uint _value) external onlyAuthorized{
        IUsdg(_token).transfer( _to, _value);
    }
