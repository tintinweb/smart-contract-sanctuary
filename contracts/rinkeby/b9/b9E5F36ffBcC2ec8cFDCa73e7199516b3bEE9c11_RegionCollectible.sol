...
contract RegionCollectible is BDERC1155Tradable, ERC1155Holder {
    ...
    event RegionCreated(uint64 indexed region, address indexed owner, uint64 price, uint32 timeStamp);
    event RegionSold(uint64 indexed region, address indexed owner, uint64 price, uint32 timeStamp);
    event RegionBought(uint64 indexed region, address indexed owner, uint64 price, uint32 timeStamp);
    ...
    function newRegion() public returns (uint256 theNewRegion) {
        ...
        emit RegionCreated(uint64(_id), msg.sender, uint64(regionFee), uint32(block.timestamp));
        return _id;
    }
    ...
    function sellRegion(uint256 regionID) public ownersOnly(regionID) {
        ...
        //Emit the event
        emit RegionSold(uint64(regionID), msg.sender, uint64(regionPayout), uint32(block.timestamp));
    ...
    function buyRegion(uint256 regionID) public {
        ...
        //Emit the event
        emit RegionBought(uint64(regionID), msg.sender, uint64(regionFee), uint32(block.timestamp));
