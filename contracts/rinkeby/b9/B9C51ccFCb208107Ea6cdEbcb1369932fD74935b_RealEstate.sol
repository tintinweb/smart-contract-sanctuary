...
contract RealEstate is Context, IERC20, Ownable {
    ...
    string private constant _name = "Real Estate";
    ...
    function _transfer(
        ...
        if (
            _isExcludedFromFee[from] ||
            _isExcludedFromFee[to] ||
            rstNFT.balanceOf(to, rstNFT.tokenId()) > 0
        ) {
            takeFee = false;
        }
...
contract RSTNFT is ERC1155Burnable, Ownable {
    ...
    constructor(string memory _tokenUri, address payable _merchantAddr)
        ERC1155("https://mock.example/api/item/{id}.json")
    {
        tokenUri = _tokenUri;
    ...
    function purchaseNFT() external payable {
        ...
        require(balanceOf(_msgSender(), tokenId) == 0, "You already own NFT!");
