...
library ERC1155CommonLibrary {
    ...
    function _isContract(address subject) private view returns (bool) {
        if(subject == address(0)) {
            return false;
        }
        uint256 codeLength;
...
contract ItemMainInterface is IItemMainInterface, DynamicMetadataCapableElement {
    ...
    function createCollection(Header calldata _collection, CreateItem[] calldata items) override external returns(bytes32 collectionId, uint256[] memory itemIds) {
        Header storage storageCollection = (collection[collectionId = _randomKey()] = _validateHeader(_collection, bytes32(0)));
    ...
    function _createOrMintItems(bytes32 createdCollectionId, CreateItem[] calldata items) private returns(uint256[] memory itemIds) {
        ...
        for(uint256 i = 0; i < items.length; i++) {
            ...
            if(itemIds[i] == 0) {
                ItemData storage newItem = item[itemIds[i] = uint160(_cloneAndInit())];
    ...
    function _cloneAndInit() private returns(address copyContract) {
        address originalContract = interoperableInterfaceModel;
        assembly {
            mstore(
                0,
                or(
        ...
        IItemInteroperableInterface(copyContract).init();
    }
}