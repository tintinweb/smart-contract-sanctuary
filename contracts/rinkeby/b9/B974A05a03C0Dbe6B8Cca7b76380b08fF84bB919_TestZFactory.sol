...
contract TestZ is ERC721Enumerable, Ownable {
  /** STORAGE */
...
contract TestZFactory is Ownable {
  /*** MODIFIERS ***/
   ...
   */
  modifier isLive() {
    require(saleLive, "TestZFactory: Sale is not currently in progress");
    _;
  }
  ...
  function mint(uint256 _quantity, address _toAddress) public payable isLive {
    require(canMint(_quantity), "TestZFactory: Unable to mint token");
    require(msg.value >= (_quantity * TOKEN_PRICE), "TestZFactory: Insufficient funds");

    TestZ testZ = TestZ(nftAddress);
    for (uint256 i=0; i<_quantity; i++) {
      testZ.mintTo(_toAddress);
    }
  }
  ...
  function canMint(uint256 _quantity) public view returns (bool) {
    ...
    TestZ testZ = TestZ(nftAddress);
    uint256 totalSupply = testZ.totalSupply();
