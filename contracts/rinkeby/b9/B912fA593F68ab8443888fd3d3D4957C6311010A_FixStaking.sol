...
contract FixStaking is AccessControl, Pausable {
    ...
    event SetMinMax(uint256 minStake);
        ...
        isPoolExist(_poolIndex)
    {
        ...
        require(
            pool.reserve >=
                _calculateRewFromAmount(totStakedAmount, _poolIndex),
        ...
        isFinished(msg.sender, _stakerIndex)
    {
        ...
        require(
            block.timestamp >= getClosedTime(msg.sender, _stakerIndex),
        ...
        require(
            pool.reserve >=
                _calculateRewFromAmount(pool.stakedAmount + reward, poolIndex),
        ...
        isFinished(msg.sender, _stakerIndex)
    {
        ...
        uint256 totalPending = staker.amount + staker.reward;
        ...
        )
    {
        ...
        StakerInfo memory stakerInf = stakers[_staker][_stakerIndex];
        closedTime = getClosedTime(_staker, _stakerIndex);
        futureReward = calculateFutureRew(_staker, _stakerIndex);
    ...
    /**
     * Calculates the reward that the person will receive at the end of the pool.
     */
    function calculateFutureRew(address _staker, uint256 _stakerIndex)
        public
        view
        returns (uint256)
    {
        ...
        uint256 rate = pools[staker.poolIndex].apy;
        uint256 duration = pools[staker.poolIndex].duration;
        uint256 rateToSec = (rate * 1e36) / 30 days;
        uint256 percent = (rateToSec * duration) / 1e18;
    ...
    /**
     * transfers the token to the user, then set as finished the index.
     */
    ...
    /**
     * calculates the reward corresponding to the amount
     */
    function _calculateRewFromAmount(uint256 _amount, uint256 _poolIndex)
        private
        view
        returns (uint256)
    {
