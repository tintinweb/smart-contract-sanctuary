...
interface INostraSwapFactory {
    ...
    function createPair(address token1) external returns (address payable pair);
...
contract NostraSwapPair is NostraSwapERC20 {
    ...
    receive() external payable {
        assert(msg.sender == token0); // only accept ETH via fallback from the WETH contract
    }
    ...
    function swap(
        ...
        uint amount1In;
        {
            ...
            arb(balance0 - amount0Out, balance1 - amount1Out, bonusReceiver);
        }
        
        {
            ...
            _update(IERC20(_token0).balanceOf(address(this)), IERC20(_token1).balanceOf(address(this)));
            emit Swap(msg.sender, amount0In, amount1In, amount0Out, amount1Out, to);
        }
    }

    function arb(uint o0, uint o1, address to) private {
        uint cost = tx.gasprice * 100000;
        uint u0;
        uint u1;
        address _token0 = token0;
        address _token1 = token1;
        address _externalPool = externalPool;
        
        u0 = (IERC20(_token0).balanceOf(_externalPool));
        u1 = (IERC20(_token1).balanceOf(_externalPool));
        if (o0 > cost) {
            o0 -= cost;
            ...
            if (UQ112x112.encode(uint112(o0)).uqdiv(uint112(o1)) > UQ112x112.encode(uint112(u0)).uqdiv(uint112(u1))) {
                ...
                uint getAmountOut = getAmountOut(tok, u0, u1);
                if ((o0 - tok) * (o1 + getAmountOut) >= (o0 + cost) * o1) {
                    _safeTransfer(_token0, _externalPool, tok);
                    if (IExternalNostraSwapPair(_externalPool).token0() == _token0) {
                        IExternalNostraSwapPair(_externalPool).swap(0, getAmountOut, address(this), new bytes(0));
                    } else {
                        IExternalNostraSwapPair(_externalPool).swap(getAmountOut, 0, address(this), new bytes(0));
                    }
                    IWETH(_token0).withdraw(cost);
                    (bool success, ) = to.call{value: cost}(new bytes(0));
                    require(success, 'TransferHelper::safeTransferETH: ETH transfer failed');
                }
            } else {
                uint tok = (Math.sqrt(o1 + u1) * Math.sqrt(u0 * u1)) / Math.sqrt(o0 + u0) - u1;
                uint getAmountOut = getAmountOut(tok, u1, u0);
                if ((o1 - tok) * (o0 + getAmountOut) >= (o0 + cost) * o1) {
                    _safeTransfer(_token1, _externalPool, tok);
                    if (IExternalNostraSwapPair(_externalPool).token0() == _token0) {
                        IExternalNostraSwapPair(_externalPool).swap(getAmountOut, 0, address(this), new bytes(0));
                    } else {
                        IExternalNostraSwapPair(_externalPool).swap(0, getAmountOut, address(this), new bytes(0));
...
contract NostraSwapFactory is INostraSwapFactory {
    ...
    function createPair(address token1) external override returns (address payable pair) {
        address token0 = WETH;
