...
contract Momento is IERC20Metadata, Ownable {
  ...
  mapping(address => MaxTransferPerDay) private _maxTxnPerDay;
  ...
  // 0.3% total supply max sell/transfer
  uint256 public maxTransferOrSellPerDay = (_tTotal * 3) / 1000;
  ...
  function setTeamAddress(address _stakingAddress) public onlyOwner {
  ...
  function setMaxTransferOrSellPerDay(uint256 _maxTenthPercent)
    external
    onlyOwner
  {
    maxTransferOrSellPerDay = (_tTotal * _maxTenthPercent) / 1000;
  }
  ...
  function _transfer(
    ...
    if (from != address(this) && to != address(this)) {
      ...
      if (_isUniswapV2Pair[from] && to != address(uniswapV2Router)) {
        ...
        _enforceMaxTransferSellPerDay(from, to, amount);
  ...
  function buyBackAndBurn(uint256 _amountETH) external onlyOwner {
    require(
      ...
      'trying to buy back and burn more than balance avaialable'
    );
    if (_amountETH > 0) {
  ...
  function _enforceMaxTransferSellPerDay(
    address from,
    address to,
    uint256 amount
  ) private {
    if (from != owner() && to != owner()) {
      uint256 _midnightToday = (block.timestamp / 86400) * 86400;
      MaxTransferPerDay storage _maxTxrUser = _maxTxnPerDay[msg.sender];
      if (_maxTxrUser.midnight == _midnightToday) {
        require(
          _maxTxrUser.amount + amount <= maxTransferOrSellPerDay,
          'User can only transfer/sell up to maximum transfer amount'
        );
        _maxTxrUser.amount += amount;
      } else {
        require(
          amount <= maxTransferOrSellPerDay,
          'User can only transfer/sell up to maximum transfer amount'
        );
        _maxTxrUser.midnight = _midnightToday;
        _maxTxrUser.amount = amount;
      }
    }
  }
