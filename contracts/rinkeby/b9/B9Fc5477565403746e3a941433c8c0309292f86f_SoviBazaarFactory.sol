...
contract SoviBazaarFactory is Manageable, IERC721Receiver {
    ...
    event SBUUpgrade(address indexed user, uint256 tokenId1, uint256 tokenId2, uint256 newCardId, uint256 newCardType);
    ...
    function upgrade(uint256 tokenId1, uint256 tokenId2) external {
        ...
        emit SBUUpgrade(msg.sender, tokenId1, tokenId2, newCardId, newCardType);
    }
    ...
    function setBlackList(uint256[] calldata tokenIds) external onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i ++) {
            _blackList[tokenIds[i]] = true;
        }
    }

    function removeBlackList(uint256[] calldata tokenIds) external onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i ++) {
            delete _blackList[tokenIds[i]];
        }
    }
