...
contract Pokemon is ERC721, VRFConsumerBase, Ownable {
    ...
    function setIvs(uint256[] memory RNGNumbers, string memory pokemonName, uint256 tokenId) internal {
        ...
        PokemonBaseStats memory ivs = PokemonBaseStats({hp: hpIv, def: defIv, atk: atkIv, spa: spaIv, spd: spdIv, spe: speIv, type1: type1, type2: type2, number: baseStats.number, name: pokemonName});
        PokemonBaseStats memory evs = PokemonBaseStats({hp: 0, def: 0, atk: 0, spa: 0, spd: 0, spe: 0, type1: type1, type2: type2, number: baseStats.number, name: pokemonName});
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        ...
        uint256 pokemonNameIndex = (RNGNumbers[7] % listOfPokemonNames.length) + 1;
        string memory name = listOfPokemonNames[pokemonNameIndex];
        (string memory type1, string memory type2) = getTypeFromName(name);
        ...
        setIvs(RNGNumbers, name, newUniquePokemonID);
        setBattleStats(name, newUniquePokemonID);
        emit PokemonCreated(tokenCounter);
        tokenCounter = tokenCounter + 1;
    }

    function getTypeFromName(string memory name) public returns (string memory, string memory ){
        PokemonBaseStats memory baseStats = pokemonNameToPokemonBaseStats[name];
    ...
    function createBaseStatPokemon(string memory name, uint256 hp, uint256 def, uint256 atk, uint256 spa, uint256 spd, uint256 spe, string memory type1, string memory type2, uint256 number) public onlyOwner {
        PokemonBaseStats storage baseStats = pokemonNameToPokemonBaseStats[name];
        baseStats.name = name;
