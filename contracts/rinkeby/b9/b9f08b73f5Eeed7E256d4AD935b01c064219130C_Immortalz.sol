...
contract Immortalz is ERC721Enumerable, Ownable, ImmortalzInterface, ImmortalzMetadataInterface {
    ...
    function addToPresaleList(address[] calldata addresses) external override onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            /**
            * @dev We don't want to reset _presaleListClaimed count
    ...
    function removeFromPresaleList(address[] calldata addresses) external override onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            /// @dev We don't want to reset possible _presaleListClaimed numbers.
            _presaleList[addresses[i]] = false;
        }
    }

    /**
    * @dev We want to be able to distinguish tokens bought during isPresaleListActive
    * and tokens bought outside of isPresaleListActive
    */
    ...
    function purchase(uint256 numberOfTokens) external override payable {
        ...
        */
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            /**
            * @dev Since they can get here while exceeding the IMMORTALZ_MAX_MINT,
            * we have to make sure to not mint any additional tokens.
            */
            if (totalPublicSupply < IMMORTALZ_MAX) {
                /**
                * @dev Public token numbering starts after IMMORTALZ_RESERVE.
