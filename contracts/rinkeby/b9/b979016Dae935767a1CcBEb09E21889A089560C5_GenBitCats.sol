...
contract GenBitCats is ERC721, Ownable, PaymentSplitter { 
  ...
  address[] private _team = [
        0x3238bf6aE11612B63107de87830B7700729F9aDD,
        0x5D52F6632B1a9D743A5290dc26a3f831f09Db6A0 // TODO change to final PS address
        ];
  ...
  uint public constant maxPresaleSupply = 256;
  uint public constant maxMintsPerTransaction = 5;
  uint public constant maxMintsPerWhitelistedAddress = 2;
  uint public constant reservedAmount = 8; 
  ...
  mapping(address => bool) public ogBitCatWhitelist;
  string public BaseURI = "https://onchain.mypinata.cloud/ipfs//" ;
  string public baseExtension  = "";
  string public hiddenTokenURI = "https://onchain.mypinata.cloud/ipfs/";
  ...
  function addToWhitelistBool(address[] memory addresses) external onlyOwner {
    for(uint256 i = 0; i < addresses.length; i++) {
  ...
  function mintPresale(uint amount) external payable onlyEOA {
    ...
    require(totalSupply() + amount <= maxPresaleSupply, "max supply reached");
  ...
  function mintOgBitCats() external  {
    require(totalSupply() + 1 <= maxSupply, "max reached");
    require(ogBitCatWhitelist[msg.sender], "only 1 per OG BitCat holder");
    ogBitCatWhitelist[msg.sender] = false;
    _mintGBC(msg.sender, 1);
  }
