...
contract ManifestGenesisERC1155 is ERC1155, Ownable {
    ...
    uint256 public TOTAL_REMAINING_1 = 222;
    uint256 public TOTAL_REMAINING_2 = 222;
    uint256 public TOTAL_REMAINING_3 = 222;
    ...
    function toggleSaleIsActive() external onlyOwner {
        if (!saleIsActive) {
            require(
                IERC20Contract( sMNFST ).balanceOf(address(this)) >= 33 * (TOTAL_REMAINING_1 + TOTAL_REMAINING_2 + TOTAL_REMAINING_3) * 10 ** 9, 
                "NOT_ENOUGH_SMNFST"
    ...
    function addWhitelistToken(address _address, uint256 _tokenId, bool _isERC1155, bool _isEnabled) public onlyOwner {
        ...
        for (uint256 i = 0; i < tokenWhitelistInfo.length; i++) {
            if (tokenWhitelistInfo[i].tokenAddress == _address) {
                // update info
                tokenWhitelistInfo[i].tokenId = _tokenId;
                tokenWhitelistInfo[i].isERC1155 = _isERC1155;
    ...
    function mint(uint256 _numHoodies) external payable {
        ...
        require(_numHoodies <= TOTAL_REMAINING_1 + TOTAL_REMAINING_2 + TOTAL_REMAINING_3, "AMOUNT_EXCEEDS_REMAINING");
        ...
        for (uint256 i = 0; i < _numHoodies; i++) {
            ...
            if (hoodieNum == TOKEN_HOODIE_1) {
                if (TOTAL_REMAINING_1 > 0) {
                    totals[0]++;
                    TOTAL_REMAINING_1--;
                    continue;
                }
            } else if (hoodieNum == TOKEN_HOODIE_2) {
                if (TOTAL_REMAINING_2 > 0) {
                    totals[1]++;
                    TOTAL_REMAINING_2--;
                    continue;
                }
            } else if (hoodieNum == TOKEN_HOODIE_3) {
                if (TOTAL_REMAINING_3 > 0) {
                    totals[2]++;
                    TOTAL_REMAINING_3--;
                    continue;
                }
    ...
    function checkSaleEligiblity(address _address) public view returns (bool) {
        ...
        for ( uint i = 0; i < tokenWhitelistInfo.length; i++ ) {
            ...
            if (tokenWhitelistInfo[i].isERC1155 == false) {
                if (IERC721Contract( tokenWhitelistInfo[i].tokenAddress ).balanceOf(_address) > 0) {
                    return true;
                }
            } else {
                if (IERC1155Contract( tokenWhitelistInfo[i].tokenAddress ).balanceOf(_address, tokenWhitelistInfo[i].tokenId) > 0) {
                    return true;
                }
            }
        }
