...
import "../market/MarketLibrary.sol";
    ...
    Proxiable
{
    ...
    /// @dev Array of ids of open markets
    ...
    /// removing markets)
    EnumerableSet.UintSet private openMarkets;
    ...
    /// @notice Emitted when an expired market has been removed*/
    event MarketEvicted(uint64 marketId);
    ...
    // E11: Can only be called by MarketController
    ...
    function initialize(
        ...
        erc1155Controller = IERC1155(_marketController.erc1155Controller());

        // Approve marketController to move tokens
        erc1155Controller.setApprovalForAll(address(marketController), true);
    ...
    /// @notice Claims any remaining collateral from all expired markets whose wToken is held by the AMM, and removes
    /// the expired market from the AMM's collection of markets
    function claimAllExpiredTokens() public {
        for (uint256 i = 0; i < openMarkets.length(); i++) {
            uint64 marketId = uint64(openMarkets.at(i));
            while (
                marketController.state(marketId) ==
                IMarketController.MarketState.EXPIRED
            ) {
                claimExpiredTokens(marketId);

                // Handle edge case: If, prior to removing the Market, i was the index of the last Market
                // in openMarkets, then after the removal `i` will point to one beyond the end of the array.
                // This means we've iterated through all of the Markets in `openMarkets`, and we should break
                // out of the while loop. At this point i == openMarkets.length(), so the outer for loop
                // will end as well
                if (i == openMarkets.length()) {
                    break;
                } else {
                    marketId = uint64(openMarkets.at(i));
                }
            }
        }
    }

    /// @notice Claims any remaining collateral from expired market whose wToken is held by the AMM, and removes
    /// the expired Market from the AMM's collection of markets
    function claimExpiredTokens(uint64 marketId) public {
        // claim the expired market's wTokens, which means it can now be safely removed
        uint256 bTokenIndex = MarketLibrary.bTokenIndex(marketId);
        uint256 wTokenIndex = MarketLibrary.wTokenIndex(marketId);

        uint256 bTokenBalance =
            erc1155Controller.balanceOf(address(this), bTokenIndex);
        if (bTokenBalance > 0) {
            marketController.exerciseOption(marketId, bTokenBalance, false);
        }

        uint256 wTokenBalance =
            erc1155Controller.balanceOf(address(this), wTokenIndex);
        if (wTokenBalance > 0) {
            marketController.claimCollateral(marketId, wTokenBalance);
        }
        ...
        openMarkets.remove(marketId);

        emit MarketEvicted(marketId);
    }
    ...
    function _sellOrWithdrawActiveTokens(
        ...
        for (uint256 i = 0; i < openMarkets.length(); i++) {
            ...
            if (
                marketController.state(marketId) ==
                IMarketController.MarketState.OPEN
            ) {
                uint256 bTokenIndex = MarketLibrary.bTokenIndex(marketId);
                uint256 wTokenIndex = MarketLibrary.wTokenIndex(marketId);
        ...
        returns (uint256)
    {
        ...
        // in getCurrentUnderlyingPrice is a valid market price in units of
        ...
        for (uint256 i = 0; i < openMarkets.length(); i++) {
            uint64 marketId = uint64(openMarkets.at(i));
            IMarketController.Market memory market =
                marketController.markets(marketId);
            ...
            if (
                ...
                uint256 bPrice =
                    getPriceForMarketInternal(market, underlyingPrice);
                // wPrice = 1 - bPrice
                uint256 wPrice = uint256(1e18) - bPrice;

                uint256 tokensValueCollateral =
                    marketController.getCollateralPerOptionToken(
                        marketId,
    ...
    /// @notice List the market ids this AMM trades
    /// @return
    function getMarkets() external view returns (uint64[] memory) {
        uint64[] memory markets = new uint64[](openMarkets.length());
        for (uint256 i = 0; i < openMarkets.length(); i++) {
            markets[i] = uint64(openMarkets.at(i));
        }
        return markets;
    }

    /// @notice Get the IMarket by address
    /// @param marketId the ID of the Market
    /// @return an IMarket, if it exists
    function getMarket(uint64 marketId)
        external
        view
        returns (IMarketController.Market memory)
    {
        require(openMarkets.contains(marketId), "E13");
        return marketController.markets(marketId);
    }
     ...
     */
    function getVirtualReserves(uint64 marketId)
        public
        view
    ...
    function getVirtualReservesInternal(
        ...
        if (market.isPutOption) {
            bTokenBalance = marketController.getCollateralPerOptionToken(
                marketId,
                bTokenBalance
            );
            wTokenBalance = marketController.getCollateralPerOptionToken(
                marketId,
                wTokenBalance
            );
        }
        ...
        uint256 bTokenPrice =
            getPriceForMarketInternal(market, getCurrentUnderlyingPrice());
    ...
    /**
     * @notice Get price of bToken for a given market
     */
    function getPriceForMarket(uint64 marketId)
        external
        view
        returns (uint256)
    {
        ...
        return
            getPriceForMarketInternal(
                marketController.markets(marketId),
                getCurrentUnderlyingPrice()
            );
    }

    function getPriceForMarketInternal(
        IMarketController.Market memory market,
        ...
        return
            ...
            calcPrice(
                market.expirationDate - block.timestamp,
                market.strikePrice,
                underlyingPrice,
                volatilityFactor,
                market.isPutOption
    ...
    function calcPrice(
        ...
        if (isPutOption) {
            ...
            // volatility, strike price, and the current market price
            timeValue =
    ...
    function bTokenBuy(
        ...
        uint256 collateralAmount =
            bTokenGetCollateralIn(marketId, bTokenAmount);
    ...
    /// @notice Sell the bToken of a given market to the AMM in exchange for collateral token
    /// @notice This call will fail if the caller tries to sell a bToken amount larger than the amount of
    /// wToken held by the AMM
    /// @param marketId The ID of the Market to buy bToken on
    ...
    function bTokenSell(
        ...
        uint256 collateralAmount =
            bTokenGetCollateralOut(marketId, bTokenAmount);
        ...
        marketController.closePosition(marketId, closeAmount);

        // Send the tokens to the seller
        collateralToken.transfer(msg.sender, collateralAmount);

        // Emit the event
        emit BTokensSold(msg.sender, marketId, bTokenAmount, collateralAmount);
     ...
     */
    function bTokenGetCollateralIn(uint64 marketId, uint256 bTokenAmount)
        public
        view
        returns (uint256)
    {
        ...
        bTokenAmount = marketController.getCollateralPerOptionToken(
            marketId,
            bTokenAmount
        );
     ...
     */
    function bTokenGetCollateralOut(uint64 marketId, uint256 bTokenAmount)
        public
        view
        returns (uint256)
    {
    ...
    /// @notice Sell the wToken of a given market to the AMM in exchange for collateral token
    /// @param marketId The ID of the Market to buy wToken on
    ...
    function wTokenSell(
        ...
        // Get initial stats
        uint256 collateralAmount =
            wTokenGetCollateralOut(marketId, wTokenAmount);
        require(collateralAmount >= collateralMinimum, "Slippage exceeded");
        ...
        // Emit the event
        emit WTokensSold(msg.sender, marketId, wTokenAmount, collateralAmount);
    ...
    function wTokenGetCollateralOut(uint64 marketId, uint256 wTokenAmount)
        public
        view
        returns (uint256)
    {
    ...
    function optionTokenGetCollateralOutInternal(
        ...
        optionTokenAmount = marketController.getCollateralPerOptionToken(
            marketId,
            optionTokenAmount
        );

        (uint256 bTokenBalance, uint256 wTokenBalance) =
            getVirtualReservesInternal(marketId, _collateralTokenBalance);
    ...
    /**
     * Calculate the amount of collateral the AMM would received if all of the
     * unredeemed collateral tokens locked in expired wTokens and bTokens were to
     * be redeemed
     */
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < openMarkets.length(); i++) {
            ...
            if (
                ...
                // redeeming this Market's bTokens and wTokens
            ...
            if (
                ...
                uint256 bTokenToSell =
                    (erc1155Controller.balanceOf(
                        address(this),
                        MarketLibrary.bTokenIndex(marketId)
                    ) * lpTokenAmount) / lpTokenSupply;
                uint256 wTokenToSell =
                    (erc1155Controller.balanceOf(
                        address(this),
                        MarketLibrary.wTokenIndex(marketId)
                    ) * lpTokenAmount) / lpTokenSupply;
    ...
    /// Market's bTokens and wToken
    /// @param marketId The index of the Market
    /// @param wTokenBalance The wToken balance for this Market owned by this AMM
    /// @param bTokenBalance The bToken balance for this Market owned by this AMM
    /// @return The total amount of collateral receivable by redeeming the Market's option tokens
    function getFairMarketValueOfOptionTokens(
        ...
        if (wTokenBalance > 0) {
            (uint256 unclaimedCollateral, ) =
                marketController.getClaimAmount(marketId, wTokenBalance);
            unredeemedCollateral += unclaimedCollateral;
        }
        if (bTokenBalance > 0) {
            (uint256 unexercisedCollateral, ) =
                marketController.getExerciseAmount(marketId, bTokenBalance);
    ...
    /// @notice Adds the address of market to the amm
    /// @dev Only the associated MarketController may call this function
    /// @dev The MarketController calls this function when it is creating a Market
    /// and adds the Market to this AMM
    function addMarket(uint64 _marketId) external override {
        require(msg.sender == address(marketController), "E11");
        openMarkets.add(_marketId);
    }

    /// @notice Returns true when interfaceId is the ID of the addMarket function or the ERC165
    /// standard, and false otherwise
    /// @dev This function exists only so the MarketController can tell when to try to add
    /// Markets it has created to the MinterAmm
    ...
    {
        return
            interfaceId == this.addMarket.selector ||
...
library MarketLibrary {
    function wTokenIndex(uint64 _marketIndex) internal pure returns (uint256) {
        return _marketIndex * 2;
    }

    function bTokenIndex(uint64 _marketIndex) internal pure returns (uint256) {
        return (_marketIndex * 2) + 1;
    }
}
