 ...
 */
contract DadMfersV2 is ERC721, Ownable {
    ...
    function devMintForAirdropToV1(uint256 [] memory specialId) external onlyOwner {       
        for (uint256 i = 0; i < specialId.length; i++) {
            ...
            require (specialId[i] > 5, "can't mint available to public");
            require (specialId[i] < 10, "must mint under 5560");
    ...
    function mint(uint numberOfTokens) external payable {
        ...
        require(_tokenSupply.current() + numberOfTokens < 5, "Purchase would exceed max supply of tokens");
