...
// File: contracts\LuchowAuction.sol

// Luchow Auction Contract 
...
contract LuchowAuction is Ownable, ERC721Holder {
    ...
    {   
        ...
        ILuchowNFT nft = ILuchowNFT(_collectionId); 
    ...
    function finalizeAuction(uint256 _auctionId) public {
        ...
        if(bidsLength == 0) {
            ...
            address _creator = ILuchowNFT(myAuction.collectionId).creatorOf(myAuction.tokenId);
            uint256 royalty = ILuchowNFT(myAuction.collectionId).royalties(myAuction.tokenId);
            ...
            ILuchowNFT(myAuction.collectionId).safeTransferFrom(address(this), lastBid.from, myAuction.tokenId);		
            auctions[_auctionId].active = false;
    ...
    function bidOnAuction(uint256 _auctionId, uint256 amount) external AuctionExists(_auctionId) {
        ...
        require(governanceToken.transferFrom(msg.sender, address(this), amount), "transfe to contract failed");
