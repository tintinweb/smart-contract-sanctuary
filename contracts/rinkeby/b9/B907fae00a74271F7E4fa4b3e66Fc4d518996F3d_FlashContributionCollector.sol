 ...
 */
contract FlashContributionCollector is Initializable, OwnableUpgradeable {
    ...
    event Initialized(address token, uint256[] tierCaps, bytes32 merkleRoot);
    ...
    function initialize(
        ...
        emit Initialized(_token, _tierCaps, _merkleRoot);
    }
    ...
    function updateTierCaps(uint256[] calldata _newTierCaps)
        external
        onlyOwner
    {
        if (state != State.ACTIVE) revert InactiveContribution();
