...
import "./ProductTokenBase.sol";

/**
 * @title Product Token
 *
 * @notice This is of the product token implementation.
 *
 * @dev See ProductTokenBase for more details
 *
 */
contract ProductHighToken is ProductTokenBase, PausableUpgradeable {

  /// @dev Link to HIGH STREET ERC20 Token instance
  address public constant HIGH = 0x71Ab77b7dbB4fa7e017BC15090b2163221420282;
  ...
  *
  */
  function buy(uint256 maxPrice_) external virtual whenNotPaused {
    require(maxPrice_ > 0, "invalid max price");
    // if(endTime != 0) {
    //   require(block.timestamp <  endTime, "sale is unavialbe expire");
    // }

    transferFromHighToken(_msgSender(), address(this), maxPrice_);

    (bool success, uint256 change)  = _buy(maxPrice_);
    if (success == true) {
      if(change > 0) {
        transferHighToken(_msgSender(), change);
      }
    }else { // If token transaction failed
      transferHighToken(_msgSender(), maxPrice_);
    }
  }
  ...
  */
  function sell(uint32 amount_) external virtual whenNotPaused {
    require(balanceOf(_msgSender()) >= amount_ || amount_ > 0, 'invalid amount');
    // if(endTime != 0) {
    //   require(block.timestamp <  endTime, "sale is unavialbe expire");
    // }
  ...
  */
  function tradein(uint32 amount_) external virtual whenNotPaused {
    require(amount_ > 0, "Amount must be non-zero.");
    require(balanceOf(_msgSender()) >= amount_, "Insufficient tokens to burn.");
    _tradein(amount_);
  }
  ...
  *
  */
  function transferHighToken(address to_, uint256 value_) internal {
  ...
  *
  */
  function transferFromHighToken(address from_, address to_, uint256 value_) internal {
    // just delegate call to the target
...
/**
  * @title ProductTokenBase
  *
  * @notice This is product token base.
  *
  ...
  */
contract ProductTokenBase is ERC20Upgradeable, OwnableUpgradeable {
	using SafeMathUpgradeable for uint256;

  /// @dev Data structure representing supplier information
  struct Supplier {
    // @dev amount of fee owned by supplier;
    // increase when buy(), sell(), tradin()
    // decrease when supplier claim
    uint256 amount;
    // @dev wallet supplier address
    address wallet;
  }
  ...
  uint256 public constant FEE_MULTIPLIER = 1e10;

  /// @dev during buy, 4% of price will be charge as fee
  uint256 public constant FEE_RATE_IN_BUY = 4e10;

  /// @dev during sell, 2% of price will be charge as fee
  uint256 public constant FEE_RATE_IN_SELL = 2e10;
  ...
  /**
    ...
    * @param price amount of token prices when tokens redeemed
    * @param fee amount of price in charged with according to platform fee
    */
  event Tradein(address indexed sender, uint32 amount, uint256 price, uint256 fee);

  /**
    * @dev Fired in updateSupplier()
    *
    * @param supplier an address...
    */
  ...
  /**
    ...
    */
  event ClaimSupplier(address indexed sender, uint256 amount);
  ...
  */
   ...
   *
   * @return success indicate process is successfully or not.
   * @return change amount of change in reserve tokens.
  */
  function _buy(uint256 _deposit) internal virtual returns (bool success, uint256 change)
  {
    ...
    emit Buy(msg.sender, price, fee);
    return (true, _deposit - price - fee);
  }
  ...
  */
  function _sell(uint32 _amount) internal virtual returns (uint256 price)
  {
    ...
    emit Sell(msg.sender, _amount, reimburseAmount, fee);
  ...
  */
  function _tradein(uint32 amount_) internal virtual {
    require(amount_ > 0, "Amount must be non-zero.");
    require(balanceOf(msg.sender) >= amount_, "Insufficient tokens to tradein.");
    require(nft != ProductNft(address(0)), "invalid nft");

    _burn(msg.sender, amount_);

    (, uint fee) = _sellReturn(amount_);
    ...
    nft.mintBatch(msg.sender, tradeinCount, amount_);

    tradeinCount = tradeinCount + amount_;
    tradeinReserveBalance = tradeinReserveBalance + tradinReturn;

    emit Tradein(msg.sender, amount_, tradinReturn, fee);
  }

  /**
   * @dev Function ... base on tradein balance to calculate the value.
   *
  ...
  /**
   * @dev Function ... increase number of fee for supplier
   *
  ...
  /**
   * @dev Function ... increase number of fee for platform
   *
   * @param fee_ value to be accumulate
   */
   ...
   */
  function updateSupplier( address wallet_) external virtual {
   ...
   * @return amount amount ...
   */
  function getSupplierAmount() external view virtual returns(uint256 amount){
    require(_msgSender() == owner() || _msgSender() == supplier.wallet, "not allowed");
  ...
  function claimSupplier(uint256 amount_) external virtual {
    ...
    emit ClaimSupplier(_msgSender(), amount_);
  }
   ...
   * @return amount amount ...
   */
  function getPlateformFeeAmount() external view virtual returns(uint256 amount){
...
contract ProductNft is Context, ERC721Enumerable, Ownable {
  ...
  function mint(address to_, uint256 tokenId_) external {
    ...
    require(tokenId_ >= 0 && tokenId_ < maxCount, "cap exceeded");
  ...
  function mintBatch(address to_, uint256 start_, uint256 count_) external {
    require(minters[_msgSender()] == true, 'permission denied');
    require(to_ != address(0), 'invalid address');
    require(start_ >= 0 && (start_ + count_ < maxCount), "cap exceeded");
