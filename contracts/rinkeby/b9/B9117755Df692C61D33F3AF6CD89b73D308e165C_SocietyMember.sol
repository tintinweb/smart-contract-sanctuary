...
//
//  If 2,000+ memberships are sold by Jan 15 there are no refunds.
//  But if less than 2,000 are sold by Jan 15, then the refund
//  implementation operates within three phases:
//
//   Phase 1: Nov 15 - Jan 15
...
contract SocietyMember is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    // There can be only 5,000 members.
    // NOTE: permanently fixed upon contract creation
    uint256 public immutable MAXIMUM_MEMBER_COUNT;
    ...
    mapping(address => uint256) private mintCountsByAddress;
    ...
    constructor(
        ...
        require(_enoughMembersToProceed <= _maximumMemberCount);
        require(_salesDeadline <= _refundDeadline);

        MAXIMUM_MEMBER_COUNT = _maximumMemberCount;
        ...
        for (uint256 i = 0; i < foundingTeam.length; i++) {
            _safeMint(foundingTeam[i], generateTokenId());
        }
    }
        ...
        nonReentrant
    {
        ...
        require(
            ids.current() + numberOfTokens <= MAXIMUM_MEMBER_COUNT,
        ...
        uint256 mintCount = mintCountsByAddress[msg.sender];
        require(
    ...
    function refund(uint256 tokenId) public nonReentrant {
        require(
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        return (
            address(this),
            SafeMath.div(SafeMath.mul(salePrice, royaltyPerMille), 1000)
        );
    }
    ...
    function isProbablyHuman(bytes memory captcha, address sender)
        ...
        returns (bool)
    {
        require(captcha.length == 65, "bad captcha, are you a robot?");

        // First we recreate the same message that was originally signed.
        // This is equivalent to how we created it elsewhere:
        //    message = web3.utils.soliditySha3("member", {t: 'address', v: sender});
