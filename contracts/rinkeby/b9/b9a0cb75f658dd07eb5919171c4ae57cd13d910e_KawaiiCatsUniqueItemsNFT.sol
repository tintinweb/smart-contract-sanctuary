...
contract KawaiiCatsUniqueItemsNFT is Ownable, ERC721, ERC721Enumerable {
     ...
     *
     */
    function mintItem(string memory _name, string memory _description, string memory _setName) public onlyOwner {
        
        uint16 mintIndex = uint16(totalSupply()); 
        
        _safeMint(msg.sender, mintIndex);
        uniqueItemArray.push(uniqueItem(mintIndex,_name,_description,_setName));
     ...
     */
    function requestCustomOutfit (string memory _picURL, uint16 _assignedNFTId, string memory _description) public payable {
        ...
        requestsArray.push(CustomRequest(_picURL, uint32(block.timestamp), _description, msg.sender, _assignedNFTId, getCustomPrice(), false)); 
        queue ++;
    } 
     ...
     */
    function requestCustomOutfitPURR (string memory _picURL, uint16 _assignedNFTId, string memory _description) public {
        require(nftInterface.balanceOf(msg.sender) > 0, "Must own at least a kawaii cat");
        require(requestCustomPurr == true, "Function inactive"); 
        require(token.getBalanceOf(msg.sender) > CUSTOM_PURR_PRICE, "Not enough PURR");
        
        requestsArray.push(CustomRequest(_picURL, uint32(block.timestamp), _description, msg.sender, _assignedNFTId, 0, false)); 
     ...
     *
     */
    function mintAndAssignItem(string memory _name, string memory _description, string memory _setName, uint requestsArrayId) public onlyOwner {
