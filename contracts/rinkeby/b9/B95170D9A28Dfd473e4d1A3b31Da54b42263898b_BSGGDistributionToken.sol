...
contract BSGGDistributionToken is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    uint internal amountPenalty = 50000; //5%
    uint internal rewardPenalty = 500000; //50%

    uint internal GAIN_MULTIPLIER_BASE = 1e6;
    uint internal AMOUNT_PENALTY_BASE = 1e6;
    uint internal REWARD_PENALTY_BASE = 1e6;
    ...
    function addTicketType(
        ...
        require(_gainMultiplier > GAIN_MULTIPLIER_BASE, "Gain multiplier lower or equal to base");
    ...
    function buyTicket(
        ...
        tickets[ticketId].amountToGain       = (_amount * ticketTypes[_ticketTypeId].gainMultiplier / GAIN_MULTIPLIER_BASE) - _amount;
    ...
    function getPendingTokens(uint _ticketId) public view returns (uint stakeAmount, uint rewardAmount) {
        ...
        if (block.timestamp < currentTicket.lockedToTimestamp){
            stakeAmount = currentTicket.amountLocked * (AMOUNT_PENALTY_BASE - amountPenalty) / AMOUNT_PENALTY_BASE;

            if (block.timestamp >= halfPeriodTimestamp){
                rewardAmount = currentTicket.amountToGain * (REWARD_PENALTY_BASE - rewardPenalty) / REWARD_PENALTY_BASE;
            }
        }else{
    ...
    function withdrawNonReservedBSGG(uint _amount, uint _ticketTypeId) public onlyOwner{
        BSGG.transfer(msg.sender, BSGGAllocation[_ticketTypeId] >= _amount ? _amount : BSGGAllocation[_ticketTypeId]);
        BSGGAllocation[_ticketTypeId] = 0;
    }
    
    function _beforeTokenTransfer(address from, address to, uint256 ticketId) internal whenNotPaused override(ERC721, ERC721Enumerable){
