...
contract MultipleDropsPublicSale is Ownable, IERC165 {
    ...
    function deposit() public payable {
        ...
        require(
            (_buyers[msg.sender] + amount > 3 && buyerLimited == true) || (!buyerLimited),
            "Only 3 purchases per wallet allowed"
        );
        require(
            _availableTokens.length >= amount,
            "Currently no NFT available. Please try again later"
        );

        for (uint p = 0; p < amount; p++) {
            uint256 randomIndex = randomAvailableTokenIndex();
            transferNft(_availableTokens[randomIndex], msg.sender);
