...
contract MiddleWare{
     ...
     */
    function getOracleAndMarketPrice(address uniswapPriceOracleAddr, string[] memory underlyingNameArray) public returns(uint[] memory, uint[] memory){
        require(uniswapPriceOracleAddr != address(0), "address cannot be 0x0");
        PriceOracle priceOracle = PriceOracle(uniswapPriceOracleAddr);
        uint[] memory marketPrice = new uint[](underlyingNameArray.length);
        uint[] memory oraclePrice = new uint[](underlyingNameArray.length);
