...
contract Token2476 is ERC1155Supply, ERC1155Pausable, Ownable, PaymentSplitter {
    uint16 public MAX_SUPPLY = 10000;
    uint16 public TOKEN_ID = 2476;
    uint16 public RESERVED_TOKENS = 10;
    uint256 private _mintedTokens = 0;
    uint16 private _maxPurchaseCount = 20;
    ...
    ) ERC1155(uri) PaymentSplitter(payees, paymentShares) {
        _saleStart = saleStart_;
        _mint(payees[0], RESERVED_TOKENS);
    }
    ...
    function setSaleEnd(uint256 saleEnd_) public onlyOwner {
        _saleEnd = saleEnd_;
    }
    ...
    function saleActive() public view returns (bool) {
        ...
        return _saleEnd > block.timestamp;
    }
    ...
    function setBaseMintPrice(uint256 price) public onlyOwner {
        emit PriceChange(_mintPrice, price);
        _mintPrice = price;
    }
    ...
    function setAllowedBurner(address addr, bool isAllowed) public onlyOwner {
        _allowedBurners[addr] = isAllowed;
    }

    modifier mintCountMeetsSupply(uint256 numberOfTokens) {
        require(
            _mintedTokens + numberOfTokens <= MAX_SUPPLY,
    ...
    function burn(address account, uint256 numberOfTokens) public {
        require(
            _allowedBurners[msg.sender],
            "Burnable: Only from allowed callers"
        );

        _burn(account, TOKEN_ID, numberOfTokens);
    }

    function _mint(address account, uint256 amount)
        internal
        mintCountMeetsSupply(amount)
    {
