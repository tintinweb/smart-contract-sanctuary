...
contract Signed is ERC721EnumerableLite, Ownable, PaymentSplitter {
  ...
  function verify( uint256 tokenPrice, string memory _tokenURI, bytes calldata signature ) public view returns ( bool ) {
    address signerCheck = getSigner( msg.sender, tokenPrice.toString(), _tokenURI, signature );
  ...
  function getSigner( address toAccount, string memory tokenPrice, string memory _tokenURI, bytes memory signature ) private view returns ( address ) {
    bytes32 hash = createHash( toAccount, tokenPrice, _tokenURI );
    return hash.toEthSignedMessageHash().recover( signature );
  }

  function createHash( address toAccount, string memory tokenPrice, string memory _tokenURI ) private view returns ( bytes32 ) {
    return keccak256( abi.encodePacked( address(this), toAccount, tokenPrice, _tokenURI, _secret ) );
  }
  ...
  function mint( uint256 tokenPrice, string memory _tokenURI, bytes calldata signature ) external payable {
      require( !_paused, "Sale is currently paused." );
      require( verify(tokenPrice, _tokenURI, signature), "Token verification failed." );
      require( msg.value >= tokenPrice, "Eth sent is incorrect." );
