...
contract DAOCC is Ownable, ReentrancyGuard, SignatureVerifyer {
    ...
    uint256[] public levelBase = [0.05 ether, 0.06 ether, 0.1 ether];
    ...
    function payForLevel(
      ...
      // If first level in round -> take 10% fee to owner
      // Hold 90% fee on contract for direct ref bonuses
      if(level == 0){ 
    ...
    function levelUp(
      ...
      users[userID].level++;
      emit LevelUp(users[userID].wallet, users[userID].level);
