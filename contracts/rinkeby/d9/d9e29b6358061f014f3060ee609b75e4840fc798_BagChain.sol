...
contract BagChain is Initializable, OwnableUpgradeable {
  ...
  bytes32 private constant WITHDRAW_TYPEHASH=keccak256(abi.encodePacked(
    "Withdraw(uint256 withdrawId,address tokenAddress,uint256 amount,uint256 deadline)"
  ));
  bytes32 private SERVER_DOMAIN_SEPARATOR;

  event Deposit(address indexed tokenAddress, uint256 amount);
  event Withdraw(uint256 indexed withdrawId, address indexed tokenAddress, uint256 amount);
  ...
  function withdraw(
    ...
    bytes32 values = keccak256(abi.encode(
      WITHDRAW_TYPEHASH,
      _withdrawId, _tokenAddress, _amount, _deadline
    ));
    _verify(values, _v, _r, _s);

    require(withdrawn[_withdrawId] == 0, "withdraw: already withdrawn");
    require(_amount > 0, "withdraw: amount is zero");
    require(block.timestamp < _deadline, "withdraw: deadline passed");

    uint256 balance = IERC20(_tokenAddress).balanceOf(address(this));
    require(_amount <= balance, "withdraw: invalid amount");
    IERC20(_tokenAddress).safeTransferFrom(address(this), msg.sender, _amount);
    withdrawn[_withdrawId] = block.number;
    emit Withdraw(_withdrawId, _tokenAddress, _amount);
  }
