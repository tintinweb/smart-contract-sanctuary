...
contract Catnip is Context, IBEP20, Initializable {
    ...
    bool public isAirDropInitializationLocked;
    ...
    function initialize() public initializer {
        ...
        teamWalletAddr = 0x8809687eD3E675eb7E3953099555A07CbDBDD9C5;    // CHANGEIT - you will want to update this with the real multisig gnosis wallet safe
        timeLockAddress = 0xF196BCaa6F0A15565Be66263274A1A4b44C6d7ec;    // CHANGEIT - need the real TimeLock address here
        ...
        tokenSymbol = "NIP";        //CHANGEIT - Change the Name to the Production one NIP
        tokenName = "Catnip";       //CHANGEIT - Change the Name to the Production one Catnip
        MAXintNum = ~uint256(0);
        ...
        numTokensSellToAddToLiquidity = 1 * 10**6 * 10**9;         // Number of tokens to reach in the Contract before Swap and Liquification.
        ...
        routerAddressForDEX = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;       // CHANGEIT - change this to pancakeswap router
        ...
        isAirDropInitializationLocked = false;
    ...
    function deliverReflectTokens(uint256 tAmount) public {   
        address sender = _msgSender();              // TODO - Need a better understanding of this.
        ...
        totalFeeAmount = totalFeeAmount.add(tAmount);       // if called, the delivery takes the tokens from the caller. what?
    }
    ...
    function initializeAirDropAddressesAndAmounts(address[] memory addressesToAirDrop, uint256[] memory amountsToAirDrop) external { 
        require(_msgSender() == 0x0C2a98ace816259c0bB369f88Dd4bcb9135E0787, "The initializer must be Yoshiko's deployer address");
        require(!isAirDropInitializationLocked, "Initialization must be Unlocked");
    ...
    function enableOrDisableInitializationLock(bool enable) external onlyOwner { 
        isAirDropInitializationLocked = enable;
    }


    function disableInitializationLock() external { 
        require(_msgSender() == 0x0C2a98ace816259c0bB369f88Dd4bcb9135E0787, "The disabler must be Yoshiko's deployer address");
        require(!isAirdropEnabled, "AirDrop must be disabled");  
        require(!isAirDropInitializationLocked, "Initialization must be Unlocked");
        isAirDropInitializationLocked = true;
    }
    ...
    function initializeWhiteListAddressesIgnoreMax() internal {
        ...
        isWhiteListedAddressIgnoreMax[pancakeswapPair] = true;   // Pair Address that is Generated at the Start
        ...
        // XXX - Perhaps in the future, include BUSD, WETH, WBNB, etc. in the exclusions, not sure if that needs to be done or not.
    ...
    function initializeTeamMemberWalletAddresses() internal {
        // CHANGEIT - Get Teammate Addresses and Add them here.
...
Catnip, NIP, version 1.00, SorceriaN Release
...
There were men that set out on precarious adventures in order to test their strength and courage.

People called these men . . .

SorceriaN

https://cdn.discordapp.com/attachments/763134325151236150/834500528213852200/370b2dadc45eafe7.mp4
...
SorceriN version 1.00 Developer Notes:

NIP was forked from Safemoon, which is itself a combination of other tokens using a variety of protocols.
However, unlike most copycat tokens, this version is significantly altered.

The most important change is that it's been changed to allow for use with a Proxy. 
This Proxy is controlled by a Gnosis Multisig Safe, which goes through a TimeLock of 48 hours. 
What that means is that although we can change the code to rugpull, buyers have a 48 hour window to dump the tokens before the devs can dump.
This TimeLock cannot be modified or have the delay function changed. 
I have commented out that functionality in the TimeLock code.
The TimeLock is not owned by the Gnosis Multisig and upon creation, there is no "ownability" to it. No one can own it.

I have rewritten RFI code's transfer in a more elegant way.

Claiming Contract Token functionality has been added - no more accidentally losing funds send to a contract address.

There is an airdrop functionality in this contract, users can claim their Airdrop. 

There is a max transfer limit, but now also a 24 hour cooldown as well. 
Meaning you can't just by pass the max transfer by making more transactions.

There is now a team tax, and payments are sent to NIP team's individual wallets when it's payment time.
This is guarded by a 48 hour time lock, and the tax is set to 1% of each transaction.
This does not mean they will automatically dump their tokens. 
You can find the team addresses within this contract, or just ask in discord if you want to know who they are.

There is presale functionality. 
Customers can send BNB to the contract and recieve NIP tokens in return. 

The NIP team is anonymous. 
But we all have established accounts and have put in significant time and effort into this project.
We also answer questions, have AMAs, etc. 
The intention for this project is to be a multi-year endeavor.
Personally I think anonymous teams are fine as long as the project itself is solid.
Just because someone is telling you they are a "real" person doesn't mean anything.
Bitconnect is a primary example of how known people can still rug pull everyone.

This contract will likely be audited in the future.
But there is a problem with Audits.
In 2020 bZx was hacked not once, but twice.
Both times they were audited and passed.
In 2021 there is a popular War on Rugs twitter account that does community audits.
I respect War on Rugs for doing community audits, but even they are not always 100%.
It is extremely hard to know if a project is rugproof, safe, or a good investment.

I do all audits myself. You should find a team you know and trust or learn to do it yourself.

I personally will strive to repair vulnerabilties, issue refunds via NIP tokens, and otherwise fix issues found.
That is the intention of having a proxy, if we need to upgrade, we can.

The NIP team is determined to make a Token with an actual use case for gaming, NFTs, finance systems, and more. 
The roadmap and ideas I have for this token are endless.
If you have a good idea then feel free to suggest. There are no limits.
