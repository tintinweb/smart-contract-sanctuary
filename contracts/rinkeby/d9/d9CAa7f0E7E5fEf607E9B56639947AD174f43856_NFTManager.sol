...
contract NFTManager is ReentrancyGuard, Ownable {
    ...
    event Create(uint256 id, uint256 max_supply, bytes indexed data);
    ...
    function create(uint256 _max_supply, bytes calldata data) external nonReentrant returns (uint256) {
        if (onlyWhitelist) {
        ...
        emit Create(tokenId, _max_supply, data);
        return tokenId;
    }
