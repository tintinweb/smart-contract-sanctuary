...
contract GhostsProject is ERC721Enumerable, Administration, GhostBase {
    ...
    uint256 internal constant MAX_PIONEER_ROUND = 30;
    ...
    uint256[MAX_PIONEER_ROUND] public _pioneerRoundExpire;
    ...
    function setTokenUriBasePreview(string calldata _tokenUriBase) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getPioneerTicketAvailablePerRound(address _address) public view returns (uint256[MAX_PIONEER_ROUND] memory) {
        uint256[MAX_PIONEER_ROUND] memory tickets;
        for (uint256 i = 0; i < currentPioneerRound; i++) {
            if (block.number > _pioneerRoundExpire[i])
                continue;
