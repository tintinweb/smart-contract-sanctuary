...
//        require(cell.nextEvolutionBlock > block.number, "You are ready to evolve");
    ...
    function uri(uint256 _optionId) override public pure returns (string memory) {
        return string(
            abi.encodePacked(
                "https://cell-bucket.s3.eu-north-1.amazonaws.com/", Strings.toString(_optionId), ".json"
            )
        );
    }
...
contract CellToken is ERC1155, ICellToken, Ownable {
    ...
    constructor(address _owner, address _proxyRegistryAddress) ERC1155("https://cell-bucket.s3.eu-north-1.amazonaws.com/{id}.json") {
        require(_owner != address(0), "Address should not be empty");
    ...
    function uri(uint256 _tokenId) override public pure returns (string memory) {
        return string(
            abi.encodePacked(
                "https://cell-bucket.s3.eu-north-1.amazonaws.com/", Strings.toString(_tokenId), ".json"
            )
        );
    }
