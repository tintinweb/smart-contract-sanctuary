    ...
    PausableUpgradeable
{
    ...
    function _handleRewards(
        ...
        // if the vault is private, all the Rand Multi Tokens of the users are exchanged into Rand Stable Tokens
        ...
        if (minimumParticipants > 0) {
            ...
            // if the vault is public, the twa balance of the users will be increased to be equal to the normal balance
            for (uint256 i = 0; i < _participants.length; i++) {
                (uint256 amount, uint256 twaAmount) = multiToken.balancesOf(
                    _participants[i],
                    _vaultId
...
library VaultsStorageLibrary {
    ...
    function removeParticipant(
        ...
        for(uint256 i = 0; i < _self.participants[_vaultId].length; i++) {
            if(_self.participants[_vaultId][i] == _participant) {
                _self.participants[_vaultId][i] = _self.participants[_vaultId][_self.participants[_vaultId].length - 1];
    ...
    function updateLockDuration(VaultsList storage _self, uint256 _vaultId, uint256 _lockDuration)
        internal
    {
