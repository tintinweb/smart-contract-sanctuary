...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable, ReentrancyGuard
{
    ...
    mapping(uint256 => uint256) public mintedTokenTimestamps;
    mapping(uint256 => uint256) public mintedTokenTxs;
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual
    {
        ...
        mintedTokenTimestamps[tokenId] = block.timestamp;
...
contract LNToken     is  ERC721Enumerable
{
    using Address for address;
    using Strings for uint256;
    ...
    uint256 private     salesPrice         = 0.0001 ether;
    
    uint256 private     totalTokens        = 10000;
    uint256 private     leftTokenCount     = totalTokens;
    uint256 private     mintedTokenCount   = 0;

    string  private     baseURI = 'https://ipfs.io/ipfs/Qmf5gSHWn99FJhY4fMH49WMNZbjBfMmufoBZpAZg4iqNLQ/';
    ...
    constructor() ERC721("LNTest", "LNT01")   // temporary Symbol and title
    {
    ...
    function    batchMint(uint256 quantity) external payable callerIsUser nonReentrant
    {
    ...
    function    getMintingDatesFromTokenIds( uint256[] memory tokenIds ) external view returns(uint256[] memory)
    {
        uint256          n          = tokenIds.length;
        uint256[] memory timestamps = new uint256[](n);

        require(n<=100, "Too many NFTs to handle at once");
        
        for (uint256 i=0; i<n; i++)
        {
            uint256 timestamp = mintedTokenTimestamps[ tokenIds[i] ];
