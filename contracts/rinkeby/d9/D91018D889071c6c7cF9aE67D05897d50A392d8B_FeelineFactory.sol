 ...
 */
contract FeelineFactory is Ownable {
    FeelineClub private fc;
    uint256 private constant MAX_ITEMS_PER_ADDRESS = 20;
    ...
    constructor() {
        ...
        fc = FeelineClub(0xb9Bf07ddA39C8469714e533857b40D2e317fcDA9);
        isActive = false;
    }

    function mint(address _to, uint256 _count) public {
        require(_count > 0, "Count must be bigger that 1");
        require(isActive == true, "Mint is not active");
        require(
            (mintedCount[msg.sender] + _count) <= MAX_ITEMS_PER_ADDRESS,
            "Cannot exceed max token per address"
        );
        fc.mintMultipleTo(_to, _count);
        mintedCount[msg.sender] = mintedCount[msg.sender] + _count;
    }

    function transferOwnsershipBackToOwner() public onlyOwner {
        fc.transferOwnership(owner());
    }
