    ...
    BlockTimestamp
{
        ...
        returns (uint256 fees0, uint256 fees1)
    {
        ...
        (uint256 tokensOwed0, uint256 tokensOwed1, , ) = _getTokensOwedAmount(
            address(this),
            position.pool,
            position.liquidity,
            position.feeGrowth0,
    ...
    {
        return IOracle(oracle).checkWethPairsAndLiquidity(token);
    }
    ...
    function _mintPilotForUser(address _recipient, uint256 _pilotAmount) internal {
        if (_pilotAmount > 0) {
    ...
    {
        return IUniStrategy(uniStrategy).getTicks(pool);
    }
    ...
    {
        return IULMState(ulmState).getPoolDetails(pool);
    }
        ...
        )
    {
        ...
        if (a.token0 == WETH || a.token1 == WETH) {
            ...
            if (
                params.pilotToken && ethTokenPair != address(0)
