...
contract UserLeveragePool is AbstractLeveragePortfolio, IUserLeveragePool, ERC20PermitUpgradeable {
    ...
    uint256 public constant WITHDRAWAL_PERIOD = 900;
    ...
    AbstractDependant
{
    ...
    event VirtualStableDeployed(address policyBook, uint256 deployedAmount, bool isLeveraged);
    ...
    function _deployVirtualStableToCoveragePools(address policyBookAddress)
        internal
        returns (bool isLeverage, uint256 deployedAmount)
    {
        if (totalLiquidity != 0) {
            ...
            if (_amountToDeploy > _currentDeployedAmount) {
                ...
                uint256 _totalAmount = deployedAmount.add(_currentDeployedAmount);

                if (_totalAmount > _maxAmount) {
                    deployedAmount = _maxAmount.sub(_currentDeployedAmount);
                }
            ...
            poolsVDeployedAmount[policyBookAddress] = _currentDeployedAmount;

            emit VirtualStableDeployed(policyBookAddress, deployedAmount, isLeverage);
        }
    }
    ...
    function _deployLeverageStableToCoveragePools(
        ...
        if (totalLiquidity != 0) {
            uint256 _currentDeployedAmount =
                poolsLDeployedAmount[policyBookAddress].add(
            ...
            uint256 _amountToDeploy =
                leveragePortfolioView.calcMaxLevFunds(
            ...
            poolsLDeployedAmount[policyBookAddress] = _currentDeployedAmount;
    ...
    function _rebalanceProvidedVirtualStable() internal {
        ...
        for (uint256 i = 0; i < _policyBooksArr.length; i++) {
            ...
            if (mpl > 0) {
                (bool isLeverage, uint256 deployedAmount) =
                    _deployVirtualStableToCoveragePools(_policyBooksArr[i]);

                _policyBookFacade.deployVirtualFundsAfterRebalance(deployedAmount, isLeverage);
            }
        }
    }
}
...
interface ICapitalPool {
    ...
    /// @param _duration uint256 amount in seconds on when the cushion starts
    /// @param _size uint256 lenght size of the window to rebalance from the start of the period
    function setLiquidityCushionPeriods(uint256 _duration, uint256 _size) external;
}
