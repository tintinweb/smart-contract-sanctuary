...
contract DotcMain is owned {
	...
	for(uint i=0; i<collateral_tokens.length; i++)
	{
		...
		require(user.balance[tk.addr] > collateral_dotc_values[i], "insuffcient collateral assets.");
		user.balance[tk.addr] = user.balance[tk.addr].sub(collateral_dotc_values[i]);
		if(tk.addr == token_addr)
		{
			require(user.balance[tk.addr] > dotc_amount, "insuffcient assets.");
