...
contract SmartLease {
    ...
    {
        int256 diff = int256(
            lease_conditions.monthly_miles_limit - _actual_monthly_miles
        );
        monthly_miles_usage_tracker.push(diff);
    }
    ...
    function penalty_to_pay() public view returns (uint256) {
    ...
    function pay_penalty() public payable only_lessee returns (string memory) {
        if (penalty_to_pay() > 0) {
            require(
                msg.value == penalty_to_pay(),
                "Dear Lessee, you must pay the exact amount of the penalty"
            );
            transfer_money_to_lessor(penalty_to_pay());
        } else {
