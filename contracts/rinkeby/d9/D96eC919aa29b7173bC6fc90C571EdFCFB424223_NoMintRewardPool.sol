...
interface IMockToken {
    function mint(address user, uint256 value) external virtual returns (bool);
}

contract NoMintRewardPool is LPTokenWrapper {

    using Address for address;

    address public rewardToken;
    uint256 public duration = 5 minutes; // making it not a constant is less gas efficient, but portable
    ...
    constructor(address _rewardToken,uint256 _duration) public
    {
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            IMockToken(rewardToken).mint(msg.sender, reward);
