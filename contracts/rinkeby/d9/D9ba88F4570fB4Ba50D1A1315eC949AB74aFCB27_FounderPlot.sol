...
contract FounderPlot is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    modifier ifCollectionExists(uint256 _collectionNumber) {
        require(
            collections[_collectionNumber].created,
    ...
    modifier ifCollectionActive(uint256 _collectionNumber) {
        require(
            collections[_collectionNumber].active,
    ...
    modifier ifCollectionNotLocked(uint256 _collectionNumber) {
        require(
            !collections[_collectionNumber].locked,
    ...
    function createCollection(
        ...
        require(
            !collections[_collectionNumber].created,
            "A collection with this collection number already exists"
        );
        require(
            _maxTokensAvailable < ONE_MILLION,
            "The maximum number of tokens available per collection must be less than 1 million"
        );
        require(
            _collectionNumber == nextCollectionNumber,
            "You are trying to create a collection that doesn't match the nextCollectionNumber"
        );
    ...
    function changeName(uint256 _collectionNumber, string memory _name)
        public
        onlyOwner
    ...
    function changeTokenPriceInWei(
        uint256 _collectionNumber,
        uint128 _tokenPriceInWei
    )
        public
    ...
    function changeMaxTokensAvailable(
        uint256 _collectionNumber,
        uint32 _maxTokensAvailable
    )
        public
    ...
    function lockCollection(uint256 _collectionNumber)
        public
        onlyOwner
        ifCollectionExists(_collectionNumber)
        ifCollectionNotLocked(_collectionNumber)
    ...
    function toggleActive(uint256 _collectionNumber)
        public
        onlyOwner
        ifCollectionExists(_collectionNumber)
        ifCollectionNotLocked(_collectionNumber)
    {
        collections[_collectionNumber].active = !collections[_collectionNumber]
            .active;
    }
        ...
        ifCollectionNotLocked(_collectionNumber)
    {
        require(
            _quantity > 0,
            "Number of tokens to purchase must be greater than 0"
        );
        Collection storage collection = collections[_collectionNumber];

        require(
            collection.tokensMinted + _quantity <=
                collection.maxTokensAvailable,
            "This transaction would exceed the maximum number of tokens in this collection"
        );
        require(
            msg.value == _quantity * collection.tokenPriceInWei,
    ...
    changed to view
    */
    function _mintNft(address _to, uint256 _collectionNumber) public {
        uint256 tokenIdToMint = (_collectionNumber * ONE_MILLION) +
            (++collections[_collectionNumber].tokensMinted);
    ...
    added
    */
    function issueToken(address to, uint256 tokenId) public {
        _safeMint(to, tokenId);
    }
    ...
    added
    */
    function issueBatch(address to, uint256[] memory tokenIds) public {
    ...
    function customTokenURI(uint256 _tokenId)
        public
        view
        returns (string memory)
    {
    ...
    function getTokenIdsInCollection(uint256 _collectionNumber)
        public
        view
        returns (uint256[] memory tokenIds)
    {
