...
contract ShaolinSamurai is ERC721, Ownable, PaymentSplitter {
    ...
    function whitelistBuy(uint256 _numberOfPasses) public payable {
        require(
            msg.value == calculateTotalPrice(_numberOfPasses),
            "INCORRECT AMOUNT SENT. SEND EXACT AMOUNTS"
        );
        require(thisContract.send(msg.value), "RECIEVER MUST BE THIS CONTRACT");
        require(
            _tokenIdCounter.current().add(_numberOfPasses) <= maxTokenSupply,
            "ATTEMPTED TO MINT PAST MAX SUPPLY"
        );
        require(
            totalAvailableForUser[msg.sender] >= 1,
            "OUT OF MINT ALLOTMENTS"
        );
    ...
    function regularSaleMint(uint256 _numberOfMints) public payable {
        require(thisContract.send(msg.value), "RECIEVER MUST BE THIS CONTRACT");
        require(
            msg.value == calculateTotalPrice(_numberOfMints),
            "INCORRECT AMOUNT SENT. SEND EXACT AMOUNTS"
        );
        require(
            _tokenIdCounter.current().add(_numberOfMints) <= maxTokenSupply,
            "ATTEMPTED TO MINT PAST MAX SUPPLY"
        );
    ...
    function calculateTotalPrice(uint256 _numberOfPasses)
        public
        view
        returns (uint256)
    {
