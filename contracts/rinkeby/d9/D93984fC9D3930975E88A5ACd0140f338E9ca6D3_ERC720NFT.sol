...
contract ERC720NFT is ERC721URIStorage, AccessControl, ERC721Enumerable, Ownable {
    ...
    address public NFT_ADMIN_WALLET = 0x951D562ECe9A5717C66985939F6D319827367a56;

    bool public isAdminNFTMinted = false;
    constructor() ERC721("SOME", "NFT") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(MINTER_ROLE, msg.sender);
        setBaseURI("https://us-east1-jadu-e23c4.cloudfunctions.net/getJetPack?tokenId=");
        MAXIMUM_TOTAL_SUPPLY = 1111;
        PRE_SALE_TIME_START = 1637172303; 
        PRE_SALE_TIME_END = 1637345103;
        PUBLIC_SALE_TIME_START = 1637348703; 
        
    }

    function mintAdminNFT() public onlyOwner {
        require(!isAdminNFTMinted, "NFT: Already Admin Tokens Minted");
        for(uint i = 0; i< 150; i++) {
            _tokenIds.increment();
            uint256 newItemId = _tokenIds.current();
            _mint(NFT_ADMIN_WALLET, newItemId);
        }
        isAdminNFTMinted = true;
    }
    ...
    returns (bool)
    {
        if(block.timestamp >= PRE_SALE_TIME_START && block.timestamp <= PRE_SALE_TIME_END){
        return true;
        }
        else{
        return false;
       ...
       function publicSaleState()
    public view
    returns (bool)
    {
        if(block.timestamp >= PUBLIC_SALE_TIME_START){
        return true;
        }
        else{
        return false;
    ...
    returns (bool)
    {
        require(preSaleWhiteListAddresses[msg.sender], "NFT, You are not whitelisted to mint pre sale token");
        require(!preSaleAddressesMint[msg.sender], "NFT, You have already minted pre sale token");
        require(block.timestamp >= PRE_SALE_TIME_START && block.timestamp <= PRE_SALE_TIME_END, "NFT: This is not pre sale time for minting");
        require(totalSupply() + 1 <= MAXIMUM_TOTAL_SUPPLY, "NFT: Maximum Supply Minted");
        require(msg.value == MINT_NFT_FEE, "NFT: Invalid Minting Fee");
        payable(NFT_ADMIN_WALLET).transfer(msg.value);
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
    ...
    returns (bool)
    {
        require(block.timestamp >= PUBLIC_SALE_TIME_START, "NFT: Public Sale not yet started");
        require(!preSaleAddressesMint[msg.sender], "NFT, You have already minted token");
        require(totalSupply() + 1 <= MAXIMUM_TOTAL_SUPPLY, "NFT: Maximum Supply Minted");
        require(msg.value == MINT_NFT_FEE, "NFT: Invalid Minting Fee");
        payable(NFT_ADMIN_WALLET).transfer(msg.value);
            _tokenIds.increment();
