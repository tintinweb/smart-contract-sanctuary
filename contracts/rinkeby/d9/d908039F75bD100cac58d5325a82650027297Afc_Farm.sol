...
contract Farm is AccessControl, ReentrancyGuard {
    ...
    function consolidate(address staker) internal {
        ...
        stakers[staker].LpPoints = rewardedPointsLp(staker);
    ...
    function rewardedPointsLp(address staker) public view returns (uint256) {
        Staker storage st = stakers[staker];
        uint256 _seconds = block.timestamp.sub(st.timestamp);
        uint256 earnPerSec = fixedConstantPerToken / (60 * 60 * 24);
        return
            (st.LpPoints + ((st.lpToSepa * earnPerSec) * _seconds) / 1e18) * 2;
    }

    function getConsolidatedRewardsLp(address buyer)
        external
        view
        returns (uint256)
    {
        return stakers[buyer].LpPoints;
    }
