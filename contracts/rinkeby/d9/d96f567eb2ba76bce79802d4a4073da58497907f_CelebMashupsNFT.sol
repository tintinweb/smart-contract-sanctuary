...
contract CelebMashupsNFT is ERC721Enumerable, Ownable { // contract name to yours
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  uint256 public cost = .01 ether; // change price per unit
  uint256 public maxSupply = 2500; // max supply, contract will stop minting after this amount is reach in your open sea colection
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(whitelisted[msg.sender] != true) {
            ...
            if( supply < 100) {   // if else statement
                cost = .00 ether;
               } else if( supply < 200 ){
                cost = .01 ether;
               } else if ( supply < 550 ){
