...
contract LootBox is Ownable, Pausable, ReentrancyGuard, VRFConsumerBase {
  ...
  function _sendTokenWithClass(
    ...
    GameToken nftContract = GameToken(nftAddress);
 ...
 * Taken from https://github.com/dandelionlabs-io/polkafantasy-game-token/blob/master/contracts/GameToken.sol
 */
contract GameToken is ERC721, ERC721Enumerable, AccessControl {
    ...
    string private baseURI;                             // define base URI
    mapping(uint256 => string) public tokensToTraits;   // link tokens to traits split by comma
    
   // @dev to be emited every time an item is minted
    event ItemMinted(address indexed _receiver, string indexed _traits, uint256 indexed _tokenId);
    
   // @dev to be emited every time the metadata for an item is changed
    event ItemUpgraded(string indexed _traits, uint256 indexed _tokenId);

   /**
    * @dev Restricted to members of the admin role.
    */
    modifier onlyAdmin() {
        require(isAdmin(msg.sender), "GameToken::onlyAdmin: Restricted to admins.");
        _;
    }

   /**
    * @dev Restricted to members of the MINTER_ROLE.
    */
    modifier onlyMinter() {
        require(isMinter(msg.sender), "GameToken::onlyMinter: Restricted to minters.");
        _;
    }
    ...
    constructor(
        ...
        require(tokenURIbytes.length > 0, "GameToken::constructor: tokenURI cannot be empty");
   ...
   /**
    * @dev Mints a token and assign the traits, emits an ItemMinted event every time is called
    * @param _traits of the token in string comma separated
    * @param _receiver address of the receiver
    */  
    function mint(string memory _traits, address _receiver) internal onlyMinter {
        
        uint mintIndex = totalSupply();
        _safeMint(_receiver, mintIndex);
        tokensToTraits[mintIndex] = _traits;
        emit ItemMinted(_receiver, _traits, mintIndex);
    }
    
   /**
    * @dev Mints a single token to a received with its traits
    * @param _traits of the token in string comma separated
    * @param _receiver address of the receiver
    */  
    function mintTo(string memory _traits, address _receiver) external onlyMinter {
        require(_receiver != address(0), "GameToken:mintTo: address with value 0");
        bytes memory traitsBytes = bytes(_traits);
        require(traitsBytes.length > 0, "GameToken::mintTo: Traits cannot be empty");
        
        mint(_traits, _receiver);
    }

   /**
    * @dev Mints a batch of token of the same type and assign the traits, emits an ItemMinted event for every token
    * @param _traits of the token in string comma separated
    * @param _receiver address of the receiver
    * @param _quantity amount of tokens to be minted
    */  
    function batchMintTo(string memory _traits, address _receiver, uint _quantity) external onlyMinter {
        require(_receiver != address(0), "GameToken:batchMintTo: the address cannot have value 0");
        ...
        bytes memory traitsBytes = bytes(_traits);
        require(traitsBytes.length > 0, "GameToken::batchMintTo: Traits cannot be empty");
   ...
   /**
    * @dev Mints a batch of token of many different types, to many different addresses
    * emits an ItemMinted event for every token minted
    * @param _traits of the tokens in string comma separated organized sequencially an array list
    * @param _receivers address of the receivers in an array list
    */  
    function batchMint(string[] memory _traits, address[] memory _receivers) external onlyMinter {
        require(_traits.length == _receivers.length, "GameToken::batchMint: invalid parameters length (they should be all same)");
        require(_receivers.length <= MAX_BATCH_MINTING, "GameToken::batchMint: Quantity cannot be bigger than MAX_BATCH_MINTING.");
                
        for (uint16 i = 0; i < _receivers.length; i++) {
            require(_receivers[i] != address(0), "GameToken:batchMint: there is an address with value 0");
            bytes memory traitsBytes = bytes(_traits[i]);
            require(traitsBytes.length > 0, "GameToken::batchMint: Traits cannot be empty");
        }
        
        for (uint i = 0; i < _receivers.length; i++) {
            mint(_traits[i], _receivers[i]);
        }
    }
    
   /**
    * @dev Mints a batch of token of the same type, to many different addresses
    * emits an ItemMinted event for every token minted
    * @param _traits of the tokens in string comma separated organized sequencially an array list
    * @param _receivers address of the receivers in an array list
    */  
    function batchMintForSameTrait(string memory _traits, address[] memory _receivers) external onlyMinter {
        require(_receivers.length <= MAX_BATCH_MINTING, "GameToken::batchMintForSameTrait: Quantity cannot be bigger than MAX_BATCH_MINTING.");
        bytes memory traitsBytes = bytes(_traits);
        require(traitsBytes.length > 0, "GameToken::batchMintForSameTrait: Traits cannot be empty");

        for (uint16 i = 0; i < _receivers.length; i++) {
            require(_receivers[i] != address(0), "GameToken:batchMintForSameTrait: there is an address with value 0");
        }
        
        for (uint i = 0; i < _receivers.length; i++) {
            mint(_traits, _receivers[i]);
        }
    }
    ...
    */  
    function batchMintWithQuantities(string[] memory _traits, address[] memory _receivers, uint[] memory _quantities) external onlyMinter {
        require(_traits.length == _receivers.length && _traits.length == _quantities.length,
                "GameToken::batchMintWithQuantities: invalid parameters length (they should be all same)");
                
        uint totalTokens = 0;
        for (uint16 i = 0; i < _receivers.length; i++) {
            require(_receivers[i] != address(0), "GameToken:batchMintWithQuantities: there is an address with value 0");
            require(_quantities[i] > 0, "GameToken::batchMintWithQuantities: there's a quantity in 0");
            bytes memory traitsBytes = bytes(_traits[i]);
            require(traitsBytes.length > 0, "GameToken::batchMintWithQuantities: Traits cannot be empty");
   ...
   /**
    * @dev updates the string for the traits, emits an ItemUpgraded event every time is called
    * @param _tokenId of the token that needs the traits to be updated
    * @param _traits traits in string format separated by comma
    */       
    function updateTraits(uint256 _tokenId, string memory _traits) external onlyMinter {
        bytes memory traitsBytes = bytes(_traits);
        require(traitsBytes.length > 0, "GameToken::updateTraits: Traits cannot be empty");
        
        tokensToTraits[_tokenId] = _traits;
        emit ItemUpgraded(_traits, _tokenId);
    }
   ...
   /**
    * @dev Creates Token URI. If token exists, the resulting URI for each
    * token will be the concatenation of the `baseURI`, the `tokenId`, and the `traits`.
    * @param _tokenId id of the token to get the URI
    */
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(_tokenId), "GameToken::tokenURI: URI query for nonexistent token");

        return string(abi.encodePacked(_baseURI(), Strings.toString(_tokenId),"/", tokensToTraits[_tokenId]));
    }

   /**
    * @dev Set a new URI for the items metadata
    * @param _tokenURI token URI in string format
    */       
    function setBaseURI(string memory _tokenURI) external onlyAdmin {
        bytes memory tokenURIbytes = bytes(_tokenURI);
        require(tokenURIbytes.length > 0, "GameToken::setBaseURI: _tokenURI cannot be empty");
   ...
   /**
    * @dev returns `true` if the account belongs to the minter role.
    * @param _account address from the account to check
    */  
    ...
    */   
    function addAdmin(address _account) public virtual onlyAdmin {
  ...
  /**
    ...
    * @param _account address from the account to add
    */  
    function removeAdmin(address _account) public virtual onlyAdmin {
