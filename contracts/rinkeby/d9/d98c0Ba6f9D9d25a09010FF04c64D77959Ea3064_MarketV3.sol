...
//	mapping(address => mapping(bytes32 => uint256)) public farmingAmount;
	...
	function _updateOrder(
		...
		delete orderIdByVersion[order.tokenAddress][order.tokenId][_version];
	...
	function createOrder(
		...
		for (uint256 i = _fromVersion; i <= _toVersion; i++) {
            nftVersion[_tokenAddress][_tokenId][i] = msg.sender;
            nftVersionOnSale[_tokenAddress][_tokenId][i] = true;
		...
		if (isERC721) {
			...
			IERC1155(_tokenAddress).safeTransferFrom(msg.sender, address(this), _tokenId, 1, '0x');
		}
...
//		newOrder.retailFee = _retailFee;
		...
		emit OrderCreated(_orderId, _fromVersion, _toVersion);
	...
	function buy(
		...
		require(nftVersion[order.tokenAddress][order.tokenId][_version] == order.owner, 'Version-not-of-saler');
	...
	function createBid(
		...
		uint256 _orderId = orderIdByVersion[_tokenAddress][_tokenId][_version];
		Order memory order = orders[_orderId];
		require(nftVersion[_tokenAddress][_tokenId][_version] == order.owner, 'Version-not-of-saler');
		require(nftVersionOnSale[_tokenAddress][_tokenId][_version], 'Version-not-on-sale');
