...
contract IDOPool is Managed {
        ...
        )
    {
        ...
        thirdRoundStart = secondRoundStart.add(secondRoundDuration);
        saleEnd = saleEndDate;
    }
        ...
        canDeposite(false)
    {
        ...
        if (_nftAddress != address(0)) {
            nftInfos[_nftAddress].deposited = nftInfos[_nftAddress]
                .deposited
                .add(_amount);

            depositedByNFT[_nftAddress][msg.sender] = depositedByNFT[
                _nftAddress
            ][msg.sender]
                .add(_amount);
        }
    ...
    function setTimePoints(
        ...
        saleEndDate = _saleStartDate
            .add(_firstRoundDuration)
            .add(_secondRoundDuration)
            .add(thirdRoundDuration);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 endFirstRound = saleStartDate.add(firstRoundDuration);
        if (
            block.timestamp > saleStartDate && block.timestamp <= endFirstRound
        ) {
            NFTInfo storage info = nftInfos[_nftAddress];
            return
                info.allocationPerNFT.mul(totalRaise).div(PERCENTAGE_100).sub(
                    depositedByNFT[_nftAddress][_sender]
                );
        } else if (
            block.timestamp > endFirstRound &&
            block.timestamp < endFirstRound.add(secondRoundDuration)
        ) {
            NFTInfo storage info = nftInfos[_nftAddress];
            return
                info.allocationForTier.mul(totalRaise).div(PERCENTAGE_100).sub(
                    info.deposited
                );
    ...
    function isThirdRound() internal view returns (bool) {
        return
            ...
            block.timestamp >= saleEndDate.sub(thirdRoundDuration);
    }
