...
contract ERC20 is Context, IERC20 {
    ...
    function transfer(address recipient, uint amount) public override  returns (bool) {
        calculateTaxFee(amount);
        uint256 _amount = amount.sub(taxfee);
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(taxfee).div(10**4); //0.05%
    }
...
contract tokenSale is PriceConsumerV3{
    ...
    address payable wallet = 0x122bA888fa8DaACd03722D6E8E81Fd0aEE163091;
    ...
    function buy(address beneficiary) payable  public
    {
        ...
        calculateTaxFee(amountOfTokens);
        _forwardFunds(msg.value);
        token.transfer(beneficiary, amountOfTokens.sub(taxfee));
  ...
  function getGoldCoin(address _beneficiary) public returns(string memory)
  {
      ...
      if(token.balanceOf(_beneficiary) >= 1000)
      {
        
          token._transfer(_beneficiary, _owner, 1000);
          return "Hurrah !! You can claim your Gold Coin.";
           
      }
      else
      {
          return "Alert !! You should have 1000 GTC tokens to claim a real Gold Coin.";
      }
      
  }

  function claimCoin() public returns (string memory) {
      return 'Congratulations!! You will be rewarded .';
  }
