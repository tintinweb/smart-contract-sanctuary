...
contract Bridge is AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  event TokenDeposited(address indexed ethToken, address indexed ektaToken, uint256 amount);
  ...
  function addTokenPairs(address _ethToken, address _ektaToken) external onlyRole(MAPPER_ROLE) nonReentrant whenNotPaused returns(bool) {
    require(EthEktaPairs[_ethToken] == address(0) && EktaEthPairs[_ektaToken] == address(0), "Already mapped");
  ...
  function updateTokenPairs(address _ethToken, address _ektaToken) external onlyRole(MAPPER_ROLE) nonReentrant whenNotPaused returns(bool) {
    require(EthEktaPairs[_ethToken] != _ektaToken, "Pair already exists");
    require(EthEktaPairs[_ethToken] != address(0), "Pair dont exists");
  ...
  function _mapToken(address _ethToken, address _ektaToken) internal {
    require(_ethToken != address(0) && _ektaToken != address(0), "Token address cant be zero address");
  ...
  function deposit(address _ethToken, uint256 _amount) external nonReentrant whenNotPaused returns(bool) {
    require(_ethToken != address(0), "Token cant be zero address");
    require(_amount > 0, "Amount cant be zero or negative numbers");

    // transfer token to contract address
    IERC20Upgradeable(_ethToken).safeTransferFrom(msg.sender, address(this), _amount);
    
    // update token and amount in user mapping
    tokenBalance[msg.sender][_ethToken] += _amount;
    address ektaToken = EthEktaPairs[_ethToken];
    emit TokenDeposited(_ethToken, ektaToken, _amount);
    return true;
  }
