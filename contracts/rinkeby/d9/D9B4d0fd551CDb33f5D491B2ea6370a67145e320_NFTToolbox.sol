...
contract NFTToolbox is ERC721, Ownable {
    ...
    constructor() ERC721("NFTToolbox", "TBX") {}
    ...
    function mintExactTokenId(uint256 _newItemId) public returns (uint256) {
        _mint(msg.sender, _newItemId);
        return _newItemId;
    }

    function mintToExactTokenId(address _recipient, uint256 _newItemId)
        public
        returns (uint256)
    {
    ...
    function superMintTo(address _recipient)
        external
        onlyOwner
        returns (uint256)
    {
        for (uint256 i = 0; i < 42; i++) {
            _mint(_recipient, claimArray[i]);
        }
    }
}
