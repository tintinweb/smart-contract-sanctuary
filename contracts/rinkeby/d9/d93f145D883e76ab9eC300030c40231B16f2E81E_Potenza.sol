...
contract Potenza is NPassCore {
    ...
    uint256 private MIN_TILE_WIDTH = 1;
    uint256 private MIN_TILE_HEIGHT = 1;
    uint256 private WIDTH = MIN_TILE_WIDTH * 20;
    uint256 private HEIGHT = MIN_TILE_HEIGHT * 20;
    uint256 private MIN_RECT_WIDTH = 4;
    uint256 private MIN_RECT_HEIGHT = 2;
    ...
    struct RenderParams {
        ...
        uint256 w;
        uint256 h;
        uint256 min_rect_width;
        uint256 min_rect_height;
        uint256 tokenId;
        uint256 randomNonce;
    }
    ...
    function getSVGBody(uint256 tokenId) private view returns (string memory) {
        RenderParams memory renderParams = RenderParams(0, 0, WIDTH, HEIGHT, MIN_RECT_WIDTH, MIN_RECT_HEIGHT, tokenId, 0);
        (string memory svgBody,,) = render(renderParams, true);
        return svgBody;
    }

    function generateRenderParams(RenderParams memory renderParams) private view returns(RenderParameters memory) {
        uint256 randomNonce = renderParams.randomNonce;
        uint256 rectX = getRandomNumberForTokenIdBetween(renderParams.tokenId, renderParams.min_rect_width+renderParams.x, renderParams.w+renderParams.x-(renderParams.min_rect_width*2), randomNonce);
        randomNonce = randomNonce + _randomSeed[renderParams.tokenId].randomAddition;
        rectX = rectX - (rectX % MIN_TILE_WIDTH);
        uint256 rectY = getRandomNumberForTokenIdBetween(renderParams.tokenId, renderParams.min_rect_height+renderParams.y, renderParams.h+renderParams.y-(renderParams.min_rect_height*2), randomNonce);
        randomNonce = randomNonce + _randomSeed[renderParams.tokenId].randomAddition;
        rectY = rectY - (rectY % MIN_TILE_HEIGHT);
        uint256 maxRectWidth = renderParams.w+renderParams.x-rectX-renderParams.min_rect_width;
        uint256 maxRectHeight = renderParams.h+renderParams.y-rectY-renderParams.min_rect_height;
        uint256 rectWidth = getRandomNumberForTokenIdBetween(renderParams.tokenId, renderParams.min_rect_width, maxRectWidth, randomNonce);
        randomNonce = randomNonce + _randomSeed[renderParams.tokenId].randomAddition;
        uint256 rectHeight = getRandomNumberForTokenIdBetween(renderParams.tokenId, renderParams.min_rect_height, maxRectHeight, randomNonce);
        randomNonce = randomNonce + _randomSeed[renderParams.tokenId].randomAddition;
    ...
    function render(RenderParams memory rendering, bool _blackTile) private view returns (string memory,uint256,bool) {
        RenderParameters memory renderParameters = generateRenderParams(rendering);
        uint256 randomNonce = renderParameters.randomNonce;
        bool blackTile = _blackTile;
        Rectangles memory rectangles = Rectangles("","","","","");
        RenderParams memory renderParams = RenderParams(0,0,0,0,0,0,0,0);


        //rectangle 0
        if(rectCanContainMoreShapes(renderParameters.rectWidth, renderParameters.rectHeight, rendering.min_rect_width, rendering.min_rect_height))   {
            renderParams = RenderParams(renderParameters.rectX, renderParameters.rectY, renderParameters.rectWidth, renderParameters.rectHeight, rendering.min_rect_width, rendering.min_rect_height, rendering.tokenId, randomNonce);
            (string memory rect, uint256 nonce, bool blackTileReturn) = render(renderParams,blackTile);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect0 = rect;
        } else {
            TileParameters memory tileParameters = TileParameters(renderParams.tokenId, renderParameters.rectX, renderParameters.rectY, renderParameters.rectWidth, renderParameters.rectHeight, randomNonce,blackTile);
        ...
        //rectangle 1
        if(rectCanContainMoreShapes(rendering.w+rendering.x-renderParameters.rectX, renderParameters.rectY-rendering.y, rendering.min_rect_width, rendering.min_rect_height)) {
            renderParams = RenderParams(renderParameters.rectX, rendering.y, rendering.w+rendering.x-renderParameters.rectX, renderParameters.rectY-rendering.y, rendering.min_rect_width, rendering.min_rect_height, rendering.tokenId, randomNonce);
            (string memory rect, uint256 nonce, bool blackTileReturn) = render(renderParams,blackTile);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect1 = rect;
        } else {
            TileParameters memory tileParameters = TileParameters(rendering.tokenId, renderParameters.rectX, rendering.y, rendering.w+rendering.x-renderParameters.rectX, renderParameters.rectY-rendering.y, randomNonce,blackTile);
            (string memory rect, uint256 nonce, bool blackTileReturn) = drawTiles(tileParameters);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect1 = rect;
        ...
        //rectangle 2
        if(rectCanContainMoreShapes(rendering.w-((renderParameters.rectX-rendering.x)+renderParameters.rectWidth),rendering.h-(renderParameters.rectY-rendering.y), rendering.min_rect_width, rendering.min_rect_height)) {
            renderParams = RenderParams(renderParameters.rectX+renderParameters.rectWidth, renderParameters.rectY, rendering.w-((renderParameters.rectX-rendering.x)+renderParameters.rectWidth), rendering.h-(renderParameters.rectY-rendering.y), rendering.min_rect_width, rendering.min_rect_height, rendering.tokenId, randomNonce);
            (string memory rect, uint256 nonce, bool blackTileReturn) = render(renderParams,blackTile);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect2 = rect;
        } else {
            TileParameters memory tileParameters = TileParameters(rendering.tokenId, renderParameters.rectX+renderParameters.rectWidth, renderParameters.rectY, rendering.w-((renderParameters.rectX-rendering.x)+renderParameters.rectWidth), rendering.h-(renderParameters.rectY-rendering.y), randomNonce,blackTile);
            (string memory rect, uint256 nonce, bool blackTileReturn) = drawTiles(tileParameters);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect2 = rect;
        ...
        //rectangle 3
        if(rectCanContainMoreShapes((renderParameters.rectX-rendering.x)+renderParameters.rectWidth, rendering.h-((renderParameters.rectY-rendering.y)+renderParameters.rectHeight), rendering.min_rect_width, rendering.min_rect_height)) {
            renderParams = RenderParams(rendering.x, renderParameters.rectY+renderParameters.rectHeight, (renderParameters.rectX-rendering.x)+renderParameters.rectWidth, rendering.h-((renderParameters.rectY-rendering.y)+renderParameters.rectHeight), rendering.min_rect_width, rendering.min_rect_height, rendering.tokenId, randomNonce);
            (string memory rect, uint256 nonce, bool blackTileReturn) = render(renderParams,blackTile);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect3 = rect;
        } else {
            TileParameters memory tileParameters = TileParameters(rendering.tokenId, rendering.x, renderParameters.rectY+renderParameters.rectHeight, (renderParameters.rectX-rendering.x)+renderParameters.rectWidth, rendering.h-((renderParameters.rectY-rendering.y)+renderParameters.rectHeight), randomNonce,blackTile);
            (string memory rect, uint256 nonce, bool blackTileReturn) = drawTiles(tileParameters);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect3 = rect;
        ...
        //rectangle 4
        if(rectCanContainMoreShapes(renderParameters.rectX-rendering.x, (renderParameters.rectY-rendering.y)+renderParameters.rectHeight, rendering.min_rect_width, rendering.min_rect_height)) {
            renderParams = RenderParams(rendering.x, rendering.y, renderParameters.rectX-rendering.x, (renderParameters.rectY-rendering.y)+renderParameters.rectHeight, rendering.min_rect_width, rendering.min_rect_height, rendering.tokenId, randomNonce);
            (string memory rect, uint256 nonce, bool blackTileReturn) = render(renderParams,blackTile);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect4 = rect;
        } else {
            TileParameters memory tileParameters = TileParameters(rendering.tokenId, rendering.x, rendering.y, renderParameters.rectX-rendering.x, (renderParameters.rectY-rendering.y)+renderParameters.rectHeight, randomNonce,blackTile);
            (string memory rect, uint256 nonce, bool blackTileReturn) = drawTiles(tileParameters);
            randomNonce = nonce;
            blackTile = blackTileReturn;
            rectangles.rect4 = rect;
    ...
    function drawTiles(TileParameters memory tileParameters) private view returns (string memory,uint256,bool) {
        ...
        RGB memory pickedColor = RGB(0,0,0);

        if (USE_COLORS) {
                pickedColor.r = uint8(getRandomNumberForTokenIdBetween(tileParameters.tokenId,0,256,randomNonce));
                randomNonce = randomNonce + _randomSeed[tileParameters.tokenId].randomAddition;
                pickedColor.g = uint8(getRandomNumberForTokenIdBetween(tileParameters.tokenId,0,256,randomNonce));
                randomNonce = randomNonce + _randomSeed[tileParameters.tokenId].randomAddition;
                pickedColor.b = uint8(getRandomNumberForTokenIdBetween(tileParameters.tokenId,0,256,randomNonce));
                randomNonce = randomNonce + _randomSeed[tileParameters.tokenId].randomAddition;
        } else {
            pickedColor = RGB(235,235,228);
        }
    ...
    function tilesLoop(TileParameters memory tileParameters, TileDimensions memory tileDimensions, RGB memory pickedColor) private pure returns (string memory, bool) {
        RGB memory colorToUse = pickedColor;
        ...
        for(uint256 k = 0; k < tileDimensions.tileRows; k++) {
            if(tileDimensions.tileColumns % 2 == 0 || tileDimensions.tilesInLine) {
                (RGB memory color, bool blackTileReturn) = alternateColor(pickedColor, blackTile);
                blackTile = blackTileReturn;
                colorToUse = color;
            }
            for(uint256 l = 0; l < tileDimensions.tileColumns; l++) {
                ...
                string memory begin = string(abi.encodePacked('<path fill="rgb(',toString(colorToUse.r),',',toString(colorToUse.g),',',toString(colorToUse.b),')" stroke="none" paint-order="stroke fill markers" d=" '));
                Coordinates memory coordinates = Coordinates(
                    (l*tileDimensions.tileWidth)+tileParameters.x,
                    ((l+1)*tileDimensions.tileWidth)+tileParameters.x,
                    ((l+1)*tileDimensions.tileWidth)+tileParameters.x,
                    (l*tileDimensions.tileWidth)+tileParameters.x,
                    (k*tileDimensions.tileHeight)+tileParameters.y,
                    (k*tileDimensions.tileHeight)+tileParameters.y,
                    ((k+1)*tileDimensions.tileHeight)+tileParameters.y,
                    ((k+1)*tileDimensions.tileHeight)+tileParameters.y);
                string memory tile = createCoordinateString(coordinates);
                string memory end = '" fill-opacity="1"></path>';
                tilesSVG = string(abi.encodePacked(tilesSVG,begin,tile,end));
            }
        }
    ...
    function createCoordinateString(Coordinates memory coordinates) private pure returns(string memory) {
        return string(abi.encodePacked(renderCoordinateString1(coordinates),renderCoordinateString2(coordinates),renderCoordinateString3(coordinates)));
    }

    function renderCoordinateString1(Coordinates memory coordinates) private pure returns(string memory) {
        return string(abi.encodePacked('M ',toString(coordinates.x1),' ',toString(coordinates.y1),' L ',toString(coordinates.x2),' ',toString(coordinates.y2)));
    }

    function renderCoordinateString2(Coordinates memory coordinates) private pure returns(string memory) {
        return string(abi.encodePacked(' L ',toString(coordinates.x3),' ',toString(coordinates.y3),' L ',toString(coordinates.x4),' ',toString(coordinates.y4)));
    }

    function renderCoordinateString3(Coordinates memory coordinates) private pure returns(string memory) {
        return string(abi.encodePacked(' L ',toString(coordinates.x1),' ',toString(coordinates.y1),' Z'));
    }

    function alternateColor(RGB memory pickedColor, bool blackTile) private pure returns (RGB memory, bool) {
        if(blackTile) {
    ...
    function getRandomNumberForTokenIdBetween(uint256 tokenId, uint256 low, uint256 high, uint256 randomNonce) private view returns (uint256) {
        ...
        if(high > low) {
            uint randomNumber = seed % (high-low);
