...
contract Oink is ERC20("Oink", "OINK"), Ownable {
    ...
    constructor(address _angryBoarsAddress,address _angryMeerKatsAddress, uint256 _baseRate) {
    ...
    function claimReward(uint256 _boarTokenId, uint256 _meerKatTokenId) public returns (uint256) {
        ...
        totalUnclaimedReward1 = computeUnclaimedReward(_boarTokenId);
        ...
        totalUnclaimedReward2 = computeUnclaimedReward(_meerKatTokenId);

        // update the lastClaim date for tokenId and contractAddress
        bytes32 lastClaimMeerKatKey = keccak256(abi.encode(_meerKatTokenId));
        lastClaim[lastClaimMeerKatKey] = block.timestamp;

        totalUnclaimedRewards = totalUnclaimedReward1 + totalUnclaimedReward2;
    ...
    function claimRewards(uint256[] calldata _boarsTokenIds,uint256[] calldata _meerTokenIds) public returns (uint256) {
        require(contractSettings.end > block.timestamp, "Time for claiming has expired");
        ...
        for(uint256 i = 0; i < _boarsTokenIds.length; i++) {
            ...
            uint256 unclaimedReward = computeUnclaimedReward(_boarsTokenId);
        ...
        for(uint256 i = 0; i < _meerTokenIds.length; i++) {
            ...
            uint256 unclaimedReward = computeUnclaimedReward(_meerTokenId);
