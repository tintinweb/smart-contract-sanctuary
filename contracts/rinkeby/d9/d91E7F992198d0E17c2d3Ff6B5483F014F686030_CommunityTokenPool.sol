...
contract CommunityTokenPool {
  address public monsterBlocksToken = 0xE2a00e6D0be0FdEB627fBAC644B67D4C5845912D;
  ...
  function depositAndRedeem(uint256[] memory _depositTokenIds, uint256[] memory _redeemTokenIds) public {
    ...
    _deposit(_depositTokenIds);
    _redeem(_redeemTokenIds);
  }

  function _deposit(uint256[] memory _tokenIds) internal {
    for (uint8 i = 0; i < _tokenIds.length; i++) {
      IERC721(monsterBlocksToken).transferFrom(msg.sender, address(this), _tokenIds[i]);
    }
  ...
  function _redeem(uint256[] memory _tokenIds) internal {
    ...
    for (uint8 i = 0; i < _tokenIds.length; i++) {
      IERC721(monsterBlocksToken).transferFrom(address(this), msg.sender, _tokenIds[i]);
    }
  }
}
