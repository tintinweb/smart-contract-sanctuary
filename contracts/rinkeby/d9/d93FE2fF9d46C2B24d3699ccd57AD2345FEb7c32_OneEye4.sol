...
contract OneEye4 is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    uint256 public MAX_ITEMS = 200;
    uint256 public constant price = 0.025 ether;
    uint256 public constant priceAfterSale = 0.05 ether;
    uint256 public presaleAmount = 20;
    ...
    constructor() ERC721("OneEye4", "OE4") {}
    ...
    function calculatePrice() internal view returns (uint256) {
        ...
        if(_tokenIdCounter.current() >= presaleAmount){
    ...
    function reserve(uint num) public onlyOwner {
        require(_tokenIdCounter.current() + num <= MAX_ITEMS,  "Reserve would exceed max supply of items");
        uint i;
        for (i = 0; i < num; i++) {
    ...
    function mint() public payable {
        ...
        require(totalSupply() < MAX_ITEMS, "Purchase would exceed max supply of items");
        _tokenIdCounter.increment();
        _safeMint(msg.sender, _tokenIdCounter.current());
