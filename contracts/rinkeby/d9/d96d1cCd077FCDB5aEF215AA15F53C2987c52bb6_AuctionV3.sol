...
contract ManagerAuction is Ownable, Pausable {
	...
	function _endAuction(
		...
		// if (_bidderRef != address(0)) {
		// 	uint256 amountToBidderRef = _orderAmount.mul(nftXUserFee).mul(ZOOM_FEE - yRefRate).div(ZOOM_FEE**2); // 1.25
		// 	_paid(_paymentToken, _bidderRef, amountToBidderRef);
		// }
		...
		// 		_paymentToken,
		// 		creator,
		// 		_orderAmount.mul(loyaltyFee).div(ZOOM_FEE) // take retailFee for retailer
		// 	);
		// }

		// _paid(_paymentToken, order.owner, _orderAmount);

		// _updateOrder(_bidder, _orderId, _version);
	}
...
contract AuctionV3 is ManagerAuction, ERC1155Holder, ERC721Holder {
	...
	function createAuction(
		...
		if (isERC721) {
			...
			for (uint256 i = _fromVersion; i <= _toVersion; i++) {
				...
				IERC1155(_tokenAddress).setNftOnSaleVersion(_tokenId, i, false);
	...
	function bidAuction(
		...
		// 	if (_auctionId == bidAuctions[i].auctionId) {
		// 		if (bidAuctions[i].bidPrice >= _price 
		// 			&& bidAuctions[i].status) {
		// 			require(false, 'price-bid-less-than-max-price');
		// 			break;
		// 		}
		// 	}
		// }
	...
	function editBidAuction(
		...
		require(block.timestamp <= currentAuction.endTime, 'not-in-time-auction');
		// for (uint256 i = 0; i < totalBidAuctions; i++) {
		// 	if (objEditBidAuction.auctionId == bidAuctions[i].auctionId) {
		// 		if (bidAuctions[i].bidPrice >= _price 
		// 			&& bidAuctions[i].status) {
		// 			require(false, 'price-bid-less-than-max-price');
		// 			break;
	...
	function acceptBidAuction(
		...
		auctions[bidAuctions[_bidAuctionId].auctionId].status = false;
		bidAuctions[_bidAuctionId].status = false;
		// _endAuction(_buyer, _paymentToken, _bidAuctionId, orderAmount, buyerRef, _version);
