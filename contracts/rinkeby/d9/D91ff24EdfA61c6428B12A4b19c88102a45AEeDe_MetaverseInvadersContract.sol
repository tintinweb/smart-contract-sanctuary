...
abstract contract ERC721MTIV is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
     ...
     function setApprovalForAll(address operator, bool approved) public virtual override {
        ...
        _transferOperatorApprovals[_msgSender()][operator] = approved;
    ...
    function _transfer(
        ...
        require(ERC721MTIV.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
...
abstract contract ERC721EnumMTIV is ERC721MTIV, IERC721Enumerable {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256 tokenId) {
        require(index < ERC721MTIV.balanceOf(owner), "ERC721Enum: Index greater than number of tokens owned");
        uint ownedNftInd;
        ...
        delete ownedNftInd;
        require(false, "ERC721Enum: No token found for the index");
    }
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < totalSupply(), "ERC721Enum: Index out of bounds");
        return index;
    }
}
...
contract MetaverseInvadersContract is ERC721EnumMTIV, Ownable, ReentrancyGuard {
    ...
    uint256 private MAX_PRESALE_MINT = 3;
    ...
    //max amount of Invaders to mint per account - SALE
    uint256 private MAX_SALE_MINT = 7;

    //map how many Invaders an address has minted during presale
    mapping(address => uint256) private saleMints;

    //Cost of INVADING THE METAVERSE
    ...
    function _transferNotice(address _from, address _to, uint256 _tokenId) internal {
        if (trustedContract != address(0)) {
            IWatcher(trustedContract).watchTransfer(_from, _to, _tokenId);
        }
    }
        ...
        payable 
    {
        ...
        //uint256 availableMints = (MAX_SALE_MINT + presaleMints[msg.sender])  - ERC721MTIV.balanceOf(msg.sender);
        uint256 availableMints = MAX_SALE_MINT - saleMints[msg.sender];
        ...
        require(msg.value >= COST * _noInvaders , "MTIV: Insufficient funds in your wallet");

        saleMints[msg.sender] += 1;
