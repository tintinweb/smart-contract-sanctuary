...
/*
 _________  ____  ____  ________      _____          _       ______
|  _   _  ||_   ||   _||_   __  |    |_   _|        / \     |_   _ \
|_/ | | \_|  | |__| |    | |_ \_|      | |         / _ \      | |_) |
    | |      |  __  |    |  _| _       | |   _    / ___ \     |  __'.
   _| |_    _| |  | |_  _| |__/ |     _| |__/ | _/ /   \ \_  _| |__) |
  |_____|  |____||____||________|    |________||____| |____||_______/
...
contract MoleculeSynthesizer is MoleculeScripter, ERC721 {
  ...
  constructor(address _elementsContract) ERC721("Molecules", "MOLECS") {
  ...
  event NewMolecule(uint moleculeId, string formula, string key, string name, uint16 scriptId);
  ...
  function _distributeFunds(uint _scriptId, string memory _formula, uint _numberOfElements) internal {
    if (msg.value > 0) {
      ...
      for (uint i = 0; i < elementIds.length; i++) {
        payable(elementBlocksContract.ownerOf(elementIds[i])).transfer(fundsPerElement);
      }

      // general element royalties
      payable(elementBlocksContract.ownerOf(royaltyHoldingElement)).transfer(
        msg.value*elementPercentage/2/100
        );
      _nextRoyaltyHolder();

      // script creator funds
      payable(scripts[_scriptId].creator).transfer(
...
contract MoleculeScripter is Ownable {
  ...
  /// @param active scripts can be minted to
  ...
  function createScript(
    ...
    uint64 _endPrice
      ...
      scripts.push(Script(_name, _scriptCode, msg.sender, false, false, false, 0, _totalSupply, _saleDuration, _startPrice, _endPrice));
      uint id = scripts.length -1;
      creatorScriptCount[msg.sender]++;
      scriptToCreator[id] = msg.sender;
      emit NewScript(id, _name, _scriptCode, msg.sender, false, false, false, 0, _totalSupply, _saleDuration, _startPrice, _endPrice);
  }

  /// @notice allows to activate / deactivate a script and sets starting time for the sale
  function scriptOnOffSwitch(uint _scriptId, bool _active) external onlyScriptCreator(_scriptId) {
    scripts[_scriptId].active = _active;
    if (_active) {
  ...
  /// @notice checks if the script is active and below its total supply
  modifier mintableScript(uint _scriptId) {
    require(scripts[_scriptId].active || msg.sender == scripts[_scriptId].creator || msg.sender == owner(), "Script not active");
