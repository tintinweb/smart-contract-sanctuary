...
import { AccountVerifier } from "../library/AccountVerifier.sol";

contract UniswapV2Oracle {
    address public immutable uniswapFactory; // = 0xc0a47dFe034B400B47bDaD5FecDa2621de6c4d95 for mainnet;
    ...
    function saveState(
        ...
        (stateRoot, blockTimestamp, blockNumber) = BlockVerifier.extractStateRootAndTimestamp(blockData);
    ...
    function saveReserve(
        ...
        ObservedData memory data
    ){
        bytes32 stateRoot = blockState[blockNumber].stateRoot;
        require(stateRoot != bytes32(0), "!saved"); // hope there aren't any valid bytes32(0) stateRoot
        bytes32 storageRoot = AccountVerifier.getAccountStorageRoot(pair, stateRoot, accountProof);
        (data.reserve0, data.reserve1, data.reserveTimestamp) = unpackReserveData(
            Rlp.rlpBytesToUint256(
                MerklePatriciaVerifier.getValueFromProof(
                    storageRoot, reserveTimestampSlotHash, reserveProof
                )
            )
        );
        data.price0Data = Rlp.rlpBytesToUint256(
            MerklePatriciaVerifier.getValueFromProof(
                storageRoot, token0Slot, price0Proof
        )
        );
        data.price1Data = Rlp.rlpBytesToUint256(
            MerklePatriciaVerifier.getValueFromProof(
                storageRoot, token1Slot, price1Proof
        )
        );
        observedData[blockNumber][pair] = data;
    }

    function unpackReserveData(uint256 packedReserveData) internal pure returns(uint112 reserve0, uint112 reserve1, uint32 reserveTimestamp) {
    ...
    function getLiquidity(
        ...
        IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(uniswapFactory, token, denominator));
        Window memory currentWindow = window[windowIndex(block.timestamp)];
        uint128 lastObserved = currentWindow.to;
        if(lastObserved == 0) {
            lastObserved = window[windowIndex(block.timestamp) - WINDOW_SIZE].to;
        ...
        //TODO : check if this value is a lot different than current value;
        ObservedData memory historicData = observedData[lastObserved][address(pair)];
        return denominationTokenIs0 ? historicData.reserve0 : historicData.reserve1;
    }
    ...
    function getExchangeRatio(
        ...
        uint256 historicePriceCumulative = calculatedPriceCumulative(
            denominationTokenIs0? historicData.reserve0 : historicData.reserve1,
            denominationTokenIs0? historicData.reserve1 : historicData.reserve0,
            denominationTokenIs0? historicData.price1Data : historicData.price0Data,
        ...
        uint256 currentPriceCumulative = calculatedPriceCumulative(
            ...
            denominationTokenIs0? pair.price1CumulativeLast() : pair.price0CumulativeLast(),
            block.timestamp - blockTimestampLast
        );
        //TODO : check if this value is a lot different than current value
        return (currentPriceCumulative - historicePriceCumulative) / (block.timestamp - state.blockTimestamp);
    }
    ...
    function calculatedPriceCumulative(
        ...
        return priceCumulativeLast + timeElapsed * uint(
            UQ112x112
            .encode(reserve)
            .uqdiv(pairedReserve)
        );
