...
contract SubscriptionManager is Ownable {
    ...
    function collectSubscription(
        ...
        token.transferFrom(subscriber, address(this), offerPriceAmount + tokenGasFee);
    }
    ...
    function refund(
        ...
        token.transferFrom(address(this), subscriber, refundAmount);
    }
    ...
    function withdrawTo(ERC20 token, address recipient, uint256 amountWei) external onlyOwner {
        require(token.balanceOf(address(this)) >= amountWei, "Not enough tokens");
        token.transfer(recipient, amountWei);
    }
    ...
    function addMerchantBalance(ERC20 token, address merchant, uint256 amountWei) external onlyOwner {
        merchantTokenBalances[merchant][token] += amountWei;
    }

    function decreaseMerchantBalance(ERC20 token, address merchant, uint256 amountWei) external onlyOwner {
