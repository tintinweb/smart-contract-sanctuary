...
contract NameTag is ERC721, Ownable, StringUpper, DenyList, Random {
    ...
    event NameChanged(string indexed from, string indexed to, uint256 indexed tokenId);
    ...
    function _preValidatePurchase(uint256 tokensAmount, uint256 namesLength) internal view {
        ...
        uint256 weiAmount = tokensAmount.mul(price());
        if (namesLength > 0) {
            weiAmount =  weiAmount.add(namesLength.mul(metadataFee()));
        }
        require(weiAmount <= msg.value);
    }

    function _getTokenAmount(uint256 _amount) internal returns (uint256) {
        if (currentWaveIndex() < waves.length - 1) {
            uint256 toNextLimitAmount = currentLimit().sub(totalSupply());

            if (_amount >= toNextLimitAmount) {
                _currentWaveIndex += 1;
    ...
    function _processPurchase(address recipient, uint256 _tokenAmount) internal returns (uint256[] memory){
    ...
    function _setName(uint256 _token, string memory _name) internal returns (bool) {
        ...
        bool status = _changeTokenName(_token, _name);
        if(status == false) {
            return false;
        }
    ...
    function setNames(uint256[] memory _tokens, string[] memory _names) public payable returns (bool[] memory) {
        ...
        require(_names.length.mul(metadataFee()) <= msg.value);
...
//    todo public or external
    function setMetadata(uint256[] memory _tokens, string[] memory _metadata) public onlyOwner () {
//        todo minter role
    ...
    function _setNames(uint256[] memory _tokens, string[] memory _names) internal returns (bool[] memory) {
        uint256 length;
        uint256 metaLength;
        if (_names.length < _tokens.length) {
            length = _names.length;
            metaLength = _tokens.length;
        } else {
        ...
        for (; index < length; index += 1) {
            ...
            if (statuses[index]) {
                ...
                _setTokenURI(_tokens[index], defaultMetadata());
            }
        }
        for (; index < metaLength; index += 1) {
            _setTokenURI(_tokens[index], defaultMetadata());
        }
    ...
    function _setDefaultMetadata(uint256[] memory _tokens) internal {
        for (uint index = 0; index < _tokens.length; index += 1) {
            _setTokenURI(_tokens[index], defaultMetadata());
        }
    }
    ...
    function _changeTokenName(uint256 tokenId, string memory _name) internal virtual returns(bool){
        ...
        emit NameChanged(oldName, _name, tokenId);
        return true;
    }


    function buyNamedTokens(uint256 _amount, string[] memory _names) external payable returns (uint256[] memory, bool[] memory) {
        require(_amount > 0);
        uint256 tokensAmount = _getTokenAmount(_amount);
        uint256 namesLength = _names.length < tokensAmount ? _names.length : tokensAmount;

        _preValidatePurchase(tokensAmount, namesLength);

        uint256[] memory tokens = _processPurchase(msg.sender, tokensAmount);
        return (tokens, _setNames(tokens, _names));
    }

    function buyTokens(uint256 _amount) external payable returns (uint256[] memory) {
        require(_amount > 0);
        uint256 tokensAmount = _getTokenAmount(_amount);

        _preValidatePurchase(tokensAmount, 0);

        uint256[] memory tokens = _processPurchase(msg.sender, tokensAmount);
        _setDefaultMetadata(tokens);
