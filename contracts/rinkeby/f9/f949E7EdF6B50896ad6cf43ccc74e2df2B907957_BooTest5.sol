...
contract BooTest5 is ERC721, ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    
    // Set variables
    
    uint256 public constant BOO_SUPPLY = 10000;
    ...
    constructor() ERC721("BooTest5", "BT5") {}
    
    function mint(uint16 numberOfTokens) public payable {
        require(isSaleActive(), "Boo sale not active");
        require(balanceOf(msg.sender).add(numberOfTokens) <= 20, "You cannot have more than 20 boos in a wallet");
        require(totalSupply().add(numberOfTokens) <= BOO_SUPPLY, "Insufficient boos available, please try to mint less boos");
