...
contract SALNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function Presalemint(address _to, uint256 _mintAmount) public payable {
    ...
    require(supply + _mintAmount <= 3000);
    require(supply >= 1000);
  ...
  function BirdMint(address _to, uint256 _mintAmount) public payable {
      uint256 supply = totalSupply();
      require(!paused);
      require(_mintAmount > 0);
      require(_mintAmount <= maxbirdAmount);
      require(tokensPerWallet[msg.sender] + _mintAmount <= 5, "MetaBillionaire: You can't mint more than 5 tokens!");
