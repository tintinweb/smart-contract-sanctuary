...
contract StakingPenaltyPool is StakingPool {
    ...
    event ChargedPenalty(uint256 feeAmount, uint256 penaltyBP);
    ...
    function withdrawStake(uint256 _amount) public override {
        uint256 feeAmount = _chargePenalty(msg.sender, _amount);
        super.withdrawStake(_amount.sub(feeAmount));
    }

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function emergencyWithdraw() public override {
        _chargePenalty(msg.sender, userInfo[msg.sender].amount);
    ...
    function _chargePenalty(address _userAddress, uint256 _claimAmount) internal returns (uint256 feeAmount) {
        ...
        if(address(tierCalculator) != address(0)) {
            penaltyBP = getPenaltyBP(tierCalculator.userLockingStarts(_userAddress));
            feeAmount = _claimAmount.mul(penaltyBP).div(MAX_BPS);
            stakingToken.safeTransfer(feeTo, feeAmount);
            user.amount = user.amount.sub(feeAmount);
        }

        emit ChargedPenalty(feeAmount, penaltyBP);
    }
}
