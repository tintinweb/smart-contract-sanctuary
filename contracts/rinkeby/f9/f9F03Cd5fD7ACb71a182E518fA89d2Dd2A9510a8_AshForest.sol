...
contract AshForest is AdminControl, ICreatorExtensionTokenURI, ILazyDelivery, ILazyDeliveryMetadata {
    ...
    constructor(address creator, address marketplace) {
        _creator = creator;
    ...
    function deliver(address sender, uint256 listingId, uint256 assetId, address, uint256, uint256) external override returns(uint256) {
        require(listingId == _listingId);
        uint tokenId = IERC721CreatorCore(_creator).mintExtension(sender);
        tokenIds[tokenId] = assetId;
        return tokenId;
    }
    ...
    function tokenURI(address creator, uint256 tokenId) external view override returns (string memory) {
        ...
        return this.assetURI(tokenIds[tokenId]);
    }
}
