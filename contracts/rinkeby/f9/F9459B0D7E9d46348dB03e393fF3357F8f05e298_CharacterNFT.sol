  ...
  UUPSUpgradeable
{
    ...
    onlyRole(MINTER_ROLE)
  {
    ...
    character.dexterity = uint8((attributes[1] % 100) + 1);
    character.constitution = uint8((attributes[2] % 100) + 1);
    character.intelligence = uint8((attributes[3] % 100) + 1);
    character.wisdom = uint8((attributes[4] % 100) + 1);
    character.charisma = uint8((attributes[5] % 100) + 1);
  }
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            uint256(character.dexterity).toString(),
            '}]}'
          )
        )
      )
