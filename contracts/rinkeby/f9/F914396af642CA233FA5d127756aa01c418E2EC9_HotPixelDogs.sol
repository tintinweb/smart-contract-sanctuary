...
contract HotPixelDogs is ERC721Enumerable, Ownable {
	using Address for address;
	using Counters for Counters.Counter;
	Counters.Counter private _tokenCount;

	string public baseURI;
	uint256 public constant price = 100000000000000000; // 0.100000000000000000 ETH
	...
	constructor(string memory baseURl) ERC721("HotPixelDogs", "HPD") {
		setBaseURI(baseURl);
	}
	...
	function mintPreSale(uint256 _amount) external payable {
		...
		require(_allowListClaimed[msg.sender] + _amount <= maxPerWallet, "Purchase exceeds max allowed");
		require(minted[msg.sender] + _amount <= maxPerWallet, "Purchase would exceed max tokens per wallet");
		require(_tokenCount.current() + _amount <= maxTokens, "Purchase would exceed max supply of tokens");
		require(msg.value >= price * _amount, "Ether value sent is not correct");
	...
	function reserve(address _to, uint256 _amount) public onlyOwner {
		require(_amount > 0 && _tokenCount.current() + _amount <= maxTokens, "Purchase would exceed max supply of tokens");
		for (uint256 i; i < _amount; i++) {
			_tokenCount.increment();
