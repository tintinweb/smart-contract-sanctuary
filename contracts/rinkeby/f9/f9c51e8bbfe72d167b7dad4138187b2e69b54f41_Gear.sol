...
contract Gear is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getRing(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "CYBER", cyberWear);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Lootbox #', toString(tokenId), '", "description": "Gear is randomized cyberpunk gear generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use Gear in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
