...
contract TestDrop is ERC721Enumerable, Ownable {
    using MerkleProof for bytes32[];

    mapping(address => bool) public alreadyMinted;

    uint16 private reserveTokensId;
    uint16 private tokensId;
    ...
    uint256 public constant maxMint = 10000;

    function _baseURI() internal view virtual override returns (string memory) {
        return baseURI;
    }

    constructor() ERC721("Test Drop", "TEDR") {
        baseURI = "ipfs://QmVof9RdE4Mq3Uo9E7WiGzRcJkwBaodWgVwG6z4659NNVs/";
        reserveTokensId = 1; // item 1-50
        tokensId = 51; // item 51-10000
    }
    ...
    function mint(bytes32[] memory proof, bytes32 leaf) public returns (uint256) {
        ...
        require(tokensId <= maxMint, "Mint limit reached");

        _safeMint(msg.sender, tokensId++);
    ...
    function reserveTokens(address to, uint8 amount) public onlyOwner {
        require(reserveTokensId + amount <= 51, "Out of stock");

        for (uint8 i = 0; i < amount; i++) _safeMint(to, reserveTokensId++);
    }
