...
contract BowerBirds is ERC721Enumerable, Ownable, PaymentSplitter, ReentrancyGuard, Pausable {
	...
	uint256 public OGIndex = 999;
	uint256 public genesisIndex;
	...
	mapping(IBurnable => bool) private mintPasses;
	...
	/**
		@dev 0 to genesisIndex (reserved for mint passes)
	 */
	function claim(IBurnable mintPass, uint256 count) external nonReentrant whenNotPaused {
		require(mintPasses[mintPass], "not a valid interface");
		require(count > 0, "cannot be Zero");
		require(mintPass.balanceOf(msg.sender, 1) == count, "need to own pass specified");
		for(uint256 i = 0; i < count; i++) {
			mintPass.burn(msg.sender, 1, 1);
  ...
  function getCurrentPrice() public view returns (uint256) {
	...
	function setAuction(uint256 _startingPrice, uint256 _endingPrice, uint256 _duration, uint256 _startedAt, uint256 _endedAt) external onlyOwner {
	...
	function setGenesisIndex(uint256 _genesisIndex) public onlyOwner {
	...
	function setMintPass(IBurnable pass, bool val) external onlyOwner {
		mintPasses[pass] = val;
	}
}
