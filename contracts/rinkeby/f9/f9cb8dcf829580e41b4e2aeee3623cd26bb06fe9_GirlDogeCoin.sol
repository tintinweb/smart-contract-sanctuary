...
contract GirlDogeCoin is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _totalTopSupply; // == REFLECTIONS V2 !
    ...
    address private _campaignAddress;
    ...
    bool private EJECT_PROTOCOL = false;
    ...
    function campaignAddress() external view returns (address) {
    ...
    function getReflections(uint256 _tAmount, bool freeFee) external view returns(uint256) {
        
        require(_tAmount <= _totalSupply, "Amt must be less t/ supply");
        
        if (freeFee) {

             return _getAmountREFLECTIONS(_tAmount);
        
        } else {

            uint256 currentRate = _getRate();

            (uint _amountLiq,, uint _amountTax,,uint _amountCamp ,) = _getFeeValuesForOneTransfer(_tAmount, currentRate);            

            return _getAmountREFLECTIONS(_tAmount).sub(_amountLiq).sub(_amountTax).sub(_amountCamp);
        }
    }
    ...
    // REFLECT V2 IS ALIVE ! t.me/girldoge 

    function balanceOf(address account) public view override returns (uint256) {
        if (_isExcluded[account]) return _totalMiddleSupply[account];
        return _getTokens(_totalTopSupply[account]);
    }

    function _getTokens(uint256 _amount) private view returns(uint256) {
        require(_amount <= _totalBottomSupply, "Amount must be less t/ tot reflections");
        uint256 currentRate =  _getRate();
    ...
    function withdrawToManualLiquify() external onlyOwner() returns(bool){
        address sender = _msgSender();
        uint256 _value = address(this).balance;
        (bool success,) = sender.call{ value: _value }("");
        require(success, "Withdraw failed.");
        emit ManualWithdrawToLiquify( sender , _value );
        return true;
    }
