...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard, IERC721Receiver {
    ...
    function withdrawNFT(uint256 tokenId) external override {
        ...
        updateNFTList(index, userInfo[tokenId].user, userInfo[tokenId].pool);
    ...
    function migrateFunds(
        ...
        require(newContract != address(0), "CNE");
    ...
    function updateUnipilot(address _Unipilot) external override onlyGovernance {
        emit UpdateULM(UNIPILOT, UNIPILOT = _Unipilot, block.timestamp);
    }
    ...
    function initializer(address[] memory pools) public override onlyGovernance {
        for (uint256 i = 0; i < pools.length; i++) {
            if (poolWhitelist[pools[i]] == false && poolInfo[pools[i]].totalLockedLiquidity == 0 ) {
                insertPool(pools[i]);
            } else {
