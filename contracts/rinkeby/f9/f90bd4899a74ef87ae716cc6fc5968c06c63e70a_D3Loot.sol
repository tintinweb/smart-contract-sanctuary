...
abstract
contract IQLF {
    function is_qualified(address account) virtual external view returns (bool, string memory);
}


// File contracts/D3Loot.sol
...
contract D3Loot is ERC721Upgradeable, OwnableUpgradeable {
    ...
    mapping(string => string[]) private trait_list;
    // how many NFT(s) claimed through `air drop`
    mapping(address => bool) public airdop_claimed;
    ...
    function mint(
        ...
        payable
    {
        require(_payment_token_index < payment_list.length, "invalid payment token");
        require (start_time < block.timestamp, "not started");
        if (whitelist != address(0)) {
            bool qualified;
            string memory error_msg;
            (qualified, error_msg) = IQLF(whitelist).is_qualified(msg.sender);
        ...
        require(id_counter < total_quantity, "no NFT left");
        uint8 actual_number_of_nft = _number_of_nft;
        if ((sold_quantity + _number_of_nft) > total_quantity) {
            actual_number_of_nft = uint8(total_quantity - sold_quantity);
        }
        {
            uint256 total = payment_list[_payment_token_index].price;
            total = total.mul(actual_number_of_nft);
            address token_address = payment_list[_payment_token_index].token_addr;
            if (token_address == address(0)) {
                ...
                // transfer to owner
                IERC20(token_address).safeTransferFrom(_msgSender(), owner(), total);
            }
            payment_list[_payment_token_index].receivable_amount += total;
        }
        {
    ...
    function claim_airdrop(
        ...
        external
    {
        require(!airdop_claimed[_msgSender()], 'already claimed');
        // 1. validate
        bytes32 leaf = keccak256(abi.encodePacked(index, msg.sender, amount));
        require(MerkleProof.verify(_proof, merkle_root, leaf), 'not qualified for airdrop');
        // 2. mint
        for (uint256 i = 0; i < amount; i++) {
            _mint(_msgSender());
        }
        // 3. update `airdop_by_addr`
        airdop_claimed[_msgSender()] = true;
    }
    ...
    function _mint(address to) internal {
        id_counter++;
        _safeMint(to, id_counter);
    }

    // TODO, set start_time, etc

    function admin_mint(address to) external onlyOwner {
        _mint(to);
    }
