...
/**
 * @title ESMintPass
 * @dev Ethereal Studios Mint Pass
 *
 * Authors: s.imo
 * Created: 15.10.2021
 */
contract ESMintPass is ERC721Enumerable, Ownable {
    ...
    // the maximum number of tokens per whitelisted address
    uint256 public constant MAX_NUM_WHITELIST_TOKENS = 2;
    // the map of whitelisted addresses
    mapping(address => bool) private whiteList;
    // flag to activate the whitelist minting of mint-pass
     ...
     */
    constructor()
    ERC721("Ethereal Studios Mint Pass", "ETHSD.IO")
    {}
    ...
    /**
     * @dev Mint 'numOfPass' new mint-pass, only whitelisted
     */
    function mintWhitelisted(uint256 numOfPass) public payable {
        require(whitelistActivated, "Whitelist not enabled");
        require(whiteList[_msgSender()], "Caller not whitelisted");
        require(balanceOf(_msgSender()).add(numOfPass) <= MAX_NUM_WHITELIST_TOKENS, "Max whitelist tokens");
        require(totalSupply().add(numOfPass) <= MAX_SUPPLY, "Sale has already ended");
        require(MINT_PRICE.mul(numOfPass) == msg.value, "Ether value sent is not correct");
    ...
    */
    ...
    /**
     * @dev Toggle the whitelist mint activation flag
     */
    function toggleWhitelistSalesState() public onlyOwner() {
        whitelistActivated = !whitelistActivated;
    }

    /**
     * @dev Set a list of address whitelist status
     */
    function setWhitelistAddress(address[] calldata wlAddress, bool value) public onlyOwner() {
        for(uint i = 0; i < wlAddress.length; i++){
            whiteList[wlAddress[i]] = value;
        }
    }
