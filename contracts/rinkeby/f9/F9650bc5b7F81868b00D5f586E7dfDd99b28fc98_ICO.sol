...
contract ICO is Ownable, ReentrancyGuard {
    ...
    function _changeCurrentAllocatedTokens(uint256 amount, uint256 ethForTokens, bool isReferral) private {
        if (amount <= current_allocatedTokens) {
            current_allocatedTokens = current_allocatedTokens.sub(amount);
            uint256 ethForStep = getCost(amount);
            if(isReferral){
                ethForStep = ethForStep.sub(ethForStep.mul(referralBonus).div(100));
            }
            ethInStep[currentIterationOfICO] = ethInStep[currentIterationOfICO].add(ethForStep);
        } else {
            ...
            while (amountForLoop > 0) {
                if (amountForLoop > current_allocatedTokens) {
                    ...
                    uint256 ethForStep = getCost(amountForLoop);
                    if(isReferral){
                        ethForStep = ethForStep.sub(ethForStep.mul(referralBonus).div(100));
                    }
                    ethInStep[currentIterationOfICO] = ethInStep[currentIterationOfICO].add(ethForStep);
    ...
    function withdrawReward() public nonReentrant {
        ...
        _changeRewardAmount();
        if (ethInStep[currentIterationOfICO] > address(this).balance) {
            ethInStep[currentIterationOfICO] = address(this).balance;
        }
        emit WithdrawedETH(msg.sender, amount);
    }

    //Function to withdraw all unused ETH from smart contract after ICO finished
    function withdrawETH() public nonReentrant onlyOwner {
        require(icoCompleted, 'ICO not completed yet');
        uint256 monthAfterICO = icoEndTime.add(4 weeks);
        require(block.timestamp > monthAfterICO, 'Month after the end of the ISO has not yet passed');
    ...
    function getRewardAmount() public view returns (uint256 amount) {
        ...
        for (uint256 i = 0; i < currentIterationOfICO; i++) {
            ...
            amount = amount.add((ethInStep[i].div(getTokensPerIteration(i).div(10 ** 10)))
            .mul(usersRewards[msg.sender][i].amountTokens.div(10 ** 10)));
            if (icoCompleted) {
                amount = amount.add((ethInStep[currentIterationOfICO].div(getTokensPerIteration(currentIterationOfICO)
                .div(10 ** 10))).mul(usersRewards[msg.sender][currentIterationOfICO].amountTokens.div(10 ** 10)));
    ...
    function buy() public payable whenIcoStart allowedInvestor {
        ...
        _changeCurrentAllocatedTokens(amount, ethForTokens, false);
    ...
    allowedInvestor whenIcoStart {
        ...
        require(amount >= minLimit, 'Amount for one purchase is too low');
        require(amount <= maxLimit, 'Limit for one purchase is reached');
        _changeCurrentAllocatedTokens(amount, ethForTokens, true);
        _sendTokens(msg.sender, amount);
    ...
    function buyFor(address buyer) public payable onlyOwner whenIcoStart {
        ...
        _changeCurrentAllocatedTokens(amount, msg.value, false);
    ...
    onlyOwner whenIcoStart {
        ...
        require(amount <= maxLimit, 'Limit for one purchase is reached');
        require(buyer != address(0), 'Buyer address should not be empty');
        _changeCurrentAllocatedTokens(amount, msg.value, true);
        IERC20(tokenAddress).transfer(buyer, amount);
        emit Bought(buyer, amount, usdPrice);
        (bool success,) = referral.call{value : bonusAmount}("");
        require(success);
