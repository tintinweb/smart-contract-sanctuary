...
contract BirdezGang is ERC721Enumerable, Ownable {
  ...
  function updateReservedMerkleRoot(bytes32 _newMerkleRoot) external onlyOwner {
  ...
  function mint(uint256 _numberOfTokens) public payable {
    ...
    require(msg.value == MINT_PRICE * _numberOfTokens, "incorrect-ether-value");
    require(
  ...
  function whitelistedMint(bytes32[] calldata merkleProof) external payable {
    address _user = msg.sender;

    require(totalSupply() + 1 <= MAX_BIRDEZ, "max-supply-reached");
    require(maxMintsPerAddress[_user] < MAX_WHITELIST_MINT, "max-mint-limit");
    ...
    require(isReserved || msg.value == MINT_PRICE, "incorrect-ether-value");
