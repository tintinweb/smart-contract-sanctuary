...
contract MyCollectible is ERC721Enumerable, Ownable, PaymentSplitter, ReentrancyGuard {
  ...
  address[] private _team = [0x09dB76fe7Bc352E4a8EfC1AB1e1Bc578a6F4D12c];
  ...
  function giftMint(address[] calldata _addresses) external onlyOwner {
    require(totalNFT + _addresses.length <= maxSupply, "Bored Bunny: max total supply exceeded");

    uint256 _newItemId;
    for (uint256 ind = 0; ind < _addresses.length; ind++) {
      require(_addresses[ind] != address(0), "Bored Bunny: recepient is the null address");
      _tokenIds.increment();
  ...
  function presaleMint(uint256 _amount) external payable nonReentrant {
    ...
    require(_amount <= presaleAmountLimit, "Bored Bunny: You can't mint so much tokens");
    require(_presaleClaimed[msg.sender] + _amount <= presaleAmountLimit, "Bored Bunny: You can't mint so much tokens");

    require(totalMint + _amount <= maxSupply - giftSupply, "Bored Bunny: max supply exceeded");
    require(_price * _amount <= msg.value, "Bored Bunny: Ether value sent is not correct");
    uint256 _newItemId;
  ...
  function burn(uint256 tokenId) external {
    ...
    require(_isApprovedOrOwner(msg.sender, tokenId), "Bored Bunny: burn caller is not owner nor approved");
    _burn(tokenId);
    totalNFT = totalNFT - 1;
  }
