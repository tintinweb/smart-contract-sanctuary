...
contract TokenRedeemer is Ownable, ReentrancyGuard {
  ...
  */
  ...
  // collection out => groupId in => RedemptionData
  mapping (address => mapping (uint256 => RedemptionData)) public redemptionData;
  ...
  */
  function _redeemToken(uint256 _tokenId, address _tokenOut) internal {
    ...
    require(redemptionData[_tokenOut][_groupIdIn].redeemer == address(0), "TokenRedeemer::redeem: token has already been redeemed for this group" );
    ...
    (bool groupInit, , , , , , , , , uint256 mintCount,) = super1155.itemGroups(groupIdOut);
    ...
    for(uint256 i = 0; i < redemptionAmount; i++) {
      ...
      amounts[i] = uint256(1);
      console.log("trace", ids[i], amounts[i]);
    }

    redemptionData[_tokenOut][_groupIdIn].redeemer = _msgSender();
    redemptionData[_tokenOut][_groupIdIn].tokenIdIn = _tokenId;
