...
contract Sora is VRFConsumerBase, ERC721, Ownable {
    ...
    string public LICENSE = ""; // The license text/url. Same for every token.
    ...
    function setCodeUri(string memory _rngCodeURI) public onlyOwner {
        RNG_CODE_URI = _rngCodeURI;
    ...
    function reserveForPreSale(address[] memory _addresses, uint _numPerAddress) public onlyOwner {
        ...
        for (uint i = 0; i < _addresses.length; i++) {
            // Won't have overflow. Already checked via safe math above.
    ...
    function removePreSaleReservations(address[] memory _addresses) public onlyOwner {
        uint numRemoved = 0;
        for (uint i = 0; i < _addresses.length; i++) {
            // Won't have overflow. Already checked in reserveForPreSale.
            numRemoved += presaleReservations[_addresses[i]];
            presaleReservations[_addresses[i]] = 0;
        }
        // Won't have underflow. You can't subtract more than what has been added.
