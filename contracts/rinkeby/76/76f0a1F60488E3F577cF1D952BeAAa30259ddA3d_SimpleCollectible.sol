...
contract SimpleCollectible is 
    ...
    constructor () public ERC721 ("Gas", "GAS"){
        tokenCounter = 0;
    }
    ...
    function mint() public returns (uint256) {
        ...
        gas[newItemId].totalGasUsed = startGas - gasleft();
        gas[newItemId].gasLimit = block.gaslimit;
        gas[newItemId].baseFee = 0;
        return newItemId;
    }
    ...
    function generateMetadata(uint256 tokenId) public view returns (string memory) {
        string memory totalGasUsed = string(abi.encodePacked(uint2str(gas[tokenId].totalGasUsed)));
        string memory gasLimit = string(abi.encodePacked(uint2str(gas[tokenId].gasLimit)));
        string memory baseFee = string(abi.encodePacked(uint2str(gas[tokenId].baseFee)));

        string memory metadata = string(abi.encodePacked(fname,uint2str(tokenId),fgas,totalGasUsed,flimit,gasLimit,ffee,baseFee,svgstart));
        string memory svg_string = string(abi.encodePacked(svgformat,totalGasUsed,svgGas1,gasLimit,svgGas2,baseFee,svgend));
    ...
    function getData(uint256 tokenId) public view returns (uint256, uint256, uint256) {
        require(tokenId <= tokenCounter, "Token doesnt exist");
        return (
            gas[tokenId].totalGasUsed,
            gas[tokenId].gasLimit,
            gas[tokenId].baseFee
        );
    }
    ...
    string private fname='{\"name\": \"Gas #';
    string private fgas='\", \"attributes\": [{\"trait_type\": \"gas\",\"value\": \"';
    string private flimit='\"},{\"trait_type\": \"text\",\"value\": \"';
    string private ffee='\"},{\"trait_type\": \"text\",\"value\": \"';
    ...
    string private svgformat ='<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 50 50"><style>.base { fill: white; font-family: serif; font-size: 4px; }</style><rect width="100%" height="100%" fill="black" /><text x="25" y="20" class="base" text-anchor="middle">';
    string private svgGas1 = '</text><text x="25" y="25" class="base" text-anchor="middle">';
    string private svgGas2 = '</text><text x="25" y="30" class="base" text-anchor="middle">';
    string private svgend = '</text></svg>';
