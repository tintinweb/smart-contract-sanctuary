...
contract FLUF is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function setImmutableIPFSBucket(string memory immutableIPFSBucket_) public onlyOwner {
        require(bytes(_immutableIPFSBucket).length == 0, "Bucket must not be set");
    ...
    function changeNameAndDescription(uint256 tokenId, string memory newName, string memory newDescription) public payable {
        ...
        require(
            _msgSender() == owner,
            "Must be owner of NFT to rename"
        );

        uint256 amountPaid = msg.value;

        if(_nameChanged[tokenId]) {
            require(amountPaid == RENAME_PRICE, "Must pay to rename a named FLUF");
        } else {
            require(amountPaid == 0, "Dont have to pay to rename it first time");
    ...
    function mintFluf(address human, uint256 amountOfFlufs) public nonReentrant payable virtual returns (uint256) {
        ...
        require(_state != State.Setup, "Cant mint FLUFs during setup");
        require(amountOfFlufs <= MAX_PURCHASE, "Exceed transcation purchase limit");

        require(totalSupply().add(amountOfFlufs) <= MAX_FLUFS, "Purchase would exceed max supply of FLUFs");
        require(FLUF_PRICE.mul(amountOfFlufs) <= msg.value, "Ether value sent is not correct");


        if(_state == State.PreParty) {
            require(_authorised[human] >= amountOfFlufs, "Address not authorised for this many flufs during PreParty");
