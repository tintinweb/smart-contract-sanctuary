...
contract Finan  is Ownable{
    ...
    function send (uint _depositerId) external checkSend(_depositerId) returns (uint){
        /* 收益+本金 */
        uint sum;
        if(keccak256(abi.encodePacked(signCycle[signDeposit[depositId].cycleId].name)) == keccak256(abi.encodePacked("KBKC"))){
        ...
        if(keccak256(abi.encodePacked(signCycle[signDeposit[depositId].cycleId].name)) == keccak256(abi.encodePacked("ETH/KBKC"))){
    ...
    function getDepositDetail(uint _id) public view returns (address,uint,uint,uint,uint){
      return (signDeposit[_id].Depositer,signDeposit[_id].amount,signDeposit[_id].createTime,signDeposit[_id].cycleId,signDeposit[_id].sendTime);
    }
