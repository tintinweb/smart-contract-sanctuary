...
contract Kyoko is OwnableUpgradeable, ERC721HolderUpgradeable, IKyoko, KyokoStorage {
    ...
    function initialize(BorrowToken _bToken, LenderToken _lToken) public initializer {
        __Ownable_init();
        bToken = _bToken;
        lToken = _lToken;
        fee = 500;
    ...
    modifier checkCollateralStatus(uint256 _bTokenId) {
        DataTypes.NFT storage _nft = NftMap[_bTokenId];
    ...
    function setFee(uint256 _fee) external override WhenNotPaused onlyOwner {
        fee = _fee;
        emit SetFee(_fee);
    }

    function addWhiteList(address _address) external override WhenNotPaused onlyOwner {
    ...
    function deposit(
    ...
    ) external override WhenNotPaused checkWhiteList(_erc20Token) returns (uint256) {
        ...
        uint256 _bTokenId = bToken.mint(msg.sender); // mint bToken
        {
            ...
            // set collateral info
            NftMap[_bTokenId] = DataTypes.NFT({
                holder: msg.sender,
                nftId: _nftId,
                nftAdr: _nftAdr,
                bTokenId: _bTokenId,
    ...
    function modify(
        ...
        address _erc20Token,
        string memory _description
    ) external override WhenNotPaused checkWhiteList(_erc20Token) {
        DataTypes.NFT storage _nft = NftMap[_bTokenId];
        require(
            bToken.ownerOf(_bTokenId) == msg.sender && !_nft.getBorrow(),
            "Not bToken owner"
        );
    ...
    function cancelOffer(uint256 _bTokenId, uint256 _lTokenId)
        ...
        WhenNotPaused
    {
        DataTypes.OFFER storage _offer = OfferMap[_bTokenId][_lTokenId];
        ...
        IERC20Upgradeable(_offer.erc20Token).safeTransfer(msg.sender, _price);
        _offer.cancel = true;
        LTokenMapOffer[_lTokenId] = _offer;
        OfferMap[_bTokenId][_lTokenId] = _offer;
     ...
     */
    function lend(uint256 _bTokenId) external override WhenNotPaused {
        DataTypes.NFT storage _nft = NftMap[_bTokenId];
        require(!_nft.getBorrow(), "This collateral already borrowed");
    ...
    function repay(uint256 _bTokenId, uint256 _amount) external override {
        DataTypes.NFT storage _nft = NftMap[_bTokenId];
    ...
    function claimCollateral(uint256 _bTokenId) external override {
        DataTypes.NFT storage _nft = NftMap[_bTokenId];
        require(
    ...
    function claimERC20(uint256 _lTokenId) external override {
        ...
        returns (uint256 repayAmount)
    {
        ...
        DataTypes.NFT memory _nft = NftMap[_bTokenId];
    ...
    function getState(uint256 _btokenId) external view override returns (bool, bool, bool, bool) {
        DataTypes.NFT storage _nft = NftMap[_btokenId];
...
contract KyokoStorage {
    ...
    uint256 public constant yearSeconds = 365 days; // Seconds per year

    uint256 public fee; // admin fee

    address[] public whiteList; // whiteList

    mapping(uint256 => DataTypes.NFT) public NftMap; // uint256 === btokenId(btokenId is a unique identifier)
    mapping(uint256 => mapping(uint256 => DataTypes.OFFER)) public OfferMap; //btokenId => ltokenId => Bidder(Only one offer can exist per person)
    mapping(uint256 => DataTypes.OFFER) public LTokenMapOffer; // ltokenid => Offer (The token holder unilaterally searches for offer information
...
interface IKyoko {
    ...
    function cancelOffer(uint256 _bTokenId, uint256 _lTokenId) external;

    function acceptOffer(uint256 _bTokenId, uint256 _lTokenId) external;

    function lend(uint256 _bTokenId) external;
    ...
    function executeEmergency(uint256 _bTokenId) external;

    function liquidate(uint256 _bTokenId) external;

    function getState(uint256 _btokenId) external view returns (bool, bool, bool, bool);
}
...
library Configuration {
    ...
    function setBorrow(DataTypes.NFT memory self, bool active) internal pure {
        self.marks =
            (self.marks & BORROW_MASK) |
            (uint8(active ? 1 : 0) << IS_BORROW_START_BIT_POSITION);
    }

    function setRepay(DataTypes.NFT memory self, bool active) internal pure {
        self.marks =
            (self.marks & REPAY_MASK) |
            (uint8(active ? 1 : 0) << IS_REPAY_START_BIT_POSITION);
    }

    function setWithdraw(DataTypes.NFT memory self, bool active) internal pure {
        self.marks =
            (self.marks & WITHDRAW_MASK) |
            (uint8(active ? 1 : 0) << IS_WITHDRAW_START_BIT_POSITION);
    }

    function setLiquidate(DataTypes.NFT memory self, bool active) internal pure {
        self.marks =
            (self.marks & LIQUIDATE_MASK) |
            (uint8(active ? 1 : 0) << IS_LIQUIDATE_START_BIT_POSITION);
    }

    function getBorrow(DataTypes.NFT storage self) internal view returns (bool) {
    ...
    function getWithdraw(DataTypes.NFT storage self) internal view returns (bool) {
        return self.marks & ~WITHDRAW_MASK != 0;
    }

    function getLiquidate(DataTypes.NFT storage self) internal view returns (bool) {
        return self.marks & ~LIQUIDATE_MASK != 0;
    }

    function getState(DataTypes.NFT storage self) internal view returns (bool, bool, bool, bool) {
        return (
