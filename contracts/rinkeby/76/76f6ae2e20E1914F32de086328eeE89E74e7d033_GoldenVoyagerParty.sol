...
contract GoldenVoyagerParty is Delegated, ERC721EnumerableB, PaymentSplitter {
  ...
  uint public personalizePrice = 0;
  mapping(uint => TokenData) personalized;
  ...
  function personalize( uint tokenId, string calldata name, string calldata description, string calldata story ) external payable {
    require( _msgSender() == ownerOf( tokenId ) || _delegates[ _msgSender() ], "GVP: Not authorized" );
    require( msg.value > personalizePrice, "Not enough ether sent" );
