...
contract Vault is IERC20, ReentrancyGuard, Ownable {
	...
	function deposit(uint256 _amount, address recipient) public nonReentrant returns (uint256) {
		...
		if (amount == type(uint256).max)
			amount = token.balanceOf(msg.sender).min(depositLimit - _totalAssets());
			// Ensure deposit limit is respected
		else require(_totalAssets() + amount <= depositLimit);
	...
	function withdraw(uint256 shares, address recipient) external nonReentrant returns (uint256) {
		...
		token.transfer(recipient, (shares * 10**token.decimals()) / (10**decimals));
	...
	function availableDepositLimit() external view returns (uint256) {
		if (depositLimit > _totalAssets()) return depositLimit - _totalAssets();
		else return 0;
	}
