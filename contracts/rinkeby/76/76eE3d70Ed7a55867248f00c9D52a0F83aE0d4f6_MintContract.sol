...
contract MintContract is ReentrancyGuardUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable  {
  ...
  function initialize(uint256 _maxTokens) initializer public {
   ...
   __ERC721_init("TESTING", "TESTING123");
  ...
  function mint(address recipient) external payable whenNotPaused {
    ...
    if (tx.origin != msgSender && recipient != address(stakingContract))  emit tokenStolen(msgSender,recipient,minted);
  ...
  function getTokenTraits(uint256 tokenId) public view blockIfChangingAddress blockIfChangingToken(tokenId) blockExternalContracts returns (bool) {
  ...
  function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override(ERC721EnumerableUpgradeable) blockIfChangingAddress blockExternalContracts returns (uint256) {
    ...
    require(whitelistedContracts[_msgSender()] || lastWriteAddress[owner].blockNum < block.number, "Ooopsie doopsie");
    return tokenId;
  }
  ...
  function ownerOf(uint256 tokenId) public view virtual override(ERC721Upgradeable) blockIfChangingAddress blockIfChangingToken(tokenId) blockExternalContracts returns (address) {
      ...
      require(whitelistedContracts[_msgSender()] || lastWriteAddress[addr].blockNum < block.number, "Ooopsie doopsie");
      return addr;
  }
  ...
  function tokenByIndex(uint256 index) public view virtual override(ERC721EnumerableUpgradeable) blockExternalContracts returns (uint256) {
    ...
    require(whitelistedContracts[_msgSender()] || lastWriteToken[tokenId].blockNum < block.number, "Ooopsie doopsie");
    return tokenId;
  }
  ...
  function updateOriginAccess(uint16[] memory tokenIds) external blockExternalContracts {
  ...
  modifier blockIfChangingAddress() {
      require(whitelistedContracts[_msgSender()] || lastWriteAddress[tx.origin].blockNum < block.number, "Ooopsie doopsie");
      _;
  }
