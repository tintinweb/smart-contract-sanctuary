...
contract FormaArt is ERC721Enumerable {
    ...
    modifier onlyEditors() {
        require(
            msg.sender == adminAddress || msg.sender == artistAddress,
            "Only admin"
        );
        _;
    }
    ...
    function setArtist(address _artistAddress) public onlyAdmin {
        require(!locked, "Only unlocked");
        artistAddress = _artistAddress;
    }

    function updateScript(string memory _script, string memory _scriptType)
        public
        onlyEditors
    {
        require(!locked, "Only unlocked");
    ...
    function mint() public payable returns (uint256 _tokenId) {
        ...
        require(totalSupply() < maxTokens, "Must not exceed max tokens");
    ...
    function _mintToken(address _toAddress)
        internal
        returns (uint256 _tokenId)
    {
    ...
    function _splitFunds() internal {
        ...
        uint256 artistPayment = pricePerToken - formaAmount;
        if (artistPayment > 0) {
            payable(artistAddress).transfer(artistPayment);
        }
    }

    function updateFormaAddress(address _formaAddress) public onlyAdmin {
