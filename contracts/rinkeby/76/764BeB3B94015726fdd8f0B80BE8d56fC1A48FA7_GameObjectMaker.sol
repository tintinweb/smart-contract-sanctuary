...
contract GameObjectMaker is ERC1155, Ownable {
    ...
    constructor(address _tokenAddress, address _sodaAddress) ERC1155("") {
        needSoda = true;
        paymentToken = ERC20(_tokenAddress);
        soda = ERC721(_sodaAddress);
        metadataPath = "https://danger-island-test-metadata.s3.amazonaws.com/json/";
    ...
    function mintForWhitelist() public {
        require(gameObjects[0].isSaleClosed == false, "Sale is closed");
        require(whitelist[msg.sender] == true, "sender is not on whitelist");
        require(
            whitelistUsed[msg.sender] == false,
            "sender has already used whitelist"
        );
        whitelistUsed[msg.sender] = true;

        uint8 landCount = 10;
        if (whitelistMultiples[msg.sender] > 0) {
            landCount = landCount * whitelistMultiples[msg.sender];
        }

        // assumes common land has id 0
        _mint(msg.sender, 0, landCount, "");
    }

    function mintForEther(uint256 gameObjectId, uint256 amount)
        external
        payable
    {
        require(
            gameObjects[gameObjectId].isSaleClosed == false,
            "Sale is closed"
        );
        if (needSoda) {
        ...
        require(
            gameObjects[gameObjectId].paidWithToken == false,
            "MintForEther: can't use ether to pay for this gameObject"
        );
        require(
            gameObjects[gameObjectId].isSaleClosed == false,
            "MintForEther: Sale is closed"
        );

        require(
            gameObjects[gameObjectId].totalSupply + amount <=
                gameObjects[gameObjectId].maxSupply,
            "MintForEther: Max supply reached"
        );
        require(
            msg.value == amount * gameObjects[gameObjectId].mintPrice,
            "Purchase: Incorrect payment"
        );

        gameObjects[gameObjectId].totalSupply += amount;
        _mint(msg.sender, gameObjectId, amount, "");
    }
    ...
    function mintForToken(
        ...
        require(
            exists(gameObjectId),
            "MintForToken: gameObject does not exist"
        );

        require(
            gameObjects[gameObjectId].paidWithToken == true,
            "MintForToken: can't use token to pay for this gameObject"
        );
        require(
            gameObjects[gameObjectId].isSaleClosed == false,
            "MintForToken: Sale is closed"
        );

        require(
            gameObjects[gameObjectId].totalSupply + gameObjectAmount <=
                gameObjects[gameObjectId].maxSupply,
            "MintForToken: Max supply reached"
        );
        require(
            paymentAmount ==
                gameObjectAmount * gameObjects[gameObjectId].mintPrice,
            "Purchase: Incorrect payment"
        );
        ...
        gameObjects[gameObjectId].totalSupply += gameObjectAmount;
        _mint(msg.sender, gameObjectId, gameObjectAmount, "");
    }
