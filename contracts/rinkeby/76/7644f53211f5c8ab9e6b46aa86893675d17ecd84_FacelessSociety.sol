...
contract FacelessSociety is ERC721Enumerable, Ownable, OffchainProof {
    ...
    uint256 public constant RESERVED_ASSETS = 1000;
    ...
    constructor(string memory baseURI) ERC721("Faceless Society", "TEST"){
        setBaseURI(baseURI);
        _safeMint(msg.sender, 1);
        
        // wallet snapshot root hex 
    ...
    function mintTo(address addr, uint noTokens) public onlyOwner {
    ...
    function batchMint(uint noTokens) public payable {
        uint availableSupply = MAX_SUPPLY.sub(RESERVED_ASSETS).sub(totalSupply());
        require(saleIsActive, "Sale is not active.");
        require(noTokens <= MAX_MINT, "Number of tokens exceeds per transaction limit");
        require(noTokens <= availableSupply, "Purchase would exceed max supply");
