...
contract Paytr{
    ...
    event PaymentInfo(uint256 amount, bytes indexed paymentReference, address supplier );
    event PayoutInfo(address sender, uint256 amount, bytes indexed paymentReference, uint dueDate, address supplier);
   ...
   constructor() {
    ...
    function transferAndSupplyErc20ToCompound (
        ...
        emit PaymentInfoErc20(_erc20Contract, supplier, amount, paymentReference);
    ...
    function createDueListAndPay() public payable {
        ...
        for (uint i = 0; i < invoices.length; i++) {
             ...
             if (block.timestamp >= invoices[i].dueDate) {
                ...
                dueInvoice memory dueList = dueInvoice(amount, sender, supplier, dueDate, paymentReference);
        ...
        for (uint i = 0; i < dueInvoices.length; i++) {
            if (dueInvoices[i].supplier != 0x0000000000000000000000000000000000000000) {
                ...
                //event PayoutInfo(address sender, uint256 amount, bytes indexed paymentReference, uint dueDate, address supplier);
                emit PayoutInfo(dueInvoices[i].sender, dueInvoices[i].amount, dueInvoices[i].paymentReference, dueInvoices[i].dueDate, dueInvoices[i].supplier );
    ...
    function createDueListAndPayErc20() public payable {
        ...
        for (uint i = 0; i < invoicesErc20.length; i++) {
             ...
             if (block.timestamp >= invoicesErc20[i].dueDate) {
                ...
                tokenAddress = invoicesErc20[i].tokenAddress;
                
                dueInvoiceErc20 memory dueListErc20 = dueInvoiceErc20(amount, sender, supplier, dueDate, paymentReference, tokenAddress);
