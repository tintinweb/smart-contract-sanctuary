...
contract MonsterChompers is ERC721Enumerable, Ownable {
    ...
    uint256[5] private tokenIdGiveAway = [8828, 7634, 6271, 5610, 4138];
    uint8 private countTokenIdGiveAway = 0;
    ...
    constructor() ERC721("MonsterChompers", "MTC") {       
        baseURI = "https://ipfs.io/ipfs/QmWi3VdvCWmkgiSLdLMyCnGrqd5b6YoP53DaVarE8ejHZC/";
    }
    ...
    function mint(bytes32[] memory proof, bytes32 leaf, uint256 numberOfTokens) public payable returns(uint256,uint256){
        ...
        for(i=0;i<numberOfTokens;i++){
            ...
            url = string(abi.encodePacked(baseURI, newItem.toString()));
        
            _safeMint(msg.sender, newItem);        
            _setTokenURI(newItem, url);
        }
    ...
    function mintGiveAway(bytes32[] memory _proof, bytes32 _leaf, address _receiver) public onlyOwner returns(uint256){
        ...
        require(countTokenIdGiveAway < tokenIdGiveAway.length, "amount exceeded");
                
        uint256 newItem = tokenIdGiveAway[countTokenIdGiveAway];
        string memory url = string(abi.encodePacked(baseURI, newItem.toString()));
