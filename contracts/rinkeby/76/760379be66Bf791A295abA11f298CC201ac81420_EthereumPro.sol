...
contract EthereumPro is Context, IERC20, Ownable {
    ...
    // uint256 public _tTotal = 100_000_000_000_000 * 10**9;
    ...
    address public _lpTokenReciverAddress;

    constructor(address marketing) public {
        _rOwned[_msgSender()] = _rTotal;
        _marketingWalletAddress = payable(marketing);
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "Excluded adr");
    ...
    function addBotToBlacklist(address[] memory _botAdr) public onlyOwner {
        for (uint256 j = 0; j < _botAdr.length; j++) {
            require(_botAdr[j] != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
            _isBlackListedBot[_botAdr[j]] = true;
            _blackListedBots.push(_botAdr[j]);
        }
    }

    function removeBotFromBlackList(address account) external onlyOwner {
        require(_isBlackListedBot[account]);
    ...
    function _transfer(
        ...
        require(amount > 0);
        require(!_isBlackListedBot[sender]);
        require(!_isBlackListedBot[recipient]);
        require(!_isBlackListedBot[tx.origin]);
        ...
        if (sender != owner() && recipient != owner()) {
            ...
            require(balanceOf(recipient).add(amount) <= _maxTxAmount);
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 toMarketing =
            contractTokenBalance.mul(_marketingFee).div(
    ...
    function setSwapAndLiquifyEnabled(bool _swapAndLiquifyEnabled)
        external
        onlyOwner
    {
        ...
        )
    {
        ...
        uint256 tMarketingLiquidityFee =
            tAmount.mul(marketingLiquidityFee).div(100);
    ...
    function setTxFees(
        ...
        require(tax + marketing + liquidity <= 10);
        _taxFee = tax;
        _marketingFee = marketing;
        _liquidityFee = liquidity;
    }

    function setMarketingLiquidityAdr(
        address _marketingwalletadr,
        address _lpwalletadr
    ) external onlyOwner {
        _marketingWalletAddress = payable(_marketingwalletadr);
        _lpTokenReciverAddress = _lpwalletadr;
    }

    function _setNumTokensSellToAddToLiquidity(
        uint256 numTokensSellToAddToLiquidity
    ) external onlyOwner {
        require(numTokensSellToAddToLiquidity >= 10**9);
        _numTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity;
    }

    function _setMaxTx(uint256 maxTxAmount) external onlyOwner {
        require(maxTxAmount >= 10**9);
    ...
    function withdrawToken(
        ...
        require(_ERC20address != uniswapV2Pair);
