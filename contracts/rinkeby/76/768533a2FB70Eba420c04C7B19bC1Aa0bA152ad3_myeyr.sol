...
contract myeyr is ERC721PresetMinterPauserAutoId, VRFConsumerBase, Data  {
    ...
    uint256 public passPrice = 100 * 10**18; // make this ;
    ...
    constructor(
        ...
        ERC721PresetMinterPauserAutoId("fnafdb", "ntttst", "ipfs://") 
    {
    ...
    // batch minter with custom pass amount and frog color
    function batchMint(address[] memory recipients, uint256 _availableFreeDays, uint256 _frogColor) public onlyOwner {
        uint256 i = 0;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        if (color >= 1 && color <= 40) device.name = names[0]; device.frogColor = 1;
        if (color >= 41 && color <= 70) device.name = names[10]; device.frogColor = 2;
        if (color >= 71 && color <= 85) device.name = names[20]; device.frogColor = 3;
        if (color >= 86 && color <= 95) device.name = names[30]; device.frogColor = 4;
        if (color >= 96 && color <= 100) device.name = names[40]; device.frogColor = 5;
    ...
    function findGeneration(uint256 level) internal pure returns (DeviceState generationLevel_){
        if (level == 0 ) return DeviceState.Gen1;
        if (level == 1 ) return DeviceState.Gen2;
        if (level == 2 ) return DeviceState.Gen3;
        if (level == 3 ) return DeviceState.Gen4;
        if (level == 4 ) return DeviceState.Gen5;
        if (level == 5 ) return DeviceState.Gen6;
        if (level == 6 ) return DeviceState.Gen7;
        if (level == 7 ) return DeviceState.Gen8;
        if (level == 8 ) return DeviceState.Gen9;
        if (level >= 9 ) return DeviceState.Gen10;
    }
    ...
    function generationUp(uint256 tokenId) internal {
        ...
        uint256 maxPlaysPerGen; // 4,6,8,10 ect..
        uint256 amount;
        
        // if !prestige 16, calculate reward. Else on prestige 16+, rewards set to 1 indefinitely and 2 plays per gen
        ...
        } else { maxPlaysPerGen = 2; amount = reward;}
            ...
            if (device.generation < 9) {
                for (uint i = 0; i < 9; i++) {
                    if (device.generation == i) {
                        ...
                        if (device.frogColor == 1) device.name = names[i.add(1)];
                        if (device.frogColor == 2) device.name = names[(i.add(10)).add(1)];
                        if (device.frogColor == 3) device.name = names[(i.add(20)).add(1)];
                        if (device.frogColor == 4) device.name = names[(i.add(30)).add(1)];
                        if (device.frogColor == 5) device.name = names[(i.add(40)).add(1)];
                        // add to local rewards counters
                        device.tokensToRevive = device.tokensToRevive.add(amount);
                        device.rewards = device.rewards.add(amount);
                        // global pending rewards counter
    ...
    function prestige(uint256 tokenId) public {
        ...
        if (device.frogColor == 1) device.name = names[0];
        if (device.frogColor == 2) device.name = names[10];
        if (device.frogColor == 3) device.name = names[20];
        if (device.frogColor == 4) device.name = names[30];
        if (device.frogColor == 5) device.name = names[40];
    ...
    function getOwnersTokenIds(address account) public view returns (uint256[] memory) {
        ...
        uint256[] memory accountsTokens_ = new uint256[](currentBalance);
    ...
    function getTimeData(uint256 tokenId) public view returns (
        ...
        if (device.prestige < MAX_CALC) { maxPlaysPerGen = device.generation.mul(2).add(4);
        } else { maxPlaysPerGen = 2;}
        ...
        if (device.createdAt != MAX_TIMESTAMP && alive(tokenId)) {
            ...
            if (block.timestamp.sub(device.lastPlayTime) > ONE_HOUR.add(device.prestige.mul(ONE_HOUR))) { // in play
                ...
                if (device.feedLevel > fullCycles) { // not in a feed
                    return (
                        true, 0, ONE_HOUR.add(device.prestige.mul(ONE_HOUR)), maxPlaysPerGen,  
                ...
                if (device.feedLevel > fullCycles) {  // not in a feed
                    return (
                        false, ONE_HOUR.add(device.prestige.mul(ONE_HOUR)).sub(elapsed), ONE_HOUR.add(device.prestige.mul(ONE_HOUR)), maxPlaysPerGen, 
                        false, device.feedLevel.mul(ONE_DAY).sub(feedElapsedTime), feedWindow
                    );
