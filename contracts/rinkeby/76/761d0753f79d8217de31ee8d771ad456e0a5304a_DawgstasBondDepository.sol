...
contract DawgstasBondDepository is Ownable {
    ...
    address public immutable Dawgsta; // token given as payment for bond
    ...
    function deposit( 
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( Dawgsta ).safeTransfer( DAO, fee ); 
        }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Dawgsta ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( Dawgsta ).totalSupply();
