...
contract Token is Context, IERC20 {
    ...
    uint256 public constant tradingSplitPercent = 50;
    ...
    struct TaxWallets {
        ...
        address payable trading;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing : payable(0xE2464dE90C0C7D118Bcd1521bAb05006cAD4ED3f),
        trading : payable(0xA2E759801006CB381A07801Be52bb638e2DC8357)
    });
    ...
    uint256 public maxEthBuy = 0.5 ether; //change to 5 * 10**18
    uint256 public maxEthSell = 0.5 ether; //change to 5 * 10**18
    ...
    function intializeContract(address[] memory accounts, uint256[] memory amounts, uint256[] memory amountsD, address _antiSnipe) external payable onlyOwner {
    ...
    function setWallets(address payable marketing, address payable trading) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.trading = payable(trading);
    }
            ...
            if (maxEthTradesEnabled) {
                if (lpPairs[from]) {
                    if (timedLimitsEnabled) {
                        if (block.timestamp <= launch + 30 minutes) {//TODO: change to launch + 2 hours
                            require(_ethBalance <= 0.00025 ether);
                            //TODO: change to 25 * 10**16
                        } else if (block.timestamp <= launch + 22 hours) {
                            require(_ethBalance <= 0.002 ether);
                            //TODO: change to 2 * 10**18
                        } else {
                    ...
                    if (timedLimitsEnabled) {
                        if (block.timestamp <= launch + 2 hours) {
                            require(_ethBalance <= 0.25 ether);
                        } else if (block.timestamp <= launch + 22 hours) {
                            require(_ethBalance <= 2 ether);
                        } else {
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (address(this).balance > 0 && _ratios.total - _ratios.liquidity > 0) {
            uint256 tradingAmount = address(this).balance * tradingSplitPercent / 100;
            _taxWallets.trading.transfer(tradingAmount);
