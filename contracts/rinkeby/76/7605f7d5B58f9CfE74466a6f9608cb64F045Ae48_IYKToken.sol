...
contract IYKToken is ERC721, Ownable {
  using ECDSA for bytes32;

  address signVerifier;
  string baseURI;

  mapping(address => uint256) redeemNonces;
  ...
  function getRedeemNonce(address recipient) external view returns (uint256) {
    return redeemNonces[recipient];
  }

  function getRedeemSigningHash(
    uint256 blockExpiry,
    address recipient,
    uint256 tokenId
  ...
  function redeemNFT(
    ...
    bytes32 message = getRedeemSigningHash(blockExpiry, recipient, tokenId).toEthSignedMessageHash();
    require(
    ...
    require(
      ...
      'Only recipient can submit this transaction'
    );
