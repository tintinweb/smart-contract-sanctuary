...
contract Aphaereses is AphaeresesBase, VRFConsumerBase {
    ...
    constructor(
        ...
        AphaeresesBase(teamAddress, signer)
    {
    ...
    {
        // Not revealed so let's show something fun instead
        if (!revealed) {
            return _unrevealedTokenURI;
        }

        return string(abi.encodePacked(_baseURI(), metadataId(tokenId)));
    }
    ...
    function metadataId(uint256 tokenId) public view returns (uint256) {
        ...
        // If we're not revealed yet, then who cares
        if (!revealed) {
            return tokenId;
        }

        uint256 numberSpecialReserved = (SPECIAL_EDITIONS_RESERVED +
            BRAND_RESERVED);
    ...
    function setRevealed() public onlyOwner isNotRevealed {
        revealed = true;
    }
    ...
    function setTokenOffset(uint256 newOffset) public onlyOwner isNotRevealed {
        _tokenOffset = newOffset;
    }
...
contract AphaeresesBase is ERC721, Ownable, Whitelisted, ReentrancyGuard {
    using Address for address;
    using ECDSA for bytes32;

    uint256 public constant MINT_PRICE = 0.123 ether;
