...
contract Escrow is Migratable {
    ...
    /**
     * @dev Completes the escrow, calculating the amount of PAPR to pay out
    ...
    /**
     * @dev Get the total amount of PAPR to pay out for the job
     * @param _escrowId Id of the escrow to complete
     ...
     * Pay out 100% of the PAPR back to the client
     * @param _escrowId Id of the escrow to complete
     * @return bool success
     */
...
contract PAPRJobs is PAPRJobsCreator {
    ...
    function initialize(ERC20 _token, PAPRJobsAdmin _paprjobsAdmin, address _dApp, address _priceOracle) 
    public 
    isInitializer("PAPRJobs", "0.1.0") {
        require(_token != address(0) && _paprjobsAdmin != address(0) && _dApp != address(0), "Addresses must be valid");

        PAPRJobsCreator.initialize(_token, _paprjobsAdmin, _dApp, _priceOracle);      
...
contract PAPRJobsCreator is Escrow {
    ...
    isInitializer("PAPRJobsCreator", "0.1.0") {
        require(_token != address(0) && _paprjobsAdmin != address(0) && _dApp != address(0) && _priceOracle != address(0));
