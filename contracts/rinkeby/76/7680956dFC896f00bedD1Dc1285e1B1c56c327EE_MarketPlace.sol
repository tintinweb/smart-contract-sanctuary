...
contract ENCNFT is ERC721, Ownable {
    ...
    function getIdsByAddress(address owner) public view returns( uint256[] memory) {
...
contract MarketPlace is Ownable {
    ...
    ENCNFT nftExample = ENCNFT(
        0xd7d366cF18D57c6aFC62Fd16FDbBBab08066988F
        );
    struct BuyerData {
    ...
    event sellerWithdrewSum(uint256 idToken, address sellerAddress, uint256 sum);
    
    constructor ()  { 
        ownwerContract = msg.sender;
    }
    
    function makeBet(uint256 idToken, string memory buyerPublicKey, address buyerAddress, string memory goalPurchase) public payable {
        require(msg.value != 0, "Rate must be more 0");
         ...
         if(buyersBoard[idToken].length > 0 && buyersBoard[idToken][buyerIndex].buyerAddress == buyerAddress) {
             ...
             buyersBoard[idToken].push(BuyerData({buyerAddress: buyerAddress, buyerPublicKey: buyerPublicKey, buyerBet: msg.value, goalOfPurchase: goalPurchase}));
         }
    ...
    function buyerWithdrawBet(uint256 idToken) public {
        ...
        emit BuyerWithdrewBet(idToken, msg.sender, buyersBoard[idToken][buyerIndex].buyerBet);
    ...
    function sellerWithdrawSum(uint256 idToken) public {
        ...
        require(succeed, "Failed to withdraw Ether");
        emit sellerWithdrewSum(idToken, msg.sender, sellersBoard[idToken][sellerIndex].sellerSum);
    ...
    function getOwnerInfo(uint256 _tokenId, address owner) public view returns( string memory balance) {
         SellerData[] storage allHistorySellersInfo = sellersBoard[
            _tokenId
        ];
        ...
        for(uint256 ind; ind < historyLength; ind++ ) {
            if(allHistorySellersInfo[ind].sellerAddress == owner) {
                balance = string(abi.encodePacked(balance, uint2str(allHistorySellersInfo[ind].sellerSum)));
            }
        }
        return balance;
    }
