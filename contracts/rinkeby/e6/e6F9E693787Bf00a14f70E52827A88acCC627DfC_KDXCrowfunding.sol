...
import "./interfaces/IKDXFactory.sol";

contract KDXCrowfunding is IKDXFactory, Ownable, ReentrancyGuard {
    ...
    /**
     *
     * @dev kdxPools store all investment info on this contract.
     *
     */
    mapping(address => InvestorInfo) public kdxPools;

    /**
     *
     * @dev kdxIndices store all indicate numbers for kdxPools
     *
     */
    address[] public kdxIndices;

    /**
     *
     * @dev this variable is the instance of active KDX token
     *
     */
    IERC20 private _kdxToken;

    /**
     *
     * @dev this variable is for locking date of the pool
     *
     */
    uint256 public _poolLockDate;

    modifier requireKDX(address _wallet) {
        /** check if investor has any KDX token */
        require(
            _kdxToken.balanceOf(address(_wallet)) > 0,
    ...
    constructor() public {
        _poolLockDate = block.timestamp;
    }

    /**
     *
     * @dev set KDX token address for contract
     *
     * @param {_token} address of IERC20 instance
     * @return {bool} return status of token address
     *
     */
    function setKDXToken(IERC20 _token)
        external
        override
        onlyOwner
        returns (bool)
    ...
    /**
     *
     * @dev get function for deployed KDX token address
     *
     * @return {address} return deployment address of KDX token
     *
     */
    function getKDXToken() public view override returns (address) {
        return address(_kdxToken);
    }

    /**
     *
     * @dev add individual tokens (IERC20) to use for our KDX.
     * ** possibly, we will support investors to invest funds with different crypto assets.
     * ** but for now, this will be used as boilerplate as we are going to use KDX token for transaction.
     *
     * @param {IERC20} token instance which is going to add
     * @param {uint256} available date
     * @param {uint256} expired date
     * @param {uint256} rate compared to our KDX
     *
     * @return {bool} return if token is successfully add or not
     *
     */
    function addTokenToKDX(
        IERC20 _token,
        uint256 _startDate,
        uint256 _endDate,
        uint256 _rate
     ...
     *
     */
    function removeTokenFromKDX(address _token)
        external
        override
        requireToken(_token)
        onlyOwner
     ...
     *
     */
    function getTokenFromKDX(address _token)
        external
        view
        requireToken(_token)
    ...
    /**
     *
     * @dev let investors to join contract, deposit stable coins
     *
     * @param {uint256} join date
     * @param {uint256} invest amount
     * @param {address} stable coin
     *
    ...
    function joinKDX(
        ...
        returns (bool)
    {
        /** check if user already invested for this contract */
        require(
            kdxPools[msg.sender].wallet != msg.sender,
        ...
        require(
            _investFund > 0 &&
                ...
                _investFund,
            "need to have enough funds to invest"
        );

        /** add new investor to investor list for specific agreeement */
        kdxPools[msg.sender].joinDate = block.timestamp;
        kdxPools[msg.sender].wallet = msg.sender;
        kdxPools[msg.sender].investAmount = _investFund;

        /** rate should not be decimal, so we use to multplay 10**6 as input with SafeMath */
        /** e.g: if rate is 0.6, input should 0.6 * 10^6 */
        kdxPools[msg.sender].kdxAmount = _investFund
            .mul(tokenPools[_token].rate)
            .div(10**6);
        kdxPools[msg.sender].active = true;

        /** 
            transfer custom token from investor to treasury
        */
    ...
    /**
     *
     * @dev set the locking time of the pool
     *
     * @return {bool} return status
     *
     */
    function setLockTime(uint256 _newLock)
        external
        override
        onlyOwner
        returns (bool)
    {
        _poolLockDate = _newLock;

        emit UpdateLockTime(_newLock);
    ...
    /**
     *
     * @dev claim fund for distribute function
     *
     * @return {bool} return status of claim
     *
     */
    function claimInvestorFund()
        external
        payable
        override
        nonReentrant
        returns (bool)
    {
        ...
        /** check if treasury have enough KDX to distribute */
        require(
            _kdxToken.balanceOf(address(this)) > kdxPools[msg.sender].kdxAmount,
            "need to have enough KDX in treasury"
        );

        /** check if pool is locked or not */
        require(block.timestamp > _poolLockDate, "token is locked");

        /** 
            transfer token from treasury to innovator
        */
        _kdxToken.transferFrom(
            address(this),
            msg.sender,
            kdxPools[msg.sender].kdxAmount
        );

        emit ClaimFund(msg.sender);
...
interface IKDXFactory {
    /**
     *
     * @dev this event will call when new token added to the contract
     * currently, we are supporting KDX token and this will be used for future implementation
     *
     */
    ...
    /**
     *
     * @dev it is calling when new investor joinning to contract
     *
     */
    ...
    /**
     *
     * @dev this event is called when investor claim for distribution
     *
     */
    event ClaimFund(address wallet);

    /**
     *
     * @dev this event is called when owner update the locking time
     *
     */
    event UpdateLockTime(uint256 date);
    ...
    function setKDXToken(IERC20 _token) external returns (bool);

    function getKDXToken() external view returns (address);
    ...
    function removeTokenFromKDX(address _token) external returns (bool);
    ...
    function claimInvestorFund() external payable returns (bool);

    function setLockTime(uint256 _newLock) external returns (bool);
}
