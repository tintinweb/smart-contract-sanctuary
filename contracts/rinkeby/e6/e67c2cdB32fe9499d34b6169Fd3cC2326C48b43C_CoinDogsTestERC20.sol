...
contract CoinDogsMixed is Context, Ownable{
 ...
 /*   function putAccessoryOnDog( uint256 dogId, uint256 accessoryId) external {
        require(dogs.ownerOf(dogId) == _msgSender(), "Illegal dog owner");
        uint256 total_accessories = accessories.balanceOf( _msgSender(), accessoryId ) ;
        require(total_accessories > 0, "Illegal accessory owner");
        uint256 reserve = total_accessories - used_accessories[_msgSender()][accessoryId];
        require( reserve > 0 && reserve <= total_accessories,"No Accessory available");
        weared_accessories[dogId][accessoryId] +=1;
        used_accessories[_msgSender()][accessoryId] +=1;
        emit putOn(dogId, accessoryId);
    }
    
    function putAccessoryOffDog( uint256 dogId, uint256 accessoryId) external {
        require(dogs.ownerOf(dogId) == _msgSender(), "Illegal dog owner");
        require(weared_accessories[dogId][accessoryId] > 0, "Illegal accessory on dog");
        weared_accessories[dogId][accessoryId] -=1;
        used_accessories[_msgSender()][accessoryId] -=1;
        emit putOff(dogId, accessoryId);
    }
    function getDogAccessories(uint256 id) external returns(uint256[] memory, uint256[] memory){
    
    }

    event putOn(uint256 dogId, uint256 accessoryId);
    event putOff(uint256 dogId, uint256 accessoryId);
    */
...
contract CoinDogsAccessories is ERC1155, Ownable
{
    using Strings for string;
