    ...
    SplitForwarderFactory
{
    ...
    /**
     * @dev Mapping from owner to approval for all verified operators
     */
    mapping(address => bool) private approvedForVerifiedOperator;

    /**
     * @dev Verified operator address (e.g. verified marketplaces)
     */
    address private verifiedOperator;

    /**
     * @dev Emitted when `approved` enables or disables approval for verified contracts to manage all of its assets.
     */
    event ApproveVerifiedForAll(address indexed approved, bool isApproved);
     ...
     * Also sets `MAX_SUPPLY` and verifiedOperators
     */
    constructor(
        string memory _name,
    ...
    function _mint(
        ...
        require(MAX_SUPPLY == 0 || tokenIds.current() < MAX_SUPPLY, 'minted too many tokens');
        tokenIds.increment();
    ...
    function airdropMintToCreator(
        ...
        require(
            ...
            'airdropMintToCreator: MINT_WITH_CREATOR or DEFAULT_ADMIN_ROLE'
        );
        address paymentAddress = _msgSender();
        if (_merkleRoot != 0) {
            require(
                _paymentAddress == address(0),
                'mintWithParams: cannot set payment address and split at the same time'
            );
            paymentAddress = createSplitForwarder(_merkleRoot);
        } else if (_paymentAddress != address(0)) {
            paymentAddress = _paymentAddress;
        } else {
            paymentAddress = _creatorAddress;
        }
        approvedForVerifiedOperator[_creatorAddress] = true;
        newTokenId = _mint(
            _tokenURI,
    ...
    /**
     * @dev Public function that allows admin to mint a token with custom parameters
     * @param _tokenURI uint256 metadata URI of the token.
     * @param _merkleRoot bytes32 merkle root to create a split for
     ...
     * @param _approveVerifiedOperators bool if true, this and future tokens minted on this contract will be approved for all verified operators addresses
     */
    function mintWithParams(
        ...
        address payable _paymentAddress,
        uint32 _tokenRoyalty,
        bool _approveVerifiedOperators
    ) public returns (uint256 newTokenId) {
        require(
            hasRole(MINTER_ROLE, _msgSender()) || hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            'mintWithParams: must have MINTER_ROLE or DEFAULT_ADMIN_ROLE'
        );
        address paymentAddress = _msgSender();
        if (_merkleRoot != 0) {
            require(
        ...
        if (_approveVerifiedOperators) {
            setApprovalForAll(verifiedOperator, true);
        }
    }

    /**
     * @dev Uses ERC721 _safeTransfer, but opens access to a set list of verified contracts approved by token owners
     */
    function safeTransferFrom(
        ...
        require(
            _isApprovedOrOwnerOrVerified(_msgSender(), tokenId),
     ...
     */
    function _isApprovedOrOwnerOrVerified(address operator, uint256 tokenId)
        internal
        view
        returns (bool)
    {
        ...
        return (operator == tokenOwner ||
            ...
            (approvedForVerifiedOperator[tokenOwner] && verifiedOperator == operator));
    }

    /**
     * @dev sets whether the tokens of `tokenOwner` are transferrable by the verified addresses on this contract
     * @param tokenOwner address to approve/revoke transfer rights for
     * @param approved bool approval status
     */
    function setApprovalForVerifiedOperators(address tokenOwner, bool approved) public {
        require(
            tokenOwner == _msgSender() || hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            'FirstDibsTokenV2: sender must be token owner or admin'
        );
        approvedForVerifiedOperator[tokenOwner] = approved;
        emit ApproveVerifiedForAll(tokenOwner, approved);
    }

    /**
     * @dev See {IERC721-isApprovedForAll}, using separate set of approval addresses
     */
    function isApprovedForVerifiedOperators(address tokenOwner) public view returns (bool) {
        return approvedForVerifiedOperator[tokenOwner];
    }

    function setVerifiedOperator(address _verifiedOperator) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            'setVerifiedOperator: must be admin to set verified operator'
        );
        verifiedOperator = _verifiedOperator;
    }
