...
contract AniftyMarketplace is ReentrancyGuard, IBaseExchange, ERC1155Holder {
    ...
    // Pause
    mapping(PauseType => bool) public isPaused;
    ...
    function offerCancel(Orders.Offer memory offer) external nonReentrant {
        require(!isPaused[PauseType.OfferCancel], "ANIFTY: PAUSED");
        require(offer.signer == msg.sender, "ANIFTY: FORBIDDEN");
        bytes32 hash = offer.hash();
        require(!isCancelled[hash], "ANIFTY: CANCELLED");
        isCancelled[hash] = true;

        emit OfferCancel(hash, offer);
    }
    ...
    function primaryCancel(Orders.MintAsk memory askOrder)
        ...
        nonReentrant
    {
        require(!isPaused[PauseType.PrimaryCancel], "ANIFTY: PAUSED");
        ...
        emit PrimaryCancel(hash, askOrder);
    }

    function secondaryCancel(Orders.Ask memory askOrder) external nonReentrant {
        require(!isPaused[PauseType.SecondaryCancel], "ANIFTY: PAUSED");
        require(askOrder.signer == msg.sender, "ANIFTY: FORBIDDEN");
        bytes32 hash = askOrder.hash();
        require(!isCancelled[hash], "ANIFTY: CANCELLED");
        isCancelled[hash] = true;

        emit SecondaryCancel(hash, askOrder);
    }
    ...
    function askUpdate(
        ...
        require(!isPaused[PauseType.AskUpdate], "ANIFTY: PAUSED");
        bytes32 prevHash = prevAskOrder.hash();
        bytes32 hash = askOrder.hash();
        require(askOrder.signer == msg.sender, "ANIFTY: INVALID_CALLER");
        require(askOrder.signer == prevAskOrder.signer, "ANIFTY: SIGNER");
        require(askOrder.askType == prevAskOrder.askType, "ANIFTY: TYPE");
        require(askOrder.nonce != prevAskOrder.nonce, "ANIFTY: NONCE");
        require(!isCancelled[prevHash], "ANIFTY: CANCELLED");
        require(mintedHash[hash] == 0, "ANIFTY: INVALID_ASK");
        require(askOrder.amount > amountFilled[prevHash], "ANIFTY: AMOUNT_LOW");
        ...
        if (mintedHash[prevHash] > 0) {
            require(
                askOrder.amount <= prevAskOrder.amount,
                "ANIFTY: AMOUNT_HIGH"
            );
            require(
                askOrder.royalty.recipients.length ==
                    prevAskOrder.royalty.recipients.length,
                "ANIFTY: RECIPIENTS_LENGTH"
            );
            require(
                askOrder.royalty.royalties.length ==
                    prevAskOrder.royalty.royalties.length,
                "ANIFTY: ROYALTIES_LENGTH"
            );
            for (uint256 i = 0; i < askOrder.royalty.recipients.length; i++) {
                require(
                    askOrder.royalty.recipients[i] ==
                        prevAskOrder.royalty.recipients[i],
                    "ANIFTY: RECIPIENTS"
                );
                require(
                    askOrder.royalty.royalties[i] ==
                        prevAskOrder.royalty.royalties[i],
                    "ANIFTY: ROYALTIES"
                );
            }
    ...
    function primaryBuy(
        ...
        Orders.Token memory token,
        uint256 amount
    ) external payable nonReentrant {
        require(!isPaused[PauseType.PrimaryBuy], "ANIFTY: PAUSED");
        ...
        require(askHash == token.askHash, "ANIFTY: UNMATCHED_HASH");
        require(
        ...
        _verify(token.hash(), operator, token.v, token.r, token.s);
        _verify(
            commission.hash(),
            operator,
            commission.v,
    ...
    function primaryClaim(
        ...
        require(!isPaused[PauseType.PrimaryClaim], "ANIFTY: PAUSED");
        bytes32 askHash = askOrder.hash();
    ...
    function secondaryBuy(
        ...
        require(!isPaused[PauseType.SecondaryBuy], "ANIFTY: PAUSED");
        bytes32 askHash = askOrder.hash();
        ...
        _validateSecondary(askOrder, secondaryRoyalty, askHash);
        _verify(askHash, askOrder.signer, askOrder.v, askOrder.r, askOrder.s);
        _verify(
            commission.hash(),
            operator,
    ...
    function secondaryClaim(
        ...
        require(!isPaused[PauseType.SecondaryClaim], "ANIFTY: PAUSED");
        bytes32 askHash = askOrder.hash();
        bytes32 bidHash = bidOrder.hash();
        bytes32 royaltyHash = secondaryRoyalty.hash();
        require(amountFilled[askHash] == 0, "ANIFTY: SOLD_OUT");
    ...
    function offerClaim(
        ...
        require(!isPaused[PauseType.OfferClaim], "ANIFTY: PAUSED");
        bytes32 offerHash = offer.hash();
        bytes32 royaltyHash = secondaryRoyalty.hash();
        require(offerHash == commission.askHash, "ANIFTY: UNMATCHED_HASH");
        _validateOffer(offer, secondaryRoyalty, offerHash);
    ...
    function _validateRoyalty(
        ...
        if (recipients.length > 0) {
            if (isPrimary) {
                require(totalRoyalty == PRECISION, "ANIFTY: ROYALTY");
            } else {
    ...
    function _transferFeesAndFunds(
        ...
        if (isEth) {
            ...
            require(msg.value == 0, "ANIFTY: PAYMENT");
        }
        ...
        }
        {
            ...
            uint256 newValue = remainder;
            for (uint256 i = 0; i < royaltyInfo.recipients.length; i++) {
                royaltyFeeAmount =
                    ((isPrimary ? newValue : value) *
                        royaltyInfo.royalties[i]) /
                    PRECISION;
                if (isEth) {
    ...
    function setSupportedTokens(address[] memory _supportTokens, bool _set)
        external
    {
        require(admin == msg.sender || owners[msg.sender], "ANIFTY: CALLER");
        for (uint8 i = 0; i < _supportTokens.length; i++) {
    ...
    function setPause(PauseType[] memory _pauseTypes, bool _set) external {
        require(admin == msg.sender || owners[msg.sender], "ANIFTY: CALLER");
        for (uint8 i = 0; i < _pauseTypes.length; i++) {
            isPaused[_pauseTypes[i]] = _set;
        }
    }

    function setOwners(address[] memory _owners, bool _set) external {
        require(admin == msg.sender, "ANIFTY: CALLER");
...
library Orders {
    ...
    // keccak256("Token(bytes32 askHash,string name,string creatorName,string description,string mediaUri)")
    bytes32 internal constant TOKEN_TYPEHASH =
        0x0df24f02ed2f715c9d68b8ba062ea017c23619ea06359b031c15b25ab54c8530;
        ...
        return
            keccak256(
                abi.encode(
                    ...
                    keccak256(bytes(token.creatorName)),
                    keccak256(bytes(token.description)),
                    keccak256(bytes(token.mediaUri))
                )
            );
    }
}
...
interface IBaseExchange {
    event OfferCancel(bytes32 indexed hash, Orders.Offer offer);
    event PrimaryCancel(bytes32 indexed hash, Orders.MintAsk askOrder);
    event SecondaryCancel(bytes32 indexed hash, Orders.Ask askOrder);
    event Update(
        bytes32 indexed prevHash,
        bytes32 indexed hash,
        Orders.MintAsk prevAskOrder,
    ...
    event SecondaryClaim(
        ...
        Orders.Bid bidOrder,
        Orders.Commission commission,
        Orders.SecondaryRoyalty secondaryRoyalty
    );
