...
contract CryptoOlv is ERC721, Ownable {
    ...
    uint256 public constant MAX_RASTA = 50;
    uint256 private constant OFFICIAL_COLABS = 20;
    ...
    constructor(string memory baseURI, string memory contractURI, address accountAddress) ERC721("Crpyto Olv", "OLV") {
        setBaseURI(baseURI);
    ...
    function mint420(uint256 quantity, address receiver) public payable {
        // validação para itens especiais
        require(hasSaleStarted || msg.sender == owner(), "sale hasn't started");
        require(quantity > 0, "quantity cannot be zero");
        require(quantity < 20, "exceeds 20");
        require(totalSupply().add(quantity) + OFFICIAL_COLABS <= MAX_RASTA || msg.sender == owner(), "sold out");
