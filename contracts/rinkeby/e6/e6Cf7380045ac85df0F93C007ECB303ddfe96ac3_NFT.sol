...
contract NFT is ERC721URIStorage, EIP712, INFT {
    string _name;
    string _symbol;
    uint256 private totalAmountOfEdition;
    ...
    bytes32 immutable TRANSFER_TYPEHASH =
        keccak256(
            "Transfer(address addressTo,uint256 tokenId,uint256 expirationTime,bytes32 salt)"
        );

    bytes32 immutable BURN_TYPEHASH =
        keccak256("Burn(uint256 tokenId,uint256 expirationTime,bytes32 salt)");
    ...
    function awardToken(address _user, uint256 _tokenID) public override {
        require(
            accessManager.isAllowed(msg.sender) == true,
        ...
        emit TokenAwarded(_user, _name, _symbol, _tokenID, tokenURI(_tokenID));
...
interface INFT {
    function awardToken(address _user, uint256 _tokenID) external;
}
