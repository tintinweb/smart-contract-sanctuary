...
contract BudBase is Ownable {
    ...
    enum Feelings { CREATIVE, HAPPY, RELAXED }

    enum Terpenes { LIMONENE, PINENE, MYRCENE }

    /* ========== PUBLIC STATE VARIABLES ========== */

    /**
     * @dev payment required to use growing if it's done automatically
     * assigning to 0 indicate growing action is not automatic
     */
...
contract BudBreed is BudToken, ICryptoBud, ReentrancyGuard, Pausable {
    ...
    function getBudEffects(uint id) external view returns (uint256 feelings, uint256 terpenes) {
        Bud storage bud = buds[id];
        feelings = uint256(bud.feelings);
        terpenes = uint256(bud.terpenes);
        ...
        returns (uint256)
    {
        ...
        require(
            ...
            "CryptoBuds: Required hetching REWARD fee not sent"
        );
    ...
    function grow(uint256 _id) external override nonReentrant {
        ...
        (
            ...
            uint64 primaryEffect,
            uint64 secondaryEffect
        ...
        seed.feelings = Feelings(primaryEffect);
        seed.terpenes = Terpenes(secondaryEffect);
