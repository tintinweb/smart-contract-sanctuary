...
contract CryptoDoggos is AccessControl, DetailedERC721 {
    ...
    uint256 breedingFee = 0.01 ether;

    uint256 oneDoggoPrice = 0.002 ether;
    uint256 threeDoggoPrice = 0.005 ether;
    uint256 fiveDoggoPrice = 0.0075 ether;
    ...
    string baseURI = "http://crypto-doggos.web.app/image/";
    ...
    function _setBaseURI(string memory base) public onlyCLevel {
        baseURI = base;
    }
    ...
    function _setOneDoggoPrice(uint256 fee) public onlyCLevel {
        oneDoggoPrice = fee;
    }
    function _oneDoggoPrice() public view returns (uint256) {
        return oneDoggoPrice;
    }
    function _setThreeDoggoPrice(uint256 fee) public onlyCLevel {
        threeDoggoPrice = fee;
    }
    function _threeDoggoPrice() public view returns (uint256) {
        return threeDoggoPrice;
    }
    function _setFiveDoggoPrice(uint256 fee) public onlyCLevel {
        fiveDoggoPrice = fee;
    }
    function _fiveDoggoPrice() public view returns (uint256) {
        return fiveDoggoPrice;
    }
    ...
    function breedDoggos(address payable _owner, uint256 _firstDoggoId, uint256 _secondDoggoId) public payable {
        require(_owner != address(0));
        ...
        for (uint8 i = 1; i < 15; i++) {
            puppyDnaStarter[20 - i] = _genePriority(doggos[_firstDoggoId].dna, doggos[_secondDoggoId].dna, seed, (21 - i));
        }
        for (uint8 i = 15; i < 21; i++) {
            // puppyDnaStarter[20 - i] = _genePriority(doggos[seed, seed, seed, (21 - i));
            puppyDnaStarter[20 - i] = _getNthDigit(seed, (21 - i));
        }
    ...
    function openLootBox(address payable _owner, uint8 size) public payable {
        require(_owner != address(0));
        require(_owner == msg.sender);
        ...
        if (size == 1) {
            require(msg.value >= oneDoggoPrice);
        } else if (size == 3) {
            require(msg.value >= threeDoggoPrice);
        } else if (size == 5) {
            require(msg.value >= fiveDoggoPrice);
        }

        for (uint256 i = 0; i < size; i++) {
            _createToken(_owner, _generateRandomDna(), startingPrice, 0, 0);
        }
    }
    ...
    function _exists(uint256 dna) internal view returns (bool) {
        return dnaToOwner[dna] != address(0);
    }
...
contract ERC721 {
    ...
    string public baseURI = "https//crypto-doggos.web.app/api/";
