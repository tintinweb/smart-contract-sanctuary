...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "Ownable: caller is'nt  owner");
        _;
    }
...
contract ePING is Context, IERC20 {
    ...
    bool public isWalletTransferFeeEnabled = true;
    bool public isContractTransferFeeEnabled = true;
    address private owner;

     struct contractDetailsStruct {
     ...
     contractDetailsStruct private contractDetails = contractDetailsStruct(
    ...
    uint256 private constant  MAX = 16 * 10**9 * 10**9;
    // uint256 private  _tTotal = 1 * 10**9 * 10**_decimals ;
    uint256 private _tTotal = 1 * 10**0 * 10**contractDetails._decimals;
    ...
    uint256 public _maxTxAmount = 5 * 10**5  * 10**contractDetails._decimals;  
    uint256 public numTokensSellToAddToLiquidity = 4 * 10**6 * 10**contractDetails._decimals;   //0.1%
   ...
   // event BalanceWithdrawn(address withdrawer, uint256 amount);
    ...
    constructor () {
        ...
        IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); //BSC Testnet
        ...
        // _tTotal= 0 * 10**9 * 10**_decimals;
        

        emit Transfer(address(0), _msgSender(), _tTotal);
    }
    function toggleWalletTransferTax() external {
        require(msg.sender == owner);
        isWalletTransferFeeEnabled = !isWalletTransferFeeEnabled;
    }

    function toggleContractTransferTax() external  {
        require(msg.sender == owner);
    ...
    function name() public view returns (string memory) {
        return contractDetails._name;
    }
    function symbol() public view returns (string memory) {
        return contractDetails._symbol;
    }
    function decimals() public view returns (uint8) {
        return contractDetails._decimals;
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        _transfer(isWalletTransferFeeEnabled ,_msgSender(), recipient,amount);
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
       if (sender.isContract()) {
            _transfer(isContractTransferFeeEnabled , sender, recipient, amount);
        } else {
            _transfer( isWalletTransferFeeEnabled ,sender, recipient, amount);
        }
    ...
    function reflectionFromToken(bool deductTransferRfi ,uint256 tAmount) external view returns(uint256) {
        require(tAmount <= _tTotal, "Amount must be less than supply");
        if (!deductTransferRfi) {
            valuesFromGetValues memory s = _getValues(true ,tAmount);
            return s.rAmount;
        } else {
            valuesFromGetValues memory s = _getValues(true ,tAmount);
    ...
    function excludeFromRFI(address account) public {
        require(msg.sender == owner);
    ...
    function excludeFromFeeAndRfi(address account) external  {
        require(msg.sender == owner);
    ...
    */
    function setRfiRatesPercents(uint8 _rfi, uint8 _lp, uint8 _research, uint8 _dev) external  {
        require(msg.sender == owner);
      feeRates.rfi = _rfi;
    ...
    function setWallets(address _research, address _dev) external  {
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external  {
    ...
    function setThreshholdForLP(uint256 threshold) external  {
        require(msg.sender == owner);
      numTokensSellToAddToLiquidity = threshold * 10**contractDetails._decimals;
    }
    ...
    function _getValues( bool takeFee , uint256 tAmount) private view returns (valuesFromGetValues memory to_return) {
        to_return = _getTValues( takeFee ,tAmount);
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi) = _getRValues(takeFee , to_return, tAmount, _getRate());

        return to_return;

    }

    function _getTValues( bool takeFee ,uint256 tAmount) private view returns (valuesFromGetValues memory s) {
    ...
    function _getRValues(bool takeFee, valuesFromGetValues memory s, uint256 tAmount,  uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi) {
    ...
    function _transfer( bool takeFee , address from, address to, uint256 amount) private {
        ...
        _tokenTransfer(shouldTakeFeeForTransfer ,from, to, amount);
    }


    function _tokenTransfer( bool takeFee ,address sender, address recipient, uint256 tAmount) private {
        if (_rOwned[recipient] == 0) {numOfHODLers++;}
        valuesFromGetValues memory s = _getValues(takeFee ,tAmount);
    ...
    function totalDevelopmentFee() external view returns (uint256) {
        return _tDevelopmentTotal;
    }
     function totalResearchFee() external view returns (uint256) {
        return _tResearchTotal;
    }

    
    function adminConfig(bool isAdmin , address adminAddress) external  {
        require(msg.sender == owner);
    ...
    modifier onlyAdmin() {
        require(adminAddresses[_msgSender()], "Caller is'nt admin.");
        _;
    }

   
    function _mint(address recipient, uint256 amount) private {
        require(amount > 0 && _tTotal <= 4 * 10**9 * 10**contractDetails._decimals , "Transfer amount must be greater than zero tTotal supply can't exceed 4B");
          uint256 _rTransferAmount = (_rTotal * amount)/_tTotal;
         _tTotal = _tTotal + amount;
         _rTotal = _rTotal + _rTransferAmount;
...
//   function _mint(address recipient, uint256 amount) private {
//         require(amount > 0, "amount must be greater than zero");
//         require(_tTotal <= 4 * 10**9 * 10**_decimals, "tTotal supply can't exceed 4B");
...
//     }
    ...
    function burn(address from, uint256 value) external override onlyAdmin {
        _burn(from, value);
    }
