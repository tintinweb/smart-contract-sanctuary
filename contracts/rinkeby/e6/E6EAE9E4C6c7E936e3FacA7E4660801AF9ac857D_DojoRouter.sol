...
contract DojoRouter is VRFConsumerBase, Ownable {
    using SafeERC20 for IERC20;
    AggregatorV3Interface internal ethUsdPriceFeed;
    AggregatorV3Interface internal linkUsdPriceFeed;
    DojoHouse private house;
    DojoToken private dojo;
     ...
     * Trades cannot be greater than 5% of the house balance (w/ 10x max multiplier, 45% maximum loss from house at a time)
     */
     ...
     * User passes in a "level" for the odds of their trade
     * 1 = whitebelt, 2 = blue belt, else = black belt
     */
    function initiateSpinTrade(uint256 _level) public payable {
        //ensure trade is large enough
        require(msg.value >= getMinimumTradeSize(), "Your trade is too small!");
        //ensure that they do not have a pending trade
        require(
            AddressToPendingTrade[address(msg.sender)]._requestID == 0,
            "You already have a trade in progress"
        );
        //add their trade to mapping
        bytes32 newRequest = requestRandomness(keyhash, fee);
        //add their pending trade to mapping of addresses
        AddressToPendingTrade[address(msg.sender)] = PendingTrade(
            newRequest,
            msg.value,
            _level
        );
     ...
     */
    function claimSpinTrade() public {
        //requires that the caller has a requestID
        //change it to a length function?
        require(
            AddressToPendingTrade[address(msg.sender)]._requestID != 0,
            "You have no trade in progress"
        );
        //get their pending trade info (requestID, amount, level)
        bytes32 requestID = AddressToPendingTrade[address(msg.sender)]
            ._requestID;
        //require their requestID to have a random number != 0
        require(requestToRandom[requestID] != 0);
        uint256 size = AddressToPendingTrade[address(msg.sender)]._amount;
        uint256 level = AddressToPendingTrade[address(msg.sender)]._level;
        //get their random number
        uint256 random = requestToRandom[requestID];
        //spin the dojo token jackpot
        dojo.spinJackpot(random, size, address(msg.sender));
        //claim payout from House
        house.spinTrade{value: size}(
            level,
            random,
            payable(address(msg.sender))
        );
        //remove the pending trade
        delete AddressToPendingTrade[address(msg.sender)];
    }
    ...
    /**
     * Set the dojo token contract address after it has been created
     */
    function setDojoToken(address _dojo) public onlyOwner {
...
contract DojoHouse is ERC20("DojoLiquidity", "DLP") {
     ...
     */
    function leave(uint256 _share) public returns (uint256) {
    ...
    function spinTrade(
        ...
        //function only to be called by DojoRouter
        require(address(msg.sender) == router);
        //the base rate if lose
        uint256 base = 65;
        uint256 odds = 1;
        uint256 multiplier = 1;
        //get the odds and multiplier for based on the _type
        if (_type == 0) {
            //white belt trade
            multiplier = 185;
            odds = 4;
            ...
            //black belt trade (use else so no need to check _type >=0, <=3 to save gas)
            odds = 31;
            multiplier = 1000;
        }
        //see if spin won
        if (_random % odds == 1) {
            _to.transfer((msg.value * multiplier) / 100);
        } else {
            _to.transfer((msg.value * base) / 100);
        }
        uint256 what = (msg.value * totalSupply()) / (address(this).balance);
        //mint 1% of the value to Treasury as fee
        _mint(treasury, what);
        //mint 1% of the value to DojoBar
        _mint(bar, what);
    }
     ...
     * If they win, depending on type, user will get back either 1.25x, 2x or 5x
     */
    function _allOrNothingTrade(
        uint256 _amount,
        uint8 _type,
        uint256 _random
    ) external {
...
/**
 * DOJO is an ERC20 token
 * Each trade that occurs on the DojoRouter, there is a chance to hit a jackpot
 * By doing so, the user mints DOJO tokens based on the size of the jackpot
 * Minting can only be done by the (immutable) router
 */
contract DojoToken is ERC20, Ownable {
    // Dojotoken Jackpot payout historys
    uint256[] public jackpotPaid;
    uint256[] public jackpotWeight;
    uint256[] public jackpotOdds;
    uint256 public rewardsPerBlock;
    uint256 public startBlock;
    //router immutable to ensure fairness of RNG
    address public immutable router;

    /**
     * Constructor to set the initial supply, initial rewards rate
     */
    constructor(
        ...
        //set the router (immutable)
        router = _router;
    }

    /**
     * Only callable by the DegenDojo router contract set at launch, a player gets a chance at winning each jackpot
    ...
    function spinJackpot(
        ...
        //require that it is the degendojo contract that calls this
        require(msg.sender == router);
        uint256 winnings = 0;
        uint256 splitRandom;
        //iterate over each jackpot (not including jackpot[8])
        for (uint256 i = 0; i < 8; i++) {
            //breakup 256 bit random number to 32 bit numbers with bitwise operator
            splitRandom = ((_random >> (32 * i)) & 0xFFFFFFFF);
            //get the odd of hitting
            //odds are capped as 1 in 2
            uint256 odds = min(2, jackpotOdds[i] / _size);
            if (splitRandom % odds == 0) {
     ...
     */
    function viewJackpots(uint256 _jackpotNumber)
        public
        view
        returns (uint256)
    {
