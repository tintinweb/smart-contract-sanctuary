...
contract NFTProject is ERC721, Ownable, Pausable {
    ...
    event UpdatedItemToOpenForAuction(uint256 itemID, bool openForAuction, uint256 price, uint256 minimumAllowedBid, uint256 auctionExpiryTimestamp, address addedBy);

    event BaseURI(string baseTokenURI, address addedBy);

    event NewHighestOfferOnItem(uint256 itemID, uint256 expiryTimestamp, address beneficiary, uint256 availableOffer);
    ...
    function enableAuctionOnArtwork(uint256 itemID, uint256 minimumAllowedBid, uint256 price, uint256 expiryTimestamp) public returns (bool) {
    ...
    function finalizeAuction(uint256 itemID, uint256 minimumAllowedBid, uint256 price, uint256 expiryTimestamp) public returns (bool) {
        require (msg.sender == itemsOnProject[itemID].owner, "Invalid Owner");
        ...
        uint256 calculatePlatformRoyality = SafeMath.mul(itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers.current()].offer, platformRoyality); 
        uint256 calculateCreatorRoyality = SafeMath.mul(itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers.current()].offer, creatorRoyality); 
        ...
        itemsOnProject[itemID].owner = payable(itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers.current()].beneficiaryAddress);

        _transfer(address(this), itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers.current()].beneficiaryAddress, itemID);
    ...
    function makeOfferOnItem (uint256 itemID, uint256 expiryTimestamp) public payable returns (bool) {
        ...
        itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers.current()].expiryTimestamp = expiryTimestamp;


        emit NewHighestOfferOnItem (itemID, expiryTimestamp, msg.sender, msg.value);
    }
        ...
        )
    {
        ...
        itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers.current()].offer,
        itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers.current()].expiryTimestamp
        );
    }
