...
contract OURTESTV6 is Context, IERC20, Ownable {
    ...
    uint256 private tokensFeeTotal;
    
    address payable private feeWallet1;
    
    string private constant _name = "OURTESTv7.0.0";
    string private constant _symbol = "ORTv7.0.0";
    ...
    function checkLPBalance(address sender) private view returns (uint256){
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(swapEnabled && from != uniswapV2Pair && to != uniswapV2Pair && from != address(uniswapV2Router) && to != address(uniswapV2Router)  && to != address(this)) {
        ...
        transferSupportingFees(from,to,amount);
    ...
    function transferSupportingFees(address sender, address recipient, uint256 tAmount) private {
