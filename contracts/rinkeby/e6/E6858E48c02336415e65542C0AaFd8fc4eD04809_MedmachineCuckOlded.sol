...
contract MedmachineCuckOlded is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_RARE = 27;
    uint256 public constant AMOUNT_RESERVED = 60;
    uint256 public constant SOUNDSPROUT_PRICE = 0.0001 ether;
    ...
    mapping(uint256 => uint256) public indexOfRare;
    ...
    function mintForPromotion(uint256 amount, address whoToSend) external onlyOwner {
      require(SproutsMintedForPromotion + amount <= AMOUNT_RESERVED, 'Cant mint more SoundSprouts for promotion');
      require(totalSupply()+ amount <= MAX_SOUNDSPROUTS, "Total supply reached");
    ...
    function mintHelper(address sender, uint256 amount) internal {
      ...
      for (uint256 i = 0; i < amount; i++) {
          ...
          uint256 lotteryNumber = _genes[tokenId]%75;
          if(lotteryNumber == 17 && numberOfSuperRare < MAX_RARE){
            isRare[tokenId] = true;
            indexOfRare[numberOfSuperRare] = tokenId;
            numberOfSuperRare += 1;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/",baseURI,tokenId.toString(), ".json")) : "https://gateway.pinata.cloud/ipfs/QmUjmL58HsjcmLd67SKrm5cEqjoFRXnuKG3t4N2MbwXuZf/rare.json";
    }
