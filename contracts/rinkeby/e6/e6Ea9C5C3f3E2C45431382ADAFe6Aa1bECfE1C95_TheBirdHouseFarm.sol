...
contract TheBirdHouseFarm is ERC20, Ownable {
    ...
    function calculateRewardByTimestamp(uint256 _blockTimestamp)
        internal
        view
        returns (uint256)
    {
    ...
    function viewRewardForToken(uint256 _tokenId)
        public
        view
        returns (uint256)
    {
        ...
        if (tokenIdToTimestamp[_tokenId] == 0) {
            return calculateRewardByTimestamp(startingTimestamp);
        } else {
            return calculateRewardByTimestamp(tokenIdToTimestamp[_tokenId]);
        }
    }
    ...
    function viewRewardsForWallet(address _owner)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i; i < tokenCount; i++) {
            ...
            rollingRewards = rollingRewards + viewRewardForToken(_tokenId);
        }

        return rollingRewards;
    }
    ...
    function claimRewards() public returns (uint256) {
        ...
        _mint(msg.sender, rollingRewards);
        return rollingRewards;
    }
}
