...
contract RocketFactory is ERC721, Ownable {
    ...
    address rocketComponentContract;
     ...
     */
    function setRocketComponentContract(address _address) external onlyOwner {
    ...
    function mint(
        ...
        RocketComponent rocketComponent =
            RocketComponent(rocketComponentContract);
        ...
        returns (RocketView memory)
    {
        ...
        RocketComponent.ComponentView memory nose =
            rocketComponent.retrieve(rockets[_tokenId].nose);
        RocketComponent.ComponentView memory body =
            rocketComponent.retrieve(rockets[_tokenId].body);
        RocketComponent.ComponentView memory tail =
            rocketComponent.retrieve(rockets[_tokenId].tail);

        bool perfectRocket =
            utils.compareStrings(nose.brand, body.brand) &&
        ...
        if (nose.hasSticker) {
            nosePath = string(abi.encodePacked(nosePath, "-", nose.sticker));
            stickers++;
        }

        string memory bodyPath = body.brand;
        if (body.hasSticker) {
            bodyPath = string(abi.encodePacked(bodyPath, "-", body.sticker));
            stickers++;
        }

        string memory tailPath = tail.brand;
        if (tail.hasSticker) {
            tailPath = string(abi.encodePacked(tailPath, "-", tail.sticker));
            stickers++;
        }
