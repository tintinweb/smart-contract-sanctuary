...
contract TypicalTigers is ERC721Enumerable, Ownable, AccessControlEnumerable {
    ...
    constructor() ERC721("TypicalTigers", "TPT") {
        ...
        _setRoleAdmin(PREMINTER_ROLE, WHITELIST_MANAGER_ROLE);
    }
    ...
    function batchAddToWhitelist(address[] calldata _addresses) external onlyRole(WHITELIST_MANAGER_ROLE) {
    ...
    function clearWhitelist() external onlyRole(WHITELIST_MANAGER_ROLE){
       uint256  whitelistCount = getRoleMemberCount(PREMINTER_ROLE);
        for (uint256 i = 0; i < whitelistCount; i++) {
            address member =  getRoleMember(PREMINTER_ROLE, i);
    ...
    function togglePresaleStatus() external onlyRole(WHITELIST_MANAGER_ROLE) {
        presaleIsActive = !presaleIsActive;
        emit SaleActivation(!presaleIsActive);
    }
