...
contract AuthorizeManager {
    ...
    address public defaultAuthorizeContract;
    ...
    event DefaultAuthorizationContractSet(address from, address authContract);

    /// @notice emitted when a token id authorization contract is set
    /// @param from address of the setter/updater
    /// @param id token id
    /// @param authContract authorization contact address
    event AuthorizationContractSet(address from, uint256 id, address authContract);
    ...
    /// @param id token id
    /// @param account to address of the transfer request
    /// @param data any orbitary data passed to transfer function
    ...
    function mustBeAuthorizedHolder(
        ...
        address authContract = authorizeContracts[id];

        /*
         * If authorization contract not set for this id, fall back to default authorization
         * contract
         */
        if (authContract == address(0)) authContract = defaultAuthorizeContract;

        // return true if no auth contract set
        if (authContract == address(0)) return true;

        try IAuthorizationContract(authContract).isAccountAuthorized(account, id, data) returns (bool response) {
            return response;
    ...
    function _setDefaultAuthorizationContract(address authContract) internal {
        ...
        defaultAuthorizeContract = authContract;
        emit DefaultAuthorizationContractSet(msg.sender, authContract);
    }
    ...
    function _setAuthorizationContract(uint256 id, address authContract) internal {
        ...
        emit AuthorizationContractSet(msg.sender, id, authContract);
    }
...
contract RoleManager is AccessControl {
    ...
    modifier onlyEditor(address account) {
        require(hasRole(EDITOR_ROLE, account), "RM: caller should be an editor");
        _;
    }

    modifier onlyIssuer(address account) {
        require(hasRole(ISSUER_ROLE, account), "RM: caller should be an issuer");
        _;
    }
    ...
    function grantEditorRole(address to) external onlyIssuer(msg.sender) {
    ...
    function revokeEditorRole(address account) external onlyIssuer(msg.sender) {
        _revokeRole(EDITOR_ROLE, account);
    }

    /// @notice grants ISSUER role to a given account, only an ISSUER can do this.
    /// @param to account to which the role to be granted.
    function grantIssuerRole(address to) external onlyIssuer(msg.sender) {
    ...
    function revokeIssuerRole(address account) external onlyIssuer(msg.sender) {
    ...
    function grantRole(bytes32 role, address account) public override onlyIssuer(msg.sender) {
        _grantRole(role, account);
    }
    ...
    function revokeRole(bytes32 role, address account) public override onlyIssuer(msg.sender) {
        _revokeRole(role, account);
    }
}
...
// File: contracts/AssetNFTToken.sol
...
/// @title AssetNFTToken TBD
/// @notice Asset NFT Token TBD
contract AssetNFTToken is ERC1155, RoleManager, AuthorizeManager {
    /// NFT tokenId to KYA mapping
    mapping(uint256 => string) private kya;

    /// token to total supply mapping
    mapping(uint256 => uint256) private tokenSupply;

    /// Base KAY string, prefixed for all token KYAs
    string public baseKya; 
    ...
    /// @notice Emitted when Base KYA is set/updated
    /// @param from adress of the updater
    /// @param baseKya Base KYA string, ex: ipfs://
    event BaseKyaUpdated(address indexed from, string baseKya);

    /// @notice Emitted when KYA of a tokenId is set
    /// @param from address of the setter
    /// @param id token id
    /// @param kya KYA, a string
    event KyaUpdated(address from, uint256 indexed id, string kya);

    /// @notice Emitted when BASE URI of the tokens is set
    /// @param baseURI URI string, ex: https://ant.com/
    event BaseURIUpdated(string baseURI);

    /// @notice constructor for setting basic configurations.
    /// @param _baseURI Base URI for all tokens, can be changed later
    /// @param issuer default issuer address, will be assigned ISSUER Role
    /// @param editor default editor address, will be assigned EDITOR Role
    constructor(
        string memory _name,
        ...
        address issuer,
        address editor
    ) ERC1155(_baseURI) RoleManager(issuer, editor) {
        require(issuer != address(0), "ANT: Invalid issuer address");
        require(editor != address(0), "ANT: Invalid editor address");
        name = _name;
        symbol = _symbol;
    }
    ...
    /// @param interfaceId EIP165 interfaceID
    /// @return bool true or false
    function supportsInterface(bytes4 interfaceId) public view override(ERC1155, RoleManager) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    /// @notice Updates Base URI string. Only ISSUER_ROLE role can do this
    /// @param _baseURI URI string
    function setBaseURI(string calldata _baseURI) external onlyIssuer(_msgSender()) {
        _setURI(_baseURI);

        emit BaseURIUpdated(_baseURI);
    }

    /// @notice sets Base KYA, this will be prefixed for all token KYAs. Only
    /// EDITOR can do this.
    /// @param _baseKya Base KYA string, ex: ipfs://
    function setBaseKya(string calldata _baseKya) external onlyEditor(_msgSender()) {
        baseKya = _baseKya;
        emit BaseKyaUpdated(_msgSender(), baseKya);
    }

    /// @notice sets KYA of a tokenId. Only EDITOR can do this
    /// @param _id token id
    /// @param _kya KYA string
    function setKya(uint256 _id, string calldata _kya) external onlyEditor(_msgSender()) {
        kya[_id] = _kya;
        emit KyaUpdated(_msgSender(), _id, _kya);
    }

    /// @notice returns KYA of a token id. This prefixes Base KYA
    /// @param id token id
    /// @return KYA string
    function getKya(uint256 id) external view returns (string memory) {
        return string(abi.encodePacked(baseKya, kya[id]));
    }

    /// @notice returns token URI in the form baseURI/{id}
    /// @param id token id
    /// @return _tokenURI token URI string
    function uri(uint256 id) public view override returns (string memory) {
        string memory baseUri = baseURI();
        return bytes(baseUri).length > 0 ? string(abi.encodePacked(baseUri, Strings.toString(id))) : "";
    }

    /// @notice returns base URI string
    /// @return _baseURI base URI string
    ...
    /// @param id token id
    /// @param value number of tokens, only one for NFT tokens
    function mint(
        address to,
        uint256 id,
        uint256 value
    ) external onlyIssuer(_msgSender()) {
        _mint(to, id, value, new bytes(0));
        /* overflow reverts */
        tokenSupply[id] = tokenSupply[id] + value;
    }

    /// @notice mints a batch of tokens, only ISSUER can do this
    /// @param to account to mint to
    /// @param ids array of token ids
    /// @param values array of amounts/values
    function mintBatch(
        address to,
        uint256[] calldata ids,
        uint256[] calldata values
    ) external onlyIssuer(_msgSender()) {
        require(ids.length == values.length, "ANT: Mismatched array lengths");
    ...
    /// @param id token id
    /// @param value number of tokens of type id to burn
    ...
    /// @param values array of amounts of each token ids
    ...
    /// @param id token id
    /// @param authContract Authorization contract address, must be a contract address
    /// which implements IAuthorizationContract interface.
    function setAuthorizationContract(uint256 id, address authContract) external onlyIssuer(_msgSender()) {
        _setAuthorizationContract(id, authContract);
    }
    ...
    /// @param authContract authorization contract address, must be a contract address
    /// which implements IAuthorizationContract interface.
    function setDefaultAuthorizationContract(address authContract) external onlyIssuer(_msgSender()) {
        _setDefaultAuthorizationContract(authContract);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (to != address(0)) {
            ...
            for (i = 0; i < ids.length; i++) {
                require(mustBeAuthorizedHolder(ids[i], to, data), "ANT: Not authorized to");
            }
        }
    }
}