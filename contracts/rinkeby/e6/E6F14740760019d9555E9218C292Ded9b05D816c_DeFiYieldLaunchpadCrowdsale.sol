...
import "./validation/LaunchpadCappedCrowdsale.sol";

contract DeFiYieldLaunchpadCrowdsale is LaunchpadCappedCrowdsale {
  ...
  modifier onlyWhileOpen() {
    require(isOpen(), "DeFiYieldLaunchpadCrowdsale: not open");
    _;
  }

  constructor(address wallet_, address[] memory collaterals_)
    LaunchpadCrowdsale(wallet_, collaterals_)
  {
  ...
  function openSale()
    ...
    onlyAdmin
  {
    require(_state == State.None, "DeFiYieldLaunchpadCrowdsale::openSale: sales is already open or closed");
    _state = State.Open;
  ...
  function closeSale()
    ...
    onlyAdmin
  {
    require(isOpen(), "DeFiYieldLaunchpadCrowdsale::closeSale: sales is already closed or not open");
    _state = State.Closed;
  ...
  function addRound(uint256 price_, uint256 totalSupply_)
    ...
    onlyAdmin
  {
    require(!isClosed(), "DeFiYieldLaunchpadCrowdsale::addRound: sales is already closed");
  ...
  function updateRound(uint256 index_, uint256 price_, uint256 totalSupply_) 
    ...
    onlyAdmin
  {
    require(_rounds[index_].defined, "DeFiYieldLaunchpadCrowdsale::updateRound: no round with provided index");
    require(_rounds[index_].state != State.Closed, "DeFiYieldLaunchpadCrowdsale::updateRound: round is already closed");
    require(!isClosed(), "DeFiYieldLaunchpadCrowdsale::updateRound: sales is already closed");
  ...
  function openRound(uint256 index_)
    ...
    onlyAdmin 
  {
    require(isOpen(), "DeFiYieldLaunchpadCrowdsale::openRound: sales is not open yet");
    require(_rounds[index_].defined, "DeFiYieldLaunchpadCrowdsale::openRound: no round with provided index");
    require(_rounds[index_].state == State.None, "DeFiYieldLaunchpadCrowdsale::openRound: round is already open or closed");
  ...
  function closeRound(uint256 index_)
    ...
    onlyAdmin 
  {
    require(isOpen(), "DeFiYieldLaunchpadCrowdsale::closeRound: sales is not open yet");
    require(_rounds[index_].defined, "DeFiYieldLaunchpadCrowdsale::closeRound: no round with provided index");
    require(_rounds[index_].state == State.Open, "DeFiYieldLaunchpadCrowdsale::closeRound: round is not open");
  ...
  function _preValidatePurchase(address beneficiary_, address collateral_, uint256 amount_)
    ...
    onlyWhileOpen
  {
    require(_state == State.Open, "DeFiYieldLaunchpadCrowdsale::_preValidatePurchase: sales is not open yet");
    require(_rounds[_activeRound].state == State.Open, "DeFiYieldLaunchpadCrowdsale::_preValidatePurchase: sales round is not open yet");
    require(
      _rounds[_activeRound].totalSupply >= _rounds[_activeRound].investment + _getTokenAmount(collateral_, amount_),
      "DeFiYieldLaunchpadCrowdsale::_preValidatePurchase: exceeded round total supply"
    );
  ...
  /**
   * @dev Override to extend the post purchase processing
   * @param beneficiary_  address receiving the tokens.
   * @param collateral_  collateral token.
   * @param amount_  amount of collateral tokens involved in the purchase.
   * @param tokens_  amount of tokens purchased.
   */
  function _postPurchase(address beneficiary_, address collateral_, uint256 amount_, uint256 tokens_)
    internal
    virtual
    override
  {
    emit TokensPurchased(beneficiary_, collateral_, amount_, tokens_, _activeRound);
  }
}
...
contract Crowdsale is Context {
  ...
  constructor(uint256 price_, address wallet_, address token_, address[] memory collaterals_) {
    ...
    require(address(token_) != address(0), "Crowdsale: token address is zero");

    for(uint256 i = 0; i < collaterals_.length; i++) {
      require(collaterals_[i] != address(0), "Crowdsale: collateral address is zero");
    ...
    virtual
  {
    ...
    require(beneficiary_ != address(0), "Crowdsale::_preValidatePurchase: beneficiary address is zero");
...
import "@openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.sol";
import "../LaunchpadCrowdsale.sol";
import "../roles/KycRole.sol";

/**
 * @title LaunchpadCappedCrowdsale
 * @dev Crowdsale where users can contribute within cap validation.
 */
abstract contract LaunchpadCappedCrowdsale is LaunchpadCrowdsale, KycRole {
    ...
    onlyAdmin
  {
    if(level_ == KycLevel.low) {
      require(_cap[KycLevel.medium] >= cap_, "LaunchpadCappedCrowdsale::setCap: cap higher than medium cap");
    }
    if(level_ == KycLevel.medium) {
      require(_cap[KycLevel.high] >= cap_, "LaunchpadCappedCrowdsale::setCap: cap higher than high cap");
    }    
    _cap[level_] = cap_;
    ...
    override
  {
    ...
    require(_minInvestment <= normalizedAmount, "LaunchpadCappedCrowdsale::_preValidatePurchase: investment amount too low");
    require(capOf(beneficiary_) >= normalizedAmount, "LaunchpadCappedCrowdsale::_preValidatePurchase: exceeded cap");
 ...
 * @dev LaunchpadCrowdsale is a base contract for managing a token crowdsale,
 * allowing investors to purchase tokens with ether. This contract implements
 * such functionality in its most fundamental form and can be extended to provide additional
 * functionality and/or custom behavior.
 * The external interface represents the basic interface for purchasing tokens, and conforms
 ...
 * behavior.
 */
abstract contract LaunchpadCrowdsale is Context {
  using SafeERC20 for IERC20;
   ...
   */
  constructor(address wallet_, address[] memory collaterals_) {
    require(wallet_ != address(0), "LaunchpadCrowdsale: wallet address is zero");

    for(uint256 i = 0; i < collaterals_.length; i++) {
      require(collaterals_[i] != address(0), "LaunchpadCrowdsale: collateral address is zero");
      _collaterals[collaterals_[i]] = Collateral({
        defined: true,
        raised: 0
      });
    ...
    external
  {
    ...
    _forwardFunds(collateral_, amount_);
    _postValidatePurchase(_msgSender(), collateral_, amount_);
    _postPurchase(_msgSender(), collateral_, amount_, tokens);
  }

  /**
   * @dev Validation of an incoming purchase. Use require statements to revert state when conditions are not met.
   * Use `super` in contracts that inherit from LaunchpadCrowdsale to extend their validations.
   * Example from LaunchpadCappedCrowdsale.sol's _preValidatePurchase method:
   *     super._preValidatePurchase(beneficiary, collateralAmount);
   *     require(collateralRaised().add(collateralAmount) <= cap);
   * @param beneficiary_  address performing the token purchase.
   * @param collateral_  collateral token.
  ...
  function _preValidatePurchase(address beneficiary_, address collateral_, uint256 amount_)
    ...
    virtual
  {
    require(_collaterals[collateral_].defined, "LaunchpadCrowdsale::_preValidatePurchase: collateral token not defined");
    require(beneficiary_ != address(0), "LaunchpadCrowdsale::_preValidatePurchase: beneficiary address is zero");
    require(amount_ != 0, "LaunchpadCrowdsale::_preValidatePurchase: collateralAmount is 0");
    ...
    returns (uint256)
  {
    ...
    if(TOKEN_DECIMALS >= collateralDecimals) {
      return (amount_ * PRECISION * (10 ** (TOKEN_DECIMALS - collateralDecimals))) / getPrice();
    }
    return (amount_ * PRECISION) / (10 ** (collateralDecimals - TOKEN_DECIMALS)) / getPrice();
  }
