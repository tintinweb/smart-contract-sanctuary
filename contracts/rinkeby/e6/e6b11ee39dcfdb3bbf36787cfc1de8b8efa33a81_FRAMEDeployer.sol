...
contract FRAMEDeployer is ERC721URIStorage, ERC721Enumerable, Pausable{
    ...
    modifier onlyOwner() {
        require(msg.sender == owner_, "ERROR");
        _;
    }

    constructor() payable ERC721("FRAME", "Frame"){
        isBase = true;
    }

    function initialize(address _owner, uint max_, uint price_) external {
        require( isBase == false);

        require(owner_ == address(0), "ERROR");
        owner_ = _owner;
        ticket_price = price_;
        MAX_Tokens = max_;
    }
        ...
        returns (uint256)
    {
        ...
        require(msg.value * numberOfTokens >= ticket_price);
