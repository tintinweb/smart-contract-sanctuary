...
contract PX is ERC721, ERC721Metadata {
    ...
    // Mapping token ID to merge count.
    mapping (uint256 => uint256) private _mergeCount;

    // Mapping token ID to all merged token IDs.
    mapping (uint256 => uint256[]) private _mergeList;



    function getMergeCount(uint256 tokenId) public view returns (uint256 mergeCount) {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        uint256 mergeCount = _mergeCount[tokenId];
        return mergeCount;
    }

    function getMergeList(uint256 tokenId) public view returns (uint256[] memory mergeList) {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        uint256[] memory mergeList = _mergeList[tokenId];
        return mergeList;
    }
    ...
    constructor(address registry_, address omnibus_, address metadataGenerator_) {
        ...
        _pak = 0x2Ce780D7c743A57791B835a9d6F998B15BBbA5a4;
        _receiver = 0x2Ce780D7c743A57791B835a9d6F998B15BBbA5a4;
    ...
    function _merge(uint256 tokenIdRcvr, uint256 tokenIdSndr) internal returns (uint256 tokenIdDead) {
        ...
        address ownerSmall = ownerRcvr;
        address ownerLarge = ownerSndr;
        ...
        if (massRcvr >= massSndr) {
            ...
            ownerSmall = ownerSndr;
            ownerLarge = ownerRcvr;
        ...
        _values[tokenIdLarge] += decodeMass(_values[tokenIdSmall]);

        if(decodeMass(_values[tokenIdLarge]) > decodeMass(_values[_alphaId])){
            _alphaId = tokenIdLarge;
            _alphaMass = decodeMass(_values[tokenIdLarge]);
            emit AlphaMassUpdate(_alphaId, _alphaMass);
        }
        
        _mergeList[tokenIdLarge].push(tokenIdSmall);
        _mergeCount[tokenIdLarge] += 1;
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) external view returns (address, uint256) {
        uint256 percentageRoyalty = 1000;
                                    
        uint256 percentageTotal = 10000;

        uint256 royaltyAmount = (salePrice * percentageRoyalty) / percentageTotal;
    ...
    function setMetadataGenerator(address metadataGenerator_) external onlyValidSender {  
        _metadataGenerator = IPXMetadata(metadataGenerator_);
    }
   
    function setBlacklistAddress(address address_, bool status) external onlyValidSender {
        _blacklistAddress[address_] = status;
    }



    function whitelistUpdate(address address_, bool status) external onlyValidSender {
        _whitelistAddress[address_] = status;
    }

    function _getWhistlist(uint256 tokenId) internal view returns (address whitelist)  {
    ...
    function mint(uint256[] memory values_) external onlyValidSender {
        ...
        emit ConsecutiveTransfer(_countMint, index, address(0), _omnibus);
...
interface IPXMetadata {    
    function tokenMetadata(
        ...
        uint256 alphaMass, 
        bool isAlpha, 
        uint256[] calldata mergeList) external view returns (string memory);
}
...
contract PXMetadata is IPXMetadata {
    ...
    function tokenMetadata(uint256 tokenId, uint256 rarity, uint256 tokenMass, uint256 alphaMass, bool isAlpha, uint256[] calldata mergeList) external view override returns (string memory) {        
        string memory base64Json = Base64.encode(bytes(string(abi.encodePacked(_getJson(tokenId, rarity, tokenMass, alphaMass, isAlpha, mergeList)))));
    ...
    function _getJson(uint256 tokenId, uint256 rarity, uint256 tokenMass, uint256 alphaMass, bool isAlpha, uint256[] calldata mergeList) private view returns (string memory) {        
        ...
        ERC721MetadataStructure memory metadata = ERC721MetadataStructure({
            ...
            createdBy: "Z",
            image: imageData,
            attributes: _getJsonAttributes(tokenId, rarity, tokenMass, mergeList.length, isAlpha)
        });
