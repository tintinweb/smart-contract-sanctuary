...
contract ERC721Bootstrap is ERC721Enumerable, Ownable {
	using Strings for uint256;

	string private _baseTokenURI = "https://goats-api.to.wtf/metadata/1";
	string private _contractURI = "https://goats-api.to.wtf/contract_uri";

	uint256 public itemPrice = 100000000000000000; // 0.1 ETH
	bool public isSaleActive = false;
	uint256 public constant totalTokenToMint = 10000;
	uint256 public purchasedTokens = 0;
	uint256 public nameNFTFee = 100000000000000; //0.01 ETH
	mapping(uint256 => string) public nftName;

	event Action(uint256 petID, uint256 value, uint256 actionID, string payload);
	event Named(uint256 tokenID, string name);

	constructor() ERC721("ERC721Name", "SYMBOL") {}
	...
	function purchaseToken(uint256 _howMany) public payable {
		...
		require(_howMany <= 10, "max 10 tokens at once");
	...
	function nameNFT(uint256 _nftID, string memory _name) external payable {
		require(ownerOf(_nftID) == msg.sender, "you are not the owner");
		require(msg.value == nameNFTFee, "send the exact value");
		nftName[_nftID] = _name;
		emit Named(_nftID, _name);
	}
	...
	function setNameNFTFee(uint256 _newFee) public onlyOwner {
		nameNFTFee = _newFee;
	}
