...
interface ILiquidityProvider {
    function apis(uint256) external returns(address, address, address);
...
contract IMasterChef is Ownable {
    using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        ICombMaster _oldMasterchef,
        ILiquidityProvider _liquidityProvider
    ) {
        comb = _comb;
        depositToken = _depositToken;
        oldMasterchef = _oldMasterchef;

        liquidityProvider = _liquidityProvider;

        _combToken = IERC20(address(_comb));
    }
    ...
    function rewardOnBlock(uint256 _blockTimestamp)
        ...
        returns (uint256 _reward)
    {
        _reward = (_blockTimestamp - startTimestamp) * rewardPerSecond;
    }
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardBlockTimestamp && lpSupply != 0) {
            uint256 pendingReward =
                rewardOnBlock(pool.lastRewardBlockTimestamp);
            uint256 combReward =
                (pendingReward * pool.allocPoint) / totalAllocPoint;
            accCombPerShare = accCombPerShare + (combReward * 1e12) / lpSupply;
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 pendingReward = rewardOnBlock(pool.lastRewardBlockTimestamp);
        uint256 combReward =
            (pendingReward * pool.allocPoint) / totalAllocPoint;
        pool.accCombPerShare =
            pool.accCombPerShare +
...
interface ICombMaster is IERC20 {
