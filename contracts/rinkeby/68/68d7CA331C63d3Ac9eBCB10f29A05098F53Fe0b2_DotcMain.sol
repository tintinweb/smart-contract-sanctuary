...
contract DotcMain is owned {
	...
	require(amount <= IERC20(token).balanceOf(msg.sender), "insuf asset wallet");
	...
	require(collateral_tokens.length>0 && collateral_tokens.length == collateral_wallet_values.length, "Collateral check");
	...
	require(order.order_id == order_id, "Order not match!");
	order.status = status;
	//根据订单状态进行惩罚,直接从抵押资产里扣除
	if(status == 71 || status == 74 || status == 76)
	{
		//买家违约
		userPenaltyMap[order.buyer].push(order_id);
		//TODO:这里如何扣除抵押？？
		//Collateral[] storage cols = buyerCollateral[order.buyer][order_id];
	}else if(status == 72 || status == 73 || status == 75)
	{
		//卖家违约
