...
contract Recipt is 
    ...
    // TODO: Format text if it's over 1 line, up to max 140-280 chars
    // https://ethereum.stackexchange.com/questions/90060/solidity-split-comma-delimited-string-into-individual-words
    ...
    // Onchain metadata
    function tokenURI_noimage(uint256 tokenId) public view returns (string memory) {
        if (!written[tokenId]) {
            return string("[]");
        }
        string memory author = toString(abi.encodePacked(data[tokenId].author));
