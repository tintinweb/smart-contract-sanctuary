...
abstract contract Signer is Ownable {
  ...
  function uniq() public view returns (bytes32) {
    return _uniq;
  }
  ...
  modifier verifySignature(
    ...
    require(!_signatures[signatureHash], 'Signer: signature already used');
    require(address(_signer) != address(0), 'Signer: signer not initialised');

    bytes memory message = abi.encode(selector, params);

    bytes32 hash = keccak256(
      abi.encode(_uniq, signature.nonce, msg.sender, message)
    );

    address addr = ecrecover(hash, signature.v, signature.r, signature.s);

    require(_signer == addr, 'Signer: invalid signature');
