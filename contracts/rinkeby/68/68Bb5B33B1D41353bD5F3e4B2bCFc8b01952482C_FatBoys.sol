...
contract FatBoys is ERC721, Ownable {
  ...
  uint256 public presaleStartTime = 1644467451; // 1900 UTC 3rd December
  uint256 public publicSaleStartTime = 1644467451; //  2000 UTC 4th December
  ...
  function devMint(uint256 _mintAmount, address _to) public onlyOwner {
    ...
    require(
      ...
      "FatBoys: total mint amount exceeded supply, try lowering amount"
    );
  ...
  function publicMint(uint256 _mintAmount) public payable {
    uint256 s = tokenSupply.current();
    require(!paused);
    require(isPublicSaleOpen() || publicSaleOpen, "FatBoys: Public Sale is not open");
    require(s + _mintAmount <= maxSupply, "FatBoys: Total mint amount exceeded");
    require(_mintAmount > 0, "FatBoys: Please mint atleast one NFT");
    require(_mintAmount <= maxMintPerTxn, "FatBoys: You can only mint up to 2 per txn");
    require(msg.value == cost * _mintAmount,"FatBoys: not enough ether sent for mint amount");
    require(addressToMintedAmount[msg.sender]+ _mintAmount <= maxMintPerWallet, "FatBoys: Exceeded max mints allowed per wallet");

    (bool successT, ) = treasury.call{ value: (msg.value*92)/100 }(""); // forward amount to treasury wallet
    require(successT, "FatBoys: not able to forward msg value to treasury");
    delete successT;

    (bool successD, ) = developer.call{ value: (msg.value*8)/100 }(""); // forward amount to developer wallet
    require(successD, "FatBoys: not able to forward msg value to developer");
    delete successD;
  ...
  function presaleMint(uint256 _mintAmount) public payable {
    uint256 s = tokenSupply.current();
    require(!paused);
    require(whitelistedAddresses[msg.sender], "FatBoys: You are not whitelisted");
    require(isPresaleOpen() || presaleOpen, "FatBoys: Presale is not open");
    require(_mintAmount > 0, "FatBoys: Please mint atleast one NFT");
    require(s + _mintAmount <= maxSupply, "FatBoys: Total mint amount exceeded");
    require(_mintAmount <= maxMintPerTxn, "FatBoys: You can only mint up to 2 per txn");
    require(msg.value == presaleCost * _mintAmount,"FatBoys: not enough ether sent for mint amount");
    require(addressToMintedAmount[msg.sender] + _mintAmount <= maxMintPerWallet, "FatBoys: Exceeded max mints allowed per wallet");
    ...
    emit PresaleMint(msg.sender, _mintAmount);
  }

//   function airdrop(address[] memory giveawayList) public onlyOwner {
//   require(!paused, "FatBoys: contract is paused");
//   require(
//     balanceOf(msg.sender) >= giveawayList.length,
//     "FatBoys: not enough in wallet for airdrop amount"
//   );
