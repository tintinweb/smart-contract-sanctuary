...
contract KoiBoi is Ownable, Pausable, ERC721{ 
    ...
    constructor() ERC721("KoiBoi", "KOIBOIS"){
        setBaseURI("https://127.0.0.1:5000");
    }


  // MINTING
  //#########
    function mint(uint256 num_purchased) public payable {
    ...
    function preSaleMint(uint num_purchased) public payable {
        require(msg.value == BOI_PRICE.mul(num_purchased), "Incorrect amount of ethereums");
        require(counter + num_purchased -1 <= MAX_BOIS, "all bois have been minted");
        require(num_purchased > 0, "You're trying to give us free money here!");
        require(num_purchased <= 2, "You can't mint that many at once");
        require(preSaleOn, "Presale is not open");
        require(_whiteList[msg.sender].approved == true, "You gotta be on the list buddy");
        ...
        _whiteList[msg.sender].approved = false;
        payable(owner()).transfer(BOI_PRICE.mul(num_purchased)); //handled in the Ownable import
    }
    ...
    function addToWhiteList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            _whiteList[addresses[i]].approved = true;
        }
    }

    function addressInWhitelist(address addr) external view returns (bool) {
        return _whiteList[addr].approved;
    }
    ...
    function removeFromWhiteList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            _whiteList[addresses[i]].approved = false;
        }
    }
    ...
    function resetBaseURI(string memory _uri) public onlyOwner {
        _current_baseURI = _uri;
    }
    ...
    function setPermanentURI(uint16 _counter, string memory _uri) public onlyOwner{
