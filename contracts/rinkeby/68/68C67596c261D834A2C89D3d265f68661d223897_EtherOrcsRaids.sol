...
contract EtherOrcsRaids {
    ...
    struct Campaign { uint16 location; uint64 end; uint176 reward; }
    ...
    function _claim(uint256 orcId) internal {
        Campaign memory cpm = campaigns[orcId]; 

        if (cpm.reward > 0 && _ended(campaigns[orcId])) {
            campaigns[orcId].reward = 0;
            boneShards.mint(commanders[orcId], cpm.reward);
        }
    } 
    ...
    function _startCampaign(uint orcId, uint256 location_,bool double) internal {
        ...
        campaigns[orcId].location  = uint16(location_);
    ...
    function _ended(Campaign memory cmp) internal view returns(bool) {
        return block.timestamp > (giantCrabHealth == 0 ? cmp.end - 1 days : cmp.end);
    }

    function _getReward(Raid memory raid, uint256 orcId, uint16 orcLevel, string memory salt) internal returns(uint176 reward) {
    ...
    function _getBaseOutcome(uint256 minLevel, uint256 maxLevel, uint256 minProb, uint256 maxProb, uint256 orcLevel) internal returns(uint256 prob) {
