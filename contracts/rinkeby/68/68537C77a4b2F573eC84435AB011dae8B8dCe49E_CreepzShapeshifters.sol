...
contract CreepzShapeshifters is Context, ERC721EnumerableBurnable, VRFConsumerBase, Ownable, ReentrancyGuard  {
    ...
    uint256 public constant CREEPZ_SUPPLY = 11111;
    uint256 public constant BASE_RATE_TOKENS = 1;
    uint256 private constant PURCHASE_TIMEOUT = 1 days;
    ...
    function shapePurchase(uint256 tokensToMint, uint256 tokenId) public nonReentrant {
      ...
      require(tokensToMint <= _maxToMint, "You can mint max 10 tokens per transaction");
      ...
      if (_msgSender() != owner()) {
        ...
        require(block.timestamp.sub(_lastPurchased[_msgSender()]) >= PURCHASE_TIMEOUT, "Time limit restriction");
    ...
    function _validateCreepzOwner(uint256 tokenId, address user) internal view returns (bool) {
      ...
      if (tokenId < CREEPZ_SUPPLY) {
        return STAKING.ownerOf(address(CREEPZ), tokenId) == user;
      } else {
        return CREEPZ.balanceOf(user) > 0;
      }
    }
    ...
    function getTokenPrice(address user, uint256 amount) internal view returns (uint256) {
      ...
      if (minted > 0) return _incrementRate.mul(amount);
      if (minted == 0 && amount > BASE_RATE_TOKENS) return _basePrice.add(amount.sub(1).mul(_incrementRate));
      return _basePrice;
    }
