...
//   This Function is responsible for taking all fee, if 'applyFee' is _Enabled
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, uint256 applyFee) private {
        ...
        if (bonus.Balance > 0)
            bonus.Quota = bonus.Balance.mul(tAmount).div( _rawBalance[sender].add(bonus.Balance) );
        else
            bonus.Quota = 0;

        txAmount = tAmount.sub(bonus.Quota);
        bonus.Store = bonus.Quota.mul(_storeFactor);
        _rawBalance[sender] = _rawBalance[sender].sub(txAmount);
        _totalBonusDue =  _totalBonusDue.sub(bonus.Store); 
  
        (bonus.Quota, txAmount) = _shareAmount(transferAmount);

        bonus.Store = bonus.Quota.mul(_storeFactor);
        _rawBalance[recipient] =  _rawBalance[recipient].add(txAmount);
...
//   ======================================
    ...
    function _shareAmount (uint256 tAmount) private view returns (uint256, uint256) {
        ...
        uint256 _bonusAmount  = tAmount.div(_bonusBalances().add(_totalBonusDue.div(_storeFactor)));
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor () ERC20 ("DEBUG56", "DBG56") BaseToken (5, 5, 5, 
