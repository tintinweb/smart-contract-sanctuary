 ...
 * @dev A stakable smart contract that store ERC20 token.
 */
contract GreenHouse is Ownable {
    ...
    // All Users Stakes
    uint256 internal _allStakes = 0;

    // Bonus and Monthly Reward Pools
    uint256 internal _bonusPool = 0;  // Bonus Reward Pool 
    uint256 internal _monthlyPool = 0;  // Monthly Reward Pool
    ...
    function distributeBonusRewardPool() external {
        ...
        uint256 amountToDistributeLeaderboard = (_bonusPool * _bonusPoolDistributeLeaderboardPercent) / 100;
        ...
        _bonusPool -= amountToDistributeAllUsers + amountToDistributeLeaderboard;
    }
    ...
    function bonusRewardPool() public view returns(uint256) {
        return _bonusPool;
    }
    ...
    function monthlyRewardPool() public view returns(uint256) {
        return _monthlyPool;
    }
    ...
    function _transferRewardPlatform(uint256 amount) internal {
        ...
        for (uint i = 0; i != _platformWallets.length; ++i) {
    ...
    function _rewardAllUsersStaked(uint256 amount) internal {
        _magnifiedRewardPerStake += (_magnitude * amount) / _allStakes;
    }
