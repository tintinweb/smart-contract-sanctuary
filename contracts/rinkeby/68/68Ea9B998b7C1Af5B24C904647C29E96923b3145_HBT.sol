...
contract HBT is Context, IBEP20, IBEP20Metadata, Pausable, Ownable{
    ...
    uint256 public feeDivisor = marketingFee + charityFee + buyBackFee;
    ...
    function setMarketingFeePercent(uint256 fee) external onlyOwner whenNotPaused {
        marketingFee = fee;
        feeDivisor = marketingFee + charityFee + buyBackFee;
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tMarketingFee).sub(tCharityFee).sub(tBuyBackFee);
        return (tTransferAmount, tFee, tMarketingFee, tCharityFee, tBuyBackFee);
    }

    function getRValues(uint256 amount, uint256 tFee, uint256 tMarketingFee, uint256 tCharityFee, uint256 tBuyBackFee) internal view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256 currentRate = getRate();
        uint256 tAmount = amount;
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketingFee).sub(rCharityFee).sub(rBuyBackFee);
        return (rAmount, rTransferAmount, rFee, rMarketingFee, rCharityFee, rBuyBackFee);
    }
    ...
    function removeAllFee() internal {
        if(taxFee == 0 && marketingFee == 0 && charityFee == 0 && buyBackFee == 0) return;
    ...
    function swapFee(address from) internal {
        ...
        if(from != uniswapV2Pair && contractTokenBalance >= minimumTokensBeforeSwap) {
            ...
            transferETHToAddress(payable(marketingWallet), transferredBalance.mul(marketingFee).div(feeDivisor));

            // Send eth to Charity address
            transferETHToAddress(payable(charityWallet), transferredBalance.mul(charityFee).div(feeDivisor));

            // Buy back tokens using eth
            swapETHForTokens(address(this), buyBackWallet, transferredBalance.mul(buyBackFee).div(feeDivisor));
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingFee, uint256 tCharityFee, uint256 tBuyBackFee) = getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rMarketingFee, uint256 rCharityFee, uint256 rBuyBackFee) = getRValues(tAmount, tFee, tMarketingFee, tCharityFee, tBuyBackFee);
        ...
        takeBuyBackFee(rBuyBackFee, tBuyBackFee);
    ...
    function takeBuyBackFee(uint256 rBuyBackFee, uint256 tBuyBackFee) internal {
        _rOwned[address(this)] = _rOwned[address(this)].add(rBuyBackFee); 
        if(_isExcluded[address(this)]){
            _tOwned[address(this)] = _tOwned[address(this)].add(tBuyBackFee);
        }
        buyBackBalance += tBuyBackFee;
    }
