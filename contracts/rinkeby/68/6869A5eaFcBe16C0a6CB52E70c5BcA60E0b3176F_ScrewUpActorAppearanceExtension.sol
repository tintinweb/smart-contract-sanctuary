...
contract ScrewUpActorAppearanceExtension is ScrewUpInGameActorExtension,IScrewUpActorAppearanceExtension {
    ...
    function getPartsMetadata(uint256[] memory partIds) external view virtual override returns (FAppearancePart[] memory){ return _getPartsMetadata(partIds);}
    function getAllPartsMetadata() external view virtual override returns (FAppearancePart[] memory){return _getPartsMetadata(_partIds);}
    ...
    function _getPartsMetadata(uint256[] memory partIds) internal view returns (FAppearancePart[] memory){
        uint256 partCount = 0;
        for(uint256 i = 0; i < partIds.length; i++){
            if(_parts[partIds[i]].ValueType != EValueType.None)
                partCount++;
        }
        ...
        for(uint256 i = 0; i < partIds.length; i++)
        {
            if(_parts[partIds[i]].ValueType != EValueType.None)
            {
                _metas[counter] = _parts[partIds[i]];
                counter++;
            }
        }
        return _metas;
...
interface IScrewUpActorAppearanceExtension{
    ...
    function getAllPartsMetadata() external view returns (FAppearancePart[] memory);
    function getPartsMetadata(uint256[] memory partIds) external view returns (FAppearancePart[] memory);
