...
contract PSH is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("PSH", "PSH") {}

    uint256 private constant MAX_QUANTITY = 3000;
    uint256 private constant RESERVED = 100;
    uint256 private constant MAX_PUBLIC = MAX_QUANTITY - RESERVED;
    uint256 private constant GEN_LIMIT = 2;
    uint256 private itemPrice = 0.01 ether;
    ...
    address cEthAddress = 0xF4c9DD03C74798daC0Ff81A8C243828dBCDe04f9;

    function mintSale(uint _numTeams) external payable{

        require(canBuy == true, 'Public sale not open');
        require(_numTeams > 0 && _numTeams <= GEN_LIMIT, "Invalid PSH count");
        require(_tokenIdTracker.current() + _numTeams <= MAX_PUBLIC, "All PSHs have been minted");
        require(_numTeams * itemPrice == msg.value, "Incorrect amount of ether sent");
             require(_numTeams == 2, "You can only buy max 2 items");


       for (uint i = 0; i < _numTeams; i++) {
            _reservedTokenIdTracker.increment();
            _safeMint(msg.sender, MAX_PUBLIC + _tokenIdTracker.current());
        }
    ...
    function mintPreSale(uint _numTeams) external payable{
                require(isWhiteListed(msg.sender), 'Address not on Reserve list');

        require(preMint == true, 'Presale not open');
        require(_tokenIdTracker.current() + _numTeams <= MAX_PUBLIC, "All PSHs have been minted");
        require(_numTeams * itemPrice == msg.value, "Incorrect amount of ether sent");
             require(_numTeams == 1, "You can only buy max 2 items");
    ...
    function mintReserve(uint _numTeams) external {
        require(isReserveListed(msg.sender), 'Address not on Reserve list');
        require(_reservedTokenIdTracker.current() + _numTeams <= RESERVED, "All reserved PSHs have been minted");
        require(_numTeams == 1, "You can only buy max 1 items for free");
