...
contract NFTAuction is Ownable, Context{
  ...
  function changeTrustedForwarder(address newForwarder) external  onlyOwner() {
        ...
        isAuctionNotStartedByOwner( _tokenId)
    {
        ...
        require(IERC1155(_nftContractAddress).balanceOf(_msgSender(),_tokenId) != 0,  "Not Allowed : Sender is not owner");
    ...
    external {
     require(escrowOwner[msg.sender], "Not Allowed: Msg sender should be escrow Owner");
     IERC1155(_nftContractAddress).safeTransferFrom(
                address(this),
                _newOwner,
                _tokenId,
           ...
           if(nftContractAuctions[_nftContractAddress][_tokenId] == _newOwner){
               nftContractAuctions[_nftContractAddress][_tokenId] = address(0);
           }
