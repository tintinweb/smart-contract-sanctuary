...
contract Pionner is Ownable, ERC721 {
    ...
    function pionnerMint(bytes memory signature, string memory key) public returns (uint256) {
        require(checkSignature(signature) == true, "SIGNATURE DENIED");
        require(keccak256(abi.encodePacked(key)) == keccak256(abi.encodePacked("L1")) || 
          keccak256(abi.encodePacked(key)) == keccak256(abi.encodePacked("L2")), "ONLY L1 OR L2"
        );
        string memory tokenURI = '';
        if (keccak256(abi.encodePacked(key)) == keccak256(abi.encodePacked("L1"))) {
            tokenURI = "https://ipfs.io/ipfs/QmYqCbFsbAVxDkW85iwsUT74swBLxo7oKQZuLR7axzJrM7";
        } else{
            tokenURI = "https://ipfs.io/ipfs/QmQXN4VUFVQZ3kT6isPNxYUbo1N64UCJAihgrWejuM6Vqp";
        }
