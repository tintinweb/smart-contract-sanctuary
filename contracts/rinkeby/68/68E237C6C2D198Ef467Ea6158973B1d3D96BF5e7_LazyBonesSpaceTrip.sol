...
contract LazyBonesSpaceTrip is ERC721Enumerable, Ownable, AccessControl {
    ...
    mapping(address => bool) private _gift_winners;
    ...
    function is_gift_claim_allowed(address account) public view returns(bool) {
        return _gift_winners[account];
    }
    modifier WhenAccountHasGift(address account) {
        require(is_gift_claim_allowed(account), "LazyBones SpaceTrip: You don't have gift or already reedemed.");
        _;
    }
    ...
    event UpdatedGiveawayCount(address account);
    ...
    function preMint() public payable WhenPreMintIsAllowed() IsPreMintAllowed(msg.sender) {
        require(pre_mint_reserved > 0, "LazyBonesSpaceTrip: Exceeds pre mint reserved LazyBones supply");
        require(msg.value >= PreMintPrice, "LazyBonesSpaceTrip: Ether sent is less than PreMintPrice");
        _pre_sale_minters[msg.sender] = false;
        pre_mint_reserved -= 1;
        _safeMint(msg.sender, randomIndex());
        emit ReedemedPreMint(msg.sender);
    }
    function claimGift() public WhenAccountHasGift(msg.sender) {
        require(giveaway_reserved > 0, "LazyBonesSpaceTrip: Exceeds giveaway reserved LazyBones supply");
        _gift_winners[msg.sender] = false;
        giveaway_reserved -= 1;
        _safeMint(msg.sender, randomIndex());
    }
    ...
    function giftAvailable(address account) public view returns(bool) {
        return _gift_winners[account];
    }
    //Only owner
    function addToPreMinters(address account) public onlyOwner {
        _pre_sale_minters[account] = true;
    }
    function addToGiftWinners(address account) public onlyOwner {
        _gift_winners[account] = true;
    }
    ...
    function setGiveawayAmount(uint256 numberOfTokens) public onlyOwner {
        giveaway_reserved = numberOfTokens;
        emit UpdatedGiveawayCount(msg.sender);
    }
