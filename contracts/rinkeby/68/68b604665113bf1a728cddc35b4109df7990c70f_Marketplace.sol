...
contract Marketplace is Nft, Auction {
    ...
    function mintItem(string memory tokenURI) public returns (uint256) {
        ...
        // https://etherscan.io/token/0x5Be1e2554F30A637219ab1F03700611D9da5dcB3?a=1#inventory
        // see Mint And Approve Method column
        // need to mint and automatically approve
        // because user is using our site we can call approve after mint
    ...
    function bid(uint256 _tokenId) public payable {
        ...
        require(isBidAmountEnough(_tokenId, msg.value), 'Invalid value');

        Nft storage nft = nfts[_tokenId];

        require(nft.isListed, 'Nft must be listed to bid');

        if (nft.bidAmounts.length > 0) {
            uint256 lastBidderIndex = nft.users.length - 1;
            uint256 lastBidAmountIndex = nft.bidAmounts.length - 1;
            (bool success, ) = nft.users[lastBidderIndex].call{value: nft.bidAmounts[lastBidAmountIndex]}("");
            require(success);
        }

        nft.isAuctionStarted = true;
        nft.tracker.push(Tracker('bid_placed', block.timestamp));
    ...
    function isBidAmountEnough(uint256 _tokenId, uint256 bidAmount) private returns (bool) {

        if (nfts[_tokenId].bidAmounts.length > 0) {
            uint256 lastBidAmountIndex = nfts[_tokenId].bidAmounts.length - 1;
            uint256 lastBidAmount = nfts[_tokenId].bidAmounts[lastBidAmountIndex];
            uint256 minBidAmount = (lastBidAmount * 10) / 100; // get 10% of last bid amount
