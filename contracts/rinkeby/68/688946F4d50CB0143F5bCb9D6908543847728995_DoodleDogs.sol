...
contract DoodleDogs is ERC721, Ownable {
    ...
    // MAX_SUPPLY is 9,999. We use < because it's cheaper than <=, gas-wise.
    // So MAX_SUPPLY is 9,999 + 1
    uint internal constant MAX_SUPPLY = 10_000;
    ...
    event FallbackHit(address indexed _who, uint indexed _amount);

    // Constructor

    constructor() ERC721("Doodle Dogs", "DOODLEDOGS") {}

    // Public Functions

    function mint(uint _amount) public payable {
        require(mintEnabled, 'Mint paused');
        require(doodleDogsMinted() + _amount < MAX_SUPPLY, 'Exceeds max Dog supply');
    ...
    // Emits a simple event if anyone sends funds straight to the contract
