    ...
    Royalty
{
    ...
    // Twinned
    address public twinContractAddress;
    mapping(uint256 => uint256) private _twin;
    ...
    event TwinContractAddress(address approved);
    event PresaleBegins();
    event SaleBegins();
    event SaleEnds();
    ...
    function setFirstProvenanceHash(bytes32 _provenanceHash)
        ...
        returns (uint256)
    {
        // Once firstProvenanceHash is set it is impossible to change it and the randomSeed
    ...
    function burn(uint256 tokenId) public {
        require(
            _isApprovedOrOwner(msg.sender, tokenId) ||
                msg.sender == twinContractAddress,
    ...
    function setTwinContractAddress(address _approvedAddress)
        external
        onlyOwner
    {
        twinContractAddress = _approvedAddress;
        emit TwinContractAddress(_approvedAddress);
    }
    ...
    // Twinned
    function setTwinId(uint256 _tokenId, uint256 _twinId) external {
        require(msg.sender == twinContractAddress, "Caller is not approved");
        require(_exists(_tokenId), "Twin operation for nonexistent token");
        _twin[_tokenId] = _twinId;
    }

    function getTwinId(uint256 _tokenId)
        external
        view
        returns (uint256 twinId)
    {
        require(_exists(_tokenId), "Twin query for nonexistent token");
        return _twin[_tokenId];
    }
        ...
        returns (bool)
    {
        ...
        if (eligibleTokensCount != contains.length) return false;
