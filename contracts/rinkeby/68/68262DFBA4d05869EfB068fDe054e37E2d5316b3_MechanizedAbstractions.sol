...
contract MechanizedAbstractions is Ownable, ERC721URIStorage, VRFConsumerBase {
    ...
    uint256[] private _vrfValues;
    uint256[] public seedValues;
    ...
    */
    ...
    function _mintSingleToken(TokenData memory token) private {
        uint256 vrfValuesIndex = (token.id.div(batchSize));
        require(vrfValuesIndex < _vrfValues.length, "NO VRF VALUE EXISTS FOR TOKEN");

        _safeMint(token.sender, token.id);

        uint256 _vrfValue = _vrfValues[vrfValuesIndex];            
        seedValues.push(uint256(keccak256(abi.encode(_vrfValue, token.timestamp, token.blockHash, token.sender, token.id))));     
    ...
    function _doMint(uint256 numberOfTokens) private {
        for (uint i = 0; i < numberOfTokens; i++) {
            ...
            uint256 vrfValuesIndex = (tokenData.id.div(batchSize));
            if (vrfValuesIndex >= _vrfValues.length) {
    ...
    */
    ...
    /**
     * @dev returns the amount of NFTs that have been revealed
     */
    function lastRevealed() external view returns(uint256) {
        return seedValues.length;
    }
