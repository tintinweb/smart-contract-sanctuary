...
contract CapitalPool is ICapitalPool, OwnableUpgradeable, AbstractDependant {
    ...
    uint256 public liquidityCushionWindowSize;
    ...
    event PoolBalancesUpdated(
        ...
        uint256 leveragePoolBalance
    );

    event LiquidityCushionLocked(uint256 startTime, uint256 endTime);
    ...
    function addPolicyHoldersHardSTBL(
        ...
        if (factors.userLeveragePoolPremium > 0) {
            userLeveragePool.addPolicyPremium(
                _epochsNumber,
                DecimalsConverter.convertTo18(factors.userLeveragePoolPremium, stblDecimals)
            );
        }

        return DecimalsConverter.convertTo18(factors.coveragePoolPremium, stblDecimals);
    }
    ...
    {
        address _policyBookAddress = _msgSender();
        factors.poolUtilizationRation = (IPolicyBook(_policyBookAddress)).totalCoverTokens();

        if (factors.poolUtilizationRation > 0 && factors.vStblOfCP > 0) {
            factors.poolUtilizationRation = factors.poolUtilizationRation.mul(PERCENTAGE_100).div(
                factors.vStblOfCP
            );
        } else {
            factors.poolUtilizationRation = 0;
        }

        factors.participatedlStblDeployedByLP = 0;
    ...
    function rebalanceLiquidityCushion() public override broadcastBalancing onlyMaintainer {
        ...
        (uint256 _startTime, uint256 _endTime) = _calcRebalancingTimeWindow(block.timestamp);

        uint256 _startIndex = liquidityRegistry.withdrawlListCounter();
        (, , uint256 _requiredLiquidity, , uint256 _newCounter) =
            liquidityRegistry.getWithdrawalRequestsInWindowTime(_startIndex, _startTime, _endTime);
        ...
        (uint256 _deposit, uint256 _withdraw) =
            getDepositAndWrtthdraw(
                _requiredLiquidity,
                liquidityCushionBalance,
                hardUsdtAccumulatedBalance
            );
        ...
        liquidityRegistry.adjustWithdrawlListCounter(_newCounter);

        emit LiquidityCushionLocked(_startTime, _endTime);
    }
    ...
    function setLiquidityCushionPeriods(uint256 _duration, uint256 _size)
        external
        override
        onlyOwner
    {
        require(_duration > 0, "CP: duration can not be zero");
        require(_size > 0, "CP: duration can not be zero");
        liquidityCushionDuration = _duration;
        liquidityCushionWindowSize = _size;

        uint256 index = liquidityRegistry.findWithdrawlAfterTime(block.timestamp.sub(_duration));
        liquidityRegistry.adjustWithdrawlListCounter(index);
    }
    ...
    function getDepositAndWrtthdraw(
        uint256 _requiredLiquidity,
        uint256 _hardUSDT,
        uint256 _cushionBalance
    ) public view returns (uint256 deposit, uint256 withdraw) {
        uint256 _availableBalance = _hardUSDT.add(_cushionBalance);
    ...
    function _calcRebalancingTimeWindow(uint256 _time)
        internal
        view
        returns (uint256 _startTime, uint256 _endTime)
    {
        require(_time > liquidityCushionDuration, "LC: too early to rebalance");
        _startTime = _time.sub(liquidityCushionDuration);
        _endTime = _startTime.add(liquidityCushionWindowSize);
    }
}
