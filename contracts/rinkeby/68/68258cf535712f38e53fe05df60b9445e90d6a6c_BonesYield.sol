...
contract BonesYield is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function computeUnclaimedReward(address _contractAddress, uint256 _tokenId) internal view returns (uint256) {
        ...
        if (lastClaimDate != uint256(0)) {
            ...
            uint256 totalReward = computeAccumulatedReward(contractSettings[_contractAddress].start, baseRate, block.timestamp);
    ...
    function computeAccumulatedReward(uint256 _lastClaimDate, uint256 _baseRate, uint256 _currentTime) internal view returns (uint256) {
        require(_currentTime > _lastClaimDate, "Last claim date must be smaller than block timestamp");

        uint256 secondsElapsed = _currentTime - _lastClaimDate;
    ...
    function deposit(uint256[] calldata _tokenIds) external {
        require(isStakingActive, "Staking is not active");
        //        require(msg.sender != stakingContractAddress, "Invalid address");de ce?
