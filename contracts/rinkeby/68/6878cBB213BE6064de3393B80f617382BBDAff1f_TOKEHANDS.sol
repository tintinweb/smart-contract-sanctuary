...
contract TOKEHANDS is ERC721, Ownable {
    ...
    address private devAddress = 0xf4e92382bd7E260ce55A9a18bf4Ad05f05f319E8;
    address private royaltyReceiver =
        0xf4e92382bd7E260ce55A9a18bf4Ad05f05f319E8;

    //TODO Update this
    uint256 public cost = 0.1 ether;
    uint256 public maxSupply = 100;
    // 2/2/2022 2:22:22pm ET
    // uint256 private saleDate = 1643829722; // true date
    uint256 private saleDate = 1642822006; // dev date
    bool public revealed = false;
    mapping(uint256 => bool) public redeemed;

    constructor() ERC721("TOKEHANDS", "TKHN") {
        setHiddenMetadataUri(
            "ipfs://QmeBn9zQyykXfQk1TUXTMmDSj5rCyWjPekc5yKVVY6nujL"
        );
    }

    event NewTokeHandMinted(address sender, uint256 tokenId);

    function getLargeWalllet() public view returns (uint256) {
        return saleDate + 5 days;
    }

    function getMaxPerWallet() public view returns (uint256) {
        //todo check this date fn timing works
        if (saleDate + 5 days >= block.timestamp) {
            return 5;
        } else {
            return 5;
        }
        ...
        mintCompliance(msg.sender, _qty)
    {
        ...
        if (!isMintActive()) revert NotOnSaleYet();
        if (ownerTokenCount + _qty >= getMaxPerWallet()) revert MaxWallet();
        if (msg.value < cost * _qty) revert LowFunds();
        _mintLoop(msg.sender, _qty);
    }

    function dualMint() public payable mintCompliance(msg.sender, 2) {
        uint256 ownerTokenCount = balanceOf(msg.sender);
        if (!isMintActive()) revert NotOnSaleYet();
        if (ownerTokenCount + 2 >= getMaxPerWallet()) revert MaxWallet();
        if (msg.value < cost * 2) revert LowFunds();
        _mintLoop(msg.sender, 2);
    }

    function mintForAddress(address _receiver, uint256 _qty)
        public
        mintCompliance(_receiver, _qty)
        onlyOwner
    {
        _mintLoop(_receiver, _qty);
    }
    ...
    function setRoyaltyAddress(address _royaltyAddress) public onlyOwner {
        royaltyReceiver = _royaltyAddress;
    }

    // Redeeming for tattoo
    // Maybe not only owner? Maybe owner of the token has to call this
    function redeemForTokenId(uint256 _tokenId) public onlyOwner {
    ...
    function qtyMinted() public view returns (uint256) {
    ...
    function withdraw() public onlyOwner {
        if (devAddress == address(0)) revert();
        if (address(this).balance > 0) revert LowFunds();
        // dev 5% of the initial sale.
        (bool devPayout, ) = payable(devAddress).call{
            value: (address(this).balance * 5) / 100
        }("");
        if (!devPayout) revert PayoutFailed();

        // Remaining to owner
        (bool ownerPayout, ) = payable(owner()).call{
            value: address(this).balance
        }("");
        if (!ownerPayout) revert PayoutFailed();
    }
