...
contract Bridge is Ownable, ReentrancyGuard {
    ...
    modifier noContractsAllowed() {
        require(!(isContract(address(msg.sender))) && tx.origin == msg.sender, "No Contracts Allowed!");
        _;
    }
    ...
    function withdraw(uint256 _amount, uint256 _chainId, uint256 _nonce, string memory _txHash, bytes32 _signHash, uint8 _v, bytes32 _r, bytes32 _s) external noContractsAllowed nonReentrant {
        ...
        require(verifySignature(_signHash, _v, _r, _s), "Invalid Signature");
        ...
        emit Withdrawal(msg.sender, _amount, _txHash, _nonce, chainId);
    }

    function verifySignature(bytes32 _signHash, uint8 _v, bytes32 _r, bytes32 _s) internal view returns(bool) {
        return (ecrecover(_signHash, _v, _r, _s) == verifierAddress);
	}
	...
	function checkLatestWithdrawalInForeignChain(address _address) public view returns (uint256, uint256, uint256, address, string memory, bool) {
