...
contract PerpetualTreasury is PerpetualRebalanceFunctions, IFunctionList, IPerpetualTreasury {
     ...
     * @param   _fTokenAmount   The amount of token to add. 64.64 float
     */
    ...
    function addAMMLiquidityToPerpetual(bytes32 _iPerpetualId, int128 _fTokenAmount) external override nonReentrant onlyAmmGovernance {
        ...
        emit UpdateAMMFundCash(_iPerpetualId, perpetual.fAMMFundCashCC, pool.fAMMFundCashCC, block.timestamp);
    }
     ...
     * @param   _fMinTokenAmount    The amount of token to return. 64.64 fixed point number.
     */
    function removeLiquidity(
        uint256 _poolId,
        int128 _fShareAmount,
        ...
        if (_fMinTokenAmount > 0) {
            require(fTokenAmountToReturn >= _fMinTokenAmount, "token amount to return is too low");
        }
     ...
     */
    function _checkPoolState(LiquidityPoolData storage _pool) internal {
        require(_pool.isRunning, "pool is not running");
        ...
        require(isActivePerpetuals, "active perpetuals not found");
    }
    ...
    /**
     * @notice Calculates amount of share tokens user has withdraw during a withdrawal period.
     *
     * @param   _pool            Reference to liquidity pool.
     * @param   _user            The address of user account.
     */
    ...
    function _checkWithdrawalRestrictions(
        ...
        if (_pool.iPnLparticipantWithdrawalPeriod > 0) {
            ...
            require(fAmountForPeriod.add(_fAmount) <= fAmountLimit, "withdrawal limit exceeded");
        }
    }
    ...
    function addAmmGovernanceAddress(address _gAddress) external override onlyOwner {
        require(_gAddress != address(0), "PerpetualTreasury::addAmmGovernanceAddress cannot add 0 address");
    ...
    modifier onlyAmmGovernance() {
        require(ammGovernanceAddresses.contains(msgSender()), "PerpetualTreasury::onlyGovernance address allowed");
        _;
    }
