...
contract CoinDeed is ICoinDeed, AccessControl, ReentrancyGuard {
    ...
    IOracle public tokenAOracle;
    IOracle public tokenBOracle;
    ...
    function _checkIfDeedIsReady() internal {
        ...
        // MAINNET:
        // FeedRegistryInterface feedRegistry = FeedRegistryInterface(coinDeedFactory.feedRegistryAddress());
        // uint256 answer = uint256(feedRegistry.latestAnswer(pair.tokenA, Denominations.USD));
        // uint8 decimals = feedRegistry.decimals(pair.tokenA, Denominations.USD);

        // if (
        //     totalStake >=
        //     deedParameters.deedSize *
        //     answer /
        //     decimals *              // Oracle Price in USD
    ...
    function exitDeed() onlyInState(DeedState.OPEN) public nonReentrant override {
        ...
        uint256 tokenAForBuyer = _sell(exitAmount);
    ...
    function setTokenOracle(address tokenAOracle_, address tokenBOracle_) external onlyRole(MANAGER) {
        tokenAOracle = IOracle(tokenAOracle_);
        tokenBOracle = IOracle(tokenBOracle_);
    }
    ...
    function withdrawStake() external override {
        require(state != DeedState.READY && state != DeedState.OPEN, "Deed is not in correct state");
        require(stakes[msg.sender] > 0, "No stake");
        require(state == DeedState.CLOSED || msg.sender != manager, "Can not withdraw your stake.");
        require(state != DeedState.SETUP || executionTime.recruitingEndTimestamp < block.timestamp, "Recruiting did not end.");
        ...
        if (state == DeedState.CLOSED) {
            ...
            deedToken.safeTransfer(coinDeedFactory.treasuryAddress(), platformFee);
        }
        ...
        // deedToken.safeTransfer(msg.sender, stakes[msg.sender]);
        stakes[msg.sender] = 0;
    ...
    function _payOff(uint256 amount) internal {
        ...
        //     deedToken.safeTransfer(msg.sender, diff);
        ...
        uint256 diff;
        if (amount > buyerLoan) {
            diff = amount - buyerLoan;
            amount -= buyerLoan;
        }
        buyIns[msg.sender] = buyIns[msg.sender] * (buyerLoan - amount) / buyerLoan;
    ...
    function claimBalance() onlyInStates([DeedState.CLOSED, DeedState.CANCELED]) external nonReentrant override {
    ...
    /**
     * Brokers and managers should receive their staked deedToken balance.
     * From the management fee total, platformFee % is deducted and sent to treasury
     * The rest is distributed proportionally to each broker according to their stakes
     **/
    ...
    //     // uint256 feeInDeedToken = _tokenPrice(pair.tokenA, fee, address(deedToken));
    //     IERC20(pair.tokenA).safeTransfer(msg.sender, fee);
    ...
    function _tokenPrice(
        ...
        // if (address(tokenAOracle) == address(0)) {
            // RINEKBY:
            uint256 tokenAPrice = uint256(tokenAOracle.latestAnswer());
            uint256 tokenBPrice = uint256(tokenBOracle.latestAnswer());

            tokenBAmount = tokenAAmount * (10 ** (18 - IERC20Metadata(tokenA).decimals()))
                * tokenAPrice
                / tokenBPrice
                / (10 ** (18 - IERC20Metadata(tokenB).decimals()));
        // } else {
            // MAINNET
        //     FeedRegistryInterface feedRegistry = FeedRegistryInterface(coinDeedFactory.feedRegistryAddress());
    ...
    function executeRiskMitigation() external override onlyInState(DeedState.OPEN) {
        ...
        uint256 actualTotalLoan = lendingPool.pendingBorrowBalance(pair.tokenA, address(this));
        uint256 actualTotalLoanInTokenB = _tokenPrice(pair.tokenA, actualTotalLoan, pair.tokenB);
    ...
    function _validateTokens(Pair memory pair) internal view {
        FeedRegistryInterface feedRegistry = FeedRegistryInterface(coinDeedFactory.feedRegistryAddress());
        require(
