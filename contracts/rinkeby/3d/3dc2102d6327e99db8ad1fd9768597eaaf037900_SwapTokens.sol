...
contract SwapTokens is Ownable {
    ...
    constructor(address _tokenFrom, address _tokenTo, uint256 _tokenFromTotal, uint256 _tokenToTotal, address _taxReceiver, uint256 taxOffsetDays) {
        ...
        taxStart = block.timestamp.sub(taxOffsetDays*24*60*60);
    ...
    function swapTokens(uint256 _amount) public {
        ...
        uint256 totalAmount = _amount.mul(tokenToTotal).div(tokenFromTotal);
        uint256 userAmount = totalAmount;
        
        if(isTaxEnabled && taxRate() > 0) {
            uint256 taxAmount = totalAmount.mul(taxRate()).div(100);
        ...
        swappedAmount[msg.sender] = swappedAmount[msg.sender].add(totalAmount);
    }
    ...
    function calculateTokens(uint256 _amount) public view returns(uint256) {
        ...
        if(isTaxEnabled && taxRate() > 0) {
            uint256 taxAmount = receiveAmount.mul(taxRate()).div(100);
            receiveAmount = receiveAmount.sub(taxAmount);
        }
        return receiveAmount;
    }
    
    function taxRate() public view returns(uint256) {
        uint256 daysPassed = (block.timestamp.sub(taxStart)).div(24*60*60);
        uint256 calculatedTaxRate = 0;
        if(daysPassed < taxPeriod()) {
            calculatedTaxRate = uint256(100).sub((daysPassed).mul(dailyTaxRate));
        }
    ...
    function taxPeriod() public view returns(uint256) {
        uint256 period = uint256(100).div(dailyTaxRate);
        return period;
    }
