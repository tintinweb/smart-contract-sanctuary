...
contract ERC721Tradable is ERC721Full, Ownable {
    ...
    function _resetTokenURI(uint256 tokenId) internal {
        require(_exists(tokenId), "ERC721: nonexistent token");
        delete _tokenURIs[tokenId];
    }
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        ...
        // If _tokenURI is set, return _tokenURI
        if (bytes(_tokenURI).length > 0) {
...
// File contracts/EveripediaNFT.sol

pragma solidity ^0.5.0;
/**
 * @title EveripediaNFT
 * EveripediaNFT - a contract for NFTs .
 */
contract EveripediaNFT is ERC721Tradable {
    constructor(address _proxyRegistryAddress, string memory collectionName, string memory tokenName, string memory baseURI)
    public
    ERC721Tradable(collectionName, tokenName, baseURI, _proxyRegistryAddress)
    {
    }
}