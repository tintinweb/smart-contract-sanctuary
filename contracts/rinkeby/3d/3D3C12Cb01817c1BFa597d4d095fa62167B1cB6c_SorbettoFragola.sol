...
contract SorbettoFragola is ERC20Permit, ReentrancyGuard, ISorbettoFragola {
    ...
    constructor(address _pool, address _strategy)
        ERC20("Popsicle LP V3 USDT/WETH", "PLP")
        ERC20Permit("Popsicle LP V3 USDT/WETH")
    {
    ...
    function init() external onlyGovernance {
        ...
        int24 baseThreshold = tickSpacing *
            ISorbettoStrategy(strategy).tickRangeMultiplier();
        ...
        updateVault(address(0))
    {
        ...
        uint160 exactSqrtPriceImpact = sqrtPriceX96.mul160(
            ISorbettoStrategy(strategy).priceImpactPercentage() / 2
        ) / GLOBAL_DIVISIONER;
        ...
        returns (uint256 shares)
    {
        shares = totalSupply() == 0
            ? uint256(liquidity)
            : uint256(liquidity).mul(totalSupply()).unsafeDiv(uint256(liquidityLast));
    }
    ...
    function _earnFees() internal returns (uint256 userCollect0, uint256 userCollect1) {
        ...
        uint256 feeToProtocol0 = collect0
            .mul(ISorbettoStrategy(strategy).protocolFee())
            .unsafeDiv(GLOBAL_DIVISIONER);
        uint256 feeToProtocol1 = collect1
            .mul(ISorbettoStrategy(strategy).protocolFee())
            .unsafeDiv(GLOBAL_DIVISIONER);
     ...
     */
    function collectFees(uint256 amount0, uint256 amount1)
        external
        nonReentrant
        updateVault(msg.sender)
    {
    ...
    modifier checkDeviation() {
        pool.checkDeviation(
            ISorbettoStrategy(strategy).maxTwapDeviation(),
            ISorbettoStrategy(strategy).twapDuration()
        );
        _;
    }
    ...
    function _tokenPerShare(uint256 collected, uint256 tokenPerShareStored)
        internal
        view
        returns (uint256)
    {
