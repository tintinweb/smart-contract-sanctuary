...
contract FacaNFT is FacaBase, VRFConsumerBase {
  using SafeMath for uint256;
  event FacaNFTRandomnessRequest(uint timestamp);
  event FacaNFTRandomnessFulfil(bytes32 requestId, uint256 tokenId);
  ...
  constructor(
    ...
    FacaBase(_tokenName, _tokenSymbol, _privateSalePrice, _publicSalePrice, _maxAirdrop, _maxSupply, _limitMint)
  ...
  function requestChainlinkVRF() public onlyOwner {
    ...
    requestedVRF = true;
    emit FacaNFTRandomnessRequest(now);
  }
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 randomNumber)
    ...
    emit FacaNFTRandomnessFulfil(_requestId, seed);
  }
...
contract FacaBase is Ownable, ERC721 {
    using SafeMath for uint256;
