...
contract ERC2981Factory is NiftyEntity {
    ...
    constructor(address _niftyRegistryContract)
        NiftyEntity(_niftyRegistryContract)
    {
    ...
    function createTokenRoyaltyInfo(address _tokenAddress)
        public
        onlyValidSender
        returns (address)
    {
...
abstract contract NiftyEntity {
    ...
    modifier onlyValidSender() {
        bool isValid = INiftyRegistry(niftyRegistryContract).isValidNiftySender(
            msg.sender
        );
...
contract ERC2981 is ERC165, IERC2981, Initializable, NiftyEntity {
    ...
    function setTokenRoyaltyRange(
        ...
        for (uint256 i = startTokenId; i <= endTokenId; i++) {
            setTokenRoyalty(i, recipient, value);
        }
    }
