...
import "./interfaces/IBNBStakingRewards.sol";

/**
 * @title Phase 2 FLEX Reward Pool for Flex Protocol, specifically for ETH.
 * @notice This contract is used to reward `rewardToken` when ETH is staked.
 */
contract BNBPhase2Pool is IBNBStakingRewards, Context, AccessControl, RewardDistributionRecipient, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  function totalSupply() public override(IBNBStakingRewards) view returns (uint256) {
    return _totalSupply;
  }

  function balanceOf(address account) public override(IBNBStakingRewards) view returns (uint256) {
    return _balances[account];
  }

  function lastTimeRewardApplicable() public override(IBNBStakingRewards) view returns (uint256) {
    return Math.min(block.timestamp, periodFinish);
  }

  function rewardPerToken() public override(IBNBStakingRewards) view returns (uint256) {
    if (totalSupply() == 0) {
  ...
  function earned(address account) public override(IBNBStakingRewards) view returns (uint256) {
    return
      balanceOf(account)
  ...
  function getRewardForDuration() external override(IBNBStakingRewards) view returns (uint256) {
  ...
  function stake(uint256 amount)
    public
    virtual
    override(IBNBStakingRewards)
    payable
    updateReward(msg.sender)
  {
    require(amount > 0, "BNBPhase2Pool/ZeroStake");
    require(amount == msg.value, "BNBPhase2Pool/IncorrectEth");
  ...
  function withdraw(uint256 amount) public override(IBNBStakingRewards) updateReward(msg.sender) {
    require(amount > 0, "BNBPhase2Pool/ZeroWithdraw");
    _totalSupply = _totalSupply.sub(amount);
    _balances[msg.sender] = _balances[msg.sender].sub(amount);

    emit Withdrawn(msg.sender, amount);
    (bool success, ) = msg.sender.call{value: amount}("");
    require(success, "BNBPhase2Pool/EthTransferFail");
  }

  function exit() external override(IBNBStakingRewards) {
    withdraw(balanceOf(msg.sender));
    getReward();
  }

  function getReward() public virtual override(IBNBStakingRewards) updateReward(msg.sender) {
    ...
    updateReward(address(0))
  {
    ...
    require(
      ...
      "BNBPhase2Pool/LowRewardBalance"
    );
  ...
  function recoverERC20(address tokenAddress, uint256 tokenAmount) external {
    require(
      ...
      "BNBPhase2Pool/HasRecoverRole"
    );
    require(
      tokenAddress != address(rewardToken), 
      "BNBPhase2Pool/NotReward"
    );
