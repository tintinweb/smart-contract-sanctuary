...
library Constants {
    // The maximum number of passes which will ever be able to be minted.
    uint256 constant MAX_PASSES = 10000;
}
...
// RecPass generates all of the passes for the rec platform.
// Each pass is rendered as a dynamic SVG and can change over time.
contract RecPass is ERC721, Ownable {
  using Counters for Counters.Counter;

  // Keeps track of the number of member tokens minted thus far.
  Counters.Counter private _memberCounter;

  // The maximum number of memberships that may ever be minted.
  uint256 public memberLimit;

  // The active address of the render contract.
  address public rendererAddress;

  // Whether or not recpasses can be traded currently.
  bool public tradable;

  // A mapping of token to role.
  // Roles will reset if transferred.
  mapping(uint256 => Types.Role) public tokenRoles;

  // An arbitrary score for the given token -- increases if incremented;
  // Scores always reset if transferred
  mapping(address => uint256) public score;

  constructor(
    address _initialRenderer
  ) public ERC721("RecPass", "REC") Ownable() {
    // Seed the first tokens;
    rendererAddress = _initialRenderer;
    _memberCounter.increment();
  }

  // ============ PUBLIC READ-ONLY FUNCTIONS ============

  // Returns the role of the given token.
  function getRole(uint256 tokenID) public view returns (Types.Role) {
  ...
  modifier canMintPass() {
    require(
      _memberCounter.current() + 1 < memberLimit,
      "No member passes currently able to mint"
    );
  ...
  modifier onlyWhenTradable() {
    require(tradable, "RecPas: cannot trade");
    _;
  }
  ...
  function setRole(uint256 tokenID, Types.Role role) public {
    tokenRoles[tokenID] = role;
  }

  function setRoleMulti(uint256[] calldata tokenIDs, Types.Role role) external {
    for (uint256 i = 0; i < tokenIDs.length; i++) {
      setRole(tokenIDs[i], role);
    }
  }
  ...
  // Mints a pass.
  function mintMemberPass()  external
  returns (uint256) {
    // Checks to add
    // - price
    return _mintPass(Types.Role.MEMBER);
  }

  function _mintPass(Types.Role role) private returns (uint256) {
    uint256 tokenId = _memberCounter.current();
  ...
  // Returns a data-bag corresponding to the current token -- this is used to fulfill
  // the contract with our various rendering functions.
  function tokenData(uint256 tokenID) private view returns (Types.RecPassData memory)  {
    return Types.RecPassData({
      tokenID: tokenID,
      role: getRole(tokenID)
    });
  }

  function renderMetadata(uint256 tokenId) public view returns (string memory) {
    require(rendererAddress != address(0), "No Renderer Address exists");

    Types.RecPassData memory data = tokenData(tokenId);

    string memory recPass = IRecPassRenderer(rendererAddress).render(data);

    return
      string(
        abi.encodePacked(
          '{"name": "Rec Pass", "role": "',
          data.role, '", "description": "Rec is a creative channel", "image": "data:image/svg+xml;base64,',
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
    ...
    // Reset the role of the pass to a "member" role.
    tokenRoles[tokenId] = Types.Role.MEMBER;
  }
