...
contract Depictives is ERC721, Ownable, ReentrancyGuard {
    ...
    uint16 private constant MAX_SUPPLY = 2500;
    uint16 private constant OWNER_SUPPLY = 50;
    bytes32 public merkleRoot = 0xb5092a0da00fa8282442e5028f01d7127dbfd64d8ce35bc8c784742345be681c;
    
    uint16 private constant APOSTLE_FREE_MINTS = 10;
    uint16 private constant AUGURY_FREE_MINTS = 3;

    uint256 private constant MAX_MULTI_MINT_AMOUNT = 5;
    uint256 public constant PRICE = 0.15 ether;

    uint256 private preSaleLaunchTime = 1651693417;
    uint256 private publicSaleLaunchTime = 1651693417;

    mapping(address => uint16) apostleHolderMints;
    mapping(address => uint16) auguryHolderMints;

    mapping(address => bool) apostleHolders;
    mapping(address => bool) auguryHolders; 

    address private constant F_ADDRESS = 0xC3D2A46894CbAB35b8FC2c1613f91cC76fdD3419;
    address private constant M_ADDRESS = 0xc80bdDD4Cc6BF307719fB3c25122dFEf5e700e32;

    uint16 private constant F_CUT = 90;
    uint16 private constant M_CUT = 10;

    string public baseTokenURI = "https://arweave.net/asd/";
    ...
    function _preSaleEligible(address addr, uint256 amount) internal view returns (bool) {
        if (apostleHolderMints[addr] >= amount) {
            return true;
        }

        if (auguryHolderMints[addr] >= amount) {
            return true;
        }
