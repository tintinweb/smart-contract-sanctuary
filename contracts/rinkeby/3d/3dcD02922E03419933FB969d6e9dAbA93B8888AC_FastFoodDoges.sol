...
contract FastFoodDoges is ERC721Namable, Ownable {
    ...
    event CreateFastFoodDoge(uint256 indexed id);
    constructor(bytes32 _whiteListRoot, 
        string memory _baseUri,
        address _pleasrDaoWallet,
        address _fffWallet) ERC721Namable("FastFoodDoge", "FFD") {
    ...
    function validSale(uint8 _count) private {
        ...
        require(total + _count <= MAX_BASE_ELEMENTS, "Max limit");
        require(_count <= MAX_WHITELIST_MINT, "Exceeds number");
    ...
    function mint(uint8 _count) public payable {
        ...
        require((publicSaleMinted[_msgSender()] += _count) <= MAX_WHITELIST_MINT);

        validSale(_count);
    }

    function mintFromWhiteList(bytes32[] calldata _merkleProof, uint8 _count) public payable {
        require(whiteListOpen, "WhiteList Sale not open");
        require((whiteListMinted[_msgSender()] += _count) <= MAX_WHITELIST_MINT);
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProof.verify(_merkleProof, whiteListRoot, leaf), "Invalid proof provided");
    ...
    function breed(uint256 ogDoge) external {
        ...
        require(totalSupply() < MAX_ELEMENTS, "Exceeds maximum number of items in collection");
    ...
    function setParams(bool _whiteListOpen, bool _saleOpen, bool _breedOpen, bool _nameChangeOpen) public onlyOwner{
        saleOpen = _saleOpen;
    ...
    function setPrices(uint256 _nameChangePrice, uint256 _bioChangePrice, uint256 _breedPrice) public onlyOwner {
