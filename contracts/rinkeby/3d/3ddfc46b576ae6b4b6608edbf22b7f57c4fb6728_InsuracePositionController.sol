...
contract InsuracePositionController is AbstractController {
    ...
    function __InsuracePositionController_init(
        ...
        stakingAsset = _stkingAssetsAddresse;
        ...
        // for staking the asset
        IERC20(stakingAsset).approve(address(_stakingController), PreciseUnitMath.MAX_UINT_256);
        //unstaking
        IERC20(lpToken).approve(address(address(_stakersPool)), PreciseUnitMath.MAX_UINT_256);
    }
    ...
    function stake(uint256 _amount) external override onlyIndex {
        ...
        if (address(base) != stakingAsset) {
            ...
            _swapTokenForToken(_amount, address(base), stakingAsset);
        }

        uint256 investmentAmount = IERC20(stakingAsset).balanceOf(address(this));
        ...
        uint256 _lptokenBalance = IERC20(lpToken).balanceOf(address(this));
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        uint256 _unstakedAmount = IERC20(stakingAsset).balanceOf(address(this));
        require(_unstakedAmount > 0, "InsuracePositionController: IPC8");

        // Convert unstaked asset to base asset
        _checkApprovals(IERC20(stakingAsset), address(_uniswapRouter), _unstakedAmount);
        uint256 _baseTokenAmount;
        if (address(base) != stakingAsset) {
            _baseTokenAmount = _swapTokenForToken(_unstakedAmount, stakingAsset, address(base));
        } else {
    ...
    function withdrawRewards() external override {
        ...
        require(
            IERC20(insurToken).balanceOf(address(this)) == _withdrawableNow,
            "InsuracePositionController IPC10"
        );

        //transfer fee in INSUR to Index
        IERC20(insurToken).transfer(feeInfo.feeRecipient, _fee);
    ...
    function _getStakingToLPRatio(uint256 currentLiquidity) internal view returns (uint256) {
        uint256 _currentTotalSupply = IERC20Upgradeable(lpToken).totalSupply();
    ...
    function netWorth() external view override returns (uint256) {
        ...
        uint256 _stakedBase = _priceFeed.howManyTokensAinB(
            address(base),
            stakingAsset,
            positionSupply()
        );
    ...
    function apy() external view override returns (uint256 _apy) {
        ...
        uint256 _poolTVLInRewardsAsset = _priceFeed.howManyTokensAinB(
            insurToken,
            stakingAsset,
            _poolTVLInStakingAsset
        );
    ...
    function maxUnstake() external view override returns (uint256 _maxUnstake) {
        _maxUnstake = _stakingController.maxUnstakeAmtPT(stakingAsset);
    }

    function canStake() public view override returns (bool) {
        uint256 tokenAmtAfterStaked = IERC20(lpToken)
            .balanceOf(address(this))
            .mul(_stakersPool.getStakedAmountPT(stakingAsset))
            .div(IERC20Upgradeable(lpToken).totalSupply());

        return
            !(tokenAmtAfterStaked >= _stakingController.perAccountCapPT(stakingAsset)) &&
            super.canStake();
    }
        ...
        returns (uint256 _amount, uint256 _unstakeAvailable)
    {
        ...
        // amount to unstake
        _amount = convertLPToStaking(ILPToken(lpToken).pendingBurnAmtPH(address(this)));
    }
        ...
        )
    {
        ...
        burnableAmtOf = convertLPToStaking(ILPToken(lpToken).burnableAmtOf(address(this)));
    }
}
