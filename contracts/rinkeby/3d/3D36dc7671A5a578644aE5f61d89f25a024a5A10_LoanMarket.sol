    ...
    */
        ...
        returns(uint256)
    {
        ...
        if(pair.token0() == _stable){
            ...
            return (_altAmount*(resD/res1)); // return amount of alt needed to buy 1 stable
        }
        else{
            uint resD = res1*(10**token1Decimals);
            return (_altAmount*(resD/res0)); // return amount of alt needed to buy 1 stable
        }
    }
        ...
        returns(uint256)
    {
        ...
        for(uint256 i = 0 ; i < stkaedCollateralTokens.length ; i++){
            ...
            uint256 collateralPriceInBorrowed = this.getTokenPrice(borrowedToken, stkaedCollateralTokens[i], stakedCollateralAmounts[i]); 
            totalCollateralInBorrowedToken =  totalCollateralInBorrowedToken.add(collateralPriceInBorrowed);
        }
        console.log("TOTAL COLLATERAL TOKENS: %d", totalCollateralInBorrowedToken);
        return (totalCollateralInBorrowedToken/loanDetails.loanAmountInBorrowed)*100;
    }
