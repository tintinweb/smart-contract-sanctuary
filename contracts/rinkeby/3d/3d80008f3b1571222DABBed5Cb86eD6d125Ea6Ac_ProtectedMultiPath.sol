   ...
   */
        ...
        internal
    {
        if (permit.length == 32 * 7) {
            (bool success, bytes memory result) = token.call(abi.encodePacked(IERC20Permit.permit.selector, permit));
...
interface IAdapter {
   ...
   * @param fromAmount Amount of source tokens to be swapped
   * @param payload can be used in future
   ...
   */
    function swap(
        ...
        Utils.Route[] calldata route) external payable;
}
...
contract FeeModel is AdapterStorageV5 {
    using SafeMath for uint256;
        ...
        internal
    {
        ...
        uint256 fee = 0;
        FeeStructure memory feeStructre = registeredPartners[partner];
        
        if ( feePercent > 0 ) {
            if (feeStructre.partnerShare > 0) {
            fee = _takeFee(
                feePercent > maxFeePercent ? maxFeePercent : feePercent,
                toToken,
                receivedAmount,
                expectedAmount,
                feeStructre.partnerShare,
                feeStructre.noPositiveSlippage,
                feeStructre.positiveSlippageToUser,
                partner
            );
            }
            else {
   ...
   */
        ...
        returns (uint256)
    {
        ...
        if (fromToken != Utils.ethAddress()) {
            Utils.permit(fromToken, data.permit);
        ...
        private
    {
        ...
        for (uint i = 0; i < path.length; i++) {
            ...
            for (uint j = 0; j < path[i].adapters.length; j++) {
                ...
                require(
                    IAugustusSwapperV5(augustus).hasRole(ROUTER_ROLE, adapter.adapter),
    ...
    function retrieveEth() private {
        ...
        if (balance > 0) {
            (bool result, ) = msg.sender.call{value: balance, gas: 4000}("");
            require(result, "Failed to transfer Ether");
        }
    }
}
