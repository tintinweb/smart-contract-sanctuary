...
contract Phloodles is Ownable, ERC721Enumerable {
    ...
    function _AirDropMint(address _To, uint256  _Amount) private {
        ...
        for(uint256 i = 1; i <= _Amount; i.add(1)){
                _safeMint( _To, activeSupply.add(1));
            }
    }
    function ERC20Mint(uint256 _NumberOfMints) public payable {
        ...
        uint256 cost = _NumberOfMints.mul(ERC20price);
        require(erc20Token.allowance(msg.sender, address(this)) >= cost, "Not enough allowance of ERC20 token");
        SafeERC20.safeTransferFrom(erc20Token, msg.sender, address(this), cost);
        
        for(uint256 i = 1; i <= _NumberOfMints; i.add(1)){
    ...
    function Mint(uint256 _NumberOfMints) public payable {
        ...
        for(uint256 i = 1; i <= _NumberOfMints; i.add(1)) {
            _safeMint(msg.sender, activeSupply.add(1));
        }
    }
