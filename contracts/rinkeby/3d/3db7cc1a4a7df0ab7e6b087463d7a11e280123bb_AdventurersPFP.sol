...
function MintPFPbyText(uint256 tokenId) public {
  ...
  function MintPFPbyTexts(uint256 startingIndex, uint256 numDHeros) public {
    ...
    function composePFP(uint256 WeapontokenId, uint256 PFPtokenId, uint256 newPFPtokenId) public returns (bool) {
        require(composeIsActive, "Compose is not active at the moment");
        require(totalSupply().add(1) <= MAX_SUPPLY, "Compose would exceed max supply of Hero");
        require(ownerOf(PFPtokenId) == msg.sender, "ERC721: use of token1 that is not own");
        
        AdvWeapon advWeapon = AdvWeapon(WeaponContractAddress);
            
        require(advWeapon.ownerOf(WeapontokenId) == msg.sender, "You do not have this weapon");
