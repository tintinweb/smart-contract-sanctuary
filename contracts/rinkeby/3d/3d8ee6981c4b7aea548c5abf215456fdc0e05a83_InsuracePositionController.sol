...
contract InsuracePositionController is AbstractController {
    ...
    function callUnstake() external override onlyIndex returns (uint256) {
        require(
            _stakingController.minUnstakeAmtPT(stakingAsset) <= _stakes &&
                _stakingController.maxUnstakeAmtPT(stakingAsset) >= _stakes,
            "InsuracePositionController: IPC5"
        );
        _stakingController.proposeUnstake(_stakes, stakingAsset);
        setUnstakingState();
        return _stakes;
    }
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        _stakingController.withdrawTokens(_stakes, stakingAsset);
    ...
    function withdrawRewards() external override {
        uint256 _fullRewards = outstandingRewards();
    ...
    function outstandingRewards() public view override returns (uint256) {
        return _rewardController.vestingWithdrawableAmountPerAccount(address(this));
    }
    ...
    function apy() external view override returns (uint256 apy) {
        ...
        apy = _caclulatePoolAPY(stakingAsset).div(PERCENTAGE_100);
    }
    ...
    {
        // No such thing as unstakableAvailable but pendingBurnAmtPH once
        // call unstake (proposeUnstake()) is called which calls proposeToBurn()
        _unstakeAvailable = ILPToken(lpToken).pendingBurnAmtPH(address(this));
        _amount = _unstakeAvailable;
    }
