...
// Create New
// poolInfoLimits 
// Pool lockedUpRewards 
// Pool lockedUpBurnRewards
...
contract RedBerryToken is BEP20("RedBerry Token", "REDB") {
    ...
    // The taxAddress which receive tax on each transaction
    address private taxAddress;

    // AntiWhale Max transfer amount rate in basis points. (default is 1% of total supply)
    uint16 public maxTransferAmountRate = 100;

    // The operator can update the antiWhale and transfer tax rate
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "REDB::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                ...
                "REDB::transfer: Tax value invalid"
            );

            super._transfer(sender, BURN_ADDRESS, burnAmount);
            super._transfer(sender, taxAddress, liquidityAmount);
            super._transfer(sender, recipient, sendAmount);
            amount = sendAmount;
        }
    }

    function setTransferTaxAddress(address _taxAddress) public onlyOperator {
        taxAddress = _taxAddress;
    }

    function getTransferTaxAddress() public view returns (address) {
        return taxAddress;
    }
    ...
    /**
     * @dev Returns the address is excluded from Transfer Tax Rate or not.
     */
    ...
    /**
     * @dev Exclude or include an address from Transfer Tax Rate.
     * Can only be called by the current operator.
     */
    ...
    /**
     * @dev Returns the addresses which are excluded from Transfer Tax Rate.
     */
     ...
     * Can only be called by the current operator.
     */
    function updateTaxBurnRate(uint16 _taxBurnRate) public onlyOperator {
        require(
            _taxBurnRate <= 100,
            "REDB::updateTaxBurnRate: Burn rate must not exceed the maximum rate."
        );
        emit TaxBurnRateUpdated(msg.sender, taxBurnRate, _taxBurnRate);
        taxBurnRate = _taxBurnRate;
    }
        ...
        onlyOperator
    {
        require(
            ...
            "REDB::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
...
// Syrup with Governance.
contract Syrup is BEP20("Syrup Token", "SRP") {
    constructor(RedBerryToken _RedBerry) {
        RedBerry = _RedBerry;
        _operator = _msgSender();
        emit OperatorTransferred(address(0), _operator);
    }
    ...
    // The Syrup Token!
    Syrup public syrup;
    // The operator can recover wrong tokens.
    address private _operator;
    ...
    function safeRedBerryTransfer(address _to, uint256 _amount)
        public
        onlyOwner
    {
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(
            newOperator != address(0),
            "SRP::transferOperator: new operator is the zero address"
        );
        emit OperatorTransferred(_operator, newOperator);
        _operator = newOperator;
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "SRP::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "SRP::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "SRP::delegateBySig: signature expired"
        );
        return _delegate(signatory, delegatee);
    }
        ...
        returns (uint256)
    {
        require(
            blockNumber < block.number,
            "SRP::getPriorVotes: not yet determined"
        );
        uint32 nCheckpoints = numCheckpoints[account];
        if (nCheckpoints == 0) {
            return 0;
    ...
    function _delegate(address delegator, address delegatee) internal {
        address currentDelegate = _delegates[delegator];
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SRPs (not scaled);
        _delegates[delegator] = delegatee;
        emit DelegateChanged(delegator, currentDelegate, delegatee);
        _moveDelegates(currentDelegate, delegatee, delegatorBalance);
    }
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "SRP::_writeCheckpoint: block number exceeds 32 bits"
        );
        if (
            nCheckpoints > 0 &&
            checkpoints[delegatee][nCheckpoints - 1].fromBlock == blockNumber
    ...
    // Recover the wrong token. Can only be called by the Operator.
    function recoverWrongToken(IBEP20 _token, uint256 _tokenAmount)
        public
        onlyOperator
    {
        require(
            address(_token) != address(RedBerry) &&
                address(_token) != address(syrup),
            "SRP::Cannot withdraw RedBerry & Syrup tokens"
        );
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 depositTime; // Token deposit time.
        uint256 totalClaimedAmount; // How many rewards claimed by user.
        uint256 rewardLockedUp; // Reward locked up.
        uint256 nextHarvestUntil; // When can the user harvest again.
        uint256 lockedRewardBurn; // Locked Reward Will Be Burned or Not
    ...
    struct PoolInfo {
        ...
        uint256 totalUsers; // Total Users in this Pool
        uint256 claimedReward; // Total climed reward
        uint256 harvestInterval; // Harvest interval in seconds
        uint256 lockedUpRewards; // Locked reward balance.
        uint256 lockedUpBurnRewards; // Locked reward burned
    }

    // Info of each pool.
    struct PoolInfoLimits {
        uint256 poolLimit; // Total Pool limit
        uint256 poolLimitPerUser; // Pool limit per user    
    }
    ...
    // Info of each pool.
    PoolInfoLimits[] public poolInfoLimits;
    ...
    uint256 public totalLockedUpBurnRewards;
    bool public isBurn;
    // Burn address
    address public constant BURN_ADDRESS =
        0x000000000000000000000000000000000000dEaD;
    ...
    uint256 public constant MAXIMUM_HARVEST_INTERVAL = 1209600;
    ...
    function add(
        ...
        require(tokenFarmExists[_lpToken] == false, "Farm already exists");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's harvestInterval, Can only be called by the owner.
    function setLockupTime(uint256 _pid, uint256 _harvestInterval)
        public
        onlyOwner
    {
        require(
            _harvestInterval <= MAXIMUM_HARVEST_INTERVAL,
            "Max Harvest Interval limit"
        );
    ...
    function setLimit(
        ...
        PoolInfoLimits storage poollimits = poolInfoLimits[_pid];
        require(
            _poolLimitPerUser > poollimits.poolLimitPerUser,
            "User Limit: New limit must be higher"
        );
        require(
            _poolLimit > poollimits.poolLimit,
            "Pool Limit: New limit must be higher"
        );

        poollimits.poolLimitPerUser = _poolLimitPerUser;
        poollimits.poolLimit = _poolLimit;
    }

    // Reward Locked amount will be burned or not
    function setBurn(bool action) public onlyOwner {
        isBurn = action;
    }
    ...
    // View function to see pending RedBerry on frontend.
        ...
        returns (uint256)
    {
        ...
        uint256 pending = user.amount.mul(accRedBerryPerShare).div(1e12).sub(
            user.rewardDebt
        );
        return pending.add(user.rewardLockedUp);
    }

    // View function to see Can user harvest redBerrys.
    ...
    function updatePool(uint256 _pid) public {
        ...
        RedBerry.mint(address(syrup), redBerryReward);
    ...
    function deposit(
        ...
        require(_pid != 0, "Deposit REDB by enterStaking");
        PoolInfo storage pool = poolInfo[_pid];
        PoolInfoLimits storage poolLimits = poolInfoLimits[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(block.number >= pool.startBlock, "Pool is not started yet");
        updatePool(_pid);
        payReferralRedb(_pid);

        if (whitelistedUser[msg.sender] != true) {
            if (poolLimits.poolLimit > 0) {
                require(
                    _amount.add(pool.totalStaked) <= poolLimits.poolLimit,
                    "Pool amount above limit"
                );
            }
            if (poolLimits.poolLimitPerUser > 0) {
                require(
                    _amount.add(user.amount) <= poolLimits.poolLimitPerUser,
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "Withdraw REDB by leaveStaking");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "withdraw not good check balance");
        updatePool(_pid);
        ...
        if (_pendingAmount > 0) {
            if (canHarvest(_pid, msg.sender)) {
                ...
                if (isBurn) {
                    user.lockedRewardBurn = user.lockedRewardBurn.add(
                        _pendingAmount
                    );
                    totalLockedUpBurnRewards = totalLockedUpBurnRewards.add(_pendingAmount);
                    pool.lockedUpBurnRewards = pool.lockedUpBurnRewards.add(_pendingAmount);
                    safeRedBerryTransfer(BURN_ADDRESS, _pendingAmount);
                } else {
                    payOrLockupPendingRedb(_pid);
                }
            }
    ...
    function enterStaking(uint256 _amount, address _referrer) public {
        ...
        PoolInfoLimits storage poolLimits = poolInfoLimits[0];
        ...
        if (user.amount > 0) {
            ...
            uint256 referarBalance = userReferalAmount[0][msg.sender];
            if (referarBalance > 0) {
                userReferalClaimedAmount[0][
                    msg.sender
                ] = userReferalClaimedAmount[0][msg.sender].add(referarBalance);
                userReferalAmount[0][msg.sender] = 0;
                _pendingAmount = pending.add(referarBalance);
            } else {
                _pendingAmount = pending;
            }
        ...
        if (_amount > 0) {
            ...
            if (!RedBerry.isExcludedFromTaxRate(address(this))) {
                uint256 transferTax = _amount
                    .mul(RedBerry.transferTaxRate())
                    .div(10000);
    ...
    // claim reward for 0 pool
    function harvestREDB() public {
        require(canHarvest(0, msg.sender), "Harvest Lock");
        enterStaking(0, address(0));
    }
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        uint256 referarBalance = userReferalAmount[0][msg.sender];
        if (referarBalance > 0) {
            userReferalClaimedAmount[0][msg.sender] = userReferalClaimedAmount[
                0
            ][msg.sender].add(referarBalance);
            userReferalAmount[0][msg.sender] = 0;
            _pendingAmount = pending.add(referarBalance);
        } else {
            _pendingAmount = pending;
        }
        user.totalClaimedAmount = user.totalClaimedAmount.add(_pendingAmount);
        if (_pendingAmount > 0) {
            if (canHarvest(0, msg.sender)) {
                pool.claimedReward = pool.claimedReward.add(pending);
                user.nextHarvestUntil = block.timestamp.add(
    ...
    function safeRedBerryTransfer(address _to, uint256 _amount) internal {
        syrup.safeRedBerryTransfer(_to, _amount);
    }
    ...
    // Pay Referral RedBerry.
    function payReferralRedb(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user
            ...
            .sub(user.rewardDebt);
        if (pending > 0) {
            payReferralCommission(msg.sender, pending, user.depositTime, _pid);
        }
    }

    // Pay or lockup pending RedBerry.
    function payOrLockupPendingRedb(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // reset lockup
                pool.lockedUpRewards = pool.lockedUpRewards.sub(pool.lockedUpRewards);
                ...
                // send user rewards
                safeRedBerryTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
            user.rewardLockedUp = user.rewardLockedUp.add(pending);       
            pool.lockedUpRewards = pool.lockedUpRewards.add(pending);
    ...
    function payReferralCommission(
        ...
        if (
            ...
            _depositTime > timeForSetNewCommissionRate
        ) {
        ...
        if (referralCommissionRate > 0) {
            ...
            if (
                ...
                RedBerry.mint(address(syrup), commissionAmount);
    ...
    // add Whilisted user by the owner, address will be excluded from Pool Limit and Deposit fee.
    // for RedBerry Auto Vault
        ...
        onlyOwner
    {
        ...
        require(
            address(_token) != address(RedBerry) &&
                address(_token) != address(syrup),
            "Cannot withdraw RedBerry & Syrup tokens"
        );
