...
contract MasterChef is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public startUnlockTime;      // start to unlock since this time (after 1 months)
    uint256 public unlockDuration;  // unlockable 1/10 for each month during 10 months

    uint internal baseBlock;
    uint internal baseTimestamp;
    ...
    function initialize(
        ...
        if(startBlock < _currentBlock) startBlock = _currentBlock;

        // after 1 months since staking start time (Polygon block time: 2 seconds)
        baseBlock = 9175035;
        baseTimestamp = 1626343025;
        blockTime = 2;

        startUnlockTime = (startBlock - _currentBlock) * blockTime + 30 days; 
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, _currentBlock);
        uint256 mcfReward = multiplier.mul(mcfPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        mcf.mint(devaddr, mcfReward.div(10));
    ...
    function unlockAvailable() public view returns(uint256) {
        if(estimateTime() < startUnlockTime) return 0;

        LockedInfo storage user = lockedInfo[msg.sender];
        if(user.lockedAmount <= 0) return 0;

        uint256 pastMonth = (estimateTime() - startUnlockTime) / 30 days;
    ...
    function unlock() external nonReentrant {
        require(estimateTime() > startUnlockTime, "No available to unlock");
        ...
        user.unlockedCount = (estimateTime() - startUnlockTime) / 30 days;
    }
    ...
    function estimateTime() internal view returns(uint) {
        return (_currentBlock - baseBlock) * blockTime + baseTimestamp;
    }
    ...
    function updateStartUnlockTime(uint256 _time) external onlyOwner {
        require(estimateTime() < startUnlockTime);
        require(_time > estimateTime(), "time should be greater than current timestamp");
