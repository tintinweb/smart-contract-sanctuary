...
contract BlindBox is NonGenerativeBB {
    ...
    */
    function StartSeries(
        address collection,string memory name,
        string memory seriesURI,
        string memory boxName,
        string memory boxURI,
        uint256 startTime,
        uint256 endTime, bool isGenerative, uint256 royalty ) onlyOwner public {
            Series1 memory series = Series1( name, seriesURI, boxName, boxURI, startTime, endTime,collection);
        if(isGenerative){
            // start generative series
            // generativeSeriesId.increment();
            // generativeSeries(collection, name, seriesURI, boxName, boxURI, startTime, endTime, royalty);
            ...
            nonGenerativeSeries(collection, name, seriesURI, boxName, boxURI, startTime, endTime, royalty);
            emit SeriesInputValue(series,nonGenerativeSeriesId.current(), isGenerative, royalty );
        }
       
        
    }
    function extraPsrams(uint256 maxBoxes,
        uint256 perBoxNftMint,
        uint256 perBoxPrice, address bankAddress,
        uint256 baseCurrency,
        uint256[] memory allowedCurrencies, bool isGenerative) public {
        if(isGenerative){
    //   setExtraParamsGen(baseCurrency, allowedCurrencies, bankAddress, perBoxPrice, maxBoxes, perBoxNftMint);  

        } else {
      setExtraParams(baseCurrency, allowedCurrencies, bankAddress, perBoxPrice, maxBoxes, perBoxNftMint);  

        }
        Series2 memory series = Series2(maxBoxes, perBoxNftMint, perBoxPrice, bankAddress, baseCurrency, allowedCurrencies );
    ...
    */
    function _openNonGenBox(uint256 boxId) private {
        ...
        for (uint256 j = 0; j < boxesNonGen[boxId].countNFTs; j++) {
          ...
          emit NonGenNFTMinted(boxId, tokenId, msg.sender, collection);
        }
...
/** 
    ...
    */
    ...
    event NonGenNFTMinted(uint256 indexed boxId, uint256 tokenId, address from, address collection );
