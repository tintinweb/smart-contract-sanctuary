...
// File: spriteImage.sol



pragma solidity ^0.8.0;

//import "./spriteStruct.sol";
...
contract NFToken is sprite,IERC721Metadata,IERC721Enumerable {
	...
	string constant INDEX_BIG_THAN_BANLANCE = "INDEX_BIG_THAN_BANLANCE";
	...
	function tokenURI(uint256 tokenId) public view  override  returns (string memory){
		// Inspired by Loot's  - MIT license
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view  override returns (uint256) {
		require(NTFBalances[owner] > index, INDEX_BIG_THAN_BANLANCE); //不曾拥有该TOken
...
contract creatSprite is NFToken {
	...
	function mint() external payable {
		...
		require(block.timestamp >= 1632727800,NOT_START_MINT);
	...
	function _getRand0to9(bytes1 inputByte) private pure returns(uint8) {
...
contract market is creatSprite {
	string constant BANLANCE_IS_ZERO = "005001";
	...
	function userWithdrawETH()public {
		require(UserETHBalance[msg.sender]>0,BANLANCE_IS_ZERO); 
