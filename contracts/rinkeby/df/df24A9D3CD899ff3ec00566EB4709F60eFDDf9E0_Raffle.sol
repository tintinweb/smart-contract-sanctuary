...
interface IERC1271 {
    function isValidSignature(address owner, bytes32 _messageHash, bytes memory _signature)
        external
        view
...
abstract contract ERC1271 is IERC1271 {
    ...
    modifier onlyValidSignature(address owner, bytes32 permissionHash, bytes memory signature) {
        require(
            isValidSignature(owner, permissionHash, signature) == VALID_SIG,
    ...
    function isValidSignature(address owner, bytes32 permissionHash, bytes memory signature)
        public
        view
        override
        returns (bytes4)
    {
        return
            SignatureChecker.isValidSignature(owner, permissionHash, signature)
                ? VALID_SIG
                : INVALID_SIG;
    }
}
