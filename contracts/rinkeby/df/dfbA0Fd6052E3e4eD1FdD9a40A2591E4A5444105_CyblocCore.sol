...
contract CyblocERC721BaseEnumerable is ERC165, IERC721Base, IERC721Enumerable, CyblocDependency, CyblocPausable {
  ...
  function _removeTokenFrom(address _from, uint256 _tokenId) private {
    ...
    if (_tokenIndex != _lastTokenIndex) {
        ...
        _ownedTokens[_from][_tokenIndex] = _lastTokenId; // Move the last token to the slot of the to-delete token
        _ownedTokenIndex[_lastTokenId] = _tokenIndex; // Update the moved token's index
    }
