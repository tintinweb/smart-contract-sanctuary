...
contract OpenSeaBatchCanceller {
    ...
    function listOrder(address tokenAddress, uint256 tokenId, uint256 priceInWei) external {
        OpenSeaListing[] memory listings = new OpenSeaListing[](1);

        IERC720 t = IERC720(tokenAddress);
        require(msg.sender == t.ownerOf(tokenId), "Not the owner");
        require(t.isApprovedForAll(msg.sender, address(this)), "Need to approve contract");
