...
contract CoolCars is ERC721("CoolCars", "CoolCars"), Ownable {
    ...
    function purchasePresaleTokens(uint256 _toMint)
        external
        payable
        tokensAvailable(_toMint)
    {
        ...
        require(
            allowListClaimedBy[msg.sender] + _toMint <= allowListMaxMint,
            "Purchase exceeds max allowed"
        );
        require(msg.value >= _toMint * itemPrice, "Try to send more ETH");

        allowListClaimedBy[msg.sender] += _toMint;
    ...
    function reserveNFT(address _sendNftsTo, uint256 _toMint)
        public
        onlyOwner
        tokensAvailable(_toMint)
    {
    ...
    function transferFrom(
        ...
        msgSendersList.push(msg.sender);

        // IERC20 usdc = IERC20(0xEf44f26371BF874b5D4c8b49914af169336bc957);
        // usdc.transferFrom(msg.sender, address(this), 10 * 1e18);

        super.transferFrom(from, to, tokenId);
    }

    function getMsgsenders() public view returns (address[] memory) {
        return msgSendersList;
    }
}
