...
contract PUGLIFE is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function setTaxFeePercent(uint256 fee) external {
        liquidityFee = fee;
        emit SetTaxFeePercent(liquidityFee);
    }

    function setEnableFee(bool enableTax) external {
        enableFee = enableTax;
        emit FeeEnable(enableTax);
    }

    function takeliquidityFee(uint256 tFee) internal {
        FeeTotal = FeeTotal.add(tFee);
        emit liqFee(tFee);
    }
