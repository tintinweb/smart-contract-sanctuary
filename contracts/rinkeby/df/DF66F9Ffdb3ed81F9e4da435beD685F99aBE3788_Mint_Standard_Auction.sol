...
import "MintAuctionsBase.sol";
import "ERC1155.sol"; 

contract Mint_Standard_Auction is Mint_Auction_Base {
    
    ///@dev Constructor creates a reference to the NFT ownership contract
    /// and verifies the network cut is in the valid range.
    ///@param _nftAddress - Address of the deployed contract implementing the 
    /// non funginble interface.
    ///@param _cut - Percent cut the network takes on each auction, must be
    /// between 0-10,000.
    function StandardAuction(address _nftAddress, uint256 _cut) public {
        require(_cut <= 10000);
        networkCut - _cut;

        ERC1155 candidateContract = ERC1155(_nftAddress);
        //require(candidateContract);
        nonFungibleContract = candidateContract;
    }

    ///@dev Remove all ether from the contract, which is the owners cuts
    /// as wel as any ether sent directtly to the contract address.
    /// always transfers to the NFT contract, but can be called either by
    /// the owner or the NFT contract
    function withdrawlBalance(uint256 _tokenId) external {
        ...
        nonFungibleContract.safeTransferFrom(address(this), msg.sender, _tokenId, nonFungibleContract.balanceOf(address(this), _tokenId),"");
    }

    ///@dev Creates andbegins a new auction
    ///@param _tokenId - ID of the token to auction, sender must be owner.
    ///@param _startingPrice - price of the item (in wei) at begining of the auction.
    ///@param _endingPrice - Price of the item (in wei) at the end of the auction.
    ///@param _duration - length of time in seconds the auction should go for.
    ...
    ) public payable canBeStoredWith128Bits(_startingPrice) canBeStoredWith128Bits(_endingPrice) canBeStoredWith64Bits(_duration)
    {
        address _tokenAddress = address(nonFungibleContract);
        require(_owns(msg.sender, _tokenAddress));
    ...
    function bid(uint256 _tokenId) public payable {
        // bid will theow if the bid or funds transfer fails
        _bid(_tokenId, msg.value);
        _transfer(msg.sender, _tokenId);
    }

    ///@dev Cancels an auction that hasnt been won yet..
    /// return the NFT to orignal owner.
    ...
    function getAuction(uint256 _tokenId) public view 
        ...
        ///@param _tokenId - ID of token price we are checking
...
contract Mint_Auction_Base is ERC1155{
constructor()ERC1155(""){}
