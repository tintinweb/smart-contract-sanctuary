...
contract DMEX_Trading {
    ...
    struct FuturesOrderPair {
        ...
        uint256 currentMultiplier;          // the latest exchange rate for the base token to USD
    ...
    function generateOrderHash (bool maker, bool takerIsBuying, address user, bytes32 futuresContractHash, uint256[13] tradeValues) public view returns (bytes32)
    {
        if (maker)
        {
    ...
    ) onlyAdmin returns (uint filledTakerTokenAmount)
    {
        ...
        */
        ...
        FuturesOrderPair memory t  = FuturesOrderPair({
            ...
            currentMultiplier : tradeValues[12],
        ...
        {       
            ...
            if (!positionExists(t.makerInversePositionHash) && !positionExists(t.makerPositionHash))
            {
                ...
                else
                {
                    ...
                    if (t.makerPrice < retrievePosition(t.makerInversePositionHash)[1])
                    {
                        ...
                        tv.makerProfit                      = calculateProfit(t.makerPrice, retrievePosition(t.makerInversePositionHash)[1], tv.qty, futuresContractHash, true, t.currentMultiplier);
                    }
                    else
                    {
                        // user has made a loss
                        tv.makerLoss                        = calculateLoss(t.makerPrice, retrievePosition(t.makerInversePositionHash)[1], tv.qty, futuresContractHash, true, t.currentMultiplier);                                        
                    }
            ...
            {        
                ...
                {    
                    ...
                    {
                        // user has made a profit
                        tv.takerProfit                      = calculateProfit(t.makerPrice, retrievePosition(t.takerInversePositionHash)[1], tv.qty, futuresContractHash, false, t.currentMultiplier);
                    }
                    else
                    {
                        // user has made a loss
                        tv.takerLoss                        = calculateLoss(t.makerPrice, retrievePosition(t.takerInversePositionHash)[1], tv.qty, futuresContractHash, false, t.currentMultiplier); 
                    }
        ...
        {      
            ...
            if (!positionExists(t.makerInversePositionHash) && !positionExists(t.makerPositionHash))
            {
                ...
                else
                {
                    ...
                    {
                        // user has made a profit
                        tv.makerProfit                      = calculateProfit(t.makerPrice, retrievePosition(t.makerInversePositionHash)[1], tv.qty, futuresContractHash, false, t.currentMultiplier);
                    }
                    else
                    {
                        // user has made a loss
                        tv.makerLoss                        = calculateLoss(t.makerPrice, retrievePosition(t.makerInversePositionHash)[1], tv.qty, futuresContractHash, false, t.currentMultiplier);                               
                    }
            ...
            if (!positionExists(t.takerInversePositionHash) && !positionExists(t.takerPositionHash))
            {
                ...
                else
                {
                    ...
                    {
                        // user has made a profit
                        tv.takerProfit                      = calculateProfit(t.makerPrice, retrievePosition(t.takerInversePositionHash)[1], tv.qty, futuresContractHash, true, t.currentMultiplier);
                    }
                    else
                    {
                        // user has made a loss
                        tv.takerLoss                        = calculateLoss(t.makerPrice, retrievePosition(t.takerInversePositionHash)[1], tv.qty, futuresContractHash, true, t.currentMultiplier);                                  
                    }   
...
//--> 264 000
    ...
    function calculateProfit(uint256 closingPrice, uint256 entryPrice, uint256 qty, bytes32 futuresContractHash, bool side, uint256 currentMultiplier) public view returns (uint256)
    {
        //uint256 multiplier = futuresContracts[futuresContractHash].multiplier;
        return safeMul(safeMul(safeSub(side ? entryPrice : closingPrice, side ? closingPrice : entryPrice), qty), currentMultiplier )  / 1e16;            
    }
    ...
    function calculateLoss(uint256 closingPrice, uint256 entryPrice, uint256 qty,  bytes32 futuresContractHash, bool side, uint256 currentMultiplier) public view returns (uint256)
    {
        //uint256 multiplier = futuresContracts[futuresContractHash].multiplier;

        return safeMul(safeMul(safeSub(side ? closingPrice : entryPrice, side ? entryPrice : closingPrice), qty), currentMultiplier) / 1e16 ;        
    }
    ...
    function closeFuturesPositionInternal (bytes32 futuresContract, bool side, address user, address poolAddress, uint256 expirationFee, uint256 currentMultiplier) private returns (bool)
    {
        bytes32 positionHash = keccak256(this, user, futuresContract, side);        
        uint256[5] memory pos = retrievePosition(positionHash);        
        ...
        {   
            // user made a profit
            v.profit = calculateProfit(v.closingPrice, pos[1], pos[0], futuresContract, !side, currentMultiplier);
        ...
        {
            // user made a loss
            v.loss = calculateLoss(v.closingPrice, pos[1], pos[0], futuresContract, !side, currentMultiplier);  
    ...
    function closeFuturesPositionForUser (bytes32 futuresContract, bool side, address user, address poolAddress, uint256 expirationFee, uint256 currentMultiplier) onlyAdmin
    {
        closeFuturesPositionInternal(futuresContract, side, user, poolAddress, expirationFee, currentMultiplier);
    }
    ...
    function batchSettlePositions (
        ...
        {
            closeFuturesPositionForUser(futuresContracts[i], sides[i], users[i], pools[i], expirationFee[i], currentMultiplier[i]);
        }
    }
