...
contract Recipt is 
    ...
    struct chainData {
        address author; // Address of the caller of writeRecipt()
    ...
    constructor () public ERC721 ("Recipt", "RECIPT") {
        tokenCounter = 0;
    ...
    function mint() public payable returns (uint256) {
        ...
        if (tokenCounter + 1 < 100) {
            price = 0.0001 ether;
        }
        if (msg.sender != owner()) {  
            require(msg.value >= price, "0.003 to mint");
        }
    ...
    function writeRecipt(uint256 id, string memory text) public returns (uint256) {
        return writeRecipt(id, 0, text);
        require(enabled, "Contract not enabled");
        require(ownerOf(id) == msg.sender, "NOT OWNER");
        require(written[id] == false, "Message already written!");
    }

    // Writes input text onto the chain with a reference to a previous message
    function writeRecipt(uint256 id, uint256 prevId, string memory text) public returns (uint256) { // TODO: TEST
        require(enabled, "Contract not enabled");
        require(ownerOf(id) == msg.sender, "NOT OWNER");
        require(written[id] == false, "Message already written!");
        chainData storage newData = data[newItemId];
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        // string memory svg_string = string(abi.encodePacked(svgformat,text,svgid,id,svgdate,date,svgend));
        // string memory svg = string(svg_string);
        // // <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">"Bush did 9/11."</text><text x="340" y="320" class="base" text-anchor="end">ID# 103</text><text x="340" y="340" class="base" text-anchor="end">09:23pm 10/11/12</text></svg>
    ...
    function withdraw() public payable onlyOwner {
        require(payable(msg.sender).send(address(this).balance), "Withdraw error");
    }
    ...
    string private fname='{\"name\": \"EthSign #';
    string private fauthor='\", \"attributes\": [{\"trait_type\": \"author\",\"value\": \"';
    string private fdate='\"},{\"display_type\": \"date\",\"trait_type\": \"date\",\"value\": \"';
    string private ftext='\"},{\"trait_type\": \"text\",\"value\": \"';
    string private fend='\"}]}';
    
    string private svgstart = '\"}],\"image\": \"data:image/svg+xml;base64,';
    // string private svgformat = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">';
    ...
    // string private svgid = '</text><text x="340" y="320" class="base" text-anchor="end">ID# ';
...
//           </p>
// </foreignObject>
//         <text x="340" y="320" text-anchor="end">ID# 0/5000</text>
//         <text x="340" y="340" text-anchor="end">7:48pm 9/29/2021</text></svg>
