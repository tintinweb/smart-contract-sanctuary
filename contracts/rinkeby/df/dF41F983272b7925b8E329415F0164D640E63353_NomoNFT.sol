...
contract NomoNFT is ERC721, Ownable {
    ...
    event UpdatedCardImagePoints(uint256 indexed _cardImageId, uint256 _points);
    ...
    function createCardImage(
        ...
        emit NewCardImageCreated(_cardImageId, _name, _imageURL, _league, _gen,_setId);
    ...
    function createCardsImages(
        ...
        for (uint256 i = 0; i < _names.length; i++) {
            createCardImage(_names[i], _imagesURLs[i], _leagues[i], _gen, _setId);
        }
    }
    ...
    // @todo set points in the update params func? because points value must be updated on the parameters update. Or we need to calculate points from parameters?
    /// @notice Updates the points of the card
    /// @param _cardImageId Card Image id
    /// @param _points New card's points value
    /// @dev also updates points update timestamp
    function updateCardImagePoints(uint256 _cardImageId, uint256 _points)
        external
        onlyExistingCardImage(_cardImageId)
        onlyOwner
    {
        cardsImages[_cardImageId].points = _points;
        cardsImages[_cardImageId].pointsUpdateTime = block.timestamp;
        emit UpdatedCardImagePoints(_cardImageId, _points);
    }
    ...
    /// @return points - represents total score of the player.
    /// @return pointsUpdateTime - unix timestamp when points was updated last time.
        ...
        )
    {
        ...
        gen = cardImage.gen;
        points = cardImage.points;
        pointsUpdateTime = cardImage.pointsUpdateTime;
    ...
    /// @notice Returns uint256 points values.
    /// @dev Reverts on non-existent card image id.
    /// @param _cardImageId Card Image id.
    function getCardImagePoints(uint256 _cardImageId)
        external
        view
        onlyExistingCardImage(_cardImageId)
        returns (uint256)
    {
        return cardsImages[_cardImageId].points;
    }
    ...
    /// @notice Updates CardImage's parameters values and points
    /// @dev Points must be calculated outside of this contract by other "updater" contract
    /// @param _cardImageId Card image to update
    /// @param _parametersValues New player's parameters values
    /// @param _points New player's points value
    function updateParameters(
        uint256 _cardImageId,
        uint256[] calldata _parametersValues,
        uint256 _points
    ) public onlyOwner {
        CardImageData storage cardImage = cardsImages[_cardImageId];
        ...
        cardImage.points = _points;
        cardImage.pointsUpdateTime = block.timestamp;
    ...
    /// @notice Updates CardsImages' parameters values and points.
    /// @dev Points must be calculated outside of this contract by other "updater" contract. All CardsImages passed to that function must have the same setId. Or you can pass CardsImages with different sets they binded to, but these sets must have the same length.
    ...
    /// @param _points New player's points values, one value for each CardImage.
    function updateParametersForMany(
        uint256[] calldata _cardsImagesIds,
        uint256[] calldata _parametersValues,
        uint256[] calldata _points
    ) external onlyOwner {
        require(_cardsImagesIds.length == _points.length, "Invalid points length");
        ...
        for (uint256 i = 0; i < _cardsImagesIds.length; i++) {
            updateParameters(
                _cardsImagesIds[i],
                _parametersValues[i * parametersLength:(i + 1) * parametersLength],
                _points[i]
            );
        }
    }
    ...
    /// @param _cardImageId Card Image id
    /// @dev Reverts on non-existent card image id. Creates a batch of new tokens binded to specified Card Image.
    function mintCards(address[] calldata _players, uint256 _cardImageId)
        external
        onlyExistingCardImage(_cardImageId)
        onlyOwner
    {
        for (uint256 i = 0; i < _players.length; i++) {
            mintCard(_players[i], _cardImageId);
        }
    }
