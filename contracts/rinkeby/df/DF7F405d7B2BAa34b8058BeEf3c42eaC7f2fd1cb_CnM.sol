...
contract CnM is ICnM, ERC721Enumerable, Ownable, Pausable {
    ...
    event CatStolen(uint256 indexed tokenId, address indexed owner, uint256 blockNum, uint256 timeStamp);
    event CrazyCatLadyStolen(uint256 indexed tokenId, address indexed owner, uint256 blockNum, uint256 timeStamp);
    event MouseStolen(uint256 indexed tokenId, address indexed owner, uint256 blockNum, uint256 timeStamp);
        ...
        if (tx.origin != recipient && recipient != address(habitat)) {
            ...
            if (tokenTraits[minted].isCat) {
                if (tokenTraits[minted].isCrazy) {
                    emit CrazyCatLadyStolen(minted, recipient, block.number, block.timestamp);
                } else {
                    emit CatStolen(minted, recipient, block.number, block.timestamp);
                }

            }
            else {
                emit MouseStolen(minted, recipient, block.number, block.timestamp);
            }
        }
   ...
   * @param tokenId the id of the token to generate traits for
    ...
    */
    ...
    /*** TODO: admin functions that needs to be deleted at launch */
    function updatePaidToken(uint256 limit) external onlyOwner {
        PAID_TOKENS = limit;
    }
}
