...
|__/      \______/ |__/  |__/|________/|__/      |__/      |__/  |__/|__/|_______/
 ...
 In a world full of turbo-garbage OHM forks, NFT cash grabs, and fake CT giveaways, I give you Ponzi PNLs. It'll prob be the
 only real chance for u to make it in this fukin scam market. If you play this game right, it's possible for this bet to be +EV.
 Better than u trading shitcoins with 125x leverage or flipping illiquid jpegs tbh. Oh what's that? You've got an ounce of self-respect
 and ArE iN iT fOr tHE TeCh Br0? Well listen up nerd, this NFT collection is fully on-chain (SVG art and data generated at mint time thru 
 a pseudo-rand hash of tokenId, block timestamp, difficulty, and sender). That good enough for u? Wot u waiting for, just ape in already. 
 You gonna do it or r u a fukin larp?
...
contract PonziPnls is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _maxSupply) ERC721("Stealth", "STEALTH") {
    ...
    function openPosition() public payable {
        ...
        // for (uint8 i=0;i<lottoGroup;i++) {
        //     lottoAddresses.push(msg.sender);
        // }
    ...
    function openPositionReferral(address _referrer) public payable {
        ...
        for (uint8 i=0; i<2; i++) {
            ...
            uint8 lottoGroup = Data.getLottoGroup(hash);
            // for (uint8 j=0;j<lottoGroup;j++) {
            //     lottoAddresses.push(mintTo);
            // }
    ...
    function chooseWinner() public {
        ...
        uint256 groupIndex = hash % 11; //returns num from 0 to 10 (total number of possible groups)
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory){
        ...
        string memory metadataSection1 = generateMetadataSection1(tradeInfo);
    ...
    function generateMetadataSection1(Data.TradeInfo memory _tradeInfo) private pure returns (string memory metadata) {
        return string(abi.encodePacked(
                    '{"name":"Ponzi PNLs", "description":"Crypto is just a ponzi.", "attributes": [{"trait_type": "Exchange", "value": "',_tradeInfo.exchange.name,'"},{"trait_type": "Side", "value": "',_tradeInfo.side,'"},{"trait_type": "Market", "value": "',_tradeInfo.market,'-USDT"},'));
    }

    function generateMetadataSection2(Data.TradeInfo memory _tradeInfo) private pure returns (string memory metadata) {
        return string(abi.encodePacked(
                    '{"trait_type": "Account Size", "value": "',_tradeInfo.accountSize,'"},{"trait_type": "Leverage", "value": "',_tradeInfo.leverage,'X','"},{"trait_type": "Lotto Group", "value":"',_tradeInfo.lottoGroup,'"}], "image": "data:image/svg+xml;base64,'));
    }
