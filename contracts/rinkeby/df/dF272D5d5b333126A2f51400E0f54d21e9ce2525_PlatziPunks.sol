...
contract PlatziPunksDNA {
    ...
    function getMarco(uint256 _dna) public view returns (string memory) {
        uint8 dnaSection = _getDNASection(_dna, 0);
        return _Marco[dnaSection % _Marco.length];
    }

    function getFondo(uint256 _dna) public view returns (string memory) {
        uint8 dnaSection = _getDNASection(_dna, 2);
        return _Fondo[dnaSection % _Fondo.length];
    }

    function getImagen(uint256 _dna) public view returns (string memory) {
        uint8 dnaSection = _getDNASection(_dna, 4);
        return _Imagen[dnaSection % _Imagen.length];
    }

    function getPremio(uint256 _dna) public view returns (string memory) {
        uint8 dnaSection = _getDNASection(_dna, 6);
        return _Premio[dnaSection % _Premio.length];
    }
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "http://83.50.130.142/sergi/";
    }
...

        {
            params = string(
                abi.encodePacked(
                    "marco=", getMarco(_dna),
                    "fondo=", getFondo(_dna),
                    "imagen", getImagen(_dna),
                    "premio", getPremio(_dna)
                )
            );
        }
