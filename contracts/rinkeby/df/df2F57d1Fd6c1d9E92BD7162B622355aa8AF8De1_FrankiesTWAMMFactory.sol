...
contract FrankiesTWAMMFactory {
  ...
  function createFrankiesTWAMM(
    ...
    res = address(new FrankiesTWAMM(_name, _symbol, _tokenA, _tokenB, _orderBlockInterval));
...
// Import and redefined to launch it under the factory and local deploy scripts

contract FrankiesTWAMM is TWAMM {
    constructor(string memory _name
                ,string memory _symbol
                ,address _tokenA
                ,address _tokenB
                ,uint256 _orderBlockInterval
    ) TWAMM(_name, _symbol, _tokenA, _tokenB, _orderBlockInterval) {
    }
}
