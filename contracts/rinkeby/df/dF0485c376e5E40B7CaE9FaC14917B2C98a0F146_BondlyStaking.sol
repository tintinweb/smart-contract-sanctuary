...
contract BondlyStaking is IBondlyStaking, OwnableUpgradeable {
	...
	function _calculateReward() public view returns (uint256) {
		...
		uint256 reward = rewardPerBlock.mul(blocksPassed);
		if (totalPool.add(reward) <= bondlyToken.balanceOf(address(this))) {
			return reward;
		} else {
			return bondlyToken.balanceOf(address(this)).sub(totalPool);
		}
	}
}
