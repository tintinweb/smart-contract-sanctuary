...
contract SlimHoodsSpecials is ERC1155, Ownable {
  ...
  mapping(uint256 => SaleState) saleStates;
  ...
  function setSaleState(uint256 specialNumber, SaleState _saleState) public onlyOwner {
    saleStates[specialNumber] = _saleState;
  }
  ...
  function unmintedSlimHoodIds(uint256 specialNumber, address addr) public view returns (uint256[] memory) {
    uint balance = slimhoodsContract.balanceOf(addr);
    uint[] memory result = new uint[](balance);
    uint256 tokenId;
    uint counter = 0;

    for (uint i = 0; i < balance; i++) {
      tokenId = slimhoodsContract.tokenOfOwnerByIndex(addr, i);
  ...
  function specialHasBeenMinted(uint256 specialNumber, uint256 slimHoodId) public view returns (bool) {
    return minted[specialNumber].contains(slimHoodId);
  }

  function mintSpecialForSlimHood(uint256 specialNumber, uint256 hoodId) public virtual {
    require(saleStates[specialNumber] == SaleState.Open, "Sale is closed");
    require(availableSpecialNumbers[specialNumber], "No special for given number");
    require(slimhoodsContract.ownerOf(hoodId) == msg.sender, "Wallet doesn't hold given SlimHood");
    require(!specialHasBeenMinted(specialNumber, hoodId), "Special already minted for this SlimHood");

    minted[specialNumber].add(hoodId);
