...
contract AniCatsMintpass is ERC1155, Ownable {
    ...
    constructor() ERC1155("https://anicats.herokuapp.com/meta/{id}") {
        giveAway(msg.sender, 1); // auto mint to contract owner

        _transferLocked = false;
        _mintpassEnabled = true;

        _minpassPlans[1] = MintpassPlan(0.0065 ether, 1);  // 0.065 per one ticket
        _minpassPlans[2] = MintpassPlan(0.0180 ether, 3);  // 0.060 per one ticket
        _minpassPlans[3] = MintpassPlan(0.0275 ether, 5);  // 0.055 per one ticket
        _minpassPlans[4] = MintpassPlan(0.0500 ether, 10); // 0.050 per one ticket
    }
