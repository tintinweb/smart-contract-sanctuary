...
contract AcyV1Arbitrage is IAcyV1Arbitrage{
    ...
    IUniswapV2Router02 immutable uniswapRouter02;
    ...
    function getMsg(address inputToken) external view returns(address,address,address){
        return (address(this),msg.sender,inputToken);
    }
    ...
    function swap(uint amountIn,address inputToken,address outputToken) payable external {
        ...
        TransferHelper.safeTransferFrom(
            arr[0], msg.sender, UniswapV2Library.pairFor(factory, arr[0], arr[1]), amountIn
        );
        uint balanceBefore = IERC20(arr[arr.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(arr, address(this));
        require(
            IERC20(arr[arr.length - 1]).balanceOf(address(this)).sub(balanceBefore) >= 0,
