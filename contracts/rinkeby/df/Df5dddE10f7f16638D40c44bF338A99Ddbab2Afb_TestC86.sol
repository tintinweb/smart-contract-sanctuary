...
contract TestC86 is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    constructor() ERC721("PolyChromes", "POLY") {}
    ...
    function setPublicSaleActive(bool val) public onlyOwner {
        publicSaleActive = val;
    }

    function isWhitelisted(address _a) public view returns(bool){
        return whitelistWallets[_a];
    }

    function addWalletsToWhitelist(address[] memory _a) public onlyOwner {
        for(uint256 i; i < _a.length; i++){
            whitelistWallets[_a[i]] = true;
        }
    }

    function removeWalletsFromWhitelist(address[] memory _a) public onlyOwner {
        for(uint256 i; i < _a.length; i++){
            whitelistWallets[_a[i]] = false;
        }
    }
    ...
    //     require(qty <= qtyAllowed, "TRANSACTION: You cant mint more than allowed");
    //     require(qty + nonce <= totalSupply, "SUPPLY: Value exceeds totalSupply");
    ...
    function buy(uint qty) external payable nonReentrant {
        if (!publicSaleActive) {
            require(presaleActive, "TRANSACTION: Presale is not active");
            require(qty + nonce <= totalSupply, "SUPPLY: Value exceeds totalSupply");
            require(whitelistWallets[msg.sender], "TRANSACTION: Wallet not whitelisted");
            ...
            require(publicSaleActive, "TRANSACTION: sale is not active");
            require(qty + nonce <= totalSupply, "SUPPLY: Value exceeds totalSupply");
            require(msg.value == price * qty, "PAYMENT: invalid value");
            for(uint i = 0; i < qty; i++){
                uint tokenId = nonce;
