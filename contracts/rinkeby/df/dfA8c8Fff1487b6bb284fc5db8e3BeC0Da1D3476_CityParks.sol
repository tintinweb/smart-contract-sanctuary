...
contract CityParks is ERC721Enumerable, Ownable {
    ...
    // TODO MAX SUPPLY
    CityParkArt.Art[maxSupply + 1] public cityParks;
    ...
    function _generateImage(uint256 mintNum) private view returns (string memory) {
        ...
        return string(abi.encodePacked(CityParkArt._generateHeader(artData.randomDifficulty,artData), trees, sun, ufo, CityParkArt._imageFooter));
    }
    ...
    function _generateTrees(Art memory artData) internal pure returns (string memory) {
        ...
        for (uint i = 0; i < artData.numTrees; i++) {
            ColorXY memory colorXY = ColorXY({
                x: seededRandom(70,300,i,artData),
                y: seededRandom(150,300,i+1,artData),
