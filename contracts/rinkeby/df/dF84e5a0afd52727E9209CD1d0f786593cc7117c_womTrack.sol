...
contract womTrack is ERC721Tradable {
    ...
    */
    ...
    function initializeWOM(address _twallet, address _dwallet, string memory _name_, string memory _symbol_, uint _reserve, uint256 _maxSupply, uint _maxPurchase, uint256 _price_, uint256 _pricePresale, address _proxyRegistryAddress_, uint256 _royaltyBps_) external {
        require(!initializedWOM, "WOM Contract instance has already been initialized"); //avoid initializing the contract many times like a real constructor
        initializedWOM = true;
        _teamWallet = payable(_twallet);
        _devWallet = payable(_dwallet);
        NAME = _name_;
        SYMBOL = _symbol_;
        _proxyRegistryAddress = _proxyRegistryAddress_;
    ...
    //c'est ici au moemnt de la création de l'instance ERC721 !!!!
    constructor() ERC721Tradable(_name, _symbol, _proxyRegistryAddress) {
