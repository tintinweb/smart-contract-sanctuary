...
contract Marketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, EIP712Upgradeable {
    ...
    function initialize(
        ...
        require(_platformFee <= 1_000, "platform fee must not exceed 1000");
     ...
     *      transfers an asset from seller to buyer,
     *      transfers the payment tokens from buyer to seller,
     *      transfers the fee from buyer to fee recipient
     *
    ...
    function _acceptSellOrder(IMarketplace.SellOrder memory _sellOrder) private {
        // calculate payment amount, _sellOrder.initialLotPrice is always greater than or equal to _sellOrder.finalLotPrice by _verifyPayment() function
    ...
    /**
     * @notice Executed by the seller to accept a buy order
     *
     * @dev whenNotPaused
    ...
    function _acceptBuyOrder(IMarketplace.BuyOrder memory _buyOrder) private {
        // calculate payment amount, _buyOrder.initialLotPrice is always greater than or equal to _buyOrder.finalLotPrice by _verifyPayment() function
    ...
    function _verifyPayment(
        ...
        require(maxLotPrice >= _initialLotPrice && _finalLotPrice >= minLotPrice, "lot price is out of range");
        require(_initialLotPrice >= _finalLotPrice, "final lot price cannot exceed initial price");

        // check unit price range
        require(maxUnitPrice >= _initialUnitPrice && _finalUnitPrice >= minUnitPrice, "unit price is out of range");
        require(_initialUnitPrice >= _finalUnitPrice, "final unit price cannot exceed initial price");
    }
    ...
    function _verifyOrderDuration(uint256 _startsAt, uint256 _expiresAt) private view {
        require(_startsAt < _expiresAt, "malformed duration");
    ...
    /**
     * @dev Verifies sell order, it's signature, current NFT ownership and payment
     *
    ...
    function _validateSellOrder(IMarketplace.SellOrder memory _sellOrder, bytes memory _signature) private view {
        ...
        _verifyOrderDuration(_sellOrder.startsAt, _sellOrder.expiresAt);

        // check timestamp
        require(_sellOrder.startsAt <= _getNow() && _getNow() <= _sellOrder.expiresAt, "inactive sell order");

        // check nft, ownership and payment
        _verifyNFTAndOwner(_sellOrder.tokenAddress, _sellOrder.tokenId, _sellOrder.owner);
        _verifyPayment(
            _sellOrder.quantity,
    ...
    /**
     * @dev Verifies buy order, it's signature, current NFT ownership and payment
     *
    ...
    function _validateBuyOrder(IMarketplace.BuyOrder memory _buyOrder, bytes memory _signature) private view {
        ...
        // check order duration
        _verifyOrderDuration(_buyOrder.startsAt, _buyOrder.expiresAt);
        ...
        require(_buyOrder.initialLotPrice == _buyOrder.finalLotPrice, "varying price not supported");

        // check duration, nft, its ownership and payment
        _verifyOrderDuration(_buyOrder.startsAt, _buyOrder.expiresAt);
        _verifyNFTAndOwner(_buyOrder.tokenAddress, _buyOrder.tokenId, msg.sender);
        _verifyPayment(
            _buyOrder.quantity,
