...
contract Vault is ERC721Holder, ERC1155Holder {
    ...
    NFT public fractions;
    NFT public underlying;
    ...
    function initialize(address _fractions, uint256 _fractionsID, address _underlying, uint256 _underlyingID) external {
        require(!initialized, "initialized");
        initialized = true;
        fractions.token = _fractions;
        fractions.id = _fractionsID;
        underlying.token = _underlying;
        underlying.id = _underlyingID;
    ...
    function updateUserPrice(uint256 _new) external {
        uint256 balance = IFERC1155(fractions.token).balanceOf(msg.sender, fractions.id);
    ...
    function sort(uint256[] memory array, uint256 begin, uint256 last) internal pure {
        if (begin < last) {
            uint256 j = begin;
    ...
    function reservePrice() public view returns(uint256 voting, uint256 reserve) {
    ...
    function start() external payable {
        ...
        require(voting * 2 >= IFERC1155(fractions.token).totalSupply(fractions.id), "start:not enough voters");
    ...
    function redeem() external {
        ...
        IFERC1155(fractions.token).burn(msg.sender, fractions.id, IFERC1155(fractions.token).totalSupply(fractions.id));
        IERC721(underlying.token).transferFrom(address(this), msg.sender, underlying.id);
    ...
    function cash() external {
        ...
        uint256 bal = IFERC1155(fractions.token).balanceOf(msg.sender, fractions.id);
        require(bal > 0, "cash:no tokens to cash out");
        uint256 share = bal * address(this).balance / IFERC1155(fractions.token).totalSupply(fractions.id);
        
        IFERC1155(fractions.token).burn(msg.sender, fractions.id, bal);
    ...
    /// @param _token the address of the ERC20 you are withdrawing
