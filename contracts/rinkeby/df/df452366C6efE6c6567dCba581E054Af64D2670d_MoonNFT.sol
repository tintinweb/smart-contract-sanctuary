...
contract MoonNFT is ReferralNFT {
    mapping (Tier => uint256) _supplyByTier;

    mapping (Tier => uint256) _reservedByTier;
    ...
    function getReservedLeft() public view override returns(uint256) {
    ...
    function claimReserved(uint256, address) public pure override {
    ...
    function mintReferral(uint256, address payable) public payable override {
        require(false, "Not implemented");
    }
    ...
    function mintTierReferral(Tier tier, uint256 nTokens, address payable referral) whenSaleStarted public payable {
        ...
        // if (referral != address(0x0) && referral != msg.sender && referral != owner()) {
        ...
        //     pendingWithdrawals[owner()] += msg.value * REFERRAL_PERCENT / 10000;
        // }

        // // so that total of pendingWithdrawals is always the same as the contract
        // pendingWithdrawals[owner()] += msg.value * (10000 - REFERRAL_PERCENT) / 10000;
...
contract ReferralNFT is AvatarNFT {
    ...
    function mintReferral(uint256 nTokens, address payable referral) whenSaleStarted external payable virtual {
        ...
        // TODO: check that msg.value is passed over!
        _updateReferral(nTokens, referral);
    }
