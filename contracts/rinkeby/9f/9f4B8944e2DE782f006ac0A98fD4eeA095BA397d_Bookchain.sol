...
import "@openzeppelin/contracts/utils/Strings.sol"; // remove
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import "@openzeppelin/contracts/interfaces/IERC2981.sol";
import "hardhat/console.sol";

contract Bookchain is ERC721Enumerable, Ownable, IERC2981 {
    using Strings for uint256;

    mapping (uint256 => uint256[]) public children;
    mapping (uint256 => uint256) public parent;
    mapping (uint256 => string) public sentence;
    mapping (address => bool) private _hasClaimedAllowlist; 
    mapping (uint256 => string) public imageStyle;

    uint256 constant STORY_ROOT = 0;
    ...
    constructor() ERC721("Lore","LORE") {
        ...
        sentence[STORY_ROOT] = '';
        imageStyle[STORY_ROOT] = '';
        _safeMint(msg.sender, STORY_ROOT);

    }

    function mintSeed(string calldata _sentence, string calldata _imageStyle) public payable {
    ...
    function mintSeedBacker(string calldata _sentence,
                            string calldata _imageStyle,
                            bytes32[] calldata _merkleProof) public
    {
        ...
        require(!_hasClaimedAllowlist[msg.sender], "Already claimed");
        _hasClaimedAllowlist[msg.sender] = true;
        _mint(STORY_ROOT, _sentence, _imageStyle);
    }
    /* Convenience methods */
    ...
    function setEarlyBakerMerkleRoot(bytes32 _root) public onlyOwner {
        mirrorMerkleRoot = _root;
    }
    function mint(uint256 _parentId, string calldata _sentence, string calldata _imageStyle) public {
        require(_parentId != 0, "mint through mintSeed");
        _mint(_parentId, _sentence, _imageStyle);
    }
    function _mint(uint256 _parentId, 
                    string calldata _sentence,
                    string calldata _imageStyle) internal returns (uint256 tokenId) {
        tokenId = totalSupply();
     ...
     * ROYALTIES
     */
    address public constant ROYALTY_ADDRESS = 0xeD2482a8544Ca1D3A986Aa686E6F003e0E988C96;
