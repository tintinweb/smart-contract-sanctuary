...
contract Vault is Ownable, Pausable, IERC721Receiver, ReentrancyGuard {
    ...
    mapping(address => ERC721Asset) public assets;
    mapping(address => mapping(address => EnumerableSet.UintSet)) _deposits;
    mapping(address => mapping(address => mapping(uint256 => uint256))) _depositedAt;
    ...
    function setAsset(address _asset) external onlyOwner {
        ...
        asset.reward = 10;
        asset.lock  = 5 minutes;
        asset.recur = 1 minutes;
    ...
    function setLock(address _asset, uint256 _seconds) external onlyOwner {
        assets[_asset].lock = _seconds;
    }


    function setReward(address _asset, uint256 _reward) external onlyOwner {
        assets[_asset].reward = _reward;
    }


    function setRecurrence(address _asset, uint256 _seconds) external onlyOwner {
        assets[_asset].recur = _seconds;
    }


    function recurrences(address _asset, uint256 _token) public view returns (uint256) {
       return (block.timestamp.sub(_depositedAt[_asset][msg.sender][_token])).div(assets[_asset].recur);
    }


    function isDeposited(address _asset, uint256 _token) public view returns(bool) {
        return _deposits[_asset][msg.sender].contains(_token);
    }


    function isReleased(address _asset, uint256 _token) public view returns(bool) {
        return block.timestamp - _depositedAt[_asset][msg.sender][_token] >= assets[_asset].lock;
    }
    ...
    function deposit(address _asset, uint256[] calldata _tokens) external whenNotPaused {
        for(uint256 token; token < _tokens.length; token++) {
            IERC721(assets[_asset].addr).safeTransferFrom(msg.sender, address(this), _tokens[token]);
            _deposits[_asset][msg.sender].add(_tokens[token]);
            _depositedAt[_asset][msg.sender][_tokens[token]] = block.timestamp;
        }
    }


    function calculateReward(address _asset, uint256 _token) public view returns (uint256) {
        if(!isDeposited(_asset, _token) || !isReleased(_asset, _token)) return 0;
        return recurrences(_asset, _token).mul(assets[_asset].reward);
    }


    function calculateRewards(address _asset, uint256[] memory _tokens) public view returns (uint256[] memory rewards) {
    ...
    function withdraw(address _asset, uint256[] calldata _tokens) external whenNotPaused {
        ...
        for(uint256 token; token < _tokens.length; token++) {
            ...
            _deposits[_asset][msg.sender].remove(_tokens[token]);
    ...
    function claimRewards(address _asset, uint256[] calldata _tokens) public whenNotPaused nonReentrant() {
        ...
        for(uint256 token; token < _tokens.length; token++) {
            ...
            _depositedAt[_asset][msg.sender][_tokens[token]] += recurrences(_asset, _tokens[token]).mul(assets[_asset].recur);
        }
