...
contract ToneExchange{
    ...
    constructor(Tone _toneTokn, UsdcToken _usdc) {
        toneTokn = _toneTokn;
        // airdrop = _airdrop;
        usdc = _usdc;
    }  
    
    function returnSellOrders() public view returns(Order[] memory) {
        return sellOrders;
    }

    function returnBuyOrders() public view returns(Order[] memory) {
        return buyOrders;
    }
    ...
    function sellLimitOrder(uint _qty, uint _price) public {
        ...
        if(buyOrders.length == 0) {
            ...
            if(_price <= target.price) {
                if(_qty <= target.qty) {
                    toneTokn.transferFrom(msg.sender, buyer[buyId], _qty);
                    buyOrders[buyId].qty -= _qty;
                    usdc.transfer(msg.sender, _qty * target.price);
                    // airdrop.airdropTracker(id, buyer[id][buyId], _qty * target.price);
                } else {
                    toneTokn.transferFrom(msg.sender, buyer[buyId], target.qty);
                    usdc.transfer(msg.sender, target.qty * target.price);
    ...
    function buyLimitOrder(string memory _title, uint _qty, uint _price) public {
        ...
        if(sellOrders.length == 0) {
            ...
            if(_price >= target.price) {
                if(_qty <= target.qty) { 
                    toneTokn.transferFrom(seller[sellId], msg.sender, _qty);
                    usdc.transfer(seller[sellId], _qty * target.price);
                    ...
                    toneTokn.transferFrom(seller[sellId], msg.sender, target.qty);
                    usdc.transfer(seller[sellId], target.qty * target.price);
                    ...
                    buyLimitOrder(_title, _qty - target.qty, _price);
                }
            } else {
                addBuyOrder(_qty, _price);
            }
    ...
    function sellMarketOrder(uint _qty) public {
        ...
        if(_qty <= target.qty) {
            toneTokn.transferFrom(msg.sender, buyer[buyId], _qty);
            usdc.transfer(msg.sender, _qty * buyOrders[buyId].price);
            // airdrop.airdropTracker(id, buyer[id][buyId], _qty * buyOrders[id][buyId].price);
            buyOrders[buyId].qty -= _qty;
        } else {
            toneTokn.transferFrom(msg.sender, buyer[buyId], target.qty);
            usdc.transfer(msg.sender, target.qty * buyOrders[buyId].price);
    ...
    function buyMarketOrder(uint _qty) public {
        ...
        if(_qty <= target.qty) {
            ...
            toneTokn.transferFrom(seller[sellId], msg.sender, _qty);
            usdc.transfer(seller[sellId], _qty * sellOrders[sellId].price);
            ...
            toneTokn.transferFrom(seller[sellId], msg.sender, target.qty);
            usdc.transfer(seller[sellId], target.qty * sellOrders[sellId].price);
    ...
    //     req.sellId = lowestAskOrder();
    //     req.askPrice = sellOrders[req.sellId].price;
    //     req.buyId = highestBidOrder();
    //     req.bidPrice = buyOrders[req.buyId].price;
    //     req.finalPrice = ((req.askPrice + req.bidPrice)/2) * (1 + _premium); 
    //     address[] memory addrs = toneTokn.returnAddressesForTokn();
    //     for(uint i = 0; i < addrs.length; i++) {
    //         toneTokn.transferFrom(addrs[i], toneTokn.owner(), _id, toknFactory.balanceOf(addrs[i], _id), "");
    //         usdc.transfer(addrs[i], toneTokn.balanceOf(addrs[i]) * req.finalPrice);
    //     }
    // }
}
