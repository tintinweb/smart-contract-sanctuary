...
contract PudgyPenguins is ERC721Burnable, ERC721Pausable, RandomlyAssigned, WithSaleStart, WithContractMetaData, WithIPFSMetaData {
    ...
    constructor(uint256 maxElements, uint256 publicSaleStartTime, string memory contractUri, string memory cid)
    ...
    function mint(uint256 _count) external payable saleIsOpen {
        ...
        if (isPublicSale) {
            ...
            require(amountMintedPublic[_msgSender()] <= MAX_BY_MINT_PUBLIC);
        } else {
            amountMintedPrivate[_msgSender()] = amountMintedPrivate[_msgSender()].add(_count);
            require(amountMintedPrivate[_msgSender()] <= MAX_BY_MINT_PRIVATE);

            uint8 requiredTicketAmount = uint8(_count / MAX_BY_MINT_PRIVATE);
            if (_count.mod(MAX_BY_MINT_PRIVATE) > 0) {
                requiredTicketAmount = requiredTicketAmount + 1;
                //TODO: review & refactor???
            }

            // e.g. 3 to mint, 3 mintables per ticket
            require(whitelist[_msgSender()] >= requiredTicketAmount, "Not enough tickets");

            whitelist[_msgSender()] = whitelist[_msgSender()] - requiredTicketAmount;
        }
