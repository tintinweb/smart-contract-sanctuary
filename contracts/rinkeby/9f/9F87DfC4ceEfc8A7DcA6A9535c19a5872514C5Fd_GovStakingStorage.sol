...
contract GovStaking is Ownable, ReentrancyGuard, Pausable {
	uint256 opt1 = 0;
	uint256 opt2 = 604800; // 1 week
	uint256 opt3 = 2629746; // 1 month
	uint256 opt4 = 15778476; // 6 month
	uint256 opt5 = 31556952; // 1 year
	uint256 opt6 = 94608000; // 3 years

	mapping(uint256 => uint256) rewardRates; // locking period => rewardRates in wei per second per locked gogo
	...
	constructor(
		...
		rewards = StakingRewards(stakingRewards);
		...
		whenNotPaused
	{
		...
		GovStakingStorage.UserInfo memory user = store.getUserInformations(
			msg.sender
		);
		...
		if (user.amount == 0) {
			...
			if (user.lockStart + user.lockPeriod <= block.timestamp) {
				...
				user.lastClaimed = block.timestamp;
				uint256 rewardRate = extendingPeriod > 0
