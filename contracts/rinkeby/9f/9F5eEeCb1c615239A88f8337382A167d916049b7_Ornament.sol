...
/// @title Ornament ERC721 Contract
/// @author Jake Allen
contract Ornament is Ownable, ERC721Enumerable {
    ...
    // SVGs, clusters and weight per tokenId. IMPORTANT: must be the same size.
    // TODO: compare gas costs of 3 arrays vs struct
    uint256 public maxTokens = 256;
    string[256] public svgs;
    uint256[256] public clusters;
    string[256] public weights;
    string public baseUrl;

    // Sale status. Toggle to enable minting. // TODO: change name of this variable
     ...
     * TODO: can we reduce gas costs with a batch upload.
     */
    function updateToken(
        ...
        onlyOwner
    {
        // TODO: require that we haven't revoked this ability yet
        svgs[tokenId] = _svg;
        clusters[tokenId] = _clusters;
    ...
    /**
     * @notice Withdraw contract balance to owner of contract (artist).
     */
    ...
    /**
     * @notice Override isApprovedForAll to whitelist user's OpenSea proxy
    ...
    function mint() external payable {
        ...
        require(totalSupply() + 1 <= maxTokens, "Maximum supply reached.");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory encodedSVG = Base64.encode(bytes(svgs[tokenId]));
        // Metadata
        uint256 _clusters = clusters[tokenId];
        string memory _weight = weights[tokenId];
        // Pack and encode the entire json string
        string memory json = Base64.encode(abi.encodePacked(
            '{"name":"', name, '", "description":"', description, '", "image":"data:image/svg+xml;base64,', encodedSVG, '", "attributes":[{"trait_type":"Clusters","value":"', _clusters,'"}, {"trait_type": "Weight", "value":"', _weight,'"}]', bytes(baseUrl).length > 0 ? string(abi.encodePacked(', "external_url":"', baseUrl,tokenId.toString(), '"')) : "",' }'
        ));
