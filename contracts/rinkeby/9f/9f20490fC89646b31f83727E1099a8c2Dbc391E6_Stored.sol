...
contract Stored is Ownable, Pausable, ReentrancyGuard {
    ...
    function depositHigh(uint256 amount_) external nonReentrant whenNotPaused {
    ...
    function withdrawHigh(uint256 amount_) external nonReentrant whenNotPaused {
        User storage user = users[msg.sender];
    ...
    function updateUserBalance(address userAddr_, uint256 amount_) external whenNotPaused onlyManager {
    ...
    function getDepositHistory(address userAddr_, uint256 depositId_) external view returns (Deposit memory) {
        return users[userAddr_].deposits[depositId_];
    }

    function getWithdrawalHistory(address userAddr_, uint256 withdrawalId_) external view returns (Withdrawal memory) {
