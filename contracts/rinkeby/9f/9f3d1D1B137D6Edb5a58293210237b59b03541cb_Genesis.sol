...
contract Genesis is ERC721, Ownable {
    ...
    //sale holders: probably need to keep this to distribute funds to his client
    ...
    uint256[] public receivePercentagePt = [3000, 3000]; //distribution in basis points, last address takes the rest (TBD)
    ...
    modifier onlyEOA() {
        require(msg.sender == tx.origin, "Genesis: Only EOA");
        _;
    }
    ...
    constructor(
        ...
        // Take note of changes in deployment args for other relevant files. No need for treasury.
    ...
    function publicMint() public payable onlyEOA {
        ...
        require(publicSaleOpen, "Genesis: Public Sale is not open");
        require(
            tokenSupply.current() + 1 <= maxSupply,
            "Genesis: Total mint amount exceeded"
        );
        require(msg.value == cost, "Genesis: Insufficient ETH");
        require(
            addressToMintedAmount[msg.sender] == 0,
            "Genesis: Wallet has already minted"
        );
        require(
            addressToMintedAmount[msg.sender] < 1,
            "You have already minted your pass"
        );
        ...
        payable
    {
        ...
        // TODO: Create Whitelist Sale Window (manually toggled)
        require(presaleOpen, "Genesis: Presale is not open");
        require(
            tokenSupply.current() + 1 <= maxSupply,
            "Genesis: Total mint amount exceeded"
        );
        ...
        // TODO: Create whitelist mint limit? (Done)
        require(
            addressToMintedAmount[msg.sender] < 1,
            "You have already minted your pass"
        );

        // TODO: Create whitelist mint limit per transaction? (Not Required)
