 ...
 */
contract TheColors is ERC721, Ownable {
    ...
    uint256 public constant colorPrice = 100000000000000000; //0.1 ETH
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply, uint256 saleStart) ERC721(name, symbol) {
        ...
        _setBaseURI('https://ipfs.io/ipfs/QmNYEsEaDGpfrQbzJNxK61yTtQXF1bU9oSFvr2ZAgMbNdM');
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
        ...
        string memory _tokenId = startingIndex > 0 ? tokenId.toString() : '-1';
    ...
    /**
     * Set some Colors aside
     */
    function reserveColors() public onlyOwner {
    ...
    /**
     * Set reveal timestamp once it's known
     */
    ...
    */
    ...
    function setFinalBaseURI(string memory baseURI) public onlyOwner {
        setBaseURI(baseURI);
    ...
    */
    function mintColor(uint numberOfTokens) public payable {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_COLORS || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % MAX_COLORS;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number - startingIndexBlock > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % MAX_COLORS;
        }
