...
contract CoolDogs is ERC721Enumerable, Ownable {
	...
	//global variables
	uint256 public constant CD_PRIVATE = 1600;
	uint256 public constant CD_PUBLIC = 5827;
	uint256 public constant CD_MAX = CD_PRIVATE + CD_PUBLIC;
	uint256 public constant CD_PRICE = 0.07 ether;
	uint256 public constant CD_PER_MINT = 20;

	//private variables addresses
	address private _signerAddress;
	address private _withdraw1 = 0xa27306b3fC394035e4d61D92cfd17ad7fbE28a34;
	address private _withdraw2 = 0xa20F9eceF204C969B0AC4D7F7dE20ABAE901E5D1;
	...
	constructor() ERC721('CoolDogs', 'CDO') {
	...
	//allows users to mint a CoolDogs
	function buy(uint256 tokenQuantity) external payable {
		require(saleLive, 'SALE_CLOSED');
		require(!presaleLive, 'ONLY_PRESALE');

		require(totalSupply() < CD_MAX, 'OUT_OF_STOCK');
		require(publicAmountMinted + tokenQuantity <= CD_PUBLIC, 'EXCEED_PUBLIC');
		require(tokenQuantity <= CD_PER_MINT, 'EXCEED_CD_PER_MINT');
		require(CD_PRICE * tokenQuantity <= msg.value, 'INSUFFICIENT_ETH');
	...
	function presaleBuy(uint256 tokenQuantity) external payable {
		...
		require(totalSupply() < CD_MAX, 'OUT_OF_STOCK');
		require(
			privateAmountMinted + tokenQuantity <= CD_PRIVATE,
			'EXCEED_PRIVATE'
		);
		require(
