...
contract ShieldMining is IShieldMining, OwnableUpgradeable, ReentrancyGuard, AbstractDependant {
    ...
    function _updateReward(
        ...
        if (_nearestLastBlocksWithReward != 0 && block.number > _nearestLastBlocksWithReward) {
    ...
    function _updateNearestLastBlocksWithReward(address _policyBook) internal {
        ...
        for (uint256 i = 0; i < lastBlocksWithReward[_policyBook].length(); i++) {
            ...
            if (
                _lastBlock <= _nearestLastBlocksWithReward &&
                _lastUpdateBlock < _nearestLastBlocksWithReward
            ) {
