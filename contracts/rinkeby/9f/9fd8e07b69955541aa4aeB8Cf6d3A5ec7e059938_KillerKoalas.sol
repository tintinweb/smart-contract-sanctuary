...
contract KillerKoalas is ERC721URIStorage, Ownable {
   ...
   uint256 public MAX_MINT = 9999; // Total Number of NFTs
   uint256 private _price = 33000000000000000; //Price of NFTs 0.033 ETH

   event UpdatedMessages(uint256 publicIssued);
   event mintedNFTID(address sender, uint256 tokenId);

   constructor(string memory baseURI) ERC721("Killer Koalas", "KK") {
     setBaseURI(baseURI);
   }
  ...
  function reserveNFTs() public onlyOwner {
     require(SafeMath.add(_tokenIds.current(), 10) <= MAX_MINT , "Exceeds maximum token supply.");
     for (uint i = 0; i < 10; i++) {
          _mintSingleNFT();
     }
   }

   function updateSaleState(bool newState) public onlyOwner {
      activeSale = newState;
    }

   function mintNFT(uint256 numTokens) public payable{
   ...
   function _mintSingleNFT() private {
      ...
      publicIssued += 1;
      emit UpdatedMessages(publicIssued);
      emit mintedNFTID(msg.sender, publicIssued);
   }
