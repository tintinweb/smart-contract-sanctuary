...
contract DateUnlockSchedule is BaseGovernanceWithUserUpgradable {
    ...
    function unlockedAmount(uint256 lockStartTimestamp, uint256 initialAmount) external view returns(uint256) {
        ...
        if(block.timestamp < unlockDates[0]) {
            _unlockedAmount = 0;
        } else if (block.timestamp >= unlockDates[unlockDates.length - 1]) {
            _unlockedAmount = initialAmount;
        } else {
            uint256 actualSumAmounts = 0;
            for(uint i = 0; i < unlockDates.length; i++){
                if(block.timestamp < unlockDates[i]){
                    _unlockedAmount = (initialAmount * actualSumAmounts) / 10 ** 18;
                }
                actualSumAmounts += normalizedAmounts[i];   
            }
        }
    ...
    function endLockTime() external view returns(uint) {
        return unlockDates[unlockDates.length - 1];
    }
