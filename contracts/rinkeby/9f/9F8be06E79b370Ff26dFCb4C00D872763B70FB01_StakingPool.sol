...
contract StakingPool is ReentrancyGuard, Pausable, Ownable {
    ...
    mapping(address => uint8) public stakedFemales;
    mapping(address => uint8) public stakedMales;
    mapping(address => uint8) public stakedBabies;
    ...
    mapping(uint256 => uint8) private maxBabies;
    mapping(uint256 => uint8) private multiplier;
    mapping(address => uint8) public userMultiplier;
    ...
    function initiateMaxBabies(uint256[] memory female, uint8[] memory _maxBabies) external onlyOwner {
    ...
    function initiateMultipliers(uint256[] memory male, uint8[] memory _multiplier) external onlyOwner {
    ...
    function _breeding() internal {
        if (stakedFemales[msg.sender] == 1 && stakedMales[msg.sender] == 1) {
            breedingEnd[msg.sender] = now + 2 minutes;
        }
    }
