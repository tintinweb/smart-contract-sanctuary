...
// import "https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/master/contracts/token/ERC721/extensions/ERC721PausableUpgradeable.sol";
...
// import "https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/master/contracts/utils/cryptography/ECDSAUpgradeable.sol";
...
contract Skoolverse is ERC721Upgradeable, ERC721PausableUpgradeable, OwnableUpgradeable {
  ...
  mapping(uint => uint) public placedOn;
  mapping(uint => address) public placedBy;
  mapping(uint => uint) public level;
  event Stay(address indexed from, uint indexed tokenId, uint timestamp);
  event Leave(address indexed from, uint indexed tokenId, uint timestamp);
  event Activated(address indexed from, uint indexed tokenId, uint timestamp);
  ...
  function stay(uint _scholarzId, uint _landId) internal {
    ...
    placedOn[_scholarzId] = block.timestamp;
  ...
  function activate(uint _landId) internal {
    ...
    require(placedOn[tenantOf[_landId]] >= 0, "Scholarz is not yet staked");
    require(block.timestamp - placedOn[tenantOf[_landId]] >= 2 weeks, "Scholarz must be staked at least 2 weeks");
    landActive[_landId] = true;
    if (level[_landId] == 0) {
      level[_landId] = 1;
    }
    emit Activated(msg.sender, _landId, block.timestamp);
  }
  ...
  function _beforeTokenTransfer(address from, address, uint256 tokenId) internal whenNotPaused override(ERC721Upgradeable, ERC721PausableUpgradeable) {
    if (from != address(0)) {
      ...
      placedOn[tenantOf[tokenId]] = 0;
      // homeOf[tenantOf[tokenId]] = 0;
      // tenantOf[tokenId] = 0; 
    }
  ...
  // external
  function claimMultiple(uint[] memory _tokenIds) external {
  ...
  function activateMultiple(uint[] memory _landIds) external {
