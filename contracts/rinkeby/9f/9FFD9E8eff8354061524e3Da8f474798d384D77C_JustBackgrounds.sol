...
contract JustBackgrounds is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 private _tokenMaxPerTxn = 10;
    uint256 private _tokenMaxSupply = 256;
    uint256 private _tokenMaxReserved = 10;
    ...
    modifier whenNotSoldOut {
        require(totalSupply() < _tokenMaxSupply, "JustBackgrounds: Contract is sold out");
        _;
    }

    modifier whenInitialized {
        require(_isInitialized, "JustBackgrounds: Metadata not initialized");
        _;
    }

    modifier whenNotInitialized {
        require(_unusedColors.length == 0, "JustBackgrounds: Unused colors modified out-of-band");
        require(!_isInitialized, "JustBackgrounds: Metadata already initialized");
        _;
    }

    modifier whenReserved {
        require(_isReserved, "JustBackgrounds: Owner tokens not reserved yet");
        _;
    }

    modifier whenNotReserved {
        require(!_isReserved, "JustBackgrounds: Owner tokens already reserved");
        _;
    }

    /// Metadata Functions ///
    function initializeMetadata() external whenNotInitialized {
    ...
    function _randomNumber(uint256 seed) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, blockhash(block.number), seed)));
    }

    /// Minting Functions ///
    function mintCollectibles(uint256 howMany) external payable whenInitialized whenNotPaused whenNotSoldOut {
        require(totalSupply() + howMany <= _tokenMaxSupply, "JustBackgrounds: Qty exceeds max supply");
        require(msg.value >= howMany * _tokenPrice, "JustBackgrounds: Not enough ether sent");
        require(howMany <= _tokenMaxPerTxn, 'JustBackgrounds: Qty exceed max per txn');
        
        for (uint256 i = 0; i < howMany; i++) {
            _mintCollectible(msg.sender, uint8(_randomNumber(i) % _unusedColors.length));
        }
    }

    /**
     * @dev Reserve owner floor collectibles.
     */
    function reserveCollectibles() external onlyOwner whenInitialized whenNotReserved {
        require(totalSupply() == 0, "JustBackgrounds: Tokens minted out of band");
        _isReserved = true;
        for (uint256 i = 0; i < _tokenMaxReserved; i++) {
            _mintCollectible(msg.sender, uint8(i));
        }
    }
     ...
     */
    function _mintCollectible(address to, uint8 colorIndex) internal nonReentrant {
        require(colorIndex < _unusedColors.length, "JustBackgrounds: ColorIndex out of range");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "GRND: URI query for nonexistent token");
        ...
        byteString = abi.encodePacked(byteString
            ,'"description": "**Just Backgrounds** (b. 2021)\\n\\n'
            ,"***", traits.name, "***, #", traits.hexCode
            ,'",'
        );
        byteString = abi.encodePacked(byteString
        ...
        if (keccak256(abi.encodePacked(traits.special)) != keccak256('None')) {
    ...
    /**
     * @dev Generates the three types of SVGs.
     */
    function _generateSvgFromTraits(Traits.ColorTraits memory traits) internal pure returns (bytes memory svg) {

        svg = abi.encodePacked(svg, "<svg xmlns='http://www.w3.org/2000/svg' width='1920' height='1920'>");

        if (keccak256(abi.encodePacked(traits.special)) == keccak256('Meme')) {
        ...
        } else if (keccak256(abi.encodePacked(traits.special)) == keccak256('Transparent')) {
    ...
    /// Sale Functions ///
    function startSale() external onlyOwner whenInitialized whenReserved {
        _unpause();
    }
...

{
  ...
  "libraries": {
    "contracts/Colors.sol": {
      "Colors": "0x860ec8772fb2b7b180a45437bffda174f6adc56a"
    },
    "contracts/Traits.sol": {
      "Traits": "0xf0db573d0c5a32767b7ddf7e4232d7112e6d8e99"
    }
  }
}