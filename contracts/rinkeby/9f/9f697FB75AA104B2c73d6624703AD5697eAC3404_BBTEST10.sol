...
/**
 * @title BBTEST10 contract
 * @author pasalog
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract BBTEST10 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using SafeMath for uint256;

    string public baseURI; // baseURI should set with URI/
    ...
    // Sale dates ===============================
    uint public presaleStartTimestamp = 1635868800;
    uint public publicSaleStartTimestamp = 1635868800;

    // Count values =============================
    uint256 public MAX_ITEMS = 10000;
    uint256 public mintedItems = 0;
    ...
    // Mint without validation
    function _mintWithoutValidation(address to, uint amount) internal {
        require(mintedItems + amount <= MAX_ITEMS, "All items sold");
        for (uint i = 0; i < amount; i++) {
            _mint(to, mintedItems);
            emit Mint(to, mintedItems);
            mintedItems += 1;
        }
    }
    ...
    function presaleMint(uint _mintAmount) public payable {
        ...
        require(totalSupply().add(_mintAmount) <= MAX_ITEMS, "Purchase would exceed max supply");
        require(presaleCost.mul(_mintAmount) <= msg.value, "Value sent is not correct");
        require(_mintAmount > 0);
        require(_mintAmount <= maxMintAmount, "Over the maxItemsPerTx");
        require(supply + _mintAmount <= MAX_ITEMS);
        require(whitelisted[msg.sender] == true, "You need to be in whitelist for presale purchase");
        require(presaleWallets[msg.sender] <= 10, "Max 10 items can be purchased by per wallet in presale period");
    ...
    */
    function publicSaleMint(uint _mintAmount) public payable {
        uint256 supply = totalSupply();
        require(!paused, "Contract is paused");
        require(block.timestamp >= publicSaleStartTimestamp, "Public sale is not opened yet");
        require(totalSupply().add(_mintAmount) <= MAX_ITEMS, "Purchase would exceed max supply of Basic Apes");
        require(publicSaleCost.mul(_mintAmount) <= msg.value, "Ether value sent is not correct");
        require(_mintAmount > 0);
        require(_mintAmount <= maxMintAmount, "Over the maxItemsPerTx");
        require(supply + _mintAmount <= MAX_ITEMS);
    ...
    function setPresaleStartTimestamp(uint _startTimestamp) external onlyOwner {
        require(msg.sender == owner(), "Now allowed address for this interaction");
    ...
    // Set presale cost - only owner
    function setPresaleCost(uint256 _newCost) public onlyOwner {
        // require(msg.sender == owner(), "Now allowed address for this interaction");
        presaleCost = _newCost;
    }

    // Set public sale cost - only owner
