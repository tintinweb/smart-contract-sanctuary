...
contract HashBridge is Ownable {
	...
	function getOrderForArbitration(address arbitratorAddress, uint startId) external view returns(string memory) {
	    for (uint i = startId; i < orders.length; i++) {
			if (orders[i].paid == false || orders[i].complete == true || orders[i].declined == true) {
			    continue;
			}
			bool appropriate = true;
			for (uint j = 0; j < votes.length; j++) {
			    if (votes[j].orderId == i && votes[j].voterAddress == arbitratorAddress) {
			        appropriate = false;
			        break;
			    }
			}
		    if (appropriate) {
		        return string(abi.encodePacked(i, orders[i].payAmount, offers[orders[i].offerId].payToken, orders[i].payAddress));
		    }
		}
		return '';
	}

	function checkPayment(uint orderId, uint payAmount, address payToken, address payAddress) external view returns(bool) {
	    for (uint i = 0; i < payments.length; i++) {
	        if (payments[i].orderId == orderId && payments[i].payAmount == payAmount && payments[i].payToken == payToken && payments[i].payAddress == payAddress) {
	            return true;
	        }
	    }
	    return false;
