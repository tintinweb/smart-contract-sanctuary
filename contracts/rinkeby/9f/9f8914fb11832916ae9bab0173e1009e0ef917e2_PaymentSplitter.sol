...
contract PaymentSplitter is Ownable {
    ...
    //Free mint period state
    bool freeMintPeriod = false;
    ...
    * @param _bonusNFTMultipliers multipliers of bonus NFTs (length must match IDs) 100% is 10^18
    ...
    function setMinXGDAOFree(uint _new) external onlyOwner{
        minXGDAOFree = _new;
    }

    function setFreeMintPeriod(bool _new) external onlyOwner {
        freeMintPeriod = _new;
    }
    ...
    * @return the price
    ...
    function getPrice(uint _buyerCount, address _referrer) public view returns(uint256) {
        ...
        // Check if still in free period
        if(freeMintPeriod == false){
