...
contract Checkerboard {
    ...
    function playPiece(
        ...
        uint _targetX;
        uint _targetY;
        (_level, _targetX, _targetY,_owner,,,) = INumbreERC721( ERC721 ).getPieceInfo(_tokenId);
        require(msg.sender == _owner, "you are not owner");
        require(_targetX == 0 && _targetY == 0, "The piece has played");
    ...
    function autoUpdateLevel(uint256 _tokenId, uint256 _x, uint256 _y) public onlyAdmin {
        Piece memory piece = pieces[_x][_y];
        piece.level.add(1);
        INumbreERC721( ERC721 ).updatePieceInfo(_tokenId, piece.level, _x, _y, 3);
    }

    function autoOffPiece(uint256 _tokenId, uint256 _x, uint256 _y) public onlyAdmin {
...
interface INumbreERC721 {
    function getPieceInfo(uint256 tokenId) external view returns (uint256 level, uint256 x, uint256 y, address user, uint256 seasonId, uint256 lastOpt, uint256 lastUplevel);
