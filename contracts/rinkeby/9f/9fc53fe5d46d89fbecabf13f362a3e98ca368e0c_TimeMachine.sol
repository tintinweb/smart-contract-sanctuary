...
contract TimeMachine {
    ...
    function stake(uint8 itemIndex, uint256 amount) public activate {
        ...
        _userBooks[msg.sender].push(book(itemIndex, 1, block.timestamp, block.timestamp.add(it.month.mul(2592000)), block.timestamp, amount, 0, _userBooks[msg.sender].length));
    ...
    function calReward(uint256 time) public view returns (uint256){
        uint256 total = 0;
        uint256 amount = 0;
        for (uint i = 0; i < _stakeUsers.length; i++) {
            amount = _notInterestOne(_stakeUsers[i], time);
    ...
    function canWithdrawAmount(uint256 bookIndex, address account) public view returns (uint256){
        ...
        if (block.timestamp < b.endTime) {
            if (amount.add(_deduceReward[account]) <= _totalInterest[account].add(_getReward[account])) {
                return 0;
            } else {
                return amount.add(_deduceReward[account]).sub(_totalInterest[account].add(_getReward[account]));
            }
        }
        return amount;
    }
    ...
    function withdraw(uint256 bookIndex) public activate {
        require(_userBooks[msg.sender][bookIndex].status == 1, "Already back");
        ...
        _userBooks[msg.sender][bookIndex].status = 2;
    ...
    function allWithdrawAmount(address account) public view returns (uint256){
        ...
        for (uint256 i = 0; i < bs.length; i++) {
            book memory b = bs[i];
            if (b.status == 1 && block.timestamp < b.endTime) {
                total = total.add(b.amount);
            }
        }
        if (total.add(_deduceReward[account]) <= _totalInterest[account].add(_getReward[account])) {
            return 0;
        } else {
            return total.add(_deduceReward[account]).sub(_totalInterest[account].add(_getReward[account]));
        }
    ...
    function allWithdraw() public activate {
        ...
        if (amount > 0) {
            ...
            for (uint256 i = 0; i < bs.length; i++) {
                ...
                if (b.status == 1 && block.timestamp < b.endTime) {
                    total = total.add(b.amount);
                    _userBooks[msg.sender][i].status = 2;
                }
            }
    ...
    function doBack(uint256 end) public onlyAdmin {
        for (uint i = _outIndex; i < _outs.length; i++) {
            if (_outs[i].outTime < end && _outs[i].status) {
                ...
                for (uint j = 0; j <= _userBooks[_outs[i].user].length; j++) {
                    if (_userBooks[_outs[i].user][j].status == 2) {
                        _userBooks[_outs[i].user][j].status = 3;
                    }
                }
