...
contract Habitat is IHabitat, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
   ...
   */
    function _claimMouseFromHabitat(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        owed = getOwed(tokenId);
        require(!(unstake && owed < MINIMUM * 1 ether), "You can't unstake mice until they have 20k $CHEDDAR.");
...
//        require(!(unstake && block.timestamp - stake.value < MINIMUM_TO_EXIT), "Still claiming...");
   ...
   */
    ...
    function getOwed(uint256 tokenId) public view returns (uint256) {
        uint256 owed = 0;
        Stake memory stake = habitat[tokenId];
        if (totalCHEDDAREarned < MAXIMUM_GLOBAL_CHEDDAR) {
