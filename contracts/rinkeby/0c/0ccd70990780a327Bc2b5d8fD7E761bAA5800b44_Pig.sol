    ...
    Ownable
{
    ...
    event StoryOfPig(uint256 indexed tokenId, string name);
    ...
    modifier onlyWhitelisted() {
        require(
            ...
            "0xPig: You're not on the whitelist."
        );
        _;
    }
    ...
    /**
     * @dev Airdrop Pigs to several addresses.
    ...
    function presale(uint8 _num) external payable onlyWhitelisted {
        require(
            ...
            "0xPig: Presale has not yet started."
        );
        require(
            (_num + presaleNumOfPlayer[msg.sender]) <= 3,
            "0xPig: Up to 3 0xPigs can be purchased."
        );
        require(
            msg.value == uint256(_num) * 6e16,
            "0xPig: You need to pay the exact price."
        );
        _mintList(_num);
    ...
    /**
     * @dev Pledge for the purchase. Each address can only purchase up to 5 0xPigs.
     * @param _num Quantity to purchase
     */
    function bloodMark(uint8 _num) external payable {
        require(
            block.timestamp >= pledgeTime,
            "0xPig: Pledge has not yet started."
        );
        require(
            (_num + pledgeNumOfPlayer[msg.sender] + claimed[msg.sender]) <= 5,
            "0xPig: Each address can only purchase up to 5 0xPigs."
        );
        require(
            totalMint + uint256(_num) <= MAX_SUPPLY - totalPledge - 200,
            "0xPig: Sorry, all 0xPigs are sold out."
        );
        require(
            msg.value == uint256(_num) * 6e16,
            "0xPig: You need to pay the exact price."
    ...
    /**
     * @dev Your 0xPigs can only be claimed at the end of the sale.
     */
    ...
    function mint(uint8 _num) external payable {
        require(
            ...
            "0xPig: Mint time has not yet arrived!"
        );
        require(
            totalMint + uint256(_num) <= MAX_SUPPLY - totalPledge - 200,
            "0xPig: Sorry, all 0xPigs are sold out."
        ...
        require(
            _num <= 5,
            "0xPig: Up to 5 0xPigs can be minted in a tx."
        );
        require(
            msg.value == uint256(_num) * 6e16,
            "0xPig: You need to pay the exact price."
    ...
    /**
     * @dev Every time the tokenId reaches a multiple of 100, a random 0xPig gets a 10x mint price return.
     * For the 2500th, 5000th, 7500th and 9999th sales, 4 random Pigs will be picked and be each gifted with a Harley Davidson motorcycle as their ride.
     */
    ...
    function setName(uint256 _tokenId, string memory _name) external {
        require(
            ...
            "0xPig: You don't own this 0xPig!"
        );
    ...
    function setStory(uint256 _tokenId, string memory _desc) external {
        ...
        emit StoryOfPig(_tokenId, _desc);
    }
