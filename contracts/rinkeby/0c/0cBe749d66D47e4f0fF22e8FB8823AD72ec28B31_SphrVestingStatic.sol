...
contract SphrVestingStatic is Ownable {
    ...
    mapping(address => uint256) public _releaseAmount;
    ...
    function removeWhitelisted(address _address) onlyOwner external{
        require(whitelisted[_address],'user is not whitelisted!');
    ...
    function addVestingSchedule(
        ...
        if(!whitelisted[benificiary]){
            whitelisted[benificiary] = true;
        }
    ...
    function vestedAmount() public view virtual returns (uint256) {
        ...
        for (uint256 i = 0; i < vestingSchedules_.length; i++) {
            if (vestingSchedules_[i].schedule < block.timestamp) {
                vestedAmount_ = vestingSchedules_[i].amount.add(vestedAmount_);
            }
        }
        vestedAmount_ = vestedAmount_.sub(_releaseAmount[msg.sender]);
    ...
    function claim() public returns (bool) {
        ...
        _releaseAmount[msg.sender] = _releaseAmount[msg.sender].add(vestedAmount_);
        return true;
    }
    ...
    function withdrawUnclaimedTokens(address _address,uint256 _amount) onlyOwner external {
        IERC20(_address).safeTransfer(owner(),_amount);
    }
}