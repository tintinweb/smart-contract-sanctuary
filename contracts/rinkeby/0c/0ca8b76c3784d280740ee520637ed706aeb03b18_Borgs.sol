...
contract StringUtils{
    ...
    function _toString(string[] memory pixals) public pure returns(string memory value){
...
contract Borgs is CommonObjects, StringUtils, NumberUtils{
    ...
    mapping (uint256 => string[]) public _borgsImage;
    ...
    function generateToken() external returns(string[] memory generatedPixals)
    {
        ...
        for(uint256 i=0;i<_layerMetaData.length;i++){
            LayerMetaData memory layerMetaData = _layerMetaData[i];
            Layer storage layer = _layers[layerMetaData.name];
            require(layer.exists == false, "No layer was found");
        ...
        // Set in token
        //string memory strPixals = _toString(generatedPixals);
