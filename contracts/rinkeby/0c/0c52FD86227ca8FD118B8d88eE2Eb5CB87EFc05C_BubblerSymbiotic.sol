...
contract BubblerSymbiotic is ContextMixin, NativeMetaTransaction, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint8 public reserved = 75;
    uint8 public reservedMinted = 0;
    ...
    bytes32 public constant MINT_TYPEHASH = keccak256("SignedMint(address minter,uint8 num)");
    ...
    function setReserved(uint8 _reserved) external onlyOwner {
        require(_reserved - reservedMinted + totalSupply() <= MAX_SYMBIOTIC, 'Exceeds MAX_SYMBIOTIC');
    ...
    function mint(uint256 num) external payable nonReentrant {
        ...
        require(supply + num <= MAX_SYMBIOTIC - reserved, 'Exceeds MAX_SYMBIOTIC');
        require(treasury != address(0), "Treasury is not set");

        uint256 costToMint = getMintPrice() * num;
    ...
    function signedMint(uint8 num, uint8 v, bytes32 r, bytes32 s) external nonReentrant {
        uint256 supply = totalSupply();
        require(reservedMinted + num <= reserved, 'Exceeds reserved');
