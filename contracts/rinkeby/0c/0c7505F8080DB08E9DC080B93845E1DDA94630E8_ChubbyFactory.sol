...
contract ChubbyFactory is FactoryERC721, Ownable {
    ...
    constructor(
        ...
        basketNftAddress = address(
            new Basket(address(this))
        );
    ...
    function ownerOf(uint256 tokenId_) public view returns (address _owner) {
        return owner();
    }
}
 ...
 * @title Basket
 *
 * Basket - a tradeable basket of eggs.
 */
contract Basket is ERC721Tradable {
    uint256 public OPTION_ID = 0;
    uint256 private MAX_EGGS_PER_BASKET = 10;
    address public factoryAddress;

    constructor(
        address factoryAddress_
    ) ERC721Tradable("Chubby Basket", "LOOTBOX") {
        factoryAddress = factoryAddress_;
    }

    function unpack(uint256 tokenId_) public {
        require(tokenId_ <= MAX_EGGS_PER_BASKET);
        require(ownerOf(tokenId_) == _msgSender());

        // Insert custom logic for configuring the item here.
        for (uint256 i = 0; i < OPTION_ID; i++) {
            // Mint the ERC721 item(s).
            FactoryERC721 factory = FactoryERC721(factoryAddress);
            factory.mint(tokenId_, _msgSender());
        }
    ...
    function baseTokenURI() override public pure returns (string memory) {
        return "https://api.rovergulf.net/nft/factory/test-chubbies/";
    }
...
contract Chick is ERC721Tradable {
    constructor() ERC721Tradable("Chubby Pops", "CHB"){}
