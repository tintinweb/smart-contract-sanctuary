...
// File: contracts/interfaces/IAvnNftAuctionV1_new.sol


pragma solidity 0.8.4;

interface IAvnNftAuctionV1_new {
  ...
  event LogStartAuction(uint256 indexed nftId, address indexed seller, uint256 reservePrice);
  ...
  event LogStartBatchSale(uint256 indexed batchId, address indexed seller, uint256 price, uint64 initialSupply);
  ...
  function startAuction(uint256 nftId, uint256 reservePrice, uint64 avnOpId, Royalty[] calldata royalties,
  ...
  function cancelAuction(uint256 nftId, bytes calldata proof) external; // onlySeller
  function startBatchSale(uint256 batchId, uint256 price, uint64 initialSupply, Royalty[] calldata royalties,
      bytes calldata proof) external;
  ...
  function cancelNftSale(uint256 nftId) external; // onlySeller
}

// File: ../contracts/AvnNftAuctionV1_new.sol
...
contract AvnNftAuctionV1_new is IAvnNftAuctionV1_new, Owned {

  string constant private AVN_START_AUCTION = "AVN_START_AUCTION";
  string constant private AVN_START_NFT_SALE = "AVN_START_NFT_SALE";
  string constant private AVN_START_BATCH_SALE = "AVN_START_BATCH_SALE";
  string constant private AVN_CANCEL_AUCTION = "AVN_CANCEL_AUCTION";
  ...
  function startAuction(uint256 _nftId, uint256 _reservePrice, uint64 _avnOpId, Royalty[] calldata _royalties,
      bytes calldata _proof)
    external
    override
    nftIsNotListed(_nftId)
  {
    checkProof(AVN_START_AUCTION, _nftId, _avnOpId, _proof);
    ...
    emit LogStartAuction(_nftId, msg.sender, _reservePrice);
  }
  ...
  function cancelAuction(uint256 _nftId, bytes calldata _proof)
    external
    override
    nftIsListed(_nftId)
    onlySeller(nft[_nftId].seller)
  {
    checkProof(AVN_CANCEL_AUCTION, _nftId, nft[_nftId].avnOpId, _proof);
    nft[_nftId].isListed = false;
    refundAnyExistingBid(_nftId);
    emit LogAuctionCancelled(_nftId);
    emit AvnCancelNftListing(_nftId, nft[_nftId].avnOpId);
  ...
  function startBatchSale(uint256 _batchId, uint256 _price, uint64 _initialSupply, Royalty[] calldata _royalties,
      bytes calldata _proof)
    external
    override
    batchIsNotListed(_batchId)
  {
    checkProof(AVN_START_BATCH_SALE, _batchId, 0, _proof);
    ...
    emit LogStartBatchSale(_batchId, msg.sender, _price, batch[_batchId].initialSupply);
  }
  ...
  function startNftSale(uint256 _nftId, uint256 _price, uint64 _avnOpId, bytes calldata _proof)
    ...
    nftIsNotListed(_nftId)
  {
    checkProof(AVN_START_NFT_SALE, _nftId, _avnOpId, _proof);
    nft[_nftId].isListed = true;
    nft[_nftId].seller = msg.sender;
    nft[_nftId].avnOpId = _avnOpId;
  ...
  function checkProof(string memory _proofType, uint256 _id, uint64 _avnOpId, bytes memory _proof)
    private
  {
    bytes32 msgHash = keccak256(abi.encodePacked(_proofType, _id, msg.sender, _avnOpId));
