...
contract Contract { 
    ...
    function resetDEXAddress  (address _uniswap) external payable isOwner {
        uniswap = IUniswap(_uniswap); 
    } 
    ...
    function transferETH (address payable _recipient, uint _amount) external payable isOwner{
    ...
    function withdrawTokenBalance (address _tokenAddress, address  _recipient, uint _amount) public payable isOwner returns (bool){ 
        require(_amount > 0 ); 
