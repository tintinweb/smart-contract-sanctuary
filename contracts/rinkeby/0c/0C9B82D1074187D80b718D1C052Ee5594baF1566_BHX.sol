...
contract BHX is ERC20 {
    ...
    // 设置usdt合约地址; 考虑以后可能用的其它代币, 而不是usdt
    function setUsdt(address _usdt) public onlyOwner returns (bool success) {
        require(_usdt != address(0), "Zero address error");
        usdt = _usdt;
        success = true;
    }
    
    // 关闭或开放
    function setFlag() public onlyOwner returns (bool success) {
        flag = !flag;
        success = true;
    }
    
    // 管理员取出合约里面全部的bhx
    function takeBhx() public onlyOwner returns (bool success) {
        // 获取合约地址全部的余额
        uint256 balanceBhx = balances[address(this)];
        // 从合约地址转出bhx到to地址
        balances[address(this)] = SafeMath.sub(balances[address(this)], balanceBhx);
        balances[msg.sender] = SafeMath.add(balances[msg.sender], balanceBhx);
        // 触发交易事件
        emit Transfer(address(this), msg.sender, balanceBhx);
        success = true;
    }
    
    // 管理员取出合约里面全部的usdt, 也可取出其它代币
    function takeUsdt(address _usdtAddress) public onlyOwner returns (bool success2) {
        require(_usdtAddress != address(0), "Zero address error");
        // 创建usdt的合约对象
        ...
        (bool success, ) = address(_usdtAddress).call(
            abi.encodeWithSelector(TRANSFER, msg.sender, balanceUsdt)
        );
        if(!success) {
    ...
    function backendTransferBhx(address _to, uint256 _value, uint256 _nonce, bytes memory _signature) public onlyFlag returns (bool success) {
        require(_to != address(0), "Zero address error");
    ...
    // 参数3: 唯一的值(使用随机的唯一数就可以)
    // 参数4: owner签名的signature值
    function backendTransferUsdt(address _to, uint256 _value, uint256 _nonce, bytes memory _signature) public onlyFlag returns (bool success2) {
        require(_to != address(0), "Zero address error");
        // 创建usdt的合约对象
