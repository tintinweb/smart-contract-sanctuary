...
contract SamplePunks is  Ownable, ERC721Enumerable {

    uint public constant MAX_SPUNKS = 2077;
    bool public hasSaleStarted = false;
    mapping (address => uint) private _SamplePunksV1Owners;
    string private _baseTokenURI;

    constructor(string memory baseTokenURI ) ERC721("SamplePunksx","SPUNKSX")  {
    ...
    function isAuthClaim(address _to) public view returns(uint) {
      return _SamplePunksV1Owners[_to];
    }

    function authClaim(address _to, uint  _NumberOfPunks) public onlyOwner{
         _SamplePunksV1Owners[_to] = _NumberOfPunks;
    }
   ...
   function getSamplePunk(uint256 numSamplePunks) public payable {
        require(totalSupply() < MAX_SPUNKS, "Sale has already ended");
        require(numSamplePunks > 0 && numSamplePunks <= 20, "You can mint minimum 1, maximum 20 SamplePunks");
        require(totalSupply()+numSamplePunks <= MAX_SPUNKS, "Exceeds MAX_SPUNKS");
        require(msg.value >= calculatePrice() * numSamplePunks, "Ether value sent is below the price");

        for (uint i = 0; i < numSamplePunks; i++) {
    ...
    function reserveGiveaway(uint256 numSamplePunks) public onlyOwner {
        uint currentSupply = totalSupply();
        require(totalSupply() + numSamplePunks <= 130, "Exceeded giveaway supply");
        require(hasSaleStarted == false, "Sale has already started");
        uint256 index;
        // Reserved for SamplePunks V1 holders airdrop and people who helped this project and giveaways
        for (index = 0; index < numSamplePunks; index++) {
   ...
   function claimSamplePunk() public {
     require(totalSupply() < MAX_SPUNKS, "Sale end");
     require(hasSaleStarted == true, "Sale has not already started");
     require(_SamplePunksV1Owners[msg.sender] > 0, "Not owner");
     address _to = msg.sender;
     uint _NumberToClaim = _SamplePunksV1Owners[_to];
     for(uint i = 0; i < _NumberToClaim; i++){
       _safeMint(_to, totalSupply());
     }
     _SamplePunksV1Owners[_to]=0;
   }
