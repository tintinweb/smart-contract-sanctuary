...
contract CompoundVault is Initializable, PausableUpgradeable, AccessControlUpgradeable{
    ...
    /// @param _protocolFees uint the protocol fees
      ...
      function  initialize(address _admin, uint256 _protocolFees) public initializer {
        __Pausable_init();
    ...
    function deposit(address _asset, uint256 _amount) external payable {
        ...
        uint256 userAmount = balanceA.sub(balanceB);                                                         // refactor                                                      
        underlyingBalances[tx.origin][_asset] = underlyingBalances[tx.origin][_asset].add(_amount);          // refactor
        collateralBalances[tx.origin][_cToken] = collateralBalances[tx.origin][_cToken].add(userAmount);     // refactor
    }
    ...
    function withdraw(address _asset, uint256 _mount) external {
        uint256 balanceB;                                                                                    
        uint256 balanceA;  
        address _cToken = collaterals[_asset];                                                                                                                                                         
    ...
    // need to be refactored with all vaults common values
    ...
    function _Computation(address _asset) internal view returns (uint256[] memory ){
        ...
        txData[1] =  rate.mul(withdrawn).div(1e18);                        // the underlying asset with interest minus fees (the amount withdrawn in underlying after all)                                      
    ...
    /// @param _amount the asset to borrow
         ...
         function repayBorrow(address _asset, uint256 _amount) public {
