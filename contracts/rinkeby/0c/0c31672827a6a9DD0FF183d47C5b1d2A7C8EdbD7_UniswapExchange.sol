...
contract UniswapExchange is Ownable, ReentrancyGuard {
    ...
    function placeOrder(
        ...
        require(msg.sender == userAddr, "Invalid user address!");
        ...
        if (orderType != OrderType.EthForTokens) {
            if (orderType == OrderType.TokensForEth) {
                ...
                if (deflationary) {
                    uint256 beforeBalance = IERC20(tokenOut)
                    ...
                    uint256 afterBalance = IERC20(tokenOut)
                        .balanceOf(address(this));
                    amounts = new uint256[](2);
                    amounts[0] = amountInOffered;
                    amounts[1] = afterBalance.sub(beforeBalance);
            ...
            uint256 beforeBalance = IERC20(tokenOut).balanceOf(
                address(this)
            );
            ...
            uint256 afterBalance = IERC20(tokenOut).balanceOf(
                address(this)
            );
            amounts = new uint256[](2);
            amounts[0] = amountInOffered;
