...
contract InsuracePositionController is AbstractController {
    ...
    function stake(uint256 _amount) external override onlyIndex returns (uint256) {
        ...
        if (address(base) != _stakingAsset) {
            ...
            _buyInsuraceAsset(_amount, address(base), _stakingAsset);
        }

        uint256 investmentAmount = IERC20(_stakingAsset).balanceOf(address(this));
        ...
        uint256 lptokenBalance = IERC20(_lpToken).balanceOf(address(this));
    ...
    */
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        uint256 _unstakedAmount = IERC20(_stakingAsset).balanceOf(address(this));
        ...
        _checkApprovals(IERC20(_stakingAsset), address(_uniswapRouter), _unstakedAmount);
        uint256 baseTokenAmount = _buyInsuraceAsset(_unstakedAmount,_stakingAsset, address(base));

        // Return to index
        base.safeTransfer(address(index), baseTokenAmount.sub(withdrawalFee));

        setUnstakingState();

        // Subtract fee?
        return _unstakedAmount.sub(withdrawalFee);
    }

    function withdrawRewards() external override {

        // TODO: get rewards_Controller address
        // RewardController(address(unknown)).withdrawReward(uint256 _amount) external whenNotPaused nonReentrant {
        // Method RewardController(address(unknown)).unlockReward( contains 
        // IStakingV2Controller(stakingController).unlockRewardsFromPoolsByController(_msgSender(), address(this), _tokenList);
        // which is allowedCaller only...

        // uint256 _rewards =  RewardController(address(unknown)).withdrawReward(uint256 _amount) 
        uint256 _rewards = 0;

        uint256 rewardsBalance = IERC20(_insurToken).balanceOf(address(this));
        require( rewardsBalance >= _rewards, "InsuracePositionController IPC8");
        ...
        _checkApprovals(IERC20(_insurToken), address(_uniswapRouter), rewardsBalance);
        uint256 baseTokenAmount = _buyInsuraceAsset(rewardsBalance,address(_insurToken), address(base));


        require(baseTokenAmount != 0, "InsuracePositionController: IPC11");
