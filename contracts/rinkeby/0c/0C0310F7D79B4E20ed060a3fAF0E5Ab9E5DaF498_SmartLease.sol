...
contract SmartLease {
    ...
    int256[] cumul_miles_usage_tracker;

    function get_cummulative_monthly_miles_usage()
        public
        returns (int256[] memory)
    {
        int256 cumulative_miles_usage = 0;
        for (uint256 i = 1; i <= monthly_miles_usage_tracker.length; i++) {
            cumulative_miles_usage =
                cumulative_miles_usage +
                monthly_miles_usage_tracker[i];
            cumul_miles_usage_tracker.push(cumulative_miles_usage);
        }
        return cumul_miles_usage_tracker;
    }
}