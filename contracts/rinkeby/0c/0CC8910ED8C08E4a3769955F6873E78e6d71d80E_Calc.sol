...
contract Calc is FractionalExponents {
  ...
  uint256 constant multiplyer = 10000;
    ...
    // psuedomath: rewardRate = (1.35 * (amount * 1000)^(1/4)) / 86400
		...
		(uint256 rewardRateMantissa, uint8 rewardRateExponent) = power(multiplyer.mul(amount), uint256(1e21), expN, expD); // get the reward rate expressed in fixed point (amount*multiplyer)^exponent
    ...
    rewardRate = rewardRateNoCoefficient.div(86400); // multiply by (1.35/86400)
