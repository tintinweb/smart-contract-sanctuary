...
contract LinearCurveWithNegativeSlope is ILevel0AuctionCurve, Pacemaker, Ownable {
    ...
    function y(Level0Model[] calldata level0s) external view override returns (uint256 value) {
        value = (_y1(level0s).mul(EPOCH_PERIOD.sub(_x())).add(EPOCH_PERIOD.mul(_x()))).div(EPOCH_PERIOD);
        if (value > maxY) {
            value = maxY;
        }
        if (value < minY) {
    ...
    function epochFromTimestamp(uint256 timestamp) public pure override(ILevel0AuctionCurve, Pacemaker)
    returns (uint256) {
        return super.epochFromTimestamp(timestamp);
    }

    function epochStartTimeFromTimestamp(uint256 timestamp) public pure override(ILevel0AuctionCurve, Pacemaker)
    returns (uint256) {
        return super.epochStartTimeFromTimestamp(timestamp);
    }

    function epochEndTimeFromTimestamp(uint256 timestamp) public pure override(ILevel0AuctionCurve, Pacemaker)
    returns (uint256) {
        return super.epochEndTimeFromTimestamp(timestamp);
    }

    function currentEpoch() public view override(ILevel0AuctionCurve, Pacemaker) returns (uint256) {
    ...
    function _y1(Level0Model[] calldata level0s) private view returns (uint256) {
        if (level0s.length == 0) {
            return maxY;
        } else if (currentEpoch().sub(level0s[level0s.length - 1].epoch) == 1) {
            return level0s[level0s.length - 1].amount.mul(2);
        } else {
            return level0s[level0s.length - 1].amount;
        }
    }
