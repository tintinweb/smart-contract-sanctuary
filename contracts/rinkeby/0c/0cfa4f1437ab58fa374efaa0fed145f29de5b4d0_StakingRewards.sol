...
contract StakingRewards is OlympusAccessControlled, ReentrancyGuard {
    ...
    function getReward() public nonReentrant {
        ...
        require(block.timestamp > userInfos[msg.sender].lastUpdateTime + 12 hours, "can't get rewards");
    ...
    function setAPRvalue() private {
        userInfos[msg.sender].apr = ((userInfos[msg.sender].tazNum.mul(10000).mul(5)).add(10 ** 8)).div(10 ** 9);
    }

    function exitTazor() external {
        withdrawTazor(userInfos[msg.sender].tazorNum);
        getReward();
    }
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            ...
            userInfos[account].reward = earned(account);
            userInfos[account].lastUpdateTime = block.timestamp;
        }
        _;
    }
