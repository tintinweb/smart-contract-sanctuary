...
contract dogeIncubator is Ownable, IERC1155Receiver, VRFConsumerBase {
    ...
    */
    ...
    event DogeAmountCheck(bool isPassed);
    event PickedRandomDogeIndex(uint256 pickedDogeIndex);
    event PickedRandomDoge(bytes2 pickedDoge);
    ...
    function finishHatchingEgg(bytes32 requestId, uint256 randomNumber) private {
        ...
        emit PickedRandomDogeIndex(randomDoge);
        
        bytes2 pickedDoge = availableDoges[randomDoge];
        
        emit PickedRandomDoge(pickedDoge);
