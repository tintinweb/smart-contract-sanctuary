...
contract RainiNFT1155v2 is ERC1155, AccessControl, ReentrancyGuard {
  ...
  function merge(uint256 _cardId, uint256 _level, uint256 _mintAmount, uint256[] memory _tokenIds, uint256[] memory _burnAmounts, uint256[] memory _data) 
    external payable nonReentrant {
      ...
      _locals.willCallPool = nftStakingPoolAddress != address(0) && _level > 0 && cardLevels[_cardId][_level-1].tokenId == 0;
      _locals.willCallSubContract = _cardLevel.tokenId != 0 && cards[_cardId].subContract != address(0);
      if (_locals.willCallPool || _locals.willCallSubContract) {
        ...
        if (_locals.willCallSubContract) {
          subContract = IRainiCustomNFT(cards[_cardId].subContract);
        }
      }

      for (uint256 i = 0; i < _tokenIds.length; i++) {
        require(_burnAmounts[i] <= balanceOf(_msgSender(), _tokenIds[i]));
        ...
        if (_locals.willCallPool || _locals.willCallSubContract) {
          ...
          if (_locals.totalPointsBurned > _locals.currentTokenToMint * _cardLevel.conversionRate || i == _tokenIds.length - 1) {
            ...
            if (_locals.willCallPool) {
              nftStakingPool.mergeTokens(_locals.currentTokenToMint + maxTokenId, mergedTokensIds, address(this));
            }
            if (_locals.willCallSubContract) {
              subContract.onMerged(_locals.currentTokenToMint + maxTokenId, mergedTokensIds, address(this), _data);
            }
      ...
      _mintToken(_msgSender(), _cardId, _level, _mintAmount, contractChar, 0, _data);
  }
  ...
  function _mintToken(address _to, uint256 _cardId, uint256 _cardLevel, uint256 _amount, bytes1 _mintedContractChar, uint256 _number, uint256[] memory _data) private {
    ...
    require(_cardLevel > 0 || cardLevel.numberMinted + _amount <= card.maxSupply);
  ...
  function mintWithPoints(uint256[] memory _cardId, uint256[] memory _amount, bool[] memory _useUnicorns, uint256[][] memory _data, address[] memory _rainbowPools, address[] memory _unicornPools)
    ...
    for (uint256 i = 0; i < _cardId.length; i++) {
      ...
      _mintToken(_msgSender(), _cardId[i], 0, _amount[i], contractChar, 0, _data[i]);
    }
  }
