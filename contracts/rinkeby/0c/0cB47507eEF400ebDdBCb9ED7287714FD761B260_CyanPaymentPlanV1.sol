...
contract CyanPaymentPlanV1 is AccessControl, ReentrancyGuard {
	...
	constructor(address cyanSigner) {
	...
	function createBNPLPaymentPlan(
		...
		require(
			...
			"Given timestamp must past older"
		);
		require(timestamp + 10 minutes >= block.timestamp, "Timestamp expired");
		require(
	...
	function createPAWNPaymentPlan(
		...
		_cyanVaultV1.lendPAWN(msg.sender, amount);
		...
		)
	{
		...
		uint256 payAmountForCollateral = amount.div(numOfPayment);

		// Calculating interest fee, Note that interest rate is x100
		uint256 interestFee = (amount.mul(interestRate)).div(10000);
		// Payment amount for interest fee payment
		uint256 payAmountForInterest = interestFee.div(numOfPayment);

		// Calculating 2.5% service fee
		uint256 serviceFee = amount.div(40);
		// Payment amount for service fee payment
		uint256 payAmountForService = serviceFee.div(numOfPayment);
		...
		)
	{
		...
		if (plan.counterPaidPayments == plan.totalNumberOfPayments) {
			...
			currentPayment =
				plan.amount -
				payAmountForCollateral.mul(plan.totalNumberOfPayments - 1) +
				interestFee -
				payAmountForInterest.mul(plan.totalNumberOfPayments - 1) +
				serviceFee -
				payAmountForService.mul(plan.totalNumberOfPayments - 1);
		}
		...
		return (
			...
			payAmountForService,
			currentPayment,
			plan.createdDate + plan.counterPaidPayments * DURATION
		);
	}
	 ...
	 * @param wNFTContract Wrapped NFT contract address
	 * @param wNFTTokenId Wrapped NFT token ID
	 * @return Is pending payment plan
	 * @return Is defaulted payment plan
	 */
		...
		returns (bool, bool)
	{
		...
		CyanWrappedNFTV1 _cyanWrappedNFTV1 = CyanWrappedNFTV1(wNFTContract);
		bool isPending = _cyanWrappedNFTV1.exists(wNFTTokenId) == false;

		(, , , , uint256 dueDate) = getNextPayment(wNFTContract, wNFTTokenId);
		bool isDefaulted = block.timestamp > dueDate;

		return (isPending, isDefaulted);
	}
...
contract CyanVaultV1 is AccessControl, ReentrancyGuard, ERC721Holder {
	using SafeMath for uint256;
	...
	// TODO(Naba): Update remaining amount calculation
	...
	function lendBNPL(uint256 amount) public nonReentrant onlyRole(CYAN_ROLE) {
		// TODO(Naba): Need to call from Payment plan ???

		uint256 maxWithdrableAmount = getMaxWithdrawableAmount();
		require(amount <= maxWithdrableAmount, "Not enough money in the Vault");

		LOANED_AMOUNT += amount;
		REMAINING_AMOUNT -= amount;
		payable(msg.sender).transfer(amount);

		emit LendBNPL(msg.sender, amount);
	}
		...
		returns (uint256)
	{
		...
		if (ethAmountForToken <= maxWithdrableAmount) {
			return tokenBalance;
		}
		return calculateTokenByETH(maxWithdrableAmount);
	}

	// Explain how did we find X here :D
	function getMaxWithdrawableAmount() public view returns (uint256) {
		return
			REMAINING_AMOUNT.sub(
				LOANED_AMOUNT
	...
	function updateSafetyFundPercent(uint256 safetyFundPercent)
		...
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
		require(safetyFundPercent >= 0, "Safety fund percent negative");
		require(
