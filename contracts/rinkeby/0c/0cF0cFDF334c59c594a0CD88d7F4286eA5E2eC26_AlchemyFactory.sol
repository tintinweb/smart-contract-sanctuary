...
contract Alchemy is IERC20 {
    ...
    //e representing the name of the governance token
    string public _name;
    ...
    // the buyout price. once its met, all nfts will be transferred to the buyer, all tokens will be burned
    ...
    constructor (IERC721 nftAddress_, address owner_, uint256 tokenId_, uint256 totalSupply_, string memory name_, string memory symbol_, uint256 buyoutPrice_, address factoryContract) {
        _owner = owner_;
    ...
    */
    ...
    function burnForETH() external {
        ...
        _burn(balance);
        msg.sender.transfer(address(this).balance.mul(balance).div(_totalSupply));
        emit Transfer(msg.sender, address(0), balance);
    }

    function Buyshares(uint256 amount) external payable {
        require(_sharesForSale >= amount, "ALC:low shares");
        require(msg.value >= amount.mul(_buyoutPrice).div(_totalSupply), "ALC:low value");
    ...
    function buyout() external payable {
        require(msg.value >= _buyoutPrice, "ALC:buy value");
        ...
        factoryowner.transfer(_buyoutPrice / 50);
    ...
    function setNftSale(uint nftarrayid, uint price, bool sale) external {
        require(msg.sender == _timelock, "ALC:Only TL");
    ...
    function buySingleNft(uint nftarrayid) external payable {
        ...
        _raisedNftArray[nftarrayid].nftaddress.approve(msg.sender, _raisedNftArray[nftarrayid].tokenid);
        _raisedNftArray[nftarrayid].nftaddress.transferFrom(address(this), msg.sender, _raisedNftArray[nftarrayid].tokenid);

        // Take 2% fee
        address payable factoryowner = IAlchemyFactory(_factoryContract).getFactoryOwner();
        factoryowner.transfer(_raisedNftArray[nftarrayid].price / 50);
    ...
    function addNft(IERC721 new_nft, uint256 tokenid) external{
        require(msg.sender == _timelock, "ALC:Only TL");

        _nftCount++;
        _raisedNftArray[_nftCount].nftaddress = new_nft;
    ...
    function returnNft() external {
        ...
        _raisedNftArray[1].nftaddress.approve(_owner, _raisedNftArray[1].tokenid);
    ...
    function executeTransaction(address target, uint value, string memory signature, bytes memory data) external payable returns (bytes memory) {
        require(msg.sender == _timelock, "ALC:Only TL");
