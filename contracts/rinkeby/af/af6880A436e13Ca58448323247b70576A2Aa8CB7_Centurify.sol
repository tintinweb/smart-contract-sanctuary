...
contract Centurify is ERC20, Ownable, ERC20Snapshot {
    ...
    uint256 public contractToken = balanceOf(address(this));
    ...
    address payable public _operationalWalletAddress;
    ...
    uint256 public _numOfTokensToExchangeForMarketing = 5 * 10**6 * 10**9;
    ...
    constructor (address payable marketingWalletAddress, address payable operationalWalletAddress) ERC20(_name, _symbol){
        _marketingWalletAddress = marketingWalletAddress;
        _operationalWalletAddress = operationalWalletAddress;
    ...
    function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
        ...
        uint256 amountToSwap = amount.mul(3).div(100);
    ...
    function sendETHToMarketing(uint256 amount) private {
        uint256 marketing = amount.div(3);
        uint256 operational = amount.mul(2).div(3);
        _marketingWalletAddress.transfer(marketing);
        _operationalWalletAddress.transfer(operational);
    }
    ...
    function _setMarketingFee(uint256 marketingFee) external onlyOwner() {
        require(marketingFee >= 1 && marketingFee <= 2, 'marketingFee should be in 1 - 2');
