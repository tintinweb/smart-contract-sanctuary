...
contract OriginalsPhysicalEditionBurn is ERC1155, ERC1155Burnable, ERC1155Supply, Pausable, ReentrancyGuard, AdminControl, Ownable {
  using Strings for uint256;

  // ========== Mutable Variables ==========

  string public baseURI;
  mapping(uint256 => uint256) public numClaimsByTokenID;
  mapping(uint256 => uint256) public maximumSupplyByTokenID;

  address[] addressesClaimed;
  mapping(address => uint256) public numClaimsPerAddress;
  ...
  struct BurnEdition {
    ...
    uint256 tokenCost; // Number of tokens to burn to claim an edition
    uint256[] supportedTokens;
    ContractType contractType;
  }
  mapping(uint256 => BurnEdition) public burnEditions;
  uint256 public numBurnEditions;

  mapping(uint256 => mapping(uint256 => bool)) public tokenSupportedByEditionId;

  // ========== Events ==========
  event ClaimedPhysicalEdition(address indexed burner, uint256 editionId, uint256 tokenId);
  ...
  function burnForPhysical(uint256[] calldata _burnTokenIds, uint256 editionId, uint256 editionTokenId, bool claimNFT) public whenNotPaused nonReentrant {
    BurnEdition memory edition = burnEditions[editionId];

    require(edition.contractAddress != address(0), "Edition not found");
    require(tokenSupportedByEditionId[editionId][editionTokenId] == true, "Token Id is not supported for edition");

    // Calculate quantity
    require(_burnTokenIds.length % edition.tokenCost == 0, "Quantity must be a multiple of token cost");
    uint256 _quantity = _burnTokenIds.length / edition.tokenCost;

    // Can only claim if there is supply remaining
    require(numClaimsByTokenID[editionTokenId] + _quantity <= maximumSupplyByTokenID[editionTokenId], "Not enough tokens remaining");
  ...
  function getMaximumSupply(uint256 tokenId) public view returns (uint256) {
  ...
  function getEdition(uint256 editionId) public view returns (uint256, uint256[] memory, address, ContractType) {
    BurnEdition memory edition = burnEditions[editionId];
    return (edition.tokenCost, edition.supportedTokens, edition.contractAddress, edition.contractType);
  }

  // ========== Admin ==========

  function addEdition(address _contractAddress, uint256 _tokenCost, uint256[] calldata _supportedTokens, ContractType contractType) public onlyAdmin {
    uint256 newEditionId = numBurnEditions + 1; 
    ...
    for(uint256 i=0; i < _supportedTokens.length; i++) {
      tokenSupportedByEditionId[newEditionId][_supportedTokens[i]] = true;
    }

    numBurnEditions++;
  }

  function updateEdition(uint256 editionId, address _contractAddress, uint256 _tokenCost, uint256[] calldata _supportedTokens, ContractType contractType) public onlyAdmin {
    ...
    // Reset and update Supported Tokens
    for(uint256 i=0; i < burnEditions[editionId].supportedTokens.length; i++) {
      tokenSupportedByEditionId[editionId][_supportedTokens[i]] = false;
    }

    burnEditions[editionId].supportedTokens = _supportedTokens;

    for(uint256 i=0; i < _supportedTokens.length; i++) {
      tokenSupportedByEditionId[editionId][_supportedTokens[i]] = true;
    }
  }

  function setMaximumSupply(uint256 tokenId, uint16 _maximumSupply) public onlyAdmin {
