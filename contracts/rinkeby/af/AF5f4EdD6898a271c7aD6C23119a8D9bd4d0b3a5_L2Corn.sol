...
contract L2Corn is ERC777, IERC777Recipient, ReentrancyGuard, IERC721Receiver, ERC165, ERC721Holder, IArbToken {
    ...
    */
    ...
    function equipCollectible(uint256 tokenID)
        ...
        senderFarm.numberOfCollectibles += 1;
    ...
    function releaseCollectible(uint256 tokenID)
        ...
        {
        bool _success = false;
        Farm storage senderFarm = farms[_msgSender()];

        // Search all farms collectibles
        for (uint i=0; i<senderFarm.numberOfCollectibles; i++) {
            // Check for collectible matching tokenID & delete
            if ( ownerOfCollectibles[_msgSender()][i].id > tokenID ) {
                delete ownerOfCollectibles[_msgSender()][i];
                senderFarm.numberOfCollectibles = senderFarm.numberOfCollectibles.sub(1);
                _success = true;
            }
        }
        ...
        returns (uint256 collectibleMultiplier)
    {
        ...
        if (targetFarm.numberOfCollectibles == 0) {
            return _percentMultiplier;
        }
            ...
            )
    {
        ...
        bool noExpiry;
        for (uint i=0; i<targetFarm.numberOfCollectibles; i++) {
