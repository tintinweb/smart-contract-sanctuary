...
abstract contract Admin is OwnableV2 {
    struct tokenInfo {
        bool isExist;
        uint8 decimal;
...
abstract contract UnifarmV2 is Admin {
...
contract UnifarmV2Fixed is Ownable {
    ...
    function getOneDayReward(
        ...
        if (lockableDetailsB[stakedToken].optionableStatus) {
            ...
            reward = (
                stakedAmount.mul(
                    uniV2.tokenDailyDistribution(stakedToken, rewardToken)
                )
            )
    ...
    function unStake(address user, uint256 stakeId) external whenNotPaused {
        ...
        if (lockableDetailsB[tokenAddress[stakeId]].optionableStatus == true) {
                ...
                require(
                    startTime[stakeId].add(uniV2.stakeDuration()) <=
                        block.timestamp,
        ...
        // State updation
        uint totalStaking = uniV2.totalStaking(tokenAddress[stakeId]).sub(totalUnStakingB[tokenAddress[stakeId]].add(totalUnStakingA[tokenAddress[stakeId]]));
        
        totalUnStakingB[tokenAddress[stakeId]] = totalUnStakingB[tokenAddress[stakeId]].add(
    ...
    function _rewardCalculation(
        ...
        while (i < intervalDays.length) {
            if (noOfDays >= intervalDays[i]) {
                uint256 balDays =
                    noOfDays.sub((intervalDays[i].sub(1)));
                ...
                if (
                    ...
                    if (referrerAddress != address(0)) {
                        uint256 refEarned =
                            (rewardsEarned.mul(uniV2.refPercentage())).div(
                                100 ether
                            );
                        rewardsEarned = rewardsEarned.sub(refEarned);
