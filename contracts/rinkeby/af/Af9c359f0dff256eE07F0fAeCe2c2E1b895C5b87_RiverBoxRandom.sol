...
contract RiverBoxRandom is IRiverBoxRandom, Ownable {
    ...
    function generateSignature(uint256 salt) public view override returns (uint256) {
        require(_msgSender() == caller, "only riverBox contract can call this function");
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, salt)));
    }

    function setCaller(address riverBox) public override onlyOwner {
        caller = riverBox;
    }
}
