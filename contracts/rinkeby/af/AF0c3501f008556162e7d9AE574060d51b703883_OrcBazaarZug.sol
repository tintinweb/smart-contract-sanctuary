...
///Do not attempt to use this directly. You need to use the Dapp
...
contract OrcBazaarZug{
    ...
    uint offeringNonce;
    address _zug;
    address husky = 0xCcB6D1e4ACec2373077Cb4A6151b1506F873a1a5;
    ...
    mapping (address => uint) zugbalances;

    constructor (address _operator) {
        operator = _operator;
    }

    function addZugAddress(address _zugAddress) external {
    ...
    function placeOffering(address _offerer, address _hostContract, uint _tokenId, uint _price) external {
        ...
        offeringNonce += 1;
        emit  OfferingPlaced(offeringId, _hostContract, _offerer, _tokenId, _price);
    }
    
    function closeOfferingWithZug(bytes32 _offeringId, address _buyer) external {
        require(zug.balanceOf(_buyer) >= offeringRegistry[_offeringId].price, "Not enough ZUG balance");
        ...
        hostContract.safeTransferFrom(offeringRegistry[_offeringId].offerer, _buyer, offeringRegistry[_offeringId].tokenId);
        zug.transferFrom(_buyer,address(this),offeringRegistry[_offeringId].price); 
        offeringRegistry[_offeringId].closed = true;
        uint sellerPayable = commission(offeringRegistry[_offeringId].price);    
        uint huskyPayable = offeringRegistry[_offeringId].price.sub(sellerPayable);
        zugbalances[husky] += huskyPayable;
        zugbalances[offeringRegistry[_offeringId].offerer] += sellerPayable;
    ...
    function closeOffering(bytes32 _offeringId) external payable {
        ...
        offeringRegistry[_offeringId].closed = true;
        uint sellerPayable = commission(offeringRegistry[_offeringId].price);    
        uint huskyPayable = msg.value.sub(sellerPayable);
        ethbalances[husky] += huskyPayable;
        ethbalances[offeringRegistry[_offeringId].offerer] += sellerPayable;
        emit OfferingClosed(_offeringId, msg.sender);
    }

       function testTransfer(uint _value) external {
       zug.transferFrom(husky,address(this), _value); 
    }

    ///2.5% of tx amount is deducted as commission for husky

    

    function commission(uint _amount) internal pure returns(uint) {
    ...
    function withdrawEthBalance() external {
        ...
        function withdrawZugBalance() external {
        require(ethbalances[msg.sender] > 0,"You don't have any balance to withdraw");
        uint amount = zugbalances[msg.sender];
        zug.transferFrom(address(this),msg.sender,amount); 
        zugbalances[msg.sender] = 0;
        emit BalanceWithdrawn(msg.sender, amount);
    }
    ...
    function viewBalances(address _address) external view returns (uint, uint) {
        return (ethbalances[_address],zugbalances[_address]);
    }
