...
contract TesterAASB is ERC721, Ownable {
    ...
    uint256 public constant maxMintsPerTransaction = 20;
    ...
    mapping(uint256 => bool) public mintPassIdHasMinted;
    ...
    string public hiddenTokenURI =
        "https://graffitimansionlabs.mypinata.cloud/ipfs/Qmf8xquZS1vqLNh43eU5UGKX2KupSKquopsRTn97TBW1PL";
    string public baseExtension = ".json";

    event MilestoneReachedStateChange(uint256 val, uint256 time);
    ...
    function addToWhitelist(address[] memory addresses, uint256 amount)
        public
        onlyOwner
    {
    ...
    function mintSale(uint256 amount) external payable onlyEOA {
        ...
        require(
            totalSupply() + amount <=
                maxSupply - (reservedAmount - reservedMintedAlready),
            "max supply reached"
        );
    ...
    function mintPresale(uint256 amount) external onlyEOA {
        ...
        require(
            ...
            "not enough whitelist balance"
        );
        whitelist[msg.sender] -= amount;
        _mintWorldstar(msg.sender, amount);
    }

    function mintWithMintPass(uint256[] memory mintPassIds)
        external
        payable
        onlyEOA
    {
        uint256 amount = mintPassIds.length;
        require(presaleOpen, "PreSale not started yet");
        require(
            totalSupply() + amount <=
                maxSupply - (reservedAmount - reservedMintedAlready),
        ...
        for (uint256 i = 0; i < amount; i++) {
            require(
                mintPass.ownerOf(mintPassIds[i]) == msg.sender,
                "mintpass Id not owned"
            );
            require(
                !mintPassIdHasMinted[mintPassIds[i]],
                "mintpass id already minted"
            );
        }
        require(msg.value == salePrice * amount, "sent incorrect price");
    ...
    function _mintWorldstar(address to, uint256 amount) private {
        for (uint256 i = 0; i < amount; i++) {
    ...
    function switchMilestoneReachedState() external onlyOwner {
        emit MilestoneReachedStateChange(
