...
contract MDCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MDC_Dividend_Tracker", "MDC_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MDC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MDC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MDC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MDC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "MDC_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ShibaMars is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MDCDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee) public onlyOwner {
       ...
       // BuyBackFee = _BuyBackFee;
        ...
        totalFees = BNBRewardsFee.add(liquidityFee); // total fee transfer and buy
    }
   ...
   // function setExtraFeeOnSell(uint256 _extraFeeOnSell) public onlyOwner {
    ...
    constructor() ERC20("Shiba Mars", "SMARS") {
        BNBRewardsFee = 5;
       ...
       // marketingFee = 0;
        //BuyBackFee = 0;
        //marketingWallet = payable(0x5E8c77C6C98eaADb1C34Eb87C227b89b8a5b3A35);
       // BuyBackWallet = payable(0xa8681e55E706436898C6f93580fc9D95a684971f);

        totalFees = BNBRewardsFee.add(liquidityFee); // total fee transfer and buy

    	dividendTracker = new MDCDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MDC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MDC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MDC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MDC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MDC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MDC: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        // if(extraFee>0)
        //    {
               // uint256 extraFeeN = extraFee/2;
               // BNBRewardsFee = BNBRewardsFee + extraFeeN;
           // }   
      //  }
        if(
            overMinTokenBalance &&
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
          ...
          //  }
           ...
           // {
            //    uint256 extraFeeN = extraFee/2;
            //    swapAndSendBNBToBuyback(extraFeeN); // swap the marketing fee on second
           // }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
       ...
       // uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(marketingFee);
        ...
        // BB fee
        //uint256 BuyBackAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(BuyBackFee);
   ...
   // function swapAndSendBNBToBuyback(uint256 tokenAmount) private {
