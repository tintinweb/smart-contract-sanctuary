...
contract InsuracePositionController is AbstractController {
    ...
    function __InsuracePositionController_init(
        ...
        _stakingAsset = _stkingAssetsAddresse;
        ...
        IERC20(_stakingAsset).approve(address(_stakingController), PreciseUnitMath.MAX_UINT_256);
    ...
    */
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        _stakes = _stakes.sub(_unstakedAmount);
    ...
    function unluckRewards() external {
        ...
        stakingAssetAddresses[0] = _stakingAsset;
    ...
    function outstandingRewards() public view override returns (uint256) {
        return _stakersPool.showHarvestRewards(address(this), _lpToken);
    }

    function _currentlyAtStake() internal view returns (uint256) {
        return _stakes;
    }
    ...
    function netWorth() external view override returns (uint256) {
        ...
        uint256 rewardsinBaseToken;
        if (_rewards != 0) {
            // returns INSUR rewards in base asset
            rewardsinBaseToken = _priceFeed.howManyTokensAinB(
                address(base),
                _insurToken,
                _rewards
            );
        } else {
            rewardsinBaseToken = 0;
        }

        // returns currently staked amount in base asset
        uint256 totalStakedinBaseToken = _priceFeed.howManyTokensAinB(
            address(base),
            _stakingAsset,
            _currentlyAtStake()
        );

        return totalStakedinBaseToken.add(rewardsinBaseToken);
    }

    function apy() external override returns (uint256 apy) {
        //TODO
        // is if same what _caclulatePoolAPY does?
        // note: return value is expected as % with with 10**5 precision
        apy=  _caclulatePoolAPY(_stakingAsset);
    }

    function productList() external view override returns (address[] memory _products) {
        _products = new address[](1);
        _products[0] = _stakingAsset;
    }
    ...
    function _caclulatePoolAPY(address stakingtokenAddress) internal returns (uint256 apy) {
        ...
        // P insur = Price of 1 INSUR to base from UniswapvV2
        uint256 insurPrice = _priceFeed.howManyTokensAinB(address(base), _insurToken, 1 ether);

        // P = Price of 1 staking token to base from UniswapvV2
        uint256 stakingTokenPrice = _priceFeed.howManyTokensAinB(
            address(base),
            _stakingAsset,
            1 ether
        );
    ...
    {
        // So far no staking requests info method on insurace
    }
}
