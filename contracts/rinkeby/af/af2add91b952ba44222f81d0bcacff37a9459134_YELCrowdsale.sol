...
/**
 * @title YELCrowdsale
 * @dev Extension of Crowdsale contract that increases the price of tokens after reaching some threshold of purchased tokens.
 * Note that what should be provided to the constructor is the initial _rate_, that is, the amount of tokens per wei contributed.
 */
contract YELCrowdsale is TimedCrowdsale {
    using SafeMath for uint256;
     ...
     */
    constructor (uint256 initialRate, uint256 factor, uint256 threshold,
//        address payable wallet,
        IERC20 token, uint256 openingTime, uint256 closingTime)

        Crowdsale(initialRate, 0x56E38ad2e2b0c278f1FAbE771562A064EecA080e, token)
        ...
        _rate5m = _initialRate.div(_factor);
        _rate10m = _rate5m.div(_factor);
        _rate15m = _rate10m.div(_factor);
        _rate20m = _rate15m.div(_factor);
