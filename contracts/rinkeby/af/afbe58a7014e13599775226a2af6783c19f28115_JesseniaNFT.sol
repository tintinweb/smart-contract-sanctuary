...
contract JesseniaNFT is ERC1155, Ownable {
    event PermanentURI(string _value, uint256 indexed _id);
    string public name = "Jessenia";
    ...
    function addToken(string memory uri)
    public
    onlyOwner
  {
    totalTokens += 1;
    tokenURI[totalTokens] = uri;
 ...
 function uri(uint256 tokenId) override public view returns (string memory) {
  ...
  function mintToMany(address[] memory to, uint256 id, uint256 qty)
    public
    onlyOwner
    tokenExists(id)
  {
  ...
  function freezeTokenData(uint256 id)
    public
    onlyOwner
    tokenExists(id)
  {
...
function updateTokenData(uint256 id, string memory newuri)
    public
    onlyOwner
    tokenExists(id)
  {
