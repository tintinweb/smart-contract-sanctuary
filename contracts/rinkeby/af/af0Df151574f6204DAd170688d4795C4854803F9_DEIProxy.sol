...
contract DEIProxy is Ownable {
	...
	constructor(
		...
		IERC20(dei_address).approve(_pool_collateral, type(uint256).max);
	}
	...
	function collateral2DEI(
		...
		if(global_collateral_ratio == 1000000) {
			...
			deus_amount = IUniswapV2Router02(uniswap_router).swapExactTokensForTokens(_dei_amount, 1, dei2deus_path, address(this), deadline)[1];
