...
contract AlphaVault is IVault, IUniswapV3MintCallback, ERC20, ReentrancyGuard {
    ...
    IUniswapV3Pool public pool; //mike v3的池子
    IERC20 public token0; //mike v3的token0
    IERC20 public token1; //mike v3的token1
    int24 public tickSpacing; //mike 池子的tickspacing

    uint256 public protocolFee; //mike 50000，表示5%的用户收益需要给protocol
    address public strategy; //mike 策略合约
    address public governance;
    address public pendingGovernance;

    int24 public baseLower; //mike 当前做市的base单
    int24 public baseUpper;
    int24 public limitLower; //mike 当前做市的limit单
    int24 public limitUpper;
    uint256 public accruedProtocolFees0; //mike protocol的token0收益
    uint256 public accruedProtocolFees1;

    uint256 public maxTotalSupply;
    mapping(address => bool) public whiteList;
    bool public openVault; //是否开放给公众
    ...
    constructor(
        ...
        protocolFee = _protocolFee; //mike 设置protocol的费率
        governance = _governance;

        require(_protocolFee < 1e6, "protocolFee");
    }

    //mike amount0Desired等的计价单位是最小单位
    //mike deposit两种token，生成的shares发给to，设置最小和最大付出的amount。deposit后不会立即去v3做市，等待下一次rebalance
    function deposit(
        ...
        )
    {
        ...
        //mike 如果有流动性，burn掉0个以计算收益，burn 0的话，只是用于更新收益，不会真的减少流动性
        ...
        //mike 根据最大的输入amount，计算可以得到的shares以及应该投入多少个token0和token1
        ...
        require(totalSupply() <= maxTotalSupply, "maxTotalSupply"); //mike 不能超过vault的最大supply
    }

    // @dev Calculates the largest possible `amount0` and `amount1` such that
    // they're in the same proportion as total amounts, but not greater than
    // `amount0Desired` and `amount1Desired` respectively.
    //mike 根据投入的token0和token1计算对应的shares
        ...
        )
    {
        ...
        (uint256 total0, uint256 total1) = getTotalAmounts(); //mike 计算本vault现有的所有token0、token1资产

        // If total supply > 0, vault can't be empty
        assert(totalSupply == 0 || total0 > 0 || total1 > 0);
        //mike 第一次的话，就直接以desired作为初始参数
        if (totalSupply == 0) {
            ...
            //mike 依据amount1占token1的资产比例，算shares
            amount1 = amount1Desired;
            shares = amount1.mul(totalSupply).div(total1);
        } else if (total1 == 0) {
            //mike 依据amount0占token0的资产比例，算shares
            amount0 = amount0Desired;
            shares = amount0.mul(totalSupply).div(total0);
        } else {
            //mike 根据token0和token1的比例，计算合适的amount0和amount1
    ...
    //mike 赎回自己的流动性，amountMin是最少获取的数量
    function withdraw(
        uint256 shares,
        uint256 amount0Min,
        uint256 amount1Min,
    ...
    function _burnLiquidityShare(
        ...
        uint128 position = _positionLiquidity(tickLower, tickUpper); //mike 得到这段范围，本vault的总流动性
        uint256 liquidity = uint256(position).mul(shares).div(totalSupply()); //mike shares占total的比重，burn掉这么多的流动性

        if (liquidity > 0) {
            (amount0, amount1) = pool.burn(tickLower, tickUpper, _toUint128(liquidity)); //mike burn掉v3上的流动性，钱还在v3

            if (amount0 > 0 || amount1 > 0) {
                //mike 将上面burn后的本金从v3撤回到to
    ...
    function rebalance(
        ...
        require(msg.sender == strategy, "strategy"); //mike 只有strategy可以调用
        _checkRange(_baseLower, _baseUpper); //mike 检查tick是否合法，一定是tickspacing的整数倍
        ...
        //mike 根据balance计算可以提供的流动性
        uint128 liquidity = _liquidityForAmounts(_baseLower, _baseUpper, balance0, balance1);
        _mintLiquidity(_baseLower, _baseUpper, liquidity); //mike 去v3做市，mint到本vault
        (baseLower, baseUpper) = (_baseLower, _baseUpper); //mike 更新base order
    ...
    //mike 从v3取回流动性和收益，收益需要缴税
    function _burnAllLiquidity(int24 tickLower, int24 tickUpper) internal {
        // Burn all liquidity in this range
        uint256 owed0 = 0;
        uint256 owed1 = 0;
        uint128 liquidity = _positionLiquidity(tickLower, tickUpper); //mike 自己所有的liquidity
        if (liquidity > 0) {
            //mike 撤销流动性，owed是burn这些流动性时，v3池子欠用户的本金
            (owed0, owed1) = pool.burn(tickLower, tickUpper, liquidity);
        }

        // Collect all owed tokens including earned fees
        //mike 取回本金+手续费到本合约，collect0-owed0是纯利润
        ...
        uint256 feesFromPool0 = collect0.sub(owed0); //mike token0做市的净收益
        uint256 feesFromPool1 = collect1.sub(owed1); //mike token1做市的净收益
        ...
        if (_protocolFee > 0) {
            ...
            accruedProtocolFees0 = accruedProtocolFees0.add(feesToProtocol0); //mike protocol的累计收益增加
     ...
     */
    //mike 本vault拥有的总token0和总token1资产
    function getTotalAmounts() public view override returns (uint256 total0, uint256 total1) {
        ...
        total0 = _balance0().add(baseAmount0).add(limitAmount0); //mike vault里剩余的+base里的+limit里的
        ...
        returns (uint128 liquidity)
    {
        ...
        (liquidity, , , , ) = pool.positions(positionKey); //mike 根据nft得到流动性
    }

    /// @dev Amounts of token0 and token1 held in vault's position.
    //mike vault在该tick拥有的所有token0、token1资产
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        //mike tokensOwed0和tokensOwed1是最近一次burn后，v3pool总共欠vault的
        amount0 = amount0.add(uint256(tokensOwed0));
        amount1 = amount1.add(uint256(tokensOwed1));
    }

    /// @dev Wrapper around `LiquidityAmounts.getAmountsForLiquidity()`.
    //mike 根据tick和流动性，获取amount
    function _amountsForLiquidity(
        ...
        //mike 根据v3的价格数量流动性三者的公式，计算出amount
        return
    ...
    //mike 根据amount计算liquidity，tickLower是tickspacing的整数倍
    function _liquidityForAmounts(
        ...
        (uint160 sqrtRatioX96, , , , , , ) = pool.slot0(); //mike 获取当前的根号price
        //mike 根据v3的价格数量流动性三者的公式，计算出流动性
        return
            LiquidityAmounts.getLiquidityForAmounts(
                sqrtRatioX96,
                TickMath.getSqrtRatioAtTick(tickLower), //mike tickLower对应的sqrtRatio
                TickMath.getSqrtRatioAtTick(tickUpper), //mike tickUpper对应的sqrtRatio
                amount0,
                amount1
            );
    }
    ...
    //mike 增加流动性的回掉函数，将token0和token1转给univ3
    function uniswapV3MintCallback(
        uint256 amount0,
        uint256 amount1,
        bytes calldata data
    ) external override {
        require(msg.sender == address(pool)); //mike 由v3池子发起
    ...
    //mike 设置对外开放
    function setOpen(bool _openVault) external onlyGovernance {
        openVault = _openVault;
    }

    //mike 设置白名单
    function addWhiteList(address _user, bool _value) external onlyGovernance {
...
contract AlphaStrategy {
    ...
    int24 public tickSpacing; //mike 60
    int24 public lastTick;
    int24 public minTickMove;

    int24 public baseThreshold; //mike 3600, 必须是tickspacing的整数倍
    int24 public limitThreshold; //mike 1200
    int24 public maxTwapDeviation; //mike 100
    uint32 public twapDuration; //mike 60
    ...
    constructor(
        ...
        pool = vault.pool(); //mike 现在是 0x4e68Ccd3E89f51C3074ca5072bbAC773960dFa36, 这个池子是0.3%手续费
        //mike 0x11b815efb8f581194ae79006d24e0d814b7697f6 是0.05%的eth-usdt池子
        tickSpacing = pool.tickSpacing(); //mike 60
    ...
    //mike admin主动rebalance
    function rebalance() external {
        require(msg.sender == keeper, "keeper");

        // check price has moved enough
        int24 tick = getTick(); //mike 获取当前slot的tick，tick可以不是tickspacing的整数倍
        int24 tickMove = tick > lastTick ? tick - lastTick : lastTick - tick;
        require(tickMove >= minTickMove, "move too little");

        int24 maxThreshold = baseThreshold > limitThreshold ? baseThreshold : limitThreshold; //mike 取baseThreshold和limitThreshold中的较大者
        require(tick > TickMath.MIN_TICK + maxThreshold + tickSpacing, "tick too low"); //mike tick下边界检查
        require(tick < TickMath.MAX_TICK - maxThreshold - tickSpacing, "tick too high"); //mike tick上边界检查

        int24 twap = getTwap(); //mike 获取最近一段时间的价格，这里仅用作边界校验，防止价格变化太剧烈
        int24 deviation = tick > twap ? tick - twap : twap - tick;
        require(deviation <= maxTwapDeviation, "maxTwapDeviation"); //mike 如果很大怎么办，剧烈变化的行情，就不rebalance

        int24 tickFloor = _floor(tick); //mike 找到该tick的底
        int24 tickCeil = tickFloor + tickSpacing; //mike 找到该tick的顶
        vault.rebalance(
    ...
    function getTwap() public view returns (int24) {
        ...
        (int56[] memory tickCumulatives, ) = pool.observe(secondsAgo); //mike 获取最近v3池子的价格和流动性
