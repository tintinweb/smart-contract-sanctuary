...
contract ERC20Rewards {
  ...
  function claim(uint256 id) public {
    ...
    if(id <= 100){
      ...
      require(token.transfer(msg.sender,50*10**18));
    } else if(id > 300 && id <= 600){
      require(token.transfer(msg.sender,25*10**18));
    } else if(id > 600 && id <= 900){
      require(token.transfer(msg.sender,125*10**17));
    } else if(id > 900 && id <= 1500){
      require(token.transfer(msg.sender,625*10**17));
    }
    claimed[msg.sender][id] = true;
    emit Claimed(msg.sender);
  }

  function claimMany(uint256[] memory ids) public {
