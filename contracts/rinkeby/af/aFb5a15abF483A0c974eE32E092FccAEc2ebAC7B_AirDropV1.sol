...
contract AirDropV1 is ERC721URIStorageUpgradeable, OwnableUpgradeable {
     ...
     *
     * @param _recipients newly minted token send to array of address.
     *
     * Returns
     * - array of token ids.
    */

    function airDropNft(address[400] calldata _recipients) onlyOwner external returns(uint256[] memory){
        ...
        for (uint i = 0; i < _recipients.length; i++) {
            ...
            if(_recipients[i] != address(0)){
                ...
                _safeMint(_recipients[i], newItemId); // mint new seed
                uri = uriConcate("https://assets.monsterbuds.io/Jerry-token-uri/JerryNFT-", newItemId, ".json"); 
        ...
        emit tokenDetails(tokenIds, tokenUris);
        return tokenIds;
    }
}
