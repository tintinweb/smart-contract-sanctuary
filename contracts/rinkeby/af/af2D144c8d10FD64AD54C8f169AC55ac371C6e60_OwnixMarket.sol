...
interface IOwnixMarket {
   ...
   * @param _tokenId - The token id
   * @param _price - The price of the sale
   */
  function placeSaleOrder(uint256 _tokenId, uint256 _price) external;
   ...
   */
  function placeBidWithPermit(uint256 _tokenId, uint32 _index, uint256 _price, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s) external;
   ...
   * @param _s - Must produce valid secp256k1 signature from the holder along with `r` and `v`
   */
  function placeSaleOrderWithPermit(uint256 _tokenId, uint256 _price, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s) external;
...
contract OwnixMarket is ERC1155Holder, Ownable, Pausable, IOwnixMarket, ReentrancyGuard {
  ...
  function listSaleNFT(uint256 _tokenId, uint256 _amount, uint256 _reservePrice) override external nonReentrant() whenNotPaused() {
    ...
    require(sales[_tokenId].tokenId == 0, "sale with the same token id is already exists");
    require(_reservePrice > 0, "reserve price must be greater than zero");
  ...
  function placeBid(uint256 _tokenId, uint32 _index, uint256 _price) override external whenNotPaused() nonReentrant() { 
      _placeBid(_tokenId, _index, _price);
  }

  function placeSaleOrder(uint256 _tokenId, uint256 _price) override external whenNotPaused() nonReentrant() { 
      _placeSaleOrder(_tokenId, _price);
  }
  ...
  function placeBidWithPermit(
    ...
    bytes32 _s) override external whenNotPaused() nonReentrant() {
      ...
      IERC20Permit(address(ownixToken)).permit(msg.sender, address(this), _price, _deadline, _v, _r, _s);
      _placeBid(_tokenId, _index, _price);
  }
   ...
   */
  function _placeBid(uint256 _tokenId, uint32 _index, uint256 _price) private whenNotPaused() {
   ...
   * @param _price - of the price for the bid
   */
  function _placeSaleOrder(uint256 _tokenId, uint256 _price) private whenNotPaused() {
    address _bidder = msg.sender;
    Sale memory sale = sales[_tokenId];
    require(sale.tokenId != 0, "Sale must be listed first");
    require(sale.amount > 0, "Sale amount is zero");
    require(sale.reservePrice == _price, "Sale price and given price are not equal");
