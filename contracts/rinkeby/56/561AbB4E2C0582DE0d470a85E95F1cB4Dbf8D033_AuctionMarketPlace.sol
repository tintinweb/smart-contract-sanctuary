...
contract AuctionMarketPlace is Pausable, Ownable {
    ...
    // +STORAGE -------------------------------------------------
    NFToken                     private NFTokenContract;
    ...
    address[]                   private validERC20TokenContractsList;
    // -STORAGE -------------------------------------------------
    ...
    constructor(address _NFTokenContract, uint8 marketPlaceSharePercentage) Ownable() {
        NFTokenContract = NFToken(_NFTokenContract);
        marketPlaceShare = marketPlaceSharePercentage;
    }

    function addERC20TokenContractAddress(address _contract) public onlyOwner {
        //TODO check address is contract and implement ERC20 stndard
    ...
    function createAuction(
        ...
        require(_owns(msg.sender, _tokenId), "this item is not in possition of this address");
    ...
    function acceptBid(uint256 _tokenId, address _winner) external whenNotPaused {
        Auction storage _auction = tokenIdToAuction[_tokenId];
        ...
        uint256 creatorShare = NFTokenContract.lookUpRoyaltyPercentage(_tokenId).mul(remaining).div(100);
        uint256 sellerShare = remaining.sub(creatorShare);
        address tokenCreator = NFTokenContract.lookUpInitialOwner(_tokenId);
        ...
        NFTokenContract.transferFrom(address(this), msg.sender, _tokenId);
    ...
    function cancelAuction(uint256 _tokenId) external whenNotPaused {
        ...
        NFTokenContract.transferFrom(address(this), _auction.seller, _tokenId);
    ...
    function _escrow(address sender, uint256 _tokenId) internal {
        NFTokenContract.transferFrom(payable(sender), payable(address(this)), _tokenId);
    }

    function _transferToken(address _receiver, uint256 _tokenId) internal {
        NFTokenContract.transferFrom(payable(address(this)), payable(_receiver), _tokenId);
    }

    function _owns(address sender, uint256 _tokeId) internal view returns (bool){
        return (NFTokenContract.ownerOf(_tokeId) == sender);
    }
...
contract FixedPriceMarketPlace is ReentrancyGuard, Ownable {
    ...
    // +EVENTS --------------------------------------------------
    event EmitAddress(address initialOwner, uint256 royaltyPercentage);
    ...
    function createMarketItem(uint256 tokenId, uint256 price) public payable nonReentrant returns (uint256) {
        ...
        idToMarketItem[itemId] = MarketItem(
            ...
            price,
            false);
        NFTokenContract.transferFrom(msg.sender, address(this), tokenId);
    ...
    function deleteMarketItem(uint256 _itemId) public {
        ...
        NFTokenContract.transferFrom(address(this), _marketItem.seller, idToMarketItem[_itemId].tokenId);
    ...
    function createMarketSale(uint256 itemId) public payable nonReentrant{
        ...
        address payable tokenCreator = NFTokenContract.lookUpInitialOwner(_tokenId);
        uint royaltyPercentage = NFTokenContract.lookUpRoyaltyPercentage(_tokenId);
       
        uint256 marketPlaceShareAmount = marketPlaceShare.mul(price).div(100);
        uint256 remaining = msg.value.sub(marketPlaceShareAmount);
        uint256 creatorShare = royaltyPercentage.mul(remaining).div(100);
        uint256 sellerShare = remaining.sub(creatorShare);


        payable(owner()).transfer(marketPlaceShareAmount);

        marketItem.seller.transfer(sellerShare);
        emit MoneyTransferred(msg.sender, marketItem.seller, sellerShare);
        tokenCreator.transfer(creatorShare);
        NFTokenContract.transferFrom(address(this), msg.sender, _tokenId);
...
contract NFToken is ERC721URIStorage, Ownable {
  ...
  Counters.Counter private _burntItemCount;
  uint256[] private burnItems;
  mapping(uint256 => address payable) private idToInitialOwner;
  mapping(uint256 => uint256) private idToRoyaltyPercentage;

  mapping(string => bool) private tokenURIs;

  bool isActive;
  FixedPriceMarketPlace fixedPriceMarketPlaceContract;
  AuctionMarketPlace auctionMarketPlaceContract;
  // -STORAGE -------------------------------------------------
  ...
  event CompleteInitialization(address NFTokenContract, address FixedPriceMarketPlaceContract, address AuctionMarketPlaceContract);
  // -EVENTS --------------------------------------------------

  constructor () ERC721('Iran NFT', 'IRNFT') Ownable() {}
  ...
  function createToken(string memory tokenURI, uint256 royaltyPercentage) public returns (uint) {
      _tokenIds.increment();
      uint256 newItemId = _tokenIds.current();
      require(royaltyPercentage <= 50 && royaltyPercentage >= 0, 'invalid royalty');
      _mint(msg.sender, newItemId);
      _setTokenURI(newItemId, tokenURI);

      idToInitialOwner[_tokenIds.current()] = payable(msg.sender);
//--- TODO check royaltyPercentage correctness
      idToRoyaltyPercentage[_tokenIds.current()] = royaltyPercentage;
      emit TokenCreated(msg.sender, newItemId, tokenURI, royaltyPercentage);
      return newItemId;
  }

  function _setTokenURI(uint256 tokenId, string memory tokenURI) internal override {
    require(!tokenURIs[tokenURI], 'duplicate URI');
  ...
  function removeToken(uint256 tokenId) public {
      ...
      _burn(tokenId);
      _burntItemCount.increment();
      burnItems.push(tokenId);
      emit TokenBurnt(msg.sender, tokenId);
  }

  function lookUpInitialOwner(uint256 tokenId) public view returns(address payable initialOwner){
      initialOwner = payable(idToInitialOwner[tokenId]);
  }

  function lookUpRoyaltyPercentage(uint256 tokenId) public view returns(uint256 royaltyPercentage){
      royaltyPercentage = idToRoyaltyPercentage[tokenId];
      return royaltyPercentage;
  }

  function verifyMint(uint256 tokenId) public view returns(uint256 royaltyPercentage, string memory tokenURIString){
      royaltyPercentage = idToRoyaltyPercentage[tokenId];
