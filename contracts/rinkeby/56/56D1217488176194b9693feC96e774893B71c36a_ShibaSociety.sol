...
contract ShibaSociety is ERC721("Shiba Society", "SHIBS"), Ownable, Pausable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (tokenId < 1000) {
            return bytes(baseURI[0]).length > 0 && totalSupply >= 1000 ? string(abi.encodePacked(baseURI[0], 
                tokenId.toString())) : string(abi.encodePacked(blindURI[0], tokenId.toString()));
            ...
            return bytes(baseURI[5]).length > 0 && totalSupply >= 6000 ? string(abi.encodePacked(baseURI[5], 
                tokenId.toString())) : string(abi.encodePacked(blindURI[1], tokenId.toString()));
    ...
    */
    function setURIs(string[2] memory _blindURI, string[10] memory _URIs) external onlyOwner {
