...
contract Marketplace {
    ...
    internal view returns(string memory, uint256, address payable, address payable, string memory) {
        return(self.board[_addr][_count].name, self.board[_addr][_count].price, self.board[_addr][_count].buyer_addr, self.board[_addr][_count].seller_addr, self.board[_addr][_count].message);
    }
