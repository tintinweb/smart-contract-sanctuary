...
contract SVGNFT is ERC721URIStorage, Ownable {
    ...
    event gnomeAdded(uint256 indexed quantity);
    ...
    function addGnome(string[] memory gnome) public {
        for (uint256 index = totalGnomes; index < (50 + totalGnomes); index++) {
            gnomesr[index] = gnome[index];
        }
        totalGnomes += 50;
        emit gnomeAdded(50);
    }

    // function getAllGnomes() public returns (string[] memory) {
    //     for (uint256 index = 0; index < 100; index++) {
    //         gn.push(gnomess[index]);
    //     }
    //     return gn;
    // }
