...
contract LosslessV2Factory is ILosslessV2Factory {
	...
	///@dev only _DAO / admin can call this function
	...
	/**
	 * @dev	 The default DAO is admin but admin can assign this role to others by calling `setDAO`
	...
	/**
	 * @dev The default DAO is admin but admin can assign this role to others by calling `setDAO`
	 * @param _DAO	new admin address
	 **/
	function setDAO(address _DAO) external override onlyDAO {
		require(_DAO != address(0), "LosslessV2Factory: set _DAO to the zero address");
		DAO = _DAO;
		emit DAOChanged(DAO);
	}
}
