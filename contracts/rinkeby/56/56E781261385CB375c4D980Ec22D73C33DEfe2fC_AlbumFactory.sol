...
abstract contract AlbumTokenSaleManager {
    event SaleProceedsSwept(address sweeper, uint256 amount);
    ...
    function buyTokens() public payable saleOver(false) {
        ...
        uint256 amount = msg.value * TOKENS_PER_MILLIETH;
        amountUnsold -= amount;
    ...
    function sweepProceeds() public saleOver(true) {
        require(msg.sender == CREATOR, "Only creator can sweep");
        require(!swept, "Already swept");
        swept = true;
        payable(msg.sender).transfer(saleProceeds);
        getToken().transfer(msg.sender, amountUnsold);
    }
}
