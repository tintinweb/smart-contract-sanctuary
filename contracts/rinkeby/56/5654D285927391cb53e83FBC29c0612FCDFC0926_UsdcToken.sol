...
/**
 * @title USDC Token Mock implementation
 * @author Keyko
 *
 * @dev Contract that implements a ERC20 token which will be
 *      used as a mock for USDC token
 *
 */
    ...
    OwnableUpgradeable
{
    ...
    function initialize() external initializer {
        ERC20Upgradeable.__ERC20_init("USDC Token", "USDC");
   ...
   /** 
    ...
    function mint(address _account, uint256 _amount) public override {
        require(
            ...
            "USDC: You don't have permission to mint tokens!"
        );
    ...
    function burn(address _account, uint256 _amount) public override {
        require(
            hasRole(MINTER_AND_BURNER_ROLE, _msgSender()),
            "USDC: You don't have permission to burn tokens!"
        );
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            _address != address(0),
            "USDC: contract address cannot be address 0!"
        );
...
/**
 * @title USDC Token Mock Interface
 * @author Keyko
 *
 * @dev Provides an interface for the USDC token mock
 *
 */
