...
contract PickRandomWinner is VRFConsumerBase, Ownable {
    ...
    event Recover(address recoveredAddress, address contractAddress);
    
    bytes32 private privKeyHash; 
	uint256 private privFee;
    address privSignedVerifier; 
    ...
    function requestWinner(bytes memory signature, uint256 blockExpiry, uint256 documentId, string memory urlHash, uint256 totalSigners) public {
        bytes32 message = requestWinnerSigningHash(blockExpiry, documentId, urlHash, totalSigners).toEthSignedMessageHash();

        address recoveredAddress = ECDSA.recover(message, signature);

        emit Recover(recoveredAddress, privSignedVerifier);

        require(recoveredAddress == privSignedVerifier, "Permission to call this function failed");
