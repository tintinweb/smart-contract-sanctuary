...
contract ClownPunkSociety is ERC721Enumerable, Ownable {
    ...
    function punkToClownAssets(uint16 punkIndex) internal view returns (bytes memory childAssets) {
        ...
        for (uint8 j = 0; j < 10 && assetIndex < 8; ++j) {
            ...
            if (j == uint8(Type.Nose)) {
                childAssets[assetIndex++] = bytes1(isMale ? 134 : 135);
            } else if (j == uint8(Type.Nose)) {
                childAssets[assetIndex++] = bytes1(isMale ? 138 : 139);
            } else if (j == uint8(Type.Hair)) {
                childAssets[assetIndex++] = bytes1(isMale ? 140 : 141);
            } else if (asset != 0) {
                childAssets[assetIndex++] = bytes1(asset);
            }
