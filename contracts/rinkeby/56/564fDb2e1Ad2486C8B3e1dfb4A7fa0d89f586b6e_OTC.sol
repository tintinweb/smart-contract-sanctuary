...
contract OTC is Ownable {
    ...
    // fees
    uint public immutable MAX_SELL_FEES = 100; // maximum fees on the sell (10%)
    uint public totalSellFees; // total fees on the sell (base 1000)
    uint public toBuyerRatioOnSellFees; // ratio (0 - 1) (base 1000) of the total fees taken which will be send to the buyer (redistribution) => usefull to make OTC more attrative (near 1) or to sustain paper (near 0)

    constructor (address _papr, address _anchor, uint _totalSellFees, uint _toBuyerRatioOnSellFees) {
        require(_totalSellFees <= MAX_SELL_FEES);
        require(_toBuyerRatioOnSellFees <= 1000);
        
        papr = _papr;
        anchor = _anchor;
        totalSellFees = _totalSellFees;
        toBuyerRatioOnSellFees = _toBuyerRatioOnSellFees;
    }

    function setTotalSellFees(uint _newSellFees) external onlyOwner {
        require(_newSellFees <= MAX_SELL_FEES, "new sell fees too high");
        totalSellFees = _newSellFees;
    }

    function setToBuyerRatioOnSellFees(uint _newRatio) external onlyOwner {
        require(_newRatio <= 1000, "new ratio too high");
        toBuyerRatioOnSellFees = _newRatio;
    }
    ...
    */
    function createBuyOrder(uint _anchorAmount, uint _maxBuyPrice, uint _lastId, uint _orderIteration) external lastIdValid(0, _maxBuyPrice, _lastId) returns (bool) {
        ...
        uint topMaxBuyPrice = AskOrderBook[topAskOrder].maxBuyPrice; // save the top max buy price before be change it
        ...
        if (_maxBuyPrice > topMaxBuyPrice) { // if the max buy price is the hightest possible, the order can possibly be directly filled
    ...
    */
    ...
    function executeTransaction(uint _buyOrderId, uint _sellOrderId) private {
        ...
        uint txFees = sellOrder.paprAmount.mul(totalSellFees).div(1000);
        uint sellPaprAmount = sellOrder.paprAmount.sub(txFees); // amount of papr the seller will be pay for (his real amount sub fees)
        uint paprAmount;
        uint anchorAmount;

        // get the limitants amounts of papr and anchor token
        if (buyOrder.anchorAmount > sellPaprAmount.mul(transactionPrice)) { // limitant is papr
            paprAmount = sellPaprAmount;
            anchorAmount = paprAmount.mul(transactionPrice);

            removeOrder(1, _sellOrderId); // entierly remove the order filled
            buyOrder.anchorAmount = buyOrder.anchorAmount.sub(anchorAmount); // update the amount remaining after this execution
        } else if (buyOrder.anchorAmount < sellPaprAmount.mul(transactionPrice)) { // limitant is anchor
            anchorAmount = buyOrder.anchorAmount;

            uint _paprAmount = anchorAmount.div(transactionPrice).div(uint(1000).sub(totalSellFees)).mul(1000); // compute the max papr amount for anchorAmount
            txFees = _paprAmount.mul(totalSellFees).div(1000); // we need to re set txFees with the real fees because paprAmount is less than sellPaprAmount
            paprAmount = _paprAmount.sub(txFees); // papr amount that will be send (with fees)

            removeOrder(0, _buyOrderId); // entierly remove the order filled
            sellOrder.paprAmount = sellOrder.paprAmount.sub(paprAmount); // update the amount remaining after this execution
        } else { // both are limitant
            removeOrder(1, _sellOrderId); // entierly remove the order filled
            removeOrder(0, _buyOrderId); // entierly remove the order filled
        }
        uint feesToBuyerAmount = txFees.mul(toBuyerRatioOnSellFees).div(1000);
        uint feesToBurnAmount = txFees.sub(feesToBuyerAmount);
        uint paprToTransferAmount = paprAmount.add(feesToBuyerAmount);

        IERC20(papr).burn(feesToBurnAmount);
        IERC20(papr).safeTransfer(buyOrder.owner, paprToTransferAmount);
        IERC20(anchor).safeTransfer(sellOrder.owner, anchorAmount);
    }
}