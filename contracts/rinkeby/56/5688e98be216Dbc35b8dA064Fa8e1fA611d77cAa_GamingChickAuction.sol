...
contract GamingChickAuction is OwnableContract, IERC721Receiver{
    ...
    mapping(uint16 => bool) private luckyAttr;
    mapping(uint16 => bool) private laborAttr;
    mapping(uint16 => bool) private bossAttr;
    ...
    constructor() public {
        ...
        for (uint256 i = 0; i < luckySpecialAttr.length; i++) {
            luckyAttr[luckySpecialAttr[i]] = true;
        }
        for (uint256 i = 0; i < laborSpecialAttr.length; i++) {
            laborAttr[laborSpecialAttr[i]] = true;
        }
        for (uint256 i = 0; i < bossSpecialAttr.length; i++) {
            bossAttr[bossSpecialAttr[i]] = true;
        }
    }
        ...
        for(uint256 i=0; i<_tokenId.length; i++){
            ...
            for (uint256 j = 0; j < attributes.length; j++) {
                if (!emptyAttr[attributes[j]]) {
                    if (_pid == 0) {
                        if (luckyAttr[attributes[j]]) {
                            uint256 count = user.specialAttr.get(uint256(attributes[j])).add(1);
                            user.specialAttr.set(attributes[j], count);
                        } else {
                            uint256 count = user.normalAttr.get(uint256(attributes[j])).add(1);
                            user.normalAttr.set(attributes[j], count);
                        }
                    } else if (_pid == 1) {
                        if (laborAttr[attributes[i]]) {
                            uint256 count = user.specialAttr.get(uint256(attributes[j])).add(1);
                            user.specialAttr.set(attributes[j], count);
                        } else {
                            uint256 count = user.normalAttr.get(uint256(attributes[j])).add(1);
        ...
        for(uint256 i=0; i < _amount; i++){
            ...
            for (uint256 j = 0; j < attributes.length; j++) {
                if (!emptyAttr[attributes[j]]) {
                    if (_pid == 0) {
                        if (luckyAttr[attributes[j]]) {
                            uint256 count = user.specialAttr.get(uint256(attributes[j])).sub(1);
                            user.specialAttr.set(attributes[j], count);
                        } else {
                            uint256 count = user.normalAttr.get(uint256(attributes[j])).sub(1);
                            user.normalAttr.set(attributes[j], count);
                        }
                    } else if (_pid == 1) {
                        if (laborAttr[attributes[i]]) {
