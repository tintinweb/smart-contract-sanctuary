...
import "./IPietaExchange.sol";

contract PietaExchange is IPietaExchange, Initializable, OwnableUpgradeable,  EIP712Upgradeable  {
    ...
    CountersUpgradeable.Counter _saleId;

    address _pietaTokenAddress;
    address _pietaNftAddress;
    

    mapping (uint256 => OriginInfo) _originusers; // _originusers[tokenId] = OriginInfo{user, 400000 (<--4%)}
    mapping (uint256 => SaleInfo) _saleInfos; // _saleInfos[saleId]
    mapping (uint256 => SaleApproveInfo) _saleAppove; // _saleAppove[saleId]
    mapping (uint256 => uint8) _tokenSaleStatus; // _tokenSaleStatus[tokenId]
    mapping (uint256 => uint256) _latestSaleId; // _latestSaleId[tokenId]
    ...
    function pietaTokenAddress() external view returns (address) {
        return _pietaTokenAddress;
    }

    function pietaNftAddress() external view returns (address) {
    ...
    function getSaleStatus(uint256 saleId) public view returns (uint8) {
        SaleInfo memory saleInfo = _saleInfos[saleId];
        if(_latestSaleId[saleInfo.tokenId] != saleId || saleInfo.expire < block.timestamp) {
            return uint8(SaleStatus.Expired);
        } else {
    ...
    function tokenValidator(address maker, uint256 tokenId) internal view returns (bool) {
        bytes memory isApprovedForAllPayLoad = abi.encodeWithSignature("isApprovedForAll(address,address)", maker, address(this));
        bytes memory approvedResult = _pietaNftAddress.functionStaticCall(isApprovedForAllPayLoad);
        ...
        bytes memory ownerOfResult = _pietaNftAddress.functionStaticCall(ownerOfPayload);
        address tokenOwner = abi.decode(ownerOfResult, (address));
        
        return isApproved && maker == tokenOwner;
    }

    function tokenAllowance(address owner) internal view returns (uint256) {
        bytes memory allowancePayload = abi.encodeWithSignature("allowance(address,address)", owner, address(this));
        bytes memory allowanceResult = _pietaTokenAddress.functionStaticCall(allowancePayload);
    ...
    function saleApprove(uint256 saleId, address toAddress, uint256 amount) public override returns(bool) {
        SaleInfo memory saleInfo = _saleInfos[saleId];
        require(saleInfo.maker != address(0), "Doesn't exist.");
        require(saleInfo.maker == _msgSender(), "No authority.");
        require(saleInfo.tradeMethod == uint8(TradeMethod.BID), "This work is not necessary.");
        require(_tokenSaleStatus[saleInfo.tokenId] == uint8(SaleStatus.OnSale), "It's not on sale.");


        uint8 status = getSaleStatus(saleId);
        require(status != uint8(SaleStatus.Expired), "Sales expired.");
        
        _saleAppove[saleId] = SaleApproveInfo(toAddress, amount);
        _tokenSaleStatus[saleInfo.tokenId] = uint8(SaleStatus.Approve);
        emit SaleApprove(saleId, saleInfo.maker, toAddress, amount);
        return true;
    }

    function transferErc721(address from, address to, uint256 tokenId) internal  {
        bytes memory safeTransferPayload = abi.encodeWithSignature("safeTransferFrom(address,address,uint256)"
                                            , from, to, tokenId);
        _pietaNftAddress.functionCall(safeTransferPayload);
    }

    function transferErc20(address from, address to, uint256 amount) internal returns(bool) {
        bytes memory transferPayload = abi.encodeWithSignature("transferFrom(address,address,uint256)"
                                            , from, to, amount);
        bytes memory result = _pietaTokenAddress.functionCall(transferPayload);
        bool r = abi.decode(result, (bool));
        return r;
    }

    function buyByEth(uint256 saleId) public payable override returns(bool) {        
        SaleInfo memory saleInfo = _saleInfos[saleId];
        require(saleInfo.maker != address(0), "Doesn't exist.");

        require(saleInfo.payType == uint8(PayType.ETH), "It's an abnormal approach.");

        uint8 status = getSaleStatus(saleId);
        require(status != uint8(SaleStatus.Expired), "Sales expired.");

        require( 
                (saleInfo.tradeMethod == uint8(TradeMethod.DIRECT) && status == uint8(SaleStatus.OnSale))
            ||  (saleInfo.tradeMethod == uint8(TradeMethod.BID) && status == uint8(SaleStatus.Approve)),
            "Unable to purchase.");

        SaleApproveInfo memory saleApproveInfo = _saleAppove[saleId];

        // Condition TradeMethod == BID 
        require(saleInfo.tradeMethod == uint8(TradeMethod.DIRECT)
            ||  (saleInfo.tradeMethod == uint8(TradeMethod.BID) && saleApproveInfo.buyer == _msgSender()),
            "It is not an approved buyer.");

        // Condition TradeMethod == BID 
        require(saleInfo.tradeMethod == uint8(TradeMethod.DIRECT) 
            || (saleInfo.tradeMethod == uint8(TradeMethod.BID) && saleApproveInfo.amount == msg.value),
            "The amount does not match.(B)");

        // Condition TradeMethod == DIRECT 
        require(saleInfo.tradeMethod == uint8(TradeMethod.BID) 
            || (saleInfo.tradeMethod == uint8(TradeMethod.DIRECT) && saleInfo.minOrAskPrice == msg.value),
            "The amount does not match.(D)");
        ...
        transferErc721(saleInfo.maker, _msgSender(), saleInfo.tokenId);

        OriginInfo memory originInfo = _originusers[saleInfo.tokenId];

        if(saleInfo.maker == originInfo.owner) { // 원작자 판매자 같으면 총 금액 전송
            (bool success, ) = payable(saleInfo.maker).call{value: msg.value}("");
            _tokenSaleStatus[saleInfo.tokenId] = uint8(SaleStatus.Complete);
    ...
    function buyByToken(uint256 saleId, uint256 tokenAmount) public override returns(bool) {
        SaleInfo memory saleInfo = _saleInfos[saleId];
        require(saleInfo.maker != address(0), "Doesn't exist.");

        require(saleInfo.payType == uint8(PayType.TOKEN), "It's an abnormal approach.");
        ...
        // Condition TradeMethod == BID 
        require(saleInfo.tradeMethod == uint8(TradeMethod.DIRECT) 
            || (saleInfo.tradeMethod == uint8(TradeMethod.BID) && saleApproveInfo.amount == tokenAmount),
            "The amount does not match.(B)");

        // Condition TradeMethod == DIRECT 
        require(saleInfo.tradeMethod == uint8(TradeMethod.BID) 
            || (saleInfo.tradeMethod == uint8(TradeMethod.DIRECT) && saleInfo.minOrAskPrice == tokenAmount),
            "The amount does not match.(D)");
        ...
        uint256 allowance = tokenAllowance(_msgSender());
        require(allowance >= tokenAmount, "Token transfer was not approved.");
        ...
        if(saleInfo.maker == originInfo.owner) { // 원작자 판매자 같으면 총 금액 전송
            transferErc20(_msgSender(), saleInfo.maker, tokenAmount);
            _tokenSaleStatus[saleInfo.tokenId] = uint8(SaleStatus.Complete);
            ...
            transferErc20(_msgSender(), saleInfo.maker, receiveAmount);
            transferErc20(_msgSender(), originInfo.owner, royalties);
            _tokenSaleStatus[saleInfo.tokenId] = uint8(SaleStatus.Complete);
            emit SaleComplete(saleId, saleInfo.maker, _msgSender(), receiveAmount, royalties);
            return true;
        }
    ...
    function cancel(uint256 saleId) public override returns(bool) {
        ...
        require(status == uint8(SaleStatus.OnSale), "Uncancellable status value.");

        _tokenSaleStatus[saleInfo.tokenId] = uint8(SaleStatus.Cancel);
    ...
    function saleRegister(bytes memory singmessag, 
        ...
        uint256 minOrAskPrice, 
        uint32 expire,
        uint32 royalties) public override returns(uint256)  {

        // 서명 검증
        require(verifyMessage(singmessag, maker, tokenId, tradeMethod, payType, minOrAskPrice, expire, royalties), 
                "The signed message is not valid.!");

        // 토큰이 maker의 소유, 컨트랙트 승인. 체크
        require(tokenValidator(maker, tokenId), 
                "You must approve your own token to sell it.");
        uint8 status = _tokenSaleStatus[tokenId];

        bool available = false;
        if(status == uint8(SaleStatus.OnSale) || status == uint8(SaleStatus.Approve)) { // 판매중..
        ...
        // 현재 같은 토큰이 판매 중인지 체크
        require(available
            , "There is the same token on sale.");
        ...
        _tokenSaleStatus[tokenId] = uint8(SaleStatus.OnSale);
...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.2;
abstract contract IPietaExchange {
