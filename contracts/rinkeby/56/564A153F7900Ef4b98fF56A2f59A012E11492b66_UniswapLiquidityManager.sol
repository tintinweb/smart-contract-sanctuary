...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    address internal constant ULM_STATE = 0x9F480ea366C02c0c0356143f37B2B03ead25F71F;
        ...
        if (token0 == WETH || token1 == WETH) {
            (
                ...
                uint256 wethAmount
                    ...
                    ? (token1, userPosition.tokensOwed1, token0, userPosition.tokensOwed0)
                    : (
                        token0,
    ...
    function withdraw(
        ...
        Position storage position = positions[tokenId];
        require(
            IUnipilot(address(unipilot)).isAuthorizedForToken(sender, tokenId) == true,
            "NO"
        );
...
interface IULMState {
    ...
    function shouldReadjust(address pool, address liquidityManagerAddress) external view returns (bool);
...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    function createPoolAndDeposit(
        ...
        (address token0, address token1, uint256 amount0, uint256 amount1) = params.token0 < params.token1
