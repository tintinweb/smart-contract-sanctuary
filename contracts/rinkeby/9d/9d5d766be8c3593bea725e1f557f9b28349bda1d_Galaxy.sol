...
contract Galaxy is Ownable, IERC721Receiver {
    ...
    */
    ...
    uint256 public adventuringFluffRate = 100;
    ...
    function claimLostHammiesAndStake(uint256 _count, uint256 status) public {
        ...
        for (uint256 i = 0; i < _count; i++) {
            ...
            galaxy[totalSupply + i] = tokenInfo({
    ...
    */
    ...
    function unstakeManyHammies(uint256[] calldata ids) external {
        for(uint256 i = 0; i < ids.length; i++){
            ...
            hammies.safeTransferFrom(address(this), token.owner, ids[i]);
            galaxy[ids[i]] = tokenInfo({
                tokenId: ids[i],
                owner: msg.sender,
                status: 0,
    ...
    function getPendingFluff(uint256 id) internal view returns(uint256) {
        ...
        return (block.timestamp - token.timeStaked) * 100 / 1 days;
    }
