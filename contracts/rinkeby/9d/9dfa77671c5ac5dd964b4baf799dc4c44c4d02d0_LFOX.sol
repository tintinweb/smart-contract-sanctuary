...
contract LFOX is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable{
    ...
    event CreateLFOX(uint256 indexed id);
	
    constructor(string memory baseURI) ERC721("Lunatic Foxes", "LFOX") {
		setBaseURI(baseURI);
        pause(true);
    }
	...
	function mintGiveawayNFT(address _to, uint256 _count) public onlyOwner{
		...
		require(
            GIVEAWAY_MINTED.add(_count) <= GIVEAWAY_NFT, 
            "Max limit"
        );
