...
contract Crowd is Ownable, AllowanceCrowdsale{
    
    // HARD TOTAL CAP
    // PRICE
    // ALLOWANCE _approverWallet
    // REFUND
    ...
    **/
    enum PresaleStage { Stage1, Stage2 }
    PresaleStage public stage = PresaleStage.Stage1;
    ...
    **/
    ...
    // Address where the invested funds are collected.
    address payable _wallet,
    
    // The token being sold
    IERC20 _token,
    
    // Address of the wallet holding the token to be sold.
    address _approverWallet
    )
    Crowdsale(_rate,  _wallet, _token)
    AllowanceCrowdsale(_approverWallet)
    {
        
    }
    
    modifier isWhitelisted(address _beneficiary) {
      require(whitelist[_beneficiary], "You are not whitelisted.");
      _;
    }
    
    // Set the stage of the Crowdsale.
    function setCrowdsaleStage(uint _stage)
    public
    onlyOwner
    {
        if(uint(PresaleStage.Stage1) == _stage) {
            stage = PresaleStage.Stage1;
        } else if (uint(PresaleStage.Stage2) == _stage) {
            stage = PresaleStage.Stage2;
        }
    }
    ...
    view
    {
      if(stage == PresaleStage.Stage1) {
          _preValidatePurchaseStage1(_beneficiary, _weiAmount, _tokens);
      } else if (stage == PresaleStage.Stage2) {
          _preValidatePurchaseStage2(_beneficiary, _weiAmount, _tokens);
      }
    }
  
    function _preValidatePurchaseStage1(address _beneficiary, uint256 _weiAmount, uint256 _tokens)
    ...
    view
    {
        ...
        require(_tokens <= remainingTokens(), "cannot buy more than the inital supply of the crowdsale.");
    }
  
    function _preValidatePurchaseStage2(address _beneficiary, uint256 _weiAmount, uint256 _tokens)
    internal
    view
    {
        require(_beneficiary != address(0), "zero address");
...
contract Crowdsale is Context, ReentrancyGuard, Ownable {
    ...
    payable
    {
        require(onSale == true , "The crowdsale has ended.");
    ...
    public
    {
        ...
        uint256 _claimTokens = _balances[_beneficiary];
...
abstract contract AllowanceCrowdsale is Crowdsale {
    ...
    function remainingTokens() public view returns (uint256) {
        return Math.min(getToken().balanceOf(tokenWallet), getToken().allowance(tokenWallet, address(this)));
    }
    ...
    function _deliverTokens(address beneficiary, uint256 tokenAmount) internal virtual override {
        getToken().safeTransferFrom(tokenWallet, beneficiary, tokenAmount);
    }
}
