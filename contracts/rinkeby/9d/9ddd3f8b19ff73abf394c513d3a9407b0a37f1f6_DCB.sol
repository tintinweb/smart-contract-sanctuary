...
contract DCB {
   ...
   function preBuyTickt(uint256 amount) public returns(bool isSuccess){
       require(amount%1==0, 'Amount must be a integer');
       uint256 balance = Token(usdtToken).balanceOf(msg.sender);
    ...
    function buyTicket(uint256 amount, string memory _number) public{
      ...
      uint256 balance = Token(usdtToken).balanceOf(msg.sender);
      require(balance >= amount, 'Balance less than transaction amount');
      TransferHelper.safeTransferFrom(address(usdtToken),address(msg.sender), address(this), amount);
