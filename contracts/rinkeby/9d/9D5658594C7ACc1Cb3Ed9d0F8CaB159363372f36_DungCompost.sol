...
contract DungCompost is LPTokenWrapper, Ownable {
     ...
     *
     * To get Human readable percent - divide result by 10^16.
     * APY mean how much ETH you can earn, if you will stake
     * LP tokens with total price of 1 ETH if compost duration
     * will be 1 year and not be changed DUN price and staked LP
     * token count.
    */
    function apyEth() external view returns (uint) {
        ...
        uint256 totalSupplyLP = lptoken.totalSupply(); // wei

        uint dungPrice;  // wei
        uint ETHperLP; // 1 LP price in ETH currency (wei)
        if (lptoken.token0() == address(dung)) {
            dungPrice = (uint256)(reserve1) * 1 ether/reserve0;
            ETHperLP = 2 ether* reserve1/totalSupplyLP; // DUNG value + ETH value in 1 LP
        } else {
            dungPrice = (uint256)(reserve0) * 1 ether/reserve1;
            ETHperLP = 2 ether* reserve0/totalSupplyLP; // DUNG value + ETH value in 1 LP
        }

        uint stakedLPinETH = stakedLP*ETHperLP/1 ether; // total staked LP token count in ETH currency

        uint earnedDungPerYear = rewardRate * 365 days; // total pool earns per year (wei)
        uint earnedEthPerYear = earnedDungPerYear*dungPrice; // wei*wei
        uint earnerEthPerYearForStakedETH = earnedEthPerYear / stakedLPinETH; // wei

        return earnerEthPerYearForStakedETH * 10000 / 1 ether;
    }

    function apyEthDebug() external view returns (uint, uint, uint, uint, uint) {
        ...
        if (stakedLP == 0) {
            // no staked tokens - infinite APY
            return (0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff, 0,0,0,0);
        }
        ...
        return (earnerEthPerYearForStakedETH * 10000 / 1 ether, dungPrice, ETHperLP, stakedLPinETH, earnedEthPerYear);
    }
    ...
    */
    ...
    function apyDungDebug() external view returns (uint, uint, uint, uint) {
        ...
        if (stakedLP == 0) {
            // no staked tokens - infinite APY
            return (0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff, 0,0,0);
        }
        ...
        uint DungPerLP; // 1 LP price in ETH currency
        if (lptoken.token0() == address(dung)) {
            DungPerLP = 2 ether * (uint256)(reserve0)/totalSupplyLP; // DUNG value + ETH value in 1 LP (wei)
        } else {
            DungPerLP = 2 ether * (uint256)(reserve1)/totalSupplyLP; // DUNG value + ETH value in 1 LP (wei)
        }
        ...
        return (earnerDungPerYearForStakedDung, DungPerLP, stakedLpInDung, earnedDungPerYear);
    }
