...
contract BaboonBrigade is ERC721Enumerable, Ownable {
    ...
    uint256 public constant BBG_PREMINT = 777;
    ...
    uint256 public lastPresaleId; // last presale token id which already sold from 777
    ...
    string public tokenBaseURIPresale;
    string public tokenBaseURIPublic;
    ...
    function buy(uint256 tokenQuantity) external payable {
        require(lastTokenId < BBG_MAX, "OUT_OF_STOCK");
        require(tokenQuantity <= BBG_PER_MINT, "EXCEED_PER_MINT");
        require(_state == State.PUBLIC, "NOT_ON_PUBIC_SALE");
    ...
    /**
     * @notice sell preminted nfts
     * @param tokenQuantity total amount of nft to be transfer
     */
    function buyPremint(uint256 tokenQuantity) external payable {
        require(tokenQuantity <= BBG_PER_MINT, "EXCEED_PER_MINT");
        require(tokenQuantity <= BBG_PER_MINT, "EXCEED_PER_MINT");
        require(_state == State.PRESALE, "NOT_ON_PRE_SALE");
        require(
            BBG_PER_PRICE * tokenQuantity <= msg.value,
            "Not Enough payments"
        );
        require(
            lastPresaleId + tokenQuantity <= BBG_PREMINT,
            "EXCEED_PREMINT_AMOUNT"
        );
        for (uint256 i = 0; i < tokenQuantity; i++) {
            uint256 tokenId = lastPresaleId + 1;
            _safeTransfer(creatorAddress, msg.sender, tokenId, "");
            lastPresaleId++;
            companyAddress.transfer(msg.value);
            emit Presale(tokenId, msg.sender, BBG_PER_PRICE);
        }
    }

    /**
     * @notice transfer preminted nfts as gift by admin
     * @param to to address
     * @param tokenQuantity token amount to transfer as gift
     */
    function premintTransfer(address to, uint256 tokenQuantity)
        external
        onlyOwner
    {
        require(
        ...
        for (uint256 i = 0; i < tokenQuantity; i++) {
            ...
            lastPresaleId++;
            emit Presale(tokenId, to, 0);
        }
    }

    /**
     * @notice premint by owner
     */
    function premint() external onlyOwner {
        require(lastTokenId < BBG_PREMINT, "PREMINT_PROCEEDED");
        for (uint256 i = 0; i < BBG_PREMINT; i++) {
            uint256 tokenId = lastTokenId + 1;
            _premint(creatorAddress, tokenId);
            lastTokenId++;
        }
    }
        ...
        returns (string memory)
    {
        ...
        string memory baseUri = tokenBaseURI;
        if (tokenId > BBG_PREMINT) {
            baseUri = tokenBaseURIPublic;
        }
    ...
    /**
     * @notice set public sale metadata uri
     * @param _tokenBaseURI token ID which need to be finished
     */
    function setTokenBaseURIPublic(string memory _tokenBaseURI)
        public
        onlyOwner
    {
        tokenBaseURIPublic = _tokenBaseURI;
    }
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
	...
	function _premint(address to, uint256 tokenId) internal {
		_balances[to] += 1;
		_owners[tokenId] = to;
