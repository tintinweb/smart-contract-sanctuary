...
contract Sample is ERC1155Supply, Ownable, Pausable {
    ...
    uint256 public constant MAX_TOKENS_PLATINUM_PUBLIC = 1900;

    bool public saleIsActivePlatinum = false;
    bool public saleIsActivePlatinumWhitelist = false;
    bool public saleIsActivePlatinumFree = false;
    bool public saleIsActiveCenturion = false;

    // Used to validate authorized mint addresses
    address private signerAddress = 0x4a9Db6E418426AAa1C65602df501F4c35559BC3e;
    ...
    mapping (address => bool) public hasAddressMintedPlatinumWhitelist;
    mapping (address => bool) public hasAddressMintedPlatinumFree;
    mapping (address => uint256) public hasAddressMintedPlatinum;
    mapping (address => uint256) public hasAddressMintedCenturion;
    ...
    function setPlatinumWhitelistSaleState(bool newState) public onlyOwner {
        require(saleIsActivePlatinumWhitelist != newState, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
        saleIsActivePlatinumWhitelist = newState;
    }
    ...
    /**
     * @notice Allow minting of Platinum tokens by anyone while the sale is active max 3 per wallet
     */
    function mintPlatinum(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply(TOKEN_ID_PLATINUM) < MAX_TOKENS_PLATINUM_PUBLIC, "MAX_TOKEN_SUPPLY_REACHED");
        require(hasAddressMintedPlatinum[msg.sender] < 3, "ADDRESS_HAS_ALREADY_MINTED_MAX_NUMBER_OF_PLATINUM_TOKENS");
        require(hasAddressMintedPlatinum[msg.sender] + numberOfTokens < 4, "WILL_EXCEED_MAX_NUMBER_PER_WALLET_ON_PURCHASE");
        require(numberOfTokens > 0, "MUST_MINT_AT_LEAST_ONE_TOKEN");
        require(numberOfTokens < 4, "CANT_MINT_MORE_THAN_THREE_TOKENS");
        
        hasAddressMintedPlatinum[msg.sender] = hasAddressMintedPlatinum[msg.sender] + 1;
        
        _mint(msg.sender, TOKEN_ID_PLATINUM, numberOfTokens, "");

        if (totalSupply(TOKEN_ID_PLATINUM) >= MAX_TOKENS_PLATINUM_PUBLIC) {
    ...
    function mintPlatinumFree(bytes32 messageHash, bytes calldata signature) external payable {
        ...
        require(!collectionMinted[TOKEN_ID_PLATINUM], "PLATINUM_TOKEN_LOCKED");
        require(totalSupply(TOKEN_ID_PLATINUM) < MAX_TOKENS_PLATINUM, "MAX_TOKEN_SUPPLY_REACHED");
    ...
    /**
     * @notice Allow minting of a single Platinum token by whitelisted addresses only
     */
    function mintPlatinumWhitelist(bytes32 messageHash, bytes calldata signature) external payable {
        require(saleIsActivePlatinumWhitelist, "SALE_NOT_ACTIVE");
        require(!collectionMinted[TOKEN_ID_PLATINUM], "PLATINUM_TOKEN_LOCKED");
        require(TOKEN_PRICE_PLATINUM == msg.value, "PRICE_WAS_INCORRECT");
        require(totalSupply(TOKEN_ID_PLATINUM) < MAX_TOKENS_PLATINUM, "MAX_TOKEN_SUPPLY_REACHED");
        require(hasAddressMintedPlatinumWhitelist[msg.sender] == false, "ADDRESS_HAS_ALREADY_MINTED_PLATINUM_WHITELIST");
        require(hashMessage(msg.sender) == messageHash, "MESSAGE_INVALID");
        require(verifyAddressSigner(messageHash, signature), "SIGNATURE_VALIDATION_FAILED");
        
        hasAddressMintedPlatinumWhitelist[msg.sender] = true;
    ...
    /**
     * @notice Allow minting of Centurion tokens by anyone while the sale is active max 3 per wallet
     */
    function mintCenturion(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply(TOKEN_ID_CENTURION) < MAX_TOKENS_CENTURION, "MAX_TOKEN_SUPPLY_REACHED");
        require(hasAddressMintedCenturion[msg.sender] < 3, "ADDRESS_HAS_ALREADY_MINTED_MAX_NUMBER_OF_CENTURION_TOKEN");
        require(hasAddressMintedPlatinum[msg.sender] + numberOfTokens < 4, "WILL_EXCEED_MAX_NUMBER_PER_WALLET_ON_PURCHASE");
        require(numberOfTokens > 0, "MUST_MINT_AT_LEAST_ONE_TOKEN");
        require(numberOfTokens < 4, "CANT_MINT_MORE_THAN_THREE_TOKENS");

        hasAddressMintedCenturion[msg.sender] = hasAddressMintedCenturion[msg.sender] + 1;
