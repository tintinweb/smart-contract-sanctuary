...
// File contracts/CryptoGasStationNFP.sol
...
// File contracts/BulkNFPTransfer.sol

pragma solidity ^0.8.0;

contract BulkNFPTransfer is Ownable {
    CryptoGasStationNFP public nfpContract;
    mapping(address => bool) public sent;

    constructor(CryptoGasStationNFP _nfpContract) {
        nfpContract = _nfpContract;
    }

    function transfer(address[] memory addresses) external onlyOwner {
        require(
            nfpContract.isApprovedForAll(msg.sender, address(this)),
            'Requires access to nft assets'
        );
        require(
            nfpContract.balanceOf(msg.sender) >= addresses.length,
            'Not enough nft assets to cover'
        );

        uint256[] memory nfps = nfpContract.getNFPsOfOwner(msg.sender);

        uint256 t = 0;

        for (uint256 i = 0; i < addresses.length; i++) {
            if (sent[addresses[i]]) continue;

            nfpContract.safeTransferFrom(msg.sender, addresses[i], nfps[t++]);
            sent[addresses[i]] = true;
        }
    }
}