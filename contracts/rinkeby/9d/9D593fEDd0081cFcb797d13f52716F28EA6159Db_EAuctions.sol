...
contract EAuctions is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    ...
    function create(address[] calldata _nftAddresses,uint[] calldata _nftIds,uint32[] calldata _nftTypes,uint _price,uint _tax, address _currency) external {
        ...
        transferTokens(msg.sender,address(this),_nftAddresses,_nftIds,_nftTypes,false,false);
    ...
    function cancel(uint auctionId) external isAccesible(auctionId) nonReentrant {
        ...
        transferTokens(address(this),msg.sender,auctions[auctionId].nftAddresses,auctions[auctionId].nftIds,auctions[auctionId].nftTypes,false,false);
    ...
    function finish(uint auctionId) external nonReentrant {
        ...
        // Transfer items from BAuctions to owner
        transferTokens(address(this),auctions[auctionId].winner,auctions[auctionId].nftAddresses,auctions[auctionId].nftIds,auctions[auctionId].nftTypes,false,false);
...
contract TokensHandler {
    ...
    function transferTokens(
        ...
        bool approveBeforeTransferFrom,
        bool approveBeforeTransferTo
    ) public {
        ...
        for(uint i = 0; i < length; ++i) {
            if ( nftTypes[i] == 0 ) {
                if ( approveBeforeTransferFrom ) {
                    IERC721(nftAddresses[i]).approve(from,nftIds[i]);
                }
                if ( approveBeforeTransferTo ) {
                    IERC721(nftAddresses[i]).approve(to,nftIds[i]);
                }
                IERC721(nftAddresses[i]).safeTransferFrom(from,to,nftIds[i]);
            } else {
                if ( approveBeforeTransferFrom ) {
                    IERC1155(nftAddresses[i]).setApprovalForAll(from,true);
                }
                if ( approveBeforeTransferTo ) {
                    IERC1155(nftAddresses[i]).setApprovalForAll(to,true);
                }
