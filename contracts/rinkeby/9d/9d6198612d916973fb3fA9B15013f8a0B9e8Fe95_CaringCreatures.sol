...
contract CaringCreatures is ERC721, Ownable {
    ...
    function mintShow(uint256 amount) public payable returns (uint256) {
        require(msg.sender == tx.origin, "Reverted");
        require(publicsaleActive, "Public sale is not active");
        require(_totalSupply + amount <= _maxSupply, "Can not mint more than max supply");
        require(amount > 0 && amount <= 11, "You can mint between 1 and 11 at once");
        require(msg.value >= amount * price, "Insufficient payment");

        if (_limitedSale) {
            require(_totalSupply + amount <= _limitedSaleLimit, "Can not mint more than limited supply");
        }
