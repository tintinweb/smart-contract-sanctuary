...
contract saturnaNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    constructor (address _v1NFT, address _sat, address _fee) ERC721("Saturna NFT" , "SATURNA"){
        ...
        {
            _supportedTokens.add(address(0x0));  // BNB
            _supportedTokens.add(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // BUSD
            _supportedTokens.add(_sat); // SAT
        }
    ...
    function createItem(
        ...
        
        {
            ...
            _condition.forOnlyV1Holder = isAdmin[msg.sender] ? _forOnlyV1Holder : false;
            _condition.v1HolderDiscount = isAdmin[msg.sender] ? _v1HolderDiscount : 0;
    ...
    function importItem(
        ...
        require(_price > 0, "invalid price");
        require(isSupportedToken(_currency), "invalid payment currency");
        require(ERC721(_collection).ownerOf(_tokenId) == msg.sender, "not owner");
        require(_v1HolderDiscount < PERCENTS_DIVIDER, "too big discount");
    ...
    function buyItem(uint256 _id, uint256 _amount) external payable nonReentrant {
        ...
        require(item.currency != address(0x0) || _amount == msg.value, "invalid eth value");
    ...
    function placeBid(uint256 _id, uint256 _amount) external payable nonReentrant {
        ...
        if(item.currency != address(0x0)) {
            ...
            require(IERC20(item.currency).transferFrom(_msgSender(), address(this), _amount), "transfer payment token failed");
        }
    ...
    */
    function acceptWinningBid(uint256 _id) external onlyItemCreator(_id) nonReentrant {
        ...
        if(bidsLength == 0) {
            ...
            // Process the payment
            _processPayment(_id, address(this), _lastBid.amount);
    ...
    /**
     * Check msg.sender can buy this item
     */
    function _checkCanBuy (uint256 _id) internal view {
        ...
        if(!item.mintable) {
            require(IERC721(item.collection).ownerOf(item.tokenId) == msg.sender, "insufficient balance");
        }

        require(item.condition.requiredSAT == 0 
            || item.condition.requiredSAT <= IERC20(info.satTokenAddress).balanceOf(msg.sender), "insufficient $SAT Balance");
        require(!item.condition.forOnlyV1Holder 
            || IERC721(info.v1NFTAddress).balanceOf(msg.sender) > 0, "require V1 NFT token");
    }

    function _processPayment(uint256 _id, address buyer, uint256 _amount) internal {
        Item memory item = items[_id];
        
        uint256 _total = _amount.mul(PERCENTS_DIVIDER.sub(item.condition.v1HolderDiscount)).div(PERCENTS_DIVIDER);
    ...
    function _refundLastBid(uint256 _id) internal {
        ...
        if(bidsLength > 0) {
            ...
            if (_lastBid.isActive && _lastBid.amount > 0 && _lastBid.bidder != address(0)) {
                _payoutUser(_lastBid.bidder, item.currency, _lastBid.amount);
