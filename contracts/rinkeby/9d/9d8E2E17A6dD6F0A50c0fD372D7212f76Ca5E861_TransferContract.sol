...
contract TransferContract {
    ...
    function swap(address _owner2, uint256 _amount1) public {
        ...
        require(
            CatTokenOld.balanceOf(owner1) >= amount1,
            "Balance not high enough for Old Cat Token Holder"
        );

        require(
            CatTokenOld.allowance(owner1, address(this)) >= amount1,
            "Token allowance too low for Cat Token owner 1"
        );

        require(
            CatTokenNew.balanceOf(owner2) >= amount2,
            "Balance not high enough for New Cat Token Holder"
        );

        require(
            CatTokenNew.allowance(owner2, address(this)) >= amount2,
            "Allowance not high enough for New Cat Token Holder"
        );

        // transfer tokens
        _safeTransferFrom(CatTokenOld, owner1, owner2, amount1);
        _safeTransferFrom(CatTokenNew, owner2, owner1, amount2);
    }
    ...
    function owner_approve_allowance(uint256 _approvalAmount) public {
        ...
        CatTokenNew.approve(owner1, _approvalAmount);
    }
}