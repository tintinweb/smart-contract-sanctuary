...
contract Land is ERC721, ERC721URIStorage, IERC721Receiver, AccessControl {
    ...
    function grantContractRole(address user) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(CONTRACT_ROLE, user);
    }
    // ============
    ...
    function setZoneTokenURI(uint256 zoneId, string memory uri) public onlyRole(MANAGER_ROLE) {
    ...
    function premint(uint256 zoneId, uint256 x, uint256 y, LandType landType) public onlyRole(MINTER_ROLE) uniqueLand(zoneId, x, y) {
        ...
        require(_landStorage != address(0), "Land Storage is not found");
        ...
        _lands[zoneId].push(LandMetadata({
            x: x,
            y: y,
            landType: landType,
            tokenId: tokenId
        ...
        if(!_isZoneExists(zoneId)) _zones.push(zoneId);
    ...
    function setLandStorage(address newStorage) public onlyRole(MANAGER_ROLE) {
        _landStorage = newStorage;
    }

    function withdraw(address account, uint256 tokenId) public onlyRole(CONTRACT_ROLE) {
        (uint256 zoneId, LandMetadata memory landData) = _getLandByTokenId(tokenId);
        require(landData.landType == LandType.AVAILABLE, "Land is not available");

        safeTransferFrom(_landStorage, account, tokenId);
        _setLandType(zoneId, landData.x, landData.y, LandType.UNAVAILABLE);
    }
