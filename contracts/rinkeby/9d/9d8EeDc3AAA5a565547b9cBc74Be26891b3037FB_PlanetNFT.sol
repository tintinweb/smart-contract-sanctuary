...
contract PlanetNFT is ERC721Tradable {
    ...
    function create_random(uint256 tokenId,
        ...
        uint256 seedfinal=seed_+block.difficulty;
        uint16 size=10+uint16(randomNum(90, seedfinal, block.timestamp));

        int16 positionX=int16(600-size)-int16(uint16(randomNum((600-size)*2, seedfinal, block.timestamp)));
        int16 positionY=int16(600-size)-int16(uint16(randomNum((600-size)*2, seedfinal, block.timestamp)))-100;

        uint16 rotationX=100+uint16(randomNum(400, seedfinal, block.timestamp));
        uint16 rotationY=100+uint16(randomNum(400, seedfinal, block.timestamp));
        uint16 rotationAngle=uint16(randomNum(360, seedfinal, block.timestamp));
        uint16 rotationDuration=5+uint16(randomNum(55, seedfinal, block.timestamp));
    ...
    function create(
        ...
        //mintTo(msg.sender);
        require(_values[newTokenId].length==0, "Planet: Token already exists.");
            ...
            string(
                abi.encodePacked(
                    ...
                    abi.encodePacked(
                        ...
                        '" stroke="#000" stroke-width="2" fill="url(#planet_color_',
                        planet.id.toString()
                    ),
                    abi.encodePacked(
                        ')" id="planet_',
