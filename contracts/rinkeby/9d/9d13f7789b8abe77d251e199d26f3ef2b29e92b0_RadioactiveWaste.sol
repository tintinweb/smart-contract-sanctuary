...
//free Rwaste for anyone to test the kaijus with
...
contract RadioactiveWaste is ERC20, Ownable {
    ...
    constructor() ERC20("RadioactiveWaste", "RWASTE"){
        START = block.timestamp;
    }

    function setKaijuAdress(address kaijuAddress) public onlyOwner {
    ...
    // !ooh
    function freeRwasteForTestPurposes(address _address, uint256 _amount) external {
        _mint(_address, _amount);
    }

    function burn(address user, uint256 amount) external {
        require(msg.sender == address(KaijuKingz), "Address does not have permission to burn");
        _burn(user, amount);
    }
