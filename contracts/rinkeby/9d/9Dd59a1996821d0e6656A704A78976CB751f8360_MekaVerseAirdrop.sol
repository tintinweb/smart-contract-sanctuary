...
contract MekaVerseAirdrop is ERC1155, Ownable, MekaVerseAllowed {
    ...
    // TODO Ajouter un moyen de détecter la notion de claim automatique sur le contrat.
    ...
    // Permet de gérer plusieur collection en même temps.
    mapping(uint256 => ClaimCollection) public collections;
    mapping(uint256 => uint256) public currentSupply;

    uint256 public collectionIndex;
    // collection => tokenId = utilisé <= Collection type !canExternal
    ...
    //                     Claim                            //
    //******************************************************//
    // I can claim my token for someone else if i want (send a gift for example)
    function claim(address _to, uint256 _collectionId, uint256[] memory _tokensId) public collectionIsOpen(_collectionId) {
        ...
        for (uint256 i = 0; i < _tokensId.length; i++) {
            ...
            // On vérifie qu'il n'a pas déjà claim son token.
            require(canClaim(_collectionId, _tokenId), "Token already used");
            // On s'assure que c'est son token.
    ...
    // L'idée est d'avoir des contrats externes qui leur permettra de mint.
