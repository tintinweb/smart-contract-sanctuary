...
contract OpenerV2 is Ownable {
    TransferFromAndBurnFrom private _pmonToken;
    ISoftMinter private _softMinter;
    ...
    uint256 private _decimalMultiplier = 10**uint256(18);
    ...
    function openBooster(uint256 amount) public {
        ...
        bytes32 softMintHash =
            _getSoftMintHashByOpenedBoosters(
                amount,
                _openedBoosters,
                msg.sender
            );

        _softMinter.addHash(msg.sender, softMintHash);
        emit Opening(msg.sender, amount, _openedBoosters);
        _openedBoosters = _openedBoosters + (amount / _decimalMultiplier);
    }

    function _getSoftMintHashByOpenedBoosters(
        uint256 amount,
        uint256 openedBoosters,
        address userAddress
    ) private returns (bytes32) {
        uint256[] memory ids = new uint256[]((amount / _decimalMultiplier) * 3);
        ...
        console.logBytes32((keccak256(abi.encode(userAddress, ids))));

        return keccak256(abi.encode(userAddress, ids));
    }
