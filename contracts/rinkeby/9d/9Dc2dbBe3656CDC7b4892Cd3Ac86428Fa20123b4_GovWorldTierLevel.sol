...
contract GovWorldTierLevel is IGovWorldTierLevel {
    ...
    constructor(
        ...
        _addTierLevel(_bronze, TierData(15000e18, 30, false, false, true, false, true, false, false, false));
        _addTierLevel(_silver, TierData(30000e18, 40, false, false, true, true, true, false, false, false));
        _addTierLevel(_gold, TierData(75000e18, 50, true, true, true, true, true, true, true, true));
        _addTierLevel(_platinum, TierData(150000e18, 70, true, true, true, true, true, true, true, true));
        _addTierLevel(_allStar, TierData(300000e18, 70, true, true, true, true, true, true, true, true));
    }
        ...
        onlyEditTierLevelRole(msg.sender)
    {
        ...
        require(_tierData.govHoldings > tierLevels[allTierLevelKeys[maxGovTierLevel()]].govHoldings, "GovHolding Should be greater then last tier level Gov Holdings");
    ...
    onlyEditTierLevelRole(msg.sender)
    {
        require(_tierLevelKeys.length == _newTierData.length, "New Tier Keys and TierData length must be equal");
    ...
    function _updateTierLevel(
        ...
        if(currentIndex > 0){
            lowerLimit = tierLevels[allTierLevelKeys[currentIndex-1]].govHoldings;
        }
        if(currentIndex < allTierLevelKeys.length-1)
            upperLimit = tierLevels[allTierLevelKeys[currentIndex+1]].govHoldings;
            
        require(_newTierData.govHoldings < upperLimit && 
            _newTierData.govHoldings > lowerLimit, "Gov Holding Should be in range of previous and next tier level");
     ...
     */
    function _saveTierLevel(bytes32[] memory _tierLevelKeys, TierData[] memory _newTierData)
    internal
    {
        for(uint i = 0; i < _tierLevelKeys.length; i++) {
        ...
        returns (TierData memory _tierData)
    {
        ...
        for (uint256 i = 1; i < allTierLevelKeys.length; i++) {
            ...
            } else if(userGovBalance >=  tierLevels[allTierLevelKeys[allTierLevelKeys.length -1]].govHoldings) {
                return tierLevels[allTierLevelKeys[allTierLevelKeys.length -1]];
            }
        }
    }
...
contract GovWorldAdminRegistry is GovWorldAdminBase {
        ...
        onlyEditGovAdminRole(msg.sender)
    {
        ...
        require(pendingEditAdminRoles[_admin].editGovAdmin,
            "GAR: Non Pending admin can not be approved for edit."
        );
...
contract GovWorldAdminBase is Ownable {
    ...
    event RemoveAdminApproved(address indexed _admin,address indexed _removedBy);
    event EditAdminApproved(address _admin,address _editedBy);
    event AdminEdited(address _admin);
