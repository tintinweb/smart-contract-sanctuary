...
contract PurgeGameSeason1Alphaaa1 is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function mint(
     ...
     if (publicSaleStatus == true) {
            require(
                _number > 0 && _number <= 10, // mint limit per tx
                "Maximum of 10 mints allowed per transaction"
            );
    } else {
        require(
...
function uint2str(uint _i) internal pure returns (string memory _uintAsString) {
    ...
    function devburn() public onlyOwner {
        uint deadcheck = randomtoken();
        _burn(deadcheck);
