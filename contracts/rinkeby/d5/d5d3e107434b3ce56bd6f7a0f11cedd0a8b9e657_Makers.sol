...
// contracts/Makers.sol
// SPDX-License-Identifier: MIT

pragma solidity ^0.8.0;

library MakersLibrary {
...
// File: contracts/ISpice.sol

// contracts/ISpice.sol
...
contract Makers is ERC721Enumerable {
    /*
...
*/
    ...
    constructor() ERC721("Makers", "MAKERS") {
        _owner = msg.sender;
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    function hash(
        ...
        //The last 8 digits are random, the first is 0, due to the maker not being burned.
    ...
    /**
     * @dev Returns the current spice cost of minting.
     */
    function currentSpiceCost() public view returns (uint256) {
    ...
    function mintInternal() internal {
        ...
        require(!MakersLibrary.isContract(msg.sender));
     ...
     */
    function mintMaker() public {
        if (totalSupply() < MINTS_PER_TIER) return mintInternal();

        //Burn this much spice
        ISpice(spiceAddress).burnFrom(msg.sender, currentSpiceCost());
...
*/
        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) {
            uint8 thisTraitIndex = MakersLibrary.parseInt(
                MakersLibrary.substring(_hash, i, i + 1)
            );
            ...
            for (
                ...
                string memory thisPixel = MakersLibrary.substring(
                ...
                uint8 x = letterToNumber(
                    MakersLibrary.substring(thisPixel, 0, 1)
                );
                uint8 y = letterToNumber(
                    MakersLibrary.substring(thisPixel, 1, 2)
                );
                ...
                svgString = string(
                    abi.encodePacked(
                        svgString,
                        "<rect class='c",
                        MakersLibrary.substring(thisPixel, 2, 4),
                        "' x='",
                        x.toString(),
                        "' y='",
                        y.toString(),
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    MakersLibrary.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "Makers #',
                                    MakersLibrary.toString(_tokenId),
                                    '", "description": "Makers is a collection of 10,000 unique makers. All the metadata and images are generated and stored 100% on-chain. No IPFS, no API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
                                    MakersLibrary.encode(
        ...
        returns (string memory)
    {
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
            tokenHash = string(
                abi.encodePacked(
                    "1",
                    MakersLibrary.substring(tokenHash, 1, 9)
                )
            );
        }
...
|     | \      / |  |  ||     ||  .  \    |  |   |     ||  |  \     | |  |   |  ||     ||  |  |\    |
 \___/   \_/\_/  |__|__||_____||__|\_|    |__|    \__,_||__|__|\____| |__|  |____|\___/ |__|__| \___|
    ...
    */
    ...
    /**
     * @dev Sets the spice ERC20 address
     * @param _spiceAddress The spice address
     */

    function setSpirceAddress(address _spiceAddress) public onlyOwner {
