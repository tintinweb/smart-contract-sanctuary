...
contract Graveyard is ERC721Enumerable {
     ...
     */
    function unstakeCatsFromGrave(uint256 graveId) public {
        require(
    ...
    function unstakeAndMove(uint256 fromGraveId, uint256 toGraveId) public {
        require(
    ...
    function claimZombies(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                tokenIdToTargetTime[tokenIds[i]] < block.timestamp
            ...
            IZombie(zombiesAddress).mintZombie(
                msg.sender,
                _tokenIdToHash(tokenIds[i])
            );

            tokenIdToYield[tokenIds[i]] = true;
