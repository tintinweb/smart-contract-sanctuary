...
library GraveyardLibrary {
    ...
    function currentMieceCost(uint256 _totalSupply) public pure returns (uint256) {
        ...
        if (_totalSupply < 1000) 
            return 1000000000000000000; // FIXME: Testing purpose

        if (_totalSupply > 1000 && _totalSupply <= 2000)
            return 1000000000000000000;

        if (_totalSupply > 2000 && _totalSupply <= 3000)
        ...
        returns (string memory)
    {
        ...
        svgString = string(
            abi.encodePacked(
                '<svg id="c" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 24 24" style="background-color:#2A2A2A"> ',
                svgString,
...
contract Graveyard is ERC721Enumerable {
    ...
    // Default digging time is a day. This gets multipled. 
