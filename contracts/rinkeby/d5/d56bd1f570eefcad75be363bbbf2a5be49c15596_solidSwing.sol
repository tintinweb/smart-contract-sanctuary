...
contract solidSwing is owned {
    ...
    event check(uint z, uint a, uint b);
    function snapShot(uint _spotPrice) public onlySigner returns(bool)
    {
      ...
      if(_spotPrice >= benchMarkValue)
      {
        ...
        reBaseFactor = benchMarkValue + diff;
        currentPrice = benchMarkValue - (benchMarkValue * diff / base);
        emit check(1,currentPrice, benchMarkValue * diff / base );
      }
      else
      {
        ...
        reBaseFactor = benchMarkValue - diff;
        currentPrice = benchMarkValue + (benchMarkValue * diff / base);
        emit check(2,currentPrice, benchMarkValue * diff / base );
      }
      snapTime = now;
      return true;
    } 
