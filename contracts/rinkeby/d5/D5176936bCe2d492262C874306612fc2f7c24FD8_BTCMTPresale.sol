...
contract BTCMTPresale is Ownable {

    uint256[5] public BONUS_BREAKPOINTS = [4,13,26,52,104];
    uint256[5] public BONUS_PERCENTAGES = [2,4,8,10,15];
    IBTCMTForPresale public immutable BTCMT;
    IERC20 public immutable USDT;
    uint256 public immutable USDT_DECIMALS_MINUS_ONE;

    uint256 public capToSell;
    uint256 public totalSold;

    constructor(address _BTCMT, address _USDT, uint256 _capToSell, uint256 _USDT_DECIMALS) {
        BTCMT = IBTCMTForPresale(_BTCMT);
        USDT = IERC20(_USDT);
        capToSell = _capToSell;
        USDT_DECIMALS_MINUS_ONE = _USDT_DECIMALS - 1;
    }
    ...
    function buy(uint256 amount, uint256 timeInWeeks) external {
        USDT.transferFrom(_msgSender(), owner(), (amount * (15 * (10 ** USDT_DECIMALS_MINUS_ONE))) / (10 ** 18));
        if (timeInWeeks > 0) {
