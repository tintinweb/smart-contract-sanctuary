...
/**
 * @title DickwadAndSprinkles
 * DickwadAndSprinkles - a contract for the DickwadAndSprinkles
 */
contract DickwadAndSprinkles is ERC721Base {
    ...
    uint256 constant basePrice = 80000000000000000; //0.08 ETH
    uint constant maxTokenPurchase = 10;
    uint constant maxWhitelistPurchase = 2;

    // whitelist features
    bool public whitelistSaleActive = false;
    uint256 public whitelistMinted;
    uint256 constant whitelistReserved = 1000;
    ...
    constructor()
        ERC721Base("Dickwad And Sprinkles", "DANDS")
    {
    }
    ...
    // get and set the public sale state
    function setWhitelistSale(bool _setWhitelistSaleState) virtual public onlyOwner{
        whitelistSaleActive = _setWhitelistSaleState;
    }
    ...
    function mintWhitelist(uint256 amount) external payable {
        require(amount != 0, "INVALID_AMOUNT");
        require(whitelistSaleActive, "WHITELIST_DISABLED");
        require(isWhitelisted[msg.sender], "NOT_WHITELISTED");
        require(amount * basePrice <= msg.value, "WRONG_ETH_AMOUNT");

        uint256 userMintedAmount = whitelistMintedAmounts[msg.sender] + amount;
        require(userMintedAmount <= maxWhitelistPurchase, "MAX_EXCEEDED");

        uint256 newSupply = whitelistMinted + amount;
        require(newSupply <= whitelistReserved, "MAX_WHITELIST_EXCEEDED");
