    ...
    ERC721BurnableUpgradeable
{
    ...
    function initialize() public initializer  {
        __Ownable_init();
        ERC721Upgradeable.__ERC721_init("NFT721", "NFT721");
        _tid = 1;
    ...
    function newtokenasbid(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal {
        ...
        require( approveValue >= (_adminfee.add(roy.add(netamount.add(char)))).div(10**Tokendecimals), "Insufficient Balance"); 
        if(_adminfee != 0){
        ...
        if(_charity[tokenId] != 0){
            t.transferFrom(msg.sender, _charityAddress, char.div(10**Tokendecimals));
        }
        if(netamount !=0){
    ...
    function _acceptBId(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        if(_charity[tokenId] != 0){
            t.transferFrom(from, _charityAddress, char.div(10**Tokendecimals));
        }
        if(netamount!=0){
