...
contract FishingDock {
  ...
  mapping(uint256 => uint256) public featureBonus;
  ...
  function create(uint256 _realmId) external {
    ...
    if (totalDocks[_realmId] == 0) {
      storeFeatureBonus(_realmId);
      storeLimit(_realmId);
    }
    ...
    docks[_realmId][_id].id = _id;
    docks[_realmId][_id].realmId = _realmId;
    docks[_realmId][_id].food = _rarity(_realmId, dataProbability);
    docks[_realmId][_id].workforce = _rarity(
      docks[_realmId][_id].food,
      dataProbability
    );
    docks[_realmId][_id].resourceId = _resourceId;
  ...
  function createTest(uint256 _realmId) external {
    address _owner = REALM.ownerOf(_realmId);

    require(_owner == msg.sender || REALM.isApprovedForAll(_owner, msg.sender));

    uint256 _id = totalDocks[_realmId];
    uint256 _resourceId = RARITY.rarity(_realmId, resourceProbability);
    ...
    docks[_realmId][_id].id = _id;
    docks[_realmId][_id].realmId = _realmId;
    docks[_realmId][_id].food = RARITY.rarity(_resourceId, dataProbability);
    docks[_realmId][_id].workforce = RARITY.rarity(
      docks[_realmId][_id].food,
      dataProbability
    );
    docks[_realmId][_id].resourceId = _resourceId;
  ...
  function collect(uint256 _realmId) external {
    ...
    uint256 _resource = 1 + featureBonus[_realmId];
  ...
  function storeLimit(uint256 _realmId) internal {
    ...
    limit[_realmId] = LIMIT + _bonus;
  }

  function storeFeatureBonus(uint256 _realmId) internal {
    uint256 _feature1 = REALM.realmFeatures(_realmId, 0);
    uint256 _feature2 = REALM.realmFeatures(_realmId, 1);
    uint256 _feature3 = REALM.realmFeatures(_realmId, 2);
    uint256 _bonus;
