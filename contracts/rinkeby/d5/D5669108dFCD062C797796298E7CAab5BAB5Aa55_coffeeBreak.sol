...
contract coffeeBreak is Ownable {
    ...
    mapping (address => uint) nomineeTeams;
...
// TEAM MANAGEMENT
    ...
    function nominateMember(address _newAddress) public { 
        require(nomineeTeams[_newAddress] != 0, "Already nominated for another team.");
        nominees.push(_newAddress);
        nomineeTeams[_newAddress] = teamIdByAddress(msg.sender) + 1; // add one so we can test ifExists
    ...
    function mergeMember(address _newAddress) public {
        ...
        for (uint i = 0;i < teams[_teamId].members.length;i++){
            if (nomineeApprovals[_newAddress][teams[_teamId].members[i]] == false)
                allApproved = false;
        }
...
// VIEW FUNCTIONS
    ...
    function getNomineeTeam(address _nominee) public view returns (uint){
        return nomineeTeams[_nominee] - 1;
    }
