...
contract OKPC is Insightful721, IERC2981, ReentrancyGuard {
  ...
  function _publicMint() private {
    ...
    if (_mintCount <= 128) artId = ((_mintCount - 1) % 128) + 1;
    else
      artId =
        (uint256(keccak256(abi.encodePacked('ART', _mintCount))) % 128) +
        1;
  ...
  function _reservedMint() private {
    ...
    if (pcId <= 128) artId = ((pcId - 1) % 128) + 1;
    else artId = (uint256(keccak256(abi.encodePacked('ART', pcId))) % 128) + 1;
...
contract OKPCMetadata is IOKPCMetadata {
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            _draw(tokenId, amount, art, parts),
            '", "attributes": ',
            abi.encodePacked(
  ...
  function draw(
    ...
    IOKPCParts.Parts memory parts = _parts.getParts(tokenId);
    return _draw(tokenId, insight, art, parts);
  }
