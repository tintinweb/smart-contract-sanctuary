...
contract NFTAuction is Ownable, ReentrancyGuard {
    ...
    uint defaultBidIncreasePercentage;
    uint defaultAuctionBidPeriod ; 
    uint minimumSettableIncreasePercentage;
    uint maximumMinPricePercentage ;
    ...
    function _doesBidMeetBidRequirements(
        ...
        uint256 bidIncreaseAmount = (nftContractAuctions[_nftContractAddress][
            _tokenId
        ].nftHighestBid.mul(10000 +
                _getBidIncreasePercentage(_nftContractAddress, _tokenId))).div(10000);
...
/*
    ...
    // The owner can settle the auction if the seller isn't doing after the 
    // auction period has ended. There will be a penalty of the 10% of highestBiddingPrice
    // that will be take from seller. 
        ...
        isAuctionOver(_nftContractAddress, _tokenId)
    {
        ...
        // Update the highestBid  to 90% of it's original, meaning 10% is cut as a penalty.
        nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid=
        (9 * nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid )/10;
        _transferNftAndPaySeller(_nftContractAddress, _tokenId);
    }
