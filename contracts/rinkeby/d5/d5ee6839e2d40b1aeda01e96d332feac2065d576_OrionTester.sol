    ...
    PaymentSplitter
{
    ...
    uint8 public PUBLIC_MINT_LIMIT = 10;
    uint8 public PRESALE_MINT_LIMIT = 2;
    uint16 public MAX_SUPPLY = 100;
    uint128 public PRICE = 0.001 ether;
    ...
    constructor(
        ...
        ERC721S("Os Testadores de NFT", "TESTERS", proxyRegistryAddress)
        PaymentSplitter(_payees, _shares)
    {
        baseTokenURI = _baseTokenURI;
        notRevealedTokenURI = _notRevealedTokenURI;
        payees = _payees;
        _initializeEIP712("Os Testadores de NFT");
        _owners.push();
    }
    ...
    function presaleMint(
        ...
        require(addressMintedBalance[msg.sender] - _quantity >= 0, "Quantity exceeds per-wallet limit");
        _mintNFT(_quantity);
        addressMintedBalance[msg.sender] -= _quantity;
    }

    function _mintNFT(uint256 _quantity) internal {
        require(_quantity * PRICE <= msg.value, "Not enough $$$");
        require(_quantity + _owners.length <= MAX_SUPPLY,"Purchase exceeds available supply");
    ...
    function mintTo(address[] calldata _addresses) external  onlyOwner {
        uint256 addrresses = _addresses.length;
        require(addrresses + _owners.length <= MAX_SUPPLY, "_quantity exceeds supply");
        for (uint256 i = 0; i < addrresses; i++) {
            _safeMint(_addresses[i]);
        }
    }

    function addMintedBalance(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            addressMintedBalance[addresses[i]] = PRESALE_MINT_LIMIT;
        }
    }    
    ...
    function setNotRevealedTokenURI(string memory _URI) external  onlyOwner {
        notRevealedTokenURI = _URI;
    }
