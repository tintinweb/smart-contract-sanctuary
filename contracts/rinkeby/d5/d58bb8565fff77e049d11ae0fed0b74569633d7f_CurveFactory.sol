...
// File: NFTBondingCurve/CurveEth.sol
...
contract CurveEth {
    ...
    */
    constructor (address payable _platformCreator, uint256 _platformRate, 
                 ...
                 uint256 _virtualBalance, uint256 _initMintPrice,
        ...
        reserve = 0;
        neolastics = new NFT1155(""); 
    }


    function mint(uint256 _balance, uint256 _maxPriceFistNFT) payable public returns (uint256 _tokenId) {
    ...
    // f(x)=m*x^n  f(x) = 10 * x^2 + 10
    // 0-10-9, 1-20-18,2-50-45,3-100-90,4-170-153,5-260-234,6-370-333,7-500-450,8-650-585,9-820-738, 10-1010-909,11-1220-1098,12-1450-1305
...
contract CurveFactory is Ownable {
    ...
    function createCurve(address payable _platformCreator, uint256 _platformRate, 
                 address payable _bussinessCreator, uint256 _bussinessRate,
                 uint256 _virtualBalance, address _erc20, uint256 _initMintPrice,
        ...
        address curve;
        if (_erc20 != address(0)) {
            curve = address(new Curve(_platformCreator, _platformRate, _bussinessCreator, _bussinessRate, _virtualBalance, _erc20, _initMintPrice, _n, _d));
        } else {
            curve = address(new CurveEth(_platformCreator, _platformRate, _bussinessCreator, _bussinessRate, _virtualBalance, _initMintPrice, _n, _d));
        }
