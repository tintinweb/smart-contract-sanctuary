    ...
    function _init1() private onlyOwner {
        ...
        for (uint8 i = 0; i < devAddresses.length; i++) {
            ...
            _claimableTokenByDev[position] = ClaimableTokenByDev({
                ...
                fornightlyCount: 1,
                amount: (totalAmountClaimableByDev - amountSentFirst) / 24
            });
        }
        ...
        _claimableTokenByDev[7] = ClaimableTokenByDev({
            timestamp: block.timestamp + claimableTimestamp,
            fornightlyCount: 1,
            amount: (totalAmountClaimableByDev7 - amountSentFirst7) / 24
        });
        contractDateCreated = block.timestamp;
        _disableTax = false;
