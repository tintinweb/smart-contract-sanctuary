...
contract PixelTigers is PixelTigers721 {
    ...
    function breed(uint256 tiger1, uint256 tiger2) external onlyOwnerToken(tiger1) onlyOwnerToken(tiger2) {
        ...
        require(tiger1 < totalAmtGenesis && tiger2 < totalAmtGenesis,"Babies can't be used to breed");
        require(tiger1 != tiger2,"Must be two different tiger parents");

        uint256 random = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender)));
        uint256 tokenId = random%8889 + totalAmtGenesis;
...
contract PixelTigers721 is ERC721Enumerable, Ownable {
    ...
    function presaleMint(uint256 _count) public payable {
        ...
        require(price.mul(_count) == msg.value,"Please send correct amount of Eth");
   ...
   function mint(uint256 _count) public payable {
        ...
        require(supply.add(_count) <= totalAmtGenesis - amtReserved, "Purchase would exceed supply");
        require(price.mul(_count) == msg.value, "Eth value sent is not correct");
