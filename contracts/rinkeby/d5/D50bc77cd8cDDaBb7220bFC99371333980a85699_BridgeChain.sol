...
contract BridgeChain is Ownable, Pausable{
  ...
  // 增加设置
  event AddConfig(address indexed operator, address indexed token, uint256 toChain, uint256 fee, uint256 singleMin, uint256 singleMax);
  // 修改手续费
  event UpdateFee(address indexed operator, address indexed token, uint256 toChain, uint256 oldFee, uint256 newFee);
  // 修改限制
  event UpdateSingle(address indexed operator, address indexed token, uint256 toChain, uint256 singleMin, uint256 singleMax);
  // 启停设置
  event UpdateEnable(address indexed operator, address indexed token, uint256 toChain, bool enable);
  // 跨进日志
  ...
  // 提取日志
  event Withdraw(address indexed operator, address indexed receiver, address indexed token, uint256 amount);
  // 提取费用日志
  ...
  // 充值资产
  function recharge(address _token, uint256 _amount) external payable {
    IERC20(_token).universalTransferFrom(msg.sender, address(this), _amount);
