...
contract FelinoidsFactory is ERC721Enumerable, Ownable, IFactoryERC721 {
    using Strings for uint256;

    address public proxyRegistryAddress;
    bool public isFactory = true;
    uint256 public firstFactoryTokenId = 53;
    uint256 public lastFactoryTokenId = 9999;
    ...
    // Deploy
    //
    constructor() ERC721("Felinoids2", "FOID2") {}

    function setFirstFactoryTokenId(uint256 _firstFactoryTokenId) public onlyOwner {
        firstFactoryTokenId = _firstFactoryTokenId;
    }

    function setLastFactoryTokenId(uint256 _lastFactoryTokenId) public onlyOwner {
        lastFactoryTokenId = _lastFactoryTokenId;
    }
    ...
    function setSupportsFactoryInterface(bool _isFactory) public onlyOwner {
    ...
    function canMint(uint256 _tokenId) public view override returns (bool) {
        ...
        bool ownedByFactoryOwner = FelinoidsFactory.ownerOf(_tokenId) == owner();
        return inRange && ownedByFactoryOwner;
    }

    function tokenIdInRange(uint256 _tokenId) public view returns (bool) {
        return (_tokenId >= firstFactoryTokenId) && (_tokenId <= lastFactoryTokenId);
    }
    ...
    function ownerOf(uint256 _tokenId) public view override returns (address _owner) {
        if (_exists(_tokenId)) {
            ...
            require(tokenIdInRange(_tokenId), "ERC721: owner query for nonexistent token");
            // not minted yet
            return owner();
        }
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        require(_tokenId >= firstFactoryTokenId, "FelinoidFactory: invalid tokenId");
        require(_tokenId <= lastFactoryTokenId, "FelinoidFactory: invalid tokenId");
