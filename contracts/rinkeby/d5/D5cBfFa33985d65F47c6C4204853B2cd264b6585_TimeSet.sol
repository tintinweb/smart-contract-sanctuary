...
interface IReferral{
    function recoverAddress(bytes32, bytes32) external returns(address);
    function formReferral(address, address) external returns(bytes32);
}
/**
...
contract PriceDiscovery is Ownable {
    ...
    uint[] public bonusCoefficient 
        = [3000000, 2000000, 1800000, 1600000, 1500000, 1300000, 1200000, 1000000, 100000];
    ...
    function deposit(uint amount, Token token, bytes32 signature, address creator) external discoveryCurrent supportedToken(token){
        ...
        if(creator!=address(0)){
            if (referrals[_msgSender()]==address(0)) {
                bytes32 message = referralContract.formReferral(creator, _msgSender());
                require(creator==referralContract.recoverAddress(message, signature), "Invalid signature provided");
                referrals[_msgSender()]=creator;
            }
            uint256 sYSLamount = (amount*bonusCoefficient[uint(Stage.Referral)]/COEFFICIENT_DECIMALS)/getDiscoveredPrice()*10**18;
            IsYSL(sYSL).mintPurchased(referrals[_msgSender()], sYSLamount, lockTime[uint(Stage.Referral)]);
        }
    }
