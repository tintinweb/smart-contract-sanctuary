...
function onERC721Received( address operator, address from, uint256 tokenId, bytes calldata data ) public pure returns (bytes4) {
    ...
    function alertStake (uint tokenId) external {
        ...
        firstStakeLockPeriod[tokenId] = block.timestamp + randomNo*1 minutes; //convert randomNo from hours to sec
        isStaked[tokenId] = true;
        stakeTime[tokenId] = block.timestamp;
        tokenToRandomHourInStake[tokenId]= randomNo*1 minutes; //conversion required
    ...
    function stake (uint[] memory tokenId) external {
        ...
        if ( stakeConfirmation [tokenId[i]] == true ){
            ...
            nftToken.setTimeStamp(tokenId[i]);
            firstStakeLockPeriod[tokenId[i]] = block.timestamp + randomNo*1 minutes; //convert randomNo from hours to sec
            nftToken.safeTransferFrom(msg.sender, address (this), tokenId[i]);
            stakeTime[tokenId[i]] = block.timestamp;
            isStaked[tokenId[i]] = true;
            tokenToRandomHourInStake[tokenId[i]]= randomNo * 1 minutes; //conversion required
    ...
    function calculateSUP (uint tokenId) internal returns (uint) {
        ...
        if (lastClaimTime == 0) {
            calculatedDuration = (block.timestamp - stakedTime)/60;//todo /60*60
            if (calculatedDuration >= tokenToRandomHourInStake[tokenId]/60) {
            return 250 ether;
            } else {
                return 0;
            }
            ...
            if (wasUnstakedRecently[tokenId] == true) {
                calculatedDuration = (block.timestamp - stakedTime)/60;
                wasUnstakedRecently[tokenId] = false;
            }
            else {
            calculatedDuration = (block.timestamp - lastClaimTime)/1 minutes;//(60*60);
            }
            if (calculatedDuration >= 12 minutes) {
    ...
    function onSuccess (uint tokenId) internal returns (bool) {
        ...
        require (lastClaim[tokenId] + 12 minutes <= block.timestamp, "Claiming before 12 hours");
