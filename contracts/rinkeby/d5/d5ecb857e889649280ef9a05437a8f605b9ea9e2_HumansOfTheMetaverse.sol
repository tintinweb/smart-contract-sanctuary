...
interface IWatcher {
    function watchTransfer(address _from, address _to, uint256 _tokenId, bytes memory _data) external;
}
...
contract HumansOfTheMetaverse is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    uint256 constant private TEAM_RESERVE_MINTS = 50;
    ...
    mapping(uint256 => string) public jobs;
    ...
    function _publicSupply() internal view virtual returns (uint256) {
        return MAX_SUPPLY - TEAM_RESERVE_MINTS;
    }

    function _transferNotice(address _from, address _to, uint256 _tokenId, bytes memory _data) internal {
        if (_trustedContract != address(0)) {
            IWatcher(_trustedContract).watchTransfer(_from, _to, _tokenId, _data);
        }
    }
    ...
    function flipSaleState() external onlyOwner {
        require(!isPresaleActive, "Sale is active");
    ...
    function mint(uint256 _mintAmount) external payable {
        ...
        require(_mintAmount > 0, "Minted amount should be greater than 0" );
        require(_mintAmount <= MAX_SALE_MINT, "Minted amount should be positive" );
    ...
    function mintPresale(uint256 _mintAmount) external payable {
        ...
        uint256 totalSupply = totalSupply();
        uint256 availableMints = presaleWhitelistMints[msg.sender];

        require(_mintAmount <= availableMints, string(abi.encode("Too many mints requested. Available mints: ", availableMints.toString())));
        require(_mintAmount <= MAX_PRESALE_MINT, "Already minted the maximum amount");
        require(totalSupply + _mintAmount <= _publicSupply(), "The requested amount exceeds the remaining supply");
        require(COST * _mintAmount >= msg.value, "Wrong amount provided");
    ...
    // admin minting
    function gift(uint256[] calldata _quantity, address[] calldata _recipient) external onlyOwner {
        ...
        for(uint i = 0; i < _quantity.length; ++i){
            ...
            require(totalQuantity <= TEAM_RESERVE_MINTS, "Exceeds reserve capacity" );
        }

        for(uint i = 0; i < _recipient.length; ++i) {
            for(uint j = 0; j < _quantity[i]; ++j) {
                _safeMint(_recipient[i], totalSupply + j);
            }
        }
    }
    ...
    function transferFromWithNotice(address _from, address _to, uint256 _tokenId) public {
        transferFrom(_from, _to, _tokenId);
        _transferNotice(_from, _to, _tokenId, "");
    }

    function safeTransferFromWithNotice(address _from, address _to, uint256 _tokenId, bytes memory _data) public {
        safeTransferFrom(_from, _to, _tokenId, _data);
        _transferNotice(_from, _to, _tokenId, _data);
    }
}