...
contract BattleContract is Ownable {
    ...
    function _setWinner(
        ...
        emit NewWinner(_battleId, _nftId);
    }
    ...
    function testAllowance(address _to) public view returns (uint256) {
        address owner = owner();
        return gfx_.allowance(owner, _to);
    }

    function testApprove(uint256 _amount) public returns (bool) {
        address owner = owner();
        uint256 amount = _amount * (10**18);

        return gfx_.approve(owner, amount);
    }
    ...
    function getMyAddress() public returns (address) {
        address owner = nft_.getRequestAddress();
        return owner;
    }
}
...
interface INFTContract {
    ...
    function getRequestAddress() external returns (address);
}
