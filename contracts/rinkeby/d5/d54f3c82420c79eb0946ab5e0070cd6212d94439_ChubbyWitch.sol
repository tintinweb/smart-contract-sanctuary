...
contract ChubbyWitch is Context, IERC20, Ownable {
    ...
    string private _name = 'ChubbyInu';
    string private _symbol = 'CHINU';
    uint8 private _decimals = 9;

    uint256 private _taxFee = 3; // 2% reflection fee for every holder
    uint256 private _charityMarketingFee = 4; // 10% charity+marketing
    uint256 private _liquidityFee = 2; // 13% into liquidity

    uint256 private _previousTaxFee = _taxFee;
    uint256 private _previousCharityMarketingFee = _charityMarketingFee;
    uint256 private _previousLiquidityFee = _liquidityFee;

    address payable private _charityWalletAddress = payable(0x0B797D594f989dF5FE5B9CEdB7dcF094567D2871);
    address payable private _marketingWalletAddress = payable(0x0B797D594f989dF5FE5B9CEdB7dcF094567D2871);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _charityMarketingFee == 0 && _liquidityFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousCharityMarketingFee = _charityMarketingFee;
        _previousLiquidityFee = _liquidityFee;

        _taxFee = 0;
        _charityMarketingFee = 0;
        _liquidityFee = 0;
    }

    function restoreAllFee() private {
        _taxFee = _previousTaxFee;
        _charityMarketingFee = _previousCharityMarketingFee;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && overMinTokenBalance && sender != uniswapV2Pair) {
            uint256 toCharityMarketing = contractTokenBalance.mul(_charityMarketingFee).div(100);
            uint256 toLiquify = contractTokenBalance.sub(toCharityMarketing);

            swapAndSend(toCharityMarketing);
            swapAndLiquify(toLiquify);
        }
...
//    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tCharityMarketingLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeCharityMarketingLiquidity(tCharityMarketingLiquidity);
    ...
    function _takeCharityMarketingLiquidity(uint256 tCharityMarketingLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 rCharityMarketingLiquidity = tCharityMarketingLiquidity.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rCharityMarketingLiquidity);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tCharityMarketingLiquidity);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tCharityMarketingLiquidityFee) = _getTValues(tAmount, _taxFee, _charityMarketingFee.add(_liquidityFee));
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tCharityMarketingLiquidityFee);
    }

    function _getTValues(uint256 tAmount, uint256 taxFee, uint256 charityMarketingLiquidityFee) private pure returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.mul(taxFee).div(100);
        uint256 tCharityMarketingLiquidityFee = tAmount.mul(charityMarketingLiquidityFee).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(charityMarketingLiquidityFee);
        return (tTransferAmount, tFee, tCharityMarketingLiquidityFee);
    }
    ...
    function _setCharityMarketingFee(uint256 charityMarketingFee) external onlyOwner() {
        require(charityMarketingFee >= 1 && charityMarketingFee <= 49, 'charityMarketingFee should be in 1 - 11');
        _charityMarketingFee = charityMarketingFee;
    }
