    ...
    Ownable
{
    ...
    event SaleConfigChanged(uint32 startTime);
    ...
    function setupSale(uint256 startTime) external onlyOwner {
        uint32 _startTime = startTime.toUint32();

        require(
            _startTime >= presaleConfig.endTime,
            "HoF: invalid start time of sale"
        );

        saleConfig = SaleConfig({startTime: _startTime});

        emit SaleConfigChanged(_startTime);
    }
