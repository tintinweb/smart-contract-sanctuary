...
contract Syndicates {
    ...
    event joinedSyndicate(address _from, uint _syndicate, uint _glitch);
    ...
    function join(uint256 _syndicate, uint256 _glitch) public {
        ...
        require(1 <= _syndicate && _syndicate <= 5, "InvalidSyndicate");
        require(tlg.ownerOf(_glitch) == msg.sender, "NotOwner");
        require(syndicate[_glitch] == 0, "AlreadyJoined");

        // TODO special requirement for syndicate 5
        ...
        emit joinedSyndicate(msg.sender, _syndicate, _glitch);
    }

    function memberOfSyndicateByIndex(uint256 _syndicate, uint256 index) public view virtual returns (uint256) {
        require(index < memberCount[_syndicate], "Member index out of bounds");
        return members[_syndicate][index];
    }

    function membersOfSyndicate(uint256 _syndicate) external view returns(uint256[] memory) {
        uint256 length = memberCount[_syndicate];
        if (length == 0) {
            ...
            for (uint256 index; index < length; index++) {
                result[index] = memberOfSyndicateByIndex(_syndicate, index);
            }
            return result;
        }
    }
