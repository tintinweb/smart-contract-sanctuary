...
contract Apes is ERC721Enumerable, Ownable {
    ...
    address public constant dev1Address = 0x91f39413c9a943aeB94f99BFEbE73dA202b570B4;
    address public constant dev2Address = 0x57247B0F2037Ae53Ed2b85D6ccf9708A89f0586e;
...
contract Forest is Ownable, IERC721Receiver {
    using SafeMath for uint256;
    ...
    */
    ...
    //Amount of $Bananas stolen by Pirate while staked
    mapping(uint256 => uint256) public bananasStolen;

    //Daily $Bananas earned by adventurers
    uint256 public adventuringBananasRate = 100 ether;
    //Total number of adventurers staked
    uint256 public totalAdventurersStaked = 0;
    //Percent of $Bananas earned by adventurers that is kept
    uint256 public adventurerShare = 50;

    //Percent of $Bananas earned by adventurers that is stolen by pirates
    ...
    /*sends any number of Apes to the galaxy
    ...
    */
    ...
    function _claim(address owner, uint256 tokenId) internal {
        if (statusById[tokenId] == 1){
            if(piratesStaked.length > 0){
                bananas.mint(owner, getPendingBananas(tokenId).mul(adventurerShare).div(100));
                distributeAmongstPirates(getPendingBananas(tokenId).mul(pirateShare).div(100));
            }
            else {
    ...
    //Distribute stolen $Bananas accross all staked pirates
    function distributeAmongstPirates(uint256 amount) internal {
        for(uint256 i = 0; i < piratesStaked.length; i++){
            bananasStolen[piratesStaked[i]] += amount.div(piratesStaked.length);
        }
    }
    ...
    function getNewOwnerForPirate(uint256 seed, uint256 tokenId) internal{
        ...
        emit ApeStolen(ownerById[tokenId], ownerById[salvagersStaked[roll]], tokenId);
        ownerById[tokenId] = ownerById[salvagersStaked[roll]];
        bananas.mint(ownerById[tokenId], bananasStolen[tokenId]);
        bananasStolen[tokenId] = 0;
    }
