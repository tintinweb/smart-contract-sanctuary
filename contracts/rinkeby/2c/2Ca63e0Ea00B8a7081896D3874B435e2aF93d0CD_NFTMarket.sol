...
contract NFTMinting is ERC721, Ownable{
   ...
   event BatchMint(uint256 _totalNft, string msg);
    ...
    function _mintNft(address creator,  string memory _TokenURI, uint256 _royaltyPercentage, address user) 
    internal 
    returns (uint256) 
    {
    ...
    function batchMint(uint256 _totalNft, string[] memory _uri, uint256[] memory _royalty) public returns (bool) {
        ...
        for(uint i = 0; i< _totalNft; i++) {
            _mintNft(msg.sender, _uri[i], _royalty[i], msg.sender);
        }
...
contract NFTMarket is ReentrancyGuard, Ownable{
  ...
  unicusNft = NFTMinting(_nftContract);
...
//struct for each market item
  ...
  function createSale(
    ...
    IERC721(nftContract).transferFrom(msg.sender, address(this), tokenId); //nft gets transferred to the contract
    if(listingPrice>0){
    payable(treasury).transfer(listingPrice); //lislistingPrice transferred to treasury
    }
    emit saleCreated(
      itemId,
      nftContract,
  ...
  function buyItem(
    uint256 itemId
    ) public payable nonReentrant isValidId(itemId)  {
    ...
    uint256 amountTominter = ((msg.value).mul((royalty))).div(100) ;
    uint256 amountToSeller = (msg.value).sub(amountTominter);
     payable(idToMarketItem[itemId].minter).transfer(amountTominter);
    payable(idToMarketItem[itemId].seller).transfer(amountToSeller);
  ...
  function EndSale(uint256 itemId) public nonReentrant isValidId(itemId){
  ...
  function royaltyForItem(uint256 item) external view isValidId(item) returns(uint256){
  ...
  function fetchMarketItems() public view returns (MarketItem[] memory) {
    ...
    uint unsoldItemCount = _itemIds.current().sub(_itemsSold.current());
    uint currentIndex = 0;

    MarketItem[] memory items = new MarketItem[](unsoldItemCount);
    for (uint i = 0; i < itemCount; i++) {
      if (idToMarketItem[i.add(1)].owner == treasury && idToMarketItem[i.add(1)].isActive ==true )
      {
  ...
  function fetchMyNFTs() public view returns (MarketItem[] memory) {
    ...
    modifier isValidId( uint256 itemId){
