...
contract PaintjobNFT is ERC721URIStorage, Ownable{
 
    event MintPaintjobs (address indexed minter, uint256 startWith, uint256 times);
    
    
    uint256 public totalPaintjobs;
    ...
    uint256 public presaleRelease = 1630445400;
    ...
    function mintPaintjob(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalPaintjobs + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintPaintjobs(_msgSender(), totalPaintjobs+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalPaintjobs++);
        }
    } 
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalPaintjobs + _times <= totalCount, "Mint amount will exceed total collection amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalPaintjobs + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintPaintjobs(_addr, totalPaintjobs+1, 1);
        _mint(_addr, 1 + totalPaintjobs++);
    } 
