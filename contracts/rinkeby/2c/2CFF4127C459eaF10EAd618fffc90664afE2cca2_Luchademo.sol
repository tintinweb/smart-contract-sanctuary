...
import "./library/VRFConsumerBase.sol";
import "./library/IUniswapV2Router02.sol";

contract Luchademo is ERC721, Ownable, ReentrancyGuard, VRFConsumerBase {
	...
	function generateLuchador(uint256 _quantity, uint256 _deadline) external payable nonReentrant {
		...
		require(_tokenIds.current() + _quantity <= 10000);
	...
	function imageData(uint256 _tokenId) public view returns (string memory) {
		...
		require(luchadores[_tokenId].dna != 0);
	...
	function metadata(uint256 _tokenId) public view returns (string memory) {
		...
		return string(abi.encodePacked(
			'{',
				...
				'"description": "Luchadores using Chainlink VRF 100% on-chain art and metadata",',
...
library EnumerableMap {
    ...
    */
    function _at(Map storage map, uint256 index) private view returns (bytes32, bytes32) {
        require(map._entries.length > index);
