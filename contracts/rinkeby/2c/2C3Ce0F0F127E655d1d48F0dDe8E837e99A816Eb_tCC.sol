...
contract tCC is ERC721Enumerable, Ownable {
    ...
    function mintWhiteList(uint256 level, bytes32[] calldata proof) external {
        ...
        require(
            MerkleProof.verify(
                proof, roots[level], keccak256(abi.encode(msg.sender))
            ), 
            "invalid proof");
        _mintCard(level, 1, msg.sender);
    ...
    function verifyWhiteList(uint256 level, bytes32[] calldata proof) external view returns (bool) {
        return MerkleProof.verify(proof, roots[level], keccak256(abi.encode(msg.sender)));
    }
