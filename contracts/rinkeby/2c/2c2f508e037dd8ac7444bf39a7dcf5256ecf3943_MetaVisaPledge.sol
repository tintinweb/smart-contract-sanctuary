...
contract MetaVisaPledge is Ownable,IERC721Receiver {
    ...
    uint256 public _continuousDays = 10 * 30 minutes; //continuous days
    uint256 public _period  = 10 minutes;  //cycle
    ...
    function onERC721Received(address operator,address from,uint256 tokenId,bytes calldata ) external override returns (bytes4) {
        require(block.timestamp>=_startTime, "MetaVisaPledge: Staking event will start on 2022.01.xx, please join https://t.me/MetaVisaOfficialCommunity for more giveaways");
        require(block.timestamp <= _startTime + _continuousDays, "MetaVisaPledge: Staking event has been ended, please join https://t.me/MetaVisaOfficialCommunity for more giveaways.");
    ...
    function cancelPledgeMVC(uint256 tokenId) external virtual returns(bool) {
        ...
        require(_metavisaLimitOrderOf[msg.sender].length() < _metavisaOrderOf[msg.sender].length(), "cancelPledgeMVC: You should unstake MetaVisa Hero Assembly Limited NFT first.");

        OrderSet.Order memory order = _metavisaOrderOf[msg.sender].idAt(tokenId);

        uint256 createTime = order.createTime;
        ...
        uint256 income = _calcIncome(createTime,_allIncomes);
    ...
    function _pledgeMVCLimit(uint256 tokenId,address account) private returns(bool) {
        require(_metavisaLimitOrderOf[account].length() < _metavisaOrderOf[account].length(), "_pledgeMVCLimit: You should stake MetaVisa Hero NFT first.");
    ...
    function cancelPledgeMVCLimit(uint256 tokenId) external virtual returns(bool) {
        ...
        uint256 income = _calcIncome(createTime,_allLimitIncomes);
    ...
    function setNextRound(uint256 startTime_,uint256 continuousDays_,uint256 period_,uint256 fixedDailyReward_,uint256 fixedDailyRewardLimit_) external onlyOwner onlyTimeEnd returns(bool) {
        require(block.timestamp<startTime_, "MetaVisaPledge:The start time must be greater than the current time");
        _startTime = startTime_;
        _continuousDays = continuousDays_ * 5 minutes;
        _period = period_ * 5 minutes;
    ...
    function delUserIncome(address[] memory accounts) external onlyOwner onlyTimeEnd returns(bool) {
        for(uint256 i = 0;i < accounts.length; i++){
            _limitIncome[accounts[i]] = 0;
        }
        return true;
    }
}