...
contract MPH is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 _saleTime = 1639497600;
    uint256 _presaleTime = 1639411200;
    ...
    function mint(uint256 _count) public payable nonReentrant {
        require(
            ...
            "My Pet Hooligan: Not enough left to mint"
        );
        require(
            totalSupply() < MAX,
            "My Pet Hooligan: Not enough left to mint"
        ...
        require(
            ...
            "My Pet Hooligan: Value below price"
        );
        ...
        if (block.timestamp >= _saleTime) {
            ...
            block.timestamp < _saleTime && block.timestamp >= _presaleTime
        ) {
            require(
                _count <= MPHPresale(_presaleContract).isInPresale(msg.sender),
                "My Pet Hooligan: Exceeds the max you can mint in the presale"
            );
            require(
                MPHPresale(_presaleContract).isInPresale(msg.sender) > 0,
                "My Pet Hooligan: You are not in the presale"
            );
            require(
                MPHPresale(_presaleContract).isInMintedPresale(msg.sender) ==
                    false,
                "My Pet Hooligan: You already minted from the presale"
            );
            for (uint256 x = 0; x < _count; x++) {
                _safeMint(msg.sender, totalSupply());
            }
    ...
    function changePreSaleTime(uint256 _newDate) public onlyOwner {
