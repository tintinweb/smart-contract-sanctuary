    ...
    */
            ...
            )
    {
        ...
        for (uint i=0; i<farm.numberOfCollectibles; i++) {
            totalMaxBoostLevel += ownerOfCollectibles[_msgSender()][i].maxBoostLevel;
            totalblocksToMaxBoost += ownerOfCollectibles[_msgSender()][i].blocksToMaxBoost;
            if ( ownerOfCollectibles[_msgSender()][i].expiry == 0 ) {
                noExpiry = true;
                finalExpiry = 0;
            }
            if ( ownerOfCollectibles[_msgSender()][i].expiry > finalExpiry && noExpiry == false ) { 
                finalExpiry = ownerOfCollectibles[_msgSender()][i].expiry;
            }
        }
    }
     ...
     */
    function getCollectiblesByFarm(address _address)
        public
        view
        returns (
            Collectible[] memory collectibles
        )
    {
        Farm memory farm = farms[_address]; // Shortcut accessor

        for (uint i=0; i<farm.numberOfCollectibles; i++) {
            collectibles[i].id = ownerOfCollectibles[_address][i].id;
            collectibles[i].name = ownerOfCollectibles[_address][i].name;
            collectibles[i].maxBoostLevel = ownerOfCollectibles[_address][i].maxBoostLevel;
            collectibles[i].blocksToMaxBoost = ownerOfCollectibles[_address][i].blocksToMaxBoost;
            collectibles[i].addedBlockNumber = ownerOfCollectibles[_address][i].addedBlockNumber;
            collectibles[i].expiry = ownerOfCollectibles[_address][i].expiry;
        }
    }
