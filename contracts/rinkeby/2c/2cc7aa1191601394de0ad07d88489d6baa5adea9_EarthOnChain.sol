...
// GO TO LINE 1884 TO SEE WHERE THE CUCUMBER CONTRACT STARTS
...
contract EarthOnChain is ERC721, Ownable {
    ...
    uint256 public constant RESOLUTION = 1000000;
    uint256 public constant MAX_LAT = 90;
    uint256 public constant MAX_LONG = 180;
    uint256 public nftReserve = 100; // Reserve 100 NFT for team - Giveaways/Prizes etc
    ...
    function getLat(uint _id) internal pure returns (int256) {
        uint rand = random(_id);
        uint number = rand % (MAX_LAT * 2);

        int lat;
        if (number > MAX_LAT) {
            lat = int(MAX_LAT - number);
        } else {
            lat = int(number);
        }
    ...
    function getLatFloat(uint256 _id) internal pure returns (uint256) {
        return getFloat((_id + 1) * 2);
    }

    function getLong(uint _id) internal pure returns (int256) {
        uint rand = random(_id);
        uint number = rand % (MAX_LONG * 2);

        int long;
        if (number > MAX_LONG) {
            long = int(MAX_LONG - number);
        } else {
            long = int(number);
        }
    ...
    function getLongFloat(uint256 _id) internal pure returns (uint256) {
        return getFloat((_id + 1) * 3);
    }

    function getFloat(uint256 _id) internal pure returns (uint256) {
        uint256 rand = uint256(keccak256(abi.encodePacked(uint2str(_id), uint2str(random(_id)))));
    ...
    function tokenURI(uint256 _id) public view virtual override(ERC721) returns (string memory) {
        ...
        int lat = getLat(_id);
        int long = getLong(_id);

        string memory image = Base64.encode(bytes(generateSVGImage(lat, long, _id)));
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            ...
                            image,
                            '",',
                            '"description": "Your latitude and longitude earth geoloc."',
                            '"attributes":[',
                            '{',
                            '"trait_type": "Latitude",',
                            '"value": "',
    ...
    function generateSVGImage(int lat, int long, uint256 _id) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
                '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: impact, serif; font-size: 25px; }</style><rect width="100%" height="100%" fill="black"/>',
                '<text x="125" y="187" class="base">',
                lat,
                '.',
                getLatFloat(_id),
                ' &#176;</text>',
                '<text x="175" y="187" class="base">-</text>',
                '<text x="200" y="187" class="base">',
                long,
                '.',
                getLongFloat(_id),
                ' &#176;</text>',
