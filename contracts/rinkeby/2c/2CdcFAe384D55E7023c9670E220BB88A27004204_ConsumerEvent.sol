...
contract ConsumerEvent is Ownable, IConsumerEvent {
    ...
    function sendEther(uint value) onlyOwner public {
    ...
    function buyTicket (address _eventContractAddress, string memory _ticketName, string memory _userName) public override payable returns (uint256) {
      ...
      _price = int(msg.value);
      _priceless = priceLess;
      _pricemore = priceMore;
        
      require(_msgSender() != address(0), "Address can not be empty");
      require((int(msg.value) >= priceLess) || (int(msg.value) <= priceMore), "Amount is not correct");
      require (ticketsSold[_ticketName] < tickets, "All tickets are sold!" );
      ticketsSold[_ticketName]++;
      
      if(boughtTicketsBase[_eventContractAddress][_msgSender()].length == 0) {
          boughtTicketsBase[_eventContractAddress][_msgSender()].push(boughtTickets(_ticketName, 0));
      }
      for(uint8 i = 0; i < boughtTicketsBase[_eventContractAddress][_msgSender()].length; i++) {
          if(keccak256(abi.encodePacked(boughtTicketsBase[_eventContractAddress][_msgSender()][i].boughtTicketName)) == keccak256(abi.encodePacked(_ticketName))) {
            boughtTicketsBase[_eventContractAddress][_msgSender()][i].numberOfBoughtTickets++;
          } else if (keccak256(abi.encodePacked(boughtTicketsBase[_eventContractAddress][_msgSender()][i].boughtTicketName)) != keccak256(abi.encodePacked(_ticketName)) && i == boughtTicketsBase[_eventContractAddress][_msgSender()].length - 1){
            boughtTicketsBase[_eventContractAddress][_msgSender()].push(boughtTickets(_ticketName, 0));
          }
      }
      ...
      userAddresses[_userName] = _msgSender();
    ...
    function getMsgValue() public view returns(int, int, int) {
        return(_price, _priceless, _pricemore);
    }
}
...
contract EventsManagment is Ownable, IEventsManagment {
    ...
    // @ticketArray: [[0,"StandardTicket", 1500, 50], [1,"Afterparty", 1750, 30], [2, "VipTicket", 3000, 20]]
    ...
    function addTickets(address _eventContractAddress, ticketDetails[] calldata ticketArray) onlyOwner public {
      for (uint8 i = 0; i < ticketArray.length; i++) {
          Tickets[_eventContractAddress].push(ticketDetails(i, ticketArray[i].ticketName, ticketArray[i].ticketPrice, ticketArray[i].ticketAmount));
      }
    }
