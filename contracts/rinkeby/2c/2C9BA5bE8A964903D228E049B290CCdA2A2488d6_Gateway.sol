...
contract Gateway is Ownable {
    ...
    enum TransferToMethod { MINT, UNLOCK }
    ...
    event TokenAdded(uint256 indexed tokenId, bool isOriginChain, address tokenAddress, TransferToMethod transferToMethod, uint256[] supportedChains);
    ...
    function addToken(bool isOriginChain, address tokenAddress, TransferToMethod transferToMethod, uint256[] memory chains)
        public
        onlyOwner
    {
        ...
        emit TokenAdded(tokenId, isOriginChain, tokenAddress, transferToMethod, chains);
    }
    ...
    function transferFromAnotherChain(
        ...
        if (tokens[tokenId].transferToMethod == TransferToMethod.MINT && !tokens[tokenId].isOriginChain) {
    ...
    function transferToAnotherChain(
        ...
        uint256 fee = feeCalculation(address(token), amount);
        uint256 amountAfterFee = amount.sub(fee);
            
        token.safeTransferFrom(msg.sender, address(this), amount);

        if (!tokens[tokenId].isOriginChain && tokens[tokenId].transferToMethod == TransferToMethod.MINT) {
            token.burn(address(this), amountAfterFee);
        }

        feeToWithdraw[address(token)] = feeToWithdraw[address(token)].add(fee);

        emit TransferredToAnotherChain(tokenId, toChainId, msg.sender, to, amount, amountAfterFee, networksFee[toChainId]);
    }
