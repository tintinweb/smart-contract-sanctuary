...
contract TestMetadata is ITestMetadata {
    ...
    constructor() {
        ...
        _imageParts.push("<svg xmlns='http://www.w3.org/2000/svg' version='1.1' width='2000' height='2000'>");
            _imageParts.push("<style>");
                ...
                _imageParts.push(".m3 #c{fill: #f6f;}"); // p
                _imageParts.push(".m3 #r{fill: #000;}");
                _imageParts.push(".m4 #c{fill: #df0;}"); // y
                _imageParts.push(".m4 #r{fill: #000;}");
                _imageParts.push(".a #c{fill: #f00 !important;}"); // r
                _imageParts.push(".a #r{fill: #f00 !important;}");
    ...
    function tokenMetadata(uint256 tokenId, uint256 rarity, uint256 tokenTest, uint256 alphaTest, bool isAlpha, uint256 mergeCount) external view override returns (string memory) {        
        string memory base64Json = Base64.encode(bytes(string(abi.encodePacked(_getJson(tokenId, rarity, tokenTest, alphaTest, isAlpha, mergeCount)))));
    ...
    function _getJson(uint256 tokenId, uint256 rarity, uint256 tokenTest, uint256 alphaTest, bool isAlpha, uint256 mergeCount) private view returns (string memory) {        
        string memory imageData = 
            bytes(_imageBaseURI).length == 0 ? 
                _getSvg(tokenId, rarity, tokenTest, alphaTest, isAlpha) :
                string(abi.encodePacked(imageBaseURI(), _getClassString(tokenId, rarity, isAlpha, true), "_", uint256(int256(_getScaledRadius(tokenTest, alphaTest, _maxRadius).toInt())).toString(), imageExtension()));

        ERC721MetadataStructure memory metadata = ERC721MetadataStructure({
            isImageLinked: bytes(_imageBaseURI).length > 0, 
            name: string(abi.encodePacked(name(), "(", tokenTest.toString(), ") #", tokenId.toString())),
            description: tokenTest.toString(),
            createdBy: "Anon",
            image: imageData,
            attributes: _getJsonAttributes(tokenId, rarity, tokenTest, mergeCount, isAlpha)
        });

        return _generateERC721Metadata(metadata);
    }        

    function _getJsonAttributes(uint256 tokenId, uint256 rarity, uint256 tokenTest, uint256 mergeCount, bool isAlpha) private pure returns (ERC721MetadataAttribute[] memory) {
        ...
        metadataAttributes[0] = _getERC721MetadataAttribute(false, true, false, "", "Test", tokenTest.toString());
        ...
        metadataAttributes[4] = _getERC721MetadataAttribute(false, true, false, "", "Collides", mergeCount.toString());
    ...
    // remember to make this function private (it is set to public just for easy testing)
    function _getSvg(uint256 tokenId, uint256 rarity, uint256 tokenTest, uint256 alphaTest, bool isAlpha) public view returns (string memory) {
        bytes memory byteString;
        int128 radius = _getScaledRadius(tokenTest, alphaTest, _maxRadius);
    ...
    function _getScaledRadius(uint256 tokenTest, uint256 alphaTest, uint256 maximumRadius) private pure returns (int128) {
        int128 radiusTest = _getRadius64x64(tokenTest);
        int128 radiusAlphaTest = _getRadius64x64(alphaTest);
        int128 scalePercentage = ABDKMath64x64.div(radiusTest, radiusAlphaTest);                
    ...
    // Radius = Cube Root(Test) * Cube Root (0.23873241463)
    // Radius = Cube Root(Test) * 0.62035049089
    function _getRadius64x64(uint256 test) private pure returns (int128) {        
        int128 cubeRootScalar = ABDKMath64x64.divu(62035049089, 100000000000);
        int128 cubeRootTest = ABDKMath64x64.divu(test.nthRoot(3, 6, 32), 1000000);
        int128 radius = ABDKMath64x64.mul(cubeRootTest, cubeRootScalar);        
        return radius;
    }            
...
contract Test is ERC721, ERC721Metadata {
 
    ITestMetadata public _metadataGenerator;
    ...
    uint256 public _alphaDensity = 10;

    uint256 public _alphaId;

    uint256 public _densityTotal;

    address public _anon;
    ...
    // Mapping token ID to all quantity testd into it.
    mapping (uint256 => uint256) private _testCount;
    ...
    function getTestCount(uint256 tokenId) public view returns (uint256 testCount) {
        require(_exists(tokenId), "nonexistent token");
        return _testCount[tokenId];
    }

    modifier onlyAnon() {
        require(_msgSender() == _anon, "msg.sender is not anon");
        _;
    }
    ...
    function ensureValidDensity(uint256 density) private pure {
        require(MIN_MASS_INCL <= density && density < MAX_MASS_EXCL, "density must be [1,100k-1).");
    }
     ...
     *
     */    
    constructor(address metadataGenerator_) {
        _nextMintId = 1;
        _metadataGenerator = ITestMetadata(metadataGenerator_);
        _name = "test.";
        _symbol = "ccc";
        ...
        // _values[1] = encodeClassAndDensity(1, 5);         
        // _owners[1] = msg.sender;
        // emit Transfer(address(0), msg.sender, 1);
        // _countToken++;
        // _balances[msg.sender]++;
        // _densityTotal = 5;
        // emit AlphaDensityUpdate(1, 5);
    }
    ...
    function test(uint256 tokenIdRcvr, uint256 tokenIdSndr) external onlyValidWhitelist notFrozen returns (uint256 tokenIdDead) {        
        ...
        tokenIdDead = _test(tokenIdRcvr, tokenIdSndr);
        ...
        if (to == _dead) {
            ...
            if (toIsWhitelisted) {
                ...
                if (currentTokenId == 0) {
                    ...
                    // compute token test, returning the dead token
                    uint256 deadTokenId = _test(currentTokenId, sentTokenId);
    ...
    function _test(uint256 tokenIdRcvr, uint256 tokenIdSndr) internal returns (uint256 tokenIdDead) {
        ...
        uint256 testdDensity = densityLarge + densitySmall;

        if(testdDensity > _alphaDensity) {
            _alphaId = tokenIdLarge;
            _alphaDensity = testdDensity;
            emit AlphaDensityUpdate(_alphaId, testdDensity);
        }
        
        _testCount[tokenIdLarge]++;

        delete _values[tokenIdSmall];

        _countToken -= 1;

        emit DensityUpdate(tokenIdSmall, tokenIdLarge, testdDensity);

        return tokenIdSmall;
    }

    function setRoyaltyBips(uint256 percentageRoyalty_) external onlyAnon {
    ...
    function setBlacklistAddress(address address_, bool status) external onlyAnon {
        _blacklistAddress[address_] = status;
    }

    function setCollab(address collab_) external onlyAnon {  
        _collab = collab_;
    }

    function setFund(address fund_) external onlyAnon {  
        _fund = fund_;
    }

    function setAnon(address anon_) external onlyAnon {
        require(address(this).balance < 1e16, "be careful");
        _anon = anon_;
    }

    function setUnitPrice(uint256 unitPrice_) external onlyAnon {
        _unitPrice = unitPrice_;
    }

    function setSaleStartTime(uint256 startTime_) external onlyAnon {
        require(
    ...
    function setSaleEndTime(uint256 endTime_) external onlyAnon {
        require(
    ...
    function updateMaxSupply(uint256 count_) external onlyAnon {
        require(_nextMintId <= count_, "invalid supply count");
        _maxSupply = count_;
    }

    function setRoyaltyReceiver(address receiver_) external onlyAnon {  
        _receiver = receiver_;
    }
    
    function setMetadataGenerator(address metadataGenerator_) external onlyAnon {  
        _metadataGenerator = ITestMetadata(metadataGenerator_);
    }
   
    function whitelistUpdate(address address_, bool status) external onlyAnon {
    ...
    function mint(uint256 _densityToMint) external payable {
        ...
        require(
            msg.value >= _densityToMint * _unitPrice,
            "insufficient ETH"
        );
        require(
        ...
        // update total density in system (must be bounded by MAX_MASS_EXCL)
        uint256 prevDensityTotal = _densityTotal;
        uint256 newDensityTotal = prevDensityTotal + _densityToMint;
        require(newDensityTotal < MAX_MASS_EXCL, "density total overflow");
    ...
    function freeze(bool state_) external onlyAnon {
        frozen = state_;
    }
    ...
    function destroyWithMercy(uint256 tokenId) external payable onlyAnon {
        _destroy(tokenId);
    }

    function destroyRandomWithMercy() external payable onlyAnon {
        uint256 tokenId = random() % (_nextMintId - 1);
        _destroy(tokenId);
    }
    ...
    function tokenURI(uint256 tokenId) public virtual view override returns (string memory) {
        ...
        return _metadataGenerator.tokenMetadata(
            ...
            tokenId == _alphaId,
            getTestCount(tokenId));
    }

    function updateTokenClass(uint tokenId, uint256 class) external onlyAnon {
        require(_exists(tokenId), "nonexistent token");
    ...
    function withdrawAll() public onlyAnon {
        uint256 currentBal = address(this).balance;
        require(currentBal > 0);
        if (_collab == _anon){
            _withdraw(_anon, address(this).balance);
        } else {
            _withdraw(_collab, currentBal * 2 / 5); // 40%
            _withdraw(_fund, currentBal / 5); // 20%
            //remainder to prevent ether dust
            _withdraw(_anon, address(this).balance); // 40%
        }
    }
