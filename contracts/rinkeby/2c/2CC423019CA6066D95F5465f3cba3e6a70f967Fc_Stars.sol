...
contract Stars is BasicMetaTransaction, ERC20PresetMinterPauser {
    ...
    /**
     * @dev Admin function to add or remove an address to the blacklist.
     * STARS cannot be sent from blacklisted addresses.
     *
     * Parameters:
     *
     * - address: the address to add to whitelist.
     * - _isBlacklisted: whitelist status to change to.
     *
     * Requirements:
     *
     * - caller must be admin.
     */
    function changeBlacklistStatus(address userAddress, bool _isBlacklisted)
        public
    {
        require(admin == msgSender(), "Caller is not an admin");
        isBlacklisted[userAddress] = _isBlacklisted;
    }
