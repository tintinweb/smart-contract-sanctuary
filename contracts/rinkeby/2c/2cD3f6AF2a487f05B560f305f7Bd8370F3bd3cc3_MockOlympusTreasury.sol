...
contract MockOlympusTreasury is Ownable {
  ...
  /**
        @notice allow approved address to deposit an asset for STRUDEL
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint STRUDEL needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(STRUDEL).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn STRUDEL for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IOHMERC20(STRUDEL).burnFrom(msg.sender, value);
  ...
  /**
        @notice allow approved address to repay borrowed reserves with STRUDEL
        @param _amount uint
     */
  function repayDebtWithOHM(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IOHMERC20(STRUDEL).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, STRUDEL, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(STRUDEL).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(STRUDEL).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns STRUDEL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match STRUDEL decimals
      value_ = _amount.mul(10**IERC20(STRUDEL).decimals()).div(
