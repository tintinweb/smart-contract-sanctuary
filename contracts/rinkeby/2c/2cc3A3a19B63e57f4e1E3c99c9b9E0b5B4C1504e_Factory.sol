...
contract Collection is ERC1155, AccessControl {
    ...
    function addWeightBatch(uint256[] memory tokenIds, uint64[] memory weights) internal {
     ...
     function addWeight(uint256 tokenId, uint64  weight) internal {
        weightByTokenAttribute[tokenId] = weight;

    }

    function mint(address _to, uint64 _weight, uint256 _id) external factoryOrAdmin {
        require(!(mintedSeeds.contains(_id)), "This seed id already minted");

        _mint(_to, _id, 1, "");

        addWeight(_id, _weight);
   
        mintedSeeds.add(_id);

        emit Minted(address(this), _to, _id, _weight);
    }

    function mintBatch(address _to, uint64[] calldata _weights, uint256[] calldata _ids,  uint256[] calldata amount_) external factoryOrAdmin {
        ...
        emit MintedBatch(address(this), _to, _ids, _weights);
    }
...
contract Factory is AccessControl {
    ...
    function createCollection(
        ...
        Collection collection =
            new Collection(_uri, _admin, address(this));
        ...
        emit CollectionCreated(address(collection), _uri);
     ...
     function mint(address _collecAdd, address account, uint256 _id, uint64 _weight) onlyAdmin() external {
         Collection collection = Collection(_collecAdd);
         
         collection.mint(account, _weight, _id);
    ...
    function mintBatch(
      ...
      external {
         ...
         collection.mintBatch(account, _weights, _ids, amount);
        ...
        for (uint256 index = 0; index < numTokens; ++index) {
            ...
            if (enabled) {
                ...
                // and unstaking right after the start of the new cycle
                       ...
                       ///#####################THIS CODE PASSIVE ONLY FOR RINKEBY#######################//
                      //require(currentCycle - tokenInfo.depositCycle >= 2, "NftStaking: token still frozen");
                     ///#####################THIS CODE PASSIVE ONLY FOR RINKEBY#######################//
