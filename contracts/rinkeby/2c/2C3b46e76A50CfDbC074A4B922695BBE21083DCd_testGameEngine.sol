...
// File: HungerBrainz-updated/contracts/base/ReentrancyGuard.sol
...
// File: HungerBrainz-updated/contracts/base/ISUPFactory.sol
...
// File: HungerBrainz-updated/contracts/TestContracts/testGameEngine.sol
...
//VITAL INTERNAL FUNCITONS
    function claimStake ( uint tokenId ) internal returns (bool){
        ...
        if (stakeConfirmation[tokenId] == false) {
            //console.log("inside stake confirmation is false");
            require (block.timestamp >= firstStakeLockPeriod[tokenId]);
            //console.log("stake period cleared");
    ...
    function unstakeNFT ( uint tokenId ) internal {
        ...
        if (stakeConfirmation[tokenId] == true) {
            ...
            //console.log("Unstake token received");
            uint burnPercent = unstakeBurnCalculator(level);
            //console.log("Burn percent calculated");
    ...
    function claimAndUnstake (bool claim,uint[] memory tokenAmount) external nonReentrant{
        ...
        for (uint i=0;i<tokenAmount.length;i++) {
            ...
            //console.log("claim and Unstake",i);
            if (claim == true) {
                claimStake(tokenAmount[i]);
                //console.log("Got claimed",i);
            }
            else {
                //console.log("Trying Claim",i);
                bool isBurnt = claimStake(tokenAmount[i]);
                //console.log("Claimed",i);
                if (isBurnt == false)
                {
                    //console.log("Trying Unstake",i);
                    unstakeNFT(tokenAmount[i]);
                    isStaked[tokenAmount[i]] = false;
                    //console.log("Got unstaked",i);
                }
