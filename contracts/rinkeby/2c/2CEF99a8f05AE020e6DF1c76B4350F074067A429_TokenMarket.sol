    ...
    */
    ...
    function activateLoan(uint256[] memory loanIds, uint256[] memory stableCoinAmounts, bool[] memory _autoSell) public {
        ...
        activatedLoanOffersFull[loanIds[i]] =  TokenLoanData.LenderDetails({
		    lender: msg.sender,
            activationLoanTimeStamp: block.timestamp,       //block.timestamp, //TODO change time to block.timestamp
    ...
    function getTotalPaybackAmount(uint256 _loanId) external view returns (uint256, uint256) {
        ...
        // TODO Error
        uint256 platformFee = apyFeeOriginal.mul(govWorldProtocolRegistry.getGovPlatformFee() * EXPO).div(BASE);
        uint256 apyFeeAfterPlatformFeeCut = apyFeeOriginal.sub(platformFee);
    ...
    function liquidateLoan(uint256 _loanId) public onlyLiquidatorRole(msg.sender) {
        ...
        // require(this.isLiquidationPending(_loanId) || (loanTermLengthPassedInDays > loanDetails.termsLengthInDays), "GTM: Liquidation Error"); TODO //uncomment this line befor  deployment
