...
contract MetacaskNFT is ERC721, Ownable, WhiteList {
  ...
  /**
   * @dev Emitted when `owner` adds `commissionAddress` to `tokenId` token.
   */
  event SetCommission(address indexed owner, address indexed commissionAddress, uint256 indexed tokenId);

  constructor() public ERC721("MetacaskNFT", "MCK") {
   ...
   */
  function mintItem(address to, string memory tokenURI, address commissionAddress, uint256 commissionRate)
      public
      onlyOwner
      onlyWhiteListed(to)
      returns (uint256)
