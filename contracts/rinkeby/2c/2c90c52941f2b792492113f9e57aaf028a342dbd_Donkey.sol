...
contract Donkey is ERC721, Ownable {
    ...
    uint16 public MAX_DONKEY = 10000; // to save gas, some place used value instead of var, so be careful during changing this value
    ...
    function mintDonkeyWithOutSign(uint numberOfTokens) public payable {
        // uint256 totalSupply = totalSupply();
        require(publicSaleIsActive, "Sale must be active to mint Donkey");
        require(numberOfTokens <= maxDonkeyPurchase, "Can only mint 20 tokens at a time");
        require(totalSupply + numberOfTokens < 10001, "Purchase would exceed max supply of Donkeys"); // ref MAX_DONKEY
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply < 10000) { // ref MAX_DONKEY
