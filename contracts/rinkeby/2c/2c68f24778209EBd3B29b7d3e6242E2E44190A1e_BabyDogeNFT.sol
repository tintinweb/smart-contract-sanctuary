...
contract BabyDogeNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public dogePrice = 800000000000000; //0.0008 ETH

    uint256 public maxDogePurchase = 20;
    ...
    bool public poolIsActive = false;
    ...
    function setBaseURI(string memory _baseURILink) public onlyOwner {
        _baseTokenURI = _baseURILink;
    }

    function sendToPool(uint256 _id) public nonReentrant {
        require(poolIsActive == true, "Pool Locked!");
        require(circulatingSupply != 0);
        transferFrom(msg.sender, address(this), _id);
        uint256 balance = IERC20(stablecoin).balanceOf(address(this));
        IERC20(stablecoin).transfer(msg.sender, balance.div(circulatingSupply));
        circulatingSupply = circulatingSupply.sub(1);
    }
    ...
    function flipPoolState() public onlyOwner {
        poolIsActive = !poolIsActive;
    }

    function convertETHToUSD() public onlyOwner {
        _swapTokens(
            stablecoin,
            _getQuote(
     ...
     */
    function setMaxDogePurchase(uint256 _maxDogePurchase) public onlyOwner {
        maxDogePurchase = _maxDogePurchase;
    }

    function setDogePrice(uint256 _dogePrice) public onlyOwner {
    ...
    function mintDoge(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Doge");
        require(
            numberOfTokens <= maxDogePurchase,
            "Check how many you can mint at a time"
        );
        require(
            totalSupply().add(numberOfTokens) <= MAX_DOGES,
            "Purchase would exceed max supply of Doges"
        );
        require(
            dogePrice.mul(numberOfTokens) <= msg.value,
