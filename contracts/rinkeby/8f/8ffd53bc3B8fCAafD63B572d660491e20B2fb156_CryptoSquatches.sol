...
contract CryptoSquatches is ERC721, Ownable {
    ...
    */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory base = baseURI();
        //string memory tokenURIWithOffset = tokenId.toString(); //((tokenId + startingIndex) % MAX_CRYPTO_SQUATCH_SUPPLY).toString();

        return string(abi.encodePacked(base, (totalSupply() + 1).toString()));
    }
