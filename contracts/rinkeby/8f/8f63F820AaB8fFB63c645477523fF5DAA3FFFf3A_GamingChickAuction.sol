...
contract GamingChickAuction is OwnableContract, IERC721Receiver, Initializable{
    ...
    event GetScore(address user, uint256 normalLength, uint256 specialLength);
    ...
    function initialize() public initializer{
        ...
        poolInfo[0].token = 0xDD81bfB457E27e36043aC46725EA0fBd2013210d;
        poolInfo[1].token = 0xbEf15aA877adEd8F2e72e9dcc188AeF6Adb3E8e6;
        poolInfo[2].token = 0x97d48881A7C5c36d103a54Df537c105E8348d4f4;
        poolInfo[3].token = 0x7Ac58167E25227B22EeA7bBe7F78ada1e60A8390;
    ...
    function removeWinnerRecords() public onlyAdmin {
        require(now > freezeTime && now > updBatchTime + 300, "Operation not allowed");
        uint256 id = batchId - 1;
        for (uint256 i = 0; i < winnerList[id].length; i++) {
            delete userInfo[0][winnerList[id][i]];
            delete userInfo[1][winnerList[id][i]];
            delete userInfo[2][winnerList[id][i]];
            delete userInfo[3][winnerList[id][i]];

            totalScoreList[winnerList[id][i]] = 0;
        }
    }
    ...
    function withdrawNFT(uint256 _pid, uint32 _amount) public {
        // require(now >= freezeTime, "Operation not allowed");
        require(_pid < poolInfo.length, "invalid _pid");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function setScore() internal {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            ...
            uint32 baseScore = 0;
            if (i == 0) {
                baseScore = user.amount * 900;
            } else if (i == 1) {
                baseScore = user.amount * 140;
            } else if (i == 2) {
                baseScore = user.amount * 60;
            } else if (i == 3) {
                baseScore = user.amount * 10;
            }
            user.baseScore = baseScore;

            for (uint256 j = 0; j < user.specialAttr.length(); j++) {
                uint32 specialAttrScore = 0;
                (, uint256 y) = user.specialAttr.at(j);
                uint32 count = uint32(y);
                if (i == 0) {
                    specialAttrScore = count * 270;
                    if (count >= 2 && count < 3) {
                        specialAttrScore += 100;
                    } else if (count >= 3 && count < 5) {
                        specialAttrScore += 150;
                    } else if (count >= 5 && count < 7) {
                        specialAttrScore += 200;
                    } else if (count >= 7) {
                        specialAttrScore += 300;
                    }
                } else if (i == 1) {
                    specialAttrScore = count * 28;
                    if (count >= 2 && count < 4) {
                        specialAttrScore += 40;
            ...
            for (uint256 j = 0; j < user.normalAttr.length(); j++) {
                ...
                (, uint256 y) = user.normalAttr.at(j);
                uint32 count = uint32(y);
                if (i == 0) {
                    if (count >= 2 && count < 3) {
                        sameAttrScore += 100;
    ...
    function updNftAttrAmount(uint256 _pid, uint16 _attr, uint256 _type) internal {
        ...
        if (specialAttr[_pid][_attr]) {
            if (user.specialAttr.contains(uint256(_attr))) {
                if (_type == 1) {
            ...
            if (user.normalAttr.contains(uint256(_attr))) {
                if (_type == 1) {
    ...
    function getScoreList(uint256 _pid, address _user) public view returns (
        ...
        for (uint256 i = 0; i < attrListLength; i++) {
            if (i < normalAttrListLength) {
                (uint256 x, uint256 y) = user.normalAttr.at(i);
                nftAttrKey[i] = x;
                nftAttrValue[i] = y;
            } else {
                (uint256 x, uint256 y) = user.specialAttr.at(attrListLength - 1 - i);
                nftAttrKey[i] = x;
                nftAttrValue[i] = y;
            }
        }
