...
contract mintingTokens is ERC721Enumerable, PaymentSplitter, Ownable {
    using Strings for uint;

    uint public PRICE = 0.07 ether;
    uint public MAX_TOKENS_PER_TRANSACTION = 20;
    uint public MAX_SUPPLY = 10000;
    uint public freeLimit = 1000;
    bool public revealed = false;

    string public notRevealedUri;

    string public _baseTokenURI = 'ipfs://QmeFvL8amR1koiz5sRwH26FDyiB6PPU6p7RfwYCdaKvesd/'; 
    string public _baseTokenSuffix = '.json';

    bool public paused = false;

    // Withdrawal addresses
    address dev = 0x6c7e14F4d8A6143380d8f268648D34BD8535bF44;
    address art = 0xeAE2D9EA270680F0c24843cB0e5D7FF12eDFBeE5;

    address[] addressList = [dev, art];
    uint[] shareList = [9500, 500];
    ...
    function mint(uint _count) external payable {
        ...
        bool ifSenderIsWhiteListed = whitelist[msg.sender];
        require( supply + _count <= MAX_SUPPLY,        "Exceeds max Fierce Model supply." );
        //mint without minting charge
        if(ifSenderIsWhiteListed || supply <= freeLimit){
    ...
    function mintTo(uint[] calldata quantity, address[] calldata recipient) external payable {
        ...
        for(uint i; i < recipient.length; ++i){
            for(uint j; j < quantity[i]; ++j){
                _safeMint( recipient[i], supply + 1, "" );
            }
        }
    }
    ...
    function setmaxMintAmount(uint _newMaxTokensPerTransaction) public  {
        MAX_TOKENS_PER_TRANSACTION = _newMaxTokensPerTransaction;
    }

    function pause(bool _updatePaused) public  {
