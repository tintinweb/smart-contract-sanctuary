...
contract AOToken is Ownable, ERC721Enumerable {
    ...
    uint256 public seedNumber = 85600054384035903;
    ...
    constructor() ERC721("test", "testttsts") {}
    ...
    function claim(uint16 _amount) external payable {
        ...
        require(
            publicClaimed + whitelistClaimed + presaleClaimed + _amount <=
                MAX_SALE_SUPPLY,
            "Exceed total"
        );
        ...
        returns (string memory)
    {
        ...
        if (bytes(tokens[tokenId].uri).length > 0 && enableTokenURI) {
            return
                string(
                    abi.encodePacked(tokens[tokenId].uri, tokenId.toString())
                );
        }
    ...
    function setTokenBaseURI(uint16[] calldata _tokenIds, string calldata _uri)
        external
        onlyOwner
    {
