...
contract MockDAI is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    // --- ERC20 Data ---
    string  public constant name     = "MockDai Stablecoin";
    string  public constant symbol   = "MockDAI";
    string  public constant version  = "1";
    uint8   public constant decimals = 18;
    uint256 public totalSupply;
    uint public dailyMockDAILimit;
    ...
    constructor(uint256 chainId_) {
        ...
        dailyMockDAILimit = 10000000000000000000000;
    }
    ...
    function adjustDailyMockDAILimit(uint _limit) external auth {
        dailyMockDAILimit = _limit;
    }

    function mint(address usr, uint wad) external {

      if(wards[msg.sender] == 0) {
        require(add(wad, daiMintedToday[msg.sender]) <= dailyMockDAILimit || sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= dailyMockDAILimit, "Over daily MockDAI Limit");
