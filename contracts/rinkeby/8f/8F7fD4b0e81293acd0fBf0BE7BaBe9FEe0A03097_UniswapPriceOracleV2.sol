...
// File: contracts\UniswapPriceOracleV2.sol
 ...
 */
    ...
    function getMarketPrice(string memory symbol) external view returns (uint) {
        // 算市场价格
        TokenConfig memory config = getTokenConfigBySymbol(symbol);
        uint marketPrice = 0;
        if(config.priceSource == PriceSource.FIXED_USD){ // Fixed price
            marketPrice = config.fixedPrice;
        }else if(config.priceSource == PriceSource.REPORTER_BNB){ // For BNB, it is divided into two steps, first calculate the pair of BNB, and then convert it into u, and u decimal place is 6 bits
    ...
    function fetchMarketBnbPrice(TokenConfig memory config, uint bnbPrice) internal view virtual returns (uint) {
        ...
        if(config.isPrice1FromUniswap){ // if isPrice1FromUniswap is true, reserve0 / reserve1 (At this point, reserve0 is the transaction currency), else reserve1 / reserve0
            if(reserve1 != 0){
    ...
    function fetchMarketPriceUSD(TokenConfig memory config) internal view virtual returns (uint) {
        uint marketPrice = 0;
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IUniswapV2Pair(config.uniswapMarket).getReserves();
        if(config.isPrice1FromUniswap){ // if isPrice1FromUniswap is true, reserve0 / reserve1 (At this point, reserve0 is the pricing currency), else reserve1 / reserve0
            if(reserve1 != 0){
