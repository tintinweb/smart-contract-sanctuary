...
// wagmi.
contract Roles is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: monospace; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" />';

        parts[1] = '<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" class="base">';

        parts[2] = getPrefix(tokenId);

        parts[3] = '</text><text x="10" y="60" class="base">';

        parts[4] = getRoot(tokenId);

        parts[5] = '</text><text x="10" y="80" class="base">';

        parts[6] = getSuffix(tokenId);

        parts[7] = '.</text></svg>';

        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7]));

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Role #', toString(tokenId), '", "description": "Now ssigning on-chain generative roles for the metaverse and beyond. 6969 available in total. Rarity is mostly up to you.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("Roles", "ROLES") Ownable() {}
}
