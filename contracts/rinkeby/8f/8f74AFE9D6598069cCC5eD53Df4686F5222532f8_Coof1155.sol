...
contract Coof1155 is PermitControl, OnChainMeta, ERC165, IERC1155, IERC1155MetadataURI {
  ...
  */
  function setURI(string calldata _uri) external virtual
    hasValidPermit(UNIVERSAL, keccak256("SET_URI")) {
    require(!uriLocked,
      "Coof1155: the collection URI has been permanently locked");
  ...
  */
  ...
  returns (uint256) {
    require(_owner != address(0),
      "Coof1155: balance query for the zero address");
  ...
  */
  function balanceOfBatch(address[] calldata _owners, uint256[] calldata _ids)
    ...
    require(_owners.length == _ids.length,
      "Coof1155: accounts and ids length mismatch");
  ...
  */
  function setApprovalForAll(address _operator, bool _approved) external
    ...
    require(_msgSender() != _operator,
      "Coof1155: setting approval status for self");
  ...
  */
  function safeTransferFrom(address _from, address _to, uint256 _id,
    ...
    require(_to != address(0),
      "Coof1155: transfer to the zero address");
    require(_from == _msgSender() || isApprovedForAll(_from, _msgSender()),
      "Coof1155: caller is not owner nor approved");
    ...
    balances[_id][_from] = balances[_id][_from].sub(_amount,
      "Coof1155: insufficient balance for transfer");
  ...
  */
  function safeBatchTransferFrom(address _from, address _to,
    ...
    require(_ids.length == _amounts.length,
      "Coof1155: ids and amounts length mismatch");
    require(_to != address(0),
      "Coof1155: transfer to the zero address");
    require(_from == _msgSender() || isApprovedForAll(_from, _msgSender()),
      "Coof1155: caller is not owner nor approved");
  ...
  */
  function coof(uint256 _coofId, address[] calldata _hosts) external {
    require(balanceOf(_msgSender(), _coofId) > 0, "Coof1155: cant spread the coof if you dont have it");
    require(_hosts.length < rNaught, "Coof1155: spreading to too many hosts");
  ...
  */
  function vaccinate(uint256 _vaxId) external {
    require(vaxx.balanceOf(_msgSender(), _vaxId) > 0, "Coof1155: cant vax if you dont have one");
    ...
    // Edit an existing item group. The name may always be updated.
      ...
      if (itemGroups[_groupId].supplyType == SupplyType.Capped) {
        require(_data.supplyType == SupplyType.Capped,
          "Coof1155: you may not uncap a capped supply type");
        require(_data.supplyData <= itemGroups[_groupId].supplyData,
          "Coof1155: you may not increase the supply of a capped type");
      ...
      require(_data.supplyData >= itemGroups[_groupId].circulatingSupply,
        "Coof1155: you may not decrease supply below the circulating amount");
      ...
      if (itemGroups[_groupId].itemType == ItemType.Nonfungible) {
        require(_data.itemType == ItemType.Nonfungible,
          "Coof1155: you may not alter nonfungible items");
      ...
      // A fungible item may change type if it is unique enough.
        ...
        if (_data.itemType == ItemType.Nonfungible) {
          require(itemGroups[_groupId].circulatingSupply <= 1,
            "Coof1155: the fungible item is not unique enough to change");
  ...
  */
  ...
  //     "Coof1155: you cannot mint a non-existent item group");
  //
  ...
  //       "Coof1155: you cannot mint a group beyond its cap");
  //   }
  //
  ...
  //       "Coof1155: you cannot mint more than a single nonfungible item");
  //
  ...
  //       "Coof1155: you cannot mint more than the alloted semifungible items");
  //   }
  //
  ...
  */
  function mintBatch(address _recipient, uint256[] calldata _ids,
    ...
    require(_recipient != address(0),
      "Coof1155: mint to the zero address");
    require(_ids.length == _amounts.length,
      "Coof1155: ids and amounts length mismatch");
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      require(_hasItemRight(_ids[i], keccak256("MINT")),
        "Coof1155: you do not have the right to mint that item");
  ...
  */
  function setMetadata(uint256 _id, string memory _metadata)
    external hasItemRight(_id, keccak256("SET_METADATA")) {
    require(!uriLocked && !metadataFrozen[_id],
      "Coof1155: you cannot edit this metadata because it is frozen");
  ...
  */
  function lockURI(string calldata _uri) external
    hasValidPermit(UNIVERSAL, keccak256("LOCK_URI")) {
    string memory oldURI = metadataUri;
    metadataUri = _uri;
    emit ChangeURI(oldURI, _uri);
    uriLocked = true;
  ...
  */
  function lockItemGroupURI(string calldata _uri, uint256 _id) external
    hasItemRight(_id, keccak256("LOCK_URI")) {
  ...
  */
  function lock() external virtual hasValidPermit(UNIVERSAL, keccak256("LOCK_URI")) {
    locked = true;
...
contract OnChainMeta {
    ...
    function _buildName(uint256 _tokenId) internal view returns (string memory) {
      ...
      return string(abi.encodePacked("zr", groupId.toString()));
    }
    ...
    // function _imageData(uint256 _tokenId) internal view returns (string memory) {
    //   string memory image = Base64.encode(bytes(_generateSVGImage(_tokenId)));
    //   return image;
    // }
...
library NFTSVG {
  ...
  function _generateSVGDefs() private view returns (string memory svgDef) {
    ...
    for(uint256 i = 0; i < 6; ++i){
      ...
      subDefs = string(abi.encodePacked(
        ...
        subIndex.toString(),
        '" >',
        '<animate attributeName="d" values="m240,230l0,0 0,0;m190,190l30,-17 30,17;m110,60l0,0 0,0;" dur="12s" repeatCount="indefinite" />',
        '<animate attributeName="fill" values="#0f0;#fff;" dur="12s" repeatCount="indefinite" begin="p1.begin+',
        (subIndex * 2).toString(),
        's"/>',
        '<animate attributeName="stroke" values="#0f0;#fff;" dur="12s" repeatCount="indefinite" begin="p1.begin+',
        (subIndex * 2).toString(),
        's"/>',
        '</path>'
      ));
    ...
    svgDef = string(
      abi.encodePacked(
        '<svg version="1.1" width="1000" height="1000" viewBox="0 0 320 320"  stroke-linecap="round" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
        '<defs>',
        '<path id="r1" >',
        '<animate id="p1" attributeName="d" values="m240,230l0,0 0,0;m190,190l30,-17 30,17;m110,60l0,0 0,0;" dur="12s" repeatCount="indefinite" />',
        '<animate attributeName="fill" values="#ff0;#f00;#f0f;#00f;#0ff;#0f0;#ff0;" dur="12s" repeatCount="indefinite" begin="p1.begin"/>',
        '<animate attributeName="stroke" values="#ff0;#f00;#f0f;#00f;#0ff;#0f0;#ff0;" dur="12s" repeatCount="indefinite" begin="p1.begin"/>',
        '</path>',
        subDefs,
