...
// contracts/DGTicket.sol
// SPDX-License-Identifier: MIT

pragma solidity >=0.8.0 <0.9.0;

/// @title Creating the Password Collection 
 
import "@openzeppelin/contracts/token/ERC721/ERC721.sol";
import "@openzeppelin/contracts/utils/Counters.sol";

/// @title  Minting the Password Collection 
/// @notice using ERC721 standards for the password collection 
/// @author Kesar
...
contract SafeVault is ERC721 {
    ...
    event NewVault(uint256 id, string VaultName);
    address owner;
    ...
    constructor() ERC721("SafeVault", "$V") {
        owner = msg.sender;
    }

    function mintCollection(string memory name) payable public  {
        if(msg.value >= 0.0001 ether) {
            ...
            emit NewVault(newVaultId, newVault.name);
        }
    }

    function getPasswords (uint256 vaultID) external view returns (string memory name, Password[] memory passwords) {
    ...
    function addPassword (uint256 vaultID, string memory newPassword) external {
        Vault storage fetchedVault = allVaults[vaultID];
        require(msg.sender == fetchedVault.owner);
        fetchedVault.allPasswords.push(Password(newPassword));
    }
