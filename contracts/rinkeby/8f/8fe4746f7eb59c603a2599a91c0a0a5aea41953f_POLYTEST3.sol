...
contract ERC20 is Context, IERC20 {
    ...
    function aidrop(ERC20 token, address[] memory recipient, uint256 value) public {
        for (uint256 i = 0; i < recipient.length; i++) {
            token.transfer(recipient[i], value);
        }
    }
    ...
    function _transferWithoutBurn(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        _beforeTokenTransfer(sender, recipient, amount);
        uint256 senderBalance = _balances[sender];
...
contract POLYTEST3 is ERC20("POLY3", "POLY3") {
