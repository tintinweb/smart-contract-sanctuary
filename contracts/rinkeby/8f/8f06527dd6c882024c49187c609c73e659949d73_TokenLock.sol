...
contract TokenLock {
    ...
    function release(uint256 releaseWho) public virtual {
        ...
        if(2 == releaseWho){
            _releaseCommunityRewards(treasury);
        }
        if(3 == releaseWho){
            _releaseCommunityRewards(strategicInvestorPhaseSeed);
        }
        if(4 == releaseWho){
            _releaseCommunityRewards(strategicInvestorPhasePrivate);
        }
        if(5 == releaseWho){
            _releaseCommunityRewards(partnershipEcosystemIncentive);
        }
    }
    ...
    function getTimeReleaseTeamAndConsultants() public view virtual returns (uint256 releaseTime) {
        if(teamAndConsultantsNumber < 24){
            ...
            releaseTime = _teamAndConsultantsTime + 4 * months;
        }
    }
    ...
    function _releaseTeamAndConsultants(address user) internal virtual returns (bool) {
        ...
        if(teamAndConsultantsNumber < 24){
            ...
            releaseTime = _treasuryTime + 4*months;
        }
