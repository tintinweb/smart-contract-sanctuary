...
contract ARTGLIXXX is ERC721, Ownable {
    ...
    // address private GLICPIXXXVER002_ADDRESS = 0x31385d3520bCED94f77AaE104b406994D8F2168C;
    // address private GLIXTOKEN_ADDRESS = 0x31385d3520bCED94f77AaE104b406994D8F2168C;
    address private GLICPIXXXVER002_ADDRESS = 0x7C86BA968Fe2d68fE15A5871984C12acE818f04F;
    address private GLIXTOKEN_ADDRESS = 0x767bF775de0CA0035275F4Ffa6810507544e1844;
    ...
    mapping(uint => uint) public ARTGLIXXX_GEN;

    mapping(address => uint256) public GLIXREWARDS;
	mapping(address => uint256) public GLIX_LAST_UPDATE;
    uint public BASE_YIELD_RATE = 10 ether; // 10 GLIX PER DAY
    uint public YIELD_END = block.timestamp + 315360000; // 10 years // 

    event RewardPaid(address indexed user, uint256 reward);

    constructor() ERC721("ARTGLIXXX","ARTGLIXXX")  {
        // YIELD_END = block.timestamp + 315360000; // 10 years
    }
    ...
    function tokenHash(uint256 tokenId) public view returns(bytes32){
        ...
        return bytes32(keccak256(abi.encodePacked(address(this), creationDates[tokenId], creators[tokenId], tokenId, ARTGLIXXX_GEN[tokenId])));
    }
    ...
    function calculateMint(uint _id) public view returns(uint) {
        return ARTGLIXXX_GEN[_id] * BASE_MINT_PRICE;
    }

    function mintArtGLIXXXX(uint _id) public {
        require(mintOpen == true, "Minting isn't open");
        require(GLICPIXXXVER002.ownerOf(_id) == msg.sender, "Not owner of GLICPIXXX");
        uint mintPrice = calculateMint(_id);
        if (mintPrice > 0) {
        require(GLIXTOKEN.balanceOf(msg.sender) > mintPrice);
        GLIXTOKEN.burn(msg.sender, mintPrice);       
        }
        uint mintIndex = _id + (TOTAL_GLICPIXXX * ARTGLIXXX_GEN[_id]);
	...
	function updateGlixRewardOnMint(address _user) internal {
		...
		if (timerUser > 0)
			GLIXREWARDS[_user] += balanceGENESIS[_user] * BASE_YIELD_RATE * (time - timerUser) / 86400;
	...
	function updateGlixReward(address _from, address _to, uint256 _tokenId) internal {
		if (_tokenId < TOTAL_GLICPIXXX) {
			...
			if (timerFrom > 0)
				GLIXREWARDS[_from] += balanceGENESIS[_from] * BASE_YIELD_RATE * (time - timerFrom) / 86400;
			if (timerFrom != YIELD_END)
				GLIX_LAST_UPDATE[_from] = time;
			if (_to != address(0)) {
				uint256 timerTo = GLIX_LAST_UPDATE[_to];
				if (timerTo > 0)
					GLIXREWARDS[_to] += balanceGENESIS[_to] * BASE_YIELD_RATE * (time - timerTo) / 86400;
    ...
    function getReward(address _to) internal {
		uint256 reward = GLIXREWARDS[_to];
		if (reward > 0) {
			GLIXREWARDS[_to] = 0;
			GLIXTOKEN.mint(_to, reward);
