...
contract CircleGame is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    function mint(address account, uint256 coinId, uint256 amount)
        ...
        payable
    {
        require(coinId == 0, "ID must be 0. Upgrade to access higher tiers");
        ...
        uint256 currMintPrice = (amount - 1).mul(10000000000000).div(2).add(initialClaimPrice);
        require(currMintPrice <= msg.value, "ETH value sent is not enough.");

        //Update mint price and numClaimed counter
        numClaimed = numClaimed.add(amount);
        initialClaimPrice = initialClaimPrice.add((amount).mul(10000000000000));

        //Mint the tokens
        _mint(account, TIER_SIX_ORANGE, amount, "");
    }
    ...
    function claimStake(uint[] memory numberOfTokens) public {
        ...
        uint adjustedNumBurned = 0;
        uint totalReward = 0;
        for (uint i = 1; i < 6; i++) {
            if (numberOfTokens[i] == 0) continue;
            /* Total possible pot is 60% for Tier 1, 70% for Tier 2, up to 100% for Tier 5*/
            uint256 totalPot = address(this).balance.div(10).mul(5 + i);

            /* Calculate payout according to numBurned, numClaimed, numberOfTokens, and coinId.  */
            uint256 adjustedNumberOfTokens = numberOfTokens[i] * 5 ** i;
            totalReward.add(totalPot.div(numClaimed.sub(numBurned)).mul(adjustedNumberOfTokens));
            adjustedNumBurned.add(adjustedNumberOfTokens);
            _burn(msg.sender, i, numberOfTokens[i]);
        }
        
        /* Burn tokens and payout reward */
        numBurned = numBurned.add(adjustedNumBurned);
