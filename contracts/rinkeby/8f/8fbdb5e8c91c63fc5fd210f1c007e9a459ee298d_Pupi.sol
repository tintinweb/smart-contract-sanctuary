...
contract Pupi is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private randomNouns = [
        ...
        "fartbox",
        "dale chihuli",
        "lewis and clark's dog seaman",
        "diddler on the roof",
        "hobo shack",
        "nipples",
        // the following are nft nouns
        "my wife's boyfriend",
        "vitalik",
        "bored ape",
        "punk",
    ...
    uint256 public price_reg = 10000000000000000; //0.02 ETH
    ...
    function getRandomLine(uint256 tokenId, uint256 offset) public view returns (string memory) {
        return pluck(tokenId, "RANDOM", randomLines, offset);
    }
    
    function getFollowedByAnother(uint256 tokenId, uint256 offset) public view returns (string memory) {
        return pluck(tokenId, "FANOTHER", followedByAnother, offset);
    }
    
    function getFollowedByNoun(uint256 tokenId, uint256 offset) public view returns (string memory) {
        return pluck(tokenId, "FNOUN", followedByNoun, offset);
    }
    
    function getRandomNoun(uint256 tokenId, uint256 offset) public view returns (string memory) {
        return pluck(tokenId, "RNOUN", randomNouns, offset);
    }


    function pluck(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray, uint256 offset) internal pure returns (string memory) {
    ...
    function decideSize(uint256 tokenId) private pure returns (uint) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory output) {
        ...
        uint partsLen = 0;
        uint256 startY = 45;
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: #ffffff; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="#ffa9d8" /><text x="40" y="45" class="base">';
        partsLen += 1;
        for (uint i = 0; i < size; i++) {
            parts[partsLen] = conditionalFunctionCall(calls[i], tokenId, i);
            partsLen += 1;
            if (i == size - 1) {
                startY += 30;
                parts[partsLen] = string(abi.encodePacked('</text><text x="40" y="', toString(startY), '" class="base">'));
                partsLen += 1;
        ...
        if (size == 2) {
            ...
            string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "poem #', toString(tokenId), '", "description": "pupi kaur poetry. is a collection of. 10,000 very-serious on-chain poems. -pupi kaur", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function mint(uint256 tokenId) public payable nonReentrant {
        ...
        require(price_reg <= msg.value, "Ether value sent is not correct");
        _safeMint(_msgSender(), tokenId);
    }

    function multiMint(uint256[] memory tokenIds) public payable nonReentrant {
        require((price_reg * tokenIds.length) <= msg.value, "Ether value sent is not correct");
    ...
    constructor() ERC721("Pupi Kaur FINAL", "PUPI") Ownable() {}
}
