...
contract AdvancedCollectible is ERC721URIStorage, VRFConsumerBase {
    ...
    // best practice: whenever updating a mapping, emit an event
    event requestedCollectable(bytes32 indexed requestId, address requester);
    event colorAssigned(uint256 indexed tokenId, COLOR color);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        COLOR color = COLOR(randomNumber % 2);
        uint256 newTokenId = tokenCounter;
        tokenIdToColor[newTokenId] = color;
        emit colorAssigned(newTokenId, color);
