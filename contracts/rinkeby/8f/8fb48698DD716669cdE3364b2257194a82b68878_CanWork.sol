...
// interface IPapr {
//     function balanceOf(address test)
//         external
//         view
//         returns (uint256 myAddress);
// }
...
contract Escrow is Migratable {
    ...
    // function getPaprTaxRate() public view returns (uint256) {
    //   return IPapr(escrowToken).balanceOf(escrowToken);
    // }
    ...
    returns (uint256) {
        ...
        uint256 _computeAmount = _amount.div(1e18).div(10);
        uint256 _amountAfterBurn = _amount.div(1e18).sub(_computeAmount);
        require(escrowToken.transferFrom(_client, address(this), _amountAfterBurn), "Client must have authorisation and balance to transfer PAPR");
        ...
        escrow.amount = _amountAfterBurn;
        ...
        emit OnCreateEscrow(escrowDapp, _client, _provider, _amountAfterBurn, daiValueInEscrow);
