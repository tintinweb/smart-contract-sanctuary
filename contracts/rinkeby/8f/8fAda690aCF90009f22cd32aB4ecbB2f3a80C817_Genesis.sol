...
contract Genesis is ERC721, Ownable {
    ...
    uint256 public publicSaleStartTime = 1644467451;
     ...
     * @param _mintAmount Amount of NFTs to mint
     */
    function publicMint(uint256 _mintAmount) public payable {
        uint256 s = tokenSupply.current();
        require(!paused);
        require(isPublicSaleOpen(), "Genesis: Public Sale is not open");
        require(
            s + _mintAmount <= maxSupply,
            "Genesis: Total mint amount exceeded"
        );
        require(_mintAmount > 0, "Genesis: Please mint atleast one NFT");
        require(
            msg.value == cost * _mintAmount,
            "Genesis: not enough ether sent for mint amount"
        );

        (bool successT, ) = treasury.call{value: msg.value}(""); // forward amount to treasury wallet
        require(successT, "Genesis: not able to forward msg value to treasury");
        delete successT;
    ...
    /**
     * @notice Whitelist Only Mint function
     * @dev Contract must not be paused
     * @dev Public Sale must be open
     * @dev Mint must not exceed max Supply
     * @dev Mint Amount must be greater than 0
    ...
    function whitelistMint(
        ...
        require(!_nonceUsed[nonce], "Genesis: Nonce already used");
        _nonceUsed[nonce] = true;

        require(
            whitelistSigned(msg.sender, nonce, signature),
            "Genesis: Invalid signature"
        );
        // TODO: Create Whitelist Sale Window
        require(isPublicSaleOpen(), "Genesis: Public Sale is not open");
        require(
            s + _mintAmount <= maxSupply,
            "Genesis: Total mint amount exceeded"
        );
        require(_mintAmount > 0, "Genesis: Please mint at least one NFT");
        require(
            msg.value == cost * _mintAmount,
            "Genesis: not enough ether sent for mint amount"
        );

        // TODO: Create whitelist mint limit?
        // TODO: Create whitelist mint limit per transaction?
        ...
        // TODO: Create Whitelist Mint Event
        emit PublicMint(msg.sender, _mintAmount);
    }

    /**
     * @notice Determine if public sale is open
     * @return bool
     */
    ...
    /**
     * @notice Burn NFT with token id = `_tokenId`
    ...
    //*** OnlyOwner Functions ***//
    /**
     * @dev Toggles metadata revealed state
     */
    ...
    /**
     * @dev Set when public sales begins in UNIX time
     */
