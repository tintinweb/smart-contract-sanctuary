...
contract Chains is ERC721Enumerable, Ownable {
    ...
    **/
    ...
    constructor(address genOneChainsAddress) ERC721("AlphaChains", "AC") {
        _genOneChainsAddress = genOneChainsAddress;
        _owner = msg.sender;
        //Declare all the rarity tiers
        //Clarity
        TIERS[0] = [3, 27, 120, 180, 270];
        //Gem
        TIERS[1] = [3, 6, 45, 60, 66, 72, 78, 84, 90, 96];
        //Chain Gems
        TIERS[2] = [3, 6, 12, 48, 66, 72, 78, 87, 99, 129];
        //Chain
        TIERS[3] = [3, 6, 24, 87, 102, 114, 126, 138];
        //Watermark
        TIERS[4] = [3, 6, 12, 12, 12, 18, 30, 507];
        //Background
        TIERS[5] = [3, 6, 12, 12, 12, 18, 30, 507];
    }

    /**
     * @dev Converts a digit from 0 - 650 into its corresponding rarity based on the given rarity tier.
     * @param _randInput The input from 0 - 650 to use for rarity gen.
    ...
    function hash(
        ...
        for (uint8 i = 0; i < 6; i++) {
            uint16 _randInput = uint16(_largeRandom % 600);
            _largeRandom = _largeRandom / 600;
    ...
    function mintInternal() internal {
        ...
        tokenIdToRarityCount[thisTokenId] += 8;
     ...
     */
    function mintWithBallerBars(uint256 tokenQuantity,uint256 ballerBarsGeneration) public {
        ...
        returns (string memory)
    {
        ...
        metadataString = string(
            abi.encodePacked(
                ...
                ChainsLibrary.toString(tokenIdToRarityCount[_tokenId]),'}'
            )
        );
    ...
    /**
     * @dev Overridden function to return total supply of both generations of contracts
     */
    function totalSupply() public view virtual override returns (uint256) {
        return ERC721Enumerable.totalSupply() + IChains(_genOneChainsAddress).totalSupply();
    }
