...
contract OlympicsStaking {
    ...
    function claim() public whenLocked canClaim {
        if (winner == 0) {
            userCanWithdraw = totalReward.mul(ledger0[msg.sender]).div(TVL0) ;
            ...
            userCanWithdraw = totalReward.mul(ledger1[msg.sender]).div(TVL1) ;
