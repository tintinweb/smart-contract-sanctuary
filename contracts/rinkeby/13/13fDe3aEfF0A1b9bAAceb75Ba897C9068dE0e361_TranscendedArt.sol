...
contract TranscendedArt is ERC721, ReentrancyGuard, Ownable {
    ...
    modifier onlyAdminOrWhitelisted() {
        require(
            isWhitelisted[msg.sender] || msg.sender == admin
        );
        _;
    }

    constructor() ERC721("TranscendedArt", "TRANCENDEDART") {
        admin = msg.sender;
    ...
    function mint(uint256 _projectId, uint256 numToMint)
        ...
        nonReentrant
    {
        require(numToMint > 0, "Must mint at least one artwork");
        require(
            projects[_projectId].invocations + numToMint <=
                projects[_projectId].maxInvocations,
            "Must not exceed max invocations"
        );
        require(projects[_projectId].active, "inactive");
        require(!projects[_projectId].paused, "paused.");
    ...
    function batchMint(uint256 _projectId, uint256 numToMint) internal {
        for (uint256 i = 0; i < numToMint; i++) {
            ...
            projects[_projectId].invocations = projects[_projectId]
                .invocations + 1;
            _safeMint(msg.sender, tokenIdToBe);
            tokenIdToProjectId[tokenIdToBe] = _projectId;

            emit Mint(msg.sender, tokenIdToBe, _projectId);
        }
    }
    ...
    function toggleProjectIsLocked(uint256 _projectId)
        public
        onlyAdminOrWhitelisted
        onlyUnlocked(_projectId)
    {
        projects[_projectId].locked = true;
    }

    function toggleProjectIsActive(uint256 _projectId) public onlyAdminOrWhitelisted {
    ...
    ) public onlyAdmin onlyUnlocked(_projectId) {
        require(
    ...
    ) public onlyUnlocked(_projectId) onlyAdminOrWhitelisted {
        require(
