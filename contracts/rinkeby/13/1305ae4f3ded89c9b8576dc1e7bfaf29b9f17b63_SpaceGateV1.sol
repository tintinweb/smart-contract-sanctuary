...
contract SpaceGateV1 is Ownable{
    ...
    function spaceGate(
        ...
        payable
    {
        require(!isPause,"Gate paused");
        require(validGate(_gate), "Gate not found");
        GateInfo storage gateInfo = Gates[_gate];
        
        address _contractToken = gateInfo.contractToken;
        require(gateInfo.min <= _amount && gateInfo.max >= _amount,"Not meet min max condition");
        ...
        if (isToken) {
            ERC20(_contractToken).transferFrom(msg.sender, address(this), amountWithFee);
        } else {
            require(msg.value >= amountWithFee,"Not enought amount");
        }
