...
contract ERC721Tradable is ERC721Enumerable, Ownable {
    ...
    function setCoordinates(uint256 tokenId, uint256 latitude, uint256 longitude) external onlyOwner {
        require(getCurrentTokenId() <= tokenId, "Token doesn't exists");
        coordinates[tokenId] = Coordinate({latitude: latitude, longitude: longitude});
    }

    function getCoordinates(uint256 tokenId) external view onlyOwner returns(Coordinate memory) {
    ...
    /**
     * @dev returns the highest token id
     */
