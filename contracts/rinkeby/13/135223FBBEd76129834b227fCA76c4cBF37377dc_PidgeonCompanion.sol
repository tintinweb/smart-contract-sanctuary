...
contract PidgeonCompanion is ERC721Enumerable, Ownable {
  ...
  PoopyPidgeon private poopy_pidgeon;

  constructor(string memory baseURI) ERC721("Poopy Pidgeon Companion", "PoopyC1")  {
      setBaseURI(baseURI);
      poopy_pidgeon = PoopyPidgeon(0x490edFB63850eC7387E6D326E27Eec19f246eaa1);
  ...
  function _unclaimedItemIds(address _inputAddress) public view returns(UnclaimedItems memory) {
      uint tokenCount = poopy_pidgeon.balanceOf(_inputAddress);

      UnclaimedItems memory result = UnclaimedItems(0, new uint256[](tokenCount));
      for(uint256 i; i < tokenCount; i++){
            uint tokenId = poopy_pidgeon.tokenOfOwnerByIndex(_inputAddress, i);
  ...
  function claim(uint256 num) public {
      ...
      require (num*2 < (uc_items.unclaimedItemCount+1) , "You can only claim 1 free companion per Pidgeon");

      for(uint256 i; i < num; i++) {
        _tokenClaimedTracker.increment();

        /** update the mapping of pidgeon ids */
  ...
  function mint(address _to, uint256 num) public payable {
      ...
      if(msg.sender != owner()) {
        if (poopy_pidgeon.balanceOf(msg.sender) < 1) {
