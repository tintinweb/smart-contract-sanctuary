...
contract AragonCourtDisputerV1 is Utils {
     ...
     */
    constructor(address _registry, address _insuranceManager, address _arbiter, uint256 _settlementDelay) {
    ...
    function dispute(
        ...
        bytes memory _metadata = AragonCourtMetadataLib.generateDefaultPayload(_termsCid, _feePayer, _index, _multiMilestone);
        uint256 _did = ARBITER.createDispute(2, _metadata);
        disputes[_mid] = _did;
        emit DisputeStarted(_cid, _index, _did);
    }
