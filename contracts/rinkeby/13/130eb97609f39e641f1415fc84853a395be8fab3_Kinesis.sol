...
contract Kinesis is ERC721Enumerable, Ownable {
    ...
    address private _signerAddress = 0x4D39A464e3d2441Dc3e95af45733D749883f0dc4;
    ...
    function addAllProjectTokensToClaimable(uint256 _projectId, uint256 _oldProjectId) external onlyAuthorized {
        ...
        for (uint256 i = 0; i < projectIdToTokenIds[_oldProjectId].length; i++) {
            addTokenToClaimable(_projectId, projectIdToTokenIds[_oldProjectId][i]);
        }
    }

    function addTokenListToClaimable(uint256 _projectId, uint256[] memory _tokenIdArr) external onlyAuthorized{
        require(projects[_projectId].claimable, "Project is not claimable");
    ...
    function addTokenToClaimable(uint256 _projectId, uint256 _tokenId) private {
        require(projects[_projectId].claimable, "Project is not claimable");
    ...
    function addProject(uint256 _pricePerTokenInWei, uint256 _mintVolume, string memory _name, string memory _description, string memory _artistName, string memory _artistWebsite, uint256 _txnLimit) public onlyAuthorized {
    ...
    function updateProjectScriptJSON(uint256 _projectId, string memory _projectScriptJSON) onlyUnlocked(_projectId) onlyArtistOrAuthorized(_projectId) external {
