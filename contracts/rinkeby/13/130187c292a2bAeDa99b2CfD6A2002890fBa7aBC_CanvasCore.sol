...
contract CanvasBase is CanvasAdministration {
    ...
    function _createSquare(address _owner, uint256 _squareId) public returns (uint256) {
        uint16 coordX = _calculateCoordX(_squareId);
        uint16 coordY = _calculateCoordY(_squareId);
        ...
        require(_squareId < 40000);

        emit SquareCreation(_owner, _squareId);

        _transfer(0, _owner, _squareId);
...
contract SquareOwnership is CanvasBase, ERC721 {
    ...
    function mint(uint256 _tokenId) external payable whenNotPaused {
      ...
      _createSquare(msg.sender, _tokenId);
