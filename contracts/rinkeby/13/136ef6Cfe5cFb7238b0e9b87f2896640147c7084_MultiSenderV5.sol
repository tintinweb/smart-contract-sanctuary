...
contract ServiceCharges is Management {
	mapping(uint => uint[2]) public priceGroups;
	function setDefaultPrices() internal {
		priceGroups[1] = [0, 100];
		priceGroups[2] = [101, 250];
		priceGroups[3] = [251, 500];
		priceGroups[4] = [501, 1000];
	}

	function changePriceGroups(uint price11, uint price12, 
		uint price21, uint price22, uint price31, uint price32, 
		uint price41, uint price42) public onlyManager {
			priceGroups[1] = [price11, price12];
			priceGroups[2] = [price21, price22];
			priceGroups[3] = [price31, price32];
			priceGroups[4] = [price41, price42];
	}
...
contract MultiSenderV5 is ServiceCharges { 
    ...
    modifier getServiceFee(string memory _paymentMethod, uint noOfRecipients, uint noOfIds, uint noOfAmounts) {
		...
		uint multiplier;
		if(noOfRecipients < priceGroups[1][0]) multiplier = 0;
		else if(noOfRecipients >= priceGroups[1][0]  && noOfRecipients <= priceGroups[1][1]) multiplier = 1;
		else if(noOfRecipients >= priceGroups[2][0] && noOfRecipients <= priceGroups[2][0]) multiplier = 2;
		else if(noOfRecipients >= priceGroups[3][0] && noOfRecipients <= priceGroups[3][0]) multiplier = 3;
		else if(noOfRecipients >= priceGroups[4][0] && noOfRecipients <= priceGroups[4][0]) multiplier = 4;
		else multiplier = 20;
