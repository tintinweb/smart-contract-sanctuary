...
contract NFTStaking is AccessControl {
  ...
  string public constant stakeingName = 'DAO';
  ...
  constructor (address staking_token, address giveaway_token) public {
    _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    _setupRole(DISTRIBUTER_ROLE, msg.sender);
    STAKING_TOKEN = IERC20(staking_token);
    GIVEAWAY_TOKEN = IERC20(giveaway_token);
    s_stakingLimit = 7e18;
    require(address(STAKING_TOKEN).isContract(), "Staking: pair is not a contract");
    require(address(GIVEAWAY_TOKEN).isContract(), "Staking: YIELD is not a contract");
    require(address(STAKING_TOKEN) != address(GIVEAWAY_TOKEN), "Staking: pair and YIELD are the same");
  }
  ...
  function stake(uint256 amount) external updateReward(msg.sender) {
    ...
    IERC20(address(STAKING_TOKEN)).safeTransferFrom(msg.sender, address(this), amount);
  ...
  function withdraw(uint256 amount) public updateReward(msg.sender) {
    ...
    IERC20(address(STAKING_TOKEN)).safeTransfer(msg.sender, amount);
  ...
  function getReward() public updateReward(msg.sender) {
    ...
    if (reward > 0) {
      ...
      GIVEAWAY_TOKEN.safeTransfer(msg.sender, reward);
  ...
  function pairInfo() public view returns (uint256 totalSupply) {
    totalSupply = STAKING_TOKEN.totalSupply();
    return totalSupply;
  }
