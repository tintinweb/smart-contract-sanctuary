 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    function mintInternal() internal {
        ...
        require(!CatsLibrary.isContract(msg.sender));
     ...
     */
    function mintCats(uint256 _times) public payable {
        require((_times > 0 && _times <= 20) && (msg.value == _times * price) && (totalSupply() < MINTS_PER_TIER), "t");
     ...
     * @dev Mint for Miece
     */
    function mintCatWithToken() public {        
        //Burn this much miece
...
*/
        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) { 
            uint8 thisTraitIndex = CatsLibrary.parseInt(
                CatsLibrary.substring(_hash, i, i + 1)
            );
            ...
            for (
                ...
                string memory thisPixel = CatsLibrary.substring(
                ...
                uint8 x = letterToNumber(
                    CatsLibrary.substring(thisPixel, 0, 1)
                );
                uint8 y = letterToNumber(
                    CatsLibrary.substring(thisPixel, 1, 2)
                );
                ...
                svgString = string(
                    abi.encodePacked(
                        svgString,
                        "<rect class='c",
                        CatsLibrary.substring(thisPixel, 2, 4),
                        "' x='",
                        x.toString(),
                        "' y='",
                        y.toString(),
                    ...
                    CatsLibrary.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "cmyKatz #',
                                    CatsLibrary.toString(_tokenId),
                                    '", "description": "cmyKatz is a collection of 10,000 unique cats. All the metadata and images stored 100% on-chain.", "image": "data:image/svg+xml;base64,',
                                    CatsLibrary.encode(
        ...
        returns (string memory)
    {
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
            tokenHash = string(
                abi.encodePacked(
                    "1",
                    CatsLibrary.substring(tokenHash, 1, 9)
                )
            );
        }
...
|     | \      / |  |  ||     ||  .  \    |  |   |     ||  |  \     | |  |   |  ||     ||  |  |\    |
 \___/   \_/\_/  |__|__||_____||__|\_|    |__|    \__,_||__|__|\____| |__|  |____|\___/ |__|__| \___|
    ...
    */
    ...
    /**
     * @dev Sets the miece ERC20 address
     * @param _mieceAddress The miece address
     */

    function setMieceAddress(address _mieceAddress) public onlyOwner {
