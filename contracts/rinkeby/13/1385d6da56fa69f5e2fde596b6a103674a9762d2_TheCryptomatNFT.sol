...
contract TheCryptomatNFT is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using SafeMath for uint256;
    using SafeCast for uint256;
    using Strings for uint256;
    
    string ticketName = "The Cryptomat";
    string ticketSymbol = "LNDRY";
    string private unrevealedBaseURI;
    string private revealedBaseURI;
    bool private paused = true;
    bool private presalePaused = true;
    address constant founderAddress = 0x60fdB2a4BeaCC39d31B118E6E5f72984cAdDfef0; // carve.eth
    address constant founderVaultAddress = 0xeFdD9A85E4bb37E37Bc834901bD3c631C9539d54; // carvevault.eth
    address constant projectMarketingAddress = 0xeFdD9A85E4bb37E37Bc834901bD3c631C9539d54; // The Cryptomat Marketing Address
    address constant projectTreasuryAddress = 0x04863B7954c3Aa034e1414cB79c65c90526d31d4; // The Cryptomat Treasury Address
    address constant developerAddress = 0x9D5233fDcaeBBBd29c1D356b53f519D2a58b6588; // nemo (dev)
    address constant artistAddress = 0x07fFd3aE33C509b395264fe6D08C38e0Cb8A795F; // Nirft (artist)
    ...
    modifier checkSetup{
        require(founderAddress != address(0), "Founders address has not been set!");
        require(founderVaultAddress != address(0), "Founders Vault address has not been set!");
        require(projectMarketingAddress != address(0), "Projects Marketing Address has not been set!");
        require(projectTreasuryAddress != address(0), "Projects Treasury address has not been set!");
        require(developerAddress != address(0), "Developer address has not been set!");
        require(artistAddress != address(0), "Artist address has not been set!");
        require(saleConfig.startTime !=0, "Sale start time not set yet");
        require(saleConfig.endTime != 0, "Sale end time not set yet");
        require(maxSupply > 0, "Max supply transaction not yet set");
    ...
    function withdrawAll() public onlyOwner {
        ...
        require(balance > 0);
        _widthdraw(founderVaultAddress, balance.mul(60).div(100)); // 60%
        _widthdraw(projectMarketingAddress, balance.mul(2).div(100)); // 2%
        _widthdraw(projectTreasuryAddress, balance.mul(8).div(100)); // 8%
        _widthdraw(artistAddress, balance.mul(5).div(100)); // 5%
        _widthdraw(developerAddress, balance.mul(5).div(100)); // 5%
        _widthdraw(founderAddress, address(this).balance); // rest
    }
    ...
    function userTokens(address toView) public view returns (uint256[] memory) {
        uint256 currentSupply = currentToken();
        uint256[] memory toRet = new uint256[](balanceOf(toView));
        uint256 found = 0;
