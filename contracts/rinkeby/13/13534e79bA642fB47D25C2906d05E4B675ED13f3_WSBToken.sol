...
contract WSBToken is IERC721Receiver, ReentrancyGuard, Ownable, WSBSwappable {
  ...
  uint16 public constant MAX_DEV_CLAIM = 7500;
  uint16 public constant MAX_PER_ACCOUNT = 20;
  uint256 public constant BUY_PRICE = 0.1 ether;
  string public constant UNREVEALED_URL = "ipfs://ipfs_url/";

  uint16 private publicTokens = 1194;
  string public baseURI = "ipfs://ipfs_url/";
  ...
  function claim(uint256 amount) external payable nonReentrant {
    ...
    require(balanceOf(msg.sender).add(amount).add(presaleContract.balanceOf(msg.sender)) <= MAX_PER_ACCOUNT, "ClaimToken: Cannot own more than 20 tokens.");
    require(ERC721Enumerable.totalSupply().add(amount) <= MAX_TOKEN, "ClaimToken: All tokens are claimed.");
  ...
  function claimDevTokens() external onlyOwner {
    for(uint16 i = MAX_TOKEN; i < MAX_DEV_CLAIM; i++){
      _safeMint(msg.sender, i);
    }
  }

  function anyRaffle() external view returns(bool) {
    return uint8(presaleContract.balanceOf(msg.sender)) > 0;
  }
