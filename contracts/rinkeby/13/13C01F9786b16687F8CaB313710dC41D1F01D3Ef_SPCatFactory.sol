...
import "./SPCatItem.sol";

contract SPCatFactory is FactoryERC1155, Ownable, ReentrancyGuard {
    using Strings for string;
    using SafeMath for uint256;

    address public proxyRegistryAddress;
    address public nftCatAddress;
     ...
     */
    uint256 constant SUPPLY_PER_TOKEN_ID = 1000000;

    constructor(
        address _proxyRegistryAddress,
        address _nftCatAddress,
        string memory _baseMetadataURI
    ) {
        proxyRegistryAddress = _proxyRegistryAddress;
        nftCatAddress = _nftCatAddress;
    ...
    function _mint(
        ...
        require(
            _canMint(_msgSender(), _assetId, _amount),
            "SPCatFactory#_mint: CANNOT_MINT_MORE"
        );

        require(_isOwnerOrProxy(_msgSender()), "Caller cannot mint boxes");
        // SPCatItem are not premined, so we need to create or mint them.
    ...
    returns (uint256)
    {
        ...
        ERC1155Tradable nft = ERC1155Tradable(nftCatAddress);
        uint256 currentSupply = nft.totalSupply(_optionId);
...
contract SPCatItem is ERC1155Tradable {
    ...
    constructor (address _proxyRegisteryAddress, string memory baseUrl)
        ERC1155Tradable(
            "SP Cat Item",
            "SPCI",
            string(abi.encodePacked(_baseUrl, "/metadata/cat/{id}")),
