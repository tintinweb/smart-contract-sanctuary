...
contract KPITokensManager is Ownable, IKPITokensManager {
    ...
    function addTemplate(address _template, string calldata _description)
        external
    {
        templates.add(_template, _description);
    }
    ...
    function verifyTemplate(address _template) external override {
        if (msg.sender != owner()) revert Forbidden();
        templates.verify(_template);
    }
...
interface IKPITokensManager {
    ...
    function verifyTemplate(address _template) external;
