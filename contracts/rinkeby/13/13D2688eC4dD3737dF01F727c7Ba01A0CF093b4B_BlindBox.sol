    ...
    */
    function StartSeries(Series memory series, bool isGenerative ) onlyOwner public {
        if(isGenerative){
            ...
            generativeSeries(series.name, series.seriesURI, series.boxName, series.boxURI, series.startTime, series.endTime, series.maxBoxes, series.perBoxNftMint, series.perBoxPrice);
        } else {
    ...
    function extraPsrams(Series memory series) internal {
...
/** 
    ...
    */
    function setURIs(uint256 seriesId, string[] memory name, string[] memory uri, uint256[] memory rarity) onlyOwner public {
        ...
        for (uint256 index = 0; index < name.length; index++) {
            ...
            _attrId.increment();
            nonGenSeries[seriesId].uris[_attrId.current()] = URI(name[index], uri[index], rarity[index]);
        }
        require( totalRarity == 100, "Rarity sum of URIs should be equal to 100");
        emit URIsAdded(seriesId,from, _attrId.current());
    }
/** 
...
/** 
    ...
    */
    function buyNonGenBox(uint256 seriesId, uint256 currencyType) validateCurrencyType(seriesId,currencyType, false) internal {
        ...
        emit BuyBoxNonGen(_boxId.current(), seriesId);
    }
...
/** 
    ...
    */
    function _openNonGenBox(uint256 boxId) private {
        ...
        address tempAddress = address(nft);
        ...
        uint256 rand1;
        uint256[] memory uris = new uint256[](boxesNonGen[boxId].countNFTs);
        uris = getRandURIs(sId, boxesNonGen[boxId].countNFTs);
        for (uint256 j = 0; j < boxesNonGen[boxId].countNFTs; j++) {
          rand1 = uint256(keccak256(abi.encodePacked(block.coinbase, rand, msg.sender, j))).mod(boxesNonGen[boxId].countNFTs); // to keep each iteration further randomize and reducing fee of invoking VRF on each iteration.
          nft.mintWithTokenURI(msg.sender, nonGenSeries[sId].uris[rand1].uri );
        }
        boxesNonGen[boxId].isOpened = true;
        nft = INFT(tempAddress);
    }
/** 
    ...
    */
    ...
    event URIsAdded(uint256 indexed boxId, uint256 from, uint256 to);
    event BuyBoxNonGen(uint256 boxId, uint256 seriesId);
...
/** 
    @dev function to start new Generative Series
        ...
        @param startTime - start time of the series, (from whom its boxes will be available to get bought)
        @param endTime - end time of the series, (after whom its boxes will not be available to get bought)
        @param maxBoxes - total boxes / maximum blindboxes to be created/sold in this series
        @param perBoxNftMint - NFTs to be minted per Blindbox opened of this series
        @param boxPrice - price of each blindbox of this series. price in terms of an ALIA/ERC20/BEP20 token
    
    */
    function generativeSeries(string memory name, string memory seriesURI, string memory boxName, string memory boxURI, uint256 startTime, uint256 endTime, uint256 maxBoxes, uint256 perBoxNftMint, uint256 boxPrice) onlyOwner internal {
        require(startTime < endTime, "invalid series endTime");
...
/** 
    ...
    @notice given series should not be ended or its max boxes already minted.
    */
    function buyGenerativeBox(uint256 seriesId) internal {
        ...
        mintGenBox(seriesId);
        // escrow alia
        ALIA.transferFrom(msg.sender, platform, genSeries[seriesId].price);
        // transfer box to buyer
...
/** 
   ...
   /** 
    ...
    @notice only owner of the contract can trigger this function
    */
    function mintGenerativeNFTs(uint256 boxId, string[] memory uris) public onlyOwner {
        ...
        require(boxesGen[boxId].countNFTs == uris.length, "insufficient URIs to mint");
         for (uint256 i = 0; i < uris.length; i++) {
            nft.mintWithTokenURI(genBoxOwner[boxId], uris[i]);
         }
