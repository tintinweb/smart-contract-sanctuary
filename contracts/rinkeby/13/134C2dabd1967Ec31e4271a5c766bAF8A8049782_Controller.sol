...
contract Controller is OwnableUpgradeable, IController {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < _underlyings.length; i++) {
            ...
            //    ethAmount += _underlyingsAmounts[i] * prices[i] / 10**18;
        }
        return ethAmount;
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _underlyings.length; i++) {
            ...
            uint8 decimals = IERC20Extend(_underlyings[i]).decimals();
            if (decimals < 18) {
    ...
    /// @return DHV price (to the chosen by adapter asset)
    function getDHVPrice(address _cluster) external view override returns (uint256) {
