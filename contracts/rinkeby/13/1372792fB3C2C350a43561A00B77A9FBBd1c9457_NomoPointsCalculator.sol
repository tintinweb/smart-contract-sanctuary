...
contract NomoPointsCalculator is Ownable {
    INomoNFT nft;

    mapping(uint256 => bool) public allowedParametersSets;
    mapping(string => uint256) public multipliers;
    uint256 public constant MULTIPLIERS_DIVIDER = 100;

    constructor(INomoNFT _nft) {
        nft = _nft;
    }

    function allowParametersSets(uint256 _setId) external onlyOwner {
        allowedParametersSets[_setId] = true;
    }

    function disallowParametersSets(uint256 _setId) external onlyOwner {
        allowedParametersSets[_setId] = false;
    }

    function setMultiplier(string memory _name, uint256 _multiplier) external onlyOwner {
        multipliers[_name] = _multiplier;
    }

    function setMultipliers(string[] memory _names, uint256[] calldata _multipliers) external onlyOwner {
    ...
    function calculatePoints(uint256 _tokenId, uint256 _gameStartTime) external view returns (uint256 points) {
        ...
        ) = nft.getCardImageDataByTokenId(_tokenId);
        ...
        for (uint256 i = 0; i < parametersNames.length; i++) {
            points += (multipliers[parametersNames[i]] * parametersValues[i]) / MULTIPLIERS_DIVIDER;
        }
    }
}
