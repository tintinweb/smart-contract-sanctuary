...
contract UserOwnedMarkets is ERC721Upgradeable {
    ...
    address[][] public otherMarkets; //list of addresses of other UserOwnedMarkets that can participate here
    ...
    event Transaction(address indexed buyer, address indexed seller, bytes32 spec, address indexed _paymentTokenAddress, uint paymentAmount, uint expiry, uint transactionID);
    ...
    public {
        ...
        emit Transaction(temp.buyer, temp.seller,  temp.spec, _paymentTokenAddress, temp.paymentAmount, temp.expiry, transactionID[marketID]);
    ...
    */
    ...
    auditNotExpired(_auditID)
    public {
        if(_marketAddressID == 0) require(ownerOf(_tokenID)==msg.sender && audits[_auditID].marketID==tokenMarketID[_tokenID],"16");
    ...
    modifier trueAssayer(uint _auditID, uint assayID) {
        ...
        else require(ERC721Upgradeable(otherMarkets[audits[_auditID].marketID][audits[_auditID].commits[assayID].marketAddressID-1]).ownerOf(audits[_auditID].commits[assayID].tokenID)==msg.sender,"20");
        _;
    }
    ...
    function updateOtherMarkets(address[] memory newSet, uint marketID) public onlyOwnerOrDelegate(marketID){
        otherMarkets[marketID] = newSet;
    }

    function addOtherMarket(address newMarket, uint marketID) public onlyOwnerOrDelegate(marketID){
