...
contract WeaponsCollectible is ERC721, VRFConsumerBase {
    ...
    enum WeaponType { Revolver, AssultRifle }
    ...
    function changeDefaultTokenURI(string memory new_tokenURI) public {
        require(owner == msg.sender, "Only the owner can change the default token URI");

        defaultTokenURI = new_tokenURI;
    }

    function requestAirdrop(uint256 userProvidedSeed) public returns(bytes32) {
        require(usedAirDrop[msg.sender] != true, "Already used the airdrop");

        bytes32 requestId = requestRandomness(keyHash, fee);
        requestIdToSender[requestId] = msg.sender;
        requestIdToTokenURI[requestId] = defaultTokenURI;
    ...
    function createCollectible(uint256 userProvidedSeed) payable public returns(bytes32) {
