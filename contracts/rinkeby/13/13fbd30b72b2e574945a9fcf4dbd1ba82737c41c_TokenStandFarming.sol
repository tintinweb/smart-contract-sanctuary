...
interface ITokenStandTreasury {
    ...
    function transferStandRewardToRef(address _ref, uint256 _amount) external;
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function stake(uint256 fid, uint256 amount, address ref) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (standReward > 0) {
                if (referral[msg.sender] != address(0)) {
                    ...
                    tokenStandTreasury.transferStandRewardToRef(referral[msg.sender], refStandReward);
                }
        ...
        emit Staked(msg.sender, fid, amount, standReward.sub(refStandReward), giftReward.sub(refGiftReward), refStandReward, refGiftReward, referral[msg.sender]);
    }
    ...
    function withdraw(uint256 fid, uint256 amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(farm.accStandPerShare).div(1e12);
        emit Withdrawn(msg.sender, fid, amount, standReward.sub(refStandReward), giftReward.sub(refGiftReward), refStandReward, refGiftReward, referral[msg.sender]);
    }
