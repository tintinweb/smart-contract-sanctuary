...
contract MerkelNFT is ERC721Enumerable, Ownable {
    ...
    address payable LOTTERY_ADDRESS=  payable(address(0xdEF79382C5D9b216387B55f8a5725db457DDE46e));
  ...
  modifier onlyWhitelisted() {
     ...
     */
    constructor() ERC721("MerkelNFT", "MERKEL") {
    }
    ...
    function withdraw() public onlyOwner {
        ...
        payable(msg.sender).transfer(balance / 3);
        payable(0xD372627d8dBD99136265816857AB785eA54eFCf5).transfer(balance / 3);
        payable(0x748C43093cc126d92910D852ff350cbAa8f664c7).transfer(balance / 3);
    }
     ...
     */
     ...
     function mintFromWhiteList(uint256 numberOfTokens) public payable onlyWhitelisted {
         ...
         require(
            ...
            "Only a maximum of 3 token per transaction can be minted during pre-sale."
        );
        require(
            totalSupply().add(numberOfTokens) <= MAX_TOKENS,
            "Exceeding maximum amount of available MERKEL NFT."
        );
        require(
            CURRENT_PRICE.mul(numberOfTokens) <= msg.value,
            "Incorrect Merkel NFT Price."
        );
        uint256 first_encounter = block.timestamp;
        uint256 tokenId;
...
/*
        if(tokenId > 2 && tokenId <= MAX_TOKENS){
