...
contract JackpotGame is Ownable, ReentrancyGuard {
  ...
  // Bet amount should be above 0.1 BNB
  uint256 public minBetAmount = 10**17;

  // Award split factors
  uint256 public constant SHARES_DENOMINATOR = 10000;

  uint256[5] public winnerAwardFactors = [9500, 9600, 9700, 9800, 9900];
  uint256[5] public marketFeeFactors = [100, 50, 50, 50, 50];
  uint256[5] public gameFeeFactors = [100, 50, 50, 50, 50];
  uint256[5] public buyBackFeeFactors = [300, 300, 200, 100, 0];
  uint256[5] public tierLimitAmounts = [
    0,
    1000000000,
    5000000000,
    10000000000,
  ...
  // Constant addresses
  address public constant MARKET_ADDRESS =
    0x82efC1C9560a9F60005C06C9b1616f1791C690A6;
  ...
  modifier gameProceed() {
    require(
      ...
      "JackpotGame: gameProceed error"
    );
    _;
  }

  modifier gameEnded() {
    require(block.timestamp > endTime, "JackpotGame: gameEnded error");
    _;
  }
  ...
  function updateMinBetAmount(uint256 amount_) external onlyOwner {
    require(amount_ > 0, "JackpotGame: invalid minBetAmount");
    minBetAmount = amount_;
  }
  ...
  function updateTierLimitAmount(uint256 tierIndex, uint256 tierLimitAmount_)
    external
    onlyOwner
  {
    require(
      tierIndex > 0 && tierIndex < tierLimitAmounts.length,
      "JackpotGame: invalid tierIndex"
    );
    require(tierLimitAmount_ > 0, "JackpotGame: invalid tierLimitAmount");

    tierLimitAmounts[tierIndex] = tierLimitAmount_;
  }

  function updateFeeFactors(
    uint256 tierIndex,
    uint256 winnerAwardFactor_,
    uint256 marketFeeFactor_,
    uint256 buyBackFeeFactor_,
    uint256 gameFeeFactor_
  ) external onlyOwner {
    require(
      tierIndex > 0 && tierIndex < winnerAwardFactors.length,
      "JackpotGame: invalid tierIndex"
    );
    require(winnerAwardFactor_ > 0, "JackpotGame: invalid winnerAwardFactor");
    ...
    require(gameFeeFactor_ > 0, "JackpotGame: invalid gameFeeFactor");
    require(
      winnerAwardFactor_ +
        marketFeeFactor_ +
        buyBackFeeFactor_ +
        gameFeeFactor_ ==
        SHARES_DENOMINATOR,
      "JackpotGame: invalid fee factors"
    );

    winnerAwardFactors[tierIndex] = winnerAwardFactor_;
    marketFeeFactors[tierIndex] = marketFeeFactor_;
    buyBackFeeFactors[tierIndex] = buyBackFeeFactor_;
    gameFeeFactors[tierIndex] = gameFeeFactor_;
  }
  ...
  function newGame(uint256 startTime_, uint256 endTime_)
    ...
    gameEnded
  {
    require(startTime_ > block.timestamp, "JackpotGame: invalid start time");
    require(
  ...
  {
    require(
      msg.value > 0 && msg.value > minBetAmount,
      "JackpotGame: invalid bid amount"
    );
  ...
  function endGame() external onlyOwner gameEnded nonReentrant {
    ...
    uint256 ticketValue = hashValue % sharesTotal;

    for (uint256 i = 0; i < _betted.length; i++) {
      if (
        ticketValue >= _betMapping[_betted[i]].startTicket &&
        ticketValue < _betMapping[_betted[i]].endTicket
      ) {
        winner = _betted[i];
        break;
      }
  ...
  function _validateFeeFactors() private {
    uint256 balance = buybackToken.balanceOf(winner);
  ...
  function _awardShares() private {
    require(
      address(this).balance >= sharesTotal,
      "JackpotGame: invalid total shares"
    );

    uint256 winnerAwardAmount = (address(this).balance *
      winnerAwardFactors[validTierIndex]) / SHARES_DENOMINATOR;
    uint256 marketFeeAmount = (address(this).balance *
      marketFeeFactors[validTierIndex]) / SHARES_DENOMINATOR;
    uint256 buyBackFeeAmount = (address(this).balance *
    ...
    if (_jackpots.length > 0) {
      uint256 gameFeeAmount = address(this).balance -
        winnerAwardAmount -
        marketFeeAmount -
        buyBackFeeAmount;

      uint256 hashValue = _random(_jackpots);
      uint256 potIndex = hashValue % _jackpots.length;
      payable(_jackpots[potIndex]).transfer(gameFeeAmount);
    }
  }
  ...
  function _swapExactETHForTokens(uint256 amount) private {
    ...
    buybackToken.transfer(DEAD_ADDRESS, buybackToken.balanceOf(address(this)));
  }
