...
contract Papr is ERC20Burnable, DoubleOperator {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if (paprPrice < paprTransferFeesExclusiveCeiling && checkAddress == true) {
            ...
            _transferWithFeesExpansion(sender, recipient, amount);
        }
    ...
    function _transferWithFeesExpansion(address sender, address recipient, uint256 amount) internal {
        uint256 feesAmount = amount.mul(feesInExpansionPeriod).div(100);
        uint256 amountAfterFees = amount.sub(feesAmount);
