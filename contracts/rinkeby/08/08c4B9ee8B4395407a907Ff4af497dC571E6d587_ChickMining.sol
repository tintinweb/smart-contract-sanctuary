...
contract ChickMining is OwnableContract, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function getUserStakingNftCount(uint256 _pid, address _user) public view returns (uint256) {
        require(_pid < poolInfo.length, "invalid _pid");
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.isNFT, 'invalid _pid');
        return  userInfo[_pid][_user].holderTokens.length();
    }

    function getUserStakingNftTokenId(uint256 _pid, address _user, uint256 _index) public view returns (uint256) {
        require(_pid < poolInfo.length, "invalid _pid");
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.isNFT, 'invalid _pid');
        return  userInfo[_pid][_user].holderTokens.at(_index);
    }
