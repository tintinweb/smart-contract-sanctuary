...
abstract contract CoboSafeModuleCore is Ownable {
...
/// @title An extended contract of Cobo Safe Module that adds more utility functions
/// @author Cobo Safe Dev Team ([email protected])
/// @notice An extened contract that implements utility functions for Cobo Safe Module
/// @dev This extends the core implementation and adds more `public` `view`
///      funcitons which are supposed to be called off-chain.
abstract contract CoboSafeModuleExtended is CoboSafeModuleCore {
    using EnumerableSet for EnumerableSet.Bytes32Set;
    using EnumerableSet for EnumerableSet.AddressSet;

    /// @notice Dump all the delegates and their granted roles
    /// @dev First enumerate all delegates, for each delegate fetch all the roles granted to it.
    /// @return List of (delegate, list of roles)
    function dumpDelegatesAndRoles() external view returns (bytes[] memory) {
        address[] memory allDelegates = getAllDelegates();

        bytes[] memory rolesForDelegates = new bytes[](allDelegates.length);
        for (uint256 index = 0; index < allDelegates.length; index++) {
            address delegate = allDelegates[index];
            bytes32[] memory roles = getRolesByDelegate(delegate);
            // abi.encode("address,bytes32[]")
            rolesForDelegates[index] = abi.encode(delegate, roles);
        }

        return rolesForDelegates;
    }

    /// @notice Given a contract, dump all the function selectors and their associated roles
    /// @dev First enumerate all function selectors, for each selector fetch all the roles associated with it.
    /// @param _contract the given contract
    /// @return List of (selector, list of roles)
    function dumpFuncsAndRolesForContract(address _contract)
        public
        view
        returns (bytes[] memory)
    {
        bytes4[] memory selectors = getFuncsByContract(_contract);

        bytes[] memory rolesForFuncs = new bytes[](selectors.length);
        for (uint256 index = 0; index < selectors.length; index++) {
            bytes4 selector = selectors[index];
            bytes32[] memory roles = getRolesByContractFunction(
                _contract,
                selector
            );
            // abi.encode("bytes4,bytes32[]")
            rolesForFuncs[index] = abi.encode(selector, roles);
        }

        return rolesForFuncs;
    }

    /// @notice Dump all contracts and function selectors with their associated roles
    /// @dev First enumerate all contracts, then for each contract enumerate all
    ///      function selectors, third for each selector fetch all the roles
    ///      associated with it.
    /// @return List of (contract, list of (selector, list of roles))
    function dumpFuncsAndRolesForAllContracts()
        external
        view
        returns (bytes[] memory)
    {
        address[] memory allContracts = getAllContracts();

        bytes[] memory rolesForAllContracts = new bytes[](allContracts.length);
        for (uint256 index = 0; index < allContracts.length; index++) {
            address _contract = allContracts[index];
            bytes[] memory contractConfigs = dumpFuncsAndRolesForContract(
                _contract
            );
            // abi.encode("address,bytes[]")
            rolesForAllContracts[index] = abi.encode(
                _contract,
                contractConfigs
            );
        }

        return rolesForAllContracts;
    }
}
...
contract CoboSafeModule is CoboSafeModuleExtended {
    using EnumerableSet for EnumerableSet.Bytes32Set;
