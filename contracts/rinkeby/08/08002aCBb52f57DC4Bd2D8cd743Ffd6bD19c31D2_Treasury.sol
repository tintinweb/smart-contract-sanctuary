...
contract Treasury is Ownable {
    ...
    event ReservesWithdrawn( address indexed token, uint amount );
    event ReservesUpdated( uint indexed totalReserves );
    event ReservesAudited( uint indexed totalReserves );
    event ChangeActivated( MANAGING indexed managing, address activated, bool result );
    event SPVUpdated( address indexed spv );

    enum MANAGING { RESERVETOKEN, LIQUIDITYTOKEN, VARIABLETOKEN, ASSETMANAGER, DEBTOR }
    struct PriceFeed {
        address feed;
        uint decimals;
    }
    ...
    uint public immutable priceMultiplier;  // 4 decimals. 1000 = 0.1
    ...
    constructor (address _KEEPER, address _USDC, address _USDT, address _DAI, address _DAO, address _vesting, address _ethPriceFeed, uint _priceMultiplier, uint _initPrice) {
    ...
    function setSPV(address _SPV) external onlyOwner() {
        require( _SPV != address(0), "Cannot be zero address");
        SPV = _SPV;
        emit SPVUpdated( SPV );
    }
    ...
    function setVesting(address _vesting) external onlyOwner() {
        require( _vesting != address(0), "Cannot be zero address");
        vesting = _vesting;
    }
    ...
    function setTreasuryRatio(uint _daoRatio, uint _spvRatio, uint _vestingRatio) external onlyOwner() {
        require( _daoRatio <= 1000, "DAO cannot be more than 10%" );
        require( _spvRatio <= 7000, "SPV cannot be more than 70%" );
        require( _vestingRatio <= 1000, "Vesting cannot be more than 10%" );
        daoRatio = _daoRatio;
        spvRatio = _spvRatio;
    ...
    function getPremium(uint _price) public view returns (uint) {
        return _price.mul( ISPV(SPV).endTime().sub(block.timestamp) ).div( ISPV(SPV).endTime().sub( ISPV(SPV).startTime() ) ).div(4);
    }
    ...
    function getPrice() public view returns ( uint ) {
        if (initPrice != 0) {
            ...
            return priceMultiplier.mul( totalReserves.add( ISPV(SPV).totalValue() ).mul(10 ** keeperDecimals).div( KEEPER.totalSupply().sub( KEEPER.balanceOf(vesting) ) ) ).div(1e4);
        }
    }
    ...
    function deposit( uint _amount, address _token, bool _stake ) external returns ( uint send_ ) {
        ...
        totalReserves = totalReserves.add( value.sub(daoValue) );
        send_ = sendOrStake(value, _stake);
    ...
    function sendOrStake(uint _value, bool _stake) internal returns (uint send_) {
        send_ = _value.mul( 10 ** keeperDecimals ).div( getPrice() );
        if ( _stake ) {
            KEEPER.mint( address(this), send_ );
            KEEPER.approve( staking, send_ );
            IStaking( staking ).stake( msg.sender, send_ );
        } else {
    ...
    function sell( uint _amount, address _token ) external {
        ...
        (uint price, uint sellPrice) = sellKeeperBurn(_amount);

        uint reserveLoss = _amount.mul(price).div( 10 ** keeperDecimals );
        ...
        uint sellAmount = tokenAmount.mul(sellPrice).div(price);
    ...
    function sellKeeperBurn(uint _amount) internal returns (uint price, uint sellPrice) {
        price = getPrice();
        uint premium = getPremium(price);
        sellPrice = price.sub(premium);

        KEEPER.burnFrom( msg.sender, _amount );
        uint vestingAmount = _amount.mul(vestingRatio).div(1e4);
        KEEPER.burnFrom( vesting, vestingAmount );
    }
    ...
    /**
        @notice allow owner multisig to withdraw assets
        @param _token address
        @param _amount uint
     */
    ...
    function withdrawEth( uint _amount ) external onlyOwner() {
        ...
        emit ReservesWithdrawn( address(0), _amount );
    }
    ...
    function SPVWithdraw( address _token, uint _amount ) external {
        ...
        require( msg.sender == SPVWallet, "Only SPV Wallet" );
        uint value = valueOfToken(_token, _amount);
        uint totalValue = totalReserves.add( ISPV(SPV).totalValue() );
        require( spvDebt.add(value) < totalValue.mul(spvRatio).div(1e4) );
        spvDebt = spvDebt.add(value);
        totalReserves = totalReserves.sub( value );
        emit ReservesUpdated( totalReserves );

        IERC20Extended( _token ).safeTransfer( SPVWallet, _amount );
        ISPV(SPV).updateTotalValue();
    }
...
interface IStaking {
    function stake(address _recipient, uint _amount) external;
}
