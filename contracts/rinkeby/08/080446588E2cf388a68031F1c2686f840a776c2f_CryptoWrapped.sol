...
contract CryptoWrapped is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    /// @notice Minimum donation price to mint premium token.
    uint256 public constant MINIMUM_DONATION_PRICE = 0.02 ether;
    ...
    bool private isMintingAllowed = true;
    ...
    /// @dev Event that is emitted upon successful minting
    ...
    modifier newMinter() {
        require(
            ...
            "CryptoWrapped: Only allowed to mint one NFT"
        );
        _;
    }
    ...
    modifier isValidHash(string memory _tokenURI, bytes32 _hash) {
        require(
            keccak256(abi.encode("wrapped", _tokenURI)) == _hash,
            "CryptoWrapped: Invalid hash sent to mintToken"
        );
        _;
    }
     ...
     * by sending an amount >= MINIMUM_DONATION_PRICE. Donations are tracked
     ...
     */
    function mintToken(string memory _tokenURI, bytes32 _hash)
        external
        payable
        nonReentrant
        mintingIsAllowed
        ...
        isValidHash(_tokenURI, _hash)
        returns (uint256)
    {
        bool isPremium = false;
        if (msg.value > 0) {
            require(
                msg.value >= MINIMUM_DONATION_PRICE,
                "CryptoWrapped: Expected minimum donation amount of 0.01 ether"
            );
            isPremium = true;
        }
        ...
        emit MintedCryptoWrapped(msg.sender, _tokenURI, newTokenId, msg.value);
    ...
    function contractURI() external pure returns (string memory) {
        return
            "https://ipfs.io/ipfs/QmPrqUUbkwBL55KKkPLS4gqngjpMyMkuAZSpWe3oWAGP36";
    }
