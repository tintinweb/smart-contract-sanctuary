...
contract MyGallery is ERC1155 {
     ...
     * mint token
     */
    function mint(address owner, uint256 initialSupply, string memory tokenName, string memory tokenURI) external {
        _id.increment();
        ...
        tokenInfos[newId] = tokenInfo;
    }

    /*
     * batch mint token
     */
    function mintBatch(address owner, uint256[] memory initialSupply, string[] memory tokenName, string[] memory tokenURI) external {
        for (uint256 i = 0; i < initialSupply.length; i++) {
            ...
            TokenInfo memory tokenInfo = TokenInfo(tokenName[i], tokenURI[i]);
            tokenInfos[newId] = tokenInfo;
        }
    }

    /*
     * get tokenName by tokenId
     */
