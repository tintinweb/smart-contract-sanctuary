...
contract Vault is IERC20, ReentrancyGuard, Ownable {
	...
	event SwapInProcessed(address sender, uint256 amount);
	...
	// total USD amount of LP providers
	uint256 public totalTokens;

	// total amount deposited for swapping
	...
	function swapOut(address account, uint256 amount) external onlyBridge notLocked returns (bool) {
		...
		emit SwapInProcessed(account, amount);
		vaultBalance -= amount;
		return true;
	}
	...
	/// @param amount the amount user wants to swap, fees for treasury and lp providers will be deducted
	function swapIn(address account, uint256 amount) external notLocked {
		...
		emit SwapOutProcessed(account, output);
		vaultBalance += output;
	}
}
