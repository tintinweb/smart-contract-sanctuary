 ...
 */
contract POLKADOG is Context, IERC20, IERC20Metadata, Pausable, Ownable {
    ...
    uint256 private _initial_total_supply;
    uint8 private _decimal;
    string private _name;
    string private _symbol;
    uint8 private _transaction_max = 5;
    uint private _max_days = 14 days;
    uint8 private _reward_percentage = 2;
    uint8 private _transaction_burn = 2;
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimal_, uint256 totalSupply_) {
        ...
        _symbol = symbol_;
        _decimal = decimal_;
        _initial_total_supply = totalSupply_;
        ...
        // _maxTxAmount = totalSupply_ * 1 / 10**2;
    }
    ...
    function includeAccount(address account) private onlyOwner() {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) private onlyOwner() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (block.timestamp <= (_deploy_timstamp + _max_days)) {
            require(amount <= ((_initial_total_supply * _transaction_max) / 10**4), "ERC20: Transfer amount exceeds max amount");
            require(recipientBalance + amount <= ((_initial_total_supply * _transaction_max) / 10**4), "ERC20: Recipient amount exceeds max amount");
        }
    ...
    function _transferStandard(address sender_address, address recipient_address, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 burn_amount) = _getTValues(tAmount);
        uint256 currentRate =  _getRate();
        uint256 fee = tFee + burn_amount;
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, fee, tLiquidity, currentRate);
        
        address sender = sender_address;
        address recipient = recipient_address;
        
        // subtract from sender
        uint256 sub_amount = rAmount - burn_amount;
        _rOwned[sender] = _rOwned[sender] - sub_amount;
        
        
        // update amount in recipient address
        uint256 transfer_amount = rTransferAmount - burn_amount;
        _rOwned[recipient] = _rOwned[recipient] + transfer_amount;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tFee = (totalAmount * _reward_percentage) / (10**2);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 burn_amount = (totalAmount * _transaction_burn) / (10**2);
        uint256 tTransferAmount = totalAmount - tFee - tLiquidity - burn_amount;
        return (tTransferAmount, tFee, tLiquidity, burn_amount);
    }
