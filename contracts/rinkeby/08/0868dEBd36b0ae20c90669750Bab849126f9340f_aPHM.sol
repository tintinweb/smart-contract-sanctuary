...
import {IaPHM} from "IaPHM.sol";

/**
 * @title aPHM
 * @author botulin
 * @notice The Alpha Phantom Token
 */
contract aPHM is IaPHM, ERC20, PhantomStorageMixin {
    using PRBMathUD60x18 for uint256;

    mapping(address => bool) public isApprovedSeller;

    constructor(address storageFactoryAddress, uint256 totalSupply)
        ERC20("Alpha Phantom", "aPHM", 18)
        PhantomStorageMixin(storageFactoryAddress)
    {
        _addApprovedSeller(address(this));
        _addApprovedSeller(address(PhantomVault()));
        _addApprovedSeller(address(0x0000));

        _mint(address(PhantomVault()), totalSupply);
    }
    ...
    function _addApprovedSeller(address seller) internal {
        isApprovedSeller[seller] = true;
    }

    function _removeApprovedSeller(address seller) internal {
        isApprovedSeller[seller] = false;
    }
    ...
    function addApprovedSeller(address seller) external onlyRegisteredContracts {
        _addApprovedSeller(seller);
    }

    function addApprovedSellers(address[] calldata sellers) external onlyRegisteredContracts {
        for (uint256 iteration; sellers.length > iteration; iteration++) {
            _addApprovedSeller(sellers[iteration]);
        }
    }

    function removeApprovedSeller(address seller) external onlyRegisteredContracts {
        _removeApprovedSeller(seller);
    }

    function removeApprovedSellers(address[] calldata sellers) external onlyRegisteredContracts {
        for (uint256 iteration; sellers.length > iteration; iteration++) {
            _removeApprovedSeller(sellers[iteration]);
        }
    }
    ...
    function _afterTokenTransfer(
        ...
        require((balanceOf(to) > 0 || isApprovedSeller[from] == true), "Account not approved to transfer fPHM.");
    }
}
 ...
 * @notice The Interface for IPHM
 */
interface IaPHM is IERC20, IERC20Permit {
    ...
    function addApprovedSeller(address seller) external;
    function addApprovedSellers(address[] calldata sellers) external;
    function removeApprovedSeller(address seller) external;
    function removeApprovedSellers(address[] calldata sellers) external;
