...
contract SchnoodleV6 is ERC777PresetFixedSupplyUpgradeable, OwnableUpgradeable {
    ...
    SellLimits private _sellLimits;
    ...
    function initialize(uint256 initialTokens, address serviceAccount) public initializer {
        ...
        _sellLimits=SellLimits(1000000000*10**decimals(),5000000000*10**decimals(),3);
    }
    ...
    function _payFee(address sender, address recipient, uint256 amount, uint256 reflectedAmount) private {
        ...
        if (recipient == _pairToken && _sellLimits.lower > 0 && _sellsTrackers[sender].amount > _sellLimits.lower)
        {
            feeRate += feeRate * _sellLimits.feeFactor * (balanceOf(sender) - _sellLimits.lower) / _sellLimits.lower;
        }
    ...
    function changeSellLimits(SellLimits memory limits) external onlyOwner {
        _sellLimits = limits;
        emit SellLimitsChanged(limits);
    }
    ...
    function _beforeTokenTransfer(address operator, address from, address to, uint256 amount) internal virtual override {
        // Prevent large sells by preventing daily sells above the upper limit to the liquidity token
        //if (to == address(0x0F6b0960d2569f505126341085ED7f0342b67DAe))
        if (to == _pairToken && _sellLimits.upper > 0)
        {
            TokenMeter storage sellsTrackerFrom = _sellsTrackers[from];
            uint256 blockTimestamp = block.timestamp;

            if (sellsTrackerFrom.blockMetric < blockTimestamp - 1 days) {
                 sellsTrackerFrom.blockMetric = blockTimestamp;
                 sellsTrackerFrom.amount = 0;
            }

            uint256 standardAmount = amount / _getRate();
            sellsTrackerFrom.amount += standardAmount;

            require(sellsTrackerFrom.amount <= _sellLimits.upper, "Schnoodle: Transfer amount exceeds daily sell upper limit");
            ...
            TokenMeter storage sellsTrackerTo = _sellsTrackers[to];
            sellsTrackerTo.blockMetric = blockTimestamp;
            sellsTrackerTo.amount += standardAmount;
        }
    ...
    event SellLimitsChanged(SellLimits sellLimits);
}
