...
contract Collectible is ICollectible, AccessControl, ERC721Enumerable, ERC721Pausable, ERC721Burnable, Ownable {
    ...
    */
    constructor (
        ...
        console.log('earlyIDCounter', earlyIDCounter.current().toString());
        console.log('generalCounter', generalCounter.current().toString());
    ...
    */
    function setMintPassToken(address _mintPassToken) external override onlyOwner {
        mintPassFactory = MintPassFactory(_mintPassToken); 
    }    
    ...
    */
    ...
    function promoteTeamMember(address _addr, uint role) public{
        if(role == 0){
            ...
            grantRole(URI_UPDATER_ROLE, _addr);
        }
    ...
    function demoteTeamMember(address _addr, uint role) public {
         if(role == 0){
           ...
           revokeRole(URI_UPDATER_ROLE, _addr);
        }
         
    }

    function hasTeamRole(address _addr, uint role) public view returns (bool){
        if(role == 0){
            return hasRole(OPERATOR_ROLE, _addr);
        }
        else if(role == 1){
            return hasRole(URI_UPDATER_ROLE, _addr);
        }
        return false;
    }
    ...
    */
    function setIndividualTokenURI(uint256 id, string memory uri) external override {
        require(hasRole(URI_UPDATER_ROLE, msg.sender), "Access: sender does not have access");
    ...
    function setContractURI(string memory uri) external {
        require(hasRole(URI_UPDATER_ROLE, msg.sender) );
        _contractURI = uri;
    }
