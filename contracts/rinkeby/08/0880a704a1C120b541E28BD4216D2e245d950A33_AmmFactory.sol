...
import "../market/IMarketController.sol";
...
contract AmmFactory is OwnableUpgradeable, Proxiable {
    ...
    /// @notice Address of the MarketController associated with this AmmFactory
    ...
    function initialize(
        ...
        IMarketController _marketController
    ) external {
        __AmmFactory_init(
            _ammImplementation,
    ...
    function __AmmFactory_init(
        ...
        require(
            address(_marketController) != address(0x0),
            "Invalid _marketController"
        );
    ...
    /// @param _sirenPriceOracle the PriceOracle contract to use for fetching market and settlement prices
    /// @param _underlyingToken The token whose price movements determine the AMM's Markets' moneyness
    /// @param _priceToken The token whose units are used for all prices
    /// @param _collateralToken The token used for this AMM's Markets' collateral
...
interface IAddMarketToAmm {
    function addMarket(uint64 _marketId) external;
}
...
/**
 ...
 @notice Manages the lifecycle of individual Markets
 @dev The id's for bTokens and wTokens on the same Market are consecutive uints
 */
interface IMarketController {
    /// @notice The basis points to use for fees on the various MarketController functions,
    ...
    /// @notice All data pertaining to an individual market
    struct Market {
        // TODO we can remove this because it's redundant; anytime a caller fetches a Market from the allMarkets
        // array, they'll already have the marketIndex so we don't need to store it here
        uint64 marketIndex;
        uint40 expirationDate;
        bool isPutOption;
        IMarketController.Tokens tokens;
        uint256 strikePrice;
    }

    /// @notice All possible states a Market can be in
    enum MarketState {
        /**
    ...
    /// @notice Emitted when the owner creates a new market
    event MarketCreated(
        uint64 marketIndex,
        Tokens tokens,
    ...
    /// @notice Emitted when the MarketController gets initialized
    event MarketControllerInitialized(
        address priceOracle,
        address vault,
    ...
    /// @notice Emitted when MarketController.mintOptions is called, and wToken is minted
    event OptionMinted(
        address minter,
        uint64 marketIndex,
    ...
    /// @notice Emitted when either the MarketController transfers ERC20 funds to the MarketVault,
    /// or the MarketController transfers funds from the MarketVault to a recipient
    event ERC20VaultTransferIn(
        address sender,
        uint64 marketIndex,
        uint256 amount
    );
    ...
    function markets(uint256 marketId)
        external
        view
        returns (IMarketController.Market memory);

    function state(uint64 _marketIndex) external view returns (MarketState);
    ...
    function getExerciseAmount(uint64 _marketIndex, uint256 _bTokenAmount)
        external
        view
        returns (uint256, uint256);

    function getClaimAmount(uint64 _marketIndex, uint256 _wTokenAmount)
        external
        view
        returns (uint256, uint256);

    function marketName(uint64 _marketIndex)
        external
        view
        returns (string memory);

    function strikePrice(uint64 _marketIndex) external view returns (uint256);

    function expirationDate(uint64 _marketIndex) external view returns (uint40);

    function underlyingToken(uint64 _marketIndex)
        external
        view
        returns (address);

    function priceToken(uint64 _marketIndex) external view returns (address);

    function collateralToken(uint64 _marketIndex)
        external
        view
        returns (address);

    function exerciseFeeBasisPoints(uint64 _marketIndex)
        external
        view
        returns (uint16);

    function closeFeeBasisPoints(uint64 _marketIndex)
        external
        view
        returns (uint16);

    function claimFeeBasisPoints(uint64 _marketIndex)
        external
        view
        returns (uint16);

    function wTokenIndex(uint64 _marketIndex) external pure returns (uint256);

    function bTokenIndex(uint64 _marketIndex) external pure returns (uint256);

    function isPutOption(uint64 _marketIndex) external view returns (bool);
    ...
    /// @notice Returns the amount of collateralToken held in the vault on behalf of the Market at _marketIndex
    /// @param _marketIndex The index of the Market in the MarketController
    function getMarketERC20Balance(uint64 _marketIndex)
        external
        view
        returns (uint256);
    ...
    function mintOptions(uint64 _marketIndex, uint256 _optionTokenAmount)
        external;
    ...
    function claimCollateral(uint64 _marketIndex, uint256 _wTokenAmount)
        external;

    function closePosition(uint64 _marketIndex, uint256 _optionTokenAmount)
        external;
}
