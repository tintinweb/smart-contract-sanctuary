...
contract LitedexPresale is TimeCrowdsale{
    ...
    function purchase(uint256 amount) external onlyWhileOpen returns(bool){
        ...
        if(cappable == false) {
            ...
            pair.transferFrom(msg.sender, address(this), tokenReached.mul(price).div(10 ** _tokendecimals));
        }
     ...
     * return true
     */
     function setCappable(bool activeCap) onlyOwner external returns(bool isCappable){
         cappable = activeCap;
         
         return true;
     }
     
     function setTotalCap(uint256 newTotalCap) onlyOwner external returns(uint256 newCap){
        cap = newTotalCap;
        return cap;
     }
     ...
     * return true
     */
     function setMinContribution(uint256 newMinContribution) onlyOwner external returns(uint256 minContribution){
     ...
     * return true
     */
     function setMaxContribution(uint256 newMaxContribution) onlyOwner external returns(uint256 maxContribution){
     ...
     * return true
     */
     function setPrice(uint256 newPrice) onlyOwner external returns(uint256 latestPrice){
         price = newPrice;
         return price;
     }
