...
contract PotLottery {
    ...
    constructor(address _owner) {
        owner = _owner;
        admin = _owner;
        accountToReceivePotFees = payable(_owner);
    ...
    function addToken(
        ...
        // updateTokenUsdValue(_tokenName, 50000); //should be removed later
    }
    ...
    function calculateWinner() public {
        ...
        */
        ...
        if (
            ...
            uint256 winningPoint = randomNumber % totalPotUsdValue;

            for (uint256 index = 0; index < entriesAddress.length; index++) {
                winningPoint -= entriesUsdValue[index];
                if (winningPoint <= 0) {
                    //That means that the winner has been found here
                    LAST_POT_WINNER = entriesAddress[index];
                    break;
                }
            }
            string
