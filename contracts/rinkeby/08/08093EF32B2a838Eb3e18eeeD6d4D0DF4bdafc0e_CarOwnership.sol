...
// тип части машины
struct CarPartType {
    /// @notice ID части машины
    uint256 Id;
    /// @notice мин индекс при генерации 0 - чать не задана
    uint256 MinIndex;
    /// @notice макс индекс при генерации 0 - часть не задана
    uint256 MaxIndex;
}
...
contract CarFactory {
    ...
    uint256 public _CarTypesCount = 1; // общее количество типов машин

    function CreateCar(uint256 carTypeId) internal returns (Car memory) {
        // ограничитель
        require(carTypeId < _CarTypesCount, "incorrect car type id");
        // генерируем части машины из ее типа
        uint256[] memory parts;
        uint256 color = 1;

        // создаем машину
        Car memory newCar = Car(color, parts);
        // смещение нонса рандома
        _RandNonce++;
        // вывод результата
        return newCar;
    }
    ...
    function _Random() internal view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(msg.sender, _RandNonce, block.timestamp)
                )
            );
    }
}
