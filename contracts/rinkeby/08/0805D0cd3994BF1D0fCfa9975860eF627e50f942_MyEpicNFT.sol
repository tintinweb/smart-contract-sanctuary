...
contract MyEpicNFT is ERC721URIStorage, VRFConsumerBase {
    ...
    function pickRandomFirstWord() public view returns (string memory) {
        uint256 rand = uint256(
            keccak256(abi.encodePacked(randomResult, "FIRST_WORD"))
        );
        return firstWords[rand % firstWords.length];
    }

    function pickRandomSecondWord() public view returns (string memory) {
        uint256 rand = random(
            string(abi.encodePacked(randomResult, "SECOND_WORD"))
        );
        return secondWords[rand % secondWords.length];
    }

    function pickRandomThirdWord() public view returns (string memory) {
        uint256 rand = random(
            string(abi.encodePacked(randomResult, "THIRD_WORD"))
        );
        return thirdWords[rand % thirdWords.length];
    }

    function pickRandomColor(string memory colorSeed)
        public
        view
        returns (string memory)
    {
        uint256 rand = random(
            string(abi.encodePacked(randomResult, colorSeed))
        );
        ...
        override
    {
        ...
        string memory bgColor = pickRandomColor("BACKGROUND_COLOR");
        string memory fontColor = pickRandomColor("FONT_COLOR");

        // Need to make sure the font and bg colors are different.
        while (keccak256(bytes(bgColor)) == keccak256(bytes(fontColor))) {
            uint256 seed = 0;
            fontColor = pickRandomColor(
                string(abi.encodePacked("FONT_COLOR", seed))
            );
            seed++;
        }
        ...
        emit NewEpicNFTMinted(sender, newItemId);
    }
