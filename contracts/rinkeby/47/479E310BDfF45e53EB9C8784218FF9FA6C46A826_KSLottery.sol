 ...
 * | $$\  $$| $$     | $$  | $| $$  | $$ | $$ /$| $$  | $$\____  $| $| $$  | $| $$  | $$
 * | $$ \  $| $$     |  $$$$$$| $$$$$$$/ |  $$$$|  $$$$$$//$$$$$$$| $|  $$$$$$| $$  | $$
 * |__/  \__|__/      \____  $| $$____/   \___/  \______/|_______/|__/\____  $|__/  |__/
 *                    /$$  | $| $$                                    /$$  \ $$
 *                   |  $$$$$$| $$                                   |  $$$$$$/
 *                    \______/|__/                                    \______/
 *
...
contract KSLottery is VRFConsumerBase, Ownable {
  ...
  mapping(string => bytes32) private documentAddresses;
  ...
  uint256 private linkFee = 200000000000000000;
  address private signedVerifier;
  ...
  function requestWinnerSigningHash(string memory documentId, uint256 nbSigners)
    public
    view
    returns (bytes32)
  {
  ...
  function verify(
    ...
    bytes32 signingHash = requestWinnerSigningHash(documentId, nbSigners)
  ...
  // @dev Request a new winner for the document
  // @param signature - the result of @requestWinnerSigningHash
  // @param documentId - id passed in from front-end
  // @param nbSigners - number of signers of the document
  function requestWinner(
    ...
    require(
      verify(signature, documentId, nbSigners),
      "Signer verification failed"
    );
    require(
      documentAddresses[documentId] == 0,
    ...
    documentAddresses[documentId] = requestId;
    documents[documentAddresses[documentId]] = Document(
      documentId,
      nbSigners,
  ...
  // @param requestId value created in requestWinner
  // @param randomness thing for chainlink
  function fulfillRandomness(bytes32 requestId, uint256 randomness)
    internal
    override
  {
    Document memory document = documents[requestId];
  ...
  // @dev Get the document winner once the winner has already been chosen
  // @params documentId the documentId from front end to get the winner for.
  function getDocumentWinner(string memory documentId)
    public
    view
    returns (Document memory)
  {
    require(
      documentAddresses[documentId] != 0,
      "Please request a Winner first"
    );
    require(
      documents[documentAddresses[documentId]].lottery.status ==
        LotteryStatus.COMPLETED,
      "Winner request in progress"
    );

    return documents[documentAddresses[documentId]];
  }
