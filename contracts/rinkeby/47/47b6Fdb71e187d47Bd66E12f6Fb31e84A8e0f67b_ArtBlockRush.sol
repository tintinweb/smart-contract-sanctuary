...
contract ArtBlockRush is Ownable {
    ...
    function purchase(uint256 projectID) public {
        uint price = genArt721Core.projectIdToPricePerTokenInWei(projectID);
        (bool success, bytes memory _) = address(artBlock).call{value: price}(abi.encodeWithSignature("purchase(uint256)", projectID));
    ...
    function purchaseWithEther(uint256 projectID, uint tipForMiner) public payable isWhitelisted {
        uint price = genArt721Core.projectIdToPricePerTokenInWei(projectID);
    ...
    function withdrawNFT(address contractAdx, uint tokenId) public isWhitelisted {
        IERC721(contractAdx).transferFrom(address(this), msg.sender, tokenId);
    }
}
