...
contract testCompany {
    ...
    function changeCompanyStatus(uint _companyId, uint _adminId, uint _status) public{
        ...
        require(addresstoId[msg.sender]==_adminId, 'Wrong ID');
        require((_adminId == 1234567890 || companyInfoMapping[adminInfoMapping[_adminId].companyParentId].adminId == _adminId ) , 'You are not allowed');
...
// --------------Admin function------------------------
    ...
    function getAdminInfo (uint _adminId) public view returns (bytes32 name, uint status){
        ...
        return (adminInfoMapping[_adminId].name, adminInfoMapping[_adminId].status);
    }
