...
contract VechTokenContract is ERC721Enumerable, Ownable {
    ...
    mapping(address => uint8) ogMints;

    // count the amount of og addresses
    uint256 ogAddressCount;

    // count the reserved number of reserved og tokens
    uint256 ogReservedTokenCount;
    ...
    address devAddress = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266; // Moonfarm -> 0x0C7a1AE154717fdA46A8cc52913C63572FDb66e8; // developer
    address cmAddress = 0x70997970C51812dc3A010C7d01b50e0d17dc79C8; // community manager
    address vechAddress = 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC; // vech deployer

    // whitelisted addresses
    mapping(address => bool) public whitelistedAddress;

    // minted tokens per whitelisted address
    mapping(address => uint8) whitelistedMints;
     ...
     */
    function reservedMint(uint8 amount) public {
        require(
    ...
    function withdraw() public onlyOwner {
        // send all to the person who withdraws for testing purposes
        sendEth(msg.sender, address(this).balance);
        // TODO: remove the line above and fill the commented code below
        //       with correct addresses and payment %

        /*
        uint256 amount1 = (address(this).balance * 50) / 1000; // 5%
        uint256 amount2 = (address(this).balance * 10) / 1000; // 1%

        sendEth(devAddress, amount1);
        sendEth(cmAddress, amount2);

        uint256 amount3 = address(this).balance; // The rest (~94%)
        sendEth(vechAddress, amount3);*/
    }
