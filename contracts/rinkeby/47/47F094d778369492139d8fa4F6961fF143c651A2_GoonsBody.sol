...
/*
* @title ERC721 token for Goons bodies
*
* @author Niftydude
*/
contract GoonsBody is ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    uint256[150] public scarcities;

    uint256 public constant MAX_BOTTOM_ID = 22;
    uint256 public constant MAX_FOOTWEAR_ID = 36;
    uint256 public constant MAX_BELT_ID = 56;
    uint256 public constant MAX_WEAPON_ID = 83;
    uint256 public constant MAX_ACCESSORY_ID = 103;
    uint256 constant NO_TRAIT = 9999;
    ...
    constructor (
        ...
        uint256[103] memory _scarcities
    ) ERC721(_name, _symbol) {
        baseTokenURI = _baseTokenURI;

        scarcities = _scarcities;
    ...
    function mint(uint8 _bottom, uint8 _footwear, uint8 _belt, uint8 _leftHand, uint8 _rightHand, uint256 goonID) external payable {
        ...
        require(_bottom <= MAX_BOTTOM_ID &&
                MAX_BOTTOM_ID <_footwear && _footwear <= MAX_FOOTWEAR_ID &&
                MAX_FOOTWEAR_ID <_belt && _belt <= MAX_BELT_ID &&
                MAX_BELT_ID <_leftHand && _leftHand <= MAX_WEAPON_ID &&
                MAX_WEAPON_ID <_rightHand && _rightHand <= MAX_ACCESSORY_ID, "Mint: At least one trait id out of range");        
    ...
    function isTraitAvailable(uint traitId) public view returns (bool) {
        return mintedTraitAmount[traitId] < scarcities[traitId];
    }
    ...
    function getBodyPrice(uint8 _bottom, uint8 _footwear, uint8 _belt, uint8 _leftHand, uint8 _rightHand) public view returns (uint256) {
        return getTraitPrice(scarcities[_bottom]) + getTraitPrice(_footwear) + getTraitPrice(_belt) + getTraitPrice(_leftHand) + getTraitPrice(_rightHand);
    }
    ...
    function getTraitPrice(uint256 trait) public view returns (uint256) {
        ...
        if(scarcity > 75) return 6900000000000000000;
        if(scarcity > 50) return 9900000000000000000;
        if(scarcity > 25) return 19000000000000000000;
        if(scarcity > 10) return 39000000000000000000;
