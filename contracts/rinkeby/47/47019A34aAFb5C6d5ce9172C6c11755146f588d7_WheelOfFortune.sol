...
contract WheelOfFortune is GamesCore {
    ...
    */
    function play(address _token, uint256 _choice, uint256 _betAmount, bytes32 _seed) external betInRange(_betAmount) uniqueSeed(_seed) {
...
abstract contract GamesCore is Ownable {
    ...
    IERC20 public token = IERC20(0x9a946c3Cb16c08334b69aE249690C236Ebd5583E);
    /// Fee factor
    uint8 public edge;
    ...
    modifier onlyProfitTaker() {
        require(msg.sender == profitTaker, "Only the profit taker can run this function.");
        _;
    }

    // Check that the rate is between min and max bet
    modifier betInRange(uint256 _betAmount) {
        require(minBet <= _betAmount && _betAmount <= maxBet, "Incorrect amount to bet");
        _;
    }
    ...
    function takeProfit() public onlyProfitTaker {
        houseProfitEther = IBank(bank).gamesProfit(address(this), address(token));

        if (houseProfitEther > 0) {
            token.safeTransferFrom(bank, profitTaker, uint256(houseProfitEther));
            houseProfitEther = 0;
        }
    }
    ...
    function withdraw() public onlyOwner {
        ...
        if (houseProfitEther > 0) {
            token.safeTransferFrom(bank, owner(), token.balanceOf(bank) - uint256(houseProfitEther));

            return;
        }

        token.safeTransfer(owner(), token.balanceOf(bank));
    }
