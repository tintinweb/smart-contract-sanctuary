    ...
    Ownable
{
    ...
    function _updateExcluded(address from, address to) private {
        if (_balances[vipTokenId][to] > 0 && _rDao.isExcluded(to) == true) {
            _rDao.transfer(to, 1);
            _rDao.includeAccount(to);
        }

        if (
            _balances[vipTokenId][from] == 0 && _rDao.isExcluded(from) == false
        ) {
            _rDao.excludeAccount(from);
        }
    }
...
contract RDAO is Context, IERC20, Ownable {
    ...
    modifier onlyGuardian() {
        require(
            _nftToken.balanceOf(msg.sender, vipTokenId) > 0 ||
                _msgSender() == owner(),
            "Not a guardian"
        );
        _;
    ...
    {
        if (
            _nftToken.balanceOf(recipient, vipTokenId) == 0 &&
            !_isExcluded[recipient]
        ) {
            excludeAccount(recipient);
        }
    ...
    function excludeAccount(address account) public onlyManager {
    ...
    function includeAccount(address account) public onlyManager {
