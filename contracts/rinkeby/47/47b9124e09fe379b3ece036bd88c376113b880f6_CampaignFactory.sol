...
contract CampaignFactory is OwnableUpgradeable, PausableUpgradeable {
    ...
    function registerCampaign(
        ...
        require(
            ...
            "ICOFactory::INVALID_RELEASE_END_TIME"
        );
        require(
            _verifyStartTime < _openTime.sub(1800),
            "ICOFactory::INVALID_VERIFY_START_TIME"
        );
        require(
...
contract TierSystem is Ownable {
    ...
    function isTiered(address _address) public view returns (bool tier) {
        ...
        } else if (block.timestamp >= campaignOpenTime.add(600)) {
            tier =
                userTier[_address].userTier >= 2 &&
                stakBank.balanceOf(_address) >= tier2;
        } else if (block.timestamp >= campaignOpenTime.add(1200)) {
            tier =
                userTier[_address].userTier >= 1 &&
                stakBank.balanceOf(_address) >= tier1;
        } else if (block.timestamp >= campaignOpenTime.add(1800)) {
            tier = true;
        }
    }
        ...
        returns (uint8 tier, uint256 timestamp)
    {
        if (userTier[_address].userTier == 3) {
            ...
            tier = 2;
            timestamp = campaignOpenTime.add(600);
        } else if (userTier[_address].userTier == 1) {
            tier = 1;
            timestamp = campaignOpenTime.add(1200);
        } else if (userTier[_address].userTier == 0) {
            tier = 0;
            timestamp = campaignOpenTime.add(1800);
        }
    }
    ...
    function _setupTierSystem(
        ...
        verifyEndTime = _verifyStartTime.add(1800);
