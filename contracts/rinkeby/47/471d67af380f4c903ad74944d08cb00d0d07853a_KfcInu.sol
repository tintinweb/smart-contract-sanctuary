...
contract KfcInu is Context, IERC20, Ownable {
    ...
    bool whaleTaxEnabled = true;
    uint256 private _whaleTaxEth = 2e18;
    uint256 private _whaleTaxSupply = 2_000_000_000_000 * 10**9;
    uint256 private _whaleTaxFee = 49;
    ...
    // We will set a minimum amount of tokens to be swapped => 5000000000
    uint256 private _numOfTokensToExchangeForMarketing = 5000000000 * 10**9;
    ...
    function deliver(uint256 tAmount) public {
        ...
        (uint256 rAmount,,,,,) = _getValues(tAmount, address(this));
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (,uint256 rTransferAmount,,,,) = _getValues(tAmount, address(this));
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tMarketing) = _getValues(tAmount, recipient);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount, address recipient) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketing) = _getTValues(tAmount, recipient, _taxFee, _marketingFee);
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tMarketing);
    }

    function _getTValues(uint256 tAmount, address recipient, uint256 taxFee, uint256 marketingFee) private view returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.mul(taxFee).div(100);
        uint256 tMarketing = tAmount.mul(marketingFee).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tMarketing);

        // every sell higher than 2 eth or more then 2% supply will be charged with whale tax
        if (whaleTaxEnabled && recipient == uniswapV2Pair) {
            bool activateWhaleTax = tTransferAmount >= _whaleTaxSupply;
            ...
            if (!activateWhaleTax) {
                ...
                uint256 amountOut = UniswapV2Library.getAmountsOut(uniswapV2Router.factory(), tTransferAmount, path)[1];
                if (amountOut > _whaleTaxEth) {
                    activateWhaleTax = true;
                }
            }

            if (activateWhaleTax) {
                uint256 newTaxFee = _whaleTaxFee.div(2);

                tFee = tAmount.mul(newTaxFee).div(100);
                tMarketing = tAmount.mul(_whaleTaxFee.sub(newTaxFee)).div(100);
                tTransferAmount = tAmount.sub(tFee).sub(tMarketing);
            }
        }
    ...
    function setWhaleTexEnabled(bool enabled) external onlyOwner(){
        whaleTaxEnabled = enabled;
    }
}