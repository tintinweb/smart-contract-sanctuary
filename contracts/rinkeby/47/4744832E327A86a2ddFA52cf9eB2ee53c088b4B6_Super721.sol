...
/**
  ...
  August 4th, 2021.
*/
contract Super721 is PermitControl, ERC165, IERC721 {// TODO: ERC-721 import {
    ...
    if (_to.isContract()) {
      try IERC721Receiver(_to).onERC721Received(_operator, _from, _id,
        ...
        if (response != IERC721Receiver(_to).onERC721Received.selector) {
          revert("SUPER721::BatchTransferAcceptanceCheck: ERC721Receiver rejected tokens");
        }
      } catch Error(string memory reason) {
        revert(reason);
      } catch {
        revert("SUPER721::BatchTransferAcceptanceCheck: transfer to non ERC721Receiver implementer");
      }
    }
  }
  ...
  */
  ...
  function getApproved(uint256 tokenId) public view override returns (address) {
      require(_tokenOwners.contains(tokenId), "ERC721: approved query for nonexistent token");

      return _tokenApprovals[tokenId];
  }

  function _isApprovedOrOwner(address spender, uint256 tokenId) internal view returns (bool) {
      require(_tokenOwners.contains(tokenId), "ERC721: operator query for nonexistent token");
      address owner = ownerOf(tokenId);
