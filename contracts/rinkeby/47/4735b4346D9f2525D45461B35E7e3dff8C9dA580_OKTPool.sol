...
// File contracts/Pool/IRewardDistributionRecipient.sol
...
// File contracts/Pool/OKTPool.sol
...
contract OKTPool is TokenWrapper, IRewardDistributionRecipient {
        ...
        checkStart
    {
        ...
        require(
            newDeposit <= (maximum.add(invites[inviter].length)  * (10 ** 18) ),
            'Deposit amount exceeds maximum'
        );
        deposits[msg.sender] = newDeposit;
        super.stake(msg.value);
        if(address(inviter) != address(0) && invites[inviter].length <= 3){
