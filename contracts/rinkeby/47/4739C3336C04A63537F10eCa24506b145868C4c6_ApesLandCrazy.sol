  ...
  UUPSUpgradeable
{
  ...
  address payable public collateral_;
  ...
  address public apesContractAddress;
  ...
  */
  ...
  mapping(uint256 => address) apesAtAddress;
  // buildingType => BaseReward
  mapping(uint256 => uint256) buildingTypeBaseRewards;
  // buildingType => BaseReward, it use the info that a building type only stack one type of ape
  mapping(uint256 => uint256) apeTypeBaseRewards;
  // ape Id => tiemstamp when it start farming
  mapping(uint256 => uint256) apeStartFarmingTimestamp;
  ...
  function initialize() public initializer {
    ...
    landEnabled_ = true;
    collateral_ = payable(address(0x0CfC2614c11AfAe3eE1cED9AEf34ca64a4Bab08A));
    ...
    aGoldAddress = address(0xc9f58137D6c25cB6380f2fb136C64a82367A6C00);
    apeContract = address(0xc32fc0fB0885f3D2B298EfE25b0f7C5cE064788F);
    buildingsContract = address(0xa07D1843fBa9e62fB153Ac80c462CCefbd44b361);
  }
  ...
  function stackBuilding(uint256 buildingNftID) public {
    ...
    uint256 genericType = apeBuilding.typeOfBuildingByNFTId(buildingNftID);
    // hasta aca va
    ...
    apeBuilding.transferFrom(sender, address(0xA1ebFAb1808308CdDBbDd1B71B34170670044174), buildingNftID);
  }
  function stackApe(uint256 apeNftId) public {
    ...
    require(apesAtAddress[apeNftId] == address(0x000), "Building already Stacked");
    uint256 apesByType = addressToApesType[sender][apeNftId].length;
    uint256 maxSlots = addressToBuildingsType[sender][apeNftId].length * 3;
    uint256 freeSlots = maxSlots - apesByType;
    require(freeSlots > 0, "Not free slots");

    addressToApesType[sender][apesInstance.typeOfApeById(apeNftId)].push(apeNftId);
  ...
  function unStackBuilding(uint256 genericType) public {
    ...
    uint256 nftId = addressToBuildingsType[sender][genericType][addressToBuildingsType[sender][genericType].length];
  ...
  /*
    At claim you claim all you've farm and you unstack all the resources involved.
    All the buildings gonna have the same timestamp (the sama as the ape on first position)
  */
  function claimRewards(uint256 genericType) public {
    ...
    uint256 maxDelta = currentTimestamp - apeStartFarmingTimestamp[addressToApesType[sender][genericType][0]];
    totalRewards += maxDelta * addressToBuildingsType[sender][genericType].length * buildingTypeBaseRewards[genericType];
    uint256 totalApesByType = addressToApesType[sender][genericType].length;
    for (uint256 index = 0; index < totalApesByType; index++) {
      uint256 apeId = addressToApesType[msg.sender][genericType][index];
      uint256 delta = currentTimestamp - apeStartFarmingTimestamp[apeId];
      totalRewards += delta * apeTypeBaseRewards[genericType];
    }
    aGold.transferFrom(address(this), sender, totalRewards);
    for (uint256 index = 0; index < addressToBuildingsType[sender][genericType].length; index++) {
      apesInstance.transferFrom(
        sender,
        address(this),
        addressToBuildingsType[sender][genericType][index]
      );
      buildsAtAddress[addressToBuildingsType[sender][genericType][index]] = address(0x000);
      addressToBuildingsType[sender][genericType].pop();
    }
    for (uint256 index = 0; index < totalApesByType; index++) {
      apesInstance.transferFrom(
        sender,
        address(this),
        addressToApesType[sender][genericType][index]
      );
      apesAtAddress[addressToApesType[sender][genericType][index]] = address(0x000);
      addressToApesType[sender][genericType].pop();
    }
  }
  ...
  function getStackedApesIdsByType(uint256 genericType) public view returns(uint256[] memory) {
    address sender = address(msg.sender);
    return addressToApesType[sender][genericType];
  }
  function getStackedBuildingsIdsByType(uint256 genericType) public view returns(uint256[] memory) {
    address sender = address(msg.sender);
    return addressToBuildingsType[sender][genericType];
  }

  //     function claimSpecialRewards(address holder) public {

  //         IERC721Upgradeable apesContract = IERC721Upgradeable(apesContractAddress);
  //     }
  // }
}
...
interface IApeBuilding {
  ...
  function typeOfBuildingByNFTId (uint256 u) external returns(uint256);
}
...
interface IApes {
  ...
  function typeOfApeById (uint256 u) external returns(uint256);
}
