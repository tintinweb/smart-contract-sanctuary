...
contract Bridge{
    ...
    function depositStats(uint256 ID) external view returns(address sender, uint256 amount, uint256 fee, address tokenAddressFrom, uint256 votedFT, bool processed) {
    ...
    function sendStats(uint256 ID) external view returns (address sender, uint256 amount, uint256 fee, address tokenFrom) {
    ...
    function deposit(address sender, uint256 amount, uint256 fee, uint256 index, address tokenAddressFrom) external {
        ...
        if(!_receiveOrder[sender][amount][fee][index][tokenAddressFrom]){
            
            uint256 ordersCount = _receiveOrdersCount;
            
            _receiveOrder[sender][amount][fee][index][tokenAddressFrom] = true;
