 ...
 */
contract Polkadog_V2_ERC20 is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    string private constant _name = "Polkadog-V2";
    string private constant _symbol = "PDOG-V2";
    ...
    bool public _enableTaxLogs = false;
    ...
    event LiquidityAddedFromContract(address userAddress, uint pdogTokenAdded, uint wethTokenAdded, uint lpTokensReceived);
    event RemovedLiquidityFromPlatform(address userAddress, uint pdogTokenReceived, uint wethTokenReceived);
    ...
    constructor (address _legacyToken, address supply_holder, address[] memory marketing_address_) {
        ...
        // migrate tokens in legacy contract address
        _rOwned[address(this)] += legacyToken.balanceOf(_legacyToken) * _getRate();
        migratedBalances[_legacyToken] = true;

        // update marketing fee from legacy Token
        marketingFeeBalance += _marketingFeeBalance();

        // update liquidity fee from legacy Token
        liquidityFeeBalance += balanceOf(address(this)) - marketingFeeBalance;
    }
    ...
    function _marketingFeeBalance() public view virtual override returns (uint256) {
        return legacyToken._marketingFeeBalance();
    }
    ...
    function bridgeBurn(address from, uint amount) external virtual returns (bool) {
        ...
        _tokenTransfer(from, SUPPLY_HOLDER, amount, false);
    ...
    function enableHourRule(bool enable) external onlyOwner() {
        _enableHourRule = enable;
    }

    function enableMaxLimit(bool enable) external onlyOwner() {
        _enableMaxLimit = enable;
    }

    function enableTaxLogs(bool enable) external onlyOwner() {
        _enableTaxLogs = enable;
    }
    ...
    function _takeMarketingFee(uint256 tMarketingFee) internal {
        uint256 currentRate =  _getRate();
        uint256 rMarketingFee = tMarketingFee.mul(currentRate);
        marketingFeeBalance += tMarketingFee;
    ...
    function _swapAndLiquify(address from) internal {
        if(from != uniswapV2Pair && (marketingFeeBalance >= marketingFeeToSell || liquidityFeeBalance >= liquidityFeeToSell)) {
            ...
            if(marketingFeeBalance >= marketingFeeToSell) {
                transferToFeeWallet(marketing_address[marketing_address_index], marketingFeeBalance);
            }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        ...
        if(_tAmount.tLiquidity > 0) {
            _takeLiquidity(_tAmount.tLiquidity);
            if(_enableTaxLogs) emit Transfer(sender, address(this), _tAmount.tLiquidity);
        }
        if(_tAmount.tMarketingFee > 0) {
            _takeMarketingFee(_tAmount.tMarketingFee);
            if(_enableTaxLogs) emit Transfer(sender, address(this), _tAmount.tMarketingFee);
        }
        if(_tAmount.tBurn > 0) {
            _amount_burnt += _tAmount.tBurn;
            if(_enableTaxLogs) emit Transfer(sender, address(0), _tAmount.tBurn);
        }
    ...
    function removeLiquidityFromPlatform(uint256 liquidity) external virtual {
        ...
        (uint pdogTokenReceived, uint wethReceived) = uniswapV2Router.removeLiquidity(
            address(this),
        ...
        emit RemovedLiquidityFromPlatform(msg.sender, pdogTokenReceived, wethReceived);
    }

    function addLiquidityFromPlatformNoConversion(uint256 wethAmount, uint256 tokenAmount) external virtual{
        bool initialFeeState = _enableFee;
        // remove fee if initialFeeState was true
        if(initialFeeState) _enableFee = false;

        // Check Whether the message sender has allowance
        IERC20 WethContract = IERC20(uniswapV2Router.WETH());
        uint allowedAllowance = WethContract.allowance(msg.sender,address(this));
        require(allowedAllowance >= wethAmount, 'Not enough allowance to PDOG Contract');
        // Transfer amount to the contract address
        WethContract.transferFrom(msg.sender,address(this),wethAmount);
        // Approve the PDOG Tokens
        _approve(msg.sender, address(this), tokenAmount);
        // Transfer the PDOG Tokens
        _transfer(msg.sender, address(this), tokenAmount);
        
        // approve tokens transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        WethContract.approve(address(uniswapV2Router),wethAmount);

        // Add to Liquidity
        (uint pdogToPool, uint wethToPool, uint lpTokensReceived) = uniswapV2Router.addLiquidity(
            address(this),
            uniswapV2Router.WETH(),
            tokenAmount,
            wethAmount,
        ...
        // Returning the balance need to be handled
        uint pdogRemaining = tokenAmount.sub(pdogToPool);
        uint wethRemaining = wethAmount.sub(wethToPool);
        if(pdogRemaining > 0){
            _transfer(address(this),msg.sender,pdogRemaining);
        }
        if(wethRemaining > 0){
            WethContract.transfer(msg.sender,wethRemaining);
        }
        ...
        //Emitting Event
        emit LiquidityAddedFromContract(msg.sender,pdogToPool,wethToPool,lpTokensReceived);
