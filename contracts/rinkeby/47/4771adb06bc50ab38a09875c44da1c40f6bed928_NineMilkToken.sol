...
contract NineCat is ERC721Namable, Ownable {
	...
	function giveaway(address receiver, uint256 amountOf9Cat) external onlyOwner {
		require(totalSupply() < MAX_9CAT, "All tokens have been minted");
		require(amountOf9Cat <= MAX_PER_MINT, "Cannot purchase this many tokens in a transaction");
		require(totalSupply() + amountOf9Cat <= MAX_9CAT, "Minting would exceed max supply");
		require(
			_totalClaimed[receiver] + amountOf9Cat <= MAX_9CAT_MINT,
			"Purchase exceeds max allowed per address"
		);
		require(amountOf9Cat > 0, "Must mint at least one dad");
		for (uint256 i = 0; i < amountOf9Cat; i++) {
			uint256 tokenId = num9CatMinted + 1;
			num9CatMinted += 1;
			_totalClaimed[receiver] += 1;
	...
	function checkPresaleEligibility(bytes memory hash, bytes memory signature)
		public
		view
		returns (bool)
	{
		bytes32 b32Hash = bytesToBytes32(hash, 0);
		require(
			ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(msg.sender))) == b32Hash,
			"Invalid Hash"
		);
		return ECDSA.recover(b32Hash, signature) == signer;
	}
	...
	function mintPresale(
		...
		require(checkPresaleEligibility(hash, signature), "You are not eligible for the presale");
		require(totalSupply() < MAX_9CAT, "All tokens have been minted");
		require(amountOf9Cat <= PRESALE_MAX_MINT, "Cannot purchase this many tokens during presale");
		require(totalSupply() + amountOf9Cat <= MAX_9CAT, "Minting would exceed max supply");
		require(
			_totalClaimed[msg.sender] + amountOf9Cat <= PRESALE_MAX_MINT,
			"Purchase exceeds max allowed"
		);
		require(amountOf9Cat > 0, "Must mint at least one dad");
		require(PRESALE_PRICE * amountOf9Cat == msg.value, "ETH amount is incorrect");
	...
	function mint(uint256 amountOf9Cat) external payable whenPublicSaleStarted {
		...
		require(totalSupply() + amountOf9Cat <= MAX_9CAT, "Minting would exceed max supply");
		require(
			_totalClaimed[msg.sender] + amountOf9Cat <= MAX_9CAT_MINT,
			"Purchase exceeds max allowed per address"
		);
		require(amountOf9Cat > 0, "Must mint at least one dad");
		require(PUBLIC_SALE_PRICE * amountOf9Cat == msg.value, "ETH amount is incorrect");
...
contract NineMilkToken is ERC20("9MILK", "9MILK"), Ownable, AccessControl {
	...
	function getTotalClaimable(address _user) external view returns (uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = stackableBalance[_user].mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(
			86400
		);
...
abstract contract ERC721Namable is ERC721Enumerable {
	...
	function retrieveNamable(uint256 tokenId) external view returns (string memory _name, string memory _bio) {
		_name = _tokenName[tokenId];
		_bio = bio[tokenId];
	}
