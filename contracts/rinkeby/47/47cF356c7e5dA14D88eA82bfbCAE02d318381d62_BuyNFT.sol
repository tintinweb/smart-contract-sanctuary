...
contract BuyNFT {
    ...
    function buyNFT(uint256 _tokenId) external {
        ...
        require(erc20.balanceOf(msg.sender) >= sellPrice[_tokenId]);
        erc20.transfer(nft.ownerOf(_tokenId), sellPrice[_tokenId]);
        nft.safeTransferFrom(nft.ownerOf(_tokenId), msg.sender, _tokenId);
        allowSell[_tokenId] = false;

        emit Transaction(nft.ownerOf(_tokenId), msg.sender, _tokenId);
    }
}