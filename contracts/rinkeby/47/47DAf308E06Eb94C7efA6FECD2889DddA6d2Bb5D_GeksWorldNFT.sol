...
contract GeksWorldNFT is ERC721, Ownable {
    ...
    constructor(uint256 maxNftSupply, address payable _treasury) ERC721("GeksWorldNFT", "GEKKO") {
        MAX_SUPPLY = maxNftSupply;
        treasury = _treasury;
    }

    function setProvenanceHash(string memory provenanceHash) public onlyOwner() {
        GEK_PROVENANCE = provenanceHash;
    }
    ...
    function setBaseURI(string memory _newBaseURI) public onlyOwner() {
        require(bytes(baseTokenURI).length == 0, 'BaseURI is already set.');
        baseTokenURI = _newBaseURI;
    }

    function setTokenURI(uint tokenID, string memory uri) public onlyOwner() {
        require(tokenID >= 0 && tokenID <= 9, "Invalid token id");
        bytes memory tempEmptyStringTest = bytes(_tokenURIs[tokenID]);
        require(tempEmptyStringTest.length == 0, "Token URI already set");
