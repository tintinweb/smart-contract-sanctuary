...
contract SVGNFT is ERC721URIStorage, Ownable {
    ...
    string[] private gnomess;

    constructor() ERC721("RandomLines", "RANDNFT") {
        tokenCounter = 0;
    }

    function addGnome(string[] memory gnome) public onlyOwner {
        for (uint256 index = 0; index < gnome.length; index++) {
            gnomess.push(string(abi.encodePacked(gnome[index])));
        }
    }

    function getAllGnomes() public view returns (string[] memory) {
        return gnomess;
    }
