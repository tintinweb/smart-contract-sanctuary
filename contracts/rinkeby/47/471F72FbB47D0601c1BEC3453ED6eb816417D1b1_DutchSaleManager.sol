...
/**
 * @notice Manages a descending price dutch sale for NFTs.
 */
contract DutchSaleManager is
    ...
    UUPSUpgradeable
{
    ...
    /// @dev Set to true if whitelist is needed for sale
    bool private shouldVerifySignatureForBid;
    /// @dev Record tokenId for buyersRecords
    mapping(uint256 => mapping(address => uint256[])) public buyersRecords;
    ...
    event SaleUpdated(uint256 indexed saleId, Sale sale);
    event SaleTokenURIUpdated(uint256 indexed saleId, string baseURI);
    ...
    modifier onlyValidSaleConfig(Sale calldata sale) {
        require(sale.startPrice > 0, "DutchSale: startPrice must > 0");
        require(sale.priceFloor > 0, "DutchSale: priceFloor must > 0");
        require(sale.startPrice > sale.priceFloor, "DutchSale: floor price > start price");
        require(sale.timePerStep > 1, "DutchSale: timePerStep must > 1");
        require(bytes(sale.baseURI).length > 0, "DutchSale: bad baseURI");
        require(address(sale.project).isContract(), "DutchSale: bad project address");
        require(sale.endTime > block.timestamp, "DutchSale: endTime should > now");
        require(sale.totalAmount > 0, "DutchSale: totalAmount must > 0");
        require(sale.saleAmount == 0, "DutchSale: saleAmount must = 0");
        require(sale.saleRevenue == 0, "DutchSale: saleRevenue must = 0");
        require(sale.endTime > sale.startTime, "DutchSale: invalid sale duration");
        require(sale.endTime - sale.startTime <= MAX_MAX_DURATION, "DutchSale: duration must be <= 1000 days");
        _;
    }
    ...
    function tokenURI(address project, uint256 tokenId) public view override returns (string memory) {
        ...
        return
            string(abi.encodePacked(sales[editionInfo.saleId].baseURI, uint256(editionInfo.printEdition).toString()));
    }

    /// @notice Get current price
    function price(uint256 saleId) public view returns (uint256) {
        Sale memory sale = sales[saleId];
        if (block.timestamp <= sale.startTime) {
            return sale.startPrice;
        }
        uint256 priceDelta = ((block.timestamp - sale.startTime) / (sale.timePerStep)) * (sale.priceStep);
        if (priceDelta > sale.startPrice) {
            return sale.priceFloor;
        }
        uint256 updatedPrice = sale.startPrice - priceDelta;
        if (updatedPrice <= sale.priceFloor) {
            return sale.priceFloor;
        } else {
            unchecked {
                return updatedPrice;
    ...
    /// @notice A buyer buy artwork at currrent price and mint NFT.
    function placeOrder(
        uint256 saleId,
        uint8 v,
        bytes32 r,
        ...
        require(bytes(sale.baseURI).length > 0, "DutchSale: sale not found");
        require(sale.saleAmount < sale.totalAmount, "DutchSale: sold out");
        require(sale.startTime <= block.timestamp, "DutchSale: sale is not started yet");
        require(sale.endTime >= block.timestamp, "DutchSale: sale is over");
        uint256 currentPrice = price(saleId);
        require(currentPrice <= msg.value, "DutchSale: eth paid should match price");
        if (shouldVerifySignatureForBid) {
            bytes32 messageHash = keccak256(abi.encode(block.chainid, address(this), _msgSender(), saleId));
            require(_verifySignedMessage(messageHash, v, r, s), "DutchSale: proper signature is required");
        }
        _sendETHToTreasury(currentPrice);
        if (msg.value > currentPrice) {
            _refund(payable(_msgSender()), msg.value - currentPrice);
        }
        sale.saleRevenue += currentPrice;
        uint256 tokenId = sale.project.managerMint(_msgSender(), "");
        uint64 randomPrintEdition = _getPrintEdition(saleId, sale.totalAmount, sale.saleAmount);
        projectTokenIdToEditionInfo[address(sale.project)][tokenId] = EditionInfo({
            saleId: saleId,
            printEdition: randomPrintEdition
        });
        sale.saleAmount += 1;
        buyersRecords[saleId][_msgSender()].push(tokenId);
        emit SaleBidPlaced(saleId, _msgSender(), msg.value, sale.endTime, tokenId, sale.saleAmount, randomPrintEdition);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    /**
     * @notice Set to true if whitelist or account info is needed for the sale
     */
    ...
    /**
     * @notice Creates an sale for the given baseURI.
     */
    function createSale(Sale calldata sale) external onlyValidSaleConfig(sale) onlyOwner {
    ...
    /**
     * @notice If an sale has been created but has not yet received bids, the configuration
     * such as the minPrice may be changed by the owner.
     */
    function updateSale(uint256 saleId, Sale calldata newSale) external onlyValidSaleConfig(newSale) onlyOwner {
        Sale storage sale = sales[saleId];
        require(bytes(sale.baseURI).length > 0, "DutchSale: sale not found");
        require(sale.startTime > block.timestamp, "DutchSale: sale in progress");
        sales[saleId] = newSale;
        emit SaleUpdated(saleId, newSale);
    }

    /**
     * @notice Admin update sale token URI, emergency condition only
     */
    function emergencyUpdateSaleTokenURI(uint256 saleId, string calldata baseURI) external onlyOwner {
        require(bytes(baseURI).length > 0, "DutchSale: bad baseURI");
        sales[saleId].baseURI = baseURI;
        emit SaleTokenURIUpdated(saleId, baseURI);
    }

    /**
     * @notice If an sale has been created but has not yet received bids, it may be canceled by the owner.
     */
    function cancelSale(uint256 saleId) external nonReentrant onlyOwner {
        Sale storage sale = sales[saleId];
        require(bytes(sale.baseURI).length > 0, "DutchSale: sale not found");
        require(sale.saleAmount == 0, "DutchSale: sale in progress");
        delete sales[saleId];
    ...
    /**
     * @notice Refund to original buyer
     */
    function _refund(address payable originalBuyer, uint256 originalAmount) internal {
        _sendValueWithFallbackWithdrawWithLowGasLimit(originalBuyer, originalAmount);
    }
     ...
     *  randomMatrix[saleId] is a mapping to record the relationship of index to value.
    ...
    function _getPrintEdition(
        ...
        mapping(uint64 => uint64) storage tokenMatrix = randomMatrix[_saleId];
        ...
        // print totalAmount starts from 1
