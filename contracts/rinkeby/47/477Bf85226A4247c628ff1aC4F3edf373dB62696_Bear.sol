...
contract Bear is IBear, ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
 ...
 //   uint256 public Price = 8e16; //0.08 ETH
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused nonReentrant {
    ...
    require(minted + amount <= MAX_TOKENS, "Supply is none");
    require(amount > 0, "You can't mint nones");
    if (minted < PAID_TOKENS) {
      require(minted + amount <= PAID_TOKENS, "The tokens you can buy with ETH are all gone, try opensea. Get in the game");
...
//add the logic honey tokens to mint cubs based on price
  ...
  function selectTraits(uint256 seed) internal view returns (BeesCubs memory t) {    
    ...
    seed >>= 16;
    t.senseIndex = selectTrait(uint16(seed & 0xFFFF), 8 + shift);
  }
  ...
  /**
     * called after deployment so that the contract can set Payment Splitter
     * @param _ps the address of the Payment Splitter
   */
    function setPaymentSplitter(address _ps) external onlyOwner {
        ps = IPaymentSplitter(_ps);
    }
...
//only ever called once for community surprise
  function Surprisemint() public onlyOwner {
          //surprise
         _mint(msg.sender, 400000000);
        //community
         _mint(msg.sender, 1200000000);
  }
  /**
