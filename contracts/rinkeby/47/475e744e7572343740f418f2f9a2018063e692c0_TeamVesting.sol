...
contract TeamVesting is Context, Ownable {
    ...
    constructor(address _tokenAddress, uint256 _newStartTime) {
        ...
        totalAmount = 50000e18; // 10% of total supply
        initialUnlock = 200; // 20% inital unlock
        ...
        returns (uint256 _amountVested)
    {
        ...
        if (period < oneYear * 2) {
            // Before 2 years, 30% unlock in 1 year
            uint256 unlockRate =
                _vestingSchedule
                    .totalAmount
                    .mul(300)
            ...
            uint256 unlockRate =
                _vestingSchedule
                    ...
                    .div(percentDenominator)
                    .div(oneYear);
            vestedAmount = unlockRate.mul(period.sub(oneYear * 2)).add(
                _vestingSchedule.totalAmount.mul(4).div(5) // 80% already unlocked
            );
        }
