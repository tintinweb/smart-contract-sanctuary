...
contract WATTS is AccessControl, Ownable, ERC20 {
    ...
    uint256 public slotieClaimStart = 1644620400; // 12 feb 00:00
    uint256 public slotieDeployTime = 1638877989; // 7 dec 12:53
    ...
    function updateReward(address from, address to) external {
        ...
        if (from != address(0) && slotieAddressToLastClaimedTimeStamp[from] < slotieClaimEndTime) {
            _slotieClaim(from, 0, 0, 0, empty, empty);
        }

        if (to != address(0) && slotieAddressToLastClaimedTimeStamp[to] < slotieClaimEndTime) {
    ...
    function slotieJrUpdateReward(address from, address to) external {
        ...
        if (from != address(0) && slotieJrAddressToLastClaimedTimeStamp[from] < slotieJrClaimEndTime) {
            _slotieJrClaim(from, 0, 0, empty);
        }
        
        if (to != address(0) && slotieJrAddressToLastClaimedTimeStamp[to] < slotieJrClaimEndTime) {
    ...
    function _transfer(
        ...
        address recipient,
        uint256 amount
    ) internal override notBlackListed(sender) {
        if(
    ...
    function setLockPeriod(uint256 newLockPeriod) external onlyOwner {
        ...
        //slotieEarnPeriod = lockPeriod - (deployedTime - slotieDeployTime);
        // slotieClaimEndTime = deployedTime + slotieEarnPeriod;
        //slotieJrEarnPeriod = lockPeriod;
    ...
    function setSlotieEarnPeriod(uint256 newSlotieEarnPeriod) external onlyOwner {
        ...
        slotieClaimEndTime = deployedTime + newSlotieEarnPeriod;
    ...
    //function setSlotieJrClaimStart(uint256 newSlotieJrClaimStart) external onlyOwner {
    //    slotieJrClaimStart = newSlotieJrClaimStart;
    //    emit setSlotieJrClaimStartEvent(newSlotieJrClaimStart);
    //}

    function setSlotieJrDeployTime(uint256 newSlotieJrDeployTime) external onlyOwner {
        slotieJrDeployTime = newSlotieJrDeployTime;
        slotieJrClaimEndTime = slotieJrDeployTime + slotieJrEarnPeriod;
    ...
    function setSlotieJrEarnPeriod(uint256 newSlotieJrEarnPeriod) external onlyOwner {
        ...
        slotieJrClaimEndTime = slotieJrDeployTime + newSlotieJrEarnPeriod;
