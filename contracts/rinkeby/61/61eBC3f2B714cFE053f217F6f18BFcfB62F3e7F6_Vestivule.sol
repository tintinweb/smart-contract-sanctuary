...
|_     _||    ___||   | ||  |   | ||  |    ___||  _____||_     _||   | ||  |   | |  |_|  ||   |    |  _____|
  ...
  */
    ...
    uint256 public buy3_Discount = 0.05 ether;
    ...
    function buy3Discount() public payable {
    ...
    function MintAlien(uint amount) external payable {
        require(amount.mul(Mint_price) <= msg.value);
        require(amount <= 12, "Only 12 can fit in the spaceship");
        require(_currentTokenId.add(amount) < MAX_SUPPLY, "Max Supply Reached");
        _mint(_msgSender(), _getNextTokenId());
            _incrementTokenId();
    }
