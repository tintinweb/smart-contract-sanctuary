...
contract Thorgi is ERC20Burnable, Ownable {
    ...
    uint256 public _totalSupply = 100000000 * 10**9;
    uint256 public _mintableSupply = 50000000 * 10**9;
    uint256 public _initialSupply = 50000000 * 10**9;
    uint8 private _decimals = 9;
    uint256 public _mul = 5;
    uint256 public _multiplier = _mul / 2;
    // string private _name = "Thorgi";
	// string private _symbol = "THORGI";
    ...
    function unstakeCorgis(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(stakedBone.length > 0) {
                ...
                uint256 multiplier = 1 + ((_multiplier / 100) * (block.timestamp - corgTimeStaked[id]));
                totalRewards += reward * multiplier;
            } else {
    ...
    function unstakePups(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(stakedBone.length > 0) {
                ...
                uint256 multiplier = 1 + ((_multiplier / 100) * (block.timestamp - pupTimeStaked[id]));
                totalRewards += reward * multiplier;
            } else {
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = corgiTokenIds[msg.sender].length; i > 0; i--) {
            ...
            if(stakedBone.length > 0) {
                ...
                uint256 multiplier = 1 + ((_multiplier / 100) * (block.timestamp - corgTimeStaked[tokenId]));
                totalRewards += reward * multiplier;
            } else {
        ...
        for (uint256 i = pupTokenIds[msg.sender].length; i > 0; i--) {
            ...
            if(stakedBone.length > 0) {
                uint256 reward = ((block.timestamp - pupTimeStaked[pupTokenId]) * pupRatePerDay);
                uint256 multiplier = 1 + ((_multiplier / 100) * (block.timestamp - pupTimeStaked[pupTokenId]));
                totalRewards += reward * multiplier;
            } else {
    ...
    function claimFromPup(uint256 tokenId) public {
        ...
        if(stakedBone.length > 0) {
            ...
            uint256 multiplier = 1 + ((_multiplier / 100) * (block.timestamp - pupTimeStaked[tokenId]));
            totalRewards += reward * multiplier;
        } else { 
    ...
    function checkAllRewards(address _address) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < corgiTokens.length; i++) {
            if(stakedBone.length > 0) {
                ...
                uint256 multiplier = 1 + ((_multiplier / 100) * (block.timestamp - corgTimeStaked[corgiTokens[i]]));
                totalRewards += reward * multiplier;
            } else { 
        ...
        for (uint256 i = 0; i < pupTokens.length; i++) {
             if(stakedBone.length > 0) {
                ...
                uint256 multiplier = 1 + ((_multiplier / 100) * (block.timestamp - pupTimeStaked[pupTokens[i]]));
                totalRewards += reward * multiplier;
            } else { 
