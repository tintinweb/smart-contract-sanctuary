  ...
  NFT721Mint
{
  ...
  function initialize(address payable f8n) public initializer {
    ...
    // The name/address defaults to empty strings and then may be set with `adminUpdateConfig`.
...
abstract contract FoundationNode is Initializable {
  ...
  /**
   * @dev Called once after the initial deployment to set the Foundation contract address.
   */
...
abstract contract FoundationAdminRole is FoundationNode {
  // This file uses 0 data slots (other than what's included via FoundationNode)

  modifier onlyFoundationAdmin() {
    require(IAdminRole(getF8n()).isAdmin(msg.sender), "FoundationAdminRole: caller does not have the Admin role");
    _;
  }
}
 ...
 * @dev Originally implemented in https://etherscan.io/address/0x44d6e8933f8271abcf253c72f9ed7e0e4c0323b3
 */
