...
contract GamingChickAuction is OwnableContract, IERC721Receiver, Initializable{
    ...
    struct UserInfo {
        ...
        EnumerableSet.UintSet depositTokens;     // Token ID
    }
    ...
    EnumerableSet.UintSet private depositTokens;
    EnumerableIntMap.UintToIntMap private nftAttr;
    ...
    event UpdScore(address user, uint32 amount);
    ...
    function initialize() public initializer{
        ...
        pool.token = 0xDD81bfB457E27e36043aC46725EA0fBd2013210d;
        poolInfo.push(pool);

        pool.token = 0xbEf15aA877adEd8F2e72e9dcc188AeF6Adb3E8e6;
        poolInfo.push(pool);

        pool.token = 0x97d48881A7C5c36d103a54Df537c105E8348d4f4;
        poolInfo.push(pool);

        pool.token = 0x7Ac58167E25227B22EeA7bBe7F78ada1e60A8390;
        poolInfo.push(pool);

	}

    function updAllowWithdraw(bool _allowWithdraw) external onlyAdmin {
    ...
    function removeWinnerRecords() public onlyAdmin {
        // require(endTime < updBatchTime && now > updBatchTime + 300, "Operation not allowed");
        uint256 id = batchId - 1;
        for (uint256 i = 0; i < winnerList[id].length; i++) {
            address addr = winnerList[id][i];
            userInfo[2][addr].amount = 0;
            userInfo[2][addr].poolScore = 0;
            userInfo[2][addr].baseScore = 0;
            userInfo[2][addr].sameAttrScore = 0;
            userInfo[2][addr].specialAttrScore = 0;
            userInfo[2][addr].nftAttr = nftAttr;
            userInfo[2][addr].depositTokens = depositTokens;

            totalScoreList[addr] = 0;
        }
    }
    ...
    function depositNFT(uint256 _pid, uint256[] memory _tokenId) public {
        ...
        for(uint256 i=0; i<_tokenId.length; i++){
            ...
            user.depositTokens.add(_tokenId[i]);
            erc721.safeTransferFrom(address(msg.sender), address(this), _tokenId[i]);
            updNftAttrAmount(_pid, _tokenId[i], 1);
        }
    ...
    function withdrawNFT(uint256 _pid, uint256 _amount) public {
        require(now >= freezeTime || allowWithdraw, "Operation not allowed");
        require(_pid < poolInfo.length, "invalid _pid");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        for(uint256 i = 0; i < _amount; i++){
            uint256 tokenId = user.depositTokens.at(0); 
            erc721.safeTransferFrom(address(this), address(msg.sender), tokenId);
            user.depositTokens.remove(tokenId);
            updNftAttrAmount(_pid, tokenId, 0);
        }
    ...
    function setScore(uint256 _pid) internal {
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        totalScoreList[msg.sender] = userInfo[0][msg.sender].poolScore + userInfo[1][msg.sender].poolScore + userInfo[2][msg.sender].poolScore + userInfo[3][msg.sender].poolScore;
        emit UpdScore(msg.sender, totalScoreList[msg.sender]);
    }

    function updNftAttrAmount(uint256 _pid, uint256 _tokenId, uint256 _type) internal {
