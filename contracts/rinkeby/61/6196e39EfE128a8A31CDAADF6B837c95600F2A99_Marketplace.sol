...
contract Marketplace is Ownable, Pausable, IMarketplace, ERC721Holder {
     ...
     */
    constructor(address _shib) public Ownable() {
        Shib = IERC20(_shib);
    }
     ...
     */
    function createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address currency) public whenNotPaused {
        _createOrder(_nftAddress, _assetId, _priceInWei, _expiresAt, currency);
    }
    ...
    function safeExecuteOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public payable whenNotPaused {
        ...
        if(order.currency == 0x0000000000000000000000000000000000000000) {
            ...
            // Transfer token amount to seller
            IERC20(order.currency).transfer(order.seller,order.price);
        }
    ...
    function acceptBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public whenNotPaused {
        ...
        if(order.currency == 0x0000000000000000000000000000000000000000) {
            order.seller.transfer(bid.price);
        }

        else {
            IERC20(order.currency).transfer(order.seller, bid.price);
        }
    ...
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address _currency) internal {
        ...
        emit OrderCreated(orderId, assetOwner, _nftAddress, _assetId, _currency, _priceInWei, _expiresAt);
    }
    ...
    function _createBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt) internal {
        ...
        if(order.currency != 0x0000000000000000000000000000000000000000) {
            IERC20(order.currency).safeTransferFrom(msg.sender, address(this), _priceInWei);
        }
    ...
    function _cancelBid(bytes32 _bidId, address _nftAddress, uint256 _assetId, address payable _bidder, uint256 _escrowAmount) internal {
        ...
        if(order.currency == 0x0000000000000000000000000000000000000000) {
            ...
            IERC20(order.currency).transfer(_bidder, _escrowAmount);
        }
