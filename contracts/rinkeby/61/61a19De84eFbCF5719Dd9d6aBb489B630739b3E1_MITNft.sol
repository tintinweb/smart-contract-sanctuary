...
contract MITNft is BaseNFT {
    ...
    //          events
    /////////////////////////////////
    event SetGene(uint256 indexed tokenId, uint256 gen);
    ...
    function batchSetGens(uint256 [] memory genes, uint256 [] memory tokenIds) external returns(bool) {
    ...
    function getNftInfoByTokenIds(uint256 [] memory tokenIds) public view returns(NftInfo [] memory) {
        NftInfo[] memory nftInfoArr = new NftInfo[] (tokenIds.length) ;
        for(uint256 i = 0; i < tokenIds.length; i++) {
            nftInfoArr[i] = getNftInfo(tokenIds[i]) ;
        }
        return nftInfoArr;
    }

    // get ed NFT Info By Page, page start zero
    function getAllNftInfoByPage(uint256 page, uint256 limit) public view returns(NftInfo [] memory, uint256 ) {
        ...
        for(uint256 i = 0 ;i < len; i++) {
            ...
            nftInfoArr[i] = getNftInfo(tokenId) ;
        }
    ...
    function getOwnerNftInfoByPage(uint256 page, uint256 limit, address owner) public view returns(NftInfo [] memory, uint256) {
        uint256 startIndex = page * limit ;
...
contract BaseNFT is AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
    ...
    function setBaseTokenURI(string memory baseTokenURI) external onlyRole(getRoleAdmin(DEFAULT_ADMIN_ROLE)) returns(bool){
        _baseTokenURI = baseTokenURI ;
        return true ;
    }
    ...
    * @dev Pauses all token transfers.
    ...
    function _migration(uint256 tokenId, uint256 gene, address owner) internal virtual onlyRole(MANAGER_ROLE) returns (bool){
        return true ;
    }
