...
contract Token is Context,Owned,  ERC20 {
    ...
    function transferFromRai(address _from,address _to,uint256 _amount) public  returns (bool success) {

        require(_from != address(0), "ERC20: transfer from the zero address");
        require(_to != address(0), "ERC20: transfer to the zero address");

        uint256 percent = calPercentofToken(_amount,200);
        _burn(_from,percent);
    ...
    function RaiFunc(address _from,uint256 _amount) public{
        uint256 percent = calPercentofToken(_amount,200);
        _burn(_from,percent);
    } 
...
contract Thunder is Context,Token{
   using SafeMath for uint256;
   constructor() {
       symbol = "THN";
       name = "Thunder";
       decimals = 18;
       totalSupply = 5000000000 * 10**9 * 10**9; 
       owner = _msgSender();
...
// token transfers
// ----------------------------------------------------------------------------
contract Thunder2Thunder is Context,Thunder, ReentrancyGuard {
   using SafeMath for uint256;

   uint256 private TotalStakedThunder = 0;
   uint256 private TeamFeesCollector = 0;
        ...
        struct USER
        {
            ...
            uint256 TotalThnRewarded;
        }
   
        mapping(address => USER) public trainer;
   
        function DepositThunder(uint256 tokens) external nonReentrant 
        {
                   
            require(Thunder(address(this)).transferFrom(_msgSender(), address(this), tokens), "Tokens cannot be transferred from user account");
            preFix(tokens);
            
        }
        
        function WithDrawThunder(uint256 tokens) external nonReentrant 
        {
        ...
        if(week > 0){
        Thunder(address(this)).transfer(_msgSender(), tokens);
        
        trainer[_msgSender()].stakedAmount = (trainer[_msgSender()].stakedAmount).sub(tokens);
        TotalStakedThunder = TotalStakedThunder.sub(tokens);
        ...
        {
            uint256 burned =  calPercentofTokens(tokens,6000000);
            awayWithYou(burned);
             Thunder(address(this)).transfer(_msgSender(), tokens.sub(burned));
             trainer[_msgSender()].stakedAmount = (trainer[_msgSender()].stakedAmount).sub(tokens);
             TotalStakedThunder = TotalStakedThunder.sub(tokens);
            if(trainer[_msgSender()].stakedAmount == 0)
            {
        ...
        function ClaimThn() external nonReentrant 
        {
         ...
         // uint256 week = time.div(604800);
          ...
          Thunder(address(this)).transfer(_msgSender(), yourThunder);
          trainer[_msgSender()].TotalThnRewarded = trainer[_msgSender()].TotalThnRewarded.add(yourThunder);
          
        }
        
        function calculateReward(address user) public view returns(uint256) 
        {
            uint256 a = percent(trainer[user].stakedAmount,TotalStakedThunder,8);
            return calPercentofTokens(TotalStakedThunder,a);
        }
        ...
        function preFix(uint256 _tokens) private
        {
          ...
          uint256 burned = calPercentofTokens(_tokens,6000000);
          uint256 stakingPool = calPercentofTokens(_tokens,93000000);
          uint256 TeamChar = calPercentofTokens(_tokens,1000000);
          TeamFeesCollector = TeamFeesCollector.add(TeamChar);
       
          TotalStakedThunder = TotalStakedThunder.add(stakingPool);
        ...
        {
            Thunder(address(this)).transfer(_msgSender(), TeamFeesCollector);
            TeamFeesCollector = 0;
        }
        
        function viewTotalStakedThunder() external view returns(uint256)
        {
            return TotalStakedThunder;
        }
