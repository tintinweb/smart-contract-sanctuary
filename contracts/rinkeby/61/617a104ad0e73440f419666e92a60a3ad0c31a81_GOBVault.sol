...
contract GOBVault is IGOBVault, Ownable {
  ...
  function nextRound() public onlyOwner {
    ...
    uint currentRoundBalance = contractBalance - _previousBalance;

    _roundBalances[_currentRound] = currentRoundBalance;
    _previousBalance = contractBalance;
  ...
  function getCurrentVaultBalance() public view returns(uint) {
    uint contractBalance = (address(this).balance);
    uint currentRoundVaultBalance = contractBalance - _previousBalance;
    return currentRoundVaultBalance;
  }
  ...
  function getUserBalance(uint round) public view returns(uint) {
    require(round != _currentRound, "You've to call next round function before");
