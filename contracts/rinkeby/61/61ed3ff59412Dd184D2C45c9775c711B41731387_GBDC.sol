...
contract IGBDC {
    ...
    string internal constant TOKEN_SYMBOL = "GBDC";
    string internal constant TOKEN_NAME = "Gold Backed Digital Currency";
    uint8 internal constant TOKEN_DECIMALS = 8;

    uint256 private constant DECIMALS_FACTOR = 10**uint256(TOKEN_DECIMALS);
    uint256 internal constant TOKEN_INITIALSUPPLY = 1;

    uint256 internal constant TOKEN_MINTCAPACITY = 1000000 * DECIMALS_FACTOR;
...
contract ERC20Mintable is IGBDC, ERC20WithFees, MinterRole {
...
contract GBDC is IGBDC, ERC20Burnable, ERC20Mintable {

    // Should we set a mint period/duration 
    // so that GBDC can only be minted once per 86,400 seconds??(24hrs)
    constructor()
        MultiOwned(
            makeAddressSingleton(msg.sender),
            1)
        ERC20( TOKEN_NAME, TOKEN_SYMBOL, TOKEN_DECIMALS, TOKEN_INITIALSUPPLY)
        ERC20WithFees( initialTxFee())
        ERC20Mintable( )
        public
    {}
}