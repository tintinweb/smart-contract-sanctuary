...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
        ...
        )
    {
        ...
        require(depositVars.shares > 0, "UNIPILOT::INSUFFICIENT_MINT_AMOUNT");
        ...
        )
    {
        ...
        if (totalLiquidity == 0) {
            ...
            shares = (Math.max(amount0, amount1)).sqrt();
        } else if (totalAmount0 == 0) {
            amount1 = amount1Desired;
            shares = (amount1.mul(totalLiquidity).div(totalAmount1)).sqrt();
        } else if (totalAmount1 == 0) {
            amount0 = amount0Desired;
            shares = (amount0.mul(totalLiquidity).div(totalAmount0)).sqrt();
        } else {
            ...
            shares = (cross.mul(totalLiquidity).div(totalAmount0).div(totalAmount1)).sqrt();
        }
    }
}
