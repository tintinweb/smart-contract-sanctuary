...
contract IncreaseAuction is HasNoEther, Pausable {
  ...
  ///  between 0-10,000.
  function IncreaseAuction(uint256 _ownerCut) public {
    ...
    canBeStoredWith64Bits(_duration)
  {
    ...
    require(_startingPrice>= _endingPrice,"endingPrice should less then startingPrice");
    // require(_duration >= 1 hours,"duration shuold greater then 1 hours");
    // require(_duration <= 24 hours,"duration shuold less then 24 hours");
    require(_duration >= 1 ,"duration shuold greater then 1 hours");
    require(_duration <= 24 ,"duration shuold less then 24 hours");
    uint256 _duration_hr = _duration*3600;//from hour to sec
  ...
  function _computeCurrentPrice(
    ...
    returns (uint256)
  {
    if (_secondsPassed >= _duration) {//exceed the time
      return _endingPrice;
    } else {
      int256 hr_pass =  int256(_secondsPassed) / 3600;
      //let buyer can buy at ending price
      int256 _totalPriceChange = int256(_endingPrice) - int256(_startingPrice);

      int256 _currentPriceChange = _totalPriceChange /int256( (_duration / 3600) - 1);
      int256 _currentPrice = int256(_endingPrice) - int256(hr_pass * _currentPriceChange);
