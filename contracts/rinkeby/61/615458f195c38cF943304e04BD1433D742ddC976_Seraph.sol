...
contract SeraphStorage {
    ...
    /// NoVote -> The peer has not voted yet or can not vote on this permit
    ...
    */
    ...
    mapping(uint256 => mapping(address => bool)) internal _callPermitPeerAllowed;
...
contract Seraph is Ownable, SeraphStorage {
    ...
    event NewKMSAdmin(address _old, address _new);
    event NewKMSClient(bytes4 _clientId, address _old, address _new);

    event NewContractEvent(bytes4 indexed _clientId, address _contractAddress, string _contractName);
    event NewFunctionEvent(bytes4 indexed _clientId, address indexed _contractAddress, address indexed _clientWallet, bytes4 functionSelector, string functionSignature);

    event SignEvent(uint256 _permitId, bytes4 indexed _clientId, address indexed _contractAddress, bytes4 indexed _functionSelector, bytes _callData, address[] _peers);
    event SignPeerEvent(uint256 indexed _permitId, address indexed _peer, PeerVote _vote);
    event HalbornVoteEvent(uint256 indexed _permitId, PeerVote _vote);

    event ExecutePermitEvent(uint256 indexed _permitId, bytes4 indexed _clientId, address indexed _contractAddress, bytes4 _functionSelector, bytes _callData);
    event ExecuteUnprotectedEvent(bytes4 indexed _clientId, address indexed _contractAddress, bytes4 indexed _functionSelector, bytes _callData);
    ...
    function _addPermit(address contractAddress, bytes4 functionSelector, bytes calldata callData, uint256 requiredPeers, address[] calldata peers) internal {
        ...
        if (requiredPeers == 0){
            ...
            for(uint256 i=0; i < requiredPeers; i++) {
                ...
                _callPermitPeerAllowed[_newPermitId][_peer] = true;
     ...
     */
    function _getPermitStageAsBytes(PermitStage stage) internal pure returns (string memory){

        require(stage <= PermitStage.HalbornRejected);

        if (PermitStage.NotExists == stage) return "NotExists";
        if (PermitStage.PermitCreated == stage) return "PermitCreated";
        if (PermitStage.PermitSigned == stage) return "PermitSigned";
        if (PermitStage.HalbornApproved == stage) return "HalbornApproved";
        if (PermitStage.Executed == stage) return "Executed";
        if (PermitStage.PermitRejected == stage) return "PermitRejected";
        if (PermitStage.HalbornRejected == stage) return "HalbornRejected";
            ...
            revert(
                string(
                    abi.encodePacked(
                        ...
                        _getPermitStageAsBytes(expected),
                        " but found ",
                        _getPermitStageAsBytes(found)
                    )
                )
            );
        }
    ...
    function initSeraph(address newGnosisHalbornWallet, bytes16 newkmsAdminKeyId, address newkmsAdminWallet) external initializer {
        ...
        emit NewKMSAdmin(address(0), newkmsAdminWallet);
    }
     ...
     * @dev The permit must exists. The only state that the permit can not be rejected from if exists
    ...
    function reject(uint256 permitId) external onlySeraphAdmin permitExists(permitId) {
        ...
        emit HalbornVoteEvent(permitId, PeerVote.Reject);
    }
    ...
    function approve(uint256 permitId) external permitExists(permitId) onlyKMSPermit(permitId) {
        ...
        emit HalbornVoteEvent(permitId, PeerVote.Accept);
    }
    ...
    function signPeer(uint256 permitId, PeerVote vote) external permitExists(permitId) {
        ...
        // NOTE: If we check _callPermitPeerAllowed here there is no need to check for
        // _requiredPeers, no peer will return true for permits that do not require peer
        require(_callPermitPeerAllowed[permitId][msg.sender], "Seraph: Signer not allowed or already voted");
        ...
        /// Peer already voted + gas refund
        _callPermitPeerAllowed[permitId][msg.sender] = false;
        ...
        emit SignPeerEvent(permitId, msg.sender, vote);
    }
     ...
     * Returning true otherwise
     */
    function isContractProtected(address contractAddress) public view returns(bool){
    ...
    function setKMSAdminWallet(bytes16 newKeyId, address newWallet) external onlyOwner {
        ...
        emit NewKMSAdmin(_old, newWallet);
    }
    ...
    function setClientKMSWallet(bytes4 clientId, bytes16 newKmsId, address newKmsWallet) external onlySeraphAdmin clientExists(clientId) {
        ...
        emit NewKMSClient(clientId, _oldWallet, newKmsWallet);
    }
