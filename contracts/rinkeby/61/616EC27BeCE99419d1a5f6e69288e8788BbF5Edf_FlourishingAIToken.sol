...
abstract contract ERC20VestableInTimestamp is ERC20Burnable, AccessControl {
    ...
    function updateVestingSchedule(
        ...
        for (uint i = 0; i < _scheduleNames.length; i++) {
            ...
            require(_ids[i] != 0, "invalid vesting schedule");
            require(
                _durations[i] > 0 &&
            ...
            if (vestingSchedule.isActive && !_isActives[i]) {
                inActiveSchedule(_ids[i]);
                scheduleQuantity = scheduleQuantity - 1;
            } else if (!vestingSchedule.isActive && _isActives[i]) {
                allActiveSchedules.push(_ids[i]);
                scheduleQuantity = scheduleQuantity + 1;
            }
    ...
    function transferVestingTokens(
        ...
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            require(
                ...
                "beneficiary is not included in the vesting schedule"
            );
            require(
                _vestingAmounts[i] > 0,
                "vesting amount to transfer is invalid"
            );

            super.transfer(_beneficiaries[i], _vestingAmounts[i]);
        }
    }
