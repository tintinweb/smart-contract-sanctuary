...
contract FlypeUPM is Ownable{
    ...
    function getPrice(address lpPair) external view returns (uint price) {                       
        ...
        (uint reserveA, uint reserveB) = IFlypePairRouter(pairRouter).getReserves(lptoken);
        (uint priceA, uint priceB) = IOracleRouter(oracleRuoter).getPrices(lptoken);
        
        uint totalSupply = IFlypePairRouter(pairRouter).totalSupply(lptoken);
        price = 2 * (sqrt(reserveA * priceA * reserveB * priceB) / totalSupply);        
    }
...
contract FlypeLibrary is Ownable {
    ...
    function getAmountOut(uint amountIn, uint priceIn, uint priceOut, address user) internal view returns (uint amountOut, uint256 fee) {
        ...
        uint numerator = priceIn * amountIn * (99800 + 2 * discountCalculator(user));
    ...
    function getAmountIn(uint amountOut, uint priceIn, uint priceOut, address user) internal view returns (uint amountIn, uint256 fee) {
        ...
        uint numerator = priceOut * amountOut * (99800 + 2 * discountCalculator(user));
