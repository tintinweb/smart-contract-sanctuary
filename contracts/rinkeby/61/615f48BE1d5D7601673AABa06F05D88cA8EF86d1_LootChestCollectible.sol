...
contract LootChestCollectible is BDERC1155Tradable, ERC1155Holder {
    ...
    */
    event LootBoxOpened(address indexed opener, uint8 indexed chestType, uint32 quantity, uint32 timeStamp);
    ...
    function buy(ChestType chestType, uint quantity) public payable {
        ...
        if(maxCreatable >= quantity){
            ...
            if(msg.value>=price){
                ...
                ChestSale storage saleRecord = recentSales[currentSaleIndex];
    ...
    function getRecentSalesVolume() public view returns (uint volume) {
        ...
        for(uint i = 0; i<RECENT_SALES_TO_TRACK; i++){
            if(recentSales[i].timestamp >= block.timestamp - RECENT_SALES_TIME_SECONDS){
                volume += getBaseValue(ChestType(recentSales[i].chest_type));
            }
        }
    }
    ...
    */
    function getCurrentFAMEPrice() public view returns (uint price) {
        //First we look at the current fame balance, and the recent sales volume, and calculate a demand ratio:
        ...
        //Then taking the ICO base value of new FAME, and multiplying by the demand ratio.
        ...
        //0.1:1     - 5,000 gwei/FAME
        //0.25:1    - 5,000 gwei/FAME
        //0.5:1     - 5,000 gwei/FAME
        //0.75:1    - 7,500 gwei/FAME
        //1.25:1    - 12,500 gwei/FAME
        //1.5:1     - 15,000 gwei/FAME
        //1.75:1    - 17,500 gwei/FAME
        //2:1       - 20,000 gwei/FAME
        ...
        uint rawDemandRatio = (getContractFAMEBalance() / adjustedSalesVolume) * 10000;
        ...
        price = NEW_FAME_ICO_VALUE * (demandRatio/10000);
    }
