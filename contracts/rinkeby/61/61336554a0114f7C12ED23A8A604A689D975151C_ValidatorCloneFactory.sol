...
contract ValidatorCloneFactory {
    ...
    constructor() {
        ADDRESS_VALIDATOR_MASTER_COPY = address(
            new AddressValidator(new address[](0))
        );
        ID_VALIDATOR_MASTER_COPY = address(
            new IdValidator(new IJukebox.ERC721Grouping[](0))
        );
    }
    ...
    function deployIdValidatorClone(
        IJukebox.ERC721Grouping[] memory erc721Groupings
...
contract AddressValidator is IERC721Validator, Ownable {
    ...
    function initialize(address _owner, address[] memory addrs) public {
        require(owner() == address(0));
        _transferOwnership(_owner);
        _setAddresses(addrs, true);
    }

    function setAddresses(address[] memory addrs, bool set) public onlyOwner {
        _setAddresses(addrs, set);
    }

    function _setAddresses(address[] memory addrs, bool set) internal {
...
contract IdValidator is IERC721Validator, Ownable {
    event IdsSet(IJukebox.ERC721Grouping[] tokenGroupings, bool set);

    mapping(address => mapping(uint256 => bool)) public acceptedTokens;

    constructor(IJukebox.ERC721Grouping[] memory tokenGroupings) {
    ...
    function initialize(
        address _owner,
        IJukebox.ERC721Grouping[] memory tokenGroupings
    ) public {
        require(owner() == address(0));
        _transferOwnership(_owner);
        _setIds(tokenGroupings, true);
    ...
    function setIds(IJukebox.ERC721Grouping[] memory tokenGroupings, bool set)
        public
        onlyOwner
    {
    ...
    function _setIds(IJukebox.ERC721Grouping[] memory tokenGroupings, bool set)
        internal
    {
        for (uint256 i = 0; i < tokenGroupings.length; i++) {
            IJukebox.ERC721Grouping memory grouping = tokenGroupings[i];
            mapping(uint256 => bool) storage acceptedIds = acceptedTokens[
                grouping.tokenAddress
            ];
            uint256[] memory idsToSet = grouping.tokenIds;
