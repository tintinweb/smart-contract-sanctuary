...
library NoobGenerator {
        ...
        returns (string memory)
    {
       ...
       string[4] memory animation = [
        ...
        string memory svg = string(
            abi.encodePacked(
                '<svg width="100%" height="100%" viewBox="0 0 640 640" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" presevAspectRatio="xMidYMid meet" style="background-color: rgb(188,188,188)"><defs>',
                filter[0], // filter1
                "</defs>",
                face,
                "</svg>"
    ...
    function random(string memory seed, string memory key)
        ...
        returns (uint256)
    {
        return uint256(keccak256(abi.encodePacked(key, seed))) % 100;
    }
}
