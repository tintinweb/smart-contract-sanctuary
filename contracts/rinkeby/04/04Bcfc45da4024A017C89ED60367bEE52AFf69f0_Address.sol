...
contract SeaToken is ERC20('Neko Token', 'NEK') {
    ...
    function updateUniRouter(address _router) external onlyOwner {
        ...
        address _uniswapPairAddress = IUniswapV2Factory(_factory).getPair(address(WETH), address(this));
        path = [IUniPair(_uniswapPairAddress).token0(), IUniPair(_uniswapPairAddress).token1()];
