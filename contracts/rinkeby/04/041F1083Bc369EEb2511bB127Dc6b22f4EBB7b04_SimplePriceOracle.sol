...
abstract contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    //TODO: Add whitelist to initialize function.
    function initialize(
        ComptrollerInterface comptroller_,
        InterestRateModel interestRateModel_,
        uint256 initialExchangeRateMantissa_,
     ...
     */
    function mintInternal(uint256 mintAmount) internal nonReentrant whitelistMod returns (uint256, uint256) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemInternal(uint256 redeemTokens) internal nonReentrant whitelistMod returns (uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemUnderlyingInternal(uint256 redeemAmount) internal nonReentrant whitelistMod returns (uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted redeem failed
            return fail(Error(error), FailureInfo.REDEEM_ACCRUE_INTEREST_FAILED);
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function borrowInternal(uint256 borrowAmount) internal nonReentrant whitelistMod returns (uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
     ...
     */
    function repayBorrowInternal(uint256 repayAmount) internal nonReentrant whitelistMod returns (uint256, uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setWhitelist(WhitelistInterface newWhitelist) public override returns (uint256) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_WHITELIST_OWNER_CHECK);
        }
        WhitelistInterface oldWhitelist = whitelist;
        // Ensure invoke whitelist.isWhitelist() returns true
        require(newWhitelist.IS_WHITELIST(), "marker method returned false");
        // Set market's whitelist to newWhitelist
        whitelist = newWhitelist;

        emit NewWhitelist(oldWhitelist, newWhitelist);
    ...
    /**
     * @dev Check protocol operation mode. In whitelist mode only members from whitelist can work with protocol.
     */
    modifier whitelistMod() {
        require(whitelist == WhitelistInterface(0) || whitelist.isWhitelisted(msg.sender));
        _;
    }
...
abstract contract CTokenInterface is CTokenStorage {
    ...
    function _setWhitelist(WhitelistInterface newWhitelist) public virtual returns (uint256);
...
abstract contract WhitelistInterface {
    /// @notice Indicator that this is a WhiteList contract (for inspection)
    bool public constant IS_WHITELIST = true;

    function addMember(address _newAccount) external virtual;

    function removeMember(address _accountToRemove) external virtual;

    function switchWhitelistMode(bool _newState) external virtual;

    function setMaxMembers(uint8 _newThreshold) external virtual;

    function isWhitelisted(address _who) external view virtual returns (bool);
}
