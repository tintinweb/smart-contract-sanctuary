...
contract MusicLabs is ERC721, ERC721URIStorage, Ownable {
    ...
    function mintNFTs(mintNftData memory _nftData, bytes32 hash, bytes memory signature) public payable {
        ...
        require(paidTypes > 0 && msg.value >= tokensFees, "Amount sent is not suffecient");
        require(freeTypes > 0 && nftFreeCopiesAllowed[_nftData.tokenId] > nftFreeCopiesMinted[_nftData.tokenId] && nftFreeCopiesAllowed[_nftData.tokenId] >= nftFreeCopiesMinted[_nftData.tokenId]+freeTypes, "Free copies not available");
        require(paidTypes > 0 && nftPaidAllowed[_nftData.tokenId] > nftPaidMinted[_nftData.tokenId] && nftPaidAllowed[_nftData.tokenId] >= nftPaidMinted[_nftData.tokenId]+paidTypes, "Paid copies not available");
