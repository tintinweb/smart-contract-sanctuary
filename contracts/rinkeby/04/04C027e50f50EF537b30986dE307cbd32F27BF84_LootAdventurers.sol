...
/**
 * @dev Uses LootComponents contract to get the different parts of a loot.
 * Returns an array of 5 elements, as follows:
 * [0] = The sourceArray (indexing starts from 0)
 * (For the remaining components, indexing starts from 1, if index is 0, means there are NO COMPONENTS)
 * [1] = Suffix
 * [2] = Name Prefix
 * [3] = Name Suffix
 * [4] = +1
...
contract LootAdventurers is ERC721URIStorage, Ownable, ReentrancyGuard {
    // Base URI
    string private _baseURIextended =
        "https://ipfs.io/ipfs/bafybeihfwvhjh4xqfmn6ltuaicd4na46dy665a7cpb7vxt56v4gfwv6zlq/parts/";

    address public vault;
    uint256 public lootersPrice = 0.02 * (10**18); //0.02 ETH
    uint256 public publicPrice = 0.05 * (10**18); //0.03 ETH
    ...
    // TODO: - Update constructor to remove addresses before deploying
    constructor(address _lootAddress) ERC721("LootAdventurers", "LOOTADV") {
        lootAddress = _lootAddress;
        lootContract = LootInterface(_lootAddress);
    }
    ...
    function testFreeMintWithLoot(uint256 lootId)
        public
        payable
        nonReentrant
        onlyOwner
    {
        ...
        require(!_exists(lootId), "Token has already been minted");
        ...
        nonReentrant
    {
        ...
        require(saleIsActive, "Sale must be active to mint");
        require(
            (lootersPrice * lootIds.length) <= msg.value,
        ...
        for (uint256 i = 0; i < lootIds.length; i++) {
            ...
            require(
                !_exists(lootIds[i]),
                "One of the token has already been minted"
            );
