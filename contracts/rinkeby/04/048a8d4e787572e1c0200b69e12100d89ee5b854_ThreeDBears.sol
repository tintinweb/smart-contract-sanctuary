...
contract ThreeDBears is ERC721Enumerable, RandomlyAssigned, Ownable {
    ...
    function discountMint(uint16[] memory tokenIds) public payable {
        uint256 length = tokenIds.length;

        require(isDiscountSaleActive, "Discount sale is not active" );
        require(msg.value >= length * discountPrice, "Ether value sent is not correct");
        require(length <= MAX_BEARS_PER_MINT, "Exceeds maximum bears you can purchase in a single transaction");
