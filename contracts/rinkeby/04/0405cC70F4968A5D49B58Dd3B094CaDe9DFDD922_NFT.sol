...
contract NFT is ERC721Enumerable, Ownable { 
    ...
    function mint(uint256 _mintAmount) external payable {
        ...
        require(
            ...
            "Can only mint maxMintAmount tokens at a time"
        );
        require(
        ...
        require(
            ownerTokenCount.add(_mintAmount) <= maxMintAmount,
            "Single Address Can only mint maxMintAmount tokens "
        );
    ...
    function preSaleBuy(uint256 _amount) external payable {
        require(
            ...
            "preSaleBuy: Only for Whitelist users."
        );
    ...
    function addWhitelistUser(address[] memory _users, uint256[] memory _amount)
        external
        onlyOwner
    {
        uint256 l = _users.length;
        for (uint256 i = 0; i < l; i = i + 1) {
            whiteList[_users[i]] = _amount[i];
        }
    }
