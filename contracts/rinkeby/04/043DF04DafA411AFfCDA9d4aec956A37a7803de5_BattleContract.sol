...
contract BattleContract is VRFConsumerBase, Ownable {
    ...
    event EndBattle(uint256 battleId, bytes32 requestId);
        ...
        _canEndBattle(_battleId)
    {
        ...
        _getRandomNumber(_battleId, participants[_battleId].length);
    }

    function _getRandomNumber(uint256 _battleId, uint256 _userProvidedSeed)
        private
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        _generateRandomNumber(currentBattleId, requestId, randomness);
    ...
    function _generateRandomNumber(
        ...
        require(
            battles[_battleId].status == Status.END,
            "BattleContract: Battle is not ended."
        );

        uint256 winnerIndex = _randNum % participants[_battleId].length;
        ...
        emit EndBattle(_battleId, _requestId);
    }
