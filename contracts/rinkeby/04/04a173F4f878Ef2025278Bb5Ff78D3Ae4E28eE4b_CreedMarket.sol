...
// File: contracts/CreedMarket.sol
...
contract CreedMarket is OrderState, TransferSafe {
	...
	ICreedMintableStore private _creedMintableStore;
	address public weth;
		...
		if (
			...
			if (order.orderType == 1 || order.orderType == 3) {
				if (!isStore) {
					...
					require(
						order.key.sellAsset.token == address(_creedMintableStore),
						"invalid sell asset"
					);
					erc1155safeMintTransferFrom(
						ICreedMintableStore(order.key.sellAsset.token),
						order.key.owner,
						buyer,
						order.key.sellAsset.tokenId,
						1,
				...
				if (!isStore) {
					...
					require(
						order.key.buyAsset.token == address(_creedMintableStore),
						"invalid buy asset"
					);
					erc1155safeMintTransferFrom(
						ICreedMintableStore(order.key.buyAsset.token),
						buyer,
						order.key.owner,
						order.key.buyAsset.tokenId,
						1,
	...
	function setMintableStore(address newMintableStore) external onlyOwner {
		...
		_creedMintableStore = ICreedMintableStore(newMintableStore);
	...
	function creedMintableStore() external view returns (address) {
		return address(_creedMintableStore);
	}
}