...
contract HailTest7 is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Bitmask for uint256;

    uint constant MAX_UINT = 2**256 - 1;

    constructor()
    ERC721("HAILTEST7", "HST7") {
    ...
    function nextLower(uint _h) internal pure returns (uint) {
        ...
        // NOTE: Once you reach a number divisible by 3 you can only
        //       keep doubling forever (>_<). This is no fun so we catch
        //       this before we ever get to that situation.
        if (((_h / 3) % 3 == 0)) {
            return MAX_UINT;
        }
    ...
    function getJsonName(uint _tokenId) internal pure returns (string memory) {
        return string(abi.encodePacked('"name": "HAILTEST7 ', Conv.uintToString(_tokenId), '"'));
    }

    function getJsonDescription(uint _tokenId) internal pure returns (string memory) {
        return string(abi.encodePacked(',"description": "HAILTEST7 ', Conv.uintToString(_tokenId), '"'));
    }

    function getJsonImage(uint _tokenId) internal pure returns (string memory) {
        return string(abi.encodePacked(',"image": "data:image/svg+xml;base64,', Base64.encode(bytes(HailTestImage.createSvgImage(_tokenId))), '"'));
    }
    ...
    function createThreeGraph(bool _log, uint _head, uint _tokenId) internal view returns (string memory) {
        ...
        return string(abi.encodePacked(
            ...
            "&network=0",   // PROD: network=1 for prod
            logStr
        ));
    }
    ...
    function contractURI() public view returns (string memory output) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked(
            '{'
                '"name": "HAILTEST7", '
                '"description": "HAILTEST7 is a work in solidity celebrating the Collatz conjecture.", '
...
library HailTestImage {
    function createSvgImage(uint _tokenId) external pure returns (string memory) {
        return string(abi.encodePacked(
            ...
            '<text x="175" y="52" class="base tit">HAILTEST7</text>'
