...
contract GenesisVault is TokenVault, ContractGuard {
    ...
    function beginGenesis() onlyOperator public {
        ...
        IBasisAsset(share).mint(address(this), totalMultipliedWBTCTokens.mul(1e10).div(2).mul(getStakingTokenPrice()).div(getShareTokenPrice()).div(1e10));
        ...
        for (uint256 i = 0; i < stakersList.length; i++) {
            ...
            uint256 pegAmount = seat.multipliedNumWBTCTokens.mul(1e10).div(2);
            uint256 shareAmount = seat.multipliedNumWBTCTokens.mul(1e10).div(2).mul(getStakingTokenPrice()).div(getShareTokenPrice()).div(1e10);
