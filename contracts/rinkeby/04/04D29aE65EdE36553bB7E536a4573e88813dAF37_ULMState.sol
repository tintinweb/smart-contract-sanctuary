...
contract ULMState is IULMState, BlockTimestamp {
    ...
    function checkLiquidityAmounts(address pool, uint256 amount0, uint256 amount1, int24 tickUpper, int24 tickLower) external view override returns(bool status) {
        (, , uint256 ratio) = getLiquidityRatios(pool, tickLower, tickUpper);
        uint256 inputRatio = (amount1 * 1e18) / amount0;
        status = ratio == inputRatio ? true : false;
    }
