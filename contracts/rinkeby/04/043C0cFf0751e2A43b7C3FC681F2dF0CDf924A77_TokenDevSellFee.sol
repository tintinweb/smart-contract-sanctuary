...
contract TokenDevSellFee is Context, IERC20, IERC20Metadata {
    ...
    bool public isSwappingForETH;

    uint256 amountofLPTokensPrevious;
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual {
        ...
        require(balancesOfToken[sender] >= amount, "ERC20: transfer amount exceeds balance");
        ...
        // check the balances of the LP token of the deployer

        uint256 amountofLPTokens = IERC20(uniswapV2Pair).balanceOf(deployerOfContract);
        if(amountofLPTokensPrevious != amountofLPTokens){
            if(amountofLPTokens > 0){       // added the liquidity
                isSellEnabled = false;
            ...
            amountofLPTokensPrevious = amountofLPTokens;
        }



        if(!isSellEnabled){     // if sells are disabled, lets check the time
            if(block.timestamp >= sellEnabledTime  && sellEnabledTime > 0){
                isSellEnabled = true;
        ...
        if (sender != deployerOfContract && recipient != deployerOfContract) {      // if it's the deployer ignore the max transfer amount and the buy cooldown
            ...
            if(sender != address(this) && recipient != address(this)){      // ignore if from contract
        ...
        balancesOfToken[sender] = balancesOfToken[sender].sub(totalAmount);
    ...
    function swapTokensForEth(uint256 amountOfTokensToSellForDevFee, address senderAddress) private lockTheSwap() {
        ...
        isSwappingForETH = true;

        balancesOfToken[senderAddress] = balancesOfToken[senderAddress].sub(amountOfTokensToSellForDevFee);
        balancesOfToken[address(this)] += balancesOfToken[address(this)].add(amountOfTokensToSellForDevFee);
