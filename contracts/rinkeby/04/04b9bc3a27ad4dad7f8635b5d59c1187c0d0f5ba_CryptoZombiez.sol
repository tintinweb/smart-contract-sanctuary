...
contract CryptoZombiez is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public maxPresaleMintPerTransaction = 2;
    ...
    constructor() ERC721("CryptoZombiez", "ZOMBIE") {
        signVerifier = 0xEDa74E41482826C1513271B46251D01892d0307d;
    }
    ...
    function getSigningHash(address recipient, uint256 tokenCount) public pure returns (bytes32) {
    ...
    // @dev Dynamically set the max mints a user can do in the main sale
    function setMaxPresaleMintPerTransaction(uint256 maxMint) external onlyOwner {
        maxPresaleMintPerTransaction = maxMint;
    }

    function isValidSignature(bytes memory sig) private view returns (bool) {
        bytes32 message = getSigningHash(msg.sender, 1).toEthSignedMessageHash();
        return ECDSA.recover(message, sig) == signVerifier;
    }

    // @dev Returns number of remaining mints for approved address
    function getAvailablePresaleMints(bytes memory sig) external view returns (uint256) {
        require(isValidSignature(sig), "Account is not authorized for presale");
        return maxPresaleMintPerTransaction - presaleClaimed[msg.sender];
    }
    ...
    function presaleMint(bytes memory sig, uint256 numberOfMints) external payable nonReentrant {
        ...
        require(isValidSignature(sig), "Account is not authorized for presale");
        require(numberOfMints <= maxPresaleMintPerTransaction - claimed, "Amount exceeds mintable limit");
        require(numberOfMints > 0, "The minimum number of mints it 1");
