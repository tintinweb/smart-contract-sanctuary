...
contract DiggCollection is PoolTokenWrapper, Ownable, Pausable, FractionalExponents {
	...
	function earned(address account, uint256 pool) public view returns (uint256) {
		...
		(uint256 rewardRateMantissa, uint8 rewardRateExponent) =
			power(multiplyer.mul(balanceOf(account, pool)), uint256(1e18), expN, expD); // get the reward rate
		uint256 rewardRateNoCoefficient =
			rewardRateMantissa.mul(uint256(1 ether)).div(uint256(1) << uint256(rewardRateExponent)); // convert to realAmount
		uint256 rewardRate = rewardRateNoCoefficient.mul(rootN).div(rootD).div(86400);
