...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    /**
     * @param rate Number of token units a buyer gets per wei(multiplies 1000)
    ...
    function _getTokenAmount(uint256 weiAmount)
        ...
        returns (uint256)
    {
        return weiAmount.mul(_rate).div(_rateMax);
    }
...
// File: contracts/access/roles/CapperRole.sol
...
// File: contracts/crowdsale/validation/IndividuallyCappedCrowdSale.sol
...
contract IndividuallyCappedCrowdsale is Crowdsale, CapperRole {
     ...
     * @param minCap Min amount of wei to be contributed
     * @param maxCap Max amount of wei to be contributed
     */
    constructor(uint256 minCap, uint256 maxCap) public {
        require(maxCap > 0, "IndividuallyCappedCrowdsale: Max Cap is 0");
        _minCap = minCap;
        _maxCap = maxCap;
    }
        ...
        view
    {
        ...
        require(
            _contributions[beneficiary].add(weiAmount) >= _minCap,
...
contract FinalizableCrowdsale is CappedCrowdsale, Ownable {
    ...
    function finalize() public onlyOwner {
        ...
        require(capReached(), "FinalizableCrowdsale: cap not reached");
...
contract PostDeliveryCrowdsale is FinalizableCrowdsale {
    ...
    function addPrivateSaleAllocation(
        address[] memory _recipients,
        uint256[] memory _allocations
    ) public onlyOwner {
        require(_recipients.length == _allocations.length);
        for (uint256 i = 0; i < _allocations.length; i++) {
            _balances[_recipients[i]] = _allocations[i];
        }
    }
...
// File: contracts/crowdsale/UniCycleCrowdSale.sol
    ...
    PostDeliveryCrowdsale
{
    constructor(
        ...
        address cycleToken,
        uint256 hardCap,
        uint256 walletMinCap,
        uint256 walletMaxCap
    )
        public
        PostDeliveryCrowdsale()
        IndividuallyCappedCrowdsale(walletMinCap, walletMaxCap)
        CappedCrowdsale(hardCap)
        Crowdsale(rate, wallet, IERC20(cycleToken))
    {}
}