...
contract TestSunFestival is ERC721Enumerable, Ownable {  
    using Address for address;

    string public baseTokenURI;    
    uint256 public reserved = 60;
    uint256 public price = 0.03 ether;
    uint256 constant MAX_SUPPLY = 9721;
    bool public presaleActive = false;
    bool public saleActive = false;    

   // Team withdrawal addresses
    address public a1;
    address public a2;
    address public a3;
    address public a4;
    ...
    // * Start of the Moon Code, with some fancy functions Moonies like :D
    constructor (string memory newBaseURI) ERC721 ("Test Sun Festival", "TSF") {
        setBaseURI(newBaseURI);
    }

    // Get the Tokens of a particular wallet so they can participate in DAO and holder giveaways
    ...
    // Admin minting of reserved tokens for giveaways
    ...
    function mintPresale(uint256 _amount) public payable {
        ...
        require( reservedAmt > 0,                "Your address is not on the whitelist" );
    ...
    function mintToken(uint256 _amount) public payable {
        ...
        require( _amount > 0 && _amount < 11,    "Can only mint between 1 and 10 at once" );
        require( supply + _amount <= MAX_SUPPLY, "Can't mint more than max supply" );
        require( msg.value == price * _amount,   "Wrong amount of ETH sent" );
    ...
    // Set new baseURI after launch
    function setBaseURI(string memory baseURI) public onlyOwner {
        baseTokenURI = baseURI;
    }
    // Set a different price if ETH changes drastically
    function setPrice(uint256 newPrice) public onlyOwner {
        price = newPrice;
    }

    // Set team addresses and withdraw funds for the team
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        // 20% per team member
        require(payable(a1).send(percent * 20));
        require(payable(a2).send(percent * 20));
        require(payable(a3).send(percent * 20));
        require(payable(a4).send(percent * 20));
        // 20% to the community wallet to invest through DAO and for charity giveaway
        require(payable(a5).send(percent * 20));
    }
}
