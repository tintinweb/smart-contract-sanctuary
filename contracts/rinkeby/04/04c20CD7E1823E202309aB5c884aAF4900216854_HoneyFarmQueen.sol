...
contract HoneyFarmQueen is Ownable, ReentrancyGuard {
    ...
    uint16 public EPOCHS_BEFORE_INACTIVE_BEE = 10; //number of epochs that a bee can claim honey before becoming inactive

    // we set it to 2^16 = 65,536 as bees max supply is 20700 so the first id that is generated, will be lower than this
    uint16 private lowestBeeId = type(uint16).max;
    ...
    uint256 public EPOCH_LENGTH = 86400; //one day

    uint256 public HIVE_CLAIM_EPOCH_LENGTH = 86400; //one day
    ...
    function claimBeesHoney(uint16[] calldata _beesIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _beesIds.length; i++) {
            ...
            uint16 epochsToReward;
            uint256 lastReward = bee.lastRewardTime;

            if (lastReward > 0 && lastReward > STARTING_POINT) {
                // solhint-disable-next-line
                //we get whole numbers for example if someone claims after 1 round and a half, he should be rewarded for 1 round.
                epochsToReward = uint16((block.timestamp - lastReward) / EPOCH_LENGTH);
            } else {
                // if no rewards claimed so far, then he gets rewards from when the rewards started.
                epochsToReward = uint16((block.timestamp - STARTING_POINT) / EPOCH_LENGTH);
            }
    ...
    function mintBee(uint16 _hiveId) external nonReentrant {
        ...
        require(lastClaimedBeePerHive[_hiveId] < block.timestamp - HIVE_CLAIM_EPOCH_LENGTH, "Mint bee cooldown");
        ...
        idsAndBees[beeId] = Bee(beeId, 1, EPOCHS_BEFORE_INACTIVE_BEE, 0, block.timestamp, 0, 0);
    ...
    /**
     * @dev after MIN_USAGE_PER_HIVE, a hive becomes inactive so it needs to be activated so we can mint more Bees
     */
    ...
    function getUnclaimedHoneyForBears(address _owner) external view returns (uint256 amount) {
        uint16[] memory bearsIds = bears.tokensOfOwner(_owner);
        for (uint16 i = 0; i < bearsIds.length; i++) {
            uint16 id = bearsIds[i];
    ...
    function getUnclaimedHoneyForBees(address _owner) external view returns (uint256 amount) {
        uint16[] memory beesIds = bees.tokensOfOwner(_owner);
    ...
    function setPauseState(
        ...
        paused.pauseHive = _pauseHive;
        paused.pauseBee = _pauseBee;
    }
...
abstract contract BearsDeluxeI is Ownable, IERC721 {
    ...
    function tokensOfOwner(address _owner) external view virtual returns (uint16[] memory);
}
