...
contract SafuInvestmentsPresale {
    ...
    function getTokenAmount(uint256 _weiAmount) internal view returns (uint256) {
        return _weiAmount.mul(tokenPriceInWei).div(10**uint256(ITokenDecimals(address(token)).decimals()));
    }
...
contract SafuInvestmentsInfo is Ownable {
    ...
    uint256 private minDevFeeInWei = 2000 ether;
...
contract TokenTimelock {
    ...
    function extendLock(
        ...
        require(msg.sender == IOwnable(SAFU).owner(), "TokenTimelock: invalid caller");
