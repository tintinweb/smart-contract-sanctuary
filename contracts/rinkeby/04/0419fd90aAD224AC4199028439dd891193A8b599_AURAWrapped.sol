...
contract AURAWrapped is ERC20("Koi AURA", "AURA") {
  IAURA KOIContract = IAURA(0x7011F2af733Bdc5b9D6faEDB0816bFc5aFB2BD40);
  ...
  function OwnerTestAURA(uint256 _KOI_ID) external view returns (uint256 _Test) {

    _Test =KOIContract.AURA(_KOI_ID);

  }

  function OwnerTestAURAallowance(uint256 _KOI_ID)
    external
    view
    returns (uint256 _Test)
  {
     _Test =KOIContract.AURAallowance(_KOI_ID, _Treasurer);
    
  }

  function OwnerTest(uint256 _KOI_ID) external view returns (address _Test,address _Sender) {
    _Test=KOIContract.ownerOf(_KOI_ID) ;
    _Sender=msg.sender;
    
  }


  function OwnerTestApproved(uint256 _KOI_ID) external view returns (bool _Test) {
    _Test= (KOIContract.getApproved(_KOI_ID) == msg.sender);
  }

  // yes i know that these modifiers are not needed since transferFrom will revert anyway
  
  function Tesstwrap(uint256 _KOI_ID, uint256 amountToWrap)
    external
