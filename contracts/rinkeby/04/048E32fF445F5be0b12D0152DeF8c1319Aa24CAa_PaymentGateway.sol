...
contract MetaMatsMarketplace is Ownable {

  MetaMats private _MetaMats;
  ...
  constructor(address _MetaMatsContractAddress, address _PaymentGatewayAddress, address payable _publisherWallet) {
    _setMetaMatsContract(_MetaMatsContractAddress);
  ...
  function _setMetaMatsContract(address _MetaMatsContractAddress) private onlyOwner{
    _MetaMats = MetaMats(_MetaMatsContractAddress);
  }

  function setOffer(uint256 price, uint256 tokenId, address tokenAddress) public{
    require(_MetaMats.ownerOf(tokenId) == msg.sender, "Only the owner of the artwork is allowed to do this");
    require(_MetaMats.isApprovedForAll(msg.sender, address(this)) == true, "Not approved to sell");
    ...
    emit artworkAdded(address(_MetaMats), msg.sender, price, tokenId, offerId, false);
  }
  ...
  function buyArt(uint256 tokenId, address tokenAddress) public payable{
    ...
    _MetaMats.safeTransferFrom(offer.seller, msg.sender, tokenId);
  ...
  function _distributeFees(uint256 tokenId, uint256 price, address payable seller) internal{
    uint8 creatorRoyalty = _MetaMats.getRoyalty(tokenId);
    ...
    address payable creator = _MetaMats.getCreator(tokenId);
...
contract MetaMats is ERC721 {
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;

  constructor () ERC721("MetaMats", "MM"){}
