    ...
    ReentrancyGuardUpgradeable
{
    ...
    function lockFundsInternal(address _addr) external onlyOwnerOrCTO
    {
        lockFunds(_addr, 806400 * 10**decimals());
    }
...
function lockFunds(address _addr, uint256 _toLock) private returns (bool) {
       
        
        lockedBalances[_addr] += _toLock;
        totalLockedTokens += _toLock;
        _balances[_addr] -= _toLock;
        emit Locked(_addr, _toLock);
        return true;
