...
contract EtherGrass2 is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
  ...
  uint256 public constant PRICE = 5.0 * 10**16;
  uint256 public constant MAX_BY_MINT = 1;
  uint256 public constant MAX_PRESALE_MINT_PER_GRASS = 1;
  address public constant creatorAddress = 0xc8799A6b6b78f3409A893bd8dd1f2399a9b05FCf;
  string public baseTokenURI;

  uint256[2] private ETHERGRASS_R_IDS = [
    87368869677544583522671190548056816605929929487680250343076510543088019570689,
    87368869677544583522671190548056816605929929487680250343076510544187531198465
  ];

  uint256[50] private ETHERGRASS_IDS = [
  ...
  mapping (address => uint256) private _presaleAddressMintCount;

  event CreateGrass(uint256 indexed id);

  OpenSeaSharedStorefrontInterface public openseaSharedStorefront;

  constructor(string memory baseURI) ERC721("EtherGrass2", "EGRASS") {
    setBaseURI(baseURI);
    pause(true);
  }
  ...
  function preSaleMint(address _to, uint256 _count) public payable {
    ...
    uint256 total = _totalSupply();
    uint256 preSaleMintAmountAllowedTotal = preSaleMintAmountAllowed(msgSender);
    uint256 preSaleMintAmountUsed = _presaleAddressMintCount[msgSender];

    require(preSaleMintAmountAllowedTotal > 0, "No More Premint Items Available");
    require(_count + preSaleMintAmountUsed <= preSaleMintAmountAllowedTotal, "Too Many Premint Items Requested");
    require(total + _count <= MAX_ELEMENTS, "Not Enough Available");
    require(total <= MAX_ELEMENTS, "Sold Out");
    require(_count <= MAX_BY_MINT, "Mint Amount Exceeded");
    require(msg.value >= price(_count), "Value Below Price");

    _presaleAddressMintCount[msgSender] = preSaleMintAmountUsed.add(_count);
  ...
  function ethergrassOwned(address _address) public view returns (uint256) {

    address[] memory senderAddressArray;
    uint256[] memory ethergrassIds; 
    for (uint256 i = 0; i < 50; i++) {
      senderAddressArray[i] = _address;
      ethergrassIds[i] = ETHERGRASS_R_IDS[i];
    }

    uint256[] memory balanceOfResult = openseaSharedStorefront.balanceOfBatch(senderAddressArray, ethergrassIds);

    uint256 ownedEtherGrassCount = 0;
    for (uint256 i = 0; i < balanceOfResult.length; i++) {
      ownedEtherGrassCount += balanceOfResult[i];
    }

    return ownedEtherGrassCount;
  }

  function preSaleMintAmountAllowed(address _address) public view returns (uint256) {

    uint256 ethergrassOwnedByAddress = ethergrassOwned(_address);
    uint256 maxAllowed = ethergrassOwnedByAddress.mul(MAX_PRESALE_MINT_PER_GRASS);
    uint256 preSaleMintAmountUsed = _presaleAddressMintCount[_address];

    if (maxAllowed <= 0) {
      return 0;
    } else {
      return maxAllowed.sub(preSaleMintAmountUsed);
    }
  }
  ...
  function getUsedPremintCount(address _address) external view returns (uint256) {
    return _presaleAddressMintCount[_address];
  }
  ...
  function setEthergrassAddress(address _ethergrassAddress) external onlyOwner {
    openseaSharedStorefront = OpenSeaSharedStorefrontInterface(_ethergrassAddress);
  }
...
abstract contract ERC721Pausable is ERC721, Ownable, Pausable {
    ...
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, tokenId);
        if (_msgSender() != owner()) {
            require(!paused(), "Token transfers not allowed while contract paused");
        }
    }
}
