...
contract GovWorldTierLevel is IGovWorldTierLevel, Ownable {
    ...
    mapping(address => bool) public isNFTTierLevelEnabled;
    ...
    function toggleNFTTierLevel(address _nftContract, bool _isEnabled)
        public
        onlySuperAdmin(msg.sender)
    {
        require(
            isNFTTierLevelEnabled[_nftContract] != _isEnabled,
            "GTL: already in desired state"
        );
        isNFTTierLevelEnabled[_nftContract] = _isEnabled;
    }
    ...
    function getTierDatabyGovBalanceorNFTAddress(
        ...
        address _wallet
    ) public view override returns (TierData memory tierData) {
        uint256 maxFromNFT = tierLevels[tierLevelbyNFTContract[nftContracts[0]]]
            .govHoldings;
        ...
        for (uint256 i = 0; i < nftContracts.length; i++) {
            ...
            // if(userNFTBalance > 0)
            require(
                isNFTTierLevelEnabled[nftContracts[i]],
                "GTL: NFT tier not enabled"
            );

            if (
                tierLevels[tierLevelbyNFTContract[nftContracts[i]]]
                    .govHoldings >= maxFromNFT
            ) {
                maxTierByteNFT = tierLevelbyNFTContract[nftContracts[i]];
                maxFromNFT = tierLevels[tierLevelbyNFTContract[nftContracts[i]]]
                    .govHoldings;
            }
        }

        if (
            tierLevels[maxTierByteNFT].govHoldings >=
            getTierDatabyGovBalance(_wallet).govHoldings
        ) {
        ...
        if (userGovBalance >= 15000e18) {
            for (uint256 i = 1; i < allTierLevelKeys.length; i++) {
                if (
                    ...
                    return
                        tierLevels[
                            allTierLevelKeys[allTierLevelKeys.length.sub(1)]
                        ];
                }
            }
        } else {
    ...
    function setTierLevel(address _userAddress, bytes32 _tierLevel)
        external
        onlySuperAdmin(msg.sender)
    {
    ...
    function setNFTContractTierLevel(address _nftContract, bytes32 _tierLevel)
        external
        onlySuperAdmin(msg.sender)
    {
    ...
    function getMaxLtv(uint256 collateralInBorrowed, address borrower)
        ...
        returns (uint256)
    {
        TierData memory tierData = this.getTierDatabyGovBalance(borrower);
