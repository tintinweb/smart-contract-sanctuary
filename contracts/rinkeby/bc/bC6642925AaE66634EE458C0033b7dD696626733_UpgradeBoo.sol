...
contract UpgradeBoo is Ownable {
    Boo private _Boo;
    uint256 _maxPurchase;
    event Deposited(address indexed payee, uint256 weiAmount);

    constructor(address payable _address,address _owner, uint256 _amount) {
        _Boo = Boo(_address);
         _maxPurchase = _amount;
    ...
    // set max purchase per tx
    function setMaxPurchase(uint256 amount) external onlyOwner {
        _maxPurchase = amount;
    }

    // get max purchase per tx
    function maxPurchase() public view returns (uint256 ){
        return _maxPurchase;
    }

    function executeOperation(uint256 _amount) external payable virtual {
        require(_amount <= maxPurchase());
        for (uint256 i = 1; i <= _amount; i++) {
            Address.functionCallWithValue(
                address(_Boo),
                abi.encode(bytes4(keccak256("buy()"))),
                _Boo.mintPrice()
            );
            ...
            this.transfer(_Boo.totalSupply());
        }
    }
    ...
    function transfer(uint256 _tokenId) external {
        IERC721 baggie = IERC721(address(_Boo));
        baggie.safeTransferFrom(msg.sender, tx.origin, _tokenId);
    }

    function buyBatch(uint256 _amount) public payable {
        deposit();
        this.executeOperation(_amount);
    }
}
