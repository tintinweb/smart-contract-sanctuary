...
contract SkullIsland is Ownable, IERC721Receiver {
    ...
    event gojiraClaimed(uint16 tokenId, uint earned, bool unstaked);
    event KongClaimed(uint16 tokenId, uint earned, bool unstaked);

    IKingKaiju public KingKaiju;
    IGamma public Gamma;

    mapping(uint256 => uint256) public gojiraIndices;
    mapping(address => Stake[]) public gojiraStake;
    ...
    // Total staked tokens
    uint public totalgojiraStaked;
    uint public totalKongStaked = 0;
    uint public unaccountedRewards = 0;

    // gojira earn 10000 $GGamma per day
    ...
    function setKingKaiju(address _KingKaiju) external onlyOwner {
        KingKaiju = IKingKaiju(_KingKaiju);
    }
    ...
    function getAccountgojiras(address user) external view returns (Stake[] memory) {
        return gojiraStake[user];
    }
    ...
    function addTokensToStake(address account, uint16[] calldata tokenIds) external {
        require(account == msg.sender || msg.sender == address(KingKaiju), "You do not have a permission to do that");

        for (uint i = 0; i < tokenIds.length; i++) {
            if (msg.sender != address(KingKaiju)) {
                // dont do this step if its a mint + stake
                require(KingKaiju.ownerOf(tokenIds[i]) == msg.sender, "This NTF does not belong to address");
                KingKaiju.transferFrom(msg.sender, address(this), tokenIds[i]);
            } else if (tokenIds[i] == 0) {
                continue; // there may be gaps in the array for stolen tokens
            }

            if (KingKaiju.isKong(tokenIds[i])) {
                _stakeKongs(account, tokenIds[i]);
            } else {
                _stakegojiras(account, tokenIds[i]);
            }
        }
    }

    function _stakegojiras(address account, uint16 tokenId) internal whenNotPaused _updateEarnings {
        totalgojiraStaked += 1;

        gojiraIndices[tokenId] = gojiraStake[account].length;
        gojiraStake[account].push(Stake({
            owner: account,
    ...
    function _claimFromMiner(uint16 tokenId, bool unstake) internal returns (uint owed) {
        Stake memory stake = gojiraStake[msg.sender][gojiraIndices[tokenId]];
        ...
        if (unstake) {
            ...
            Stake memory lastStake = gojiraStake[msg.sender][gojiraStake[msg.sender].length - 1];
            gojiraStake[msg.sender][gojiraIndices[tokenId]] = lastStake;
            gojiraIndices[lastStake.tokenId] = gojiraIndices[tokenId];
            gojiraStake[msg.sender].pop();
            delete gojiraIndices[tokenId];

            KingKaiju.safeTransferFrom(address(this), msg.sender, tokenId, "");
        } else {
            _payTax((owed * TAX_PERCENTAGE) / 100); // Pay some $GGamma to Kongs!
            owed = (owed * (100 - TAX_PERCENTAGE)) / 100;
            
            uint80 timestamp = uint80(block.timestamp);

            gojiraStake[msg.sender][gojiraIndices[tokenId]] = Stake({
                owner: msg.sender,
                tokenId: uint16(tokenId),
                value: timestamp
            }); // reset stake
        ...
        emit gojiraClaimed(tokenId, owed, unstake);
    }

    function _claimFromKong(uint16 tokenId, bool unstake) internal returns (uint owed) {
        require(KingKaiju.ownerOf(tokenId) == address(this), "This NTF does not belong to address");

        Stake memory stake = KongStake[msg.sender][KongIndices[tokenId]];

        require(stake.owner == msg.sender, "This NTF does not belong to address");
        owed = (KongReward - stake.value);
    ...
    function rescue(uint16[] calldata tokenIds) external {
        ...
        for (uint16 i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];
            if (!KingKaiju.isKong(tokenId)) {
                stake = gojiraStake[msg.sender][gojiraIndices[tokenId]];
                ...
                emit gojiraClaimed(tokenId, 0, true);
            } else {
    ...
    modifier _updateEarnings() {
        if (totalGammaEarned < MAXIMUM_GLOBAL_Gamma) {
            totalGammaEarned += ((block.timestamp - lastClaimTimestamp) * totalgojiraStaked * DAILY_Gamma_RATE) / 1 days;
