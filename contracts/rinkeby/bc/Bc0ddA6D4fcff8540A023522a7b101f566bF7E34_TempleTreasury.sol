...
contract TempleTreasury is Ownable {
    ...
    // Extra config per pool, managed by treasury
    struct PoolSettings {
      uint96 sharePerHarvest;
      bool isInitialized;
    }

    // all active treasury strategies. Broadly speaking, a strategies is anything
    // treasury uses to increase intrinsic value over time.
    ...
    function numStrategies() public view returns (uint256) {
    ...
    function harvest(uint256 scalingFactor) external onlyOwner {
      ...
      uint256 reserveAsDai = DAI.balanceOf(address(this));

      for (uint256 i = 0; i < strategies.length; i++) {
        reserveAsDai += ITemplePool(strategies[i]).reval();
      }
      ...
      uint256 newHarvest = ((reserveAsDai * intrinsicValueRatio.temple / intrinsicValueRatio.dai) - totalTempleSupply) * scalingFactor / 100;
    ...
    function distributeHarvest() external onlyOwner {
      ...
      for (uint256 i = 0; i < strategies.length; i++) {
        uint256 allocatedRewards = harvestedRewards * poolSettings[strategies[i]].sharePerHarvest / totalHarvestShares;
        ...
        SafeERC20.safeTransfer(TEMPLE, strategies[i], allocatedRewards);
        emit HarvestDistributed(strategies[i], allocatedRewards);
      }
     ...
     */
    function addPool(address impl, uint256 amountDai, uint256 amountTemple, uint96 sharePerHarvest) external onlyOwner {
      PoolSettings storage settings = poolSettings[impl];

      require(!settings.isInitialized, "The given investment pool is already part of treasury");

      // Add to set of strategies
      settings.sharePerHarvest = sharePerHarvest;
      settings.isInitialized = true;
      totalHarvestShares += sharePerHarvest;
      strategies.push(impl);

      // Allocate DAI
      // TODO: Do we use allowance or just transfer
      SafeERC20.safeTransfer(DAI, impl, amountDai);

      // Mint and Allocate TEMPLE
      if (amountTemple > 0) {
        TEMPLE.mint(address(this), amountTemple);
        SafeERC20.safeIncreaseAllowance(TEMPLE, address(MINT_ALLOWANCE), amountTemple);
        MINT_ALLOWANCE.increaseMintAllowance(impl, amountTemple);
      }
    }

    /**
     * Update a given pool's share of the harvest
     */
    function updatePoolHarvestShare(address impl, uint96 sharePerHarvest) external onlyOwner {
      PoolSettings storage settings = poolSettings[impl];
      require(settings.isInitialized, "No pool with given address in treasury");

      totalHarvestShares = totalHarvestShares - settings.sharePerHarvest + sharePerHarvest;
      settings.sharePerHarvest = sharePerHarvest;
    }
     ...
     */
    function removePool(uint256 idx, address poolAddress) external onlyOwner {
      require(idx < strategies.length, "No pool at the specified index");
      require(strategies[idx] == poolAddress, "Pool at index, and passed in address don't match");

      PoolSettings storage settings = poolSettings[poolAddress];

      strategies[idx] = strategies[strategies.length-1];
      strategies.pop();
      totalHarvestShares -= settings.sharePerHarvest;
      delete poolSettings[poolAddress];

      MINT_ALLOWANCE.burnUnusedMintAllowance(poolAddress);
    }
}
...
/**
 * Interface implemented by all temple strategies.
 */
interface ITemplePool {
    /** 
