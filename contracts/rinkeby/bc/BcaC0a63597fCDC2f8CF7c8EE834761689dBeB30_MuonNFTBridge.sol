...
contract MuonNFTBridge is Ownable, IERC721Receiver {
  ...
  MuonNFTBridgeTokenFactory tokenFactory;
  IMuonV02 muon;
  ...
  constructor(address _muon) {
    ...
    tokenFactory = new MuonNFTBridgeTokenFactory(address(this));
    muon = IMuonV02(_muon);
  }
  ...
  function depositFor(
    ...
    IMuonNFTBridgeERC721 token = IMuonNFTBridgeERC721(tokens[tokenId]);
    if (mintable[tokenId]) {
  ...
  function claim(
    ...
    require(sideContracts[txParams[0]] != address(0), 'side contract not exist');
    ...
    IMuonNFTBridgeERC721 token = IMuonNFTBridgeERC721(tokens[txParams[2]]);
  ...
  function addBridgeToken(
    uint256[2] calldata tokenInfo, //[tokenId, sourceChain]
    string calldata _name,
    string calldata _symbol,
    bytes calldata _reqId,
    IMuonV02.SchnorrSign[] calldata _sigs
  ) public {
    require(tokens[tokenInfo[0]] == address(0), 'already exist');

    bytes32 hash = keccak256(
      abi.encodePacked(tokenInfo[0], tokenInfo[1], _name, _symbol, APP_ID)
    );
    require(muon.verify(_reqId, uint256(hash), _sigs), '!verified');
    ...
    address tokenContract = tokenFactory.create(
      ...
      tokenInfo[1], // sourceChain
      address(uint160(tokenInfo[0]))
    );
    tokens[tokenInfo[0]] = tokenContract;
    ids[tokenContract] = tokenInfo[0];

    mintable[tokenInfo[0]] = true;

    emit AddToken(tokenContract, tokenInfo[0], true);
  }
  ...
  function ownerSetSideContract(uint256 _network, address _addr)
    public
    onlyOwner
  {
  ...
  function emergencyWithdrawERC721Tokens(
    ...
    IMuonNFTBridgeERC721(_tokenAddr).safeTransferFrom(address(this), _to, _id);
  }
...
import "./MuonNFTBridgeToken.sol";

contract MuonNFTBridgeTokenFactory{

    address public bridge;
    
    event New(address addr, string name, string symbol, uint256 sourceChain, address sourceContract);

    constructor(address _bridge){
        bridge = _bridge;
    }

    function create(string calldata _name, string calldata _symbol, uint256 _sourceChain, address _sourceContract) external returns(address created){
        created = address(new MuonNFTBridgeToken(bridge, _name, _symbol, _sourceChain, _sourceContract));
        emit New(created, _name, _symbol, _sourceChain, _sourceContract);
    }
}
...
contract MuonNFTBridgeToken is ERC721Enumerable{
    /**
     * @dev The proxy url redirects the requests to 
     * the actual URL or the source NFT.
     * 
     * URL format: bridge-url/soureChain/SourceContract
     */
    string public _baseTokenURI = "https://nft-bridge-proxy.muon.net/";
    ...
    constructor(
        ...
        // bridge-url/soureChain/SourceContract
        _baseTokenURI = string(abi.encodePacked(_baseTokenURI, 
            sourceChain.toString(),
            "/", uint256(uint160(address(sourceContract))).toHexString(),
            "/"
        ));
    }


    function mint(address to, uint256 id) external onlyBridge{
        _mint(to, id);
    }

    function burn(uint256 id) external onlyBridge{
        _burn(id);
    }
    ...
    function sourceInfo() public view returns(
        uint256 _sourceChain,
        address _sourceContract
    ){
