...
contract Zapper is Ownable, IZapper {
    ...
    address constant public wbnb = address(0xc778417E063141139Fce010982780140Aa0cD5Ab);
    address constant public prntr = address(0x5A0fDD515a4dA3aa5705261D546cD22152d93f26);
    address constant public papr = address(0x7bf5F00AdF1a71bad1Fdbb916e4F0cC229f9c541);   

    address[] public prntrToPaprRoute = [prntr, busd, wbnb];
    address[] public paprToPrntrRoute = [papr, busd, prntr];
    ...
    function LazySeamusPAPRLazr(uint256 _amount, address sender) public {

        ROUTER.swapExactTokensForTokens(_amount, 0, prntrToPaprRoute, msg.sender, now.add(600));

    }

    function LazySeamusPRNTRLazr(uint256 _amount, address sender) public {

        ROUTER.swapExactTokensForTokens(_amount, 0, paprToPrntrRoute, msg.sender, now.add(600));
    ...
    function _swap(address _from, uint amount, address _to, address receiver) private returns (uint) {
        ...
        uint[] memory amounts = ROUTER.swapExactTokensForTokens(amount, 0, path, receiver, block.timestamp + 600);
        return amounts[amounts.length - 1];
    }
