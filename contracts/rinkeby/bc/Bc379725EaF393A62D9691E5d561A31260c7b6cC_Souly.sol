...
contract Souly is ERC721BurnableUpgradeable, AccessControlUpgradeable {
    ...
    initializer public {
        ...
        _domainSeparator = hash(EIP712DomainType("SuspendedSoul","2",1,address(this)));
        __ERC721Burnable_init();
        __ERC721_init("SuspendedSoul", "SLY");
    ...
    function setDomainSeparator(string memory name, string memory version, uint256 chainId) external onlyAdmin {
        _domainSeparator = hash(EIP712DomainType(name, version, chainId, address(this)));
    }
    ...
    function executeTransferFrom(address payable from, address to, uint256 tokenId, uint256 amount, uint256 validUntil, bytes memory signature
        ...
        require(msg.value == amount, "Souly: Invalid amount");
        ...
        from.transfer(amount.sub(platformCommission_).sub(creatorCommission_));
    }
