...
contract RepToken is ChainlinkClient {
    ...
    //StockPriceAPI public stockPriceAPI;

    constructor() public {
        owner = payable(msg.sender);
        //stockPriceAPI = _stockPriceAPI;
    ...
    function addPrediction(
        ...
        predictions[msg.sender].push(
            Prediction(msg.sender, _symbol, _date, _price)
        );
    ...
    function mint(address _predictor) private {
        totalSupply_ += 1;
        balances_[_predictor] += 1;
        emit RepTokensMinted(msg.sender, msg.value, totalSupply_);
    }

    function burn(address _predictor) private {
        totalSupply_ -= 1;
        balances_[_predictor] -= 1;
        emit RepTokensBurned(msg.sender, msg.value, totalSupply_);
    }

    function evaluatePredictions(address _predictor) public returns (uint256) {
        require(
            _predictor == msg.sender,
            "Only transaction invoker can execute this function!"
        );
    ...
    {
        string memory a = "https://api.twelvedata.com/time_series?symbol=";
        string memory b = "&exchange=XETR&start_date=";
        string memory c = "&end_date=";
        string memory d =
            "&interval=1min&apikey=d8f072b5b5314d29b71c1ff807cf4109";
        Chainlink.Request memory request =
            buildChainlinkRequest(jobId, address(this), this.eval.selector);

        request.add(
            "get",
            string(abi.encodePacked(a, _symbol, b, _date, c, _date, d))
        );
    ...
    function eval(bytes32 _requestId, bytes32 _close)
        public
