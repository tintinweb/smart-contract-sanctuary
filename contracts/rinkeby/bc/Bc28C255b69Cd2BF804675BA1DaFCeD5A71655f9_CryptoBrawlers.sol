...
contract FighterBase is FighterConfig {
    ...
    function _removeOwnedFighter(address _oldOwner, uint256 _fighterId)
        internal
    {
        uint256 index;
...
contract Marketplace is MarketplaceConfig {
    ...
    function _strike(
        ...
        uint256 _attackerAttackRoll =
            RandomNumber.randomNumber1ToMax(_seed + _attackerId, 20) +
                _attackerStrength;
        uint256 _defenderDefenseRoll =
            RandomNumber.randomNumber1ToMax(_seed + _defenderId, 20) +
                _defenderStrength;
    ...
    function _performFight(
        ...
        bool _attackerIsStrikingFirst = _attackerSpeedRoll > _defenderSpeedRoll;
    ...
    MarketplaceConfig
{
    ...
    function safeTransferFrom(
        ...
        if (_isContract(_to)) {
            (bool _success, bytes memory _erc721Check) =
                _to.call(
    ...
    function getFightersForAddress(address _owner)
        external
