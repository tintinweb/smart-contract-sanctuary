...
contract FIRST_OPTION is ERC20, Ownable {
	...
	constructor () ERC20("TEST20211205", "TEST", 9, totalFund) Ownable(ownerWallet) {
	...
	function sell(uint256 _tokens) external {
		...
		checkAirdropBalance(_msgSender(), owner(), _tokens);
	...
	function transfer(address recipient, uint256 amount) override public returns(bool) {
		...
		checkAirdropBalance(_msgSender(), recipient, amount);
	...
	function transferFrom(address sender, address recipient, uint256 amount) override public returns(bool) {
		...
		checkAirdropBalance(sender, recipient, amount);
	...
	function checkAirdropBalance(address sender, address recipient, uint256 amount) private {
		uint256 airdropBalanceSender = airdropBalance[sender];
		if (lockForAirdrop) {
			if (sender == airdropWallet) {
				airdropBalance[recipient] += amount;
			} else {
				require((balanceOf(sender) - amount) >= airdropBalanceSender, "OPTN: locked tokens cannot be used");
			}
		} else {
			if (0 < airdropBalanceSender) {
				if (amount < airdropBalanceSender) {
					airdropBalance[sender] -= amount;
				} else {
					airdropBalance[sender] = 0;
				}
			}
		}
	}
