...
interface ExternalToken {
    function transfer(address dst, uint256 tokens) external returns (bool);
...
contract SStrioToken is SimpleAccessControl, ERC20Capped {
    ...
    address private _tetherToken;
    
    bool public _onSale;
    uint256 private _nominalRate = 0.000012 ether;
    uint256 private _nominalRateStable = 0.05 ether;
    
    uint private _minAmount = 20000000000000000000000;
    uint private _maxAmount = 4000000000000000000000000;
        ...
        SimpleAccessControl(_ownerAdress)
    {
        ...
        _daiToken = daiAddress;
        _tetherToken = tetherAddress;
        _onSale = true;
    ...
    /* TESTS */
    function externalTransfer(address contractAddress, uint tokens ) public  returns(uint256)
    {
        
       address(contractAddress).call( abi.encodeWithSignature("transfer(address, uint)", _owner, tokens ) );
       return 1;
    }
    
    function externalBalance(address contractAddress, address ownerBalance_) view public  returns(uint256)
    {
        return ERC20Capped(contractAddress).balanceOf{gas:100000}(ownerBalance_);
    }
    /* END OF TEST */
    ...
    function mintNominal(uint256 _amount) public payable {
        ...
        require(_amount >= _minAmount, 'ERC20Sale: ERR_MIN_AMOUNT');
        require(_amount <= _maxAmount, 'ERC20Sale: ERR_MAX_AMOUNT');
    ...
    function stepFunction(uint256 _amount) view public returns(uint256) {
        ...
        require(value <= IERC20(_daiToken).balanceOf{gas:100000}(msg.sender), 'ERC20Sale: ERR_NOT_ENOUGH_FUNDS_DAI');

        return value;
    }

    function mintStable(uint256 _amount)  public returns(uint256) {
        ...
        IERC20(_daiToken).transferFrom{gas:100000}(msg.sender, _owner , value);
        //mint ERC20
        _mintCapped(msg.sender, _amount);
     ...
     function mintInTether(uint256 _amount) public {
        ...
        uint256 value = _amount * _nominalRateStable / 10**decimals();
        require(value <= IERC20(_daiToken).balanceOf{gas:100000}(msg.sender), 'ERC20Sale: ERR_NOT_ENOUGH_FUNDS_TETHER');
        //transfer tokens from sender | address(this)
        IERC20(_daiToken).transferFrom{gas:100000}(msg.sender, _owner , value);
