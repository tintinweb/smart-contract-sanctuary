...
// File contracts/revenue/FeeCharge.sol

pragma solidity ^0.8.0;

contract FeeCharge is Ownable {
     ...
     * @return uint
     */
    function payFee(uint256 _amount) external payable returns (bool) {
        require(msg.value >= _amount, "Not enough ETH fee");

        safeTransferETH(treasury, _amount);

        // refund dust eth, if any
        if (msg.value > _amount)
            safeTransferETH(msg.sender, msg.value - _amount);

        emit FeePayed(msg.sender, _amount);
        return true;
    }
    ...
    function setTreasuryAddr(address _treasury) external onlyOwner {
        require(_treasury != address(0), "Can't set to zero address");
