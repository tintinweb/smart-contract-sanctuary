...
interface IArrLandNFT {
    ...
    function sendGiveAway(address _to, uint256 _tokenCount, uint256 _generation) external returns (uint256);
...
contract ArrlandGiveAway is Ownable{
    ...
    event giveawayClaimed(address to, uint256 tokenId);
    event teamMint(address to, uint256 tokenId);
    ...
    function claimGiveAway() public {
        ...
        for (uint i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = IArrLandNFT(arrlandNFT).spawn_pirate(msg.sender, 0, 1);
            emit giveawayClaimed(msg.sender, tokenId);
        }
    }
