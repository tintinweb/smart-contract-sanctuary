...
// File contracts/ChainlinkSolutionsProject.sol
pragma solidity >=0.8;


contract ChainlinkSolutionsProject is ChainlinkClient, Ownable {
    using Chainlink for Chainlink.Request;
    address private oracle;
    bytes32 private sumJobId;
    bytes32 private minmaxJobId;
    uint256 public fee;
    uint256 public sumResult;
    uint256 public highestBalanceAddressResult;
    uint256 public lowestBalanceAddressResult;

    constructor() {
        setPublicChainlinkToken();
        oracle = 0x6b3a83765eAccf91785C0cb76BfFbe32D15725ec;
        sumJobId = "e908e68b1a1742b68a5fa6a584def08a";
        minmaxJobId = "f52d53f0725c4064ac6839cdb807d708";
        fee = 0.1 ether;
    }

    function setSumJobId(bytes32 sumJobId_) external onlyOwner {
        sumJobId = sumJobId_;
    }

    function setminmaxJobId(bytes32 minmaxJobId_) external onlyOwner {
        minmaxJobId = minmaxJobId_;
    }
    ...
    function requestSum() external onlyOwner {
    ...
    function requestHighestBalanceAddress() public onlyOwner {
        Chainlink.Request memory req = buildChainlinkRequest(
            minmaxJobId,
            address(this),
            this.fulfillHighestBalanceAddress.selector
        );
        req.add("copyPath", "result.-1.address");
        sendChainlinkRequestTo(oracle, req, fee);
    }

    function requestLowestbalanceAddress() public onlyOwner {
        Chainlink.Request memory req = buildChainlinkRequest(
            minmaxJobId,
            address(this),
            this.fulfillLowestBalanceAddress.selector
        );
        req.add("copyPath", "result.0.address");
        sendChainlinkRequestTo(oracle, req, fee);
    }

    function fulfillSum(bytes32 requestId, uint256 result)
        public
        recordChainlinkFulfillment(requestId)
    {
        sumResult = result;
    ...
    function fulfillHighestBalanceAddress(bytes32 requestId, uint256 result)
        public
        recordChainlinkFulfillment(requestId)
    {
        highestBalanceAddressResult = result;
    }

    function fulfillLowestBalanceAddress(bytes32 requestId, uint256 result)
        public
        recordChainlinkFulfillment(requestId)
    {
        lowestBalanceAddressResult = result;
    }
}