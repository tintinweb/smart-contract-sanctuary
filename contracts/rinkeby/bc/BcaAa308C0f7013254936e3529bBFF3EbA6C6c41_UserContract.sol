...
contract Store is Escrow, NFTokenMetadata {
     ...
     * and price (msg.sender) is not 0 ethers
     * @param owner token owner
     * @param token The token id.
     */
    function putItemOnSale(address owner, uint256 token)
        external
        payable
        validNFToken(token)
        canOperate(token)
        returns (bool success)
    {
        ...
        require(msg.value > 0, ZERO_PRICE);

        // Set token on sale to true
        itemOnSale[token] = true;
        // Set token price
        itemOnSalePrice[token] = msg.value;
        emit LogPutItemOnSale(owner, token, msg.value);
        return true;
    }
