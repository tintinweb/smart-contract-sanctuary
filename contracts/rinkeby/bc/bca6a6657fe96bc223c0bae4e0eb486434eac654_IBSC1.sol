...
// File: contracts/interface/IBSC1Factory.sol

pragma solidity ^0.5.0;

interface IBSC1Factory {
    function getBSC1(uint256 tokenId)
        external view
        returns (
            uint256 grade,
            uint256 quality,
...
// File: contracts/interface/IBSC1Token.sol

pragma solidity ^0.5.0;



contract IBSC1Token is IERC721 {
...
contract IBSC1 is IPool {
    ...
    IBSC1Factory public _BSC1Factory = IBSC1Factory(0x0);
    IBSC1Token public _BSC1Token = IBSC1Token(0x0);

    address public _teamWallet = 0x997Cd5A9e45030A7D5fE2D886c4B396B7EE467dD;
    address public _rewardPool = 0x997Cd5A9e45030A7D5fE2D886c4B396B7EE467dD;
    ...
    mapping(address => uint256[]) public _playerBSC1;
    mapping(uint256 => uint256) public _BSC1MapIndex;

    address public _governance;

    event NotifyReward(uint256 reward);
    event StakedBSC1(address indexed user, uint256 amount);
    event WithdrawnBSC1(address indexed user, uint256 amount);
    ...
    function initialize(
        ...
        _BSC1Token = IBSC1Token(BSC1Token);
        _BSC1Factory = IBSC1Factory(BSC1Factory);
    ...
    function seizeErc721(IERC721 token, uint256 tokenId) external {
        require(token != _BSC1Token, "reward");
    ...
    6	stakingpowa	1.6+ 1.8*(stakingpowa-9980)/100
    */

    function getFixRate(uint256 class,uint256 stakingpowa) public pure returns (uint256){

        require(class > 0 && class <7, "the bsc1 not token");
        ...
        if( class == 1 ){
           ...
           return unfold.add(130000);
            ...
            unfold = stakingpowa.sub(9000)*20000/1000;
           return unfold.add(140000);
        }else if( class == 5){
            unfold = stakingpowa.sub(9800)*20000/500;
            return unfold.add(160000);
        }else{
            unfold = stakingpowa.sub(9980)*20000/100;
            return unfold.add(180000);
        }
    }

    function getStakeInfo( uint256 BSC1Id ) public view returns ( uint256 stakeRate, uint256 amount){

        uint256 class;
        uint256 stakingpowa;
        (class, stakingpowa, amount, , , , ) = _BSC1Factory.getBSC1(BSC1Id);
        require(amount > 0,"the BSC1 not token");
        stakeRate = getFixRate(class,stakingpowa);
    }

    // stake BSC1
    function stakeBSC1(uint256 BSC1Id)
        public
        updateReward(msg.sender)
    {
        require(block.timestamp >= _startTime, "not start");

        uint256[] storage BSC1Ids = _playerBSC1[msg.sender];
        if (BSC1Ids.length == 0) {
            BSC1Ids.push(0);
            _BSC1MapIndex[0] = 0;
        }
        BSC1Ids.push(BSC1Id);
        _BSC1MapIndex[BSC1Id] = BSC1Ids.length - 1;

        uint256 stakeRate;
        uint256 ercAmount;
        (stakeRate, ercAmount) = getStakeInfo(BSC1Id);
        ...
        _BSC1Token.safeTransferFrom(msg.sender, address(this), BSC1Id);

        _lastStakedTime[msg.sender] = now;

        emit StakedBSC1(msg.sender, BSC1Id);
    ...
    function withdrawBSC1(uint256 BSC1Id)
        public
        updateReward(msg.sender)
    {
        require(BSC1Id > 0, "the BSC1Id error");

        uint256[] memory BSC1Ids = _playerBSC1[msg.sender];
        uint256 BSC1Index = _BSC1MapIndex[BSC1Id];

        require(BSC1Ids[BSC1Index] == BSC1Id, "not BSC1Id owner");

        uint256 BSC1ArrayLength = BSC1Ids.length-1;
        uint256 tailId = BSC1Ids[BSC1ArrayLength];

        _playerBSC1[msg.sender][BSC1Index] = tailId;
        _playerBSC1[msg.sender][BSC1ArrayLength] = 0;
        _playerBSC1[msg.sender].length--;
        _BSC1MapIndex[tailId] = BSC1Index;
        _BSC1MapIndex[BSC1Id] = 0;
        ...
        _BSC1Token.safeTransferFrom(address(this), msg.sender, BSC1Id);

        emit WithdrawnBSC1(msg.sender, BSC1Id);
    }

    function withdraw()
        public
    {
        uint256[] memory BSC1Id = _playerBSC1[msg.sender];
        for (uint8 index = 1; index < BSC1Id.length; index++) {
            if (BSC1Id[index] > 0) {
                withdrawBSC1(BSC1Id[index]);
            }
        }
    }

    function getPlayerIds( address account ) public view returns( uint256[] memory BSC1Id )
    {
        BSC1Id = _playerBSC1[account];
    }
    ...
    function setGovernBSC1ce(address governance)  public  onlyGovernance
    {
