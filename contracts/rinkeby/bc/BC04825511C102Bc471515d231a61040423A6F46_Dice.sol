...
contract Dice is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function _buildJson(uint256 _tokenId) internal view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Dice #', toString(_tokenId), '", "description": "Dice, a pseudo randomized onchain dice that anyone can roll.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function claim(uint256 _tokenId) public nonReentrant {
        require(_tokenId > 0 && _tokenId < 91, "Token ID invalid");
        _safeMint(_msgSender(), _tokenId);
    }
    
    function ownerClaim(uint256 _tokenId) public nonReentrant onlyOwner {
        require(_tokenId > 90 && _tokenId < 101, "Token ID invalid");
