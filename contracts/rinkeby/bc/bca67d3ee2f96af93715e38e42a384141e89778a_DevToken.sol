 ...
 */
contract DevToken is Ownable, ERC721Pausable {
    using SafeMath for uint256;
    ...
    mapping(address => uint) public ImgPerAddress;
    mapping(uint => address) public AddressPerId;
    ...
    constructor(string memory baseURI) ERC721("SuperTeenageHero", "STH") {  
        _setBaseURI(baseURI);
    }
    ...
    */
    function mintPack(string[] memory tokenURI, uint256[] memory mintedImg) public payable saleIsOpen returns (uint256) {
        require((ImgPerAddress[msg.sender] + mintedImg.length) <= MAX_BY_MINT, "Exceeds number");
        // require(msg.value <= price(mintedImg.length), "Value below price");
        ...
        for (uint256 i = 0; i < tokenURI.length; i++) {
            ...
            ImgPerAddress[msg.sender] += 1;
        }
        // emit MintPack(msg.sender, newItemId);
    
    }
    
    function mintPackIndividual(string memory tokenURI, uint256 imgNum) public payable onlyOwner returns (uint256) {
        require(!ImgStatus[imgNum], "This number is existed");
            
        // to get id to mint
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        
        // mint an nft and set tokenUri
        _safeMint(msg.sender, newItemId);
        _setTokenURI(newItemId, tokenURI);
        
        // add PACK id to map
        
        ImgStatus[imgNum] = true;
        emit MintPack(msg.sender, newItemId);
        return newItemId;
    }
    
    function changeData(uint256 requestId, string memory tokenURI) public onlyOwner {
        _setTokenURI(requestId, tokenURI);
        emit MintPack(msg.sender, requestId);
    }
    ...
    function _mintAnElement(address _to, string memory tokenURI) private {
        ...
        _setTokenURI(newItemId, tokenURI);
        AddressPerId[newItemId] = msg.sender;
        emit MintPack(msg.sender, newItemId);
    }
    
    function burnIndividual(uint256 requestId) public {
        require(ownerOf(requestId) == msg.sender, "You are not owner");
        uint256 balance = address(this).balance;
        _burn(msg.sender, requestId);
        ImgPerAddress[msg.sender] -= 1;
        AddressPerId[requestId] = address(this);
        _widthdraw(msg.sender, balance.div(1000));
    }
    
    function getIdPerAddress(address _address) public view returns(uint256[] memory) {
        uint256 currentSupply = _tokenIds.current();
        uint256[] memory idList;
        uint256 key = 0;
        for(uint256 i=0; i< currentSupply; i++) {
            if(AddressPerId[i] == _address) {
                idList[key] = i;
                key++;
            }
        }
