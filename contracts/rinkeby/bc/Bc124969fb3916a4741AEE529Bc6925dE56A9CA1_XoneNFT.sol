...
contract XoneNFT is ERC721Enumerable, Ownable, ERC721Burnable, ERC721URIStorage {
  ...
  uint256 public Max_Airdrop_Perc_Every_Plan = 3;
  ...
  constructor() ERC721('XONE NFT', 'XONE') {
    createPlan('Jan-2022', false, false, 2500, 0.1 ether, 0.08 ether, 2, 154, 3);
  }
  ...
  function createPlan(
    ...
    require((_team_amount.mul(Denominator)).div(_max_amount)<= Max_Team_Perc_Every_Plan,'XONE: Limit max team amount error');
    require((_airdrop_amount.mul(Denominator)).div(_max_amount)<= Max_Airdrop_Perc_Every_Plan,'XONE: Limit max airdrop amount error');
  ...
  function mint(
    ...
    require(_amount == _tokenUrls.length, 'XONE: _amount must match length of _tokenUrls');
    ...
    for (uint256 i = 0; i < _amount; i++) {
      ids[i] = _mintAnElement(_to, _tokenUrls[i]);
    }
    return ids;
  }
  ...
  function _mintAnElement(address _to, string memory _tokenUrl) private returns (uint256) {
    ...
    _safeMint(_to, id);
    _setTokenURI(id, _tokenUrl);
    emit Minted(_to, id);
    return id;
  }

  function _mintForBulk(address _to, string memory _tokenUrl) private {
    uint256 id = _totalSupply();
    _tokenIdTracker.increment();
    _safeMint(_to, id);
    _setTokenURI(id, _tokenUrl);
  ...
  function mintTeam(
    ...
    for (uint256 i = 0; i < _amount; i++) {
      _mintForBulk(_teamWallet, _tokenUrls[i]);
    }
  }

  function mintWallet1(uint256 _amount, string[] memory _tokenUrls) external onlyOwner {
    require(_amount > 0);
    require(_amount == _tokenUrls.length, 'XONE: _amount must match length of _tokenUrls');
    ...
    for (uint256 i = 0; i < _amount; i++) {
      _mintForBulk(_wallet1, _tokenUrls[i]);
    }
  }

  function mintWallet2(uint256 _amount, string[] memory _tokenUrls) external onlyOwner {
    require(_amount > 0);
    require(_amount == _tokenUrls.length, 'XONE: _amount must match length of _tokenUrls');
    ...
    for (uint256 i = 0; i < _amount; i++) {
      _mintForBulk(_wallet2, _tokenUrls[i]);
    }
  }
  ...
  function airdrop(
    ...
    require(plans[_plan].state == true, 'XONE: Plan is not existed');
    require(_amount > 0);
    require(_tokenUrls.length == _amount);
    require(_tokenUrls.length == _addresses.length);
    ...
    for (uint256 i = 0; i < _amount; i++) {
      _mintForBulk(_addresses[i], _tokenUrls[i]);
    }
  }
}
