...
contract StablePool {
    ...
    uint public totalHighAP; // sum of high weights
    uint public totalLowAP; // sum of low weights
    ...
    function collectFees() external {
        ...
        shareToken.mint( address(shareToken), feesCollected ); // mint pool token
    ...
    function check( bool _add, address _token, uint _amount ) public view returns ( bool ) {
        ...
        if( _add ) { // use upper bound
            ...
            uint maximum = totalTokens.mul( tokenInfo[ _token ].highAP ).div( totalHighAP ); // maximum balance of token
            ...
            uint minimum = totalTokens.mul( tokenInfo[ _token ].lowAP ).div( totalLowAP ); // minimum balance of token
            uint balance = IERC20( _token ).balanceOf( address(this) ); // current balance of token
