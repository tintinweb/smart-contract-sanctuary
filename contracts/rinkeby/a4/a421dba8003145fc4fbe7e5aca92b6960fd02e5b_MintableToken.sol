...
contract MintableToken is Ownable, IERC721, IERC721Metadata, ERC721Burnable, ERC721Base {
    ...
    constructor (
        ...
        admin = _admin; // Account which will get approved while requesting redeem and burn
    }


    function mint(string memory tokenURI, uint _royality, bool _redeemable, bool _burnable) public {
        _mint(msg.sender, this.totalSupply(), _royality);
        verified[this.totalSupply()-1] = false; 
        redeemable[this.totalSupply()-1] = _redeemable;
        burnable[this.totalSupply()-1] = _burnable;
        _setTokenURI(this.totalSupply()-1, tokenURI);
    }
    
    function verify(uint _tokenID) public {
        require(msg.sender==admin, 'Only Admin!');
        verified[_tokenID] = true; 
    }
