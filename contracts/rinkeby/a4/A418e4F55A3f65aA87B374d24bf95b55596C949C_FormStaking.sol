...
contract FormStaking {

    // base APR
    uint256 private BASE_APR = 40;
    ...
    function _calculateYieldTotal(address user) private view returns(uint256) {
        uint256 stakingTime = _calculateYieldTime(user);
        uint APR = getAPRValue();
        uint256 yieldRate = (APR * ONE_ETH) / ONE_YEAR;
        uint256 earnedYield = (stakingTime * yieldRate) / 100;
        uint256 netYield = stakingBalance[user] * earnedYield;
        uint256 netYieldFormatted = netYield / ONE_ETH;
        return netYieldFormatted;
    }
    function getUsersYieldAmount(address user) public view returns(uint256) {
        require(
            stakingBalance[user] > 0,
            "You do not stake any tokens");
        uint256 yieldEarned = _calculateYieldTotal(user);
    ...
    function stake(uint256 amount) public {
        ...
        if(stakingBalance[msg.sender] > 0){
            uint256 yieldEarned = _calculateYieldTotal(msg.sender);
