 ...
 * word-nft Contract
 */
contract WordNFT is ERC721, ReentrancyGuard, Ownable
{
	struct Word
	{
		string word;
	...
	mapping(uint => Word) private wordsMapping;
	mapping(string => bool) private wordExistsMapping;

	uint private end_date;
	uint public max_words_per_transaction = 10;
	uint public word_price = 100000000000000000;
	uint public total_words = 0;
	uint public max_word_supply = 8888;
	bool public max_word_supply_overriden = false;


	/**
	 * @dev Set up the contract and randomly set the cut off time for season one.
	 */
	constructor() ERC721("word-nft", "NAME") Ownable()
	{
	...
	/**
	 * @dev Public payable method to process a single word.
	 *
	 * @param _word string memory
	 */
	function mintASingleWord(string memory _word) public payable nonReentrant
	{
		require(validateContract());
		require(msg.value >= word_price, "Not enough ether.");
		require(validateWord(_word));
	...
	/**
	 * @dev Public payable method to process an array words.
	 *
	 * @param _words string[] memory
	 */
	function mintMultipleWords(string[] memory _words) public payable nonReentrant
	{
		require(validateContract());

		uint num_words = _words.length;

		require(msg.value >= (word_price * num_words), "Not enough ether.");
		require((num_words > 0) && (num_words <= max_words_per_transaction), "Too many words.");

		// Loop through each word to make sure each of them are valid.
		for (uint i=0; i<num_words; i++)
		{
			require(validateWord(_words[i]));
		}

		// All good to go - now mint all the words.
		for (uint i=0; i<num_words; i++)
		{
			mintWordToken(_words[i]);
		}
	}
	...
	function validateContract() internal returns(bool)
	{
		...
		if ((block.timestamp > end_date) && (!max_word_supply_overriden))
		{
			uint new_max_word_supply = total_words + 50;
			if (new_max_word_supply < max_word_supply) max_word_supply = new_max_word_supply;
			max_word_supply_overriden = true;
		}

		require((total_words < max_word_supply), "Season one has ended.");
	...
	/**
	 * @dev Check the word to make sure it's valid.
	 *
	 * @param _word string memory
	 * @return bool
	 */
	function validateWord(string memory _word) internal view returns(bool)
	{
		require(wordExistsMapping[_word] != true, "word exists.");
		require(onlyValidCharacters(_word), "Lowercase letters and numbers only.");
		require(((bytes(_word).length >= 2) && (bytes(_word).length <= 10)), "Must be between 2 and 10 characters.");
	...
	/**
	 * @dev Create a new token with the given word and store the .
	 *
	 * @param _word string memory
	 */
	function mintWordToken(string memory _word) internal
	{
		uint new_token_id = getTokenIdfromString(_word);
		...
		if (total_words == 0) comment = "first.";

		_safeMint(_msgSender(), new_token_id);

		wordsMapping[new_token_id] = Word(_word, bg_color, text_color, comment, true);
		wordExistsMapping[_word] = true;
		total_words++;
	}
	...
	{
		require(wordsMapping[_token_id].exists == true, "Invalid ID.");

		Word memory word = wordsMapping[_token_id];

		string memory svg = buildSVGFromToken(word);

		string memory comment_output = "it's just a name.";
		string memory comment_trait = "none.";

		if (keccak256(abi.encodePacked(word.comment)) == keccak256(abi.encodePacked("")))
		{
			comment_output = word.comment;
			comment_trait = word.comment;
		}
		...
		string memory json = string(
			abi.encodePacked(
				...
				'"attributes": [{"trait_type": "Background Color", "value": "', word.bg_color, '"},{"trait_type": "Text Color", "value": "', word.text_color, '"},{"trait_type": "Comment", "value": "', comment_trait, '"}],',
				'"background_color": "', word.bg_color, '"}'
			)
		);
	...
	/**
	 * @dev Build the SVG using the data from the word token.
	 *
	 * @param word word memory
	 * @return string memory
	 */
	function buildSVGFromToken(Word memory word) internal pure returns(string memory)
	{
		string memory glitch = "";
		bytes32 filter = "";

		if (keccak256(abi.encodePacked(word.text_color)) == keccak256(abi.encodePacked("glitch")))
		{
	...
	/**
	 * @dev Public method to allow users to look up their word before attempting to mint.
	 *
	 * @param _word string memory
	 * @return string memory
	 */
	function iswordAvailable(string memory _word) public view returns(string memory)
	{
		require(validateWord(_word));
		return string(abi.encodePacked("'", _word, "' is still available."));
	}
