...
contract Farm is Ownable, Authorizable, ReentrancyGuard {
    ...
    ERC20 private _token =
        ERC20(address(0x26a77595Aa80350af52A14116E197E53b8B92601));
    ...
    event TokenAddressChanged(address indexed _token);
    ...
    function setTokenAddress(ERC20 token_) external onlyOwner {
        _token = token_;
        emit TokenAddressChanged(address(_token));
    }
    ...
    function giveAway(address _address, uint256 points) external onlyOwner {
    ...
    function rewardedPoints(address staker) public view returns (uint256) {
        ...
        uint256 _seconds = block.timestamp - stakerData.timestamp;
        // uint256 rewarded = stakerData.points + (_seconds * fixedConstantPerToken);

        uint256 fixedConstantPerToken = 100 * 1e18;

        uint256 earnPerSec = (fixedConstantPerToken / (60 * 60 * 24));
        uint256 rewarded = ((stakerData.amount * earnPerSec) * _seconds) / 1e18;

        return rewarded;
    }

    function getStakerWithStakeDuration(address staker)
        external
        view
        returns (Staker memory, uint256)
    {
        Staker memory stakerData = stakers[staker];

        if (stakerData.amount < 1000) {
            revert("Not enough amount provided.");
        }

        // solium-disable-next-line security/no-block-members
        uint256 blockTime = block.timestamp;
        uint256 stakeTime = (blockTime - stakerData.timestamp);

        require(
            stakerData.timestamp < blockTime,
            "Staker timestamp should less than block.timestamp"
        );

        return (stakerData, stakeTime);
    }
    ...
    function deposit(uint256 amount) public nonReentrant() {
        ...
        consolidate(sender);
        total = total + amount;
        stakerData.amount = stakerData.amount + amount;

        // solium-disable-next-line security/no-block-members
        stakerData.timestamp = block.timestamp;

        stakers[sender] = stakerData;
...
contract Authorizable is AccessControl {
    // DEPRECATED modifier onlyAuthorized
