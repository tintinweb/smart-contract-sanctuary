...
contract DopAirdrop is IAirdrop, Ownable {
    ...
    function isClaimable(address account) public view override returns (bool) {
        // TODO need to check if msg.sender supplied collateral to any erc20 or nft lending pools.
        for (uint256 idx = 0; idx < comptrollers.length; idx++) {
            IERC20[] memory cTokens = IComptroller(comptrollers[idx]).getAllMarkets();
            for (uint256 cIdx = 0; cIdx < cTokens.length; cIdx++) {
                IERC20 cToken = cTokens[cIdx];
                if (cToken.balanceOf(account) > 0) return true;
            }
        }
        return false;
    }

    function addComptroller(address comptroller) public onlyOwner {
    ...
    function claim(uint256 index, uint256 amount, bytes32[] calldata merkleProof) external override {
        require(!isClaimed(index), 'DopAirdrop: Drop already claimed.');
        ...
        require(isClaimable(account), 'DopAirdrop: users needs to supply collateral first');

        // Mark it claimed and send the token.
        _setClaimed(index);
        require(IERC20(token).transfer(account, amount), 'DopAirdrop: Transfer failed.');
...
interface IComptroller {
    ...
    function getAllMarkets() external view returns (IERC20[] memory);
}