...
contract RawLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  // Random limit
  uint256 private random_limit = 10000;
  ...
  function publicMint(uint256 tokenId) external payable nonReentrant {
    ...
    require(tokenId >= 0 && tokenId < 50000, "Token Id must between 0 and 50000");
  ...
  function mintWithLoot(uint256 lootId, uint256 tokenId) external nonReentrant {
    require(lootContract.ownerOf(lootId) == msg.sender || temporalLootContract.ownerOf(tokenId) == msg.sender, "You must own Loot or TemporalLoot");

    require(tokenId >= 50000 && tokenId < block.number.div(100), "Token Id must between 50000 and the latest block divided by 10");
