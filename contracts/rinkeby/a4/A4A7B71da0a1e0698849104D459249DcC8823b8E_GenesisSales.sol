 ...
 */
contract GenesisSales is Ownable {
    ...
    constructor(address _usdt, address[] memory _ids) {
        usdt = _usdt;

        require(_ids.length == 3, "Genesis: erc1155 addresses are not setup");
        tokenCommon = _ids[0];
        tokenRare = _ids[1];
        tokenGold = _ids[2];
    ...
    function buyTokens(
        ...
        require(_paymentToken == usdt, "Genesis: unsuportted token");
        require(
            idExists[_tokenId] == tokenCommon ||
                idExists[_tokenId] == tokenRare ||
                idExists[_tokenId] == tokenGold,
            "Genesis: this tokenId has not supported"
        );
        ...
        if (idExists[_tokenId] == tokenCommon) {
            ...
            require(
                ...
                "Genesis: not enough Common tokens in contract"
            );
        ...
        if (idExists[_tokenId] == tokenRare) {
            ...
            require(
                ...
                "Genesis: not enough Rare tokens in contract"
            );
        ...
        require(priceInToken > 0, "Genesis: insufficient amount rarity");
    ...
    function setURI(string memory _newUri, uint256 _tokenId) public onlyOwner {
        ...
        IERC1155(idExists[_tokenId])._setURI(_newUri);
    }
}
...
interface IERC1155 {
     ...
     */
    function _setURI(string memory newuri) external;
}
