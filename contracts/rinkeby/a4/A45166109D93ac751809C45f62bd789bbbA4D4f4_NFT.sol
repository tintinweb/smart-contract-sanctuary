...
contract NFT is ERC721URIStorage, Ownable {
  ...
  uint256 public constant NUM_RESERVED = 200;
  uint256 public constant MAX_NUM_MINT = 20;
  uint public count_reserved;

  uint public fee = 60000000000000000 wei; // 0.06 ETH
  bool public saleIsActive = false;
  string public baseUri = "ipfs://placeholder/";
  address PSA = 0x0000000000000000000000000000000000000000;
  constructor() ERC721("TestNFT1", "TNFT1") {
  }
  
  function mintVariableAmountOfTokens(address to, uint amount) public payable {
    require(saleIsActive, "Tokensale is not active right now.");
    require(amount <= MAX_NUM_MINT, "Maximum number of mints per function call exceeded.");
    require(msg.value >= fee * amount, "Price was not paid in full.");
  ...
  function mintReservedByOwner(address to, uint amount) public onlyOwner {
  ...
  function UintToString(uint256 value) internal pure returns (string memory) {
    // helps to create the right tokenURI
  ...
  function changePSA(address newPSA) public onlyOwner {
    PSA = newPSA;
  }
