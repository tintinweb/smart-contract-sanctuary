...
contract LootTunes is ERC721Enumerable, ReentrancyGuard, Ownable {
    using SafeMath for uint256;

    string public PROVENANCE = "";
    uint256 public publicPrice = 0.03 ether;
    string public baseURI = "https://loottunes.fun/api/token/";

    //Loot Contract
    address public lootAddress = 0xFF9C1b15B16263C61d017ee9F65C50e4AE0113D7;
    LootInterface lootContract = LootInterface(lootAddress);

    constructor() ERC721("Tunes (for Adventurers)", "LOOTTUNE") {
    }
    ...
    function mintWithLoot(uint lootId) public payable nonReentrant {
        require(lootId > 0 && lootId <= 8000, "Not a valid loot ID. Did you mean to use the regular mint function?");
        require(lootContract.ownerOf(lootId) == msg.sender, "Not this loot's owner");
    ...
    function multiMintWithLoot(uint256[] memory lootIds) public payable nonReentrant {
        for (uint256 i = 0; i < lootIds.length; i++) {
            require(lootContract.ownerOf(lootIds[i]) == msg.sender, "Not this loot's owner");
    ...
    function mint(uint lootId) public payable nonReentrant {
        ...
        require(lootId > 8000 && lootId <= 12000, "Token ID invalid (only IDs 8000 to 12000 are mintable)");
        _safeMint(msg.sender, lootId);
    }
    ...
    function multiMint(uint256[] memory lootIds) public payable nonReentrant {
        ...
        for (uint256 i = 0; i < lootIds.length; i++) {
            require(lootIds[i] > 8000 && lootIds[i] < 12000, "Token ID invalid (only IDs 8000 to 12000 are mintable)");
