...
contract BlxxtMF is ERC721Enumerable, Ownable {
    ...
    uint256 private constant MINT_PER_BLOOT = 3;
    ...
    constructor(string memory baseURI) ERC721("BlxxtMF", "BLXXTMF")  {
        setBaseURI(baseURI);
    }
    ...
    function mint() public payable {
        ...
        //require(bloot.balanceOf(msg.sender) * MINT_PER_BLOOT >= balanceOf(msg.sender), "You can mint 3 BlootMF per held Bloot");
        
        if (msg.value >= FOUNDER_DONATION) {
            require(founderList.length <= MAX_FOUNDER, "No more Founder slots available");
            founderMap[msg.sender] = true;
            founderList.push(msg.sender);
        } else if (msg.value >= DIAMOND_DONATION) {
            require(diamondList.length <= MAX_DIAMOND, "No more diamond slots available");
            diamondMap[msg.sender] = true;
            diamondList.push(msg.sender);
        } else {
            require(freeCounter + 1 <= MAX_FREE, "No more free mints available.");
            freeCounter++;
        }
