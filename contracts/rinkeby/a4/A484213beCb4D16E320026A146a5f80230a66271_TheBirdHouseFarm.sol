...
*/
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(stakerToTokenIds[msg.sender].length + tokenIds.length < MAX_WALLET_STAKED, "Must have less than 30 birds staked!");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                TheBirdHouseContract.ownerOf(tokenIds[i]) == msg.sender && tokenIdToStaker[tokenIds[i]] == nullAddress,
