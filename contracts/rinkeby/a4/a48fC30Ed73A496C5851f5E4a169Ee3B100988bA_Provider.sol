...
contract LockFactory is AccessRestriction {
    ...
    function getLockAddress(bytes12 _salt) public returns (address) {
        address lockAddress = unlock.publicLockAddress();
        ...
        return lockAddress.createClone2(salt);
    }

    function createNewLock(bytes32 key) internal {
        bytes12 salt = bytes12(keccak256(abi.encodePacked(key)));
        unlock.createLock(100, address(0), 100000000000000, 20, "blu", salt);

        IPublicLock lock = IPublicLock(address(uint160(getLockAddress(salt))));
        lockToKey[key] = lock;
    }
