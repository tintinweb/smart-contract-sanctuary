...
contract NameTag is ERC721, Ownable, StringUpper, DenyList, Random {
    ...
    function getFreeTokenPosition(uint256 position) public view virtual returns (uint256) {
        return randomTokenPositions[position];
    }
    ...
    function _setNamesMetadata(uint256[] memory _tokens, string[] memory _names, string[] memory _metadata) public returns (bool[] memory) {
        ...
        for (uint index = 0; index < _tokens.length; index += 1) {
            ...
            if (statuses[index] && index < _metadata.length) {
    ...
    function setNames(uint256[] memory _tokens, string[] memory _names) public returns (bool[] memory) {
        require(_tokens.length == _names.length);

        string[] memory metadata;
        return _setNamesMetadata(_tokens, _names, metadata);

    }

    function setNamesMetadata(uint256[] memory _tokens, string[] memory _names, string[] memory _metadata) public returns (bool[] memory) {
        require(_tokens.length == _names.length && _tokens.length == _metadata.length);

        return _setNamesMetadata(_tokens, _names, _metadata);
    }

    function setMetadata(uint256[] memory _tokens, string[] memory _metadata) public returns (bool[] memory) {
        ...
        for (uint index = 0; index < _tokens.length; index += 1) {
            ...
            if (msg.sender != ownerOf(_tokens[index])) {
        ...
        require(fullStatus); // at least one metadata was set
    ...
    function buyTokens(string[] memory _names) external payable returns (uint256[] memory, bool[] memory) {
        ...
        uint256[] memory tokens = _processPurchase(recipient, tokensAmount);
