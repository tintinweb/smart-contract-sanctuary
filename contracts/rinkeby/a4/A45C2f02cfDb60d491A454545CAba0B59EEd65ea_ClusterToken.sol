...
contract ClusterToken is ERC20Pausable, AccessControl, IClusterToken {
    ...
    /// @notice Minimum amount of dust ETH to send to user during assemble.
    ...
    /// @notice Function that allows contract owner to update dust amount.
    function setDustAmount(uint256 _dustAmount) external onlyAdmin {
    ...
    function assemble(uint256 clusterAmount, bool coverDhvWithEth) external payable override whenNotPaused checkBuyer returns (uint256) {
        ...
        uint256 denominator = _swapEthToUnderlyings(msg.value, clusterAmount);
    ...
    function _swapEthToUnderlyings(uint256 _ethAmount, uint256 _clusterAmount) internal returns (uint256) {
        address adapter = IController(clusterControllerAddress).adapters(address(this));

        (uint256[] memory underlyingAmounts, uint256[] memory ethPortion, uint256 clusterPrice, uint256 ethCalculated) = IController(
