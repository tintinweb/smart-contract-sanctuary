...
contract Buildspace is ERC721URIStorage {
    mapping(string => uint256) cohortLimits;
    mapping(string => uint256) cohortTokensMinted;
        ...
        returns (uint256)
    {
        ...
        uint256 nextCohortTokenId = cohortTokensMinted[_cohortId];
        string memory _uri = string(
            abi.encodePacked(
                _cohortId,
                "-",
                uint2str(nextCohortTokenId),
                "/metadata.json"
            )
        );
        ...
        claimed[msg.sender][_cohortId] = true;

        _setTokenURI(newTokenId, _uri);

        cohortTokensMinted[_cohortId] = nextCohortTokenId + 1;
        ...
        returns (uint256)
    {
        ...
        require(
            !claimed[to][_cohortId],
            "Buildspace: address has already claimed airdrop."
        );

        uint256 nextCohortTokenId = cohortTokensMinted[_cohortId];
        string memory _uri = string(
            abi.encodePacked(_cohortId, "-", uint2str(nextCohortTokenId))
        );
