...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
    ...
    uint256 public maxNoOfPaths;
    uint256 public maxNoOfCommands;
    uint256 public size;
    ...
    function create() public returns (bytes32 _requestId) {
        ...
        //math.random() // keccak(block.difficulty)
        ...
        override
    {
        ...
        //issue: chainlink vrf has a max guess if 200,000 gas (computation units)
    }

    function finishMint(uint256 _tokenId) public {
        //  [x] check to see if its been minted and a random number is returned
        // [x] generate random svg code
        // [x] turn that into imageURI
        // [x] format imageURI to tokenURI
        require(
    ...
    {
        uint256 numberOfPaths = (_randomNumber % maxNoOfPaths) + 1;
        finalSvg = string(
            abi.encodePacked(
    ...
    {
        uint256 noOfPathCommands = (_randomNumber % maxNoOfCommands) + 1;
        pathSvg = "<path d='";
        for (uint256 i = 0; i < noOfPathCommands; i++) {
        ...
        returns (string memory pathCommand)
    {
        ...
        uint256 param2 = uint256(
            keccak256(abi.encode(_randomNumber, size * 2 + 1))
        ) % size;
        uint256 param3 = uint256(keccak256(abi.encode(_randomNumber, size))) %
            size;
        uint256 param4 = uint256(
            keccak256(abi.encode(_randomNumber, size * 2 + 10))
        ) % size;
        pathCommand = string(
            abi.encodePacked(
                " ",
