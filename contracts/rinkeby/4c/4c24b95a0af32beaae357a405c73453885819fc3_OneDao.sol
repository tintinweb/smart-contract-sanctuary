...
contract OneDao is ERC721, ERC721Enumerable, Ownable {
    ...
    Counters.Counter public goldenCounter;
    Counters.Counter public silverCounter;

    uint256 private constant GOLDEN_PRICE = 5 * 10**17; // 0.5 ETH
    uint256 private constant SILVER_PRICE = 1 * 10**17; // 0.1 ETH

    uint256 private constant MAX_GOLDEN_COUNT = 100;
    ...
    function buyGold() external payable virtual {
        require(
            goldenCounter.current() < MAX_GOLDEN_COUNT,
            "OneDao: Exceed MAX_GOLDEN_COUNT"
        );
        require(
            GOLDEN_PRICE == msg.value,
            "OneDao: Ether value sent is too low"
        );

        _safeMint(msg.sender, goldenCounter.current());
        goldenCounter.increment();
    }
