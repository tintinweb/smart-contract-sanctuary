...
contract Cry is ERC721Enumerable, Ownable {
   using Strings for uint256;
  ...
  //constants
    ...
    uint256 private maxPublicMintAmount = 5;
    uint256 private reservedOwnerMint=50;
  ...
  //Actions
    ...
    constructor(string memory _name, string memory _symbol,string memory _initBaseURI,string memory _initcontractURI) ERC721(_name, _symbol) {
        setBaseURI(_initBaseURI);
        setcontractURI(_initcontractURI);
        mint_function(msg.sender, reservedOwnerMint);
    ...
    function mint_function(address _to, uint256 _mintAmount) internal {
        require(paused==false,"Contract is paused");
        require(_mintAmount > 0,"Please give positive amount");
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= maxSupply,"There is not enough token");
  ...
  // public
   function preSaleMint(uint256 _mintAmount, uint256 _MaxMintAmount,bool _earlyInvest, bytes32[] memory proof) external payable {
        require(presale==true, "Pre-sale minting is not active");
        require(_earlyInvest || msg.value >= presaleCost * _mintAmount,"please give eth");
        require( MerkleProof.verify(proof,merkleRoot, keccak256(abi.encodePacked(msg.sender,_MaxMintAmount,_earlyInvest))),"Not Verified");
        
        if (!whitelistUsed[msg.sender]){
            whitelistUsed[msg.sender]=true;
            whitelistRemaining[msg.sender]=_MaxMintAmount;
        }
        require(whitelistRemaining[msg.sender]-_mintAmount>=0,"Mint Remaining false");
        whitelistRemaining[msg.sender]-=_mintAmount;
        mint_function(msg.sender,_mintAmount);
    }
    
    function publicSaleMint(uint256 _mintAmount) external payable {
        require(publicsale==true, "Public-sale minting is not active");
        require(maxPublicMintAmount > _mintAmount,"You Can't take more maxMintAmount");
        require(maxPublicMintAmount > publicSaleRemaining[msg.sender]+_mintAmount,"You Can't take more maxMintAmount");
        require(msg.value >= publicsaleCost * _mintAmount,"Please give eth");
    ...
    function withdraw() public payable {
        require(payable(ownerAdress).send(address(this).balance));
    }
}