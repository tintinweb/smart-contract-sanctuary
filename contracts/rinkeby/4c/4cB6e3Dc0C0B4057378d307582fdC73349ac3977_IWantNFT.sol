...
contract IWantNFT is ERC721Enumerable, Ownable {
    ...
    // The next token ID for each primary key
    // Eg (0->999, 1000->1999, 2000->2999, 3000->3999, 4000->4999)
    ...
    // Base metadata URI
    string public constant BASE_URI = "http://storage.treum.io/iwantnft/";

    // Storage for overriding default metadata URIs
    ...
    function mint(uint256 primaryId) external payable {
        ...
        require(primaryId < 4, "Invalid primary id");
     ...
     * @param tokenIds array of token IDs to have their URIs updated
     * @param _newTokenURIs array of new URI strings corresponding to the tokenIDs array
     */
    function setTokenURIBatch(uint256[] memory tokenIds, string[] memory _newTokenURIs) onlyOwner external {
        require(tokenIds.length == _newTokenURIs.length, "Incorrect array length");
