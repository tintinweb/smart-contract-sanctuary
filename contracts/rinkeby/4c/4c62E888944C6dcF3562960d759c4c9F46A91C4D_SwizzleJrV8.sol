...
import "./SwizzleJr.sol";

contract SwizzleJrV8 is SwizzleJr {
    uint256 private constant PRICE_PER_NFT = 0.01 ether;

    constructor()
        SwizzleJr(
            "SwizzleJrV8",
            "TVS-SwizzleJr-Dev-V8",
...
abstract contract SwizzleJr is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 private constant MAX_MINT_PER_TRANS = 10;
    uint256 private constant MAX_COMBINABLE_TOKENS = 12;
    string private constant TOKEN_URI_PREFIX = "/ss";
    string private constant TOKEN_URI_POSTFIX = ".json";
    ...
    constructor(
        ...
        // This works for minting 1 instance per flavor as well as 2 instances per flavor
        // but higher numbers of instances per flavor would require using int instead of bit
        lastBitEnabled = enableLastBit;
        ...
        for (uint256 i = 1; i <= numberOfFlavors; i++) {
            if (lastBitEnabled) {
                ...
                _flavorURIs[((i + 1) * 100) + 1] = defaultFlavorURI;
                _flavorURIs[((i + 1) * 100) + 2] = defaultFlavorURI;
            } else {
                // No fuss numbering.
                _flavorURIs[i * 100] = defaultFlavorURI;
            }
        }
    }

    function mintSwizzles(uint256 count) external payable nonReentrant {
        ...
        _mintSwizzles(_msgSender(), count);
    }

    function forgeSwizzles(uint256[] memory ownerTokenIndexes, uint256 opCode)
        external
        nonReentrant
    {
        require(
            ownerTokenIndexes.length <= MAX_COMBINABLE_TOKENS,
            "Too many tokens"
        );
        ...
        uint256 idx;
        uint256[] memory ownedTokenIds = new uint256[](
            ownerTokenIndexes.length
        );
        for (idx = 0; idx < ownerTokenIndexes.length; idx++) {
            // Validate existence and ownership.
            ownedTokenIds[idx] = tokenOfOwnerByIndex(
                _msgSender(),
                ownerTokenIndexes[idx]
            );
        }

        uint256 targetId = maxNftsForSale + 1;
        uint256 targetFlavor = 0;
        uint256 checkValue = 0;

        for (idx = 0; idx < ownedTokenIds.length; idx++) {
            uint256 tokenFlavor = _tokenFlavors[ownedTokenIds[idx]];
            ...
            if (ownedTokenIds[idx] < targetId) targetId = ownedTokenIds[idx];
        }
        ...
        // Now burn the pieces
        for (idx = 0; idx < ownedTokenIds.length; idx++) {
            _burn(ownedTokenIds[idx]);
        }

        // Make sure the slot is now free.
        require(!_exists(targetId), "Can't re-mint");

        // Execute the mint.
        _safeMint(_msgSender(), targetId);
        _setTokenURI(targetId, targetFlavor);
    }

    function burnSwizzle(uint256 tokenId) external nonReentrant {
        require(
    ...
    function _getEntropy(uint256 tokenId)
        internal
        view
        virtual
        returns (uint256)
    ...
    function _randomFlavorFor(uint256 tokenId) internal view returns (uint256) {
        require(numberOfFlavors <= 9, "Too many flavors");

        uint256 someEntropy = _getEntropy(tokenId);

        // We need two pseudorandom numbers to pick our selected image,
        // so use the first half and second half of the 256 bit number
        // for this.
        uint256 rightBitSelection = uint128(someEntropy) % maxNftsForSale;
        uint256 leftBitSelection = uint128(someEntropy >> 128) % maxNftsForSale;

        uint256 theFlavor;
        uint256 lastBit = 0;

        if (lastBitEnabled) {
            // Distributed equally. Start at 200, for forging compatibility.
            theFlavor = ((rightBitSelection % numberOfFlavors) + 2) * 100;

            // Distributed based on golden ratio.
            lastBit = (leftBitSelection % 100) > 61 ? 2 : 1;
        } else {
            theFlavor = ((rightBitSelection % numberOfFlavors) + 1) * 100;
        }
    ...
    function _mintSwizzles(address minter, uint256 count) private {
        require(minter != address(0), "Invalid address");
        require(
            _coinMintCounter.current() + count <= maxNftsForSale,
            "Limit exceeded"
        );
