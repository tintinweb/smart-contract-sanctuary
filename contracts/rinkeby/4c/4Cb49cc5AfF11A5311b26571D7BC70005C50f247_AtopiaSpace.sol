...
interface ICenter {
	...
	function work(uint256 tokenId, uint16 package, uint256 working) external returns (uint256);
...
contract AtopiaSpace is ERC721 {
	...
	function work(
		...
		if (package > 0) {
			...
			lives[tokenId] = centers[centerId - 1].work(tokenId, package, lives[tokenId]);
		} else {
			require(job == centerId && currentPackage > 0);
			centers[centerId - 1].work(tokenId, package, lives[tokenId]);
			lives[tokenId] = 0;
		}
		emit LifeUpdated(tokenId, lives[tokenId]);
	}
