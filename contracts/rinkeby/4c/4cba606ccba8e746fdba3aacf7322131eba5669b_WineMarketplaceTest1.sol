...
contract WineMarketplaceTest1 is WineOrdersHandler, Ownable {
    ...
    function getDeals() external view onlyOwner returns(WineOrdersHandler.Deal  [] memory){
        return _deals;
    }
...
//    }
    ...
    function makeDeal_Test(address seller_, address buyer_, address cellar_, uint256 price_) external returns (uint256) {
        Deal memory deal = Deal({
        dealId : _dealIdTracker.current(),
        askOrderId : 1,
        bidOrderId : 2,
        ...
        tokenId : 1,
        numberOfBottles : 10,
        numberOfBottlesInCase : 12,
        cellar : cellar_,
        currency : "USD",
        pricePerBottle18 : price_,
        createdAt : block.timestamp
        });
