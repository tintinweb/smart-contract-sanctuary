...
contract eternal_page_v1_2b {
    ...
    event _RewardsDistributed(uint256 user_xdai_rewards_, uint256 user_staking_rewards_);
    ...
    uint256 public MODERATION_APPROB_MIN_VOTE_COUNT =  1000 * (10**18); //1000 tokens
    uint256 public MIN_STAKE_TO_START_PROPOSAL = 1000 * (10**18);
    uint256 public MIN_VOTE_TOTAL_AMOUNT =  10000 * (10**18); //10 000 tokens casted into the vote to pass
    ...
    uint256 public STANDARD_VOTE_DURATION = 60 seconds;
    ...
    uint256 public BIDDING_PRICE_RATIO_PERCENT = 120; // MUST BE STRICLY > 100
    ...
    uint256 public staking_rewards_pool = 0;
    ...
    */
        ...
        public
    {
        ...
        uint256 available_amount = staked_amount.sub(getLockedTokens(msg.sender));
        require(available_amount >= _stake, "not enough available_amount to unstake _stake");
        ...
        public
    {
        ...
        require(last_collect_epoch<epoch && (last_epoch_date - last_collect_timestamp) >= EPOCH_DURATION,"can't distribute : too early to call");
        
        // compute current staking reward amount for this epoch
        uint256 staking_rewards_amount = staking_rewards_pool*1/100;
        staking_rewards_pool = staking_rewards_pool.sub(staking_rewards_amount);
        ...
        for (uint256 s = 0; s < stakeholders.length; s += 1){
            ...
            if(balances[sh].last_epoch_voted < epoch && (last_epoch_date.sub(balances[sh].stake_timestamp)) >= EPOCH_DURATION){
                ...
                uint256 user_staking_rewards_ = staking_rewards_amount.mul(userStake).div(last_epoch_total_stake_);
                
                balances[sh].last_collected_epoch = epoch;
                //-------- distribute staking rewards
                balances[sh].available_token_balance += user_staking_rewards_; //if we distribute in user
                ...
                total_token_rewards += user_staking_rewards_;
            }
        }
        
        last_collect_timestamp = block.timestamp;
        last_collect_epoch = epoch;
        emit  _RewardsDistributed(total_xdai_rewards, total_token_rewards);
    }
    ...
    function CloseModApprobationVote() public {
        ...
        for (uint i = 0; i < active_moderators_.length; i++) {
            ...
            if( moderators[a].votes_slash / (moderators[a].votes_ok+moderators[a].votes_slash) * 100 >= MODERATION_QUORUM_SLASHING ){ // if more than 70% votes in favor of slashing current moderator
                ...
                staking_rewards_pool += slashed_token_amount;
    ...
    */
    ...
    @param _prevProposalID      (linked-list optimization) IDs of the proposal that the user has voted the maximum number of tokens in which is still less than or equal to numTokens, by default put 0 
    @param _numTokens           number of tokens to "cast" into the vote, they are unlocked when the proposal is expired/passed
    */
    function VoteOnProposal(uint _proposalID, bool decision ,uint _prevProposalID, uint256 _numTokens) public {
        require(votePeriodActive(_proposalID), "votePeriod is passed for _proposalID");
        require(_numTokens>0, "_numTokens must be > 0");
        require(_proposalID != 0,"_proposalID must be >0");
        
        // make sure msg.sender has enough voting rights == staked tokens
        require(balances[msg.sender].staked_token_balance >= _numTokens,"_proposalID must be >0");      

        // Check if _prevProposalID exists in the user's DLL or if _prevProposalID is 0
        require(_prevProposalID == 0 || dllMap[msg.sender].contains(_prevProposalID),"_prevProposalID not contained in dllMap or invalid");

        uint nextProposalID = dllMap[msg.sender].getNext(_prevProposalID);

        // edge case: in-place update
        if (nextProposalID == _proposalID) {
            nextProposalID = dllMap[msg.sender].getNext(_proposalID);
        }
        ...
        if (decision == true) {// apply numTokens to appropriate poll choice
            ...
            proposals[_proposalID].votesAgainst += _numTokens;
        }

        require(validPosition(_prevProposalID, nextProposalID, msg.sender, _numTokens),"validPosition error");
        dllMap[msg.sender].insert(_prevProposalID, _proposalID, nextProposalID);
        bytes32 UUID = attrUUID(msg.sender, _proposalID);
        ...
        emit _Voted(_proposalID, _numTokens, msg.sender);
    }
    ...
    @param _prevProposalIDs     Array of IDs of the proposals that the user has voted the maximum number of tokens in which is still less than or equal to numTokens
    @param _numTokens           Array of the number of tokens to "cast" into each vote, they are unlocked when the proposal is expired/passed
    */
    function VoteOnProposals(uint[] calldata  _proposalIDs, bool[] memory _decisions, uint[] calldata _prevProposalIDs, uint256[] memory _numTokens) external {
        // make sure the array lengths are all the same
        require(_proposalIDs.length == _decisions.length, "input arrays must be same length");
        require(_decisions.length == _prevProposalIDs.length, "input arrays must be same length");
        require(_prevProposalIDs.length == _numTokens.length, "input arrays must be same length");
        
        // loop through arrays, committing each individual vote values
        for (uint i = 0; i < _proposalIDs.length; i++) {
            VoteOnProposal(_proposalIDs[i], _decisions[i], _prevProposalIDs[i], _numTokens[i]);
        }
    }
    ...
    */
    function ValidateProposal(uint _proposalID) public {
        ...
        // unlock tokens from voters of this proposal automatically (instead of manual rescue)
        for(uint v = 0; v < proposals[_proposalID].voters.length; v++){
            address user = proposals[_proposalID].voters[v];
            if(dllMap[user].contains(_proposalID)){
                dllMap[user].remove(_proposalID);
            }
        }
    ...
    */
    ...
    /**
    @dev Unlocks tokens locked in unvalidated vote where proposal has ended
    @param user             user to rescue tokens
    @param _proposalID      Integer identifier associated with the target proposal
    */
    function rescueTokens(address user, uint _proposalID) public {
        require(isExpired(proposals[_proposalID].voteEndDate),"proposal _proposalID hasn't expired");
        require(dllMap[user].contains(_proposalID),"dllMap doesn't contains this _proposalID for given user");

        dllMap[user].remove(_proposalID);
        emit _TokensRescued(_proposalID, user);
    }


    /**
    @dev Unlocks tokens locked in unrevealed votes where proposals have ended
    @param user             user to rescue tokens
    @param _proposalIDs     Array of integer identifiers associated with the target proposals
    */
    function rescueTokensInMultipleProposals(address user, uint[] memory _proposalIDs) public {
        // loop through arrays, rescuing tokens from all
        for (uint i = 0; i < _proposalIDs.length; i++) {
            rescueTokens(user, _proposalIDs[i]);
        }
    }
    
    
    /**
    @dev Compares previous and next proposal's committed tokens for sorting purposes
    @param _prevID      Integer identifier associated with previous proposal in sorted order
    @param _nextID      Integer identifier associated with next proposal in sorted order
    @param _voter       Address of user to check DLL position for
    @param _numTokens   The number of tokens to be committed towards the proposal (used for sorting)
    @return valid       Boolean indication of if the specified position maintains the sort
    */
    function validPosition(uint _prevID, uint _nextID, address _voter, uint _numTokens) internal view returns (bool valid) {
    ...
    */
    ...
    @param tokens                   amount of tokens to be transfered, to be called after approve() on the token contract
    @param amount_for_staking       amount of tokens to be transfered to the staking-rewards pool
    @param amount_for_owners        amount of tokens to be transfered to the block-owners-rewards pool
    */
    function DepositRewardsPool(uint256 tokens, uint256 amount_for_staking, uint256 amount_for_owners) external {
        require(token.balanceOf(msg.sender) >= tokens, "token balance too low");
        require(amount_for_staking + amount_for_owners == tokens, "amount_for_staking + amount_for_owners must be equal to tokens");
        // add the deposited tokens into existing balance 
        staking_rewards_pool = staking_rewards_pool + amount_for_staking;
        owners_rewards_pool = owners_rewards_pool + tokens.sub(amount_for_owners);
    ...
    */
    ...
    @param _voter           Address of user to check against
    @param _proposalID      Integer identifier associated with target proposal
    @return numTokens       Number of tokens committed to proposal in sorted proposal-linked-list
    */
    function getNumTokens(address _voter, uint _proposalID)  internal view returns (uint numTokens) {
        return store.getAttribute(attrUUID(_voter, _proposalID), "numTokens");
    }

    /**
    @dev Gets top element of sorted proposal-linked-list
    @param _voter           Address of user to check against
    @return proposalID      Integer identifier to proposal with maximum number of tokens committed to it
    */
    function getLastNode(address _voter)  public view returns (uint proposalID) {
    ...
    @param _voter           Address of user to check against
    @return numTokens       Maximum number of tokens committed in proposal specified
    */
    ...
    is found, if the provided _proposalID matches the found nodeID, this operation is an in-place
    ...
    */
    function getInsertPointForNumTokens(address _voter, uint _numTokens, uint _proposalID) internal view returns (uint prevNode) {
