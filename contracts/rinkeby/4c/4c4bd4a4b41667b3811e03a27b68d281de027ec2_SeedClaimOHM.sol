...
/**
 *  This contract allows Olympus seed investors and advisors to claim tokens at par value.
 *  It has been revised to consider claims as staked immediately for accounting purposes.
 *  This means that the amount claimed increases with staking rewards, and claimants only receive
 *  the network Margin of Dilution once they have maxed their claims. This also penalizes sellers,
 *  since the balance sold is still considered staked within this contract. This is a strong step
 *  taken to rebalance the exposure profile of seed investors and advisors. This change is applied
 *  retroactively, effectively making it implemented from launch.
 */
contract SeedClaimOHM {
    ...
    function claim( uint _amount ) external {
        ...
        require( info.max.sub( claimed( msg.sender ) ) >= _amount, 'Claimed over max' );

        DAI.safeTransferFrom( msg.sender, address( this ), _amount );
        
        DAI.approve( address( treasury ), _amount );
        uint OHMToSend = treasury.deposit( _amount, address( DAI ), 0 );

        terms[ msg.sender ].wClaimed = info.wClaimed.add( wOHM.sOHMTowOHM( _amount ) );

        OHM.safeTransfer( msg.sender, OHMToSend );
    }
    ...
    /**
     *  @notice view OHM claimable for address
     *  @param _address address
     *  @return uint
     */
    function redeemableFor( address _address ) public view returns (uint) {
        uint max = circulatingSupply().mul( terms[ _address ].percent ).mul( 1000 );
        return max.sub( claimed( _address ) );
    }

    /**
     *  @notice view OHM claimed by address
     *  @param _address address
     *  @return uint
     */
