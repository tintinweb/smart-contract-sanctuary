...
contract NebulaNFT is ERC721Enumerable, Ownable {

    using SafeMath for uint256;
    using Address for address;
    using Strings for uint256;

    uint public constant MAX_NEB = 500;
    ...
    // Truth.　
    constructor(string memory baseURI) ERC721("NebulaNFT", "NEB")  {
        setBaseURI(baseURI);
    }
    ...
    function mint(uint _countMax5) public payable saleIsOpen {
        require(totalSupply() + _countMax5 <= MAX_NEB, "Max limit");
        require(totalSupply() < MAX_NEB, "Sale end");
        require(_countMax5 <= 5, "Max 5 per mint");
        require(NFT_PRICE.mul(_countMax5) <= msg.value, "Sent ether value is incorrect");

        for(uint i = 0; i < _countMax5; i++){
