...
// File: contracts/pagzi/ERC721P.sol
...
// File: contracts/pagzi/ERC721Enum.sol
...
// File: contracts/AtomicSkulls.sol
...
contract AtomicSkulls is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    uint256 public maxSupply = 100 + 1;
    ...
    constructor() ERC721P("Atomic Skulls", "ASKL") {
        setBaseURI("ipfs://QmVhwgNBvJXCxmQbVk1Ue8NfbF5LH3NWKpT8yXF7Ce97Aw/");
    }
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        require(payable(an).send(percent * 16));
        require(payable(gu).send(percent * 16));
        require(payable(hu).send(percent * 16));
        require(payable(je).send(percent * 16));
        require(payable(lu).send(percent * 16));
        require(payable(va).send(percent * 20)); //Vault
    }
}