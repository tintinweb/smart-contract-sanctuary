...
contract Traits is Ownable, ITraits {
	 ...
	 * @return a valid SVG of the Ape / Wolf
	 */
	function drawSVG(uint256 tokenId) public view returns (string memory) {
		IWoolf.ApeWolf memory s = woolf.getTokenTraits(tokenId);
		uint8 shift = s.isApe ? 0 : 6;

		string memory svgString = string(
			abi.encodePacked(
				s.isApe ? drawTrait(traitData[0 + shift][s.skin]) : drawTrait(traitData[0 + shift][s.skin]),
				s.isApe ? drawTrait(traitData[1 + shift][s.eyes]) : drawTrait(traitData[1 + shift][s.eyes]),
				s.isApe ? drawTrait(traitData[2 + shift][s.mouth]) : drawTrait(traitData[2 + shift][s.mouth]),
				s.isApe ? "" : drawTrait(traitData[3 + shift][s.clothing]),
				s.isApe ? drawTrait(traitData[4 + shift][s.headwear]) : drawTrait(traitData[4 + shift][s.alphaIndex])
			)
		);
	...
	function compileAttributes(uint256 tokenId) public view returns (string memory) {
		...
		if (s.isApe) {
			...
			traits = string(
				abi.encodePacked(
					attributeForTypeAndValue(_traitTypes[0], traitData[6][s.skin].name),
					",",
	...
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		...
		string memory metadata = string(
			abi.encodePacked(
				...
				s.isApe ? "Ape #" : "Wolf #",
				tokenId.toString(),
				'", "description": "Thousands of Ape and Wolves compete on a farm in the metaverse. A tempting prize of $MPeach awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
