...
contract LiquidityProvider is Ownable {
    ...
    function addLiquidity(uint amountTokenDesired) external onlyOwner {
        ...
        uniswap.addLiquidityETH.value(address(this).balance)(address(token), amountTokenDesired, amountTokenDesired, address(this).balance, address(this), now + 2 hours);
        emit Provided(amountTokenDesired, address(this).balance);
    }

    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(now >= lock.add(4 hours), "You can claim LP tokens after 4hours weeks");
