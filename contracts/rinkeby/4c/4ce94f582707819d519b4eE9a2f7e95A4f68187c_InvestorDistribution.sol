...
contract InvestorDistribution is Pausable, ReentrancyGuard {
    ...
    function addInvestor(address _investor, uint256 _amount) external whenNotPaused {
        ...
        investors[_investor] = Investor(_amount, _amount);
    ...
    function modifyInvestor(address _investor, address _new) external whenNotPaused nonReentrant {
        ...
        require(investors[_investor].remaining > 0);
        require(msg.sender == admin, "Unauthorized");

        investors[_new] = Investor(investors[_investor].total, investors[_investor].remaining);
        investors[_investor] = Investor(0, 0);
    ...
    function _claim(uint256 _value) private {
        ...
        uint256 _total = investors[msg.sender].total;
        uint256 _remaining = investors[msg.sender].remaining;
        uint256 _claimed = _total - _remaining;
        require(_remaining > 0, "Nothing to claim.");

        _updateEmission();

        uint256 claimable = _available_supply() * _total / investors_supply;
        ...
        investors[msg.sender].remaining -= _value;
    ...
    function _updateEmission() private {
        if (block.timestamp >= startEpochTime + RATE_TIME) {
            ...
            if (startEpochSupply >= investors_supply) {
                startEpochSupply = investors_supply;
                rate = 0;
            }
            else {
    ...
    // Dev can access all tokens in the contract after 5 year period
    // to take care of fringe cases of lost or unclaimed tokens
    ...
    function dev_rugpull() public {
        ...
        mainToken.safeTransfer(msg.sender, bal);
