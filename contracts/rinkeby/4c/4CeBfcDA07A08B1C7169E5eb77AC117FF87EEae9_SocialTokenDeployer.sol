...
contract SocialTokenDeployer {
    ...
    event TokenSpawned(address token, address owner, address spawner, uint256 totalCap, uint256 initial, uint256 reserved);
    ...
    function spawn(
        ...
        emit TokenSpawned(tokenAddress, msg.sender, msg.sender, totalCap, initialMint, totalCap - initialMint);
    }
...
contract SocialERC20 is ERC20, AccessControl, Initializable {
    ...
    function initialize(
        ...
        require(tax.denominator > 0, "Taxation denominator must not be zero");
        require(tax.numerator <= tax.denominator, "Cannot tax more than 100%");
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        uint256 taxAmount = (amount * _tax.numerator) / _tax.denominator;
    ...
    function setTax(Tax memory newTax) external onlyRole(OWNER_ROLE) {
        require(newTax.denominator > 0, "Taxation denominator must not be zero");
        require(newTax.numerator <= newTax.denominator, "Cannot tax more than 100%");
        _tax = newTax;
    }
