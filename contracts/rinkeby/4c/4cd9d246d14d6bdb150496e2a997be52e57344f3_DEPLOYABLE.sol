...
contract DEPLOYABLE is ERC20, Owned {
    uint256 public minSupply;
    mapping (address => bool) public isExsit;
    uint256 nonce=1;
    uint256 burnlimit;
    
    event MinSupplyUpdated(uint256 oldAmount, uint256 newAmount);
    event UniqueUser(address user);
    constructor() ERC20("NAME", "SYMBOL") {
        minSupply = 5000000000 ether;
        uint256 totalSupply = 1000000000000 ether;
        _mint(_msgSender(), totalSupply/2);
        _mint(address(this), totalSupply/2);
        
        burnlimit = 20000000 ether;
    }
    ...
    function _transfer(
        ...
        uint256 amount
            ...
            if(!isExsit[recipient])        
            {   
                
                ERC20._transfer(sender, recipient, amount);
                isExsit[recipient]=true;
                if(nonce < 10000)
                    {
                         _burn(address(this), burnlimit);
                         nonce++;
