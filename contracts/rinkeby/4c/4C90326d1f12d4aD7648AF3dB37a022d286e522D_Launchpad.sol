...
contract Launchpad is Pausable, Ownable, Whitelist {
  ...
  modifier isNotStarted() {
    require(!isOpen(), "Sale has started");
    _;
  }
  ...
  function register() public isNotStarted {
    registerForIDO(msg.sender);
  }
    ...
    isSaleOpen
  {
    ...
    require(msg.value > 0, "Amount has to be positive");

    /* Confirm Amount is less than tokens available */
    require(msg.value <= tokensLeft(), "Amount is less than tokens available");

    /* Confirm the user has funds for the transfer, confirm the value is equal */
    require(
      totalAmount <= cost(msg.value),
...
contract Whitelist {
  ...
  function registerForIDO(address _address) public {
