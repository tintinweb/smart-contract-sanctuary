...
contract Eternity is ERC721Enumerable, Ownable, ReentrancyGuard {

	uint256 public constant ETERNITY_MAX_AMOUNT = 2000;
	uint256 public constant TEAM_FUND = 1000;
	uint256 public ETERNITY_PER_TX = 15;
	uint256 public ETERNITY_PRICE = 0.08 ether;
	address public founder = 0x323d99e72AeD86310911E595116681D3e2E45316;

	uint256 public mintedPresaleCount;

	mapping (uint256 => uint256) private _eternityBirthdays;
	...
	function mintPublic(uint256 amount) external payable nonReentrant() {
		...
		require(amount <= ETERNITY_PER_TX, "You can't mint that many tokens");
		require(totalSupply() + amount <= ETERNITY_MAX_AMOUNT, "Mint would exceed max supply of tokens");
		require(msg.value == amount * ETERNITY_PRICE, "You didn't send the right amount of eth");
	...
	function _mintMultiple(address owner, uint256 amount) private {
		for (uint256 i = 0; i < amount; i++) {
			...
			_eternityBirthdays[tokenId] = block.timestamp;
	...
	function enableClaimFoundingMembers(address[] memory members, uint256[] memory amounts) external onlyOwner {
		...
		for (uint256 i = 0; i < members.length; i++) {
			...
			require(amounts[i] <= ETERNITY_PER_TX, "amount is more than the allowable");
	...
	function setIpfsHash(string[] memory ipfsHashs, uint256[] memory idTokens) external onlyOwner {
		...
		for (uint256 i = 0; i < idTokens.length; i++) {
			...
			require(idTokens[i] <= ETERNITY_MAX_AMOUNT, "id is more than the allowable");
	...
	function mintPresaleMember() external payable nonReentrant() {
		...
		mintPresaleMemberWithAmount(amount);
	}

	function mintPresaleMemberWithAmount(uint256 amount) public payable nonReentrant() {
		uint256 availableAmount = _availableClaims[msg.sender];
		require(availableAmount >= amount, "Wrong amount");
		...
		_availableClaims[msg.sender] -= amount;
		_mintMultiple(msg.sender, amount);
	}
	...
	function setEternityBirthdaysPrice(uint256 newPrice) public onlyOwner {
		require(newPrice > 0, "Wrong price");
		emit SetNewValue("ETERNITY_PRICE", ETERNITY_PRICE, newPrice);
		ETERNITY_PRICE = newPrice;
	}

	function setEternityBirthdaysPerTx(uint256 newPerTx) public onlyOwner {
		require(newPerTx > 0, "Wrong per tx");
		emit SetNewValue("ETERNITY_PER_TX", ETERNITY_PER_TX, newPerTx);
		ETERNITY_PER_TX = newPerTx;
	}
	...
	function getBirthday(uint256 tokenId) public view returns (uint256) {
		require(tokenId < totalSupply(), "That eternity has not been make yet");
		return _eternityBirthdays[tokenId];
	}

	function getIpfsTokenUri(uint256 tokenId) external view returns (string memory) {
		require(tokenId < totalSupply(), "That eternity has not been make yet");
		require(getBirthday(tokenId) < block.timestamp, "That eternity has not grown yet");
