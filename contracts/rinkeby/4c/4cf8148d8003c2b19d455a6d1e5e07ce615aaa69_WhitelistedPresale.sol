...
contract WhitelistedPresale is Ownable {
    ...
    modifier onlyAllowedBuyer(uint256 amount) {
        require(_buyers_[msg.sender].bought + amount <= _buyers_[msg.sender].cap, "Presale: this address is not allowed to buy.");
        _;
    }

    function setBuyerCap(address buyer, uint256 cap) public onlyOwner {
        _buyers_[buyer].cap = cap;
    }

    function setBuyerCapBatch(address[] memory buyers, uint256[] memory amount) public onlyOwner {
    ...
    uint256 _startTime_ = 0;
    uint256 _endTime_ = 0;
    uint256 _timeLength_ = 7 days;

    modifier onlyDuringPresale() {
        require(block.timestamp >= _startTime_ && block.timestamp < _endTime_, "Presale: Presale is not now.");
        _;
    }

    function start() public onlyOwner {
        _startTime_ = block.timestamp;
        _endTime_ = _startTime_ + _timeLength_;    
    }

    function end() public onlyOwner {
        _endTime_ = block.timestamp;
    }
