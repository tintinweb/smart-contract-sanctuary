...
contract MonsterBlocks is MonsterBlockCoreERC721, VRFConsumerBase {
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal override {
    for (uint8 k = 0; k < vrfRequestIds[_requestId].length; k++) {
      uint256 blockDna = uint256(keccak256(abi.encode(_randomNumber, numberOfTraits * k)));
      monsterBlocksDna[vrfRequestIds[_requestId][k]] = blockDna;

      for (uint8 i = 0; i < numberOfTraits; i++) {
        uint256 currentWeight = traitWeighting;
        uint256 dnaModuloWeight = (uint256(keccak256(abi.encode(blockDna, i * k))) % currentWeight) + 1;
