...
contract NFTMarket is ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => SellHistory)) private tokenIdToSellHistory;
    ...
    function createMarketItem(
        ...
        require(minPrice <= maxPrice, "max price must be greater than min price");
        require(minPrice > 0);
    ...
    /// @notice refund offer if market item is no longer available
    /// @param itemId id of market item
    /// @param offerId id of offer that want to be refunded
    function refundOffer(uint256 itemId, uint256 offerId) public nonReentrant {
        uint256 currentBlock = block.number;
        uint256 amount = itemIdToOffer[itemId][offerId].amount;
        require(
            msg.sender != idToMarketItem[itemId].seller,
        ...
        require(
            (idToMarketItem[itemId].endBlock < currentBlock) ||
                (idToMarketItem[itemId].sold == true),
            "item hasn't exceeded refund stage"
        );
        require(
            itemIdToOffer[itemId][offerId].refundable,
            'offer has been refunded'
        ...
        require(
            itemIdToOffer[itemId][offerId].amount !=
                idToMarketItem[itemId].currentPrice,
            'winner can not refund'
        );
    ...
    /// @notice Returns only items that a user has buy
    ...
    /// @notice Returns only items a user has sell
    /// @param _user id of market item
