...
contract Escrow {
        ...
        returns (address)
    {
        ...
        if (holder[_tokenId][_editionNumber] == address(0)) {
            //after burining the token edition, token holder is set as owner again - check
    ...
    function placeOrder(
        ...
        if (_timeline == 0) {
            ...
            timeline = block.timestamp.add(_timeline.mul(1));
        }
        ...
        returns (bool)
    {
        ...
        require(_order.saleType == 1, "Wrong order type");
        require(
            _editionNumber > 0 &&
                _editionNumber <= tokenEditions[_order.tokenId],
    ...
    {
        Order memory _order = order[_orderNonce];
        require(_order.saleType == 2, "Can't cancel first hand orders");
