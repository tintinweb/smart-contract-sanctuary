...
contract ERC777Distributor is Initializable, OwnableUpgradeable, IERC777RecipientUpgradeable, ERC777Upgradeable {
    ...
    function _indexNewUpgradableToken(address token, address superToken) public onlyOwner {
        upgradeToDistributable[token] = superToken;
        tokenAllowed[token] = true;
        _indexNewSuperToken(superToken);
    }

    function _indexNewSuperToken(address token) public onlyOwner {
        if(!tokenAllowed[token]) {
            distributableTokens.push(token);
            tokenAllowed[token] = true;
