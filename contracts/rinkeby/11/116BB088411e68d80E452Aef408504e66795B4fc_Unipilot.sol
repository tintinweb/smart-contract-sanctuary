...
/// As of now only one manager of uniswap i.e. UniswapLiquidityManager contract
...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    modifier onlyLiquidityManager() {
        require(exchangeWhitelist[msg.sender], "UNIPILOT::NOT_LIQUIDITY_MANAGER");
        _;
    }

    modifier isExchangeWhitelist(address handler) {
        require(exchangeWhitelist[handler], "UNIPILOT::EXCHANGE_NOT_WHITELISTED");
        _;
    }

    /// @notice sets new address for the admin role
    function setGovernance(address _governance) external onlyGovernance {
        require(_governance != address(0), "UNIPILOT::INVALID_GOVERNANCE_ADDRESS");
    ...
    function setExchangeAddress(address exchangeAddress) external onlyGovernance {
        require(exchangeAddress != address(0), "UNIPILOT::INVALID_EXCHANGE_ADDRESS");
        exchangeWhitelist[exchangeAddress] = true;
    }

    function isAuthorizedForToken(address sender, uint256 tokenId)
        external
        view
        override
        returns (bool approved)
    {
        approved = _isApprovedOrOwner(sender, tokenId);
    }
    ...
    function createPoolAndDeposit(
        ...
        bytes[] memory data
    ) external payable isExchangeWhitelist(params.exchangeAddress) {
    ...
    /// @dev pass tokenId 0 to mint new position
        ...
        isExchangeWhitelist(params.exchangeAddress)
    {
        ...
        (uint256 totalAmount0, uint256 totalAmount1, uint256 totalLiquidity) = IHandler(
            params.exchangeAddress
        ).getReserves(params.token0, params.token1, data);
        ...
        if (amount0 > 0)
            pay(params.token0, _msgSender(), params.exchangeAddress, amount0);
        if (amount1 > 0)
            pay(params.token1, _msgSender(), params.exchangeAddress, amount1);

        IHandler(params.exchangeAddress).deposit(
            params.token0,
            params.token1,
            _msgSender(),
            amount0,
    ...
    /// @dev Calculates the shares depending upon reserves & total liquidity of pool 
...
abstract contract PeripheryPayments is IPeripheryPayments {
    ...
    address internal constant PILOT = 0x22bF7a840568514449ebc008926B5FF8cB43FEa8;
