...
contract humarsContract is ERC721Enumerable, Ownable {
    ...
    function claimFreeNFT() public payable {
        require(!paused, "Smart contract is paused");
        require(whitelisted[msg.sender] > 0, "You dont have any free NFT!");
        uint256 supply = totalSupply();
        require(supply + whitelisted[msg.sender] <= totalCount, "Supply limit reached!");
        
        for (uint256 i = 1; i <= whitelisted[msg.sender]; i++) {
