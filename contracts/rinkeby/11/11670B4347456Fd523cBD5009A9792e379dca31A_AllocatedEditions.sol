...
contract AllocatedEditions is IERC721, IERC165, IERC721Metadata, Governable, Reentrancy {
    ...
    // To emit the initial premint events, call the `emitAllocationTransfers`
    // function, which can only run once.
    bool public didEmitAllocationTransfers = false;
    ...
    function emitAllocationTransfers() public {
        require(!didEmitAllocationTransfers, "already emitted");

        for (uint256 i = 0; i < allocation; i++) {
            // Add a transfer event to give the token to the owner.
