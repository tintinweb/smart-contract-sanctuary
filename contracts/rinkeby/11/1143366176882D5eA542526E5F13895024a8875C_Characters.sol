...
contract Characters is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    Counters.Counter private _loots;
    Counters.Counter private _plebs;
    mapping (uint256 => bool) private _usedLoots;

    uint256 public price = 20000000000000000; //0.02 ETH

    //Loot Contract
    //address public lootAddress = 0xFF9C1b15B16263C61d017ee9F65C50e4AE0113D7;
    address public lootAddress = 0xcc9B9B0B4A07f509aCc6c7641191D8079270ea8E;
...
"Dog with backpack for loot",
"Cat with backpack for loot",
"Baby Panda",
"Whelp",
"Hound",
"Gorrila",
...
"Ape"
    ...
    function getEyeColor(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "EYECOLOR", eyeColor);
    }
    ...
    function getTrait(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "TRAIT", trait);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[9] = getTrait(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getPower(tokenId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Characters #',
                        toString(tokenId),
                        '", "description": "Project is randomized things generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret.", "image": "data:image/svg+xml;base64,',
    ...
    function mint() public payable nonReentrant {
        ...
        uint256 plebs = _plebs.current();
        require(tokenId <= 10000, "Token ID invalid");
        require(plebs <= 5000, "Already minted more than 5000 items");
    ...
    function mintWithLoot(uint256 lootId) public payable nonReentrant {
        ...
        require(!_usedLoots[lootId], "Loot already used.");
        _tokenIds.increment();
        _loots.increment();
        uint256 loots = _loots.current();
        require(loots <= 5000, "Already minted more than 5000 items");
        _safeMint(_msgSender(), _tokenIds.current());
        _usedLoots[lootId]=true;
    }

    function multiMintWithLoot(uint256[] memory lootIds) public payable nonReentrant {
        for (uint256 i = 0; i < lootIds.length; i++) {
            require(!_usedLoots[lootIds[i]], "Loot already used.");
    ...
    constructor() ERC721("Characters", "CHARS") Ownable() {}
}
