...
contract BeaverCrafterClubNFTs is ERC721Tradable {
    ...
    uint256 public constant earlySaleStart = 1633386995;

    /// @notice End of the early sale period (in Unix second)
    uint256 public constant earlySaleEnd = 1633388995;
    ...
    function vipMint(
        ...
        require(isWhitelisted(index, msg.sender, merkleProof), "Account is not whitelisted");
        for (uint256 i = 0; i < amount; i++) {
            tokenIdCount.increment();
            _mint(to, tokenIdCount.current());
        }
    ...
    function _mint(address to, uint256 tokenId) internal override {
        ...
        require(!to.isContract(), "No automatic minting allowed");
