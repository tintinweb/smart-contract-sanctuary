...
contract Exchange  is Ownable,StringPlay{
    ...
    function exchangeComput(uint _amount,uint _pairid,bool _reset) public view returns (uint) {
        uint _toAmount;
        uint _feeAmount;
        uint _realAmount;
        if(_reset){
            ...
            _realAmount = _amount*decimals/signExchangePair[_pairid].rate;
            if(signExchangePair[_pairid].scale){
                _feeAmount = _realAmount*(decimals - signExchangePair[_pairid].fee)/decimals;
            }else{
                _feeAmount = signExchangePair[_pairid].fee;
            }
            _toAmount = _realAmount + _feeAmount;
        }
        return _toAmount;
    }
