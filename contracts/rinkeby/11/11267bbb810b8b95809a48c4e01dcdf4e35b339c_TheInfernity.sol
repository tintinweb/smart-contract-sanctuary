...
contract TheInfernity is ERC721, Ownable {
    ...
    uint256 public constant guardianPrice = 0.01 ether; // Price per collectible
    uint256 public constant maxPurchase = 4; // Max 4 token per purchase
    uint256 public reserve = 40; // 40 max free mints for owner
    uint256 public constant maxSupply = 9999; // 4999 + 5000 Total Supply
    uint256 public constant maxFemales = 5000; // 5000 Priestess Supply
    uint256 public constant maxMales = 4999; // 4999 Priest Supply
    ...
    constructor() ERC721("The Infernity Test", "TIT") {}
    
    function withdraw() external onlyOwner {
        uint balance = address(this).balance;
        _withdraw(devAddress, balance.mul(2).div(100));
    ...
    function addToPresaleList(address[] calldata entries) external onlyOwner {
        for(uint256 i = 0; i < entries.length; i++) {
            ...
            require(!presalerList[entry], "DUPLICATED_ENTRY");
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        guardianProvenance = provenanceHash;
    }
    ...
    function mint(bytes32 hash, uint256 mqty, uint256 fqty, string memory g, bytes memory signature) external payable {
        ...
        require(femaleIndex.add(fqty) <= maxFemales, "EXCEED_FEMALE_SUPPLY");
        require(femalePurchases[msg.sender].add(fqty) <= 2, "EXCEED_FEMALE_LIMIT");
        require(maleIndex.add(mqty) <= maxMales, "EXCEED_MALE_SUPPLY");
        require(malePurchases[msg.sender].add(mqty) <= 2, "EXCEED_MALE_LIMIT");
        
        if (compareStrings(g, 'female')) {
            for (uint i = 0; i < fqty; i++) {
                _safeMint(msg.sender, femaleIndex);
                femaleIndex = femaleIndex.add(2);
                femalePurchases[msg.sender]++;
            }
        } else if (compareStrings(g, 'male')) {
            for (uint i = 0; i < mqty; i++) {
    ...
    function presaleMint(uint256 mqty, uint256 fqty, string memory g, bytes32 hash, bytes memory signature) external payable {
        uint256 qty = mqty+fqty;
        ...
        require(msg.value >= guardianPrice.mul(qty), "INCORRECT_ETH");
        require(femaleIndex.add(fqty) <= maxFemales, "EXCEED_FEMALE_SUPPLY");
        require(femalePurchases[msg.sender].add(fqty) <= 1, "EXCEED_FEMALE_LIMIT");
        require(maleIndex.add(mqty) <= maxMales, "EXCEED_MALE_SUPPLY");
        require(malePurchases[msg.sender].add(mqty) <= 1, "EXCEED_MALE_LIMIT");
