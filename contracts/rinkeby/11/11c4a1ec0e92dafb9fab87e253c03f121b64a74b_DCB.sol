...
contract DCB {
   ...
   function preBuyTickt(uint amount) public returns(bool isSuccess){
    ...
    function buyTicket(uint amount) public returns (bool isSuccess){
      ...
      uint256 balance = IERC20(usdtToken).balanceOf(msg.sender);
      require(balance >= amount, 'Balance less than transaction amount');
      bool isOk = IERC20(usdtToken).approve(address(this), amount);
      require(isOk,'approve was failed!');
      bool isTrans = IERC20(usdtToken).transferFrom(msg.sender, address(this), amount);
