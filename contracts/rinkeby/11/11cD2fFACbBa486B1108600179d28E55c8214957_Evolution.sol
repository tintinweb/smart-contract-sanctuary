...
// An allocation of 100 Evolution Puzzles are reserved for owner & future expansion packs

contract Evolution is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable {

    constructor() ERC721("EvolutionPuzzles", "EP") {}
    ...
    function tokenURI(uint256 tokenId) virtual override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Evolution Puzzles #', Strings.toString(tokenId), '", "description": "Evolution Puzzles", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"', ',"attributes":', atrrOutput, '}'))));
    ...
    function mint(uint256 tokenId) public nonReentrant {
        require(!_exists(tokenId) && tokenId > 0 && tokenId < 9901, "Evolution Puzzles id is invalid");
        randomiseAttributes(tokenId);
        _safeMint(msg.sender, tokenId);
    }

    function mintForOwner(uint256 tokenId) public nonReentrant onlyOwner {
        require(!_exists(tokenId) && tokenId > 9900 && tokenId < 10001, "Evolution Puzzles id is invalid");
        randomiseAttributes(tokenId);
        _safeMint(msg.sender, tokenId);
    }
