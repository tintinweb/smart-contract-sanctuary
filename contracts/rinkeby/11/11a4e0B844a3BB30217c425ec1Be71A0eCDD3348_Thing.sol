 ...
 */
contract Thing is ERC1155Tradable {
    ...
    mapping (uint256 => ThingInfo) thingInfo;

    constructor(address _proxyRegistryAddress)
        ERC1155Tradable(
            "The Thing",
            "THING",  
            "https://thethingbackend.herokuapp.com/api/thing/{id}",
            _proxyRegistryAddress
        ) {}

    function contractURI() public pure returns (string memory) {
        return "https://thethingbackend.herokuapp.com/api/contract/thing";
    }
    ...
    function mintTo(
        ...
        create(_to, _id, _quantity, "https://thethingbackend.herokuapp.com/api/thing/", "");
        thingInfo[_id] = ThingInfo({name:name, description:description, image:image, exturnal_url:exturnal_url});
        quantities[_id] = _quantity;
        _hashExists[_hash] = true;
        emit MintToken(_id, product_id);
    }

    function getInfo(uint256 token_id) public view returns (string memory, string memory, string memory, string memory, uint256)  {
        ThingInfo memory product = thingInfo[token_id];
        return (product.name, product.description, product.exturnal_url, product.image, quantities[token_id]);
    }

    function getAccessoryCount() public view returns(uint256 count) {
