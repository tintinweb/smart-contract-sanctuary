...
contract BadBear is ERC721, Ownable {
    ...
    address private MISHKA2 = 0x176DE1E7EDa994822dc7058dc46277aAa3721463;
    uint256 private m_MskMiniumHold = 500000 * 10**18;

    uint256 private m_PreMintPrice = 0.1 ether; // 0.1Eth
    uint256 private m_PublicMintPrice = 0.2 ether; // 0.1Eth
    ...
    constructor() ERC721("Bad Bears", "BB") {
    ...
    function totalSupply() public view returns (uint256) {
        return m_TokenIdCounter.current();
    }
    ...
    function reserve(address _address, uint256[] memory ids)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < ids.length; i++) {
            require(ids[i] <= MAX_SUPPLY, "Token Id is incorrect.");
            require(_exists(ids[i]), "Token id exists.");
            if (m_BurnList[ids[i]]) delete m_BurnList[ids[i]];

            _safeMint(_address, ids[i]);
        }
    }

    function reserve(address _address, uint256 _numberOfTokens)
        external
        onlyOwner
    {
    ...
    function mint(uint256 _numberOfTokens) public payable {
        ...
        require(_numberOfTokens > 0, "Can't mint");
        ...
        if (m_IsPublic) {
            ...
            require(
                ...
                "You are not in the white list or don't hold the minimum MSK amount."
            );
            require(
                msg.value >= m_PreMintPrice.mul(_numberOfTokens),
                "Ether value sent is not correct"
            );
        }
    ...
    function devMint(uint256 _numberOfTokens) external {
        require(m_DevWallet[msg.sender], "Address is not dev wallet");
