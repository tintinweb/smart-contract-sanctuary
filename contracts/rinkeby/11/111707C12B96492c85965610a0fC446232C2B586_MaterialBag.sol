...
interface ILootAirdrop {
    function claimForLootBag(uint256) external payable;
...
contract MaterialBag is ERC1155 {
    ...
    IERC721 lootBag  = IERC721(0xe088C7eAd29B578F058e9849Bc4488D76f815d49);
    ...
    function open(uint256 tokenId) external {
        lootBag.safeTransferFrom(msg.sender, address(this), tokenId);
    }

    /// @notice Claims an airdrop for a token owned by MaterialBag. The airdrop is then
    /// claimable by the owner of the reassembled pieces.
    function claimAirdropForMaterialBag(ILootAirdrop airdrop, uint256 tokenId) external payable {
        airdrop.claimForLootBag{value: msg.value}(tokenId);
    }

    /// @notice Allows you to claim an airdrop that has already been claimed by MaterialBag
    ...
    function claimAirdrop(ILootAirdrop airdrop, uint256 tokenId) external {
        require(lootBag.ownerOf(tokenId) == msg.sender, "you do not own the materialbag for this airdrop");
    ...
    function onERC721Received(
        ...
        require(msg.sender == address(lootBag));
        open(from, tokenId);
        return MaterialBag.onERC721Received.selector;
    }

    /// @notice Opens your Loot Bag and mints you 8 ERC-1155 tokens for each material
    /// in that bag
    ...
    function reassemble(uint256 tokenId) external {
        ...
        // 2. give back the bag
        lootBag.safeTransferFrom(address(this), msg.sender, tokenId);
    }
    ...
    */
    ...
    function contractURI() external pure returns (string memory) {
      string memory json = '{"name": "MaterialBag", "description": "MaterialBag lets you unbundle your Loot Bags into individual ERC1155 NFTs or rebundle items into their original Loot Bags."}';
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '"description" : ', '"MaterialBag lets you unbundle your Loot Bags into individual ERC1155 NFTs or rebundle items into their original Loot Bags.", ',
