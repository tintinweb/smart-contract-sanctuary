...
// Incremental changes (commit by commit) from the original to this version: https://github.com/liquidity-protocol/liquidity/pull/271
...
/*
 * On deployment a new Uniswap pool will be created for the pair USDQ/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of LQDY token,
 ...
 * in order to avoid getting LQDY tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either LQDY token contract is deployed, and therefore LQDY tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
        ...
        onlyOwner
    {
        ...
        _notifyRewardAmount(lqdyToken.getLpRewardsEntitlement(), _duration);
    ...
    function claimReward() public override {
        ...
        lqdyToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == lqdyToken.balanceOf(address(this)));
...
interface IUnipool {
    function setParams(address _lqdyTokenAddress, address _uniTokenAddress, uint256 _duration) external;
