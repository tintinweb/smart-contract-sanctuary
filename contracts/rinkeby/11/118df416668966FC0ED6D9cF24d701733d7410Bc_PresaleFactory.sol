...
contract PresaleFactory is IPresaleFactory, Ownable {
  ...
  */
   ...
   * @param _minBuy The minimum amount of stablecoin that could be used to buy in
   * @param _maxBuy The maximum amount of stablecoin that could be used to buy in
...
interface IPresaleFactory {
   ...
   * @param _maxBuy The maximum amount that could be used to buy in
   * @return uint256 The id of the presale
   * @return uint256 The address of the presale
   */
  function createPresale(
...
contract Presale is IPresale, Ownable {
   ...
   */
  mapping(Tier => uint256) public override tierMaxBuy;
  ...
  constructor(
    ...
    uint256 _endBlock, uint256 _softCap, uint256 _hardCap, uint256 _minBuy, 
    uint256 _maxBuy
  ) {
  ...
  function buy(uint256 _tokenAmount) external override payable {
    ...
    // Check if token amount is atmost as much as user's tier of max buy
    require(
      tokensBought[_msgSender()] <= tierMaxBuy[userTier].mul(price) 
      , "Presale::buy: Tokens bought should be less than maximum amount"
    );
  }
   ...
   */
  function _setTierMaxBuy() internal {
    tierMaxBuy[Tier.BRONZE] = maxBuy;
    tierMaxBuy[Tier.SILVER] = maxBuy.mul(2);
    tierMaxBuy[Tier.GOLD] = maxBuy.mul(3);
    tierMaxBuy[Tier.PLATINUM] = maxBuy.mul(45).div(10);
    tierMaxBuy[Tier.DIAMOND] = maxBuy.mul(9);
    tierMaxBuy[Tier.NONE] = 0;
  }
...
interface IPresale {
  ...
  /**
   * @notice The minimum amount of stablecoin that could be used to buy in
   * @return uint256 The amount in ETH
   */
  function minBuy() external view returns (uint256);

  /**
   * @notice The maximum amount of stablecoin that could be used to buy in
   * @return uint256 The amount in ETH
   */
   ...
   */
  function tierMaxBuy(Tier _tier) external view returns (uint256);
