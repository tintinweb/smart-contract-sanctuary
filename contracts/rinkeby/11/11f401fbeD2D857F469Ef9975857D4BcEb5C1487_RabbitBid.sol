...
// File: contracts\RabbitBid.sol

// RabbitBid - NFT Bit contract
...
contract RabbitBid is Ownable, ERC1155Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	constructor (address _tokenAddress, uint256 _fee, address _feeAddress) {	
		rabbitToken = IERC20(_tokenAddress);	
		feeAddress = _feeAddress;
		swapFee = _fee;
	}
	
    function setRabbitToken(address _address) external onlyOwner {
        require(_address != address(0x0), "invalid address");
		rabbitToken = IERC20(_address);
    }   
    ...
    function sell(uint256 _id, uint256 _amount) external {
		...
		uint256 token_amount = order.price.mul(_amount);
        uint256 orderer_balance = rabbitToken.balanceOf(orderer);
		require(orderer_balance >= token_amount, "insufficient token balance : orderer don't have enough tokens");

        IRabbitNFT nft = IRabbitNFT(orders[_id].collection);
		...
		if(swapFee > 0) {
			require(rabbitToken.transferFrom(orderer, feeAddress, token_amount.mul(swapFee).div(PERCENTS_DIVIDER)), "failed to transfer admin fee");
		}
		// transfer IgarataArt token to creator
		if(order.creatorFee > 0) {
			require(rabbitToken.transferFrom(orderer, creator, token_amount.mul(order.creatorFee).div(PERCENTS_DIVIDER)), "failed to transfer creator fee");
		}
		// transfer IgarataArt token to owner
		uint256 ownerPercent = PERCENTS_DIVIDER.sub(swapFee).sub(order.creatorFee);
		require(rabbitToken.transferFrom(orderer, msg.sender, token_amount.mul(ownerPercent).div(PERCENTS_DIVIDER)), "failed to transfer to owner");
