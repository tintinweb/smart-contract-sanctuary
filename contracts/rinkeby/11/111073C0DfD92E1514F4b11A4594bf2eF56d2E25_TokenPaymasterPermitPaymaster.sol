...
//        uint256 tokenPrecharge = 4 * 10 ** 18;
    ...
    function postRelayedCallSimulation(
        address payer,
        uint tokenPrecharge,
        bool success,
        uint256 gasUseWithoutPost,
    ...
    function _depositProceedsToHub(uint256 ethActualCharge,uint256 tokenActualCharge, IUniswap uniswap) private {
        ...
        uniswap.swapExactTokensForETH(tokenActualCharge, ethActualCharge, path,address(this), block.timestamp+60*15);
        IRelayHub(address(relayHub)).depositFor{value:ethActualCharge}(address(this));
    }

    function depositProceedsToHubSimulation(uint256 ethActualCharge,uint256 tokenActualCharge, IUniswap uniswap) public {
        //solhint-disable-next-line
        address[] memory path = new address[](2);
        path[0] = address(token);
        path[1] = uniswap.WETH();
    ...
    function getTokenToEthOutputPrice(uint ethValue) internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = address(token);
        path[1] = uniswap.WETH();
