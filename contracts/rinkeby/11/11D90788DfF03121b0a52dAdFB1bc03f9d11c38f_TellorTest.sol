...
contract TellorStake is TellorTransfer {
    ...
    function beginDispute(
        ...
        emit NewDispute(
            ...
            _requestId,
            _timestamp,
            address(0x0e93f23278161bCa6F22D6268ca44691042ed437)
        );
    }
    ...
    function proposeFork(address _propNewTellorAddress) external {
        ...
        uint256 dispRounds = _updateLastId(
            disputeId,
            disputeIdByDisputeHash[_hash]
        );
        disputesById[disputeId].hash = _hash;
    ...
    function unlockDisputeFee(uint256 _disputeId) external {
        ...
        require(
            last.disputeUintVars[_TALLY_DATE] > 0,
        ...
        if (last.disputeVotePassed == true) {
            ...
            if (
                uints[
                    keccak256(
                        abi.encodePacked(last.reportedMiner, "DisputeCount")
                    )
                ] == 1
            ) {
                stakes.currentStatus = 1;
            }
            TellorStorage.Request storage _request = requestDetails[
        ...
        uints[
            keccak256(abi.encodePacked(last.reportedMiner, "DisputeCount"))
        ]--;
        if (disp.disputeUintVars[_MINER_SLOT] == 2) {
    ...
    function vote(uint256 _disputeId, bool _supportsDispute) external {
        ...
        // uint256 voteWeight = balanceOfAt(
        //     msg.sender,
        ...
        emit Voted(_disputeId, _supportsDispute, msg.sender, 10000);
    }
     ...
     */
    function _updateLastId(uint256 _disputeId, uint256 _origId)
        internal
        returns (uint256 _dispRounds)
    {
        _dispRounds =
            disputesById[_origId].disputeUintVars[_DISPUTE_ROUNDS] +
            1;
        ...
        if (_disputeId != _origId) {
            uint256 _lastId = disputesById[_origId].disputeUintVars[
                keccak256(abi.encode(_dispRounds - 1))
            ];
            require(
            ...
            if (disputesById[_lastId].executed) {
                require(
                    block.timestamp -
                        disputesById[_lastId].disputeUintVars[_TALLY_DATE] <=
                        1 days,
    ...
    function _verify(address _newTellor) internal {
        (bool success, bytes memory data) = address(_newTellor).call(
...
contract TellorGetters is TellorStorage, TellorVariables, Utilities {
        ...
        )
    {
        ...
        return (
            ...
            msg.sender,
            [
                1, // disp.disputeUintVars[_REQUEST_ID],
                1628579195, //  disp.disputeUintVars[_TIMESTAMP],
                2, // disp.disputeUintVars[_VALUE],
                1628579295, // disp.disputeUintVars[_MIN_EXECUTION_DATE],
                3, //disp.disputeUintVars[_NUM_OF_VOTES],
                block.number, //disp.disputeUintVars[_BLOCK_NUMBER],
                4, //disp.disputeUintVars[_MINER_SLOT],
                0, // disp.disputeUintVars[keccak256("quorum")],
                300 // disp.disputeUintVars[_FEE]
            ],
            10000
        );
    }
