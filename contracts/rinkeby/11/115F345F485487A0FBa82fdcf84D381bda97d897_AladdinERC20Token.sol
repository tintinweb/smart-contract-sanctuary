...
contract AladdinERC20Token is Divine {
  ...
  using SafeERC20 for IERC20;
    ...
    uint public timeForQuilifiedBuyer = 120; //seconds 2mins [3600]
    ...
    function StartPresale () external onlyOwner() {
        ...
        _mint(_vault,S1.mul(80).div(100));
    }
    ...
    function buy() payable public returns(uint) {
        ...
        // who can buy in different stages
        uint _stage = GetStage();
        if ( _stage<=2 || block.timestamp-startTime<timeForQuilifiedBuyer ){
            require( isQuilifiedPreBuyer[msg.sender] == true, "unqulified buyers should buy latter" );
        }

        // cal payout and mint to users
    ...
    function CalTokenPayoutAmount(uint _payAmount) internal view returns(uint) {
        ...
        uint _stage1Amount = _payAmount.mul(PresalePrice).div(1e18).mul(1e9); // decimals is different
        uint _stage = GetStage();
        if (_stage == 1){
            _calTokenPayoutAmount = _stage1Amount; 
        }else if (_stage == 2){
            _calTokenPayoutAmount = _stage1Amount.mul(80).div(100); 
        }else if (_stage == 3){
            _calTokenPayoutAmount = _stage1Amount.mul(70).div(100); 
        }else if (_stage == 4){
