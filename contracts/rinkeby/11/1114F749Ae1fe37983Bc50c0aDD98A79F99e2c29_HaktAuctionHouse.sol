...
contract HaktAuctionHouse is IAuctionHouse, ReentrancyGuard, Ownable {
    ...
    function createAuction(
        ...
        // require(msg.sender == IERC721(tokenContract).getApproved(tokenId), "Caller must be approved or owner for token id");
     ...
     */
    function setAuctionApproval(uint256 auctionId, bool approved) external override auctionExists(auctionId) onlyOwner {
        require(auctions[auctionId].firstBidTime == 0, "Auction has already started");
        _approveAuction(auctionId, approved);
    }

    function setAuctionReservePrice(uint256 auctionId, uint256 reservePrice) external override auctionExists(auctionId) onlyOwner {
        require(auctions[auctionId].firstBidTime == 0, "Auction has already started");
    ...
    nonReentrant
    {
        ...
        require(auctions[auctionId].approved, "Auction must be approved");
        require(
