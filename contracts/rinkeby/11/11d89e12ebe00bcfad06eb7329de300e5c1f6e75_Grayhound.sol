...
//        && sender != uniswapV2Pair
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && overMinTokenBalance) {
            ...
            uint256 toCharityMarketing = contractTokenBalance.div(_charityMarketingFee.add(_liquidityFee)).mul(_charityMarketingFee);
            uint256 toLiquify = contractTokenBalance.sub(toCharityMarketing);

            swapAndLiquifyAndSend(toCharityMarketing, toLiquify);
        }
    ...
    function swapAndLiquifyAndSend(uint256 contractTokenBalanceToSend, uint256 contractTokenBalanceToLiquify) private lockTheSwap {
        // split the contract balance into halves
        uint256 half = contractTokenBalanceToLiquify.div(2);
        uint256 otherHalf = contractTokenBalanceToLiquify.sub(half);
        ...
        // swap tokens for ETH
        uint256 toSwapForEth = half.add(contractTokenBalanceToSend);
        swapTokensForEth(toSwapForEth); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 toLiquifyRatio = toSwapForEth.div(half);
        uint256 fromSwap = address(this).balance.sub(initialBalance);
        uint256 newBalance = fromSwap.div(toLiquifyRatio).mul(toSwapForEth);
