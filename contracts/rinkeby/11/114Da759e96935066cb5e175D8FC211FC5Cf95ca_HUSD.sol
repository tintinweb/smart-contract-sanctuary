    ...
    AccessControlUpgradeable
{
    ...
    function swap(
        ...
        require(
            isWhitelistedERC20(_from),
            "Swap:: _from ERC20 not whitelisted"
        );
        require(isWhitelistedERC20(_to), "Swap:: _to ERC20 not whitelisted");
        require(_amount > 0, "Swap:: Can not swap Zero tokens");
        require(
            _from == address(this) || _to == (address(this)),
            "Swap:: Invalid Pair"
        );
        require(_from == _to, "Swap:: _from & _to can not be same token");
        uint256 _fromDecimal = getTokenDecimals(_from);
        uint256 _toDecimal = getTokenDecimals(_to);
        uint256 amount = 0;
        ...
        if (_fromDecimal == _toDecimal) {
            ...
            decimalDiff = _toDecimal - _fromDecimal;
            amount = _amount / 10**decimalDiff;
        } else if (_fromDecimal > _toDecimal) {
            decimalDiff = _fromDecimal - _toDecimal;
            amount = _amount * 10**decimalDiff;
        }
        _swap(_from, _to, amount);
    }
    ...
    function _swap(
        ...
        IERC20Upgradeable(_to).safeTransferFrom(
            address(this),
            msg.sender,
            _amount
        );
