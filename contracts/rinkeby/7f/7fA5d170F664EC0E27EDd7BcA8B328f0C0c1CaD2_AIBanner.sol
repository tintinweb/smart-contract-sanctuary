...
contract AIBanner is ERC721URIStorage, Ownable {
    ...
    function mintBanner(string memory imageURI, string[] memory attributeArr)
        public
        payable
    {
    ...
    function formatTokenURI(
        ...
        string[] memory attributeArr
    ) private pure returns (string memory) {
        string memory baseURL = "data:application/json;base64,";
        string memory attrutributesStr = "";
        for (uint256 j = 0; j < attributeArr.length - 1; j++) {
            attrutributesStr = string(
                abi.encodePacked(attrutributesStr, attributeArr[j], ",")
            );
        }
        attrutributesStr = string(
            abi.encodePacked(
                "[",
                attrutributesStr,
                attributeArr[attributeArr.length - 1],
                "]"
            )
        );
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name": "Banner","description": "NFT AI Generated Banner!", "attributes":',
                                attrutributesStr,
                                ', "image":',
                                imageURI,
                                '"}'
