...
contract NFTERC1155Auction is ERC1155Holder, Ownable{
    ...
    address[] public feeRecipients = [0x0b40a188D28F8cF561Ad7B665dfb0f3D3b77BffF];
        ...
        notZeroAddress(_nftRecipient)
    {
        ...
        nftContractAuctions[_nftContractAddress][_tokenId][msg.sender] .nftRecipient = _nftRecipient;
        _makeBid(_nftContractAddress, _tokenId,_nftSellerAddress, _erc20Token, _tokenAmount);
    }
   ...
   function _resetAuction(NFT memory _nft)
    ...
    function _transferNftAndPaySeller(
       NFT memory _nft
        ...
        address _nftSeller = nftContractAuctions[_nft.nftContractAddress][_nft.tokenId][msg.sender]  .nftSeller;
        address _nftHighestBidder = nftContractAuctions[_nft.nftContractAddress][_nft.tokenId ][msg.sender].nftHighestBidder;
        address _nftRecipient = _getNftRecipient(NFT(_nft.nftContractAddress, _nft.tokenId, msg.sender));
        uint256 _nftHighestBid = nftContractAuctions[_nft.nftContractAddress][_nft.tokenId][msg.sender].nftHighestBid;
        ...
        uint256[] memory batchTokenIds = nftContractAuctions[_nft.nftContractAddress][_nft.tokenId][msg.sender].batchTokenIds;
        uint256 numberOfTokens = batchTokenIds.length;
        if (numberOfTokens > 0) {
            uint256[] memory batchAmounts = nftContractAuctions[_nft.nftContractAddress][_nft.tokenId][msg.sender]. batchAmounts;
            ...
            for (uint256 i = 0; i < numberOfTokens; i++) {
                nftOwner[_nft.nftContractAddress][batchTokenIds[i]][msg.sender] = address(0);
            }
        } else {
            uint256 _amount = nftContractAuctions[_nft.nftContractAddress][_nft.tokenId][msg.sender].amount;
    ...
    function settleAuction(address _nftContractAddress, uint256 _tokenId)
        external
    {
        _isAuctionOver(NFT(_nftContractAddress, _tokenId, msg.sender));
        _transferNftAndPaySeller(NFT(_nftContractAddress, _tokenId , msg.sender));
