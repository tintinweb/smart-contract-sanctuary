 ...
 * Aoki Redeem
 */
contract AokiRedeemSequential is AdminControl, ERC721RedeemBase, ICreatorExtensionTokenURI {
    ...
    mapping(address => mapping(uint256 => bool)) _redeemedTokens;


    constructor(address creator_,address ethercards, uint256 ecMax, address[] memory contractz) ERC721RedeemBase(creator_, 1, 250) {
        allowed[ethercards] = ecMax;
        require(contractz.length == 2, "Don't forget the contracts!");
        for (uint j = 0; j < contractz.length; j++) {
            allowed[contractz[j]] = 250;
        }
    }

    function cando(address token) internal returns (bool) {
        uint256 balance = allowed[token];
        if (balance > 0) {
            allowed[token] = balance - 1;
            return true;
        }
        return false;
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721RedeemBase, AdminControl, IERC165) returns (bool) {
    ...
    function redeem(address tokenContract, uint256 tokenId) public {
        ...
        require(cando(tokenContract) && !_redeemedTokens[tokenContract][tokenId], "Invalid token");
