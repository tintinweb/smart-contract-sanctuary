...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    */
    ...
    function _getTValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            // amount after fee
            values.tTransferAmount = values.amount - values.tBurnFee - values.tRewardFee - values.tLiquifyFee - values.tAdminFee;
        }
    ...
    function _getRValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            values.rTransferAmount = values.rAmount - values.rBurnFee - values.rRewardFee - values.rLiquifyFee - values.rAdminFee;
        }
    ...
    */
    ...
    function enableAutoSwapAndLiquify(uint32 taxLiquify_, uint32 taxLiquifyDecimals_, address routerAddress, uint256 minTokensBeforeSwap_) public onlyOwner {
        ...
        excludeAccountFromReward(_uniswapV2Pair);
...
contract XXX is ExtendedReflections {

    uint256 private _tokenSupply = 1000000;

    uint32 private _taxBurn = 1;
    uint32 private _taxReward = 2;
    uint32 private _taxAdmin = 3;
    uint32 private _taxLiquify = 4;
    ...
    constructor (address _routerAddress) ExtendedReflections("xxx", "XXX", _decimals, _tokenSupply) {
