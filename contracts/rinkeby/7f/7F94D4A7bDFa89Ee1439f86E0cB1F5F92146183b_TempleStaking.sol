 ...
 */
contract TempleStaking is Ownable, ITemplePool {
    using ABDKMath64x64 for int128;
    
    struct User {
        // Amount of temple staked
        uint256 Deposited;

        // Accumalted reward factor at the point of staking as an ABDKMath64x64
        // Required to calculated compounded reward as 
        // Deposited * accRewardsFactor / AccumulationFactorAtStake
        int128 AccumulationFactorAtStake;

        // Every time a user stakes, we capture their accumulated rewards
        // at point of staking. Denominated in TEMPLE
    ...
    struct LockedTemple {
        ...
        // Block at which the rewards are locked until
        uint256 LockedUntil;
    }

    // used for mint and stake
    mapping(address => LockedTemple) public locked;
    ...
    // the block this contract was created, used to work out what epoch we are in
    uint256 public firstBlock;

    event StakeCompleted(address _staker, uint256 _amount, uint256 _lockedUntil);
    event PoolUpdated(uint256 _epochsProcessed, uint256 _currentEpoch, uint256 _accumulationFactor);
    ...
    function setEpy(uint256 _numerator, uint256 _denominator) external onlyOwner {
        updatePool();
        epy = ABDKMath64x64.fromUInt(1).add(ABDKMath64x64.fromUInt(_numerator).div(ABDKMath64x64.fromUInt(_denominator)));
    }
    ...
    /** Sets exit queue contract */
    function setExitQueue(address _EXIT_QUEUE) external onlyOwner {
        EXIT_QUEUE = ExitQueue(_EXIT_QUEUE);
    }
    ...
    function pendingRewards(address _staker) external view returns(uint256) {
        ...
        uint256 _nUnupdatedEpochs = ((block.number - firstBlock) / epochSize) - lastUpdatedEpoch;
        int128 _accumulationFactor = accumulationFactor.mul(epy.pow(_nUnupdatedEpochs));

        return ABDKMath64x64.fromUInt(user.AccumulatedRewards + user.Deposited)
                        .mul(_accumulationFactor)
                        .div(user.AccumulationFactorAtStake)
                        .sub(ABDKMath64x64.fromUInt(user.Deposited))
                        .toUInt();
    }
    ...
    function updatePool() public {
        ...
        // number of unupdated epochs from last call
        uint256 _nUnupdatedEpochs = _currentEpoch - lastUpdatedEpoch;

        accumulationFactor = accumulationFactor.mul(epy.pow(_nUnupdatedEpochs));
        lastUpdatedEpoch = _currentEpoch;

        emit PoolUpdated(_nUnupdatedEpochs, _currentEpoch, accumulationFactor.mul(10000).toUInt());
    }

    /** helper to increase a users staked tokens */
    function _stake(address _staker, uint256 _amount) internal {
        ...
        if (user.Deposited != 0) {
            user.AccumulatedRewards = ABDKMath64x64.fromUInt(user.AccumulatedRewards + user.Deposited)
                        .mul(accumulationFactor)
                        .div(user.AccumulationFactorAtStake)
                        .sub(ABDKMath64x64.fromUInt(user.Deposited))
                        .toUInt();
        }
        ...
        user.AccumulationFactorAtStake = accumulationFactor;
    }

    /** Stake temple with optional lock-in */
    ...
    /** stake temple with lock-in period */
    function stakeWithLockIn(address _staker, uint256 _amount, uint256 _lockinPeriod) external {
        LockedTemple storage lockedTemple = locked[_staker];

        // Reset if past the lock-in period
        if (block.number > lockedTemple.LockedUntil) {
            lockedTemple.Amount = 0;
            lockedTemple.LockedUntil = 0;
        }

        lockedTemple.Amount += _amount;
        uint256 lockedUntil = (currentEpoch() + _lockinPeriod) * epochSize + firstBlock;
        if (lockedTemple.LockedUntil < lockedUntil) {
            lockedTemple.LockedUntil = lockedUntil;
        }

        _stake(_staker, _amount);

        SafeERC20.safeTransferFrom(TEMPLE, msg.sender, address(this), _amount);
        emit StakeCompleted(_staker, _amount, lockedTemple.LockedUntil);
    }

    /**
     * Unstake a given amount for a user.
     *
     * It's implemented as a 100% unstake, and re-stake of the balance (if any)
     */
    function unstake(uint256 _amount) external {        
        User storage user = userDetails[msg.sender];
        LockedTemple storage lockedTemple = locked[msg.sender];

        require(user.Deposited > 0, "User has no stake");

        // Reset user state. Expect user.Deposited and user.AccumulatedRewards
        // to be the user's temple balance attributable to that user
        _stake(msg.sender, 0);

        uint256 _maxUnstakeable = user.Deposited + user.AccumulatedRewards;

        if (lockedTemple.LockedUntil > block.number) {
            _maxUnstakeable -= lockedTemple.Amount;
        }

        if (_amount > _maxUnstakeable) {
            _amount = _maxUnstakeable;
        }

        // book keeping + join exit queue if _amount is > 0
        if (_amount > 0) {
            user.Deposited += user.AccumulatedRewards;
            user.Deposited -= _amount;
            user.AccumulatedRewards = 0;

            SafeERC20.safeIncreaseAllowance(TEMPLE, address(EXIT_QUEUE), _amount);
            EXIT_QUEUE.join(msg.sender, _amount);
        }

        emit UnstakeCompleted(msg.sender, _amount);    
    }

    function reval() public pure override returns (uint256) {
      return 0; // no DAI held so no value reported back for mark to market purposes
    }
}
...
contract ExitQueue is Ownable {
    ...
    event JoinQueue(address _user, uint256 _amount);    
    event Withdrawal(address _user, uint256 _amount);    
