...
contract DROTokenTrust is IDROTokenTrust, TokenAllowance {
    ...
    function etherBalanceOf(address account) public view override returns (uint) {
        return _balances[account][address(0)];
    }

    function tokenBalanceOf(address account, address tokenAddress) public view override returns (uint) {
...
interface IDROTokenTrust {
    function etherBalanceOf(address account) external view returns (uint256);
    function tokenBalanceOf(address account, address tokenAddress) external view returns (uint256);
