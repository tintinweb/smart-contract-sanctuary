...
contract Coin98Swap is Ownable {
  ...
  function configureFee(
   ...
   function swapExactTokensForTokens(
        ...
        // Get fee from user
        require(swapToken.transferFrom(msg.sender, address(this), swap_fee),"C98Swap: Fee not enough");
        swapRouter.swapExactTokensForTokens(amountIn,amountOutMin,path,to,deadline);
        total_fee = total_fee.add(swap_fee);
        total_swap = total_swap.add(1);
   }
    ...
    function swapTokensForExactTokens(
        ...
        IUniswapV2Router02 swapRouter = IUniswapV2Router02(amm_address);
        uint256 swap_fee = calFee(amountOut);
        IERC20 swapToken = IERC20(path[0]);
        // Get fee from user
        require(swapToken.transferFrom(msg.sender, address(this), swap_fee),"C98Swap: Fee not enough");
        swapRouter.swapExactTokensForTokens(amountOut,amountInMax,path,to,deadline);
        total_fee = total_fee.add(swap_fee);
        total_swap = total_swap.add(1);
   }
    ...
    function swapExactETHForTokens(
        ...
        swapRouter.swapExactETHForTokens{ value: msg.value.sub(swap_fee) }(amountOutMin,path,to,deadline);
        total_fee = total_fee.add(swap_fee);
        total_swap = total_swap.add(1);
   }
