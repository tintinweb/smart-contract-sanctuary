...
contract AcyV1Arbitrage is IAcyV1Arbitrage{
    ...
    function swap(uint amountIn,address inputToken,address outputToken) payable external {
       ...
       // require(msg.sender != UniswapV2Library.pairFor(factory, inputToken, outputToken), "Unauthorized");
        IERC20 token = IERC20(inputToken);
        //token.approve(address(sushiRouter), amountIn);
        
        // require(token.approve(address(this), amountIn), 'approve failed.');
        //uint amountRequired = UniswapV2Library.getAmountsIn(factory, amountToken, arr)[0];
        // uniswapRouter02.swapExactTokensForTokensSupportingFeeOnTransferTokens(
        //     amountIn,
        ...
        /* 20210723*/
        // IERC20(address(inputToken)).approve(address(this), amountIn);
        require(token.approve(address(this), amountIn), 'approve failed.');
