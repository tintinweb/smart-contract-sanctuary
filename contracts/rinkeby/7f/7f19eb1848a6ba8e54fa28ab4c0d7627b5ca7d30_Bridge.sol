...
contract Bridge{
    ...
    function receiveOrderIndex(address receiver, uint256 amount, uint256 fee, uint256 claimingFee, uint256 index, address tokenAddressFrom) external view returns(uint256) {
        return _receiveOrderIndex[receiver][amount][fee][claimingFee][index][tokenAddressFrom];
    }
    ...
    function claim(address receiver, uint256 amount, uint256 fee, uint256 claimingFee, uint256 index, address tokenAddressFrom) external {
        ...
        if(_processed[orderIndex]){
            ...
            require(signTransaction(orderIndex, msg.sender, receiver, amount, fee, tokenAddressFrom));
                if(_mainContractOfThat[tokenAddressFrom] != address(0)){ //sending from the parallel contract back to the main
                    IERC20 token = IERC20(_mainContractOfThat[tokenAddressFrom]);
                    token.transfer(receiver, amount);
                }
