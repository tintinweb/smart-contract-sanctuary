...
contract KRAP is ERC20, Ownable {
    ...
    function setIkrapContractAddress(address _contractAddress) external onlyOwner {
    ...
    function swapTokens(address to, uint amount) public onlyIkrap {
        require(ikrapContractAddress != address(0), "IKRAP contract address has not been set");
        require(to != address(0), "Invalid address");
        require(amount > 0, "Can't swap 0 tokens");

        transferFrom(msg.sender, to, amount);
    }
}
// File: IKRAP/contracts/IKRAP.sol
...
contract IKRAP is ERC20, Ownable {
    ...
    mapping (uint => uint) public supplyPerRound;
    // The release rate for each round (in percentage)
    ...
    address private krapContractAddress;

    constructor(
        string memory name,
        string memory symbol,
        uint[] memory _maxSupplyPerRound,
        uint[] memory _releaseRatePerRound

    ) ERC20(name, symbol) {
        require(_releaseRatePerRound.length == 3 && _maxSupplyPerRound.length == 3, "Must have 3 funding rounds");
        uint initialSupply = 0;

        for (uint i; i < _releaseRatePerRound.length; i++) {
    ...
    function setKrapContractAddress(address _contractAddress) external onlyOwner {
        krapContractAddress = _contractAddress;
    }
    ...
    function setAllocation(uint round, uint amount, address to) external onlyOwner {
        ...
        require(amount + supplyPerRound[round] < maxSupplyPerRound[round], "Exceeds maximum supply");

        supplyPerRound[round] += amount;
    ...
    function releaseTokensHelper(uint round, address[] memory addresses, mapping (address => uint) storage allocation) private onlyOwner {
        for (uint i; i < addresses.length; i++) {
            address to = addresses[i];
            uint totalAllocation = allocation[to];
            uint amount = totalAllocation * releaseRatePerRound[round] / 100;
    ...
    function releaseTokens(uint round) external onlyOwner {
        ...
        if (round == 1) {
            releaseTokensHelper(round, addressesForRound1, allocationForRound1);
        } else if (round == 2) {
            releaseTokensHelper(round, addressesForRound2, allocationForRound2);
        } else if (round == 3) {
            releaseTokensHelper(round, addressesForRound3, allocationForRound3);
        }
    ...
    function swapTokens(uint amount) public {
        require(krapContractAddress != address(0), "KRAP contract address has not been set");
        require(amount > 0, "Can't swap 0 tokens");
        require(balanceOf(msg.sender) >= amount, "Not enough tokens");

        KRAP(krapContractAddress).swapTokens(msg.sender, amount);
