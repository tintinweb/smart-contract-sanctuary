...
contract LoanMarket is LoanMarketBase {
    ...
    */
    ...
    function tokenLoanOfferAdjusted(
        ...
        require(_isLoanInactive(_loanIdAdjusted), "GLM, Loan is Active, can't be adjusted");
        ...
        for(uint256 index =  0 ; index < loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts.length ; index ++){
            collatetralInBorrowed+=  this.getTokenPriceInBorrowed(_newBorrowStableCoin, loanOffersToken[_loanIdAdjusted].stakedCollateralTokens[index], loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts[index]);
        }
        uint256 ltv  = this.calculateLTV(loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts, loanOffersToken[_loanIdAdjusted].stakedCollateralTokens, _newBorrowStableCoin, _newLoanAmountBorrowed);
        uint256 maxLtv = this.getMaxLtv(collatetralInBorrowed, msg.sender);
    ...
    function tokenloanOfferCancel(uint256 _loanId) public {
        ...
        require(_isTokenLoanIDExists(_loanId), "GLM, Loan not existed, Loan can't be cancel");
        require(_isLoanInactive(_loanId), "GLM, can't be cancel");
    ...
    function activateLoanFullToken(uint256 _loanId, uint256 _stableCoinAmount) public {
        ...
        //TODO keep the APYFEE  to govworld  before  transfering the stable coins to borrower.
        ...
        loanDetailsToken.loanStatus = LoanData.LoanStatus.ACTIVE;
...
abstract contract LoanMarketBase is ILoanMarket {
    ...
    function _getAPYFee(uint256 _loanId) internal view returns(uint256) {
        ...
        // APY Fee Formula
        uint256 apyFee = ((loanDetailsToken.loanAmountInBorrowed.mul(loanDetailsToken.apyOffer).div(100)).div(365)).mul(loanDetailsToken.termsLengthInDays).div(1e18);
