...
contract TheLabyrinth is ERC721, ReentrancyGuard, Ownable {
    ...
    address private stringifyDoorContractAddress = 0xC50f0e63452d80B51e0928040C7a02b7D2F0475b;

    mapping(uint256 => uint256) private usedDoors;
    mapping(uint256 => uint256) private whiteDoors;

    using Counters for Counters.Counter;
    Counters.Counter private _triangleSupply;
    Counters.Counter private _hexagonSupply;
    Counters.Counter private _octagonSupply;
    Counters.Counter private _decagonSupply;
    Counters.Counter private _dodecagonSupply;
    ...
    function claim(uint256[] memory doors, uint256[5] memory tokenTypes) public nonReentrant  {
        ...
        require((_triangleSupply.current() + tokenTypes[0]) <= maxSupplyByType[0], "The mint would exceed max supply of triangles.");
        require((_hexagonSupply.current() + tokenTypes[1]) <= maxSupplyByType[1], "The mint would exceed max supply of hexagons.");
        require((_octagonSupply.current() + tokenTypes[2]) <= maxSupplyByType[2], "The mint would exceed max supply of octagons.");
        require((_decagonSupply.current() + tokenTypes[3]) <= maxSupplyByType[3], "The mint would exceed max supply of decagons.");
        require((_dodecagonSupply.current() + tokenTypes[4]) <= maxSupplyByType[4], "The mint would exceed max supply of dodecagons.");
        ...
        for (uint i = 0; i < tokenTypes[0]; i++) {
            _triangleSupply.increment();
            _triangleClaimed.increment();
            totalSupply.increment();
            whiteMazesSupply.increment();
            claimed.increment();

            _safeMint(_msgSender(),  _triangleSupply.current());
            emit TokenMinted(_msgSender(), _triangleSupply.current());
        }

        // Mint hexagons
        for (uint i = 0; i < tokenTypes[1]; i++) {
            _hexagonSupply.increment();
            _hexagonClaimed.increment();
            totalSupply.increment();
            whiteMazesSupply.increment();
            claimed.increment();

            _safeMint(_msgSender(),  1500 + _hexagonSupply.current());
            emit TokenMinted(_msgSender(), 1500 + _hexagonSupply.current());
        }

        // Mint octagons
        for (uint i = 0; i < tokenTypes[2]; i++) {
            _octagonSupply.increment();
            _octagonClaimed.increment();
            totalSupply.increment();
            whiteMazesSupply.increment();
            claimed.increment();

            _safeMint(_msgSender(),  2700 + _octagonSupply.current());
            emit TokenMinted(_msgSender(), 2700 + _octagonSupply.current());
        }

        // Mint decagons
        for (uint i = 0; i < tokenTypes[3]; i++) {
            _decagonSupply.increment();
            _decagonClaimed.increment();
            totalSupply.increment();
            whiteMazesSupply.increment();
            claimed.increment();

            _safeMint(_msgSender(),  3600 + _decagonSupply.current());
            emit TokenMinted(_msgSender(), 3600 + _decagonSupply.current());
        }

        // Mint dodecagons
        for (uint i = 0; i < tokenTypes[4]; i++) {
            _dodecagonSupply.increment();
            _dodecagonClaimed.increment();
            totalSupply.increment();
            whiteMazesSupply.increment();
            claimed.increment();

            _safeMint(_msgSender(),  4200 + _dodecagonSupply.current());
            emit TokenMinted(_msgSender(), 4200 + _dodecagonSupply.current());
        }
    ...
    function getSupplyPerType() public view returns (uint256[5] memory) {
        return [_triangleSupply.current(), _hexagonSupply.current(), _octagonSupply.current(), _decagonSupply.current(), _dodecagonSupply.current()];
    }
