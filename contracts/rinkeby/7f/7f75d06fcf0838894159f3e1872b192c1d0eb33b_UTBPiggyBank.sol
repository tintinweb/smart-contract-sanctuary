...
contract UTBPiggyBank is UTBBase {
    using SafeMath for uint256;
        ...
        )
    {
        ...
        limits[Rarity.COMMON] = 1000;
        limits[Rarity.RARE] = 200;
    ...
    function breakOut(uint256 tokenId)
        public
        onlyNFTOwner(tokenId)
        onlyStatus(tokenId, Status.NORMAL)
    {
        setTokenState(tokenId, uint8(Status.BROKEN));
        uint256 tokenAsset = tokenAssets[tokenId];
        uint256 points = feePoints[assets[tokenAsset].rarity].mul(25).div(100);
        point.mint(msg.sender, points);
    }

    function piggyETH(uint256 tokenId) public view returns (uint256 amount) {
        if (tokenState(tokenId) == uint8(Status.BROKEN)) {
    ...
    function piggyERC20s(uint256 tokenId)
        public
        view
    ...
    function piggyERC20Amount(uint256 tokenId, address token)
        public
        view
    ...
    function piggyERC721s(uint256 tokenId)
        public
        view
        returns (address[] memory tokens)
    {
    ...
    function piggyERC721Ids(uint256 tokenId, address token)
        public
        view
    ...
    function piggyERC1155s(uint256 tokenId)
        public
        view
        returns (address[] memory tokens)
    {
    ...
    function piggyERC1155Ids(uint256 tokenId, address token)
        public
        view
    ...
    function claimDeposits(uint256 tokenId)
        ...
        override
        onlyNFTOwner(tokenId)
        onlyStatus(tokenId, Status.BROKEN)
    {
