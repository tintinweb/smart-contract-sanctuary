...
contract Manager is Ownable, IERC721Receiver {
    ...
    uint public constant MINIMUM_TIME_TO_EXIT = 180;
    ...
    function setGoldHunter(address _goldHunter) external onlyOwner {
        testToken = ITestToken(_goldHunter);
    }
    ...
    function addTokensToStake(address account, uint16[] calldata tokenIds) external {
        require(account == msg.sender || msg.sender == address(testToken), "You do not have a permission to do that");

        for (uint i = 0; i < tokenIds.length; i++) {
            if (msg.sender != address(testToken)) {
