...
contract FAPP is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	0x661a3b8a02E70e3b4E0623C3673e78F0C6A202DD,
	0xcb0d3a38aF2837Fa0E4360a8785deF6905d38231,
	0x9C7F33E860cDE7182C89daD1E32e945b318ee05c
	];
	uint256[] private _founderPercents = [7, 15, 78];

	uint256 private constant PHALLUS_MAX_AMOUNT = 8888;
	uint256 private constant TEAM_FUND = 1000;
	uint256 private PHALLUS_PER_TX = 15;
	uint256 public PHALLUS_PRICE = 0.08 ether;
	uint256 private constant PHALLUS_TIME_FOR_GROWN = 14 days;
	...
	function mintPresaleMember() external payable nonReentrant() {
		...
		require(totalSupply() + amount <= PHALLUS_MAX_AMOUNT, "Mint would exceed max supply of tokens");
		require(mintedPresaleCount + amount <= TEAM_FUND, "Mint would exceed max pre-sale tokens");
		require(msg.value == amount*PHALLUS_PRICE, "You didn't send the right amount of eth");

		_mintMultiple(msg.sender, amount);
		mintedPresaleCount += amount;
		_availableClaims[msg.sender] = 0;
	}
	...
	function setPercent(uint256 newPercent, uint256 index) public onlyOwner {
		...
		require(percentCount() <= 100, "Wrong percent");
		emit SetNewPercent(founders[index], newPercent);
	}

	function setPhallusPrice(uint256 newPrice) public onlyOwner {
		require(newPrice > 0, "Wrong price");
		emit SetNewValue("PHALLUS_PRICE", PHALLUS_PRICE, newPrice);
		PHALLUS_PRICE = newPrice;
	}

	function setPhallusPerTx(uint256 newPerTx) public onlyOwner {
	...
	function percentCount() private view returns(uint256 _count) {
		uint256 length = _founderPercents.length;
	...
	function getIpfsTokenUri(uint256 tokenId) external view returns (string memory) {
		...
		return string(abi.encodePacked(tokenURIPrefix, _ipfsHash[tokenId]));
	}
