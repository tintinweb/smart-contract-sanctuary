...
contract Jukebox is IJukebox, IERC721Receiver, ReentrancyGuard {
    event OrderCreated(
        address validator,
    ...
    function addOrder(
        ...
        if (!token.hasRole(token.MINTER_ROLE(), msg.sender)) {
        ...
        bytes32 salt = keccak256(msg.data);
    ...
    function disableOrder(bytes32 salt) public {
        if (orders[salt].disabled) {
            revert OrderAlreadyDisabledError();
        }
        ERC20PresetMinterPauser token = ERC20PresetMinterPauser(
            orders[salt].tokenAddress
        );
        if (!token.hasRole(ORDER_DISABLER_ROLE, msg.sender)) {
            revert MissingOrderDisablerRoleError();
        }
        orders[salt].disabled = true;
    ...
    {
        order = orders[salt];
        if (order.disabled || order.expirationTime <= block.timestamp) {
    ...
    function onERC721Received(
        ...
        token.mint(from, order.tokensPerTribute);
    ...
    function batchFill(bytes32 salt, ERC721Grouping[] memory erc721Groupings)
        public
        nonReentrant
    {
        Order storage order = getValidOrder(salt);
        IERC721Validator validator = IERC721Validator(order.validator);
        ERC20PresetMinterPauser erc20Token = ERC20PresetMinterPauser(
            order.tokenAddress
        );
        uint256 tributeCounter = 0;
        for (uint256 i = 0; i < erc721Groupings.length; i++) {
            address erc721Address = erc721Groupings[i].tokenAddress;
            IERC721 erc721 = IERC721(erc721Address);
            uint256[] memory ids = erc721Groupings[i].tokenIds;
            tributeCounter += ids.length;
            for (uint256 j = 0; j < ids.length; j++) {
                uint256 id = ids[j];
                if (!validator.meetsCriteria(erc721Address, id)) {
                    revert InvalidNftError();
                }
                ...
                emit OrderFilled(salt, msg.sender, erc721Address, id);
            }
        }
        order.fills -= tributeCounter;
        // Payout to order filler
        erc20Token.mint(msg.sender, order.tokensPerTribute * tributeCounter);
    }
}
