...
interface IPapr {
function getfeesTaxRate(uint256 index) external view returns(uint256);
...
contract Escrow is Migratable {
    ...
    uint256 public maxPriceTaxRates;
    uint256 public minPriceTaxRates;
    ...
    isInitializer("Escrow", "0.1.3") {
        ...
        escrowId = 0;
        maxPriceTaxRates = 15e17;
        minPriceTaxRates = 6e17;
        paprMultiplierStepExpansion = 10e16;
     ...
     * Calculates the DAI value equivalent of the deposited PAPR tokens and stores this to hedge the escrow later
    ...
    function getPaprTaxRate() private returns (uint256) {
     return IPapr(escrowToken).getfeesTaxRate(10);
   }

    function _countTimesOverCents() private returns (uint256) {
    ...
    returns (uint256) {
        ...
        require(escrowToken.transferFrom(_client, address(this), _amount), "Client must have authorisation and balance to transfer PAPR");
        // uint256 tax = getPaprTaxRate();
        // uint256 _computeAmount = _amount.div(tax);
        // uint256 _amountAfterBurn = _amount.sub(_computeAmount);
        // (bool success) = escrowToken.delegatecall(abi.encodeWithSignature("transfer(address recipient, uint256 amount)", address(this), _amount));
        // require(success);
    ...
    /**
     * @dev Completes the escrow, calculating the amount of PAPR to pay out based on the DAI value
    ...
    returns (bool) {
        ...
        require(escrowToken.transfer(escrows[_escrowId].provider, escrows[_escrowId].paidToProviderAmount), "Escrow must hold enough PAPR for payout");
    ...
    /**
     * @dev Get the total amount of PAPR to pay out for the job, based on the DAI value
     * @param _escrowId Id of the escrow to complete
     * @return uint256 amount of PAPR
     */
    function getTotalPayoutPAPR(uint _escrowId) 
    internal
    view 
    returns (uint256) {
        //uint256 busdValue = escrows[_escrowId].totalValueDai;
        uint256 totalPayoutPAPR = escrows[_escrowId].amount;
        //uint256 totalPayoutPAPR = busdValue.mul(escrowAmount);

        require(totalPayoutPAPR > 0, "Oracle must return a non zero payout");
     ...
     * Pay out 100% of the DAI value in PAPR back to the client
     * @param _escrowId Id of the escrow to complete
     * @return bool success
     */
