...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "VokenV1"; //dont forget; n the bottom of the code make sure it looks like this: contract [yourtokenname] is VokenToken
    string internal constant SYMBOL = "VKNv1";

    uint16 internal constant FEES_DIVISOR = 10**3;
    uint8 internal constant DECIMALS = 3; //FIXME: change back to 18
    uint256 internal constant ZEROES = 10**DECIMALS;

    uint256 private constant MAX = ~uint256(0); // OK; max value of an uint256
    uint256 internal constant TOTAL_SUPPLY = 10_000 * ZEROES; // total supply FIXME: change back to 10_000_000
     ...
     * See: `Liquifier`
     */
    uint256 internal constant thresholdSwapToLiquidity = TOTAL_SUPPLY / 10_000; // 0.01% of the total supply (1000 vokens)

    // --------------------- Fees Settings ------------------- //
    address internal rndAddress =
        0x1bD55f1D3390F0668829694F18993F5b5fa67C8E;
    address internal fundAddress = 0x552135F8C23d57f5FB56b1FA31Ae880Da238cE17;
    address internal controlledBurnAddress = 0x40A82A87C5030a12020C4ca614D14F5f06AAc85F;
    ...
    function _addFees() private {
        ...
        _addFee(FeeType.ExternalToETH, 50, fundAddress);
        _addFee(FeeType.Liquidity, 40, address(this));
        _addFee(FeeType.ExternalToETH, 40, rndAddress);
    }
...
abstract contract Liquifier is Ownable, Manageable {
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        (uint256 tokenAmountSent, uint256 ethAmountSent, uint256 liquidity) = _router
            ...
            owner(), //FIXME: can I put the contract address?
            block.timestamp
        );

        // fix the forever locked BNBs as per the certik's audit TODO CHECK "withdrawLockedEth"
        /**
    ...
    Tokenomics
{
    ...
    mapping(address => mapping(address => uint256)) internal _allowances; //whutisthis
    ...
    // FIXME: don't know if it's here, but be able to withdraw fund from contract to somewhere on order of manager
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
...
contract VokenTokenV1 is VokenToken {
