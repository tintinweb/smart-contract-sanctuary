...
contract BitConnect {
    ...
    internal
    {
        ...
        for(uint i=0; i<9; i++)
        {
            ...
            uint256 value = _currentPrice*tokenBalanceLedger_[_idToDistribute];
            value += _currentPrice*totalStake[_idToDistribute];
            uint256 _holdingLevel1 = level1Holding_[_idToDistribute]*_currentPrice;
            uint pct;
            pct = percent_[i];
            if(_idToDistribute != address(0) && value >= (1746 * (10** decimals)) && _holdingLevel1 >= (holding_[i]) )
            {
                if(eligible(i+1, _idToDistribute))
                {
                    ...
                    _idToDistribute = _idToDistribute;
