...
contract OlympusBondDepository is Ownable {
    ...
    function _deposit( 
        ...
        // slippage protection
        require( maxPrice_ >= price, "More than max price" );

        IERC20( DAI ).safeTransferFrom( msg.sender, address(this), amount_ );
        uint value = amount_.div( 1e9 );

        uint payout = bondInterestFor( value, price );
        ...
        uint index = depositorInfo[ depositor_ ].lastIndex;
        // Store depositor info
        depositorInfo[ depositor_ ].bonds[ index ] = Bond({
        ...
        depositorInfo[ depositor_ ].lastIndex = index++;
        return true;
    }
    ...
    function redeem() external returns ( bool ) {
        Depositor storage info = depositorInfo[ msg.sender ];
        
        for ( uint i = info.firstIndex; i < info.lastIndex; i++ ) {
            Bond memory bond = info.bonds[ i ];

            if ( bond.vested && i == info.firstIndex ) { // increment first index if first bond is vested
                depositorInfo[msg.sender].firstIndex = info.firstIndex++;
            } else if ( !bond.vested ) {
                uint percentVested = _calculatePercentVested( msg.sender, i );

                if ( percentVested >= 10000 ) { // if fully vested
                    IERC20( OHM ).transfer( msg.sender, bond.payoutRemaining );
                    totalDebt = totalDebt.sub( bond.payoutRemaining );
                    depositorInfo[msg.sender].bonds[ i ] = Bond({
                        payoutRemaining: 0,
                        lastBlock: block.number,
                        vestingPeriod: 0,
                        pricePaid: bond.pricePaid,
                        vested: true
                    });
                } else {
                    // calculate and send vested OHM
                    uint payout = bond.payoutRemaining.mul( percentVested ).div( 10000 );
                    ...
                    depositorInfo[msg.sender].bonds[ i ] = Bond({
                        payoutRemaining: bond.payoutRemaining.sub( payout ),
                        lastBlock: block.number,
                        vestingPeriod: bond.vestingPeriod.sub( blocksSinceLast ),
                        pricePaid: bond.pricePaid,
                        vested: false
                    });
                }
    ...
    function getDepositorInfo( address address_, uint index_ ) external view returns ( 
        ...
        Bond memory bond = depositorInfo[ address_ ].bonds[ index_ ];
        _payoutRemaining = bond.payoutRemaining;
        _lastBlock = bond.lastBlock;
        _vestingPeriod = bond.vestingPeriod;
        _pricePaid = bond.pricePaid;
        _vested = bond.vested;
    }
     ...
     */
    function percentVestedFor( address depositor_, uint index_ ) external view returns ( uint _percentVested ) {
    ...
    function _calculatePercentVested( address depositor_, uint index_ ) internal view returns ( uint _percentVested ) {
        Bond memory bond = depositorInfo[ depositor_ ].bonds[ index_ ];
        uint blocksSinceLast = block.number.sub( bond.lastBlock );
     ...
     */
    function pendingPayoutFor( address depositor_, uint index_ ) external view returns ( uint _pendingPayout ) {
        uint percentVested = _calculatePercentVested( depositor_, index_ );
        uint payoutRemaining = depositorInfo[ depositor_ ].bonds[ index_ ].payoutRemaining;
     ...
     */
    function bondInterestFor( uint value_, uint price_ ) public pure returns ( uint ) {
    ...
    function bondPrice() public view returns ( uint _price ) {        
        _price = controlVariable.mul( _calcDebtRatio() ).add( uint(1000000000) ).div( 1e7 );
