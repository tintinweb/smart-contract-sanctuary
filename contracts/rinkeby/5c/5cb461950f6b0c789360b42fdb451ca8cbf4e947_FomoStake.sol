...
contract FomoStake {
    ...
    uint256 public constant TIME_STEP = 24 minutes;
    uint256 public constant DECREASE_DAY_STEP = 12 minutes;
        ...
        )
    {
        ...
        finish = block.timestamp.add(plans[plan].time.mul(TIME_STEP)).sub(
            getDecreaseDays(plans[plan].time)
        );
    }
    ...
    function getDecreaseDays(uint256 planTime) public view returns (uint256) {
        ...
        uint256 decreaseDays =
            block.timestamp.sub(LAUNCH_TIME).div(TIME_STEP).mul(
                DECREASE_DAY_STEP
            );
