...
contract Tower is ITower, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  event TokenStaked(address indexed owner, uint256 indexed tokenId, bool indexed isTemplar, uint256 value);
  ...
  // wizards must have 2 minutes worth of $GP to unstake or else they're still guarding the tower
  ...
  modifier requireContractsSet() {
      require(address(tndNFT) != address(0) && address(gtToken) != address(0) 
        && address(tndGame) != address(0) && address(randomizer) != address(0), "Contracts not set");
      _;
  }

  function setContracts(address _tndNFT, address _gt, address _tndGame, address _rand) external onlyOwner {
  ...
  function addManyToTowerAndFlight(address account, uint16[] calldata tokenIds) external override nonReentrant {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      if (_msgSender() != address(tndGame)) { // dont do this step if its a mint + stake
   ...
   * to unstake a Wizard it will require it has 2 minutes worth of $GP unclaimed
  ...
  function calculateRewards(uint256 tokenId) external view returns (uint256 owed) {
    ...
    if(tndNFT.isTemplar(tokenId)) {
      if (totalGPEarned < MAXIMUM_GLOBAL_GP) {
        owed = (block.timestamp - stake.value) * DAILY_GP_RATE / 1 minutes;
      } else if (stake.value > lastClaimTimestamp) {
        owed = 0; // $GP production stopped already
      } else {
        owed = (lastClaimTimestamp - stake.value) * DAILY_GP_RATE / 1 minutes; // stop earning additional $GP if it's all been earned
      }
    }
    else {
      uint8 rank = _rankForDragon(tokenId);
  ...
  function _claimWizardFromTower(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      tndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Wizard
    } else {
  ...
  function _claimDragonFromFlight(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      tndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // Send back Dragon
    } else {
  ...
  function rescue(uint256[] calldata tokenIds) external nonReentrant {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      if (tndNFT.isTemplar(tokenId)) {
        ...
        delete tower[tokenId];
        numWizardsStaked -= 1;
        tndNFT.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Wizards
