...
contract ConchitaToken is IERC721, Ownable, ERC721Enumerable {
    ...
    function mint(uint256 _mintAmount) public payable {
        require(saleIsActive, "Sale must be active to mint a Conchita");
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
        require(msg.value >= cost * _mintAmount, "insufficient funds");

        if(onlyWhitelisted && whitelisted[msg.sender] == true) {
            ...
            require(maxSupply <= _mintAmount, "Can only mint 20 tokens at a time");
            for (uint256 i = 1; i <= _mintAmount; i++) {
                _safeMint(msg.sender, supply + i);
            }
        }
