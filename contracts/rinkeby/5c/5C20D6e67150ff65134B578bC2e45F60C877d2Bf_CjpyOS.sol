...
contract CurrencyOS {
    ...
    function setYmtOSProxy(address _ymtOSProxyAddr) external onlyGovernance onlyOnce {
    ...
    modifier onlyYamato() {
        if (yamatoes.length == 0) {
            ...
            for(uint i = 0; i < yamatoes.length; i++){
    ...
    function _mintCurrency(address to, uint amount) internal {
