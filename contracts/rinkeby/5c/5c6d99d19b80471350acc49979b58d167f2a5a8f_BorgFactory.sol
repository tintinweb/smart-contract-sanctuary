...
contract Borgs is BorgFactory, NumberUtils{
    ...
    function getBorgImage(uint256 borgId) public view returns(string[] memory){
        ...
        return combineBorgPeices(borg.borgPeices, IMAGE_SIZE);
    }
    ...
    function generateToken() external
    {
        ...
        for(uint256 i=0;i<_layerMetaData.length;i++){
            ...
            borgPeiceNames[i] = layerItem.borgPeiceName;
        }
        ...
        borg.borgPeices = borgPeiceNames;
    ...
    function breedBorgs(uint256 borgId1, uint256 borgId2) external{
        ...
        require(borg1.borgPeices.length == borg2.borgPeices.length, 'Borg layer counts do not match');
        ...
        string[] memory borgPeiceNames1 = borg1.borgPeices;
        string[] memory borgPeiceNames2 = borg2.borgPeices;
        string[] memory borgPeiceNames = filterRarestBorgPeices(borgPeiceNames1, borgPeiceNames2);
    ...
    function filterRarestBorgPeices(string[] memory borgPeiceNames1, string[] memory borgPeiceNames2) public returns(string[] memory){
        // Check the peices size is the same
        require(borgPeiceNames1.length == borgPeiceNames1.length, 'Borg layer counts do not match');
        
        /*
        for(uint256 i=0;i<borgPeiceNames.length;i++){
            _borgPeices[borgPeiceNames[i]
        }
        */
        return new string[](0);
    }
