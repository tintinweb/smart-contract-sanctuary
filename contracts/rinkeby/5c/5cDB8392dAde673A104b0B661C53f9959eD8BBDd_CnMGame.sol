...
contract CnMGame is Ownable, ReentrancyGuard, Pausable {
  ...
  event RollCommitted(address indexed owner, uint16 indexed tokenId);
  event MintRevealed(address indexed owner, uint256 indexed amount);
  event RollRevealed(address indexed owner, uint256 indexed tokenId);
  ...
  // roll commit id -> offchain random
  // mapping(uint16 => uint256) private _commitRollRandoms;
