...
contract DeliCon {
    ...
    /* This is the share the courier owned by fulfilling the delivery, 
     * THIS IS NOT IN PERCENT
     * 4 = 1/4 = 25 %
     * 5 = 1/5 = 20 %
     * 10 = 1/10 = 10 %
     */
    uint256 profit_share_courier = 10;
 ...
 // ["test", "test", "test"], "hello", 0xDEADBEEF
    ...
    */
    ...
    function restaurant_reclaim_deposit(uint256 order_num, bytes calldata signature) public {
        require(orders[order_num].restaurant == msg.sender, "You are not the restaurant that is responsible for this order");
        require(orders[order_num].rest_deposit > 0, "You already claimed the deposit for this order");
        
        Order storage order = orders[order_num];
        Restaurant storage rest = restaurants[order.restaurant];
        require(is_valid_release_signature(order.courier, signature, order_num), "Signature is not valid");
    ...
    function courier_reclaim_deposit(uint256 order_num, bytes calldata signature) public {
        require(orders[order_num].courier == msg.sender, "You are not the courier that is responsible for this order");
        require(orders[order_num].cour_deposit > 0, "You already claimed the deposit for this order");
        require(orders[order_num].status != OrderStatus.fulfilled, "This order has already been fulfilled");
        require(((block.timestamp - orders[order_num].last_status_change) < 24 hours) || orders[order_num].status != OrderStatus.cancelled, "The order has been cancelled and your 24 hours to claim the deposit have passed");
        
        Order storage order = orders[order_num];
        Courier storage cour = couriers[order.courier];
        Restaurant storage rest = restaurants[order.restaurant];
        
        require(is_valid_release_signature(order.customer, signature, order_num), "Signature is not valid");
        
        cour.funds += order.cour_deposit;
        
        /* Pay the participants */
        uint256 profit_courier = order.amount / profit_share_courier;
        cour.funds += profit_courier;
        rest.funds += order.amount - profit_courier;
    ...
    function is_valid_release_signature(address signer, bytes calldata signature, uint256 order_num) internal view returns(bool) {
        ...
        release_data = _prefixed(keccak256(abi.encodePacked(order_num, this)));
