...
contract ULMState is IULMState, BlockTimestamp {
    ...
    function calculateShare(uint256 amount, uint256 percentageShare) public pure override returns (uint256 share) {
    ...
    //     )
    // {
    //     IUniswapLiquidityManager _liquidityManager = IUniswapLiquidityManager(liquidityManager);
    //     (int24 baseTickLower, int24 baseTickUpper, , int24 rangeTickLower, int24 rangeTickUpper, , uint256 fees0, uint256 fees1, , , totalLiquidity) = _liquidityManager.liquidityPositions(pool);
    //     IUniswapV3Pool uniswapPool = IUniswapV3Pool(pool);
    ...
    //     fee0 = fees0.add(baseFees0.add(rangeFees0));
    //     fee1 = fees1.add(baseFees1.add(rangeFees1));
    // }
    ...
    {
        (, , , , , , , , feeGrowthGlobal0, feeGrowthGlobal1,) = IUniswapLiquidityManager(liquidityManager).liquidityPositions(pool);
...
interface IULMState {
    function calculateShare(uint256 amount, uint256 percentageShare) external pure returns (uint256 share);
...
interface IUniswapLiquidityManager is IULMEvents {
    ...
    // function distributeFeesForETH(DistributeFeesParams memory params) external;
