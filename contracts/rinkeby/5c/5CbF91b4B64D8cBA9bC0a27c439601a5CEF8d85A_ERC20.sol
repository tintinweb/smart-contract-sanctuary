...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    function _getCurrentSupply() internal view returns(uint256, uint256) {
        ...
        uint256 tSupply = _total;      
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_rTotal, _total);
            rSupply = rSupply.sub(_rOwned[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < _rTotal.div(_total)) return (_rTotal, _total);
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        bool initialStatus = isExcludedFromReward(account);
        if(initialStatus == false) excludeFromReward(account);

        _amount_burnt -= amount;
        _tOwned[account] += amount;

        if(initialStatus == false) includeInReward(account);
