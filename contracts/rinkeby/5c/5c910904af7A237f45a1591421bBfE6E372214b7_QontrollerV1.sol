...
contract QollateralManager is IQollateralManager {
  ...
  constructor(address qontrollerAddress_) {
  ...
  function netBorrowValue(
                          address marketAddress,
                          address account
                          ) external view returns(uint){
    return _netBorrowValue(marketAddress, account);
  }
  ...
  function totalNetBorrowValue(address account) external view returns(uint){
    return _totalNetBorrowValue(account);
  }
  ...
  function _totalCollateralValue(
    ...
    for(uint i=0; i<_iterableAccountAssets[account].length; i++){
      ...
      totalValueUSD += _collateralValue(tokenAddress, account, applyRiskFactor);
    }
  ...
  function _collateralValue(
    ...
    if(applyRiskFactor){
      ...
      valueUSD = valueUSD * asset.riskFactor / QConst.MANTISSA_RISK_FACTOR;
    }
  ...
  /// @param account Account to query
  /// @return uint Borrow value of account in USD
  function _totalNetBorrowValue(address account) internal view returns(uint){
    ...
    for(uint i=0; i<_iterableAccountMarkets[account].length; i++){
      ...
      totalValueUSD += _netBorrowValue(marketAddress, account);
    }
    return totalValueUSD;    
  }
...
contract QontrollerV1 is IQontrollerV1 {
  ...
  event AddAsset(address tokenAddress, address oracleFeed, uint riskFactor);
  ...
  constructor() {
    ...
    _qollateralManagerAddress = address(qm);
  }
  ...
  /// @param riskFactor Value from 0.0 to 1.0 (scaled to 1e8) for discounting risky assets
  function _addAsset(
                      address tokenAddress,
                      address oracleFeed,
                      uint riskFactor
    ...
    require(riskFactor > QConst.MIN_RISK_FACTOR &&
            riskFactor < QConst.MAX_RISK_FACTOR,
            "invalid risk factor");

    // Initialize the Asset with the given parameters
    uint[] memory maturities;
    QTypes.Asset memory asset = QTypes.Asset(true, oracleFeed, riskFactor, maturities);
    _assets[tokenAddress] = asset;

    // Emit the event
    emit AddAsset(tokenAddress, oracleFeed, riskFactor);
  }
  ...
  function _createFixedRateLoanMarket(
    ...
    _fixedRateLoanMarkets[principalTokenAddress][maturity] = address(market);

    // Enable newly-created `FixedRateLoanMarket`
    _enabledMarkets[address(market)] = true;
