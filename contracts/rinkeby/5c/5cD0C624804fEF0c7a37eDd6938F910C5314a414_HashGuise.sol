...
contract HashGuise {
    ...
    function mintNFT() public payable {
        require(distributionCurve(false) != 0, "HashGuise::mintNFT: Sale has already ended");
        require(distributionCurve(false) <= msg.value, "HashGuise::mintNFT: Ether value sent is not correct");

        uint256 returnAmount = msg.value;
        while (returnAmount >= distributionCurve(false) && distributionCurve(false) != 0){
            returnAmount = returnAmount.sub(distributionCurve(false));
