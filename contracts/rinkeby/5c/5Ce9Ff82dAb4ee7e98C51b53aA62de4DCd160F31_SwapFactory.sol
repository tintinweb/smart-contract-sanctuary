...
contract SwapFactory is IUniswapV2Factory {
    ...
    function setProtocolFee(uint _protocolFeeDenominator) external {
        require(msg.sender == feeToSetter, 'Swap: FORBIDDEN');
        require(_protocolFeeDenominator > 0, 'Swap: FORBIDDEN_FEE');
    ...
    function changeTotalFee(uint _totalFee) external {
        require(msg.sender == feeToSetter, 'Swap: FORBIDDEN');
        require(_totalFee > 0, 'Swap: FORBIDDEN_FEE');
        totalFee = _totalFee;
    }
}
...
abstract contract SwapERC20 is IUniswapV2ERC20 {
    using SafeMath_0 for uint;

    string public override constant name = 'Swap LP';
