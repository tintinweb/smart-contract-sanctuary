...
contract MultiSwap is Ownable {
    ...
    uint maxReserve = 5 * 1000000 * 10e9;
    uint maxSlippage = 10;
    uint totalRoute;
    ...
    function swapRateInSat(address ROUTER02, uint ETHamountInSat, address[] memory _froms, address _to) external view returns (uint) {
        ...
        path[0] == path[1] ? amount = amount.add(tokenAmount) : amount = amount.add(ROUTER.getAmountsOut(tokenAmount, path)[path.length - 1]);
        }
    ...
    function swap(address _from, uint amount, address _to) external payable {
        ...
        if (_from == BNB) {
            ...
            if (_to == WBNB) {
                _send(WBNB,msg.sender,amount);
            } else {
                _swap(WBNB,amount,_to,msg.sender);
            }
            ...
            require(amount <= token.balanceOf(msg.sender),"token amount less than available");
            ...
            if (sender.balance < maxReserve && _to != BNB) {
                amount > maxReserve - sender.balance ? newAmount = amount - (maxReserve - sender.balance) : newAmount = amount;
            } else {
                newAmount = amount;
            }
            if (_to == WBNB) {
    ...
    function multiSwap(address[] memory _froms, address _to) external payable {
        ...
        uint newAmountBNB;
        address payable sender = msg.sender;
        require(msg.sender == owner() || (msg.sender != owner() && msg.value >= maxReserve), "send fee");
        require(totalRoute > 0,"route = 0");
        for (uint i=0;i<_froms.length;i++) {
            if (_froms[i] == BNB) {
                require(msg.value > 0,"eth = 0");
                ...
                require(amount <= IBEP20(_froms[i]).balanceOf(msg.sender),"token amount less than available");
                ...
                _froms[i] == WBNB ? amountBNB = amountBNB.add(amount) : amountBNB = amountBNB.add(_swap(_froms[i], amount, WBNB, address(this)));
            }
        }
        if (sender.balance < maxReserve && _to != BNB) {
            amountBNB > maxReserve - sender.balance ? newAmountBNB = amountBNB - (maxReserve - sender.balance) : newAmountBNB = amountBNB;
        } else {
    ...
    function _send(address token, address _to, uint amount) private {
        ...
        IBEP20(token).transferFrom(address(this),_to,amount);
    }
