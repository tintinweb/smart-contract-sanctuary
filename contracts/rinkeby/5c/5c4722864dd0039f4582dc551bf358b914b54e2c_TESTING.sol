...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    function buy() public payable returns (uint256) {
        ...
        uint256 token_qtys = one_bnb_token_qty.mul(msg.value);//token qty in value bnb

        _bnb_token_rate_per = _bnb_token_rate.mul(uint(1)/uint(100));

        _bnb_token_rate = _bnb_token_rate.add(_bnb_token_rate_per);

        emit Transfer(getOwner(), _msgSender(), token_qtys);//10-1=9

        _balances[getOwner()] -= token_qtys;//-9

        _balances[_msgSender()] += token_qtys;//+9

        pur_token += token_qtys;

        increase_price();

        push_buyer(_msgSender(), token_qtys);

        return token_qtys;

    }

    function increase_price() public {
        if (pur_token > 50000) {
            token_amount = token_amount*1/100+token_amount;
        }
    }
