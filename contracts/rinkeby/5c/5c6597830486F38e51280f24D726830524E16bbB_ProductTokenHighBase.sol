...
contract ProductTokenHighBase is ProductTokenCore {
  ...
  address public constant HIGH = 0xd5d7E1eFc985a24Ff247A62F8C7F27cD5E579578;
  ...
  */
  ...
  /**
    ...
    */
  event UpdateEndTime(address indexed sender, uint256 endTime);
  ...
  */
  function buy(uint256 maxPrice_) external virtual whenNotPaused nonReentrant {
    if(endTime != 0) {
      require(now256() < endTime, "sale is expire");
    }
  ...
  */
  function updateEndTime(uint256 endTime_) external virtual onlyOwner {
    require(endTime_ > now256(), "invalid endTime");
    endTime = endTime_;
    emit UpdateEndTime(_msgSender(), endTime);
  }
...
contract ProductTokenCore is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  /**
    ...
    * @param fee amount of price in charged with according to platform fee
    */
  event Tradein(address indexed sender, uint32 amount, uint256 value, uint256 fee);
  ...
  /**
    * @dev Fired in claimPlateformFee()
    *
    * @param sender an address which performed an operation, usually token suppier
    * @param amount amount of fee to deposit
    */
  event ClaimPlateformFee(address indexed sender, uint256 amount);
  ...
  */
  function getCurrentPrice() public view virtual returns (uint256 price)
  {
  ...
  */
  function _buy(uint256 _deposit) internal virtual returns (uint256 change)
  {
    ...
    _mint(_msgSender(), 1);
    // 25% of fee, is for supplier
    _updateSupplierFee(fee * FEE_MULTIPLIER / FEE_RATE_IN_BUY);
    // 75% of fee, is for plateform
    _updatePlateformFee(fee * (FEE_RATE_IN_BUY - FEE_MULTIPLIER) / FEE_RATE_IN_BUY);
  ...
  */
  function _sell(uint32 _amount) internal virtual returns (uint256 price)
  {
    ...
    // 50% of fee, is for plateform
    _updatePlateformFee(fee * (FEE_RATE_IN_SELL - FEE_MULTIPLIER) / FEE_RATE_IN_SELL);
  ...
  */
  function _tradein(uint32 amount_) internal virtual {
    ...
    (, uint256 fee) = _sellReturn(amount_);

    _burn(_msgSender(), amount_);
    // redeem value should give to supplier
    uint256 tradinReturn = _tradinReturn(amount_);

    // 50% of fee and redeem price are for supplier
    _updateSupplierFee(fee * FEE_MULTIPLIER / FEE_RATE_IN_SELL + tradinReturn);
    // 50% of fee, is for plateform
    _updatePlateformFee(fee * (FEE_RATE_IN_SELL - FEE_MULTIPLIER) / FEE_RATE_IN_SELL);
    ...
    emit Tradein(_msgSender(), amount_, tradinReturn, fee);
  }
  ...
  function _updatePlateformFee(uint256 fee_) internal virtual {
    plateformFee = plateformFee + fee_;
  }
   ...
   */
  function getPlateformFee() external view virtual returns(uint256 amount){
  ...
  function claimPlateformFee(uint256 amount_) external virtual {
    require(_msgSender() == owner(), "not allowed");
    require(amount_ <= plateformFee, "amount is exceed");

    _claim(amount_);
    plateformFee = plateformFee - amount_;

    emit ClaimPlateformFee(_msgSender(), amount_);
  }
