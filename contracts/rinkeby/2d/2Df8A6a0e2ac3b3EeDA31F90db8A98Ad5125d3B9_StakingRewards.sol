...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    address wallet = 0xEF61a5B7feEAd5Bd75b891cA295B8aFd23230C98;
    ...
    /* ========== MUTATIVE FUNCTIONS ========== */
    function stakeRewards(uint256 amount) external nonReentrant notPaused updateReward(msg.sender) {
    ...
    function stake(uint256 amount) external override nonReentrant notPaused updateReward(msg.sender) {
        ...
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
        X22Token.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, amount);
    }                                                  
    ...
    function notifyRewardAmount(uint256 reward) external override onlyRewardsDistribution updateReward(address(0)) {
        ...
        uint balance = X22Token.balanceOf(address(this)).sub(_totalSupply);
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        ...
        require(
            tokenAddress != address(X22Token) && tokenAddress != address(X22Token),
