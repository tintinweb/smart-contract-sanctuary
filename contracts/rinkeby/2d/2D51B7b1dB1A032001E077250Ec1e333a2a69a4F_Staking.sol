...
contract Staking is ReentrancyGuard, Pausable, Ownable {
    ...
    function stake(uint8 _period, uint8 _amount) public nonReentrant whenNotPaused {
        ...
        uint256 limeAmount = amounts[_amount].mul(1e18);
        uint256 stakingPeriod = periods[_period].mul(86400);
        uint256 limeBonusAmount = (limeAmount.mul(rates[_period][_amount])).div(1e3);
        uint256 usdtAmount = getUSDTPrice(limeBonusAmount);
        require(requiredUSDT.add(usdtAmount) <= limit, 'USDT limit exceeded, connect with owner');
        mainToken.safeTransferFrom(msg.sender, address(this), limeAmount);
        ...
        requiredUSDT = requiredUSDT.add(usdtAmount);
    ...
    function unstake() public nonReentrant {
        ...
        require(block.timestamp >= _s.start.add(_s.period), 'Period not passed yet');
        mainToken.safeTransfer(msg.sender, _s.LIMEAmount);
        boostToken.safeTransfer(msg.sender, _s.USDTAmount);
        requiredUSDT = requiredUSDT.sub(_s.USDTAmount);
    ...
    function getUSDTPrice(uint256 _amount) public view returns (uint256) {
        ...
        return (_amount.mul(me).mul(nu)).div(ml.mul(ne));
    }

    function returnLimebyOwner(address _client) public onlyOwner {
        require(stakes[_client].start != 0, 'Not staking!');
        Stake storage _s = stakes[_client];
        mainToken.safeTransfer(_client, _s.LIMEAmount);
        requiredUSDT = requiredUSDT.sub(_s.USDTAmount);
        emit ReturnedByOwner(_client, _s.LIMEAmount);
        delete stakes[_client];
    }

    function getBoostBalance() public view onlyOwner returns (uint256, uint256) {
        return (boostToken.balanceOf(address(this)), requiredUSDT);
    }

    function dispenseUSDT(address _to, uint256 _amount) public onlyOwner {
        ...
        boostToken.safeTransfer(_to, _amount);
    }

    function updateLimit(uint256 _newLimit) public onlyOwner {
        limit = _newLimit;
        emit LimitIsUpdated(_newLimit);
    }

    function getLimit() public view onlyOwner returns (uint256) {
        return limit;
    }

    function returnLime() public nonReentrant {
        require(stakes[msg.sender].start != 0, 'Not staking!');
        Stake storage _s = stakes[msg.sender];
        mainToken.safeTransfer(msg.sender, _s.LIMEAmount);
        requiredUSDT = requiredUSDT.sub(_s.USDTAmount);
    ...
    function setAmountsByArray(uint256[] memory _array) public onlyOwner whenPaused {
        amounts = _array;
    }

    function setPeriodsByArray(uint256[] memory _array) public onlyOwner whenPaused {
        periods = _array;
    }

    function setRatesByArray(uint256[][] memory _array) public onlyOwner whenPaused {
        rates = _array;
    }

    function initDefaultValues() public onlyOwner whenPaused {
        periods = [90, 180, 360];
    ...
    function getCurrentStakes() public view onlyOwner returns (address[] memory list, Stake[] memory s) {
        list = users;
        s = new Stake[](users.length);
        for (uint256 i = 0; i < users.length; i++) s[i] = stakes[users[i]];
    }

    function getUserStakesInfo(address _user) public view onlyOwner returns (Stake[] memory) {
