...
contract TestCatryoshkas is ERC721, ERC721Enumerable, Ownables {
    ...
    function mintTokens(uint256 numberOfTokens_) public payable isSupplyAvailable(numberOfTokens_) {
        ...
        if(!hasSaleStarted()){
            ...
            require(totalMintedSupply() <= _presaleMints, "Exceeds presale limit");
    ...
    function giveAway(uint256 startIndex_,  uint256 endIndex_, address to_) public onlyOwners {
        
        _mintReserveTokens(startIndex_, endIndex_, to_);

    }

     function _mintReserveTokens(uint256 startIndex_, uint256 endIndex_, address to_ ) internal {
        
        uint256 numberOfTokens = endIndex_ - startIndex_;
        
        require( _reserveSupply.add(numberOfTokens) <= _MAX_RESERVE_SUPPLY, "Exceeds reserve");

        _reserveSupply = _reserveSupply.add(numberOfTokens);

        for (uint256 counter = 0; counter < numberOfTokens; counter++) {
    ...
    function breed(uint256 idCat1_ , uint256 idCat2_) public {
        ...
        require(_bigCats <= _MAX_BIG_CATS, "all big cats minted");
        ...
        uint256 mintIndex = availableMints().add(_bigCats);
        _mintReserveTokens(mintIndex, mintIndex.add(1), _msgSender());

        _bigCats = _bigCats.add(1);
    ...
    function totalMintedSupply() public view returns(uint256) {
        ...
        return totalSupply().sub(_reserveSupply);
