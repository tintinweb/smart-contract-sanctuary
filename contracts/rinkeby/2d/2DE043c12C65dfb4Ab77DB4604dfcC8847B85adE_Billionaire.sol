...
contract Billionaire is ERC721Enumerable, Pausable, PaymentSplitter {
    ...
    uint256 public immutable maxGiftSupply;
    uint256 public immutable maxPresaleSupply;
    uint256 public immutable maxCountPerUser;
    uint256 public giftCount;
    uint256 public presaleCount;
    uint256 public pricePresale;
    PriceSale public priceSale;
    ...
    event ChangePricePresale(uint256 _price);
    ...
    constructor(
        ...
        uint256 _maxCountPerUser
    )
        ERC721("Billionaire Token", "BILlIONAIRE")
        PaymentSplitter(_team, _teamShares)
    {
        require(_maxTotalSupply > 0, "Billionaire: max Total Supply is zero");
        require(_maxGiftSupply > 0, "Billionaire: max Gift Supply is zero");
        require(
            _maxPresaleSupply > 0,
            "Billionaire: max Presale Supply is zero"
        );
        require(
            _maxCountPerUser > 0,
            "Billionaire: max Count Per User is zero"
        );
        require(
            _maxGiftSupply <= _maxTotalSupply,
            "Billionaire: max Gift Supply  out of range"
        );

        require(
            _maxPresaleSupply <= _maxTotalSupply,
            "Billionaire: max Presale Supply  out of range"
        );
        maxTotalSupply = _maxTotalSupply;
        maxGiftSupply = _maxGiftSupply;
        maxPresaleSupply = _maxPresaleSupply;
        maxCountPerUser = _maxCountPerUser;
    }
    ...
    function contractURI() public pure returns (string memory) {
        return "https://ipfs.billionaireclubnft.com/ipfs/QmaRJCHSW94hHBApvzF3SMnTwiLw9Z7HiDLkGx1qiMLhu1";
    }
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Billionaire: Can't add a null address"
            );
            if (_presaleList[_addresses[ind]] == false) {
                _presaleList[_addresses[ind]] = true;
                _presaleClaimed[_addresses[ind]] > 0
                    ? _presaleClaimed[_addresses[ind]]
                    : 0;
            }
        }
    }
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Billionaire: Can't remove a null address"
            );
    ...
    function setPricePresale(uint256 _pricePresale) external onlyOwner {
        ...
        emit ChangePricePresale(_pricePresale);
    }

    function setPricesale(
        uint256[] calldata _startTime,
        uint256[] calldata _price
    ) external onlyOwner {
        require(
            _startTime.length == 4,
            "Bilionaire:  srart time length is not valid"
        );
        require(
            _price.length == 4,
            "Bilionaire:  srart time length is not valid"
        ...
        require(
            _startTime[0] > 0 &&
                _startTime[1] > _startTime[0] &&
                _startTime[2] > _startTime[1] &&
                _startTime[3] > _startTime[2],
            "Bilionaire: invalid time range"
        );

        require(
            _price[0] > 0 && _price[1] > 0 && _price[2] > 0 && _price[3] > 0,
            "Bilionaire: price is zero"
        );

        priceSale = PriceSale(
            _startTime[0].toUint32(),
            _price[0].toUint32(),
            _startTime[1].toUint32(),
            _price[1].toUint32(),
            _startTime[2].toUint32(),
            _price[2].toUint32(),
            _startTime[3].toUint32(),
            _price[3].toUint32()
        );
    }

    function getPricesale() internal view returns (uint256) {
        uint256 _price;
        PriceSale memory _priceSale = priceSale;
        if (
            block.timestamp >= _priceSale.startTime1 &&
            block.timestamp < _priceSale.startTime2
        ) {
            _price = _priceSale.price1;
        }
        if (
            block.timestamp >= _priceSale.startTime2 &&
            block.timestamp < _priceSale.startTime3
        ) {
            _price = _priceSale.price2;
        }
        if (
            block.timestamp >= _priceSale.startTime3 &&
            block.timestamp < _priceSale.startTime4
        ) {
            _price = _priceSale.price3;
        }

        if (block.timestamp >= _priceSale.startTime4) {
            _price = _priceSale.price4;
        }
        ...
        whenNotPaused
    {
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            ...
            require(
                _totalClaimed[_addresses[ind]] <= maxCountPerUser,
                "Bilionaire: total claimed  exceedes max Count Per User"
            );
            _tokenIds.increment();
            ...
            _giftClaimed[_addresses[ind]] = _giftClaimed[_addresses[ind]].add(
                1
            );
            _totalClaimed[_addresses[ind]] = _totalClaimed[_addresses[ind]].add(
                1
            );
        }
        giftCount = giftCount.add(_addresses.length);
    }

    function specialGiftMint(
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            ...
            totalNFT = totalNFT.add(1);
            _safeMint(_addresses[ind], _ids[ind]);
            _giftClaimed[_addresses[ind]] = _giftClaimed[_addresses[ind]].add(
                1
            );
            _totalClaimed[_addresses[ind]] = _totalClaimed[_addresses[ind]].add(
    ...
    function setUpPresale(
        ...
        require(_maxCount > 0, "Billionaire: zero amount");
        require(
            _startTime > 0 && _endTime > _startTime,
            "Billionaire: invalid time range"
        );
    ...
    function setUpSale(
        ...
        uint256 _maxCount,
        uint256 _unlockTime,
        uint256 _unlockedMaxCount
    ) external onlyOwner {
        require(
            _maxCount > 0 && _unlockedMaxCount > 0,
            "Bilionaire: zero amount"
        );
        require(
            _startTime > 0 && _unlockTime > _startTime,
            "Bilionaire: invalid time range"
        );
        ...
        saleConfig = SaleConfig(
            _startTime.toUint32(),
            _maxCount.toUint32(),
            _unlockTime.toUint32(),
            _unlockedMaxCount.toUint32()
        );
    ...
    function presaleMint(uint256 _amount) public payable whenNotPaused {
        ...
        require(
            ...
            "Bilionaire: presale not configured"
        );
        require(
        ...
        require(
            ...
            "Bilionaire: presale ended"
        );
        require(
            _presaleClaimed[msg.sender] + _amount <= _presaleConfig.maxCount,
            "Bilionaire: presale max count exceeded"
        );
        require(
            _totalClaimed[msg.sender] <= maxCountPerUser,
            "Bilionaire: total claimed  exceedes max Count Per User"
        );
        require(
        ...
        require(pricePresale > 0, "Bilionaire:  price not set");
        require(
            pricePresale * _amount == msg.value,
            "Bilionaire: incorrect Ether value"
        );

        _presaleClaimed[msg.sender] = _presaleClaimed[msg.sender].add(_amount);
        _totalClaimed[msg.sender] = _totalClaimed[msg.sender].add(_amount);
        uint256 _newItemId;
        ...
        emit PresaleMint(msg.sender, _amount, pricePresale);
    }

    function saleMint(uint256 _amount) public payable whenNotPaused {
        SaleConfig memory _saleConfig = saleConfig;
        require(_amount > 0, "Bilionaire: zero amount");
        require(_saleConfig.startTime > 0, "Bilionaire: sale not configured");
        require(
        ...
        require(
            _saleClaimed[msg.sender] + _amount <=
                (
                    block.timestamp >= _saleConfig.unlockTime
                        ? _saleConfig.unlockedMaxCount
                        : _saleConfig.maxCount
                ),
            "Bilionaire: amount exceeded max count"
        );
        require(
            totalNFT + _amount <= maxTotalSupply,
            "Bilionaire: max supply exceeded"
        );
        uint256 _price = getPricesale();
        require(_price > 0, "Bilionaire: token price not set");
        require(
            _price * _amount == msg.value,
            "Bilionaire: incorrect Ether value"
        );

        _saleClaimed[msg.sender] = _saleClaimed[msg.sender].add(_amount);
        _totalClaimed[msg.sender] = _totalClaimed[msg.sender].add(_amount);
        uint256 _newItemId;
        for (uint256 ind = 0; ind < _amount; ind++) {
            _tokenIds.increment();
