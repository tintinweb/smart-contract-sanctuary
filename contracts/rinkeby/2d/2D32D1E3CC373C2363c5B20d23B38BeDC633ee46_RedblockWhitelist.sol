...
contract RedblockWhitelist is Ownable {
    ...
    function whitelist() external {
        ...
        require(isEligibleForWhitelist(msg.sender), "Whitelist: not eligible for whitelisting");
    ...
    function isEligibleForWhitelist(address user) public view returns (bool) {
        return
            _punks.balanceOf(user) > 0 ||
            _meebits.balanceOf(user) > 0 ||
            _veeFriends.balanceOf(user) > 0;
    }
}
