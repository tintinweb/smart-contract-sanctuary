...
contract LockedStakingRewards is Ownable, ReentrancyGuard {
    ...
    uint256 public constant depositDuration = 30 minutes;
    ...
    function getUserTokenAmountBefore(address _user, uint256 _pool) public view returns(uint256) {
        if(block.timestamp > pool[_pool].startOfDeposit) {
            return sharesToToken(_shares[_user][_pool], _pool);
        }
        return sharesToToken(_shares[_user][_pool], _pool) * 1e4 / pool[_pool].tokenPerShareMultiplier;
    }
