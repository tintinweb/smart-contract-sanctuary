...
contract NomoTokens is ERC721, Ownable {
    ...
    ///@notice Stores created card images links
    mapping(string => bool) public cardsImagesLinks;

    event NewCardImageCreated(uint256 indexed _cardImageId, string _name, uint256 _points, string _link);
    ...
    event ChangedCardImagePoints(uint256 indexed _points, uint256 indexed _cardImageId);
    event ChangedCardImageLink(string indexed _link, uint256 indexed _cardImageId);

    /// @notice Contract constructor
    constructor() public ERC721("NomoTokens", "NOMO") {}

    /// @notice Creates card image for athlete card
    /// @param _name The name of the card corresponds to the name of the athlete
    /// @param _link Link of the athlete data on the third-party source
    function createCardImage(
        string calldata _name,
        string calldata _link
    ) external onlyOwner {
        require(keccak256(abi.encodePacked(_name)) != keccak256(abi.encodePacked("")), "Empty name");
        require(keccak256(abi.encodePacked(_link)) != keccak256(abi.encodePacked("")), "Empty name");
        require(!cardsImagesLinks[_link], "Existing link");
        uint256 _cardImageId = cardsImages.length;
        cardsImages.push(CardImageData({name: _name, points: 0, link:_link}));
        cardsImagesLinks[_link] = true;
        emit NewCardImageCreated(_cardImageId, _name, 0, _link);
    }
    ...
    /// @notice Changes the link of the card
    /// @param _cardImageId Card image index
    /// @param _newCardLink New card's link
    function changeCardImageLink(uint256 _cardImageId, string calldata _newCardLink) external onlyOwner {
        cardsImages[_cardImageId].link = _newCardLink;
        emit ChangedCardImageLink(_newCardLink, _cardImageId);
    }
        ...
        )
    {
        ...
        points = cardsImages[cardImageId].points;
        link = cardsImages[cardImageId].link;
    }
        ...
        )
    {
        ...
        points = cardsImages[_cardImageId].points;
        link = cardsImages[_cardImageId].link;
    }
