    ...
    ILendingPool
{
     ...
     */
    uint256 public RESERVE_FACTOR_MANTISSA; // accrued interest has not been converted to reserve interest
    IDao public dao;
    ...
    struct UserAssetInfo {
        ...
        uint256 supplyIndex;
        //
        // We do some fancy math here. Basically, any point in time, the amount of DTokens
    ...
    function initialize(
        ...
        dao = IDao(_dao);
        RESERVE_FACTOR_MANTISSA = _reserveFactorMantissa;
    ...
    function createPool(address _tokenAddress) public override onlyOwner {
        PoolInfo storage pool = poolInfo[_tokenAddress];
        require(!pool.isCreated, "Coindeed: pool exists");
        uint256 tokenDecimals = IERC20(_tokenAddress).decimals();
        // lending pool
    ...
    function removeExpireDeed(address _address) external override {
        require(
            deedInfo[_address].isValid,
            "Coindeed: This deed doesn't exist"
        );
        _removeExpireDeed(_address);
    }

    function _removeExpireDeed(address _address) internal {
        delete deedInfo[_address];
    }
        ...
        returns (uint256)
    {
        ...
        if (getBlockNumber() > pool.accrualBlockNumber && cash != 0) {
            ...
            uint256 tokenReward = blockDelta
                .mul(supplyIndex.sub(user.supplyIndex))
                .mul(REWARD_FACTOR)
                .div(BASE_DENOMINATOR);
            accTokenPerShare = accTokenPerShare.add(
                tokenReward.mul(1e18).div(cash.add(pool.totalBorrows))
            );
        }
        ...
        returns (uint256)
    {
        ...
        uint256 totalBorrow = deed.totalBorrow;
        uint256 cash = _token == address(0)
            ? address(this).balance
            : IERC20(_token).balanceOf(address(this));
        ...
        if (
            ...
            uint256 borrowIndex = borrowRatePerBlock
                .mul(blockDelta)
                .mul(borrowIndex)
                .div(1e18)
                .add(borrowIndex);
        }
        return deed.borrow.mul(borrowIndex.sub(deed.borrowIndex)).div(1e18);
    }

    //Function for user to claim their DToken
    function claimDToken(address _to, address _tokenAddress) external {
        updatePool(msg.sender, _tokenAddress);
        dao.claimDToken(_to);
    }
        ...
        nonReentrant
    {
        ...
        if (userAsset.amount > 0) {
            ...
            if (pending > 0) {
                if (_tokenAddress == address(0)) {
                    ...
                    IERC20(_tokenAddress).safeTransfer(msg.sender, pending);
                }
            } //Send reward token
        }
        if (_amount > 0) {
        ...
        userAsset.rewardDebt = userAsset.amount.mul(pool.accTokenPerShare).div(
            1e18
        );
        ...
        nonReentrant
    {
        require(
            deedInfo[msg.sender].isValid,
            "Coindeed: Only deed can repay token back to pool"
        );
        require(
        ...
        require(deed.totalBorrow >= _amount, "Coindeed: exceed borrow amount");
    ...
    function getCashPrior(address _token) internal view returns (uint256) {
        ...
        return
            IERC20(_token).balanceOf(address(this)).mul(10**18).div(
                10**tokenDecimals
            );
    }
    ...
    function updatePool(address _user, address _token) internal {
        PoolInfo storage pool = poolInfo[_token];
        ...
        uint256 totalSupply = _token == address(0)
            ? address(this).balance.add(pool.totalBorrows)
            : IERC20(_token).balanceOf(address(this)).add(pool.totalBorrows);
        if (totalSupply == 0) {
            pool.accrualBlockNumber == getBlockNumber();
            return;
        }
        ...
        uint256 tokenReward = blockDelta.mul(pool.supplyIndex); //Number of token to reward user
        uint256 dividedReward = tokenReward.mul(REWARD_FACTOR).div(
            BASE_DENOMINATOR
        ); // Divide tokenReward in half
        if (dividedReward > 0) {
            //50% used to transfer to DAO
            if (_token == address(0)) {
                (bool sent, ) = address(dao).call{value: dividedReward}(""); // send ETH to contract DAO
                require(sent, "Coindeed: Transfer failed");
            } else {
                IERC20(_token).safeTransfer(address(dao), dividedReward); // send token to contract DAO
            }

            dao.receiveRewardTokenFromPool(_user, _token, dividedReward);
            // exchange tokenReward
            // The other 50% used to convert reward from USD back to Token that lender deposit in pool
            pool.accTokenPerShare = pool.accTokenPerShare.add(
                (tokenReward.sub(dividedReward)).mul(1e18).div(totalSupply)
            );
        }
    }
        ...
        returns (uint256)
    {
        ...
        vars.borrowIndexNew = vars
            .simpleInterestFactor
            .mul(pool.borrowIndex)
            .div(1e18)
            .add(pool.borrowIndex);
