...
contract Token is Context, IERC20 {
    ...
    function intializeContract(address[] memory accounts, uint256[] memory amounts, uint256[] memory amountsD, address _antiSnipe) external payable onlyOwner {
        ...
        startingSupply = 100_000_000_000; //666_000_000_000_000
                ...
                if(lpPairs[from]) {
                    if (timedLimitsEnabled) {
                        if(block.timestamp <= launch + 30 minutes) { //TODO: change to launch + 2 hours
                            require(_ethBalance <= 0.00025 ether); //TODO: change to 25 * 10**16
                        } else if (block.timestamp <= launch + 22 hours) {
                            require(_ethBalance <= 0.002 ether); //TODO: change to 2 * 10**18
                        } else {
