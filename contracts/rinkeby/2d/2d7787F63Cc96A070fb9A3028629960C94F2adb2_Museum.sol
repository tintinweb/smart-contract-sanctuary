...
contract Museum is ERC721Enumerable, PaymentSplitter, Ownable {
  ...
  function addIndexToID(bytes memory indexToId) public onlyOwner {
  ...
  function getMaxMints(uint8 saleState) public view returns (uint256) {
    ...
    } else if (saleState == _A) {
      return _whitelists[_A][msg.sender];
    } else if (saleState == _BC) {
      return _whitelists[_A][msg.sender] + _whitelists[_BC][msg.sender];
    }
  ...
  function getRandomIndex() private returns (uint256){
    ...
    bytes1 byteIndex = _indexToId[randomIndex];
    return uint(uint8(byteIndex));
  }
