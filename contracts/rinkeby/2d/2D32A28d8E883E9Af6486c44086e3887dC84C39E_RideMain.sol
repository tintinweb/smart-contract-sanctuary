...
contract RidePassenger is RideBase, VRFConsumerBase {
    ...
    mapping(bytes32 => Tmp) internal askSaltToTmp;

    mapping(address => bool) public addressToProcessing; // TODO: if no use at frontend set internal
    mapping(address => bytes32) public addressToTixId; // can remove if know how get events from fulfillrandomness

    event AskSalt(bytes32 indexed idx, address requester);
    event GotSalt(bytes32 indexed idx, address requester, bytes32 tixId);
    ...
    function requestTicket(
        ...
        delete addressToTixId[msg.sender];
        ...
        emit AskSalt(idx, msg.sender);
    }

    function fulfillRandomness(bytes32 _idx, uint256 _salt) internal override {
        address passenger = askSaltToTmp[_idx].passenger;
        bytes32 tixId = keccak256(
            abi.encode(passenger, block.timestamp, _salt)
        );
        tixIdToTicket[tixId].passenger = passenger;
        tixIdToTicket[tixId].minimumBadge = askSaltToTmp[_idx].minimumBadge;
        tixIdToTicket[tixId].destination = askSaltToTmp[_idx].destination;
        tixIdToTicket[tixId].estimatedMetres = askSaltToTmp[_idx]
            .estimatedMetres;
        tixIdToTicket[tixId].estimatedMinutes = askSaltToTmp[_idx]
            .estimatedMinutes;
        addressToActive[passenger] = true;
        addressToProcessing[passenger] = false;

        addressToTixId[passenger] = tixId; // read this at frontend
        emit GotSalt(_idx, passenger, tixId);
    }
