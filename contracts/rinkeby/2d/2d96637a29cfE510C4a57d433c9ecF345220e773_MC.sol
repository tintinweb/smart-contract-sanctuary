...
contract MC is Context, IBEP20, Ownable, ReentrancyGuard {
	...
	address private routerAddress; //uniswap router
	...
	constructor(address payable _routerAddress) public {
		_rOwned[_msgSender()] = _rTotal;
		routerAddress = _routerAddress;

		//call preActivate() after publishing
	}

	//call this after the publishing
	function preActivate() public onlyOwner() {
	...
	function excludeFromReward(address account) public onlyOwner() {
		require(_excluded.length < 100, "max 100 addresses in isEcluded list"); //anti out-of-gas
	...
	uint256 public _taxFee = 4; //*Platform Fund for Content Creators + Development Costs R&D
	...
	function claimBNBReward() public isHuman nonReentrant {
		...
		require(balanceOf(msg.sender) >= 0, "Error: must own $MC to claim reward");
	...
	function swapAndLiquify(address from, address to) private {
		...
		if (
			...
			UswapTokensForEth(address(pancakeRouter), tokenAmountToBeSwapped);
			...
			UaddLiquidity(address(pancakeRouter), owner(), otherPiece, bnbToBeAddedToLiquidity);
	...
	function UcalculateBNBReward(
		...
		uint256 reward = bnbPool.mul(100).mul(currentBalance).div(100).div(totalSupply);

		return reward;
	}

	function UcalculateTopUpClaim(
	...
	function UswapTokensForEth(address routerAddress, uint256 tokenAmount) public {
