...
contract DeadbearsFightClub is ERC721Enumerable, Ownable {
    uint256 public constant OWNER_SUPPLY = 100;
    uint256 public constant MAX_SUPPLY = 1000;
    uint256 public constant FINAL_SUPPLY = 1500;

    uint256 private _launchTimeEpochSeconds = 1628863200;

    string private _baseTokenURI = "https://cloudfront.net/fooobar/";
    bool private _ripCord = false;
    bool private _rerollEnabled = false;

    constructor() ERC721("Deadbears Fight Club", "DEADBEARS") {}
    ...
    function setRerollEnabled(bool val) public onlyOwner {
        _rerollEnabled = val;
    }
    ...
    function getPrice() public pure returns (uint256) {
        return 44000000000000000; // 0.044 eth in wei
    }
    ...
    function reroll(uint256 id1, uint256 id2) public {
        uint256 totalSupply = totalSupply();
        require(ownerOf(id1) == msg.sender, "Sender doesn't own the first token");
        require(ownerOf(id2) == msg.sender, "Sender doesn't own the second token");
        require(totalSupply < FINAL_SUPPLY, "Sold out");
        require(_rerollEnabled, "Rerolls aren't enabled");

        safeTransferFrom(msg.sender, 0xdeaDDeADDEaDdeaDdEAddEADDEAdDeadDEADDEaD, id1);
        safeTransferFrom(msg.sender, 0xdeaDDeADDEaDdeaDdEAddEADDEAdDeadDEADDEaD, id2);
