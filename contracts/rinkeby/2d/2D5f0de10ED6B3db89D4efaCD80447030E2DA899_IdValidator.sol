...
contract IdValidator is IERC721Validator, Ownable {
    ...
    struct TokenGrouping {
        address tokenAddress;
        uint256[] tokenIds;
    }

    constructor(TokenGrouping[] memory tokenGroupings) {
        setIds(tokenGroupings, true);
    }

    function setIds(TokenGrouping[] memory tokenGroupings, bool set)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < tokenGroupings.length; i++) {
            TokenGrouping memory grouping = tokenGroupings[i];
