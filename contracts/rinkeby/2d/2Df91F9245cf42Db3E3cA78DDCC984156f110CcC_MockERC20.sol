...
contract Farms is Ownable, ReentrancyGuard {
  ...
  // Info of each pool.
  struct PoolInfo {
    uint256 allocPoint; // How many allocation points assigned to this pool. VERYs to distribute per block.
    uint256 lastRewardBlock; // Last block number that VERYs distribution occurs.
    uint256 accVERYPerShare; // Accumulated VERYs per share, times 1e12. See below.
  }
  ...
  // VERY tokens created per block.
  uint256 public veryPerBlock;
  ...
  uint256 private constant ACC_VERY_PRECISION = 1e12;

  event LogPoolAddition(uint256 indexed pid, uint256 allocPoint, IERC20 indexed lpToken);
  event LogSetPool(uint256 indexed pid, uint256 allocPoint);
  event LogUpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accVERYPerShare);
  ...
  constructor(
    VERYToken _very,
    address _devaddr,
    uint256 _veryPerBlock
  ) public {
    very = _very;
    devaddr = _devaddr;
    veryPerBlock = _veryPerBlock;
  }

  // Comment out because there are no use of these functions below
  ...
  // function setXtkPerBlock(uint256 _xtkPerBlock) external onlyOwner {
  //   xtkPerBlock = _xtkPerBlock;
  // }
  ...
  function addPool(
    ...
    poolInfo.push(PoolInfo({ allocPoint: allocPoint, lastRewardBlock: lastRewardBlock, accVERYPerShare: 0 }));
    emit LogPoolAddition(stakeTokens.length.sub(1), allocPoint, _stakeToken);
  }

  /// @notice Update the given pool's XTK allocation point contract. Can only be called by the owner.
  ...
  //        uint256 xtkReward = blocks.mul(xtkPerBlock).mul(pool.allocPoint).mul(ACC_XTK_PRECISION).div(totalAllocPoint);
  //        pool.accXtkPerShare = pool.accXtkPerShare.add((xtkReward.div(stakeTokenSupply)));
  //      }
  //      pool.lastRewardBlock = block.number;
  //      poolInfo[pid] = pool;
  //      emit LogUpdatePool(pid, pool.lastRewardBlock, stakeTokenSupply, pool.accXtkPerShare);
  //    }
  //  }

  /// @notice Deposit LP tokens for VERY allocation.
  ...
  function deposit(
    ...
    // user.rewardDebt = user.rewardDebt.add(amount.mul(pool.accVERYPerShare) / ACC_VERY_PRECISION);
  ...
  function withdraw(
    ...
    // user.rewardDebt = user.rewardDebt.sub(amount.mul(pool.accVERYPerShare) / ACC_VERY_PRECISION);
...
contract VERYToken is ERC20, Ownable {
  ...
  constructor() public ERC20("VERY Token", "VERY") {
    _setupDecimals(18);
...
interface IFarms {
  ...
  // OnlyOwner functions
  // function setXtkPerBlock(uint256 _xtkPerBlock) external;
