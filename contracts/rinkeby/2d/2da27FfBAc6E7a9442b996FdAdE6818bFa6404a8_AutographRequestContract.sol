...
//import "@zoralabs/core/dist/contracts/Market.sol";
...
contract AutographRequestContract is Ownable {
     ...
     */
    constructor(address _celebrityContract, address _autographContract, address _zoraMedia) public {
        celebrityContract = CelebrityContract(_celebrityContract);
        //autographContract = AutographContract(_autographContract);
        zoraMediaAddress = _zoraMedia;
        feePercent = 10; // %
    }
     ...
     */
    function signRequest(uint id, string memory nftHash, string memory metadata, IMedia.MediaData memory mediaData, IMarket.BidShares memory bidShares) public {
        Request memory request = requests[id];
        ...
        IMedia(zoraMediaAddress).mint(mediaData, bidShares);
        //Media(zoraMediaAddress).mint(mediaData, bidShares);
