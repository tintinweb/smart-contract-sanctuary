...
contract FloorTreasury is FloorAccessControlled, ITreasury {
    ...
    mapping(address => uint256) public _riskOnValuation; // 18 decimal in ETH terms 
     ...
     */
    function setRiskOnValuation(address _token, uint256 _valuation) external onlyPolicy {
        require(permissions[STATUS.RISKONTOKEN][_token], "Risk on permission not given");
        _riskOnValuation[_token] = _valuation;
    }
    ...
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        ...
        value_ = _amount.mul(10**IERC20Metadata(address(FLOOR)).decimals()).div(10**IERC20Metadata(_token).decimals()).mul(10**3);


        // TODO: what if this is PUNK-ETH
        if (permissions[STATUS.LIQUIDITYTOKEN][_token]) {
            value_ = IBondingCalculator(bondCalculator[_token]).valuation(_token, _amount).mul(10**3);
        }

        // if token is not ETH or ETH-pegged
        if (permissions[STATUS.RISKONTOKEN][_token]) {
            // calculate value of risk on token as determined by policy
            value_ = value_.mul(riskOnValuation(_token)).div(10**IERC20Metadata(address(FLOOR)).decimals());
        }
    }
     ...
     * @return uint256
     */
    function riskOnValuation(address _token) public view override returns (uint256) {
        require(_riskOnValuation[_token] > 0, "Token has no valuation");
        return _riskOnValuation[_token];
    }
