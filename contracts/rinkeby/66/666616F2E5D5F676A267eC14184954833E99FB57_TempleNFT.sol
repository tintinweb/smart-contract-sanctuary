...
contract TempleNFT is ERC721PresetMinterPauserAutoId {
    using Strings for uint256;
   
    uint16 public maxSupply = 1500;
    uint256 public price = 0.1 ether; // start at 0.1 ETH
    ...
    string URIRoot = "https://gateway.pinata.cloud/ipfs/";
    ...
    constructor() ERC721PresetMinterPauserAutoId("Temple Art NFT", "TART", URIRoot) {
        _tokenIds.increment();
    }
    ...
    function buy(uint pack, string[] memory _tokenURIs) payable external {
        ...
        // single
        if (pack == 0) {
          uint256 packPrice = price;
          require(msg.value >= packPrice, 'Not enough ETH');
          _mintWithTokenURI(_tokenURIs[0]);
        }
        // pack
        if (pack == 1) {
          uint256 packPrice = price * 5;
          require(msg.value >= packPrice, 'Not enough ETH');
          for (uint i = 0; i < _tokenURIs.length; i++) {
            _mintWithTokenURI(_tokenURIs[i]);
          }
        }
        // whalepack
        if (pack == 2) {
          uint256 packPrice = price * 30;
          require(msg.value >= packPrice, 'Not enough ETH');
          for (uint i = 0; i < _tokenURIs.length; i++) {
            _mintWithTokenURI(_tokenURIs[i]);
          }
    ...
    function _mintWithTokenURI(string memory _tokenURI) private {
      _mint(msg.sender, _tokenIds.current());
      tokenURIList[_tokenIds.current()] = _tokenURI;
      _tokenIds.increment();
    }

    function _mintNFT(uint16 amount)
    private
    {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
      ...
      return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenURIList[_tokenId])) : "";
    }
