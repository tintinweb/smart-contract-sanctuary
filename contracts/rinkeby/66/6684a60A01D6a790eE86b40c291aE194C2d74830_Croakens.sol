...
contract Croakens is ERC20, Ownable, ReentrancyGuard {
  ...
  function getPending(address account) internal view returns (uint256) {
    return SwampVerse.balanceOf(account)
      * BASE_RATE
      * (block.timestamp - (lastUpdate[account] >= 0 ? lastUpdate[account] : START))
      / 1 days;
  }

  function update(address from, address to) external {
    require(msg.sender == address(SwampVerse),  "Caller cannot update rewards");
  ...
  function burn(address from, uint256 amount) external {
    require(msg.sender == address(SwampVerse), "Caller cannot burn tokens");
    _burn(from, amount);
  }
