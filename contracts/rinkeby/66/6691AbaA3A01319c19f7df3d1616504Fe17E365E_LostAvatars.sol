    ...
    ReentrancyGuard
{
    ...
    uint256 private mintingCost = 50000000000000000;
    uint256 private constant reserved = 2;
    ...
    function mint(uint256 numberOfTokens) external payable nonReentrant {
        ...
        require(
            ...
            "Sorry but you can only mint 20 Bikers per transaction!"
        );
        require(
            totalSupply().add(numberOfTokens) <= maxSupply - reserved,
            "Not enough Bikers left to mint that amount! Everybody panic!"
        );
        require(
            mintingCost.mul(numberOfTokens) <= msg.value,
            "You sent the wrong amount of ETH, Bad Biker."
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < maxSupply - reserved) {
    ...
    function reserveMints() public onlyOwner {
         ...
         require(
            totalSupply().add(reserved) <= maxSupply,
            "Not enough Bikers left to mint that amount! Everybody panic!"
        );
