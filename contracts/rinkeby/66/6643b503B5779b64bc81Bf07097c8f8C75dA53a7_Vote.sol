...
contract Vote is Ownable {
  ...
  address[] recipients;
  mapping(uint128 => mapping(address => uint256)) recipientVotes;
  uint128 month;
  uint256 totalVotes;

  uint128 votingPeriod;
  uint256 lastVotingPeriod;
  ...
  function votePeriodEnd() public view returns (uint256) {
    return lastVotingPeriod + votingPeriod * 60 * 60 * 24;
  }
    ...
    returns (uint256)
  {
    ...
    uint256 daysSince = 1 + (votePeriodEnd() - timestamp) / 60 / 60 / 24;
  ...
  function getVotingPower() public view returns (uint256) {
    return votingPeriod - ((block.timestamp - lastVotingPeriod) / 60 / 60 / 24);
  }

  // Sets class variables for total number of votes
  function countVotes() internal {
    for (uint256 i; i < voteIdKeys.length; i++) {
      // Only count vote if voted
      _Vote memory currentVote = voteId[voteIdKeys[i]];
      if (currentVote.isVoted) {
        if (recipientVotes[month][currentVote.to] == 0) {
          // Check if recipient has been seen before by checking if there are any votes
          recipients.push(currentVote.to);
        }
        // Adds amplified vote to the recipients vote count and the total count
        uint256 amplifiedVote = amplify(
          currentVote.amount,
          currentVote.timestamp
        );
        recipientVotes[month][currentVote.to] += amplifiedVote;
        totalVotes += amplifiedVote;
      }
    }
  }
  ...
  function getLastMonthVotes() public view returns (RecipientToVote[] memory) {
    RecipientToVote[] memory array = new RecipientToVote[](recipients.length);
    for (uint256 i; i < recipients.length; i++) {
      // Each array slot contains a recipient and their votes for the month
      array[i] = (
        RecipientToVote(recipients[i], recipientVotes[month][recipients[i]])
      );
    }
    return array;
  }
...
contract Vault is Ownable {
  ...
  function prepareFundDistribution() public onlyVoteContract {
    ...
    if (TAPwBalance > 0) {
      ...
      uint256 remaining = TAPwToken.balanceOf(address(this));
