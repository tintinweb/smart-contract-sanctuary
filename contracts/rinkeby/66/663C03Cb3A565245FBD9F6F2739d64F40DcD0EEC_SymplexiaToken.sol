...
//   ======================================  
 ...
 //       (uint256 _reservesT0, uint256 _reservesT1,) = IUniswapV2Pair(liquidityPair).getReserves();
        uint256 _reservesT0 = balanceOf(liquidityPair);
        uint256 _reservesT1 = address(liquidityPair).balance;

        // Return amount of Token1 needed to buy Token0 (ETH/BNB)
        if (_reservesT0 * _reservesT1 == 0) return 0;
...
abstract contract Taxable is FlowFlexible, AutoLiquidity {
    ...
    uint256 public _buyStock;
    uint256 public _sellStock;
...
//   ==========================================
    ...
    function _afterTokenTransfer(address sender, address recipient, uint256 amount) internal { 
        ...
        if (_lastTokenPrice == _newTokenPrice)  {return;}

        if (_lastTokenPrice == 0) {
           _lastTokenPrice = _newTokenPrice;
           return; 
        }

        _lastTokenPrice = _newTokenPrice;

        if      (sender    == liquidityPair) {_buyStock  += amount;}
        else if (recipient == liquidityPair) {_sellStock += amount;}
        else return;

        if (_buyStock >= _sellStock)    {_buyStock  -= _sellStock;
                                         _sellStock  = 0;     }
        else                            {_sellStock -= _buyStock;
                                         _buyStock   = 0;     }

        if (_buyStock > (Wallet[liquidityPair].balance  / 10)) 
                { needAttenuation   = true;  }
        else    { needAttenuation   = false; }     
...
//   ======================================
    ...
    function attenuateImpulse() external onlyRole(Treasury_Analyst) {
        ...
        uint256 numTokensToLiquidity = _buyStock * 2;
...
contract SymplexiaToken is  Taxable {
   ...
   // swapRouterAddress
    ...
    constructor ()  ERC20("BETA53", "BTG53") BaseToken ( 
