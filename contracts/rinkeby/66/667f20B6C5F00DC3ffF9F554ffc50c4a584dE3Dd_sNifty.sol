...
interface IsNIFTY is IERC20 {
    function rebase(uint256 niftyProfit_, uint256 epoch_) external returns (uint256);
...
import "./interfaces/IgNIFTY.sol";
import "./interfaces/IsNIFTY.sol";
import "./interfaces/IStaking.sol";

contract sNifty is IsNIFTY, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgNIFTY public gNIFTY; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked NIFTY", "sNIFTY", 9) ERC20Permit("Staked NIFTY") {
    ...
    function setgNIFTY(address _gNIFTY) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gNIFTY) == address(0), "gNIFTY:  gNIFTY already set");
        require(_gNIFTY != address(0), "gNIFTY:  gNIFTY is not a valid contract");
        gNIFTY = IgNIFTY(_gNIFTY);
    }
    ...
    /**
        @notice increases rNIFTY supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sNIFTY of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sNIFTY
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sNIFTY: insufficient balance");
    }
    ...
    // toG converts an sNIFTY balance to gNIFTY terms. gNIFTY is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gNIFTY.balanceTo(amount);
    }

    // fromG converts a gNIFTY balance to sNIFTY terms. sNIFTY is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gNIFTY.balanceFrom(amount);
    }

    // Staking contract holds excess sNIFTY
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply
                .sub(balanceOf(stakingContract))
                .add(gNIFTY.balanceFrom(IERC20(address(gNIFTY)).totalSupply()))
