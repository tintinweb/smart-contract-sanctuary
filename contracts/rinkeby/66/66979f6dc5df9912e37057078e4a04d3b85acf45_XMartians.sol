...
contract XMartians is ERC721Enumerable, Ownable {
    ...
    */
    ...
    uint16 public maxMintQty = 20;
    uint16 public supplyCustom = 0;
    ...
    function batchGiveAwayAllowance(
        address[] calldata addr,
        uint8[] calldata count,
        uint8[] calldata collectionNumber
    ) external onlyOwner {
        ...
        _iterateAddToGiveAway(addr,count,collectionNumber);
    }
    
    function _iterateAddToGiveAway(
        address[] calldata addr,
        uint8[] calldata count,
        uint8[] calldata collectionNumber
    ) internal {
    ...
    function batchAddHolderReservedCount(
        ...
        _iterateAddHolderReservedCount(addr,count,collectionNumber);
    }
    
    function _iterateAddHolderReservedCount(
        address[] calldata addr,
        uint8[] calldata count,
        uint8[] calldata collectionNumber
    ) internal {
  ...
  // internal
    ...
    function mintCustom(uint8 mintQty) public payable { _mintCustom(mintQty); }
  ...
  // public
    function _iterateMint(uint8 subCollection, uint8 mintQty) internal {
        ...
        uint256 _afterMintSupply = _currentSupply + mintQty;
    ...
    function _mintTx(uint8 subCollection) internal {
        ...
        currentSubCollectionSupply[subCollection] = currentSubCollectionSupply[subCollection] + 1;

        _safeMint(_msgSender(), tokenId);

        urlToId[tokenId] = subCollection;
        distributePayment();
    }


    function _mintCustom(uint8 mintQty) internal {
        checkPaused();
        if(autoFloor){ _updateAutoFloorPrice(); }

        uint256 _mintAmount = 1;
        uint256 grandSupply = totalSupply();
        if (_msgSender() != owner()) {
            require(msg.value >= cost[4] * _mintAmount, "insufficient funds");
        }

        addressUrlType[grandSupply + 1] = 4;
        for (uint256 i; i < mintQty; i++) {
            _mintTx(4);
        }
        _safeMint(_msgSender(), grandSupply + 1);
        supplyCustom++;
        idUrlCustom[grandSupply + 1] = baseURIs[4];
        urlToId[grandSupply + 1] = supplyCustom;
    }
    ...
    function checkMaxSupply(uint256 afterMintSupply_) internal pure {
        require(afterMintSupply_ <= MAX_SUPPLY, "max NFT limit exceeded");
    }
    ...
    function checkSupplyAndReserved(uint8 collectionNumber, uint16 mintQty) internal {
        ...
        if(holderReserved_ == 0) {
            require(
                ...
                "max NFT limit exceeded for collection"
            );
        } else {
            require(
    ...
    function checkTxCost(uint256 msgValue, uint256 totalMintCost) internal pure {
        require(msgValue >= totalMintCost, "insufficient funds");
    }

    function checkMintQty(uint8 mintQty, uint16 maxMintQty_) internal pure {
        require(mintQty <= maxMintQty_, "XMartianNFT: checkMintQty() Exceeds Max Mint QTY");
    }

    function checkPaused() internal view {
        require(!paused || _msgSender() == owner(), "the contract is paused");
    }
    ...
    function checkEqualListLengths(
        ...
        pure
     {
        require(
            ...
            "XMartianNFT: checkEqualListLengths() list lengths do not match"
        );
     }
