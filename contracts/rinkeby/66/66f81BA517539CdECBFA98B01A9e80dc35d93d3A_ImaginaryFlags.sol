...
contract ImaginaryFlags is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    ...
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Flag #', Strings.toString(tokenId), '", "description": "Imaginary Flags is a collection of imaginary country flags generated and stored on chain.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
  ...
  constructor() ERC721("ImaginaryFlags", "IMF") Ownable() {
    _tokenIdCounter.increment();
  }
}
