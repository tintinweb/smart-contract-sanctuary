...
contract Doji1155Accounting is Ownable{
	...
	struct NFT1155ClaimInfo {
		address nftContract;
	  uint[] ercID;
	  uint[] balance;
	}
	mapping (uint256 => NFT1155ClaimInfo[]) public nft1155ClaimInfo;
	...
	function random1155(address _nftContract, uint _ercID, uint _amount) external onlyClaimContract {
	  require(_amount > 0);
	  uint256 luckyFuck = pickLuckyHolder();
	  for(uint256 i = 0; i < nft1155ClaimInfo[luckyFuck].length; i++) {
			if (nft1155ClaimInfo[luckyFuck][i].nftContract == _nftContract) {
				for(uint256 j = 0; j < nft1155ClaimInfo[luckyFuck][i].ercID.length; j++) {
					if (nft1155ClaimInfo[luckyFuck][i].ercID[j] == _ercID) {
						nft1155ClaimInfo[luckyFuck][i].balance[j] += _amount;
					}
				}
			} else {
				NFT1155ClaimInfo memory newClaim;
				newClaim = NFT1155ClaimInfo({
					nftContract: _nftContract,
					ercID: new uint[](0),
					balance: new uint[](0)
				});
				nft1155ClaimInfo[luckyFuck].push(newClaim);
				nft1155ClaimInfo[luckyFuck][nft1155ClaimInfo[luckyFuck].length].ercID.push(_ercID);
				nft1155ClaimInfo[luckyFuck][nft1155ClaimInfo[luckyFuck].length].balance.push(_ercID);
			}
	  }
	  emit LuckyHolder1155(luckyFuck, _nftContract, _ercID, _amount);
	}

	function send1155(address _nftContract, uint _ercID, uint _amount, uint256 chosenHolder) public {
		ERC1155(_nftContract).safeTransferFrom(msg.sender,  address(claimContract), _ercID, _amount, 'true');
	  for(uint256 i = 0; i < nft1155ClaimInfo[chosenHolder].length; i++) {
			if (nft1155ClaimInfo[chosenHolder][i].nftContract == _nftContract) {
				for(uint256 j = 0; j < nft1155ClaimInfo[chosenHolder][i].ercID.length; j++) {
					if (nft1155ClaimInfo[chosenHolder][i].ercID[j] == _ercID) {
						nft1155ClaimInfo[chosenHolder][i].balance[j] += _amount;
					}
				}
			} else {
				NFT1155ClaimInfo memory newClaim;
				...
				nft1155ClaimInfo[chosenHolder].push(newClaim);
				nft1155ClaimInfo[chosenHolder][nft1155ClaimInfo[chosenHolder].length].ercID.push(_ercID);
				nft1155ClaimInfo[chosenHolder][nft1155ClaimInfo[chosenHolder].length].balance.push(_ercID);
			}
	  }

		emit ChosenHolder1155(chosenHolder, _nftContract, _ercID, _amount);
	}

	function pickLuckyHolder() internal view onlyClaimContract returns (uint) {
		uint256 rando = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, claimContract._currentBaseTokensHolder())));
		return (rando % claimContract._currentBaseTokensHolder()) + 1;
	}

	function viewNFTsPendingForFlootID(uint ID)view external returns (NFT1155ClaimInfo[] memory) {
	  return nft1155ClaimInfo[ID];
	}

	function viewNFTsPendingForFlootIDByIndex(uint ID, uint index)view external returns (NFT1155ClaimInfo memory) {
	  return nft1155ClaimInfo[ID][index];
	}

	function viewNumberNFTsPendingForFlootID(uint ID) view external returns (uint) {
		return nft1155ClaimInfo[ID].length;
	}
	...
	// 	  for(uint256 j = 0; j < nft1155ClaimInfo[userInventory[index]].length; j++) {
	// 			  pending++;
	// 	  }
	//   }
	//   return pending;
...
contract DojiClaimsProxy is Initializable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    uint256 public currentBaseTokensHolder;
    bool public halt;
    ...
    struct BaseTokenIDinfo {
        uint256 tokenID;        // ID of BaseToken token
    ...
    struct PayoutInfo {
        ...
        uint256 distroPerBaseToken;      // amount each BaseToken is entitled to
        uint256 paidOut;            // total paid out to BaseTokens
    }
    mapping (uint256 => mapping (uint256 => BaseTokenIDinfo)) public baseTokenIDinfo;     // keeps track of pending and claim rewards
    ...
    function initialize(address _baseToken, address _NFT721accounting, address _NFT1155accounting) public initializer {
      ...
      updateNewHolders(baseToken.totalSupply(),0);
    }
    ...
    function onERC1155Received(
        ...
        if (data.length == 0){
          NFT1155accounting.random1155(operator, tokenID, value);
        }
    ...
    function updatePayout(uint256 _pid) public {
        ...
        uint256 baseTokenSupply = baseToken.totalSupply();
        require(baseTokenSupply > 0, "No one owns baseToken yet");
        uint256 totalDebt;
        uint256 newBaseTokens;
        
        if(baseTokenSupply > currentBaseTokensHolder){
            newBaseTokens = baseTokenSupply - currentBaseTokensHolder;
            updateNewHolders(newBaseTokens, _pid);
        }
        ...
        for (uint256 tokenIndex = 0; tokenIndex < baseTokenSupply; tokenIndex++) {
            uint tokenID = baseToken.tokenByIndex(tokenIndex);
            totalPaidOut += baseTokenIDinfo[_pid][tokenID].paidOut;
            totalDebt += baseTokenIDinfo[_pid][tokenID].rewardDebt;
        }
        ...
        payout.distroPerBaseToken = totalDistro * 1000 / baseTokenSupply;
        payout.balance = totalDistro;
    }
    
    function updateNewHolders(uint256 newBaseTokens, uint256 _pid) internal {
        PayoutInfo storage payout = payoutInfo[_pid];
        uint256 baseTokenSupply = baseToken.totalSupply();

        for (uint256 tokenIndex = currentBaseTokensHolder; tokenIndex < baseTokenSupply; tokenIndex++) {
            uint tokenID = baseToken.tokenByIndex(tokenIndex);
            baseTokenIDinfo[_pid][tokenID].rewardDebt = payout.distroPerBaseToken / 1000;
            baseTokenIDinfo[_pid][tokenID].tracked = true;
        }
        
        currentBaseTokensHolder += newBaseTokens;
    }

    function _currentBaseTokensHolder() view external returns (uint) {
      return currentBaseTokensHolder;
    }
    ...
    function claimAcctPending(uint256 _pid) public {
        ...
        uint256[] memory userInventory = baseToken.walletInventory(msg.sender);
        require(userInventory.length > 0);
        uint256 pending = payout.distroPerBaseToken * userInventory.length / 1000;
        ...
        for (uint256 index = 0; index < userInventory.length; index++) {
          paidout += baseTokenIDinfo[_pid][userInventory[index]].paidOut;
          rewardDebt += baseTokenIDinfo[_pid][userInventory[index]].rewardDebt;
        }
        ...
        for (uint256 index = 0; index < userInventory.length; index++) {
          baseTokenIDinfo[_pid][userInventory[index]].paidOut += payoutPerTokenID; 
        }
    ...
    function claimTokenPending(uint256 _pid, uint256 _tokenID) public {
        require(!halt, 'Claims temporarily unavailable');
        require(baseToken.ownerOf(_tokenID) == msg.sender);
        
        updatePayout(_pid);
        
        PayoutInfo storage payout = payoutInfo[_pid];

        uint256 pending = payout.distroPerBaseToken / 1000;
        uint256 paidout = baseTokenIDinfo[_pid][_tokenID].paidOut;
        uint256 rewardDebt = baseTokenIDinfo[_pid][_tokenID].rewardDebt;
        ...
        // add new payout to each tokenID's paid balance 
        baseTokenIDinfo[_pid][_tokenID].paidOut += claimAmount;
    ...
    function viewTokenPending(uint256 _pid, uint256 _id) public view returns(uint256){
        PayoutInfo storage payout = payoutInfo[_pid];
        if(!baseTokenIDinfo[_pid][_id].tracked){
            return 0;
        }
        //uint256 pending = viewLatestClaimAmount(_pid) / 1000;
        uint256 pending = payout.distroPerBaseToken / 1000;
        uint256 paidout = baseTokenIDinfo[_pid][_id].paidOut;
        uint256 rewardDebt = baseTokenIDinfo[_pid][_id].rewardDebt;
    ...
    function addPayoutPool(address _payoutToken) public onlyOwner {
        payoutInfo.push(PayoutInfo({
            ...
            balance: 0,
            pending: 0,
            distroPerBaseToken: 0,
            paidOut: 0
        }));
        for (uint256 tokenIndex = 0; tokenIndex < baseToken.totalSupply(); tokenIndex++) {
            uint tokenID = baseToken.tokenByIndex(tokenIndex);
            baseTokenIDinfo[payoutInfo.length - 1][tokenID].tracked = true;
        }
    }
    
    function rescueTokens(address _recipient, IERC20Upgradeable _ERC20address, uint256 _amount) public onlyOwner returns(bool) {
...
contract Doji721Accounting is Ownable {
    event LuckyHolder(uint256, address);
