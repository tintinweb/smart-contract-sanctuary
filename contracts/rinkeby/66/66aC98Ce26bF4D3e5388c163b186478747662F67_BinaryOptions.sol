...
contract BinaryOptions is Ownable, Pausable, Oracle {
    ...
    uint256 dvdDecimalAdjust = 10 ^ 12;
    ...
    //all events
    event Bid(address biddingAddress, uint256 amount);
    event BidTry(address biddingAddress, uint256 amount);
    event ResultLong(uint256 strike, uint256 actual);
    event ResultShort(uint256 strike, uint256 actual);
    event PriceFeed(uint256);
    ...
    function getDVDBalance(address _address)
        external
        view
        onlyOwner
        returns (uint256)
