...
contract TraumaLlamas is ERC721, Ownable {
  ...
  modifier doesNotExceedMaxPurchaseCount(uint256 numberOfTokens) {
    require(MAX_PER_TRANSACTION >= numberOfTokens, 'Can only mint 20 Llamas at a time');
    _;
  }
  ...
  function editPresaleReserved(address[] memory _addr, uint256[] memory _amount)
    public
    onlyOwner
  {
  ...
  function mintReservedLlamas(address _to, uint256 _reserveAmount) public onlyOwner {
    require(
  ...
  function mintPublicPresale(uint256 numberOfTokens)
    public
    payable
    validatePurchasePrice(numberOfTokens)
    mintCountMeetsPresaleSupply(numberOfTokens)
