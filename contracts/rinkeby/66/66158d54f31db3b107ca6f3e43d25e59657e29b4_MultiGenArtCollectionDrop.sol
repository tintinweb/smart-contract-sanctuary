...
contract MultiGenArtCollectionDrop is
    Context,
    ERC165,
    IERC721,
    IERC721Metadata,
    ...
    Ownable
{
    ...
    modifier onlyArtist(uint256 _collectionId) {
        require(
            ...
            "MultiGenArtCollectionDrop: only artist can call this function"
        );
        _;
    }
    ...
    function checkMint(
        ...
        require(
            ...
            "MultiGenArtCollectionDrop: incorrect collection id"
        );
        uint256 counter;
        ...
        require(
            ...
            "MultiGenArtCollectionDrop: max invocations reached"
        );

        address _membershipOwner = _genArtInterface.ownerOf(_membershipId);
        require(
            _membershipOwner == _sender,
            "MultiGenArtCollectionDrop: sender must be membership owner"
        );
        ...
        require(
            ...
            "MultiGenArtCollectionDrop: no valid membership"
        );
        uint256 maxMint = getAllowedMintForMembership(_groupId, _membershipId);
        require(
            maxMint >= _amount,
            "MultiGenArtCollectionDrop: no mints avaliable"
        );
    ...
    function checkFunds(
        ...
        if (_isEthPayment) {
            require(
                ...
                "MultiGenArtCollectionDrop: incorrect amount sent"
            );
        } else {
            require(
                _collectionGroupsMap[_groupId].priceGen.mul(_amount) <= _value,
                "MultiGenArtCollectionDrop: insufficient $GEN balance"
            );
        }
    }
    ...
    function mintGen(
        ...
        require(
            _genAllowed,
            "MultiGenArtCollectionDrop: Mint with $GENART not allowed"
        );
    ...
    // function setGroupPaused(uint256 _groupId, bool _paused) public onlyOwner {
    //     _collectionGroupsMap[_groupId].paused = _paused;
    // }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = MultiGenArtCollectionDrop.ownerOf(tokenId);
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        require(
            _msgSender() == owner,
            "MultiGenArtCollectionDrop: only token owner can burn"
        );
    ...
    function _transfer(
        ...
        require(
            MultiGenArtCollectionDrop.ownerOf(tokenId) == from,
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(MultiGenArtCollectionDrop.ownerOf(tokenId), to, tokenId);
    }
    ...
    {
        require(
            index < MultiGenArtCollectionDrop.balanceOf(owner),
    ...
    {
        require(
            index < MultiGenArtCollectionDrop.totalSupply(),
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = MultiGenArtCollectionDrop.balanceOf(to);
        ...
        private
    {
        ...
        uint256 lastTokenIndex = MultiGenArtCollectionDrop.balanceOf(from) - 1;
