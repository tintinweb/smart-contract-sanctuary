...
contract KatanaNSamurai is ERC721, Ownable {
	...
	using SafeMath for uint256;
	uint public MAX_PUNKS = 10000;
	uint public MAX_GIVEAWAYS = 500; 
	uint public PRICE = 50000000000000000; // 0.05 ETH
	uint public numGiveaways = 0;
	uint public numWhitelist = 0;
	bool public hasSaleStarted = false;
	bool public hasPresaleStarted = false;

	mapping (string => bool) public validwhitelist;
	...
	function Pre_mintSamurai(uint256 numPurchase, string memory name) public payable {
		require(hasPresaleStarted == true, "Sale hasn't started.");
		require(totalSupply() < MAX_PUNKS, "Sold out!");
		require(numPurchase > 0 && numPurchase <= 50, "You can mint minimum 1, maximum 50 punks.");
		require(totalSupply().add(numPurchase) <= MAX_PUNKS, "Exceeds MAX_PUNKS.");
		require(validwhitelist[name] == true);
		require(msg.value >= PRICE.mul(numPurchase), "Ether value sent is below the price.");
	...
	function addWhitelistName(string memory name) public onlyOwner {
		validwhitelist[name] = true;
		numWhitelist = numWhitelist.add(1);
	}

	// Delete valid shogunate name
	function deleteWhitelistName(string memory name) public onlyOwner {
		validwhitelist[name] = false;
		numWhitelist = numWhitelist.sub(1);
	}
	...
	function get_MAX_PUNKS(uint _MAX_num) public onlyOwner {
		MAX_PUNKS = _MAX_num;
	}

	function get_MAX_GIVEAWAYS(uint _MAX_GA_num) public onlyOwner {
		MAX_GIVEAWAYS = _MAX_GA_num;
	}

	function get_PRICE(uint _price) public onlyOwner {
		PRICE = _price;
	}
