...
contract ChainlinkRandoms is AccessControl, IRandoms, VRFConsumerBase {
    ...
    function getRandomSeed(address user) external view override returns (uint256) {
        return uint256(keccak256(abi.encodePacked(user, seed, block.timestamp, blockhash(block.number))));
    }

    function getRandomSeedWithNonce(address user, uint256 nonce) external view override returns (uint256) {
        return uint256(keccak256(abi.encodePacked(user, nonce, seed, block.timestamp, blockhash(block.number))));
    }
