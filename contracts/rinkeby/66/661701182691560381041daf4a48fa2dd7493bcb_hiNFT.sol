...
contract hiNFT is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 private MAX_SUPPLY = 8888;
    uint256 private PRICE = 0.025 ether;
    uint256 private MAX_PER_TX = 10;
    ...
    string memory _initNotRevealedUri) ERC721P('hiNFT', 'hiNFT') {setBaseURI(_initBaseURI); setNotRevealedURI(_initNotRevealedUri);}
    ...
    function mint(uint256 _mintAmount) external payable nonReentrant {
        ...
        require(_mintAmount <= MAX_PER_TX, "Minted amount exceeds transaction limit" );
    ...
    function tokenURI(uint256 _tokenId) external view virtual override returns (string memory) {
       ...
       if(revealed == false) {
           return string(abi.encodePacked(notRevealedUri, _tokenId.toString(), ".json"));
