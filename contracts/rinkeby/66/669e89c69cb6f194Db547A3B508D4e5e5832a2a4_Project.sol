...
contract Project is ERC721Namable, Ownable {
    ...
    uint256 constant public nftPerAddressLimitPresale = 3; // maximum number of mint per wallet for presale
    uint256 public cost = 0.08 ether;
    uint256 public maxMintPerTxn = 10; // maximum number of mint per transaction
    uint256 public sequenceSupply; // from 0
	uint256 public bebeCount; // baby count
    ...
    mapping(address => uint256) public presaleAddressMintedAmount; // number of NFT minted for each wallet during presale
    ...
    function devMint(uint256 _mintAmount) external onlyOwner onlyStart {
        require(sequenceSupply + _mintAmount <= MAX_GENESIS_SUPPLY, "exceed totalSupply");
		yieldToken.updateRewardOnMint(msg.sender);

        for (uint256 i = 1; i <= _mintAmount; i++) {
            sequenceSupply++;
            require(sequenceSupply <= MAX_GENESIS_SUPPLY, "exceed totalSupply");
            _safeMint(msg.sender, sequenceSupply);
        }
        balanceOG[msg.sender] += _mintAmount;
    }
    
    function giveaways(Drop[] calldata drops) external onlyOwner onlyStart {
        require(balanceOG[msg.sender] >= drops.length, "balance not enough");
        for (uint256 i = 0; i < drops.length; i++) {
          require(ownerOf(drops[i].tokenId) == msg.sender);
          safeTransferFrom(msg.sender, drops[i].dest, drops[i].tokenId, "0x00");
        }
    }
	...
	function breed(uint256 _sire, uint256 _matron) external {
		...
		bebeCount++;
        uint256 kidId = MAX_GENESIS_SUPPLY + bebeCount;
    ...
    function publicMint(uint256 _mintAmount) external payable onlyStart {
        ...
        require(sequenceSupply + _mintAmount <= MAX_GENESIS_SUPPLY, "exceed OGSupply");
