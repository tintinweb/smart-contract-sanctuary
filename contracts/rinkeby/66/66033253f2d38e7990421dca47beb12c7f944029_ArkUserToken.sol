...
contract ArkToken is Ownable, ERC1155Base {
    string public name;
    ...
    function mint(address user, uint256 id, Fee[] memory fees, uint256 supply, string memory uri) public {
...
contract ArkUserToken is ArkToken {
    event CreateERC1155_v1(address indexed creator, string name, string symbol);

    constructor(string memory name, string memory symbol, string memory contractURI, string memory tokenURIPrefix) ArkToken(name, symbol, contractURI, tokenURIPrefix) public {
        emit CreateERC1155_v1(msg.sender, name, symbol);
    }

    function mint(address user, uint256 id, Fee[] memory fees, uint256 supply, string memory uri) onlyOwner public {
        super.mint(user, id, fees, supply, uri);
    }
}