    ...
    function createTokenIdHash(uint256 tokenId) public view returns (Hash memory) {
        ...
        return
            Hash({
                ...
                bandana: weightedRarityGenerator(uint48(uint48(pseudoRandomBase >> 96) % 6969), 2) == 1,
                bandanaColor: uint48(uint48(pseudoRandomBase >> 144) % 8),
                boots: weightedRarityGenerator(uint48(uint48(pseudoRandomBase >> 192) % 6969), 3) == 1,
                bootsColor: uint48(uint48(pseudoRandomBase >> 240) % 8),
                pupil: uint48(uint48(pseudoRandomBase) % 4),
                tongue: weightedRarityGenerator(uint48(uint48(pseudoRandomBase) % 6969), 4) == 1,
                tail: uint48(uint48(pseudoRandomBase) % 2) == 1
            });
    }
