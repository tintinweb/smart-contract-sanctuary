    ...
    ReentrancyGuardUpgradeable
{
    ...
    function buyCoverInsurace(IDistributor.BuyInsuraceQuote memory quote) external override payable nonReentrant {
        ...
        if (quote.currency != ETH){
            ...
            IERC20Upgradeable token = IERC20Upgradeable(quote.currency);
            address  masterInsurace = address(masterCover);
            
             require(token.allowance(msg.sender,address(this)) >= quote.premium,
                ...
                if (token.allowance(address(this), masterInsurace) != uint256(0)) {
                          token.approve(masterInsurace, uint256(0));
                }
                    // set to maxint
                token.approve(masterInsurace, quote.premium);
