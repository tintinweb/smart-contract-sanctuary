...
struct TStakableNftCollectionInfo
{
    ...
    uint256     whitelisterPeriodGain;  // for example add +2 PEZ if the person buys during a whitelisting period
...
struct TNftStaker
{
    ...
    string      collection;
    uint256     baseDurationGain;       // 10 normally, or 12 if whitelisted guy
}
...
contract Stakable   is Ownable,UsageGuard
{
    ...
    uint256     internal    STAKING_HALVING_TIMESTAMP = 1956528000;                         // January 1st 2032 at 00:00:00 UTC
    uint256     internal    STAKING_END_TIMESTAMP     = 2019686400;                         // January 1st 2034 at 00:00:00 UTC (no more staking)
                           ...
                           bool isCalledByAdmin)                        internal returns(bool)
    {
        ...
        if (!isForbiddenWalletForApp(collectionName) || isCalledByAdmin)
        {
            ...
            if (INftToken(nft.collectionAddress).isWalletHasNftTokenId(userWallet, tokenId))
            {
                ...
                else 
                {
                    ...
                    if (INftToken(nft.collectionAddress).isWhiteListUserDuringWhitelistingPeriod(userWallet))
                    {
                        staker.periodGain = nft.whitelisterPeriodGain;      // BRAVO you've gained more tokens per period!!
                    }
                    else        // standard mode
                    {
    ...
    //----------------------------------------------------------------------
    function    collectNftStakeRewards(string memory collectionName, uint256[] memory tokenIds) public view returns(TNftReward[] memory)
    {
        TStakableNftCollectionInfo memory nft = stakableNftCollections[collectionName];
        ...
        for (uint i=0; i<n; i++)
        {
            ...
            if (INftToken(nft.collectionAddress).isWalletHasNftTokenId(msg.sender, tokenId))
            {
                uint256 stakingHash = forgeNftStakingHash(collectionName, msg.sender, tokenId);
            
                uint256 reward = calculateRewardForNft(nftStakers[stakingHash], collectionName);
    ...
    //----------------------------------------------------------------------
    function    calculateRewardForNft(TNftStaker memory staker, string memory collectionName) internal view returns(uint256)
    {
        ...
        TStakableNftCollectionInfo memory collection = stakableNftCollections[ collectionName ]; 
        
        if (timeNow < collection.endTimestamp)                      // Ya du reward a recuperer
        {
            if (timeNow < STAKING_HALVING_TIMESTAMP)                // on est dans la periode normale
            {
                reward = (age * staker.periodGain) / staker.periodDurationInSec;
            }
            else 
            {
                if (since >= STAKING_HALVING_TIMESTAMP)             // on est dans la periode de Halving
                {
                    reward = ((age * staker.periodGain) / staker.periodDurationInSec) / 2;
                }
                else                                                // on est entre les deux. Faire 2 calcules
                {
                    age    = STAKING_HALVING_TIMESTAMP - since;
                    reward = (age * staker.periodGain) / staker.periodDurationInSec;        // gain sur la tranche normale
                    
                    age     = timeNow - STAKING_HALVING_TIMESTAMP;
...
contract    PEZToken    is  Context, IERC20, IERC20Metadata , Ownable, ReentrancyGuard, Stakable//, IPEZToken
{
    ...
    function    claimNftRewards(string memory collectionName, uint256[] memory tokenIds) external
    {
        claimNftRewardsEx(collectionName, tokenIds);
    }
    //---------------------------------------------------------------------
    function    claimNftRewardsEx(string memory collectionName, uint256[] memory tokenIds) public
    {
        uint256 n = tokenIds.length;
        
        require(n<=100, "Too many NFTs to be claimed at once");
        
        TNftReward[] memory rewards = collectNftStakeRewards(collectionName, tokenIds);
        ...
        for (uint256 i=0; i<n; i++)
        {
            ...
            if (staker.isStaking==false)            continue;       // ne pas donner de reward, car il appartient plus ou pas staker
            ...
            emit ClaimedNftReward(msg.sender, reward, staker.collection);
        }
    }
    //----------------------------------------------------------------------
