...
abstract contract GovernancePrimitive is Component {
    ...
    event GovernancePrimitiveStarted(uint256 indexed executionId, Execution execution);
    event GovernancePrimitiveExecuted(uint256 indexed executionId, Execution execution);
...
contract Processes is UpgradableComponent { 
    ...
    event ProcessStarted(uint256 indexed executionId, GovernancePrimitive.Proposal proposal);
    event ProcessSet(string indexed nameSearch, string name, Process process);
        ...
        authP(PROCESSES_SET_ROLE) 
    {
        ...
        emit ProcessSet(name, name, process);
    }
