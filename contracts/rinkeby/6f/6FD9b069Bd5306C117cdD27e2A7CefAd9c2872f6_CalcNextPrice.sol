...
contract CalcNextPrice {
    ...
    function getNextPriceInUSD(address asset) public view returns (uint) {
        uint ETHUSDPrice = uint(AggregatorInterface(oracle.ETHUSDPriceFeed()).latestAnswer());
        uint AssetETHCourse = getNextCourseInETH(asset);

        // div 1e8 is chainlink precision for ETH
        return ETHUSDPrice.mul(AssetETHCourse).div(1e8);
    }

    function getNextCourseInETH(address asset) public view returns (uint) {
        if (asset == Registry(oracle.registry()).pETH()) {
            // ether always worth 1
            return 1e18;
        }

        return calcNextCourseInETH(asset);
    }

    function calcNextCourseInETH(address asset) public view returns (uint) {
        if (asset == Registry(oracle.registry()).pETH()) {
            // ether always worth 1
            return 1e18;
        }
        ...
        return getNextETHAmount(asset, amountIn);
    }

    function getNextETHAmount(address asset, uint amountIn) public view returns (uint) {
        ...
        FixedPoint.uq112x112 memory priceAverage = getNextPriceAverage(pair,asset);
        ...
        if (token0 == oracle.WETHToken() || token1 == oracle.WETHToken()) {
            ...
            if (token0 == asset) {
                ...
                return result.mul(oracle.getCourseInETH(token1)).div(10**power);
            } else {
                power = EIP20Interface(token0).decimals();
                return result.mul(oracle.getCourseInETH(token0)).div(10**power);
            }
        }
    }

    function getNextPriceAverage(address pair, address asset) public view returns (FixedPoint.uq112x112 memory) {
        (, , uint32 blockTimeStamp) = oracle.getReservesFromPair(asset);
        (FixedPoint.uq112x112 memory priceAverageFromOracle, uint priceCumulativePrevious, uint32 blockTimeStampPrevious) = oracle.cumulativePrices(pair, asset);

        if (blockTimeStamp == blockTimeStampPrevious) {
            return priceAverageFromOracle;
        }

        FixedPoint.uq112x112 memory priceAverage;
        uint32 timeElapsed = blockTimeStamp - blockTimeStampPrevious;

        if (asset == IUniswapV2Pair(pair).token0()) {
            priceAverage = FixedPoint.uq112x112(uint224((IUniswapV2Pair(pair).price0CumulativeLast() - priceCumulativePrevious) / timeElapsed));
        } else {
            priceAverage = FixedPoint.uq112x112(uint224((IUniswapV2Pair(pair).price1CumulativeLast() - priceCumulativePrevious) / timeElapsed));
        }
    ...
    function getPoolPriceAverage(address pair_, address asset) public view returns (FixedPoint.uq112x112 memory) {
        ...
        if (asset == pair.token0()) {
            uint currentPrice = uint(FixedPoint.fraction(reserve1, reserve0)._x);

            priceAverage = FixedPoint.uq112x112(uint224((currentPrice)));
        } else {
            uint currentPrice = uint(FixedPoint.fraction(reserve0, reserve1)._x);
