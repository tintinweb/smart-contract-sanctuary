...
contract Niftysistas is ERC721, Ownable, Pausable {
    ...
    uint[] private lowerBounds = [797, 778, 773, 603, 300, 185, 149, 100, 734, 632, 476, 424, 241];
    uint[] private upperBounds = [815, 796, 777, 631, 423, 240, 184, 148, 772, 733, 602, 475, 299];
    ...
    function purchase(uint[] memory singleTraits, uint[] memory bodyTraits, uint[] memory dresstop, uint[] memory dressbottom, uint[] memory accessoires, uint[] memory face) external payable {
        ...
        require(!paused() || msg.sender == owner() || isWhitelisted[msg.sender], "Purchases are paused.");
    ...
    function whitelist(address addressToWhitelist, bool whitelist) onlyOwner external {
        isWhitelisted[addressToWhitelist] = whitelist;
    }
    ...
    function detachTrait(uint sistaid, uint[] memory traitsToDetach) external returns (bool) {
    ...
    function assignDetachedTrait(uint sistaid, uint[] memory traitid, uint[] memory positionAfter) external returns (uint) {
        ...
        for (uint i=0; i < traitid.length; i++) {
            traitsContract.burn(msg.sender, traitid[i]);
        }
