...
interface AlinxNFT is IERC721 {
    function chest(address to) external;

    function multiChest(address to, uint256 amount) external;
...
contract AlinxNFTchester is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IAlinxRouter public alinxRouter;
    AlinxNFT public alinxToken;
    ...
    constructor(
        address _alinxRouter,
        address _alixToken,
        address _alinxToken
    ) {
        alinxRouter = IAlinxRouter(_alinxRouter);
        alixToken = IERC20(_alixToken);
        alinxToken = AlinxNFT(_alinxToken);
        _generateKey(bytes32(block.timestamp));
        seedKey = bytes32("AlinxNFTchester");
    }
    ...
    function setManager(address _config) public onlyOwner {
        alinxRouter = IAlinxRouter(_config);
    }

    function setNFT(address _alinxToken) public onlyOwner {
        alinxToken = AlinxNFT(_alinxToken);
    }

    function setERC20(address _alixToken) public onlyOwner {
        alixToken = IERC20(_alixToken);
    }

    function chestMultiple(uint256 _amount) internal {
        uint256 totalFee = alinxRouter.priceKey().mul(_amount);
        if (totalFee > 0) {
            alixToken.transferFrom(
                _msgSender(),
                alinxRouter.feeAddress(),
                totalFee
            );
        }
        if (_amount == 1) {
            alinxToken.chest(_msgSender());
        } else {
            alinxToken.multiChest(_msgSender(), _amount);
        }
        _generateKey(bytes32(block.timestamp));
    }
    ...
    function buy(uint256 _amount) public {
        ...
        chestMultiple(_amount);
    }

    function openChest(uint256 _tokenId) external {
        if (
            _msgSender() != tx.origin ||
            alinxToken.ownerOf(_tokenId) != tx.origin ||
            alinxToken.ownerOf(_tokenId) != _msgSender() ||
            Address.isContract(_msgSender()) ||
            Address.isContract(alinxToken.ownerOf(_tokenId))
        ) {
            revert("reject");
        }

        uint256 fee = alinxRouter.feeChest();

        if (fee > 0) {
            alixToken.transferFrom(_msgSender(), alinxRouter.feeAddress(), fee);
        }
        uint256 rnd = random(privateKey, 4);
        uint8 level = uint8(parserLevel(rnd));
        alinxToken.openChest(_tokenId, _msgSender(), level);
    ...
    function parserLevel(uint256 _random) internal pure returns (uint256) {
        ...
        if (_random > 9899) {
            return 5;
        } else if (_random > 8699) {
            return 4;
        } else if (_random > 6599) {
            return 3;
        } else if (_random > 4799) {
            return 2;
        } else {
            return 1;
        }
    ...
    function adminOpenChest(uint256 _tokenId, uint8 _level) public onlyOwner {
        alinxToken.openChest(_tokenId, alinxToken.ownerOf(_tokenId), _level);
    }
