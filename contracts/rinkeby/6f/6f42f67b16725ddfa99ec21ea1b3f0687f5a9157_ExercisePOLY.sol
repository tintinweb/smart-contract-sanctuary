...
contract ExercisePOLY {
    ...
    function getPOLYAbleToClaim( address _vester ) public view returns (uint) {
        uint circulatingSupply = IERC20( OHM ).totalSupply();
        for ( uint i = 0; i < nonCirculatingAddresses.length; i++ ) {
            uint balance = IERC20( OHM ).balanceOf( nonCirculatingAddresses[i] );
            circulatingSupply.sub( balance );
        }
        return ( circulatingSupply.mul( percentCanVest[ _vester ] ).mul( 1e9 ).div( 10000 ) ).sub( amountClaimed[ _vester ] );
    }

    function addNonCirculatingAddress( address _address ) public returns ( bool ) {
        require( msg.sender == owner, "Sender is not owner" );
        nonCirculatingAddresses[ nonCirculatingAddresses.length ] = _address;
        return true;
    }
}