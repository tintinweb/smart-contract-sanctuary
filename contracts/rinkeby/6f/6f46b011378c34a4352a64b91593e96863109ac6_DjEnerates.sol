...
contract DjEnerates is ERC721 {
    ...
    uint128 private resDjens = 100; //total reserve token for rewards and wages
     ...
     */
    function reserveDjens() public onlyOwner {        
        require(availableTokens.length >= resDjens, "No tokens left to reserve");
        require(hasReserved==false,"100 DJens have been already reserved, you can not reserve more");
        uint256[] memory tokenIds = new uint256[](resDjens);

        totalMintedTokens += resDjens;

        for (uint256 i; i < resDjens; i++) {
