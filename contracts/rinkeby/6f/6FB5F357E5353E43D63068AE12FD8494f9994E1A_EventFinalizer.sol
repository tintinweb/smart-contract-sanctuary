...
contract EventFinalizer is VRFConsumerBase, AccessControl {
  struct Redeem {
    address requestor;
    address nft;
  }
  ...
  modifier onlyAdmin() {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "EventFinalizer: caller is not admin");
    _;
  }

  modifier onlyIfEventClosed(address _nft) {
    require(eventStore.isEventClosed(_nft), "EventFinalizer: event is not closed");
    _;
  }
  ...
  constructor(
    ...
    VRFConsumerBase(_vrfCoordinator, _link) {
      require(_eventStore != address(0), "EventFinalizer: _eventStore is zero address");
  ...
  function redeem(uint256 _seed, address _nft) external onlyIfEventClosed(_nft) {
    require(LINK.balanceOf(address(this)) >= linkFee, "EventFinalizer: not enough LINK to pay fee");
    require(redeemies[_nft][_msgSender()] == 0, "EventFinalizer: redeem already in progress");

    bytes32 requestId = requestRandomness(linkKeyHash, linkFee, _seed);

    redeemies[_nft][_msgSender()] = REDEEM_IN_PROGRESS;
    redeemers[requestId] = Redeem(_msgSender(), _nft);

    emit RedeemRequested(_nft, _msgSender());
  }

  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    Redeem memory redeemer = redeemers[requestId];

    require(redeemies[redeemer.nft][redeemer.requestor] == REDEEM_IN_PROGRESS, "EventFinalizer: redeem already ended");
    redeemies[redeemer.nft][redeemer.requestor] = 0;
    
    uint256 count = eventStore.getTicketCount(redeemer.nft, redeemer.requestor);
    for(uint256 i = 0; i < count; i++) {
      uint256 tokenId = eventStore.swapTicketToNft(redeemer.nft, redeemer.requestor, randomness);
      EventErc1155(redeemer.nft).mint(redeemer.requestor, tokenId, 1);

      emit RedeemFilled(redeemer.nft, redeemer.requestor, tokenId);
    }
  }
