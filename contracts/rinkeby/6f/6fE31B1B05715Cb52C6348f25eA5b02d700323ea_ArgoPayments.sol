...
contract ArgoPayments is MultiOwnable, Pausable, GovernanceOwnable {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < slabAmounts_.length; i++) {
            Discount memory _discount =
                Discount(slabAmounts_[i], slabPercents_[i]);
    ...
    function charge(address u, uint256 b) public onlyManager whenNotPaused {
        ...
        uint256 total = initial.sub(discount);
    ...
    function _calculateFee(uint256 b) internal view returns (uint256) {
        uint256 _amount =
            (b.mul(buildTimeRate).mul(DECIMAL)).div(getUnderlyingPrice());
        ...
        returns (uint256)
    {
        ...
        uint256 buildPrice =
            (b.mul(buildTimeRate).mul(DECIMAL)).div(underlyingPrice);
        uint256 deploymentPrice = d.mul(DECIMAL).div(underlyingPrice);
        uint256 amount = buildPrice.add(deploymentPrice);
        ...
        returns (uint256)
    {
        ...
        if (discountsEnabled) {
            ...
            discount = amount.mul(percent.mul(PRECISION)).div(PERCENT);
        }
        return discount;
    }
