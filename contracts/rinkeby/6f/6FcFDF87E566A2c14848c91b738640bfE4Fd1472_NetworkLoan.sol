...
contract NetworkLoan is NetworkLoanBase {
    ...
    */
     function createLoanOffer(NetworkLoanData.LoanDetails memory loanDetails)
        public
        payable
        {
        ...
        require(msg.value > 0, "GLM: Loan Amount Invalid");
    ...
    function LoanAdjusted(
        ...
        require(borrowerOffers[_loanIdAdjusted].loanStatus == NetworkLoanData.LoanStatus.INACTIVE, "GLM, Loan cannot adjusted");
        require(borrowerOffers[_loanIdAdjusted].borrower == msg.sender, "GLM, Only Borrow Adjust Loan");
        _newLoanAmountBorrowed = msg.value;
     ...
     */
    function loanOfferCancel(uint256 _loanId) public payable {

        require(borrowerOffers[_loanId].loanStatus == NetworkLoanData.LoanStatus.INACTIVE , "GLM, Loan cannot be cancel");
        require(borrowerOffers[_loanId].borrower == msg.sender, "GLM, Only Borrow can cancel");
     ...
     */
    function activateLoanFull(uint256 _loanId, uint256 _stableCoinAmount, bool _autoSell) public {

        require(borrowerOffers[_loanId].loanStatus == NetworkLoanData.LoanStatus.INACTIVE, "GLM, not inactive");
        require(borrowerOffers[_loanId].borrower != msg.sender, "GLM, self activation forbidden");
        require(borrowerOffers[_loanId].loanAmountInBorrowed == _stableCoinAmount, "GLM, insufficient amount");
    ...
    function payback(uint256 _loanId, uint256 _paybackAmount) public {
        ...
        require(borrowerOffers[_loanId].borrower == msg.sender, "GLM, not borrower");
        require(borrowerOffers[_loanId].loanStatus == NetworkLoanData.LoanStatus.ACTIVE, "GLM, not active");
        require(_paybackAmount > 0  && _paybackAmount <= borrowerOffers[_loanId].loanAmountInBorrowed, "GLM: Invalid Loan Amount");
        ...
        {   
            ...
            IERC20(loanDetails.borrowStableCoin).transferFrom(msg.sender, address(this), _paybackAmount);
            ...
            emit PartialLoanPaybacked(loanId, _paybackAmount, borrower);
        }
    ...
    function liquidateBorrowerCollateral(uint256 _loanId) public onlyLiquidatorRole(msg.sender) {
        ...
        if(lenderDetails.autoSell  == true) {
            ...
            swapInterface.swapExactETHForTokensSupportingFeeOnTransferTokens(loanDetails.collateralAmount, path, address(this), block.timestamp + 5 minutes);
...
interface INetworkLoan {
    ...
    function getAltCoinPriceinStable(address _stableCoin,uint256 _collateralAmount) external view returns (uint256);

    /**
    @dev function that will get APY fee of the loan amount in borrower
     */
    function getAPYFee(NetworkLoanData.LoanDetails memory _loanDetails) external returns(uint256);
    ...
    event AutoLiquidated(uint256 _loanId, NetworkLoanData.LoanStatus loanStatus);
