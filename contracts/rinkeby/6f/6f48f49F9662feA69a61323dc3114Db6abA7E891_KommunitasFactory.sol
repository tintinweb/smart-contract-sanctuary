...
contract KommunitasFactory{
    ...
    function createProject(address _tokenProject) public onlyOwner returns(address project){
        ...
        bytes memory bytecode = type(KommunitasProject).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_tokenProject));
        assembly {
        ...
        emit ProjectCreated(_tokenProject, project, allProjects.length);
    }
