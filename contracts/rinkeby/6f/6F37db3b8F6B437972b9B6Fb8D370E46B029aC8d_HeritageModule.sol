...
contract HeritageModule is Module {
        ...
        onlyHeir
    {
        ...
        uint total = address(manager).balance;
        for (uint256 i = 0; i < heirs.length; i++) {
            uint heritage = (heirPercentage[heirs[i]] * total) / 100;
            manager.execTransactionFromModule(heirs[i], heritage, "0x", Enum.Operation.Call);
        }
        // Split ERC-20 tokens between heirs
        IERC20 token = IERC20(_erc20Addresses[0]);
        uint erc20Total = token.balanceOf(address(manager));
        if(erc20Total > 0) {
            for (uint256 i = 0; i < heirs.length; i++) {
                uint256 heritage = (heirPercentage[heirs[i]] * erc20Total) / 100;
                bytes memory data = abi.encodeWithSignature("transfer(address,uint256)", heirs[i], heritage);
                manager.execTransactionFromModule(_erc20Addresses[0], 0, data, Enum.Operation.Call);
            }
        }
    }
