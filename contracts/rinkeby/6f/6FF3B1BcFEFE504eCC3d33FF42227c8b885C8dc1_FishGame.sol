...
contract FishGame is ERC1155, Ownable, ReentrancyGuard{
    ...
    constructor(address _fishermanAddress,address _vrf,address _fillet,address _goodFish) ERC1155("https://game.example/api/item/{id}.json") {
    ...
    uint public coolDownTime = 2 minutes;// 1 days -> 2 minutes for testing
    ...
    function calculateWormOnGoodFish(uint tokenId) internal view returns(uint){
        uint goodFishCount = goodFish.balanceOf(_msgSender());
    ...
    function claimWormOnGoodFish(uint tokenId) external nonReentrant{
        require(fisherman.ownerOf(tokenId) == _msgSender(),"Caller doesn't own the token(s)");
        require(level[tokenId]>0, "Game not initiated yet");
        require (block.timestamp - coolDownForClaimingWithGoodFish[msg.sender] >= coolDownTime, "CoolDown time not over");
        if(lastClaimFishWorm[tokenId] == 0){
            ...
            coolDownForClaimingWithGoodFish[msg.sender] = block.timestamp;
        }
        else{
            require(lastClaimFishWorm[tokenId] + coolDownTime <= block.timestamp, "Cooldown time not exceeded");
            uint wormValue = calculateWormOnGoodFish(tokenId);
            wormValue = ((block.timestamp - lastClaimFishWorm[tokenId])/coolDownTime) * wormValue* 1 ether;
            _mint(_msgSender(), WORM, wormValue, "");
            lastClaimFishWorm[tokenId] = block.timestamp;
            coolDownForClaimingWithGoodFish[msg.sender] = block.timestamp;
        }
    ...
    function cooker(uint amount,uint tokenId) internal{
        ...
        // _mint(_msgSender(), FILLET, amount *1 ether,"");
    }

    // function mintFilletToOwner (uint _amount) external onlyOwner {
    //     _mint(_msgSender(), FILLET, _amount * 1 ether, "");
    // }
