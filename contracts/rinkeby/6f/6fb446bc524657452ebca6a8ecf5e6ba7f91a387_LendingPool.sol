    ...
    ILendingPool
{
        ...
        returns (uint256)
    {
        ...
        if (getBlockNumber() > pool.accrualBlockNumber && pool.totalBorrows != 0) {
            uint256 blockDelta = getBlockDelta(
                pool.accrualBlockNumber,
                getBlockNumber()
            );
        ...
        return totalBorrow.sub(deed.borrow);
    }
        ...
        nonReentrant
    {
        ...
        userAsset.rewardDebt = userAsset.amount.mul(pool.accDTokenPerShare).div(
            1e18
        );
        ...
        returns (uint256)
    {
        ...
        vars.simpleInteresSupplyFactor = supplyRate.mul(vars.blockDelta);
