...
contract PPAShuttlepass is ERC1155, Ownable {
  ...
  string public baseUri = "https://assets.jointheppa.com/shuttlepasses/metadata/{id}.json";
  ...
  function mint(uint passType, uint amount) public payable {
    ...
    */
    publicNumMinted[msg.sender] = SafeMath.add(publicNumMinted[msg.sender], amount);
  ...
  function earlyMint(uint passType, uint amount, uint whitelistType, bytes32[] calldata merkleProof) public payable {
    ...
    if (saleState == 1) {
       ...
       */
      sale1NumMinted[msg.sender] = SafeMath.add(sale1NumMinted[msg.sender], amount);
       ...
       */
      sale2NumMinted[msg.sender] = SafeMath.add(sale2NumMinted[msg.sender], amount);
  ...
  function _internalMint(uint passType, uint amount) internal {
    ...
    if (passType == GOLD) {
      checkPayment(SafeMath.mul(goldPriceWei, amount));
    }
    else if (passType == SILVER) {
      checkPayment(SafeMath.mul(silverPriceWei, amount));
    } 
    else if (passType == BRONZE) {
      checkPayment(SafeMath.mul(bronzePriceWei, amount));
    } else {
  ...
  function incrementNumMinted(uint amount) internal {
    numMinted = SafeMath.add(numMinted, amount);
