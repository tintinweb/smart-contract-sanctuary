...
import "./LibW80.sol";
import "./W80OperatorRole.sol";

contract ERC721W80V1 is ERC721, RoyaltiesV1Impl, Ownable, W80OperatorRole {
    mapping(uint256 => address[]) private creators;
    mapping(uint256 => LibW80.RequestStatus) private requestStatus;
    ...
    function saveFees(uint256 _id, LibPart.Part[] memory _fees)
        external
        onlyOwner
    {
        _saveFees(_id, _fees);
    ...
    function updateAccount(
        ...
        require(_msgSender() == _from, "ERC721W80: not allowed");
    ...
    function getRequestStatus(uint256 _id)
        external
        view
        returns (address _owner, LibW80.RequestStatus _requestStatus)
    {
        _owner = ownerOf(_id);
        _requestStatus = requestStatus[_id];
    }

    function setRequestStatus(uint256 _id, LibW80.RequestStatus _requestStatus)
        external
    {
        require(
            _msgSender() == owner() || _isW80Operator(),
            "ERC721W80: not allowed"
        );
        requestStatus[_id] = _requestStatus;
    }
    ...
    //         owner() == _msgSender() ||
    //             _isW80Operator() ||
    //             requestStatus[tokenId] == LibW80.RequestStatus.RECEIVED,
    //         "ERC721W80: should be received first"
    //     );
    ...
    function mintByOperator(
        ...
        LibPart.Part memory fee;
        fee.account = address(uint160(_creator));
        fee.value = _feeBp;
        ...
        _saveFees(tokenId, fees);
        creators[tokenId] = [owner(), _creator];
        requestStatus[tokenId] = LibW80.RequestStatus.ORDERED;
    }
}
...
contract W80OperatorRole is Ownable {
    mapping(address => bool) operators;

    function addW80Operator(address operator) external onlyOwner {
        operators[operator] = true;
    }

    function removeW80Operator(address operator) external onlyOwner {
        operators[operator] = false;
    }

    function _isW80Operator() internal view returns (bool) {
        return operators[_msgSender()];
    }

    modifier onlyW80Operator() {
        require(
