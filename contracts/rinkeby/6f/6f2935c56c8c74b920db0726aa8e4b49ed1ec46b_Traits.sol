...
contract Traits is Ownable, ITraits {
	...
	function drawSVG(uint256 tokenId) public view returns (string memory) {
		...
		string memory svgString = string(
			abi.encodePacked(
				s.isApe ? drawTrait(traitData[0 + shift][s.skin]) : drawTrait(traitData[0 + shift][s.alphaIndex]),
				...
				s.isApe ? drawTrait(traitData[4 + shift][s.headwear]) : drawTrait(traitData[4 + shift][s.headwear])
			)
		);
	...
	function testURI(uint256 tokenId) public view returns (string[8] memory) {
		IWoolf.ApeWolf memory s = woolf.getTokenTraits(tokenId);
