...
contract Thorgi is ERC20Burnable, Ownable {
    ...
    IERC721Enumerable private constant pupIERC721Enumerable = IERC721Enumerable(pupAddress);
    ...
    function stakeCorgis(uint256[] memory tokenIds) public stakingEnabled {
    ...
    function stakePups(uint256[] memory tokenIds) public stakingEnabled {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
            require(pupIERC721Enumerable.ownerOf(id) == msg.sender && corgiOwner[id] == address(0), "TOKEN_IS_NOT_YOURS");
            pupIERC721Enumerable.transferFrom(msg.sender, address(this), id);
    ...
    function stakeBones(uint256 tokenId) public stakingEnabled {
        require(boneTokenIds[msg.sender].length < 2, "You can only stake 1 bone");
        require(pupIERC721Enumerable.ownerOf(tokenId) == msg.sender && corgiOwner[tokenId] == address(0), "TOKEN_IS_NOT_YOURS");
    ...
    function unstakePups(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            pupIERC721Enumerable.transferFrom(address(this), msg.sender, id);
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = pupTokenIds[msg.sender].length; i > 0; i--) {
            ...
            pupIERC721Enumerable.transferFrom(address(this), msg.sender, pupTokenId);
            if(stakedBone.length > 0) {
    ...
    function testClaim(uint256 timestamp) public {
        uint256 reward = ((block.timestamp - timestamp) * corgiRatePerDay);
        _mint(msg.sender, reward);
    }

    function testMultiplier(uint256 timestamp) public view returns(uint256) {
        uint256 totalRewards = 0;
    ...
    // CLAIM FUNCTIONS
    function claimFromCorgi(uint256 tokenId) public {
        require(corgiOwner[tokenId] == msg.sender, "You are not the owner");
        uint256 totalRewards = 0;
        uint256[] memory stakedBone = getStakedBone(msg.sender);
        ...
        _mint(msg.sender, totalRewards);
        corgTimeStaked[tokenId] = block.timestamp;
    }
    
    function claimFromPup(uint256 tokenId) public {
        require(pupOwner[tokenId] == msg.sender, "You are not the owner");
        uint256 totalRewards = 0;
        uint256[] memory stakedBone = getStakedBone(msg.sender);
        ...
        _mint(msg.sender, totalRewards);
        pupTimeStaked[tokenId] = block.timestamp;
    }
    ...
    function checkRewardsFromCorgis(uint256 tokenId) public view returns (uint256) {
    ...
    function checkRewardsFromPups(uint256 tokenId) public view returns (uint256) {
    ...
    function checkAllRewards(address _address) public view returns (uint256) {
        uint256 totalRewards = 0;
        uint256[] memory stakedBone = getStakedBone(msg.sender);

        uint256[] memory corgiTokens = corgiTokenIds[_address];
        for (uint256 i = 0; i < corgiTokens.length; i++) {
            if(stakedBone.length > 0) {
                uint256 numOfDays = ((block.timestamp - corgTimeStaked[corgiTokens[i]]) / 1 days) * 1e18;
                uint256 reward = ((block.timestamp - corgTimeStaked[corgiTokens[i]]) * corgiRatePerDay);
                uint256 multiplier = 1e18 + (numOfDays * 150 / 10000);
                totalRewards += (reward * multiplier) / 1e18;
            } else { 
                totalRewards += ((block.timestamp - corgTimeStaked[corgiTokens[i]]) * corgiRatePerDay);
            }
        }
        
        uint256[] memory pupTokens = pupTokenIds[_address];
        for (uint256 i = 0; i < pupTokens.length; i++) {
             if(stakedBone.length > 0) {
