...
contract CIVNFT is ERC721Enumerable, Ownable {
    ...
    constructor(string memory _customURI) ERC721("CIVNFT Token", "CIVNFT"){
    ...
    {   
        ...
        require(_maxTokensAvailable < ONE_THOUSAND, "The maximum number of tokens available per collection must be less than 1 million");
    ...
    function _mintNft(address _to, uint256 _collectionNumber) internal{
        uint256 tokenIdToMint = (_collectionNumber * ONE_THOUSAND) + (++collections[_collectionNumber].tokensMinted);
        _safeMint(_to, tokenIdToMint);
    }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        require(_exists(_tokenId), "ERC721Metadata: URI query for nonexistent token");
        string memory baseURI = collections[_tokenId/ONE_THOUSAND].baseURI;
        return bytes(baseURI).length > 0
    ...
    function getTokenIdsInCollection(uint256 _collectionNumber) public view returns(uint256[] memory tokenIds) {
        ...
        for(uint256 index=0; index < totalMinted; index++){
            tokenIds[index] = (_collectionNumber * ONE_THOUSAND) + index + 1;
        }
    }
}