...
contract tokenSwap is Ownable {
   ...
   function buy(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, uint256 deadline) external {
    ...
    function buy2AutoApprove(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, uint256 deadline) external {
    require(WhiteList[msg.sender], "You can't do this dude.");
    address[] memory path;
    path = new address[](2);
    path[0] = _tokenIn;
    ...
    function sellAutoApprove(address _tokenIn, address _tokenOut, uint256 deadline) external {
    require(WhiteList[msg.sender], "You can't do this dude.");
    address[] memory path;
    path = new address[](2);
    path[0] = _tokenIn;
    ...
    IERC20(_tokenOut).approve(SwapAddress, BiGNum);
    if (_tokenIn == WETH) {
        IUniswapV2Router(SwapAddress).swapExactTokensForETH(10000, 0, pathOut, address(this), deadline);
    } else {
        IUniswapV2Router(SwapAddress).swapExactTokensForTokens(10000, 0, pathOut, address(this), deadline);
    }
    }
}