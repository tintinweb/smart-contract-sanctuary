...
contract forked from the beautiful cryptocoven contract
*/
...
contract BLOOMERS is ERC721, IERC2981, Ownable, ReentrancyGuard {
   ...
   modifier maxBloomsPerWallet(uint256 numberOfTokens) {
       require(
           balanceOf(msg.sender) + numberOfTokens <= MAX_BLOOMS_PER_WALLET,
           "Max blooms to mint is three"
       );
       _;
   }
 
   modifier canMintBlooms(uint256 numberOfTokens) {
       require(
           tokenCounter.current() + numberOfTokens <=
               maxBlooms - maxGiftedBlooms,
           "Not enough blooms remaining to mint"
       );
       _;
   }
 
   modifier canGiftBlooms(uint256 num) {
       require(
           numGiftedBlooms + num <= maxGiftedBlooms,
           "Not enough blooms remaining to gift"
       );
       require(
           tokenCounter.current() + num <= maxBlooms,
           "Not enough blooms remaining to mint"
       );
       _;
   }
   ...
   constructor(
       ...
       uint256 _maxBlooms,
       uint256 _maxCommunitySaleBlooms,
       uint256 _maxGiftedBlooms
   ) ERC721("BLOOMER", "BLOOM") {
       openSeaProxyRegistryAddress = _openSeaProxyRegistryAddress;
       maxBlooms = _maxBlooms;
       maxCommunitySaleBlooms = _maxCommunitySaleBlooms;
       maxGiftedBlooms = _maxGiftedBlooms;
   }
   ...
   function mint(uint256 numberOfTokens)
       ...
       publicSaleActive
       canMintBlooms(numberOfTokens)
       maxBloomsPerWallet(numberOfTokens)
   {
       ...
       isValidMerkleProof(merkleProof, communitySaleMerkleRoot)
   {
       ...
       require(
           numAlreadyMinted + numberOfTokens <= MAX_BLOOMS_PER_WALLET,
           "Max blooms to mint in community sale is three"
       );
 
       require(
           tokenCounter.current() + numberOfTokens <= maxCommunitySaleBlooms,
           "Not enough blooms remaining to mint"
       );
   ...
   function claim(bytes32[] calldata merkleProof)
       ...
       canGiftBlooms(1)
   {
       require(!claimed[msg.sender], "Bloom already claimed by this wallet");
   ...
   function reserveForGifting(uint256 numToReserve)
       ...
       nonReentrant
       onlyOwner
       canGiftBlooms(numToReserve)
   {
       numGiftedBlooms += numToReserve;
   ...
   function giftBlooms(address[] calldata addresses)
       external
       nonReentrant
       onlyOwner
       canGiftBlooms(addresses.length)
   {
       uint256 numToGift = addresses.length;
       numGiftedBlooms += numToGift;
   ...
   function rollOverBlooms(address[] calldata addresses)
       external
       nonReentrant
       onlyOwner
   {
       require(
           ...
           "All blooms are already rolled over"
       );
