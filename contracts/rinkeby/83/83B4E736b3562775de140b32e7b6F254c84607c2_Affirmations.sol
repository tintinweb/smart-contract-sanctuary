...
contract Affirmations is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    uint256 public maxAffirmationsPerMint = 11;
    ...
    function manifest(uint256 numberOfMints) public payable {
        if (whitelistStarted) require(numberOfMints == 1 && mintStartIndexDrawn, "Only 1 mint before public sale");
        else require(saleStarted && mintStartIndexDrawn, "Sale hasn't started");

        require(numberOfMints > 0 && numberOfMints <= maxAffirmationsPerMint, 'Can mint up to 11 affirmations at a time');
        require(balanceOf(msg.sender).add(numberOfMints) <= 33, 'Maximum 33 affirmations per wallet');
        require(totalSupply().add(numberOfMints) <= totalAffirmations, 'Not enough affirmations left');
