/**
Metafi Capital: $Metafi
- DeFi 3.0 with a thesis. A DAO to invest in the future of the metaverse!

Tokenomics:
5% of each buy/sell goes to existing holders.
5% of each buy/sell goes into the treasury.
...
contract MetaversiFi is Context, IERC20, Ownable {
  ...
  string private _name = 'MetaversiFi';
  string private _symbol = 'METAFI';
  ...
  address payable public _MetafiWalletAddress;
  ...
  uint8 _sellTaxMultiplier = 1; // 1 means that sell tax = buy tax
  ...
  constructor(
    address payable MetafiWalletAddress,
    address payable marketingWalletAddress
  ) {
    _MetafiWalletAddress = MetafiWalletAddress;
    _marketingWalletAddress = marketingWalletAddress;
    _rOwned[_msgSender()] = _rTotal; // start with all liquidity in the msg.sender's account
    ...
    uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(
      address(this),
      _uniswapV2Router.WETH() // creating a pair between metafi token and WETH on uniswap
    );
  ...
  function _transfer(
    ...
    if (
      ...
      !(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) // you interacting with uniswap
    ) {
      takeFee = true;
    }
  ...
  function sendETHToTeam(uint256 amount) private {
    _MetafiWalletAddress.call{ value: amount.div(2) }('');
  ...
  function _setMetafiWallet(address payable MetafiWalletAddress) external onlyOwner {
    _MetafiWalletAddress = MetafiWalletAddress;
  }
