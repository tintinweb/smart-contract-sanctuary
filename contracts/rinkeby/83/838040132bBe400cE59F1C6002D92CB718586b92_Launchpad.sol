...
// File: contracts/IVybeDAO.sol
// SPDX-License-Identifier: MIT
interface IVybeDAO {
    event NewProposal(uint64 indexed proposal);
    event FundProposed(
        uint64 indexed proposal,
    ...
    function addVote(uint64 proposalID) external;
    function removeVote(uint64 proposalID) external;
    function forwardVYBE() external;
    function completeProposal(uint64 proposalID, address[] calldata stakers) external;
...
// File: contracts/launchpad/MicroERC20.sol
contract MicroERC20 is ERC20 {
  constructor(string memory name, string memory symbol) ERC20(name, symbol) public {
    _mint(msg.sender, 100000 * 1e18);
  }
}
// File: contracts/launchpad/ILaunchpad.sol
interface ILaunchpad {
  event DevFundChanged(address indexed oldDevFund, address indexed newDevFund);
  event TokenDeployed(string indexed symbol, string indexed name, address indexed token);
  event NewProposal(uint64 indexed proposal);
  event SellProposed(
      uint64 indexed proposal,
  ...
  event ProposalPassed(uint64 indexed proposal);
  event ProposalRemoved(uint64 indexed proposal);
  function transferDevFund(address newDevFund) external;
  function deploy(string memory symbol, string memory name, uint256 vybePerToken) external returns (address);
  function proposeSell(address token, uint256 quantity, uint256 price) external returns (uint64);
  function addVote(uint64 proposalID) external;
  function removeVote(uint64 proposalID) external;
  function completeProposal(uint64 proposalID, address[] calldata stakers) external;
}
...
contract Launchpad is ReentrancyGuard, Ownable, ILaunchpad {
    ...
    constructor(address DAO, address devFund, address router) Ownable(msg.sender) public {
        _DAO = DAO;
        IVybeStake _stake = IVybeStake(IVybeDAO(_DAO).stake());
    ...
    function migrate(address token, address newLaunchpad) onlyOwner() public {
      IERC20(token).transfer(newLaunchpad, IERC20(token).balanceOf(address(this)));
    }
