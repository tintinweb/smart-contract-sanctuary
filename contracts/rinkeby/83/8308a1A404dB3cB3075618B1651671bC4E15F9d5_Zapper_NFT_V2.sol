...
contract Zapper_NFT_V2 is ERC1155, Ownable, SignatureVerifier_V2 {
    ...
    //Mapping from token ID to token ID of higher rarity of same type
    ...
    event Claim(address account, uint256 voltsRecieved, uint256 nonce);
    ...
    /**
     * @dev Adds a new NFT type
     * @dev set nextRarity after addition for crafting
     * @param cid Content identifier
     * @param _cost Cost in Volts for the NFT. If 0 it cannt be redeemed
     * @return id The newly created token ID
     */
    function add(string calldata cid, uint256 _cost)
        external
        onlyOwner
        returns (uint256 id)
    {
        require(bytes(cid).length > 0, "Missing Content Identifier");

        id = _nextId();

        string memory _uri = _createUri(cid);
        idToUri[id] = _uri;
        cost[id] = _cost;
     ...
     */
    function claim(uint256 voltsEarned, bytes calldata signature)
        external
        pausable
        beforeDeadline
    {
        ...
        emit Claim(msg.sender, voltsEarned, nonces[msg.sender]);
    }

    /**
     * @notice Maps the rarity classes for use in crafting
     * @dev toID should be 0 for max rarity classes
     * @param fromID An array of lower rarity IDs
     * @param toID An array of higher rarity IDs which can be
     * crafted from the fromID
     */
    function setNextRarity(uint256[] calldata fromID, uint256[] calldata toID)
        external
        onlyOwner
    {
        require(fromID.length == toID.length, "Mismatched array lengths");

        for (uint256 i = 0; i < fromID.length; i++) {
            nextRarity[fromID[i]] = toID[i];
        }
    }
    ...
    function mint(uint256 id, uint256 quantity) external pausable {
        ...
        require(voltBalance[msg.sender] >= voltsSpent);
        ...
        pausable
    {
        ...
        for (uint256 i = 0; i < ID.current(); i++) {
            require(_exists(ids[i]), "Invalid ID");
    ...
    function redeem(uint256 id, uint256 quantity) external pausable {
        ...
        tokenSupply[id]--;
        voltBalance[msg.sender] += (cost[id] * redeemModifier) / 10000;
    }
        ...
        pausable
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            voltsRecieved += (cost[ids[i]] * redeemModifier) / 10000;
            tokenSupply[ids[i]]--;
        }
    ...
    function craft(uint256 id, uint256 quantity) external pausable {
        ...
        require(quantity % 3 == 0, "Incorrect quantity for crafting");

        _burn(msg.sender, id, quantity);

        uint256 craftQuantity = quantity / 3;
        ...
        pausable
    {
        ...
        for (uint256 i = 0; i < quantities.length; i++) {
            ...
            require(craftID > 0, "Already maximum rarity");
            require(quantities[i] % 3 == 0, "Incorrect quantity for crafting");
            craftQuantities[i] = quantities[i] / 3;
            craftIds[i] = craftID;
            tokenSupply[ids[i]] -= quantities[i];
     ...
     */
    function _createUri(string memory cid)
        internal
        view
    ...
    /**
     * @dev returns the next NFT ID to be minted
     */
    ...
    function _exists(uint256 id) internal view returns (bool) {
        return (bytes(idToUri[id]).length > 0);
    }
     ...
     */
    function updateRedeemModifier(uint256 _redeemModifier) public onlyOwner {
     ...
     */
    function updateDeadline(uint256 _deadline) public onlyOwner {
        deadline = _deadline;
    }
}
