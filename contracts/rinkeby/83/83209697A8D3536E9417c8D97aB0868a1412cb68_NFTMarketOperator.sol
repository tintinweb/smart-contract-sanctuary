...
contract NFTMarketOperator is Ownable {
    ...
    mapping(string => ListingItem) public _listingItems;
    uint256 internal _operatorEthFeeBalance;
    mapping(address => uint256) internal _operatorTokenFeeBalance;
    ...
    function listItemWithEth(
        ...
        uint256 _tokenId,
        uint256 _priceInEth,
        uint256 _feeInPercent,
        address _payTo
    ) external onlyOwner returns (bool) {
        require(
            keccak256(abi.encodePacked(_listingItems[_id].id)) ==
                keccak256(abi.encodePacked("")),
            "NFTMarketOperator: this item has already listed"
        );

        IERC721 assetOperator = IERC721(_assetAddress);
        require(
            assetOperator.isApprovedForAll(_payTo, address(this)) == true,
            "NFTMarketOperator: invalid grant"
        );
        require(
            assetOperator.ownerOf(_tokenId) == _payTo,
            "NFTMarketOperator: you are not owner of this item"
        );

        _listingItems[_id] = ListingItem({
            id: _id,
    ...
    function unlistItem(string memory _id) external onlyOwner returns (bool) {
        require(
            keccak256(abi.encodePacked(_listingItems[_id].id)) ==
                keccak256(abi.encodePacked(_id)),
            "NFTMarketOperator: this item has already listed"
        );
        ListingItem storage item = _listingItems[_id];
        require(
            item.available == true,
    ...
    function buyItemInEth(string memory _id) external payable returns (bool) {
        require(
            keccak256(abi.encodePacked(_listingItems[_id].id)) ==
                keccak256(abi.encodePacked(_id)),
            "NFTMarketOperator: this item has already listed"
        ...
        require(
            item.available == true,
            "NFTMarketOperator: this item has been sold or not available"
        );
        require(
            msg.value == item.priceInEth,
        ...
        _operatorEthFeeBalance = _operatorEthFeeBalance + fee;
        transferAssetItem(
            item.assetAddress,
            item.tokenId,
            item.payTo,
        ...
        returns (bool)
    {
        ...
        _operatorTokenFeeBalance[item.paymentToken] =
            _operatorTokenFeeBalance[item.paymentToken] +
            fee;
