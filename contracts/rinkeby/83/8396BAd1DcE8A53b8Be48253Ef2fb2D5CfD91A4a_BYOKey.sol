...
abstract contract AbstractBYOKey is ERC1155Pausable, ERC1155Supply, ERC1155Burnable, Ownable {
...
import "./AbstractBYOKey.sol";
import "./TokenInterface.sol";

contract BYOKey is AbstractBYOKey {
    ...
    constructor(string memory _name,
        string memory _symbol) ERC1155("https://api.byopills.com/key/") {
        name_ = _name;
        symbol_ = _symbol;
    }
    ...
    function publicCollect (
        ...
        require (byoKeys[_keyIdx].isCollectingOpen, "Collecting is not open yet for this key.");           
        require (byoKeys[_keyIdx].isWhitelistBased == false, "This key is whitelist based.");              
        require (byoKeys[_keyIdx].maxPerTx == 0 || _numPasses <= byoKeys[_keyIdx].maxPerTx);               
        if (byoKeys[_keyIdx].mintPrice > 0 && msg.value < _numPasses.mul(byoKeys[_keyIdx].mintPrice)) {
            revert ("Ethereum sent is not sufficient.");
        }
        _mint(msg.sender, _keyIdx, _numPasses, "");
    }
    ...
    function whitelistCollect (uint256 _numPasses,
        ...
        bytes32[] calldata merkleProof) external payable {
            ...
            require(_maxAmount != 0 && balanceOf(msg.sender, 0).add(_numPasses) <= _maxAmount, "You are minting more than your available mints.");

            if (byoKeys[_keyIdx].enforceBalance) {
                TokenInterface token = TokenInterface(byoKeys[_keyIdx].linkedAsset);
                uint256 bal = token.balanceOf(msg.sender);
                require (_numPasses <= bal, "You do not own enough of the linked asset contract.");  
            }
    ...
    function burnFromRedeem(
        ...
        require(byoKeys[_keyIdx].redeemableAddress == msg.sender, "Only allowed from redeemable address");
        _burn(_account, _keyIdx, _amount);
    }  

    function uri(uint256 _id) public view override returns (string memory) {
        require(totalSupply(_id) > 0, "Token does not exist");    
