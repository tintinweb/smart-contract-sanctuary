    ...
    ReentrancyGuardUpgradeable
{
     ...
     * During this process the creator transfer the amount to the Vault and mint
     * the same amount in the multi token contract associated to the party just created
     */
    ...
    function participantDeposit(
        ...
        require(stableToken.balanceOf(participant) >= _amount, 
            "You don't have enough founds!");

        IRandStableToken randStableToken = IRandStableToken(
            address(stableToken)
        );
        randStableToken.burn(participant, _amount);

        partiesList.increaseBalance(_partyId, _amount);
        // uint256 endBlock = blockNumberCreated.add(duration);
        // uint256 twaAmount = endBlock.sub(block.number).mul(_amount).div(
        //     endBlock.sub(blockNumberCreated)
        // );
        multiToken.mint(participant, _partyId, _amount, _amount, "");

        partiesList.addParticipant(_partyId, participant);
        emit ParticipantJoinedParty(_partyId, participant);
    ...
    function participantWithdraw(uint256 _partyId, uint256 _amount)
        public
        nonReentrant
    {
        PartiesStorageLibrary.PartyState state;
        ...
        require(amount > _amount, "Insufficient founds");
        uint256 _twaAmount = twaAmount.mul(_amount).div(amount);
        multiToken.burn(msg.sender, _partyId, _amount, _twaAmount);
        stableToken.safeTransferFrom(vaultAddress, msg.sender, amount);

        emit ParticipantWithdraw(_partyId, msg.sender, _amount);
    }

    function startYielding(uint256 _partyId) public onlyRole(VAULT_ADMIN_ROLE) {
    ...
    ) public onlyRole(VAULT_ADMIN_ROLE) nonReentrant {
        PartiesStorageLibrary.PartyState state;
        uint256 minimumParticipants;
