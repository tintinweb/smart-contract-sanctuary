...
contract paymentContract is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable  {
  ...
  function commitMint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    _pendingCommits[_msgSender()][_nextID] = PendingCommits(stake, amt);
    _pendingCommitId[_msgSender()] = _nextID;
    pendingMintAmt += amount;

  }

  function commitMint_headDAO(uint256 amount, bool stake) external payable whenNotPaused {
    address msgSender = _msgSender();
    uint256 minted = ERC721Contract.minted() + pendingMintAmt;
    ...
    require(daoMinted + amount <= deposits.length && amount > 0 && amount <= 10, "Invalid Amount to mint");
    require(tx.origin == msgSender, "Only EOA");
    require(msg.value == 0);
