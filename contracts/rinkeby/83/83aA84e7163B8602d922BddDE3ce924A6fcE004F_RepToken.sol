...
contract RepTokenDB {
        ...
        )
    {
        ...
        uint256[] memory unixDates =
            new uint256[](predictions[_predictor].length);
...
contract RepToken is ChainlinkClient {
    ...
    //     uint256 _price
    // ) external {
    //     repTokenDB.addPrediction(_symbol, _date, _unixDate, _price);
    // }
    ...
    {
        return repTokenDB.getPredictions(_predictor);
    }
    ...
    function evaluatePredictions(address _predictor) external {
        ...
        for (uint256 i = 0; i < prices.length; i = i.add(1)) {
            if (unixDates[i] > block.timestamp || checks[i] == true) {
                ...
                bytes32 requestId = requestStockPrice(symbols[i], dates[i]);
                checks[i] = true;
