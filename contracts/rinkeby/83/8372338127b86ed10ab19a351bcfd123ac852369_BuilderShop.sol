...
contract NiftyBuilderInstance is ERC721, ERC721Burnable, ERC721Enumerable {
    ...
    uint immutable private id;
    uint immutable private countType;

    mapping (uint => uint) private _numNiftyPermitted;
    mapping (uint => Counters.Counter) private _numNiftyMinted;
    ...
    function massMintNFTs(address collector_address, uint num_to_mint, uint nifty_type) onlyValidSender public {
        ...
        uint specificTokenId00 = _numNiftyMinted[nifty_type].current() + 1;
        uint tokenId00 = bm.encodeTokenId(id, nifty_type, specificTokenId00);

        for (uint i = 0; i < num_to_mint; i++) {
            giftNifty(collector_address, nifty_type);
        }
        uint specificTokenId01 = _numNiftyMinted[nifty_type].current();
        uint tokenId01 = bm.encodeTokenId(id, nifty_type, specificTokenId01);
