...
contract PaintCreator is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, Pausable {
  ...
  constructor() ERC721("Paint Creator", "PAINT") Ownable() {}
  ...
  function reserveTokens(address _to, uint256 _reserveAmount) public onlyOwner {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        // string memory attributes = Base64.encode(bytes(string())

        string memory attributes = '[{"key": "value", "also": "value"}]';

        // string memory singleAttribute = string(abi.encodePacked("trait_type": "Base",)

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"attributes": attributes, "name": "Generated #', toString(tokenId), '", "description": "Altered Acoustics description.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
