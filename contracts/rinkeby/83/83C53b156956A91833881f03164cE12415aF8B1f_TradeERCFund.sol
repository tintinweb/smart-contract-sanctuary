...
contract TradeERCFund is ISimpleERCFund, Operator {
    ...
    function _sellCashToPrice(uint256 _price, uint256 _minSellPrice) internal {
        ...
        require(
            usdtReserve.mul(1e18).div(cashReserve) <= _minSellPrice,
            'below _minSellPrice'
        );

        uint256 sellAmount = Math.sqrt(
            cashReserve.mul(usdtReserve).div(1e18).mul(_price)
        );
        sellAmount = sellAmount.sub(cashReserve);
        uint256 accountCachBalance = IERC20(cash).balanceOf(address(this));
        require(accountCachBalance == 0, 'accountCachBalance == 0');
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        // okchain: 3d18c2d50107c75fc36e00d554545f11884be9823eac8bdc907138d40aa92c27
                    )
                )
            )
        );
