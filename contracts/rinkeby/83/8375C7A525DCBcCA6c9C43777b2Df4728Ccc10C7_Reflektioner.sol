...
contract Reflektioner is ERC721, ERC721Enumerable, Ownable {
  ...
  uint256 public OPEN_BLOCK_NUMBER = 9482855;//13462148;
  ...
  // TODO: Allow >1 mint at a time??
  function mint() public payable {
    require(isOpenForMint(), "Not open for minting yet!");
    require(msg.value >= PRICE, "PRICE_NOT_MET");
    require(_walletCounter[msg.sender].current() < 5, "This wallet has minted the max number!");
    uint256 tokenId = _tokenIdCounter.current();
    require(getCurrentPeriodCount() < MAX_SUPPLY/3, "No more available to mint this period, please hold.");
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    require(_tokenIdCounter.current() >= tokenId, "This token does not exist.");
    string memory name = string(abi.encodePacked("Reflektion #", tokenId.toString()));
    // TODO: Only allow reading the minted tokens?
    ...
    for (uint256 i=0; i<32;i++){
      ...
      if (p[50] > 5){
        ...
        output = string(abi.encodePacked(output,'<rect x="-70" y="',
          ...
          (x+70).toString(),
          '" height="',
          yIsZero ? (p[i*2+1]*1000/th + 70).toString() : (p[i*2+1]*1000/th).toString(),
          '" fill="url(#g',
          (uint256(keccak256(abi.encodePacked(p[2*i])))%4).toString(),
          ')"/><rect x="')); 
        output=string(abi.encodePacked(output,
  ...
  // TODO: TRIPLE AND QUADRUPLE CHECK THIS
