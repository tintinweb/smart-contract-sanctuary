	...
	UUPSUpgradeable
{
	...
	event GiveawaySettled(uint256 indexed collectionId, address winner, uint256 tokenId, string seed);
	...
	function tokenIds(uint256 _collectionId) external collectionExists(_collectionId) view returns (uint256[] memory) {
		return collections[_collectionId].tokenIds;
	}

	function startGiveaway(uint256 _collectionId) external collectionExists(_collectionId) onlyOwner nonReentrant {
		require(collections[_collectionId].ends == 0, "Giveaway already started");
		_nextGiveaway(_collectionId);
	}

	function restartGiveaway(uint256 _collectionId) external collectionExists(_collectionId) onlyOwner nonReentrant {
		_nextGiveaway(_collectionId);
	}
		...
		collectionExists(_ticket.collectionId)
	{
		...
		uint256 tokenId = c.tokenIds[c.currentTokenIndex];
		require(minted[_ticket.collectionId][tokenId] == false, "Already minted");
		...
		emit GiveawaySettled(_ticket.collectionId, _ticket.winner, tokenId, _ticket.seed);

		c.token.transferFrom(c.owner, _ticket.winner, tokenId);
	}

	/// @dev number returned includes the current token
	function tokensLeft(uint256 _collectionId) external view collectionExists(_collectionId) returns (uint256) {
		Collection memory c = collections[_collectionId];
	...
	function _verify(SignedTicket calldata _ticket) private view returns (address) {
		bytes32 digest = _hashTypedData(_ticket);
		return ECDSAUpgradeable.recover(digest, _ticket.signature);
	}

	function _hashTypedData(SignedTicket calldata _ticket) private view returns (bytes32) {
