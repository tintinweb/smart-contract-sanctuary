...
contract UserOwnedMarkets is ERC721Upgradeable {
    uint public _tokenIds;
    mapping(uint => uint) tokenMarketID;
    ...
    uint[] assayerPrice;// = 1000000000000000; // starting price 0.001 
    uint[] minAuditPrice;// = 1000000000000000; // starting price 0.001 
	...
	function initialize(string memory name, string memory symbol, address _paymentTokenAddress, address _owner, uint _assayerPrice, uint _minAuditPrice, uint16 _feePercent, uint _auditTimeExtension) initializer public {
		__ERC721_init(name, symbol);
		paymentTokenAddress.push(_paymentTokenAddress); //Test token - wETH
        owner.push(_owner); //multisig contract / gnosis safe
		assayerPrice.push(_assayerPrice);
		minAuditPrice.push(_minAuditPrice);
	...
	function createMarket(address _paymentTokenAddress, uint _assayerPrice, uint _minAuditPrice, uint16 _feePercent, uint _auditTimeExtension) public {
    ...
    public {
		require(IERC20(_paymentTokenAddress).balanceOf(buyer) >= paymentAmount , "0");
        ...
        temp.expiry = block.number + blocks; // TODO: seconds, minutes, hours, ...
    ...
    function addAssayersPrice(uint number, uint marketID) public view returns(uint) {
        uint price;
        for (uint i; i < number; i++) 
            price += assayerPrice[marketID]*2**i;
        return price;
    }

    function addAssayer(uint marketID) 
	public {
		require(IERC20(paymentTokenAddress[marketID]).balanceOf(msg.sender) >= assayerPrice[marketID] , "0");
		require(IERC20(paymentTokenAddress[marketID]).transferFrom(msg.sender, owner[marketID], assayerPrice[marketID]), "1"); 
        ERC721Upgradeable._mint(msg.sender, _tokenIds);
		tokenMarketID[_tokenIds] = marketID;
        _tokenIds+=1;
        assayerPrice[marketID] *= 2;
    }

    function addAssayers(uint number, uint marketID) 
	public {
		require(IERC20(paymentTokenAddress[marketID]).balanceOf(msg.sender) >= addAssayersPrice(number, marketID) , "0");
		require(IERC20(paymentTokenAddress[marketID]).transferFrom(msg.sender, owner[marketID], addAssayersPrice(number, marketID)), "1"); 
        for (uint i; i < number; i++) {
            ERC721Upgradeable._mint(msg.sender, _tokenIds);
			tokenMarketID[_tokenIds] = marketID;
            _tokenIds+=1;
    ...
    function payWithFee(uint _transactionID, uint marketID) internal {
        uint fee=transactions[marketID][_transactionID].paymentAmount/10000*feePercent[marketID];
        IERC20(transactions[marketID][_transactionID].paymentTokenAddress).transfer(msg.sender, transactions[marketID][_transactionID].paymentAmount - fee);
        IERC20(transactions[marketID][_transactionID].paymentTokenAddress).transfer(owner[marketID], fee);
        transactions[marketID][_transactionID].finished = true;
    }

    /** 
    * @dev allows a user to withdraw funds from transaction
    */
    ...
    public {//require some payment?
		require(IERC20(paymentTokenAddress[marketID]).balanceOf(msg.sender) >= auditPrice , "0");
        ...
        require(auditPrice >= minAuditPrice[marketID], "11");
    ...
    auditNotExpired(_auditID)
    public {
        if(_marketAddressID == 0) require(ownerOf(_tokenID)==msg.sender && audits[_auditID].marketID==tokenMarketID[_tokenID],"16"); // _marketAddressID = 0 in cases when assayers are only from local market and not from "otherMarkets"
        //else require(ERC721Upgradeable(otherMarkets[audits[_auditID].marketID][_marketAddressID-1]).ownerOf(_tokenID)==msg.sender,"17");
		else require(ownerOf(_tokenID)==msg.sender && tokenMarketID[_tokenID]==otherMarkets[audits[_auditID].marketID][_marketAddressID-1],"17");
    ...
    modifier trueAssayer(uint _auditID, uint assayID) {
        if(audits[_auditID].commits[assayID].marketAddressID==0) require(ownerOf(audits[_auditID].commits[assayID].tokenID)==msg.sender,"19");
        //else require(ERC721Upgradeable(]).ownerOf(audits[_auditID].commits[assayID].tokenID)==msg.sender,"20");
        else require(ownerOf(audits[_auditID].commits[assayID].tokenID)==msg.sender && tokenMarketID[audits[_auditID].commits[assayID].tokenID]==otherMarkets[audits[_auditID].marketID][audits[_auditID].commits[assayID].marketAddressID-1],"20");
        _;
    }
    ...
    function updateMinAuditPrice(uint newprice, uint marketID) 
    onlyOwner(marketID)
    public {
        minAuditPrice[marketID]=newprice;
    }
