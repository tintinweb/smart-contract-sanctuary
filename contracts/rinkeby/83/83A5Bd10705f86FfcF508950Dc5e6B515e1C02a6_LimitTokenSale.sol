...
contract LimitTokenSale is Ownable, ReentrancyGuard {
  ...
  uint8 private constant DECIMALS = 14;

  IERC20 public saleToken;
  IERC20 public collateralToken;
  TokenVesting public tokenVesting;
  
  bytes32[] internal saleNames;
  mapping(string => Sale) internal sales;
  mapping(address => bool) internal whitelisted;

  event Submitted(string sale, address beneficiary, uint256 amount, uint256 price);
  event Claimed(string sale, address beneficiary, uint256 amount);
  event Recovered(address token, uint256 amount);
  event Withdrawn(uint256 amount);

  constructor(address _saleToken, address _collateralToken, address _tokenVesting) {
   ...
   * @param _price        price of sale token
   * @param _presale      percent of sale tokens release straight away
   * @param _totalSupply  sale total supply
   * @param _pid          vesting pool id
   *
   * @dev method initialize new sale
   */
  function initSale(string memory _sale, uint256 _price, uint256 _presale, uint256 _totalSupply, uint8 _pid) external onlyOwner {
    require(_presale <= 100, "TokenSale: Invalid presale percent");
    
    saleToken.safeTransferFrom(msg.sender, address(this), _totalSupply);

    sales[_sale].state = State.closed;
    sales[_sale].price = _price;
    sales[_sale].presale = _presale;
    sales[_sale].totalSupply = _totalSupply;
    sales[_sale].pid = _pid;
  }

  /**
   * @param _sale name of sale
   *
   * @dev method returns sale details
   */
  function getSale(string memory _sale) external view returns(State, uint256, uint256, uint256, uint256, uint8) {
    return (sales[_sale].state, sales[_sale].price, sales[_sale].presale, sales[_sale].totalSupply, sales[_sale].totalAmount, sales[_sale].pid);
  }

  /**
   * @param _sale         name of sale
   * @param _price        new price of sale token
   * @param _presale      new percent of sale tokens release straight away
   * @param _totalSupply  new sale total supply
   * @param _pid          new vesting pool id
   *
   * @dev method sets new parameters to the sale
   */
  function updateSale(string memory _sale, uint256 _price, uint256 _presale, uint256 _totalSupply, uint8 _pid) external onlyOwner {
    require(sales[_sale].state == State.closed, "TokenSale: Sale is already finalized");
    require(_presale <= 100, "TokenSale: Invalid presale percent");

    if(sales[_sale].totalSupply < _totalSupply) {
      saleToken.safeTransferFrom(msg.sender, address(this), _totalSupply.sub(sales[_sale].totalSupply));
    } else {
      saleToken.safeTransfer(msg.sender, sales[_sale].totalSupply.sub(_totalSupply));
    }
   ...
   * @param _sale name of sale
   *
   * @dev method open the sale
   */
  function openSale(string memory _sale) external onlyOwner {
    sales[_sale].state = State.open;
  }

  /**
   * @param _sale name of sale
   *
   * @dev method finalize the sale
   */
  function finalizeSale(string memory _sale) external onlyOwner {
    sales[_sale].state = State.finalized;
  }
  ...
  function recoverERC20(address _token, uint256 _amount) external onlyOwner {
    require(_token != address(saleToken) || _token != address(collateralToken), "TokenSale: cannot recover sale or collateral tokens");
  ...
  /**
   * @param _sale   id of sale
   * @param _amount amount of tokens user would like to buy
   *
   * @dev method submits beneficiaries to the sale 
   */
  function submit(string memory _sale, uint256 _amount) external nonReentrant {
    require(whitelisted[msg.sender], "TokenSale: Beneficiary is not whitelisted");
    require(sales[_sale].state == State.open, "TokenSale: Sale is not open");

    uint256 price = _amount.mul(sales[_sale].price).div(10 ** DECIMALS);
    collateralToken.safeTransferFrom(msg.sender, address(this), price);

    sales[_sale].totalAmount = sales[_sale].totalAmount.add(_amount);
    sales[_sale].beneficiaries[msg.sender] = sales[_sale].beneficiaries[msg.sender].add(_amount);
    
    emit Submitted(_sale, msg.sender, _amount, price);
  }
   ...
   */
  function claim(string memory _sale) external nonReentrant {
    require(sales[_sale].state == State.finalized, "TokenSale: Sale is not finalized");
    require(sales[_sale].beneficiaries[msg.sender] > 0, "TokenSale: Beneficiary is not a participant");

    uint256 tokenAmount = 0;
    if(sales[_sale].totalAmount > sales[_sale].totalSupply) {
      tokenAmount = sales[_sale].totalSupply.mul(sales[_sale].beneficiaries[msg.sender]).div(sales[_sale].totalAmount);
      uint256 overPrice = sales[_sale].beneficiaries[msg.sender].sub(tokenAmount).mul(sales[_sale].price).div(10 ** DECIMALS);
      collateralToken.safeTransfer(msg.sender, overPrice);
    } else {
      tokenAmount = sales[_sale].beneficiaries[msg.sender];
    }

    if(sales[_sale].presale > 0) {
      uint256 presaleAmount = tokenAmount.mul(sales[_sale].presale).div(100);
      tokenAmount = tokenAmount.sub(presaleAmount);
      saleToken.safeTransfer(msg.sender, presaleAmount);
    }
    saleToken.safeTransfer(address(tokenVesting), tokenAmount);
    tokenVesting.addBeneficiary(sales[_sale].pid, msg.sender, tokenAmount);

    emit Claimed(_sale, msg.sender, tokenAmount);
  }
...
contract TokenVesting is Ownable {
   ...
   */
  function initVestingPool(string memory _name, uint256 _startTime, uint256 _endTime) external onlyOwner() returns(uint8) {
   ...
   */
  function setVestingPool(uint8 _pid, string memory _name, uint256 _startTime, uint256 _endTime) external onlyOwner() {
  ...
  function addBeneficiary(uint8 _pid, address _beneficiary, uint256 _lockedAmount) external onlyOwnerOrSale() {
    ...
    require(lockPools[_pid].startTime > block.timestamp, "TokenVesting: pool is running");
