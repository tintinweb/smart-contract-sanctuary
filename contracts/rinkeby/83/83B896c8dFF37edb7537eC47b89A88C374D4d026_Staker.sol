...
contract Staker {
  ...
  function stake() public payable stakeNotCompleted {
    ...
    console.log("stake sender %s balance is %s ",msg.sender, balances[msg.sender]);
  ...
  function withdraw(address payable add) public stakeNotCompleted {
    require(openForWithdraw == true);
    uint userBalance = balances[add];
    require(userBalance > 0, "user has no funds");
    balances[add] = 0;
    (bool sent,) = add.call{value: userBalance}("");
    require(sent);
  }
