...
// Pancake interface for swap and add liquidity
...
contract CreoLPFarm is Ownable {
    using SafeMath for uint256;
    ...
    event SwapTokenForLP(address indexed user, uint256 amount);

    constructor(
        IBEP20 _creoToken,
        IUniswapV2Router02 _swap
        ) public {
    ...
    receive() external payable {
        emit SwapTokenForLP(tx.origin,msg.value);
    }

    function farmBalance() public view returns(uint256) {
        uint256 creoFee = totalLocked.div(uint256(100));
        uint256 deduct = totalLocked.add(unclaimedRewards).sub(creoFee);
    ...
    function stake(uint256 amount) public {
        ...
        require(
            ...
            "CreoFarm: You cannot stake less than 5 tokens");
    ...
    function unstake(uint256 amount) public {
        require(
            ...
            "CreoFarm: Insufficient staking balance"
        );
        ...
        // re-entrancy gaurd
        amount = 0;
        // deduct creo burn of 1% to prevent errors
        uint256 burnValue = balTransfer.div(uint256(100));
        uint256 toTransfer = balTransfer.sub(burnValue);   
    ...
    function _calculateYieldTime(address user) private  returns(uint256){
        // max time in seconds
        uint256 maxTime = 4200;
        uint256 end = now;
        uint256 totalTime = end.sub(startTime[user]);
        uint256 allTime = totalTime.add(stakedTime[user]);
        if (allTime > maxTime){
            uint256 excess = allTime.sub(maxTime);
            totalTime = totalTime.sub(excess);
        }
        stakedTime[user] = stakedTime[user].add(totalTime);
        return totalTime;
    }

    function _calculateYieldTotal(address user) private returns(uint256) {
        uint256 time = _calculateYieldTime(user);
    ...
    function _swapRewardForLp(address user) private {
        uint256 rewards = rewardBalance[user];
        ...
        if(stakingBalance[user] != 0){
            uint256 currYield = _calculateYieldTotal(user);
            rewards = rewards.add(currYield);
            startTime[user] = now;
        }
        
        uint256 toSwap = rewards.div(2);
        require(creoToken.approve(address(pancakeSwap), rewards), 'approve failed.');
        uint256 liq = rewards.div(2);
        address[] memory path = new address[](2);
        path[0] = address(creoToken);
        path[1] = pancakeSwap.WETH();
        uint256 maxSwapVal = pancakeSwap.getAmountsOut(toSwap,path)[1];
        uint256 minSwapOut = maxSwapVal.sub(maxSwapVal.mul(3).div(100));
        pancakeSwap.swapExactTokensForETHSupportingFeeOnTransferTokens(toSwap,minSwapOut,path,address(this), now + 900);
        uint256 liqETH = minSwapOut;
        pancakeSwap.addLiquidityETH{ value:liqETH }(address(creoToken),liq,liq.sub(liq.mul(50).div(100)) ,liqETH.sub(liqETH.mul(50).div(100)),user,now + 900);
        claimedRewards = claimedRewards.add(rewards);
        emit YieldWithdraw(user, rewards);
    }
   
    

    function withdrawYield() public {
        _swapRewardForLp(_msgSender());
    } 
}