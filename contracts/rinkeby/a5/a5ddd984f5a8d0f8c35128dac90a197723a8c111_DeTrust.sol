...
contract DeTrust is IDeTrust, Initializable {
    ...
    */
    ...
    function getBalance() external override view returns (uint balance) {
    ...
    function getTrustListAsBeneficiary() external override view returns (Trust[] memory) {
        uint[] memory trustIds = beneficiaryToTrustIds[tx.origin];
    ...
    function getTrustListAsSettlor() external override view returns (Trust[] memory) {
        uint[] memory trustIds = settlorToTrustIds[tx.origin];
        uint length = trustIds.length;
    ...
    function addTrustFromBalance(
        ...
        uint totalAmount) external override lock returns (uint tId) {
        ...
        return _addTrust(name, beneficiary, settlor,
                         startReleaseTime, timeInterval,
                         amountPerTimeInterval, totalAmount);
    }
    ...
    function addTrust(
        ...
        uint startReleaseTime,
        uint timeInterval,
        uint amountPerTimeInterval) payable external override lock returns (uint tId) {

        uint totalAmount = msg.value;
        require(totalAmount > 0, "msg.value is 0");

        return _addTrust(name, beneficiary, tx.origin,
                         startReleaseTime, timeInterval,
                         amountPerTimeInterval, totalAmount);
    }

    function topUp(uint tId) payable external override lock {
    ...
    function _release(uint tId, address beneficiary, address to) internal {
        ...
        if (isDeleted) {
            ...
            uint[] storage settlorTIds = settlorToTrustIds[t.settlor];
            uint k;
    ...
    function _addTrust(
        ...
        uint timeInterval,
        uint amountPerTimeInterval,
        uint totalAmount) internal returns (uint _id) {
        ...
        emit TrustAdded(name, settlor, beneficiary, _id,
	                startReleaseTime, timeInterval,
	                amountPerTimeInterval, totalAmount);
        return _id;
    }

    function _releaseTrust(Trust storage t)
        ...
        if (releaseAmount >= t.totalAmount) {
            ...
            emit TrustReleased(t.id, t.beneficiary, releaseAmount);
            t.totalAmount = 0;
...
interface IDeTrust {
     ...
     */
    event TrustReleased(uint indexed trustId, address indexed beneficiary, uint totalAmount);
