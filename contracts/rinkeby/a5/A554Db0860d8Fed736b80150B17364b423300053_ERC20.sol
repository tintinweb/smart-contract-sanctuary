...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    string private constant _name = "Archangel";
    ...
    uint256 public tradeLimitForLiqFee = 2;
    ...
    uint256 public constant liquidityFeeToSell = 100 * 10**18;
    uint256 public constant marketingFeeToSell = 100 * 10**18;
    ...
    bool public enableMaxTransactionAmount = false;

    /**
     * @dev Throws error if sender or recipient is blacklisted.
     */
    modifier whenNotBlacklisted(address sender, address recipient) {
        require(!_isBlackListed[sender], "Sender address is blacklisted");
        require(!_isBlackListed[recipient], "Recipient address is blacklisted");
        _;
    }
    ...
    constructor (address _marketingAddress) {
        ...
        updateMarketingAddress(_marketingAddress);
        ...
        // update the rest of the contract variables
    ...
    /**
     * @dev Returns true if the address is blacklisted, and false otherwise.
     */
    ...
    /**
     * @dev include address to blacklist.
     */
    function includeInBlacklist(address account) external virtual onlyOwner {
        _excludeFromReward(account);
        _isBlackListed[account] = true;
    }
    
    /**
     * @dev exclude address from blacklist
     */
    function excludeFromBlacklist(address account) external virtual onlyOwner {
     ...
     */
    function updateBurnFee(uint256 feeInPercentage) external onlyOwner {
        burnFee = feeInPercentage;
    }
    ...
    /**
     * @dev update trade limit for liquidity fee in percentage
     */
    function updateTradeLimitForLiqFee(uint256 limitInPercentage) external onlyOwner {
        tradeLimitForLiqFee = limitInPercentage;
    }

    /**
     * @dev update marketing address
     */
    function updateMarketingAddress(address _marketingAddress) public onlyOwner {
        require(_marketingAddress != address(0), "Marketing address cant be zero address");
     ...
     */
    function transfer(address recipient, uint256 amount) external virtual override whenNotBlacklisted(msg.sender, recipient) whenNotPaused returns(bool) {
     ...
     * `amount`.
     */
    function transferFrom(address sender, address recipient, uint256 amount) external virtual override whenNotBlacklisted(msg.sender, recipient) whenNotPaused returns (bool) {
     ...
     */
    function approve(address spender, uint256 amount) external virtual override whenNotBlacklisted(msg.sender, spender) whenNotPaused returns (bool) {
     ...
     * - `spender` cannot be the zero address.
     */
    function increaseAllowance(address spender, uint256 addedValue) external virtual whenNotBlacklisted(msg.sender, spender) whenNotPaused returns (bool) {
     ...
     * `subtractedValue`.
     */
    function decreaseAllowance(address spender, uint256 subtractedValue) external virtual whenNotBlacklisted(msg.sender, spender) whenNotPaused returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        //if any account belongs recipient _isIncludedInFee account then take fee
        //else remove fee
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) internal {
        bool takeLiqFee = true;
        if(!takeFee) {
            removeAllFee();
        }
        if(takeFee && recipient == uniswapV2Pair) {
            takeLiqFee = amount >= calculateTradeLimitForLiqFee();
            if(!takeLiqFee) removeLiqFee();
        }
        ...
        if(!takeLiqFee)restoreLiqFee();
    }
  
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tReflection, uint256 tMarketing, uint256 tBurn, uint256 tLiquidity) = getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection, uint256 rMarketing, uint256 rLiquidity) = getRValues(tAmount, tReflection, tMarketing, tBurn, tLiquidity);
    ...
    function _swapAndLiquify(address from) internal {
        if(from != uniswapV2Pair && (liquidityFeeBalance >= liquidityFeeToSell || marketingFeeBalance >= marketingFeeToSell)) {
            ...
            if(marketingFeeBalance >= marketingFeeToSell) {
                transferToFeeWallet(marketingAddress, marketingFeeBalance);
            }
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tReflection).sub(tMarketing).sub(tBurn).sub(tLiquidity);
        return (tTransferAmount, tReflection, tMarketing, tBurn, tLiquidity);
    }

    function getRValues(uint256 tAmount, uint256 tReflection, uint256 tMarketing, uint256 tBurn, uint256 tLiquidity) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        uint256 currentRate = getRate();
        ...
        uint256 rTransferAmount = rAmount.sub(rReflection).sub(rMarketing).sub(rBurn).sub(rliquidity);
        return (rAmount, rTransferAmount, rReflection, rMarketing, rliquidity);
    }
    ...
    function calculateMaxTransactionAmount() internal view returns(uint256) {
        ...
        return reserve0.mul(transactionLimit).div(10**2);
    }

    function calculateTradeLimitForLiqFee() internal view returns(uint256) {
        (uint256 reserve0,,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        return reserve0.mul(tradeLimitForLiqFee).div(10**2);
    }
    ...
    function takeBurnFee(address sender, uint256 tBurn) internal {        
        if(tBurn > 0) {
            _amount_burnt += tBurn;
    ...
    function removeAllFee() internal {
        if(reflectionFee == 0 && marketingFee == 0 && burnFee == 0 && liquidityFee == 0) return;
    ...
    function removeLiqFee() internal {
    ...
    function restoreLiqFee() internal {
        liquidityFee = _previousLiquidityFee;
    }
    ...
    function withdrawBnbFromContract(address user, uint256 amount) external onlyOwner {
        ...
        require(amount <= getBalance(), "Bridge: Address cant be zero address");
