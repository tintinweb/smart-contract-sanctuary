...
contract VineyardV1 is ERC721Enumerable, Ownable {
    ...
    WineBottle public wineBottle;

    mapping(uint256 => uint256[]) internal tokenAttributes;
    ...
    // CONSTRUCTOR
    constructor(string memory baseURI) ERC721("Hash Valley Vineyard", "VNYD") {
        setBaseURI(baseURI);
    }
    ...
    function newVineyards(uint256[] calldata _tokenAttributes) public payable {
        ...
        emit VineyardMinted(tokenId, _tokenAttributes[0], _tokenAttributes[1], _tokenAttributes[2], _tokenAttributes[3]);
    }
    ...
    function setBottleContract(address _bottle) public onlyOwner {
        wineBottle = WineBottle(_bottle);
    }
    ...
    function currSeason() public view returns (uint256) {
        ...
        return (block.timestamp - gameStart) / seasonLength + 1;
    }

    function plant(uint256 _tokenId) public isOwner(_tokenId) {
    ...
    function harvest(uint256 _tokenId) public isOwner(_tokenId) {
        ...
        planted[_tokenId] = 0;
        xp[_tokenId] += 100;
        uint256 bottleId = wineBottle.newBottle(_tokenId);
    ...
    function water(uint256 _tokenId) public isOwner(_tokenId) {
    ...
    function plantingTime() public view returns (bool) {
        ...
        uint256 plantingBegins = gameStart + (currSeason() - 1) * seasonLength;
    ...
    function harvestTime() public view returns (bool) {
        if (gameStart == 0) return false;
        uint256 harvestEnds = gameStart + currSeason() * seasonLength;
    ...
    modifier isOwner(uint256 _tokenId) {
        require(ownerOf(_tokenId) == msg.sender, "Vineyard not owned");
        _;
    }
}
