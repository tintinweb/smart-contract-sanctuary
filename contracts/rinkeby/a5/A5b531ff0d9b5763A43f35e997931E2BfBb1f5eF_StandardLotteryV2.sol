...
abstract contract ShinLotterysUpgradeable is
	Initializable,
	OwnableUpgradeable,
...
abstract contract ShinLotterysAbstract is
  ShinLotterysUpgradeable,
  IShinRandConsumer
{
  using SafeERC20Upgradeable for IERC20Upgradeable;
  using AddressUpgradeable for address;
  ...
  uint256 public maxPriceTicketInShin;
  uint256 public minPriceTicketInShin;
  ...
  IERC20Upgradeable public shinToken;

  IShinRand public randomGenerator;
   ...
   * @dev Callable by owner
   */
  function setRandomGenerator(IShinRand _address) external onlyOwner {
  ...
  /**
   * @notice Transfers $Shin to address
   * @param _addr destination address
   * @param _amount $Shin token amount
  ...
  function transferTo(address _addr, uint256 _amount)
    ...
    onlyTransferer
  {
    shinToken.safeTransfer(_addr, _amount);
  }
 ...
 */
contract StandardLotteryV2 is ShinLotterysAbstract {
  ...
  struct Lottery {
    ...
    uint256 startTime;
    uint256 endTime;
    uint256 ticketRate; // $Shin price per ticket
    ...
    uint256 amountCollectedToken; // Collected $Shin token amount which transfered to DeLotto
  ...
  function __StandardLottery_init(
    IERC20Upgradeable _shinToken,
    IShinRand _randomGenerator
  ) public initializer {
    ShinLotterysUpgradeable.initialize();

    currentLotteryId = 0;
    currentTicketId = 1;
    unwonPreviousPot = 0;

    shinToken = _shinToken;
    ...
    maxPriceTicketInShin = 50000 * (10**5);
    minPriceTicketInShin = 1000 * (10**5);
  ...
  /**
   * @notice Buy a bundle for the current lottery staking $Shin
  ...
  function buyTickets(
    ...
    // Calculate number of $Shin to breakdown
    uint256 amountShinToTransfer = _calculateTotalPriceForBulkTickets(
      _lotteries[_lotteryId].ticketRate,
      _purchasedTicketCount
    );

    uint256 deLottoAmount = amountShinToTransfer.mul(breakDownDeLottoPot).div(
      10000
    );
    uint256 deGrandAmount = amountShinToTransfer.mul(breakDownDeGrandPot).div(
      10000
    );
    uint256 teamAmount = amountShinToTransfer.mul(breakDownTeamWallet).div(
      10000
    );
    shinToken.safeTransferFrom(
      address(msg.sender),
      address(this),
      deLottoAmount
    );
    ...
    shinToken.safeTransferFrom(address(msg.sender), teamWallet, teamAmount);
    shinToken.safeTransferFrom(
      address(msg.sender),
      DEAD_ADDRESS,
      amountShinToTransfer.sub(deLottoAmount).sub(deGrandAmount).sub(
        teamAmount
      )
    );
  ...
  function claimTickets(
    ...
    // Initializes the rewardInShinToTransfer
    uint256 rewardInShinToTransfer;
    ...
    for (uint256 i = 0; i < _ticketIds.length; i++) {
      ...
      rewardInShinToTransfer += rewardForTicketId;
    }

    // Transfer money to msg.sender
    shinToken.safeTransfer(msg.sender, rewardInShinToTransfer);

    emit TicketsClaim(
      msg.sender,
      rewardInShinToTransfer,
      _lotteryId,
      _ticketIds.length
    );
  }
    ...
    whenNotPaused
  {
    ...
    // Calculate prizes in $Shin for each bracket by starting from the highest one
   ...
   * @param _ticketRate price of a ticket in $Shin
  ...
  function startLottery(
    ...
    require(
      (_ticketRate >= minPriceTicketInShin) &&
        (_ticketRate <= maxPriceTicketInShin),
      "Outside of limits"
    );
  ...
  function burnUnclaimed(uint256 _lotteryId) external onlyOperator {
    ...
    uint256 remain = shinToken.balanceOf(address(this));
    if (remain > 0) {
      shinToken.safeTransfer(DEAD_ADDRESS, remain);
    }
  ...
  /**
   * @notice Increase pot by Shin team
   * @param _lotteryId lottery id
    ...
    onlyOwner
  {
    ...
    shinToken.safeTransferFrom(address(msg.sender), address(this), _amount);
  ...
  /**
   * @notice Set $Shin price ticket upper/lower limit
   * @dev Only callable by owner
   * @param _minPriceTicketInShin: minimum price of a ticket in $Shin
   * @param _maxPriceTicketInShin: maximum price of a ticket in $Shin
   */
  function setMinAndMaxTicketPriceInShin(
    uint256 _minPriceTicketInShin,
    uint256 _maxPriceTicketInShin
  ) external onlyOwner {
    require(
      _minPriceTicketInShin <= _maxPriceTicketInShin,
      "minPrice must be < maxPrice"
    );

    minPriceTicketInShin = _minPriceTicketInShin;
    maxPriceTicketInShin = _maxPriceTicketInShin;
  }
