...
contract ERC721Tradable is ERC721, Ownable{
	...
	function _canAirdrop(address _user) private view returns(bool) {
	    ...
	    if(partnerContractA.balanceOf(_user) > 0) {
	        return true;
	    } else if (partnerContractB.balanceOf(_user) > 0) {
	        return true;
	    } else if (partnerContractC.balanceOf(_user) > 0) {
	        return true;
	    } else if (partnerContractD.balanceOf(_user) > 0) {
	        return true;
	    } else{
	        return false;
	    }
...
contract CosmosChamber is ERC721Tradable {
	constructor(address _proxyRegistryAddress) public ERC721Tradable("CosmosChamber", "CC", 6001, 800, 500, 500, 4201, _proxyRegistryAddress) {
		_setBaseMetadataURI("https://www.cosmoschanmber.art/metadata/nft/");
	}
