...
contract NFTMarket is ReentrancyGuard{
    ...
    struct MarketItem {
        ...
        address payable prevseller;
        address payable seller;
        address payable owner;
        uint256 price; // 
        uint256 sold;   // 0 => no sell, 1 => first sale, 2 => second sale
    }
    ...
    function buyMarketSale (
        ...
        if(idToMarketItem[itemId].sold == 0) {
            idToMarketItem[itemId].seller.transfer(buyingPrice(msg.value)); // 85%
            payable(address(this)).transfer(buyingFee(msg.value)); // 15%
            idToMarketItem[itemId].prevseller = idToMarketItem[itemId].seller;
            idToMarketItem[itemId].sold = 1;
        } else if(idToMarketItem[itemId].sold == 1) {
            idToMarketItem[itemId].seller.transfer(buyingPrice(msg.value)); // 85%
            payable(address(this)).transfer(buyingResellMarketFee(msg.value)); // 10%
            idToMarketItem[itemId].prevseller.transfer(buyingResellCreatorFee(msg.value)); // 5%
            idToMarketItem[itemId].sold = 0;
        }
