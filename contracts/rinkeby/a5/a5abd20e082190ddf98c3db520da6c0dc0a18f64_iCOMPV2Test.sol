    ...
    AccessControlUpgradeable
{
        ...
        returns (uint256[] memory)
    {
       ...
       uint256 underlyingMax = _getAssetAmount(collaterals[_id]);
        ...
        txData[2] = _getAssetAmount( redeemedCollaterals - feesInCollateral );  // amount retuerned to user

        txData[3]  =  redeemedCollaterals;  // collateral withdrawn from compound
        txData[4]  = feesInCollateral;      // collateral payed in fees <-- stays in the vault until claimed

        if((txData[2] + txData[4]) > collaterals[_id]){  // fees + user reddemed
           txData[0] = 1;
           return txData;
        }
