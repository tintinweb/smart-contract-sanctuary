    ...
    */
    function mint(uint256 value) public {
        ...
        if (block.timestamp.sub(lastTime) >= 366 days) {
            maximumSupply = maximumSupply.mul(100 + inflationRate).div(100);
            lastTime = block.timestamp;
        }
    ...
    function maxSupply() public view returns(uint256) {
        return maximumSupply.add(maximumSupply.mul(inflationRate).mul(block.timestamp.sub(lastTime)).div(366 days).div(100));
    }
}