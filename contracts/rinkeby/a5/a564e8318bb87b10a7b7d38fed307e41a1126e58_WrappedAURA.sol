...
interface IAURA {
  ...
  function AURA(uint256) external returns (uint256);

  function AURAallowance(uint256, uint256) external returns (uint256);
...
contract WrappedAURA is ERC20("Koi AURA", "AURA") {
  IAURA KOIContract = IAURA(0x4f06ef2A6b718e60611Be90bf35A4448086734CE);
 ...
 _Treasurer = 9;
  ...
  modifier hasEnoughAndApproved(uint256 _KOI_ID, uint256 amount) {
    require(
      ...
      "Summoner doesnt have enough $RAR"
    );
    require(
