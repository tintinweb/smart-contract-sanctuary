...
contract TheORB is Context, IBEP20, Ownable {
    ...
    constructor () {
        ...
        tokenBalance[0x0C2a98ace816259c0bB369f88Dd4bcb9135E0787] = tenThousandTokens.add(sixThousandTokens);   // Yoshiko
        tokenBalance[0x2b30eca9e19B480533db8EC37fa2faC035E32082] = tenThousandTokens;   // Space Cat
    ...
    function unStakeORB() public {
        ...
        if(!isAllStakingUnlocked){      // checks to see if the unlock has happened, if not do the normal timer check
            require(block.timestamp > timeStartedStaking[stakerAddress] + 3 days, "You cannot unstake, it has not been 3 days.");
        }
    ...
    function stakeUnlockTime(address stakerAddress) public view returns (uint256){
        uint256 stakeUnlockTimeForStaker = timeStartedStaking[stakerAddress] + 3 days;
    ...
    function unlockAllStaking() public onlyOwner {
        require(block.timestamp > timeStakingIsEnabled + 5 days, "Must be at least 5 days after staking has started.");
    ...
    function presaleBuy(uint256 keyCode) external payable {
        ...
        require(amountOfORBtoGive < amountORBminForLPCreation, ("Not enough ORB left in the Presale. Please check the ORB left in the contract itself and Adjust"));
    ...
    function endPresaleProvideLiquidity() external onlyOwner {
        ...
        timeStakingIsEnabled = timePresaleEndedAndLiquidityProvided + 1 days;
