...
contract AxonsVoting is IAxonsVoting, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    address public filaments = 0x90806D30c0B74aA196534C66cbc2B8df82c2F39B;
    ...
    function registerVoterWithFilament(uint256 tokenId) public {
        ...
        require(registeredAddress != msg.sender, "Already registered! Don't waste your gas");
        ...
        registeredVoters[msg.sender] = true; // Register to vote
        ...
        if (IAxons(axons).balanceOf(registeredAddress) == 0 && IERC721Enumerable(filaments).balanceOf(registeredAddress) == 0) {
            // Unregister prior owner if they hold no Filaments or Axons
            registeredVoters[registeredAddress] = false;
        }
    }
    ...
    function vote(bool[10] memory votes, uint256 auctionCounter) public {
        ...
        require(registeredVoters[msg.sender] || IAxonsToken(axonsToken).isGenesisAddress(msg.sender), 'Must own a Filament or Axon (and register it to vote) or be a genesis $AXON holder to vote');
