...
contract nft is ERC721, Ownable {
    ...
    Counters.Counter private numberOfTotalTokens;
    ...
    uint private constant mintCostGenesisSale = 0.1 ether;
    uint private constant mintCostPresale = 0.15 ether;
    uint private constant minMintCostPublicSale = 0.2 ether;
    uint private constant maxMintCostPublicSale = 2 ether;
    ...
    uint private maxReservedMints = 50;
    
    //addresses of the different shareholders and owners
    address payable public communityWallet = payable(0x06a2568067132fA8a5A2d7012E1d1641702eB3D5);
    address payable public teamWallet = payable(0x02d274Ad889bf84E5431293B9DCE985A6C12703c);
    ...
    //blacklisted tokenIds
    mapping(uint => bool) private blacklist;
    uint[] private initialBlacklisted;
    
    //declaring initial values for variables
    constructor() ERC721('Who is Emily?', 'EMILY') {
        ...
        initialBlacklisted = [9942, 9936, 9887, 9836, 9817, 9809, 9551, 948, 9412, 9185, 8874, 8433, 7952, 7554, 7272, 7209, 6807, 6543, 6300, 6254, 5921, 5882, 5775, 5573, 5489, 5185, 4906, 4358, 4345, 4371, 4471, 4477, 4183, 3802, 3650, 3535, 3115, 2989, 2945, 2821, 2625, 2587, 2348, 2148, 2020, 1495, 1239, 806, 691];

        for (uint i = 0; i < initialBlacklisted.length; i++) {
            addToBlacklist(initialBlacklisted[i]);
        }
    ...
    function tokenId() internal view returns(uint256) {
        ...
        while (blacklist[nextToken] == true || _exists(nextToken)) {
            nextToken += 1;
        }
        return nextToken;
    }
    ...
    */
    ...
    function genesisMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        State saleState_ = saleState();
        require(saleState_ == State.GenesisSale, "Not in Genesis Sale!");
        require(numberOfTotalTokens.current() + number <= maxTokensGenesisSale, "Not enough NFTs left to mint..");
    ...
    function presaleMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        ...
        require(numberOfTotalTokens.current() + number <= maxTokensPresale, "Not enough NFTs left to mint..");
        require(mintsPerAddress[msg.sender] + number <= maxMintPresale, "Maximum 2 Mints per Address allowed!");
    ...
    function publicSaleMint(uint256 number) public payable {
        ...
        require(numberOfTotalTokens.current() + number <= maxTotalTokens - (maxReservedMints - _reservedMints), "Not enough NFTs left to mint..");
    ...
    function burnAllUnmintedTokens() public onlyOwner {
        maxTotalTokens = numberOfTotalTokens.current();
    }
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(communityWallet, (balance * 15) / 100);
        _withdraw(payable(owner()), accountBalance()); //to avoid dust eth, gets a little more than 85%
    }
    ...
    function totalSupply() public view returns(uint) {
        return numberOfTotalTokens.current();
    }
    ...
    function mintCost() public view returns(uint) {
        ...
        if (saleState_ == State.NoSale || saleState_ == State.GenesisSale) {
            ...
            uint256 deduction = ((block.timestamp - publicSaleLaunchTime) / 7200) * mintIncrement;
            if (deduction <  maxMintCostPublicSale - minMintCostPublicSale) {
                return maxMintCostPublicSale - deduction;
            }
            else {
                return minMintCostPublicSale;
            }
        }
    }
    ...
    //blacklist a token
    function addToBlacklist(uint number) internal {
