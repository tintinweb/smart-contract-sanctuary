...
contract KoiRouter {
    // Events
    event Deposit(address user, address token, uint256 id, uint256 amount);
    ...
    function deposit(
        ...
        if (token == koiToken) {
            // if koiToken, burn NFT
            IKoiToken(koiToken).burnFrom(msg.sender, id, amount);
        } else {
            // if not koiToken, transfer to vault
        ...
        emit Deposit(msg.sender, token, id, amount);
    }
}
