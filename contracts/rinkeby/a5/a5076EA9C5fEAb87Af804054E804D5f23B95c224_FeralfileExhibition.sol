...
contract FeralfileExhibition is ERC721Enumerable, Authorizable {
        ...
        onlyAuthorized
    {
        ...
        require(_artwork.artist != address(0), "artwork is not found");
        // require(!_artwork.minted);

        for (uint256 i = 0; i < _ipfsCIDs.length; i++) {
            uint256 editionNumber = allArtworkEditions[_artworkID].length;
            uint256 editionID = uint256(
                keccak256(abi.encode(_artworkID, editionNumber))
            );
