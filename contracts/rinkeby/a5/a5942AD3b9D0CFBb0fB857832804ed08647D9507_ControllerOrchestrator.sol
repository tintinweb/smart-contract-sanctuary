...
interface IPCVDepositOrchestrator is IOrchestrator {
    function init(
        ...
        uint256 depositDecimalsMultiplier,
        bool isPrice0,
        address erc20TokenAddress
...
contract ERC20UniswapPCVController is IUniswapPCVController, UniRef, Timed {
    ...
    function _returnToPeg() internal {
        (uint256 feiReserves, uint256 erc20Reserves) = getReserves();
        if (feiReserves == 0 || erc20Reserves == 0) {
            return;
        }
        ...
        // calculate amount ERC20 needed to return to peg then swap
        uint256 amountErc20 =
            _getAmountToPegOther(feiReserves, erc20Reserves, _peg);
        _swapErc20(amountErc20, erc20Reserves, feiReserves);
    }

    // event DebugEvent(uint256 erc20, uint256 balance);

    function _swapErc20(
        uint256 amountErc20,
        uint256 erc20Reserves,
        uint256 feiReserves
    ) internal {
        uint256 balance = erc20TokenAddress.balanceOf(address(this));
        uint256 amount = Math.min(amountErc20, balance);

        uint256 amountOut =
            UniswapV2Library.getAmountOut(amount, erc20Reserves, feiReserves);
        
        assert(erc20TokenAddress.transfer(address(pair), amount));
