...
contract RocketFactoryMarket is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    */
    ...
    modifier isNotBeingTransacted(uint256 _itemId, uint8 _itemType) {
        require(
    ...
    modifier isOnSale(uint256 _itemId, uint8 _itemType) {
        require(
    ...
    modifier isNotOnSale(uint256 _itemId, uint8 _itemType) {
        require(
            itemsToSale[_itemType][_itemId].seller == address(0x0),
    ...
    modifier isOnAuction(uint256 _itemId, uint8 _itemType) {
        require(
    ...
    modifier isNotOnAuction(uint256 _itemId, uint8 _itemType) {
        require(
            itemsToAuction[_itemType][_itemId].seller == address(0x0),
    ...
    */
    ...
    function checkBalance(address _erc20) external onlyOwner returns (uint256) {
    ...
    */
    function makeAnOffer(
        ...
        uint256 _price,
        address _erc20
    ) public payable callerIsUser() nonReentrant() isNotBeingTransacted(_itemId, _itemType) {
        require(
            itemTypeToTokenAddress[_itemType] != address(0x0),
            "The item type does not exist"
        );
    ...
    function acceptOffer(address _offerer, uint8 _itemType, uint256 _itemId) public callerIsUser() isNotBeingTransacted(_itemId, _itemType) {
