...
contract MemeBank is IMemeBank, IMemeTraits, Ownable, Authorizable {
  mapping(uint256 => MemeTraits) public memes;
  mapping(uint256 => string) public memeURIs;
  ...
  function setTokenURI(uint256 tokenId, string memory meme) external override onlyAdmin {
    memeURIs[tokenId] = meme;
    existingIds[tokenId] = true;
  }

  function getTokenURI(uint256 tokenId) public view override returns (string memory) {
    return memeURIs[tokenId];
  }
}
...
interface IMemeBank {
  ...
  function setTokenURI(uint256 tokenId, string memory meme) external;
}
