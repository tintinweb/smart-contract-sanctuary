...
contract EscrowForAragonCourtV1 is EscrowContract, AmendablePreSigned, WithPreSignedMilestones {
    ...
    IEscrowDisputeManager public immutable TRUSTED_DISPUTER;
     ...
     * @param _disputer Address of contract which manages disputes on Greet Escrow side.
     */
    constructor(address _registry, address _disputer) EscrowContract(_registry) {
        TRUSTED_DISPUTER = IEscrowDisputeManager(_disputer);
    }
    ...
    function registerContract(
        ...
        for (uint8 _i=0; _i<_milestones.length; _i++) {
            ...
            emit NewMilestone(_cid, _index, _mid, _mp.paymentToken, _mp.deliveryDeadline, _mp.amount);
        }
    }
    ...
    function registerMilestone(
        ...
        _registerMilestone(_cid, _index, _paymentToken, _treasury, _payeeAccount, _refundAccount, _deliveryDeadline, _amount);
     ...
     */
    function dispute(bytes32 _cid, uint8 _index, bool _ignoreCoverage, bytes calldata _evidence) external isDisputer(_cid) {
        bytes32 _termsCid = getLatestApprovedContractVersion(_cid);
        TRUSTED_DISPUTER.dispute(msg.sender, _cid, _index, _termsCid, _ignoreCoverage, _evidence);
    }
    ...
    function submitEvidence(bytes32 _cid, uint8 _index, bytes calldata _evidence) external isDisputer(_cid) {
        TRUSTED_DISPUTER.submitEvidence(msg.sender, _cid, _index, _evidence);
    }
    ...
    function closeEvidencePeriod(bytes32 _cid, uint8 _index) external isDisputer(_cid) {
        TRUSTED_DISPUTER.closeEvidencePeriod(_cid, _index);
    }
    ...
    function appealDispute(
        ...
        TRUSTED_DISPUTER.appealDispute(msg.sender, _cid, _index, _lastRoundId, _ruling, _feeToken, _appealFee);
    }
    ...
    function confirmAppealDispute(
        ...
        TRUSTED_DISPUTER.confirmAppealDispute(msg.sender, _cid, _index, _lastRoundId, _ruling, _feeToken, _confirmAppealFee);
    }
    ...
    function resolveDispute(bytes32 _cid, uint8 _index) external {
        ...
        uint256 _ruling = TRUSTED_DISPUTER.ruleDispute(_cid, _index, _mid);
        if (_ruling == RULE_PAYER_WON) {
            emit CanceledMilestone(_mid, TRUSTED_DISPUTER.ARBITER(), _available);
            ...
            emit ReleasedMilestone(_mid, TRUSTED_DISPUTER.ARBITER(), _available);
            delete milestones[_mid].releasedAmount;
            delete milestones[_mid].refundedAmount;
...
abstract contract WithMilestones is ContractContext, MilestoneContext {
    ...
    function _registerMilestone(
        ...
        _registerMilestoneStorage(_mid, _paymentToken, _treasury, _payeeAccount, _refundAccount, _deliveryDeadline, _amount);
        emit NewMilestone(_cid, _index, _mid, _paymentToken, _deliveryDeadline, _amount);
    }
...
interface IEscrowDisputeManager {
    function dispute(address _feePayer, bytes32 _cid, uint8 _index, bytes32 _termsCid, bool _ignoreCoverage, bytes calldata _evidence) external;
