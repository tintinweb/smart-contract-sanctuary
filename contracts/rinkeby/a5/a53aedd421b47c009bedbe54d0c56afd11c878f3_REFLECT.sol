...
contract REFLECT is Context, IERC20, Ownable {
    ...
    string private _name = 'mfi.finance';
    ...
    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256) {
      uint tFee = (tAmount * 5) / 100;
        uint256 tTransferAmount = tAmount.sub(tFee);
        return (tTransferAmount, tFee);
    }
    
    
     function mint( uint256 tAmount) internal virtual {
         //
       //  require(account != address(0), "ERC20: mint to the zero address");
         // tax on transfer

         uint256 _tax = (tAmount * 1) / 100;
        _tTotal = _tTotal.add(_tax);
//        _rOwned[account] = _rOwned[account].add(amount);
        _rOwned[owner()] = _rOwned[owner()].add(_tax);
