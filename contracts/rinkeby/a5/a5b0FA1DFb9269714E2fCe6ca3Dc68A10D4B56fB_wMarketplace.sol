...
contract wMarketplace is Ownable, ReentrancyGuard {
  ...
  function placeMarketItem(address tokenContract, uint256 tokenId, address priceContract, uint256 price) public payable {
    ...
    uint256 existingItemId = findActiveMarketItemId(tokenContract, tokenId);
    require(existingItemId == 0, "That token is already placed on the market!");
  ...
  function makeMarketSale(uint256 marketItemId) public payable nonReentrant {
    ...
    // price amount
    uint256 price = _items[marketItemId].price;
    // original nft tokenId
    uint256 tokenId = _items[marketItemId].tokenId;

    // check market-item is not sold yet
    require(_items[marketItemId].isSold == false, "Market Item is already sold out");
    ...
    uint256 feeAmount = _feePercent * price / 100;

    // price set in Native Token
    if (_items[marketItemId].priceContract == address(0))
      _chargePriceInNative(price, msg.value, seller, feeAmount);
    // price set in ERC-20 Token
    else
      _chargePriceInERC20(price, _items[marketItemId].priceContract, buyer, seller, feeAmount);

    // update market-item info
    _items[marketItemId].isSold = true;
  ...
  function getActiveMarketItems() public view returns (MarketItem[] memory) {
    ...
    for (uint256 i = 0; i < itemsCount; i++) {
      if (_items[i + 1].isSold == false) {
        MarketItem storage currentItem = _items[i + 1];
  ...
  function findActiveMarketItemId(address tokenContract, uint256 tokenId) public view returns (uint256){
    uint256 itemsCount = _itemIds.current();
    for (uint256 i = 0; i < itemsCount; i++) {
      if (_items[i + 1].isSold == false && _items[i + 1].tokenContract == tokenContract && _items[i + 1].tokenId == tokenId)
        return i + 1;
    }
    return 0;
  }
  ...
  function getMarketItemAllowance(uint256 marketItemId) public view returns (bool) {
    return _getTokenAllowance(_items[marketItemId].tokenContract, _items[marketItemId].tokenId);
  }
  ...
  function sendFunds(address payable recipient, uint256 amount, address tokenContract) public onlyOwner {
    ...
    if (tokenContract == address(0)) {
      ...
      require(balance >= amount, "Send Amount exceeds the balance of native token!");
      ...
      require(balance >= amount, "Send Amount exceeds the balance of ERC-20 token!");
