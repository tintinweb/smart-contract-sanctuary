...
import "./interfaces/IGoblinsGame.sol";
import "./interfaces/IMine.sol";
import "./interfaces/IGold.sol";
import "./interfaces/ITraits.sol";
import "./interfaces/IGooblins.sol";

contract GoblinsGame is IGoblinsGame, Ownable, ReentrancyGuard, Pausable {
	struct LastWrite {
		uint64 time;
		uint64 blockNum;
	}
	...
	uint256 public mintPrice = 0.06969 ether;
	uint256 private startedTime;

	// max $GOLD cost
	uint256 private maxGoldCost = 72000 ether;

	mapping(address => Whitelist) private _whitelistAddresses;
	uint16 private constant MAX_PRESALE_MINTS = 2;

	// reference to the mine for choosing random Dragon thieves
	IMine public mine;
	...
	modifier requireContractsSet() {
		require(
			address(gold) != address(0) &&
				...
				address(gooblins) != address(0) &&
				address(mine) != address(0),
			"Contracts not set"
		);
		_;
	}
	...
	/**
	 * mint a token - 90% Villagers, 10% Goblins
	 * The first 25% are claimed with eth, the remaining cost $GOLD
	 */
	function mint(uint256 amount, bool stake) external payable whenNotPaused nonReentrant {
		require(tx.origin == _msgSender(), "Only EOA");
		uint16 minted = gooblins.minted();
		uint256 maxTokens = gooblins.getMaxTokens();
		uint256 paidTokens = gooblins.getPaidTokens();
		...
		for (uint256 i = 0; i < amount; i++) {
			...
			if (!stake || recipient != _msgSender()) {
				gooblins.mint(recipient, seed);
			} else {
				gooblins.mint(address(mine), seed);
			}
			totalGoldCost += mintCost(minted, maxTokens, paidTokens);
		}
		gooblins.updateOriginAccess(tokenIds);
		if (totalGoldCost > 0) {
			gold.burn(_msgSender(), totalGoldCost);
			gold.updateOriginAccess();
		}
		if (stake) {
			mine.addManyToMineAndTheiving(_msgSender(), tokenIds);
		}
	 ...
	 * the remaining 80% have a 10% chance to be given to a random staked goblin
	 * @param seed a random value to select a recipient from
	 * @return the address of the recipient (either the minter or the goblin thief's owner)
	 */
	function selectRecipient(
		...
		address thief = mine.randomGoblinOwner(seed >> 144); // 144 bits reserved for trait selection
	...
	function setmaxGoldCost(uint256 _amount)
		external
		requireContractsSet
		onlyOwner
	{
