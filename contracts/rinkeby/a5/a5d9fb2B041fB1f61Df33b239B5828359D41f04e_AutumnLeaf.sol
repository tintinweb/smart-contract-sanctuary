...
contract AutumnLeaf is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    '<path d="M248.018 341.596C283.576 292.013 307.008 270.739 349.97 240.538M248.022 341.087C220.939 294.5 201.186 272.311 158.978 238.747M249.018 119.247C245.215 216.348 245.394 270.041 249.018 364.747C255.772 389.469 255.225 402.127 249.018 423.247" stroke="black" stroke-width="4" stroke-linecap="round"',
    ...
    string[4] private bunches = ["No bunch","aspen","pin cherry"];
    ...
    function getSelectedleaves(Leaf memory leaf,uint256 tokenId) internal view returns(string memory){
        ...
        if(leaf.bunches==1){
            ...
            stemcolors[leaf.leafcolor],'"','/>','</g>'));
        }
        if(leaf.bunches==2){
            colorindex = getColor(tokenId,'PIN_CHERRY',0,24);
