...
contract AuroraRoundTwo  is Ownable {
	...
	uint256 public aurDecinal;
    uint256 public price = 4 * 10**usdtDecimal / 100;
	uint256 public thelowestquantitybuytogetbonus = 500 * 10**usdtDecimal;
	bool public offbuy;
	bool public openclaim;
	address addressreceive;

	struct HistoryBuyToken{
		uint256 timebuy;
		uint256 blockbuy;
		uint256 usdtbuy;
		uint256 aurbuy;
	...
	struct HistoryClaimToken{
		uint256 timebuy;
		uint256 blockbuy;
		uint256 aurbuy;
	}

	mapping(address => HistoryBuyToken[]) public historyBuyToken;
	mapping(address => HistoryClaimToken[]) public historyClaimToken;
	mapping(address => uint256) public totalAur;
	mapping(address => address) public mappingsponsor;
	mapping(address => bool) public boolsponsor;
    constructor( address _tokenUsdt ,address _tokenAur,  address _addressreceive, uint256 _usdtDecimal, uint256 _aurDecinal) public {
	...
	function ClaimToken() public {
		...
		uint256 balanceclaim = totalAur[msg.sender];
		tokenAur.mint(msg.sender, balanceclaim);
		totalAur[msg.sender] =0;
		historyClaimToken[msg.sender].push(HistoryClaimToken(block.timestamp , block.number , balanceclaim));
	}

	function BuyTokenAur(address _sponsor, uint256 _balance) public {
		require(!offbuy, "Ended");
		address sponsor;
		if(boolsponsor[msg.sender] || _sponsor == msg.sender || mappingsponsor[_sponsor] == msg.sender){
			sponsor = mappingsponsor[msg.sender];
		}else {
			mappingsponsor[msg.sender] = _sponsor;
			sponsor = _sponsor;
			boolsponsor[msg.sender] = true;
		}
		// tokenUsdt.safeTransferFrom(msg.sender, addressreceive , _balance);
		tokenUsdt.safeTransferFrom(msg.sender, address(this) , _balance);
		if(_balance >= thelowestquantitybuytogetbonus){
			uint256 bonus = 5 * _balance * 10**aurDecinal / (100 * price);
			tokenAur.mint(sponsor, bonus);
		}
		uint256 receiveAur = _balance * 10**aurDecinal / price;
		totalAur[msg.sender].add(receiveAur);
		historyBuyToken[msg.sender].push(HistoryBuyToken(block.timestamp , block.number , _balance , receiveAur));
	}
}