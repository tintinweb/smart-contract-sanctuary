...
contract Gems is ERC1155, Ownable {
    ...
    function createRecipe(
        ...
        recipes[recipe] = Recipe(price, amount, true);
        return true;
    }
    ...
    function create(
        ...
        if (a == 0) {
            _mint(msg.sender, counter, 1, _data);
            recipes[_data].remaining--;
            crafted[_s] = true;
            craftedById[counter] = _s;
            counter++;
            return true;
        }
        return false;
    ...
    function getGem(uint id) public view returns (bytes memory) {
        return craftedById[id];
    }
}