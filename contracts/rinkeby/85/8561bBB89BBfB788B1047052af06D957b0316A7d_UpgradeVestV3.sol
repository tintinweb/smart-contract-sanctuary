...
contract UpgradeVestV3 is Initializable,OwnableUpgradeable,ERC20PausableUpgradeable{
	...
	function initialize(address _publicAddress) initializer public{
		__Ownable_init();
        __ERC20_init('VEST','VST');		
		__ERC20Pausable_init();
		_mint(owner(),3900000 ether);
		_mint(_publicAddress,1100000 ether);
  ...
  /**
	...
	function  _claimTGETokens(address _userAddresses,uint8 _vestingIndex) public onlyAfterTGE() checkVestingStatus(_userAddresses,_vestingIndex) returns(bool){
		// Get Vesting Details
	 	VestAccountDetails memory vestData = userToVestingDetails[_userAddresses][_vestingIndex];
	...
	function _claimVestTokens(address _userAddresses,uint8 _vestingIndex) public checkVestingStatus(_userAddresses,_vestingIndex) returns(bool){
		// Get Vesting Details
	 	VestAccountDetails memory vestData = userToVestingDetails[_userAddresses][_vestingIndex];
	...
	function initializedV2() external{
		require (!_initializedV2,"Version 2 Already initialized");
		_mint(owner(),4000000 ether);
		_initializedV2 = true;
	}

		/**

		@notice Updates the claimed TGE tokens and One month tokens amount for a particular address in the contract
		@dev Simply markes isTgeTokensClaimed boolean to True without specifying the actual TGE tokens send for every user.
		@dev gets the one month token for each address and updates it as the claimed amount for that address.
		@param _userAddresses array of addresses of each User
		@param _vestingIndex index of the vesting Type
		*/
		function updateClaimedTokensState(address[] calldata _userAddresses,uint8 _vestingIndex) external onlyOwner returns(bool){
		uint256 oneMonthVestTokensClaimed = 0;
		uint256 totalUsers = _userAddresses.length;
		for(uint256 i=0; i< totalUsers; i++){
		    // Get Vesting Details
		  address user = _userAddresses[i];
		  VestAccountDetails memory vestData = userToVestingDetails[user][_vestingIndex];
		  require(
		  userToVestingDetails[user][_vestingIndex].isVesting,
		  "User NOT added to the provided vesting Index"
		);
		  oneMonthVestTokensClaimed = vestData.monthlyTokens;
		  vestData.totalVestingTokensClaimed += oneMonthVestTokensClaimed;
		  userToVestingDetails[user][_vestingIndex] = vestData;
		}
			return true;
		}
}
