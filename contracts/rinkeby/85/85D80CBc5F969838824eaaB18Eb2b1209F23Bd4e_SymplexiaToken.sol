...
//   ======================================
    ...
    function _tokenLiquify(address sender, uint256 amountToTransfer) internal {    

        if ( autoLiquidity && !inSwapAndLiquify ) {
            uint256 _tokenToLiquidity;

            if (sender == liquidityPair && isAdjustable ) {
                uint256 _stabilityRatio = (Wallet[liquidityPair].balance * 10) / 100;
                _liquidityDeviation    += amountToTransfer;
                if (Wallet[regulatoryFunds].balance < _liquidityDeviation) {
                    Wallet[wicksellReserves].balance += Wallet[regulatoryFunds].balance;
                    Wallet[regulatoryFunds].balance   = 0;
                    isAdjustable  = false;
                }
                if (_liquidityDeviation >= _stabilityRatio && isAdjustable) {
                    Wallet[regulatoryFunds].balance -= _stabilityRatio;      
                    Wallet[address(this)].balance   += _stabilityRatio;
                    _liquidityDeviation             -= _stabilityRatio;
                    _tokenToLiquidity                = _stabilityRatio/20;
                    swapAndLiquify (_tokenToLiquidity); 	                //*** Add Liquidity  ***
                }

            } else {
                uint256 _tokenBalance = balanceOf(address(this));
                _tokenToLiquidity = (_tokenBalance > _maxTokensPerTx) ? _maxTokensPerTx : _tokenBalance;

                if ( _tokenToLiquidity >= _liquidityThreshold ) { 
                     _tokenToLiquidity  = _liquidityThreshold;
                    swapAndLiquify (_tokenToLiquidity);                     //*** Add Liquidity  *** 
                }
            }
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor ()  ERC20("BETA20", "BTG20") BaseToken ( 
