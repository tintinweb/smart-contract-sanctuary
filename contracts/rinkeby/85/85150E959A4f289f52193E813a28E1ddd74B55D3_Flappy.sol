...
contract Flappy is ERC721("Flappy", "FLP"), ERC721Enumerable, Ownable {
    using Strings for uint256;
    using SafeMath for uint256;
    string private baseURI;
    string private blindURI;
    bool public isPresaleActive1;
    bool public isPresaleActive2;
    bool public isActive;
    bool public reveal;
    uint256 public constant BUY_LIMIT_PER_TX = 10;
    uint256 public constant MAX_NFT_PUBLIC = 9930;
    ...
    uint256 public NFTPrice = 0;  // 0.07 ETH
    uint256 public NFTPricePs1 = 55000000000000000;  // 0.55 ETH
    uint256 public NFTPricePs2 = 60000000000000000;  // 0.06 ETH
    mapping(address => bool) private whiteList;
    mapping(address => uint256) private whiteListClaimedPs1;
    mapping(address => uint256) private whiteListClaimedPs2;
    ...
    function setPresale1Active(bool _isActive) external onlyOwner
    {
        isPresaleActive1 = _isActive;
    }

     function setPresale2Active(bool _isActive) external onlyOwner
     {
         isPresaleActive2 = _isActive;
     }
 ...
 */
    ...
    function mintNFT(uint256 _numOfTokens) public payable
    {
       ...
       // require(!isPresaleActive1, 'Only presale 1 from White List');
       // require(!isPresaleActive2, 'Only presale 2 from White List');
    ...
    function mintNFTDuringPresale1(uint256 _numOfTokens) public payable
    {
        require(isActive, 'Contract is not active');
        require(isPresaleActive1, 'Presale 1 is closed');
        require(!isPresaleActive2, 'Only presale 2 from White List');
        ...
        require(whiteListClaimedPs1[msg.sender].add(_numOfTokens) <= WHITELIST_MAX_MINT, 'Purchase exceeds max whiteed');
        require(totalSupply().add(_numOfTokens).sub(giveawayCount) <= MAX_NFT_PUBLIC, 'Purchase would exceed max public supply of NFTs');
        require(NFTPricePs1.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");

        whiteListClaimedPs1[msg.sender] += _numOfTokens;
        for (uint i = 0; i < _numOfTokens; i++) {
            _safeMint(msg.sender, totalSupply().sub(giveawayCount));
        }
    }
     ...
     function mintNFTDuringPresale2(uint256 _numOfTokens) public payable
     {
         require(isActive, 'Contract is not active');
         require(!isPresaleActive1, 'Only presale 1 from White List');
         require(isPresaleActive2, 'Presale 2 is closed');
         require(whiteList[msg.sender], 'You are not on the White List');
         require(totalSupply() < MAX_NFT_PUBLIC, 'All public tokens have been minted');
         require(_numOfTokens <= WHITELIST_MAX_MINT, 'Cannot purchase this many tokens');
         require(whiteListClaimedPs2[msg.sender].add(_numOfTokens) <= WHITELIST_MAX_MINT, 'Purchase exceeds max whiteed');
         require(totalSupply().add(_numOfTokens).sub(giveawayCount) <= MAX_NFT_PUBLIC, 'Purchase would exceed max public supply of NFTs');
         require(NFTPricePs2.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");

         whiteListClaimedPs2[msg.sender] += _numOfTokens;
         for (uint i = 0; i < _numOfTokens; i++) {
             _safeMint(msg.sender, totalSupply().sub(giveawayCount));
         }
     }
  ...
  */
    function addToWhiteList(address[] memory _addresses) external onlyOwner
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            whiteListClaimedPs1[_addresses[i]] > 0 ? whiteListClaimedPs1[_addresses[i]] : 0;
            // whiteListClaimedPs2[_addresses[i]] > 0 ? whiteListClaimedPs2[_addresses[i]] : 0;
        }
    }
 ...
 */
     function onWhiteList(address _addr) external view
     returns (bool)
     {
