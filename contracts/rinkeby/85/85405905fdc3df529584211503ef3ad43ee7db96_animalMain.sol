...
contract animalMain is  ERC721,Ownable {
    ...
    function createAnimal() public payable{
        ...
        if(rand<30) roletype = uint16(role.Cat);
        else if (rand<60) roletype = uint16(role.Dog);
        else if (rand<75) roletype = uint16(role.Tiger);
        else if (rand<90) roletype = uint16(role.Lion);
        else if (rand<95) roletype = uint16(role.Dolphin);
        else if (rand<100) roletype = uint16(role.Whale);
        else roletype = uint16(role.Dog);
