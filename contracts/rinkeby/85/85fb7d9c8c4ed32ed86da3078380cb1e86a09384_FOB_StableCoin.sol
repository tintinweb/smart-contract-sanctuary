...
contract FOB_StableCoin {
    ...
    uint private constant ARBITRATOR_PERCENT = 5*(10**18);
...
//      ///////////////////////////  End of  Modifiers  ///////////////////
    ...
    constructor(address _token_address, string[4] memory _all_strings, uint[6] memory _all_uints, address _buyer, address _seller, address _vessel_validator,
        ...
        require(kyc_deadline > (block.timestamp - 18000), "kyc_deadline error!");
        require(kyc_deadline < delivery_time, "kyc_deadline is no less than delivery_time");
    ...
    function Deposit_to_contract() public payable inState(State.supplied) {
        ...
        require( msg.sender == seller || msg.sender == buyer, "Permission denied! Only Seller and Buyer can call this function.");
        uint total_price = unit_price * demand ;
        uint seller_share = (total_price * penalty_percent) / PERCENTAGE_RATE ;
        uint buyer_share = total_price;
        
        if(msg.sender == buyer)
        {
            require(Token.balanceOf(buyer) >= buyer_share, "Buyer Insufficient Funds!");
            require(Token.transferFrom(msg.sender, address(this), buyer_share), "Failed on transferFrom Buyer!");
            depositors_balances[msg.sender] += buyer_share;
            emit Buyer_paid_the_amount(buyer, buyer_share);
        }
        else
        {
            require(Token.balanceOf(seller) >= seller_share, "Seller Insufficient Funds!");
            require(Token.transferFrom(msg.sender, address(this), seller_share), "Failed on transferFrom Seller!");
        ...
        if(
            ...
            uint broker_fee = (total_price * broker_percent) / PERCENTAGE_RATE;
            current_state = State.payment_done;
            require(Token.transfer(broker, broker_fee), "Token broker transfer failed!");
        }
    }

    function contract_termination() public payable {
        // 18000 is equal 5 hours
        require((block.timestamp-1800)  < kyc_deadline);
        require(current_state != State.disagreement);
    ...
    {
        // 18000 is equal 5 hours
        require((block.timestamp-1800) < kyc_deadline, "KYC deadline expired!");
        require( (quality_controller == address(0)) || (current_state ==State.vessel_validator_reject_ship) );
    ...
    function checkout() public payable inState(State.done)
    {
        ...
        require(send, "Failed to checkout");
    }
    ...
    {
        require( (_offender == seller) || (_offender == buyer) || (_offender == address(this) ));
        require( (_beneficiary == seller) || (_beneficiary == buyer) || (_beneficiary == address(this) ));
