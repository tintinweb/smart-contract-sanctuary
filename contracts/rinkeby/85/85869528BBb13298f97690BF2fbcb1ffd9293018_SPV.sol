...
contract SPV is Ownable {
    ...
    function treasuryWithdraw( uint _index, address _token, uint _amount ) external {
        ...
        uint price = tokens[ _index ].price;
        uint value = _amount.mul( price ).div( 10 ** IERC20Extended( _token ).decimals() );
        treasuryDebt = treasuryDebt.add(value);
        updateTotalValue();
    ...
    function returnToTreasury( uint _index, address _token, uint _amount ) external {
        ...
        uint price = tokens[ _index ].price;
        uint value = _amount.mul( price ).div( 10 ** IERC20Extended( _token ).decimals() );
        if ( value > treasuryDebt ) {
            value = treasuryDebt;
        }
        treasuryDebt = treasuryDebt.sub(value);
        updateTotalValue();
