...
contract TLGStakingV1 is Pausable, AccessControl {
    ...
    uint256 public lostPerDay = 1e18;
    uint256 public ROUNDING_PRECISION = 1e12;
    uint256 public initialGlitchReward = 10e18;
    ...
    function calculateInitialRewards() public view returns (uint256) {
        uint256 totalReward = 0;
        for (uint256 i = 0; i < stakedGlitches[msg.sender].length; i++) {
            uint256 glitch = stakedGlitches[msg.sender][i];
            // if the initial reward for a glitch isn't marked as already claimed
            if (!alreadyClaimedInitial[glitch]) {
                totalReward += initialGlitchReward;
            }
        }
        return totalReward;
    }
    ...
    function claimRewards() external whenNotPaused {
        uint256 initialReward = calculateInitialRewards();
        if (initialReward > 0) {
            for (uint256 i = 0; i < stakedGlitches[msg.sender].length; i++) {
                uint256 glitch = stakedGlitches[msg.sender][i];
                alreadyClaimedInitial[glitch] = true;
            }
        }
        uint256 reward = calculateRewards();

        LOST.mint(msg.sender, reward + initialReward);
...
/*
    ...
    Minting is paused after initialization.

    DEFAULT_ADMIN_ROLE, MINTER_ROLE and PAUSER_ROLE is sender by default
    MINTER_ROLE should be set to the minter contract afterwards
...
contract TheLostGlitchesComic is ERC721, ERC721Enumerable, Ownable {
