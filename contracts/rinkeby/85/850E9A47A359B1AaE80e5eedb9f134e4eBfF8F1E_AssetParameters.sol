...
contract AssetParameters is Ownable, IAssetParameters {
        ...
        returns (uint256)
    {
        (uint256 _price, uint8 _currentPriceDecimals) =
            IPriceManager(_registry.getPriceManagerContract()).getPrice(_assetKey, _assetDecimals);

        if (_currentPriceDecimals > PRICE_DECIMALS) {
            _price = _price / 10**(_currentPriceDecimals - PRICE_DECIMALS);
        } else {
            _price = _price * 10**(PRICE_DECIMALS - _currentPriceDecimals);
        }
    ...
    function addLiquidityPool(
        ...
        IPriceManager(_registry.getPriceManagerContract()).addOracle(
            _assetKey,
            _assetAddr,
            _mainOracle,
            _backupOracle
...
contract LiquidityPool is ILiquidityPool, ERC20 {
    ...
    mapping(address => uint256) public override liquidityAmounts;
    mapping(address => BorrowInfo) public override borrowInfos;

    uint256 public override aggregatedLiquidityAmount;
    ...
    function getTotalSupplyAmount() external view override returns (uint256) {
        return aggregatedLiquidityAmount + aggregatedBorrowedAmount;
    }

    function getCurrentLiquidityAmount(address _userAddr)
        external
        view
        override
        returns (uint256)
    ...
    function getAnnualBorrowRate() public view override returns (uint256 _annualBorrowRate) {
        ...
        IAssetParameters.InterestRateParams memory _params =
            IAssetParameters(_registry.getAssetParametersContract()).getInterestRateParams(
                assetKey
            );
    ...
    function getAPY() external view override returns (uint256) {
        ...
        return
            (_currentInterest *
                (DECIMAL -
                    IAssetParameters(_registry.getAssetParametersContract()).getReserveFactor(
                        assetKey
                    ))) / _currentTotalSupply;
    }
    ...
    function getAmountInUSD(uint256 _assetAmount) public view override returns (uint256) {
        IAssetParameters _parameters = IAssetParameters(_registry.getAssetParametersContract());
        uint8 _tokenDecimals = ERC20(assetAddr).decimals();

        uint256 _assetPrice = _parameters.getAssetPrice(assetKey, _tokenDecimals);

        return (_assetAmount * _assetPrice) / 10**_tokenDecimals;
    }

    function getAmountFromUSD(uint256 _usdAmount) external view override returns (uint256) {
        IAssetParameters _parameters = IAssetParameters(_registry.getAssetParametersContract());
        uint8 _tokenDecimals = ERC20(assetAddr).decimals();

        uint256 _assetPrice = _parameters.getAssetPrice(assetKey, _tokenDecimals);

        return (_usdAmount * 10**_tokenDecimals) / _assetPrice;
    }

    function getFreezeStatus() public view override returns (bool) {
        return IAssetParameters(_registry.getAssetParametersContract()).isPoolFrozen(assetKey);
    }
    ...
    function updateCompoundRate() public override returns (uint256) {
        IInterestRateLibrary _library =
            IInterestRateLibrary(_registry.getInterestRateLibraryContract());
    ...
    function withdrawLiquidity(address _userAddr, uint256 _liquidityAmount)
        ...
        onlyDefiCore
    {
        uint256 _aggregatedLiquidityAmount = aggregatedLiquidityAmount;

        require(
            _aggregatedLiquidityAmount >= _liquidityAmount,
        ...
        aggregatedLiquidityAmount = _aggregatedLiquidityAmount - _liquidityAmount;
        liquidityAmounts[_userAddr] -= _liquidityAmount;
        ...
        require(
            getBorrowPercentage() <
                IAssetParameters(_registry.getAssetParametersContract()).getMaxUtilizationRatio(
                    assetKey
                ),
    ...
    function repayBorrow(address _userAddr, uint256 _repayAmount)
        ...
        returns (uint256)
    {
        return _closeBorrow(_userAddr, _userAddr, _repayAmount);
    }
    ...
    function liquidate(
        ...
        IERC20(assetAddr).transfer(_liquidatorAddr, _liquidityAmount);
    }
    ...
    function liquidationBorrow(
        ...
        return _closeBorrow(_userAddr, _liquidatorAddr, _amountToLiquidate);
    }
    ...
    function _closeBorrow(
        ...
        uint256 _reserveFunds =
            _getPercentageOfNumber(
                _currentInterest,
                IAssetParameters(_registry.getAssetParametersContract()).getReserveFactor(assetKey)
            );

        totalReserves += _reserveFunds;
        aggregatedLiquidityAmount += _closeAmount - _reserveFunds;

        IERC20(assetAddr).transferFrom(_closureAddr, address(this), _closeAmount);

        return getAmountInUSD(_closeAmount);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            ...
            if (
                from != address(0) &&
                _defiCore.isCollateralAssetEnabled(from, assetKey) &&
                to != address(0)
            ) {
                uint256 _newBorrowLimit =
                    _defiCore.getNewBorrowLimitInUSD(msg.sender, assetKey, amount, false);
                require(
            ...
            if (to != address(0)) {
                IAssetsRegistry(_registry.getAssetsRegistryContract()).addSupplyAsset(
                    assetKey,
                    to
                );
            }
