...
interface IBaseDoNFT is IERC721Receiver {
    ...
    function getDoNftInfo(uint256 tokenId) external view returns(uint256 oid, uint64[] memory ,uint64[] memory,uint64 nonce);
...
contract BaseDoNFT is OwnableContract,ReentrancyGuard,ERC721,IBaseDoNFT {
    ...
    function getDuration(uint256 tokenId,uint256 index) public view returns(uint64 start, uint64 end){
        DoNftInfo storage info = doNftMapping[tokenId];
        return getDuration(info.durationList.at(index));
    }
    ...
    function getDoNftInfo(uint256 tokenId) public view returns(uint256 oid, uint64[] memory starts,uint64[] memory ends,uint64 nonce){
        DoNftInfo storage info = doNftMapping[tokenId];
        oid = info.oid;
        nonce = info.nonce;
        uint256 length = info.durationList.length();
    ...
    function mintWNft(uint256 oid) public nonReentrant virtual returns(uint256 tid) {
        ...
        require(onlyApprovedOrOwner(msg.sender,oNftAddress,oid));
    ...
    function getFingerprint(uint256 tokenId) public view returns(bytes32 print){
        (uint256 oid, uint64[] memory starts,uint64[] memory ends,uint64 nonce) = getDoNftInfo(tokenId);
        print = keccak256(abi.encodePacked(oid,starts,ends,nonce));
    }
...
contract WrapDoNFT is BaseDoNFT,IWrapDoNFT{
    ...
    function redeem(uint256 tokenId,uint256[] calldata doids) public{
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        require(couldRedeem(tokenId, doids),"cannot redeem");
        DoNftInfo storage info = doNftMapping[tokenId];
...
contract DclDoNFT is WrapDoNFT {
    ...
    function setOperator(address to,uint256 doid,uint256 durationId) public virtual{
