...
contract Farm is ERC20, ERC20Detailed {
    ...
    // Modifiers
    modifier onlyGov() {
        require(msg.sender == governance, "Farm-Token: You are not the governance");
        _;
    }

    constructor () public ERC20Detailed("Farm.finance", "Farm", 18, 100000000 * 10 ** 18) {
    ...
    function mint(address _account, uint256 _amount) external {
        require(minters[msg.sender], "Farm-Token: You are not the minter");
    ...
    function acceptGov()
        external {
        require(msg.sender == pendingGov, "Farm-Token: You are not the pending governance");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal {
        ...
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= cap(), "Farm-Token: Capacity exceeded");
        }
    }
}