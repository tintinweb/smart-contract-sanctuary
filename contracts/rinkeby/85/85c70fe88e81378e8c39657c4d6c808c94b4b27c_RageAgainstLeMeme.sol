...
// GO TO LINE 1889 TO SEE WHERE THE Rage against le meme CONTRACT STARTS
...
contract RageAgainstLeMeme is ERC721, Ownable {
    ...
    uint public constant maxPurchase = 20; // to save gas, some place used value instead of var, so be careful during changing this value
    uint256 public maxRageAgainstLeMeme = 10000; // to save gas, some place used value instead of var, so be careful during changing this value
    ...
    // ############################# constructor #############################
    constructor() ERC721("RageAgainstLeMeme", "RALM") { }
    ...
    function isRevealed() public view returns(bool) {
        ...
        return maxRageAgainstLeMeme == totalSupply();
    }
    ...
    function startPublicSale(uint256 duration, uint256 startPrice)
        external
        onlyOwner
    {
    ...
    function getMintPrice() public view whenPublicSaleActive returns (uint256) {
        ...
        if (elapsed >= saleDuration) {
            ...
            uint256 currentPrice = ((saleDuration - elapsed) *
                saleStartingPrice) / saleDuration;
            return
    ...
    function mint(uint numberOfTokens) public payable {
        ...
        require(currentTotalSupply.add(numberOfTokens) < 10001, "Purchase would exceed max supply"); // ref maxRageAgainstLeMeme
    ...
    function setStartingIndex() public {
        ...
        newStartingIndex = uint(blockhash(collectionStartingIndexBlock)) % maxRageAgainstLeMeme;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(collectionStartingIndexBlock) > 255) {
            newStartingIndex = uint(blockhash(block.number - 1)) % maxRageAgainstLeMeme;
        }
