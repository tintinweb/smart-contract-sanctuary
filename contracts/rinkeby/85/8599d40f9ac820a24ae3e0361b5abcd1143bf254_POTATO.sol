...
contract POTATO is ERC20, Ownable {
    ...
    uint256 constant public BASE_RATE = 1 ether;  //1 POTATO a day
   ...
   function setHarryAddress(address contractAddress) public onlyOwner {
        harryAddress = contractAddress;
   }
    
    
    
    function updateReward(address from, address to) external {
        require(msg.sender == harryAddress);
        if(from != address(0)){
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == harryAddress , "Address needs to aprove burn first");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        Iharry harry = Iharry(harryAddress);
        return harry.balanceOf(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
    }
