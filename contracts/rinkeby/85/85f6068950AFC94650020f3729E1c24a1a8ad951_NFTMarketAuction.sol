...
contract NFTMarketAuction is ERC721Holder {
    string public name;
    ERC721 public itemToken;
    ...
    constructor(address _itemTokenAddress) {
        itemToken = ERC721(_itemTokenAddress);
    ...
    event AuctionFinished(uint256 _tokenId, address indexed _awarder, uint256 price);
    
    address payable public withdrawAddress = payable(0x33C8AFDF13dD5F629C1684c8FD4c4FEa2eB1eb95);
    address payable public recipientAddr = payable(0x33C8AFDF13dD5F629C1684c8FD4c4FEa2eB1eb95);
    ...
    function createAuction(
        ...
        itemToken.safeTransferFrom(msg.sender, address(this), _tokenId);
        ...
        totalHoldings++;
         ...
         Bidder memory bidder =  
            ...
            bidders.push(bidder);
            tokenIdToBidder[_tokenId] = bidders;
    ...
    function bid(uint256 _tokenId) public payable {
        ...
        //Bidder[] memory bidders = tokenIdToBidder[_tokenId];
    ...
    function finish(uint256 _tokenId) public payable{
        Auction memory auction = tokenIdToAuction[_tokenId];
        //Bidder[] memory bidders = tokenIdToBidder[_tokenId];
        //require(
        ...
        //refund losers funds
        Bidder memory awarder = tokenIdToBidder[_tokenId][tokenIdToBidder[_tokenId].length - 1];
        for (uint256 i = 1; i < tokenIdToBidder[_tokenId].length - 1; i++) {
            Bidder memory bidder = tokenIdToBidder[_tokenId][i];
            bidder.addr.transfer(bidder.amount);
        }
        
       
        //itemToken.transferFrom(auction.seller, awarder.addr, _tokenId);
        ...
        itemToken.safeTransferFrom(address(this), _bider, _tokenId);
   ...
   function canceltAuction(uint256 _tokenId)
    public
    {
	     Auction memory auction = tokenIdToAuction[_tokenId];
	     require(
        ...
        itemToken.safeTransferFrom(address(this), auction.seller, auction.tokenId);
