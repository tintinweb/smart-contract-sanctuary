...
contract FISHGAME is ERC721Enumerable, Ownable {
    ...
    uint public FISH_PRESALE_MAX_LIMIT = 1800; //presale
    uint public FISH_WALLET_PRESALE_LIMIT = 2; //FISHperwalletInWholeDuration
    uint public FISH_WALLET_PUBLICSALE_LIMIT = 2; //FISHperwalletInWholeDuration
    ...
    function setFISH_MAX_COUNT(uint _maxCount) external onlyOwner{
        FISH_MAX_COUNT = _maxCount;
    }

    function setFISH_PRESALE(uint _presale) external onlyOwner{
        FISH_PRESALE_MAX_LIMIT = _presale;
    }

    function setFISH_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        FISH_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setFISH_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        FISH_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setFISH_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        FISH_MAINSALE_PRICE = _mainSalePrice;
    }

    function setFISH_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
    ...
    function presaleBuy( bytes memory signature, uint tokenQuantity) external payable{
        ...
        require(preSaleAmountMinted + tokenQuantity <= FISH_PRESALE_MAX_LIMIT, "EXCEED_PRIVATE");
