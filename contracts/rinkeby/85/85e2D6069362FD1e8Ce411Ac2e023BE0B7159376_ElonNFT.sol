...
contract ElonNFT is ERC721Tradable {
    using SafeMath for uint256;
    address constant WALLET = 0x6e25cFa7e590B0d2a86c3f3d60c78e2816b74dF0;
    ...
    function mint(address to, uint numberOfTokens) public payable {
        ...
        if(preSaleIsActive) {
            require(numberOfTokens <= MAX_MINT_WHITELIST, "Exceeds 20 NFTs per wallet pre-sale limit.");
            require(isWhitelisted(to), "Your address is not whitelisted.");
            require(whitelist[to].hasMinted.add(numberOfTokens) <= MAX_MINT_WHITELIST, "Exceeds 20 NFTs per wallet pre-sale limit.");
            require(whitelist[to].hasMinted <= MAX_MINT_WHITELIST, "Exceeds 20 NFTs per wallet pre-sale limit.");
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 walletBalance = balance.mul(5).div(100);
