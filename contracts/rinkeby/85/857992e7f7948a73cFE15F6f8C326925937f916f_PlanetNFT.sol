...
contract PlanetNFT is ERC721Tradable {
    ...
    //private variables for mint planet
    ...
    event PlanetDetailMerge(uint256 currentTokenId, uint256 sendTokenId);
    event readyMerge(uint256 tokenIdRcvr , uint256 tokenIdSndr);

    /**
     * Constrctor for init basic setting
     */
    constructor(address metadataGenerator_, address receiver_, address _proxyRegistryAddress)
        ERC721Tradable("PlanetNFT", "P", _proxyRegistryAddress) 
    {
    ...
    function batch_create_random(
        uint8[] calldata tokenId_,
        uint256[] calldata seed_)  public onlyAdmin {
        for (uint256 i = 0; i < seed_.length; i++) {
            create_random(tokenId_[i], seed_[i]);
        }
    }

    function batch_mint_create_random(
        uint256[] calldata seed_)  public onlyAdmin {

        for (uint256 i = 0; i < seed_.length; i++) {
            mintTo(_msgSender());
            create_random(mintTokenId, seed_[i]);
        }
    }

    function create_random(uint256 tokenId,
        uint256 seed_) internal onlyAdmin{
        ...
        newPlanet.randomness=seed_+block.difficulty;

        newPlanet.bkgColor=_metadataGenerator.pickRandomColor(newPlanet.randomness);
        newPlanet.color=_metadataGenerator.pickRandomColor(newPlanet.randomness+randomNum(87, newPlanet.randomness, block.timestamp));
        newPlanet.color2=_metadataGenerator.pickRandomColor(newPlanet.randomness+randomNum(97, newPlanet.randomness, block.timestamp));
        newPlanet.lineColor=_metadataGenerator.pickRandomColor(newPlanet.randomness+randomNum(111, newPlanet.randomness, block.timestamp));

        //random size
        newPlanet.radius=30+uint16(randomNum(170, newPlanet.randomness, block.timestamp));

        //random angle
        uint16 _angletype=uint16(randomNum(4, newPlanet.randomness, block.timestamp));

        newPlanet.rotationX=20+uint16(randomNum(600-newPlanet.radius, newPlanet.randomness, block.timestamp));
        ...
        if(_angletype==0){
            newPlanet.rotationAngle=0;
            newPlanet.positionX=int16(700-newPlanet.radius-newPlanet.rotationX)-int16(uint16(randomNum((700-newPlanet.radius-newPlanet.rotationX)*2, newPlanet.randomness, block.timestamp)));
            newPlanet.positionY=int16(700-int16(newPlanet.radius)-int16(2*newPlanet.rotationY))-int16(uint16(randomNum((700-newPlanet.radius-newPlanet.rotationY)*2, newPlanet.randomness, block.timestamp)));

        }else if(_angletype==1){
            newPlanet.rotationAngle=45;
            newPlanet.positionX=int16(700-newPlanet.radius)-int16(uint16(randomNum(uint256(700-newPlanet.radius-uint256(longRadius)*707/1000)*2, newPlanet.randomness, block.timestamp)));
            newPlanet.positionY=int16(700-int16(newPlanet.radius)-int16(uint16(2*uint256(longRadius)*707/1000)))-int16(uint16(randomNum(uint256(700-newPlanet.radius-uint256(longRadius)*707/1000)*2, newPlanet.randomness, block.timestamp)));

        }else if(_angletype==2){
            newPlanet.rotationAngle=90;
//            newPlanet.positionX=int16(700-newPlanet.radius-newPlanet.rotationY)-int16(uint16(randomNum((700-newPlanet.radius-newPlanet.rotationY)*2, newPlanet.randomness, block.timestamp)));
//            newPlanet.positionY=int16(700-newPlanet.radius-2*newPlanet.rotationX)-int16(uint16(randomNum((700-newPlanet.radius-newPlanet.rotationX)*2, newPlanet.randomness, block.timestamp)));

        }else if(_angletype==3){
            newPlanet.rotationAngle=135;
//            newPlanet.positionX=int16(700-int16(newPlanet.radius)-int16(uint16(2*uint256(longRadius)*707/1000)))-int16(uint16(randomNum(uint256(700-newPlanet.radius-uint256(longRadius)*707/1000)*2, newPlanet.randomness, block.timestamp)));
//            newPlanet.positionY=int16(700-int16(newPlanet.radius)-int16(uint16(2*uint256(longRadius)*707/1000)))-int16(uint16(randomNum(uint256(700-newPlanet.radius-uint256(longRadius)*707/1000)*2, newPlanet.randomness, block.timestamp)));
        }


//        int16 positionX=int16(600-size)-int16(uint16(randomNum((600-size)*2, seedfinal, block.timestamp)));
//        int16 positionY=int16(600-size)-int16(uint16(randomNum((600-size)*2, seedfinal+17, block.timestamp)))-100;

        newPlanet.rotationDuration=3+uint16(randomNum(17, newPlanet.randomness, block.timestamp));

        createNewPlanet();

        //create(tokenId, seed_, bkgColor_, color_, color2_, linecolor_, size, [positionX, positionY], [rotationX, rotationY, rotationAngle, rotationDuration]);

    }

    function createNewPlanet() internal onlyAdmin{
        require(!_mintingFinalized, "Planet: Minting is finalized.");
        notSoldOut();
        require(_values[newPlanet.id].length==0, "Planet: Token already exists.");

        //newPlanet=newPlanet_;
    ...
    function create(
        ...
        validateParams(color_, color2_, size, position, rotation);
    ...
    function validateParams(
        string memory color, //rgb, saturation
        string memory color2, //rgb, saturation
        uint16 size,
        int16[2] memory position, //x y
        uint16[4] memory roation //x y angle duration
    ) internal pure {
        require(bytes(color).length>0, string(abi.encodePacked("invalid color", color)));
        require(bytes(color2).length>0, string(abi.encodePacked("invalid color2", color2)));
        require(size > 0 && size < 500, string(abi.encodePacked("invalid width size", size)));

        require(
            position[0] <= 800 && position[0] > -800,
            string(abi.encodePacked("invalid position x", position[0]))
        );
        require(
            position[1] <= 800 && position[1] > -800,
            string(abi.encodePacked("invalid position y", position[1]))
        );
        require(roation[0] > 0 && roation[0] < 800, string(abi.encodePacked("invalid roation x", roation[0])));
        require(roation[1] > 0 && roation[1] < 800, string(abi.encodePacked("invalid roation y", roation[1])));
        require(roation[2] > 0 && roation[2] < 360, string(abi.encodePacked("invalid roation angle", roation[2])));
        require(roation[3] > 0 && roation[3] < 60, string(abi.encodePacked("invalid roation duration", roation[3])));
    }
    ...
    modifier onlyValidWhitelist() {
        require(_whitelistAddress[msg.sender], "Planet: Invalid msg.sender");
        _;
    }
    ...
    function _transfer(
        ...
        if (to == _dead) {
            ...
            if (from == to) {
                ...
                // we make an exception here, as it鈥檚 easy to follow that a self transfer
            ...
            doMerge(currentTokenId, sentTokenId);
        }
...
*/
    ...
    function doMerge(
        ...
        emit readyMerge(tokenIdRcvr , tokenIdSndr);
    ...
    function setRoyaltyBips(uint256 percentageRoyalty_) external onlyAdmin{
        require(
    ...
    function whitelistUpdate(address address_, bool status) external onlyAdmin{
        if (address_ == owner()) {
            require(
                status != false,
...
/*
    function totalSupply() public view virtual override (ERC721Tradable) returns (uint256) {
    ...
    function _msgSender()
        internal view override (ERC721Tradable, Context)
...
*/
    ...
    function pickRandomColor(uint256 seed_)
        ...
        returns (string memory)
    {
        return Colors.pickRandomColor(colorsPool, seed_);
    }
    ...
    function _initPathPatterns() private {}
}
    ...
    * */
            ...
            string(
                abi.encodePacked(
                    ...
                    abi.encodePacked(
                        ...
                        '" stroke="#000" stroke-width="0" fill="url(#planet_color_',
                        planet.id.toString()
                    ),
                    abi.encodePacked(
                        ')" id="planet_',
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="800" height="800" viewBox="-800 -800 1600 1600" style="stroke-width:0;"><defs><radialGradient id="RadialGradient1"><stop offset="10%" stop-color="',
                    bkgColor,
                    '"/><stop offset="100%" stop-color="#000"/></radialGradient></defs> <rect id="svg_bg" height="1600" width="1600" y="-800" x="-800" stroke="#000" fill="url(#RadialGradient1)"></rect>',
                    body,
                    "</svg>"
                )
            );
...
library Colors {
    ...
    /** generate color */
    function pickRandomColor(string[] memory colors, uint256 seed_)
        internal
        view
        returns (string memory)
    {
