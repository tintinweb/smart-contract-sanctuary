...
contract GhostMarketplace is Ownable {
  ...
  function getGhostsOwned(address subject) external view returns(uint[] memory,bool[] memory) {
  ...
  function isActiveListing(uint ghostID) public view returns(bool) {
    return priceOf[ghostID]!=0 && ghostsContract.getApproved(ghostID)==ghostsContract.ownerOf(ghostID);
  }
