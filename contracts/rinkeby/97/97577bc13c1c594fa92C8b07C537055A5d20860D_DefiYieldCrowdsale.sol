...
contract Crowdsale is Context {
  ...
  */
  function collateralRaised() external view returns (uint256) {
   ...
   */
  function buyTokens(address beneficiary, uint256 collateralAmount) external {
...
contract DefiYieldCrowdsale is WhitelistCrowdsale {
  ...
  constructor(address wallet, IERC20 token, IERC20 collateral, TokenVesting vesting)
    Crowdsale(1, wallet, token, collateral) {

    _vesting = vesting;
    token.safeApprove(address(_vesting), 2**256 - 1);
  }
   ...
   */
  function getVestingPool(uint256 index) external view returns (uint8) {
    return _rounds[index].pool;
  }
   ...
   * @param pool Vesting pool
   */
  function addRound(uint256 rate, uint256 totalSupply, uint8 pool) external onlyAdmin {
    require(_state == State.None, "DefiYieldCrowdsale: sales is already open or closed");
   ...
   * @param pool Vesting pool
   */
  function updateRound(uint256 index, uint256 rate, uint256 totalSupply, uint8 pool) external onlyAdmin {
    require(_state != State.Closed, "DefiYieldCrowdsale: sales is already closed");
  ...
  function openRound(uint256 index) external onlyAdmin {
    ...
    require(_rounds[index].defined, "DefiYieldCrowdsale: no round with provided index");
    require(_rounds[index].state == State.None, "DefiYieldCrowdsale: round is already open or closed");
  ...
  function closeRound(uint256 index) external onlyAdmin {
    require(_state == State.Open, "DefiYieldCrowdsale: sales is not open yet");
    require(_rounds[index].defined, "DefiYieldCrowdsale: no round with provided index");
    require(_rounds[index].state == State.Open, "DefiYieldCrowdsale: round is not open");
  ...
  function withdrawTokens(address beneficiary) external {
    ...
    for(uint256 i; i < roundsLength; i++) {
      uint256 amount = _balances[beneficiary][i];

      if(amount == 0) {
        continue;
      }

      _balances[beneficiary][i] = 0;
      _tokensWithdrawn = _tokensWithdrawn.add(amount);
      uint256 directWithdrawAmount = amount.mul(_directWithdrawPercentage).div(100);
      uint256 vestingAmount = amount.sub(directWithdrawAmount);
      ...
      if (vestingAmount > 0) {
        _vesting.addBeneficiary(_rounds[i].pool, beneficiary, vestingAmount);
      }
    }
  }
   ...
   */
  function balanceOf(uint256 index, address account) public view returns (uint256) {
  ...
  function _processPurchase(address beneficiary, uint256 tokenAmount) internal virtual override {
    ...
    _balances[beneficiary][_activeRound] = _balances[beneficiary][_activeRound].add(tokenAmount);
  ...
  function recoverERC20(address token, uint256 amount) external onlyAdmin {
    if (address(getToken()) == token) {
      ...
      require(tokensAfterAdminWithdrawal >= tokensLocked, "DefiYieldCrowdsale: Cannot withdraw already sold tokens");
    }
