...
contract NightOwls is ERC721Enumerable, CollaborativeOwnable, ReentrancyGuard {
  ...
  uint256 public constant maxPrepaidQuantity = 4;
  ...
  function remainingForMint() public view returns (uint256) {
    ...
    uint256 available = maxSupply.sub(ts.add(reservedCount).add(reservedForPrepaid));
    return available;
  }
  ...
  function whitelistMint(bytes32[] calldata merkleProof, uint256 quantity) external payable nonReentrant {
    require(whitelistMintIsActive);
    require(quantity > 0 && quantity <= whitelistMintLimit && quantity <= remainingForMint());
    require(!_whitelistMinted[_msgSender()]);
    require(whitelistMintPrice.mul(quantity) <= msg.value);

    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(merkleProof, whitelistMerkleRoot, leaf));
    ...
    _whitelistMinted[_msgSender()] = true;
  }

  function prepaidMint(bytes32[] calldata merkleProof, uint256 quantity) external nonReentrant {
    require(prepaidMintIsActive);
    require(quantity > 0 && quantity <= reservedForPrepaid);
    require(!_prepaidMinted[_msgSender()]);

    bytes32 leaf = keccak256(abi.encodePacked(_msgSender(), quantity));
    ...
    _prepaidMinted[_msgSender()] = true;
    reservedForPrepaid = reservedForPrepaid.sub(quantity);
  }
  ...
  function setPrepaidMerkleRoot(bytes32 newRoot, uint256 newReserved) external onlyCollaborator {
    require(newReserved <= maxSupply.sub(reservedCount));
