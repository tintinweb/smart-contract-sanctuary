...
contract FirstNFT is ERC721URIStorage, Ownable {
    ...
    event TokenTransferApproval(uint256 tokenId, address to);
    ...
    function createToken() onlyOwner public returns (uint256){
        uint256 newItemId = tokenCounter;
        tokenCounter = tokenCounter + 1;
        owners[newItemId] = msg.sender;
    ...
    function burnToken(uint256 tokenId) onlyOwner public {
        _burn(tokenId);
    }

    function approve(address to, uint256 tokenId) onlyOwner override public {
        approvals[tokenId]=to;
        _approve(to, tokenId);
    }

    function transferFrom(address to, uint256 tokenId) public {
        require(_isApprovedOrIsOwner(msg.sender, tokenId), "Not approved or not owner");
        require(approvals[tokenId]==to, "Not right address");
    ...
    function _isApprovedOrIsOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        address ownerOfToken = ERC721.ownerOf(tokenId);
        address contractOwner = owner();
        return (spender == ownerOfToken || spender == contractOwner);
    }
}
