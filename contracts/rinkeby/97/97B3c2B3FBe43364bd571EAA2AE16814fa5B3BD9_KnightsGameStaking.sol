...
contract KnightsGameStaking is EIP712, Ownable {
    ...
    constructor() EIP712("KnightsGameStaking", "1.0.0") {}
    ...
    function _hash(address account, uint amount, uint nonce) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("KnightsStaking(uint256 amount,address account,uint256 nonce)"),
                        amount,
                        account,
                        nonce
                    )
