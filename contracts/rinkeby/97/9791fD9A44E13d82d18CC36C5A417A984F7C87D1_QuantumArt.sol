...
contract QuantumArt is IQuantumArt, OSContractURI, Ownable, Manageable, ERC721Enumerable {
    ...
    constructor(address admin) ERC721("Quantum Art", "QART") {
    ...
    function setContractURI(string calldata uri) onlyOwner override public {
    ...
    function createDrop(address artist, uint128 max) onlyRole(MANAGER_ROLE) public {
        ...
        drops[_nextDropId++] = Drop({artist:artist, max:max, circulating:0});
    }
    ...
    function mintTo(uint256 dropId, address to) onlyRole(MINTER_ROLE) public override returns (uint256) {
        ...
        require(drops[dropId].circulating + 1 <= drops[dropId].max, "MINT:DROP MAX REACHED");
        uint256 tokenId = (dropId * SEPARATOR) + (drops[dropId].circulating++);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(dropToIPFS[dropId]).length > 0) {
            return string(abi.encodePacked("ipfs://", dropToIPFS[dropId], "/", virtualTokenId.toString()));
        } else {
            return string(abi.encodePacked(_baseURI(), dropId.toString(), "/", virtualTokenId.toString()));
        }
    }

    function getArtist(uint256 dropId) public view override returns (address) {
        return drops[dropId].artist;
    }
