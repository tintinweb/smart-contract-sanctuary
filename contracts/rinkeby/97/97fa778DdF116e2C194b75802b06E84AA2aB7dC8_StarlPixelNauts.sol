...
/// @notice A contract for pixel nauts in the starlink ecosystem
contract StarlPixelNauts is Ownable, ERC721Enumerable {
    ...
    string public pendingTokenUri;
    mapping(address => uint256) public addressCanMint;
    mapping(uint256 => bytes32) private tokenHashes;

    constructor (string memory _initBaseURI, string memory _pendingTokenUri) ERC721("Starlink PixelNauts", "STPN") {
        setBaseURI(_initBaseURI);
        pendingTokenUri = _pendingTokenUri;
    }
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            ...
            require(
                ...
                "You cant mint more items than double maxMintAmount"
            );
        }
        ...
        if (addressCanMint[msg.sender] == 0) {
            addressCanMint[msg.sender] = maxMintAmount.mul(2).sub(_mintAmount);
        } else {
            addressCanMint[msg.sender] = addressCanMint[msg.sender].sub(_mintAmount);
        }
    }

    function revealTokenURIs(uint256 tokenIdFrom, bytes32[] calldata hashes) public onlyOwner {
        uint256 supply = totalSupply();
        require(tokenIdFrom <= supply, "invalid tokenIdFrom");
        require(tokenIdFrom.add(hashes.length).sub(1) <= supply, "invalid length of hashes");

        for(uint256 i=0; i<hashes.length; i++) {
            bytes32 hash = hashes[i];
            tokenHashes[tokenIdFrom+i] = hash;
        }
    }
        ...
        return
            hash[0] > 0
                ? string(
                    abi.encodePacked(
                        currentBaseURI,
                        toBase58(concat(sha256MultiHash, abi.encodePacked(hash)))
                    )
                )
                : pendingTokenUri;
    }
