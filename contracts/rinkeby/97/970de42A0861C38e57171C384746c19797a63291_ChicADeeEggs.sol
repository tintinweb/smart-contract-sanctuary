...
contract ChicADeeEggs is ERC20, Ownable {
    ...
    function _ownsChic(uint256 id) internal virtual returns(bool owns){
        ...
        // (bool success, bytes memory payload) = chicContract.call{value: msg.value, gas: 22000}(
        //     abi.encodeWithSignature("balanceOf(address, uint256)", msg.sender, id)
        // );

        // require(success, "Could not get Chic balance");
    ...
    function _harvestEggs(uint256 id) internal virtual {
        ...
        // Both users trigger block reset
    ...
    function startOneLaying(uint256 id) public virtual {
        ...
        require(_ownsChic(id), "Ownerfy: must own all these Chics");
    ...
    function checkChicBalance(uint256 id) public view returns (uint256 balance){
        if(chicStatusMap[id].startBlock != 0) {
          return block.number - chicStatusMap[id].startBlock;
        }
        return 0;   
    }
