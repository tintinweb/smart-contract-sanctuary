...
contract DEXF is BEP20Interface, Pausable {
    ...
    constructor () {
        ...
        if (pair != address(0)) {
            _pancakeswapV2Pair = pair;
        } else {
            _pancakeswapV2Pair = address(IPancakeSwapV2Factory(_pancakeswapV2Router.factory())
    ...
    function setStakingContract(address stakingContract) external onlyOwner {
        require(_stakingContract == address(0), "Dexf: Staking contract already initialized");
    ...
    function _isSell(address sender, address recipient) internal view returns (bool) {
        ...
        return sender != address(_pancakeswapV2Pair) && recipient == address(_pancakeswapV2Pair);
    }

    function _isBuy(address sender) internal view returns (bool) {
        // Transfer from pair is a buy swap
        return sender == address(_pancakeswapV2Pair);
    }
