...
contract MyLootNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
// defines options for each asset/ line of the file. 
...
// gets a random string. keccak256 is an online hash function. 
function random(string memory input) internal pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(input)));
    }
// creates assets based on the random string from above. 
...
// creates the NFT with the random assets. 
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #', toString(tokenId), '", "description": "testing 1,2,3...", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        output = string(abi.encodePacked('data:application/json;base64,', json));

        return output;
    }

// Sets the maximum number of tokens that can be created. 
    function claim(uint256 tokenId) public nonReentrant {
        require(tokenId > 0 && tokenId < 9000, "Token ID invalid");
        _safeMint(_msgSender(), tokenId);
    }

// Adds 1 to the token IDs when one is printed.
    ...
    constructor() ERC721("MyLootNFT", "MYLOOTNFT") Ownable() {}
}
