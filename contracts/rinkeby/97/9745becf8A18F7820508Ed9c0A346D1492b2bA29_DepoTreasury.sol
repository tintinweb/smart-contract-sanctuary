...
contract DepoTreasury is DepoAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for DEPO
    ...
    function deposit(
        ...
        // mint DEPO needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        DEPO.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn DEPO for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        DEPO.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new DEPO using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        DEPO.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or DEPO from the treasury, using sDEPO as collateral. This might allow an
     * sDEPO holder to provide DEPO liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(DEPO)) {
            require(permissions[STATUS.DEPODEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sDEPO.changeDebt(value, msg.sender, true);
        require(
            sDEPO.debtBalances(msg.sender) <= debtLimit[msg.sender],
            "Treasury: exceeds limit"
        );
        totalDebt = totalDebt.add(value);

        if (_token == address(DEPO)) {
            DEPO.mint(msg.sender, value);
            depoDebt = depoDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
        ...
        override
    {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sDEPO.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with DEPO
     * @param _amount uint256
     */
    function repayDebtWithDEPO(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] ||
                permissions[STATUS.DEPODEBTOR][msg.sender],
            notApproved
        );
        DEPO.burnFrom(msg.sender, _amount);
        sDEPO.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        depoDebt = depoDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(DEPO), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SDEPO) {
            sDEPO = IsDEPO(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SDEPO) {
            // 9
            sDEPO = IsDEPO(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(DEPO.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns DEPO valuation of asset
     * @param _token address
     * @param _amount uint256
    ...
    {
        value_ = _amount.mul(10**IERC20Metadata(address(DEPO)).decimals()).div(
    ...
    function baseSupply() external view override returns (uint256) {
        return DEPO.totalSupply() - depoDebt;
    }
}
