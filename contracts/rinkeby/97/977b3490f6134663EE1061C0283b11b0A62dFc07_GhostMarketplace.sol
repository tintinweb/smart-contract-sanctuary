...
contract GhostMarketplace is Ownable {
  ...
  function getAllGhostsOnSale() public view returns(uint[] memory){
    ...
    for (uint i = 0;i<totalSupply;i++) {
      if (isListed(i) && ghostsContract.getApproved(i)==address(this)) {
        ghostsOnSale++;
      }
    }
  ...
  function getInventory(address subject) external view returns(uint[] memory,bool[] memory) {
    uint balance = ghostsContract.balanceOf(subject);
    uint[] memory ghosts = new uint[](balance);
    bool[] memory listedStatuses = new bool[](balance);

    for (uint i=0;i<balance;i++) {
      ghosts[i] = ghostsContract.tokenOfOwnerByIndex(subject,i);
      listedStatuses[i] = isActiveListing(ghosts[i]);
    }
    return (ghosts,listedStatuses);
  }
  ...
  function isActiveListing(uint ghostID) public view returns(bool) {
    return priceOf[ghostID]!=0 && ghostsContract.getApproved(ghostID)==address(this);
  }
