...
contract AAANAME is Context, IERC20, Ownable { // CHANGE AAANAME to nominal name
    using SafeMath for uint256;

    string private constant _name = "Pearl Test"; // CHANGE token name
    string private constant _symbol = "TPEARL";   // CHANGE token symbol
    ...
    uint256 public _liquidityCutPct = 2;
        ...
        if (!_isAdmin[from] && !_isAdmin[from]) {
            ...
            if (!inSwap && swapEnabled && to == uniswapV2Pair) { // Selling
                ...
                if (sellNumber[from] == 0) { //CHANGEIT Adjust cooldown timings
                    ...
                    _teamFee = 10;
                    sellNumber[from]++;
                    sellCooldown[from] = firstSell[from] + (240 seconds);
                }
            }
