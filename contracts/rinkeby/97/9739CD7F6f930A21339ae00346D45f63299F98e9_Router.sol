...
contract Router is Ownable, ReentrancyGuard, IERC1155Receiver {
  ...
  function estimateSwapAnyTicketFee(
  ...
  function estimateSwapAnyTokenFee(
    uint256[] memory fromIds_,
    uint256[] memory fromValues_,
    uint256[] memory toIds_,
    uint256[] memory toValues_
  ...
  function estimateSwapAnyTicketDai(
    uint256[] memory fromIds_,
    uint256[] memory fromValues_,
    uint256[] memory toIds_,
    uint256[] memory toValues_
    ...
    for (uint256 i; i < fromIds_.length; i++) {
      (,,address poolAddress) = factory.tickets(fromIds_[i]);
      daiBalance += int256(IPool(poolAddress).estimateSwapToDaiByToken(fromValues_[i] * 1 ether));
    }

    for (uint256 i; i < toIds_.length; i++) {
      (,,address poolAddress) = factory.tickets(toIds_[i]);
      daiBalance -= int256(IPool(poolAddress).estimateSwapFromDaiByToken(toValues_[i] * 1 ether));
    }

    return daiBalance;
  }

  function estimateSwapAnyTokenDai(
    uint256[] memory fromIds_,
    uint256[] memory fromValues_,
    uint256[] memory toIds_,
    uint256[] memory toValues_
  ...
  function swapToken(uint256 tokenId0_, uint256 amount0_, uint256 tokenId1_, uint256 amount1Min_) external nonReentrant {
    ...
    IERC20(factory.daiToken()).transferFrom(_msgSender(), address(this), estimateTokenSwapFee(tokenId0_, amount0_, tokenId1_));
  ...
  function _swapAny(
    ...
    IERC20(factory.daiToken()).transferFrom(
      _msgSender(),
      address(this),
      estimateSwapAnyTokenFee(fromIds_, fromValues_, toIds_, toValues_)
    );

    for (uint256 i; i < fromIds_.length; i++) {
      (,address tokenAddress, address poolAddress) = factory.tickets(fromIds_[i]);
    ...
    int256 delta = estimateSwapAnyTokenDai(fromIds_, fromValues_, toIds_, toValues_);
    if (delta > 0) {
    ...
    for (uint256 i; i < toIds_.length; i++) {
      (,address tokenAddress, address poolAddress) = factory.tickets(toIds_[i]);
      IERC20(factory.daiToken()).approve(poolAddress, bigNum);
      ITicketToken(tokenAddress).approve(poolAddress, bigNum);
