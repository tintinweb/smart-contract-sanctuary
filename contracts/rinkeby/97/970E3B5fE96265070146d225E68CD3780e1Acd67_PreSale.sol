...
// PreSale must be ended before the start of the general minting.
contract PreSale is Ownable, IPreSale{
    ...
    uint256 public TOTAL_PRESALE;
    uint256 public override reservedAmount = 0;
    bool public override preSaleIsActive = false;

    constructor(uint256 maxPerWallet, uint256 totalPresaleAmount){
        MAX_PER_WALLET = maxPerWallet;
        TOTAL_PRESALE = totalPresaleAmount;
    }
    ...
    function getPresaleSpot(uint256 numberOfTokens) public override payable {
        ...
        require(whitelist[msg.sender] + numberOfTokens <= MAX_PER_WALLET, "Max allowance exceeded.");
        require(reservedAmount + numberOfTokens <= TOTAL_PRESALE, "Can not mint this much.");
