...
contract DHStaking is ERC721, ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Staked DeadHeads", "sDEAD") {
        // uint[5] memory dhAvailablePeriods = [uint(1), 3, 6, 12, 24];
        // uint[5] memory dhMonthlyRewards = [uint(4000), 8000, 24000, 48000, 80000];
        
        // addContract(0xC3F78f50222b3CAB2eD2F73D07Ed28131d1EBAA3, "", dhAvailablePeriods, dhMonthlyRewards);
    }
    ...
    function unclaimedRewards(uint localTokenId) public view returns (uint) {
        ...
        return (rewardsUntilNow > rewardLimit ? rewardLimit : rewardsUntilNow)
            * stakedToken.monthlyRewards / 100 / 4 - localTokenIdToClaimedRewards[tokenContract][localTokenId];
    }
    ...
    function unstake(uint localTokenId) external {
    ...
    function tokenURI(uint256 localTokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        address tokenContract = localTokenIdToContract[localTokenId];
        PartnerContract storage _contract = contracts[tokenContract];
        uint tokenId = localTokenIdToStakedToken[tokenContract][localTokenId].tokenId;
        return string(abi.encodePacked(_contract.baseURI, tokenId.toString()));
    }
