    ...
    RandomlyAssigned
{
    ...
    bytes32[10000] public tokenIDNames;
    ...
    mapping(address => uint8) public pledgeNumOfUser;
    mapping(address => uint8) public claimed;

    event WhitelistedAddressRemoved(address addr);
    event GivenName(uint256 indexed tokenId, bytes32 name);

    /**
     * @dev throws error if called by any wallet that is not whitelisted
     */
    modifier onlyWhitelisted() {
        require(whitelist[msg.sender], "Placeholder: You're not on the whitelist.");
        _;
    }
    ...
    constructor(
        ...
        address _payment
    ) ERC721(_name, _symbol) RandomlyAssigned(10000, 1) {
        baseTokenURI = _uri;
    ...
    /**
     * @dev Pledge for the purchase. Each address can only purchase up to 5 Placeholders. Only whitelisted users can pledge.
     * @param _num Quantity to purchase
     */
    function pledge(uint8 _num) external payable onlyWhitelisted {
        require(!paused, "Placeholder: Pledge has not started yet.");
        require(
            (_num + pledgeNumOfUser[msg.sender] + claimed[msg.sender]) <= 5,
            "Placeholder: Each address can only purchase up to 5 Placeholders."
        );
        require(
            totalMint + uint256(_num) <= MAX_SUPPLY - totalPledge - 10,
            "Placeholder: Sorry, all Placeholders have been sold."
        );
        require(
            msg.value > uint256(_num) * 6e16,
            "Placeholder: You need to pay at least the required cost."
        );
        pledgeNumOfUser[msg.sender] = pledgeNumOfUser[msg.sender] + _num;
        totalPledge += uint256(_num);
    }

    /**
     * @dev Your Placeholders can only be claimed at the end of the sale.
     */
    function claim() external {
        require(!paused, "Placeholder: Claim has not started yet.");
        _mintList(pledgeNumOfUser[msg.sender]);
        claimed[msg.sender] += pledgeNumOfUser[msg.sender];
        pledgeNumOfUser[msg.sender] = 0;
    }
    ...
    function mint(uint8 _num) external payable {
        require(
            totalMint + uint256(_num) <= MAX_SUPPLY - totalPledge - 10,
            "Placeholder: Sorry, all Placeholders are sold out."
        );
        require(_num <= 5, "Placeholder: Up to 5 Placeholders can be minted in a tx.");
        require(
            msg.value > uint256(_num) * 8e16,
            "Placeholder: You need to pay at least the required cost."
        );
        _mintList(_num);
    ...
    */
    ...
    /**
     * @dev allow people to set the name of their Placeholder
     */
    function setName(uint256 _tokenId, bytes32 _name) external {
        require(
            ownerOf(_tokenId) == msg.sender,
            "Placeholder: You don't own this Placeholder!"
        );
        tokenIDNames[_tokenId] = _name;
        emit GivenName(_tokenId, _name);
    }

    /**
     * @dev Return the name of a Placeholder given ID
     */
    function getName(uint256 _tokenId) external view returns (bytes32) {
        return tokenIDNames[_tokenId];
    }
    ...
    /**
     * @dev Check the unclaimed Placeholder amount of a wallet
     */
    function getUnclaimedAmount(address _owner) external view returns (uint8) {
        return pledgeNumOfUser[_owner] - claimed[_owner];
    }
    ...
    function tokenURI(uint256 _tokenId)
        ...
        returns (string memory)
    {
        require(_exists(_tokenId), "Placeholder: This token does not exist!");
        if (revealed == false) {
    ...
    /**
     * @dev withdraw all balance of this contract to the Payment contract
     */
    ...
    /**
     * @dev get the ids of the Placeholders owned by _owner
