...
contract TerrainsContractData is Ownable, ERC721, ERC721Holder {
    ...
    function setGlobalParameters(address methods) external onlyOwner {
        ITerrains(methods).setGlobalParameters(methods);
    }

    function createTerrain(Arena.Default memory terrain) external onlyOwner {
        ITerrains(methodsContractAddress).createTerrain(terrain);
    }
    
    function editTerrain(uint256 _id, Arena.Default memory terrain) external onlyOwner {
        ITerrains(methodsContractAddress).editTerrain(_id,terrain);
    }
    
    function getTerrain(uint256 _id) external view returns(Arena.Default memory) {
        return terrains[_id];
    }
