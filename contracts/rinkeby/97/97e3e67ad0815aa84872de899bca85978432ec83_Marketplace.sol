...
contract Marketplace is Library {
    ...
    function ViewAllow(string memory _user, address _addr, uint256 _count)
    public view returns(bool, bool, bool) {
        if(keccak256(abi.encodePacked(_user)) == keccak256(abi.encodePacked("buyer"))) {
            return(BuyerBoard.board[_addr][_count].isAllow.isBuyerAllow, BuyerBoard.board[_addr][_count].isAllow.isSellerAllow, BuyerBoard.board[_addr][_count].isAllow.isDealFlag);
        }
        else if(keccak256(abi.encodePacked(_user)) == keccak256(abi.encodePacked("seller"))) {
            return(SellerBoard.board[_addr][_count].isAllow.isBuyerAllow, SellerBoard.board[_addr][_count].isAllow.isSellerAllow, SellerBoard.board[_addr][_count].isAllow.isDealFlag);
        }
        else {
            revert();
        }
