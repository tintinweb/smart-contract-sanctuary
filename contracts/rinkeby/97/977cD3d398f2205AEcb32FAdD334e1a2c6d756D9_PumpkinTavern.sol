...
contract PumpkinTavern is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_RESERVED_PUMPKINS = 500;
    ...
    constructor() ERC721("Spooky Pumpkin Tavern","Pumpkin") {
        ...
        _contractURI = 'ipfs://QmPcA2rsEVFkTdojCEcpt5Lj6sx5PXB7qKntyy8dZ8NvEJ';
        _baseTokenURI = 'ipfs://QmYXeQ5otZfUB7gN2faARbxaQB2mtHKZ6EWRYqi2RqnJ9C';
    }
    ...
    function claimReserved(address recipient, uint256 amount) external onlyOwner {
        require(reservedClaimed.add(amount) <= MAX_RESERVED_PUMPKINS, "Minting would exceed reserved number");
    ...
    function mintPresale(uint256 amount) external payable isPresaleActive {
        ...
        require(amount <= presaleMaxMint, "Cannot purchase this many tokens during presale");
        require(totalSupply().add(amount) <= MAX_SUPPLY, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender].add(amount) <= presaleMaxMint, "Purchase exceeds max allowed");
        require(amount > 0, "Must mint at least one");
        require(msg.value >= NFT_PRICE.mul(amount), 'ETH amount to purchase is too low');
