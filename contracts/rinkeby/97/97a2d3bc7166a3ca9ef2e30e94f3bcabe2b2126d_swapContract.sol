...
contract swapContract is AdminsContract{
    ...
    function approveForShare(IERC20 _token) internal{
    ...
    function exchangeTokenAirdrop(address _airdropAddress, uint256 _amount) public isAdmin{
        IERC20 _token = IERC20(_airdropAddress);
        if (_amount/userIDCounter > 0){
            for (uint256 i = 0; i < userIDCounter; i++){
                _token.transfer(userId[i], _amount/userIDCounter);
            }
        }
    }
}