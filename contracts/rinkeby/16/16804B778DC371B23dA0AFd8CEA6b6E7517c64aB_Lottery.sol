...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    function startLottery() public onlyOwner {
        require(lottery_state == LOTTERY_STATE.CLOSED, "not closed");
        lottery_state = LOTTERY_STATE.OPEN;
    }

    // function endLottery() public onlyOwner {
    //     uint256 rand = uint(keccak256(abi.encodePacked(block.number, msg.sender, block.difficulty, block.timestamp)));
    //     uint256 randWinnerIdx = rand % players.length;
    //     winner = players[randWinnerIdx];
