...
contract Test2 is IERC721 {
    ...
    uint public constant TOKEN_LIMIT = 2;
    ...
    // makes bytes32 and 10000
    string[1] public ipfsHashes = ['bafybeibnsoufr2renqzsh347nrx54wcubt5lgkeivez63xvivplfwhtpym'];
    ...
    function mint() external payable reentrancyGuard returns (uint) {
        uint salePrice = 1*10**17 wei;
        require(msg.value >= salePrice, "Insufficient funds to purchase.");
        if (msg.value > salePrice) {
            msg.sender.transfer(salePrice);
        }
        // look into multi-sig setup for beneficiary address? sign a separate  contract and unlock the funds 
    ...
    function _mint(address _to) internal returns (uint) {
        ...
        tokenIdToIpfsHash[id] = ipfsHashes[id];
        _addNFToken(_to, id);
    ...
    function tokenURI(uint256 _tokenId) external view validNFToken(_tokenId) returns (string memory) {
        return string(abi.encodePacked("ipfs.io/ipfs/", ipfsHashes[_tokenId],"/metadata.json"));
    }
