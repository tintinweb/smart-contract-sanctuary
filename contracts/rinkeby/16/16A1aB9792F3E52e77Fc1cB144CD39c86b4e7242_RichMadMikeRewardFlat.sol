...
contract RichMadMikeRewardFlat is ERC721Enumerable, Ownable {
	//call up functions
	using Strings for uint256;

	//global variables
	uint256 public constant PRIVATE_AMT = 10;
	uint256 public constant PUBLIC_AMT = 40;
	uint256 public constant MAX_AMT = PRIVATE_AMT + PRIVATE_AMT;
	uint256 public constant PRESALE_PRICE = .01 ether;
	uint256 public constant REGULAR_PRICE = .035 ether;
	uint256 public constant SUK_PER_MINT = 20;
	uint256 public constant RESERVED = 5;
	uint256 public constant GIFT_AMOUNT = 10;


	//private variables addresses
	address private _signerAddress;
	address private _withdraw = 0x874552a6902841A3f8A4eE5B9427982E3930782D;
	...
	constructor() ERC721('RichMadMikeFlat', 'RMMF') {
	...
	function buy(uint256 tokenQuantity) external payable {
		...
		require(publicAmountMinted + tokenQuantity <= PRIVATE_AMT, 'EXCEED_PUBLIC');
		require(tokenQuantity <= SUK_PER_MINT, 'EXCEED_SUK_PER_MINT');
		require(REGULAR_PRICE * tokenQuantity <= msg.value, 'INSUFFICIENT_ETH');
		...
		for (uint256 i = 0; i < tokenQuantity; i++) {
        	...
        	giftValue = (supply + 1) % 5 == 0 ? address(this).balance * GIFT_AMOUNT / 100 :  0;
	...
	function presaleBuy(uint256 tokenQuantity) external payable {
		...
		require(totalSupply() < MAX_AMT, 'OUT_OF_STOCK');
		require(
			privateAmountMinted + tokenQuantity <= PRIVATE_AMT,
			'EXCEED_PRIVATE'
		);
		require(
	...
	// Generate randome # to pick who gets ether gift
