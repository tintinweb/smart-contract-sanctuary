...
contract JustBackgrounds is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    /**
     * @dev Mints collectible and selects random color.
     */
    function mintCollectible() external whenNotPaused {
        require(availableSupply() > 0, "GRNDS: Backgrounds sold out");
        _mintBackground(msg.sender, _generateRandomIndex(block.difficulty));
    }

    /**
     * @dev Mints reserved collectibles.
     */
    ...
    /**
     * @dev Mints next token, assigns color, and removes from unused array.
     */
     ...
     */
    function _generateRandomIndex(uint256 seed) private view returns (uint8) {
        uint256 randomNum = uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, blockhash(block.number), seed)));
        return uint8(randomNum % _unusedColors.length);
    }
...

{
  ...
  "libraries": {
    "contracts/Colors.sol": {
      "Colors": "0xeacbff9eab49caf2a4895953dee6ec856df31cca"
    },
    "contracts/Traits.sol": {
      "Traits": "0xc673e4e104e913b0946f040ed8085040bf060daa"
    }
  }
}