 ...
 * {ERC721Enumerable}.
 */
contract ImperiumNFT is Ownable, Context, ERC165, IERC721, IERC721Metadata {
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, uint256 tokenId) public virtual ownerOrManager{
     ...
     * Emits a {Transfer} event.
     */
    function _burn(uint256 tokenId) public virtual ownerOrManager{
        address owner = this.ownerOf(tokenId);
...
contract NFTFactory is Ownable
{
    ...
    function createNFT(string memory name, string memory symbol, string memory baseURI) public onlyOwner returns(string memory)
    {
        ImperiumNFT nft = new ImperiumNFT(name, symbol, baseURI);
        registry.push(NFTRegistry(name, nft));
        return nft.name();
    }
    
    function getRegistry() public view returns (NFTRegistry[] memory)
    {
        return registry;
    }
}