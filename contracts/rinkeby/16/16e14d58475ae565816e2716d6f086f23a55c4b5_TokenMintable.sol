...
contract TokenMintable is Token, AccessControl {
    ...
    constructor(address minter, address owner, string memory name, string memory symbol, uint256 initialSupply, address forwarder, IPolicy policy) 
    ...
    function mint(address to, uint256 amount, bytes memory userData, bytes memory operatorData) public onlyRole(MINTER_ROLE) {
        _mint(to, amount, userData, operatorData, true);
    }
    
    function mint(address to, uint256 amount, bytes memory userData, bytes memory operatorData, bool requireReceptionAck) public onlyRole(MINTER_ROLE) {
        _mint(to, amount, userData, operatorData, requireReceptionAck);
    }

    function _msgSender() internal view virtual override(Context, Token) returns (address sender) {
