...
contract TestTriangle is ERC721Enumerable, Ownable, Payment, Guard {
	...
	//phase limits
	uint256 private maxPhase0 = 1500;
	...
	//phase counters
	uint256 public phase0ReserveCount;
	...
	//phase 0 reserve (whitelist)
	function reservePhase0(address _address, address contractAddressHash, uint256 contractType) public {
	
	//phase active
	require(phase0,"Phase is not live yet");
	
	//check if address already reserved
	require(_reserved[msg.sender] == 0,"Already Reserved");

	//check reserve limit
	require(phase0ReserveCount <= maxPhase0, "Phase limit has been hit");
	
	//ERC1155 check
	if(contractType == 1155){
	ERC1155Token contractAddressToken = ERC1155Token(contractAddressHash);
    require(contractAddressToken.balanceOf(msg.sender,6) > 0, "Doesn't own the token");
	...
	function phase0switch(bool _status) public onlyOwner {
	phase0 = _status;
	}
	...
	function setphase0max(uint256 _newMax) public onlyOwner {
	maxPhase0 = _newMax;
	}
