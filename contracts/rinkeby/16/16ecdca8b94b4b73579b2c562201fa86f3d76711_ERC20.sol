...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     */
    function battleFA(uint16 _fromId, uint16 _toId) internal {
    ...
    /**
     * @dev Gets a random NFT id different from the _Id input.
     */
    function getRandomNFTId(uint16 _Id) public view returns (uint16){
        bool success;
        uint randomNFTId;
        uint _seed;
        ...
        {
            randomNFTId = uint(keccak256(abi.encodePacked(block.timestamp, _seed))).mod(IFA(_nftAddress).totalSupply()).add(1);
            if(randomNFTId == _Id)
            {
                _seed = _seed.add(1);
            }
     ...
     */
    function battle(uint16 _fromId) public {
        ...
        uint16 _toId = getRandomNFTId(_fromId);
        battleFA(_fromId, _toId); 
     ...
     */
    function setWinReward(uint _winReward) public onlyOwner {
     ...
     */
    function setLoseReward(uint _loseReward) public onlyOwner {
