...
contract Router is Ownable, ReentrancyGuard {
  ...
  public view returns(int256) {
    ...
    for(uint256 i; i < fromIds_.length; i++) {
      (,,address poolAddress) = factory.tickets(fromIds_[i]);
      daiBalance += int256(IPool(poolAddress).estimateSwapToDaiByToken(fromValues_[i]));
    }

    for(uint256 i; i < toIds_.length; i++) {
      (,,address poolAddress) = factory.tickets(toIds_[i]);
      daiBalance -= int256(IPool(poolAddress).estimateSwapFromDaiByToken(toValues_[i]));
    }
  ...
  function swapToken(uint256 tokenId0_, uint256 amount0_, uint256 tokenId1_, uint256 amount1Min_) external nonReentrant {
    require(tokenId0_ != tokenId1_, "AavegotchiTicketSwap: token ids is equal");
    ...
    require(supported0, "AavegotchiTicketSwap: token 0 is not supported");
    require(supported1, "AavegotchiTicketSwap: token 1 is not supported");

    uint256 token1Out = estimateTokenSwapOut(tokenId0_, tokenId1_, amount0_);
    require(token1Out >= amount1Min_, "AavegotchiTicketSwap: min swap amount reached");

    ITicketToken(tokenAddress0).transferFrom(_msgSender(), address(this), amount0_);
    uint256 daiAmount = IPool(poolAddress0).swapToDaiByToken(amount0_);
    uint256 out = IPool(poolAddress1).swapFromDaiByDai(daiAmount);
    ITicketToken(tokenAddress1).transfer(_msgSender(), out);

    emit TokenSwapped(_msgSender(), tokenId0_, amount0_, tokenId1_, token1Out);
  }
  ...
  external nonReentrant {
    require(fromIds_.length == fromValues_.length, "AavegotchiTicketSwap: Mismatch in fromIds and fromValues length");
    require(toIds_.length == toValues_.length, "AavegotchiTicketSwap: Mismatch in toIds and toValues length");

    uint256 beforeBalance = IERC20(factory.daiToken()).balanceOf(address(this));
 
    for(uint256 i; i < fromIds_.length; i++) {
      (bool supported, address tokenAddress, address poolAddress) = factory.tickets(fromIds_[i]);
      require(supported, "AavegotchiTicketSwap: From ticket not supported");
    ...
    for(uint256 i; i < toIds_.length; i++) {
      (bool supported, address tokenAddress, address poolAddress) = factory.tickets(toIds_[i]);
      require(supported, "AavegotchiTicketSwap: To ticket not supported");
      IPool(poolAddress).swapFromDaiByToken(toValues_[i]);
      ITicketToken(tokenAddress).transfer(_msgSender(), toValues_[i]);
    }

    int256 delta = int256(IERC20(factory.daiToken()).balanceOf(address(this))) - int256(beforeBalance);
    if (delta > 0) {
  ...
  function withdrawDai(uint256 amount_) external onlyOwner {
    IERC20(factory.daiToken()).transfer(_msgSender(), amount_);

    emit DaiWithdrawn(amount_);
  }
}
