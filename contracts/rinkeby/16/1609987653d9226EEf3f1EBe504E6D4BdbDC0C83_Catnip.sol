...
// CHANGEIT - will need to set the fees to zero and the max transfer and time to a very large number. After the presale ends, you should reset those things.
...
contract Catnip is Context, IBEP20, Initializable {
    ...
    function initialize() public initializer {
        ...
        teamWalletAddr = 0x3972FCD297a3E3d1E06BE2aB434a815D31e2B254;    // CHANGEIT - you will want to update this with the real multisig gnosis wallet safe
        timeLockAddress = 0xE330A6574D5235e069794b6bE2b051C19C5423AB;    // CHANGEIT - need the real TimeLock address here
        ...
        tokenSymbol = "NIP1";        //CHANGEIT - Change the NAme to the PRoduction one NIP
        tokenName = "Catnip1";       //CHANGEIT - Change the NAme to the PRoduction one Catnip
        MAXintNum = ~uint256(0);
        ...
        reflectTokensOwned[payableTeamWalletAddr()] = _rTotal; 
    ...
    function takeTeamFee(uint256 taxTeamFee) private {
        ...
        if (isAccountExcludedFromReward[teamWalletAddr]){
            totalTokensOwned[teamWalletAddr] = totalTokensOwned[teamWalletAddr].add(taxTeamFee);
        }
    }
    ...
    ////////////// PRESALE FUNCTION
    // TODO Build PResale Function
    function presaleBuy(uint256 amountToBuy, uint256 keyCode) external {
        ...
        approveInternal(address(this), buyer, amountToBuy);        // approves amount to claim from owner address
        transferFromIgnoreMaxLimit(address(this), buyer, amountToBuy);       // transfers the airdrop amount from the owner address to the claimer
        approveInternal(address(this), buyer, 0);        // sets the approved amount back to zero     
    }
