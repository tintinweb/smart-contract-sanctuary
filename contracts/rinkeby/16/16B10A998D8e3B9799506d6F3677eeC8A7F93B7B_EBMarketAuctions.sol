...
contract EBMarketAuctions is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    ...
    event NewAuction(uint indexed AuctionID, address[] nftAddresses, uint[] nftIds, uint32[] nftTypes, uint price, uint tax, address indexed currency);
    event AuctionCancelled(uint indexed AuctionID, address[] nftAddresses, uint[] nftIds, uint32[] nftTypes, uint price, uint tax, address indexed currency);
    event NewBid(uint indexed AuctionID, address[] nftAddresses, uint[] nftIds, uint32[] nftTypes, uint price, uint tax, address indexed currency);
    ...
    function create(address[] calldata _nftAddresses,uint[] calldata _nftIds,uint32[] calldata _nftTypes,uint _valability,uint _price,uint _tax, address _currency) external {
        ...
        emit NewAuction(id,_nftAddresses,_nftIds,_nftTypes,_price,_tax,_currency);
    ...
    function cancel(uint auctionId) external isAccesible(auctionId) nonReentrant {
        ...
        emit AuctionCancelled(auctionId,auctions[auctionId].nftAddresses,auctions[auctionId].nftIds,auctions[auctionId].nftTypes,auctions[auctionId].price,auctions[auctionId].tax,auctions[auctionId].currency);
    }
    ...
    function bid(uint auctionId) external payable isAccesible(auctionId) nonReentrant {
        ...
        emit NewBid(auctionId,auctions[auctionId].nftAddresses,auctions[auctionId].nftIds,auctions[auctionId].nftTypes,auctions[auctionId].price,auctions[auctionId].tax,auctions[auctionId].currency);
    }
