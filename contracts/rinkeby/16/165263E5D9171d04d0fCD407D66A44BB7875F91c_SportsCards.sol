...
contract SportsCards is ERC721, ERC721URIStorage, Ownable {
  ...
  function addCities(string[] calldata newCities) public onlyOwner {
    uint256 i;
  ...
  function addSports(string[] calldata newSports) public onlyOwner {
    uint256 i;

    for (i = 0; i < newSports.length; i++) {
      sports.push(newSports[i]);
    }
  }
  ...
  function withdrawCreditsFor(address owner, address receiver) onlyOwner public {
    uint amount = credits[owner];
  ...
  function tokenURI(uint256 tokenId) override(ERC721, ERC721URIStorage) public view returns (string memory) {
    ...
    parts[0] = string(abi.encodePacked('{"name": "', cityName(tokenId), ' ', sportName(tokenId),' #', Strings.toString(cardNumber(tokenId)), '",'));
    parts[1] = string(abi.encodePacked('"description": "', cityName(tokenId), ' ', sportName(tokenId),' #', Strings.toString(cardNumber(tokenId)), '\\nSN-', _leftPadNumber(Strings.toString(sequenceNumber(tokenId))), '", '));
    parts[2] = string(abi.encodePacked('"external_url": "https://sportsdao.gg/collectibles/', Strings.toString(tokenId), '", '));
    parts[3] = string(abi.encodePacked('"image": "ipns://', ipns, '/', Strings.toString(tokenId),'.png", "attributes": ['));
    parts[4] = string(abi.encodePacked('{"trait_type": "City", "value": "', cityName(tokenId), '"},'));
    parts[5] = string(abi.encodePacked('{"trait_type": "Sport", "value": "', sportName(tokenId),'"},'));
