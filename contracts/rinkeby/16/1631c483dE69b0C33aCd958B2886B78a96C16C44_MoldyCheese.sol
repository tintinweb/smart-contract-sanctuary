...
// 
contract MoldyCheese is ERC20Burnable, Ownable {
    ...
    address public ghostmiceAddress;
    ...
    constructor() ERC20("MoldyCheese", "MoldyCheese") {}

    function setGhostMiceAddress(address _ghostmiceAddress) public onlyOwner {
        ghostmiceAddress = _ghostmiceAddress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(ghostmiceAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(ghostmiceAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
