...
contract RainiEvolvingNFTv1 is AccessControl {
  ...
  function uri(uint256 id) external view returns (string memory) {
    ...
    return string(abi.encodePacked(baseUri, nftContract.getPathUri(_tokenVars.cardId), "/", _tokenVars.mintedContractChar, "l", uint2str(_tokenVars.level), "n", uint2str(_tokenVars.number), "s", uint2str(getState(id)), ".json"));
  }
