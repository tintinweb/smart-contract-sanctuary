...
contract Booster is ERC1155, Ownable, Pausable {
    ...
    constructor() ERC1155("https://booster.herokuapp.com/meta/{id}") {
    }
    ...
    function mint(uint _amount) external payable whenNotPaused {
        require(minted <= MAX_SUPPLY, "Booster supply reached limit");
        require(mintPrice(_amount) == msg.value, "Incorrect ethers value");

        minted += _amount;

        _mint(msg.sender, TICKET_ID, _amount, "");
    }

    function burn(address account, uint amount) external payable whenNotPaused {
        // TODO: create correctly working burn method
        require(msg.sender == account ||  isOperatorApproved[msg.sender], "You are not allowed to do that");
        _burn(account, TICKET_ID, amount);
    }
