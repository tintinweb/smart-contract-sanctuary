...
contract CL is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getHeadLevel(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 itemNum = rand % (heads.length + 50);

        //Non Motorcycle Heads are Rarer
        if (itemNum > 8) {
           itemNum = 0;
        }
    ...
    function getHead(uint256 tokenId) public view returns (string memory) {             
        string memory output = heads[getHeadLevel(tokenId)];
        return output;
    }    
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        output = string(abi.encodePacked(output, parts[9], parts[10], parts[10], parts[11], parts[12], parts[13], parts[14]));
...
library headFactory {
  ...
  function _getHelmet(uint256 number) internal pure returns (string memory) {
    string memory helmet =    
      ...
      //string memory all = string(abi.encodePacked(helmet, straight_long, pigtails, half_buzzed, blue_vertical, horns, beanie));

    return string(abi.encode('<g transform="scale(2 2)  translate(42 13)">', heads[number], '</g>'));
...
library tireFactory {
  ...
  function _getTires() internal pure returns (string memory) {
    string memory tires =     
            ...
            unicode'<rect x="4" y="17" width="1" height="1" fill="#30323F" />'
            unicode'</g>';
          return string(abi.encode('<g transform="scale(2 2)  translate(47 36)">', tires, '</g>'));
        }
