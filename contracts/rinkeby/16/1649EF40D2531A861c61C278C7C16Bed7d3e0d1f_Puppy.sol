...
contract Puppy is ERC721Burnable, Ownable {
    ...
    mapping (uint256 => bool) public ogDogeMintedPuppy;
    ...
    event Mint (uint256 indexed puppyId,
                address indexed ogDogeOwner,
                uint256 ogDogeId,
                uint256 mintTime);

    event Breed (uint256 indexed puppyId,
    ...
    function breedPuppyByDoggo (address doggoOwner,
        ...
        require(signatory == owner() , "GovernorAlpha::castVoteBySig: invalid signature");

        _safeMint(doggoOwner, breedIndex + 1);

        emit Breed (breedIndex + 1,
                    doggoOwner,
                    doggoId,
                    dogeOwner,
                    dogeId,
    ...
    */
    function mintPuppy() external {
        require(mintIsActive, "Mint is not enable now.");

        uint256 ogDogeCount = dogePound.balanceOf(msg.sender);
        uint256 puppyId = mintIndex;
        uint256 mintCount = 0;

        for(uint256 i = 0; i < ogDogeCount; i++) {
            if(mintCount >= maxToMint)
                break;

            uint256 ogDogeIndex = dogePound.tokenOfOwnerByIndex(msg.sender, i);
            if(!ogDogeMintedPuppy[ogDogeIndex]) {
                _safeMint(msg.sender, puppyId);
                puppyId = puppyId + 1;
                mintCount = mintCount + 1;
                ogDogeMintedPuppy[ogDogeIndex] = true;
            }
        }
    ...
    * Mint Puppy by Owner
    */
    function mintPuppyByOwner(address _to, uint256 _count) external onlyOwner {
        require(mintIsActive, "Mint is not enable now.");
