...
contract DRAGONS  is ERC721Enumerable, Ownable
{
    ...
    uint public juvenilePrice = 0.04 ether;
    uint public ancientPrice = 0.02 ether;
    uint public greatWyrmPrice = 0.01 ether;
    uint public maxQuantity =30;
    uint public baseSupply=9999;
    uint public ageSupply=10000;
    ...
    mapping(uint => EVOLVING) public evol;

    mapping(address => bool) public _addressExist; 
    mapping(string => NewArt) public _newArt;
    constructor(string memory baseURI) ERC721("DRAGONS OF MIDGUARD", "DRAGONS")  {
        setBaseURI(baseURI);
    }
    ...
    function mint(uint chosenAmount) public payable   {
        ...
        EVOLVING storage evolvData = evol[_tokenId];
    ...
    function addNewArt(string memory _artName,uint _quantity,uint _price)public onlyOwner
    {
          _newArt[_artName]=NewArt({
            name :_artName,
            quantity:_quantity,
            price:_price
           });
    ...
    function newArtMint(string memory _artName, uint _chosenAmount) public payable 
    {
        require(isPaused == false, "Sale is not active at the moment");
        require(_chosenAmount > 0, "Number of tokens can not be less than or equal to 0");
        require(_chosenAmount <= maxQuantity,"Chosen Amount exceeds MaxQuantity");
        NewArt storage newArt = _newArt[_artName];
        require(newArt.quantity>0,"Sold Out");
        require(newArt.quantity>=newArt.quantity-_chosenAmount,"chosenAmount is greater than totalSupply of this Art");
    ...
    function mintByAdmin()  public onlyOwner  {
    ...
    function updateBasePrice(uint _basePrice) public {
