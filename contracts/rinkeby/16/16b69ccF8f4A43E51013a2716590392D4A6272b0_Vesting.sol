...
contract Vesting {
    ...
    function addRecipient(address _recipient, uint256 _totalAmount, bool isPresaleBuyer) private {
        ...
        require(totalAllocated.add(_totalAmount) <= vestingAllocation, "addRecipient: Total Allocation Overflow");

        totalAllocated = totalAllocated.add(_totalAmount);
        
        recipients[_recipient] = VestingSchedule({
            totalAmount: isPresaleBuyer ? recipients[_recipient].totalAmount.add(_totalAmount) : _totalAmount,
            amountWithdrawn: 0
        });
    }
    ...
    function addNewRecipients(address[] memory _newRecipients, uint256[] memory _totalAmounts) external onlyMultiSigAdmin {
        ...
        for (uint256 i = 0; i < _newRecipients.length; i++) {
            addRecipient(_newRecipients[i], _totalAmounts[i], false);
        }
