...
contract PiggyCrew is ERC721, Ownable, PiggyStorageV0 {
    ...
    // reserve batch size
    uint256 public constant reserveBatchNum = 10;
    ...
    function setMintingState(bool _value) external onlyOwner {
    ...
    function whitelistMint(uint256[] calldata _whitelistIDs, bytes[] calldata _signatures) external payable {
        require(_whitelistIDs.length == _signatures.length, "whitelistMint: Batch size mismatch");

        require(id.current() + _whitelistIDs.length <= maxSupply, "whitelistMint: Minting would exceed max supply");
        require(whitelistPrice * _whitelistIDs.length <= msg.value, "whitelistMint: Ether value sent is not correct");

        for (uint i = 0; i < _whitelistIDs.length; i++) {
            require(!hasMinted[_whitelistIDs[i]], "whitelistMint: Whitelist already claimed");
            require(_verify(getMessageHash(_msgSender(), _whitelistIDs[i]), _signatures[i]), "whitelistMint: Invalid Signature");
