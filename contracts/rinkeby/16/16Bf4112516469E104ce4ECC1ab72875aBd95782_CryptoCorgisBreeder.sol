...
contract CryptoCorgisBreeder is Ownable, Breeder, ERC1155 {
  using SafeMath for uint256;

  event CorgiSold(uint256 tokenId, uint256 blockNumber, bytes32 corgiDna);

  uint256 public constant MAX_CORGIS_MINTED = 10000;
  uint256 public constant CORGI_LIFESPAN_BLOCKS = 32;
  uint256 public constant FIRST_CORGI_PRICE_ETH = 1e16;
  uint256 public constant INCREMENTAL_PRICE_ETH = 1e14;
  uint256 public constant NUMBER_MUTANT_CORGIS = 27;
  ...
  mapping(uint256 => uint256) public tokenIdToBlockNumber;
  mapping(uint256 => uint256) public blockNumberToTokenId;
  mapping(uint8 => bool) public mutantCorgiIdMinted;
  ...
  function canMint(uint256 _blockNumber, uint256 _amount) public view override returns (bool) {
    ...
    return blockNumberToTokenId[_blockNumber] == 0;
  }
  ...
  function isMutantCorgi(bytes32 _corgiDna) public pure returns (bool, uint8) {
    ...
    if (lastTwoBytesValue < MUTANT_CORGI_PROBABILITY) {
      return (true, uint8(lastTwoBytesValue / 6));
    }
    return (false, 0);
  }
  ...
  function mint(
    ...
    if (isMutant) {
      require(mutantCorgiIdMinted[mutantId] == false, "CryptoCorgisBreeder: Crypto Corgi mutant already claimed");
      mutantCorgiIdMinted[mutantId] = true;
    }
    tokenIdToBlockNumber[nextCorgiId] = _blockNumber;
    blockNumberToTokenId[_blockNumber] = nextCorgiId;
    _mint(_toAddress, nextCorgiId, 1, _data);
    corgisMinted = nextCorgiId;
    emit CorgiSold(nextCorgiId, _blockNumber, corgiDna);
  }
}
