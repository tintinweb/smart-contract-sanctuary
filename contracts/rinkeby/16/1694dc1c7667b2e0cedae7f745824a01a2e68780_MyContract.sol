...
function withdraw()public{
  ...
  uint256 _eth = 100000000000000000;
...
function transferIn(uint256 amount,string oid,string hx)external{
  usdt.transferFrom(msg.sender,address(this), amount);
  order memory ors=order(hx,amount);
  orderMapping[oid]=ors;
}
function getInfokey(string key)public constant returns (string, uint256){
  return (orderMapping[key].hx,orderMapping[key].order_amount);
}
