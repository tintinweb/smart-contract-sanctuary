...
interface IClvnArt {
	function balanceArts(address owner) external view returns (uint256);
}

contract ClvnCoin is ERC20, Ownable {
	IClvnArt public ClvnArt;
	...
	constructor(address clvnArtAddress) ERC20('ClvnCoin', 'CCOIN') {
		ClvnArt = IClvnArt(clvnArtAddress);
		START = block.timestamp;
	}

	function updateReward(address from, address to) external {
		require(msg.sender == address(ClvnArt));
		if (from != address(0)) {
	...
	function burn(address user, uint256 amount) external {
		require(
			allowedAddresses[msg.sender] || msg.sender == address(ClvnArt),
