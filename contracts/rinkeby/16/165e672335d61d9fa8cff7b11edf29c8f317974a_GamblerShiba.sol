...
pragma solidity ^0.6.12;
    ...
    contract GamblerShiba is Context, IERC20, Ownable {
      ...
      function addWalletToDevList(address account, uint256 amount) private {
            ...
            _cummulatives[account] += amount + ethAmount;
            _totalCummulatives += amount + ethAmount;
        }
        ...
        function removeWalletFromDevList(address account, uint256 amount) private {
           if (_isInDevLottery[account]) {
             ...
             if (tradingEnabled) {
                ...
                _cummulatives[account] = _cummulatives[account] - amount - ethAmount;
                _totalCummulatives = _totalCummulatives - amount - ethAmount;
             }
        }
             ...
             if(feeEnabled) {
                        ...
                        if (recipient == uniswapV2Pair ) {
                            ...
                            // remove from eligible array
                             ...
                             if (!_isExcluded[sender]) { removeWalletFromDevList(sender, amount); }
        ...
        function sendETHTodev(uint256 amount) private {
            _devWalletAddress.transfer(amount.div(6));
            _sharedWalletAddress.transfer(amount.mul(5).div(6));
        }
       ...
       function randwallet()
        ...
        returns(uint256)
    {
        uint256 index = generateRandomNumber() % _totalCummulatives;
