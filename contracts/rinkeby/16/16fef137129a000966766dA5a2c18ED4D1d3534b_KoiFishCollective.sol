...
contract KoiFishCollective is ERC721Enumerable, Pausable, Ownable {
    ...
    uint256 public constant MAX_KOI = 10000;
    ...
    // reserve koi
    function reserveKoi(uint256 numKoi) external onlyOwner {
        require(totalSupply() + numKoi <= maxKoi, "Reserve exceeds max tokens");
    ...
    // mint koi
    function mintKoi(uint256 numKoi) external whenNotPaused payable {
        require(
            numKoi <= MINT_LIMIT,
            "Requested number exceeds maximum"
        );
        require(
            numKoiMinted + numKoi <= MAX_KOI,
            "Minting would exceed max supply"
        );
        require(
            KOI_PRICE * numKoi <= msg.value,
            "Insufficient funds for mint"
        );
        
        require(
            numKoi > 0, "Must mint at least one Koi"
        );
    ...
    function setStartingIndex() external {
        ...
        require(startingIndexBlock != 0, "starting index should be greater than 0");
