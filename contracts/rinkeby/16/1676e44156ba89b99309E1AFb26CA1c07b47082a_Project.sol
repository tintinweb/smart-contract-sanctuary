...
contract Project is IProject {
    ...
    function inviteContractor(
        ...
        _newTotalPhaseCost += (_newTotalPhaseCost * builderFee) / 1000;
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _phaseLength; i++) {
            ...
            checkPrecision(_phaseCosts[i]);
            uint256 _oldCostWithFee = _oldCost + (_oldCost * builderFee) / 1000;
            uint256 _newCostWithFee = _phaseCosts[i] +
                (_phaseCosts[i] * builderFee) /
                1000;
        ...
        contractorAccepted
    {
        ...
        require(_phaseID < phaseCount, "Project::!Phase");
        ...
        contractorAccepted
    {
        ...
        require(!phases[_phaseID].paid, "Project::Paid");
        require(_taskSerial == taskSerial, "Project::!taskSerial");
        require(_projectAddress == address(this), "Project::!projectAddress");
        require(
            _hash1.length == _hash2.length && _hash1.length == _cost.length,
            "Project::Lengths !match"
        );
        ...
        for (uint256 i = 0; i < _hash1.length; i++) {
            ...
            phases[_phaseID].phaseToTaskList.push(taskSerial);
            nonFundedPhaseToTask[_phaseID].push(taskSerial);
            eventsInstance.taskCreated(_phaseID, taskSerial);
        }
    }
    ...
    function fundProject() public override {
        ...
        if (!_incompleteFund) {
            uint256 _count;
            uint256 _firstNonFundedPhaseID;
            ...
            if (_incompleteFund) {
                ...
                uint256[] memory _nonFundedTaskPhasesList = new uint256[](
                    _nonFundedTaskPhases.length - _firstNonFundedPhaseID
                );
                uint256 _phaseID;
                for (
                    uint256 i = _firstNonFundedPhaseID;
                    i < _nonFundedTaskPhases.length;
                    i++
                ) {
                    _nonFundedTaskPhasesList[_phaseID] = _nonFundedTaskPhases[
                        i
                    ];
                    _phaseID += 1;
                }
        ...
        nonReentrant
    {
        ...
        uint256 _taskCost = tasks[_taskID].cost;
        uint256 _oldCostWithFee = _taskCost + (_taskCost * builderFee) / 1000;
        uint256 _newCostWithFee = _newCost + (_newCost * builderFee) / 1000;
        if (_newCost != _taskCost) {
            ...
            if (_newCost < _taskCost) {
                ...
                tasks[_taskID].alerts[1] = false; // non-funded
        ...
        if (_newSC != tasks[_taskID].subcontractor) {
            tasks[_taskID].alerts[2] = false; // SCConfirmed false
            if (_newSC != address(0)) {
                _inviteSC(_taskID, _newSC); // inviteSubcontractor
            } else {
...

{
  ...
  "libraries": {
    "contracts/libraries/Tasks.sol": {
      "Tasks": "0x3ba35f1101fae3339ddb716f964922afb3fbcd31"
    }
  }
}