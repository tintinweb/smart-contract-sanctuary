...
//   }
    ...
    function refreshDelegates() public {
        address delegator = msg.sender;
        address delegatee = msg.sender;

        address currentDelegate = delegates[delegator];
        uint96 currentDelegatorBalance = uint96(accountTokens[delegator]);
        delegates[delegator] = delegatee;

        emit DelegateChanged(delegator, currentDelegate, delegatee);

        if (currentDelegate != address(0)) {
            uint32 currentDelegateNum = numCheckpoints[currentDelegate];
            uint96 currentDelegateOld = currentDelegateNum > 0 ? checkpoints[currentDelegate][currentDelegateNum - 1].votes : 0;
            uint96 currentDelegateNew = sub96(currentDelegateOld, currentDelegateOld, "PPie::_moveVotes: vote amount underflows");

            uint32 blockNumber = safe32(block.number, "PPie::_writeCheckpoint: block number exceeds 32 bits");

            if (currentDelegateNum > 0 && checkpoints[currentDelegate][currentDelegateNum - 1].fromBlock == blockNumber) {
                checkpoints[currentDelegate][currentDelegateNum - 1].votes = currentDelegateNew;
            } else {
                checkpoints[currentDelegate][currentDelegateNum] = Checkpoint(blockNumber, currentDelegateNew);
                numCheckpoints[currentDelegate] = currentDelegateNum + 1;
            }

            emit DelegateVotesChanged(currentDelegate, currentDelegateOld, currentDelegateNew);
        }

        if (delegatee != address(0)) {
            uint32 delegateeNum = numCheckpoints[delegatee];
            uint96 delegateeOld = delegateeNum > 0 ? checkpoints[delegatee][delegateeNum - 1].votes : 0;
            uint96 delegateeNew = currentDelegatorBalance;

            uint32 blockNumber = safe32(block.number, "PPie::_writeCheckpoint: block number exceeds 32 bits");

            if (delegateeNum > 0 && checkpoints[delegatee][delegateeNum - 1].fromBlock == blockNumber) {
                checkpoints[delegatee][delegateeNum - 1].votes = delegateeNew;
            } else {
                checkpoints[delegatee][delegateeNum] = Checkpoint(blockNumber, delegateeNew);
                numCheckpoints[delegatee] = delegateeNum + 1;
            }

            emit DelegateVotesChanged(delegatee, delegateeOld, delegateeNew);
        }
    }
