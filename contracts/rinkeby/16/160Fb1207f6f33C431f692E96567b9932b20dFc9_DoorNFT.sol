...
contract DoorNFT is ERC721Enumerable, Ownable {
    using Address for address;
    ...
    //list of token id with the price
    ...
    function mintPresale(uint256 amount) internal {
        ...
        for (uint256 i = 1; i <= amount; i++) {
            ...
            tokenPrices[_tokenIdCounter.current()] = price;
    ...
    function setTokenSalePrice(uint tokenid , uint _amount) public {
        require(ownerOf(tokenid) == msg.sender,"only token owner can call the function");
        tokenPrices[tokenid] = _amount;
        }

    function getTokenSalePrice(uint _tokenId) public view returns(uint){
    ...
    function transferFrom(
        ...
        require(msg.value == tokenPrices[tokenId],"amount of ether should be equal to token current price");
        uint value = msg.value / 100 * 10;
        payable(owner()).transfer(value);
        payable(ownerOf(tokenId)).transfer(msg.value-value);
        _transfer(from, to, tokenId);
    }
