...
contract DasGestell is ERC721Enumerable, ReentrancyGuard, Ownable {
    // Interfaces
    ...
    function mint() public payable nonReentrant {
        ...
        bool minterIsOwner = minter == owner();
        ...
        require(mintingState != MintingState.Closed || minterIsOwner, "DasGestell:MINT_DISABLED");

        require(totalSupply() < MAX_SUPPLY, "DasGestell:MAX_SUPPLY_REACHED");

        require(msg.value >= price, "DasGestell:INVALID_PRICE");

        require(tokenId > 0 && tokenId <= 200, "DasGestell:INVALID_TOKEN");

        require(openPaletteCount > 0 || minterIsOwner || mintingState == MintingState.Public,
            "DasGestell:NO_OPEN_PALETTE"
        );

        require(canMint(minter), "DasGestell:LIMITED_MINTING");

        require(!_exists(tokenId), "DasGestell:ALREADY_MINTED");
