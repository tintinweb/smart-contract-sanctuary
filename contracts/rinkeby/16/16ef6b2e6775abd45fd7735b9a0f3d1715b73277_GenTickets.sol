...
contract GenTickets is ERC1155, ERC1155Receiver {
    ...
    function redeemTicket(address _to, uint256 _id, uint256 _amount) public { //address, 0, 1
        ...
        // Tranche past cliff
        uint tranche = _id.div(numTicketTypes); // 1/4 = 0 //0/4 = 0
        ...
        if (tranche >= 1){
            redeemedTokens  = (ticketType.ticketSize).mul(_amount).mul(tranche);
        }
        ...
        bytes memory data;
        _mint(_to, _id.add(numTicketTypes * eligibleTraches), _amount, data); //check later
    }
