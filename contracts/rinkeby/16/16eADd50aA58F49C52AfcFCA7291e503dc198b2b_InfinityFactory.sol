...
import "./InfinityTokensPublic.sol";

contract InfinityFactory is Ownable {
    ...
    event NewInfinityTokenCreated(address indexed owner, address tokenAddress);
    ...
    function createContract(string memory name, string memory symbol) public returns (address) {
        ...
        infinityTokensPublic asset = new infinityTokensPublic(name, symbol, factoryAddress, msg.sender);
        asset.transferOwnership(msg.sender);
        emit NewInfinityTokenCreated(msg.sender, address(asset));
        return address(asset);
    }

    function setClonableTokenAddress(address _clonableTokenAddress) public onlyOwner {
        clonableTokenAddress = _clonableTokenAddress;
    }

    function createCloneContract(string memory name, string memory symbol ) public returns (address) {
        address factoryAddress = whatIsMyAddress();
        address clonedToken = Clones.clone(clonableTokenAddress);
        infinityTokensPublic asset = infinityTokensPublic(clonedToken);
        asset.init(name, symbol, factoryAddress, msg.sender);
        asset.transferOwnership(msg.sender);
        emit NewInfinityTokenCreated(msg.sender, clonedToken);
        return clonedToken;
    }
  
    function updateCosts(uint newTeamBPS) public onlyOwner {
         teamBPS = newTeamBPS;
    }
  
    function getTeamFee() public view returns (uint256 teamFee) {
        teamFee = teamBPS;
    }
...
contract infinityTokensPublic is ERC721, OwnableClone, HasSecondarySaleFees {
    ...
    function multiURI(uint256 tokenId, uint256 artworkIndex) external view returns (string memory) {
        ...
        return string(abi.encodePacked(multiURI1, addressToString(address(this)), multiURI2, integerToString(tokenId), multiURI3, integerToString(artworkIndex)));
    }
