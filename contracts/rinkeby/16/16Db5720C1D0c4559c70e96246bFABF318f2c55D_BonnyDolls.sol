...
contract BonnyDolls is Ownable, ERC721V2 {
    ...
    address public constant t1 = 0xf9Cc7c350ce727CeB84e4C0A95a00EABe88C09E4;
    address public constant t2 = 0xEbE38B545198578975571096717fB3377647dEaE;
    address public constant t3 = 0xf9Cc7c350ce727CeB84e4C0A95a00EABe88C09E4;
    address public constant t4 = 0xf9Cc7c350ce727CeB84e4C0A95a00EABe88C09E4;

    address private _signer = 0xEbE38B545198578975571096717fB3377647dEaE;

    string public baseTokenURI;

    event CreateBonnyDoll(uint256 indexed id);

    constructor(string memory baseURI) ERC721V2("Bonny Dolls", "BDS") {
        setBaseURI(baseURI);
    }
    ...
    function presaleMint(uint256 _amount, uint256 _price, bytes memory _signature) external payable saleIsOpen  {
        ...
        // TODO: 
        // 1. check price and value
        // 2. add address as claimed after mint

        address _minter = _msgSender();

        address signer = verifyAddress(_minter, _amount, _price, _signature);
    ...
    function mintWithErc20(address _tokenAddress, uint256 _count, uint256 _price) external saleIsOpen {
        uint256 total = _owners.length;
        require(total + _count <= MAX_ELEMENTS, "Max limit");
        require(total <= MAX_ELEMENTS, "All NFTs are sold out");
        require(_count <= MAX_BY_MINT, "Exceeds number");
    ...
    function claim() public {
        ...
        uint256 total = _owners.length;
        require(total + _count <= MAX_ELEMENTS, "Max limit");
        require(total <= MAX_ELEMENTS, "All Bonny Dolls are sold out");
    ...
    function verifyAddress(address _minter, uint256 _amount, uint256 _price, bytes memory _signature) public pure returns (address) {
        return ECDSA.recover(keccak256(abi.encode(_minter, _amount, _price)), _signature);
    }

    function getHash(address _minter, uint256 _amount, uint256 _price) public pure returns (bytes32) {
        return keccak256(abi.encode(_minter, _amount, _price));
    }
    ...
    function withdrawEther() external onlyOwner {
        ...
        _withdrawEther(t3, balance.mul(15).div(100));

        _withdrawEther(t4, address(this).balance);
    }
    ...
    function withdrawTokens(address _tokenAddress) external onlyOwner {
        ...
        _withdrawTokens(_tokenAddress, t3, balance.mul(15).div(100));

        _withdrawTokens(_tokenAddress, t4, IERC20(_tokenAddress).balanceOf(address(this)));
    }
