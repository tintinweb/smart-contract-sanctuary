...
contract TweetyDoge is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "TweetyDoge";
    ...
    uint256 public _marketingFee = 400; // 4% of every transaction is sent to _marketing wallet
    uint256 public _totalFees    = _taxFee.add(_liquidityFee).add(_marketingFee);

    uint256 private _previousTaxFee       = _taxFee;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 private _previousMarketingFee = _marketingFee;

    uint256 public _maxTxAmount     = 10000000 * 10**18; // 10 Million 1% 
    uint256 public _numTokensToSell = 200000 * 10**18; // 200k
    uint256 public _maxWalletBalance = 20000000 * (10**18); // 20,000,000 20 Million 2% of total supply
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        _taxFee = taxFee;
        _totalFees    = _taxFee.add(_liquidityFee).add(_marketingFee);
    }
    ...
    function setMaxWalletBalance(uint256 amount) external onlyOwner{
        _maxWalletBalance = _tTotal.mul(amount).div(100);
    }
    function setMinTokensForSell(uint256 _minToken) external onlyOwner {
        _numTokensToSell = _minToken * 10 **18;
    }
    ...
    function _transfer(
        ...
        */
        ...
        bool isOverMinTokenBalance = contractTokenBalance >= _numTokensToSell;
        if (
            ...
            contractTokenBalance = _numTokensToSell;
            uint256 _rFees = _totalFees.sub(_taxFee);
            uint256 liquidityTokens = contractTokenBalance.mul(_liquidityFee).div(_rFees);
            swapAndLiquify(liquidityTokens);
            uint256 marketingTokens = contractTokenBalance.mul(_marketingFee).div(_rFees);
    ...
    function swapAndSendToMarketing(uint256 tokens) private  {
        ...
        payable(_marketingWallet).call{value: newBalance, gas: 30000}("");
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        takeTransactionFee(address(this), tMarketing, currentRate);
