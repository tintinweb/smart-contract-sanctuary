...
contract ArtKidsOnline is ERC721Enumerable, Ownable {
  ...
  uint256 public constant BIDDING_END_TIME = 1622379600;
  ...
  constructor() ERC721("Art Kids Online Test", "FART") {        
  }
  ...
  function voteOnArtKid(uint256 myNftId, uint256 artKidToVoteForIndex) public returns (bool) {
    // Pay winner if bidding period has ended
    if (block.timestamp > BIDDING_END_TIME) {
     payWinningArtKid(); 
     return true;
    }
    ...
    require(artKids[artKidToVoteForIndex].hasVoted = false);
    // Token has been voted with
    artKids[artKidToVoteForIndex].hasVoted = true;
