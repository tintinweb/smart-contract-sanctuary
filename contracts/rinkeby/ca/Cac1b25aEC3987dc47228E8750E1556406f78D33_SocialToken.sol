...
contract SocialToken is ERC20 {
    ...
    constructor(
        ...
        require(uint256(AMMType.SIGMOID) >= _ammType);
        ammType = AMMType(_ammType);
        ...
        returns (uint256)
    {
        ...
        if (ammType == AMMType.LINEAR) {
            ...
            if (upperBound**(TWO_DECIMAL) > lowerBound**TWO_DECIMAL) {
                integral = upperBound**TWO_DECIMAL - lowerBound**TWO_DECIMAL;
            } else {
                integral = lowerBound**TWO_DECIMAL - upperBound**TWO_DECIMAL;
            }

            return multiplier.mul(integral).div(TWO_DECIMAL);
        }
        if (ammType == AMMType.QUADRATIC) {
            multiplier = targetPrice.div(targetSupply**TWO_DECIMAL);
            if (upperBound**(TWO_DECIMAL) > lowerBound**TWO_DECIMAL) {
                integral =
                    upperBound**THREE_DECIMAL -
                    lowerBound**THREE_DECIMAL;
            } else {
                integral =
                    lowerBound**THREE_DECIMAL -
                    upperBound**THREE_DECIMAL;
            }
            return multiplier.mul(integral).div(THREE_DECIMAL);
        }
        if (ammType == AMMType.EXPONENTIAL) {
            multiplier = targetPrice.mul(eD**targetSupply).div(
                eN**targetSupply
            );
            if (
                eN**upperBound.div(eD**upperBound) >
                eN**lowerBound.div(eD**lowerBound)
            ) {
                integral =
                    eN**upperBound.div(eD**upperBound) -
                    eN**lowerBound.div(eD**lowerBound);
            ...
            return multiplier.mul(integral);
        }
        return multiplier = targetPrice.div(targetSupply);
    }
     ...
     */
    function priceToMint(uint256 numTokens) internal returns (uint256) {
        uint256 effectiveSupply;
        uint256 newSupply;

        if (initialSupply >= totalSupply()) {
            effectiveSupply = initialSupply.sub(totalSupply());
        } else {
            effectiveSupply = totalSupply().sub(initialSupply);
        }
        newSupply = effectiveSupply.add(numTokens);

        uint256 amount = curveIntegral(newSupply, effectiveSupply);
        return
            amount.mul(MAX_PERCENTAGE.add(tradingSpread)).div(MAX_PERCENTAGE);
    }
     ...
     */
    function rewardForBurn(uint256 numTokens) internal returns (uint256) {
        uint256 effectiveSupply;
        uint256 lowSupply;
        ...
        lowSupply = effectiveSupply.sub(numTokens);

        uint256 fundingAmount = curveIntegral(lowSupply, effectiveSupply);
        return
            fundingAmount.mul(MAX_PERCENTAGE.sub(tradingSpread)).div(
                MAX_PERCENTAGE
            );
    }
     ...
     */
    function buyToken(uint256 numTokens) public {
        uint256 priceForTokens = priceToMint(numTokens);
        require(
            fundingToken.transferFrom(msg.sender, address(this), priceForTokens)
        );
     ...
     */
    function sellToken(uint256 numTokens) public {
        require(balanceOf(msg.sender) >= numTokens);
        _burn(msg.sender, numTokens);

        uint256 fundingTokensToReturn = rewardForBurn(numTokens);
        fundingToken.transfer(msg.sender, fundingTokensToReturn);

        emit Burned(numTokens, fundingTokensToReturn);
    }
}
