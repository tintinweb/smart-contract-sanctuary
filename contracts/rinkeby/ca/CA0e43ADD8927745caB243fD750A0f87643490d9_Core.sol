...
contract Core is OwnableUpgradeable, ICore {
    ...
    function createCondition(
        ...
        require(timestamp_ < ILP(lpAddress).phase2end(), "DISTANT_FUTURE");
        ...
        newCondition.fundBank[0] =
            (conditionsReinforcementFix * odds2_) /
            (odds1_ + odds2_);
        newCondition.fundBank[1] =
            (conditionsReinforcementFix * odds1_) /
            (odds1_ + odds2_);
        ...
        )
    {
        ...
        require(
            (condition.fundBank[1] + amount_) / condition.fundBank[0] < 10000 &&
                (condition.fundBank[0] + amount_) / condition.fundBank[1] <
                10000,
            "Core: Big difference"
        );
        require(
        ...
        require(
            outcomeWin_ == 1 || outcomeWin_ == 2,
            "Core: Wrong bet outcome"
        );
        lastBetID += 1;
        ...
        condition.totalNetBets[outcomeWin_ - 1] += amount_;
        ...
        returns (bool success, uint256 amount)
    {
        ...
        if (!currentBet.payed && (condition.outcomeWin == 999)) {
    ...
    // code 999 for cancelled conditions
    function cancel(uint256 conditionID_) external onlyMaintainer {
        Condition storage condition = conditions[conditionID_];
        require(condition.timestamp > 0, "Azuro: condition not exists");
        require(
            block.timestamp >= condition.timestamp,
        ...
        condition.outcomeWin = 999;

        ILP(lpAddress).addReserve(condition.reinforcement, 0);
        emit ConditionResolved(conditionID_, 999, 0);
    }
