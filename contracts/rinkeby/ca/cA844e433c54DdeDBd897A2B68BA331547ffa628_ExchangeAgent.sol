...
contract ExchangeAgent is IExchangeAgent, ReentrancyGuard {
    ...
    function getTokenAmountForUSDT(address _token, uint256 _inputAmount) external view override returns (uint256) {
        return _getNeededTokenAmount(USDT_TOKEN, _token, _inputAmount);
    }

    function tokenConvertForUSDTWithTwapPrice(address _token, uint256 _convertAmount)
        external
        override
        onlyWhiteList
        nonReentrant
        returns (uint256)
    {
        ...
        uint256 twapPriceInUSDT = _getNeededTokenAmount(_token, USDT_TOKEN, _convertAmount);
        require(twapPriceInUSDT > 0, "UnoRe: no pairs");
        uint256 realConvertedAmount = (twapPriceInUSDT * (100 * SLIPPAGE_PRECISION - slippage)) / 100 / SLIPPAGE_PRECISION;

        uint256 convertedAmount = _convertTokenForUSDT(UNISWAP_ROUTER, _token, _convertAmount, realConvertedAmount);
    ...
    function _convertTokenForUSDT(
        ...
        IUniswapRouter02 _dexRounter = IUniswapRouter02(_dexAddress);
        address _factory = _dexRounter.factory();
        uint256 usdtBalanceBeforeSwap = IERC20(USDT_TOKEN).balanceOf(msg.sender);
        address inpToken = _dexRounter.WETH();
        if (_token != address(0)) {
            inpToken = _token;
            TransferHelper.safeApprove(_token, address(_dexRounter), _convertAmount);
        }
        if (IUniswapFactory(_factory).getPair(inpToken, USDT_TOKEN) != address(0)) {
            ...
            path[1] = USDT_TOKEN;
            if (_token == address(0)) {
                _dexRounter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: _convertAmount}(
                    _desiredAmount,
                    path,
                    msg.sender,
                    block.timestamp
                ...
                _dexRounter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    _convertAmount,
                    _desiredAmount,
                    path,
                    msg.sender,
    ...
    function _getNeededTokenAmount(
        ...
        uint256 expectedAmount = ITwapOraclePriceFeed(twapOraclePriceFeed).consult(_token0, _inputAmount);
