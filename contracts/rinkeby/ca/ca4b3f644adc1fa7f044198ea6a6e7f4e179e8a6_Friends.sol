...
contract FriendsBase is ERC20Permit, Configurable {
    ...
    bytes32 internal constant _factorProfitBuy_ = "factorQuota";
    bytes32 internal constant _factorQuota_     = "factorProfitBuy";
...
contract Friends is FriendsBase, Extendable {
    ...
    function DOMAIN_SEPARATOR() virtual override public view viewExtend returns (bytes32) {
        return super.DOMAIN_SEPARATOR();
    }

    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) virtual override public extend {
        return super.permit(owner, spender, value, deadline, v, r, s);
    }

    function buyTotal() public view viewExtend returns(uint) {
        return _buyTotal;
    }

    function buyBuf() public view viewExtend returns(uint) {
    ...
    function price1At(uint vol) public view viewExtend returns(uint) {
        //if(buyTotal() == 0)
        //    return config[_factorPrice_];
        return config[_factorPrice_].mul(buyBuf().add(vol).add(1)).div(buyTotal().add(vol).add(1));
    }

    function price1(uint vol) public view viewExtend returns(uint) {
        return price1At(0).add(price1At(vol)).div(2);
    }

    function price2() public view viewExtend returns(uint) {
    ...
    function price() public view viewExtend returns(uint) {
        uint p1 = price1At(0);
        uint p2 = price2();
        if(p1 == 0)
            return p2;
    ...
    function prin4Bal(uint bal) public view viewExtend returns(uint) {
        return bal.mul(1e18).div(index);
    }

    function bal4Prin(uint prin) public view viewExtend returns(uint) {
        return prin.mul(index).div(1e18);
    }

    function balanceOf(address who) virtual override public view viewExtend returns(uint bal) {
        bal = _balances[who];
        if(_accts[who].isCmpd)
            bal = bal4Prin(bal);
    }

    function quotaOf(address who) public view viewExtend returns(uint) {
        return _accts[who].quota;
    }

    function lockedOf(address who) public view viewExtend returns(uint) {
        Account storage acct = _accts[who];
        (uint locked, uint unlockEnd) = (acct.locked, acct.unlockEnd);
        return _currLocked(locked, unlockEnd);
    }

    function _currLocked(uint locked, uint unlockEnd) internal view viewExtend returns(uint) {
    ...
    function unlockedOf(address who) public view viewExtend returns(uint) {
        return balanceOf(who).sub(lockedOf(who));
    }

    function unlockEndOf(address who) public view viewExtend returns(uint) {
        return _accts[who].unlockEnd;
    }

    function isCmpdOf(address who) public view viewExtend returns(bool) {
        return _accts[who].isCmpd;
    }

    function nonceOf(address who) public view viewExtend returns(uint) {
        return _accts[who].nonce;
    }

    function rewardOf(address who) public view viewExtend returns(uint) {
    ...
    function calcForce(Twitter memory twitter) public view viewExtend returns(uint) {
    ...
    function calcAirClaim(Twitter[] memory twitters) public view viewExtend returns(uint amt) {
    ...
    function calcQuota(Twitter[] memory twitters) public view viewExtend returns(uint amt) {
    ...
    function moreQuotaOf(address who, Twitter[] memory twitters) public view viewExtend returns(uint amt) {
    ...
    function APY() public view viewExtend returns(uint y) {
        (, uint r, uint period) = calcRebaseProfit(address(0));
        r = r.add(1e18);
        y = 1e18;
    ...
    function calcRebaseProfit(address who) public view viewExtend returns(uint profit, uint ratio, uint period) {
        ...
        if(who != address(0))
            profit = profit.mul(balanceOf(who)).div(cmpdSupply);
    }
    ...
    function _setAcct(address sender, uint quota, uint locked, uint lockSpan, address referrer, bool isCmpd) internal {
        ...
        totalProfit = totalProfit.add(locked.mul(config[_factorProfitAir_]));
    }
    ...
    function _updateLocked(address sender, uint amt, uint lockSpan) internal {
        ...
        totalProfit = totalProfit.add(amt.mul(config[_factorProfitBuy_]));
    ...
    function isAirClaimed(address sender, bytes32 id) public view viewExtend returns(uint flag) {
    ...
    //function buyMoreWithToken(address token, uint amt, bytes32 referrer, uint nonce, Twitter[] memory twitters, Signature[] memory signatures) external compound(referrer) extend {
    //    address sender = _msgSender();
    //    verify(sender, referrer, nonce, twitters, signatures);
    //    _updateQuota(sender, twitters);
    ...
    //function buyWithToken(address token, uint amt, bytes32 referrer) external compound(referrer) extend {
    //    address sender = _msgSender();
    //    uint value = _swapTokenToEth(sender, token, amt);
    //    _buy(sender, value);
    //}

    function calcMaxEthInOf(address who, Twitter[] memory twitters) public view viewExtend returns(uint) {
        return calcEthIn(moreQuotaOf(who, twitters));
    }

    function calcEthIn(uint quota) public view viewExtend returns(uint) {
        uint r1 = calcRatio1();
        if(r1 == 0)
            return uint(-1);
    ...
    function calcEthIn1(uint quota) public view viewExtend returns(uint) {
        return price1(quota).mul(quota).div(1e18);
    }

    function calcOut1(uint v) public view viewExtend returns(uint a) {
    ...
    function calcRatio1() public view viewExtend returns(uint r) {
        uint p1 = price1At(0);
        uint p2 = price2();
        if(p2 == 0)
            return 1e18;
    ...
    function calcOut(address sender, Twitter[] memory twitters, uint value) public view viewExtend returns(uint a) {
...
contract FriendsEx is FriendsBase, Extended {
    ...
    function __Friends_init_unchained() public initializer {
        ...
        config[_factorProfitAir_] = 100;
        config[_factorProfitBuy_] = 100;
        ...
        config[_rebaseSpan_     ] = 20*365 days;
