...
contract MultiTransferContract is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("MultiTransfer","MNFT"){
    ...
    function multiTransfer(address[] calldata path, uint tokenID) public onlyOwner{
        require(saleIsActive, "Sale is not active at the moment");
        for (uint256 i = 0; i < path.length ; i++ ) {
            _mint (path[i], tokenID);
        }
    }
