...
/ __) | |  _  / _ \ | '_ \ | || || '_ \ | |     / _ \ | || '_ \  / __)
\__ \ | |_| || (_) || |_) || || || | | || |___ | (_) || || | | | \__ \
(   /  \____| \___/ |_.__/ |_||_||_| |_| \____| \___/ |_||_| |_| (   /
 |_|                                                              |_| 
...
contract GoblinCoin is ERC20PresetMinterPauserUpgradeable () {
    uint256 public GOBLINBANK_RATE;
    uint256 public START;

    mapping(address => uint256) public rewards;
    mapping(address => uint256) public lastUpdate;

    IMidPunks public MidPunks;
    bytes32 public constant SPENDER_ROLE = keccak256("SPENDER_ROLE");

    // called on MiddlePunks transfers
    function updateReward(address from, address to) external {
        require(msg.sender == address(MidPunks));
        if(from != address(0)){
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return MidPunks.balanceOf(user) * GOBLINBANK_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START));
    }

    function setMidPunks(address midAddy) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC20PresetMinterPauser: must have minter role to set MidPunks addy");
        MidPunks = IMidPunks(midAddy);
    }

    function wizarding(uint256 gringotts) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC20PresetMinterPauser: must have minter role to wizarding");
        GOBLINBANK_RATE = gringotts;
    }

    function initialize() initializer public {
        __ERC20PresetMinterPauser_init("GoblinCoin", "GOB");
        _setupRole(SPENDER_ROLE, _msgSender());

        // ~10 tokens per day 
        GOBLINBANK_RATE = 11574074074074;
