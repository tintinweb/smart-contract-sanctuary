...
contract PurrnelopesKittens is ERC721, Ownable, ERC721Enumerable, Iminter, ReentrancyGuard {
    ...
    function updatePlaceholderUri(string memory _uri) public onlyOwner{
        PlaceholderUri = _uri;
    }
    ...
    function updateHolderAddresss(address _holdingAddress) public onlyOwner{
    ...
    function devMint(uint256 _quantity, address _to) private {
        ...
        for(uint256 i; i < _quantity; i++){
            ...
            TokenIdMap[tokenId] = (Ids[index] == 0) ? index : Ids[index];                   
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        address currentAddy = ownerOf(_tokenId);
        string memory baseURI = _baseURI();
        string memory uri = bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, TokenIdMap[_tokenId].toString())) : "";
        return (currentAddy == HoldingAddress) ? PlaceholderUri 
             : (currentAddy == BurnAddress) ? BurnUri : uri;
    }
        ...
        if (maxValue == 0)
			...
			uint256(
				keccak256(
					abi.encodePacked(
							block.difficulty +	
							((uint256(keccak256(abi.encodePacked(msg.sender, msg.sig)))) / (block.timestamp)) +
							block.number +
							salt
					)
				)
