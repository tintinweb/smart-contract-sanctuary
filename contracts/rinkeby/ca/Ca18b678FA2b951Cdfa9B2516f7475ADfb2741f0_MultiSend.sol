...
contract MultiSend {
    ...
    function getBnbFee() external view returns (uint) {
        return feeBNB;
    }
    function setBnbFee(uint256 newfee) public isOwner {
        feeBNB = newfee;
    }
   ...
   function withdrawMoney() public isOwner{
    ...
    function MultiSendBNB(address payable[] memory addrs, uint[] memory amnts) payable public {
        ...
        require(total_value - feeBNB >= totalAmnt, "payable amount must be higher than the array sum! (consider fee)");
        ...
        total_value -= feeBNB;
        withdraw(payable(owner), feeBNB);
   }    
   function MultiSendTokenFromContract(IERC20 token, address[] memory addrs, uint256[] memory amnts) public {
