...
contract BattleDoge is ERC721, VRFConsumerBase, Ownable {
    using SafeMath for uint256;
   ...
   // }
     ...
     */
     ...
     modifier Gladiator() {
         require (isFighter[msg.sender], "must be a fighter");
         _;
     }
    constructor(address _VRFCoordinator, address _LinkToken, bytes32 _keyhash)
        public payable 
        VRFConsumerBase(_VRFCoordinator, _LinkToken)
        ERC721("BattleDoge", "BD")
    {   
        VRFCoordinator = _VRFCoordinator;
   ...
   function Attack (uint256 tokenId, uint256 _tokenId , uint256 _number)  public  returns (uint256 _winner)   {
     ...
     Game memory newGame = Game(index, tokenId, _tokenId, _number, ownerOf(_winner), block.timestamp);
       ...
       if ( sqrt(_number ** 2) + ( characters[tokenId].strength ** 10/3) + (characters[tokenId].intelligence ** 10/3) > (characters[_tokenId].strength ** 10/3) + (characters[_tokenId].intelligence ** 10/3) ** _number / i){
  ...
  function _levelDown(uint tokenId) private  {
   ...
   function feed (uint256 tokenId) public {
    ...
    function getAllTokensForUser(address user) public view returns(uint256[] memory){
        uint256 wolfPack = balanceOf(user);
        if(wolfPack == 0) {
            return new uint256[](0);
        }
        else{
            uint[] memory pack = new uint256[](wolfPack);
            uint256 totalPets = characters.length;
            uint256 b;
            uint256 packIndex = 0;

            for(b = 0; b < totalPets; b++ ){
                if(ownerOf(b) == user){
                    pack[packIndex] = b;
                    packIndex++;
                }
