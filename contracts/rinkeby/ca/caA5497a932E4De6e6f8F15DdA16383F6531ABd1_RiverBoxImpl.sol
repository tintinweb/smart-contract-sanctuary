    ...
    UUPSUpgradeable
{
    ...
    // ratio of dev commission from box payment in eth/bnb
    uint256 public royaltyRate;

    // address used to receive dev commission
    address public royaltyReceiver;

    // ratio precision e.g eth_amount = payment_eth.mul(ratio).div(precision)
    ...
    /* ================ UTIL FUNCTIONS ================ */
    function _pickLocation(uint256 index) internal returns (uint16) {
    ...
    function pawnsStock() public view returns (uint256) {
        return _stockLocations.length;
    }

    function batchStockInfo(uint256 startIndex, uint256 length) public view returns (uint256[] memory) {
        ...
        uint256[] memory ret = new uint256[](realLength);
        for (uint256 idx = 0; idx < realLength; ++idx) {
            ret[idx] = _stockLocations[startIndex.add(idx)];
        }
        return ret;
    }
    ...
    function verifyFusion(uint256[] memory tokenIds) public view override returns (bool) {
        ...
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            ...
            require(_exists(tokenId), "Token not exists");
            if (_tokenDetailsMapping[tokenId].fusionCount > 0 || _tokenDetailsMapping[tokenId].airDropResourceId > 0)
                return false;
    ...
    {
        uint256 royaltyAmount = _value.mul(royaltyRate).div(ratePrecision);
        emit RoyaltyReceived(royaltyReceiver, royaltyAmount, _tokenId);
        return (royaltyReceiver, royaltyAmount, _data);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == 0xc155531d || super.supportsInterface(interfaceId);
    }
     ...
     */
    function buy(uint256 quality, address referrer) public payable override whenNotPaused notContract nonReentrant {
        ...
        promotion = MathUpgradeable.min(promotion, pawnsStock().sub(quality));
        uint256 salt = uint256(keccak256(abi.encodePacked(quality, _msgSender(), totalSupply())));
        uint256 seed = IRiverBoxRandom(randomGeneratorAddress).generateSignature(salt);
        promotion = promotion.add(quality);
        for (uint256 i = 0; i < promotion; ++i) {
            seed = seed.add(totalSupply());
            uint256 randomIdx = uint256(keccak256(abi.encodePacked(seed))).mod(pawnsStock());
            uint16 pawnLocationId = _pickLocation(randomIdx);
            seed = seed.add(1).add(uint256(pawnLocationId));
            uint256 signature = uint256(keccak256(abi.encodePacked(seed)));
            Product memory newItem = Product(pawnLocationId, 0, 0, signature, new uint256[](0));
            uint256 newId = _awardItem(_msgSender());
            _tokenDetailsMapping[newId] = newItem;
            emit BoxAwarded(_msgSender(), newId);
        }
     ...
     */
    function fuse(uint256[] memory tokenIds) public override whenNotPaused notContract nonReentrant {
        require(verifyFusion(tokenIds), "not a valid list of tokens");
        uint16 parentLocationId = _tokenHierarchyMapping[_tokenDetailsMapping[tokenIds[0]].locationId].parentLocationId;
        Product memory newItem = Product(parentLocationId, 0, 0, 0, tokenIds);
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            _tokenDetailsMapping[tokenIds[i]].fusionCount = _tokenDetailsMapping[tokenIds[i]].fusionCount + 1;
        }
        uint256 newId = _awardItem(_msgSender());
        _tokenDetailsMapping[newId] = newItem;
        emit FusedItemAwarded(_msgSender(), newId);
    }
     ...
     */
    function setBaseURI(string memory newBaseURI) public override {
    ...
    function batchSetPawnLocationStock(uint16[] memory locationIds, uint16[] memory locationIdStocks) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
     ...
     */
    function setBoxPrice(uint256 newPrice) public override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        boxPrice = newPrice;
        emit BoxPriceChanged(boxPrice, newPrice);
    }

    function setRoyaltyRate(uint256 newRoyaltyRate) public override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        royaltyRate = newRoyaltyRate;
    }

    function setRoyaltyReceiver(address newAddress) public override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
     ...
     */
    function setRandomGenerator(address newAddress) public override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
    ...
    function withdraw(uint256 amount) public override {
        ...
        uint256 devAmount = amount.mul(uint256(devShare)).div(ratePrecision);
        uint256 marketAmount = amount.mul(uint256(marketShare)).div(ratePrecision);
    ...
    function batchAirDrop(address[] memory receivers, uint16[] memory airDropResourceIds) public override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        require(receivers.length == airDropResourceIds.length, "receivers length must equal airDropResourceIds length");
        for (uint16 idx = 0; idx < airDropResourceIds.length; ++idx) {
            Product memory newItem = Product(0, 0, airDropResourceIds[idx], 0, new uint256[](0));
            uint256 newId = _awardItem(receivers[idx]);
            _tokenDetailsMapping[newId] = newItem;
            emit BoxAwarded(receivers[idx], newId);
        }
    }

    /**
     * @dev For debugging only, !!!! Delete it
     */
    function debugMint(uint16 locationId, address receiver) public {
        uint256 signature = IRiverBoxRandom(randomGeneratorAddress).generateSignature(totalSupply());
        Product memory newItem = Product(locationId, 0, 0, signature, new uint256[](0));
        uint256 newId = _awardItem(receiver);
        _tokenDetailsMapping[newId] = newItem;
        emit BoxAwarded(receiver, newId);
    }
...
interface IRiverBox {
    ...
    struct Product {
        ...
        uint16 airDropResourceId;
        uint256 signature; // properties
    ...
    event BoxAwarded(address indexed payer, uint256 indexed tokenId);

    event FusedItemAwarded(address indexed payer, uint256 indexed tokenId);

    event RoyaltyReceived(address indexed poolAddress, uint256 amount, uint256 indexed tokenId);

    /* ================ UTIL FUNCTIONS ================ */

    /**
     * @dev Interface for IERC2981 for the NFT Royalty Standard
     */
    function royaltyInfo(
        uint256 _tokenId,
        uint256 _value,
    ...
    function setRoyaltyRate(uint256) external;
    ...
    function batchAirDrop(address[] memory receivers, uint16[] memory locationIds) external;
}
