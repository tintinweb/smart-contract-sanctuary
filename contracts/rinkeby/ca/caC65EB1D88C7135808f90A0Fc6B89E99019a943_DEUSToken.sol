...
contract DEIPool is AccessControl {
	...
	function mint1t1DEI(uint256 collateral_amount, uint256 DEI_out_min, uint256 collateral_price, uint256 expireBlock, bytes[] calldata sigs)
		external
		notMintPaused
	{
		...
		require(expireBlock >= block.number, "mintAlgorithmicDEI: signature is expired.");
        bytes32 sighash = keccak256(abi.encodePacked(collateral_address, collateral_price, expireBlock, getChainID()));
		require(IDEIStablecoin(dei_contract_address).verify_price(sighash, sigs), "mint1t1DEI: invalid signatures");
	...
	function mintAlgorithmicDEI(
		...
		require(IDEIStablecoin(dei_contract_address).verify_price(sighash, sigs), "mintAlgorithmicDEI: invalid signatures");
	...
	function mintFractionalDEI(
		...
		// uint256 deus_price = DEI.deus_price();
		require(expireBlock >= block.number, "mintFractionalDEI: signature is expired.");
		...
		require(IDEIStablecoin(dei_contract_address).verify_price(sighash, sigs), "mintFractionalDEI: invalid signatures");
		...
		require(
			global_collateral_ratio < COLLATERAL_RATIO_MAX &&
				global_collateral_ratio > 0,
			"CR needs to be between .000001 and .999999"
		);
		require(
			collateral_token.balanceOf(address(this)) - unclaimedPoolCollateral + collateral_amount <= pool_ceiling,
	...
	function redeem1t1DEI(uint256 DEI_amount, uint256 COLLATERAL_out_min, uint256 collateral_price, uint256 expireBlock, bytes[] calldata sigs)
		external
		notRedeemPaused
	{
		require(
		...
		require(expireBlock >= block.number, "mintAlgorithmicDEI: signature is expired.");
        bytes32 sighash = keccak256(abi.encodePacked(collateral_address, collateral_price, expireBlock, getChainID()));
		require(IDEIStablecoin(dei_contract_address).verify_price(sighash, sigs), "redeem1t1DEI: invalid signatures");
	...
	function redeemFractionalDEI(
		...
		// uint256 deus_price = DEI.deus_price();
		require(expireBlock >= block.number, "redeemFractionalDEI: signature is expired");
		bytes32 sighash = keccak256(abi.encodePacked(collateral_address, collateral_price, deus_contract_address, deus_current_price, expireBlock, getChainID()));
		require(IDEIStablecoin(dei_contract_address).verify_price(sighash, sigs), "redeemFractionalDEI: invalid signatures");

		uint256 global_collateral_ratio = IDEIStablecoin(dei_contract_address).global_collateral_ratio();
		require(
			global_collateral_ratio < COLLATERAL_RATIO_MAX &&
				global_collateral_ratio > 0,
			"redeemFractionalDEI: CR needs to be between .000001 and .999999"
		);
		...
		uint256 collateral_amount = (collateral_dollar_value * PRICE_PRECISION) / (col_price_usd);
	...
	function redeemAlgorithmicDEI(
		...
		require(expireBlock >= block.number, "redeemAlgorithmicDEI: signature is expired.");
		bytes32 sighash = keccak256(abi.encodePacked(deus_contract_address, deus_current_price, expireBlock, getChainID()));
		require(IDEIStablecoin(dei_contract_address).verify_price(sighash, sigs), "redeemAlgorithmicDEI: invalid signatures");

		uint256 global_collateral_ratio = IDEIStablecoin(dei_contract_address).global_collateral_ratio();

		require(global_collateral_ratio == 0, "redeemAlgorithmicDEI: CR must be 0");
		uint256 deus_dollar_value_d18 = DEI_amount;

		deus_dollar_value_d18 = (deus_dollar_value_d18 * (uint256(1e6) - redemption_fee)) / (PRICE_PRECISION); //apply fees
		...
		require(DEUS_out_min <= deus_amount, "redeemAlgorithmicDEI: Slippage limit reached");
		// Move all external functions to the end
		IDEIStablecoin(dei_contract_address).pool_burn_from(msg.sender, DEI_amount);
		IDEUSToken(deus_contract_address).pool_mint(address(this), deus_amount);
	}
	...
	function collectRedemption() external {
		require(
			...
			"collectRedemption: Must wait for redemption_delay blocks before collecting redemption"
		);
		bool sendDEUS = false;
	...
	function recollateralizeDEI(RecollateralizeDEI memory inputs) external {
		require(recollateralizePaused == false, "recollateralizeDEI: Recollateralize is paused");

		require(inputs.expireBlock >= block.number, "recollateralizeDEI: signature is expired.");
		...
		require(IDEIStablecoin(dei_contract_address).verify_price(sighash, inputs.sigs), "recollateralizeDEI: invalid signatures");
		...
		.calcRecollateralizeDEIInner(
			collateral_amount_d18,
			getCollateralPrice(inputs.collateral_price),
			global_collat_value,
			dei_total_supply,
			global_collateral_ratio
		);
		...
		require(inputs.DEUS_out_min <= deus_paid_back, "recollateralizeDEI: Slippage limit reached");
		TransferHelper.safeTransferFrom(
			address(collateral_token),
			msg.sender,
			address(this),
	...
	function buyBackDEUS(
		...
		require(buyBackPaused == false, "buyBackDEUS: Buyback is paused");
		require(expireBlock >= block.number, "buyBackDEUS: signature is expired.");
		...
		require(IDEIStablecoin(dei_contract_address).verify_price(sighash, sigs), "buyBackDEUS: invalid signatures");
		...
		require(
			...
			"buyBackDEUS: Slippage limit reached"
		);
...
interface IDEIStablecoin {
	...
	function globalCollateralValue(uint256 eth_usd_price, uint256 eth_collat_price) external view returns (uint256);
...
contract DEIPoolLibrary {
    ...
    function calcMintFractionalDEI(MintFF_Params memory params) public pure returns (uint256, uint256) {
        ...
        {    
            ...
            deus_dollar_value_d18 = (params.deus_amount * params.deus_price_usd) / (1e6);
