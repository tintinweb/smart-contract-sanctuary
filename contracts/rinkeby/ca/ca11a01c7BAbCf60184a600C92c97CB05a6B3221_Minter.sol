...
contract Minter is AccessControlEnumerableUpgradeable, Provenance, PaymentSplitterUpgradeable {
        ...
        internal
    {
        ...
        bool isFreeMint = IRegistry(registry).getProjectIsFree(tokenContract);
        
        require(numberOfTokens > 0, "Minter: numberOfTokens is 0");
        require(price != 0 && isFreeMint == false, "Minter: price not set in registry and is not free mint");
...
interface IRegistry {
    ...
    function getProjectMaxWalletPurchase(address project)
        ...
        view
        returns(uint256);
    function getProjectIsFree(address project)
        external
        view
        returns(bool);
}
