 ...
 *         Users are able to create listings for ERC1155 tokens
 ...
 */
contract ERC1155ListingMarketplace is ERC1155Holder, OnlyNonContractsOrApproved {
  ...
  /**
   * @notice Creates a listing for an ERC1155 token
   *
   ...
   * @param _tokenQty the quantity of the token to be listed
   */
  function createListing(address _tokenAddress, uint256 _tokenId, uint256 _tokenQty) external onlyNonContractsOrApproved returns (bytes32) {
    IERC1155(_tokenAddress).safeTransferFrom(tx.origin, address(this), _tokenId, _tokenQty, "");

    bytes32 id = keccak256(abi.encodePacked(_tokenAddress, _tokenId, _tokenQty, tx.origin, block.timestamp));
  ...
  function removeListing(bytes32 _id) external onlyListingOwner(_id) onlyNonContractsOrApproved {
    ...
    IERC1155(listing.tokenAddress).safeTransferFrom(address(this), msg.sender, listing.tokenId, listing.tokenQty, "");
  ...
  function acceptListingOffer(bytes32 _id) external onlyListingOwner(_id) onlyIfListingHasOffer(_id) onlyNonContractsOrApproved {
    ...
    if(!listing.offeror.isContract()) {
      IERC1155(listing.tokenAddress).safeTransferFrom(address(this), listing.offeror, listing.tokenId, listing.tokenQty, "");
    }
