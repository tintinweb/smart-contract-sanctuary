...
contract TestNfts is ERC721Enumerable, Ownable {
  ...
  string[] public wordValues = ["watcher","ragnarok","1000","dump_meta","xz3ns","okbye"];
  ...
  function mint() public payable {
    ...
    Word memory newWord = Word(
      string(abi.encodePacked('TNFT #', uint256(supply+1).toString())),
      "This is your entry pass to Idyll!",
  ...
  function buildImage(uint256 _tokenId) public view returns(string memory) {
    ...
    return Base64.encode(bytes(abi.encodePacked(
      ...
      '<text dominant-baseline="middle" fill="hsl(',currentWord.textHue,',100%,80%)" text-anchor="middle" font-size="41" y="50%" x="50%">',currentWord.value,'</text>',
      '</svg>'
    )));
  }
