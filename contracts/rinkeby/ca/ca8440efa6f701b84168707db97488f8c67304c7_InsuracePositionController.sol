...
contract InsuracePositionController is AbstractController {
    ...
    */
    ...
    function netWorth() external view override returns (uint256) {
        ...
        // rewards are 0, and dai/insur ratio is 20000 insur to 1 DAI_MOCK
        // TODO: init pool with 1:1 ratio
        _rewards = _rewards + 1 * 20000;

        // returns currently staked amount in base asset
        uint256 totalStakedinBaseToken = _priceFeed.howManyTokensAinB(
            address(base),
            _stakingAsset,
            _currentlyAtStake(_stakingAsset)
        );

        // returns INSUR rewards in base asset
        uint256 rewardsinBaseToken = _priceFeed.howManyTokensAinB(
            address(base),
            _insurToken,
            _rewards
        );
