...
import "./modules/Reservable.sol";

contract Collection is Reserveable {
    bool public isValid;
    ...
    function setup(
        ...
        require(!isValid, "Collection: Setup can only be done once");
        isValid = true;
        setupBaseCollection(
...
contract Reserveable is Revealable {
    ...
    function reserveTokens(uint256 _reserveTokens) external onlyAdmin {
        require(
            tokensCount == 0,
            "Reserveable: cannot reserve after selling tokens"
        );
        require(
            _reserveTokens + presaleReservedTokens < maximumTokens,
            "Reserveable: tokens overlap"
        );
        reservedTokens = _reserveTokens;
        startingTokenIndex = _reserveTokens;
    }

    function transferReservedToken(address _receiver) external onlyAdmin {
        uint256 currentTokenId = reserveTokenCounter;
        require(
            currentTokenId < reservedTokens,
            "Reserveable: no more reserved tokens left"
        );
        reserveTokenCounter++;
...
contract Revealable is Presaleable {
    ...
    function setRevealableDetails(
        ...
        require(
            _revealAfterTimestamp >= block.timestamp,
            "Revealable: reveal time is in past"
        );
        if (
    ...
    function setRevealAfterTimestamp(uint256 _revealAfterTimestamp)
        ...
        onlyAdmin
    {
        require(isRevealable, "Revealable: collection is not revealable");
        require(
            _revealAfterTimestamp >= block.timestamp,
            "Revealable: reveal time is in past"
        );
    ...
    function setProjectURI(string memory _projectURI) external onlyAdmin {
        ...
        returns (string memory)
    {
        ...
        return
            bytes(projectURI).length > 0 ? super.tokenURI(tokenId) : loadingURI;
    }
...
contract Presaleable is PaymentSplitable {
    ...
    // control logic
    bool public presaleActive; // check if token presale is active or not
    ...
    modifier presaleAllowed() {
        require(isPresaleAllowed(), "Presaleable: allowed only if presalable");
        _;
    }
    ...
    function setupPresale(
        ...
        if (_presaleStartTime != 0) {
            require(
                _presaleReservedTokens != 0,
                "Presaleable: presale cannot have zero tokens"
            );
            require(
                _presaleStartTime > block.timestamp,
                "Presaleable: presale cannot start in past"
            );
            require(
                _presaleMaxHolding != 0,
                "Presaleable: invalid max holding value"
            );
        ...
        presaleAllowed
    {
        ...
        require(
            ...
            "Project: Invalid address"
        );
        require(
            presaleWhitelist[_whitelistAddress] == address(0),
            "Project: address already whitelisted"
        );
    ...
    function removeWhitelist(
    ...
    ) external onlyAdmin presaleAllowed {
        require(
        ...
        require(
            ...
            "Project: invalid previous address"
        );
        whitelistCount--;
        ...
        onlyAdmin
    {
        require(
            ...
            "Presaleable: invalid presale sale start time"
        );
    ...
    function presaleBuy()
        ...
        presaleAllowed
    {
        require(isPresaleActive(), "Presaleable: presale is inactive");
        require(
            msg.value == presalePrice,
            "Presaleable: transfer exact amount"
        );
        require(
            isPresaleWhitelisted() ? isWhitelisted(msg.sender) : true,
            "Presaleable: only whitelisted addresses allowed"
        );
        require(
            balanceOf(msg.sender) + 1 <= presaleMaxHolding,
            "Presaleable: out of buying limit"
        );
        ...
        presaleAllowed
    {
        ...
        require(
            tokensCount + _quantity <= presaleReservedTokens,
            "Presaleable: presale out of stock"
        );
        require(
        ...
        require(
            _quantity <= maxPurchase,
            "Presaleable: cannot buy more than buying limit"
        );
        require(
    ...
    function isPresaleWhitelisted() public view returns (bool) {
        return
            isPresaleAllowed() &&
            (presaleWhitelist[SENTINEL_ADDRESS] != address(0));
    }
    ...
    function _presaleWhitelistBatch(address[] memory _whitelists) private {
        ...
        for (uint256 i; i < _whitelists.length; i++) {
            ...
            require(
                ...
                "Project: Invalid whitelist address"
            );
            require(
                presaleWhitelist[whitelistAddress] == address(0),
                "Project: address already whitelisted"
            );
            presaleWhitelist[currentWhitelistAddress] = whitelistAddress;
            currentWhitelistAddress = whitelistAddress;
        }
        presaleWhitelist[currentWhitelistAddress] = SENTINEL_ADDRESS;
    }
}
...
contract PaymentSplitable is BaseCollection {
    ...
    function setupPaymentSplitter(
        ...
        require(
            _nftify != address(0),
            "PaymentSplitable: nftify address cannot be zero"
        );
        ...
        onlyAdmin
    {
        require(
            totalReleased == 0,
            "PaymentSplitable: set before releasing any payment"
        );
        _setPayees(_payees, _shares);
    }
        ...
        private
    {
        require(
            ...
            "PaymentSplitable: arrays length should match"
        );
        ...
        require(
            totalSharesAdded + NFTIFY_SHARES == TOTAL_SHARES,
            "PaymentSplitable: total shares do not match"
        );
    }

    function release(uint256 _amount) external {
        address _payee = msg.sender;
        require(payees[_payee].shares > 0, "PaymentSplitable: zero share");
        uint256 payment = pendingPayment(_payee);
        require(
            payment != 0 && payment >= _amount,
            "PaymentSplitable: requested amount greater than due"
        );
...
import "../common/access/Authorized.sol";
import "openzeppelin-solidity/contracts/security/Pausable.sol";
import "openzeppelin-solidity/contracts/security/ReentrancyGuard.sol";

contract BaseCollection is Authorized, Pausable, ERC721Enumerable {
    ...
    function setupBaseCollection(
        ...
        require(
            _admin != address(0),
            "BaseCollection: admin cannot be zero address"
        );
        require(
            _maximumTokens != 0,
            "BaseCollection: maximum tokens cannot be zero"
        );
        require(
            _maximumTokens >= _maxHolding && _maxHolding >= _maxPurchase,
            "BaseCollection: holding or max purchase cannot be mroe than total supply"
        );
        _name = name_;
        _symbol = symbol_;
        admin = _admin;
    ...
    function setMetadata(string memory _metadata) external {
        require(
            !isSetupComplete() || msg.sender == admin,
            "BaseCollection: only admin or before setup"
        );
        require(
            bytes(_metadata).length != 0,
            "BaseCollection: cannot store empty metadata"
        );
        metadata = _metadata;
    }
        ...
        onlyAdmin
    {
        require(
            ...
            "BaseCollection: invalid public sale start time"
        );
    ...
    function buy() external payable virtual whenNotPaused {
        require(isSaleActive(), "BaseCollection: sale is inactive");
        require(msg.value == price, "BaseCollection: transfer exact amount");
        require(
            balanceOf(msg.sender) + 1 <= maxHolding,
            "BaseCollection: out of buying limit"
        );
        _manufacture(msg.sender);
    }
    ...
    function buy(uint256 _quantity) external payable virtual whenNotPaused {
        ...
        require(
            ...
            "BaseCollection: transfer exact amount"
        );
        require(
            _quantity <= maxPurchase,
            "BaseCollection: cannot buy more than buying limit"
        );
        require(
            balanceOf(msg.sender) + _quantity <= maxHolding,
            "BaseCollection: out of buying limit"
        );
        _manufacture(msg.sender, _quantity);
    }
    ...
    function _manufacture(address _buyer, uint256 _quantity) internal {
        uint256 currentTokenId = tokensCount + startingTokenIndex;
        require(
            currentTokenId + _quantity <= maximumTokens,
            "BaseCollection: out of stock"
        );
...
contract Authorized {
    ...
    /**
     * @dev             transfer administration rights
