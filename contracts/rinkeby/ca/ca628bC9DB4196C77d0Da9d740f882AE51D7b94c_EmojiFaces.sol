...
contract EmojiFaces is ERC721, ERC721Enumerable, Ownable {
	...
	/* We have to decide here wether we want to reveal the name or wait */

    constructor() ERC721("MyTokenNew", "MTKNN") {}
    ...
    function mintToken(uint256 numberOfTokens) external payable {
        require(
            saleIsActive, 
            "Sale must be active to mint token!"
        );
        require(
            numberOfTokens > 0 && numberOfTokens <= maxTokenPurchase, 
            "You can only mint one or more tokens at a time."
        );
        require(
            totalSupply().add(numberOfTokens) <= MAX_TOKENS,
            "Purchase would exceed maximum supply of tokens"
        );
        require(
    ...
    function presaleToken(uint256 numberOfTokens) external payable {
        require(
            presaleIsActive, 
            "Presale must be active to mint token!"
        );
        require(
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
        ...
        if (isRevealed == false) {
            return "ipfs://QmRAcBibPn4HAPe6gGZTUHxqcHy3KtbdqhpmigDsra6L4S/africanMask.jpg";
        }
