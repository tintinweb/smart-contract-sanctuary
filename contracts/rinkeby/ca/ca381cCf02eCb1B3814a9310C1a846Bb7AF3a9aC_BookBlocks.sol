...
contract BookBlocks is ERC721Enumerable, Ownable {
    ...
    function mint(string memory _editionName, uint256 _count) external payable isValidMint {
        ...
        require(msg.value == _price * _count, "Invalid value.");
        require(
        ...
        editions[_editionName].balance += _count * _price;
    }

    // Used to mint to an admin wallet for auction sales.
    // Alternatively this can be used by other BookBlocks contracts.
    function reserve(string memory _editionName, uint256 _count) external onlyAdmin {
        require(
            editions[_editionName].supply - editions[_editionName].totalMinted >= _count,
            "Exceeds max supply."
            );
    ...
    // Creates a drop for a specific BookBlocks edition and reserves the tokenIDs for it.
    ...
    // Withdraws the proceeds from one edition.
    ...
    // Withdraws the proceeds from multiple editions.
    // Note: 
    //  This looks like it should emit the event even if it fails, but events 
    //  are reverted upon tx failure. The balance of an edition is subtracted before
    //  withdrawing to prevent reentrancy attacks.
    function withdrawMultipleEditions(string[] memory _names) external onlyAdmin {
        uint256 proceeds;
