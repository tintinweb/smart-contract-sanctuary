...
contract SecurityToken is ERC1404, Ownable {
    ...
    uint8 constant ERR_RECIPIENT_CODE = 1;
    uint8 constant ERR_BONDING_CURVE_CODE = 2;
    uint8 constant ERR_NOT_WHITELISTED_CODE = 3;
    string constant SUCCESS_MESSAGE = "SecurityToken: SUCCESS";
    string constant ERR_RECIPIENT_MESSAGE = "SecurityToken: RECIPIENT SHOULD BE IN THE WHITELIST";
    string constant ERR_BONDING_CURVE_MESSAGE = "SecurityToken: CAN TRANSFER ONLY TO BONDING CURVE";
    string constant ERR_NOT_WHITELISTED_MESSAGE = "SecurityToken: ONLY WHITELISTED USERS CAN TRANSFER TOKEN";
    ...
    function addAWOperator(address operator) public onlyOwner{
    ...
    function addSCOperator(address operator) public onlyOwner{
    ...
    /// @param operator Address of the AWO
    function removeAWOperator(address operator) public onlyOwner{
    ...
    /// @param operator Address of the SCO
    function removeSCOperator(address operator) public onlyOwner{
    ...
    function mint(address account, uint256 amount) public onlySCOperator{
    ...
    function burn(address account, uint256 amount) public onlySCOperator{
    ...
    function removeFromWhitelist(address account) public onlyAWOperator{
        require(whitelist[account] == true,
            "SecurityToken.removeFromWhitelist: User is not on the Whitelist");
        require(account != bondingCurve,
            "SecurityToken.removeFromWhitelist: Impossible to remove bondingCurve from Whitelist");
    ...
    function approve(address spender, uint256 amount)
        ...
        require(whitelist[spender] == true,
            "SecurityToken.approve: Can approve only a whitelisted user to spend tokens");
        require(whitelist[msg.sender] == true,
            "SecurityToken.approve:Only a whitelisted user can approve the spend of tokens");
        _approve(_msgSender(), spender, amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        returns (bool) {
        require(whitelist[msg.sender] == true,
            "SecurityToken.transferFrom: You have to be in whitelist to send tokens");
        _transfer(sender, recipient, amount);
    ...
    function setupBondingCurve(address curve) public onlyOwner {
        bondingCurve = curve;
    ...
    function getOperator(address operator) public view returns (bool, bool) {
...
contract CoordinationToken is BondingCurveToken {

    uint256 constant GAS_LIMIT = 50000000000;
    uint32 constant MAX_RESERVE_RATIO = 1000000;
    uint256 constant HUNDRED_PERCENT = 1e18;
    uint256 phi; // 1e18 = 100%, 1e16 = 1%, 0 = 0%
    ...
    constructor(SecurityToken token, uint256 fee) BondingCurveToken(333333, GAS_LIMIT) ERC20("Coordination","CRD") {
        securityToken = token;
    ...
    function bondToMint(uint256 amountToBond) public {
        if(totalSupply() == 0){
            uint256 result;
            uint8 precision;
            (result, precision)  = power(amountToBond * MAX_RESERVE_RATIO, reserveRatio, reserveRatio, MAX_RESERVE_RATIO );
            _mint(msg.sender, result >> precision);
            emit BondedToMint(msg.sender, amountToBond, result >> precision);
        } else {
            _curvedMint(amountToBond);
            emit BondedToMint(msg.sender, amountToBond, calculateCurvedMintReturn(amountToBond));
        }
        securityToken.transferFrom(msg.sender, address(this), amountToBond);
    }
    ...
    function burnToWithdraw(uint256 amountToWithdraw) public {
        require(balanceOf(msg.sender) >= amountToWithdraw,
            "CoordinationToken.burnToWithdraw: Not enough funds to withdraw");
        uint256 burned = _curvedBurn(amountToWithdraw);
        uint256 currentFee = (burned * phi) / HUNDRED_PERCENT;
        securityToken.transfer(msg.sender, burned - currentFee);
    ...
    /// @notice Function that returns SecurityToken address
    /// @return address of SecurityToken
    function getSecurityToken() public view returns(address){
        return address(securityToken);
    }
}


// File contracts/DelphiaEscrow.sol

pragma solidity 0.8.4;



contract DelphiaEscrow is Ownable {
    ...
    modifier onlyRDOperator() {
        require(owner() == msg.sender || operators[msg.sender] == true,
            "DelphiaEscrow: Only RD operators are allowed to distribute rewards");
        _;
    }

    /// @notice Constructor of DelphiaEscrow
    /// @param token Address of CoordinationToken token used as a payment
    constructor (CoordinationToken token) public {
    ...
    function addRDOperator(address operator) public onlyOwner{
    ...
    /// @param operator Address of the RDO
    function removeRDOperator(address operator) public onlyOwner{
        require(operators[operator] == true,
        "DelphiaEscrow.removeRDOperator: There is no such operator");
    ...
    function deposit(uint256 amount) public {
        require(coordinationToken.transferFrom(msg.sender, address (this), amount),
            "DelphiaEscrow.deposit: Cannot transfer token to the Escrow");
    ...
    */
    function withdraw() public {
        require(withdrawBalances[msg.sender] > 0,
            "DelphiaEscrow.withdraw: There is nothing to withdraw");
        coordinationToken.transfer(msg.sender, withdrawBalances[msg.sender]);
        emit Withdrawn(msg.sender, withdrawBalances[msg.sender]);
     ...
     */
    function distribute(Receipt[] memory receipts) public onlyRDOperator{
        require(receipts.length <= 200,
            "DelphiaEscrow.distribute: Cannot distribute more then 200 receipts at a time");
        for(uint64 j = 0; j < receipts.length; j++){
            require(activeBalance >= receipts[j].amount,
                "DelphiaEscrow.distribute: Insufficient funds to distribute");
    ...
    /// @notice Function to check if user is an operator
    /// @param operator Address to check
    /// @return If address has operator rights
    function getOperator(address operator) public view returns(bool){
        return operators[operator];
    }

    /// @notice Function to check the withdraw balance of the user
    /// @param user Address to get withdraw balance
    /// @return Withdraw balance of the user
    function getWithdrawBalance(address user) public view returns(uint256){
        return withdrawBalances[user];
    }

    /// @notice Function to get Active balance of the contract(not locked by withdrawals)
    /// @return ActiveBalance of the contract
