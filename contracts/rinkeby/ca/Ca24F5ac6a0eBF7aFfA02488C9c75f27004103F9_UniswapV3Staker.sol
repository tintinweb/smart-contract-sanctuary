...
contract UniswapV3Staker is IUniswapV3Staker, Multicall {
    ...
    function getMindsyncRewardInfo(IncentiveKey memory key, uint256 tokenId)
        external
        view
        returns (uint256 rewardMAI, uint256 rewardETH)
    {
        bytes32 incentiveId = IncentiveId.compute(key);
        (, uint128 liquidity) = stakes(tokenId, incentiveId);
        require(liquidity > 0, 'UniswapV3Staker::getMindsyncRewardInfo: stake does not exist');

        Deposit memory deposit = deposits[tokenId];
        (rewardMAI, rewardETH) = RewardMath.computeMindsyncRewardAmount(
            key.pool,
            deposit.tickLower,
            deposit.tickUpper,
            liquidity
    ...
    function getMindsyncRewardInfo2(uint256 tokenId)
        external
        view
        returns (uint256 rewardMAI, uint256 rewardETH)
    {
...
library RewardMath {
    ...
    function computeRewardAmount(
        ...
        // this operation is safe, as the difference cannot be greater than 01/stake.liquidity
    ...
    function computeMindsyncRewardAmount(
        IUniswapV3Pool pool,
        int24 tickLower,
        int24 tickUpper,
        uint128 liquidity
    ) internal view returns (uint256 rewardMAI, uint256 rewardETH) {
