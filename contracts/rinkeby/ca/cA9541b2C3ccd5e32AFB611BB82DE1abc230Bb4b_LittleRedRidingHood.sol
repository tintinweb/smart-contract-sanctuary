...
/* TODO
   ...
   - do we still need to include IER165?
 ...
 */
contract LittleRedRidingHood is ERC721Enumerable, AccessControl, ReentrancyGuard {

    uint public constant MAX_PUBLIC_MINT = 15;
    uint public constant PRICE_PER_TOKEN = 0.05 ether;
    uint constant REDEMPTION_RATE = 3;
    uint constant REDEMPTION_MAX = 300;
    uint constant MAX_TOKENS = 500;
    ...
    mapping(uint => int) private offsets;
    mapping(uint => Hemisphere) private hemispheres;

    mapping(address => range[]) private _approvedTokenRange;

    constructor(address dtAddress, string memory _baseURI) ERC721("Little Red Riding Hood", "LRRH") {
        
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(SUPPORT_ROLE, msg.sender);

        BASE_URI = _baseURI;
        dt = DateTimeAPI(dtAddress);
    ...
    function withdraw(address payable recipient, uint256 amount) external onlyRole(SUPPORT_ROLE) {
    ...
    function redeem(address[] calldata contracts, uint256[] calldata tokenIds) external nonReentrant {
        uint ts = totalSupply();
