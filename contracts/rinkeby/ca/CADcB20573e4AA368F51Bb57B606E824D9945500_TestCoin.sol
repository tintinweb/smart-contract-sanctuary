...
contract TestCoin is ERC721Enumerable, Ownable {

    uint256 public constant MAX_NFT_SUPPLY = 100;
    uint public constant MAX_PURCHASABLE = 5;
    uint256 public CYPHER_PRICE = 44000000000000000;
    bool public saleStarted = true;

    using Strings for uint256;
    mapping(uint256 => string) private _tokenURIs;

    constructor() ERC721("TestCoin", "TSTCONONCONON") {}

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "URI query for nonexistent token");
        string memory _tokenURI = _tokenURIs[tokenId];

        require(bytes(_tokenURIs[tokenId]).length > 0, "Token URI must be set");
        string memory base = _baseURI();
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) external onlyOwner {
        ...
        require(bytes(_tokenURIs[tokenId]).length == 0, "URI can't be set twice");
        _tokenURIs[tokenId] = _tokenURI;
    }
