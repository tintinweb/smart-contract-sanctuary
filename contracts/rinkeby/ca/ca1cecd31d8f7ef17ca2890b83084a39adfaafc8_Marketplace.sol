...
contract PGN is SafeMath {
	...
	function mining() 
	public {
		if((block.timestamp >= basicTime-5) && (block.timestamp <= basicTime+5)) {
		    uint256 giveAmount = (basicAmount*(balanceOf[address(this)]/totalSupply))+1;
		    transferFrom(address(this), msg.sender, giveAmount);
		}
		else if(block.timestamp > basicTime+5) {
