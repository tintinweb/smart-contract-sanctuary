...
//1.Token Name (Media Contract) : ZORA to ARTO @
//2.Auction Contract :- Curator (Admin) can end an auction before the timer ends
//3.Variable name is changed Zora to media @
//4.Minimum bid increment of 5% has been removed so that a user can  place any greater bid . @
//*Placing the same bid is not allowed
//5.Bid shares are defined as function parameter which is :-
//> Creator Share => 10%
//> Owner Share => 90%
//> Previous owners share => 0%
...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        if(auctions[auctionId].tokenOwner == msg.sender || auctions[auctionId].curator == msg.sender){
            require(
