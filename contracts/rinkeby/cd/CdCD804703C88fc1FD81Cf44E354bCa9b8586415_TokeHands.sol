...
contract TokeHands is ERC721, Ownable {
    ...
    modifier mintCompliance(address _sender) {
        uint256 ownerTokenCount = balanceOf(_sender);
        require(ownerTokenCount < 1, "Can only hold one tokehand per wallet");
    ...
    function setRevealedUri(string memory _redeemedUri) public onlyOwner {
        ...
        returns (string memory)
    {
        ...
        if (redeemed[_tokenId] == true) {
            string memory currentRedeemedUri = _redeemedURI();
            return
    ...
    function withdraw() public onlyOwner {
        ...
        require(devPayout, "dev payout failed");

        // Remaining to owner
        (bool ownerPayout, ) = payable(owner()).call{
            value: address(this).balance
        }("");
        require(ownerPayout, "owner payout failed");
    }
    ...
    function _redeemedURI() internal view virtual returns (string memory) {
        return redeemedUri;
    }
}
