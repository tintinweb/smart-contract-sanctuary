...
contract BeckerCore is Ownable, ERC721Full {
	...
	*/
	...
	function mint(string memory _name, bytes memory _data, uint256 _documentType, string memory _json) public onlyOwner payable returns (uint) {
		...
		uint256 identificador = beckers.push(ArielBeckerArt(_name, _data, msg.sender, block.timestamp, _documentType, _json));

		_mint(msg.sender, identificador);
		_setTokenURI(identificador, baseTokenURI());
		transferFrom(address(0), msg.sender, identificador);
		emit Transfer(address(0), msg.sender, identificador);
