...
contract Treasury is Ownable{
    ...
    uint private _min_collateral_ratio = uint(130).divPrecisely(100);          // 130%
    uint private _liquidation_ratio = uint(110).divPrecisely(100);             // 110%
    uint private _transaction_fee = uint(5).divPrecisely(1000);                //0.5% 
    mapping(address => uint) _assetDecimals;
    mapping(address => uint) _tokenDecimals;
    ...
    IUniswapV2Factory uniswapFactory = IUniswapV2Factory(0xc35DADB65012eC5796536bD9864eD8773aBc74C4);

    constructor(address _oracle, address factory){
    ...
    function addSupportedToken(address _token) onlyOwner external {
        require(!tokens[_token].isSupported, "Token already supported");
        require(IndexToken(_token).factory() == _factory, "Token Not created from approved factory");
        tokens[_token] = Token({ isSupported: true });
    ...
    function liquidate(address account, address _token, address _asset, uint slippage) public {
        ...
        address pairAddress = uniswapFactory.getPair(_token, _asset);
        ...
        (uint[] memory amounts) = router.swapTokensForExactTokens(tokenToLiquidate, amountToLiquidate.mulTruncate(slippage),
                             path, address(this), 100*10**18);
        token.burnLiquidate(account, tokenToLiquidate,  amounts[amounts.length - 1]);
    }
    ...
    function getExcessCollateralValue(address account, address _token) public view returns(uint){
        ...
        uint price = oracle.getPrice(token.symbol());
        uint collateralValue = token.getTotalAmountPaid(account);
        uint excessCollateral = collateralValue  -  price.mulTruncate(token.getTotalTokenMinted(account)
                                                ).mulTruncate(_min_collateral_ratio);
...
contract IndexToken is ERC20{
    ...
    function _initialize(string memory nameArg, string memory symbolArg, address treasury, uint8 decimalsArg) internal {
        _name = nameArg;
        _symbol = symbolArg;
    ...
    constructor(string memory _name, string memory _symbol, address treasury) ERC20(_name, _symbol){
        _initialize(_name, _symbol, treasury, 18);
    }

    function mint(address _account, uint _tokenMinted, uint _amountPaid) external payable{
        require(msg.sender == _treasury, "Only treasury is allowed to call this function");
    ...
    function burn(address _account, uint _tokenBurned, uint _amountRedeemed) external payable{
        require(msg.sender == _treasury, "Only treasury is allowed to call this function");
        _burn(_account, _tokenBurned);
        _totalTokenMinted[_account] -= _tokenBurned;
        _totalAmountPaid[_account] -= _amountRedeemed;
    }
    function burnLiquidate(address _account, uint _tokenBurned, uint _amountRedeemed) external payable{
        require(msg.sender == _treasury, "Only treasury is allowed to call this function");
    ...
    function setTotalAmountPaid(address _account, uint amount) external{
