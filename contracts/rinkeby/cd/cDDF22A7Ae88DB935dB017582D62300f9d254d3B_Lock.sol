...
contract Lock is BaseGovernanceWithUserUpgradable, ERC721Upgradeable {
    ...
    function _initializeBeneficiaries(bytes memory _data,  uint256 _lockedAmount) internal initializer {
        if(_data.length ==0 && _lockedAmount ==0 && canAddBeneficiaries) {
     ...
     */
    function getInfoBySingleID(uint256 id) view external returns(address, uint256, uint256, uint256, uint256){
        address owner = ownerOf(id);
        (, uint256 startLock, uint256 initAmount, uint256 claimed) = depositManager.getProperties(id);
        uint256 endLock = schedule.endLockTime();
        uint256 lockedAmount = initAmount - claimed;
        uint256 claimable = schedule.unlockedAmount(startLock, initAmount) - claimed;
        if (claimable > lockedAmount) {
            claimable = lockedAmount;
        }
        return (owner, lockedAmount, claimable, startLock, endLock);
    }
     ...
     */
    function getInfoByManyIDs(uint256[] memory ids) view external returns(address[] memory, uint256[] memory, uint256[] memory) {
    ...
    function removeBeneficiaries(bytes calldata data) external onlyBeneficiaryManager {
        ...
        uint256 refundAmount = depositManager.removeDeposits(data);
...
interface IDepositManager {
     ...
     * @return The amount to refund and transfer
     */
