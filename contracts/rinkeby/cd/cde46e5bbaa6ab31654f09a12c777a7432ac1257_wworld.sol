...
/**
* Submitted for verification at blockscout.com on 2021-03-31 12:01:58.882985Z
*/
...
contract wworld is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_AVATARS = 10000;
    bool public hasSaleStarted = false;
    address public feeAddress1 = 0x7B9c08898B60345671BaC9091F438382A1Fc4799;
    address public feeAddress2 = 0x341B7Cb6aD822CD6b2B18D3a42b5DA1A721468fE;
    address public feeAddress3 = 0xf369FF2f36BA46cC37cf125eC62D3638Ad378A58;
    address payable feeReceiver2;
    address payable feeReceiver3;

    string public METADATA_PROVENANCE_HASH = "";

    constructor() ERC721("W-World","WWORLD")  {
        setBaseURI("https://w-world.com/api/avatar/");
        feeReceiver = payable(feeAddress1);
        feeReceiver2 = payable(feeAddress2);
        feeReceiver3 = payable(feeAddress3);
   ...
   function buyAvatars(uint256 count) public payable {
        require(totalSupply() < MAX_AVATARS, "Sale has already ended");
        require(count > 0 && count <= 50, "You can generate minimum 1, maximum 50 Avatars");
        require(totalSupply().add(count) <= MAX_AVATARS, "Exceeds MAX_AVATARS");
    ...
    function buy(uint256 _tokenId) public payable {
        ...
        if (Bazaar[_tokenId].price >= 0) {
            ...
            uint256 fee1 = fee / 2;
            uint256 fee2 = fee / 4;
            uint256 fee3 = fee - fee1 - fee2;
            uint256 withFee = SafeMath.sub(msg.value, fee);

            seller.transfer(withFee);
            feeReceiver.transfer(fee1);
            feeReceiver2.transfer(fee2);
            feeReceiver3.transfer(fee3);
        }
    ...
    function withdrawal() public payable onlyOwner {
        ...
        uint256 balance2 = freeBalance / 4;
        uint256 balance3 = freeBalance - balance1 - balance2;

        require(payable(feeReceiver).send(balance1));
        require(payable(feeReceiver2).send(balance2));
        require(payable(feeReceiver3).send(balance3));
    }
}