	...
	Ownable 
{
    ...
    uint256 public presaleTime = 1633141341; //1633633200--10/7
    uint256 public presaleEndTime = 1633156161; //1633719600--10/8
    // time where public are allowed to purchase
    uint256 public pledgeTime = 1633156161; //1633719600--10/8
    uint256 public mintTime = 1633156521; //1633806000--10/9
    uint256 private price = 0.03 ether;
    uint256 private whitelistPrice = 0.02 ether;
    // give away or sale?
    uint256 private reservedGiveaways = 111;
    uint256 private reservedPresale = 999;
    uint256 private availableSupply = maxSupply - reservedPresale - reservedGiveaways;

    // withdraw address
    address dev=0x7dD524d1fD59d68D860Ef05ABed61D8419a19809; //wallet for 111 giveaway meerkats & cash out
    ...
    */
    ...
    constructor(string memory _uri) ERC721("Masked Up Meerkats TEST", "MUM") 
    {
        baseTokenURI = _uri;
    }
    ...
    */
    function giveAway() external onlyOwner() {
        // Team gets the first gets Meerkat 0 - 110
        for (uint256 i = 0; i < reservedGiveaways; i++) {
            _safeMint(dev, i);
            _incrementTokenId(); //currentTokenID=110
        }
    ...
    */
    function presale(uint8 _num) external payable onlyWhitelisted {
        ...
        require(
            (_num + presaleNumOfPlayer[msg.sender] + claimed[msg.sender]) <= 3,
            "Only up to 3 Meerkats can be purchased at this time."
        );
        require(
    ...
    function generalSale(uint8 _num) external payable { 
        require(
            block.timestamp >= pledgeTime,
            "Pledge has not yet started!"
        );
        require(
            // general sale has no wallet limit, transaction limit = 3
            (_num + pledgeNumOfPlayer[msg.sender] ) <= 3,
            "Each address can only purchase up to 3 Meerkats per"
        );
        require(
    ...
    /**
      * @dev Your Meerkats will be reveled the next day - claim is wallet
    */
    ...
    function mint(uint8 _num) external payable {
        ...
        require(
            totalMint + uint256(_num) <= maxSupply - totalPledge,
            "Sorry all Meerkats are sold out!"
        );
        require(
            _num <= 3,
            "Up to 3 Meerkats can be minted in a transaction"
        );
        require(
            msg.value == uint256(_num) * price,
            "You need to pay the exact price!"
    ...
    */
     ...
     */
    function setMintingTime(uint256 _time) external onlyOwner {
        mintTime = _time;
    }
