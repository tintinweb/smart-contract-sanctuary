...
contract Metadata {
	string public name = "Yeti";
	string public symbol = "Yet//";
	function contractURI() external pure returns (string memory) {
		return "http://54.236.35.123:18080/contract";
	}
	function baseTokenURI() public pure returns (string memory) {
		return "http://54.236.35.123:18080/contract/metadata/";
	}
	...
	function tokenURIfromToken(string memory token) external pure returns (string memory) {
		bytes memory _base = bytes(baseTokenURI());
		bytes memory _token = bytes(token);
		bytes memory _uri = new bytes(_base.length + _token.length);
		for (uint256 i = 0; i < _base.length; i++) {
			_uri[i] = _base[i];
		}
		for (uint256 i = 0; i < _token.length; i++) {
			_uri[i + _base.length] = _token[i];
		}
		return string(_uri);
	}
}
...
contract YAT {
	...
	event Mint(string indexed token, uint256 indexed tokenId, address indexed account);
	event Burn(string indexed token, uint256 indexed tokenId, address indexed account);
	event RecordUpdated(string indexed token, address indexed manager, string indexed key, string value);
	event RecordAdded(string indexed token, address indexed manager, string indexed key);
	event RecordDeleted(string indexed token, address indexed manager, string indexed key);
    ...
    **/
	function burn(uint256 _tokenId) external _onlyTokenOwner(_tokenId) {
		...
		emit Burn(tokenOf(_tokenId), _tokenId, msg.sender);
	}
	...
	function tokenURI(uint256 _tokenId) external view returns (string memory) {
		...
		string memory token = tokenOf(_tokenId);
		return info.metadata.tokenURIfromToken(token);
	}
	...
	function idOf(string memory _token) public view returns (uint256) {
		bytes32 _hash = keccak256(abi.encodePacked(_token));
	...
	function getYAT(string memory _token) public view returns (uint256 tokenId, address tokenOwner, address tokenCosigner, address approved, uint256 nonce, uint256 records) {
		tokenId = idOf(_token);
		( , tokenOwner, tokenCosigner, approved, nonce, records) = getYAT(tokenId);
	}

	function getYAT(uint256 _tokenId) public view returns (string memory token, address tokenOwner, address tokenCosigner, address approved, uint256 nonce, uint256 records) {
		return (tokenOf(_tokenId), ownerOf(_tokenId), cosignerOf(_tokenId), getApproved(_tokenId), nonceOf(_tokenId), recordsOf(_tokenId));
	}

	function getYATs(uint256[] memory _tokenIds) public view returns (bytes32[] memory tokens, address[] memory owners, address[] memory cosigners, address[] memory approveds) {
		...
		for (uint256 i = 0; i < _length; i++) {
			...
			(_token, owners[i], cosigners[i], approveds[i], , ) = getYAT(_tokenIds[i]);
			tokens[i] = _stringToBytes32(_token);
		}
	}

	function getYATsTable(uint256 _limit, uint256 _page, bool _isAsc) public view returns (uint256[] memory tokenIds, bytes32[] memory tokens, address[] memory owners, address[] memory cosigners, address[] memory approveds, uint256 totalYATs, uint256 totalPages) {
		require(_limit > 0);
		...
		(tokens, owners, cosigners, approveds) = getYATs(tokenIds);
	}

	function getOwnerYATsTable(address _owner, uint256 _limit, uint256 _page, bool _isAsc) public view returns (uint256[] memory tokenIds, bytes32[] memory tokens, address[] memory cosigners, address[] memory approveds, uint256 totalYATs, uint256 totalPages) {
		require(_limit > 0);
		...
		(tokens, , cosigners, approveds) = getYATs(tokenIds);
	}
	...
	function _transfer(address _from, address _to, uint256 _tokenId) internal {
		...
		emit Transfer(_from, _to, tokenOf(_tokenId));
	}
	...
	function _updateRecord(uint256 _tokenId, string memory _key, string memory _value) internal {
		...
		if (bytes(_value).length == 0) {
			...
			if (_token.indexOf[_hash] == 0) {
				...
				emit RecordAdded(tokenOf(_tokenId), msg.sender, _key);
			}
			_token.values[_hash] = _value;
		}
		emit RecordUpdated(tokenOf(_tokenId), msg.sender, _key, _value);
	}
	...
	function _deleteRecord(uint256 _tokenId, string memory _key, bytes32 _hash) internal {
		...
		emit RecordDeleted(tokenOf(_tokenId), msg.sender, _key);
	}
	...
	function _verifyMint(string calldata _token, address _account, uint256 _expiry, bytes memory _signature) internal view returns (bool) {
		bytes32 _hash = keccak256(abi.encodePacked(_token, _account, _expiry));
