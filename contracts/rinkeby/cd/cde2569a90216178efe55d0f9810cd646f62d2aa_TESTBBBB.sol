...
contract TESTBBBB is ERC721Enumerable, Ownable {
    ...
    uint256 constant public MAX_MINT_WHITELIST = 5;
    
    uint256 private reservedAmount = 120;
    ...
    event PrivateSaleStopped();
    event TokenMinted(uint256 supply);
  
    constructor (string memory newBaseURI) ERC721 ("TESTBBBB", "TEB") {
    ...
    function isWhitelistCanMint(address addr) public view returns (bool isWhiteListCanMint) {
        return whitelist[addr].hasMinted <= MAX_MINT_WHITELIST;
    }
    ...
    function giveaway(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= reservedAmount, "Exceeds reserved Cat supply" );
    ...
    function mint(uint256 num) public payable {
        ...
        require( totalSupply() + num < MAX_NFT_SUPPLY - reservedAmount, "Exceeds maximum supply" );
        require( price.mul(num) <= msg.value, "Ether sent is not correct" );
        
        if(_presaleActive){
            require(isWhitelisted(msg.sender), "Is not whitelisted");
            require(whitelist[msg.sender].hasMinted.add(num) <= MAX_MINT_WHITELIST, "Can only mint 5 while whitelisted");
            require(isWhitelistCanMint(msg.sender), "Can only mint 5 while whitelisted");
