...
contract GoodOneBidStorage {
    ...
    // List owner by token address => token id => address
    mapping(address => mapping(uint256 => address)) public listTokenOwnerPool;

    // List reserve price by token address => token id => uint256
    mapping(address => mapping(uint256 => uint256)) public listTokenReservePrice;

    // list toekn expires by token address => token id => uint256
    mapping(address => mapping(uint256 => uint256)) public listTokenExpires;
    ...
    event NFTListedDurationExtended(
      address _owner,
      address _tokenAddress, 
      uint256 _tokenId, 
      uint256 _amount,
...
contract GoodOneBid is Ownable, Pausable, GoodOneBidStorage {
    ...
    */
    ...
    function listNFT(address _tokenAddress, uint256 _tokenId, uint256 _amount, uint256 _reservePrice, uint256 _duration) public {
        require(
          ERC1155Interface(_tokenAddress).balanceOf(msg.sender, _tokenId) == _amount, 
          "Createor does not have NFT amount"
        );

        ERC1155Interface(_tokenAddress).safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "");

        uint256 expiresAt = block.timestamp.add(_duration);

        listTokenOwnerPool[_tokenAddress][_tokenId] = msg.sender;
        listTokenReservePrice[_tokenAddress][_tokenId] = _reservePrice;
        listTokenExpires[_tokenAddress][_tokenId] = expiresAt;
    ...
    function unlistNFT(address _tokenAddress, uint256 _tokenId, uint256 _amount) public {
        require(
          ERC1155Interface(address(this)).balanceOf(msg.sender, _tokenId) == _amount, 
          "Createor does not have NFT amount"
        );

         require(
           listTokenOwnerPool[_tokenAddress][_tokenId] == msg.sender || msg.sender == owner(),
          "Must be creator or owner to unlist NFT");

        ERC1155Interface(_tokenAddress).safeTransferFrom(address(this), msg.sender, _tokenId, _amount, "");

        delete listTokenOwnerPool[_tokenAddress][_tokenId];
        delete listTokenReservePrice[_tokenAddress][_tokenId];
        delete listTokenExpires[_tokenAddress][_tokenId];
    ...
    function changeReservePrice(address _tokenAddress, uint256 _tokenId, uint256 _amount, uint256 _reservePrice) public {
        require(
           listTokenOwnerPool[_tokenAddress][_tokenId] == msg.sender || msg.sender == owner(),
    ...
    */
    function placeBid(
        ...
        whenNotPaused()
    {
        address owner = listTokenOwnerPool[_tokenAddress][_tokenId] = msg.sender;
        uint256 reservePrice = listTokenReservePrice[_tokenAddress][_tokenId];
        uint256 listExpires = listTokenExpires[_tokenAddress][_tokenId];
        ...
        require(_price > highestBid.price && _price > reservePrice, "Price should be bigger than highest Bid and reserve price");

        require(block.timestamp > listExpires, "List has ended");

        if(listExpires.sub(block.timestamp) < FIFTEEN_MINUTE) {
            
            uint256 newListTokenExpires = block.timestamp.add(FIFTEEN_MINUTE);
            listTokenExpires[_tokenAddress][_tokenId] = newListTokenExpires;

            emit NFTListedDurationExtended(
                owner,
                _tokenAddress, 
                _tokenId, 
                _amount,
    ...
    */
        ...
        whenNotPaused()
    {
        ...
        address nftOwner = listTokenOwnerPool[_tokenAddress][_tokenId];
     
        require(msg.sender == nftOwner, "Sender must be the NFT owner");
