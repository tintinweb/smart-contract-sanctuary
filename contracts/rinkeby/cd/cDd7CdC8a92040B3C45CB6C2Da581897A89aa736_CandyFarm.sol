...
contract CandyFarm is Initializable, OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
    ...
    function initialize(
        ...
        rewardPerTime = _rewardTokenAmount.mul(REWARD_PRECISION) / (_endDate - startDate);

        // Set the lastRewardTime as the startDate
        lastRewardTime = startDate;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 stakedAmount = _takeFeesAndReturnStakedAmount(_amount);
            user.amount = user.amount.add(stakedAmount);
        }

        user.rewardDebt = user.amount.mul(accTokenPerShare) / (PRECISION_FACTOR_DIV);
     ...
     * @param _newEndTime: new end date
     * @return Amount new reward tokens added to keep the same reward rate
     */
    function extendPool(uint256 _newEndTime) external returns(uint256) {
        // calculate required amount
        uint256 newRewardAmount = _extendPoolInternal(_newEndTime);
        rewardToken.safeTransferFrom(msg.sender, address(this), newRewardAmount);
        return newRewardAmount;
    }

    function _extendPoolInternal(uint256 _newEndTime) internal returns(uint256) {
        // calculate required amount
        require(endDate > block.timestamp, "Candy farm must be active to extend");
        require(_newEndTime > endDate, "New end time must be larger than before");

        uint256 timeDiff = _newEndTime.sub(endDate);
        uint256 newRewardAmount = (timeDiff.mul(rewardPerTime) / REWARD_PRECISION).add(1);

        endDate = _newEndTime;
        rewardTokenAmount = rewardTokenAmount.add(newRewardAmount);

        ExtendDuration(_newEndTime);
        RewardTokenAmountIncreased(rewardTokenAmount);
     ...
     */
    function increaseReward(uint256 _additionalAmount) external {
    ...
    function _increaseRewardInternal(uint256 _additionalAmount) internal {
        ...
        rewardPerTime = rewardTokenAmount.mul(REWARD_PRECISION) / (endDate - startDate);
     ...
     * @param _additionalAmount: additional reward token amount
     */
    function increaseRewardAndDuration(uint256 _newEndTime, uint256 _additionalAmount) external {
        uint256 extendAmount = _extendPoolInternal(_newEndTime);
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTime && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(bonusReward.mul(PRECISION_FACTOR_MUL) / (stakedTokenSupply));
            return (user.amount.mul(adjustedTokenPerShare) / (PRECISION_FACTOR_DIV)).sub(user.rewardDebt);
        } else {
    ...
    function _getUserPendingReward(uint256 userAmount, uint256 userRewardDebt) private view returns(uint256) {
        return userAmount.mul(accTokenPerShare) / (PRECISION_FACTOR_DIV).sub(userRewardDebt);
    }
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardTime, block.timestamp);
        uint256 bonusReward = multiplier.mul(rewardPerTime);
        accTokenPerShare = accTokenPerShare.add(bonusReward.mul(PRECISION_FACTOR_MUL) / (stakedTokenSupply));
        lastRewardTime = block.timestamp;
    }

    /**
     * @notice Take fees from amount and return staked amount after fees
     * @param _amount: Amount before fees
     * @return Amount after fees
     */
    function _takeFeesAndReturnStakedAmount(uint256 _amount) private returns(uint256) {
        // calculate fees
        uint256 feesAmount = _amount.mul(feesController.feesPpm()) / 1000;

        // send fees
        stakedTokenPop.safeTransfer(feesController.feesTo(), feesAmount);

        // return amount after fees 
        return _amount.sub(feesAmount);
    }
