...
contract MaterialBag is ERC721, ReentrancyGuard, Ownable, MaterialMetadata {
    ...
    constructor() ERC721("Material Bags (for Adventurers)", "MaterialBag") {
        //for testing
    ...
    function mintWithLoot(uint lootBagId) public payable nonReentrant {
        ...
        require(((membersPrice < msg.value) || (membersPrice > msg.value)) , "Ether value sent is not correct");
    ...
    function multiMintWithLoot(uint[] memory lootBagIds) public payable nonReentrant {
        require(saleIsActive, "Sale must be active to mint");
        require((membersPrice * lootBagIds.length) < msg.value || (membersPrice * lootBagIds.length) > msg.value, "Ether value sent is not correct");
    ...
    function mint(uint lootBagId) public payable nonReentrant {
        require(saleIsActive, "Sale must be active to mint");
        require(publicPrice < msg.value || publicPrice > msg.value, "Ether value sent is not correct");
    ...
    function multiMint(uint[] memory lootBagIds) public payable nonReentrant {
        require(saleIsActive, "Sale must be active to mint");
        require((publicPrice * lootBagIds.length) < msg.value || (publicPrice * lootBagIds.length) > msg.value, "Ether value sent is not correct");
    ...
    function ownerMint(uint lootBagId) public payable nonReentrant onlyOwner {
        require(ownersPrice < msg.value || ownersPrice > msg.value, "Ether value sent is not correct");
    ...
    function ownerMultiMint(uint[] memory lootBagIds) public payable nonReentrant onlyOwner {
        require((ownersPrice * lootBagIds.length) < msg.value || (ownersPrice * lootBagIds.length) > msg.value, "Ether value sent is not correct");
    ...
    function materialAttributes(uint256 id) internal view returns (string memory) {
        string memory res = string(abi.encodePacked('[', materialTrait("Gem", materialTokenName(gemId(id)))));
        res = string(abi.encodePacked(res, ", ", materialTrait("Rune", materialTokenName(runeId(id)))));
        res = string(abi.encodePacked(res, ", ", materialTrait("Material", materialTokenName(materialId(id)))));
        res = string(abi.encodePacked(res, ", ", materialTrait("Charm", materialTokenName(charmId(id)))));
        res = string(abi.encodePacked(res, ", ", materialTrait("Tool", materialTokenName(toolId(id)))));
        res = string(abi.encodePacked(res, ", ", materialTrait("Element", materialTokenName(elementId(id)))));
        res = string(abi.encodePacked(res, ", ", materialTrait("Requirement", materialTokenName(requirementId(id)))));
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #', Strings.toString(tokenId), '", "description": "MaterialBag is randomized adventurer gear generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use MaterialBag in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '", ''"attributes": ', materialAttributes(tokenId),'}'))));
    ...
    function materialIds(uint256 tokenId) public view returns (MaterialItemIds memory) {
        require(ownerOf(tokenId) == msg.sender, "Not the owner of this material bag.");
        return
            MaterialItemIds({
                gem: gemId(tokenId),
                rune: runeId(tokenId),
    ...
    function seeMaterials(uint256 tokenId) public view returns (MaterialItemNames memory) {
        require(ownerOf(tokenId) == msg.sender, "Not the owner of this material bag.");
        MaterialItemIds memory items = materialIds(tokenId);
        return
            MaterialItemNames({
                gem: materialTokenName(items.gem),
                rune: materialTokenName(items.rune),
                material: materialTokenName(items.material),
                charm: materialTokenName(items.charm),
                tool: materialTokenName(items.tool),
                element: materialTokenName(items.element),
                requirement: materialTokenName(items.requirement)
            });
    }
