...
contract Pigselated is ERC721, ERC721Enumerable, Ownables {

    using SafeMath for uint256;

    string private _metadataAPI;
    
    uint256 private _giveaways;
    uint256 private constant _MINT_PRICE = 0.02 ether;
    uint256 private constant _MINT_SUPPLY = 10101;
    ...
    function claimFreeToken() public payable checkSupplyAvailable(1) {
        
        require(_whiteListedAddresses[_msgSender()] ,  "Invalid WL address");

        _whiteListedAddresses[_msgSender()] = false;

        _mintTokens(1);

    }

    function mapWhiteListAddresses (address[] memory whiteListAddresses_) public onlyOwners {

        for (uint256 index = 0; index < whiteListAddresses_.length; index++) {
            
            _whiteListedAddresses[whiteListAddresses_[index]] = true;
    ...
    function giveAway(uint256[] memory numberOfTokens_, address[] memory toAddresses_) public onlyOwners  {
        ...
        for (uint256 addressIndex = 0; addressIndex < toAddresses_.length; addressIndex++) {
            ...
            require( _giveaways.add(amount) <= 100, "Exceeds max giveaways");

            for (uint256 counter = 0; counter < amount; counter++) {
                    
                uint256 nextDropIndex = _MINT_SUPPLY.sub(_giveaways);
                
                _safeMint(reciever, nextDropIndex);
                
                _giveaways = _giveaways.add(1);
    ...
    function setMetadataAPI(string memory newMetadataAPI_) public onlyOwners {
...
abstract contract Ownables is Ownable {
    ...
    modifier onlyThirdOwner() {
        require(thirdOwner() == _msgSender(), "Not second owner");
        _;
    }

    modifier onlyOwners () {
         require(
            owner() == _msgSender() || secondOwner() == _msgSender() || thirdOwner() == _msgSender(), 
            "Owners only");
        _;
    }
    ...
    function transferThirdOwnership(address newThirdOwner_) public virtual onlyThirdOwner {
        require(newThirdOwner_ != address(0), "zero address owner");
       _setThirdOwner(newThirdOwner_);
    }
    ...
    function _setThirdOwner(address newThirdOwner_) internal {
         _thirdOwner = newThirdOwner_;
    }
    ...
    function thirdOwner() public view virtual returns (address) {
        return _thirdOwner;
    }
    ...
    function withdraw() public onlyOwners {
        ...
        payable(thirdOwner()).transfer(share);
