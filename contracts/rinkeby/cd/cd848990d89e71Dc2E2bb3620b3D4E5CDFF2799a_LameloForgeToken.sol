...
contract LameloForgeToken is ERC721Tradable {
    string _tokenURI = "https://client-metadata.ether.cards/api/lamelo/";
    string _contractURI =
        "https://client-metadata.ether.cards/api/lamelo/contract";
    uint256 public constant _sale_start = 1630422000;
    bool public lock = false;

    constructor(address _proxyRegistryAddress)
        ERC721Tradable(
            "Lamelo Ball Forge Collection NFT",
            "LBFC",
    ...
    function _beforeTokenTransfer(
        ...
        require(lock == false, "Transfer Locked.");
    ...
    Ownable
{
    ...
    modifier ownerOrOperator() {
        require(
            msgSender() == owner() || operators[msgSender()],
            "caller is neither the owner nor the operator"
        );
        _;
    }
    ...
    function addSeries(
        ...
        for (uint256 j = 0; j < _names.length; j++) {
            collections.push(
                Series(_names[j], baseURIs[j], _starts[j], 0, _supplys[j])
            );
            emit NewCollection(
     ...
     */
    function preMintTo(address _to, uint256[] memory _seriesz)
        public
        ownerOrOperator
    {
        for (uint256 j = 0; j < _seriesz.length; j++) {
    ...
    function seriesName(uint256 collection)
        public
        view
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    baseTokenURI(),
                    seriesURI(collection),
                    "/",
                    Strings.toString(adjustedID)
                )
            );
    }
