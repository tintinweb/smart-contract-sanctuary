...
contract BeetTokenVesting is Ownable {
  ...
  function recoverErc20(address token_, uint256 amount_) external onlyOwner {
    require(token_ != address(token), "BeetTokenVesting::recoverErc20: cannot recover vesting tokens");
  ...
  function recoverEther() external onlyOwner {
    ...
    require(success, "BeetTokenVesting::recoverEther: failed to send Ether");
  ...
  function initVestingPool(string calldata name_, uint256 startTime_, uint256 endTime_)
    ...
    returns (uint8) 
  {
    require(block.timestamp < startTime_, "BeetTokenVesting::initVestingPool: invalid pool start time");
    require(startTime_ < endTime_, "BeetTokenVesting::initVestingPool: invalid pool end time");
  ...
  function setVestingPool(uint8 pid_, string calldata name_, uint256 startTime_, uint256 endTime_)
    ...
    onlyOwner
  {
    require(lockPools[pid_].startTime != 0, "BeetTokenVesting::setVestingPool: pool does not exist");
    require(lockPools[pid_].startTime > block.timestamp, "BeetTokenVesting::setVestingPool: pool is already running");
    require(startTime_ > block.timestamp, "BeetTokenVesting::setVestingPool: invalid pool start time");
    require(startTime_ < endTime_,"BeetTokenVesting::setVestingPool: invalid pool end time");
    require(lockPools[pid_].initialStartTime + ALLOWED_VESTING_UPDATE_THRESHOLD >= startTime_,
      "BeetTokenVesting::setVestingPool: new start date is to large"
    );
    require(lockPools[pid_].initialStartTime - ALLOWED_VESTING_UPDATE_THRESHOLD <= startTime_,
      "BeetTokenVesting::setVestingPool: new start date is to small"
    );
    require(lockPools[pid_].initialEndTime + ALLOWED_VESTING_UPDATE_THRESHOLD >= endTime_,
      "BeetTokenVesting::setVestingPool: new end date is to large"
    );
    require(lockPools[pid_].initialEndTime - ALLOWED_VESTING_UPDATE_THRESHOLD <= endTime_,
      "BeetTokenVesting::setVestingPool: new end date is to small"
    );
  ...
  function addBeneficiary(uint8 pid_, address beneficiary_, uint256 lockedAmount_)
    external
  {
    require(pid_ < lockPools.length, "BeetTokenVesting::addBeneficiary: non existing pool");
  ...
  function addBeneficiaryBatches(uint8 pid_, address[] calldata beneficiaries_, uint256[] calldata lockedAmounts_)
    external
  {
    require(beneficiaries_.length == lockedAmounts_.length, "BeetTokenVesting::addBeneficiaryBatches: params invalid length");
    require(pid_ < lockPools.length, "BeetTokenVesting::addBeneficiaryBatches: non existing pool");
  ...
  function claim(uint8 pid_) external returns (uint256 amount) {
    ...
    require(amount > 0, "BeetTokenVesting::claim: can't claim 0 amount");
