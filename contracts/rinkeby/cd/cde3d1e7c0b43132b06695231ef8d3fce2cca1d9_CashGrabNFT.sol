...
contract CashGrabNFT is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function bulkMint(uint amountOfAddresses, address[] memory _addressList) public onlyOwner {
        require((_tokenIds.current() + amountOfAddresses) <=  MAX_CASH_GRAB_NFTS, "Mint would exceed max supply of Cash Grabs");
        
        for(uint i = 0; i < amountOfAddresses; i++) {
            _tokenIds.increment();
    ...
    function purchase(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= maxPurchaseAmt, "Can only mint 20 tokens at a time");
        require((cashGrabPrice * numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        mintCashGrabs(numberOfTokens);
    }
    
    function mintCashGrabs(uint amountOfTokens) internal nonReentrant {
        require((_tokenIds.current() + amountOfTokens) <=  MAX_CASH_GRAB_NFTS, "Mint would exceed max supply of Cash Grabs");
    ...
    function setStartingIndex() internal {
        ...
        if ((block.number - startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % MAX_CASH_GRAB_NFTS;
        }
    }
