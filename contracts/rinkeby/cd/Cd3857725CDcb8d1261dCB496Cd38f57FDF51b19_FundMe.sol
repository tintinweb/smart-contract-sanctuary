...
contract FundMe {
    ...
    address[] funders;
    address prices; //address of price feed

    constructor(address price_address) {
        ...
        owner = msg.sender;
        prices = price_address; //depending on the network we use we may need to change price feed address.
    }
    ...
    function priceOfOneGwei() public view returns (uint256) {
        AggregatorV3Interface prices = AggregatorV3Interface(address(prices)); //casting address to type aggregatorv3interface... anyway this whole thing is like a struct, u need this address to behave like this av3i in order to call the members that are in it... in this case the functions
    ...
    function numOfDecimals() public view returns (uint8) {
        AggregatorV3Interface decimals = AggregatorV3Interface(address(prices));
