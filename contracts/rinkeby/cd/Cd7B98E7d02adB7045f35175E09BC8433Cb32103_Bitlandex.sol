...
contract Bitlandex is ERC721, Ownable{
    uint256 public tokenCounter;
    constructor () public ERC721 ("BitlanDex", "BD"){
        tokenCounter = 0;
    }
    ...
    function updateTokenURI(uint256 _tokenId, string memory _uri) public {
        require(_exists(_tokenId), "bitlanDexERC721: Can not update nonexistent token");
        uriDetails storage data = dataInfo[_tokenId];
        data.uri = _uri;
        _setTokenURI(_tokenId, _uri);
    }
    
    function getTokenDetails(uint256 _tokenId) view public returns(uint256,string memory){
        require(_exists(_tokenId), "bitlanDexERC721:No details for nonexistent token");
