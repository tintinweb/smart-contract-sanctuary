...
contract APG is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    constructor(string memory _baseTokenURI, string memory _ghostTokenURI) ERC721(_name, _symbol) {
        baseTokenURI = _baseTokenURI;
        ghostTokenURI = _ghostTokenURI;
        _initializeEIP712(_name);
    }

    function set_reveal_time(uint256 time) external onlyOwner {
        reveal_time = time;
    }
    ...
    function buyTen() public payable {
        require(block.timestamp >= startTime, "It's not time yet");
    ...
    function buyAmount(uint256 count) internal {
        require(_currentTokenId.add(count) < MAX_SUPPLY, "Max Supply Reached");
