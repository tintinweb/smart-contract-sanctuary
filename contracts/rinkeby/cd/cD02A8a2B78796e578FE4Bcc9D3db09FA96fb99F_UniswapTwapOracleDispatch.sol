...
contract UniswapTwapOracleDispatch is IOracleDispatch, RevestAccessControl {
    ...
    function updateOracle(address asset, address compareTo) external override returns (bool) {
        ...
        if(!twap.initialized) {
            if(twap.timestampLatest == 0) {
                ...
                twap.lastUpdateTwapPrice = getTwapPrice(asset, compareTo, cumLast, lastTime, lastTimeTwapUpdated, lastPrice);
    ...
    function getTwapPrice(address asset, address compareTo, uint cumulativeLast, uint timestamp, uint lastTime, uint lastCumPrice) internal view returns (uint) {
