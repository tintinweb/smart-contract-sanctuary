...
contract LoanMarket is LoanMarketBase {
    ...
    @param _loanType, Type of the loan SINGLE_TOKEN || MULTI_TOKEN
    ...
    @param _stakedCollateralTokens, addressed of the staked collateral approved by the GOV Admins
    @param _stakedCollateralAmounts, the amount of the staked colletral borrower is staking
    @param _borrowStableCoin any stable coin address USDT || DAI || USDC || TUSD || PAX || BUSD

    */
     function createLoanOfferToken(
        ...
        require(LoanData.LoanType.SINGLE_TOKEN  == _loanType || LoanData.LoanType.MULTI_TOKEN == _loanType,"GLM: Invalid Loan Type");
        if(LoanData.LoanType.SINGLE_TOKEN  == _loanType) {//for single tokens collateral length must be one.
            require(_stakedCollateralTokens.length == 1,"GLM: Multi-tokens not allowed in SINGLE TOKEN loan type.");
        }
        uint256 ltv  = _calculateLTV(_stakedCollateralAmounts, _stakedCollateralTokens, _borrowStableCoin, _loanAmountInBorrowed, true);
        require(_isLtvUnderTier(ltv), "GLM: LTV Must Exceed Tier Level Limit");
    ...
    function createLoanOfferNFT(
        ...
        require(_isLtvUnderTier(newLoanId), "GLM: LTV Exceeds Tier Level Limit");
    ...
    function tokenLoanOfferAdjusted(
        ...
        loanOffersToken[_loanIdAdjusted] = LoanData.LoanDetails(
            ...
            _newAPYOffer,
            oldLoanDetails.loanType, //single or multitoken
            oldLoanDetails.loanStatus,
            _newTermsLengthInDays,
            _isPrivate,
            _isPartialFunding,
            _isInsured,
            msg.sender,
            _newBorrowStableCoin
        );
    ...
    function _isLtvUnderTier(uint256 _ltv) internal view returns (bool) {
        ...
        returns (uint256)
    {
        ...
        return _calculateLTV(stakedCollateralAmounts, stakedCollateralTokens, borrowedToken, loanDetails.loanAmountInBorrowed, _isTier);
    }
