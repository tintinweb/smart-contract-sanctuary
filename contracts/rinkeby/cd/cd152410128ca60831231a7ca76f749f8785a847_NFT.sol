...
contract NFT is ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address marketplaceAddress, string memory _baseURI, string memory _notRevealedURI) public initializer {
        __ERC721_init("Metaverse", "METT");
        marketAddress = marketplaceAddress;
        mintingFee = 0.015 ether;
        baseURI = _baseURI;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(currentBaseURI).length > 0
            ? string(abi.encodePacked(currentBaseURI, tokenURIs[tokenId]))
            : "";
    }
    ...
    function createToken(string memory _tokenURI) public payable returns (uint) {
        require(msg.value == mintingFee, "minting fee is required");
