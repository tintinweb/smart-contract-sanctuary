...
import "./MerkleLaunchpad.sol";

contract MerkleLaunchpadFactory is Ownable, ReentrancyGuard {
  ...
  function launch(string memory _launchpadTitle) external returns (MerkleLaunchpad){
    if(whitelistEnforced){
      require(whitelistedOperators[msg.sender], "FACTORY: OPERATOR NOT WHITELISTED");
    }
    MerkleLaunchpad launchpad = new MerkleLaunchpad(_launchpadTitle);
