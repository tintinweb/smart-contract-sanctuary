...
contract ACPIMaster is IACPIMaster, AccessControl {
    ...
    function withdrawAll(address payable vault)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        acpiOne.withdraw(vault, payable(address(acpiOne)).balance);
        acpiTwo.withdraw(vault, payable(address(acpiTwo)).balance);
        acpiThree.withdraw(vault, payable(address(acpiThree)).balance);
        acpiFour.withdraw(vault, payable(address(acpiFour)).balance);
    }
