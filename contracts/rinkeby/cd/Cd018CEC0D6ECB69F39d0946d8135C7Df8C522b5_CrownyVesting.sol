...
contract CrownyVesting is Ownable {
    ...
    event ClaimedUnallocated(address _address, uint256 _amount);
    ...
    function setVesting(address _beneficiary, uint32[] calldata _junctures, uint256 _vestingAmount, uint256 _vestingIncrement, uint256 _initialClaim) external onlyOwner {
        require (_junctures.length > 0, "CrownyVesting: junctures and amounts cannot be empty");
        require (__vesters[_beneficiary] == false, "CrownyVesting: address already vesting");
        require (_vestingAmount <= __token.balanceOf(address(this)).sub(__balanceAllocated), "CrownyVesting: contract doesn't have enough tokens (yet)");
    ...
    function emergencyClaim(address _beneficiary, uint256 _amount) external onlyOwner {
        uint256 maximumClaimable = __vestingAmounts[_beneficiary].sub(__claimed[_beneficiary]);

        require(_amount <= maximumClaimable, "CrownyVesting: nope, even in emergencies you can't get more than you deserve");
    ...
    function revokeVesting(address _beneficiary) external onlyOwner {
        ...
        __handleRevoke(_beneficiary);
        
        emit Revoked(_beneficiary);
    }

    function claimUnallocatedTokens(address _beneficiary) external onlyOwner {
        uint256 unallocatedAmount = __token.balanceOf(address(this)).sub(__balanceAllocated);
        
        __token.safeTransfer(_beneficiary, unallocatedAmount);

        emit ClaimedUnallocated(_beneficiary, unallocatedAmount);
    }
    ...
    function claimTokens() external {
        uint256 amount = allowedToClaim(_msgSender());

        require (amount > 0, "CrownyVesting: nothing to claim here");

        __handleClaim(_msgSender(), amount);

        emit Claimed(_msgSender(), amount);
    }

    function viewTotalVestingAmount(address _beneficiary) external view returns (uint256) {
        return __vestingAmounts[_beneficiary];
    }

    function viewVestedAmount(address _beneficiary) external view returns (uint256) {
        return __claimed[_beneficiary].add(allowedToClaim(_beneficiary));
    }

    function viewClaimed(address _beneficiary) external view returns (uint256) {
    ...
    function allowedToClaim(address _beneficiary) public view returns (uint256) {
    ...
    function __handleRevoke(address _beneficiary) internal {
        ...
        __token.safeTransfer(_msgSender(), tokenDifference);
    }
