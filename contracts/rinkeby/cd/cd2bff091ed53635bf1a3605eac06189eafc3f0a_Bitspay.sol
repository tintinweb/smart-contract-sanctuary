...
contract Bitspay {
  ...
  function deposit(uint tariffPlam, address referer) external payable {
        ...
        investor[msg.sender].deposits.push(Deposit(tariffPlam, amount, block.timestamp,false));

        emit DepositAt(msg.sender, tariffPlam, amount);
  ...
  function withdrawable(address user) public view returns (uint amount) {
    ...
    for (uint i = 0; i < investor.deposits.length; i++) {
      ...
      bool hasWithdrawal = dep.isWithdrawal;
      
      if(hasWithdrawal == false){
