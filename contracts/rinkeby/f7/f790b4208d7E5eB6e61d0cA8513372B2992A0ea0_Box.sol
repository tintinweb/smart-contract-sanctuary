...
interface ICard {
    function mintCount(uint256, uint16) external;
}

contract Box is ERC1155, Ownable {
    mapping(uint256 => bool) opened;

    address private CardContract;
    uint16[] private CountInBox = [10, 5, 4, 1]; // common, rare, epic, legendary

    constructor(address cardContract) ERC1155("cardpack") {
        CardContract = cardContract;
    }

    function mint(address to, uint256 boxType) public {
        _mint(to, boxType, 1, "");
    }

    function setCardContract(address _cardContract) public onlyOwner {
        CardContract = _cardContract;
    }

    function openBox(uint256 boxType) public {
        require(balanceOf(msg.sender, boxType) > 0);
        ICard(CardContract).mintCount(boxType, CountInBox[boxType]);
        _burn(msg.sender, boxType, 1);
    }
}