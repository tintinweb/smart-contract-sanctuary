...
// Created using NoBuddy - TODO github url
contract HubaNft is ERC721, ERC721Enumerable, Ownable {

    // price of a single NFT
    uint256 public constant TOKEN_PRICE = 80000000000000000; // 0.08 ETH

    // max. number of tokens that can be purchased at one time
    ...
    function purchase(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= MAX_PURCHASE, "Can only mint 1 tokens at a time");
        require(totalSupply() + numberOfTokens <= MAX_TOTAL_SUPPLY, "Purchase would exceed MAX_TOTAL_SUPPLY");
