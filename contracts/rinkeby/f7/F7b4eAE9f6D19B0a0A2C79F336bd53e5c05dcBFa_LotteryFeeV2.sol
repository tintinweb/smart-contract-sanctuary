...
// https://docs.xsigma.fi/sig-lp-token/fee-payout-for-lps
contract LotteryFeeV2 is VRFConsumerBase, Ownable {
    address constant public GAS_TOKEN = 0x0000000000b3F879cb30FE243b4Dfee438691c04;
    ...
    /**
    * Anyone can call this function to roll the dice once per game period
    */
    ...
    function test1(uint256 n) public returns (uint256) {
        uint256 j = 0;
    ...
    function test2(uint256 n) public returns (uint256) {
        saveGas();
        uint256 j = 0;
        for (uint256 i = 0; i < n; i++) {
            j = j + i;
    ...
    //======================protected functions==================================//
    function saveGas() internal {
        // uint256 value = 25710 + `value` * (1148 + 5722 + 150);
        uint256 value = 10; // 0.1 GST2 = 100k gas saving
        ERC20(GAS_TOKEN).approve(msg.sender, value);
        IGastoken(GAS_TOKEN).freeFrom(address(this),value);
    }

    function call3PoolDonataLpFees() internal {
        IMultisig(multisig).execTransaction(
            xsigma3poolDelegator,
            0,
            "0x524c3901",//abi.encodeWithSignature("donate_admin_fees()"),
            Enum.Operation.Call,
            150000,
            0,
            0,
    ...
    function checkRolledDice(uint256 diceFace) internal {
        if (diceFace == DICE_WINNING_FACE || address(LINK) == 0x01BE23585060835E02B77ef475b0Cc51aA1e0709) {
            saveGas();
            isWon = true;
...
//        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with faucet");
    ...
    function balanceOfToken(IERC20 token) view external returns (uint256) {
