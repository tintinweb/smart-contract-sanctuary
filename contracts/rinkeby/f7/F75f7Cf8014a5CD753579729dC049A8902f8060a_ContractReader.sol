...
contract ContractReader is IGovernable, Initializable, Controllable {
  ...
  function userInfo(address _user, address _vault) public view returns (UserInfo memory) {
    ...
    return UserInfo(
      ...
      userDepositedShareUsdc(_user, _vault),
      rewardTokens,
  ...
  function userUnderlyingBalance(address _user, address _vault) public view returns (uint256) {
    return IERC20(vaultUnderlying(_vault)).balanceOf(_user);
  }
  ...
  function userDepositedUnderlying(address _user, address _vault) public view returns (uint256) {
    return ISmartVault(_vault).underlyingBalanceWithInvestmentForHolder(_user);
  }
  ...
  function userDepositedShareUsdc(address _user, address _vault)
  public view returns (uint256) {
    uint256 price = getPrice(address(_vault));
    return userDepositedShare(_user, _vault).mul(price).div(PRECISION);
  }
