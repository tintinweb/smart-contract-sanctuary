...
interface ISniper {
	...
	function setLaunch(uint256 maxTx, uint16 taxes, address pair) external;
	function checkTransfer(address sender, address recipient, uint256 amount) external returns (bool);
	function calculateTaxes(address sender, uint256 amount) external view returns (uint256);
	function updateTxAmount(uint256 amount) external;
...
contract ETHBBToken is IERC20 {
	...
	constructor(string memory _a, string memory _b, address[] memory _c, uint256 _d, uint256 _e, uint16 _f, address _g) {
		...
		dexRouter = IRouter(routerAddr); lpPair = IFactory(dexRouter.factory()).createPair(address(this), dexRouter.WETH());
	...
	function transfer(address recipient, uint256 amount) external returns (bool) {
		tokenTransfer(msg.sender, recipient, amount);
		return true;
	}
	...
	function tokenTransfer(address sender, address recipient, uint256 amount) internal {
		...
		} else if (isLiquidityTransfer) {
			require(antiSnipe.checkTransfer(sender, recipient, amount), "Rejected/Blacklisted");
	...
	function feelessTransfer(address sender, address recipient, uint256 amount) internal {
	...
	function enableTrading() external onlyOwner {
		...
		tradingEnabled = true;
		antiSnipe.setLaunch(maxTxAmount, taxesAmount, lpPair);
	}
	...
	function doubleUpMaxes() external onlyOwner {
		maxTxAmount = maxTxAmount * 2; antiSnipe.updateTxAmount(maxTxAmount * 2);
