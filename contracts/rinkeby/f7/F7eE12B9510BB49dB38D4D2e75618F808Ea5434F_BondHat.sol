...
contract BondHat is ERC721, VRFConsumerBase {
    ...
    mapping(uint256 => Hat) public tokenIdToHat;
    mapping(bytes32 => uint256) public requestIdToTokenId;
    event requestedHat(bytes32 indexed requestId);
    ...
    function handcraftBondHat(string memory tokenURI, uint256 userProvidedSeed)
        public payable returns (bytes32){
        require(NFT_PRICE == msg.value, "Sent ether value is incorrect");
            bytes32 requestId = requestRandomness(keyHash, fee, userProvidedSeed);
            requestIdToSender[requestId] = msg.sender;
            requestIdToTokenURI[requestId] = tokenURI;
            emit requestedHat(requestId);
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        address hatOwner = requestIdToSender[requestId];
        string memory tokenURI = requestIdToTokenURI[requestId];
        uint256 newItemId = tokenCounter;
        _safeMint(hatOwner, newItemId);
        _setTokenURI(newItemId, tokenURI);
        Hat hat = Hat(randomNumber % 2);
        tokenIdToHat[newItemId] = hat;
