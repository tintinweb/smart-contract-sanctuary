...
// Following the recent worldwide pandemic, emerging reports suggest that several ball species have begun exhibiting strange characteristics. Our research team located across the globe has commenced efforts to study and document these unusual phenomena.
...
// The fate of the world's balls depends on it.

// from our website (https://boringballs.co)

// Ballheads is a community-centered enterprise focussed on preserving our research about the emerging reports that several ball species have begun exhibiting strange characteristics following the recent worldwide pandemic. 
...
contract Ballheads is ERC721, Ownable {
    ...
    // Reserve 125 Balls for team - Giveaways/Prizes etc
    uint public ballReserve = 125;
    ...
    function changeBallName(uint _tokenId, string memory _name) public {
        require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this ball!");
        require(sha256(bytes(_name)) != sha256(bytes(ballNames[_tokenId])), "New name is same as the current one");
        ballNames[_tokenId] = _name;
        
        emit ballNameChange(msg.sender, _tokenId, _name);
        
    }
    
    function viewBallName(uint _tokenId) public view returns( string memory ){
        require( _tokenId < totalSupply(), "Choose a ball within range" );
        return ballNames[_tokenId];
    }
    
    
    // GET ALL BALLS OF A WALLET AS AN ARRAY OF STRINGS. WOULD BE BETTER MAYBE IF IT RETURNED A STRUCT WITH ID-NAME MATCH
    function ballNamesOfOwner(address _owner) external view returns(string[] memory ) {
        uint256 tokenCount = balanceOf(_owner);
        if (tokenCount == 0) {
            // Return an empty array
            return new string[](0);
            ...
            uint256 index;
            for (index = 0; index < tokenCount; index++) {
                result[index] = ballNames[ tokenOfOwnerByIndex(_owner, index) ] ;
            }
            return result;
        }
    }
