...
// File: contracts/CoolestContract.sol
...
contract CoolestContract is ERC721Enumerable, Ownable {
    // declaring required variables and they are understandble by the name
    using Strings for uint256;
    string NFT_URI;
    string public extension = ".json";
    string public hidden_URI;
    uint256 public cost = 0.01 ether; // Cost per NFT
    uint32 public nftPerAddressLimit = 3; // Whitelist or presale max mint amount per address
    uint32 public maximum_supply = 10000; // Max number of NFTs
    ...
    bool public onlyWhitelisted = true; // Presale or public sale
    ...
    function mint(uint256 _mintcount) public payable {
        ...
        if (msg.sender != owner()) {
            if (onlyWhitelisted == true) {
                ...
                require(
                    ownerMintedCount + _mintcount <= nftPerAddressLimit,
                    "Max NFT per address reached. You cannot mint more."
                );
            }
    ...
    // Function to check whether the user is whitelisted or not
    ...
    function setNFTPerAdressLimit(uint32 _limit) public onlyOwner {
