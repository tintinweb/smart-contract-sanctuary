...
contract PoolService is Ownable {
    ...
    constructor(address addressRepository, address vault, bool isHighRisk) public {
        ...
        _PositionRepository = PositionRepository(_addressRepository.getPositionRepository());
        _priceRepository = IPriceRepository(_addressRepository.getPriceRepository());
    ...
    modifier onlyLiquidatePosition(address holder) {
        require(_liquidatePositionList[msg.sender], "Allowed for who can liquidate position only");
        require(getStatusLiquidate(holder), "Position state isn't for liquidation");
        _;
    }
    ...
    function allowTokenForTrading(address token, address priceFeedContract) external onlyOwner {
        ...
        _vaultService.approveOnUniswap(token, _vaultService.getMainRouterName());
    ...
    function allowRouterForTrading(string memory router) external onlyOwner {
        address routerAddress = _addressRepository.getRouter(router);
        for (uint256 i = 0; i < _allowedTokensList.length; i++) {
            if (IERC20_short(_allowedTokensList[i]).allowance(address(_vaultService), routerAddress) == 0) {
                _vaultService.approveOnUniswap(_allowedTokensList[i], router);
            }
        }
    }
    ...
    function liquidatePosition(address holder) external onlyLiquidatePosition(holder) {
    ...
    function _closePosition(address holder, address liquidator) internal {
        ...
        if (totalBalanceInVaultTokens > backToVault) {
            ...
            backToVault = totalBalanceInVaultTokens;
        }
            ...
            if (addr != _underlyingTokenAddress) {
                if (amount > 0) {
                    ...
                    _vaultService.swap(
                        ...
                        path,
                        deadline,
                        _vaultService.getMainRouterName()
                    );
                }
            } else {
                underlying = amount;
    ...
    function calcPositionBalance(address holder) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < tokensCount; i++) {
            ...
            uint256 tokenValueInVaultCurrency = _priceRepository.getAmountOut(addr, _underlyingTokenAddress, amount);
...
contract VaultService is PriceModel, Ownable {
    ...
    // Main router name
    string private _mainRouterName;
    ...
    constructor(address addressRepository,
        ...
        address gToken,
        string memory mainRouterName) public {
        _admin = msg.sender;
        _profit = 0;
        _balanceLiquidity = 0;
        _rate = S_1;
        ...
        _mainRouterName = mainRouterName;

        approveOnUniswap(_underlyingTokenAddress, _mainRouterName);
        _updateCumulativeIndex(S_1);
    }

    function setMainRouterName(string memory name) external onlyOwner {
        _mainRouterName = name;
    }

    function approveOnUniswap(address token, string memory router) public onlyOwner {
        address routerAddress = _addressRepository.getRouter(router);
        ERC20(token).approve(routerAddress, MAX_INT);
    }
    ...
    function removeLiquidity(uint256 amount) external {
        ...
        if (withdrawAmount > amount) {
            apy = withdrawAmount.sub(amount).mul(S_1).mul(100).div(amount);
        }
        ...
        emit RemoveLiquidity(msg.sender, amount, apy);
    }
    ...
    function getAPY(address holder) external view returns (uint256) {
        if (_balancesLiquidity[holder] == 0) {
            ...
            uint256 currentBalance = _dieselToken.balanceOf(holder).mul(_rate).div(S_1);
            if (currentBalance <= _balancesLiquidity[holder]) {
                return 0;
            } else {
                return currentBalance.sub(_balancesLiquidity[holder]).mul(S_1).mul(100).div(_balancesLiquidity[holder]);
            }
        }
    }
    ...
    function getMainRouterName() external view returns (string memory) {
        return _mainRouterName;
    }
}
