...
contract Demothree is Context, Ownable, IERC20 {

	using SafeMath for uint256;
    using Address for address;
    string private _name = "Demothree";
    string private _symbol = "DEMOTHREE";
    ...
    function _getValues(uint256 amount) private returns (uint256) {
        ...
        uint256 teamTax = _extractTeamFund(amount);
    	uint256 burnableFundTax = _extractBurnableFund(amount);
    	uint256 businessTax = operationalTax.add(burnableFundTax).add(ripeBananasTaxFee).add(charityTax).add(teamTax);
    ...
    function _extractTeamFund(uint256 amount) private returns (uint256) {
    	uint256 teamFundContribution = _getExtractableFund(amount, teamFundRate);
    	teamFund = teamFund.add(teamFundContribution);
    	_balance[address(this)] = _balance[address(this)].add(teamFundContribution);
    	return teamFundContribution;
    }
