...
// 
contract Cigareth is ERC20Burnable, Ownable {
    ...
    //Mapping of Ape to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    //Mapping of Ape to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to Ape
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("Cigareth", "CIGS") {}

    function setApesAddress(address _apesAddress) public onlyOwner {
        apesAddress = _apesAddress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(
            stakerToTokenIds[msg.sender].length + tokenIds.length <=
                MAX_WALLET_STAKED,
            "Must have less than 31 Apes staked!"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(apesAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(apesAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
