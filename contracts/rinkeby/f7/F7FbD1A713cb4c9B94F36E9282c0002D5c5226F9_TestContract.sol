...
contract TestContract is Ownable {
    ...
    uint256 public recipientsPerTx;

    event MultisentTokens(address indexed token, uint256 amount);
    event MultisentNative(uint256 amount);

    function multisendTokens(address token, address[] calldata recipients, uint256[] calldata amounts) external {
        require(recipients.length <= getRecipientsPerTx(), "Too much recipients in one transaction");
        require(recipients.length == amounts.length, "Amount of recipients and amounts not equal");
        ...
        emit MultisentTokens(token, totalSent);
    }
    ...
    function multisendNative(address[] calldata recipients, uint256[] calldata amounts) external payable {
        ...
        emit MultisentNative(msg.value);
    }

    function updateRecipientsPerTx(uint256 value) external onlyOwner {
        recipientsPerTx = value;
    }

    function getRecipientsPerTx() public view returns (uint256) {
        return recipientsPerTx;
    }
