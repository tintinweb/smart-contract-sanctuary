...
// Zora Auction House Contract Interface
interface IAuctionHouse {
    ...
    /**
     * @notice Omitted setAuctionReservePrice(), createBid(), and endAuction()
     * setAuctionReservePrice(): in interest of gas. cancelAuction() if needed.
     * createBid(): if outbid, any returned funds would be distributed to splits.
     * endAuction(): any address can call endAuction.
     */
 ...
 * added Zora minting & AH functionality
 * and OpenZeppelin's Ownable & IERC721Receiver
 */
contract SplitProxy is SplitStorage, IERC721Receiver {
    ...
    function mintToAuction(
        ...
        require(msg.sender == owner());
        require(auctionCurrency == 0x0000000000000000000000000000000000000000 || auctionCurrency == wethAddress);
        IMedia(zoraMedia).mint(mediaData, bidShares);
        IAuctionHouse(zoraAH).createAuction(_tokenId, zoraMedia, duration, reservePrice, payable (address(0)), 0, auctionCurrency);
    }

    function cancelAuction(uint256 auctionId) external {
        require(msg.sender == owner());
        IAuctionHouse(zoraAH).cancelAuction(auctionId);
    }
    ...
    function safeTransferNFT(
        ...
        require(msg.sender == owner());
        IMedia(tokenContract).safeTransferFrom(address(this), newOwner, tokenId);
    }
...
contract SplitStorage {
    ...
    address public immutable zoraAH = 0xc778417E063141139Fce010982780140Aa0cD5Ab; // rinkeby
}
