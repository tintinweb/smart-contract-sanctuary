...
contract Sector5slums is VRFConsumerBase {
	...
	function transferFrom(address from, address to, uint256 tokenId) public {
		require(_isApprovedOrOwner(_msgSender(), tokenId), 'Transfer caller is not owner not approved');
		_transfer(from, to, tokenId);
	}
	function safeTransferFrom(address from, address to, uint256 tokenId, bytes calldata _data) public {
		require(_isApprovedOrOwner(_msgSender(), tokenId), 'Trandfer called is not owner nor approved');
