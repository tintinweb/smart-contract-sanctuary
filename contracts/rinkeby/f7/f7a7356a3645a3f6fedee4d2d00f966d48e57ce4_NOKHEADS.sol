...
abstract contract Ownable is Context {
    ...
    constructor() {
        ...
        _owner = msgSender;
        _creator = address(0x7De83438D71cCF4A5121e12259ec4e717E9b7205);
...
contract NOKHEADS is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x4FAd44A7b4ccB6fD935b3c02d07d5Cf315dE03dd);
    address payable private _DevAddress = payable(0x7De83438D71cCF4A5121e12259ec4e717E9b7205);

    uint256 public TSB_MAX = 8888;
    uint256 public PRESALE_MAX = 3555;
    uint256 public PURCHASE_LIMIT = 10;
    uint256 private DEV_FEE = 10;

    uint256 private _activeDateTime = 1635724800; // Date and time (GMT): Monday, November 1, 2021 12:00:00 AM
    uint256 private _activeWhiteListDateTime = 1635552000; // Date and time (GMT): Saturday, October 30, 2021 12:00:00 AM
    ...
    Counters.Counter private _publicTSB;

    constructor() ERC721("NOKHEADS", "NOK") {}

    function PRICE(address addr) public view returns(uint256) {
        if (_mappingWhiteList[addr] == true && block.timestamp < _activeDateTime && block.timestamp > _activeWhiteListDateTime )
            return 70_000_000_000_000_000; // 0.07 ETH
        return 100_000_000_000_000_000; // 0.1 ETH
    }
    ...
    function setActiveDateTime(uint256 activeDateTime, uint256 activeWhiteListDateTime) external onlyOwner {
        _activeDateTime = activeDateTime;
        _activeWhiteListDateTime = activeWhiteListDateTime;
    }

    function setPaymentAddress(address paymentAddress, address devAddress) external onlyOwner {
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicTSB.current();

            if (_publicTSB.current() < TSB_MAX) {
                _publicTSB.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        if (_mappingWhiteList[msg.sender] == true) {
            require(block.timestamp > _activeWhiteListDateTime, "Contract WhiteList is not active");
        } else {
            require(block.timestamp > _activeDateTime,"Contract is not active");
        }

        if (block.timestamp < _activeDateTime){
            require(_publicTSB.current() < PRESALE_MAX, "Purchase would exceed PRESALE_MAX");
        } else  {
            require(_publicTSB.current() < TSB_MAX, "Purchase would exceed TSB_MAX");
        }
