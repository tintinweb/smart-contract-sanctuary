...
contract GenesisVault is TokenVault, ContractGuard {
    ...
    function setWeeklyEmission(uint _newWeeklyEmissions) onlyOperator public {
        weeklyEmissions = _newWeeklyEmissions;
    }

    function setVariableReduction(uint _newVariableReduction) onlyOperator public {
        variableReduction = _newVariableReduction;
    }
    ...
    function getShareTokenPrice() public view returns (uint256) {
        uint256 totalLPTokensValue = (totalSupply().mul(2) + totalMultipliedWBTCTokens).mul(getStakingTokenPrice());
        uint256 shareTokenPrice = totalLPTokensValue.div(weeklyEmissions).div(variableReduction).div(1e18);
    ...
    function beginGenesis() onlyOperator onlyOnce public {
        ...
        generated = true; //MDW - moved this below the require so we could rerun until someone staked
        ...
        //TODO - are we using uniswap to start?  
