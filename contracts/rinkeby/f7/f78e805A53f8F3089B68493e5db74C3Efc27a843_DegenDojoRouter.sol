...
contract DegenDojoRouter is VRFConsumerBase {
    ...
    mapping(address => bytes32) private AddressToRequest;
    event requestTradeWithERC20(bytes32 indexed requestID, address user);
    ...
    /**
     * Initiate a trade with ETH
     */

    /**
     * Initiates a trade with ERC20
     */
    function initiateTradeWithERC20(ERC20 token) public {
        //require that the current address is not in a trade
        require(AddressToRequest[address(msg.sender)] == 0);
        //get the request ID for trade initation
        bytes32 requestId = requestRandomness(keyhash, fee);
        //assign the requestID to the sender
        AddressToRequest[address(msg.sender)] = requestId;
        //collect the given token from user

        //unsure if emit is required?
        emit requestTradeWithERC20(requestId, msg.sender);
    }
