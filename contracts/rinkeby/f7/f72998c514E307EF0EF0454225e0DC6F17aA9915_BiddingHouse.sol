...
contract BiddingHouse {
    ...
    constructor(
        ...
        uint256 numSlots,
        Nft[] memory initialWinners
    ) {
        owner = _owner;
        epochBoundary = startingEpochBoundary;
        EPOCH_INTERVAL_SECONDS = epochInterval;
        NUM_SLOTS = numSlots;
        for (uint256 i = 0; i < numSlots && i < initialWinners.length; i++) {
            winners.push(
                Bid({value: 0, sender: owner, nft: initialWinners[i]})
            );
        }
    }
    ...
    function getCandidates() public view returns (Bid[] memory _candidates) {
        if (!isCurrentEpochOver()) {
