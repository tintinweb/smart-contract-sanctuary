...
// BoringLionsCo is a community-centered enterprise focussed on preserving our research about the emerging reports that several banana species have begun exhibiting strange characteristics following the recent worldwide pandemic.
...
contract LionKingz is ERC721, Ownable {
    using SafeMath for uint256;

    string public LION_PROVENANCE = ""; // IPFS URL WILL BE ADDED WHEN BANANAS ARE ALL SOLD OUT

    string public LICENSE_TEXT = ""; // IT IS WHAT IT SAYS

    bool licenseLocked = false; // TEAM CAN'T EDIT THE LICENSE AFTER THIS GETS TRUE

    uint256 public constant lionPrice = 70000000000000000; // 0.070 ETH

    uint256 public constant maxLionPurchase = 20;

    uint256 public constant MAX_LIONS = 1000;

    bool public saleIsActive = false;

    mapping(uint256 => string) public lionNames;

    // Reserve 200 Lions for team - Giveaways/Prizes etc
    uint256 public lionReserve = 200;

    event lionNameChange(address _by, uint256 _tokenId, string _name);

    event licenseisLocked(string _licenseText);

    constructor() ERC721("Lion Kingz", "KING") {}
    ...
    function changeLionName(uint256 _tokenId, string memory _name) public {
        require(
            ownerOf(_tokenId) == msg.sender,
            "Hey, your wallet doesn't own this lion!"
        );
        require(
            sha256(bytes(_name)) != sha256(bytes(lionNames[_tokenId])),
            "New name is same as the current one"
        );
        lionNames[_tokenId] = _name;

        emit lionNameChange(msg.sender, _tokenId, _name);
    }

    function viewLionName(uint256 _tokenId)
        public
        view
        returns (string memory)
    {
        require(_tokenId < totalSupply(), "Choose a lion within range");
        return lionNames[_tokenId];
    }

    // GET ALL BANANAS OF A WALLET AS AN ARRAY OF STRINGS. WOULD BE BETTER MAYBE IF IT RETURNED A STRUCT WITH ID-NAME MATCH
    function lionNamesOfOwner(address _owner)
        external
        view
        returns (string[] memory)
    {
        ...
        if (tokenCount == 0) {
            ...
            uint256 index;
            for (index = 0; index < tokenCount; index++) {
                result[index] = lionNames[tokenOfOwnerByIndex(_owner, index)];
            }
            return result;
        }
    }
