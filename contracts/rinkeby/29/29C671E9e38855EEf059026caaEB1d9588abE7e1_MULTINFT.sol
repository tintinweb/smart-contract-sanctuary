...
contract MULTINFT is ERC1155, Ownable{
    ...
    string baseUrl = "https://gateway.pinata.cloud/ipfs/QmPcLoau3aewhCydsA4jZrEBQiM6rjh4bXjyvvsqLFb1pM";
    ...
    event statusTransfer(bool status,uint256[] ids);

    constructor(address _token) ERC1155(string(abi.encodePacked(baseUrl,"/{id}.json"))) {
    // constructor(address _token) ERC1155("https://gateway.pinata.cloud/ipfs/QmPcLoau3aewhCydsA4jZrEBQiM6rjh4bXjyvvsqLFb1pM/{id}.json") {
    ...
    function transferItemFromBox(
        uint256 amount
    )public {
        uint256 money = token.balanceOf(msg.sender);
        ...
        for(uint i=0;i<amount;i++){
            ...
            _ids[i] = (nftIndex[_rand]);
            _amount[i] = (nftAmount[_rand]);
    ...
    function randomOfBox() public view returns (uint256){
        ...
        uint256 _rand =  uint256(keccak256(abi.encodePacked(block.difficulty,block.timestamp,availableBoxTotal))).mod(availableBoxTotal);

        while(_status){
            _total = _total.add(availableBox[_i]);
            if(_rand <= _total){
                _status = false;
            }else{
                _i = _i.add(1);
