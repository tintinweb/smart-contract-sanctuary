...
interface INFTToken is IERC721{
    ...
    function setOpenTransfer(bool isOpen) external;
}
...
import "../interfaces/tokens/INFTToken.sol";

contract NFTToken is INFTToken, ERC721, Managed {
    ...
    constructor(
        ...
        bool infinity,
        uint256 supply
    ) ERC721(name, symbol) Managed(management) {
    ...
    function getIds(
        ...
        if (to > _nftsByUser[_owner].length()) {
            to = _nftsByUser[_owner].length();
        }

        result = new uint256[](to - _offset);

        for (uint256 i = _offset; i < to; i++) {
            result[i - _offset] = _nftsByUser[_owner].at(i);
        }
    }
    ...
    function mint(address recipient)
        external
        override
        requirePermission(CAN_MINT_NFT)
    {
        count++;
        require(count < totalSupply, "NFT: the coins ran out");
        _tokenIds.increment();
        _mint(recipient, _tokenIds.current());
        require(
            _nftsByUser[recipient].add(_tokenIds.current()),
            "NFT: token id can't found"
        );
    }
    ...
    function burn(uint256 tokenId)
        external
        override
        requirePermission(CAN_BURN_NFT)
    {
        count--;
        require(
            _nftsByUser[ownerOf(tokenId)].remove(tokenId),
            "NFT: token id can't found"
        );
        _burn(tokenId);
    }

    function setOpenTransfer(bool isOpen)
        external
        override
    ...
    function _beforeTokenTransfer(
        ...
        require(
            hasPermission(msg.sender, CAN_TRANSFER_NFT) || isTransferOpen,
            "ERC721: you don't have permission to transfer"
        );
    }
}
