    ...
    EverlensAccessControl
{
        ...
        isValidOrderId(orderId)
    {
        ...
        (bool feeSent, ) = feeAddress.call{value: currFees}("");
        (bool amtSent, ) = currAuctionOrder.seller.call{
            value: currAuctionOrder.price
        }("");
        require(feeSent && amtSent, "Failed to send Ether");
    ...
    function createUnlimitedAuction(uint256 tokenId, uint256 minAuctionPrice)
        external
        nonReentrant
        whenNotPaused
    {
        ...
        isValidOrderId(orderId)
    {
        ...
        (bool lastBidReturn, ) = currBidDetails.bidderAddr.call{
            value: currBidDetails.bidAmount
        }("");
        require(lastBidReturn, "Error in last bid return amount");
        ...
        OnlySeller(orderId)
    {
        ...
        uint256 feeAmt = (currBidDetails.bidAmount * feePercent) /
            (10000 + feePercent);
        uint256 amtToTransfer = currBidDetails.bidAmount - feeAmt;

        (bool feeTransfer, ) = feeAddress.call{value: feeAmt}("");
        (bool sellerAmtTransfer, ) = currAuctionOrder.seller.call{
            value: amtToTransfer
        }("");

        require(
            feeTransfer && sellerAmtTransfer,
            "Error in sending amount to seller or feesAddress"
        );
        ...
        OnlySeller(orderId)
    {
        AuctionOrder memory currAuctionOrder = auctionOrders[orderId];
        require(
            currAuctionOrder.seller != address(0) && currAuctionOrder.tokenId != 0,
        ...
        //Unlimited Auction cancel
            ...
            (bool returnBidAmt, ) = currBidDetails.bidderAddr.call{
                value: currBidDetails.bidAmount
            }("");
            require(returnBidAmt, "Return the bid amount to the bidder");
        }
