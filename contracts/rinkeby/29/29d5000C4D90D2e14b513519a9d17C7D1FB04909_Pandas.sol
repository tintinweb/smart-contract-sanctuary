...
contract Pandas is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAXPANDAS = 30;
    uint256 public maxPandasPurchase = 5;
    uint256 public reservedPandas = 5;
    uint256 public reservedPandasCustom = 5;
    ...
    constructor(address _proxyRegistryAddress, string memory baseURI) ERC721("32px Pandas FINAL", "PANDAS") {
        setBaseURI(baseURI);
        isSaleActive = false;
    ...
    function mintPanda(uint256 numberOfPandas) public payable {
        ...
        require(numberOfPandas >= 0 && numberOfPandas <= maxPandasPurchase,
            "You can only mint 5 Pandas at a time!");
        require(totalSupply().add(numberOfPandas) <= MAXPANDAS - reservedPandas - reservedPandasCustom,
        ...
        for (uint256 i = 0; i < numberOfPandas; i++){
            ...
            if (totalSupply() < MAXPANDAS) {
                _safeMint(msg.sender, tokenNr+1);
            }
        }
    }
    ...
    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= reservedPandas, "Exceeds reserved Panda supply" );
