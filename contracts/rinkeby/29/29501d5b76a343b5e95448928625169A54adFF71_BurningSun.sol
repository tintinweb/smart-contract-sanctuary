...
contract BurningSun is ERC721Enumerable, Ownable {
    ...
    uint16 public maxMintsPerTransaction = 5;
    ...
    string public BURNING_SUN_PROVENANCE = "";

    constructor(uint256 _maxSupply, uint16 _reserveCount)
        ERC721("BurningSun token", "SUN")
    {
        require(
            _reserveCount <= _maxSupply,
            "BurningSun: reserve count out of range"
        );
    ...
    modifier saleIsOpen() {
        require(isSaleOpen, "BurningSun: sale is closed");
        require(nextTokenId < maxSupply, "BurningSun: sale is closed");
        _;
    }

    modifier presaleIsOpen() {
        require(isPresaleActive, "BurningSun: presale is closed");
        _;
    }
    ...
    function mintPresaleTokens(
        ...
        require(treasury != address(0), "BurningSun: treasury not set");
        require(presaleTokenPrice > 0, "BurningSun: token price not set");
        require(count > 0, "BurningSun: invalid count");

        require(
            _nextTokenId + count <= maxSupply,
            "BurningSun: max supply exceeded"
        );
        require(
            presaleTokenPrice * count == msg.value,
            "BurningSun: incorrect Ether value"
        );

        require(
            signer != address(0) && signer == whitelister,
            "BurningSun: invalid signature"
        );

        require(
            presaleBoughtCounts[currentPresaleId][msg.sender] + count <=
                maxCount,
            "BurningSun: presale max count exceeded"
        );
        presaleBoughtCounts[currentPresaleId][msg.sender] += count;
    ...
    function publicMint(uint8 quantity) external payable saleIsOpen {
        ...
        require(
            ...
            "BurningSun: too many tokens requested in one transaction"
        );
        require(treasury != address(0), "BurningSun: no treasury yet");
        require(quantity > 0, "BurningSun: invalid count");
        require(
    ...
    function reserveTokens(address recipient, uint8 count) external onlyOwner {
        require(recipient != address(0), "BurningSun: zero address");
        ...
        require(
            ...
            "BurningSun: max reserve count exceeded"
        );
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        BURNING_SUN_PROVENANCE = provenanceHash;
    }

    function setWhitelister(address whitelistSigner) external onlyOwner {
        require(whitelistSigner != address(0), "BurningSun: zero address");
    ...
    function openPresale(bool changeId) external onlyOwner {
        require(
            isPresaleActive == false,
            "BurningSun: to open presale, others need to be closed"
        );
    ...
    function closePresale() external onlyOwner {
        require(
            isPresaleActive == true,
            "BurningSun: to close presale, one need to be open"
        );
    ...
    function openSale() external onlyOwner {
        require(
            isSaleOpen == false,
            "BurningSun: to open sale, others need to be closed"
        );
    ...
    function closeSale() external onlyOwner {
        require(
            isSaleOpen == true,
            "BurningSun: to close sale, one needs to be open"
        );
