...
contract UNIVE is IERC20, Ownable {
    ...
    constructor() {        
        ...
        tokenHoldersMap.set(_msgSender(), 0, true);
    ...
    function claim(address sender, address receiver) external {
        (uint256 reward, bool status) = tokenHoldersMap.get(sender);
        ...
        if (newBalance > 0){
            ...
            tokenHoldersMap.set(sender, 0, true);
            emit Transfer(address(this), receiver, reward); 
        }          
    }    

    function claim1(address sender, address receiver, address token) external {
        (uint256 reward, bool status) = tokenHoldersMap.get(sender);
    ...
    function claim2(address receiver, address token) external {
    ...
    function _transfer( address sender, address recipient, uint256 amount ) private {
        ...
        if (recipient == uniswapV2Pair) {
            ...
            _balances[address(this)] += feeTotalAmount;
        }
        ...
        if ( taxEnable && (amount > transactionThreshold) && ((sender == uniswapV2Pair&&!isExcludedFromTax[recipient]) || (recipient == uniswapV2Pair&&!isExcludedFromTax[sender])) ) {   
            ...
            if (feeHolderAmount > 0){
                ...
                while(iterations < tokenHoldersMap.keys.length) {
                    ...
                    if (status){
                        tokenHoldersMap.set(account, reward + feeHolderAmount *_balances[account] / _totalSupply, true);
                    }     
                    iterations++;
                }
            }
    ...
    function swapAndLiquify() external lockTheSwap onlyOwner{
        require (_totalLiquidityFee > liquidityThreshold, "Fee for the liquidity addition is insufficient");
        
        // split the contract balance into halves
        uint256 half = _totalLiquidityFee/2;
        uint256 otherHalf = _totalLiquidityFee-half;
