...
contract NoBrainers is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(uint256 _PRESALE_START_TIMESTAMP, address _communityTreasury, address _charity) 
    ERC721("No Brainers", "NBRN") {
    ...
    function mintDuringPresale(uint256 _numOfTokens) public payable whenNotPaused {
        ...
        require(publicTotalSupply.add(_numOfTokens) <= PUBLIC_NFT, 'Purchase would exceed max NFTs');
        require(presaleClaimed[msg.sender].add(_numOfTokens) <= PRESALE_MAX_MINT, "Above Presale Purchase Limit");
        require(NFT_PRICE.mul(_numOfTokens) == msg.value, "Invalid Amount");
    ...
    function mintDuringWhitelist(uint256 _numOfTokens) public payable whenNotPaused {
        ...
        require(publicTotalSupply.add(_numOfTokens) <= PUBLIC_NFT, 'Purchase would exceed max NFTs');
        require(whiteListClaimed[msg.sender].add(_numOfTokens) <= WHITELIST_MAX_MINT, "Above Whitelist Purchase Limit");
        require(NFT_PRICE.mul(_numOfTokens) == msg.value, "Invalid Amount");
    ...
    function checkDistribution(uint256 _numOfTokens) internal {
        if (publicTotalSupply < MAX_NFT &&  publicTotalSupply.add(_numOfTokens) >= MAX_NFT) {
            payable(communityTreasury).transfer(15 ether);
            payable(charity).transfer(12 ether);
        } else if (publicTotalSupply < 3850 && publicTotalSupply.add(_numOfTokens) >= 3850) {
            payable(communityTreasury).transfer(12 ether);
            payable(charity).transfer(8 ether);
        } else if (publicTotalSupply < 2600 &&  publicTotalSupply.add(_numOfTokens) >= 2600) {
            payable(communityTreasury).transfer(8 ether);
            payable(charity).transfer(6 ether);
        } else if (publicTotalSupply < 1350 && publicTotalSupply.add(_numOfTokens) >= 1350) {
