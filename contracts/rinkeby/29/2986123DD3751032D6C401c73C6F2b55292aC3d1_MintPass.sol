...
contract MintPass is ERC1155, ERC1155Burnable, ERC1155Supply, ERC1155Pausable, Ownable {
  ...
  uint256 public constant total = 1050;
  address private _redeemFrom;
  bytes32 private _rootHash;

  mapping(address => uint256) public _claimed;

  event TokensClaimed(address sender, uint256 amount, uint256 claimed);
  ...
  function burnFromRedeem(address account, uint256 count) external onlyRedeemer {
    _burn(account, 0, count);
  }
  ...
  function mintPassWithAllocation(
    ...
    if (!MerkleProof.verify(proof, _rootHash, keccak256(abi.encode(msg.sender, allocated)))) {
      revert InvalidProof();
    }

    if (totalSupply(0) + count > total) {
      revert InsufficientSupply(total, totalSupply(0), count);
    }

    uint256 claim = _claimed[msg.sender];
    uint256 free = (claim < allocated) ? allocated - claim : 0;
    uint256 paid = count - free;

    _claimed[msg.sender] += free;
    emit TokensClaimed(msg.sender, count, free);
  ...
  function mintPass(uint256 count) external payable {
