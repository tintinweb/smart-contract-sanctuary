...
contract RacerKeySale is IERC1155Receiver, AccessControlEnumerable {
    ...
    // Manager
    function changeSalePrice(uint256 price) public onlyRole(MANAGER_ROLE) {
        salePrice = price;
    }

    function changeMaxPurchaseQuantity(uint256 quantity)
        public
    ...
    function purchaseKeys(uint256 quantity) public payable {
        require(
            _keysPurchased[msg.sender] < maxPurchaseQuantity,
            "Max quantity already purchased"
        );
        uint256 availableQty = SafeMath.sub(
            maxPurchaseQuantity,
        ...
        require(availableQty <= maxPurchaseQuantity, "Max quantity exceeded");
        require(
    ...
    Ownable
{
    ...
    bool public keysRedeemed;
    address[] private _keyholders;

    constructor()
        ERC1155("https://api.luckymachines.io/api/metadata/racer-key")
    {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    ...
    function setKeysRedeemed() public onlyRole(REDEEMER_ROLE) {
        keysRedeemed = true;
        if (paused()) {
            _unpause();
        }
