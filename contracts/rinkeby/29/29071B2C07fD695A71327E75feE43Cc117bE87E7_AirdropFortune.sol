...
contract AirdropFortune is Divine {

  using SafeMath for uint256;
  using SafeERC20 for IERC20;

    bool public airdropOpen;
    bool public convertOpen;
    uint public floor = 2*1e15; 
    uint public airdropAmount = 20;
    uint public airdropAmountLimit = 8;
    address public FTN;
    mapping( address => uint ) public airDropTime;
    
    constructor() Divine("aFortune", "aFTN", 0) {
    }

    /**
     *  @notice this contract is used for airdropping, aFTN can be converted to FTN 1:1 after presaling
     *  
     */
    function Airdrop(address _refAdd) payable external {
      require( airdropOpen == true ,"aridrop is not open");
      require( airDropTime[msg.sender] == 0 ,"already took air drop");
      require( msg.value >= floor );
      // give out token for airdrop account 
      _mint( msg.sender, airdropAmount ); 
      airDropTime[msg.sender] += 1;
      // give out token for ref account
      if ( _refAdd!=address(0) && _refAdd!=msg.sender && airDropTime[_refAdd]<airdropAmountLimit ){
        _mint( _refAdd, airdropAmount.div(2) );
        airDropTime[_refAdd] += 1 ;
      }
    }

    function setairdropAmount (uint _airdropAmount) external onlyOwner() {
      airdropAmount = _airdropAmount;
    }

    function setairdropAmountLimit (uint _airdropAmountLimit) external onlyOwner() {
    ...
    function toggleConvert() external onlyOwner() {
        convertOpen = !convertOpen;
    }

    function setFTNAddress(address _FTN)external onlyOwner() {
        FTN = _FTN;
    }

    /**
     *  @notice convertAFTNtoFTN function is used to convert aFTN to FTN with a ratio of 1:1 
     */
    function convertAFTNtoFTN(uint _amount) external {
      require( convertOpen == true ,"convert is not open");
      transferFrom(msg.sender,address(this),_amount);
      IERC20( FTN ).safeTransfer( msg.sender, _amount*1e9);
    }
