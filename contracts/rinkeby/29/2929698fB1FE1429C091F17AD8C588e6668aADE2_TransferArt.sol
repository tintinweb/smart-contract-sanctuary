...
contract TransferArt is ERC721Enumerable {
    ...
    constructor(address _manager) ERC721("Transfer Art", "Yoo") {
    }

    function mintFeeWei() public view returns(uint256){
        return _startMintFeeWei + (_nonce * 1e16);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        if(_transferHistory[tokenId].length < 100) {
                        ...
                        Base64.encode(
                            bytes(
                                abi.encodePacked(
                                    ...
                                    '", "description":"',
                                    'The image of this NFT is created by the first 100 transfers, populating a 10x10 grid of squares. A new colored square is added on each transfer. The `to` address of the transfer determines the color.',
                                    '", "image": "',
                                    'data:image/svg+xml;base64,',
                                    Base64.encode(bytes(generateSVG(_transferHistory[tokenId]))),
                                    '"}'
                                )
                            )
                        )
    ...
    function generateSVG(address[] memory transferHistory) private pure returns (string memory svg) {
        return string(
            abi.encodePacked(
                '<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 100" width="100" height="100" xml:space="preserve">',
                '<style type="text/css">',
                    '.s0{width:10px;height:10px;}',
                '</style>',
    ...
    function generateRect(address account, uint256 index) private pure returns (string memory) {
        string memory r = UintStrings.decimalString(uint8(keccak256(abi.encodePacked(account))[0]), 0, false);
        string memory g = UintStrings.decimalString(uint8(keccak256(abi.encodePacked(account))[1]), 0, false);
        string memory b = UintStrings.decimalString(uint8(keccak256(abi.encodePacked(account))[2]), 0, false);
        string memory a = UintStrings.decimalString(uint8(keccak256(abi.encodePacked(account))[3]), 2, false);

        return string(
            abi.encodePacked(
            '<rect x="',
            UintStrings.decimalString((index % 10) * 10, 0, false),
            '" y="',
            UintStrings.decimalString((index / 10) * 10, 0, false),
            '" class="s0" fill="rgba(',
            r,
