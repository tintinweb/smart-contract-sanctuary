...
contract ForkFactory is Ownable {
    ...
    // uint256 constant FORK_STATE_BEFORE_GENESIS = 0;
    // uint256 constant FORK_STATE_EDEN_GARDEN = 1;
    // uint256 constant FORK_STATE_EVANGELIZATION = 2;

    // uint256 constant NUMBER_OF_GENESIS_FORK = 2100;
    ...
    */
        ...
        onlyOwner
    {
        ...
        for (uint i = 0; i < to.length; i++) {
            require (addrIdMap[to[i]] == 0, "mint0: $to");

            uint256 tokenId = fork.mint(address(this), ipfsAddr[i]);
            idAddrMap[tokenId] = to[i];
            addrIdMap[to[i]] = tokenId;
            emit Minted(address(this), tokenId);
            /// @dev  Comment out the first three lines and use the following two to send Fork to the same address multiple times.

            // fork.safeTransferFrom(address(this), to[i], tokenId);
            // emit Claimed(to[i], tokenId);
        }
        return;     
    }
    ...
    // set ratio of 1720 FSM transferred to the savior
...
contract Fork is Ownable, ERC721, ERC721Burnable {
    ...
    /// @notice The tokenId is from one instead zero. ZERO means void.  
