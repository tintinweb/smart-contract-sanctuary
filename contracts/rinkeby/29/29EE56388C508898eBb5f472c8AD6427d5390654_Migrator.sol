...
contract Migrator is AccessControl {
    ...
    function verify(bytes32 hash, uint256[] calldata expireBlocks, bytes[] calldata sigs)
		internal
		view
		returns (bool)
	{
    ...
    function migrate(uint[] calldata amount, uint256[] calldata expireBlocks, uint8 migrationStatus, bytes[] calldata sigs)
        public 
    {
        ...
        bytes32 _hash = keccak256(abi.encodePacked(msg.sender, expireBlocks, migrationStatus, getChainID(), amount));
		require(verify(_hash.toEthSignedMessageHash(), expireBlocks, sigs), "Migrator::migrate: Invalid signatures");
    ...
    event Migrate(address userAddress, uint[] amount, uint8 migrationStatus);
