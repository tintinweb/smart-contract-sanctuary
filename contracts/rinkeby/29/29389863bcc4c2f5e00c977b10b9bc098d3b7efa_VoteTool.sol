...
contract VoteTool {
    ...
    Proposal[]  proposals;
    mapping(bytes32=>uint256) proposalMap;
    mapping(address => mapping(bytes32=>uint256)) userVoteMap;
    ...
    function vote(bytes32  _proposal,uint256 _n)public payable{
        require(_n>0,"zero");
        ...
        for(;userNum<nextNum;userNum++){
            amount = amount.add(numPower(userNum).mul(ONE_ETH));
        }
    ...
    function addProposal(bytes memory _proposal)public onlyOwner {
        bytes32 p = hash(_proposal);
        require(proposalMap[p] == 0,"exists");
        proposals.push(Proposal(p,0,0,1));
        proposalMap[p] = proposals.length ;
    }
    
    
    function expireProposal(bytes32 _proposal)public onlyOwner{
    ...
    function getProposal(uint256 _index)public view returns(bytes32,uint256,uint256,uint8){
        require(_index<proposals.length,"out");
        Proposal memory p = proposals[_index];
        return (p.name,p.voteCount,p.amount,p.status);
    }
    
    function getUserVoteNum(address _addr, bytes32 _proposal)public view returns(uint256){
