...
abstract contract ERC721JsonMetadata is ERC721 {
...
contract WIFnft is ERC721JsonMetadata, Ownable {
    ...
    enum TokenType {Bronze, Silver, Gold, Platinum, Diamond}
    uint256 private constant TOKEN_TYPE_COUNT = 5;
    ...
    mapping(TokenType=>SaleInfo) public saleProperties;

    constructor() ERC721(NAME, SYMBOL) {
        nextId.increment();
    }

    function sale(TokenType ttype, uint256 nftAmount) external payable {
    ...
    function setupSale(uint256[] calldata prices, uint256[] calldata caps) external onlyOwner {
        require((prices.length == caps.length) && (caps.length == TOKEN_TYPE_COUNT), "Wrong array lengths");
        for(uint256 i=0; i < TOKEN_TYPE_COUNT; i++){
            saleProperties[TokenType(i)] = SaleInfo({
                price: prices[i],
                minted: 0,
                cap: caps[i]
            });
