...
//  Treasury: planet wrapper
//
//    This contract implements an extremely simple wrapper for planets.
//    It allows owners of Azimuth planets points to deposit them and mint new WSTR (wrapped planet) tokens,
//    and in turn to redeem WPLT tokens for Azimuth planets.

contract Treasury is Context {
    // MODEL

    //  assets: planets currently held in this pool
    //  note: planets are 32 bit numbers and we always handle them as uint32.
    // stars were 16 bit.
    ...
    PlanetToken public planetToken = new PlanetToken(0);

    uint256 constant public ONE_PLANET = 1e18;

    // EVENTS

    event Deposit(
        uint32 indexed prefix,
        uint32 indexed planet,
        address sender
    );
    ...
    function getAllAssets()
        view
        external
        returns (uint32[] memory allAssets)
    {
        return assets;
    }
    ...
    //  deposit(planet): deposit a planet you own, receive a newly-minted wrapped planet token in exchange
    //
    function deposit(uint32 _planet) external
    {
        require(azimuth.getPointSize(_planet) == IAzimuth.Size.Planet, "Treasury: must be a planet");
        ...
        if (
            ...
            azimuth.canTransfer(_planet, _msgSender()) &&
            azimuth.getSpawnCount(_planet) == 0
        ) {
            // transfer ownership of the _star to :this contract
            // note: _star is uint16, ecliptic expects a uint32 point
            ecliptic.transferPoint(_planet, address(this), true);
        }
        ...
        else if (azimuth.canSpawnAs(azimuth.getPrefix(_planet), _msgSender())) {
        ...
        //
        planetToken.mint(_msgSender(), ONE_PLANET);
        emit Deposit(azimuth.getPrefix(_planet), _planet, _msgSender());
    }

    //  redeem(): burn one star token, receive ownership of the most recently deposited star in exchange
    //
    function redeem() external returns (uint32) {
        // must have sufficient balance
        require(planetToken.balanceOf(_msgSender()) >= ONE_PLANET, "Treasury: Not enough balance");
        ...
        uint32 _planet = assets[assets.length-1];
        assets.pop();

        // burn the tokens
        planetToken.ownerBurn(_msgSender(), ONE_PLANET);
        ...
        IEcliptic ecliptic = IEcliptic(azimuth.owner());
        // note: _star is uint16, ecliptic expects a uint32 point
        ecliptic.transferPoint(_planet, _msgSender(), true);

        emit Redeem(azimuth.getPrefix(_planet), _planet, _msgSender());
        return _planet;
    }
}
...
//  PlanetToken: ERC20-compatible fungible wrapped planet token
//
...
contract PlanetToken is Context, Ownable, ERC20 {
    constructor(
        uint256 initialSupply
    )
        Ownable()                   
        ERC20("WrappedPlanet", "WPLT")
    {
