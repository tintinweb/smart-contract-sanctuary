...
import "./erc-721/NFT.sol";

contract ERC721Factory is Ownable {

    event Create721(address);

    constructor() {
    }

    function createToken(string memory _name, string memory _symbol, string memory baseURI, uint salt) external {
        bytes memory bytecode = getCreationBytecode(_name, _symbol, baseURI);
        address tokenAddress = deploy(bytecode, salt);
        NFT token = NFT(address(tokenAddress));
        emit Create721(tokenAddress);
    }
    ...
    //adding constructor arguments to bytecode
    function getCreationBytecode(string memory _name, string memory _symbol, string memory baseURI) internal view returns (bytes memory) {
        return abi.encodePacked(type(NFT).creationCode, abi.encode(_name, _symbol, baseURI));
    }
