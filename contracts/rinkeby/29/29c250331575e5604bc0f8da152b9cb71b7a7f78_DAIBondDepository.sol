...
interface IStaking {
    function stake( uint amount_, address recipient_ ) external returns ( bool );
}
...
contract DAIBondDepository is Ownable {
    ...
    struct Terms {
        ...
        uint DAOShare; // vs bond payout, in hundreths of a %. i.e. 50 = 0.5%
        uint maxPayout; // vs circulating supply, in hundreths of a %. i.e. 50 = 0.5%
    }
    Terms public terms;
    ...
    function deposit( 
        ...
        uint price = bondPrice(); // DAI price of bond (for depositor info)

        require( maxPrice_ >= price, "Slippage limit: more than max price" ); // slippage protection

        uint value = amount_.mul( 10 ** IERC20( OHM ).decimals() ).div( 10 ** IERC20( DAI ).decimals() );
        ...
        // calculate profits
        uint daoProfit = payout.mul( terms.DAOShare ).div( 10000 );
        ...
        ITreasury( treasury ).deposit( amount_, DAI, profit );
         
        IERC20( OHM ).safeTransfer( DAO, daoProfit ); // Transfer dao profits
    ...
    /** 
        ...
        @return payout_ uint
     */ 
    function redeem( bool stake_ ) external returns ( uint ) {        
        ...
        if ( percentVested >= 10000 ) { // if fully vested, pay full amount & clear info
            ...
            return stakeOrSend( stake_, info.payoutRemaining );
        } else {
            ...
            return stakeOrSend( stake_, payout );
        }
    }

    /**
        @notice allow user to stake payout automatically if stake_ is true
        @param stake_ bool
        @param amount_ uint
        @return uint
     */
    function stakeOrSend( bool stake_, uint amount_ ) internal returns ( uint ) {
        if ( !stake_ ) {
            IERC20( OHM ).transfer( msg.sender, amount_ ); // send payout
        } else {
            IERC20( OHM ).approve( staking, amount_ );
            IStaking( staking ).stake( amount_, msg.sender ); // stake payout
        }
        return amount_;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        ...
        return circulatingOHM.mul( terms.maxPayout ).div( 10000 );
    }
    ...
    function bondPrice() public view returns ( uint _price ) {        
        _price = terms.controlVariable.mul( _calcDebtRatio() ).add( 1000000000 ).div( 1e7 );
        if ( _price < terms.minimumPrice ) {
            _price = terms.minimumPrice;
        }
    }
