...
contract BOOTY is IBOOTY, ERC20, Ownable, Pausable {
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public virtual override(ERC20, IBOOTY) disallowIfStateIsChanging returns (bool) {
    require(admins[_msgSender()] || lastWrite[sender] < block.number , "Only admins can call this");
  ...
  modifier disallowIfStateIsChanging() {
    require(admins[_msgSender()] || lastWrite[tx.origin] < block.number, "Only admins can call this");
    _;
  }
  ...
  function balanceOf(address account) public view virtual override disallowIfStateIsChanging returns (uint256) {
    require(admins[_msgSender()] || lastWrite[account] < block.number, "Only admins can call this");
    return super.balanceOf(account);
  }

  function transfer(address recipient, uint256 amount) public virtual override disallowIfStateIsChanging returns (bool) {
    require(admins[_msgSender()] || lastWrite[_msgSender()] < block.number, "Only admins can call this");
