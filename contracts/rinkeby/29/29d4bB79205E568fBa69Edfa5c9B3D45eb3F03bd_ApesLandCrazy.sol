  ...
  UUPSUpgradeable
{
  ...
  */
  ...
  mapping(uint256 => uint256) NftIDClaimed;
  //Special Reward Blocks
  uint256 specialStartBlockTime;
  uint256 specialEndBlockTime;
  ...
  function initialize() public initializer {
    ...
    aGoldAddress = address(0xf1108700Bb74bFFC7c2acA9664959BdC31316Be2);
  ...
  function setCollateral(address val) public onlyOwner {
    collateral_ = payable(address(val));
  }
  ...
  function unStackBuilding(uint256 genericType) public {
    ...
    uint256 maxSlots = addressToBuildingsType[sender][genericType].length * 3;
    uint256 freeSlots = maxSlots - apesByType;
    require(freeSlots > 2, "still apes on building");
  ...
  function unStackApes(uint256 genericType) public {
    ...
    uint256 nftId = addressToApesType[sender][genericType][addressToApesType[sender][genericType].length - 1];
  ...
  */
  function claimRewards(uint256 genericType) public {
    ...
    totalRewards += (currentTimestamp - apeStartFarmingTimestamp[addressToApesType[sender][genericType][totalApesByType - 1]])
      * apeTypeBaseRewards[genericType];
    // Add building profits and un-stack it if applies
    if (((totalApesByType - 1) % 3) == 0) {
      totalRewards += (currentTimestamp - apeStartFarmingTimestamp[addressToApesType[sender][genericType][totalApesByType - 1]])
        * buildingTypeBaseRewards[genericType];

      apeBuilding.transferFrom(
        address(this),
        sender,
        addressToBuildingsType[sender][genericType][addressToBuildingsType[sender][genericType].length - 1]
      );
      buildsAtAddress[addressToBuildingsType[sender][genericType][addressToBuildingsType[sender][genericType].length - 1]] = address(0x000);
      addressToBuildingsType[sender][genericType].pop();
    }
    // Un-stack ape
    apesInstance.transferFrom(
      address(this),
      sender,
      addressToApesType[sender][genericType][totalApesByType - 1]
    );
    apesAtAddress[addressToApesType[sender][genericType][totalApesByType - 1]] = address(0x000);
    addressToApesType[sender][genericType].pop();
    // adjust rewards
  ...
  function getApeAge(uint256 apeNftId) internal returns (uint256){
