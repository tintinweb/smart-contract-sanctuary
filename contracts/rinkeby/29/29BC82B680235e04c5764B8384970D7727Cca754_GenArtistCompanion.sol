...
contract GenArtistCompanion is ReentrancyGuard, ERC721Enumerable, Ownable {
    ...
    function claimByTokenIds(uint256 _projectId, uint256 _parentTokenId) external {
        require(projects[_projectId].isClaimActive, 'Claim must be active to mint tokens');
        require(parent.ownerOf(_parentTokenId) == msg.sender, "Must own parent token.");
        _internalClaim(msg.sender, _projectId, _parentTokenId);
    }
    ...
    function _internalClaim(address _to, uint256 _projectId, uint256 _parentTokenId) internal {
        uint256 tokenId = (_projectId * ONE_MILLION) + _parentTokenId;
