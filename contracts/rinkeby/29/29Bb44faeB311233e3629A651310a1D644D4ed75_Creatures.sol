...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // If both are set, concatenate the baseURI and tokenURI (via abi.encodePacked).в чате
        if (bytes(_tokenURI).length > 0) {
            return string(abi.encodePacked(base, _tokenURI));
        }
...
//v0.0.1
contract Creatures is ERC721URIStorage, Ownable {
    ...
    function setName(uint256 tokenId, string memory _name) external {
        ...
        if  (bytes(animals[tokenId].name).length == 0 ) {
