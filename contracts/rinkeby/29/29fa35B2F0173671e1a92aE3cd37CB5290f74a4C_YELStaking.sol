...
abstract contract OwnableStaking is OwnableData {
...
/**
 * This contract is made to reward YEL token holder
 * who are willing to stake their $YEL hence supporting
 * the jump start of popsicle.finance
 * 
 * The contract shall reward 110% of deposited YEL tokens
...
contract YELStaking is OwnableStaking {
    ...
    // total amount of YEL token that users deposited to this contract
    ...
    // Info of each user.
    struct UserInfo {
        uint256 depositedAmount; // How many YEL tokens the user has provided.
    ...
    modifier openDeposit() {
        require(openToDeposit, "YELToken_Staking: all deposits are closed");
        _;
    }
    ...
    // Deposit YEL tokens to YELTStaking.
    function deposit(uint256 _amount) external openDeposit {
        require(_amount > 0, "Insufficient balance"); //save some gas
        UserInfo storage user = userInfo[msg.sender];
        yelToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.depositedAmount += _amount;
        totalDeposited += _amount;
        emit Deposit(msg.sender, _amount);
    }
    
    // Withdraw YEL tokens from YELTStaking. SafeRewardTransfer gets triggered for pendingReward
    ...
    function withdraw() external {
        ...
        if (pendingTokenReward > 0) { // save some gas
            ...
            if(safeAmount > 0){
                ...
                yelToken.transfer(msg.sender, safeAmount);
            }
    ...
    function getSafeRewardTransferAmount(uint256 _amount) internal view returns(uint256) {
        uint256 rewardTokenBalance = yelToken.balanceOf(address(this));
