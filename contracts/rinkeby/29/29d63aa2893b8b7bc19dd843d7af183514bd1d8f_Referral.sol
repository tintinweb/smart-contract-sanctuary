...
contract Referral is Ownable {
  ...
  uint256 constant REFERRAL_COMMISION = 1 * (10 ** uint256(18));
  ...
  function recordReferral(address _user, address _referrer) public onlyAdmin {
    if (_user != address(0) && _referrer != address(0) && _user != _referrer && referrers[_user] == address(0)) {
      ...
      recordReferralCommission(_referrer, REFERRAL_COMMISION);
    }
  }

  function recordReferralCommission(address _referrer, uint256 _commission) public onlyAdmin {
    if (_referrer != address(0) && _commission > 0) {
      totalReferralCommissions[_referrer] += _commission;
      
      xDNA.transfer(_referrer, _commission);
