...
contract ERC721 is Context, ERC165, IERC721, IERC721Enumerable {
    ...
    /// @dev The Birth event is fired whenever a new plant comes into existence.
    event Birth(address owner, uint256 tokenId, uint256 plantId);

    /*** DATA TYPES ***/
    struct Plant {
        // id of the plant
        uint256 plantId;
        // The timestamp from the block when this Plant came into existence.
        uint64 birthTime;
    }

    /*** STORAGE ***/
    /// @dev An array containing the Plant struct for all Plants in existence. The ID
    ///  of each Plant is actually an index into this array. Note that ID 0 is a negaPlant,
    ///  the unPlant.
    Plant[] plants;
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, uint256 tokenId, uint256 plantId) internal virtual {
        _safeMint(to, tokenId, plantId, "");
    }
     ...
     */
    function _safeMint(address to,uint256 tokenId,uint256 plantId,bytes memory _data) internal virtual {
        _mintPlant(to, tokenId, plantId);
     ...
     * Emits a {Transfer} event.
     */
    function _mintPlant(address to,uint256 tokenId,uint256 plantId) internal virtual {
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(tokenId), "ERC721: token already minted");

        Plant memory _plant = Plant({plantId: plantId, birthTime: uint64(now)});

        plants.push(_plant);

        emit Birth(to, tokenId, _plant.plantId);
...
contract PlantValue is ERC721Pausable, AccessControl, Ownable, IPlantValue {
    ...
    uint256 public price = 100 * 10**2;
    address public CPCToken = 0x81014513e97a9C2506bb889dCE5bbbd681C99C35;
    bool public isBurn;
    uint256 public rate;
    ...
    uint256 private _reserved = 100000;


    constructor() public ERC721("Seed", "Seed") {
    ...
    function getRangeId() external view returns (uint256[] memory) {
        return rangeOfId;
    }
    ...
    function addPlantId(uint256[] calldata _plantId) external onlyOwner {
        for (uint256 i = 0; i < _plantId.length; i++) {
    ...
    function mint(uint256 num, address from, address to) external onlyOwner {
        uint256 supply = totalSupply();
        require(to != address(0));
        require(rangeOfId.length > 0, "INSUFFFICIENT PLANT ID");
        require( num > 0, "you can mint 0 plants");
        require( num <= 110, "You can mint maximum of 100 plants");
        require( supply + num < 110 - _reserved, "Exceeds maximum Plants spply");
        ...
        for(uint256 i = 0; i <= 110; i++) {
            ...
            _mintPlant(to, tokenId, supply + i);
            _remove(index);
            nextTokenId++;
            if (i == 110) {
                break;
    ...
    function createPlant(address _owner) external {
        require(rangeOfId.length > 0, "INSUFFFICIENT PLANT ID");
    ...
    function createBundlePlant(address _owner, uint256 _numberPlant)
        external
        override
        onlyBundle
    {
        for (uint256 i = 0; i < _numberPlant; i++) {
            uint256 tokenId = nextTokenId;
            uint256 index = _randomPlantId();
            uint256 plantId = rangeOfId[index];
    ...
    function createPlantFromFarm(address _owner) external override onlyFarm {
        require(rangeOfId.length > 0, "INSUFFFICIENT PLANT ID");
    ...
    function burnPlant(uint256 _tokenId) external {
        ...
        IERC20(CPCToken).transfer(msg.sender, amount);
    }
    ...
    function getPlant(uint256 _id)
        public
        view
        returns (uint256 birthTime, uint256 plantId)
    {
        Plant storage plant = plants[_id - 1];
    ...
    function _removeSeedFarm(uint256 _index) private {
    ...
    function getBalance() public view returns (uint256) {
        return IERC20(CPCToken).balanceOf(address(this));
    }

    function withdrawBalance() external onlyOwner {
        IERC20(CPCToken).transfer(owner(), getBalance());
    }
