...
contract WickedRouter {
    ...
    function balanceOf(address _token) external view returns (uint256) {
        return IBasicERC20(_token).balanceOf(address(this));
    }

    function _swap(uint[] memory amounts, address[] memory path, address _to) private returns (uint, uint, address) {
        uint amount0Out;
        uint amount1Out;
        address input;
        address output;
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            if (overlayUse) {
                overlayRouter.swap(PancakeLibrary.pairFor(factory, input, output), amount0Out, amount1Out);
            } else {
                IPancakePair(PancakeLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
            }
        }
        return (amount0Out, amount1Out, PancakeLibrary.pairFor(factory, input, output));
    }
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransfer(path[0], address(overlayRouter), amounts[0]);
        overlayRouter.firstTransfer(path[0], PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]);

        (uint amount0Out, uint amount1Out, address _pair) = _swap(amounts, path, address(this));

        if (amount0Out > 0) {
            overlayRouter.lastTransfer(IPancakePair(_pair).token0(), amount0Out);
        }
        if (amount1Out > 0) {
            overlayRouter.lastTransfer(IPancakePair(_pair).token1(), amount1Out);
        }
...
contract OverlayRouter {
    ...
    function firstTransfer(address token, address pair, uint firstAmount) external {
        require(msg.sender == owner);
        IBasicERC20(token).transfer(pair, firstAmount);
    }
    ...
    function lastTransfer(address _token, uint _amount) external {
        require(msg.sender == owner);
        IBasicERC20(_token).transfer(owner, _amount);
    }
