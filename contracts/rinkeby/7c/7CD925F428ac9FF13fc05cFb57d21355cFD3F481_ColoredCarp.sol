...
contract ColoredCarp is iColoredCarp, ERC721, ReentrancyGuard, Ownable {
    ...
    ERC721("Colored Carp", "CC")
    {
        require(__recipient != address(0), "ColoredCarp: Invalid address");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ColoredCarp: URI query for nonexistent token");
    ...
    function buy(uint256 tokenId) external override nonReentrant payable {
        require(_isOnSale, "ColoredCarp: Not on sale");
        require(msg.value == PRICE, "ColoredCarp: Invalid value");
        require(tokenId < _nextReservedTokenId, "ColoredCarp: Invalid id");
    ...
    function buyBundle(uint256[] memory tokenIdList) external override nonReentrant payable {
        ...
        require(_isOnSale, "ColoredCarp: Not on sale");
        require(msg.value == PRICE * count, "ColoredCarp: Invalid value");
        _remainingAmountForSale -= count;

        for (uint256 i; i < count; i++) {
            require(tokenIdList[i] < _nextReservedTokenId, "ColoredCarp: Invalid id");
    ...
    function updateBaseURI(string calldata newBaseURI) external override onlyOwner {
        require(!_isMetadataFroze, "ColoredCarp: Metadata is froze");
        __baseURI = newBaseURI;
    }

    function freezeMetadata() external override onlyOwner {
        require(!_isMetadataFroze, "ColoredCarp: Already froze");
