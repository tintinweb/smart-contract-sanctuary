    ...
    ReentrancyGuard
{
    ...
    function freeze(uint[] memory ids) external onlyOwner {
    ...
    function cappedByMode() public view returns(uint256) {
        ...
        if (getSaleMode() == 3) return maxSupply - totalPresaleMinted - maxReserve;
        return 0;
    }
...
contract SaleTrait is Ownable{
    ...
    function presaleSoldOut() public view returns(bool) {
        return totalPresaleMinted == presaleCapped;
    }

    function publicSaleSoldOut() public view returns(bool) {
        uint256 supplyWithoutReserve = maxSupply - maxReserve;
        uint256 mintedWithoutReserve = totalPublicMinted + totalPresaleMinted;
