...
contract Jigsaw1 is ERC721Enumerable, Ownable {
    ...
    function mint() external payable {
        ...
        // console.log('=> sc:mint:mintFee             :', mintFee);
        // console.log('=> sc:mint:price               :', price);
        // console.log('=> sc:mint:gasprice            :', tx.gasprice);
        // console.log('=> sc:mint:estimatedGasForMint :', estimatedGasForMint);
     ...
     */
    function safeMint(address[] memory _to, uint256[] memory _nftCount) external payable onlyOwner {
        require(block.timestamp >= fiatSaleBegin || privateCounter >= jigsawPresale,
        'ER: Fiat sale is not started'
        );
        require(block.timestamp < publicSaleBegin,'ER: Fiat sale is currently closed');
        ...
        for (uint256 i = 0; i < totalAccounts; i++) {
            ...
            for (uint256 j = 0; j < totalTokensToMint; j++) {
                if (hasMinted[to]<MAX_MINTED_NUMBER) {
                    require(totalSupply() < jigsawTotal,'ER: The sale is sold out');
                    require(fiatCounter < jigsawFiat,'ER: Not enough Jigsaws left for the fiat sale');
    ...
    function privateMint() external payable {
        require(block.timestamp >= privateSaleBegin,'ER: Private sale is not started');
        require(block.timestamp < fiatSaleBegin,'ER: Private sale is currently closed');
        require(privateSaleEntries[msg.sender],'ER: You are not qualified for the presale');
        require(totalSupply() < jigsawTotal, 'ER: The sale is sold out');
        require(privateCounter < jigsawPresale,'ER: Not enough Jigsaws left for the presale');
        require(hasMinted[msg.sender]<MAX_WHITELISTED_MINTED_NUMBER, 'ER: You have already minted maximum for whitelisted sale');
    ...
    function setGameStartandEndTime(uint256 _startedAt, uint256 _endedAt, uint256 _totalFee) public onlyOwner{
    ...
    //TODO: public fun for testing make it internal function before deployement to mainent.
    /**
     @notice Calculates communityAmount and charityAmount, used in internal calls.
    */
    function getCommunityAmount() public onlyOwner view returns (uint256 _communityAmount, uint256 _charityAmount) {
        uint256 delayPeriod;
        ...
        if(delayPeriod >= 0 days && delayPeriod < 1 days){
            transferenceAmount = (totalFee * communityRatio * delaySeconds * 500) / (1 days * 1e8);
        } else if(delayPeriod >= 1 days && delayPeriod < 2 days){
            transferenceAmount = (totalFee * communityRatio * 500) / (1e8);
            transferenceAmount += (totalFee * communityRatio * delaySeconds * 1000) / (1 days * 1e8);
        } else{
            transferenceAmount = (totalFee * communityRatio * 500) / (1e8);
            transferenceAmount += (totalFee * communityRatio * 1000) / (1e8);
            transferenceAmount += (totalFee * communityRatio * 1500 * ((delayDays.sub(2)*86400).add(delaySeconds))) / (1 days * 1e8);
        }
