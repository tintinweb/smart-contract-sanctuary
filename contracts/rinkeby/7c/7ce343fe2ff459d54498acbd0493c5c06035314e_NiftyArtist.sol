...
contract NiftyArtist is ERC721EnumerableUpgradeable,  AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint public constant MAX_TOKENS = 8800;
    // Max at launch before ltd edition chars unlock
    uint public constant MAX_TOKENS_INIT = 880;
    // Track current supply cap in range [MAX_TOKENS_INIT, MAX_TOKENS]
    uint internal CURR_SUPPLY_CAP;

    uint256[500] private _______gap;   // Leave a gab which can be used to add a new extensions.
    ...
    function initialize(string memory name, string memory symbol, string memory uri) public virtual initializer {
        ...
        CURR_SUPPLY_CAP = MAX_TOKENS_INIT;
    ...
    function royaltyInfo(uint256 tokenId_, uint256 value_) public view returns (address _reciever, uint256 _royaltyAmount) {
        // @TODO: Enable different royalities per token.
