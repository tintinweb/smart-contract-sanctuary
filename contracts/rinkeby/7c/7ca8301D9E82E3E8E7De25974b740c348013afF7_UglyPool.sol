...
contract UglyPool is IERC721Receiver, Ownable() {
    ...
    constructor(){
        ...
        createPool(IERC721(0x0000000000000000000000000000000000000000),emptyArray,0);

        //testing data (regular contract)
        createPool(IERC721(0x6d0de90CDc47047982238fcF69944555D27Ecb25),emptyArray,50000000000000000);
    }

    function createPool(IERC721 _registry, uint[] memory _tokenIds, uint _price) public payable { //, bool _random
    ...
    function getPoolId(address _owner, IERC721 _registry) private view returns(uint) {
    ...
    function getPoolTokenIds(IERC721 _registry) public view returns (uint[] memory){ // address _owner, 
        uint _poolId = getPoolId(msg.sender, _registry);
    ...
    // function ethBalance(IERC721 _registry) public view returns (uint) { // , address _poolOwner
    //     uint _poolId = getPoolId(msg.sender, _registry);
    //     return pools[_poolId].balance;
    // }

    function nftBalance(IERC721 _registry) public view returns (uint) { // , address _poolOwner
        uint _poolId = getPoolId(msg.sender, _registry);
    ...
    function poolSize(uint _Id) public view returns (uint) {
    ...
    function setTokenIds(uint[] memory _myArray, uint _poolId) public  {
