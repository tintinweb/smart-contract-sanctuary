...
//        int16 positionY=int16(600-size)-int16(uint16(randomNum((600-size)*2, seedfinal+17, block.timestamp)))-100;
    ...
    function handlePayment(uint16 mintAmount) internal {
        // check for suficient payment

        uint256 currentPrice=MINT_PRICE;
/*
        uint256 currentPrice=MINT_PRICE*(1+totalSupply()/100);
        if(totalSupply()>1000){
            uint256 currentPrice=MINT_PRICE*11;
        }
*/
        if (msg.sender != owner()) {
            require(msg.value >= currentPrice*uint256(mintAmount), "insuficient payment");
        }
