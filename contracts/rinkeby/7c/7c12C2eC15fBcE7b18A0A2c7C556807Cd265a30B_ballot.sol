...
interface membership {
    ...
    function memberId(address user) external view returns(uint);
...
contract ballot {
    ...
    modifier arkMember(address user) {
        require(mem.memberId(user)>0,"You are not a member");
        _;
    }
    ...
    function postStatus(string memory _value) public arkMember(msg.sender) {
        pst[id].user = msg.sender;
        pst[id].value = _value;
        pst[id].upvote = 0;
        pst[id].downvote = 0;
        id += 1;
    ...
    function totalUpvote(uint _id) public arkVoter(msg.sender) view returns(uint) {
        if (pst[_id].upvote == 0) {
    ...
    function totalDownvote(uint _id) public arkVoter(msg.sender) view returns(uint) {
    ...
    function message(uint _id) public arkMember(msg.sender) view returns(string memory) {
        if (keccak256(bytes(pst[_id].value)) != keccak256(bytes(""))) {
    ...
    function totalVote(uint _id) public arkVoter(msg.sender) view returns(int) {
