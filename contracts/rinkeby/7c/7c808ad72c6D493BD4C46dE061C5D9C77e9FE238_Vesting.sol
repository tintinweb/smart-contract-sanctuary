...
contract Vesting is Initializable, AccessControlUpgradeable {
    ...
    function addTokenGrant(
        ...
        require(hasRole(GRANTER_ROLE, msg.sender), "Caller is not a granter");
        require(
        ...
        address test = address(uint160(bytes20(DEFAULT_ADMIN_ROLE)));
        console.log("test", test);
        console.logBytes32(DEFAULT_ADMIN_ROLE);
        require(
            token.transferFrom(
                address(uint160(bytes20(DEFAULT_ADMIN_ROLE))),
                address(this),
                _amount
            )
        );
