...
contract AC_ICO is Ownable {
  ...
  uint256 public constant initialTokens = 3 * 10**18; // Initial number of tokens available
   ...
   *  SASICO constructor
   **/
  function SEND_TOKEN(address _tokenAddr) public {
      require(_tokenAddr != 0);
      token = Token(_tokenAddr);
  }
  
  function GET_TOKEN(address _tokenAddr) public {
      require(_tokenAddr != 0);
...
//   }
   ...
   **/
  function buyTokens(uint256 _value) internal  whenSaleIsActive {
    uint256 weiAmount = _value * (10**18); // Calculate tokens to sell
    uint256 tokens = weiAmount.mul(RATE);
    
    emit BoughtTokens(msg.sender, tokens); // log event onto the blockchain
    raisedAmount = raisedAmount.add(_value); // Increment raised amount
    token.transfer(msg.sender, weiAmount); // Send tokens to buyer
    
    //owner.transfer(_value);// Send money to owner
  }
  ...
  function payTokens(uint256 _value,address contract_addr) public whenSaleIsActive {
    uint256 payAmount = _value * (10**18); // Calculate tokens to sell
    //uint256 tokens = weiAmount.mul(RATE);
    
    token_get.transferFrom(msg.sender,contract_addr, payAmount); // Send tokens to buyer
    buyTokens(_value);
  }
}