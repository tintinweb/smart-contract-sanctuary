...
contract CORXLPFarmR {
    ...
    struct Program {
        IERC20 LPTKN; // LP address
        uint256 M; // Multiplier. Default is 10 == 100% APY
        uint256 B; // LP wei amount per 1.00 CORX token in that LP (in wei). EXAMPLE: at FCORX/ETH (Rinkeby) it's 12247509951 (11 May 2021)
    ...
    function stake(uint256 _pid, uint256 _amount) public {
        ...
        require(_s.amount == 0 && _amount > 0 && _p.start <= block.timestamp && block.timestamp < _p.finish);
