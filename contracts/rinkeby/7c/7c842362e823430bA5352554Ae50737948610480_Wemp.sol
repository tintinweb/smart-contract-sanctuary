...
contract Wemp is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    constructor () {
        ...
        charityWallet = 0xa3547a336973573744083114291de06072C722bf;
    ...
    function getRValues(uint256 amount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tCharityFee) internal view returns (uint256, uint256, uint256) {
        uint256 currentRate = getRate();
        uint256 tAmount = amount;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tCharityFee)  = getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = getRValues(tAmount, tFee, tLiquidity, tBurn, tCharityFee);
