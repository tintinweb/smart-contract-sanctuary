...
contract CryptoPills is Ownable, ERC721Enumerable {
    ...
    uint[100] public supply;

    uint maxPerIssue = 100;
    ...
    address private marketingAddress =
        0xAeE8ba9F3a6D4B7976b0095a05Cea126A04Fdac7; // marketing

    constructor(string memory inputBaseUri)
        ERC721("Crypto-Pills test exploit", "TE")
    {
    ...
    function buyPresale(uint256 numberOfTokens)
        external
        payable
        onlyPresaleWhitelist
    {
        require(preSaleActive(), "Presale is not active");
        require(numberOfTokens <= 1, "Too many tokens for one transaction");
        require(
            msg.value >= baseprice.mul(numberOfTokens),
    ...
    function buy(uint issue) external payable {
