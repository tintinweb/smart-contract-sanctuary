...
contract Barn is Ownable, IERC721Receiver, Pausable {
  ...
  // number of Tortoise staked in the Barn
  ...
  /**
   * adds Tortoise and Goats to the Barn and Pack
  ...
  function addManyToBarnAndPack(address account, uint16[] calldata tokenIds) external {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      if (isTortoise(tokenIds[i])) 
        _addTortoiseToBarn(account, tokenIds[i]);
      else 
  ...
  /**
   * adds a single Tortoise to the Barn
   * @param account the address of the staker
   * @param tokenId the ID of the Tortoise to add to the Barn
   */
  function _addTortoiseToBarn(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
    barn[tokenId] = Stake({
      owner: account,
  ...
  /**
   * realize $EGG earnings and optionally unstake tokens from the Barn / Pack
  ...
  function claimManyFromBarnAndPack(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      if (isTortoise(tokenIds[i]))
        owed += _claimTortoiseFromBarn(tokenIds[i], unstake);
      else
   ...
   */
  function _claimTortoiseFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (totalWoolEarned < MAXIMUM_GLOBAL_EGG) {
   ...
   * Goats earn $EGG proportional to their Alpha rank
  ...
  function randomGoatOwner(uint256 seed) external view returns (address) {
    ...
    uint256 cumulative;
    seed >>= 32;
    // loop through each bucket of Goats with the same alpha score
