...
contract AuctionMint is ERC721URIStorage, Ownable, Pausable {
    ...
    event BidRefunded(address indexed bidder, uint256 refundAmount);
    ...
    uint256 public constant AUCTION_MINT_QTY = 3333;
    ...
    constructor() ERC721("AuctionMintContract", "AMC") {
        contractAddress = address(this);
        for(uint256 i = 0; i < NUM_AUCTIONS; i++) {
            auctionRemaingItems[i] = AUCTION_MINT_QTY;
        }
    ...
    function refundBidders(address payable[] calldata bidders) external onlyOwner whenRefundsAllowed whenAuctionEnded {
        ...
        for(uint256 i = 0; i < bidders.length; i++) {
            ...
            emit BidRefunded(bidder, refundAmt);
        }
    }
    ...
    function claimRefund() external whenRefundsAllowed whenAuctionEnded {
        ...
        emit BidRefunded(_msgSender(), refundAmt);
    }

    function claimAuctionMints() public whenAuctionEnded {
        require(auctionWhitelist[_msgSender()].quantityToMint > 0, "Address didn't win auction");
        uint256 startingSupply = currentSupply;
        for(uint256 i = 0; i < auctionWhitelist[_msgSender()].quantityToMint; i++){
        ...
        if(currentSupply - startingSupply < auctionWhitelist[_msgSender()].quantityToMint) {
            auctionWhitelist[_msgSender()].quantityToMint -= (currentSupply - startingSupply);
        }
        else {
            auctionWhitelist[_msgSender()] = Bid(0, 0);
        }
