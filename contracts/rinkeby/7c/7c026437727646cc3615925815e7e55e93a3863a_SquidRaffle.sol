...
contract SquidRaffle is VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    uint16 constant totalRounds = 5;
    uint16 constant tokenPerRound = 30;
    ...
    function requestRandom(uint8 _roundId) public {
        ...
        uint startIdx = tokenPerRound * (_roundId - 1) + 1;
        uint endIdx = startIdx + tokenPerRound - 1;
        require(endIdx <= tokenCounts, 'This round is not finished yet');
        requests[_roundId] = getRandomNumber();
    }
    ...
    function pickWinner(uint _roundId) public nonReentrant {
        require(_roundId > 0 && _roundId <= totalRounds, 'invalid roundId'); 
        require(winners[_roundId].tokenId == uint16(0), 'This round already has winner');
        ISquidNFT NFT = ISquidNFT(nftAddress);    
        uint startIdx = tokenPerRound * (_roundId - 1) + 1;
        uint randSeed = getRoundRand(_roundId);
        require(randSeed != uint(0), 'Round randdom seed is not available yet');
        
        uint pickIdx = randSeed  % tokenPerRound;
        ...
        if (consonants[0] == 7 && consonants[1] == 14 && consonants[2] == 4) {         
            ethPrize = ojingeoPrize;
        } else if (consonants[0] == consonants[1] && consonants[0] == consonants[2] && _roundId > 5) {
            ethPrize = grandPrize;
            grandPrize = 1;
        } else {
            ethPrize = 0;           
        }
        winners[_roundId] = Winner(uint16(tokenId), uint16(winnerIdx), ethPrize, winnerAddress);
        if (ethPrize > 0) {
            (bool sent, ) = winnerAddress.call{value: ethPrize*10**18}('');
