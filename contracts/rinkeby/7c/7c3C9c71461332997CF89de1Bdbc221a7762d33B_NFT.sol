...
contract NFT is ERC721, Ownable, PaymentSplitter {
  ...
  uint public constant reservedAmount = 3;
  uint public constant salePrice = 0.33 ether;
  ...
  string public baseUri = "https://gateway.pinata.cloud/ipfs/QmTRhcgh65gfSBm4TvwTW267TV74YWhgdXccXdEYPeiATb";
  
  event Minted(address minter, uint tokenID);

  constructor() ERC721("NFT Collection", "ttt") PaymentSplitter(_team, _teamShares) {}
  ...
  function mintSale() external payable onlyEOA {
    require(saleOpen , "Sale not started yet");
    require(!addressHasMinted[msg.sender] , "address already minted");
    require(totalSupply() + 1 <= maxSupply - (reservedAmount - reservedMintedAlready), "max supply reached");
    require(msg.value == salePrice , "sent incorrect price");
