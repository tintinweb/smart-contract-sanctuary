...
contract UmiTokenFarm is Context, Ownable, ReentrancyGuard, Pausable {
     ...
     */
    function fundingContract(address _tokenAddress, uint256 _amount)
        external
        isContract(_tokenAddress)
        nonReentrant
    {
        ...
        require(
            getToken(_tokenAddress).transferFrom(
                msg.sender,
                address(this),
                _amount
            ),
    ...
    function getApy(address _tokenAddress) public view returns (uint256) {
        uint256 apy = apys[_tokenAddress];
     ...
     */
    function getERC20TokenBalance(address _tokenAddress, address addr)
        public
        view
        isContract(_tokenAddress)
        returns (uint256)
        ...
        whenNotPaused
    {
        ...
        stake(
            _tokenAddress,
            ++lastStakeIds[_tokenAddress][msg.sender],
            _amount
        );
    }
     ...
     */
    function unstakeCertainAmount(
        address _tokenAddress,
        uint256 _stakeId,
        uint256 _amount
    ) external isContract(_tokenAddress) whenNotPaused nonReentrant {
        require(
            _stakeId > 0 && _stakeId <= lastStakeIds[_tokenAddress][msg.sender],
            "wrong stake id"
        );
     ...
     * @param _stakeId User's unique stake ID.
     */
    function unstake(address _tokenAddress, uint256 _stakeId)
        external
        isContract(_tokenAddress)
        whenNotPaused
        nonReentrant
     ...
     */
    function makeRequestedUnstake(
        address _tokenAddress,
        uint256 _stakeId,
        uint256 _amount
    ) internal {
        uint256 requestDate = unstakeRequestsDates[_tokenAddress][msg
            .sender][_stakeId];
    ...
    function _unstake(
        ...
        require(
            balances[_tokenAddress][_sender][_id] > 0 &&
                balances[_tokenAddress][_sender][_id] >= _amount,
            "insufficient funds"
        );
        // calculate interest
        ...
        uint256 amount = _amount == 0
            ...
            : _amount;
        balances[_tokenAddress][_sender][_id] = balances[_tokenAddress][_sender][_id]
            .sub(amount);
        ...
        if (totalFunding[_tokenAddress] >= interest) {
            ...
            totalFunding[_tokenAddress] = totalFunding[_tokenAddress].sub(
                interest
            );
        } else {
     ...
     * @param _id User's unique stake ID.
     */
    function claim(address _tokenAddress, uint256 _id)
        external
        isContract(_tokenAddress)
        whenNotPaused
        nonReentrant
    {
        require(
            _id > 0 && _id <= lastStakeIds[_tokenAddress][msg.sender],
        ...
        require(
            totalFunding[_tokenAddress] >= interest,
