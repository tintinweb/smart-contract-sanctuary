...
contract Market is InitOwner{
    ...
    function _nBalance(uint t,address a,uint i,address d,uint n) private {
      // 判断NFT
      if(t == 721){
    ...
    function _nTransfer(uint t,address s,address c,address b,uint i,uint n) private {
      if(t == 1155){
    ...
    function _erc20Transfer(Order memory order,NFTExchange memory signID) private {
      if(order.token != address(0)){
        // 是否开通交易的ERC20代币
        require(TransactionCurrency[order.token], "ERC20 Not opened");
        // 如果是ERC20判断余额是否足够
        IERC20 erc20Coin = IERC20(order.token);
        require(erc20Coin.balanceOf(msg.sender) < order.price, "Insufficient Balance !");
        _safeEnter(order.token,order.price);
        if(publishFee > 0){
          _safeOut(signID.creator,order.token,publishFee*order.price/decimals);
        }
        if(fee > 0){
          _safeOut(KabukicoinWallte,order.token,fee*order.price/decimals);
        }
        if(recommendFee > 0){
          _safeOut(order.gooder,order.token,recommendFee*order.price/decimals);
        }
        if(signID.nonce == 0){
          _safeOut(signID.creator,order.token,(1-fee-publishFee-recommendFee)*order.price/decimals);
        }else{
          _safeOut(signID.belong,order.token,(1-fee-publishFee-recommendFee)*order.price/decimals);
        }
        
      }else{
        if(publishFee > 0){
          Address.sendValue(payable(signID.creator),publishFee*order.price/decimals);
        }
        if(fee > 0){
          Address.sendValue(payable(KabukicoinWallte),publishFee*order.price/decimals);
        }
        if(recommendFee > 0){
          Address.sendValue(payable(order.gooder),recommendFee*order.price/decimals);
        }
        if(signID.nonce == 0){
          Address.sendValue(payable(signID.creator),(1-fee-publishFee-recommendFee)*order.price/decimals);
        }else{
          Address.sendValue(payable(signID.belong),(1-fee-publishFee-recommendFee)*order.price/decimals);
        }
      }
    }
    function test1(Order memory order,bytes[] memory signs) public view returns(address){
    ...
    function test3(Order memory order) public payable returns(uint){
      NFTExchange memory signID = TradeRecord[order.art][order.id];
      _erc20Transfer(order,signID);
      return order.price;
    }
    function test4(Order memory order) public returns(address){
      NFTExchange memory signID = TradeRecord[order.art][order.id];
      _nTransfer(order.nft,order.art,signID.creator,signID.belong,order.id,signID.nonce);
      return order.art;
    }
    // 0x0000000000000000000000000000000000000000
    function Buy(Order memory order,bytes[] memory signs) public payable returns (bool result){
      // 过滤签名的时间戳小于当前时间戳
      require(Address.isContract(order.token) && Address.isContract(order.art) && order.create/1000 < block.timestamp && order.sale/1000 < block.timestamp && !forbidSignature[signs[0]], "Illegal parameter !");
