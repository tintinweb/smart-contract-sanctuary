...
contract TFTMarketPlace is ReentrancyGuard {
    ...
    uint256 public _offerAmount = 0;
    OfferObject[] _offerObjects;
    ...
    event eveSupportNft(address nft, bool support);
    ...
    //Offer Event
    event eveCancelOffer(uint256 indexed id, uint256 tokenId);
    event eveNewOffer(
        uint256 indexed id,
        uint256 tokenId,
        address seller,
    ...
    //Sales Modifier
    modifier checksalesindex(uint256 index) {
        require(index <= _salesObjects.length, "index overflow");
        _;
    }

    modifier salesMustNotSellingOut(uint256 index) {
        require(index <= _salesObjects.length, "index overflow");
    ...
    //Offer Modifier
    modifier checkofferindex(uint256 index) {
        require(index <= _offerObjects.length, "index overflow");
        _;
    }

    modifier offerMustNotSellingOut(uint256 index) {
        require(index <= _offerObjects.length, "index overflow");
        OfferObject storage obj = _offerObjects[index];
        require(obj.status == 0, "sry, selling out");
        _;
    }

    modifier onlyOfferOwner(uint256 index) {
        require(index <= _offerObjects.length, "overflow");
        OfferObject storage obj = _offerObjects[index];
        require(
            obj.buyer == msg.sender || msg.sender == _governance,
            "author & governance"
        );
        _;
    }
    ...
    // --- Functions ---
    function initialize(address payable tipsFeeWallet, uint256 tipsFeeRate)
        public
    {
    ...
    function startSales(
        ...
        uint256 _amount,
        address _currency
    ) external nonReentrant validAddress(_nft) returns (uint256) {
    ...
    function buy(uint256 index, uint256 _amount)
        public
        payable
        nonReentrant
        salesMustNotSellingOut(index)
    {
        ...
        if (obj.currency == address(0x0)) {
            ...
            IERC20(obj.currency).safeTransferFrom(
                msg.sender,
                _tipsFeeWallet,
                tipsFee
            );
    ...
    //Offer
    function getOffer(uint256 index)
        external
        view
        checkofferindex(index)
        returns (OfferObject memory)
    {
        return _offerObjects[index];
    }

    function getOfferPrice(uint256 index)
        external
        view
        checkofferindex(index)
        returns (uint256)
    ...
    function cancelOffer(uint256 index)
        external
        checkofferindex(index)
        onlyOfferOwner(index)
        offerMustNotSellingOut(index)
        nonReentrant
    {
        OfferObject storage obj = _offerObjects[index];
        obj.status = 2;

        emit eveCancelOffer(index, obj.tokenId);
    }
    ...
    function offer(
        ...
        if (_offerObjects.length == 0) {
            OfferObject memory zeroObj;
            zeroObj.tokenId = 0;
            ...
            zeroObj.amount = 0;
            zeroObj.currency = address(0x0);
            _offerObjects.push(zeroObj);
        }

        _offerObjects.push(obj);
        _offerAmount++;
    ...
    function sell(uint256 index, uint256 _amount)
        public
        payable
        nonReentrant
        offerMustNotSellingOut(index)
    {
        ...
        OfferObject storage obj = _offerObjects[index];
        require(obj.amount <= amount, "umm.... It's too much");
        uint256 unitPrice = this.getOfferPrice(index);
        uint256 price = unitPrice * amount;
        uint256 tipsFee = price.mul(_tipsFeeRate).div(_baseRate);
        uint256 purchase = price.sub(tipsFee);

        if (obj.currency == address(0x0)) {
            uint256 buyerBalance = IERC20(0x0).balanceOf(obj.buyer);
            require(
                buyerBalance >= this.getOfferPrice(index),
                "umm..... you are not rich enough to buy"
            );

            obj.buyer.transfer(purchase);
            obj.buyer.transfer(tipsFee);
        } else {
            IERC20(obj.currency).safeTransferFrom(
                obj.buyer,
                _tipsFeeWallet,
