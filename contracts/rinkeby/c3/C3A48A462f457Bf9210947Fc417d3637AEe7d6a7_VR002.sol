...
contract VR002 is ERC721Enumerable, Ownable {
	struct TokenInfo {
		uint8 claimed;
	...
	ERC721Lushsux public Lushsux_io = ERC721Lushsux(0x6B93c3A2bea70649f1b8895723689F40134C5511);
	address public burnerWallet = 0xf99209b3ba36082DFE3FEdbada0B190fcde2BdD8; // need to fill
	string public parentURI;
	bool public salesActive;

	mapping(address => bool)[3] public whitelists;

	constructor () ERC721("VR002", "VR002") {
		tokens[0] = TokenInfo(0, 50, 60, 0, 1, 0.1 ether);
		tokens[1] = TokenInfo(0, 25, 30, 60, 5, 0.1 ether);
		tokens[2] = TokenInfo(0, 10, 12, 90, 10, 0.01 ether);
		tokens[3] = TokenInfo(0, 50, 60, 102, 0, 0.1 ether);
	}
	...
	function mintToken(uint8 _tokenNo) external payable {
		...
		require(msg.value >= tokens[tokenId].PRICE, "Claim: Ether value incorrect");
	...
	// have to pre-approve on the minting page first before using
