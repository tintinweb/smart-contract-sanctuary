...
contract Ponzi is IPonzi {
     ...
     */
    function deposit(address _owner, uint256 addedValue) 
        external 
        payable
        override 
        fundsSent
        returns (bool) 
    {
        ...
        c.amount += addedValue;
        emit Deposit(_owner, addedValue);
        return true;
    }
     ...
     * `subtractedValue`.
     */
    function withdrew(address _owner, uint256 subtractedValue)
        external 
        payable
        override
        withdrawable(_owner)
        returns (bool) 
    {
        uint256 currentAllowance = ACCs[_owner].amount;
        require(currentAllowance >= subtractedValue, "ERC20: decreased allowance below zero");
        ACC storage c = ACCs[_owner];
        c.amount -= subtractedValue;
        c.depositTime = block.timestamp;
        emit Withdrew(_owner, subtractedValue);
        return true;
    }
