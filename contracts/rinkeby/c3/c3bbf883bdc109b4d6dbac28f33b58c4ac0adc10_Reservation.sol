...
contract Reservation {
    ...
    function getRoom(string memory roomCode) public returns(RoomTable memory){
        bytes32 roomId = keccak256(abi.encodePacked(
            roomCode
        ));
        RoomTable memory checkRoom;
        RoomTable memory room = rooms[roomId];
        currentDate = block.timestamp;
        uint roomQuantity = room.roomQuantity;
        
        for(uint i=0; i < room.reservations.length; i++){  
            if(currentDate >= room.reservations[i].checkInDate && currentDate < room.reservations[i].checkOutDate){
                roomQuantity = roomQuantity - 1;
            }
        }
        checkRoom.reservations = room.reservations;
        checkRoom.roomQuantity = room.roomQuantity;
        checkRoom.roomId = room.roomId;
