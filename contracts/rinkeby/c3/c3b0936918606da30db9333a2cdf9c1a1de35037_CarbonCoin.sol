...
contract CarbonCoin is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private nextSupplyCalculateTime = 0;
    uint256 private excludedSupply = 0;
    
    function getNextSupplyCalculateTime() public view returns (uint256){
        return nextSupplyCalculateTime;
    }

    function getExcludedSupply() private returns (uint256){
        if (nextSupplyCalculateTime >= block.timestamp && excludedSupply != 0){
            return excludedSupply;
        }
        for (uint256 i = 0; i < _excludedFromReward.length; i++){
            excludedSupply += _tOwned[_excludedFromReward[i]];
        }
        nextSupplyCalculateTime += 5 minutes;
        return excludedSupply;
    }

    function checkReward() public nonReentrant returns (uint256) {
        if (_isExcludedFromReward[msg.sender]){
            return 0;
        }
        uint256 excludedSupplyAmount = getExcludedSupply();

        uint256 cTotalSupply = uint256(_tTotal)
        .sub(balanceOf(address(0)))
        .sub(balanceOf(0x000000000000000000000000000000000000dEaD))
        .sub(excludedSupplyAmount)
    ...
    function activate() public onlyOwner {
        ...
        setClaimEventTime(60);
        nextSupplyCalculateTime = block.timestamp;
    }
