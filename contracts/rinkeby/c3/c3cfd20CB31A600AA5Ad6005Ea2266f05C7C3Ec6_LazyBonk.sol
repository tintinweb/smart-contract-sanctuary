...
contract LazyBonk is ERC721Enumerable, Ownable, AccessControl {
    ...
    uint256 public constant TOTAL_NUMBER_OF_LAZY_BONES = 9999;
    uint[TOTAL_NUMBER_OF_LAZY_BONES] internal indices;
    uint internal nonce = 0;

    uint256 private constant preMintLimit = 1;
    uint256 private constant publicMintLimit = 20;

    //TODO Check for giveaway supply count
    uint256 public giveaway_reserved = 20;
    uint256 public pre_mint_reserved = 500;
    ...
    //Initial team
    address notron = 0x36A59A0B623a4B9EF9d4b4bb11F2aAC40B2dc239;
    ...
    string private _currentBaseUrl = "https://gateway.pinata.cloud/ipfs/QmTpm6rYT8udvRhYF8tcPehn9qzvNEpXjEa5JaGZZskeoo/";
  ...
  constructor() ERC721("LazyBonk", "LB") {
    setBaseURI(_currentBaseUrl);
     ...
     _setupRole(DEFAULT_ADMIN_ROLE, notron);
     _setupRole(WHITE_LIST_ROLE, msg.sender);
     _setupRole(WHITE_LIST_ROLE, notron);
  }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "LazyBonks: URI query for nonexistent token");
  ...
  //Public 
  function claim(uint256 num) public payable whenMintNotPaused() {
        uint256 supply = totalSupply();
        uint256 tokenCount = balanceOf(msg.sender);
        require( num <= 20,                                                             "LazyBonesSpaceTrip: You can mint a maximum of 20 Lazy Bones" );
        require( tokenCount + num <= 20,                                                "LazyBonesSpaceTrip: You can mint a maximum of 20 Lazy Bones" );
        require( supply + num <= TOTAL_NUMBER_OF_LAZY_BONES - giveaway_reserved,        "LazyBonesSpaceTrip: Exceeds maximum Galaxy Eggs supply" );
        require( msg.value >= PublicMintPrice * num,                                    "LazyBonesSpaceTrip: Ether sent is less than PRICE * num" );
  ...
  //Pre sale
  function preMint() public payable whenPreMintNotPaused() preMintAllowedAccount(msg.sender) {
    require(balanceOf(msg.sender) <= 1, 'LazyBonesSpaceTrip: Only one token per address allowed');
    require(pre_mint_reserved > 0, 'LazyBonesSpaceTrip: Exceeds pre mint reserved Lazy Bones limit');
    require(msg.value >= PreMintPrice, 'LazyBonesSpaceTrip: Ether sent is less than price');
    uint256 supply = totalSupply();
