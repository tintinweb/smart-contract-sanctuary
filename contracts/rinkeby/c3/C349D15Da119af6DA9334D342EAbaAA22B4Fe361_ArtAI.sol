...
contract ArtAI is Context, ERC165, IERC721, IERC721Metadata, Ownable, Pausable {
    ...
    // alignment id to alignment strings
    string[] public _alignmentList;
    ...
    string private _metadataURI = "https://art-ai.com/api/heroes/metadata/";
    ...
    constructor(
        ...
        _dispositionList.push("Bad");

        _alignmentList.push("Lawful");
        _alignmentList.push("Neutral");
        _alignmentList.push("Chaotic");
    }
    ...
    function addAlignment(string calldata newAlignment) external onlyOwner {
        _dispositionList.push(newAlignment);
    }
    ...
    function addAddressToWhitelist(address whitelistAddress)
        external
        onlyOwner
    {
    ...
    function addAddressesToWhitelist(address[] calldata whitelistAddressList)
        external
        onlyOwner
    {
        for (uint256 i; i < whitelistAddressList.length; i++) {
            _whitelistAddresses[whitelistAddressList[i]] = true;
        }
    }
    ...
    function _verifyClass(uint256 _classId) internal view returns (bool) {
        ...
        IERC721 gen1 = IERC721(_gen1Address);
        IERC721 gen2 = IERC721(_gen2Address);
        bool ownsEponym = gen1.balanceOf(_msgSender()) > 0 ||
            gen2.balanceOf(_msgSender()) > 0;
    ...
    function _verifyParams(
        ...
        require(_alignmentId < _alignmentList.length, "align");
        require(
    ...
    function _updateStoredValues(
        ...
        Hero memory _newHero = Hero(
            ...
            _alignmentId,
            1,
            Random.rand1To10(_seed1),
            Random.rand1To10(_seed2),
            Random.rand1To10(_seed3),
            Random.rand1To10(_seed4),
            false
        );
    ...
    function _isMintAvailable(address _user) private view returns (bool) {
        if (publicSaleIsOpen()) {
            return true;
        }

        if (whitelistIsOpen()) {
            if (balanceOf(_user) < _getEponymBalance(_user)) {
                return true;
            }
    ...
    function _checkFreeElligibility(bytes32[] memory _merkleProof)
        private
        view
        returns (bool)
    {
    ...
    function ownerMint(
        ...
        _tokenIdToHero[_newTokenId].isHonorary = false;
    ...
    function mint(
        ...
        require(_isMintAvailable(_msgSender()), "mint");
        require(_tokenSupply < maxSupply, "max supply");
    ...
    function rewardMint(
        ...
        require(_checkFreeElligibility(_merkleProof), "elligible");
        _verifyParams(
            _imageName,
            _imageHash,
            _imageId,
        ...
        )
    {
        ...
        if (statsAreRevealed()) {
            return (
                ...
                _alignmentList[_hero.alignment],
                _hero.level,
                _hero.strength,
                _hero.dexterity,
                _hero.vitality,
