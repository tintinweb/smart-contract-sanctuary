...
contract Aludel is IAludel, Powered, Ownable {
    ...
    function unstakeAndClaim(
        ...
        //maintain powerSwitch functionality
        if (Ownable.owner() != msg.sender) {
            //check that owner of vault is the transaction sender
            require(
                IUniversalVault(vault).owner() == msg.sender,
                "Aludel: call by Vault Owner Only"
            );
        }
        ...
        if (out.newStakesCount == 0) {
            ...
            //Check if a partial stake update is required
