...
interface IDappRegistry {
  ...
  function authorisations(uint8 _registryId, address _dapp)
    external
    view
    returns (bytes32);
}
...
contract MultiCallHelper {
   ...
   */
  function isMultiCallAuthorised(address _wallet, Call[] calldata _transactions)
    external
    view
    returns (bool)
  {
   ...
   * @param _transactions The sequence of transactions.
   */
  function multiCallAuthorisation(
    address _wallet,
    Call[] calldata _transactions
  ) external view returns (uint256[] memory registryIds) {
  ...
  function isAuthorised(
    ...
    uint256 registries = uint256(dappRegistry.enabledRegistryIds(_wallet));
    ...
    for (
      ...
      if (isEnabled) {
        ...
        uint256 auth = uint256(
          dappRegistry.authorisations(uint8(registryId), _spender)
        );
  ...
  function isAuthorisedInRegistry(
    address _wallet,
    Call[] calldata _transactions,
    uint8 _registryId
    ...
    for (uint256 i = 0; i < _transactions.length; i++) {
      ...
      uint256 auth = uint256(dappRegistry.authorisations(_registryId, spender));
      uint256 validAfter = auth & 0xffffffffffffffff;
      if (0 < validAfter && validAfter <= block.timestamp) {
        // if the current time is greater than the validity time
        address filter = address(uint160(auth >> 64));
