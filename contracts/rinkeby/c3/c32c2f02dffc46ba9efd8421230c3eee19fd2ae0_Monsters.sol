...
contract Monsters is ERC721PresetMinterPauserAutoId, Garage {
        ...
        returns (string memory)
    {
        ...
        string memory baseURI = _baseURI();
        string memory convertTokenId = uint2str((tokenId * block.number) % 15);
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, convertTokenId))
                : "";
    }
