...
contract Rocks721 is Context, ERC721, Ownable {
    ...
    // using Counters for Counters.Counter;
    // Counters.Counter private tokenToMint;
    ...
    function preMint() public payable {
        ...
        require(tokenToMint >= 8000, "pre-sale is sold out");
    ...
    function mint() public payable {
        ...
        require(tokenToMint >= 0, "sale is sold out");
        // require(msg.value == pricePerPiece, "must send in correct amount");
        mintItem();
    }
    
    function mintItem() private returns (uint256 [] memory) {
        //This is tricky - should maybe not do it like this - should  
        // require(totalSupply() < maxNumberOfPieces, "series is sold out");
        require(msg.value >= pricePerPiece, "not enough eth sent "); 

        //This doesn't 
        uint256 amount = msg.value / 10000000000000000;

        uint256 [] memory tokensMinted = new uint256[](amount);

        for(uint256 i = 0; i < amount; i ++){
            // uint256 tokenToMint = 9999 - totalSupply();
            while(_exists(tokenToMint)){
                tokenToMint.trySub(1);
            }
            _safeMint(msg.sender, tokenToMint);
            tokensMinted[i] = tokenToMint;
            tokenToMint.trySub(1);
        }
