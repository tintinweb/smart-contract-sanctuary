...
interface IERC721 is IERC165 {
    ...
    event linkURI(uint256 value, uint256 indexed id);
...
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable,VRFConsumerBase {
    ...
    mapping(uint256 => uint256 ) public _linktokenURIs;
  ...
  // mapping for token creator
    ...
    uint256 public linkfee;
     ...
     using SafeMathChainlink for uint256;
     mapping(bytes32 => uint256) public randomness_map;
    ...
    constructor (string memory name_, string memory symbol_) VRFConsumerBase( 0xb3dCcb4Cf7a26f6cf6B120Cf5A73875B7BBc655B, 0x01BE23585060835E02B77ef475b0Cc51aA1e0709 ) {
        _name = name_;
        _symbol = symbol_;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) public override returns(uint256) {
        ...
        randomness_map[requestId] = randomResult;
        emit randomnumber(randomResult);
        _setlinkURI(tokenCounter,randomResult);
        return randomResult;
    ...
    */
    ...
    function _setlinkURI(uint256 tokenId, uint256 _tokenURI) internal virtual {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        _linktokenURIs[tokenId] = _tokenURI;
         emit linkURI(_tokenURI, tokenId);
    }
...
contract RAJNIPUNK is ERC721 {
    //uint256 public tokenCounter;
    address public owner;
    using SafeMathChainlink for uint256;
    uint256 public _check;
    bytes32 public req_ID;
    uint256 private checkmap;
    ...
    function createCollectible( uint256 fee) public returns (uint256) {
        getRandomNumber();
        uint256 newItemId = tokenCounter;
        _safeMint(msg.sender, newItemId, fee);
        //_setlinkURI(newItemId, req_ID);
