...
contract AssetPool is IAssetPool, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    // onlyLending
    function withdraw(address who, uint amount) external override nonReentrant returns (bool) {
...
interface IAssetPool {
    // onlyLending
    function deposit(address who, uint amount) external returns (bool);

    // onlyLending
    function withdraw(address who, uint amount) external returns (bool);

    function availableReward(address who) external view returns (uint);
    function claimReward(address who, uint amount) external returns (uint);

    // onlyGateway
    function transferAsset(address to, uint amount) external returns (bool);
}
