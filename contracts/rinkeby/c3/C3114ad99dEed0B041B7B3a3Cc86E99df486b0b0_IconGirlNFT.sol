...
contract IconGirlNFT is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor () ERC721 ("IconGirl", "$IGT") {
        setBaseURI("https://icongirlnft.com/api/getTokenURI/");  
        pause(true);      
    }
    ...
    function checkBurnValidity(address account, uint256 _tokenId) internal view returns (bool) {         
        uint256[] memory tokenIDs = tokensOfOwner(account); 
        bool found=false;
        for (uint256 k = 0; k < tokenIDs.length; k++) {
            if (tokenIDs[k] == _tokenId) {
    ...
    function burnIconGirl(uint256 tokenId, uint256 amount) public saleIsOpen {  
        require(checkBurnValidity(msg.sender, tokenId), "Burn: Only owner of this token can burn");
        _burn(tokenId);    
        for (uint256 i = 0; i < amount; i++) {
            _mintAnElement();
        }
