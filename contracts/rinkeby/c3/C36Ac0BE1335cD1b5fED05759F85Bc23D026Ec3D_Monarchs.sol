...
contract Monarchs is ERC721, Ownable {
    ...
    uint256 public constant monarchPrice = 150000000000000000; // 0.15 ETH
    ...
    // Reserve 10 MONARCHS for team - Giveaways/Prizes etc
    uint public monarchReserve = 10;
    ...
    function tokenLicense(uint _id) public view returns(string memory) {
        require(_id < totalSupply(), "CHOOSE A MONARCH WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function mintMonarch(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= maxMonarchPurchase, "Can only mint 20 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_MONARCHS, "Purchase would exceed max supply of Monarchs");
        require(msg.value >= monarchPrice.mul(numberOfTokens), "Ether value sent is not correct");
    ...
    function changeMonarchName(uint _tokenId, string memory _name) public {
        require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this monarch!");
        require(sha256(bytes(_name)) != sha256(bytes(monarchNames[_tokenId])), "New name is same as the current one");
        monarchNames[_tokenId] = _name;
        
        emit monarchNameChange(msg.sender, _tokenId, _name);
        
    }
    
    function viewMonarchName(uint _tokenId) public view returns( string memory ){
        require( _tokenId < totalSupply(), "Choose a monarch within range" );
        return monarchNames[_tokenId];
    }
