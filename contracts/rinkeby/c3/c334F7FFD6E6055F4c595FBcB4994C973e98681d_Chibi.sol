...
contract Chibi is ERC721Enumerable, Ownable {
    uint256 public constant TOTAL_CHIBI = 5000;
    ...
    constructor(string memory baseURI) ERC721("Chibi", "Chibi")  {
        changeBaseURI(baseURI);
        
        // @TODO: Preminting for test
    }
    ...
    modifier mintingIsAvailable{
        require(totalSupply() < TOTAL_CHIBI, "No Chibi left to mint.");
        _;
    }
    ...
    function mintChibi(uint256 qty) public payable mintingIsAvailable {
        ...
        require(totalSupply() < TOTAL_CHIBI, "All Chibis are minted.");
        require(qty <= MAX_PURCHASE, "Buying too many per transaction.");
        require(SafeMath.add(totalSupply(), qty) <= TOTAL_CHIBI, "Going over 5000 chibi with the purchase.");
        require(msg.value >= getPrice(qty), "Invalid price.");
     ...
     **  Internal functions
     */ 
    function getPrice(uint256 qty) internal view returns (uint256) {
        return price * qty;
    }
}