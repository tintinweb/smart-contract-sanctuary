...
contract allocator is Ownable {
    ...
    function getMeRandomCardsWithCallback(IERC721Enumerable token, address[] memory vaults, address recipient) external noCallbacks onlyAuth {
        getMeRandomCards(token, vaults, recipient, true);
    }

    function getMeRandomCardsWithoutCallback(IERC721Enumerable token, address[] memory vaults, address recipient) external noCallbacks onlyAuth {
        getMeRandomCards(token, vaults, recipient, false);
    }

    function getMeRandomCards(IERC721Enumerable token, address[] memory vaults, address recipient, bool withCallback ) internal {
