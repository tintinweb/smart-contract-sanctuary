...
import "./IERC20Redeeming.sol";

// Interfaces
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "../tokens/IMintableERC20.sol";

contract ERC20Redeeming is Base, IERC20Redeeming {
    ...
    mapping(address => uint256) public inputTokenRatios;

    constructor(address settingsAddress, address outputTokenAddress) public Base(settingsAddress) {
        require(outputTokenAddress.isContract(), "OUTPUT_TOKEN_MUST_BE_CONTRACT");
        output = outputTokenAddress;
    }

    function redeem(address inputToken, uint256 amount) external override whenPoolIsNotPaused() {
        _beforeUserRedeem(msg.sender, inputToken, amount);
        _safeInputTokenBurn(msg.sender, inputToken, amount);
        console.log("----> Rate", inputTokenRatios[inputToken]);
        uint256 amountToReceive = amount.mul(inputTokenRatios[inputToken]).div(AMOUNT_SCALE);
        console.log("----> amountToReceive", amountToReceive);
        _safeOutputTokenMint(address(this), msg.sender, amountToReceive);
        emit Redeemed(msg.sender, inputToken, amount);
    }
    ...
    function addInputToken(
        ...
        require(inputTokenRatios[inputToken] == 0, "INPUT_TOKEN_ALREADY_EXISTS");
        uint256 ratio = ratioNumerator.mul(AMOUNT_SCALE).div(ratioDenominator);
        inputTokenRatios[inputToken] = ratio;
        emit InputTokenAdded(inputToken, ratio);
    }

    function removeInputToken(
        address inputToken
    ) external override onlyConfigurator(msg.sender) {
        require(inputTokenRatios[inputToken] != 0, "INPUT_TOKEN_DOESNT_EXIST");
        uint256 ratio = inputTokenRatios[inputToken];
        inputTokenRatios[inputToken] = 0;
        emit InputTokenRemoved(inputToken, ratio);
    }

    function pauseContract() external override onlyPauser(msg.sender) whenPoolIsNotPaused() {
        isPaused = true;
        emit ContractPauseSet(true);
    }

    function unpauseContract() external override onlyPauser(msg.sender) whenPoolIsPaused() {
        isPaused = false;
        emit ContractPauseSet(false);
    }

    /* View Functions */

    /* Internal Functions  */
    function _safeInputTokenTransferFrom(
        address from,
        address to,
        address inputToken,
        uint256 amount
    ) internal {
        IERC20(inputToken).safeTransferFrom(from, to, amount);
    }

    function _safeInputTokenBurn(
        address from,
        address inputToken,
        uint256 amount
    ) internal {
        IERC20(inputToken).safeTransferFrom(from, address(BURN_ADDRESS), amount);
    }
    ...
    function _beforeUserRedeem(
        address account,
        address inputToken,
        uint256 amount
    ) internal view {
        require(inputTokenRatios[inputToken] > 0, "INPUT_TOKEN_MUST_BE_VALID");
        require(
            IERC20(inputToken).allowance(account, address(this)) >= amount,
            "ALLOWANCE_LT_AMOUNT"
        );
    }

    function _requireIsNotPaused() internal view {
        require(!isPaused, "CONTRACT_IS_PAUSED");
    }

    function _requireIsPaused() internal view {
        require(isPaused, "CONTRACT_IS_NOT_PAUSED");
    }

    /** Modifiers */
    modifier whenPoolIsNotPaused() {
        _requireIsNotPaused();
        _;
    }

    modifier whenPoolIsPaused() {
        _requireIsPaused();
        _;
    }
}
...
interface IERC20Redeeming {
    function redeem(address inputToken, uint256 amount) external;
    ...
    /** Events */
    event Redeemed(address indexed user, address inputToken, uint256 amount);

    event InputTokenAdded(address indexed inputToken, uint256 ratio);

    event InputTokenRemoved(address indexed inputToken, uint256 ratio);

    event ContractPauseSet(bool pause);
}
