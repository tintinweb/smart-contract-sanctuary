...
//   ==========================================
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 amount) internal { 
        ...
        if (isAdjustable) {
            bool _isNeedAttenuation;
            (_isNeedAttenuation, _tradingTrack.buyingStack, _tradingTrack.sellingStack) = _getDeviation(_newTokenPrice);

            if (_isNeedAttenuation && sender != liquidityPair) {_attenuateImpulse(_tradingTrack.buyingStack);}
    ...
    function _getDeviation (uint256 _newTokenPrice) internal view returns (bool isNeedAttenuation, uint256 buyingStack, uint256 sellingStack) {  
        ...
        isNeedAttenuation = _tradingTrack.buyingStack > (Wallet[liquidityPair].balance  / 10);
    }
//   ======================================
...
//   ======================================
    ...
    function attenuateImpulse() external onlyRole(Treasury_Analyst) {
        ...
        _attenuateImpulse(numTokensToLiquidity);
    }
...
contract SymplexiaToken is  Taxable {
   ...
   // swapRouterAddress
    ...
    constructor ()  ERC20("BETA65", "BTG65") BaseToken ( 
