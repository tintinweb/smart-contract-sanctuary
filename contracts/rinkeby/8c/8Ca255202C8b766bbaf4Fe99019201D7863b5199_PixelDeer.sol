...
contract PixelDeer is ERC721Tradable, PaymentSplitter {
    ...
    uint32 private maxSupplyPublic = 4_950;
    uint32 private maxSupplyTeam = 50;
    ...
    modifier onlyTeam {
        require(
            msg.sender == devAddress || msg.sender == artistAddress,
    ...
    */
    constructor(
        ...
        currentBaseURI = "http://18.130.130.58/api/";  // TODO: Replace this with the actual base uri of the backend.
    }
    ...
    function setWhitelist(address[] calldata newWhitelist) external onlyTeam {
    ...
    function isWhitelisted(address user) public view returns (bool) {
        for (uint iWhitelistedAddress = 0; iWhitelistedAddress < whitelist.length; iWhitelistedAddress++) {
            if (whitelist[iWhitelistedAddress] == user) {
                return true;
            }
        }
        return false;
