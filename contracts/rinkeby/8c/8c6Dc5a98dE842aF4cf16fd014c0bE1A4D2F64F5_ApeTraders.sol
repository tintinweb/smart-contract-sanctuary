...
contract ApeTraders is ERC721, Ownable {
    ...
    function mint(uint256 count) external payable {
        require(!_paused, "Sales is currently paused");
        require(_tokenIds.current() < _maxSupply, "Cannot mint more than max supply");
    ...
    function mintPresale(uint256 count) public payable {
        ...
        require(count > 0, "Need to mint at least 1");
    ...
    function reserve(uint256 count) public onlyOwner {
        ...
        require(count > 0 , "Need to reserve at least 1");
        for (uint256 i = 0; i < count; i++) {
            _tokenIds.increment();
