...
library SafeERC20 {
    ...
    //  function safeTransferETH(IERC20 token, address to, uint256 value) internal {
...
contract ArtworkNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    event SetMinttransferCoinAddrAddress(address indexed previousOwner, address indexed newOwner);
    ...
    constructor(
        ...
        emit SetMinttransferCoinAddrAddress(address(0), CoinAddr);
    }
      ...
      function mint(address to, string memory _tokenURI) public payable returns (uint256 tokenId) {
        ...
        IERC20(CoinAddr).safeTransfer(mintFeeAddr, mintFeeAmount);
        tokenId = nextTokenId;
        _mint(to, tokenId);
        nextTokenId++;
        _setTokenURI(tokenId, _tokenURI);
        if (msg.value > mintFeeAmount) IERC20(CoinAddr).safeTransfer(msg.sender, msg.value - mintFeeAmount);
    }
    ...
    function transferMintFeeAddress(address _mintFeeAddr) public {
      ...
      function transferCoinAddrAddress(address _CoinAddr) public {
        require(_msgSender() == CoinAddr, 'FORBIDDEN');
        CoinAddr = _CoinAddr;
        emit SetMinttransferCoinAddrAddress(_msgSender(), CoinAddr);
    }
