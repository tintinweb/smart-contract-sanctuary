...
contract PandaPunk is ERC721Enumerable, Ownable, GenRand {
    using SafeMath for uint256;
    using Strings for uint256;

    string public constant PandaPunk_Provenance = "";

    // This is the max number of pandas that can be claimed
    uint public constant MAX_TOKENS = 331;
    // This is the max number of available pandas, must be a prime
    uint public constant MAX_PANDAS = 499;
    ...
    uint public constant initialPhaseNumTokens = 318;
    ...
    function calculatePrice(uint256 _id) public view returns (uint256) {
        ...
        if (_id >= 328) {
            return 1000000000000000000;        // 328-331:  0.80 ETH
        } else if (_id >= 318) {
            return 800000000000000000;         // 318-328:  0.80 ETH
        } else if (_id >= 288) {
            return 640000000000000000;         // 288-318:  0.64 ETH
        } else if (_id >= 228) {
            return 320000000000000000;         // 228-288:  0.32 ETH
        } else if (_id >= 108) {
            return 160000000000000000;         // 108-228: 0.16 ETH
        } else if (_id >= 48) {
            return 80000000000000000;          // 48-108:  0.08 ETH 
        } else if (_id >= 18) {
            return 40000000000000000;          // 18-48:   0.04 ETH 
        } else {
            return 20000000000000000;          // 0 - 18   0.02 ETH
        }
    }
...
//        }
   ...
   function adoptPandaPunk(uint256 numPandas) public payable {
        ...
        require(numPandas > 0 && numPandas <= 5, "You can adopt minimum 1, maximum 5 pands");
...
import "../PandaPunk.sol";
import "../utils/math/SafeMath.sol";

// An extension to PandaPunk, add a feature called reAssign
// follow the spirit of blockchain, holders of PandaPunk NFT can vote to ask for a panda reassignment, each token corresponds to one voting right
...
contract PandaPunkExt is PandaPunk {
    using SafeMath for uint256;
    ...
    uint256 public constant numThreshold = 165; // the threshold: a reassign will be triggered if numTotalVotes > numThreshold
    mapping (uint256 => bool) public hasVoted; // indicate if a tokenId has voted for the current round

    constructor(string memory baseURI) PandaPunk(baseURI) {
        numTotalVotes = 0;
        numRounds = 1;
    ...
    function canVoteOfToken(uint256 tokenId) public view returns (bool){
        require(isInitialPhaseEnd(), "PandaPunkExt: voting is allowed after the initial phase of sale!");
        require(_exists(tokenId), "PandaPunk: invalid query for nonexistent token");
    ...
    function numberOfTotalVotes() public view returns(uint256) {
        return numTotalVotes;
    }
    
    function numberOfCurrentRound() public view returns(uint256) {
        return numRounds;
    }
    
    function currentVotingFee() public view returns(uint256) {
        return votingFee;
    }
    ...
    function voteFromTokenIds(uint256[] memory ids) public payable {
        ...
        require(numVotes > 0, "PandaPunkExt: this address does not have available votes!");
        require(msg.value >= numVotes.mul(votingFee), "PandaPunkExt: low voting fee sent");
