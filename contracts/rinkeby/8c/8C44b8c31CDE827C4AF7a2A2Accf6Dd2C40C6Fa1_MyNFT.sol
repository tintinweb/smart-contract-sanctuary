...
*/
contract MyNFT is ERC721,IERC721Metadata{
    using Strings for uint256;
    uint256 startOfSales;
    uint256 endOfSales;
    uint256 priceOfNFT;
    uint256 tokenIds;
    ...
    modifier onlyOwner(){
        require(msg.sender == owner,"only Onwer is Authorized to call");
            _;
    }
    ...
    function startSales(uint256 setPrice)external onlyOwner{
        startOfSales = block.timestamp; 
        endOfSales = startOfSales + 30 days;
        
        priceOfNFT = setPrice * (10**18);
    }
    
    function newPriceSet(uint256 newPrice)external onlyOwner{
        priceOfNFT = newPrice;
    }
    
    
    function buyNFT() external payable{
        require(block.timestamp >= startOfSales && startOfSales !=0,"Sales has not started ");
        require(block.timestamp <= endOfSales,"Sales has Ended");
        require(totalSupplyCap > 0 ," NFTs Sold out");
        require(msg.value == priceOfNFT," Pls Pay exact Price | you can buy 1 NFT per call");
