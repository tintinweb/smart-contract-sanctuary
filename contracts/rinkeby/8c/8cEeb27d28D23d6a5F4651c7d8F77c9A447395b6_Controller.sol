...
contract OwnableContractOwner is OwnableContract {
    ...
    function callClaimOwnership(OwnableContract contractToOwn)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function callReclaimToken(OwnableContract ownedContract, ERC20 _token)
        external
        onlyOwner
        returns (bool)
    {
...
// File: contracts\token\WrappedToken.sol
    ...
    OwnableContractOwner
{
    ...
    function setMembers(address _members)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function burn(uint256 value) external onlyFactory returns (bool) {
        require(!token.paused(), "token is paused.");
        token.burn(value);
        return true;
    }
