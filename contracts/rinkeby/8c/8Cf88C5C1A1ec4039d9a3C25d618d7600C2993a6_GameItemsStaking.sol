...
import "./IERC1155721Transferrable.sol";
import "./staking/NftStaking.sol";

contract GameItemsStaking is NftStaking {
    ITransferrableGameItems private immutable _gameItemsContract;

    /**
     * Constructor.
     * @dev See {NftStaking-constructor}
    ...
    constructor(
        ...
        ITransferrableGameItems whitelistedNftContract_,
    ...
    {
        _gameItemsContract = whitelistedNftContract_;
    }
    ...
    function _validateAndGetNftWeight(uint256 tokenId)
        ...
        returns (uint64)
    {
        uint64 weight = _gameItemsContract.tokenWeight(tokenId);
        require(weight != 0, "GameItemsStaking: invalid token weight");

        return weight;
    }
}

interface ITransferrableGameItems is IGameItems, IERC1155721Transferrable {}
...
// This contract is based on https://github.com/animocabrands/ethereum-contracts-nft_staking/blob/master/contracts/staking/NftStaking.sol
// Solidity upgraded to ^0.8.0 and openzeppelin imports is updated to match version 4.3.1
...
abstract contract NftStaking is ERC1155TokenReceiver, Ownable {
    ...
    function withdrawLostCycleRewards(
        ...
        require(
            cycle < _getCycle(block.timestamp),
            "NftStaking: non-past cycle"
        );
        require(
    ...
    function _updateHistory(
        ...
        if (historyLength != 0) {
            ...
            Snapshot storage snapshot_ = history[snapshotIndex];
            snapshotStake = uint256(
                int256(uint256(snapshot_.stake)).add(stakeDelta)
            ).toUint128();

            if (snapshot_.startCycle == currentCycle) {
                // update the snapshot if it starts on the current cycle
                snapshot_.stake = snapshotStake;
