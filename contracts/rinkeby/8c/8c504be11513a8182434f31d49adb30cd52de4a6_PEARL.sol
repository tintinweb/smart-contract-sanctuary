...
- ✔️ Supply Name: Pearl of the Orient Seas
- ✔️ Symbol: $PEARL
- ✔️ Buy Tax per transaction: 1% (redistributes to all holders)
- ✔️ Sell Tax per transaction: 2% (redistributes to all holders)
- ✔️ Manual burning capability
- ✔️ Liquidity Fee per transaction: 1%
- ✔️ Send to Marketing Wallet per transaction: 1% (Send it on ETH)
- ✔️ Send to Dev Wallet per transaction: 1% (Send it on ETH)
- ✔️ Send to Charity Wallet per transaction: 1% (Send it on ETH)
- ✔️ Make teamFee adjustable
- ✔️ Implement dynamic sell logic
- ✔️ Swap taxFee with teamFee

        //For BSC_MAINNET: 0x10ED43C718714eb63d5aA57B78B54704E256024E
        //For BSC_TESTNET: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        //For RINKEBY: 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        //For PCS_SIM: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        //For ETH_MAINNET: 0xE592427A0AEce92De3Edee1F18E0157C05861564 (uniV3)
*/
...
contract PEARL is Context, IERC20, Ownable { // CHANGE AAANAME to nominal name
    using SafeMath for uint256;

    string private constant _name = "Pearl of the Orient Seas"; // CHANGE token name
    string private constant _symbol = "PEARL";   // CHANGE token symbol
    ...
    uint256 private constant _tTotal = 1000000000000 * 10**9; // CHANGE total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal; // Was private
    uint256 public _taxFee = 2;
    uint256 private _teamFee = 8;
    uint256 public _storedTeamFee = _teamFee;

    uint256 public _teamCutPct = 4;
    uint256 public _marketingCutPct = 1;
    uint256 public _charityCutPct = 2;
    uint256 public _liquidityCutPct = 1;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isAdmin[from] && !_isAdmin[from]) {
            ...
            if (!inSwap && swapEnabled && to == uniswapV2Pair) { // Selling
                require(amount <= balanceOf(uniswapV2Pair).mul(3).div(100) && amount <= _maxTxAmount);
                require(sellCooldown[from] < block.timestamp);
                if(firstSell[from] + (1 days) < block.timestamp) {
                    sellNumber[from] = 0;
                }
                if (sellNumber[from] == 0) { //CHANGEIT Adjust cooldown timings
                    _taxFee = 2;
                    sellNumber[from]++;
                    ...
                    _teamFee = 2;
                    sellNumber[from]++;
                    sellCooldown[from] = block.timestamp + (1800 seconds);
                }
                else if (sellNumber[from] == 2) {
                    _taxFee = 4;
                    _teamFee = 4;
                    sellNumber[from]++;
                    sellCooldown[from] = block.timestamp + (3600 seconds);
                }
                else if (sellNumber[from] == 3) {
                    _taxFee = 6;
                    _teamFee = 10;
                    sellNumber[from]++;
                    sellCooldown[from] = firstSell[from] + (14400 seconds);
                }
            }
    ...
    function sendETHToFee(uint256 beforeSplit) private {
        uint256 _totalTeamFeesExcludingLiquidity = _teamFee.sub(_liquidityCutPct);
