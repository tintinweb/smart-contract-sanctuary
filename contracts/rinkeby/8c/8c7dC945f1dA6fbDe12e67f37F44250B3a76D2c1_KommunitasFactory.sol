...
interface IKommunitasFactory{
    event ProjectCreated(address indexed adminProject, address project, uint index);
    ...
    function createProject(address, address, uint, uint, uint, uint, uint, uint[3] memory, uint[2] memory, uint, uint, uint) external returns (address);
...
contract KommunitasProject{
     ...
     * @param _adminProject Admin/owner of Project
    ...
    function initialize(
        ...
        require(_tge > booster[3].end, "Token Generation Event must be started after public round ended");
    ...
    function getTotalPurchase(address _user) public view returns(uint total){
        total = purchasePerRound[_user][1].add(purchasePerRound[_user][2]).add(purchasePerRound[_user][3]);
    }
    ...
    function getUserAllocToken(address _user) internal view returns(uint userAllocToken){
        ...
        } else if(boosterProgress() == 2){
            if(whitelist[_user] > 0){
                userAllocToken = whitelist[_user];
            } else{
                userAllocToken = getUserAllocation_d8(_user).mul(sale.sub(booster[1].achieve).sub(whitelistTotalAlloc())).div(10 ** 8);
            }
        } else if(boosterProgress() == 3){
    ...
    function amountInCalc(uint _tokenReceived, uint _amountIn, address _user) internal view returns(uint amountInFinal, uint tokenReceivedFinal){
        if(boosterProgress() < 3){
            if(whitelist[_user] == 0){
                require(sale.sub(sold()).sub(whitelistTotalAlloc()) > 0, "You are out of token sale");
    ...
    function amountInCalcInner(address _user, uint _tokenReceived, uint _amountIn) internal view returns(uint amountInFinal, uint tokenReceivedFinal){
        ...
        tokenReceivedFinal = _tokenReceived;
        if(boosterProgress() < 3){
            ...
            if(purchasePerRound[_user][boosterProgress()].add(_tokenReceived) > getUserAllocToken(_user)){
                tokenReceivedFinal = getUserAllocToken(_user).sub(purchasePerRound[_user][boosterProgress()]);
                amountInFinal = tokenReceivedFinal.mul(booster[boosterProgress()].price).div(10 ** tokenProjectDecimals);
            }
        }else{
            ...
            if(_tokenReceived > maxPublicBuy){
                tokenReceivedFinal = maxPublicBuy;
    ...
    function moveFund() public {
        require(block.timestamp > booster[3].end, "Still in progress");
    ...
    function buyTokenByETH(address[] calldata _path) payable isBoosterProgress isNotPaused public {
        ...
        require(getUserAllocToken(msg.sender) > 0, "You never staked KOM before");
        ...
        if(!isBuyer(msg.sender)){
            ...
            bytes memory userRecipient = bytes(recipient[msg.sender]);
            if(userRecipient.length == 0){
                recipient[msg.sender] = toAsciiString(msg.sender);
            }
        }else{
        ...
        if(ethFinal < msg.value){
            IWETH(factory.weth()).transfer(msg.sender, msg.value.sub(ethFinal));
        }
    ...
    function buyTokenByToken(uint _amountIn, address[] calldata _path) isBoosterProgress isNotPaused public {
        ...
        require(_path[0] == factory.allPayments(factory.getPaymentIndex(_path[0])), "Token payment not found");
        require(_path[1] == address(payment), "Must swap to accepted token");
        require(getUserAllocToken(msg.sender) > 0, "You never staked KOM before");
     ...
     */
    function setRecipient(string memory _recipient) public {
...
contract KommunitasFactory is IKommunitasFactory{
    ...
    mapping(address => address) public override getProject;
    mapping(address => Payment) public getPayment;
    ...
    function createProject(
        ...
        require(_payment != address(0) && _adminProject != address(0), "Can't assign to address(0)");
        require(getProject[_adminProject] == address(0), "Project exist");
        require(_payment == allPayments[getPaymentIndex(_payment)], "Payment not acceptable");
        
        project = address(new KommunitasProject{salt: keccak256(abi.encodePacked(_adminProject))}());

        getProject[_adminProject] = project;
        allProjects.push(project);
        
        KommunitasProject(getProject[_adminProject]).initialize(
            _payment,
            _adminProject,
        ...
        emit ProjectCreated(_adminProject, project, allProjects.length-1);
    }
    ...
    function setPayment(address _token) public override onlyOwner{
        ...
        if(allPayments.length > 0) {
            require(_token != allPayments[getPayment[_token].paymentIndex], "Token payment is exist");
        }
        
        allPayments.push(_token);
        getPayment[_token].paymentIndex = allPayments.length-1;
    }
    ...
    function removePayment(address _token) public override onlyOwner{
        require(_token != address(0), "Can't set to address(0)");
        require(_token == allPayments[getPayment[_token].paymentIndex], "Token payment not found");
        
        uint indexToDelete = getPayment[_token].paymentIndex;
        address addressToMove = allPayments[allPayments.length-1];
        
        allPayments[indexToDelete] = addressToMove;
        getPayment[addressToMove].paymentIndex = indexToDelete;
        
        allPayments.pop();
        delete getPayment[_token];
    }
    ...
    /**
     * @dev Get index of payment token
     * @param _token Token address
     */
    function getPaymentIndex(address _token) public view override returns (uint index) {
        if(_token == allPayments[getPayment[_token].paymentIndex]){
            index = getPayment[_token].paymentIndex;
        }
    }
}