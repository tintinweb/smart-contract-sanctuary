    ...
    */
    function mintPack(string[] memory tokenURI, uint256[] memory mintedImg) public payable saleIsOpen returns (uint256) {
        ...
        for(uint256 j = 0; j < mintedImg.length; j++) {
            require(!ImgStatus[mintedImg[j]], "This polar has already been created");
    ...
    function mintPackIndividual(string memory tokenURI) public payable onlyOwner returns (uint256) {
        // to get id to mint
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
