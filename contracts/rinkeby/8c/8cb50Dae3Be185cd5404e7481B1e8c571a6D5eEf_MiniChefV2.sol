...
interface IMasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. RADIO to distribute per block.
        uint256 lastRewardBlock;  // Last block number that RADIO distribution occurs.
        uint256 accSushiPerShare; // Accumulated RADIO per share, times 1e12. See below.
    }
...
/// @notice The (older) MasterChef contract gives out a constant number of RADIO tokens per block.
/// It is the only address with minting rights for RADIO.
...
contract MiniChefV2 is BoringOwnable, BoringBatchable {
    ...
    /// `rewardDebt` The amount of RADIO entitled to the user.
    struct UserInfo {
        uint256 amount;
        int256 rewardDebt;
    }
    ...
    /// Also known as the amount of RADIO to distribute per block.
    struct PoolInfo {
    ...
    /// @notice Address of RADIO contract.
    IERC20 public immutable RADIO;
    ...
    /// @param _sushi The RADIO token contract address.
    ...
    /// @notice Update the given pool's RADIO allocation point and `IRewarder` contract. Can only be called by the owner.
    ...
    /// @notice Sets the radio per second to be distributed. Can only be called by the owner.
    /// @param _sushiPerSecond The amount of Radio to be distributed per second.
    ...
    /// @notice View function to see pending RADIO on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user Address of user.
    /// @return pending RADIO reward for a given user.
    ...
    /// @notice Deposit LP tokens to MCV2 for RADIO allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of RADIO rewards.
    function harvest(uint256 pid, address to) public {
        ...
        // Interactions
        if (_pendingSushi != 0) {
            RADIO.safeTransfer(to, _pendingSushi);
        }
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
    /// @param to Receiver of the LP tokens and RADIO rewards.
...
contract ComplexRewarderTime is IRewarder,  BoringOwnable{
    ...
    /// @notice Sets the radio per second to be distributed. Can only be called by the owner.
    /// @param _rewardPerSecond The amount of Radio to be distributed per second.
...
contract MasterChefV2 is BoringOwnable, BoringBatchable {
    ...
    /// @param _MASTER_CHEF The RadioSwap MCV1 contract address.
    /// @param _sushi The RADIO token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting rights for RADIO.
    ...
    /// @notice Calculates and returns the `amount` of RADIO per block.
    ...
    /// @notice Harvests RADIO from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to this MCV2 contract.
