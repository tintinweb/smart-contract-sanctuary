...
contract Lootchadores is ERC721Enumerable, Ownable, ERC165Storage, PaymentSplitter {
    ...
    function getCharismaStr(uint256 abilityId) public view returns (string memory) {
        return extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getCharisma(abilityId));
    }

    function getCharismaInt(uint256 abilityId) public view returns (uint256) {
        return safeParseInt(getCharismaStr(abilityId), 0);
    }
