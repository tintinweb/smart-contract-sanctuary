  ...
  ERC1155HolderUpgradeable
{
  ...
  event Imported(address indexed tokenAddress, address indexed from, uint256 tokenId);
  event Exported(address indexed tokenAddress, address indexed to, uint256 tokenId);
  ...
  function onERC721Received(address, address from, uint256 tokenId, bytes memory) public virtual override(ERC721HolderUpgradeable) returns (bytes4) {
    ...
    emit Imported(tokenAddress, from, tokenId);
    return this.onERC721Received.selector;
  }

  function onERC1155Received(address, address from, uint256 tokenId, uint256, bytes calldata) public virtual override(ERC1155HolderUpgradeable) returns (bytes4) {
    address tokenAddress = msg.sender;
    emit Imported(tokenAddress, from, tokenId);
    return this.onERC1155Received.selector;
  }

  function exportToken(bytes4 interfaceId, address tokenAddress, address to, uint256 tokenId) public {
    require(hasRole(MODERATOR_ROLE, msg.sender), "TokenBridgeUpgradeable: must be moderator");

    address from = address(this);
    _exportToken(interfaceId, tokenAddress, from, to, tokenId);
  }

  function exportTokenWithSignature(bytes4 interfaceId, address tokenAddress, uint256 tokenId, string memory nonce, bytes memory signature) public {
    require(verifySignature(interfaceId, tokenAddress, tokenId, nonce, signature), "TokenBridgeUpgradeable: invalid signature");
  ...
  function mintCollectibleToken(bytes4 interfaceId, address tokenAddress, uint256 tokenId, string memory nonce, bytes memory signature) public {
    require(verifySignature(interfaceId, tokenAddress, tokenId, nonce, signature), "TokenBridgeUpgradeable: invalid signature");
  ...
  function verifySignature(bytes4 interfaceId, address tokenAddress, uint256 tokenId, string memory nonce, bytes memory signature) public returns (bool) {
    ...
    bytes32 hash = ethSignedHash(hashToSign(interfaceId, tokenAddress, from, to, tokenId, nonce, bridge));
    address signer = recoverSigner(hash, signature);
    require(signer != address(0) && hasRole(MODERATOR_ROLE, signer), "TokenBridgeUpgradeable: signer must be moderator");

    return true;
  }

  function hashToSign(bytes4 interfaceId, address tokenAddress, address from, address to, uint256 tokenId, string memory nonce, address bridge) public pure returns (bytes32) {
    return keccak256(abi.encodePacked(interfaceId, tokenAddress, from, to, tokenId, nonce, bridge));
  }
   ...
   * Internal functions
   */
  function _exportToken(bytes4 interfaceId, address tokenAddress, address from, address to, uint256 tokenId) internal virtual {
    if (interfaceId == INTERFACE_ID_IERC721) {
      ...
      IERC1155Upgradeable(tokenAddress).safeTransferFrom(from, to, tokenId, 1, "");
    }
    else {
      revert("TokenBridgeUpgradeable: token contract interface not supported");
    }

    emit Exported(tokenAddress, to, tokenId);
  }
