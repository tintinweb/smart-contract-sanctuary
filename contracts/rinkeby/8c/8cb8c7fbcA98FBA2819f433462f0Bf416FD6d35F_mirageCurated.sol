...
contract mirageCurated is CustomERC721Metadata {
  ...
  function mint(address _to, uint256 _projectId, address _by) external returns (uint256 _tokenId) {
   ...
   function earlyMint(address _to, uint256 _projectId, address _by) external returns (uint256 _tokenId) {
      require(isMintWhitelisted[msg.sender], "Must mint from whitelisted minter contract.");
      require(projects[_projectId].early || _by == projectIdToArtistAddress[_projectId], "Project not in early mint phase");
      require(!projects[_projectId].paused || _by == projectIdToArtistAddress[_projectId], "Purchases are paused.");
  ...
  function _mintToken(address _to, uint256 _projectId) internal returns (uint256 _tokenId) {
      uint256 tokenIdToBe = (_projectId * ONE_MILLION) + projects[_projectId].artworks + 50; //adding 50 in order to skip pieces claimable by sentient members
   ...
   function claimSentient(uint256 _projectId, uint256 membershipId) public {
      require(projects[_projectId].active || projects[_projectId].early, "Project must exist, and be active or in early mint state");
  ...
  function sentientMint(address _to, uint256 _projectId, uint256 _membershipId) internal returns (uint256 _tokenId) {
      uint256 tokenIdToBe = (_projectId * ONE_MILLION) + _membershipId;
      //projects[_projectId].artworks = projects[_projectId].artworks.add(1); // commented out b/c will interfere with tokenIdToBe in _mintToken function
      _mint(_to, tokenIdToBe);
      tokenIdToProjectId[tokenIdToBe] = _projectId;
      projectIdToTokenIds[_projectId].push(tokenIdToBe);
      emit Mint(_to, tokenIdToBe, _projectId);
  ...
  function toggleProjectIsActive(uint256 _projectId) public onlyWhitelisted {
   ...
   function toggleEarlyMint(uint256 _projectId) public onlyWhitelisted {
      projects[_projectId].early = !projects[_projectId].early;
  }
  ...
  function addProject(string memory _projectName, string memory tokenURI, address _artistAddress, uint256 _pricePerTokenInWei, uint256 _maxArtworks, uint256 _maxEarly) public onlyWhitelisted {
      ...
      projects[projectId].early = false;
  ...
  function updateProjectMaxArtworks(uint256 _projectId, uint256 _maxArtworks) onlyArtistOrWhitelisted(_projectId) public {
      require((!projects[_projectId].locked || _maxArtworks<projects[_projectId].maxArtworks), "Only if unlocked");
   ...
   function updateProjectMaxEarly(uint256 _projectId, uint256 _maxEarly) onlyArtistOrWhitelisted(_projectId) public {
      require((!projects[_projectId].locked || _maxEarly<projects[_projectId].maxEarly), "Only if unlocked");
  ...
  function updateProjectBaseURI(uint256 _projectId, string memory _newBaseURI) onlyArtistOrWhitelisted(_projectId) public {
  ...
  function projectTokenInfo(uint256 _projectId) view public returns (address artistAddress, uint256 pricePerTokenInWei, uint256 artworks, uint256 maxArtworks, uint256 maxEarly, address additionalPayee, uint256 additionalPayeePercentage, bool active, bool early, string memory currency) {
      ...
      early = projects[_projectId].early;
