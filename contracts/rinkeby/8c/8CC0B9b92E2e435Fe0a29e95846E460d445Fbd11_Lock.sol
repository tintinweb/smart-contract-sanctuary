...
// TODO Add events
// TODO Allow or not an user/beneficiary transfer their NFTs
// TODO view functions

contract Lock is BaseGovernanceWithUserUpgradable, ERC721Upgradeable {
    ...
    function initialize(
        ...
        initializer 
        {
            ...
            __Lock_init(_schedule, _depositManager, _splitManager, _tokenERC20);
            __init_beneficiaries( _data, _lockedAmount, _canAddOrRemove);
    }
    ...
    function __init_beneficiaries(bytes calldata _data, uint _lockedAmount, bool _canAddOrRemove) internal initializer {

        require(tokenERC20.allowance(_msgSender(), address(this)) >= _lockedAmount, "ERROR: Not approve enought tokens to send");
        tokenERC20.safeTransferFrom(_msgSender(), address(this), _lockedAmount);

        depositManager.addDeposits(_data, _lockedAmount);
        canAddOrRemove = _canAddOrRemove;
    }
    ...
    function addBeneficiaries(bytes calldata data, uint256 totalAmount) external onlyBeneficiaryManager {
        require(canAddOrRemove, "ERROR: Cannot add new beneficiaries");
        require(tokenERC20.allowance(_msgSender(), address(this)) >= totalAmount, "ERROR: Not approve enought tokens to send");
    ...
    function removeBeneficiaries(bytes calldata data) external onlyBeneficiaryManager {
        require(canAddOrRemove, "ERROR: Cannot remove beneficiaries");
        uint refundAmount = depositManager.removeDeposits(data);
        tokenERC20.safeTransfer(_msgSender(), refundAmount);
    }
     ...
     * @param ownershipProof Additional data which may be required by DepositManager to verify nftId ownership
     */
    function claimUnlocked(uint256 nftId, uint256 amountToClaim, bytes calldata ownershipProof) external {
        (bool _success, uint _ID) = _isApprovedOrOwner(nftId, _msgSender(), ownershipProof);
        require(_success, "ERROR: You are not the owner or are approved for this NFT");
        (, uint additionTime, uint initialAmount, ) =  depositManager.getProperties(_ID);
        uint unlockedAmount = schedule.unlockedAmount(additionTime, initialAmount);
        require(unlockedAmount >= amountToClaim, "ERROR: Not enought free tokens - Schedule");
        depositManager.updateClaimedAmount(_ID, amountToClaim);
        tokenERC20.safeTransfer(_msgSender(), amountToClaim);
    }
     ...
     * @param ownershipProof Additional data which may be required by DepositManager to verify nftId ownership
     */
    function split(uint originId, uint[] memory splitParts, address[] memory addresses, bytes calldata ownershipProof) external {
        (bool _success, uint _ID) = _isApprovedOrOwner(originId, _msgSender(), ownershipProof);
        require(_success, "ERROR: You are not the owner or are approved for this NFT");
        depositManager.split(_ID, splitParts, addresses);
    }
    /**
     */
...
abstract contract BaseGovernanceWithUserUpgradable is Initializable, ContextUpgradeable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function __BaseGovernanceWithUser_init() internal initializer {
        __Context_init_unchained();
        __ERC165_init_unchained();
    ...
    function __BaseGovernanceWithUser_init_unchained() internal initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());   // Grant DEFAULT_ADMIN to creator. Other role management scan be performed elswhere
    }

    function _authorizeUpgrade(address newImplementation) internal virtual override onlyRole(GOVERNANCE_ROLE) {}
...
interface IDepositManager {
    ...
    /**
     * @notice Get all the properties from an ID-NFT.
     * @param id NFT ID to get the properties
     * @return Beneficiary or owner of the ID-NFT 
     * @return Timestamp of when the beneficiary was added
     * @return The initial amount that was locked on this ID-NFT
     * @return The amount claimed so far 
     */
    function getProperties(uint id) external view returns(address, uint, uint, uint);

    /**
     * @notice Add a deposit with beneficiaries
     * @param data ABI-encoded data of beneficiaries (arrays of addresses and amounts, etc - specific to IDepositManager)
     * @param totalAmount Total amount of tokens to be locked for additional beneficiaries
     */
    ...
    /**
     * @notice Remove a deposit with beneficiaries
     * @param data ABI-encoded data of beneficiaries (arrays of addresses and amounts, etc - specific to IDepositManager)
     * @return The amount to refund
     */
    function removeDeposits(bytes calldata data) external returns(uint);
    ...
    /**
     * @notice Update the claimed amount to a benefiary and an NFT ID
     * @param id NFT ID
     * @param amountToClaim The amount to update
     */
     ...
     * @dev data param will be empty if the DM contract is not using the MerkleTree
     * @param Id NFT ID
     * @param beneficiary The beneficiary of the ID
...
interface IUnlockSchedule {
    ...
    function unlockedAmount(uint256 lockStartTimestamp, uint256 initialAmount) external returns(uint256 _unlockedAmount);
