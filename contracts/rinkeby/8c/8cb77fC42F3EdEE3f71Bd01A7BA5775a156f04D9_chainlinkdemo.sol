...
contract chainlinkdemo is ChainlinkClient{
    ...
    function setMintRate() public returns (uint256){
        //update the amount of tokens in the pool, then update to the new token mint rate
        updateTokenPool();
        tokenMintRate = price;
        return tokenMintRate;
    }

    function updateTokenPool() public returns (uint256){
        ...
        if(lastUpdateBlock >= startBlock){
            uint256 blockDifference = currentBlock-lastUpdateBlock;
            pendingTokens = tokenMintRate*blockDifference;
            tokenPool = tokenPool+pendingTokens;
        }
        return tokenPool;
    }
