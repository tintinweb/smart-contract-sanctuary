...
contract MockCurveSwap {
        ...
        returns (uint256)
    {
        if (flag == 0) {
            ...
            if (tokenId == 1) {
                return tokenAmount / 981524505583;
            }

            if (tokenId == 2) {
                return tokenAmount / 981481275472;
            }
        }
...
contract MockConvexBooster is Ownable {
    ...
    constructor() public {
        curve3CRV = new MockCurveLP("Mock Curve LP-3CRV", "MCLP", msg.sender);
    ...
    function demoAddPool() public {
        poolInfo.push(
            PoolInfo({
                swapAddress: address(new MockCurveSwap(0)),
                targetPid: 9, // convex pool 9 = 3crv (Curve.fi DAI/USDC/USDT) https://etherscan.io/address/0x6c3F90f043a72FA612cbac8115EE7e52BDe6E490
                lpToken: address(curve3CRV),
                staker: address(0),
                rewardPool: address(new ConvexRewardPool(address(this))),
                stash: address(0)
            })
        );
        ...
        poolInfo.push(
            PoolInfo({
                swapAddress: address(new MockCurveSwap(1)),
                targetPid: 25, // convex pool 25 = steCRV (Curve.fi ETH/stETH) https://etherscan.io/address/0x06325440D014e39736583c165C2963BA99fAf14E
    ...
    function demoMintCurve3CRV() public {
        curve3CRV.mint(1000000 * curve3CRV.decimals());
    }

    function demoMintCurveSteCRV() public {
        curveSteCRV.mint(1000000 * curveSteCRV.decimals());
    }
    ...
    function liquidate(
        ...
        address underlyToken = ICurveSwap(pool.swapAddress).coins(
            uint256(_coinId)
        );
