...
contract GraveyardSale is ERC721, Ownable, RBAC {
  ...
  uint256 public constant WALLET_CAP = 30;
  uint256 public constant maxPremintPurchase = 3;
  ...
  function premint(uint256 _toPremint) external onlyPreminter() payable
  {
    ...
    require((balanceOf(msg.sender) + _toPremint) <= 3, "Limit of 3 NFTs during presale");
    ...
    // Checks that final total supply of tokens (if sale occurs) would not exceed the max supply limit
    require(
      totalSupply().add(_toPremint) <= MAX_NFTS,
      'Purchase would exceed max supply of NFTs'
    );
    /**
         Checks that the buyer sends at least the total price of the transaction (ETH), calculated as number
