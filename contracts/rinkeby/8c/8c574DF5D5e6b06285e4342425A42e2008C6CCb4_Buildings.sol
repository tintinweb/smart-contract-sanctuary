...
contract Buildings is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private effect = [
        "Recover all MP",
        "Recover 10% MP",
        "Recover 10% HP",
        "Recover all HP",
        "Time between actions is lengthened",
        "Repair all equipments",
        "Remove all negative effects",
        "Recover all HP for pets",
        "Recover 10% HP for pets",
        "Recover all MP for pets",
        "Recover 10% MP for pets",
        "Increase the success rate of weapon forging"
    ];
    ...
    string[] private mailbox = [
        "Mail Slot Mailbox",
        "Wall Mount Mailbox",
        "Post Mount Mailbox",
        "Column Mount Mailbox",
        "Package Drop Boxes Mailbox",
        "Freestanding Mailbox",
        "No  Mailbox"
    ];

    string[] private specialName = [
        "The Golden Gate",
        "The Hidden Gem",
        "The Magic Lord",
        "The Iron Ground",
    ...
    function getMaterial(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "MATERIAL", material);
    }
    ...
    function getExterior(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "EXTERIOR", exterior);
    }

    function getDoor(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "DOOR", door);
    }

    function getEffect(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "EFFECT", effect);
    }

    function getRoof(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "ROOF", roof);
    }

    function getMailbox(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "MAILBOX", mailbox);
    }
    ...
    function pluck(
        ...
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("BUILDING") && greatness > 14) {
            output = string(abi.encodePacked('"', specialName[rand % specialName.length], '" ', output));
        }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[1] = getBuilding(tokenId);

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = getMaterial(tokenId);

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = getStyle(tokenId);

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = getExterior(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = getDoor(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getRoof(tokenId);

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = getMailbox(tokenId);

        parts[14] = '</text><text x="10" y="160" class="base">';

        parts[15] = getEffect(tokenId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Building #',
                        toString(tokenId),
                        '", "description": "Buildings are randomized generated and stored on chain. Images and other functionality are intentionally omitted for others to interpret. Feel free to use buildings in any way you want. Inspired and compatible with Loot (for Adventurers) and Characters (for Adventurers).", "image": "data:image/svg+xml;base64,',
    ...
    function withdraw() public onlyOwner {
        payable(0x5D708A3b3918C3acE4D17851884672076822FFCf).transfer(address(this).balance);
    }
    ...
    constructor() ERC721("Buildings", "CHAR") Ownable() {}
}
