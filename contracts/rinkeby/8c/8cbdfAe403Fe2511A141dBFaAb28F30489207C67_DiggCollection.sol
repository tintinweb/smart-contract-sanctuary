...
contract DiggCollection is PoolTokenWrapper, Ownable, Pausable {
	using SafeMath for uint256;
	...
	function lastUpdatedEpoch(address account, uint256 pool) public view returns (uint256) {
		return pools[pool].lastUpdateTime[account];
	}
	...
	function earned(address account, uint256 pool) public view returns (uint256) {
        ...
        if(poolBalance == 0) return p.points[account];


        // PowerLib._generalPower((multiplyer.mul(poolBalance) « 127).div(1e18), 10000/4)
		// // psuedomath: rewardRate = (1.35 * (balance * 1000)^(1/4)) / 86400
		// (uint256 mantissa, uint8 exponent) = PowerLib.power(multiplyer.mul(poolBalance), uint256(1e18), rootN, rootD);
		// uint256 noCoefficient = mantissa.mul(uint256(1 ether)).div(uint256(1) << uint256(exponent));
		// uint256 rewardRate = noCoefficient.mul(rootN).div(rootD).div(86400);

        uint256 rewardRate = power((multiplyer.mul(poolBalance) << 127).div(1e18), 10000/4).div(84600);
