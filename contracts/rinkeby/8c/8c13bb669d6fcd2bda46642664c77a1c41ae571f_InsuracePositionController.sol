...
contract InsuracePositionController is AbstractController {
    ...
    address[] public _stakingAssetAddress;
    ...
    function __InsuracePositionController_init(
        ...
        address[] memory _stkingAssetsAddresses,
        ...
        _insurToken = _rewardController.insur();
        _stakingAssetAddress = _stkingAssetsAddresses;
        _lpToken = _stakingController.tokenToLPTokenMap(_stakingAssetAddress[0]);
        base.safeApprove(address(_indexAddress), PreciseUnitMath.MAX_UINT_256);
        IERC20(_stakingAssetAddress[0]).approve(
            address(_stakingController),
            PreciseUnitMath.MAX_UINT_256
        );
        IERC20(_lpToken).approve(address(address(_stakersPool)), PreciseUnitMath.MAX_UINT_256);
        _stakes = 0;
    }

    function stake(uint256 _amount) external override onlyIndex {
        address _stakingAsset = _stakingAssetAddress[0];
        ...
        if (address(base) != _stakingAsset) {
            ...
            _swapInsuraceAsset(_amount, address(base), _stakingAsset);
        }
    ...
    */
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        uint256 baseTokenAmount = _swapInsuraceAsset(
            _unstakedAmount,
            _stakingAsset,
            address(base)
        );
        ...
        // Return to index
        index.depositInternal(baseTokenAmount);
    ...
    /**
        @dev Withdraw staking rewards in INSUR token.

        ⚠️ Note: The reward for the staking will be INSUR tokens
          calculated at a per-block level according to the per-block
          reward configured. The reward can be unlocked any time,
          but subject to per-block linear vesting over 7 days.

          Each time you initialize another unlock action,
          the vesting time will be reset to 7 days.
     */
    function unluckRewards() external {
        _rewardController.unlockReward(
            _stakingAssetAddress, // _tokenList
    ...
    function withdrawRewards() external override {
        (uint256 withdrawableReward, uint256 vestingReward) = _rewardController.getRewardInfo();

        require(withdrawableReward != 0, "InsuracePositionController: IPC9");

        _rewardController.withdrawReward(withdrawableReward);

        uint256 rewardsBalance = IERC20(_insurToken).balanceOf(address(this));

        require(rewardsBalance > 0, "InsuracePositionController IPC10");

        //Calculate fee
        uint256 _feeAmountScale = rewardsBalance.mul(feeInfo.successFeePercentage);
        ...
        require(rewardsBalance >= _fee, "InsuracePositionController: IPC11");

        rewardsBalance = rewardsBalance.sub(_fee);

        //transfer fee in INSUR to Index
        IERC20(_insurToken).transfer(feeInfo.feeRecipient, _fee);

        // swap INSUR into base
        _checkApprovals(IERC20(_insurToken), address(_uniswapRouter), rewardsBalance);
        uint256 _baseTokenAmount = _swapInsuraceAsset(
            rewardsBalance,
            address(_insurToken),
            address(base)
        );
    ...
    {
        _resultAmounts[0] = _currentlyAtStake(_stakingAssetAddress[0]).add(_amount);
    }
    ...
    function _currentlyAtStake(address _product) internal view returns (uint256) {
        return _stakingController.totalStakedCapPT(_product);
    }
    ...
    function netWorth() external view override returns (uint256) {
        ...
        return
            _priceFeed.howManyDAIsInInsur(
                _currentlyAtStake(_stakingAssetAddress[0]).add(_rewards)
            );
    }

    function productsInfo() external view override returns (ProductInfo[] memory _infos) {
        _infos[0].productAddress = _stakingAssetAddress[0];
        _infos[0].stakingAmount = _currentlyAtStake(_stakingAssetAddress[0]);
    }
    ...
    function maxUnstake() external view override returns (uint256 _maxUnstake) {
        _maxUnstake = _stakingController.maxUnstakeAmtPT(_stakingAssetAddress[0]);
    }
...
interface IPriceFeed {
    ...
    function howManyDAIsInInsur(uint256 insurAmount) external view returns (uint256);
