...
contract AlphaVault is IUniswapV3MintCallback, ReentrancyGuard {
    ...
    ) external nonReentrant onlyKeeper notPaused {
    ...
    function burn() external nonReentrant onlyKeeper notPaused {
        (uint128 totalLiquidity, , , , ) = position(lastTickLower, lastTickUpper);
        require(
        ...
        uint256 amount0 = burned0.add(fees0);
        uint256 amount1 = burned1.add(fees1);
        emit BurnAndCollect(
            lastTickLower,
            lastTickUpper,
            totalLiquidity,
    ...
    function suspend() external {
        ...
        require(
            (msg.sender == keeper || msg.sender == admin),
            "only keeper and admin can suspend"
        );
        paused = true;
