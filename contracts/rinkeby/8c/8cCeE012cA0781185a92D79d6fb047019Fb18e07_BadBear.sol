...
//     constructor() ERC721("Bad Bear NFT1", "BADBEAR1") {
//         _initializeEIP712("Bad Bear NFT1");
//     }
...
contract BadBear is ERC721, Ownable, ContextMixin {
    using SafeMath for uint256;

    uint256 private constant MAX_SUPPLY = 5555;

    address private MISHKA2 = 0x72D7b17bF63322A943d4A2873310a83DcdBc3c8D;
    uint256 private m_MskMiniumHold = 500000 * 10**18;

    uint256 private m_PreMintPrice = 1 * 10**17; // 0.1Eth
    uint256 private m_PublicMintPrice = 2 * 10**17; // 0.1Eth
    ...
    uint256 private m_TokenId = 0;

    string private m_baseURI;
    string private m_ContractURI;

    constructor() ERC721("Bad Bear NFT", "BADBEAR") {
        _mint(address(this), 0);
        m_TokenId = m_TokenId + 1;
    }
    ...
    function mintBadBear(uint256 numberOfTokens) public payable {
        require(m_IsMintable, "Sale must be active to mint Bad Bear");

        require(numberOfTokens > 0, "Can't mint");

        require(
            totalSupply().add(numberOfTokens) <= MAX_SUPPLY,
            "Purchase would exceed max supply of Bad Bears"
        );

        IERC20 mishkaV2 = IERC20(MISHKA2);
        require(
            mishkaV2.balanceOf(msg.sender) >= m_MskMiniumHold,
            "Need to hold some amount."
        );

        if (m_IsPublic) {
            require(
                msg.value >= m_PublicMintPrice.mul(numberOfTokens),
                "Ether value sent is not correct"
            );
        } else {
            require(m_WhiteList[msg.sender], "You are not in white list");
            require(
                msg.value >= m_PreMintPrice.mul(numberOfTokens),
                "Ether value sent is not correct"
            );
        }
    ...
    // ######## Mishka V2 #########
    function setMishkaV2(address _address) external onlyOwner {
        MISHKA2 = _address;
    }

    function setMskMiniumHold(uint256 _amount) external onlyOwner {
        m_MskMiniumHold = _amount.mul(10**18);
    }

    function getMskMiniumHold() external view returns (uint256) {
        return m_MskMiniumHold.div(10**18);
    }
}
