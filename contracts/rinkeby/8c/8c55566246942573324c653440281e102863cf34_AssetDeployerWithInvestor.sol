...
contract AssetDeployerWithInvestor is Ownable {
    address public registry;
    address public coin; // ERC20 token (stablecoin; could be USDC)
     ...
     * @param _investor Address of investor which will receive ether
     * @param _ethAmount Total amount of ether the investor will receive
     * @param _coinAmount Total amount of coin the investor will receive
     */
    function deployAssetWithThresholdInvestor(
        uint _assetId,
        uint[] memory _projectIds,
        uint[] memory _fractions,
        ...
        if (_investor != address(0)) {
            // contract is owner, so we can call addThresholdInvestor
            asset.addThresholdInvestor(_investor, _ethAmount, _coinAmount);
        }
     ...
     * fraction - more the fund contract will receive)
     * @param _investor Address of investor which will receive ether
     * @param _fraction A fraction of income to pay to this investor
     */
    function deployAssetWithFractionInvestor(
        uint _assetId,
        uint[] memory _projectIds,
        uint[] memory _fractions,
        address payable _investor,
        ...
        ITokitRegistry(registry).registerAsset(_assetId, address(asset));
        if (_investor != address(0)) {
            // contract is owner, so we can call addFractionInvestor
            asset.addFractionInvestor(_investor, _fraction);
        }
        if (_salesAgent != address(0)) {
            asset.setSalesAgent(_salesAgent, _saEthThreshold, _saCoinThreshold, _saFraction);
        }
