  ...
  Ownable
{
  ...
  uint public diceWildcardMintFee = 0.05 ether;
  ...
  */
  ...
  function purchaseAsDiceHolder(
    uint[] memory deckMatchedDiceIds, uint[] memory wildcardDiceIds
  ...
  function purchaseWildcardGeneralSale(uint numForPurchase) public payable whenNotPaused{
  ...
  function requestRandomNumber() public onlyOwner returns (bytes32 requestId) {
      require(randomNumber == 0, "Random number is already set.");
  ...
  function beginDiceHolderSale() public onlyOwner {
    require(salePhase == 0, "Must be in phase 0");
    salePhase = 1;
  }

  function beginGeneralSale() public onlyOwner {
    require(salePhase == 1, "Must be in phase 1");
    salePhase = 2;
  }
  ...
  // We have decided not to utilize this on-chain, but it's still required
  // by the interface
  function yieldRate(uint256) external pure returns (uint256) {
    return 0;
  }
  ...
  */
  ...
  function setMaxDeedsPerAddress(uint n) public onlyOwner {
    maxDeedsPerAddress = n;
  }

  function setDiceDeckMintFee(uint _fee) public onlyOwner {
    diceDeckMintFee = _fee;
  } 

  function setDiceWildcardMintFee(uint _fee) public onlyOwner {
    diceWildcardMintFee = _fee;
  } 

  function setGeneralWildcardMintFee(uint _fee) public onlyOwner {
