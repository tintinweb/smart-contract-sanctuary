...
contract CoinDeed is ICoinDeed, AccessControl {
    ...
    function ready() onlyRole(DEFAULT_ADMIN_ROLE) onlyInState(DeedState.SETUP) external override {
        _checkIfDeedIsReady();
    }
    ...
    function buy(uint256 minAmountOut) onlyInState(DeedState.READY) external override {
        totalFee = coinDeedAddressesProvider.buy(
            pair,
            executionTime,
            deedParameters,
            totalSupply,
    ...
    function sell(uint256 minAmountOut) onlyInState(DeedState.OPEN) external override {
        ...
        coinDeedAddressesProvider.sell(pair, executionTime, minAmountOut, totalSupply);
        ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitStateChanged(state);
    }
    ...
    /** @notice Calling this function will have the caller pay off the appropriate amount of debt
      ...
      * TODO: Put more of this logic into the library and make the
      * intermediary library functions internal to save some gas
      * TODO: Conform to CEI */
...
library CoinDeedAddressesProviderUtils {
    ...
    function tokenRatio(
        ...
        uint8 decimalsA = feedRegistry.decimals(tokenA, Denominations.USD) + IERC20MetadataUpgradeable(tokenA).decimals();
        uint8 decimalsB = feedRegistry.decimals(tokenB, Denominations.USD) + IERC20MetadataUpgradeable(tokenB).decimals();
    ...
    function readyCheck(
        ...
        // Combining token A decimals and feed registry decimals here
        uint8 tokenADecimals = feedRegistry.decimals(tokenA, Denominations.USD) + IERC20MetadataUpgradeable(tokenA).decimals();
        uint8 dTokenDecimals = feedRegistry.decimals(coinDeedAddressesProvider.deedToken(), Denominations.USD);
        ...
        if (
            ...
            totalStake >=
            (10 ** dTokenDecimals) * // We want the stake required to be scaled to the dtoken decimals
            ...
            stakingMultiplier / BASE_DENOMINATOR // Staking multiplier
        )
        {
            return true;
        }
    ...
    function _validateBuyAndReturnManagementFee(
        ...
        if(totalSupply >= deedParameters.deedSize) {
