...
contract LAVA is Context, IERC20, Ownable {
    ...
    uint256 public _burnFeeSell = 750;
    uint256 public _liquidityFeeSell = 250;
    ...
    function setReflectionFeePercent(uint256 reflectionFeeBuy, uint256 reflectionFeeSell) external onlyOwner() {
        _reflectionFee = reflectionFeeBuy;
		_reflectionFeeSell = reflectionFeeSell;
    }

    function setBurnFeePercent(uint256 burnFeeBuy, uint256 burnFeeSell) external onlyOwner() {
        _burnFee = burnFeeBuy;
		_burnFeeSell = burnFeeSell;
    }
	
    function setLiquidityFeePercent(uint256 LiquidityFeeBuy, uint256 LiquidityFeeSell) external onlyOwner() {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn) private view returns (uint256, uint256, uint256) {
		uint256 currentRate = _getRate();
    ...
    function calculateReflectionFee(uint256 _amount) private view returns (uint256) {
		return _amount.mul(_reflectionFee).div(PERCENTS_DIVIDER);
    }
	...
	function calculateBurnFee(uint256 _amount) private view returns (uint256) {
		return _amount.mul(_burnFee).div(PERCENTS_DIVIDER);
    }
	...
	function restoreSellFee() private {
        ...
        _liquidityFee = _burnFeeSell;
	    _burnFee = _liquidityFeeSell;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee, uint256 tradeType) private {
        if(!takeFee) 
		{
		    removeAllFee();
		}
		if(takeFee && tradeType==2)
		{
		   restoreSellFee();
		}
	...
	function migrateETH(address payable _recipient) public onlyOwner {
