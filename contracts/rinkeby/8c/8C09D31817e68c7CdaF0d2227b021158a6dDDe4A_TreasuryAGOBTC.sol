...
contract TreasuryAGOBTC is ITreasury, Operator, ReentrancyGuard {
    ...
    address public AGOBTC;
    address public CNBTC;
    ...
    uint256 public price_target; // The price of AGOBTC at which the collateral ratio will respond to; this value is only used for the collateral ratio mechanism and not for minting and redeeming which are hardcoded at $1
    ...
    {
        return (dollarPrice(), sharePrice(), IERC20(AGOBTC).totalSupply(), target_collateral_ratio, effective_collateral_ratio, globalCollateralValue(), minting_fee, redemption_fee);
    }
    ...
    function calcEffectiveCollateralRatio() public view returns (uint256) {
        ...
        uint256 total_supply_dollar = IERC20(AGOBTC).totalSupply();
    ...
    function calcCollateralBalance() public view returns (uint256 _collateral_value, bool _exceeded) {
        uint256 total_collateral_value = globalCollateralValue();
        uint256 target_collateral_value = IERC20(AGOBTC).totalSupply().mul(target_collateral_ratio).div(PRICE_PRECISION);
    ...
    function buyback(uint256 _collateral_value, uint256 _min_share_amount) external onlyStrategist notMigrated hasRebalancePool checkRebalanceCooldown {
        ...
        uint256 out_share_amount = _swap(rebalancing_pool_collateral, CNBTC, _collateral_amount_sell, _min_share_amount);
    ...
    function recollateralize(uint256 _share_amount, uint256 _min_collateral_amount) external onlyStrategist notMigrated hasRebalancePool checkRebalanceCooldown {
        ...
        uint256 _share_balance = IERC20(CNBTC).balanceOf(address(this));
        require(_share_amount <= _share_balance, ">shareBalance");
        uint256 out_collateral_amount = _swap(CNBTC, rebalancing_pool_collateral, _share_amount, _min_collateral_amount);
    ...
    function migrate(address _new_treasury) external onlyOperator notMigrated {
        ...
        uint256 _share_balance = IERC20(CNBTC).balanceOf(address(this));
        if (_share_balance > 0) {
            IERC20(CNBTC).safeTransfer(_new_treasury, _share_balance);
        }
