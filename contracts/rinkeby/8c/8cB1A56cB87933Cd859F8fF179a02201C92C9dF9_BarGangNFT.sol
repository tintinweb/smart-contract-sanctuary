...
contract BarGangNFT is ERC721, Ownable {
    ...
    *******************************/
    ...
    uint MINT_MAX_PER_TRANSACTION = 10;
    ...
    uint256 PRESALE_MINT_AVAILABLE_TIME = 86400;    // presale is available only for 1 hour
    
    mapping(address => bool) private _whitelist;
    mapping(address => uint) private _countlist;

    event privateSaleSuccess(bool result);
    ...
    function requestPrivateSale(uint mintCount) external payable {
        ...
        require(secondsPassed < PRESALE_MINT_AVAILABLE_TIME, "Presale Minting is ended.");
        require(_whitelist[msg.sender], "This address is not included in whitelist.");
        require(_countlist[msg.sender] + mintCount <= PRIVATESALE_MAX_PER_USER, "Overflow 5 tokens");
    ...
    function requestPublicSale(uint mintCount) external payable {
        ...
        require(mintCount <= MINT_MAX_PER_TRANSACTION, "Overflow 10 tokens per transaction.");
    ...
    function setStage(uint256 value) public onlyOwner{
        ...
        _stagingValue = value;
        if (value == 1) {
            _presaleStartedAt = now;
        } else if (value == 2) {
            _publicStartedAt = now;
        } 
    }

    function getPresaleMintingAvailableTime() public view returns(uint256) {
        if(_presaleStartedAt > 0) {
            uint256 secondsPassed = 0;
            secondsPassed = now.sub(_presaleStartedAt);
            if(secondsPassed > PRESALE_MINT_AVAILABLE_TIME) {
                return 0;
            } else {
                return PRESALE_MINT_AVAILABLE_TIME - secondsPassed;
            }
        } else {
            return 0;
        }
    ...
    function setPresaleMintingAvailableTime(uint256 value) public onlyOwner {
        require(value >= 0, "Invalid value");
        PRESALE_MINT_AVAILABLE_TIME = value;
    }
