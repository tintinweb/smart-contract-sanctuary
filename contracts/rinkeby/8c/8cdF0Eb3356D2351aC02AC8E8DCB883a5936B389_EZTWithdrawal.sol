...
contract EZTWithdrawal {
    ...
    bytes32 public constant WITHDRAW_TYPEHASH = keccak256("Withdraw(address user,uint256 amount,uint256 nonce)");
    ...
    function withdraw(address user, uint256 amount, uint256 nonce, uint8 v, bytes32 r, bytes32 s) external {
        ...
        nonces[user] = nonce.add(1);
        
        emit Withdraw(user, amount, nonce);
    }
    
    function getDigest(address user, uint256 amount, uint256 nonce) public view returns (bytes32 domainSeparator, bytes32 structHash, bytes32 digest) {
        domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(CONTRACT_NAME)), getChainId(), address(this)));
        structHash = keccak256(abi.encode(WITHDRAW_TYPEHASH, user, amount, nonce));
