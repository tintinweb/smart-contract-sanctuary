...
contract BatchDistributor {
    ...
    /**
     * @dev The function `distributeEther` uses `calldata` in order to ensure
     * immutability of the parameters (remember: memory variables can be manipulated,
     * i.e you can reassign the values as you need) and to be more gas efficient.
     */
    function distributeEther(Batch calldata batch) external payable {
        for (uint256 i = 0; i < batch.txns.length; i++) {
    ...
    /**
     * @dev The function `distributeToken` uses `calldata` in order to ensure
     * immutability of the parameters (remember: memory variables can be manipulated,
     * i.e you can reassign the values as you need) and to be more gas efficient.
     *
     * We also use the general ERC-20 interface in order to be contract-agnostic.
