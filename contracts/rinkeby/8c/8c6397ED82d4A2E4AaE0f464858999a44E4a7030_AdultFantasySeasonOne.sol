    ...
    IERC2981
{
        ...
        ERC721("AdultFantasySeasonOne", "AFC")
    {
        ...
        require(address(signingAddress) != address(0), "Address is zero");
    ...
    address private _signingAddress; /// For verifying mint passes
    ...
    function purchaseTokenMintPass(
        ...
        bytes[] memory signedRandomHexes
    ) external payable {
        require(
        ...
        require(
            isValidSignedHex(randomNumbers[0], signedRandomHexes[0]),
            "Random Hex is invalid"
        );
    ...
    function purchaseToken(
        ...
        for (uint256 index = 0; index < signedRandomHexes.length; index++) {
            uint256 randomNumber = randomNumbers[index];
            bytes memory signedRandomHex = signedRandomHexes[index];
            require(
                isValidSignedHex(randomNumber, signedRandomHex),
                "Random Hex is invalid"
            );
    ...
    function batchPurchaseReservedTokenMintPass(
        ...
        require(
            ...
            "Random Hexes length doesn't match signatures"
        );
    ...
    /// Set contracIdentifier for validating mint passes
    ...
    function isValidAccessMessage(
        ...
        return
            _signingAddress == hash.toEthSignedMessageHash().recover(signature);
    }

    function isValidSignedHex(uint256 randomNumber, bytes memory signedRandomHex)
        public
        view
        returns (bool)
    {
        bytes32 hash = keccak256(
            abi.encodePacked(randomNumber, contractIdentifier, mintCounts[msg.sender], msg.sender)
        );

        return
            _signingAddress ==
            hash.toEthSignedMessageHash().recover(signedRandomHex);
    }
...
interface IAFCharacter {
    function takeRandomCharacter(uint256 randomNumber) external returns (uint256);
