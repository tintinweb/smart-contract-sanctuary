...
contract BallerBars is Context, ERC20Burnable, Ownable {
    ...
    **/
    ...
    /**
     * @dev Burns amount of BB for account that hasn't been claimed
     * @param _tokenIds Token ID to burn BB for
     * @param amount Amount of BB to burn
     */
    function burnUnclaimed(uint256[] memory _tokenIds, uint256 amount) public {
        require(msg.sender == _genTwoChainsAddress,"NOT_APPROVED_CALLER");

        uint256 totalAccumulated = 0;

        uint256[] memory calculated = new uint256[](_tokenIds.length);

        for (uint256 i = 0; i < _tokenIds.length; i++) {

            address chainsAddress = getChainsAddress(getChainsGeneration(_tokenIds[i]));

            require(ERC721(chainsAddress).ownerOf(_tokenIds[i])==tx.origin,"NOT_TOKEN_OWNER");

            IChains chainsContract = IChains(chainsAddress);

            uint256 thisAccumulated = _accumulated(_tokenIds[i], chainsContract);

            if (totalAccumulated + thisAccumulated < amount){
                totalAccumulated += thisAccumulated;
                calculated[i] = thisAccumulated;
            }else{
                uint256 remaining = amount - totalAccumulated;
                totalAccumulated += remaining;
                calculated[i] = remaining;
                break;
            }

        }

        require(totalAccumulated==amount,"INSUFFICIENT_CLAIMABLE");

        for (uint256 i = 0; i < calculated.length; i+=1) {
            ...
            uint256 tokenEmissionPerDay = (0.1 ether + ((33 ether / 1000) * _getBoost(_tokenIds[i],chainsContract))) * _baseMultiplier;

            _lastClaim[_tokenIds[i]] = __lastClaim(_tokenIds[i],chainsContract) + (calculated[i] / (tokenEmissionPerDay / SECONDS_IN_A_DAY));
        }
    }
