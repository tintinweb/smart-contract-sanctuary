...
/**
 * @title FAN token governor.
 */
contract FANGovernor is
  GovernorUpgradeable,
  ...
  GovernorVotesQuorumFractionUpgradeable
{
  ...
  /**
   * @dev `FANGovernor` contract initializer.
   * @param tokenAddress "FAN" ERC20 token address
   * @param timelockController Governance `TimelockController`
   */
  function initialize(
  ...
  {
    __Governor_init("FANGovernor");
    __GovernorVotes_init(tokenAddress);
    __GovernorTimelockControl_init(timelockController);
    __GovernorVotesQuorumFraction_init(1); // 1% of supply

    /*
    NOTE
    
    testing values:
    delay 1595 blocks (2 minutes)
    period 7974 blocks (10 minutes)
    threshold 1000 tokens
    timelock delay 120000 milliseconds (2 minutes)

    safe off position values:
    delay 2097002520 blocks (5 years)
    period 2097002520 blocks (5 years)
    threshold 600000000 tokens (60% of the token space)
    timelock delay 157788000000 milliseconds (5 years)

    potential active governance values:
    delay _ block (5 days)
    period _ blocks (10 days)
    threshold _ tokens (1 million tokens)
    timelock delay _ milliseconds (2 days)
    */

    _votingDelay = 2097002520;
    _votingPeriod = 2097002520;
    _proposalThreshold = 600000000;
  }
