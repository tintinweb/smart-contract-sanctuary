...
contract JPandFriends is Context, ERC721, Ownable {
  ...
  // string public constant name = 'J. Pierce & Friends';
  // string public constant symbol = 'JPF';
  //uint256 public whitelistPrice = 37 * 10**15;
  ...
  event String(address to, bytes sig);
    ...
    // Whitesale mint requires address
    function mint(uint8 _count, address _to, uint256 code) public payable saleIsOpen {
        ...
        if(listOnly) {
          ...
          require(totalBought[_to] < 4, "Max amount purchased");
          
          totalBought[_to] = totalBought[_to] + _count;

          _mintElements(_count, _to);
          Sale(_to, _count, cost, salePrice);

        } else {

          require(totalBought[_to] < 12, "Max amount purchased");
          totalBought[_to] = totalBought[_to] + _count;
          _mintElements(_count, _to);
          Sale(_to, _count, cost, salePrice);
    ...
    // function addWhitelist() public saleIsOpen {
    //     uint256 total = _totalSupply();
    //     uint256 eggCost = whiteListPrice.mul(_count);
    //     require(total + _count <= MAX_WHITELIST, "Max limit");
    //     require(total <= MAX_WHITELIST, "Sale end");
