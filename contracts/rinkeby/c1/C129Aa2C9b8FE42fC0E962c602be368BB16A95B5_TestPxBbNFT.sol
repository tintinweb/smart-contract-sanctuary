...
contract TestPxBbNFT is Ownable, ERC721URIStorage, ERC721Enumerable, VRFConsumerBase {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    // Merkle Root for proving whitelist address
    bytes32 public merkleRoot = 0x070e2c388c27f9af9b24f3861e2f9d898ed340a3b953b0d191aa5435d1ceb760;
    ...
    ERC721(name, symbol) {
        ...
        for (uint i = 0; i < whitelistedTokens; i++) {
            _tokenIds.increment();
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        if (tokenId > maxTokens - reservedTokens) {
            sequenceId = Strings.toString(tokenId);
        } else {
            if (startingIndex >= 0) {
                sequenceId = Strings.toString((tokenId + startingIndex) % (maxTokens - reservedTokens));
            } else {
                sequenceId = "-1";
            }
        }
    ...
    function setStartingIndex() public onlyOwner{
        ...
        getRandomNumber();
        // startingIndex = 50;
        // revealTimestamp = block.timestamp + 84000;
    }

    function getStartingIndex() public returns (uint256) {
    ...
    function mintToken(uint numberOfTokens) public payable {
        ...
        require((totalSupply() + numberOfTokens) <= maxTokens, "Purchase would exceed max supply of Apes");
