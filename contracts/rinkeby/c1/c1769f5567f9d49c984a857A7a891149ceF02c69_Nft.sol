...
contract Nft is Ownable, ERC721Enumerable, ERC721Referralable, ToggleableSale, WhitelistVerifier {
  ...
  function mintForCommunity(address _to, uint256 _numberOfTokens) external onlyOwner {
    ...
    require(totalSupply() + _numberOfTokens <= MAX_ITEMS, 'Minting would exceed cap');
    require(
      communityMinted + _numberOfTokens <= COMMUNITY_ITEMS,
      'Minting would exceed community cap'
    );
...
contract WhitelistVerifier is Ownable {
  ...
  function updateWhitelist(address[] memory addresses, bool remove) external onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
      ...
      claimList[addresses[i]] > 0 ? claimList[addresses[i]] : 0;
    }
  }
}
...
abstract contract ERC721Referralable is Ownable, ERC721Enumerable {
  // test - acc3 mm
  address public charityDonationAddress = 0x7957bF3A045b0FD356FF6DC33B44E15089549d04;
  // test
  uint256 public singleReferalValue = 0.01 ether;
