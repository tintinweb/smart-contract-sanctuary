...
contract amlkwnwnkwnkwk is ERC721Enumerable, VRFConsumerBase, Ownable {
    ...
    address internal tokenAddress = 0x0CC8c52D259aCb911E0Eec1Ac920fAA727b840e7;
    ...
    event Claim(address owner, uint256 tokenId, uint256 availableFreeDays, uint256 packChoice, uint256 petChoice);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        output = string(abi.encodePacked(
            ...
            Utils.toString(device.availableFreeDays),
            svgData[5], // owner
    ...
    // address public dateContract = 0x92482Ba45A4D2186DafB486b322C6d0B88410FE7; // deploy date contract to read from
    // function getDate(uint _timestamp) internal pure returns(string memory) {
    //     uint16 year = DateTime.getYear(_timestamp);
    //     string memory month = DateTime.stringMonth(_timestamp);
    //     string memory day = DateTime.stringDay(_timestamp);
    //     return string(abi.encodePacked(month, "/", day, "/", Utils.toString(uint256(year))));
    // }
    
    // function getTime(uint _timestamp) internal pure returns(string memory) {
    //     string memory hour = DateTime.stringHour(_timestamp);
    //     string memory minute = DateTime.stringMinute(_timestamp);
    //     return string(abi.encodePacked(hour, ":", minute, " UTC"));
    // }
    
    // function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    //     Device memory device = unit[tokenId];
    //     Pack memory pack = packs[device.packChoice];
    //     if (device.packChoice == 0 && device.pet > 0) {
    //         Pack memory promo_pack = packs[device.pet];
    //         if (growthStage(tokenId) == DeviceState.Egg) return promo_pack.promo_metadata[0];
    //         if (growthStage(tokenId) == DeviceState.Alive) return promo_pack.promo_metadata[2];
    //         return promo_pack.promo_metadata[1];
    //     }
    //     if (growthStage(tokenId) == DeviceState.Egg) return pack.batch_metadata[0];
    //     if (growthStage(tokenId) == DeviceState.Alive) return pack.batch_metadata[device.pet];
    //     return string(abi.encodePacked(_baseURI(),pack.batch_metadata[1]));
    // }
    
       // find growth stage for feeding, dead or alive
    function growthStage(uint256 tokenId) internal view returns (DeviceState) {
        Device memory device = unit[tokenId];
    ...
    function withdraw(address _tokenAddress) public onlyOwner {
        ...
        TokenInterface(_tokenAddress).transferFrom(address(this), owner(), TokenInterface(_tokenAddress).balanceOf(address(this)));
    }
    ...
    0,1 promo loaded
    */
     // batch minter with custom pass amount, pack and pet
    function batchMint(address[] memory recipients, uint256 _availableFreeDays, uint256 _packChoice, uint256 _petChoice) public onlyMinter {
    ...
    function claim(address recipient, uint256 tokenId, uint256 _availableFreeDays, uint256 _packChoice, uint256 _petChoice) public onlyMinter {
        ...
        emit Claim(recipient, id, _availableFreeDays, _packChoice, _petChoice);
    }
    ...
    function purchaseNFT(uint256 packNumber) public {
        ...
        _tokenIds.increment();
        uint256 id = _tokenIds.current().add(1200);
    ...
    function getTokensReadyToBatch(uint256 batchType) internal view returns(uint256[] memory) {
        uint256[] memory ownersTokens = getOwnersTokenIds(msg.sender);
        ...
        for (uint256 i = 0; i < ownersTokens.length; i++) {
            ...
            if (batchType == 1) { // feed
                if (uint256(block.timestamp.sub(device.createdAt)).div(ONE_DAY) == device.feedLevel && alive(ownersTokens[i])) {
                    batchArray[count] = ownersTokens[i];
                    count++;
                }    
            }
            if (batchType == 2) { // play
    ...
    function returnReadyTokens(uint256 batchType) public view returns(uint256[] memory) {
        return getTokensReadyToBatch(batchType);
    }
    ...
    function batchPlay() public {
        uint256[] memory ownersTokens = returnReadyTokens(2);
        for (uint i = 0; i < ownersTokens.length; i++) {
            if (ownersTokens[i] != 0) {
                play(ownersTokens[i]);
            }
    ...
    function prestige(uint256 tokenId) public {
        ...
        device.availableFreeDays = device.availableFreeDays.add(device.prestige);
    ...
    function batchPrestige() public {
        uint256[] memory ownersTokens = returnReadyTokens(3);
        for (uint i = 0; i < ownersTokens.length; i++) {
            if (ownersTokens[i] != 0) {
                prestige(ownersTokens[i]);
            }
        }
    }
    ...
    function batchGetReward() public {
        uint256[] memory ownersTokens = returnReadyTokens(4);
        for (uint i = 0; i < ownersTokens.length; i++) {
            if (ownersTokens[i] != 0) {
                getReward(ownersTokens[i]);
            }
        }
    }
    ...
    function Unit(uint256 tokenId) public view returns (
        ...
        uint256 generationTicker_, uint256 lastPlayTime_, uint256 availableFreeDays_, uint256 prestige_,
        uint256 rewards_, uint256 tokensToRevive_, uint256 packChoice_, uint256 petFromPack_ ) {
            
        Device memory device = unit[tokenId];
        
        return (
            device.name, device.createdAt, device.feedLevel, device.generation,
            device.generationTicker, device.lastPlayTime, device.availableFreeDays, device.prestige,
            device.rewards, device.tokensToRevive, device.packChoice, device.pet
        );
    }
    
    // function timeData(uint256 tokenId) public view returns(
    ...
    //     return getTimeData(tokenId);
    // }
    
    // function getTimeData(uint256 tokenId) internal view returns (
    //     bool isItPlayTime_, uint256 secondsUntilNextPlay_, uint256 timeBetweenPlays_, uint256 playsPerGeneration_,
    //     bool isInFeedWindow_, uint256 secondsUntilNextFeed_, uint256 secondsLeftInFeedWindow_) {
    ...
    //     if (device.createdAt != MAX_TIME && alive(tokenId)) {
