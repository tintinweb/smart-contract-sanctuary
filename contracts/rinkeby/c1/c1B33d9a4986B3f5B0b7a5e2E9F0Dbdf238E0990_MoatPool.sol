...
contract MoatPool is Ownable {
    ...
    constructor(address _uctroller, address _uc, uint _ucReward) public {
        uctroller = Uctroller(_uctroller);
        uc = _uc;
        ucPerBlock = _ucReward;
    }
        ...
        returns (uint256)
    {
        ...
        return uint(account[_user]).mul(_accPerShare).div(1e12).sub(
                    accountUcDebts[_user]
                );
    }
    ...
    function deposit(uint amount) external {
        ...
        if (account[msg.sender] > 0) {
            uint256 pending =
                account[msg.sender].mul(accPerShare).div(1e12).sub(
                    accountUcDebts[msg.sender]
                );
            if (pending > 0) {
                IUtopiaToken(uc).issueTo(msg.sender, pending);
            }
        }
        if (totalSupply == 0) {
            ...
            uint256 value_u = uctroller.getUsrUTokenValue(address(this));
            uint256 value_s = uctroller.getUsrSTokenValue(address(this));
            uint256 value_mix = value_u.add(value_s);
            uint value_amount = amount.mul(uctroller.getStablePrice());
            uint256 tokenAmount =
                value_amount.mul(totalSupply).div(value_mix);
            account[msg.sender] = account[msg.sender].add(tokenAmount);
            totalSupply = totalSupply.add(tokenAmount);
        }
        uctroller.stableToken().transferFrom(msg.sender, address(this), amount);

        accountUcDebts[msg.sender] = account[msg.sender]
            .mul(accPerShare)
            .div(1e12);
    }
    ...
    function withdraw(uint tokenAmount) external {
        ...
        require(account[msg.sender] >= tokenAmount, "tokenAmount error");
        uint256 ratio_100000 = tokenAmount.mul(100000).div(totalSupply);
        UToken[] memory uTokens = uctroller.getAllUtokens();
        for (uint256 i = 0; i < uTokens.length; i++) {
            UToken uToken = uTokens[i];
            uint256 balanceOf = uToken.balanceOf(address(this));
            uint256 amount = balanceOf.mul(ratio_100000).div(100000);
            if (amount > 0) {
                uToken.transfer(msg.sender, amount);
            }
        }
        uint256 balanceOf_s = uctroller.stableToken().balanceOf(address(this));
        uint256 amount_s = balanceOf_s.mul(ratio_100000).div(100000);
        uctroller.stableToken().transfer(msg.sender, amount_s);

        account[msg.sender] = account[msg.sender].sub(tokenAmount);
        totalSupply = totalSupply.sub(tokenAmount);
        accountUcDebts[msg.sender] = account[msg.sender]
            .mul(accPerShare)
            .div(1e12);

        uint balanceOf_uc = EIP20Interface(uc).balanceOf(address(this));
        uint amount_uc = balanceOf_uc.mul(ratio_100000).div(100000);
        EIP20Interface(uc).transfer(msg.sender, amount_uc);
    }
}
