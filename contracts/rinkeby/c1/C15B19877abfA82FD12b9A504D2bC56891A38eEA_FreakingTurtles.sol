...
contract FreakingTurtles is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant PRE_SOLD = 1000;
    ...
    function mintReserved(uint256 numberOfTokens) public onlyOwner {
        ...
        require(preSaleIsActive == false, "Pre Sale must be inactive to reserve tokens");
        require(saleIsActive == false, "Sale must be inactive to reserve tokens");
        require(numberOfTokens > 0, "At least one token must be reserved");
        require(supply < RESERVED, "Too much tokens have already been reserved");
    ...
    function mintPreSale(uint256 numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= RESERVED + PRE_SOLD, "Purchase would exceed max supply of pre sale");
        require(PRE_SALE_PRICE.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 supply = totalSupply();
            if (supply < RESERVED + PRE_SOLD) {
