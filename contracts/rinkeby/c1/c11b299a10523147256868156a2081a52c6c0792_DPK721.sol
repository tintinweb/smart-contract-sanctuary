...
contract ERC721Tradable is ERC721, Ownable, Random {
    ...
    event NFTGenerated(uint256 indexed _nft);
    event NewNFTPrice(uint256 indexed _newprice);
    
    string internal baseMetadataURI;
	address proxyRegistryAddress;
	uint256 public totalNFTs;
    uint256 public genesisNFTs;
    uint256 public ownermintNfts;
    ...
    function addNFTCapacity(uint256 amount) public onlyOwner {
        require(totalNFTs + amount < 50000, "ERC721Tradable#uri: Maximum limit exceeded");

        uint256 currentNFTs = totalNFTs;
	    totalNFTs = totalNFTs + amount;
        for(uint i = currentNFTs; i < totalNFTs; i++){
            uint id = i;
            NFTtoCreator[id] = address(this);            
        }
	}
    ...
    function getGenesisNFTleft() public view returns(uint256) {
        return SafeMath.sub(totalNFTs, genesisNFTs);
    }

    function isMintGenesisNFT(uint256 id) public view returns(bool){
        return (id >= 0) && (id < totalNFTs);
    }

    function getNFTForAll() public view returns(uint256)  {
        return SafeMath.add(genesisNFTs,ownermintNfts);
    }
...
contract DPKMarket is ERC721Tradable {
...
contract DPK721 is DPKMarket {
    ...
    function mintNFTForOwner(address _address) public  onlyOwner returns(uint256) {

        uint256 nftId = ownermintNfts + 50000;
        _mint(_address, nftId);
        NFTtoCreator[nftId] = _address;
        ownermintNfts += 1;

        emit NFTGenerated(nftId);
    
        return nftId;
    }
    
    function buyNFT(uint256 _id) payable public returns(uint) {
        require(msg.value >= bnbPrice, 'BNB not enough');
        require(isMintGenesisNFT(_id), 'Not in casting list');
        require(getNFTCreator(_id) == address(this), 'It has been forged by others');

        _mint(msg.sender, _id);
        NFTtoCreator[_id] = msg.sender;
        genesisNFTs += 1;
        
        emit NFTGenerated(_id);
        
        return _id;
    }
    
    function buyBoxwithUSDT(uint256 _id) public returns(uint256) {
	...
	constructor(address usdtAddress, address _proxyRegistryAddress) public ERC721Tradable("Dont play with kitty", "DPK", 3, _proxyRegistryAddress) {
