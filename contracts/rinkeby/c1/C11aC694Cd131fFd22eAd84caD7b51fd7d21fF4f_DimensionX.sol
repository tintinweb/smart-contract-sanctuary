...
contract DimensionX is ERC721, Whitelist {
    uint32 cardsIssued;
    uint32 cardCounter;
    uint32 cardId;
    uint256 mintingFee;
    ...
    uint32[] advancer = [1,4,1,3,1,3,11,4,3,8,4,7,4,6];
    constructor() ERC721("DimensionX", "DMX") {
        
        mintingFee = 0.02 ether;
        feeAddress = payable(address(this));
        cardId = 1;
        cardCounter = 0;
        cardsIssued = 0;
    }
    ...
    function mintFreeCard() public onlyWhitelisted {
        require(removeAddressFromWhitelist(msg.sender));
    ...
    function mintCards(uint32 NumberOfCards)
        public
        payable
    {   require(NumberOfCards <= 5, "5 cards max only please");
    ...
    {
        if(cardsIssued%321 == 14) {
            //ultra rare
            uint rareId = (cardsIssued/321)*960 + 59;
            _mint(msg.sender, rareId);
    ...
    function mintAnyCard(uint32 newCardId) public onlyOwner returns (uint32) {
        ...
        //something that combines with might chimera, we need to mint a card that does not exist yet.
    ...
    function combineCards(uint32 inCardA, uint32 inCardB) public returns (uint32){
        //card combination logic goes here
        require(ERC721.ownerOf(inCardA) == msg.sender, 'both cards must be owned by opener');
        require(ERC721.ownerOf(inCardB) == msg.sender, 'both cards must be owned by opener');
