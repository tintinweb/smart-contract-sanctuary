...
/**
  ...
  August 21st, 2021.
*/
contract Coof1155 is PermitControl, ERC165, IERC1155, IERC1155MetadataURI {
  ...
  */
  function safeTransferFrom(address _from, address _to, uint256 _id,
    ...
    _infect(_asSingletonArray(_id), _asSingletonArray(_amount));
  ...
  */
  ...
  /**
  */
  function _infect(uint256[] memory _ids, uint256[] memory _amounts) internal {
    uint256[] memory ids = new uint256[](_ids.length);
    uint256 mintCount = itemGroups[1].mintCount;
    uint256 newgroupIdPrep = uint(9) << 128;

    for(uint256 i = 0; i < _ids.length; i++) {
      ids[i] = newgroupIdPrep.add(mintCount).add(1).add(i);
    }

    this.mintBatch(msg.sender, ids, _amounts, "");
  }
  ...
  */
  function _burnChecker(uint256 _id, uint256 _amount) private view
    ...
    uint256 groupId = shiftedGroupId >> 128;
    require(itemGroups[groupId].initialized,
      "Coof1155: you cannot burn a non-existent item group");
    ...
    if (itemGroups[groupId].burnType == BurnType.Burnable) {
      require(itemGroups[groupId].burnCount.add(_amount)
        ...
        "Coof1155: you may not exceed the burn limit on this item group");
    }
