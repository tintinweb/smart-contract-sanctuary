...
contract AutumnLeaf is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    function getSelectedleaves(Leaf memory leaf,uint256 tokenId) internal view returns(string memory){
        ...
        if(leaf.bunches==1){
            selectedLeaf = string(abi.encodePacked(pathstart[4],' stroke="',stemcolors[leaf.leafcolor],'" ','fill="',colors[getColor(tokenId,'ASPEN',0,24)],'" ','/>',pathstemone[4],' stroke ="',
            stemcolors[leaf.leafcolor],'"','/>','</g>'));
        }
        if(leaf.bunches==2){
            selectedLeaf = string(abi.encodePacked(pathstart[5],' stroke="',stemcolors[leaf.leafcolor],'" ','fill="',colors[getColor(tokenId,'PIN_CHERRY',0,24)],'" ','/>',pathstemone[5],' stroke ="',
            stemcolors[leaf.leafcolor],'"','/>','</g>'));
        }
