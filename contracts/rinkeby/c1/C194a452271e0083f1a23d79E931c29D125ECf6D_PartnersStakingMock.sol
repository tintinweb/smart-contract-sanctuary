...
contract PartnersStaking is IPartnersStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
	...
	function _withdraw(uint256 _amount) internal nonReentrant updateReward(msg.sender) {
...
import "../PartnersStaking.sol";

contract PartnersStakingMock is PartnersStaking {
	function withdraw_(uint256 _amount) public {
		super._withdraw(_amount);
	}

	function getReward_() public {
		super._getReward();
	}

	function getFutureRewardTokens_() public view returns (uint256) {
		return super._getFutureRewardTokens();
	}

	function calculateBlocksLeft_(uint256 _from, uint256 _to) public view returns (uint256) {
		return super._calculateBlocksLeft(_from, _to);
	}
}
