...
interface IChubbyMetalApe {
    ...
    function mintMetalApe(uint256 _chubbyApeTokenId, address _targetAddress)
        external;
...
/// @title ChubbyApeEquipment Tokens
/// @author ChubbyApe
...
contract ChubbyApeEquipment is Ownable, ERC1155, PartsTokensMetadata {
    ...
    */
    ...
    /**
    這是用來宣告疊加用的變數 讓用戶知道現在是第幾包
     */
    uint256 public declareId = 0;
    ...
    */
    ...
    modifier mintOnlyOnce(uint256 _chubbyApeTokenId) {
        require(!minted[_chubbyApeTokenId], "Mint equipment once");
        minted[_chubbyApeTokenId] = true;
        _;
    }
     ...
     */
    function _declareBasicEquipmentToMetalApe() internal {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            _balances[ids[i]][address(chubbyApe)] += 1;
        }
        emit TransferBatch(
            _msgSender(),
            address(0),
    ...
    /** ChubbyMetalApe settings*/
    // function setChubbyMetalApe(address _chubbyMetalApe) public onlyOwner {
    //     chubbyMetalApe = IChubbyMetalApe(_chubbyMetalApe);
    // }
    ...
    function mintChubbyApeEquipmentForFree(uint256 _chubbyApeTokenId)
        public
        mintOnlyOnce(_chubbyApeTokenId)
    {
    ...
    function mintMetalApeThenEquipment(
        ...
        uint256[] memory _itemIds
    ) public mintMetalApeOnlyOnce(_chubbyApeTokenId) {
        require(saleIsActive, "Mint is not active");
        require(
        ...
        //產生出一個MetalApe然後記錄裝備
        chubbyMetalApe.mintMetalApe(_chubbyApeTokenId, _msgSender());
    ...
    1. left Id ,right Id
     */
    function mintMetalApeThroughPairingAndGetBaseEquipmentThenPutOn(
        uint256 _leftPairId,
        uint256 _rightPairId
    ) public {
        require(saleIsActive, "Mint is not active");
        ...
        _declareBasicEquipmentToMetalApe();
    }
