...
contract MainnetMultiBridgePool is AccessControl, ReentrancyGuard {
  ...
  event ConfigSet(address token, address outAddress, uint256 maxTxsCount, uint256 txsCount);
  ...
  address private          FACTORY   = Uniswap(UNIROUTER).factory();
  ...
  function getFee(address _token, uint256 _amount) 
    public view returns (uint256) {
      TokenInfo memory t = tokens[_token];
      if (!t.exists) return 0;
      if (t.percentFee == 0) return t.fee;
      if (t.tokenToEth != 0) return t.fee.add(_amount.div(t.tokenToEth).mul(t.percentFee).div(10 ** t.percentFeeDecimals));
      
      address lp = Uniswap(FACTORY).getPair(_token, WETH);
      ...
      return t.fee.add(eth.mul(t.percentFee).div(10 ** t.percentFeeDecimals));
  }
  ...
  function setConfig(address _token, address _outAddress, uint256 _maxTxsCount, uint256 _txsCount) 
    external onlyOwner {
      require(tokens[_token].exists, "MainnetMultiBridgePool: token unsupported");
      ...
      emit ConfigSet(_token, _outAddress, _maxTxsCount, _txsCount);      
  }
