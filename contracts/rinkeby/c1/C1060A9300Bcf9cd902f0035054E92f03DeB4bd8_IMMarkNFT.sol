...
contract IMMarkNFT is BaseERC721 {
    enum ProjectState {
        Prepare, //0
        WhitelistSpringSale, //1
        WhitelistSummerSale, //2
        WhitelistAutumnSale, //3
        WhitelistWinterSale, //4
        PublicSale, //5
        Finished //6
    }
    ...
    mapping(ProjectState => uint256) _mintLimit;
    ...
    uint256[] private _ratio = [28, 18, 18, 18, 8, 10];

    constructor(string memory _tokenURI) ERC721("I'm Mark", "IMMARK") {
        _baseInit();
        _baseTokenURI = _tokenURI;
        state = ProjectState.Prepare;
        signerAddress = 0x8fA17eDAB5Bc95cc62c79F42bc2d565F7eA82543;
        _mintLimit[ProjectState.WhitelistSpringSale] = 2;
        _mintLimit[ProjectState.WhitelistSummerSale] = 0;
        _mintLimit[ProjectState.WhitelistAutumnSale] = 0;
        _mintLimit[ProjectState.WhitelistWinterSale] = 0;
    ...
    // update token URI
    function updateTokenURI(string memory tokenURI) external onlyOwner {
    ...
    function mint(
        ...
        // check if the _state passed in is correct and matching current project state
        require(
    ...
    ERC721Burnable
{
    ...
    function _baseInit() internal {
        // start from 1
        _tokenIdTracker.increment();
    }

    function _baseMint() internal {
        // mint the token
