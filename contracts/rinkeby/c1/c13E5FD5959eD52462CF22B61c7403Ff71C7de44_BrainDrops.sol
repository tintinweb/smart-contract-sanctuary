...
contract BrainDrops is ERC721, Ownable {
    ...
    mapping(uint256 => uint256) public projectIdToMaxAmount;
    ...
    /**
     * Set the starting index for a project
     */
    function setProjectStartingIndex(uint256 _projectId) public {
        ...
        projectIdToStartingIndex[_projectId] = uint(block.number.mul(randomizerContract.random())) % projectIdToMaxAmount[_projectId];
    ...
    function addProject(string memory _projectName, string memory _projectBaseURI, address _artistAddress, uint256 _pricePerTokenInWei, bool _dynamic, uint256 _maxAmount) public onlyWhitelisted {
        uint256 projectId = nextProjectId;
        projectIdToArtistAddress[projectId] = _artistAddress;
        projects[projectId].name = _projectName;
        projectIdToBaseURI[projectId] = _projectBaseURI;
        projectIdToMaxAmount[projectId] = _maxAmount;
      ...
      returns (uint256)
    {
      ...
      uint tokenIdToBe = ((projects[_projectId].invocations + projectIdToStartingIndex[_projectId]) % projectIdToMaxAmount[_projectId]) + (_projectId * ONE_MILLION);
