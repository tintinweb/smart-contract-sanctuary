...
contract FlokiPFP is ERC721, Ownable {
    ...
    /**
     * 0x177312b692F92f1D2498637fcFC90B79b1FC7719 bronze
     * 0x90Ff5796b1e7711F1b13afd69D2E36e28A60C708 silver
     * 0x1E18de8a026A1CAB515E105A26eA95Cf35300037 diamond
     * 0x064F9547a78bD5Ba35a7aEB2221DE69b86CD6307 ruby
     */
    constructor(
        string memory _name,
    ...
    function mint() external payable {
        if (_hasGenesisNFT(msg.sender)) {
            require(
                msg.value == GENESIS_MINT_COST,
                "FlokiPFP: Invalid mint value provided; should be 0.03 ETH."
            );
        } else {
            require(
                msg.value == REGULAR_MINT_COST,
                "FlokiPFP: Invalid mint value provided, should be 0.05 ETH."
            );
        }
        ...
        if (!_superMinted[_potentialSuperId]) {
            ...
            uint256 _rng = uint256(keccak256(abi.encodePacked(_potentialSuperId, msg.sender, block.timestamp, nextTokenId)));
            ...
            bytes32 _seed = keccak256(abi.encodePacked(msg.sender, block.timestamp, nextTokenId));
    ...
    function getData(uint256 _tokenId) external view returns (uint8, uint8, uint8, uint8, uint8, uint8) {
    ...
    function _generate(bytes32 _seed) private view returns (PFP memory) {
        PFP memory _pfp;

        _pfp.background = Backgrounds[uint256(keccak256(abi.encodePacked(_seed, "Background"))) % Backgrounds.length];
        _pfp.eyes = Eyes[uint256(keccak256(abi.encodePacked(_seed, "Eyes"))) % Eyes.length];
        _pfp.hat = Hat[uint256(keccak256(abi.encodePacked(_seed, "Hat"))) % Hat.length];
        _pfp.mouth = Mouth[uint256(keccak256(abi.encodePacked(_seed, "Mouth"))) % Mouth.length];
        _pfp.shirt = Shirt[uint256(keccak256(abi.encodePacked(_seed, "Shirt"))) % Shirt.length];
        _pfp.ear = Ear[uint256(keccak256(abi.encodePacked(_seed, "Ear"))) % Ear.length];
    ...
    function _getSuperRange(uint256 _tokenId) private pure returns (uint256) {
        if (_tokenId <= 1250) {
            ...
            return 1;
        } else if (_tokenId <= 3750) {
            return 2;
        } else if (_tokenId <= 5000) {
            return 3;
        } else if (_tokenId <= 6250) {
            return 4;
        } else if (_tokenId <= 7500) {
            return 5;
        } else if (_tokenId <= 8750) {
            return 6;
        } else {
            return 7;
        }
    ...
    function _getSuperOdds(uint256 _tokenId) private pure returns (uint256) {
        if (_tokenId <= 1250) {
            return 1250 - _tokenId;
        } else if (_tokenId <= 2500) {
            return 2500 - _tokenId;
        } else if (_tokenId <= 3750) {
            return 3750 - _tokenId;
        } else if (_tokenId <= 5000) {
            return 5000 - _tokenId;
        } else if (_tokenId <= 6250) {
            return 6250 - _tokenId;
        } else if (_tokenId <= 7500) {
            return 7500 - _tokenId;
        } else if (_tokenId <= 8750) {
            return 8750 - _tokenId;
        } else {
            return 10000 - _tokenId;
        }
    }
}
