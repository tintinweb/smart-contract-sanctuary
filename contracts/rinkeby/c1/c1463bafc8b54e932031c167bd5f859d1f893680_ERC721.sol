...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 public total_mr_count = 0;
    ...
    mapping(uint256 => MRinfo) private mrInfos;
 ...
 */
     ...
     */
    constructor(string memory name_, string memory symbol_ , string memory base_uri) {
        _name = name_;
        _symbol = symbol_;
        _baseURi = base_uri;
    }
    ...
    function mintMrId(
        string memory _mrName,
        uint256 _mrRate,
        uint256 _mrPrice,
        string memory _mrMetaDataURI,
        uint256 _maxCount
    ) public onlyOwner returns (uint256) {
        uint256 newMrId = total_mr_count;
        total_mr_count++;
    ...
    function mintTokenByETH(uint8 _mrId, uint256 _count) public payable {
        ...
        require(
            (mrCount[_mrId] + _count) <= mrInfos[_mrId].maxCount,
            "exceeded the maximum mintable count of purchases."
        );
        require(
            msg.value == mrInfos[_mrId].mrPrice * _count,
    ...
    /**
     * @dev Mint NFTs.only by admin
     */
    function mintTokenByOnlyByAdmin(
        address _to,
        uint8 _mrId,
        uint256 _count
    ) public onlyOwner {
