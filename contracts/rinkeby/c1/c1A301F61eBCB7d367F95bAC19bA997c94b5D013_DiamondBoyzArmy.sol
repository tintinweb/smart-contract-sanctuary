...
contract DiamondBoyzArmy is ERC721URIStorage {
    ...
    uint256 public immutable MAX_DBAS = 10000;
    ...
    function mintDBA(
        ...
        require(
            _tokenIds.current() + numberOfTokens <= MAX_DBAS,
            "DBA: Mint limit reached"
        );

        require(
            balanceOf(recipient) + numberOfTokens <= maxDbaPurchase,
            "DBA: Mint limit per user reached"
        );
    ...
    function reserveDBA(address recipient, string[] memory tokenURI, uint numberOfTokens)
        public
        onlyOwner
        returns (uint256[] memory) 
    {
        ...
        require(
            ...
            "DBA: Mint limit per owner reached"
        );
    ...
    function setTokenURI(uint256 tokenId, string memory _newTokenURI) public onlyOwner {
        require(isURLReserved(_newTokenURI) == false, "DBA: URL already reserved or tries to set URI of nonexistent token");
