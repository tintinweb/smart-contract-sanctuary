...
contract AlphaSociety is ERC721Enumerable, Ownable {
    ...
    uint256 private saleFlag = 1;
    bool private metadataLocked = false;

    mapping(address => uint256) private amountPublicMintedPerUser;
    mapping(address => uint256) private amountPreSaleMintedPerUser;
    ...
    function mintWhitelist(bytes32[] calldata _merkleProof, uint256 _amount) external payable onlyWhitelist(_merkleProof) {        
        uint256 supply = this.totalSupply();
        require(supply + _amount <= availableNFTs, "EXCEEDS_TOTAL_SUPPLY");
        require(msg.value == cost * _amount, "INVALID_VALUE");
        require(saleFlag == 1, "MINTING_PAUSED");
        require(_amount <= 5, "_AMOUNT_TOO_HIGH");
        require(amountPreSaleMintedPerUser[msg.sender] + _amount <= 5, "EXCEEDS_MINT_LIMIT");
    ...
    function mint(uint256 _amount) external payable {
        ...
        require(_amount <= 3, "_AMOUNT_TOO_HIGH");
        require(amountPublicMintedPerUser[msg.sender] + _amount <= 3, "EXCEEDS_MINT_LIMIT");
        ...
        amountPublicMintedPerUser[msg.sender] += _amount;
    }
