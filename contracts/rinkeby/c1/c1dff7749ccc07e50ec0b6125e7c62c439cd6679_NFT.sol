...
contract NFT is ERC721, Ownable {
    ...
    bytes32 public root = 0x7f5ac2647a6c170233be86651caf417afbe3be9cc9ad9c75ede39c465aa45fe0; // todo
    bytes32 public presaleRoot = 0x7f5ac2647a6c170233be86651caf417afbe3be9cc9ad9c75ede39c465aa45fe0; // todo

    constructor(uint256 maxNFTSupply, uint256 _airdropSupply, uint256 _presaleSupply, address payable fundReceiver) ERC721("Brave Series More Want", "BSMW") {
    ...
    */
    function mint(address to, uint numberOfTokens, bytes32[] memory proof) public payable {
        ...
        if (!ForAllIsActive) {
            require(listPurchases[operator] + numberOfTokens <= maxNFTPurchase, 'Error: Error: Each person can mint only 2 in one phase');
            require(MerkleProof.verify(proof, root, keccak256(abi.encodePacked(operator))), "You are not allowed to mint");
        }
    ...
    function presaleMint(address to, uint numberOfTokens, bytes32[] memory proof) public payable {
        ...
        require(NFTPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        require(presaleListPurchases[operator] + numberOfTokens <= maxNFTPurchase, 'Error: Each person can mint only 2 tokens in one phase');
        require(MerkleProof.verify(proof, presaleRoot, keccak256(abi.encodePacked(operator))), "You are not allowed to mint during presale");

        // The contract never holds any Ether. Everything gets redirected to treasury directly.
        treasury.transfer(msg.value);
        
        for(uint i = 1; i <= numberOfTokens; i++) {
            presaleListPurchases[operator]++;
            _safeMint(to, _nowTokenId + i);
        }
        nowTokenId += numberOfTokens;
    }
