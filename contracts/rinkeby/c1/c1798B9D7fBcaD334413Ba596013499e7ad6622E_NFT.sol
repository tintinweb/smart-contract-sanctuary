...
contract NFT is ERC721, PaymentSplitter {
  ...
  uint public saleStartDate = block.timestamp + 1 seconds; // TODO change to correct duration
  ...
  function calculateTokenPriceDA() public view returns(uint){
    // one interval == 3 hours
    uint elapsedIntervalsDA = (block.timestamp - saleStartDate) / 180; //TODO set final time value
    if (elapsedIntervalsDA >= 4){
      return endPriceDA;
    }
    else {
