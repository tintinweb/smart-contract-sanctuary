...
contract POOC is ERC721EnumerableB, Ownable, PaymentSplitter, ReentrancyGuard {
  ...
  constructor(address[] memory _payees, uint256[] memory _shares)
    ERC721B("Proof of concept 2", "POC2")
    PaymentSplitter(_payees, _shares) {
    _owners.push(address(0));
  }

  function mintPreSale(uint count, bytes32 hash, bytes memory signature) external payable nonReentrant() {
    require(isActive, "Must be active" );
		bytes32 message = keccak256(abi.encodePacked(
      "\x19Ethereum Signed Message:\n32",
      keccak256(abi.encodePacked(msg.sender, count)))
    );
    require(recoverSigner(message, signature) == signerAddress, "Wrong signature");
