...
contract WalletSimple {
  ...
  event SafeModeToggled(address msgSender);
  event Transacted(
   ...
   */
  function toggleSafeMode() external onlySigner {
      if(safeMode == false){
           safeMode = true;
      }else{
           safeMode = false;
     ...
     SafeModeToggled(msg.sender);
  }
  
  function showStateOfSafemode()external view onlySigner returns(bool){
      return(safeMode);
  }
