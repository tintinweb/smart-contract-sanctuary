...
contract Invoker is ChainlinkClient{
    using Chainlink for Chainlink.Request;



    bytes32 public currentJobID;
    address public currentOracleAddress = 0x807bf4842C40c130a10E388E4c410e837330a75A;
    uint256 public fee;
    //contract address of the targetted function call
    address public target;
    uint256 public timSet;
    constructor() public {
    ...
    function newTarget(address _target) public {
        target = _target;
    }

    function scheduleCall() public returns (bytes32 requestID) {
        Chainlink.Request memory request = buildChainlinkRequest(currentJobID, address(this), this.fulfill.selector);
        request.add("get", "http://worldtimeapi.org/api/timezone/Australia/Brisbane");
        request.add("path", "unixtime");
        return sendChainlinkRequestTo(currentOracleAddress, request, fee);


     }

     function fulfill(bytes32 _requestId, uint256 unixtime) public recordChainlinkFulfillment(_requestId) {
         LoanContract l = LoanContract(target); 
         l.timePassed(unixtime); //will need to pass in some sort of key/check to ensure no one can just call this function and set the time passed. 
     ...
     function setNewJobID(bytes32 _newID) public {
         currentJobID = _newID;
     }
}
