...
contract Puppy is ERC721Enumerable, Verify, ReentrancyGuard{
    constructor(address _verificationAddress, string memory _base) ERC721("Puppy", "PUP") Verify(_verificationAddress){
        baseURI = _base;
    }
    ...
    //address internal punksAddress = 0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB; //mainnet
    address internal punksAddress = 0x424f62Dd208074FbD4BfBE4B99844638F306bEA1;
    uint256 public punkMintsRemaining = 250;
    uint256 public giveawaysRemaining = 200;
    uint256 randomMintMod = 3; // 1/3 chance
    ...
    function _mintPuppy(address _to) internal {
        require(totalSupply() < MAX_SUPPLY, 'Reached max supply');
        _mint(_to, totalSupply()); // totalSupply is new ID
    ...
    function mintWithPunk(uint256 _punkId) external nonReentrant {
        ...
        require(hashIsEligible(tokenBlockHash), "Try again, bad timing");
        require(punkHashIsUnused(tokenBlockHash), "Punk already used this block");
        punkMintsRemaining--;
    ...
    function hashIsEligible(uint256 tokenBlockHash) public view returns (bool){
        uint256 index = tokenBlockHash % randomMintMod;
        return (index == 0);
    }

    function punkHashIsUnused(uint256 tokenBlockHash) public view returns (bool){
        return (punksUsedBlock[tokenBlockHash] == 0);
    }

    function getTokenBlockHash(uint _tokenId, uint blockNumber) public view returns (uint){
