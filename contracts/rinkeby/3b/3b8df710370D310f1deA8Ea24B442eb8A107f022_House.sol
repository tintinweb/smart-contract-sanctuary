...
contract House is IHouse, ERC721Enumerable, Ownable, Pausable {
    ...
    event ShackMinted(address indexed owner,uint256 indexed tokenId);
    ...
    event ShackStolen(address indexed owner,uint256 indexed tokenId);
    ...
    event ShackBurned(uint256 indexed tokenId);
    event RanchBurned(uint256 indexed tokenId);
    event MansionBurned(uint256 indexed tokenId);

    event ShackStaked(address indexed owner,uint256 indexed tokenId);
    ...
    event ShackUnStaked(address indexed owner,uint256 indexed tokenId);
    ...
    function mint(address recipient, uint256 seed) external override whenNotPaused {
        ...
        if(tx.origin != recipient && recipient != address(tower)) {
            ...
            if(tokenTraits[minted].roll == 0) {
                emit ShackStolen(recipient, minted);
    ...
    */
    function generate(address recipient, uint256 tokenId, uint256 seed) internal returns (HouseStruct memory t) {
        t = selectTraits(seed);
        tokenTraits[tokenId] = t;
        emit TokenMinted(recipient, tokenId);
        if(t.roll == 0) {
            emit ShackMinted(recipient, tokenId);
        } else if(t.roll == 1) {
    ...
    */
    function emitShackStakedEvent(address owner, uint256 tokenId) external override whenNotPaused {
        emit ShackStaked(owner, tokenId);
    }
    ...
    */
    function emitShackUnStakedEvent(address owner, uint256 tokenId) external override whenNotPaused {
        emit ShackUnStaked(owner, tokenId);
    }
