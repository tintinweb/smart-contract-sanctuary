...
contract DannyNFT is DannyBase, VRFConsumerBase {
  ...
  event DannyMint(uint256 tokenId);
  event DannyReveal(bytes32 requestId, uint256 tokenId);
  ...
  /**
   * @dev get current VRF params
   */
  function getCurrentVRFDetail() public view onlyOwner returns( address _coordinator, address _linkToken, bytes32 _keyHash) {
    return (coordinator, linkToken, keyHash);
  }
  ...
  function metaMapping() public view onlyOwner returns (uint256[] memory meta) {
    return metaIds;
  }

  function shuffle() public onlyOwner returns (uint256[] memory meta) {
    _shuffle();
    return metaIds;
  }
  ...
  function _mintDanny(address _to,uint256 numberToken) internal returns (bool) {
    for (uint256 i = 0; i < numberToken; i++) {
      ...
      if (tokenIndex < MAX_SUPPLY) _safeMint(_to, tokenIndex);
    }
    return true;
  }
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 randomNumber)
    ...
    emit DannyReveal(_requestId, seed);
  }
  ...
  function _shuffle() public onlyOwner {
    // shuffle meta id
    for (uint256 i = 5; i < MAX_SUPPLY; i++) {
      uint256 j = (uint256(keccak256(abi.encode(seed, i))) % (MAX_SUPPLY - 5)) + 5;
   ...
   */
  function earlyMint() public offline onlyOwner {
    require(!earlyMinted, "Owner minted token");
    _mintDanny(_msgSender(), 60); // mint for marketing & influencer
    earlyMinted = true;
  }
