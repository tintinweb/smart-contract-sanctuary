...
contract ImplAndTermsV2 is Storage, Ownable, ERC20Init {
    ...
    function stake(uint tokenAmount) public {
        stakeInternal(msg.sender, tokenAmount, 0, address(0), address(0), address(0), false);
    }

    function stake(uint tokenAmount, uint holdTime) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, address(0), address(0), address(0), false);
    }

    function stake(uint tokenAmount, uint holdTime, address referer) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, referer, address(0), address(0), true);
    }

    function stake(uint tokenAmount, uint holdTime, address referer, address beneficiary) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, referer, beneficiary, address(0), true);
    }

    function stake(uint tokenAmount, uint holdTime, address referer, address beneficiary, address influencer) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, referer, beneficiary, influencer, true);
    }

    function stakeInternal(address staker, uint tokenAmount, uint holdTime, address referer, address beneficiary, address influencer, bool donatsForDevelopers) internal {
        require(
