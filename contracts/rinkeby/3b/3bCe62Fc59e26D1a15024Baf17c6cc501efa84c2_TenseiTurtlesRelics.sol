...
contract TenseiTurtlesRelics is Delegated, ERC1155Base{
  string public name = "Tensei Turtles: Relics";
  string public symbol = "MFLASK";
  ...
  constructor()
    ...
    string memory tokenUri = "https://ipfs.tenseiturtles.io/flask/";
    string memory tokenName = "Mysterious Flask";
    setToken( 0,            //id
      tokenName, tokenUri,
      555,                  //supply
      false, 1 ether,       //burn
  ...
  //payable
  function mint( uint id, uint quantity ) public virtual override payable{
    require( accessList[ msg.sender ] >= quantity, "MFLASK: Not enough grants" );
    accessList[ msg.sender ] -= quantity;

    super.mint( id, quantity );
  }


  //delegated
  function setAccessList( address[] calldata accounts, uint[] calldata quantities ) external onlyDelegates{
    require( accounts.length == quantities.length, "MFLASK: Must provide equal accounts and quantities" );
  ...
  function withdraw() external onlyOwner {
    require(address(this).balance >= 0, "MFLASK: No funds available");
...
contract ERC1155Base is Delegated, ERC1155 {
  ...
  function burnFrom( address account, uint[] calldata ids, uint[] calldata quantities ) external payable onlyDelegates {
    require( ids.length > 0,                  "MFLASK: No id(s) provided" );
    require( ids.length == quantities.length, "MFLASK: Must provide equal ids and quantities");

    for(uint i; i < ids.length; ++i ){
      require( ids[i] < tokens.length, "MFLASK: Specified token (id) does not exist" );
  ...
  function setProxy(address proxy) external onlyDelegates{
    require( proxyRegistryAddress != proxy, "New value matches old" );
  ...
  function setSupply(uint id, uint supply) public onlyDelegates {
    ...
    Token storage token = tokens[id];
    require( token.supply != supply,  "New value matches old" );
