...
contract StakBankFactory is Ownable, Pausable, Initializable {
    ...
    {
        return getStakBanks[_creator][_lpToken];
    }
     ...
     */
    function getCreatedStakBanksLengthByToken(address _creator, IERC20 _lpToken)
        public
        view
        returns (uint256)
    {
        return getStakBanks[_creator][_lpToken].length;
    }
    ...
    function registerStakBank(
        ...
        uint256 tokenIndex =
            getCreatedStakBanksLengthByToken(msg.sender, _lpToken);
        bytes32 salt =
            keccak256(abi.encodePacked(msg.sender, _lpToken, tokenIndex));
        assembly {
            pool := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IStakBank(pool).initialize(_name, _lpToken, _rewardToken);
        getStakBanks[msg.sender][_lpToken].push(pool);
...
contract StakBank is LPTokenWrapper, IRewardDistributionRecipient, Pausable {
    using SafeMath for uint256;
    ...
    function earned(address account) public view returns (uint256) {
        return
            balanceOf(account)
                .mul(
                rewardPerToken().sub(userInfo[account].userRewardPerTokenPaid)
            )
                .mul(getMultiplier(_msgSender()))
                .div(1e2)
                .div(1e18)
    ...
    function collectRemainingRewards(address tokenAddress) public onlyOwner {
    ...
    function updateBaseDate(uint256 amount, address account) internal {
        uint256 oldBaseDate = userInfo[account].baseDate;
        uint256 oldStake = balanceOf(account);
        // weighted average following: [(oldBaseDate * oldStake) + (now * deposit)]/(oldStake + deposit)
        uint256 newBaseDate =
            ((oldBaseDate.mul(oldStake)).add(block.timestamp.mul(amount))).div(
                oldStake.add(amount)
            );
        userInfo[account].baseDate = newBaseDate;
    }
    ...
    function getMultiplier(address account) public view returns (uint256) {
        ...
        uint256 baseDate = userInfo[account].baseDate;
        // get elapsed seconds
        uint256 secondsElapsed = block.timestamp.sub(baseDate);
        // transform to elaspsed months
        uint256 integerMonthsElapsed =
            secondsElapsed.sub(secondsElapsed.mod(30 days)).div(30 days);
        ...
        updateReward(address(0))
    {
        if (block.timestamp >= periodFinish) {
            require(duration > 0, "LPBANK::INVALID_DURATION");
