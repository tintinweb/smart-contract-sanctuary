...
contract CryptoBirdos is AccessControl, DetailedERC721 {
    ...
    uint256 threeBirdoPrice = 0.005 ether;
    uint256 fiveBirdoPrice = 0.0075 ether;


    uint256 dnaDigits = 24;
   ...
   function _generateRandomDna(uint256 block) public view returns (uint256) {
        uint256 rand = uint(keccak256(abi.encodePacked(block)));
    ...
    function _concatenateDna(uint8[24] memory genes) internal view returns(uint256 dna) {
        dna = 0;
    ...
    function breedBirdos(address payable _owner, uint256 _firstBirdoId, uint256 _secondBirdoId) public payable {
        ...
        uint8[24] memory puppyDnaStarter;

        for (uint8 i = 1; i < 19; i++) {
            puppyDnaStarter[20 - i] = _genePriority(birdos[_firstBirdoId].dna, birdos[_secondBirdoId].dna, seed, (25 - i));
        }
        for (uint8 i = 19; i < 25; i++) {
            // puppyDnaStarter[20 - i] = _genePriority(birdos[seed, seed, seed, (21 - i));
            puppyDnaStarter[24 - i] = _getNthDigit(seed, (25 - i));
        }
