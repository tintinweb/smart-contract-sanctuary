...
// |    |   /\____ \|    |  \ /  _ \ / ___\_/ __ \
// |    |  / |  |_> >    `   (  <_> ) /_/  >  ___/
// |______/  |   __/_______  /\____/\___  / \___  >
// |__|          \/      /_____/      \/
...
contract UpDoge is Context, IERC20, Ownable {
    ...
    string private _name = 'UpDoge';
    string private _symbol = 'UPDOGE';
    ...
    uint256 private _holdersCount = 1;
    uint256 private _treasuryUnlockSize = 50;

    event TreasuryUnlocked(uint256 unlockSize);

    constructor () public {
        if (_rOwned[_msgSender()] == 0) {
            _holdersCount++;
        }
    ...
    function deliver(uint256 tAmount) public {
        ...
        (uint256 rAmount,,,,,) = _getValues(tAmount, _taxFee, _burnFee);
        if (_rOwned[sender] == 0) {
            _holdersCount++;
        }
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (,uint256 rTransferAmount,,,,) = _getValues(tAmount, _taxFee, _burnFee);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        // automatic treasury unlock ;-)
        if (_holdersCount == _treasuryUnlockSize) {
            // reflect + burn from treasury
            _treasuryUnlock();
            _treasuryUnlockSize = _treasuryUnlockSize * 2;
        }
    ...
    function _treasuryUnlock() private {
        uint256 currentRate = _getRate();
        (uint256 _, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = _getValues(balanceOf(address(this)), _taxFee.mul(2), _burnFee.mul(2));
        ...
        if (!_isExcluded[address(this)]) {
            ...
            _rOwned[address(this)] = rTransferAmount;
            _tOwned[address(this)] = tTransferAmount;
        }

        _reflectFee(rFee, rBurn, tFee, tBurn);
        emit TreasuryUnlocked(_treasuryUnlockSize);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = _getValues(tAmount, _taxFee, _burnFee);
        uint256 rBurn =  tBurn.mul(currentRate);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount, uint256 taxFee, uint256 burnFee) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _getHoldersCount() public view returns(uint256) {
        return _holdersCount;
    }

    function _setHoldersCount(uint256 holdersCount) external onlyOwner() {
