...
contract MillionFarm is Ownable {
    ...
    function add(uint256 _allocPoint, IERC20 _stake, uint16 _depositFeeBP, bool _withUpdate) external onlyOwner {
        require(_depositFeeBP <= 500, "add: invalid deposit fee basis points"); // 5% max possible fee
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    //can reset pool fees/reward allocations
    ...
    //update reward emissions every 6 months-- max of 5 halvings
    ...
    //View pending rewards on frontend
    ...
    function changeBank(address _lionBank) external {
        require(msg.sender==lionBank);
        lionBank=_lionBank;
    }
