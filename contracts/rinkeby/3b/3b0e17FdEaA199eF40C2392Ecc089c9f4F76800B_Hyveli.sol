...
/**
 * @title Hyveli contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract Hyveli is ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
    ...
    **/
    ...
    // Maximum supply of tokens allowed during pre-sale.
    uint256 public constant maxPresaleSupply = 2000;

    // Pre-sale price.
    uint256 public presalePrice = 1000000000000000; //0.001 ETH

    // Main launch price.
    uint256 public price = 100000000000000; //0.01 ETH

    // Minimum quantity required for pre-sale.
    uint256 public constant minPresaleQuantity = 2;

    // Maximum quantity of tokens per purchase.
    uint256 public constant maxQuantityPerPurchase = 10;

    // Maximum quantity of tokens that a single holder is allowed to have.
    uint256 public constant maxQuantityPerAddress = 400;

    // Maximum pre-sale purchases a single address can make. 
    uint256 public constant presalePurchasesMax = 2;
    ...
    **/
    ...
    function setBaseURI(string memory _tokenuri) public onlyOwner whenNotPaused returns(string memory) {
        baseURI = _tokenuri;
        return baseURI;
    }
    ...
    function presaleMint(uint _quantity) public whenNotPaused nonReentrant payable {
        require(presaleIsOpen, "Pre-sale must be active in order to mint tokens.");
        require(_quantity >= minPresaleQuantity, "Minimum quantity of 2 to mint in pre-sale.");
        require(presalePrice.mul(_quantity) == msg.value, "Ether value is not correct. Please check the price and update your transaction value.");
        ...
        if (msg.sender != owner()) {
            if (whitelistState == true) {
                ...
                require(ownerMintedCount + _quantity <= maxQuantityPerAddress, "Maximum allowed quantity of tokens per holder exceeded.");
            }
            require(msg.value >= presalePrice.mul(_quantity), "Ether value is not correct. Please check the price and update your transaction value.");
        }
    ...
    function mintToken(uint _quantity) public whenNotPaused nonReentrant payable requireMint(_quantity) {
        ...
        require(price.mul(_quantity) == msg.value, "Ether value is not correct. Please check the price and update your transaction value.");
    ...
    function reserveToken(uint _quantity) public whenNotPaused nonReentrant onlyOwner {
