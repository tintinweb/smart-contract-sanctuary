...
import "./core/BlockBeatsTypes.sol";
import "./interfaces/IBlockBeatsRenderer.sol";
...
contract BlockBeats is ERC721Enumerable, Ownable, ReentrancyGuard {
    mapping(uint256 => BlockBeatsTypes.BlockBeat) beats;

    address public renderingContractAddress;

    event GenerateBeat(uint256 indexed tokenId, uint256 dna);
    ...
    uint256 private beatZeroHash;
    uint256 private beatZeroDNA;
    ...
    function founderMint(uint256 _count) public nonReentrant returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < _count; i++) {
            ...
            mint(MAX_PUBLIC_BEATS + _reservedTokenIds.current());
        }
    ...
    function mint(uint256 tokenId) internal {
        BlockBeatsTypes.BlockBeat memory beat;
        beat.dna = uint256(keccak256(abi.encodePacked(
                tokenId,
                msg.sender,
                block.difficulty,
                block.timestamp
    ...
    function configureBeatZero(uint256 _beatZeroHash, uint256 _beatZeroDNA) external onlyOwner {
        require(beatZeroHash == 0, "Beat Zero has already been configured");
        beatZeroHash = _beatZeroHash;
        beatZeroDNA = _beatZeroDNA;
    }

    function mintBeatZero(string memory seed) external {
        require(beatZeroHash != 0, "Beat Zero has not been configured");
        require(!_exists(0), "Beat Zero has already been minted");
        require(checkHash(seed) == beatZeroHash, "Incorrect seed");

        BlockBeatsTypes.BlockBeat memory beat;
        beat.dna = beatZeroDNA;
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        IBlockBeatsRenderer renderer = IBlockBeatsRenderer(renderingContractAddress);
        return renderer.tokenURI(_tokenId, beats[_tokenId]);
    }
    ...
    function tokenURIForSeed(uint256 _tokenId, uint256 seed) public view virtual returns (string memory) {
        ...
        IBlockBeatsRenderer renderer = IBlockBeatsRenderer(renderingContractAddress);
        return renderer.tokenURI(_tokenId, beat);
    }

    function getDna(uint256 _tokenId) public view returns (uint256) {
        return beats[_tokenId].dna;
    }
...
interface IBlockBeatsRenderer {
    function tokenURI(uint256 tokenId, BlockBeatsTypes.BlockBeat memory beatData) external view returns (string memory);
}
