...
contract HodlEm is ERC721, ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("Hodl Em!", "HODL") {}
    ...
    function getHand(uint256 tokenId) public view returns (uint8[10] memory) {
        uint256 rand = uint256(keccak256(abi.encodePacked(Strings.toString(tokenId), address(this))));
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(tokenId < _tokenIdCounter.current(), "Hand not yet claimed.");
        ...
        bytes memory brandText = abi.encodePacked(
          '</text></g><text x="250" y="83" style="fill: #C2BD88;font-size:0.85em" text-anchor="middle">Hodl\'Em</text><text x="250" y="100" style="fill: #C2BD88;font-size:0.5em;font-style:italic" text-anchor="middle">Series One</text><text x="250" y="387" style="fill: #C2BD88;font-size:0.65em;font-style:italic" text-anchor="middle">#',
          Strings.toString(tokenId),
          '</text><text x="100" y="100" style="fill: #C2BD88;"><textPath startOffset="-100%" xlink:href="#pokey" style="font-size:0.6em;">You Can\'t Bluff the Blockchain.<animate additive="sum" attributeName="startOffset" from="0%" to="100%" begin="0s" dur="25s" repeatCount="indefinite" /></textPath><textPath startOffset="5.475%" xlink:href="#pokey" style="font-size:0.6em;">You Can\'t Bluff the Blockchain.<animate additive="sum" attributeName="startOffset" from="0%" to="100%" begin="0s" dur="25s" repeatCount="indefinite" /></textPath><textPath startOffset="-45%" xlink:href="#pokey" style="font-size:0.6em;">Losers Fold\'Em, Winners Hodl\'Em.<animate additive="sum" attributeName="startOffset" from="0%" to="100%" begin="0s" dur="25s" repeatCount="indefinite" /></textPath><textPath startOffset="55%" xlink:href="#pokey" style="font-size:0.6em;">Losers Fold\'Em, Winners Hodl\'Em.<animate additive="sum" attributeName="startOffset" from="0%" to="100%" begin="0s" dur="25s" repeatCount="indefinite" /></textPath></text></svg>'
          );
        ...
        return string(abi.encodePacked("data:application/json;base64,",
            Base64.encode(bytes(abi.encodePacked(
                '{"name":"Hodl\'Em Hand #',Strings.toString(tokenId),
                '","external_url":"https://www.hodlem.net/","image":"',image,
                '","description":"Hodl\'Em Series One. The first ever Hodl\'Em tournament, shuffled and dealt on-chain. 52 card deck. One mint = One shuffle. Five card draw dealt on contract. 10,000 hands. 311,875,200 possibilities. One Champion. May the best hand win.","attributes":[{"trait_type":"Hand","value":"',trait,'"}]}'
            )))));
    }
...
library Cards {
    function getTrait(uint8[10] memory hand) internal pure returns (string memory) {
        ...
        else if (card1+1 == card2 && card2+1 == card3 && card3+1 == card4 && card4+1 == card5 && card5==13 &&
