...
/// @title BNV Opensea Proxy Registry contract
/// @author Sensible Lab
/// @dev Following the implementation on Opensea-creature
/// @dev https://github.com/ProjectOpenSea/opensea-creatures/blob/master/contracts/ERC721Tradable.sol
...
interface IBNVTokenBase is IERC721 {
    ...
    /// @notice Emitted when royalty `amount` is paid `tokenId` by `sender`
    event RoyaltyPaid(uint256 indexed tokenId, uint256 amount, address indexed sender);
    ...
    function mint(address to, uint256 tokenId, uint256 productId, string memory uri) external;
    ...
    function royaltyRateOf(uint256 productId) external view returns (uint256);

    function exists(uint256 tokenId) external view returns (bool);
    
    function productOf(uint256 tokenId) external view returns (uint256);

    function beneficiariesOf(uint256 productId) external view returns (address[] memory);

    function beneficiarySplitsOf(uint256 productId) external view returns (uint256[] memory);
...
abstract contract BNVTokenBase is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, IBNVTokenBase {
    ...
    // Mapping of product Id to beneficiaries address
    mapping(uint256 => EnumerableSet.AddressSet) private _beneficiarySets;

    // Mapping of product Id to beneficiaries rate
    ...
    function mint(address to, uint256 tokenId, uint256 productId, string memory uri) public virtual override onlyOwner {
        // call ERC721 safe mint
        _safeMint(to, tokenId);
        // set token URI
        _setTokenURI(tokenId, uri);
        // map token Id to product Id
    ...
    function burn(uint256 tokenId) public virtual onlyOwner {
        ...
        // call ERC721 burn
        _burn(tokenId);
        // set product id for this token to 0
        _productIds[tokenId] = 0;
    ...
    function payRoyalty(uint256 tokenId) public payable virtual override {
        ...
        emit RoyaltyPaid(tokenId, msg.value, _msgSender());
    }
    ...
    function royaltyPayableOf(uint256 tokenId) public view virtual override returns (uint256) {
        uint256 royaltyPayable = _lastSoldPrice[tokenId].mul(_royaltyRate[_productIds[tokenId]]).div(PERCENTAGE_DECIMAL);
        if (royaltyPayable >= _royaltyBalance[tokenId]) {
            return royaltyPayable.sub(_royaltyBalance[tokenId]);
        } else {
            return 0;
        }
    }

    /// @notice get royalty rate of product
    function royaltyRateOf(uint256 productId) public view virtual override returns (uint256) {
        return _royaltyRate[productId];
    }
    ...
    /// @notice product id of that token
    function productOf(uint256 tokenId) public view virtual override returns (uint256) {
        return _productIds[tokenId];
    }

    /// @notice beneficiaries of `productId`
    function beneficiariesOf(uint256 productId) public view virtual override returns (address[] memory) {
        address[] memory arr = new address[](_beneficiarySets[productId].length());
        for (uint i = 0; i < _beneficiarySets[productId].length(); i++) {
            arr[i] = _beneficiarySets[productId].at(i);
        }
        return arr;
    }

    /// @notice beneficiaries splits of `productId`
    function beneficiarySplitsOf(uint256 productId) public view virtual override returns (uint256[] memory) {
        uint256[] memory arr = new uint256[](_beneficiarySplitSets[productId].length);
        for (uint i = 0; i < _beneficiarySplitSets[productId].length; i++) {
            arr[i] = _beneficiarySplitSets[productId][i];
        }
        return arr;
    }
    ...
    /// @notice add product info
    function _addProduct(uint256 productId, uint256 rate, address[] memory beneficiaries, uint256[] memory beneficiarySplits) public virtual onlyOwner {
        require(beneficiaries.length == beneficiarySplits.length, "Invalid beneficiary data");
        // set royalty rate for product
        _royaltyRate[productId] = rate;
        
        // add to beneficiaries
        _addBeneficiaries(productId, beneficiaries, beneficiarySplits);
    }

    /// @notice set beneficiaries
    function _setBeneficiaries(uint256 productId, address[] memory beneficiaries, uint256[] memory beneficiarySplits) public virtual onlyOwner {
        // remove existing
        while (_beneficiarySets[productId].length() > 0) {
            _beneficiarySets[productId].remove(_beneficiarySets[productId].at(0));
            _beneficiarySplitSets[productId].pop();
        }
        // add to beneficiaries
        _addBeneficiaries(productId, beneficiaries, beneficiarySplits);
    }
    ...
    /// @notice get base URI, use IPFS
    function _baseURI() internal view virtual override returns (string memory) {
        return _uri;
    }

    /// @notice add beneficiaries
    function _addBeneficiaries(uint256 productId, address[] memory beneficiaries, uint256[] memory beneficiarySplits) internal virtual {
        // add beneficiaries
        for (uint i = 0; i < beneficiaries.length; i++) {
            _beneficiarySets[productId].add(beneficiaries[i]);
            _beneficiarySplitSets[productId].push(beneficiarySplits[i]);
        }
    }
    ...
    function _splitRoyaltyForBeneficiaries(uint256 tokenId, uint256 amount) internal virtual {
        ...
        for (uint i = 0; i < _beneficiarySets[_productIds[tokenId]].length(); i++) {
            uint256 splitAmount = amount * _beneficiarySplitSets[_productIds[tokenId]][i].div(PERCENTAGE_DECIMAL);
            Address.sendValue(payable(_beneficiarySets[_productIds[tokenId]].at(i)), splitAmount);
        }
...
contract BNVToken is BNVTokenBase, IBNVToken {
    ...
    constructor(string memory baseURI, address openseaProxyAddress) ERC721("BNV2", "BNV2") {
        _setBaseURI(baseURI);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override(BNVTokenBase) {
        if (from != address(0x0) && to != address(0x0)) {
            require(_msgSender() == ownerOf(tokenId) || isWhiteList(ownerOf(tokenId), _msgSender()), "Only whitelist / token owner is allowed");
        }
        super._beforeTokenTransfer(from, to, tokenId);
    }
    ...
    function approve(address to, uint256 tokenId) public override(BNVTokenBase) {
        require((_msgSender() == ownerOf(tokenId) && isWhiteList(ownerOf(tokenId), to)) || isWhiteList(ownerOf(tokenId), _msgSender()), "Only whitelist is allowed");
        super.approve(to, tokenId);
    }
    ...
    function getWhitelist() external view override returns (address[] memory) {
        address[] memory arr = new address[](_whitelist.length());
        for (uint i = 0; i < _whitelist.length(); i++) {
            arr[i] = _whitelist.at(i);
        }
        return arr;
    }
    ...
    function isWhiteList(address owner, address operator) public view override returns (bool) {
        // Check whether is in whitelist
        if (_whitelist.contains(operator)) {
            return true;
        }
        // Whitelist OpenSea proxy contract
        OpenseaProxyRegistry proxyRegistry = OpenseaProxyRegistry(_openseaProxyAddress);
    ...
    /// @notice get proxy registry address, only callable by owner
    function _getProxyRegistry() external view onlyOwner returns (address) {
    ...
    function _removeFromWhitelist(address existingAddress) external onlyOwner {
        _whitelist.remove(existingAddress);
    }

    /// @notice set opensea proxy registry address
    function _setProxyRegistry(address newAddress) external onlyOwner {
        _openseaProxyAddress = newAddress;
    }
