...
contract GlobalGoats is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_MintedForPromotion = 20;
    ...
    uint256 public constant MAX_MintedGoldenGoats = 100;
     ...
     uint256 public constant MAX_PreSaleGoldenGoats = 50;

    uint256 public constant MAX_PreSaleMintedGoats = 350;

    uint256 public  goldenGoatsCount;

    uint256 public  preSalegoldenGoats;
    ...
    string private _metaBaseUri = "https://globalgoats.org/meta/";
    ...
    goldenGoatsCount = 0;
    preSalegoldenGoats = 0;
     }

//public mint
    function mint(uint16 numberOfTokens,bool IsGG ) public payable {
        ...
        if(numberOfTokens >= 10){
         require(goldenGoatsCount.add(numberOfTokens) <= MAX_MintedGoldenGoats, "Cannot mint more Golden goats");
        _mintGGTokens(1,msg.sender);
        
        _mintTokens(numberOfTokens - 1);
        }
        //If Golden goats set to true 
        if(IsGG == true){
        require(goldenGoatsCount.add(numberOfTokens) <= MAX_MintedGoldenGoats, "Cannot mint more Golden goats");
                _mintGGTokens(numberOfTokens,msg.sender);
        }else{
...
//Pre-Sale Mint
    function preSalemint(uint16 numberOfTokens,bool IsGG) public payable {
        require(saleIsActive(), "Sale is not active");
        require(numberOfTokens <= MAX_PURCHASE, "Can only mint max 20 tokens per transaction");
        ...
        //If minted token is greater than 10
         if(numberOfTokens >= 10){
         require(goldenGoatsCount.add(numberOfTokens) <= MAX_MintedGoldenGoats, "Cannot mint more Golden goats");
        goldenGoatsCount += 1;
        preSalegoldenGoats += 1;
        _mintGGTokens(1,msg.sender);
        _mintTokens(numberOfTokens - 1);
        }
        //If Golden goats set to true 
        if(IsGG == true){
        require(goldenGoatsCount.add(numberOfTokens) <= MAX_MintedGoldenGoats, "Cannot mint more Golden goats");
        require(preSalegoldenGoats.add(numberOfTokens) <= MAX_PreSaleGoldenGoats, "Cannot mint more Golden goats for Pre-sale");
                goldenGoatsCount += numberOfTokens;
                preSalegoldenGoats += numberOfTokens;
                _mintGGTokens(numberOfTokens,msg.sender);
    ...
    //Golden Goats Minting
    function reserveGoldenGoatsMint(uint16 numberOfTokens , address _toAddress) external onlyOwner {
        require(goldenGoatsCount.add(numberOfTokens) <= MAX_MintedGoldenGoats, "Cannot mint more Golden goats");
        require(totalSupply().add(numberOfTokens) < collection_Supply, "Insufficient supply");
         goldenGoatsCount += numberOfTokens;
        _mintGGTokens(numberOfTokens,_toAddress);
    }
    ...
    /* Internal functions */
    function _mintGGTokens(uint16 numberOfTokens ,address _toAddress) internal {
       for (uint16 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = totalSupply();
            //uint256 tokenId = _tokenIdTracker.current();
           ...
           //push GG
           //GoldenGoat.userAddress.push(_toAddress);
           //GoldenGoat.tokenId.push(tokenId);
           goldenGoats.push(tokenId);
    ...
    function transfergg(uint256 tokenId) external payable{
        ...
        //Transfer Token 
        //safeTransferFrom(tokencurrentOwner, msg.sender, tokenId);
