...
contract qMarket is Initializable {
  ...
  event itemSold(uint256 id, uint256 tokenId, address tokenAddress, address buyer, uint256 askingPrice, uint256 royaltiesPaid, address royaltiesReceiver);
  ...
  function buyItem(uint256 id) payable external ItemExists(id) IsForSale(id) hasTransferApproval(itemsForSale[id].tokenId, itemsForSale[id].tokenAddress){
    ...
    require(msg.sender != itemsForSale[id].seller, "You cannot sell to yourself");
    ...
    if (checkRoyalties(itemsForSale[id].tokenAddress)) {
      // contract supports ERC2981 Royalties
      (address recipient, uint256 royalty1) = getRoyalties(itemsForSale[id].tokenId, itemsForSale[id].tokenAddress, msg.value);
      beneficiary1 = payable(recipient);
      itemsForSale[id].seller.transfer(msg.value-royalty1);
    ...
    emit itemSold(id, itemsForSale[id].tokenId, itemsForSale[id].tokenAddress, msg.sender, itemsForSale[id].askingPrice, royaltiesPaid, beneficiary1);
  }
