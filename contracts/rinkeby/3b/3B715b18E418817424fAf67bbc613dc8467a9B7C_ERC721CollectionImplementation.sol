...
contract ERC721CollectionImplementation is ERC721CollectionBase, AdminControlUpgradeable {
     ...
     * Initializer
     */
    function initialize(address creator, uint16 maxTokens_, uint16 purchaseLimit_, uint16 presalePurchaseLimit_, uint256 tokenPrice_, address signingAddress) public initializer {
        __Ownable_init();
 ...
 */
abstract contract ERC721CollectionBase is ReentrancyGuard, CreatorExtension, IERC721Collection, ICreatorExtensionTokenURI {
    ...
    /**
     * Perform other validation as necessary
     */
    function _validatePurchaseRestrictions(uint16 amount) internal virtual {
      // Override for other restriction checks
    }
    ...
    function purchase(uint16 amount, bytes32 message, bytes calldata signature, string calldata nonce) external payable virtual override nonReentrant {
        ...
        require(amount <= tokensRemaining(), "Too many requested");
        ...
        _validatePurchaseRestrictions(amount);
        for (uint i = 0; i < amount; i++) {
            _mint(msg.sender);
        }
    }
    ...
    /**
     * See {IERC721Collection-state}
     */
    function state() external override view returns(CollectionState memory) {
        return CollectionState(maxTokens, purchaseLimit, presalePurchaseLimit, _addressMintCount[msg.sender], tokenPrice, maxTokens-tokenCount, active, startTime, endTime, presaleInterval);
    }
    
    /**
     * @dev See {IERC721Collection-tokensRemaining}.
     */
    function tokensRemaining() public view virtual override returns(uint16) {
    ...
    /**
     * @dev See {IERC721Collection-mintedTokens}.
     */
    ...
    /**
     * @dev See {IERC721Collection-nonceUsed}.
     */
 ...
 */
interface IERC721Collection is IERC165 {

    event Unveil(uint256 collectibleId, address tokenAddress, uint256 tokenId);
    event CollectionActivated(uint256 startTime, uint256 endTime, uint256 presaleInterval);
     ...
     */
    function tokensRemaining() external view returns(uint16);
