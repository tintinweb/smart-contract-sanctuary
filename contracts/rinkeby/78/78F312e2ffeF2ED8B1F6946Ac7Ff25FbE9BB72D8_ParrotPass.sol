...
contract ParrotPass is ERC721Burnable, Ownable {
    ...
    uint256 public constant MAX_PASSES_PER_ROUND = 1000;

    bool public mintPaused = true;
    bool public freeMintPaused = true;

    IERC721 public currentFreeToken;

    address public vault;
    uint256 public price = 3 * 10**16; //0.02 ETH;
    ...
    function mint(uint256 numPasses) public payable {
        ...
        require(
            paidPassesMinted.add(numPasses) <= MAX_PASSES_PER_ROUND,
            'Not enough passes left in this round of minting.'
        );
        require(
        ...
        for (uint256 i = 0; i < numPasses; i++) {
            paidPassesMinted++;
            if (paidPassesMinted >= MAX_PASSES_PER_ROUND) {
                paidPassesMinted = 0;
                mintPaused = true;
            }
    ...
    function freeMint() public payable {
        ...
        require(
            currentFreeToken.balanceOf(msg.sender) >= 1,
            'You dont own the token needed to claim a free ParrotPass.'
        );
        require(
        ...
        require(
            freePassesMinted.add(1) <= MAX_PASSES_PER_ROUND,
            'Not enough free passes left in this round of minting.'
        );
        freePassesMinted++;
        if (freePassesMinted >= MAX_PASSES_PER_ROUND) {
            freeMintPaused = true;
            freePassesMinted = 0;
        }
    ...
    function setCurrentFreeToken(address _newTokenAddress) public onlyOwner {
        currentFreeToken = IERC721(_newTokenAddress);
    }
    ...
    function startNextMintingRound() public onlyOwner {
        freePassesMinted = 0;
        paidPassesMinted = 0;
        mintPaused = false;
