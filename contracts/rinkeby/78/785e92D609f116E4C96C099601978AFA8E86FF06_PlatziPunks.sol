...
contract PlatziPunksDNA {
    ...
    // This pseuorandom function is deterministic, not for production
    function deterministicPseudoRandomDNA(uint256 _tokenID, address _minter)
    public
    pure
    returns(uint256)
    {
        uint256 combinedParameters = _tokenID + uint160(_minter);
        bytes memory encodedParameters = abi.encodePacked(combinedParameters);
        bytes32 hashedParameters = keccak256(encodedParameters);
        return uint256(hashedParameters);
    }
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA{
    ...
    returns (string memory)
    {
        require(_exists(tokenId) , "Non existen tokenId");
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "PlatziPunks #',
                tokenId.toString(),
                '", "description": "PlatziPunks description my bro", "image":"',
                image,
                '"}'
                )
            );
