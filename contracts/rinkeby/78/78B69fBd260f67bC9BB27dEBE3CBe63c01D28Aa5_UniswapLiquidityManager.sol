...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    function _distributeFeesInTokens(
        ...
        if (_token0 == WETH || _token1 == WETH) { // hure
            if (!wethToken) {
    ...
    function transferLiquidity(address _token0, address _token1, bool wethToken, address _recipient, uint256 amount0Removed, uint256 amount1Removed) private {
        if (_token0 == WETH || _token1 == WETH) {
            (address tokenAlt, uint256 altAmount, address tokenWeth, uint256 wethAmount) = _token0 == WETH
                    ? (_token1, amount1Removed, _token0, amount0Removed)
                    : (_token0, amount0Removed, _token1, amount1Removed);

            if (wethToken) {
                if (amount0Removed > 0) TransferHelper.safeTransfer(tokenWeth, _recipient, wethAmount);
                if (amount1Removed > 0) TransferHelper.safeTransfer(tokenAlt, _recipient, altAmount);
            } else {
    ...
    function distributeFeesAndLiquidity(DistributeFeesParams memory params) public returns (uint256 indexAmount0, uint256 indexAmount1) {
        // only unipilot
        ...
        if (params.pilotToken && ethToken0Pair != address(0) && ethToken1Pair != address(0)) {
            (indexAmount0, indexAmount1) = _distributeFeesInPilot(
                params.recipient,
                token0,
                token1,
        ...
        transferLiquidity(token0, token1, params.wethToken, params.recipient, params.amount0Removed, params.amount1Removed);
    ...
    function withdraw(
        ...
        (uint256 amount0Removed, uint256 amount1Removed) = removeLiquidityWithFees(false, position.pool, params.recipient, params.liquidity);
...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    function _getAndIncrementNonce(uint256 tokenId) internal view override returns (uint256) {
