...
// DexMasterFarmer is the master of DEXSWAP. He can make DEXSWAP and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once DEXSWAP is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract DexMasterFarmer is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DEXSWAPs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DEXSWAPs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DEXSWAPs distribution occurs.
        uint256 accDexPerShare; // Accumulated DEXSWAPs per share, times 1e12. See below.
    }
    ...
    // DEXSWAP tokens created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early DEXSWAP makers.
    ...
    // The block number when DEXSWAP mining starts.
    ...
    IMigratorToDexSwap public migrator;
    ...
    event SendDexReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "DexMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's DEXSWAP allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToDexSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 dexswapForDev;
        uint256 dexswapForFarmer;
        (dexswapForDev, dexswapForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        
        if (dexswapForDev > 0) {
            dexswap.mint(devaddr, dexswapForDev);
            // For more simple, I lock reward for dev if mint reward in bonus time
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                dexswap.lock(devaddr, dexswapForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            }
        }
        dexswap.mint(address(this), dexswapForFarmer);
        pool.accDexPerShare = pool.accDexPerShare.add(dexswapForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer) {
        ...
        uint256 dexswapCanMint = dexswap.cap().sub(dexswap.totalSupply());

        if (dexswapCanMint < amount) {
            forDev = 0;
            forFarmer = dexswapCanMint;
        }
        else {
    ...
    // View function to see pending DEXSWAPs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 dexswapForFarmer;
            (, dexswapForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accDexPerShare = accDexPerShare.add(dexswapForFarmer.mul(1e12).div(lpSupply));
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            uint256 masterBal = dexswap.balanceOf(address(this));
            ...
            if(pending > 0) {
                dexswap.transfer(msg.sender, pending);
                uint256 lockAmount = 0;
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    lockAmount = pending.mul(PERCENT_LOCK_BONUS_REWARD).div(100);
                    dexswap.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendDexReward(msg.sender, _pid, pending, lockAmount);
            }
    ...
    // Deposit LP tokens to DexMasterFarmer for SUSHI allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "DexMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from DexMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "DexMasterFarmer::withdraw: not good");
    ...
    // Safe sushi transfer function, just in case if rounding error causes pool to not have enough DEXSWAPs.
    function safeDexTransfer(address _to, uint256 _amount) internal {
        uint256 dexswapBal = dexswap.balanceOf(address(this));
        if (_amount > dexswapBal) {
            dexswap.transfer(_to, dexswapBal);
        } else {
            dexswap.transfer(_to, _amount);
        }
    }
