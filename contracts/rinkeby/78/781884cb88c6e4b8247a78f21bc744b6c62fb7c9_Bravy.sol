...
contract Bravy is ERC721Enumerable, Ownable {
  ...
  uint256 public maxSupplyForFree = 7753;
  uint256 public maxMintAmount = 3;
  bool public paused = true; // pauses the minting functionality. After the announcement turn to false
  bool public revealed = false; // only after every NFT is mint we will remove the hidden image and show the real one by turning it to true
  ...
  function mint(uint256 _mintAmount) public {
    ...
    require(supply + _mintAmount <= maxSupplyForFree);
  ...
  function ownerMint(uint256 _mintAmount) public onlyOwner {
    require(totalSupply() >= maxSupplyForFree);  
