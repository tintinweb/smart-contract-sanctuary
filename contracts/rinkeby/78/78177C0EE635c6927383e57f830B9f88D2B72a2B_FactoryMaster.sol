...
contract FactoryMaster {
    ...
    ERC20MintableCappable[] public childrenErc20MintCap;
    ...
    event ChildCreatedERC20MintableCappable(address childAddress, string name, string symbol);
    event ChildCreatedERC20Standard(address childAddress, string name, string symbol);
    ...
    function createChild(Types types,string memory name,string memory symbol) external payable {
        ...
        if (types == Types.erc20Burn){
          ...
          if (types == Types.erc20Mintable){
            ...
            ERC20Mintable child = new ERC20Mintable("MinhToken", "MTK");
            childrenErc20Mint.push(child);
            emit ChildCreatedERC20Mintable(address(child), name, symbol);
        }

           if (types == Types.erc20MintableCappable){
            require(msg.value>=fee_erc20,"ERC20:value must be greater than 0.2");

            ERC20MintableCappable child = new ERC20MintableCappable(1000, name, symbol);
            childrenErc20MintCap.push(child);
            emit ChildCreatedERC20MintableCappable(address(child), name, symbol);
        }
        ...
        //     emit ChildCreatedERC20Standard(address(child), name, symbol);
        // }
...
contract ERC20MintableCappable is  ERC20{
