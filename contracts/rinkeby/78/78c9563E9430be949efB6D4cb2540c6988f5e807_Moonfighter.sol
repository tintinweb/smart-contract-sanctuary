...
contract Moonfighter is ERC721Enumerable, Ownable {
    ...
    function mintViaWTF(uint256 numberOfGasmasks) public payable {
        require(isSaleActive, "Sale is not active!");
        require(numberOfGasmasks >= 0 && numberOfGasmasks <= maxGasmasksPurchase,
            "You can only mint 20 Gasmasks at a time!");
        require(totalSupply().add(numberOfGasmasks) <= MAXGASMASK - reservedGasmasks,
        ...
        if(totalSupply() >= maxFreeMintsPerWallet){
            require(wtfContract.balanceOf(msg.sender) >= _priceWTF.mul(numberOfGasmasks), 
                "Not enough WTF fot this purchase!");
             wtfContract.transferFrom(msg.sender, wtfRecipient, _priceWTF.mul(numberOfGasmasks));
        }else{
            require(freeMintsWallet[msg.sender].add(numberOfGasmasks) <= maxFreeMintsPerWallet, 
                "You can only mint 50 Gasmasks for free!");
            freeMintsWallet[msg.sender] += numberOfGasmasks;
