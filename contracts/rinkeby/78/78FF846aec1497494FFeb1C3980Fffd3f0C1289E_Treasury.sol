...
contract Treasury is Ownable{
    ...
    IndexTokenFactory private factory;
    ...
    constructor(address _oracle, address _factory, address portfolioTokenAddress, address farmAddress,
                address uniswapRouterAddress, address uniswapFactoryAddress){
    ...
    function addSupportedCurrency(string memory name) onlyOwner external{
        ...
        currencyNames[name] = Currency({isSupported: true, name: name});
        allCurrencyNames.push(name);
    }
    function addSupportedAsset(address _asset, uint currencyId) onlyOwner external {
        require (currencyNames[allCurrencyNames[currencyId]].isSupported, "Currency not supported!");
        require(!assets[_asset].isSupported, "Asset already supported");
        assets[_asset] = Asset({ isSupported: true, currency: currencyId });
        allAssets.push(_asset);
        _assetDecimals[_asset] = ERC20(_asset).decimals();
    }

    function addSupportedToken(address _token, uint currencyId) onlyOwner external {
        require (currencyNames[allCurrencyNames[currencyId]].isSupported, "Currency not supported!");
        ...
        tokens[_token] = Token({ isSupported: true, currency: currencyId });
    ...
    function treasuryAssetBalance(uint currency) view public returns(uint){
        uint balance = 0;
        for (uint i = 0; i < allAssets.length; i++){
            if (assets[allAssets[i]].currency == currency){
                uint assetBalance = IERC20(allAssets[i]).balanceOf(address(this));
                balance += assetBalance.scaleBy(_assetDecimals[allAssets[i]], 18);
            }
        }
        return balance;
    }
