...
contract CrazyVikings is ERC721Enumerable, Ownable {
    ...
    uint256 public constant PRE_SALE_MAX_MINT = 3;
    string private constant HIDDEN_TOKEN_URI = "https://ipfs.io/ipfs/QmNSzW2yzPyE2drCEoLYNS7yxQqu6MWPCu7ndxbaXM4Dz5";
    ...
    event CrazyVikingsMinted(uint256 id, address owner);

    constructor() ERC721("CrazyVikings", "VIK")  {
        
    }

    function mintViking(uint256 numberOfTokens) external payable {

        uint256 _supply = totalSupply();
        require(saleActive, "Sale is inactive");
        require(numberOfTokens <= MAX_MINT, "You can mint 20 tokens at a time");
        require(_supply + numberOfTokens <= (SUPPLY - _reserve), "Purchase exceeds maximum supply of Crazy Vikings");
        require(msg.value >= PRICE * numberOfTokens, "Ether sent is not correct");

        for(uint256 i = 1; i <= numberOfTokens; i++) {
            uint id = _supply + i;
            _safeMint(msg.sender, id);
            emit CrazyVikingsMinted(id, msg.sender);
        }
    }

    function reserveVikings(address _to, uint256 _amount) external onlyOwner() {
    ...
    function presaleViking(uint256 numberOfTokens) external payable {
