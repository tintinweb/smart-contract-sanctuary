...
contract Drrt is Context, ERC165, Ownable, IERC721Metadata {
    ...
    uint public constant MAX_TOKENS_MINT_AT_ONCE = 20;

    uint public constant MAX_TOKENS_SUPPLY = 9090;

    uint public constant TOKEN_PRICE = 70000000000000000;
    ...
    function tokenByIndex(uint256 tokenIndex) public view returns (uint256) {
        (uint256 index, ) = _tokenOwners.at(tokenIndex);
        return index;
    }
    ...
    function mintGiveaway(address to) public onlyOwner {
        require(_givenTokens <= MAX_GIVEAWAY_TOKENS, "Giveaway limit is exceeded");
    ...
    function mint(uint tokenAmount) public payable {
        ...
        require(totalSupply().add(tokenAmount) <= MAX_TOKENS_SUPPLY - MAX_GIVEAWAY_TOKENS, "Purchase would exceed max supply");
        require(tokenAmount <= MAX_TOKENS_MINT_AT_ONCE, "Can only mint 20 tokens at a time");

        require(msg.value >= TOKEN_PRICE * tokenAmount, "Ether value sent is not correct");

        for (uint i = 0; i < tokenAmount; i++) {
            _safeMint(_msgSender(), totalSupply());
        }

        if (msg.value - TOKEN_PRICE > 0) {
            payable(_msgSender()).transfer(msg.value - TOKEN_PRICE);
        }
    }

    function tokenURI(uint tokenIndex) public view returns (string memory) {
    ...
    function approve(address to, uint tokenIndex) public virtual override {
    ...
    function _isApprovedOrOwner(address spender, uint tokenIndex) internal view returns (bool) {
        ...
        address owner = ownerOf(tokenIndex);
        return (spender == owner || getApproved(tokenIndex) == spender || isApprovedForAll(owner, spender));
    }
