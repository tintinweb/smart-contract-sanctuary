...
contract Reservation {
    ...
    function clearancePrivate(bytes32 dealId) private onlyCustomerOrHotel(dealId) {
        ReservationDeal storage deal = deals[dealId];
        require(deal.dealState == StateType.Reserved || deal.dealState == StateType.Cancel, "Invalid dealState");
        
        uint price = deal.price;
        uint priceOwner = price / 10;
        uint priceHotel = priceOwner * 9;
        if (deal.dealState == StateType.Reserved) {
            payable(owner).transfer(priceOwner);
            payable(deal.hotel).transfer(priceHotel);
