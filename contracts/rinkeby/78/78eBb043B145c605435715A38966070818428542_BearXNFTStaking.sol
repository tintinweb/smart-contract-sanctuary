...
contract BearXNFTStaking is OwnableUpgradeable{
    ...
    function initialize() public initializer{
        __Ownable_init();
        BearXNFTAddress = 0xe3905E39D162866dccB5C1B4Fde302E9DF9e5A14;
        ROOTxTokenAddress = 0xF6B654Ec1D7738435d0D21DA9104e039C7b88584;
        SROOTxTokenAddress = 0x8f6BF21B653a526C679C5f41dCf775D7aC817Bdd;
        totalStakes = 0; 
        MaxSROOTXrate = 100*10**18;
        MinSROOTXrate = 50*10**18;
        MaxRate = 11050*10**18;
...
///SROOT 
            for(uint256 i = 0; i < stakes[_addr].length; i++){
                if(isSpecialBear(stakes[_addr][i].nft_id)){
                    uint256 dd = calDay(stakes[_addr][i].claimedDate_SROOT);
                    claimAmountOfSROOTxToken = claimAmountOfSROOTxToken.add(200 * dd);
                }
            }
...
///WETH
            for(uint256 i = 0; i < stakes[_addr].length; i++){
                if(isSpecialBear(stakes[_addr][i].nft_id)){
                     uint256 dd = calDay(stakes[_addr][i].claimedDate_WETH);
                    claimAmountOfWETH = claimAmountOfWETH.add(200 * dd);
                }
            }
