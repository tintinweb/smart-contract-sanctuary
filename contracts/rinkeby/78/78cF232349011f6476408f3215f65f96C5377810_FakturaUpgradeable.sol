...
UUPSUpgradeable
{
    ...
    function initialize(
        ...
        __TreasuryNode_init(payable(msg.sender), _creatorPaymentAddress, _secondaryFakturaFeeBasisPoints, _secondaryCreatorFeeBasisPoints);
    ...
    function safeMint(address to, string memory uri) public onlyOwner {
        ...
        _setTokenURI(_tokenIdCounter.current(), append(uri, '?tokenId=', _tokenIdCounter.current(), '&hash=', keccak256(abi.encodePacked(msg.sender, _tokenIdCounter.current()))));
        _tokenIdCounter.increment();
    }

    function append(string memory uri, string memory t, uint tokenId, string memory h, bytes32 hash) internal pure returns (string memory) {

        return string(abi.encodePacked(uri, t, tokenId, h, hash));
