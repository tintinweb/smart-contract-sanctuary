...
contract BasicIssuanceModule is ModuleBase, ReentrancyGuard {
    ...
    address private constant router2Address = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    ...
    function buyAndissue(ISetToken _setToken, uint256 _quantity,uint256 deadline) payable public  returns(bool){
        require(_quantity > 0, "buyAndissue: buy quantity must be > 0");
        (
            address[] memory components,
            uint256[] memory componentQuantities
        ) = getRequiredComponentUnitsForIssue(_setToken, _quantity);
        ...
        path[0] = weth;
        uint preSendEthAmount = 0;
        for (uint256 i = 0; i < components.length; i++) {
            path[1] = components[i];
            uint[] memory expireAmount = IUniswapV2Router02(router2Address).getAmountsIn(componentQuantities[i],path);
            preSendEthAmount += expireAmount[0];
        }
        require(preSendEthAmount * 1003/1000 <= msg.value,"buyAndissue: ETH_VALUE_NOT_ENOUGH");
        for (uint256 i = 0; i < components.length; i++) {
            path[1] = components[i];
            uint[] memory expireAmount = IUniswapV2Router02(router2Address).getAmountsIn(componentQuantities[i],path);
            uint sendEthAmount = expireAmount[0] * 1003 / 1000;
            IUniswapV2Router02(router2Address).swapETHForExactTokens{value:sendEthAmount}(componentQuantities[i],path,address(this),deadline);  
            require(IERC20(components[i]).balanceOf(address(this)) >= componentQuantities[i],"buyAndissue: UNDERLYING_TOKEN_NOT_ENOUGH");
            IERC20(components[i]).transfer(address(_setToken),componentQuantities[i]);
        }
        
        _setToken.mint(msg.sender, _quantity);
        emit SetTokenIssued(address(_setToken), msg.sender, msg.sender, address(0), _quantity);
        (bool success,) = msg.sender.call{value:address(this).balance}(new bytes(0));
        require(success, 'buyAndissue: ETH_TRANSFER_FAILED');
        return success;
    }
    
    function getPreSendValue(ISetToken _setToken, uint256 _quantity) view public returns(uint256){
         require(_quantity > 0, "buyAndissue: buy quantity must be > 0");
        (
            address[] memory components,
            uint256[] memory componentQuantities
        ...
        return preSendEthAmount * 1003/1000;
    }
}
