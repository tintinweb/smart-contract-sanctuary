...
interface IClaims {
    ...
    function setClaim(uint256 tokenId, address issuer, bytes32 key, bytes32 value) external;
    function setSelfClaim(uint256 tokenId, bytes32 key, bytes32 value) external;
    function getClaim(uint256 tokenId, address issuer, bytes32 key) external view returns(bytes32);
    function removeClaim(uint256 tokenId, address issuer, bytes32 key) external;

    event ClaimSet(address indexed issuer, uint256 indexed tokenId, bytes32 indexed key, bytes32 value, uint updatedAt);
    event ClaimRemoved(address indexed issuer, uint256 indexed tokenId, bytes32 indexed key, uint removedAt);
}
...
contract DogERC721 is ERC721, IClaims, Ownable {
    ...
    Dog[] public pack;
    mapping(uint256 => mapping(address => mapping(bytes32 => bytes32))) public registry;
    ...
    // create or update claims
    function setClaim(uint256 tokenId, address issuer, bytes32 key, bytes32 value) external override onlyOwner() {
        _setClaim(tokenId, issuer, key, value);
    }

    function setSelfClaim(uint256 tokenId, bytes32 key, bytes32 value) external override onlyDogOwner(tokenId) {
        _setClaim(tokenId, msg.sender, key, value);
    }

    function getClaim(uint256 tokenId, address issuer, bytes32 key) external override view returns(bytes32) {
