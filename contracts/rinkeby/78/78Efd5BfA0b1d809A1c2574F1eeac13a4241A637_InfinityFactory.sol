...
import "./sales/DirectSales.sol";
import "./sales/DutchAuctions.sol";

contract InfinityFactory is Ownable {
    uint256 public teamBPS;
    address public clonableTokenAddress;
    address public clonableDirectSalesAddress;
    address public clonableDutchAuctionsAddress;
    ...
    event NewDirectSalesCreated(address indexed owner, address contractAddress);
    event NewDutchAuctionsCreated(address indexed owner, address contractAddress);
    ...
    function setClonableDirectSalesAddress(address _clonableDirectSalesAddress) public onlyOwner {
        clonableDirectSalesAddress = _clonableDirectSalesAddress;
    }

    function setClonableDutchAuctionsAddress(address _clonableDutchAuctionsAddress) public onlyOwner {
        clonableDutchAuctionsAddress = _clonableDutchAuctionsAddress;
    }
    ...
    function createTokenContract(string memory name, string memory symbol ) public returns (address) {
        ...
        infinityTokensPublic asset = infinityTokensPublic(clone);
        asset.init(name, symbol, factoryAddress, msg.sender);
        asset.transferOwnership(msg.sender);
        emit NewInfinityTokenCreated(msg.sender, clone);
        return clone;
    }

    function createDirectSalesContract(string memory name ) public returns (address) {
        address factoryAddress = whatIsMyAddress();
        address clone = Clones.clone(clonableDirectSalesAddress);
        DirectSales cloneContract = DirectSales(clone);
        cloneContract.init(name, factoryAddress);
        cloneContract.transferOwnership(msg.sender);
        emit NewDirectSalesCreated(msg.sender, clone);
        return clone;
    }
  
    function createDutchAuctionsContract(string memory name ) public returns (address) {
        address factoryAddress = whatIsMyAddress();
        address clone = Clones.clone(clonableDutchAuctionsAddress);
        DutchAuctions cloneContract = DutchAuctions(clone);
        cloneContract.init(name, factoryAddress);
        cloneContract.transferOwnership(msg.sender);
        emit NewDutchAuctionsCreated(msg.sender, clone);
        return clone;
    }
...
contract DirectSales is OwnableClone, RevenueSplit, SalesHistory {
    using EnumerableSet for EnumerableSet.UintSet;
    ...
    event ListingPurchased( uint256 indexed listingId, address buyer, address recipient );
    ...
    function purchase(uint256 listingId, address _recipient) public payable {
        ...
        postSale(listing.tokenAddress, listing.offeringId, msg.sender, _recipient, listing.price, block.timestamp);
        emit ListingPurchased(listingId, msg.sender, _recipient);
	}
    ...
    function getAllListings() public view returns (OutputListing [] memory listings) {
        ...
        for(uint256 idx = 0; idx < numListings; idx++) {
            ...
            listings[idx].price        = listingsById[lid].price;
        }
    }
