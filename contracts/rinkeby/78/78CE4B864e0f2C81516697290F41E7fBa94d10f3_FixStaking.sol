...
contract FixStaking is AccessControl {
    ...
    // staker address => term index => staker Info
    mapping(address => mapping(uint256 => StakerInfo)) public stakers;
    // staker address => staker index
    ...
    /**
     @param _duration epoch time
     @param _apy 1 year rate should be 18 decimal
     @param _mainPenaltyRate Percentage of penalty to be deducted from the user's principal
    ...
    function deposit(uint256 _amount, uint256 _poolIndex) external {
        ...
        staker.closedTime = block.timestamp + pools[_poolIndex].duration;
    ...
    function withdraw(uint256 _stakerIndex) external {
        ...
        require(staker.amount > 0, "Insufficient amount");
        ...
        delete stakers[msg.sender][_stakerIndex];
        stakerIndex[msg.sender] -= 1;
        ...
        returns (uint256)
    {
        ...
        uint256 rateToSec = ((rate / 12) * 1e36) / 30 days;
        uint256 percent = (rateToSec * duration) / 1e36;

        return (staker.amount * percent) / 1e18;
    }
