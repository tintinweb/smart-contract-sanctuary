...
contract KCLWealth is Ownable {
    ...
    uint public basicTax = 2;
    
    uint public standardTax = 3;
    
    uint public premiumTax = 5;
    ...
    function buyKCLPoints(uint _amount, Plan plan) public {
        require(plan == Plan.Basic || plan == Plan.Standard || plan == Plan.Premium, "Not a valid plan");
        ...
        uint proportionBasic = _amount.div(100).mul(basicTax) / basicInvestors.length;
        for (uint i = 0; i < basicInvestors.length; i++) {
            if (!withdrawn[basicInvestors[i]]) {
                myKCLRewards[basicInvestors[i]] = myKCLRewards[basicInvestors[i]].add(proportionBasic);       
            }
        }
        
        
        uint proportionStandard = _amount.div(100).mul(standardTax) / standardInvestors.length;
        for (uint i = 0; i < standardInvestors.length; i++) {
            if (!withdrawn[standardInvestors[i]]) {
                myKCLRewards[standardInvestors[i]] = myKCLRewards[standardInvestors[i]].add(proportionStandard);       
            }
        }
        
        uint proportionPremium = _amount.div(100).mul(premiumTax) / premiumInvestors.length;
        for (uint i = 0; i < premiumInvestors.length; i++) {
            if (!withdrawn[premiumInvestors[i]]) {
    ...
    function buyKCLPoints(uint _amount, address _referrer, Plan plan) public {
        require(plan == Plan.Basic || plan == Plan.Standard || plan == Plan.Premium, "Not a valid plan");
        require(!blacklist[msg.sender], "You are blacklisted");
        require(_amount >= minimumInvestment, "Purchase is below minimum");
        require(_amount <= maximumInvestment, "Purchase is above maximum");
        ...
        if (referrer[up] != address(0)) {
            while (referrer[up] != address(0)) {
                ...
                if (counter < 5) {
                    up = referrer[up];
                    if (counter == 2) {
                        myKCLReferrals[up] = myKCLReferrals[up].add(_amount.div(100).mul(up2Tax));
                    } else if (counter == 3) {
                        myKCLReferrals[up] = myKCLReferrals[up].add(_amount.div(100).mul(up3Tax));
                    } else {
                        myKCLReferrals[up] = myKCLReferrals[up].add(_amount.div(100).mul(up4Tax));                 
                    }                    
                } else {
                    break;
                }
    ...
    function getFullTax() public view returns(uint) {
        uint fullTax = SafeMath.add(adminTax, basicTax).add(standardTax).add(premiumTax).add(up1Tax).add(up2Tax).add(up3Tax).add(up4Tax);
        return fullTax;
    }
