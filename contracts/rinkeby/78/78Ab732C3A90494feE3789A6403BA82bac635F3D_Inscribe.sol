...
contract Inscribe is InscribeInterface {
    ...
    function setPermissions(bytes32[] memory inscriptionIds, bool[] memory permissions) external override {
        ...
        for (uint256 i = 0; i < inscriptionIds.length; i++) {
             _addWhitelist(msg.sender, inscriptionIds[i], permissions[i]);
        }
    }
    ...
    function addInscription(
        ...
        // Add whitelist
        if (addWhiteList) {
             _addWhitelist(msg.sender, inscriptionId, true);
        }
    ...
    */
    ...
    function _addWhitelist(address owner, bytes32 inscriptionId, bool permission) private {
        _whiteList[inscriptionId][owner] = permission;
        emit WhitelistAdjusted(owner, inscriptionId, permission);
    }

    function _updateInscriptionNonce(NFTLocation memory location) private {
        bytes32 inscriptionHash = keccak256(abi.encodePacked(location.nftAddress, location.tokenId, location.chainId));
...
interface InscribeInterface {
    ...
    event WhitelistAdjusted(address owner,
                            bytes32 inscriptionId, 
                            bool permission);
