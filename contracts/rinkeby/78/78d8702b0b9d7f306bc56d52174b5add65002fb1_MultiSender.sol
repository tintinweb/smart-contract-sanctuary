...
contract MultiSender is Ownable{
    ...
    address public receiverAddress = 0xf01DBF91fE13e921d3EC80999C76B2B9a17Eb928;
    address public txAddress = 0xf01DBF91fE13e921d3EC80999C76B2B9a17Eb928;
    address public charity = 0xa4EaE3200fc72b47F863B681f1A565132b20Ba46;
    uint public txFee = 0.05 ether;
    uint public devFee = 0.02 ether;
  ...
  function removeFromVIPList(address[] _vipList) onlyOwner public {
   ...
   function ethSendSameValue(address[] _to, uint _value) internal {
        ...
        if(vip){
            ...
            require(remainingValue >= sendAmount.add(txFee).add(devFee)) ;
        }
		...
		txAddress.transfer(txFee);
		charity.transfer(devFee);
    ...
    function coinSendSameValue(address _tokenAddress, address[] _to, uint _value)  internal {
	    ...
	    bool vip = isVIP(msg.sender);
        if(!vip){
		    require(sendValue >= (txFee + devFee));
        }
		require(_to.length <= 255);
