...
contract HashAvatars is ERC1155Base {
    string public name;
    ...
    constructor(string memory _name, string memory _symbol, address[] memory _founders, string memory contractURI, string memory tokenURIPrefix) ERC1155Base(contractURI, tokenURIPrefix) public {
        name = _name;
        symbol = _symbol;
        founders = _founders;
    ...
    function mint(uint256 id, Fee[] memory fees, uint256 supply, string memory uri) public payable {
        require(msg.value == 1*10**18,"The cost to mint is 1 xdai");
        bytes32 uriHash = keccak256(abi.encodePacked(uri));
        require(!uriHashUsed[uriHash],"Token with same uri already was minted");
        require(id == totalSupply().add(1) && id <= 7*10**9);
        uriHashUsed[uriHash] = true;
        _sendToFounders(msg.value);
        _totalSupply = _totalSupply.add(1);
        _mint(id, fees, supply, uri);
    }

    function _sendToFounders(uint256 total) private {
      uint256 part = total.div(founders.length);
...
contract ERC20Rewards is Ownable,ReentrancyGuard {
  IERC20 public token;
  HashAvatars public collectible;
  ...
  event Claimed(address indexed creator,uint256 indexed tokenId);

  constructor(IERC20 _token,HashAvatars _collectible,uint256 _reward) public {
      token = _token;
  ...
  function claim(uint256 id) external {
    require(collectible.creators(id) == msg.sender);
    require(!claimed[msg.sender][id],"Already claimed");
    uint256 divAmount = 1;
    if(id > 10){
      divAmount = id.mod(10);
    }
    require(token.transfer(msg.sender,reward.div(divAmount)),"Contract without sufficient ERC20");
  ...
  function withdrawERC20(address _to,address _token,uint256 _total) external onlyOwner nonReentrant {
      IERC20 t = IERC20(_token);
      t.transfer(_to,_total);
  }
