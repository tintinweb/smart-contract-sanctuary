...
contract Escrow is Migratable {
    ...
    //event OnCreateEscrow(address indexed dapp, address indexed client, address indexed provider, uint256 amount, uint256 paprbusdlpBalance, uint256 proofDeposit);
    ...
    //     function breakLP(uint256 LPamount) internal {
    //     IUniswapV2Pair pair = IUniswapV2Pair(want);
    //     address token0 = pair.token0();
    //     address token1 = pair.token1();
    //     IUniswapRouterETH(unirouter).removeLiquidity(token0, token1, LPamount, 0, 0, address(this), block.timestamp + 600);
    // }

    // function depositToVault() public {
        
    // IPAPRBUSDVault(vault).depositAll();
    ...
    returns (uint256) {
        ...
        escrow.id = id;
        //escrow.PAPRBUSDlpBalance = paprLPBalance;
    ...
    returns (
      ...
      {      
        ...
        return (
            ...
            //escrows[_escrowId].PAPRBUSDlpBalance,
