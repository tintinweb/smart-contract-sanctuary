...
contract TnDGame is ITnDGame, Ownable, ReentrancyGuard, Pausable {
  ...
  modifier requireContractsSet() {
      require(address(gtToken) != address(0) && address(traits) != address(0) 
        && address(tndNFT) != address(0) && address(tower) != address(0) && address(altar) != address(0)
         && address(randomizer) != address(0)
        , "Contracts not set");
      _;
  }

  function setContracts(address _gt, address _traits, address _tnd, address _tower, address _altar, address _rand) external onlyOwner {
  ...
  function sacrifice(uint256 tokenId, uint256 gtAmt) external whenNotPaused nonReentrant {
    ...
    require(gtMintCost > 0, "Sacrificial altar currently closed");
    if(nft.isTemplar) {
      // Wizard sacrifice requires 3x $GT curve
      ...
      // Dragon sacrifice requires 4x $GT curve
