...
contract MintableToken is ERC20Upgradeable {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "MAT: ONLY OWNER");
        _;
    }

    modifier onlyMinter() {
        require(msg.sender == minter, "MAT: ONLY MINTER");
        _;
    }
    ...
    function setCapacity(uint256 _cap) external onlyOwner {
        require(_cap >= totalSupply(), "MAT: INVALID CAPACITY");
        cap = _cap;
    }

    function mint(address user, uint256 amount) public onlyMinter {
        require(totalSupply() + amount < cap, "MAT: FULL CAPACITY");
        _mint(user, amount);
    }
