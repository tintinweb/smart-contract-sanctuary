...
contract GenTickets is ERC1155, ERC1155Receiver {
    ...
    function redeemTicket(address _to, uint256 _id, uint256 _amount) public { //address, 0, 1
        ...
        uint averageTokensAvailableToClaim = (ticketType.ticketSize.div((ticketType.totalTranches).sub(ticketType.cliffTranches))).mul(_amount); // 10/(5-1)*1 = 2
        ...
        uint mulCliffedLevel = (eligibleTraches + ticketType.cliffTranches).sub(tranche); //1+1-0=2 | 2+1-2=1 | 3+1-3=1
        ...
        bytes memory data;
        _mint(_to, _id.add(numTicketTypes.mul(mulCliffedLevel)), _amount, data); //check later 0 + 4*2 = 8 | 8+4*1 = 12 | 12+4*1=16 | 16+4*1=20
    }
