...
contract Staking {
     ...
     */
    constructor() {
        tiers.push(Tier(7 days, 25));
        tiers.push(Tier(30 days, 100));
        tiers.push(Tier(90 days, 150));
        tiers.push(Tier(180 days, 200));
    ...
    function withdraw(uint index) public returns (bool) {
        BEP20 token = BEP20(stakeTokenAddr);
        address sender = msg.sender;
        uint amount = withdrawable(sender, index);
        require(amount > 0, "User has withdrawn or has not staked");
        token.transfer(sender, amount);
        user[sender].deposits[index].withdrawn = true;
        return true;
    }
