...
contract Migrator is AccessControl {
    ...
    // Variables
    mapping (address => uint8) public usersStatus;
    ...
    function migrate(uint[] calldata amount, uint expireBlock, uint8 migrationStatus, bytes[] calldata sigs)
        public 
    {
        ...
        bytes32 _hash = keccak256(abi.encodePacked(msg.sender, expireBlock, migrationStatus, getChainID(), amount));
		require(verify(_hash.toEthSignedMessageHash(), sigs), "Migrator::migrate: Invalid signatures");
