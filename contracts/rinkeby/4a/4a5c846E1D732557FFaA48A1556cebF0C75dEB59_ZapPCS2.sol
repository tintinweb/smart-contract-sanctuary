...
contract ZapPCS2 is OwnableUpgradeable {
    ...
    address private constant AFIB = 0xC6bfcf0469a74b36c070b807162fFcbF7B0a1103;
    ...
    function lpPrice(address token) public view returns (uint256){
        require(isFlip(token), "not LP token");
            ...
            uint256 price  = (r0*price0 + r1*price1)/pair.totalSupply();
            return price;
    }
    ...
    function _swap(address _from, uint amount, address _to, address receiver) private returns (uint) {
        ...
        if (intermediate != address(0) && (_from == WBNB || _to == WBNB)) {
            ...
            // [WBNB, AFIB] or [AFIB, WBNB]
            path = new address[](2);
            path[0] = _from;
            path[1] = _to;
        } else {
            // [USDT, AFIB] or [AFIB, USDT]
            path = new address[](3);
            path[0] = _from;
            path[1] = WBNB;
            path[2] = _to;
