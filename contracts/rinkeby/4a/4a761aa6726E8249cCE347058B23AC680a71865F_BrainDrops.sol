...
contract BrainDrops is ERC721, Ownable {
    ...
    constructor() ERC721("BrainDrops", "BRAIN") {
    ...
    mapping(uint256 => uint256) public projectIdToStartingIndexBlock;
    ...
    function setProjectStartingIndex(uint256 _projectId) public {
        ...
        require(projectIdToStartingIndexBlock[_projectId] != 0, "Starting index block must be set");

        projectIdToStartingIndex[_projectId] = uint(blockhash(projectIdToStartingIndexBlock[_projectId])) % projectIdToMaxAmount[_projectId];
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(projectIdToStartingIndexBlock[_projectId]) > 255) {
            projectIdToStartingIndex[_projectId] = uint(blockhash(block.number - 1)) % projectIdToMaxAmount[_projectId];
        }
    ...
    /**
     * Set the starting index block for a project, essentially unblocking
     * setting starting index
     */
    function emergencySetStartingIndexBlock(uint256 _projectId) public onlyOwner {
        require(projectIdToStartingIndexBlock[_projectId] == 0, "Starting index is already set");

        projectIdToStartingIndexBlock[_projectId] = block.number;
    }
      ...
      returns (uint256)
    {
      ...
      if (projectIdToStartingIndexBlock[_projectId] == 0) {
        projectIdToStartingIndexBlock[_projectId] = block.number;
      }
