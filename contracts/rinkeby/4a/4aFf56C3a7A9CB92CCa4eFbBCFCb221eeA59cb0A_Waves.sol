...
contract Waves is ERC721, Ownable {
    mapping(address => uint256) private _mintedList;
    uint256 totalAvailable;
    uint256 tokenCounter;

    constructor() ERC721("Waves", "WAVES") public { 
        totalAvailable = 5000;
        tokenCounter = 0;
    }

    function mintToken(uint tokenId, string memory tokenURI, uint _numberOfMintsAllowed) public returns (uint) {
        require(_mintedList[msg.sender] < _numberOfMintsAllowed, 'You have minted your max number of Waves');
        require(tokenCounter <= totalAvailable);
