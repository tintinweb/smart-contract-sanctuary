...
contract BlockBeats is ERC721Enumerable, Ownable, ReentrancyGuard {

    event GenerateBeat(uint256 indexed tokenId);
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    Counters.Counter private _reservedTokenIds;

    uint256 private constant MAX_BEATS = 10000;
    uint256 private constant FOUNDERS_RESERVE_AMOUNT = 85;
    uint256 private constant MAX_PUBLIC_BEATS = MAX_BEATS - FOUNDERS_RESERVE_AMOUNT;
    ...
    constructor() ERC721("Block Beats", "BBT") {}
    ...
    function setProvenanceHash(string memory prHash) public onlyOwner {
        provenanceHash = prHash;
    }

    function mintPublicSale(uint256 _count) external payable nonReentrant whenPublicSaleActive returns (uint256, uint256) {
        require(_count > 0 && _count <= MAX_PER_ADDRESS, "Invalid Beat count");
        require(_tokenIds.current() + _count <= MAX_PUBLIC_BEATS, "All Beats have been minted");
    ...
    function mintEarlyAccess(uint256 _count) external payable nonReentrant whenEarlyAccessActive returns (uint256, uint256) {
        require(_count != 0, "Invalid Beat count");
    ...
    function founderMint(uint256 _count) public nonReentrant returns (uint256, uint256) {
        require(_count > 0 && _count <= MAX_PER_ADDRESS, "Invalid Beat count");
        require(_reservedTokenIds.current() + _count <= FOUNDERS_RESERVE_AMOUNT, "All reserved Beats have been minted");
        require(founderMintCountsRemaining[msg.sender] >= _count, "You cannot mint this many reserved Beats");

        uint256 firstMintedId = MAX_PUBLIC_BEATS + _tokenIds.current() + 1;
        for (uint256 i = 0; i < _count; i++) {
            _reservedTokenIds.increment();
            _safeMint(msg.sender, MAX_PUBLIC_BEATS + _reservedTokenIds.current());
        }
