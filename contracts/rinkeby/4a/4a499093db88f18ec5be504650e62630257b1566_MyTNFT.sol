...
contract MyTNFT is ERC721Enumerable, Ownable {
    ...
    event Airdrop(address to, uint qty);
    event Withdraw(uint amount);

    // constructor function
    constructor() ERC721("MyTNFT", "MTNFT") {
    }

    // airdrop superstars
    function airdropSuperstars(address addr, uint256 tokenCount) public onlyOwner {
      // CAUTION - NO CHECK FOR SUPPLY IN THIS
      uint supply = totalSupply();
      uint i;
      for (i = 1; i <= tokenCount; i++) {
        _safeMint(addr, supply + i);
      }
      emit Airdrop(addr, tokenCount);
    }
    ...
    function remainingViplistedCount(address addr) public view returns (uint remainingViplisted) {
      // TODO to check the working of line below when no address is mapped.
      List memory record;
      record = viplist[addr];
    ...
    // Giftlist count updations
    // it will override previous counts if entry is already present
    ...
    function remainingGiftlistCount(address addr) public view returns (uint remainingGifts) {
      // TODO to check the working of line below when no address is mapped.
      // Figure out gas for below initialization
      List memory record;
    ...
    function claimSuperstarGifts(uint256 tokenCount) public {
      ...
      require(_remainingGiftlistCount > 0, "No gifts allotted OR Already claimed");
    ...
    function mintSuperstar(uint256 tokenCount) public payable {
      ...
      if(presaleIsActive) {
        ...
        require(_remainingViplistedCount > 0, "Not viplisted OR Already minted all");
        require(tokenCount <= _remainingViplistedCount, "Exceeds max allowed mint in viplist");
        viplist[msg.sender].hasMinted = viplist[msg.sender].hasMinted + tokenCount;
      } else {
        require(tokenCount <= maxPublicMintPerTx, "Exceeds max public per tx count");
      }
