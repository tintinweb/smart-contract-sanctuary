...
contract FChain is ERC721, Ownable, ERC721Enumerable {
  ...
  event FChMinted(uint256 tokenId, address owner);

  constructor() ERC721("FChain", "FC") {}
  ...
  function mintFCh(uint256 numberOfTokens) external payable {
    ...
    for (uint256 i = 0; i < numberOfTokens; i++) {
      ...
      if (totalSupply() < MAX_TOKENS) {
        ...
        emit FChMinted(id, msg.sender);
      }
    }
  }

  function presaleFCh(uint256 numberOfTokens) external payable {
  ...
  returns (string memory)
  {
    ...
    if (isRevealed == false) {
      return
      "ipfs://Qmc43hxTGtRupjE56usL6LnkR8muEVa7Hqp4Ni2ajwVJBr/hidden.json";
    }
