...
contract MegaSena is VRFConsumerBase {
    ...
    * To test wins. 
    ...
    function distributePrize() public{
        uint winners = betToAddresses[uint(keccak256(abi.encode(winningNumbers)))].length;
        uint splitPrize = address(this).balance / winners; // have to learn how to calculate this correctly.
        for(uint i = 0; i < winners; i++){
            address payable a = payable(betToAddresses[uint(keccak256(abi.encode(winningNumbers)))][i]);
