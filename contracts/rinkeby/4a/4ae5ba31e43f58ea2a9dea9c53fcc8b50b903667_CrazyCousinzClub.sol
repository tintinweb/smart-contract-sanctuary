...
contract CrazyCousinzClub is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant CCC_MAX = 10000;
    uint256 public PRICE = 40_000_000_000_000_000; // 0.04 ETH

    string private _contractURI = "";
    mapping(uint256 => string) _mappingTokenURI;
    bool private _isActive = false;

    Counters.Counter private _publicCCC;

    constructor() ERC721("Crazy Cousinz Club", "CCC") {}
    ...
    function purchase(string memory strUri) external payable {
        require(_isActive, "Contract is not active");
        require(
            _publicCCC.current() < CCC_MAX,
            "Purchase would exceed CCC_MAX"
        );
        ...
        uint256 tokenId = _publicCCC.current();

        if (_publicCCC.current() < CCC_MAX) {
            _publicCCC.increment();
            _safeMint(msg.sender, tokenId);

            if (tokenId > 0) {
                require( keccak256(abi.encodePacked((_mappingTokenURI[tokenId - 1]))) != keccak256(abi.encodePacked((strUri))));
            }

            _mappingTokenURI[tokenId] = strUri;
        }
    }
        ...
        returns (string memory)
    {
        ...
        return _mappingTokenURI[tokenId];
    }
