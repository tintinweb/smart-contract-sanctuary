...
contract InitializableOwnable {
    ...
    modifier notInitialized() {
        require(!_INITIALIZED_, "SHANHU_INITIALIZED");
        _;
    }
...
// File: contracts/SHANHUMysteryBox/MysteryBoxV1.sol




contract Shanhutest is ERC721URIStorage, InitializableOwnable {
    using Address for address;
    ...
    function sendToken(address to, uint256 id)  external onlyOwner {
        uint256 range = _TOKEN_IDS_.length;

        if(id != range - 1) {
            _TOKEN_IDS_[id] = _TOKEN_IDS_[range - 1];
        }
        _TOKEN_IDS_.pop();
