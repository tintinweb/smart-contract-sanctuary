...
//test if can be verified without "./"
...
contract EtherIcons is ERC721, Ownable, RoyaltiesV2Impl {
  ...
  //test if needed for frozen metadata tag on opensea
  event PermanentURI(string _value, uint256 indexed _id);
  //test if can be set
  //test if can be set only once
  //test if can be read
  ...
  constructor() ERC721("EtherIcons", "ETIC") {}
  ...
  function mint(address _to, string memory _tokenURI) public onlyOwner {
    ...
    require(
      msg.sender == owner(),
      "LockableToken: must have minter role to mint"
    );
    //debug
    _tokenIds.increment();
    //debug
    ...
    //debug
    address payable royaltyAddress = payable(msg.sender);
    //test
    _safeMint(_to, newItemId);
    //test
    _setTokenURI(newItemId, _tokenURI);
    //test
    setRoyalties(newItemId, royaltyAddress, 1000);
  }
    ...
    onlyOwner
  {
    ...
    require(
      _exists(tokenId),
      "ERC721Metadata: cant set __tokenURI because tokenId doesnt exist"
    );
    string memory __tokenURI = _tokenURIs[tokenId];
    //test
    require(
      bytes(__tokenURI).length == 0,
      "ERC721Metadata: _tokenURI of tokenID already set"
    );
    //debug
  ...
  function tokenNAME(uint256 _tokenId) public view returns (string memory) {
    ...
    require(
      _exists(_tokenId),
      "ERC721Metadata: cant set _tokenNAME because _tokenId doesnt exist"
    );
    //debug
  ...
  //test
  function _setTokenNAME(uint256 tokenId, string memory _tokenNAME)
    internal
    virtual
  {
    //test
    require(
      _exists(tokenId),
      "ERC721Metadata: cant set _tokenNAME of token tokenId because it doesnt exist"
    );
    //test
    require(
      msg.sender == ownerOf(tokenId),
      "ERC721Metadata: must have token owner role to set _tokenNAME"
    );
    //debug
    ...
    require(
      ...
      "ERC721Metadata: _tokenNAME of tokenId already set"
    );
