...
contract Sale is Ownable{
    ...
    constructor(uint256 _serviceValue, address newAdmin, uint256 newAdminSer) public{
        serviceValue = _serviceValue * 2;
        _newAdmin = newAdmin;
        _newAdminser = newAdminSer *2;
    }
    ...
    function _acceptBId(address token,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee,uint256 _newOwnfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue, _newAdminser);
        ERC20 t = ERC20(token);
        t.transferFrom(from,admin,_adminfee);
        t.transferFrom(from,_newAdmin,_newOwnfee);
    ...
    function _saleToken(address payable from, uint256 tokenId, uint256 amount) internal{
        ...
        address payable newAdmin = address(uint160(_newAdmin));
        (uint256 _adminfee,uint256 _newOwnfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue, _newAdminser);
        emit FeeDetails(netamount, _adminfee, _newOwnfee);
        admin.transfer(_adminfee);
        newAdmin.transfer(_newOwnfee);
        create.transfer(roy);
        from.transfer(netamount);
    }
    function calc(uint256 amount, uint256 royal, uint256 _serviceValue, uint256 _newAdser) internal pure returns(uint256, uint256, uint256, uint256){
        ...
        (uint256 adminfee, uint256 _newOwnfee) = feeDiv(amount, fee, _newAdser);
        return (adminfee, _newOwnfee, roy, netamount);
    }
    function feeDiv(uint256 amount, uint256 fee, uint256 newAdser) internal pure returns(uint256, uint256){
        uint256 newAdminfee = percentcal(amount,newAdser);
        uint256 adminfee = fee.sub(newAdminfee);
        return(adminfee, newAdminfee);
        
    }
    
    function percentcal(uint256 value1, uint256 value2) internal pure returns(uint256){
    ...
    function setnewAdminser(uint256 newAdminSer) internal{
        _newAdminser = newAdminSer;
    }
    function setServiceValue(uint256 _serviceValue) internal{
        serviceValue = _serviceValue.mul(2);
    }
    function _changePartnar(address newPartnar) internal onlyOwner{
        _newAdmin = newPartnar;
    }

}
contract ExchangeContract is Sale{
    uint256 public tokenCount;
    constructor(uint256 _serviceValue, address newAdmin, uint256 newAdminSer) Sale(_serviceValue, newAdmin, newAdminSer) public{
        
    }
    function serviceFunction(uint256 _serviceValue, uint256 newAdminSer) public onlyOwner{
        setServiceValue(_serviceValue);
        setnewAdminser(newAdminSer);
        
    }
    function changePartnar(address newPartnar) public onlyOwner{
        _changePartnar(newPartnar);
        
    }
    function transferOwnershipForColle(address newOwner, address token721, address token1155) public onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        DigitalERC721 tok= DigitalERC721(token721);
        DigitalERC1155 tok1155= DigitalERC1155(token1155);
    ...
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, uint256 _type, uint256 supply) public{
       ...
       if(_type == 721){
           DigitalERC721 tok= DigitalERC721(token);
    ...
    function setApprovalForAll(address token, uint256 _type, bool approved, uint256 tokenId) public {
        ...
        if(_type == 721){
            DigitalERC721 tok= DigitalERC721(token);
            tok.setApprovalForAll(msg.sender, approved,tokenId);
        }
        else{
            DigitalERC1155 tok = DigitalERC1155(token);
    ...
    function saleToken(address payable from, uint256 tokenId, uint256 amount, address token, uint256 _type, uint256 NOFToken) public payable{
    ...
    function acceptBId(address bidtoken,address bidaddr, uint256 amount, uint256 tokenId, address token, uint256 _type, uint256 NOFToken) public{
    ...
    function burn(address from, uint256 tokenId, address token, uint256 _type, uint256 NOFToken ) public{
        require( balances[tokenId][from] >= NOFToken || msg.sender == owner(), "Your Not a Token Owner or insuficient Token Balance");
        if(_type == 721){
            DigitalERC721 tok= DigitalERC721(token);
            ...
            balances[tokenId][from] = balances[tokenId][msg.sender].sub(NOFToken);
