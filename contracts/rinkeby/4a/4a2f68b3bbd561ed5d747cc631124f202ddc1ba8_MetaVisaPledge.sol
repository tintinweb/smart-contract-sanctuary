...
library OrderSet {
    ...
    function at(Set storage set, uint256 valueId) internal view returns (Order memory) {
        require(set._indexes[valueId] != 0, "StakeSet: set._indexes[valueId] != 0");
        uint256 index = set._indexes[valueId] - 1;
...
contract MetaVisaPledge is Ownable,IERC721Receiver {
    ...
    mapping(address => OrderSet.Set) private _metavisaOrderOf;
    mapping(address => OrderSet.Set) private _metavisaLimitOrderOf;
    mapping(address => uint256) private _income; //The pledge income
    mapping(address => uint256) private _limitIncome; //The pledge income
    ...
    uint256 private _continuousDays = 30 days; //continuous days
    uint256 private _period  = 1 hours;  //cycle
    uint256 private _fixedDailyReward = 166.6 * 10**6; //Fixed gross income is paid on a daily basis
    uint256 private _fixedDailyRewardLimit = 200 * 10 **6; //Fixed gross income is paid on a daily basis
    ...
    /**
     * @dev Initializes the contract by setting _nftAddress.
     */
    constructor(IERC721[2] memory nftAddress_,uint256 startTime_) {
    ...
    function onERC721Received(address operator,address from,uint256 tokenId,bytes calldata ) external override returns (bytes4) {
        ...
        if(address(_nftAddress[0]) == msg.sender){
            _pledgeMVC(tokenId,from);
        }else if(address(_nftAddress[1]) == msg.sender){
    ...
    function _pledgeMVC(uint256 tokenId,address account) private returns(bool) {
        checkUpdateIncomes(_allIncomes,_metavisaOrderOf[address(0)].length(),_fixedDailyReward);
    ...
    function cancelPledgeMVC(uint256 tokenId) external virtual returns(bool) {
        ...
        OrderSet.Order memory order = _metavisaOrderOf[msg.sender].at(tokenId);
        checkUpdateIncomes(_allIncomes,_metavisaOrderOf[address(0)].length(),_fixedDailyReward);
        ...
        IERC721(_nftAddress[0]).safeTransferFrom(address(this),msg.sender,tokenId); 

        emit CancelPledge(address(_nftAddress[0]),msg.sender,tokenId,block.timestamp); 
        return true;
    }
    ...
    function checkUpdateIncomes(uint256[] storage allIncomes,uint256 totalOrders, uint256 fixedDailyReward) private returns(bool) {
        uint256 num = (block.timestamp - _startTime) / _period;
        if(num > allIncomes.length){
            uint256 income;
            if(totalOrders > 0){
                    income = fixedDailyReward / totalOrders;
            }
            updateIncomes(allIncomes,income,num - allIncomes.length);
        }
        return true;
    }
    ...
    function getIncome(address account) public view virtual returns(uint256) {
        ...
        for(uint256 i = 0;i<orders.length;i++){
            ...
            uint256 incomes = _fixedDailyReward / _metavisaOrderOf[address(0)].length();
    ...
    /**
     * @dev get '_account' stakes by page_metavisaOrderOf
     */
    function getMetavisaOrders(address account, uint _index, uint _offset) public view returns (OrderSet.Order[] memory orders) {
        uint totalSize = _metavisaOrderOf[account].length();
    ...
    function _pledgeMVCLimit(uint256 tokenId,address account) private returns(bool) {
        ...
        checkUpdateIncomes(_allLimitIncomes,_metavisaLimitOrderOf[address(0)].length(),_fixedDailyRewardLimit);
    ...
    function cancelPledgeMVCLimit(uint256 tokenId) external virtual returns(bool) {
        ...
        OrderSet.Order memory order = _metavisaLimitOrderOf[msg.sender].at(tokenId);
        checkUpdateIncomes(_allLimitIncomes,_metavisaLimitOrderOf[address(0)].length(),_fixedDailyReward);
        ...
        IERC721(_nftAddress[1]).safeTransferFrom(address(this),msg.sender,tokenId); 

        emit CancelPledge(address(_nftAddress[1]),msg.sender,tokenId,block.timestamp);  
        return true;
    }
    ...
    function getIncomeLimit(address account) public view virtual returns(uint256) {
        ...
        if(_metavisaLimitOrderOf[msg.sender].length() == 0){
        ...
        for(uint256 i = 0;i<orders.length;i++){
            ...
            uint256 theoryNum = (block.timestamp - _startTime) / _period;
            uint256 incomes = _fixedDailyRewardLimit / _metavisaLimitOrderOf[address(0)].length();
            sumIncome +=  incomes*(theoryNum -_allLimitIncomes.length);
        }
        return _limitIncome[account]+sumIncome;
    }

    function getMetavisaLimitOrders(address account, uint _index, uint _offset) public view returns (OrderSet.Order[] memory orders) {
        uint totalSize = _metavisaLimitOrderOf[account].length();
    ...
    function setFixedDailyReward(uint256 fixedDailyReward_) external virtual onlyOwner onlyTimeEnd {
        _fixedDailyReward = fixedDailyReward_;
    }

    function setFixedDailyRewardLimit(uint256 fixedDailyRewardLimit_) external virtual onlyOwner onlyTimeEnd {
        _fixedDailyRewardLimit = fixedDailyRewardLimit_;
    }

    function setPeriod(uint256 period_) external virtual onlyOwner onlyTimeEnd {
        _period = period_;
    }

    function setOntinuousDays(uint256 continuousDays_) external virtual onlyOwner onlyTimeEnd {
        _continuousDays = continuousDays_ * 1 days;
    }

    function setStartTime_(uint256 startTime_) external virtual onlyOwner onlyTimeEnd {
        _startTime = startTime_;
    }
}