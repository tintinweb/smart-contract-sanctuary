...
contract Staking3Months is Ownable {
    ...
    function setRewardPerc(uint256 _rewardPerc) external {
        rewardPerc = _rewardPerc;
        emit RewardPercSet(_rewardPerc);

    }

    function setDepositTime(uint256 _depositTime) external {
        depositTime = _depositTime;
        emit DepositTimeSet(_depositTime);

    }

    function setStakingCap(uint256 _stakingCap) external {
