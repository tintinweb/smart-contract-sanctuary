...
contract Vault is IERC20, ReentrancyGuard, Ownable {
	...
	function withdraw(uint256 amount, address recipient) external nonReentrant notLocked returns (uint256) {
		...
		token.transfer(recipient, (tokensToTransfer * 10**token.decimals()) / (10**decimals));
	...
	function tokenAllocationOf(address account) public view returns (uint256) {
		return (_balances[account] * totalTokens) / totalSupply;
	}
	...
	function swapOut(address account, uint256 amount) external notLocked {
		...
		totalTokens += ((fee - adminFee) * (10**decimals)) / 10**token.decimals();
