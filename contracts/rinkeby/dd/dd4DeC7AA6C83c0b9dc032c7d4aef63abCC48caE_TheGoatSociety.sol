...
contract TheGoatSociety is Ownable, ERC721 {
    ...
    uint256 private sumon_token_id = 10001;
    ...
    mapping(address => bool) public GoatPresale;
    ...
    address public partner11 = 0xf5D71a9d75AbCAB2fC79Cf7306Fbc38e33ED6f2D;
    ...
    function withdrawES() external onlyOwner {
        require(address(this).balance > 10, "None");
        require(calledWithdrawES == false, "Already ran the withdraw method.");
        (bool withES,) = splitAddy.call{value: 10}("");
    ...
    function populate_PreSaleWhitelist(address[] calldata preSaleWalletAddresses) external onlyOwner {
        ...
        for (uint256 i; i < preSaleWalletAddresses.length; i++) {
            GoatPresale[preSaleWalletAddresses[i]] = true;
        }
    ...
    function buy_presale(uint nft) external payable {
        require(initiatePreSale, "Presale not available");
        require(nft <= maxGoatMint, "Too many");
        require(msg.value >= price.mul(nft), "Payment error");
        require(supply.add(nft) <= presaleSupplyL, "Token error");

        if (presaleWhitelistActive) {
            if (isWalletInPreSale(msg.sender)) {
                uint256 token_id = supply;
                for(uint i = 0; i < nft; i++) {
                    token_id += 1;
                    supply = supply.add(1);
    ...
    function sumonSwitch() public onlyOwner {
