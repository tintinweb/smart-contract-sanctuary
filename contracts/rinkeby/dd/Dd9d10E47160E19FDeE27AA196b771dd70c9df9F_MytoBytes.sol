...
contract MytoBytes is  ERC721Enumerable, ERC2981ContractWideRoyalties{
    ...
    uint constant numGenOneCreatures = 100;
    uint constant setAsideForCreatorsAndMarketing = 5;
    string constant myBaseURI = 'ipfs://QmRPG7qLvPd8swj115iTnPyTfE32Ke6XSJchajgtdRkZUj/';
    ...
    uint constant mintStartTime = 1634497200;
    uint constant openMintStartTime = 1634583600;


    uint256[maxNumberCreatures] public energyFoodLevel;
    uint256[maxNumberCreatures] public energyBlockLevel;
    uint16[numGenOneCreatures - setAsideForCreatorsAndMarketing] public genTokensAvailable;
    uint16[maxNumberCreatures - numGenOneCreatures] public splitTokensAvailable;
    uint256 public nextClaimableToken;
    ...
    function feed(uint16 tokenID) public payable {
        ...
        uint blockDelta = block.number - energyBlockLevel[tokenID];
        ...
        if (energyFoodLevel[tokenID] + blockDeltaValue >= energyRequired && totalSupply() < maxNumberCreatures) {
            ...
            uint randomIndex = random(maxNumberCreatures - totalSupply());
            uint endIndex = maxNumberCreatures - totalSupply() - 1;
            uint newTokenID = splitTokensAvailable[randomIndex];
            if (newTokenID == 0) {
            ...
            if (splitTokensAvailable[endIndex] == 0) {
                ...
                splitTokensAvailable[randomIndex] =  splitTokensAvailable[endIndex];
            }
    ...
    event Deposit2(uint16[95] value);
    ...
    function claim() private {
        ...
        uint randomIndex = random(numGenOneCreatures - totalSupply());
        uint endIndex = numGenOneCreatures - totalSupply() - 1;
        uint newTokenID = genTokensAvailable[randomIndex];
        if (newTokenID == 0) {
            newTokenID = randomIndex;
        }
    ...
    function creatorClaim(uint tokenID) public {
    ...
    function claimOnWhitelist(bytes32[] calldata merkleProof) public {
        require(block.timestamp >= mintStartTime, "claiming hasn't started" );
        require(
    ...
    function random(uint maxRand) private view returns (uint) {
        return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender))) % maxRand;
    }
