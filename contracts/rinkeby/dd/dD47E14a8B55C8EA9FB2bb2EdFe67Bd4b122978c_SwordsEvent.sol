...
contract SwordsEvent is ERC721, ISwordsEvent, AccessControl, ReentrancyGuard {
    // * CONSTANTS * //
    bytes32 public constant PRODUCER = keccak256("PRODUCER");
    uint256 public constant MINT_FEE = 0.001 ether;
    uint256 public constant TOTAL_PRIZE = MINT_FEE * 30;
    ...
    address private _eventTeam;
    address private _artistWhite;
    address private _artistBlack;
    ...
    constructor(
        uint256 eventId_,
        address eventTeam_,
        address artistWhite_,
        address artistBlack_,
        address marketContract_
    ) ERC721("32 Swords", "SWORDS") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(PRODUCER, _msgSender());
        eventId = eventId_;
        _eventTeam = eventTeam_;
        _artistWhite = artistWhite_;
        _artistBlack = artistBlack_;
        _marketContract = marketContract_;
    }

    function mint(uint256 tokenId) public payable onlyNonKings(tokenId) {
        require(tokenId >= 1 && tokenId <= 32, "tokenId out of range");
        require(!_exists(tokenId), "token already minted");
        require(msg.value == MINT_FEE, "not right amount of ether");
    ...
    function startEvent() public onlyRole(PRODUCER) {
    ...
    function destroyCapturedPiece(uint256 tokenId, uint256 capturedBy)
        public
        onlyRole(PRODUCER)
    ...
    function claimPrize() public {
        ...
        require(tokenOwned[msg.sender] != 0, "no surviving tokens held");
        require(
            prizeClaimed[tokenOwned[msg.sender]] == false,
            "already claimed for this token"
        );
        payable(msg.sender).transfer(TOTAL_PRIZE / tokensActive);
        prizeClaimed[tokenOwned[msg.sender]] = true;
    }
    ...
    function _transfer(
        ...
        ISwordsMarket(_marketContract).removeAsk(eventId, tokenId);
        tokenOwned[from] = 0;
    ...
    // * PUBLIC ADDRESSES * //
    function eventTeam() public view returns (address) {
        return _eventTeam;
    }

    function artistWhite() public view returns (address) {
        return _artistWhite;
    }

    function artistBlack() public view returns (address) {
        return _artistBlack;
    }

    // * MARKET CONTRACT FUNCTIONS * //
    function exchangeTransfer(uint256 tokenId, address recipient) external {
        require(_msgSender() == _marketContract, "only market contract");
    ...
    {
        ISwordsMarket(_marketContract).setAsk(eventId, tokenId, askAmount);
    }
        ...
        onlyExistingToken(tokenId)
    {
        ...
        ISwordsMarket(_marketContract).setBid{value: msg.value}(
            eventId,
            tokenId,
            bidAmount,
            _msgSender()
    ...
    function removeBid(uint256 tokenId)
        ...
        onlyExistingToken(tokenId)
    {
        ISwordsMarket(_marketContract).removeBid(
            eventId,
            tokenId,
            _msgSender()
        );
    ...
    function acceptBid(
        ...
        ISwordsMarket(_marketContract).acceptBid(
            eventId,
            tokenId,
            bidAmount,
            bidder
