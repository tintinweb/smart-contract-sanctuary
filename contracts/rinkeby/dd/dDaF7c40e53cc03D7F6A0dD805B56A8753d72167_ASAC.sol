...
contract ASAC is Ownable, ERC721 {
    ...
    uint256 public paidMintedQty = 0;
    ...
    function callerMaxQtyToMint() external view virtual returns (uint256) {
        // free
        if (mintedQty < FREE_MINT_MAX_QTY) {
            return maxFreeQtyPerWallet - minterToTokenQty[msg.sender];
        }
        // paid
        else {
            return MAX_QTY_PER_WALLET - minterToTokenQty[msg.sender];
        }
    }

    function mintPriceInWei(uint256 _mintQty)
        external
        view
        virtual
        returns (uint256)
    ...
    function setMaxQtyPerTx(uint256 _maxQtyPerTx) external onlyOwner {
