...
contract PlanetNFT is ERC721Tradable {
    ...
    uint256 public constant TOKEN_LIMIT = 50000;
    uint256 public constant MINT_PRICE = 50000000000000000; // cheap mint price at begining -0.05eth
    address public constant _dead = 0x000000000000000000000000000000000000dEaD;

    bool public _mintingFinalized;
    bool public frozen;

    uint16 public constant referalPercent = 10;
    uint16 public constant referalDiscount = 95;
    ...
    event PlanetMintRefer(uint256 amount, address referee, address referar);
    ...
    function mint(uint16 mintAmount, address referalAddr) public payable{
        handlePayment(mintAmount, msg.value, referalAddr);
    ...
    function createNewPlanet() internal{
        ...
        returns (string memory)
    {
        ...
        uint16 addressLen=uint16(bytes(fullAddress).length);

        /*
        string memory sliceAddress=string(abi.encodePacked(
            Utils.substring(fullAddress, 0, 10),
            "...",
            Utils.substring(fullAddress, 2, addressLen-1)
        ));
        */

        return _metadataGenerator.tokenMetadata(tokenId, _values[tokenId], Utils.substring(fullAddress, 2, addressLen-1));
    }


    function baseTokenURI() public pure virtual override returns (string memory) {
        return "https://planetnft";
    }
    ...
    function _splitFunds(uint256 amount) internal {
        if (amount > 0) {
            ...
            uint256 partA = amount.mul(90).div(100);
    ...
    function discountPrice(address referalAddr) public view returns (uint256){
        uint256 _currentPrice= currentPrice();
        if(referalAddr==address(payable(referalAddr)) && referalAddr != _msgSender() && referalAddr != tx.origin){
            _currentPrice=_currentPrice.mul(referalDiscount).div(100);
        }
        return _currentPrice;
    }

    function handlePayment(uint16 mintAmount, uint256 payment, address referalAddr) internal{
        // check for suficient payment
 
        if (_msgSender() != owner()) {
            uint256 amount=discountPrice(referalAddr).mul(uint256(mintAmount));

            require(payment >= amount, "insuficient payment");

            emit PlanetMintRefer(mintAmount, _msgSender(), referalAddr);

            uint256 referal=0;

            // lookup the referer by the referal token id owner
            if(referalAddr==address(payable(referalAddr))){
                address payable referer = payable(referalAddr);
            ...
            _splitFunds(amount.sub(referal));
