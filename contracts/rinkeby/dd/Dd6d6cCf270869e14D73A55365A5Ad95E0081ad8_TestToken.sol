...
contract TestToken is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    address private openSeaProxyRegistryAddress = 0x4879c2a20C05D45b655da4b840A5ebe8b0ed0ef3;
    ...
    uint256 public constant PRESALE_SALE_PRICE = 0.0 ether;
    uint256 public maxPreSaleTokens = 150;
    bytes32 public preSaleMerkleRoot;
    bool public isPreSaleActive;

    uint256 public maxGiftedTokens = 150;
    uint256 public numGiftedTokens;
    bytes32 public claimListMerkleRoot;

    ITGold public tgold;
    uint public tgoldPerDay = 1 ether;
...
//        isCorrectPayment(PRE_SALE_PRICE, numberOfTokens)
        isValidMerkleProof(merkleProof, preSaleMerkleRoot)
    {
        ...
        require(
            numAlreadyMinted + numberOfTokens <= MAX_TOKENS_PER_WALLET,
            "Max tokens to mint in presale is three"
        );
    ...
    function transferFrom(address from_, address to_, uint tokenId_) public override {
        updateRewardOnTransfer(from_, to_);
    ...
    function changeReward(uint _reward) external onlyOwner {
        tgoldPerDay = _reward;
    }
    ...
    //     // return (erc721Balance_ * (tgoldPerDay * (time_ - lastUpdate_))) / secondsInDay;
    //     return erc721Balance_.mul(tgoldPerDay.mul((time_.sub(lastUpdate_)))).div(secondsInDay);
    // }
    function __calculateYieldReward(address address_) internal view returns (uint) {
        uint _erc721Balance = balanceOf(address_);
        ...
        if (_lastUpdate > yieldStartTime) {
            return _erc721Balance.mul(tgoldPerDay.mul((_time.sub(_lastUpdate)))).div(secondsInDay);
        } else {
            return 0;
        }
    }
    ...
    function __claimYieldReward(address address_) internal {
        ...
        if (_pendingRewards > 0) {
            ...
            ITGold(tgold).mint(address_, _pendingRewards);
    ...
    function updateRewardOnTransfer(address from_, address to_) internal{
        // Note: this is called only on a transfer of TOKEN
