...
import "./libs/PaymentSplitter.sol";
import "./libs/ReentrancyGuard.sol";
import "./libs/Strings.sol";


contract DuckArt is ERC721, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    uint public maxMintPublic = 3;
    uint public maxMintFirst = 8;
    uint public maxMintSecond = 6;
    uint public firstSaleDuration = 860 seconds;
    uint public secondSaleDuration = 86 seconds;
    WorkflowStatus workflow;

    address[] private _team = [0x10cD8079755bC23b7E644F60627478e62E8817D5,
                                0x486992EC99a2e81875E7feC7FAa170dEDF2497Dc];
    ...
    constructor() ERC721("DuckArt", "DART") PaymentSplitter(_team, _teamShares) {
    ...
    function setTeam(address[] memory _a) external onlyOwner {
        _team = _a;
    }

    function setTeamShares(uint[] memory _s) external onlyOwner {
        _teamShares = _s;
    }
    ...
    //     require(payable(m1).send(percent * 99));
    //     require(payable(m2).send(percent * 1));
    // }
    ...
    function giveaway(address to, uint qty) external onlyOwner nonReentrant {
    ...
    function buy(uint qty) external payable nonReentrant {
        ...
        require(qty + nonce <= totalSupply, "SUPPLY: Value exceeds totalSupply");
        WorkflowStatus status = getCurrentStatus();

        if (status == WorkflowStatus.FirstSale) {
            _mintFirstSale(qty);
        } else if (status == WorkflowStatus.SecondSale) {
            _mintSecondSale(qty);
        } else if (status == WorkflowStatus.Sale) {
            require(msg.value == price * qty, "PAYMENT: invalid value");
            require(mintPublicSale[msg.sender] + qty <= maxMintPublic, "PUBLIC SALE: Max 3 mint");
    ...
    function _mintFirstSale(uint qty) private {
        ...
        require(mintWhitelist1[msg.sender] + qty <= maxMintFirst, "FIRST SALE: Max 8 mint");
    ...
    function _mintSecondSale(uint qty) private {
        ...
        require(mintWhitelist2[msg.sender] + qty <= maxMintSecond, "SECOND SALE: Max 6 mint");
    ...
    function getCurrentStatus() internal returns(WorkflowStatus) {
        if (!saleActive) {
            return WorkflowStatus.Before;
        }
        WorkflowStatus _current;
        if (block.timestamp <= saleStartTime + firstSaleDuration) {
            if (workflow != WorkflowStatus.FirstSale) {
                workflow = WorkflowStatus.FirstSale;
                emit WorkflowStatusChange(WorkflowStatus.Before, workflow);
            }
            _current = WorkflowStatus.FirstSale;
        } else if (
            (block.timestamp >= saleStartTime + firstSaleDuration) &&
            (block.timestamp <= saleStartTime + firstSaleDuration + secondSaleDuration)
        ) {
            if (workflow != WorkflowStatus.SecondSale) {
                workflow = WorkflowStatus.SecondSale;
                emit WorkflowStatusChange(WorkflowStatus.FirstSale, workflow);
            }
            _current = WorkflowStatus.SecondSale;
        } else {
            if (workflow != WorkflowStatus.Sale) {
                workflow = WorkflowStatus.Sale;
                emit WorkflowStatusChange(WorkflowStatus.SecondSale, workflow);
            }
            _current = WorkflowStatus.Sale;
        }
        ...
        if (revealed == false) {
            // return unrevealedURI;
              return bytes(unrevealedURI).length > 0
                ? string(abi.encodePacked(unrevealedURI, tokenId.toString(), ".json"))
                : "";
        }
