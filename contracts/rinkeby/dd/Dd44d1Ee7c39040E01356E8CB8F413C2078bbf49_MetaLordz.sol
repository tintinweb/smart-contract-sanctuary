...
contract MetaLordz is ERC721, Ownable {
    ...
    uint256 public constant metalordzPrice = 20000000000000000;
    uint public constant maxMetalordzPurchase = 3;
    uint256 public constant MAX_METALORDZ = 5000;
    uint public metalordzReserve = 100;
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] memory ) {
     ...
     function mintMetalordz(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= maxMetalordzPurchase, "Can only mint 10 tokens at a time");
        require(msg.value == metalordzPrice.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 3,"Max 20 mints per wallet allowed");
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply() < MAX_METALORDZ) {
               ...
               saleIsActive = !saleIsActive;
                msg.sender.transfer(numberOfTokens.sub(i).mul(metalordzPrice));
                break;
            }
        }
    }
   ...
   // to set the merkle proof
    ...
    function whitelistedMints(uint numberOfTokens, bytes32[] calldata merkleProof ) payable external  {
        ...
        require(msg.value == metalordzPrice.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 5,"Max 20 mints allowed per whitelisted wallet");
