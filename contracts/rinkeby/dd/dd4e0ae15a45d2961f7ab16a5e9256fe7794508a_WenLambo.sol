...
contract WenLambo is Context, IERC20, Ownable {
    ...
    string private _name = "WenLambo";
    string private _symbol = "WenLambo";
    ...
    function setDistributionFeePercent(uint256 distributionFee)
        external
        onlyOwner
    {
    ...
    function _distributionToAllHolder(
        ...
        )
    {
        (
            ...
            uint256 tDistribution,
            uint256 tMarketingAndBurn
        ) = _getTValues(tAmount);
        (
            uint256 rAmount,
            uint256 rTransferAmount,
            uint256 rDistribution
        ) = _getRValues(tAmount, tDistribution, tMarketingAndBurn, _getRate());
        return (
            rAmount,
            rTransferAmount,
            rDistribution,
        ...
        )
    {
        ...
        uint256 tTransferAmount = tAmount.sub(tDistribution).sub(
            tMarketingAndBurn
        );
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rDistribution).sub(
            rMarketingAndBurn
        );
    ...
    function _takeMarketingAndBurnToken(
        uint256 tMarketingAndBurn,
        address sender
    ) private {
        if (_marketingFee + _burnFee > 0) {
            uint256 tMarketing = tMarketingAndBurn.mul(_marketingFee).div(
    ...
    function calculateDistributionFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
    ...
    function calculateBurnAndMarketingFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
    ...
    function removeAllFee() private {
        if (_distributionFee == 0 && _burnFee == 0 && _marketingFee == 0)
            return;
