...
contract blahtest5 is ERC721A, Ownable {
    ...
    constructor() ERC721A("blahtest5", "blahtest5", MAX_MINT_PER_TX) {}
    ...
    modifier isPaymentSufficient(uint256 quantity) {
        if (totalSupply() + quantity > 20 && totalSupply() < 20) {
            require(
                PRICE * (totalSupply() + quantity - 20) <= msg.value,
                "There was not enough/extra ETH transferred to mint an NFT."
            );
        } else if (totalSupply() + quantity > 20 && totalSupply() > 20) {
            require(
