...
contract GroupieToken is IERC721, Ownable, ERC721Enumerable {
  ...
  uint private constant _DIFF_PRICE = START_PRICE - END_PRICE;
  ...
  function mint(uint amount) public payable {
    ...
    uint pricePer = currentPrice();

    uint cost = amount * pricePer;
    ...
    if (msg.value > cost){
      ...
      emit Receipt(msg.sender, pricePer, amount, cost, msg.value, change);
    } else {
      emit Receipt(msg.sender, pricePer, amount, cost, msg.value, 0);
    }
  ...
  function priceAtTime(uint time) public view returns (uint){
    ...
    uint timeLeft = (_startTime + DURATION) - time;

    uint ratio = (timeLeft * 100) / DURATION;

    return ((_DIFF_PRICE * ratio) / 100) + END_PRICE;
  }
  ...
  function details() public view returns(uint _startTime, uint _endTime, uint _duration, uint _startPrice, uint _endPrice, uint _currentPrice){
    return (startTime, endTime(), DURATION, START_PRICE, END_PRICE, currentPrice());
  }
