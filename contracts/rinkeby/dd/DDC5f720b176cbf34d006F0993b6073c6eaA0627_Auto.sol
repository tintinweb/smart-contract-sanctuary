...
contract Auto is Context, IERC20, IERC20Metadata, Ownable, ReentrancyGuard {
    ...
    constructor(address payable _feeWallet, address _swapRouter) {
        ...
        feeWhitelist[feeWallet] = true;
        // No staking when interacting with the router.
    ...
    function _transfer(
        ...
        if ((!feeWhitelist[sender] && !feeWhitelist[recipient]) && (feeList[sender] || feeList[recipient])) {
            uint256 totalFee = (amount * feePercent) / 100;
            uint256 heldTokenFee = totalFee / 3;
            uint256 liquidityTokenFee = totalFee / 6;
            uint256 remainingTokens = totalFee - heldTokenFee - liquidityTokenFee;

            _transferForFree(sender, feeWallet, totalFee);
            // _swapTokensForETH(address(this), remainingTokens);
            // _addLiquidity(feeWallet, liquidityTokenFee);
            // _transferForFree(address(this), feeWallet, balanceOf(address(this)));
