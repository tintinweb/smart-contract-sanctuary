...
contract Portal is IPortal, ReentrancyGuard {
    ...
    event Harvested(address indexed recipient, address indexed portal);
    event Withdrawn(
    ...
    event UnStaked(address indexed unstaker, address indexed portal);
    ...
    constructor(
        ...
        require(
            _stakeLimit != 0,
            "Portal: Stake limit needs to be more than 0"
        );
        require(
            _contractStakeLimit != 0,
            "Portal: Contract Stake limit needs to be more than 0"
        );
        ...
        nonReentrant
    {
        ...
        require(
            user.balance + amount <= userStakeLimit,
            "Portal: user stake limit exceeded"
        );
        require(
            totalStaked + amount <= contractStakeLimit,
            "Portal: contract stake limit exceeded"
        );
    ...
    function withdraw(uint256 amount) public nonReentrant {
        ...
        require(
            amount <= user.balance,
            "Portal: withdraw amount exceeds available"
        );
        totalStaked = totalStaked - amount;
        user.balance = user.balance - amount;
    ...
    function harvest(uint256[] memory tokenIndices, address recipient)
        public
        nonReentrant
    {
        User storage user = users[recipient];
    ...
    function addReward(uint256[] memory rewards, uint256 newEndBlock)
        external
        nonReentrant
    {
        ...
        uint256 rewardTokensLength = rewardsToken.length;
        require(
            rewards.length == rewardsToken.length,
            "Portal: rewards length mismatch"
        );
        ...
        for (uint256 i = 0; i < rewardTokensLength; i++) {
            ...
            require(
                minimumRewardRate[i] <= rewardRate[i],
                "Portal: invalid reward rate"
            );
            uint256 newRewardRatio = remainingReward == 0
                ? tokenMultiplier
                : (rewards[i] * tokenMultiplier) / remainingReward;
            providerRatios[i] = providerRatios[i] + newRewardRatio;
            totalRewardRatios[i] = totalRewardRatios[i] + providerRatios[i];
            rewardsToken[i].safeTransferFrom(
                msg.sender,
                address(this),
                rewards[i]
            );
    ...
    function removeReward() external nonReentrant {
        ...
        for (uint256 i = 0; i < rewardTokensLength; i++) {
            ...
            uint256 providerPortion = (remainingReward * providerRatios[i]) /
                totalRewardRatios[i];
            totalRewardRatios[i] = totalRewardRatios[i] - providerRatios[i];
            providerRatios[i] = 0;
            totalRewards[i] = totalRewards[i] - providerPortion;
            rewardRate[i] =
                (remainingReward - providerPortion) /
                rewardsDuration;
    ...
    function migrate(uint256 _amount, address _portal) external nonReentrant {
        ...
        require(
            ...
            "Portal: migrate amount exceeds balance"
        );
        stakingToken.approve(_portal, _amount);
        IPortal(_portal).stake(_amount, msg.sender);
    }

    function rewardPerTokenStaked(uint256 tokenIndex)
        public
        view
        returns (uint256)
    {
        ...
        return
            totalStaked > distributionLimit
                ? rewardPerTokenSnapshot[tokenIndex] +
                    (((lastBlockRewardIsApplicable() - lastBlockUpdate) *
                        rewardRate[tokenIndex] *
                        tokenMultiplier) / totalStaked)
                : rewardPerTokenSnapshot[tokenIndex];
    }
        ...
        returns (uint256)
    {
        ...
        return
            ...
            ((user.balance *
                (rewardPerTokenStaked(tokenIndex) -
                    user.userRewardPerTokenPaid[tokenIndex])) /
                tokenMultiplier);
    }

    function getTokenMultiplier(uint256 tokenIndex)
        public
        view
        returns (uint256)
    {
        uint256 tokenDecimals = IERC20Metadata(rewardsToken[tokenIndex])
            .decimals();
    ...
    function totalEarned(uint256 tokenIndex) public view returns (uint256) {
        ...
        return
            ...
            ((totalStaked *
                (rewardPerTokenStaked(tokenIndex) -
                    rewardPerTokenSnapshot[tokenIndex])) / tokenMultiplier);
    }
    ...
    function harvestForDuration(uint256 tokenIndex)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < rewardTokensLength; i++) {
            ...
            if (totalStaked > distributionLimit) {
                _rewardPerTokenSnapshot =
                    _rewardPerTokenSnapshot +
                    (((_lastBlockRewardIsApplicable - lastBlockUpdate) *
                        rewardRate[i] *
                        _tokenMultiplier) / totalStaked);
            }

            distributedReward[i] =
                distributedReward[i] +
                ((totalStaked *
                    (_rewardPerTokenSnapshot - rewardPerTokenSnapshot[i])) /
                    _tokenMultiplier);
            ...
            user.rewards[i] =
                ...
                ((user.balance *
                    (_rewardPerTokenSnapshot -
                        user.userRewardPerTokenPaid[i])) / _tokenMultiplier);
    ...
    function getProviderRewardsRatio(address provider)
        public
        view
