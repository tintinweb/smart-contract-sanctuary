...
contract PlatziPunksDNA {
    ...
    // This pseudo random function is deterministic and should not used in production
    // To implement a true random function, see https://docs.chain.link/docs/chainlink-vrf/
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    // We overcharge the functionalities of the type uint256 with the String library's functionalities 
    using Strings for uint256;
    ...
    constructor(uint256 _maxSupply) payable ERC721("PlatziPunks", "PP") {
        maxSupply = _maxSupply;
    }

    // This method is called when someone wants to mint a new NFT.
    function mint() public {
        uint256 current = _idCounter.current();
        require(
            current < maxSupply,
            "PlatziPunks: mint: total supply exceeded"
        );
        ...
        returns (string memory)
    {
        require(
            _exists(_tokenId),
            "PlatziPunks: tokenURI: URI query for nonexistent token"
        );
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                ...
                _tokenId.toString(),
                '",',
                '"description": "Platzi Punks are randomized avatars to learn how NFTs are made",',
                '"image": "',
                image,
                '"',
                "}"
