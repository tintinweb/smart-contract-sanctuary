...
*/
    ...
    TheBirdHouse private TheBirdHouseContract;
    ...
    {
        TheBirdHouseContract = TheBirdHouse(theBirdHouseAddress);
    }
    ...
    function claimCompanions(uint256 companionCount) public {
        uint256 supply = totalSupply();

        require(companionCount > 0, "Must claim as least one companion");

        uint256 birdBalance = TheBirdHouseContract.balanceOf(msg.sender);
        require(birdBalance > 0, "Must hold at least one bird!");

        require(
            birdBalance >= companionCount,
            "Can't claim more companions than birds!"
        );

        require(
            supply + companionCount <= maxSupply,
            "Maximum companions already minted!"
        );
