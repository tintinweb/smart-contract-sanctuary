...
contract BaboonBet is ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    uint256 public _openSaleTime; 
    ERC721 NFT = ERC721(0x7216bD0430952426bf63E72a5773E8e44E798A60);

    constructor(address manager, uint saleTime) ERC721("BaboonBet", "BBET") {
        _manager = payable(manager);
        _openSaleTime = block.timestamp.add(saleTime);
    }
    ...
    function buy(uint256 baboonsQty) external payable {
        ...
        require(
            ...
            "Sale exceeds available Monkeys"
        );
        uint256 salePrice = baboonPrice.mul(baboonsQty);
        require(msg.value >= salePrice, "Insufficient Amount"); 

        if (block.timestamp < _openSaleTime) {
            require(NFT.balanceOf(msg.sender) > 0, "Initial Sale for Monkey Owners");
        }
