 ...
 * @author Keyko
 *
 * @dev Contract in charge of the yielding strategy
 *
 */

contract YieldSource is
    Initializable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    using SafeERC20Upgradeable for StableERC20;
    ...
    /**
     * @notice Emmited when the balance of the Yield Source has increased
     * @param amount_usdc the new balance of USDC tokens of the Yield Source contract
     */
     event BalanceIncreased(uint256 amount_usdc);

    /**
     * @dev Address of the ERC20 used for yielding
     */
    StableERC20 private usdcToken;
    ...
    function initialize(address _usdcTokenAddress, address _backendAddress)
        ...
        initializer
    {
        usdcToken = StableERC20(_usdcTokenAddress);
    ...
    /**
     * @notice used to increase the balance of the Yield Source
     * @param _percentage the percentage with which the balance should be increased
     * @dev in order to be able to represent decimals we divide the balance of the contract 
     *      by 1000 this means that 4 as a parameter is actually 0.04
     * @dev for the algorithm to work, the balance of the contract should be greater than 10000
     *      if it's not, 1 usdc token is minted instead of the '_percentage'
     */
    function increaseBalance(uint256 _percentage)
        external
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        uint256 amount = usdcToken.balanceOf(address(this)).mul(_percentage);
        amount = amount.div(10000);
        if(amount == 0) amount = 1; 
        usdcToken.mint(address(this), amount);
        emit BalanceIncreased(amount);
    }

    /**
     * @dev used to withdraw USDC tokens from the yield source
     * @param _account adress that will receive the USDC tokens
     * @param _amount amount of USDC tokens that will be withdrawed
     */
    function withdraw(address _account, uint256 _amount)
        external
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        usdcToken.safeTransfer(_account, _amount);
    ...
    ERC20CappedUpgradeable
{
    ...
    */
    ...
    /** @dev Removes `amount` of tokens and assigns them to `account`, decreseaing
     * the total supply.
     *
    ...
    function burn(address account, uint256 amount) external returns (bool) {
        require(
            AccessControlUpgradeable.hasRole("minter", msg.sender),
            "Address not granted for burning tokens"
        );
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            _address != address(0),
            "Rand Stable Token: Vault contract address cannot be address 0!"
        );
        AccessControlUpgradeable.grantRole("minter", _address);
    }
