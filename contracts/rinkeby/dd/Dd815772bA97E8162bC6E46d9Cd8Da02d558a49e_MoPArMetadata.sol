...
interface IMoPAr {
    ...
    function getCollection(uint256 collectionId) external view returns (bool exists, string memory name, string memory artist, bytes32[] memory signatures, uint256[] memory tokenIds, bool paused, uint128 max, uint128 circulating);
}
