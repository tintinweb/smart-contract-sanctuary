...
contract testPP is ERC721Enumerable, ERC721Burnable, Ownable, AccessControl {
...
contract testPPdrop is Ownable, Pausable, ReentrancyGuard {
    
    testPP private token;
    ...
    function mint(uint256 quantity) external payable whenNotPaused() nonReentrant() {
        require(msg.value >= price * quantity, "PPdrop: Insufficient value");
        require(quantity <= MAX_MINT_PER_ORDER, "PPdrop: Exceeds order limit");
        require(token.canMint(quantity), "PPdrop: Exceeds total");
    ...
    function mintGiveaways(address[] calldata addresses) external onlyOwner() {
        require(token.canMint(addresses.length), "PPdrop: Exceeds total");
