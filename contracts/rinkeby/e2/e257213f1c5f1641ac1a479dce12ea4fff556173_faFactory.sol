...
contract faFactory is Ownable, ERC721, ERC721Enumerable {
  ...
  function _createFA(uint32 _dna, uint16 _rarity) private validDna(_dna) {
    uint8[5] memory _stats = [10, 10, 10, 10, 10];  
    faArray.push(FA(_dna, uint32(block.timestamp), 0, 0, _rarity, 5, _stats, "lime", "floating", ""));  
  ...
  function _generateRandomRarity() internal view returns (uint16) {
    uint randRarity = uint(keccak256(abi.encodePacked(msg.sender, block.timestamp, totalSupply()))).mod(100);
    if (randRarity >= 95) {
