...
contract MasterChef is Ownable {
    ...
    uint256 public constant BURN_BONUS_DAYS = 35 minutes;
    // uint256 public constant TIME_STEP = 1 days;
    uint256 public constant TIME_STEP = 5 minutes;
    IUniswapV2Router02 public SWAP_ROUTER;
    uint256 public TOR_BURN_TOTAL;
    uint256[] public REFERRAL_PERCENTS = [25, 15, 5];
    ...
    event UpdatePool(uint256 indexed pid, uint256 timeAt);
    ...
    function add(
        ...
        uint256 _lastRewardAt =
            block.timestamp > STARTED_AT ? block.timestamp : STARTED_AT;
        TOTAL_ALLOC_POINT = TOTAL_ALLOC_POINT.add(_allocPoint);

        POOL_INFOR.push(
            PoolInfo({
                lpToken: IMint(_lpToken),
                allocPoint: _allocPoint,
                lastRewardAt: _lastRewardAt,
                accTorPerShare: 0,
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.timestamp <= _pool.lastRewardAt) {
            return;
        }
        ...
        // uint256 _lpSupply =
        //     _pool.lpToken.balanceOf(address(this)).sub(_pool.freezeTokens);

        if (_lpSupply == 0 || _pool.allocPoint == 0) {
            _pool.lastRewardAt = block.timestamp;
            return;
        }

        uint256 _multiplier =
            getMultiplier(_pool.lastRewardAt, block.timestamp);
        uint256 _torReward =
        ...
        _pool.accTorPerShare = _pool.accTorPerShare.add(
            _torReward.mul(1e18).div(_lpSupply)
        );
    ...
    function getMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {
        return _to.sub(_from).div(TIME_STEP).mul(BONUS_MULTIPLIER);
    }
    ...
    function preparePath(address _token0, address _token1)
        internal
        pure
        ...
        returns (uint256, uint256)
    {
        ...
        uint256[] memory _amounts =
            SWAP_ROUTER.swapExactTokensForTokens(
                _swapAmount,
                0,
                preparePath(address(_pool.lpToken), address(TOR)),
                address(this),
                block.timestamp
            );

        return (_amount.sub(_swapAmount), _amounts[1]);
    }
    ...
    function deposit(
        ...
        if (_pool.depositFeeBP > 0) {
            uint256 _depositFee =
                _denomination.mul(_pool.depositFeeBP).div(10000);
        ...
        emit Deposit(_msgSender(), _amount, _bonusForTors);
    }
    ...
    function bindNote(
        ...
        (uint256 _tors, uint256 _burnForTors, uint256 _bonusForTors) =
    ...
    function withdraw(
        ...
        _pool.freezeTokens = _pool.freezeTokens.add(_denomination);
        ...
        )
    {
        ...
        if (address(_pool.lpToken) == address(TOR)) {
            ...
            _lpSupply = _lpSupply.sub(_g.totalDeposit);
        } */

        if (block.timestamp > _pool.lastRewardAt && _lpSupply > 0) {
            uint256 _multiplier =
                getMultiplier(_pool.lastRewardAt, block.timestamp);
            uint256 _torReward =
                _multiplier.mul(TOR_PER_DAY).mul(_pool.allocPoint).div(
        ...
        uint256 _tors =
            _depositInfo.denomination.mul(_pool.accTorPerShare).div(1e18).sub(
                _depositInfo.rewardDebt
            );

        if (_depositInfo.at.add(BURN_BONUS_DAYS) <= block.timestamp) {
            return (
                _tors,
        ...
        return (_tors, _depositInfo.bonusForTors, 0);
    }
