...
import "../soldeps/openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
...
contract SyntFabric is OwnableUpgradeable, ISyntFabric {
     ...
     */
    function getAddressSalted(
        address _creator,
        bytes32 _bytecodeHash,
        uint256 _salt
    ) public view returns (address) {
        bytes32 hash = keccak256(
            abi.encodePacked(bytes1(0xff), _creator, _salt, _bytecodeHash)
        );
     ...
     * @return address of contract
     */
    function getAddressComplex(address _origin, uint256 _nonce)
        public
        pure
        returns (address)
    {
    ...
    /**
     * @notice function for creation representation by owner
     * @param _rtoken address of real token
     * @param _chainID Chain id of the network
     * @param _stokenName Name of synthetic token
     * @param _stokenSymbol Sumbol for synthetic token
     * @param _oldNonce Number of transactions of sender
     */
    function createRepresentationByTokenOwner(
        address _rtoken,
        uint256 _chainID,
        ...
        require(
            getAddressComplex(msg.sender, _oldNonce) == _rtoken,
            "Symb: wrong data params"
        );
        setRepresentation(_rtoken, _chainID, _stokenName, _stokenSymbol);
    }

    /**
     * @notice function for creation representation by owner with CREATE2 opcode
     * @param _rtoken address of real token
     * @param _chainID Chain id of the network
     * @param _stokenName Name of synthetic token
     * @param _stokenSymbol Sumbol for synthetic token
     * @param _oldSalt Old salt
     * @param _bytecodeHash Contract bytecode hash
     */
    function createRepresentationByTokenOwnerSalted(
        address _rtoken,
        uint256 _chainID,
        string memory _stokenName,
        string memory _stokenSymbol,
        ...
        require(
            getAddressSalted(msg.sender, _bytecodeHash, _oldSalt) == _rtoken,
            "Symb: wrong data params"
        );
        setRepresentation(_rtoken, _chainID, _stokenName, _stokenSymbol);
    }
     ...
     * @dev Internal function used in createRepresentationByAdmin, createRepresentationByTokenOwner,
     * createRepresentationByTokenOwnerSalted
     */
    function setRepresentation(
        ...
        require(
            stoken == address(0x0),
            "Symb: token representation already exist"
        );
...
import "../soldeps/openzeppelin/contracts/access/Ownable.sol";
import "../soldeps/openzeppelin/contracts/token/ERC20/extensions/draft-ERC20Permit.sol";
