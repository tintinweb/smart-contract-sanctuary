...
contract Waifu is ERC721, Ownable {
    ...
    constructor() ERC721("Waifu", "WAIFU") {}
    ...
    */
    function mint() public payable {
        ...
        require(!hasPurchased[msg.sender], "Caller has already purchased a token.");
    ...
    */
    function ownerMint(address _receiver) public onlyOwner {
        ...
        require(!hasPurchased[_receiver], "Receiver has already purchased a token.");
        require(_receiver != address(0), "Receiver cannot be zero address.");
        require(tokenIndex <= MAX_SUPPLY, "Minted token would exceed total supply.");

        if (msg.sender != _receiver) {
            hasPurchased[_receiver] = true;
        }
    ...
    */
    function authenticateUser(uint _tokenId) public view returns (bool) {
        require(_exists(_tokenId), "Token does not exist.");
        require(ownerOf(_tokenId) == msg.sender, "User does not own the specified token.");
    ...
    * only callable by the owner.
    * 
    * @param tokenURI_ A string value to replace the current '_tokenURI'.
    */
