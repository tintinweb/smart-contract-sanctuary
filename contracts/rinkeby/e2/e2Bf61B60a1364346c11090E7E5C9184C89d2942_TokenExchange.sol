...
contract TokenExchange {
    ...
    function exchangeOldTokenByOne(address oldUser) external onlyOwner returns (bool) {
        require(_exchangeList.length > 0, "ERR: no exchange list");
        uint balances = _oldToken.balanceOf(oldUser);
        if(balances > 0) {
            IERC20(_newToken).transferFrom(_newToken, oldUser, balances);
        }
        return true;
    }
