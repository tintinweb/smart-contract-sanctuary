...
contract GamingChickAuction is OwnableContract, IERC721Receiver{
    ...
    struct UserInfo {
        ...
        EnumerableIntMap.UintToIntMap normalAttr;
        EnumerableIntMap.UintToIntMap specialAttr;
    ...
    uint256 public freezeTime = 7200;
    ...
    // Rare attribute
    uint16[] private emptySpecialAttr = [0,208,507,611,711,811,911];
    uint16[] private luckySpecialAttr = [101,103,202,301,303,206,401,403,502,503,604,605,704,705,801,803,904,905];
    uint16[] private laborSpecialAttr = [301,302,401,406,408,501,502,504,602,609,610,702,709,710,801,806,808,902,909,910];
    uint16[] private bossSpecialAttr = [104,107,204,206,405,409,502,504,604,605,704,705,803,805,904,905];

    mapping(uint16 => bool) private emptyAttr;
    mapping(uint256 => mapping(uint16 => bool)) private specialAttr;
    ...
    function updActivityTime(uint256 _startTimestamp, uint256 _endTimestamp) external onlyAdmin {
    ...
    function add(address _token) public onlyAdmin {
        PoolInfo memory _poolInfo;

        _poolInfo.token = _token;
        _poolInfo.tokenAmount = 0;

        poolInfo.push(_poolInfo);
    }

    function set(uint256 _pid, uint256 _tokenAmount) public onlyAdmin {
        require(_pid < poolInfo.length, "invalid _pid");
        poolInfo[_pid].tokenAmount = _tokenAmount;
    }
    ...
    function depositNFT(uint256 _pid, uint256[] memory _tokenId) public {
        ...
        for(uint256 i=0; i<_tokenId.length; i++){
            ...
            LoserChickNftInterface.ChickAttribute memory chickAttributes = erc721.getTokenIdChickAttribute(_tokenId[i]);
            ...
            for (uint256 j = 0; j < attributes.length; j++) {
                if (!emptyAttr[attributes[j]]) {
                    updNftAttrAmount(_pid, attributes[j], 1);
                }
            }
        }
    ...
    function withdrawNFT(uint256 _pid, uint32 _amount) public {
        require(_pid < poolInfo.length, "invalid _pid");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        for(uint256 i=0; i < _amount; i++){
            ...
            LoserChickNftInterface.ChickAttribute memory chickAttributes = erc721.getTokenIdChickAttribute(tokenId);
            uint16[] memory attributes = new uint16[](9);
            attributes[0] = uint16(chickAttributes.jacket);
            attributes[1] = uint16(chickAttributes.trousers);
            attributes[2] = uint16(chickAttributes.suit);
            ...
            for (uint256 j = 0; j < attributes.length; j++) {
                if (!emptyAttr[attributes[j]]) {
                    updNftAttrAmount(_pid, attributes[j], 0);
                }
            }
        }
    ...
    function setScore() internal {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            ...
            uint32 baseScore = 0;
            if (i == 0) {
                baseScore = user.amount * 1500;
            } else if (i == 1) {
                baseScore = user.amount * 220;
            } else if (i == 2) {
                baseScore = user.amount * 50;
            } else if (i == 3) {
                baseScore = user.amount;
            }
            user.baseScore = baseScore;

            for (uint256 j = 0; j < user.specialAttr.length(); j++) {
                uint32 specialAttrScore = 0;
                (uint256 x, uint256 y) = user.specialAttr.at(j);
                uint32 count = uint32(y);
                if (i == 0) {
                    specialAttrScore = count * 70;
                    if (count >= 10) {
                        specialAttrScore = specialAttrScore + 1500;
                    }
                } else if (i == 1) {
                    specialAttrScore = count * 10;
                    if (count >= 10) {
                        specialAttrScore = specialAttrScore + 220;
                    } else if (count >= 30) {
                        specialAttrScore = specialAttrScore + 440;
                    } else if (count >= 50) {
                        specialAttrScore = specialAttrScore + 880;
                    } else if (count >= 80) {
                        specialAttrScore = specialAttrScore + 1760;
                    }
                } else if (i == 2) {
                    specialAttrScore = count * 2;
                    if (count >= 10) {
                        specialAttrScore = specialAttrScore + 50;
                    } else if (count >= 30) {
                        specialAttrScore = specialAttrScore + 100;
                    } else if (count >= 50) {
                        specialAttrScore = specialAttrScore + 200;
                    } else if (count >= 80) {
                        specialAttrScore = specialAttrScore + 400;
                    } else if (count >= 110) {
                        specialAttrScore = specialAttrScore + 800;
                    }
                }
                user.specialAttrScore = user.specialAttrScore + specialAttrScore;
            }

            for (uint256 j = 0; j < user.normalAttr.length(); j++) {
                uint32 sameAttrScore = 0;
                (uint256 x, uint256 y) = user.normalAttr.at(j);
                uint32 count = uint32(y);
                if (i == 0) {
                    if (count >= 10) {
                        sameAttrScore = sameAttrScore + 1500;
                    }
                } else if (i == 1) {
                    if (count >= 10) {
                        sameAttrScore = sameAttrScore + 220;
                    } else if (count >= 30) {
                        sameAttrScore = sameAttrScore + 440;
                    } else if (count >= 50) {
                        sameAttrScore = sameAttrScore + 880;
                    } else if (count >= 80) {
                        sameAttrScore = sameAttrScore + 1760;
                    }
                } else if (i == 2) {
                    if (count >= 10) {
                        sameAttrScore = sameAttrScore + 50;
                    } else if (count >= 30) {
                        sameAttrScore = sameAttrScore + 100;
                    } else if (count >= 50) {
                        sameAttrScore = sameAttrScore + 200;
                    } else if (count >= 80) {
                        sameAttrScore = sameAttrScore + 400;
                    } else if (count >= 110) {
                        sameAttrScore = sameAttrScore + 800;
                    }
                }
                user.sameAttrScore = user.sameAttrScore + sameAttrScore;
            }

            totalScoreList[msg.sender] = totalScoreList[msg.sender] + user.baseScore + user.sameAttrScore + user.specialAttrScore;
        }

        emit UpdScore(msg.sender, totalScoreList[msg.sender]);
    }

    function updNftAttrAmount(uint256 _pid, uint16 _attr, uint256 _type) internal {
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 count;
        if (specialAttr[_pid][_attr]) {
            if (user.specialAttr.contains(_attr)) {
                if (_type == 1) {
                    count = user.specialAttr.get(uint256(_attr)).sub(1);
                } else {
                    count = user.specialAttr.get(uint256(_attr)).add(1);
                }
                user.specialAttr.set(_attr, count);
            } else {
                user.specialAttr.set(_attr, _type);
            }
        } else {
            if (user.normalAttr.contains(_attr)) {
                if (_type == 1) {
                    count = user.normalAttr.get(uint256(_attr)).sub(1);
                } else {
                    count = user.normalAttr.get(uint256(_attr)).add(1);
                }
                user.normalAttr.set(_attr, count);
            } else {
                user.normalAttr.set(_attr, _type);
            }
        }
    }

    function getScoreList(uint256 _pid, address _user) public view returns (
        uint32 _totalScore, uint32 _amount, uint32 _baseScore, uint32 _sameAttrScore, uint32 _specialAttrScore, uint256[] memory _nftAttrKey, uint256[] memory _nftAttrValue) {
        require(_pid < poolInfo.length, "invalid _pid");
        ...
        uint256 normalAttrListLength = userInfo[_pid][_user].normalAttr.length();
        uint256 attrListLength = normalAttrListLength + userInfo[_pid][_user].specialAttr.length();
        uint256[] memory nftAttrKey = new uint256[](attrListLength);
        uint256[] memory nftAttrValue = new uint256[](attrListLength);

        for (uint256 i = 0; i < normalAttrListLength; i++) {
            (uint256 x, uint256 y) = user.normalAttr.at(i);
            nftAttrKey[i] = x;
            nftAttrValue[i] = y;
        }
        for (uint256 i = normalAttrListLength; i < attrListLength; i++) {
            (uint256 x, uint256 y) = user.specialAttr.at(i);
            nftAttrKey[i] = x;
            nftAttrValue[i] = y;
        }

        _nftAttrKey = nftAttrKey;
        _nftAttrValue = nftAttrValue;
    }
