...
contract GenTickets is ERC1155, ERC1155Receiver {
    ...
    function redeemTicket(address _to, uint _id, uint _amount) public { //address, 0, 1

        uint tier = _id.mod(numTicketTypes); //0 mod 4 = 0
        ...
        // Tranche past cliff
        uint tranche = _id.div(numTicketTypes); //0/4=0 | 8/4=2 | 12/4=3 | 16/4=4 | 20/4=5
        ...
        if (ticketType.trancheLength >= 1){
            eligibleTraches = (getBlockTimestamp().sub((ticketType.trancheLength).mul(ticketType.cliffTranches).add(TGE))).div(ticketType.trancheLength);
        ...
        uint availableToClaim = (ticketType.ticketSize).div((ticketType.totalTranches).sub(ticketType.cliffTranches)); // 10/(5-1)*1 = 2
        uint averageTokensAvailableToClaim = availableToClaim.mul(_amount); //check here? mul overflow
        
        uint actualTranche; // (tranche - (ticketType.cliffTranches)); //0-1=-1 | 2-1=1 | 3-1 = 2 | 4-1=3 | 5-1=4
        if (tranche <= ticketType.cliffTranches){
            actualTranche = 0;
        } else{
            actualTranche = tranche.sub(ticketType.cliffTranches); // if kept 1 or anything then no issues
        }
        
        uint redeemedTokens;
        if (actualTranche >= 1){
            redeemedTokens  = averageTokensAvailableToClaim.mul(actualTranche);//actualTranche //problem is here only --final
        } else{
            redeemedTokens =0;
        }
        ...
        // uint mulCliffedLevel = (eligibleTraches + ticketType.cliffTranches).sub(tranche);
        ...
        bytes memory data;
        _mint(_to, _id.add(numTicketTypes.mul((eligibleTraches.add(ticketType.cliffTranches)).sub(tranche))), _amount, data); //check later 0 + 4*2 = 8 | 8+4*1 = 12 | 12+4*1=16 | 16+4*1=20
    }
