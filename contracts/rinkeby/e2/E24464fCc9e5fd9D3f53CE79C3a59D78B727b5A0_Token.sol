...
contract Token is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    modifier whenPresaleActive() {
        require(presaleActive, "TT: Presale is not active");
        _;
    }

    modifier whenPresalePaused() {
        require(!presaleActive, "TT: Presale is not paused");
        _;
    }
    ...
    function merkleRoot() external view onlyOwner returns (bytes32) {
        return _merkleRoot;
    }

    function startPresale(
        uint256 presaleDuration_, uint256 presalePrice_, uint8 presaleLimit_, bytes32 merkleRoot_
    ...
    function remainingPresaleTime() external view whenPresaleActive returns (uint256) {
        require(presaleStartTime > 0, "TT: Presale hasn't started yet");
    ...
    function mint(uint256 tokensAmount) internal returns (uint256[] memory) {
    ...
    function presaleMint(
        uint256 tokensAmount, bytes32[] memory merkleProof
    ) external payable whenPresaleActive nonReentrant returns (uint256[] memory) {
        _preValidatePurchase(tokensAmount);

        require(_remainingPresaleTime() > 0, "TT: Presale is over");
        ...
        return mint(tokensAmount);
    }

    function publicMint(uint256 tokensAmount) external payable whenSaleActive nonReentrant returns (uint256[] memory) {
        _preValidatePurchase(tokensAmount);
