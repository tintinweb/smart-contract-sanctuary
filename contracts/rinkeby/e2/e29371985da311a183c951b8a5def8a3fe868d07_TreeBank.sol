...
contract TreeBank is
    ...
    
 {
    ...
    mapping(uint256 => uint256) public _charity;
    ...
    constructor(
        ...
        address charityAddrss,
        string memory tokenName,
        address tokenAddress
    ) ERC721("TreeBank", "TRK") {
        _tid = id;
        serviceValue = _serviceValue;
        _charityAddress = payable(charityAddrss);
    ...
    function mint(
        ...
        token_id[tokenId] = Metadata(name, ipfsimage,ipfsmetadata);
         ...
         _charity[tokenId] = charity.mul(1e18);
        ...
        )
    {
        ...
        uint256 char = pERCent(amount, _char);
        uint256 netamount = amount.sub(fee.add(roy.add(char)));
        fee = fee.add(fee);
        return (fee, roy, char, netamount);
    }
    ...
    function newtokenasbid(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal {
        ...
        require(balances[tokenId][from] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 char, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue, _charity[tokenId]);
        require( approveValue >= _adminfee.add(roy.add(netamount.add(char))), "Insufficient Balance");       
        t.transferFrom(msg.sender,admin,_adminfee);
        t.transferFrom(msg.sender,_creator[tokenId],roy);
        if(_charity[tokenId] != 0){
            t.transferFrom(msg.sender, _charityAddress, char);
        }
    ...
    function _acceptBId(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        if(_charity[tokenId] != 0){
            t.transferFrom(from, _charityAddress, char);
        }
    ...
    function _saleToken(
        ...
        (uint256 _adminfee, uint256 roy, uint256 char, uint256 netamount) = calc(
            amount,
            _royal[tokenId],
            serviceValue,
            _charity[tokenId]
        );
        require( msg.value == _adminfee.add(roy.add(netamount.add(char))), "Insufficient Balance");
        admin.transfer(_adminfee);
        create.transfer(roy);
        if(_charity[tokenId] != 0){
           _charityAddress.transfer(char);
        }
     ...
     function acceptBId(string memory bidtoken,address bidaddr, uint256 amount, uint256 tokenId) public{
        ...
        safeTransferFrom(msg.sender, bidaddr, tokenId);
    }
    ...
    function burnToken(uint256 id,address from) public{
        require( balances[id][from] == 1, "Your Not a Token Owner or insufficient Token Balance");
            burn(id);
