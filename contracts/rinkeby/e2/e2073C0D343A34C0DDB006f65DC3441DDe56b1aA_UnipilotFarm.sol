...
contract UnipilotFarm is IUnipilotFarm, IERC721Receiver {
    ...
    function depositNFT(address unipoolAddress, uint256 tokenId)
        external
        onlyOwner(tokenId)
        returns (bool)
    {
        ...
        emit Deposit(
            ...
            poolInfo[unipoolAddress].globalReward
        );
...
interface IUnipilotFarm {
    //funct update reward per block (pool,val)
    //func update multiplier

    // pool add => user add -> array of token id
    ...
    event Deposit(address, uint256, uint256, uint256, uint256);
    event WithdrawReward(address, uint256, uint256, uint256, uint256, uint256);
