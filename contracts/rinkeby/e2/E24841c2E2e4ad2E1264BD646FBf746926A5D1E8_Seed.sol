...
contract Seed is ERC20Burnable, Ownable {
    ...
    address public strawberriesAddress;

    //Mapping of strawberry to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    //Mapping of strawberry to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to strawberries
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("Seed", "SEED") {}

    function setStrawberriesAddress(address _strawberriesAddress) public onlyOwner {
        strawberriesAddress = _strawberriesAddress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(
            ...
            MAX_WALLET_STAKED,
            "Must have less than 31 strawberries staked!"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(strawberriesAddress).ownerOf(tokenIds[i]) == msg.sender &&
                tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(strawberriesAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
