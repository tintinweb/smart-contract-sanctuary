...
contract PoHTwitterV1 is PoHTwitterAPI {
    event Verification(
    ...
    constructor(
        address _poHAddress, // address _chanlinkAddress, // string memory _chainlinkJobIdconstructor(
    ...
    ) PoHTwitterAPI(_linkAddress, _oracleAddress, _fee, _feePower, _apiURL) {
        _setPoHAddress(_poHAddress);
    }
    ...
    function _isRegistered(address _address) internal view returns (bool) {
        return IProofOfHumanity(poHAddress).isRegistered(_address);
    }

    function verify(string memory _tweetId) external {
        require(_isRegistered(_msgSender()));
        requestTweetData(_tweetId);
    }

    /**
     * Receive the response in the form of uint256
     */
    function fulfillTweetRequest(bytes32 _requestId, uint256 _userId)
        public
        override
        recordChainlinkFulfillment(_requestId)
    {
        require(_userId != 0, "API Error");
        address _caller = requestAddress[_requestId];
        verified[_caller] = _userId;
        emit Verification(_caller, _userId, _requestId);
    }
}
...
// Abstract
abstract contract PoHTwitterAPI is ChainlinkClient, Ownable {
    ...
    mapping(bytes32 => address) internal requestAddress;
    ...
    constructor(
        ...
        // jobId = _jobIdentifier;
        jobId = "c637797f3e9a468489e88d5441a16a3e";
        // oracleFee = _fee;
        oracleFee = _fee * (10**_feePower);
        apiBaseURL = _apiURL;
    }
    ...
    function requestTweetData(string memory _tweetId)
        public
        returns (bytes32 requestId)
    {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId,
            address(this),
            this.fulfillTweetRequest.selector
        );
        // url/tweetId/address
        string memory _url = string(
            abi.encodePacked(apiBaseURL, _tweetId, "/", _msgSender())
        );
        request.add("get", _url);
        request.add("path", "user_id");

        requestId = sendChainlinkRequestTo(oracle, request, oracleFee);

        emit NewRequest(requestId, _msgSender(), _tweetId);
    }
