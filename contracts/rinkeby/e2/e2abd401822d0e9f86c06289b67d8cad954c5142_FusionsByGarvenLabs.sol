   ...
   event VStatus(bool status, address _address);
    ...
    function mint(uint mintAmount) public payable whenNotPaused() nonReentrant() {
        ...
        if(presale == true){
            ...
            require(mintAmount <= preSaleMaxMintAmount[msg.sender],"You are not eligible to mint this number of NFT");  // Not needed
        ...
        if(presale == false){
            ...
            require(msg.value >= amount,"inSufficient Funds");
            require(mintAmount <= maxPublicAmount,"cannot mint more than the amount"); // Not needed
    ...
    function verification(bytes32[] memory proof, uint maxMintAmount) whenNotPaused() nonReentrant() public returns(bool) {        
        ...
        // console.log(verified); // not Needed
        // console.log(MerkleProof.verify(proof, root, leaf)); // not Needed
