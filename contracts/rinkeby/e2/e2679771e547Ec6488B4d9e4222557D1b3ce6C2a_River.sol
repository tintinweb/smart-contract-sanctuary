...
//   struct SheepWolflol {bool isSheep; uint8[] traitarray; uint8 alphaIndex;}
//   function getTokenTraits(uint256 tokenId) external view returns (SheepWolflol memory);
...
contract River is Ownable, IERC721Receiver, Pausable, VRFConsumerBase {
  using Address for address;
  using Counters for Counters.Counter;
  

  uint8 public constant MAX_ALPHA = 8;                                    // maximum alpha score for a Wolf                              
  struct Stake {
    uint16 tokenId;
    uint80 value;
    address owner;
  ...
  // - get total staked tokens for user
  // - Amount of $salmon claimed from user's staked bears
  // - Amount of $salmon claimed from user's staked fishermen
  ...
  BIGYO woolf;                                                          // reference to the Woolf NFT contract
  ITWool wool;                                                           // reference to the $WOOL contract for minting $WOOL earnings


  mapping(uint256 => Stake) public barn;                                // maps tokenId to stake
  mapping(uint256 => Stake[]) public pack;                              // maps alpha to all Wolf stakes with that alpha

  mapping(uint256 => uint256) public packIndices;                       // tracks location of each Wolf in Pack
  uint256 public totalAlphaStaked = 0;                                  // total alpha scores staked
  uint256 public unaccountedRewards = 0;                                // any rewards distributed when no wolves are staked
  uint256 public woolPerAlpha = 0;                                      // amount of $WOOL due for each alpha point staked


  uint256 public  DAILY_WOOL_RATE = 10000 ether;                        // sheep earn 10000 $WOOL per day
  uint256 public  MINIMUM_TO_EXIT = 2 days;                             // sheep must have 2 days worth of $WOOL to unstake or else it's too cold
  uint256 public  constant WOOL_CLAIM_TAX_PERCENTAGE = 20;              // wolves take a 20% tax on all $WOOL claimed
  uint256 public  constant MAXIMUM_GLOBAL_WOOL = 2400000000 ether;      // there will only ever be (roughly) 2.4 billion $WOOL earned through staking




  uint256 public totalWoolEarned;                                       // amount of $WOOL earned so far
  uint256 public totalSheepStaked;                                      // number of Sheep staked in the Bar
  uint256 public lastClaimTimestamp;                                    // the last time $WOOL was claimed

  bool public rescueEnabled = false;                                    // emergency rescue to allow unstaking without any checks but without $WOOL
  ...
  constructor(address _woolf, address _wool, address _vrfCoordinator, address _link) 
      VRFConsumerBase(_vrfCoordinator, _link)
  { 
    woolf = BIGYO(_woolf);                                                // reference to the Woolf NFT contract
    wool = ITWool(_wool);                                                //reference to the $WOOL token
  ...
  function setInit(address _woolf, address _wool) external onlyOwner{
  ...
  function addManyToBarnAndPack(address account, uint16[] calldata tokenIds) external {    // called in mint

    require(account == _msgSender() || _msgSender() == address(woolf), "DONT GIVE YOUR TOKENS AWAY");    /// SEE IF I CAN ADD THE MF CONTRACT BAN
  ...
  function _addWolfToPack(address account, uint256 tokenId) internal {
    ...
    pack[alpha].push(Stake({                                                  // Add the wolf to the Pack
      owner: account,
      tokenId: uint16(tokenId),
  ...
  function calculateReward(uint16[] calldata tokenIds) public view returns (uint256 owed) {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      if (isSheep(tokenIds[i]))
        owed += calcRewardSheep(tokenIds[i]);
      else
        owed +=  calcRewardWolf(tokenIds[i]);
    }
  
  }


  function calcRewardSheep(uint256 tokenId) public view returns (uint256 owed) {
  ...
  function calcRewardWolf(uint256 tokenId) public view returns (uint256 owed) {
  ...
  // Basically, withdraws $WOOL earnings for a single Sheep and optionally unstake it.
  // 20% Wolf Tax, 50% chance all goes to wolf if unstaking. 
  function _claimSheepFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      delete barn[tokenId];
      totalSheepStaked -= 1;
      woolf.safeTransferFrom(address(this), _msgSender(), tokenId, "");         // send back Sheep        //RE-ENTRY *********


    } else {

      _payWolfTax(owed * WOOL_CLAIM_TAX_PERCENTAGE / 100);                    // percentage tax to staked wolves    //RE-ENTRY ********* 
      barn[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
  ...
  // Basically, withdraws $WOOL earnings for a single wolf and optionally unstake it.
  function _claimWolfFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    owed = calcRewardWolf(tokenId);                                         // Calculate portion of tokens based on Alpha

    if (unstake) {
      totalAlphaStaked -= alpha;                                            // Remove Alpha from total staked
      Stake memory lastStake = pack[alpha][pack[alpha].length - 1];         // Shuffle last Wolf to current position PT 1 
      pack[alpha][packIndices[tokenId]] = lastStake;                        // Shuffle last Wolf to current position PT 2
      packIndices[lastStake.tokenId] = packIndices[tokenId];                // Shuffle last Wolf to current position PT 3
      pack[alpha].pop();                                                    // Remove duplicate

      delete packIndices[tokenId];                                          // Delete old mapping
      woolf.safeTransferFrom(address(this), _msgSender(), tokenId, "");     // Send back Wolf        //RE-ENTRY*******
  ...
  function _payWolfTax(uint256 amount) internal {
    ...
    if (totalAlphaStaked == 0) {                                              // if there's no staked wolves > keep track of $WOOL due to wolves
      unaccountedRewards += amount; 
      return;
    }


    woolPerAlpha += (amount + unaccountedRewards) / totalAlphaStaked;         // makes sure to include any unaccounted $WOOL 
  ...
  // allows owner to enable "rescue mode" && simplifies accounting, prioritizes tokens out in emergency
  ...
  function isSheep(uint256 tokenId) public view returns (bool sheep) {
    // SheepWolf memory t = woolf.getTokenTraits(tokenId);(sheep, , , , , , , , , ) = woolf.tokenTraits(tokenId);
  ...
  function _alphaForWolf(uint256 tokenId) public view returns (uint8) {
  ...
  function randomWolfOwner(uint256 seed) external view returns (address) {
    ...
    for (uint i = MAX_ALPHA - 3; i <= MAX_ALPHA; i++) {                     // loop through each bucket of Wolves with the same alpha score
      cumulative += pack[i].length * i;
      if (bucket >= cumulative) continue;                                   // if the value is not inside of that bucket, keep going

      return pack[i][seed % pack[i].length].owner;                          // get the address of a random Wolf with that alpha score
    }
...
contract BIGYO is IWoolf, ERC721Enumerable, Ownable, Pausable, VRFConsumerBase {
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    if (minted < PAID_TOKENS) {
      ...
      uint256 mintCostEther = MINT_PRICE * MINT_PRICE;
