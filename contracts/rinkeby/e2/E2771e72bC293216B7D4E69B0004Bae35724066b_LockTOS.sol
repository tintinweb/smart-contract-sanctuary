...
contract LockTOS is LockTOSStorage, AccessibleCommon, ILockTOS {
        ...
        )
    {
        ...
        // console.log('alivelocksOf %s, %s ', _addr, len );
        uint256 _size = 0;
        for(uint i =0; i < len; i++){
            uint256 _id = userLocks[_addr][i];
            //console.log('_id %s, %s ', i, _id );
        ...
        for(uint i =0; i < len; i++){
            ...
            if(_lock.end > block.timestamp && _id > 0 ) {
                ...
                uint256[] memory _info = new uint256[](6);
                _info[0] = _id;
                _info[1] = _lock.start;
                _info[2] = _lock.end;
                _info[3] = _lock.amount;
                _info[4] = uint256(_lock.boostValue);
                _info[5] = balanceOfLock(userLocks[_addr][i]);

                // console.log('_lock.start %s  ', _lock.start);
                // console.log('_lock.end %s  ', _lock.end);
                // console.log('_lock.amount %s  ', _lock.amount);
                // console.log('_lock.boostValue %s  ', _info[4]);
                // console.log('_lock.balanceOfLock %s  ', _info[5]);

                datas[_size++] = LibLockTOS.LockedBalanceInfo(
                    _id,_lock.start, _lock.end,  _lock.amount, _info[4], _info[5]
                );
