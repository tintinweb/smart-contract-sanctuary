...
contract ViiraGenesisMintable is ERC721, IERC2981, Ownable {
    ...
    constructor() ERC721("Viira Genesis", "VIIRAGEN") {}

    /// Only allow certain minters
    modifier onlyAllowedMinter() {
        require(allowedMinters[msg.sender] || msg.sender == owner(), ALLOWED_ERROR);
        _;
    }
    ...
    function mint(uint256[] calldata tokenIds) external onlyAllowedMinter {
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice)
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        return (owner(), (salePrice * 1000) / 10000);
    }

    // Admin functions

    /// Set base media uri
    function setBaseURI(string memory uriPrefix, string memory uriPostfix)
        external
        onlyOwner
    {
        advancedConfig.uriBase = uriPrefix;
        advancedConfig.uriExtension = uriPostfix;
    }
