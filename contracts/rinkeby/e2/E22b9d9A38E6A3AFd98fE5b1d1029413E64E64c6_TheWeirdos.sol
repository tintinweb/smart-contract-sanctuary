 ...
 * Inspired By: @nftchance & @masonnft
 * The contracts from Nuclear Nerds are a work of art and they go to considerable
 * lengths to ensure that the contracts are secure and that the contracts are
 * optimised to reduce the gas burden on the end user.
 */
...
contract TheWeirdos is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_PUBLIC_PER_TX = 2;
    uint256 public constant MINT_PRICE = 0.02 ether; //TODO: CHANGE THIS
    ...
    function togglePublicSale(uint256 _MAX_SUPPLY, bool _PUBLIC_SALE_ACTIVE)
        external
        onlyOwner
    {
        require(CLONING_EVENT == false, "Cloning event is active.");
    ...
    function publicMint(uint256 count) public payable {
        ...
        require(count < MAX_PUBLIC_PER_TX, "Exceeds max per transaction.");
        require(count * MINT_PRICE == msg.value, "Invalid funds provided.");
    ...
    function withdraw() public {
        ...
        require(success, "Failed to send to the Bank.");
    }
