...
contract JoePair is JoeERC20 {
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        uint balance0 = IERC20Joe(_token0).balanceOf(address(this));
        uint balance1 = IERC20Joe(_token1).balanceOf(address(this));
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'Joe: K');
        }
    ...
    function sync() external lock {
        _update(IERC20Joe(token0).balanceOf(address(this)), IERC20Joe(token1).balanceOf(address(this)), reserve0, reserve1);
    }
}
