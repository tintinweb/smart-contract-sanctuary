    ...
    function startRoundAll() external onlyOwner {
        for(uint256 i; i < holdersEFR.length; i++) {
            for(uint256 i2; i2 < tokens.length; i2++) {
                ...
                IERC20(tokens[i2]).transfer(holdersEFR[i], balanceOf(holdersEFR[i]) * 10**18 / totalHolded * IERC20(tokens[i2]).balanceOf(address(this)) / 10**18);

                emit LogAddress("sent", holdersEFR[i]);
            }
        }
    }
