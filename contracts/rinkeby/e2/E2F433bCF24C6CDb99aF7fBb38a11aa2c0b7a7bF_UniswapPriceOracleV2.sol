...
contract UniswapConfig {
    enum PriceSource {
        FIXED_ETH, /// 0: implies the fixedPrice is a constant multiple of the ETH price (not support now)
...
contract UniswapPriceOracleV2 is UniswapConfig {
    ...
    UniswapConfig(gTokens_, underlyings_, symbolHashs_, baseUints_, priceSources_, fixedPrices_, uniswapMarkets_,
        ...
        for (uint i = 0; i < gTokens_.length; i++) {
            ...
            if (config.priceSource == PriceSource.REPORTER_HT || config.priceSource == PriceSource.REPORTER_USDT) {
    ...
    function priceInternal(TokenConfig memory config) internal view returns (uint) {
        if (config.priceSource == PriceSource.REPORTER_HT || config.priceSource == PriceSource.REPORTER_USDT) return prices[config.symbolHash];
    ...
    function postPriceInternal(string memory symbol) internal {
        ...
        uint anchorPrice;
        if(config.priceSource == PriceSource.REPORTER_USDT) {
            anchorPrice = fetchAnchorPriceUSDT(symbol, config, config.baseUnit);
        }
        else{
    ...
    /**
 * @dev Fetches the current token/usdt  price from uniswap, with 6 decimals of precision.
 * @param conversionFactor : token decimals, such as  1e18
 */
    function fetchAnchorPriceUSDT(string memory symbol, TokenConfig memory config, uint conversionFactor) internal virtual returns (uint) {
        (uint nowCumulativePrice, uint oldCumulativePrice, uint oldTimestamp) = pokeWindowValues(config);
