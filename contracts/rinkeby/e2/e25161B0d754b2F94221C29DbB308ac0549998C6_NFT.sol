...
contract NFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => bool) public inSell;
    mapping(uint256 => uint256) public price;

    event PriceHistory(uint256 indexed _tokenId, uint256 _price, address _owner);

    constructor() ERC721("Codiste", "CDST") {}
    ...
    function removeFromSell(uint256 _tokenID) public {
        require(msg.sender == ownerOf(_tokenID), "You are not owner of NFT");
        inSell[_tokenID] = false;
    }

    function buy(uint256 tokenId) public payable {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        require(msg.sender != ownerOf(tokenId), "You are owner");
        require(msg.value == price[tokenId]);
        _transfer(ownerOf(tokenId), msg.sender, tokenId);

        emit PriceHistory(tokenId, msg.value, msg.sender);
    }
}
