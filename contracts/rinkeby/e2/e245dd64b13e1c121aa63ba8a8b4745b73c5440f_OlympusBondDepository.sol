...
contract OlympusBondDepository is Ownable {
    ...
    mapping( address => mapping( uint => Bond ) ) public bondInfo; 
    ...
    function _deposit( 
        ...
        uint payout = bondInterestFor( value );
        ...
        bondInfo[ depositor_ ][ index[ depositor_ ] ] = Bond({
        ...
        index[ depositor_ ] = index[ depositor_ ]++;
        return true;
    }
    ...
    function redeem() external returns ( bool ) {        
        for ( uint i = 0; i < index[ msg.sender ]; i++ ) {
            Bond memory bond = bondInfo[ msg.sender ][ i ];
    ...
    function getDepositorInfo( address address_, uint index_ ) external view returns ( 
        ...
        Bond memory bond = bondInfo[ address_ ][ index_ ];
    ...
    function _calculatePercentVested( address depositor_, uint index_ ) internal view returns ( uint _percentVested ) {
        Bond memory bond = bondInfo[ depositor_ ][ index_ ];
        uint blocksSinceLast = block.number.sub( bond.lastBlock );
    ...
    function pendingPayoutFor( address depositor_, uint index_ ) external view returns ( uint _pendingPayout ) {
        ...
        uint payoutRemaining = bondInfo[ depositor_ ][ index_ ].payoutRemaining;
     ...
     */
    function bondInterestFor( uint value_ ) public view returns ( uint ) {
    ...
    function bondPrice() public view returns ( uint _price ) {        
        _price = controlVariable.mul( _calcDebtRatio() ).div( 1e7 );
