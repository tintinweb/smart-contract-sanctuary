...
contract Stakers is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public constant stakersReserve = 2;
    uint256 public constant stakersPublic = 10;
    uint256 public constant stakersMax = stakersReserve + stakersPublic;
    uint256 public constant stakersPrice = 0.00000085 ether;
    uint256 public constant stakersPurchaseLimit = 10;
    uint256 public constant presalePurchaseLimit = 5;
    ...
    address addr5 = 0xaA49136A0865A7B30c676D5d7Abb18a224cF39Fc;
 ...
 function mintStaker(uint256 numberOfTokens) external payable {
  ...
  //  require(!preSaleLive, 'Only allowing from Allow List');
    ...
    require(publicMint + numberOfTokens <= stakersPublic, 'Purchase would exceed PUBLIC');
    ...
    for (uint256 i = 0; i < numberOfTokens; i++) {
      if (publicMint < stakersPublic) {
        uint256 tokenId = stakersReserve + publicMint + 1;
        publicMint += 1;
  ...
  //  require(preSaleLive, 'Allow List is not active');
    ...
    require(publicMint + numberOfTokens <= stakersMax, 'Purchase would exceed PUBLIC');
    require(presalerListPurchases[msg.sender] + numberOfTokens <= presalePurchaseLimit, 'Purchase exceeds max allowed');
    require(stakersPrice * numberOfTokens <= msg.value, 'ETH amount is not sufficient');

         for (uint256 i = 0; i < numberOfTokens; i++) {
            publicMint += 1;
            presalerListPurchases[msg.sender] += 1;
  ...
  function gift(address[] calldata to) external onlyOwner {
    require(totalSupply() < stakersMax, 'All tokens have been minted');
    require(totalGifts + to.length <= stakersReserve, 'Not enough tokens left to gift');

    for(uint256 i = 0; i < to.length; i++) {
      uint256 tokenId = totalGifts + 1;
