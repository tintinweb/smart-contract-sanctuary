...
contract COSMICWYVERNSOFFSPRINGS is ERC721Enumerable, Ownable {
    using Strings for uint;
    ...
    constructor() ERC721("COSMIC-WYVERNS' OFFSPRINGS", "COSMIC-WYVERNS' OFFSPRINGS") {}

    function setCOSMICWYVERNSOFFSPRINGS_MAX_COUNT(uint _maxCount) external onlyOwner{
        COSMICWYVERNSOFFSPRINGS_MAX_COUNT = _maxCount;
    }
    

    function setCOSMICWYVERNSOFFSPRINGS_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        COSMICWYVERNSOFFSPRINGS_MAINSALE_PRICE = _mainSalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= COSMICWYVERNSOFFSPRINGS_MAX_COUNT, "EXCEED_MAX");
        for (uint i = 0; i < receivers.length; i++) {
            _safeMint(receivers[i], totalSupply() + 1);
        }
    }

    function mintEgg(bytes memory signature) external payable{

        require (mintLive, "Minting is not live");
        require (matchAddresSigner(signature), "Signature Not Matched");
        require (msg.value >= COSMICWYVERNSOFFSPRINGS_MAINSALE_PRICE,"The value passed is less than expected");
        require (COSMICWYVERNSOFFSPRINGS_MAX_COUNT > totalSupply(), "Max Supply Reached");
