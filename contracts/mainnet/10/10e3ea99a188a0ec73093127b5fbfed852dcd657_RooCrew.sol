...
contract RooCrew is ERC721, Ownable {
	...
	uint public constant MAX_PRESALE_ROOS = 600;
	...
	function mintPresaleRoo(uint256 numRoos) public payable {
		require(hasPresaleStarted, "Presale has not started");
        require(totalSupply() < MAX_PRESALE_ROOS, "Presale has already ended");
        require(numRoos > 0 && numRoos <= 1, "You can mint only 1 presale Roo");
        require(totalSupply().add(numRoos) <= MAX_PRESALE_ROOS, "Exceeds MAX_PRESALE_ROOS");
        require(rooPrice.mul(numRoos) <= msg.value, "Not enough Ether sent for this tx");
