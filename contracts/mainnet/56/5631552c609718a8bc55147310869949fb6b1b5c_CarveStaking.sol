...
abstract contract GovernanceToken is GovernanceStorage, ERC20 {
    using SafeMath for uint256;
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CARVE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CARVE::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "CARVE::delegateBySig: signature expired");
    ...
    function _delegate(address delegator, address delegatee) internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CARVEs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CARVE::_writeCheckpoint: block number exceeds 32 bits");
...
contract MockERC20 is ERC20, Ownable {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "MockERC20::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "MockERC20::delegateBySig: invalid nonce"
        );
        require(block.timestamp <= expiry, "MockERC20::delegateBySig: signature expired");
        return _delegate(signatory, delegatee);
    }
        ...
        returns (uint256)
    {
        require(
            ...
            "MockERC20::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        address currentDelegate = _delegates[delegator];
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying MockERC20s (not scaled);
        _delegates[delegator] = delegatee;
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "MockERC20::_writeCheckpoint: block number exceeds 32 bits"
        );
...
contract GovernorAlpha {
    ...
    string public constant name = "Carve Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint) { return carve.totalSupply() / 25; } // 4% of Supply

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint) { return carve.totalSupply() / 100; } // 1% of Supply
    ...
    event ProposalCreated(uint id, address proposer, address[] targets, uint[] values,
                          string[] signatures, bytes[] calldatas, uint startBlock, uint endBlock, string description);
    ...
    constructor(address timelock_, address carve_, address guardian_) {
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures,
                     bytes[] memory calldatas, string memory description) public returns (uint) {
        require(carve.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(),
    ...
    function cancel(uint proposalId) public {
        ...
        require(state_ != ProposalState.Executed, "GovernorAlpha::cancel: cannot cancel executed proposal");

        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || carve.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(),
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint256 votes = carve.getPriorVotes(voter, proposal.startBlock);
