...
contract MintOne is Ownable {
    ...
    uint256 public cost = 0.08383 ether;
    uint256 public maxMintAmount = 5;

    uint256 public maxSupply = 10038; //phase one max 
    ...
    constructor(
        ...
        beeNFT = _beeNFT;
        beesEscrow = _beesEscrow;
        cost = _cost;
        whiteListTime = _listTime;
        publicTime = _publicTime;
    }
    // public
    function mint(uint256 _mintAmount) public payable {
        uint256 supply = beeNFT.totalSupply();
        require(!paused, "Paused" );
        require(_mintAmount > 0, "Can not mint 0" );
        require(_mintAmount <= maxMintAmount, "Max Mint Amount" );
        require(supply + (_mintAmount*2) < maxSupply, "Max Mint Amount" );
        require(msg.value >= cost * _mintAmount);
        require(block.timestamp >= whiteListTime);

        if(block.timestamp <= publicTime){
            //WhiteList only
            uint256 quota = whitelisted[msg.sender];
            require(_mintAmount <= quota, "No whitelist quota");
            for(uint256 i = 0; i < _mintAmount; i++){
                whitelisted[msg.sender]--;
                beeNFT.mint(msg.sender, supply + (i*2)); //Mint To User
                beeNFT.mint(beesEscrow, supply + (i*2 + 1)); //Mint To Bess university
            }
    ...
    function isWhitelisted(address _user) public view returns (uint256 amount){
    ...
    function setTimes(uint256 _whitelist, uint256 _public) public onlyOwner {
        whiteListTime = _whitelist;
        publicTime = _public;
    }
    ...
    function whitelistUsers(address[] calldata users, uint256[] calldata quantity) public onlyOwner {
        require(users.length == quantity.length, "Must provide equal quantities and users" );
        for(uint256 i = 0; i < users.length; i++){
            whitelisted[users[i]] = quantity[i];
        }
    }
