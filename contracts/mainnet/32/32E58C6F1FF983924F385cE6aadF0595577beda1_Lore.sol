...
/**
 * Adds the following layers onto the existing LOOT system.
 * - Certain Realms as Origin City for your Token ID (chosen from 5% of existing minted realm names + citizenship status chosen from 5 rarities).
 * - Origin Story.
 * - Race (Same as existing  Character races, but with rarities).
 * - Profession (Ranked between 5 rarities).
 * - Alignment (Chosen from the 9 alignments).
 * - Age (18 - 109).
 * - Pantheon (Chosen from lore-friendly DnD pantheons).
 * - Gods / Divinity (Ranked between 5 rarities).
 */
...
contract Lore is ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	// The list of realms as of 9/1/2021 - 7AM EST.
	// Realms claimed after this date are not included.
	// Also realms with unparsable names are not included.
	// Filtered using [^\x00-\x7F] to remove all non-ASCII.
	// The realms were then chosen from that list with a 5% chance.
	// The realms listed below are the resulting list of realms.
    string[] private realms = [
		"Zarmaln",
		"Syushstes",
		"Shabe",
		"Waswashauk",
    ...
    function getRealm(uint256 tokenId) public view returns (string memory) {
        return pluckRealm(tokenId, "REALM");
    }
    
    function getAlignment(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "ALIGNMENT", alignments);
    }
    ...
    function getPantheon(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "PANTHEON", pantheons);
    }
	
    function getDeity(uint256 tokenId) public view returns (string memory) {
        return pluckDeity(tokenId, "DEITY");
    }
    ...
    function pluckDeity(uint256 tokenId, string memory keyPrefix) internal view returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
        string memory output = deities[rand % deities.length];
        uint256 greatness = rand % 21;
		if (greatness == 0) {
            output = string(abi.encodePacked("Cursed by ", output));
        } else if (greatness < 14) {
            output = string(abi.encodePacked("Follower of ", output));
        } else if (greatness >= 14 && greatness < 18) {
            output = string(abi.encodePacked("Blessed by ", output));
        } else if (greatness >= 18) {
            if (greatness < 20) {
                output = string(abi.encodePacked("Chosen by ", output));
            } else {
                output = string(abi.encodePacked("Right Hand of ", output));
            }
        }
        return output;
    }
    ...
    function pluckRealm(uint256 tokenId, string memory keyPrefix) internal view returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
        string memory output = realms[rand % realms.length];
        uint256 greatness = rand % 21;
		if (greatness == 0) {
            output = string(abi.encodePacked("Outcast of ", output));
        } else if (greatness > 0 && greatness <= 10) {
            output = string(abi.encodePacked("Refugee of ", output));
        } else if (greatness > 10 && greatness < 14) {
            output = string(abi.encodePacked("Serf of ", output));
        } else if (greatness >= 14 && greatness < 19) {
            output = string(abi.encodePacked("Citizen of ", output));
        } else if (greatness >= 19) {
            if (greatness == 19) {
                output = string(abi.encodePacked("Nobility of ", output));
            } else {
                output = string(abi.encodePacked("Royalty of ", output));
            }
        }
        return output;
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[7] = getRealm(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = getAlignment(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getAge(tokenId);

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = getPantheon(tokenId);

        parts[14] = '</text><text x="10" y="160" class="base">';

        parts[15] = getDeity(tokenId);
	...
	// Because others need a chance too.
    function claim(uint256 tokenId) public nonReentrant {
        require(tokenId >= 8001 && tokenId < 9750, "Token ID invalid, public sales for 8001-9749.");
        _safeMint(_msgSender(), tokenId);
    }
	
	// Because the loot holders come first.
    function claimForLoot(uint256 tokenId) public nonReentrant {
        require(tokenId > 0 && tokenId < 8001, "Token ID invalid, loot owners can claim 1-8000.");
        require(loot.ownerOf(tokenId) == msg.sender, "Not Loot owner");
        _safeMint(_msgSender(), tokenId);
    }
    
	// Because I want to make something too.
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > 9750 && tokenId < 10001, "Token ID invalid, owner can claim last 250.");
    ...
    constructor() ERC721("Lore", "LORE") Ownable() {}
}
