 ...
 */
contract Familiars is ERC721, ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    string[] private species = ["Hippogriff", "Drake", "Direwolf", "Basilisk", "Ape", "Gorilla", "Felinid", "Manticore", "Phoenix"];
    string[] private clothing = ["Naked", "Crown", "Blanket", "Mage Robe", "Boots", "Potato Sack",  "Divine Robe"];
    string[] private color = ["Golden", "Silver", "Red", "Blue", "Shiny", "Gray", "Black", "White", "Magenta"];
    string[] private weapon = ["Spear", "Sword", "Katana", "Bow", "Claw", "Teeth", "Mace", "Shield"];


    //Loot Contract
    address public lootAddress = 0x79E2d470f950f2Cf78eeF41720E8ff2cf4B3CD78;
    ...
    function getSpecies(uint256 tokenId) public view returns (string memory) {
        return pluckSt(species, 9, tokenId, "Species");
    }
    
    function getLevel(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "Level");
    }
    
    function getClothing(uint256 tokenId) public view returns (string memory) {
        return pluckSt(clothing, 7, tokenId, "Clothing");
    }
    
    function getColor(uint256 tokenId) public view returns (string memory) {
        return pluckSt(color, 9, tokenId, "Color");
    }

    function getWeapon(uint256 tokenId) public view returns (string memory) {
        return pluckSt(weapon, 8, tokenId, "Weapon");
    }
    
    function getHealth(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "Health");
    }
    ...
    function getCuteness(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "Cuteness Coefficient");
    }
    ...
    function pluckSt(string[] memory source, uint256 aSize, uint256 tokenId, string memory keyPrefix) internal pure returns (string memory) {
        uint256 roll1 = random(string(abi.encodePacked(keyPrefix, toString(tokenId), "1")));
        uint256 min = roll1;
        uint256 roll2 = random(string(abi.encodePacked(keyPrefix, toString(tokenId), "2")));
        min = min > roll2 ? roll2 : min;
        ...
        // get 3 highest dice rolls
        uint256 seed = roll1 + roll2 + roll3 + roll4 - min;
        uint256 stat = seed % aSize;
        
        string memory output = string(abi.encodePacked(keyPrefix, ": ", source[stat]));
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[13] = getMagic(tokenId);
        
        parts[14] = '</text><text x="10" y="160" class="base">';

        parts[15] = getCuteness(tokenId);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Pet #', toString(tokenId), '", "description": "Welcome, weary traveler, and take your pick of the cutest pet of the land. This shall be your companion through all your travails and hardships! Remember, never fear, for a familiar face is always near.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("Familiars (for Adventurers)", "Fam") {
    }
