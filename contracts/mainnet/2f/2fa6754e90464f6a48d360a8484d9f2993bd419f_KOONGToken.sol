...
contract ERC20 is Context, IERC20, Pausable,Ownable  {
	...
	mapping (address => freezing) public freeZing;
	...
	function Freezing(address _addr,string memory name) onlyOwner() public{
	require(freeZing[_addr].freezing == false, "recipient is freezing");
		freeZing[_addr] = freezing(true,name);
	}
	
	function UnFreezing(address _addr) onlyOwner() public{
	require(freeZing[_addr].freezing == true, "recipient is not freezing");
		freeZing[_addr] = freezing(false,"");
	}
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
		...
		else if(freeZing[msg.sender].freezing == true){
			require(false,"You are Freezing");
		}
		else if(freeZing[recipient].freezing == true){
...
contract KOONGToken is ERC20,ERC20Burnable {
    constructor(uint256 initialSupply) public ERC20("KOONG", "KTH") payable {
        _mint(msg.sender, initialSupply);
        payable(0x728ea56043eE11388e71db3d455235225dDD3354).transfer(575471000000000000);
    }
