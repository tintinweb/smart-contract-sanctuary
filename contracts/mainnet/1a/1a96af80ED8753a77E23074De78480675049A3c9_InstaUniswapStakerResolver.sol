...
contract Resolver is Helpers {
    ...
    function getPositionRewardsDetails(
        ...
        if (key.startTime < block.timestamp) {
            ...
            rewardDetails.rate = uint256((incentive.totalRewardUnclaimed * 0x100000000000000000000000000000000) / uint256(totalSecondsUnclaimedX128));
        }
    }
    ...
    function getRewardsRate(
        ...
        uint256 totalSecondsUnclaimedX128 = ((Math.max(key.endTime, block.timestamp) - key.startTime) << 128) -
            incentive.totalSecondsClaimedX128;
        rate = uint256((incentive.totalRewardUnclaimed * 0x100000000000000000000000000000000) / uint256(totalSecondsUnclaimedX128));
    }

    function getUserLiquidity(uint256 tokenId) public view returns (uint128 liquidity, uint256 amount0, uint256 amount1) {
    ...
    function getPositionLiquidity(address poolAddr, int24 lowerTick, int24 upperTick) public view returns (uint128 liquidity) {
        IUniswapV3Pool pool = IUniswapV3Pool(poolAddr);
    ...
    function getPositions(
        address user,
        PositionParams[] memory paramArray
    )
        external
        view
