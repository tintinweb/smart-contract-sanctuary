...
// File: contracts/Curve/Curve_ZapOut_General_V3.sol
...
contract Curve_ZapOut_General_V3 is ZapOutBaseV1 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    */
    function _fillQuote(
        ...
        if (
            _fromTokenAddress == ETHAddress || _fromTokenAddress == address(0)
        ) {
            valueToSend = _amount;
        } else {
    ...
    function removeLiquidityReturn(
        ...
        if (underlying) {
            ...
            uint256 metaTokensRec = ICurveSwap(swapAddress)
                .calc_withdraw_one_coin(liquidity, 1);
