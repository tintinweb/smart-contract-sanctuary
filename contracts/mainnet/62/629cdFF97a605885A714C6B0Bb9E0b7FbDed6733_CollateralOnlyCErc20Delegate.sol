...
/**
 * @title CollateralOnlyCErc20  Contract
 */
contract CollateralOnlyCErc20 is CToken, CErc20Interface {
    /**
    ...
    function initialize(address underlying_,
        ...
        super.initialize(comptroller_, interestRateModel_, 1e18, name_, symbol_, decimals_);
    ...
    function mint(uint mintAmount) external returns (uint) {
        uint err;
        // automatic enter market
        if (!comptroller.checkMembership(msg.sender, address(this))) {
            err = comptroller.enterMarket(address(this), msg.sender);
            require(err == uint(Error.NO_ERROR), "AUTOMATIC_ENTER_MARKET_FAILED");
        }
        (err,) = mintInternal(mintAmount);
        return err;
    }
    ...
    /**
     * @notice Reject borrow
     */
    function borrow(uint borrowAmount) external returns (uint) {
        borrowAmount;

        return fail(Error.REJECTION, FailureInfo.JUST_REJECTION);
    }

    /**
     * @notice Reject repayBorrow
     */
    function repayBorrow(uint repayAmount) external returns (uint) {
        repayAmount;
    ...
    /**
     * @notice Reject repayBorrowBehalf
     */
    function repayBorrowBehalf(address borrower, uint repayAmount) external returns (uint) {
        borrower;
        repayAmount;
    ...
    // override accrueInterest to save gas 
    function accrueInterest() public returns (uint) {
        accrualBlockNumber = getBlockNumber();
        return uint(Error.NO_ERROR);
    }

    // override exchangeRateStoredInternal to save gas 
    function exchangeRateStoredInternal() internal view returns (MathError, uint) {
        return (MathError.NO_ERROR, initialExchangeRateMantissa);
    }

    // override _setInterestRateModelFresh, just be silent
    function _setInterestRateModelFresh(InterestRateModel newInterestRateModel) internal returns (uint) {
        newInterestRateModel;
        return uint(Error.NO_ERROR);
    }

    // override borrowRatePerBlock
    function borrowRatePerBlock() external view returns (uint) {
        return 0;
    }

    // override supplyRatePerBlock
    function supplyRatePerBlock() external view returns (uint) {
        return 0;
    }
...
// Modified from compound/CErc20Delegate
/**
 * @title Defilend's CollateralOnlyCErc20Delegate Contract
 * @author DeFIL
 */
contract CollateralOnlyCErc20Delegate is CollateralOnlyCErc20, CDelegateInterface {
    /**
