...
abstract contract TransferFee is OwnableUpgradeSafe, ITST {
    ...
    /**
     * @dev Sets the value, _feeAccount that collects tranfer fee, fee percentange, maximum, minimum amount of wei for trasfer fee.
 ...
 *  - charge fee on token transfer
 *  - ability for owner to burn (destroy) tokens
 *  - a minter role that allows for token minting (creation)
 *  - a pauser role that allows to stop all token transfers
 */
contract TST is
    ...
    TransferFee
{
     ...
     *
     * See {ERC20, TransferFee-constructor}.
     */
    function initialize(
        string memory name,
