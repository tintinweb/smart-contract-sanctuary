...
// StoneToken with Governance.
contract StoneToken is ERC20("StoneToken", "STONE"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "STONE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "STONE::delegateBySig: invalid nonce");
        require(now <= expiry, "STONE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "STONE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying STONEs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "STONE::_writeCheckpoint: block number exceeds 32 bits");
...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to StoneSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // StoneSwap must mint EXACTLY the same amount of StoneSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterChef is the master of Stone. He can make Stone and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that STONEs distribution occurs.
    ...
    // Block number when bonus STONE period ends.
    uint256 public bonusEndBlock;
    // STONE tokens created per block.
    uint256 public stonePerBlock;
    // Bonus muliplier for early stone makers.
    ...
    // The block number when STONE mining starts.
