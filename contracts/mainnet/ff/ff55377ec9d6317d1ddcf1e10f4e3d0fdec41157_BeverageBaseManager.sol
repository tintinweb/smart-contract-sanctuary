...
// Dependency file: contracts/interfaces/IAdapter.sol
...
// Root file: contracts/manager/BeverageBaseManager.sol
...
contract BeverageBaseManager {
    ...
    /**
     * ONLY OPERATOR: Update the SetToken manager address. Operator and Methodologist must each call
     * this function to execute the update.
     *
     * @param _newManager           New manager address
     */
    function setManager(ISetToken _setToken, address _newManager) external onlyOperator {
        require(_newManager != address(0), "Zero address not valid");
        _setToken.setManager(_newManager);
    }

    /**
     * ONLY OPERATOR: Add a new adapter that the BeverageBaseManager can call.
     *
    ...
    /**
     * ONLY OPERATOR: Remove an existing adapter tracked by the BeverageBaseManager.
     *
     ...
     */
    function addModule(ISetToken _setToken, address _module) external onlyOperator {
        _setToken.addModule(_module);
    }
     ...
     */
    function removeModule(ISetToken _setToken, address _module) external onlyOperator {
        _setToken.removeModule(_module);
    }
