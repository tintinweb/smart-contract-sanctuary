...
contract ERC721SaleV3 is Ownable, IERC721Receiver, AbstractSale {
    using AddressLibrary for address;
    using UintLibrary for uint256;
    using StringLibrary for string;
    uint private constant maxBuyerFeePercentage = 10000;
    ...
    event BuyERC20(address indexed token, uint256 indexed tokenId, address seller, address buyer, uint256 price, uint256 nonce, address erc20Token);

    TransferProxy public transferProxy;
    ERC721SaleNonceHolder public nonceHolder;
    ERC20TransferProxy public erc20TransferProxy;

    constructor(TransferProxy _transferProxy, ERC20TransferProxy _erc20TransferProxy, ERC721SaleNonceHolder _nonceHolder, address payable beneficiary) AbstractSale(beneficiary) public {
    ...
    function buy(IERC721 token, uint256 tokenId, uint256 price, uint256 sellerFee, Sig memory signature) public payable {
        ...
        require(msg.value == price.add(buyerFeeValue), "Buy:: msg.value is incorrect" );
        ...
        require(owner == ownerOf, "Buy:: owner should sign correct message");
    ...
    function buyERC20(IERC721 token, uint256 tokenId, IERC20 erc20Token, uint256 price, uint256 sellerFee, Sig memory signature) public {
        address payable owner = address(uint160(token.ownerOf(tokenId)));
        uint256 nonce = nonceHolder.getNonce(token, tokenId);
        uint256 buyerFeeValue = price.mul(buyerFee).div(maxBuyerFeePercentage);
        require(erc20TransferProxy.userBalance(erc20Token, msg.sender) >= price.add(buyerFeeValue), "BuyERC20:: Buyer don't have enough balance" );
        string memory messageSigned = prepareMessage(address(token), tokenId, address(erc20Token), price, sellerFee, nonce);
        address ownerOf = StringLibrary.recover(messageSigned, signature.v, signature.r, signature.s);
        require(owner == ownerOf, "BuyERC20:: owner should sign correct message");
        transferProxy.erc721safeTransferFrom(token, owner, msg.sender, tokenId);
        transferERC20(token, tokenId, erc20Token, price, sellerFee, owner);
        nonceHolder.setNonce(token, tokenId, nonce + 1);
        emit BuyERC20(address(token), tokenId, owner, msg.sender, price, nonce, address(erc20Token));
    }
    
    function transferERC20(IERC721 token, uint256 tokenId, IERC20 erc20Token, uint256 price, uint256 sellerFee, address owner) internal {
        uint256 sellerFeeToSlash = transferERC20FeeToBeneficiary(price, sellerFee, address(erc20Token));
        
        // Transfer Creator Royalty to Creator
        uint256 creatorFee = getCreatorFee(address(token), tokenId);
        address creator = address(0);
        uint256 creatorSlash = 0;

        if(creatorFee != 0) {
            creator = getCreator(address(token), tokenId);
            creatorSlash = price.mul(creatorFee).div(maxBuyerFeePercentage);
            erc20TransferProxy.erc20safeTransferFrom(IERC20(erc20Token), msg.sender, creator, creatorSlash);
        }

        // Transfer Seller
        uint256 sellerSlash = price.sub(creatorSlash).sub(sellerFeeToSlash);
        erc20TransferProxy.erc20safeTransferFrom(IERC20(erc20Token), msg.sender, owner, sellerSlash);
    }
    ...
    function getCreatorFee(address erc721Adddress, uint256 tokenId) internal view returns (uint256) {
        uint256[] memory fees = IERC721SaleToken(erc721Adddress).getFeeBps(tokenId);
        if(fees.length > 0) {
            return fees[0];
        } else {
            return 0;
    ...
    function getCreator(address erc721Adddress, uint256 tokenId) internal view returns (address) {
        address[] memory creators = IERC721SaleToken(erc721Adddress).getFeeRecipients(tokenId);
    ...
    function prepareMessage(address token, uint256 tokenId, address erc20Token, uint256 price, uint256 fee, uint256 nonce) public pure returns (string memory) {
        return prepareMessage(token, tokenId, price, fee, nonce).append(". erc20: ", erc20Token.toString());
    }
