    ...
    LnRewardCalculator
{
        ...
        returns (uint256 total)
    {
        ...
        if (!syncUserInfo[_user]) {
            uint256 iReward4;
            iReward4 = _calcOldSimpleStakingReward(blockNb, _user);
            total = total.add(iReward4);
        }

    }
    
    function _calcOldSimpleStakingReward(uint256 curBlock, address _user)
        internal
        view
        returns (uint256)
    {
        ...
        (poolAmount, poolLastRewardBlock, poolAccRewardPerShare) = mOldSimpleStaking.getPoolInfo();
        
        uint256 calcUserReward;
        uint256 calcUuserAmount;
        uint256 calcUuserDebt;
        
        (calcUserReward, calcUuserAmount, calcUuserDebt) = mOldSimpleStaking.getUserInfo(_user);
        
        uint256 accRewardPerShare = poolAccRewardPerShare;
        uint256 lpSupply = poolAmount;
        if (curBlock > poolLastRewardBlock && lpSupply != 0) {
            uint256 multiplier = curBlock.sub(
                poolLastRewardBlock,
                "cr curBlock sub overflow"
            );
        ...
        uint256 newReward = calcUuserAmount.mul(poolAccRewardPerShare).div(1e20).sub(
            calcUuserDebt,
            "cr newReward sub overflow"
        );
        return newReward.add(calcUserReward);
    }
