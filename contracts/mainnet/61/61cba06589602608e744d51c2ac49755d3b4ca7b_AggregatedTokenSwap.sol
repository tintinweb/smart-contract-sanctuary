...
contract AggregatedTokenSwap is CompressedCaller {
        ...
        returns (LibOrder.OrderInfo[] memory ordersInfo)
    {
        ...
        for (uint i = 0; i < ordersInfo.length; i++) {
            ...
            uint256 remainingOrderAmount = orders[i].takerAssetAmount.sub(ordersInfo[i].orderTakerAssetFilledAmount);
            ...
            uint256 possibleAmount = (allowance < balance) ? allowance : balance;

            if (possibleAmount < remainingOrderAmount) {
                ordersInfo[i].orderTakerAssetFilledAmount = orders[i].takerAssetAmount.sub(possibleAmount);
            }

            if (respectIntersections) {
                for (uint j = 0; j < i; j++) {
                    if (orders[j].takerAddress == orders[i].takerAddress) {
