...
import "./interfaces/ICyberToothedSharks.sol";

contract MysticalWhales is ERC721Enumerable, Ownable {
  ...
  ICyberToothedSharks public CTS;
  ...
  uint256 public cost = 800 ether;
  uint256 public maxSaleSupply = 1;
  uint256 public maxEvolveSupply = 30;
  ...
  function publicMint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintPerTxn, "RRC: You can only mint up to 1 per txn");
  ...
  function areOwned(uint256[] memory tokenIds) public view returns (bool out) {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++){
      if (CTS.ownerOf(tokenIds[i]) != msg.sender){
        out = false;
      }
    }
  }
...
interface ICyberToothedSharks is IERC721Enumerable{
