...
contract DumbMonke is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x1ce422dd5E6a2567b76134765919fC80Df9Ad28f);

    uint256 public MONKE_MAX = 3333;
    uint256 public PRESALE_PRICE = 0.033 ether;
    ...
    uint256 public PRESALE_MINT_LIMIT = MONKE_MAX;
    uint256 public PUBLIC_MINT_LIMIT = MONKE_MAX;
    ...
    uint256 private _activeDateTime = 1640908800; // (GMT): Friday, December 31, 2021 12:00:00 AM
    uint256 private _revealDateTime;
    
    string private _tokenBaseURI = "";
    string private _revealURI = "";
    
    Counters.Counter private _publicMONKE;

    constructor() ERC721("Dumb Monke", "MONKE") {}
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicMONKE.current();

            if (_publicMONKE.current() < MONKE_MAX) {
                _publicMONKE.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicMONKE.current() < MONKE_MAX,"Purchase would exceed MONKE_MAX");
