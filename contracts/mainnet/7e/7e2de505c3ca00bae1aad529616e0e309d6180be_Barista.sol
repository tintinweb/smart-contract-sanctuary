...
// EspressoToken with Governance.
contract EspressoToken is ERC20("Espresso.farm", "ESPR"), Ownable {

    // espresso is an exact copy of SUSHI 
    using SafeMath for uint256;
...
// File: contracts/Barista.sol

pragma solidity ^0.6.5;


interface IMigratorBarista {
    // Perform LP token migration from legacy UniswapV2 to EspressoSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // EspressoSwap must mint EXACTLY the same amount of EspressoSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// Barista is an exact copy of SushiSwap
// we have commented an few lines to remove the dev fund
// the rest is exactly the same

// Barista is the master of Espresso. He can make Espresso and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once ESPR is sufficiently
...
contract Barista is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ESPRs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accEspressoPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accEspressoPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Block number when bonus ESPR period ends.
    uint256 public bonusEndBlock;
    // ESPR tokens created per block.
    uint256 public espressoPerBlock;
    // Bonus muliplier for early espresso makers.
    ...
    // The block number when ESPR mining starts.
    ...
    // Update the given pool's ESPR allocation point. Can only be called by the owner.
    ...
    // View function to see pending ESPRs on the frontend side.
    ...
    function updatePool(uint256 _pid) public {
        ...
        // open the pool when its time to open
        if (block.number > pool.lastRewardBlock && pool.isOpen == false) {
            totalAllocPoint = totalAllocPoint.add(pool.allocPoint);
            pool.isOpen = true;
        }

        // update pool winner and minority winner if needed
        updatePoolWinner();
    ...
    // Deposit LP tokens to Barista for ESPR allocation.
    ...
    // Safe espresso transfer function, just in case if rounding error causes pool to not have enough ESPRs.
    ...
    function getPoolWinner() external view returns (uint256) {
        // if still on the same period, give the number
        if ( endRewardBlock >= block.number) {
            return selectedPoolPid;
        }
        
        // else show the pseudo winner
    ...
    function updatePoolWinner() public {
        ...
        // check if already passed the period
        if (block.number > endRewardBlock) {
    ...
    function exchange(uint256 _amount) external {
        ...
        // burn the ESPR according to rate
    ...
    // View function to see pending ESPRs on frontend.
    function pendingMinorityEspresso(address _account) external view returns (uint256) {
        // if reward has been produced on the previous period and not claim it yet
        if (endMinorityBlock >= block.number) {
            if (selectedMinorityGid < 9) {
                if ((minorityInfo[selectedMinorityGid][_account].endMinorityBlock) == (endMinorityBlock - amountMinorityBlock - 1)) {
                    // if previous period has user' vote
                ...
                if (minorityInfo[nextSelectedMinorityGid][_account].endMinorityBlock == this.calculateStartMinorityBlock().sub(1)) {
                    if (minorityInfo[nextSelectedMinorityGid][_account].endMinorityBlock < block.number) 
                    {
                        ...
                        if (minorityInfo[nextSelectedMinorityGid][_account].amountMinorityVote > 0) {
                            // if round still not changed
                           
                            //calculate pseudo pending
    ...
    function updateMinorityWinner() public {
        // if the time to change period
        if (block.number > endMinorityBlock) {
            // change the block period
            ...
            if (nextSelectedMinorityGid < 9) {    
                // get the minority reward to be distributed (25% from total)
                amountMinorityReward = amountMinorityRewardPool.mul(25).div(100);
                
                // leave the 75% of the total reward for the next period
                amountMinorityRewardPool = amountMinorityRewardPool.sub(amountMinorityReward);
                
                // keep the espresso reward for everyone who eligible
                espresso.mint(address(this), amountMinorityReward);
                
                // save the previous period vote
                prevPeriodMinorityVote = minorityTotalAmountVote[nextSelectedMinorityGid];
            }   
                
            // reset the total period for current round

            // reset the minority pool amount
            ...
            // the selected minority pool in the previous round
            selectedMinorityGid = nextSelectedMinorityGid;
                
            // neutralize the selected minority pool
            nextSelectedMinorityGid = 9;
          
            // reset the inflation rate for latte
    ...
    function voteMinority(uint256 _amount, uint256 _groupId) public {
        ...
        // check if user vote still on the previous round
        if ( minorityInfo[_groupId][_msgSender()].endMinorityBlock < block.number) {
            // withdraw and clear the user vote to guarantee the vote amount
            withdrawMinorityReward();
        }
        
        // adding vote to the user information
