   ...
   mapping(string => tokenfee) public Tokendetail;
    ...
    function newtokenasbid(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal {
       ...
       uint256 dyntokfee =  Tokendetail[tokenAss].tokenfee;
        ...
        require( approveValue >= val, "Approve and Value Not Match");
        require(balances[tokenId][from] > 0, "Is Not a Owner");
    ...
    function _acceptBId(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        require( approveValue >= val, "Approve and Value is not Matching");
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], dyntokfee);
        emit Checking(approveValue, _adminfee, netamount, roy);
        require( approveValue >= _adminfee.add(roy.add(netamount)), "Approve and Calculation Not Match");
    ...
    function changePrice(uint256 value, uint256 tokenId) public{
        require( balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        require( value < order_place[msg.sender][tokenId].price, "price greaterthan previous price");
