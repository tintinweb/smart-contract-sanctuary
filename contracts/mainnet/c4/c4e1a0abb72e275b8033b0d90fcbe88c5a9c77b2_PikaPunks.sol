...
contract PikaPunks is ERC721, Ownable {
    uint256 public constant MAX_SUPPLY = 700;
    uint256 private mintCount = 0;

    uint256 constant PRICE = 80000000000000000;
    string baseTokenURI;
    bool public saleOpen = false;

    event Minted(uint256 totalMinted);

    constructor(string memory baseURI) ERC721("PikaPunks", "Pika") {
        setBaseURI(baseURI);
    }
    ...
    function mint(address _to, uint256 _count) external payable {
        ...
        if (msg.sender != owner()) {
            ...
            require(
                _count <= 7,
                "Maximum 7 NFTs can be minted per transaction"
            );
            require(
