...
contract IntegralOracle is IIntegralOracle {
    ...
    function integralBid(int256 q) private view returns (int256) {
        ...
        for (uint256 i = 1; i < bidExponents.length; i++) {
            ...
            if (q <= qCurrent) {
                ...
                int256 p = (pCurrent.sub(pPrevious)).f18Div(qCurrent.sub(qPrevious)).f18Mul(q.sub(qCurrent)).add(
                    pCurrent
                );
    ...
    function integralBidInverted(int256 s) private view returns (int256) {
        ...
        for (uint256 i = 1; i < bidExponents.length; i++) {
            ...
            int256 A = _POINT_FIVE.f18Mul(pPrevious.add(pCurrent)).f18Mul(qCurrent.sub(qPrevious));
            if (s <= C.add(A)) {
                int256 a = _POINT_FIVE.f18Mul(pCurrent.sub(pPrevious)).f18Div(qCurrent.sub(qPrevious));
                int256 b = pPrevious;
                int256 c = C.sub(s);
                int256 h = ((b.f18Mul(b).sub(_FOUR.f18Mul(a).f18Mul(c))).f18Sqrt().sub(b)).f18Div(_TWO.f18Mul(a));
    ...
    function derivativeBid(int256 t) public view returns (int256) {
        for (uint256 i = 1; i < bidExponents.length; i++) {
            ...
            int256 qCurrent = bidQs[i];
            if (t <= qCurrent) {
                return (pCurrent.sub(pPrevious)).f18Div(qCurrent.sub(qPrevious)).f18Mul(t.sub(qCurrent)).add(pCurrent);
            }
        }
        revert('IO_OVERFLOW');
    }
