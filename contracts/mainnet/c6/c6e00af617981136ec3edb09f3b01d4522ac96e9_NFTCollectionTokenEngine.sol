...
contract NFTCollectionTokenEngine {
    ...
    function setNftTokenValues(
        ...
        address newNftContract = address(
            new HUMBLNFT(name, owner, symbol, uri, cap, royaltyAmount, creator)
        );
...
contract HUMBLNFT is ERC721URIStorage, Royalties, Ownable {
    ...
    ERC721(name, symbol) Royalties(royaltyAmount, creator){
        require(cap >= 1);
        _cap = cap;
        for(uint idCounter = 1; idCounter <= _cap ; idCounter++) {
        _mint(owner, idCounter);
        _setTokenURI(idCounter, uri);
        }
    }
