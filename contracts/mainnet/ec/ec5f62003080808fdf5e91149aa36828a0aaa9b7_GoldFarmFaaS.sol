...
contract GoldFarmFaaS is IRewardDistributionRecipient, ReentrancyGuard {
    ...
    IERC20 public rewardToken;// FEG ERC20
    IERC20 public rewardToken1;// GOLD ERC20
    IERC20 public lpToken; // FEAST ERC20
    ...
    uint256 public txfee = 5; //Amount of frictionless rewards of the LP token 
    
     uint256 public duration = 365 days;
    uint256 public duration1 = 30 days;
       ...
       public
   {
    ...
    function getReward1() public updateReward1(msg.sender) noContract(msg.sender) {
        uint256 reward = earned1(msg.sender);
        if (reward > 0) {
            rewards1[msg.sender] = 0;
            rewardToken1.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }
    ...
    function setFarmRewards1(uint256 reward, uint256 _duration2)
        public
        onlyRewardDistribution
        nonReentrant
        updateReward1(address(0))
    {
        ...
        if(rewardRate1.mul(duration1) <= rewardToken1.balanceOf(address(this))){
            ...
            if (block.timestamp >= periodFinish1) {
                ...
                uint256 remaining = periodFinish1.sub(block.timestamp);
                uint256 leftover = remaining.mul(rewardRate1);
                rewardRate1 = reward.add(leftover).div(duration1);
            }
