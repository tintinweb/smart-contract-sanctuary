...
contract ERC20 is Context, IERC20 {
    ...
    /**
     * @dev Returns the amount of taxed tokens.
     */
    ...
    /**
     * @dev Taxes `amount` tokens from `account`, increasing the
     * taxed supply.
     *
     * Emits a {Transfer} event with `to` set to the tax address.
     *
     * Requirements
     *
     * - `account` cannot be the zero address.
    ...
    /**
     * @dev Sets {_taxAddress} to a value other than the initial one.
     */
...
contract YvsToken is ERC20("YVS.Finance", "YVS", 18, 2000000), Ownable {
    function setTaxAddress(address taxAddress_) public onlyOwner {
        _setupTaxAddress(taxAddress_);
    }
