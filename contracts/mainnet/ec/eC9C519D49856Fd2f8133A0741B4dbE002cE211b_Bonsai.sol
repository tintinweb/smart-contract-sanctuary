...
contract Bonsai is Ownable, ERC165, ERC721 {
    // Libraries
    ...
    // This hash is the SHA256 output of the concatenation of the IPFS image hash data for all 8888 Bonsai.
    string public provenance = "";

    // This contract will then return a decentralised IPFS URI when tokenURI() is called.
    string public folder;

    // This value will be set by an admin to an IPFS url that will list the hash and CID of all Bonsai.
    string public provenanceURI = "";

    // After sale is complete, and provenance records updated, the contract will be locked by an admin and then
    // the state of the contract will be immutable for the rest of time.
    bool public locked = false;

    address public WATER_ADDRESS = 0x65040331cD089670fFB25eE15845a7c876B8A728;
    ...
    constructor() ERC721("Bonsai", "BNSI") {
        _setBaseURI("https://us-central1-zenft-65a6d.cloudfunctions.net/app/");
        ownerMint(50);
        folder = "";
    }
    ...
    function mint(uint256 quantity) public payable {
        require(open, "Bonsai drop not open yet");
        require(quantity > 0, "Quantity must be at least 1");

        // Limit buys to 888 Bonsai
        if (quantity > 888) {
            quantity = 888;
        }
        ...
        for (uint256 i = 0; i < quantity; i++) {
            _mintBonsai(msg.sender);
        }
    }

    function mintWithWater(uint256 quantity) public payable {
        require(open, "Bonsai drop not open yet");
        require(quantity > 0, "Quantity must be at least 1");
        ...
        uint256 price = getWaterPrice(quantity);

        IERC20(WATER_ADDRESS).transferFrom(msg.sender, address(this), price);
    ...
    function getQuantityFromValue(uint256 value) public view returns (uint256) {
        return value.div(0.08 ether);
    }

    function getPrice(uint256 quantity) public view returns (uint256) {
        require(quantity <= MAX_SUPPLY);
        return quantity.mul(0.08 ether);
    }

    function getWaterPrice(uint256 quantity) public view returns (uint256) {
        require(quantity <= MAX_SUPPLY);
        return quantity.mul(3e24);
    }
    ...
    // Private Methods
    function _mintBonsai(address owner) private {
        _tokenIds.increment();
