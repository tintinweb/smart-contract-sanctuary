...
contract BabyYoda is ERC20("BabyYoda", "yoda"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "yoda::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "yoda::delegateBySig: invalid nonce");
        require(now <= expiry, "yoda::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "yoda::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "yoda::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts/yodaMasterFarmer.sol
...
interface IMigratorToyodaSwap {
    // Perform LP token migration from legacy UniswapV2 to yodaSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // yodaSwap must mint EXACTLY the same amount of yodaSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// yodaMasterFarmer is the master of yoda. He can make yoda and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once yoda is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract yodaMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of yodas
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accyodaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accyodaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. yodas to distribute per block.
        uint256 lastRewardBlock;  // Last block number that yodas distribution occurs.
        uint256 accyodaPerShare; // Accumulated yodas per share, times 1e12. See below.
    }

    // The yoda TOKEN!
    BabyYoda public yoda;
    // Dev address.
    address public devaddr;
    // yoda tokens created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early yoda makers.
    ...
    IMigratorToyodaSwap public migrator;
    ...
    event SendyodaReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "yodaMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's yoda allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToyodaSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 yodaForDev;
        uint256 yodaForFarmer;
        (yodaForDev, yodaForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        
        if (yodaForDev > 0) {
            yoda.mint(devaddr, yodaForDev);
            //For more simple, I lock reward for dev if mint reward in bonus time
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                yoda.lock(devaddr, yodaForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            }
        }
        yoda.mint(address(this), yodaForFarmer);
        pool.accyodaPerShare = pool.accyodaPerShare.add(yodaForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer) {
        ...
        uint256 yodaCanMint = yoda.cap().sub(yoda.totalSupply());

        if (yodaCanMint < amount) {
            forDev = 0;
            forFarmer = yodaCanMint;
        }
        else {
    ...
    // View function to see pending yodas on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accyodaPerShare = pool.accyodaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 yodaForFarmer;
            (, yodaForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accyodaPerShare = accyodaPerShare.add(yodaForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accyodaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Safe yoda transfer function, just in case if rounding error causes pool to not have enough yodas.
    function safeyodaTransfer(address _to, uint256 _amount) internal {
        uint256 yodaBal = yoda.balanceOf(address(this));
        if (_amount > yodaBal) {
            yoda.transfer(_to, yodaBal);
        } else {
            yoda.transfer(_to, _amount);
        }
    }
