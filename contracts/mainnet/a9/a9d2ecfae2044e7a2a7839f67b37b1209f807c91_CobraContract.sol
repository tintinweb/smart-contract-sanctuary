...
contract CobraContract is ERC721Enumerable, Ownable, PaymentSplitter {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    uint256 realMintAmount = _mintAmount;
    
    if(isWhitelisted1(msg.sender)){
        realMintAmount = _mintAmount + 1;
    } else if(isWhitelisted2(msg.sender)){
        realMintAmount = _mintAmount + 2;
    } else {
        realMintAmount = _mintAmount;
    }
    
    for (uint256 i = 1; i <= realMintAmount; i++) {
  ...
  function isWhitelisted2(address _user) public view returns (bool) {
    for (uint i = 0; i < whitelistedAddresses2.length; i++) {
