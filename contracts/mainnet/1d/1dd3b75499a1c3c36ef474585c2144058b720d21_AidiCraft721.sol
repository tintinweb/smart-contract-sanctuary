...
interface IInterfaceAidiFee {
    ...
    function getTokenFee(string memory _type) external pure returns (uint256);
}

contract AidiCraft721 is
 Initializable,
    ERC721Upgradeable,
    ...
    ERC721BurnableUpgradeable
{
    ...
    uint256 deci;
    address feecontract;
    IInterfaceAidiFee aidifeecon;

    function initialize() public initializer {
        __Ownable_init();
        ERC721Upgradeable.__ERC721_init("AidiCraft721", "AidiCraft721");
        _tid = 1;
        addTokenType("WETH", 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2); 
        addTokenType("VERSE", 0xe18841d7A75866688E291703BdE66c3378bd74A3);
        addTokenType("USDT",0xdAC17F958D2ee523a2206206994597C13D831ec7); 
        feecontract = 0x2dc51f611312Aecd5428dfC7fc9B051b9adEF5Dc;
        aidifeecon = IInterfaceAidiFee(feecontract);

    }

    function SetFeecontract (address feecon) public onlyOwner{
        feecontract= feecon;
        aidifeecon = IInterfaceAidiFee(feecon);
    ...
    function getServefromFecon() public view returns (uint256) {
        return aidifeecon.getServiceFee();
    }
   ...
   function Royalmint(
        ...
        if (!_exists(tokenId)) {
            ...
            token_id[tokenId] = Metadata(name, ipfsimage, ipfsmetadata);
            _creator[tokenId] = from;
            _royalityaddress[tokenId] = royalityadd;
            _safeMint(from, tokenId);
            _royal[tokenId] = royal;
            balances[tokenId][from] = supply;
            if (value != 0) {
    ...
    function _orderPlace(
        ...
        require(balances[tokenId][from] > 0, "You should be the token owner");
        Order memory order;
    ...
    {
        require(_exists(tokenId), "The token is not minted yet!");
        Metadata memory date = token_id[tokenId];
    ...
    function newtokenasbid(
        ...
        uint256 dyntokfee = aidifeecon.getTokenFee(tokenAss);

        uint256 val = pERCent(amount, dyntokfee).add(amount);
        IERC20Upgradeable t = IERC20Upgradeable(
            Tokendetail[tokenAss].tokenAddress
        );
        ...
        if (roy > 0) {
            ...
            t.transferFrom(msg.sender,  _royalityaddress[tokenId], roy);
        }
        if (netamount > 0) {
    ...
    function _acceptBId(
        ...
        if (roy > 0) {
            ...
            t.transferFrom(from, _royalityaddress[tokenId], roy);
        }
        if (netamount > 0) {
    ...
    function _saleToken(
        ...
        uint256 servfee = aidifeecon.getServiceFee();
        uint256 val = pERCent(amount, servfee).add(amount);
        ...
        address payable create = payable( _royalityaddress[tokenId]);
    ...
    function changePrice(uint256 value, uint256 tokenId) public {
        ...
        require(
            value < order_place[msg.sender][tokenId].price,
            "Price should be greater than the previous price"
        );
    ...
    function burnToken(uint256 id, address from) public {
        require(
            balances[id][from] == 1,
            "You should be the token owner and must have sufficient balance!"
        );
        burn(id);
