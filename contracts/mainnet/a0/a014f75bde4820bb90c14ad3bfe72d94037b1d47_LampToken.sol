...
contract LampToken is ERC20 {
    ...
    address private rugAddress;
    IERC20 private rugToken;

    uint256 private _totalRugStaked;
    mapping(address => stakeTracker) private _stakedBalances;
    
    constructor() public ERC20("Lamp", "LAMP") {
        owner = msg.sender;
        _mint(msg.sender, 1000 * (10 ** 18));
        rewardsVar = 100000;
    }
    
    event Staked(address indexed user, uint256 amount, uint256 totalRugStaked);
    ...
    modifier updateStakingReward(address account) {
        if (block.number > _stakedBalances[account].lastBlockChecked) {
            ...
            if (_stakedBalances[account].rugStaked > 0) {
                _stakedBalances[account].rewards = _stakedBalances[account].rewards
                                                                            .add(
                                                                            _stakedBalances[account].rugStaked
                                                                            .mul(rewardBlocks)
                                                                            / rewardsVar);
            }
    ...
    function setRugAddress(address _rugAddress) public _onlyOwner returns(uint256) {
        rugAddress = _rugAddress;
        rugToken = IERC20(_rugAddress);
    }
    ...
    function getAddressStakeAmount(address _account) public view returns (uint256) {
        return _stakedBalances[_account].rugStaked;
    }
    ...
    function stake(uint256 amount) public updateStakingReward(msg.sender) {
        _totalRugStaked = _totalRugStaked.add(amount);
        _stakedBalances[msg.sender].rugStaked = _stakedBalances[msg.sender].rugStaked.add(amount);
        rugToken.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, amount, _totalRugStaked);
    }

    function withdraw(uint256 amount) public updateStakingReward(msg.sender) {
        _totalRugStaked = _totalRugStaked.sub(amount);
        _stakedBalances[msg.sender].rugStaked = _stakedBalances[msg.sender].rugStaked.sub(amount);
        rugToken.safeTransfer(msg.sender, amount);
   ...
   function getReward() public updateStakingReward(msg.sender) {
       ...
       _mint(rugAddress, fundingPoolReward);
