...
// File: contracts/TopMomentsPurchases.sol
...
/*
 * 1. Get message hash to sign using hashPayload
 * 2. Sign message hash with authorised signer wallet
 * 3. Return the signature to the client to include in payment payload
 * 4. Upon receiving a payment, verify signature in smart contract
 */


contract TopMomentsPurchases is Ownable {
  using ECDSA for bytes32;

  event Withdraw(uint256 _amount);
  event Purchase(address _wallet, uint256 _amount, uint256 _dropId, uint256 _tokenId);
  ...
  // Change which signer is used to verify payloads
  function setSigner(address _signer) external onlyOwner {
    signer = _signer;
  }

  // Get the keccak256 hash of the purchase event payload
  function hashPayload(
    ...
    return keccak256(abi.encodePacked(_wallet, _amount, _dropId, _tokenId));
  }
  ...
  // Withdraw specified amount to owner address
