...
// File: contracts/Cryptogolf-ether.sol
...
contract CryptoGolf is ERC721("CryptoGolf", "GOLFPUNKS"), ERC721Enumerable, Ownable {
    ...
    string private baseURI = "ipfs://QmVaBgZGeu9Jey8cidv5xyHAteD2s7gP4uijBTSmZtbyVQ/";
    ...
    address private golfContractAddress = address(0x69685772d4ac0ffC2578F31F8A5c0009E900BAf4);
    ...
    function mintNFT(uint256 _numOfTokens, uint256 _timestamp) public payable {
        ...
        for (uint256 i = 1; i <= _numOfTokens; i++) {
            ...
            emit Mint(msg.sender, totalSupply() + 1, _timestamp);
        }
    }
    ...
    function getOwnerTokens(address _owner) public view returns (string memory){
        ...
        for (uint256 i = 0; i < ERC721.balanceOf(_owner); i++) {
            ...
            json = string(abi.encodePacked(json,
                    "{\"tokenURI\":\"", tokenURI(token_id),
                    "\",\"listPrice\":\"", string(abi.encodePacked(
                        listedPrice[token_id][_owner].tokenPrice.toString(), ",", 
                        listedPrice[token_id][_owner].tokenType.toString(), ",", 
                        listedPrice[token_id][_owner].timestamp.toString())),
                    "\",\"highestOffer\":\"", string(abi.encodePacked(
                        highestOffer[token_id][_owner].price.toString(), ",",
                        highestOffer[token_id][_owner].currency.toString())),
                    "\",\"lastSold\":\"", string(abi.encodePacked(
                        lastSoldPrice[token_id].price.toString(), ",", 
                        lastSoldPrice[token_id].currency.toString(), ",", 
                        lastSoldPrice[token_id].timestamp.toString())),
                    "\"}"
                )
            );
    ...
    function getJsonData(uint256 _tokenId, address _owner, string memory _jsonStr) private view returns (string memory) {
        return string(abi.encodePacked(_jsonStr,
            "{\"tokenURI\":\"", tokenURI(_tokenId),
            "\",\"owner\":\"", toAsciiString(abi.encodePacked(_owner)),
            "\",\"listPrice\":\"", string(abi.encodePacked(
                listedPrice[_tokenId][_owner].tokenPrice.toString(), ",", 
                listedPrice[_tokenId][_owner].tokenType.toString(), ",", 
                listedPrice[_tokenId][_owner].timestamp.toString())),
            "\",\"highestOffer\":\"", string(abi.encodePacked(
                highestOffer[_tokenId][_owner].price.toString(), ",", 
                highestOffer[_tokenId][_owner].currency.toString())),
            "\",\"lastSold\":\"", string(abi.encodePacked(
                lastSoldPrice[_tokenId].price.toString(), ",", 
                lastSoldPrice[_tokenId].currency.toString(), ",", 
                lastSoldPrice[_tokenId].timestamp.toString())),
            "\"}"));
    }

    function getSampleTokens() public view returns (string memory) {
        string memory json;
        uint256 token_id = 0;
        address owner;
        json = "[";
        uint256 to = totalSupply() > 7 ? 7 : totalSupply();
        for (uint256 i = 0; i < to; i++) {
            token_id = ERC721Enumerable.tokenByIndex(i);
            owner = ERC721.ownerOf(token_id);
            json = getJsonData(token_id, owner, json);
            json = string(abi.encodePacked(json, ","));
        }
        json = string(abi.encodePacked(json, "]"));
        return json;
    ...
    function getTokenDetail(uint256 _tokenId) public view returns (string memory) {
        string memory json;
    ...
    function toAsciiString(bytes memory data) public pure returns (string memory) {
    ...
    // List for sell
    function setlistedPrice( uint256 _tokenId, uint256 _price, uint256 _type, uint256 _timestamp) public {
        require(
        ...
        listedPrice[_tokenId][owner] = ListToken(_type, _price, _timestamp);

        ERC721.approve(address(this), _tokenId);
    }

    function getListedPrice(uint256 _tokenId) public view returns (ListToken memory) {
        address owner = ERC721.ownerOf(_tokenId);
        return listedPrice[_tokenId][owner];
    }

    function removeFromListedPrice(uint256 _tokenId) public {
        address owner = ERC721.ownerOf(_tokenId);
        require(msg.sender == owner, "Not permission for this action");

        delete listedPrice[_tokenId][owner];
        delete highestOffer[_tokenId][owner];

        ERC721.approve(address(0), _tokenId); //return token ownership

        for (uint256 i = 0; i < offeredPricesOfToken[_tokenId][owner].length; i++) {
    ...
    // Approve by owner
    function transferToken( address _to, uint256 _tokenId, uint256 _timestamp ) external {
        require(
            _exists(_tokenId),
    ...
    function _transferFrom(
        ...
        lastSoldPrice[_tokenId] = LastSoldPrice(1, approvedPrice, _timestamp);
    ...
    function approveTokenWithGolf(
        ...
        lastSoldPrice[_tokenId] = LastSoldPrice(2, approvedPrice, _timestamp);
    ...
    function cancelOffer(
        ...
        address owner = ERC721.ownerOf(_tokenId);
        // require(msg.sender == owner, "Not permission for this action");
        OfferedPrice[] storage offers = offeredPricesOfToken[_tokenId][owner];
        uint256 index = 0;
        for (uint256 i = 0; i < offeredPricesOfToken[_tokenId][owner].length; i++) {
            if (address(offeredPricesOfToken[_tokenId][owner][i].buyer) == address(_to)) {
        ...
        for(uint256 j = 0; j < offers.length; j++) {
            if(j != index) {
                offeredPricesOfToken[_tokenId][owner][j] = offers[j];
            } else {
                if(offers.length >  1 && j < offers.length - 1) {
                    offeredPricesOfToken[_tokenId][owner][j] = offers[j + 1];
                }
            }
        }
    ...
    function _setOfferedPrice(
        ...
        if (!isExist) {
            offeredPricesOfToken[_tokenId][owner].push(
                OfferedPrice(_buyer, _price, _tokenType, _timestamp)
            );
        }
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            ...
            json = string(abi.encodePacked(json,
                    ...
                    "\",\"currency\":\"", data[i].tokenType.toString(),
    ...
    function buyNFTWithGolf(
        ...
        lastSoldPrice[_tokenId] = LastSoldPrice(2, _price, _timestamp);
    ...
    function buyNFT(uint256 _tokenId, uint256 _timestamp) public payable {
        ...
        lastSoldPrice[_tokenId] = LastSoldPrice(1, msg.value, _timestamp);
