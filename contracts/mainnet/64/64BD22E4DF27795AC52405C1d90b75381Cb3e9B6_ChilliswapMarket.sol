...
import "./IMarket.sol";
import "./ChilliSwapNFT.sol";
import "./IERC20.sol";


contract ChilliswapMarket is IMarket,Ownable {

    // uint256 public constant DAY = 86400;
    // uint256 public constant FIFTEEN_MINUTES = 900;

    // uint256 public constant AUCTION_TIME = 20;
    ...
    function listOnAuction(uint256 tokenId, uint256 price, CURRENCY currency, uint256 _days ) public override returns (Auction memory) {
        require(ChilliSwapNFT(ERC721_TOKEN_ADDRESS).ownerOf(tokenId) == msg.sender, "EA");
        ...
        ChilliSwapNFT(ERC721_TOKEN_ADDRESS).transferFrom(msg.sender, address(this), tokenId);
    ...
    function claimNft(uint256 _tokenId) public override returns(uint256) {
        ...
        if (auctionItem.owner == address(0)) {
            ChilliSwapNFT(ERC721_TOKEN_ADDRESS).transferFrom(address(this), auctionItem.seller, _tokenId);
        } else {
            ChilliSwapNFT(ERC721_TOKEN_ADDRESS).transferFrom(address(this), auctionItem.owner, _tokenId);

            ChilliSwapNFT.Artwork memory artwork = ChilliSwapNFT(ERC721_TOKEN_ADDRESS).getArtwork(_tokenId);
...
//   function transferAnyERC721Token(address _address, uint token) public onlyOwner returns (bool success) {
//         return ChilliSwap(_address).transferFrom(address(this), owner(), token);
// }
