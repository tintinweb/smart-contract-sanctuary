...
contract MonkeySociety is ERC721Enumerable, Pausable, PaymentSplitter {
    ...
    uint256 public maxTotalSupply = 11100;
    uint256 public maxGiftSupply = 400;
    ...
    address[] private _team = [0xdc80dFa06B7AD9331120D302551d10208901D9d6];
    ...
    function getPrice() public view returns (uint256) {
        ...
        if (block.timestamp <= _presaleConfig.startTime + 27 hours) {
            _price = 69000000000000000; //0.069 ETH
        } else if (block.timestamp <= _saleConfig.startTime + 6 hours) {
            _price = 180000000000000000; //0.18 ETH 
        } else if ((block.timestamp > _saleConfig.startTime + 6 hours) && (block.timestamp <= _saleConfig.startTime + 12 hours)) {
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        require(
            block.timestamp <=
                _presaleConfig.startTime + 27 hours,
    ...
    function mainMint(uint256 _amount) external payable whenNotPaused {
        ...
        if (
            block.timestamp <=
            (presaleConfig.startTime + 27 hours)
        ) {
            presaleMint(_amount);
        } else {
            saleMint(_amount);
