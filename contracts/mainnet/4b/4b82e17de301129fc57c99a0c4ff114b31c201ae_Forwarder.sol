    ...
    MixinReceiver
{
    ...
    function executeTransactionv2(uint256 salt,
        address signerAddress,
        bytes memory data,
    ...
    // maker lists NFT for x eth with this contract address as sender's address
    ...
    function BuyOrderWithEth(
        ...
        uint256 refEthFeeAmount,
        address payable refRecipient
    )
        public
        payable
        returns (
        ...
        )
    {
        ...
        uint256 remaingEthAmount = msg.value.safeSub(wethSpentAmount);
        WETH.withdraw(remaingEthAmount);
        
        require(remaingEthAmount >= ethFeeAmount + refEthFeeAmount,'underpaid');
        
        require(remaingEthAmount >= wethSpentAmount.safeMul(fee).safeDiv(feeBase),'underpaid');
        
        if (refEthFeeAmount>0){
            require(refEthFeeAmount < ethFeeAmount.safeMul(26).safeDiv(100),'max referal exceeded');
            _transferEthToAddress(refEthFeeAmount,refRecipient);
            _transferEthToAddress(ethFeeAmount.safeSub(refEthFeeAmount),owner);
        ...
        uint256 wethRemaining = remaingEthAmount.safeSub(ethFeeAmount);
        //dust
