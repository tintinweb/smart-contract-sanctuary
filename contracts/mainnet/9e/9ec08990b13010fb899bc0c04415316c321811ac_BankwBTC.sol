...
contract BankwBTC is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BankwBTCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bank wBTC", "BANKWBTC") {
        dividendTracker = new BankwBTCDividendTracker();
    ...
    receive() external payable {
      ...
      function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BankwBTC: The Uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BankwBTC: Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "BankwBTC: Account is already excluded from fees");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "BankwBTC: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            ...
            if(!openForPresale){
                require(canTransferBeforeTradingIsEnabled[from], "BankwBTC: This account cannot send tokens until trading is enabled");
            }
        }
    ...
    function swapAndSendToDev(uint256 tokens) private {
        ...
        address payable _devAndMarketingAddress = payable(0xb18545C370d5699727dC6e5C6947A1FdA8e52281);
...
contract BankwBTCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BankwBTC_Dividend_Tracker", "BankwBTC_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BankwBTC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BankwBTC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BankwBTC contract.");
    }
    ...
    function excludeFromDividends(address account) external onlyOwner {
      ...
      function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "BankwBTC_Dividend_Tracker: Cannot update gasForTransfer to same value");
        emit GasForTransferUpdated(newGasForTransfer, gasForTransfer);
        gasForTransfer = newGasForTransfer;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BankwBTC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BankwBTC_Dividend_Tracker: Cannot update claimWait to same value");
