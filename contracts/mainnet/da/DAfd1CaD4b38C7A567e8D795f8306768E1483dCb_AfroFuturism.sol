...
contract AfroFuturism is ERC721Enumerable, Ownable{
    ...
    uint256 private reservedPeriod = 309000;               
    ...
    modifier lock() {
        require(unlocked == 1, 'AfroFuturism: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    constructor() ERC721
        ("Afrofuturism", "AF") 
    {
//      _baseTokenURI = "https://gateway.pinata.cloud/ipfs/Qmch9tvV7cnnUQJ6wTBuH7JxtgWRzf9EL8yCiGp6ZbJZf3/Vision";
    ...
    function generateRandomTokenId() private view returns (uint256){
        ...
        uint _randomNumber;
        while (index < MAX_TOKEN_SUPPLY)
        {
            index++;
            
            _randomNumber = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, index))) % MAX_TOKEN_SUPPLY + 1;
            
            if (_tokenStatus[_randomNumber] != Status.Minted)
                break;
        }
        
        if (index == MAX_TOKEN_SUPPLY)
            return 0;
            
        ticketNumber = _randomNumber;
        
        return ticketNumber;
    }
    
    function mintAF() payable public lock {
        require(_tokenIdTracker.current() < MAX_TOKEN_SUPPLY, "Too Many Tokens");
