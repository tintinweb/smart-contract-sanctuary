...
contract TeamAdmin {
    ProForwarderInterface private Pro_Inc = ProForwarderInterface(0x0);
    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    function setup(address _addr) onlyDevs() public {
        require( address(Pro_Inc) == address(0) );
        Pro_Inc = ProForwarderInterface(_addr);
    }
    ...
    */
    function addAdmin(address _who, bytes32 _name, bool _isDev) public onlyDevs() {
