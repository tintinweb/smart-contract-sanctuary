...
contract ZekeInu is Context, IERC20, Ownable {
    ...
    string private _name = "Zeke Inu";
    string private _symbol = "ZEKE";
    uint8 private _decimals = 9;

    uint256 public _taxFee = 0;
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 10; //(3% liquidityAddition + 7% Marketing)
    ...
    uint256 public _maxBuyAmount = 500000000 * 10**9;
    uint256 public _maxBuyAmount1 = 1000000000 * 10**9;
    ...
    uint256 private buyLimitEnd1;
    uint256 private buyLimitEnd2;
    ...
    constructor (address payable addr1, address payable addr2, address payable addr3) {
        ...
        _isExcludedFromFee[fundWallet] = true;
    ...
    function Launch() public onlyOwner(){
        require(limit == true, 'limit is already false');
            ...
            buyLimitEnd1 = block.timestamp + (120 seconds);
            buyLimitEnd2 = block.timestamp + (180 seconds);
    }
    ...
    function _transfer(
        ...
        {
            require(limit == false, "You have no power here!");
        }
            ...
            if(from == pancakePair && to != address(pancakeRouter) && !_isExcludedFromFee[to]) {
              require(amount <= _maxTxAmount);
                    if(buyLimitEnd > block.timestamp) {
                      _liquidityFee = 95;
                    } else if(buyLimitEnd1 > block.timestamp) {
                        require(amount <= _maxBuyAmount);
                        _liquidityFee = 10;
                        require(cooldown[to].buy < block.timestamp, "Your buy cooldown has not expired.");
                        cooldown[to].buy = block.timestamp + (30 seconds);
                      }
                      else if(buyLimitEnd2 > block.timestamp) {
                          require(amount <= _maxBuyAmount1);
                          _liquidityFee = 10;
                          require(cooldown[to].buy < block.timestamp, "Your buy cooldown has not expired.");
                          cooldown[to].buy = block.timestamp + (30 seconds);
                        }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
          require(maxTxPercent <= 100, "Maximum tax limit is 100 percent");
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(
            10**2);
            _maxBuyAmount = _tTotal.mul(maxTxPercent).div(
                10**2);
                _maxBuyAmount1 = _tTotal.mul(maxTxPercent).div(
                    10**2);
    }
