...
// File: contracts\vaults\sVault.sol
...
contract sVault is ERC20, ReentrancyGuard, Pausable, Initializable {
    ...
    uint256 public earnLowerlimit; //auto earn if vault reach this vaule
    ...
    constructor(
        ...
        public
        ERC20(
            string(abi.encodePacked("snp ", ERC20(_token).name())),
            string(abi.encodePacked("s", ERC20(_token).symbol()))
        )
    {
        initialize(_token, _earnLowerlimit, _controller);
    }
    ...
    function harvest(address reserve, uint256 amount)
        external
        nonReentrant
        whenNotPaused
    {
