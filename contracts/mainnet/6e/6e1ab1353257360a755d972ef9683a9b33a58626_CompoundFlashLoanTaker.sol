...
contract CompoundFlashLoanTaker is CompoundSaverProxy, ProxyPermission, FLHelper {

    // address payable public constant _compoundSaverFlashLoan = 0x0b06b30F0ad2Ad14123dD97C133978eEE554059C;
    ...
    function repayWithLoan(
        ...
        uint256 _gasCost,
        FLType _flType,
        address payable _compoundSaverFlashLoan
    ) public payable {
        uint maxColl = getMaxCollateral(_cAddresses[0], address(this));
        uint availableLiquidity = getFLLiquidity(_flType, _exData.srcAddr);
        ...
        if (_exData.srcAmount <= maxColl || availableLiquidity == 0) {
            ...
            givePermission(_compoundSaverFlashLoan);

            _getFL(_flType, getUnderlyingAddr(_cAddresses[0]), loanAmount, paramsData, _compoundSaverFlashLoan);

            removePermission(_compoundSaverFlashLoan);
    ...
    function boostWithLoan(
        ...
        if (_exData.srcAmount <= maxBorrow || availableLiquidity == 0) {
            ...
            _getFL(_flType, getUnderlyingAddr(_cAddresses[1]), loanAmount, paramsData, _compoundSaverFlashLoan);
