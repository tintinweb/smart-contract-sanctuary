...
// File: contracts\WrapperSimple.sol

pragma solidity ^0.6.0;

/**
 * Generic ERC-721 wrapper contract that mints ERC-20 tokens that can be exchanged with any vaulted tokens 1:1. Until key collisions
 * on mappings are impossible / resolved, this will have a vulnerability where tokens can be lost. That is only the case when trying
 * to withdraw by id. Withdrawing without specifying ids will still work (assuming arrays aren't mappings under the hood).
 *
 * This assumes that the target contract conforms to ERC-721 (specifically `safeTransferFrom()`).
 */
contract WrapperSimple is ERC20, IERC721Receiver {
    ...
    /** @dev The tokens stored in the vault. */
    uint256[] private _tokenIds;

    /** @dev The mapping of token ids to their index, 1-based. */
    mapping(uint256 => uint256) private _indices;

    /** @dev The address of the _user that is wrapping / unwrapping tokens. This will not be persisted, to reduce gas usage. */
    address _depositor;

    // OpenZeppelin - ReentrancyyGuard
    // We include this here instead of the `nonReentrant` modifier to reduce gas costs.
    //
	...
	constructor () ERC20("Wrapped Gods Unchained", "WGODS") public {
        // So we don't need to do the math on every mint / burn
        _baseWrappedAmount = uint256(10) ** decimals();

        address contractAddress = 0x6EbeAf8e8E946F0716E6533A6f2cefc83f60e8Ab; // Gods Unchained
    ...
    function onERC721Received(address operator, address from, uint256, bytes calldata) override external returns (bytes4) {
        // We must be in the middle of a deposit
    ...
    function deposit(uint256[] calldata tokenIds) external {
        // On the first call to nonReentrant, _notEntered will be true. Any calls after this point will fail.
    ...
    function withdrawAny(address destination, uint256 count) external {
        ...
        for (uint256 i; i < count; i ++) {
            ...
            // Attempt to transfer the token. If this fails then something really went wrong.
