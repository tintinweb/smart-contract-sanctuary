...
pragma solidity ^0.6.12;
    ...
    /* 
        ...
        Keeping up with all these new projects across chains is a daunting task on your own; it takes time, liquidity and know-how to be consistantly successful.  
        $BCC takes the complexities and time away from the average user traveling throughout the NFT space. 
        Our team and advisors all have acess to groundbreaking NFT startup projects and can properly determine value on existing projects. 
        We will use our expertise along with advisement from our advisors to buy and sell NFT's for profit, which then that profit will be used to buyback $BCC.

        Website: https://www.bcc.holdings
        TG: https://t.me/BCCholding
...
contract BCC is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isBlackhat;

    string private _name = "BlueChip Capital Token";
    ...
    uint256 private _sellMultiplier = 3;
    uint256 private _maxTxAmount = 500000 * 10**9; //500K
    uint256 private numTokensSellToAddToLiquidity = 100000 * 10**9; //100K
    uint256 private _liqAddBlock;
    uint256 private snipeBlockAmt;
    uint256 public blackhatsCaught;
    
    address public uniswapV2Pair;
    address payable public _marketingWalletAddress = 0xaCC32d650a908e98E5a02fc0a42A18B2860c5882;
    address payable public _treasuryWalletAddress = 0x6311356734Bca619F5627BcC3238a5134A65d5f3;
    ...
    event BlackhatCaught(address blackhatAddress);
    ...
    constructor(uint256 _snipeBlockAmt) public {
        ...
        _isExcludedFromFee[_treasuryWalletAddress] = true;  
    ...
    function _transfer(
        ...
        require(!_isBlackhat[from], "blackhats can not transfer");
        require(!_isBlackhat[to], "blackhats can not transfer");
        require(!_isBlackhat[msg.sender], "blackhats can not transfer");
        ...
        if (!_hasLiqBeenAdded) {
            ...
            if (_liqAddBlock > 0 
                ...
                if (block.number - _liqAddBlock < snipeBlockAmt) {
                    _isBlackhat[to] = true;
                    excludeFromReward(to);

                    blackhatsCaught ++;
                    emit BlackhatCaught(to); //pow
                }
            }
        }
        ...
        if (takeFee && to == uniswapV2Pair && !inSwapAndLiquify) {
            _liquidityFee = _liquidityFee.mul(_sellMultiplier);
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        sendETHToCapitalFund(newBalance);
    ...
    function sendETHToCapitalFund(uint256 newBalance) private {
        _treasuryWalletAddress.transfer(newBalance.div(2)); 
        _marketingWalletAddress.transfer(newBalance.div(2)); 
    }
    ...
    function isBlackhatCheck(address account) public view returns (bool) {
        return _isBlackhat[account];
    }
    
    function addBlackhat(address blackhatAddress) external onlyOwner() {
        require(blackhatAddress != uniswapV2Pair, "ERC20: Can not add uniswapV2Pair to blackhat list");
        require(blackhatAddress != address(uniswapV2Router), "ERC20: Can not add uniswapV2Router to blackhat list");

        _isBlackhat[blackhatAddress] = true;
    }
    
    function removeBlackhat(address blackhatAddress) external onlyOwner() {
        require(_isBlackhat[blackhatAddress], "ERC20: Is not blackhat");
        _isBlackhat[blackhatAddress] = false;
    }
    ...
    function setFees(uint256 burnFee, uint256 taxFee, uint256 liquidityFee, uint256 sellMultiplier) external onlyOwner() {
        require(burnFee >= 0 && burnFee <= 15 && taxFee >= 0 && taxFee <= 15 && liquidityFee >= 0 && liquidityFee <= 15, "Must be a number between 0 and 15");
    ...
    function setPair(address payable _uniswapV2Pair) external onlyOwner() {
    ...
    function transferAnyTokens(address _tokenAddr, address _to, uint256 _amount) external onlyOwner() {
        require(_tokenAddr != address(this), "Tranfer failed. Can't remove BCC");
    ...
    function createSweepablePair(IEmpireFactory _factory, bool update) external onlyOwner() {
        ...
        _isExcluded[sweepablePair] = true;
        _excluded.push(sweepablePair);
