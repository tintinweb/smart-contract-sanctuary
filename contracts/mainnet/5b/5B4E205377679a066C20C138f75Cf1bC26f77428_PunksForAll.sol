...
contract PunksForAll is ERC721URIStorage, Ownable{

    event mintpunks (address indexed minter, uint256 startWith, uint256 times);


    uint256 public totalpunks;
    uint256 public totalCount = 4500; //bruhTotal
    ...
    constructor() ERC721("PunksForAll", "PFA") {
    ...
    function mintpunk(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalpunks + _times <= totalCount, "too much");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalpunks + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit mintpunks(_addr, totalpunks+1, 1);
        _mint(_addr, 1 + totalpunks++);
    }
