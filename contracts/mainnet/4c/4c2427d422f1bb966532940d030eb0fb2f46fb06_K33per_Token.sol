...
contract ERC20 is Context, IERC20 {
    ...
    constructor (string memory name, string memory symbol) public {
        ...
        hardcap = 135000 ether; // 470 K33per2 - max. pre-sale supply!
        forK33perholders =  135000 ether; // 470 K33per2 - for K33per holders!
        _dewpresale = 0x6AB2A1D3306c5fa7423F489334fb166221d0AaE4; // dev addr for presale
  ...
  function _transfernewun(address _newun) internal virtual {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        function _forK33perholders(address account) internal virtual {
        require(_totalSupply == 0, "The reward for a K33per holders you can only get 1 time!");
        require(account != address(0), "ERC20: mint to the zero address");

        _beforeTokenTransfer(address(0), account, forK33perholders);

        _totalSupply = _totalSupply.add(forK33perholders);
        _balances[account] = _balances[account].add(forK33perholders);
        emit Transfer(address(0), account, forK33perholders);
    }
    
      /**
     *
     * Only 80000 K33per2 for Pre-Sale!
     * IF SUPPLY < 80 000 K33per2
     * 
     */
   receive() external payable {
        require(now <= endDate, "Pre-sale of tokens is completed!");
        require(_totalSupply <= hardcap, "Maximum presale tokens - 80000 K33per2!");
        uint tokens;
        if (now <= bonusEnds) {
            tokens = msg.value * 2;
        } else {
...
// K33per_Token
contract K33per_Token is ERC20("High APY Finance", "K33per"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
    }
    function forK33perholdersv1(address _to) public onlyOwner {
        _forK33perholders(_to);
    }
