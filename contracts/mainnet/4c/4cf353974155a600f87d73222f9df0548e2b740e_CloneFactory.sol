 ...
 * the owner.
 */
contract CloneFactory is TieredOwnership {
  //event emits new contract address
  event Deployed(address indexed proxyAddress, bytes32 indexed salt) anonymous;
   ...
   * @param salt is a bytes32 specific to the contract being cloned
   * @param data is the initialization data of the logic contract
   * Can only be called by the current owner.
   *
   * Emits a {Deployed} event.
   */
  function createProxy(address beacon, bytes32 salt, bytes memory data) external onlyOwner() {
    address payable clone = payable(Clones.cloneDeterministic(proxyImplementation, salt));
    CloneableProxy(clone).initialize(beacon, data);
    emit Deployed(clone, salt);
  }
}