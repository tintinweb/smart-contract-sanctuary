...
contract Strategy is ConvexStable {
    ...
    constructor(address _vault) public ConvexStable(_vault) {
        curve = address(0xDeBF20617708857ebe4F679508E7b7863a8A8EeE);
        id = 24;
        ...
        _setPathTarget(2, 0); // aave path target

        dex = new address[](3);
        dex[0] = sushiswap; // crv
        dex[1] = sushiswap; // cvx
        dex[2] = sushiswap; // aave
        _approveDex();
    }
    ...
    function _approveDex() internal override {
        super._approveDex();
        IERC20(aave).approve(dex[2], 0);
        IERC20(aave).approve(dex[2], type(uint256).max);
    }
    ...
    function _migrateRewards(address _newStrategy) internal override {
        ...
        claim();
        IERC20(stkaave).safeTransfer(_newStrategy, IERC20(stkaave).balanceOf(address(this)));
        IERC20(aave).safeTransfer(_newStrategy, IERC20(aave).balanceOf(address(this)));
    }
    ...
    function _claimableInETH() internal override view returns (uint256 _claimable) {
        ...
        if (_aave > 0) {
            ...
            path[0] = aave;
            path[1] = weth;
            uint256[] memory swap = Uni(dex[2]).getAmountsOut(_aave, path);
        ...
        )
    {
        ...
        uint256 _stkaave = IERC20(stkaave).balanceOf(address(this));
        if (_stkaave > 0) {
            (bool isCooldown, uint cooldown, 
                uint cooldown_seconds, uint unstake_window) = _checkCooldown();
            if (isCooldown) {
                claim(); // claim aave rewards
                IStakedAave(stkaave).redeem(address(this), _stkaave);
            }
            if (cooldown == 0 || 
                block.timestamp > cooldown_seconds.add(unstake_window)) {
        ...
        if (_aave > 0) {
            ...
            Uni(dex[2]).swapExactTokensForTokens(_aave, uint256(0), path, address(this), now);
        }
    ...
    function claim() public {
        // claim aave rewards
        IStakedAave(stkaave).claimRewards(address(this), type(uint256).max);
    }
    ...
    function _checkCooldown() 
        ...
        returns (
            bool isCooldown, 
            uint cooldown, 
            uint cooldown_seconds, 
            uint unstake_window
        )
    {
        cooldown = IStakedAave(stkaave).stakersCooldowns(address(this));
        cooldown_seconds = IStakedAave(stkaave).COOLDOWN_SECONDS();
        unstake_window = IStakedAave(stkaave).UNSTAKE_WINDOW();
        cooldown_seconds = cooldown.add(cooldown_seconds);
        isCooldown = (
            cooldown != 0 &&
