...
contract TheInuGodfather is Context, IERC20, Ownable {
    using SafeMath for uint256;

    string private constant _name = "The Inu Godfather";
    string private constant _symbol = "THEINU";
    ...
    address payable private _marketingAddress = payable(0x10E6514EbF4fb9821d1F1dc196F60EC157605cc3);
        ...
        returns (uint256)
    {
        require(
            rAmount <= _rTotal,
            "THEINU: Amount must be less than total reflections"
        );
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            // Trade start check
            if (!tradingOpen) {
                require(preTrader[from], "THEINU: Cannot send tokens until trading is enabled");
            }
              
            require(amount <= _maxTxAmount, "THEINU: Max transaction limit exceeded");
            require(!bots[from] && !bots[to], "THEINU: Your account is blacklisted!");
            
            if(to != uniswapV2Pair) {
                require(balanceOf(to) + amount < _maxWalletSize, "THEINU: Balance exceeds max wallet size!");
            }
    ...
    function setMarketingAddress(address _addr) external onlyOwner {
        _marketingAddress = payable(_addr);
    ...
    function unblockBots(address[] memory bots_) external onlyOwner {
        for (uint256 i = 0; i < bots_.length; i++) {
            bots[bots_[i]] = false;
        }
    }
    ...
    // Set minimum tokens required to perform a swap
    ...
    function allowPreTrading(address account, bool allowed) external onlyOwner {
        require(preTrader[account] != allowed, "THEINU: Already set.");
