...
contract QUOXKA is ERC721, ERC721Enumerable, ERC721URIStorage, HasSecondarySaleFees, Ownable {
    ...
    function viewTokenItem(uint256 tokenId) private view returns (string memory){
        uint256 seed1 = bigValue(tokenId, 1);
        uint256 seed2 = bigValue(tokenId, 2);
        uint256 seed3 = bigValue(tokenId, 3);
    ...
    function bigValue(uint256 tokenId, uint256 _seed) private view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(Strings.toString(tokenId), blockhash(_seed)))) % 9007199254740990;
    }
