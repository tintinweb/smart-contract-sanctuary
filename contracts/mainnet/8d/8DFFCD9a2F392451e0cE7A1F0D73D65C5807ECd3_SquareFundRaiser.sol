...
contract SquareFundRaiser is ReferenceTable{
    ...
    bool public swap_to_utility_enabled = false; ///TODO
    ...
    // TetherERC20 public fundingStableCoin = TetherERC20(0xD9BA894E0097f8cC2BBc9D24D308b98e36dc6D02); RINKEBY
    TetherERC20 public fundingStableCoin = TetherERC20(0xd1A7524e8A69771D4F00F754425F6E82A982Ed7C); // FUJI

    uint96 public cUsdtPerSquare = 20;

    uint96 public nextFundingCap = 10e12; // 3 million token ($600k)

    uint96 public fundingCapCurrentState = 0;
    ...
    uint public constant totalSupply = 210000000e6; // 210 million Square
     ...
     */
    constructor(address account, address fundingStableCoinAddress) {
     ...
     */
    function fundViaUSDT(string memory userId ,uint rawAmount) external returns (bool) {
        require(msg.sender != owner(), "Square::caller cannot be the owner");
        require(keccak256(bytes(userId)) == keccak256(bytes(getCurrentReference())), "Square::caller must be the registered address");
        
        address _funder = msg.sender;
        address _owner = owner();
        uint96 amountUSDT = safe96(rawAmount, "Square::transfer: amount exceeds 96 bits");
        uint96 amountSQUARE = safe96((amountUSDT * 100) / cUsdtPerSquare , "Square::transfer: converted amount of DPS exceeds 96 bits");

        fundingCapCurrentState = fundingCapCurrentState + amountSQUARE;
        require(fundingCapCurrentState <= nextFundingCap, "Square::no more DPS available for distribution in this step. Wait the next step in order to continue funding");
     ...
     */
    function getRemainingDPSInPhase() external view returns (uint) {
    ...
    function approve(address spender, uint rawAmount) external returns (bool) {
        ...
        if (rawAmount == type(uint).max ) {
            ...
            amount = safe96(rawAmount, "Square::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external onlyOwner returns (bool) {
        uint96 amount = safe96(rawAmount, "Square::transfer: amount exceeds 96 bits");
     ...
     * @return Whether or not the transfer succeeded
     */
    function transferWithReference(string memory ref, uint rawAmount) external onlyOwner returns (bool) {
        uint96 amountSQUARE = safe96(rawAmount, "Square::transfer: amount exceeds 96 bits");
        fundingCapCurrentState = fundingCapCurrentState + amountSQUARE;
        require(fundingCapCurrentState <= nextFundingCap, "Square::no more DPS available for distribution in this step. Reset the nextFundingCap to start next phase");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external onlyOwner returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "Square::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != type(uint96).max) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "Square::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(signatory != address(0), "Square::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Square::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "Square::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "Square::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "Square::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "Square::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "Square::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "Square::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "Square::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "Square::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "Square::_writeCheckpoint: block number exceeds 32 bits");
