...
contract NovaCreed is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    address payable platformAddress = payable(0xb98e9F2107793CF74c31c31cc97B57D6D08e9A99);
    ...
    uint256 public howManyToMint = 18;
    ...
    function createTokens(uint256 _howMany) external payable {
        ...
        require(_howMany > 0,"NCToken: minimum 1 tokens need to be minted!");
        require(_howMany <= tokensRemainingToBeMinted(),"NCToken: purchase amount is greater than the token available!");
        require(_howMany <= howManyToMint,"NCToken: max tokens you can mint at once exceeded!");
        ...
        if(msg.sender != owner()){
            ...
            if(isWhitelist == true){
                ...
                require(ownerMintedCount + _howMany <= nftPerAddressLimit,"NFTs Per Address during pre-sale is limited");
            }
        }
        
        require(msg.value == price.mul(_howMany),"NCToken: insufficient ETH to mint!");
    ...
    function _mintToken(address to) private {
        ...
        require(
            ...
            "NCToken: one of these tokens already exists!"
        );
        ...
        adminOnly
    {
        require(
            _howMany > 0,
            "NCToken: minimum 1 tokens need to be minted!"
        );
        /*require(_howMany <= 20, "NCToken: max 20 tokens at once!");*/
        require(
            getAdminRemainingMint() > 0,
            "NCToken: All admin token minted"
        );
        require(_howMany <= getAdminRemainingMint(), "NCToken: Token should be less than remaining amount to mint");
        if (mintedTokens == 0) {
            lastIPFSID = getRandom(
                1,
                totalTokenToMint,
    ...
    function getIpfsIdToMint() private view returns (uint256 _nextIpfsId) {
        require(
            !isAllTokenMinted(),
            "NCToken: all tokens have been minted!"
        );
    ...
    function setPrice(uint256 newPrice) external {
        require(
            hasRole(ADMIN_ROLE, msg.sender),
            "NCToken: caller is not an admin!"
        );
        price = newPrice;
    }
