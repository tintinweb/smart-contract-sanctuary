...
contract Utility is CoreConstants {
    ...
    function getNftInfo(
        ...
        for (uint256 i = 0; i < length; i++) {
            IYieldFarm.NFTMetadata memory metadata = IYieldFarm(yieldFarmAddresses[i]).getNftMetadata(nftIds[i]);
            if (metadata.claimed) {
                result[i] = NFTInfo(
                    nftIds[i],
                    address(0),
    ...
    function getYieldFarmMultiplier(address[] memory yieldFarmAddresses) external view returns (uint256[] memory) {
        require(yieldFarmAddresses.length <= 10);
        ...
        for (uint256 i = 0; i < length; i++) {
            result[i] = IYieldFarm(yieldFarmAddresses[i]).multiplier();
        }
