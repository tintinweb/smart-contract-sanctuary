...
// upcore Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract upcoreVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of upcores
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accupcorePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accupcorePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. upcores to distribute per block.
        uint256 accupcorePerShare; // Accumulated upcores per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing upcore governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's upcores allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing upcore governance consensus
    ...
    // View function to see pending upcores on frontend.
    function pendingupcore(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accupcorePerShare = pool.accupcorePerShare;

        return user.amount.mul(accupcorePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the upcore token.
    // ----
    uint256 private upcoreBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = upcore.balanceOf(address(this)).sub(upcoreBalance);

        if(newRewards > 0) {
            upcoreBalance = upcore.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 upcoreRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        upcoreRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 upcoreRewardFee = upcoreRewardWhole.mul(DEV_FEE).div(10000);
        uint256 upcoreRewardToDistribute = upcoreRewardWhole.sub(upcoreRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(upcoreRewardFee);

        pool.accupcorePerShare = pool.accupcorePerShare.add(
            upcoreRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to upcoreVault for upcore allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accupcorePerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accupcorePerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from upcoreVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming upcore farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accupcorePerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeupcoreTransfer(from, pending);
        }
    ...
    // Safe upcore transfer function, just in case if rounding error causes pool to not have enough upcores.
    function safeupcoreTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 upcoreBal = upcore.balanceOf(address(this));
        if (_amount > upcoreBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", upcoreBal);

            upcore.transfer(_to, upcoreBal);
            upcoreBalance = upcore.balanceOf(address(this));

        } else {
            upcore.transfer(_to, _amount);
            upcoreBalance = upcore.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safeupcoreTransfer(devaddr, devSend);
        }
    ...
    // allowing upcore governance token holders to do this functions.
