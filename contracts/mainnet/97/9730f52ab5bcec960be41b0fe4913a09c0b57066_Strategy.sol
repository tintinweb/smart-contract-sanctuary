...
contract Strategy is CurveVoterProxy {
        ...
        )
    {
        ...
        if (_snx > 0) {
            IERC20(snx).safeApprove(sushiswap, 0);
            IERC20(snx).safeApprove(sushiswap, _snx);

            Uni(sushiswap).swapExactTokensForTokens(_snx, uint256(0), pathReward, address(this), now);
        }
