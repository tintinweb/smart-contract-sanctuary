...
contract TestaFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function activate(uint256 _pid) public onlyEOA nonReentrant {
        require(getCountDown(_pid) >= numberOfBlock, "Cannot activate until 5000 blocks pass");
        currentLiquidity = getLiquidity(_pid);
        PoolInfo storage pool = poolInfo[_pid];

        if(pool.initStartBlock == pool.startBlock){
            pool.startBlock = getLatestBlock();
            pool.startLiquidity = currentLiquidity;
        }else{
            currentLiquidity > pool.startLiquidity ? progressive++ : progressive--;
    ...
    function getTestaReward(uint256 _pid) public view returns (uint256){
        ...
        uint256 testaPerDollar = ethPerDollar.div(uint256(_reserve0).div(uint256(_reserve1)).mul(1e18));
        uint256 testaAmount = activeReward.mul(1e18).div(testaPerDollar);
        return testaAmount;
    }
