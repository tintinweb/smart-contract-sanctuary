...
contract UFOR is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "UFO Reloaded";
    string private _symbol = "UFOR";
    ...
    uint256 internal _tokenTotal = 51_000e9;
    ...
    uint256 public _taxFee = 510;
    uint256 public _liquidityFee = 510;

    uint256 public _rebalanceCallerFee = 666;
    ...
    uint256 public rebalanceInterval = 14 minutes;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "UFOR: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'UFOR: The contract it self cannot be excluded');
        require(!_isExcluded[account], "UFOR: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "UFOR: Account is already included");
    ...
    function _transfer(
        ...
        require(now > liquidityAddedAt + 3 minutes  || amount <= 510e9, "You cannot transfer more than 510 tokens.");
