...
contract Icre8ConvertV1 is Ownable, Pausable, ReentrancyGuard {
  ...
  constructor(address tokenAddress, address usdtAddress, address priceFeedAddress) Ownable() Pausable() ReentrancyGuard() {
    contractToken = IERC20(tokenAddress);
    __usdtToken = IERC20(usdtAddress);
  ...
  */
  function convert() public payable whenNotPaused nonReentrant returns (uint256 tokenAmount) {
    ...
    require(contractToken.balanceOf(address(this)) >= amountToBuy, "Vendor contract does not enough tokens in its balance");  
  ...
  */
  ...
  function buyWithUSDT(uint256 tokenAmount) public payable whenNotPaused nonReentrant {
    require(msg.sender != address(0), "Address is zero");
    require(tokenAmount <= 5000000000 || tokenAmount >= 332000000, "Minimum 332 USDT and Max 5000 USDT");
    // Transfer USDT Tether from msg.sender to contract
    // check for allowance 
    if((__usdtToken).allowance(msg.sender, address(this)) >= tokenAmount) {
      (__usdtToken).safeTransferFrom(msg.sender, address(this), tokenAmount);
    } else {
      emit UsdtTokenFetchFailed(msg.sender, tokenAmount);
    }
    // init usdt eth price
    uint256 priceViaFeed = (getLatestPrice());
    // finally transfer ICRE8 
    require(priceViaFeed > 0, "Third party vendor call failed");
      uint256 calculateEth = (tokenAmount).mul(priceViaFeed).div(10 ** 18);
      uint256 amountToBuy = ((calculateEth).div(tokensPerEth)).mul(10 ** 17);
      contractToken.safeTransfer(msg.sender, amountToBuy);
    // require(sendTokens, "Failed to transfer tokens to user");
    // emit the event
    emit Icre8TokensBoughtViaUSDT(msg.sender, tokenAmount, amountToBuy);
  }
  ...
  */
  function withdraw() public nonReentrant onlyOwner {
    (bool w,) =  payable(msg.sender).call{value:address(this).balance}("");
    require(w);
  }

  function withdrawToken (address tokenAddress) public payable nonReentrant onlyOwner {
    IERC20 ___token = IERC20(tokenAddress);
    uint256 balance = ___token.balanceOf(address(this));
    ___token.safeTransfer(payable(msg.sender), balance);
  }
