...
// File: localhost/contracts/JT9.sol
...
contract JT9 is ERC1155, Ownable, DateTime {
    using SafeMath for uint256;
    using Address for address;
    ...
    function setJT(IERC20 _jt) public onlyOwner {
        jt = _jt;
    }
    ...
    function redeem(address account, uint256 level, uint256 id, uint256 dividend, bool mintable) external {
        ...
        if (dividend > 0) {
            jt.safeTransfer(account, dividend);
        }
    }
    ...
    function withdrawForDividend(uint256 amount) public onlyOwner {
        jt.safeTransfer(msg.sender, amount);
    }
    ...
    function batchGen(address[] memory addrs, uint256 level, uint256 dividend) public onlyOwner {
        ...
        for(uint i = 0; i < addrs.length; i++) {
            ...
            if (eachDividend > 0) {
                jt.safeTransfer(addrs[i], eachDividend);
            }
        }
    }
