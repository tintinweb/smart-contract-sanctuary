...
contract XanaAuction is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address payable _platformFeeRecipient, IERC20 _RWT, address _rewardTokenOwner, IXanaAddressRegistry _addressRegistry, uint256 _rewardFee)
        public
        initializer
    {
        require(
        ...
        nonReentrant
    {
        ...
        if (payAmount > 0) {
            if (auction.payToken == address(0)) {
                ...
                if (payAmount != 0) {
                    RWT.safeTransferFrom(rewardTokenOwner, IERC721(_nftAddress).ownerOf(_tokenId), payAmount.mul(rewardFee).div(10000));
                    RWT.safeTransferFrom(rewardTokenOwner, winner, payAmount.mul(rewardFee).div(10000));
                }
            }
        }
