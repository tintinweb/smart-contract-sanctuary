...
https://t.me/OrigamiShiba
https://www.origamishiba.com/

4% redistribution to all holders
10% tax on sell transactions to the marketing wallet

As the origami unfolds, top 1000 holders have a chance to be the ace, king, or queen origami shiba every 12 hours.

1. 50% of the dev fee goes to this origami pool
2. Ace: receives 30% of pool
3. King: receives 20% of the pool
4. Queen: receives 10% of the pool

40% will be kept in the pool for the next draw period.

How it works: top 1000 holders on etherscan is exported every 12 hours

1. A random number will be generated every 12 hours ranging from 1 to 1 trillion for each slot - Ace, King, Queen
2. Every token owned is counted as an entry from top 1000 holders and wherever the random number lands will be determined the slot owner every 12 hours
3. This number will be announced at 12:00 AM/PM PST every single day and etherscan txn will be provided to telegram as proof of payout
...
contract OrigamiShiba is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint8 private _decimals = 9;
    string private _name = "OrigamiShiba";                                      // name
    string private _symbol = "OSHIB";                                           // symbol
    uint256 private _tTotal = 1000 * 10**9 * 10**uint256(_decimals);            // total supply

    // % to holders
    uint256 public defaultTaxFee = 4;
    ...
    address payable public marketingWallet = payable(0xaa3763570f6AfCAd6f8e7763Bad25D5EC2fA4Cb9); // marketing wallet
