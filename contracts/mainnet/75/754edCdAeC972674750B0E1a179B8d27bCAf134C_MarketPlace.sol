...
contract MarketPlace is OwnableUpgradeable, Payments {
    ...
    // rougeeToken IERC20 refference
    IERC20 public rougeeToken;
    ...
    // Mapping from ERC721 contract to mapping of tokenId to sale price using Token Rougee
    mapping(address => mapping(uint256 => SalePrice)) private tokenPricesRougee;
    ...
    // Mapping from ERC721 contract to mapping of tokenId to current bid using Token Rougee
    mapping(address => mapping(uint256 => ActiveBid)) private tokenCurrentBidsRougee;
        ...
        public
    {
        ...
        __Ownable_init();
        rougeeToken = IERC20(0xA1c7D450130bb77c6a23DdFAeCbC4a060215384b);
    }
     ...
     * @param _amount uint256 wei value that the item is for sale
     */
    function setSalePriceForRougee(
        address _originContract,
        uint256 _tokenId,
        uint256 _amount
    ) external {
        ...
        tokenPricesRougee[_originContract][_tokenId] = SalePrice(msg.sender, _amount);
        emit SetSalePrice(_originContract, _amount, _tokenId);
    }
    ...
    function safeBuyUsingRougee(
        ...
        // Make sure the tokenPrice is the expected amount
        require(
            tokenPricesRougee[_originContract][_tokenId].amount == _amount,
            "safeBuy::Purchase amount must equal expected amount"
        );
        buyUsingRougee(_originContract, _tokenId);
    }
     ...
     * @param _tokenId uint256 ID of the token.
     */
    function buyUsingRougee(address _originContract, uint256 _tokenId) public payable {
        // The owner of the token must have the marketplace approved
        ownerMustHaveMarketplaceApproved(_originContract, _tokenId);
        ...
        // Check that the person who set the price still owns the token.
        require(
            erc721.ownerOf(_tokenId) == tokenPricesRougee[_originContract][_tokenId].seller,
            "buy::Current token owner must be the person to have the latest price."
        );

        SalePrice memory sp = tokenPricesRougee[_originContract][_tokenId];

        // Check that token is for sale.
        require(sp.amount > 0, "buy::Tokens priced at 0 are not for sale.");

        require(
            rougeeToken.allowance(msg.sender, address(this)) >= sp.amount,
            'Not enough Allowance for Rougee Tokens'
        );
        ...
        rougeeToken.transferFrom(msg.sender, tokenOwner, (sp.amount) );
     ...
     * @return uint256 sale price of the token
     */
    function tokenPriceInRougee(address _originContract, uint256 _tokenId)
        external
        view
        returns (uint256)
    {
        ...
        if (erc721.ownerOf(_tokenId) == tokenPricesRougee[_originContract][_tokenId].seller) {
            return tokenPricesRougee[_originContract][_tokenId].amount;
        }
        return 0;
    }
     ...
     * @return uint256 sale price of the token including the fee.
     */
    function tokenPriceRougeeFeeIncluded(address _originContract, uint256 _tokenId)
        public
        view
        returns (uint256)
    {
        ...
        if (_priceSetterStillOwnsTheToken(_originContract, _tokenId)) {
            return
                tokenPricesRougee[_originContract][_tokenId].amount.add(0);
        }
        return 0;
    }
    ...
    function changeRougeeToken (address Rougee) public onlyOwner returns (address){
        rougeeToken = IERC20(Rougee);
        return address(rougeeToken);
    }
