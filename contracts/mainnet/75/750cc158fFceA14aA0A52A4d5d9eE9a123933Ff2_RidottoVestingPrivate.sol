...
contract RidottoVestingPrivate is Ownable, Readable {
    using SafeERC20 for IERC20;

    IERC20 immutable public RDT;
    uint128 constant public HUNDRED_PERCENT = 1000;
    uint32 public TGEdate = 1659025800; // Thursday, 28 July 2022 16:30:00 GMT
    bool public paused;

    uint constant LOCK_TYPES = 1;
    enum LockType {
        Private
    }
    ...
    constructor(IERC20 rdt, address newOwner) {
        RDT = rdt;
        db.config[LockType.Private] = LockConfig(0, 0, 12 * months, 1);
        transferOwnership(newOwner);
    }

    function getUserInfo(address who) external view returns(Lock[1] memory) {
        return db.locks[who];
    }
