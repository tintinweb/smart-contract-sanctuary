...
contract Museum is ERC721Enumerable, PaymentSplitter, Ownable {
  ...
  string private _baseUri = "https://ipfs.io/ipfs/QmWkjj1kTz2eMomdB9sLEoRBSfSC15km7WbJtDHHARsGxm/";
  ...
  mapping(uint8 => mapping(address => uint256)) _whitelists;
  ...
  function addWhitelisted(uint8 salePhase, address[] memory _addresses) public onlyOwner {
    for (uint i = 0; i < _addresses.length; i++) {
      _whitelists[salePhase][_addresses[i]] = 3;
    }
  }

  function isWhitelisted(uint8 salePhase, address _address) view public returns (bool) {
    return _whitelists[salePhase][_address] > 0;
  }

  function addIndexToID(uint256[] memory indexToId) public onlyOwner {
    for (uint i = 0; i < indexToId.length; i++) {
      _indexToId.push(indexToId[i]);
    }
  }
  ...
  function getMaxMints(uint8 saleState) public view returns (uint256) {
    if (saleState == 0) {
      return 0;
    } else if (saleState == _SALE) {
      return MAX_PER_PUBLIC_SALE - _buyers[msg.sender];
    } else {
      return _whitelists[saleState][msg.sender];
    }
  }
  ...
  function mint(uint256 amount) public payable {
    ...
    if (isWhitelisted(saleState, msg.sender)) {
      _whitelists[saleState][msg.sender] = _whitelists[saleState][msg.sender] - amount;
    }
