...
contract Farming is IFarming, Ownable {
    ...
    constructor(address _arvoToken, uint256 _maxPoolsCount, address _terminalContract) public {
    ...
    function deposit(uint256 _poolId, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 waiting = user.amount.mul(pool.recentRewardsPerBlock).div(1e18).sub(user.outgoing);
            if (waiting > 0) {
    ...
    function upgradePool(uint256 _poolId) public {
        ...
        pool.recentRewardsPerBlock = pool.recentRewardsPerBlock.add(mainReward.mul(1e18).div(pooledLp));
        pool.recentUpdatedBlock = block.number;
    }
    ...
    function waitingRewards(uint256 _poolId, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.recentUpdatedBlock && pooledLp != 0) {
            uint256 blockSub = block.number.sub(pool.recentUpdatedBlock);
            uint256 mainReward = blockSub.mul(rewardsPerPool);
            recentRewardsPerBlock = recentRewardsPerBlock.add(mainReward.mul(1e18).div(pooledLp));
        }
        return user.amount.mul(recentRewardsPerBlock).div(1e18).sub(user.outgoing);
    }
}
