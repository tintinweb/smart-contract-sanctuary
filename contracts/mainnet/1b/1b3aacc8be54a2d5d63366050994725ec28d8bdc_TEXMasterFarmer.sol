...
// TexToken with Governance.
contract TexToken is ERC20("Total.Exchange", "TEX"), Ownable {
    uint256 private _cap = 80000000e18;
    uint256 private _initial_supply = 35000000e18;
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "TEX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "TEX::delegateBySig: invalid nonce");
        require(now <= expiry, "TEX::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "TEX::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "TEX::_writeCheckpoint: block number exceeds 32 bits");
    ...
    constructor() public {
        ...
        _mint(msg.sender, _initial_supply); // Mint 5000 TEX for bounty program
        _moveDelegates(address(0), msg.sender, _initial_supply);
    }
}

interface IMigratorToTEXSwap {
    // Perform LP token migration from legacy UniswapV2 to TEXSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // TEXSwap must mint EXACTLY the same amount of TEXSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// TEXMasterFarmer is the master of TEX. He can make TEX and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once TEX is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract TEXMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of TEXs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTEXPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accTEXPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. TEXs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that TEXs distribution occurs.
        uint256 accTEXPerShare; // Accumulated TEXs per share, times 1e12. See below.
    }
    ...
    // TEX tokens created per block.
    uint256 public REWARD_PER_BLOCK; // 1000000000000000000
    // Bonus muliplier for early TEX makers.
    ...
    // The block number when TEX mining starts.
    uint256 public START_BLOCK;

    uint256 public constant PERCENT_LOCK_BONUS_REWARD = 75; // lock 75% of bounus reward in 1 year
    uint256 public constant PERCENT_FOR_DEV = 5; // 10% reward for dev

    uint256 public burnPercent = 3; // init 0% burn tex

    uint256 public lockFromBlock;
    uint256 public lockToBlock;

    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorToTEXSwap public migrator;
    ...
    event SendTEXReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    event Lock(address indexed to, uint256 value);
    ...
    constructor(
        ...
        lockFromBlock = block.number + 12742826;
        lockToBlock = lockFromBlock + 12742826;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "TEXMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's TEX allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToTEXSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 texForBurn;
        uint256 texForDev;
        uint256 texForFarmer;
        (texForBurn, texForDev, texForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);

        if (texForBurn > 0) {
            tex.mint(address(this), texForBurn);
            tex.burn(texForBurn);
        }

        if (texForDev > 0) {
            // Mint unlocked amount for Dev
            tex.mint(devaddr, texForDev.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
            // Mint locked amount for Dev
            //For more simple, I lock reward for dev if mint reward in bonus time
            tex.mint(address(this), texForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            farmLock(devaddr, texForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
        }
        tex.mint(address(this), texForFarmer);
        pool.accTEXPerShare = pool.accTEXPerShare.add(texForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forBurn, uint256 forDev, uint256 forFarmer) {
        ...
        uint256 texCanMint = tex.cap().sub(tex.totalSupply());

        if (texCanMint < amount) {
            forBurn = 0;
            forDev = 0;
            forFarmer = texCanMint;
        }
        else {
    ...
    // View function to see pending TEXs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accTEXPerShare = pool.accTEXPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 texForFarmer;
            (, , texForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accTEXPerShare = accTEXPerShare.add(texForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accTEXPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTEXPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = tex.balanceOf(address(this));
            ...
            if(pending > 0) {
                tex.transfer(msg.sender, pending.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
                ...
                emit SendTEXReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accTEXPerShare).div(1e12);
        }
    }

    // Deposit LP tokens to TEXMasterFarmer for TEX allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "TEXMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from TEXMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "TEXMasterFarmer::withdraw: not good");
    ...
    // Safe tex transfer function, just in case if rounding error causes pool to not have enough TEXs.
    function safeTEXTransfer(address _to, uint256 _amount) internal {
        uint256 texBal = tex.balanceOf(address(this));
        if (_amount > texBal) {
            tex.transfer(_to, texBal);
        } else {
            tex.transfer(_to, _amount);
        }
    }
    ...
    function farmLock(address _holder, uint256 _amount) internal {
        require(_holder != address(0), "ERC20: lock to the zero address");
        require(_amount <= tex.balanceOf(address(this)), "ERC20: lock amount over blance");
        _locks[_holder] = _locks[_holder].add(_amount);
        _totalLock = _totalLock.add(_amount);
        if (_lastUnlockBlock[_holder] < lockFromBlock) {
            _lastUnlockBlock[_holder] = lockFromBlock;
    ...
    function unlock() public {
        ...
        uint256 amount = canUnlockAmount(msg.sender);
        // just for sure
        if (amount > tex.balanceOf(address(this))) {
            amount = tex.balanceOf(address(this));
        }
        tex.transfer(msg.sender, amount);
        _locks[msg.sender] = _locks[msg.sender].sub(amount);
        _lastUnlockBlock[msg.sender] = block.number;
        _totalLock = _totalLock.sub(amount);
    }
    ...
    function setTransferBurnRate(uint256 _tranferBurnRate) public onlyOwner {
        tex.setTransferBurnRate(_tranferBurnRate);
    }
    ...
    function removeTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        tex.removeTransferBurnExceptAddress(_transferBurnExceptAddress);
    }
