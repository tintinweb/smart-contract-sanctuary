...
contract Sushiswap_ZapOut_General_V2 is ReentrancyGuard, Ownable {
    ...
    @param FromSushiPoolAddress Pool from which to remove liquidity
    ...
    */
    function ZapOutWithPermit(
        ...
        // permit
        (bool success, ) = FromSushiPoolAddress.call(permitData);
    ...
    */
    ...
    {
        IUniswapV2Pair pair = IUniswapV2Pair(FromSushiPoolAddress);
        ...
        IERC20 sushiPool = IERC20(FromSushiPoolAddress);

        sushiPool.safeTransferFrom(msg.sender, address(this), IncomingLP);

        sushiPool.safeApprove(address(sushiSwapRouter), IncomingLP);
        ...
        emit zapOut(msg.sender, FromSushiPoolAddress, token0, amountA);
        emit zapOut(msg.sender, FromSushiPoolAddress, token1, amountB);
    }
    ...
    */
    function ZapOut(
        ...
        uint256 tokensRec = tokenBought.sub(totalGoodwillPortion);
    ...
    function _removeLiquidity(address FromSushiPoolAddress, uint256 IncomingLP)
        internal
        returns (uint256 amountA, uint256 amountB)
    {
        IUniswapV2Pair pair = IUniswapV2Pair(FromSushiPoolAddress);
    ...
    function _swapTokens(
        ...
        address token0 = IUniswapV2Pair(FromSushiPoolAddress).token0();
        address token1 = IUniswapV2Pair(FromSushiPoolAddress).token1();
    ...
    */
        ...
        )
    {
        ...
        uint256 balance0 = IERC20(token0).balanceOf(FromSushiPoolAddress);
        uint256 balance1 = IERC20(token1).balanceOf(FromSushiPoolAddress);
