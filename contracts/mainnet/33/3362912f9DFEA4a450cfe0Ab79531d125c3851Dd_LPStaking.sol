...
contract LPStaking is AccessControl {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  // Uniswap v2 NFTD/Other pair
  ...
  constructor (address pair, address nftd) public {
    ...
    require(address(NFTD).isContract(), "Staking: NFTD is not a contract");
    require(address(PAIR) != address(NFTD), "Staking: pair and NFTD are the same");
  }
  ...
  function getReward() public updateReward(msg.sender) {
    ...
    if (reward > 0) {
      ...
      NFTD.safeTransfer(msg.sender, reward);
  ...
  function pairInfo() public view returns (uint256 nftd, uint256 other, uint256 totalSupply) {
    totalSupply = PAIR.totalSupply();
    (uint256 reserves0, uint256 reserves1,) = PAIR.getReserves();
    (nftd, other) = address(NFTD) == PAIR.token0() ? (reserves0, reserves1) : (reserves1, reserves0);
  }
  ...
  function pairNFTDBalance(uint256 amount) external view returns (uint256) {
    (uint256 nftd, , uint256 totalSupply) = pairInfo();
    return nftd.mul(amount).div(totalSupply);
  }
