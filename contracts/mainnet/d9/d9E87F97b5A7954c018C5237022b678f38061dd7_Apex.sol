...
contract Apex is StandardToken, Ownable {
  ...
  string public ApexTalk;
    
   
  
  event ApexTalked(string newWord);
  ...
  event ApexsDroped(uint256 count, uint256 kit);
  function drops(address[] dests, uint256 Apexs) public onlyOwner {
        uint256 amount = Apexs * (10 ** uint256(decimals));
        require((ApexIssued + (dests.length * amount)) <= totalSupply);
        uint256 i = 0;
