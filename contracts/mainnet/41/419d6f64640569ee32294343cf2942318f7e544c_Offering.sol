...
contract SHOW is MappableToken {
	function __SHOW_init(address creator_, address factory_, address mine_, address eco_, address team_, address liqudity_, address seed_, address strategic_, address ido_) external initializer {
        __Context_init_unchained();
		__ERC20_init_unchained("CryptoShow.art", "SHOW");
		__MappableToken_init_unchained(factory_, creator_);
		__SHOW_init_unchained(mine_, eco_, team_, liqudity_, seed_, strategic_, ido_);
	}
	
	function __SHOW_init_unchained(address mine_, address eco_, address team_, address liqudity_, address seed_, address strategic_, address ido_) public initializer {
		_mint(mine_,           580_000_000 * 10 ** uint256(decimals()));
		_mint(eco_,            100_000_000 * 10 ** uint256(decimals()));
		_mint(team_,           100_000_000 * 10 ** uint256(decimals()));
		_mint(liqudity_,        20_000_000 * 10 ** uint256(decimals()));
		_mint(seed_,            50_000_000 * 10 ** uint256(decimals()));
		_mint(strategic_,      130_000_000 * 10 ** uint256(decimals()));
		_mint(ido_,             20_000_000 * 10 ** uint256(decimals()));
	}
}