...
/**
 * Implementation of the Forward token
 */
contract ForwardRootToken is ERC20, Pausable, ERC20Permit, Ownable {
    bool public initialized;

    function initialize(address tokenowner) external {
        require(!initialized, "already initialized");
        require(tokenowner != address(0), "Invalid address");
        initializeERC20("Forward", "$FORWARD");
        initializePausable();
        initializeOwnable(tokenowner);
        initializeERC20Permit("Forward");

        _mint(tokenowner, 5000000000 ether); // mint 5 bil $FORWARD tokens
        initialized = true;
    }
    ...
    function refundTokens(IERC20 tokenAddress) external onlyOwner {
