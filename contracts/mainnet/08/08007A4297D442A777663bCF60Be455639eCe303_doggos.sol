...
// 
contract doggos is ERC721Enumerable, Ownable {

    using doggosLibrary for uint8;
    ...
    constructor() ERC721("ocDoggos", "OCD") {
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    function mintInternal() internal {
        ...
        require(!doggosLibrary.isContract(msg.sender));
     ...
     */
    function mintDoggos(uint256 _times) public payable {
        require((_times > 0 && _times <= 10) && (msg.value == _times * price) && (totalSupply() < MAX_SUPPLY), "t");
...
*/
        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) { 
            uint8 thisTraitIndex = doggosLibrary.parseInt(
                doggosLibrary.substring(_hash, i, i + 1)
            );
            ...
            for (
                ...
                string memory thisPixel = doggosLibrary.substring(
                ...
                uint8 x = letterToNumber(
                    doggosLibrary.substring(thisPixel, 0, 1)
                );
                uint8 y = letterToNumber(
                    doggosLibrary.substring(thisPixel, 1, 2)
                );
                ...
                svgString = string(
                    abi.encodePacked(
                        svgString,
                        "<rect class='c",
                        doggosLibrary.substring(thisPixel, 2, 4),
                        "' x='",
                        x.toString(),
                        "' y='",
                        y.toString(),
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    doggosLibrary.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "ocDoggos #',
                                    doggosLibrary.toString(_tokenId),
                                    '", "description": "On-Chain Doggos is a collection of 4,000 uniquely generated Doggos. All the metadata and images are 100% stored on the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
                                    doggosLibrary.encode(
        ...
        returns (string memory)
    {
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
            tokenHash = string(
                abi.encodePacked(
                    "1",
                    doggosLibrary.substring(tokenHash, 1, 9)
                )
            );
        }
