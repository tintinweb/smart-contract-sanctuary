...
import "./interfaces/ICharmingFishes.sol";

contract GeniusDolphins is ERC721Enumerable, Ownable {
  ...
  uint256 public maxSaleSupply = 100;
  uint256 public maxEvolveSupply = 500;
  uint256 public maxMintPerTxn = 5;
  ...
  function publicMint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintPerTxn, "RRC: You can only mint up to 5 per txn");
  ...
  function areOwned(uint256[] memory tokenIds) public view returns (bool out) {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++){
      if (CS.ownerOf(tokenIds[i]) != msg.sender){
        out = false;
      }
    }
  }
