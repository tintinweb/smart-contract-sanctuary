...
contract PartyBonesMinter is Ownable, Pausable{
    ...
    mapping(uint256 => bool) internal eligible_bone;
    constructor () payable {}
    
    mapping(address => bool) public authorised_contracts;
    
    // returns current token minting count
    function check_additionalmintedCount() view public returns (uint256)
    {
        return _mintedTokens.current();
    }
    
    function check_eligible(uint256 tokenid) view public returns(bool)
    {
        return(eligible_bone[tokenid]);
    }
    ...
    function checkRedeem(address address_) public view returns(uint256)
    {
        PartyBones partyBones = PartyBones(addressPartyBones);
        uint256 counter = 0;
        uint256[] memory bones_owned = partyBones.retrieveTokens(address_);
        
        for (uint256 i = 0; i < bones_owned.length; i++){
            if (eligible_bone[bones_owned[i]] == false) {
                counter = counter + 1;
            }
            else{
                continue;
    ...
    function claimBones(uint claim_count) RedemptionOpen public returns (uint256)
    {   
        require(checkRedeem(msg.sender) > 0);
        require(claim_count <= 10);
        uint256 counter = 0;
        PartyBones partyBones = PartyBones(addressPartyBones);
        uint256[] memory bone_eligible_list = partyBones.retrieveTokens(msg.sender);
        for (uint256 i = 0; i < bone_eligible_list.length; i++){
            if ((eligible_bone[bone_eligible_list[i]] == true) || (counter >= claim_count) ){
                continue;
            } 
            else{
                counter = counter + 1;
                
                partyBones.mint_primary_admin(msg.sender, 3);

                uint256[] memory newItemIds = partyBones.retrieveTokens(msg.sender);
                uint256 newItemId1 = newItemIds[newItemIds.length-1];
                eligible_bone[newItemId1] = true;
                uint256 newItemId2 = newItemIds[newItemIds.length-2];
                eligible_bone[newItemId2] = true;
                uint256 newItemId3 = newItemIds[newItemIds.length-3];
                eligible_bone[newItemId3] = true;
                eligible_bone[bone_eligible_list[i]] = true;
                }
            }
        return (counter);
    }
    ...
    // mint your Party Bone!
    function createBone(uint256 numberOfTokens) public payable saleIsOpen whenNotPaused
    {   
        PartyBones partyBones = PartyBones(addressPartyBones);
        require(numberOfTokens < 21, "No more than 20 Tokens at one time!");
        require(_mintedTokens.current() + numberOfTokens <= MAX_Bones, "This exceeds the maximum number of Party Bones on sale!");
        ...
        for (uint256 i = 0; i < numberOfTokens; i++){
            partyBones.mint_primary_admin(msg.sender, 1);
            uint256[] memory newItemIds = partyBones.retrieveTokens(msg.sender);
            uint256 newItemId = newItemIds[newItemIds.length-1];
            eligible_bone[newItemId] = true;
            _mintedTokens.increment();
        }
    }
    ...
    {   
        ...
        require(_mintedTokens.current() + numberOfTokens <= MAX_Bones, "This exceeds the maximum number of Party Bones on sale!");
        for (uint256 i = 0; i < numberOfTokens; i++){
            partyBones.mint_primary_admin(address_, 1);
            uint256[] memory newItemIds = partyBones.retrieveTokens(address_);
            uint256 newItemId = newItemIds[newItemIds.length-1];
            eligible_bone[newItemId] = true;
            _mintedTokens.increment();
        }
