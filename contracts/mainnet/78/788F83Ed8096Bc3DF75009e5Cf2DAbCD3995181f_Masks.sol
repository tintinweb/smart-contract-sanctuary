...
contract Masks is Context, Ownable, ERC165, IMasks, IERC721Metadata {
    ...
    uint256 public constant SALE_START_TIMESTAMP = 1620303554;

    // Time after which hash masks are randomized and allotted
    uint256 public constant REVEAL_TIMESTAMP = SALE_START_TIMESTAMP + (86400 * 3);
    ...
    // Mapping from token ID to whether the Mask was minted before reveal
     ...
     */
    constructor (string memory name, string memory symbol, address nctAddress, address _proxyRegistryAddress) {
        _name = name;
        _symbol = symbol;
        _nctAddress = nctAddress;
        nameChangePrice = NAME_CHANGE_PRICE;
    ...
    function getNFTPrice() public view returns (uint256) {
        ...
        if (currentSupply >= 99) {
            return 10000000000000000000; // 100 10 ETH
        } else if (currentSupply >= 89) {
            return 5000000000000000000; // 89 - 99 0.9 ETH
        } else if (currentSupply >= 74) {
            return 3000000000000000000; // 75 - 89 0.9 ETH
        } else if (currentSupply >= 59) {
            return 2000000000000000000; // 60 - 74 0.9 ETH
        } else if (currentSupply >= 44) {
            return 1000000000000000000; // 45 - 59 0.9 ETH
        } else if (currentSupply >= 29) {
            return 500000000000000000; // 30 - 44 0.5 ETH
        } else if (currentSupply >= 14) {
            return 300000000000000000; // 15 - 29 0.3 ETH 
        } else {
            return 100000000000000000; // 0 - 14 0.1 ETH
        }
    }
    ...
    */
    ...
    /**
     * @dev Changes the name for Mask tokenId
     */
    ...
    */
    ...
    function setNameChangePrice(uint256 _nameChangePrice) onlyOwner public {
        require(_nameChangePrice > 0, "Masks: INVALID NAME CHANGE PRICE");
    ...
    function baseTokenURI() public pure returns (string memory) {
        return "https://api.synodica.art/nft/";
    }
...
/**
 *
 * NameChangeToken Contract 
 * @dev Extends standard ERC20 contract
 */
contract Synodica is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant PRE_REVEAL_MULTIPLIER = 15;
    ...
    /**
     * @dev When accumulated NCTs have last been claimed for a token index
     */
    ...
    /**
     * @dev Accumulated NCT tokens for a token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = IMasks(_masksAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) / 10 : INITIAL_ALLOTMENT;
    ...
    function accumulatedForMultiNFTs(uint256[] memory tokenIndices) public view returns (uint256) {
        uint256 totalAccumulatedForMulti;
        for (uint i = 0; i < tokenIndices.length; i++) {
            uint256 totalAccumulatedForSingle = accumulated(tokenIndices[i]);
            totalAccumulatedForMulti = totalAccumulatedForMulti.add(totalAccumulatedForSingle);
        }
        
        return totalAccumulatedForMulti;
    }
    ...
    /**
     * @dev Claim mints NCTs and supports multiple token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        uint256 claimFee = totalClaimQty.mul(fee) / 1000;
        if (claimFee > 0) {
            _mint(feeTo, claimFee);
        }
        _mint(msg.sender, totalClaimQty - claimFee); 
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the mask contract. For better UX.
