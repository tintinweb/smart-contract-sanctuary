...
contract Strategy is ConvexStable {
    ...
    constructor(address _vault) public ConvexStable(_vault) {
        ...
        _setPathTarget(2, 0); // lqty path target

        dex = new address[](3);
        dex[0] = sushiswap; // crv
        dex[1] = sushiswap; // cvx
        dex[2] = uniswapv3; // lqty
        _approveDex();
    }
    ...
    function _approveDex() internal override {
        super._approveDex();
        IERC20(lqty).approve(dex[2], 0);
        IERC20(lqty).approve(dex[2], type(uint256).max);
    }

    // >>> include other rewards
    function _migrateRewards(address _newStrategy) internal override {
        super._migrateRewards(_newStrategy);
        IERC20(lqty).safeTransfer(_newStrategy, IERC20(lqty).balanceOf(address(this)));
    }
    ...
    function _claimableInETH() internal override view returns (uint256 _claimable) {
        ...
        if (_lqty > 0) {
            address _dex = dex[2];
            if (_dex == uniswapv3) {
                uint256 amountOut = Uni(quoter).quoteExactInput(abi.encodePacked(lqty, uint24(3000), weth), _lqty);
                _claimable = _claimable.add(amountOut);
            }
            else if (_dex == uniswap) {
                address[] memory path = new address[](2);
                path[0] = lqty;
                path[1] = weth;
                uint256[] memory swap = Uni(_dex).getAmountsOut(_lqty, path);
        ...
        )
    {
        ...
        if (_lqty > 0) {
            address _dex = dex[2];
            if (_dex == uniswapv3) {
                Uni(_dex).exactInput(Uni.ExactInputParams(
                    abi.encodePacked(
                        lqty,
                        uint24(3000),
                        weth,
                ...
                Uni(_dex).swapExactTokensForTokens(_lqty, uint256(0), path, address(this), now);
            }
        }
