...
contract METAMILLIONAIRE is ERC721, Ownable {
    ...
    Counters.Counter private _raccoonIdTracker;

    string public PROVENANCE = "";

    uint256 public totalSaleElement;
    uint256 public totalRaccoonElement;
    ...
    constructor() ERC721("METAMILLIONAIRE", "METAMILLIONAIRE") {
        totalSaleElement = 11500; // for public
        totalRaccoonElement = 500;
    ...
    function _totalSupply() internal view returns (uint256) {
        return _tokenIdTracker.current().add(_raccoonIdTracker.current());
    }
     ...
     * Mint An Element
     */
    function _mintAnElement(address _to, bool owner) internal {
        uint256 id = _totalSupply();
        if (owner) {
            _raccoonIdTracker.increment();
        } else {
    ...
    function mintByOwner(address _to, uint256 _amount)
        ...
        checkSaleIsActive
    {
        uint256 totalSupply = _raccoonIdTracker.current();

        require(totalSupply <= totalRaccoonElement, "Presale End");
        require(
            totalSupply + _amount <= totalRaccoonElement,
            "Max Limit To Presale"
        );
        require(_amount <= maxByMint, "Exceeds Amount");

        for (uint256 i = 0; i < _amount; i += 1) {
            _mintAnElement(_to, true);
        }
    }
