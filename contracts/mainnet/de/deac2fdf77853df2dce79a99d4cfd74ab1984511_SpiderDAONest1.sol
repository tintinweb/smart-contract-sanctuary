...
// File: contracts\SpiderDAONest1.sol
...
contract SpiderDAONest1 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant REWARD_PERC = 3; //3 %
    ...
    function canWithdraw(address _addy) public view returns (bool) {
        if (block.timestamp >= deposited[_addy].add(ONE_MONTH)) {
            return true;
        } else {
            return false;
        }
        ...
        _positive(amount)
    {
        ...
        require(
            block.timestamp >= deposited[msg.sender].add(ONE_MONTH),
