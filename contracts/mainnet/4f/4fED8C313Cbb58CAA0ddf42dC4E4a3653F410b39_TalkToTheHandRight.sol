...
contract TalkToTheHandRight is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            ...
            if (allowFreeMint == true && isFreeMinted(msg.sender) == true) {
                if (ownerFreeMintedCount + _mintAmount <= maxFreeMintAmount) {
                    require(msg.value == 0, "Free mints doesn't need amount.");
                } else {
                    require(msg.value >= cost * _mintAmount,"Max Free NFT per address exceeded. Need funds to mint.");
                }
            } else {
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            if (allowFreeMint == true && isFreeMinted(msg.sender) == true && (ownerFreeMintedCount + _mintAmount) <= maxFreeMintAmount) {
    ...
    function withdraw() public payable onlyOwner {
        // This will pay DAO 10% of the initial sale.
        // =============================================================================
        if(daoShare > 0){
