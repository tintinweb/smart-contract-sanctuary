...
contract Strategy is CurveVoterProxy {
    ...
    constructor(address _vault) public CurveVoterProxy(_vault) {
        ...
        gauge = address(0xd662908ADA2Ea1916B3318327A97eB18aD588b5d);
        keepCRV = 1000;
        ...
        )
    {
        ...
        if (_aave > 0) {
            IERC20(aave).safeApprove(sushiswap, 0);
            IERC20(aave).safeApprove(sushiswap, _aave);

            Uni(sushiswap).swapExactTokensForTokens(_aave, uint256(0), pathReward, address(this), now);
        }
