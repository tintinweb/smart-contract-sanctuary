...
contract StudioShibli is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xcF31195B2C7d618A79962438f00D6D2D726A9950); // Marketing Address

    address payable public liquidityAddress =
        payable(0x000000000000000000000000000000000000dEaD); // Liquidity Address
        
    address payable public gameAddress =
        payable(0x29EA5d401615de6eB6321D802EAE4206922FCa79); // Game payout Address
        
    address payable public devAddress =
        payable(0x6358731710985DF37b7a2976a447e41B00FD196C); // Development Address
    ...
    string private constant _name = "Studio Shibli";
    string private constant _symbol = "SHIBLI";
    ...
    uint256 private _gameTokensToSwap;
    
    bool private gasLimitActive = true;
    uint256 private gasPriceLimit = 550 * 1 gwei;
    ...
    constructor() payable {
        ...
        maxTransactionAmount = _tTotal * 25 / 10000; // .25% max txn
    ...
    // only use if there is an issue with Airdrop feature, should be ok though!
    function removeAirdropLimit() external onlyOwner{
        airDropLimitInEffect = false;
    }
    ...
    function _transfer(
        ...
        if(airDropLimitInEffect){ // Check if Limit is in effect
            if(airDropLimitLiftDate <= block.timestamp){
                ...
                if(_isAirdoppedWallet[from] && senderBalance.sub(amount) < _airDropTokensRemaining[from]){
                    ...
                    require(_airDropAddressNextSellDate[from] <= block.timestamp, "_transfer:: Please read the contract for your next sale date.");
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (!automatedMarketMakerPairs[to]) {
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyDevFee + _buyGameFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            } 
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellDevFee + _sellGameFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        ...
        bool success;
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _devTokensToSwap + _marketingTokensToSwap + _gameTokensToSwap;
        ...
        uint256 bnbForGame = bnbBalance.mul(_gameTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForDev = bnbBalance.mul(_devTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance - bnbForMarketing - bnbForGame - bnbForDev;
        ...
        (success,) = address(gameAddress).call{value: bnbForGame}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _gameTokensToSwap += tLiquidity * _buyGameFee / _liquidityFee;
            ...
            _gameTokensToSwap += tLiquidity * _sellGameFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyDevFee, uint256 buyGameFee)
        external
        onlyOwner
    {
        ...
        _buyGameFee = buyGameFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDevFee + _buyGameFee <= 20, "Must keep taxes below 20%");
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellDevFee, uint256 sellGameFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDevFee + _sellGameFee <= 30, "Must keep taxes below 30%");
    }
    ...
    function setGameAddress(address _gameAddress) external onlyOwner {
        gameAddress = payable(_gameAddress);
    }
