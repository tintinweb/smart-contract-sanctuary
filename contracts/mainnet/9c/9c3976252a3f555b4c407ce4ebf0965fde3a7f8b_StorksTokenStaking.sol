...
// File: contracts/StorksTokenstaking
...
contract StorksTokenStaking is Ownable {
    ...
    function stake(uint8 _pid, uint256 _amount) external returns (bool) {
       ...
       _amount = _amount - (_amount.div(20));
    ...
    /**dev can claim stucked tokens or ETH from the smartcontract, if 
      *accidently sent by somone. To claim native tokens, he has to wait
      *atleast 100 days from the deployment of this smartcontract
     **/  

    function claimStuckTokens(address _token) external onlyOwner {
    if (_token == address(this)) {
        uint256 canWithdraw = 90 days;
        require (block.timestamp >= canWithdraw + 10 days);
        }
