...
interface INFTDistributor {
    ...
    event Claimed(uint256 index, address account, uint256 code);
}
...
// File: contracts/simplified/NFTDistributor.sol
...
contract NFTDistributor is INFTDistributor, Ownable {
    address public immutable babelTowerDAC;
    address public immutable metisDAC;
    bytes32 public immutable override merkleRoot;
    uint256 public claimedNumber;
    
    // This is a packed array of booleans.
    mapping(uint256 => uint256) private claimedBitMap;

    constructor(bytes32 merkleRoot_) {
        babelTowerDAC = address(new BabelTowerDAC(msg.sender));
        metisDAC = address(new MetisDAC());
        merkleRoot = merkleRoot_;
    }
    
    function presetRoles() public onlyOwner {
        BabelTowerDAC(babelTowerDAC).transferOwnership(msg.sender);
        MetisDAC(metisDAC).transferOwnership(msg.sender);
        BabelTowerDAC(babelTowerDAC).grantRole(keccak256("MINTER_ROLE"), msg.sender);
        BabelTowerDAC(babelTowerDAC).grantRole(keccak256("MINTER_ROLE"), address(this));
        MetisDAC(metisDAC).grantRole(keccak256("MINTER_ROLE"), address(this));
        BabelTowerDAC(babelTowerDAC).grantRole(bytes32(0x00), msg.sender);
        MetisDAC(metisDAC).grantRole(bytes32(0x00), msg.sender);
    }
    ...
    function claim(
        ...
        require(!isClaimed(index), 'NFTDistributor: Drop already claimed.');

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, code, power));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), 'NFTDistributor: Invalid proof.');

        // Mark it claimed and send the token.
        _setClaimed(index);
        claimedNumber++;
        if (claimedNumber <= 256) {
            require(DAC(babelTowerDAC).claim(account, code, power), 'NFTDistributor: Transfer BabelTowerDAC failed.');
        } else {
            require(DAC(metisDAC).claim(account, code, power), 'NFTDistributor: Transfer MetisDAC failed.');
        }

        emit Claimed(index, account, code);
    }
}