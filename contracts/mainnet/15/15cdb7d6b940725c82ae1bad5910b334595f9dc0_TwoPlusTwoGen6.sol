...
$$ |         $$ |   $$ |            $$ |  $$ |$$   ____|$$ |  $$ |$$ /  $$ |
$$$$$$$$\    \__|   $$$$$$$$\       \$$$$$$  |\$$$$$$$\ $$ |  $$ | $$$$$$  |
\________|          \________|       \______/  \_______|\__|  \__| \______/
****************************************************************************/
...
// File contracts/TwoPlusTwoGen6.sol
...
contract TwoPlusTwoGen6 is ERC721Enumerable, Ownable {

    TPT public TPTGen2; // Subtraction
    TPT public TPTGen3; // Multiplication
    PENCIL public PENCILToken; // Utility Token

    bool public isActive = false;

    uint256 public burnCost = 750 * 10 ** 18;
    uint256 public maxSupply = 1953;
    string private _baseURIPath;

    bool[1953] private _gen2Claimed;
    bool[1953] private _gen3Claimed;

    event Minted(uint256 gen2TokenId, uint256 gen3TokenId, uint256 burnCost, uint256 tokenId);
    event RolledOver(bool status);

    constructor(address gen2, address gen3, address pencil) ERC721("TwoPlusTwoGen6", "TPT6") {
        TPTGen2 = TPT(gen2);
        TPTGen3 = TPT(gen3);
    ...
    function checkClaim(address recipient) external view returns (uint256) {
        ...
        uint256 gen3Count = TPTGen3.balanceOf(recipient);
        require(gen3Count > 0, "Wallet must own a Gen3 NFT");
        ...
        for (uint256 i; i < gen3Count; i++) {
            uint256 tokenId = TPTGen3.tokenOfOwnerByIndex(recipient, i);
            if (_gen3Claimed[tokenId] == true) {
                gen3Claimed++;
            }
        }

        if (gen3Count - gen3Claimed == 0) {
            return 0;
        }

        uint256 max = (gen2Count - gen2Claimed) < (gen3Count - gen3Claimed) ? gen2Count - gen2Claimed : gen3Count - gen3Claimed;
    ...
    function claimForOwner(address recipient) external onlyOwner {
        ...
        uint256 gen2Count = TPTGen2.balanceOf(recipient);
        uint256 gen3Count = TPTGen3.balanceOf(recipient);
        require(gen2Count > 0 && gen3Count > 0, "Recipient has no unclaimed NFTs available");

        uint256[] memory gen3TokenIds = new uint256[](gen3Count);
        uint256 gen3TokenIdsLength;

        for (uint256 i; i < gen3Count; i++) {
            uint256 gen3TokenId = TPTGen3.tokenOfOwnerByIndex(recipient, i);

            if (_gen3Claimed[gen3TokenId] == false) {
                gen3TokenIds[gen3TokenIdsLength] = gen3TokenId;
                gen3TokenIdsLength++;
            }
        }

        require(gen3TokenIdsLength > 0, "No unclaimed Gen3 NFTs available");
        uint256 mintIndex;

        for (uint256 i; i < gen2Count; i++) {
            uint256 gen2TokenId = TPTGen2.tokenOfOwnerByIndex(recipient, i);

            if (_gen2Claimed[gen2TokenId] == false && gen3TokenIdsLength > 0) {
                gen3TokenIdsLength--;
                mintIndex = totalSupply();
                _mint(recipient, mintIndex);
                _gen2Claimed[gen2TokenId] = true;
                _gen3Claimed[gen3TokenIds[gen3TokenIdsLength]] = true;
                emit Minted(gen2TokenId, gen3TokenIds[gen3TokenIdsLength], 0, mintIndex);
            }

            if (gen3TokenIdsLength == 0) {
                break;
            }
        }
    }

    // Flip the mint state active/inactive
    ...
    function mintTokens(uint256 limit) public {
        ...
        uint256 gen3Count = TPTGen3.balanceOf(msg.sender);
        require(gen3Count > 0, "Wallet must own a Gen3 NFT");
        ...
        for (uint256 i; i < gen3Count; i++) {
            ...
            uint256 gen3TokenId = TPTGen3.tokenOfOwnerByIndex(msg.sender, i);
        ...
        for (uint256 i; i < gen2Count; i++) {
            ...
            if (_gen2Claimed[gen2TokenId] == false && gen3TokenIdsLength > 0) {
                ...
                emit Minted(gen2TokenId, gen3TokenIds[gen3TokenIdsLength], burnCost, mintIndex);
            }

            if (minted == limit || gen3TokenIdsLength == 0) {
                break;
            }
        }

        require(minted > 0, "No unclaimed Gen2 NFTs available");
    }
