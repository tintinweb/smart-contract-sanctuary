...
// Dependency file: contracts/GainswapERC20.sol

// pragma solidity =0.6.12;

// import 'contracts/libraries/SafeMath.sol';

contract GainswapERC20 {
    using SafeMathGainswap for uint;

    string public constant name = 'Gainswap LP Token';
    string public constant symbol = 'Gainswap_LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'Gainswap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'Gainswap: INVALID_SIGNATURE');
...
// Dependency file: contracts/interfaces/IGainswapCallee.sol

// pragma solidity =0.6.12;

interface IGainswapCallee {
    function GainswapCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}


// Dependency file: contracts/GainswapPair.sol

// pragma solidity =0.6.12;

// import 'contracts/GainswapERC20.sol';
// import 'contracts/libraries/Math.sol';
// import 'contracts/libraries/UQ112x112.sol';
// import 'contracts/interfaces/IERC20.sol';
// import 'contracts/interfaces/IyToken.sol';
// import 'contracts/interfaces/IGainswapFactory.sol';
// import 'contracts/interfaces/IGainswapCallee.sol';

contract GainswapPair is GainswapERC20 {
    using SafeMathGainswap for uint;
    ...
    uint8 public fee = 3;
    uint private tmptmp = 1;
    uint private unlocked = 1;
    modifier lock() {
        require(unlocked == 1, 'Gainswap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function owner() public view returns (address) {
        return IGainswapFactory(factory).feeTo();
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'Gainswap: OVERFLOW');
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'GainswapV2: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
        _reserve0 += dummy0;
        _reserve1 += dummy1;
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'Gainswap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'Gainswap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'Gainswap: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'Gainswap: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IGainswapCallee(to).GainswapCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = b0();
        balance1 = b1();
        }
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'Gainswap: INSUFFICIENT_INPUT_AMOUNT');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'Gainswap: K');
        }
...
// Root file: contracts/GainswapFactory.sol

pragma solidity =0.6.12;

// import 'contracts/interfaces/IGainswapFactory.sol';
// import 'contracts/GainswapPair.sol';

contract GainswapFactory is IGainswapFactory {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(GainswapPair).creationCode);
    }

    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, 'Gainswap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Gainswap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Gainswap: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(GainswapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        GainswapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external override {
        require(msg.sender == feeToSetter, 'Gainswap: FORBIDDEN');
        feeTo = _feeTo;
    }
