...
contract Poseidon is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MINT_PUBLIC = 10000;
    ...
    function setProvenance(string memory _provenance) external onlyOwner {
        require(bytes(provenance).length == 0, "PROVENANCE_ALREADY_SET");
    ...
    function maxLevel(address _address) public view returns (uint256) {
        uint256 _maxLevel = 0;
    ...
    // View account total level
    function totalLevel(address _address) public view returns (uint256) {
        uint256 _totalLevel = 0;
        uint256 _balanceOfAddress = balanceOf(_address);
        for (uint256 i = 0; i < _balanceOfAddress; i++) {
            uint256 _tokenId = tokenOfOwnerByIndex(_address, i);
            _totalLevel += level(_tokenId);
        }
        return _totalLevel;
    }
}