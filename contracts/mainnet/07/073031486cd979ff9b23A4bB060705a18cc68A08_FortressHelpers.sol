...
contract FortressHelpers is ERC721Holder {
	...
	function cost() public view returns (uint128) {
		(etherFreakers.middlePrice() * 1005 / 1000) + 1 + fortress.joinFeeWei();
	}
...
contract EtherFreakers is ERC721 {
     ...
     */
    function discharge(uint128 freakerId, uint128 amount) external {
        address owner = ownerOf(freakerId);
        require(msg.sender == owner, "Sender does not own freaker");
        require(isFreaker(freakerId), "Not a freaker");
