...
contract VestingTeam is Ownable {
    ...
    uint256 private constant CLAIM_DELAY = 90 days; // 3months - 1 quarter
    uint256 private constant RELEASEAMOUNT = 600000 ether;
    ...
    IERC20 private immutable kstToken;
    event Claimed(uint256 amount, uint256 epoch);

    constructor(address _kstTokenAddress, uint256 _startTime) {
        kstToken = IERC20(_kstTokenAddress);
        startTime = _startTime; //tge +1 year
        END_DAY = _startTime.add(450 days); // 5 quarters
    }
    ...
    function claimTokens() external {
        ...
        if (block.timestamp > END_DAY) {
            ...
            if (currentEpoch > epoch) {
                uint256 multiplier = 1+ currentEpoch.sub(epoch);
                kstToken.safeTransfer(
                    beneficiary,
                    RELEASEAMOUNT.mul(multiplier)
                );
                epoch = epoch.add(multiplier);
            } else {
                kstToken.safeTransfer(beneficiary, RELEASEAMOUNT);
                epoch = epoch.add(1);
            }
    ...
    function claimDust() public {
        ...
        kstToken.safeTransfer(
            beneficiary,
            kstToken.balanceOf(address(this))
        );
    }

    function setBeneficiary(address _addy) external onlyOwner {
