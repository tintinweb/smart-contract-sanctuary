...
interface IStaking:
    def deposit(_pid: uint256, _amount: uint256, _recipient: address) -> uint256: nonpayable
...
# For deposit&stake shorthand
staking: public(address)
poolId: public(uint256)

# For contract whitelist during "Contract Invocation Lockdown"
contractLockdown: public(bool)
contractWhitelist: public(address)
...
def initialize(
    ...
    if nameOverride == "":
        self.name = concat(DetailedERC20(token).symbol(), " sVault")
    else:
        self.name = nameOverride
    if symbolOverride == "":
        self.symbol = concat("sv", DetailedERC20(token).symbol())
    else:
...
@external
def setStaking(staking: address, poolId: uint256):
    """
    @notice
        Used to change the address of `staking`.

        This may only be called by governance.
    @param staking The new staking address to use.
    @param poolId The new poolId address to use.
    """
    assert msg.sender == self.governance
    if self.staking != ZERO_ADDRESS:
        # Revoke allowance due to staking contract changed
        ERC20(self).approve(self.staking, 0)

    self.staking = staking
    self.poolId = poolId
    ERC20(self).approve(staking, 0)
    ERC20(self).approve(staking, MAX_UINT256)


@external
def setContractLockdown(lockdown: bool):
    """
    @notice
        During lock down, contract can't call deposit of this vault

        This may only be called by governance.
    @param lockdown Whether to enable lockdown.
    """
    assert msg.sender == self.governance
    self.contractLockdown = lockdown


@external
def setContractWhitelist(contract: address):
    """
    @notice
        During lock down, this contract can still invoke deposit function,
            normally this should be address of WETHGateway

        This may only be called by governance.
    @param contract Address to whitelist.
    """
    assert msg.sender == self.governance
    self.contractWhitelist = contract
...
def deposit(_amount: uint256 = MAX_UINT256, recipient: address = msg.sender, stakeInstantly: bool = False) -> uint256:
    """
    @notice
    ...
    # During lockdown, all contract except contractWhitelist can't deposit
    if self.contractLockdown and msg.sender.is_contract:
        assert msg.sender == self.contractWhitelist
    ...
    # See @dev note, above.
    shares: uint256 = 0
    if not stakeInstantly:
        shares = self._issueSharesForAmount(recipient, amount)

        # Tokens are transferred from msg.sender (may be different from _recipient)
        self.erc20_safe_transferFrom(self.token.address, msg.sender, self, amount)

    else:
        # Ensure we do have staking contract
        assert self.staking != ZERO_ADDRESS
        shares = self._issueSharesForAmount(self, amount)

        # Tokens are transferred from msg.sender (may be different from _recipient)
        self.erc20_safe_transferFrom(self.token.address, msg.sender, self, amount)
        # Stake shares to Staking contract
        IStaking(self.staking).deposit(self.poolId, shares, recipient)
...
@external
def avgAPY() -> uint256:
    yearlyProfit: uint256 = 0
    for strategy in self.withdrawalQueue:
        if strategy == ZERO_ADDRESS:
            break
        params: StrategyParams = self.strategies[strategy] 
        totalHarvestTime: uint256 = params.lastReport - params.activation
        if totalHarvestTime > 0:
            yearlyProfit = yearlyProfit + (params.totalGain * SECS_PER_YEAR / totalHarvestTime)

    return yearlyProfit * convert(10000, uint256) / self._totalAssets()
