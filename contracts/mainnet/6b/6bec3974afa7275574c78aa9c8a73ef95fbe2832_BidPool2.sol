...
contract SwapETHOptimal is ReentrancyGuard{
    ...
    address internal bid;

    uint256 public receivedLPtoken;

    constructor(address _bid) public {
        bid=_bid;
        weth = router.WETH();
        ...
        nonReentrant
    {
        ...
        uint256 left=0;
        {
            ...
            //emit  DebugString(path[0], path[1], swapAmt,"swap path");
            uint[] memory am=router.swapExactTokensForTokens(swapAmt, 0, path, address(this), now);
            uint256 nbid=am[1];
            
             //emit  DebugString(msg.sender, bid, nbid,"swap bid ammount");
            ...
            //emit  DebugString(msg.sender, bid, left,"left bid");
            //emit  DebugString(msg.sender, weth, IERC20(weth).balanceOf(address(this)),"left weth");
    ...
    function getWethReserves() external view returns(uint256) {
              
        (uint256 token0Reserve, uint256 token1Reserve,) = ISwapPair(lptoken).getReserves();
        (uint256 wethReserve, ) = weth ==
...
contract LPTokenWrapper  {
    ...
    function addStake(uint256 amount) internal {
...
contract BidPool2 is LPTokenWrapper, Ownable,SwapETHOptimal{
    
    address public constant bidaddress=0x00000000000045166C45aF0FC6E4Cf31D9E14B9A;
    IERC20 public bidtoken = IERC20(bidaddress);
    ...
    constructor() SwapETHOptimal(bidaddress) public {
        lpt=lptoken;
    }
    ...
    function stakePool2(uint256 amount) public updateReward(msg.sender) {
    ...
    function autoStakeETHPool2() external payable updateReward(msg.sender){
        doETH2UniswapStake();
        
        super.addStake(receivedLPtoken);
        emit Staked(msg.sender, receivedLPtoken);
    }
    
    function () external
     payable updateReward(msg.sender){
        doETH2UniswapStake();
    ...
    function withdrawPool2(uint256 amount) public updateReward(msg.sender) {
        require(amount > 0, "Cannot withdraw 0");
        super.withdraw(amount);
        emit Withdrawn(msg.sender, amount);
    }
    
    function withdrawAndClaimPool2(uint256 _amount) external {
        withdrawPool2(_amount);
        getRewardBid();
    }

    function exitPool2() external {
        withdrawPool2(balanceOf(msg.sender));
        getRewardBid();
    }

    function getRewardBid() public updateReward(msg.sender) {
        uint256 reward = earned(msg.sender);
        if (reward > 0) {
            rewards[msg.sender] = 0;
            bidtoken.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    function notifyBidRewardAmount(uint256 reward)
        external
        onlyOwner
