...
contract Staking is IStaking, Initializable, AccessControlUpgradeable {
    ...
    function unstakeInternalCommon(
        ...
        if (stakingDays >= basePeriod) {
            ...
            uint256 bpdAmount = calcBPD(start, actualEnd, shares);
    ...
    function setBPDPools(
        uint96[5] calldata poolAmount,
        uint96[5] calldata poolShares
    ...
    function addBPDMaxShares(
        ...
        for (uint16 i = oldBpdInterval[0]; i < newBpdInterval[1]; i++) {
            ...
            bpd.shares[i] += uint96(shares); // we only do integer shares, no decimals
        }
    }
        ...
        returns (uint256)
    {
        ...
        for (uint16 i = bpdInterval[0]; i < bpdInterval[1]; i++) {
            bpdAmount += shares1e18.div(bpd.shares[i]).mul(bpd.pool[i]);
        }
    ...
    function getBPD()
        external
        view
        returns (uint96[5] memory, uint96[5] memory)
    {
        return (bpd.pool, bpd.shares);
    }
}
