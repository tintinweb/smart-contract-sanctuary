...
contract SakeLockV2 is ERC20("SakeLockToken", "SakeLock"), Ownable {
    using SafeMath for uint256;
    using Address for address;

    SakeToken public sake;
    SakeMasterV2 public sakeMasterV2;
    address public withDrawAddr;

    constructor(
        SakeToken _sake,
        SakeMasterV2 _sakeMasterV2,
        address _withDrawAddr
    ) public {
        require(address(_sake) != address(0) && address(_sakeMasterV2) != address(0), "invalid address");
        sake = _sake;
        sakeMasterV2 = _sakeMasterV2;
    ...
    function deposit(uint256 _pid) public onlyOwner {
        _approve(address(this), address(sakeMasterV2), 1);
        sakeMasterV2.deposit(_pid, 1, 0);
    }

    function withdrawFromSakeMaster(uint256 _pid) public {
        sakeMasterV2.deposit(_pid, 0, 0);
    }
