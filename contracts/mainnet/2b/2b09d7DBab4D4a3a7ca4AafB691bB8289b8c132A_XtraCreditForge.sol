    ...
    ICreatorExtensionTokenURI
{
    ...
    string private _endpoint =
        "https://client-metadata.ether.cards/api/aoki/XtraCredit/";

    uint256 public forge_start = 1636732800; // GMT: Friday, November 12, 2021 4:00:00 PM PDT 9 AM.
    ...
    */
    function checkClaim(uint256 _tokenID) public view returns (bool) {
        //
        bool checkRange = (384 <= _tokenID && _tokenID <= 423);
        if (!checkRange) {
            // not in range.
            return false;
        } else {
    ...
    function forge(
        uint16 _checkToken, // InternalConflict
        uint16 _checkToken2, // LoveMeLoveMeNot
        uint16 _burnToken //  DistortedReality
    ) public forgeActive() {
        // Attempt Burn
        // Check that we can burn
        require(384 <= _checkToken && _checkToken <= 403, "!I");
        require(404 <= _checkToken2 && _checkToken2 <= 423, "!L");
