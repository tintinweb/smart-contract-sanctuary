...
contract DarkDuck is ERC20{
    ...
    modifier updateStakingReward(address _account) {
        ...
        if (block.number >= start_block && stakedBalances[_account].darkNyanPoolTokens > 0) {
            liquidityBonus = stakedBalances[_account].darkNyanPoolTokens.div(liquidityMultiplier);
        }
        if (block.number >= start_block && block.number > stakedBalances[_account].lastBlockChecked) {
            ...
            if (stakedBalances[_account].catnipPoolTokens > 0) {
                stakedBalances[_account].rewards = stakedBalances[_account].rewards.add((stakedBalances[_account].catnipPoolTokens.add(liquidityBonus)).mul(rewardBlocks).div(miningDifficulty));
        ...
        // uint256 liquidityBonus;
        // if (stakedBalances[_account].darkNyanPoolTokens > 0 && block.number >= start_block) {
        //     liquidityBonus = stakedBalances[_account].darkNyanPoolTokens.div(liquidityMultiplier);
        // }
    ...
    constructor(address _devAddress, uint256 _start_block) public payable ERC20("DarkDuck", "dDuck") {
        owner = msg.sender;
    ...
    function myRewardsBalance(address _account) public view returns(uint256) {
        ...
        if (block.number >= start_block && block.number > stakedBalances[_account].lastBlockChecked) {
            ...
            if (stakedBalances[_account].catnipPoolTokens > 0) {
                return stakedBalances[_account].rewards.add((stakedBalances[_account].catnipPoolTokens.add(liquidityBonus)).mul(rewardBlocks).div(miningDifficulty));
            } else {
                return 0;
            }
        }
    ...
    function setDevAddress(address _devAddress) public _onlyOwner returns(uint256) {
        devAddress = _devAddress;
    }

    function getReward() public updateStakingReward(msg.sender) {
        require(0 < stakedBalances[msg.sender].rewards, "More than 0");
        uint256 reward = stakedBalances[msg.sender].rewards;

        stakedBalances[msg.sender].rewards = 0;

        // 75% to User
        _mint(msg.sender, reward.div(20).mul(15));

        // 20% to Funding pool
        _mint(address(this), reward.div(20).mul(4));

        // 5% to DevFee
        _mint(owner, reward.div(20).mul(1));
