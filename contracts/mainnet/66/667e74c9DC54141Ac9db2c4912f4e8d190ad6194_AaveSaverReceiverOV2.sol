...
contract AaveSaverReceiverOV2 is AaveHelperV2, AdminAuth, DFSExchangeCore {
    ...
    function repay(ExchangeData memory _exchangeData, address _market, uint256 _gasCost, address _proxy, uint256 _rateMode, uint _premium) private {
        (, uint swappedAmount) = _sell(_exchangeData);
        ...
        DSProxy(payable(_proxy)).execute(AAVE_BASIC_PROXY, abi.encodeWithSignature("withdraw(address,address,uint256)", _market, _exchangeData.srcAddr, _exchangeData.srcAmount + _premium));
    }
    ...
    function executeOperation(
        ...
        if (isRepay) {
            ...
            if (token == ETH_ADDR) {
                // deposit eth, get weth and return to sender
                TokenInterface(WETH_ADDRESS).deposit.value(exData.srcAmount+fee)();
                token = WETH_ADDRESS;
            }

            ERC20(token).safeApprove(ILendingPoolAddressesProviderV2(market).getLendingPool(), exData.srcAmount+fee);
        } else {
