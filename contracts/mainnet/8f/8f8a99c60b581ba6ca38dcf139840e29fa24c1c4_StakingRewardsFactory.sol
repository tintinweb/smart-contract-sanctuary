...
contract StakingRewards is Ownable, IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
   ...
   // for vetting the staking pool and or unknown project.
    ...
    function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external payable nonReentrant updateReward(msg.sender) {
        uint256 collectionAddfee = 0.045 ether;  
        ...
        emit StakeTimeUpdated(msg.sender, depositTime[msg.sender]);
    ...
    function stake(uint256 amount) external payable nonReentrant updateReward(msg.sender) {
        uint256 collectionAddfee = 0.0045 ether;  
        require(amount > 0, "Cannot stake 0");
        depositTime[msg.sender] = block.timestamp;
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        require(reward > 0, "stake address not found");
        if (block.timestamp > depositTime[msg.sender] + minStakingTime || block.timestamp > periodFinish) {
            depositTime[msg.sender] = block.timestamp;
    ...
    event StakeTimeUpdated( address indexed user, uint256 depositTime);
...
contract StakingRewardsFactory is Ownable, ReentrancyGuard{
    ...
    function deploy(address stakingToken, address payable collectionAddress, uint256 rewardAmount, uint256 rewardsDuration, uint256 minStakeingTime, uint256 id ) public onlyOwner {
     ...
     // Withdraw any addition tokens not used from factory
    function withdrawRewardToken (uint256 amount) public onlyOwner returns (uint256) {
    address OwnerAddress = owner();  
    if (OwnerAddress == msg.sender)    
     IERC20(rewardsToken).transfer(OwnerAddress, amount);
     return amount;
      ...
      // Withdraw extra tokens when finished 
    function withdrawRewardTokenFromPool (uint256 id, uint256 amount) public onlyOwner returns (uint256) {
     address OwnerAddress = owner(); 
     StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[id];
    if (OwnerAddress == msg.sender)
     StakingRewards(info.stakingRewards).withdrawRewardToken(amount);
     return amount;
    }
