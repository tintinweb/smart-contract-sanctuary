...
contract FancyStakingPool is BasePool, IFancyStakingPool {
    ...
    ) BasePool(_name, _symbol, _depositToken, _liquidityMiningManager, _escrowPool, _escrowDuration) {
        require(
            _maxLockDuration >= MIN_LOCK_DURATION,
            "FancyStakingPool.constructor: max lock duration must be greater or equal to mininmum lock duration"
        );
        maxBonus = _maxBonus;
    ...
    function deposit(
        ...
        require(_amount > 0, "FancyStakingPool.deposit: cannot deposit 0");
    ...
    function withdraw(uint256 _depositId, address _receiver) external {
        require(_depositId < depositsOf[_msgSender()].length, "FancyStakingPool.withdraw: Deposit does not exist");
        Deposit memory userDeposit = depositsOf[_msgSender()][_depositId];
        require(block.timestamp >= userDeposit.end, "FancyStakingPool.withdraw: too soon");
