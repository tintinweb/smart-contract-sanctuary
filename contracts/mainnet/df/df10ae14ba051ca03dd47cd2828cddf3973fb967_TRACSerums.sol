...
contract TRACSerums is ERC1155Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function burnBatch(uint16 skims, uint16 ones, uint16 twos, uint16 wholes, uint16 mutants) external {
    require(tx.origin == trac, "unauthorized");

    uint256[] memory ids; ids[1] = 1; ids[2] = 2; ids[3] = 3; ids[4] = 4;
    uint256[] memory amounts;

    if (skims > 0) {
      require(balanceOf(msg.sender, SKIM_MILK_ID) >= skims, "insufficient skim balance");
      amounts[SKIM_MILK_ID] = skims;
      supply[SKIM_MILK_ID] -= uint16(skims);
    }
    if (ones > 0) {
      require(balanceOf(msg.sender, ONE_PERCENT_MILK_ID) >= ones, "insufficient ones balance");
      amounts[ONE_PERCENT_MILK_ID] = ones;
      supply[ONE_PERCENT_MILK_ID] -= uint16(ones);
    }
    if (twos > 0) {
      require(balanceOf(msg.sender, TWO_PERCENT_MILK_ID) >= twos, "insufficient twos balance");
      amounts[TWO_PERCENT_MILK_ID] = twos;
      supply[TWO_PERCENT_MILK_ID] -= uint16(twos);
    }
    if (wholes > 0) {
      require(balanceOf(msg.sender, WHOLE_MILK_ID) >= wholes, "insufficient wholes balance");
      amounts[WHOLE_MILK_ID] = wholes;
      supply[WHOLE_MILK_ID] -= uint16(wholes);
    }
    if (mutants > 0) {
      require(balanceOf(msg.sender, MUTANT_MILK_ID) >= mutants, "insufficient mutant balance");
      amounts[MUTANT_MILK_ID] = mutants;
      supply[MUTANT_MILK_ID] -= uint16(mutants);
    }
