...
contract MongooseMoon is Context, IERC20, Ownable {
    ...
    string private _name = "Mongoose Moon";
    ...
    bool inSwapAndWithdraw;
    bool public swapAndWithdrawEnabled = true;

    uint256 public maxTxAmount = 500000000000 * 10**_decimals;
    uint256 public numTokensToSwap = 100000000 * 10**_decimals;

    event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
    event SwapAndWithdrawEnabledUpdated(bool enabled);

    modifier lockTheSwap() {
        inSwapAndWithdraw = true;
        _;
        inSwapAndWithdraw = false;
    }
    ...
    function setReflectionFeePercent(uint256 _reflectionFee) external onlyOperator {
        reflectionFee = _reflectionFee;
    }

    function setTxFeePercent(uint256 _txFee) external onlyOperator {
        require(_txFee <= PERCENT_BP, "!max tx fee");
        txFee = _txFee;
    }
    ...
    function setSwapAndWithdrawEnabled(bool _enabled) public onlyOperator {
        swapAndWithdrawEnabled = _enabled;
        emit SwapAndWithdrawEnabledUpdated(_enabled);
    }
    ...
    function calculateTxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(txFee).div(PERCENT_BP);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            !inSwapAndWithdraw &&
            from != uniswapV2Pair &&
            swapAndWithdrawEnabled
        ) {
            contractTokenBalance = numTokensToSwap;
            //withdraw
            swapAndWithdraw(contractTokenBalance);
        }
    ...
    function swapAndWithdraw(uint256 contractTokenBalance) private lockTheSwap {
        // swap tokens for ETH
    ...
    function setFeeRecipient(address _recipient) external {
        require(feeRecipient == msg.sender, "!fee recipient");
        feeRecipient = _recipient;
    }

    function updateUniRouter(address _router) external onlyOperator {
	...
	function removeFromBlackList(address _addr) public onlyOperator {
        blackList[_addr] = false;
    }

    function addToBlackList(address _addr) public onlyOperator {
        blackList[_addr] = true;
    }

	function setMinimunTokenAmountToSwap(uint _amount) external onlyOperator {
		numTokensToSwap = _amount;
	}

    function setOperator(address _user) external onlyOperator {
        _operator = _user;
    }
}