...
contract Kyubi is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Kyūbi";
    string private constant _symbol = "9TAILS";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                if(firstsell[from] + (2.5 hours) < block.timestamp){
                    sellnumber[from] = 0;
                }
                if (sellnumber[from] == 0) {
                    ...
                    sellnumber[from]++;
                    sellcooldown[from] = firstsell[from] + (2.5 hours);
                }
