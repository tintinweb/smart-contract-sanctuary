...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    function getSecondsLeftInLiquidityGenerationEvent() public view returns (uint256) {
        ...
        console.log("12 hours since start is", contractStartTimestamp.add(12 hours), "Time now is", block.timestamp);
        return contractStartTimestamp.add(12 hours).sub(block.timestamp);
    }

    function liquidityGenerationOngoing() public view returns (bool) {
        console.log("12 hours since start is", contractStartTimestamp.add(12 hours), "Time now is", block.timestamp);
        console.log("liquidity generation ongoing", contractStartTimestamp.add(12 hours) < block.timestamp);
        return contractStartTimestamp.add(12 hours) > block.timestamp;
    }
    ...
    function emergencyDrain24hAfterLiquidityGenerationEventIsDone() public onlyOwner {
        require(contractStartTimestamp.add(13 hours) < block.timestamp, "Liquidity generation grace period still ongoing"); // About 24h after liquidity generation happens        
        // (bool success, ) = msg.sender.call.value(address(this).balance)("");
        // require(success, "Transfer failed.");
