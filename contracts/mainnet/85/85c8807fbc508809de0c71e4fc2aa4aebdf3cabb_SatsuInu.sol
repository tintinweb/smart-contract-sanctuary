...
contract SatsuInu is Context, IERC20, Ownable {
    ...
    string private constant _name = "Satsu Inu";
    string private constant _symbol = "SATSU";
    ...
    constructor () {
        _marketingFeeAddr = payable(0xDaB5dc22350f9a6Aff03Cf3D9341aAD0ba42d2a6);
        _devFeeAddr = payable(0x00000000029f7A89722504d452353F8f58462062);
    ...
    function withdrawStuckTokens(address _contract, uint256 _amount) external onlyOwner() {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isBuy(from)) {
            // 25% tax for selling under 24 hours
            if (_buyMap[from] != 0 &&
        ...
        if (from != owner() && to != owner()) {
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _excludedFromFee[to] && cooldownEnabled) {
                // Standard 30 second cooldown in-between buys
    ...
    function withdraw(uint256 amount) public onlyOwner {
        ...
        _devFeeAddr.transfer(address(this).balance);
    }
