...
contract AngryPitbull is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0xdE2B893b402507d57B8145af222B58f4Df545c58);
    address payable private _DevAddress;

    uint256 public APC_MAX = 10000;
    uint256 public PRESALE_PRICE = 70_000_000_000_000_000; // 0.07 ETH
    uint256 public PUBLIC_PRICE = 70_000_000_000_000_000; // 0.07 ETH
    uint256 private DEV_FEE = 3;
    ...
    uint256 private _activeDateTime = 1640224800; // Date and time (GMT): Thursday, December 23, 2021 2:00:00 AM
    
    string private _tokenBaseURI = "";
    
    Counters.Counter private _publicAPC;

    constructor() ERC721("Angry Pitbull Club", "APC") {
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicAPC.current();

            if (_publicAPC.current() < APC_MAX) {
                _publicAPC.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicAPC.current() < APC_MAX,"Purchase would exceed APC_MAX");
