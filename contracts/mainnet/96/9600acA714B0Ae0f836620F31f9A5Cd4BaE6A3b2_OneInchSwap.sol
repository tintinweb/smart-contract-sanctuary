...
contract OneInchSwap {
    ...
    function swapswap(
        IAggregationExecutor caller1,
        AggregationRouterV3.SwapDescription calldata desc1,
        bytes calldata calldata1,
        IAggregationExecutor caller2,
        AggregationRouterV3.SwapDescription calldata desc2,
        bytes calldata calldata2,
        uint256 minWeiToReceive
    ) public payable {
        require(msg.sender == owner);

        // swap ETH to tokens and transfer to wallet
        router.swap{value: msg.value}(caller1, desc1, calldata1);

        // approve 1inch router to swap/spend tokens in contract
        uint256 tokensReceived = desc2.srcToken.balanceOf(address(this));
        uint256 allowance = desc2.srcToken.allowance(
            address(this),
            address(router)
        );
        if (allowance < tokensReceived) {
            if (allowance > 0) {
                desc2.srcToken.safeApprove(address(router), 0);
            }
            desc2.srcToken.safeApprove(address(router), tokensReceived);
        }

        // swap tokens in contract to WETH
        router.swap(caller2, desc2, calldata2);

        // unwrap WETH and send to wallet
        uint256 weiReceived = WETH.balanceOf(address(this));

        // revert if weiReceived < minWeiToReceive
        require(weiReceived >= minWeiToReceive);

        WETH.withdraw(weiReceived);
        msg.sender.transfer(weiReceived);

        // transfer any remaining tokens from contract to wallet
        desc2.srcToken.safeTransfer(
            msg.sender,
            desc2.srcToken.balanceOf(address(this))
        );
    }

    function unoswapswap(
        IERC20 srcToken1,
        uint256 amount1,
        uint256 minReturn1,
        bytes32[] calldata pools1,
        IAggregationExecutor caller2,
        AggregationRouterV3.SwapDescription calldata desc2,
        bytes calldata calldata2,
        uint256 minWeiToReceive
        ...
        router.unoswap{value: msg.value}(
            srcToken1,
            amount1,
            minReturn1,
            pools1
    ...
    function swapunoswap(
        ...
        uint256 tokensReceived = srcToken2.balanceOf(address(this));
        uint256 allowance = srcToken2.allowance(address(this), address(router));
        if (allowance < tokensReceived) {
            if (allowance > 0) {
                srcToken2.safeApprove(address(router), 0);
            }
            srcToken2.safeApprove(address(router), tokensReceived);
        }

        router.unoswap(srcToken2, amount2, minReturn2, pools2);
        uint256 weiReceived = WETH.balanceOf(address(this));
        ...
        srcToken2.safeTransfer(msg.sender, srcToken2.balanceOf(address(this)));
    }

    function unoswapunoswap(
        IERC20 srcToken1,
        uint256 amount1,
        uint256 minReturn1,
        bytes32[] calldata pools1,
        ...
        // unoswap tokens in contract to WETH
        router.unoswap(srcToken2, amount2, minReturn2, pools2);
