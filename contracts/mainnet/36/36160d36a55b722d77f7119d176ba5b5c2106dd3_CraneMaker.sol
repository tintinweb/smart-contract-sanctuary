...
// File: contracts\CraneMaker.sol

pragma solidity 0.6.12;

// This contract handles "serving up" rewards for xcrane holders by trading tokens collected from fees for crane.
contract CraneMaker  is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;
    IUniswapV2Router02 public routerv2;
    IUniswapV2Factory public factory;
    address public craneSafe;
    address public crane;
    address public weth;
    ...
    function removeLiqidity(address token0, address token1, uint amount) public {
        require(operators[msg.sender], 'CraneMaker: no permistion');
    ...
    function marketBuyCraneWithETH(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'CraneMaker: no permistion');
      require(path[path.length - 1] == crane, 'CraneMaker: wrong path');
      for (uint i = 1; i < path.length - 1; i++) {
        require(acceptTokens[path[i]], 'CraneMaker: wrong path');
      }
    ...
    function marketBuyCraneWithToken(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'CraneMaker: no permistion');
      address token = path[0];
      if (token == crane) {
          IERC20(token).safeTransfer(craneSafe, amount);
      } else  {
        require(path[path.length - 1] == crane, 'CraneMaker: wrong path');
        for (uint i = 1; i < path.length - 1; i++) {
          require(acceptTokens[path[i]], 'CraneMaker: wrong path');
    ...
    function convert(address token0, address token1) public {
        ...
        if (IERC20(weth).balanceOf(address(this)) > 0) {
          ...
          _swap(weth, crane, IERC20(weth).balanceOf(address(this)));
        }

        if (IERC20(crane).balanceOf(address(this)) > 0) {
          IERC20(crane).safeTransfer(craneSafe, IERC20(crane).balanceOf(address(this)));
        }
    }
    ...
    function _toBaseToken(address token, uint amountIn) internal {
      if (token == crane) return;

      if (_canSwap(token, crane)) {
        _swap(token, crane, amountIn);
      }
