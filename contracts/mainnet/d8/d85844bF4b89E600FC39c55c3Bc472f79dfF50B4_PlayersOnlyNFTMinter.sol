...
contract PlayersOnlyNFTMinter is Ownable, ReentrancyGuard {
    ...
    function publicMint(address _toAddress, uint256 amount) external payable nonReentrant {
        require(amount <= 3, "3 mints max per transaction");
        ...
        if (threeLimit) {
            require(currentMints.add(amount) <= 3, "Already minted 3!");    
        }
    ...
    function ownerMint(address _toAddress, uint256 amount) external onlyOwner {
        require(counter.add(amount) <= MAX_SUPPLY, "Purchase would exceed max supply");
        require(reserveCounter.add(amount) <= 102, "Purchase would exceed max supply for owner");
    ...
    function disableThreeLimit() external onlyOwnerOrDev {
        threeLimit = false;
    }
    ...
    function transferCollectionOwnership(address _newOwner) external onlyOwner {
        playersOnlyNFT.transferOwnership(_newOwner);
    }
}
