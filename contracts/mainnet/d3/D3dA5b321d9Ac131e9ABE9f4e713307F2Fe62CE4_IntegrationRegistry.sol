...
contract IntegrationRegistry is Ownable {
   ...
   */
  function getIntegrationAdapter(address _module, string memory _name)
    external
    view
    returns (address)
  {
   ...
   *
   * @param  _module       The address of the module associated with the integration
   * @param  _name_hash     Hash of human readable adapter name
   *
   * @return               Address of adapter
   */
  function getIntegrationAdapterWithHash(address _module, bytes32 _name_hash)
    external
    view
    returns (address)
  {
    return integrations[_module][_name_hash];
  }
