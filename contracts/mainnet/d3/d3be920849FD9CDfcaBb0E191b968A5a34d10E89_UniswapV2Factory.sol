...
contract UniswapV2ERC20 is IUniswapV2ERC20 {
    ...
    string public constant name = 'Celswap';
    string public constant symbol = 'CSW';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'Celswap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'Celswap: INVALID_SIGNATURE');
...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'Celswap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Celswap: TRANSFER_FAILED');
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'Celswap: OVERFLOW');
    ...
    function mint(address to) external lock liquidityProviderOnly returns (uint liquidity) {
        ...
        require(liquidity > 0, 'Celswap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'Celswap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'Celswap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'Celswap: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'Celswap: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'Celswap: INSUFFICIENT_INPUT_AMOUNT');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'Celswap: K');
        }
