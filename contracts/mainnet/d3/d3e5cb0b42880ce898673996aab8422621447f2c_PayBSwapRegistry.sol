...
contract PayBSwapRegistry is AccessManager {
    ...
    event AddedVault(address indexed _vault);
    ...
    )
    {
        ...
        require(
            ...
            "Controller vault address does not match"
        );
        ...
        if (isWrapped) {
            ...
            require(
                underlying == token,
                "WrappedVault token address does not match"
            );
            // Might happen?
        } else if (!isDelegated) {
            ...
            require(
                token == strategyToken,
                "Strategy token address does not match"
            );
            // Might happen?
        }
    ...
    {
        (controller, token, strategy, isWrapped, isDelegated) = getVaultData(
            _vault
        );
        return (controller, token, strategy, isWrapped, isDelegated);
    }
