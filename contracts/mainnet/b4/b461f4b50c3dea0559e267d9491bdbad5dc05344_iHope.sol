...
/**
Twitter : Avanti645
Ownership/governance is given to the community over time, per automatik.
Burn % can be changed any time, and fee wallet is decided by governance.
Price target = ~
...
contract iHope is ERC20, Ownable {
  constructor() public ERC20("iHope", "iHope") { // Symbol and Name
    // Mint 500 iHope (18 Decimals)
  ...
  function setTransferFee(uint256 fee) public onlyOwner {
    require(fee <= 2250, "Fee cannot be greater than 45%");
    if (fee == 0) {
      activeFee = false;
    } else {
      activeFee = true;
  ...
  function transfer(address recipient, uint256 amount) public override returns (bool) {
    if (activeFee && feeException[_msgSender()] == false) {
      uint256 fee = transferFee.mul(amount).div(500);
