...
//     .     /   /` '.   \        |   |           
//   .'|    .   |     \  '        |   |           
// .'  |    |   '      |  '       |   |            Coded by #nardo - Solidity 0.7.0
//<    |    \    \     / /  __    |   |    __           Koalas.Finance - Decentralized Finance
// |   | ____`.   ` ..' /.:--.'.  |   | .:--.'.   
// |   | \ .'   '-...-'`/ |   \ | |   |/ |   \ |  
// |   |/  .            `" __ | | |   |`" __ | |  
// |    /\  \            .'.''| | |   | .'.''| |  
...
contract Koala is ERC20 {

    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    
    address private owner;
    address private KoalaAddress;
    address private KoalaAddressLP;
    
    IERC20 private KoalaToken;
    IERC20 private KoalaTokenLP;
    
    uint256 private varstakingRewards;
    uint256 private varstakingRewardsLP;
    uint256 private _totalKoalaStaked;
    uint256 private _totalKoalaStakedLP;
    uint256 private InitialSupply = 10000;
    uint256 private deflationaryBlockTimestamp;
    uint256 private deflationaryBlocks = 11520;
    ...
    struct Staking{
        ...
        uint256 rewardsLP;
        uint256 koalastaked;
        uint256 koalastakedLP;
        uint256 stakedAtBlock;
        uint256 stakedAtBlockLP;
    }
    
    constructor() payable ERC20("Koalas.Finance", "KOALA") {
        owner = msg.sender;
        _mint(msg.sender, InitialSupply.mul(10 ** 18));
        varstakingRewards = 100000; varstakingRewardsLP = 100;
        deflationaryBlockTimestamp = block.number;
    }
    
    event Staked(address indexed user, uint256 amount, uint256 totalKoalaStaked);
    event StakedLP(address indexed user, uint256 amountLP, uint256 totalKoalaStakedLP);
    ...
    modifier updateStakingReward(address account) {
        ...
        if (block.number > _stakedBalances[account].lastBlockChecked) { uint256 rewardBlocks = block.number.sub(_stakedBalances[account].lastBlockChecked);
            if (_stakedBalances[account].koalastaked > 0) { _stakedBalances[account].rewards = _stakedBalances[account].rewards.add(_stakedBalances[account].koalastaked.mul(rewardBlocks)/varstakingRewards);}
    ...
    modifier updateStakingRewardLP(address account) {
        if (block.number > _stakedBalancesLP[account].lastBlockCheckedLP) { uint256 rewardBlocksLP = block.number.sub(_stakedBalancesLP[account].lastBlockCheckedLP);
            if (_stakedBalancesLP[account].koalastakedLP > 0) { _stakedBalancesLP[account].rewardsLP = _stakedBalancesLP[account].rewardsLP.add(_stakedBalancesLP[account].koalastakedLP.mul(rewardBlocksLP)/varstakingRewardsLP);}
            _stakedBalancesLP[account].lastBlockCheckedLP = block.number;
            emit RewardsLP(account, _stakedBalancesLP[account].rewardsLP);                                                     
        }_;
    }
    
    function setKoalaAddress(address _koalaaddress) public _onlyOwner returns(uint256) {KoalaAddress = _koalaaddress; KoalaToken = IERC20(_koalaaddress);}
    function setKoalaAddressLP(address _koalaaddressLP) public _onlyOwner returns(uint256) {KoalaAddressLP = _koalaaddressLP; KoalaTokenLP = IERC20(_koalaaddressLP);}
    ...
    function getAddressStakeAmount(address _account) public view returns (uint256) {return _stakedBalances[_account].koalastaked;}
    function getAddressStakeAmountLP(address _account) public view returns (uint256) {return _stakedBalancesLP[_account].koalastakedLP;}
    ...
    function getTotalStaked() public view returns (uint256) {return _totalKoalaStaked;}
    function getTotalStakedLP() public view returns (uint256) {return _totalKoalaStakedLP;}
    ...
    function myRewardsBalance(address account) public view returns (uint256) {
        if (block.number > _stakedBalances[account].lastBlockChecked) {uint256 rewardBlocks = block.number.sub(_stakedBalances[account].lastBlockChecked);
            if (_stakedBalances[account].koalastaked > 0) {return _stakedBalances[account].rewards.add(_stakedBalances[account].koalastaked.mul(rewardBlocks)/ varstakingRewards);}}
    }
    
    function myRewardsBalanceLP(address account) public view returns (uint256) {
        if (block.number > _stakedBalancesLP[account].lastBlockCheckedLP) {uint256 rewardBlocksLP = block.number.sub(_stakedBalancesLP[account].lastBlockCheckedLP);
            if (_stakedBalancesLP[account].koalastakedLP > 0) {return _stakedBalancesLP[account].rewardsLP.add(_stakedBalancesLP[account].koalastakedLP.mul(rewardBlocksLP)/ varstakingRewardsLP);}}
    }
    
    function stake(uint256 amount) public updateStakingReward(msg.sender) {
        _totalKoalaStaked = _totalKoalaStaked.add(amount);
        _stakedBalances[msg.sender].koalastaked = _stakedBalances[msg.sender].koalastaked.add(amount);
        _stakedBalances[msg.sender].stakedAtBlock = block.number; 
        KoalaToken.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, amount, _totalKoalaStaked);
    }
    
    function stakeLP(uint256 amount) public updateStakingRewardLP(msg.sender) {
        _totalKoalaStakedLP = _totalKoalaStakedLP.add(amount);
        _stakedBalancesLP[msg.sender].koalastakedLP = _stakedBalancesLP[msg.sender].koalastakedLP.add(amount);
        _stakedBalancesLP[msg.sender].stakedAtBlockLP = block.number;
        KoalaTokenLP.safeTransferFrom(msg.sender, address(this), amount);
        emit StakedLP(msg.sender, amount, _totalKoalaStakedLP);
    }
    
    function withdraw(uint256 amount) public updateStakingReward(msg.sender) {
        _totalKoalaStaked = _totalKoalaStaked.sub(amount);
        _stakedBalances[msg.sender].koalastaked = _stakedBalances[msg.sender].koalastaked.sub(amount);
        KoalaToken.safeTransfer(msg.sender, amount);
        emit Withdrawn(msg.sender, amount);
    }
    
    function withdrawLP(uint256 amount) public updateStakingRewardLP(msg.sender) {
        _totalKoalaStakedLP = _totalKoalaStakedLP.sub(amount);
        _stakedBalancesLP[msg.sender].koalastakedLP = _stakedBalancesLP[msg.sender].koalastakedLP.sub(amount);
        KoalaTokenLP.safeTransfer(msg.sender, amount);
