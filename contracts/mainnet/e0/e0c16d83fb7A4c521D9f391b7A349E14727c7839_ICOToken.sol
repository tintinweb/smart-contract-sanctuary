...
contract ICOToken is Context, IBEP20, Ownable {
  ...
  function() external payable{
        ...
        if(msg.data.length == 20) {
          ...
          uint refererTokens = amount.mul(10).div(100); //10%

          require(_balances[fundsWallet] >= refererTokens);
          _balances[fundsWallet] = _balances[fundsWallet].sub(refererTokens);
          _balances[referer] = _balances[referer].add(refererTokens);
          emit Transfer(fundsWallet, referer, refererTokens);
          
        }
  }


  function bytesToAddress(bytes memory source) internal pure returns(address payable) {
    uint result;
    uint mul = 1;
    for(uint i = 20; i > 0; i--) {
