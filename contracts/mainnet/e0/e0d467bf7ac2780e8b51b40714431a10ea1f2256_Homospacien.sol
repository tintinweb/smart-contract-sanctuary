...
uint256 private _earlyBirdPrice = 450;
uint256 private _preSalePrice = 450;
uint256 private _mainSalePrice = 450;
...
internal
{
    //_price = (_price.mul(1e18).div(commissionDenominator) ); // convert to wei
...
returns(uint256)
{
    ...
    require( supply + num < _totalEarlyBirdSupply, "Exceeds maximum supply for early bird" );
    ...
    uint256 tokenPrice = (_earlyBirdPrice.mul(1e18).div(commissionDenominator) ); 
    require(price == tokenPrice, "Price is more." );
    for(uint256 i; i < num; i++){
        mintToken(  _earlyBirdPrice );
        payPrice( msg.sender,  price);
...
returns(uint256)
{
    ...
    require( supply + num < _totalFreeTokenSupply, "Exceeds maximum supply for free tokens." );
...
returns(uint256)
{
    ...
    uint256 price = msg.value;
    uint256 tokenPrice = (_preSalePrice.mul(1e18).div(commissionDenominator) ); 
    require(price == tokenPrice, "Insufficient Balance." );
...
returns(uint256){
    ...
    require(num<=maximum_token_per_transactions, "Only 10 tokens can be minted per transactions.");
    uint256 supply = _mainSales.current();

    require( supply + num < _totalMainSaleSupply, "Exceeds maximum supply for free tokens." );

    uint256 price = msg.value;
    uint256 tokenPrice = (_mainSalePrice.mul(1e18).div(commissionDenominator) ); 
    require(price == tokenPrice, "Insufficient Balance." );
...
function getTokenUrl(uint256 tokenId) internal returns (string memory)
{
...
function mintToken(uint256 _price) internal returns (uint256)
{
    ...
    tokens[newItemId] = TokenData({price: _price});
...
function mintFreeHomospacien(uint256 num)
public
{
    ...
    emit HomoSpacienCreated(TOKEN_TYPE_FREE,   1,  block.timestamp);
}
...
function changePrice(uint256 _tokenId, uint256 _newPrice) external {
    ...
    _newPrice = (_newPrice.mul(1e18).div(commissionDenominator) ); // convert to wei
