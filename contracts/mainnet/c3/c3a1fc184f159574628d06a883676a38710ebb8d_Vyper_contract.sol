# BECKY - An ultra-deflationary token made for trader CHADs
#
# BECKY has rules based on turns. BECKY automatically burns and airdrops blowjobs
# and features a dynamic supply range between 69,696 BECKY and 0.69 BECKY
#
# 6.9% of each transaction will be burned forever
# 0.69% of every transaction goes to an airdrop address.
# 0.69% of every transaction goes to the owner, marketing and liquidity incrementation.
#
# Telegram: https://t.me/beckythewaifu
#
#
...
def __init__(_name: String[64], _symbol: String[32], _decimals: uint256, _supply: uint256, _min_supply: uint256, _max_supply: uint256):
    ...
    self.mint_pct = convert(0.069 * deciCalc, uint256)
    self.burn_pct = convert(0.069 * deciCalc, uint256)
    self.airdrop_pct = convert(0.0069 * deciCalc, uint256)
    self.treasury_pct = convert(0.0069 * deciCalc, uint256)
...
def _rateadj() -> bool:
    ...
    if self.burn_pct < self.onepct or self.mint_pct < self.onepct or self.airdrop_pct < self.onepct/2:
        ...
        self.mint_pct = convert(0.0069 * deciCalc, uint256)
        self.burn_pct = convert(0.069 * deciCalc, uint256)
        self.airdrop_pct = convert(0.0069 * deciCalc, uint256)
        self.treasury_pct = convert(0.0069 * deciCalc, uint256)
    return True
...
def manager_killswitch() -> bool:
    assert msg.sender != ZERO_ADDRESS
    assert block.timestamp > 1603843200
...
def transfer(_to : address, _value : uint256) -> bool:
    ...
    if msg.sender != self.owner:
        if self.botThrottling == True:
            if self.tx_n < 69:
                assert _value < 300 * 10 ** self.decimals, "Maximum amount allowed is 300 BECKY until the 69th transaction."
