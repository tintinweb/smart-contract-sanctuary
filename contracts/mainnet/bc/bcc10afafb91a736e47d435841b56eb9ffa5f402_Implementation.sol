...
contract Setters is State, Getters {
    ...
    function _addLiquidity(address pool, address token, address anotherToken, uint256 amount) internal returns (uint256) {
        ...
        IERC20(anotherToken).transferFrom(msg.sender, pool, anotherAmount);
...
contract Collateral is Comptroller, ReentrancyGuard {
    ...
    function mint(address token, uint256 value) external nonReentrant {
        Require.that(
            epoch() == 0 || bootstrappingAt(epoch().sub(1)),
            FILE,
            "Must in bootstrapping"
        );
        ...
        if (token == usdc()) {
            dollar().mint(msg.sender, value.mul(1e12).mul(10000).div(getCollateralRatio()));
        } else if (token == dai()) {
            dollar().mint(msg.sender, value.mul(10000).div(getCollateralRatio()));
        }
    }
