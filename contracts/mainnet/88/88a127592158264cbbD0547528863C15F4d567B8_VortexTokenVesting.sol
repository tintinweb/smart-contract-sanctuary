...
contract VortexTokenVesting is Ownable {
        ...
        returns (uint256 _availablePercentage)
    {
        ...
        uint256 oneYear = _initialTimestamp + 365 days;
        ...
        // console.log("Current TimeStamp %s %s", _currentTimeStamp ,_currentTimeStamp <= sixMonths);
        // console.log(_currentTimeStamp > sixMonths && _currentTimeStamp < oneYear);
        // console.log(_currentTimeStamp > eighteenMonths && _currentTimeStamp < twentyFourMonths);

        if (_currentTimeStamp <= sixMonths) {
            return 16600000000000000000;
        } else if (_currentTimeStamp > sixMonths && _currentTimeStamp < oneYear) {
            return 33200000000000000000;
        } else if (_currentTimeStamp > oneYear && _currentTimeStamp < eighteenMonths) {
    ...
    function _getAdvisorsPercentage(uint256 _currentTimeStamp) private view returns (uint256 _availablePercentage) {
        ...
        uint256 duration = _initialTimestamp + 365 days + 150 days;
        ...
        //0.273972603%
        // console.log("Every Day Relase: %s", everyDayReleasePercentage);
        if (_currentTimeStamp > cliffDuration) {
            // Date difference in days - (endDate - startDate) / 60 / 60 / 24; // 40 days
            if(_currentTimeStamp < duration) {
                uint256 noOfDays = BokkyPooBahsDateTimeLibrary.diffDays(cliffDuration, _currentTimeStamp);
                // console.log("Number of days: %s", noOfDays);
                uint256 currentUnlockedPercentage = noOfDays.mul(1e18).mul(everyDayReleasePercentage).div(1e18);
        ...
        returns (uint256 _availablePercentage)
    {
        ...
        if (currentTimeStamp > _initialTimestamp) {
            if (distributionType == DistributionType.RESERVE) {
                ...
                if (currentTimeStamp <= _initialTimestamp + _year) {
                    return uint256(0).mul(1e18);
                } else if (currentTimeStamp > _initialTimestamp + _year) {
                    return uint256(100).mul(1e18);
                }
