...
contract TwoBitClick is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  uint256 public constant upgradePrice = 0.0099 ether;
  uint public constant maxTokenPurchase = 22;
  ...
  constructor(address rendererAddress, address upgraderAddress) ERC721("TwoBitClick", "TWOBIT") { 
  ...
  function mintToken() public payable nonReentrant {
    ...
    require(remainingMint(msg.sender).sub(1) >= 0, "Can only mint 22 Two Bits");
    require(mintedCount <= MAX_TOKENS, "Purchase exceeds supply");
  ...
  function mint() internal {
    mintedCount++;
    uint256 seed = random(mintedCount);
    generate(3, 0, 0, mintedCount, seed, 0);
