...
contract SagaGrantMinter is Ownable, MixinPausable {
  ...
  modifier onlyUnderGrantedCount(address permitted, uint256 mintingAmount) {
    require(mintedCount[permitted] + mintingAmount <= grantedCount[permitted], 'granted supply minted');
    _;
  }
  ...
  function mint(address[] calldata _dsts, uint256[] calldata _txHashes) public payable whenNotPaused() onlyUnderGrantedCount(_msgSender(), _dsts.length) onlyUnderMaxSupply(_dsts.length) {
  ...
  function mintByProxy(address[] calldata _dsts, uint256[] calldata _txHashes) public payable onlyProxy() whenNotPaused() onlyUnderGrantedCount(_txOrigin(), _dsts.length) onlyUnderMaxSupply(_dsts.length) {
    require(_dsts.length == _txHashes.length, "dsts, txhashes length mismatch");
