...
contract MetaJoseon is ERC721Enumerable, Ownable {
  ...
  constructor(
    ...
    _setOgwhitelistUsers(_ogWhitelistedAddresses);
    _setWlwhitelistUsers(_wlWhitelistedAddresses);
  }

  // internal
  function _setOgwhitelistUsers(address[] memory _users) private onlyOwner {
    delete ogWhitelistedAddresses;
    ogWhitelistedAddresses = _users;
  }

  function _setWlwhitelistUsers(address[] memory _users) private onlyOwner {
    delete wlWhitelistedAddresses;
    wlWhitelistedAddresses = _users;
  } 
  ...
  function isOgWhitelisted(address _user) public view returns (bool) {
    for (uint i = 0; i < ogWhitelistedAddresses.length; i++) {
      if (ogWhitelistedAddresses[i] == _user) {
          return true;
      }
    }
    return false;
  ...
  function isWlWhitelisted(address _user) public view returns (bool) {
    for (uint i = 0; i < wlWhitelistedAddresses.length; i++) {
      if (wlWhitelistedAddresses[i] == _user) {
          return true;
      }
    }
    return false;
