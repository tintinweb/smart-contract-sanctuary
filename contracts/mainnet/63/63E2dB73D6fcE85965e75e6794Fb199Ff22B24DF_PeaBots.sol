...
// File: contracts/PeaBots.sol

/**
 * @title PeaBots contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract PeaBots is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    ...
    uint256 public MAX_ELEMENTS = 40;
    uint256 public REVEAL_TIMESTAMP;
    uint256 public reserve_count = 10;
    ...
    address[] private winnerlistAddr;

    constructor() ERC721("PeaBots", "PEABOT") {
    ...
    */
    ...
    function setMaxElements(uint256 _count) external onlyOwner {
        MAX_ELEMENTS = _count;
    }

    function setReserveCount(uint256 _count) external onlyOwner {
    ...
    function reserve(uint256 _count) public onlyOwner {
        require(_count <= reserve_count, "Exceeds reserved NFT supply" );
    ...
    */
    function mint(uint256 _count) public payable {
        ...
        require(total.add(_count) <= MAX_ELEMENTS - reserve_count, "Exceeds max supply");
        require(total <= MAX_ELEMENTS - reserve_count, "Sale ended");
        require(mintPrice.mul(_count) <= msg.value, "Ether value below price");
        require(_count > 0 && _count <= MAX_MINTABLE_IN_TXN, "You can only mint between 1-20 tokens at a time.");
    ...
    function addAddressDumpToWhitelist(address[] memory addrs) onlyOwner public returns(bool success) {
        for(uint i = 0; i < addrs.length; i++) {
            if (isWhitelisted(addrs[i])) continue;
            whitelistAddr.push(addrs[i]);
            whitelist[addrs[i]].addr = addrs[i];
            whitelist[addrs[i]].claimAmount = 0;
    ...
    function removeFromWhitelist(address addr) external onlyOwner returns(bool success){
        require(isWhitelisted(addr), "Address not exist");
        uint256 index;
        for (uint256 i = 0; i < whitelistAddr.length; i++) {
            if (whitelistAddr[i] == addr) {
                index = i;
                break;
            }
        }
        whitelistAddr[index] = whitelistAddr[whitelistAddr.length - 1];
        whitelistAddr.pop();
        delete whitelist[addr];
        success = true;
    }

    function addAddressDumpToWinnerlist(address[] memory addrs, uint[] memory claimAmountList) onlyOwner public returns(bool success) {
        for(uint i = 0; i < addrs.length; i++) {
            if (isWinnerlisted(addrs[i])) continue;
            winnerlistAddr.push(addrs[i]);
            winnerlist[addrs[i]].addr = addrs[i];
            winnerlist[addrs[i]].claimAmount = claimAmountList[i];
            winnerlist[addrs[i]].hasMinted = 0;
        }
        success = true;
    }
    ...
    function getWinnerlist() public view returns(address[] memory) {
        return winnerlistAddr;
    }

    function removeFromWinnerlist(address addr) external onlyOwner returns(bool success){

        require(isWinnerlisted(addr), "Array not exist");
        uint256 index;
        for (uint256 i = 0; i < winnerlistAddr.length; i++) {
            if (winnerlistAddr[i] == addr) {
                index = i;
                break;
            }
        }
        winnerlistAddr[index] = winnerlistAddr[winnerlistAddr.length - 1];
        winnerlistAddr.pop();
        delete winnerlist[addr];
        success = true;
    }
}