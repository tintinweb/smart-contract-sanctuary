...
// File contracts/IGravisCollectible.sol
...
// File contracts/IERC20Optional.sol
...
// File contracts/NFTPrivateSeller.sol
...
contract NFTPrivateSeller is IERC721Receiver, Ownable, Whitelist {
    ...
    // @dev nft - address of Gravis NFT token
    address public nft;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _nftTypes.length; i++) {
            (, totalSupply, , , , ) = IGravisCollectible(nft).getTypeInfo(_nftTypes[i]);

            require(totalSupply == 0, "token type was issued before from another minter");
        }
    ...
    function buy(
        ...
        _buyBatch(_stablecoin, _type, _amount, 1);
    }
    ...
    function _buyBatch(
        ...
        require(resolvedStablecoins[_stablecoin], "Sales: stablecoin is not accepted");
        require(resolvedNFTs[_type], "Sales: nft is not accepted");

        (uint256 price, , , , , ) = IGravisCollectible(nft).getTypeInfo(_type);

        IERC20 token = IERC20(_stablecoin);
        uint256 decimals = uint256(IERC20Optional(_stablecoin).decimals());
        price = _items.mul(price.mul(10**decimals));

        require(token.allowance(msg.sender, address(this)) >= price, "Sales: required tokens amount not approved");
        ...
        uint256 tokenId;
        tokenId = IGravisCollectible(nft).mint(msg.sender, _type, _items);
