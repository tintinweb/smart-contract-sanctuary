...
interface IAddressRegistry {
    ...
    enum AddressTypes { Token, Controller, Pool }
...
contract Staking is IStaking, Initializable, Ownable, Pausable, ReentrancyGuard {
    ...
    IDelegateFunction public delegateFunction;
    ...
    function deposit(uint256 amount, uint256 scheduleIndex) external override {
        _depositFor(msg.sender, amount, scheduleIndex, address(0), address(0));
    }
    ...
    function depositFor(
        ...
        _depositFor(account, amount, scheduleIndex, address(0), address(0));
    }

    function depositForRequireDelegation(
        address account,
        uint256 amount,
        uint256 scheduleIndex,
        address votingDelegatedTo,
        address rewardsDelegatedTo
    ) external override onlyPermissionedDepositors {
        _depositFor(account, amount, scheduleIndex, votingDelegatedTo, rewardsDelegatedTo);
    }
    ...
    function depositWithSchedule(
        ...
        _addSchedule(schedule);
        _depositFor(account, amount, scheduleIx, address(0), address(0));
    }
    ...
    function setDelegateFunctionAddress(address funcAddress) external override onlyOwner {
        require(funcAddress != address(0), "INVALID_ADDRESS");

        delegateFunction = IDelegateFunction(funcAddress);

        emit DelegateFunctionAddressSet(funcAddress);
    }
    ...
    function _depositFor(
        ...
        if (votingDelegatedTo != address(0)) {
            DelegateMapView memory votingDelegate = delegateFunction.getDelegation(account, "voting");
            require(votingDelegate.otherParty == votingDelegatedTo, "VOTING_NOT_DELEGATED");
            require(votingDelegate.mustRelinquish, "VOTING_DELEGATION_NOT_LOCKED");
        }

        if (rewardsDelegatedTo != address(0)) {
            DelegateMapView memory rewardsDelegate = delegateFunction.getDelegation(account, "rewards");
            require(rewardsDelegate.otherParty == rewardsDelegatedTo, "REWARDS_NOT_DELEGATED");
            require(rewardsDelegate.mustRelinquish, "REWARDS_DELEGATION_NOT_LOCKED");
        }
...
interface IStaking {
    ...
    event DestinationsSet(address fxStateSender, address destinationOnL2);
    event EventSendSet(bool eventSendSet);
    event DelegateFunctionAddressSet(address funcAddress);
    ...
    function depositForRequireDelegation(address account, uint256 amount,  uint256 scheduleIndex, address votingDelegatedTo, address rewardsDelegatedTo) external;
    ...
    function setDelegateFunctionAddress(address funcAddress) external;
...
contract AddressRegistry is IAddressRegistry, Initializable, AccessControl {
    ...
    EnumerableSet.AddressSet private controllerAddresses;
    EnumerableSet.AddressSet private poolAddresses;

    bytes32 public constant REGISTERED_ADDRESS = keccak256("REGISTERED_ROLE");
    ...
    function initialize() public initializer {
        ...
        addressSets[AddressTypes.Token] = tokenAddresses;
        addressSets[AddressTypes.Controller] = controllerAddresses;
        addressSets[AddressTypes.Pool] = poolAddresses;
    }
    ...
    function addToRegistry(address[] calldata _addresses, AddressTypes _index) external override onlyRegistered {
        ...
        for (uint256 i = 0; i < arrayLength; i++) {
            ...
            structToAddTo.add(_addresses[i]);
        }
    ...
    function removeFromRegistry(address[] calldata _addresses, AddressTypes _index) external override onlyRegistered {
        ...
        for (uint256 i = 0; i < arrayLength; i++) {
            ...
            require(structToRemoveFrom.contains(currentAddress), "DOES_NOT_EXIST");
            structToRemoveFrom.remove(currentAddress);
        }
