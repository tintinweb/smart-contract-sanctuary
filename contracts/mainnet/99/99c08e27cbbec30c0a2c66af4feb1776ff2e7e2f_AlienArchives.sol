...
// File: contracts/AlienArchives.sol
...
contract AlienArchives is ERC721, Ownable {
    ...
    constructor() ERC721("Alien Archives", "AA") {
      require(frozenBaseUri == false, "The baseURI is already frozen and can no longer be changed");
      _setBaseURI('https://www.alienarchives.org/metadata/');

      for (uint i = 0; i < 150; i++) {
        uint mintIndex = totalSupply() + 1;
        _safeMint(CURATORS_ADDRESS, mintIndex);
      }
    }

    function setBaseURI(string memory baseURI) public onlyOwner {
      require(frozenBaseUri == false, "The baseURI has already been frozen");
      _setBaseURI(baseURI);
    }
    ...
    function setProvenanceHash(string memory provenanceHashString) public onlyOwner {
      require(frozenProvenanceHash == false, "The provenance hash has already been frozen");
      PROVENANCE_HASH = provenanceHashString;
    }

    function freezeProvenanceHash() public onlyOwner {
      require(frozenProvenanceHash == false, "The provenance hash has already been frozen");
      frozenProvenanceHash = true;
    }

    function reserveTokens(address to, uint256 amountToReserve) public onlyOwner {
      require(totalSupply().add(amountToReserve) <= MAX_TOKENS, "Reserve mint would exceed max supply of tokens");
    ...
    function withdrawAll() public payable {
      require((msg.sender == CURATORS_ADDRESS || msg.sender == CRYPTOCRACY_ADDRESS), "You are not allowed to withdraw");
    ...
    function mintPresaleTokens(uint numberOfTokens) public payable {
      ...
      require(PRICE_PER_PRESALE_TOKEN.mul(numberOfTokens) <= msg.value, "Not enough Ether was sent to mint the requested number of tokens");
    ...
    function mintTokens(uint numberOfTokens) public payable {
      ...
      require(totalSupply().add(numberOfTokens) <= MAX_TOKENS, "This transaction would exceed the max allowed number of tokens");
      require(PRICE_PER_TOKEN.mul(numberOfTokens) <= msg.value, "Not enough Ether was sent to mint the requested number of tokens");
      require(numberOfTokens <= MAX_TOKENS.sub(RESERVED_TOKENS).sub(totalSupply()), "This tx, plus unminted reserves, would exceed max allowed number of tokens");
