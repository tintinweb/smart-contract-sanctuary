...
contract LSDStampverse is ERC721Enumerable, Ownable {
    ...
    uint256 public constant stampPrice = 15000000000000000; // 0.015 ETH
    uint public constant maxTokenPurchase = 19;
    uint256 public MAX_TOKENS = 5000;
    uint256 public MAX_FREE_TOKENS = 1000;
    bool public saleIsActive = false;
    
    mapping(uint256 => bool) public tier3Token;

    string private _baseURIextended;

    constructor() ERC721("LSD Stampverse", "LSD") {
    ...
    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        require(totalSupply() + _amount <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
        uint256 supply = totalSupply() + 1;
        for(uint256 i; i < _amount; i++){
            _safeMint( _to, supply + i );
            tier3Token[supply + i] = true;
        }
    }
    ...
    function mintLSD(uint numberOfTokens) public payable {
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() + 1 < MAX_TOKENS) {
                _safeMint(msg.sender, mintIndex);
                tier3Token[mintIndex] = true;
    ...
    function buyStamp(uint tokenId) public payable {
        require(saleIsActive, "Free Sale must be active to mint Tokens");
        require(stampPrice <= msg.value, "Ether value sent is not correct");
        require(tier3Token[tokenId] == false, "Token already on tier 3");
        require(ownerOf(tokenId) == msg.sender, "Sender should be owner of the token");
        tier3Token[tokenId] = true;
    }
