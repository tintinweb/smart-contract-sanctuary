...
contract BrokePunks is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable {
    ...
    string public _baseURI = "https://raw.githubusercontent.com/leandrodemedeiros/brokepunks/main/nft/";
    uint256 public mintMax = 6;
    ...
    function getItemsRedeemed() external view returns (uint256 minted, uint256 max){
        minted = mintCount;
        max = mintMax;
    }
    constructor() public {
    ...
    function setGoldenPrice(uint256 _value) public onlyOwner {
        price = _value;
    }

    function setGoldLimit(uint256 _value) public onlyOwner {
        mintMax = _value;
    }

    function setBaseURI(string memory baseURIPrefix) public onlyOwner {
        _baseURI = baseURIPrefix;
    }
    ...
    modifier whenNotPaused(){
        require(_pause == false, "Not available yet");
        _;
    }
    ...
    function buy() whenNotPaused public payable {
        require( price <= msg.value, "ether value sent is too low");
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
        _tokenIdCounter.increment();
        require( mintCount <= mintMax, "max minting reached" );
