...
contract LandYield is Ownable, ReentrancyGuard {
  ...
  mapping (address => uint256) private _landYieldRatios;
  ...
  function landYieldRatio(address _address) external view returns (uint256) {
    return _landYieldRatios[_address];
  }

  function setLandYieldRatio(address _address, uint256 _ratio) external onlyManager {
    require(_address != address(0), "Invalid Address");
    require(_ratio > 0 && _ratio < 100, "Invalid Yield Ratio");
    _landYieldRatios[_address] = _ratio;
  }

  // Called by registered primary token sales contracts for distributing the profits for land owners and treasury
  function distributeSalesYield() external payable nonReentrant {
    require(msg.value > 0, "Insufficient Yield");

    uint256 yieldRatio = _landYieldRatios[msg.sender];
    require(yieldRatio > 0 && yieldRatio < 100, "Unregistered Yield Source");
    ...
    uint256 landYield = (msg.value * yieldRatio) / 100;
    _totalYieldPerLand += landYield / landCount;
    _owedTreasuryYield += (msg.value - landYield) + (landYield % landCount);
  }
