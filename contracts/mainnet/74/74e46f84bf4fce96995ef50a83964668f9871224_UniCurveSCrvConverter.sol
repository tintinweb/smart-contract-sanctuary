// hevm: flattened sources of src/strategy-converters/uni-curve-scrv-converter.sol
...
////// src/strategy-converters/uni-curve-scrv-converter.sol
...
contract UniCurveSCrvConverter {
    using SafeMath for uint256;
    ...
    function convert(
        ...
        // 2. Removes liquidiy from Uniswap
        ...
        // 3. Converts WETH to stablecoin
        address stablecoin = IUniswapV2Pair(_fromWant).token0() == weth
            ? IUniswapV2Pair(_fromWant).token1()
            : IUniswapV2Pair(_fromWant).token0();

        uint256 _weth = IERC20(weth).balanceOf(address(this));
        IERC20(weth).approve(univ2Router, _weth);
        ...
        // 3. Adds liquidity to susdv2 pool
        uint256 _stablecoin = IERC20(stablecoin).balanceOf(address(this));
        ...
        uamounts[_getIndex(stablecoin)] = _stablecoin;
        if (stablecoin != usdt) {
            IERC20(stablecoin).approve(zap, _stablecoin);
        } else {
            USDT(stablecoin).approve(zap, _stablecoin);
        }
        ICurveZap(zap).add_liquidity(uamounts, 0);

        // 4. Sends scrv token back to msg.sender
        uint256 liquidity = IERC20(_toWant).balanceOf(address(this));
        IERC20(_toWant).transfer(msg.sender, liquidity);

        return liquidity;
    }

    function _getIndex(address stablecoin) internal pure returns (uint256) {
        if (stablecoin == dai) {
            return 0;
        }
