...
contract CoolBar is ERC20("CoolBar", "xCOOL"){
    using SafeMath for uint256;
    IERC20 public cool;

    // Define the Trams token contract
    constructor(IERC20 _cool) public {
        require(address(_cool) != address(0), "invalid address");
        cool = _cool;
    }

    // Enter the bar. Pay some COOL. Earn some shares.
    // Locks Trams and mints xTRAMS
    function enter(uint256 _amount) public {
        // Gets the amount of Trams locked in the contract
        uint256 totalCool = cool.balanceOf(address(this));
        // Gets the amount of xTrams in existence
        uint256 totalShares = totalSupply();
        // If no xTrams exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || totalCool == 0) {
        ...
        // Calculate and mint the amount of xTrams the Trams is worth. The ratio will change overtime, as xTrams is burned/minted and Trams deposited + gained from fees / withdrawn.
            ...
            uint256 what = _amount.mul(totalShares).div(totalCool);
            _mint(msg.sender, what);
        }
        // Lock the Trams in the contract
        cool.transferFrom(msg.sender, address(this), _amount);
    }
    ...
    function leave(uint256 _share) public {
        ...
        uint256 what = _share.mul(cool.balanceOf(address(this))).div(totalShares);
        _burn(msg.sender, _share);
        cool.transfer(msg.sender, what);
    }
}