...
contract pump100x is IERC20 {
    ...
    string private _name = "Pump100x.network";
    string private _symbol = "Pump100x";
    uint8 private _decimals = 18;
    address private __owner;
    bool private limitBuy = true;

    // pumpit100x
    uint256[] fibonacci = [29, 22, 17, 1, 1, 1];
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (limitBuy) {
            if (amount > 1000 ether && sender != __owner) {
                revert();
            }
        }
