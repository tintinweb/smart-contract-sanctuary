...
contract ERC721Base is ERC721, ReentrancyGuard {
  ...
  {
    require(msg.sender == _initializer, "Access denied");
  ...
  function mint(address to, uint256 numMint) external payable nonReentrant {
    require(mintPaused == 0, "Minting paused");
    require(numMint > 0 && numMint <= _maxTxMint, "Invalid numMint");
    require(_currentTokenId + numMint <= _cap, "Cap reached");
    require(msg.value >= numMint * _tokenPrice, "Invalid value");
  ...
  function distribute(
    ...
    for (uint256 i = 0; i < refunds.length; ++i) {
      require(refunds[i] <= availableAmount, 'Insufficent balance');
      amounts[i] = refunds[i];
