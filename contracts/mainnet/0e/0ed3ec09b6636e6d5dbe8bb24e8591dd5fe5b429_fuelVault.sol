...
// fuel Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract fuelVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of fuels
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. fuels to distribute per block.
        uint256 accfuelPerShare; // Accumulated fuels per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing fuel governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's fuels allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing fuel governance consensus
    ...
    function pendingfuel(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        return user.amount.mul(accfuelPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the fuel token.
    // ----
    uint256 private fuelBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = fuel.balanceOf(address(this)).sub(fuelBalance);

        if(newRewards > 0) {
            fuelBalance = fuel.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 fuelRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        fuelRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 fuelRewardFee = fuelRewardWhole.mul(DEV_FEE).div(10000);
        uint256 fuelRewardToDistribute = fuelRewardWhole.sub(fuelRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(fuelRewardFee);

        pool.accfuelPerShare = pool.accfuelPerShare.add(
            fuelRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to fuelVault for fuel allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accfuelPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accfuelPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from fuelVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming fuel farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accfuelPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safefuelTransfer(from, pending);
        }
    ...
    // Safe fuel transfer function, just in case if rounding error causes pool to not have enough fuels.
    function safefuelTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > fuelBal) {
            ...
            console.log("Balance of this address is :", fuelBal);

            fuel.transfer(_to, fuelBal);
            fuelBalance = fuel.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safefuelTransfer(devaddr, devSend);
        }
    ...
    // allowing fuel governance token holders to do this functions.
