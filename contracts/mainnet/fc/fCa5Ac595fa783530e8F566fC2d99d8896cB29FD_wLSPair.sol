...
contract wLSPair is ERC20, Ownable {
    ...
    event LSPairChanged(address indexed newLSPair);

    event LongTokenChanged(address indexed longToken);
    ...
    /// @notice Changes the LSPair
    function setLongToken(address _longToken) external onlyOwner {
        longToken = _longToken;

        emit LongTokenChanged(_longToken);
    }

    /// @notice Changes the LSPair
    function setLsPair(address _lsPair) external onlyOwner {
        lsPair = ILSPair(_lsPair);

        emit LSPairChanged(_lsPair);
    }
...
interface ILSPair {
    ...
    /**
     * @notice Token used as short in the LSP. Mint and burn rights needed by this contract.
     */
