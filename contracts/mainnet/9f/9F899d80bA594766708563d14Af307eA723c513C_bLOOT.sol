...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(base, tokenId.toString(),".jpg"));
    }
...
contract bLOOT is ERC721, Ownable {
    ...
    function reserveBloots(address _to, uint256 _reserveAmount) public onlyOwner {        
        ...
        require(_reserveAmount > 0 && _reserveAmount <= bLOOTReserve, "Not enough reserve left for team");
        for (uint i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, supply + i);
        }
        bLOOTReserve = bLOOTReserve.sub(_reserveAmount);
    }


    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        BLOOT_PROVENANCE = provenanceHash;
    }
    ...
    function tokenLicense(uint _id) public view returns(string memory) {
        require(_id < totalSupply(), "CHOOSE A BLOOT WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function mintBloot(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_BLOOT, "Purchase would exceed max supply of bloots");
    ...
    function changeBlootName(uint _tokenId, string memory _name) public {
        ...
        require(sha256(bytes(_name)) != sha256(bytes(blootNames[_tokenId])), "New name is same as the current one");
    ...
    function blootNamesOfOwner(address _owner) external view returns(string[] memory ) {
        ...
        if (tokenCount == 0) {
            ...
            uint256 index;
            for (index = 0; index < tokenCount; index++) {
                result[index] = blootNames[ tokenOfOwnerByIndex(_owner, index) ] ;
            }
            return result;
        }
    }
