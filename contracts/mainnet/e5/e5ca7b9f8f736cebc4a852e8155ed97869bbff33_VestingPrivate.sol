...
// File contracts/VestingPrivate.sol
...
contract VestingPrivate is Ownable {
    ...
    uint256 public dailyVest;
    uint256 public dailyVestLength;
    ...
    /*
     * Note: dailyShares will be provided in thousands to represent 3 digits after the decimal point.
     * Ex. 10% = 10000
     */
    ...
    constructor(
        ...
        dailyVest = _dailyVest;
        dailyVestLength = _dailyVestLength;
        initialValue = _initialValue;
    }

	
    function percDiv(uint256 period, uint256 _share, uint256 initialValue)
        public
        pure
        returns (uint256)
    {
        return period.mul(_share).add(initialValue);
    }
    ...
    function addRecipient(
        ...
        require(
            recipients[_recipientAddress].sharePerDay == 0,
    ...
    function addMultipleRecipients(
        ...
        require(
            _recipients.length == _dailyShares.length,
            "The two arrays are with different length"
        );
        for (uint256 i; i < _recipients.length; i++) {
            addRecipient(_recipients[i], _dailyShares[i]);
        }
    }
        ...
        returns (uint256 _owedAmount, uint256 _calculatedAmount)
    {
        ...
        _calculatedAmount = percDiv(
            period,
            recipients[msg.sender].sharePerDay,
            initialValue
        );
        _owedAmount =
            _calculatedAmount -
        ...
        //incase of emergency or stuck funds
