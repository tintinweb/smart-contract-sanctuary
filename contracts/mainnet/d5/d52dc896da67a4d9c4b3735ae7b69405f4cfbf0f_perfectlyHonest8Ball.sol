...
// PERFECTLY HONEST 8 BALL ETH CONTRACT
...
// File: browser/perfectlyHonest8Ball.sol
pragma solidity ^0.8.0;


contract perfectlyHonest8Ball is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_BALLS = 8888;
    bool public hasSaleStarted = false;
    // THE IPFS HASH OF ALL TOKEN DATAS WILL BE ADDED HERE WHEN ALL BALLS ARE FINALIZED.
    string public metadataProvenanceHash = "";

    constructor() ERC721("perfectlyHonest8Ball","PH8BALL")  {
        setBaseURI("https://perfectlyhonest8ball.herokuapp.com/api/token/");
    ...
    function calculatePrice() public view returns (uint256) {
        require(hasSaleStarted == true, "Magic hasn't started");
        require(totalSupply() < MAX_BALLS, "Magic has already ended");
        return 10000000000000000; // 0.01ETH
    }

    
   function mintBALL(uint256 numBalls) public payable {
        require(totalSupply() < MAX_BALLS, "Magic has already ended");
        require(numBalls > 0 && numBalls <= 100, "You can ask minimum 1, maximum 100 BALLS");
        require(totalSupply().add(numBalls) <= MAX_BALLS, "Exceeds MAX_BALLS");
        require(msg.value >= calculatePrice().mul(numBalls), "Ether value sent is below the price");
