...
contract Comfy is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdTracker;

    uint256 public constant MAX_NFT = 8888;
    uint256 public constant PRICE = 2 * 10**16;
    uint256 public constant MAX_BY_MINT = 1000;
	
    address public creatorAddress;
    string public baseTokenURI;
	
    event CreateComfy(uint256 indexed id);
	
    constructor(string memory baseURI, address payable creator) ERC721("Comfy Cats NFT", "Comfy") {
        setBaseURI(baseURI);
    ...
    function mint(address _to, uint256 _count) public payable saleIsOpen {
        ...
        require(msg.value >= price(_count), "Value is below price");
