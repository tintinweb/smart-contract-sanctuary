...
contract TransferRule is Ownable, ITransferRules, ChainRuleBase {
	...
	event RestrictionAdded(address addr, uint256 amount, uint256 until);
	...
	function addRestrictions(
		...
		for (uint i=0; i<l; i++) {
			...
			emit RestrictionAdded(addressArray[i], amountArray[i], untilArray[i]);
		}
	}
	
	function getRestriction(address addr) public view returns(Item memory) {
	    return restrictions[addr];
	}
	...
	*/
	...
	function _validate(address from, address to, uint256 value) internal virtual override returns (address _from, address _to, uint256 _value, bool _success, string memory _errmsg) {
		...
		if (restrictions[from].untilTime > block.timestamp) {
			if (to == _tradedToken) {
				_success = false;
				_errmsg = "you recently claimed new tokens, please wait until a month has elapsed to claim again";
			} else if (balanceFrom.sub(value) < restrictions[from].lockedAmount) {
				_success = false;
				_errmsg = "you recently claimed new tokens, until a month has elapsed you can send at most 10% of your remaining tokens";
			}
		}
