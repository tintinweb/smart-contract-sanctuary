...
contract AirDropperErc1155 is Ownable, ERC1155Receiver {
    ...
    external
    {
        ...
        require(airDrop.paidable != true, "This piece is not claimable");
        require(address(airDrop.token) != address(0), "Airdrop with given ID doesn't exists");
        require(!isClaimed(airdropId, index), "Account already claimed");
    ...
    external payable
    {
        ...
        require(msg.value >= airDrop.piecePrice * amount, "Ether value sent is below the price");
        ...
        // Mark it claimed and send the token.
        _setClaimed(airdropId, index);
        payable(airDrop.payout).transfer(airDrop.piecePrice);
        airDrop.token.safeTransferFrom(address(this), account, airDrop.tokenId, 1, "");
   ...
   function adminWithdrawTokensFromExpiredAirdrop(uint256 airdropId) external onlyOwner {
    ...
    event AddedAirdrop(uint256 airdropId, ERC1155 token, uint256 tokenId, uint256 amount, bool paidable);
