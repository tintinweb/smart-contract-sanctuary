...
contract MasterChef_pizzas is Ownable {
 ...
 //constructor
   // function initContract 
	 constructor (address _pizzas,address masterChef, uint256 _rewardRate,uint256 _starttime) public onlyOwner{	
	     
		require(initialized == false,"has initialized");
		 pizzas = IERC20(_pizzas);
    ...
    function pendingPizza(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 pizzaReward = multiplier.mul(rewardRate).mul(pool.allocPoint).div(totalAllocPoint);
        ...
        if(_amount > 0) {
			...
			for(uint256 i = 0;i<2;i++){
				if(_referrer!= address(0) && isUserExists(_referrer)){
					...
					if(i == 0){
					    if(user.reward.mul(15).div(100) > 0){
					        users[_referrer].referReward = 	users[_referrer].referReward.add(user.reward.mul(15).div(100));
					        safePizzaTransfer(_referrer, user.reward.mul(15).div(100));
					    }
					}else if(i == 1){
					    if(user.reward.mul(10).div(100) > 0){
					       users[_referrer].referReward = 	users[_referrer].referReward.add(user.reward.mul(10).div(100));
