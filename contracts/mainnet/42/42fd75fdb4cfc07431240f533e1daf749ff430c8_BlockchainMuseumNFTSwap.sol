...
// File: node_modules\@openzeppelin\contracts\token\ERC1155\utils\ERC1155Receiver.sol
...
// File: contracts\BlockchainMuseumNFT.sol

// BlockchainMuseum NFT token
...
contract BlockchainMuseumNFT is ERC1155, AccessControl {
    ...
    event ItemAdded(uint256 id, string uri, uint256 supply, address creator);
    ...
    function addItem(
        ...
        require(
            hasRole(MINTER_ROLE, msg.sender) || isPublic,
        ...
        tokenSupply[currentID] = supply;

        _creators[currentID] = msg.sender;
        _creatorFee[currentID] = _fee;
        if (bytes(_uri).length > 0) {
            customUri[currentID] = _uri;
            emit URI(_uri, currentID);
        }
        ...
        emit ItemAdded(currentID, _uri, supply, msg.sender);
        return currentID;
    }
...
// File: contracts\BlockchainMuseumSwap.sol

// BlockchainMuseum - NFT Swap contract
...
interface IBlockchainMuseumNFT {
...
contract BlockchainMuseumNFTSwap is Ownable, ERC1155Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
    ...
    IERC20 public blockchainMuseumToken;
    IBlockchainMuseumNFT public blockchainMuseumNFT;
    ...
    event Swapped(address buyer, uint256 id, uint256 amount, Item item);

	constructor () {		
		
	}
	function initialize(address _tokenAddress, address _nftAddress, uint256 _fee, address _feeAddress, string memory _name, string memory _uri) external onlyOwner {
		blockchainMuseumToken = IERC20(_tokenAddress);
		blockchainMuseumNFT = IBlockchainMuseumNFT(_nftAddress);
		feeAddress = _feeAddress;
		swapFee = _fee;
		collections.push(_nftAddress);
		collectionCreators[_nftAddress] = msg.sender;
		emit CollectionCreated(_nftAddress, msg.sender, _name, _uri, true);//"BlockchainMuseum","https://nft.app/blockchainMuseum/"
	}

    function setblockchainMuseumToken(address _address) external onlyOwner {
        require(_address != address(0x0), "invalid address");
		blockchainMuseumToken = IERC20(_address);
    }

    function setNFTAddress(address _address) external onlyOwner {
		require(_address != address(0x0), "invalid address");
        blockchainMuseumNFT = IBlockchainMuseumNFT(_address);
    }
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(BlockchainMuseumNFT).creationCode;
        ...
        IBlockchainMuseumNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function list(address _collection, uint256 _token_id, uint256 _amount, uint256 _price) public {
		require(_price > 0, "invalid price");
		require(_amount > 0, "invalid amount");
		IBlockchainMuseumNFT nft = IBlockchainMuseumNFT(_collection);
        uint256 nft_token_balance = nft.balanceOf(msg.sender, _token_id);
	...
	function delist(uint256 _id) external {
		...
		IBlockchainMuseumNFT(items[_id].collection).safeTransferFrom(address(this), items[_id].owner, items[_id].token_id, items[_id].balance, "delist from BlockchainMuseum Marketplace");
		items[_id].balance = 0;
    ...
    function buy(uint256 _id, uint256 _amount) external {
		...
		uint256 blockchainMuseumAmount = item.price.mul(_amount);
		uint256 token_balance = blockchainMuseumToken.balanceOf(msg.sender);
		require(token_balance >= blockchainMuseumAmount, "insufficient token balance");

		// transfer BlockchainMuseum token to admin
		if(swapFee > 0) {
			require(blockchainMuseumToken.transferFrom(msg.sender, feeAddress, blockchainMuseumAmount.mul(swapFee).div(PERCENTS_DIVIDER)), "failed to transfer admin fee");
		}
		// transfer BlockchainMuseum token to creator
		if(item.creatorFee > 0) {
			require(blockchainMuseumToken.transferFrom(msg.sender, item.creator, blockchainMuseumAmount.mul(item.creatorFee).div(PERCENTS_DIVIDER)), "failed to transfer creator fee");
		}
		// transfer BlockchainMuseum token to owner
		uint256 ownerPercent = PERCENTS_DIVIDER.sub(swapFee).sub(item.creatorFee);
		require(blockchainMuseumToken.transferFrom(msg.sender, item.owner, blockchainMuseumAmount.mul(ownerPercent).div(PERCENTS_DIVIDER)), "failed to transfer to owner");

		// transfer NFT token to buyer
		IBlockchainMuseumNFT(items[_id].collection).safeTransferFrom(address(this), msg.sender, item.token_id, _amount, "buy from BlockchainMuseum Marketplace");
		...
		totalEarning = totalEarning.add(blockchainMuseumAmount);
		totalSwapped = totalSwapped.add(1);

        emit Swapped(msg.sender, _id, _amount, items[_id]);
    }
}