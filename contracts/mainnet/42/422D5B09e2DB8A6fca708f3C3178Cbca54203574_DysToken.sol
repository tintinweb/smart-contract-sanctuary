...
contract DysToken is ERC20Burnable, Ownable {
    /*
*/
    ...
    constructor() ERC20("DysToken", "DYSTO") {}
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(block.timestamp < CLAIM_END_TIME, "Cannot stake after Dec 31, 2021");
        require(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(dystomiceAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(dystomiceAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
