...
contract HoundAndHareGame is ERC721Enumerable, Ownable {

    uint256 public MAX_NFT_SUPPLY = 7777;
    uint256 public RESERVED_NFT = 6000;

    uint256 public NFT_PRICE = 0.04 ether;
    
    uint256 public MAX_NFT_WALLET_PRESALE = 4;
    uint256 public MAX_NFT_WALLET = 8;
    ...
    address[] rareTokenOwners;

    address immutable moderator;
    ...
    constructor(uint256 _publicSaleStartTimestamp, address _moderator) ERC721("Hound & Hare Game", "H&H") {
    ...
    function hasFinalSaleStarted() public view returns(bool){
        return (block.timestamp >= publicSaleStartTimestamp || reservedClaimed >= RESERVED_NFT) ? true : false;
    }
    ...
    function setRareTokenIds(uint256[] memory Ids) external onlyOwner {
        for(uint index = 0; index < Ids.length; index++){
            rareTokenIds.push(Ids[index]);
            rareTokenOwners.push(ownerOf(Ids[index]));
        }
    }

    function getRareTokenOwners() public view returns(address[] memory) {
            return rareTokenOwners;
    }
    ...
    function withdrawAll() public onlyOwner {
        ...
        uint256 adminBalance = balance/2;
        payable(msg.sender).transfer(adminBalance);
        payable(moderator).transfer(balance-adminBalance);
