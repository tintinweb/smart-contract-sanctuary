...
contract RulerCore is Ownable, IRulerCore, IERC3156FlashLender, ReentrancyGuard {
  ...
  function invest(address _token, uint256 _amount) external override onlyOwner {
    address sadr = address(strategy);
    require(sadr != address(0), "Ruler: strategy not set");
    require(_amount > 0, "Ruler: cannot invest 0");
    uint256 bal = IERC20(_token).balanceOf(address(this));
    uint256 amountToInvest = _amount > bal ? bal : _amount;
    _approve(IERC20(_token), sadr, amountToInvest);
    strategy.invest(_token, amountToInvest);
    emit InvestInStrategy(sadr, _token, _amount);
  }

  function divest(address _token, uint256 _amount) external override onlyOwner {
    _divest(_token, _amount);
  }
  ...
  function repay(
    ...
    // if core doesn't have enough tokens to repay, withdraw the amount from the strategy
    if (IERC20(_col).balanceOf(address(this)) < colAmountToPay) {
      _divest(_col, colAmountToPay);
    }
  ...
  function setStrategy(address _address) external override onlyOwner {
    require(_address != address (0), "Ruler: address cannot be 0");
    emit AddressUpdated('strategy', address(strategy), _address);
    strategy = IStrategy(_address);
  }
  ...
  function _sendAmtPostFeesOptionalAccrue(IERC20 _token, uint256 _amount, uint256 _feeRate, bool _accrue) private {
    ...
    uint256 amountToSend = _amount - fees;
    // if divest has not been called yet and there is a lack of funds in core
    if (_token.balanceOf(address(this)) < amountToSend) {
      _divest(address(_token), amountToSend);
    }
    _safeTransfer(_token, msg.sender, amountToSend);
    if (_accrue) {
  ...
  function _divest(address _token, uint256 _amount) private {
    require(address(strategy) != address(0), "Ruler: strategy not set");
    require(strategy.totalDeposits(_token) > 0, "Ruler: no funds invested");
    strategy.divest(_token, _amount);
    emit DivestFromStrategy(address(strategy), _token, _amount);
  }
}
...
interface IRulerCore {
  ...
  event InvestInStrategy(address indexed strategy, address indexed token, uint256 indexed amount);
  event DivestFromStrategy(address indexed strategy, address indexed token, uint256 indexed amount);
  ...
  function setStrategy(address _addr) external;
