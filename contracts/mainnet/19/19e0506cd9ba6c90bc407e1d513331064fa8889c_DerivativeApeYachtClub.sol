...
// File: contracts/DerivativeApeYachtClub.sol



pragma solidity ^0.8.0;

// @title: Derivative Ape Yacht Club
// @author: null.eth
...
contract DerivativeApeYachtClub is ERC721, Ownable {
    ...
    constructor(string memory name, string memory symbol, address baycAddress) public ERC721(name, symbol) {
    ...
    function mint(uint256 apeId)
        external
        payable
    {
        require(
            _isMinted[apeId] != true,
            "This derivative has already been minted."
        );
        require(
            bayc.ownerOf(apeId) == msg.sender,
            "You do not own this ape to mint the corresponding derivative."
        );
        require(
            getMintPrice() <= msg.value,
            "Ether value is less than minting cost."
        );
        _safeMint(msg.sender, apeId);
        _isMinted[apeId] = true;
        _tokenIdCounter.increment();
        if (msg.value > getMintPrice()) {
            Address.sendValue(payable(msg.sender), msg.value - getMintPrice());
        }
    }
}