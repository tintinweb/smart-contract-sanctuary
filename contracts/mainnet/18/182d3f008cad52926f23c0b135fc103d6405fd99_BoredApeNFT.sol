...
pragma solidity ^0.6.12;
    ...
    contract BoredApeNFT is Context, IERC20, Ownable {
        ...
        string private _name = 'BoredApeNFT';
        string private _symbol = 'BoredApeNFT';
        uint8 private _decimals = 9;


        uint256 public _taxFee = 0;
        uint256 public _marketingFee = 8;
        uint256 public _liquidityPoolFee = 8;
        ...
        function removeAllFee() private {
            if(_taxFee == 0 && _marketingFee == 0 && _liquidityPoolFee == 0) return;
        ...
        function sendETHToFee(uint256 amount) private {
            uint256 swapFee = _marketingFee.add(_liquidityPoolFee);
            payable(_marketingWalletAddress).transfer(amount.div(swapFee).mul(_marketingFee));
            payable(_liquidityPool).transfer(amount.div(swapFee).mul(_liquidityPoolFee));
        }
        ...
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
            (uint256 tTransferAmount, uint256 tFee, uint256 tSwap) = _getTValues(tAmount, _taxFee, _marketingFee, _liquidityPoolFee);
            uint256 currentRate =  _getRate();
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
            return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tSwap);
        }

        function _getTValues(uint256 tAmount, uint256 taxFee, uint256 marketingFee, uint256 liquidityPoolFee) private pure returns (uint256, uint256, uint256) {
            uint256 tFee = tAmount.mul(taxFee).div(100);
            uint256 tSwap = tAmount.mul(marketingFee.add(liquidityPoolFee)).div(100);
