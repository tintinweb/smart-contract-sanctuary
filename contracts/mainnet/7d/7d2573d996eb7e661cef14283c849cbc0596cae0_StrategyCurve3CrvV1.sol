...
// File: contracts/strategies/curve/StrategyCurve3CrvV1.sol
...
contract StrategyCurve3CrvV1 is StrategyBase {
    // Curve stuff
    ...
    function harvest() public override onlyBenevolent {
        ...
        if (_to > 0) {
            if (devFundFee > 0) {
                uint256 _devFundFee = _to.mul(devFundFee).div(devFundMax);
                IERC20(to).transfer(
            ...
            if (comFundFee > 0) {
                uint256 _comFundFee = _to.mul(comFundFee).div(comFundMax);
                IERC20(to).transfer(
            ...
            _to = IERC20(to).balanceOf(address(this));
