...
// File contracts/libraries/alchemist/LiquityVaultV2.sol
...
library LiquityVaultV2 {
  using LiquityVaultV2 for Data;
  using LiquityVaultV2 for List;
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;
  ...
  function harvest(Data storage _self, address _recipient) internal returns (uint256, uint256) {
    return _self.directWithdraw(_recipient, 0, true);
  }
...
// File contracts/YumLUSDVault.sol
...
contract YumLUSDVault is  ReentrancyGuard {
  using CDP for CDP.Data;
  using FixedPointMath for FixedPointMath.uq192x64;
  using LiquityVaultV2 for LiquityVaultV2.Data;
  using LiquityVaultV2 for LiquityVaultV2.List;
  ...
  LiquityVaultV2.List private _vaults;
  ...
  {
    require(_governance != ZERO_ADDRESS, "YumLUSDVault: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "YumLUSDVault: sentinel address cannot be 0x0.");
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "YumLUSDVault: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "YumLUSDVault: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "YumLUSDVault: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "YumLUSDVault: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "YumLUSDVault: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "YumLUSDVault: collateralization limit above maximum.");
  ...
  function initialize(IVaultAdapterV2 _adapter) external onlyGov {
    ...
    require(!initialized, "YumLUSDVault: already initialized");

    require(transmuter != ZERO_ADDRESS, "YumLUSDVault: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "YumLUSDVault: cannot initialize rewards address to 0x0");
  ...
  function harvest(uint256 _vaultId) external expectInitialized returns (uint256, uint256) {
    ...
    LiquityVaultV2.Data storage _vault = _vaults.get(_vaultId);
  ...
  function flushActiveVault() internal returns (uint256) {
    ...
    LiquityVaultV2.Data storage _activeVault = _vaults.last();
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "YumLUSDVault: Loan-to-value ratio breached");
    } else {
  ...
  modifier expectInitialized() {
    require(initialized, "YumLUSDVault: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "YumLUSDVault: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapterV2 _adapter) internal {
    require(_adapter != IVaultAdapterV2(ZERO_ADDRESS), "YumLUSDVault: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "YumLUSDVault: token mismatch.");

    _vaults.push(LiquityVaultV2.Data({
      adapter: _adapter,
      totalDeposited: 0
    }));
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "YumLUSDVault: not an emergency, not governance, and user does not have permission to recall funds from active vault");
