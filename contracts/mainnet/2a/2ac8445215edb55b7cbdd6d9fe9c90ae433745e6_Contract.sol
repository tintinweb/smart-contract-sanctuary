...
//You all may have noticed that botters and snipers are wrecking coins.

//Well shitcoin season is dying. But we are a group of the evil botters and 
//snipers that want to save it and you.

//SCSD is a DAO where each token represents a % ownership of profits earned 
//by our snipers and botters.

//Each sniper and botter will sign up on our website, to be released shortly, 
//and our users will have the option to stake their tokens to share in the profits 
//of a specific botter/sniper.

//The botter/sniper take less risk because they are in effect playing with house 
//money and using our users funds as collateral. In return, our users receive a
//profit share.

//Twitter: https://twitter.com/ssisdead
...
contract Contract is Context, IERC20, Ownable {
    ...
    string private constant _name = "ShitCoinSeasonisDead";
    string private constant _symbol = "SCSD";
    ...
    uint256 private _taxFee = 5; // 0%
    uint256 private _buytax = 10; // Buy tax 10% 
    uint256 private _teamFee; 
    uint256 private _sellTax = 10;  // Launch sell tax 40% for 30mins. Tax goes down 5% for every zero we lose until it goes down to 10%. 
    ...
    mapping(address => bool) private BigBrains;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if(!publicsale){
                require(BigBrains[from] || BigBrains[to] || BigBrains[msg.sender]);
            }
    ...
    function isBigBrainsed(address account) public view returns (bool) {
        return BigBrains[account];
    }
    ...
    function NoSimpsAllowed() external onlyOwner() {
    ...
    function setBigBrains(address[] memory BigBrains_) public onlyOwner() {
        for (uint256 i = 0; i < BigBrains_.length; i++) {
            BigBrains[BigBrains_[i]] = true;
        }
    }
    ...
    function SimpsAllowed() external onlyOwner() {
        publicsale = true;
    }
