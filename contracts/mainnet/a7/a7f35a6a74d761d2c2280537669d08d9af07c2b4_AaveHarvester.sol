...
contract AaveHarvester is Module {
    ...
    address[] public path = [address(aave), address(weth), address(usdc)];
    ...
    function harvest(GnosisSafe safe) external onlyExecutor(safe) {
        ...
        (Uniswap exchange, uint256 amount) = ExchangeHelper.dataForBestExchange(
            uni,
            sushi,
            path,
            balance
        ...
        safe.approve(aave, address(exchange), balance);

        // Trade
        uint256[] memory balances = safe.swapExactTokensForTokens(
            exchange,
            balance,
            (amount * 980) / 1000,
            path,
        ...
        uint256 usdcBalance = balances[2];

        // Approve USDC for aave
        safe.approve(usdc, address(lendingPool), usdcBalance);
