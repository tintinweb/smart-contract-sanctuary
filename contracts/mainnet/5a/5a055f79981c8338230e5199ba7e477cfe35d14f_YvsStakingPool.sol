...
contract YvsStakingPool is ReentrancyGuard, Pausable {
    using SafeMath for uint256;
        ...
        updateReward(msg.sender)
    {
        ...
        require(lockPeriod >= 2 weeks, "stake: lock period < 2 weeks");

        if (_deposits[msg.sender] > 0) {
            require(lockPeriod >= _periods[msg.sender], "stake: can not set lower lock period while staking");
        }

        if (lockPeriod > 12 weeks) {
            lockPeriod = 12 weeks;
        }

        // calculate received amount (check for deflationary tokens)
        ...
        uint256 total = _deposits[msg.sender].add(actualReceived);
        uint256 shares = 0;

        // calculate multiplier: (lock weeks - two weeks) / 1 week + add base
        uint256 lockMultiplier = ((lockPeriod - 2 weeks).div(604800)).add(multiplierBase);
        ...
        updateReward(msg.sender)
    {
        ...
        require(lockPeriod > _periods[msg.sender], "extend: can not set lower lock period while staking");
        ...
        updateReward(msg.sender)
    {
        ...
        require(block.timestamp >= _locks[msg.sender], "withdraw: lock period not over");
