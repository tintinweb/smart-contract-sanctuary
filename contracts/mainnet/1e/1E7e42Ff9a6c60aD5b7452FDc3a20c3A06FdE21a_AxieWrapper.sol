...
// File: AxieWrapper.sol

contract AxieWrapper is ERC721("Wrapped Axie", "wAXIE") {
	IERC721 constant public AXIE = IERC721(0xF5b0A3eFB8e8E4c201e2A935F110eAaF3FFEcb8d);

	constructor() public {
		_setBaseURI("https://axieinfinity.com/api/axies/");
	}

	function wrap(uint256 _tokenId) public {
		_mint(msg.sender, _tokenId);
		AXIE.transferFrom(msg.sender, address(this), _tokenId);
	}

	function wrapMany(uint256[] calldata _tokenIds) external {
	...
	function unwrap(uint256 _tokenId) public {
		...
		_burn(_tokenId);
		AXIE.safeTransferFrom(address(this), msg.sender, _tokenId);
	}

	function unwrapMany(uint256[] calldata _tokenIds) external {
		for (uint256 i = 0; i < _tokenIds.length; i++)
			unwrap(_tokenIds[i]);
	}

	function onERC721Received(address _from, uint256 _tokenId, bytes calldata _data) external returns (bytes4) {
		require(msg.sender == address(AXIE));
		_mint(_from, _tokenId);
		return AxieWrapper.onERC721Received.selector;
	}
}