...
contract DiddlyDoodleApes is ERC721URIStorage, Ownable{

    event mintDoodleApes (address indexed minter, uint256 startWith, uint256 times);


    uint256 public totalDoodleApe;
    ...
    constructor() ERC721("DiddlyDoodleApes", "DDA") {
    ...
    function mintDoodleApe(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalDoodleApe + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit mintDoodleApes(_msgSender(), totalDoodleApe+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalDoodleApe++);
        }
    }
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalDoodleApe + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit mintDoodleApes(_addr, totalDoodleApe+1, 1);
        _mint(_addr, 1 + totalDoodleApe++);
    }
