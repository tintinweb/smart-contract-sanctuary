...
contract YieldToken is ERC20("Bobo", "BOBO") {
	using SafeMath for uint256;

uint256 constant public rewardtimeframe = 1800; //43200
	uint256 constant public BASE_RATE = 5 ether; 
	uint256 constant public BASE_RATEresult = 5; 
	uint256 constant public INITIAL_ISSUANCE = 300 ether;

	    bool public isPauseEnabled;

	uint256 constant public END = 2110215600;
...
function startwork(uint256 _ape) external {
     require(msg.sender == address(ApesContract.ownerOf(_ape)), "You are not the owner of this Ape");
     require(!isPauseEnabled, "Staking is on pause");
     require(lastUpdate[_ape] < 1, "Ape is already grinding");
		uint256 time = min(block.timestamp, END);
			lastUpdate[_ape] = time;
	}

	function getReward(uint256 _ape) external {
		require(msg.sender == address(ApesContract.ownerOf(_ape)), "You are not the owner of this Ape");
		require(!isPauseEnabled, "Staking is on pause");
		uint256 time = min(block.timestamp, END);
				uint256 apedate =lastUpdate[_ape];
				if(apedate == 0) {
		   apedate = 1636830000;
		}
		uint256 ramount = uint(time - apedate) / rewardtimeframe * BASE_RATE; //daily 
			lastUpdate[_ape] = time;
			require(ramount > 1, "Your ape didn't worked hard enough");
...
function removeFromBurnList(address[] calldata _addresses) external
	...
	function getTotalClaimable(uint256 _ape) external view returns(uint256) {
	    require(!isPauseEnabled, "Staking is on pause");
		uint256 time = min(block.timestamp, END);
		uint256 apedate =lastUpdate[_ape];
				if(apedate == 0) {
		...
		uint256 ramount = uint(time - apedate) / rewardtimeframe * BASE_RATEresult; //daily 
	//uint256 ramount = rewards[_ape] + pending;
		    //require(lastUpdate[_ape] > 0, "Ask your Ape to start grinding for you");
		return ramount;
