...
contract eTacoChef is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    */
    ...
    function setETaco(IERC20 _etaco) external onlyOwner {
        etaco = _etaco;
    }
    ...
    function transferETACO(uint256 _amount) external onlyOwner {
        safeETacoTransfer(msg.sender, _amount);
    }
    ...
    function setUser(
        ...
        if (userInfo[_pid][_user].amount == 0) {
            updatePool(_pid);
            uint256 accRewardPerShare = poolInfo[_pid].accRewardPerShare;
            userInfo[_pid][_user] = UserInfo(
                _amount,
                _amount.mul(accRewardPerShare).div(1e12)
            );
        }
    }
    ...
    function setUserAdmin(
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            userInfo[_pid][_users[i]].rewardDebt = _amounts[i]
