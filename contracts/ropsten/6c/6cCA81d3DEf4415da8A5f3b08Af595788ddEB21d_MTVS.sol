...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 public addr1Ratio = 5;
    uint256 public addr2Ratio = 2;
    uint256 public burnRatio = 8;
    uint256 public finalSupply;
    address public addr1 = 0xb890482DB284C053249FB2d6AFECD024b9881C6f;
    address public addr2 = 0x48fffa333B4441c028Afbe5302F1979f27E976De;
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_) {
        ...
        _owner = msg.sender;
        _decimals = decimals_;
        finalSupply = 100000000 * 10 ** uint256(_decimals);
    }
    ...
    function setFinalSupply(uint256 _amount) public onlyOwner {
        finalSupply = _amount;
    }
    
    function setRatio(uint256 _burnRatio, uint256 _addr1Ratio, uint256 _addr2Ratio) public onlyOwner {
        require((_burnRatio + _addr1Ratio + _addr2Ratio) < 100, "100 ??");
        burnRatio = _burnRatio;
        addr1Ratio = _addr1Ratio;
        addr2Ratio = _addr2Ratio;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 addr1Amount = 0;
        uint256 addr2Amount = 0;
        uint256 burnAmount = 0;
        if(addr1Ratio > 0 && _totalSupply > finalSupply){
            addr1Amount = amount * addr1Ratio / 100;
        }
        if(addr2Ratio > 0 && _totalSupply > finalSupply){
            addr2Amount = amount * addr2Ratio / 100;
        }
        if(burnRatio > 0 && _totalSupply > finalSupply){
            burnAmount = amount * burnRatio / 100;
        }
        withdrawLP(sender,amount);
        _balances[sender] = senderBalance - (amount - burnAmount);
        if(addr1Amount > 0){
            _balances[addr1] += addr1Amount;
            emit Transfer(sender, addr1, addr1Amount);
        }
        if(addr2Amount > 0){
            _balances[addr2] += addr2Amount;
            emit Transfer(sender, addr2, addr2Amount);
        }
        _balances[recipient] += (amount - addr1Amount - addr2Amount - burnAmount);
        depositLP(recipient, (amount - addr1Amount - addr2Amount - burnAmount));
        if(burnAmount > 0){
    ...
    function addRewards(uint256 _amount) public {
        ...
        withdrawLP(msg.sender, _amount);
        _balances[msg.sender] -= _amount;
        _balances[address(this)] += _amount;
        depositLP(address(this), _amount);
