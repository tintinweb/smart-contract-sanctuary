...
contract HeadShotLedger is Context, IERC20, Ownable {
    ...
    function getTrackerBalance(uint256 code_, address account_) public view returns (uint256) {
    ...
    function setTrackerFieldString(uint256 code_, string memory key_, string memory value_) public onlyOwner {
        HeadShotTracker headShotTracker = HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.setFieldString(key_, value_);
    }
    function setTrackerFieldNumber(uint256 code_, string memory key_, uint256 value_) public onlyOwner {
        HeadShotTracker headShotTracker = HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.setFieldNumber(key_, value_);
    }
    function setTrackerFieldAddress(uint256 code_, string memory key_, address value_) public onlyOwner {
        HeadShotTracker headShotTracker = HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.setFieldAddress(key_, value_);
    }

    function increaseBalance(uint256 code_, address account_, uint256 balance_) public onlyOwner returns (uint256){
        HeadShotTracker headShotTracker = HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.increaseBalance(account_, balance_);
    }
    function decreaseBalance(uint256 code_, address account_, uint256 balance_) public onlyOwner returns (uint256){
        HeadShotTracker headShotTracker = HeadShotTracker(payable(getTrackerAddress(code_)));
