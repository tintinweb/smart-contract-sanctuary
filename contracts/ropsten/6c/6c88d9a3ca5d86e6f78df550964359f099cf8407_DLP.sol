...
contract DLP is Ownable {
    ...
    constructor() public 
    {
        dLRNFT = DLRNFT(0x340203BDdfe28318BD065668ba7Da489429848c0);
        dLBNFT = DLBNFT(0x928870Ef67328d0BB8b607783448F761bb1E710C);
    }
    ...
    function claim(uint256 _launchIndex) external {
        ...
        if(newClaim==0){
            ...
            require(dLBNFT.ticketClaimed(tokenId)==0,"claimed");
            dLBNFT.claimTicket(tokenId);
    ...
    function preSale(address _token, uint256 amount) external onlyOwner {
        require(phase!=1,"already presale");
        IERC20 token = IERC20(_token);
        require(amount>=token.balanceOf(address(this)),"not enough token balance");
        phase = 1;
        emit Phase(phase,TokenLaunched[launchIndex]);
        launchIndex++;
        Token[launchIndex] = _token;
        TokenPresale[launchIndex] = amount;
    }

    function postSale() external onlyOwner {
