...
contract DepositToken_10 {
    ...
    function distributeReferrerBonus(address payable _directReferrer, uint platformFee) internal returns (uint){
        ...
        _directReferrer.transfer(platformFee.mul(20).div(100)); //20% of Platform Fee
        ...
        if(referrals[_directReferrer] != address(0x0)){
            ...
            referrals[_directReferrer].transfer(platformFee.mul(10).div(100)); //10% of Platform Fee
        ...
        if(referrals[referrals[_directReferrer]] != address(0x0)){
            // Sending ether to level 2 referrer and deducting that amount from platformFee
            referrals[referrals[_directReferrer]].transfer(platformFee.mul(10).div(100)); //10% of Platform Fee
            finaPlatformFee = finaPlatformFee.sub(platformFee.mul(10).div(100));
        }
