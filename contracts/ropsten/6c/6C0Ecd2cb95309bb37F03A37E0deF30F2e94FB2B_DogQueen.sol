...
//address setup
    ...
    */
    ...
    function updatePool(uint256 amount, uint256 pool) internal {
        emit DebugUint256("queen->updatePool->amount", amount);
        emit DebugUint256("queen->updatePool->pool", pool);
        if (pool == 0) {
            return;
        }
        _accPerShare = _accPerShare.add(amount.mul(1e12).div(pool));    //避免小数, mul(1e22)
        emit DebugUint256("queen->updatePool->_accPerShare", _accPerShare);
    }
