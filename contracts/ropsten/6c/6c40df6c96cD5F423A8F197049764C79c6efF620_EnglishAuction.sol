...
contract EnglishAuction is UpdateProxy{
    ...
    mapping(address => mapping(uint => bool)) internal _claimed;
    ...
    function bid() external payable {
        ...
        _claimed[msg.sender][a.bidders] = false;
    ...
    function withdraw(uint _auctionNo,uint bidId) external {
        ...
        bids storage b = _noOfBidsPerPerson[msg.sender][_auctionNo];
        require(block.timestamp >= a.endAt,"time is not up yet to claim or withdraw" );
        require(_claimed[msg.sender][bidId] == false, "you have already claimed ");
        if(_bidPerPerson[msg.sender][bidId] > 0 && _claimed[msg.sender][bidId] == false){
            for(uint i =0; i < b._bids.length; i++ ){
                if(b._bids[i] == bidId){
                    uint bidderNumbers = a.bidders-5;
                    if(bidId <= bidderNumbers && a.bidders > 5 ){
                        (payable(msg.sender)).transfer(_bidPerPerson[msg.sender][bidId]);
                        _claimed[msg.sender][bidId] == true;
                        _bidPerPerson[msg.sender][bidId] = 0;
                        emit Lost(msg.sender);
                    }else if(bidId > bidderNumbers ){
        ...
        }else if(_bidPerPerson[msg.sender][bidId] == 0 && _claimed[msg.sender][bidId] == true){
            revert();
        }
    }
