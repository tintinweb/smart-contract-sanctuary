...
/****************************************************************************************************************************************
|* Weclome to the DYNAMICS TOKEN source code.                                                                                           |
|* This project supports the following:                                                                                                 |
|* 1. Token reflections.                                                                                                                |
|* 1. Automatic Liquidity reflections.                                                                                                  |
|* 1. Automated reflections of Ethereum to hodlers.                                                                                     |
|* 1. Token Buybacks.                                                                                                                   |
|* 1. A Token airdrop system where tokens can be injected directly back into pools for Liquidity, Ethereum reflections and Buybacks.    |
|* 1. Burning Functions.                                                                                                                |
|* 1. Token reflections.                                                                                                                |
|* 1. Token reflections.                                                                                                                |
|* 1. Token reflections.                                                                                                                |
...
//    bool enableLowLevelCalls = false; // TODO - Remove
...
//    address public buybackSinkAddress;
    ...
    address payable public _projectWallet = payable(0x56B4846E407171Dca6E575F2Efe634F131B56c0B); // TODO - change for live
    ...
    constructor (uint256 _supply, uint256 ADDTOLIQUIDITYTHRESHOLD) payable {
...
//            if(!inSwapAndLiquify && to != uniswapV2Pair && to != address(uniswapV2Router) && from != address(uniswapV2Router) && swapAndLiquifyEnabled){
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(buyBackEnabled && address(this).balance >= minBuyBack){
            emit BuyBackTriggered(0); // TODO - Remove - this is for testing
        ...
        } else if(contractTokenBalance >= numTokensSellToAddToLiquidity){
            ...
            if(redist >= buyback && redist >= liq){
                // Swap for redistribution
                emit SwapAndLiquify(1,1,1); // TODO - Remove - this is for testing
                uint256 ethBought = 0;
                ...
                // Swap for buyback
                emit SwapAndLiquify(2,2,2); // TODO - Remove - this is for testing
                contractTokenBalance = buyback;
                swapEthBasedFees(contractTokenBalance);
                buybackTokenTracker.buyback = buybackTokenTracker.buyback.sub(contractTokenBalance);
            } else {
                // Swap for LP
                emit SwapAndLiquify(3,3,3); // TODO - Remove - this is for testing
    ...
    function _transferStandard(address sender, address recipient, uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount) private {
        ...
        address(reflectionContract).call{gas: gasleft()}(abi.encodeWithSignature("logTransactionEvent(address, address)",sender, recipient));
    }
    ...
    function swapEthBasedFees(uint256 rAmount) private returns(uint256 ethBought){
        uint256 initialBalance = address(this).balance;
        _approve(address(this), address(uniswapV2Router), rAmount);
        // swap tokens for ETH
        swapTokensForEth(address(this), rAmount); // <- this breaks the ETH ->  swap when swap+liquify is triggered
...
//    }
    ...
    function initMint(uint256[] memory values, address ownerAddress) external onlyOwner {
        ...
        _rOwned[ownerAddress] = xferVal.mul(_getRate());
        runOnce = 1;
        emit Transfer(address(this), ownerAddress, xferVal);
    }
    ...
    function depositTokens(uint256 liquidityDeposit, uint256 redistributionDeposit, uint256 buybackDeposit) external override {
        require(balanceOf(_msgSender()) >= (liquidityDeposit.add(redistributionDeposit).add(buybackDeposit)).mul(_getRate()), "You do not have the balance to performe this action");
...
//    function allowUnsafeCalls(bool enable) external onlyOwner {
//        enableLowLevelCalls = enable;
//    }
}
