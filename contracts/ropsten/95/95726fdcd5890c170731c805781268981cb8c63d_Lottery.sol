...
contract Lottery {
    ...
    uint256 public constant DURATION = 20; //5520; // One Day
    uint256 public constant ROUND_DURATION = 10; //5300; 23 hours
	...
	function endRound() internal returns(uint32[6] numbers){
    ...
    function random(bytes32 _seed1, uint32 _seed2) public view returns(uint32 _number){
		if(blockhash(roundInfo[roundCounter - 1].targetBlock) == 0x0){
		   return (uint32(keccak256(_seed1,_seed2, block.timestamp,block.difficulty))%49) + 1;
		}
		return (uint32(keccak256(_seed1,_seed2))%49) + 1;
    } 
