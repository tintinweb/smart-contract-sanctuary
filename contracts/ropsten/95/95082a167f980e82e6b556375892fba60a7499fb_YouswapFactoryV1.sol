...
interface IYouswapFactoryV1 {
    ...
    priority：矿池排序
     */
    event UpdatePool(bool action, uint256 pool, address lp, string name, uint256 startBlock, uint256 rewardTotal, uint256 rewardPerBlock, uint256 multiple, uint256 priority);
    ...
    lp：lp合约地址
     */
    event EndPool(uint256 pool, address lp);
    ...
    amount：质押数量
     */
    event Stake(uint256 pool, address lp, address from, uint256 amount);
    ...
    amount：解质押数量
     */
    event UnStake(uint256 pool, address lp, address to, uint256 amount);
    ...
    amount：奖励数量
     */
    event WithdrawReward(uint256 pool, address lp, address to, uint256 amount);
    ...
    amount：奖励数量
     */
    event Mint(uint256 pool, address lp, uint256 amount);
...
contract YouswapFactoryV1 is IYouswapFactoryV1 {
    ...
    uint256 public poolCountttt;//矿池数量
    mapping(uint256 => PoolInfo) public poolInfo;//矿池信息
    mapping(uint256 => PoolViewInfo) public poolViewInfo;//矿池信息
    ...
    uint256 public constant inviteSelfReward = 5;//质押资奖励，5%
    ...
    function deposit(uint256 _pool, uint256 _amount) override external {
        ...
        require((address(0) != _poolInfo.lp) && (_poolInfo.startBlock <= block.number), ErrorCode.MINING_NOT_STARTED);
        ...
        IERC20(_poolInfo.lp).safeTransferFrom(msg.sender, address(this), _amount);
        ...
        emit Stake(_pool, _poolInfo.lp, msg.sender, _amount);
    }
    ...
    function withdraw(uint256 _pool, uint256 _amount) override external {
        ...
        if (0 < _amount) {
            ...
            emit UnStake(_pool, _poolInfo.lp, msg.sender, _amount);
        }
    ...
    function computeReward(uint256 _pool) override external {
        PoolInfo storage _poolInfo = poolInfo[_pool];
        if ((address(0) != _poolInfo.lp) && (_poolInfo.lastRewardBlock < block.number) && (0 < _poolInfo.totalPower) && (_poolInfo.rewardProvide < _poolInfo.rewardTotal)) {
            ...
            emit Mint(_pool, _poolInfo.lp, reward);

            if (0 < _poolInfo.endBlock) {
        ...
        if (address(0) != _poolInfo.lp && (_poolInfo.startBlock <= block.number)) {
            ...
            if ((0 < _userInfo.invitePower) || (0 < _userInfo.pledgePower)) {
                ...
                if (0 < reward) {
                    ...
                    emit WithdrawReward(_pool, _poolInfo.lp, _user, reward);
                }
            }
            ...
            if (address(0) != _upper1) {
                ...
                if ((0 < _upper1Info.invitePower) || (0 < _upper1Info.pledgePower)) {
                    ...
                    if (0 < reward) {
                        ...
                        emit WithdrawReward(_pool, _poolInfo.lp, _upper1, reward);
                    }
                }
                ...
                if (address(0) != _upper2) {
                    ...
                    if ((0 < _upper2Info.invitePower) || (0 < _upper2Info.pledgePower)) {
                        ...
                        if (0 < reward) {
                            ...
                            emit WithdrawReward(_pool, _poolInfo.lp, _upper2, reward);
                        }
                    }
                }
            }
    ...
    function powerScale(uint256 _pool, address _address) override external view returns (uint256) {
        ...
        return (_userInfo.invitePower.add(_userInfo.pledgePower).mul(100)).div(_poolInfo.totalPower);
    }

    function pendingReward(uint256 _pool, address _address) override external view returns (uint256) {
        PoolInfo storage _poolInfo = poolInfo[_pool];
        if (address(0) != _poolInfo.lp && (_poolInfo.startBlock <= block.number)) {
        }
        return pledgeUserInfo[_pool][_address].pledgePower;
    }
    ...
    function addPool(string memory _name, address _lp, uint256 _startBlock, uint256 _rewardTotal) override external returns (bool) {
        ...
        uint256 _pool = poolCountttt;
        poolCountttt = poolCountttt.add(1);
        ...
        _poolViewInfo.priority = _pool.mul(100);
        ...
        emit UpdatePool(true, _pool, _poolInfo.lp, _poolViewInfo.name, _poolInfo.startBlock, _poolInfo.rewardTotal, _poolInfo.rewardPerBlock, _poolViewInfo.multiple, _poolViewInfo.priority);
    ...
    function setInviteSelfReward(uint256 _pool, uint256 _inviteSelf) override external {
        ...
        PoolInfo storage _poolInfo = poolInfo[_pool];
        require((address(0) != _poolInfo.lp) && (0 == _poolInfo.endBlock), ErrorCode.MINING_NOT_STARTED);
   ...
   function setRewardTotal(uint256 _pool, uint256 _rewardTotal) override external {
        ...
        emit UpdatePool(false, _pool, _poolInfo.lp, _poolViewInfo.name, _poolInfo.startBlock, _poolInfo.rewardTotal, _poolInfo.rewardPerBlock, _poolViewInfo.multiple, _poolViewInfo.priority);
   }
   function setName(uint256 _pool, string memory _name) override external {
        require(operateOwner[msg.sender], ErrorCode.FORBIDDEN);
        PoolViewInfo storage _poolViewInfo = poolViewInfo[_pool];
        require(address(0) != _poolViewInfo.lp, ErrorCode.MINING_NOT_STARTED);
        _poolViewInfo.name = _name;
   ...
   function setMultiple(uint256 _pool, uint256 _multiple) override external {
    ...
    function setPriority(uint256 _pool, uint256 _priority) override external {
        ...
        PoolViewInfo storage _poolViewInfo = poolViewInfo[_pool];
        require(address(0) != _poolViewInfo.lp, ErrorCode.MINING_NOT_STARTED);
        _poolViewInfo.priority = _priority;
