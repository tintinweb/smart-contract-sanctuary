...
contract AthleteToken is ERC721 {
    ...
    uint256 public siteFee=20;
    uint256 public sendFee=5;
    ...
    function createOfAthleteCard(string _athleteId) public payable returns (uint256 _newAthleteId) {
        address _creator = msg.sender;
        uint256 _sellPrice = msg.value;
        
        require( _creator != address(0) );
        require( _creator != ceoAddress || _creator != cooAddress );
        if ( _sellPrice <= 0 ) {
            _sellPrice = initPrice;
        }
        Athlete memory _athlete = Athlete({ athleteId: _athleteId, creator: _creator, sellPrice: _sellPrice });
        uint256 newAthleteId = athletes.push(_athlete) - 1;
        
        require(newAthleteId == uint256(uint32(newAthleteId)));
        emit Birth(newAthleteId, _creator);

        _transfer(address(0), _creator, newAthleteId);
        return newAthleteId;
    }
    function changeSellPriceForAthlete( uint256 _tokenId, uint256 _newSellPrice ) public returns( string athleteId, address creator, uint256 sellPrice, address owner) {
    ...
    function getAthlete(uint256 _tokenId) public view returns ( string athleteId, address creator, uint256 sellPrice, address owner) {
        Athlete storage athlete = athletes[_tokenId];
        athleteId = athlete.athleteId;
        creator   = athlete.creator;
        sellPrice = athlete.sellPrice;
    ...
    function purchase(uint256 _tokenId) public payable {
        ...
        address actualWallet = athletes[_tokenId].creator;
        actualWallet.transfer(sendFee);
            
        ceoAddress.transfer(siteFee);
    }
    function priceOf(uint256 _tokenId) public view returns (uint256 price) {
        return athletes[_tokenId].sellPrice;
    }
