...
contract Task1 {
    ...
    uint256 initialPrice; // perFUNG in 6 decimals
    ...
    constructor(address _FLS, address _FUNG, uint256 _initial) public {
        owner = msg.sender;
        FLS = TRC20(_FLS);
        FUNG = TRC20(_FUNG);
        initialPrice = _initial;
        
        pairStatus[_FLS][_FUNG] = true;
        pairStatus[_FUNG][_FLS] = true;
 ...
 /*   function swap(address fromToken, address toToken, uint256 inAmount)  {
        require(pairStatus[fromToken][toToken], "Invalid Pair");
        
        uint256 outAmount;
        
        if(address(FLS) == fromToken) 
...
*/    
 ...
 function swap(address fromToken, address toToken, uint inAmount, uint outAmount) public {
        require(msg.sender == fromToken || msg.sender == toToken, "Not authorized");
        require(
            FLS.allowance(fromToken, address(this)) >= inAmount,
            "Token 1 allowance too low"
        );
        require(
            FUNG.allowance(toToken, address(this)) >= outAmount,
            "FUNG allowance too low"
        );

        _safeTransferFrom(FLS, fromToken, toToken, inAmount);
        _safeTransferFrom(FUNG, toToken, fromToken, outAmount);
 }
