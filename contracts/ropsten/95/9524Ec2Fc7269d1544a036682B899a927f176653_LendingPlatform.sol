...
contract LendingPlatform is Ownable {
    ...
    IPriceConsumer public priceConsumerUSDT;
    IPriceConsumer public priceConsumerDAI;
    ...
    constructor(IPriceConsumer _priceConsumerDAI, IPriceConsumer _priceConsumerUSDT) {
    ...
    function matchRequestAsLender(uint256 requestId) public {
        ...
        if (r.currency == Currency.DAI) {
            require(depositedDAI[msg.sender] >= r.cryptoAmount, "Not enough DAI deposited");
            depositedDAI[msg.sender] = depositedDAI[msg.sender] - r.cryptoAmount;
            depositedDAI[r.borrower] = depositedDAI[r.borrower] + r.cryptoAmount;
        } else {
            require(depositedUSDT[msg.sender] >= r.cryptoAmount, "Not enough USDT deposited");
            depositedUSDT[msg.sender] = depositedUSDT[msg.sender] - r.cryptoAmount;
            depositedUSDT[r.borrower] = depositedUSDT[r.borrower] + r.cryptoAmount;
        }
        
        if (genesisPhase) {
            require(myToken.transfer(msg.sender, amountToReward), 'Could not transfer tokens');
            require(myToken.transfer(r.borrower, amountToReward), 'Could not transfer tokens');
        }
    ...
    function matchRequestAsBorrower(uint256 requestId, uint256 ltv) public {
        ...
        depositedWEI[msg.sender] = depositedWEI[msg.sender] - r.weiAmount;

        if (genesisPhase) {
            require(myToken.transfer(msg.sender, amountToReward), 'Could not transfer tokens');
            require(myToken.transfer(r.lender, amountToReward), 'Could not transfer tokens');
        }
    ...
    function withdrawUSDT(uint256 _amount) public {
        ...
        depositedUSDT[msg.sender] = depositedUSDT[msg.sender] - _amount;
    ...
    function withdrawDAI(uint256 _amount) public {
        ...
        depositedDAI[msg.sender] = depositedDAI[msg.sender] - _amount;
    ...
    function withdrawETH(uint256 _amount) public {
        ...
        depositedWEI[msg.sender] = depositedWEI[msg.sender] - _amount;
        payable(msg.sender).transfer(_amount);
        
        emit CoinWithdrawn(msg.sender, _amount, Currency.ETH);
    }
    
        function _updateNexenTokenAddress(IERC20 _myToken) public onlyOwner {
        myToken = _myToken;
    }
    ...
    function _setGenesisPhase(IERC20 _myToken, bool _genesisPhase, uint256 _amountToReward) public onlyOwner {
        myToken = _myToken;
    ...
    function _recoverNexenTokens(uint256 _amount) public onlyOwner {
        require(myToken.transfer(msg.sender, _amount), 'Could not transfer tokens');
    }
