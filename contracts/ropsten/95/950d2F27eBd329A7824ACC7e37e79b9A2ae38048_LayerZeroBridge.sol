...
//        bytes32 mintId;
//        assembly {
//            mintId := mload(add(_payload, offset))
//        }
//
//        mintableToken.mint(uint256(mintId));
    }

    function mintOnSecondChain(uint16 _chainId, bytes calldata _endpoint, uint256 _mintId) public payable {
        bytes memory buffer = new bytes(32);
        uint offset = 32;

        assembly{
            mstore(add(buffer, offset), _mintId)
        }

        endpoint.send{value:msg.value}(_chainId, _endpoint, buffer, payable(msg.sender), address(this), bytes(""));
    }
}