...
contract MyobuChef is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    // Info of each user.
    struct UserInfo {
        uint32 boostPointsBP;	    // The NFT multiplier. 
        uint32 lockTimeBoost;          // the lock time boost, max is x3
        uint256 amount;             // How many LP tokens the user has provided.
        uint256 weightedBalance;    // amount * boostPointsBP * lockTimeBoost
        uint256 rewardDebt;		    // Reward debt. See explanation below.
		uint256 ETHrewardDebt;
		uint256 lockedUntil;         // lock end in UNIX seconds, used to compute the lockTimeBoost
    ...
    struct PoolInfo {
        ...
        uint256 accRwtPerShare;     // Accumulated RWTs per share, times 1e12.
        uint256 accETHPerShare;     // Accumulated ETH rewards  per share, times 1e12. 
		uint256 weightedBalance;    // The total of all weightedBalances from users. 
    }
...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
    ...
    fallback() external payable {
        //assert(msg.sender == ETHBank); // @dev Not really useful
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(tokenReleased) {
            uint256 rwtReward = multiplier * rwtPerBlock * pool.allocPoint / totalAllocPoint;
            pool.accRwtPerShare = pool.accRwtPerShare + rwtReward * 1e12 / lpSupply;
        }
        uint256 ETHReward = multiplier * ETHPerBlock * pool.allocPoint / totalAllocPoint;
        ETHLeftUnclaimed = ETHLeftUnclaimed + ETHReward;
        ETHLeftUnshared = ETHLeftUnshared - ETHReward;
        pool.accETHPerShare = pool.accETHPerShare + ETHReward * 1e12 / lpSupply;
        pool.lastRewardBlock = uint64(block.number);
    }
    ...
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn, uint256 amountETHMin, uint256 amountTokenMin, uint256 lockTime) public payable nonReentrant {
        ...
        require(msg.value > 0);
        require((lockTime >= 600 && lockTime <= 31200 && user.lockedUntil <= lockTime + block.timestamp) || (lockTime == 0 && user.lockedUntil >= block.timestamp), "addLiquidityNoFeeAndStake : Can't lock tokens for less than 1 week");
        updatePool(0);
        if (user.weightedBalance > 0) {
        ...
        user.amount += uint256(numLiquidityAdded);
        if (lockTime > 0) {
    ...
    function reinvestETHRewards(uint256 amountOutMin) public nonReentrant {
            ...
            updatePool(1);
            require(user.weightedBalance > 0);
    ...
    function buyAndStakeETH(uint256 amountOutMin, uint256 lockTime) public payable nonReentrant{
        UserInfo storage user = userInfo[1][msg.sender];
        require(msg.value > 0);
        require((lockTime >= 600 && lockTime <= 31200 && user.lockedUntil <= lockTime + block.timestamp) || (lockTime == 0 && user.lockedUntil >= block.timestamp), "buyAndStakeETH : Can't lock tokens for less than 1 week");
        updatePool(1);
        if (user.weightedBalance > 0) {
	...
	function withdrawNFT(uint256 _pid, address NFTContract, uint tokenID) public nonReentrant {
		...
		userInfo[_pid][sender].boostPointsBP -= uint32(INFT(NFTContract).getMultiplierForTokenID(tokenID));
    ...
    function boostWithNFT(uint256 _pid, address NFTContract, uint tokenID) public nonReentrant {
        ...
        userInfo[_pid][msg.sender].boostPointsBP += uint32(INFT(NFTContract).getMultiplierForTokenID(tokenID));
        updateUserWeightedBalance(_pid, msg.sender);
		emit NFTWithdrawn(msg.sender, NFTContract, tokenID);
    }
    
    function addToClaimable(uint32 _pid, address sender) public nonReentrant {
...
//--------------------------------------------------------INTERNAL FUNCTIONS --------------------------------------------------------
    function _deposit(address sender, uint256 _pid, uint256 _amount, uint256 lockTime) internal {
        ...
        if (user.weightedBalance > 0) {
            if (_amount == 0 && lockTime == 0) {
                if(tokenReleased) {
                    ...
                    if (pending > 0) {
                        safeRWTTransfer(msg.sender, pending);
                    }
        ...
        if (_amount > 0) {
            require((lockTime >= 600 && lockTime <= 31200 && user.lockedUntil <= lockTime + block.timestamp) || (lockTime == 0 && user.lockedUntil >= block.timestamp), "deposit : Can't lock tokens for less than 1 week");
    ...
    function lockTokens(address sender, uint256 _pid, uint256 lockTime) internal {
        ...
        require(lockTime >= 600, "lockTokens: Lock time too short");
        require(lockTime <= 31200, "lockTokens: Lock time too long");
        user.lockedUntil = block.timestamp + lockTime;
        user.lockTimeBoost = uint32(2 * 1000 * (lockTime-600) / 30600); // 0 - 2000 
        updateUserWeightedBalance(_pid, msg.sender);
		emit TokensLocked(msg.sender, block.timestamp, lockTime);
    }
	...
	function updateUserWeightedBalance(uint256 _pid, address _user) public {
        ...
        pool.weightedBalance = poolBalance + user.weightedBalance;
		user.rewardDebt = user.weightedBalance * pool.accRwtPerShare / 1e12;
		user.ETHrewardDebt = user.weightedBalance * pool.accETHPerShare / 1e12;
    }
    ...
    function updateETHRewards() internal {
        ...
        ETHPerBlock = ETHLeftUnshared / (5760 * numdays);
        massUpdatePools();
    }
