...
contract HopeDexV0 is Initializable, AccessControl{
    ...
    //FIXME
    uint constant BIRTHDAY = 1607616210;                            // Now time when the contract was deployed
    uint constant MIN_ORDER_USD = 50 * 10 ** 18;                    // Minimum order size is $50 in USD precision
    uint constant MAX_ORDER_USD = 500 * 10 ** 18;
    //uint constant MAX_THRESHOLD_USD = 10 * 10 ** 6;                // Order limits removed when Hope_USD exceeds $10
    //uint constant MAX_ORDER_FACTOR = 5000;                          // Max Order Volume will be 5K * Hope_usd until threshold ($10).  
    ...
    //FIXME
    uint constant _Hope_USD = 1030100; //1335574612014710427;                  // Hope:USD exchange rate last recorded
    ...
    uint public ETH_USD = 1524070103464304062291;                                            // Last recorded ETH_USD rate
    ...
    modifier onlyOracle {
        require(hasRole(ORACLE_ROLE, msg.sender), "HopeDex 403 oracle");
        _;
    }
    
    modifier onlyMigration {
        require(hasRole(MIGRATION_ROLE, msg.sender), "HopeDex 403 migration");
        _;
    }
    
    modifier onlyReserve {
        require(hasRole(RESERVE_ROLE, msg.sender), "HopeDex 403 reserve.");
        _; 
    }
    ...
    modifier checkWaitTime() {
        require(users[msg.sender].time < SELL_WAIT_TIME, "Not strated.");
        _;
    }

    event HopeTIssued(address indexed user, uint amountUsd, uint amountHope);
    event HopeTRedeemed(address indexed user, uint amountUsd, uint amountHope);
    ...
    event SetETHUsd(address setter, uint ETHUsd6);
    ...
    function adminSetSellOrderLimit(uint limit) external onlyAdmin{
        require(limit > 0);
    ...
    function sellHOPE(uint quantityHope) external checkWaitTime accrueByTime ifRunning returns(bytes32 orderId) {
        ...
        orderId = _openSellOrder(remainingHope);
    }
    ...
    function _openSellOrder(uint quantityHope) private returns(bytes32 orderId) {
        ...
        if(convertHopeToUsd(quantityHope) > MIN_ORDER_USD && buyOrderIdFifo.count() == 0) { 
            orderId = keyGen();
    ...
    function buyHOPE(uint amountETH, uint256 parentId) external accrueByTime ifRunning{
        ...
        // update quotas
        users[msg.sender].quota += orderUsd.div(2); //50%
        users[userIds[parent]].quota += orderUsd.div(4); //25%

        uint256 parentETH = amountETH.mul(15).div(100); //15%
        balance.add(ETH_ASSET, userIds[parent], parentETH, 0);

        uint remainingETH = _fillSellOrders(amountETH.sub(parentETH));
    ...
    function _setETHToUsd() private returns(uint ETHUsd6) {
        ...
        emit SetETHUsd(msg.sender, ETHUsd6);
    ...
    function maxOrderAmountUSD() public view returns(uint){
        if(Hope_USD < 10e6){ //100k transactions
    ...
    function checkQuota(uint256 usdAmount) private{
        if(users[msg.sender].period > 0){
            ...
            require(users[msg.sender].roundAmount <= users[msg.sender].quota, "Quota exceeded.");
        }
    }
...
contract HopeDex is HopeDexV0 {
    using SafeMath for uint;

    uint public HOPEStakingDuration = 14 days;
    ...
    function initUser(
        ...
        uint _stakeBalance,
        uint _stakeDate
    ) external onlyMigration payable {
        balance.add(ETH_ASSET, _userAddr, msg.value, 0);
