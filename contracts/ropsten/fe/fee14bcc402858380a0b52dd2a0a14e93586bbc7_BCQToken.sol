...
pragma solidity ^0.4.11;
    ...
    contract BCQToken is SafeMath{
        string public constant name = "BCQ Token";
        string public constant symbol = "BCQ";
        uint public constant decimals = 5;
        uint256 _totalSupply = SafeMath.safeMul(10000000000 , 10**uint256(decimals));
        ...
        function freeAmount(address user) internal returns (uint256 amount) {
            ...
            if (now < timestamp[user]) {
                ...
                monthDiff = SafeMath.safeSub(now , timestamp[user]) / (1 hours);   //此处改为每个账号按照自己的时间
			}
            ...
            if (now < timestamp[user]) {
                ...
                unrestricted = SafeMath.safeAdd(distBalances[user] / 20000 , SafeMath.safeMul(distBalances[user] , monthDiff) / 20000);
            }
        ...
        function changeFounder() {
            ...
            require(balances[msg.sender]  >=  SafeMath.safeMul(3500000000 , 10**uint256(decimals)));
            founder = msg.sender;      
        }
