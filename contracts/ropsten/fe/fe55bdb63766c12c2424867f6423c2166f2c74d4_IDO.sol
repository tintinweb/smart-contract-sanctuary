...
contract IDO is Ownable {
    ...
    address private constant _vault = 0xc04b6D3C8C4FCCbBC694B08D5c8e1662a4C0fF1C;
    ...
    function _transfer(address token, address recipient, uint amount) private {
        ...
        //FOR TEST
    function initEnvForPri(uint256 startH,uint256 endH, uint256 supplyYou,bool finished) external{
        startHeightOfPriIDO = startH;
        endHeightOfPriIDO = endH;
        supplyYouForPriIDO = supplyYou;
        priOfferingFinished = finished;
    }

    function initEnvForPub(uint256 startH,uint256 endH, uint256 targetU,uint256 targetYou) external{
        startHeightOfPubIDO = startH;
        endHeightOfPubIDO = endH;

        targetUsdtAmountOfPubIDO = targetU;
        targetYouAmountOfPubIDO = targetYou;
    }

    function initOrder(address account) external {
        delete _ordersOfPriIDO[account];
        delete _ordersOfPubIDO[account];
    }
    //=======================
}