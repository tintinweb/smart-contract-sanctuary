...
contract PartyFactory is Ownable {
    ...
    event CreateEvent(address caller, string partyName, string partyId, address partyAddress, string ownerId, string joinPartyId, uint256 initialDeposit, uint256 cut);
    event WithdrawEvent(address userAddress, uint256 withdrawPercentage, address partyAddress, uint256 amount);
    event LeavePartyEvent(address userAddress, address partyAddress);
    event KickPartyEvent(address userAddress, address kickedAddress, address partyAddress);
    event ClosePartyEvent(address partyAddress, address ownerAddress);
    event Qoute0xSwap(IERC20 sellTokenAddress, IERC20 buyTokenAddress, address partyAddress, address swapTarget, string transactionType, uint256 sellAmount, uint256 buyAmount);
    ...
    function createParty(
        ...
        emit CreateEvent(msg.sender, _partyInfo.partyName, _partyInfo.idParty, partyClone, _partyInfo.ownerId, _joinPartyId, _initialDeposit, cut);
    ...
    function fillQuote(
        ...
        if (sellTokenAddress == tokenAddress) {
            emit Qoute0xSwap(sellTokenAddress, buyTokenAddress, _partyAddress, swapTarget, "BUY", sellAmount, buyAmount);
        } else {
            emit Qoute0xSwap(sellTokenAddress, buyTokenAddress, _partyAddress, swapTarget, "SELL", sellAmount, buyAmount);
        }
    }
    ...
    function leaveParty(
        ...
        Party(_partyAddress).leaveParty(_partyAddress, msg.sender, _n, _tokenData, _leaveRSV);
        emit LeavePartyEvent(msg.sender, _partyAddress);
    }
    function kickParty(
        address _partyAddress,
        ...
        Party(_partyAddress).kickParty(_partyAddress, msg.sender, _quittingMember, _n, _tokenData, _kickRSV);
        emit KickPartyEvent(msg.sender, _quittingMember, _partyAddress);
    }
