...
contract MOFDiscover {
    ...
    function checkAddress(address _nfts, uint256 _min, uint256 _max) public view returns (address[] memory) {
        ERC721 _nftCollectionAddress = ERC721(_nfts);
        address[] memory cti = new address[](_max - _min);
    ...
    function checkUri(address _nfts, uint256 _min, uint256 _max) public view returns (string[] memory) {
        ERC721 _nftCollectionAddress = ERC721(_nfts);
        string[] memory cti = new string[](_max - _min);
        for(uint256 i = _min; i < _max; i++) {
