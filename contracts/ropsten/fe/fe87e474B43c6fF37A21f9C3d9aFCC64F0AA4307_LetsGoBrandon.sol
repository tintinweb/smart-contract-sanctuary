...
contract LetsGoBrandon is Context, IERC20, Ownable {
	...
	uint256 public feeProcessingThreshold = tTotal / 100000;
	...
	constructor() {
		...
		developmentAddress = payable(0xe3b985844AC8948a730d7C61e80DFC0e9dfA0BDd);
	...
	function _processFees(uint256 amount) private lockTheSwap {
		uint256 feeSum = fees.marketingFee + fees.liquidityFee + fees.developmentFee;
		if (feeSum == 0) return;

		//Calculate amounts
		uint256 amountForMarketing = amount * ((fees.marketingFee + fees.developmentFee) / feeSum);
...
//
		...
		//uint256 marketingInEth = balanceEth * (fees.marketingFee / (fees.marketingFee + fees.developmentFee));
	...
	function _processFeesTEST(uint256 amount) public view returns(uint256,uint256) {
		uint256 feeSum = fees.marketingFee + fees.liquidityFee + fees.developmentFee;
		if (feeSum == 0) return(0,0);

		//Calculate amounts
		uint256 amountForMarketing = amount * (fees.marketingFee + fees.developmentFee) / feeSum;
		uint256 amountForLiquidity = amount - amountForMarketing;
		return (amountForMarketing,amountForLiquidity);
		//Do processing
		//_swapExactTokensForETH(amountForMarketing);
		//_addLiquidity(amountForLiquidity);
//
