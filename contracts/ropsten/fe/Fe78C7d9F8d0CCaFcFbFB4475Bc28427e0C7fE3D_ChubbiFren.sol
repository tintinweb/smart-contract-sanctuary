...
contract ChubbiAuction is ChubbiReserve {
    ...
    // The amount of tokens a user is allowed to have before we reject their bids.
    uint256 public userTokenLimit;

    // The count of how many tokens were bid successfully by an address
    mapping(address => uint256) internal userTokenCount;
     ...
     */
    function setUserTokenLimit(uint256 _amount) external onlyOwner {
        require(_amount > 0, "Invalid limit");
        userTokenLimit = _amount;
    }

    /**
     * @dev Get the token limit for a specific address.
     */
    function getUserTokenLimit(address _address)
        external
        view
        returns (uint256)
    {
        return userTokenLimit - userTokenCount[_address];
    }
    ...
    function bid(uint256 _tokenAmount) external payable whenNotPaused {
        ...
        require(
            userTokenCount[msg.sender] < userTokenLimit,
            "Cannot bid for more tokens"
        );

        // Only allow user to buy up to their maximum limit
        uint256 remainingBalance = userTokenLimit.sub(
            userTokenCount[msg.sender]
        );
        uint256 amountToBuy = Math.min(_tokenAmount, remainingBalance);
        assert(amountToBuy <= remainingBalance);
        ...
        amountToBuy = Math.min(amountToBuy, tokensRemaining);
        ...
        // Update our state before giving user tokens
        userTokenCount[msg.sender] = userTokenCount[msg.sender].add(
            amountToBuy
        );
    ...
    function _getCurrentPrice(
        ...
        // Every 100 blocks reduce the price by 0.05 ether
        uint256 priceDiff = _currentBlock.sub(_startBlock).div(100).mul(
            0.05 ether
        );
