...
contract LostPunks is ERC721Enumerable, Ownable {
    ...
    function mintLostPunk(uint16 fatherIndex, uint16 motherIndex) external {
        ...
        uint16 fatherChild1 = child1Mapping[fatherIndex];
        uint16 motherChild1 = child1Mapping[motherIndex];
        if (fatherChild1 != 0) {
            require(motherMapping[fatherChild1] != motherIndex, "Cannot mint children for siblings");
        }
        if (motherChild1 != 0) {
            require(fatherMapping[motherChild1] != fatherIndex, "Cannot mint children for siblings");
        }
