...
abstract contract BaseRegistrar is Ownable, IERC721 {
    ...
    // The namehash of the TLD this registrar owns (eg, .myst)
    ...
    /**
     * @dev Reclaim ownership of a name in MYST, if you own it in the registrar.
     */
...
contract BaseRegistrarImplementation is ERC721, BaseRegistrar  {
    ...
    constructor(MYST _myst, bytes32 _baseNode) ERC721("","") {
        myst = _myst;
        baseNode = _baseNode;
    }

    modifier live {
        require(myst.owner(baseNode) == address(this));
        _;
    }
    ...
    function setResolver(address resolver) external override onlyOwner {
        myst.setResolver(baseNode, resolver);
    }
    ...
    function _register(uint256 id, address owner, uint duration, bool updateRegistry) internal live onlyController returns(uint) {
        ...
        _mint(owner, id);
        if(updateRegistry) {
            myst.setSubnodeOwner(baseNode, bytes32(id), owner);
        }
 ...
 */
contract MYSTRegistrarController is Ownable {
    ...
    function registerWithConfig(string memory name, address owner, uint duration, bytes32 secret, address resolver, address addr) public payable {
        ...
        if(resolver != address(0)) {
            ...
            // Set the resolver
            base.myst().setResolver(nodehash, resolver);
