...
contract TimeDistributionMon is Ownable {
	...
	require(_timePoints.length >= 2, "TimeDistribution::addInfo: time points should not null");
        require(
    ...
    // careful gas
    function addMultiInfo(address[] memory accounts, uint256[] memory amounts, uint256[] memory beginTsArray, uint256[] memory endTsArray, uint256[][] memory _timePoints) public onlyOwner {
        ...
        for(uint256 i=0; i < accounts.length; i++) {
            addInfo(accounts[i], amounts[i], beginTsArray[i], endTsArray[i], _timePoints[i]);
        }
    }

    function getTimePoint(address account) public view returns (uint256) {
        uint[] memory timePoints = infos[account].timePoints;
        if (block.timestamp <= timePoints[0]) {
            ...
            for(uint i; i < timePoints.length; i++) {
                if (block.timestamp >= timePoints[i] && block.timestamp < timePoints[i+1]) {
                    return timePoints[i];
                }
            }
        }
