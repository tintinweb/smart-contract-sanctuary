 ...
 */
contract AlkimiStaking is IStaking, Ownable, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */
    uint256 public constant override STAKING_CAP = 30_000_000E18;
    uint256 public constant override REWARDS_DURATION = 1 hours;
    ...
    constructor(
        ...
        stakingPeriodEnd = block.timestamp.add(1 hours);
    }
    ...
    function tokensUnlockedTimestamp() external view override returns (uint256) {
        return (rewardsPeriodStart > 0) ? rewardsPeriodStart.add(15 minutes) : 0;
    }
    ...
    function grossEarnings(address account) public view override returns (uint256 earnings) {
        return stakedAmount[account].mul(rewardAmount).mul(lastTimeRewardApplicable().sub(rewardsPeriodStart)).div(totalStaked).div(1 hours);
    }
    ...
    function APY(address account) external view override returns (uint256) {
        ...
        { // avoid Stack too deep
            ...
            unclaimedReward = (timeSinceRewardsStarted > 0) ? rewardRemaining.sub(rewardRemaining.mul(timeSinceRewardsStarted).div(1 hours)) : 1;
            timeNormalizationFactor = (timeSinceRewardsStarted != 1 hours && timeSinceRewardsStarted != 0) ? SafeMath.div(SafeMath.mul(1 hours, 1E5), SafeMath.sub(1 hours, timeSinceRewardsStarted)) : 1;
        ...
        return (rewardAmount > 0 && stakeAmount > 0) ? (stakeAmount.mul(1E5).mul(unclaimedReward).div(totalStaked).div(stakeAmount)).mul(timeNormalizationFactor).mul(rewardNormalizationFactor).div(1E10) : 0;
    }
    ...
    function unstake() external override nonReentrant updateReward(msg.sender) {
        ...
        require(block.timestamp >= rewardsPeriodStart.add(15 minutes), "Cannot unstake until three months after rewards active");
    ...
    function increaseStakingPeriod(uint256 numDays) external override onlyOwner {
        stakingPeriodEnd = stakingPeriodEnd.add(numDays.mul(1 minutes));
        emit StakingWindowIncrease(numDays);
    }

		/**
     * @dev Sets periods dates.
     */
    function setPeriods(uint256 stakingStart, uint256 stakingEnd, uint256 rewardsStart, uint256 rewardsEnd) external onlyOwner {
				stakingPeriodStart = stakingStart;
        stakingPeriodEnd = stakingEnd;
        rewardsPeriodStart = rewardsStart;
        rewardsPeriodEnd = rewardsEnd;
    }
