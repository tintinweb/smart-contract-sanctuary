 ...
 * allowing purchasers to purchase tokens with ether.
 ...
 * contribution of the purchaser needs to be of at least 5 ETH. A maximum of 15 million tokens
 ...
 * 
 * Refunds:
 * A purchaser can be refunded by the owner. Calling the refund function resets the tokens bought
 * to zero for that purchaser. The Ether refund needs to be processed manually. It is important
 * to record how many tokens the purchaser had bought before calling refund().
 *
*/
contract Crowdsale {
  ...
  /**
   * Event for private purchase logging
   * @param purchaser who paid for the tokens
   * @param beneficiary who got the tokens
   * @param value weis paid for purchase
   * @param amount amount of tokens purchased
   * @param rate the rate at which the tokens were purchased
   */
  event PrivatePurchase(address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount, uint256 rate);
   ...
   * @param rate the rate at which the tokens were purchased
   */
  event TokenRefund(address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount, uint256 rate);
  ...
  /**
   * Buy a token at presale price. Converts ETH to as much tokens the sender can purchase. Any change
   ...
   * @param _beneficiary the purchaser
   * @param _presaleTokens the number of tokens which the purchaser has bought
   */
  function recordPresalePurchase(address _beneficiary, uint256 _presaleTokens) public onlyOwner {
    uint256 _weiPaid = _presaleTokens.div(presaleRate);
    weiRaised = weiRaised.add(_weiPaid);

    emit PrivatePurchase(_beneficiary, _beneficiary, _weiPaid, _presaleTokens, presaleRate);
  ...
  function transferTokens() public onlyOwner {
    for (uint256 i = 0; i < purchasers.length; i = i.add(1)) {
      address purchaser = purchasers[i];

      uint256 tokens = regularTokensSold[purchaser];
      uint256 presaleTokens = presaleTokensSold[purchaser];
      
      regularTokensSold[purchaser] = 0;
      presaleTokensSold[purchaser] = 0;

      if (tokens > 0) {
        _deliverTokens(token, purchaser, tokens);
      }

      if (presaleTokens > 0) {
        _deliverTokens(token, purchaser, presaleTokens);
      }
    }
  }
  ...
  function transferTokensWithOffsetAndLimit(uint256 offset, uint256 limit) public onlyOwner {
    for (uint256 i = offset; i <  _min256(purchasers.length,offset+limit); i = i.add(1)) {
      address purchaser = purchasers[i];
  ...
  /**
   * Clears the number of tokens bought by an purchaser. The ETH refund needs to be processed
   * manually.
   */
  function refund(address _purchaser) onlyOwner public {
    require(_purchaser != owner);

    uint256 _regularTokens = regularTokensSold[_purchaser];
    totalRegularTokensSold = totalRegularTokensSold.sub(_regularTokens);
    uint256 _regularWeiRefunded = _regularTokens.div(rate);
    weiRaised = weiRaised.sub(_regularWeiRefunded);

    uint256 _presaleTokens = presaleTokensSold[_purchaser];
    totalPresaleTokensSold = totalPresaleTokensSold.sub(_presaleTokens);
    uint256 _presaleWeiRefunded = _presaleTokens.div(presaleRate);
    weiRaised = weiRaised.sub(_presaleWeiRefunded);

    regularTokensSold[_purchaser] = 0;
    presaleTokensSold[_purchaser] = 0;

    if (_regularTokens > 0) {
      emit TokenRefund(_purchaser, _purchaser, _regularWeiRefunded, _regularTokens, rate);
    }
    if (_presaleTokens > 0) {
      emit TokenRefund(_purchaser, _purchaser, _presaleWeiRefunded, _presaleTokens, presaleRate);
    }
  ...
  * Accessor for Index
  */
  function getpurchaserAtIndex(uint256 _index) public view returns(address) {
    return purchasers[_index];
  }

  /**
  * Return the length of the purchasers array
  */
  ...
  /**
   * Get the number of tokens which an purchaser can purchase at presale rate.
   */
  function getPresaleAllocation(address purchaser) view public returns(uint256) {
    return presaleAllocations[purchaser];
  }

  /**
   * Set the number of tokens which an purchaser can purchase at presale rate.
   */
  function setPresaleAllocation(address purchaser, uint allocation) onlyOwner public {
    presaleAllocations[purchaser] = allocation;
  }
