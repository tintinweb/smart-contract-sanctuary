...
// File: contracts/BytesLibrary.sol
...
contract Exchange is OrderState, TransferSafe {
	...
	IDirtyPunkStore private _dirtyPunkStore;
	address public weth;
		...
		if (
			...
			if (order.orderType == 1 || order.orderType == 3) {
				if (!isStore) {
					...
					require(
						order.key.sellAsset.token == address(_dirtyPunkStore),
						"invalid sell asset"
					);
					erc1155safeMintTransferFrom(
						IDirtyPunkStore(order.key.sellAsset.token),
						order.key.owner,
						buyer,
						order.key.sellAsset.tokenId,
						1,
				...
				if (!isStore) {
					...
					require(
						order.key.buyAsset.token == address(_dirtyPunkStore),
						"invalid buy asset"
					);
					erc1155safeMintTransferFrom(
						IDirtyPunkStore(order.key.buyAsset.token),
						buyer,
						order.key.owner,
						order.key.buyAsset.tokenId,
						1,
	...
	function setMintableStore(address newMintableStore) external onlyOwner {
		...
		_dirtyPunkStore = IDirtyPunkStore(newMintableStore);
	...
	function dirtyPunkStore() external view returns (address) {
		return address(_dirtyPunkStore);
	}
}