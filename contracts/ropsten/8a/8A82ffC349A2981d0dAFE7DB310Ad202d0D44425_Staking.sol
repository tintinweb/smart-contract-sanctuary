...
contract Staking is ReentrancyGuard, Context, IStaking {
    ...
    function claimDividends(uint256 stakeIndex, uint256 maxDepth) external override {
        ...
        if (newIncomeIndex == poolInfo[tokenStaked].incomeInfo[timeIntervalIndex].length &&
            block.timestamp >=
            stakeInfo[sender][stakeIndex].timestampStakeStart + _TIME_DURATIONS[timeIntervalIndex]
        ) {
            AssetLib.safeTransfer(tokenStaked, sender, amountStaked);
    ...
    function _inputBnb(address token, uint256 amount, bool isForce) private {
        ...
        if (amount == 0 ||
            (isForce == false && amount < minAmountInPoolToDistributed)
        ) {
    ...
    function _withdrawTokenToBnb(
        ...
        if (token != goodToken) {
            ...
            uint256 amountToken;
            uint256 amountETH;
            try IPancakeRouter02(dexRouter).removeLiquidityETH(
                goodToken,
                amount,
                0,
                0,
            ...
            result += amountETH;
            (uint256 result2, bool isValid2) = _withdrawTokenToBnb(goodToken, goodToken, amountToken);
            if (isValid2 == true) {
                result += result2;
            } else {
            ...
            path[0] = token;
            path[1] = weth;
            try IPancakeRouter02(dexRouter).swapExactTokensForETH(
                amount,
                0,
                path,
                address(this),
