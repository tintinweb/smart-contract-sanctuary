...
contract PunkGenerations {
    ...
    function borderImage(bytes memory pixels) internal pure returns (bytes memory) {
        ...
        return result;
/*                bool onOff = (pixels[p] != 0x0) || (pixels[p+1] != 0x0) || (pixels[p+2] != 0x0) || (pixels[p+3] != 0x0);
                if (!previousOnOff && onOff && x > 0) {
                    result[p-4] = 0xFF;
                    result[p-3] = 0x04;
                    result[p-2] = 0xB4;
                    result[p-1] = 0xFF;
                }
                if (previousOnOff && !onOff) {
                    result[p] = 0xFF;
                    result[p+1] = 0x04;
                    result[p+2] = 0xB4;
                    result[p+3] = 0xFF;
        ...
        for (uint8 x = 0; x < 24; x++) {
            ...
            for (uint8 y = 0; y < 24; y++) {
                uint p = (y * 24 * 4) + (x * 4);
                bool onOff = (pixels[p] != 0x0) || (pixels[p+1] != 0x0) || (pixels[p+2] != 0x0) || (pixels[p+3] != 0x0);
                if (!previousOnOff && onOff && y > 0) {
                    result[p - 24*4] = 0xFF;
                    result[p - 24*4 + 1] = 0x04;
                    result[p - 24*4 + 2] = 0xB4;
                    result[p - 24*4 + 3] = 0xFF;
                } else if (previousOnOff && !onOff) {
                    result[p] = 0xFF;
                    result[p+1] = 0x04;
                    result[p+2] = 0xB4;
                    result[p+3] = 0xFF;
    ...
    function punkAssetsImageSvg(bytes memory punkAssets) internal view returns (string memory svg) {
        bytes memory pixels = borderImage(punkAssetsImage(punkAssets));
