...
contract MyTokenWalletAdmin {
    ...
    function listToken(address tokenAddress) public owned returns(bool) {
        require(address(0) != tokenAddress);
        return wallet.addToken(tokenAddress);
    }
    function checkToken(address tokenAddress) public view returns(bool) {
        return wallet.acceptToken(tokenAddress);
    }
    function delistToken(address tokenAddress) public returns(bool) {
        require(wallet.acceptToken(tokenAddress));
        return wallet.removeToken(tokenAddress);
    }
