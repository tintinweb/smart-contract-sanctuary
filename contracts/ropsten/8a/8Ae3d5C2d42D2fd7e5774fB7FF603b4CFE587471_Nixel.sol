...
contract Nixel is ERC721Enumerable, Ownable {
    ...
    function transferSpawn() public onlyOwner {
        setBlockOwner(_msgSender(), 0, 0);
        setBlockTokenId(0, 0, totalBlocks + 1);
        setBlockLocation(totalBlocks + 1, 0, 0);
        _NFTName[0][0] = "Nixel Spawn";
        emit MintNixel(_msgSender(), totalBlocks + 1, 0, 0);
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    mapping(int256 => mapping(int256 => uint256)) private _blockTokenId;

    // Mapping for tokingId to block location
    mapping(uint256 => int256[]) private _blockLocation;
     ...
     * @param y_pos int location of the token to query // e.g. location_y = "1"
     */
    function setBlockTokenId(int256 x_pos, int256 y_pos, uint256 tokenId) internal virtual
    {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not owner nor approved");
        _blockTokenId[x_pos][y_pos] = tokenId;
    }
     ...
     */
    function setBlockLocation(uint256 tokenId,int256 x_pos, int256 y_pos) internal virtual
    {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not owner nor approved");
        _blockLocation[tokenId] = [x_pos,y_pos];
    }

    
    /**
     * @dev Returns Data for a given token location.
     * @param owner address of the new block owner
     * @param x_pos int location of the token to query // e.g. location_x = "0"
     * @param y_pos int location of the token to query // e.g. location_y = "1"

     */
    function setBlockOwner(address owner,int256 x_pos, int256 y_pos) internal virtual
    {
        require(_blockOwner[x_pos][y_pos] == _msgSender() || _blockOwner[x_pos][y_pos] == address(0), "ERC721: caller is not owner");
        _blockOwner[x_pos][y_pos] = owner;
    }
