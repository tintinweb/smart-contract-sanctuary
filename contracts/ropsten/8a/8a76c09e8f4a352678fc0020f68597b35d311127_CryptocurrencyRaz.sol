...
contract CryptocurrencyRaz is Ownable {
    ...
    mapping (address=>WinOrLose[]) public pastWinnings;
    mapping (address=>WinOrLose[]) public pastLosings;
    
    uint[] razList;
    uint[] empty;
    
    WinOrLose[] winOrLoseArray;
    WinOrLose WinOrLoseObject;
    ...
    function resetRaz(uint razNumber,address winningAddress, uint winningNumber) internal 
    {
        for (uint i=1;i<=maxBetsForEachRaz[razNumber];i++)
        {
            if (numbersTaken[razNumber][i] == winningAddress && i == winningNumber)
            {
                ...
                WinOrLoseObject = WinOrLose({timestamp:now,razNo:razNumber});
                winOrLoseArray.push(WinOrLoseObject);
