...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    {
        require(_owner == msg.sender , "Not Accessable");
    ...
    function Buy11()payable public returns(bool){
        uint256 rate = 40;
        require(msg.value <= 1e18, "maximum buying is limited to 1 eth");
        uint256 amount = msg.value*rate/1e10;
        require(_balances[msg.sender]+amount<=40e8);
        require(_totalSupply<=40e8);
        _mint(msg.sender,amount);
        return (true);
       ...
       function Buy2()payable public returns(bool){
        require(msg.value <= 1e18, "maximum buying is limited to 1 eth");
        uint256 amount = msg.value/(25e7);
        _mint(msg.sender,amount);
        return (true);
        
    }
    
    function getprice(uint256 tokensss)public pure returns (uint256){
       uint256 price = 1e10/40;
        return tokensss*price;
    }
    ...
    function claimlocked()public returns(bool success){
        ...
        if(msg.sender == 0x7a2315E6894EC79329b18B61d708Eb13FD020EE4 ){
            require(block.timestamp >= contractcreationtime + 365 minutes && !first,"time limit not reached ");
            _mint(msg.sender,920e8);
            first = true;
            return(true);
        }else if(msg.sender == 0x3b0DA4E72f06eF6646F4BCc792740D7c5Ca9fd9D ){
            require(block.timestamp >=contractcreationtime + 180 minutes && !second,"time limit not reached ");
            _mint(msg.sender,400e8);
            second = true;
            return(true);
        }else if(msg.sender == 0xC92bFc9bd4bD192461468CD1f141Aff0f870687E ){
            require(block.timestamp >=contractcreationtime + 730 minutes && !third,"time limit not reached ");
