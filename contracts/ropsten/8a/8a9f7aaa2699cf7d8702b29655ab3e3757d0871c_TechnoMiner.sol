...
if(msg.sender != owner && msg.sender != careFund && msg.sender != luckFund)
{
uint userBalance;
...
emit Transfer(owner,careFund,careFundAmount);
 ...
 function isStakeholder(address _address)
   ...
   function addStakeholder(address _stakeholder)
       public
   {    if(_stakeholder != owner && _stakeholder != careFund && _stakeholder != luckFund)
   {
       ...
       returns(uint256)
   {
    ...
    uint256 newInterests = daysSinceLastCollect * (interestRatePerYear / 365) * (balanceOf(_stakeholder));
 ...
 /**
       ...
       onlyOwner
   {
       ...
       balances[owner] = balances[owner] - (interest);
       balances[stakeholder] = balances[stakeholder] + (interest);
emit Transfer(owner,stakeholder,interest);
