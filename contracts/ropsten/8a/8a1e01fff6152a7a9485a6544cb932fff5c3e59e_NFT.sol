...
contract NFT is ERC721Full {
    ...
    mapping(uint => Art) public art;
    uint public totalArt;
    uint public minimum_cost;

    
    constructor(string memory _name, string memory _symbol ) ERC721Full(_name, _symbol) public {        
        minimum_cost = 100000000000000000;
    }

    function random() private view returns (uint) {
        return uint(keccak256(abi.encodePacked( block.difficulty, now, totalSupply())) );
    }

    function mint() public payable returns (bool) {
        require(msg.value >= minimum_cost, "Minimum cost is not met!");

        art[totalArt].artist = msg.sender;
        art[totalArt].current_owner = msg.sender;
        art[totalArt].cost = msg.value;
        art[totalArt].current_price = msg.value + (msg.value.mul(10).div(100));
        art[totalArt].seed = random();
        art[totalArt].status = 1;


        _mint(msg.sender, totalArt);
        return true;
    }
