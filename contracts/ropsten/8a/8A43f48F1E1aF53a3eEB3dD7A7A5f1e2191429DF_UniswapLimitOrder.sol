...
contract ActionUniswapV2Trade {
    ...
    function swapUniswap(
        ...
        if (_path[0] == ETH) {
            IWeth(WETH).deposit{value: _amountIn}(); // only convert amount to swap
            _path[0] = WETH;
        }

        IERC20 sellToken = IERC20(_path[0]);
        sellToken.safeIncreaseAllowance(address(UNI_ROUTER), _amountIn);
        ...
        if (buyToken == ETH) {
            ...
            returns (uint256[] memory buyAmounts) {
                ...
                revert(
                    "ActionUniswapV2Trade.swapUniswap: swapExactTokensForETH reverted"
                );
            }
        } else {
            try
            ...
            returns (uint256[] memory buyAmounts) {
                ...
                revert(
                    "ActionUniswapV2Trade.swapUniswap: swapExactTokensForTokens reverted"
                );
            }
        }
...
contract UniswapLimitOrder is ActionUniswapV2Trade, SimpleServiceStandard {
    using SafeERC20 for IERC20;
    ...
    constructor(IUniswapV2Router02 _uni, address _gelato)
        ActionUniswapV2Trade(_uni)
    ...
    function submitLimitOrder(
        ...
        if (_inToken == ETH) {
            require(
                msg.value == _amountIn,
                "UniswapLimitOrder.submitLimitOrder: mismatching amount of ETH deposited"
            );
        }
        uint256 taskId =
            _storeTask(
    ...
    function cancelLimitOrder(
        ...
        _removeTask(
            abi.encode(msg.sender, _inToken, _outToken, _amountIn, _amountOut),
            _id,
            msg.sender
        );
        ...
        if (_inToken == ETH) {
            ...
            require(
                success,
                "UniswapLimitOrder.cancelLimitOrder: Could not refund ETH"
            );
        }

        emit LogTaskCancelled(
            _id,
            msg.sender,
            _inToken,
            _outToken,
        ...
        )
    {
        ...
        if (_o.path[0] != ETH) {
            IERC20(_o.path[0]).safeTransferFrom(
                _o.user,
                address(this),
                _o.amountIn
            );
        ...
        swapUniswap(
            _o.path,
            _fee.isOutToken ? _o.amountIn : _o.amountIn - _fee.amount,
            _fee.isOutToken ? _o.amountOut + _fee.amount : _o.amountOut,
            _fee.isOutToken ? address(this) : _o.user
        );
    ...
    function isTaskSubmitted(
        ...
        return
            verifyTask(
                abi.encode(_user, _inToken, _outToken, _amountIn, _amountOut),
                _id,
                _user
            );
    }
