...
contract Orbet {
    ...
    constructor() {
        owner = msg.sender;
        winnerPercentage = 900000;
        callerPercentage = 10000;
        initiatorPercentage = 10000;
        vaultPercentage = 10000;
    ...
    function endBet(uint betStructId) public {
        ...
        for (uint i = 0; i < teamAddresses.length; i++) {
            IERC20(currencyToken).transfer(teamAddresses[i], ABDKMath.toUInt(ABDKMath.mul(ABDKMath.div(ABDKMath.fromUInt(teamPercentages[i]), ABDKMath.fromUInt(1000000)), ABDKMath.fromUInt((betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount)))));
        }
        totalVaultRewards = ABDKMath.add(totalVaultRewards, ABDKMath.div(ABDKMath.mul(ABDKMath.fromUInt(betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount), ABDKMath.div(ABDKMath.fromUInt(vaultPercentage), ABDKMath.fromUInt(1000000))), ABDKMath.fromUInt(totalVaultDeposits)));
        IERC20(currencyToken).transfer(msg.sender, ABDKMath.toUInt(ABDKMath.mul(ABDKMath.div(ABDKMath.fromUInt(callerPercentage), ABDKMath.fromUInt(1000000)), ABDKMath.fromUInt((betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount)))));
        IERC20(currencyToken).transfer(betStruct.Initiator, ABDKMath.toUInt(ABDKMath.mul(ABDKMath.div(ABDKMath.fromUInt(initiatorPercentage), ABDKMath.fromUInt(1000000)), ABDKMath.fromUInt((betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount)))));
        IERC20(currencyToken).transfer(address(0), ABDKMath.toUInt(ABDKMath.mul(ABDKMath.div(ABDKMath.fromUInt(burnPercentage), ABDKMath.fromUInt(1000000)), ABDKMath.fromUInt((betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount)))));
    ...
    function getBetStructPosition(uint betId, address bettor) public view returns (bool, uint) {
        ...
        return (position.Position, position.BetAmount);
    }
