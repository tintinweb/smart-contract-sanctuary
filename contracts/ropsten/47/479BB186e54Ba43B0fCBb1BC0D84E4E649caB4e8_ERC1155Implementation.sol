    ...
    ERC1155Signature
{
    ...
    // function _checkCollectionAction(address user, uint256[] memory tokens)
    //     internal
    //     override
    //     returns (bool)
    // {
    ...
    // ) internal override {
    //     require(
    //         _checkCollectionAction(user, tokens),
    ...
    //         hasRole(
    //             SIGNER_ERC1155_ROLE,
    //             _getSigner(user, tokens, fee, metaData, v, r, s)
    //         ),
    //         "SignedAdmin should sign tokenId"
    //     );
    // }
...
contract ERC1155Signature {
    ...
    bytes32 constant SIGNCOLLECTION_TYPEHASH =
        keccak256(
            "SignCollection(address creator,uint256[] tokens,uint128 fee,string metaData)"
        );
    ...
    function _hash(SignCollection memory signCollection)
        internal
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    SIGNCOLLECTION_TYPEHASH,
                    signCollection.creator,
                    keccak256(abi.encodePacked(signCollection.tokens)),
                    signCollection.fee,
                    keccak256(bytes(signCollection.metaData))
                )
            );
    }
