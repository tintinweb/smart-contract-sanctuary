...
// File: contracts/SakeVoterCalc2.sol
...
contract SakeVoterCalc2 {
    ...
    function totalSupply() external view returns (uint256) {
        ...
        voterTotal =
            sake.totalSupply().sub(totalBarSakes).mul(balancePow) +
    ...
    function balanceOf(address _voter) external view returns (uint256) {
        ...
        _votes = sake.balanceOf(_voter).mul(balancePow) + _vBarSakeNum.mul(stakePow);
        if (sqrtEnable == true) {
            return sqrt(_votes);
        }
        return _votes;
