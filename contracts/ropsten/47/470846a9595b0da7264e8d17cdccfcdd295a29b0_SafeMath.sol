...
contract SmartOneX5 {    
    ...
    function transferLevelPayment(uint256 _level, address _user) internal
    {
        ...
        if(levelPayment > 0){
            payable(creator).transfer(levelPayment);
    ...
    {
        return [users[users[_user].referrals[0]].id, users[users[_user].referrals[1]].id];
    }
        ...
        )
    {
        ...
        return (user.id, user.referrerID, user.referrals, user.levelExpiresAt, branch);
    }
}
