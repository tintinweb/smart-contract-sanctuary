...
contract YieldFarm is Ownable, ERC721
{
	...
	struct Epoch
	{
		...
		uint256 ZooTokensInEpoch;                      // Amount of LP tokens in this epoch.
	}
	...
	function updateInfo() external
	{
		...
		epochs[currentEpoch + 1].ZooTokensInEpoch += epochs[currentEpoch].ZooTokensInEpoch;// Sets number of stakers in epoch.
	...
	function deposit(INonfungiblePositionManager.MintParams calldata params) external
	{
		...
		epochs[currentEpoch + 1].ZooTokensInEpoch = (epochs[currentEpoch + 1].ZooTokensInEpoch).add(amount0);// Increases amount of Lp tokens in next epoch.
	...
	function _withdraw(address who, uint256 id) internal
	{
		...
		if (positions[id].endEpoch <= positions[id].startEpoch)
			epochs[currentEpoch + 1].ZooTokensInEpoch = (epochs[currentEpoch + 1].ZooTokensInEpoch).sub(value);// Decreases amount of tokens in current epoch.
		else
			epochs[currentEpoch].ZooTokensInEpoch = (epochs[currentEpoch].ZooTokensInEpoch).sub(value);// Decreases amount of tokens in current epoch.
	...
	function getPendingRewards(uint id) external view returns (uint256 amount) {
		...
		for (uint256 i = positions[id].startEpoch; i < currentEpoch; i++)        // Calculate number of epochs staker was participated.
		{
			if (epochs[i].ZooTokensInEpoch != 0) {
				claimValue = claimValue.add(((epochs[i].reward).mul(staked)).div(epochs[i].ZooTokensInEpoch));// Calculates reward for staker.
			}
		}
		return claimValue;
	}
