...
contract MasterChef is Ownable {
    ...
    IBEP20 public rewardToken; // must be liquid
    ...
    function add(uint256 _rewardRate, IBEP20 _lpToken, IBEP20 _lpQuoteToken, PriceFeedRouter _router) public onlyOwner {
    ...
    function quoteTokenToRewardToken(uint256 _quoteTokenAmount, address _quoteTokenAddress, PriceFeedRouter _router) public view returns(uint256) {
        if (_quoteTokenAddress == address(rewardToken)) return _quoteTokenAmount;
    ...
    function _roi(uint256 _amount, uint256 _createdAt, uint256 _lastWithdrawal, uint256 _rewardRate) internal view returns(uint256) {
        uint256 period = Math.min(block.timestamp, _createdAt.add(YEAR_IN_SECONDS)); // reward for a year only
    ...
    function pendingReward(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        if (pool.lpToken == rewardToken) {
          ...
          if (quoteRoi > 0) {
              pending = quoteTokenToRewardToken(quoteRoi, address(pool.lpQuoteToken), pool.router);
          }
        }
    ...
    function emergencyRewardTokenWithdraw(uint256 _amount) public onlyOwner {
