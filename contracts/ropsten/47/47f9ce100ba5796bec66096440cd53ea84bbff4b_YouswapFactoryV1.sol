...
contract YouswapFactoryV1 is IYouswapFactoryV1 {
    ...
    function provideReward(uint256 _pool, uint256 _rewardPerShare, address _lp, address _user, address _upper1, address _upper2) internal {
        ...
        if ((0 < userInfo.invitePower) || (0 < userInfo.pledgePower)) {
            inviteReward = userInfo.invitePower.mul(_rewardPerShare).sub(userInfo.inviteRewardDebt).div(1e12);
            pledgeReward = userInfo.pledgePower.mul(_rewardPerShare).sub(userInfo.pledgeRewardDebt).div(1e12);
            ...
            if (0 < reward) {
                ...
                emit WithdrawReward(_pool, _lp, _user, reward);
            }
        }
        ...
        if (address(0) != _upper1) {
            ...
            if ((0 < upper1Info.invitePower) || (0 < upper1Info.pledgePower)) {
                inviteReward = upper1Info.invitePower.mul(_rewardPerShare).sub(upper1Info.inviteRewardDebt).div(1e12);
                pledgeReward = upper1Info.pledgePower.mul(_rewardPerShare).sub(upper1Info.pledgeRewardDebt).div(1e12);
                ...
                if (0 < reward) {
                    ...
                    emit WithdrawReward(_pool, _lp, _upper1, reward);
                }
            }
            ...
            if (address(0) != _upper2) {
                ...
                if ((0 < upper2Info.invitePower) || (0 < upper2Info.pledgePower)) {
                    inviteReward = upper2Info.invitePower.mul(_rewardPerShare).sub(upper2Info.inviteRewardDebt).div(1e12);
                    pledgeReward = upper2Info.pledgePower.mul(_rewardPerShare).sub(upper2Info.pledgeRewardDebt).div(1e12);
                    ...
                    if (0 < reward) {
                        ...
                        emit WithdrawReward(_pool, _lp, _upper2, reward);
                    }
                }
            }
        }
    ...
    function pendingReward(uint256 _pool, address _user) override external view returns (uint256) {
        ...
        if (address(0) != poolInfo.lp && (poolInfo.startBlock <= block.number)) {
            ...
            uint256 rewardPerShare = poolInfo.rewardPerShare.add(reward.mul(1e12).div(poolInfo.totalPower));

            UserInfo memory userInfo = pledgeUserInfo[_pool][_user];
            inviteReward = userInfo.invitePower.mul(rewardPerShare).sub(userInfo.inviteRewardDebt).div(1e12);
            pledgeReward = userInfo.pledgePower.mul(rewardPerShare).sub(userInfo.pledgeRewardDebt).div(1e12);
        }
