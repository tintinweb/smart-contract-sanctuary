...
//   }
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, uint256 initialSupply_)  ERC20(name_,symbol_,decimals_) {
        _tokensSold = 0;
        _onSale = true;
        _tokenPrice = 0.5*10**2;
        if (initialSupply_ > 0) {
            ERC20._mint(owner(), initialSupply_*10**decimals_);
        }
    }
    ...
    function buyTokens(uint256 numberOfTokens, address buyer) external returns(bool) {

        // require(msg.value >= amount, "ERC20: msg.Value is not same as amount");
        
        _transfer(owner(),buyer, numberOfTokens);
