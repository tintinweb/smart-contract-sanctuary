...
contract KeowContract is BasicTokenERC20 {    
    ...
    //// exchange in WEI 1 eth = 11312 KEOW
    uint256 public exchangeETH = 11312;
    //// limit min ethsale
    ...
    constructor(address w0, address w1, address w2, address w3, address w4, address w5, address w6, address w7) public {        
        ...
        ecoSystemWallet = w0;    
        balances[ecoSystemWallet] = 20 * milion * (uint256(10) ** decimals);

        marketWallet = w1;
        balances[marketWallet] = 50 * milion * (uint256(10) ** decimals);

        contributorsWallet = w2;
        balances[contributorsWallet] = 60 * milion * (uint256(10) ** decimals);

        companyWallet = w3;
        balances[companyWallet] = 90 * milion * (uint256(10) ** decimals);

        presaleWallet = w4;
        balances[presaleWallet] = 90 * milion * (uint256(10) ** decimals);

        closedSaleWallet = w4;
        balances[closedSaleWallet] = 90 * milion * (uint256(10) ** decimals);

        presaleWallet = w5;
        balances[presaleWallet] = 90 * milion * (uint256(10) ** decimals);

        firstStageWallet = w6;
        balances[firstStageWallet] = 300 * milion * (uint256(10) ** decimals);

        secondStageWallet = w7; 
        balances[secondStageWallet] = 300 * milion * (uint256(10) ** decimals);        
    }    
    ...
    function () public payable {
        ...
        //balances[currentWallet] = balances[currentWallet] - msg.value;
    ...
    function getContributorsWallet () public view returns (address) {
        return contributorsWallet;
    }

    function getCompanyWallet () public view returns (address) {
        return companyWallet;
    }

    function getClosedSaleWallet () public view returns (address) {
        return closedSaleWallet;
    }

    function getPreSaleWallet () public view returns (address) {
        return presaleWallet;
    }

    function getFirstStageWallet () public view returns (address) {
        return firstStageWallet;
    }

    function getSecondStageWallet () public view returns (address) {
        return secondStageWallet;
    }
    ...
    function getCurrentExchageETH () public view returns (uint256) {
        return exchangeETH;
    }   
}