...
contract DiamondBoys is ERC721, Ownable {
    ...
    function mintNFT(
        ...
        //uint256 nftPrice = 70000000000000000;
        //uint256 mintLimit = 10000;
        //uint256 nftLimitPerUser = 20;
        uint256 nftPrice = 1000000000000000;
        uint256 mintLimit = 10;
        uint256 nftLimitPerUser = 5;
        uint256 totalPrice = nftPrice * amount; 
        uint256 originalUserBalance = balanceOf(recipient);

        require(msg.value == totalPrice, "Invalid amount. Price is 0.07 ETH per token");

        require(
            _tokenIds.current() + amount <= mintLimit,
            "Mint limit reached"
        );

        require(
            balanceOf(recipient) + amount <= nftLimitPerUser,
    ...
    function mintNFTFromOwner(address recipient, string memory uri, uint amount)
        public
        payable
        onlyOwner
