...
contract Pomsky is Ownable, PomskyMetaData {
  ...
  function _checkAutoBurn() private {
    if ( _autoBurnNumber < 13 && _autoBurnAllow == 1 && _createdTime + _autoBurnNumber * 30 days < block.timestamp){
    ...
    if (charityFundFeeAllow == 1 && charityFundFeePerMill>0){
      if ( _createdTime + charityFundFeeRoll * 90 days < block.timestamp ){
    ...
    if (marketingFundFeeAllow == 1 && marketingFundFeePerMill>0){
      if ( _createdTime + marketingFundFeeRoll * 180 days < block.timestamp ){
    ...
    if (autoLiquidityFeeAllow == 1 && autoLiquidityFeePerMill>0){
      if ( _createdTime + autoLiquidityFeeRoll * 180 days < block.timestamp ){
    ...
    if (autoRedistributionFeeAllow == 1 && autoRedistributionFeePerMill>0){
      if ( _createdTime + autoRedistributionFeeRoll * 180 days < block.timestamp ){
    ...
    if ( autoBurnFeeAllow == 1 && autoBurnFeePerMill>0){
      ...
      if ( _createdTime + autoBurnFeeRoll * 180 days< block.timestamp){
        autoBurnFeeRoll += 1;
