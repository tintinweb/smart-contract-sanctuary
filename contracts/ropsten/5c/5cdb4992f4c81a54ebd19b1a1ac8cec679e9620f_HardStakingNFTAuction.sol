...
contract HardStakingNFTAuction is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    event Staked(address indexed user, uint stakeAmount, uint totalRelativeBid, uint indexed auctionId, bool isRestake);
    event NewAuction(uint indexed roundId, uint indexed tokenId);
    event NewTopBid(uint indexed auctionId, uint bidAmount, address indexed bidder);
