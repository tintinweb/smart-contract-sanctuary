...
contract MatrixInu is Context, IERC20, Ownable {
	...
	string constant NAME = "Matrix Inu";
	string constant SYMBOL = "MINU";
	...
	uint256 unprocessedLiquidityFee = 0;
	...
	uint256 extraLiquidityFee = 10; //for early sale, per wallet//
	...
	constructor() {
		...
		isExcludedFromRewards[self] = true;
		excluded.push(self);
		excluded.push(owner());
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if(!automatedMarketMakerPairs[from]) 
		{
			...
			if(automatedMarketMakerPairs[to]){ //user is selling//
				...
				//first 5 minutes from launch ONLY - bot protection = 15% liquidity fee
				if(blockTime < launchedAtTime + 5 minutes){
					extraFees = 15;
				}
				//first week early sale = 10% liquidity fee
				else if(blockTime < firstTransTime + 7 days){
					extraFees = extraLiquidityFee;
				}
	...
	function _takeOtherFees(uint256 rOtherFee, uint256 tOtherFee, uint256 rExtraFee, uint256 tExtraFee) private {
		address self = address(this);

		rOwned[self] += rOtherFee + rExtraFee;

		if (isExcludedFromRewards[self]) {
			tOwned[self] += tOtherFee + tExtraFee;
		}
		unprocessedLiquidityFee += tExtraFee;
	}
	...
	function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool shouldTakeFees, uint256 extraFee) private {
		...
		if (shouldTakeFees) {
			...
			_takeOtherFees(set.rOtherFee, set.tOtherFee, set.rExtraFee, set.tExtraFee);
	...
	function _processFees(uint256 amount) private lockTheSwap {
		amount = amount - unprocessedLiquidityFee;
		...
		uint256 amountForLiquidity = amount - amountForMarketing + unprocessedLiquidityFee;

		//Do processing
		_swapExactTokensForETH(amountForMarketing);
		_addLiquidity(amountForLiquidity);
		unprocessedLiquidityFee = 0;
	...
	function setExtraLiquidityFees(uint256 exLFee) external onlyOwner {
		require(exLFee <= 10, "Cannot set liquidity fee to above 10%");
		extraLiquidityFee = exLFee;
	}

	function setIsFeeProcessingEnabledAndThreshold(bool value,uint256 threshold) public onlyOwner {
		isFeeProcessingEnabled = value;
        feeProcessingThreshold = threshold;
	}
	...
	function setMaxTransactionAmounts(uint256 maxBuy, uint256 maxSell, uint256 maxWallet) external onlyOwner {
        ...
        require(maxWallet >= (tTotal / 200), "Must set max wallet to at least 0.5% of total supply");
