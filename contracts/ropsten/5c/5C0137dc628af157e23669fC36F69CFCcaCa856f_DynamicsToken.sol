...
contract DynamicsToken is Context, IERC20, Ownable {
    ...
    uint256 private buyerDiscountPrice = 2 ether; // eth;
...
//                }
                ...
                (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tOther) = _getTValues(amount, buyFees.liquidity, buyFees.reflection, nonReflectiveFee);
                ...
                (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(amount, tFee, tLiquidity, tOther, _getRate());
...
//                }
                ...
                (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tOther) = _getTValues(amount, 0, finalSaleFee.div(2), finalSaleFee.div(2));
                (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(amount, tFee, tLiquidity, tOther, _getRate());
                ...
                (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tOther) = _getTValues(amount, transferFees.liquidity, transferFees.reflection, nonReflectiveFee);
...
//                if(contractTokenBalance > balanceOf(uniswapV2Pair).mul(_getRate()).div(100)) {
//                    contractTokenBalance = balanceOf(uniswapV2Pair).mul(_getRate()).div(100);
//                }
//                swapTokensForEth(contractTokenBalance);
//            }

        } else {
            _transferStandard(from,to, amount, amount, amount);
        }
