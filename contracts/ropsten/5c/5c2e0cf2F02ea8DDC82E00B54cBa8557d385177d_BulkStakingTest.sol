...
contract BulkStakingTest is Ownable, Readable {
    ...
    uint public constant RELEASE_DATE = 1625142837;
    ...
    function restake() external {
        Lock storage user = db.locked[msg.sender];
        user.readyAt = 0;
        emit Restaked(msg.sender, user.secondary);
    }

    function requestWithdrawal() external {
        Lock storage user = db.locked[msg.sender];
        uint readyAt = block.timestamp + 1 hours;
    ...
    event WithdrawRequested(address user, uint readyAt);
