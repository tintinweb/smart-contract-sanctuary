...
contract GoblinGhosts is Context, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    address GoblinContract = 0x8453004641379644A6905689B98b01B86Cc00997;
    ...
    function sacrifice(uint[] memory sacrifices) public payable {
        require(!paused || msg.sender == owner(), "The the pit is not open!");
        require(sacrifices.length > 1, "You have to burn atleast 2 Goblins.");
        ...
        for(uint i = 0; i < sacrifices.length-1; i++) {
            ...
            mintGhost(_tokenIdTracker.current(), coins);
        }
    }
    
    function mintGhost(uint id, uint coins) private {
        _mint(msg.sender, id);
        
        uint randomNumber =  uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, totalSupply())));

        goldCoins[id] = coins;
        emit CreateGhost(id, coins, randomNumber);
    }
