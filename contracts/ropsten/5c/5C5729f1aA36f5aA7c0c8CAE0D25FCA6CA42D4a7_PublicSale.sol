...
contract PublicSale is ReentrancyGuard, ISale {
    ...
    /// @param _price public sale cost in ether
    /// @param _totalSupply max public supply
    constructor(
        ...
        returns (uint256)
    {
        ...
        require(numberSold < totalSupply, "Sold out");
        require(msg.value == price, "Wrong price");

        numberSold += 1;

        uint256 mintedId = mintable.mintEdition(msg.sender);

        emit Sale(price, msg.sender, mintedId);
    }

    /// @dev Returns sales info
    /// @return (totalSupply, numberSold, price)
    function saleInfo()
        public
        view
        override
    ...
    /**
    @dev Set the public sale price
    @param _salePrice if sale price is 0 sale is stopped, otherwise that amount 
                    of ETH is needed to start the sale.
    */
    function setSalePrice(uint256 _salePrice) external onlyMintableOwner {
        price = _salePrice;
    }

    /// @dev Set number of NFTs that can be purchased
    /// @param _supply new number allowed for public sale
    function setSaleNumbers(uint256 _supply)
        public
        onlyMintableOwner
    {
        totalSupply = _supply;
...
interface ISale {
    event Sale(uint256 salePrice, address to, uint256 tokenId);
