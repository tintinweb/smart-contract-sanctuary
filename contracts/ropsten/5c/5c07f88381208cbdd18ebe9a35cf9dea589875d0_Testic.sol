...
contract Testic is ERC20, PoSTokenStandard, Ownable {
    ...
    uint public presaleSupply = 300000e18;
    ...
    function Testic() {
        maxTotalSupply = 5000000e18;
        totalInitialSupply = 200000e18;
...
//------------------------------------------------------------------------------
    ...
    //To call Ethic internal minting
    //
    //Send any amount, even 0, of Ethic tokens
    //back to yourself (same Ethereum address that holds Ethic token) 
    //to triggering Etheral internal minting.
    //Best option wallet are Metamask.
    ...
    function getProofOfStakeReward(address _address) internal returns (uint) {
        ...
        //5th year effective annual interest rate is 10%
        return _coinAge.mul(interest).div(365);
    }
...
//------------------------------------------------------------------------------
    ...
    event ChangeETHMin(uint _value);
    event ChangeETHMax(uint _value);

    bool public closed;
    
    uint public rate = 1000; // 1 ETHIC = 10 MATIC
    uint public startDate = now;
    uint public ETHMin = 0.001 ether; //Minimum purchase
    uint public ETHMax = 100 ether; //Maximum purchase
    ...
    function changeETHMin(uint256 _ETHMin) public onlyOwner {
        ETHMin = _ETHMin;
        ChangeETHMin(ETHMin);
    }

    function changeETHMax(uint256 _ETHMax) public onlyOwner {
        ETHMax = _ETHMax;
        ChangeETHMax(ETHMax);
    }
