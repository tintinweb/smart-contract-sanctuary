...
contract SupplyChain {
    ...
    modifier onlylOrderDeliveringCompany(uint256 _index) {
    ...
    function startDeliveringOrder(
        uint256 _index
    ) public onlylOrderDeliveringCompany(_index) orderCreated(_index) {
    ...
    function stopDeliveringOrder(
        uint256 _index    
        ) public onlylOrderDeliveringCompany(_index) {
            Order storage order = orders[_index];
