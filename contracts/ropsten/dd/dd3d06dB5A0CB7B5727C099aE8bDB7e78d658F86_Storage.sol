...
contract Storage is Initializable, OwnableUpgradeable {
    ...
    function addStudentsToCourse(uint256 courseID, uint256[] memory students_) external onlyCore {
        for (uint256 i = 0; i < MAX_STUDENTS_FOR_COURSE; i++) {
            if (courses[courseID].students[i] == 0) {
                for (uint256 j = 0; j < students_.length; j++) {
                    courses[courseID].students[i + j] = students_[j];

                    for (uint256 k = 0; k < MAX_COURSES_FOR_STUDENTS; k++) {
                        if (students[students_[j]].courses[i] == 0) {
                            students[students_[j]].courses[i] = courseID;
                            break;
                        }
                    }
                }
    ...
    function getSchedule(uint256 courseID)
    external
    view
    onlyCore
    returns (uint256[] memory, uint256[] memory)
    {
        uint256[] memory days_ = new uint256[](schedule[courseID].length);
        uint256[] memory time_ = new uint256[](schedule[courseID].length);

        for (uint256 i = 0; i < schedule[courseID].length; i++) {
            days_[i] = schedule[courseID][i].day;
            time_[i] = schedule[courseID][i].time;
        }

        return (days_, time_);
    }

    function getMarks(uint256 courseID, uint256 studentID)
    external
    view
    onlyCore
    returns (uint8[] memory)
    {
        return marks[courseID][studentID];
    }
...
interface IStorage {
    ...
    function addStudentsToCourse(uint256 courseID, uint256[] memory students) external;

    function setScheduleForCourse(uint256 courseID, accounts.DateTime[] memory _schedule) external;
