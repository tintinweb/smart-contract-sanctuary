...
contract ActuralMiner is Ownable{
    ...
    function ReferralActivated(address addr) private{
        ...
        uint8 i = 0;
        do{
            ReferralCode = bytes8(uint64(keccak256(abi.encodePacked(addr)))+uint64(i));
            i++;
    ...
    isValidated(msg.sender,validationCode)
    {
        ...
        if(ReferralThreshold > accounts[msg.sender].Tokens && ReferralThreshold <= (accounts[msg.sender].Tokens + token))
    ...
    //Bug!! need to check affCode exist or not in advance
    //Bug!! need to check inventory exist or not in advance
    function buy_Vault(uint256 vaultAmount,bytes8 affCode) payable public
    {
        getETHUSD();
        address Addr = msg.sender;
        uint256 totalBalance = accounts[Addr].Balance + accounts[Addr].AffiliateBalance;
        ...
        if(referralMap[affCode]!=address(0)){
            priceNow = priceNow*CouponRatio/10000;
            //Affiailate function
            accounts[referralMap[affCode]].AffiliateBalance += vaultAmount/(10000/ReferralRatio); 
        }

        if(ReferralThreshold > accounts[Addr].Tokens && ReferralThreshold <= (accounts[Addr].Tokens + token))
            ReferralActivated(Addr);

        uint256 token = vaultAmount/priceNow;
        if(accounts[Addr].AffiliateBalance>vaultAmount){
            accounts[Addr].AffiliateBalance-=vaultAmount;
        }
        else{
            accounts[Addr].Balance -= (vaultAmount-accounts[Addr].AffiliateBalance);
    ...
    function checkReferral(bytes8 affCode) external view returns(address){
