...
contract MetaKidz_V1 is ERC721Enumerable, Ownable {
    ...
    uint256 public MAX_MetaKidz_PRE_SALE = 1000;
    uint256 public MAX_SUPPLY = 10000;
    bool public saleIsActive = false;
    bool public presaleIsActive = false;
    bool public isMetadataLocked = false;
    uint256 public metakidzReserve = 200;
    ...
    constructor() ERC721("METAKIDZ(V1)","MTKDZ"){
    ...
    function reserveMetakidz(address _to, uint256 _reserveAmount)
        public
        onlyOwner
    {
        uint256 supply = totalSupply();
        require(
            _reserveAmount > 0 && _reserveAmount <= metakidzReserve,
            "Not enough reserve left for team"
        );
        for (uint256 i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, supply + i);
        }
        metakidzReserve = metakidzReserve.sub(_reserveAmount);
    }

    function mintMetakidz(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY.sub(metakidzReserve), "Purchase would exceed max supply of Metakidz");
    ...
    function mintMetakidzPresale(uint numberOfTokens) public payable {
        ...
        require(_allowListClaimed[msg.sender] + numberOfTokens <= 3,"Exceeds supply of presale Metakidz you can mint.");
        require(numberOfTokens > 0, "Number of tokens can not be less than or equal to 0");
        require(NFT_PRICE.mul(numberOfTokens) <= msg.value, "Sent ether value is incorrect");
        require(totalSupply().add(numberOfTokens) <= MAX_MetaKidz_PRE_SALE, "Purchase would exceed max supply of One Rackoons");
