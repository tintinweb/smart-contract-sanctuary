...
contract ZoomToken is ERC20Interface, Owned, SafeMath {
...
/* to be called when ICO is closed. burns the remaining tokens except the company share (60360000), the tokens reserved
	...
	function burn() {
		...
		if (!burned && now > startTime) {
			uint difference = safeSub(balances[owner], reservedAmount);
			balances[owner] = reservedAmount;
			_totalSupply = safeSub(_totalSupply, difference);
			burned = true;
			Burned(difference);
		}
	}
