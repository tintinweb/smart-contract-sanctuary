...
contract VCAuction is IVCAuction, Manageable, Migrateable, ExternallyCallable {
    ...
    function init(
        ...
        address[] memory v2DivTokens = contracts.stakingV2.getDivTokens();

        for (uint256 i = 0; i < v2DivTokens.length; i++) {
            divTokens.add(v2DivTokens[i]);
            tokenPricePerShare[v2DivTokens[i]] = contracts.stakingV2.getTokenPricePerShare(
                v2DivTokens[i]
            );
        }
    }
    ...
    function getDeductBalances(address staker, address token) external view returns (int256) {
        if (isVcaRegistered[staker] == false) {
            return contracts.stakingV2.getDeductBalances(staker, token).toInt256();
        }
    ...
    function getTokenInterestEarned(address accountAddress, address tokenAddress)
        ...
        returns (uint256)
    {
        if (isVcaRegistered[accountAddress] == false) {
            return
                ((contracts.stakingV2.getTotalSharesOf(accountAddress) *
                    tokenPricePerShare[tokenAddress]) -
                    contracts.stakingV2.getDeductBalances(accountAddress, tokenAddress)) / 1e36;
        }
    ...
    function getTotalSharesOf(address account) external view returns (uint256) {
        if (isVcaRegistered[account] == false) {
            return contracts.stakingV2.getTotalSharesOf(account);
        }
