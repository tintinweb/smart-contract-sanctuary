...
contract Party {
    ...
    function fillQuote(
        ...
        uint256 fee = sellAmount * 5 / 1000;
        uint256 outAmount = fee + sellAmount;
        require(outAmount < sellTokenAddress.balanceOf(address(this)), "Balance not enough. (Amount + Fee)");
        require(sellTokenAddress.approve(spender, type(uint256).max));
        (bool success, ) = swapTarget.call{value: msg.value}(swapCallData);
        require(success, "SWAP_CALL_FAILED");
        payable(msg.sender).transfer(address(this).balance);
        sellTokenAddress.safeTransfer(PLATFORM_ADDRESS, fee);
    ...
    function leaveParty(address quittingMember, uint256 addressWeight, Swap[] memory tokenData) external payable onlyMember isAlive {
        for (uint256 index = 0; index < tokenData.length; index++) {
            uint256 fee = tokenData[index].sellAmount * 5 / 1000;
            uint256 outAmount = fee + tokenData[index].sellAmount;
            require(outAmount < tokenData[index].sellTokenAddress.balanceOf(address(this)), "Balance not enough. (Amount + Fee)");
            require(tokenData[index].sellTokenAddress.approve(tokenData[index].spender, type(uint256).max), "Approval invalid");
            // (bool success, ) = tokenData[index].swapTarget.call{value: msg.value}(tokenData[index].swapCallData);
            // require(success, "SWAP_CALL_FAILED");
            // payable(msg.sender).transfer(address(this).balance);
            // tokenData[index].sellTokenAddress.safeTransfer(PLATFORM_ADDRESS, fee);
            // tokenData[index].sellTokenAddress.approve(tokenData[index].spender, 0); 
        }
        uint256 _tempBalance = (getPartyBalance() * addressWeight) /
            10**6;
    ...
    function closeParty(Weight[] memory weightArray, Swap[] memory tokenData) external onlyOwner isAlive {
        //closeParty([25,25,50])
        //memberlist[a,b,c]
        for (uint256 index = 0; index < tokenData.length; index++) {
            fillQuote(
        ...
        for (uint256 index = 0; index < weightArray.length; index++) {
            uint256 value = weightArray[index].weightPercentage * getPartyBalance();
            uint256 fee = value * 5 / 1000;
