...
contract Vault is Ownable {
    ...
    /***************** Calls **************** */
    function transferFunds(address _tokenAddress, address _recipient, uint256 _amount) public onlyDispatcher {
    ...
    function newMultiSig(address _multiSigAddress) public onlyMultiSig {
        require(_multiSigAddress != address(0), "Cannot set address to 0");
        multiSigAddress = _multiSigAddress;
        emit NewMultiSigEvent(_multiSigAddress);
    }

    function newDispatcher(address _dispatcherAddress) public onlyMultiSig {
    ...
    function addToken(address _tokenAddress) public onlyMultiSig {
    ...
    function removeToken(address _tokenAddress) public onlyMultiSig {
    ...
    function popTokenArray(address _tokenAddress) internal {
...
contract Dispatcher is Ownable {
    ...
    /*****************  Setters **************** */
    function newThreshold(uint256 _threshold) public onlyMultiSig {
    ...
    function newVault(address _vaultAddress) public onlyMultiSig {
        require(_vaultAddress != address(0), "Vault address cannot be 0");
        vault = Vault(_vaultAddress);
        emit NewVault(_vaultAddress);
    }

    function newBridgeController(address _bridgeControllerAddress) public onlyMultiSig {
    ...
    function addNewValidator(address _validatorAddress) public onlyMultiSig {
    ...
    function removeValidator(address _validatorAddress) public onlyMultiSig {
    ...
    function proposeNewTxn(address _userAddress, address _tokenAddress, uint256 _amount, string memory _note) public onlyBridgeController{
    ...
    function approveTxn(uint256 _proposal) public onlyValidators oneVoteTransfer(_proposal){
    ...
    event NewThreshold(uint256 newThreshold);
