...
contract ICOTest{
	...
	function getFarmerData1( address contract_address) public  returns(uint256, uint256, uint256) {							
				return ( 
				IFarmerCommon(contract_address).getContractBalance(), 
				IFarmerCommon(contract_address).totalInvested(),
				IFarmerCommon(contract_address).INVEST_MIN_AMOUNT()
				);
	}
	function getFarmerData2( address adr, address contract_address) public  returns(uint256, uint256, uint256,uint256,uint256) {							
