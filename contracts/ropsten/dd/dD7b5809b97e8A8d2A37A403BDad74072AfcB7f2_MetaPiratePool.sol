...
interface IMetaPiratePool {
    event TransferWithLockRequest(
        address indexed to_,
        bytes32 indexed requestID_,
        uint256 indexed releaseTime_,
        uint256 amount_
    );
    event TransferWithLockAudit(
        address indexed to_,
        bytes32 indexed requestID_,
        uint256 indexed releaseTime_,
        uint256 amount_,
        address auditor_,
        AuditLevel level_,
        AuditStatus auditStatus_
    );
    event BindingToken(address tokenAddress_);
    ...
    function bindingToken(address tokenAddress_) external returns (bool);

    function getTransferRequest(bytes32 requestID_)
        external
        view
        returns (SendRequest memory request, AuditRecord memory audit);

    function listPendingTransaction(AuditLevel level)
        external
        view
        returns (bytes32[] memory);

    function auditTransferForSigner(bytes32 requestID_, AuditStatus status_)
        external
        returns (bool);

    function auditTransferForAdmin(bytes32 requestID_, AuditStatus status_)
        external
        returns (bool);
