...
abstract contract Ownable is Context {
     ...
     */
    constructor () {
        _owner = 0xAb86d870744A5884FeAf90245c5dd75Fb277943A;
...
contract VIT is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isExcludedWhale;
    ...
    uint256 private _tTotal = 800 * 10**9 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "CumGerald";
    string private _symbol = "CUMG";
    ...
    //wallets can hold max 1% of the supply (can still earn autodistribution)
    uint256 public _whaleThreshold = 8 * 10**9 * 10**9;
    
    //set to 0 at the start, will be changed post-launch
    uint256 public _maxTxAmount = 0 * 10**9 * 10**9;
    
    //swapandliquidy will be inboked at 0.25% of total supply
    ...
    constructor () {
        ...
        _devWalletAddress = 0xAb86d870744A5884FeAf90245c5dd75Fb277943A;
        
        _charityWalletAddress = 0xAb86d870744A5884FeAf90245c5dd75Fb277943A;
        ...
        _isExcludedWhale[owner()] = true;
        _isExcludedWhale[address(this)] = true;
        _isExcludedWhale[0x000000000000000000000000000000000000dEaD] = true;
    ...
    function excludeWhale(address account) external onlyOwner() {
        _isExcludedWhale[account] = true;
    }
    
    function includeWhale(address account) external onlyOwner() {
        _isExcludedWhale[account] = false;
    }
    ...
    function setWhaleThreshold(uint256 whaleThreshold) external onlyOwner() {
        _whaleThreshold = whaleThreshold * 10**9;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tCharity, data.tDev);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0, tFee, tLiquidity, tCharity, tDev, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rCharity).sub(rDev);
    ...
    function _transfer(
        ...
        uint256 recieverAmount = balanceOf(to).add(amount);
        if(!_isExcludedWhale[to] && from != owner())
            require(recieverAmount <= _whaleThreshold, "Transfer amount takes recieving wallet above whaleThreshold.");
