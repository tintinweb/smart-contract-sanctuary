...
contract DebtToken is ERC20, Ownable {
    ...
    uint256 private _initialLiquidity = 37700000 * 10 ** 18;
    ...
    struct UserInfo {
        ...
        uint64 lastUpdate;     // time at which the user made his last transaction.
        uint32 currentTier;    // user's current tier.
        //
    ...
    // info of each tier.
    struct TierInfo {
        uint128 threshold;          // the amount needed to be part of that tier
        uint32 sellingFee;         // the transaction fee for all users from this tier when selling tokens. 1 = 1%
        uint32 dailyHoldingFee;    // the daily fee an user pays to the above tier. 1 = 1%
        uint64 accDebtPerShare;  // accumulated TINGs per share, times 1e12. 
...
//------------------------------------------------------------//
    ...
    function addTier(uint128 _threshold, uint32 _sellingFee, uint32 _dailyHoldingFee) 
        public 
        onlyOwner 
    {
    ...
    function changeFeesOfTier(uint256 _tier, uint32 _newSellingFee, uint32 _newDailyHoldingFee)
        public
        onlyOwner
    {
    ...
    function changeThresholdOfTier(uint256 _tier, uint128 _newThreshold)
        public
        onlyOwner
    {
...
//------------------------------------------------------------//  
    ...
    function manageTier(address _user, uint256 _newBalance, uint256 _oldBalance) internal {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (boostBalance < tiers[i].threshold) {
                ...
                user.rewardDebt = uint128(_newBalance.mul(tierB.accDebtPerShare).div(1e12));
                user.lastUpdate = uint64(now);
                if (oldTier != i) {
                    user.currentTier = uint32(i);
    ...
    function manageDebt(address _user, uint256 _debt) internal {
        ...
        if (_userTier != maxTier && _debt > 0) {
            ...
            if (aboveTier.tierSupply != 0) {	
                ...
                aboveTier.accDebtPerShare = uint64(accDebt);	
            }	
    ...
    function manageFee(address _user, uint256 _fee) internal {
        ...
        if (lowerTier.tierSupply != 0) {	
            ...
            lowerTier.accDebtPerShare = uint64(accDebt);	
        }	
