...
contract ETHBridge is Ownable {
    ...
    function withdraw(bytes calldata _signature, string memory _withdrawId, address _ercToken, uint _amount) external {
        ...
        bytes32 message = _verifyMessage(_ercToken, _amount, _withdrawId);
    ...
    function _ethSignedHash(bytes32 messageHash) public pure returns(bytes32) {
        return messageHash.toEthSignedMessageHash();
    }

    function _verifyMessage(address _ercToken, uint256 _amount, string memory _withdrawId) public view returns(bytes32 message){
        uint chainId;
        assembly {
            chainId := chainid()
        }
        message = keccak256(abi.encodePacked(chainId, address(this), _ercToken, _amount, msg.sender, _withdrawId));
    }
