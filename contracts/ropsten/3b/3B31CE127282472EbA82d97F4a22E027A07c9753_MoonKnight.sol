...
contract MoonKnight is IMoonKnight, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    uint public totalKnights = 20000;
    ...
    function setClaimPrice(uint value) external onlyOwner {
        require(value > 0, "MoonKnight: invalid value");
        claimPrice = value;
    }

    function setTotalKnights(uint amount) external onlyOwner {
        require(amount > totalKnights, "MoonKnight: invalid amount");
        totalKnights = amount;
    }
    ...
    function claimMoonKnight(uint amount) external override payable {
        ...
        uint floorPrice = claimPrice * 1000 / BPS;

        require(_knights.length + amount <= totalKnights, "MoonKnight: sold out");
        require(value == claimPrice * amount, "MoonKnight: incorrect value");
        ...
        if (startingIndex == 0 && (_knights.length == totalKnights || block.timestamp >= _revealTime)) {
    ...
    function addFloorPriceToKnight(uint knightId) external override payable {
        ...
        Knight storage knight = _knights[knightId];
        uint newFloorPrice = knight.floorPrice + amount;

        require(amount > 0, "MoonKnight: no value sent");
        require(newFloorPrice <= floorPriceCap, "MoonKnight: cannot add more");

        knight.floorPrice = newFloorPrice;

        emit KnightPriceIncreased(knightId, newFloorPrice, amount);
    }
    ...
    function _finalizeStartingIndex() private {
        startingIndex = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % totalKnights;
