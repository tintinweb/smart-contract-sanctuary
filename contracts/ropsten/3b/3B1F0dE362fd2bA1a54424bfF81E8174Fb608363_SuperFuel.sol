...
//        emit Transfer(address(0), address(this), _presaleReserve);
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(lotteryContract.checkAndPayJackpot()){
    ...
    function rewardsInfo() external view returns(string memory jackpotRewardCurrency, uint256 jackpotAmount, uint256 currentDrawNumber, string memory reflectionRewawrdsCurrency){
        jackpotRewardCurrency = lotteryContract.rewardCurrency();
        jackpotAmount = lotteryContract.jackpotAmount();
        currentDrawNumber = lotteryContract.draw();
        reflectionRewawrdsCurrency = reflectorContract.rewardCurrency();
    }
...
interface ISmartLottery is IBaseDistributor, IUserInfoManager, IAuthorizedListExt, IAuthorizedList {
    struct RewardInfo{
        string name;
...
abstract contract SmartBuyback is AuthorizedList, LPSwapSupport{
    ...
    uint256 public minAutoBuyback = 0.001 ether;
    ...
    function doBuyback(uint256 poolTokens, uint256 sellAmount) internal {
        if(!autoBuybackEnabled || inSwap || address(this).balance < minAutoBuyback) {return;}
