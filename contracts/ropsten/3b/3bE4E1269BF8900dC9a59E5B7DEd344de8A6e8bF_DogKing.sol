...
interface IDogQueen {
    function mint(address owner, uint256 userbalance, uint256 swapAmount, uint256 userpool) external returns (uint256);
    function deposit(address owner, uint256 amount, uint256 userBalances) external;
    function withdraw(address owner, uint256 amount, uint256 userBalances) external;
}
...
//address setup
    ...
    address private _dogQueen;
    address private _pairAddr;
    //in test net
    address private _operater = 0xB3c89EEa97E7b2F95691EACD80eB7AA24DC011D9;
    ...
    */
    ...
    //uint256 private _airdropRate = 1;    //1 precent 100;
    //uint256 private _uniRote = 39;       //39 precent 100;
    bool private _openTransaction = false;
    
    uint256 private _userBalances;
    
    uint256 private _starttime;
    uint256 private _starttimeOffset = 300 seconds;  //30 days
    uint8 private _centuryNum = 1;
...
//in uniswap, the function was called when eth to token. 
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        emit DebugAddress("DogKing->transfer()->recipient", recipient);
        ...
        if (_openTransaction) {
            ...
            if (msg.sender == _pairAddr) {  //pair call.
                ...
                if(res - _usedChiToken[msg.sender] >= 4) {
                    ...
                    _ChiTokenAmount = _ChiTokenAmount.sub(4);
                }
                
                userBalances = balanceOf(recipient);
                IDogQueen(_dogQueen).mint(recipient, userBalances, amount, _userBalances);
                _userBalances = _userBalances.add(amount);
                
                _transfer(_msgSender(), recipient, amount);
                return true;
            } else {  //user transfer.
                userBalances = balanceOf(recipient);
                IDogQueen(_dogQueen).deposit(recipient, amount, userBalances);
                
                userBalances = balanceOf(msg.sender);
                IDogQueen(_dogQueen).withdraw(msg.sender, amount, userBalances);
...
//in uniswap, the function was called when token to eth. uniswap router call the function, put token of sender to uniswap pool.
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        if (_openTransaction) {
            ...
            if (msg.sender == _uniRouter) {  //uniswap call.
                ...
                userBalances = balanceOf(sender);
                IDogQueen(_dogQueen).mint(sender, userBalances, amount, _userBalances);
                _userBalances = _userBalances.sub(amount);
                ...
                IDogQueen(_dogQueen).deposit(recipient, amount, userBalances);
                userBalances = balanceOf(sender);
                IDogQueen(_dogQueen).withdraw(sender, amount, userBalances);
