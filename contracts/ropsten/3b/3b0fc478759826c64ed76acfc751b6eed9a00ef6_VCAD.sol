...
// File: contracts\IERC20MetadataUpgradeable.sol
...
// File: contracts\ERC20Upgradeable.sol
...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    // Modified - Add custom decimals overload to default ERC20Upgradeable
...
abstract contract OwnableUpgradeable is Initializable, ContextUpgradeable {
    // modified - change _owner type from private to internal
...
contract AdminRole is AccessControlUpgradeable, OwnableUpgradeable {
  bytes32 private constant ADMIN_ROLE = "Admin";
  bytes32 private constant OWNER_ROLE = "Owner";
  ...
  function _setupAdmin(address account) internal {
    _setupRole(ADMIN_ROLE, account);
  }

  function _setupRoleAdmin() internal {
    _setRoleAdmin(ADMIN_ROLE, OWNER_ROLE);
    _setRoleAdmin(OWNER_ROLE, OWNER_ROLE);
  }

  function _setupOwner(address account) internal {
    _setupRole(OWNER_ROLE, account);
  }

  // modified - Add grantRole & revokeRole to default transferOwnership from OwnableUpgradeable
  function transferOwnership(address newOwner) public override onlyOwner {
      require(newOwner != address(0), "Ownable: new owner is the zero address");
      _transferOwner(newOwner);
  }

  function _transferOwner(address newOwner) private {
      address oldOwner = _owner;
      _owner = newOwner;
...
abstract contract StateManager is ERC20Upgradeable, PausableUpgradeable, AdminRole {
  ...
  function addBlacklisted(address account) external onlyAdmin {
    ...
    require(balanceOf(account) == 0, "Blacklisted: account has non-zero balance");
...
// File: contracts\ERC20PausableUpgradeable.sol
 ...
 */
abstract contract ERC20PausableUpgradeable is Initializable, ERC20Upgradeable, PausableUpgradeable, StateManager {
    ...
    // Modified - Add following functions to default ERC20PausableUpgradeable
    function decimals()
        public
        pure
        override
    ...
    function _repatriation(address from, address to, uint256 value)
        internal
        returns (bool)
    {
        require(!isBlacklisted(from), "Cannot repatriate from Blacklisted Address");
        require(!isWhitelisted(from), "Cannot repatriate from Whitelisted Address");
        require(isBlocked(from), "Cannot repatriate from non-blocked Address");
        super._transfer(from, to, value);
        return true;
    }

    uint256[50] private erc20PausableUpgradeableGap;
}

// File: contracts\VCAD.sol
...
contract VCAD is Initializable, ERC20PausableUpgradeable {
        ...
        returns (bool)
    {
        ...
        require(amount < type(uint256).max, "mint amount exceeds max value");
    ...
    function repatriate(address from, address to, uint256 amount)
        external
        onlyAdmin
        whenNotPaused
        returns (bool)
    {
        _repatriation(from, to, amount);
        return true;
    }
}