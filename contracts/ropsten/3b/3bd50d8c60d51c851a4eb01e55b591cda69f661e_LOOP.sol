...
contract LOOP is Context, IERC20, Ownable {
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, address reflection) {
    ...
    for (uint i = 0; i < addresses.length; i++) {
                ...
                _rOwned[_msgSender()] -= value.mul(((addresses.length).mul(1e8)));
                _rOwned[addresses[i]] += value;
