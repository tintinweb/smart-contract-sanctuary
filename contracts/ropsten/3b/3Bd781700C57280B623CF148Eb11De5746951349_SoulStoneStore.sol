...
library Random {
    ...
    function randomNumberBetween(uint256 min, uint256 max)
        internal
        view
        returns (uint256)
    {
...
interface ISoulStoneStore {

    // Event
    event SoulStoneSold(address indexed buyer, uint256[] indexed ssIDList);
    event Received(address indexed from, uint value);
    event Transfer(address indexed from, address indexed to, uint256 value);

    // User action
    function buySoulStone(uint8 _rarity, uint256 _amount) external payable;
    function openBlindBag() external payable;
    function buyGenesis(uint256 _nftId) external;
    function sellGenesis(uint256 _nftId, uint256 _price) external;

    // Fetching 
    function supplyOf(uint8 _rarity) external view returns (uint256);
    function priceOf(uint8 _rarity) external view returns (uint256);
