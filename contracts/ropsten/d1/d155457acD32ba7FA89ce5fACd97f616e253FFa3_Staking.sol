...
contract Staking is ReentrancyGuard, Context, IStaking {
    ...
    function deposit(uint256 amount, address sender) external override onlyFactory returns (bool) {
    ...
    function withdraw(uint256 amount, address sender) external override onlyFactory returns (bool) {
        require(amount > 0, "Input error");
        bool isEndStake;
        users[sender].lastRewardId = _upgradeReward(sender);
        users[sender].amountStaked -= amount;
