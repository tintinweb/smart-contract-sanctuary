...
contract Pettest  is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  using SafeMath for uint8;
  using SafeMath for uint256;
  // total tokens
  uint256 public constant MAX_TOKENS = 4444;
  // maximum tokens for giveaways
  uint256 public constant MAX_GIVEAWAY_TOKENS = 224; 
  uint256 public constant TOKEN_PRICE = 2500000000000000000; //2.5 AVAX
  ...
  constructor(string memory baseURI) ERC721("Petroleum DAO", "gOIL") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 numTokens) external payable nonReentrant {
    ...
    require(numTokens > 0 && numTokens <= 50, "The number of tokens must be between 1 and 50");
    ...
    require(msg.value >= TOKEN_PRICE.mul(numTokens), "Not enough AVAX for transaction");
