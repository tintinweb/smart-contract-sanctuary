...
contract DopexNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    bool public ownerNotReserverdMint = true;
    ...
    function random(uint256 n) internal view returns (uint256) {
        uint256 randomnumber = uint256(keccak256(
            abi.encodePacked(_tokenIdCounter.current(), msg.sender,block.difficulty))) % n;
    ...
    function safeMint(address to, uint256 _mintNumber) public payable {
        require(mintOn);
    ...
    function reserveNFT() public onlyOwner {
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            removeID(randomID);
            _tokenIdCounter.increment();
            ownerMints[msg.sender].push(randomID);
        }

        ownerNotReserverdMint = false;
    }
