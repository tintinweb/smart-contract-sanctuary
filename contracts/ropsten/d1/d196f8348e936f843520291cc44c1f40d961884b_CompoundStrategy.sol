...
contract CompoundStrategy is BaseRhoStrategy {
    ...
    function withdrawUnderlyingInternal(uint256 _amount, address to) internal override {
        ...
        underlying.safeTransfer(to, _amount);
    }

    function withdrawAllCashAvailableInternal(address to) internal override returns(uint256) {
        ...
        if (balance > withdrawable) {
            ...
            underlying.safeTransfer(to, withdrawable);
            return withdrawable;
        }
        require(
    ...
    function collectRewardToken(address to) external override onlyRole(VAULT_ROLE) whenNotPaused nonReentrant {
        if (address(comp) != address(0) && address(tokenExchange) != address(0)) {
            ...
            tokenExchange.sellExactInput(comp, underlying, to, comp.balanceOf(address(this)));
        }
    }
    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function withdrawUnderlyingInternal(uint256 _amount, address to) internal virtual;
    ...
    function withdrawUnderlying(uint256 _amount) external override onlyRole(VAULT_ROLE) whenNotPaused nonReentrant {
        ...
        withdrawUnderlyingInternal(_amount, _msgSender());
        emit WithdrawUnderlying(_amount, _msgSender());
    }

    /**
     * @dev core logic of withdrawing all funds from DeFi protocol
     */
    function withdrawAllCashAvailableInternal(address to) internal virtual returns(uint256) ;


    function withdrawAllCashAvailable(address to) external override onlyRole(VAULT_ROLE) whenNotPaused nonReentrant returns(uint256) {
        uint256 amount = withdrawAllCashAvailableInternal(to);
        emit WithdrawAllCashAvailable(to);
        return amount;
    }
