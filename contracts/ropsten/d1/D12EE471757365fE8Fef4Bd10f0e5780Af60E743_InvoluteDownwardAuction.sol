...
contract InvoluteDownwardAuction {
    ...
    // Events
    event AuctionStarted(bytes16 auctionId, bytes32 auctionChecksum, uint256 endTime, uint256 minDecrease, uint256 ETHConversionRate);
    event LowestBidDecreased(bytes16 auctionId, address bidder, uint256 amount, uint256 lowestBidEUR);
    event AuctionEnded(bytes16 auctionId, address winner, uint256 amount, uint256 lowestBidEUR);
    ...
    function createAuction(bytes16 _auctionId, bytes32 _auctionChecksum, uint256 _biddingTime, uint256 _minDecrease, uint256 _ETHConversionRate) public {
        ...
        a.ETHConversionRate = _ETHConversionRate;

        emit AuctionStarted(_auctionId, a.auctionChecksum, a.auctionEndTime, a.minDecrease, a.ETHConversionRate);
    ...
    function bid(bytes16 _auctionId) public payable {
        ...
        a.lowestBidEUR = msg.value * a.ETHConversionRate / 1 ether;
        
        a.bids[msg.sender].timestamp = block.timestamp;
        a.bids[msg.sender].amount = msg.value;
        a.bids[msg.sender].lowestBidEUR = msg.value * a.ETHConversionRate / 1 ether;

        emit LowestBidDecreased(_auctionId, msg.sender, msg.value, msg.value * a.ETHConversionRate / 1 ether);
    ...
    function auctionEnd(bytes16 _auctionId) public {
        ...
        // 2. Effects
        a.auctionEnded = true;
        emit AuctionEnded(_auctionId, a.lowestBidder, a.lowestBid, a.lowestBid * a.ETHConversionRate / 1 ether);
