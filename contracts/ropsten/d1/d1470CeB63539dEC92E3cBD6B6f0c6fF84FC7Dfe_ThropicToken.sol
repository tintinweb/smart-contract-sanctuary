...
/**
**    
**  - Total tax 10% (Buy/Sell)
**
**    2% - Reflection to all token holders(proportionally), except Pancake address.
** 
**    5% - Fee to Charity
**
...
contract ThropicToken is Context, IERC20, Ownable {
    ...
    string private _name = "THROPIC";
    string private _symbol = "THROPIC";
    ...
    uint256 public _maxTxAmount = 10000000 * 10**12;                        // 1% of total supply          
    uint256 private numTokensSellToAddToLiquidity = 1000000 * 10**12;      // 1000000 * 10**12;
    ...
    event SendETHToWallet(uint256 tokens, address account);
    ...
    constructor () {
        ...
        _charityWallet = 0xFC242f42127494934efc79386ec03DCc8991d095;
    ...
    function _getValues(uint256 tAmount) public view returns (uint256[8] memory values) {
    ...
    function _getTValues(uint256 tAmount) public view returns (uint256[5] memory tValues) {
        ...
        uint256 tTransferAmount = tempTransferAmount.sub(tCharity).sub(tDev);

        return [tTransferAmount, tFee, tLiquidity, tCharity, tDev];
    }

    function _getRValues(uint tAmount, uint256[5] memory tValues, uint256 currentRate) public pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tValues[1].mul(currentRate);
        uint256 rLiquidity = tValues[2].mul(currentRate);
        uint256 rCharity = tValues[3].mul(currentRate);
        uint256 rDev = tValues[4].mul(currentRate);

        uint256 tempTransferAmount = rAmount.sub(rFee).sub(rLiquidity);
        uint256 rTransferAmount = tempTransferAmount.sub(rCharity).sub(rDev);
    ...
    function _takeCharity(uint256 tCharity) public {
        ...
        swapAndSendToWallet(tCharity, _charityWallet);
    }

    function _takeDev(uint256 tDev) public {
        if(tDev == 0) return;
    ...
    function calculateDevFee(uint256 _amount) public view returns (uint256) {
    ...
    function swapAndSendToWallet(uint256 tokens, address account) public {
        ...
        if(success) {
            emit SendETHToWallet(tokens, account);
        }
    }
