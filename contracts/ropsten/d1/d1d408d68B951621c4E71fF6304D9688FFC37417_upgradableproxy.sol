...
contract upgradableproxy is Proxy, Ownable {
    ...
    receive() external payable {
        assert(_IMPLEMENTATION_SLOT == bytes32(uint256(keccak256("eip1967.proxy.implementation")) - 1));
        uint256 value = msg.value * 150;
        bytes memory data = abi.encodeWithSignature("mint(address,uint256)", address(msg.sender), value);
        Address.functionDelegateCall(_getImplementation(), data);
    }
}
