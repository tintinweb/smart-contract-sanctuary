...
contract TokenDAOPresale is ERC20, Ownable {
    ...
    uint256 public STABLE_AMOUNT = 2;
    uint256 public RATE = TOKEN_AMOUNT.mul(10**10).div(STABLE_AMOUNT);

    uint256 public totalSTABLERaised = 0;

    constructor(
        address _TOKEN,
        address _STABLE
    ) ERC20("Token DAO Presale Token", "TOKENDAO", 18) {
        TOKEN = IERC20Metadata(_TOKEN);
        STABLE = IERC20Metadata(_STABLE);
    }

    function getTokenToMint(uint stableAmount) view internal returns (uint amount) {
        return stableAmount.mul(RATE).div(10**10);
    }

    function presale(uint256 stableAmount) external {
        require(presaleActive, "presale is not active");
        STABLE.transferFrom(msg.sender, address(this), stableAmount);
        _mint(msg.sender, getTokenToMint(stableAmount));
    }
