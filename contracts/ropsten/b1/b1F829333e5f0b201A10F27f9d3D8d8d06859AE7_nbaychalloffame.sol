...
contract nbaychalloffame is ERC721, Ownable {
    ...
    constructor(address payable shareholderAddress_, address baseContract_) ERC721("nbaychalloffame", "NBAYC Hall of Fame") {
        require(shareholderAddress_ != address(0));
        shareholderAddress = shareholderAddress_;
        baseContract = baseContract_;
        totalSupply_ = BaseContract(baseContract_).totalSupply();
        initSupply_ = totalSupply_;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (tokenId <= initSupply_) {
            return BaseContract(baseContract).tokenURI(tokenId);
        }
