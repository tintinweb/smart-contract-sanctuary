...
contract UnityFundToken is Context, IERC20, Ownable {
    ...
    string private _name = "UTestToken";
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount - tFee - tLiquidity - tCharity - tDev;
    ...
    function _getRTransferAmount(uint256 rAmount, uint256 rFee, Fees memory fees, uint256 currentRate) private pure returns (uint256) {
        uint256 rCharity = fees.tCharity * currentRate;
        uint256 rLiquidity = fees.tLiquidity * currentRate;
        uint256 rDev = fees.tDev * currentRate;
        uint256 rTransferAmount = rAmount - (rFee) - (rCharity) - (rLiquidity) - (rDev);
        return rTransferAmount;
    }
    ...
    function _takeCharity(uint256 tCharity, address sender) private {
        ...
        _rOwned[_charityWalletAddress] = _rOwned[_charityWalletAddress] + rCharity;
        if(_isExcluded[_charityWalletAddress]) {
            _tOwned[_charityWalletAddress] = _tOwned[_charityWalletAddress] + tCharity;
        }
