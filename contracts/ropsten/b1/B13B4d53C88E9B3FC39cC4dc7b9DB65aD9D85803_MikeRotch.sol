...
contract MikeRotch is Context, IERC20, Ownable 
{
    ...
    {
        uint256 weight = (100 / (feeRates.rfi + feeRates.toSwap)) / 100;
        uint256 proportion = weight * feeRates.rfi;
        uint256 tokensToSupply = (contractTokenBalance * proportion) / 2;
        uint256 tokensToSwap = tokensToSupply + (contractTokenBalance * weight * feeRates.marketing);
        
        uint256 initialBalance = address(this).balance;
        swapTokensForETH(tokensToSwap);
        uint256 newBalance = address(this).balance - initialBalance;
        
        uint256 liquidityETH = newBalance - (newBalance * weight * feeRates.marketing);
