...
contract Airdrop {
  ...
  function publicAmount() external view returns (uint, uint) {
    return (currentAirdropAmount, maxAirdropAmount);
  }
  function publicPrefixed(address recipient, uint amount) external pure returns (bytes32) {
    return keccak256(abi.encodePacked(
      '\x19Ethereum Signed Message:\n32', 
   ...
   function publicHash(address recipient, uint amount) external pure returns (bytes32) {
    return keccak256(abi.encodePacked(
      recipient, 
      amount
    ));
  }
  function publicRecoverSigner(address recipient, uint amount, bytes memory sig)
    external
    pure
    returns (address)
  {
    ...
    return ecrecover(prefixed(keccak256(abi.encodePacked(
      recipient, 
      amount
    ))), v, r, s);
  }
