...
interface ICryptoAnts is IERC721 {
  ...
  /// @dev Emitted when an Ant died.
  event AntDead();

  /// @dev Emitted when an Ant hatches new eggs.
  event EggsHatched(uint256 amount);
  ...
  function setNewEggsRange(uint256 newEggsRange) external;

  function setProbabilityOfDying(uint256 newProbabilityOfDying) external;
  ...
  /// @dev Occurs when trying to hatch new eggs but the minimum hatch period didn't complete.
  error HatchPeriodNotCompleted();

  /// @dev Occurs when trying to hatch with wrong _antId.
  error AntDoesntExist();
}
