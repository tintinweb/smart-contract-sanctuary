...
contract RewardsGenerator is IRewardsGenerator, OwnableUpgradeable, AbstractDependant {
    ...
    modifier onlyBMIDAIStaking() {
        require (_msgSender() == bmiDaiStakingAddress, "RewardsGenerator: Caller is not a bmiDAIStaking contract");
        _;
    }
    ...
    function totalStakedPolicyBooks() public view returns (uint256) {
        return _allStakedPolicyBooks.length();
    }

    function updateAverage(uint256 offset, uint256 limit) public {
    ...
    function updateAverage(address policyBookAddress) external {
        require (_allStakedPolicyBooks.contains(policyBookAddress), 
        ...
        ) 
    {
        ...
        rewardPerBlockPolicyBook = 
            info.rewardMultiplier.mul(info.totalStaked).mul(rewardPerBlock).mul(PRECISION)
        ...
        allBlocks > 0 ?
        average = 
            prevStakedBlocks.mul(info.average)
        ...
        totalStaked > 0 ?  
        toUpdateAverage = 
            rewardPerBlockPolicyBook.mul(100)
            .div(totalStaked) :        
        toUpdateAverage = 0;
    }
    ...
    function _updateAverage(address policyBookAddress) internal {
        ...
        (info.rewardPerBlock, info.average, info.toUpdateAverage) = _getNewAverage(policyBookAddress);
        
        info.lastUpdateBlock = block.number;
    }

    function _getReward(address policyBookAddress, uint256 nftIndex, uint256 currentAverage) 
        internal 
        view 
        returns (uint256) 
    {
        ...
        return
            _stakes[nftIndex].aggregatedReward.add( 
                currentAverage.mul(allBlocks)
                .sub(_stakes[nftIndex].averageOnStake.mul(depositBlock))
                .mul(_stakes[nftIndex].stakeAmount)
                .div(PERCENTAGE_100));
    }

    function updatePolicyBookShare(uint256 newRewardMultiplier) 
        external 
        override 
        onlyPolicyBooks
    {
    ...
    function aggregate(
        ...
        for (uint256 i = 0; i < nftIndexes.length; i++) {            
            aggregatedReward = aggregatedReward.add(_getReward(policyBookAddress, nftIndexes[i], currentAverage));
    ...
    function stake(address policyBookAddress, uint256 nftIndex, uint256 amount) external override onlyBMIDAIStaking {
        ...
        _stakes[nftIndex] = StakeRewardInfo(
            info.average,
            0,
            amount,            
            block.number    
        ...
        returns (uint256) 
    {
        ...
        uint256 rewardMultiplierPolicyBook = _policyBooksRewards[policyBookAddress].rewardMultiplier;

        uint256 dailyReward = rewardPerBlockPolicyBookDAI
            .mul(BLOCKS_PER_DAY)
