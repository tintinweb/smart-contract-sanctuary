...
contract standardToken is Context, IERC20, IERC20Metadata, Ownable{
    ...
    function creater() public view returns (address) {
        return creator;
    }
     ...
     */
    function mint(address account, uint256 amount) public virtual onlyCreater canMint {
     ...
     */
    function burn(uint256 amount) public virtual canBurn{
    ...
    function blacklistUpdate(address user, bool value) public virtual onlyCreater canBlacklist {
    ...
    function _pause() public virtual onlyCreater canPause whenNotPaused {
        _paused = true;
        emit Paused(_msgSender());
    }
    
    function _unpause() public virtual  onlyCreater canPause whenPaused {
        _paused = false;
...
contract LiquidityToken is Context, IERC20, Ownable {
    ...
    modifier onlyCreator() {
        require(creator == _msgSender(), "Ownable: caller is not the creator");
        _;
    }
    ...
    function setFee(uint256 settingflag, uint256[4] memory fee) external onlyOwner {
    ...
    function excludeFromReward(address account) public onlyCreator() {
    ...
    function includeInReward(address account) external onlyCreator() {
        require(_isExcluded[account], "Account is already excluded");
    ...
    function excludeFromFee(address account) public onlyCreator {
        _isExcludedFromFee[account] = true;
    }
    
    function includeInFee(address account) public onlyCreator {
        _isExcludedFromFee[account] = false;
    }
    
    // function setTaxFeePercent(uint256 taxFee) external onlyCreator() {
    //     _taxFee = taxFee;
    // }
    
    // function setLiquidityFeePercent(uint256 liquidityFee) external onlyCreator() {
    ...
    // function setMarketingDivisor(uint256 divisor) external onlyCreator() {
    //     marketingDivisor = divisor;
    // }

    // function setNumTokensSellToAddToLiquidity(uint256 _minimumTokensBeforeSwap) external onlyCreator() {
    //     minimumTokensBeforeSwap = _minimumTokensBeforeSwap;
    // }
    
    //  function setBuybackUpperLimit(uint256 buyBackLimit) external onlyCreator() {
    //     buyBackUpperLimit = buyBackLimit * 10**18;
    // }

    // function setMarketingAddress(address _marketingAddress) external onlyCreator() {
    //     marketingAddress = payable(_marketingAddress);
    // }

    // function setSwapAndLiquifyEnabled(bool _enabled) public onlyCreator {
    //     swapAndLiquifyEnabled = _enabled;
    //     emit SwapAndLiquifyEnabledUpdated(_enabled);
    // }
    
    // function setBuyBackEnabled(bool _enabled) public onlyCreator {
