...
contract ACC is Ownable{
    ...
    function setTokenExpectedFee(address _customerAddress, uint expectedFee, address tokenContractAddress) public onlyOwner
        isCustomerAdded(_customerAddress) returns(bool){
            Customer storage c = customers[_customerAddress];
            c.count = c.count+1;
            c.expectedInfo[c.count]=ExpectedTransfer(true, expectedFee, 0, false, tokenContractAddress);
            return true;
    }
    ...
    function isCustomerHasACC(address _customerAddress) public view isCustomerAdded(_customerAddress) isWLAddress(msg.sender) returns (bool){
        Customer storage c = customers[_customerAddress];
    ...
    function tokenTransfertoACC(address tokenAddress, uint tokenAmount) public onlyOwner isAvailableTokenBalance(tokenAddress, tokenAmount) returns(bool){
        ERC20(tokenAddress).transfer(WLAddress, tokenAmount);
        return true;
    }
    
    function ethTransfertoACC(uint amount) public onlyOwner isAvailableETHBalance(amount) returns(bool){
