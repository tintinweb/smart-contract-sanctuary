...
contract CollateralDetails is Ownable {
    ...
    event recordCertificate(uint position, string document, string book, uint256 bags, uint256 tokens, string execType);
    ...
    function getCollateral(uint position_) public view returns(string memory document, string memory book, uint256 bags, uint256 tokens ,string memory execType){
        return (_collateral[position_].document, _collateral[position_].book, _collateral[position_].bags, _collateral[position_].tokens, _collateral[position_].execType);
    }
    
    function getLastRecord() public view returns(uint256 id,string memory document, string memory book, uint256 bags, uint256 tokens ,string memory execType){
        return (_collateralList.length-1, _collateral[_collateralList.length-1].document, _collateral[_collateralList.length-1].book, _collateral[_collateralList.length-1].bags, _collateral[_collateralList.length-1].tokens, _collateral[_collateralList.length-1].execType);
    }
    
    function registryCollateral(string memory document_, string memory book_, uint256 bags_, uint256 tokens_, string memory execType_) internal onlyOwner{
        _collateralList.push(Collateral(document_,book_,bags_,tokens_,execType_));
        _collateral[_collateralList.length-1] = Collateral(document_,book_,bags_,tokens_,execType_);
        emit recordCertificate(_collateralList.length-1, document_,book_,bags_,tokens_,execType_);
    }
    
}

contract StandardToken is IERC20, Ownable, Pausable, CollateralDetails{
    ...
    constructor (string memory name_, string memory symbol_, uint256 totalSupply_, uint8 decimals_, string memory document_, string memory book_, uint256 bags_)  {
        _name = name_;
        _symbol = symbol_;
        _totalSupply = totalSupply_.mul(10 ** decimals_);
        _decimals = decimals_;
        _balances[msg.sender] = _balances[msg.sender].add(_totalSupply);
        registryCollateral(document_, book_, bags_, _totalSupply, "CONTRACT START");
    ...
    * 
    */
    function burn(uint256 amount_, string memory document_, string memory book_, uint256 bags_) public onlyOwner whenNotPaused{
        ...
        registryCollateral(document_, book_, bags_, amount_, "BURN");
    ...
    * 
    */
    function mint(uint256 amount_, string memory document_, string memory book_, uint256 bags_)public onlyOwner whenNotPaused{
        uint256 _amount = amount_.mul(10 ** _decimals);
        _totalSupply = _totalSupply.add(_amount);
        _balances[msg.sender] = _balances[msg.sender].add(_amount);
        uint256 _currentBags = getCurrentBags();
        setCurrentBags(_currentBags.add(bags_));
        registryCollateral(document_, book_, bags_, amount_, "MINT");
