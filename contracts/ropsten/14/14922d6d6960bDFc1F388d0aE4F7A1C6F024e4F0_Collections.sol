...
interface ICollections {
    function createCollection(bytes32 root, string calldata URI) external;
    function detachItem(uint256 collectionId, string calldata URI, bytes calldata proof) external;
    function joinItem(uint256 collectionId, uint256 tokenId) external;
}

interface IItems is IERC721Metadata {
...
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721URIStorage.sol";
import {ICollections, IItems} from "./Interfaces.sol";

contract Items is IItems, ERC721URIStorage {
    ...
    modifier onlyCollectionContract() {
        require(msg.sender == collectionContract);
        _;
    }

    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        collectionContract = msg.sender;
    }

    function mint(address to, string memory URI) onlyCollectionContract external override returns(uint256) {
        _currentTokenId++;
