...
contract Claimable{
    ...
    function _getClaimableBalance(
        ...
        require(now >= userMapping[_tokenAddress][_userAddress].vestingStartTime, "TGE doesn't started yet! Please Comeback Later.."); // enter when vesting starts
        require(now >= userMapping[_tokenAddress][_userAddress].cliffTimePeriod,"Cliff Time doesn't End yet! Please Try Later..");
            ...
            //FIXME: known issues : check if balance is too low
            //FIXME: known issues : multiple every amount with 100
            //FIXME: known issues : Counting Days from day 1 even users withdrawals his/her day 1 tokens
            //FIXME: known issues : If user didn't withdrawal his/her TGE claimable balance than handle other equality operator
            ...
            if(userMapping[_tokenAddress][_userAddress].withdrawalBalance>=userMapping[_tokenAddress][_userAddress].orginalBalanceAfterCliff + userMapping[_tokenAddress][_userAddress].claimableBalance){
                return 0;
            }
            if(userMapping[_tokenAddress][_userAddress].withdrawalBalance == userMapping[_tokenAddress][_userAddress].totalBalance){
                return 0;
            }
    ...
    function _addUserAndBalances(address[] memory _userArray, uint _percentageShareTGE,uint _startTgeTime, uint _cliffTimePeriod, uint _interval, uint _vestingEndTime, uint _claimableBalance, ERC20 token, address _tokenAddress,uint _vestingPercentage, uint[] memory _balc) public {
        ...
        for(uint i= 0;i < _userArray.length; i++) {
            ...
            tokenArray.push(_tokenAddress);
                ...
                userMapping[_tokenAddress][_userArray[i]] = (usersDetails(_userArray[i],_balc[i], _balc[i], _balc[i],_startTgeTime,_percentageShareTGE,_startTgeTime +_cliffTimePeriod * oneMinute, _interval * oneMinute, _startTgeTime+ _cliffTimePeriod* oneDay + _vestingEndTime * oneDay, _claimableBalance, _vestingPercentage, 0));
