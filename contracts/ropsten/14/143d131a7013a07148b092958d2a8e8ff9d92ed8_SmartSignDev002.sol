...
contract SmartSignDev002 {
    ...
    // end region
    // region Create
    function setNewRegister(string _dataHash, bytes32 _csv, uint256 _timestamp)  
        isOwner(msg.sender) 
        isDeprecated() 
        public returns(string, bytes32, uint256){
    ...
    function getRegister(string dataHash, bytes32 csv) 
        ...
        isDeprecated()
        public returns(string, bytes32, uint256){
            if(!isRepeated(dataHash)) return ("Not found", csv, 0);
            return (
