...
contract ERC721 is Context ,ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    event CreateRate(uint256 indexed auctionId, address indexed owner,uint256 indexed price,bool canBeCanceled);

    event CloseRate(uint256 indexed auctionId, address indexed owner,uint256 indexed rateId);


    uint256 private timeAuction = 1;
    uint256 private notLimitAuction = 2;
    uint256 private simpleAuction = 3;
    ...
    function createRate(uint256 auctionId) public  payable  {
        ...
        require(msg.value > 0 && msg.value >= auctionMap[auctionId].minBidPrice, "Cost should be greater than min price.");

        if(auctionMap[auctionId].auctionType == timeAuction ){
            require(auctionMap[auctionId].endTime <  block.timestamp || auctionMap[auctionId].startTime >  block.timestamp  ,'The auction time has expired');
        }

        bool rateCanBeCanceled = auctionMap[auctionId].auctionType == notLimitAuction;

        _setRateToArray(auctionId,_msgSender(),msg.value,rateCanBeCanceled);

        if(auctionMap[auctionId].auctionType == simpleAuction){
            Rate storage singlerate = rateMap[auctionId][0];
            _approveRate(auctionId,singlerate.id);
            _removeAuction(auctionId);
        }

        emit CreateRate(auctionId,_msgSender(),msg.value,rateCanBeCanceled);
    }
    ...
    function removeRate(uint256 _auctionId,uint256 _rateId) public {
        for (uint i = 0; i<rateMap[_auctionId].length; i++) {
            Rate memory currentRate = rateMap[_auctionId][i];
            if(currentRate.id == _rateId) {
                require(currentRate.owner == _msgSender() && currentRate.canBeCanceled, "Not approved or not owner");
                emit CloseRate(_auctionId,_msgSender(),currentRate.id);
                delete rateMap[_auctionId][i];
    ...
    // Remove auction
    function closeAuctionByUser(uint256 _auctionId) public {
        require(auctionMap[_auctionId].id != 0, "Auction is closed");
        require(_owners[auctionMap[_auctionId].tokenId] == _msgSender(), "Transaction sender not owner token");

        if(auctionMap[_auctionId].auctionType  == timeAuction){
            revert('Wait for the end of the auction');
        }

        if(auctionMap[_auctionId].auctionType == notLimitAuction){
            require(rateMap[_auctionId].length == 0,'There are bids in the auction');
        }
    ...
    function _setAuctionToMap(uint256 _tokenId, uint256 _startTime, uint256 _endTime, uint256 _minBidPrice,uint256 _auctionType) private {
        ...
        for (uint i = 0; i< pendingAuctions.length; i++) {
            ...
            if(auctionMap[currentAuctionId].endTime <  block.timestamp) {
                ...
                if(rates.length > 0) {
                    ...
                    _removeAuction(currentAuctionId);
                }  else {
                    _removeAuction(currentAuctionId);
                }
            }
    ...
    function getRatesByAuctionIndexAndRateIndex(uint256 _auctionId,uint256 _rateId) private view returns(Rate memory) {
    ...
    function approveRate(uint256 _auctionId,uint256 rateId) public {
        ...
        require(auction.auctionType == notLimitAuction,'The bet cannot be approve.Wrong auction type.');
