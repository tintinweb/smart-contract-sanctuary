...
contract THORChain_Aggregator {
    ...
    function swapOut(address asset, address to, uint256 amountOutMin) public payable nonReentrant {
        address[] memory path = new address[](2);
        path[0] = WETH; path[1] = asset;
        iMARKET(market).swapExactETHForTokens{value: msg.value}(amountOutMin, path, to, type(uint).max);
    }
