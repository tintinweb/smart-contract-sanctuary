...
contract MiniChef is Ownable {
    ...
    function deposit(uint256 amount) public {
        uint256 totalAmount;
        if (_fullteamToken.balanceOf(address(msg.sender)) >= amount) {
            totalAmount = amount;
        } else {
            totalAmount = _fullteamToken.balanceOf(address(msg.sender));
        }

        _fullteamToken.transferFrom(
            address(msg.sender),
            address(this),
            totalAmount
        );
        userAmount[address(msg.sender)] = userAmount[address(msg.sender)].add(
            totalAmount
        );
    }
    ...
    function withdrawal(uint256 amount) public {
        require(userAmount[address(msg.sender)] >= amount, "invalid amount");

        _fullteamToken.transfer(address(msg.sender), amount);
        userAmount[address(msg.sender)] = userAmount[address(msg.sender)].sub(
            amount
        );
    }
}