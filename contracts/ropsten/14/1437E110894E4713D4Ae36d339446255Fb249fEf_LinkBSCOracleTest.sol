...
contract LinkBSCOracleTest is Ownable {
    ...
    function getOracleAddress(string memory pair_name) public view returns(address) {
        for(uint i = 1; i <= OracleId ; i++) {
            address OracleAddr = IdToOracle[i];
            string memory OraclePairName = Oracles[OracleAddr];
            if (keccak256(abi.encodePacked(OraclePairName)) == keccak256(abi.encodePacked(pair_name))) {
                return OracleAddr;
            }
        }
    ...
    function removeOracle(address oracle_addr) public onlyOwner returns(bool) {
        delete Oracles[oracle_addr];
        return true;
    }
    
    function removeOracles(address[] memory oracles_addr) public onlyOwner returns(bool) {
        for(uint i = 0; i <= oracles_addr.length ; i++) {
            address oracle_addr = oracles_addr[i];
            delete Oracles[oracle_addr];
        }
