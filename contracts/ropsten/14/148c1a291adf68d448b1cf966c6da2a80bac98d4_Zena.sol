...
contract Zena is ERC20, proofOfAgeProtocol, Ownable {
    using SafeMath for uint256;

    string public name = "Zena";
    string public symbol = "ZENA";
    ...
    //Default pool fee is 5%, and it send directly to internal Proof of Age minting pool
    uint public poolFee = 500;
    ...
    mapping(address => uint256) shareTransfer;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint poolSupply = value.mul(poolFee).div(1e4);
        balances[msg.sender] = balances[msg.sender].sub(value).sub(poolSupply);
        
        uint valueAfterFee = value.sub(poolSupply);
