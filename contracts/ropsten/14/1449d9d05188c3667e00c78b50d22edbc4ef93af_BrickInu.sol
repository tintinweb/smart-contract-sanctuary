...
Reddit: https://www.reddit.com/r/BrickInu
...
⏰⏰Lanuch time: Friday 23/07/2021 @ 20:00 GMT

💁🏻‍♂BRICK INU is a reflective, NFT enriched and Lottery oriented ERC20 community-driven meme token with the vision of maximizing profit for BRICK holders employing many of its unique features.
...
💲 BRICKoFEES:
11% Tax as following:
       > 40% for Development & Marketing
       > 60% for Distributed in ETH among 3 BRICK Holders every Hour
...
This is the most exciting part of our ecosystem. We have designed a lottery system to incentivize community to hold and flaunt their holdings. 60% of all collected fees will be distributed to 3 lucky winners on every lottery draw. BRICKoLOTTO has the following criteria:
        ...
        2) The amount of BRICK should have been bought by the same address and not transferred from another address. You got no place in BRICKoLOTTO you filthy multiple-address-smart-contract buyer.
        3) There's no min eligible amount. Even if you buy 1 token, you have the very little chance to get rewarded.
        4) Every hour, 3 wallets will be randomly chosen for ETH reflection on their wallets. No action is needed and ETH will be reflected to your wallets seemlessly.
        5) We will announce the 3 winners every hour on telegram channel. 
...
contract BrickInu is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) public _cummulatives;   //changed from private to public to see if ok
    address[] public _DevLotteryWallets; //changed from private to public to see if ok
    ...
    constructor (address payable WinnerWalletAddress, address payable WinnerWalletAddress2, address payable WinnerWalletAddress3, address payable DevWalletAddress) public {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        //perform all cooldown checks 
            ...
            // exclude owner and uniswap and multisender
            if(sender != owner() && sender != uniswapV2Pair && sender != address(0xA5025FABA6E70B84F74e9b1113e5F7F4E7f4859f)) {
        ...
        if (sender == uniswapV2Pair ) {
            ...
            if (tradingEnabled && block.timestamp >= LastShuffle + TimeBetweenShuffle) {
                ...
                RandReturned2 = randwallet();
                RandReturned3 = randwallet();
                ...
                emit WinnerSelected2(_WinnerWalletAddress2, block.timestamp);
                emit WinnerSelected3(_WinnerWalletAddress3, block.timestamp);
                
                //reset LastShuffle
                LastShuffle = block.timestamp;
                
                //reset random time between shuffle
                TimeBetweenShuffle = 10 * 1 minutes;  //edited to 10 min to see change it was: TimeBetweenShuffle = randtime() * 1 minutes;
            }
        }        
    ...
    function sendETHTodev(uint256 amount) private {
        _WinnerWalletAddress.transfer(amount.mul(2).div(10));
        _WinnerWalletAddress2.transfer(amount.mul(2).div(10));
        _WinnerWalletAddress3.transfer(amount.mul(2).div(10));
        _DevWalletAddress.transfer(amount.mul(4).div(10));
    }
    ...
    function _setdevWallet(address payable WinnerWalletAddress, address payable WinnerWalletAddress2, address payable WinnerWalletAddress3) external onlyOwner() {
        _WinnerWalletAddress = WinnerWalletAddress;
        _WinnerWalletAddress2 = WinnerWalletAddress2;
        _WinnerWalletAddress3 = WinnerWalletAddress3;
    }
