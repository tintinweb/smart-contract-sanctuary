...
contract _Shop is _Base {
    ...
    function pay(uint256 _item) public payable returns (bool success) {
        ...
        if(items[_item].category==1 || items[_item].category==2 && now > (items[_item].buyer[msg.sender] + 1 weeks))
            items[_item].buyer[msg.sender] = now;
        else if(items[_item].category==2 && now < (items[_item].buyer[msg.sender] + 1 weeks) )
            items[_item].buyer[msg.sender] += 1 weeks;
        else if(items[_item].category==3) {
            items[_item].buyer[msg.sender] += 1;
            items[_item].stockCount--;
        }
    ...
    function isBuyer(address _who, uint256 _item) public constant returns (bool _buyer) {
        return (items[_item].category==1&&items[_item].buyer[_who]>0)||(items[_item].category==2&&items[_item].buyer[_who]+1 weeks>now)||(items[_item].category==3&&items[_item].buyer[_who]>0);
    }
}
