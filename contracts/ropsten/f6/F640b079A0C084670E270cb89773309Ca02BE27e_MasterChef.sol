...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 durationOfPool;       // How many allocation points assigned to this pool. KWIKs to distribute per block.
        uint256 rewardAPY;  // Last block number that KWIKs distribution occurs.
    }
    ...
    function pendingKwik(uint256 _pid, address _user) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        // uint256 accKwikPerShare = pool.accKwikPerShare;
        uint256 finalClaimEndTime = user.depositTime.add(pool.durationOfPool.mul(60));
        uint256 rewardUserPerDay = user.amount.mul(pool.rewardAPY).div(100).div(pool.durationOfPool);
        uint256 stakingDays;
        if(block.timestamp <= finalClaimEndTime){
            stakingDays = (block.timestamp.sub(user.lastClaim)).div(60);
        }
        else{
            stakingDays = (finalClaimEndTime.sub(user.lastClaim)).div(60);
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        // updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = pendingKwik(_pid , msg.sender);
