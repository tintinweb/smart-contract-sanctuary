...
contract LostPunks is ERC721Enumerable, Ownable {
    ...
    string private constant SVG_HEADER = 'data:image/svg+xml;utf8,<svg id="punk-svg" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 24 24">';
    string private constant SVG_FOOTER = '<style>rect{width:1px;height:1px;} #punk-svg{shape-rendering: crispedges;} .backdrop{width:24px;height:24px;}</style></svg>';
        ...
        for (uint y = 0; y < 24; y++) {
            for (uint x = 0; x < 24; x++) {
                ...
                if (uint8(pixels[p + 3]) > 0) {
                    ...
                    svg = string(abi.encodePacked(svg, rectSvg(x, y, false, color)));
                }
            }
        }
