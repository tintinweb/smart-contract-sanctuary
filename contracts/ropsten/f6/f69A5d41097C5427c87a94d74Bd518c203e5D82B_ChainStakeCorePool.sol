...
abstract contract ChainStakePoolBase is IPool, ReentrancyGuard {
     ...
     */
    function unstakeAll(uint256 _amount) external override {
      //check for total reward token on factory address
     require(factory.isReward() || factory.rewardStatus(),"Error:Reward token not Supplied ");
    ...
    function vestingAllowed(address _staker) internal returns (bool) {
        User storage user = users[_staker];
        if(blockNumber() >= factory.endBlock()){
            user.userLastRewardDistribution = uint256(now256());
            user.remainderTime = 0;
            return true;
        }
 ...
 /**
    ...
    return calculatePendingRewardToTransfer(_staker).add(pendingYieldRewards(_staker));
    }
...
/**
     ...
     *
     * @param _rewardStatus status can be alter by owner, reward status has effect on unstake functionality.
     */
    function updateRewardStatus(bool _rewardStatus) external onlyOwner {
    ...
    function updateEndBlock(uint256 _rewardsupply) external onlyOwnerOrSubAdmin {
        ...
        //is reward check
         require(isReward,"Transfer Error : Reward token transfered to factory address");
