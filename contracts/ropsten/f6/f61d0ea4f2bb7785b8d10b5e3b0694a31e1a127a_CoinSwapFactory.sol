...
interface ICoinSwapPair is ICoinSwapERC20 {
    ...
    function initialize(address, address, uint224) external;
    ...
    function getReserves() external view returns (uint224 _reserve, uint224 _circleData);
    function priceCumulative() external view returns (uint);
    function mint(address to) external returns (uint liquidity);
    function burn(address to) external returns (uint192 amount);
...
contract CoinSwapPair is ICoinSwapPair, CoinSwapERC20 {
    ...
    address public override patron;
    address public override factory;
    address public override token0; // token0 < token1
    address public override token1;
    uint224 private reserve; //reserve0(96) | reserve1(96) | blockTimestampLast(32)
    uint private unlocked = 1;
    uint public override priceCumulative; //=Delta_y/Delta_x: 96-fractional bits; allows overflow
    ...
    function initialize(address _token0, address _token1, uint224 circle) external override {
    ...
    function ICO(uint224 _circleData)  external override {
        require( (tx.origin==patron) && (circleData >> 216) >0, 'CSWP:3');//to close ICO, set (circleData >> 216) = 0x00
        circleData = _circleData;
    }

    function setPatron(address _patron)  external override {
        require( (tx.origin==patron), 'CSWP:11');
        patron = _patron;
    }
    
    function getReserves() public override view returns (uint224 _reserve, uint224 _circleData) {
        _reserve = reserve;
    ...
    function burn(address to) external override lock returns (uint192 amount) {
        uint224 _reserve = reserve;
    ...
    function swap(uint amountOut, address to, bytes calldata data) external override lock {       
    ...
    function skim(address to) external override lock {
        ...
        address _token1 = token1; 
        uint224 _reserve = reserve;
        _safeTransfer(_token0, to, IERC20(_token0).balanceOf(address(this))-uint96(_reserve >>128));
        _safeTransfer(_token1, to, IERC20(_token1).balanceOf(address(this))-uint96(_reserve >>32));
	    revisemu(uint192(_reserve>>32));
    }

    // force reserves to match balances
    function sync() external override lock {
        _update(IERC20(token0).balanceOf(address(this))<<128 | IERC20(token1).balanceOf(address(this))<<32 | uint32(reserve));
    }
}
...
contract CoinSwapFactory is ICoinSwapFactory {
    ...
    function createPair(address tokenA, address tokenB, uint224 circle) external override returns (address pair) {  
