...
contract NewSmartPyramid {
	...
	function withdraw() notOnPause public {
		...
		if (block.timestamp >= x.c(msg.sender) + 10 minutes) {
			uint _payout = (x.d(msg.sender).mul(x.getInterest(msg.sender)).div(1000)).mul(block.timestamp.sub(x.c(msg.sender))).div(1 days);
