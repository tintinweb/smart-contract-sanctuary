...
contract CRD is Context, ERC20, Ownable, ICRD, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event LOG_MASTER_TRANSFER (
        address indexed to,
        uint256 amount
    );

    event LOG_SET_MASTER_ADDRESS (
    ...
    modifier onlyAuthorizedMintCaller() {
        require(_msgSender() == owner() || _authorizedMintCaller[_msgSender()],"CRD: MINT_CALLER_NOT_AUTHORIZED");
        _;
    }

    constructor (
        address _masterAddr
    ) public ERC20("CRD Swap Token", "CRD") {
    ...
    function setMasterAddr(address _new) external override {
        address _masteraddr=masterAddr;
        require(_msgSender() == _masteraddr, "CRD: setter must be master");
        require(_new != address(0), "CRD: set to the zero address");
        require(_new != _masteraddr, "CRD: master address is same");
        uint _amount = lockedOf(_masteraddr);
        if (_amount > 0) {
            _transfer(_masteraddr, _new, _amount);            
        }
        masterAddr = _new;
        emit LOG_SET_MASTER_ADDRESS(_new);
    }

    function transferFromMaster(address recipient, uint256 amount) external override returns (bool) {
        address _masteraddr = masterAddr;
        address _senderaddr = _msgSender();
        require(_senderaddr == _masteraddr, "CRD: transfer account is not masteraddr");
        require(amount <= balanceOf(_masteraddr), "CRD: transfer amount exceed");        
        _mintUnlocked(_senderaddr, amount);
        transfer(recipient, amount);       
        emit LOG_MASTER_TRANSFER(recipient, amount);
        return true;
    }

    function burn(uint256 amount) external override returns (bool) {
        address _masteraddr = masterAddr;
        require(_msgSender() == _masteraddr, "CRD: burner must be master");
        require(lockedOf(_masteraddr) >= amount, "CRD: burn amount exceed lockedof");
        _burn(_masteraddr, amount);        
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override(ERC20, IERC20) returns (bool) {
        ...
        _approve(sender, _msgSender(), allowance.sub(amount, "CRD: TRANSFER_AMOUNT_EXCEEDED"));
        return true;
    }
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {        
        require(_msgSender() != masterAddr, "CRD: masterAddr can not be staked");
        require(_unlockFactor[token] > 0, "CRD: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "CRD: BLOCK_GAP_NOT_SET");        
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_msgSender() != masterAddr, "CRD: masterAddr can not be unstaked");
        require(amount > 0, "CRD: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "CRD: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    function claimUnlocked(address token) external override nonReentrant returns (bool) {
        require(_msgSender() != masterAddr, "CRD: masterAddr can not be claimed");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    function mintUnlockedToken(address to, uint256 amount) onlyAuthorizedMintCaller external override {
        ...
        require(totalSupply() <= 10**26, "CRD: TOTAL_SUPPLY_EXCEEDED");
    }
