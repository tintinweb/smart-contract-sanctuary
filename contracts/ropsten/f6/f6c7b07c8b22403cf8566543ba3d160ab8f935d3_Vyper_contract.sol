...
TPET: constant(address) = 0x240f43A6943837F7107911A325A33387151Ce51A
...
def __init__():
    self.DOMAIN_SEPARATOR = keccak256(
        concat(
            DOMAIN_TYPE_HASH,
            keccak256(convert("sTpet", Bytes[6])),
...
def woof(amount: uint256 = MAX_UINT256, receiver: address = msg.sender) -> bool:
    mint_amount: uint256 = min(amount, ERC20(TPET).balanceOf(msg.sender))
    assert ERC20(TPET).transferFrom(msg.sender, self, mint_amount)
...
def unwoof(amount: uint256 = MAX_UINT256, receiver: address = msg.sender) -> bool:
    ...
    assert ERC20(TPET).transfer(receiver, burn_amount)
    return True
