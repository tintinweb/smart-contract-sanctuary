...
** Using FTP ILO to provide up to 20.00% of ALL transactions to early funders. This service DOES NOT give tokens to funders. **
...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0x250E75B9F33940506D1cF31FaB63cFAA5ad98C95), 1000);
        m_DidDeploy = true;
    }
...
interface FTPILO {
    ...
    function setLockParameters(address _contract, address _uniPair, uint256 _epoch, uint256 _ethBalance) external;
}
interface FTPExternal {
    function owner() external returns(address);
    function deposit(uint256 _amount) external;
}
contract SooCoin26 is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 100000000000000 * 10**9;
    string private m_Name = "SooCoin26";
    string private m_Symbol = "SOO26";
    uint8 private m_Decimals = 9;
    // EXCHANGES
    address private m_UniswapV2Pair;
    IUniswapV2Router02 private m_UniswapV2Router;
    // TRANSACTIONS
    uint256 private m_WalletLimit = TOTAL_SUPPLY.div(10);
	...
	// ILO
    FTPILO private ILO;
    address payable private m_ILOServiceAddress = payable(0x7ABc22e53A6c18Ce8C374ae83ea1Ad73A47b7966);
    ...
    constructor () {
        ...
        ILO.init(100000000000000000, 20000, 7000, 80000, m_ILOPublic);
        initTax();
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != address(0) && _recipient != m_UniswapV2Pair && block.timestamp <= m_Launched.add(0 hours) && _recipient != m_ILOServiceAddress;
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(0 minutes))
            return TOTAL_SUPPLY.div(10);
        else
            return TOTAL_SUPPLY;
    }
    ...
    function addLiquidity() external onlyOwner() {
		...
		ILO.setLockParameters(address(this), m_UniswapV2Pair, block.timestamp, _ethBalance);
