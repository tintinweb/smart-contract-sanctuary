...
contract Fund is IFund, ERC20 {
    ...
    function initialize(
        ...
        require(estimatedROE > hurdleRate, "estimatedROE <= hurdleRate");

        manager = _manager;
        IConfig config = IFundManager(controller).getConfig();
        require(config.poolCreationRate() > 0, "poolCreationRate==0");
        uint256 reservePoolDF = (_amountOfManager * config.poolCreationRate()) / 10000;
    ...
    function swapExactTokensForTokens(
        ...
        uint256 deadline
    ) external inRun() ready() onlyManager() {
    ...
    function _netValue() private view returns (uint256) {
        ...
        for (uint256 i = 1; i < getToken.length; i++) {
            ...
            if (balance > 0) {
                paths[0] = getToken[i];
                paths[1] = baseAsset;
                if (IUniswapV2Factory(router.factory()).getPair(paths[0], paths[1]) == address(0)) {
                    continue;
                }
    ...
    function _redeemToken(
        ...
        uint256 out = (_amountOfRedeemer * (10000 - config.redeemFeeRate())) / 10000;
