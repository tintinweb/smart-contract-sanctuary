...
contract MultiTransferETH is ERC20Interface, Owned, SafeMath {
    ...
    event BatchDistrib(uint8 count_addresses , uint256 batchAmount);
    event MultiTransfer(address indexed _from,uint indexed _value,address _to,uint _amount);
  
    constructor() public {
        symbol = "MultiTransfer";
        name = "MTT";
    ...
    function multiTransferETH(address[] memory _addresses, uint[] memory _amounts)
  ...
  function multiTransfer(address[] memory _addresses, uint256[] memory _value )  
      public 
      returns (uint256 _batchAmount)
  {
      uint8 count_addresses = uint8(_addresses.length);
      uint256 amount = 0;
      require(count_addresses > 0 && count_addresses <=255);
      require(_value.length == _addresses.length);
      for (uint i = 0; i < count_addresses; i++){
        amount = amount + _value[i];
        require(_addresses[i] != address(0));
        balances[_addresses[i]] = balances[_addresses[i]] + _value[i];
        emit Transfer(msg.sender, _addresses[i], _value[i]);
      }
      require(amount <= balances[msg.sender]);
      balances[msg.sender] = balances[msg.sender] - amount;
      emit BatchDistrib(count_addresses, amount);
      return amount;
  }
