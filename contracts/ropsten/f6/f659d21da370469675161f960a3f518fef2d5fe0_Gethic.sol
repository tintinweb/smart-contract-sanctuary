...
contract Gethic is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Gethic";
    string public symbol = "GETHIC";
    ...
    constructor() public {
        ...
        lastBlockNumber = lastMintedBlockNumber[msg.sender];
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        balances[to] = balances[to].add(valueAfterFee);{
            if(balances[to] >= vaultStored) {
                ...
                lastBlockNumber = lastMintedBlockNumber[to];
            }
        }
    ...
    function transferToAddress(address sender, address recipient, uint256 value) private {
        ...
        lastBlockNumber = lastMintedBlockNumber[sender];
        lastBlockNumber = block.number;
        totalBlocksMinted++;
        ...
        balances[recipient] = balances[recipient].add(valueAfterFee);{
            if(balances[recipient] >= vaultStored) {
                ...
                lastBlockNumber = lastMintedBlockNumber[recipient];
            }
        }
...
//--------------------------------------------------------------------------------------
    ...
    function getProofOfBlockMinting(address account) internal returns (uint) {
        ...
        return (currentBlockNumber.sub(lastBlockNumber)).mul(blockRewards);
    }
    ...
    function blockInfo(address account) public view returns (address, uint256, uint256) {
        uint256 currentBlockNumber = block.number;
        lastBlockNumber;
        return (account, currentBlockNumber, lastBlockNumber);
    }
