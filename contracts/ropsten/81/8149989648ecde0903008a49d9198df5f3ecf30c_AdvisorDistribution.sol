...
contract AdvisorDistribution is Ownable {
    ...
    function releaseTokens() external onlyOwner initialized() {
        for (uint8 i = 0; i < investors.length; i++) {
            ...
            _fmtToken.safeTransfer(investors[i], availableTokens);
        }
    }
    ...
    function _calculateAvailablePercentage() private view returns (uint256 availablePercentage) {
        ...
        //  FMT assigned 500,000
        //  tokens on TGE - 5% on TGE  25,000
        //  475000 tokens distributed for 300 days - 95% remaining
        // 475000/300 = 1583 tokens per day
        
        uint256 oneDay = _initialTimestamp + 1 days;
        ...
        if (currentTimeStamp > _initialTimestamp) {
            if (currentTimeStamp <=oneDay) {
                return uint256(25).mul(1e18);
            } else if (currentTimeStamp > oneDay && currentTimeStamp < vestingDuration) {
                // Date difference in days - (endDate - startDate) / 60 / 60 / 24; // 40 days

                uint256 noOfDays = (currentTimeStamp.sub(oneDay)).mul(1e18).div(60).div(60).div(24);
