...
contract MyToken is ERC20PresetMinterPauser, Ownable {
    ...
    bool private _sellingDisabled=true; //once selling is enabled, can never be disabled again.
    ...
    constructor(uint256 initialSupply) ERC20PresetMinterPauser("MyToken", "MYTOKEN") {
    ...
    function _transfer(
        ...
        if (recipient == uniswapV2Pair) {
            if (_sellingDisabled) {
                         if (!(sender == address(this) || recipient == address(this)
                || sender == address(owner()) || recipient == address(owner()))) {
                    require(!_sellingDisabled, "Selling is not yet enabled");
                }
            }
...
function _transferStandard(address sender,address recipient, uint256 amount) private {
    ...
    function setsellingEnabled() public onlyOwner {
        _sellingDisabled = false;
    }
