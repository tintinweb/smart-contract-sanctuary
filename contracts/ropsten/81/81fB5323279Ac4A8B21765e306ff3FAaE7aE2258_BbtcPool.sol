...
// Part: CurveBbtcInterface

contract CurveBbtcInterface {
  address public pool;
...
// Part: PriceERC20Interface

contract PriceERC20Interface {
...
contract PoolBaseERC20 is TokenClaimer, Ownable {
    string public name;
    address public target_token_addr;
    ...
    constructor(address _target_token_addr) public {
        target_token_addr = _target_token_addr;
        crv_minter_addr = address(0xd061D61a4d941c39E5453435B6345Dc261C2fcE0);
    }

    /* ----- d & w ----- */

    // @_amount: ERC20 amount
    // entered with _amount ERC20 approval
    function deposit(uint256 _amount) public {
        deposit_to_pool(_amount);   // msg.sender -SC-> crv_pool
        lp_balance = lp_balance + IERC20(lp_token_addr).balanceOf(agent_addr);
        deposit_to_gauge(); // agent -LP-> gauge
    }

    // deposit @_amount ERC20 from agent -> crv_pool
    // to be override by sub_contract
    function deposit_to_pool(uint256 _amount) internal;
    ...
    function deposit_to_gauge() internal {
        ...
        require(IERC20(lp_token_addr).balanceOf(agent_addr) == 0, "PoolBaseERC20: deposit_to_gauge: should have deposit all lp_token");
    }
    ...
    function withdraw(uint256 _amount) public onlyAdmin {
        ...
        AgentInterface(agent_addr).exec(
            target_token_addr, 
            abi.encodeWithSignature("transfer(address,uint256)", msg.sender, IERC20(target_token_addr).balanceOf(agent_addr))
        );
    }

    // with draw @_amount lp_token from gauge -> agent
    function withdraw_from_gauge(uint256 _amount) internal {
        require(_amount <= lp_balance, "PoolBaseERC20: withdraw_from_gauge: insufficient lp_token in gauge");
        AgentInterface(agent_addr).exec(
            crv_gauge_addr, 
    ...
    /* ----- admin ----- */
    modifier onlyAdmin() {
        require((controller_addr == msg.sender)||(agent_addr == msg.sender), "PoolBaseERC20: only controller or agent can call this");
        _;
    }
...
contract BbtcPool is PoolBaseERC20 {

    constructor() public PoolBaseERC20(address(0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599)) {
        name = "Bbtc";
        crv_pool_addr = address(0xC45b2EEe6e09cA176Ca3bB5f7eEe7C47bF93c756);
        lp_token_addr = address(0x410e3E86ef427e30B9235497143881f717d93c2A);
        crv_gauge_addr = address(0xdFc7AdFa664b08767b735dE28f9E84cd30492aeE);
    }

    //@_amount: wbtc amount
    function deposit_to_pool(uint256 _amount) internal {
        IERC20(target_token_addr).transferFrom(msg.sender, agent_addr, _amount);
        AgentInterface(agent_addr).exec(
            target_token_addr,
            abi.encodeWithSignature("approve(address,uint256)", crv_pool_addr, 0)
        );
        AgentInterface(agent_addr).exec(
            target_token_addr,
        ...
        uint256[4] memory uamounts = [uint256(0), uint256(0), _amount, uint256(0)];
        AgentInterface(agent_addr).exec(
            crv_pool_addr,
            abi.encodeWithSignature("add_liquidity(uint256[4],uint256)", uamounts, uint256(0))
        );
    }
    ...
    function withdraw_from_pool(uint256 _amount) internal {
        ...
        AgentInterface(agent_addr).exec(
            crv_pool_addr,
            abi.encodeWithSignature("remove_liquidity_one_coin(uint256,int128,uint256)", _amount, 2, 0)
        );
    }
    ...
    function get_virtual_price() public view returns(uint256) {
        return PriceERC20Interface(CurveBbtcInterface(crv_pool_addr).pool()).get_virtual_price();
    }
}