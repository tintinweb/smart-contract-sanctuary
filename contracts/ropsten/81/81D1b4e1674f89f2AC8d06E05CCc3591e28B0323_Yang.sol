...
contract ERC20Fee is Context, IERC20, Ownable {
  ...
  uint256 private proposalVotingDuration = 86400;
  uint256 private minSelfVoteAmount = 10;
  ...
  struct Proposal {
    ...
    uint256 proVotes;
    uint256 conVotes;
    mapping (address => uint256) _proVotesAddress;
    mapping (address => uint256) _conVotesAddress;
  }
  ...
  function makeProposal(ProposalType _type, uint256 value, uint256 vote) public {
    require(vote >= minSelfVoteAmount, "makeProposal: self voting amount must be >=minSelfVoteAmount");
    require(_balances[_msgSender()] >= vote, "makeProposal: transfer amount exceeds balance");
    ...
    proposals[proposals.length.sub(1)]._proVotesAddress[_msgSender()] = vote;
    _transferExcluded(_msgSender(), address(burnAddress), vote);
  ...
  function voteProposal(uint256 index, uint256 amount, bool pro) public {
    ...
    if (pro) {
      ...
      proposals[index]._proVotesAddress[_msgSender()] = proposals[index]._proVotesAddress[_msgSender()].add(amount);
    } else {
      proposals[index].conVotes = proposals[index].conVotes.add(amount);
      proposals[index]._conVotesAddress[_msgSender()] = proposals[index]._conVotesAddress[_msgSender()].add(amount);
    }
  ...
  function proposalProposerVotes(uint256 index) public view returns (uint256) {
    require(proposals.length > index, "voteProposal: proposal doesn't exist");
    return proposals[index]._proVotesAddress[proposals[index].proposer];
  }
