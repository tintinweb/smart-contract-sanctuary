...
contract MyToken is Context, IERC20, Ownable {
    ...
    string private constant _name = "EthToken";

    string private constant _symbol = "ETHTK";
    ...
    function setFee(uint256 multiplier, bool _firstSell) private {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled){
                ...
                require(sellcooldown[from] <= block.timestamp, 'Too soon');



                uint feeMul = 1;

                bool _firstSell = false;



                if (amount <= balanceOf(uniswapV2Pair).mul(1).div(100)) {

                    sellcooldown[from] = block.timestamp + (30 minutes);

                    feeMul = 2;

                }else if(amount <= balanceOf(uniswapV2Pair).mul(2).div(100)){
                    ...
                    sellcooldown[from] = block.timestamp + (240 minutes);
