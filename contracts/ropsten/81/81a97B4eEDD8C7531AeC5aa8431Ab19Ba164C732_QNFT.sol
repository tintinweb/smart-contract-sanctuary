    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function withdrawETH(address payable _multisig) external override {
        require(
    ...
    function _bulkMintNfts(
        ...
        require(_count == _metaIds.length, "QNFT: length doesn't match");
        require(
            totalSupply() + _count <= maxSupply,
            "QNFT: nft count reached the total supply"
        );

        uint256 totalQstkAmount = _lockAmount * _count;
        ...
        require(
            nftCountByCharacter[_characterId] + _count <=
                nftSettings.characterMaxSupply(_characterId),
            "QNFT: character count reached at max supply"
        );
