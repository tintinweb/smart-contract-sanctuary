...
contract ProofOfBlockProtocolLayer {
    ...
    function annualBlockRewards() internal view returns (uint256);
    event ProofOfBlockMinting(address indexed to, uint256 _blockMinting);
    event BlockMarking(address _by, uint256 _time, uint256 _blockNumber);
}
...
contract Rethic is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Rethic";
    string public symbol = "RETHIC";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        emit BlockMarking(msg.sender, lastBlockTime, lastBlock);
...
//--------------------------------------------------------------------------------------
    ...
    function proofOfBlockMinting(address account) public returns (bool) {
        require(balances[account] > vaultStored);
        if(balances[account] <= vaultStored) revert();
    ...
    function annualBlockRewards() internal view returns (uint blockRewards) {
    ...
    function getProofOfBlockMinting(address account) internal view returns (uint) {
        ...
        return ((totalBlocksMinted - lastMintingTime) * blockRewards);
    }
