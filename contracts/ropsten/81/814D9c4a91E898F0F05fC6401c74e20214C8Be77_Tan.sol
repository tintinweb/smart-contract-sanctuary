...
contract CodeCheck {
    ...
    {
        REMOTE_CONTRACT = _remoteContract;
    }

    function checkCodeOG(address _address, string memory _code) external view
        onlyRemote
        returns (bool)
    {
        return keccak256(abi.encodePacked(O)) == keccak256(abi.encodePacked(_code));
    }

    function checkCodeWL(address _address, string memory _code) external view
        onlyRemote
        returns (bool)
    {
        return keccak256(abi.encodePacked(W)) == keccak256(abi.encodePacked(_code));
    }

    modifier onlyRemote() {
        require(
            msg.sender == REMOTE_CONTRACT,
            "Invalid caller"
        );
        _;
    }

    address private immutable REMOTE_CONTRACT;
    string private constant O = "O";
    string private constant W = "W";
}
...
abstract contract State  {
    ...
    modifier onlyPremintOrPublicMint() {
        require(
            (state == StateType.PREMINT) ||
            (state == StateType.MINT),
            INVALID_STATE);
        _;
    }
    ...
    enum StateType
    {
        ...
        MINTCLOSED        , // no more minting allowed; random seed Gen-1 requested
        GENERATION1STARTED  // random seed Gen-1 available, Gen-1 Tans revealed
    }
...
abstract contract VRFD20 is VRFConsumerBase {
    ...
    event GenerationSeedRequested(bytes32 indexed requestId, uint indexed generation);
    event GenerationSeedReceived (bytes32 indexed requestId, uint indexed generation, uint indexed seed);
    ...
    function requestGenerationSeed(uint requestForGeneration) internal
    {
        ...
        require(
            generationSeed[requestForGeneration] == 0,
            "Seed already requested");
        ...
        emit GenerationSeedRequested(requestId, requestForGeneration);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        // emit GenerationSeedReceived(requestId, generation, randomness);
    }
