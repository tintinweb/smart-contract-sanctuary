...
contract Noonercoin is ERC20 {
    ...
    function mintToken(address add) public returns (bool) {
        ...
        if (timeDiff > fromTime) {
            ...
            if (timeDiff < _cycleTime) {
                ...
                uint256 flag = mintingRateNoonerCoin *
                    10**18 +
    ...
    function popRandomVariable() public returns (bool) {
        ...
        if (occuranceOfRandonNumber[randomNumber] >= 24) {
            ...
            for (
                uint256 index = 0;
                index < __remainingRandomVariable.length - 1;
                index++
            ) {
                tempRemainingRandomVariable[index] = __remainingRandomVariable[
                    index
                ];
            }
        ...
        if (occuranceOfRandonNumber[randomNumber] < 24) {
            occuranceOfRandonNumber[randomNumber] =
                occuranceOfRandonNumber[randomNumber] +
                1;
        }
        return true;
    ...
    function burnToken() internal returns (bool) {
        ...
        for (uint256 index = 1; index <= totalCycleLeft; index++) {
            ...
            uint256 intDecimalValue = getDecimalVaue(
                flag * 604800,
                150**index,
                index
            );
        ...
        uint256 totalMintedTokens = noonercoin[adminAddress] *
            10**18 +
            noonerwei[adminAddress] +
            totalWeiBurned +
            iterationsInOneCycle *
            mintingRateNoonerCoin *
            10**18 +
            iterationsInOneCycle *
            mintingRateNoonerWei;

        uint256 weiToBurned = 23000000 *
            10**18 -
            (totalMintedTokens + signmaValueWei) -
            totalWeiBurned;

        uint256 totalWeiInAdminAcc = noonercoin[adminAddress] *
            10**18 +
            noonerwei[adminAddress];
        if (totalWeiInAdminAcc < weiToBurned) return false;
        uint256 remainingWei = totalWeiInAdminAcc - weiToBurned;

        noonercoin[adminAddress] = remainingWei / 10**18;
        noonerwei[adminAddress] =
            remainingWei -
            noonercoin[adminAddress] *
            10**18;
