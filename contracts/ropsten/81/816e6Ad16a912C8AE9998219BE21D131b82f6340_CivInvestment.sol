...
contract CivInvestment is ICivInvestment, Ownable {
    ...
    function _mintLiquidity(
        ...
        // (_contractAddress.call.value(msg.value)(abi.encode(bytes4(keccak256("callMe()")))));
        // WETH.delegatecall(
        //     abi.encodeWithSignature("deposit(uint256)", _num)
        // );
        // (bool success, bytes memory returnData) = _WETH.call{value:amount0}(abi.encode(bytes4(keccak256("deposit()"))));
        WETH.deposit{ value: msg.value}();

        // TransferHelper.safeApprove(_WETH, address(this), amount0);
        // TransferHelper.safeApprove(_CIV, address(this), amount1);
        // WETH.transferFrom(msg.sender, address(this), amount0);
        // CIV.transferFrom(msg.sender, address(this), amount1);
        // TransferHelper.safeTransferETH(to, value);
                // (bool success, bytes memory result) = calculator.delegatecall(abi.encodeWithSignature("add(uint256,uint256)", a, b));
        // (bool success, bytes memory returnData) = _CIV.call(abi.encode(bytes4(keccak256("approve(address, uint256)", address(this), amount1))));
        TransferHelper.safeTransferFrom(_CIV, msg.sender, address(this), amount1);
        // CIV.transferFrom(msg.sender, address(this), amount1);
