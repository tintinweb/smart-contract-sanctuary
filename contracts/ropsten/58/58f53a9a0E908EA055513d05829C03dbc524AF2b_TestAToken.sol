...
contract TestAToken is ERC20PresetFixedSupply, LGEWhitelisted {
    constructor()
    ...
    function createLGEWhitelist(
        ...
        if (durations.length > 0) {
            ...
            for (uint256 i = 0; i < durations.length; i++) {
                WhitelistRound storage wlRound = _lgeWhitelistRounds.push();
        ...
        )
    {
        if (_lgeTimestamp > 0) {
            ...
            for (uint256 i = 0; i < _lgeWhitelistRounds.length; i++) {
                ...
                wlCloseTimestampLast = wlCloseTimestampLast.add(
                    wlRound.duration
                );
        ...
        if (sender == _lgePairAddress && recipient != _lgePairAddress) {
            ...
            if (wlRoundNumber > 0) {
                ...
                if (wlRound.purchased[recipient] < wlRound.amountMax)
                    amountRemaining = wlRound.amountMax.sub(
                ...
                wlRound.purchased[recipient] = wlRound.purchased[recipient].add(
                    amount
                );
            }
        }
