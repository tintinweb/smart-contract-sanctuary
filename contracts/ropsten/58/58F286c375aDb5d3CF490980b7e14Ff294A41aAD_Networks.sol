...
contract Networks is AccessControl  
{
	...
	function NetworkViewSequential(uint num)public view returns(uint,bool,uint256,string memory,string memory)
	{
		uint id;
		id = network_ids[num];
		return (network_list[id].id, network_list[id].disabled, network_list[id].chain_id, network_list[id].name, network_list[id].url);
	}
	function NetworkListSequential(bool flag) public view returns(uint[] memory)
	{
		require(flag,'Insert 1 for Start');
		uint256[] memory out;
		uint id;
