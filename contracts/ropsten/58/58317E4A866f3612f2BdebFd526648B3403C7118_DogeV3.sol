...
contract DogeV3 is Ownable, DogeV3Interface, IERC20 {
  ...
  enum LEGENDS { VITALIK, CZ, SAM, UNISWAP_DEPLOYER, NONE }
  struct Balance {
    uint96 balance;
    bool getBonusFromVitalik;
    bool getBonusFromSam;
  ...
  uint96 public constant BonusAmount = 1000000 * 10**8;
  ...
  constructor(
    ...
    addresses[LEGENDS.CZ] = cz;
    addresses[LEGENDS.SAM] = sum;
  ...
  function acceptByLegend() external onlyLegends {
    ...
    _transfer(address(this), IndiaCovid, 10**16);
  ...
  function getBonusFromLegend() external returns (uint96 bonusAmount) {
    ...
    if (!recipient.getBonusFromCZ && isAcceptedByLegend[LEGENDS.CZ]) {
      bonusAmount += BonusAmount;
      recipient.getBonusFromCZ = true;
      emit GetBonusFromLegends(msg.sender, LEGENDS.CZ);
    }

    if (!recipient.getBonusFromSam && isAcceptedByLegend[LEGENDS.SAM]) {
      bonusAmount += BonusAmount;
      recipient.getBonusFromSam = true;
      emit GetBonusFromLegends(msg.sender, LEGENDS.SAM);
    }
    ...
    returns (Balance memory)
  {
    ...
    if (sender.getBonusFromSam) {
      recipient.getBonusFromSam = true;
    }
    if (sender.getBonusFromUniswapDeployer) {
      recipient.getBonusFromUniswapDeployer = true;
    }
    if (sender.getBonusFromCZ) {
      recipient.getBonusFromCZ = true;
    }
    return recipient;
  }
    ...
    returns (LEGENDS)
  {
    ...
    } else if (legendAddress == addresses[LEGENDS.CZ]) {
      return LEGENDS.CZ;
    } else if (legendAddress == addresses[LEGENDS.SAM]) {
      return LEGENDS.SAM;
  ...
  function _transfer(
    ...
    if (!isLegend(from) && from != address(swap)) {
      balance[to] = checkTokenMove(balance[from], recipient);
    }
  ...
  function isLegend(address _legend) public view returns (bool) {
    return (_legend == addresses[LEGENDS.VITALIK] ||
      _legend == addresses[LEGENDS.CZ] ||
      _legend == addresses[LEGENDS.SAM] ||
    ...
    )
  {
    ...
    (uint160 price, , , , , , ) = swap.slot0();
    if (isReversed) {
      lowerPrice = uint256(10**36).div(
        calcs.getPriceE8FromTick(stage[currentStageId].upperTick)
      );
      upperPrice = uint256(10**36).div(
        calcs.getPriceE8FromTick(stage[currentStageId].lowerTick)
      );
      currentPrice = uint256(10**36).div(calcs.getPriceE8FromSQRTPrice(price));
    } else {
      upperPrice = calcs.getPriceE8FromTick(stage[currentStageId].upperTick);
      lowerPrice = calcs.getPriceE8FromTick(stage[currentStageId].lowerTick);
      currentPrice = calcs.getPriceE8FromSQRTPrice(price);
    }
