...
interface IDarenOrder {
    ...
    function getOrderByPK(uint _pk) external view returns (uint orderID);
...
contract DarenBonus {
    ...
    uint public totalTransactionAmount;
    uint public allocRatioBase;
    uint public allocRatio;
    ...
    function completeOrder(address _buyer, address _seller, uint _value, uint _fee) external {
        require(darenOrders[msg.sender], "Invalid order address.");

        uint bonusAmount = _fee.mul(allocRatioBase).div(allocRatio);
        userBonus[_buyer] = userBonus[_buyer].add(bonusAmount);
        userBonus[_seller] = userBonus[_seller].add(bonusAmount);
    ...
    function getCurrentReward() public view returns (uint) {
        uint bonus = userBonus[msg.sender];
