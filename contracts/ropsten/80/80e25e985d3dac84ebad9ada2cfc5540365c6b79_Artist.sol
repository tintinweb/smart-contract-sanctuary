...
contract ArtistBase is Ownable,ERC721Full,ReentrancyGuard {
    ...
    whenNotPaused
     ...
     function getRoundPrice(uint256 round) public
     returns (uint256)
     {
         if(round>priceList.length){
             uint256 lastValue=priceList[priceList.length-1];
             for(uint256 i=priceList.length;i<round;i++){
                 lastValue=lastValue.mul(11).div(10);
                 priceList.push(lastValue);
             }
             return lastValue;
         }
         return priceList[round-1];
     }     
    ...
    function _bid(address inviterAddress, uint256 artid) nonReentrant internal
     {
        ...
        curprice=getRoundPrice(r);
        uint256 payprice=getRoundPrice(r+1);
        require(msg.value>payprice, "ERR_NOT_ENOUGH_MONEY");
