...
contract SubGameSwap is Ownable {
    ...
    event NewSwap(address from, uint256 fromCoinType, string to, uint256 toCoinType, uint256 amount);
    event Send(address to, uint256 toCoinType, string from, uint256 fromCoinType, uint256 amount);
    ...
    function swap(string calldata to, uint256 toCoinType, uint256 fromCoinType, uint256 amount) external {
        ...
        emit NewSwap(msg.sender, fromCoinType, to, toCoinType, amount);
    }

    function send(address to, uint256 toCoinType, string calldata from, uint256 fromCoinType, uint256 amount) external onlyOwner {
        require(amount >= 10, "amount must greater or equal to 10");
        require(tokens[toCoinType].coinType == toCoinType, "token not allowed");
        tokens[toCoinType].tokenAddress.safeTransfer(to, amount);
        emit Send(to, toCoinType, from, fromCoinType, amount);
    }

    function getToken(uint256 coinType) public view returns (Tokens memory) {
