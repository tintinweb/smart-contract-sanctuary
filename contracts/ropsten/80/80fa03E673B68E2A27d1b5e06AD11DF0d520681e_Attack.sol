...
contract Attack is IAttack, Ownable, Pausable {
    ...
    constructor() {
        ...
        settings.hiveProtectionBear = 3 * 60; //*60 // per success
    ...
    function _resolveCollection(uint256 hiveId) private {
        ...
        if (err == 0) {
            ...
            owed = _collect(t.strength, hiveId, seed, a.howMuch);

            if (owed > 0) {
                hiveContract.setKeeperAttackData(hiveId, uint32(block.timestamp), uint32(owed));
                address _owner;
                if (beesContract.getOwnerOf(a.tokenId) != address(hiveContract)) _owner = beesContract.getOwnerOf(a.tokenId);
                else _owner = hiveContract.getWaitingRoomOwner(a.tokenId);
                beesContract.increaseTokensPot(_owner, owed);
     ...
     * @param blockNumber a value ensure different outcomes for different sources in the same block
