...
abstract contract WrappedERC20 is ERC20 {

    ERC20 public immutable underlyingToken;

    constructor(ERC20 _underlyingToken) ERC20(string(abi.encodePacked("Wrapped ", _underlyingToken.name())), string(abi.encodePacked("w", _underlyingToken.symbol()))) {
    ...
    function deposit(address _recipient, uint256 _amount) public virtual {
        bool success = underlyingToken.transferFrom(msg.sender, address(this), _amount);
        require(success, "Transfer from failed");
        _mint(_recipient, _amount);
    }

    function withdraw(address _recipient, uint256 _amount) public virtual {
        _burn(msg.sender, _amount);
        bool success = underlyingToken.transfer(_recipient, _amount);
        require(success, "Transfer failed");
    }
}
