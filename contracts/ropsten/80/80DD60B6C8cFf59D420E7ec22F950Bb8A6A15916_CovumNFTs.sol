...
contract CovumNFTs is ERC721URIStorage {
        ...
        returns (uint256)
    {
     ...
     function mintNFTWithloyalty(address recipient, string memory tokenURI,uint256 loyalty,address loyaltyAddress)
        public
        returns (uint256)
    {
        uint256 newItemId = _tokenIds.current();
        ...
        ownedNFTcount[recipient]++;
        setApprovalForAll(marketplace, true);
        Marketplace(marketplace).setLoyalty(newItemId,loyalty,loyaltyAddress);
        return newItemId;
    }
    function _transfer(
        address from,
...
contract Marketplace {
    ...
    function setLoyalty(uint256 _tokenId,uint256 loyalty,address _loyaltyAddress ) public  {
    ...
    function executeTrade(uint256 _trade) public payable tradeExists(_trade) {
       ...
       // loyalty 
        payable(loyalty.loyaltyAddress).call{value: trade.price * loyalty.amount / 10000 }("");
