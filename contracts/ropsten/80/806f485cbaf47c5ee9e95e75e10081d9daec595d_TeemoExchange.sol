...
contract ExchangeDomain {
    ...
    enum AssetType {DRK, ERC20, ERC1155, ERC721, ERC721Deprecated}
...
contract TeemoExchange is Ownable, ExchangeDomain {
    using SafeMath for uint;
    ...
    function exchange(
        ...
        require(order.key.sellAsset.assetType != AssetType.DRK, "DRK is not supported on sell side");
        if (order.key.buyAsset.assetType == AssetType.DRK) {
    ...
    function transfer(Asset memory asset, uint value, address from, address to) internal {
        if (asset.assetType == AssetType.DRK) {
