...
contract StakeCOC is Owned, ReentrancyGuard {
    ...
    function STAKE(uint256 tokens) external nonReentrant returns(bool){
        ...
        if (stakers[msg.sender].stakedAmount > 0) {
            ...
            if(rewardToGive>0)
            {
                CLAIMREWARDCONTRACT(msg.sender);       
            }
 ...
 function MyTotalRewards(address user) external view returns(uint256 poolreward)
  {
     ...
     function CLAIMREWARDCONTRACT(address user) private  {

            uint256 timeToday = block.timestamp - creationTimeContract; //what day it is
            uint256 timeT = timeToday.div(86400);
            require(stakers[user].stakedAmount > 0,"you need to stake some coins");
          
            uint256 rewardToGive = calculateReward(timeT,user);
            require(MOJITO(rewardToken).transfer(user,rewardToGive), "ERROR: error in sending reward from contract");
            emit CLAIMEDREWARD(user, rewardToGive);
            stakers[user].TotalMRewarded = (stakers[user].TotalMRewarded).add(rewardToGive);
