...
contract MasterChef is Ownable, ReentrancyGuard, IMasterChef, Constants {
    ...
    function pendingLotl(uint8 _pid, address _user) external view returns (uint256) {
        ...
        if (currentBlock > pool.lastRewardBlock && lpSupply != 0) {
            uint128 lotlReward =  lotlPerBlock * (currentBlock - pool.lastRewardBlock) * pool.allocPoint  / totalAllocPoint;
        ...
        for (uint8 i=0; i < length; i++){
            ...
            for(uint32 j; j< userLength; j++){
                ...
                if(user.stakedSince > 0){
                    ...
                    rewardUser.rewardPoolShare = rewardUser.rewardPoolShare + uint64(user.amount * 1e12 / lpSupply * pool.allocPoint / totalAllocPoint * timeReward / poolTimeReward);
