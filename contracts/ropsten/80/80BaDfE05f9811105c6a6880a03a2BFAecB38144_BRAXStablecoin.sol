...
// File: contracts\Incentive\IIncentive.sol
// License: MIT
...
// File: contracts\BRAX\Pools\IBraxPool.sol
// License: MIT
...
contract BRAXStablecoin is ERC20Custom, AccessControl {
    using SafeMath for uint256;
    /* ========== STATE VARIABLES ========== */
    enum PriceChoice { BRAX, BXS }
    IUniswapPairOracle public BraxUsdOracle;
    IUniswapPairOracle public BraxBxsOracle;
    ...
    IIncentive public brax_incentive; 
    ...
    uint256 public price_target; // The price of BRAX at which the collateral ratio will respond to; this value is only used for the collateral ratio mechanism and not for minting and redeeming which are hardcoded at $1
    ...
    constructor(
        ...
        brax_step = 2500; // 6 decimals of precision, equal to 0.25%
        global_collateral_ratio = 1000000; // BRAX system starts off fully collateralized (6 decimals of precision)
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal override {
        ...
        if (address(brax_incentive) != address(0)) {
            brax_incentive.incentivize(sender, recipient, msg.sender, amount);
        }
    }
    /* ========== VIEWS ========== */
    // Returns X BRAX = 1 USD
    function brax_price() public view returns (uint256) {
        address usd_address = BraxUsdOracle.getPairToken(address(this));
        return uint256(BraxUsdOracle.consult(usd_address, PRICE_PRECISION));
    }
    // Returns X BXS = 1 Brax
    function bxs_price_in_brax()  public view returns (uint256) {
        return  uint256(BraxBxsOracle.consult(address(this), PRICE_PRECISION));
    }
    // Returns X BXS = 1 USD
    ...
    // Iterate through all BRAX pools and calculate all value of collateral in all pools globally 
    function globalCollateralValue() public view returns (uint256) {
        uint256 total_collateral_value_d18 = 0; 
        for (uint i = 0; i < brax_pools_array.length; i++){ 
            // Exclude null addresses
            if (brax_pools_array[i] != address(0)){
                total_collateral_value_d18 = total_collateral_value_d18.add(IBraxPool(brax_pools_array[i]).collatDollarBalance());
            }
        }
    ...
    function refreshCollateralRatio() public {
        ...
        if (brax_price_cur > price_target.add(price_band)) { //decrease collateral ratio
            ...
            if(global_collateral_ratio.add(brax_step) >= 1000000){
    ...
    function swapCollateralAmount() external view returns(uint256){
        address usd_address = BraxUsdOracle.getPairToken(address(this));
        return BraxUsdOracle.getSwapTokenReserve(usd_address);
    }
    ...
    // This function is what other BRAX pools will call to mint new BRAX 
    ...
    function removePool(address pool_address) public onlyByOwnerOrGovernance {
        require(brax_pools[pool_address] == true, "address doesn't exist already");
    ...
    function setBraxIncentive(address _brax_incentive) external onlyByOwnerOrGovernance {
        brax_incentive = IIncentive(_brax_incentive);
    } 
    function setBRAXStep(uint256 _new_step) public onlyByOwnerOrGovernance {
        brax_step = _new_step;
    }  
    ...
    // Sets the brax_usd(Dai) Uniswap oracle address 
    function setBraxUsdOracle(address _oracle) public onlyByOwnerOrGovernance {
        BraxUsdOracle = IUniswapPairOracle(_oracle);  
        require(BraxUsdOracle.containsToken(address(this)),"!brax");
    }
    // Sets the brax_bxs Uniswap oracle address 
    function setBraxBxsOracle(address _oracle) public onlyByOwnerOrGovernance {
        BraxBxsOracle = IUniswapPairOracle(_oracle);
        require(BraxBxsOracle.containsToken(address(this)),"!brax");
    }
