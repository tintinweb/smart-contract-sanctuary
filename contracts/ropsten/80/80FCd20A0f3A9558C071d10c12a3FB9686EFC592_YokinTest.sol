...
contract ERC20 is Context, IHRC20, Ownable {
    ...
    // event startHolding(address account);
    // event endHolding(address account);
    ...
    IREWARD public RewardToken = IREWARD(address(0x0D257631574a015A69c1baf15dfc18a00348c373)); 
    ...
    uint16 interestRatio = 25;  // 0.25%
    uint16 bonusRatio = 1000;   // 10%
    address public devWallet = 0x5c35Ae532E9276aec5F8657aeC477585e411802a;
    ...
    */
    ...
    function _transfer (address sender, address recipient, uint256 amount) internal {
        ...
        if(bLocked == false) {
        //     if(_isExcludedFromFee[sender] == false && _isExcludedFromFee[recipient] == false) {
        //         taxFeeAmount = amount.mul(taxFee).div(1000);
        //         receiveAmount = amount.sub(taxFeeAmount);
        //         _balances[devWallet] = _balances[devWallet].add(taxFeeAmount);
        //         emit Transfer(sender, devWallet, taxFeeAmount);
        //     }
        //     else {
    ...
    function updateInterestRatio(uint16 _interestRatio) public onlyOwner {
        require(_interestRatio > 0, "Interest Ratio can't be less than zero");
        require(_interestRatio < 10000, "Interest Ratio can't exceed 100%");
    ...
    function startHolding(address account) internal {
        ...
        LockedDatas[account].balance = balanceOf(account);
    ...
    function calcInterestRewardAmount(uint256 balance, uint256 lockedDays) public view returns(uint256 amount) {
        require(balance > 0, "Balance can't be less than zero");
        require(lockedDays > 0, "Locked day can't be less than zero");

        uint256 interestAmount = balance.mul(interestRatio).mul(lockedDays).div(10000);
...
// YokinToken
contract YokinTest is ERC20('YokinTest_v2', 'YK_v2', 8), MinterRole {
