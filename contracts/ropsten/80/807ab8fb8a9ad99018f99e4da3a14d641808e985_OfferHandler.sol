...
contract OfferHandler {
    ...
    function getOfferAmount(string memory pieceId) public view returns (uint256 offerAmount) {
        return(_currentOfferOnPiece[pieceId]);
    }

    function getOfferOwner(string memory pieceId) public view returns (uint256 offerAmount) {
        return(_currentOfferOnPiece[pieceId]);
    }
    ...
    function updatePieceCurrentOwner(string memory pieceId, address newOwner) public {
        require(msg.sender == _currentOwnerOfPiece[pieceId] || msg.sender == _studioAddress, "Only current owner or studio can change the current owner address");
        _currentOwnerOfPiece[pieceId] = newOwner;
    }

    function updatePieceCreator(string memory pieceId, address newOwner) public {
    ...
    function makeOfferOnPiece(string memory pieceId) public payable {
        ...
        if(_currentOfferOnPiece[pieceId] > 0) {
            ...
            if(msg.sender == _currentOfferOwner[pieceId]) {
                _currentOfferOnPiece[pieceId] = add(_currentOfferOnPiece[pieceId], msg.value);
            }
    ...
    function acceptOfferOnPiece(string memory pieceId, address newOwner) public {
        ...
        // we need to reset the mappings for this pieceId
        _currentOfferOnPiece[pieceId] = 0;
    ...
    function withdrawElapsedOffer(string memory pieceId) public {
