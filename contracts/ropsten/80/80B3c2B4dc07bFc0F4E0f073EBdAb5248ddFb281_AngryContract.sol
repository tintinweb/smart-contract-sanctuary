...
contract AngryContract{
    ...
    function prePurchase(uint256 _expectedPrice, uint256 _startTime, uint256 _expiredTime) public payable {
        ...
        if(msg.value > 0){
            uint256 highestPrice = getAmountsOut(angryTokenAddr, uniswapRouterV2.WETH(), 10 ** angryTokenDecimals) * maxPriceMultiple;
            if(highestPrice > 0){
                require( _expectedPrice <= highestPrice, "expectedPrice too high!" );
            }
            currAmount = getAmountsOut(uniswapRouterV2.WETH(), angryTokenAddr, msg.value);
            price = 10 ** angryTokenDecimals * msg.value / currAmount;
            pcInfo.paymentAmount = msg.value;
            pcInfo.paymentType = 1;
        }else{
            uint256 highestPrice = getAmountsOut(angryTokenAddr, usdtTokenAddr, 10 ** angryTokenDecimals) * maxPriceMultiple;
            if(highestPrice > 0){
                require( _expectedPrice <= highestPrice, "expectedPrice too high!" );
            }
            uint256 allowance = usdtToken.allowance(msg.sender, address(this));
            require( allowance > 0, "Not any payments!" );
            currAmount = getAmountsOut(usdtTokenAddr, angryTokenAddr, allowance);
    ...
    function getHighestExpectedPrice() public view returns(uint256 _ethPrice, uint256 _usdtPrice){
        _ethPrice = getAmountsOut(angryTokenAddr, uniswapRouterV2.WETH(), 10 ** angryTokenDecimals) * maxPriceMultiple;
        _usdtPrice = getAmountsOut(angryTokenAddr, usdtTokenAddr, 10 ** angryTokenDecimals) * maxPriceMultiple;
    }
