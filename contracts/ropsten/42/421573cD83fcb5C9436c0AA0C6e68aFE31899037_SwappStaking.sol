...
contract SwappStaking is ReentrancyGuard {
    ...
    address constant private swapp = 0xc52FD807131e8cb3bc5FD278db6a18768aebf233;
	address constant private minter = 0xb739F63006c83d39e20d3b3B9c5907e020F9a1aE;
	address public farm;
    ...
    mapping(address => mapping(uint256 => Pool)) public poolSize; //TODO make private
	...
	uint256 constant public NR_OF_EPOCHS = 10;
    ...
    constructor () {
        epoch1Start = block.timestamp + 3600;
        epochDuration = 1800; // 28 days
     ...
     */
    function deposit(address tokenAddress, uint256 amount, address referrer, uint128 endEpoch) public nonReentrant {
        ...
        pNextEpoch.size = stakedSwapp; //IERC20(tokenAddress).balanceOf(address(this));
        pNextEpoch.set = true;
    ...
    function calcDurationBonusMultiplier(uint128 epochId, address staker) external view returns (uint256) {
        ...
        if (topupData.totalTopups > 0) {
            ...
            for (uint256 i = 1; i <= topupData.totalTopups; i++) {
                ...
                // multiplier += epochId.sub(startEpoch).mul(CALC_MULTIPLIER).div(epochId);
                // usedTopups += 1;
                dividend += epochId.sub(startEpoch).mul(CALC_MULTIPLIER).div(epochId) * amount;
                divider += amount;
                // uint256 currentMultiplierWeight = currentMultiplier * amount;
                
            }
            // if (usedTopups > 0) {
            //     return multiplier.div(usedTopups);
            // }
            if (divider > 0) {
    ...
    function allowEmergencyWithdrow() external onlyOwner{
    ...
    function withdraw(address tokenAddress, uint256 amount) public nonReentrant {
        ...
        if (balances[msg.sender][tokenAddress] == 0) {
            ...
            Stake storage stake = stakes[msg.sender];
            if (currentEpoch < stake.endEpoch) {
                uint256 balanceBefore = balances[msg.sender][tokenAddress] + amount;
                uint256 reduceMultiplier = balances[msg.sender][tokenAddress] * CALC_MULTIPLIER / balanceBefore;
    		    SwappYieldFarm(farm).reduceDurationBonus(msg.sender, reduceMultiplier);
    		}
        }
        
        uint256 penalty = calcPenalty(amount);
        uint256 amountToMint = amount - penalty;
        stakedSwapp = stakedSwapp - amount;
