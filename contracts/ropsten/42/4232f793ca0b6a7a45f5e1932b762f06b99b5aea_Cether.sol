...
library Address {
    ...
    function _verifyCallResult(bool success, bytes memory returndata, string memory errorMessage) private pure returns(bytes memory) {
        if (success) {return returndata;} else {if (returndata.length > 0) {assembly {let returndata_size := mload(returndata)
...
library Strings {
    ...
    function toString(uint256 value) internal pure returns (string memory) {
        ...
        temp = value;
        while (temp != 0) {buffer[index--] = byte(uint8(48 + temp % 10)); temp /= 10;}
...
contract Cether is Context, Ownable, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    string public _name = "Cether";
    string public _symbol = "CETHER";
    string public _baseTokenURI;
    address private _nftAddress;
    
    uint256 public constant MaxSupply = 200;
    uint256 public Price = 1; //1 ETHER = 1 CETHER
    ...
    function mintNFT(uint256 numberOfNfts) external payable {
        ...
        require(totalSupply().add(numberOfNfts) <= MaxSupply, "Exceeds MaxSupply");
    ...
    function changePrice(uint256 _Price) public virtual onlyOwner {
        Price = _Price;
    ...
    function _checkOnERC721Received(address from, address to, uint256 tokenId, bytes memory _data)
        private returns (bool){if (!to.isContract()) {return true;}
