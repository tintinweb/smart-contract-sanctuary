...
library PrivateSaleKeeper {
    ...
    function deleteUser(PrivateData storage self, address key) internal returns (bool success) {
        if(contains(self, key) == false) {
            success = false; 
        } else {
            delete self.privateAddresses[key];
        }
    }

    function replace(PrivateData storage self, address key, address newKey) internal returns (bool success) {

        require(contains(self, key) ==  true, "The owner address does not exist in the private sale records");
        
        if(contains(self, key) == false) {
            success = false; 
        } else {
            uint256[3] memory records = self.privateAddresses[key];
            self.privateAddresses[newKey] = records;
            deleteUser(self, key);
...
contract ERC20 is IERC20, Ownable {
    ...
    function checkContains(address _account) public view returns (bool) {
        return salesKeeper.contains(_account);
    }
    
    // Amount of tokens bought during private sale
    function getAmounts(address _to) public view returns (uint256[3] memory) {
        require(_to == _msgSender() || isOwner(), "You dont have right to use this function");
        return salesKeeper.get(_to);
    }

    function getPrivateSaleAmount(address _to, uint _phase) public view returns (uint256) {
        require(_to == _msgSender(), "You dont have right to use this function");
        require(_phase > 0 && _phase < 4, "Invalid phase number (1 - 3)");
        return getAmounts(_to)[_phase-1];
    }
    ...
    function internalFundedAmount(address _account) public view returns (uint) {
        require(_account == _msgSender() || isOwner(), "You dont have right to use this function");
        return internalRecordKeeper.getFund(_account);
    }

    function internalReleaseDate(address _account) public view returns(uint16, uint8, uint8) {
        require(_account == _msgSender() || isOwner(), "You dont have right to use this function");

        uint timeStamp = startDay + (internalRecordKeeper.getLockup(_account) * 1 days);
        return getDate(timeStamp);
    }

    // To be called by address owners
    function changePrivateAddress(address _old, address _new) public returns (bool response) {

        // Sender to owner to transer

        require(_msgSender() == _old, "You do not have the priviledge to call this function");

        response = salesKeeper.replace(_old, _new);

        require(response == true, "owner address does not exit");

        if(response ==  true) {
            uint256 _amount = balanceOf(_old);

            address _spender = getOwner();

            // check if the allowance given to owner by the sender is >= amount
            if(allowance(_old, _spender) < _amount) {
                approve(_spender, _amount);
                // transferFrom(_old, _new, _amount);
                _transfer(_old, _new, _amount);
            }
            emit SwitchAddress(_old, _new, now);
        } else {
            response = false;
        }
    }

    function releaseUser(address _user) public returns (bool) {
        return salesKeeper.deleteUser(_user);
    }

    // Show all the tokens that a user can transfer at that moment
    function availableForTransfer(address _owner) public view returns (uint256) {
        require(_owner == _msgSender() || _msgSender() == address(this), "You don't have right to use this function");

        uint256 presaleAmountNotAvailable = 0;
        uint256[3] memory records = getAmounts(_owner);
        for (uint i=1; i<records.length+1; i++) {
            if (now < getReleaseTimeStamp(i)) {
                presaleAmountNotAvailable += getPrivateSaleAmount(_owner, i);
            }
        }

        // check internal fund amount
        if( now < startDay + (internalRecordKeeper.getLockup(_owner) * 1 days)) {
            presaleAmountNotAvailable += internalFundedAmount(_owner);
        }

        uint256 publicSaleToken = balanceOf(_owner) - presaleAmountNotAvailable;
    ...
    event SwitchAddress(address _old, address _new, uint256 time);
}
...
contract XPLLToken is ERC20Mintable, ERC20Detailed, ERC20Pausable {
    ...
    constructor (string memory _tokenName, string memory _tokenSymbol, uint8 _decimal, uint _initialSupply) public ERC20Detailed(_tokenName, _tokenSymbol, _decimal) {

        _privateSaleStatus = 1;

        uint256 tokenAmount = _initialSupply * (10 ** uint256(decimals()));
        _mint(_msgSender(), tokenAmount);
    }

    function privateTransfer(address _recipient, uint256 _amount, uint _phase) public onlyOwner {
        ...
        require(_recipient != _msgSender(), "Private sale recipient is owner");
        
        SavePrivateSaleRecord(_recipient, _amount, _phase);

        transfer(_recipient, _amount);

        emit PrivateSale(_recipient, _amount, _phase, now);
    }

    function internalTransfer(address _recipient, uint256 _amount, uint lockupdays) public onlyOwner {

        require(_recipient != _msgSender(), "Private sale recipient is owner");
        
        SaveInternalRecord(_recipient, _amount, lockupdays);

        transfer(_recipient, _amount);

        emit InternalFund(_recipient, _amount, now);
    }
    ...
    event PrivateSale(address _recipient, uint256 _amount, uint _phase, uint time);
    event InternalFund(address _recipient, uint256 _amount, uint time);
}