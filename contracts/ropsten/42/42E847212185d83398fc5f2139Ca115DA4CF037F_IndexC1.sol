...
contract IndexC1 is Ownable {
  ...
  function exitMarket() public {
    ...
    for (uint i = 0; i < allocationBalances[msg.sender].length; i++) {
      ...
      uint amountInO = allocationBalances[msg.sender][i].amountIn;
      uint amountInE = amountInO.div(ETHER_BASE);
      
      IERC20 token = IERC20(tokenAddress);
      token.approve(UNISWAP_ROUTER_ADDRESS, amountInE);
      // token.approve(address(this), allocationBalances[msg.sender][i].amountIn);
      // token.approve(UNISWAP_ROUTER_ADDRESS, allocationBalances[msg.sender][i].amountIn);
      ...
      emit SwapInit(tokenAddress, amountInO, amountInE);
