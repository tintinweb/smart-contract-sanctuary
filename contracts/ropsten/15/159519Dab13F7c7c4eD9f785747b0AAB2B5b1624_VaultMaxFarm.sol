...
interface IStrategy {
    ...
    // Transfer want tokens VaultMaxFarm -> strategy
    function deposit(address _userAddress, uint256 _wantAmt)
        external
        returns (uint256);

    // Transfer want tokens strategy -> VaultMaxFarm
...
contract VaultMaxFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address public VMAX = 0xca91c053FBb83B7FCc990591d1EA8AF3EF6C719e;

    address public rewardPoolAddress = 0x9fe63E9E21161bdb97C24FD620aF678aFb3F5789;

    uint256 public ownerVMAXReward = 115; // 10%

    uint256 public VMAXMaxSupply = 1e25;
    uint256 public VMAXPerBlock = 1000000000000000000000; // VMAX tokens created per block
    uint256 public startBlock = 9892020; //https://bscscan.com/block/countdown/3888888
    ...
    function updatePool(uint256 _pid) public {
        ...
        VMAXToken(VMAX).mint(
            owner(),
            VMAXReward.mul(ownerVMAXReward).div(1000)
        );
    ...
    // Want tokens moved from user -> VaultMaxFarm (VMAX allocation) -> Strat (compounding)
    ...
    function removeFromBlacklist(address toRemoveFromBlacklist) public onlyOwner {
      blackList[toRemoveFromBlacklist]=false;
    }
