...
contract BlockchainRegistry is IRegistry, AccessControl {
    ...
    mapping(address => uint) private registerIndex;
    ...
    // uint256 public constant REGISTRY_REVISION = 0x5;
    ...
    **/
    ...
    // The smart contract is upgradeable with above function serving as the constructor
    /* constructor(address _acceptedToken) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(OWNER_ROLE, msg.sender);
        acceptedToken = IERC20(_acceptedToken);
    ...
    function updateBlackList(address account, bool blocked) external onlyOwners {
        ...
        override
    {
        ...
        if ((r.user != address(0)) && (r.approved)) {
            ...
            // satellite wallets will be duplicated otherwise because their
        ...
        if (ACCOUNT_TYPE(accountType) == ACCOUNT_TYPE.SATTELITE) {
            r.pending = true;
        }
    ...
    function getAllAccounts() external view onlyOwners returns (REGISTRATION[] memory) {
    ...
    function newServiceRequest(
        ...
        require(!blacklist[_msgSender()], "cannot create request with blacklisted address");
        ...
        external
    {
        ...
        if (!approve) {
            ...
            registry[_msgSender()].pending = false;
        ...
        registerArray[registerIndex[_msgSender()]] = r;
    }
    ...
    function updateUserRegistration(address clientOrSatellite, bool approve)
        ...
        onlyOwners
    {
        REGISTRATION storage r = registry[clientOrSatellite];
        if (!approve) {
            ...
            if (r.acc_type == ACCOUNT_TYPE.CLIENT) {
                r.pending = false;
                r.approved = approve;
                r.approvedBy[0] = _msgSender();
            } else {
        ...
        registerArray[registerIndex[clientOrSatellite]] = r;
