...
/*
* @author Hamdi Allam <a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="660e070b020f48070a0a070b5f5126010b070f0a4805090b">[email&#160;protected]</a>
...
contract BlockHash {
    ...
    function addBlocks(
        ...
        public
    {
        require(starts.length > 0 && starts[0] == 0 && starts[starts.length - 1] == blocksData.length, "Wrong starts argument");
        ...
        bytes32 result;
        assembly {
            ptr := add(blocksData, 0x20)
            offsetPtr := add(starts, 0x20)
        }
        while (offset != blocksData.length) {
            assembly {
                offsetPtr := add(offsetPtr, 0x20)
                nextOffset := mload(offsetPtr)
                length := sub(nextOffset, offset)
