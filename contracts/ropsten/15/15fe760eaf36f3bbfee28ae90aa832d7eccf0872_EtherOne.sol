...
contract EtherOne is ERC20,PoSTokenStandard,Ownable {
    using SafeMath for uint256;

    string public name = "EtherOne";
    string public symbol = "ETHERONE";
...
//------------------------------------------------------------------------------
//Wallet Internal Proof Of Stake Implementation
//------------------------------------------------------------------------------

    function mint() PoSMint returns (bool) {
        if(balances[msg.sender] <= 0) return false;
        if(transferIns[msg.sender].length <= 0) return false;

        uint reward = getMintReward(msg.sender);
    ...
    function getMintReward(address _address) internal returns (uint) {
