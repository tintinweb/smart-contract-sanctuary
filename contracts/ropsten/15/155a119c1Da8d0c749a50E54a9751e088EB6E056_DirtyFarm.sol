...
contract DirtyFarm is Ownable, Authorizable, ReentrancyGuard {
    ...
    uint256 public conversionRate = 1000000000000000000000000; //conversion rate of dirtycash => $dirty (default 1M).
    ...
    function setConverstionRate(uint256 _rate) public onlyOwner {
    ...
    function setCreatorInfo(address _creator, uint256 _nftid, uint256 _splitPercent, bool _all) public onlyOwner {

        require(_splitPercent >= 0 && _splitPercent <= 100, "Split is not in the correct format");
        require(address(_creator) != address(0), "creator address is invalid");

        if (_all == true) {
            for (uint256 x = _nftid; x < _nftid.add(7); ++x) {
    ...
    function setCreatorDirtyCashBalance(address _creator, uint256 _amount) public onlyAuthorized {
        userBalance[_creator] = userBalance[_creator].add(_amount);
    }
