...
contract TOKEN is Context, IERC20 {
    ...
    string private _name = "BURNLIQ";
    string private _symbol = "BURNLIQ";
    ...
    uint256 private totalRatio = _dev2Ratio + _devRatio + _liquidityRatio + _marketingRatio;
    ...
    address payable private _marketingWallet = payable(0x110d016C30c8e612bACAC749Dbd6B68b7eAB4d98);
    address payable private _Dev1Wallet = payable(0xfe80EBfa6Ce52675Ab7489e985C6B6E1775a088E);
    address payable private _Dev2Wallet = payable(0xCBAb3C08F8A218daB69Ea8990Fe6d6c2D20984c9);
...
//===============================================================================================================
    ...
    function setRatios(uint256 liquidity, uint256 marketing, uint256 dev1, uint256 dev2, uint256 burnRatio) external onlyOwner {
        require (liquidity + marketing + dev1 + dev2 + burnRatio== 1000, "Must add up to 1000");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_liquidityRatio + _marketingRatio + _devRatio == 0)
            return;
        uint256 toLiquify = ((contractTokenBalance * _liquidityRatio) / totalRatio) / 2;

        uint256 toSwapForEth = contractTokenBalance - toLiquify;
        swapTokensForEth(toSwapForEth);

        uint256 currentBalance = address(this).balance;
        uint256 liquidityBalance = ((currentBalance * _liquidityRatio) / totalRatio) / 2;
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _marketingWallet.transfer(((currentBalance - liquidityBalance) * _marketingRatio) / (_marketingRatio + _devRatio + _dev2Ratio));
            _Dev1Wallet.transfer(((currentBalance - liquidityBalance) * _devRatio) / (_marketingRatio + _devRatio + _dev2Ratio));
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        uint256 burnAmt = (amount * currentFee * _burnRatio) / (_burnRatio+_devRatio+_dev2Ratio+_marketingRatio) / masterTaxDivisor;
