...
contract WeirdElves is ERC721Enumerable, Ownable, CybermonValues {
    ...
    function freeMint(uint256 numberOfTokens) public payable {
        ...
        require((totalSupply + numberOfTokens) <= MAX_SUPPLY, "Purchase exceeding max token supply");
        
        uint256[] memory mintableTokens = getMintableTokens(msg.sender);
        require(mintableTokens.length >= numberOfTokens, "You dont own enough Cybereggs");
    ...
    function presaleMint(uint256 numberOfTokens) public payable {
        ...
        require(isWhitelisted(msg.sender), "You are not able to mint");
        require(numberOfTokens <= MAX_NFT_PRESALE, "Can only mint a max of 5 tokens");
        require(NFT_PRICE_WHITELIST * numberOfTokens == msg.value, "Sent ether value is incorrect");
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        require(NFT_PRICE_PUBLIC * numberOfTokens == msg.value, "Sent ether value is incorrect");
    ...
    function getMintableTokens(address sender) private returns(uint256[] memory) {
        Cybermon c = Cybermon(cybermonContract);
        uint256[] memory mintableTokens;
    ...
    function isWhitelisted(address sender) public view returns (bool) {
        if(_whiteList[sender] || (_presaleList[sender] && presalePhase))
            return true;
    ...
    function _baseURI() internal view virtual override returns (string memory) {
     ...
     function setCybermonContract(address contractAddress_) public onlyOwner {
        cybermonContract = contractAddress_;
    }
