...
contract DirtyFarm is Ownable, Authorizable, ReentrancyGuard {
    ...
    function updateVariablePoolReward() private {
        ...
        uint256 ratio;
        uint256 multiplier;
        uint256 rewardSegment = poolReward.mul(1).div(2);

        if (runningTotal0 >= maxLPStake) {
            lpratio = runningTotal0.div(maxLPStake);
        } else {
            lpratio = maxLPStake.mul(1).div(maxLPStake);
        }

        if (runningTotal1 >= maxDirtyStake) {
             stakelpratio = runningTotal1.div(maxDirtyStake); 
             stakelpratio = stakelpratio.div(maxLPStake);
        } else {
            stakelpratio = maxDirtyStake.mul(1).div(maxDirtyStake);
        }   

        multiplier = lpratio.add(stakelpratio);
        
        poolReward = rewardSegment.mul(multiplier);
        ...
        if (stakelpratio > lpratio) {
            ratio = stakelpratio.div(lpratio);
        ...
        if (stakelpratio < lpratio) {
            ratio = lpratio.div(stakelpratio);
