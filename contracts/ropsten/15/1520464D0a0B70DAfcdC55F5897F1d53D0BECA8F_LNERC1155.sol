...
contract LNERC1155 is ERC1155, Ownable, Pausable {
    ...
    */
    ...
    function create(address _initialOwner, uint _initialSupply, bool _cloneable, bytes calldata _metadata) external onlyOwner returns (uint) {
        ...
        Patents[newItemId] = Patent(newItemId, _initialOwner, _initialSupply, uri(newItemId), _cloneable, _metadata);
        return newItemId;
    }

    function clone(address _otherOwner, uint _tokenId, uint _supply) external creatorOnly(_tokenId) returns (uint) {
        Patent storage item = Patents[_tokenId];
    ...
    function getItem(uint _tokenId) public view returns (Patent memory) {
        return Patents[_tokenId];
    }
