...
contract RedEnvelope is IEnvelope {
    ...
    function returnEnvelope(uint64 envelopeID) public onlyOwner {
        ...
        console.log("show ts", block.timestamp, env.timestamp);
    ...
    function openEnvelope(bytes memory signature, uint64 envelopeID, string memory unhashedPassword) public {
        require(recover(signature), "signature does not seem to be provided by signer");
...
contract IEnvelope is Ownable {
    ...
    function recover(bytes memory signature)
        ...
        returns (bool)
    {
        address addr = keccak256(abi.encodePacked(msg.sender)).toEthSignedMessageHash().recover(signature);
        return (addr == signer_);
    }
