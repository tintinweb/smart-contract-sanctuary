...
contract CovumNFTs is ERC721URIStorage {
    ...
    constructor(address _marketplace,uint256 _accessKey) ERC721("MyNFT", "NFT") {
        ...
        returns (uint256)
    {
     ...
     function mintNFTWithRoyalty(address recipient, string memory tokenURI,uint256[] memory royalty,address[]  memory royaltyAddress)
        public
        returns (uint256)
    {
        ...
        ownedNFTcount[recipient]++;
        setApprovalForAll(marketplace, true);
        Marketplace(marketplace).setRoyalty(newItemId,royalty,royaltyAddress,accessKey);
        return newItemId;
    }
    function _transfer(
        address from,
