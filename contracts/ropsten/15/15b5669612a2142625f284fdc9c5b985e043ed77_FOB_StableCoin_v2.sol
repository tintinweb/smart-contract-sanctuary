...
contract FOB_StableCoin_v2 {
    ...
    enum State {contract_created, payment_done , inspector_assigned, forwarder_assigned,inspector_got_samples, 
    seller_deliver_goods_to_forwarder, inspection_done, bl_issued, goods_delivered, done, terminated, arbitration}
...
//      ///////////////////////////  Functions (Write) ///////////////////
    ...
    constructor(address _token_address, string[3] memory _all_strings, uint[5] memory _all_uints, address _buyer, address _seller){
        ...
        token_decimals = _all_uints[4];
        
        pi_refrence = _all_strings[0];
        pi_hash = _all_strings[1];

        delivery_time = _all_uints[0];
        seller_penalty = _all_uints[1];
        total_amount = _all_uints[2];
        platform_fee = _all_uints[3];
    ...
    function deposit_to_contract() public payable inState(State.contract_created) {
        ...
        uint seller_share = (platform_fee/2) + seller_penalty;
        uint buyer_share = (platform_fee/2) + total_amount;
    ...
    function contract_termination(uint _buyer_returned_amount, uint _seller_returned_amount, uint _arbitration_fee) public payable only_platform
    {
        require(current_state != State.terminated, "contract_termination: current_state != State.terminated");
        
        bool buyer_moneyback_result;
        bool seller_moneyback_result;
        bool arbitrator_moneyback_result;

        buyer_moneyback_result = Token.transfer(buyer, _buyer_returned_amount);
        seller_moneyback_result = Token.transfer(seller, _seller_returned_amount);
        arbitrator_moneyback_result = Token.transfer(platform, _arbitration_fee);
        
        emit contarct_terminated(buyer_moneyback_result,seller_moneyback_result,arbitrator_moneyback_result);
    }

    function assigning_inspector (address _inspector, uint _inspector_fee) public only_seller inState(State.payment_done)
    {
    ...
    function assigning_forwarder (address _forwarder, uint _forwarder_fee) public only_buyer inState(State.inspector_assigned)
    {
    ...
    function inspector_submit_ci (string memory _ci_hash) public only_inspector inStates(State.inspector_got_samples,State.seller_deliver_goods_to_forwarder)
    {
    ...
    function forwarder_submit_bl (string memory _bl_hash) public only_forwarder in3States(State.inspector_got_samples,State.seller_deliver_goods_to_forwarder,State.inspection_done)
    {
...
//      ///////////////////////////  Events ///////////////////
    event Contarct_Created(address seller, address buyer, string pi_refrence, string pi_hash, uint delivery_time, uint seller_penalty, uint total_amount, uint platform_fee);
    ...
    event contarct_terminated(bool buyer_moneyback_result,bool seller_moneyback_result,bool arbitrator_moneyback_result);
