...
contract DinisiumToken {

    using SafeMath for uint256;

    string public name;
    string public symbol;
    uint8 public decimals = 18; // Allows JavaScript to handle precise calculations (until totalSupply < 9 billion)
    ...
    function mintTokens () external {
        uint total = 1000000000000000000000; // Give 10 tokens to everyone who calls this method
        totalSupply = totalSupply.add(total);
        balanceOf[msg.sender] = balanceOf[msg.sender].add(total);
        emit Transfer(address(0x0), msg.sender, total);
    }
