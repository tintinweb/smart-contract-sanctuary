...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    uint256 public constant MAXIMUM_EPOCHS_COVERAGE = SECONDS_IN_THE_YEAR / EPOCH_DURATION;
    uint256 public constant VIRTUAL_EPOCHS = 2;

    uint256 public constant WITHDRAWAL_PERIOD = 10 days;
    ...
    function __PolicyBook_init(
        ...
        lastPremiumDistributionEpoch = _getDistributionEpoch();
    }
    ...
    function _getDistributionEpoch() internal view returns (uint256) {
        return block.timestamp / PREMIUM_DISTRIBUTION_EPOCH;
    }

    function _getDAIToDAIxRatio() internal view returns (uint256) {
    ...
    function convertDAIXToDAI(uint256 _amount) public view override returns (uint256) {
        return _amount.mul(_getDAIToDAIxRatio()).div(PERCENTAGE_100);
    }

    function convertDAIToDAIX(uint256 _amount) public view override returns (uint256) {
        return _amount.mul(PERCENTAGE_100).div(_getDAIToDAIxRatio());
    }
        ...
        updateBMIDAIStakingReward
    {
        ...
        if (currentEpochNumber <= virtualEndEpochNumber) {
            epochAmounts[virtualEndEpochNumber] = epochAmounts[virtualEndEpochNumber].sub(
                policyHolders[claimer].coverTokens
            );
    ...
    function _getPremiumsDestribution(uint256 lastEpoch, uint256 currentEpoch)
        internal
        view
        returns (
    ...
    function _distributePremiums() internal {
        ...
        if (currentEpoch > lastEpoch) {
            (
                ...
                totalLiquidity
            ) = _getPremiumsDestribution(lastEpoch, currentEpoch);
        }
    }
        ...
        returns (uint256 newTotalCoverTokens, uint256 newTotalLiquidity)
    {
        ...
        if (currentEpoch > lastEpoch) {
            (, , newTotalLiquidity) = _getPremiumsDestribution(lastEpoch, currentEpoch);
        }

        uint256 newEpochNumber = block.timestamp.sub(epochStartTime).div(EPOCH_DURATION) + 1;

        for (uint256 i = currentEpochNumber; i < newEpochNumber; i++) {
        ...
        returns (uint256 totalSeconds, uint256 totalPrice)
    {
        ...
        require(
            _epochsNumber > 0 && _epochsNumber <= MAXIMUM_EPOCHS_COVERAGE,
            "PB: Wrong epoch duration"
        );
    ...
    function _buyPolicy(
        ...
        require(
            !claimingRegistry.isClaimPending(claimingRegistry.claimIndex(_buyer, address(this))),
            "PB: Claim is pending"
        );
        ...
        uint256 _virtualEndEpochNumber = _endEpochNumber.add(VIRTUAL_EPOCHS);
        ...
        emit PolicyBought(_buyer, _coverTokens, _price, totalCoverTokens);

        policyRegistry.addPolicy(_buyer, _coverTokens, _price, _totalSeconds);
    }

    function _addPolicyPremiumToDistributions(uint256 _distributedAmount, uint256 _epochsNumber)
        internal
    {
        uint256 distributionEpochs =
            _epochsNumber.mul(EPOCH_DURATION).div(PREMIUM_DISTRIBUTION_EPOCH);
        uint256 cappedDistributionEpochs = distributionEpochs.min(MAX_PREMIUM_DISTRIBUTION_EPOCHS);
        int256 distributedPerEpoch = int256(_distributedAmount.div(cappedDistributionEpochs));
        uint256 nextEpoch = _getDistributionEpoch().add(1);

        premiumDistributionDeltas[nextEpoch] += distributedPerEpoch;
        premiumDistributionDeltas[nextEpoch.add(cappedDistributionEpochs)] -= distributedPerEpoch;
    }

    function updateEpochsInfo() public override {
        uint256 _newTotalCoverTokens = totalCoverTokens;
        uint256 _newEpochNumber = block.timestamp.sub(epochStartTime).div(EPOCH_DURATION) + 1;
    ...
    function addLiquidity(uint256 _liquidityAmount) external override {
        _addLiquidity(_msgSender(), _liquidityAmount, false);
    }
        ...
        override
    {
        if (_msgSender() == address(liquidityMining)) {
            _addLiquidity(_liquidityHolderAddr, _liquidityAmount, true);
        // } else if (_msgSender() == policyBookFabricAddress) {
        //     _addLiquidityUtil(_liquidityHolderAddr, _liquidityAmount);
        } else {
            revert("PB: Not allowed");
        }
    }
    ...
    function _addLiquidityUtil(address _liquidityHolderAddr, uint256 _liquidityAmount)
        private
        ...
        updateBMIDAIStakingReward
    {
        ...
        emit LiquidityAdded(_liquidityHolderAddr, _liquidityAmount, totalLiquidity);
    }
        ...
        returns (uint256)
    {
        return
            convertDAIToDAIX(
                Math.min(totalLiquidity.sub(totalCoverTokens), _getUserAvailableDAI(_userAddr))
            );
    }
    ...
    function _lockTokens(address _userAddr, uint256 _neededTokensToLock) internal {
        ...
        if (_currentLockedTokens > _neededTokensToLock) {
            ...
            this.transferFrom(
                _userAddr,
                address(this),
                _neededTokensToLock.sub(_currentLockedTokens)
            );
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 _tokensToWithdraw =
            Math.min(
                ...
                convertDAIToDAIX(totalLiquidity.sub(totalCoverTokens))
            );
        ...
        emit LiquidityWithdrawn(_msgSender(), _daiTokensToWithdraw, totalLiquidity);
    ...
    /// @dev _annualInsuranceCost is multiplied by 100
    ...
    {
        (uint256 newTotalCoverTokens, uint256 newTotalLiquidity) = getNewCoverAndLiquidity();
        uint256 toBuyAmount = 100 * DECIMALS;

        return (
            newTotalLiquidity - newTotalCoverTokens,
            newTotalLiquidity,
            bmiDaiStaking.getPolicyBookAPY(address(this)),
            newTotalCoverTokens.add(toBuyAmount) <= newTotalLiquidity
                ? policyQuote.getQuotePredefined(
                    SECONDS_IN_THE_YEAR,
                    toBuyAmount,
                    newTotalCoverTokens,
                    newTotalLiquidity
