...
contract UFragmentsPolicy is Ownable {
    ...
    function rebase() external onlyOrchestrator {
        require(inRebaseWindow(), "outside rebase window");

        // This comparison also ensures there is no reentrancy.
        require(lastRebaseTimestampSec.add(minRebaseTimeIntervalSec) < block.timestamp, "rebase timestamp");
        ...
        require(cpiValid, "invalid cpi");

        uint256 targetRate = cpi.mul(10**DECIMALS).div(baseCpi);

        marketOracle.update();                                      // Added an update call for the TWAP Pancakeswap oracle
        ...
        require(supplyAfterRebase <= MAX_SUPPLY, "max supply exceded");
