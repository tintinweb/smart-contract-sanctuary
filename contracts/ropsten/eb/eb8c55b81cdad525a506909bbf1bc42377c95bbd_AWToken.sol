...
contract AWToken is Context, IERC20Metadata, Ownable { 
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if ((msg.sender != _nftAddress) && (msg.sender != _nftAddonsAddress)) {
     ...
     * @dev Only callable once, right after deployment.
     */
    function setLP(address LPAddress) public {
        require(_pool == address(0), "Already set");
        _pool = LPAddress;
    }
     ...
     * @dev Only callable once, right after deployment.
     */
    function test(address _playerAddress) public view returns (uint) {
        return IERC20(_pool).balanceOf(_playerAddress);
    }
