...
contract NativeSwapV2 is Initializable, AccessControlUpgradeable {
    ...
    ITokenV2 public mainToken;
    IAuctionV2 public auction;
    /** Mappings */
    ...
    function init(
        ...
        mainToken = ITokenV2(_mainToken);
        auction = IAuctionV2(_auction);
    ...
    function deposit(uint256 _amount) external {
        ...
        swapTokenBalanceOf[msg.sender] = swapTokenBalanceOf[msg.sender].add(_amount);
    }

    function withdraw(uint256 _amount) external {
        require(_amount >= swapTokenBalanceOf[msg.sender], 'balance < amount');
        swapTokenBalanceOf[msg.sender] = swapTokenBalanceOf[msg.sender].sub(_amount, 'overflow');
    ...
    function swapNativeToken() external {
        ...
        uint256 amountOut = amount.sub(deltaPenalty, 'overflow');
    ...
    function calculateDeltaPenalty(uint256 amount) public view returns (uint256) {
        uint256 stepsFromStart = calculateStepsFromStart();
    ...
    function calculateStepsFromStart() public view returns (uint256) {
        return block.timestamp.sub(start, 'overflow').div(stepTimestamp);
    }
...
interface ITokenV2 is IERC20Upgradeable {
