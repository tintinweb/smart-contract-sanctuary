    ...
    AccessControlEnumerableUpgradeable
{
    ...
    function balanceOfUnderlyingStored() external view override returns(uint256) {
        return cToken.balanceOf(address(this)) * cToken.exchangeRateStored();
    }

    function getSupplyRate() external view override returns (uint256 _amount) {
        return cToken.supplyRatePerBlock() * BLOCK_PER_YEAR; // in 18
    }
...
interface IRhoStrategy {
     ...
     */
    function balanceOfUnderlyingStored() external view returns(uint256);
    /**
     * @dev Deploy the underlying to DeFi platform
     */
     ...
     */
    function getSupplyRate() external view returns (uint256 _amount);
     ...
     * be converted into the underlying asset
     */
    function setRewardConversionThreshold(uint256 _threshold) external returns (uint256);
