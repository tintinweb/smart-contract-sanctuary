...
contract SadPugs is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant PRICE_PER_CA = 6 * 10**15; // 0.05ETH Per SadPugs

    uint256 private constant MAX_ELEMENTS_CROWDSALE = 1 * 10**4; // 10000 SadPugs in CrowdSale
    uint256 private constant MAX_ELEMENTS_PRESALE = 5 * 10**2; // 500 SadPugs in PreSale

    uint256 private constant MAX_MINT_CROWDSALE = 20; // Upper Limit is 20 in CrowdSale
    ...
    address public constant developerAddress = 0x3eDDb12271A76016793f178891EB36811b18ef01; // Wallet Address of the Cory
    ...
    event CreateSadPugs(address to, uint256 indexed id);
    ...
    constructor (string memory baseURI) ERC721("SadPugs", "CATH") {
        setBaseURI(baseURI);
        pause(true);
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        require(total <= maxSalesAmount, "MINT: Please go to the Opensea to buy SadPugs.");
        require(_ids.length <= maxMintAmount, "MINT: Current count exceeds maximum mint count.");
        require(msg.value >= price(_ids.length), "MINT: Current value is below the sales price of SadPugs");
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        _widthdraw(developerAddress, balance.mul(10).div(100));
