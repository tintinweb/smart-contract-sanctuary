...
contract OnxLocker is Initializable, ReentrancyGuard {
    ...
    uint256 public constant LOCK_DURATION = 4 weeks;
    ...
    string private _name = "Vote Locked Onx Token";
    string private _symbol = "vlOnx";
    ...
    function initialize(address _multisigWallet, address _governanceToken)
        public
        initializer
    {
        governor = msg.sender;
    ...
    function setStakingToken(address token, bool isPool)
        external
    ...
    function balanceOfToken(address _user, address _token)
        public
        view
        returns (uint256 amount)
    {
        ...
        console.log("locks", locks.length, _token);

        uint256 nextUnlockIndex = userBalance.nextUnlockIndex;

        //start with current boosted amount
        amount = balances[_user][_token].boosted;

        console.log("boostes", amount);
        ...
        for (uint256 i = nextUnlockIndex; i < locksLength; i++) {
            if (locks[i].unlockTime <= block.timestamp) {
                ...
                console.log("minus boosted", amount);
            } else {
        ...
        if (
            ...
            amount = amount - locks[locksLength - 1].boosted;
            console.log(
                "remove amount in the current epoch",
                amount,
                locks[locksLength - 1].boosted
            );
        }
        return amount;
    }
    ...
    function balanceOf(address _user) external view returns (uint256 amount) {
        console.log("Total tokens amount", _totalTokens);

        for (uint256 i = 0; i < _totalTokens; i++) {
            address token = tokens[i];

            amount = amount + balanceOfToken(_user, token);
        }
        return amount;
    }
    ...
    function totalSupply() external view returns (uint256) {
        ...
        return
            calcTotalSupply(epochindex > 0 ? epochindex - 1 : 0, cutoffEpoch);
    }
    ...
    function findEpochId(uint256 _time) external view returns (uint256 epoch) {
        ...
        _time = (_time / LOCK_DURATION) * LOCK_DURATION;
    ...
    function transferStakingToken(
        ...
        IERC20(item.token).safeTransfer(_address, amount);
    }
    ...
    function lock(address _token, uint256 _amount) external nonReentrant {
        ...
        IERC20(item.token).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function _getBoostedAmount(
        ...
        if (staked.isPool) {
            IUniswapV2Pair pair = IUniswapV2Pair(staked.token);

            uint256 ts = pair.totalSupply();
            uint256 b = pair.balanceOf(_staker);

            (uint256 reserve1, uint256 reserve2, ) = pair.getReserves();

            return ((reserve1 + reserve2) * b) / ts;
        } else {
            return _amount;
        }
    }
    ...
    function _lock(
        ...
        uint256 boostedAmount = _getBoostedAmount(_account, _token, _amount);
        // console.log("Get boosted amount", boostedAmount);
        ...
        // console.log("Add locked and boosted", lockAmount, boostedAmount);
        ...
        // console.log("Gov token", governanceToken);
        // console.log("Staking token", _token);
        IERC20(governanceToken).safeApprove(_account, 0);

        // console.log("Approved", 0);
        IERC20(governanceToken).safeApprove(_account, boostedAmount);

        // console.log("Approved", boostedAmount);

        ITokenMinter(governanceToken).mint(_account, boostedAmount);

        // console.log("Minted to", _account, boostedAmount);
    ...
    function _processExpiredLocks(
        ...
        if (
            isShutdown ||
            locks[length - 1].unlockTime <= block.timestamp - _checkDelay
        ) {
        ...
        ITokenMinter(governanceToken).burn(_account, boostedAmount);

        emit Withdrawn(_account, locked, _relock);

        //relock or return to user
        if (_relock) {
            _lock(_token, _staker, locked);
        } else {
    ...
    function processExpiredLocks(address token, bool _relock)
        external
        nonReentrant
    {
        _processExpiredLocks(token, msg.sender, _relock, msg.sender, 0);
    }

    function kickExpiredLocks(address token) external nonReentrant {
    ...
    function emergencyWithdraw(address token) public onlyMultisigOrGovernor {
        ...
        uint256 balance = IERC20(item.token).balanceOf(address(this));

        IERC20(item.token).safeTransfer(multisigWallet, balance);
