...
/// @title Aura ERC721
/// @author jongold.eth cosmiccomputation.eth
/// @notice
/// @dev
contract Aura is Context, ERC721, AccessControl, ERC721Pausable {
    ...
    // store the standalone renderer
    string public permalink;
    ...
    /// @param givenBaseTokenURI the API
    /// @param givenPermalink the standalone renderer
    constructor(
        string memory name,
        string memory symbol,
        uint256 givenPricePerPiece,
        string memory givenBaseTokenURI,
        string memory givenPermalink,
        address givenOracleToken,
        address givenOracleAccount,
        address givenWithdrawalAddress
    ) ERC721(name, symbol) {
    ...
    function buy() public payable {
        require(
            !paused() || hasRole(MINTER_ROLE, _msgSender()),
            "Sale must be active"
        );
        require(msg.value == pricePerPiece, "must send in the correct amount");

        // TODO: allow minting-for-homies too
        _mintPiece(_msgSender());
    }

    /// @notice Mint a token with hash of recepient, to recepient
    /// @param recepient who the token is minted for
    /// @return tokenID
    function _mintPiece(address recepient) private returns (uint256) {
        // TODO: do I need this mapping?
        // require(
        //     addressToTokenId[newOwner] == 0,
        //     "Aura already minted for address"
        // );

        uint256 tokenId = _tokenIds.current();

        bytes32 newHash = _psuedoRandomHash();
    ...
    function setWithdrawalAddress(address givenWithdrawalAddress)
        public
    ...
    function pause() public virtual {
        require(
            ...
            "ERC721Pausable: must have pauser role to pause"
        );
        _pause();
    }
    ...
    function unpause() public virtual {
        require(
            hasRole(PAUSER_ROLE, _msgSender()),
            "ERC721Pausable: must have pauser role to unpause"
        );
        _unpause();
    }
