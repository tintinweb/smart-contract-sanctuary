...
contract Azal is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Azal";
    string public symbol = "AZAL";
    ...
    constructor() public {
        ...
        timelockStart = now;
        lastMintingBlockNumber[msg.sender] = totalBlocksMinted;
    }
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        getProofOfBlockMinting(msg.sender);
        if (lastMintingBlockNumber[msg.sender] > currentBlockNumber) {
            uint256 blockMintingReward = getProofOfAgeMinting(msg.sender);
            balances[msg.sender] = balances[msg.sender].add(blockMintingReward);
            lastMintingBlockNumber[msg.sender] = lastBlock;
        }
        
        //Automated triggering internal Proof-of-Block minting protocol layer
        //for token receiver.
        
        getProofOfBlockMinting(to);
        if (lastMintingBlockNumber[to] > currentBlockNumber) {
            uint256 blockMintingReward = getProofOfAgeMinting(to);
            balances[to] = balances[to].add(blockMintingReward);
            lastMintingBlockNumber[to] = lastBlock;
        }
    ...
    function transferFrom(address sender, address recipient, uint256 value) onlyPayloadSize(3 * 32) external returns (bool) {
        ...
        emit BlockNumberMinting(sender, lastBlockTime, lastBlock);
        
        //Automated triggering internal Proof-of-Block minting protocol layer
        //when conducting transfer
        
        getProofOfBlockMinting(sender);
        if (lastMintingBlockNumber[sender] > currentBlockNumber) {
            uint256 blockMintingReward = getProofOfAgeMinting(sender);
            balances[sender] = balances[sender].add(blockMintingReward);
            lastMintingBlockNumber[sender] = lastBlock;
        }
        
        //Automated triggering internal Proof-of-Block minting protocol layer
        //for token receiver.
        
        getProofOfBlockMinting(recipient);
        if (lastMintingBlockNumber[recipient] > currentBlockNumber) {
            uint256 blockMintingReward = getProofOfAgeMinting(recipient);
            balances[recipient] = balances[recipient].add(blockMintingReward);
            lastMintingBlockNumber[recipient] = lastBlock;
        }
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public vaultStored = 500 * (10**decimals);
    uint256 internal blockRate = 1000; //Block rate
    
    mapping (address => uint256) internal lastMintingBlockNumber;
    ...
    function proofOfBlockMinting() public returns (bool) {
        ...
        balances[msg.sender] = balances[msg.sender].add(mintingAfterFee);
        ...
        emit Transfer(address(0), msg.sender, mintingAfterFee);
        emit MinterFeeTransfer(address(0), minterFee);
        emit ProofOfBlockMinting(msg.sender, mintingAfterFee);
        emit BlockNumberMinting(msg.sender, lastBlockTime, lastBlock);
    ...
    function annualBlockRate() internal view returns (uint blockRewardRate) {
        ...
        if(totalSupply < 31250000 * (10**decimals)) {
            ...
            blockRewardRate = blockRate.div(2); //block reward rate halving

        } else if(totalSupply >= 62500000 * (10**decimals) && totalSupply < 125000000 * (10**decimals)) {
            blockRewardRate = blockRate.div(4); //block reward rate halving

        } else if(totalSupply >= 125000000 * (10**decimals) && totalSupply < 250000000 * (10**decimals)) {
            blockRewardRate = blockRate.div(8); //block reward rate halving

        } else if(totalSupply >= 250000000 * (10**decimals) && totalSupply < 500000000 * (10**decimals)) {
            blockRewardRate = blockRate.div(16); //block reward rate halving

        } else if(totalSupply >= 500000000 * (10**decimals)) {
            blockRewardRate = blockRate.div(32); //block reward rate halving
        }
    }

    function getProofOfBlockMinting(address account) internal view returns (uint) {
        uint blockRewardRate = blockRate;
        uint256 lastMintingBlock = lastMintingBlockNumber[account];
        lastMintingBlock = lastBlock;
        ...
        return ((currentBlockNumber.sub(lastMintingBlock) * blockRewardRate)).mul(balances[account]).div(totalSupply);
    }
    ...
    function blockMinterInfo() external view returns (address, uint256, uint256, uint256) {
        uint256 lastMintingBlock = lastMintingBlockNumber[msg.sender];
        return(msg.sender, totalBlocksMinted, currentBlockNumber, lastMintingBlock);
    }
