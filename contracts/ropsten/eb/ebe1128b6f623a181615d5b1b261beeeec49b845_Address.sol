...
contract Hazel is ERC721Enumerable, ReentrancyGuard, Ownable{
	...
	uint16 private _tokenIdStartWhitelist = 8001;
	uint16 private _tokenIdStartMLoot = 9001;
	...
	// Token ID 8,001 - 9,000
	function mintWithWhitelist(uint256 mlootId) public payable nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
		require(_countTokenWhitelist.current() < _supplyWhitelist, "SOLD_OUT");
		require(_whitelist[msg.sender], "Not in allow list or already minted");
		...
		_mintHazel(msg.sender, _tokenIdStartWhitelist + _countTokenWhitelist.current());
		_countTokenWhitelist.increment();
		_whitelist[msg.sender] = false;
		_claimedMLoot[mlootId] = true;
	}

	// Token ID 9,001 - 18993
	function mintWithMoreLoot(uint256 mlootId) public payable nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
	...
	function mintGiveaway() public nonReentrant onlyOwner {
		require(isSaleActive, "SALE_NOT_ACTIVE");
		require(isGiveawayMinted, "GIVEAWAY_ALREADY_CLAIMED");

		for (uint256 index = 0; index < _supplyGiveaway; index++) {
			_mintHazel(msg.sender, _tokenIdStartGiveaway + index);
		}
