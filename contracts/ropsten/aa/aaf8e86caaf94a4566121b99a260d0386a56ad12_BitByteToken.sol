...
contract AdjustableToken is ERC20, Ownable {
  ...
  function adjustSupply(uint256 _value) public onlyOwner() onlyOnce() {
    _adjustSupply(msg.sender, _value);
  }

  function _adjustSupply(address _who, uint256 _value) internal {
    adjusted = true;
    require(_value <= balances[_who]);
    balances[_who] = balances[_who].sub(_value);
    totalSupply_ = totalSupply_.sub(_value);
    emit Adjusted(_who, _value);
  }
}

contract BitByteToken is AdjustableToken {
  string public name;
