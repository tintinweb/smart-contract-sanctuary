...
contract NftFactoryManager {
   ...
   function createPack (
    ...
    function getPack(uint _packID) public view returns(MomentState.Pack memory pack) {
        pack = packID_to_pack[_packID];
    }
}
...
contract CreatorPool {
    ...
    // Getters
    function getCreatorCutOnPackPurchase(uint _value) internal view returns (uint valueToPay) {
        valueToPay = _value * creatorCutOnPackPurchase/100;
    }

    function getCommunityCutOnPackPurchase(uint _value) internal view returns (uint valueToPay) {
        valueToPay = _value * communityCutOnPackPurchase/100;
    }

    function getResaleTransactionFee(uint _value) internal view returns (uint valueToPay) {
        valueToPay = _value * resaleTransactionFee/100;
    }

    function getCreatorCutOfTransactionFee(uint _value) internal view returns (uint valueToPay) {
        valueToPay = _value * creatorCutOfTransactionFee/100;
    }

    function getCommunityCutOfTransactionFee(uint _value) internal view returns (uint valueToPay) {
        valueToPay = _value * communityCutOfTransactionFee/100;
    }

    function getProtocolCutOfTransactionFee(uint _value) internal view returns (uint valueToPay) {
        valueToPay = _value * protocolCutOfTransactionFee/100;
    }

    function getContributorShare(address _creator, address _contributor) public view returns (uint share) {
        share = creatorPool[_creator].contributionToPool[_contributor];
    }
