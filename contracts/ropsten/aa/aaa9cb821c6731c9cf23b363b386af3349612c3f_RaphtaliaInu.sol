...
contract RaphtaliaInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Raphtalia Inu", "RAPHTALIA") {
    	...
    	marketingWallet = address(0x5F29d92eF65858ac30AfE30bb9d226EE2452282C); // set as marketing wallet
        liquidityWallet = address(owner()); // liquidity will go to dead address at first, can update later if needed.
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (
                ...
                //normal transfer
                        ...
                        require(amount <= maxTransactionAmount, "Tansfer amount exceeds the maxTransactionAmount.");
                }
            }
        }
        ...
        if(takeFee && (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from])) {
            ...
            super._transfer(from, address(marketingWallet), marketingFees);
            super._transfer(from, address(this), liquidityFees);
        	
        	amount -= liquidityFees + marketingFees;
        }
    ...
    function setRouterVersion(address _router)
        public
        onlyOwner
    ...
    // send tokens for liquidity to contract directly, then call this function and pass the amount of ETH to launch with (not required, can still use Uniswap to add liquidity manually)
    function supplyLiquidity() external payable onlyOwner returns (bool){
        addLiquidity(balanceOf(address(this)), msg.value);
        return true;
    }
    
    function swapBack(uint256 contractTokenBalance) internal {
        uint256 amountToLiquify = contractTokenBalance.div(2);
        ...
        if(amountToLiquify > 0){
            addLiquidity(amountToLiquify, amountBNB);
        }
        
        // buyback amount left on contract if enabled
    }
