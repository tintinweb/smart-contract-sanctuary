        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < _expiredTimestamps.length; i = i.add(1)) {
             require(!(stakerData[_expiredTimestamps[i]].isCompleted),"Already Unstaked");
                    ...
                    _drawnAmount = stakerData[_expiredTimestamps[i]].altQuantity;
                    
                    if(withdrawAmount.add(_drawnAmount) > _amount){
                        
                        _drawnAmount = _amount.sub(withdrawAmount);
                    
                        stakerData[_expiredTimestamps[i]].altQuantity = stakerData[_expiredTimestamps[i]].altQuantity.sub(_drawnAmount);
                    }
                    else{
                    ...
                    uint256 _days = _curentTime.sub(_expiredTimestamps[i]).div(1 days);
                    ...
                    if(_days > 0) {
                        ...
                        stakerData[_expiredTimestamps[i]].reward = stakerData[_expiredTimestamps[i]].reward.add(rewardAmount);
                    }
