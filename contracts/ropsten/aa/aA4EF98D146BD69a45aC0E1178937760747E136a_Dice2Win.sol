...
contract Dice2Win {
    ...
    uint constant MAX_BET_MASK = 2 ** MAX_MASK_MODULO; // 2 的 40 次方

/*
    EVM BLOCKHASH操作码最多只能查询256个块。 鉴于settleBet使用placeBet的块哈希作为补充熵源之一，我们无法处理早于此阈值的下注。 
    在极少数情况下，由于技术问题或极端的以太坊拥塞，dice2.win副主持人可能无法在此时间段内调用settleBet； 
    这样的投注可以通过调用refundBet进行退款。
*/
    uint constant BET_EXPIRATION_BLOCKS = 250; //------原版本是250，我改为了240

    // Some deliberately invalid address to initialize the secret signer with.
    // Forces maintainers to invoke setSecretSigner before processing any bets.
    address constant DUMMY_ADDRESS = 0xa31135Ef1E44f2d76505E04B55aa442aA5492e1C;   //0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
                                   
    // Standard contract ownership transfer.
    address payable public owner;
    address payable private nextOwner;

    // Adjustable max bet profit. Used to cap bets against dynamic odds.可调最大投注利润。 用于限制动态赔率的下注。
    uint public maxProfit;

    // The address corresponding to a private key used to sign placeBet commits.
    address public secretSigner;

    // Accumulated jackpot fund.累积的累积奖金。
    ...
    event Commit(uint commit); // 记录 commit 公开给玩家自己调用 refundBet()方法
    ...
    //  50000000000000000000  设置maxProfit
    ...
    //  568000000000000000  设置jackpotSize
    ...
    // builds a prefixed hash to mimic the behavior of eth_sign.构建前缀哈希以模仿eth_sign的行为。
    ...
    function placeBet(uint256 betMask, uint8 modulo, uint256 commitLastBlock, uint256 commit, bytes32 r, bytes32 s, uint8 v) external payable {
        // Check that the bet is in 'clean' state.
        Bet storage bet = bets[commit];
                                //address(0)是零地址 0x0000000000000000000000000000000000000000
        ...
        require (modulo > 1 && modulo <= MAX_MODULO, "Modulo should be within range.");
                         ...
                         //0.01 ether         //300000 ether
        require (amount >= MIN_BET && amount <= MAX_AMOUNT, "Amount should be within range.");
                                                                    
                                       // MAX_BET_MASK = 2 的 40 次方 = 1099511627776
        require (betMask > 0 && betMask < MAX_BET_MASK, "Mask should be within range.");

        // Check that commit is valid - it has not expired and its signature is valid.
        require (block.number <= commitLastBlock, "Commit has expired.");
        //Sign "keccak256(abi.encodePacked(commitLastBlock, commit))" on local.而prefixed（message）= Hashmessage
        bytes32 signatureHash = prefixed(keccak256(abi.encodePacked(commitLastBlock, commit)));
        
        require (secretSigner == ecrecover(signatureHash, v, r, s), "ECDSA signature is not valid.");

        uint rollUnder;  // 越大用户赢得越少
        uint mask;
                   // MAX_MASK_MODULO = 40
        if (modulo <= MAX_MASK_MODULO) {
            ...
            rollUnder = ((betMask * POPCNT_MULT) & POPCNT_MASK) % POPCNT_MODULO;  //  取决于 betMask
            mask = betMask;
        } else {
            // Larger modulos specify the right edge of half-open interval of winning bet outcomes.
            // 较大的模数指定获胜投注结果的半开间隔的右边缘。
                                             //modulo > MAX_MASK_MODULO = 40
        ...
        // Enforce max profit limit. 实施最大利润限制。
        ...
        bet.amount = amount;
        bet.modulo = modulo;//uint8(modulo);
    ...
    //这是用于结算99％赌注的方法。 
    //要使用特定的“commit提交”来下注，settleBet应该提供一个“reveal”号，该数字将使Keccak256哈希“commit”。
   // “ blockHash”是croupier看到的placeBet块的块哈希； 此外，它还声称可以防止更改以太坊改组的投注结果。
                                            //placebet投注所在的区块哈希
    function settleBet(uint256 reveal, bytes32 blockHash) external onlyCroupier {
        ...
        require (block.number <= placeBlockNumber + BET_EXPIRATION_BLOCKS, "Blockhash can't be queried by EVM.");
                                                //placebet的块哈希
        require (blockhash(placeBlockNumber) == blockHash, "settleBet error");
    ...
    //此方法用于结算投注在叔叔区块中的赌注。
    //这时显示给玩家一些下注结果，但是由于以太坊链重组，在placeBet高度的筹码有所不同。 
    //我们提供了placeBet交易收据的完整merkle证明，以提供无可辩驳的证据，表明叔叔区块哈希确实存在于某个时刻。
                                                            // placebet投注所在的叔块高度
    function settleBetUncleMerkleProof(uint256 reveal, uint40 canonicalBlockNumber) external onlyCroupier {
        ...
        // Verify placeBet receipt. 证明placeBet收据
    ...
    function settleBetCommon(Bet storage bet, uint256 reveal, bytes32 entropyBlockHash) private {
        ...
        bytes32 entropy = keccak256(abi.encodePacked(reveal, entropyBlockHash)); // entropy=熵
// %操作符是求余数，保留整数的意思，这个是取余运算，不是除法运算。一共有三种情况（够除有余数、不够除、没有余数。）
// 够除有余数：案例1：12除以5=2，余数是2，即5*2+2=12，所以12%5=2
//             案例2：7除以3=2，余数是1，即3*2+1=7，所以7%3=1
// 不够除，余数为除数：
//             案例1：1除以2不够除，除数为1，所以余数为1，即1%2=1
//             案例2：2除以3不够除，除数为2，所以余数为2，即2%3=2
// 没有余数，所以为0：
//             案例1：0%1=0
//             案例2：1%1=0
//             案例3：2%1=0
        ...
        if (modulo <= MAX_MASK_MODULO) {
            ...
            if ((2 ** dice) & bet.mask != 0) { // & 运算中  1&1=1,1&0=0,0&0=0 (二进制)
                diceWin = diceWinAmount;  // 用户猜中中奖的情况
            }
    ...
    /*
    退款交易-返回在规定时间内未处理的掷骰的下注金额。 由于EVM的限制，无法处理此类块（有关详细信息，请参见上面的BET_EXPIRATION_BLOCKS注释）。 
    如果您遇到这种情况，请与dice2.win支持人员联系，但是没有什么可以阻止您自己调用此方法。
    */
    /*
    EVM BLOCKHASH操作码最多只能查询256个块。 鉴于settleBet使用placeBet的块哈希作为补充熵源之一，我们无法处理早于此阈值的下注。 
    在极少数情况下，由于技术问题或极端的以太坊拥塞，dice2.win副主持人可能无法在此时间段内调用settleBet； 
...
*/
    function refundBet(uint256 commit) external {
        ...
        // Check that bet has already expired.       //BET_EXPIRATION_BLOCKS = 250
        ...
        // Send the refund.
        //无论是谁调用，返还的是曾经下注的账户地址, -------完美！！！
        sendFunds(bet.gambler, amount, amount);
    }

    // Get the expected win amount after house edge is subtracted. 在减去房屋边缘后获得预期的获胜金额。
    function getDiceWinAmount(uint256 amount, uint256 modulo, uint256 rollUnder) private pure returns (uint256 winAmount, uint256 jackpotFee) {//jackpotFee=头奖费用
        
                            // 👉 rollUnder <= modulo
        require (0 < rollUnder && rollUnder <= modulo, "Win probability out of range."); //获胜概率超出范围。
                              //0.1 ether       //0.001 ether
    ...
    function sendFunds(address payable beneficiary, uint256 amount, uint256 successLogAmount) private {
    ...
    uint constant POPCNT_MULT = 0x0000000000002000000000100000000008000000000400000000020000000001;  //51422017416311072843540092035432428522196642838281063615168513
    uint constant POPCNT_MASK = 0x0001041041041041041041041041041041041041041041041041041041041041;  //1794892256282803128465572064246774365285063006349835132060044394963931201
    uint constant POPCNT_MODULO = 0x3F;  //63
    ...
    function verifyMerkleProof(uint256 seedHash, uint256 offset) pure private returns (bytes32 blockHash, bytes32 uncleHash) {
    ...
    // Helper to check the placeBet receipt.  帮忙检查placeBet收据。
    // "offset" is the location 位置 of the proof beginning 开始 in the calldata.
    // RLP layout: [triePath, str([status, cumGasUsed, bloomFilter, [[address, [topics], data]])]
    function requireCorrectReceipt(uint256 offset) view private {
    ...
    // Memory copy.
    function memcpy(uint256 dest, uint256 src, uint256 len) pure private {
        // Full 32 byte words
        for(; len >= 32; len -= 32) {
