...
interface IERC20 {
	...
	// DEV moves amount tokens from the caller account to recipient.
	function transfer(address recipient, uint256 amount) external returns (bool);

	// DEV returns the remaining number of tokens that spender will be allowed to spend on behalf of owner through function transferFrom (0 by default). Value changes when function approve or transferFrom are called.
	...
	// DEV emitted when the allowance of a spender for an owner is set by a call to function approve, value is the new allowance.
...
library SafeMath {
	...
	// DEV returns the integer division of two unsigned integers, reverts on division by zero. Result is rounded towards zero.
	function div(uint256 a, uint256 b) internal pure returns (uint256) { return div(a, b, "SafeMath: division by zero"); }

	// DEV returns the integer division of two unsigned integers, reverts with custom message on division by zero. Result is rounded towards zero.
...
library Address {
	...
	// DEV returns TRUE if account is valid contract.
	function isContract(address account) internal view returns (bool) {

		// according to EIP-1052, 0x0 is the value returned for not-yet created accounts and 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470 is returned for accounts without code - `keccak256('')`.
		bytes32 codehash;
	...
	// DEV replacement for Solidity's transfer: sends amount wei to the recipient, forwarding all available gas and reverting on errors.
...
// DEV contract module which provides a basic access control mechanism, where there is an account - owner - that can be granted exclusive access to specific functions.
contract Ownable is Context {
	...
	// Unlocks the contract for owner when _lockTime is exceed.
...
contract loremipsum2 is Context, IERC20, Ownable {
	...
	string private _name = "loremipsum2";
	string private _symbol = "loremipsum2";
	...
	// to recieve ETH from UniSwapV2Router on swaping.
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		// Is the token balance of this contract address over the min number of tokens that we need to initiate a swap + liquidity lock? Don't get caught in a circular liquidity event. Don't swap and liquify if sender is UniSwap pair.
	...
	function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
		...
		// Capture the contract current ETH balance. This is so that we can capture exactly the amount of ETH that the swap creates + not make the liquidity event include any ETH that has been manually sent to the contract.
		uint256 initialBalance = address(this).balance;

		// Swap tokens for ETH.
		swapTokensForEth(half); // Breaks the ETH -> HATE swap when swap + liquify is triggered.
	...
	// Responsible for taking all fee - if takeFee is true.
