...
contract StandardToken is IERC20 {
  ...
  uint256 tokensForMMM = 20000000000000000000000000;                            // airdrop tokens for MMM
  uint256 minTokensInSwap = 500000000000000000000;                              // Minimum tokens user have to swap
...
function purchaseTokens() external failSafe payable payableCheck{
    ...
    emit PurchaseToken(msg.sender, tokens);
  }
   ...
   */
   function _preValidatePurchase(uint256 _ETH) internal view { 
        ...
        require(_ETH >= minTokensInSwap, "tokens cannot be less than 500");
   ...
   */
    function _getTokenAmount(uint256 _ETH) internal view returns (uint256) {
      uint256 tokens;
         tokens = _ETH.mul(_tokenPriceETH);
      return tokens;
    }
