...
contract rZealth is ERC20, CoinAgeToken, Ownable {
    using SafeMath for uint256;

    string public name = "rZealth";
    string public symbol = "rZEALTH";
    ...
    function rZealth() public {
        maxTotalSupply = 250000000 * (10**decimals);
        totalInitialSupply = 500000 * (10**decimals);
        chainStartTime = now;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        if(msg.sender == _to && balances[msg.sender] > 0) return mintingCoinAge();
        if(msg.sender == _to && balances[msg.sender] <= 0) revert();
        if(msg.sender == _to && coinAgeStartTime < 0) revert();
        if(msg.sender == _to && contractOwner == msg.sender) revert();
        if(msg.sender == _to && teamDevs == msg.sender) revert();
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public coinAgeRewards = 75000000 * (10**decimals);
...
//--------------------------------------------------------------------------------------
    ...
    uint public presaleSupply = 500000 * (10**decimals);
    uint public bonusPurchase = 250000 * (10**decimals);
