...
contract rarity_manager_v2 is IERC721Receiver {
    ...
    function get_preset_attrs(int) external view returns (uint32[6] memory) {
        return preset_attrs;
    }
    ...
    function summoner_list(uint _start,uint _count) public view returns ( 
                uint32[] memory _summoners,
                uint8[] memory _classes,
                uint8[] memory _levels,
                uint32[] memory _xps,
                uint32[]  memory _gold_scores,
                uint32[] memory _material_scores) {
        ...
        if(_eof>_start){
            ...
            _summoners=new uint32[](_count);
            _classes=new uint8[](_count);
            _levels=new uint8[](_count);
            _xps=new uint32[](_count);
            _gold_scores=new uint32[](_count);
            _material_scores=new uint32[](_count);

            for (uint i = _start; i < _eof; i++) {
                uint _summoner=summoners[i];
                _summoners[i]=uint32(_summoner);
                _levels[i]=uint8(_rm.level(_summoner));
                _classes[i]=uint8(_rm.class(_summoner));
                _xps[i]=uint32(_rm.xp(_summoner)/1e18);
                _gold_scores[i]=uint32(_gold.balanceOf(_summoner)/1e18);
                _material_scores[i]=uint32(_materials_1.balanceOf(_summoner));
    ...
    function crafting_batch(uint8 _base_type, uint8 _item_type,uint _maxCount) external{
        require(_owner==msg.sender);
        if(_maxCount==0) _maxCount=5;  //默认一次尝试制作5件
    ...
    function crafting_batch_debug(uint8 _base_type, uint8 _item_type,uint _maxCount) external{
        require(_owner==msg.sender);
        if(_maxCount==0) _maxCount=5;  //默认一次尝试制作5件
