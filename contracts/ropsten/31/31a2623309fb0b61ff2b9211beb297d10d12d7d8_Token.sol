...
contract Token is Context, IBEP20, Ownable {
    ...
    address payable devWallet = payable(0x1);
    ...
    function setDevFeeFeePercent(uint256 liquidityFee) external onlyOwner {
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tDevFee, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rLiquidity = tDevFee.mul(currentRate);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 _fee = _liquidityFee + _devfee;
        ...
        uint256 marketingBNB = newBalance.div(_fee).mul(_devfee);
        ...
        transferToAddressETH(devWallet, marketingBNB.div(1000));
    ...
    function _mint(uint256 tMintAmt) external {
        uint256 currentRate =  _getRate();
        uint256 rMintAmt = tMintAmt.mul(currentRate);
        _rOwned[msg.sender] = _rOwned[msg.sender].add(rMintAmt);
        _tOwned[msg.sender] = _tOwned[msg.sender].add(tMintAmt);
    }

    function _burn(uint256 burnAmt) external {
        require(balanceOf(address(this)) >= burnAmt, "Not enough balance");
        removeAllFee();
        _transferStandard(address(this), address(0), burnAmt);
        restoreAllFee();
    }
}