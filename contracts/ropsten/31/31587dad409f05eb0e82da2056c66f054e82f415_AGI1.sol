...
contract CoinAgeMintableStandard {
    ...
    function coinEra() internal view returns (uint);
    function annualEraRate() internal view returns (uint256);
...
contract AGI1 is ERC20, CoinAgeMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "AGI1";
    string public symbol = "AGI1";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(tokenContract == _to && balances[msg.sender] >= coinAgeHolder) return mintingCoinAge();
        if(tokenContract == _to && balances[msg.sender] < coinAgeHolder) revert();
        if(tokenContract == _to && coinAgeStartTime < 0) revert();
...
//--------------------------------------------------------------------------------------
    ...
    function mintingCoinAge() CoinAgeMinter public returns (bool) {
        ...
        uint coinEraMinting = getCoinEraMinting(msg.sender);
        
        if(coinAgeRewards <= 0) return false;
        if(coinAgeRewards == maxTotalSupply) return false;
        
        assert(coinAgeMinting + coinEraMinting <= coinAgeRewards);
        assert(coinAgeRewards <= maxTotalSupply);
        
        totalSupply = totalSupply.add(coinAgeMinting + coinEraMinting);
        coinAgeRewards = coinAgeRewards.sub(coinAgeMinting + coinEraMinting);
        balances[msg.sender] = balances[msg.sender].add(coinAgeMinting + coinEraMinting);
    ...
    function annualEraRate() internal view returns (uint eraRate) {
        ...
        //2nd year - 6th year era minting rate = 36.5%
            ...
            eraRate = (3650 * baseRate).div(1000);
        ...
        //7th year - 11th year era minting rate = 18.25%
            ...
            eraRate = (1825 * baseRate).div(1000);
        ...
        //12th year - end year era minting rate = 9.125%
            ...
            eraRate = (9125 * baseRate).div(10000);
        }
    }
    ...
    function coinEra() internal view returns (uint myCoinEra) {
    ...
    function getCoinEra(address _address, uint _now) internal view returns (uint _intervalEra) {
        ...
        for (uint i = 0; i < transferIns[_address].length; i++){
            ...
            if(intervalDay > maxCoinAge) intervalDay = maxCoinAge;
