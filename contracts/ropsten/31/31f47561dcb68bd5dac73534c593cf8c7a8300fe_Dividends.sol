...
contract Dividends {
    ...
    constructor(address tokenAdr) public {
        token = HomburgerToken(tokenAdr);
    }
    
    function checkBeneficiary(address adr) internal view returns (bool) {
        for (uint256 i; i<beneficiaries.length; i++) {
            if (beneficiaries[i] == adr) {
                return true;
            }
        }
        return false;
    ...
    function registerAccount() public {
        require(!checkBeneficiary(msg.sender));
    ...
    function depositETH() payable public {
        ...
        // calculation
        for (uint256 i; i<beneficiaries.length; i++) {
            uint256 currentBalance = token.balanceOf(beneficiaries[i]);
            snapshot[beneficiaries[i]] += currentBalance*totalAmount/totalSupply;
        }
    }
    ...
    function claimDividend() public {
        ...
        msg.sender.transfer(snapshot[msg.sender]);
    }
}