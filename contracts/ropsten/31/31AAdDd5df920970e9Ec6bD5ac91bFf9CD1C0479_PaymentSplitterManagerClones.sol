...
contract PaymentSplitterCloneable is Context {
    ...
    function initialize(address[] memory payees_, uint256[] memory shares_) public {
     ...
     * @dev Getter for the amount of shares held by an account.
     */
    function sharesOfAccount(address account) public view returns (uint256) {
        return _shares[account];
    }

    function shares() public view returns (uint256[] memory) {
        uint256[] memory sharesByIndex = new uint256[](_payees.length);
        for (uint i = 0; i < _payees.length; i++) {
            sharesByIndex[i] = _shares[_payees[i]];
        }
        return sharesByIndex;
    }
