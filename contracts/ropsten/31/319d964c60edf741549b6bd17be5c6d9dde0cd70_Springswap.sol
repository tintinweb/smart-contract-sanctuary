...
contract Springswap {
  address immutable factory;//of uniswap **
  uint constant deadline = 10 days; //trade dealines
  IUniswapV2Router02 immutable sushiRouter;  //sushiwap liquidity pool which can be used ans uniswap 

  address payable _owner; 
  event EventUniswapV2call(address evAddr, bytes evData); 

  constructor(address _factory, address _sushiRouter)  public {
    _owner = payable(msg.sender);
    factory = _factory;   //uniswap address factory
    sushiRouter = IUniswapV2Router02(_sushiRouter); //sushiswap address router
  }
  ...
  //uniswap will call back this method
  function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
      ...
      emit EventUniswapV2call(_sender, _data);
  }
}
