...
contract Lottery  {
    ...
    */
    ...
    modifier checkFeeDialNumber(){
        uint256 convertUSDtoETH = getEthfromUSD();
        if ( tx_fee_paid_by_the_game == 1){//fee paid owner
            uint256 txCostEstimate =  msg.value + ( tx.gasprice * gasLimit);
            require(convertUSDtoETH <= txCostEstimate,"Value not enough money!");
              _;
        }else{
            require(msg.value >= convertUSDtoETH ,"Value  not enough money  !");
              _;
        }
    }
    ...
    function adminGetFeeDial() public constant returns(address _address_contract,uint256 _feeETH,uint256 _feeUSD ){
        uint256 ethCent = price.USD(0);
        return ( address(this), ethCent * 100 * feeUsd,feeUsd );
    }
    //GET balance 
