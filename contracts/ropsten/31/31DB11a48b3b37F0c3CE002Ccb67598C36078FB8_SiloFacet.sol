...
contract SiloEntrance is SiloExit {
    ...
    **/
    ...
    function farmBeans(address account, uint32 update) private {
        uint256 beans = balanceOfFarmableBeansV3(account);
        if (beans > 0) {
            s.si.beans = s.si.beans.sub(beans);
        }

        if (s.a[account].lastSIs < s.season.sis) {
            beans = beans.add(farmLegacyBeans(account, update));
        }
    ...
    function farmLegacyBeans(address account, uint32 update) public returns (uint256 beans) {
        if (update < s.hotFix3Start) {
            uint256 depBeans = balanceOfFarmableBeansV1(account);
            if (depBeans > 0) {
        ...
        uint256 beans = balanceOfFarmableBeansV2(unclaimedRoots);
        if (beans > 0) {
            s.v2SIBeans = s.v2SIBeans.sub(beans);
...
contract SiloExit {
    ...
    **/
    ...
    function totalFarmableBeans() public view returns (uint256) {
        return s.si.beans.add(s.legSI.beans).add(s.v2SIBeans);
    }
    ...
    function balanceOfFarmableBeans(address account) public view returns (uint256 beans) {
        ...
        beans = beans.add(balanceOfFarmableBeansV3(account));
    }

    function balanceOfFarmableBeansV3(address account) public view returns (uint256 beans) {
    ...
    function balanceOfFarmableBeansV1(address account) public view returns (uint256 beans) {
        if (s.s.roots == 0 || s.legSI.beans == 0 || lastUpdate(account) >= s.hotFix3Start) return 0;
