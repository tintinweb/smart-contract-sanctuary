...
abstract contract StandardToken is ERC20 {
 ...
 function transfer(address _to, uint256 _value) virtual public returns (bool) {
  ...
  function whenICollectedMyInterest(address _owner) public view returns (uint256 interest) {
    return interestCollectedAt[_owner];
  }
...
//         token = new ERC20Basic();
    ...
    function collect_interest() public {
        //uint256 lastCollectionAt = interestCollectedAt[msg.sender];
        
    
    uint256 newInterests = calculateMyInterest(msg.sender);
//require(newInterests <0 ,"Interest will be available after 1 Day.");

balances[owner] = balances[owner] - (newInterests);
balances[msg.sender] = balances[msg.sender] + (newInterests);
interestCollectedAt[msg.sender] = block.timestamp;
...
if(interestCollectedAt[_to] == 0)
{
   interestCollectedAt[_to] = block.timestamp - 1 days;
}
...
emit Transfer(owner,careFund,careFundAmount);
 ...
 function isStakeholder(address _address)
   ...
   //    require(lastCollectedAt != 0,"Interest Last Collection Time Not Available.");
           uint256 daysSinceLastCollect = ((block.timestamp - lastCollectedAt ) / 86400 );
...
function buy() payable public {
   //  require(beneficiary != address(0));
    ...
    approve(msg.sender,amountTobuy);
  transferFrom(owner,msg.sender,amountTobuy);
...
function buyTokens() public payable {
   ...
   // require(validPurchase());
    ...
    uint256 checkt = (tokens / (1* 10**decimals));
        require(balances[owner] >= checkt, uintToString(checkt,false)) ;
...
//tokens = (tokens) / (10 ** decimals);
  ...
  function getRateOfToken() public pure
       returns(uint256 rate)
       {
          // uint256 eth = 1 ether ;
           uint256 tokenc = 10000  ;// 1:10,000 ratio
           rate =  tokenc;
