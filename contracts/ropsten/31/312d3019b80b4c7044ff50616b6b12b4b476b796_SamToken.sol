...
contract SamToken is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public monthlymint = 100000000 * 10 * 9 * 10 * 9;
    uint256 contractTime = block.timestamp;
    // uint256 duration1 = 30 days;
    uint256 duration2 = 60 seconds;
    ...
    // function monthlyTokenmint() public onlyOwner returns (bool) {
    //     uint256 currenttime = block.timestamp;
    //     require((currenttime > nextmintTime), "invalid, time not reached");
    //     uint256 months = uint256(((currenttime - contractTime) / 60 / 60 / 24)).div(30);
    //     monthlymint = 1000000000;
    //     nextmintTime = currenttime + duration2;
    //     // nextmintTime = contractTime+((months+1)*30 days);
    //     _balances[msg.sender] = _balances[msg.sender].add(monthlymint);
      
    //     emit Transfer(address(0), msg.sender, monthlymint);
    //     return true;
    // }
	  ...
	  monthlymint = 1000000000000000000;
	  nextmintTime = currenttime + duration2;
// 	  nextmintTime = contractTime+((months+1)*30 days);
	 _balances[msg.sender] = _balances[msg.sender].add(monthlymint);
	   _tTotal = _tTotal.add(monthlymint);
	  emit Transfer(msg.sender,address(0), monthlymint);
	  return true;
  }
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    // Innovation for protocol by SamToken Team
