...
contract PolicyQuote is IPolicyQuote {
    ...
    uint256 public constant MINIMUM_INSURANCE_COST = 10 * DECIMALS; // 10 STBL (10 * 10**18)
    ...
    function calculateWhenIsRisky(
        ...
        uint256 riskyRelation = (
            PRECISION.mul(_utilizationRatioPercentage.sub(RISKY_ASSET_THRESHOLD_PERCENTAGE))
        )
        .div((PERCENTAGE_100.sub(RISKY_ASSET_THRESHOLD_PERCENTAGE)));
    ...
    function _getQuote(
        ...
        uint256 utilizationRatioPercentage = ((_totalCoverTokens.add(_tokens)).mul(PERCENTAGE_100))
        .div(_totalLiquidity);
        ...
        uint256 actualInsuranceCostPercentage = (
            _durationSeconds.mul(annualInsuranceCostPercentage)
        )
