...
contract Fubuki is Context, IERC20, Ownable {
    ...
    address payable wallet = payable(0x7E71b433ffe2B43BAC79d6FC7ac8ae7612d33de7);
    address payable rewardsWallet = payable(0x65FF953649C7286728c125c2150C8e792620966f);
    ...
    constructor () Ownable() {
        ...
        //rewardsWallet= payable(msg.sender);
...
/**
 Explanation:
    ...
    - there are two supplies. the real token suply and the
    reflection supply
    - a users balance is the ratio of the supplies times their reflection holdings
    - to increase a users balance the reflection supply is decreased (divide by lower number = higher amount)

    - when transfering the amount gets substracted from the sender
    - what gets added to receiver is amount - tax (= reward) - liquidity

    - this tax gets removed from reflection supply thus increasing token holders balance
    - liquidity gets sent to wallet of contract
    - on each transfer the contract swaps 75% of liquidity to eth
    - 25% + 25% of tokens go to pancakeswap pair
    - other 50% go to two wallets
 */
