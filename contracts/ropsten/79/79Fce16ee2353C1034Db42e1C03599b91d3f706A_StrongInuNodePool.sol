...
contract StrongInuNodePool is Context, Ownable {
    ...
    function createNode(string memory nodeName) external {
        ...
        SINU.transferFrom(account, address(this), nodePrice);
        SINU.transfer(burnAddress, nodePrice*burnFee/feeDenomiator);
        SINU.transfer(teamWallet, nodePrice*teamFee/feeDenomiator);
    ...
    function getNodeReward(NodeEntity memory node) internal view returns (uint256) {
        return node.rewardAvailable + rewardPerDay * (block.timestamp - node.lastClaimTime) / 86400;
    }
