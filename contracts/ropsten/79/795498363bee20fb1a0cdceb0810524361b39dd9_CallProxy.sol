...
contract CallProxy is StorageSlots {
    ...
    */
    ...
    /*
      This is an explicit entry point to isValid(), that is then calls the implementation contract.
      This is a hack to add the isValid() to the contract ABI, to allow Etherscan (or alike)
      to discover isValid automatically, and provide access to it.
    */
        ...
        returns(bool)
    {
        ...
        (bool success, bytes memory returndata) = _implementation.staticcall(
            abi.encodeWithSignature("isValid(bytes32)",fact));
