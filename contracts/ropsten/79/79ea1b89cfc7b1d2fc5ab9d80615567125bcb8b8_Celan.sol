...
contract Celan {
    ...
    */ 
    ...
    function register(uint _referid,uint level)public isLock isContractcheck(msg.sender)  payable{
        require(users[msg.sender].isExist == false,"User already exist");
        require(level == 1,"wrong level given");
        ...
        _paylevel(userList[_referid],level,amount.div(referalUpline[level]));
        emit Registration(msg.sender,userList[_referid],level,msg.value,block.timestamp,directAddress);
    }
     /**
    * @dev buyLevel : User can move to next level
    */ 
  function buyLevel(uint _level)public isLock payable{
    ...
    function _paylevel(address referer,uint level,uint amount)internal{
        ...
        users[referer].referalEarnings =  users[referer].referalEarnings .add(amount);
        require(address(uint160(referer)).send(amount),"transfer level  failed");
        emit Referalcommission(referer,level,amount);
    ...
    */ 
    ...
    function viewUsers(address user,uint level)public view returns(address[]memory,bool){
        return (users[user].referals,
        users[user].activeLevel[level]);
    }
