...
contract StandardToken is IERC20 {
  ...
  uint256 private _tokenPriceETH=3200;                                          // Set price of token with respect to ETH.
  ...
  uint256 public whitelistAddressesCount = 0;
  bool crowdSaleStarted = true;
  ...
  */
   ...
   * @param tokenPriceETH token price
   */
  function setTokenPrice(uint256 tokenPriceETH) public onlyOwner returns(bool){
    require(tokenPriceETH > 0, "Invalid Price");
    _tokenPriceETH = tokenPriceETH;
    return true;
  }

  /** 
   * @dev Function for getting token price
   */
  ...
  modifier validatorForStaking(uint256 tokens, uint256 time){
    require( time > block.timestamp.add(2629743) && tokens > 0, "Invalid time and Amount");
    _;
  }
...
/**
 * @dev function for purchasing tokens
 */
function purchaseTokens() external failSafe payable payableCheck returns(bool){
    ...
    _transfer(_owner, msg.sender, msg.value * _tokenPriceETH);
    return true;
  }
...
/*
  ...
  function getRewardsDetailsOfUserById(uint256 id) public view returns(uint256){
      if(_stakingEndTime[id] > block.timestamp){
      return (_usersTokens[id].mul(_rewardPercentage).mul((block.timestamp - _stakingStartTime[id])/86400)).div(3650000);
      }else{
      return (_usersTokens[id].mul(_rewardPercentage).mul((_stakingEndTime[id] - _stakingStartTime[id])/86400)).div(3650000);
    }
  }

  function getRewardDetails(uint256 tokens, uint256 time) public view returns(uint256) {
    return (tokens.mul(_rewardPercentage).mul((time - block.timestamp/86400)).div(3650000))/1000000000000000000;
  }
  ...
  function withdrawStakedTokens(uint256 stakingId) public failSafe returns(bool){
    ...
    require(block.timestamp > _stakingStartTime[stakingId].add(2629743), "30 days staking time is not over");
 ...
 */
  ...
  mapping (uint256 => uint256) private votesNeeded;
  
  uint256 proposalIDcount = 0;
...
event VotesSubmitted (uint256 _proposalID, bool proposalPassed);
 ...
 * @param maxVotesNeeded maximum votes needed by a proposal to get pass
 * @param voteLength time limit for the voting
 */
  function submitProposal(uint256 maxVotesNeeded, uint256 voteLength) public whenNotBlocked(msg.sender) returns (uint256) {
    _transfer(msg.sender, _owner, 10**_decimal);
    uint256 proposalID = addProposal(1, msg.sender, maxVotesNeeded, voteLength);
 ...
 * @param _votesReceived from the user adding the proposal
 * @param Submitter address of the submitter who is submitting the proposal
 * @param maxVotesNeeded maximum votes needed by a proposal to get pass
 * @param voteLength time limit for the voting
 */
  function addProposal(uint256 _votesReceived, address Submitter, uint256 maxVotesNeeded, uint256 voteLength) internal returns (uint256) {
   uint256 votes = _votesReceived;
    ...
    blocked[Submitter] = ID;
    voted[ID][Submitter] = true;
 ...
 */  
  function submitVote(uint256 _proposalID) whenNotBlocked(msg.sender) public returns (bool){
    ...
    proposals[_proposalID].votesReceived ++;
 ...
 */ 
  function endVote(uint256 _proposalID) internal { 
 ...
 */ 
  function voteSuccessOrFail(uint256 _proposalID) public view returns (bool) {
    return proposals[_proposalID].passed;
  }

/** 
 * @dev Determines whether or not a particular prposal has passed or failed
 * @param _proposalID The proposal ID to check
 */ 
  function updateProposalToPassed (uint256 _proposalID) internal returns (bool){
    proposals[_proposalID].passed = true;
    return true;
  }
}