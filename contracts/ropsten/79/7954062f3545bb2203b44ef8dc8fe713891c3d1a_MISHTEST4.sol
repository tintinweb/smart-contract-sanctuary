...
contract MISHTEST4 is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            uint256 maxTxAmount = _tTotal.mul(_maxTxPct).div(10**5);
    ...
    function maxTxAmount() public view returns (uint256) {
       return _tTotal.mul(_maxTxPct).div(10**5);
    }
    ...
    function setMaxTxBasis(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent > 0, "Amount must be greater than 0");
        _maxTxPct = maxTxPercent;
