...
contract sLimelight is IsLMLT, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgLMLT public gLMLT; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked LMLT", "sLMLT", 9) ERC20Permit("Staked LMLT") {
    ...
    function setgLMLT(address _gLMLT) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gLMLT) == address(0), "gLMLT:  gLMLT already set");
        require(_gLMLT != address(0), "gLMLT:  gLMLT is not a valid contract");
        gLMLT = IgLMLT(_gLMLT);
    }
    ...
    /**
        @notice increases rLMLT supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sLMLT of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sLMLT
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sLMLT: insufficient balance");
    }
    ...
    // toG converts an sLMLT balance to gLMLT terms. gLMLT is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gLMLT.balanceTo(amount);
    }

    // fromG converts a gLMLT balance to sLMLT terms. sLMLT is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gLMLT.balanceFrom(amount);
    }

    // Staking contract holds excess sLMLT
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gLMLT.balanceFrom(IERC20(address(gLMLT)).totalSupply())).add(
...
interface IgLMLT is IERC20 {
  ...
  function migrate( address _staking, address _sLMLT ) external;
}
...
interface IsLMLT is IERC20 {
    function rebase( uint256 lmltProfit_, uint epoch_) external returns (uint256);
