...
contract EthanolX is Ownable, IERC20Metadata {
    ...
    function weekLyPay() external {
        require(block.timestamp >= (weeklyPayouts[_msgSender()] + 7 days), "EthanolX: ETH ");
        ...
        uint256 _userHolding = balanceOf(_msgSender());
        uint256 _percentage =  (_userHolding * 2) / 100;

        uint256 _amount = (weeklyEtherPayouts * _percentage) / 100;
    ...
    function _addLiquidity(uint256 tokenAmount) private {
        ...
        // if(IWETH(uniswapV2Router.WETH()).allowance(owner(), address(this)) >= amounts[1]) {
        //     revert("EthanolX: insufficient allowance from owner() address to EthanolX contract");
        // }
