...
contract ERC20 is Context, IERC20, Owned {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 lotto = (amount/100)*1;
        uint256 burn = (amount /100) *(burnRate);
        uint256 fees = (amount /100) *(feesRate);
        uint256 senderAmount = amount - (burn+fees+lotto);
        lottoPool += lotto;
        totalFees += fees;
        feesForEpochCurrent += fees;
        if (lottoPool > lottoPayout && stakers.length > 0){
            calculateLotto();
        }
    ...
    function unstakeTokens() public {
        ...
        require(totalLP > balance, "balance is greater than totalLP"); //these are just incase an unknown exploit is found, prevents a person from draining contract
        require(totalFees > share[msg.sender], "share is greater than totalFees");
        IERC20(LP).transferFrom(address(this),msg.sender, balance);
        IERC20(address(this)).transferFrom(address(this),msg.sender, share[msg.sender]);
