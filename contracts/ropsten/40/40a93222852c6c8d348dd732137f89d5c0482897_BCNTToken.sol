...
contract PrivateToken is StandardToken {
    ...
    /**
    * @dev Deposit is the function should only be called from OriginToken
    * When the user wants wants deposit their private Token to Origin Token. They should
    * let the origin Token invoke this function.
    * @param _depositor address. The person who wants to deposit.
    */

    function deposit(address _depositor) public onlyOriginToken returns(uint256){
      require(balances[_depositor] != 0);
      uint256 _value = balances[_depositor];
      balances[_depositor] = 0;
      return _value;
    }

    modifier onlyOriginToken() {
      require(msg.sender == originToken);
      _;
    }

    constructor(address admin, string _name, string _symbol, uint8 _decimals, uint256 _totalSupply) public{
      originToken = msg.sender;
      name = _name;
      symbol = _symbol;
      decimals = _decimals;
      totalSupply_ = _totalSupply;
      balances[admin] = _totalSupply;
      emit Transfer(address(0), originToken, _totalSupply);
    }
}
 ...
 contract LockToken is StandardToken, Ownable {
   ...
   */
   function deposit() public returns (bool){
     ...
     uint256 _value = privateToken.deposit(msg.sender);
   ...
   */
   function adminDeposit(address _depositor) public onlyOwner returns (bool){
     isDepositAllowed();
     uint256 _value = privateToken.deposit(_depositor);
     require(_value > 0);
   ...
   /**
   *  @dev Start Public sale and allow owner to deposit the token.
   *  normal users could deposit their tokens after the tokens unlocked
   */
   function startPublicSale(uint256 _unLockTime) public onlyOwner {
     require(!isPublic);
     isPublic = true;
    ...
    */
    constructor() public {
        ...
        privateToken = new PrivateToken(
          msg.sender, "Bincentive Private Token", "BCNP", decimals, INITIAL_SUPPLY
       );
    }
}