...
contract Tan is ERC721, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory shape = SHAPE_NAME[uint8(SHAPE_TYPE[uint8(getShapeForTanId(tanId))])];

        // tokenID defines generation (see TANS_PER_GENERATION)
        Generation generation = Generation.Gen1;// getGenerationForTanId(tanId);
        ...
        return string(abi.encodePacked("id: ", tokenId.toString(), ", shape: ", shape, ", generation: ", uint8(generation).toString(), ", rarity: ", rarity.toString(), ", description: ", rarityDescription.description, ", category: ", RARITY_CATEGORY_NAME[uint256(rarityDescription.category)]));
    }

    function getRarityForTanId(uint16 tanId) public view returns (uint8) {
        uint8 generation = uint8(getGenerationForTanId(tanId));
        require(generation > uint8(Generation.Gen1) && generation <= uint8(MAX_GENERATIONS), "Invalid tan ID");
