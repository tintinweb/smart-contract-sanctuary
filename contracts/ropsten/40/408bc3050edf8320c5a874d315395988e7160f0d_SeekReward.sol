...
contract SeekReward {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if (users[_addr].depositTime > 0) {
            users[_addr].cycle++;
            require(users[_addr].payouts >= this.maxPayoutOf(users[_addr].depositAmount), "SeekReward: Deposit already exists");
        ...
        if (upline != address(0)) {
            token.transfer(upline, _amount.mul(10e18).div(100e18));
            users[upline].referalBonus = users[upline].referalBonus.add(_amount.mul(10e18).div(100e18));
        }
        if (up != address(0)) {
            token.transfer(up, _amount.mul(5e18).div(100e18));
     ...
     * maximum payout based on(daily ROI, referral bonus, matchbonus)
    ...
    function payoutOf(address _addr) external view returns(uint256 payout, uint256 max_payout) {
        ...
        if (users[_addr].depositPayouts < max_payout) {
            payout = ((users[_addr].depositAmount.mul(1e18).div(100e18)).mul((block.timestamp.sub(users[_addr].depositTime)).div(1))).sub(users[_addr].depositPayouts);
     ...
     */
    function userInfo(address _addr) external view returns(address upline, uint40 deposit_time, uint256 deposit_amount, uint256 payouts, uint256 match_bonus) {
     ...
     */
    function userInfoTotals(address _addr) external view returns(uint256 referrals, uint256 total_deposits, uint256 total_structure) {
     ...
     */
    function contractInfo() external view returns(uint256 _total_users, uint256 _total_deposited, uint256 _total_withdraw) {
