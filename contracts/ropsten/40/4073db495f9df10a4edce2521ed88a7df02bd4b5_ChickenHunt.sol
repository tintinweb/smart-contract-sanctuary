...
contract CHStock is ERC20Interface {
  ...
  function _giveShares(address _user, uint256 _eth) internal {
    if (_eth > 0) {
      totalShares = totalShares.add(_eth);
      deduction[_user] = deduction[_user].add(dividendsPerShare.mul(_eth));
      shares[_user] = shares[_user].add(_eth);
      dividendsPerShare = dividendsPerShare.add(_eth.mul(correction) / totalShares);

      emit Transfer(address(0), _user, _eth);
    }
  }
...
contract CHAltar is CHArena {
  ...
  event NewAltarRecord(uint256 _id, uint256 _eth);
  event ChickenToAltar(address indexed _user, uint256 _id, uint256 _chicken);
  event EtherFromAltar(address indexed _user, uint256 _id, uint256 _eth);
  ...
  function chickenToAltar(uint256 _chicken) external {
    ...
    require(_altarRecord.eth * _chicken / _chicken == _altarRecord.eth);
    ...
    _altarRecord.total = _altarRecord.total.add(_chicken);
    ...
    )
  {
    ...
    _totalChicken = _altarRecord.total;
    _eth = _altarRecord.eth;
  ...
  function _resolveTradeBook(TradeBook storage _tradeBook) internal {
    if (_tradeBook.chicken > 0) {
      ...
      uint256 _eth = _oldAltarRecord.eth.mul(_tradeBook.chicken) / _oldAltarRecord.total;
      delete _tradeBook.chicken;
      ethBalance[msg.sender] = ethBalance[msg.sender].add(_eth);

      emit EtherFromAltar(msg.sender, _tradeBook.altarRecordId, _eth);
    }
  }
  ...
  function _getAltarRecord(uint256 _id) internal returns (AltarRecord storage _altarRecord) {
    ...
    if (_altarRecord.eth == 0) {
      uint256 _eth = altarFund / 10;
      _altarRecord.eth = _eth;
      altarFund -= _eth;
...
contract ChickenHunt is CHCommittee {
  ...
  function withdraw() external {
    uint256 _eth = ethBalance[msg.sender];
