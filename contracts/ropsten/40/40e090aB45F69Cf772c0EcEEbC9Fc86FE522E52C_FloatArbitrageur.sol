...
contract FloatArbitrageur is Arbitrageur, IAuction {
    ...
    address private dai = address(0xc2118d4d90b274016cB7a54c03EF52E6c537D957);
    ...
    function testTokenTransfers(address token, uint256 tokenAmount) public {
        ...
        approveUnlimitedTransfer(sushi, weth);
        ...
        approveTransfer(sushi, token, tokenAmount);
    ...
    function testTokenApproves() public {
        address[1] memory spenders = [sushi];
        address[2] memory tokens = [weth, dai];
        for (uint8 i = 0; i < 2; ++i) {
            for (uint8 j = 0; j < 3; ++j) {
                approveUnlimitedTransfer(spenders[i], tokens[j]);
                require(IERC20(tokens[j]).allowance(address(this), spenders[i]) == type(uint256).max);
            }
        }
    }
    ...
    function _sushiBuyToken(
        ...
        uint256 maxSlippage,
        uint256 deadline
    ) public returns (uint256 wethSpent) {
    ...
    function _sushiSellToken(
        ...
        uint256 maxSlippage,
        uint256 deadline
    ) public returns (uint256 wethEarned) {
