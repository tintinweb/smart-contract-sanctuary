...
contract Payments {
    ...
    modifier invoices(bytes32 id) {
        require(!details[id].active, "Given id already active.");
        _;
        emit Invoice(
            id,
            details[id].supplier,
...
contract TokenPayments is Ownable, Payments {
        ...
        cancels(id)
    {
        ...
        require(
            token.transfer(details[id].purchaser, refund),
            "Transfer to purchaser failed during cancel."
        );
        require(
            token.transfer(details[id].supplier, fee),
            "Transfer to supplier failed during cancel."
        );
    }
    ...
    {
        require(
            token.transfer(details[id].purchaser, getTotal(id)),
