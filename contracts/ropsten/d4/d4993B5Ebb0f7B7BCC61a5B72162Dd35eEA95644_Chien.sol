...
contract Chien is ERC721Token {
    ...
    function hach(string value) internal pure returns (string) {
        return bytes32string(sha256(abi.encodePacked(value)));
    }

    function rien() public view returns (string) {
        return "vive modez";
    }

    function zouze(string value) external view returns (int8[16384][3]) {
        int256 x = 64;
        int256 y = 64;
        int256 totalIndex = 0;
        int256 colorIndex = 0;
        ...
        bytes memory cha;
        int8[16384][3] memory result;
        for (uint256 i = 0; i < 151; i++) {
            tmp = hach(tmp);
            cha = bytes(tmp);
            for (uint256 iChar = 0; iChar < 64; iChar++) {
                if (cha[iChar] == 48 || cha[iChar] == 49) {
                    x += 1;
                } else if (cha[iChar] == 50 || cha[iChar] == 51) {
                    x += -1;
                } else if (cha[iChar] == 52 || cha[iChar] == 53) {
                    y += 1;
                } else if (cha[iChar] == 54 || cha[iChar] == 55) {
                    y += -1;
                } else if (cha[iChar] == 56 || cha[iChar] == 57) {
                    x += 1;
                    y += 1;
                } else if (cha[iChar] == 97 || cha[iChar] == 98) {
                    x += -1;
                    y += 1;
                } else if (cha[iChar] == 99 || cha[iChar] == 100) {
                    x += 1;
                    y += -1;
                } else if (cha[iChar] == 101 || cha[iChar] == 102) {
                    x += -1;
                    y += -1;
                }
