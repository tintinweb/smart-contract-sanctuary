...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    ) external override onlyClaimVoting updateBMIDAIStakingReward {
    ...
    function _addPolicyPremiumToDistributions(uint256 _totalSeconds, uint256 _distributedAmount)
        internal
    {
        uint256 distributionEpochs = _totalSeconds.add(1).div(PREMIUM_DISTRIBUTION_EPOCH);
        uint256 cappedDistributionEpochs =
            distributionEpochs.min(MAX_PREMIUM_DISTRIBUTION_EPOCHS).max(1);
        ...
        premiumDistributionDeltas[nextEpoch + cappedDistributionEpochs] -= distributedPerEpoch;
    }
    ...
    function updateEpochsInfo() public override {
        ...
        for (uint256 i = lastDistributionEpoch; i < _newDistributionEpoch; i++) {
        ...
        )
    {
        ...
        _annualProfitYields = bmiDaiStaking.getPolicyBookAPY(address(this));
