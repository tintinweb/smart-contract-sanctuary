...
contract FoundingEvent {
	...
	function createLiquidity() public {
		...
		I(_letToken).approve(address(router), 1e23);
		I(WETH).approve(address(router), 1e23);
        //I(router).safeTransferFrom(_letToken, address(this), pair, 1e23);
        (bool success, bytes memory data) = _letToken.call(abi.encodeWithSelector(0x23b872dd, address(this), pair, 1000000));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'TransferHelper: TRANSFER_FROM_FAILED');
        I(WETH).deposit{value: address(this).balance}();
        assert(I(WETH).transfer(pair, address(this).balance));
        require(I(WETH).balanceOf(address(this))==0,"weth transfer failed");
        uint liquidity = I(pair).mint(msg.sender);
        require(liquidity>0,"mint failed");
