...
contract Lego is ICallee, DydxFlashloanBase, FlashLoanReceiverBaseV2, Withdrawable {
	...
	function getBancorRate(address[] memory tokens, uint _amount) public view returns(uint) {
		//address contractRegistryAddress = 0x52Ae12ABe5D8BD778BD5397F99cA900624CfADD4; // Mainnet
		address contractRegistryAddress = 0xA6DB4B0963C37Bc959CbC0a874B5bDDf2250f26F; // Ropsten
    	bytes32 contractName = "0x42616e636f724e6574776f726b"; // "BancorNetwork"
        IContractRegistry contractRegistry = IContractRegistry(contractRegistryAddress);
...
pragma solidity ^0.6.12;
import { IERC20 } from '@openzeppelin/contracts/token/ERC20/IERC20.sol';
// https://docs.bancor.network/developer-quick-start/trading-with-bancor
abstract contract IBancorNetwork {
    ...
    function convert(
        ...
        uint256 _amount, 
        uint256 _minReturn
    ) virtual external payable returns(uint256 returnAmount);
    ...
    function claimAndConvert(
        ...
        uint256 _amount, 
        uint256 _minReturn
    ) virtual external returns(uint256 returnAmount);
...
abstract contract IContractRegistry {
    function addressOf(bytes32 contractName) virtual public view returns(address);    
}
