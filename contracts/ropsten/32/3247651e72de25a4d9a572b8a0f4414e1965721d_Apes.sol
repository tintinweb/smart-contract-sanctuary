...
contract Apes is ERC721Enumerable, Ownable {
    ...
    function Reclaim(uint256 numTokens) public payable ReclaimHasNotEnded {        
        if(msg.sender != owner()){
            require(isReclaimActive, "Sale is not active.");
        }
    ...
    function ReclaimAddyNAmount(address[] memory addytoreclaim, uint256[] memory amounttoreclaim) external onlyOwner() {
        for(uint i = 0; i < addytoreclaim.length; i++) {
            reclaimtotal[addytoreclaim[i]] = amounttoreclaim[i];
        }
    }
