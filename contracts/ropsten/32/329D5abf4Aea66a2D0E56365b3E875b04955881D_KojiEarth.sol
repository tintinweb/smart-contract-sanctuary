...
BNB reflections (of the 5% reflected in BNB):
    40% to holders
    15% to charity
    15% to WBNB side of pool
    15% to NFT rewards
    15% to admin
      (if staking is active)
        7.5% to admin
        7.5% to stake pool
...
launched june 1st on ethereum, now officially moved to ___
*/
    ...
    function shouldDistribute(address shareholder) internal view returns (bool) {
     ...
     function distributeDividend(address shareholder) public {
        ...
        if(amount > 0){
            ...
            uint256 netamount = amount.mul(999999999).div(1000000000); //this is so we aren't short on dust in the holding wallet
            ...
            shares[shareholder].totalExcluded = shares[shareholder].totalExcluded.add(netamount);
