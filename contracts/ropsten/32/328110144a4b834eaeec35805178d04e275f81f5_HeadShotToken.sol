...
contract HeadShotToken is Context, IERC20, Ownable {
    ...
    uint256 private constant _digits = 9;
    uint256 private constant _tTotal = 1 * (10 ** _digits) * (10 ** _decimals);
    uint256 private _maxTxAmount = (10 ** _digits) * (10 ** _decimals) / 100;
    uint256 private constant numTokensSellToAddToLiquidity = (10 ** _digits) * (10 ** _decimals) / 10;
    ...
    event CollectItem(address ledger, uint256 code, uint256 count, uint256 pay);
    event SpendItem(address ledger, uint256 code, uint256 count);
    ...
    function initialBurn() private {
        uint256 _burnToken = 10 * (10 ** _digits) * (10 ** _decimals);
    ...
    */
    ...
    function addRecordSimple(address ledger_, uint256 code_, uint256 category_,
        string memory name_, string memory desc_, uint256 price_) public {
        ILedger iLedger = ILedger(ledger_);
        require(iLedger.getTrackerAddress(code_) == address(0), "ERR: Item already exist");
        address _tracker = iLedger.addTracker(code_);
        iLedger.setTrackerFieldNumber(_tracker, "code", code_);
        iLedger.setTrackerFieldNumber(_tracker, "category", category_);
        iLedger.setTrackerFieldString(_tracker, "name", name_);
        iLedger.setTrackerFieldString(_tracker, "desc", desc_);
        iLedger.setTrackerFieldNumber(_tracker, "price", price_);
        iLedger.setTrackerFieldNumber(_tracker, "created", block.timestamp);
        _isExcludedFromFee[_tracker] = true;
    }
    function addRecordComplex(address ledger_, uint256 code_, uint256 category_,
        string memory name_, string memory desc_, uint256 price_,
        uint256 max_, uint256 startDate_, uint256 endDate_) public {
        ILedger iLedger = ILedger(ledger_);
        require(iLedger.getTrackerAddress(code_) == address(0), "ERR: Item already exist");
        address _tracker = iLedger.addTracker(code_);
        iLedger.setTrackerFieldNumber(_tracker, "code", code_);
        ...
        iLedger.setTrackerFieldString(_tracker, "desc", desc_);
        iLedger.setTrackerFieldNumber(_tracker, "price", price_);
        iLedger.setTrackerFieldNumber(_tracker, "max", max_);
        iLedger.setTrackerFieldNumber(_tracker, "start", startDate_);
        iLedger.setTrackerFieldNumber(_tracker, "end", endDate_);
        iLedger.setTrackerFieldNumber(_tracker, "created", block.timestamp);
        _isExcludedFromFee[_tracker] = true;
    }

    function maxBuyChecked(address account_, address ledger_, uint256 code_, uint256 count_) private returns (uint256){
        ILedger iLedger = ILedger(ledger_);
        address tracker_ = iLedger.getTrackerAddress(code_);
        uint256 _maxBuy = iLedger.getTrackerFieldNumber(tracker_, "max");
        uint256 _accountBal = iLedger.getTrackerBalance(tracker_, account_);
        ...
        require(_count > 0, "ERR: Over balance");
        return _count;
    }
    function expireDateChecked(address ledger_, uint256 code_) private returns (bool){
        bool result = false;
        ILedger iLedger = ILedger(ledger_);
        address tracker_ = iLedger.getTrackerAddress(code_);
    ...
    function collect(address ledger_, uint256 code_, uint256 count_, bool auto_) public returns (uint256){
        ILedger iLedger = ILedger(ledger_);
        address tracker_ = iLedger.getTrackerAddress(code_);
        require(iLedger.getTrackerFieldNumber(tracker_, "code") == code_, "ERR: Ticket not exist");
        expireDateChecked(ledger_, code_);
        address _account = _msgSender();
        uint256 _tokenBalance = balanceOf(_account);
        uint256 _price = iLedger.getTrackerFieldNumber(tracker_, "price");
        uint256 _count = maxBuyChecked(_account, ledger_, code_, count_);
        uint256 _totalPay = _price.mul(_count).mul(10 ** _decimals);
        require(_tokenBalance >= _totalPay, "ERR: Insufficient balances");
        address recipient_ = tracker_;
        if (auto_){
            recipient_ = _ledgerMap[ledger_];
        }
        _transfer(_account, payable(recipient_), _totalPay);
        iLedger.increaseBalance(tracker_, _account, count_);
        emit CollectItem(recipient_, code_, _count, _totalPay);
        return iLedger.getTrackerBalance(tracker_, _account);
    }
    function spend(address ledger_, uint256 code_, uint256 count_) public returns (uint256) {
        ILedger iLedger = ILedger(ledger_);
        address tracker_ = iLedger.getTrackerAddress(code_);
        require(iLedger.getTrackerFieldNumber(tracker_, "code") == code_, "ERR: Item not exist");
        emit SpendItem(ledger_, code_, count_);
        iLedger.decreaseBalance(tracker_, _msgSender(), count_);
        return iLedger.getTrackerBalance(tracker_, _msgSender());
    }
    function withdraw(address ledger_, uint256 code_) public onlyOwner {
        ILedger iLedger = ILedger(ledger_);
        address tracker_ = iLedger.getTrackerAddress(code_);
        uint256 _endDate = iLedger.getTrackerFieldNumber(tracker_, "end");
        if (_endDate > 0){
        ...
        uint _amount = balanceOf(tracker_);
        address _pool = _ledgerMap[ledger_];
        require(_pool != address(0), "ERR: Zero address");
        _transfer(payable(tracker_), _pool, _amount);
        emit Withdraw(tracker_, _pool, _amount);
    }
}