...
contract HardStakingNFTAuctionCudstodial is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    event NewAuction(uint indexed auctionId, uint indexed tokenId, uint indexed collectionId, bool isCustodial);
    ...
    constructor(
        ...
        //minAuctionStartPrice = _minAuctionStartPrice;
    ...
    function startNewAuction(uint tokenId, uint startBidAmount, bool isAmountInEth, string memory description, bool isCustodial) public {
        ...
        emit NewAuction(auctionId, auctionToken.tokenToCollection(tokenId), tokenId, isCustodial);
    }
    ...
    function _initAuction(uint auctionId) internal {
        uint auctionEnd = block.timestamp + tokenAuctionParams[auctions[auctionId].tokenId].auctionRoundDuration;
