...
contract TokenTeste is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 1_000 * 10**_decimals;
    
    uint256 public maxSellAmount = 281000000 * 10**_decimals;
    uint256 public maxBuyAmount = 281000000 * 10**_decimals;
    uint256 public initialMaxBuyAmount = 281000000 * 10**_decimals;
    uint256 public initialMaxSellAmount = 281000000 * 10**_decimals;
    uint256 public maxWalletBalance = 1500000 * 10**_decimals;
    
    // Anti Dump //
    mapping (address => uint256) public _lastSell;

    address public marketingAddress = 0x583dd101087608c68c5DEC86454506b004F127AA;
    address public drawAddress = 0x583dd101087608c68c5DEC86454506b004F127AA;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "TokenTeste";
    string private constant _symbol = "TokenTeste";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !swapping){
            require(tradingEnabled, "Os trade nao foi ativado ainda");
            if(to != pair){
                require(balanceOf(to) + amount <= maxWalletBalance, "Voce esta excedendo o valor maximo da carteira.");
            }
            if(from != pair){
                uint256 timePassed = block.timestamp - _lastSell[from];
                uint256 timeSinceStart = block.timestamp - startTime;
                if(timeSinceStart <= 12 hours){
                    require(timePassed >= 30 minutes, "Deve esperar 30 minutos entre os trades");
                }
                else if(timeSinceStart > 12 hours && timeSinceStart <= 24 hours){
                    require(timePassed >= 45 minutes, "Deve esperar 45 minutos entre os trades");
                }
                else{
                    require(timePassed >= 10 minutes, "Deve esperar 10 minutos entre os trades");
                }
    ...
    function rescueAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out the tokens");
