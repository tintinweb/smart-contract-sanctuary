...
contract ApGoldCoin is StandardToken {
    ...
    function setTradable(bool _tradeable) public {
    ...
    function crowdsaleTransfer(address _to, uint256 _value) public isOwner returns (bool) {
...
contract ApgCrowdSale {
    ...
    uint256 constant presaleEthToAPG   = 1200;
    uint256 constant crowdsaleEthToAPG =  750;
    uint256 constant ethAllocatedForUserConst =  15 ether;
    ...
    function _transferTokens(address recipient, uint256 eth, uint256 apg) private {
        require( token.crowdsaleTransfer( recipient, apg ) );
    ...
    function buyTokens(address recipient) public isRunning payable {
        ...
        require( p.ethAllocatedForUser >= msg.value );
        p.ethAllocatedForUser.sub( msg.value );
    ...
    function grantTokens(address recipient, uint256 apg) public isStarted isOwner {
        require( remAPG >=  apg);
        _transferTokens( recipient, 0, apg );
    }
    
    function grantPresaleTokens(address[] recipients, uint256[] eths) public isStarted isOwner {
