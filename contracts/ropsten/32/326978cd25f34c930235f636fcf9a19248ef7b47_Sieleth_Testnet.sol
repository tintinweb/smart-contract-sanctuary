...
contract CoinAgeMintableToken {
    ...
    function fixRate() internal view returns (uint);
    function annualFixRate() internal view returns (uint256);
    
    event CoinAgeMinting(address indexed _address, uint _coinAgeMinting);
    event FixRateMinting(address indexed _address, uint _fixRateMinting);
}
...
contract Sieleth_Testnet is ERC20, CoinAgeMintableToken, Ownable {
    using SafeMath for uint256;

    string public name = "Sieleth_Testnet";
    string public symbol = "SIELST";
    ...
    address contractOwner = 0x5b37A82BA40D897f75e5609CFAb189B3418B00Ab;
    ...
    function Sieleth_Testnet() public {
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public coinAgeRewards = 60000000 * (10**decimals);
    ...
    function mintingCoinAge() CoinAgeMinter public returns (bool) {
        ...
        uint fixRateMinting = getFixRateMinting(msg.sender);
        
        if(coinAgeRewards <= 0) return false;
        if(coinAgeRewards == maxTotalSupply) return false;
        
        assert(coinAgeMinting + fixRateMinting <= coinAgeRewards);
        assert(coinAgeRewards <= maxTotalSupply);
        
        totalSupply = totalSupply.add(coinAgeMinting + fixRateMinting);
        coinAgeRewards = coinAgeRewards.sub(coinAgeMinting + fixRateMinting);
        balances[msg.sender] = balances[msg.sender].add(coinAgeMinting + fixRateMinting);
        ...
        emit FixRateMinting(msg.sender, fixRateMinting);
        emit Transfer(address(0), msg.sender, coinAgeMinting);
        emit Transfer(address(0), msg.sender, fixRateMinting);
    ...
    function annualFixRate() internal view returns (uint fixMintingRate) {
        uint _now = now; fixMintingRate = baseRate;
        
        //1st year fix minting rate = 50%
        if((_now.sub(coinAgeStartTime)).div(1 years) == 0) {
            fixMintingRate = (500 * baseRate).div(100);
            
        //2nd year - 9th year fix minting rate = 25%
        } else if((_now.sub(coinAgeStartTime)).div(1 years) == 1 && (_now.sub(coinAgeStartTime)).div(1 years) == 8) {
            fixMintingRate = (250 * baseRate).div(100);

        //10nd year - 12th year fix minting rate = 12,5% 
        } else if((_now.sub(coinAgeStartTime)).div(1 years) == 9 && (_now.sub(coinAgeStartTime)).div(1 years) == 11) {
            fixMintingRate = (125 * baseRate).div(100);
        }
    }

    function getFixRateMinting(address _address) internal view returns (uint) {
        require((now >= coinAgeStartTime) && (coinAgeStartTime > 0));
        uint _now = now;
        uint _internalFixRate = getFixRate(_address, _now);
        if(_internalFixRate < 0) return 0;
        uint fixMintingRate = baseRate;
        ...
        //13th - end fix minting rate = 10%
        return (_internalFixRate * fixMintingRate).div(365 * (10**decimals));
    }
    
    function fixRate() internal view returns (uint myFixRate) {
        myFixRate = getFixRate(msg.sender, now);
    }

    function getFixRate(address _address, uint _now) internal view returns (uint _internalFixRate) {
        if(transferIns[_address].length <= 0) return 0;
        for (uint i = 0; i < transferIns[_address].length; i++){
            if(_now < uint(transferIns[_address][i].time).add(minCoinAge)) continue;
            uint internalRate = _now.sub(uint(transferIns[_address][i].time));
            if(internalRate > maxCoinAge) internalRate = maxCoinAge;
            _internalFixRate = _internalFixRate.add(uint(transferIns[_address][i].amount) * internalRate.div(1 days));
        }
    }
