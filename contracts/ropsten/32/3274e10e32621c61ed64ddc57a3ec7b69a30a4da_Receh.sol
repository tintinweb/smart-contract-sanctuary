...
contract Receh is ERC20, ProofOfAgeProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string public name = "Receh";
    string public symbol = "RECEH";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint256 valueAfterFee = value.sub(tokensFee);
...
//--------------------------------------------------------------------------------------
    ...
    function changeTokenTax(uint256 _tokenTax) public onlyOwner {
        tokenTax = _tokenTax;
...
//--------------------------------------------------------------------------------------
    ...
    mapping(address => bool) blockMinter;
    
    uint256 internal tokenPerBlock = 50 * (10**decimals);
    uint256 internal miningSupply = 21000000 * (10**decimals);
    
    uint256 internal currentBlockNumber;
    uint256 internal lastMintingBlockNumber;
    
    uint256 internal minBlockNumber = 100;
    
    event BlockMinting(address indexed to, uint256 time, uint256 blocknumber, uint256 _blockRewards);
    

    function blockMinting() public {
        uint256 blockTime = getBlockTime();
        uint256 blockRewards = getBlockReward();
        totalSupply = totalSupply.add(blockRewards);
        miningSupply = miningSupply.sub(blockRewards);
        balances[msg.sender] = balances[msg.sender].add(blockRewards);
        emit Transfer(address(0), msg.sender, blockRewards);
        emit BlockMinting(msg.sender, blockTime, lastMintingBlockNumber, blockRewards);
    }
    
    function getBlockTime() internal returns (uint blockTime){
        currentBlockNumber = block.number;
        lastMintingBlockNumber = currentBlockNumber;
        blockTime = currentBlockNumber.sub(lastMintingBlockNumber);
        return blockTime;
    }
    
    function getBlockReward() internal view returns (uint blockReward) {
        uint256 rewardPerBlock = tokenPerBlock;
        uint256 blockRewardReleases = currentBlockNumber.add(rewardPerBlock);
        blockReward = blockRewardReleases.mul(lastMintingBlockNumber);
        return blockReward;
    }
    
    function blockInfo() public view returns(address, uint256, uint256) {
        return (msg.sender, currentBlockNumber, lastMintingBlockNumber);
    }
    ...
    function setMiningSupply(uint256 value) public onlyOwner {
        miningSupply = miningSupply.add(value);
    }
    
    function blockMinterAdd(address account) public {
        blockMinter[account] = true;
    }
    
    function blockMinterRemove(address account) public onlyOwner {
        blockMinter[account] = false;
    }
    
    function isBlockMinter(address account) public view returns (bool) {
        return blockMinter[account];
    }
}