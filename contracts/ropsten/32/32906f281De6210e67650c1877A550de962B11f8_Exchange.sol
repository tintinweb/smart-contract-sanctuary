...
interface INFT {
    ...
    function mintWithURI(address to, string memory url) external returns (bool);
}
...
contract Exchange is OrderState, TransferSafe {
    ...
    uint256 public NFT_Buying_Amount = 0.5e18;
    
    // auth token for exchange
    mapping(address => bool) public isAllowed;
    
    constructor(address payable beneficiary, address payable buyerfeesigner, address wethAddr, uint beneficiaryfee,address _nft) public {
    ...
    function createNFT() external payable{
        require(msg.value > 0,"Invalid creating amount");
        require(msg.value == NFT_Buying_Amount,"Incorrect amount");
        require(address(uint160(owner())).send(msg.value));
         NFT.mint(msg.sender);
    }

    function createNFT_URI(string memory url) external payable{
        require(msg.value > 0,"Invalid creating amount");
        require(msg.value == NFT_Buying_Amount,"Incorrect amount");
        require(address(uint160(owner())).send(msg.value));
         NFT.mintWithURI(msg.sender,url);
    }
    ...
    function updateBuyingAmount(uint256 amount) public onlyOwner {
         NFT_Buying_Amount = amount;
    }
