...
contract HoneyX is Ownable, IERC20, IERC20Metadata {
    ...
    uint8 private _devIsInitialized3;
    ...
    mapping(uint8 => uint256[3][]) private _claimableAmountForDevByPosition;
    ...
    function _init2() public onlyOwner {
        require(_devIsInitialized2 == 1, "_devInit must be called first");
        for (uint8 a = 1; a <= 6; a++) {
            uint256 timestamp7 = 1209600;
    ...
    function claimToken() public {
        ...
        IERC20(this).transfer(_msgSender(), _amountClaimable(yourPosition));
    }
        ...
        returns (uint256)
    {
        ...
        for (
            ...
            if (
                ...
                amount += _claimableAmountForDevByPosition[devPosition][i][2];
            }
        }
        return amount;
    }
    ...
    function airdrop(address[] memory airdropAddresses) public onlyOwner {
        for (uint256 i = 0; i <= airdropAddresses.length; i++) {
            IERC20(this).transfer(
                airdropAddresses[i],
                _airdropTotal / airdropAddresses.length
            );
        }
    }
}