...
contract LostPunks is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Lost Punks", "LP") {
        // Rinkeby
    ...
    function verifyPunkOwnership(uint16 index) internal view {
        if (index < cryptoPunksCount) {
            ...
            require(ownerOf(index) == msg.sender, "Does not own this Lost Punk.");
        }
    }
    ...
    function tokenURI(uint256 index) public view override returns (string memory)
    {
        ...
        string memory json = base64Encode(bytes(string(abi.encodePacked(
            '{"name": "Lost Punk #',
            toString(index), 
            '", "description": "The Lost Punks are the next generation descendants of the original CryptoPunks. Fully generated on-chain. Inspired by but not affiliated with Larva Labs.", "image": "data:image/svg+xml;base64,', 
            base64Encode(bytes(punkImageSvg(uint16(index)))), 
            '"}'))));
