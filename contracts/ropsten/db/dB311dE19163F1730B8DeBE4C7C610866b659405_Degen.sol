...
abstract contract Router {
    ...
    function getBestQuote(address[] memory path, uint256 amountIn, OrderType orderType, uint256 disableFlags) public view returns (uint8, uint256,uint256[] memory) {
        ...
        uint8 temp = 0;
        for(uint8 i = 1; i < rates.length; i++) {
   ...
   modifier onlySystem() {
    ...
    function executeCrossExchange(
        ...
        if(path[0] == USDT) {
            ...
            uint256 minAmountExpected = _swap(orderType, path, assetInOffered, minExpectedAmount, msg.sender, address(this), dexId_deadline[0], distribution, dexId_deadline[1]);
    ...
    function executeCrossExchange(
        ...
        if(path[0] == USDT) {
            ...
            address tokenB = path[path.length-1];
            path[path.length-1] = USDT;
            uint256 minAmountExpected = _swap(orderType, path, assetInOffered, minExpectedAmount, msg.sender, address(this), dexId, distribution, block.timestamp + deadlineLimit);
