...
// File: contracts/utils/OperatorRole.sol
...
// File: contracts/utils/OwnableOperatorRole.sol
...
// File: contracts/utils/BytesLibrary.sol
...
// File: contracts/tokens/ERC1155/IERC1155.sol
...
// File: contracts/OrdersHolder.sol
...
contract OrdersHolder {
    ...
    function add(Domain.Order calldata order) external {
    ...
    function exists(Domain.Order calldata order) external view returns (bool) {
        bytes32 key = prepareKey(order);
    ...
    function prepareKey(Domain.Order memory order) internal pure returns (bytes32) {
...
// File: contracts/ContractState.sol
...
contract ContractState is OwnableOperatorRole {

    // keccak256(OrderKey) => completed
    mapping(bytes32 => uint256) public completed;

    function getCompleted(Domain.OrderKey calldata key) view external returns (uint256) {
        return completed[getCompletedKey(key)];
    }

    function setCompleted(Domain.OrderKey calldata key, uint256 newCompleted) external onlyOperator {
        completed[getCompletedKey(key)] = newCompleted;
    }

    function getCompletedKey(Domain.OrderKey memory key) pure public returns (bytes32) {
...
contract MintableToken is Ownable, IERC721, IERC721Metadata, ERC721Burnable, ERC721Base {
    ...
    constructor () public ERC721Base('NFTMaketplave', 'NFTM', 'https://example.com', '') {
        _registerInterface(bytes4(keccak256('MINT_WITH_ADDRESS')));
        transferOwnership(address(0x895EEcC6efD2B491da62126076EaCBE688BE855F));
    }
