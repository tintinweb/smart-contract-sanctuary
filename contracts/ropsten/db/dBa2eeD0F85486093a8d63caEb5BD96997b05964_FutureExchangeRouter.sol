...
contract FutureExchangeRouter is IFutureExchangeRouter{
    ...
    function withdrawLiquidityFuture(
        ...
        uint256 allowance = ILiquidityToken(liquidityToken).allowance(msg.sender, address(this));
        if (allowance < amountLiquidity) {
            ILiquidityToken(liquidityToken).approve(address(this), type(uint256).max);
        }
