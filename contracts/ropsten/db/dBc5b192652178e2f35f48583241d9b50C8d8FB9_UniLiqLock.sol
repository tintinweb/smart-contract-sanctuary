...
contract UniLiqLock is Ownable, ReentrancyGuard {
    ...
    function isLpTokenLocked(address _lpToken) external view returns (bool lockExists) {
        if (tokenLocks[_lpToken].length == 0) {
            return false;
        }else{
            return true;
        }
