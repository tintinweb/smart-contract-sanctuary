...
// new log with the previous strategy price
    ...
    event newStrat(bytes32 indexed name, uint32 indexed date, bytes32[2] underlying, uint64 price, int64[2] position);
    event newHiddenRecord(uint32 indexed date, bytes32 hashRecord);
    event newBiDiscloseRecord(uint32 indexed date, int64 positionOne, int64 positionTwo, bytes16 secretPath);
    ...
    constructor(bytes32 _name, address _priceFeeder, uint32 _date, bytes32[2] _underlying, int64[2] _position, uint64 _price) public{
        owner = msg.sender;
    ...
    function hashPosition(uint32 date, int64[2] position, bytes16 secretPath) public pure returns(bytes32) {
      return(sha256(abi.encodePacked(date,position[0],position[1],secretPath)));
    }
    ...
    function deleteHiddenPosition(uint32 date, int64[2] position, bytes16 secretPath) public onlyAuthorized {
        bytes32 recomputedHash = sha256(abi.encodePacked(date,position[0],position[1],secretPath));
        delete hiddenStates[recomputedHash];
    }

    function addExplicitPosition(uint32 date, int64[2] position) public onlyAuthorized {
      uint32 udlDateOne = feeder.getDate(underlying[0]);
      uint32 udlDateTwo = feeder.getDate(underlying[1]);

      // the currently stored underlying price must match
      require(udlDateOne == date);
      require(udlDateTwo == date);

      uint64[2] memory udlPreviousPrice = [feeder.getPreviousPrice(underlying[0]),feeder.getPreviousPrice(underlying[1])];
      uint64[2] memory udlPrice = [feeder.getPrice(underlying[0]),feeder.getPrice(underlying[1])];

      uint64 previousStrategyPrice = explicitState.price;
      uint64 newStratPrice = StrategyLib.biAdvance(date,udlPreviousPrice,udlPrice,position,previousStrategyPrice);
    ...
    function revealHiddenPosition(uint32 date, int64[2] position, bytes16 secretPath) public onlyAuthorized {
        ...
        emit newBiDiscloseRecord(date,position[0],position[1],secretPath);
