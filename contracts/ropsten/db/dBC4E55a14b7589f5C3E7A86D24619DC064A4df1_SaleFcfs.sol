...
contract SaleFcfs is IERC677Receiver, Ownable, ReentrancyGuard {
    ...
    // Total amount of offeringToken that will be offered to tiers
    uint public immutable offeringAmountTiers;
    ...
    constructor(
        ...
        offeringAmountTiers = _offeringAmountTiers;
    ...
    function getParams() external view returns (uint, uint, uint, uint, uint, uint, uint, bool, bool) {
        return (startTime, endTime, raisingAmount, offeringAmount, offeringAmountTiers, perUserCap, totalAmount, paused, finalized);
    }
    ...
    function _deposit(address user, uint amount) private nonReentrant {
        ...
        if (block.timestamp < startTime + ALLOCATION_DURATION) {
            ...
            require(totalAmount <= offeringAmountTiers, "reached phase 1 total cap");
        } else {
