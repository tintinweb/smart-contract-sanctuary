...
contract Hazel2 is ERC721Enumerable, ReentrancyGuard, Ownable{
	...
	function mintWithMLoot(uint256 mlootId, uint8 face, uint8 eyes, uint8 background) public payable nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
		require(_countTokenMLoot.current() < _supplyMLoot - _supplyWhitelist && _isWhitelistSaleActive, "SOLD_OUT");
		require(_countTokenMLoot.current() < _supplyMLoot && !_isWhitelistSaleActive, "SOLD_OUT");
		require(MINT_PRICE <= msg.value, "WRONG_ETHER_VALUE");
		_checkMLoot(mlootId, msg.sender);
		_checkTraits(face, eyes, background);
	...
	function multiMintWithMLoot(uint256[] calldata mlootIds, uint8[] calldata face, uint8[] calldata eyes, uint8[] calldata background) public payable nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
		require(_countTokenMLoot.current() + mlootIds.length < _supplyMLoot - _supplyWhitelist && _isWhitelistSaleActive, "MAX_TOKEN_EXCEED");
		require(_countTokenMLoot.current() + mlootIds.length < _supplyMLoot && !_isWhitelistSaleActive, "MAX_TOKEN_EXCEED");
