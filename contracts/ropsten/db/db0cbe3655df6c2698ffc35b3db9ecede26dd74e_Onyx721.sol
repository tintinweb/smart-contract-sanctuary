...
// File: contracts/Onyx721Royalties.sol


pragma solidity ^0.8.0;

contract OnyxRoyalties{
...
// File: contracts/Onyx721Base.sol
...
contract Onyx721 is ERC721, Ownable, ERC721URIStorage, OnyxRoyalties{
    ...
    function checkNftPrice(uint id) public view returns(uint){
        require(_exists(id), "Error! Token ID Does't Exist");
        return NFT_Price[id];
    }

    // If user has Minted token Localy and want to Add Price on Blockchain for First time
    function setNftPrice(uint id, uint NftPrice) public {
        require(_owners[id]==msg.sender, "Error! you're Not Owner of this NFT ");
        require(NFT_Price[id]==0, "Error! You can't Modify Price");
        NFT_Price[id]= NftPrice;
    }

    // In pending
    function setProvenanceHash(string memory ProvenanceHash_) public onlyOwner{
        provenanceHashValue = ProvenanceHash_;
    }

    /*  
    ** Token URI(Meta-Data) will be Genereated from IPFS 
    ** This Function should accept payment of NFT price and Send to Local Minter,
    ** NFT will be locally created the buyer will hit purchase button and NFT will be Minted to his address
    ** will fetch Amount & local Minter Address from DB
    ** Amount will be deducted from the wallet who is purchasing    
    */
    function MintTo(address payable to, uint tokenId, string memory TokenURI , address payable localMinter, uint Percentage, uint NftPrice) public payable { 
        ...
        //Should Add Royalties Functions here
        _royaltyAndOnyxFee(NftPrice, Percentage, localMinter, localMinter );
    }

    /* Direct Minting to Blockchain (Not Locally) 
    will Use Current wallet Address for Minting
    ** Directly Mint to his own account 
    ** Will Automatically Fetch Wallet Address and Deduct Amount 
    */
    function MintNow (uint tokenId, string memory tokenURI, uint minterPercentage, uint NftPrice) public {
        require(saleIsActive, "Error! Sale is Not Active");
        require(minterPercentage <= 50, "Error! Maximum Minting Percentage is 50% ");
        require(NftPrice>= 100000000000 wei, "Error! Low NFT Price ");
    ...
    function safeTransferFromOnyx(address payable from, address payable to, uint tokenId, bytes memory data) payable public {
    ...
    function transferFromOnyx(address payable from, address payable to, uint tokenId) payable public {
        _updateNftPrice( tokenId);
        RoyaltyInfo memory royalties = _royalties[tokenId];
        _royaltyAndOnyxFee(NFT_Price[tokenId], royalties.amount, royalties.recipient, payable(_owners[tokenId]));
        _transfer(from, to, tokenId);
    ...
    function BurnTokken(uint tokenID) public {
