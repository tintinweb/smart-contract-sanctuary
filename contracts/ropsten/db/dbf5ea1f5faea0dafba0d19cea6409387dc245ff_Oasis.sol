...
contract Oasis is ERC20Interface, Owned {
	...
	function getfromsun(address addr, uint money, uint amount) private returns(bool){
	    ...
	    if(f1 != address(0) && f2 != address(0) && f1 != f2) {
	    ...
	    if(f1 != address(0) && f2 != address(0) && f3 != address(0) && f2 != f3) {
	...
	function buy(uint keyid) onlySystemStart() public payable returns(bool) {
		...
		uint d = gettoday();
		uint t = getyestoday();
		address top1 = myidkeys[keyid];
		address top2 = fromaddr[top1];
		address top3 = fromaddr[top2];
		if(keyid > 0) {
		    if(mykeysid[user] != keyid && top1 != address(0)) {
		        if(fromaddr[user] == address(0)) {
		            fromaddr[user] = top1;
		            mysuns[top1].push(user);
    		        mysunsdaynum[top1][d]++;
		...
		if(daysgeteths[d] > daysgeteths[t]*subper/100 && daysgeteths[t] > 0) {
		    if(money == amount*4) {
    		    money = amount*3;
    		}else{
    		    money = amount*2;
		...
		uint sendmoney = 0;
		if(fromaddr[user] != address(0) && fromaddr[user] == top1) {
		    
		    if(mysuns[top1].length >= mans[0]) {
    	         sendmoney = (money*per/1000)*pers[0]/100;
        	     runs[top1] = runs[top1].add(sendmoney);
        	     addrunmoney(top1, sendmoney, 0);
        	     myprizedayget[top1][d] = myprizedayget[top1][d].add(amount);
        	     myprizedaygetdata[top1][d] = myprizedaygetdata[top1][d].add(amount);
        	        //setuserlevel(f1);
    	     }
    	     if(fromaddr[top1] != address(0) && fromaddr[top1] == top2) {
    	         sendmoney = (money*per/1000)*pers[1]/100;
        	     runs[top2] = runs[top2].add(sendmoney);
        	     addrunmoney(top2, sendmoney, 0);
        	     myprizedayget[top2][d] = myprizedayget[top2][d].add(amount);
        	     myprizedaygetdata[top2][d] = myprizedaygetdata[top2][d].add(amount);
        	     if(fromaddr[top2] != address(0) && fromaddr[top2] == top3) {
        	         sendmoney = (money*per/1000)*pers[2]/100;
            	     runs[top3] = runs[top3].add(sendmoney);
            	     addrunmoney(top3, sendmoney, 0);
            	     myprizedayget[top3][d] = myprizedayget[top3][d].add(amount);
            	     myprizedaygetdata[top3][d] = myprizedaygetdata[top3][d].add(amount);
        	     }
    	     }
		}
	    ...
	    if(now - timedata[timedata.length - 1] > lasttime && lastmoney > 0) {
	    ...
	    if(moneydata.length > 0) {
	        ...
	        //sendmoney = amount*pubper/100;
	        for(; start < moneydata.length; start++) {
	            //all += moneydata[i];
	            //address user = mansdata[ii];
	            uint m = (amount*pubper/100)*moneydata[start]/sendmoney;
	            eths[mansdata[start]] = eths[mansdata[start]].add(m);
	            addmoney(mansdata[start], m, 100);
	        }
