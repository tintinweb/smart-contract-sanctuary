...
// File: contracts/EvilNoundles.sol
...
contract NoundlesTheory is ERC721, Ownable {
    ...
    */
    function _handleRainbowMinting(address _to, uint256 index) private {
        ...
        if(ownerOf(evilTokenId) != msg.sender && MigrationMapToMap[evilTokenId] != 0 && BetaTheory.ownerOf(MigrationMapToMap[evilTokenId]) == ownerOf(evilTokenId)){
    ...
    function migrateOldNoundles(uint256[] memory tokens) public payable {
        ...
        for(uint256 i = 0; i < tokens.length; i += 1){
            ...
            // Mint it.
            _handleMinting(msg.sender, __noundles + offset, foundTypes[i]);
    ...
    */
    ...
    function setMigrationEnabled(bool _arg) external onlyOwner { migrationEnabled = _arg; }
    ...
    function setOldTheoryAddress(address _old) external onlyOwner { BetaTheory = InterfacePatched(_old); }
