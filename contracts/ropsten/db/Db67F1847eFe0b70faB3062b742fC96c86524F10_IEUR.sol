...
contract RestrictedToken is ERC20 {
    ...
    constructor(
        ...
        _allowAddresses(allowedAddresses_);
    }
    ...
    function _transfer(
        ...
        require(_isAllowed(sender), "KYC: sender not allowed to send");
        require(_isAllowed(recipient), "KYC: recipient not allowed to receive");

        super._transfer(sender, recipient, amount);
    }

    function allowAddresses(address[] memory addresses) external {
        _allowAddresses(addresses);
    }
    ...
    function blockAddresses(address[] memory addresses) external {
        _blockAddresses(addresses);
    }
    ...
    function _allowAddresses(address[] memory addresses) internal {
        for (uint256 i = 0; i < addresses.length; i++) {
            _allowAddress(addresses[i]);
        }
    }

    function _allowAddress(address addr) internal {
        _allowList[addr] = true;
    }

    function _blockAddresses(address[] memory addresses) internal {
        for (uint256 i = 0; i < addresses.length; i++) {
            _blockAddress(addresses[i]);
        }
    }
