...
contract Vault is IVault, AccessControlEnumerableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function retireStrategy(address strategy) external override whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (target.bonusToken() != address(0)) {
            ...
            if (bonusToken.balanceOf(address(this)) > 0 || target.bonusTokensAccrued() > 0) {
                try target.collectRewardToken() {} catch Error(string memory reason) {
                    emit Log(reason);
                }
            }
        }
