...
// File contracts/LicentaToken.sol

contract LicentaTokenV0 is Ownable, ERC721 {
    uint256 private _currentTokenId;

    mapping(string => bool) internal _existsURIMapping;
    ...
    constructor() ERC721("LicentaTokenV0", "LTK0") {
        _currentTokenId = 0;
    }

    function _existsURI(string memory URI) internal view returns (bool) {
        return _existsURIMapping[URI];
    }
        ...
        returns (uint256)
    {
        require(
            !_existsURI(URI),
            "URI already in use. Duplicates are not allowed"
        );
        uint256 nextId = incrementId();
        _tokenURIs[nextId] = URI;
        _existsURIMapping[URI] = true;
        _safeMint(to, nextId);
        console.log("Minting done for tokenId", nextId);
        return nextId;
    }
}