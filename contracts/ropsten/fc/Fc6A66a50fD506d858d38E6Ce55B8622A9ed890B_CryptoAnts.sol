...
contract CryptoAnts is ERC721, ICryptoAnts, ReentrancyGuard {
  ...
  // ^ in case owner wants to fill the contract so users can keep selling their ants
}
...
interface ICryptoAnts is IERC721 {
  ...
  event EggsBought(address ownerId, uint256 amount, uint256 unitPrice);
  ...
  function hatch(uint256 antId) external;

  function probOfDeath() external view returns (uint256);

  function sellAnt(uint256 antId) external;

  function updateEggPrice(uint256 price) external;

  function updateProbOfDeath(uint256 probability) external;
