...
contract Marketplace is Context {
    ...
    mapping(uint256 => bool) committeeProposalsExecuted;
    ...
    */
    ...
    function getCommitteeProposal(uint256 index) external view returns (address, // proposer,
        ...
        uint256[2] memory, // startTime, uint256, // endTime,
        bool[2] memory, // voteResultCalculated, voteResult,
        // bool, // 
        uint256 // votedAddressCount
        ){
        ...
        return (committeeProposals[index].proposer, 
            ...
            [committeeProposals[index].votingInfo.startTime, committeeProposals[index].votingInfo.endTime],
            [committeeProposals[index].votingInfo.voteResultCalculated,committeeProposals[index].votingInfo.voteResult],
    ...
    function setDelegate(address delegatee) external returns (bool) {
        delegation[_msgSender()] = delegatee;
        emit DelegationSet(_msgSender(), delegatee);
        return true;
    }
    ...
    function addCommitteeProposal(address[] calldata addCommitteeAddresses, address[] calldata removeCommitteeAddresses) external returns (bool) {
        ...
        require(!hasDuplicate(addCommitteeAddresses), "+ duplicates");
    ...
    function voteCommitteeProposal(uint256 proposalIndex, uint8 yesNoAbstain) external returns (bool) {
        ...
        emit VoteCasted("CommitteeProposal", proposalIndex, votedAddressID, _msgSender(), yesNoAbstain);
        return true;
    }
    ...
    // separating the two functions, so that we have some auditing time?
    // otherwise, if submission is true, run executeCommitteeProposal
    // potential problem when remove of a previous contain add of a later
    ...
    function executeCommitteeProposal(uint256 proposalIndex) external returns (bool) {
        ...
        // We don't need this, when vote result is true, calculated has to be true
        ...
        require(!committeeProposalsExecuted[proposalIndex], "Already executed");
        ...
        committeeCount = uint8(lengthAfterExecution);
        committeeProposalsExecuted[proposalIndex] = true;
    ...
    function getProposal(uint256 index) external view returns (address, // proposer,
        bytes32,// proposalHash;
        uint8, // proposalType; // 0 for MetaMine Fund Usage, 1 for Token Circulation
        //preapproval voting info
        uint256[2] memory, // votedAddressCount,         uint256 // votedAddressCount
        uint256[4] memory, // startTime,         // uint256, // endTime,
        bool[4]  memory//, // voteResultCalculated,         // bool, // voteResult,
        //general voting info
        // uint256[2] memory, // startTime,         // uint256, // endTime,
        // bool[2] memory // voteResultCalculated,         // bool, // voteResult,
        ){
    ...
    function committeePreapprovalVote(uint256 proposalIndex, bool yesOrNo) external returns (bool) {
        ...
        emit VoteCasted("ProposalCommitteePreapproval", proposalIndex, committeeVoterID, _msgSender(), yesOrNo ? 1 : 0);
        return true;
    }
    ...
    function voteProposal(uint256 proposalIndex, uint8 vote) external returns (bool) {
        ...
        emit VoteCasted("Proposal", proposalIndex, voterID, _msgSender(), vote);
        return true;
    }
