...
contract FlurryCollectRewardUpkeep is OwnableUpgradeable, IFlurryUpkeep {
    ...
    function checkUpkeep(bytes calldata) external override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        for (uint256 i = 0; i < vaults.length; i++) {
            uint256 arrLength = vaults[i].config().getStrategiesListLength();
        ...
        for (uint256 i = 0; i < vaults.length; i++) {
            ...
            for (uint16 j = 0; j < tempCollectList.length; j++) {
                if (tempCollectList[j] == true) {
                    ...
                    teaser[0] = j;
                    try vaults[i].config().collectStrategiesRewardTokenByIndex(teaser) returns (bool[] memory sold) {
                        if (!sold[0]) {
                            continue;
                        }
                        upkeepNeeded = true;
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        for (uint256 i = 0; i < vaults.length; i++) {
            ...
            vaults[i].config().collectStrategiesRewardTokenByIndex(data);
        }
    }
