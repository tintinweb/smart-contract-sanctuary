 ...
 * @title EkonGoldToken
 *
 * @dev ERC20 token backed by gold periodically audited reserve.
 */
contract EkonGoldToken is Pausable, StandardToken, BlackList {
  ...
  event Issue(uint amount); // Called when new token are issued
  event Redeem(uint amount); // Called when tokens are redeemed
  event Deprecate(address newAddress); // Called when contract is deprecated
  event Params(uint feeBasisPoints, uint maxFee); // Called if contract ever adds fees
