...
contract HeroAlliance is ERC721, Ownable, Validator {
    ...
    constructor(uint saleStartTime, uint initialSalePrice) ERC721("Hero Alliance", "HRS") {
        _setBaseURI("https://cryptoheroes.art/hero/id/");
        _saleStartTime = saleStartTime;
        _initialSalePrice = initialSalePrice;
    }
    ...
    function getCurrentPrice() public view returns (uint256) {
        ...
        if (currentSupply >= 9000) {
            return _initialSalePrice * 5;
        }  else if (currentSupply >= 8000) {
            return _initialSalePrice * 45 / 10;
        }  else if (currentSupply >= 7000) {
            return _initialSalePrice * 4;
        } else if (currentSupply >= 6000) {
            return _initialSalePrice * 35 / 10;
    ...
    function claimHero() external payable {
        ...
        (bool success,) = payable(owner()).call{ value: currentPrice * (100 - _floorPricePercentage) / 100 }("");
        require(success, "HeroAlliance: transfer fund to owner failed");
    }
    ...
    function changeHeroName(uint heroId, string memory newName) external onlyOwnerOf(heroId) {
        ...
        require(_validateName(newName) == true, "HeroAlliance: name is invalid");
