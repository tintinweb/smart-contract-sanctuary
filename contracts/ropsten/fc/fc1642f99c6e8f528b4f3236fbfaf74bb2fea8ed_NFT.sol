...
contract NFT is ERC1155, Ownable {
    ...
    function setBlindBox(uint256 _blindboxId, uint256[] memory _blindInfo, uint256[] memory _nftId , uint256[] memory _nftAmount) public  onlyOwner returns(bool){
        require(_blindMap[_blindboxId].isuse,"Blind box already exists");
        require(_blindInfo.length == 3,"Field error");
        ...
        _blindMap[_blindboxId].blindPrice = _blindInfo[0];
        _blindMap[_blindboxId].starSaleTime = _blindInfo[1];
        _blindMap[_blindboxId].endSaleTime = _blindInfo[2];
