...
contract GluonBinaryOptions {
    ...
    function addOption(string optionCode, uint durationInBlocks) isOwner() public returns(bool success){
        bytes32 id = keccak256(abi.encodePacked(optionCode));
        return addOption(id, optionCode, durationInBlocks);
    }   
    function addOption(bytes32 identifier, string optionCode, uint durationInBlocks)
        isOwner()
    ...
    function predict(bytes32 identifier, Result result) public
        isValidResult(result)
        payable returns(bool success) {
