...
// ----------------------------------------------------------------------------
// Easy Coin Swap Token contract
//
// Symbol                      : ECS
// Name                        : ECS Token
// Initial total supply        : 200,000,000.000000000000000000
// Decimals                    : 18
// Number to burn              : 100,000,000
// Co-founders total supply    : 15%
// ^Timelocked                 : 6 months (26 weeks)
// Company supply              : 5%
// ^Timelocked                 : 4 weeks
// Total distributed to public : 80%
// Author                      : Wesley Shillingford
//
// (Uses OpenZeppelin contracts for the majority of code)
// ----------------------------------------------------------------------------
...
contract ECSToken is ERC827Token, Ownable, BurnableToken {
  ...
  constructor() public {
    ...
    // TODO: Fill in with actual wallet addresses for co-founders and company
    ...
    // Create the time locked contracts (TODO: Remove testing values);
    //uint foundersLockedTime = now.add(26 weeks);
    //uint companyLockedTime = now.add(4 weeks);
    uint foundersLockedTime = now.add(5 minutes);
    uint companyLockedTime = now.add(4 minutes);

    // Emit the events, as cheaper to store them as logs than as state variables, and would pollute public API
    address[4] memory timelockAddresses;
  ...
  function setBalances(address[4] memory timelocks) private {
    uint share = totalSupply_ / 20;
    
    // 80% supply will be distributed during giveaway phase
    balances[msg.sender] = totalSupply_ - (share * timelocks.length);

    // Set 5% for all co-founders as well as for company use.
  ...
  // Ability to transfer out any accidentally sent ERC20 tokens
