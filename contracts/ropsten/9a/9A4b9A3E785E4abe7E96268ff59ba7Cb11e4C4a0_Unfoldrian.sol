...
contract Unfoldrian is UnfoldrianPricing, EIP712 {
    ...
    function _hash(address account, uint256[] memory tokenIds, uint256 expiry) internal view returns (bytes32)
    {
        return _hashTypedDataV4(keccak256(abi.encode(
        keccak256("Voucher(uint256[] tokenIds,address minter,address contract,uint256 expiry)"),
            tokenIds,
            account,
            address(this),
            expiry
    ...
    function mintWithN(
        ...
        require(_verify(signerAddress, _hash(recipient, tokens, expiry), signature), "UNFOLDRIAN:INVALID_SIGNATURE");
