...
contract Exchange is OrderState, TransferSafe {
    ...
    constructor (address payable beneficiary, address buyerfeesigner, uint256 beneficiaryfee,  address wethAddr) public {
    ...
    function buy(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, uint256 royaltyFee, address payable royaltyReceipt) payable external {
       require( ( block.timestamp<= order.expiryTime ), "signature expired" );
    ...
    function transferErc20Fee(address token, uint256 amount, address  _seller, address buyer,uint256 royaltyFee, address  royaltyReceipt ) internal{
        ...
        (uint256 protocalfee, uint256 secoundaryFee, uint256 remaining) = transferFeeView(amount, royaltyFee);
        if(protocalfee > 0){
            require(IERC20(token).transferFrom(buyer,beneficiaryAddress,protocalfee),"invalid");
        }
        if( (secoundaryFee > 0) && (royaltyReceipt!= address(0x00)) ){
                require(IERC20(token).transferFrom(buyer,royaltyReceipt,secoundaryFee),"invalid");
        }
        if(remaining > 0){
            require(IERC20(token).transferFrom(buyer,_seller,remaining),"invalid");
        }
    }
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        ShizukaniMintableStore = IShizukaniMintableStore(newMintableStore);
