...
contract TITANToken is Context, IERC20, Ownable {
    ...
    constructor(address tokenOwner) {
        ...
        _initialMaxTransferAmount = 2e18; // initial around  0.2 eth(2 TITAN)

        _mint(_tokenOwner, 100E18); // must be update later
    }
            ...
            if (sender != _vault && sender != _tokenOwner) {
                if (_initialMaxTransfers != 0) {
                    require(
                        ...
                        "Can't transfer more than 2 TITAN for initial 50 times."
                    );
