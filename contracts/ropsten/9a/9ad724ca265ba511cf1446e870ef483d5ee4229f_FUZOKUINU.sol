...
/**
FUZOKU INU
TELEGRAM : https://t.me/FuzokuInuEntry
WEBSITE :  http://www.FuzokuINU.io
*/
...
contract FUZOKUINU is Context, IERC20 {
    ...
    string private _name = "Fuzoku INU";
    string private _symbol = "FUZINU";
    ...
    uint256 public _liquidityRatio = 7;
    uint256 public _marketingRatio = 6;
    uint256 public _BBTRatio = 2;
    ...
    address payable private _marketingWallet = payable(0x23A98A9e3B4D1Da964C501C1D7D93a834E136a81);
    address payable private _BBteamWallet = payable(0x0b5024e231ABDC198bE56410dbb72A5F06D0fa6C);
...
//===============================================================================================================
    ...
    function setWallets(address payable marketingWallet, address payable BBteamWallet) external onlyOwner {
        _marketingWallet = payable(marketingWallet);
        _BBteamWallet = payable(BBteamWallet);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_liquidityRatio + _marketingRatio + _BBTRatio == 0)
            return;
        uint256 toLiquify = ((contractTokenBalance * _liquidityRatio) / (_liquidityRatio + _marketingRatio + _BBTRatio)) / 2;

        uint256 toSwapForEth = contractTokenBalance - toLiquify;
        swapTokensForEth(toSwapForEth);

        uint256 currentBalance = address(this).balance;
        uint256 liquidityBalance = ((currentBalance * _liquidityRatio) / (_liquidityRatio + _marketingRatio + _BBTRatio)) / 2;
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _marketingWallet.transfer(((currentBalance - liquidityBalance) * _marketingRatio) / (_marketingRatio + _BBTRatio));
            _BBteamWallet.transfer(address(this).balance);
        }
    }
