...
contract LostPunks is ERC721Enumerable, Ownable {
    ...
    function colorForGeneration(uint16 index) internal pure returns (bytes3) {
        uint24[9] memory colors = [
            0xFFFFFF, 0xFAD121, 0xA070DD,
            0xFA3C6A, 0xFD9DCB, 0xFC9144,
            0x39D27D, 0x9CB5FE, 0xDEC5A8];
        return bytes3(colors[index < colors.length ? index : colors.length - 1]);
    }
    ...
    string private constant SVG_FOOTER = '<style>rect{width:1px;height:1px;} #punk-svg{shape-rendering: crispedges;}</style></svg>';
    ...
    function punkAssetsImageSvg(bytes memory punkAssets, uint16 index) internal view returns (string memory svg) {
        ...
        bytes3 bgColor = colorForGeneration(generationMapping[index]);
        svg = string(abi.encodePacked(SVG_HEADER, rectSvg(0, 0, 24, 24, bgColor)));
        for (uint y = 0; y < 24; y++) {
            for (uint x = 0; x < 24; x++) {
                uint p = (y * 24 + x) * 4;
                if (uint8(pixels[p + 3]) > 0) {
                    bytes3 color = bytes3(
                        (uint24(uint8(pixels[p])) << 16) |
                        (uint24(uint8(pixels[p+1])) << 8) |
                        (uint24(uint8(pixels[p+2]))));
    ...
    function rectSvg(uint x, uint y, uint w, uint h, bytes3 color) internal pure returns (string memory) {
