...
contract REEPmining {
    ...
    event ChangeDefaultMiningRate(uint value);
}
...
contract Reep0x is ERC20, REEPminting, REEPmining, Ownable {
    using SafeMath for uint256;

    string public name = "Reep0x";
    string public symbol = "REEP0X";
...
//------------------------------------------------------------------------------------
    ...
    uint public miningRewards = 25000000 * (10**decimals);
    uint public miningStartTime;
    
    uint public defaultMiningRate = 10**17;
    uint public miningInterval = 365 days;
    
    uint public minimumDeposit = 1000 * (10**decimals);
    ...
    function getREEPmining(address account) internal view returns (uint) {
        ...
        uint miningRate = defaultMiningRate; {
            
        if((_now.sub(miningStartTime)).div(1 years) == 0) {
            miningRate = (500 * defaultMiningRate).div(100);
        } else if((_now.sub(mintingStartTime)).div(1 years) == 1) {
            miningRate = (500 * defaultMiningRate).div(100);
        } else if((_now.sub(mintingStartTime)).div(1 years) == 2) {
            miningRate = (500 * defaultMiningRate).div(100);
            ...
            miningRate = (250 * defaultMiningRate).div(100);
            ...
            miningRate = (125 * defaultMiningRate).div(100);
            ...
            miningRate = (100 * defaultMiningRate).div(100);
        ...
        uint coinMining = coinBase.mul(miningRate).mul(timeDiff).div(miningInterval);
        return coinMining;
    ...
    function changeDefaultMiningRate(uint256 _defaultMiningRate) public onlyOwner {
        defaultMiningRate = _defaultMiningRate;
        emit ChangeDefaultMiningRate(defaultMiningRate);
    }
    
    function setDefaultMiningRate(uint _defaultMiningRate) public onlyOwner {
        defaultMiningRate = _defaultMiningRate;
    }
