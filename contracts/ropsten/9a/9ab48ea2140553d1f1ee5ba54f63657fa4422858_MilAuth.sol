...
contract MilAuth is MilAuthInterface {
    ...
    mapping (address => bool) private closedGames_;            // (addr => closed) returns game closed or not
    ...
    constructor()
        public
    {
        address admin1  = 0x9757B9051175CAc6fC8F59F84Ff272A4075f51b3;
        address admin2  = 0xC4ff23dAF771eb8855A180A4f123475e65528CDa;
        address admin3  = 0x5294943c2AA15Cd8F2Dc0D0Deab3F5658c070978;

        admins_[admin1] = Admin(true, true, "admin1");      // admin
        admins_[admin2] = Admin(true, true, "admin2");      // admin
        admins_[admin3] = Admin(true, true, "admin3");      // admin

        adminCount_ = 3;
        devCount_ = 3;

        requiredSignatures_ = 3;
        requiredDevSignatures_ = 3;
    }
    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    // FALLBACK
    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    function registerGame(address _gameAddr)
        ...
        onlyDevs
    {
        require(address(_gameAddr) != 0, "gameAddr is invalid");

        if (MSFun.multiSig(msData, requiredDevSignatures_, "registerGame") == true)
        {
            MSFun.deleteProposal(msData, "registerGame");
        ...
        onlyDevs
    {
        ...
        if (MSFun.multiSig(msData, requiredDevSignatures_, "closeGame") == true)
        {
            MSFun.deleteProposal(msData, "closeGame");

            closedGames_[_gameAddr] = true;
        }
    }
    ...
    */
    ...
    function checkGameRegiester(address _gameAddr) external view returns(bool) {return(registeredGames_[_gameAddr]);}
    function checkGameClosed(address _gameAddr) external view returns(bool) {return(closedGames_[_gameAddr]);}
}
