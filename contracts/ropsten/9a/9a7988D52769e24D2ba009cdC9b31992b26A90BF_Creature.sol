...
interface ICreature is IERC721Enumerable {
    ...
    function addBankerInfo(uint256 _num, uint8 _gen) external;

    function addRebelInfo(uint256 _num, uint8 _tenureScore) external;

    function getBankerInfo(uint256 _num) external view returns (uint8);

    function getRebelInfo(uint256 _num) external view returns (uint8);
