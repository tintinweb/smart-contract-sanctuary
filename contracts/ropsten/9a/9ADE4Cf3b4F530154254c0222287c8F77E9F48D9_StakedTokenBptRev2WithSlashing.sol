 ...
 * @author Aave (based on minimal changes of feat/slash, don't use as it is)
 **/
contract StakedTokenBptRev2WithSlashing is
  IStakedAave,
  ...
  IStakedTokenV3
{
  ...
  function stake(address onBehalfOf, uint256 amount) external override(IStakedToken, IStakedAave) {
   ...
   **/
  function redeem(address to, uint256 amount) external override(IStakedToken, IStakedAave) {
    require(amount != 0, 'INVALID_ZERO_AMOUNT');
    //solium-disable-next-line
   ...
   **/
  function cooldown() external override(IStakedToken, IStakedAave) {
   ...
   **/
  function claimRewards(address to, uint256 amount) external override(IStakedToken, IStakedAave) {
