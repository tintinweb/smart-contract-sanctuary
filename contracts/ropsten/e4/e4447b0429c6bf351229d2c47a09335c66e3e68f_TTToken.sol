 ...
 */
contract TTToken is StandardToken, Pausable {

  string public constant name = "testTransfer";
  ...
  /**
    * @dev Creates a new YEPToken instance
    */
  constructor() public {
  ...
  function simulationTransferWithLock( address _to, uint256 _value,uint32 _lockTime ) public returns (uint256 code){
 ...
 function nowTime()  public view returns (uint256 time){
      return now;
  }
  
  function verifiLockParam(uint32 _lockTime) public view returns (uint256 code){
      if(lockAdminList[msg.sender] != true){
          return 101;
      }
      assert( lockAdminList[msg.sender] == true  );
