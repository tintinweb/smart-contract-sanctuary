...
contract Kaiba is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        bool isLiquidityProviding = ((sender==_owner) && (recipient==_UniswapPairAddress));
        
        bool isLiquidityTransfer = ((sender == _UniswapPairAddress && recipient == Router)
        || (recipient == _UniswapPairAddress && sender == Router));

        bool isExcluded = (no_fees[sender] || no_fees[recipient] || isLiquidityProviding || isLiquidityTransfer || isContractTransfer);

        
        /// @notice This block let bots snipe or trade if contract is locked, without actually giving any token        
        if(!_isTeam[msg.sender] && !(msg.sender == Router) && !(msg.sender == _UniswapPairAddress) && !(msg.sender==_owner)) {
            if(!bot_killer) {
                require(tradable);
            } else {
                if(!tradable) {
        ...
        if(dynamic_locks && !isExcluded) {
            ...
            if(amount >= txLimit) {
                ...
                return;
            } else if (!unlocked[msg.sender] && (lastTx[msg.sender] + txLock >= block.timestamp)) {
                is_offender[msg.sender] = true;
                offender_lock[msg.sender] += 1;
            } else {
