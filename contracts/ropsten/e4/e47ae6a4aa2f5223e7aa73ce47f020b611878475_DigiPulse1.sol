...
contract DigiPulse1 {
  ...
  uint constant tokenSupply = 20000 * 1e8;
  uint8 constant dgtRatioToEth = 250;
  uint constant raisedInPresale = 5000;
  mapping (address => uint256) ethBalanceOf;
  address owner;

  // For LIVE
  uint constant startOfIco = 1533124800; // Wednesday, 01-Aug-18 12:00:00 UTC
  uint constant endOfIco = 1533124800; // Wednesday, 01-Aug-18 12:00:00 UTC
  ...
  // to account which will send tokens manually out
    ...
    balanceOf[0xFaC7e32Ba28D0e9C600ECd1b3BFD965BEE34c0FF] += raisedInPresale;
    allocatedSupply += raisedInPresale;
    Transfer(0, 0xFaC7e32Ba28D0e9C600ECd1b3BFD965BEE34c0FF, raisedInPresale);
	}
    ...
    uint256 bountyAmount = allocatedSupply * 100 / 98 * 2 / 100;
		balanceOf[0xd8826D5c1CA458fC0530c5Fe067A71a920BE18A5] += bountyAmount;
    allocatedSupply += bountyAmount;
    Transfer(0, 0xd8826D5c1CA458fC0530c5Fe067A71a920BE18A5, bountyAmount);
	}
}