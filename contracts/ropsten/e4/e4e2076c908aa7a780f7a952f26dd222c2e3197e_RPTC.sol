 ...
 * @title RPTC token
 **/
 contract RPTC is StandardBurnableToken, PausableToken {
     
    using SafeMath for uint256;
    string public constant name = "RPTC TEST Token";
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        if (lockedUserEntity[owner].length > 0) {
            for (uint i = 0; i < lockedUserEntity[owner].length; i++) {
                if (lockedUserEntity[owner][i]._releaseTime <= now) {
                    totalBalance = totalBalance.add(lockedUserEntity[owner]
[i]._amount);
                }
            }
        }
    ...
    function balanceOfTotal(address owner) public view returns 
(uint256) {
        uint256 totalBalance = super.balanceOf(owner);
        if(lockedUserEntity[owner].length >0 ){
            for(uint i=0; i< lockedUserEntity[owner].length;i++){
...
[i]._amount);
    ...
    function balanceOfLocked(address owner) public view returns 
(uint256) {
        uint256 lockedBalance; 
        if(lockedUserEntity[owner].length >0 ){
            for(uint i=0; i< lockedUserEntity[owner].length;i++){
                lockedBalance = lockedBalance.add(lockedUserEntity[owner]
[i]._amount);
            }
        }
    ...
    /* Seperate balanceOf function code
    function balanceOf(address owner) public view returns (uint256) {
