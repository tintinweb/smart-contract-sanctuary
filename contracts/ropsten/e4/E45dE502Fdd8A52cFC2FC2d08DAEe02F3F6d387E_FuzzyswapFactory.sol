...
library Oracle {
    ...
    function _averageTick(
        Observation[65535] storage self,
        uint32 time,
        int24 tick,
        uint16 index,
        uint128 liquidity
    ) private view returns (int24 average){
        ...
        if (lte(time, oldest.blockTimestamp, time - TIME_AGO)){
                if (last.blockTimestamp + TIME_AGO > time){
                    ...
                    average = int24((last.tickCumulative - b) / (TIME_AGO + last.blockTimestamp - time));
                }
                else {
                    index = index == 0 ? 65535 - 1 : index - 1;
                    average = self[index].initialized ?
                        int24(
                ...
                average = (last.blockTimestamp == oldest.blockTimestamp) ?
                    tick :
                    int24(
    ...
    function getSurroundingObservations(
        ...
        if (lte(time, beforeOrAt.blockTimestamp, target)) {
            if (beforeOrAt.blockTimestamp == target) {
                ...
                return (beforeOrAt, transform(beforeOrAt, target, tick, liquidity, _averageTick(self, time, tick, index, liquidity)));
            }
        }
    ...
    function observeSingle(
        ...
        if (secondsAgo == 0) {
            ...
            if (last.blockTimestamp != time) last = transform(last, time, tick, liquidity, _averageTick(self, time, tick, index, liquidity));
    ...
    /// @return indexUpdated The new index of the most recently written element in the oracle array
     function write(
        ...
        self[indexUpdated] = transform(last, blockTimestamp, tick, liquidity, _averageTick(self, blockTimestamp, tick, index, liquidity));
    }
}
