...
contract NWar is NPass {
     ...
     * @param tokenId Id to be minted
     */
    function mintWithN(uint256 tokenId, uint256 updatedPriceForNHoldersInWei) public payable virtual nonReentrant {
        require(
            // If no reserved allowance we respect total supply contraint
            (reservedAllowance == 0 && totalSupply() < maxTotalSupply) || reserveMinted < reservedAllowance,
            "NPass:MAX_ALLOCATION_REACHED"
        ...
        require(n.ownerOf(tokenId) == msg.sender, "NPass:INVALID_OWNER");
        require(msg.value == updatedPriceForNHoldersInWei, "NPass:INVALID_PRICE");
    ...
    function mintMultipleWithN(uint256[] memory tokenIds) public payable virtual nonReentrant {
        uint256 price = tokenIds.length * priceForNHoldersInWei;
        for (uint256 tokenId = 0; tokenId < tokenIds.length; tokenId++) {
            mintWithN(tokenIds[tokenId], price);
        }
    }
}
