...
contract Oasis{
	...
	function indexshow2(address user) public view returns(/*uint totaleths,uint lttime,uint ltmoney,address ltaddr,uint myeths,uint mycans,*/uint dayuserget,uint dayusereth,uint keyprices,uint mykeynum,uint ltkeynum,uint mykeyid){
	    /*
	    //address user = msg.sender;
	    totaleths = userethnum[tags];//0
	    lttime = timedata[timedata.length - 1];//1
