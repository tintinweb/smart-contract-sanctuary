...
interface iHatchlingz {
    ...
    function _hatchTime(uint256 hatchTime) external view returns (uint256);
...
contract Yolk is ERC20, Ownable {
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return (Hatchlingz._walletBalanceOfChicken(user) * CHICKEN_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400) +
        (Hatchlingz._walletBalanceOfDragon(user) * DRAGON_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400) +
        (Hatchlingz._walletBalanceOfPhoenix(user) * PHOENIX_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400);
    }
