...
contract DelegateContract is Ownable, IERC721Receiver
{
   ...
   function instantBuy(address _erc20TokenContract, address _nftTokenAddress, TokenType _tokenType, uint256 _tokenId, bytes memory _data) public 
   {
       address _buyer =  msg.sender;
       require(msg.sender == _buyer || msg.sender == authorizedCaller, "Not authorized buyer");
       SaleItem memory saleItem = instantBuyDetails[_nftTokenAddress][_tokenId];
       require(saleItem.isOpen && saleItem.erc20TokenAddress == _erc20TokenContract, "Order is not Open or erc20 token addresses dont match.");
       transferERC20(saleItem.erc20TokenAddress, msg.sender, saleItem.seller, saleItem.price);
       if(_tokenType == TokenType.ERC721){
            IERC721 _erc721Contract = IERC721(_nftTokenAddress);
            _erc721Contract.safeTransferFrom( saleItem.seller, msg.sender, _tokenId);
            emit TransactionCompleted(_erc20TokenContract, _nftTokenAddress, saleItem.seller, msg.sender, saleItem.price, _tokenId, 1, PricingType.INSTANT_BUY);
       }
       else if(_tokenType == TokenType.ERC1155)
       {
           ...
           emit TransactionCompleted(_erc20TokenContract, _nftTokenAddress, saleItem.seller, msg.sender, saleItem.price, _tokenId, saleItem.quantity, PricingType.INSTANT_BUY);
       }
       else{
   ...
   function OfferForSale(address _erc20TokenAddress, address _nftTokenAddress, uint256 _tokenId, uint256 _quantity, TokenType _tokenType, uint256 _startingPrice, string memory _userId, PricingType _pricingType, uint256 _startTime, uint256 _endTime) public
   {
       ...
       require(!bid.isOpen, "Bid for the token is currently active");
   ...
   function placeBid(address _erc20TokenAddress, address _nftTokenAddress, uint256 _currentBid, uint256  _tokenId, string memory _userId) public 
   {
       Bid memory bid = bidDetails[_nftTokenAddress][_tokenId];
       address _previousBidder = bid.highestBidder;
       uint256 _previousBid = bid.highestBid;
       
       require(_erc20TokenAddress == bid.erc20TokenAddress, "Unsupported ERC20 token for this bid");
       require(_currentBid > bid.highestBid);
       
       IERC20 erc20Contract = IERC20(_erc20TokenAddress);
       erc20Contract.transferFrom(msg.sender, address(this), _currentBid);

       if(bid.highestBidder != address(0))
       {
           erc20Contract.transferFrom(address(this), _previousBidder, _previousBid);
       }
       bid.highestBidder = msg.sender;
       bid.highestBid = _currentBid;
       bidDetails[_nftTokenAddress][_tokenId] = bid;
       emit BidPlaced(_erc20TokenAddress, _nftTokenAddress, _previousBidder, msg.sender, _previousBid, _currentBid, _tokenId, _userId);
   }
   ...
   function closeBid(address _nftTokenAddress, uint256 _tokenId, TokenType _tokenType, bytes memory _data) public 
   {
      ...
      if (_tokenType == TokenType.ERC721)
      {
          transferERC721(_nftTokenAddress, bid.creator, bid.highestBidder, _tokenId);
      }
      else if (_tokenType == TokenType.ERC1155)
      {
          transferERC1155(_nftTokenAddress, bid.creator, bid.highestBidder, _tokenId, bid.quantity, _data);
      }else
      {
          revert("Token type must be specified");
      }
      
      bid = Bid(address(0), 0, false, 0, address(0), address(0), PricingType.Undefined, 0, 0);
