...
contract SFT is Context, Ownable, ISFT, ISFTMetadata, ERC721 {
    using Address for address;
    using Strings for uint256;
    ...
    /* @dev Returns whether `operator` is the contract owner */
    function _isOwner(address operator) internal view virtual returns (bool) {
     ...
     */
    function _isSemiTypeMintApproved(address) internal view virtual returns (bool) {
        return true;
    }
    ...
    function semiTypeMint(address _to, uint256 _tokenType, uint256 _value, bytes memory _data) public virtual override {
        ...
        require(_isSemiTypeMintApproved(_operator), "SFT: semiTypeMint caller is not owner nor approved");
    ...
    function _semiMint(address _owner, address _operator, address _to,
                       uint256 _tokenType, bytes memory _data) internal virtual {
    ...
    function semiMint(
        ...
        _semiMint(_from, _msgSender(), _to, _tokenType, _data);
    }
