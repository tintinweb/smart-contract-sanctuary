...
// File: contracts/ICrc.sol

/*
Crc interface

This defines the core Crc contract interface as an extension to the
 ...
 * @title Crc interface
 */
abstract contract ICrc is IStaking, Ownable {
    // events
    ...
    event CrctSpent(address indexed user, uint256 amount);
    event CrctWithdrawn(uint256 amount);
    ...
    // ICrc
    /**
     * @return staking token for this Crc
     */
    function stakingToken() external virtual view returns (address);

    /**
     * @return reward token for this Crc
     */
    function rewardToken() external virtual view returns (address);

    /**
     * @notice fund Crc by locking up reward tokens for distribution
    ...
    /**
     * @notice fund Crc by locking up reward tokens for future distribution
     * @param amount number of reward tokens to lock up as funding
     * @param duration period (seconds) over which funding will be unlocked
    ...
    /**
     * @notice withdraw CRCT tokens applied during unstaking
     * @param amount number of CRCT to withdraw
     */
    function withdraw(uint256 amount) external virtual;

    /**
     * @notice unstake while applying CRCT token for boosted rewards
     * @param amount number of tokens to unstake
     * @param crct number of CRCT tokens to apply for boost
     */
    function unstake(
        uint256 amount,
        uint256 crct,
    ...
    /**
     * @notice clean crc, expire old fundings, etc.
     */
    function clean() external virtual;
}


// File: contracts/CrcPool.sol

/*
Crc token pool

Simple contract to implement token pool of arbitrary ERC20 token.
This is owned and used by a parent Crc

*/

pragma solidity ^0.6.12;

contract CrcPool is Ownable {
...
other contracts and specifically the CRCT multiplier calculation
...
This implements the core Crc contract, which allows for generalized
staking, yield farming, and token distribution. This also implements
the CRCT spending mechanic for boosted reward distribution.
 ...
 * @title Crc
 */
contract Crc is ICrc, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    // token pool fields
    CrcPool private immutable _stakingPool;
    CrcPool private immutable _unlockedPool;
    CrcPool private immutable _lockedPool;
    ...
    uint256 public totalCrctRewards;
    uint256 public totalStakingShareSeconds;
    uint256 public lastUpdated;

    // crct fields
    IERC20 private immutable _crct;
     ...
     * @param crct_ address for CRCT token
     */
    constructor(
        ...
        require(
            bonusMin_ <= bonusMax_,
            "Crc: initial time bonus greater than max"
        );

        _stakingPool = new CrcPool(stakingToken_);
        _unlockedPool = new CrcPool(rewardToken_);
        _lockedPool = new CrcPool(rewardToken_);
    ...
    function fund(
        ...
        // validate
        require(amount > 0, "Crc: funding amount is zero");
        require(start >= block.timestamp, "Crc: funding start is past");
        require(
            fundings.length < MAX_ACTIVE_FUNDINGS,
            "Crc: exceeds max active funding schedules"
        );
    ...
    function withdraw(uint256 amount) external override onlyOwner {
        require(amount > 0, "Crc: withdraw amount is zero");
        require(
            amount <= _crct.balanceOf(address(this)),
            "Crc: withdraw amount exceeds balance"
        );
        // do transfer
        _crct.safeTransfer(msg.sender, amount);

        emit CrctWithdrawn(amount);
    }
    ...
    function _stake(
        ...
        // validate
        require(amount > 0, "Crc: stake amount is zero");
        require(
            beneficiary != address(0),
            "Crc: beneficiary is zero address"
        );
        ...
        uint256 mintedStakingShares = (totalStakingShares > 0)
            ...
            : amount.mul(INITIAL_SHARES_PER_TOKEN);
        require(mintedStakingShares > 0, "Crc: stake amount too small");
     ...
     * @param amount number of tokens to unstake
     * @param crct number of CRCT tokens applied to unstaking operation
     * @return number of reward tokens distributed
     */
    function _unstake(uint256 amount, uint256 crct)
        private
        nonReentrant
        returns (uint256)
    {
        // validate
        require(amount > 0, "Crc: unstake amount is zero");
        require(
            totalStakedFor(msg.sender) >= amount,
            "Crc: unstake amount exceeds balance"
        );
        ...
        // compute and apply CRCT token bonus
        uint256 crctWeightedShareSeconds = crctBonus(crct)
            .mul(timeWeightedShareSeconds)
            .div(10**BONUS_DECIMALS);

        uint256 rewardAmount = totalUnlocked()
            .mul(crctWeightedShareSeconds)
            .div(totalStakingShareSeconds.add(crctWeightedShareSeconds));

        // update global stats for distributions
        if (crct > 0) {
            totalCrctRewards = totalCrctRewards.add(rewardAmount);
        }
        ...
        if (crct > 0) {
            _crct.safeTransferFrom(msg.sender, address(this), crct);
            emit CrctSpent(msg.sender, crct);
        }
        return rewardAmount;
    }
    ...
    function _unstakeFirstInLastOut(uint256 amount) private returns (uint256) {
        ...
        require(stakingSharesToBurn > 0, "Crc: unstake amount too small");
    ...
    /**
     * @notice compute CRCT bonus as a function of usage ratio and CRCT spent
     * @param crct number of CRCT token applied to bonus
     * @return multiplier value
     */
    function crctBonus(uint256 crct) public view returns (uint256) {
        if (crct == 0) {
            return 10**BONUS_DECIMALS;
        }
        require(
            crct >= 10**BONUS_DECIMALS,
            "Crc: CRCT amount is between 0 and 1"
        );

        uint256 buffer = uint256(10**(BONUS_DECIMALS - 2)); // 0.01
        uint256 r = ratio().add(buffer);
        uint256 x = crct.add(buffer);
    ...
    /**
     * @return portion of rewards which have been boosted by CRCT token
     */
    function ratio() public view returns (uint256) {
        if (totalRewards == 0) {
            return 0;
        }
        return totalCrctRewards.mul(10**BONUS_DECIMALS).div(totalRewards);
    }

    // Crc -- informational functions
    ...
    /**
     * @notice preview estimated reward distribution for full unstake with no CRCT applied
     ...
     * @param crct number of CRCT tokens that would be applied
     * @return estimated reward
     * @return estimated overall multiplier
     * @return estimated raw user share seconds that would be burned
     * @return estimated total unlocked rewards
        ...
        )
    {
        ...
        // apply crct bonus
        uint256 crctBonusShareSeconds = crctBonus(crct)
        ...
        uint256 expectedTotalShareSeconds = totalStakingShareSeconds
            ...
            .add(crctBonusShareSeconds)
            .sub(rawShareSeconds);

        uint256 reward = (totalUnlocked().add(deltaUnlocked))
            .mul(crctBonusShareSeconds)
