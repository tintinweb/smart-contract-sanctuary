pragma solidity ^0.4.25;
contract KiOS {
    function payment() public payable returns(bool);
}
contract KiOS_Gate {
    address public admin;
    address public reference;
    event Forwarded(address indexed _sender, address indexed _receiver, uint _amount);
    constructor(address _admin, address _reference) public {
        admin = _admin;
        reference = _reference;
    }
    function() public payable {
        payment();
    }
    function payment() public payable returns(bool) {
        require(msg.value > 0);
        if (!KiOS(admin).payment.value(msg.value)())
        admin.transfer(msg.value);
        emit Forwarded(msg.sender, admin, msg.value);
        return true;
    }
}
contract KiOS_Factory {
    address public admin = msg.sender;
    event GateCreated(address indexed _gateAddress, address indexed _destinationAddress);
    function create(address _reference) public payable returns(address) {
        uint amount = msg.value;
        address gate = address(new KiOS_Gate(msg.sender, _reference));
        if (msg.sender != admin) {
            require(msg.value >= 5 finney);
            amount -= 5 finney;
            if (!KiOS(admin).payment.value(5 finney)())
            admin.transfer(5 finney);
        }
        if (amount > 0) {
            if (!KiOS(gate).payment.value(amount)())
            msg.sender.transfer(amount);
        }
        emit GateCreated(gate, msg.sender);
        return gate;
    }
}