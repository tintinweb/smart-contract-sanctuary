...
contract Lottery49 {
    ...
    function setPeriods(uint _periods) external onlyAuthorized {
        ...
        for(uint8 i=1;i<8;i++){
            draws[_periods].gss[i]=Gstatus({start_status:true,draw_status:false});
        }
        emit getStatus(Periods,betclose);
    }
    ...
    function getDrawStatus(uint _periods) constant internal returns(bool){
        Pstatus storage rec=draws[_periods];
        for(uint8 i=1;i<8;i++){
            Gstatus storage rec1=rec.gss[i];
            if(!rec1.draw_status) return false;
        }
        return true;
    }
    function drawWinner(uint _periods,uint8 _btype) public onlyAuthorized returns (bool) {
        require(!draws[_periods].draw_status && draws[_periods].start_status);
        require(!draws[_periods].gss[_btype].draw_status && draws[_periods].gss[_btype].start_status);
        ...
        draws[_periods].gss[_btype].draw_status=true;
        if(getDrawStatus(_periods)){
