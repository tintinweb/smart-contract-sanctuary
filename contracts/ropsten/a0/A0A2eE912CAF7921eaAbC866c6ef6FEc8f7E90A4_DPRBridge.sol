...
contract DPRBridge is DPRBridgeValidator {
        ...
        event ConfirmMessage(bytes32 messageID);
       ...
       */
        ...
        */
        ...
        function setTransfer(uint amount, bytes32 substrateAddress) public updateUserDepositInfo(msg.sender, amount) updateContractDepositInfo(amount){
        ...
        */
        function withdrawTransfer(bytes32 messageID, bytes32  substrateSender, address recipient, uint availableAmount)  public 
        updateUserWithdrawInfo(msg.sender, availableAmount) 
        updateContractWithdrawInfo(availableAmount)
        onlyManyValidatorsApprove(messageID, msg.sender)
         {  
            if(checkApproveStatus(messageID)){
        ...
        */
        function confirmWithdrawTransfer(bytes32 messageID) public withdrawMessage(messageID) onlyManyValidatorsConfirm(messageID, msg.sender) {
            if(checkConfirmStatus(messageID)){
                Message storage message = messages[messageID];
                uint256 withdraw_amount = message.availableAmount;
                address recipient = message.spender;
