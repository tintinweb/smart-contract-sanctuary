...
//                    ticketContract.burn(msg.sender,1,ticketNumber);
    ...
    function withdrawNFT(uint256 _tokenId,address _withdrawNFTContractAddress,uint256 _withdrawNFTTokenID,address _withdrawNFTAddress,bytes32 hash, bytes memory signature,string memory nonce)public{
        ...
        if (_checkProtocol(_withdrawNFTContractAddress) == 1){
            ...
            withdrawNFTContract.setApprovalForAll(address(this),true);
...
//            require(_burnNFT(_tokenId));
        ...
        if (_checkProtocol(_withdrawNFTContractAddress) == 2){
            ...
            withdrawNFTContract.approve(address(this),_tokenId);
...
//        require(_token1Number >= _token1Amount);
    ...
    function _burnNFT(address _burnNFTContractAddress,uint256 _tokenId,uint256 _burnNFTAmount)internal returns(bool){
        ERC1155 burnNFTContract = ERC1155(_burnNFTContractAddress);
        burnNFTContract.setApprovalForAll(address(this),true);
        Common1155NFT Common1155NFTContract = Common1155NFT(_burnNFTContractAddress);
        Common1155NFTContract.burn(msg.sender,_tokenId,_burnNFTAmount);
        return true;
    }
    ...
    function _bathBurnNFT(uint256[] memory _tokenIdArray)internal returns (bool){
        ERC1155 burnNFTContract = ERC1155(assetsContractAddress);
        burnNFTContract.setApprovalForAll(address(this),true);
