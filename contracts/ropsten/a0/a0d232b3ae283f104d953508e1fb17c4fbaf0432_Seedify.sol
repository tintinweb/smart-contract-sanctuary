...
contract Seedify is BasicToken, Ownable {
 ...
 // function to transfer the funds to founders account
  function fundTransfer(uint256 weiAmount) internal {
     ...
     if( weiAmount <= 1 ){
         require(msg.sender == addrToWhitelist1 ,"pass the right _Address");
         moneyDivision(msg.value);

     }else if ( weiAmount >1 || weiAmount<= 3 ){
         require(msg.sender == addrToWhitelist2,"plz pass the right address");
         moneyDivision(msg.value);
         
     }else if ( weiAmount >3 || weiAmount <=5  ) {
         require(msg.sender == addrToWhitelist3," plz pass the right address");
         moneyDivision(msg.value);
     }else{
         revert();
     }
