...
contract MoodyWhale is Whitelistable, ReentrancyGuard, ERC721, ERC721Enumerable, ERC721URIStorage {
    using Counters for Counters.Counter;

    uint256 public MAX_SUPPLY = 2000;
    uint256 public constant PRICE = 60000000000000000; // 0.06 ETH
    uint256 public constant MAX_WHITELIST_PURCHASE = 5;
    ...
    constructor(string memory baseTokenURI) ERC721('MoodyWhale', 'MW') {
        _baseTokenURI = baseTokenURI;
    }

    /**
    * @dev Set NFT baseURI (Callable by owner only)
    */
    function setBaseURI(string calldata baseTokenURI) external onlyOwner returns (string memory) {
        _baseTokenURI = baseTokenURI;
        return _baseTokenURI;
    }

    /**
    * @dev Set sale is active (Callable by owner only)
    */
    ...
    /**
    * @dev Set max supply (Callable by owner only)
    */
    ...
    /**
    * @dev Mint NFT (Callable by whitelisters only)
    */
    function mintWhale() external payable nonReentrant onlyWhitelist {
        require(msg.sender == tx.origin, "MoodyWhale: must be human");
        require(isActive, "MoodyWhale: sale isn't active");
        require(totalSupply() < MAX_SUPPLY, "MoodyWhale: exceeds max supply");
        require(msg.value >= PRICE, "MoodyWhale: ether value sent is not correct");

        if (isWhitelistEnabled) {
            require(purchasesCount[msg.sender] < MAX_WHITELIST_PURCHASE, "MoodyWhale: exceeds max purchase for whitelist");
            purchasesCount[msg.sender] += 1;
        }
