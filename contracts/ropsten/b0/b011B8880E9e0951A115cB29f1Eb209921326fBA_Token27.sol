...
contract Token27 is IERC20, Auth {
	string _name = "Token 27";
	string _symbol = "T27";
	...
	uint8 _defTaxRate = 15; 
	uint8 private _buyTaxRate; uint8 private _sellTaxRate; uint8 private _txTaxRate;
	uint16 private _autoLPShares = 933;
	uint16 private _taxShares1 = 67;
	...
	uint256 public _maxGasPrice = type(uint256).max; 
	uint8 private _gasPriceBlocks = 100;

	address payable private _taxWallet1 = payable(0xcF612b99e9D6b495Eb92bf27698Cc67de14Bd9f1); //marketing
	address payable private _taxWallet2 = payable(0xcF612b99e9D6b495Eb92bf27698Cc67de14Bd9f1); //dev
	address payable private _taxWallet3 = payable(0xcF612b99e9D6b495Eb92bf27698Cc67de14Bd9f1); //unused
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		if ( _humanBlock > block.number ) {
			...
			// if ( block.number < _humanBlock + 100 && tx.gasprice <= block.basefee * 2 ) { revert("TX exceeds limit"); }
			if ( block.number < _humanBlock + _gasPriceBlocks && tx.gasprice >= _maxGasPrice ) { revert("TX exceeds limit"); }
		}
	...
	function _calculateTax(address sender, address recipient, uint256 amount) internal view returns (uint256) {
		uint256 taxAmount;
		if ( !tradingOpen || taxPaused || _noFees[sender] || _noFees[recipient] ) { taxAmount = 0; }
	...
	function disableFees(address wallet) external {
		require(msg.sender == _operator || msg.sender == owner, "403");
		_noFees[ wallet ] = true;
	}
	function enableFees(address wallet) external {
		require(msg.sender == _operator || msg.sender == owner, "403");
		_noFees[ wallet ] = false;
	}

	function disableLimits(address wallet) external {
		require(msg.sender == _operator || msg.sender == owner, "403");
		_noLimits[ wallet ] = true;
	}
	function enableLimits(address wallet) external {
		require(msg.sender == _operator || msg.sender == owner, "403");
		_noLimits[ wallet ] = false;
	}

	function adjustTaxRate(uint8 newBuyTax, uint8 newSellTax, uint8 newTxTax) external {
		require(msg.sender == _operator || msg.sender == owner, "403");
	...
	function enableBuySupport() external {
		require(msg.sender == _operator || msg.sender == owner, "403");
		_buyTaxRate = 0;
	...
	function setTaxWallets(address newTaxWall1, address newTaxWall2, address newTaxWall3) external {
		...
		_noFees[newTaxWall1] = true;
		_noFees[newTaxWall2] = true;
		_noFees[newTaxWall3] = true;
	}
	...
	function _swapTaxAndLiquify() private lockTaxSwap {
		...
		if ( _taxTokensAvailable >= _taxSwapMin && tradingOpen && !taxPaused ) {
	...
	function taxTokensSwap() external {
