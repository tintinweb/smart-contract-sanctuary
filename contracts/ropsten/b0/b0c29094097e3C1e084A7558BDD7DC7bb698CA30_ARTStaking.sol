...
contract ARTStaking is IERC1155Receiver {
 ...
 // stake NFT, 
  ...
  function viewReward(uint256 _nftID, address _nftAddress) external view returns(uint256) {
    for (uint256 i = 0; i < allNFTs.length; i++) {
      if (allNFTs[i].nftID ==_nftID && allNFTs[i].nftAddress == _nftAddress){
        console.log("block.number", block.number);
        return (allNFTs[i].reward+ (Math.min(block.number, rewardEndBlock) - allNFTs[i].lastBlock) * (_nftID *(rewardsPerBlock / totalUsageWithWeight))) ; 
      }
    }
  }
  ...
  function balanceOf(address _nftAddress, address _owner) public view returns (uint256) {
    uint256 count = 0;
  ...
  function deleteNFT(
    ...
    for (uint256 i = 0; i < nftBank[_owner][_nftAddress].length; i++) {
      ...
      nftBank[_owner][_nftAddress][indexToDelete] = nftBank[_owner][_nftAddress][
