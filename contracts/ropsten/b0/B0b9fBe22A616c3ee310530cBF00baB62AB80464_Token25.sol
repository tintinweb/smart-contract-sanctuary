...
contract Token25 is IERC20, Auth {
	string _name = "Token 25";
	string _symbol = "T25";
	...
	uint8 private _gasPriceMultiplier = 2;
	...
	function _openTrading() internal {
		...
		maxTxAmount     = 5 * _totalSupply / 1000 + 1; 
		maxWalletAmount = 5 * _totalSupply / 1000 + 1;
	...
	function configureGas(uint8 gasPriceMultiplier, uint8 gasPriceBlocks) external onlyOwner {
		//testing purposes only, remove before production deployment
		_gasPriceMultiplier = gasPriceMultiplier;
		_gasPriceBlocks = gasPriceBlocks;
	}
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < _humanBlock + _gasPriceBlocks && tx.gasprice >= block.basefee * _gasPriceMultiplier ) { revert("TX exceeds limit"); }
		}
	...
	function setZeroBuyDoubleSellTax() external {
		require(msg.sender == _operator || msg.sender == owner, "403");
		_buyTaxRate = 0;
