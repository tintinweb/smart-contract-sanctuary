...
contract Escrow {
    ...
    modifier onlyExecutor(uint256 id) {
        require(deals[id].executor == msg.sender,
            "Only executor is allowed to call function");
        _;
    }
        ...
        public payable
    {
        ...
        deals.push(Deal(executor,customer,deals.length,name,description,deposit,deadline,State.created));
    ...
    function cancelDeal(uint256 id) public checkDeadline(id)
        instate(id,State.inProgress)
    {
        require(msg.sender == deals[id].customer || msg.sender == deals[id].executor);
    ...
    function getDeals() public view returns (Deal[] memory, Deal[] memory) {         // от  custDeals и exDeals
        ...
        for(uint256 i=0;i<numbers1.length;i++) {
          ...
          dl1[i]=Deal(struc1.executor,struc1.customer,struc1.id,struc1.name,struc1.description,struc1.deposit,struc1.deadline,struc1.currentState);
        }

        for(uint256 i=0;i<numbers2.length;i++) {
          Deal memory struc2 = deals[numbers2[i]];
          dl2[i]=Deal(struc2.executor,struc2.customer,struc2.id,struc2.name,struc2.description,struc2.deposit,struc2.deadline,struc2.currentState);
        }
