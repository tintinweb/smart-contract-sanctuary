...
contract BMEX is Context, IERC20, IERC20Metadata, BlacklistManagerRole, Pausable {
    ...
    constructor(address owner, address vestingContract) {
        _name = "BitMEX Test Token";
        _symbol = "TMEX";
        _vestingContract = vestingContract;
        _mint(owner, 450000000*10**8);
        _transferOwnership(owner);
    }

    function initialize() external {
        IVesting(_vestingContract).setToken(address(this));
        _mint(address(this), 450000000*10**8);
        _approve(address(this), _vestingContract, 414000000*10**8);
        IVesting(_vestingContract).initialDeposit(IVesting(_vestingContract).treasury(), 76500000*10**8, 0, 12);
        IVesting(_vestingContract).initialDeposit(IVesting(_vestingContract).team(), 90000000*10**8, 9, 33);
        IVesting(_vestingContract).initialDeposit(IVesting(_vestingContract).ecosystemFund(), 135000000, 3, 21);
        IVesting(_vestingContract).initialDeposit(IVesting(_vestingContract).longTermLockUp(), 112500000, 24, 60);
        _transfer(address(this), IVesting(_vestingContract).treasury(), 36000000*10**8);
    }
