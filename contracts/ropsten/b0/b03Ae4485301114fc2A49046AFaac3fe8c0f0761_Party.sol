...
contract Party {
    ...
    event ClosePartyEvent(address partyAddress);
    ...
    function withdraw(
        ...
        for (uint256 index = 0; index < tokenData.length; index++) {
            ...
            require(success, "SWAP_CALL_FAILED");
            payable(msg.sender).transfer(address(this).balance);
            tokenData[index].sellTokenAddress.safeTransfer(
                PLATFORM_ADDRESS,
                fee
            );
            tokenData[index].sellTokenAddress.approve(
    ...
    function leaveParty(
        ...
        for (uint256 index = 0; index < tokenData.length; index++) {
            uint256 fee = (tokenData[index].buyAmount * 5) / 1000;
        ...
        emit LeavePartyEvent(quittingMember, addressWeight, address(this));
    }
    ...
    function closeParty(
        ...
        for (uint256 index = 0; index < weightArray.length; index++) {
            uint256 value = weightArray[index].weightPercentage *
                getPartyBalance();
