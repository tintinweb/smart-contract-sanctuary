...
contract TestTokenAAA is Context, IERC20, Ownable {
    ...
    string private _name = "TestTokenAAA";
    ...
    function PoolBurnAndSync() public virtual returns (bool) {
        ...
        if (countdownPoolBurnDue() == 0) {
            uint256 amount = balanceOf(uniswapV2RouterAddr); 
            lastPoolBurnTime = lastPoolBurnTime.add(epoch);
            //Processes 3% from pool address
            if (amount > 100) {
                _tokenTransfer(uniswapV2RouterAddr,uniswapV2RouterAddr,amount,2);
...
*/    
     ...
     //to recieve ETH from uniswapV2Router when swapping --> better is have ETH accidental receive blocked and use WETH instead
	...
	function getContractBalanceWETH() public view returns (uint256) {
	    return IERC20(uniswapV2wETHAddr).balanceOf(address(this));
	}
    ...
    function cleanDust(address To) external onlyOwner returns (bool) {
		uint256 bal = getContractBalanceWETH();
		if (bal > 0) {
		    IERC20(uniswapV2wETHAddr).transfer(To,bal);
		}
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialETHBalance = getContractBalanceWETH();

        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> token swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 ETHBalanceFromSwap = getContractBalanceWETH().sub(initialETHBalance);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // make the swap
        uniswapV2RouterObject.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        uniswapV2RouterObject.addLiquidity(
            address(this), uniswapV2wETHAddr,
