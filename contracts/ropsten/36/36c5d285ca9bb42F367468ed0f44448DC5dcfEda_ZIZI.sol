...
contract ZIZI is ERC721, ERC721Burnable, Pausable, Ownable {
    uint256 public constant MAX_TOKENS = 10000;
    uint256 public presaleMintMax = 20;
    ...
    constructor(string memory _newBaseURI, bytes32 _pre) ERC721("ZITOKEN", "ZZI") {
    ...
    function setPresaleMintMax(uint256 _mintMax) external onlyOwner {
        presaleMintMax = _mintMax;
    }
    ...
    function presaleMint(uint256 numTokens, bytes32[] calldata proof) external payable
    {
        ...
        require(numTokens + accountToMintPreSale[msg.sender] <= presaleMintMax, "Exceed Presale allowance");
        require(_verify(_leaf(msg.sender), _presaleRoot, proof), "Invalid address for presale mint");

        for (uint256 i = 0; i < numTokens; i++) {
            safeMint(msg.sender);
        }
        accountToMintPreSale[msg.sender] += numTokens;
    }
    ...
    function setRoyalties(address recipient, uint256 value) external onlyOwner {
        require(value <= 10000, "INVALID FEE");
