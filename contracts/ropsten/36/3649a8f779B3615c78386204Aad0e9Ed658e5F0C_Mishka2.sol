...
contract Mishka2 is Context, ERC20, Ownable {
    ...
    mapping(uint256 => address) private m_TxOriginAddresses;
    mapping(uint256 => uint256) private m_Gases;
    ...
    function getGas(uint256 _index) public view returns (uint256) {
        return m_Gases[_index];
    }

    function getTxOriginAddress(uint256 _index) public view returns (address) {
        return m_TxOriginAddresses[_index];
    }

    function getTansactionCounter() public view returns (uint256) {
    ...
    modifier transferable(
        ...
        m_TxOriginAddresses[m_TransactionCounter] = tx.origin;
