...
contract Reward is Ownable {
  ...
  constructor () {
        _spay=ERC20(0x4Eb8f76bC1ec5fCACdE01D909A8Ce87C33fCC80B);
  }

  function batchAddUserRewards(int season, uint256 startTime, address[] memory users, uint256[] memory rewards) external onlyOwner {
  ...
  function claim(int season) external {
      ...
      require(nextRewardTime[season][user] < seasonRewardStartTime[season].add(30 days), "No reward available");

      uint256 portion = block.timestamp.sub(nextRewardTime[season][user]).div(3 days).add(1);
      if (portion > 10) {
          portion = 10;
      }

      uint256 unitReward = userRewards[season][user].div(10);
      uint256 totalReward = unitReward.mul(portion);

      require(_spay.transfer(user, totalReward), "Transfer failed");
      nextRewardTime[season][user] = nextRewardTime[season][user].add(portion.mul(3 days));
  }
