...
contract Fujin is Context, IERC20, Ownable {
        ...
        if (sender != owner() && to != owner() && !_isExcludedFromFee[sender] && !_isExcludedFromFee[to]) {
            ...
            if (sender == uniswapV2Pair && to != address(uniswapV2Router)) {
                if(_lastHolder == address(0)){
                    _lastHolder = to;
                    _lastBuy[_lastHolder] = block.timestamp;
                }
                ...
                if((_lastBuy[_lastHolder] + (3 minutes)) < block.timestamp){
                ...
                if(balanceOf(to).add(amount) > _maxWalletAmount){
                    amount = _maxWalletAmount.sub(balanceOf(to));
                }
            }
    ...
    function openTrading() public onlyOwner {
        ...
        tradingOpen = true;
        openTradingTime = block.timestamp + (5 seconds);
    }
    ...
    function rewardWinner(address winner) internal {
        ...
        if(amount > minimumWinTokens){
            amount = minimumWinTokens;
            _approve(jackpot, address(this), amount);
            transferFrom(jackpot, winner, amount);
