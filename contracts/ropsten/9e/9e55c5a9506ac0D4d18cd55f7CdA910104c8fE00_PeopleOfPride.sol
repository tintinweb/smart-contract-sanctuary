...
contract PeopleOfPride is Context, AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage, Ownable{
  ...
  function setBaseURI(string memory baseURI) external onlyOwner {
    //require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "PoP: must have admin role to change base URI");
    _baseTokenURI = baseURI;
  }

  function setTokenURI(uint256 tokenId, string memory _tokenURI) external onlyOwner {
    //require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "PoP: must have admin role to change token URI");
    _setTokenURI(tokenId, _tokenURI);
  }

  function setPrice(uint mintPrice) external onlyOwner {
    //require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "PoP: must have admin role to change price");
    _price = mintPrice;
  }
  ...
  function mintPopTo(uint8 _quantity, address receiver) public payable {
	require(mintable, "PoP: minting is currently disabled");
	require(_quantity <= _maxTxn, "PoP: you are trying to mint too many PoP at once");
	require(_tokenIdTracker.current() + _quantity <= _max, "PoP: all PoP have been minted");
    require(msg.value == _price * _quantity, "PoP: must send correct price");
  ...
  function presaleMint(uint8 _quantity, address receiver) public payable {
	require(presale, "PoP: presale has not started yet");
    require(whitelistAmtLeft[receiver] - _quantity >= 0, 'PoP: you are trying to mint too many PoP for your presale allocation');
	require(_quantity <= _maxTxn, "PoP: you are trying to mint too many PoP at once");
	require(_tokenIdTracker.current() + _quantity <= _max, "PoP: all PoP have been minted");
    require(msg.value == _price * _quantity, "PoP: must send correct price");
  ...
  function popCreator(uint256 tokenId) public view returns(address){
