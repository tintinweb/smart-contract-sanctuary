...
contract StandardBond is Ownable
{
   ...
   struct Terms {
    ...
    mapping(address => Terms) BondTerms; // principle address => Terms Struct (generic for all users).
    mapping(address => Info) BondInfo; //  bond info (generic for all users).
    mapping(address => mapping(address => Bond)) DepositorInfo; // principal(user => bondPersonalInfo) user based information of bond..
    ...
    uint _initialDebt)public onlyOwner
    {
        ...
        BondInfo[_principle].totalDebt = _initialDebt;
        BondInfo[_principle].lastDecay = block.number;
        if (_calculator != address(0))
        {
            BondInfo[_principle].isLiquidityBond = true;
        }
        else
        {
            BondInfo[_principle].isLiquidityBond = false;
        }
    ...
    function deposit(address _principle, uint _amount, uint _maxPrice, address _depositor)public returns(uint)
    {
        require( _depositor != address(0), "Invalid address" );

         decayDebt(_principle);
        require( BondInfo[_principle].totalDebt <= BondTerms[_principle].maxDebt, "Max capacity reached" );
        ...
        BondInfo[_principle].totalDebt = BondInfo[_principle].totalDebt.add( value ); 
    ...
    function decayDebt(address _principle) internal {
        BondInfo[_principle].totalDebt = BondInfo[_principle].totalDebt.sub( debtDecay(_principle) );
        BondInfo[_principle].lastDecay = block.number;
    }

    function debtDecay(address _principle) public view returns ( uint decay_ ) {
        uint blocksSinceLast = block.number.sub( BondInfo[_principle].lastDecay );
        decay_ = BondInfo[_principle].totalDebt.mul( blocksSinceLast ).div( BondTerms[_principle].vestingTerm);
        if ( decay_ > BondInfo[_principle].totalDebt ) {
            decay_ = BondInfo[_principle].totalDebt;
        }
    }
    ...
    function currentDebt(address _principle) public view returns ( uint ) {
        return BondInfo[_principle].totalDebt.sub( debtDecay(_principle) );
    }

     function bondPriceInUSD(address _principle) public view returns ( uint price_ )
    {
        if( BondInfo[_principle].isLiquidityBond ) {
    ...
    function redeem( address _principle, address _recipient, bool _stake ) external returns ( uint ) {        
