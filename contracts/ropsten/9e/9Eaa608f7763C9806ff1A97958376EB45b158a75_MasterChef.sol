...
contract MasterChef is Ownable {
    ...
    function pendingKwik(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if(block.timestamp <= finalClaimEndTime){
            stakingDays = (block.timestamp.sub(user.lastClaim)).div(5);
        }
        else{
            stakingDays = (finalClaimEndTime.sub(user.lastClaim)).div(5);
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0){
            ...
            if(block.timestamp <= user.depositTime.add(pool.durationOfPool.mul(5))){
                user.lastClaim = block.timestamp;
            }else{
                user.lastClaim = user.depositTime.add(pool.durationOfPool.mul(5));
            }
        }
