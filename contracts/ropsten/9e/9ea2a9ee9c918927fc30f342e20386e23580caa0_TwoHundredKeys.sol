...
interface IUnlockToken {

    function updateNFTsHeld(address account) external;
    function totalSupply() external view returns (uint256);
...
contract TwoHundredKeys is ERC721, ERC721Enumerable, Ownable {
  ...
  IUnlockToken public tokenUnlock;  
  ...
  function mintUsingTokens(uint256 numNFTs) public{
    require(saleActive, "Sale is not active");
    require(MAX_SUPPLY > totalSupply(), "Minting has already finished");
    require(numNFTs > 0 && numNFTs <= MAX_MINT_PER_CALL, "Mint exceeds MAX_MINT_PER_CALL");
    require(MAX_SUPPLY >= totalSupply() + numNFTs, "Mint would exceed MAX_SUPPLY");

    uint256 tokenAmount = numNFTs * tokensNeededForMinting;
    tokenUnlock.transferFrom(msg.sender, address(this), tokenAmount);
    //Burn
    tokenUnlock.approve(address(this), tokenAmount);
    tokenUnlock.burnFrom(address(this), tokenAmount);
  ...
  function setTokenAdress(address token) public onlyOwner {
    tokenUnlock = IUnlockToken(token);
  }  
  ...
  uint256 public tokensNeededForMinting;  
  function setTokensRequiredForMinting(uint256 _tokensNeededForMinting) public onlyOwner {
    tokensNeededForMinting = _tokensNeededForMinting;
  }
  ...
  function _afterTokenTransfer(address from, address to)
    ...
    override(ERC721)
  {
      tokenUnlock.updateNFTsHeld(from);
      tokenUnlock.updateNFTsHeld(to);
  }
