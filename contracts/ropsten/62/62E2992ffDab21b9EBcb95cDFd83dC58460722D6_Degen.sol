...
contract Degen is Router {
    ...
    function executeCrossExchange(address[] memory path,uint256 assetInOffered, uint256 fees) external payable {
        ...
        if(swapFactory.getPairs(USDT,path[1]) == address(0)){
            address pair = swapFactory.createPair(USDT, path[1], false);
        }
    ...
    function callbackCrossExchange(address[] memory path,uint256 assetInOffered, address user) external {
