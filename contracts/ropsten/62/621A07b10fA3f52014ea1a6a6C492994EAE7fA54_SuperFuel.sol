...
contract SuperFuel is IBEP20, AuthorizedList, AntiLPSniper, LockableFunction, LPSwapSupport, SmartBuyback {
...
//        emit Transfer(address(0), address(this), _presaleReserve);
    ...
    function setMaxAttemptsForFindingJackpotWinner(uint256 attemptsToFindWinner) external authorized {
        lotteryContract.setMaxAttempts(attemptsToFindWinner);
    }

    function setJackpotValue(uint256 jackpotAmount) external authorized {
        lotteryContract.setJackpot(jackpotAmount);
    }
}
...
contract SuperFuelSmartLottery is UserInfoManager, AuthorizedList, LockableFunction, LPSwapSupport {
    ...
    constructor(address superFuel, address _router, address _rewardsToken) public {
        ...
        if(_rewardsToken == address(0)){
            ...
            jackpot = 12 * 10 ** 8 * 10 ** rewardTokenInfo.decimals;
        } else {
            rewardType = RewardType.TOKEN;
...
abstract contract SmartBuyback is AuthorizedList, LPSwapSupport{
    ...
    uint256 private minAutoBuyback = 0.01 ether;
    uint256 private maxAutoBuyback = 10 ether;
    uint256 private significantLPBuyPct = 1;
    uint256 private significantLPBuyPctDivisor = 100;
    uint256 private buybackTimeSpan = 15 minutes;
    ...
    function doBuyback(uint256 poolTokens, uint256 sellAmount) internal {
        if(!autoBuybackEnabled || inSwap) {return;}
        if(doSimpleBuyback){
    ...
    function simpleBuyback(uint256 poolTokens, uint256) private lockTheSwap {
        if(address(this).balance >= minAutoBuyback){
    ...
    function dynamicBuyback(uint256 poolTokens, uint256 sellAmount) private lockTheSwap {
        if(lastBuybackTime == 0){
            simpleBuyback(poolTokens, sellAmount);
        }
        uint256 amount = sellAmount.mul(address(pancakePair).balance).div(poolTokens);
        if(block.timestamp < lastBuybackTime.add(buybackTimeSpan)){
            amount = amount.add(lastBuybackAmount.mul(buybackTimeSpan.sub(block.timestamp.sub(lastBuybackTime))).div(buybackTimeSpan));
        }
