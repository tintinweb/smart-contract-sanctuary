...
contract UniswapV2Pair is UniswapV2ERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) public virtual lock {
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'DVF_AMM: K');
        }
...
// File: contracts/PairWithL2Overlay.sol

// a-Identifier: UNLICENCED
pragma solidity >=0.6.12;
...
contract PairWithL2Overlay is UniswapV2Pair, Ownable {
  using SafeMathUniswap for uint;
  ...
  function initialize(address _token0, address _token1, address _operator) public {
    operators[_operator] = true;
    _Ownable_Initialize(_operator);
  ...
  */
  function transfer(address to, uint value) public override returns (bool) { 
    // Unify duplication with method bellow
    if (isLayer2Live && !isOperator() && to == address(this)) {
      // Withdrawal -> burn
      address from = msg.sender;
      Withdrawal storage w = delayedTransfers[from];
      _validateDelayedTransfers(w.time, w.amount, value);
      require(super.transfer(to, value), "DVF_AMM: TRANSFER_FAILED");
      delete delayedTransfers[from];
      return true;
    }
  ...
  */
  ...
  /**
  * @dev Queue tokens to be burned after a delay, uses transaction sender
  */
  function queueBurn(uint value) external returns (uint) {
    return _queueBurn(value, msg.sender);
  }
