...
contract mg is ERC721Enumerable, ReentrancyGuard, Ownable {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "gm #',
                        toString(tokenId),
                        '", "description": "69 good mornings stored on chain.", "image": "data:image/svg+xml;base64,',
    ...
    function claim() public nonReentrant {
        require(currTokenId < 70, "all gm's have been minted.");
        //require(balanceOf(_msgSender()) == 0, "only one gm per address");
        
        uint gmIndex = getRandom();
        currTokenId++;
        _safeMint(_msgSender(), currTokenId);
        tokenIdToGm[currTokenId] = gms[gmIndex];
