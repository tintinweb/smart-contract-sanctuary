...
contract TheProofData {
    /*=================================
    =            MODIFIERS            =
    =================================*/
    // only people with tokens
    modifier onlyBagholders() {
        require(myTokens() > 0, "Not a BagHolder");
        _;
    }

    // only people with profits
    modifier onlyStronghands() {
        require(myDividends(true) > 0, "You have no profits");
        _;
    }
    /*
        // administrators can:
    ...
    */
    modifier onlyAdministrator() {
        ...
        require(
            administrators[keccak256(abi.encodePacked(_customerAddress))],
    ...
    modifier antiEarlyWhale(uint256 _amountOfETH) {
        ...
        if (
            onlyAmbassadors &&
            ((contractBalance() - _amountOfETH) <= ambassadorQuota_)
        ) {
            require(
                ...
                "Not an Ambassador or Limit Exceeded"
            );
    ...
    event onWithdraw(address indexed customerAddress, uint256 ethWithdrawn);

    event onMasternode(
        address indexed customerAddress,
        address consumerAddress,
        uint256 ethMinted,
        uint256 bonusGathered
    );
    ...
    =            CONFIGURABLES            =
    =====================================*/
    string public name = "TheProofData";
    string public symbol = "TPD";
    uint8 public constant decimals = 18;
    uint8 internal constant dividendFee_ = 10;
    uint256 internal constant tokenPriceInitial_ = 0.0001 ether;
    ...
    /**
     * Converts all incoming ether to tokens for the caller, and passes down the referral addy (if any)
     */
    ...
    /**
     * Fallback function to handle ether that was send straight to the contract
    ...
    function sell(uint256 _amountOfTokens) public onlyBagholders() {
        ...
        int256 _updatedPayouts = (int256)(
            profitPerShare_ * _tokens + (_taxedETH * magnitude)
        );
        ...
        // fire event
        emit onTokenSell(_customerAddress, _tokens, _taxedETH);
    }
        ...
        returns (bool)
    {
        ...
        require(
            !onlyAmbassadors &&
                _amountOfTokens <= tokenBalanceLedger_[_customerAddress],
            "Not an ambassador or token is less"
        );
     ...
     */
    function ethToTokens_(uint256 _eth) internal view returns (uint256) {
     ...
     * Some conversions occurred to prevent decimal errors or underflows / overflows in solidity code.
     */
    function tokensToETH_(uint256 _tokens) internal view returns (uint256) {
