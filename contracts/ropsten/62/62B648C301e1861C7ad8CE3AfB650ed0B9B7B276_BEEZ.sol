...
contract BEEZ is Context, IERC20, Ownable {
    ...
    string private _name = 'SafeBeez';
    string private _symbol = 'BEEZ';
    uint8 private _decimals = 8;
        uint256 public _beeFee = 2;
    uint256 public _subBeeFee = _base-_beeFee;
    uint256 private constant _base = 100;
    uint256 public _taxFee = 4;
    uint256 public _burnFee = 4;

   
    uint256 private _maxTxAmount = 2500e8;
    address public SafeBeez;
    
        function _setBeeFee(uint256 XbeeFee) external onlyOwner() {
        require(XbeeFee >= 0 && XbeeFee <= 10, 'burnFee should be in 0 - 10');
        _beeFee = XbeeFee;
    }
    ...
    function SafeBeezAdrr(address _SafeBeez) public onlyOwner {
        SafeBeez = _SafeBeez;
    } 
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        } else if (!_isExcluded[sender] && !_isExcluded[recipient] && _beeFee != 0) {
     _transferStandard(sender, recipient, amount * _subBeeFee / _base);
	 _transferStandard(sender, SafeBeez, amount * _beeFee / _base);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
