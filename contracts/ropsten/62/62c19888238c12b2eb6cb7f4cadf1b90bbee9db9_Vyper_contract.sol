...
depositWhiteList: public(HashMap[address, bool])

withdrawWhiteList: public(HashMap[address, bool])
...
event ExcessReturn:
    shares: uint256            # Mark phase2 flag 
...
def initialize(
    ...
    self.performanceFee = 2000  # 20% of yield (per Strategy)
    ...
    self.phase2Flag = False
    log Phase2Flag(False)
    # EIP-712
...
@external
def updatePhase2Flag(flag: bool):
    assert msg.sender == self.governance
    self.phase2Flag = flag
    log Phase2Flag(flag)
...
@external
def setDepositWhiteList(addr: address, status: bool):
    assert self.governance == msg.sender
    self.depositWhiteList[addr] = status

@external
def setWithdrawWhiteList(addr: address, status: bool):
    assert self.governance == msg.sender
    self.withdrawWhiteList[addr] = status
...
def deposit(_amount: uint256 = MAX_UINT256, recipient: address = msg.sender) -> uint256:
    ...
    assert self.depositWhiteList[msg.sender]
...
def withdraw(
    ...
    """
    assert self.withdrawWhiteList[msg.sender]
...
def _assessFees(strategy: address, gain: uint256) -> uint256:
    ...
    total_fee: uint256 = performance_fee + strategist_fee
    excess_return: uint256 = 0

    if self.phase2Flag and total_fee < gain:
        excess_return = gain - total_fee

    # ensure total_fee is not more than gain
    if total_fee > gain:
        total_fee = gain
    if total_fee > 0:  # NOTE: If mgmt fee is 0% and no gains were realized, skip
        reward: uint256 = self._issueSharesForAmount(self, total_fee + excess_return)
        ...
        if excess_return > 0 :
            excess_reward: uint256 = (
                excess_return
                * reward
                / (total_fee + excess_return)
            )
            log ExcessReturn(excess_reward)
    return total_fee
