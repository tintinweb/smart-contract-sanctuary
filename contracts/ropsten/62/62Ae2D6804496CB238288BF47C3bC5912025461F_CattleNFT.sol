...
contract CattleNFT is ERC721 {
    ...
    */
    ...
    /** @notice the function to return attributes given the DNAHash.
        @param _DNAHash the DNA hash given
        @return the attributes relating to the DNAHash
     */
    function DNAHashToAttributes(bytes32 _DNAHash) public view returns(
        string memory,
        string memory,
        bytes32, 
        address
    ) {
        uint256 tokenId = DNAHashToTokenID(_DNAHash);
        Cow memory cow;
        cow = cowStruct[tokenId];
        string memory _NLIS = cow.NLIS;
        string memory _PIC = cow.PIC;
        bytes32 _DNAHash = cow.DNAHash;
        address _Owner = cow.Owner;

        return(_NLIS, _PIC, _DNAHash, _Owner);
