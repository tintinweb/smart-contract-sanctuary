...
*/
    ...
    function advanceLoan(uint16 tokenId, address tokenAddress) OnlyItemOwner(tokenAddress,tokenId) HasTransferApproval(tokenAddress,tokenId) external returns (uint256){
        ...
        itemsUnderLoan.push(LoanItem(newItemId, true, tokenAddress, payable(msg.sender), uint32(block.timestamp), uint32(block.timestamp) + TERM_IN_DAYS, TERM_IN_DAYS, tokenId, LOAN_AMOUNT, SERVICE_FEE, LOAN_AMOUNT + SERVICE_FEE, LOAN_AMOUNT + SERVICE_FEE));
    ...
    function extendLoan(uint256 id) payable public ItemExists(id) IsUnderLoan(id) OnlyItemBorrower(id) IsCurrentlyCustodied(id){
        ...
        itemsUnderLoan[id].endDate      = (itemsUnderLoan[id].endDate + TERM_IN_DAYS);
