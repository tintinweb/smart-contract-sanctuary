...
contract TestToken is ERC1155, ERC1155Holder, Ownable {
    ...
    */
        ...
        for (uint256 i = 0; i < salesList[id].length; i++) {
            ...
            if(item.price <= price) {
                ...
                if(remain <= item.amount) {
                    _balances[id][item.user] -= remain;
                    _balances[id][msg.sender] += remain;
                    item.amount -= remain;
                    paidAmount = remain * item.price;
                    remain = 0;
                } else {
                    _balances[id][item.user] -= item.amount;
                    _balances[id][msg.sender] += item.amount;
                    remain -= item.amount;
    ...
    function Listing(uint256 id, uint256 price, uint256 amount) public payable{
        require(
            amount <= _balances[id][msg.sender],
