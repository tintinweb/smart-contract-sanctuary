...
contract Miner6 is Context, IERC20, Ownable {
  ...
  constructor() public {
    ...
    _totalSupply = finalTotalSupply; // 1b total tokens after all vesting
...
//   }
  ...
  event PegOutEvent(uint256 value, string destination_chain);

  function pegOut(uint256 _value, string memory destination_chain) public onlyOwner{
      require(_value < _totalSupply, "Cannot peg out more than totalSupply");
      _burn(msg.sender, _value);
      emit PegOutEvent(_value, destination_chain);
  }

  event PegIntEvent(uint256 value, string origin_chain, string origination_transaction);

  function pegInt(uint256 value, string memory origin_chain, string memory origination_transaction) public onlyOwner{
      _mint(msg.sender, value);
      emit PegIntEvent(value, origin_chain, origination_transaction);
  }
}