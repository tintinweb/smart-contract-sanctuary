...
contract CompoundPool is StakingPool {
    ...
    function balanceOf(address account) virtual override public view returns (uint) {
        return _balances[account].add(rewards[account]).mul(compoundPerToken().add(1e18)).div(compoundStored[account].add(1e18));
    }
...
contract CompoundPoolStakedToken is CompoundPool, ContextUpgradeSafe, IERC20 {
    ...
    function stakeTo(uint256 amount, address to) virtual override public {
        super.stakeTo(amount, to);
