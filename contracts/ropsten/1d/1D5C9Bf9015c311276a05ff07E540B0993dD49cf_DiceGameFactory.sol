...
contract DiceGameFactory is Ownable{ 
  ...
  enum Step { BetsPending, ScoresPending, Payment}
  ...
  function newGame() public {
    ...
    Game memory game = Game(players, winner, lowScore, scores, bet, roomName, false, false, Step.BetsPending);
    games.push(game);
  ...
  function joinGame(uint _gameId) public notInGame(_gameId){
    Game storage game = games[_gameId];
    require(game.currentStep == Step.BetsPending, "Cannot join until next round begins");
  ...
  function getStatus(uint _gameId) public view returns (Step) {
    return games[_gameId].currentStep;
} 
