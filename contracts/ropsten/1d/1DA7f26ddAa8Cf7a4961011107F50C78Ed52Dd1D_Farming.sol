...
contract Farming is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256[] cumulativeMoneyPerShare; // [1]
        uint256[] deposits;
    }
        ...
        returns (uint256 depositForRound)
    {
        ...
        if (pool.deposits.length == 0 || depositForRound > pool.deposits.length)
            depositForRound = pool.deposits.length + 1;
    }
        ...
        returns (uint256 pending)
    {
        ...
        uint256 end = currentRound == 1 ? 1 : currentRound.sub(1);
        ...
        if (pool.cumulativeMoneyPerShare.length < end)
            end = pool.cumulativeMoneyPerShare.length;
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (farmAmount != 0) {
            ...
            } else if (currentRound > pool.deposits.length) {
                pool.deposits.push(
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        uint256 pendingRewards = pendingMoney(_pid, msg.sender);
        uint256 currentRound = getCurrentRoundId(_pid);
        uint256 withdrawFor = currentRound == 1 ? 1 : currentRound.sub(1);
    ...
    function updatePool(uint256 _pid) public {
        require(
            rewards.length != 0,
            "Farming:updatePool:: REWARDS_NOT_COLLECTED_YET"
        );

        PoolInfo storage pool = poolInfo[_pid];

        uint256 lastUpdatedPoolIndex = pool.cumulativeMoneyPerShare.length;

        // gives global pool id for last index pool was updated
        uint256 rewardIndex = pool.globalRoundId.add(lastUpdatedPoolIndex).sub(
            1
        );
        ...
        for (uint256 round = 1; round <= totalRounds; round++) {
            uint256 share = (
                (rewards[rewardIndex.add(round)].mul(pool.allocPoint))
                    .div(totalAllocPoint)
                    .mul(REWARD_PRECISION)
                    .div(pool.deposits[lastUpdatedPoolIndex.add(round)])
            );

            // add last money per share to avoid loops in withdraw
            if (pool.cumulativeMoneyPerShare.length != 0)
                share = share.add(
                    pool.cumulativeMoneyPerShare[
                        lastUpdatedPoolIndex.add(round).sub(1)
                    ]
                );
