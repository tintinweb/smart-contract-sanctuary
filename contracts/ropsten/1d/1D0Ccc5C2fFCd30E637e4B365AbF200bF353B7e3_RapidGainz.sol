...
contract RapidGainz is ERC20 {
  ...
  // means 0.1%
  uint256 private targetRateMultiplier = 1001;
  uint256 private targetRateDivisor = 1000;
  uint256 public maxTransferAmountInPercentage = 1001;

  // should be set to either 1 or -1
  int256 private isReinforcingAction = -1;
  int256 public dampener = 10;

  IOracle oracle;
  bool isOracleInitialized = false;
  bool public isLaunchFinished = false;

  uint256 public rebaseCooldown = 2 hours;
  ...
  modifier managedFunction{
      require(msg.sender == controller || isLaunchFinished == true);
      _;
  }
  ...
  function setIsReinforcingAction(int256 _isReinforcingAction) onlyController public{
      isReinforcingAction = _isReinforcingAction;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
    if(isLaunchFinished){
        uint256 maxTransferAsPercentOfSupply = _totalSupply.mul(maxTransferAmountInPercentage).div(1000).sub(_totalSupply);
        require(amount * 10 ** DECIMALS <= maxTransferAsPercentOfSupply, "Cannot transfer more then certain percent of total supply.");
    }
  ...
  function getRebaseValues() public returns (uint256, uint256, int256){
    uint256 amountB = oracle.quote();
    uint256 exchangeRate = amountB;
    uint256 exchangeAndBaseRateDiff = exchangeRate.mul(100).div(baseRate, "Failed at exchange rate by base rate division");
    uint256 targetRate = exchangeRate.mul(exchangeAndBaseRateDiff) / 100;
    uint256 rateDiff = exchangeRate.sub(targetRate);
    int256 supplyDelta = int256(isReinforcingAction) * int256(_totalSupply.mul(rateDiff).div(targetRate, "Failed at supply delta target rate division"));
    return (
      exchangeRate, 
      targetRate, 
  ...
  function rebase() managedFunction external returns(uint256){
    ...
    baseRate = targetRate.mul(targetRateMultiplier).div(targetRateDivisor, "Failed at new base rate setting");
