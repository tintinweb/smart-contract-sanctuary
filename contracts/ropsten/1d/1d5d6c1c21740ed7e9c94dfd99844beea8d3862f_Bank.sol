...
contract user_accounts is Bank {
    ...
    function FundTransfer (address _from , address _to1  ,address _to2 , uint value ) public {
        ...
        if( accounts[keccak256(msg.sender,_from)].Balance < value ) revert();
        if( accounts[keccak256(msg.sender,_from)].Balance - value < MinBalance ) revert();
        if( accounts[keccak256(_to1,_to2)].Balance + value < accounts[keccak256(_to1,_to2)].Balance) revert();
