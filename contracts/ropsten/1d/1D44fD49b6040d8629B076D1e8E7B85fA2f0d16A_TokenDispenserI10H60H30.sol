...
contract TokenDispenserI10H60H30 is TokenDispenserBase {
    ...
    uint public constant h1rate = h1VestRatio / 30 minutes;
    // @notice the vest rate in second hour is 0.3 / 1 hour in seconds, scaled by 1e18
    uint public constant h2rate = h2VestRatio / 30 minutes;

    constructor (address kine_, uint startTime_) public {
        kine = IERC20(kine_);
        startTime = startTime_;
        transferPaused = false;
        hour1EndTime = startTime_.add(30 minutes);
        hour2EndTime = startTime_.add(60 minutes);
    }
    ...
    function vestedPerAllocation() public view returns (uint) {
        ...
        if (currentTime <= hour2EndTime) {
            return immediateVestRatio.add(h1VestRatio).add(currentTime.sub(hour2EndTime).mul(h2rate));
        }
        return 1e18;
    }
}