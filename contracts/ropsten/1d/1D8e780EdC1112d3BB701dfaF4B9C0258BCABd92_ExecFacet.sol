...
import {IGelatoCoreV1} from "../../../interfaces/gelato/IGelatoCoreV1.sol";
...
contract ExecFacet is BFacetOwner, BFacetExecutors {
    ...
    address public immutable GELATO_V1;
    address public immutable GELATO_PROVIDER;

    constructor(address _gelatoV1, address _gelatoProvider) {
        GELATO_V1 = _gelatoV1;
    ...
    function exec(
        ...
        if (_service == GELATO_V1)
            _withdrawExcessExecutorStake(type(uint256).max);
        ...
        if (_service == GELATO_V1) _providerRefund(_receivedFee, _data);
    ...
    function _providerRefund(uint256 _amount, bytes calldata _data) private {
        address provider = abi.decode(_data[68:100], (address));
        if (provider == GELATO_PROVIDER) {
            // solhint-disable no-empty-blocks
            try
                IGelatoCoreV1(GELATO_V1).provideFunds{value: _amount}(
                    GELATO_PROVIDER
                )
            {} catch {
                revert("ExecFacet:_providerRefund: failed");
            }
        }
    }

    function _withdrawExcessExecutorStake(uint256 _withdrawAmount) private {
        // solhint-disable no-empty-blocks
        try
            IGelatoCoreV1(GELATO_V1).withdrawExcessExecutorStake(
                _withdrawAmount
            )
        {} catch {
            revert("ExecFacet:_withdrawExcessExecutorStake: failed");
        }
    }
...
function _getGelatoFee(
    ...
    gelatoFee =
        ...
        _getGasPrice(IGelatoDiamond(_gelato).getGasPriceOracle());
...
function _getGasPrice(address _gasPriceOracle) view returns (uint256) {
