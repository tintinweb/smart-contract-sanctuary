...
contract LiquidityBalancer is IGovernable, Controllable {
  ...
  // sellAmount should have target token decimals
  // remAmount should have LP token decimals
  // however targetPrice and targetTvl should have PRECISION_DECIMALS
  ...
  public onlyHardWorkerOrGovernance {
    ...
    (address oppositeToken, uint256 oppositeTokenStacked, , uint256 tokenStaked)
    = getLpInfo(_lp, _token);
    uint256 currentTvl = oppositeTokenStacked.mul(2);

    uint256 sellAmount = computeSellAmount(tokenStaked, oppositeTokenStacked, priceTargets[_token]);
    // don't sell more than a half for adding a liquidity after sell
    sellAmount = Math.min(sellAmount, tokenBalance.div(uint256(2)));
    uint256 remAmount = computeRemAmount(currentTvl, _lp);
    remAmount = Math.min(remAmount, lpBalance);

//    bool needToSell = sellAmount > 0 && tokenBalance.div(uint256(2)) >= sellAmount && price > priceTargets[_token];
//    bool needToRemove = remAmount > 0 && lpBalance >= remAmount && currentTvl > lpTvlTargets[_lp];

    if (needToRemove(_token, _lp)) {
  ...
  function needToSell(address _token, address _lp) public view returns (bool) {
    uint256 tokenBalance = IERC20(_token).balanceOf(address(this));
    (, uint256 oppositeTokenStacked, uint256 price, uint256 tokenStaked) = getLpInfo(_lp, _token);

    uint256 sellAmount = computeSellAmount(tokenStaked, oppositeTokenStacked, priceTargets[_token]);
    // don't sell more than a half for adding a liquidity after sell
    sellAmount = Math.min(sellAmount, tokenBalance.div(uint256(2)));

    return sellAmount > 0 && tokenBalance.div(uint256(2)) >= sellAmount && price > priceTargets[_token];
  }

  function needToRemove(address _token, address _lp) public view returns (bool) {
    uint256 lpBalance = IERC20(_lp).balanceOf(address(this));
    ...
    return remAmount > 0 && lpBalance >= remAmount && currentTvl > lpTvlTargets[_lp];
  }
  ...
  function addLiquidity(address _lp, address token0, address token1) internal {
    ...
    IERC20(token0).approve(routerAddress, 0);
    IERC20(token0).approve(routerAddress, amount0);
    IERC20(token1).approve(routerAddress, 0);
    IERC20(token1).approve(routerAddress, amount1);
