...
// Contract created specifically for ModelCoin by https://gentokens.com/ 
...
contract ModelCoin is Context, IERC20 { 
    ...
    uint8 public _FeeReflection = 1; 
    uint8 public _FeeLiquidity = 2;
    uint8 public _FeeMarketing = 6;
    uint8 public _FeeDev = 1; 

    uint8 public constant _FeeMaxPossible = 10;

    // 'Previous fees' are used to keep track of fee settings when removing and restoring fees
    uint8 private _previousFeeReflection = _FeeReflection;
    uint8 private _previousFeeLiquidity = _FeeLiquidity;
    uint8 private _previousFeeMarketing = _FeeMarketing;
    uint8 private _previousFeeDev = _FeeDev; 

    // The following settings are used to calculate fee splits when distributing bnb to liquidity and external wallets
    uint8 private _promoFee = _FeeMarketing+_FeeDev;
    uint8 public _FeesTotal = _FeeMarketing+_FeeDev+_FeeLiquidity+_FeeReflection;

    // Fee for the auto LP and the all bnb wallets - used to process fees 
    uint8 private _liquidityAndPromoFee = _FeeMarketing+_FeeDev+_FeeLiquidity;
    ...
    */
    ...
    event E_set_Fees(uint8 Liquidity, uint8 Marketing, uint8 Reflection);


    function _set_Fees(uint8 Liquidity, uint8 Marketing, uint8 Reflection) external onlyOwner() {
    ...
    */
    ...
    event E_set_Number_Of_Transactions_Before_Liquify_Trigger(uint8 number_of_transactions);
    ...
    function set_Number_Of_Transactions_Before_Liquify_Trigger(uint8 number_of_transactions) external onlyOwner {
        ...
        emit E_set_Number_Of_Transactions_Before_Liquify_Trigger(number_of_transactions);
    }
    ...
    */
    ...
    event E_set_Max_Transaction_Percent_X100(uint256 _maxTxAmount);
    ...
    function set_Max_Transaction_Percent_X100(uint256 max_Transaction_Percent) external onlyOwner() {
        ...
        emit E_set_Max_Transaction_Percent_X100(_maxTxAmount);
    }
    
    
    // Set the maximum permitted wallet holding (percent of total supply x 100)
    event E_set_Max_Wallet_Holding_Percent_X100(uint256 _maxWalletToken);
    ...
    function set_Max_Wallet_Holding_Percent_X100(uint256 max_Wallet_Holding_Percent) external onlyOwner() {
        ...
        emit E_set_Max_Wallet_Holding_Percent_X100(_maxWalletToken);
    }
        ...
        uint256 amount
          ...
          if (launchPhase){
                ...
                if(TradeOpen){
                        ...
                        if ((block.number > launchBlock + 5) && (_maxTxAmount != _tTotal.mul(2).div(100))){
