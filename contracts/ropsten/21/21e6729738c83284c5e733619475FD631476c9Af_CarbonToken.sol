...
contract CarbonToken is Ownable, ERC1155Base {
    string public name;
    string public symbol;

    constructor(string memory _name, string memory _symbol, address newOwner, string memory contractURI, string memory tokenURIPrefix) ERC1155Base(contractURI, tokenURIPrefix) {
        require(bytes(_name).length > 0, "required _name");
        require(bytes(_symbol).length > 0, "required _symbol");
        require(bytes(tokenURIPrefix).length > 0, "required tokenURIPrefix");
        require(bytes(contractURI).length > 0, "required contractURI");
