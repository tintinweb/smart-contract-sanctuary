# @version 0.2.8
# (c) CoinFLEX
# Pool for FlyUSD/FlyUSD2
'''
@title StableSwapPair
@author CoinFLEX
@license Copyright (c) CoinFLEX, 2021 - all rights reserved
@notice Pool for swapping between two CoinFLEX tokens
@dev Swaps between FlyUSD and FlyUSD2
'''
...
PRECISION_MUL: constant(uint256[N_COINS]) = [10000000000000000, 10000000000000000]
RATES: constant(uint256[N_COINS]) = [10000000000000000000000000000000000, 10000000000000000000000000000000000]
...
@pure
@internal
def _balances_mem(_balances: uint256[N_COINS]) -> uint256[N_COINS]:
  result: uint256[N_COINS] = RATES
  for i in range(N_COINS):
...
def get_D_mem(_balances: uint256[N_COINS], amp: uint256) -> uint256:
  return self.get_D(self._balances_mem(_balances), amp)
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256) -> uint256:
  ...
  assert D1 > D0 # dev: D1 is less than D0
...
def exchange(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
  ...
  assert not self.is_killed  # dev: is killed
  old_balances: uint256[N_COINS] = self.balances
  xp: uint256[N_COINS] = self._balances_mem(old_balances)
  rates: uint256[N_COINS] = RATES
  x: uint256 = xp[i] + dx * rates[i] / PRECISION
  y: uint256 = self.get_y(i, j, x, xp)
  ...
  assert ERC20(self.coins[i]).transferFrom(msg.sender, self, dx) # dev: failed transfer
  assert ERC20(self.coins[j]).transfer(msg.sender, dy) # dev: failed transfer    
...
def remove_liquidity(
  ...
  for i in range(N_COINS):
    ...
    amounts[i] = value
    assert ERC20(self.coins[i]).transfer(msg.sender, value) # dev: failed transfer
...
def remove_liquidity_imbalance(
  ...
  for i in range(N_COINS):
    if _amounts[i] != 0:
      assert ERC20(self.coins[i]).transfer(msg.sender, _amounts[i]) # dev: failed transfer
...
def remove_liquidity_one_coin(
  ...
  assert ERC20(self.coins[i]).transfer(msg.sender, dy) # dev: failed transfer
...
def withdraw_admin_fees():
  ...
  for i in range(N_COINS):
    ...
    if value > 0:
      assert ERC20(coin).transfer(msg.sender, value) # dev: failed transfer
