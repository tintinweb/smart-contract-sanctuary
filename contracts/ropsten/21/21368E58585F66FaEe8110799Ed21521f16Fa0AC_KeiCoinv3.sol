...
contract KeiCoinv3 is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 50000 * 10**6 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = 'KeiCoin';
    ...
    uint256 private _txFee = 0;
    uint256 private _previousTaxFee = _txFee;
    uint256 private _burnFee = 0;
    uint256 private _previousburnFee = _burnFee;
    uint256 public _totalFeePercentage = _txFee + _burnFee;
    ...
    function reflect(uint256 tAmount) public {
        ...
        (uint256 rAmount,,,) = _getValues(tAmount, _totalFeePercentage);
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (,uint256 rTransferAmount,,) = _getValues(tAmount, _totalFeePercentage);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 tTransferAmount, uint256 currentRate) = _getValues(amount, _totalFeePercentage);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        ...
        uint256 tFee = amount.mul(_txFee).div(100);
    ...
    function _getValues(uint256 tAmount, uint256 totalFeePercentages) private view returns (uint256, uint256, uint256, uint256) {
        uint256 currentRate =  _getRate();

        uint256 totalFee = tAmount.mul(totalFeePercentages).div(100);
