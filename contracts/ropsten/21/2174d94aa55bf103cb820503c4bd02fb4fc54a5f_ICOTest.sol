...
interface IFarmerCommon {
	...
	function getUserInfo(address adr) external returns(uint256, uint256, uint256); // gives total deposiuted, total withdrawn, total referrals
	function getUserReferralBonus(address adr) external returns(uint256); // referral bonus available , withdraw function gets both withdraw avaiable + referral avaiable
...
contract ICOTest{
	...
	function getFarmerData2( address adr, address contract_address) public  returns(uint256, uint256,uint256, uint256,uint256) {							
				uint256 totalDeposit; uint256 totalWithdrawn; uint256 totalReferrals;
				(totalDeposit, totalWithdrawn, totalReferrals) = IFarmerCommon(contract_address).getUserInfo(adr);
				return ( 
				totalDeposit, totalWithdrawn, totalReferrals,				
