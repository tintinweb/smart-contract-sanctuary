...
contract OWN{
    address payable public owner;address payable public newOwner;
...
contract ALFA is ERC20, OWN{
    ...
    constructor()public payable{
        owner=msg.sender;
        balances[owner]=10000000*10**18;balances[address(this)]=90000000*10**18;
    ...
    function totalSupply()public returns(uint){return _totalSupply;}
    
    function balanceOf(address tokenOwner)public returns(uint256 balance){return balances[tokenOwner];}
    ...
    function approveAndCall(address spender, uint256 tokens, bytes memory data)public returns(bool success){
        ...
        ApproveAndCallFallBack(spender).receiveApproval(msg.sender,tokens,address(this),data);return true;}
    ...
    function bytesToAddress(bytes memory source)internal pure returns(address addr){assembly{addr:=mload(add(source,0x14))}return addr;}
