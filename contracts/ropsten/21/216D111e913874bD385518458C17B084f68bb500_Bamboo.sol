...
contract Bamboo is ERC20("Bamboo Token", "$BAMBOO"), Ownable {
    ...
    uint256 public claimUnitPerPeriod = 10 ** 18;
    ...
    mapping(uint256 => bool) public claimStarted;	
    ...
    function _claimReward(uint256 _tokenId) internal returns (uint256) {            
        uint256 claimAmount = claimUnitPerPeriod;
        if (claimStarted[_tokenId]){
            claimAmount = now.sub(lastClaimedTime[_tokenId]).mul(claimUnitPerPeriod).div(claimPeriod);    
        } else {
            claimStarted[_tokenId] = true;
        }            
    ...
    function claim() external {
       ...
       for (uint256 i=0; i < tokenCount; i++){
           ...
           amount = amount.add(_claimReward(tokenId));
       }
    ...
    function _calcClaimableAmount(uint256 _tokenId) internal view returns(uint256){              
        uint256 claimAmount = claimUnitPerPeriod;
        if (claimStarted[_tokenId]){
            claimAmount = now.sub(lastClaimedTime[_tokenId]).mul(claimUnitPerPeriod).div(claimPeriod);    
        } 
    ...
    function getHolderClaimableAmountForTokenId(address _holder, uint256 _tokenId) public view returns(uint256) {
        ...
        uint amount = _calcClaimableAmount(_tokenId);
        return amount; 
	}
    ...
    function getHolderClaimableTotalAmount(address _holder) public view returns(uint256) {
        ...
        for (uint256 i=0; i < tokenCount; i++){
           ...
           uint amount = _calcClaimableAmount(tokenId);
