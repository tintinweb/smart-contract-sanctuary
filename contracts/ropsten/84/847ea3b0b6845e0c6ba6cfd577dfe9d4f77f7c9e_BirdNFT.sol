...
contract BirdNFT is ERC721, Ownable {
    ...
    uint256 public purchasedBirdNFTs = 0;
    ...
    //purchase multiple BirdNFT at once
    function purchaseBirdNFT(uint256 _howMany) external payable {
        require(_howMany > 0, "Minimum 1 tokens need to be minted");
        require(
            _howMany <= BirdNFTRemainingToBeMinted(),
            "Purchase amount is greater than the token available"
        );
        require(isSaleActive, "Sale is not active");
        require(_howMany <= 20, "max 20 purchasedBirdNFT at once");
        require(itemPrice * _howMany == msg.value, "Insufficient ETH to mint");
        for (uint256 i = 0; i < _howMany; i++) {
            _mintBirdNFT(_msgSender());
        }
    }
    ...
    function _mintBirdNFT(address _to) private {
        if (purchasedBirdNFTs == 0) {
            _lastIpfsId = random(
                1,
                totalTokenToMint,
        ...
        purchasedBirdNFTs++;
        require(!_exists(purchasedBirdNFTs), "Mint: Token already exist.");
        _mint(_to, purchasedBirdNFTs);
        _tokenURIs[purchasedBirdNFTs] = _lastIpfsId;
    }

    // Gift NFT to people
    function gift(address[] calldata to) external onlyOwner {
        require(
            to.length <= BirdNFTRemainingToBeMinted(),
    ...
    function BirdNFTRemainingToBeMinted() public view returns (uint256) {
        return totalTokenToMint - purchasedBirdNFTs;
    }

    function isAllTokenMinted() public view returns (bool) {
        return purchasedBirdNFTs == totalTokenToMint;
    }
    ...
    function getIpfsIdToMint() private view returns (uint256 _nextIpfsId) {
        ...
        if (
            ...
            purchasedBirdNFTs < totalTokenToMint
        ) {
            _nextIpfsId = 1;
        } else if (purchasedBirdNFTs < totalTokenToMint) {
