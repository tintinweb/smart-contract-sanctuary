...
contract MON is ERC20, ERC20Burnable, Ownable {
    ...
    address constant public THEMOONMONSTERAddress = 0x089409e540fd0Da934a95D28fFf8e748D466c29B;
    ...
    function setSaleState(bool newState) public onlyOwner {
     ...
     function totalAvailable(address ownwer) public view returns (uint256) {
        uint[] memory tokens = THEMOONMONSTER(THEMOONMONSTERAddress).tokensOfOwner(ownwer);
        uint arrayLength = tokens.length;
        uint256 rewards = 0;
        uint256 time = block.timestamp;
        for (uint i=0; i<arrayLength; i++) {
              uint256 tokenid = tokens[i];
              uint256 lastTime = lastUpdate[tokenid];
              if (lastTime > 0){
    ...
    function claim() public payable  {
        ...
        uint[] memory tokens = THEMOONMONSTER(THEMOONMONSTERAddress).tokensOfOwner(msg.sender);
        uint arrayLength = tokens.length;
    ...
    function tokenMon(uint tokenid) public view returns(uint256) {
         uint256 rewards = 0;
         uint256 time = block.timestamp;
         uint256 lastTime = lastUpdate[tokenid];
         if (lastTime > 0){
