...
/// WTM token, ERC20 compliant
contract WTM is Token, Owned {
    using SafeMath for uint256;
 
    string public constant name    = "wtml.finance";  //The Token's name
    uint8 public constant decimals = 18;               //Number of decimals of the smallest unit
    string public constant symbol  = "WTM";            //An identifier   
    
    
    // packed to 256bit to save gas usage.
    struct Supplies {
        // uint128's max value is about 10000.
    ...
    function transfer(address _to, uint256 _amount) returns (bool success) {
        ...
        // according to WTM's total supply, never overflow here
...
// Contract to sell and distribute WTM tokens
contract WTMSale is Owned{
    ...
    uint256 public constant totalSupply         = (10000) * (10 ** 18); // 1 billion WTM, decimals set to 18
    
    uint256 constant privateSupply              = totalSupply * 80 / 100;  // 80% for private ICO
    uint256 constant commercialPlan             = totalSupply * 5 / 100; // 5% for commercial plan
    uint256 constant reservedForTeam            = totalSupply * 5 / 100;  // 5% for team
    uint256 constant reservedForOperations      = totalSupply * 10 / 100; // 10 for operations
    ...
    uint256 public constant officialLimit = 0;
    ...
    uint256 constant wtmPerEth = 20;  // normal exchange rate
    uint256 constant wtmPerEthEarlyStage = wtmPerEth + wtmPerEth * 15 / 100;  // early stage has 15% reward
 
    uint constant minBuyInterval = 10 minutes; // each account can buy once in 10 minutes
    uint constant maxBuyEthAmount = 2 ether;
   
    WTM wtm; // WTM token contract follows ERC20 standard
 
    address ethVault; // the account to keep received ether
    address wtmVault; // the account to keep non-public offered WTM tokens
 
    uint public constant startTime = 1616068800; // time to start sale
    uint public constant endTime = 1630152000;   // tiem to close sale
    uint public constant earlyStageLasts = 1 days; // early bird stage lasts in seconds
    ...
    function buy() payable {
        ...
        require(blockTime() >= wtm.lastMintedTimestamp(msg.sender) + minBuyInterval);
        ...
        if (requested > 0) {
            wtm.mint(msg.sender, requested, true, blockTime());
    ...
    function offerToChannel(address _channelAccount, uint256 _wtmAmount) onlyOwner {
        Stage stg = stage();
        ...
        soldOut.channels = _wtmAmount.add(soldOut.channels).toUINT120();
        ...
        onSold(_channelAccount, _wtmAmount, 0);
    }
 
    /// @notice initialize to prepare for sale
    /// @param _wtm The address WTM token contract following ERC20 standard
    /// @param _ethVault The place to store received ETH
    /// @param _wtmVault The place to store non-publicly supplied WTM tokens
    function initialize(
        WTM _wtm,
        address _ethVault,
        address _wtmVault) onlyOwner {
        require(stage() == Stage.Created);
 
        // ownership of token contract should already be this
        require(_wtm.owner() == address(this));
 
        require(address(_ethVault) != 0);
        require(address(_wtmVault) != 0);      
    ...
    event onSold(address indexed buyer, uint256 wtmAmount, uint256 ethCost);
}