...
abstract contract BRingFarmAdmin is Ownable, Pausable {
    ...
    mapping (address => mapping (address => uint256)) public tokenDistributionByPeriod;
    mapping (address => mapping (address => bool)) public tokenBlockedStatus;
    uint256[] public intervalDays = [1, 2, 3, 4, 5];
    ...
    event DistributionByPeriodDetails(
    ...
    function setDistributionByPeriod(
        address[] memory stakedToken,
        address[] memory rewardToken,
        uint256[] memory distributionList
    ) public onlyOwner {
        require(
            stakedToken.length == rewardToken.length &&
                rewardToken.length == distributionList.length,
            "Invalid Input"
        );
        ...
        for (uint8 i = 0; i < stakedToken.length; i++) {
            ...
            tokenDistributionByPeriod[stakedToken[i]][
                rewardToken[i]
            ] = distributionList[i];
            
            emit DistributionByPeriodDetails(
                stakedToken[i],
                rewardToken[i],
                distributionList[i],
                block.timestamp
...
contract BRingFarm is BRingFarmAdmin {
    ...
    function claimRewards(address userAddress, uint256 stakeId, uint256 stakedAmount, uint256 totalStake) internal {
     ...
     function _rewardCalculation(
        ...
        while (i < intervalDays.length
            && i < tokensSequenceList[stakingDetails[userAddress].tokenAddress[stakeId]].length)
        {    
    ...
    function getRewardForPeriod(
        ...
        reward = (stakedAmount.mul(tokenDistributionByPeriod[stakedToken][rewardToken])).div(totalStake);
    }
