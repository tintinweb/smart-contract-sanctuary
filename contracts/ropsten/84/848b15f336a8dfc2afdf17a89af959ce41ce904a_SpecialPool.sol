...
contract SpecialPool is Ownable {
    ...
    function unStake(uint256 stakeID, uint256 _amount) external validateStake(stakeID) {
        UserData storage stake = userData[stakeID];
        require(stake.tokenQuantity > _amount, "Insufficient amount");
        uint256 pendingReward = calculateRewards(stakeID).sub(stake.rewardDebt);
        
        require(IBEP20(TOKEN).balanceOf(address(this)) >= pendingReward, "Insufficient balance for reward");
        
        stake.rewardDebt = calculateRewards(stakeID);
        totalStaking = totalStaking.sub(stake.tokenQuantity);
        stake.tokenQuantity = stake.tokenQuantity.sub(_amount);
        stake.updatedTimestamp = now;
        
        safeTokenTransfer(msg.sender, _amount.add(pendingReward));
    ...
    function calculateRewards(uint256 stakeID) public view returns(uint256 reward) {
        UserData storage stake = userData[stakeID];
        // get Interval time
        uint256 time = block.timestamp.sub(stake.updatedTimestamp);
        // Gte number of minutes
        uint256 day = time.div(1 minutes);
        // Reward within 30 minutes
        if(day < 30 minutes) {
        ...
        // Reward after 30 minutes
        if(day > 30 minutes && day < 90 minutes) {
