...
contract DaluaNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    uint256 PLATFORM_SHARE_PERCENT = 2;
    uint256 ROYALTY_PERCENT = 2;
    ...
    function transferByAcceptOffer(uint256 tokenId, address newOwner, bytes32 hash, bytes memory signature, uint payThrough) external payable onlyNftOwner(tokenId) {
        ...
        uint256 twoPoint5P = calculatePercentValue(msg.value, PLATFORM_SHARE_PERCENT);
        uint256 amountToTransfer = msg.value-twoPoint5P;
        transferwethToOwner(newOwner, amountToTransfer, msg.sender);
