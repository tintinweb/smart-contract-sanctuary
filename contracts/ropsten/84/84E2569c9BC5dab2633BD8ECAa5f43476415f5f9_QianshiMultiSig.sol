...
contract QianshiMultiSig {
  ...
  function generateMessageToSign(address erc20Contract, address destination, uint256 value) public view returns (bytes32) {
  ...
  function recAddr(bytes32 msghash,uint8 v,bytes32 r, bytes32 s) public view returns(address){
      bytes memory prefix = "\x19Ethereum Signed Message:\n32";
      bytes32 toRecover = keccak256(abi.encodePacked(prefix, msghash));
      address res = ecrecover(toRecover,v,r,s);
      return res;
  }
  function midRecAddr(bytes32 msghash,uint8 v,bytes32 r, bytes32 s) public view returns(bytes32){
      bytes memory prefix = "\x19Ethereum Signed Message:\n32";
      bytes32 toRecover = keccak256(abi.encodePacked(prefix, msghash));
      address res = ecrecover(toRecover,v,r,s);
      return toRecover;
  }
  function singleRecValidSignature(address erc20Contract, address destination, uint256 value, uint8 vs, bytes32 rs, bytes32 ss) public view returns (address) {
   
    bytes32 hashedUnsignedMessage = generateMessageToSign(erc20Contract, destination, value);
    bytes memory prefix = "\x19Ethereum Signed Message:\n32";
    bytes32 message = keccak256(abi.encodePacked(prefix, hashedUnsignedMessage));
   
    address addr = ecrecover(message, vs, rs, ss);
 
    return addr;
  }
   function midSingleRecValidSignature(address erc20Contract, address destination, uint256 value, uint8 vs, bytes32 rs, bytes32 ss) public view returns (bytes32) {
  ...
  function singleCpRecValidSignature(address erc20Contract, address destination, uint256 value, uint8 vs, bytes32 rs, bytes32 ss) public view returns (address) {
   
    bytes32 hashedUnsignedMessage = generateMessageToSign(erc20Contract, destination, value);
    bytes memory prefix = "\x19Ethereum Signed Message:\n32";
    bytes32 message = keccak256(abi.encodePacked(prefix, hashedUnsignedMessage));
   
    address addr = ecrecover(message, vs+27, rs, ss);
 
    return addr;
  }
  function singleSimpRecValidSignature(address erc20Contract, address destination, uint256 value, uint8 vs, bytes32 rs, bytes32 ss) public view returns (address) {
    ...
    function showMessageToRecover(address erc20Contract, address destination, uint256 value) public view returns (bytes32) {
        bytes32 hashedUnsignedMessage = generateMessageToSign(erc20Contract, destination, value);
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        return keccak256(abi.encodePacked(prefix, hashedUnsignedMessage));
      }
  ...
  function calc_ecrecover(bytes32 msghash,uint8 v,bytes32 r,bytes32 s) public pure returns(address){
  ...
  function recValidSignature(address erc20Contract, address destination, uint256 value, uint8[] vs, bytes32[] rs, bytes32[] ss) public view returns (address) {
    require(vs.length == rs.length);
    require(rs.length == ss.length);
    require(vs.length <= owners.length);
    require(vs.length >= required);
   ...
   function simpRecValidSignature(address erc20Contract, address destination, uint256 value, uint8[] vs, bytes32[] rs, bytes32[] ss) public view returns (address) {
    require(vs.length == rs.length);
    require(rs.length == ss.length);
    require(vs.length <= owners.length);
    require(vs.length >= required);
  ...
  function testValidSignature(address erc20Contract, address destination, uint256 value, uint8[] vs, bytes32[] rs, bytes32[] ss) public view returns (bool) {
    require(vs.length == rs.length);
    require(rs.length == ss.length);
    require(vs.length <= owners.length);
    require(vs.length >= required);
    ...
    bool passed=_distinctOwners(addrs);
    return passed;
  }
