...
contract CyberKeys is ERC1155Supply, Ownable {
    ...
    mapping(uint256=>uint8) public remainingAllowance;
    ...
    constructor (address cybbAddress) ERC1155("https://cyberbabies.io/api/keyToken/{id}.json") {
        ...
        //     remainingAllowance[i] = 3;
        // }
        // setRemainingAllowance(robots, 12);
        // setRemainingAllowance(reptiles, 10);
        // setRemainingAllowance(zombies, 8);
        // setRemainingAllowance(witches, 7);
        // setRemainingAllowance(aliens, 6);
    ...
    function setRemainingAllowance(uint256[] calldata cybbIds, uint8 newAllowance) public onlyOwnerOrManager {
        for (uint256 i; i < cybbIds.length; i++) {
            remainingAllowance[cybbIds[i]] = newAllowance;
        }
    }
    ...
    function getTotalAllowance(uint256[] memory cybbIds) public view returns(uint8 totalAlllowance){
        ...
        for (uint256 i; i < cybbIds.length; i++) {
            totalAllowance += remainingAllowance[cybbIds[i]];
        }
    ...
    function claimAllowanceKeys(uint256 cybbId, uint8 amount) public payable allowanceSaleIsLive {
        ...
        require(amount <= remainingAllowance[cybbId], "Not enough allowance remaining for this id");

        claimedSupply += amount;
        remainingAllowance[cybbId] = remainingAllowance[cybbId] - amount;
    ...
    function multiClaimAllowanceKeys(uint256[] calldata cybbIds, uint8 amount) public payable allowanceSaleIsLive {
        ...
        for (uint8 i; i < cybbIds.length; i++){
            ...
            totalAllowance += remainingAllowance[cybbIds[i]];
            if (totalAllowance == amount || totalAllowance < amount) remainingAllowance[cybbIds[i]] = 0;
            else if (totalAllowance > amount){
                remainingAllowance[cybbIds[i]] = totalAllowance - amount;
                break;
            }
        }
        claimedSupply += amount;
