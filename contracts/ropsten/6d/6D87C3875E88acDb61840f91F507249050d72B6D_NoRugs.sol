...
contract NoRugs is Context, IERC20, Ownable {
    ...
    string private constant _name = "NoRugs.org";
    string private constant _symbol = "NORUGSORG";
    uint8 private constant _decimals = 9;
    uint256 private constant _tTotalSupply = 100000000000 * 10 ** 9; 

	uint256 private constant _buyMaxTxAmount = 500000000 * 10 ** 9; 
    uint256 private constant _sellMaxTxAmount = 500000000 * 10 ** 9;
    ...
    // Ropsten, Rinkeby, Kovan, ETH Mainnet - UniSwap Router V2
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            ...
            _tv.tTransferAmount = 0; _tv.tLiquidity = 0;
            _rv.rTransferAmount = 0; _rv.rLiquidity = 0;
            return _rv.rAmount;
        } else {
            (rValues memory _rv, tValues memory _tv) = _getValues(tAmount); 
            _tv.tTransferAmount = 0; _tv.tLiquidity = 0; 
    ...
    function _getValues(uint256 tAmount) private view returns (rValues memory, tValues memory) {
        ...
        rValues memory _rv = _getRValues(tAmount, _tv.tLiquidity, _getRate());
        return (_rv, _tv);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (tValues memory) {
        ...
        _tv.tTransferAmount = tAmount.sub(_tv.tLiquidity);
        return (_tv);
    }

    function _getRValues(uint256 tAmount, uint256 tLiquidity, uint256 currentRate) private pure returns (rValues memory) {
        rValues memory _rv;
        _rv.rAmount = tAmount.mul(currentRate);
        _rv.rLiquidity = tLiquidity.mul(currentRate);
        _rv.rTransferAmount = _rv.rAmount.sub(_rv.rLiquidity);
        return (_rv);
    }
