...
contract VerifierSignature {
    function getSwapMessageHash(
        ...
        return keccak256(abi.encodePacked(token0, token0Amount, token1, token1Amount, to, data, allowanceTarget, _nonce));
    }
    ...
    function verifySwap(
        ...
        bytes32 messageHash = getSwapMessageHash(token0, token0Amount, token1, token1Amount, to, data, allowanceTarget, _nonce);
...
contract Withdraw is VerifierSignature, Ownable {
  ...
  function withdraw(
    ...
    bytes memory signature
      ...
      require(
        verifySwap(_signer, token0, token0Amount, token1, token1Amount, to, data, allowanceTarget, _nonce, signature),
        "Invalid signature"
      );
  ...
  function transfer(
    ...
    uint commission
      ...
      uint256 fee = token0Amount.mul(commission).div(100);
      uint256 leftOver = token0Amount.sub(fee);

      TransferHelper.safeTransferFrom(token0, _signer, wallet, fee);
      TransferHelper.safeTransferFrom(token0, _signer, destination, leftOver);
  }
}
