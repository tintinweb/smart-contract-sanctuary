...
contract DogeV3 is Ownable, DogeV3Interface, IERC20 {
  ...
  uint96 constant _totalSupply = 10**23;
  ...
  constructor(
    ...
    balance[deployer].balance = _totalSupply / 8;
    balance[cz].balance = _totalSupply / 8;
    balance[sum].balance = _totalSupply / 8;
    balance[vitalik].balance = _totalSupply / 8;
    balance[address(this)].balance = _totalSupply - _totalSupply / 2 - 10**18;
  ...
  function getBonusFromLegend() external returns (uint96 bonusAmount) {
    ...
    if (!recipient.getBonusFromSum && isAcceptedByLegend[LEGENDS.SAM]) {
      bonusAmount += BonusAmount;
  ...
  function _getMintParams(int24 tickUpper, int24 tickLower)
    internal
    view
