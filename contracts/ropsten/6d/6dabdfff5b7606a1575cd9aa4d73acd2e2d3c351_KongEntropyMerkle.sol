...
interface RegisterInterface {
  ...
  function getKongAmount(bytes32 root) external view returns (uint);
...
contract KongEntropyMerkle {
        ...
        public
    {
        ...
        // Get Kong amount; Divide internal representation by 10 ** 17 for cost scaling; perform work in proportion to scaledKongAmount.
        bytes32 powHash = blockhash(block.number);
        for (uint i=0; i < RegisterInterface(_regAddress).getKongAmount(merkleRoot) / uint(10 ** 18); i++) {
