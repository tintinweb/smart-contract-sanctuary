...
// File: contracts/PeachPresale.sol
...
contract PeachPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale Peach, 1 BUSD
	uint256 BUSDPerToken = 1;
	// Amount of BUSD received in presale
	uint256 busdReceived = 0;

    uint256 maxHardCap = 10 * 10 ** 4 * 10 ** 18;

    uint256 public minimumDepositEthAmount = 5 * 10 ** 18;
    uint256 public maximumDepositEthAmount = 2500 * 10 ** 18;
	...
	function buy(uint256 _amount) public nonReentrant {
		...
		require (busdReceived + _amount <= maxHardCap, "Presale hardcap reached");
        require(_amount >= minimumDepositEthAmount, "low amount than min");
        require(_amount + (tokensOwned[msg.sender] * BUSDPerToken) <= maximumDepositEthAmount, "high amount than max");
	...
	function getPeachTokensLeft() external view returns (uint256) {
		return Peach.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some Peach tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed Peach tokens");
		require(Peach.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough Peach tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		Peach.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldPeach() external onlyOwner {
		uint256 amount = Peach.balanceOf(address(this)) - totalTokensSold;
		Peach.safeTransfer(msg.sender, amount);
	}

	function withdrawAllPeach() external onlyOwner {
		Peach.safeTransfer(msg.sender, Peach.balanceOf(address(this)));
	}
}