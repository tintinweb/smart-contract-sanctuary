...
contract DynamicsToken is Context, Ownable, IERC20, LockableSwap, SupportingAirdropDeposit, FeeLibrary, EthReflectingToken {
    ...
    bool public _cooldownEnabled = true; // TODO - True for prod
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(buyBackEnabled && address(this).balance >= minBuyBack){
            ...
            buybackTokenTracker.liquidity = buybackTokenTracker.liquidity.add((balanceOf(address(this)).sub(contractTokenBalance)));
            ...
            if(redist >= buyback && redist >= liq){
                ...
                if(contractTokenBalance >= numTokensSellToAddToLiquidity.mul(1000)){
                    contractTokenBalance = numTokensSellToAddToLiquidity.mul(1000);
                }
...
//        address(reflectionContract).call{gas: gasleft()}(abi.encodeWithSignature("logTransactionEvent(address, address)",sender, recipient));
    ...
    function init(address reflectorAddress, address tokenInjectorAddress) public onlyOwner{
        ...
        _isExcludedFromFee[airdropTokenInjector] = true;
        address _uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        // set the rest of the contract variables
        initSellFees();
        openTrading(true);  // TODO Remove for mainnet
        runOnce = 0;
        _isExcludedFromFee[tokenInjectorAddress];
    ...
    function updateProjectWalletContract(address payable wallet) public onlyOwner {
        require(wallet != _projectWallet, "Minimum Gas must be over 200,000");
