...
contract HeadShotTracker is ERC20, Ownable {
    ...
    function increaseBalance(address payable account, uint256 addBalance) external onlyOwner {
    ...
    function decreaseBalance(address payable account, uint256 addBalance) external onlyOwner {
        uint256 currBalance = _balances[account];
        uint256 newBalance = currBalance - addBalance;
...
contract HeadShot is Context, IERC20, Ownable {
    ...
    mapping(uint256 => address) private _trackers;
    address[] private _trackerList;
    ...
    */
    ...
    function addSales(uint256 category_, uint256 code_, string memory name_,
        ...
        require(_sales[code_].code != code_, "ERR");
        ...
        if (_creator == owner() || _creator == _ecosystemWalletAddress || _creator == _salesWalletAddress){
            ...
            _trackers[code_] = trackerAddress;
            _trackerList.push(trackerAddress);
            return true;
        }
        return false;
    }
    ...
    function buy(uint256 _code, uint256 _count) public returns (bool) {
        if (_sales[_code].code == _code){
            ...
            HeadShotTracker headShotTracker = HeadShotTracker(payable(_trackers[_code]));
            headShotTracker.increaseBalance(payable(_buyer), _count);
            /*
