...
interface ICryptoAnts is IERC721, IERC721Enumerable {
  //
  // Events
  //

  // Emits the buyer and amount of eggs bought
  event EggsBought(address indexed buyer, uint256 eggsAmount);

  // Emits the owner (seller) and ID of the ant sold
  event AntSold(address indexed owner, uint256 antId);

  // Emits the owner and ID of the ant that died creating eggs
  event AntDead(address indexed owner, uint256 antId);

  // Emits the owner and amount of egss created
  event EggsCreated(address indexed owner, uint256 eggsAmount);

  // Emits the owner and ID of the ant created
  event AntCreated(address indexed owner, uint256 antId);

  // Emits the new price for an egg
  event EggPriceChanged(uint256 price);

  // Emits the new price for an ant
  event AntPriceChanged(uint256 price);

  // Emits the ether balance Withdrawn
  ...
  // Ether sent is less than the price needed
  error WrongEtherSent();

  // Sender has not sufficient eggs to create an ant
  error NoEggs();

  // Sender is not the owner of the ant
  error Unauthorized();

  // The time between eggs creation has not passed
  error WrongEggCreationTime();

  // Invalid price sent, it cannot be zero
  error WrongPrice();
  ...
  /// @notice Buy eggs with ether
  /// @dev Need to send the exact value because extra ether will remain in the contract
  /// @dev Cannot send less value than an individual egg price
  function buyEggs() external payable;

  /// @notice Create a new ant from an egg
  /// @dev Sender needs to have at least an egg previously
  /// @dev Egg will be burned and ant minted
  function createAnt() external;

  /// @notice Sell an ant for ether
  /// @dev Sender needs to be the ant owner
  /// @dev Ant will be burned and the ether will be sent to sender
  /// @param antId The ID of the ant that will be sold
  function sellAnt(uint256 antId) external;

  /// @notice Create eggs from an ant
  /// @dev Sender needs to be the ant owner
  /// @dev 10 minutes have to passed between new creations
  /// @dev The ant can create between 0 an 20 eggs
  /// @dev The ant has a chance to die (30%)
  /// @param antId The ID of the ant that will create eggs
  function createEggs(uint256 antId) external;

  /// @notice Set egg price
  /// @dev Only callable from owner (governance)
  /// @param newPrice The new price for an egg
  function setEggPrice(uint256 newPrice) external;

  /// @notice Set ant price
  /// @dev Only callable from owner (governance)
  /// @param newPrice The new price for an ant
  function setAntPrice(uint256 newPrice) external;

  /// @notice Withdraw all ether balance from this contract
  /// @dev Only callable from owner (governance)
  /// @dev The balance is sent to the owner
  function withdrawBalance() external;

  /// @notice View ether balance from this contract
  /// @return balance The ether balance from this contract
  function getContractBalance() external view returns (uint256 balance);

  /// @notice View current ant ID
  /// @return currentAntId The current ant ID from the counter
  function getCurrentAntId() external view returns (uint256 currentAntId);
}
