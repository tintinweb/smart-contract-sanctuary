...
contract Elementals is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor(uint256 _reserved) ERC721("Elementals Vol. 1", "ELMTS") {
    ...
    function mintEarth(uint256 total) public payable canMint(total) {
        require(earthTokenId + total < 2001 - earthReserves, "Earth Elementals Sold Out!");
    ...
    function mintWater(uint256 total) public payable canMint(total) {
        require(waterTokenId + total < 4001 - waterReserves, "Water Elementals Sold Out!");
        for (uint256 i; i < total; i++) {
    ...
    function mintFire(uint256 total) public payable canMint(total) {
        require(fireTokenId + total < 6001 - fireReserves, "Fire Elementals Sold Out!");
        for (uint256 i; i < total; i++) {
    ...
    function mintWind(uint256 total) public payable canMint(total) {
        require(windTokenId + total < 8001 - windReserves, "Wind Elementals Sold Out!");
        for (uint256 i; i < total; i++) {
    ...
    function mintLightning(uint256 total) public payable canMint(total) {
        require(lightningTokenId + total < 10001 - lightningReserves, "Lightning Elementals Sold Out!");
        for (uint256 i; i < total; i++) {
