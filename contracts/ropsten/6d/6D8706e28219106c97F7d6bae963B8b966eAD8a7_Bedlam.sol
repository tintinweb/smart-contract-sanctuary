...
contract BaseERC721 is ERC721Enum, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
...
contract PhaseOne is Ownable {
    ...
    mapping(uint => PhaseOneToken) internal phaseOne;

    function getPhaseOne(uint id) public view returns (uint256, uint256) { 
        require(phaseOne[id].isPhaseOne, "Invalid Id");
        return (phaseOne[id].tokenId, phaseOne[id].rarity);
    }

    function updatePhaseOne(uint256 id, uint256 tokenId) internal {
        phaseOne[id].isMinted = true;
        phaseOne[id].tokenId = tokenId;
    }

    function tagAllPhaseOne(uint [] memory ids, uint [] memory rarity) external onlyOwner {
        require(ids.length == rarity.length, "Invalid lengths");
        for (uint256 i = 0; i < ids.length; i ++) {
            uint256 openseaId  = ids[i];
            phaseOne[openseaId].isPhaseOne = true;
            phaseOne[openseaId].rarity = rarity[i];
        }
    }
}
