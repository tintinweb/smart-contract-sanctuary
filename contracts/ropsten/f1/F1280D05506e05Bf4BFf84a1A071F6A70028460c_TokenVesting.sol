...
contract TokenVesting is Ownable {
    ...
    event WithdrawAllTokenGrant(
        address indexed recipient,
        uint256 indexed amount
    );
    event GrantRevoked(
        address indexed recipient,
        uint256 indexed amountVested,
        uint256 indexed amountNotVested
    );
    ...
    function calculateGrantClaim(address token, address _recipient)
        public
        view
    ...
    function withdrawAllTokenGrant(address token, address recipient)
        external
        onlyOwner
    {
    ...
    function revokeTokenGrant(address token, address _recipient)
        external
        onlyOwner
    {
        Grant storage tokenGrant = tokenGrants[_recipient][token];
