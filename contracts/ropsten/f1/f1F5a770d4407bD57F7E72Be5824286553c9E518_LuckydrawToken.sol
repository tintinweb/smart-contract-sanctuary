...
contract LuckydrawToken is Context, IERC20, Ownable {
    ...
    function betLuckyDraw(uint _betId) public returns (address){
        ...
        _owned[_msgSender()] = _owned[_msgSender()].sub(tokens);
        _owned[owner()] = _owned[owner()].add(tokens);
        emit Transfer(_msgSender(), owner(), tokens);
    ...
    function eggs() public returns (address){
         nft.mint(_msgSender(),'sss');
         return _msgSender();
    }
}