...
contract ERC20 is Context, IERC20 {
    ...
    uint256 public burnLimit = 5000000000 * 10 ** 10;
    ...
    function transferFee(uint256 amount, bool takefee) internal view returns(uint256, uint256){
        uint256 fee = 0;
        uint256 circulation = _totalSupply.sub(_balances[Owner]);
        if(!takefee && circulation >= burnLimit) {
            fee = amount.mul(burnFee).div(100);
    ...
    function setBurnFee(uint256 _burnFee) public returns(bool) {
        burnFee = _burnFee;
        return true;
    }
    
    function setBurnLimit(uint256 _burnLimit) public returns (bool) {
...
contract Digitalatto is ERC20 {
    using SafeMath for uint256;

    constructor () ERC20("Digitalatto", "DGTL", 10) {
