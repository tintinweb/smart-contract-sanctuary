...
interface IYouswapFactoryV1 {
    ...
    priority：矿池排序
     */
    event UpdatePool(bool action, uint256 pool, address indexed lp, string indexed name, uint256 startBlock, uint256 rewardTotal, uint256 rewardPerBlock, uint256 multiple, uint256 priority);
    ...
    */
    ...
    计算奖励
     */
    function computeReward(uint256) external;
    ...
    修改矿池邀请自奖励系数
     */
    function setInviteSelfReward(uint256, uint256) external;
    
    /**
    修改矿池1级邀请奖励系数
     */
    function setInvite1Reward(uint256, uint256) external;

    /**
    修改矿池2级邀请奖励系数
     */
    function setInvite2Reward(uint256, uint256) external;
...
contract YouswapFactoryV1 is IYouswapFactoryV1 {
    ...
    uint256 public rewardPerBlock = 0;//区块奖励
    ...
    function deposit(uint256 _pool, uint256 _amount) override external {
        ...
        provideReward(_pool, msg.sender, upper1, upper2);

        addPower(_pool, msg.sender, _amount, upper1, upper2);

        setRewardDebt(_pool, msg.sender, upper1, upper2);
    ...
    function withdraw(uint256 _pool, uint256 _amount) override external {
        ...
        setRewardDebt(_pool, msg.sender, _upper1, _upper2);
    }
    ...
    function computeReward(uint256 _pool) override public {
        PoolInfo storage poolInfo = poolInfos[_pool];
        if ((address(0) != poolInfo.lp) && (poolInfo.lastRewardBlock < block.number) && (0 < poolInfo.totalPower) && (poolInfo.rewardProvide < poolInfo.rewardTotal)) {
            uint256 reward = (block.number - poolInfo.lastRewardBlock).mul(poolInfo.rewardPerBlock);
            if ((poolInfo.rewardTotal.sub(poolInfo.rewardProvide)) <= reward) {
            ...
            poolInfo.rewardPerShare = poolInfo.rewardPerShare.add(reward.mul(1e12).div(poolInfo.totalPower));
    ...
    function addPower(uint256 _pool, address _user, uint256 _amount, address _upper1, address _upper2) internal {
        ...
        if (address(0) != poolInfo.lp && (poolInfo.startBlock <= block.number)) {
            ...
            if (address(0) != _upper1) {
                uint256 inviteSelfPower = pledgePower.mul(poolInfo.inviteSelfReward).div(100);
                userInfo.invitePower = userInfo.invitePower.add(inviteSelfPower);
                poolInfo.totalPower = poolInfo.totalPower.add(inviteSelfPower);

                uint256 invite1Power = pledgePower.mul(poolInfo.invite1Reward).div(100);
                ...
                if (address(0) != _upper2) {
                    uint256 invite2Power = pledgePower.mul(poolInfo.invite2Reward).div(100);
    ...
    function provideReward(uint256 _pool, address _user, address _upper1, address _upper2) internal {
        PoolInfo storage poolInfo = poolInfos[_pool];
        if (address(0) != poolInfo.lp && (poolInfo.startBlock <= block.number)) {
            uint256 reward = 0;
            ...
            if ((0 < userInfo.invitePower) || (0 < userInfo.pledgePower)) {
                inviteReward = userInfo.invitePower.mul(poolInfo.rewardPerShare).sub(userInfo.inviteRewardDebt).div(1e12);
                pledgeReward = userInfo.pledgePower.mul(poolInfo.rewardPerShare).sub(userInfo.pledgeRewardDebt).div(1e12);
                    
                userInfo.inviteReward = userInfo.inviteReward.add(inviteReward);
                userInfo.pledgeReward = userInfo.pledgeReward.add(pledgeReward);

                reward = inviteReward.add(pledgeReward);
                if (0 < reward) {
                    you.mint(_user, reward);

                    emit WithdrawReward(_pool, poolInfo.lp, _user, reward);
                }
            }
            ...
            if (address(0) != _upper1) {
                ...
                if ((0 < upper1Info.invitePower) || (0 < upper1Info.pledgePower)) {
                    inviteReward = upper1Info.invitePower.mul(poolInfo.rewardPerShare).sub(upper1Info.inviteRewardDebt).div(1e12);
                    pledgeReward = upper1Info.pledgePower.mul(poolInfo.rewardPerShare).sub(upper1Info.pledgeRewardDebt).div(1e12);

                    upper1Info.inviteReward = upper1Info.inviteReward.add(inviteReward);
                    upper1Info.pledgeReward = upper1Info.pledgeReward.add(pledgeReward);
                        
                    reward = inviteReward.add(pledgeReward);
                    if (0 < reward) {
                        you.mint(_upper1, reward);

                        emit WithdrawReward(_pool, poolInfo.lp, _upper1, reward);
                    }
                }
                ...
                if (address(0) != _upper2) {
                    ...
                    if ((0 < upper2Info.invitePower) || (0 < upper2Info.pledgePower)) {
                        inviteReward = upper2Info.invitePower.mul(poolInfo.rewardPerShare).sub(upper2Info.inviteRewardDebt).div(1e12);
                        pledgeReward = upper2Info.pledgePower.mul(poolInfo.rewardPerShare).sub(upper2Info.pledgeRewardDebt).div(1e12);

                        upper2Info.inviteReward = upper2Info.inviteReward.add(inviteReward);
                        upper2Info.pledgeReward = upper2Info.pledgeReward.add(pledgeReward);
                            
                        upper2Info.inviteRewardDebt = 0;
                        upper2Info.pledgeRewardDebt = 0;
                        
                        reward = inviteReward.add(pledgeReward);
                        if (0 < reward) {
                            you.mint(_upper2, reward);

                            emit WithdrawReward(_pool, poolInfo.lp, _upper2, reward);
                        }
                    }
                }
            }
    ...
    function setRewardDebt(uint256 _pool, address _user, address _upper1, address _upper2) internal {
        PoolInfo storage poolInfo = poolInfos[_pool];
        if (address(0) != poolInfo.lp && (poolInfo.startBlock <= block.number)) {
            UserInfo storage userInfo = pledgeUserInfo[_pool][_user];
            userInfo.inviteRewardDebt = userInfo.invitePower.mul(poolInfo.rewardPerShare);
            userInfo.pledgeRewardDebt = userInfo.pledgePower.mul(poolInfo.rewardPerShare);

            if (address(0) != _upper1) {
                UserInfo storage upper1Info = pledgeUserInfo[_pool][_upper1];
                upper1Info.inviteRewardDebt = upper1Info.invitePower.mul(poolInfo.rewardPerShare);
                upper1Info.pledgeRewardDebt = upper1Info.pledgePower.mul(poolInfo.rewardPerShare);

                if (address(0) != _upper2) {
                    UserInfo storage upper2Info = pledgeUserInfo[_pool][_upper2];
                    upper2Info.inviteRewardDebt = upper2Info.invitePower.mul(poolInfo.rewardPerShare);
                    upper2Info.pledgeRewardDebt = upper2Info.pledgePower.mul(poolInfo.rewardPerShare);
                }
            }
        }
    }
    ...
    function pendingReward(uint256 _pool, address _user) override external view returns (uint256) {
        ...
        return inviteReward.add(pledgeReward);
    }
    ...
    function addPool(string memory _name, address _lp, uint256 _startBlock, uint256 _rewardTotal) override external returns (bool) {
        ...
        poolInfo.inviteSelfReward = inviteSelfReward;
        poolInfo.invite1Reward = invite1Reward;
        poolInfo.invite2Reward = invite2Reward;
    ...
    function setInviteSelfReward(uint256 _pool, uint256 _inviteSelf) override external {
        ...
        poolInfo.inviteSelfReward = _inviteSelf;
    }

    function setInvite1Reward(uint256 _pool, uint256 _invite1Reward) override external {
        require(operateOwner[msg.sender], ErrorCode.FORBIDDEN);
        PoolInfo storage poolInfo = poolInfos[_pool];
        require((address(0) != poolInfo.lp) && (0 == poolInfo.endBlock), ErrorCode.POOL_NOT_EXIST_OR_END_OF_MINING);
        poolInfo.invite1Reward = _invite1Reward;
    }

    function setInvite2Reward(uint256 _pool, uint256 _invite2Reward) override external {
        require(operateOwner[msg.sender], ErrorCode.FORBIDDEN);
        PoolInfo storage poolInfo = poolInfos[_pool];
        require((address(0) != poolInfo.lp) && (0 == poolInfo.endBlock), ErrorCode.POOL_NOT_EXIST_OR_END_OF_MINING);
        poolInfo.invite2Reward = _invite2Reward;
    }
