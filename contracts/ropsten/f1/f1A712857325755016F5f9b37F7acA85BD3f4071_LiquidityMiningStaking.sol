...
import "./BMIStaking.sol";

contract LiquidityMiningStaking is Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	using SafeERC20 for IERC20;

	IERC20 public rewardsToken;
	IERC20 public stakingToken;
	BMIStaking public bmiStaking;
	...
	function restake() external nonReentrant updateReward(msg.sender) {
		...
		if (reward > 0) {
			...
			IERC20 bmiStakingToken = bmiStaking.stkBMIToken();
	...
	function getFutureRewardTokens() internal view returns (uint256) {
		uint256 blocksLeft =
			calculateBlocksLeft(firstBlockWithReward, lastBlockWithReward);
		return blocksLeft.mul(rewardPerBlock);
	}

	function calculateBlocksLeft(uint256 _from, uint256 _to)
		internal
		view
		returns (uint256)
	{
