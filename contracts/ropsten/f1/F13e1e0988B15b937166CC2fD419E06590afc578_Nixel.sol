...
contract Nixel is ERC721Enumerable, Ownable {
    ...
    function createSpawn(int256 x_pos, int256 y_pos, int256 newMinMax, uint256 newMaxBlocks) public onlyOwner {
        require(
        ...
        require(totalBlocks == maxBlocks, "Cannot create spawns until all current maxBlocks minted");
    ...
    function getSpawn(int256 spawnId)
        public
        view
...
interface IERC721Enumerable is IERC721 {
    ...
    function blockOwner(int256 x_pos, int256 y_pos) external view returns (address);
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    // Mapping for owner of block location
    mapping(int256 => mapping(int256 => address)) internal _blockOwner;

    // Mapping for tokenID of block location
    mapping(int256 => mapping(int256 => uint256)) internal _blockTokenId;

    // Mapping for tokenId to block location
    mapping(uint256 => int256[]) internal _blockLocation;

    // Mapping for renter address to block location
    mapping(int256 => mapping(int256 => address)) internal _renterAddress;

    // Mapping for rent stop time to block location
    mapping(int256 => mapping(int256 => uint256)) internal _rentStop;
    ...
    /**
     * @dev Returns address for a given token location.
     * @param x_pos int location of the token to query // e.g. location_x = "0"
     * @param y_pos int location of the token to query // e.g. location_y = "1"
     */
    function blockOwner(int256 x_pos, int256 y_pos) public view virtual override returns (address)
    {
        return _blockOwner[x_pos][y_pos];
    }

    /**
     * @dev Returns tokenId for a given token location.
     * @param x_pos int location of the token to query // e.g. location_x = "0"
     * @param y_pos int location of the token to query // e.g. location_y = "1"
     */
    function blockTokenId(int256 x_pos, int256 y_pos) public view virtual returns (uint256)
    {
        return _blockTokenId[x_pos][y_pos];
    }

    /**
     * @dev Returns Block Location for a given token id.
     * @param tokenId int location of the token to query // e.g. tokenId = "1", returns [0,0]
     */
    function blockLocation(uint256 tokenId) public view virtual returns (int256[] memory)
    {
        return _blockLocation[tokenId];
    }
    ...
    function transferFrom(
        ...
        require(_blockOwner[blockLocation(tokenId)[0]][blockLocation(tokenId)[1]] == _msgSender() || _msgSender() == stakingContractAddress,"Cannot transfer block you do not own");
        require(
            getRenter(blockLocation(tokenId)[0], blockLocation(tokenId)[1]) == address(0),
            "NFT is currently being rented"
        );
                _blockOwner[blockLocation(tokenId)[0]][blockLocation(tokenId)[1]] = to;
