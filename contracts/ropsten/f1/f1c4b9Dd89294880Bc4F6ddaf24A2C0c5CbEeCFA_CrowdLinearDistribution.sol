...
contract CrowdLinearDistribution is Ownable {
    ...
    function create(address beneficiary, uint256 start, uint256 cliff, uint256 initialShare, uint256 periodicShare, uint256 vestingType, uint256 factor, uint256 balance) onlyOwner external {
        ...
        require(initialShare <= balance, "CrowdLinearDistribution: initialShare must be equal or less than balance");
        require(periodicShare <= balance, "CrowdLinearDistribution: periodicShare must be equal or less than balance");
        require(cliff >= start, "CrowdLinearDistribution: cliff must be equal or greater than start");
        require(balance > 0, "CrowdLinearDistribution: balance must be greater than zero");
    ...
    function _vestedAmount(address beneficiary) private view returns (uint256) {
        ...
        uint256 sum = tokenVesting._periodicShare + initialRelease;

        for (uint256 i = 2; i <= _months; ++i) {
