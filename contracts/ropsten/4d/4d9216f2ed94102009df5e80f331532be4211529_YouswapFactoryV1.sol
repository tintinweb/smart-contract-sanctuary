...
contract YouswapFactoryV1 is IYouswapFactoryV1 {
    ...
    function pendingReward(uint256 _pool, address _user) override external view returns (uint256) {
        ...
        if (address(0) != poolInfo.lp && (poolInfo.startBlock <= block.number)) {
            ...
            totalReward = totalReward.add(userInfo.invitePower.mul(rewardPerShare).sub(userInfo.inviteRewardDebt).div(1e12));
            totalReward = totalReward.add(userInfo.pledgePower.mul(rewardPerShare).sub(userInfo.pledgeRewardDebt).div(1e12));
        }
