...
contract PunkGenerations {
    ...
    constructor() {
        ...
        assets = Assets(0x2e45533676925d6071327527168E086d2F116dd8);
        cryptoPunksData = CryptoPunksData(0xEd9999d49A73853621505b91e0fB203B297a1e4E);
        punksCount = 10000;
    }
    ...
    function breedPunkAssets(uint16 fatherIndex, uint16 motherIndex) internal view returns (bytes memory childAssets) {
        ...
        bytes memory fatherAssets = parseAssets(cryptoPunksData.punkAttributes(fatherIndex));
        bytes memory motherAssets = parseAssets(cryptoPunksData.punkAttributes(motherIndex));
        ...
        if (!info.isDifferent) {
            if (info.cigaretteIndex > 0) {
                childAssets[info.cigaretteIndex] = 0;
            } else if (info.earringIndex > 0) {
                childAssets[info.earringIndex] = 0;
    ...
    function borderImage(bytes memory pixels, bytes4 color) internal pure returns (bytes memory) {
        bytes memory result = new bytes(2304);
        for (uint y = 0; y < 24; y++) {
            bool previousOn = false;
            for (uint x = 0; x < 24; x++) {
                uint p = (y * 24 * 4) + (x * 4);
                bool on = (pixels[p] != 0x0) || (pixels[p+1] != 0x0) || (pixels[p+2] != 0x0) || (pixels[p+3] != 0x0);
                if (!previousOn && on && (x > 0)) {
                    result[p-4] = color[0];
                    result[p-3] = color[1];
                    result[p-2] = color[2];
                    result[p-1] = color[3];
                ...
                if (previousOn && !on) {
                    ...
                    result[p] = pixels[p];
                    result[p+1] = pixels[p+1];
                    result[p+2] = pixels[p+2];
                    result[p+3] = pixels[p+3];
                }
                previousOn = on;
            }
        }
        for (uint x = 0; x < 24; x++) {
            ...
            for (uint y = 0; y < 24; y++) {
                ...
                if (!previousOn && on && (y > 0)) {
                    result[p-(24*4)] = color[0];
                    result[p-(24*4)+1] = color[1];
                    result[p-(24*4)+2] = color[2];
                    result[p-(24*4)+3] = color[3];
                } else if (previousOn && !on) {
                    result[p] = color[0];
                    result[p+1] = color[1];
                    result[p+2] = color[2];
                    result[p+3] = color[3];
    ...
    function borderImage2(bytes memory pixels, bytes4 color) internal pure returns (bytes memory) {
        bytes memory result = new bytes(2304);
        for (uint y = 0; y < 24; y++) {
            bool previousOn = false;
            for (uint x = 0; x < 24; x++) {
                uint p = (y * 24 * 4) + (x * 4);
                bool on = (pixels[p] == 0x0) && (pixels[p+1] == 0x0) && (pixels[p+2] == 0x0) && (pixels[p+3] == 0xFF);
                if (!previousOn && on && (x > 0)) {
                    result[p-4] = color[0];
                    result[p-3] = color[1];
                    result[p-2] = color[2];
    ...
    function punkAssetsImageSvg(bytes memory punkAssets) internal view returns (string memory svg) {
        bytes memory pixels = borderImage2(punkAssetsImage(punkAssets), 0xadc9d6ff);
    ...
    function parseAssets(string memory attributes) public view returns (bytes memory punkAssets) {
        punkAssets = new bytes(8);
