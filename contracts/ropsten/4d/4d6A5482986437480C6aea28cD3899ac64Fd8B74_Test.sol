...
contract Test is Context, IERC20, Ownable {
    ...
    address payable private teamWallet = payable(0x19f2f44903eAAB263FA02313c0BF9492d63b4300); // team  Wallet
    address payable private ecosystemWallet = payable(0x2F34F339A83B03Ae627cFD01665EBF5c81101BBB); // Ecosystem Wallet 0%
    address payable private marketdevWallet = payable (0x831A4C44a06901adF6B12e6559543A5CD29A388C); // marketdev Wallet
    ...
    uint256 private totalFee =
        liquidityFee.add(teamFee).add(marketdevFee).add(ecosystemFee);
    ...
    function _transfer(
        ...
        if(tradingOpen && to == uniswapV2Pair) { //sell
            currenttotalFee= _sellLiquidityFee.add(_sellteamFee).add(_sellmarketdevFee);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountETHteam = amountETH.mul(teamFee).div(
            totalETHFee
        );
         uint256 amountETHEcosystem = amountETH.mul(ecosystemFee).div(
            totalETHFee
        );
        //Send to team wallet and marketdev wallet
    ...
    function setWallets(address _teamWallet,address _marketdevWallet) external onlyOwner() {
    ...
    function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
     ...
     function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _teamFee,uint256 _marketdevFee) external onlyOwner {
        ...
        teamFee = _teamFee;
        totalFee = liquidityFee.add(teamFee).add(marketdevFee).add(ecosystemFee);

    }

    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _teamFee,uint256 _marketdevFee) external onlyOwner {
