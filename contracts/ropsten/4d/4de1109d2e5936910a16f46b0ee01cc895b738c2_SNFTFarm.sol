...
contract SNFTFarm {
    ...
    constructor(IERC20 _SNFTLP, IERC20 _snftToken, address _tresaddr) {
            SNFTLP = _SNFTLP;
    ...
    require(amount > 10 && SNFTLP.balanceOf(msg.sender) >= amount, "Under minimum staking requirements");
            
     if(isStaking[msg.sender] == true){
        uint256 toTransfer = calculateYieldTotal(msg.sender);
        snftBalance[msg.sender] += toTransfer;
        }
        SNFTLP.transferFrom(msg.sender, address(this), ((amount * 975) / 1000));
        SNFTLP.transferFrom(msg.sender, address(tresaddr), ((amount * 25) / 1000));
