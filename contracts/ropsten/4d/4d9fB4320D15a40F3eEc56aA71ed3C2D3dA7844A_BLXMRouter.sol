...
contract BLXMRewardProvider is Ownable, BLXMTreasuryManager, IBLXMRewardProvider {
    ...
    function addRewards(address tokenA, address tokenB, uint totalAmount) external override lock onlyOwner returns (uint amount) {
        ...
        for (uint i = 1; i < left; i++) {
            ...
            Statistics storage statistics = dailyStatistics[treasury][i];
    ...
    function _calcRewards(Position memory position) internal view returns (uint amount, bool isLocked) {
        ...
        require(treasury != address(0), 'TREASURY_NOT_FOUND');
        uint today = BLXMLibrary.today();
        require(tag[treasury].syncDay == today, 'NOT_SYNC');
    ...
    function _burn(address to, uint liquidity, uint idx) internal lock returns (uint amountA, uint amountB, uint rewardAmount) {
        ...
        uint _totalLiquidity = totalLiquidity;
        {
            ...
            (uint reserveA, uint reserveB) = BLXMLibrary.getReserves(treasury, position.tokenA, position.tokenB);
            uint percentageByTotal = liquidity.wdiv(_totalLiquidity);
            amountA = reserveA.wmul(percentageByTotal);
            amountB = reserveB.wmul(percentageByTotal);
    ...
    function _arrangeFailedRewards(address treasury, uint rewardAmount) internal {
        ...
        if (rewards != 0) {
            ...
            for (uint i = day; i < target; i++) {
                dailyStatistics[treasury][i].rewards = dailyStatistics[treasury][i].rewards.add(rewards);
            }
        }
    }
    ...
    function _syncStatistics(address treasury) internal {
        uint today = BLXMLibrary.today();
        uint day = tag[treasury].syncDay;
        if (day == 0) {
            tag[treasury].syncDay = today;
        } else if (day < today) {
            Statistics storage statistics = dailyStatistics[treasury][day];
            while (day < today) {
...
interface IBLXMRewardProvider {
    ...
    function tag(address) external view returns(uint, uint8);
