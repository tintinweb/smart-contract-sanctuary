...
contract RewardToken is ERC20, ERC20PresetMinterPauser, ERC20Capped {
  ...
  uint256 public constant MINTING_PERIOD = 1 days * 365;
  uint256 public constant LOG_RANGE_FACTORIAL = 231308923578266000000; // log2((week_number+1)!)
  uint256 public constant BASE_WEEK_EMISSION = HARD_CAP / LOG_RANGE_FACTORIAL;
  ...
  function changeAdmin(address _newAdmin) public onlyAdmin {
    // with scheduling minting, it will be safe to have changeable delay minter
  ...
  function maxTotalSupplyForCurrentBlock() public view returns (uint256){
    uint256 week = Math.min(MINTING_PERIOD.div(1 weeks).add(1), currentWeek());
    uint256 maxTotalSupply;
    for (uint256 i = 0; i <= week; i++) {
      uint256 emissionPerWeek =
      BASE_WEEK_EMISSION.mul(
        _log2(
          i.add(1).mul(10 ** 18)
        )
      );
      maxTotalSupply = maxTotalSupply.add(emissionPerWeek);
    }
