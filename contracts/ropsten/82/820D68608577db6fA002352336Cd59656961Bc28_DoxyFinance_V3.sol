...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    mapping(address => uint256) public _firstSell;
    mapping(address => uint256) public _totSells;

    mapping(address => uint256) public _firstBuy;
    mapping(address => uint256) public _totBuy;

    mapping(address => bool) public _isExcludedFromFee;
    mapping(address => bool) public _includeInSell;
    mapping(address => bool) public _isBadActor;

    uint256 public maxSellPerDay = 150 * 10**18;
    uint256 public minTokensToSwap = 50000 * 10**9;

    uint256 public _totalSupply;
    uint256 public maxTxAmount;

    uint256 public buyLimit = 50 * 10**18;
    uint256 public sellLimit  = 50 * 10**18; 
    ...
    address public pancakeSwapRouter = address(0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45) ; // 10ed43c718714eb63d5aa57b78b54704e256024e for mainnet;
    ...
    function getPrice(uint256 toks) public view returns (uint256) {
    ...
    function _transfer(
        ...
        if (recipient == pancakePair) {
            ...
            if(!liquiFlag) {
                ...
                if (getPrice(maxSellPerDayLimit) > maxSellPerDay) {
                    require(block.timestamp > timeLimit + 24 * 1 hours , "maxSellPerDay Limit Exceeded");
                    timeLimit = block.timestamp;
                    maxSellPerDayLimit = 1000000000;
                }
                ...
                if (block.timestamp < _firstSell[sender] + 24 * 1 hours) {
                    require(
                        getPrice(_totSells[sender] + amount) <= sellLimit,
        ...
        if (sender == pancakePair) { 
            if (block.timestamp < _firstBuy[recipient] + 24 * 1 hours) {
                require(
                    getPrice(_totBuy[recipient] + amount) <= buyLimit,
                    "You can't sell more than buyLimit"
                );
                _totBuy[recipient] += amount;
            } else {
                require(
                    getPrice(amount) <= buyLimit,
                    "You can't sell more than buyLimit"
                );
