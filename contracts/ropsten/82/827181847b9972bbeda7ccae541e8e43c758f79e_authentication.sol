...
contract authentication{
    ...
    function newtask(uint id,uint tuav,uint req,uint newmsg) internal returns(uint number){
   ...
   modifier  checktime(uint timestamp)  {         
    ...
    //-------------------------------------------------初始化------------------------------------------------------//
    function iniTS(address TS_ad,uint TS_num) public{
        newTS(TS_ad,TS_num);
    }
    function iniUAV(address UAV_ad,uint num ,uint credit) public{
            newTicket(num,credit,UAV_ad,now);
    }
    //-------------------------------------------------初始化------------------------------------------------------// 
    //--------------------------------PPT12页--------------------------------//
    //差 初始化 是否使用 签名？
    
    // 基站发送信息给智能合约，合约修改ticket ；调用send函数向对应地址发送资金、信息
    /*
    这里是只实现了一次修改一个无人机的信息
      TS的编号,id_ts      =    ida
      upload = {
    ...
    */
    ...
    returns(bool flag){
        ...
        if(checkTicketstate(id_uav,state_temp)){
            ...
            state_temp = Ticket_state.born;
            checkTicketstate(id_uav,state_temp);
            return true;
        }
   ...
   */
   ...
   returns(bool flag)
   {
       ...
       if(gettask_ID(tuav_id) == 0x4)
            return false;
       if(gettask_cerdit(tuav_id)>getTicket_cerdit(id))
            return false;
   ...
   */
   ...
   //-----------------------------------子链的---------------------------------//
    ...
    //-----------------------------对task的操作--------------------------------------//
     event showit(string ,bytes32);
     event showit2(string , uint);
   //1.通信，为了方便，没弄加解密，且仅记录通信内容//

   function commuincation_func(bytes32 newmsg,address id_a,address id_b,bytes32 hash_value)
   public
   returns(bool flag)
   {
       bytes32 test_hash = sha256(abi.encodePacked(newmsg,id_a,id_b));
       bytes32 hash;
       if(test_hash!=hash_value)
       return false;
       hash = sha256(abi.encodePacked(newmsg,id_a,id_b,hash_value));
       emit showit("Caculated hash by block : ", hash);
       return id_b.call("Caculated hash by block : ", hash);
   }

   //2.上传结果，产生验证，会保存结果
  
      function result_func(uint t_id,bytes32 newmsg,address id_a,bytes32 hash_value)
      public
      returns(bool flag)
      {
       bytes32 test_hash = sha256(abi.encodePacked(t_id,newmsg,id_a));
       ...
       emit showit2("Task result uploaded successfully, and the result ：",result);
       return true;
   }
}