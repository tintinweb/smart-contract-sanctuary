...
contract RastaDividendNFTStaking is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public defaultPrice = 200000000000000000;

    uint256 public endTime;
    uint256 public rewardCycle;

    mapping(address => UserInfo) public userInfo;
    mapping(uint256 => bool) public ownTokens;
    ...
    modifier updateReward(bool withdrawal) {
        ...
        if (totalSupply > 0) {
            ...
            accPerShare = accPerShare.add(reward.mul(1e12).div(totalSupply.mul(defaultPrice)));
        }
        ...
        uint256 pending = penaltyFlag ? 0 : user.amount.mul(defaultPrice).mul(accPerShare).div(1e12).sub(user.rewardDebt);
        user.pendingRewards = user.pendingRewards.add(pending);
        
        _;
        
        user.rewardDebt = user.amount.mul(defaultPrice).mul(accPerShare).div(1e12);
    ...
    function setOwnToken(uint256 _tokenId, bool exist) external onlyOwner {
        ownTokens[_tokenId] = exist;
    }
    ...
    function stakeAll() external {  
        ...
        for (uint i = 0; i < balance; i++) {
            ...
            if(ownTokens[_tokenId]) _tokenIds[i] = _tokenId;
        }

        for (uint i = 0; i < balance; i++) {
           if(_tokenIds[i] > 0) _stake(_tokenIds[i]);
        }
    }

    function stake(uint256 _tokenId) external {
        require(parentNFT.ownerOf(_tokenId) == msg.sender, "only owner can stake!");
        require(ownTokens[_tokenId], "only can stake rasta NFT!");
        _stake(_tokenId);
    }
    ...
    function _stake(uint256 _tokenId) internal nonReentrant whenNotPaused updateReward(false) updateUserList {
        ...
        emit Stake(msg.sender, defaultPrice);
    }
    ...
    function _unstake(uint256 _tokenId) internal nonReentrant updateReward(true) updateUserList {
        ...
        emit UnStake(msg.sender, defaultPrice);
    }
    ...
    function claimable(address _user) external view returns (uint256) {
        ...
        if (totalSupply > 0) {
            uint256 multiplier = Math.min(block.timestamp, endTime).sub(lastUpdateTime);
            uint256 reward = multiplier.mul(rewardRate);
            curAccPerShare = accPerShare.add(reward.mul(1e12).div(totalSupply.mul(defaultPrice)));
        }
        uint amount = user.amount.mul(defaultPrice);
        return amount.mul(curAccPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingRewards);
    }
