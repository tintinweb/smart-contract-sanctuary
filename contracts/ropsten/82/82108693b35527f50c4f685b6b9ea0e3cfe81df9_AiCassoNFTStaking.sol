...
contract AiCassoNFTStaking is ERC721Receiver, Ownable {
    ...
    function stake(uint256 numberOfTokens) public {
        require(numberOfTokens <= AiCassoNFT.balanceOf(msg.sender));

        for (uint256 i = 0; i < numberOfTokens; i++) {
            AiCassoNFT.safeTransferFrom(msg.sender, address(this), AiCassoNFT.tokenOfOwnerByIndex(msg.sender, i));
        }
    }
