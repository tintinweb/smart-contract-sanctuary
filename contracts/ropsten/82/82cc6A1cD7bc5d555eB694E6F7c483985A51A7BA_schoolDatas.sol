...
contract schoolDatas is ERC721, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
 ...
 //   bool public hasReserved = false;
    ...
    constructor() ERC721("schoolDatas", "Girl") { }

    function safeMint(uint256 _amount) public payable {
        require(!Address.isContract(msg.sender), "Contracts are not allowed to mint");
        require(minted[msg.sender] + _amount <= maxPerWallet, "Purchase would exceed max tokens per wallet");
        require(_tokenIdCounter.current() + _amount <= totalTokens, "Purchase would exceed max supply of tokens");
