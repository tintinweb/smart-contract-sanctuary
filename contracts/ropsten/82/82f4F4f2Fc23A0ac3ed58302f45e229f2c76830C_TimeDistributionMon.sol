...
// File contracts/distribution/TimeDistributionMon.sol
...
/**
@notice Linear release of BOR monthly
 */
contract TimeDistributionMon is Ownable {
    ...
    function addInfo(
        ...
        require(
            infos[account].amount == 0,
            "Timedistribution::account is not a new user"
        );
        require(amount != 0, "TimeDistribution::addInfo: amount should not 0");
        require(
            _timePoints.length >= 2,
            "TimeDistribution::addInfo: time points should not null"
        );
        require(
    ...
    function addMultiInfo(
        ...
        require(
            ...
            "TimeDistribution::addMultiInfo:function params length not equal"
        );
        require(
            accounts.length == beginTsArray.length,
            "TimeDistribution::addMultiInfo:function params length not equal"
        );
        require(
            accounts.length == endTsArray.length,
            "TimeDistribution::addMultiInfo:function params length not equal"
        );
    ...
    function getTimePoint(address account) public view returns (uint256) {
        uint256[] memory timePoints = infos[account].timePoints;
        if (block.timestamp <= timePoints[0]) {
            return timePoints[0];
        } else if (block.timestamp >= timePoints[timePoints.length - 1]) {
            return timePoints[timePoints.length - 1];
        } else {
            for (uint256 i; i < timePoints.length; i++) {
                if (
                    block.timestamp >= timePoints[i] &&
                    block.timestamp < timePoints[i + 1]
                ) {
                    return timePoints[i];
                }
            }
    ...
    function pendingClaim() public view returns (uint256) {
        ...
        uint256 nowtime = getTimePoint(msg.sender);
        return
    ...
    function claim() public {
        ...
        require(claimAmount > 0, "pending claim amount should not 0");
        DistributionInfo storage info = infos[msg.sender];
    ...
    function changeUserAdmin(address oldUser, address newUser)
        public
        onlyOwner
    {
        require(
            infos[newUser].amount == 0,
            "Timedistribution::newUser is not a new user"
        );
