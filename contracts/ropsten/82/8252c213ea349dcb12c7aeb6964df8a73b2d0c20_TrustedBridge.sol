...
library $
{
	...
	Network constant NETWORK = Network.Ropsten;
...
contract TrustedBridge is ReentrancyGuard
{
	...
	uint256 constant WITHDRAW_GRACE_PERIOD = 45 minutes; // after transfer reference timestamp
	...
	function construct(address _token) external
	{
	...
	function deposit(uint256 _targetChainId, address _server, uint256 _sourceAmount, uint256 _targetAmount, uint256 _timestamp, uint256 _transferId) external onlyEOA nonReentrant
	{
		...
		emit Deposit(_client, _server, _sourceAmount, _targetAmount, _timestamp, _transferId);
	}
	...
	function withdraw(uint256 _sourceChainId, address _client, uint256 _sourceAmount, uint256 _targetAmount, uint256 _timestamp, uint256 _transferId) external nonReentrant
	{
		...
		transfers[_transferId].timestamp = now;
		emit Withdraw(_client, _server, _sourceAmount, _targetAmount, _timestamp, _transferId);
	}

	event Deposit(address indexed _client, address indexed _server, uint256 _sourceAmount, uint256 _targetAmount, uint256 _timestamp, uint256 indexed _transferId);
	event Withdraw(address indexed _client, address indexed _server, uint256 _sourceAmount, uint256 _targetAmount, uint256 _timestamp, uint256 indexed _transferId);
}
...
contract Operator is Ownable, ReentrancyGuard
{
	...
	constructor (address _bridge, address _vault) public
	{
		bridge = _bridge;
		vault = _vault;
	...
	{
		bytes32 _hash = keccak256(abi.encode(_transferId, _txId));
		return ECDSA.recover(_hash, _signature) == _agent;
	}
...
contract Panel is Ownable, ReentrancyGuard
{
	...
	uint256 constant DEFAULT_VARIABLE_FEE_RATE = 3e16; // 3%
	uint256 constant DEFAULT_FIXED_FEE_AMOUNT = 0e18; // 0
	...
	uint256 public variableFeeRate = DEFAULT_VARIABLE_FEE_RATE;
	uint256 public fixedFeeAmount = DEFAULT_FIXED_FEE_AMOUNT;
	uint256 public networkConfirmations = DEFAULT_NETWORK_CONFIRMATIONS;

	constructor (address _bridge, address _operator) public
	{
		bridge = _bridge;
		operator = _operator;
	}
	...
	{
		uint256 _feeAmount = _amount.mul(variableFeeRate).div(1e18).add(fixedFeeAmount);
	...
	function setFee(uint256 _newVariableFeeRate, uint256 _newFixedFeeAmount) external onlyOwner nonReentrant
	{
		require(_newVariableFeeRate <= 1e18, "invalid fee rate");
		uint256 _oldVariableFeeRate = variableFeeRate;
		uint256 _oldFixedFeeAmount = fixedFeeAmount;
		variableFeeRate = _newVariableFeeRate;
		fixedFeeAmount = _newFixedFeeAmount;
		emit FeeChange(_oldVariableFeeRate, _oldFixedFeeAmount, _newVariableFeeRate, _newFixedFeeAmount);
	}
	...
	event FeeChange(uint256 _oldVariableFeeRate, uint256 _oldFixedFeeAmount, uint256 _newVariableFeeRate, uint256 _newFixedFeeAmount);
...
library LibDeployer1
{
	function publishGRO() public returns (address _address)
	{
		...
		return Create2.deploy(0, keccak256("GRO"), _bytecode);
	}
	...
	function publishSignatureRegistry() public returns (address _address)
	{
		...
		return Create2.deploy(0, keccak256("SignatureRegistry"), _bytecode);
	}
	...
	function publishTimeLockedVault() public returns (address _address)
	{
		...
		return Create2.deploy(0, keccak256("TimeLockedVault"), _bytecode);
	}
}
...
library LibDeployer2
{
	function publishTrustedBridge(address _token) public returns (address _address)
	{
		...
		_address = Create2.deploy(0, keccak256("TrustedBridge"), _bytecode);
		TrustedBridge(_address).construct(_token);
		return _address;
	}

	function publishOperator(address _bridge, address _vault) public returns (address _address)
	{
		bytes memory _bytecode = abi.encodePacked(type(Operator).creationCode, abi.encode(_bridge, _vault));
		return Create2.deploy(0, keccak256("Operator"), _bytecode);
	}

	function publishPanel(address _bridge, address _operator) public returns (address _address)
	{
		bytes memory _bytecode = abi.encodePacked(type(Panel).creationCode, abi.encode(_bridge, _operator));
		return Create2.deploy(0, keccak256("Panel"), _bytecode);
	}
}