...
abstract contract Router {
    ...
    function calculate1InchReturn( uint256 amountIn, address[] memory path, OrderType orderType,uint256 /*disableFlags*/) public view returns(uint256,uint256[] memory) {
        ...
        if(orderType == OrderType.EthForTokens){
            path[0] = ETH;
            try OneSplit.getExpectedReturn(IERC20(path[0]), IERC20(path[1]), amountIn, 100, 0) returns(uint256 _amount, uint256[] memory _distribution){
...
contract Degen is Router, Ownable {
   ...
   modifier onlySystem() {
    ...
    constructor(address _tokenAddress, address _degEthPool, address _swapFactory, address _system) Router( _Uni, _sushi, _oneSplit) public {
    ...
    function executeSwap(OrderType orderType, address[] memory path, uint256 assetInOffered, uint256 fees, uint256 minExpectedAmount, uint256 deadline) external payable  {
        ...
        uint256 disableFlags = getDisabledDEX(msg.sender);
        ...
        if(dexId == 0){
            ...
            swap(path[0], path[1], assetInOffered, minAmountExpected, distribution, 0);
        }
    ...
    function executeCrossExchange(address[] memory path,uint256 assetInOffered, uint256 fees,  uint256 minExpectedAmount) external payable {
        ...
        (uint dexId, uint256 minAmountExpected, uint256[] memory distribution) = getBestQuote(path, assetInOffered,OrderType.TokensForTokens ,disableFlags);
        require(minExpectedAmount <= minAmountExpected  , "Degen : Higher slippage than allowed.");
