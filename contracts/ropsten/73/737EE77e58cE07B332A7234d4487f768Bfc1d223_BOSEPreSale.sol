...
contract BOSEToken is Ownable, ERC20Burnable {

    using Address for address;

    /// Constant that holds token distribution.
    uint256 constant public MULTIPLIER = 10 ** 18;
    uint256 constant public UNISWAP_ALLOCATION = 4000 * MULTIPLIER;
    uint256 constant public PRESALE_ALLOCATION = 6000 * MULTIPLIER;
    ...
    function initialMint(address _saleContractAddress) public onlyOwner {
        require(!_mintingClosed, "BOSEToken: Intital minting closed");
        require(_saleContractAddress.isContract(), "BOSEToken: Not a valid contract address");
        // Close the minting.
        _mintingClosed = true;
...
// File: contracts/BOSEPreSale.sol
...
contract BOSEPreSale is ReentrancyGuard, Ownable {

    using Address for address payable;
    using SafeMath for uint256;

    BOSEToken public token;

    /// Sale phases timings.
    ///
    /// Early Phase starts once open() function is called.
    /// while token rate during this phase is  1 ETH = 150 tokens.
    ///
    ...
    uint256 constant public EARLY_PHASE_MINIMUM_INVESTMENT_CAP = 1 ether / 100;
    uint256 constant public LATE_PHASE_MINIMUM_INVESTMENT_CAP = 1 ether / 100;
    
    /// Multiplier to provide precision.
    uint256 constant public MULTIPLIER = 10 ** 18;

    /// Rates for different phases of the sale.
    uint256 constant public EALRY_PHASE_RATE = 150 * MULTIPLIER;  /// i.e 150 tokens = 1 ETH.
    uint256 constant public LATE_PHASE_RATE = 150 * MULTIPLIER;  /// i.e 150 tokens = 1 ETH.
    ...
    function open() public onlyOwner {  
        require(!isOpen, "BOSEPreSale: Sale is already open");
        require(token.balanceOf(address(this)) > uint256(0), "BOSEPreSale: Balance is insufficient");
        isOpen = true;
    }

    /// @dev Transfer tokens to the owner address.
    function reclaimTokens(uint256 _amount) public onlyOwner {
        require(token.transfer(_msgSender(), _amount), "BOSEPreSale: Transfer failed");
    }
    ...
    function buyTokens() public payable nonReentrant {
        ...
        require(msg.value > 0, "BOSEPreSale: Zero investments aren't allowed");
        ...
        require(token.transfer(msg.sender, tokensToSale), "BOSEPreSale: Transfer failed");
        // Emit event.
    ...
    function getROI(uint256 _amount) internal returns(uint256 _roi) {
        if (fundCollected < EARLY_PHASE_CAP) {
            require(_amount >= EARLY_PHASE_MINIMUM_INVESTMENT_CAP, "BOSEPreSale: Less than minimum allowed investment amount in early phase");
            ...
            require(_amount >= LATE_PHASE_MINIMUM_INVESTMENT_CAP, "BOSEPreSale: Less than minimum allowed investment amount in late phase");
    ...
    function _hasSaleOpen() internal view {
        require(isOpen, "BOSEPreSale: Sale has yet to be started");
        require(fundCollected < EARLY_PHASE_CAP + LATE_PHASE_CAP, "BOSEPreSale: Cap is reached, Sale closed");
    }

    function _checkForZeroAddress(address _target) internal pure {
        require(_target != address(0), "BOSEPreSale: Invalid address");
    }
