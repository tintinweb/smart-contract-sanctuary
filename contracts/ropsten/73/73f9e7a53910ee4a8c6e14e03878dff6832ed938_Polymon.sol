...
contract Polymon is ERC721Enumerable, Ownable{
    ...
    uint256[5] totalSupplies = [0, 0, 0, 0, 0];
    uint256[5] totalBurned = [0, 0, 0, 0, 0];
    uint256[5] blockEnds = [9999, 19999, 24999, 27499, 27509];
    
    uint256 public constant NFT_PRICE = 1000000000000000;//80000000000000000; // 0.08 ETH
    uint256 public constant NFT_PRICE_PRESALE = 1000000000000000;//75000000000000000; // 0.075 ETH
    
    uint256 public constant MAX_NFT_PURCHASE = 1000; //20
    ...
    function reserveTokens(uint256 numberOfTokens) public onlyOwner {
        ...
        require(supply < (RESERVED_TOKENS - 1), "Max tokens reserved");
    ...
    function polyPresaleMint(uint256 numberOfTokens) public payable {
        ...
        require((totalSupply + numberOfTokens) < (MAX_PRESALE_SUPPLY - 1), "Purchase exceeding max presale token supply");
    ...
    function polyMint(uint256 numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0, "Number of tokens can not be less than or equal to 0");
        require((totalSupply + numberOfTokens) < (MAX_EGG_SUPPLY - 1), "Purchase exceeding max token supply");
        require(numberOfTokens <= MAX_NFT_PURCHASE,"Can only mint a max of 20 tokens per transaction");
    ...
    function getGender(uint256 tokenId) private pure returns (Gender) {
      return (Gender)(tokenId % 2);
    }
    ...
    function getGenerationStats() public view returns (generationStat[] memory)
    {
        ...
        for (uint256 i; i < totalSupplies.length; i++) {
            ...
            stats[i].max_supply = blockEnds[i];
            stats[i].burned_supply = totalBurned[i];
        }
        return stats;
    }
   
    function oppositeGender(uint256 tokenId1, uint256 tokenId2) private pure returns (bool) {
