...
// File: contracts/PantherToken.sol
...
contract test is BEP20 {
    ...
    // Min amount to liquify. (default 500 tests)
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "DYN::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
     ...
     */
    constructor() public BEP20("test3", "test3") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && _inSwapAndLiquify == false
            && address(testSwapRouter) != address(0)
            && testSwapPair != address(0)
            && sender != testSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "DYN::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "DYN::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(testSwapRouter), tokenAmount);
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "DYN::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "DYN::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateTestSwapRouter(address _router) public onlyOperator {
        testSwapRouter = IUniswapV2Router02(_router);
        testSwapPair = IUniswapV2Factory(testSwapRouter.factory()).getPair(address(this), testSwapRouter.WETH());
        require(testSwapPair != address(0), "DYN::updateTestSwapRouter: Invalid pair address.");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "DYN::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "DYN::delegateBySig: invalid nonce");
