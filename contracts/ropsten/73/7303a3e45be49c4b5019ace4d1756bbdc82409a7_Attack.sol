    ...
    ERC1820Implementer
{
    ...
    Counters.Counter private _castleIds;

    IERC1820Registry private _erc1820;
    IERC777 public antToken;

    bytes32 private TOKENS_RECIPIENT_INTERFACE_HASH;
    bytes32 public TOKENS_SENDER_INTERFACE_HASH;
    ...
    mapping(uint256 => Castle) idToCastle;

    event MarketItemCreated(uint256 indexed itemId);

    event CastleCreated(Castle[] indexed castleIds);
    ...
    function initialize(address token) public initializer {
        ...
        TOKENS_RECIPIENT_INTERFACE_HASH = keccak256("ERC777TokensRecipient");
        TOKENS_SENDER_INTERFACE_HASH = keccak256("ERC777TokensSender");
        antToken = IERC777(token);
    ...
    function createCastle(
        ...
        require(price >= 50 * 1e18, "Price must bigger than 50 ANT");

        Castle[] memory ids = new Castle[](number);
        for (uint256 i = 0; i < number; i++) {
            _castleIds.increment();
            uint256 castleId = _castleIds.current();
            uint256 hatchTime = block.timestamp + 3 days;
            uint256 rare = randomRare();

            TypeAnt typeEgg = typeAnt;
            if (isRandom == true) {
                typeEgg = getTypeRandom();
            }

            idToCastle[castleId] = Castle(
                castleId,
                hatchTime,
                rare,
                typeEgg,
            ...
            ids[i] = idToCastle[castleId];
        }

        bytes memory data = "BuyCastle";
        antToken.operatorSend(msg.sender, address(this), price, data, data);

        emit CastleCreated(ids);
    }
    ...
    function randomRare() internal returns (uint256) {
        uint256 value = randomNumber(100);
        if(value >= 0 && value <= 2){
            return 6;
        }
        if(value >= 3 && value <= 7){
    ...
    function getTypeRandom() internal returns (TypeAnt) {
        uint256 maxSizeEnum = 4;
        uint256 value = randomNumber(maxSizeEnum);
        return TypeAnt(value);
    }

    function growCharacterWithFee(
        address nftContract,
        uint256 castleId,
        uint256 tokenId,
        uint256 price
    ) public payable nonReentrant {
        TypeAnt typeAnt = idToCastle[castleId].typeAnt;
        uint256 hatchTime = idToCastle[castleId].hatchTime;
        uint256 rare = idToCastle[castleId].rare;

        require(hatchTime > block.timestamp, "This egg hatched");
        require(price >= 5 * 1e18, "Fee must bigger than 5 ANT");

        idToCastle[castleId].hatchTime = block.timestamp;
        ...
        bytes memory data = "GrowCastle";
        antToken.operatorSend(msg.sender, address(this), price, data, data);

        emit MarketItemCreated(itemId);
    }

    function fetchMyCastles() public view returns (Castle[] memory) {
        uint256 itemCount = _castleIds.current();
        uint256 size = 0;
        uint256 indexCastle = 0;

        for (uint256 i = 0; i < itemCount; i++) {
            uint256 currentId = i + 1;
            if (
                idToCastle[currentId].owner == msg.sender &&
                idToCastle[currentId].hatchTime > block.timestamp
            ) {
                size += 1;
            }
        }

        Castle[] memory castles = new Castle[](size);
        for (uint256 i = 0; i < itemCount; i++) {
            uint256 currentId = i + 1;
            if (
                idToCastle[currentId].owner == msg.sender &&
                ...
                Castle storage currentItem = idToCastle[currentId];
                castles[indexCastle] = currentItem;
                indexCastle++;
            }
        }
        return castles;
    ...
    function growCharacterWithoutFee(
        address nftContract,
        uint256 castleId,
        uint256 tokenId
    ) public payable nonReentrant {
        ...
        require(hatchTime <= block.timestamp, "This egg doesn't hatch");
        ...
        nonReentrant
    {
        ...
        require(sold == false, "You can't buy this nft!");

        bytes memory data = "BuyItem";
        antToken.operatorSend(
            msg.sender,
    ...
    function resellMarketItem(
        ...
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        bool isBot = idToMarketItem[itemId].isBot;
        ...
        require(isBot == false, "It's a bot");

        bytes memory data = "SellItem";
        antToken.operatorSend(
            msg.sender,
            address(this),
            (price * fee) / 100,
    ...
    /* Returns all bots */
    function fetchBots() public view returns (MarketItem[] memory) {
        uint256 totalItemCount = _itemIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idToMarketItem[i + 1].isBot == true) {
                itemCount += 1;
            }
        }
    ...
    function fetchMyNFTs() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                ...
                idToMarketItem[i + 1].isBot == false
            ) {
                itemCount += 1;
            }
        }
    ...
    function fetchDetailItem(uint256 itemId)
        public
        view
    ...
    function setFee(uint256 newFee) public {
        ...
        require(newFee <= 2, "The fee is too big");
        fee = newFee;
    }
...
contract Util {
  ...
  function _util_init() internal {
    randNonce = 0;
  }

  function cooldownTime(uint rare) internal view returns(uint) {
    return block.timestamp + (1 days) / rare;
  }

  function randomNumber(uint size) internal returns(uint) {
    randNonce++;
    uint randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, randNonce))) % size;
    return randomnumber;
  }
}
