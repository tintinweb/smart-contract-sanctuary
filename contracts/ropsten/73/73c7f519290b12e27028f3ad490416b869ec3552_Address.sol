...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private charNamePrefixes = [
    "The Bloodthirsty",
    "The Great",
    "The Holy",
    "The Unholy",
    ...
    string[] private charNameSuffixes = [
    "of House Osraige",
    "of House Laois",
    "of House Muma",
    "of House Larmuman",
    ...
    function pluckChar(
        ...
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("RACE") && greatness > 14) {
            output = string(abi.encodePacked(output, " ", charSuffixes[rand % charSuffixes.length]));
        }
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("PROFESSION") && greatness >= 19) {
            string[2] memory name;
            name[0] = charNamePrefixes[rand % charNamePrefixes.length];
            name[1] = charNameSuffixes[rand % charNameSuffixes.length];
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[16] = getWeapon(tokenId);

        parts[17] = '</text><text x="10" y="180" class="base">';

        parts[18] = getChest(tokenId);

        parts[19] = '</text><text x="10" y="200" class="base">';

        parts[20] = getHead(tokenId);

        parts[21] = '</text><text x="10" y="220" class="base">';

        parts[22] = getWaist(tokenId);

        parts[23] = '</text><text x="10" y="240" class="base">';

        parts[24] = getFoot(tokenId);

        parts[25] = '</text><text x="10" y="260" class="base">';

        parts[26] = getHand(tokenId);

        parts[27] = '</text><text x="10" y="280" class="base">';

        parts[28] = getNeck(tokenId);

        parts[29] = '</text><text x="10" y="300" class="base">';

        parts[30] = getRing(tokenId);

        //Mount

        parts[32] = '</text><text x="185" y="40" class="base">';

        parts[31] = getCreature(tokenId);

        parts[32] = '</text><text x="185" y="60" class="base">';

        parts[33] = getSaddle(tokenId);

        parts[34] = '</text><text x="185" y="80" class="base">';

        parts[35] = getEquipment(tokenId);

        parts[36] = '</text><text x="185" y="100" class="base">';

        parts[37] = getInventory(tokenId);
