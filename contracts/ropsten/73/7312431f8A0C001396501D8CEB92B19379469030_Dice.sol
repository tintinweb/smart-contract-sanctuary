...
contract Dice is ERC721Enumerable, ReentrancyGuard, Ownable {
    uint nonce;
    uint256[] private _rolls;
    mapping(uint256 => uint256) private _rollsIndex;
    ...
    function getCurrentRoll(uint256 tokenId) public view returns (uint) {
       return _rolls[_rollsIndex[tokenId]];
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; } .roll { fill: white; font-family: serif; font-size: 42px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">';
        parts[1] = string(abi.encodePacked('#', tokenId, ' - ', adjective, ' ', material, ' Dice'));
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Dice #', Strings.toString(tokenId), '", "description": "Dice", "attributes": [{"trait_type": "roll", "value": "',  currentRoll, '"}, {"trait_type": "adjective", "value": "', adjective, '"}, {"trait_type": "material", "value": "', material, '"}], "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function _setRoll(uint256 tokenId, uint256 numericRoll) private {
        _rollsIndex[tokenId] = _rolls.length;
        _rolls.push(numericRoll);
