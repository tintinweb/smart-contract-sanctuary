...
// Fairmoon $KBR
// Website: https://kybercoin.club
// Telegram: https://t.me/kybercoin
...
contract Kuber is Context, IERC20, Ownable {
    ...
    string private _name = "Kuber";
    string private _symbol = "KBR";
    ...
    uint256 private _previousBurnFee = _liquidityFee;
    ...
    uint256 public _maxTxAmount = 10000 * 10**3 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 15000 * 10**3 * 10**9;
    ...
    // Owner cannot transfer out Fairmoon from this smart contract
    function transferAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out Fairmoon!");
    ...
    function setWalletBanStatus(address user, bool banned) external onlyOwner {
        if (banned) {
            require(1618531200 + 3 days > block.timestamp, "Owner cannot longer ban wallets");
    ...
    function _transferStandard(
        ...
        _reflectFee(rFee, tFee);
        //Burn some tokens
        uint256 _burn_amount = calculateBurnFee(tAmount);
        _burn(_msgSender() ,_burn_amount);
        emit Transfer(sender, recipient, tTransferAmount);
    }
