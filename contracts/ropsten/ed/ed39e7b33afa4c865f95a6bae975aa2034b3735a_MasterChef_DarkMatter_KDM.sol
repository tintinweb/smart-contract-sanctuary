...
contract MasterChef_DarkMatter_KDM is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        // deposit fee can't excess more than 6.9%
        require(_depositFeeBP <= 690, "add: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
      // deposit fee can't excess more than 6.9%
       require(_depositFeeBP <= 690, "set: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
