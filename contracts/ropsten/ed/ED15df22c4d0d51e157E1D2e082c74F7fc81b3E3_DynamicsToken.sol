...
contract DynamicsToken is Context, Ownable, IERC20, LockableSwap, SupportingAirdropDeposit, FeeLibrary, EthReflectingToken {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if(!inSwapAndLiquify && from != uniswapV2Pair && tradingOpen){
                selectSwapEvent();
            }
        } else {
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(buyBackEnabled && address(this).balance >= minBuyBack){
            ...
            _rOwned[address(airdropTokenInjector)] = _rOwned[address(airdropTokenInjector)].add(excessTokens.mul(_getRate()));
            ...
            if(redist >= buyback && redist >= liq && ethSwapEnabled){
            ...
            } else if(buyback >= liq && buyBackEnabled){
                // Swap for buyback
    ...
    function buyBackTokens(uint256 amount) private {
        ...
        swapETHForTokens(amount, address(airdropTokenInjector));
    ...
    function setBuyBackRange(uint256 _minBuyBackWei, uint256 _maxBuyBackWei) public onlyOwner {
    ...
    function startInit(address reflectorAddress, address tokenInjectorAddress, address projectContractAddress) external onlyOwner {
    ...
    function init() public onlyOwner{
        ...
        initSellFees();
        openTrading(true);  // TODO Remove for mainnet
        runOnce = 0;
    ...
    function checkWithPool(uint256 aNumber) private returns(uint256 anAppropriateNumber){
        ...
        uint256 fractionOfPool = amountInPool().div(1000);
