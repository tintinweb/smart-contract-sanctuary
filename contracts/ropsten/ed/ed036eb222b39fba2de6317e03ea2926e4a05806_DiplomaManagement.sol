...
contract DiplomaManagement is Ownable {
    ...
    function searchDiplomaByID(uint256 _id)public returns (uint256 diplomaId, string fullName, string birthDay, string date, DiplomaStatus status, DiplomaTypes _type, DiplomaRanks rank, address organizer) {
        require(_id < numDiplomas);
        require(balanceOf[msg.sender] >= searchingFee);// searching fee
        
        Diploma memory d = diplomas[_id];
        
        balanceOf[msg.sender] = balanceOf[msg.sender].sub(searchingFee);// sub balance of sender
        
        balanceOf[wallet] = balanceOf[wallet].add(searchingFee.div(2));// plus 50% fee into owner address
        
        balanceOf[d.organizer] = balanceOf[d.organizer].add(searchingFee.div(2)); //plus 50% fee into organizer address
        
        return (d.diplomaId, d.fullName, d.birthDay, d.date, d.status, d._type, d.rank, d.organizer);
    }
    ...
    // users can deposit their ether into contract.
