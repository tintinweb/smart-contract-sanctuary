...
contract KYC is Ownable{
    ...
    modifier isKYCMainAddress(address addr) {
        require(addr != ZERO_ADDRESS);
    ...
    function addCustomerFromKYC(address _customerAddress) private isKYCAddress(msg.sender) isCustomerNotAdded(_customerAddress) returns (bool){
        customers[_customerAddress] = Customer(_customerAddress, false, now, "", false);
        return true;
    }
    ...
    function getCustomerInfo(address _customerAddress) private view onlyOwner isCustomerAdded(_customerAddress) returns (address, bool, uint, bytes, bool){
        Customer memory c = customers[_customerAddress];
        return (c.addr, c.isLocked, c.expiredTime, c.signature, c.approvalStatus);
    }
    ...
    function isCustomerHasKYCfromWL(address _customerAddress) private view isCustomerAdded(_customerAddress) isWLAddress(msg.sender) returns (bool){
        Customer memory c = customers[_customerAddress];
    ...
    function startPaymentProcess(address _paymentAddress) public isKYCMainAddress(msg.sender) isValidAddress(_paymentAddress) returns(bool){
