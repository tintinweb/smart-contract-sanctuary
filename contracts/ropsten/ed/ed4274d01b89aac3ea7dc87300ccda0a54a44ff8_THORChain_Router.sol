...
contract THORChain_Router {
    ...
    event TransferOutAndCall(address indexed vault, address target, address asset, uint amount, address finalAsset, address to, string memo);
    ...
    // FinalAsset and To come from originating memo
    // Memo passed in here is the "OUT:HASH" type
    function transferOutAndCall(address payable target, address finalAsset, address to, string memory memo) public payable nonReentrant {
        uint256 _safeAmount = msg.value;
        (bool success, ) = target.call{value:_safeAmount}(abi.encodeWithSignature("swapOut(address,address)", finalAsset, to));
        if (!success) {
            payable(address(msg.sender)).transfer(_safeAmount); // Fire and forget, can't refund
        }
        emit TransferOutAndCall(msg.sender, target, address(0), _safeAmount, finalAsset, to, memo);
    }
