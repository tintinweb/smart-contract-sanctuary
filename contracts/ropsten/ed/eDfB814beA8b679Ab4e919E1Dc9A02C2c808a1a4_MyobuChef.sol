...
contract MyobuChef is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    struct UserInfo {
		...
		uint256 lockTimeBoost;          // the lock time boost, max is x3
...
//--------------------------------------------------------VIEW FUNCTIONS --------------------------------------------------------
	...
	uint256 PendingRWT = (userWeightedAmount * accRwtPerShare / 1e12) - user.rewardDebt;
    uint256 PendingETH = (userWeightedAmount * accETHPerShare / 1e12) - user.ETHrewardDebt;
...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(tokenReleased) {
            uint256 rwtReward = (multiplier * rwtPerBlock * pool.allocPoint) / totalAllocPoint;
            rwt.mint(rwtReward);
            pool.accRwtPerShare = uint256(pool.accRwtPerShare + rwtReward * 1e12 / lpSupply);
        }
        uint256 ETHReward = (multiplier * ETHPerBlock * pool.allocPoint) / totalAllocPoint;
        ETHLeftUnclaimed = ETHLeftUnclaimed + ETHReward;
        ETHLeftUnshared = ETHLeftUnshared - ETHReward;
        pool.accETHPerShare = uint256(pool.accETHPerShare + ETHReward * 1e12 / lpSupply);
        pool.lastRewardBlock = uint64(block.number);
    }
    ...
    function withdraw(uint32 _pid, uint256 _amount) public nonReentrant {
        ...
        require(user.lockedUntil >= block.timestamp, "withdraw: Tokens locked, if you're trying to claim your rewards use the deposit function"); // deposit(pid, 0) will harvest your rewards
        ...
        user.rewardDebt = ((user.amount - _amount) * pool.accRwtPerShare / 1e12);
        ...
        else if(user.lockedUntil <= block.timestamp){
            updateUserWeightedBalance(_pid, msg.sender);
        }
    ...
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn, uint256 amountETHMin, uint256 amountTokenMin, uint256 lockTime) public payable nonReentrant {
        ...
        updatePool(0);
        if (userInfo[0][msg.sender].weightedBalance > 0) {
            if(tokenReleased) {
                uint256 pending = (userInfo[0][msg.sender].weightedBalance * poolInfo[0].accRwtPerShare / 1e12) - userInfo[0][msg.sender].rewardDebt;
                if (pending > 0) {
                safeRWTTransfer(msg.sender, pending);
                }
                userInfo[0][msg.sender].rewardDebt = (userInfo[0][msg.sender].weightedBalance * poolInfo[0].accRwtPerShare / 1e12);

            }
            uint256 ETHPending = (userInfo[0][msg.sender].weightedBalance * poolInfo[0].accETHPerShare / 1e12) - userInfo[0][msg.sender].ETHrewardDebt;
            if (ETHPending > 0) { 
                safeETHTransfer(msg.sender, ETHPending);
                userInfo[0][msg.sender].ETHrewardDebt = (userInfo[0][msg.sender].weightedBalance * poolInfo[0].accETHPerShare / 1e12);
        ...
        userInfo[0][msg.sender].amount += uint256(numLiquidityAdded);
        require(lockTime >= 604800 || userInfo[0][msg.sender].lockedUntil >= block.timestamp, "addLiquidityNoFeeAndStake : Can't lock tokens for less than 1 week");
        if (lockTime > 0) {
            lockTokens(0, lockTime);
        }
    ...
    function reinvestETHRewards(uint256 amountOutMin) public nonReentrant {
            ...
            updatePool(1);
            require(user.weightedBalance > 0, "reinvestETHRewards: No tokens staked");
            require(user.lockedUntil >= block.timestamp, "reinvestETHRewards: Tokens not locked");
            if(tokenReleased) {
                uint256 pending = (user.weightedBalance * poolInfo[1].accRwtPerShare / 1e12) - user.rewardDebt;
                if (pending > 0) {
                safeRWTTransfer(msg.sender, pending);
                userInfo[1][msg.sender].rewardDebt = (user.weightedBalance * poolInfo[1].accRwtPerShare / 1e12);
                }
            }
            address[] memory path = new address[](2);
            path[0] = WETH;
            path[1] = address(Myobu);
            uint256 ETHPending = (user.weightedBalance *  poolInfo[1].accETHPerShare / 1e12) - user.ETHrewardDebt;
            if (ETHPending > 0) { 
                ...
                userInfo[1][msg.sender].amount += uint256(amountSwapped);
                updateUserWeightedBalance(1, msg.sender);
                userInfo[1][msg.sender].ETHrewardDebt = (user.weightedBalance * poolInfo[1].accETHPerShare / 1e12);
				emit Deposit(msg.sender, 1, amountSwapped);
            }
    }
    function buyAndStakeETH(uint256 amountOutMin, uint256 lockTime) public payable {
        ...
        if (_amount > 0) {
            require(lockTime >= 604800 || user.lockedUntil >= block.timestamp, "buyAndStakeETH : Can't lock tokens for less than 1 week");
            user.amount = user.amount + uint256(_amount);
        }
		else if (user.lockedUntil != 0 && user.lockedUntil <= block.timestamp) {
			updateUserWeightedBalance(1, msg.sender);

		}
		if (lockTime > 0) {
		    lockTokens(1, lockTime);
		}
...
//--------------------------------------------------------INTERNAL FUNCTIONS --------------------------------------------------------
    function _deposit(address sender, uint256 _pid, uint256 _amount, uint256 lockTime) internal {
        ...
        if (_amount > 0) {
            require(lockTime >= 604800 || user.lockedUntil >= block.timestamp, "deposit : Can't lock tokens for less than 1 week");
    ...
    function lockTokens(uint256 _pid, uint256 lockTime) internal {
        require(userInfo[_pid][msg.sender].amount > 0, "lockTokens: No tokens to lock"); 
        require(userInfo[_pid][msg.sender].lockedUntil <= block.timestamp + lockTime, "lockTokens: Tokens already locked");
        require(lockTime <= 31449600, "lockTokens: Lock time too long");
        userInfo[_pid][msg.sender].lockedUntil = block.timestamp + lockTime;
        userInfo[_pid][msg.sender].lockTimeBoost = 2 * ((1000 * lockTime) / 31449600); // 0 - 2000 
        updateUserWeightedBalance(_pid, msg.sender);
		emit TokensLocked(msg.sender, block.timestamp, lockTime);
    }
    
    // calculate and update the user weighted balance
	function updateUserWeightedBalance(uint256 _pid, address user) public {
        uint256 amount = userInfo[_pid][user].amount;
		uint256 poolBalance = poolInfo[_pid].weightedBalance - userInfo[_pid][user].weightedBalance;
		if (userInfo[_pid][user].lockedUntil != 0 && userInfo[_pid][user].lockedUntil <= block.timestamp) {
		    userInfo[_pid][user].lockedUntil = 0;
		    userInfo[_pid][user].lockTimeBoost = 0;
		}
		userInfo[_pid][user].weightedBalance = uint256(amount * (1000 + userInfo[_pid][user].lockTimeBoost) * (1000 + userInfo[_pid][user].boostPointsBP)) / 1000000;
		poolInfo[_pid].weightedBalance = uint256(poolBalance + userInfo[_pid][user].weightedBalance);
		
    }
    
    // triggered on refills. 
    function updateETHRewards() internal {
        // ETHPerBlock = ETHLeftUnshared / nb of blocks in 7 days. 
        // The contact assumes it's not going to receive any ether for another 7 days and computes the rewards for this period accordingly.
        // Refilling early isnt an issue, any unshared ETH left will make it into the next rewards cycle
        ETHPerBlock = ETHLeftUnshared / (5760 * 7);
        massUpdatePools();
    }
