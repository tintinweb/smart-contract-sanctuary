...
contract YeouidoTokens is
    MinterAccess,
    RelayRecipient,
    ERC721("Yeouido", "YEO"),
    IYeouidoTokens
{
    IYeouidoCards public yeouidoCards;
    INextContract public nextContract;

    constructor(address yeouidoCardsAddress, address relayAddress)
        public
        RelayRecipient(relayAddress)
    {
        require(
            yeouidoCardsAddress != address(0),
            "YeouidoCards address is required"
        );
        yeouidoCards = IYeouidoCards(yeouidoCardsAddress);
    }
    ...
    function createCardAndMintToken(
        ...
        uint256 cardId = yeouidoCards.createCard(
            playerId,
            season,
            scarcity,
            serialNumber,
    ...
    function mintToken(uint256 cardId, address to)
        ...
        returns (uint256)
    {
        require(yeouidoCards.cardExists(cardId), "Card does not exist");
    ...
    {
        (name, yearOfBirth, monthOfBirth, dayOfBirth) = yeouidoCards.getPlayer(
            playerId
        );
    }
    ...
    {
        (name, country, city, yearFounded) = yeouidoCards.getClub(clubId);
    }
