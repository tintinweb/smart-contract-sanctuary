...
contract Saving is TokenWraper {
    ...
    function rewardEarned(address account) public view returns(uint256) {
        ...
        uint256 _rangeTime = _currentTime.sub(savedTimestamp[msg.sender]);
        uint256 _rangeTimeClaimed = _currentTime.sub(claimRewardTimestamp[msg.sender]);
        _rangeTime = _rangeTime.sub(_rangeTimeClaimed);
