...
contract MACToken is MintableToken, BurnableToken, PausableToken {
    constructor(address _owner, address _minter)
        StandardToken(
            "MAC Token",   // Token name
            "MAC", // Token symbol
            18  // Token decimals
        )
        HasOwner(_owner)
...
contract MACTokenSafe is TokenSafe {
    ...
    public
  {
    ...
    init(
      1, // Group Id
      1545761700 // Release date = 2018-12-25 18:15 UTC
    );
    add(
      1, // Group Id
      0xEB0b6b9cCe9F6a18aDeb95166e7066cE0A78CB1a,  // Token Safe Entry Address
      40000000000000000000  // Allocated tokens
    );
  }
}
...
/**
 * @title MACTokenFundraiser
 */

contract MACTokenFundraiser is MintableTokenFundraiser, IndividualCapsFundraiser, CappedFundraiser, RefundableFundraiser, GasPriceLimitFundraiser {
    MACTokenSafe public tokenSafe;
        ...
        public
    {
        ...
        tokenSafe = new MACTokenSafe(token);
        MintableToken(token).mint(address(tokenSafe), 40000000000000000000);

        initializeBasicFundraiser(
            1545758340, // Start date = 2018-12-25 17:19 UTC
            1545760800,  // End date = 2018-12-25 18:00 UTC
            10000, // Conversion rate = 10000 MAC per 1 ether
            0xEB0b6b9cCe9F6a18aDeb95166e7066cE0A78CB1a     // Beneficiary
        );

        initializeIndividualCapsFundraiser(
            (0.002 ether), // Minimum contribution
            (0.002 ether)  // Maximum individual cap
        );
        ...
        initializeCappedFundraiser(
            (0.004 ether) // Hard cap
        );

        initializeRefundableFundraiser(
            (0.004 ether)  // Soft cap
        );
