...
contract Token721 is IERC721 {
    ...
    function balanceOf(address _owner) external view override returns (uint256) {
        require(_owner != address(0), "address(0) is invalid.");
    ...
    function approve(address _approved, uint256 _tokenId) public override {
        address owner = tokenOwner[_tokenId];
        require(msg.sender == owner || ownerOperator[owner][msg.sender], "Dont have permission to approve this token");
    ...
    function validSender(uint _tokenId, address _owner) private view returns (bool) {
    ...
    function transfer(address _from, address _to, uint256 _tokenId) private {
        ...
        require(validSender(_tokenId, _from), "Dont have permission to send this token.");
