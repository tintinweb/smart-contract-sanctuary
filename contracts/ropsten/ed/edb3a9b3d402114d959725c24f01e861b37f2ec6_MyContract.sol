...
contract MyContract{
    ...
    function token_transfer (uint token_index) public {
        ...
        uint total_payment = 10 * random_number + (block.timestamp - Tokens[token_index].timestamp_when_received) / 6; // add in time-panelty payment
    ...
    function winner_winner_chicken_breakfast () public{
        if (is_it_game_over() == true){
            ...
            uint256 payout_amount = Players[player_id].balance * 1000 gwei;
    ...
    function kill_afk_player () public {
        for (uint i = 1; i <= tokenCount; i += 1){
            ...
            if (current_balance - time_panelty_payment <= 0){
            ...
            // Players[Tokens[i].id].balance = current_balance - total_payment;
        }
    }
        ...
        if (living_total > 0){
            for (uint i = 1; i <= peopleCount; i++){
                if (Players[i].active == true){
                    ...
                    // Players[i].balance = Players[i].balance*1000 + deserved_amount*900;
