...
//    event NewCommunist(address account);
    ...
    string private _name = "Marx3";
    string private _symbol = "MARX3";
 ...
 /*   function withdrawDividends(address communist) public {
    ...
    function calculateDividends(address communist) public view returns (uint256) {
        require(communist == msg.sender);
        uint256 myDividends = _balances[dividendsAccount] / _accounts.length;
        return myDividends;
    }
    
     function calculateDividendsOther(address communist) public view returns (uint256) {
        require(communist == msg.sender);
        uint256 myDividendsOther = _balances[dividendsAccount].div(_accounts.length);
        return myDividendsOther;
    }
     ...
     function withdrawDividends(address communist) public {
        ...
        uint256 _dividends = _balances[dividendsAccount];
        //uint256 myDividends = _dividends.div(_accounts.length);
        _transfer(dividendsAccount, communist, _dividends);
        
        emit DividendPayment(communist, _dividends);
    }
