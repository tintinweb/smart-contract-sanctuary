...
contract TokenDistribution is Ownable {
    ...
    function _calculateAvailablePercentage() private view returns (uint256 availablePercentage) {
        // 25% on listing and rest daily distribution from day 2 for 9 months (9 months)

//  2680000 FMT Assigned
// 670000 tokens on TGE- 25% on TGE
// 2010000 tokens distributed for 270 days- 75% remaining
// 2010000/270=7444.444444 tokens per day
// 75/270=0.277777778% everyday released


  uint256 secondDay = initialTimestamp + 1 days;
        uint256 nineMonth = initialTimestamp + 270 days;
        uint256 remainingDistroPercentage = 75;
...
uint256 currentTimeStamp = block.timestamp;
        ...
        if (currentTimeStamp > initialTimestamp) {
            if (currentTimeStamp <= secondDay) {
                return uint256(25).mul(1e18);
            } else if (currentTimeStamp > secondDay && currentTimeStamp < nineMonth) {
                // Date difference in days - (endDate - startDate) / 60 / 60 / 24; // 40 days

                uint256 noOfDays = (currentTimeStamp.sub(secondDay)).mul(1e18).div(60).div(60).div(24);
