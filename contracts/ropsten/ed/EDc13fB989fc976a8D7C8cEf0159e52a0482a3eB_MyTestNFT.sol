...
library Base64 {
  ...
  function encode(bytes memory _data) internal pure returns(bytes memory) {
    ...
    for (uint256 index = 0; index < _data.length; index += 3) {
      uint8 _val = uint8(_data.getValueAt(index) & 0x3f);
      _encoded = bytes.concat(_encoded, _encode6bits(_val));

      _val = uint8(_data.getValueAt(index) >> 6 | (_data.getValueAt(index + 1) & 0x0f) << 2);
      _encoded = bytes.concat(_encoded, _encode6bits(_val));

      _val = uint8(_data.getValueAt(index + 1) >> 4 | (_data.getValueAt(index + 2) & 0x03) << 4);
      _encoded = bytes.concat(_encoded, _encode6bits(_val));

      _val = uint8(_data.getValueAt(index + 2) >> 2);
      _encoded = bytes.concat(_encoded, _encode6bits(_val));
    }

    return bytes.concat(_encoded, bytes1("=").repeat(4 - _encoded.length % 4));
  }
