...
contract Hazel2 is ERC721Enumerable, ReentrancyGuard, Ownable{
	...
	bool private isGiveawayMinted = false;
	bool private isSpecialSetMinted = false;

	uint16 private constant _supplyLoot = 8000;
	uint16 private constant _supplyWhitelist = 1000;
	uint16 private constant _supplyMLoot = 9993;
	uint16 private constant _supplyGA = 1000;
	...
	uint16 private _tokenIdStartGiveaway = 7778;
	uint16 private _tokenIdStartMLoot = 8001;
	uint16 private _tokenIdStartGA = 18994;
	...
	// Type = Loot, MLoot, GA
	event HazelMinted(uint256 indexed id, uint256 indexed fromTokenType, uint256 indexed fromTokenId, uint256[] traits);

	constructor() ERC721("Hazel2", "HZL") {}
	...
	// Token ID 1 - 8,000
	function mintWithLoot(uint256 lootId, uint256[] calldata traits) public payable nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
		require(lootId > 0 && lootId < 7778, "TOKEN_ID_OUT_OF_RANGE");
		require(lootContract.ownerOf(lootId) == msg.sender, "MUST_OWN_TOKEN_ID");
		require(MINT_PRICE <= msg.value, "WRONG_ETHER_VALUE");
		_checkTraits(traits);

		_mintHazel(msg.sender, lootId);
		_countTokenLoot.increment();
		emit HazelMinted(lootId, 0, lootId, traits);
	}

	// Token ID 7,777-8,000
	function mintGiveaway(uint256 lootId, uint256[] calldata traits) public nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
		require(lootId > 7777 && lootId < 8001, "TOKEN_ID_OUT_OF_RANGE");
		// Only allowed to mint specific loot id
		require(_giveawayList[msg.sender] == lootId, "ADDRESS_NOT_WHITELISTED");
		_checkTraits(traits);
	...
	function mintWithWhitelist(uint256 mlootId, uint256[] calldata traits) public payable nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
		require(_countTokenMLoot.current() < _supplyMLoot, "SOLD_OUT");
		require(_whitelist[msg.sender], "ADDRESS_NOT_WHITELISTED");
		require(MINT_PRICE <= msg.value, "WRONG_ETHER_VALUE");
		_checkMLoot(mlootId, msg.sender);
		_checkTraits(traits);

		_mintHazel(msg.sender, _tokenIdStartMLoot + _countTokenMLoot.current());
		emit HazelMinted(_tokenIdStartMLoot + _countTokenMLoot.current(), 1, mlootId, traits);
	...
	// Token ID 8,001 - 18993
	function mintWithMLoot(uint256 mlootId, uint256[] calldata traits) public payable nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
	...
	function _checkTraits(uint256[] calldata traits) private pure {
		// Total 3 traits: [face, eyes, background]
		require(traits.length == 3, "TRAITS_MISMATCH");
		// 12 type of face
		require(traits[0] >= 0 && traits[0] < 12, "TRAIT_OUT_OF_RANGE");
		// 12 type of eyes
		require(traits[1] >= 0 && traits[1] < 12, "TRAIT_OUT_OF_RANGE");
		// 8 type of background
		require(traits[2] >= 0 && traits[2] < 8, "TRAIT_OUT_OF_RANGE");
	}
	...
	function getMintLeftLoot() public view returns (uint256) {
		return _supplyLoot - _countTokenLoot.current() - _supplyGiveaway;
	}

	function getMintLeftWhitelist() public view returns (uint256) {
		return _supplyWhitelist - _countTokenWhitelist.current();
	}

	function getMintLeftMLoot() public view returns (uint256) {
		return _supplyMLoot - _countTokenMLoot.current();
	}
