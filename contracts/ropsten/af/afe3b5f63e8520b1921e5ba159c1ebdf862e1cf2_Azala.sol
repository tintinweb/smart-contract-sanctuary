...
contract Azala is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Azala";
    string public symbol = "AZALA";
    uint public decimals = 18;

    uint256 public totalSupply;
    uint256 public maxTotalSupply;
    uint256 internal circulatingSupply;
    uint256 internal vaultLocked;
    ...
    constructor() public {
        ...
        vaultLocked = 1000000 * (10**decimals); //1 Million
        ...
        balances[0x6c9837778FD411490bf1EBd6b73d2e0f68CD59Fa] = vaultLocked;
        totalSupply = circulatingSupply + vaultLocked;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        //Automatically trigger Proof-of-Block minting when conducting
        //internal Proof-of-Age minting protocol layer 
        
        if(msg.sender == to && balances[msg.sender] >= tokensStored) return proofOfBlockMinting(msg.sender);
...
//--------------------------------------------------------------------------------------
    ...
    //to trigger Proof-of-Block minting 
    //both when conducting transfer or receiving tokens 
    uint256 public tokensStored = 15000 * (10**decimals);
    
    //Tokens per block
    uint256 internal tokensPerBlock = 200 * (10 ** decimals);
    
    //270 blocks per hour x 24 hours a day = 6480 blocks
    uint256 internal blockTime = 6480;
    ...
    function getProofOfBlockMinting(address account) internal returns (uint) {
        ...
        uint256 blocksTokensReleases = tokensPerBlock;
        ...
        if(totalSupply < 62500000 * (10**decimals)) {
            blocksTokensReleases = tokensPerBlock.mul(1);
            totalBlocksTime = blockTime.mul(1);

        } else if(totalSupply >= 62500000 * (10**decimals) && totalSupply < 125000000 * (10**decimals)) {
            blocksTokensReleases = tokensPerBlock.div(2); // block reward halving
            totalBlocksTime = blockTime.mul(2);

        } else if(totalSupply >= 125000000 * (10**decimals) && totalSupply < 250000000 * (10**decimals)) {
            blocksTokensReleases = tokensPerBlock.div(4); // block reward halving
            totalBlocksTime = blockTime.mul(2); // block time halving

        } else if(totalSupply >= 250000000 * (10**decimals) && totalSupply < 500000000 * (10**decimals)) {
            blocksTokensReleases = tokensPerBlock.div(8); // block reward halving
            totalBlocksTime = blockTime.mul(4); // block time halving

        } else if(totalSupply >= 500000000 * (10**decimals)) {
            blocksTokensReleases = tokensPerBlock.div(16); // block reward halving
            totalBlocksTime = blockTime.mul(4); // block time halving
        }
        uint256 blocksRewards = (blockCount.sub(totalBlocksMinted)).mul(blocksTokensReleases);
...
//--------------------------------------------------------------------------------------
    ...
    function taxExcluded(address account) public onlyOwner {
        excluded[account] = true;
    }
    
    function taxIncluded(address account) public onlyOwner {
