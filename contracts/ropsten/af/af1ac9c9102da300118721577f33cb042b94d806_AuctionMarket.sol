...
,ERC20Payment
{
		...
		if(offer.tokenAddress == address(0)){
			...
			if (existing.value > 0) {
				TransferHelper.safeTransferFrom(
					offer.tokenAddress, address(0), existing.bidder, existing.value
				);
			}
		}
    ...
    function _cancelBid(uint256 _tokenId) internal virtual {
		...
		if(offer.tokenAddress == address(0)){
			...
			TransferHelper.safeTransferFrom(
				offer.tokenAddress, address(0), bid.bidder, bid.value - cancelFee
			);
			TransferHelper.safeTransferFrom(
				offer.tokenAddress, address(0), offer.seller, cancelFee
			);
		}
    ...
    function _endAuction(uint256 _tokenId) internal virtual {
		...
		if(offer.tokenAddress == address(0)){
			...
			TransferHelper.safeTransferFrom(
				offer.tokenAddress, address(this), offer.seller, _sellerValue
			);
			TransferHelper.safeTransferFrom(
				offer.tokenAddress, address(this), _owner, _commissionValue
			);
		}
