...
contract MochiRegiLite is SafeMath {
    ...
    function changeOwner(address _newOwner) public onlyBy(owner){
        require(_newOwner != address(0), "address cannot be 0x0"); 
        owner = _newOwner;
    }
    ...
    //again only accessable by owner, in the event someone sends native token to the contract, way to recover it.
    ...
    function completePayment(uint _serviceFee, uint _registerNumber, uint _amountToPayCents, IERC20 _erc20,
      uint _adjustForDecimal) internal{
            ...
            bool result1 = _erc20.transferFrom(msg.sender, address(this), coinAmountMinusSerivceFee);
            require(result1, "transfer in was not successful");
            bool result2 = true; 
            if (serviceFeeTo1 > 0){
                result2 = _erc20.transfer(serviceFeePayTo1, serviceFeeTo1);
            }
            bool result3 = true; 
            if (serviceFeeTo2 > 0){
                result3 = _erc20.transfer(serviceFeePayTo2, serviceFeeTo2);
            }
            bool result4 = true;
            if (directPayOutAddress != address(0)){
                result3 = _erc20.transfer(directPayOutAddress, coinAmountMinusSerivceFee);
            }
            require(result2 && result3 && result4, "transfer out was not successful");
            if (directPayOutAddress == address(0)){
...
contract MochiRegiLiteFactory is SafeMath{
