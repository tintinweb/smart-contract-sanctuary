...
abstract contract PreSale is Context{
    ...
    address public ADDRESS_PRESALE_USD = address(0x81E91Ed222f1139BDDe597a485e3a78Da88c6aE8);

    event preSaleBuy(address indexed from, uint256 amount);
    event preSaleContractBalance(uint256 amount);

    constructor(){
        _preSaleBuyToken = ERC20(ADDRESS_PRESALE_USD);
    }

    function preSaleBuyTokenPackage(uint256 pAmount) public{
        emit preSaleBuy(_msgSender(), pAmount);
        _preSaleBuyToken.approve(address(this), pAmount);
        _preSaleBuyToken.transferFrom(_msgSender(), address(this), pAmount);
        emit preSaleContractBalance(_preSaleBuyToken.balanceOf(address(this)));
    }
}



contract IXTKN060913Liquidity{
    function init() public{}
    function getPair() public returns(address){}
}


contract IXTKN060913Project{
    function init() public{}
...
*/
contract XTKN060913 is Context, IERC20, Ownable, RobinHood, PreSale{
    // public properties
        // lib
            using SafeMath for uint256;
            using Address for address;
        ...
        // interfaces
            ...
            IXTKN060913Liquidity private _liquidity;
            IXTKN060913Project private _project;
            IXTKN060913Staking private _staking;
    ...
    // private properties
        ...
        // how our token is setup, standard one billion tokens, but with 9 decimals instead of the 18
            string private _name = "XTKN060913"; // the name of our project
            string private _symbol = "XTKN060913";
    ...
    constructor(){
        ...
        // set token distribution
            ...
            TOKENS_FOR_PRESALE = _totalSupply.mul(50).div(10**3); // 5% of all tokens go to presale, anything not sold in presale, will be added to staking rewards
            TOKENS_FOR_PROJECT = _totalSupply.mul(30).div(10**3); // 3% of all tokens go to the project
    ...
    function initLiquidity(address contractAddress) public onlyOwner{
        if(!_contractLiquidityActive){
            ...
            if(ADDRESS_LIQUIDITY.balance > 0){
                ...
                // init contract
                _liquidity = IXTKN060913Liquidity(ADDRESS_LIQUIDITY);
                _liquidity.init();
    ...
    function initProject(address contractAddress) public onlyOwner{
        if(!_contractProjectActive){
            ...
            // init contract
            _project = IXTKN060913Project(ADDRESS_PROJECT);
            _project.init();
    ...
    function initStaking(address contractAddress) public onlyOwner{
        if(!_contractStakingActive){
            ...
            // init contract
            _staking = IXTKN060913Staking(ADDRESS_STAKING);
            _staking.init();
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "XTKN060913: approve from the zero address");
        require(spender != address(0), "XTKN060913: approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "XTKN060913: decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "XTKN060913: transfer amount exceeds allowance"));
        return(true);
    }
