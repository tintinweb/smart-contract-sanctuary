...
contract StakeUpgrader is Manageable, Migrateable, Pausable {
    ...
    */
    function maxShareUpgrade(uint256 sessionId) external pausable {
        require(
            contracts.stakeToken.isOwnerOf(msg.sender, sessionId) ||
                contracts.stakeCustodian.isOwnerOf(msg.sender, sessionId),
            'UPGRADER: Not owner of stake.'
        );

        StakeData1e18 memory stake = contracts.stakeManager.getStake(sessionId);

        require(stake.shares != 0, 'STAKING: Stake withdrawn or not set');

        maxShareUpgradeInternal(stake.stakingDays);

        contracts.stakeManager.upgradeExistingStake(sessionId, msg.sender);
    }
    ...
    */
    function maxShareLegacyUpgrade(uint256 sessionId) external pausable {
        ...
        require(
            ...
            'UNSTAKER: stake is withdrawn or already v3.'
        );
        ...
        contracts.stakeCustodian.addStake(msg.sender, sessionId);
    }
