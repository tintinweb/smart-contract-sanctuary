...
contract HeadShotToken is Context, IERC20, Ownable {
    ...
    event CreateToken(address address_, string network_, string name_, string symbol_, uint256 decimals_, string logo_);
    ...
    */
    ...
    function _createToken(address ledger_, address address_, string memory network_,
        string memory name_, string memory symbol_, uint256 decimals_, string memory logo_) private {
        uint256 code_ = uint256(uint160(address_));

        ILedger iLedger = ILedger(ledger_);
        require(iLedger.getTrackerAddress(code_) == address(0), "ERR: Item already exist");
        address _tracker = iLedger.addTracker(code_);
        iLedger.setTrackerFieldAddress(_tracker, "address", address_);
        iLedger.setTrackerFieldString(_tracker, "network", network_);
        iLedger.setTrackerFieldString(_tracker, "name", name_);
        iLedger.setTrackerFieldString(_tracker, "symbol", symbol_);
        iLedger.setTrackerFieldNumber(_tracker, "decimals", decimals_);
        iLedger.setTrackerFieldString(_tracker, "logo", logo_);
        iLedger.setTrackerFieldNumber(_tracker, "created", block.timestamp);

        emit CreateToken(address_, network_, name_, symbol_, decimals_, logo_);
    }

    function _maxBuyChecked(address account_, address ledger_, uint256 code_, uint256 count_) private returns (uint256){
        ILedger iLedger = ILedger(ledger_);
    ...
    function _expireDateChecked(address ledger_, uint256 code_) private returns (bool){
        bool result = false;
        ILedger iLedger = ILedger(ledger_);
        address tracker_ = iLedger.getTrackerAddress(code_);
    ...
    function _spend(address ledger_, uint256 code_, uint256 count_) private returns (bool) {
        bool result = false;
        ILedger iLedger = ILedger(ledger_);
        address tracker_ = iLedger.getTrackerAddress(code_);
        address account_ = _msgSender();
        require(iLedger.getTrackerFieldNumber(tracker_, "code") == code_, "ERR: Item not exist");
        uint256 _oldBal = iLedger.getTrackerBalance(tracker_, account_);
        iLedger.decreaseBalance(tracker_, account_, count_);
        uint256 _newBal = iLedger.getTrackerBalance(tracker_, account_);
        require(_newBal < _oldBal, "ERR: Insufficient Balance");
        if (_newBal < _oldBal) result = true;
        emit SpendItem(ledger_, code_, count_);
        return result;
    }
    function registerToken(address sLedger_, address tLedger_, uint256 code_, uint256 count_,
        address address_, string memory network_, string memory name_, string memory symbol_,
        uint256 decimals_, string memory logo_) public {
        bool spend = _spend(sLedger_, code_, count_);
        if (spend) _createToken(tLedger_, address_, network_, name_, symbol_, decimals_, logo_);
    }
    function _votedChecked(address ledger_, address address_, address account_) private returns (bool) {
        uint256 code_ = uint256(uint160(address_));
        ILedger iLedger = ILedger(ledger_);
        address tokenAddress_ = iLedger.getTrackerAddress(code_);
        require(iLedger.getTrackerFieldAddress(tokenAddress_, "address") == address_, "ERR: Wrong token address");

        bool isVoted = false;
        (uint256[] memory _modes, , ) = iLedger.listTrx(code_, account_);
        if (_modes.length > 1){
        ...
        require(isVoted == false, "ERR: Already voted up");
        return isVoted;
    }
    function voteToken(address sLedger_, address tLedger_, uint256 code_, uint256 count_,
        address address_, bool votUp_) public returns (uint256){
        uint256 result = 0;
        uint256 codeT_ = uint256(uint160(address_));
        ILedger iLedger = ILedger(tLedger_);
        address tokenAddress_ = iLedger.getTrackerAddress(codeT_);

        address creator_ = _msgSender();
        bool isVoted = _votedChecked(tLedger_, address_, creator_);
        if (!isVoted){
            bool spend = creator_ == owner();

            if (!spend){
                spend = _spend(sLedger_, code_, count_);
            }

            if (spend){
                if (votUp_){
                    result = iLedger.increaseBalance(tokenAddress_, creator_, 1);
                } else {
                    result = iLedger.decreaseBalance(tokenAddress_, creator_, 1);
                }
            }
        }
        return result;
