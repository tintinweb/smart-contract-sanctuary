...
contract LaunchX is Ownable {
  ...
  function calculateAmount(uint256 ethAmountInWei) public view returns (uint256) {
    uint256 rate = ethAmountInWei.mul(exchangeRate);
    return (rate * 10 ** uint256(_token.decimals())).div(1 ether);
  }
  ...
  function buyTokens() external payable {
    ...
    // Emit buy event
    emit EBuyTokens(senderAddress, value, totalToken, rewardedAmount, whitelist[senderAddress].amount, whitelist[senderAddress].rewardedAmount, soldAmount, totalRaise, totalParticipant, totalRedeemed);
  }
}