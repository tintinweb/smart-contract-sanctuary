...
contract Claim_Codes {
  ...
  event Country_Blocked(bytes2 country_code);
  event Country_Unblocked(bytes2 country_code);
  ...
  mapping(bytes2 => bool) public blocked_countries;

  function block_country(bytes2 country_code) public only_controller {
    blocked_countries[country_code] = true;
    emit Country_Blocked(country_code);
  }
  function unblock_country(bytes2 country_code) public only_controller {
    blocked_countries[country_code] = false;
    emit Country_Unblocked(country_code);
  }

  function veriry_signature(bytes calldata claim_code, bytes32 message_hash) internal pure returns(address) {
  ...
  function redeem_targeted(bytes calldata claim_code, uint256 denomination, uint8 tranche, uint256 expiry, uint256 nonce, bytes2 country_code) public {
    ...
    address recovered_address = veriry_signature(claim_code, message_hash);
