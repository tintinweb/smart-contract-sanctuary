...
contract Blarity is ReentrancyGuard {
  ...
  function creatorClaimFundTransfer(uint id) public onlyOwner {
    ...
    for(i = 0; i < delegators.length; i++) {
      totalWeight += squareRoot(delegators[i].weight);
    }
    Vote[] storage votes = votings[id];
    uint positive = 0;
    uint negavtive = 0;
    for(i = 0; i < votes.length; i++) {
      if (votes[i].isApproved) { positive += squareRoot(votes[i].weight); }
      else { negavtive += squareRoot(votes[i].weight); }
    }
  ...
  function squareRoot(uint x) internal pure returns (uint y) {
    uint z = (x + 1) / 2;
    y = x;
    while (z < y) {
        y = z;
