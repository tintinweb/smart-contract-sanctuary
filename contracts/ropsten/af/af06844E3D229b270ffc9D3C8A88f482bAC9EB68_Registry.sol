...
contract Registry is Whitelist {
    ...
    event LogNewContract(string indexed contractName, address indexed contractAddress, bool contractiTypeActivated);
    event LogRemovedContract(string indexed contractName, address indexed contractAddress, bool contractTypeDeactivated);
    ...
    function newContract(string calldata contractName, address contractAddress, uint256 _deployed, string calldata _abiVersion, string calldata _tag) external onlyWhitelist {
        ...
        emit LogNewContract(contractName, contractAddress, activation);
    }

    function getKeys() external view returns (string[] memory, bool[] memory) {
        uint256 keyLength = keys.length; 
        bool[] memory _activeKeys = new bool[](keyLength);
        for (uint256 i; i < keyLength; i++) {
            _activeKeys[i] = activeKey[keys[i]];
        }
        return (keys, _activeKeys);
    }
    ...
    function removeContract(string calldata contractName) external onlyWhitelist {
        ...
        emit LogNewContract(contractName, _contract, deactivated);
    }
