...
contract MPHTEST is ERC721Enumerable, Ownable, ReentrancyGuard {
    uint public constant MAX = 8888;
	string _baseTokenURI;
	bool _didWeGetTheReserves = false;
	uint _saleTime = 1638853572; // TEST sale time
	uint _presaleTime = 1637280000; // TEST presale time
	...
	/**
     * @dev Public function to allow minting between _saleTime and _aftersaleTime, checks for transaction value to meet price,
	 * and checks that the amount being minted is less or equal to 5
     *
     *
     * @dev Second function to it is to allow minting during presale time at a maximum of 3 per wallet
     */
    function mint(uint _count) public payable nonReentrant {
        ...
        if (block.timestamp >= _saleTime) {
            ...
            require(_count <= 3, "My Pet Hooligan: Exceeds the max you can mint in the presale");
            require(MPHPresaleTest(_presaleContract).isInPresale(msg.sender) == true, "My Pet Hooligan: You are not in the presale");
            require(MPHPresaleTest(_presaleContract).isInMintedPresale(msg.sender) == false, "My Pet Hooligan: You already minted from the presale");
            for (uint x = 0; x < _count; x++) {
                _safeMint(msg.sender, totalSupply());
            }
            MPHPresaleTest(_presaleContract).addToMinted(msg.sender);
        }
    }
