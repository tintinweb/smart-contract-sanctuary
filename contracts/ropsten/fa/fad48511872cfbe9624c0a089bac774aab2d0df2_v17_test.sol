...
contract v17_test is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // contract activation sets reward to 7 days, easyreward to 1 day
    uint256 private rewardCycleBlock = 100 days;
    uint256 private easyRewardCycleBlock = 100 days;
    ...
    // contractd activation sets it to one week after activation
    uint256 private disableEasyRewardFrom = 0;

    // determines the threshold of tokens on the token contract
    // after which as swap to BNB and LP position is done
    // contract activation sets it to 20B token (=1/3th of total supply moved at 6% fee)
    // should always be lower than the maxTxAmount!
    ...
    string private _name = "v17_test Token";
    string private _symbol = "v17_test";
    uint8 private _decimals = 9;

    address private charityWallet = address(0x173f47c48ED932a7f13FaC2d3bA3453035168F08);
    ...
    uint256 private _maxTotalSupplyPrct = 10000; // // initiated at 100%, later set to 10 -> 10/10000 = 0.1%
    uint256 private _minBalanceToClaim = 5 * 10**6 * 10**9; //5M tokens needed to be able to claim BNB
    ...
    function consultAntiWhaleParameters() external view returns(
        ...
        // no reason to add another master account that would have access to these functions
    function activateContract() external onlyOwnerOrPresales {
        ...
        _taxFee = 1; // in %
        _liquidityFee = 6; // in %, combines liquidity & bnb rewards
        _charityFee = 1; // in %
        _burnFee = 2; // in %
    ...
    //     // reward claim
    //     setDisableEasyRewardFrom(block.timestamp + 1 weeks);
    //     setRewardCycleBlock(2 days);
    //     setEasyRewardCycleBlock(7 days);

    //     // protocol
    //     setMaxTxAmount(10 * 10**12 * 10**decimals());

    //     // set _maxTotalSupplyPrct
    //     setMaxTotalSupply(10); // uses factor 10 000

    //     // set _maxBalancePrct
    //     setMaxBalancePrct(10); // in %

    //     setSwapAndLiquifyEnabled(true);

    //     //to do: add more variables as needed
    //     setTaxFee(1); // in %
    //     setLiquidityFee(6); // in %, combines liquidity & bnb rewards
    //     setChariftyFee(1); // in %
    //     setBurnFee(2); // in %

    //     // set thresholdtoupdate
    //     setThresholdToUpdate(20); //in %

    //     // set _rewardThreshold
    //     setRewardThreshold(1 * 10**17); //0.1BNB

    //     // set _minTokenNumberToSell to 20B tokens
    //     // 1/50000th of the tokens at 6% fee = 1/30th total supply transfered
    //     setMinTokenNumberToSell(20 * 10**9 * 10**decimals());
