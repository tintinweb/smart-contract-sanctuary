...
library Bet {
  ...
  function New(address bettor, uint value, uint level) internal pure returns(bet memory b ) {
    
    (uint[3] memory vals, uint[3] memory durs) = betsForLevel(level);
    if (value >= vals[0]) {
      b.amount = vals[0];
      b.duration = durs[0];
  ...
  function betsForLevel(uint level) internal pure returns(uint[3] memory vals, uint[3] memory durs) {
    if (level <= 15) {
      (vals[0], vals[1], vals[2]) = (100 finney, 10 finney, 1 finney); 
      (durs[0], durs[1], durs[2]) = (3 minutes, 15 minutes, 30 minutes);
    } else if (16 <= level && level <= 30) {
      (vals[0], vals[1], vals[2]) = (300 finney, 30 finney, 3 finney); 
      (durs[0], durs[1], durs[2]) = (2 minutes, 10 minutes, 20 minutes);
    } else {
      (vals[0], vals[1], vals[2]) = (1000 finney, 100 finney, 10 finney); 
      (durs[0], durs[1], durs[2]) = (1 minutes, 5 minutes, 10 minutes);
    }
  }
...
contract LastHero is Accessibility {
  ...
  function durationForBetAmount(uint betAmount) public view returns(uint duration) {
    Bet.bet memory bet = Bet.New(msg.sender, betAmount, level);
    duration = bet.duration;
  }

  function avaliableBets() public view returns(uint[3] memory vals, uint[3] memory durs) {
    (vals,durs) = Bet.betsForLevel(level);
  }
  ...
  function doBet() public payable notFromContract {
    ...
    Bet.bet memory bet = Bet.New(msg.sender, msg.value, level);
