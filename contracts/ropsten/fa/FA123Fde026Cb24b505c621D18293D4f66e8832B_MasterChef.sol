...
contract MasterChef is Ownable, ReentrancyGuard, IMasterChef, Constants {
    ...
    struct UserInfo {
        ...
        uint256 stakedSince;            // Weighted block.number since last stake.  
    ...
    struct PoolInfo {
        ...
        uint256 totalTimeAlloc;      // Total amount of time allocation points.
    ...
    function pendingRewards(address _user) external view returns (uint256 _lotl, uint256 _busd) {
        ...
        if(user.rewardPoolShare > 0){
            uint256 busdPending = rewardInfo.amountBUSD * user.rewardPoolShare / 1e12;
            uint256 lotlPending = rewardInfo.amountLOTL * user.rewardPoolShare / 1e12;
    ...
    function deposit(uint8 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.stakedSince == 0 && _amount > 0)
        {
            ...
            pool.totalTimeAlloc = pool.totalTimeAlloc + user.stakedSince;
        }
    ...
    */
    ...
    function calculateRewardPool() external override {
        ...
        for (uint8 i=0; i < length; i++){
            ...
            uint256 poolTimeReward = (pool.totalTimeAlloc - block.number * pool.totalUserStaked);
            // with half a day modifier
            //uint256 poolTimeReward = (pool.totalTimeAlloc - block.number * pool.totalUserStaked) / 14400;
            for(uint32 j; j< userLength; j++){
                ...
                if(user.stakedSince > 0){
                    ...
                    rewardUser.rewardPoolShare = rewardUser.rewardPoolShare + user.amount * 1e12 / totalAllocPoint * pool.allocPoint / lpSupply * timeReward / poolTimeReward;
    ...
    function currentHoldingFactor(uint8 _pid, address _user) public view returns(uint256 holdFactor){
        ...
        if(user.stakedSince > 0)
        {
            ...
            //return (block.number - user.stakedSince) / 14400;
        }
        else return 0;
