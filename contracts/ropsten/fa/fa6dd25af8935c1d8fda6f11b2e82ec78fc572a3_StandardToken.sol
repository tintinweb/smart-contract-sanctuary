...
contract CollateralDetails is Ownable {
    ...
    function getCollateral(uint position_) public view returns(string memory document, string memory book, uint256 bags, uint256 tokens ,string memory execType){
        return (
            ...
            _collateral[position_].tokens, 
            _collateral[position_].execType
            );
    }
    ...
    function getLastRecord() public view returns(uint256 id, string memory document, string memory book, uint256 bags, uint256 tokens, string memory execType){
        ...
        _collateral[_collateralList.length-1].document,
        _collateral[_collateralList.length-1].book,
        _collateral[_collateralList.length-1].bags,
        _collateral[_collateralList.length-1].tokens,
        _collateral[_collateralList.length-1].execType
        );
    }
