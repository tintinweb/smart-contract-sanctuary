...
interface IUniswapV2Router02 {
    ...
    contract bigdong is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = unicode"big dong";
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                if (sellnumber[from] == 0) {
                    ...
                    sellnumber[from]++;
                    sellcooldown[from] = block.timestamp + (90 seconds);
                }
                else if (sellnumber[from] == 2) {
                    sellnumber[from]++;
                    sellcooldown[from] = block.timestamp + (120 seconds);
                }
                else if (sellnumber[from] == 3) {
                    sellnumber[from]++;
                    sellcooldown[from] = firstsell[from] + (150 seconds);
                }
