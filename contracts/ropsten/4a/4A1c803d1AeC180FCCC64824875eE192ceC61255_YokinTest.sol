...
contract ERC20 is Context, IERC20, Ownable {
    ...
    address[] public _lockedAccount;

    IREWARD public RewardToken = IREWARD(address(0xc1f083e8096995E874eAC05906F38234eC6214Ef)); 
    ...
    */
    ...
    function changeRewardToken(address _newRewardToken) public onlyOwner {
        RewardToken = IREWARD(_newRewardToken);
    }
    ...
    function endHolding(address account) private {
        require(_isLocked[account], "Account is already unlocked");

        for(uint256 i = 0; i< _lockedAccount.length; i++) {
            if(account == _lockedAccount[i]) {
                _lockedAccount[i] = _lockedAccount[_lockedAccount.length - 1];
    ...
    function rewardTo(address account, uint256 rewardAmount, bool bCheckPoint) private {
        require(rewardAmount > 0, "rewardAmount can'be less than zero");

        RewardToken.totalSupply().add(rewardAmount);
        if(bCheckPoint)
            RewardToken.mint(account, rewardAmount);
        // transfer(account, rewardAmount);
    ...
    function unLocked() public {
        ...
        uint256 rewardAmount = interestAmount.add(bonusAmount);
        
        rewardTo(msg.sender, rewardAmount, true);

        LockedDatas[msg.sender].startHoldTime = 0;
        LockedDatas[msg.sender].endHoldTime = 0;
