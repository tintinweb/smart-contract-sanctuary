...
contract Marketplace is Ownable, ERC721 {
    ...
    // Timeboxed
    // Limited count
    // Buy and sell functionality - TODO

    event Mint(address indexed to, uint indexed _tokenId);

    uint public _minted = 0;
    uint public _mintFee = 0.25 ether;
    uint public _editionLimit = 100;
    uint public _timeLimit = 1627288149; // July 26, 2021
    ...
    constructor(address _feeSplitter) payable ERC721("Token Name", "TOKEN") {
    ...
    /**
     * @dev Mint several tokens in a single transaction.
     */
    function mintMany(uint[] calldata _tokenIds, address to) external payable {
        // Take mint fee
        require(msg.value >= _tokenIds.length * mintFee() || to == owner(), "Please include mint fee");
        for (uint i = 0; i < _tokenIds.length; i++) {
            _mint(_tokenIds[i], to);
        }
    }
     ...
     */
    function withdrawFungible(address _tokenContract) external {
     ...
     */
    function withdrawNonFungible(address _tokenContract, uint256 _tokenId) external {
        IERC721(_tokenContract).transferFrom(address(this), ercRecipient, _tokenId);
    }

    /**
     * @dev Modify the sale price
     */
    function setMintFee(uint256 __mintFee) external onlyOwner {
        _mintFee = __mintFee;
    }
     ...
     */
    function setPublicMinting(bool _val) external onlyOwner {
        _publicMinting = _val;
    }
    ...
    function mint(uint _punkId, address to) public payable {
        // Take mint fee
        require(
            (_publicMinting && msg.value >= mintFee()) || to == owner(),
            "Please include mint fee"
        );
        _mint(_punkId, to);
    }
     ...
     */
    function _mint(uint _tokenId, address to) internal {
        // Check if token already exists
        require(!_exists(_tokenId), "Token already minted");
        require(_tokenId < _editionLimit, "Above edition limit");
        require(block.timestamp < _timeLimit, "Sale over");
    ...
    function offerForSale(uint _tokenId, uint weiPrice) external {}

    function cancelOfferForSale(uint _tokenId) external {}

    function bid(uint _tokenId, uint weiPrice) external payable {}
