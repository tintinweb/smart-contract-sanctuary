...
contract PROGE is Context, IERC20, Ownable {
    ...
    uint256 private _contractReserves = 9;
    ...
    constructor(address payable _presa, address payable _rogeTreasury) {
        ...
        _isExcludedFromFee[address(rogeTreasury)] = true;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
     ...
     function setSwapAndLiquify(bool enable) public onlyOwner() {
        swapEnabled = enable;
    }   
    ...
    function manualBuyBackRoge() public onlyOwner() {
        ...
        distributeAndROGE(contractETHBalance);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) { //exlude buy sell code from owner
        
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                require(tradeAllowed, "not live");
                uint contractETHBalance = address(this).balance;
                if (contractETHBalance > 0) {
                  distributeAndROGE(address(this).balance);
                }
            }
            uint contractTokenBalance = balanceOf(address(this));
    ...
    function removeAllFee() private {
        if (_reflection == 0 && _contractReserves == 0 && _progeBurn == 0) return;
        _reflection = 0;
        _contractReserves = 0;
        _progeBurn = 0;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = _burnProge(amount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tContract) = _getValues(tAmount, tBurn);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _reserveForContract(tContract);
        _reflectFee(rFee, tFee);
        emit Transfer(sender, recipient, tTransferAmount);
    }

    function _reserveForContract(uint256 tContract) private {
        uint256 currentRate = _getRate();
        uint256 rContract = tContract.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rContract);
    }

    function _burnProge(uint amount) private returns (uint, uint) {  
        ...
        _rOwned[address(animalSanctuary)] = _rOwned[address(animalSanctuary)].add(rBurn);
        _tOwned[address(animalSanctuary)] = _tOwned[address(animalSanctuary)].add(tBurn);
    ...
    function _getValues(uint256 tAmount, uint256 tBurn) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tContract) = _getTValues(tAmount, _reflection, _contractReserves, tBurn);
        uint256 currentRate = _getRate();
    ...
    function _getTValues(uint256 tAmount, uint256 taxFee, uint256 teamFee, uint256 tBurn) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 tContract = tAmount.mul(teamFee).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tContract).sub(tBurn);
    ...
    function distributeAndROGE(uint contractTokenBalance) private lockTheSwap {
        uint presaEth = contractTokenBalance.div(8);
        uint rogeTreasuryEth = contractTokenBalance.div(4);
        uint ethAmount = contractTokenBalance.sub(presaEth).sub(rogeTreasuryEth);
        presa.transfer(presaEth);
        rogeTreasury.transfer(rogeTreasuryEth);
        //swapETHforRoge(ethAmount);
    }
