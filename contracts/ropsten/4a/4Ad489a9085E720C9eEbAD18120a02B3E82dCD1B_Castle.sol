...
contract Castle is Ownable, IERC721Receiver {
    ...
    event WarriorClaimed(uint16 tokenId, uint earned, bool unstaked);
    event VikingClaimed(uint16 tokenId, uint earned, bool unstaked);

    ISilverHunter public silverHunter;
    ISilver public silver;

    mapping(uint256 => uint256) public warriorIndices;
    mapping(address => Stake[]) public warriorStake;
    ...
    // Total staked tokens
    uint public totalWarriorStaked;
    ...
    function getAccountWarriors(address user) external view returns (Stake[] memory) {
        return warriorStake[user];
    }
    ...
    function addTokensToStake(address account, uint16[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (silverHunter.isViking(tokenIds[i])) {
                ...
                _stakeWarriors(account, tokenIds[i]);
            }
        }
    }

    function _stakeWarriors(address account, uint16 tokenId) internal whenNotPaused _updateEarnings {
        totalWarriorStaked += 1;

        warriorIndices[tokenId] = warriorStake[account].length;
        warriorStake[account].push(Stake({
            owner: account,
    ...
    function _stakeVikings(address account, uint16 tokenId) internal {
        ...
        vikingStake[account].push(Stake({
            owner: account,
            tokenId: uint16(tokenId),
            value: uint80(vikingReward)
            }));

        emit TokenStaked(account, tokenId, vikingReward);
    }
    ...
    function claimFromStake(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (!silverHunter.isViking(tokenIds[i])) {
                owed += _claimFromWarrior(tokenIds[i], unstake);
            } else {
    ...
    function _claimFromWarrior(uint16 tokenId, bool unstake) internal returns (uint owed) {
        Stake memory stake = warriorStake[msg.sender][warriorIndices[tokenId]];
        ...
        if (unstake) {
            ...
            Stake memory lastStake = warriorStake[msg.sender][warriorStake[msg.sender].length - 1];
            warriorStake[msg.sender][warriorIndices[tokenId]] = lastStake;
            warriorIndices[lastStake.tokenId] = warriorIndices[tokenId];
            warriorStake[msg.sender].pop();
            delete warriorIndices[tokenId];
            ...
            warriorStake[msg.sender][warriorIndices[tokenId]] = Stake({
                owner: msg.sender,
                tokenId: uint16(tokenId),
                value: timestamp
            }); // reset stake
    ...
    function rescue(uint16[] calldata tokenIds) external {
        ...
        for (uint16 i = 0; i < tokenIds.length; i++) {
            ...
            if (!silverHunter.isViking(tokenId)) {
                stake = warriorStake[msg.sender][warriorIndices[tokenId]];
    ...
    function _payTax(uint _amount) internal {
        ...
        vikingReward += (_amount + unaccountedRewards) / totalVikingStaked;
    ...
    modifier _updateEarnings() {
        if (totalSilverEarned < MAXIMUM_GLOBAL_SILVER) {
            totalSilverEarned += ((block.timestamp - lastClaimTimestamp) * totalWarriorStaked * DAILY_SILVER_RATE) / 1 days;
