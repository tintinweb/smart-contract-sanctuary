...
/// @notice Facilities for registering new watches, transferring ownership,
///         updating fees, and changing watch settings.
/// @dev Watch ownership is implemented through {ERC721}.
/// @custom:security-contact [email protected]
contract CypherWatch is ERC721, ERC721Burnable, ERC721Enumerable {
    /// @dev The symbols used to convert numbers to hex strings.
    bytes16 private constant _HEX_SYMBOLS = "0123456789abcdef";

    /// @notice The address authorized to register new watches and update fees.
    /// @dev This address is fixed to the creator of the contract.
    address payable public _manufacturer; // TODO: remove _
    ...
    uint256 public _changeFee; // TODO: remove _

    /// @dev An NFT, uniquely defined by an ERC-721 contract and a token ID.
    struct NFT {
        address _contract;
        uint256 _tokenID;
    }
    ...
    /// @notice Allows the {_manufacturer} to register new watches.
    /// @dev The watch ID should resolve when appended to the {_baseURI}.
    /// @param watchID The unique ID of the new watch.
    function register(uint256 watchID) public {
        require(
            msg.sender == _manufacturer,
            "Only the manufacturer can register new watches"
        );
        require(!_exists(watchID), "This watch is already registered");
    ...
    /// @param nftContract The address of the NFT's ERC-721 contract.
    ...
    function changeNFT(
        ...
        require(
            _nft[watchID]._contract != nftContract &&
                _nft[watchID]._tokenID != nftTokenID,
        ...
        _nft[watchID] = NFT(nftContract, nftTokenID);
    }

    /// @notice TODO
    /// @dev TODO
    function getNFT(uint256 watchID) public view returns (address, uint256) {
        NFT memory nft = _nft[watchID];
        return (nft._contract, nft._tokenID);
    }
    ...
    {
        require(_exists(watchID), "This watch is not registered");
