        ...
        if(vol > 0) {
            ...
            if(path.length == 0) {
                ...
                if(msg.value > 0 && path[0] == WETH) {
                    ...
                    IERC20(undOrCur).safeTransferFrom(sender, address(this), amount);
                    IERC20(undOrCur).approve(address(rounter), amount);
...
/*
    function swapRoute2(address[2] calldata und_cur, uint[2] memory floor_cap, int[2] calldata dCallPut, int[2] calldata maxes, bytes calldata data, address[][2] calldata paths) external payable nonReentrant returns (address[2] memory call_put, int[2] memory dUndCur) {
