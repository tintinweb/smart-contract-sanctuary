...
contract ERC20 is Context, IERC20, IERC20Metadata {
  ...
  function transfer(address recipient, uint256 amount) 
    ...
    if(excludedFromTax[msg.sender] == true) { 
     ...
     charityAmount = amount* CHARITY_TAX;
     charityAmount = charityAmount / oneHunnit;
      
     
     
 
      _transfer(_msgSender(), charity, charityAmount);
      uint recipientAmount = amount - charityAmount;
  ...
  function transferFrom(address sender, address recipient, uint256 amount) 
    ...
    if(excludedFromTax[sender] == true) { 
        ...
        uint256 currentAllowance = balanceOf(sender);
  ...
  function updateCharity(address _charity) external {
   ...
   function updateCharityTax(uint256 _charityTax) external {
    require(msg.sender == admin, 'only admin');
    CHARITY_TAX = _charityTax;
  }
...
/** contract Token is ERC20 {
  ...
  constructor()  ERC20('Whaler', 'Whale') {
    _mint(msg.sender, 1000000000 * 10 ** 18); 
    admin = msg.sender;
    excludedFromTax[msg.sender] = true;
  }
