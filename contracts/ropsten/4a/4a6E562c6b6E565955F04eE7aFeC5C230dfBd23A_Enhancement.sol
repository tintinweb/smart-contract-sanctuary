...
VRFConsumerBase
{
    ...
    mapping(uint256 => uint256) public environmentProbability;

    // Shine Probability
    mapping(uint256 => uint256) public shineProbability;

    // Efficiency Probability
    mapping(uint256 => uint256) public efficiencyProbability;
...
contract Enhancement is IEnhancement, Ownable, VRFConsumerBase {
    EthereumClockToken public ethClockToken;
    // Enhancement lock time
    uint256 enhanceLockTime = 30 days;

    mapping(uint256 => bool) pendingGodTierList;

    mapping(uint256 => bool) pendingEnhancedList;

    mapping(uint256 => bool) pendingFailedList;

    mapping(uint256 => bool) enhanceRequested;

    mapping(uint256 => uint256) enhanceRequestTime;
    ...
    function calcProbability() internal {
        uint256 maxTokenLevel = ethClockToken.maxTokenLevel();
    ...
    function enhanceRequest(uint256 tokenId) external {
        ...
        require(
            randomResultRequested == false,
            "Raffle: cannot collect whitelist twice"
        );
        ...
        require(isAvailableEnhance(tokenId), "Not avaiable to enhance");
        //Toggle randomness requested
        randomResultRequested = true;
        // Call for random number
        requestIdToToken[requestRandomness(keyHash, fee)] = tokenId;
    }
    ...
    override
    {
        ...
        require(tokenId == 0, "fulfillRandomness: requestId does not exist");
        // Store random number as randomResult
        randomResult = randomness;
    ...
    function enhanceProcess(uint256 tokenId) internal returns (bool) {
        ...
        uint256 percentValue = randomResult % 10000;
    ...
    function enhance(uint256 tokenId, string memory _tokenURI) external {
        ...
        require(
            ethClockToken.enhance(tokenId, _tokenURI),
            "Failed: enhancement"
        );
        pendingEnhancedList[tokenId] = false;
    }

    function godTier(uint256 tokenId, string memory _tokenURI) external {
        ...
        require(ethClockToken.godTier(tokenId, _tokenURI), "Failed: God-Tier");
        pendingGodTierList[tokenId] = false;
    }

    function fail(uint256 tokenId, string memory _tokenURI) external {
        ...
        require(ethClockToken.fail(tokenId, _tokenURI));
