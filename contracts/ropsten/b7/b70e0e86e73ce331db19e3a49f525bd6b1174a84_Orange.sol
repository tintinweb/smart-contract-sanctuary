...
pragma solidity ^0.6.12;
    ...
    contract StakingDividends is Ownable {
        ...
        function stake(uint256 amount) public {
            ...
            stakedBalance[user] = stakedBalance[user].add(amount);
            totalStakedAmount = totalStakedAmount.add(amount);
        }
        
        function unstake(uint256 amount) public {
            address user = _msgSender();
            require(stakedBalance[user] >= amount, "amount is invalid");
            uint256 getAmountsOut = amount.mul(IOrange(owner()).totalStakeReward()).div(totalStakedAmount);
            token.transfer(user,amount.add(getAmountsOut));
            stakedBalance[user] = stakedBalance[user].sub(amount);
    ...
    contract Orange is Context, IERC20, Ownable {
        ...
        uint256 public _liquidityPoolFee = 3;
        ...
        StakingDividends public stakingDividends;
        ...
        constructor (address payable charityWalletAddress) public {
            ...
            stakingDividends = new StakingDividends();
            ...
            _isExcludedFromFee[address(stakingDividends)] = true;
        ...
        function removeAllFee() private {
            if(_taxFee == 0 && _charityFee == 0 && _liquidityPoolFee == 0) return;
        ...
        function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
            ...
            uint256 lpPoolAmt = amount.mul(_liquidityPoolFee).div(100);

            if (_isExcluded[sender] && !_isExcluded[recipient]) {
                _transferFromExcluded(sender, recipient, amount.sub(lpPoolAmt));
            } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
                _transferToExcluded(sender, recipient, amount.sub(lpPoolAmt));
            } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
                _transferStandard(sender, recipient, amount.sub(lpPoolAmt));
            } else if (_isExcluded[sender] && _isExcluded[recipient]) {
                _transferBothExcluded(sender, recipient, amount.sub(lpPoolAmt));
            } else {
                _transferStandard(sender, recipient, amount.sub(lpPoolAmt));
            }
            ...
            _transferStandard(sender, _liquidityPool, lpPoolAmt);
        ...
        function _transferStandard(address sender, address recipient, uint256 tAmount) private {
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tCharity,uint256 tStake) = _getValues(tAmount);
            _rOwned[sender] = _rOwned[sender].sub(rAmount);
        ...
        function _takeStake(uint256 tStake) private {
            ...
            totalStakeReward = totalStakeReward.add(tStake);
            _rOwned[address(stakingDividends)] = _rOwned[address(stakingDividends)].add(rStake);
            if(_isExcluded[address(stakingDividends)])
                _tOwned[address(stakingDividends)] = _tOwned[address(stakingDividends)].add(tStake);
                
            emit Transfer(address(this),address(stakingDividends),tStake);
        } 
        ...
        function _takeCharity(uint256 tCharity) private {
            ...
            emit Transfer(address(this),_charityWalletAddress,tCharity);
        }
        ...
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256,uint256) {
            (uint256 tTransferAmount, uint256 tFee, uint256 tCharity,uint256 tStake) = _getTValues(tAmount, _taxFee, _charityFee,_stakingFee);
            uint256 currentRate =  _getRate();
            {
                uint256 _tAmount = tAmount;
                (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(_tAmount, tFee,tStake, currentRate);
                return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tCharity,tStake);
            }
            
        }

        function _getTValues(uint256 tAmount, uint256 taxFee, uint256 charityFee,uint256 stakingFee) private pure returns (uint256, uint256, uint256,uint256) {
            uint256 tFee = tAmount.mul(taxFee).div(100);
            uint256 tCharity = tAmount.mul(charityFee).div(100);
            uint256 tStake = tAmount.mul(stakingFee).div(100);
            uint256 tTransferAmount = tAmount.sub(tFee.add(tCharity).add(tStake));
            return (tTransferAmount, tFee, tCharity,tStake);
        }

        function _getRValues(uint256 tAmount, uint256 tFee, uint256 tStake,uint256 currentRate) private pure returns (uint256, uint256, uint256) {
            uint256 rAmount = tAmount.mul(currentRate);
            uint256 rFee = tFee.mul(currentRate);
            uint256 rStake = tStake.mul(currentRate);
            uint256 rTransferAmount = rAmount.sub(rFee.add(rStake));
