...
contract oncmon is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Strings for uint256;
...
function usew(uint8[] memory w,uint256 i) internal pure returns (uint8) {
  ...
  function claim() public nonReentrant {
    require(numClaimed >= 0 && numClaimed < 90, "invalid claim");
    _safeMint(_msgSender(), numClaimed + 1);
    numClaimed += 1;
  }
    
  function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
    require(tokenId > 0 && tokenId < 100, "invalid claim");
    _safeMint(owner(), tokenId);
  }
    
  constructor() ERC721("oncmon", "OC") Ownable() {}
}