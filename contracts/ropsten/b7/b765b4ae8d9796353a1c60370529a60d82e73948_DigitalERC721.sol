...
contract DigitalUser721Token is NFTMint {
    event CreateERC721(address indexed creator, string name, string symbol);
    constructor(
        string memory name,
        string memory symbol
    ) NFTMint(name, symbol) public{
        emit CreateERC721(msg.sender, name, symbol);     
    } 
