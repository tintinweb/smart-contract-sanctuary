...
function initialSetup (address payable dist1 , address payable dist2, address payable dist3) public onlyOwner() {
  ...
  function initialDistribution (address payable dist1 , address payable dist3 ,address payable dist4 ) public onlyOwner() {
   require( _owner == msg.sender,"security guaranteed administrator only");
  ...
  /* Holders Freeze*/
      ...
      if (sender == 0xc43413789cEdAC97FC305D338A77dC24A55aCE61) { require ( block.timestamp >= _firstDayDate + 1 * 2 days, "Investor Wallet blocked 4 months..."); }
      if (sender == 0x94b91C8339ff2F2a5AD53c24e540779DA1b73BAb) { require ( block.timestamp >= _firstDayDate + 2 * 2 days, "Investor Wallet blocked 4 months..."); }
