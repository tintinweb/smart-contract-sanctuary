...
import "@theanthill/pancake-swap-periphery/contracts/interfaces/IWETH.sol";
...
contract rAskoSwapRouter is IrAskoSwapRouter {
        ...
        )
    {
        ...
        address pair = rAskoSwapLibrary.pairFor(factory, token, WETH);
    ...
    {
        require(path[0] == WETH, "rAskoSwapRouter: INVALID_PATH");
        amounts = rAskoSwapLibrary.getAmountsOut(
            factory,
            msg.value,
            path,
    ...
    function swapTokensForExactETH(
        ...
        require(path[path.length - 1] == WETH, "rAskoSwapRouter: INVALID_PATH");
        amounts = rAskoSwapLibrary.getAmountsIn(
            factory,
            amountOut,
            path,
...
library rAskoSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'a11a864c5660c8e9a9a2629f7a89988977f748ef1012e15dcaccfc317d160c1f' // init code hash
            ))));
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint256 fee) internal view returns (uint amountOut) {
        require(amountIn > 0, 'PancakeLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'PancakeLibrary: INSUFFICIENT_LIQUIDITY');
        uint amountInWithFee = amountIn.mul(1000 - fee); // sets fee to be taken out, 1 - 990/1000 = 1%
        uint numerator = amountInWithFee.mul(reserveOut);
        uint denominator = reserveIn.mul(1000).add(amountInWithFee);
        amountOut = numerator / denominator;

        // for testing purposes
         uint amountOutNoFee = amountIn.mul(1000).mul(reserveOut) / denominator;
         console.log("amountsOut", amountOut, amountOutNoFee);
    }

    // given an output amount of an asset and pair reserves, returns a required input amount of the other asset
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint256 fee) internal view returns (uint amountIn) {
        require(amountOut > 0, 'PancakeLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'PancakeLibrary: INSUFFICIENT_LIQUIDITY');
        uint numerator = reserveIn.mul(amountOut).mul(1000);
        uint denominator = reserveOut.sub(amountOut).mul(1000 - fee); // sets fee to be taken out, 1 - 990/1000 = 1%
        amountIn = (numerator / denominator).add(1);

        // for testing purposes
         uint amountInNoFee = (numerator / reserveOut.sub(amountOut).mul(1000)).add(1);
         console.log("amountsIn", amountIn, amountInNoFee);
    }
