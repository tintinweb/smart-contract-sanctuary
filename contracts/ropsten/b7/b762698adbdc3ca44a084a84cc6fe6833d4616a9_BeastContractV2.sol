...
contract BeastContractV2 is Context, IERC20, Ownable {
    ...
    uint256 private _maxTxAmount = 10000000000 * 10**8;

    // We will set a minimum amount of tokens to be swaped => 5M
    uint256 private _numOfTokensToExchangeForBeast = 5 * 10**6;
    ...
    function numOfTokens() public view returns (uint256) {
        return _numOfTokensToExchangeForBeast;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (sender != owner() && recipient != owner() || !_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]) require(
            amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount."
        );
        ...
        if (!inSwap && overMinTokenBalance && sender != uniswapV2Pair && recipient != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D)) {
    ...
    function setNumOfTokens(uint256 amount) external onlyOwner(){
        _numOfTokensToExchangeForBeast = amount;
    }
