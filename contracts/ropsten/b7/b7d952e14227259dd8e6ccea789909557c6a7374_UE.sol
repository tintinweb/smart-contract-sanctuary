...
contract UE is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("JAJA", "LEROS")  {
        setBaseURI(baseURI);
    }
    ...
    function mint(address _to, uint _count) public payable saleIsOpen {
        ...
        require(_count <= 20, "UntamedElephants: Exceeds the max you can mint");
        require(msg.value >= price(_count), "UntamedElephants: Value below price");
        require((timeLimit[msg.sender]+600) <= block.timestamp, "UntamedElephants: Buy cooldown, 10 mins between mints");
    ...
    function price(uint _count) public pure returns (uint256) {
        return 100000000000000 * _count; // 0.01 ETH for now, soon to decide
    }
