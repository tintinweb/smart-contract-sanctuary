...
contract Ownable {
  ...
  /**
   * OwnableFunc
   * @dev OwnableFunc constructor sets the `owner` of the contract to sender
   */
  function OwnableFunc() public {
    owner = msg.sender;
  }
...
/**
 * @title Token
 * @dev API interface for interacting with the LUCK Token contract 
 */
interface Token {
...
/**
 * @title LuckTokenICO
 * @dev LuckTokenICO contract is Ownable
 **/
contract LuckICO is Ownable {
  using SafeMath for uint256;
  Token token;

  uint256 public constant RATE = 2375; // Number of tokens per Ether
  uint256 public constant CAP = 5350; // Cap in Ether
  uint256 public constant START = 1617447178; // Mar 26, 2018 @ 12:00 EST
  uint256 public constant DAYS = 45; // 45 Day
  
  uint256 public constant initialTokens = 6000000; // Initial number of tokens available
  ...
  /**
   * LuckTokenICO
   * @dev LuckTokenICO constructor
   **/
  function LuckTokenICO(address _tokenAddr) public {
