...
contract VaultConfig is IVaultConfig, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    function indicativeSupplyRate() external view override returns (uint256) {
        ...
        uint256 optimalRate;
        uint256 adjustRebasinfSupply = IRhoToken(rhoToken).adjustedRebasingSupply();
        ...
        if (adjustRebasinfSupply == 0) {
            return type(uint256).max;
        }
        return (optimalRate * (1e18 - reserveBoundary[0]) * totalSupply) / 1e18 / adjustRebasinfSupply;
    }
