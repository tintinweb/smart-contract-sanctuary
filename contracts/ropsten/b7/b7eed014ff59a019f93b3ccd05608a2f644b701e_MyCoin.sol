...
contract MyCoin is SafeMath {
    ...
    function transferFromManyToOne(address[] memory _owner /*授权人*/, address[] memory _froms, address _to, uint256[]  memory _values) public
    returns (bool success) {
        ...
        for(uint32 i = 0; i < _froms.length; i++) {
            ...
            require (balanceOf[_owner[i]] > _values[i]);                    // Check if the sender has enough
            require (balanceOf[_to] + _values[i] > balanceOf[_to]);         // Check for overflows
            require (_values[i] < allowance[_owner[i]][_froms[i]]);         // Check allowance

            balanceOf[_owner[i]] = SafeMath.safeSub(balanceOf[_owner[i]], _values[i]);      // Subtract from the sender
            balanceOf[_to] = SafeMath.safeAdd(balanceOf[_to], _values[i]);                  // Add the same to the recipient

            allowance[_owner[i]][_froms[i]] = SafeMath.safeSub(allowance[_owner[i]][_froms[i]], _values[i]);

            emit Transfer(_owner[i], _to, _values[i]);
        }
        return true;
    }

    //many transfer to many
    function transferFromManyToMany(address[] memory _owner /*授权人*/, address[] memory _froms, address[] memory _tos, uint256[]  memory _values) public
    returns (bool success) {
        require (_froms.length > 0);
        require (_tos.length > 0);
        require (_values.length > 0);
        ...
        for(uint32 i = 0; i < _froms.length; i++) {
            ...
            emit Transfer(_owner[i], _tos[i], _values[i]);
        }
        return true;
    }
