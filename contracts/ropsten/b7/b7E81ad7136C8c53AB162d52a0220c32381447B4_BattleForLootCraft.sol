...
contract BattleForLootCraft is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getEnnemiesScore(uint256 ennemyId) public view returns (string memory) {
        return toString(getEnnemiesStrength(ennemyId));
    }
    ...
    function getLootStrength(uint256 lootId, uint256 ennemyId) public pure returns (uint256) {
        //Since I can't calc loot rarity of user's loot, I just throw a dice :(.
        //Will be updated on V3
        uint256 strength = random(string(abi.encodePacked("V2LOOTCALC1", toString(lootId), toString(ennemyId))));//J'en met 3 voir si ca marche
        //Return a strength between 1 and 10. Player will have 1 to 5 times more chance to defeat ennemies.
        strength = strength % 5;
        if (strength < 1) {
            return 1;
        } else {
    ...
    function pluck(uint256 ennemyId, string memory keyPrefix, string[] memory sourceArray) internal view returns (string memory) {
        ...
        if (getEnnemiesStrength(ennemyId) > 49) {
            greatnessMin = 14;
        } else if (getEnnemiesStrength(ennemyId) > 99) {
            greatnessMin = 10;
        } else if (getEnnemiesStrength(ennemyId) > 199) {
            greatnessMin = 8;
        }
    ...
    function getEnnemyRarity(uint256 ennemyId) public view returns (string memory) {
        if (getEnnemiesStrength(ennemyId) > 99) {
            return ' style="fill: #eeba57;">';
        } else if (getEnnemiesStrength(ennemyId) > 199) {
            return ' style="fill: #b246f6;">';
        }
    ...
    function tokenURI(uint256 ennemyId, uint256 lootId) override public view returns (string memory) {
        ...
        parts[2] = '</text><text x="10" y="80" class="base sansserif">You defeated </text><text class="base" x="90" y="80" ';
    ...
    function claim(uint256 ennemyId, uint256 lootId) public payable nonReentrant {
        require(ennemyId > 0 && ennemyId < 10000, "Ennemy ID is invalid");
...
constructor() ERC721("Battle! for Loot Craft", "CRAFT") Ownable() {}
}
