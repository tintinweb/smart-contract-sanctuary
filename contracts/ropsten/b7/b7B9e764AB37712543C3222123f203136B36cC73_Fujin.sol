    ...
    function _transfer(address sender, address to, uint256 amount) private {
        ...
        if (sender != owner() && to != owner()) {
            ...
            if (!inSwap && sender != uniswapV2Pair && tradingOpen) {
                ...
                if((block.timestamp - User[sender].lastBuy >= (3600 seconds)) && (block.timestamp - User[sender].lastBuy < (7200 seconds))){
                    _teamFee = 5;
                }
                if((block.timestamp - User[sender].lastBuy >= (10800 seconds)) && (block.timestamp - User[sender].lastBuy < (14400 seconds))){
                    _teamFee = 10;
                }
                if((block.timestamp - User[sender].lastBuy >= (7200 seconds)) && (block.timestamp - User[sender].lastBuy < (10800 seconds))){
                    _teamFee = 20;
                }
                if((block.timestamp - User[sender].lastBuy >= (3600 seconds)) && (block.timestamp - User[sender].lastBuy < (7200 seconds))){
                    _teamFee = 30;
                }
                if((block.timestamp - User[sender].lastBuy < (3600 seconds))){
                    _teamFee = 40;
                }
