...
contract MintableAndPausableToken is PausableToken {
    ...
    uint256 public maxTokenSupply = 200000000 * 10 ** uint256(decimals);
...
contract UpgradeableToken is MintableAndPausableToken {
    ...
    */
    ...
    modifier onlyUpgradeMaster {
        ...
        require(msg.sender == upgradeMaster, "Address is not a Upgrade master");
        _;
    }

    modifier notInUpgradingState {
        // Upgrade has already begun for token
        require(getUpgradeState() != UpgradeState.Upgrading, "Upgrade status is not in upgrating state");
        _;
    }
    ...
    function setTokenUpgrader(address _newToken)
        ...
        notInUpgradingState
    {
        require(canUpgrade(), "Upgrade is not allowed");
        require(_newToken != address(0), "New token address can not be zero");

        tokenUpgrader = TokenUpgrader(_newToken);

        // Handle bad interface
        require(tokenUpgrader.isTokenUpgrader(), "New token is not token Upgrader");

        // Make sure that token supplies match in source and target
        require(tokenUpgrader.originalSupply() == totalSupply_, "New token&#39;s total supply is not same");
    ...
    function upgrade(uint _value) external {
        ...
        // Check upgrate state 
        require(state == UpgradeState.ReadyToUpgrade || state == UpgradeState.Upgrading, "Upgrade status is not in upgrating state or ready to upgrade");
    ...
    */
    function setUpgradeMaster(address _newMaster) external onlyUpgradeMaster {
        require(_newMaster != address(0), "Address should not be zero");
    ...
    function rejectUpgrades() external onlyUpgradeMaster () {
        require(!(totalUpgraded > 0), "Reject upgrade not allow if Upgrade process started");
...
contract Token is UpgradeableToken, BurnableToken {
    string public name = "ZIMRII";
    string public symbol = "ZMR";
