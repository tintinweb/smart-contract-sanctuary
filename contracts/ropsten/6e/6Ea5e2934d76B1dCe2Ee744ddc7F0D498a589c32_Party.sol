...
contract Party {
    ...
    IERC20 token = IERC20(0xfD1e0d4662Ef8Ab1Ff675Da33b8be58A39436261);
     ...
     */
     // TODO: Ask about calculateWeight when using totalfund from BE
    function calculateWeight(uint256 newBalanceParty, address sender, uint256 typeTransaction)
        internal isAlive
    {
        uint256 _temp = 0;
        for (uint256 index = 0; index < memberList.length; index++) {
            if (weight[memberList[index]] == 0){
                weight[memberList[index]] = ((newBalanceParty - getPartyBalance()) * 10 ** 6) / newBalanceParty;
                _temp += weight[memberList[index]];
            } else {
                if (memberList[index] == sender) {
                    if (typeTransaction == 1) { //deposit
                        uint normalizeDeposit = (weight[memberList[index]] * getPartyBalance()) / 10 ** 6;
                        uint newValue = normalizeDeposit + (newBalanceParty - getPartyBalance());
                        weight[memberList[index]] = (newValue * 10 ** 6) / newBalanceParty ;
                        _temp += weight[memberList[index]];
                        emit Log(typeTransaction, newBalanceParty, normalizeDeposit, newValue, weight[memberList[index]], getPartyBalance());
                    }
                    if (typeTransaction == 2) { //withdraw
                        uint normalizeWithdraw = (weight[memberList[index]] * getPartyBalance()) / 10 ** 6;
                        uint differenceValue = normalizeWithdraw - (getPartyBalance() - newBalanceParty);
                        weight[memberList[index]] = (differenceValue * 10 ** 6) / newBalanceParty ;
                        _temp += weight[memberList[index]];
                        emit Log(typeTransaction, newBalanceParty, normalizeWithdraw, differenceValue, weight[memberList[index]], getPartyBalance());
                    }
                } else {
                    uint normalize = (weight[memberList[index]] * getPartyBalance());
     ...
     */
    function withdraw(address partyAddress, uint256 amount, uint256 n, uint256 newBalanceParty) external isAlive onlyMember {
        require(
        ...
        uint256 penalty;
        if (n != 0) {
            //(10000000 * (17 * 10 ** 8 + (2-1) * 4160674157) * 10 ** 6) / 10 ** 16
            // penalty = (amount * (17 * 10 ** 8 + (n-1) * 4160674157) * 10 ** 6) / 10 ** 18;
    ...
    function checkDifferenceInBalance() external isAlive view returns (int) {
    ...
    function getAllowance() external isAlive view returns (uint256) {
    ...
    function getMemberList() external isAlive view returns (address[] memory) {
        return memberList;
    }
    ...
    function approveERC20 (IERC20 erc20Token ,address _to, uint256 amount, Sig memory inputApproveRSV) external isAlive onlyOwner {
        require(verifySigner(PLATFORM_ADDRESS, messageHash(abi.encodePacked(_to, amount)), inputApproveRSV), "Approve Signature Failed");
        // TODO perlu validasi ammount dan address
        // agar owner tidak sembarangan isi value nya
        erc20Token.approve(_to, amount);
    }
    ...
    function closeParty() external onlyOwner isAlive {
        OWNER = address(0x0);
        for (uint256 index = 0; index < memberList.length; index++) {
            uint _temp = weight[memberList[index]] * balanceParty / 10 ** 6; 
