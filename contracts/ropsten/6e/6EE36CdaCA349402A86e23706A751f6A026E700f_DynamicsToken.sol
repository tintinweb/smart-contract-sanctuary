...
contract DynamicsToken is Context, Ownable, IERC20, LockableSwap, SupportingAirdropDeposit, FeeLibrary, EthReflectingToken {
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(buyBackEnabled && address(buybackContract).balance >= minBuyBack){
            ...
            shouldSwap = false;
            if((redist >= buyback || !buyBackEnabled) && (redist >= liq || !swapAndLiquifyEnabled) && ethSwapEnabled){
            ...
            } else if((buyback >= liq || !swapAndLiquifyEnabled) && buyBackEnabled){
                // Swap for buyback
                uint256 ethBought = 0;
    ...
    function updatebuybackContractAddress(address payable _buybackContract) public onlyOwner {
