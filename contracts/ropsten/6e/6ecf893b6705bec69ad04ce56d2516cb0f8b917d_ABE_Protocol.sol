...
//-----------------------------------------------------------------------
//ABE minting protocol algorithm
//authored, constructed, recoded and assembly by Azreh Ahargun
//Donate : 0xFbe39c66C15C2BCce55df782E7433e4F0Eca2071
//-----------------------------------------------------------------------
...
contract ABE_Protocol is ERC20, ERC20Detailed,
    ...
    address internal contractOwner = 0xFbe39c66C15C2BCce55df782E7433e4F0Eca2071;
    address internal teamDevs = 0xFbe39c66C15C2BCce55df782E7433e4F0Eca2071;
    
    //Vault wallet locked automatically until specified time 
    
    address internal vaultWallet = 0x52Db647768B9aA98E8F0fB765aC8e8f8f2993126;
    uint256 internal unlockDate = 1625850000;
    uint256 internal timelockStart;
    ...
    constructor() public ERC20Detailed("ABE_Protocol","ABE", 18){
        owner = msg.sender;
        
        maxTotalSupply = 1000000000 * (10**18);
        circulatingSupply = 9000000 * (10**18);
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint256 shareTax = value.mul(shareFee).div(1e4); {
            
            if(msg.sender == contractOwner) shareTax = 0;
            if(msg.sender == teamDevs) shareTax = 0;
            if(msg.sender == vaultWallet) shareTax = 0;
            if(excluded[msg.sender] == true) shareTax = 0;

        }
        
        uint256 valueAfterShare = value.sub(shareTax);
        ...
        emit ShareTransfer(msg.sender, address(0), shareTax);
        
        totalSupply = totalSupply.sub(shareTax);
    ...
    function transferToAddress(address sender, address recipient, uint256 value) internal {
        ...
        uint256 shareTax = value.mul(shareFee).div(1e4); {
            ...
            if(sender == contractOwner) shareTax = 0;
            if(sender == teamDevs) shareTax = 0;
            if(sender == vaultWallet) shareTax = 0;
            if(excluded[sender] == true) shareTax = 0;
            if(recipient == contractOwner) shareTax = 0;
            if(recipient == teamDevs) shareTax = 0;
            if(recipient == vaultWallet) shareTax = 0;
        ...
        balances[recipient] = balances[recipient].add(valueAfterShare);{
        ...
        emit Transfer(sender, recipient, valueAfterShare);
        emit ShareTransfer(sender, address(0), shareTax);
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal shareFee = 250; //default on chain share fee 2,5%
    
    event ChangeShareFee(uint256 value);
    event ShareTransfer(address indexed from, address indexed to, uint256 value);
    
    function changeShareFee(uint256 _shareFee) external onlyOwner{
        shareFee = _shareFee;
    ...
    function getBurnRate() internal returns (uint256){
        if(totalSupply < mintingTier.mul(1)) {
            shareFee = shareFee.mul(1); //On chain share fee = 2,5%
            ...
            shareFee = shareFee.mul(2); //On chain share fee = 5%
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public blockTransactionRewards;
    
    event ChangeBlockTransaction(uint256 value);
    ...
    function totalBlockMinted() internal view returns (uint256){
    ...
    function setGenesisMinting(uint256 _genesisMintingBlockNumber) external onlyOwner {
    ...
    function setBlockTransaction(uint256 _blockTransactionRewards) external onlyOwner {
        ...
        emit ChangeBlockTransaction(blockTransactionRewards);
    }
    
    function blockMintingInfo(address account) external view returns (address, uint256, uint256, uint256) {
        uint256 blocksCount = block.number.sub(genesisMintingBlockNumber);
        return(account, blocksCount, totalBlocksMinted, blockTransactionRewards);
    }
...
//--------------------------------------------------------------------------------------
    ...
    function() external payable {
        ...
        totalSupply = totalSupply.add(purchasedAmount).add(bonusAmount);
