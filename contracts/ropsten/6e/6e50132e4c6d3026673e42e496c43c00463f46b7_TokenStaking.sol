...
contract TokenStaking {
    ...
    mapping(address => uint256) private _userStakedAmount; // verranno inseriti solo gli importi inviati al contratto // da usare solo per visualizzazioni e non per funzionmento contratto
    ...
    /* ----- SEZIONE MANAGER ----- */
    /* Funzione MANAGER incrementa l'importo di rewards -da usare come prima opzione dopo aver creato il contratto - */
    ...
    function startRewards() external onlyOwner{ // onlyOwner
        ...
        _stopRewards = _startRewards + 1 days; //365
    }
    ...
    {
        if(_userLastUpdate[owner] != 0 || _userLastUpdate[owner] != 0)
        {
            uint256 time = block.timestamp;
