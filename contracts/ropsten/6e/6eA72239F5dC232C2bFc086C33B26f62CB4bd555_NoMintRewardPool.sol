...
contract NoMintRewardPool  is IRewardPool, Controllable {
    ...
    event Staked(uint256 indexed user, uint256 amount);
    event Withdrawn(uint256 indexed user, uint256 amount);
    event RewardPaid(uint256 indexed user, uint256 reward, address beneficiary);
    ...
    function stakeFor(uint256 holder, uint256 amount)public updateReward(holder) onlyManager{
    ...
    function withdrawOf(uint256 holder, uint256 amount) internal {
        require(amount > 0, "Cannot withdraw 0");
        require(_balances[holder] >= amount, "Amount greater than the balance");
    ...
    // todo： 增加调用约束条件
    function pushReward(uint256 recipient, uint256 amount, address beneficiary) public updateReward(recipient) onlyManager returns(uint256){
        ...
        if (reward > 0) {
            ...
            emit RewardPaid(recipient, reward, beneficiary);
        }
        return reward;
    }
