...
/**
 * @title PoSHMintableToken
 * @dev the interface of Proof-Of-Stake and Hold Mintable Token
 */
...
contract PoSHMintableToken {
    ...
    uint256 public currentEthBlockNumber;
    uint256 public lastEthBlockNumber;
    uint256 public newEthBlock;
    
    function mint() public returns (bool);
    function mintingBlocks() public returns (bool);
    
    event Mint(address indexed _address, uint _tokensMinting, uint _blocksMinting);
...
contract BlockMinter is ERC20, PoSHMintableToken, Ownable {
    using SafeMath for uint256;

    string public name = "BlockMinter";
    ...
    function BlockMinter () public {
        ...
        currentEthBlockNumber = block.number;
        lastEthBlockNumber; newEthBlock;
        tokensPerBlock;
...
//------------------------------------------------------------------------------
    ...
    function mint() PoSHMinter public returns (bool) {
        ...
        uint256 blocksMinting = getNewMintingBlock();
        ...
        emit Mint(msg.sender, tokensMinting, blocksMinting);
    ...
    function mintingBlocks() PoSHMinter public returns (bool) {
        uint256 blocksMinting = getNewMintingBlock();
        if(block.number == newEthBlock) return mint();
        emit BlockMinting(msg.sender, blocksMinting);
        return true;
    }
    
    function getNewMintingBlock() PoSHMinter public returns (uint){
        uint256 currentEthBlockNumber = block.number;
        lastEthBlockNumber = currentEthBlockNumber;
        newEthBlock = lastEthBlockNumber;
        return newEthBlock;
    }
    
    function getMintingReward() internal returns (uint) {
        require((now >= mintingStartTime) && (mintingStartTime > 0));
        uint256 currentEthBlockNumber = block.number; lastEthBlockNumber = currentEthBlockNumber;
        uint256 blockMinted = currentEthBlockNumber.sub(lastEthBlockNumber);
