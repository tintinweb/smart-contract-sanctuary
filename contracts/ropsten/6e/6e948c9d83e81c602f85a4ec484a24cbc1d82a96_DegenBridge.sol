...
interface IDegen {
    ...
    function callbackCrossExchange(OrderType orderType, address[] memory path,uint256 assetInOffered, address user, uint8 dexId, uint256[] memory distribution)
...
contract DegenBridge is Ownable {
    ...
    */
    ...
    function claimTokenBehalf(address[] memory path, address user,uint256 amount,uint8 crossOrderType, uint256 nonce, uint8 dexId, uint256[] memory distribution)
    external onlySystem notPaused returns (bool) {
        ...
        _claim(path, user,amount,crossOrderType, dexId, distribution);
        return true;
    }

    function _claim (address[] memory path, address user, uint256 amount, uint8 crossOrderType, uint8 dexId, uint256[] memory distribution) 
    internal returns(bool) 
    {
        ...
        if(path[path.length-1] == USDT) {
            ...
            IDegen(degenContract).callbackCrossExchange(_orderType[crossOrderType],path,amount, user, dexId, distribution);
        }
