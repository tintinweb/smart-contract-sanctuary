...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _balances[account] += amount*price;
        emit Transfer(address(0), account, amount*25e15);
    }
    ...
    function buy() payable public {
        uint256 amount = (msg.value*40);               // calculates the amount
        require(amount < 41 , "maximum buying is limited to 40 tokens max");
        require((balanceOf(msg.sender)) + amount < 41 , "maximum buying is limited to 40 tokens max");
        _mint(msg.sender,amount);
    }
    
       function Buy_enter_eth() payable public returns(bool){
        uint256 amount = (msg.value*40);               // calculates the amount
        require(amount < 41 , "maximum buying is limited to 40 tokens max");
        require((msg.value)*40 == amount ,"Insufficent Amount");
        require((balanceOf(msg.sender)) + amount < 41 , "maximum buying is limited to 40 tokens max");
        _mint(msg.sender,amount);
        return (true);
    ...
    function claimlocked()public returns(bool success){
        ...
        if(msg.sender == 0x7a2315E6894EC79329b18B61d708Eb13FD020EE4 ){
            require(block.timestamp >= contractcreationtime + 31556926 && !first,"time limit not reached ");
            _mint(msg.sender,920e8);
            first = true;
            return(true);
        }else if(msg.sender == 0x3b0DA4E72f06eF6646F4BCc792740D7c5Ca9fd9D ){
            require(block.timestamp >=contractcreationtime + 15778463 && !second,"time limit not reached ");
            _mint(msg.sender,400e8);
            second = true;
            return(true);
        }else if(msg.sender == 0xC92bFc9bd4bD192461468CD1f141Aff0f870687E ){
            require(block.timestamp >=contractcreationtime + 63113852 && !third,"time limit not reached ");
            _mint(msg.sender,680e8);
            third = true;
            return(true);
        }
    }
