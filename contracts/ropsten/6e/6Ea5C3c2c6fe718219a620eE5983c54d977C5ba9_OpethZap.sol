...
import {IOpeth} from "./interfaces/IOpeth.sol";
...
contract OpethZap {
    ...
    constructor(
        ...
        IERC20(_usdc).safeApprove(exchange, uint(-1));
    }
    ...
    function mintWithEth(
        ...
        (/* weth */, address oToken, /* _underlying == _opeth */) = opeth.getOpethDetails(_oToken);
        WETH9(address(weth)).deposit{value: _opeth}();
        weth.safeApprove(address(opeth), _opeth);
        IERC20(oToken).safeApprove(address(opeth), _opeth.div(1e10));
        _mint(opeth, _opeth, _oTokenPayment, _maxPayment, _0xFee, _0xSwapData);
    }
    ...
    function mint(
        ...
        uint _oToken = _opeth.div(1e10);
        (address underlying, address oToken, uint _underlying) = opeth.getOpethDetails(_oToken);
...
interface IOpeth {
    ...
    function getOpethDetails(uint _oToken) external view returns(address, address, uint);
}
