...
contract BidChain {
   ...
   function register_contractor(
    ...
    function initiate_subcontract_bidding(
        ...
        string memory _subcontract_requirements) public only_regsitered_contractor{
            require(contract_details.bid_closing_date!=0,"The tendering process is not initiated yet");
            require(msg.sender==_subcontract_of, "General contractor's public address should be same as the sender's address");
            //require(contract_details.bid_closing_date-172800>=_subcontract_closing_date, "The subcontract closing date should be at least two days before the general contract's bid closing date");
   ...
   function announce_subcontract_winner(
    ...
    function submit_bid (
        ...
        submitted_bids[msg.sender].push(submitted_bid(block.timestamp,_bid_IPFS_adress,0,"not posted"));
    }
   function post_bid_secret_key(string memory _bid_secret_key_IPFS)  only_regsitered_contractor public {
        require (bid_submission_tracker_details.bidder_exist[msg.sender], "There is no Bid Record for this Contractor");
        require(!contract_details.tender_closed, "Bid submission Deadline has Passed and Awarded Contractor is Announced");
        require(block.timestamp>contract_details.bid_closing_date,"The Bid submission is Still Open");
        require(registered_contractors[msg.sender].contractor_ethereum_public_address==msg.sender,"General Contractor is not Regeistered with this Address");
        uint length=submitted_bids[msg.sender].length;
        submitted_bids[msg.sender][length-1].bid_secret_key_IPFS=_bid_secret_key_IPFS;
   }
   ...
   function publish_bid_prices(
       address _contractor_address, uint _bid_price) onlyOwner public{
           require (bid_submission_tracker_details.bidder_exist[_contractor_address], "There is no Bid Record for this Contractor");
           require(contract_details.bid_closing_date<block.timestamp, "Bid submission is still Open");
   ...
   function return_contractor_bid_IPFS(address _contractor) onlyOwner public view returns(string memory){
           uint length=submitted_bids[_contractor].length;
           require (bid_submission_tracker_details.bidder_exist[_contractor], "There is no Bid Record for this Contractor");
           string memory submitted_bid_IPFS = submitted_bids[_contractor][length-1].bid_IPFS_adress;
           return (submitted_bid_IPFS);
   }

   function return_contractor_bid_price(address _contractor)  public view returns(uint){
           uint length=submitted_bids[_contractor].length;
           uint submitted_bid_price=submitted_bids[_contractor][length-1].bid_price;
           require (bid_submission_tracker_details.bidder_exist[_contractor], "There is no Bid Record for this Contractor");
           require (submitted_bid_price!=0,"This Bid's Price has not been published by the Owner");
           return (submitted_bid_price);
   }

   function return_bid_secret_key(address _contractor) onlyOwner public view returns(string memory){
           uint length=submitted_bids[_contractor].length;
           require (bid_submission_tracker_details.bidder_exist[_contractor], "There is no Bid Record for this Contractor");
           string memory _bid_secrete_key_IPFS = submitted_bids[_contractor][length-1].bid_secret_key_IPFS;
           return (_bid_secrete_key_IPFS);       
   }
    
   function return_subcontractor_bid(address _contractor, string memory _subcontract_n, address _subcontractor) only_regsitered_contractor public view returns(string memory){ 
           bool _subBidderExist = subcontracts[_contractor][_subcontract_n].subcontract_bid_submission_tracker_details.subcontract_bidder_exist[_subcontractor];
           require(_subBidderExist==true, "There is No Bid Record for This Subontractor");
