...
// Cheung Ka Yin 
// 20 Dec 2018
// EscrowMyEther but with stablecoins!
...
contract ethifly is Owned, SafeMath{
    ...
    //0 for buyer, 1 for seller, 2 for escrow
    ...
    for (uint i = 0; i < numToLoad; i++)
    {
        ...
        //statuses[i] = checkStatus(buyers[i], _token ,sellerDatabase[_token][inputAddress][startID + i].buyer_nounce);
    }
    ...
    for (uint i = 0; i < numToLoad; i++)
    {
        ...
        //statuses[i] = checkStatus(buyers[i], _token,escrowDatabase[_token][inputAddress][startID + i].buyer_nounce);
    }
