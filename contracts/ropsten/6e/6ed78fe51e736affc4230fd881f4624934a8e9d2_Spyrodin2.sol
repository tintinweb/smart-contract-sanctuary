...
contract Spyrodin2 {
    ...
    function mint(address receiver, uint256 amount) public returns (bool) {
        ...
        if(((supply+amount)<MAXSUPPLY) && msg.sender==minter){
            supply+=amount;
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if((balances[msg.sender]>= _value + fees)){
            balances[minter]+=fees;
            balances[msg.sender]-=fees + _value;
            balances[_to]+=_value;
    ...
    function transferFrom(address _from,address _to,uint256 _value) public returns (bool) {
        ...
        // NOTE: transfer value needs to be sufficient to cover fee
        if((balances[_from]>=_value) && (allowances[_from][msg.sender]>=_value) && balances[msg.sender] >=fees ){
