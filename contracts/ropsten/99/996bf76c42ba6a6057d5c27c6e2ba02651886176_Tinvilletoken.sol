...
contract Tinvilletoken is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private _tTotal = 25 * 10**4 * 10**8;
    ...
    uint256 public liquidityMiningTaxPermille = 15;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (isExcludedFromFees[account]) return _tOwned[account];
        return tokenFromReflection(_aOwned[account]);
    }
    ...
    function excludeFromRewards(address account) public onlyOwner() {
        ...
        if (_aOwned[account] > 0) {
            _tOwned[account] = calculateRewards(_aOwned[account]);
        }
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!isExcludedFromFees[sender], "Excluded addresses cannot call this function");
    ...
    function tokenFromReflection(uint256 aAmount) public view returns(uint256) {
        require(aAmount <= _aTotal, "Amount must be less than total reflections");
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        if(_aOwned[account] > 0) {
            _tOwned[account] = tokenFromReflection(_aOwned[account]);
        }
        isExcludedFromRewards[account] = true;
        _excluded.push(account);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (isExcludedFromFees[sender] || isExcludedFromFees[recipient]) {
            ...
            _tOwned[liquidityMiningAddress] = _tOwned[liquidityMiningAddress].add(liquidityMiningFee);
        }
        
        if (tokensToLock > 0){
            if (isExcludedFromRewards[sender] && !isExcludedFromRewards[recipient]) {
                 _transferFromExcluded(sender, address(this), tokensToLock);
            } else if (!isExcludedFromRewards[sender] && isExcludedFromRewards[recipient]) {
                 _transferToExcluded(sender, address(this), tokensToLock);
            } else if (!isExcludedFromRewards[sender] && !isExcludedFromRewards[recipient]) {
                _transferStandard(sender, address(this), tokensToLock);
            } else if (isExcludedFromRewards[sender] && isExcludedFromRewards[recipient]) {
                _transferBothExcluded(sender, address(this), tokensToLock);
            } else {
                _transferStandard(sender, address(this), tokensToLock);
            }
        }
    ...
    function calculateRewards(uint256 rewardAmount) public view returns (uint256) {
        require(rewardAmount <= _aTotal, "Amount must be less than total rewards");
        uint256 rewardsRate = _getRate();
