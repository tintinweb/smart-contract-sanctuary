...
contract DiceBetTracker {
    uint256 public totalHouseMoney;
    ...
    function makeBet(uint256 _dicePick) public payable {
        ...
        Bet storage bet;
        require(totalHouseMoney > msg.value, "house is broke");
        bet.payout = msg.value * 2;
        totalHouseMoney -= msg.value;
...
//     If win the payout will be 2:1 and if loss the house takes the money
    function houseFillBet(uint256 _diceRoll, address payable gambler)
        public
        payable
        onlyOwner
    {
        Bet storage bet = getBet(gambler);
        if (_diceRoll == bet.dicePick) {
            uint256 oneEth = 10 * 10**18;
            // TODO: add rounding to make sure payout is less than or equal to 1 Eth
            require(bet.payout <= oneEth);
            bet.isWin = true;
            bet.diceRoll = _diceRoll;
            bet.isFilled = true;
            gambler.transfer(bet.payout);
        } else {
            // TODO: need to actually send payout to the gamblers address
            bet.isWin = false;
            bet.diceRoll = _diceRoll;
            totalHouseMoney += bet.payout;
            bet.payout = 0;
            bet.isFilled = true;
        }
    }

    function getBet(address gambler) internal returns (Bet storage) {
        Bet[] storage bets = addressToUserBets[gambler];
        Bet storage firstUnfilledBet =  bets[0];
        ...
        for (
            ...
            if (!bets[betIndex].isFilled) {
                ...
                Bet storage firstUnfilledBet =  bets[betIndex];
