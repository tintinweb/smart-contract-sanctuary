...
contract Staking is ReentrancyGuard, Context, IStaking {
    ...
    function deposit(uint256 amount) external view override returns (bool) {
    ...
    function withdraw(uint256 amount) external override onlyFactory returns (bool) {
        require(amount > 0, "Input error");
        address sender = _msgSender();
        bool isEndStake;
        users[sender].amountStaked -= amount;
    ...
    function addReward(uint256 amountReward) external override {
        rewards.push(RewardInfo(amountReward, totalStaked));
    }
    ...
    function availDividends(address user) external view override onlyFactory returns (uint256) {
        return users[user].amountClaimed;
    }
...
interface IStaking {
    ...
    function deposit(uint256 amount) external view returns (bool);
