...
contract DEXHIGH is RedBlackTree, SafeMath
{
    ...
    function LimitOrder(address token, bool isSell, uint amount, uint price) public returns (uint32)
    {
        ...
        else
        {
            ...
            balance.available = SafeMath.safeSub(balance.available, SafeMath.safeMul(amount, price) / 100);
            balance.reserved = SafeMath.safeAdd(balance.reserved, SafeMath.safeMul(amount, price) / 100);
        }
    ...
    function matchOrder(address token, Pair storage pair, Order order, uint32 id) private
    {
        ...
        while (currentOrderId != 0 && order.amount > 0 && ((order.sell && order.price <= orders[currentOrderId].price) || (!order.sell && order.price >= orders[currentOrderId].price)))
        {
            ...
            if (order.sell == true)
            {
                ...
                balanceETH.reserved -= tradeAmount * matchingOrder.price / basePrice;
                balanceETH.available += tradeAmount * matchingOrder.price / basePrice;
            }
            else
            {
                balanceETH = balances[0][order.owner];
                ...
                balanceETH.reserved -= tradeAmount * order.price / basePrice;
                balanceETH.available += tradeAmount * (order.price - matchingOrder.price) / basePrice;
                balanceToken.available += tradeAmount;
                ...
                balanceETHCp.available += tradeAmount * matchingOrder.price / basePrice;
            }
