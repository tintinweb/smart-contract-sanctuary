...
//    }
    ...
    function getListConversionRates(KyberNetwork network) public view returns(ConversionRatesInterface[] memory conversionRates) {
        uint numKyberReserves = getNumberKyberReserves(network);
        KyberReserve reserve;
        ConversionRatesInterface conversionRate;
        uint id;
        uint resultCount = 0;
        for(id = 0; id < numKyberReserves; id++) {
            reserve = getKyberReserveAtIndex(network, id);
            conversionRate = reserve.conversionRatesContract();
    ...
    function getListTokens(KyberNetwork network) public view returns (ERC20[] memory orderReserveTokens, KyberReserve[] memory kyberReserves) {
