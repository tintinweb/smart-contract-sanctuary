...
contract iUventaToken is MintableToken {
    ...
    string public constant name = "iUventa016";
    
    string public constant symbol = "iUv016";
...
contract Crowdsale is Ownable {
  ...
  //  uint hardcap;
    ...
    //    StartEmissionRate = 10000000000000000000000; // сколько токенов выдаем за 1 Эвир
    ...
    uint DeltaEmission = Emission-TokenSumm; // сколько осталось выпустить токенов на данном этапе
...
// если мы собираемся выпустить токенов меньше или столько же, сколько осталось довыпустить в этом шаге. Потом они уже выпускаться не будут и деньги вернутся
      ...
      if (DeltaEmission > tokens) {
       ...
       // TokenSumm = TokenSumm + tokens;
        ...
        token.mint(restricted, tokens);
        MintedTokens(TokenSumm);   
        Emission=Emission.mul(EmissionGrowthCoefficient+100).div(100);
        ...
        uint balance = msg.value - UsedValue;
        ...
        if (DeltaEmission > tokens) {
          ...
          multisig.transfer(UsedValue2 + UsedValue); // отправляем нам сумму на которую выпустили токенов
          msg.sender.transfer(msg.value - UsedValue2 - UsedValue); // возвращаем оставшиесы деньги
          token.mint(msg.sender, DeltaEmission);
          token.mint(restricted, DeltaEmission);  
          MintedTokens(TokenSumm);
          Emission=Emission.mul(EmissionGrowthCoefficient+100).div(100);
