...
contract CivInvestment is ICivInvestment, Ownable {
    ...
    function _mintLiquidity(
        ...
        TransferHelper.safeApprove(_WETH, address(v3NftManager), wethAmount);
        TransferHelper.safeApprove(_CIV, address(v3NftManager), civAmount);
        ...
        INonfungiblePositionManager.MintParams memory params =
            ...
            // Note that the pool defined by WETH/CIV and fee tier 0.3% must already be created and initialized in order to mint
        ...
        if (amount0 < wethAmount) {
            TransferHelper.safeApprove(_WETH, address(v3NftManager), 0);
            uint256 refund0 = wethAmount - amount0;
            TransferHelper.safeTransfer(_WETH, msg.sender, refund0);
        }

        if (amount1 < civAmount) {
            TransferHelper.safeApprove(_CIV, address(v3NftManager), 0);
            uint256 refund1 = civAmount - amount1;
            TransferHelper.safeTransfer(_CIV, msg.sender, refund1);
        }
...
import './ICivDevEvent.sol';

interface ICivInvestment is ICivDevEvent, IERC721Receiver {

    /**
    * @dev Info to describe Trade information by nft id.
    * NOTE: [state] -1/Withdraw, 0/pending, 0~1/filled-percent, 1/Ready to withdraw
    */
    struct TradeInfo {
        uint256 id;
        address token0;
    ...
    /**
     * Retrieve the ETH from the holding contract.
     * Swap the total ETH in token0, token1 (where one of the token could be EHT or not) 
     * to use it  for the liquidity pool opening position
     * Create Uniswap V3 position for the token0, token1 using the amount0, amount1 earned with the swap.
     * It will select the percentage of commission { 0: 0.05, 1: 0.3, 2: 2: 1}%
     * Also it will open in the range of price min - max as amount of token1 per 1 token0
     * Only the owner could call this method
     */ 
    function addLiquidityV3Position(uint256 ethAmountToUse, address token0, address token1, 
                                    uint24 percentage, int24 minToken1ToToken0Swap, 
...
interface ICivHolding {
    /**
     * Ask the Holding contract for the amount of ETH, the transfer will send to the msg.sender 
     * the ETH if it is allowed to.
     */
    function transferETHtoMe(uint256 amount) external;
    
    /**
    * @dev Allow user for transfering ETH from contract
    * NOTE: 
    */
    function blockTransfer() external;

    /**
    * @dev get if user allowed for transfering ETH
    * NOTE: 
    */
    function isAllowed(address _address) external returns (bool);
...
// enum CommissionPercentage { smaller05, medium03, large1 }

interface ICivDevEvent {

    event NewTicks(int24 lowerTick, int24 upperTick);

    event NewLogAddress(address _data, string str);
    event NewLogInt24(int24 _data, string str);
    event NewLogUint256(uint256 _data, string str);
    event receiveERC721(uint256 _tokenId);
