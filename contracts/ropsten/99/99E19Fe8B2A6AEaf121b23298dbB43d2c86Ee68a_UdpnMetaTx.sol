...
contract UdpnMetaTx is EIP712Base {
   ...
   bytes32 private constant _TYPEHASH = keccak256(bytes("UdpnRequest(address from,address to,uint256 value,uint256 gas,uint256 nonce,bytes functionSignature)"));
   ...
   function getBalance(address tokenAddress) external view returns(uint256 amount) {
      return ERC20(tokenAddress).balanceOf(owner);
   }
}
