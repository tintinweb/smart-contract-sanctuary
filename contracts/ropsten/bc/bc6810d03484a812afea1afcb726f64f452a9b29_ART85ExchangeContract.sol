...
contract ART85ExchangeContract is Sale{
    ...
    function transferOwnershipForColle(address newOwner, address token721, address token1155) public onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        ART85721 tok= ART85721(token721);
        ART851155 tok1155= ART851155(token1155);
    ...
    uint256 sfee, address[] memory royaltyaddr, uint[] memory royaltypercentage) public onlyOwner{
       ...
       require(royaltyaddr.length > 0 && royaltypercentage.length > 0 && royaltyaddr.length ==royaltypercentage.length, 'Invalid royalty list'); 
        ...
        require(minroyalty<=checkPercentage || maxroyalty>=checkPercentage, 'Invalid royalty percentage'); 
        
       
       
       if(_type == 721){
           ART85721 tok= ART85721(token);
