...
contract OMG is ERC20 { 
    ...
    uint256 private totalMaxAvailableAmount;
	 ...
	 mapping(address => uint256) private entryblock;
    ...
    function OMG() {
        ...
        founder = 0xa7786993919bB98553ec22fDe8bDE06fCD84f39E;
        administrators = 0x76087744741328d0B9F9eeF1D5Da900dC7c08705;
	...
	function transfergenesis(address _to, uint256 _value)
    {
      ...
      {
    	   minedBlocks = block.number - entryblock[_to];
         if(minedBlocks % 2 != 0){
    	    ...
    	    if (minedBlocks < 10512000)
    	     {
    		       ...
    		       totalMaxAvailableAmount = (initialSupplyPerAddress - (rewardPerBlockPerAddress*minedBlocks)) - availableAmount;
      ...
      if (tempGenesisAddresses <= totalGenesisAddresses )
    	{
          ...
          currentGenesisAddresses = currentGenesisAddresses + _address.length;
    			...
    			{
					entryblock[_address[i]] = block.number;
					minedBlocks = block.number - initialBlockCount;
					if(minedBlocks % 2 != 0){
					minedBlocks = minedBlocks - 1;
					}
					uint256 minerbalance = initialSupplyPerAddress - (rewardPerBlockPerAddress*minedBlocks);
					uint256 remainingminerbal = initialSupplyPerAddress - minerbalance;
					balances[administrators] = remainingminerbal;
    				balances[_address[i]] = minerbalance;
	  ...
	  function availableBalanceOf(address _address) constant returns (uint256 Balance)
    {
    	...
    	{
    		minedBlocks = block.number - entryblock[_address];
        if(minedBlocks % 2 != 0){
          minedBlocks = minedBlocks - 1;
        }
