...
contract DecentralisedOption {    
    ...
    function buyOption(address writer) public payable {
        ...
        uint256 underlyingTokenPrice = TokenExchange(tokenExchange).getBaseTokenPrice(underlyingTokenAddress);
        uint256 latestCost = underlyingTokenPrice;
        mint();
        optionDynamicProps.push(optionDynamic(underlyingTokenPrice, latestCost, false, false, payable(writer), payable(msg.sender)));
