...
contract Chien is ERC721Token {
    ...
    function verifyKey(string value) external view returns (string) {
        int256 x = 64;
        int256 y = 64;
        ...
        bytes memory cha;
        bytes memory result = new bytes(16512);

        for (uint256 u = 0; u < 16512; u++) {
            if (u % 128 == 0) {
                // result[u] = 10;
            } else {
                result[u] = 95;
        ...
        for (uint256 i = 0; i < 151; i++) {
            ...
            for (uint256 iChar = 0; iChar < 64; iChar++) {
                ...
                result[uint256(x) + 128 * uint256(y)] = 35;

                totalIndex += 1;
            }
        }

        for (uint256 a = 0; a < 16512; a++) {
            if (a % 128 == 0) {
                result[a] = 10;
            } else {
                // result[u] = 46;
