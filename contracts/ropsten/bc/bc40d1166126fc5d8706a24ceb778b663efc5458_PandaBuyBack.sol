...
contract PandaBuyBack is Context, IERC20, Ownable {
    ...
    uint256 public _buyBackFee=500;//meanse 5 goes to buyback
    
    
    address buyBackAddress=0x11fa854E60087F1859e9Ceb00dE5c8cA1A027F56; //Dummy wallet buyback
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify && sender != uniswapV2Pair) {
            ...
            if(now > lastRebalance + rebalanceInterval 
                ...
                && buyBackBalance >= minEthBeforeSwap){
                buyAndBurnToken(buyBackBalance);
                swap = false;
            }
            //buy eth
            if(swap) {
                uint256 _buyBackBalance = balanceOf(buyBackAddress);
                bool overMinTokenBalance = _buyBackBalance >= minTokensBeforeSwap;
    ...
    function swapTokensForEth() private {
        uint256 tokenAmount = balanceOf(buyBackAddress);
        uint256 ethAmount = buyBackAddress.balance;
        ...
        ethAmount = buyBackAddress.balance.sub(ethAmount);
