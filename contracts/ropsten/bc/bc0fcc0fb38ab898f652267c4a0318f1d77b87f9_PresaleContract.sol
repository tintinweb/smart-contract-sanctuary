...
contract PresaleContract is Ownable {
    ...
    IBEP20 public tokenBUSD = IBEP20(0xd9d6e7f4dF7A11a57c4D596b313d007708A8B65D);
    IBEP20 public tokenPCA = IBEP20(0x506c020BC1710a77627eDb520c8aEaded6aEA92c);
    ...
    function amountClaimableInPrivateVesting(address account) public view returns (uint256){
    ...
    /* PUBLIC SALE */
    uint256 public publicHardCap;
    uint256 public publicSoldAmount;
    uint256 public publicRate;
    uint256 public publicMinDeposit;
    uint256 public publicMaxDeposit;
    mapping (address => uint256) public publicBUSDDeposit;
    bool public publicSaleEnable;
    uint256 public openTime = 1643500800; // 30/1/2022 - https://www.epochconverter.com/
    mapping (address => uint256) public publicVestingAmount;

    function ownerSetupPublicSale(uint256 totalAmount, uint256 rate, uint256 min, uint256 max, uint256 epochTimeOpenForClaim) public onlyOwner{
        publicHardCap = totalAmount;
        publicRate = rate;
        openTime = epochTimeOpenForClaim;
        publicMinDeposit = min;
        publicMaxDeposit = max;
    }
    ...
    function buyPublicSale(uint256 busdAmount) public {
        ...
        require(busdAmount >= publicMinDeposit, "Minimum amount is 1 BUSD");

        publicBUSDDeposit[msg.sender] = publicBUSDDeposit[msg.sender].add(busdAmount);
        require(publicBUSDDeposit[msg.sender] < publicMaxDeposit, "Exceed maximum amount BUSD deposit");
        tokenBUSD.transferFrom(msg.sender, address(this), busdAmount);

        uint256 tokenAmount = busdAmount.mul(publicRate);
        publicVestingAmount[msg.sender] = publicVestingAmount[msg.sender].add(tokenAmount);

        publicSoldAmount = publicSoldAmount.add(tokenAmount);
        require(publicSoldAmount <= publicHardCap,"Exceed total amount for public sale");
    }

    /* CLAIM & INFOR */
    function claimToken() public {
        // Public
        if (publicVestingAmount[msg.sender] > 0 && block.timestamp > openTime) {
        ...
        // Private
        Vesting storage vestPlan = privateList[msg.sender];
        uint256 claimableAmount = amountClaimableInPrivateVesting(msg.sender);
    ...
    function nextClaim (address account) public view returns (uint256){
        ...
        if(vestPlan.totalBalance > 0) { // This account bought private
            ...
            if(nextClaimPrivateVesting > openTime && openTime > 0 && publicVestingAmount[msg.sender] > 0){
                return openTime;
            }else {
    ...
    function lockedAmount(address account) public view returns(uint256){
        Vesting memory vestPlan = privateList[account];
        return  (vestPlan.totalBalance -  vestPlan.totalClaimed) + publicVestingAmount[msg.sender] - availableAmount(account);        
    }

    function availableAmount(address account) public view returns(uint256){
        uint256 available = amountClaimableInPrivateVesting(account);
        if(block.timestamp > openTime)
            available += publicVestingAmount[msg.sender];
    ...
    function ownerWithdrawUnsoldPCA() public onlyOwner {
        tokenPCA.transfer(msg.sender, tokenPCA.balanceOf(address(this)) - publicSoldAmount - privateSoldAmount);
    }
}