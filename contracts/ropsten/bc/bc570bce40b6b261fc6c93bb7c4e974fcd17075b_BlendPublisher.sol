...
contract BlendPublisher{
    ...
    event newBlendDiscloseRecord(uint32 indexed date, int64[] positions, bytes32 underlyingReturnsHash, uint64 price, bytes16 secretPath);
    ...
    function revealHiddenPosition(uint32 date, int64[] positions, bytes32 underlyingPricesHashes, uint64 stratPrice, bytes16 secretPath) public onlyAuthorized {
        bytes32 recomputedHash = sha256(abi.encodePacked(date,positions,secretPath));
        ...
        emit newBlendDiscloseRecord(date, positions, underlyingPricesHashes, stratPrice, secretPath);
