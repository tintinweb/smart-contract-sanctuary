...
abstract contract NilPassCore is ERC721Enumerable, ReentrancyGuard, AccessControl, INilPass, IPricingStrategy {
   ...
   function canMint(address account, bytes calldata data)public virtual override view returns (bool);
...
contract Visions is VisionsPricing {
    ...
    Counters.Counter private _generatedIds;
    ...
    // Nov 16 18:00 UTC
    uint32 karmaSaleLaunchTime = 1637085600;
    // Nov 16 19:00 UTC (1 hour later)
    uint32 nHolderSaleLaunchTime = karmaSaleLaunchTime + 3600;
    // Nov 16 20:00 UTC (1 hour later)
    uint32 publicSaleLaunchTime = nHolderSaleLaunchTime + 3600;

    DerivativeParameters params = DerivativeParameters(false, false, 0, 333, 4);
    ...
    function setupAirdrop(address[] calldata addresses, uint256[] calldata amount) public onlyAdmin {
        for (uint256 i = 0; i < addresses.length; i++) {
            airdropAmount[addresses[i]] = amount[i];
        }
    }
    ...
    function mintWithN(
        ...
        // Make sure N isnt already used
        for (uint256 i = 0; i < maxTokensToMint; i++) {
            require(!nUsed[tokenIds[i]], "Visions:N_ALREADY_USED");
        }

        for (uint256 i = 0; i < maxTokensToMint; i++) {
            uint256 tokenId = _tokenIds.current() + AIRDROP_RESERVE;
    ...
    function claimAirdrop(address recipient) public nonReentrant {
        uint256 airdropNum = airdropAmount[recipient];
        require(airdropNum > 0, "Visions:CANT_CLAIM_AIRDROP");
        require(_generatedIds.current() <= AIRDROP_RESERVE, "Visions:ALL_AIRDROPPED_CLAIMED");

        for (uint256 i = 0; i < airdropNum; i++) {
            uint256 tokenId = _generatedIds.current();
            _mint(recipient, tokenId);
            airdropAmount[recipient]--;
            _generatedIds.increment();
        }
    }

    function sweepAirdrop(address recipient) public virtual onlyDAO nonReentrant {
        uint256 unclaimedAmount = AIRDROP_RESERVE - _generatedIds.current();
        for (uint256 i = 0; i < unclaimedAmount; i++) {
            uint256 tokenId = _generatedIds.current();
            _mint(recipient, tokenId);
            _generatedIds.increment();
        }
    ...
    function totalMintsAvailable() public view override returns (uint256) {
        if (_isKarmaSaleActive()) {
            return KARMASALE_AMOUNT + 1 - _tokenIds.current();
        } else if (_isnHolderSaleActive()) {
            return KARMASALE_AMOUNT + NHOLDERSALE_AMOUNT + 1 - _tokenIds.current();
        } else {
            return params.maxTotalSupply + 1 - _tokenIds.current();
        }
    }
    ...
    function canMint(address account, bytes calldata data) public view virtual override returns (bool) {
        ...
        if (
            ...
            hasN &&
            getKarma(data) >= 1020 &&
            _tokenIds.current() <= KARMASALE_AMOUNT
        ) {
            return true;
        }
        ...
        if (
            ...
            hasN &&
            getKarma(data) >= 1000 &&
            _tokenIds.current() <= KARMASALE_AMOUNT + NHOLDERSALE_AMOUNT
        ) {
            return true;
        }

        // Public
        if (_isPublicSaleActive() && mintsAvailable && balanceBelow && _tokenIds.current() <= params.maxTotalSupply) {
            return true;
        }
