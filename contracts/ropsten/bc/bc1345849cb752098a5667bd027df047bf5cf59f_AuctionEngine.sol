...
contract AuctionEngine {
    ...
    address yoaAddress = 0xbc15c1d374e73fee2eff91a801a6f0b15844fa6c;
    ...
    function createAuctionYoa(address _assetAddress,
                           uint256 _assetId,
                           address _paymentWallet,
                           uint256 _startPrice, 
                           uint256 _startTime, 
        ...
        uint256 index = auctionsYoa.push(auction) - 1;
    ...
    function createMarketplaceYoa(address _assetAddress,
        ...
        uint256 index = marketplacesYoa.push(marketplace) - 1;
    ...
    function bidYoa(uint256 auctionIndex, uint256 amount) public returns (bool) {
        IERC20 token = IERC20(yoaAddress);
        Auction storage auction = auctionsYoa[auctionIndex];
        require(auction.creator != address(0));
        require(isActiveYoa(auctionIndex));
        ERC721 asset = ERC721(auction.assetAddress);
        require(asset.ownerOf(auction.assetId) == auction.creator);
        require(asset.getApproved(auction.assetId) == address(this));
        ...
        // refund last highest bidder
        if (auction.currentBidOwner != address(0)) {
            token.transfer(auction.currentBidOwner, auction.currentBidAmount);
        }
    ...
    function buy(uint256 marketplaceIndex) public payable returns (bool) {
        ...
        require(asset.ownerOf(marketplace.assetId) == marketplace.creator);
        require(asset.getApproved(marketplace.assetId) == address(this));
    ...
    function buyYoa(uint256 marketplaceIndex, uint256 amount) public returns (bool) {
        ...
        require(token.balanceOf(msg.sender) >= amount);
        require(amount >= marketplace.price);
        
        marketplace.buyer = msg.sender;
        token.transferFrom(msg.sender, marketplace.paymentWallet, amount);
        asset.transferFrom(marketplace.creator, msg.sender, marketplace.assetId);
    ...
    function getTotalAuctionsYoa() public view returns (uint256) { return auctionsYoa.length; }
    function getTotalMarketplace() public view returns (uint256) { return marketplaces.length; }
    function getTotalMarketplaceYoa() public view returns (uint256) { return marketplacesYoa.length; }

    function isActive(uint256 index) public view returns (bool) { return getStatus(index) == Status.active; }
    function isActiveYoa(uint256 index) public view returns (bool) { return getStatusYoa(index) == Status.active; }
    ...
    function getAuctionInformationYoa(uint256 index) public view returns (address) {
    ...
    function getMarketplaceInformationYoa(uint256 index) public view returns (address) {
    ...
    function getCurrentBidOwnerYoa(uint256 auctionIndex) public view returns (address) { return auctionsYoa[auctionIndex].currentBidOwner; }
    
    function getPaymentWallet(uint256 auctionIndex) public view returns (address) { return auctions[auctionIndex].paymentWallet; }
    function getPaymentWalletYoa(uint256 auctionIndex) public view returns (address) { return auctionsYoa[auctionIndex].paymentWallet; }
    ...
    function getCurrentBidAmountYoa(uint256 auctionIndex) public view returns (uint256) { return auctionsYoa[auctionIndex].currentBidAmount; }

    function getBidCount(uint256 auctionIndex) public view returns (uint256) { return auctions[auctionIndex].bidCount; }
    function getBidCountYoa(uint256 auctionIndex) public view returns (uint256) { return auctionsYoa[auctionIndex].bidCount; }
    ...
    function getWinnerYoa(uint256 auctionIndex) public view returns (address) {
        require(isFinishedYoa(auctionIndex));
        return auctionsYoa[auctionIndex].currentBidOwner;
    }  
    ...
    function claimTokensYoa(uint256 auctionIndex) public { 
        require(isFinishedYoa(auctionIndex));
        Auction storage auction = auctionsYoa[auctionIndex];
        ...
        token.transfer(auction.paymentWallet, auction.currentBidAmount);
    ...
    function claimAssetYoa(uint256 auctionIndex) public {
        require(isFinishedYoa(auctionIndex));
        Auction storage auction = auctionsYoa[auctionIndex];
