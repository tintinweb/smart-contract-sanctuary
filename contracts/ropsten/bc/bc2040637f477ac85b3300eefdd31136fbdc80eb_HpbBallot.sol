...
contract FetchVoteInterface {
    ...
    /**
	 * 获取所有候选人的详细信息 Get detailed information about all candidates.
	 */
...
contract HpbBallot is Ownable ,AdminInterface,VoteInterface,FetchVoteInterface {
    ...
    // The voter&#39;s address corresponds to the voter&#39;s ordinal number (voter
	// Array subscript),
    // making it easy to query and reduce gas consumption
    mapping (address => uint) voterIndexMap;

    // 最终获选者总数（容量，获选者数量上限）
    // the total number of final winners (capacity, the upper limit of the
    ...
    constructor () payable public {
        ...
        // 设置第一位置
        // Set the first position.
        candidateArray.push(Candidate(msg.sender, &#39;CandidateName&#39;, 0, new address[](0)));
    }
    ...
    function addCandidate(address _candidateAddr, bytes32 _name) onlyAdmin public {
        ...
        candidateArray.push(Candidate(_candidateAddr, _name, 0, new address[](0)));
        candidateArray[index].voterMapAddrs.push(msg.sender);
        emit CandidateAdded(_candidateAddr, _name);
    }

    function batchAddCandidate(address[] _candidateAddrs, bytes32[] _names) onlyAdmin public {
        for (uint i=0;i < _candidateAddrs.length;i++) {
            addCandidate(_candidateAddrs[i], _names[i]);
        }
    }
    ...
    function deleteCandidate(address _candidateAddr) onlyAdmin public {
        ...
        for (uint n=1;n < candidateArray[index].voterMapAddrs.length;n++) {
            ...
            // 撤回已经投的票
            voterArray[voterIndex].voteNumber = voterArray[voterIndex].voteNumber.sub(voterArray[voterIndex].candidateMap[_candidateAddr]);
	 ...
	 * 投票
	 */
    function  vote(address voterAddr, address candidateAddr, 
        uint num) onlyApproved(voterAddr) public {
	 ...
	 * 用于批量投票 For non locked voting
	 */
    function  batchVote(address voterAddr, address[] candidateAddrs, 
        uint[] nums) onlyApproved(voterAddr) public {
    ...
    function refreshVoteForAll() public {
        ...
        uint vcl=voterArray.length - 1;
        for (uint i = 1;i <= vcl;i++) {
            _refreshVoteForVoter(voterArray[i].voterAddr);
        }
    }

    function _refreshVoteForVoter(address voterAddr) internal {
        uint index=voterIndexMap[voterAddr];
        if (index != 0) {
            uint vvcl=voterArray[index].candidateMapAddrs.length - 1;
            uint balance=voterAddr.balance;
            ...
            if (balance < minLimit || balance < voteNumber) {
                for (uint i = 1;i <= vvcl;i++) {
                    ...
                    if (_num < minLimit) {
                        _cancelVote(voterAddr, _addr, _num);
                    } else {
                        uint  n_num=balance.mul(1 finney).div(voteNumber).mul(_num).div(1 finney);
                        if (n_num < minLimit) {
                            _cancelVote(voterAddr, _addr, _num);
                        } else {
                            _cancelVote(voterAddr, _addr, _num.sub(n_num));
                        }
                    }
                }
            }
	 ...
	 * 撤回对某个候选人的投票 Withdraw a vote on a candidate.
	 */
    function cancelVoteForCandidate(address voterAddr, address candidateAddr, 
        uint num) onlyApproved(voterAddr) public {
        _cancelVote(voterAddr, candidateAddr, num);
    }

    function _cancelVote(address voterAddr, address candidateAddr, 
        uint num) internal {
        uint index=voterIndexMap[voterAddr];
        ...
        if (candidateArray[candidateIndex].voterMap[voterAddr] == 0) {
            ...
            for (uint k=vIndex;k < candidateArray[candidateIndex].voterMapAddrs.length - 1;k++) {
                candidateArray[candidateIndex].voterMapAddrs[k] = candidateArray[candidateIndex].voterMapAddrs[k+1];
            }
            delete candidateArray[candidateIndex].voterMapAddrs[candidateArray[candidateIndex].voterMapAddrs.length - 1];
    ...
    function  batchCancelVoteForCandidate(address voterAddr, address[] candidateAddrs, 
        uint[] nums) onlyApproved(voterAddr) public {
        for (uint i=0;i < candidateAddrs.length;i++) {
            cancelVoteForCandidate(voterAddr, candidateAddrs[i], nums[i]);
        }
    }
    ...
    function _doVote(address candidateAddr, uint index,uint num) internal {
        ...
        if (candidateArray[candidateIndex].voterMapAddrsIndex[voterArray[index].voterAddr] == 0) {
            ...
            candidateArray[candidateIndex].voterMap[voterArray[index].voterAddr] = candidateArray[candidateIndex].voterMap[voterArray[index].voterAddr].add(num);
        ...
        // 候选者得票数累加
        candidateArray[candidateIndex].voteNumber = candidateArray[candidateIndex].
        	voteNumber.add(num);
