...
contract OrionVoting is IOrionVoting, ReentrancyGuardUpgradeable, OwnableUpgradeable
{
    ...
    //  User-voted-per-pool state. Now holds only amount, but can grow
    struct OrionUserVotingState
    {
        uint56 voted_amount;
    }
    ...
    {
        require(pool_states_[poolAddress].state & kVoteAvailable != 0, "vote n/a");

        //  Sanity check
        require(amount > 0, "invalid vote size");
    ...
    {
        require(pool_states_[poolAddress].state & kCeaseAvailable != 0, "cease n/a");

        require(amount > 0, "invalid cease size");
    ...
    function claimRewards(uint56 amount, address to) override external {
        ...
        require(pool_states_[msg.sender].state & kRewardsAvailable != 0, "reward n/a");
