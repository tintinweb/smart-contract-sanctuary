...
contract IRFAN is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = unicode"IRFAN";
    string private constant _symbol = "IRFAN";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                if(firstsell[from] + (2 minutes) < block.timestamp){
                    sellnumber[from] = 0;
                }
                if (sellnumber[from] == 0) {
                    ...
                    sellnumber[from]++;
                    sellcooldown[from] = block.timestamp + (7 minutes);
                }
                else if (sellnumber[from] == 3) {
                    sellnumber[from]++;
                    sellcooldown[from] = firstsell[from] + (9 minutes);
                }
