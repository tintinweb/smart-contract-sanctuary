...
contract Tenshi is Context, IERC20, Ownable {
    ...
    // We will set a minimum amount of tokens to be swaped
    uint256 private _numOfTokensToExchangeForKishu = 5 * 10**9;
    ...
    constructor (address payable kishuWalletAddress) public {
    ...
    function sendETHToKishu(uint256 amount) private {
        _kishuWalletAddress.transfer(amount);
    }
    ...
    function _setkishuFee(uint256 kishuFee) external onlyOwner() {
        require(kishuFee >= 1 && kishuFee <= 25, 'kishuFee should be in 1 - 25');
        _kishuFee = kishuFee;
    }
