...
// File: contracts/abstracts/Address.sol
...
// File: contracts/abstracts/ExtendedERC20.sol
...
contract ExtendedERC20 is Context, IERC20 {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal pure { 
       require(amount > 0, "ERC20WithSafeTransfer: invalid amount");  
       require(from != to, "ERC20WithSafeTransfer: sender and recipient must be different");  
    }

}

// File: contracts/abstracts/ERC20Burnable.sol
 ...
 */
contract ERC20Burnable is Context, ExtendedERC20 {
...
// File: contracts/abstracts/ERC20Pausable.sol
...
// File: contracts/abstracts/ERC20Mintable.sol
 ...
 * @dev Extension of {ERC20} that allows users to burn its token or burnFrom its allowance.
 */
contract ERC20Mintable is ExtendedERC20 {
    using Address for address;
 ...
 */
contract ERC20Reservable is ExtendedERC20 {
    using Address for address;
    using ECDSA for bytes32;
    ...
    mapping (address => mapping(uint256 => Reservation)) internal _reserved;
    ...
    function execute(address sender, Reservation storage reservation, uint256 transactionFee) internal returns (bool success) {        
        ...
        //fee = reservation fee + transaction fee, the executor will receive the reservatio fee only, 
        _transfer(sender, executor, fee.sub(transactionFee));
        _transfer(sender, recipient, amount);
        return true;
    }
    ...
    function _unreservedBalance(address account) internal view returns (uint256 amount) {
        return super.balanceOf(account).sub(_totalReserved[account]);
    }

    function _checkReservedBalance(address from, uint256 amount) internal view {
        if (from != address(0)) {
...
// File: contracts/abstracts/ETHLessTransfer.sol
...
// File: contracts/roles/CurrencyBoard.sol
...
contract CurrencyBoard is Context, Ownable {
    ...
    Roles.Role private _Collectors;
    Roles.Role private _Gluwas;
    address private _feeCollection;
    ...
    event CollectorAdded(address indexed account);
    event CollectorRemoved(address indexed account);
    ...
    constructor(address sender) public {
        if (!isAdmin(sender)) {
            ...
            {
                _feeCollection = sender;
            }
        }
    }   
  
    modifier onlyAdmin() {
        require(isAdmin(_msgSender()), "Admin Role: caller does not have the Admin role");
        _;
    }
    ...
    modifier onlyCollector () {
        require(isCollector (_msgSender()), "Controller  Role: caller does not have the Controller  role");
        _;
    }

    modifier onlyGluwa () {
        require(isGluwa(_msgSender()), "Controller  Role: caller does not have the Controller  role");
        _;
    }
    ...
    function setFeeCollectionAddress(address account) external onlyAdmin {
        _feeCollection = account;
    }

     function getFeeCollectionAddress() external view returns (address) {
        return _getFeeCollectionAddress();
    }

    function _getFeeCollectionAddress() internal view returns (address) {
        return _feeCollection;
    }
    ...
    function _addController (address account) private {
        _Controllers.add(account);
        emit ControllerAdded(account);
    }

    function _removeController (address account) private {
        _Controllers.remove(account);
        emit ControllerRemoved(account);
    }

    function isCollector (address account) public view returns (bool) {
        return _Collectors.has(account);
    }

    function addCollector (address account) external onlyAdmin {
        _addCollector(account);
    }

    function removeCollector (address account) external onlyAdmin  {
        _removeCollector (account);
    }

    function renounceCollector () external onlyCollector {
        _removeCollector (_msgSender());
    }

    function _addCollector (address account) private {
        _Collectors.add(account);
        emit CollectorAdded(account);
    }

    function _removeCollector (address account) private {
        _Collectors.remove(account);
        emit CollectorRemoved(account);
    }
    ...
    function addGluwa(address account) external onlyAdmin {
        _addGluwa(account);
    }

    function removeGluwa(address account) external onlyAdmin  {
        _removeGluwa(account);
    }

    function renounceGluwa() external onlyGluwa {
        _removeGluwa(_msgSender());
    }

    function _addGluwa(address account) private {
        _Gluwas.add(account);
        emit GluwaAdded(account);
    }

    function _removeGluwa(address account) private {
        _Gluwas.remove(account);
        emit GluwaRemoved(account);
    }

    function setFee(uint256 amount) external onlyCollector returns (bool) {
        _fee = amount;
        return true;
    }
...
// File: contracts/ControlledLuniverseGluwacoin.sol
 ...
 */
contract ControlledLuniverseGluwacoin is ERC20Pausable, CurrencyBoard, ERC20Burnable, ERC20Reservable, ETHlessTransfer {
    ...
    function pause() external onlyController returns (bool) {        
        return super._pause();
    }

    function unpause() external onlyController returns (bool) {        
        return super._unpause();
    }   

    function transferTo(address recipient, uint256 amount) public whenNotPaused returns(bool)
    {
    ...
    function execute(address sender, uint256 nonce) public returns (bool success) {
        ...
        if (!isGluwa(reservation._executor))
        {
            transactionFee = _fee.mul(2);
        }
        super.execute(sender, reservation, transactionFee);
        _collect(sender, transactionFee);
        return true;
    }
    ...
    function _beforeTokenTransfer(address from, uint256 amount) internal view { 
       require(balanceOf(from) >= amount, "ERC20WithSafeTransfer: insufficient balance");  
        _checkReservedBalance(_msgSender(), amount);
    }
