...
contract FutureExchangeRouter is IFutureExchangeRouter {
    ...
    function getListFutureTokensInPair(address token)
        external
        view
        override
    ...
    function isFutureToken(
        ...
        address futureToken = IFutureTokenFactory(futureTokenFactory)
        .getFutureToken(tokenA, tokenB, expiryDate);
        require(
            futureToken != address(0),
            "Future Exchange Router: FUTURE_TOKEN_DOES_NOT_EXISTS"
        );
        return futureToken;
    }
    ...
    function addLiquidityFuture(
        ...
        if (futureToken == address(0)) {
            futureToken = IFutureTokenFactory(futureTokenFactory)
            .createFutureToken(tokenA, tokenB, expiryDate);
        ...
        if (getLiquidityToken[futureToken] == address(0)) {
            LiquidityToken _liquidityToken = new LiquidityToken();
            liquidityToken = address(_liquidityToken);
            getLiquidityToken[futureToken] = liquidityToken;
        }
    ...
    function swapFuture(
        ...
        uint256 amountOut = getAmountsOutFuture(
            amountIn,
            tokenIn,
            tokenOut,
            expiryDate
        ...
        IFutureTokenFactory(futureTokenFactory).transferFromFuture(
            tokenOut,
            futureToken,
            address(this),
            amountOut
        );
        IFutureTokenFactory(futureTokenFactory).mintFuture(
            futureToken,
            to,
            amountMint
        );
    ...
    function closeFuture(
        ...
        IERC20(futureToken).transferFrom(
            msg.sender,
            futureTokenFactory,
            amountMinted
        );
        IFutureTokenFactory(futureTokenFactory).burnFuture(
            futureToken,
            amountMinted
        );
    ...
    function getAmountsOutFuture(
        ...
        return
            PrecogV2Library.getAmountsOutFuture(
                futureTokenFactory,
                amountIn,
                tokenIn,
                tokenOut,
    ...
    function getAmountsInFuture(
        ...
        return
            PrecogV2Library.getAmountsInFuture(
                futureTokenFactory,
                amountOut,
                tokenIn,
                tokenOut,
