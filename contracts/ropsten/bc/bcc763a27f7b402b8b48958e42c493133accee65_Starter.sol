...
contract Starter is Configurable {
    ...
    function withdrawable() public view returns (uint amt, uint vol) {
        if(!completed)
            return (0, 0);
        amt = IERC20(currency).balanceOf(address(this)).sub(totalPurchasedCurrency.mul(uint(1e18).sub(settleRate)).div(1e18));
        vol = IERC20(underlying).balanceOf(address(this)).sub(totalPurchasedCurrency.mul(settleRate).div(price));
    }
