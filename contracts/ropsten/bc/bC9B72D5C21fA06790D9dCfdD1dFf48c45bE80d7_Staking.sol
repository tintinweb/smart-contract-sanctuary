...
contract Staking is  AccessControl, MultiSig {
    ...
    mapping(uint256 => RewardRecord) public rewardRecords;
        ...
        external
    {
        ...
        RewardRecord memory record = rewardRecords[txId];
        if (record.tokens.length == 0) {
            record = RewardRecord(tokens, amounts, numBlocks, to);
            rewardRecords[txId] = record;
        }
