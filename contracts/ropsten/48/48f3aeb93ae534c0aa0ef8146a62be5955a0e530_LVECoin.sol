...
contract Ownable {
    ...
    // TODO:測試用
    // get proposal owner sign number(多數決)
    function testCount() public view returns(uint256) {
        uint256 signedCount = 0;
        for(uint256 i = 0; i < owners.length; i++) {
            signedCount++;
        }
    ...
    function getProposalInfo(uint256 _prpsIndex) public view isOwner isPrpsExists(_prpsIndex) returns(uint256 _prpsType, uint256 _signedCount, bool _isFinalized, address _fromAddr, address _toAddr){
