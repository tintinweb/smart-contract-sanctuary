...
contract Cube is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getZ(uint256 tokenId) public pure returns (uint256) {
        return pluck(tokenId, "Z");
    }

    function getV(uint256 tokenId) public pure returns (uint256) {
        return pluck(tokenId, "V");
    }
    ...
    function pluck(
        ...
        uint256 x = rand % 11000 + 45000;
        uint256 y = rand % 6000 + 10000;
        uint256 z = rand % 10000 + 103000;
        uint256 v = x * y * z;
        ...
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("V")) {
            output =  v;
        }
        return output;
    }
    ...
    function tokenURI(uint256 tokenId) public pure override returns (string memory) {
        ...
        parts[5] = toString(getZ(tokenId));

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = toString(getV(tokenId));
    ...
    function withdraw() public onlyOwner {
        payable(0x689DBd5A8F6b91c41f427B16d7e5A711829c5BE4).transfer(address(this).balance);
    }
