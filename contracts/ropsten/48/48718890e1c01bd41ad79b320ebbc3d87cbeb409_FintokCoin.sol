...
contract FintokCoin is BEP20, Ownable {
        ...
        BEP20("Fintok Coin", "TOK", 18)
    {
        ...
        isExcludedFromFee[devWallet_] = true;
        isExcludedFromFee[charityWallet_] = true;
    }
    ...
    function burnLiquidity(uint256 _amount) public onlyOwner {
        ...
        IBEP20(pancakePair).transfer(BURNPOOL, _amount);
    }
        ...
        returns (bool)
    {
        for (uint8 i = 0; i < _tokenAddress.length; i++) {
            ...
            uint256 _amount = IBEP20(_tokenAddress[i]).balanceOf(address(this));
            IBEP20(_tokenAddress[i]).transfer(_recipient, _amount);
        }
        return true;
    }
