...
contract SyrexStaking is Ownable {
    ...
    function userEarned(address staker) public view returns (uint256) {
        ...
        uint256 rewardsPerMin = rewardPerDay / (1 days / 1 minutes);
        uint256 minutesStaked = block.timestamp / 1 minutes - _stakers[staker].start / 1 minutes;
    ...
    function _getRewardsPerDay(address staker) private view returns (uint256) {
        uint256 totalCirculating = getCirculating();
        uint256 rewardsPerDay = _totalRewards / (_stakers[staker].period / 1 days);
