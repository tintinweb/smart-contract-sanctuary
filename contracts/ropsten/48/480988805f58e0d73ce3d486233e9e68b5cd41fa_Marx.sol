...
contract Marx is Context, Ownable, IERC20, IERC20Metadata {
    ...
    event newBuy(uint256 value, uint256 amount, uint256 lastPrice);
    event newSell(uint256 value, uint256 amount, uint256 lastPrice);
    ...
    uint256 private _lastPrice = 1000 wei;
    uint256 private _tick = 2 wei;
    ...
    function _calculateBuy(uint256 value) public view returns (uint256) {
        if (numTokensCirculating == 0) {
            uint256 averagePrice = (((_tick.div(value)).add(_lastPrice))).mul(2);
            uint256 amountBuy = value.div(averagePrice);
            return amountBuy;
            
        } else {
            uint256 pond = (numTokensCirculating.div(1000000000000)) ** 2;
            uint256 tick = _tick.mul(pond);
            uint256 averagePrice = (((tick.div(value)).add(_lastPrice))).mul(2);
            uint256 amountBuy = value.div(averagePrice);
            return amountBuy;
        }
    }
    ...
    function buy() external payable returns (uint256) {
        ...
        uint256 marxReleased = (_totalSupply.div(2)).sub(numTokensCirculating);
        
        require(totalAmount < marxReleased);
        
        _transfer(address(this), _msgSender(), totalAmount);
        numTokensCirculating += totalAmount;
        _lastPrice += totalAmount.mul(_tick);
        
        emit newBuy(valueBuy, totalAmount, _lastPrice);
    ...
    function _calculateSell(uint256 amount) public view returns (uint256) {
        uint256 averagePriceSell = _lastPrice.sub((_tick.mul(amount)));
        uint256 priceSell = (_lastPrice.add(averagePriceSell)).div(2);
        uint256 totalSell = priceSell.mul(amount);
        return totalSell;
    }
    ...
    function sell(uint256 amount) external {
        ...
        require(totalWei <= address(this).balance);
        payable(msg.sender).transfer(totalWei);
        
        _lastPrice -= amount.mul(10000).mul(_tick);
        _transfer(_msgSender(),address(this), amount.mul(10000));
        
        emit newSell(totalWei, amount, _lastPrice);
    }
