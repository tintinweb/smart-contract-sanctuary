...
contract BIRBToken is ERC20, Ownable {
    ...
    ERC20 internal WETH = ERC20(WETHAddress);

    IUniswapV2Factory public uniswapFactory = IUniswapV2Factory(uniswapAddress);
    ...
    modifier when3DaysBetweenLastSnapshot() {
        require((block.timestamp - lastRewardTime) >= 45 minutes, "BIRBToken: not enough days since last snapshot taken.");
        _;
    }

    event PoolHatched(address user, uint256 hatchAmount, uint256 newTotalSupply, uint256 newUniswapPoolSupply, uint256 userReward, uint256 newNestReward);
    ...
    event UniswapPoolCreated(address uniswapPool, address WETH, address createdBy);
    ...
    event MintNewSupply(address MintedBy, uint256 newSupply);
    event WETHAddressUpdated(address WETHAddress, address updatedBy);
    event UniswapAddressUpdated(address WETHAddress, address updatedBy);
    ...
    function updateUniswapAddress(address newUniswapAddress) external onlyOwner {
        
        uniswapAddress = newUniswapAddress;
        emit UniswapAddressUpdated(uniswapAddress, msg.sender);
    }
    
    function updateWETHAddress(address newWETHAddress) external onlyOwner {
        
        WETHAddress = newWETHAddress;
        emit WETHAddressUpdated(WETHAddress, msg.sender);
    }

    function setUniswapPool() external onlyOwner {
        require(uniswapPool == address(0), "BIRBToken: Pool already created");
        uniswapPool = uniswapFactory.createPair(address(WETH), address(this));
        
        emit UniswapPoolCreated(uniswapPool, address(WETH), msg.sender);
    }
    ...
    function hatchNest() external {
        ...
        _balances[uniswapPool] = _balances[uniswapPool].sub(hatchAmount);
        ...
        emit PoolHatched(msg.sender, hatchAmount, _totalSupply, balanceOf(uniswapPool), userReward, poolReward);
    }

    function getHatchAmount() public view returns (uint256) {
        if (paused) return 0;
        require((block.timestamp - lastHatchTime) >= 15 minutes, "BIRBToken: Already Hatched");
        uint256 tokensInUniswapPool = balanceOf(uniswapPool);
        return (tokensInUniswapPool.mul(HATCH_RATE).div(100));
    }
