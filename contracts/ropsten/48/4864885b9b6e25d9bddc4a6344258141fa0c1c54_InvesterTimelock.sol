 ...
 * owner.
 */
contract InvesterTimelock {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  uint256 public SECONDS_PER_MONTH;
  ...
  constructor(
    ...
    SECONDS_PER_MONTH=_seconds_per_month == 0 ? 30 days : _seconds_per_month;
    
    uint256 t=0;
    for (uint256 i = 0; i < _airdrops.length; i++) {
      address ia=_airdrops[i].to;
      uint256 am=_airdrops[i].amount;
      Invester memory iv=Invester({to: ia, amount:am,released:0});
      investers.push(iv);
      emit AddInvester(ia,am);
      investersIndex[_airdrops[i].to]=i+1; //cannot be 0
      t+=_airdrops[i].amount;
    }
    totalSupply=t;
  }
