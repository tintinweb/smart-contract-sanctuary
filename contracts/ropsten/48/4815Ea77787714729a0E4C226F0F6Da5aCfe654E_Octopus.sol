...
contract Octopus is Ownable {
    ...
    function sell(address _optionAddress, uint256 _weiNotional) payable public {
        ...
        require(IOption(_optionAddress).getMaturity() < block.timestamp, "Expired option");
    ...
    function buyWithUSDT(address _optionAddress, uint256 _usdtAmount) payable public {
        ...
        require(ERC20(usdtToken).transferFrom(msg.sender, address(this), _usdtAmount), "Transfer failed");
    ...
    function claimCollateralAtMaturityForSellers(address _optionAddress) public {
        ...
        uint256 decimals = IOption(_optionAddress).getDecimals();
        
        uint256 totalToReturn = seller.collateral - seller.notional.mul(optionWorth).div(1 * 10 ** decimals);

        SafeERC20.safeTransferFrom(usdtToken, address(this), msg.sender, totalToReturn);
    ...
    function claimCollateralAtMaturityForBuyers(address _optionAddress, uint256 _numberOfTokens) public {
        ...
        uint256 totalToReturn = _numberOfTokens.mul(optionWorth).div(1 * 10 ** decimals);
    ...
    function buyTokensInUniswap(address _optionAddress, uint256 _tokensAmount) internal returns (uint256)  {
        ...
        if (allowance > 0 && allowance < _tokensAmount) {
            usdtToken.approve(address(uniswapRouter), 0);
        }
        if (allowance == 0) {
            usdtToken.approve(address(uniswapRouter), _tokensAmount);
        }
        //uint256[] memory amountsOutMin = uniswapRouter.getAmountsOut(_tokensAmount, path);
        //TODO: Use amountsOutMin[0]
        uint[] memory amounts = uniswapRouter.swapExactTokensForTokens(_tokensAmount, 0, path, msg.sender, block.timestamp);
        return amounts[0];
    }
    
    function sellTokensInUniswap(address _optionAddress, uint256 _tokensAmount) public returns (uint256)  {
        address[] memory path = new address[](2);
    ...
    function _setEthFinalPriceAtMaturity(address _optionAddress) public onlyOwner {
        ...
        //require(IOption(_optionAddress).getMaturity() > block.timestamp, "Still not expired");
