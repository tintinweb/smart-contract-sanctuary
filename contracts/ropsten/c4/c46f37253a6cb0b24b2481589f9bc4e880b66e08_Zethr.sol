...
contract Zethr {
  ...
  uint constant internal               referrer_percentage1   = 15;
  uint constant internal               referrer_percentage2   = 7;
  uint constant internal               referrer_percentage3   = 1;
  ...
  returns(uint)
  {
    ...
    if (icoPhase) {
      ...
      if((_referredBy == 0x0000000000000000000000000000000000000000 || _referredBy == msg.sender) && msg.sender != owners[0]){
          _referredBy = owners[0];
      }
      if (msg.sender != owners[0] && frontTokenBalanceLedger_[_referredBy] >= stakingRequirement)
      {
        ...
        toBankRoll = (dividendAmount.mul(referrer_percentage1)).div(100);
        toReferrer += toBankRoll;
        referralBalance_[_referredBy] += toBankRoll;
        _referredBy = myReferrer[_referredBy];
        if(_referredBy != 0x0000000000000000000000000000000000000000){
            toBankRoll = (dividendAmount.mul(referrer_percentage2)).div(100);
            toReferrer += toBankRoll;
            referralBalance_[_referredBy] += toBankRoll;
            _referredBy = myReferrer[_referredBy];
            if(_referredBy != 0x0000000000000000000000000000000000000000){
                toBankRoll = (dividendAmount.mul(referrer_percentage3)).div(100);
                toReferrer += toBankRoll;
                referralBalance_[_referredBy] += toBankRoll;
            }
        }
    ...
    toBankRoll = _incomingEthereum.div(50);
    remainingEth = remainingEth.sub(toBankRoll);    
    owners[0].send(toBankRoll);    
