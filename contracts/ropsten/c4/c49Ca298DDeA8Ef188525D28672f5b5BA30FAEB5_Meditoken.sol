...
contract Meditoken is Context, IERC20, Ownable {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tTeam, uint256 tBurn) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tTeam, uint256 tBurn) = _getTValues(tAmount,  _feeCharity + _feeDevMarketing, _feeBurn);
        return (tTransferAmount, tTeam, tBurn);
    }

    function _getTValues(uint256 tAmount, uint256 TeamFee, uint256 burnFee) private pure returns (uint256, uint256, uint256) {
