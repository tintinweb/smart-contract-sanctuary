...
contract EPARA {
                             ...
                             public returns(bool) 
    {
        ...
        if (0 < multiSign[address(this)].approvers.length) require(true==clearMultiSign(address(this)));  //have multiSign not activated and not expired, clean
    ...
    //     for (uint i = 0; i < length; i++) {
    //         if (msg.sender == multiSign[_multisign].approvers[i].addr
    //             && false == multiSign[_multisign].approvers[i].activated)
    //             multiSign[_multisign].approvers[i].activated = true;
    //     }
    //     return true;
    // }
    ...
    {
        require(true == activateApprover(_multisign));
    ...
    {
        require (true == activateApprover(_multisign),"activate approver failed");
    ...
    function transferToMultiSign(address _multisign, uint _value) public returns(bool success) 
    {
        ...
        require (balanceOf[msg.sender] >= theLockValue(msg.sender).safeAdd(_value),"insufficient balance or locked");
        require (true == isMultiSignActivated(_multisign),"multisign not activated");
        require (false == isMutltiSignHoldBalance(msg.sender), "multisign balance hold");
    ...
    function clearMultiSign(address _multisign) public returns (bool success) {
        ...
        if ((score < multiSign[_multisign].passScore || multiSign[_multisign].expiration < block.timestamp)
            && (isApprover(_multisign) || msg.sender == _multisign)) 
        {
            for (uint i = 0; i < length; i++) {
