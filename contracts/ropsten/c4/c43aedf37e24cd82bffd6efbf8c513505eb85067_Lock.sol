...
contract Lock is Ownable {
    ...
    // mapping indicating if the token is eligible to be accepted as fees
    ...
    event FeeTokensUpdated(address _feeTokenAddress, bool _isEligible);
    event AssetLocked(
    ...
    modifier canLockAsset(address token) {
        ...
        require(
            _tokens[index - 1].status == TokenStatus.ACTIVE,
            "Lock: Token not active!!"
        );

        require(
            !_tokens[index - 1].emergencyUnlock,
    ...
    modifier canClaim(uint256 id) {
        ...
        require(
            _idVsLockedAsset[id].lockOwner == msg.sender,
            "Lock: Only Lock owner can claim asset"
        );
    ...
    modifier _checkValidValue(
        ...
        if (amountinEth && feeToken == ETH_ADDRESS) {
            require(
                value >= amount + fee,
                "Lock: Value sent less than required."
            );
        } else if (amountinEth && feeToken != ETH_ADDRESS) {
    ...
    constructor(address payable wallet, address feeToken) {
        ...
        isFeeToken[feeToken] = true;
        isFeeToken[ETH_ADDRESS] = true;
    }
        ...
        )
    {
        ...
        if (index > 0) {
            ...
            return (token.minAmount, token.emergencyUnlock, token.status);
        }
    }
        ...
        )
    {
        ...
        return (token, amount, startDate, endDate, lastLocked, status, owner);
    }
     ...
     * @param _feeTokenAddress Fees token address for locking
     * @param _isEligible If the given token is eligible to be accepted as fee
     */
    function setFeeToken(address _feeTokenAddress, bool _isEligible)
        external
        onlyOwner
    {
        isFeeToken[_feeTokenAddress] = _isEligible;
        emit FeeTokensUpdated(_feeTokenAddress, _isEligible);
    }
    ...
    function updateToken(
        ...
        uint256 index = _tokenVsIndex[tokenAddress] - 1;
        Token storage token = _tokens[index];
        token.minAmount = minAmount;
        token.emergencyUnlock = emergencyUnlock;

        emit TokenUpdated(index, tokenAddress, minAmount, emergencyUnlock);
    }
     ...
     */
    function addToken(address token, uint256 minAmount)
        public
        onlyOwner
    ...
    /**
     * @dev Allows admin to add new tokens to the list
     * @param tokens Address of the tokens
     * @param minAmounts Minimum amount of tokens to lock for this token
     */
    function addTokens(address[] calldata tokens, uint256[] calldata minAmounts)
        external
    {
        require(
            tokens.length == minAmounts.length,
            "Lock: Length is unequal!!"
        );
        require(tokens.length != 0, "Lock: No token provided!!");

        for (uint256 index = 0; index < tokens.length; index++) {
            addToken(tokens[index], minAmounts[index]);
        }
    }
    ...
    function inactivateToken(address token)
        ...
        tokenExist(token)
    {
        uint256 index = _tokenVsIndex[token] - 1;
    ...
    function addAmount(
        ...
        addCheckedAmount(amount, fee, feeToken, tokenAddress, msg.value);

        lockedAsset.amount = lockedAsset.amount + amount;
        lockedAsset.lastLocked = block.timestamp;

        emit AmountAdded(msg.sender, id, amount);
    }

    function addCheckedAmount(
        uint256 amount,
        uint256 fee,
        address feeToken,
        address tokenAddress,
        uint256 value
    ) private _checkValidValue(value, amount, tokenAddress, fee, feeToken) {
        if (ETH_ADDRESS == tokenAddress) {
            _transferFee(feeToken, fee);
        } else {
            _transferFee(feeToken, fee);
    ...
    function claimable(uint256 id) public view returns (bool) {
        ...
        if (
            ...
            (asset.endDate <= block.timestamp ||
                _tokens[_tokenVsIndex[asset.token] - 1].emergencyUnlock)
        ) {
            return true;
        }
        return false;
    ...
    function isActive(address token) public view returns (bool) {
        ...
        if (index > 0) {
            return (_tokens[index - 1].status == TokenStatus.ACTIVE);
        }
        return false;
    }
    ...
    function _lock(
        ...
        Token memory token = _tokens[_tokenVsIndex[tokenAddress] - 1];
        uint256 newAmount = 0;
        require(
        ...
        bool feesInEth = feeToken == ETH_ADDRESS;

        if (ETH_ADDRESS == tokenAddress) {
            newAmount = value - fee;
            _lockETH(amount, fee, feeToken, endDate, value);

            if (feesInEth) {
                remValue = remValue - (amount + fee);
            } else {
                remValue = remValue - (amount);
            }
        } else {
            if (feesInEth) {
                remValue = remValue - (fee);
            }
            _lockERC20(tokenAddress, amount, fee, feeToken, endDate, value);
        }
    ...
    function _lockETH(
        ...
        uint256 endDate,
        uint256 value
    ) private _checkValidValue(value, amount, ETH_ADDRESS, fee, feeToken) {
    ...
    function _lockERC20(
        ...
        uint256 endDate,
        uint256 value
    ) private _checkValidValue(value, amount, token, fee, feeToken) {
    ...
    {
        feeCalculated = (amount * (lockingFee)) / (10000);
    }

    function _transferFee(address feeToken, uint256 fee) private {
        require(isFeeToken[feeToken], "Lock: Please provide valid fee token!!");

        if (feeToken == ETH_ADDRESS) {
            (bool success, ) = _wallet.call{value: fee}("");
            require(success, "");
        } else {
            IERC20(feeToken).safeTransferFrom(msg.sender, _wallet, fee);
        }
    }
}