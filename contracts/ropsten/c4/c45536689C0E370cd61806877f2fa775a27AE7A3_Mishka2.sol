...
contract Mishka2 is Context, ERC20, Ownable {
    ...
    modifier transferable(
        ...
        if (!m_WhiteList[_recipient]) {
            ...
            if (m_CoolDownSeconds > 0) {
                require(
                    m_Cooldown[_sender] < block.timestamp,
    ...
    function addBlackListMultiple(address[] memory _addresses)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    function setNumOfTokensForDisperse(uint256 _numOfTokensForDisprese)
        external
        onlyOwner
    {
        m_NumOfTokensForDisperse = _numOfTokensForDisprese.mul(10**18);
    }

    function getNumOfTokensForDisprese() external view returns (uint256) {
