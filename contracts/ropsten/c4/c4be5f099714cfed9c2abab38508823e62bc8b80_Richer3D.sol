...
contract Richer3D {
    ...
    function getBounsWithRoundID(uint256 _roundID) private view returns(uint256 _bouns) {
        ...
        DataModal.RoundInfo storage roundInfo = rInfoXrID[_roundID];
        for(uint256 d=1;d<=roundInfo.totalDay;d++){
            DataModal.DayInfo storage dayinfo = rInfoXrID[_roundID].dayInfoXDay[d];
            for(uint256 i=1;i<=dayinfo.playerNumber;i++) {
                uint256 amount = dayinfo.amountXIndex[i];
                _bouns = _bouns.add(amount.mul(891).div(1000));  
            }
            for(uint256 j=1;j<=roundInfo.number;j++) {
                address address2 =roundInfo.addressXnumber[j];
                if(d>=2) {
    ...
    function getIncreaseBalance(uint256 _dayID,uint256 _roundID) private view returns(uint256 _balance) {
        DataModal.DayInfo storage dayInfo = rInfoXrID[_roundID].dayInfoXDay[_dayID];
        for(uint256 i=1;i<=dayInfo.playerNumber;i++) {
            uint256 amount = dayInfo.amountXIndex[i];
    ...
    function getMineInfoInDay(address _userAddress,uint256 _roundID, uint256 _dayID) private view returns(uint256 _totalMine,uint256 _myMine,uint256 _additional) {
        for(uint256 i=1;i<=_dayID;i++) {
            DataModal.DayInfo storage dayInfo = rInfoXrID[_roundID].dayInfoXDay[i];
            for(uint256 j=1;j<=dayInfo.playerNumber;j++) {
                address userAddress = dayInfo.addXIndex[j];
                uint256 amount = dayInfo.amountXIndex[j];
                if(_totalMine == 0) {
    ...
    function getBounsEarnings(address _userAddress) private view returns(uint256 _bounsEarnings) {
        for(uint256 i=1;i<roundNumber;i++) {
            ...
            if(winnerDay == 0) {
                ...
                DataModal.DayInfo storage dayInfo = rInfoXrID[i].dayInfoXDay[winnerDay];
                for(uint256 player=1;player<=dayInfo.playerNumber;player++) {
                    address useraddress = dayInfo.addXIndex[player];
                    uint256 amount = dayInfo.amountXIndex[player];
                    if(useraddress == _userAddress) {
    ...
    function getUserP3DDivEarnings(address _userAddress) private view returns(uint256 _myP3DDivide) {
        ...
        for(uint256 i=1;i<roundNumber;i++) {
            ...
            if(p3dDay == 0) {
                ...
                DataModal.DayInfo storage dayInfo = rInfoXrID[i].dayInfoXDay[p3dDay];
                for(uint256 player=1;player<=dayInfo.playerNumber;player++) {
                    address useraddress = dayInfo.addXIndex[player];
                    uint256 amount = dayInfo.amountXIndex[player];
                    if(useraddress == _userAddress) {
