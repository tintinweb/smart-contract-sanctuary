...
contract Dex2 {
  ...
  // Withdraw token (other than ETH) from the contract.
    function withdrawToken(address traderAddr, uint16 tokenCode) external {
        ...
        emit WithdrawEvent(traderAddr, tokenCode, tokens[tokenCode].symbol, uint64(amountE8),exeStatus.lastOperationIndex);
    }
  ...
  // Set information of a token.
    function setTokenInfo(uint16 tokenCode, string symbol, address tokenAddr, uint64 scaleFactor,uint minDeposit) public {
        if (msg.sender != admin) revert();
        if (marketStatus != ACTIVE) revert();
  ...
  //------------------------------ Private Functions: ----------------------------------------------
    ...
    function initiateWithdraw(uint176 tokenAccountKey, uint64 amountE8) private {
        ...
        emit InitiateWithdrawEvent(address(tokenAccountKey), uint16(tokenAccountKey >> 160) /*tokenCode*/,amountE8, pendingWithdrawE8);
    }
    ...
    function setOrders(uint224 orderKey, uint32 pairId, uint8 action, uint8 ioc,uint64 priceE8, uint64 amountE8, uint64 expireTimeSec) private {
    ...
    function matchOrder(uint224 makerOrderKey, Order makerOrder,uint224 takerOrderKey, Order takerOrder) private {
    ...
    // Update orders.
      ...
      // Write orders back to storage.
        setOrders(makerOrderKey, makerOrder.pairId, makerOrder.action, makerOrder.ioc,makerOrder.priceE8, makerOrder.amountE8, makerOrder.expireTimeSec);
        setOrders(takerOrderKey, takerOrder.pairId, takerOrder.action, takerOrder.ioc,takerOrder.priceE8, takerOrder.amountE8, takerOrder.expireTimeSec);

        emit MatchOrdersEvent(address(makerOrderKey), uint64(makerOrderKey >> 160) /*nonce*/,address(takerOrderKey), uint64(takerOrderKey >> 160) /*nonce*/);
    }
    ...
    function parseNewOrder(uint224 orderKey, uint8 v, uint[] body, uint i) private view returns (Order) {
      ...
      // verify the signature of the trader
        ...
        Order memory order;
        order.pairId = uint32(bits); 
        bits >>= 32;
        order.action = uint8(bits); 
        bits >>= 8;
        order.ioc = uint8(bits); 
        bits >>= 8;
        order.priceE8 = uint64(bits); 
        bits >>= 64;
        order.amountE8 = uint64(bits); 
        bits >>= 64;
