...
abstract contract Router {
    ...
    function getBestQuote(address[] memory path, uint256 amountIn, OrderType orderType) public view returns (uint, uint256, uint256[] memory) {
...
contract Degen is Router {
    ...
    constructor(address _tokenAddress, address _degEthPool) Router(_oneSplit, _Uni, _sushi) public {
    ...
    function executeSwap(OrderType orderType, address[] memory path, uint256 assetInOffered, uint256 fees) external payable{
        ...
        (uint dexId, uint256 minAmountExpected, uint256[] memory distribution) = getBestQuote(path, assetInOffered, orderType);
        if(dexId == 0){
            if(orderType == OrderType.EthForTokens) {
                 path[0] = ETH;
            }
