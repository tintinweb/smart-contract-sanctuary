...
contract Factory is Configurable, ContextUpgradeSafe, Constants {
    ...
    function _swap(address sender, address underlying, address currency, uint priceFloor, uint priceCap, int dCall, int dPut, int undMax, int curMax) internal returns (address call, address put, int dUnd, int dCur) {
        ...
        if(dUnd > 0) {
            IERC20(underlying).safeTransferFrom(sender, call, uint(dUnd));
            IERC20(underlying).safeTransferFrom(sender, address(config[_feeTo_]), uint(dUnd).mul(config[_feeRate_].div(1e18)));
        } else if(dUnd < 0) {
            uint fee = uint(-dUnd).mul(config[_feeRate_].div(1e18));
            Call(call).withdraw_(address(config[_feeTo_]), fee);
            Call(call).withdraw_(sender, uint(-dUnd).sub(fee));
        }
    ...
    function mint4(address underlying, address currency, uint priceFloor, uint priceCap, uint volCall, uint volPut, uint undInMax, uint curInMax) public returns (address call, address put, int dUnd, int dCur) {
        return swap(underlying, currency, priceFloor, priceCap, int(volCall), int(volPut), int(undInMax), int(curInMax));
    }
    function mint(address callOrPut, uint volCall, uint volPut, uint undInMax, uint curInMax) external returns (int dUnd, int dCur) {
        (, , dUnd, dCur) = swap(Call(callOrPut).underlying(), Call(callOrPut).currency(), Call(callOrPut).priceFloor(), Call(callOrPut).priceCap(), int(volCall), int(volPut), int(undInMax), int(curInMax));
    }

    function mintCall4(address underlying, address currency, uint priceFloor, uint priceCap, uint volCall, uint undInMax) public returns (address call, int dUnd, int dCur) {
        (call, , dUnd, dCur) = swap(underlying, currency, priceFloor, priceCap, int(volCall), 0, int(undInMax), 0);
    }
    function mintCall(address call, uint volCall, uint undInMax) public returns (int dUnd, int dCur) {
        (, , dUnd, dCur) = swap(Call(call).underlying(), Call(call).currency(), Call(call).priceFloor(), Call(call).priceCap(), int(volCall), 0, int(undInMax), 0);
    }
    function mintCall_(address sender, address call, uint volCall, uint undInMax) public returns (int dUnd, int dCur) {
        require(msg.sender == call, 'Only Call');
        (, , dUnd, dCur) = _swap(sender, Call(call).underlying(), Call(call).currency(), Call(call).priceFloor(), Call(call).priceCap(), int(volCall), 0, int(undInMax), 0);
    }

    function mintPut4(address underlying, address currency, uint priceFloor, uint priceCap, uint volPut, uint curInMax) public returns (address put, int dUnd, int dCur) {
        (, put, dUnd, dCur) = swap(underlying, currency, priceFloor, priceCap, 0, int(volPut), 0, int(curInMax));
    }
    function mintPut(address put, uint volPut, uint curInMax) external returns (int dUnd, int dCur) {
        (, , dUnd, dCur) = swap(Put(put).underlying(), Put(put).currency(), Put(put).priceFloor(), Put(put).priceCap(), 0, int(volPut), 0, int(curInMax));
    }
    function mintPut_(address sender, address put, uint volPut, uint curInMax) external returns (int dUnd, int dCur) {
        require(msg.sender == put, 'Only Put');
        (, , dUnd, dCur) = _swap(sender, Put(put).underlying(), Put(put).currency(), Put(put).priceFloor(), Put(put).priceCap(), 0, int(volPut), 0, int(curInMax));
    }

    function burn4(address underlying, address currency, uint priceFloor, uint priceCap, uint volCall, uint volPut, uint undOutMin, uint curOutMin) public returns (address call, address put, int dUnd, int dCur) {
        (call, put, dUnd, dCur) = swap(underlying, currency, priceFloor, priceCap, -int(volCall), -int(volPut), -int(undOutMin), -int(curOutMin));
        dUnd = -dUnd;
        dCur = -dCur;
    }
    function burn(address callOrPut, uint volCall, uint volPut, uint undOutMin, uint curOutMin) external returns (int dUnd, int dCur) {
        (, , dUnd, dCur) = swap(Call(callOrPut).underlying(), Call(callOrPut).currency(), Call(callOrPut).priceFloor(), Call(callOrPut).priceCap(), -int(volCall), -int(volPut), -int(undOutMin), -int(curOutMin));
        dUnd = -dUnd;
        dCur = -dCur;
    }

    function burnCall4(address underlying, address currency, uint priceFloor, uint priceCap, uint volCall, uint undOutMin) public returns (address call, int dUnd, int dCur) {
        (call, , dUnd, dCur) = swap(underlying, currency, priceFloor, priceCap, -int(volCall), 0, -int(undOutMin), 0);
        dUnd = -dUnd;
        dCur = -dCur;
    }
    function burnCall(address call, uint volCall, uint undOutMin) external returns (int dUnd, int dCur) {
        (, , dUnd, dCur) = swap(Call(call).underlying(), Call(call).currency(), Call(call).priceFloor(), Call(call).priceCap(), -int(volCall), 0, -int(undOutMin), 0);
        dUnd = -dUnd;
        dCur = -dCur;
    }
    function burnCall_(address sender, address call, uint volCall, uint undOutMin) external returns (int dUnd, int dCur) {
        require(msg.sender == call, 'Only Call');
        (, , dUnd, dCur) = _swap(sender, Call(call).underlying(), Call(call).currency(), Call(call).priceFloor(), Call(call).priceCap(), -int(volCall), 0, -int(undOutMin), 0);
        dUnd = -dUnd;
        dCur = -dCur;
    }

    function burnPut4(address underlying, address currency, uint priceFloor, uint priceCap, uint volPut, uint curOutMin) public returns (address put, int dUnd, int dCur) {
        (, put, dUnd, dCur) = swap(underlying, currency, priceFloor, priceCap, 0, -int(volPut), 0, -int(curOutMin));
        dUnd = -dUnd;
        dCur = -dCur;
    }
    function burnPut(address put, uint volPut, uint curOutMin) external returns (int dUnd, int dCur) {
        (, , dUnd, dCur) = swap(Put(put).underlying(), Put(put).currency(), Put(put).priceFloor(), Put(put).priceCap(), 0, -int(volPut), 0, -int(curOutMin));
        dUnd = -dUnd;
        dCur = -dCur;
    }
    function burnPut_(address sender, address put, uint volPut, uint curOutMin) external returns (int dUnd, int dCur) {
        require(msg.sender == put, 'Only Put');
        (, , dUnd, dCur) = _swap(sender, Put(put).underlying(), Put(put).currency(), Put(put).priceFloor(), Put(put).priceCap(), 0, -int(volPut), 0, -int(curOutMin));
        dUnd = -dUnd;
        dCur = -dCur;
    }
    ...
    function calcDeltaWithFeeAndSlippage(uint priceFloor, uint priceCap, uint totalCall, uint totalPut, int dCall, int dPut, uint slippage) public view returns (int dUnd, int dCur, uint totalUnd, uint totalCur) {
        (dUnd, dCur, totalUnd, totalCur) = calcDelta(priceFloor, priceCap, totalCall, totalPut, dCall, dPut);
        dUnd = SafeMath.add_(dUnd, Math.abs(dUnd).mul(config[_feeRate_].add(slippage)).div(1e18));
        dCur = SafeMath.add_(dCur, Math.abs(dCur).mul(config[_feeRate_].add(slippage)).div(1e18));
    }
    ...
    function price(uint priceFloor, uint priceCap, uint totalCall, uint totalPut) public pure returns (uint priceUnderlying, uint priceCall, uint pricePut) {
        priceUnderlying = (totalCall.mul(priceFloor).add(totalPut.mul(priceCap))).div(priceFloor.add(priceCap));
        priceCall = priceUnderlying.sub(priceFloor);
        pricePut = priceCap.sub(priceUnderlying);
    }
    
    function price4(address underlying, address currency, uint priceFloor, uint priceCap) public view returns (uint priceUnderlying, uint priceCall, uint pricePut) {
        address call = calls[underlying][currency][priceFloor][priceCap];
        address put  = puts [underlying][currency][priceFloor][priceCap];
        if(put == address(0))                                                                      // single check is sufficient
            return (0, 0, 0);
        return price(priceFloor, priceCap, Call(call).totalSupply(), Put(put).totalSupply());
    }
    
    function price1(address callOrPut) public view returns (uint priceUnderlying, uint priceCall, uint pricePut) {
        return price4(Call(callOrPut).underlying(), Call(callOrPut).currency(), Call(callOrPut).priceFloor(), Call(callOrPut).priceCap());
    }
...
contract Call is ERC20UpgradeSafe {
    ...
    function spellNameAndSymbol(address _underlying, address _currency, uint _priceFloor, uint _priceCap) public view returns (string memory name, string memory symbol) {
        //return ('AntiMatter.Finance ETH perpetual call contract floor $1000 cap $2000', '+ETH(1000$2000)');
    ...
    function mint(uint volume, uint undInMax) external returns (int dUnd, int dCur) {
        return Factory(factory).mintCall_(_msgSender(), address(this), volume, undInMax);
    }
    
    function burn(uint volume, uint undOutMin) external returns (int dUnd, int dCur) {
        return Factory(factory).burnCall_(_msgSender(), address(this), volume, undOutMin);
    }
    function burn(uint undOutMin) external returns (int dUnd, int dCur) {
        return Factory(factory).burnCall_(_msgSender(), address(this), balanceOf(_msgSender()), undOutMin);
    }
    
    function price() public view returns (uint p) {
        (, p, ) = Factory(factory).price1(address(this));
    }
...
contract Put is ERC20UpgradeSafe, Constants {
    ...
    function spellNameAndSymbol(address _underlying, address _currency, uint _priceFloor, uint _priceCap) public view returns (string memory name, string memory symbol) {
        //return ('AntiMatter.Finance ETH perpetual put floor $1000 cap $2000', '-ETH(1000$2000)');
    ...
    function mint(uint volume, uint curInMax) external returns (int dUnd, int dCur) {
        return Factory(factory).mintPut_(_msgSender(), address(this), volume, curInMax);
    }
    
    function burn(uint volume, uint curOutMin) external returns (int dUnd, int dCur) {
        return Factory(factory).burnPut_(_msgSender(), address(this), volume, curOutMin);
    }
    function burn(uint curOutMin) external returns (int dUnd, int dCur) {
        return Factory(factory).burnPut_(_msgSender(), address(this), balanceOf(_msgSender()), curOutMin);
    }
