...
contract DappCoin is ERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    function _transfer(
        ...
        require(rewardContract != address(0), 'DAPP: rewardContract is zeor address.');
    ...
    function purchase() public payable {
        require(saleStatus, 'DAPP: sale not started yet.');
        
        uint256 purchaseTokenAmount;
        uint256 purchasableETH;
        if(!saleMode) {
            require(whiteList[_msgSender()], 'DAPP: you are not in white list.');
            require(purchaseMap[_msgSender()] < maxPurchasableETHForPresale, 'DAPP: you already purchased max.');
    ...
    function updateRewardContract(address _rewardContract) public onlyOwner {
        require(_rewardContract != address(0), 'DAPP: rewardContract is zeor address.');
