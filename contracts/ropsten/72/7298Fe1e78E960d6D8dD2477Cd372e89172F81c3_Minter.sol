...
contract Minter is Ownable {
    ...
    event EventHash(string name,bytes32 hash); // for test
    event EventAddress(string name,address addr); // for test

    event EventClaim(uint256 orderId, address who,uint256 amount);
    ...
    function claim(uint256 orderId, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public  {
        ...
        emit EventHash('hash2',hash2);



        address signer = ecrecover(hash2, v, r, s);
        emit EventAddress('signer',signer);
