...
contract Deal_Lock {
    ...
    uint locked = 0; //reentrancy guard;  
    uint j = 1; //contract creator ccount 
    struct Agreement {
    ...
    constructor() {
        ...
        j = 1; //contract creator ccount 
    }
    
    
///////////////////////////////////////
///// Functions call by customer //////
///////////////////////////////////////
    ...
    function cancelContract(address payable _creator, uint _contractID) public {
        ...
        if (checkIfBuyerSeller(_creator,_contractID) == WhoIs.SellerId) {
            //answer  = "this is the seller";
            safePay(myAgreements[_creator][_contractID].seller, myAgreements[_creator][_contractID].deal_amount );
            myAgreements[_creator][_contractID].state = State.Completed;
        }
            ...
            //answer  = "this is the buyer";
            uint SPA = myAgreements[_creator][_contractID].seller_protection_percent; //seller protection amaount
            safePay(myAgreements[_creator][_contractID].buyer, ((SPA + 100)*myAgreements[_creator][_contractID].deal_amount)/100 );
            myAgreements[_creator][_contractID].state = State.Completed;
        }
...
///////////////////////////////////////
    ...
    function feesendlockbox(address payable _creator, uint _contractID) internal returns (uint feeamount){
        ...
        //code below takes 0.25% fee for successful transactions
...
///////////////////////////////////////
    ...
    //Complete - function for protocol/contract owner to claim
    ...
    function collectExpired() internal {
        for (uint i=0; i<Agreements.length; i++) {
            if (block.timestamp > (myAgreements[Agreements[i].creator][Agreements[i].contractid].start_date + myAgreements[Agreements[i].creator][Agreements[i].contractid].contract_duration)){
