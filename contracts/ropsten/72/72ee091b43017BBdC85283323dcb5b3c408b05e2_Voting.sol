...
contract Voting {
    ...
    function determineWinner() public returns(string memory name){
        ...
        for(uint8 ppl = 0; ppl < people.length; ppl++) {
            if(people[ppl].voteCount > winningVoteCount) {
                winningVoteCount = people[ppl].voteCount;
                winnerName = people[ppl].name;
            }
        }
