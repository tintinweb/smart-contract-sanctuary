  ...
  */
  function removeFromVIPList(address[] _vipList) onlyOwner public {
   ...
   function ethSendSameValue(address[] _to, uint _value) internal {
        ...
        if(vip){
            require(remainingValue >= sendAmount, "insuficient money");
        }else{
            require(remainingValue >= sendAmount.add(txFee), "insuficient money with fee") ;
        }
		require(_to.length <= 255, "addr length too long");

		for (uint8 i = 1; i < _to.length; i++) {
			remainingValue = remainingValue.sub(_value);
			require(_to[i].send(_value), "failed transact");
		}
