...
interface ITangibleNFT is IERC1155 {
    ...
    /// @dev Function allows a Factory to mint requested amount of specified token ID to the given address.
    function mint(uint256 tokenId, uint256 count, address to) external;

    /// @dev Function allows the Factory to burn requested amount of specified token ID from the given address.
    function burn(uint256 tokenId, uint256 count, address from) external;
    ...
    /// @dev The function returns when the paid storage started.
    function storageStartTime(uint256 tokenId) external view returns (uint256);
    ...
    /// @dev The function accepts one argument — the number of years to pay storage for.
    function payForStorage(uint256 tokenId, uint256 _years) external;
...
interface IFactory {
    ...
    event MintingFeePaid(address indexed payer, address indexed nft, uint256 indexed tokenId, uint256 amount);

    /// @dev The function which does lazy minting.
    function mint(MintVoucher calldata voucher, address to) external;
    ...
    function newCategory(string memory name) external returns (ITangibleNFT);

    /// @dev The function returns an address of category NFT.
    function category(string memory name) external view returns (ITangibleNFT);
}
...
abstract contract AdminAccess is AccessControl {
    ...
    modifier onlyAdmin() {
        require(isAdmin(msg.sender), "NAD");
        _;
    }
