...
contract Ethic_TestNet3 is ERC20, ERC20Detailed,
    ...
    constructor() public ERC20Detailed("Ethic_TestNet3","ETHEST3", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    //Proof-of-Hold and Proof-of-Balances minting protocol layer
...
//--------------------------------------------------------------------------------------
    ...
    function getProofOfBalancesMinting(address account) internal returns (uint256) {
        ...
        lastBlockTime = lastMintedBlockTime[account];
        uint256 balancesAccount = balances[account];
        uint256 totalBlocksTime = blocksTime;
        uint256 stakingRewardRate = baseStakingRate;
        
        //1st - 10th year annual stake reward rate = 200%
        if((now.sub(balanceStakeStartTime)).div(365 days) == 0 && (now.sub(balanceStakeStartTime)).div(365 days) == 9) {
            stakingRewardRate = baseStakingRate.mul(16);
        //11th - 20th year annual stake reward rate = 100%
        } else if((now.sub(balanceStakeStartTime)).div(365 days) == 10 && (now.sub(balanceStakeStartTime)).div(365 days) == 19) {
            stakingRewardRate = baseStakingRate.mul(8);
        //21th - 30th year annual stake reward rate = 50%
        } else if((now.sub(balanceStakeStartTime)).div(365 days) == 20 && (now.sub(balanceStakeStartTime)).div(365 days) == 29) {
            stakingRewardRate = baseStakingRate.mul(4);
        //31th - 40th year annual stake reward rate = 25%
        } else if((now.sub(balanceStakeStartTime)).div(365 days) == 30 && (now.sub(balanceStakeStartTime)).div(365 days) == 39) {
            stakingRewardRate = baseStakingRate.mul(2);
        }
        //41th - end year annual stake reward rate = 12.5%
        uint256 rewardPercentage = stakingRewardRate.div(1e4);
        return balancesAccount.mul(rewardPercentage).mul(epochCount).div(totalBlocksTime);
    }
    
    function balancesMintingInfo(address account) external view returns (address, uint256) {
        uint256 epochCount = now.sub(lastClaimedTime[account]);
