...
contract Bamboo is ERC20("Bamboo Token", "$BAMBOO"), Ownable {
    ...
    uint256 public claimPeriod = 5 minutes; //1 days;
    ...
    function _checkClaimTime(address _holder) internal view returns (bool) {
        if (lastClaimedTime[_holder] + claimPeriod <= now ) {
            return true;
        } else {
            return false;
        }        
    ...
    function claim() external {
       ...
       _lock(msg.sender);              
       uint256 claimAmount = _calcClaimableAmount(msg.sender);          
        _mint(msg.sender, claimAmount);
        emit ClaimedReward(msg.sender, claimAmount);
       _unlock(msg.sender);
    }
    
    function _calcClaimableAmount(address _holder) internal view returns(uint256){
         if (_checkClaimTime(_holder)){
            uint256 claimAmount = PandaNFTContract.balanceOf(_holder).mul(claimUnitPerPeriod.mul((now.sub(lastClaimedTime[_holder])))).div(86400);
            return claimAmount;
        } else return 0;
    }
	
    function getHolderClaimableAmount(address _holder) public view returns(uint256) {
		if (now > claimEndTime) return 0;
		if (!_checkHolder(_holder)) return 0;
        return _calcClaimableAmount(_holder); 
	}
