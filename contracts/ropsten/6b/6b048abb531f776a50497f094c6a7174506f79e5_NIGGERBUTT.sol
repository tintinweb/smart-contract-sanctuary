...
/**
    Knvoy is a blockchain-based platform comprised of decentralized applications (DApps), which are
    designed to make investing easier and safer.
    
    This native DeFi token as a means to facilitate operations and incentivize our users. The token 
    and the platform go hand-in-hand.
    
    Our goal with Knvoy Token is to raise interest and utilize it to help create liquidity for 
    burn and development. So in a sense it's a way to raise money to develop the app. 
    How will this benefit you as the investor? I'm glad you asked.
    
    Knvoy Token will have a total supply of 1 Trillion tokens. Why so much? Because we are going to 
    burn those suckers like crazy. After launch we will begin our MAJOR BUYBACK AND BURNS. What this 
    means is that as Knvoy as a company expands and grows we will begin to take a portion of the 
    quarterly earnings and buy a chunk of Knvoy Tokens with the sole purpose of burning them. This 
    will cause a major price increase during each quarter.
    
    Tokenomics:
    
    Total supply: 1.0 T
    Buy Tax: 3% (Burn / Development)
    Sell Tax: 8% sell tax (3% Reflections, 3% to Burn / Development, 2% to Liquidity)
    
    You can learn more about Knvoy using these links:
    
    Website: https://Knvoy.com/token
    Telegram: https://t.me/Knvoy
    Twitter: https://twitter.com/KnvoyCrypto
    Facebook: https://www.facebook.com/Knvoy
    LP lock: https://Knvoy.com/lplock
*/
...
contract NIGGERBUTT is IBEP20, Ownable {
    ...
    string private constant _name = "NiggerButt";
    string private constant _symbol = "NBT";
    uint8 private constant _decimals = 9;
    uint256 private constant _tTotalSupply = 1000000000000000000000000000; 
	uint256 private constant _buyMaxTxAmount = 10000000000000000000000000; 
    uint256 private constant _sellMaxTxAmount = 1000000000000000000000000;
	
    uint256 private _buyTaxFee = 2;
    uint256 private _buyLiquidityFee = 3;
    uint256 private _buyBurnFee = 3;
    
    uint256 private _sellTaxFee = 2;
    uint256 private _sellLiquidityFee = 3;
    uint256 private  _sellBurnFee = 3;

    uint256 private constant numTokensSellToAddToLiquidity = 500000000 * 10 ** 9; 
    ...
    constructor() {
        ...
        initialTransfer(_msgSender(), address(this) , 850000000000);
    ...
    function excludeFromReward(address account) public onlyOwner() {
        require(account != burnWallet, 'We can not exclude donation and burn wallet.');
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            ...
            _tv.tTransferAmount = 0; _tv.tFee = 0; _tv.tLiquidity = 0; _tv.tBurnFee = 0;
            _rv.rTransferAmount = 0; _rv.rFee = 0; _rv.rLiquidity = 0; _rv.rBurnFee = 0;
            return _rv.rAmount;
        } else {
            (rValues memory _rv, tValues memory _tv) = _getValues(tAmount); 
            _tv.tTransferAmount = 0; _tv.tFee = 0; _tv.tLiquidity = 0; _tv.tBurnFee = 0;
    ...
    function _getValues(uint256 tAmount) private view returns (rValues memory, tValues memory) {
        ...
        rValues memory _rv = _getRValues(tAmount, _tv.tFee, _tv.tLiquidity, _tv.tBurnFee,  _getRate());
        return (_rv, _tv);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (tValues memory) {
        ...
        _tv.tTransferAmount = tAmount.sub(_tv.tFee).sub(_tv.tLiquidity).sub(_tv.tBurnFee);
        return (_tv);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurnFee, uint256 currentRate) private pure returns (rValues memory) {
        rValues memory _rv;
        ...
        _rv.rTransferAmount = _rv.rAmount.sub(_rv.rFee).sub(_rv.rLiquidity).sub(_rv.rBurnFee);
        return (_rv);
    }
