...
contract BlockchainCoursePupils {
    ...
    event addPupilEvent(string, string, string);
    event editPupilEvent(string, string, string);
    ...
    function addPupil(string calldata group, string calldata name,  string calldata yearsOfSchooling)public{
        ...
        if(checkPupilsAddress[msg.sender] == false){
            ...
            emit addPupilEvent(group, name, yearsOfSchooling);           
        }
        else{
            emit editPupilEvent(group, name, yearsOfSchooling);    
        }
    }
