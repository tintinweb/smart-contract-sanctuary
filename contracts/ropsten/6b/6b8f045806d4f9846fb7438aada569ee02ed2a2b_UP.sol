...
contract UP is ERC20UpgradeSafe, OwnableUpgradeSafe {
    
    using SafeCast for int256;
    using SafeMath for uint256;
    using Address for address;
    
    event BurnHappened(uint256 amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(BurnEnabled && LastBurnStamp + BurnInterval >= block.timestamp && _tFeePercent >= _totalSupply.sub(BurnAmount)){
            BurnPoolTokens(BurnAmount);
            LastBurnStamp += 1 minutes;
            BurnAmount += 50e18;
        }
    ...
    function BurnPoolTokens(uint256 amount) internal returns (uint256) {
