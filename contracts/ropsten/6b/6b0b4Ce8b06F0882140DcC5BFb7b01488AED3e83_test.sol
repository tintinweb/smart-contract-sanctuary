...
contract test is IERC721Receiver {
    ...
    function debug_adjust() external {
        require(_owner==msg.sender);
        _summoners_adjust();
    }
        
    function debug_clear() external {
        require(_owner==msg.sender);
        delete summoners;
    }
    
    function debug_push(uint[] memory _ids) external {
       require(_owner==msg.sender);
       for(uint i=0;i<_ids.length;i++){
           summoners.push(_ids[i]);
       }
    }
    
    
    //_count正好=xp0的数量 
    function _summoners_adjust() internal returns (uint _count) {
        uint _newSOF=0; 
        _count=0;
        for(uint _oldSOF=0;_oldSOF<summoners.length;_oldSOF++){
            uint _id=summoners[_oldSOF];
            if(_id==0){
                _count++;
            }else{
                if(_newSOF!=_oldSOF) {
                    summoners[_newSOF++]=summoners[_oldSOF];
                }
            }
        }
    ...
    function summoner_withdraw_xp0(uint _maxcount) external {
        require(_owner==msg.sender);
        require(summoners.length>1);
        
        address _inner=address(this);
        uint _eof=summoners.length;
        uint _count=0;
        //第0位置放金币，永远不撤回
        for(uint i=1;i<_eof;i++){
