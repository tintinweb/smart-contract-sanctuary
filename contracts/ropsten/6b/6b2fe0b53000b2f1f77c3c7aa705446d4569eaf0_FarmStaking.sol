...
contract FarmStaking {
  ...
  function updatePool() public {
    ...
    uint256 multiplier = getMultiplier(farmInfo.lastRewardBlock, block.number);
    uint256 tokenReward = multiplier.mul(farmInfo.blockReward);
    farmInfo.accRewardPerShare = farmInfo.accRewardPerShare.add(tokenReward.mul(1e18).div(tokenSupply));
    farmInfo.lastRewardBlock = block.number < farmInfo.endBlock ? block.number : farmInfo.endBlock;
  }
  ...
  function deposit(uint256 _amount) public {
    ...
    updatePool();
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(farmInfo.accRewardPerShare).div(1e18).sub(user.rewardDebt);
    ...
    user.rewardDebt = user.amount.mul(farmInfo.accRewardPerShare).div(1e18);
