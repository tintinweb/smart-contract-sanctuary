...
contract Masterchef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accDollPerShare;   // Accumulated MMOs per share, times 1e12. See below.
    ...
    function pendingDoll(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDollPerShare = pool.accDollPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ddexxReward = multiplier.mul(ddexxPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accDollPerShare = accDollPerShare.add(ddexxReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accDollPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accDollPerShare = pool.accDollPerShare.add(ddexxReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDollPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeDollTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accDollPerShare).div(1e12);
    ...
    function safeDollTransfer(address _to, uint256 _amount) internal {
