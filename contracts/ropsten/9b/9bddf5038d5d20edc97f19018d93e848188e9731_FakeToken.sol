...
/// @title FakeToken contract - standard ERC20 token with Short Hand Attack and approve() race condition mitigation.
contract FakeToken is SafeMath, ERC20, Ownable {
 string public name = "FakeToken Token";
 string public symbol = "FKK";
 ...
 constructor (address oldTokenAdddress) public {
   ...
   totalSupply = safeAdd(totalSupply, FROZEN_TOKENS);
   balances[owner] = FROZEN_TOKENS;
 }
 ...
 function upgradeTokens(uint amountToUpgrade) public {  
    ...
    require(amountToUpgrade <= allowance(msg.sender, address(this)));   
