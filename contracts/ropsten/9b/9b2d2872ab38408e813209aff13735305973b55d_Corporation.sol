...
contract Corporation {
    ...
    // Superuser authorized to make unilateral changes, such as reassigning
    // tokens when the original real-world owner has lost their keys.
    address superuser;
    ...
    function setSuperuser(address _superuser) external {
        require(
            msg.sender == superuser,
            "Corporation: unauthorized setSuperuser"
        );
        superuser = _superuser;
        emit SuperuserChanged({
            oldSuperuser: msg.sender,
            newSuperuser: _superuser
        });
    }

    function setMetadataURI(string calldata _metadataURI) external {
        require(
            authorizedSuperuser(msg.sender),
    ...
    // components.
    function createSeries(string memory _name, Verifier _verifier)
        external
        returns (
            uint256 _seriesId,
    ...
    function authorizedSuperuser(address _who) public view returns (bool) {
        return _who == superuser;
    }
    ...
    function mayReceive(uint256 _seriesId, address _who)
        external
        view
        returns (bool)
    {
        return series[_seriesId].verifier.mayReceive(_who);
    }
        ...
        external
    {
        require(
            authorizedSuperuser(msg.sender),
            "Corporation: unauthorized authorize"
        );
    ...
    function mint(
        ...
        require(
            series[_seriesId].verifier.mayReceive(_who),
...
contract UnrestrictedStockSeries is ERC20 {
    ...
    constructor() ERC20("UnrestrictedStockSeries", "STOCK") {}
...
contract RestrictedStockSeries is ERC721Enumerable {
    ...
    constructor() ERC721("RestrictedStockSeries", "RSU") {}
        ...
        returns (address _owner, uint256 _amount)
    {
        ...
        require(
            _isApprovedOrOwner(_operator, _tokenId) ||
                corp.authorizedSuperuser(_operator),
            "RestrictedStockSeries: unauthorized burnForUnlock"
        );
    ...
    // only be called by the superuser.
    function reassign(uint256 _tokenId, address _newOwner) external {
        require(
            corp.authorizedSuperuser(msg.sender),
