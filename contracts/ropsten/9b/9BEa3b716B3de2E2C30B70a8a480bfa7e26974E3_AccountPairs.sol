...
contract AccountPairs is Ownable {
    ...
    mapping(string => Record[]) recordsByPaymentMethod;
    ...
    function createRecord(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(_paymentMethod, _userUuid, _internalWalletAddress, _network, _currency)
        );

        require(records[hash].length == 0, "Record already exist");

        records[hash] = _depositAddress;
        recordsByPaymentMethod[_paymentMethod].push(
            Record(
                _paymentMethod,
                _userUuid,
    ...
    function getAllRecordsByPaymentMethod(string memory _paymentMethod)
        external
        view
        returns (Record[] memory)
    {
        return recordsByPaymentMethod[_paymentMethod];
    }
