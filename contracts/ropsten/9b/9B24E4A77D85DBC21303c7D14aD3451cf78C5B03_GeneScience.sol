...
contract GeneScience is IGeneScience {
    ...
    /// @dev return the appearance of genes
    function appearance(uint256 _genes) public pure returns (uint8[12] memory) {
        uint8[12] memory traits;
        for (uint256 i = 0; i < 12; i++) {
            traits[i] = uint8(_sliceNumber(_genes, uint256(5), i * 5 * 4));
        }
        return traits;
    }
    ...
    /// @dev return the attribute of genes
    function attribute(uint256 _genes) public pure returns (uint8[16] memory) {
        uint8[16] memory traits;
        for (uint256 i = 0; i < 16; i++) {
            traits[i] = uint8(_sliceNumber(_genes, uint256(8), (i * 8 * 2) + 8));
        }
        return traits;
    }
