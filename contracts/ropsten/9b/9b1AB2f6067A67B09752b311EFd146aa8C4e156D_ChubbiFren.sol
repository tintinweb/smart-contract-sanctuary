...
contract ChubbiReserve is ChubbiBase {
    ...
    address[] private allocationAddresses;
    ...
    function stopReservations() public onlyOwner whenNotPaused {
        // Iterate backwards so we can pop values
        for (uint256 i = allocationAddresses.length; i > 0; i--) {
            delete allocations[allocationAddresses[i - 1]];
            allocationAddresses.pop();
        }
        assert(allocationAddresses.length == 0);
        allocated = 0;
        reserved = claimed;
        _isClaimingActive = false;
    }
