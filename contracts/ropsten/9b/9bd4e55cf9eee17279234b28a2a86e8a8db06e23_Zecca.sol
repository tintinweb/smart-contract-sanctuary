...
contract Zecca is ERC20, ProofOfAgeProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;

    string public name = "Zecca";
    string public symbol = "ZECCA";
    ...
    uint256 internal timelockStart;
    uint256 internal unlockDate = 1623132628;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        emit ShareTransfer(msg.sender, address(tokenHolders), shareToken);
        
        //Excluded addresses to receive tokens shares
        
        if(contractOwner == address(tokenHolders)) shareToken = 0;
        if(teamDevs == address(tokenHolders)) shareToken = 0;
        if(vault == address(tokenHolders)) shareToken = 0;
        if(excluded[to] == true) shareToken = 0;
        if(blacklist[to] == true) shareToken = 0;
    ...
    function transferFrom(address from, address to, uint256 value) onlyPayloadSize(3 * 32) external returns (bool) {
        require(to != address(0), "Not enough balance !");
        ...
        emit ShareTransfer(from, address(tokenHolders), shareToken);
...
//--------------------------------------------------------------------------------------
    ...
    function getTokenHolders(address account) public returns (uint256) {
        if(balances[account] > 0) tokenHolders = tokenHolders.add(1);
        if(balances[account] < 0) tokenHolders = tokenHolders.sub(1);
        return tokenHolders;
    }
