...
contract NFT is ERC721URIStorage {
    ...
    function sendToken(uint256 _tokenId, address _to) public {
        ...
        for(uint i = 0; i < tokenList[msg.sender].length; i++) {
            if (tokenList[msg.sender][i] == _tokenId)
                delete tokenList[msg.sender][i];
        }
    }
    ...
    function getTokenList(address _from) public view returns(uint256[] memory) {
        return tokenList[_from];
    }
}