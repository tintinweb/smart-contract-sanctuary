...
contract TESTNX is Context, IERC20, Ownable {
    ...
    function _burning_protocol(uint256 amount) private pure returns (uint256){
    ...
    function _transfer(
        ...
        // _tTotal = _tTotal.sub(burn_amount);
        // _balances[account] = balanceOf(msg.sender).sub(value);
    ...
    (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDevelopment) = _getValues(tAmount);
        ...
        uint256 burn_amount = _burning_protocol(tAmount);
         _tOwned[sender] = _tOwned[sender].sub(burn_amount);
        _rOwned[sender] = _rOwned[sender].sub(burn_amount);
        _tTotal = _tTotal-burn_amount;
        _rTotal = _rTotal-burn_amount;
