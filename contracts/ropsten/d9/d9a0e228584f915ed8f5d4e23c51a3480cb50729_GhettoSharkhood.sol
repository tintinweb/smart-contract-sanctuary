...
contract GhettoSharkhood is Context, ERC165, IERC721, IERC721Metadata, Ownable, MinterMintable, GhettoSharkhoodInterface {
     ...
     */
    constructor() {
        _name = "GhettoSharkhood";
        _symbol = "GSH";
        uint256 teamReserved = 200;
        
        for(uint256 tokenId = 1; tokenId <= teamReserved; tokenId ++) {
            _owners[tokenId] = msg.sender;
        }
        _balances[msg.sender] = teamReserved;
        
        _currentTokenId_ = teamReserved;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = GhettoSharkhood.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(GhettoSharkhood.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(GhettoSharkhood.ownerOf(tokenId), to, tokenId);
    }
     ...
     * sequenceId = (tokenId + offset) % totalSupply + 1;
     */
    uint256 _offset_ = 0;
    ...
    string private _placeholderBaseURI_ = "";

    /**
     * This function overrides the function defined in ERC721 contract
     */
    function _baseURI() internal view virtual returns (string memory) {
        return _isRevealed_ ? _baseURI_ : _placeholderBaseURI_;
    }
     ...
     */
    function setPlaceholderURI(string memory placeholderBaseURI) public onlyOwner {
        _placeholderBaseURI_ = placeholderBaseURI;
    }
    ...
    // 1 ~ 200 is team reserved token.
    uint256 _currentTokenId_ = 200;
    ...
    modifier enoughToMint(uint256 amount) {
        require(_currentTokenId_ + amount <= _totalSupply_, "GhettoSharkhood: GhettoSharkhoold are all minted.");
        _;
    }
    ...
    function mintSpeficiedAmount(address owner, uint256 amount) internal returns (uint256[] memory) {
        ...
        for(uint256 i = 1; i <= amount; i ++) {
            _safeMint(owner, _currentTokenId_ + i);
            minted_[i - 1] = _currentTokenId_ + i;
        }
    ...
    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, Strings.toString(_sequenceId(tokenId_)), ".json")) : "";
    }
    ...
    function randomNumber() public view virtual returns (uint256) {
