...
contract ShareDefiProtocol is Ownable {
    ...
    uint256 constant BUYTAX_RATE = 200;
    uint256 constant SELLTAX_RATE = 300;
    uint256 constant OWNERTAX_RATE = 100;
    ...
    constructor (address _owner) public{
        _name = "ShareDefi Protocol";
        _symbol = "SHADE";
        _decimals = 18;
        _mint(_owner, 100000*10**uint256(_decimals));
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual returns(uint256) {
        ...
        emit Transfer(recipient, stackingPoolAddress, amountBuyTax);
        remainingAmount = amount-amountBuyTax;
         //1% to owner
        uint256 amountOwnerTax = amount.mul(OWNERTAX_RATE).div(PERCENTS_DIVIDER);
        emit Transfer(sender, _owner, amountOwnerTax);
        remainingAmount = amount-amountOwnerTax;
        ...
        emit Transfer(sender, stackingPoolAddress, amountSellTax);
