...
contract CCXSale is WhitelistDaonomicCrowdsale, RefundableDaonomicCrowdsale {
  ...
  uint256 constant public START = 1538352000; // 01 oct 2018 00:00:00
...
contract Migration {
  event WhitelistCreated(address addr);
  event TokenCreated(address addr);
  event SaleCreated(address addr);
  event HolderCreated(address _address, string name);

  uint256 constant public END = 1549152000; // 3 Feb 2019 00:00:00

  constructor(address _beneficiary, address _operator) public {
    CCXToken token = new CCXToken();
    token.pause();
    emit TokenCreated(token);

    createHolder(token, _beneficiary, "Team", 60000000 * 10 ** 18, END - 365 * 86400, 365 * 86400, 15000000 * 10 ** 18);
    createHolder(token, _beneficiary, "Bounty", 15000000 * 10 ** 18, END, 15 * 86400, 15000000 * 10 ** 18);
    createHolder(token, _beneficiary, "Airdrop", 15000000 * 10 ** 18, END, 15 * 86400, 15000000 * 10 ** 18);
    createHolder(token, _beneficiary, "Advisors", 15000000 * 10 ** 18, END - 182 * 86400, 182 * 86400, 7500000 * 10 ** 18);
    createHolder(token, _beneficiary, "Advertising", 18000000 * 10 ** 18, END, 14 * 86400, 9000000 * 10 ** 18);
    createHolder(token, _beneficiary, "Reserve", 27000000 * 10 ** 18, END, 1, 27000000 * 10 ** 18);

    WhitelistImpl whitelist = new WhitelistImpl();
    whitelist.transferOwnership(_beneficiary);
    emit WhitelistCreated(whitelist);
    CCXSale sale = new CCXSale(token, whitelist, 270, _operator);
    token.transferOwnership(sale);
    sale.transferOwnership(_beneficiary);
    emit SaleCreated(sale);
  }

  function createHolder(CCXToken _token, address _owner, string _name, uint _total, uint _start, uint _vestingInterval, uint _value) internal {
    TokenHolder holder = new TokenHolder(_start, _vestingInterval, _value, _token);
    _token.mint(holder, _total);
    holder.transferOwnership(_owner);
    emit HolderCreated(holder, _name);
  }
}