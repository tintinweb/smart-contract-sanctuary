...
// File: contracts/ERC1155Royalties.sol


pragma solidity ^0.8.0;
contract Onyx1155Royalties{
    uint public onyxFee;
...
contract ERC1155 is Context, ERC165, IERC1155 {
    ...
    //Mapping from Address to TokenID to TokenURI
...
contract Onyx1155 is ERC1155, ERC1155Burnable, Onyx1155Royalties { 
    ...
    function SetNftPrice(address ownerAdd,uint id, uint price) public {  
        //Only Owner can Change Price
        NFT_Price[ownerAdd][id]=price; 
    }
    ...
    */
    function Mint (uint id, uint amount,  bytes memory data, string memory tokenURI, uint NftPrice,uint RoyaltyValueOfMinter ) public {
        require(IsPaused == false, "Contract is Paused");
        //Only owner can Mint again to Increment
        // require(msg.sender==OwnerofNft)
        _mint(_msgSender(), id, amount, data);
        TokenURI[_msgSender()][id] = tokenURI;
        NFT_Price[_msgSender()][id]= NftPrice;
        _setTokenRoyalty(payable(msg.sender),id,payable(msg.sender), RoyaltyValueOfMinter);
    }

    // localy Minted and Want to Mint directlty on Purchaser Address
    // Will Accept Payments For NFTs 
    // Deduct Royalties and Onyx Fee
    ...
    function LocalMintedNfts (address to, uint id, uint amount, bytes memory data, string memory tokenURI, uint NftPrice, address payable MinterAddress, uint RoyaltyValueOfMinter) public payable{
        ...
        _setTokenRoyalty(to, id,MinterAddress, RoyaltyValueOfMinter);
        //Send Amount to Local Minter
        // Deduct Royalties
        _royaltyAndOnyxFee(NftPrice*amount, RoyaltyValueOfMinter, MinterAddress, MinterAddress );
    }
    // Batch Minting Public Function
    // Direct minting on Blockchain 
    function MintBatch(address to, uint[] memory ids, uint[] memory amounts, string[] memory TokenUriArr, bytes memory data, uint[] memory price, uint[] memory RoyaltyValue) public{
        
        require(IsPaused == false, "Contract is Paused");
        require(ids.length == TokenUriArr.length, "TokenURI and Token ID Length Should be Same");
        _mintBatch(to, ids, amounts, TokenUriArr ,data, price, RoyaltyValue );
    }

    //Batch Minting Direct on Blockchain Internal Function
    function _mintBatch(address to,uint256[] memory ids,uint256[] memory amounts,string[] memory Uri,bytes memory data, uint[] memory price, uint[] memory RoyaltyValue) internal virtual {
        require(to != address(0), "ERC1155: mint to the zero address");
        require(ids.length == amounts.length, "ERC1155: ids and amounts length mismatch");
        address operator = _msgSender();
        _beforeTokenTransfer(operator, address(0), to, ids, amounts, data);
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            _balances[ids[i]][to] += amounts[i];
            TokenURI[to][ids[i]] = Uri[i];
            NFT_Price[_msgSender()][ids[i]]= price[i];
            _setTokenRoyalty(to,ids[i], payable(_msgSender()), RoyaltyValue[i]);
        }
        emit TransferBatch(operator, address(0), to, ids, amounts);
    ...
    **  Get NFT price
    */
    function OnyxsafeTransferFrom(address from, address to, uint id, uint amount, bytes memory data ) public payable{
        require(msg.value >= (NFT_Price[from][id])*amount, "Error! insufficient Balance ");
        //require(msg.value >= _deductOnyxFee(NFT_Price[from][id]), "Error! Insufficient Balance");
        _safeTransferFrom(from, to, id, amount, data);
        // struct memory obj = mapping [address][id];
        royaltyInfo memory royalties = _royalties[from][id];
        //"from" is NFT Seller
        _royaltyAndOnyxFee( ((NFT_Price[from][id])*amount), royalties.amount, royalties.recipient, payable(from));
        ...
        //Extra Portion
        _setTokenRoyalty(to, id, royalties.recipient, royalties.amount);
    } 
    ...
    function TokenUri(address Addr, uint id) public view returns(string memory){
        require(bytes(TokenURI[Addr][id]).length != 0, "Token ID Does Not Exist");
        return TokenURI[Addr][id];
    }
