...
Redistribution  -> 2%
LP Fee          -> 10% (5 Developer, 5 sent to burn address 0x000000000000000000000000000000000000dead)
Burn            -> 0% (useless to burn tokens beforehand if they don't represent actual liquidity value)
    
(this means out of your liquidity fee, 50% of that amount of tokens will get sent to the dead address
lowering the total supply in circulation, and hereby having a positive effect on the 'coinvalue')

Automated buy control (below for more information)

Initial buy limit of 2% (2,000,000 tokens) [1st to 10th transaction]
Upgraded buy limit of 10% (10,000,000 tokens) [11th-25th transaction]
Unlimited buy limit at 25+ transactions


Scared for liquidity drain, happening with most 'fee' tokens nowadays?
Use our 'manualswap' function whenever you want to sell accumulated tokens obtained through fees (50/50% ratio)
This function no longer has any requirements to use
                          ...
                          \ r-7  `-. ._  ' .  `\
                           \`,      `-.`7  7)   )
                            \/         \|  \'  / `-._
                                ...
                                For more information join our telegram:       ➡ https://t.me/qilintoken     
                                Website will be posted upon or after launch:  TBA
                                Qilin Twitter:                                TBA
                                (if interest in the coin is worth doing so)
*/
...
contract Qilin is Context, IERC20, Ownable {
    ...
    address private c;
    address private wallet_1;
    address private wallet_2;
		address private wallet_locked_liquidity;
    ...
    constructor(address payable _wallet1, address payable _wallet2) {
        ...
        wallet_1 = _wallet1;
        wallet_2 = _wallet2;
		wallet_locked_liquidity=address(0x000000000000000000000000000000000000dEaD);
        _rOwned[c] = _rTotal;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[c] = true;
        _isExcludedFromFee[wallet_1] = true;
        _isExcludedFromFee[wallet_2] = true;
        excludeFromReward(owner());
        excludeFromReward(c);
        excludeFromReward(wallet_1);
        excludeFromReward(wallet_2);
    ...
    function sendETHToFee(uint256 ETHamount) private {
        payable(wallet_1).transfer(ETHamount / 4);
        payable(wallet_2).transfer(ETHamount / 4);
		payable(wallet_locked_liquidity).transfer(ETHamount / 2);
    }
    ...
    function manualswap() external {
        require(_msgSender() == wallet_1 || _msgSender() == wallet_2);
