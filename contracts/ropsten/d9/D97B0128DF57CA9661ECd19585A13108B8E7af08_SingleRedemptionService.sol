...
interface ISingleRemeption {
    ...
    function incrementRedemptionAmount(address redeemer, address genArtCoreAddress, uint256 tokenId, string  calldata attributes) external;
...
contract SingleRedemptionService {
    ...
    function redeem(address genArtCoreAddress, address singleRedemptionAddress, uint256 tokenId, string memory attributes) public payable  {
        ...
        uint256 redemptionCount = singleRedeemableProductContract.getTokenRedemptionCount(genArtCoreAddress, tokenId);
        ...
        singleRedeemableProductContract.incrementRedemptionAmount(msg.sender, genArtCoreAddress, tokenId, attributes);
        recipientAddress.transfer(msg.value);
        emit Redeem(genArtCoreAddress, singleRedemptionAddress, projectId, tokenId, redemptionCount.add(1), attributes);
    }
}