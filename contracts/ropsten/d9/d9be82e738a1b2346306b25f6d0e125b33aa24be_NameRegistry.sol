...
interface INameRegistry {
  /**
    EVENTS
     */
  event NameSet(address _from, string _newName);
  ...
  /** 
    ...
    @notice successful transaction emits NameSet event
   */
...
library StringUtils {
  ...
  /**
   * @notice checkEmptySpace checks if the string input has trailing white space at the beginning or end
   * @param _stringInput is the string input
   * @return true if there are white spaces at the beginning or end of the string input
   */
  function checkEmptySpace(string memory _stringInput) internal pure returns (bool) {
    bytes memory stringInputToByte = bytes(_stringInput);
    if (stringInputToByte[0] == 0x20 || stringInputToByte[stringInputToByte.length - 1] == 0x20) {
      return true;
    } else return false;
  }

  /**
   * @notice checkEmptyString checks if the string input is empty
   * @param _stringInput is the string input
   * @return true if the input is empty empty
   */
  function checkEmptyString(string memory _stringInput) internal pure returns (bool) {
    if (bytes(_stringInput).length == 0) {
      return true;
    } else return false;
  }
}
...
contract NameRegistry is INameRegistry {
  ...
  //@notice ValidateNameInput validates whether the name input is empty or whether it begins or ends with trailing white space
  //@param _newName name requested for an account
  modifier ValidateNameInput(string memory _newName) {
    require(!_newName.checkEmptyString(), validationErrMsg);
    require(!_newName.checkEmptySpace(), validationErrMsg);
    _;
  }
  ...
  ERROR MESSAGES
   */
  //@notice whiteSpaceErrMsg sets the error message when an empty string is requested for an account
  string private validationErrMsg =
    'Try another name. Account name cannot be empty or have trailing white space at the beginning or end.';
  ...
  /** 
    ...
    @notice successful transaction emits NameSet event
   */
  function setName(string calldata _newName) public override ValidateNameInput(_newName) CheckUniqueName(_newName) {
    _releaseName();
    _setName(_newName);
    emit NameSet(msg.sender, _newName);
  }
