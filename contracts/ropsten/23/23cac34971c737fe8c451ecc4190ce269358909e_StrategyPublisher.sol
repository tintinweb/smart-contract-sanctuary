...
library StrategyLib{
  ...
  *stratPrice: strategy price of the day (taken the previous day)
  ...
  event newBiRecord(bytes15 indexed name,uint32 indexed date,int256 stratVariation, uint64 prevStratPrice, uint64 stratPrice, uint64 truncatedStratPrice, uint64 prevUnderPrice, uint64 underPrice, int64 position, uint64 secondPrevUnderPrice, uint64 secondUnderPrice, int64 secondPosition, int category);
  ...
  -------------------------------------------------------------------------*/
  function libbibatchbacktest(bytes15 name, uint64 strategyInitPrice, uint32[]dates, uint64[2][] prices, int64[2][] positions) public returns (uint64) {
       ...
       for(uint8 i=1; i<dates.length; i++){
           ...
           int256 variation = calculateVariation(prices[0][i-1], prices[0][i], positions[0][i]) + calculateVariation(prices[1][i-1], prices[1][i], positions[1][i]);
           ...
           emit succinctBiRecord(dates[i], truncatedStrategyPrice, prices[0][i-1], prices[0][i], positions[0][i], prices[1][i-1], prices[1][i], positions[1][i],-1);
