...
contract ZWExchange is ExchangableToken, Ownable {
    ...
    event EventFromWei(address indexed _address,
                      uint256 _amount_wei,
                      uint256 token_decimals,
                      uint256 amount);

    event EventToWei(address indexed _address,
                    uint256 _amount,
                    uint256 token_decimals,
                    uint256 amount_wei);
        ...
        returns (bool)
    {
        ...
        uint256 latest_exchange_day = dateTime.getDay(latest_exchange_timestamp);
