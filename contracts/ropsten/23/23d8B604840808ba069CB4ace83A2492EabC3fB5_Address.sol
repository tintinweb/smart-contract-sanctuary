...
contract SafeMoon is Context, IERC20, Ownable {
    ...
    //mangage Transfer
    bool public _transferFlag = true;
    ...
    //record the TransferFlag updating change.
    event TransferFlagUpdated(bool flag);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint tBurn) = _getTValues(tAmount);
    ...
    function _transfer(
        ...
        if (_transferFlag){     //If the _transferFlag is true, transaction will be done.
            _tokenTransfer(from,to,amount,takeFee);
        }
    }
    
    //mangae transfer(Stop or Resume)
    function setTransferFlag(bool flag) public  {
        _transferFlag = flag;
        emit TransferFlagUpdated(flag);
    }
