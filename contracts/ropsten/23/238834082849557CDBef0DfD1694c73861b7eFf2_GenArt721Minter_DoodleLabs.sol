...
contract GenArt721Minter_DoodleLabs {
  ...
  function purchaseTo(address _to, uint256 _projectId, bool isDeferredRefund) internal returns(uint256 _tokenId){
    ...
    if (keccak256(abi.encodePacked(genArtCoreContract.projectIdToCurrencySymbol(_projectId))) != keccak256(abi.encodePacked("ETH"))){
      ...
      _splitFundsETH(_projectId, isDeferredRefund); // culprit
    }
  ...
  function _splitFundsETH(uint256 _projectId, bool isDeferredRefund) internal {
    if (msg.value > 0) {
