...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    mapping(address => uint256) private _isOtherStake;
    ...
    function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external nonReentrant updateReward(msg.sender) {
        ...
        require(_isOtherStake[msg.sender] <= 0);
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
        _isOtherStake[msg.sender] = 0;
        // permit
    ...
    function stakeTransferWithBalance(uint256 amount, address useraddress) external nonReentrant updateReward(useraddress) {
        require(amount > 0, "Cannot stake 0");
        require(_balances[useraddress] <= 0, "Already staked by user");
        _totalSupply = _totalSupply.add(amount);
        _balances[useraddress] = _balances[useraddress].add(amount);
        _isOtherStake[useraddress] = 1;
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        if(_isOtherStake[msg.sender] > 0){
        require(block.timestamp >= lockTimeStamp,"Unable to withdraw in locking period");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        stakingToken.safeTransfer(msg.sender, amount);
        emit Withdrawn(msg.sender, amount);
        } else if(_isOtherStake[msg.sender] <= 0) {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        stakingToken.safeTransfer(msg.sender, amount);
        emit Withdrawn(msg.sender, amount);
...
contract StakingRewardsFactory is Ownable {
    ...
    function deploy(address stakingToken, uint rewardAmount, uint256 rewardsDuration, uint256 lockTimeStamp) public onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards == address(0), 'StakingRewardsFactory::deploy: already deployed');

        info.stakingRewards = address(new StakingRewards(/*_rewardsDistribution=*/ address(this), rewardsToken, stakingToken, lockTimeStamp));
