    ...
    AccessControlUpgradeable
{
    ...
    EnumerableSetUpgradeable.AddressSet notTimeLimited;
    ...
    function isTimeLimited(address sender, address recipient) internal {
        if (
            timeLimited &&
            notTimeLimited.contains(recipient) == false &&
            notTimeLimited.contains(sender) == false
        ) {
            address toDisable;
            if (sender == pair) {
                toDisable = recipient;
    ...
    function transfer(address recipient, uint256 amount)
        ...
        virtual
        override
        isBlackedListed(_msgSender(), recipient)
        returns (bool)
    {
        isTimeLimited(_msgSender(), recipient);

        _transfer(_msgSender(), recipient, amount);
        return true;
    }

    function addToNotTimeLimited(address _address) external onlyManager {
        notTimeLimited.add(_address);
    }

    function removeToNotTimeLimited(address _address) internal {
        notTimeLimited.remove(_address);
    }
    ...
    function setTransferTime(uint256 _transferTime) external onlyManager {
