...
contract TokenDispatch is Management {
    ...
    constructor() public {
        _youToken = 0x1093EcdBACa4168F136f3BEfd17a261bfbbeDdA3;
    ...
    function approveTeam(address account,address agent, uint256 approved) external lock onlyOwner {
        ...
        _teams[_teamId].reserved = approved;
    ...
    function nextClaimHeightOf(uint8 teamId) external view returns (uint256) {
        require(_teams[teamId].approved, 'YouSwap: NOT_EXIST');
        return _teams[teamId].lastClaimedHeight + _blocksOfMonth;
    }
    ...
    function claim(uint8 teamId) external lock {
        ...
        require(totalClaimed <= totalSupplyForTeam, 'YouSwap: EXCEEDS_THE_UPPER_LIMIT');
