...
interface IMerkleDistributor {
    // Returns the first block number where the users can claim the rewards.
    function startBlockNumber() external view returns (uint256);
    // Returns the last block number where the users can claim the rewards.
    function endBlockNumber() external view returns (uint256);
    ...
    function claim(uint256 index, address account, uint256 tokenType, uint256 amount, bytes32[] calldata merkleProof) external;
    // Burn all the non-claimed tokens after the claimable period.
    function burnAll(uint256 tokenType) external;
    // This event is triggered whenever a call to #claim succeeds.
    event Claimed(uint256 indexed index, address indexed account, uint256 tokenType, uint256 amount);
    // This event is triggered whenever a call to #burnAll succeeds.
    event Burnt(uint256 indexed tokenType, uint256 indexed amount);
}
