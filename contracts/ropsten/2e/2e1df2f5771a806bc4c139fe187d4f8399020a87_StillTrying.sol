...
contract StillTrying{
    address public owner;
    address public receiver;
    address ledger;
    uint AllowanceDay = 10000000000000000 wei;
    uint dayCooldown;
    uint64 coolDownTime = 300 seconds;
  ...
  function getUserAllowance(address ins) view public returns (uint) {
      if (Users[ins].userCoolDown <= now){
          Users[ins].userAllowance = AllowanceDay;
      }
      else{
          Users[ins].userAllowance;
      }
  ...
  function withdraw(uint _Amount) public{
        ...
        if (User.userLogin == false){
          Login();
            ...
            require(now >= User.userCoolDown && address(this).balance >= _Amount && User.userAllowance >= _Amount);
            //We have to make sure the withdrawal is subtracted from the daily allowance.
            User.userAllowance -= _Amount;
