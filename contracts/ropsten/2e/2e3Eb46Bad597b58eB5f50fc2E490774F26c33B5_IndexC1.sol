...
contract IndexC1 is Ownable {
  ...
  function exitMarket() public {
    ...
    for (uint i = 0; i < allocationBalances[msg.sender].length; i++) {
      ...
      uint[] memory returnedAmounts = getAmountOut(path, amountInO);
      
      IERC20 token = IERC20(tokenAddress);
      token.approve(UNISWAP_ROUTER_ADDRESS, amountInO);
      ...
      emit SwapInit(tokenAddress, amountInO, amountInE, returnedAmounts);
