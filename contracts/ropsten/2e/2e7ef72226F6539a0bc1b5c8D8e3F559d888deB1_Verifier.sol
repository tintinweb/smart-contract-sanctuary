...
contract Taxi is Initializable {
  using SafeMath for uint256;
  using BytesLib for bytes;

  // The addresses which can submit claims.
  // For large guilds, 100 transactions per day may not be enough
  mapping(address => bool) private _claimers;

  // ISLPContract SLP = ISLPContract(0xa8754b9Fa15fc18BB59458815510E40a12cD2014); // real slp
  ISLPContract private constant SLP =
    ISLPContract(0x15487e4Fc8dB4A1Ec83c3BF2d64B0468001123C8); // test slp

  // optional.
  // By default, any scholar payout address specified by the claimer is allowed.
  // In practice, this means a malicious claimer could pass in their own address when making a claim.
  ...
  modifier onlyClaimers() {
    require(_claimers[msg.sender], "Not a verified claimer");
    _;
  }

  constructor(address _guildAddress) {
  ...
  function initialize(
    address _feeAddress,
    uint256 _fee,
    address[] memory additionalClaimers
  ) public initializer {
    feeAddress = _feeAddress;
    fee = _fee;

    _claimers[guildAddress] = true;
    _claimers[feeAddress] = true;
    for (uint256 i = 0; i < additionalClaimers.length; i++) {
      _claimers[additionalClaimers[i]] = true;
    }
  }
  ...
  function onApprovalReceived(
    ...
    scholarPayoutAddress[owner] = data.toAddress(0);
  ...
  // just to make slightly easier to test via GUI, can remove later
  function makeClaim(
    address owner,
    uint256 amount,
    uint256 createdAt,
    ...
    uint256 newBalance = SLP.checkpoint(owner, amount, createdAt, signature);
    uint256 newSlp = newBalance - prevBalance;
    uint256 feeAmount = newSlp.mul(fee).div(100); // todo: choose rounding direction
    ...
    SLP.transferFrom(owner, scholar, scholarAmount);
  }

  function makeClaims(Claim[] memory claims) public onlyClaimers {
  ...
  function addClaimer(address claimer) public onlyClaimers {
...
interface ISLPContract {
  ...
  function approveAndCallPayable(
    address _spender,
    uint256 _value,
    bytes memory _data
  ...
  function cappedSupply() external view returns (uint256);
  ...
  function checkpointAndCall(
    ...
    bytes memory _data
  ) external payable returns (uint256 _balance);

  function dailyPotionLimit() external view returns (uint256);

  function decimals() external view returns (uint8);

  function getCheckpoint(address _owner)
    external
    view
    returns (uint256 _amount, uint256 _createdAt);
  ...
  function removeMinters(address[] memory _removedMinters) external;

  function setDailyPotionLimit(uint256 _dailyPotionLimit) external;
