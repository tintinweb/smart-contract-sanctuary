...
interface IEquipment {
    ...
    event ItemCreated(uint indexed itemId, string name, uint16 maxSupply, EquipmentSlot slot, Rarity rarity);
    event ItemUpgradable(uint indexed itemId, uint indexed nextTierItemId, uint8 upgradeAmount);

    /**
     * @notice Create an item.
     */
    function createItem(string memory name, uint16 maxSupply, EquipmentSlot slot, Rarity rarity) external;
     ...
     */
    function rollEquipmentGacha(uint vendorId, uint amount) external payable;
     ...
     */
    function putItemsIntoStorage(address account, uint256[] memory itemIds) external;
