...
contract MahaspunkParty is Market, Ownable, ReentrancyGuard {
    ...
    constructor( address _teamWallet, uint _reserveRemain) public Market("MahasPunk Party", "MSP") {
        require(_reserveRemain <= (MAX_SUPPLY.sub(Max_PreMint.add(Max_Team))), "constructor :: reserveRemain exceed the max limit");
        isActive = false;
        isPreMintActive = false;
        price = 0.06e18;
        teamWallet = _teamWallet;
        reserveRemain = _reserveRemain;
        Max_Mint = MAX_SUPPLY.sub(Max_PreMint.add(Max_Team.add(reserveRemain)));
    }
    ...
    function mint(uint256 qty) public payable nonReentrant {
        ...
        require(qty <= Max_Mint, "qty>Max_Mint");
        require(msg.value == price.mul(qty), "Invalid ether for minting.");
    ...
    function teamMint(uint256 qty) public payable nonReentrant {
        ...
        require(isActive,"team mint inactive!");
        require(qty <= 100, "qty>$(MAX_QTY)");
        require((totalSupply() + qty) <= (MAX_SUPPLY), "qty>supply");
