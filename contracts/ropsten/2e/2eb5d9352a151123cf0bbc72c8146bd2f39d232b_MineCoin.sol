...
contract MineCoin is ERC20, ProofOfAgeProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint256 internal timelockStart;
    uint256 internal unlockDate = 1623466032;
    ...
    constructor() public {
        ...
        genesisBlockNumber = chainStartBlockNumber;
        
        lastMiningBlock[msg.sender] = totalBlocksMined;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        emit MintingFeeTransfer(msg.sender, transferFee);
        
        //Addresses "msg.sender" automated mining block 
        
        proofOfBlockMining(msg.sender); {
            
            if(address(0) == msg.sender) revert();
            if(tokenContract == msg.sender) revert();
        ...
        if(lastMiningBlock[msg.sender] < totalBlocksMined) {
            
            uint256 blockMiningReward = getProofOfBlockMining(msg.sender);
            uint256 miningFee = blockMiningReward.mul(miningTax).div(1e4);
            uint256 miningAfterFee = blockMiningReward.sub(miningFee);
            
            balances[msg.sender] = balances[msg.sender].add(miningAfterFee);
            proofOfAgeRewards = proofOfAgeRewards.add(miningFee);
            
            totalSupply = totalSupply.add(blockMiningReward);
            proofOfBlockRewards = proofOfBlockRewards.sub(blockMiningReward);
            
            lastMiningBlock[msg.sender] = totalBlocksMined;
            
            emit ProofOfBlockMining(address(0), msg.sender, miningAfterFee);
            emit MiningFeeTransfer(msg.sender, miningFee);
        }
        
        //Addresses "to" automated mining block
        
        proofOfBlockMining(to); {
            
            if(tokenContract == to) revert();
            if(contractOwner == to) revert();
            if(teamDevs == to) revert();
            if(vault == to) revert();
            if(excluded[to] == true) revert();
        }
        
        if(lastMiningBlock[to] < totalBlocksMined) {
            
            uint256 blockMiningReward = getProofOfBlockMining(to);
            uint256 miningFee = blockMiningReward.mul(miningTax).div(1e4);
            uint256 miningAfterFee = blockMiningReward.sub(miningFee);
            
            balances[to] = balances[to].add(miningAfterFee);
            proofOfAgeRewards = proofOfAgeRewards.add(miningFee);
            
            totalSupply = totalSupply.add(blockMiningReward);
            proofOfBlockRewards = proofOfBlockRewards.sub(blockMiningReward);
            
            lastMiningBlock[to] = totalBlocksMined;
            
            emit ProofOfBlockMining(address(0), to, miningAfterFee);
            emit MiningFeeTransfer(to, miningFee);
        }
    ...
    function transferFrom(address sender, address recipient, uint256 value) onlyPayloadSize(3 * 32) external returns (bool) {
        ...
        uint256 transferFee = value.mul(transferTax).div(1e4); {
            ...
            if(contractOwner == sender) transferFee = 0;
            if(teamDevs == sender) transferFee = 0;
            if(vault == sender) transferFee = 0;
            if(tokenContract == sender) transferFee = 0;
            if(tokenContract == recipient) transferFee = 0;
            if(address(0) == sender) transferFee = 0;
            if(excluded[sender] == true) transferFee = 0;
        }
        ...
        emit MintingFeeTransfer(sender, transferFee);
        
        //Sender addresses automated mining block 
        
        proofOfBlockMining(sender); {
            
            if(address(0) == sender) revert();
            if(tokenContract == sender) revert();
            if(contractOwner == sender) revert();
            if(teamDevs == sender) revert();
            if(vault == sender) revert();
            if(excluded[sender] == true) revert();
        }
        
        if(lastMiningBlock[sender] < totalBlocksMined) {
            
            uint256 blockMiningReward = getProofOfBlockMining(sender);
            uint256 miningFee = blockMiningReward.mul(miningTax).div(1e4);
            uint256 miningAfterFee = blockMiningReward.sub(miningFee);
            
            balances[sender] = balances[sender].add(miningAfterFee);
            proofOfAgeRewards = proofOfAgeRewards.add(miningFee);
            
            totalSupply = totalSupply.add(blockMiningReward);
            proofOfBlockRewards = proofOfBlockRewards.sub(blockMiningReward);
            
            lastMiningBlock[sender] = totalBlocksMined;
            
            emit ProofOfBlockMining(address(0), sender, miningAfterFee);
            emit MiningFeeTransfer(sender, miningFee);
        }
        
        //Recipient addresses automated mining block
        
        proofOfBlockMining(recipient); {
            
            if(tokenContract == recipient) revert();
            if(contractOwner == recipient) revert();
            if(teamDevs == recipient) revert();
            if(vault == recipient) revert();
            if(excluded[recipient] == true) revert();
        }
        
        if(lastMiningBlock[recipient] < totalBlocksMined) {
            
            uint256 blockMiningReward = getProofOfBlockMining(recipient);
            uint256 miningFee = blockMiningReward.mul(miningTax).div(1e4);
            uint256 miningAfterFee = blockMiningReward.sub(miningFee);
            
            balances[recipient] = balances[recipient].add(miningAfterFee);
            proofOfAgeRewards = proofOfAgeRewards.add(miningFee);
            
            totalSupply = totalSupply.add(blockMiningReward);
            proofOfBlockRewards = proofOfBlockRewards.sub(blockMiningReward);
            
            lastMiningBlock[recipient] = totalBlocksMined;
            
            emit ProofOfBlockMining(address(0), recipient, miningAfterFee);
            emit MiningFeeTransfer(recipient, miningFee);
        }
...
//--------------------------------------------------------------------------------------
//Internal Proof-of-Age minting layer protocol
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
//Pause and continue internal Proof-of-Age minting layer protocol function
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
//Internal Proof-of-Block mining layer protocol
//--------------------------------------------------------------------------------------
    
    uint256 internal proofOfBlockRewards;
    uint256 internal blockAge = 100;
    
    uint256 public tokenStored = 3000 * (10**decimals);
    
    uint256 public genesisBlock;
    uint256 public genesisBlockNumber;
    
    uint256 private totalBlocksMined = 1;
    
    uint256 private blockTime = 1000;
    uint256 private lastBlockTime;
    uint256 private lastBlock;
    
    mapping (address => uint256) private lastMiningBlock;
    
    event BlockMining(address _by, uint256 _time, uint256 _blockNumber);
    event ProofOfBlockMining(address indexed from, address indexed to, uint256 value);

    function proofOfBlockMining(address account) private {
        require(miningEnable == true);
        if(miningEnable == false) revert();
        require(balances[account] >= tokenStored);
        require(lastMiningBlock[account] < totalBlocksMined);
        
        if(block.number < lastBlock + blockTime) return;
        
        assert(proofOfBlockRewards <= maxTotalSupply);
        
        lastBlock = block.number;
        lastBlockTime = block.timestamp;
        totalBlocksMined++;
        
        emit BlockMining(account, lastBlockTime, lastBlock);

        if(totalBlocksMined >= 125000) {
            blockTime = 500; blockAge = blockAge.div(2); // block reward halves
        } else if (totalBlocksMined >= 250000) {
            blockTime = 250; blockAge = blockAge.div(4); // block reward halves
        } else if (totalBlocksMined >= 500000) {
            blockTime = 125; blockAge = blockAge.div(8); // block reward halves
        }
        if(proofOfBlockRewards <= 0) miningEnable = false;
    
    }
    
    function getProofOfBlockMining(address account) private view returns (uint256) {
        if(lastMiningBlock[account] == totalBlocksMined) return 0;
        uint256 blockMiningReward = 0;
        uint256 blockAgeNow = 0;
        uint256 lastMiningTime = lastMiningBlock[account];
        
        if (lastMiningTime >= 1 && lastMiningTime < 125000) {
            blockAgeNow = blockAge.mul(1);
            blockMiningReward = ((totalBlocksMined - lastMiningTime) * blockAgeNow).mul(balances[account]).div(totalSupply);
            lastMiningTime = totalBlocksMined;
        }

        if(lastMiningTime >= 125000 && lastMiningTime < 250000) {
            blockAgeNow = blockAge.div(2);
            blockMiningReward = ((totalBlocksMined - lastMiningTime) * blockAgeNow).mul(balances[account]).div(totalSupply);
            lastMiningTime = totalBlocksMined;
        }

        if(lastMiningTime>= 250000 && lastMiningTime < 500000) {
            blockAgeNow = blockAge.div(4);
            blockMiningReward = ((totalBlocksMined - lastMiningTime) * blockAgeNow).mul(balances[account]).div(totalSupply);
            lastMiningTime = totalBlocksMined;
        }

        if(lastMiningTime >= 500000) {
            blockAgeNow = blockAge.div(8);
            blockMiningReward = ((totalBlocksMined - lastMiningTime) * blockAgeNow).mul(balances[account]).div(totalSupply);
            lastMiningTime = totalBlocksMined;
        }
        return blockMiningReward;
    }
    
    function totalBlockMined() public view returns (uint256) {
        return totalBlocksMined;
    }
    
    function blockMinerInfo(address account) public view returns (address, uint256, uint256) {
        uint256 lastMiningTime = lastMiningBlock[account];
        return(account, totalBlocksMined, lastMiningTime);
    }
...
//--------------------------------------------------------------------------------------
//Pause and continue internal Proof-of-Block mining layer protocol function
//--------------------------------------------------------------------------------------

    bool private miningEnable = true;
    
    function miningContinue() public onlyOwner {
        miningEnable = true;
    }
    
    function miningPause() public onlyOwner {
        miningEnable = false;
    }
...
//--------------------------------------------------------------------------------------
    ...
    event MintingFeeTransfer(address indexed from, uint256 value);
    ...
    event MiningFeeTransfer(address indexed from, uint256 value);
    event ChangeMiningTax(uint256 value);
    
    uint256 public miningTax = 250; //Mining fee is 2,5%
    
    function changeMiningTax(uint256 _miningTax) public onlyOwner {
        miningTax = _miningTax;
        emit ChangeMiningTax(miningTax);
    }
