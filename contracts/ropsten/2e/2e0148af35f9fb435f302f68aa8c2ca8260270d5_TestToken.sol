...
/**
   ...
   # TestToken tokenomics
   
   5% fee Liquidity Pool Auto Add
   5% fee Project Development/Marketing wallet
...
contract TestToken is Context, IERC20, Ownable {
  ...
  function setNumtokentoLiq(uint256 numToken) external onlyOwner() {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateOtherFee(tAmount);
    ...
    function removeAllFee() private {
        if(_otherFee == 0 && _liquidityFee == 0) return;
