...
contract ERC721 is Context ,ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    // Create auctions
    function createTokenAndTimeAuction(uint256 startTime,uint256 endTime,uint256 minBidPrice,address tokenAddress) public  {
    ...
    function createTokenAndSimpleAuction(uint256 minBidPrice,address tokenAddress) public  {
        uint256 tokenId = createToken();
    ...
    function createRate(uint256 tokenId,uint256 tokensAmount) public  payable  {
        ...
        if(auction.auctionType == timeAuction ){
            ...
            _setRateToAuction(auction.id,_msgSender(),tokensAmount);
        }
    ...
    function checkPendingAuction(uint256 tokenId) public {
        ...
        if(maxRate.owner != address(0)) {
            ...
            if(auction.tokenAddress == address(0)) {
                ...
                //Send money to old owner
                require(IERC20(auction.tokenAddress).transferFrom(address(this), owner, getQuantityByTotalAndPercent(maxRate.cost,ownerPercent)),"Payment token transfer error.");
                //Send money to system
                require(IERC20(auction.tokenAddress).transferFrom(address(this), _systemAddress, getQuantityByTotalAndPercent(maxRate.cost,systemPercent)),"Payment token transfer error.");
                //Send money to creator
                require(IERC20(auction.tokenAddress).transferFrom(address(this), _creatoAddress, getQuantityByTotalAndPercent(maxRate.cost,creatorPercent)),"Payment token transfer error.");
            }
