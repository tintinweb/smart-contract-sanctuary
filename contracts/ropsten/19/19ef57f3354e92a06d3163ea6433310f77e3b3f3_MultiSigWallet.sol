...
contract MultiSigWallet {
    ...
    uint256 constant internal ERR_CODE_TRANSACTION_EXEC_FAIL = 5;           //钱包转账交易执行失败
    uint256 constant internal ERR_CODE_ADD_EXISTED_OWNER = 6;               //加入一个已存在的成员
    uint256 constant internal ERR_CODE_ADD_OWNER_REACH_UPLIMIT = 7;         //成员已满
    uint256 constant internal ERR_CODE_TRANSFER_NOT_EXIST = 8;              //转账交易不存在
    uint256 constant internal ERR_CODE_TRANSFER_DUPLICATE_CONFIRM = 9;      //转账交易重复确认
    uint256 constant internal ERR_CODE_TRANSACTION_DUPLICATE_EXEC = 10;     //转账交易重复执行
    uint256 constant internal ERR_CODE_TRANSFER_EXPIRED = 11;               //转账交易过期
