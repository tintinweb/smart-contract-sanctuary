  ...
  EIP712MetaTransactionUpgradeable
{
  ...
  event WithdrawnOwnerBalance(address user, uint256 amount);
  ...
  // Next item ID by owner
  mapping(address => uint256) internal nextItemId;

  // Items by owner (userAddress => (itemId => item)
  mapping(address => mapping(uint256 => Item)) internal items;

  // All items IDs of an owner (userAddress => itemIds[])
  mapping(address => uint256[]) internal itemIds;
   ...
   */
  function initialize(address _trustedAddress) public initializer {
  ...
  function withdraw() external onlyOwner {
    ...
    uint256 balance = totalBalance.sub(ownersBalances);
  ...
  function createCampaign(
    ...
    uint256 priceInWei = 15 * (decimals / 10) * _linksAmount; // 1.5MATIC/Link
  ...
  function claim(
    ...
    // Mark as rewarded
    addressRewarded[_campaignManager][_campaignId][_claimer] = true;
    _campaign.linksRewardedCount = _campaign.linksRewardedCount.add(1);
    _campaign.remainingBalance = _campaign.remainingBalance.sub(
      _campaign.amountPerLink
    );
  ...
  function multisend(
    ...
    uint256 pricePerSender = 1000000000000000; // 0.01 MATIC

    require(amountsLength == recipientsLength, "ERR__INVALID_ARGS");
    require(
      msg.value == pricePerSender * recipientsLength,
  ...
  function depositToken(
    ...
    uint256 priceInWei = _price.mul(_amount.div(decimalsZeros)).div(100).mul(2); // 2% fee
  ...
  function buyToken(
    ...
    ownersBalances = ownersBalances.add(msg.value);
  ...
  function getItem(address _owner, uint256 _itemId)
    external
    view
    returns (
      uint256,
  ...
  {
    require(
      items[_owner][_itemId].itemId == _itemId,
      "ERR__ITEM_DOES_NOT_EXIST"
    );

    Item memory _item = items[_owner][_itemId];
    ...
    return (
      ...
      _item.title,
      _item.price,
      _item.initialBalance,
      _item.remainingBalance
    );
  }

  function withdrawOwnerBalance() external {
    address payable owner = msg.sender;
    require(balances[owner] > 0, "ERR_NO_BALANCE_TO_WITHDRAW");
    uint256 toWithdraw = balances[owner];
    balances[owner] = 0;
    ownersBalances = ownersBalances.sub(toWithdraw);
    require(ownersBalances >= 0, "ERR_NO_GENERAL_BALANCE_TO_WITHDRAW");

    owner.transfer(toWithdraw);

    emit WithdrawnOwnerBalance(owner, toWithdraw);
  }
  ...
  function addItem(
    ...
    items[_owner][_itemId] = _item;
    itemIds[_owner].push(_itemId);

    emit ItemAdded(_itemId, _owner, _amount, _price);
  }
  ...
  function buyItem(
    ...
    Item memory _item = items[_owner][_itemId];
    require(_item.remainingBalance > 0, "ERR__NO_ITEMS_AVAILABLE");
    require(
      _item.remainingBalance >= _amount,
      "ERR__NOT_ENOUGHT_ITEMS_AVAILABLE"
    );

    uint256 _totalPrice = _item.price.mul(_amount);
    uint256 _fee = _totalPrice.mul(2).div(1000);
    uint256 _totalPriceWithFee = _totalPrice.add(_fee);
    require(msg.value == _totalPriceWithFee, "ERR__TOTAL_PRICE_MUST_BE_PAID");

    items[_owner][_itemId].remainingBalance = _item.remainingBalance.sub(1);

    uint256 ob = _totalPriceWithFee.sub(_fee);
    balances[_owner] = balances[_owner].add(ob);
    ownersBalances = ownersBalances.add(ob);

    emit ItemBought(_itemId, _owner, msg.sender, _amount);
  }
  ...
  function getCoinviseBalance() external view returns (uint256) {
    uint256 totalBalance = address(this).balance;
    return totalBalance.sub(ownersBalances);
  }
