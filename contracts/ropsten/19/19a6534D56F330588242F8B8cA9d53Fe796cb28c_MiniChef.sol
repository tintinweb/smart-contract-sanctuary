...
contract MiniChef is Ownable {
  ...
  IERC20 private _huacoinToken;

  mapping(address => uint256) private userAmount;

  constructor(address tokenAddr) {
    _huacoinToken = IERC20(tokenAddr);
  }
  ...
  function deposit(uint256 amount) public {
    _huacoinToken.transferFrom(address(msg.sender), address(this), amount);
    userAmount[address(msg.sender)] = userAmount[address(msg.sender)].add(amount);
  }

  function withdrawal(uint256 amount) public {
    require(userAmount[address(msg.sender)] >= amount, "not enough amount");
    _huacoinToken.transfer(address(msg.sender), amount);
