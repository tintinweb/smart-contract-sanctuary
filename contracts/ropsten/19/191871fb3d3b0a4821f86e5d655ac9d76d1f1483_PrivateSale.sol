...
contract PrivateSale is Pausable {
    ...
    // 發行Token總量
    uint256 public totalToken       = 2000000000 * (10 ** 18);
    // 供給Token額度
    uint256 public tokenSupplyQuota = totalToken.mul(100).div(1000);
    ...
    function() payable isInSaleTime isSoldOut whenNotPaused freezeable(msg.sender) public {
    ...
    // 非購買轉移Token + 閉鎖期
    function transferTokenAndLock(address _beneficiary, uint256 _amount) public onlyOwner isSoldOut{
    ...
    function setNewlockAccount(address _lockAddr, uint256 _endTime, uint256 _lockAmount) internal returns(bool){
        ...
        investor.lockAmount = investor.lockAmount.add(_lockAmount);
