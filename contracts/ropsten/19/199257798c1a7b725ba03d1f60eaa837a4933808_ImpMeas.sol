...
// File: src\contracts\ImpMeasNFT.sol
...
//import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.0.0/contracts/access/AccessControlEnumerable.sol";

//import "@openzeppelin/contracts/utils/Counters.sol";


contract ImpMeas is ERC721Enumerable {	
	using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
	mapping (uint256 => uint32[32]) private _coefficients;
  
    constructor() ERC721("ImpMeas", "ImpMeas") {
	}
	
	function mint(uint32[32] memory _newcoefficients) public returns (uint256) {
		uint256 newId  = _tokenIds.current();
		_safeMint(msg.sender, newId);
		_coefficients[newId] = _newcoefficients;
		_tokenIds.increment();
      return newId;
    }
	
	function coefficientsOf(uint256 tokenId) public view returns (uint32[32] memory) {
		uint32[32] memory coefficients = _coefficients[tokenId];
		address owner = ownerOf(tokenId);		
        require(owner != address(0), "ERC721: coefficients query for nonexistent token");
        return coefficients;
    }
