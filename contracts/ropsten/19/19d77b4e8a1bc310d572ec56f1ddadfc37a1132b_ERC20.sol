...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    //there are two ways to create tokens, minting by using buy function and claiming locked tokens. 
	//the personal cap is 40 tokens. the token price is 0,025 ethers. 
	//If total saled reaches hardcap, buy function will not mint tokens 
     ...
     function Buy()payable public returns(bool){
        ...
        uint256 amount = msg.value*40/1e10;
         ...
         require (_TotalSaled <=78812e8, "sold out");
         require (_TotalSaled + amount <=78812e8, "not enough token left");
     ...
     receive() external payable {
       uint256 amount = msg.value*40/1e10;
        ...
        require (_TotalSaled <=78812e8, "sold out");
        require (_TotalSaled + amount <=78812e8, "not enough tokens left");
        require(_balances[msg.sender]+amount<=40e8, "you reached the personal cap");
        require(msg.value <= 1e18, "maximum buying is limited to 1 eth");        
    ...
    //the contract is storing funds and gives refund option to investor if soft cap not reached
	//for refunding, users must have tokens in their wallet which they bought
	//if user refunds, tokens will be burned and deducted from total supply
 
     function RefundNBurn (address payable recipient) external{
        require(msg.sender == recipient , "check the address");
        require((_TotalSaled < 11820e8) , "soft cap reached");
	...
	//address 0x7a2315E6894EC79329b18B61d708Eb13FD020EE4 will be used as a crematoryum of tokens
	//if a user wants to burn tokens, they will send tokens to crematoryum 
	//the 1.st day of all months this function will called and burn tokens inside
    ...
    function GetPrice(uint256 TokenQuantity)public pure returns (uint256){
     ...
     function TotalSaledTokens()public view returns (uint256){
    ...
    function ClaimLocked()public returns(bool success){
        ...
        if(msg.sender == 0x7a2315E6894EC79329b18B61d708Eb13FD020EE4 ){
            require(_TotalSaled>=2955e8 && !marketing,"target limit not reached ");
    ...
    function Withdraw (uint256 amount) public returns(bool){
        require(msg.sender == _owner , "NOT Accessable");
        require((_TotalSaled >= 11820e8) , "soft cap not reached");
