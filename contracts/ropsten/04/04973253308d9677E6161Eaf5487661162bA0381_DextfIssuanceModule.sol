...
/*
    ...
    Based on DextfIssuanceModule.sol
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
    http://www.apache.org/licenses/LICENSE-2.0
...
/**
 * @title DextfIssuanceModule
 * @author Set Protocol
 *
 * Module that enables issuance and redemption functionality on a SetToken. 
 * During an issue the getRequiredComponentUnitsForIssue are used. 
 * In a redemption look at the actual balance of tokens in the vault
 *
 */
contract DextfIssuanceModule is ModuleBase, ReentrancyGuard  {
    using Invoke for ISetToken;
        ...
        onlyValidAndInitializedSet(_setToken)
    {
        ...
        emit DextfFundIssued(address(_setToken), msg.sender, _to, hookContract, _quantity);
    }
        ...
        onlyValidAndInitializedSet(_setToken)
    {
        ...
        address[] memory components,
        uint256[] memory componentQuantities
        ) = getComponentUnitsForRedeem(_setToken, _quantity);
        ...
        emit DextfFundRedeemed(address(_setToken), msg.sender, _to, _quantity);
    }
    ...
    returns (address[] memory, uint256[] memory)
    {
        ...
        uint256 totalSupply = IERC20(address(_setToken)).totalSupply();
        if (totalSupply > 0){
            (
            address[] memory componentsToRedeem,
            uint256[] memory componentRedeemQuantities
            ) = getComponentUnitsForRedeem(_setToken, _quantity);

            for (uint256 i = 0; i < components.length; i++) {
                require(components[i] == componentsToRedeem[i], "Issue component must be the same as the redeem component");
                // We get the maximum to avoid token drains when dividends are paid
                notionalUnits[i] = notionalUnits[i].max(componentRedeemQuantities[i]);
            }
        }
    ...
    /**
     * Compute the amount of component token to release for a given quantity of SetToken.
     *
     * @param _setToken             Instance of the SetToken to issue
     * @param _quantity             Quantity of SetToken to issue
     * @return address[]            List of component addresses
     * @return uint256[]            List of component units to redeem for the quantity of SetTokens
     */
    function getComponentUnitsForRedeem(
    ISetToken _setToken,
    uint256 _quantity
    )
    public
    ...
    require(totalSupply > 0, "Total supply must be > 0");
        ...
        // This is the fund fraction corresponding to _quantity, multiplied by the preciseUnit (i.e. 1e18)
    uint256 fundFraction = _quantity.mul(PreciseUnitMath.preciseUnit()).div(totalSupply);
    ...
    uint256 unit = IERC20(component).balanceOf(address(_setToken));
    notionalUnits[i] = unit.preciseMul(fundFraction);
    }
