...
// File: contracts/lootJewelry.sol
...
/**
 * @title LootGems contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract LootJewels is ERC721, ReentrancyGuard, Ownable {
    
    uint256 private s_publicPrice = 20000000000000000; //0.02 ETH
    ...
    constructor() ERC721("Adventure Jewels for Loot", "LOOTJEWEL") {
    }
    ...
    function getGemColourHSL(uint256 p_tokenId) public view returns (string memory) {
        ...
        string memory gemHue = toString(uint256(gemHues[gemId]));
        string memory gemSaturation = toString(uint256(gemSaturations[conditionId]));
        string memory gemLightness = toString(uint256(gemLightnesses[gemId]));
        string memory output = string(abi.encodePacked(gemHue,',',gemSaturation,'%,',gemLightness,'%'));
        return output;  
    }
    ...
    function getGemPowerName(uint256 p_tokenId) public view returns (string memory) {
       require(_exists(p_tokenId),"Not minted");       
        uint256 powerId = findGemPowerId(p_tokenId, "POWER");
        string[11] memory gemPowerNames = [
            "the Gods",
            "Eternal Youth",
            "Healing",
            "the Warrior",
    ...
    function getGemBoost(uint256 p_tokenId) public view returns (string memory) {
        require(_exists(p_tokenId),"Not minted");        
        uint256 powerId = findGemPowerId(p_tokenId, "POWER");
        string[11] memory gemBoost = [
            "Wisdom",
            "Charisma",
            "Constitution",
            "Strength",
    ...
    function getGemProtection(uint256 p_tokenId) public view returns (string memory) {
        require(_exists(p_tokenId),"Not minted");        
        uint256 powerId = findGemPowerId(p_tokenId, "POWER");
        string[11] memory gemProtection = [
            "Magic",
            "Encounters",
            "Poison",
            "Blunt Weapons",
    ...
    /* 
    Finder functions - 
    These use the rarity table alongside a seed phrase (contact creation timestamp + seed word) to select Gem attributes */
    
    function findGemId(uint256 p_tokenId, string memory p_seedString) internal view returns (uint256) {
        uint8[8] memory rarityTable = [
            1,
            2,
            4,
            8,
        ...
        uint256 r_attrId = rarityTable.length-1;
        for (uint i=0; i<rarityTable.length-1; i++) {
            if (rarity>=rarityTable[i] && rarity<rarityTable[i+1]) {
                r_attrId = i;
            }
        }
        return r_attrId;
    ...
    function tokenURI(uint256 p_tokenId) override public view returns (string memory) {
        string memory gemCondition = getGemCondition(p_tokenId);
        string memory gemStone = getGemStone(p_tokenId);
        string memory gemPower = getGemPowerName(p_tokenId);
        string memory gemCut = getGemCut(p_tokenId);
        string memory jewelryType = getJewelryType(p_tokenId);
        string memory gemBoost = getGemBoost(p_tokenId);
        string memory gemProtection = getGemProtection(p_tokenId);
        string memory gemEffect = getGemEffect(p_tokenId);
        string memory gemColourHSL = getGemColourHSL(p_tokenId);
        
        string[23] memory parts;
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }.colored { fill:hsl(';
        parts[1] = gemColourHSL;
        parts[2] = ')}</style><rect width="100%" height="100%" fill="black" stroke="hsl(';
        parts[3] = gemColourHSL;
        parts[4] =')" stroke-width="5px" /><text x="10" y="20" class="base colored" font-weight="bold">';
        parts[5] = string(abi.encodePacked(gemCondition," ",gemStone," ",jewelryType," of ", gemPower));
        parts[6] = '</text><text x="10" y="60" class="base colored">Gem: ';
        parts[7] = gemStone;
        parts[8] = '</text><text x="10" y="80" class="base">Condition: ';
        parts[9] = gemCondition;
        parts[10] = '</text><text x="10" y="100" class="base">Cut: ';
        parts[11] = gemCut;
        parts[12] = '</text><text x="10" y="120" class="base">Jewelry: ';
        parts[13] = jewelryType;
        parts[14] = '</text><text x="10" y="160" class="base">It boosts ';
        parts[15] = gemBoost;
        parts[16] = '</text><text x="10" y="180" class="base">It protects against ';
        parts[17] =  gemProtection;
        parts[18] = '</text><text x="10" y="200" class="base">It ';
        parts[19] = gemEffect;
        parts[20] = '</text><text text-anchor="end" x="95%" y="95%" class="base">#';
        parts[21] = toString(p_tokenId);
        parts[22] = "</text></svg>";
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Jewelry #', toString(p_tokenId), '", "description": "Loot Jewelry for Adventurers is generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function mintWithLoot(uint p_lootId) public payable nonReentrant {
        ...
        s_randomSeeds[p_lootId] = block.timestamp
            ^ block.difficulty 
            ^ p_lootId
            ^ tx.gasprice
    ...
    function multiMintWithLoot(uint[] memory p_lootIds) public payable nonReentrant {
        for (uint i=0; i<p_lootIds.length; i++) {
            ...
            s_randomSeeds[p_lootIds[i]] = block.timestamp 
                ^ block.difficulty 
                ^ p_lootIds[i] 
                ^ tx.gasprice
                ^ uint256(keccak256(abi.encodePacked(msg.sender)));
    ...
    function mint(uint p_tokenId) public payable nonReentrant {
        ...
        if (s_isPrivateSale){
            require(p_tokenId > 8000 && p_tokenId <= 11800, "Token ID invalid");
        } else {
            require(p_tokenId > 0 && p_tokenId <= 11800, "Token ID invalid");
        }
        _safeMint(msg.sender, p_tokenId);
        s_randomSeeds[p_tokenId] = block.timestamp
            ^ block.difficulty
            ^ p_tokenId
            ^ tx.gasprice
            ^ uint256(keccak256(abi.encodePacked(msg.sender)));
    ...
    function multiMint(uint[] memory p_tokenIds) public payable nonReentrant {
        ...
        for (uint i=0; i<p_tokenIds.length; i++) {
            if (s_isPrivateSale){
                require(p_tokenIds[i] > 8000 && p_tokenIds[i] <= 11800, "Token ID invalid");
            } else {
                require(p_tokenIds[i] > 0 && p_tokenIds[i] <= 11800, "Token ID invalid");
            }
            _safeMint(msg.sender, p_tokenIds[i]);
            s_randomSeeds[p_tokenIds[i]] = block.timestamp
                ^ block.difficulty
                ^ p_tokenIds[i]
                ^ tx.gasprice
                ^ uint256(keccak256(abi.encodePacked(msg.sender)));
    ...
    function random2(bytes memory p_input) internal pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(p_input)));
    }
