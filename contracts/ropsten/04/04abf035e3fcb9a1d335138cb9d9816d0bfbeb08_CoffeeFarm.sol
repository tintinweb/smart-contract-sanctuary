...
contract CoffeeFarm {
  ...
  constructor(address _stake, address _reward) public {
    stakeToken = Token(_stake);
    rewardToken = Token(_reward);
    owner = msg.sender;
    trigger = msg.sender;
  }
  ...
  function depositedOf(address _address) public view returns (uint _amount) {
    Farmer storage farmer = farmers[_address];
    return farmer.deposited;
  }

  function rewardOf(address _address) public view returns (uint _amount) {
    Farmer storage _farmer = farmers[_address];
    ...
    return claimableOrigin.mul(decimal).div(targetAndOrigin);
  }
  ...
  function claim() public {
    ...
    rewardToken.transfer(msg.sender, claimableTarget);
    farmer.lastClaimedBlock = block.number;
  ...
  function leave() public {
    ...
    stakeToken.transfer(msg.sender, farmer.deposited);
    uint claimableTarget = getClaimableTarget(farmer);
    rewardToken.transfer(msg.sender, claimableTarget);
    farmer.deposited = 0;
  ...
  function getClaimableTarget(Farmer memory _farmer) private returns (uint) {
    ...
    return Math.min(claimableTarget, rewardToken.balanceOf(address(this)));
  }
}