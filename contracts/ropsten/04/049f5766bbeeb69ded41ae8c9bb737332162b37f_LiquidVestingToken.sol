...
interface ILiquidVestingToken {
    ...
    function addRecipients(address[] memory _recipients, uint256[] memory _amounts) external;
...
contract LiquidVestingToken is ERC20Upgradeable, OwnableUpgradeable, ILiquidVestingToken {
    using SafeMathUpgradeable for uint256;
    ...
    modifier onlyAfterActivation() {
        require(block.timestamp >= activationTimestamp, "Cannot transfer before activation timestamp");
        _;
    }
    ...
    function overrideFee(uint256 _newFee) external override {
        require(_msgSender() == ILiquidVestingTokenFactory(factory).feeCollector(), "Caller is not fee collector");
        require(
    ...
    function addRecipient(address _recipient, uint256 _amount) public override onlyOwner inType(AddType.Airdrop) {
    ...
    function addRecipients(address[] memory _recipients, uint256[] memory _amounts)
        external
        override
        onlyOwner
        inType(AddType.Airdrop)
    {
        require(_recipients.length == _amounts.length, "Recipients should be the same length with amounts");
        uint256 totalAmount;
    ...
    function addMerkleRoot(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, keccak256(abi.encode(_merkleRoot, _totalAmount)))
        );
        ...
        require(
            recoveredAddress != address(0) &&
                recoveredAddress == ILiquidVestingTokenFactory(factory).merkleRootSigner(),
            "Invalid signature"
        );
    ...
    function claimTokensByMerkleProof(
        ...
        require(_recipient != address(0), "Recipient cannot be zero address");
        require(_rootId < merkleRoots.length, "Merkle root with this index does not exists");
        bytes32 root = merkleRoots[_rootId];
        require(merkleRootSpent[root][_recipient] != true, "User can claim tokens only once");

        require(checkProof(_proof, leafFromAddressAndNumTokens(_recipient, _amount), root), "Invalid proof");
    ...
    function claimProjectTokensByFeeCollector() external override {
        ...
        require(_msgSender() == feeCollector, "Cannot claim project tokens if caller is not fee collector");

        uint256 amount = redeemToken.balanceOf(address(this));
        redeemToken.transfer(feeCollector, amount);
        emit ClaimedByFeeCollector(feeCollector, amount);
    }

    function redeem(address _recipient, uint256 _amount) public override {
        require(block.timestamp >= redeemTimestamp, "Cannot redeem tokens before unlock timestamp");
        require(_recipient != address(0), "Recipient cannot be zero address");
        require(_amount > 0, "Amount should be more than 0");

        uint256 amount = IERC20(address(this)).balanceOf(_recipient);
        require(_amount <= amount, "Cannot burn more than available amount of tokens");
    ...
    function transfer(address recipient, uint256 amount) public override onlyAfterActivation returns (bool) {
        uint256 feeFromAmount;

        if (overridenFee == 0) {
            feeFromAmount = amount.mul(ILiquidVestingTokenFactory(factory).fee()).div(PERSENTAGE_RATE);
        } else {
            feeFromAmount = amount.mul(overridenFee).div(PERSENTAGE_RATE);
        }

        super.transfer(ILiquidVestingTokenFactory(factory).feeCollector(), feeFromAmount);
    ...
    function transferFrom(
        ...
        super.transferFrom(sender, ILiquidVestingTokenFactory(factory).feeCollector(), feeFromAmount);
    ...
    function leafFromAddressAndNumTokens(address _a, uint256 _n) internal pure returns (bytes32) {
