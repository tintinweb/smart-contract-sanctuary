...
// File: contracts/TimeMachineToken.sol

contract TimeMachineToken is ERC20("TimeMachineToken", "BDP"), Ownable {
    using SafeMath for uint256;

    uint256 public NUMBER_BLOCKS_PER_DAY;
    uint256 public cap = 8000000e18 + 1e18;
    address public BDPMaster;
    address public BDPMasterPending;
    uint public setBDPMasterPendingAtBlock;

    uint256 public seedPoolAmount = 2400000e18;

    uint256 constant public PARTNERSHIP_TOTAL_AMOUNT = 2000000e18;
    uint256 constant public PARTNERSHIP_FIRST_MINT = 800000e18;
    uint256 public partnershipAmount = PARTNERSHIP_TOTAL_AMOUNT;
    uint256 public partnershipMintedAtBlock = 0;

    uint256 constant public TEAM_TOTAL_AMOUNT = 800000e18;
    uint256 public teamAmount = TEAM_TOTAL_AMOUNT;
    uint256 public teamMintedAtBlock = 0;

    uint256 constant public FUTURE_TOTAL_AMOUNT = 2800000e18;
    uint256 constant public FUTURE_EACH_MINT = 933333e18;
    uint256 public futureAmount = FUTURE_TOTAL_AMOUNT;
    uint256 public futureCanMintAtBlock = 0;

    uint256 public startAtBlock;

    constructor (uint _startAtBlock, uint _numberBlockPerDay) public {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (from == address(0)) {
            require(totalSupply().add(amount) <= cap, "TimeToken: cap exceeded");
        }
    }
    ...
    function confirmMaster() public onlyOwner {
        require(block.number - setBDPMasterPendingAtBlock > 2 * NUMBER_BLOCKS_PER_DAY, "TimeToken: cannot confirm at this time");
        BDPMaster = BDPMasterPending;
    }

    function setMaster(address _BDPMaster) public onlyOwner {
        require(BDPMaster == address(0x0), "TimeToken: Cannot set master");
        BDPMaster = _BDPMaster;
    }

    function mint(address _to, uint256 _amount) public {
        require(msg.sender == BDPMaster, "TimeToken: only master farmer can mint");
        require(seedPoolAmount > 0, "TimeToken: cannot mint for pool");
        require(seedPoolAmount >= _amount, "TimeToken: amount greater than limitation");
    ...
    function mintForPartnership(address _to) public onlyOwner {
        ...
        require(block.number >= startAtBlock + 7 * NUMBER_BLOCKS_PER_DAY, "TimeToken: Cannot mint at this time");
        require(partnershipAmount > 0, "TimeToken: Cannot mint more token for partnership");
    ...
    function mintForTeam(address _to) public onlyOwner {
        ...
        require(block.number >= startAtBlock + 7 * NUMBER_BLOCKS_PER_DAY, "TimeToken: Cannot mint at this time");
        require(teamAmount > 0, "TimeToken: Cannot mint more token for team");
        // This first minting
    ...
    function mintForFutureFarming(address _to) public onlyOwner {
        require(block.number >= startAtBlock + 30 * NUMBER_BLOCKS_PER_DAY, "TimeToken: Cannot mint at this time");
        require(futureAmount > 0, "TimeToken: Cannot mint more token for future farming");


        if (block.number >= startAtBlock + 30 * NUMBER_BLOCKS_PER_DAY 
            && futureAmount >= FUTURE_TOTAL_AMOUNT) {
            _mint(_to, FUTURE_EACH_MINT);
            futureAmount = futureAmount.sub(FUTURE_EACH_MINT);
        }

        if (block.number >= startAtBlock + 45 * NUMBER_BLOCKS_PER_DAY 
            && futureAmount >= FUTURE_TOTAL_AMOUNT.sub(FUTURE_EACH_MINT)) {
            _mint(_to, FUTURE_EACH_MINT);
            futureAmount = futureAmount.sub(FUTURE_EACH_MINT);
        }

        if (block.number >= startAtBlock + 60 * NUMBER_BLOCKS_PER_DAY
            && futureAmount > 0) {
...
// File: contracts/timeMachineMaster.sol 

contract TimeMachineMaster is Ownable {
    using SafeMath for uint256;
    ...
    TimeMachineToken public timeMachine;
    ...
    address public teamRewardAddr;
    address public timeMachineWorker;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "timeMachineMaster::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // --------- buy token with fee---------------
    function setFee(uint256 _pid, uint256 _fee, uint256 _buyTokenCap) public onlyOwner{
        poolInfo[_pid].protocolFee = _fee;
        poolInfo[_pid].buyTokenCap = _buyTokenCap; 
    }

    function setWorkerAddr(address _teamAddr,address _workerAddr) public onlyOwner{
        teamRewardAddr = _teamAddr;
        timeMachineWorker = _workerAddr;
    }

    function updateProtocolFee(uint256 _pid) public{
         PoolInfo storage pool = poolInfo[_pid];
         //check fee and cap
         if(poolInfo[_pid].protocolFee == 0) return;
         if(pool.currentFeeAmount < pool.buyTokenCap) return;
         //fee to team
         uint256 _teamFee =  pool.currentFeeAmount.mul(33).div(100);
         pool.lpToken.safeTransfer(teamRewardAddr,_teamFee);
         //fee to time machine (buy token、 add lp ...)
         pool.lpToken.safeTransfer(timeMachineWorker,pool.currentFeeAmount.sub(_teamFee));
         //update current fee amount
         pool.currentFeeAmount = 0;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        timeMachine.mint(address(this), reward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            uint256 fee = _amount.mul(pool.protocolFee).div(10000);
            user.amount = user.amount.add(_amount.sub(fee));
            pool.currentFeeAmount = pool.currentFeeAmount.add(fee);
        }
        updateProtocolFee(_pid);
    ...
    function safeTransferReward(address _to, uint256 _amount) internal {
        uint256 bal = timeMachine.balanceOf(address(this));
        if (_amount > bal) {
            timeMachine.transfer(_to, bal);
        } else {
            timeMachine.transfer(_to, _amount);
        }
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint) {
        ...
        uint256 amountCanMint = timeMachine.cap().sub(timeMachine.totalSupply());
