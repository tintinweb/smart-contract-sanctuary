...
// ----------------------------------------------------------------------------
// PPSwap013: 5/26/2021. We will charge swap fees by ETH not by PPS. Instead both swap parties will be rewardsed with some PPS! The swap fees are 
...
contract PPSwap is ERC20Interface, SafeMath{
    ...
    // function acceptOffer will be called by accountB/Kathy, but we need to make sure this agrees with the offer by Bob, anybody can be Kathy
    function acceptOffer(address accountA, 
                        address tokenA, 
                        address tokenB, 
                        uint amtA, 
                        uint amtB)
        ...
        // we give rewards to both parties 
        if(balances[address(this)] > 0){
            ...
            balances[msg.sender] = safeAdd(balances[msg.sender], rewardsPerParty);
        }
        
        // Kathy will pay for the swap fee in ETH, the caller needs to make sure this 
        require(msg.value >= swapfeePerTrans, "Not sufficient swap fees. ");
        return acceptOfferImp(accountA, msg.sender, tokenA, tokenB, amtA, amtB);                      
    }
