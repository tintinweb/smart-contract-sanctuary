...
interface INFTStore{
    ...
    function mint(address from, address to, uint256 id, uint256 blockExpiry, uint8 v, bytes32 r, bytes32 s, uint256 supply, string memory uri) external returns(bool);
...
contract TransferSafe {
    ...
    function erc1155safeMintTransferFrom(INFTStore token, address from, address to, uint256 id, uint256 value,
        uint256 blockExpiry, uint8 v, bytes32 r, bytes32 s, string memory uri ) internal {
        
        require(token.mint(from, to, id, blockExpiry, v, r, s, value, uri), "TransferSafe:erc1155safeMintTransferFrom:: transaction Failed");
    } 
...
contract Exchange is OrderState, TransferSafe {
    ...
    function sell(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, uint256 royaltyFee, address payable royaltyReceipt, bool isStore, mintParams memory storeParams ) payable external {
    ...
    function buy(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, uint256 royaltyFee, address payable royaltyReceipt, bool isStore, mintParams memory storeParams) payable external {
       require((block.timestamp<= order.expiryTime), "Signature expired" );
    ...
    function transferToken(Order calldata order, address buyer , bool isStore, mintParams memory storeParams) internal {
        if(order.key.sellAsset.assetType == AssetType.ERC721){
            if(order.orderType == 1 || order.orderType == 3) {
                if(!isStore){
                    ...
                    erc1155safeMintTransferFrom(INFTStore(order.key.sellAsset.token), order.key.owner, buyer, order.key.sellAsset.tokenId, 1, 
                        storeParams.blockExpiry,storeParams.v,storeParams.r, storeParams.s, storeParams.uri);
                }
            }
                ...
                if(!isStore){
                    ...
                    erc1155safeMintTransferFrom(INFTStore(order.key.buyAsset.token), buyer, order.key.owner, order.key.buyAsset.tokenId, 1,
                                                storeParams.blockExpiry,storeParams.v,storeParams.r, storeParams.s, storeParams.uri);
            ...
            if(order.orderType == 1 || order.orderType == 3) {
                if(!isStore){
                    ...
                    erc1155safeMintTransferFrom(INFTStore(order.key.sellAsset.token), order.key.owner, buyer, order.key.sellAsset.tokenId, order.selling,
                                                storeParams.blockExpiry,storeParams.v,storeParams.r, storeParams.s, storeParams.uri);
                ...
                if(!isStore){
                    ...
                    erc1155safeMintTransferFrom(INFTStore(order.key.buyAsset.token), buyer, order.key.owner, order.key.buyAsset.tokenId, order.buying,
                                                storeParams.blockExpiry,storeParams.v,storeParams.r, storeParams.s, storeParams.uri);
    ...
    function bid(
        Order calldata order, Sig calldata sig,Sig calldata buyerSig, Sig calldata buyerFeeSig, address buyer, uint256 buyingAmount, 
        uint256 royaltyFee, address payable royaltyReceipt, bool isStore, mintParams memory storeParams) external {
        require((block.timestamp <= order.expiryTime ), "Signature expired");
    ...
    function transferEthFee(uint256 amount, address payable _seller, uint256 royaltyFee, address payable royaltyReceipt ) internal{
        (uint256 protocolfee, uint256 secoundaryFee, uint256 remaining) = transferFeeView(amount, royaltyFee);
        if(protocolfee > 0){
    ...
    function transferWethFee(uint256 amount, address  _seller, address buyer,uint256 royaltyFee, address  royaltyReceipt ) internal{
        (uint256 protocolfee, uint256 secoundaryFee, uint256 remaining) = transferFeeView(amount, royaltyFee);
        if(protocolfee > 0){
            require(IWETH(weth).transferFrom(buyer,beneficiaryAddress,protocolfee), "Failed protocol fee transfer");
        }
        if( (secoundaryFee > 0) && (royaltyReceipt!= address(0x00)) ){
                require(IWETH(weth).transferFrom(buyer,royaltyReceipt,secoundaryFee), "Failed royalty fee transfer");
        }
        if(remaining > 0){
    ...
    function transferErc20Fee(address token, uint256 amount, address  _seller, address buyer,uint256 royaltyFee, address  royaltyReceipt ) internal{
        ...
        (uint256 protocolfee, uint256 secoundaryFee, uint256 remaining) = transferFeeView(amount, royaltyFee);
        if(protocolfee > 0){
            require(IERC20(token).transferFrom(buyer,beneficiaryAddress,protocolfee), "Failed protocol fee transfer");
        }
        if( (secoundaryFee > 0) && (royaltyReceipt!= address(0x00)) ){
                require(IERC20(token).transferFrom(buyer,royaltyReceipt,secoundaryFee), "Failed royalty fee transfer");
        }
        if(remaining > 0){
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        NFTStore = INFTStore(newMintableStore);
