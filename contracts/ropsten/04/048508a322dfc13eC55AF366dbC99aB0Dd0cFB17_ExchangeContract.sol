...
contract ExchangeContract is Sale{
    ...
    function burn(address from, address admin, uint256 tokenId, address token, uint256 _type, uint256 NOFToken ) public{
        require( balances[tokenId][from] <= NOFToken || from == admin, "Your Not a Token Owner or insuficient Token Balance");
        if(_type == 721){
            MyERC721 tok= MyERC721(token);
            tok._burn(tokenId, from, admin);
            ...
            DigitalERC1155 tok= DigitalERC1155(token);
            tok.burn(from, admin, tokenId, NOFToken);
            balances[tokenId][from] = balances[tokenId][from].sub(NOFToken);
