...
contract Hive is Ownable, IERC721Receiver, Pausable {
  ...
  function _addBeeToHive(
    ...
    if (hives[hiveId].timestamp == 0) hives[hiveId].timestamp = uint80(block.timestamp);
    hives[hiveId].occupancy += 1;
    hives[hiveId].bees[tokenId] = Bee({ owner: account, tokenId: uint16(tokenId), value: uint80(block.timestamp) });
    totalBeesStaked += 1;
  ...
  function manyBearsAttack(
    ...
    uint256 owed = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      // if (forrest[tokenIds[i]].owner != _msgSender()) {
      //   BearsAttacked(_msgSender(), tokenIds[i], 0, true);
      //   continue;
      // }
      // if (forrest[tokenIds[i]].owner != _msgSender()) continue;
      // require(bees.ownerOf(tokenIds[i]) == _msgSender(), 'YOU ARE NOT THE OWNER');
      // require(bees.getTokenNumType(tokenIds[i]) == 1, 'TOKEN NOT A BEAR');
      uint256 num = ((random(tokenIds[i]) & 0xFFFF) % 100);
      if (num < 50) {
        if (!transfer) forrest[tokenIds[i]].pot += 10000;
        else owed += 10000;
        BearsAttacked(_msgSender(), tokenIds[i], 10000, false);
      }
    }
    if (transfer) honey.mint(_msgSender(), owed);
  }
  ...
  function getInfoOnHive(uint256 hiveId)
    public
    view
    returns (
      uint80,
  ...
  {
    return (hives[hiveId].timestamp, hives[hiveId].subtract, hives[hiveId].occupancy);
  }
  ...
  function getInfoOnHives() public view returns (string memory) {
    ...
    for (uint16 i = 0; i < totalNumberOfHives; i++) {
      result = string(abi.encodePacked(result, uint16(i).toString(), ':', uint16(hives[i].occupancy).toString(), ',', uint16(hives[i].occupancy).toString(), ';'));
    }
    return result;
  }
...
contract CryptoBees is ERC721, Ownable, Pausable {
  ...
  Honey private honey = Honey(0x3b6F57900FAb6f0EC8D3d1d3538159eE96B5c105);
  ...
  function mintForHoney(uint256 amount, bool stake) external payable whenNotPaused {
    require(tx.origin == _msgSender(), 'Only EOA');
    require(minted + amount <= MAX_TOKENS, 'All tokens minted');
    require(amount > 0 && amount <= 10, 'Invalid mint amount');
    if (minted < PAID_TOKENS) {
    ...
    honey.transferFrom(msg.sender, address(this), totalHoneyCost);
  }
  ...
  function getTokenTraits(uint256 tokenId) external view returns (uint256) {
    require(_exists(tokenId), 'ERC721Metadata: Nonexistent token');
    return ((tokenTraits[tokenId] & 0xFFFF) % 100);
  }

  function getTokenNumType(uint256 tokenId) external view returns (uint256) {
    return _getTokenNumType(tokenId);
  }
  ...
  function _getTokenNumType(uint256 tokenId) private view returns (uint256) {
    require(_exists(tokenId), 'ERC721Metadata: Nonexistent token');
    uint256 trait = tokenTraits[tokenId];
    uint256 num = ((trait & 0xFFFF) % 100);
    uint256 numType;
    if (num == 0) numType = 0;
    else if (num < 10) numType = 1;
    else numType = 2;
    return numType;
  }
  ...
  function _getTokenTextType(uint256 tokenId) private view returns (string memory) {
    require(_exists(tokenId), 'ERC721Metadata: Nonexistent token');
    uint256 numType = _getTokenNumType(tokenId);
    string memory textType;
    if (numType == 0) textType = 'BEEKEEPER';
    else if (numType == 1) textType = 'BEAR';
    else textType = 'BEE';
    return textType;
  }
  ...
  function getTokenImage(uint256 tokenId) private view returns (string memory) {
    ...
    uint256 numType = _getTokenNumType(tokenId);
    string memory image;
    if (numType == 0) image = 'QmTUuGDbndWZDYYr6pE1aeutZLpSuZi44KMZxeUw1VB2D8';
    else if (numType == 1) image = 'QmVPMv3Kxg94vAJo4fQY2FGnYTYp4RM1dq7anwr9psbz9P';
    else image = 'QmfCnnjNDndTuRZLZFJhLVtQ8m533pEnBis4Y2NH3BvZdF';
    return image;
  }
