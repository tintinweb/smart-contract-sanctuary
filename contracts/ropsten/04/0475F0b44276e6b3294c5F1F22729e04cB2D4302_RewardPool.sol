...
contract RewardPool is Ownable, Constants, IRewardPool {
    ...
    function swapToLotl(uint256 _amount) private{
    ...
    function transferAllBUSD (uint256 _amount) private {
        IERC20(busdAddr).transfer(address(chef), _amount);
        emit TransferAllBUSD(msg.sender, _amount);
    }    

    // Initiates the reward calculation in the MasterChefContract
    function distributeRewardPool () public onlyOwner {
        uint256 amountToBurn = IERC20(busdAddr).balanceOf(address(this))/5;
        uint256 amountToTransfer = IERC20(busdAddr).balanceOf(address(this)) - amountToBurn;
        swapToLotl(amountToBurn);
        burnLotl(amountToBurn);
        transferAllBUSD(amountToTransfer);
    ...
    // Burns LOTL.
    function burnLotl (uint256 _amount) private {
        IERC20(lotlToken).transfer(burnAddr, _amount);
        emit BurnLotl(msg.sender, _amount);
    }
