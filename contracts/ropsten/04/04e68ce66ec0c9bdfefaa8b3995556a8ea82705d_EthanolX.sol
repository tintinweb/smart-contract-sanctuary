...
contract EthanolX is ERC20, Ownable {
    ...
    function _refundGasUsed(address _account) private returns(bool) {
        require(getPair(address(this), uniswapV2Router.WETH()) != address(0), "EthanolX: No ENOX-WETH pair found");
        if(_msgSender() != getPair(address(this), uniswapV2Router.WETH())) return false;
        ...
        amounts = getAmountsOut(_gasCost);

        _mint(_account, amounts[0]);
        return true;
    }

    function _calculateRewards(address _account) public view returns(uint256) {
        uint256 _unclaimedDays = (block.timestamp - cashbacks[_msgSender()].timestamp) / 5 minutes;
    ...
    function claimCashback() external {
        ...
        require(block.timestamp >= cashbacks[_msgSender()].timestamp + 5 minutes, "");

        // this checks whether it's the first time the caller is claiming his/her rewards
        cashbacks[_msgSender()].timestamp == 0 
            ? cashbacks[_msgSender()].timestamp = startBlock 
            : cashbacks[_msgSender()].timestamp = cashbacks[_msgSender()].timestamp;
