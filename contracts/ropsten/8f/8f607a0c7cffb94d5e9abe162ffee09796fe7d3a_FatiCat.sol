...
contract FatiCat is Ownable, ERC721URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC721("FatiCat", "FACT") {}

    // variables
    uint256 public totalTokens = 0;
    address payable private ownerWallet = payable(0xac95d40965870965F7253381B28f144b388673E1);
    mapping(uint256 => address payable) tokenFirstOwner;
    ...
    onlyOwner
    {
        ...
        tokenFirstOwner[id] = payable(owner);
        totalTokens ++;
    }
    ...
    function transferEther(address payable _to, uint256 _amount) public onlyOwner {
        _to.transfer(_amount);
        emit transfer(_to, _amount, address(this).balance);
    }

    function depositEther(address from, address to, uint256 tokenId) public payable {
        uint256 fee = msg.value * 10 / 100;
        // uint256 tokenFirstOwnerfee = msg.value * 15 / 100;
        transferEther(ownerWallet, fee);
        transferEther(tokenFirstOwner[tokenId], msg.value-fee);
        // tokenFirstOwnerfee);
