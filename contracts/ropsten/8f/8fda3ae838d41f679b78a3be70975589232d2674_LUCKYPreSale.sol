...
contract LUCKYPreSale is Ownable {
    ...
    mapping(address => bool) boughtLUCKY;
    ...
    function getAllotmentPerBuyer() public view returns (uint256) {
        ...
        //TODO : Calcul du montant min / max en fonction de chaque buyer. 
        return IERC20(bLCK).balanceOf(address(this)).div(totalWhiteListed);
    }

    function purchasebLCK(uint256 _amountMIM) external returns (bool) {
        ...
        IERC20(MIM).safeTransferFrom(msg.sender, addressToSendMim, _amountMIM);
        IERC20(bLCK).safeTransfer(msg.sender, _purchaseAmount);

        return true;
    }

    function sendRemainingbLCK(address _sendbLCKTo)
        external
        onlyOwner()
        returns (bool)
    {
        ...
        IERC20(bLCK).safeTransfer(
            _sendbLCKTo,
            IERC20(bLCK).balanceOf(address(this))
        );
