...
contract HeadShotToken is Context, IERC20, Ownable {
    struct SalesInfo {
        uint256 code;
        string name;
        string desc;
    ...
    mapping(uint256 => SalesInfo) private _sales;
    SalesInfo[] _salesList;

    //[address][code][date]
    mapping(address => mapping(uint256 => mapping(uint256 => BuyInfo))) private _buysInfo;
    BuyInfo[] _buyList;

    //[address][code][date]
    mapping(address => mapping(uint256 => mapping(uint256 => SellInfo))) private _sellsInfo;
    SellInfo[] _sellList;

    //[address][code]
    mapping(address => mapping(uint256 => BalanceInfo)) private _balancesInfo;
    ...
    uint256 private constant _tTotal = 1000 * 10 ** 6 * 10 ** _decimals;
    uint256 public _maxTxAmount = 1 * 10 ** 6 * 10 ** _decimals;
    uint256 private constant numTokensSellToAddToLiquidity = 100 * 10 ** 6 * 10 ** _decimals;
    ...
    constructor () {
        ...
        addSales(1001, 'Register Ticket', 'Ticket for token listing register', 20000);
        addSales(1002, 'Verify Ticket', 'Ticket for token listing verify', 40000);
        addSales(1003, 'Vote Ticket', 'Ticket for token listing vote', 1000);
    }
    ...
    */
    ...
    function addSales(uint256 code_, string memory name_, string memory desc_, uint256 price_) public onlyOwner {
        require(code_ != 0, "SALES: Code can't empty");
        require(_sales[code_].code != code_, "SALES: Code must unique");
        SalesInfo memory salesInfo = SalesInfo(code_, name_, desc_, price_.mul(10 ** _decimals));
        _sales[code_] = salesInfo;
        _salesList.push(salesInfo);
    }

    function updateSales(uint256 code_, string memory name_, string memory desc_, uint256 price_) public onlyOwner {
        require(code_ != 0, "SALES: Code can't empty");
        if (_sales[code_].code == code_){
            SalesInfo memory salesInfo = SalesInfo(code_, name_, desc_, price_.mul(10 ** _decimals));
            _sales[code_] = salesInfo;
            //Update Sales Info
            for (uint index = 0; index < _salesList.length - 1; index++) {
                if (_salesList[index].code == code_){
                    _salesList[index] = salesInfo;
                }
            }
        }
    }

    function getSales(uint256 _code) public view returns (uint256, string memory, string memory, uint256) {
        if (_sales[_code].code == _code){
            return (
            _sales[_code].code,
            _sales[_code].name,
            _sales[_code].desc,
            _sales[_code].price.div(10 ** _decimals)
            );
        }
        return (0, '', '', 0);
    }

    function listSales() public view returns (
        uint256[] memory,
        string[] memory,
        string[] memory,
        uint256[] memory
    ) {
        uint salesCount = _salesList.length;
        uint256[] memory _codes = new uint256[](salesCount);
        string[] memory _names = new string[](salesCount);
        string[] memory _descs = new string[](salesCount);
        uint256[] memory _prices = new uint256[](salesCount);

        if (salesCount > 0){
            for (uint i = 0; i < salesCount; i++) {
                _codes[i] = _salesList[i].code;
                _names[i] = _salesList[i].name;
                _descs[i] = _salesList[i].desc;
                _prices[i] = (_salesList[i].price).div(10 ** _decimals);
            }
        }

        return (_codes, _names, _descs, _prices);
    }

    function buy(uint256 _code, uint256 _count) public returns (bool) {
        if (_sales[_code].code == _code){
            address _buyer = _msgSender();
            uint256 _price = _sales[_code].price;
            uint256 _totalPay = _price * _count;
            uint256 _date = block.timestamp;

            _transfer(_msgSender(), _salesWalletAddress, _totalPay);
            _buysInfo[_buyer][_code][_date].date = _date;
            _buysInfo[_buyer][_code][_date].count = _count;

            BuyInfo memory buyInfo = BuyInfo(_buyer, _code, _date, _count);
            _buyList.push(buyInfo);

            _balancesInfo[_buyer][_code].balance += _count;
            return true;
        }
        return false;
    }

    function sell(uint256 _code, uint256 _count) public returns (bool) {
        if (_sales[_code].code == _code){
            address _buyer = _msgSender();
            uint256 _date = block.timestamp;
            ...
            _buysInfo[_buyer][_code][_date].date = _date;
            _buysInfo[_buyer][_code][_date].count = _count;
            SellInfo memory sellInfo = SellInfo(_buyer, _code, _date, _count);
            _sellList.push(sellInfo);
    ...
    function listBalances(address buyer) public view returns (
        uint256[] memory,
        string[] memory,
        string[] memory,
        uint256[] memory
        ...
        string[] memory _names = new string[](salesCount);
        string[] memory _descs = new string[](salesCount);
        uint256[] memory _balances = new uint256[](salesCount);

        if (salesCount > 0){
            for (uint i = 0; i < salesCount; i++) {
                uint256 _code =  _salesList[i].code;
                _codes[i] = _code;
                _names[i] = _salesList[i].name;
                _descs[i] = _salesList[i].desc;
                _balances[i] = _balancesInfo[buyer][_code].balance;
            }
        }

        return (_codes, _names, _descs, _balances);
    }

    function listPurchase(address buyer, uint limit, uint page) public view returns (
        uint256[] memory,
        uint256[] memory,
        uint256[] memory
    ) {
        uint rowStart = (page - 1) * limit;
        uint rowEnd = rowStart + limit;
        uint rowCount = rowEnd - rowStart + 1;
        ...
        uint buyCount = _buyList.length;
        if (buyCount > 0){
            for (uint i = 0; i < buyCount; i++) {
                if (buyer == _buyList[i].buyer){
                    _codes[id] = _buyList[i].code;
                    _dates[id] = _buyList[i].date;
                    _counts[id] = _buyList[i].count;
                    id++;
                }
            }
        }
        return (_codes, _dates, _counts);
    }

    function listSell(address buyer, uint limit, uint page) public view returns (
        uint256[] memory,
        uint256[] memory,
        uint256[] memory
    ) {
        ...
        uint buyCount = _sellList.length;
        if (buyCount > 0){
            for (uint i = 0; i < buyCount; i++) {
                if (buyer == _sellList[i].buyer){
                    _codes[id] = _sellList[i].code;
                    _dates[id] = _sellList[i].date;
                    _counts[id] = _sellList[i].count;
                    id++;
                }
            }
        }
