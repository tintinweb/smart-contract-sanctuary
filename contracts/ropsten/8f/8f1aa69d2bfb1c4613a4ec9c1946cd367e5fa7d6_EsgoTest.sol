...
contract EsgoTest{    
    ...
    function getAwardCost(uint txId)public constant returns(uint){
        return esgoTXs[txId].award_cost;
    }
    ...
    function addFundTX(uint txId,uint participant)public payable{
        ...
        if(fundTXs[txId].funders==0){
            if(msg.value==esgoTXs[txId].txVal/2)
            {
                
            }else if(msg.value<esgoTXs[txId].txVal/2){
                revert();
            }else{
                
            }    
        }
        if(esgoTXs[txId].txVal== fundTXs[txId].funded) {
        ...
        if(esgoTXs[txId].txVal>=(msg.value+fundTXs[txId].funded)){
            txVal = msg.value;
            
        }    
        else
            txVal = esgoTXs[txId].txVal - fundTXs[txId].funded;
    ...
    function getFundAllTx(uint txId)public constant returns(uint) {
        return (esgoTXs[txId].txVal-fundTXs[txId].funded);
    }
    ...
    function getEsgoTXCount() public constant returns(uint) {
        return esgoTXs.length;
    }
  ...
  //      oraclize_query(2, "URL", "");
    ...
    function autoDestruct() public payable {
        for(uint i=0;i<esgoTXs.length;i++){
            uint txId = esgoTXs[i].txId;
            addUsers(msg.sender);

            if(esgoTXs[txId].atdtime<block.timestamp)
            if(fundTXs[txId].funded!=esgoTXs[txId].txVal)
            if(fundTXs[txId].fulfill != 2){
                fundTXs[txId].fulfill = 2;        
                address vendor = esgoTXs[txId].vendor;
                uint ul = users.length;
                address customer = esgoTXs[txId].customer;
                ...
                for(uint iu = 0;iu<ul;iu++){
                    ...
                    for(uint iutx = 0;iutx<utl;iutx++){
                        if(userTXs[fuser][iutx].participant == 2 && userTXs[fuser][iutx].txId == txId){
                            balances[fuser] = balances[fuser] + userTXs[fuser][iutx].txVal +  ((esgoTXs[txId].award_cost*userTXs[fuser][iutx].txVal)/1000000000000000000);
                            break;
                        }
                    }
                }
    ...
    function autoTearDownAndDestruct() public payable {
        for(uint i=0;i<esgoTXs.length;i++){
            ...
            if(fundTXs[txId].funded==esgoTXs[txId].txVal){
                fundTXs[txId].fulfill = 2;        
                address vendor = esgoTXs[txId].vendor;
                balances[vendor] = balances[vendor]+esgoTXs[txId].txVal;
                uint ul = users.length;
                        ...
                        if(userTXs[fuser][iutx].participant == 2 && userTXs[fuser][iutx].txId == txId){
                            if(disputeTxs[txId].disputed==false){
                              ...
                              //  balances[fuser] = balances[fuser] + (userTXs[fuser][iutx].txVal*awardallocate/(1000000000000000000));
                                balances[fuser] = balances[fuser] + ((esgoTXs[txId].award_cost*userTXs[fuser][iutx].txVal)/1000000000000000000);
                ...
                fundTXs[txId].fulfill = 2;        
                vendor = esgoTXs[txId].vendor;
                ul = users.length;
                customer = esgoTXs[txId].customer;
                balances[vendor] = balances[vendor] - tx.gasprice;
                balances[msg.sender] = balances[msg.sender] + tx.gasprice;
                for(iu = 0;iu<ul;iu++){
                    fuser = users[iu];
    ...
    function tearDown(uint txId) public payable{
        if(fundTXs[txId].funded==esgoTXs[txId].txVal) {
            ...
            uint ul = users.length;
            aprTxs[txId].tearBlock = block.number;
           // uint awardallocate = esgoTXs[txId].fee*(aprTxs[txId].tearBlock - aprTxs[txId].startBlock)/10000;
            uint awardallocate = esgoTXs[txId].fee;
            if((block.timestamp - disputeTxs[txId].startTx)<disputeTxs[txId].escrow_duration)
                awardallocate = esgoTXs[txId].fee/disputeTxs[txId].escrow_duration*(disputeTxs[txId].escrow_duration-(block.timestamp - disputeTxs[txId].startTx));
          //  uint useraward = esgoTXs[txId].award-awardallocate;
            address customer = esgoTXs[txId].customer;
            balances[customer] = balances[customer]+esgoTXs[txId].fee-awardallocate;
            for(uint iu = 0;iu<ul;iu++){
                address fuser = users[iu];
                uint utl= userTXs[fuser].length;
                for(uint iutx = 0;iutx<utl;iutx++){
            ...
            address customer2 = esgoTXs[txId].customer;
            balances[customer2] = balances[customer2]+esgoTXs[txId].txVal;
        }  
    }
    ...
    function getEsgoTX(uint index) public constant returns(uint, address, address, uint, uint, uint){
        return(esgoTXs[index].txId,esgoTXs[index].customer,esgoTXs[index].vendor,esgoTXs[index].txVal,esgoTXs[index].escrow_duration,esgoTXs[index].fee);
    } 
  
    function getEsgoTXFund(uint index) public constant returns(uint, uint){
    ...
    function addEsgoTX(address vendor,uint escrow_duration,uint fee,uint award_cost,uint award_escrow_duration) public payable {
        if(fee>balances[vendor])  {
            msg.sender.send(msg.value);
            revert();
        }    
        
        uint index = getEsgoTXCount();
        ...
        esgoTXs.push(esgoTX(index,msg.sender,vendor,msg.value,atdtime,escrow_duration,fee,award_cost,award_escrow_duration));
