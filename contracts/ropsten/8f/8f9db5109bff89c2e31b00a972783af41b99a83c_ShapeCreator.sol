...
contract ShapeCreator {
    ...
    string svgWrapperStart = '<svg viewBox="0 0 9000 9000" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="9000" height="9000" fill="white"/>';
    ...
    uint256 baseX = 24;
    uint256 stepX = 24;
    uint256 baseWidth = 8952;
    uint256 stepWidth = 48;
    uint256 subshapeCount = 180;
    ...
    function getShapeSvgByIndex(uint256 index) public view returns(string memory){
        ...
        string[2] memory _shapeBorderRadii = ["10000", "0"];
        ...
        for(i = 0; i < subshapeCount; i++){
            svgString = string(
                abi.encodePacked(
                    svgString, 
                    shapeSvgPart1, 
                    uintToStr(baseX + stepX * i),
                    shapeSvgPart2,
                    uintToStr(baseX + stepX * i),
                    shapeSvgPart3,
                    uintToStr(baseWidth - stepWidth * i)
                )
            );
            svgString = string(
                abi.encodePacked(
        ...
        return string(abi.encodePacked(svgWrapperStart, svgString, svgWrapperEnd));
    }
