...
contract ERC223 {
	...
	function transfer(address _to, uint _value, bytes _data, string _callback) public returns (bool);
...
contract T is ERC20, ERC223 {
	...
	function transfer(address[] _tos, uint256[] _values) public isRunning returns (bool) {
		...
		for (uint i = 0; i < cnt; i++) {
			...
			emit Transfer(msg.sender, to, val, empty);
