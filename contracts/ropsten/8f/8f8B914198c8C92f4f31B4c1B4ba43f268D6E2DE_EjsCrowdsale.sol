...
abstract contract MintedCrowdsale is Crowdsale {
        ...
        override
    {
        ...
        IERC20Mintable(tokenSelling()).mint(beneficiary, tokenAmount);
    }
}
...
contract Crowdsale is ReentrancyGuard, ICrowdsale {
    ...
    constructor(
        ...
        require(
            paymentDecimals_.length == paymentTokens_.length,
            "Crowdsale: Mismatch in number of payment tokens and decimals"
        );
        require(
            rates_.length == paymentTokens_.length,
            "Crowdsale: Mismatch in number of payment tokens and rates"
        );
        ...
        for (uint256 i = 0; i < paymentTokens_.length; i++) {
            ...
            uint256 paymentDecimal_ = paymentDecimals_[i];
            require(
                paymentDecimal_ <= TOKEN_MAX_DECIMALS,
            ...
            _paymentDecimals[paymentToken] = paymentDecimal_;
        ...
        returns (uint256 tokenAmount)
    {
        ...
        require(weiAmount > 0, "Crowdsale: zero wei amount");
        require(
            isPaymentToken(paymentToken),
            "Crowdsale: payment token unaccepted"
        );
        ...
        returns (uint256 tokenAmount)
    {
        ...
        tokenAmount = weiAmount.mul(TOKEN_SELLING_SCALE).div(rate_);
    }
        ...
        virtual
    {
        ...
        if (_paymentDecimals[paymentToken] < TOKEN_MAX_DECIMALS) {
            uint256 decimalsDiff =
                uint256(TOKEN_MAX_DECIMALS).sub(_paymentDecimals[paymentToken]);
