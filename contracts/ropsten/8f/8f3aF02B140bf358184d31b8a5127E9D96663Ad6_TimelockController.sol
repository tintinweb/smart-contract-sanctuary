...
/**
 * @dev VaultMax functions that do not require less than the min timelock
 */
interface IVaultMax {
    function add(
        uint256 _allocPoint,
...
contract TimelockController is AccessControl {
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        IVaultMax(_vaultmaxAddress).set(_pid, _allocPoint, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        IVaultMax(_vaultmaxAddress).add(0, _want, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
