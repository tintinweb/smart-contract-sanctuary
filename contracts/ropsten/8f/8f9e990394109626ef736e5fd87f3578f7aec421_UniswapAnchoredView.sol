...
contract UniswapAnchoredView is UniswapConfig {
    ...
    bytes32 constant ethHash = keccak256(abi.encodePacked("AVAX"));
    ...
    function priceInternal(TokenConfig memory config) internal view returns (uint) {
        ...
        if (config.priceSource == PriceSource.FIXED_ETH) {
            ...
            require(usdPerEth > 0, "AVAX price not set, cannot convert to dollars");
    ...
    function fetchEthPrice() internal returns (uint) {
        return fetchAnchorPrice("AVAX", getTokenConfigBySymbolHash(ethHash), ethBaseUnit);
    }
