...
contract Tygr241 is ERC721, Ownable {
  using Strings for uint256;
  ...
  constructor() ERC721("Tygr241", "TY241") {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    require(!paused, "{'status': false, 'message':'Minting is Paused'}");
    require(!preSaleIsActive, "{'status': false, 'message':'Pre-sale is active'}");
    require(cost * _mintAmount <= msg.value, "{'status': false, 'message':'Ether value sent is not correct'}");
