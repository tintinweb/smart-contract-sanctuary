...
//время ожидания до забора приза
    uint constant public MAX_IDLE_TIME = 3 minutes; //Maximum time the deposit should remain the last to receive prize
   ...
   //описание номера очереди и номер депозита в очереди
    ...
    function () public payable {
        ...
        if(msg.value > 0){
            ...
            require(getStageStartTime(stage+1) >= now + MAX_IDLE_TIME); //время старта в текущей очереди 
...
//отправка приза
    ...
    function getCurrentStageByTime() public view returns (int) {
        return int(now - 18 hours) / 10 minutes - 17840*1 days/10 minutes;
    ...
    function getStageStartTime(int _stage) public pure returns (uint) {
        return 18 hours + uint(_stage + 1)*10 minutes + 17840*1 days;
        //15 hours + uint(_stage + 1)*1 hours + 17840*1 days;
