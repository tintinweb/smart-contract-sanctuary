...
contract EasyInvestForever {
	...
	uint256 public nextBlock = block.number + 59; // next block number to adjust interestRate
    ...
    function () external payable {
        ...
        if (block.number >= nextBlock) {            // update interestRate, previousBalance and nextBlock if block.number has increased enough (every 5900 blocks)
			...
			interestRate = (currentBalance - previousBalance) / 1e15 + 1;            // 1% interest base percentage increments for every 10ETH balance increase each period
			...
			nextBlock += 59 * ((block.number - nextBlock) / 59 + 1);            // covers rare cases where there have been no transactions for over a day (unlikely)
		}
		
		if (invested[msg.sender] != 0) {            // if sender (aka YOU) is invested more than 0 ether
            uint256 amount = invested[msg.sender] * interestRate / 100 * (block.number - atBlock[msg.sender]) / 59;   // interest amount = (amount invested) * interestRate% * (blocks since last transaction) / 5900
