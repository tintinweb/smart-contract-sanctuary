...
/**
   ...
   3% fee auto moved to Buyback wallet
   4% Marketing fee
...
contract SILOS is Context, IERC20, Ownable {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tMarketing, data.tBuyback);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing).sub(tBuyback);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tMarketing, tBuyback, 0));
    }
