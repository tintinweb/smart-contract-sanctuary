...
contract EightBitMafia is ERC721Enumerable, Ownable {
    ...
    {
        if (tokenId >= 1000) return string(abi.encodePacked(tokenId));

        if (tokenId >= 100) return string(abi.encodePacked("0", tokenId));

        if (tokenId >= 10) return string(abi.encodePacked("00", tokenId));

        return string(abi.encodePacked("000", tokenId));
    }
