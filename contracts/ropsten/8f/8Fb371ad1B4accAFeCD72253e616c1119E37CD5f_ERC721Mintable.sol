...
contract ERC721Mintable is ERC721Enumerable, AccessControl {
    ...
    bytes32 public constant BRIDGE_ROLE = keccak256("BRIDGE_ROLE"); // 52ba824bfabc2bcfcdf7f0edbb486ebb05e1836c90e78047efeb949990f72e5f
    ...
    modifier adminOrBridge() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(BRIDGE_ROLE, msg.sender), "Only admins or bridge");
        _;
    }
     ...
     */
    function mintTo(address _to, uint256 tokenId) external adminOrBridge {
        _mint(_to, tokenId);
    }
     ...
     */
    function mint(uint256 tokenId) external adminOrBridge {
        _mint(msg.sender, tokenId);
    }

    function setCoordinates(uint256 tokenId, uint256 latitude, uint256 longitude) external adminOrBridge {
        coordinates[tokenId] = Coordinate({latitude: latitude, longitude: longitude});
    }

    function getCoordinates(uint256 tokenId) external view returns(Coordinate memory) {
