...
contract Soup is ERC721URIStorage, Ownable {
        ...
        string memory json = encode(
            string(
                abi.encodePacked(
                    '{"name": "Plot #',
                    toString(tokenId),
                    '", "description": "Soup has it all, and more 002", "image": "data:image/svg+xml;base64,',
                    output,
                    '"}'
                )
            )
    ...
    function claim(uint256 tokenId) public payable {
        ...
        require(
            msg.value >= 3000000000000000,
            "Not enough ETH sent; check price!"
        );
        _safeMint(_msgSender(), tokenId);
    }

    function claimByGeohash(string calldata hash) public payable {
