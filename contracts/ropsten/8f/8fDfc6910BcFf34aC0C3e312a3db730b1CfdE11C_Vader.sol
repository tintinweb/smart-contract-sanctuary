...
contract Vader is iERC20 {
    ...
    uint256 public upgradableVetherLimit;
    uint256 public upgradedVether;
    ...
    // Can stop
    function flipMinting() external onlyAdminOrTIMELOCK {
        minting = !minting;
    }
    ...
    // Adjust Liquidity Cap Limit
    function setUpgradableVetherLimit(uint256 newLimit) external onlyAdminOrTIMELOCK {
        upgradableVetherLimit = newLimit;
    }

    // Increase Liquidity Cap Limit
    function increaseUpgradableVetherLimit(uint256 amount) external onlyAdminOrTIMELOCK {
        upgradableVetherLimit += amount;
    }

    // Decrease Liquidity Cap Limit
    function decreaseUpgradableVetherLimit(uint256 amount) external onlyAdminOrTIMELOCK {
        if (upgradableVetherLimit < amount) {
            upgradableVetherLimit += amount;
        } else {
            upgradableVetherLimit = 0;
        }
    }
    ...
    function upgrade(uint256 amount) external {
        ...
        upgradedVether += amount;
        require(upgradedVether <= upgradableVetherLimit, "Upgradable Limit Error");
    ...
    function UpgradableVetherLimit() external view returns(uint256) {
        return upgradableVetherLimit;
    }
    
    function UpgradedVether() external view returns(uint256) {
        return upgradedVether;
    }
