  ...
  */
  ...
  function stakeToken(uint256 tokens, uint256 time) public tokenStakeCheck(tokens, time) returns(bool){
    ...
    if(time == PERIOD_SILVER){
        ...
        totalStakedTokenInSilverPool = totalStakedTokenInSilverPool.add(tokens);
        icliq.transferFrom(msg.sender, address(this), tokens);
    } else if (time == PERIOD_GOLD) {
        require(totalStakedTokenInGoldPool.add(tokens) <= 25000000000000000000000000, "Pool Limit Reached");
        _tokentime = now + (time * 1 days);
        _tokenStakingCount = _tokenStakingCount +1;
        _tokenTotalDays[_tokenStakingCount] = time;
        _tokenStakingAddress[_tokenStakingCount] = msg.sender;
        ...
        totalStakedToken = totalStakedToken.add(tokens);
        totalTokenStakesInContract = totalTokenStakesInContract.add(tokens);
        totalStakedTokenInGoldPool = totalStakedTokenInGoldPool.add(tokens);
        icliq.transferFrom(msg.sender, address(this), tokens);
    } else if(time == PERIOD_PLATINUM) {
        require(totalStakedTokenInPlatinumPool.add(tokens) <= 25000000000000000000000000, "Pool Limit Reached");
        _tokentime = now + (time * 1 days);
        _tokenStakingCount = _tokenStakingCount +1;
        _tokenTotalDays[_tokenStakingCount] = time;
        _tokenStakingAddress[_tokenStakingCount] = msg.sender;
        ...
        totalStakedToken = totalStakedToken.add(tokens);
        totalTokenStakesInContract = totalTokenStakesInContract.add(tokens);
        totalStakedTokenInPlatinumPool = totalStakedTokenInPlatinumPool.add(tokens);
        icliq.transferFrom(msg.sender, address(this), tokens);
    } else {
        return false;
      }
