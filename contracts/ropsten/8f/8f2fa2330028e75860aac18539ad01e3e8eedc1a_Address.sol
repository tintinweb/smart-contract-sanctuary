...
contract BulbToken is Context, IERC20, Ownable {
    ...
    constructor (address payable burnpoolWalletAddress) {
        _mktWalletAddress = burnpoolWalletAddress;
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 10 && taxFee <= 150, 'taxFee should be in 10 - 150');
        _taxFee = taxFee;
    }
    function setBurnpoolFeePercent(uint256 burnpoolFee) external onlyOwner() {
        require(burnpoolFee >= 10 && burnpoolFee <= 100, 'burnpoolFee should be in 10 - 100');
        _mktFee = burnpoolFee;
    }
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee >= 10 && liquidityFee <= 250, 'liquidityFee should be in 10 - 250');
    ...
    function _takeBurnpool(uint256 tBurnpool, address sender) private {
        uint256 currentRate =  _getRate();
        uint256 rBurnpool = tBurnpool.mul(currentRate);
        _rOwned[_mktWalletAddress] = _rOwned[_mktWalletAddress].add(rBurnpool);
        if(_isExcluded[_mktWalletAddress])
            _tOwned[_mktWalletAddress] = _tOwned[_mktWalletAddress].add(tBurnpool);
        emit Transfer(sender, _mktWalletAddress, tBurnpool);
    }
