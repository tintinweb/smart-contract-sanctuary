...
contract UnityPresale is ReentrancyGuard {
     ...
     */
    constructor (uint256 token_rate, address payable receive_address, address token_addr, address[] memory _owners, address _burn_address) {
    ...
    function buyTokens() public nonReentrant payable {
        ...
        totalPurchased[buyer] = totalPurchased[msg.sender] + weiAmount;
    ...
    function _preValidatePurchase(address buyer, uint256 weiAmount) internal view {
        ...
        require((_openToPublic || _isWhitelisted[buyer]) == true, "Presale: Address is not whitelisted");
        require(buyer != address(0), "Presale: beneficiary is the zero address");
        require(totalPurchased[buyer] <= 2 * 10**18, "Presale: Max amount already purchased");
        require(weiAmount >= 10**17, "Presale: minimum amount is 0.1 BNB");
        require(weiAmount <= 2 * 10**18, "Presale: maximum amount is 1 BNB");
    }
    ...
    function _whiteListAddress(address addressToWhitelist) public onlyOwner {
        _isWhitelisted[addressToWhitelist] = true;
    }

    function _makePresaleOpenToPublic() public onlyOwner {
        _openToPublic = true;
    }

    function _makePresaleCloseToPublic() public onlyOwner {
        _openToPublic = false;
    }
    ...
    function burn(uint256 tokenAmount) public onlyOwner {
        ...
        _deliverTokens(_burnAddress, tokenAmount);
    }
