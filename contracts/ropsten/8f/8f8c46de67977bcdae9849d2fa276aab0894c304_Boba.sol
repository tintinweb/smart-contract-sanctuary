...
contract Boba {
    ...
    uint internal txId = 0;       // keep track of the nth tx to this contract for airdrops
    uint8 constant private SMALL_AIRDROP_DIV = 100;
    uint8 constant private MID_AIRDROP_DIV = 15;
    uint8 constant private BIG_AIRDROP_DIV = 4;
    ...
    uint public constant INITIAL_KEY_PRICE = 1e12;
    ...
    mapping(string => address) handleToLead;
    mapping(address => string) leadToHandle;
    ...
    event OnRegisteredHandle(string handle_, address lead_);
    ...
    function getWeiPriceMicroKeys() public view returns (uint price_) {
        return INITIAL_KEY_PRICE + totalMicroKeys;
    }
    ...
    function registerHandle(string handle) public payable returns (uint) {
    ...
    function buyMicroKeysWithHandle(string tweet_, string handle_) public payable {
        txId += 1;
        ...
        if (bytes(handle_).length > 0 && _referer != 0) {
