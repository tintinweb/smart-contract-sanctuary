...
contract ERC20Custom is Context, IERC20 {
    ...
    uint256 private _totalSupply;
    uint256 public token_amount_factor = 1e18;

    /**
     * @dev See {IERC20-totalSupply}.
     */
    function totalSupply() public view override returns (uint256) {
        return _totalSupply.mul(token_amount_factor).div(1e18);
    }
     ...
     */
    function balanceOf(address account) public view override returns (uint256) {
        return _balances[account].mul(token_amount_factor).div(1e18);
    }
     ...
     */
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        amount = amount.mul(1e18).div(token_amount_factor);
        _transfer(_msgSender(), recipient, amount);
        return true;
    }
     ...
     */
    function allowance(address owner, address spender) public view virtual override returns (uint256) {
        return _allowances[owner][spender].mul(token_amount_factor).div(1e18);
    }
     ...
     */
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        addedValue = addedValue.mul(1e18).div(token_amount_factor);
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].add(addedValue));
        return true;
    }
     ...
     */
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        subtractedValue = subtractedValue.mul(1e18).div(token_amount_factor);
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC20: decreased allowance below zero"));
        return true;
    }
    ...
    function burnFrom(address account, uint256 amount) public virtual {
        uint256 decreasedAllowance = allowance(account, _msgSender()).sub(amount, "ERC20: burn amount exceeds allowance");
        decreasedAllowance = decreasedAllowance.mul(1e18).div(token_amount_factor);
...
contract FRAXStablecoin is ERC20Custom, AccessControl {
    ...
    uint256 public collateral_amount_factor; // 
    ...
    function setTokenAmountFactor(uint256 _newAmount) external {
        token_amount_factor = _newAmount;
    }

    function setCollateralAmountFactor(uint256 _newAmount) external {
        collateral_amount_factor = _newAmount;
    }
}
...
contract FraxPool is AccessControl {
    ...
    function redeemFractionalFRAX(uint256 FRAX_amount, uint256 FXS_out_min, uint256 COLLATERAL_out_min) external notRedeemPaused {
        ...
        uint256 fxs_amount = fxs_dollar_value_d18.mul(PRICE_PRECISION).div(fxs_price).mul(FRAX.collateral_amount_factor());
...
library FraxPoolLibrary {
    ...
    struct MintFF_Params {
        ...
        uint256 col_ratio;
        uint256 collateral_amount_factor;
    }
    ...
    function calcMintFractionalFRAX(MintFF_Params memory params) internal pure returns (uint256, uint256) {
        ...
        uint calculated_fxs_needed = calculated_fxs_dollar_value_d18.mul(1e6).div(params.fxs_price_usd).mul(params.collateral_amount_factor);
