...
contract BaseV1Pair {
  ...
  uint256 immutable decimals0;
  uint256 immutable decimals1;
  ...
  uint256 public reserve0Last;
  uint256 public reserve1Last;
  ...
  constructor() {
    ...
    (address _token0, address _token1, bool _stable) = BaseV1Factory(msg.sender)
      .getInitializable();
    (token0, token1, stable) = (_token0, _token1, _stable);
    fees = address(new BaseV1Fees(_token0, _token1));
    uint256 _decimals0 = 1;
    uint256 _decimals1 = 1;
    if (_stable) {
  ...
  function _update(
    ...
    if (timeElapsed > periodSize) {
      observations.push(
        Observation(blockTimestamp, price0CumulativeLast, price1CumulativeLast)
      );
    }
  ...
  function swap(
    ...
    uint256 _balance1;
    {
      ...
      if (data.length > 0)
        IBaseV1Callee(to).hook(msg.sender, amount0Out, amount1Out, data); // callback, used for flash loans
      _balance0 = erc20(_token0).balanceOf(address(this));
      _balance1 = erc20(_token1).balanceOf(address(this));
    }
