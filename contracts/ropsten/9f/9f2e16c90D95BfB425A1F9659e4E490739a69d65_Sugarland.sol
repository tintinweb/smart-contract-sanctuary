...
contract Sugarland is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 45 * 10**12 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public antiWhaleAmt = _tTotal / 100;
    uint256 public swapTokensAtAmount = 10 * 10**6 * 10**_decimals;

    address public marketingAddress = 0x901f19780F2298a51B17578d5868732fF94e759e;
    ...
    function applySellFee() internal{
        feeRates.sellFee = extraSellFee;
    }
    ...
    function _takeSellFee(uint256 rSell, uint256 tSell) private {
        totFeesPaid.sell += tSell;
        
        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tSell;
        }
        _rOwned[address(this)] +=rSell;
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rMarketing, to_return.rLiquidity, to_return.rSell) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tSell = tAmount * feeRates.sellFee/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tSell;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity, uint256 rSell) {
        ...
        rSell = s.tSell*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rSell;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity, rSell);
    }
    ...
    function resetSellFee() internal{
        feeRates.sellFee = 0;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!swapping && swapEnabled && canSwap && from != pair){
            uint256 toSwap = swapTokensAtAmount.div(feeRates.liquidity + feeRates.marketing).div(feeRates.liquidity);
            swapAndLiquify(toSwap);
            uint256 marketingTokens = swapTokensAtAmount - toSwap;
            swapToMarketing(marketingTokens);
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        _takeSellFee(s.rSell, s.tSell);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity);
        emit Transfer(sender, address(this), s.tMarketing);
    }
    
    function swapToMarketing(uint256 tokens) private lockTheSwap{ 
        swapTokensForBNB(tokens, marketingAddress);
    }
    ...
    function updateExtraSellFee(uint256 newFee) external onlyOwner{
        extraSellFee = newFee;
    }
