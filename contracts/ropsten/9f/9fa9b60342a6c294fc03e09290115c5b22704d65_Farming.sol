...
contract Farming is Ownable, ReentrancyGuard {
	...
	function harvest(uint256 _pid) public nonReentrant {
		...
		require(rewardAmount > 0, "no claimable reward");
	...
	function _claim(uint256 _pid, address account) internal {
		...
		require(user.stakingAmount > 0, "no balance to withdraw");
	...
	function updatePoolLP(uint _pid, address _lptoken) external onlyOwner {
		...
		_updatePool(_pid);
		require(pool.lptotal == 0 , "Pool needs staked balance");
	...
	function user_blocks_before_claim(uint _pid, address _address) public view returns (int256) {
    	int256 amount = int(get_user_blockstart(_pid,_address)) + 60000 - int(block.number);
    	return amount;
	}
