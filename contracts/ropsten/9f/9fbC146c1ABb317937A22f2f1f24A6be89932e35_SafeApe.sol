...
contract SafeApe is Ownable {
    ...
    function stake(uint256 _pid, uint256 _amount) public payable {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        
        require( pool.apingTime < now, "stake: stake is finished");
        require( now >= pool.startTime, "stake: stake is not started");
    ...
    function endApe(uint256 _pid) external onlyOwner {
        uint256 price = getTokenPrice(_pid);
        require (price >= poolInfo[_pid].endPrice || poolInfo[_pid].endTime < now, "endApe: Ape can't be finished.");

        uint256 feeAmount = poolInfo[_pid].assignedAmount.mul(poolInfo[_pid].feeRate).div(10000);
        distributionAmount = poolInfo[_pid].assignedAmount.sub(feeAmount);

        poolInfo[_pid].finalized = true;
        emit FinalizedApe(_pid);
    }

    function getTokenPrice(uint256 _pid) public view returns (uint256) {
        uint256 price;
        uint256 baseValue = 10**18;
        IUniswapV2Pair pair = IUniswapV2Pair(poolInfo[_pid].lpAddress);
        (uint Res0, uint Res1,) = pair.getReserves();
        IERC20 tokenA = IERC20(pair.token0());
        IERC20 tokenB = IERC20(pair.token1());

        if (tokenA == stakingToken){
            uint256 rate = baseValue.mul(tokenB.decimals()).div(tokenA.decimals());
            price = rate.mul(Res1).div(Res0);
            
        }else if ( tokenB == stakingToken){
            uint256 rate = baseValue.mul(tokenA.decimals()).div(tokenB.decimals());
            price = rate.mul(Res0).div(Res1);
        }
    ...
    function claim(uint256 _pid) public {
        ...
        uint256 claim_amount = distributionAmount.mul(user.stakeAmount).div(pool.stakeAmount);
        if(claim_amount > 0) {
    ...
    function withdrawFee(uint256 _pid) external onlyOwner {
        uint256 balanceApe = poolInfo[_pid].apeToken.balanceOf(address(this));
        require(balanceApe > 0 , "widrawFee: There is no withdrawable tokens");
        if (balanceApe < distributionAmount)
            distributionAmount = balanceApe;
        uint256 length = feeInfo.length;
        for (uint256 fid = 0; fid < length; ++fid) {
            FeeInfo storage fee = feeInfo[fid];
            uint256 feeAmount = distributionAmount.mul(fee.feeRate).mul(distributionAmount).div(10000);
            poolInfo[_pid].apeToken.transfer(fee.wallet, feeAmount);
        }
    }

    function getApeStatus(uint256 _pid) public view returns (uint256) {
        uint256 status;
        uint256 price = getTokenPrice(_pid);
