...
contract KYC is Ownable{
    ...
    */
    ...
    function setTokenExpectedFee(address _customerAddress, uint expectedFee, address tokenContractAddress) public onlyOwner
        isCustomerAdded(_customerAddress) returns(bool){
            Customer storage c = customers[_customerAddress];
            c.count = c.count+1;
            c.expectedInfo[c.count]=ExpectedTransfer(true, false, expectedFee, now, false, tokenContractAddress);
            return true;
    }
    ...
    function tokenTransfertoKYC(address _partnerAddress, address tokenAddress, uint tokenAmount) public onlyOwner isAvailableTokenBalance(tokenAddress, tokenAmount) returns(bool){
        require(partnerAddress == _partnerAddress);
        ERC20(tokenAddress).transfer(partnerAddress, tokenAmount);
        return true;
    }
    
    function ethTransfertoKYC(address _partnerAddress, uint amount) public onlyOwner isAvailableETHBalance(amount) returns(bool){
        require(partnerAddress == _partnerAddress);
    ...
    function refundETHToCustomer(address _address, uint refundAmount) public onlyOwner isCustomerAdded(_address) isAvailableETHBalance(refundAmount) returns(bool){
        require(refundAmount > 0);
        Customer storage c = customers[_address];
        require(c.expectedInfo[c.count].isPaid);
        require(c.expectedInfo[c.count].isEth);
        require(!c.expectedInfo[c.count].isUsed);
        require(c.expectedInfo[c.count].expectedFee > refundAmount);
