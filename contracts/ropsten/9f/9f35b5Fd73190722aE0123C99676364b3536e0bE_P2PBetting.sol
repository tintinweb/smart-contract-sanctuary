...
abstract contract IERC721Receiver {
    ...
    function onERC721Received(address operator, address from, uint256 tokenId, bytes memory data)  external virtual returns (bytes4);
}
...
contract P2PBetting is Ownable {
    ...
    mapping ( uint256 => PlayerDetails ) private _playerDetails;
    ...
    constructor(address payable _wallet, address _signer) {
        wallet = _wallet;
        signer = _signer;
    ...
    function viewPlayerDetails(uint256 _playerID) external view returns(PlayerDetails memory){
        return _playerDetails[_playerID];
    }

    function viewPlayerIDs(address _playerAddress) external view returns(UserIDs memory){
        return _userID[_playerAddress];
    }
    ...
    function claimTokens(uint256 _playerID, uint256 _pairID, uint256 blockTime, uint256 flag,  uint8 v, bytes32 r, bytes32 s) external payable {
        ...
        uint256 anotherPlayerID;
        if(pair.creatorID == _playerID) { anotherPlayerID = pair.joinerID; }
        else if (pair.joinerID == _playerID) { anotherPlayerID = pair.creatorID; }
        PlayerDetails storage player2 = _playerDetails[anotherPlayerID];
        ...
        if(flag == 0){
            ...
            require(payable(wallet).send(msg.value),"Fee Transaction Failed");
