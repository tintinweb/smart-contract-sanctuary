...
contract RapidGainz is ERC20 {
  ...
  bool private isLaunchFinished = false;
  ...
  function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
    if(isLaunchFinished){
        uint256 zeroPointOnePercentOfSupply = _totalSupply.mul(maxTransferAmountInPercentage).div(1000);
        require(amount <= zeroPointOnePercentOfSupply);
    }
