...
contract RewardsDistributionRecipient {
    ...
    function notifyRewardAmount(uint256 reward,uint256 reward1, uint256 duration) external;
...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    function earned1(address account) public view returns (uint256) {
        return _balances[account].mul(reward1PerToken().sub(userReward1PerTokenPaid[account])).div(1e18).add(rewards1[account]);
    }
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        uint256 reward1 = rewards1[msg.sender].mul(1e14);
        if (reward > 0) {
        ...
        if (reward1 > 0) {
            ...
            emit Reward1Paid(msg.sender, reward1);
        }
    }
    ...
    function notifyRewardAmount(uint256 reward,uint256 reward1, uint256 rewardsDuration) external onlyRewardsDistribution updateReward(address(0)) {
        ...
        if (block.timestamp >= periodFinish) {
            ...
            rewardRate1 = reward1.add(leftover).div(rewardsDuration);
        }
        ...
        emit Reward1Added(reward1, periodFinish);
    }
    ...
    event Reward1Added(uint256 reward1, uint256 periodFinish);
    event Staked(address indexed user, uint256 amount);
    event Withdrawn(address indexed user, uint256 amount);
    event RewardPaid(address indexed user, uint256 reward);
    event Reward1Paid(address indexed user, uint256 reward1);
}
...
contract StakingRewardsFactory is Ownable {
    ...
    function deploy(address stakingToken, uint rewardAmount, uint rewardAmount1, uint256 rewardsDuration) public onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards == address(0), 'StakingRewardsFactory::deploy: already deployed');

        info.stakingRewards = address(new StakingRewards(/*_rewardsDistribution=*/ address(this), rewardsToken, rewardsToken1, stakingToken));
    ...
    function update(address stakingToken, uint rewardAmount, uint rewardAmount1, uint256 rewardsDuration) public onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
    ...
    function notifyRewardAmount(address stakingToken) public {
        ...
        if (info.rewardAmount > 0 && info.rewardAmount1 > 0 && info.duration > 0) {
            ...
            require(
                IERC20(rewardsToken1).transfer(info.stakingRewards, rewardAmount1),
                'StakingRewardsFactory::notifyRewardAmount: transfer failed'
            );
            StakingRewards(info.stakingRewards).notifyRewardAmount(rewardAmount,rewardAmount, duration);
        }
    }
