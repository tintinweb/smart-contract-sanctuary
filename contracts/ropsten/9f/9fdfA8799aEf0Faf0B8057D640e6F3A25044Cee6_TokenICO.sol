...
/**
 * @title TokenICO
 * @dev TokenICO contract is Ownable
 **/
contract TokenICO is Ownable {
  ...
  uint256 public START; // Sep 14, 2021 @ 12:00 EST
  uint256 public constant DAYS = 45; // 45 Day
  uint256 public maxBuyAmount = 5 * 10**16; // 0.05 ETH
  uint256 public minBuyAmount = 5 * 10**15; // 0.005 ETH
  ...
  /**
   * TokenICO
   * @dev TokenICO constructor
   **/
  constructor(address _tokenAddr) {
      token = ICOToken(_tokenAddr);
  ...
  function isActive() public view returns (bool) {
    return (
        ...
        block.timestamp <= START.add(DAYS * 10 days) && // Must be before the end date
  ...
  function buyTokens() public payable whenSaleIsActive {
    ...
    require(weiAmount >= minBuyAmount, 'Eth value is more than 0.005ETH');
    require(weiAmount <= maxBuyAmount, 'Eth value is less than 0.05ETH');
