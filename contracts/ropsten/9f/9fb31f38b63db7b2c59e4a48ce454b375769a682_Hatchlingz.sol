...
contract Hatchlingz is ERC721, ERC721Enumerable, Ownable {
   ...
   //   SET BACK TO FALSE BEFORE
    ...
    function setAllowList(address[] calldata addresses, uint8 numAllowedToMint) external onlyOwner {
     ...
     function setPhoenixMetadata(string[] calldata metadata) external onlyOwner {
        for (uint256 i = 0; i < metadata.length; i++) {
            _phoenixMetadata[i] = metadata[i];
        }
    }
    
      function setDragonMetadata(string[] calldata metadata) external onlyOwner {
        for (uint256 i = 0; i < metadata.length; i++) {
            _dragonMetadata[i] = metadata[i];
        }
    }
    
      function setChickenMetadata(string[] calldata metadata) external onlyOwner {
        for (uint256 i = 0; i < metadata.length; i++) {
