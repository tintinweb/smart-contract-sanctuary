...
//            try controllingToken.balanceOf(queue[i]) returns (uint256 bal) {
//                dynaHodler[queue[i]][currentRound].tokenBalance = bal;
//                if(isExcludedFromReflections[queue[i]]){
//                    excludedTokensSnapshotted = excludedTokensSnapshotted.add(bal);
//                    emit UserSnapshot(queue[i], bal);
//                }
//            } catch {}
            (bool success, bytes memory data) = address(controllingToken).call(abi.encodeWithSignature("balanceOf(address)",queue[i]));
            if(success){
