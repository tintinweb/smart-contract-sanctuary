...
contract Crowdsale is Finalizable{
  ...
  event WhitelistedStatusUpdated(address indexed _address, bool _status);
 ...
 // Allows the owner or admin to add/remove people from the whitelist.
   function addWhitelist(address _address) public onlyOwnerOrAdmin returns (bool) {
      return addWhitelistInternal(_address, true);
   }


   function addWhitelistInternal(address _address, bool _status) private returns (bool) {
      ...
      emit WhitelistedStatusUpdated(_address, _status);
   ...
   function setWhitelistedBatch(address[] _addresses) public onlyOwnerOrAdmin returns (bool) {
      require(_addresses.length > 0);

      for (uint256 i = 0; i < _addresses.length; i++) {
         require(addWhitelistInternal(_addresses[i], true));
      }
