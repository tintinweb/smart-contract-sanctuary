...
abstract contract LotteryWinner {
    ...
    function addWinner(address winner, uint256 amount, uint256 time, uint256 currentIndex) internal returns (uint256 index) {
        ...
        if (lastWinners.length > 2) {
            lastWinners[0] = lastWinners[1];
            lastWinners[1] = lastWinners[2];
            lastWinners[2] = winnerInfo;
        } else {
    ...
    function getLastWinners() external view returns(uint256[] memory, uint256[] memory, address[] memory, uint256[] memory) {
        uint256 len = lastWinners.length;
        
        uint256[] memory amount = new uint256[](len);
        uint256[] memory time = new uint256[](len); 
        address[] memory winner = new address[](len);
        uint256[] memory index = new uint256[](len);
        
        for (uint256 i = 0; i < len; i++) {
            amount[i] = lastWinners[i].amount;
            time[i] = lastWinners[i].time;
            winner[i] = lastWinners[i].winner;
            index[i] = lastWinners[i].index;
        }
        
        return(amount, time, winner, index);
    }
}
