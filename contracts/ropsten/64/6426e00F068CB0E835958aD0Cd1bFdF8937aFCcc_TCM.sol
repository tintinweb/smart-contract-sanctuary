...
contract TCM is ERC721, ERC721Enumerable, Ownable, Salable {
    ...
    mapping(uint256 => bool) private tokensUsedByMintPass;

    constructor(address newMintPassAddress) ERC721("TexasChainsawMassacre", "TCM") {
        isPublicSaleActive = false;
        isWhitelistSaleActive = false;
        maxBuyAtOncePublic = 20;
        maxBuyAtOncePresale = 5;

        //0.0666 Ethereum
        mintCost = 66600000000000000;
        maxNFTCirculation = 10666;
    ...
    //The number we allow to be minted in one function call
    function setMaxBuyAtOncePublic(uint256 newMaxMintAtOnce) external onlyOwner {
        maxBuyAtOncePublic = newMaxMintAtOnce;
    }

    //The number we allow to be minted during the presale
    function setMaxBuyAtOncePresale(uint256 newMaxBuyAtOncePresale) external onlyOwner {
        maxBuyAtOncePresale = newMaxBuyAtOncePresale;
    }
    ...
    function mintInPresale(uint256 numberToBuy) external payable onlyWhenPreSaleIsActive {
        ...
        require(mintPass.balanceOf(msg.sender) > 0, "TCM: Does not own a mint pass");

        //Get the first token this user owns, and log it so it can't be used again
        uint256 mintPassTokenIndex = mintPass.tokenOfOwnerByIndex(msg.sender, 0);
        require(tokensUsedByMintPass[mintPassTokenIndex] == false, "TCM: Mint Pass already used");
