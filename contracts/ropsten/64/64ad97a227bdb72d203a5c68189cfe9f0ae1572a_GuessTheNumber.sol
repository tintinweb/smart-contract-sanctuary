...
contract GuessTheNumber 
{
    ...
    // записываем инфу о том, что игра запрещена/разрешена
    event isGameAllowed(string);
    ...
    constructor(uint80 _minBet, uint80 _maxBet, bool _gameAllowed ){
        minBet = _minBet;
        maxBet = _maxBet;
        if(_gameAllowed == true){
            emit isGameAllowed("game allowed");
        }
        else{
            emit isGameAllowed("game not allowed");
        }
        gameAllowed = _gameAllowed;
        owner = msg.sender;
    }
    ...
    function playGuessingGame (string memory _playerName, uint8 _number)public payable{
        // игра должна быть разрешена
        // либо игра запрещена, но кто-то уже сделал ход
        require(gameAllowed == true || (gameAllowed == false && players.length != 0));
        // проверяем ставку игрока
    ...
    // разрешаем или запрещаем игру
    function setGameAllowed(bool _gameAllowed)public onlyOwner(msg.sender){
        if(_gameAllowed == true){
            emit isGameAllowed("game allowed");
        }
        else{
