...
contract HeadShoot is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1 * 10**6 * 10**_decimals;
    ...
    */
    ...
    function setThresholdForLP(uint256 threshold) external onlyOwner {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        reflectDevFee(s.tDev);

        emit Transfer(sender, recipient, s.tTransferAmount);
    }

    function reflectDevFee(uint256 tDev) private {
        uint256 currentRate =  _getRate();
