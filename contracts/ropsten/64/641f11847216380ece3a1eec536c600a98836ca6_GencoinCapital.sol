...
contract GencoinCapital is Context, IERC20, Ownable {
    ...
    //uint256 public _maxWalletToken = _tTotal.div(1000).mul(20); //4% 

    uint256 public _Launch_Phase1maxWalletToken = _tTotal.div(1000).mul(100); //1%
    uint256 public _maxWalletToken = _tTotal.div(1000).mul(100); //4%     
    ...
    function _transfer(
        ...
        if(!_isMaxWalletExempt[to] && from != owner()){
            if(block.timestamp <= launchedAtTime + 12 hours) {
