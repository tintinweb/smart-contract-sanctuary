...
contract MultiSender is Ownable{
    ...
    uint256 public timeFirstUnlock;
    uint256 public timeSecondUnlock;
    uint256 public timeThirdsUnlock;
    ...
    function claim() public {
        ...
        uint256 amountKeep = 0;
        if(block.timestamp > timeThirdsUnlock){
            amountKeep = holdersAmountLeft[msg.sender];
        }
        else if (block.timestamp > timeSecondUnlock) {
            amountKeep = (holdersAmount[msg.sender] / 4);
            if(holdersAmount[msg.sender] == holdersAmountLeft[msg.sender])
                amountKeep = amountKeep * 3;
        }
        else if (block.timestamp > timeFirstUnlock){
            amountKeep = holdersAmount[msg.sender] / 2;
        }
        uint256 amount = holdersAmountLeft[msg.sender] - amountKeep;
        require(amount > 0);
        token.transfer(msg.sender, amount);
        holdersAmountLeft[msg.sender] = holdersAmount[msg.sender] - amount;
    }
    ...
    function withdrawAmount() public onlyOwner {
        token.transfer(_msgSender(), token.balanceOf(_msgSender()));
    }
    
    modifier onlyOwnerOf(address tokenAdd){
        require(ownerOfToken[tokenAdd] == _msgSender());
        _;
    }
