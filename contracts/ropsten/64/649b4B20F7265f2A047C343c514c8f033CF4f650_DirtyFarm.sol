...
contract DirtyFarm is Ownable, Authorizable, ReentrancyGuard {
    ...
    uint256 public rewardSegment = poolReward.mul(100).div(200);
    ...
    function updateVariablePoolReward() private {
        ...
        if (stakeratio == lpratio) { //ratio of pool rewards should remain the same (65 lp, 35 stake)
            adjustPools(0, 65, true);
            adjustPools(1, 35, true);
        }

        if (stakeratio > lpratio) {
            ratio = SafeMath.div(stakeratio, lpratio, "stakeratio > lpratio division error");
 
            if (ratio > 12000000000000000000) {

                 adjustPools(0, 5, true);
                 adjustPools(1, 95, true);
             }
             
             if (ratio > 11000000000000000000 && ratio <= 12000000000000000000) {
                   
                 adjustPools(0, 10, true);
                 adjustPools(1, 90, true);
             }

            if (ratio > 10000000000000000000 && ratio <= 11000000000000000000) {
                  
                 adjustPools(0, 15, true);
                 adjustPools(1, 85, true);
             }

            if (ratio > 9000000000000000000 && ratio <= 10000000000000000000) {
                 
                 adjustPools(0, 20, true);
                 adjustPools(1, 80, true);
             }
            
            if (ratio > 8000000000000000000 && ratio <= 9000000000000000000) {
                 
                 adjustPools(0, 25, true);
                 adjustPools(1, 75, true);
             }

            if (ratio > 7000000000000000000 && ratio <= 8000000000000000000) {
                 
                 adjustPools(0, 30, true);
                 adjustPools(1, 70, true);
             }

            if (ratio > 6000000000000000000 && ratio < 7000000000000000000) {
                 
                 adjustPools(0, 35, true);
                 adjustPools(1, 65, true);
             }

            if (ratio > 5000000000000000000 && ratio <= 6000000000000000000) {
                 
                 adjustPools(0, 40, true);
                 adjustPools(1, 60, true);
             }
            
            if (ratio > 4000000000000000000 && ratio <= 5000000000000000000) {
                 
                 adjustPools(0, 45, true);
                 adjustPools(1, 55, true);
             }

            if (ratio > 3000000000000000000 && ratio <= 4000000000000000000) {
                 
                 adjustPools(0, 50, true);
                 adjustPools(1, 50, true);
             }

            if (ratio > 2000000000000000000 && ratio <= 3000000000000000000) {
                 
                 adjustPools(0, 55, true);
                 adjustPools(1, 45, true);
             }

            if (ratio > 1000000000000000000 && ratio <= 2000000000000000000) {
                 
                 adjustPools(0, 60, true);
                 adjustPools(1, 40, true);
             }
        ...
        if (lpratio > stakeratio) {
            ...
            if (ratio > 12000000000000000000) {
                 ...
                 adjustPools(1, 5, true);
                 adjustPools(0, 95, true);
             }
 
            if (ratio > 11000000000000000000 && ratio <= 12000000000000000000) {
                 
                 adjustPools(1, 10, true);
                 adjustPools(0, 90, true);
             }
             
              if (ratio > 10000000000000000000 && ratio <= 11000000000000000000) {
                  
                 adjustPools(1, 15, true);
                 adjustPools(0, 85, true);
             }

            if (ratio > 9000000000000000000 && ratio <= 10000000000000000000) {
                 
                 adjustPools(1, 20, true);
                 adjustPools(0, 80, true);
             }

            if (ratio > 8000000000000000000 && ratio <= 9000000000000000000) {
                  
                 adjustPools(1, 25, true);
                 adjustPools(0, 75, true);
             }
            
            if (ratio > 7000000000000000000 && ratio <= 8000000000000000000) {
                  
                 adjustPools(1, 30, true);
                 adjustPools(0, 70, true);
             }
