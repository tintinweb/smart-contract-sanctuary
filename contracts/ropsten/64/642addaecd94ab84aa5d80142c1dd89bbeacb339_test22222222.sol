...
contract test22222222 is Context, IERC20, Ownable {
    ...
    // 
    string private _name = "test22222222";                                           // name
    string private _symbol = "test22222222";                                            // symbol
    ...
    uint256 private LaunchTime = block.timestamp;
    bool public LaunchingSoon = false;
    ...
    function _transfer(
        ...
        //Limitation on the maximum wallet balance (for first 12 hours)
        if(!_isExcludedFromFee[to] && to != uniswapV2Pair){
            if(timestamp[to] < LaunchTime + 30 minutes){
                require(balanceOf(to).add(amount) <= maxWalletBalance, 'Recipient balance is exceeding maxWalletBalance! Try again after 12 hours post launch');
            }
        }        
    ...
    function setLaunchingSoon(bool _LaunchingSoon) external onlyOwner() {
        LaunchingSoon = _LaunchingSoon;
