...
contract Datagold is ERC20Interface, SafeMath{
    ...
    */
    ...
    returns (bool success)
    {
        ...
        transfer(address(this), dgcAmt);
        transferDAT_toClient(safeMul(dgcAmt, 100));
        
        return true;
    }    
    
    function transferDAT_toClient(uint datAmt)
    private
    returns(bool success)
    {  
         ERC20Interface DAT = ERC20Interface(_DATcontract);
         require(DAT.balanceOf(address(this)) >= datAmt, "The DGC contract does not have enough DAT balance for this swap.");
         DAT.transfer(tx.origin, datAmt);
    ...
    returns (bool success)
    {
        ...
        DAT.transfer(address(this), datAmt);
        
        transferDGC_toClient(safeDiv(datAmt, 100));
        
        return true;
    }
    
    function transferDGC_toClient(uint dgcAmt)
    private
    returns(bool success)
    {
        require(balanceOf(address(this)) >= dgcAmt, "The DGC contract does not have enough DGC balance for this swap.");
        transfer(tx.origin, dgcAmt);
