...
// File: contracts/utils/SlidingWindowOracle.sol
...
contract SlidingWindowOracle2_Flatterned {
    using FixedPoint for *;
    using SafeMath for uint;
    ...
    function computeAvgPrice(
        ...
        uint returnPrice = priceAverage.mul(1).decode144();
    ...
    function consult(address tokenIn, address tokenOut) external view returns (uint amountOut) {
        ...
        if (token0 == tokenIn) {
            return computeAvgPrice(firstObservation.price0Cumulative, price0Cumulative, timeElapsed);
        } else {
            return computeAvgPrice(firstObservation.price1Cumulative, price1Cumulative, timeElapsed);
        }
    }
}