...
contract Seekreward {
    ...
    constructor(address  _own1, address  _own2, address token) public {
        admin1 = _own1;
        admin2 = _own2;
    ...
    function dep(uint amount)  external {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        _token.transfer(admin1, adminFee.div(2));
        emit Adminshare(_addr, admin1, adminFee.div(2), block.timestamp);
        _token.transfer(admin2, adminFee.div(2));
        emit Adminshare(_addr, admin2, adminFee.div(2), block.timestamp);
        adminFee = 0;
        _refAmount(_addr, users[_addr].upline, _amount);
    }

    function deposit(address _upline, uint amount) isLock isContractcheck(msg.sender) external {
    ...
    function withdraw() isLock external {
        ...
        _token.transfer(msg.sender, to_payout);
