...
contract Shop
{
    ...
    function deleteGood(string memory _name) public payable returns(uint)
    {
        // Только хозяин корзины может вызвать эту функцию
        require(owner == msg.sender);
    ...
    function deleteGoodfromCart(string memory _name) public payable returns(uint)
    {
        // Только хозяин корзины может вызвать эту функцию
        for(uint32 i = 0; i < carts[msg.sender].goodsName.length; i++){
            if(keccak256(bytes(carts[msg.sender].goodsName[i])) == keccak256(bytes(_name))){
                delete carts[msg.sender].goodsName[i];
                delete carts[msg.sender].goodsCount[i];
            }
        }
    }
    
    function changeAmountfromcart(string memory _name, uint16 _count) public payable returns(uint)
    {
        // Только хозяин корзины может вызвать эту функцию
        for(uint32 i = 0; i < carts[msg.sender].goodsName.length; i++){
            if(keccak256(bytes(carts[msg.sender].goodsName[i])) == keccak256(bytes(_name))){
