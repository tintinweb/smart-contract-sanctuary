...
contract CulturedOrangutans is ERC721, Ownable  {
    ...
    event CulturedOrangutansPriceChanged(uint256 price);
    event MaxTokenAmountChanged(uint256 value);
    event MaxPurchaseChanged(uint256 value);
    event CulturedOrangutansReserved();
    ...
    function reserveCulturedOrangutans() public onlyOwner onReserve {
    ...
    modifier onReserve() {
        ...
        _;
        reserved = true;
        emit CulturedOrangutansReserved();
    }
   ...
   function withdraw() public onlyOwner {
    ...
    constructor() ERC721("Cultured Orangutans", "ORANGS") {
    ...
    function mint(uint amount) public payable{
        require(amount > 0);
        require(amount < 21);
        require(_tokenIdCounter.current() + amount <= maxTokens, "Purchase would exceed max supply of Cultured Orangutans");
        require(tokenPrice * amount == msg.value, "Ether value sent is not correct");
        if(!mainSaleActive){
            require(_tokenIdCounter.current() + amount <= preSaleTokens, "Purchase would exceed max pre-sale supply of Cultured Orangutans");
        }
