...
contract TokenGeyser is IStaking, Ownable {
    ...
    function pendingUserReward(address user) public view returns (uint256) {
        ...
        uint256 rewardAmount = 0;
//        while (sharesLeftToBurn > 0) {
        uint256 length = accountStakes.length;
        for (uint256 k = 0; k < length; k++) {
            Stake storage lastStake = accountStakes[accountStakes.length - 1 - k];
            uint256 stakeTimeSec = now.sub(lastStake.timestampSec);
            uint256 newStakingShareSecondsToBurn = 0;
            // fully redeem a past stake
            newStakingShareSecondsToBurn = lastStake.stakingShares.mul(stakeTimeSec);
    ...
    function totalUserReward(address user) public view returns (uint256) {
        // Global accounting
        uint256 newStakingShareSeconds =
            now
            .sub(_lastAccountingTimestampSec)
            .mul(totalStakingShares);
        uint256 totalSharesSeconds = _totalStakingShareSeconds.add(newStakingShareSeconds);
        ...
        uint256 newUserStakingShareSeconds =
            ...
            .sub(totals.lastAccountingTimestampSec)
            .mul(totals.stakingShares);
        uint256 totalUserShareSeconds =
            totals.stakingShareSeconds
            .add(newUserStakingShareSeconds);

        uint256 totalUserRewards = (totalUserShareSeconds > 0)
            ? totalUnlocked().mul(totalUserShareSeconds).div(totalSharesSeconds)
            : 0;
