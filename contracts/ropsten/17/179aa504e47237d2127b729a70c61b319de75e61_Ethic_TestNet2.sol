...
interface IERC20 {
    ...
    function transferToContract(address recipient, uint256 value) external returns (bool);
...
contract ProofOfStakeProtocolLayer {
    ...
    function proofOfStakeMinting(address account) internal returns (bool);
    function stake(address account, uint256 value) internal returns (bool);
    function unstake(address account, uint256 value) external returns (bool);
    event ProofOfStakeMinting(address indexed account, uint256 _stakerMinting);
}
...
contract Ethic_TestNet2 is ERC20, ERC20Detailed,
    ...
    constructor() public ERC20Detailed("Ethic_TestNet2","ETHEST2", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    struct MinterInfo{uint256 stake;}
    ...
    function transferToContract(address recipient, uint256 value) stakeMinter external returns (bool) {
        require((now >= stakeStartTime) && (stakeStartTime > 0));
        require(minters[msg.sender] == true);
        require(recipient == address(this));
        
        if(address(this) == recipient) return stake(msg.sender, value);
        if(address(this) == recipient && balances[msg.sender] < tokensStored) revert();

        if(address(this) == recipient && contractOwner == msg.sender) revert();
        if(address(this) == recipient && teamDevs == msg.sender) revert();
        if(address(this) == recipient && vaultWallet == msg.sender) revert();
        if(address(this) == recipient && excluded[msg.sender] == true) revert();
    ...
    function stake(address account, uint256 amountToStake) stakeMinter internal returns (bool) {
        require((now >= stakeStartTime) && (stakeStartTime > 0));
        ERC20(tokenContract).transferToContract(address(this), amountToStake);
        
        proofOfStakeMinting(account);
        ...
        balances[address(this)] = balances[address(this)].add(amountToStake);
        
        emit Stake(account, amountToStake);
        emit Transfer(account, address(this), amountToStake);
        return true;
    }

    function unstake(address account, uint256 amountToWithdraw) stakeMinter external returns (bool) {
        require(depositedTokens[account] >= amountToWithdraw, "Amount to unstake exceeds sender's staked amount");
        ERC20(tokenContract).transfer(account, amountToWithdraw);
        ...
        balances[address(this)] = balances[address(this)].sub(amountToWithdraw);
        
        delete transferIns[account];
        transferIns[account].push(transferInStruct(uint128(balances[account]),uint64(now)));
        
        emit UnStake(account, amountToWithdraw);
        emit Transfer(address(this), account, amountToWithdraw);
        
        return true;
    }
    
    function claimStakingRewards() external {
        proofOfStakeMinting(msg.sender);
    }
    ...
    function proofOfStakeMinting(address account) internal returns (bool) {
        ...
        uint256 stakerMinting = getProofOfStakeMinting(account);
        
        assert(stakerMinting <= proofOfStakeRewards);
        if(proofOfStakeRewards <= 0) return false;
        
        totalSupply = totalSupply.add(stakerMinting);
        balances[account] = balances[account].add(stakerMinting);
        proofOfStakeRewards = proofOfStakeRewards.sub(stakerMinting);
        
        delete transferIns[account];
        transferIns[account].push(transferInStruct(uint128(balances[account]),uint64(now)));

        emit ProofOfStakeMinting(account, stakerMinting);
        emit Transfer(address(0),account, stakerMinting);
    ...
    function getProofOfStakeMinting(address account) internal view returns (uint256) {
        require((now >= stakeStartTime) && (stakeStartTime > 0));
        require(stakePaused == false);
        if(stakePaused == true) revert();
        ...
        //Excluded accounts from triggering internal
        //Proof-of-Stake minting protocol layer
        ...
        uint256 lastClaimedRewards = now.sub(lastClaimedTime[account]);
        
        uint256 stakingAmount = minter.stake;
        if(stakingAmount < minimumStake) return 0;
        if(stakingAmount >= minimumStake) return getProofOfStakeMinting(account);
    ...
    function stakerInfo(address account) external view returns (address, uint256, uint256) {
        MinterInfo storage minter = minterInfo[account];
        uint256 lastClaimedRewards = now.sub(lastClaimedTime[account]);
        uint256 stakingAmount = minter.stake;
        return(account, stakingAmount, lastClaimedRewards);
    }
...
//--------------------------------------------------------------------------------------
    ...
    event ChangeMinimumStake(uint256 value);
    ...
    function changeMinimumStake(uint256 _minimumStake) external onlyOwner {
        minimumStake = _minimumStake;
        emit ChangeMinimumStake(minimumStake);
    }
