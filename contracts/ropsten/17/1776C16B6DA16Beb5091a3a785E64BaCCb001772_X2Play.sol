 ...
 */
 ...
 contract X2Play is Ownable {
     ...
     function _invest(address from, uint256 amount) internal {
         ...
         if (queue[cycle][queue[cycle].length-1].start == 0) {
             queue[cycle][queue[cycle].length-1].start = uint32(now);
         } else if (users[from].batches.length > 1) {
             if (
                 users[from].batches[users[from].batches.length-1].cycle == cycle
                 &&
                 users[from].batches[users[from].batches.length-1].index == queue[cycle].length-1
                 ) {
