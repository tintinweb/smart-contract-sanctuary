...
contract TronMah {
  ...
  function withdrawInv() external {
    ...
    uint amount = this.revenueOf(msg.sender, block.timestamp);
            uint256 amounts = amount +  user.unwithdrawn_bonuse;
    if (payable(msg.sender).send(amounts)) {
      users[msg.sender].withdrawn += amounts;
      total_users_withdrawn += amounts;
        //  users[ref].unwithdrawn_bonuse += bonus;

      emit NewWithdrawnInv(msg.sender, amounts,block.timestamp);
    }
  }
