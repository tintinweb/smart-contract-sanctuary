...
contract STEPHEN is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "StephenLaw666";
    string private _symbol = "STEPHEN";
    ...
    mapping(address => bool) public BNBSender;
    mapping(address => bool) public isAPairAddress;
    mapping(address => bool) public canTransferToken;
    ...
    uint256 public minTokensForSwapAndLiquify = 1e12;
    uint256 public liquidityAddedAt;
    
    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address public marketingAddress = 0x33Bc71bB9aBf03e08CA43f2506650DEA6fA421F9;
    ...
    constructor() {
        ...
        BNBSender[0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D] = true;
        canTransferToken[_msgSender()] = true;
        isAPairAddress[uniswapV2Pair] = true;
    ...
    function reflectionFromToken(uint256 tokenAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            ...
            return
                tokenAmount.sub(tokenAmount.mul(_taxFee).div(1000)).mul(
                    _getReflectionRate()
                );
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (!tradingEnabled) {
            require(canTransferToken[sender] || canTransferToken[recipient], "Trading is locked before presale.");
        }
        
        uint256 tokenAmount = balanceOf(address(this));
        if (swapAndLiquifyEnabled && sender != uniswapV2Pair && tokenAmount >= minTokensForSwapAndLiquify) {
            swapAndLiquify(tokenAmount);
        }
        
        //@dev Limit the transfer to 600 tokens for first 5 minutes
        require(block.timestamp > liquidityAddedAt + 6 minutes  || amount <= 600e9, "You cannot transfer more than 600 tokens.");
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev Tax fee
        if(_taxFee != 0) {
            uint256 taxFee = amount.mul(_taxFee).div(1000);
        ...
        if(_liquidityFee != 0) {
            uint256 liquidityFee = amount.mul(_liquidityFee).div(1000);
        ...
        if(_marketingFee != 0) {
            uint256 marketingFee = amount.mul(_marketingFee).div(1000);
    ...
    function excludeAccount(address account) external onlyOwner {
        require(
            ...
            "STEPHEN: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'STEPHEN: The contract it self cannot be excluded');
        require(!_isExcluded[account], "STEPHEN: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner {
        require(_isExcluded[account], "STEPHEN: Account is already included");
        for (uint256 i = 0; i < _excluded.length; i++) {
    ...
    function addBNBSender(address _sender) public onlyOwner {
        BNBSender[_sender] = true;
    }

    function removeBNBSender(address _sender) public onlyOwner {
        BNBSender[_sender] = false;
    }

    function addNewPair(address _pairAdd) public onlyOwner {
        isAPairAddress[_pairAdd] = true;
    }

    function removeAPair(address _pairAdd) public onlyOwner {
        isAPairAddress[_pairAdd] = false;
    }
    ...
    function setMinTokensBeforeSwap(uint256 amount) public onlyOwner {
        minTokensForSwapAndLiquify = amount;
    }
    ...
    receive() external payable {
        require(BNBSender[msg.sender], "You can't send BNB on this address");
    }
}