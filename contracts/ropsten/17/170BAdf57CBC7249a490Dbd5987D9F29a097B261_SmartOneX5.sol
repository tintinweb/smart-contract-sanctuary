...
contract SmartOneX5 {    
    ...
    function transferLevelPayment(uint256 _level, address _user) internal
    {
        ...
        for (uint256 i = 1; i <= uplines; i++) {
            ...
            // Si el referrer existe y esta tiene un plan activo
            if (referrer != address(0) && (users[_user].levelExpiresAt[_level] == 0 || getUserLevelExpiresAt(referrer, _level) < block.timestamp)) {
                continue;
            }
