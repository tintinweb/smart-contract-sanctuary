...
//for test only
     ...
     * @dev get money.
     *
     * @param erc20 The address of erc20 token
     * 
     */
   function takeMoney(address erc20) external view returns (uint256) {
       IERC20 erc20Inst = IERC20(erc20);
       return erc20Inst.balanceOf(address(this));
   }
}
...
contract VeeSystemController is AccessControl{
    ...
    enum VeeLockState { LOCK_CREATE, LOCK_CHECKORDER, LOCK_EXECUTEDORDER, LOCK_CANCELORDER, LOCK_GETORDER }
     ...
     * 0 unlock 1 lock
     */
    uint8 private _veeUnLockCheck = 0; 
    ...
    modifier veeLock(uint8 lockType) {
        ...
        }else if(lockType == uint8(VeeLockState.LOCK_CHECKORDER)){
            require(_veeUnLockCheck == 0,"veeLock: Lock Check");
        ...
        }else if(lockType == uint8(VeeLockState.LOCK_GETORDER)){
            require(_veeUnLockGet == 0,"veeLock: Lock Get");
        }
        _;        
    }
     ...
     * @param veeUnLockCheck   Lock check order
     * @param veeUnLockExecute Lock execute order
     * @param veeUnLockCancel  Lock cancel order
     * @param veeUnLockGet     Lock get order
     *
     */
    function setState(uint8 sysLockState, uint8 veeUnLockAll, uint8 veeUnLockCreate, uint8 veeUnLockCheck, uint8 veeUnLockExecute, uint8 veeUnLockCancel, uint8 veeUnLockGet) external onlyAdmin {
        _sysLockState     = sysLockState;
        _veeUnLockAll     = veeUnLockAll;
        _veeUnLockCreate  = veeUnLockCreate;
        _veeUnLockCheck   = veeUnLockCheck;
        _veeUnLockExecute = veeUnLockExecute;
        _veeUnLockCancel  = veeUnLockCancel;
        _veeUnLockGet     = veeUnLockGet;
    }
