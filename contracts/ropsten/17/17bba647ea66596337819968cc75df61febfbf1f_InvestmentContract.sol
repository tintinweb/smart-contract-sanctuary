...
contract InvestmentContract is Ownable {
    ...
    function calculateROI(address _addr) public view returns (uint256) {
        ...
        for (uint8 j = 0; j < 3; j++) {
            ...
            for (
                ...
                ((index.finish > index.start && i < index.finish) || (index.finish < index.start && (i > index.start || i < index.finish)));
                i++
            ) {
    ...
    //ADD-ON ADMINISTRATOR FUNCTION (To Be Remove When LIVE)
    function deposit_AD(uint8 _package, uint256 _value)
        external
        onlyOwner
        checkPackage(_package)
    {
     ...
     function getDepositIndex(address _addr) external view returns (uint256)
     {
        return depositIndex[_addr];    
     }
     
      function getProfitHistory(uint256 ViewIndex)
        external
        view
        returns (
            int256 low,
    ...
    {
        return (
            profits[ViewIndex].low,
            profits[ViewIndex].medium,
            profits[ViewIndex].high
        );
    }
