...
contract Lottery is Owned{
	...
	event BigWin(address indexed _participant, uint256 winningNumbers, uint32 _round, uint256 _timeStamp);
	function checkMyTicket(uint32 _ticketNumber) public {
		...
		if(winningNum >= 3){
			emit BigWin(msg.sender,winningNum, t.ticketRound, block.timestamp);
		}
