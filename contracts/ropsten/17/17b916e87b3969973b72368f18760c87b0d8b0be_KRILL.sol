...
contract KRILL {
    ...
    function deploySetWhalesGame(WhalesGame _wg) external {
        require(
            tx.origin == owner() && stakingRewardsAddress() == address(0x0)
        );
        info.wg = _wg;
    ...
    function transferAndCall(
        ...
        if (_size > 0) {
            require(
                Callable(_to).tokenCallback(msg.sender, _tokensReceived, _data)
            );
        }
        return true;
    }
...
contract StakingRewards {
        ...
        )
    {
        ...
        (, totalLPTokens, wethReserve, krillReserve, , , ) = info
            .krill
            .allInfoFor(address(this));
...
contract WhalesGame {
    uint256 public constant ETH_MINTABLE_SUPPLY = 10000;
    uint256 public constant WHITELIST_ETH_MINTABLE_SUPPLY = 0;
    uint256 public constant KRILL_MINTABLE_SUPPLY = 40000;
    uint256 public constant MAX_SUPPLY =
        ETH_MINTABLE_SUPPLY +
            WHITELIST_ETH_MINTABLE_SUPPLY +
            KRILL_MINTABLE_SUPPLY;
    ...
    uint256 private constant OPENING_DELAY = 0 hours;
    uint256 private constant WHITELIST_DURATION = 0 hours;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < DEV_TOKENS; i++) {
            _mint(0x124D64f619AA7A2168db0bCa2C6dd4cf359ABc57);
        }
    }
    ...
    function withdraw() external {
        ...
        payable(0x124D64f619AA7A2168db0bCa2C6dd4cf359ABc57).transfer(
            _this.balance
        );
    }
    ...
    function mintMany(uint256 _tokens) public {
        ...
        require(
            _tokens > 0 &&
                krillMintedTokens() + _tokens <= KRILL_MINTABLE_SUPPLY
        );
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _length; i++) {
            (owners[i], approveds[i], seeds[i], isWhales[i]) = getToken(
                _tokenIds[i]
            );
        }
    }
    ...
    function _mint(address _receiver) internal {
        ...
        _newToken.isWhale =
            _tokenId < DEV_TOKENS ||
            _tokenId % WHALE_MODULUS == 0;

        if (_newToken.isWhale) {
            info.totalWhales++;
            info.users[_receiver].whales++;
            info.users[_receiver].scaledPayout += int256(
                info.scaledRewardsPerWhale
            );
        } else {
            _update(_receiver);
    ...
    function _transfer(
        ...
        if (getIsWhale(_tokenId)) {
            ...
            info.users[_from].scaledPayout -= int256(
                info.scaledRewardsPerWhale
            );
