...
abstract contract LotteryWinner {
    ...
    function getLastWinners() external view returns(uint256[] memory amount, uint256[] memory time, address[] memory winner, uint256[] memory index) {
        uint256 len = _lastWinners.length;
        ...
        for (uint256 i = 0; i < len; i++) {
            amount[i] = _lastWinners[i].amount;
            time[i] = _lastWinners[i].time;
            winner[i] = _lastWinners[i].winner;
            index[i] = _lastWinners[i].index;
        }
    }
}
