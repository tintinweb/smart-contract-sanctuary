...
/**
   _     ___   ____   ___   ___  _____   ___  
  )_\   (  _( )  _)\ ) __( / _( )__ __( (  _( 
 /( )\  _) \  | '__/ | _)  ))_    | |   _) \  
)_/ \_()____) )_(    )___( \__(   )_(  )____) 
 */
...
import {IEpochs} from "./external/interfaces/IEpochs.sol";
    ...
    Ownable
{
    ...
    mapping(uint256 => uint256[12]) tokenIdToBirth;

    // (11, 11)
    IEpochs private epochsContract;
    ...
    constructor(
        ...
        epochsContract = _epochsContract;
        ...
        returns (uint256)
    {
        ...
        while (atEditionId.current() <= endAt) {
            ...
            tokenIdToBirth[atEditionId.current()] = epochsContract.getEpochs(
                block.number
            );
            atEditionId.increment();
        }
    ...
    */
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    // "?hash=",
                    // Strings.toString(tokenIdToHash[tokenId])
                )
            );
    }
    ...
    function tokenInfo(uint256 tokenId)
        public
        view
        returns (bytes32 hash, uint256[12] memory birth)
    {
        require(_exists(tokenId), "No token");
    ...
    function updateEpochsAddress(IEpochs _epochsContract) public onlyOwner {
        epochsContract = _epochsContract;
    }
