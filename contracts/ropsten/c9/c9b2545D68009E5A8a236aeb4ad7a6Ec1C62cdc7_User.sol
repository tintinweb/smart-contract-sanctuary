...
contract User{
    ...
    function getUser() external view returns(
        ...
        string memory category, uint256 validity, uint256 startDrive, 
        uint256 dtpAmount, uint256 fines, uint256 balance_str
        ) {
        return (
            users[msg.sender].FIO, users[msg.sender].vu.number, 
            users[msg.sender].vu.category, users[msg.sender].vu.validity, users[msg.sender].startDrive, 
            users[msg.sender].dtpAmount, users[msg.sender].fine, users[msg.sender].balance_str
            ); 
    }    
