...
contract DivineAnarchyToken is IDivineAnarchyToken, ERC165, Ownable, Pausable, ReentrancyGuard {
    ...
    constructor (string memory name_, string memory symbol_, string memory baseURI_, address myAddress) {
        _name = name_;
        _symbol = symbol_;
        _baseURI = baseURI_;
        ...
        _beforeTokenTransfer(address(0), myAddress, 0);
        _balances[myAddress] += 1;
        _owners[0] = myAddress;
        _tokenClass[0] = THE_UNKNOWN;
        _tokenClassSupplyCurrent[THE_UNKNOWN] = 1;
        
        _beforeTokenTransfer(address(0), myAddress, 1);
        _beforeTokenTransfer(address(0), myAddress, 2);
        _beforeTokenTransfer(address(0), myAddress, 3);
    ...
    function airdropToTreasury(uint256[][] memory treasuryRandom) external onlyOwner {
        address divineTreasuryWallet = 0xd4E24Db1609Bd014C0c71b07280f0D6f4464fAC7;
