...
contract Metatree is ERC721A, Ownable {
    ...
    function setReserve(uint256 _reserved) public onlyOwner {
        require(_reserved < MAX_SUPPLY - totalSupply(), "reserved have to greater than max");
        reserved = _reserved;
    }
    ...
    function mintPresale(uint256 num) external payable {        
        ...
        if(msg.value - presalePrice * num > 0) payable(msg.sender).transfer(msg.value - presalePrice * num);         
    ...
    function mint(uint256 num) external payable {        
        ...
        if(msg.value - publicsalePrice * num > 0) payable(msg.sender).transfer(msg.value - publicsalePrice * num);   
        _safeMint(_msgSender(), num);
    }
