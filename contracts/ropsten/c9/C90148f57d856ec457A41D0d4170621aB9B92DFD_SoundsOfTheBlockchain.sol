...
contract SoundsOfTheBlockchain is ERC721("Sounds of The Blockchain", "SoTB"), HasSecondarySaleFees{
	...
	function unlockSound(uint256[] memory mouldIds) external onlyTeam {
		for(uint i = 0; i < mouldIds.length; i++) {
			require(soundMoulds[mouldIds[i]].live == false, "Mould is already live!");
			soundMoulds[mouldIds[i]].live = true;
			emit SoundMouldUnlocked(mouldIds[i]);
		}
	}
	...
	function updateTeamAddress(address payable newTeamAddress) public onlyTeam {
        teamAddress = newTeamAddress;
    }

    function updateSecondaryFee(uint256 newSecondaryBps) public onlyTeam {
	...
	function getMouldInfo(uint256 _mouldID) external view returns (string memory name, string memory collection, uint256 price, uint256 mintLimit, uint256 amountMinted) {
		SoundMould memory mould = soundMoulds[_mouldID];
		return (mould.name, mould.collection, mould.price, mould.mintLimit, mould.amountMinted);
	}
}