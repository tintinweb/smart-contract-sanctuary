...
contract ROAR {
    ...
    uint public totalSupply = 250_000_000e18; // 250,000,000 Alphadex
...
contract AlphadexStaticFarms is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 depositTime; // Time of Token Deposit
        uint256 unlockTime; 
        bool withdrawn;
        //
        ...
        //
        //   pending reward = (user.amount * pool.accRoarPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accRoarPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        // IERC20 rewardToken;     // Reward Token for Users
        uint256 durationOfPool;       // How many allocation points assigned to this pool. ROARs to distribute per block.
        uint256 rewardAPY;  // Last block number that ROARs distribution occurs.
    }
    ...
    function add(uint256 _durationOfPool, IERC20 _lpToken, uint256 _rewardAPY) public onlyOwner {
        // if (_withUpdate) {
        //     massUpdatePools();
        // }
        ...
        poolInfo.push(PoolInfo({
            lpToken: _lpToken,
            durationOfPool: _durationOfPool,
    ...
    function pendingRoar(uint256 _pid, address _user) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        // uint256 accRoarPerShare = pool.accRoarPerShare;
        // uint256 finalClaimEndTime = user.depositTime.add(pool.durationOfPool.mul(5));
        uint256 rewardUserPerDay = user.amount.mul(pool.rewardAPY).div(100).div(pool.durationOfPool);
        uint256 stakingDays;
        if(block.timestamp <= user.unlockTime){
            stakingDays = block.timestamp.sub(user.depositTime).div(5);
        }
        else{
            stakingDays = pool.durationOfPool;
        }
        uint256 userClaimableReward = rewardUserPerDay.mul(stakingDays);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.unlockTime = block.timestamp.add(pool.durationOfPool.mul(5));
    ...
    function withdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(block.timestamp >= user.unlockTime , "withdraw: Unlock Time not Reached");
        uint256 pending = pendingRoar(_pid, msg.sender);
        safeRoarTransfer(msg.sender, pending);
    ...
    function safeRoarTransfer(address _to, uint256 _amount) internal {
