...
contract CDcontract {
    ...
    uint public lastUserId = 3; //当前分裂ID
    ...
    function registration(address userAddress, address referrerAddress, uint area) private {
        ...
        address place;
        uint userArea;
        if (tract[referrerAddress].left == address(0) || tract[referrerAddress].right == address(0)) {
            if (tract[referrerAddress].left == address(0)) {
                if (tract[referrerAddress].right != address(0)) {
                    bumpCommission(userAddress, msg.value);
                }
                tract[referrerAddress].left = userAddress;
            } else {
                if (tract[referrerAddress].left != address(0)) {
                    bumpCommission(userAddress, msg.value);
                }
                tract[referrerAddress].right = userAddress;
            }
            place = referrerAddress;
            userArea = users[referrerAddress].area;
        } else {
            place = getUserPlace(referrerAddress, area);
            if (area == 1) {
                if (tract[place].right != address(0)) {
                    bumpCommission(userAddress, msg.value);
                }
                tract[place].left = userAddress;
            } else {
                if (tract[place].left != address(0)) {
                    bumpCommission(userAddress, msg.value);
                }
                tract[place].right = userAddress;
            }
            userArea = users[place].area;
        }
        if (userArea == 1) {
        ...
        emit Registration(userAddress, referrerAddress, users[userAddress].id, users[referrerAddress].id, userArea, place);
    }
        ...
        while (true) {
            if (area == 1) {
                if (tract[referrer].left != address(0)) {
                    referrer = tract[referrer].left;
                } else {
                    return referrer;
                }
            } else {
                if (tract[referrer].right != address(0)) {
                    referrer = tract[referrer].right;
                } else {
                    return referrer;
                }
            }
    ...
    function referralCommission(address userAddress, uint8 matrix, uint8 level, uint price) private {
        ...
        if (referrerAddress != address(0)) {
            ...
            emit CommissionSend(reReferrerAddress, userAddress, matrix, level, 1, 1, referralCommission);
    ...
    function pointCommission(address userAddress, uint8 matrix, uint8 level, uint price) private {
        ...
        uint pointCommission = price * 5 / 100;
        for (i = 1; i <= 12; i++) {
    ...
    //对碰奖，上级最多获得入金12%
    function bumpCommission(address userAddress, uint price) private {
        address referrerAddress = users[userAddress].referrer;
        if (referrerAddress != address(0)) {
            ...
            address(uint160(referrerAddress)).transfer(bumpCommission);
            emit CommissionSend(referrerAddress, userAddress, 0, 0, 3, 1, bumpCommission);
        }
    }
