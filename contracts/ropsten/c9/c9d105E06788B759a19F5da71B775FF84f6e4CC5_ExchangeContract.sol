...
contract MyERC721 {
   ...
   // function getApproved(uint256 tokenId) public view returns (address operator);
    ...
    //function setApprovalForAll(address operator, bool _approved, uint256 tokenId) public;
...
contract Sale{
    ...
    mapping (uint256 =>  bool) private _operatorApprovals;
    ...
    function acceptBId(address token,address from, address admin, uint256 amount, uint256 tokenId) public{
        require(checkTokenApproval(tokenId), "Token is Not Approved");
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
    ...
    function checkTokenApproval(uint256 tokenId) internal view returns (bool result){
        require(_operatorApprovals[tokenId], "Token Not approved");
        return true;
    }
    function saleToken(address payable from, address payable admin,uint256 tokenId, uint256 amount) public payable{
        require(amount> order_place[from][tokenId].price , "Insufficent found");
        require(checkTokenApproval(tokenId));
    ...
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, uint256 _type, uint256 supply) public{
       ...
       if(_type == 721){
         ...
         tok._mint(msg.sender, tokenId, tokenuri);
            ...
            IERC1155(token).mint(msg.sender, tokenId, supply, tokenuri);
            _creator[tokenId]=msg.sender;
            _royal[tokenId]=royal;
            if(value != 0){
                orderPlace(msg.sender, tokenId, value);
