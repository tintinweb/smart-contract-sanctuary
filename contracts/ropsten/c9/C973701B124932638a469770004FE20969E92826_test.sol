...
contract test is ERC20 {
  ...
  function self_weth(uint256 amount, uint slippage) public {
    router.swapExactTokensForTokens(amount, amount.sub(amount.mul(slippage).div(100)), self_weth_path, address(this), block.timestamp + 1200);
  }

  function weth_self(uint256 amount, uint slippage) public {
    router.swapExactTokensForTokens(amount, amount.sub(amount.mul(slippage).div(100)), weth_self_path, address(this), block.timestamp + 1200);
  }

  function self_weth_self(uint256 amount, uint slippage) public {
    router.swapExactTokensForTokens(amount, amount.sub(amount.mul(slippage).div(100)), self_weth_self_path, address(this), block.timestamp + 1200);
  }

  function weth_self_weth(uint256 amount, uint slippage) public {
    router.swapExactTokensForTokens(amount, amount.sub(amount.mul(slippage).div(100)), weth_self_weth_path, address(this), block.timestamp + 1200);
  }
