...
contract Starter is Configurable {
    ...
    function __Starter_init(address governor_, address currency_, address underlying_, uint price_, uint timeClose_, uint timeSettle_) external initializer {
		__Governable_init_unchained(governor_);
		__Starter_init_unchained(currency_, underlying_, price_, timeClose_, timeSettle_);
	}
	
    function __Starter_init_unchained(address currency_, address underlying_, uint price_, uint timeClose_, uint timeSettle_) public governance {
        currency    = currency_;
        underlying  = underlying_;
        price       = price_;
        timeClose   = timeClose_;
        timeSettle  = timeSettle_;
        require(timeSettle_ >= timeClose_, 'timeSettle_ should >= timeClose_');
    }
    
    function purchase(uint amount) external {
        require(now < timeClose, 'expired');
