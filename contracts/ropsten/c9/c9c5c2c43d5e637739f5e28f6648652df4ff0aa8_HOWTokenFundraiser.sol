...
contract HOWTokenSafe is TokenSafe {
    ...
    public
  {
    ...
    init(
      0, // Group Id
      1532763480 // Release date = 28 Jul 2018 07:38 UTC
    );
    add(
      0, // Group Id
      0x77e2Db188f841c063bb31825dc92201C920498dC, // Token Safe Entry Address
      10000000000000000000  // Allocated tokens
    );
  }
}
...
contract HOWTokenFundraiser is MintableTokenFundraiser, IndividualCapsFundraiser, CappedFundraiser, RefundableFundraiser, GasPriceLimitFundraiser {
    ...
    public
  {
    ...
    MintableToken(token).mint(address(tokenSafe), 10000000000000000000);

    initializeBasicFundraiser(
      1532763300, // Start date = 28 Jul 2018 07:35 UTC
      1536933600,  // End date = 14 Sep 2018 14:00 UTC
      10, // Conversion rate = 10 HOW per 1 ether
      0x76be16a937de8eebecb28acb2f0b6c444d40330b     // Beneficiary
    );

    initializeIndividualCapsFundraiser(
      (0.05 ether), // Minimum contribution
      (0.5 ether)  // Maximum individual cap
    );
