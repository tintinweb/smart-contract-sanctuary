...
contract TokenBurner is Ownable, ReentrancyGuarded{
    
    ERC20TokenBurnable token;
    ...
    function _destroy() virtual internal{
...
contract EthTokenSwapPoolContract is ReentrancyGuarded {
    ...
    
  {
    ...
    poolTokenSupply = tokenContractAddress.totalSupply();
...
function requestSwap(address payable _requestor, uint256 _amount) public nonReentrant poolActive
{
    ...
    TokenBurner lit = new TokenBurner(tokenContractAddress, smallestTokenUnits);
    SafeERC20.safeTransfer(tokenContractAddress,address(lit), smallestTokenUnits);
    lit.letBurn();
...
function getTokenPoolSupply(ERC20TokenBurnable _tokenaddr) public view returns(uint256 _balanceINwei)
{
     require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
...
function swapToken(ERC20TokenBurnable _tokenaddr, uint256 _amount) public nonReentrant
{
    require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
