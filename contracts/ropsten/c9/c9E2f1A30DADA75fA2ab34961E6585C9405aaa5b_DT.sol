...
contract DT is Context, Ownable, IERC20, LockableSwap, FeeLibrary, EthReflectingToken {
    ...
    function initMint(uint256[] memory values, address ownerAddress, address reflectorAddress) external onlyOwner {
        ...
        for(uint256 i = 0; i < addresses.length; i++){
            xferVal = values[i].mul(10 ** 18);
            _rOwned[addresses[i]] = xferVal;
            emit Transfer(address(0), addresses[i], xferVal);
            sentTotal = sentTotal.add(xferVal);
        }
        if(sentTotal > _tTotal)
            revert();
        xferVal = _tTotal.sub(sentTotal);
        _transferStandard(address(0), ownerAddress, xferVal, xferVal, xferVal);
        runOnce = 1;
    }
...
//                bytes memory data = payable(address(reflectionContract)).functionCallWithValue(abi.encodeWithSelector(reflectionContract.depositEth.selector, ""), amount, "Unable to send ETH");
...
//        (bytes memory data) = payable(address(reflectionContract)).functionDelegateCall(abi.encodeWithSelector(reflectionContract.reflectRewards.selector, ""), "Unable to reflect");
