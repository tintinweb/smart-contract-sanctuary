...
contract TestContract is ERC721("TEST CONTRACT", "TCT"), ERC721Enumerable, Ownable {
    ...
    uint256 public constant nftsPublicNumber = 10931;
    uint256 private constant nftsNumber = 11111;
    uint256 public tokenPrice = 100000000000000000;  // 0.1 ETH
    bool public reveal;
    bool public isActive;
    bool public isPresaleActive;
    uint256 public constant maxTokensWhiteList = 2;
    ...
    */
     ...
     * It is payable. Amount is calculated as per (tokenPrice.mul(_numOfTokens))
    */
    function mintNFT(
        ...
        payable
    {
        ...
        require(totalSupply().add(_numOfTokens) <= nftsPublicNumber, "Purchase would exceed max public supply of NFTs");
        require(tokenPrice.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");
    ...
    */
        ...
        payable
    {
        ...
        require(totalSupply() < nftsPublicNumber, 'All public tokens have been minted');
        require(_numOfTokens <= maxTokensWhiteList, 'Cannot purchase this many tokens');
        require(totalSupply().add(_numOfTokens) <= nftsPublicNumber, 'Purchase would exceed max public supply of NFTs');
        require(whiteListClaimed[msg.sender].add(_numOfTokens) <= maxTokensWhiteList, 'Purchase exceeds max whiteed');
        require(tokenPrice.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");
    ...
    */
    function mintByOwner(
        ...
        onlyOwner
    {
        require(_tokenId > nftsPublicNumber, "Tokens number to mint must exceed number of public tokens");
        require(_tokenId <= nftsNumber, "Tokens number to mint cannot exceed number of MAX tokens");
        _safeMint(_to, _tokenId);
    }

    /*
     * Function to get token URI of given token ID
     * URI will be blank untill totalSupply reaches nftsPublicNumber
    */
    function tokenURI(
        uint256 _tokenId
    )
