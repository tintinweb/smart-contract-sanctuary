...
contract BridgeBase is Context, Ownable {
    ...
    address system;
    address bridgeFeesAddress = 0x5c7255AF30cfafA7D449920B9ab6Ee7D3Ef5b51a;
  ...
  function setSystem(address _system) external onlyOwner returns(bool){
    ...
    function swapBack (address to, uint256 amount) external onlySystem {
      ...
      uint256 fees = amount- temp;
      
      TransferHelper.safeApprove(catoshiAddress,address(this),fees);
      TransferHelper.safeTransferFrom(catoshiAddress, address(this),bridgeFeesAddress, fees);
