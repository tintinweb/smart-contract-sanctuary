...
contract EMESwapFee is SafeOwnable, Initializable {
    using SafeMath for uint;
    using Address for address;
    ...
    function initialize(address vault_, IEMERouter router_, IEMEFactory factory_, address WETH_, address destroyToken_, address USDT_, address receiver_, address caller_) external initializer {
    ...
    function canRemove(IEMEPair pair) internal view returns (bool) {
    ...
    function doHardwork(address[] calldata pairs, uint minAmount) external {
        ...
        for (uint i = 0; i < pairs.length; i ++) {
            IEMEPair pair = IEMEPair(pairs[i]);
