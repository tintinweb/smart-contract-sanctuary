...
contract SecretSocietyofOddFellows is ERC721Enumerable, Whitelist {
  using Strings for uint256;
  using SafeMath for uint256;
  string private baseURI;
  bool public isSaleStarted=false;
  string public notRevealedUri;
  uint256 public preSaleCost = 0.0007 ether;
  uint256 public generalSaleCost = 0.0008 ether;
  uint256 public maxSupply = 10011;
  uint256 public nftPerAddressLimitOnPreSale = 10;
  uint256 public nftPerAddressLimitOnGeneralSale = 20;
...
function mintAsOwner(uint _mintAmount) public onlyOwner{
    ...
    if(isSaleStarted) {
        require(_mintAmount <= nftPerAddressLimitOnGeneralSale, "Max NFTs per address exceeded for general-sale.");
    }
    ...
    function mint(uint _mintAmount) public payable {
        ...
        if(block.timestamp < preSaleTime) revert("pre-sale is not started");
        require(!paused, "the contract is paused");
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= maxSupply, "Exceeds maximum Originators supply");       
  ...
  function startPresale() public onlyOwner {
      isSaleStarted=true;
      preSaleTime = block.timestamp + 2 minutes;
      generalSaleTime = block.timestamp + 7 minutes;
  }
