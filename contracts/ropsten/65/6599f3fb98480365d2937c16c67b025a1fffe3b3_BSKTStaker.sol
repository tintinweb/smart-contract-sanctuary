...
contract BSKTStaker is Ownable {
     ...
     * Get staked rank
     */
    function getStakedRank() external view returns (uint256) {
        uint256 rank = 1;
        address msgSender = _msgSender();
        uint256 senderStakedAmount = _stakeMap[msgSender].stakedAmount;
        
        for(uint i=0; i<_stakers.length; i++) {
            if(_stakers[i] != msgSender && senderStakedAmount < _stakeMap[_stakers[i]].stakedAmount)
                rank = rank.add(1);
        }
        return rank;
    }
