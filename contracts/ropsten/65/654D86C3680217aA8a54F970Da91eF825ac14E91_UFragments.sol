...
contract UFragments is ERC20Detailed, Ownable {
    ...
    function initialize(string name_, string symbol_, address owner_, address pool0_, address pool1_)
        public
        onlyDeployer
        initializer
    {
        ...
        uint256 pool0Val = 25000 * (10 ** DECIMALS);
        uint256 pool1Val = 50000 * (10 ** DECIMALS);
        uint256 reserveVal = 25000 * (10 ** DECIMALS); 
        uint256 pool0Gons = pool1Val.mul(_gonsPerFragment);
        uint256 pool1Gons = pool1Val.mul(_gonsPerFragment);

        _gonBalances[pool0_] = pool0Gons;
        _gonBalances[pool1_] = pool1Gons;

        emit Transfer(address(0x0), pool0_, pool0Val);
        emit Transfer(address(0x0), pool1_, pool1Val);
        emit Transfer(address(0x0), owner_, reserveVal);
    }
