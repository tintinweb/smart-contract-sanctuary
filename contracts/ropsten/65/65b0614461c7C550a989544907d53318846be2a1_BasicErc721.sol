...
contract BasicErc721 is ERC721, ERC721URIStorage, Pausable, Ownable {
    ...
    uint private constant maxTokensPerTransaction = 5;
    uint256 private tokenPrice = 10000000000000000; //0.01 ETH
    uint256 private constant nftsNumber = 7777;
    uint256 private constant nftsPublicNumber = 7777;
    ...
    */
        ...
        payable
    {
        ...
        require(_tokenIdCounter.current().add(tokensNumber) <= nftsPublicNumber, "Tokens number to mint exceeds number of public tokens");
        require(tokensNumber <= maxTokensPerTransaction, "Max tokens per transaction number exceeded");
        // require(NFTPrice.mul(tokensNumber) == msg.value, "Ether value sent is not correct");
    ...
    */
        ...
        payable
    {
        ...
        // require(_tokenIdCounter.current().add(tokensNumber) <= nftsPublicNumber, "Tokens number to mint exceeds number of public tokens");
        require(tokensNumber <= WHITELIST_MAX_MINT, 'Cannot purchase this many tokens');
        require(_tokenIdCounter.current().add(tokensNumber) <= nftsPublicNumber, 'Purchase would exceed max public supply of NFTs');
        require(whiteListClaimed[msg.sender].add(tokensNumber) <= WHITELIST_MAX_MINT, 'Purchase exceeds max whiteed');
        // require(NFTPrice.mul(tokensNumber) == msg.value, "Ether value sent is not correct");
