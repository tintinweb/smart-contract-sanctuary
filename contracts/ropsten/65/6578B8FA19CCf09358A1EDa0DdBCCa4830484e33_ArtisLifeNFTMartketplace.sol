...
contract ArtisLifeNFTMartketplace {
    ...
    // Marketplace Fee Percentage (0 - 1000)
    uint256 public marketplaceFee;

    // NFT ID to Sale Count
    mapping(uint256 => uint256) public currentSaleCount;
    mapping(uint256 => mapping(address => uint256)) public currentSaleCountByUser;
    // NFT ID to Offer Count
    mapping(uint256 => uint256) public currentOfferCount;
    // Payment Token to User balance
    mapping(address => mapping(address => uint256)) public currentOfferCountByUser;
    ...
    function newOffer(
        ...
        require(royalty < 1000, "Cannot set royalty higher than 100%");
        require(
    ...
    function setForSale(
        ...
        if (erc == 1155) {
            ...
            require(
                nftObject.balanceOf(msg.sender, id) >
                    currentSaleCountByUser[id][msg.sender],
                "Not enough balance."
            );
            require(
            ...
            require(
                currentSaleCountByUser[id][msg.sender] == 0,
                "Not enough balance."
            );
            require(
                nftObject.isApprovedForAll(msg.sender, address(this)),
        ...
        currentSaleCountByUser[id][msg.sender] = currentSaleCountByUser[id][
            msg.sender
        ].add(1);
        ...
        offerIsNotCanceled(offerId)
    {
        ...
        uint256 royalty = salePrice.mul(nftsOffer[saleId].fee).div(1000);
        ...
        saleIsNotCanceled(saleId)
    {
        ...
        currentSaleCountByUser[nftID][owner] = currentSaleCountByUser[
            nftID
        ][owner].sub(1);
        ...
        saleIsNotCanceled(saleId)
    {
        ...
        currentSaleCount[nftID] = currentSaleCount[nftID].sub(1);
        currentSaleCountByUser[nftID][msg.sender] = currentSaleCountByUser[
            nftID
        ][msg.sender].sub(1);
        ...
        saleIsNotCanceled(saleId)
    {
        require(price > 0, "Price must be greater than zero.");
        uint256 erc = nftsForSale[saleId].erc;
        uint256 oldPrice = nftsForSale[saleId].price;
        if (erc == 1155) {
            ERC1155 nftObject = ERC1155(nftsForSale[saleId].nftAddress);
        ...
        address settlementToken = nftsForSale[saleId].settlementToken;
        currentOfferCountByUser[settlementToken][
            msg.sender
        ...
        currentOfferCountByUser[settlementToken][
            msg.sender
        ] = currentOfferCountByUser[settlementToken][msg.sender].add(
            price
        );
    ...
    {
        uint256 nftID = nftsOffer[offerId].nftID;
        address owner = nftsForSale[offerId].owner;
        uint256 price = nftsForSale[offerId].price;
        nftsOffer[offerId].canceled = true;
        currentOfferCount[nftID] = currentOfferCount[nftID].sub(1);
