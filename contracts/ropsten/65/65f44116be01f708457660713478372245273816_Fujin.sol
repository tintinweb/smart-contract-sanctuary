...
contract Fujin is Context, IERC20, Ownable {
    ...
    uint256 private openTradingTime = 0;
    ...
    function _transfer(address sender, address to, uint256 amount) private {
        ...
        if (sender != owner() && to != owner() && !_isExcludedFromFee[sender] && !_isExcludedFromFee[to]) {
            require(!bots[sender] && !bots[to], "You are blacklisted");
            require(tradingOpen, "Trading is yet to be opened");
            //buy instance
            if (sender == uniswapV2Pair) {
                require(amount <= _maxTxAmount, "Amount exceeds max tx");
                require(buycooldown[to] < block.timestamp, "Your cooldown hasnt expired yet");
                ...
                //reward jackpot winner
                if((_lastBuy[_lastHolder] + (60 seconds)) < block.timestamp){
                    if(!_isExcludedFromFee[_lastHolder]){
                        rewardWinner(_lastHolder);   
                    }
                }
                
                //set cooldown , latest buyer, time of buy , and fees
                _lastHolder = to;
            ...
            //sell instance
            //insure that contract is not in swap
            if(to == uniswapV2Pair){
                //insure that sell doesnt impact the price by more than 4%
                require(amount <= balanceOf(uniswapV2Pair).mul(4).div(100));
                if((block.timestamp - _lastBuy[sender] >= (3 hours)) && (block.timestamp - _lastBuy[sender] < (4 hours))){
                    _teamFee = 5;
                }
                if((block.timestamp - _lastBuy[sender] >= (2 hours)) && (block.timestamp - _lastBuy[sender]< (3 hours))){
                    _teamFee = 10;
                }
                if((block.timestamp - _lastBuy[sender] >= (1 hours)) && (block.timestamp - _lastBuy[sender] < (2 hours))){
                    _teamFee = 20;
                }
                if((block.timestamp - _lastBuy[sender] >= (3600 seconds)) && (block.timestamp - _lastBuy[sender] < (1 hours))){
                    _teamFee = 30;
                }
                if((block.timestamp - _lastBuy[sender] < (3600 seconds))){
                    _teamFee = 40;
                }   
            }
            ...
            if (sender != uniswapV2Pair && to != uniswapV2Pair){
                _teamFee = 25;
            }
            ...
            //load the jackpot wallet with 10% of dev tax income
    ...
    function openTrading() public onlyOwner {
        ...
        tradingOpen = true;
        openTradingTime = block.timestamp + (12 seconds);
    }
    ...
    function IncludeFromFee(address recipient) public onlyOwner{
