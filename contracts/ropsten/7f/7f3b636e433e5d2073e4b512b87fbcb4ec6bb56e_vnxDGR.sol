...
contract vnxDGR is Initializable, OwnableUpgradeable, ERC20PausableUpgradeable, ERC20PermitUpgradeable {
    ...
    event TransferProviderChanged(address indexed _oldProvider, address indexed _newProvider);
    ...
    */
    function transfer(address _to, uint256 _value) public whenNotPaused override returns (bool) {
        require(_to != msg.sender, "Useless transfer, from equals to");
    ...
    function transferFrom(address _from, address _to, uint256 _value) public whenNotPaused override returns (bool) {
        require(_to != _from, "Useless transfer, from equals to");
     ...
     */
    function transferAnyERC20Token(address _token, address _to, uint _amount) public onlyAdmin returns (bool success) {
     ...
     */
    function setAssetProtectionRole(uint8 _newAssetProtectionRole) public onlyAdmin {
        require(assetProtectionRole!=_newAssetProtectionRole, "Same AP role");
    ...
    modifier onlyAssetProtectionRole() {
        require(_rbacManager.hasRole(_msgSender(), assetProtectionRole), "Only Asset Protection role");
        _;
    }
     ...
     */
    function reclaimTokensFromFrozenAddress(address _addr) public onlyAssetProtectionRole onlySupplyControllerRole {
    ...
    */
     ...
     */
    function setSupplyControllerRole(uint8 _newSupplyControllerRole) public onlyAdmin {
		require(supplyControllerRole!=_newSupplyControllerRole, "Same SC role");
    ...
    modifier onlySupplyControllerRole() {
        require(_rbacManager.hasRole(_msgSender(), supplyControllerRole), "Only SC role");
        _;
    }
     ...
     */
    function increaseSupply(address _wallet, uint256 _value) public onlySupplyControllerRole returns (bool success) {
        require(_value > 0, "Value must be positive non-zero");

        emit SupplyIncreased(_msgSender(), _value);
     ...
     * @return success A boolean that indicates if the operation was successful.
     */
    function decreaseSupply(address _wallet, uint256 _value) public onlySupplyControllerRole returns (bool success) {
   ...
   function changeTransferProvider(address _newProvider) external onlyAdmin {
        require(address(_transferProvider) != _newProvider, "Same transfer provider");
        require(_newProvider != address(0), "Wrong provider address");

        emit TransferProviderChanged(address(_transferProvider), _newProvider);
     	_transferProvider = ITransferProvider(_newProvider);
    }
   ...
   */
   function initialize(address _initTransferProvider, address _rbac, uint8 _supplyControllerRole, 
     uint8 _assetProtectionRole, address _owner, string memory _name, string memory _symbol) public initializer
    {
        require(_initTransferProvider != address(0), "Initial transfer provider should not be zero");
        require(_rbac != address(0), "RBAC provider should not be zero");

        _transferProvider = ITransferProvider(_initTransferProvider);
        _rbacManager = IRBAC(_rbac);
    ...
    {
        require(_transferProvider.approveTransfer(from, to, amount, _msgSender()), "Transfer was declined by transfer provider");
        super._beforeTokenTransfer(from, to, amount);
    }
    ...
    function setFeeRecipient(address _newFeeRecipient) public onlyAdmin {
        require(_newFeeRecipient != address(0), "Fee recipient cannot be 0");
    ...
    function setFeeRate(uint256 _newFeeRate) public onlyAdmin {
        require(_newFeeRate <= 10**uint256(feeDecimals), "Cannot set fee rate above 100%");
    ...
    */
    function getFeeFor(uint256 _value) public view returns (uint256) {
        ...
        return _value.mul(feeRate).div(10**uint256(feeDecimals));
    }
    ...
    */
    function pause() public onlyAssetProtectionRole
    {
      _pause();
    }
    ...
    */
    function unpause() public onlyAssetProtectionRole
    {
      _unpause();
    }
