...
abstract contract BaseStrategy {
    ...
    function withdrawToVault(uint256 correspondingShares, uint256 totalShares) external onlyVault virtual returns (uint256 value, uint256 partialClaimValue, uint256 claimValue)  {
        ...
        console.log('withdraw totalLpCount:%d', totalLpCount);
        //* 1 ** lpDecimals();
        uint256 withdrawLpTokensCount = totalLpCount.mul(correspondingShares).div(totalShares);
        console.log('withdrawLpTokensCount:%d', withdrawLpTokensCount);
        if (withdrawLpTokensCount > 0) {
            ...
            console.log('valueOfLpTokens:', valueOfLpTokens);
            //提矿卖出
            ...
            if (preTotalAssets > 0 && pricePerShare > lastPricePerShare) {
                ...
                //                console.log("%s,deltaTime:%d,delta assets",name(),deltaSeconds,deltaOfPricePerShare);
                console.log("preTotalAssets:%d,totalAssets:%d", preTotalAssets, totalAssets);
                ...
                console.log("lastPricePerShare:%d, pricePerShare:%d, cul apy:%d", lastPricePerShare, pricePerShare, apy);
            }
    ...
    function withdrawOneToken() external onlyVault virtual returns (uint256 value, uint256 partialClaimValue, uint256 claimValue) {
        ...
        if (totalLpCount >= 10 ** uint256(lpDecimals())) {
            ...
            console.log('withdrawOneToken-pricePerShare:%d', pricePerShare);
            ...
            console.log('oneOfRewards:', oneOfRewards);
    ...
    function invest() public onlyVault {
        ...
        if (beforeInvest == 0 && afterInvest > 0) {
            ...
            prevTimestamp = block.timestamp;
            console.log("%s,first invest , wantBalance:%d,pricePerShare:%d", name(), wantBalance, pricePerShare);
        }
    }
...
contract YearnUSDTStrategyV2 is BaseStrategy {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    address public yVault = address(0x7Da96a3891Add058AdA2E826306D812C638D87a7);
    ...
    function protocol() public pure override returns (uint256) {
        return uint256(ProtocolEnum.Yearn);
    }

    function name() public pure override returns (string memory) {
        return 'YearnUSDTStrategyV2';
    }
    ...
    function lpDecimals() internal view override returns (uint256) {
        return IYearnVaultV2(yVault).decimals();
    }
    ...
    function withdrawSome(uint256 shares) internal override returns (uint256) {
        ...
        console.log("=========",shares);
        //从yVault中赎回
        IYearnVaultV2(yVault).withdraw(shares);
        console.log("=========",shares);
        uint256 amount = want.balanceOf(address(this));
        //shares * IYearnVaultV2(yVault).getPricePerFullShare() / 1e18;
        return amount;
    }
    ...
    function cutOffPosition(uint256 _debtOutstanding) external onlyVault override returns (uint256){
        if (_debtOutstanding > 0) {
            ...
            console.log('YearnUSDT step1 _debtOutstanding:%d,_balance:%d', _debtOutstanding, _balance);
            if (_debtOutstanding > _balance) {
                uint256 totalAssets = estimatedTotalAssets();
                if (_debtOutstanding > totalAssets) {
                    //全部赎回
                    IYearnVaultV2(yVault).withdraw();
                    //TODO::这里可能产生了一些损失导致赎回时金额不够
                } else {
                    uint256 needShares = (_debtOutstanding - _balance).mul(10 ** lpDecimals()).div(IYearnVaultV2(yVault).pricePerShare());
                    console.log("needBalance:%d needShares:%d",_debtOutstanding - _balance,needShares);
                    if(needShares>0){
    ...
    function investInner() internal override {
        uint256 amount = want.balanceOf(address(this));
        if (amount > 0) {
            want.safeApprove(yVault, 0);
            want.safeApprove(yVault, amount);
            IYearnVaultV2(yVault).deposit(amount);
            console.log('deposit to yearn %d USDT', amount);
        }
    }
    ...
    function getInvestVaultAssets() external view override returns (uint256) {
        console.log('=============================YearnUSDT total assets:', IYearnVaultV2(yVault).totalAssets());
        return IYearnVaultV2(yVault).totalAssets();
    }
}