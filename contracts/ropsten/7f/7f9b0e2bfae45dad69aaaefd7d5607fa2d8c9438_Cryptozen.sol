...
// File: contracts/Cryptozen.sol

// SPDX-license-Identifier: MIT

contract Cryptozen is Context, Ownable {
    address payable private _feeAddress;
    uint256[3][9] public _tiers;

    constructor(address payable feeAddress, uint256[3][9] memory tiers) public {
    ...
    function getTiers() public view returns (uint256[3][9] memory) {
        return _tiers;
    }

    function getTier(uint256 index) public view returns (uint256[3] memory) {
        return _tiers[index];
    }
    ...
    {
        if (amount >= _tiers[0][1] && amount < _tiers[1][0]) {
            return _tiers[0];
        }
    ...
    function getFeePercentageByAmount(uint256 amount)
        public
        view
        returns (uint256)
    {
        return getTierByAmount(amount)[1];
    }
    ...
    function transferSameToken(
        ...
        tokenContractAddress.transferFrom(
            _msgSender(),
            address(_feeAddress),
            calculateTransferFee(amount, getFeePercentageByAmount(amount))
        );
