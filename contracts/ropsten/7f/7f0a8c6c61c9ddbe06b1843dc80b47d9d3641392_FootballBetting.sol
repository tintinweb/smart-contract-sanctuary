...
contract FootballBetting {
  ...
  event BetAccepted(address creator, uint id, address acceptor,uint ratio,uint amount,uint typeId,string _option);
  ...
  function acceptBet(address betCreator, uint betId, uint typeId, uint ratio, string memory _options) public payable {
    ...
    emit BetAccepted(betCreator, betId, betAcceptor, ratio, totalAmount,typeId,_options);
  }
