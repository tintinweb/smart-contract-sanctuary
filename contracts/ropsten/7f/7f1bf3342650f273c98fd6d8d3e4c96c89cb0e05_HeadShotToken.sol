...
contract HeadShotToken is Context, IERC20, Ownable {
    ...
    mapping(address => mapping(uint256 => BuyInfo)) private _buysInfo;
    ...
    */
    ...
    function addSales(uint256 code_, string memory name_, string memory desc_, uint256 price_) public onlyOwner {
        ...
        //Add Sales Info
        _salesList.push(SalesInfo(code_, name_, desc_, price_.mul(10 ** _decimals)));
    }
            ...
            for (uint index = 0; index < _salesList.length - 1; index++) {
                if (_salesList[index].code == code_){
                    _salesList[index] = SalesInfo(
                        code_, name_, desc_, price_.mul(10 ** _decimals)
                    );
                }
            }
        }
    ...
    function buy(uint256 _code, uint256 _count) public returns (bool) {
        if (_sales[_code].code == _code){
            ...
            uint256 price = _sales[_code].price;
            uint256 totalPay = price * _count;

            _transfer(_msgSender(), _salesWalletAddress, totalPay);
            _buysInfo[buyer][_code].balance += _count;
            return true;
        }
        return false;
    }

    function getPurchases(address buyer) public view returns (uint256[] memory, string[] memory, string[] memory, uint256[] memory, uint256[] memory) {
        uint256[] memory codes;
        string[] memory names;
        string[] memory descs;
        uint256[] memory prices;
        uint256[] memory balances;
        if (_salesList.length > 0){
            for (uint index = 0; index < _salesList.length - 1; index++) {
                uint256 code = _salesList[index].code;
                codes[index] = code;
                names[index] = _salesList[index].name;
                descs[index] = _salesList[index].desc;
                prices[index] = _salesList[index].price.div(10 ** _decimals);
                balances[index] = _buysInfo[buyer][code].balance;
            }
        }
        return (codes, names, descs, prices, balances);
    }
}