...
// Dependency file: contracts/interfaces/ITomiTransferListener.sol
...
// Dependency file: contracts/interfaces/ITomiPair.sol
...
// Dependency file: contracts/interfaces/ITomiGovernance.sol
...
// Dependency file: contracts/libraries/TomiSwapLibrary.sol
...
contract TomiPlatform is Ownable {
    ...
    function _calcTGASRate(
        ...
        if (tokenA == WETH || tokenA == TGAS) {
            tokenAValue = tokenA == WETH ? amountA : TomiSwapLibrary.quoteEnhance(FACTORY, TGAS, WETH, amountA);
        }
        if (tokenB == WETH || tokenB == TGAS) {
            tokenBValue = tokenB == WETH ? amountB : TomiSwapLibrary.quoteEnhance(FACTORY, TGAS, WETH, amountB);
        }
    ...
    function _swapFee(
        ...
        for (uint256 i = 0; i < path.length - 1; i++) {
            ...
            if (input == TGAS) {
                ITomiPair(currentPair).swapFee(fee, TGAS, GOVERNANCE);
                _transferNotify(currentPair, GOVERNANCE, TGAS, fee);
            } else {
                ...
                ITomiPair(TomiSwapLibrary.pairFor(FACTORY, input, TGAS)).swapFee(feeOut, TGAS, GOVERNANCE);
                _transferNotify(currentPair, TomiSwapLibrary.pairFor(FACTORY, input, TGAS), input, fee);
                _transferNotify(TomiSwapLibrary.pairFor(FACTORY, input, TGAS), GOVERNANCE, TGAS, feeOut);
                fee = feeOut;
            }
            if (fee > 0) ITomiGovernance(GOVERNANCE).addReward(fee);
        }
    }
