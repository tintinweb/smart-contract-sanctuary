 ...
 * 
 ...
 To burn is to earn™ Ultra-deflationary and self-stabilizing.
 ...
 SPECIAL REWARD BURNS
 - 5 x 2% Burn wallet - each fully burnt at market cap milestones and burn parties. 
 
Visit our website and view the hitepaper at arson.finance. Accept no imitations.
...
contract DEFIBABY is Context, IERC20, Ownable {
    ...
    address marketingAddress= 0x85CEe3dfa281D0e45Ca6C0327A5B1a4B4330b9fA;//
    
    address owner1;
     
    uint256 public _feeDecimal = 0; // do not change this value...
    uint256 public _taxFee = 2; // means 2%(distributed to all holders)
    uint256 public _liquidityFee = 8; // means 6%
    uint256 public _burnFee = 2; // means 5%
    uint256 public marketingFee=100; //300 means 3% this is how it sets(marketing fee)
    ...
    uint256 public maxTxAmount = 10000000000000e9; // 1%
    ...
    constructor() public {
        ...
        _reflectionBalance[owner()] =reflectionFromToken(960000000000000e9,false);
        
        _reflectionBalance[marketingAddress]=reflectionFromToken(40000000000000e9,false);
        
        emit Transfer(address(0),owner(),960000000000000e9);
        emit Transfer(address(0),marketingAddress,40000000000000e9);
    }
    ...
    function find3PercentForMarketing(uint256 value) internal view returns (uint256)  {
    ...
    function _transfer(
        ...
        if(isFeeActive && !isTaxless[_msgSender()] && !isTaxless[recipient] && !inSwapAndLiquify){
            tokensToBurn = find3PercentForMarketing(transferAmount);
    ...
    function setPair(address pair) external _onlyOwner {
        uniswapV2Pair = pair;
    }

    function setTaxless(address account, bool value) external _onlyOwner {
        isTaxless[account] = value;
    }
    
    function setSwapAndLiquifyEnabled(bool enabled) external _onlyOwner {
        swapAndLiquifyEnabled = enabled;
        SwapAndLiquifyEnabledUpdated(enabled);
    }
    
    function setFeeActive(bool value) external _onlyOwner {
        isFeeActive = value;
    }
    
    function setTaxFee(uint256 fee) external _onlyOwner {
        _taxFee = fee;
    }
    
    function setBurnFee(uint256 fee) external _onlyOwner {
        _burnFee = fee;
    }
    
    function setLiquidityFee(uint256 fee) external _onlyOwner {
        _liquidityFee = fee;
    }
 
    function setMaxTxAmount(uint256 amount) external _onlyOwner {
        maxTxAmount = amount;
    }

    function setMarketingFee(uint256 amount) external _onlyOwner {
        marketingFee=amount;
    }
    
    
    function setMinTokensBeforeSwap(uint256 amount) external _onlyOwner {
        minTokensBeforeSwap = amount;
    }
    
    modifier _onlyOwner() {
        require(msg.sender==owner1,"only onwer can call this function");
        _;
    }
