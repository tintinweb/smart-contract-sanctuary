...
contract Marketplace is AccessControlUpgradeable, MarketplaceSignature {
    ...
    function addAuctionLot(
        ...
        require(startTime > 0, "Start time must be bigger that zero");
        require(
...
interface ITrading {
    ...
    function changeAmount(uint256 lotId, uint128 amount) external;

    function delTradeLot(uint256 lotId) external;

    function getOwner(uint256 lotId) external view returns (address);

    function getLotById(uint256 lotId) external view returns (TradeLot memory);
}
...
interface IAuction {
    ...
    function getOwner(uint256 auctionId) external returns (address);
...
contract MarketplaceSignature {
    ...
    bytes32 constant AUCTIONDATA_TYPEHASH =
        keccak256(
            "AuctionData(address auctionCreator,uint256 tokenId,uint256 amount,uint256 startPrice,uint128 startTime,uint128 endTime,uint128 minDelta)"
        );

    bytes32 constant TRADEDATA_TYPEHASH =
        keccak256(
            "TradeData(address lotCreator,uint256 tokenId,uint256 price,uint128 amount,uint128 endTime)"
        );
    bytes32 constant CANCELAUCTIONDATA_TYPEHASH =
        keccak256("CancelAuctionData(address bidder,uint256 auctionId)");
    ...
    function _hash(AuctionData memory auctionData)
        internal
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    AUCTIONDATA_TYPEHASH,
                    auctionData.auctionCreator,
                    auctionData.tokenId,
                    auctionData.amount,
                    auctionData.startPrice,
                    auctionData.startTime,
                    auctionData.endTime,
                    auctionData.minDelta
                )
            );
    }

    function _hash(TradeData memory tradeData) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    TRADEDATA_TYPEHASH,
                    tradeData.lotCreator,
                    tradeData.tokenId,
                    tradeData.price,
                    tradeData.amount,
                    tradeData.endTime
    ...
    function _hash(CancelAuctionData memory cancelAuctionData)
        internal
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    CANCELAUCTIONDATA_TYPEHASH,
                    cancelAuctionData.bidder,
                    cancelAuctionData.auctionId
                )
            );
    }

    function _getAuctionCancelSigner(
        address bidder,
        uint256 auctionId,
        uint8 v,
        bytes32 r,
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                _hash(CancelAuctionData({bidder: bidder, auctionId: auctionId}))
            )
        );
        return ecrecover(digest, v, r, s);
    }

    function _getAuctionSigner(
        address auctionCreator,
        uint256 tokenId,
        uint256 amount,
        uint256 startPrice,
