...
contract NFTToken is ERC721Enumerable, Ownable{
    ...
    event minting(uint256 id, address indexed creator, string indexed tokenUri, string indexed tokenName);

    constructor() ERC721("NFTToken", "NFTToken") {
    ...
    function mint(string calldata tokenUri,string calldata tokenName) public payable returns(uint256)  {
        require(msg.value >= MINT_PRICE, "not enough");
        require(balanceOf(msg.sender) < tokenLimit, "You can't mint anymore.");
        _tokenIds.increment();
        ...
        emit minting(newCharacter, msg.sender, tokenUri, tokenName);
        withdraw_all();
        return newCharacter;
    }
    ...
    function updateTokenLimit(uint256 _tokenLimit) external onlyOwner{
        tokenLimit = _tokenLimit;
    }

    function _transfer(address from, address to, uint256 tokenId) internal override {
        require(balanceOf(to) < tokenLimit, "Can't transfer");
