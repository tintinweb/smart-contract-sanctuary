...
contract CoinEraMintableStandard {
    ...
    function annualEraRate() internal view returns (uint);
...
contract AGI is ERC20, CoinAgeMintableStandard, CoinEraMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "AGI";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(tokenContract == _to && balances[msg.sender] >= coinAgeHolder) return mintingCoinEra();
        if(tokenContract == _to && balances[msg.sender] < coinAgeHolder) revert();
...
//--------------------------------------------------------------------------------------
    ...
    //How to trigger internal CoinEra minting :
    //Send transaction without any amount of coins/tokens to token contract address
    //for triggering coins/tokens internal CoinEra minting from wallet.
    //Best option wallet is Metamask. It's pretty easy.

    function mintingCoinEra() CoinAgeMinter public returns (bool) {
        require(balances[msg.sender] >= coinAgeHolder);
        if(balances[msg.sender] < coinAgeHolder) return false;
        if(transferIns[msg.sender].length < 0) return false;
        if(contractOwner == msg.sender) revert();
    ...
    function annualEraRate() internal view returns (uint eraRate) {
        uint _now = now; eraRate = coinEraRate;
        
        //1st year era minting rate = 73%
        if((_now.sub(coinEraStartTime)).div(1 years) == 0) {
            eraRate = (7300 * coinEraRate).div(1000);
            
        //2nd year - 6th year era minting rate = 36.5%
        } else if((_now.sub(coinEraStartTime)).div(1 years) == 1 && (_now.sub(coinEraStartTime)).div(1 years) == 5) {
            eraRate = (3650 * coinEraRate).div(1000);
            
        //7th year - 11th year era minting rate = 18.25%
        } else if((_now.sub(coinEraStartTime)).div(1 years) == 6 && (_now.sub(coinEraStartTime)).div(1 years) == 10) {
            eraRate = (1825 * coinEraRate).div(1000);
        
        //12th year - end year era minting rate = 9.125%
        } else if((_now.sub(coinEraStartTime)).div(1 years) == 11 && (_now.sub(coinEraStartTime)).div(1 years) == 15) {
            eraRate = (9125 * coinEraRate).div(10000);
        }
    }
    ...
    function getCoinEraMinting(address _address) internal view returns (uint) {
        ...
        uint _now = now;
        uint _intervalEra = getCoinEra(_address, _now);
        if(_intervalEra < 0) return 0;
        uint eraRate = coinEraRate;
        ...
        return (_intervalEra * eraRate).div(365 * (10**decimals));
    }
    ...
    function changeCoinEraRate(uint256 _coinEraRate) public onlyOwner {
        coinEraRate = _coinEraRate;
        emit ChangeCoinEraRate(coinEraRate);
    }
