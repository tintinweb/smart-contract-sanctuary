...
contract MasterChef is Ownable {
    ...
    function pendingKwik(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if(block.timestamp <= finalClaimEndTime){
            stakingDays = ((block.timestamp.sub(user.depositTime)).div(5).sub(user.lastClaim));
        }
        else{
            stakingDays = (finalClaimEndTime.sub(user.depositTime)).div(5).sub(user.lastClaim);
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0){
            ...
            if(block.timestamp <= user.depositTime.add(pool.durationOfPool.mul(5))){
                user.lastClaim = (block.timestamp.sub(user.depositTime)).div(5);
            }else{
                // user.lastClaim = user.depositTime.add(pool.durationOfPool.mul(5)).div(5);
                user.lastClaim = pool.durationOfPool;
            }
        }
