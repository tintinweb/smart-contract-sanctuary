...
contract MiningCore is Config, Ownable {
    ...
    function mining(address msgSender, address operator, address from, uint256 tokenId, bytes calldata) internal virtual{
        require(address(machine)==msgSender,"not allowed");
        require(!Address.isContract(operator),"Contract invocation is not allowed");
        if(from!=address(0)){
            burn(from,tokenId);
        }
    }
