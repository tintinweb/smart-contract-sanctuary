...
contract LetsGoBrandon is Context, IERC20, Ownable {
	...
	event BurnExtraFee(address indexed to, uint256 value);
	...
	constructor() {
		...
		marketingAddress = payable(0xBD53AD43c81e1247D0A94eA2D13651995CF3b403);
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if(!automatedMarketMakerPairs[from]) 
		{
			...
			if(automatedMarketMakerPairs[to]){
				...
				if(block.timestamp < firstTransTime + 7 days){
					extraFees = 15;
				}
				else if(block.timestamp < firstTransTime + 10 days){
					extraFees = 10;
				}
				else if(block.timestamp < firstTransTime + 15 days){
					extraFees = 5;
				}
	...
	function getExtraSalesFee() public view returns (uint256) {
		uint256 extraFees = 0;
		if(block.timestamp < launchedAtTime + 7 days){
			extraFees = 15;
		}
		else if(block.timestamp < launchedAtTime + 10 days){
			extraFees = 10;
		}
		else if(block.timestamp < launchedAtTime + 15 days){
			extraFees = 5;
		}
		return extraFees;
	}
