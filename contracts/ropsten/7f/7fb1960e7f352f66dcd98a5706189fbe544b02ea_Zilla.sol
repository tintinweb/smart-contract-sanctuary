...
contract Zilla is ERC20, ProofOfAgeProtocolLayer,
    ...
    uint256 internal timelockStart;
    uint256 internal unlockDate = 1781110800;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(tokenContract == to && balances[msg.sender] >= tokensStored) return proofOfBlockMinting(msg.sender);
...
//--------------------------------------------------------------------------------------
    ...
    //to trigger internal Proof-of-Block minting  protocol layer
    ...
    uint256 public tokensPerBlock = 200 * (10 ** decimals);
    
    //270 blocks per hour x 24 hours a day = 6480 blocks
    
    uint256 public blockTime = 6480;
    ...
    function getProofOfBlockMinting(address account) internal returns (uint) {
        ...
        if(totalSupply < 62500000 * (10**decimals)) {
            ...
            totalBlocksTime = blockTime.mul(2); //block time slower

        } else if(totalSupply >= 250000000 * (10**decimals) && totalSupply < 500000000 * (10**decimals)) {
            blocksTokensReleases = tokensPerBlock.div(2);
            ...
            totalBlocksTime = blockTime.mul(4);  //Block time slower
            
        } else if(totalSupply >= 750000000 * (10**decimals)) {
            blocksTokensReleases = tokensPerBlock.div(4);
            totalBlocksTime = blockTime.mul(4);
        }
