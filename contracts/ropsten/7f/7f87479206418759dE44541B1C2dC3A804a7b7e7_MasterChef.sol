...
contract MasterChef is Ownable, ReentrancyGuard, IMasterChef, Constants {
    ...
    struct UserInfo {
        ...
        uint32 stakedSince;            // Weighted blocknumber since last stake.  
    ...
    // Modifier to restrict access to owner and rewardAddress.
    modifier onlyAdmins(){
        require(msg.sender == owner() || msg.sender == rewardAddress, "onlyAdmins: FORBIDDEN");
        _;
    }

    // TODO TEST IF ADDING WORKS
    // TODO TEST IF ADDING IN REWARD POOL WORKS
    // Add a new lp to the pool. Can only be called by the owner.

    function add(uint8 _allocPoint, IERC20 _lpToken, IERC20 _tokenA, IERC20 _tokenB, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        ...
        rewardPool.addLpToken(_lpToken, _tokenA, _tokenB);
    ...
    // to - from
    function getBlockDistance(uint32 _to, uint32 _from) public view returns (uint32) {
        return _to - _from;
    }
    ...
    function pendingLotl(uint8 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getBlockDistance(pool.lastRewardBlock, uint32(block.number));
    ...
    function calculateTimeRewards (uint32 _stakedSince) private returns (uint16)  {
        return uint16(_stakedSince / 28800);
    } 
    ...
    function deposit(uint8 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0 && _amount > 0){
            ...
            if(user.amount > _amount){
                user.stakedSince = uint32(user.stakedSince + _amount * 1e12 / user.amount * getBlockDistance(user.stakedSince, uint32(block.number)) / 1e12);
            }

            if(user.amount == _amount){
                user.stakedSince = uint32(block.number - getBlockDistance(user.stakedSince, uint32(block.number)) / 2);
            }

            if(user.amount < _amount){
                user.stakedSince = uint32(block.number - user.amount * 1e12 / _amount * getBlockDistance(user.stakedSince, uint32(block.number)) / 1e12);
            }
        }
        // Save last block number as staking timestamp
        if (user.amount == 0 && _amount > 0){
             //TODO TEST IF FIRST STACKE SETS STAKED SINCE
            user.stakedSince = uint32(block.number);
        }
        if (user.amount > 0) {
    ...
    */
        ...
        for (uint8 i=0; i < length; i++){
            ...
            for(uint32 j; j< userLength; j++){
                ...
                if(user.stakedSince > 0){
                    ...
                    uint16 timeReward = calculateTimeRewards(user.stakedSince);
    ...
    function currentHoldingFactor(uint8 _pid) public view returns(uint32 holdFactor){
        UserInfo storage user = userInfo[_pid][msg.sender];
        if(user.stakedSince > 0)
        {
            return uint32(block.number - user.stakedSince);
        }
        else return 0;
...
interface IRewardPool{
    function addLpToken(IERC20 _lpToken, IERC20 _tokenA, IERC20 _tokenB) external;
}
