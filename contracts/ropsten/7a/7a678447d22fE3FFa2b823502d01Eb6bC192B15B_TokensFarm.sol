...
contract TokensFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event EarlyWithdrawPenaltyChange(EarlyWithdrawPenalty indexed penalty);
    event MinTimeToStake(uint256 indexed minTimeToStake);
    event IsEarlyWithdrawAllowed(bool indexed allowed);
    event StakeFeePercent(uint256 indexed stakeFeePercent);
    event RewardFeePercent(uint256 indexed rewardFeePercent);
    event FlatFeeAmount(uint256 indexed flatFeeAmount);
    event IsFlatFeeAllowed(bool indexed allowed);
        ...
        returns(bool)
    {
        ...
        if(noOfUsers == 1){
            // User maybe have more stakes than one
            totalAmount = 0;
            ...
            // If he is withdrawing everything that he's got
            if(amount == totalAmount){
                participants[id[user] - 1] = participants[participants.length - 1];
                id[participants[participants.length - 1]] = id[participants[id[user]]] - 1;
                delete id[user];
                participants.pop();
                noOfUsers--;
    ...
    {
        minTimeToStake = _minTimeToStake;
        emit MinTimeToStake(minTimeToStake);
    }
    ...
    */
    ...
    {
        isEarlyWithdrawAllowed = _isEarlyWithdrawAllowed;
        emit IsEarlyWithdrawAllowed(isEarlyWithdrawAllowed);
    }
    ...
    {
        stakeFeePercent = _stakeFeePercent;
        emit StakeFeePercent(stakeFeePercent);
    }
    ...
    {
        rewardFeePercent = _rewardFeePercent;
        emit RewardFeePercent(rewardFeePercent);
    ...
    {
        flatFeeAmount = _flatFeeAmount;
        emit FlatFeeAmount(flatFeeAmount);
    }
    ...
    {
        isFlatFeeAllowed = _isFlatFeeAllowed;
        emit IsFlatFeeAllowed(isFlatFeeAllowed);
    }
        ...
        validateStakeByStakeId(msg.sender, stakeId)
    {
        ...
        // NEWLY ADDED
        // Remove user if he withdraws whole amount
