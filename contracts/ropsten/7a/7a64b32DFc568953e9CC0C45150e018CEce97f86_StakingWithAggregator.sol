...
contract Staking is ReentrancyGuard, StakingInterface {
  ...
  modifier updateTerm() {
    ...
    if (_currentTerm < latestTerm) {
      ...
      if (nextTerm < latestTerm) {
        ...
        _termInfoList[latestTerm] = TermInfo({stakeAdd: 0, stakeRemove: 0, stakeSum: nextStakeSum, rewardSum: 0});
      }
  ...
  function _stake(address account, uint128 amount) internal {
    ...
    _accountInfoList[account].added = _accountInfoList[account].added.add(int256(amount)).toInt128(); // added when the term is shifted (the user)
  ...
  function _getNextStakeSum() internal view returns (uint128 nextStakeSum) {
    ...
    return currentTermInfo.stakeSum.add(currentTermInfo.stakeAdd).sub(currentTermInfo.stakeRemove).toUint128();
  }
  ...
  function _getNextTotalRemaining() internal view returns (uint256 nextTotalRemaining) {
    ...
    nextTotalRemaining = _totalRemaining.add(_getTotalRewardAdded()).add(currentTermInfo.stakeRemove);
  }
    ...
    )
  {
    ...
    nextTermStaking = termInfo.stakeSum.add(termInfo.stakeAdd).sub(termInfo.stakeRemove).toUint128();
  }
