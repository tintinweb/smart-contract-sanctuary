...
contract BlucaEggAirdrop is BlucaEggFactory {
    ...
    mapping(address => bool) isWhitelistMapping;
    mapping(address => bool) isClaimed;
    uint256 airdropFee = 0.0003 ether;
    ...
    modifier onlyAirdropWhitelist() {
        require(isWhitelistMapping[msg.sender], "address must in whitelist");
        _;
    }

    function isWhitelist() external view returns (bool) {
        return isWhitelistMapping[msg.sender];
    }
    ...
    function setWhitelist(
        ...
        for (uint256 idx = 0; idx < _addresses.length; idx++) {
            ...
            isWhitelistMapping[_addresses[idx]] = true;
        }
    }
    ...
    function updateIsClaimed(address _address) internal {
    ...
    function validateWhitelistParameter(
        ...
        require(
            addressCount > 0 &&
                ...
                elementCount > 0 &&
                tokenUriCount > 0,
            "each parameter cannot be empty"
        );
        require(
            addressCount <= 1000 &&
                rarityCount <= 1000 &&
                elementCount <= 1000 &&
                tokenUriCount <= 1000,
            "each parameter cannot have more than 1,000 items"
        );
        require(
            addressCount == rarityCount &&
                addressCount == elementCount &&
                addressCount == tokenUriCount,
            "every parameter must have equaly size"
        );
    }
}
