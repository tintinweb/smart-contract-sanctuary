...
//////////////////////////////////////////////////////////
//   _____  ___ ______ ___________ _   _ _     _        //
//  /  ___|/ _ \|  ___|  ___| ___ | | | | |   | |       // 
//  \ `--./ /_\ | |_  | |__ | |_/ | | | | |   | |       // 
//   `--. |  _  |  _| |  __|| ___ | | | | |   | |       // 
//  /\__/ | | | | |   | |___| |_/ | |_| | |___| |____   //
//  \____/\_| |_\_|   \____/\____/ \___/\_____\_____/   //
//                                                      //
/////////////////////////////////////////////////////////                                                
 ...
 *
 * This implements a BEP20 token named Safebull. On deployment it mints initialSupply to the
 ...
 * fees structure.
 */
contract SafeBull is Context, ERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint16 public _burnFee;
    uint16 public _rewardFee;
    uint16 public _liquidityFee;
    uint16 public _reserveFee;
    uint16 public _totalFee;

    address public _liquidityWallet;    // Holds LP wallet address
    ...
    event LiquidityWalletChanged(address fromAddress, address toAddress);
     ...
     * @notice constructor
     *
     * Mints 666,666,666 tokens and sends to deployer's address
     * Sets fees percents
     * Sets wallet address for dev and reserve
     *
     * Requirements -
     * - liquidityWallet and reserveWallet cannot be 0 address
     */
    constructor(
        address liquidityWallet,
        address reserveWallet
    )
        ERC20("SafeBull", "SAFEBULL")
    {
        _mint(_msgSender(), 1000000E18);    // Mint 1.000.000.000.000.000 tokens

        _rewardFee = 100;                   // 1 percent
        _burnFee = 100;                     // 1 percent
        _liquidityFee = 100;                // 1 percent
        _reserveFee = 100;                  // 1 percent
        _totalFee = _rewardFee + _burnFee + _liquidityFee + _reserveFee;

        require(liquidityWallet != address(0), "liquidity wallet address cannot be 0 address");
    ...
    */
    ...
    function _feesDistribution(uint256 amount) internal {
        ...
        if (tokensInCirculation > 0) {
            ...
            uint256 liquidityFeeAmount = amount.mul(uint256(_liquidityFee)).div(10000);
            uint256 reserveFeeAmount = amount.mul(uint256(_reserveFee)).div(10000);
            
            // Distribute fees
            _transfer(owner(), _liquidityWallet, liquidityFeeAmount);   // 1% of transaction amount sent to dev wallet
            ...
            emit TransactionFeeDistributed(rewardFeeAmount + burnFeeAmount + liquidityFeeAmount + reserveFeeAmount);
        }
    ...
    * This are individual accounts which holds the SAFEBULL tokens
    *
    ...
    */
    ...
    /**
     * @notice Updates _liquidityWallet with given _account
     * _account cannot be a contract or 0 address
     */
    function updateLiquidityWallet(address _account)
        public
        onlyOwner
    {
        ...
        _liquidityWallet = _account;
        emit LiquidityWalletChanged(_account, _liquidityWallet);
    }
