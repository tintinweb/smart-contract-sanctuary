...
//viewable functions
	...
	function showTenant() public view returns (address addr) {       //show tenant address.
		return tenant;
	}

	function showRentPerMonth() public view returns (uint256 balance) {       //show tenant address.
		return rentPer30Day;
	}

	function showDebugValue() public view returns (uint256 balance) {       //show tenant address.
		return debugValue;
	}
...
//renter function
    ...
    function payRent(uint8 _months) public payable isMultipleOf eligibleToPayRent{          //needs to be eligible to pay rent
		...
		debugValue = _rentdue + _additionalBlocks;
        _taxdeduct = (msg.value/100 * tax);                                 //deduct taxes
		debugValue = _taxdeduct;
        accumulated += (msg.value - _taxdeduct);     
		debugValue = accumulated;                               //accumulate revenues
