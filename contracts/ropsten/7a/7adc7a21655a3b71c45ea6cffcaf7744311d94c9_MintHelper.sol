...
contract MintHelper is Ownable {
  ...
  using SafeMath for uint;
    ...
    function proxyMergeMint(uint256 nonce, bytes32 challenge_digest, address[] tokens) //Renamed from function proxyMergeMint2(uint256 nonce, bytes32 challenge_digest, address[] tokens)
    public onlyOwner
    returns (bool)
    {
        require(ERC918Interface(mintableToken).mint(nonce, challenge_digest));
