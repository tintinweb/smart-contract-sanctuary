...
contract AuctionBase is Migrateable, Manageable, ExternallyCallable {
    ...
    struct AuctionReserves1e18 {
        bool filled; // fill auctions on first bid/withdraw of day
        uint8 mode;
        uint256 eth; // 1e-6 Amount of Eth in the auction
        uint256 token; // 1e-6 Amount of Axn in auction for day
        uint256 uniswapLastPrice; // 1e-6 Last known uniswap price from last bid
        uint256 uniswapMiddlePrice; // 1e-6 Using middle price days to calculate avg price
    }
