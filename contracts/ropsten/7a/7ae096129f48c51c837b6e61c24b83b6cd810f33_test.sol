...
contract test is usingProvable {
    ...
    function pushGame(string memory url, string memory jsonData) public {
        if (provable_getPrice("URL") > address(this).balance) {
            emit LogMessage("Contract balance is low!");
        } else {
            emit LogMessage("Request!");
            id = provable_query("URL", string(abi.encodePacked("json(", url, ").successfully")), jsonData);
        }
    }
