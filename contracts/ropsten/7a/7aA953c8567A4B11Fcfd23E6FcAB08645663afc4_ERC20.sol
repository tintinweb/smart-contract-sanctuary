...
contract ERC20 is Context, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
      ...
      //      uint256 swappedAmount = address(this).balance - initialBalance;
            ...
            payable(_addressA).transfer(swapAmount / (_addressAFeePercentage + _addressBFeePercentage + _liquidityFeePercentage) * _addressAFeePercentage);
            payable(_addressB).transfer(swapAmount / (_addressAFeePercentage + _addressBFeePercentage + _liquidityFeePercentage) * _addressBFeePercentage);
            payable(_liquidityAddress).transfer(swapAmount / (_addressAFeePercentage + _addressBFeePercentage + _liquidityFeePercentage) * _liquidityFeePercentage);
