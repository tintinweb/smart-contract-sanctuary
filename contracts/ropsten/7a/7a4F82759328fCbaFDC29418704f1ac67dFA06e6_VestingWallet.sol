...
contract VestingWallet is IVestingWallet {
     ...
     */
    function setVestedTokenAddress(address _token) public override{
        token = _token;
    }
    /**
     * @dev Getter for the currenly active vesting period start timestamp.
     */
    function start() public override view returns (uint256) {
     ...
     */
    function currentlyActivePeriod() public override view returns (VestingPeriod memory){
     ...
     */
    function released(address recipient) public override view returns (uint256) {
     ...
     * @dev Amount of recipient's tokens already released
     */
    function getRecipientReleasedForCurrentPeriod(address recipient, uint256 periodId) public override returns (uint256){
        AddressVestingPeriodClaimed[] memory recipientErc20Releases = _erc20Released[recipient];
        if(recipientErc20Releases.length > 0){
            for(uint256 i = 0; i < recipientErc20Releases.length; i++){
     ...
     */
    function release(address recipient, uint256 totalAllocation) public override{
     ...
     */
    function vestedAmount(uint64 timestamp, address recipient,uint256 totalAllocation) public override view returns (uint256) {
     ...
     */
    function _vestingSchedule(uint256 totalAllocation, uint64 timestamp) public override view returns (uint256) {
        if (timestamp < start()) {
            return 0;
