...
contract launch is Ownable {
    ...
    constructor() public {
        maxPayableAmount = 1000000000000000000;
    }
     ...
     * @return {bool}
     */
    function addWhitelist(address[] memory addresses)
        external
        onlyOwner
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < addressesLength; i++) {
            ...
            Whitelist memory whitelist_ =
                Whitelist(address_, 0, 0, true, false);
    ...
    function getStartTime() public pure returns (uint256) {
        return 1618252200; // EQUALIZER TIMESTAMP MOON
    }
    ...
    //get those tokens which are not sold
    function getRemainingTokens() external onlyOwner {
        uint256 remainingAmount = getTotalToken() - soldAmount;
        _token.safeTransfer(
            _msgSender(),
            remainingAmount
        );
