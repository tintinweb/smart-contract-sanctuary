...
// File contracts/hPools/HPoolFactory.sol
...
contract HPoolFactory is PausableUpgradeable, HordUpgradable {
    ...
    address public hPoolImplementation;
    address[] deployedHPools;

    modifier onlyHPoolManager() {
        require(msg.sender == hPoolManager, "Restricted only to HPoolManager.");
        _;
    }
        ...
        initializer
    {
        ...
        hPoolImplementation = _hPoolImplementation;
    }
    ...
    function setHPoolManager(address _hPoolManager) external {
        require(hPoolManager == address(0), "HPoolManager can not be 0x0 address.");
        require(_hPoolManager != address(0), "HPoolManager address is already set.");

        hPoolManager = _hPoolManager;
    }

    /**
    * @notice Function is setting address of deployed HPool contract.
    *
    * @param  _hPoolImplementation - address of new implementation of HPool contract
    */
    function setHPoolImplementation(
        address _hPoolImplementation
    )
    external
    onlyHordCongress
    {
        require(
            _hPoolImplementation != address(0),
            "HPoolImplementation can not be 0x0 address"
        );
    ...
    returns (address)
    {
        // Creates clone of HPool contract
        address cloneHPool = Clones.clone(hPoolImplementation);

        // Deploy the HPool contract
        IHPool(cloneHPool).initialize(
            hPoolId,
            bePoolId,
            hordCongress,
        ...
        deployedHPools.push(cloneHPool);
