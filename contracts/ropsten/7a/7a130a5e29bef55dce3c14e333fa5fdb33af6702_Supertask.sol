...
contract Supertask {
    ...
    event ContractCreation(address user, uint Fee, uint Volume, uint index, uint timestamp);
    ...
    function CreateContract() public payable {
        ...
        emit ContractCreation(msg.sender, msg.value, volume, NewContracts.length, block.timestamp);
    }
...
contract Contract {
    ...
    function Dispatching(bytes32 Memo) public {
        ...
        require(index + 2 <= chainLength, "You have no one to dispatch the good to.");
