...
contract L2 is ILayer2, Deserializer, PermissionGroups, ReentrancyGuard {
    ...
    /// This function sets publicKey and withdrawTo and increases numOccupiedAccounts for each account in the array parameters.
    /// Operator calls this function once to sync L1 accounts with L2 accounts after deploying a new contract.
    function setNewUser(
        bytes32[] calldata publicKey,
        address[] calldata withdrawTo
    ) external onlyOperator {
        require(publicKey.length == withdrawTo.length, "sizes are not matched");

        uint32 accountID = numOccupiedAccounts;
        require(accountID + publicKey.length <= MAX_ACCOUNT_ID, "overflow accountID");

        for (uint256 i = 0; i < publicKey.length; i++) {
            require(!pubKeyToAccountData[publicKey[i]].isAdded, "pubKey is already added");
            pubKeyToAccountData[publicKey[i]] = AccountData({isAdded: true, accountID: accountID});
            withdrawAddresses[accountID] = withdrawTo[i];
            accountID++;
        }

        numOccupiedAccounts = accountID;
    }
