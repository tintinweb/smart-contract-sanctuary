...
contract FreeAirdrop is Ownable {
    
  constructor() {
    owner = msg.sender;
  }
    
    function AirTransfer( address _tokenAddress, address payable[] memory _recipients, uint256[] memory _values ) payable public returns (bool) {
        require(_recipients.length > 0);
 
        IBEP20 token = IBEP20(_tokenAddress);
        
        //token.transfer( owner, _tipSomeTokensToDevPls );
