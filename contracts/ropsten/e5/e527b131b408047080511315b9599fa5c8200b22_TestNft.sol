...
contract TestNft is ERC721 {
    ...
    function approve(address _approved, uint256 _tokenId) override external payable {
        _handleApprove(_approved,_tokenId);
    }
    ...
    function _handleApprove(address _approved, uint256 _tokenId) private{
        address owner = _tokenIdOwnerMap[_tokenId];
    ...
    function approveOrder(address contractAddr,uint256 _tokenId,uint256 _price) public {
        _handleApprove(contractAddr,_tokenId);
        
        TestOrder o =TestOrder(contractAddr);
        o.addOrder(address(this),msg.sender,_tokenId,_price);
    }
}

abstract contract TestOrder {
    function addOrder(address _addr,address _tokenOwner, uint256 _tokenId, uint256 _price) public virtual;
}