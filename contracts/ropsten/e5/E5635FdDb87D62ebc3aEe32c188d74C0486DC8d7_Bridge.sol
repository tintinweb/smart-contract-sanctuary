...
contract Bridge is BridgeAdmin, Pausable {
    ...
    event WithdrawingNative(address indexed to, uint256 value);
    event WithdrawingToken(address indexed to, address indexed token, uint256 value);
    event WithdrawDoneNative(address indexed to, uint256 value);
    ...
    function withdrawNative(address payable to, uint value) public
    onlyOperator
    whenNotPaused
    positiveValue(value)
    returns(bool)
   ...
   //     require(taskHash == keccak256((abi.encodePacked(to,value,proof))),"taskHash is wrong");
    //    uint256 status = logic.supportTask(logic.WITHDRAWTASK(), taskHash, msg.sender, operatorRequireNum);
        emit WithdrawingNative(to, value);
        emit WithdrawDoneNative(to, value);
        to.transfer(value);
        return true;
