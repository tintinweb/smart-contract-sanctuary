...
contract Vega_2_TEST_STANDIN_DO_NOT_DEPLOY is ERC20 {
  address controller;

  event Controller_Changed(address indexed new_controller);

  constructor(uint256 total_supply_, string memory name_, string memory symbol_) ERC20 (name_, symbol_) {
  ...
  function faucet() public {
      require(last_faucets[msg.sender] < (block.timestamp - 86400), "limited to 1 call per 24 hours, try again later");
