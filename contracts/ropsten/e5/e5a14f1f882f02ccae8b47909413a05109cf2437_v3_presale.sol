...
contract v3_presale is Ownable {
...
// -- presale --
    function setPriceAndCap(uint256 _individualCapInBNB, uint256 _nbOfTokenPerBNB) external onlyOwner beforeSale {
        individualCapInBNB = _individualCapInBNB; //use 18 decimals
    ...
    function addWhitelisting(address[] calldata adrs) external onlyOwner {
        require(individualCapInBNB > 0, "First set the cap & price");
        for (uint256 i = 0; i < adrs.length; i++) {
            if (allowanceOf[adrs[i]] == 0) {
                allowanceOf[adrs[i]] = individualCapInBNB;
...
// -- post sale --
    ...
    function concludeAndAddLiquidity() external onlyOwner {
        ...
        if (balance_token.div(BNB_for_pool) >= nbOfTokenPerBNB) {
            balance_token = nbOfTokenPerBNB.mul(BNB_for_pool);
            } 
        else {
            BNB_for_pool = balance_token.div(nbOfTokenPerBNB);
            }
