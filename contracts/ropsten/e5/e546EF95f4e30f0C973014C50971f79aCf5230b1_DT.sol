...
contract DT is Context, Ownable, IERC20, LockableSwap, EthReflectingToken {
    using SafeMath for uint256;
    using Address for address;
    using FeeLibrary for FeeLibrary.Fees;
    using FeeLibrary for FeeLibrary.EthBuybacks;
    using FeeLibrary for FeeLibrary.SellFees;
    using FeeLibrary for FeeLibrary.SellFeeLevels;
    using FeeLibrary for FeeLibrary.FeeLevels;
    ...
    event UpdateFees(FeeLibrary.Fees oldFees, FeeLibrary.Fees newFees);
//    event UpdateTransferFees(Fees oldFees, Fees newFees);
//    event UpdateBuyerFees(Fees oldFees, Fees newFees);
    event UpdateSellerFees(FeeLibrary.SellFees oldSellFees, FeeLibrary.SellFees nesSellFees);
...
//    event SwapAndLiquifyEnabledUpdated(bool enabled);
    ...
    struct LibraryLevels{
        FeeLibrary.FeeLevels LEVEL1;
        FeeLibrary.FeeLevels LEVEL2;
        FeeLibrary.FeeLevels LEVEL3;
        FeeLibrary.FeeLevels LEVEL4;
        FeeLibrary.FeeLevels LEVEL5;
    }

    LibraryLevels FeeLevels;

    FeeLibrary.SellFeeLevels private sellFees;
    FeeLibrary.SellFeeLevels private previousSellFees;

    FeeLibrary.Fees public buyFees = FeeLibrary.Fees({reflection: 3, project: 3, liquidity: 2, burn: 2, marketing: 1, ethReflection: 0});
    FeeLibrary.Fees public transferFees = FeeLibrary.Fees({reflection: 1, project: 1, liquidity: 1, burn: 0, marketing: 0, ethReflection: 0});

    FeeLibrary.Fees private previousBuyFees;
    FeeLibrary.Fees private previousTransferFees;
    ...
    FeeLibrary.EthBuybacks public buybackTokenTracker = FeeLibrary.EthBuybacks({liquidity : 0, redistribution : 0, buyback : 0});
    ...
    function removeAllFee() private {
        previousBuyFees.setFrom(buyFees);
        previousTransferFees = transferFees;
        previousSellFees.setFrom(sellFees);

        buyFees.setToZero();
        transferFees.setToZero();
        sellFees.setToZero();
    }

    function restoreAllFee() private {
        buyFees.setFrom(previousBuyFees);
        transferFees = previousTransferFees;
        sellFees.setFrom(previousSellFees);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            (rAmount, tTransferAmount, rTransferAmount) = calculateForOwner(amount);
        }
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(automatedReflectionsEnabled && (!distributionRoundCompleted || nextRoundStartTime > block.timestamp)){
            reflectionContract.reflectRewards();
        }
    }
    ...
    function calculateSell(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        if(block.timestamp > trader[from].lastBuy.add(sellFees.level[FeeLevels.LEVEL5].saleCoolDownTime)) {
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLevels.LEVEL1].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLevels.LEVEL1].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLevels.LEVEL2].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLevels.LEVEL2].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLevels.LEVEL3].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLevels.LEVEL3].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLevels.LEVEL4].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLevels.LEVEL4].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLevels.LEVEL5].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLevels.LEVEL5].saleCoolDownFee;
        }

        uint256 finalSaleFee = _minimumSizeFee;
        uint256 poolSize = amountInPool();
        if(amount < poolSize.mul(sellFees.level[FeeLevels.LEVEL5].saleSizeLimitPercent).div(100)){
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(amount > poolSize.mul(sellFees.level[FeeLevels.LEVEL1].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLevels.LEVEL1].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[FeeLevels.LEVEL2].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLevels.LEVEL2].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[FeeLevels.LEVEL3].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLevels.LEVEL3].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[FeeLevels.LEVEL4].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLevels.LEVEL4].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[FeeLevels.LEVEL5].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLevels.LEVEL5].saleSizeLimitPrice;
        }
    ...
    function _burn(uint256 tAmount) private {
        ...
        reflectionContract.updateTotalSupply(_tTotal);
    }
    ...
    function updateBuyerFees(uint256 reflectionFee, uint256 projectFee, uint256 liquidityFee, uint256 burnFee, uint256 marketingFee, uint256 ethReflectionFee) public onlyOwner {
        FeeLibrary.Fees memory oldBuyFees = buyFees;
        buyFees.setTo(reflectionFee, projectFee, liquidityFee, burnFee, marketingFee, ethReflectionFee);
    ...
    function updateTransferFees(uint256 reflectionFee, uint256 projectFee, uint256 liquidityFee, uint256 burnFee, uint256 marketingFee, uint256 ethReflectionFee) public onlyOwner {
        FeeLibrary.Fees memory oldTransferFees = transferFees;
        transferFees.setTo(reflectionFee, projectFee, liquidityFee, burnFee, marketingFee, ethReflectionFee);
    ...
    function updateSellerFees(uint8 _level, uint256 upperTimeLimitInHours, uint256 timeLimitFeePercent, uint256 saleSizePercent, uint256 saleSizeFee ) public onlyOwner {
        ...
        FeeLibrary.FeeLevels level = FeeLibrary.FeeLevels(_level - 1);
        FeeLibrary.SellFees memory oldSellFees = sellFees.level[level];
        sellFees.level[level].setTo(upperTimeLimitInHours * 1 hours, timeLimitFeePercent, saleSizePercent, saleSizeFee);
        emit UpdateSellerFees(oldSellFees, sellFees.level[level]);
    }
...
//    }
    ...
    function buyBackTokens(uint256 amount) internal {
        if (amount > minBuyBack && amount < address(this).balance) {
            if(amount > maxBuyBack)
                amount = maxBuyBack;
            SwapWithLP.swapETHForTokens(uniswapV2Router, amount, buybackSinkAddress);
        }
    ...
    function init(address trueOwner) public onlyOwner{
        ...
        FeeLevels = LibraryLevels({
            LEVEL1: FeeLibrary.FeeLevels.LEVEL1,
            LEVEL2: FeeLibrary.FeeLevels.LEVEL2,
            LEVEL3: FeeLibrary.FeeLevels.LEVEL3,
            LEVEL4: FeeLibrary.FeeLevels.LEVEL4,
            LEVEL5: FeeLibrary.FeeLevels.LEVEL5
        });

        sellFees.initSellFees();
//        openTrading();  // TODO Remove for mainnet
//        transferOwnership(trueOwner);
        runOnce = 0;
    }

    function getSellFees() external view returns(FeeLibrary.SellFees memory, FeeLibrary.SellFees memory, FeeLibrary.SellFees memory, FeeLibrary.SellFees memory, FeeLibrary.SellFees memory) {
        return sellFees.getAll();
    }
}
...
contract EthReflectingToken is Ownable, ISupportingExternalReflection {
    ...
    bool automatedReflectionsEnabled = false;

    function takeEthReflection(uint256 amount) internal virtual {
        if(amount > 0)
            _takeEthReflection(amount);
    }

    function _takeEthReflection(uint256 amount) private returns(bool success) {
        success = false;
        if(address(this).balance >= amount){
            if(automatedReflectionsEnabled){
                success = reflectionContract.depositEth{value: amount}();
            } else {
                (success,) = address(reflectionContract).call{value: amount}("");
            }
        }
    }
...
//    address public uniswapV2Pair;
    ...
    function swapETHForTokens(IUniswapV2Router02 uniswapV2Router, uint256 amount, address destination) public {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
...
library FeeLibrary {
    ...
    function setToZero(Fees storage fees) public {
        fees.reflection = 0;
        fees.project = 0;
        fees.liquidity = 0;
        fees.burn = 0;
    ...
    function setToZero(SellFees storage fees) public {
    ...
    function setTo(SellFees storage fees, uint256 upperTimeLimitInHours, uint256 timeLimitFeePercent, uint256 saleSizePercent, uint256 saleSizeFee) public {
    ...
    function setTo(SellFees storage fees, SellFees storage newFees) public {
    ...
    function setToZero(SellFeeLevels storage leveledFees) public {
    ...
    function setFrom(SellFeeLevels storage leveledFees, SellFeeLevels storage newLeveledFees) public {
    ...
    function initSellFees(SellFeeLevels storage sellFees) public {
...

{
  ...
  "libraries": {
    "contracts/libs/FeeLibrary.sol": {
      "FeeLibrary": "0xf7327c2a284834cd95ea6669f27525d7e5000c8c"
    },
    "contracts/libs/SwapWithLP.sol": {
      "SwapWithLP": "0xa4fa0bc80b1f45f1d3126b556679428744bb56aa"
    }
  }
}