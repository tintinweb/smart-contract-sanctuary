...
contract AuctionWithAdmin is Ownable {
	...
	function withdraw(uint256 _tokenId) public {
	    ...
	    if(msg.sender == token_seller[_tokenId]){
	        payable(admin).transfer(auction[_tokenId].highestBid.mul(auction_token_fee).div(100));
	        payable(msg.sender).transfer(auction[_tokenId].highestBid.mul(100 - auction_token_fee).div(100));
