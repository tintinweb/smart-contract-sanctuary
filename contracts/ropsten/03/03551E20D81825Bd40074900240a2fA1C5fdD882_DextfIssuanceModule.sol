...
contract DextfIssuanceModule is ModuleBase, ReentrancyGuard  {
    ...
    returns (address[] memory, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            uint256 quantityFromPosition = _setToken.getDefaultPositionRealUnit(components[i]).toUint256().preciseMulCeil(_quantity);
            // We get the maximum to avoid token drains when dividends are paid
            notionalUnits[i] = quantityFromPosition.max(componentRedeemQuantities[i]);
        }
