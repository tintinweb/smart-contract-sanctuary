...
contract BloxMoveLiquidity is ILiquidity, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    */
    ...
    /**
    * @dev Receive native currency from from treasury
    */
    ...
    /**
    * @dev Replace constructor for initialize
    * @param _rate0 Unlocked bucket reward rate
    * @param _rate30 Locked 30 days reward rate
    * @param _rate60 Locked 30 days reward rate
    * @param _rate90 Locked 30 days reward rate
    */
    ...
    */
    ...
    function _setLiquidity(uint _amount, uint16 _lockedDays) private {
        ...
        Liquidity memory liquidity = _getLiquidty(target);
    ...
    function _getLiquidty(uint _day) private view returns(Liquidity memory) {
        Liquidity memory liquidity = dayLiquiditys[_day];
        if (liquidity.total == 0) {
            (uint total, uint unlocked, uint locked30Days, uint locked60Days, uint locked90Days) = _getLiquiditys(_day);
    ...
    */
    function removeLiquidity(uint _idx, uint _liquidityOut) external override nonReentrant returns(uint amountToken, uint amountWei, uint rewards) {
        ...
        uint day = block.timestamp.div(1 days).add(1);
        Liquidity memory liquidity = _getLiquidty(day);
        liquidity.total -= _liquidityOut;
        if (lockedDays == 0) {
            liquidity.unlocked -= _liquidityOut;
            
        } else if (lockedDays == 30) {
            liquidity.locked30Days -= _liquidityOut;

        } else if (lockedDays == 60) {
            liquidity.locked60Days -= _liquidityOut;

        } else if(lockedDays == 90) {
            liquidity.locked90Days -= _liquidityOut;
        }
        dayLiquiditys[day] = liquidity;
