...
contract HeadShot is Ownable, IERC20 {
    ...
    function updateDividendFee(uint256 value) external onlyOwner {
        dividendFeeBPS = value;
        calcAllFee();
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "SHOT: approve from the zero address");
        require(spender != address(0), "SHOT: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "SHOT: mint to the zero address");
        _totalSupply += amount;
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "SHOT: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "SHOT: burn amount exceeds balance");
