...
interface Ohdat721NFT{
    function mint(address account, uint256 id)external;
    function exists(uint256 tokenId) external view returns (bool);
}


contract OhdatExchange721 is AccessControl, Pausable {
    using SafeMath for uint256;
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    string  public name = "OhdatExchange721";
    ...
    function _mintNft(uint256 _mintNftTokenId)internal whenNotPaused returns(bool) {
        Ohdat721NFT Ohdat721NftContract = Ohdat721NFT(nftContractAddress);
        Ohdat721NftContract.mint(msg.sender,_mintNftTokenId);
        return true;
    }
    ...
    function _validateNftMintable(uint256 _contractTokenId,address _contractAddress)internal whenNotPaused view returns(bool){
        Ohdat721NFT Ohdat721NftContract = Ohdat721NFT(_contractAddress);
        if (Ohdat721NftContract.exists(_contractTokenId)){
            return false;
        }else{
            return true;
        }
