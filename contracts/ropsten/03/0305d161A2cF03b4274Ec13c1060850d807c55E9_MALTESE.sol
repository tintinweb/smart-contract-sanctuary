...
contract MALTESE is ERC20, Ownable {
    ...
    uint8 public feeliq = 5;
    uint8 public feeburn = 2;
    uint8 public feedev = 2;
    
    uint8 public feesum = feeliq + feeburn + feedev;
    
    address payable public devwallet = payable(0x60719aA7c772dbdf50478d33afea640a8c2e964f);

    uint256 public buylimit = 1e5;
    ...
    mapping(address => lock) private vesting;
    bool private vestingEnabled = true;
    ...
    constructor() ERC20("Maltese Coin", "MALTESE") {
        ...
        IUniswapV2Router02 _router = IUniswapV2Router02(UniRouter);
        router = _router;

        //pair = IUniswapV2Factory(_router.factory()).createPair(address(this), _router.WETH());
    }
    
    function Transfer_vested(address recipient, uint256 amount, uint256 timetorelease) public {
        ...
        //add time to release to current blocktime, only if no vesting exist at address
        if (vesting[recipient].releasetime == 0) {
            vesting[recipient].releasetime = block.timestamp + timetorelease;
        }
        
        _transfer(_msgSender(), recipient, amount);
        
        vesting[recipient].amount += balanceOf(recipient) - balancebefore;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        //check vesting
        require(vesting[sender].releasetime <= block.timestamp || balanceOf(sender) - amount >= vesting[sender].amount,"can't withdraw that many tokens, still vested");
        ...
        //reset vesting if necessary
        if (vesting[sender].releasetime <= block.timestamp && vesting[sender].releasetime > 0) {
            vesting[sender].releasetime = 0;
            vesting[sender].amount = 0;
        }
    }
    ...
    function changeVestingState(bool state) external onlyOwner {
        //fallback so vesting can be disabled if abused in any kind of way
        vestingEnabled = state;
    }
