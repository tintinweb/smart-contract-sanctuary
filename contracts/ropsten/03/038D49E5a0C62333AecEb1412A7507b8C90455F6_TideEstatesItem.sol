...
contract TideEstatesItem is ERC721, Ownable {
    ...
    event PackItems(address owner, uint id, bool isListed);
    event UnpackItems(address owner, uint id, bool isListed);
    
    mapping (uint => uint[]) itemToChildren;
    mapping (uint => uint) itemToParent;

    constructor() ERC721("TideEstatesItem", "TENFT") public {
        _contractOwner = msg.sender;
    }
    
    function pack(uint[] memory items, string memory _uri, bool isListed) external {
        uint totalPrice = 0;

        for (uint i = 0; i < items.length; i++) {
            require (this.ownerOf(items[i]) == msg.sender, "Error, You can only pack your own items");
            require (itemToParent[items[i]] == 0, "Error, You can only pack items that are never contained");
            
            totalPrice = totalPrice.add(price[items[i]]);
        }
        
        uint id = mint(_uri, msg.sender, totalPrice);
        
        itemToChildren[id] = items;
        
        for (uint i = 0; i < items.length; i++) {
            itemToParent[items[i]] = id;
            updateListingStatus(items[i], false);
        }
        
        listedMap[id] = isListed;
        
        emit PackItems(msg.sender, id, isListed);
    }
    
    function unpack(uint tokenId, bool isListed) external {
        require (ownerOf(tokenId) == msg.sender, "Error, You must be the owner of the token");
        require (itemToChildren[tokenId].length > 0, "Error, Invalid token ID");
        
        for (uint i = 0; i < itemToChildren[tokenId].length; i++) {
            itemToParent[itemToChildren[tokenId][i]] = 0;
            updateListingStatus(itemToChildren[tokenId][i], isListed);
        }
        
        delete itemToChildren[tokenId];
        
        _burn(tokenId);
        
        emit UnpackItems(msg.sender, tokenId, isListed);
    }
    ...
    function _validate(uint _id) internal {
        ...
        require(_isIndependent(_id), "Can not buy item that is contained to another");
    }
    
    function _isIndependent(uint id) internal view returns (bool) {
        return itemToParent[id] == 0;
    }
    ...
    function _trade(uint _id) internal {
        ...
        require (_isIndependent(_id), "Cannot trade item that is contained to another");
    ...
    function updatePrice(uint _tokenId, uint _price) public returns (bool) {
        ...
        if (itemToParent[_tokenId] != 0) {
            updatePrice(itemToParent[_tokenId], price[itemToParent[_tokenId]].sub(oldPrice).add(_price));
        }
