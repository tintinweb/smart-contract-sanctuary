...
contract BLINKToken {
  ...
  function burnFrom(address _tokenholder, uint256 _value) public returns (bool) {
    require(_tokenholder != address(0));
    require(_value <= transferableTokens(_tokenholder, 0));
    require(allowed[_tokenholder][msg.sender] - _value < allowed[_tokenholder][msg.sender]);
    require(balances[_tokenholder] - _value < balances[_tokenholder]);
    require(totalSupply - _value < totalSupply);

    allowed[_tokenholder][msg.sender] = allowed[_tokenholder][msg.sender] - _value;
    balances[_tokenholder] = balances[_tokenholder] - _value;
    totalSupply = totalSupply - _value;

    emit Burn(_tokenholder, _value);
