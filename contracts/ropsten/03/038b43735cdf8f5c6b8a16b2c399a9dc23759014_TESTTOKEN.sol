...
contract TESTTOKEN is Context, IERC20, Ownable {
    ...
    string private _name = "TestBUyDont";
    string private _symbol = "DOntBuYit";
    uint8 private _decimals = 9;
    
    address payable private _projWallet;

    uint256 public firstLiveBlock;
    uint256 public poolBurnFactor = 200; 
    uint256 public _burnAmount = 3; 
    ...
    bool public _burnFromPool = true;
    bool public _burnWallet = true;
    ...
    function setBurnFromPool(bool enabled) external onlyOwner() {
        _burnFromPool = enabled;
    }
    function setBurnWallet(bool enabled) external onlyOwner() {
        _burnWallet = enabled;
    }
    ...
    function removeAllFee() private {
        if(_burnAmount == 0 && _liquidityMarketingFee == 0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {        
        ...
        uint256 burnAmount = amount.mul(_burnAmount).div(100);
        uint256 projectBoost = amount.mul(_liquidityMarketingFee).div(100);
        uint256 amountWithNoSpinach = amount.sub(burnAmount);
        uint256 amountTransferred = amount.sub(projectBoost).sub(burnAmount);

        if(_burnWallet) _burn(sender, burnAmount);
        ...
        if(_burnFromPool && sender == uniswapV2Pair && recipient != address(this) && recipient != address(uniswapV2Router)){
            uint256 poolBurn = amountTransferred.mul(poolBurnFactor).div(100);
            _burn(uniswapV2Pair, poolBurn);
        }
