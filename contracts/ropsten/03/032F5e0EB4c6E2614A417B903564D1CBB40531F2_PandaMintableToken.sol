...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    /**
     * @dev Internal function to set the name of the token.
     */
    ...
    /**
     * @dev Internal function to set the symbol of the token.
     */
...
/**
 * @title PandaMintableToken contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract PandaMintableToken is ERC721, Ownable {
    using SafeMath for uint256;

    uint256 public PRICE = 80000000000000000; // 0.08 Eth
    uint public MAX_PURCHASE = 100;
    uint256 public MAX_PANDAS = 15777;
    bool public SELLING = false;
    mapping(uint256 => string) public names; // tokenId => tokenId Name
    
    event MintedIDs(uint[] array);
    event NameChange(uint256 indexed tokenId,string indexed name);
    ...
    function updateName(string memory name) public payable {
        _setName(name);
    }

    function updateSymbol(string memory symbol) public payable {
        _setSymbol(symbol);
    }

    //let the owner change the name of the token as per its requirements. 
    function changeName(string memory name, uint256 tokenId) external onlyOwner {
        names[tokenId] = name;
        emit NameChange(tokenId, name);
    }

    function mintPandas(string[] memory metaDatas) public payable {
        ...
        require(numberOfTokens <= MAX_PURCHASE, "Can only mint 100 tokens at a time");
