...
contract EntityOfferRegistry is Owner {
    ...
    function stopSubscription(address _subscriptionOwner, uint _offerIndex) public isExistingOffer(_offerIndex) {
        require(subscribers[_subscriptionOwner][_offerIndex] > block.timestamp, "Subscription is already inactive");

        uint oldExpirationTimestamp = subscribers[msg.sender][_offerIndex];
