...
contract BaseZooFunctions is IZooFunctions, Ownable
{
	...
	function decideWins(uint256 votesForA, uint256 votesForB, uint256 random) override external pure returns (bool)
	{
	...
	function getRandomNumber(uint256 seed) override external view returns (uint256 random) {
	...
	function computeVotesByDai(uint256 amount) override external view returns (uint256 votes)
	{
	...
	/// @return votes - final amount of votes after calculating.
	function computeVotesByZoo(uint256 amount) override external view returns (uint256 votes)
	{
...
contract NftBattleArena is Ownable, ERC721
{
	...
	{
		require(allowedForStaking[token] == true, "not allowed!");             // Requires for nft-token to be from allowed contract.
		require(getCurrentStage() == Stage.FirstStage, "Wrong stage!");            // Requires to be at first stage in battle epoch.
		...
		stakingPositions[numberOfPositions].rewardDate = block.timestamp;
	...
	function createNewVotingPosition(uint256 stakingPositionId, uint256 amount) public returns (uint256 votes)
	{
		...
		require(_exists(stakingPositionId), "wrong id");                   // Requires for id to exist.
		require(typeOfPositions[stakingPositionId] == PositionType.StakerPosition, "Wrong position type");// Requires to be staker position type.
		require(stakingPositions[stakingPositionId].endEpoch == 0, "wrong id");   // Requires for nft from staker position to be staked.
	...
	function pairNft(uint256 stakingPositionId) external
	{
		...
		require(_exists(stakingPositionId), "wrong id");                           // checks if this id exist in battles nft.
		require(numberOfNftsWithNonZeroVotes / 2 > nftsInGame / 2, "there is no opponent"); // Requires enough nft for pairing.
		require(typeOfPositions[stakingPositionId] == PositionType.StakerPosition, "Wrong position type"); // Requires to be staker position type.
	...
	function voteWithZoo(uint256 stakingPositionId, uint256 votingPositionId, uint256 amount) public returns (uint256 votes)
	{
		...
		require(ownerOf(votingPositionId) == msg.sender, "wrong id");      // Checks for existence and ownership of voting position.
		...
		require(votes <= votingPositions[votingPositionId].daiVotes); // Requires for votes from zoo to be less than votes from dai.
	...
	{
		require(pairsInEpoch[currentEpoch][pairIndex].playedInEpoch != true, "winner already chosen!"); // Requires to be called only once for pair in epoch.
	...
	function updateEpoch() public {
		...
		require(block.timestamp >= epochStartDate + epochDuration || numberOfPlayedPairsInEpoch[currentEpoch] == pairsInEpoch[currentEpoch].length, "not time yet"); // Requires fourth stage to end, or determine every pair winner.
	...
	{
		require(votingPositions[votingPositionId].endEpoch == 0, "Position liquidated");  // Requires to be not liquidated yet.
		require(getCurrentStage() == Stage.FirstStage, "Wrong stage!");                   // Requires to be at first stage.
		require(ownerOf(votingPositionId) == msg.sender, "wrong id");               // Requires to be owner of position.
	...
	function recomputeDaiVotes(uint256 votingPositionId) external
	{
		...
		stakingPositions[votingPositionId].rewards[currentEpoch].votes += newVotes - votes; // Increases rewards for staker position for added amount of votes in this epoch.
	...
	function recomputeZooVotes(uint256 votingPositionId) external
	{
		...
		require(newZooVotes <= votingPositions[votingPositionId].daiVotes); // Requires for votes from zoo to be less than votes from dai.
		...
		stakingPositions[votingPositionId].rewards[currentEpoch].votes += delta;    // Adds amount of recently added votes to reward for staker position for current epoch.
	...
	function addZooToPosition(uint256 votingPositionId, uint256 amount) external returns (uint256 votes)
	{
		...
		require(zooVotes + votes <= votingPositions[votingPositionId].daiVotes);// Requires for votes from zoo to be less than votes from dai.
	...
	// function claimRewardForStaking(uint256 stakingPositionId) external 
	// {
	// 	require(ownerOf(stakingPositionId) == msg.sender, "Not the owner!");
	// 	uint256 rewardDate = stakingPositions[stakingPositionId].rewardDate + 182 days;
	// 	require(block.timestamp > rewardDate);
	// 	uint256 reward = 10 * 10 ** 18; // 10 zoo. 
	// 	zoo.transfer(msg.sender, reward);
	// 	rewardDate += 182 days;
	// }

	// function claimRewardForVoting(uint256 votingPositionId) external 
	// {
	// 	require(ownerOf(votingPositionId) == msg.sender, "Not the owner!");
	// 	uint256 rewardDate = VotingPosition[votingPositionId].rewardDate + 182 days; // todo: set require.
	// 	require(block.timestamp > rewardDate);
	// 	uint256 reward = 10 * 10 ** 18; // 10 zoo todo: set reward.
	// 	zoo.transfer(msg.sender, reward);
	// 	rewardDate += 182 days;
	// }
