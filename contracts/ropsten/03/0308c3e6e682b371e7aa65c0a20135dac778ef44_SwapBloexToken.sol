...
contract SwapBloexToken is ERC20, ERC20Detailed {
    ...
    function deposit(uint _amount) public {
        ...
        //pay bt
        btToken.safeTransferFrom(address(this),msg.sender, bt_amount);
    ...
    //getUsdtSwapAmount
    function getUsdtSwapAmount(uint256 _btAmount) public view returns (uint256) {
         //swap back usdt amount
        uint usdt_amount = _btAmount.mul(btRate).div(rateBase).div(1e12);  //6 decimals
        return usdt_amount;

    }    
    //getBtwapAmount
    function getBtwapAmount(uint256 _usdtAmount) public view returns (uint256) {
        //get bt amount
        uint bt_amount = _usdtAmount.mul(rateBase).div(btRate).mul(1e12);  //18 decimals      
        return bt_amount;
    }    
