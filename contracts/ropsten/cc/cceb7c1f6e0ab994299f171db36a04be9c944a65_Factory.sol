...
contract Factory is Configurable, ContextUpgradeSafe, Constants {
    ...
    function __Factory_init(address governor, address implCall, address implPut, address feeTo, address swapRounter) public initializer {
        __Governable_init_unchained(governor);
        __ReentrancyGuard_init_unchained();
        __Factory_init_unchained(implCall, implPut, feeTo, swapRounter);
    }

    function __Factory_init_unchained(address implCall, address implPut, address feeTo, address swapRounter) public governance {
        productImplementations[_Call_]  = implCall;
        productImplementations[_Put_]   = implPut;
        setFee(0.005 ether, feeTo);     // 0.5%
        config[_swapRounter_]           = uint(swapRounter);
        config[_WETH_]                  = uint(IUniswapV2Router01(swapRounter).WETH());
    ...
    function _transfer(address payable sender, address callOrPut, address undOrCur, int vol, int max, address[] memory path) internal {
        ...
        IUniswapV2Router01 rounter = IUniswapV2Router01(config[_swapRounter_]);
        ...
        if(vol > 0) {
            ...
            if(path.length == 0) {
                ...
                if(msg.value > 0 && undOrCur == WETH) {
                    require(msg.value >= v, 'msg.value not enough');
                    IWETH(config[_WETH_]).deposit{value: v}();
                    if(msg.value > v)
                ...
                if(msg.value > 0 && path[0] == WETH) {
                    uint balance = address(this).balance;
                    rounter.swapETHForExactTokens{value: msg.value}(v, path, callOrPut, deadline);
                    if(address(this).balance > balance)
                        sender.transfer(address(this).balance - balance);
                } else {
                    uint amount = rounter.getAmountsIn(v, path)[0];
                    IERC20(path[0]).safeTransferFrom(sender, address(this), amount);
                    IERC20(path[0]).approve(address(rounter), amount);
                    rounter.swapTokensForExactTokens(v, uint(-max), path, callOrPut, deadline);
                }
            }
        } else if(vol < 0) {
            uint v = uint(-vol).sub(fee);
            Call(callOrPut).withdraw_(undOrCur == WETH || path.length > 0 ? address(this) : sender, v);
            if(path.length == 0) {
                require(-int(v) <= max, _slippage_too_high_);
                if(undOrCur == WETH) {
                    IWETH(config[_WETH_]).withdraw(v);
                ...
                IERC20(undOrCur).approve(address(rounter), v);
                if(path[path.length - 1] == WETH)
                    rounter.swapExactTokensForETH(   v, uint(-max), path, sender, deadline);
                else
                    rounter.swapExactTokensForTokens(v, uint(-max), path, sender, deadline);
            }
        }
        Call(callOrPut).withdraw_(address(config[_feeTo_]), fee);
    }
    ...
    function _checkMistakeETH(address payable sender, address underlying, address currency, int dUnd, int dCur, address[] calldata undPath, address[] calldata curPath) internal {
        address WETH = address(config[_WETH_]);
        if(!(msg.value == 0
            || (undPath.length > 0 && undPath[0] == WETH && dUnd > 0)
            || (curPath.length > 0 && curPath[0] == WETH && dCur > 0)
            || (undPath.length == 0 && underlying == WETH && dUnd > 0)
            || (curPath.length == 0 && currency == WETH && dCur > 0))
        )
            sender.transfer(msg.value);
    }
    ...
    function _swap(address payable sender, address underlying, address currency, uint priceFloor, uint priceCap, int dCall, int dPut, int undMax, int curMax, bytes memory data) internal nonReentrant returns (address call, address put, int dUnd, int dCur) {
        ...
        require(SafeMath.add_(dUnd, Math.abs(dUnd).mul(feeRate).div(1e18)) <= undMax && SafeMath.add_(dCur, Math.abs(dCur).mul(feeRate).div(1e18)) <= curMax, _slippage_too_high_);
...
/*
    ...
    function swapRoute(SwapRouteParam calldata s) external payable nonReentrant returns (address call, address put, int dUnd, int dCur) {
        ...
        //require(SafeMath.add_(dUnd, Math.abs(dUnd).mul(feeRate).div(1e18)) <= s.undMax && SafeMath.add_(dCur, Math.abs(dCur).mul(feeRate).div(1e18)) <= maxes[1], _slippage_too_high_);
        ...
        if(s.data.length > 0)
            IFlashSwapCallee(_msgSender()).onFlashSwap(_msgSender(), call, put, s.dCall, s.dPut, dUnd, dCur, s.data);
...
*/
    ...
    function _onFlashSwapRoute(address sender, address[2] memory call_put, int[2] calldata dCallPut, int[2] memory dUndCur, bytes memory data) internal {
        IFlashSwapCallee(sender).onFlashSwap(sender, call_put[0], call_put[1], dCallPut[0], dCallPut[1], dUndCur[0], dUndCur[1], data);        
    }
/*
...
/*
    ...
    function calcDeltaRoute(uint priceFloor, uint priceCap, uint totalCall, uint totalPut, int dCall, int dPut, uint slippage, address[] calldata undPath, address[] calldata curPath) public view returns (int undMax, int curMax, uint totalUnd, uint totalCur) {
        (undMax, curMax, totalUnd, totalCur) = calcDeltaWithFeeAndSlippage(priceFloor, priceCap, totalCall, totalPut, dCall, dPut, slippage);
        undMax = calcAmountRoute(undMax, undPath);
        curMax = calcAmountRoute(curMax, curPath);
    }
    
    function calcAmountRoute(int amount, address[] calldata path) public view returns (int) {
        IUniswapV2Router01 rounter = IUniswapV2Router01(config[_swapRounter_]);
        if(path.length > 0)
            if(amount > 0)
                return int(rounter.getAmountsIn(uint(amount), path)[0]);
            else if(amount < 0)
                return -int(rounter.getAmountsOut(uint(-amount), revertPath(path))[path.length-1]);
        return amount;
    }
