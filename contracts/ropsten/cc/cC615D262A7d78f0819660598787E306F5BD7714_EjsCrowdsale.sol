...
/**
 * @title PausableCrowdsaleHelper
 * @dev Helper for crowdsale where purchases can be paused and unpaused by the pauser account.
 */
contract PausableCrowdsaleHelper is Pausable {
    address public pauserAccount;

    constructor() {
        pauserAccount = msg.sender;
    }

    modifier onlyBy(address account) {
        require(
            msg.sender == account,
            "PausableCrowdsaleHelper: sender unauthorized"
        );
        _;
    }

    function pause() external onlyBy(pauserAccount) {
        _pause();
    }

    function unpause() external onlyBy(pauserAccount) {
        _unpause();
    }

    function setPauserAccount(address account) external onlyBy(pauserAccount) {
        require(account != address(0), "PausableCrowdsaleHelper: zero account");

        pauserAccount = account;
    }
}
