...
contract PURRtoken is Context, IERC20Metadata, Ownable { 
    ...
    uint256 public WIN_REWARD = 1000000000000000000; //1 PURR/win
    ...
    function tokensToClaim(uint16 tokenIndex) public view returns (uint256) {
        ...
        uint256 totalAccumulated = (block.timestamp).sub(lastHarvested).mul(PURR_PER_DAY).div(SECONDS_PER_DAY).mul(_rarityMultiplier);
