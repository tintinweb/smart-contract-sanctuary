...
contract Settings {
    ...
    uint256 internal percentNextround = 350;
    uint256 internal percentCost = 250;
...
contract BleedFomo is Ownable, Settings{
    ...
    uint256 private endTimeSettingIncrease2 = 10 minutes;
    uint256 private nextroundInterval = 30 minutes; //from gameend to gamestart
    ...
    uint256 private conditionKeyPriceIncrease = 5;
    ...
    function _calcCheckoutTime(uint256 _now) private {
        uint256 _flagtime = 1537424580;  // GMT+8 2018/9/11/16:00
        if (checkoutTime == 0) {
    ...
    function flushShareNext() private {
        uint256 k = 0;
        ...
        for(k=0; k<useraddrs_nextround.length; k++){
            ...
            if(userkeys[_tmpaddr_next] == 0){
                useraddrs.push(_tmpaddr_next);
            }
            userkeys[_tmpaddr_next] = userkeys[_tmpaddr_next].add(userkeys_nextround[_tmpaddr_next]);
    ...
    function CheckOut(uint256 _executeCount) public returns (uint256) {
        ...
        emit onCheckOut(checkoutCurr, useraddrs.length);
        return rst;
    }
    ...
    function joinGame(address _sender, uint256 _val, address _upper)  payable validValue(_val) public {
        ...
        require(nkeys >= 5000, "to litte eth");   // num of keys must > 0.6
