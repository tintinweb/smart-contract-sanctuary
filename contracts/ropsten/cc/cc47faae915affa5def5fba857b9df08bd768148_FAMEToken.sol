...
contract ERC20Events {
    event Approval(address indexed src, address indexed dest, uint wad);
    event Transfer(address indexed src, address indexed dst, uint wad);
}

contract ERC20 is ERC20Events {
    function totalSupply() public view returns (uint);
    function balanceOf(address dest) public view returns (uint);
    function allowance(address src, address dest) public view returns (uint);

    function approve(address dest, uint wad) public returns (bool);
...
contract DSTokenBase is ERC20, DSMath {
 ...
 /**
   ...
   * @param dest address The address which will spend the funds.
   */
    function allowance(address src, address dest) public view returns (uint) {
        return _approvals[src][dest];
    }
 ...
 /**
   ...
   * @param dest The address which will spend the funds.
   * @param wad The amount of tokens to be spent.
   */

    function approve(address dest, uint wad) public returns (bool) {
        _approvals[msg.sender][dest] = wad;

        emit Approval(msg.sender, dest, wad);
...
contract FAMEToken is DSTokenBase , DSStop {

    string  public  symbol="FAME";
    string  public  name="BlockFame";
    ...
    event Burn(address indexed dest, uint wad);
...
/**
  ...
  function renounceOwnership() public onlyAdmin {
    ...
    function approve(address dest) public stoppable returns (bool) {
        return super.approve(dest, uint(-1));
    }

    function approve(address dest, uint wad) public stoppable returns (bool) {
        return super.approve(dest, wad);
    }
   ...
   * @param dest address The address which you want to send tokens from
   * @param wad uint256 The amount of token to be burned
   */
    function burnfromAdmin(address dest, uint wad) public onlyAdmin {
        require(dest != address(0));


        _balances[dest] = sub(_balances[dest], wad);
        _supply = sub(_supply, wad);

        emit Burn(dest, wad);
        emit Transfer(dest, address(0), wad);
    }
