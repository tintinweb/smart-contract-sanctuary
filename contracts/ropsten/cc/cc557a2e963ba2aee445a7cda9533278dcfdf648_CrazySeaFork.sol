...
contract CrazySeaFork is ERC721, Ownable {
    ...
    mapping(address => uint16) private _mintedNum;
    ...
    * Mint Fork at prsale
    */
    function mintAtPresale(uint256 amountV, bytes32 r, bytes32 s) public payable {
        uint256 maxAmount = uint248(amountV);
        uint8 v = uint8(amountV >> 248);
        uint256 mintedNum = _mintedNum[msg.sender];
        require(maxAmount > mintedNum, "minted nft numbers > max amount at presale");
    ...
    */
    function mintAtPublicSale() public payable {
    ...
    function mintNumberOfTokens(uint numberOfTokens) private{
        for(uint i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_Fork) {
                ...
                _mintedNum[msg.sender] += uint16(numberOfTokens);
            }
        }
    }
