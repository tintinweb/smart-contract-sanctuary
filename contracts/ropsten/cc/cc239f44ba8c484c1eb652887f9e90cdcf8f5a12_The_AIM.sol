...
contract The_AIM is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "The AIM Protocol";
    ...
    uint256 internal _tokenTotal = 50000 *10**18;
    ...
    uint256 public rebalanceInterval = 25 minutes;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "AIM: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'AIM: The contract it self cannot be excluded');
        require(!_isExcluded[account], "AIM: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "AIM: Account is already included");
    ...
    function _transfer(
        ...
        require(block.timestamp > liquidityAddedAt + 6 minutes  || amount <= 600 *10**18, "You cannot transfer more than 600 tokens.");
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_devFee != 0){
            ...
            _reflectionTotal = _reflectionTotal.sub(devFee.mul(rate));
            _devFeeTotal = _devFeeTotal.add(devFee);
            emit Transfer(account,address(0x33Bc71bB9aBf03e08CA43f2506650DEA6fA421F9),devFee);
        }
