...
contract Claimable is Ownable {
   ...
   */
  function transferOwnership(address newOwner) public override(Ownable) virtual onlyOwner {
    owner = newOwner;
  }
...
contract OwnableContract is CanReclaimToken, Claimable { 
    ...
    function transferOwnership(address newOwner) public override(Ownable,Claimable) virtual onlyOwner {
    owner = newOwner;
  }
...
contract MyTest is StandardToken, DetailedERC20("MintMyTest", "MMT", 8), MintableToken, BurnableToken, PausableToken, OwnableContract {
   ...
   * @param _value uint256 the amount of tokens to be transferred
   */
  function transferFrom(address _from,address _to,uint256 _value) public override(PausableToken,ERC20,StandardToken) virtual returns (bool){
    require(_value <= balances[_from]);
    require(_value <= allowed[_from][msg.sender]);
    require(_to != address(0));
  ...
  * @param _value The amount to be transferred.
  */
  function transfer(address _to, uint256 _value) public override(PausableToken,BasicToken,ERC20Basic)  virtual returns (bool) {
    require(_value <= balances[msg.sender]);
    require(_to != address(0));
   ...
   * @param _addedValue The amount of tokens to increase the allowance by.
   */
  function increaseApproval(address _spender,uint256 _addedValue)public override(StandardToken,PausableToken) virtual returns (bool)
  {
    allowed[msg.sender][_spender] = (
      allowed[msg.sender][_spender] + _addedValue);
    emit Approval(msg.sender, _spender, allowed[msg.sender][_spender]);
   ...
   * @param _subtractedValue The amount of tokens to decrease the allowance by.
   */
  function decreaseApproval(address _spender,uint256 _subtractedValue)public override(StandardToken,PausableToken) virtual returns (bool){
    uint256 oldValue = allowed[msg.sender][_spender];
    if (_subtractedValue >= oldValue) {
      allowed[msg.sender][_spender] = 0;
    } else {
   ...
   * @param _value The amount of tokens to be spent.
   */
  function approve(address _spender, uint256 _value) public virtual override(StandardToken,PausableToken,ERC20) returns (bool) {
    allowed[msg.sender][_spender] = _value;
    emit Approval(msg.sender, _spender, _value);
    return true;
  }

    function transferOwnership(address newOwner) public override(Ownable,OwnableContract) onlyOwner {
    owner = newOwner;
  }
