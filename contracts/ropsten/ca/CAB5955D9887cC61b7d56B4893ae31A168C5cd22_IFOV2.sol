...
contract IFOV2 is Whitelist{
     ...
     * @param _pid: pool id
     */
    function depositPool(uint256 _amount, uint8 _pid) external   notContract {
        ...
        amountPool[msg.sender][_pid] = amountPool[msg.sender][_pid] + (_amount);
     ...
     * @param _pid: pool id
     */
    function harvestPool(uint8 _pid) external   notContract {
    ...
    function setPool(
        ...
        require(viewtotalOfferingAmountPools() <= offeringToken.balanceOf(address(this)), 'can only offer what you have as balance');
        ...
        )
    {
     ...
     // calculate userOfferingAmount, if priceA is reached then allocation*OfferingAmount, otherwise amountPool[user]*priceA
      if (_poolInformation[1].totalAmountPool * (_poolInformation[1].priceA) > _poolInformation[1].offeringAmountPool){
        userOfferingAmount = amountPool[_user][1] * (allocation) / (1e12);
      }else{
    ...
    function _getUserAllocationPool(address _user, uint8 _pid) internal view returns (uint256) {
        if (_poolInformation[_pid].totalAmountPool > 0) {
            return amountPool[_user][_pid] * (1e18) / (_poolInformation[_pid].totalAmountPool * (1e6));
        } else {
            return 0;
        }
    }
    ...
    function viewtotalOfferingAmountPools() public view returns(uint256){
        uint sum = 0;
        for (uint j = 0; j < numberPools; j++){
