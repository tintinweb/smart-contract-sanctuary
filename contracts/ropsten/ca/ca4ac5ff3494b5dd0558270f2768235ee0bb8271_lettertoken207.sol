...
contract lettertoken207 is ERC721Token {
  ...
  function create(uint8 data1, uint8 data2,uint64 data3, uint64 data4, uint64 data5) public payable returns (uint256 _tokenId) {

    if (data5<=msg.value)
    {
    string memory tokenUri = createTokenUri(data1,data2,data3,data4,data5);
  ...
  function get(uint256 _id) public view returns (address owner,uint8 data1,uint8 data2,uint64 data3,uint64 data4,uint64 data5,uint64 startBlock) {
    return (
      tokenOwner[_id],
      tokens[_id].data1,
      tokens[_id].data2,
