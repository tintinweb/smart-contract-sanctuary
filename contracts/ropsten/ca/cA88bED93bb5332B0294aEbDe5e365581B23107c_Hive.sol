...
contract Hive is Ownable, IERC721Receiver, Pausable {
    ...
    CryptoBees bees = CryptoBees(0x6FfD623f8C73f2B6bAC5a998aa8b1b8073C05bFA);
    ...
    function manyBearsAttack(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(bees.getTokenType(i) == 1, "TOKEN NOT A BEAR");
            ...
            if (num < 50) {
                owed += 10000 ether;
                if (!transfer) bees.increateTokensPot(tokenIds[i], 10000);
                bees.updateTokensLastAttack(tokenIds[i], uint48(block.timestamp));
                hives[i].lastStolenHoneyTimestamp = uint48(block.timestamp);
                hives[i].successfulAttacks++;
                BearsAttacked(_msgSender(), tokenIds[i], 10000 ether, false);
            } else {
                BearsAttacked(_msgSender(), tokenIds[i], 0, false);
            }
            hives[i].totalAttacks++;
        }
        if (transfer && owed > 0) honey.mint(_msgSender(), owed);
    }
    ...
    {
        return (hives[hiveId].startedTimestamp, hives[hiveId].subtract, hives[hiveId].occupancy);
    }
...
contract CryptoBees is ERC721, Ownable, Pausable, ReentrancyGuard {
    ...
    Honey private honeyContract;
    Hive private hiveContract;
    ...
    constructor(address _HONEY_CONTRACT, address _HIVE_CONTRACT) ERC721("CryptoBees Game", "CRYPTOBEES") {
        ...
        hiveContract = Hive(_HIVE_CONTRACT);
    }
    ...
    function increateTokensPot(uint256 tokenId, uint32 amount) external nonReentrant {
        require(_exists(tokenId), "ERC721Metadata: Nonexistent token");
        require(_msgSender() == address(hiveContract), "DONT CHEAT!");
        tokenData[tokenId].pot += amount;
    }

    function updateTokensLastAttack(uint256 tokenId, uint48 timestamp) external nonReentrant {
        require(_exists(tokenId), "ERC721Metadata: Nonexistent token");
        require(_msgSender() == address(hiveContract), "DONT CHEAT!");
    ...
    //
    //
    function mintForHoney(uint256 amount, bool stake) external nonReentrant whenNotPaused {
        require(tx.origin == _msgSender(), "Only EOA");
        require(minted + amount <= MAX_TOKENS, "All tokens minted");
        require(amount > 0 && amount <= 10, "Invalid mint amount");
        if (minted < PAID_TOKENS) {
        ...
        honeyContract.transferFrom(msg.sender, address(this), totalHoneyCost);
    }
    ...
    //     require(_exists(tokenId), "ERC721Metadata: Nonexistent token");
    //     return ((tokenData[tokenId] & 0xFFFF) % 100);
    // }
