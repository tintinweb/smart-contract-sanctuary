...
contract DMarketNFTToken is MinterAccess, ERC721 {
    ...
    function mintBatch(address[] memory receivers, uint64[] memory tokenIDs) public virtual onlyMinter {
        require(receivers.length > 0,"must be some receivers");

        require(receivers.length == tokenIDs.length, "must be the same number of receivers/tokenIDs");

        for (uint i = receivers.length-1; i >=0; i--) {
