...
contract CryptsyToken is CryptsyBase, ERC721 {
    ...
    function create() public {
        uint256 tokenId = cryptsys.length + 1;
        _mint(msg.sender, tokenId);
    }

    function generateCryptsyToken() public returns (uint newCryptsyId) {
        Cryptsy memory _cryptsy = Cryptsy({
            conceptionTime: uint32(now),
            creativity: 3324165,
            intuition: 2536621,
        ...
        newCryptsyId = cryptsys.push(_cryptsy) - 1;
        require(newCryptsyId != uint(0));
    }
}