 ...
 uint256 _tokenId;
  constructor() {
   ...
   function mint (address _to,uint256 no_of_tokens_to_create, string[] calldata _uri) external {
    ...
    super._setTokenUri(_tokenId, _uri[j]);
  ...
  function transferamount(address creator,address admin, address nft_creator, uint256 amount,uint256 adminpersent, uint256 nft_creator_persent) public payable
  {   address payable _creator = payable(creator);
      address payable _admin = payable(admin);
      address payable _nft_creator = payable(nft_creator);
      uint256 creatoramount=amount*(100-adminpersent-nft_creator_persent)/100;
      uint256 adminamount=amount*adminpersent/100;
      uint256 nft_creator_amount = amount*nft_creator_persent/100;
  
      _creator.transfer(creatoramount);
      _admin.transfer(adminamount);
      _nft_creator.transfer(nft_creator_amount);
  } 
