...
import "./LiquidVestingToken.sol";

contract LiquidVestingTokenFactory is Ownable {
  address private tokenImplementation;
  address public merkleRootSigner;
  address public feeCollector;
  uint256 public fee;

  event VestingTokenCreated(address indexed redeemToken, address vestingToken);

  mapping(address => address[]) public vestinTokensByOriginalToken;
  ...
  function setFee(uint256 _fee) external onlyOwner {
    require(_fee >= 0 && _fee <= 5000, "Fee goes beyond rank");
  ...
  function createLiquidVestingToken(
    ...
    LiquidVestingToken.AddType addRecipientsType
  ) public {
    require(redeemToken != address(0), "Company token cannot be zero address");
    require(
    ...
    for (uint256 i = 0; i < name.length; i++) {
      ...
      LiquidVestingToken(address(token)).initialize(
        name[i],
        symbol[i],
        _msgSender(),
        address(this),
      ...
      vestinTokensByOriginalToken[redeemToken].push(address(token));
