 ...
 */
contract MasterMint is ReentrancyGuard, Ownable, INOwnerResolver {
    ...
    event MintAdded(uint256 mintId, MintState mintState);
    event MintReplaced(uint256 mintId, MintState mintState);

    constructor(
        IVestedNil vNil_,
        INOwnerResolver nOwnerResolver_,
        address collectionAccount_,
        IArtistMiningCalculator calculator_,
        ProtocolConfiguration memory configuration_
    ) {
        require(address(vNil_) != address(0), "MasterMint:NIL_ADDRESS");
        require(collectionAccount_ != address(0), "MasterMint:ILLEGAL_COLLECTION_ACCOUNT");
        vNil = vNil_;
        nOwnerResolver = nOwnerResolver_;
        collectionAccount = collectionAccount_;
     ...
     */
    function addMint(INilPass nilPass, address creator) external onlyOwner {
        MintState memory mintState = _createMint(nilPass, creator);
        mints.push(mintState);
        emit MintAdded(mints.length - 1, mintState);
    }
    ...
    function replaceMint(
        ...
        require(mintId < mints.length, "MasterMint:ILLEGAL_MINT_ID");
        MintState memory mintState = mints[mintId];
        require(mintState.status == MintStatus.PENDING, "MasterMint:ILLEGAL_STATUS");
        mintState = _createMint(nilPass, creator);
        mints[mintId] = (mintState);
        emit MintReplaced(mintId, mintState);
    }

    function _createMint(INilPass nilPass, address creator) internal view returns (MintState memory) {
    ...
    function calculateThreshold(INilPass nilPass) public view returns (uint256 artistMiningThreshold) {
        ...
        artistMiningThreshold = Math.max(artistMiningThreshold, configuration.artistMiningLowWatermark);
    }

    function setMintCreator(uint256 mintId, address newCreator) external {
        MintState storage mintState = mints[mintId];
        require(msg.sender == mintState.creator, "MasterMint:ACCESS_DENIED");
        require(newCreator != address(0), "MasterMint:COLLECTION_ACCOUNT");
        mintState.creator = newCreator;
    }

    function setMintStatus(uint256 mintId, MintStatus status) external onlyOwner {
        require(mintId < mints.length, "MasterMint:ILLEGAL_MINT_ID");
        MintState storage mintState = mints[mintId];
        require(status > MintStatus.PENDING, "MasterMint:ILLEGAL_STATUS");
        mintState.status = status;
    }

    function mintWithN(uint256 mintId, uint256[] calldata tokenIds) external payable virtual nonReentrant {
        MintState storage mintState = mints[mintId];
        require(mintState.status == MintStatus.OPEN, "MasterMint:MINT_NOT_ACTIVE");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(nOwnerResolver.ownerOf(tokenIds[i]) == msg.sender, "MasterMint:NOT_N_OWNER");
        }
        mintState.totalPaidInWei += uint128(msg.value);
        mintState.nMinted += uint16(tokenIds.length);
        mintState.nilPass.mintWithN(msg.sender, tokenIds, msg.value);
    }
    ...
    function mint(uint256 mintId, uint8 amount) external payable virtual nonReentrant {
        ...
        require(mintState.status == MintStatus.OPEN, "MasterMint:MINT_NOT_ACTIVE");
        mintState.totalPaidInWei += uint128(msg.value);
        mintState.nilPass.mint(msg.sender, amount, msg.value);
    }
    ...
    function mintTokenIds(uint256 mintId, uint256[] calldata tokenIds) external payable virtual nonReentrant {
        ...
        require(mintState.status == MintStatus.OPEN, "MasterMint:MINT_NOT_ACTIVE");
        mintState.totalPaidInWei += uint128(msg.value);
        mintState.nilPass.mintTokenId(msg.sender, tokenIds, msg.value);
    }

    function mintData(uint256 mintId) external view returns (MintState memory, INilPass.MintParams memory) {
        MintState memory mintState = mints[mintId];
        return (mintState, mintState.nilPass.mintParameters());
    }

    function creatorWithdraw(uint256 mintId, bool withNil) external nonReentrant {
        MintState memory mintState = mints[mintId];
        require(msg.sender == mintState.creator || msg.sender == owner(), "MasterMint:ACCESS_DENIED");
        require(mintState.status == MintStatus.OPEN, "MasterMint:ILLEGAL_STATE");
        if (withNil) {
            _mintNil(mintId, mintState);
        }
        uint128 newSalesProceeds = mintState.totalPaidInWei - mintState.paymentWithdrawn;
        if (newSalesProceeds > 0) {
            uint256 fees = (mintState.protocolFeesInBPS * newSalesProceeds) / 10000;
            feesCollectable += fees;
            mints[mintId].paymentWithdrawn += newSalesProceeds;
            payable(mintState.creator).transfer(newSalesProceeds - fees);
        }
    }

    function protocolWithdraw(bool withNil) external nonReentrant onlyOwner {
        if (nilCollectable > 0 && withNil) {
            vNil.mint(collectionAccount, nilCollectable);
            nilCollectable = 0;
        }
        if (feesCollectable > 0) {
            uint256 toCollect = feesCollectable;
            feesCollectable = 0;
            payable(collectionAccount).transfer(toCollect);
        }
    }

    function _mintNil(uint256 mintId, MintState memory mintState) internal {
        if (mintState.nMinted > mintState.artistMiningThreshold) {
            uint128 artistMining = Math
                .min(
                    mintState.calculator.calculateArtistMining(mintId, mintState.totalPaidInWei),
            ...
            uint128 nilToMint = artistMining > mintState.artistMiningWithdrawn
                ? artistMining - mintState.artistMiningWithdrawn
                : 0;

            if (nilToMint > 0) {
                totalNilMinted += nilToMint;
                mints[mintId].artistMiningWithdrawn += nilToMint;
                uint128 fees = (mintState.protocolFeesInBPS * nilToMint) / 10000;
                nilCollectable += fees;
                vNil.mint(mintState.creator, nilToMint - fees);
            }
        }
    }

    function setCollectionAccount(address newCollectionAccount) external onlyOwner {
        require(newCollectionAccount != address(0), "MasterMint:COLLECTION_ACCOUNT");
        collectionAccount = newCollectionAccount;
    }

    function setCalculator(IArtistMiningCalculator calculator_) public onlyOwner {
        require(address(calculator_) != address(0), "MasterMint:INVALID_ADDRESS");
        calculator = calculator_;
    }

    function setConfiguration(ProtocolConfiguration memory configuration_) public onlyOwner {
        require(configuration_.protocolFeesInBPS < 2000, "MasterMint:INVALID_FEES");
        require(configuration_.artistMiningHighWatermark < 8888, "MasterMint:INVALID_HIGH_WM");
        require(configuration_.artistMiningLowWatermark > 0, "MasterMint:INVALID_LOW_WM");
        require(
            configuration_.artistMiningLowWatermark < configuration_.artistMiningHighWatermark,
            "MasterMint:INVALID_LOW_WM"
        );
        require(configuration_.artistMiningThresholdInBPS < 10000, "MasterMint:INVALID_THRESHOLD");
        require(configuration_.artistMiningThresholdInBPS > 0, "MasterMint:INVALID_THRESHOLD");
    ...
    function tokenExists(uint256 mintId, uint256 tokenId) external view returns (bool) {
        MintState memory mintState = mints[mintId];
        if (address(mintState.nilPass) == address(0)) return false;
        return mintState.nilPass.tokenExists(tokenId);
    }
    ...
    function tokensExist(uint256 mintId, uint256[] calldata tokenIds) external view returns (bool[] memory) {
        ...
        MintState memory mintState = mints[mintId];
        if (address(mintState.nilPass) == address(0)) return result;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            result[i] = mintState.nilPass.tokenExists(tokenIds[i]);
        }
        return result;
    }
}
