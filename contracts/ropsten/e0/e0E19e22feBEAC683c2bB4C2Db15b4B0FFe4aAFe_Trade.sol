...
contract Trade is Ownable, Pausable {
    ...
    function mintNFT(string memory _tokenURI, uint256 _initERC20Price) public payable returns (uint256) {
  ...
  function getBalanceOfERC20(address _userAddress) public view returns(uint256) {
    //IERC20 token = IERC20(CURRENCY_RANKING_ERC20_TOKEN_ADDRESS);
    uint256 balance = erc20Token.balanceOf(_userAddress);
    return balance;
  }    
  ...
  function setForSaleWithGoalPrice(uint256 _tokenId, bool _isForSale, uint256 _goalPrice) public payable whenNotPaused returns(bool) {
 ...
 */
//contract RankingERC20Token is RankContext, IERC20 {
contract RankingERC20Token is IERC20, Context {    
    using SafeMath for uint256;
    ...
    uint constant INITIAL_SUPPLY = 100000000000 * (10**18);    
    ...
    function setSender(address payable sender) external {
        console.log("RankingERC20Token, setSender, sender=%s", sender);
        _setMsgSender(sender);
    }        
   */

    function msgSender() external view returns (address payable) {
        console.log("RankingERC20Token, msgSender, _msgSender()=%s", _msgSender());
        return _msgSender();
    }    
    ...
    function myTransfer(address sender, address receiver, uint256 amount) public returns (bool) {
        console.log("RankingERC20Token, myTransfer, sender=%s, receiver=%s, amount=%s", sender, receiver, amount);
    ...
    function myAllowance(address sender, address receiver) public view returns (uint256) {
        console.log("RankingERC20Token, myAllowance, owsenderner=%s, receiver=%s, _allowances[sender][receiver]=%s", sender, receiver, _allowances[sender][receiver]);
    ...
    function myApprove(address sender, address receiver, uint256 amount) public returns (bool) {
        console.log("RankingERC20Token, myApprove, sender=%s, receiver=%s, amount=%s", sender, receiver, amount);
    ...
    function myIncreaseAllowance(address sender, address receiver, uint256 addedValue) public returns (bool) {
        console.log("RankingERC20Token, myIncreaseAllowance, sender=%s, receiver=%s, addedValue=%s", sender, receiver, addedValue);
    ...
    function myDecreaseAllowance(address sender, address receiver, uint256 subtractedValue) public returns (bool) {
        console.log("RankingERC20Token, decreaseAllowance, sender=%s, receiver=%s, subtractedValue=%s", sender, receiver, subtractedValue);
    ...
    function _transfer(address sender, address receiver, uint256 amount) internal virtual {
        console.log("RankingERC20Token, _transfer, sender=%s, receiver=%s, amount=%s", sender, receiver, amount);
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        console.log("RankingERC20Token, _mint, account=%s, amount=%s", account, amount);
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        console.log("RankingERC20Token, _burn, account=%s, amount=%s", account, amount);
    ...
    function _approve(address sender, address receiver, uint256 amount) internal virtual {
        ...
        console.log("RankingERC20Token, _approve, sender=%s, receiver=%s, amount=%s", sender, receiver, amount);
...
contract RankingNFT is ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable, Pausable {
    ...
    */
     ...
     * @dev See {IERC721-setApprovalForAll}.
     */
    function mySetApprovalForAll(address sender, address operator, bool approved) public {
    ...
    function isApprovedForAll(address owner, address operator) public view override returns (bool) {
        ...
        console.log("isApprovedForAll, owner=%s, operator=%s, result=%s", owner, operator, result);
        return result;
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        ...
        //WARNING: This below makes an error to transfer
