...
contract Pixel is ERC721PresetMinterPauserAutoId {
    ...
    constructor(I1MIL _mil1) ERC721PresetMinterPauserAutoId('Pixel', 'PXL', '')  {
        mil1 = _mil1;
        isPreSale = true;
...
//        require(owner != address(0), "ERC721: owner query for nonexistent token");
    ...
    function getPixelInfo(uint x, uint y) public view returns (uint, string memory){
        return (pixelColors[x][y], ownerUrls[ownerOf(getPixelId(x, y))]);
    }
    ...
    function buy(uint xTopLeft, uint yTopLeft, uint xBottomRight, uint yBottomRight) public {
        ...
        require(!isPreSale || partners.isVip(msg.sender), "Sorry, only partners can buy pixels today");
        ...
        if (result) {
            for (uint ix = xTopLeft; ix <= xBottomRight; ix++) {
                for (uint iy = yTopLeft; iy <= yBottomRight; iy++) {
                    mint(tx.origin, getPixelId(ix, iy));
                }
            }
        }
    }
