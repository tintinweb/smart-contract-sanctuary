...
contract LibertyNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    //first uint -> index; second uint -> x value; third uint -> y value
    ...
    uint256 public remintFeeAmount;
    address public remintFeeTokenAddr;
    // first uint -> index; second address -> owner; third uint -> x value; fourth uint -> y value;
    mapping(uint256 => mapping(address => mapping(uint256 => mapping(uint256 => bool)))) public remintMap;
    ...
    event MintToken(uint256 indexed tokenId, uint256 index, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength, uint256 createTime, uint256 updateTime, bool blur, uint256 govCounter, bool unsafe);
    event TokenURIUpdated(uint256 indexed tokenId, string oldTokenURI, string newTokenURI, bool blur, uint256 updateTime);
    event GovToken(uint256 indexed tokenId, uint256 govCounter, bool blur, bool isSafe, uint256 updateTime);
    event ResetToken(uint256 indexed tokenId, uint256 govCounter, bool blur, bool isSafe, uint256 updateTime);
    ...
    constructor(
        ...
        address _remintFeeTokenAddr,
        address _feeAddr,
        uint256 _mintFeeAmount,
        uint256 _modifyFeeAmount,
        uint256 _reMintFeeAmount
        ...
        remintFeeTokenAddr = _remintFeeTokenAddr;
        feeAddr = _feeAddr;
        mintFeeAmount = _mintFeeAmount;
        modifyFeeAmount = _modifyFeeAmount;
        remintFeeAmount = _reMintFeeAmount;
    ...
    function mint(address to, string memory _tokenURI, uint256 index, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength) public returns (uint256 tokenId) {
        ...
        uint256 _mintFeeAmount = mintFeeAmount.mul(xLength).mul(yLength);
        TransferHelper.safeTransferFrom(mintFeeTokenAddr, msg.sender, feeAddr, _mintFeeAmount);
        tokenId = nextTokenId;
        _mint(to, tokenId);
        nextTokenId++;
        ...
        emit MintToken(tokenId, index, startX, startY, xLength, yLength, block.timestamp, block.timestamp, false, 0, false);
    }
    ...
    function validatePixel(uint256 index, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength) public view returns(bool) {
        ...
        require(startX<1250 && startX.add(xLength)<=1250,"horizontal exceed boundary");
        require(startY<800 && startY.add(yLength)<=800,"vertical exceed boundary");
    ...
    function markAsWaitingReminted(uint256 index, address addr, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength) internal {
        mapping(uint256 => mapping(uint256 => bool)) storage remintMapByAddr = remintMap[index][addr];
        for (uint256 x = startX; x < startX.add(xLength); x = x + minimumStep) {
            for (uint256 y = startY; y < startY.add(yLength); y = y + minimumStep) {
                remintMapByAddr[x][y] = true;
            }
        }
    }

    function markAsReminted(uint256 index, address addr, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength) internal {
        mapping(uint256 => mapping(uint256 => bool)) storage remintMapByAddr =  remintMap[index][addr];
        for (uint256 x = startX; x < startX.add(xLength); x = x + minimumStep) {
            for (uint256 y = startY; y < startY.add(yLength); y = y + minimumStep) {
                remintMapByAddr[x][y] = false;
            }
        }
    }

    function validatePixelForReminted(uint256 index, address addr, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength) public view returns(bool) {
        require(enableIndexMap[index], "index is not enabled yet");
        require(xLength!=0 && yLength!=0,"length should not be 0");
        ...
        for (uint256 x = startX; x < startX.add(xLength); x = x + minimumStep) {
            for (uint256 y = startY; y < startY.add(yLength); y = y + minimumStep) {
                require(remintMapByAddr[x][y], "some pixels are not yours");
            }
        }
        return true;
    }

    function remint(address to, string memory _tokenURI, uint256 index, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength) public returns (uint256 tokenId) {
        require(validatePixelForReminted(index, _msgSender(), startX, startY, xLength, yLength), "invalid pixel");
        uint256 _reMintFeeAmount = remintFeeAmount.mul(xLength).mul(yLength).div(10);
        TransferHelper.safeTransferFrom(remintFeeTokenAddr, _msgSender(), feeAddr, _reMintFeeAmount);
        tokenId = nextTokenId;
        _mint(to, tokenId);
        nextTokenId++;
        _setTokenURI(tokenId, _tokenURI);
        ...
        nftLibertyMap[tokenId] = liberty;
        markAsReminted(index, _msgSender(), startX, startY, xLength, yLength);
        //emit MintToken(tokenId, index, startX, startY, xLength, yLength, block.timestamp, block.timestamp, false, 0, false);
    }

    function refactor(uint256 tokenId) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId), 'caller is not owner nor approved');
        _burn(tokenId);
        Liberty memory liberty = nftLibertyMap[tokenId];
        markAsWaitingReminted(liberty.index, _msgSender(), liberty.startX, liberty.startY, liberty.xLength, liberty.yLength);
    ...
    function setBaseURI(string memory baseURI) public {
        ...
        string memory oldBaseURI = super.baseURI();
        _setBaseURI(baseURI);
        emit BaseTokenURIUpdated(oldBaseURI, baseURI);
    }

    function setTokenURI(uint256 tokenId, string memory tokenURI, bool blur) public whenNotPaused {
        require(_isApprovedOrOwner(_msgSender(), tokenId), 'caller is not owner nor approved');
        string memory oldTokenURI = super.tokenURI(tokenId);
        if (bytes(oldTokenURI).length != 0) {
        ...
        emit TokenURIUpdated(tokenId, oldTokenURI, tokenURI, liberty.blur, liberty.updateTime);
    }
    ...
    function blurNFT(uint256 tokenId) public whenNotPaused {
        ...
        liberty.updateTime = block.timestamp;
        emit GovToken(tokenId, liberty.govCounter, liberty.blur, liberty.unsafe, liberty.updateTime);
    }
    ...
    function resetNFT(uint256 tokenId) public {
        ...
        liberty.blur = false;
        liberty.updateTime = block.timestamp;
        emit ResetToken(tokenId, liberty.govCounter, liberty.blur, liberty.unsafe, liberty.updateTime);
    }
