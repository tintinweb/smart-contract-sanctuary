...
/**
 * @title WildLionSociety contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract WildLionSociety is ERC721, Ownable {
    using SafeMath for uint256;

    //string public PROV = "b80d60a4defcca5af3ed6526d8c0f86089b9400659c89da2b2725b32f8686d4a";//What to do with this
    uint256 public constant LionPrice = 60000000000000000; // 0.06 ETH
    uint public constant maxLionPurchase = 10;
    uint256 public MAX_LIONS = 7077;
    ...
    function reserveLions(uint amount) public onlyOwner {        
        require(!saleIsActive, "Can not reserve lions while the sale is active");
        require(teamTokenCount.add(amount) <= 77, "Can not mint more than 77 tokens for the team");
    ...
    function mintLions(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Lions");
        require(numberOfTokens <= maxLionPurchase, "Can only mint 10 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_LIONS, "Purchase would exceed max supply of Lions");
        require(LionPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
