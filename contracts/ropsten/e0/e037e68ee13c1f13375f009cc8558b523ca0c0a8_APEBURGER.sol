...
/*
   Welcome to APEBURGER
   
   This is a community token. If you want to create a telegram I suggest to name it to @APEBURGERBSC
   ...
   I will renounce the ownership to burn addresses to transfer #APEBURGER to the community, make sure it's 100% safe.

   I will add .2 BNB and all the left 10% total supply to the pool
   Can you make #APEBURGER 10000X? 
   ...
   it's the best part of the #APEBURGER idea, increasing the liquidity pool automatically, 
...
contract APEBURGER is Context, IERC20, Ownable {
    ...
    string private _name = "APEBURGER";
    ...
    uint256 public _maxWalletToken = 5000000000000 * 10**18;
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            ...
            uint256 tokenRecepient = balanceOf(to);
            require(
                tokenRecepient + amount <= _maxWalletToken,
    ...
    function _getAntiDumpMultiplier() private view returns (uint256) {
        ...
        uint256 hour = 60 * 60;
        if (time_since_start < 5 * hour) {
            return (8);
        } else if (time_since_start < 10 * hour) {
            return (7);
        } else if (time_since_start < 15 * hour) {
            return (6);
