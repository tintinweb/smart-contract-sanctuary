...
contract baseRule{
    ...
    function _triggerCoolCountDown(uint catID) internal{
        cats[catID].coolcountdown = block.timestamp + 1 seconds;
    }
...
contract fight is catStore {
    ...
    function luCat() constant existCat(ownerToCat[msg.sender]) external returns(uint32,uint32,uint32,uint32,uint,uint,uint) {
        return (
