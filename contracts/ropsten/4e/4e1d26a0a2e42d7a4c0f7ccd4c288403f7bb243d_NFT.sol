...
contract NFT is ERC721 
{
    ...
    constructor(string memory name, string memory symbol, uint _limit_supply) ERC721(name, symbol) public
	{
	...
	{
		require(total_supply <= limit_supply, "limit_supply of mints already reached");
        require(owner == msg.sender); // 소유자만이 token을 생성할 수 있습니다
        uint256 tokenId = tokens.length; // 유일한 token ID
        tokens.push(Token(title, description, image));
