...
mapping (uint256 => tokenInfo) public tokenBook;
  ...
  address public tokenContractAddress = address(0x31fc184Dc0462e0a9fB3127190f50CAd65c019cf);

  bool public crowdSaleStarted = false;
  uint256 public totalRaisedInETH = 0;
   ...
   */
  function setMinTokensInPreSale(uint256 value) public onlyOwner returns(bool){
    require(value > 0,"Invalid Value");
    minimumTokensInPreSale = value;
    return true;
  }
   ...
   */
  function setMaxTokensInPreSale(uint256 value) public onlyOwner returns(bool){
    require(value > 0,"Invalid Value");
    MaximumTokensInPreSale = value;
    return true;
  }
   ...
   * @param value to set the new min value
   */
  function setMinTokensInPublicSale(uint256 value) public onlyOwner returns(bool){
    require(value > 0,"Invalid Value");
    minimumTokensInPublicSale = value;
    return true;
  }
   ...
   * @param value to set the new max value
   */
  function setMaxTokensInPublicSale(uint256 value) public onlyOwner returns(bool){
    require(value > 0,"Invalid Value");
    MaximumTokensInPublicSale = value;
    return true;
  }
    ...
    */
    ...
    function pause() public onlyOwner {
      ...
      require(crowdSaleStarted == true, "crowdsale not started");
    ...
    function restartSale() public onlyOwner {
      require(currentStage == Stages.Pause, "current stage is not paused");
    ...
    require(currentStage == Stages.PreSaleEnd, "pre-sale not ended");
    ...
    if (currentStage == Stages.PreSaleStart) return 'Pre Sale is running';
    else if (currentStage == Stages.PreSaleEnd) return 'Pre Sale is over';
    else if (currentStage == Stages.PublicSaleStart) return 'Public Sale is running';    
    else if (currentStage == Stages.PublicSaleEnd) return 'Public Sale is over';   
    else if (currentStage == Stages.Pause) return 'Sale is paused';
    else if (currentStage == Stages.CrowdSaleNotStarted) return 'CrowdSale has not started yet';
    return 'Not Found';    
    }
   ...
   */
   function buyTokens(address beneficiary) CrowdsaleStarted public payable {
    require(Paused != true, "crowdsale is pause");
    uint256 ETHAmount = msg.value;
    require(ETHAmount != 0, "amount cannot be zero");    
   ...
   */
   function _preValidatePurchase(uint256 _ETH) internal view { 
     ...
     if (currentStage == Stages.PreSaleStart) {
        require(_ETH >= minimumTokensInPreSale, "tokens cannot be less than 500");
        require(_ETH <= MaximumTokensInPreSale, "cannot cross max limit");

      }
      else if (currentStage == Stages.PublicSaleStart) {
        require(_ETH >= minimumTokensInPublicSale, "tokens cannot be less than 500");
        require(_ETH <= MaximumTokensInPublicSale, "cannot cross max limit");
    ...
    */
    ...
    function _validateCapLimits(uint256 _ETH) internal {
      ...
      require(totalRaisedInETH <= hardCap, "hard cap reached");
   }
