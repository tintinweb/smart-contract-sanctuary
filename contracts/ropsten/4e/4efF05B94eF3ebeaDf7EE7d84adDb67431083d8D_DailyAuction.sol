...
contract CalendarSteward {
    ...
    */
    ...
    function listNewToken(
        ...
        uint256 tokenID = assetToken.mintItem(to, month, dayOfMonth, status);
...
contract CommonCalendar is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {
  ...
  // function getForeClosedDay(uint8 _monthNumber, uint8 _dayOfMonth) public view returns(bool) {
  //   return foreCloseDates[_monthNumber][_dayOfMonth];
  // }
  // ----
  ...
  function mintItem(address to, uint8 month, uint8 dayOfMonth, bool status)
      public
      notAlreadyMinted(month, dayOfMonth)
      onlyValidInputs(month, dayOfMonth)
      notAlreadyAuctioned(month, dayOfMonth, status)
      returns (uint256)
  {
  ...
  function addAuctionDay(uint8 month, uint8 day) public {
...
contract DailyAuction is AccessControl {
    ...
    function mintDay(uint8 year, uint8 month, uint8 dayOfMonth, uint256 _newPrice, uint256 _deposit, string memory dayName
        ) public payable {
    ...
    function sendEtherToAddress(address payable _address, uint amount) external onlyRole(ADMIN_ROLE) {
