...
contract ERC721 is Context ,ERC165, IERC721, IERC721Metadata, Ownable, Pausable {
    ...
    mapping (uint256 =>  Rate[]) private rateMap;
    uint256 rateCount;
    ...
    function createToken() public  {
        ...
        function unplace(uint256 tokenId) private {
            ...
            function createRate(uint256 tokenId) public  payable  {
                address own = _msgSender();

                uint256 cost = msg.value;

                require(cost > 0, "Cost should be greater than zero.");
                _createRate(tokenId,cost,own);
            }


            function _createRate(uint256 tokenId, uint256 cost,address own) internal  virtual {
                _setRate(tokenId,cost,own);
            }

            function _setRate(uint256 tokenId, uint256 cost,address rateOwnAddress) private {

                Rate[] storage rates = rateMap[tokenId];

                Rate memory rate;
                rate.tokenId = tokenId;
                rate.cost = cost;
                ...
                function getRatesByTokenIndex(uint256 tokenId) public view returns(Rate[] memory) {
                    return rateMap[tokenId];
                }

                function getRatesByTokenIndexAndRateIndex(uint256 tokenId,uint256 rateId) public view returns(Rate memory) {
                    Rate[] storage rateArray = rateMap[tokenId];
                ...
                function incrementRateCount() private {
                    ...
                    function approveRate(uint256 tokenId,uint256 rateId) public {

                        require(_msgSender() == ERC721.ownerOf(tokenId) , "address not owner");

                        Rate memory winnRate = getRatesByTokenIndexAndRateIndex(tokenId,rateId);

                        address payable owner = payable(address(uint160(ERC721.ownerOf(tokenId))));


                        // Transfer token
                        safeTransferFrom(owner, winnRate.owner, tokenId);

                        // Process Payment
                        owner.transfer(winnRate.cost);

                        // Return Money from rate
                        _returnMoneyFromRate(tokenId,rateId);

                        delete rateMap[tokenId];
                    }

                        function _returnMoneyFromRate(uint256 tokenId,uint256 rateId) private {
                            Rate[] storage rateArray = rateMap[tokenId];
                            ...
                            function _setPlacement(uint256 tokenId, uint256 cost) private {
                                ...
                                function _baseURI() internal view virtual returns (string memory) {
                                    return "https://nftwhales.com/nft-detail?id=";
                                }
