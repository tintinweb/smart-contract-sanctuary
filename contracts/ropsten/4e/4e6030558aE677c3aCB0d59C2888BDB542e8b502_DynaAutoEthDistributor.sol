...
abstract contract  AutomatedExternalReflector is Context, LockableSwap, Ownable, IAutomatedExternalReflector {
    ...
    function logTransactionEvent(address from, uint256 senderAmount, address to, uint256 recipientAmount) external override returns (bool) {
    ...
    function logUserTransaction(address user, uint256 value, bool isSender) private {
        queue.add(user);
        if(Address.isContract(user) && !contractWhitelist[user]){
            excludeFromReflections(user, true);
        }
    ...
    function reflectRewards() external override returns (bool) {
        if(!inSwapAndLiquify && gasleft() >= minGas)
    ...
    function _reflectRewards() private lockTheSwap returns(bool allComplete) {
        ...
        while(gasLeft > minGas && gasLeft > endGas && currentQueueIndex < stopProcessingAt && currentQueueIndex < queue.length()){
            address payable hodler = payable(queue.at(currentQueueIndex));
    ...
    function _sendEthTo(address payable hodler) private {
    ...
    function getRemainingPayeeCount() public view override returns(uint256 count) {
    ...
    function updateTokenAddress(address token, bool andPair) public {
    ...
    function allowUnsafeTransactions(bool allow) public {
