 ...
 // A mapping of all of the auctions currently running.
    ...
    function buy(uint _id) external payable {
        ...
        if (currency[_id] == 1) {
            ...
            require(IERC20(paymentTokenAddress).transferFrom(payable(_newOwner), payable(_previousOwner), _sellerValue));
            require(IERC20(paymentTokenAddress).transferFrom(payable(_newOwner), payable(creatorMap[_id]), _royaltyValue));
            require(IERC20(paymentTokenAddress).transferFrom(payable(_newOwner), payable(_contractOwner), _commissionValue));
        }
        ...
        auctionNotExpired(tokenId)
    {
        ...
        if (auctions[tokenId].amount == 0) {
            ...
            if (currency[tokenId] == 1) {
                ...
                require(IERC20(paymentTokenAddress).transferFrom(payable(address(this)), payable(auctions[tokenId].bidder), auctions[tokenId].amount));
            }
        }
        // Deposit token 
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            ...
            if (currency[tokenId] == 1) {
                ...
                require(IERC20(paymentTokenAddress).transferFrom(payable(address(this)), payable(_contractOwner), _commissionValue));
                require(IERC20(paymentTokenAddress).transferFrom(payable(address(this)), payable(creatorMap[tokenId]), _royaltyValue));
                require(IERC20(paymentTokenAddress).transferFrom(payable(address(this)), payable(Creator), amount.sub(_royaltyValue).sub(_commissionValue)));
            }
