...
//    }
    ...
    function getListTokens(KyberNetwork network) public view returns (OrderbookReserve[] memory oReserveTokens, KyberReserve[] memory conversionRates) {
        ...
        oReserveTokens = new OrderbookReserve[](numOrderReserveTokens);
        conversionRates = new KyberReserve[](numConversionRates);
        ...
        for(rID = 0; rID < network.getNumReserves(); rID++) {
            ...
            if (network.reserveType(reserve) == KyberNetwork.ReserveType.PERMISSIONLESS) {
                ...
                if (oReserve != address(0)) {
                    ...
                    //     numOrderReserveTokens += 1;
                    // }
                    oReserveTokens[numOrderReserveTokens] = oReserve;
                    numOrderReserveTokens += 1;
                }
            } else {
                kReserve = KyberReserve(reserve);
                if (kReserve != address(0)) {
                    conversionRates[numConversionRates] = kReserve;
                    numConversionRates += 1;
                }
            }
        }
