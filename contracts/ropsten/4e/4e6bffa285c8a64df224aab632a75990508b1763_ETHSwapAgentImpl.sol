...
contract ETHSwapAgentImpl is Context, Initializable {
    ...
    event Initialized(uint256 fee, address indexed erc20Addr, string name, string symbol, uint8 decimals);
    ...
    function initialize(uint256 fee, address erc20Addr, address payable ownerAddr) public initializer {
        swapFee = fee;
        owner = ownerAddr;
    ...
    function fillBSC2ETHSwap(bytes32 bscTxHash, address toAddress, uint256 amount) onlyOwner external returns (bool) {
        ...
        IERC20(registeredERC20).safeTransfer(toAddress, amount);

        emit SwapFilled(registeredERC20, bscTxHash, toAddress, amount);
        return true;
    }

    function swapETH2BSC(uint256 amount) payable external notContract returns (bool) {
        require(msg.value == swapFee, "swap fee not equal");

        IERC20(registeredERC20).safeTransferFrom(msg.sender, address(this), amount);
        if (msg.value != 0) {
