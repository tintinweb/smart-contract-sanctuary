...
contract Staking is ReentrancyGuard, StakingInterface {
  ...
  uint256 internal _totalRemainingRewards; // total unsettled amount of rewards and withdrawal
  uint256 internal _removedSum; // the total withdrawal reserving amount
  ...
  event AddReward(address indexed account, uint256 indexed term, uint256 amount);
  ...
  modifier updateTerm() {
    ...
    if (_currentTerm < latestTerm) {
      ...
      _totalRemainingRewards = _totalRemainingRewards.add(rewardAdded).add(_removedSum);
      _removedSum = 0;
  ...
  modifier updateReward(address account) {
    ...
    for (uint256 term = startTerm; term < _currentTerm && term < startTerm + MAX_TERM; term++) {
      ...
      if (termInfo.stakeSum != 0) {
        ...
        emit AddReward(account, term, rewardsAdded);
      }
  ...
  function _withdraw(address account, uint128 amount) internal {
    ...
    _removedSum = _removedSum.add(amount);
  ...
  function _getRewardAdded() internal view returns (uint256 rewards) {
    ...
    // rewardAdded = currentBalance - stakeSum - stakeAdd - removedSum - _totalRemainingRewards
    uint256 rewardAdded =
      currentBalance.sub(
        _getNextStakeSum().add(_removedSum).add(_totalRemainingRewards),
        "system error"
      );
    return rewardAdded;
  }
...
interface StakingAggregatorInterface {
  ...
  function getAccountInfo(address account)
    ...
    returns (
      ...
      uint256[] memory withdrawableStakingAmountList
    );
}
...
contract StakingAggregator is StakingAggregatorInterface {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < numOfStakingContracts; i++) {
      (uint128 stakeAdd, uint128 stakeSum, uint256 rewardSum) =
        _stakingContracts[i].getTermInfo(term);
    ...
    )
  {
    ...
    withdrawableStakingAmountList = new uint256[](numOfStakingContracts);
    for (uint256 i = 0; i < numOfStakingContracts; i++) {
