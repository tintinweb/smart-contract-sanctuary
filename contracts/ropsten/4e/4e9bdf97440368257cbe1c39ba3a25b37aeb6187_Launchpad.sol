...
contract Launchpad is ReentrancyGuard, Initializable {
  using SafeMath for uint256;
  ...
  uint256 public maxCap = 2000000000000000000;    // 2 WETH
  ...
  bool public everyoneCanWidthaw = false;
  ...
  function initialize(
      IBEP20 _lpToken,  // WETH Contract
      IBEP20 _offeringToken, // Presale Token Contract
      uint256 _startBlock, // Start Time
      uint256 _endBlock, // End Time
      uint256 _offeringAmount, // Token Amount settled for presale
      uint256 _raisingAmount, // Min. Amount to raise, has to be set manually, for this case, 1 WETH min cap 1000000000000000000
      address _adminAddress // who can call
  ) public initializer {
      lpToken = _lpToken;
  ...
  function deposit(uint256 _amount) public {
    ...
    require (_amount > 100000000000000000, 'need _amount > 0'); // minimum amount to contribute in the presale i.e 0.1 WETH
    require(userInfo[msg.sender].amount.add(_amount) <= 500000000000000000, 'maximum contribution reached'); // max amount to contribute in the sale i.e 0.5 WETH
    require(totalAmount.add(_amount) <= maxCap);
 ...
 // to change end block (to be called as soon as max cap is reached)
    ...
    function activateWithdrawal() public onlyAdmin{
        everyoneCanWidthaw = true;
    }
  ...
  function claim() public nonReentrant {
    ...
    require (everyoneCanWidthaw == true, 'withdraw not activated');
    uint256 offeringTokenAmount = userInfo[msg.sender].amount;
    uint256 refundingTokenAmount = userInfo[msg.sender].amount;
    if(totalAmount >= raisingAmount ){
        offeringToken.safeTransfer(address(msg.sender), offeringTokenAmount.mul(1976));
    }
    else {
  ...
  function getOfferingAmount(address _user) public view returns(uint256) {
    ...
    //   uint256 allocation = getUserAllocation(_user);
      return userInfo[msg.sender].amount.mul(1976);
    }
    else {
  ...
  function finalWithdraw() public onlyAdmin {
    ...
    if(lpToken.balanceOf(address(this)) > 0) {
      lpToken.safeTransfer(address(msg.sender), lpToken.balanceOf(address(this)));
    }
  }
  ...
  function burnTokens() public onlyAdmin{
    require (block.number > endBlock, 'not withdrawal time');
    require (totalAmount >= raisingAmount, 'min cap not reached');
    uint256 totalToBurn = maxCap - totalAmount; 
    if(offeringToken.balanceOf(address(this)) > 0) {
      offeringToken.safeTransfer(0x000000000000000000000000000000000000dEaD, totalToBurn.mul(1976));
    }
  }
}