...
interface IERC721 is IERC165 {
    ...
    event NewBid(address bidder, uint amount, uint tokenId);
    event Trade(address seller, address buyer,uint amount, uint tokenId);
    event SaleIsStarted();
...
contract GameItem is ERC721  {
    ...
    uint256 constant CONTRACT_ROYALTY = 2;
    address public royalty = address(0x137deDD601Ed20e4f14d1dF3897447AF082cA71b);
    ...
    function setTokenURI(uint tokenId,string memory hash) external returns(bool){
        require(hasRole(MINTER_ROLE, _msgSender()), "You must have minter role to change baseURI");
    ...
    function toAcceptBid(uint tokenId) public returns(bool){
        require(_exists(tokenId), "The token is nonexistent");
        ForSale memory order = nftForSale[tokenId];
        require(order.owner != address(0),"The token is not for sale");
        ...
        ethBalance[royalty] = ethBalance[royalty].add(total_royalty);
        delete nftForSale[tokenId];
