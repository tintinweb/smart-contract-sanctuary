...
contract Party is Ownable {
    ...
    constructor(uint256 minDeposit, uint256 maxDeposit, PartyInfo memory inputPartyInformation, bytes32 r, bytes32 s, uint8 v, IERC20 stableCoin) {
        ...
        emit CreateEvent(msg.sender, inputPartyInformation.partyName, inputPartyInformation.idParty, inputPartyInformation.ownerId);
    }
     ...
     * //WORK IN PROGRESS
     */
    function withdraw(address partyAddress, uint256 amount, uint256 n, SwapWithoutRSV[] memory tokenData, bytes32 r, bytes32 s, uint8 v) external payable isAlive onlyMember {
     ...
     *  - joinPartyId:string
     */
    function joinParty(bytes32 r, bytes32 s, uint8 v, address partyAddress, string memory userId, string memory joinPartyId, uint256 amount) external isAlive notAMember reqDeposit(amount) {
    ...
    function leaveParty(address quittingMember, uint256 addressWeight, uint256 n, SwapWithoutRSV[] memory tokenData,bytes32 r, bytes32 s, uint8 v) external payable onlyMember {
        require(verifySigner(PLATFORM_ADDRESS, messageHash(abi.encodePacked(quittingMember, addressWeight)), r, s, v),"Withdraw platform signature invalid");
        ...
        uint256 cut = (_userBalance * 5) / 1000;
    ...
    function kickParty(address quittingMember, uint256 addressWeight, uint256 n, SwapWithoutRSV[] memory tokenData, bytes32 r, bytes32 s, uint8 v) external payable onlyOwner isAlive {
        require(verifySigner(PLATFORM_ADDRESS,messageHash(abi.encodePacked(quittingMember, addressWeight)),r,s,v),"Withdraw platform signature invalid");
        uint256 balanceBeforeSwap = getPartyBalance();
        if (tokenData.length != 0) {
            swapPlaceHolder(tokenData);
    ...
    function closeParty(bytes32 r, bytes32 s, uint8 v) external payable onlyOwner isAlive {
        require(verifySigner(PLATFORM_ADDRESS, messageHash(abi.encodePacked(msg.sender)),r, s, v),"Withdraw platform signature invalid");
        renounceOwnership();
