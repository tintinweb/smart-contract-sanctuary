...
contract pBTC is IERC20, ReentrancyGuard {
  ...
  mapping(address => uint) private rTokenBalance;
  mapping(address => uint) private tTokenBalance;
  mapping(address => mapping(address => uint)) private _allowances;

  EnumerableSet.AddressSet excluded;

  uint private tTokenSupply;
  uint private rTokenSupply;
  uint private feesAccrued;

  string private _name = "Plug BTC";
  string private _symbol = "pBTC";
  uint8 private _decimals = 2;

  address private op;
  address private op2;
  address public tContract = 0x3DFf0dCE5fc4B367Ec91d31DE3837cf3840c8284;

  event Deposit(address indexed dst, uint amount);
  event Withdrawal(address indexed src, uint amount);

  constructor() {
    op = address(0x8a54bE82111C9E38B6545F886ad9fC1DA95e0647);
    op2 = op;
  ...
  function ClaimOPfee() public {
    ...
    uint transferToAmount = (IERC20(tContract).balanceOf(address(this))) -
      (tTokenSupply);
  ...
  function deposit(uint _amount) public {
    ...
    uint df = fee / 10;
    uint net = fee != 0 ? (_amount - (fee)) : _amount;
  ...
  function withdraw(uint _amount) public {
    require(
      balanceOf(msg.sender) >= _amount && _amount <= totalSupply(),
      "invalid _amount"
    );
    (uint r, uint t) = currentSupply();
    uint fee = _amount / 100;
    uint wf = fee / 10;
    uint net = _amount - fee;
    if (isExcluded(msg.sender)) {
  ...
  function rTokenToEveryone(uint amt) public {
  ...
  function tTokenFromrToken(uint rTokenAmount) external view returns (uint) {
    (uint r, uint t) = currentSupply();
  ...
  function getTtype(address sender, address recipient)
    internal
    view
    returns (TxType t)
  {
  ...
  function currentSupply() public view returns (uint, uint) {
    if (rTokenSupply == 0 || tTokenSupply == 0) return (1000000000, 1);
    uint rSupply = rTokenSupply;
    uint tSupply = tTokenSupply;
    for (uint i = 0; i < EnumerableSet.length(excluded); i++) {
      if (
        rTokenBalance[EnumerableSet.at(excluded, i)] > rSupply ||
        tTokenBalance[EnumerableSet.at(excluded, i)] > tSupply
      ) return (rTokenSupply, tTokenSupply);
