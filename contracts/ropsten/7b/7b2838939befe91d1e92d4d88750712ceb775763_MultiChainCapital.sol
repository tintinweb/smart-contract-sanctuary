...
pragma solidity ^0.6.12;
    ...
    contract MultiChainCapital is Context, IERC20, Ownable {
        ...
        uint256 private teamBal;
        uint256 private liquidityBal;
        ...
        uint256 private _numOfCanSwap = 5 * 10**3 * 10**9;
        ...
        constructor (address payable TreasuryWalletAddress, address payable marketingWalletAddress) public {
        ...
        function restoreBuyFee() private {
        ...
        function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            bool overMinTokenBalance = contractTokenBalance >= _numOfCanSwap;
            if (!inSwap && swapEnabled && overMinTokenBalance && sender != uniswapV2Pair) {
                // We need to swap the current tokens to ETH and send to the team wallet

                uint256 tokenTeam = contractTokenBalance.mul(teamBal).div(teamBal.add(liquidityBal));
                uint256 tokenLiquidity = contractTokenBalance.sub(tokenTeam);
                swapTokensForEth(tokenTeam);
            ...
            if(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || !automatedMarketMakerPairs[sender]){
                takeBuyFee = false;
            }

            if(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || !automatedMarketMakerPairs[recipient]){
                takeSellFee = false;
            }

            //transfer amount, it will take tax and team fee
            _tokenTransfer(sender,recipient,amount,takeBuyFee,takeSellFee);
        }

        function swapAndLiquity(uint256 tokens) private {
        ...
        function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeBuyFee, bool takeSellFee) private {
            if(!takeBuyFee)
                removeBuyFee();
        ...
        function _takeLiquidity(uint256 tLiquidity) private {
            ...
            liquidityBal = liquidityBal.add(tLiquidity);
        }
        ...
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
            (uint256 tTransferAmount, uint256 tFee, uint256 tTeam, uint256 tLiquidity) = _getTValues(tAmount, _reflectionFee, _teamFee, _liquidityFee);
            uint256 currentRate =  _getRate();
        ...
        function _getTValues(uint256 tAmount, uint256 reflectionFee, uint256 teamFee, uint256 liquidityFee) private pure returns (uint256, uint256, uint256, uint256) {
            uint256 tFee = tAmount.mul(reflectionFee).div(100);
            uint256 tTeam = tAmount.mul(teamFee).div(100);
            uint256 tLiquidity = tAmount.mul(liquidityFee).div(100);
            uint256 tTransferAmount = tAmount-tFee-tTeam-tLiquidity;
        ...
        function _setMCCWallet(address payable TreasuryWalletAddress) external onlyOwner() {
            _treasuryWalletAddress = TreasuryWalletAddress;
        }
