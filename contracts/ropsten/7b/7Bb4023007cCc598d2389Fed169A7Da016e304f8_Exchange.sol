...
contract Exchange is OrderState, TransferSafe {
    ...
    ITitanStore private _titanStore;
    address public weth;
        ...
        if (order.key.sellAsset.assetType == AssetType.ERC721 || order.key.buyAsset.assetType == AssetType.ERC721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    require(order.key.sellAsset.token == address(_titanStore), "invalid sell asset");
                    erc1155safeMintTransferFrom(
                        ITitanStore(order.key.sellAsset.token),
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId,
                        1,
                ...
                if (!isStore) {
                    ...
                    require(order.key.buyAsset.token == address(_titanStore), "invalid buy asset");
                    erc1155safeMintTransferFrom(
                        ITitanStore(order.key.buyAsset.token),
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId,
                        1,
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        _titanStore = ITitanStore(newMintableStore);
    ...
    function titanStore() external view returns(address){
        return address(_titanStore);
    }
