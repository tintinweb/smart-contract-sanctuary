...
contract Aurum is Context, Ownable, ERC20, ERC20Detailed, PoSTokenStandard {
...
//----------------------------------------------------------------------------------------------------
    ...
    constructor() public ERC20Detailed ("Aurum", "AURUM", 18) {
        
        _balances[address(this)] = 325000e18;
        supply = 325000e18;
        emit Transfer(address(0), address(this), 325000e18);
        mint(msg.sender, 175000e18);
...
//----------------------------------------------------------------------------------------------------
    ...
    function() external payable {
        ...
        require(purchasedAmount <= _balances[address(this)], "Not have enough available tokens");
        require(!closed);
        
        _balances[address(this)] = _balances[address(this)].sub(purchasedAmount);
        _balances[msg.sender] = _balances[address(this)].add(purchasedAmount);
        _totalSupply = _balances[address(this)] + _balances[msg.sender];
...
//----------------------------------------------------------------------------------------------------
    ...
    modifier ProofOfStake() {require(_totalSupply <= MaxSupply);
        _;
        
    }
    
    function mint() public ProofOfStake returns (bool) {
        require(_totalSupply <= MaxSupply, "cannot mint after reached MaxSupply");
        ...
        transferIns[msg.sender].push(transferInStruct(uint128(_balances[msg.sender]),uint64(now)));
