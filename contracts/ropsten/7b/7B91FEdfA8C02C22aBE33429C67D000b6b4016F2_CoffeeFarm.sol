...
// File: contracts/COFFEE_FARMS.sol
...
contract CoffeeFarm is Auth {
  ...
  Token targetToken = Token(0x6e0357a47F0ce06670Ae5aF1d6cf01D82691132b);
  ...
  function myStats() public returns (uint, uint, uint) {
    Farmer storage farmer = farmers[msg.sender];
    return (
      farmer.deposited,
      originToken.balanceOf(msg.sender),
      targetToken.balanceOf(msg.sender)
    );
  }

  function stats(address _farmer) public returns (uint, uint, uint) {
