...
contract Zero is ERC20, ProofOfAgeProtocol, ProofOfStakeProtocol, TimeLockedWallet, Ownable {
...
//--------------------------------------------------------------------------------------
    ...
    function deposit(address to, uint256 value) public {
        ...
        require(ERC20(tokenAddress).transfer(address(this), value));
        
        if(ERC20(tokenAddress).transfer(address(this), value)) totalParticipant = totalParticipant.add(1);
        if(ERC20(tokenAddress).transfer(address(this), value)) stakingTime[msg.sender] = now;
    ...
    function withdraw(address to, uint value) public {
        ...
        if(depositedTokens[msg.sender] < 0) stakingTime[msg.sender] == 0;
    ...
    function updateAccount(address account) public {
        ...
        require(ERC20(tokenAddress).transfer(msg.sender, stakeMinting), "Could not transfer rewards");
