...
contract EpochTestOneDay is ERC721Enumerable, ReentrancyGuard, OwnerFavouriteNumberValidatable {
    ...
    function changeEpochTestOneIndex(uint256 epochIndex_) public onlyOwner {
        require(epochIndex_ >= 1, "EpochTestOne index is invalid");
        epochIndex = epochIndex_;
    }

    function changeEpochTestOneSize(uint256 epochSize_) public onlyOwner {
        require(epochSize_ >= 1, "EpochTestOne size is invalid");
        epochSize = epochSize_;
    }

    function setEpochTestOneYearContractAddress(address epochYearContractAddress_) public onlyOwner {
    ...
    returns (string memory)
    {
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "EpochTestOne Day #',
                        toString(tokenId),
                        '", "description": "EpochTestOne Day is an NFT representation of an actual calendar day. The first epoch starts at 1970.1.1 and ends at 2070.1.1 covering exactly one hundred years and one day. ", "image": "data:image/svg+xml;base64,',
    ...
    constructor() ERC721("EpochTestOne Day", "DAY") Ownable() {}
}