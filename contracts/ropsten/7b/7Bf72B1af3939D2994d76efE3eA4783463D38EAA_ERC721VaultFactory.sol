...
contract TokenVault is ERC20Upgradeable, ERC721HolderUpgradeable {
    ...
    /// @notice a boolean to indicate if the reserve asset is ETH or not
    bool public withETH;

    /// @notice the reserve asset token ERC20
    IERC20 public asset;
    ...
    function initialize(
        ...
        bool _withETH = _asset == address(0);
        require(
            _withETH || ISettings(settings).assetWhitelisted(_asset),
            "initializer: asset token is not whitelisted"
        );
    ...
    function startWithETH() external payable {
        require(withETH, "start:the reserve asset is not ETH");
    ...
    /// @notice kick off an auction. Must send reservePrice in asset
    function start(uint256 amount) external payable {
        require(!withETH, "start:the reserve asset is ETH");
        require(auctionState == State.inactive, "start:no auction starts");
        require(amount >= reservePrice(), "start:too low bid");
        uint256 allowance = asset.allowance(msg.sender, address(this));
        require(allowance >= amount, "start:check the token allowance");
        require(
            votingTokens * 1000 >=
                ISettings(settings).minVotePercentage() * totalSupply(),
            "start:not enough voters"
        ...
        emit Start(msg.sender, amount);
    }

    /// @notice an external function to bid on purchasing the vaults NFT. The msg.value is the bid amount
    function bidWithETH() external payable {
        require(withETH, "bid:the reserve asset is not ETH");
    ...
    /// @notice an external function to bid on purchasing the vaults NFT. bid with asset ERC20 token
    function bid(uint256 amount) external payable {
        require(!withETH, "start:the reserve asset is ETH");
        require(auctionState == State.live, "bid:auction is not live");
        uint256 increase = ISettings(settings).minBidIncrease() + 1000;
        require(amount * 1000 >= livePrice * increase, "bid:too low bid");
        uint256 allowance = asset.allowance(msg.sender, address(this));
        require(allowance >= amount, "bid:check the token allowance");
        require(block.timestamp < auctionEnd, "bid:auction ended");
        asset.transferFrom(msg.sender, address(this), amount);
        ...
        asset.transfer(winning, livePrice);
    ...
    function cashWithETH() external {
        require(withETH, "cash:the reserve asset is not ETH");
    ...
    /// @notice an external function to burn ERC20 tokens to receive asset ERC20 tokens from ERC721 token purchase
    function cash() external {
        require(!withETH, "cash:the reserve asset is ETH");
        require(auctionState == State.ended, "cash:vault not closed yet");
        uint256 bal = balanceOf(msg.sender);
        require(bal > 0, "cash:no tokens to cash out");
        uint256 share = (bal * address(this).balance) / totalSupply();
...
contract ERC721VaultFactory is Ownable, Pausable {
    ...
    /// @param _asset the address of reserve asset. if it is address(0), then it is ETH
    ...
    function mint(
        ...
        require(
            _withETH || ISettings(settings).assetWhitelisted(_asset),
            "mint: asset token is not whitelisted"
        );
        ...
        emit Mint(_token, _id, _withETH, _asset, _listPrice, vault, vaultCount);
...
contract FNFT is ERC165Storage, IERC721, IERC721Metadata {
