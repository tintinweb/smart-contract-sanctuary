...
contract CashPoints is StandardToken { // ПОДЛЕЖИТ ИЗМЕНЕНИЮ. Обновить название контракта.
    ...
    */
    ...
    function CashPoints() {
        balances[msg.sender] = 100000000000000000000000000;               // Предоставить создателю контракта все начальные токены. В нашем случае количество равно 100000000. Если вы хотите, чтобы количество равнялось число X, а десятичные равнялись 5, установите следующее значение X * 100000. (ПОДЛЕЖИТ ИЗМЕНЕНИЮ)
        totalSupply = 100000000000000000000000000;                        // Обновить общий выпуск (100000000 для примера) (ПОДЛЕЖИТ ИЗМЕНЕНИЮ)
        name = "Cash Points";                                   // Установить название токена для отображения на дисплее (ПОДЛЕЖИТ ИЗМЕНЕНИЮ)
        decimals = 18;                                               // Количество десятичных знаков после запятой для отображения на дисплее (ПОДЛЕЖИТ ИЗМЕНЕНИЮ)
        symbol = "CPS";                                             // Идентификатор токена для отображения на дисплее (ПОДЛЕЖИТ ИЗМЕНЕНИЮ)
