...
contract RidiculousRhinos is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    /**
     * Max free mints allowed 
     */

    uint256 public freeMaxMint = 2;
    ...
    mapping(address => uint256) private _freeClaim;
    ...
    /**
     * Flip free minting status
     */

    function flipFreeMintStatus() public onlyOwner {
     ...
     */
     ...
     function mintFree(uint256 numberOfTokens) public payable {
        require(freeIsActive, "Sale must be active to mint");
        require(
            numberOfTokens <= 2,
            "You can only mint 2 rhinos at a time"
        );
        require(
            totalSupply() + numberOfTokens <= maxFreeTokens,
            "Purchase would exceed max available free rhinos"
        );
        
        require(
            _freeClaim[msg.sender] + numberOfTokens <= freeMaxMint,
