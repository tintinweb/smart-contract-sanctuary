...
contract MasterChef is Ownable, ReentrancyGuard, IMasterChef, Constants {
    ...
    struct UserInfo {
        ...
        uint64 rewardPoolShare;        // Share of reward pool.
        ...
        //  5. rewardPerShare = pool.allocPoint * timeReward * user.amount / totalAllocPoint / lpSupply / poolTimePoints
    ...
    function pendingLotl(uint8 _pid, address _user) external view returns (uint128) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid + 1][_user];
        uint32 currentBlock = uint32(block.number);
        uint128 accLotlPerShare = pool.accLotlPerShare;
        uint128 lpSupply = uint128(pool.lpToken.balanceOf(address(this)));
        if (currentBlock > pool.lastRewardBlock && lpSupply != 0) {
            uint128 lotlReward =  (currentBlock - pool.lastRewardBlock) * lotlPerBlock * pool.allocPoint / totalAllocPoint;
    ...
    function pendingRewards(address _user) external view returns (uint128 _lotl, uint128 _busd) {
        ...
        if(user.rewardPoolShare > 0){
            uint128 busdPending = rewardInfo.amountBUSD * user.rewardPoolShare / 1e12;
            uint128 lotlPending = rewardInfo.amountLOTL * user.rewardPoolShare / 1e12;
    ...
    function calculateTimeRewards (uint32 _stakedSince) private returns (uint32)  {
        /*
    ...
    function deposit(uint8 _pid, uint128 _amount) public nonReentrant {
        ...
        if (_amount > 0) 
        {
            ...
            {
                pool.totalTimeAlloc = pool.totalTimeAlloc - user.stakedSince;
                user.stakedSince = uint32(user.stakedSince + (currentBlock - user.stakedSince) * taxedAmount / (user.amount + taxedAmount));
                pool.totalTimeAlloc = pool.totalTimeAlloc + user.stakedSince;
            }
            ...
            if (pool.depositFeeBP > 0) 
            {
                ...
                pool.lpToken.safeTransfer(address(rewardPool), depositFee);
    ...
    function safeLotlTransfer(address _to, uint128 _amount) internal {
        uint128 lotlBal = uint128(lotl.balanceOf(address(this)));
    ...
    function safeBusdTransfer(address _to, uint128 _amount) internal {
        uint128 busdBal = uint128(IERC20(busdAddr).balanceOf(address(this)));
        bool transferSuccess = false;
        if (_amount > busdBal) {
    ...
    */
    ...
    function calculateRewardPool() external override {
        ...
        for (uint8 i=0; i < length; i++){
            ...
            uint64 poolTimeReward = (pool.totalTimeAlloc - currentBlock * pool.totalUserStaked);
            ...
            for(uint32 j; j< userLength; j++){
                ...
                if(user.stakedSince > 0){
                    ...
                    rewardUser.rewardPoolShare = rewardUser.rewardPoolShare + uint64(user.amount * 1e12 / totalAllocPoint * pool.allocPoint / lpSupply * timeReward / poolTimeReward);
    ...
    function currentHoldingFactor(uint8 _pid, address _user) public view returns(uint32 holdFactor){
        UserInfo storage user = userInfo[_pid+1][_user];
        if(user.stakedSince > 0)
        {
