...
contract TestUniswapFlashSwap is IUniswapV2Callee {
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external override {
        ...
        // do stuff here
        // uint[] memory amountOutSwap = IUniswapV2Router(ROUTER).swapExactTokensForTokens(amountBorrow, 1, path, address(this), block.timestamp);
        // require(amountOutSwap[0] > amountToRepay, "Not profitable swap trade");
        // uint amountProfit = amountOutSwap[0] - amountToRepay;
        ...
        emit Log("fee", fee);
        // emit Log("amount for swap", amountOutSwap[0]);
