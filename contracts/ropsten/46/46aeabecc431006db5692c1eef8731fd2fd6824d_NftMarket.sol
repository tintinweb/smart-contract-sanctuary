...
contract NftMarket is Owned {
    ...
    function NewNft1(string memory _tokenURI, uint256 _royalty)
        external
        _lock_
        returns (uint256)
    {
    ...
    function NewNft2()  external _lock_ returns (address){
    ...
    function NewNft3()  external _lock_ returns (address){
        
        ERC20Like(abcToken).transfer(revenueRecipient, mintFee);
        return address(this);
    }
    
    function NewNft4()  external view returns (address){
