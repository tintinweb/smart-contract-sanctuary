...
//cardType |  1-12 is constelle  |  13-16 is Tenjin
contract ConstelleCard is ERC721Enumerable, Member {
    using Strings for uint256;

    mapping(address => bool) public packages;

    uint256 public constant ID_PREFIX_MASK = uint136(type(uint8).max) << 128;

    constructor() ERC721("Hi5Constelle", "Hi5Constelle") {}

    uint256 public constant NFT_SIGN_BIT = 1 << 135;

    // string public uriPrefix =
    //     "ipfs://bafybeidzfq3xqsafissxnvanwl52yb2ccel3agybocs2fmbkmh555ouq4a";
    string public uriPrefix =
        "https://ipfs.fleek.co/ipfs/bafybeicikkcds5cqhcpvqawhczjl4ans33o5vahmllyjc22zm455fyrzua/json/";

    function mint(address to, uint8 cardType) public {
        require(packages[msg.sender], "Hi5Card: package only");
        uint256 tokenId = NFT_SIGN_BIT |
            ((uint136(cardType) << 128) & ID_PREFIX_MASK) |
            (uint256(uint64(block.timestamp)) << 64) |
            uint64(totalSupply() + 1);

        _safeMint(to, tokenId);
    }

    function batchMint(address to, uint8[] memory cardTypes) external {
            ...
            bytes(baseURI).length > 0
                ? string(
                    abi.encodePacked(
                        ...
                        Base64.encode(
                            bytes(
                                uint256(uint8(tokenId >> 128) ^ uint8(1 << 7))
                                    .toString()
                            )
                        )
                    )
