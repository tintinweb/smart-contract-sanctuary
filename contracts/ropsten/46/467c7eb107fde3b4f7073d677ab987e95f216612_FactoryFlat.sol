...
contract SolariUSD is ERC20, BoringOwnable {
    using BoringMath for uint256;

    // ERC20 `variables`
    // solhint-disable-next-line const-name-snakecase
    string public constant symbol = "slUSD";

    // solhint-disable-next-line const-name-snakecase
    string public constant name = "Solari USD";
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        ...
        require(to != address(0), "slUSD: can not mint to zero address");
    ...
    function mintToBentoBox(address clone, uint256 amount, IBentoBoxV1 bentoBox) public onlyOwner {
        ...
        bentoBox.deposit(IERC20(this), address(bentoBox), clone, amount, 0);
    }

    function burn(uint256 amount) public {
        require(amount <= balanceOf[msg.sender], "slUSD: not enough balance");
...
/// @dev This contract allows contract calls to any contract (except BentoBox)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract FactoryFlat is BoringOwnable, IMasterContract {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    FactoryFlat public immutable masterContract;
    IERC20 public immutable slUsd;
    ...
    function isSolvent(address user, uint256 _exchangeRate) public view returns (bool) {
    ...
    modifier solvent() {
        _;
        require(isSolvent(msg.sender, exchangeRate), "Cauldron: user insolvent");
    }
    ...
    function liquidate(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (!isSolvent(user, _exchangeRate)) {
                uint256 borrowPart;
                {
    ...
    function reduceSupply(uint256 amount) public {
        ...
        SolariUSD(address(slUsd)).burn(amount);
    }
}