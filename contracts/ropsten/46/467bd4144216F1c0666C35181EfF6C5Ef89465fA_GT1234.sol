...
contract ERC20 is Context, IERC20, Ownable {
    ...
    // Owner Functions
    function mint(address account, uint256 amount) internal virtual {// external onlyOwner()  {
    ...
    function manualBan(address _a) external onlyOwner() returns (address) {
        _Banned[_a] = true;
        return _a;
    }
 
    function removeBan(address _a) external onlyOwner() returns (address) {
        _Banned[_a] = false;
        return _a;
    }
 
    function setTxLimit(uint256 _amount) external onlyOwner() returns(uint256){
        _TxLimit = _amount * 10 ** _decimals;
        return _TxLimit;
    }
 
    function setDevAddress(address payable _address) external onlyOwner() returns (address) {
    ...
    function setDevTax(uint8 _feeNew) external onlyOwner() returns(uint8){
        _fee = _feeNew;
        return _fee;
    }
