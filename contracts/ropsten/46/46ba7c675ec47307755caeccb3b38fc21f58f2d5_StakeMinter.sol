...
contract StakeMinter is IStakeMinter, Pausable, ExternallyCallable {
    ...
    */
    ...
    function mintExistingLegacyStake(uint256 id) external pausable {
        mintExistingLegacyStakeInternal(msg.sender, id);

        contracts.stakeToken.mint(msg.sender, id);
    }
    ...
    function mintExistingLegacyStakeFrom(
        ...
        uint256 shares = mintExistingLegacyStakeInternal(from, id);

        contracts.stakeCustodian.addStake(to, id);

        contracts.vcAuction.transferSharesAndRebalance(from, to, shares);
    }
    ...
    /** @dev Mint existing legacy stake from
        ...
        Modifier: OnlyManager
     */
    function mintExistingLegacyStakeInternal(address staker, uint256 id)
        internal
        returns (uint256)
    {
        require(id <= settings.lastSessionIdV2, 'STAKE MINTER: invalid stakeId.');
        require(
            contracts.stakeManager.getStakeWithdrawnOrExists(id) == false,
            'STAKE MINTER: stake is withdrawn or already v3.'
        );
        ...
        ) = contracts.stakingV2.sessionDataOf(staker, id);
        ...
        contracts.stakeManager.createExistingStake(
            id,
            amount,
            shares,
            start,
