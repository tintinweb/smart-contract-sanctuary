...
contract DirtyFarm is Ownable, Authorizable, ReentrancyGuard {
    ...
    function updateVariablePoolReward() private {
        ...
        uint256 ratio;
        uint256 multiplier;
        uint256 rewardSegment = poolReward.mul(100).div(200);
        ...
        if (runningTotal1 >= maxDirtyStake) {
             stakelpratio = SafeMath.div(runningTotal1, maxDirtyStake, "stakelpratio >= maxDirtyStake division error"); 
             stakelpratio = SafeMath.div(stakelpratio, maxLPStake, "stakelpratio / maxLPStake division error");
        } else {
            stakelpratio = SafeMath.div(maxDirtyStake, maxDirtyStake, "stakelpratio maxDirtyStake / maxDirtyStake division error");
        }   

        multiplier = SafeMath.add(lpratio, stakelpratio);
        ...
        if (stakelpratio > lpratio) {
            ratio = SafeMath.div(stakelpratio, lpratio, "stakelpratio > lpratio division error");
        ...
        if (stakelpratio < lpratio) {
            ratio = SafeMath.div(lpratio, stakelpratio,  "stakelpratio < lpratio division error");
