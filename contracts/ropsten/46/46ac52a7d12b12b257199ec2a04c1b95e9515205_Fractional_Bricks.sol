...
// Fractional Bricks to the moon baby!
 
contract Fractional_Bricks is ERC721, ERC721Enumerable, Ownable {
...
function Mint(uint256 _mintAmount)external payable{
    ...
    require(msg.value >= cost.mul(_mintAmount), "Insufficient funds for transaction");
    for (uint256 i = 0; i < _mintAmount; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
    }
}
 
function governanceMint(uint256 _mintAmount)external payable{
    require(!paused, "Contract is paused!");
    require(governanceWhitelisted == true);
  ...
  */
  ...
  function turnPublicMintOn() external onlyOwner{
        publicMintState = true;
   }
 
  function turnGovernanceOff() external onlyOwner{
        governanceWhitelisted = false;
   }
