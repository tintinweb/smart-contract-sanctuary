...
contract Spud3D {
    ...
    function GetSpud(address MN) public payable
    {
        ...
        if(blocknr == Spudgame[curround].blocknumber) 
        {
        ...
        if(blocknr - 256 <= Spudgame[curround].blocknumber && blocknr != Spudgame[curround].blocknumber)
        {
        
        uint256 RNGresult = uint256(blockhash(Spudgame[round].blocknumber)) % RNGdeterminator;
        emit SpudRnG(Spudgame[curround].player , RNGresult) ;
        Spudgame[curround].Result = RNGresult;
        Pot += 1 finney;
        if(RNGresult == 1)
        {
            // won payout
            uint256 RNGrotator = uint256(blockhash(Spudgame[round].blocknumber)) % nextspotnr;
        ...
        if(block.number.sub(256) > Spudgame[round].blocknumber && RNGresult != 1)
        {
            //win
            // won payout
            Pot += 1 finney;
