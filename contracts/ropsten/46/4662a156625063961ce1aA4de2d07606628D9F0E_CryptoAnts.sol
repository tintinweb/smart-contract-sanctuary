...
interface ICryptoAnts is IERC721 {
  event AntCreated(address ownerId, uint256 antId);
  event AntDead(address ownerId, uint256 antId, uint256 reason);
  event EggsBought(address ownerId, uint256 amount);
  event EggsBurn(address ownerId, uint256 amount);
  event EggsMinted(uint256 antId, address ownerId, uint256 amount);

  function antsCreated() external view returns (uint256);

  function antToHatchTime(uint256 antId) external view returns (uint256);

  function antToOwner(uint256 antId) external view returns (address);

  function buyEggs(uint256 amount) external payable;

  function eggPrice() external view returns (uint256);

  function eggs() external view returns (IEgg);

  function probOfDeath() external view returns (uint256);

  error AlreadyExists();
  error AmountCannotBeZero();
  error CallFailed();
  error HatchNotEnabled();
  error NoEggs();
  error NoReentrancy();
