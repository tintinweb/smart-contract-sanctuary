...
contract Visions is VisionsPricing {
    ...
    Counters.Counter private _airdropClaimed;
    ...
    uint16 public constant SALE_AMOUNT = 333;
    ...
    DerivativeParameters params = DerivativeParameters(false, false, 0, SALE_AMOUNT + AIRDROP_RESERVE, 4);
    ...
    function claimAirdrop(address recipient) public nonReentrant {
        ...
        require(_airdropClaimed.current() <= AIRDROP_RESERVE, "Visions:ALL_AIRDROPPED_CLAIMED");

        for (uint256 i = 0; i < airdropNum; i++) {
            uint256 tokenId = _airdropClaimed.current();
    ...
    function sweepAirdrop(address recipient) public virtual onlyDAO nonReentrant {
        uint256 unclaimedAmount = AIRDROP_RESERVE - _airdropClaimed.current();
    ...
    function openMintsAvailable() public view override returns (uint256) {
        if (_isPublicSaleActive()) {
            return params.maxTotalSupply + 1 - _tokenIds.current() - AIRDROP_RESERVE;
