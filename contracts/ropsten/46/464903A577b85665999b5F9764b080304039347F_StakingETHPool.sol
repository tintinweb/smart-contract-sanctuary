...
contract StakingETHPool is State, AccessControl {
	...
	function removeMember(address member) public virtual onlyTeam {
		require(member != address(0), "member cant be zero address");
		revokeRole(ETHPOOL_ROLE, member);
	}
	...
	{
		staked = g_state.stakedAmount;
		rewards = address(this).balance - g_state.stakedAmount;
	}
...
library StorageETHPoolLib {
	...
	struct State {
		...
		mapping(address => StakeData) stakers;
...
library StakingETHPoolImpl {
	using SafeMath for uint256;
	using StorageETHPoolLib for StorageETHPoolLib.State;

	function found(StorageETHPoolLib.State storage state, uint256 val) public {
		require(msg.value == val, "wrong value");
		require(state.stakedAmount > 0, "nothing will claim this");
		uint256 index = state.rewardsCounter;
		if (index > 1) {
			// if there's a previus reward, should be pass at least a week
			uint256 weekNum = block.timestamp.sub(state.initialTimeStamp).div(
				1 weeks
			);
			uint256 prevWeekNum = state
				.rewards[index - 1]
				.initialTimeStamp
				.sub(state.initialTimeStamp)
				.div(1 weeks);

			require(prevWeekNum < weekNum, "please wait to the next week");
		}
		state.rewards[index].amount = val;
		state.rewards[index].currentStaked = state.stakedAmount;
		state.rewards[index].initialTimeStamp = block.timestamp;
		state.rewardsCounter++;
	}

	function stake(
		StorageETHPoolLib.State storage state,
		address staker,
		uint256 val
	) public {
		require(msg.value == val, "wrong value");
		require(msg.value > 0, "wrong value");
		state.stakedAmount = state.stakedAmount.add(val);

		state.stakers[staker].amount = val;
		state.stakers[staker].initialTimeStamp = block.timestamp;
	}

	function unstake(StorageETHPoolLib.State storage state, address staker)
		public
		returns (uint256)
	{
		uint256 amount = state.stakers[staker].amount;
		require(amount > 0, "nothing to unstake");
		uint256 reward = amount;
		uint256 initialTimeStamp = state.stakers[staker].initialTimeStamp;

		for (uint256 index = state.rewardsCounter - 1; index > 0; index--) {
			if (state.rewards[index].initialTimeStamp < initialTimeStamp) {
				break;
			}
			uint256 stepReward = state.rewards[index].amount.mul(amount).div(
				state.rewards[index].currentStaked
			);
			reward = reward.add(stepReward);
		}
		payable(staker).transfer(reward);

		state.stakedAmount = state.stakedAmount.sub(amount);
		delete state.stakers[staker];
...
// @aaiello: not using it right now, just in case we need to use Stake contract in other contract
interface IStakeETHPool {
...
contract State {
	StorageETHPoolLib.State g_state;
}
