...
contract Bridge is BridgeTypes, Ownable {
    ...
    // events
    event CONtoCONX(address indexed from, bytes32  swapID, uint256 indexed amount);
    event CONXtoCON(address indexed from, bytes32 indexed swapID, uint256 indexed amount);
...

    {
        ...
        require(history[depositId].amount == 0, "This id already used!");
        // store amount
        ...
        emit CONtoCONX(msg.sender, depositId, _amount);
    ...
    function getMsgForSign(bytes32 id, uint256 _amount, address _to) public pure returns(bytes32) {
        return keccak256(abi.encode(id,_amount, _to));
    }

    function getActual(bytes32 id, uint256 _amount, address _to) public pure returns(bytes32) {
        bytes32 actualMsg = getMsgForSign(id, _amount, _to);
        return actualMsg.toEthSignedMessageHash();
    }
        ...
        public
    {
        ...
        emit CONXtoCON(msg.sender, withdrawId, _amount);
    }
