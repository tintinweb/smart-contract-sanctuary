...
contract AUniswapV3NPM {
    ...
    // address is immutable, but we initialize it to facilitate etherscan verification
    address payable private UNISWAP_V3_NPM_ADDRESS;

    bytes4 immutable private APPROVE_SELECTOR = bytes4(keccak256(bytes("approve(address,uint256)")));
    
    function initUniswapLiquidityContract() external {
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
