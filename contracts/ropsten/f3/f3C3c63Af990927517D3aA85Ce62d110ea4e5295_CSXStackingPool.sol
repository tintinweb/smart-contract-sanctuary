...
contract CSXStackingPool is Ownable, ReentrancyGuard {
    ...
    event RewardHolderChange(address indexed previousHolder, address indexed newHolder);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                ...
                .sub(user.rewardDebt);
            if (pending > 0) {
                lpToken.safeTransferFrom(address(rewardTokenHolder),address(msg.sender), pending);
            }
        }
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        lpToken.safeTransferFrom(address(rewardTokenHolder),address(msg.sender), _amount);
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && lpTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare.add(
                cakeReward.mul(PRECISION_FACTOR).div(lpTokenSupply)
            );
            return
                user
                    .amount
