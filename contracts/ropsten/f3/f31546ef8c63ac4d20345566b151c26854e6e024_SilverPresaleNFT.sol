...
contract SilverPresaleNFT is ERC721CappedSale {
    using Strings for uint256;

    string private constant INIT__NFT_NAME = 'Silver Presale NFT';
    string private constant INIT__NFT_SYMBOL = 'SILVER';
    uint256 private constant INIT__CAP = 10;
    uint256 private constant INIT__MAX_BUY_AMOUNT = 1;
    uint256 private constant INIT__PRICE_PER_TOKEN = 5*10**15;
    ...
    function tokenURI(uint256 tokenId) public pure override returns (string memory) {
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Silver Presale NFT #',
                        toString(tokenId),
                        '", "description": "Silver Presale NFT", "image": "https://dummyimage.com/400x400/000/fff&text=SILVER+PRESALE"}'
                    )
                )
            )
        );
