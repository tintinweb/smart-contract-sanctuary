...
// File: contracts\erc20\ERC20UpgradeableProxy.sol
...
contract  ETHSwapAgentImpl is Context, Initializable {
    ...
    address public erc20ProxyAdmin;
    address public erc20Implementation;
    uint256 public swapFee;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    event SwapPairCreated(bytes32 indexed bscRegisterTxHash, address indexed erc20Addr, address indexed bep20Addr, string symbol, string name, uint8 decimals);
    event SwapStarted(address indexed erc20Addr, address indexed bep20Addr, address indexed fromAddr, uint256 amount, uint256 feeAmount);
    ...
    function initialize(address erc20Impl, uint256 fee, address payable ownerAddr, address erc20ProxyAdminAddr) public initializer {
        erc20Implementation = erc20Impl;
        swapFee = fee;
        owner = ownerAddr;
        erc20ProxyAdmin = erc20ProxyAdminAddr;
    }
    ...
    */
     ...
     * @dev createSwapPair
     */
    function createSwapPair(bytes32 bscTxHash, address bep20Addr, string calldata name, string calldata symbol, uint8 decimals) onlyOwner external returns (address) {
        require(swapMappingBSC2ETH[bep20Addr] == address(0x0), "duplicated swap pair");

        ERC20UpgradeableProxy proxyToken = new ERC20UpgradeableProxy(erc20Implementation, erc20ProxyAdmin, "");
        IProxyInitialize token = IProxyInitialize(address(proxyToken));
        token.initialize(name, symbol, decimals, 0, true, address(this));

        swapMappingBSC2ETH[bep20Addr] = address(token);
        swapMappingETH2BSC[address(token)] = bep20Addr;

        emit SwapPairCreated(bscTxHash, address(token), bep20Addr, symbol, name, decimals);
     ...
     * @dev fillBSC2ETHSwap
     */
    function fillBSC2ETHSwap(bytes32 bscTxHash, address bep20Addr, address toAddress, uint256 amount) onlyOwner external returns (bool) {
        require(!filledBSCTx[bscTxHash], "eth tx filled already");
        address bscTokenAddr = swapMappingBSC2ETH[bep20Addr];
        ...
        emit SwapFilled(bscTokenAddr, bscTxHash, toAddress, amount);
    ...
    function swapETH2BSC(address erc20Addr, uint256 amount) payable external notContract returns (bool) {
        address bep20Addr = swapMappingETH2BSC[erc20Addr];
        ...
        ISwap(erc20Addr).burn(amount);
        if (msg.value != 0) {
            owner.transfer(msg.value);
        }

        emit SwapStarted(erc20Addr, bep20Addr, msg.sender, amount, msg.value);
        return true;
    }
}