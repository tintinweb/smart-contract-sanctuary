 ...
 * @dev see https://eips.ethereum.org/EIPS/eip-721
 */
contract GameERC721 is Initializable, ContextUpgradeSafe, ERC165UpgradeSafe, IERC721, IERC721Metadata, IERC721Enumerable, ERC721UpgradeSafe,Governable {

    uint256 public maxId;
    address public admin;
    
    function __GameERC721_init(address governor_,string memory name, string memory symbol,address admin_) external initializer {
		__Governable_init_unchained(governor_);
        __Context_init_unchained();
        __ERC165_init_unchained();
        __GameERC721_init_unchained(name, symbol,admin_);
        maxId=0;
    }

    function __GameERC721_init_unchained(string memory name, string memory symbol,address admin_)  public governance {
        __ERC721_init_unchained(name,symbol);
        admin = admin_;
    }
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view override returns (bool) {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        address owner = ownerOf(tokenId);
        return (spender == owner || spender == admin|| getApproved(tokenId) == spender || isApprovedForAll(owner, spender));
    }
    

    function mintAuto(address to) public virtual {
...
// File: NftGame/contracts/IERC20.sol
...
//contract GameMain is Governable,ContextUpgradeSafe,IERC721Receiver {
contract GameMain is Governable,ContextUpgradeSafe,IERC721Receiver {    
    using SafeMath for uint256;
   ...
   function __GameMain_init(address governor_) external initializer {
		__Governable_init_unchained(governor_);
        __Context_init_unchained();
        __GameMain_init_unchained();
    }

    function __GameMain_init_unchained()  public governance{
        //tokens = tokens_;
    }
    ...
    function setHelmet(address _helmet) public governance{
        helmet = _helmet;
    }
    
    function setComToken(address _comToken) public governance{
        comToken = _comToken;
    }
    ...
    function transferNFTGovernorship(address newGovernor) public governance {
        for (uint i=0;i<8;i++){
            GameERC721 NFT = GameERC721(tokens[i]);
            NFT.transferGovernorship(newGovernor);
        }
    ...
    function testCreateNFT() public governance {
        ...
        GameERC721 NFT = new GameERC721();
        if (index==0)
            NFT.__GameERC721_init(address(this),"NFT0","NFT0",address(this));
        else if (index==1)
            NFT.__GameERC721_init(address(this),"NFT1","NFT1",address(this));
        else if (index==2)
            NFT.__GameERC721_init(address(this),"NFT2","NFT2",address(this));
        else if (index==3)
            NFT.__GameERC721_init(address(this),"NFT3","NFT3",address(this));
        else if (index==4)
            NFT.__GameERC721_init(address(this),"NFT4","NFT4",address(this));
        else if (index==5)
            NFT.__GameERC721_init(address(this),"NFT5","NFT5",address(this));
        else if (index==6)
            NFT.__GameERC721_init(address(this),"NFT6","NFT6",address(this));
        else if (index==7)
            NFT.__GameERC721_init(address(this),"NFT7","NFT7",address(this));
        else if (index==100)
            NFT.__GameERC721_init(address(this),"comNFT","NCom",address(this));
        if (index<8)
    ...
    function bet10() public {
        require(redeemTime > now, "bet closed");   
        require(!needClaim10(),"claim10 first");
    ...
    function needClaim() public view  returns (bool){
        uint betBlock = infos[msg.sender].betBlock;
        return (block.number < betBlock+256 && betBlock>0 );   
    }

    function needClaim10() public view  returns (bool){
        uint bet10Block = infos[msg.sender].bet10Block;
        return (block.number < bet10Block+256 && bet10Block>0 );   
    }
    
    function calcNFT(uint hash) public view returns (address) {
        uint pos = hash%100+1;
        if (pos<=25)
            return tokens[7];
        else if (pos<=43)
    ...
    function claim() public returns (address){
        require(redeemTime > now, "claim closed");   
        uint betBlock = infos[msg.sender].betBlock;
        require(block.number < betBlock+256 , "claim expired");   
        uint hash = uint(keccak256(abi.encodePacked(infos[msg.sender].betNum,blockhash(betBlock))));
        address nftAddr = calcNFT(hash);
        GameERC721 NFT = GameERC721(nftAddr);
        NFT.mintAuto(msg.sender);
        //IERC721(token0).safeTransferFrom(address(this),sender, tokenId0);
        infos[msg.sender].betBlock = 0;
        return nftAddr;
    }
    
    function claim10() public returns(address[] memory){
        require(redeemTime > now, "claim10 closed");   
        uint betBlock = infos[msg.sender].betBlock;
        address[] memory NftAry =new address[](10);  
        require(block.number < betBlock+256 , "claim expired");   
        for (uint i=0;i<10;i++){
            uint hash = uint(keccak256(abi.encodePacked(infos[msg.sender].bet10Num*10+i,blockhash(betBlock))));
            address nftAddr = calcNFT(hash);
            GameERC721 NFT = GameERC721(nftAddr);
            NFT.mintAuto(msg.sender);
            NftAry[i] = nftAddr;
   ...
   function ComposeEnable() public view  returns (bool){
        if (now >redeemTime)
            return false;
        bool ret = true;
        for (uint i=0;i<8;i++){
   ...
   function redeemState() public view  returns (bool redeemTimeEnable, bool comTokenEnable){
        if (redeemTime > now)
            redeemTimeEnable = false;
        else
            redeemTimeEnable = true;
    ...
    function redeem() public{
            ...
            uint256 amount = IERC20(helmet).balanceOf(address(this)).mul(num).div(totalNFT);
            TransferHelper.safeTransferFrom(helmet, address(this),msg.sender, amount);
        }

    function withdraw(address receive_,uint256 amount) public governance{
        TransferHelper.safeTransferFrom(helmet, address(this),receive_, amount);
    }
        ...
        isPoolNotClosed(poolId)
    {
        ...
        if (pool.token1 == address(0)){
            ...
            TransferHelper.safeTransferFrom(pool.token1, sender, address(this), pool.amount1);
        }
        ...
        isPoolClosed(poolId)
    {
        ...
        //none player
        if (pool.curPlayer==0){
            IERC721(pool.token0).safeTransferFrom(address(this),sender, pool.tokenId0);
            pools[index].claim = true;
        ...
        if (pool.token1 == address(0)){
            //require(ethAmount1>=pool.amount1,"The bet amount is too low");
            TransferHelper.safeTransferETH(sender,pool.amount1);
        }
        else{
            TransferHelper.safeTransfer(pool.token1,sender, pool.amount1);
        }
        pools[poolId-1].claim = true;
        emit Claimed(sender, poolId);
    }
    ...
    function isWinner(uint poolId,address sender) public view returns (bool) {
        require(pools[poolId-1].closeAt < now, "It's not time to start the prize");
         if (allPlayer[poolId][sender] == 0)
             return false;
         if (pools[poolId-1].curPlayer==1) //one one player
            return true; 
         uint index = poolsExt[poolId].lastHash%pools[poolId-1].curPlayer+1;
         if (index == allPlayer[poolId][sender])
            return true;
        else
            return false;
    }
    ...
    modifier isPoolNotClosed(uint id) {
        require(pools[id-1].closeAt > now, "this pool is closed");
        _;
    }
