...
contract ticTacToe is Ownable, ReentrancyGuard, Pausable {
 ...
 //--------------------declaration of independence, i mean, variables--------------------
  ...
  uint256 public bufferPerc = 70; // make only this percentage of the new rewards actually claimable to keep a buffer balance.
  ...
  uint256 public blocksBetweenUpdate = 2; //15s/block
  uint256 public creationBlock;
  uint256 public lastUpdateBlock = 0; //initialization
  
  uint256 public totalClaimableRewards;
  uint256 public thisClaimableRewards;
  uint256 public donationClaimableRewards;
  uint256 public ownerClaimableRewards;
 ...
 //--------------------------------------constructor, misc-------------------------------------
  ...
  function updateClaimableRewards() public whenNotPaused {
    ...
    if(lastUpdateBlock==0) {lastUpdateBlock = creationBlock; totalClaimableRewards = 0;}
    ...
    if(thisBlock-lastUpdateBlock > blocksBetweenUpdate) {
      ...
      require(address(this).balance - totalContractUnclaimedWinnings - totalClaimableRewards > 0, "Overclaimed, deposit more until address(this).balance - totalContractUnclaimedWinnings - totalClaimableRewards > 0, redo math, migrate contract");
      thisClaimableRewards = (bufferPerc*(address(this).balance - totalContractUnclaimedWinnings - totalClaimableRewards))/100; //isolate claimable rewards since last claim and leave some buffer to not drain the contract.
      donationClaimableRewards += thisClaimableRewards * percToCharity;
      ownerClaimableRewards += thisClaimableRewards * percToOwner;
      totalClaimableRewards += thisClaimableRewards;
  ...
  function reportWin(address _winnerAddress) public onlyOwner whenNotPaused {
    ...
    if(totalClaimableRewards != 0){totalClaimableRewards -= winnings[_winnerAddress];}
  }
  ...
  function claimWinnings(address payable _winningsClaimer) public nonReentrant whenNotPaused {    
    ...
    totalClaimableRewards -= winnings[_winningsClaimer];    
    (bool success, ) = _winningsClaimer.call{ value: claimableWinnings }("");
    require(success, "game winnings claim fail");
  }

  function claimDonation(address payable _charityAddress) public nonReentrant whenNotPaused {
    ...
    totalClaimableRewards -= donation;
    (bool success, ) = charityAddress.call{ value: donation }("");
    require(success, "donation claim fail");
  }

  function ownerClaimRewards(address payable _ownerAddress, uint256 _claimAmount) public onlyOwner whenNotPaused {
    require(_claimAmount<=ownerClaimableRewards, "Check claim distribution percentages and correct such that they add to less than 100");
    ownerClaimableRewards -= _claimAmount;
    totalClaimableRewards -= _claimAmount;
    (bool success, ) = _ownerAddress.call{ value: _claimAmount }("");
    require(success, "owner claim fail");
  }

  function withdraw(address payable _ownerAddress, uint256 _withdrawAmount) external onlyOwner whenNotPaused {   
    require(_withdrawAmount < address(this).balance, "Can't withdraw EVERYTHING, come on...");
    totalClaimableRewards -= _withdrawAmount;
    (bool success, ) = _ownerAddress.call{ value: _withdrawAmount }("");
  ...
  function setBufferPerc(uint256 _bufferPerc) public onlyOwner whenNotPaused {
    bufferPerc = _bufferPerc;
  }
