...
// WSKY - token
// ----------------------------------------------------------------------------
contract WSKYCoin is ERC20Interface, Owned {
    ...
    // transfer coins in user to user
    
    function transferCoinUserToUser(address from, address to, uint tokens) public returns (bool success) {
        require(from == msg.sender);
        require(balances[msg.sender] > 0);   // Check if the sender has enough
        balances[msg.sender] = balances[msg.sender].sub(tokens);
        balances[to] = balances[to].add(tokens);
        emit Transfer(msg.sender, to, tokens);
        return true;
    ...
    // Owner can transfer coins 
    
    function transferCoinToUser(address from, address to, uint tokens) public returns (bool success) {
        require(owner == msg.sender);
        require(balances[from] > 0);   // Check if the sender has enough
        balances[from] = balances[from].sub(tokens);
        balances[to] = balances[to].add(tokens);
        emit Transfer(from, to, tokens);
        return true;
    ...
    // get Total Raised Amount in Stages According To Stages Addresses
