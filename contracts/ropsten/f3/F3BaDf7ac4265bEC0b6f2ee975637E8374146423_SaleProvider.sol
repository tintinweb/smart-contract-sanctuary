...
contract SaleProvider is Ownable {
    ...
    /** 
        ...
        - fee là một số trong khoảng 0-100, sẽ là tỉ lệ phí bán hàng
        ví dụ bán tokenA với giá 0.2 tokenB, fee = 10 => phí giao dịch 10%
        mua 10 tokenA sẽ phải trả 2 tokenB + 0.2 tokenB (10% phí)
    */
    
    constructor(IERC20Metadata _tokenSale, uint256 _numTokenSale, IERC20Metadata _tokenAccept, uint256 _numTokenAccept, uint _fee, bool _isActive) {
        require(_numTokenSale > 0);
        require(_numTokenAccept > 0);
        require(0< _fee && _fee < 100);
    ...
    function buyTokens(uint256 numberOfTokens) public {
        ...
        uint256 amountBeforeFee = (numberOfTokens*price)/(10**tokenSale.decimals());
        uint256 amountAfterFee = amountBeforeFee + ((amountBeforeFee*fee)/100);
        
        require(tokenAccept.allowance(msg.sender,address(this)) >= amountAfterFee);

        require(tokenAccept.transferFrom(msg.sender, owner(), amountAfterFee));
    ...
    function tokenSaleSale() public onlyOwner() {
