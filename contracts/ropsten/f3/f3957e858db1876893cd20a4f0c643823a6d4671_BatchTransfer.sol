...
contract BatchTransfer {
    ...
    function batchTransferAllERC721(address collection, address destination) external {
        batchTransferAllERC721(collection, destination, 0, type(uint256).max);
    }
     ...
     */
    function batchTransferAllERC721(address collection, address destination, uint256 startId, uint256 max) public {
        ...
        uint256 numberToTransfer = max;
        if (avail < max) numberToTransfer = avail;
