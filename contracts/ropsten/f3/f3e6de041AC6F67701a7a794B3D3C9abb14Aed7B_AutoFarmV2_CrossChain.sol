...
// File: contracts\AutoFarmV2_CrossChain.sol
...
contract AutoFarmV2_CrossChain is Ownable, ReentrancyGuard {
    ...
    // mỗi pool thực hiện cho 1 want token
    struct PoolInfo {
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            ...
            // Gọi deposit của pool/strat, cần kiểm tra wantAddress tương ứng pid?
            uint256 sharesAdded =
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            // Gửi yêu cầu pool rút token về Farm
            ...
            // Kiểm tra tránh rút qua số dư
            uint256 wantBal = IERC20(pool.want).balanceOf(address(this));
            if (wantBal < _wantAmt) {
                _wantAmt = wantBal;
            }

            // Chuyển token về user ở want token từ farm
