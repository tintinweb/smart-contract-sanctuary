...
contract LP_Staking {
  ...
  function get_current_epoch_number() public view returns(uint8){
    uint8 result = uint8((block.timestamp - staking_start) / epoch_seconds);
  ...
  function get_available_reward(address account) public view returns(uint256){
    ...
    Epoch memory prev_epoch = epochs[epochs.length-2];

    if(user.last_epoch_withdrawn == current_epoch_number){
      //not enough time has past to seal an epoch so the reward MUST be zero
      return 0;
    }

    //do latest sealed epoch and back
    epoch_span = (current_epoch.epoch_number - prev_epoch.epoch_number) -1; //(to exclude current)
    ...
    for(uint8 epoch_idx = uint8(epochs.length-2); epoch_idx > 0; epoch_idx--){
