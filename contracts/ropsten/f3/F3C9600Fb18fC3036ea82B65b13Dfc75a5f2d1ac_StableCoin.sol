...
/**
 * @dev Interface of the Stablecoin implementation.
 */

interface IStableCoin {
    
    // The event emitted when the auditor is replaced
    event ChangeAuditor(address newauditor);

    // The event emitted when the custodial is replaced
    event ChangeCustodial(address newcustodial);

    // The event emitted when contract is initialized
    event Constructed(
        string tokenName,
        string tokenSymbol,
        address custodial,
    ...
    // The event emitted when the tokens are wiped
    event Wipe(address account, uint256 amount);

    // The event emitted when the tokens are minted
    event Mint(address account, uint256 amount);

    // The event emitted when the tokens are burned
    ...
    /**
     * @dev Replace the custodial address
     */
    function replaceCustodial(address newCustodial) external;

    /**
     * @dev Replace the auditor address
     */
    function replaceAuditor(address newAuditor) external;
