...
function find_crafter_ready(uint8 _base_type, uint8 _item_type,uint _start) external view returns(uint _summonerId,uint _next) {
        address _inner=address(this);
        uint _len = _rm.balanceOf(_inner);

        for(uint _i=_start;_i<_len;_i++ ){
            uint _id=_rm.tokenOfOwnerByIndex(_inner,_i);
            (bool _crafted,,,)= _crafting.simulate(_id, _base_type, _item_type, 0);
            if(_crafted){
                return (_id,_i+1);
            }
        }
    ...
    function _find_crafter_ready(uint8 _base_type, uint8 _item_type,uint _start) internal returns(uint _summonerId,uint _next) {
        address _inner=address(this);
        uint _len = _rm.balanceOf(_inner);
    ...
    function craft_batch(uint8 _base_type, uint8 _item_type,uint _maxCount) external{
        ...
        for(uint i=0;i<_maxCount;i++){
            (_crafter,_next)=_find_crafter_ready(_base_type,_item_type,_next);
            if(_next==0) break;
            _crafting.craft(_crafter, _base_type, _item_type, 0);
        }
    }
    ...
    function gold_approve_all(uint _to,uint _val, uint _start,uint _count) external {
        require(_owner==msg.sender);
    ...
    //批量提取金币
    function gold_claim_all(uint _class,uint _maxCount) external {
        require(_owner==msg.sender);
        if(_maxCount==0) _maxCount=20; //默认一次搞20个
    ...
    function summon_batch(uint _class,uint _count) external{
        ...
        for (uint i = 0; i < _count; i++) {
            ...
            _gold.approve(_summoner,_crafting.SUMMMONER_ID(),1000000e18);
        }
    }
    //批量招募-制造者
    function summon_crafts(uint _class,uint _count) external{
        require(_owner==msg.sender);
