...
contract WojakFarming is Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	...
	function withdraw_all_LP(uint256 _pid) public nonReentrant {
		UserList storage user = userList[_pid][msg.sender];
	    withdraw_LP(_pid, user.stakingAmount);
	}
	...
	function user_blocks_before_claim(uint _pid, address _address) public view returns (uint256) {
    	return get_user_blockstart(_pid,_address) + 100 - block.number;
	}
	
	function get_blocks_before_claim(uint _pid) public view returns (uint256) {
