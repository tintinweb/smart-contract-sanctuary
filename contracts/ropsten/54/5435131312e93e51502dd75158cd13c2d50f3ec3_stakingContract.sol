...
contract stakingContract is owned {
    ...
    */
    constructor() public {
        tswap = IERC20(0x8CfDaeb56Ebb87229d311109Cf02Be3aAE57eb3C);
        poolDuration = 30 days;
    }
    
    //Stake function for users to stake SWAP token
    function stake(uint256 _poolId, uint256 amount) public {
        require(stakingPool[_poolId].tokenAddress != address(0), "Invalid pool id");
    ...
    function WithdrawStaking(uint256 _poolId, uint256 amount) public {
        ...
        require(usersStaking[_poolId][msg.sender] >= amount, "You are trying to withdaw more than your stake");
        require(now < stakingPool[_poolId].poolEndTime, "Staking pool is closed"); //staking pool is closed for staking
    ...
    function calculateRewardTesting(uint256 _poolId, address _userAddress) public view returns (uint256 _percnt, uint256 _diff, uint256 _userRewardPerSecond) {
        ...
        uint256 duration = stakingPool[_poolId].poolEndTime.sub(stakingPool[_poolId].poolStartTime);
        uint256 userRewardPerSecond = totalRewards.div(duration).mul(percnt).div(100000000);   
    ...
    function withdrawRewards(uint256 _poolId) public {
        ...
        IERC20(stakingPool[_poolId].tokenAddress).transfer(msg.sender, amount);
    ...
    function createPool(address _tokenAddress, uint256 _tokenAmount) public onlyOwner {
        ...
        stakingPool[_id].poolStartTime = now;
        stakingPool[_id].poolEndTime = now + poolDuration;
    ...
    function getTotalRewardsWithdrawnInPool(uint256 _poolId) public constant returns ( uint256 _totalWithdrawn) {
        return stakingPool[_poolId].rewardsWithdrawn;
    }
    
    //get total SWAP token staked in the contract
    function getTotalStaked() public constant returns ( uint256 _totalStaked) {
        return totalStaked;
    }
    
    //get total SWAP token staked by pool id
    function getTotalStakedByPoolId(uint256 _poolId) public constant returns ( uint256 _totalStaked) {
    ...
    //get all pool ids
    function getAllPoolIds() public view returns (uint256[] _allPoolIds) {
        return allPoolIds;
    }
