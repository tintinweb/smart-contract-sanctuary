...
interface DystoTHEMOONMONSTERs {
    function tokensOfOwner(address ownwer) external view returns (uint256[] memory);

}

contract MOONSTER is ERC20, ERC20Burnable, Ownable {

    address constant public DystoAddress = 0x8C31Fba2d3Fd185Ec62F56b2dDe00923AA333df3;
    ...
    constructor() ERC20("Monster", "MOON") {}
    ...
    function setSaleState(bool newState) public onlyOwner {
     ...
     function totalAvailable(address ownwer) public view returns (uint256) {
        uint[] memory THEMOONMONSTERs = DystoTHEMOONMONSTERs(DystoAddress).tokensOfOwner(ownwer);
        uint arrayLength = THEMOONMONSTERs.length;
        uint256 rewards = 0;
        uint256 time = block.timestamp;
        for (uint i=0; i<arrayLength; i++) {
              uint256 THEMOONMONSTERid = THEMOONMONSTERs[i];
              uint256 lastTime = lastUpdate[THEMOONMONSTERid];
              if (lastTime > 0){
    ...
    function updateTime(address ownwer) private {
        ...
        for (uint i=0; i<arrayLength; i++) {
              uint256 THEMOONMONSTERid = THEMOONMONSTERs[i];
              lastUpdate[THEMOONMONSTERid]=time;
    ...
    function claim() public payable  {
        ...
        uint[] memory THEMOONMONSTERs = DystoTHEMOONMONSTERs(DystoAddress).tokensOfOwner(msg.sender);
        uint arrayLength = THEMOONMONSTERs.length;
    ...
    function reservedMonster(address to, uint256 amount) public onlyOwner {
         require(claimed + amount <= MAX_CLAIM, "Value exceeds the max limit");
         _mint(to, amount);
         claimed += amount;
    }

    function unclaimedMonster() public view onlyOwner returns(uint256) {
         uint256 unclaimed = MAX_CLAIM - claimed;
         return unclaimed;
    }

    function THEMOONMONSTERMonster(uint THEMOONMONSTERid) public view returns(uint256) {
         uint256 rewards = 0;
         uint256 time = block.timestamp;
         uint256 lastTime = lastUpdate[THEMOONMONSTERid];
         if (lastTime > 0){
