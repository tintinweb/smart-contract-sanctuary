...
abstract contract MamaCocoNoBSSupport is NoBSAdvSupport {
    ...
    function getRewardType() external view returns (string memory reflector0, string memory reflector1) {
        reflector0 = noBSReflectors[0].rewardCurrency();
        reflector1 = noBSReflectors[1].rewardCurrency();
    }

    function getUnpaidEarnings() external view returns (uint256 reflector0, uint256 reflector1) {
        reflector0 = noBSReflectors[0].getUnpaidEarnings(_msgSender());
        reflector1 = noBSReflectors[1].getUnpaidEarnings(_msgSender());
    }

    function getShares() external view returns (uint256 reflector0Amount, uint256 reflector0TotalExcluded, uint256 reflector0TotalRealised, uint256 reflector1Amount, uint256 reflector1TotalExcluded, uint256 reflector1TotalRealised) {
        (reflector0Amount, reflector0TotalExcluded, reflector0TotalRealised) = noBSReflectors[0].getShares(_msgSender());
        (reflector1Amount, reflector1TotalExcluded, reflector1TotalRealised) = noBSReflectors[1].getShares(_msgSender());
    }
