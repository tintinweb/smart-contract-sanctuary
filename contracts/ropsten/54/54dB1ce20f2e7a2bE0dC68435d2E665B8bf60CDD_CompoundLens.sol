...
//import "../../Core/LendingNetwork/OTokens/CErc20.sol";
...
import "../RainMaker/RainMaker.sol";
...
//    function oracle() external view returns (PriceOracle);
    function getOracleForCToken(address cToken) view external returns (PriceOracle);
...
//    function compSpeeds(address) external view returns (uint);
    function rainMaker() external view returns (RainMaker);
...
contract CompoundLens {
    ...
    function cTokenMetadata(CToken cToken) public returns (CTokenMetadata memory) {
        ...
        CToken cToken = CToken(address(cToken));
    ...
    function cTokenUnderlyingPrice(CToken cToken) public returns (CTokenUnderlyingPrice memory) {
        ComptrollerLensInterface comptroller = ComptrollerLensInterface(address(cToken.comptroller()));
        PriceOracle priceOracle = comptroller.getOracleForCToken(address(cToken));
...
//            delegate: comp.delegates(account),
    ...
    function accountBalance(address payable  account) public view returns (uint) {
