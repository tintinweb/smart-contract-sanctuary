...
abstract contract NFT is AdminOps, ERC721Enumerable, Payable {
  ...
  string public baseURI = ""; //-> could have been "https://monument.app/artefacts/"
...
abstract contract Taxes {
  ...
  event TaxesChanged (
    uint256 newTaxOnBuildingMonument,
    uint256 newTaxOnMintingAnArtifact,
    address indexed actionedBy
  );

  uint256 public taxOnBuildingMonument = 0; // `26 * (10 ** 13)` was around $1 in Oct 2021
  uint256 public taxOnMintingAnArtifact = 0;

  /// @notice To set new taxes for Building and Minting
  /// @param _onBuildingMonument Tax in wei, for building a Monument.
  /// @param _onMintingArtifact Tax in wei, for minting an Artifact.
  function setTaxes(uint256 _onBuildingMonument, uint256 _onMintingArtifact)
    public
    returns (uint256, uint256)
  {
    permissionManagement.adminOnlyMethod(msg.sender);

    taxOnBuildingMonument = _onBuildingMonument;
    ...
    return (_onBuildingMonument, _onMintingArtifact);
  }

  /// @notice Taxes are sent to the Beneficiary
  function _chargeMonumentTax()
    internal
    returns (bool)
  {
    require(
      msg.value >= taxOnBuildingMonument || 
    ...
    if (msg.value >= taxOnMintingAnArtifact) {
      (bool success, ) = permissionManagement.beneficiary().call{value: taxOnBuildingMonument}("");
...
/// @author [email protected]
/// @notice This contract shall be deployed everytime a user mints an artefact. This contract will manage the split sharing of royalty fee that it receives.
contract Splits {
    ...
    /// @dev Takes in the array of Splitters and Percentages, fills the storage, to set the Split rules accordingly.
    /// @param _splitters An array of addresses that shall be entitled to some percentage share of the total royalty supplied to the contract, from the market, preferrably.
    /// @param _percentages An array of numbers that represent percentages, all its elements must add up to a total of 100, and must be in order of splitters supplied during construction of the contract.
    constructor(address[] memory _splitters, uint256[] memory _percentages) {
        require(_splitters.length == _percentages.length);

        uint256 _totalPercentage;

        for (uint256 i = 0; i < _splitters.length; i++) {
            require(_splitters[i] != address(0));
            require(_percentages[i] > 0);
            require(_percentages[i] <= 100);
        ...
        for (uint256 i = 0; i < _splitters.length; i++) {
            royalties[_splitters[i]] = _percentages[i];
        }
    ...
    /// @notice This is a payable function that distributes whatever amount it gets, to all the addresses in the splitters array, according to their royalty percentage share set in royalties mapping.
    function distributeFunds() public payable {
        ...
        for (uint256 i = 0; i < splitters.length; i++) {
            payable(splitters[i]).transfer((msg.value / 100) * percentages[i]);
        }
    }

    /// @notice Takes in an address and returns how much percentage share of the total royalty the address was originally entitled to.
    /// @param _address Address whose royalty precentage share information to fetch.
    /// @return uint256 - Percentage Royalty the address was originally entitled to.
    function royaltySplitInfo(address _address) public view returns (uint256) {
        uint256 royaltyPercentage = royalties[_address];
...
abstract contract Payable {
  ...
  function sendToBeneficiary(uint256 _amountInWei) public returns(uint256) {
    permissionManagement.adminOnlyMethod(msg.sender);
  ...
  function sendERC20ToBeneficiary(address _erc20address, uint256 _tokenAmount) public returns(address, uint256) {
    permissionManagement.adminOnlyMethod(msg.sender);
  ...
  function sendERC721ToBeneficiary(address _erc721address, uint256 _tokenId) public returns(address, uint256) {
    permissionManagement.adminOnlyMethod(msg.sender);
