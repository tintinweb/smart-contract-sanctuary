...
contract ClaimingRegistry is IClaimingRegistry, Initializable, AbstractDependant {
    ...
    uint256 internal constant ANONYMOUS_VOTING_DURATION_CONTRACT = 1 days; // 1 weeks
    uint256 internal constant ANONYMOUS_VOTING_DURATION_EXCHANGE = 2 days; // 30 days
    
    uint256 internal constant EXPOSE_VOTE_DURATION = 1 days; // 1 weeks
    uint256 internal constant PRIVATE_CLAIM_DURATION = 1 hours; // 3 days
        ...
        returns (uint256 _newClaimIndex) 
    {
        ...
        uint256 claimAmount = IPolicyBook(policyBookAddress).userStats(claimer).coverTokens;
