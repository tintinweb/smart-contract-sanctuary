...
contract Marketplace is Ownable, ReentrancyGuard, IERC1155Receiver {

    address constant SPC_TOKEN_ADDR = address(0x00a2f017966d967ec697c7a20cf9d0b43cb8d4ff1d);
    ...
    constructor(address _addressNFT, address _addressSPC) {
        CustomNFT = NFTCollection(_addressNFT);
        SPCToken = IERC20(_addressSPC);
        mkOwner = msg.sender;
        allNFTCounter = 0;
    }
    ...
    function mintSpectraNFT(string memory _tokenHash, uint256 auctionInterval, uint256 _price, uint24 royaltyValue, uint8 kind) external payable nonReentrant onlyAdmin {
        ...
        CustomNFT.mintNFT(msg.sender, allNFTCounter, 1, "");
        ...
        _allNFTInfo[allNFTCounter] = NFTInfo(allNFTCounter, _tokenHash, msg.sender, msg.sender, msg.sender, _price, block.timestamp, auctionInterval, kind, royaltyValue, false);
        allNFTCounter++;
    ...
    function mintBatchSpectraNFT(string[] calldata _tokenHash, uint256 _price, uint24 royaltyValue, uint8 kind) external payable nonReentrant onlyAdmin {
        ...
        for (uint256 i = 0; i < _tokenHash.length; i++) {
          ...
          _allNFTInfo[allNFTCounter] = NFTInfo(allNFTCounter, _tokenHash[i], msg.sender, msg.sender, msg.sender, _price, block.timestamp, 0, kind, royaltyValue, false);   
          allNFTCounter++;
        //   putNFT2Marketplace(_tokenHash[i], 0, _price, kind);
        }

        CustomNFT.mintBatchNFT(msg.sender, arrayTokenId, arrayTokenAmount, "");
    ...
    function putNFT2Marketplace(string memory _tokenHash, uint256 auctionInterval, uint256 price, uint8 kind) public onlyNFTOwner(_tokenHash) {
        ...
        CustomNFT.safeTransferFromWithProvision(msg.sender, address(this), _getTokenIdFromHash[_tokenHash], 1);
    }
    ...
    function transferNFT(string memory _tokenHash, uint8 kind) external nonReentrant {
        require(_tokenHashExists[_tokenHash], "Non-Existing NFT hash value....");
        require(kind >= 0, "Invalid cryptocurrency...");
        ...
        CustomNFT.safeTransferFromWithProvision(address(this), lastBidder, indexOfToken, 1);
