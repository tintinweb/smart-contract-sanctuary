...
contract HardStakingNFTAuction is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    event RescueAuctionToken(address indexed to, uint auctionId, uint tokenId);
    ...
    function rescueUnbiddenToken(uint auctionId) public { 
        ...
        emit RescueAuctionToken(msg.sender, auctionId, auction.tokenId);
    }
    ...
    function _burnTokenForStakeToken(uint amount) private returns (uint) {
        ...
        if (bidAuctionFeePercentage != 0) {
            uint feeAmount = amount.mul(bidAuctionFeePercentage) / 100;
            burnToken.transfer(address(auctionFeeManager), feeAmount);
            auctionFeeManager.processFee(msg.sender, feeAmount);
    ...
    function changeAuctionEnd(uint auctionId, uint deadline) external {
        require(auctionId <= lastAuctionId, "HardStakingNFTAuction: No such auction Id");
