...
contract DirtyFarm is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (_pid == 1) { //$Dirty tokens
                ...
                require(_amount <= 2100000000000000000000000000, "You cannot stake more than 2.1 Billion $Dirty");
                ...
                require(_amount <= 10000000000000000000000, "You cannot stake more than 10k LP Tokens");
                pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
                pool.runningTotal = pool.runningTotal.add(_amount);
                user.amount = user.amount.add(_amount);
            }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (_pid == 1) { //$Dirty tokens
                ...
                uint256 totalRewards = lpSupply.sub(pool.runningTotal); //get amount of user original deposit
                if (totalRewards == 0) {
                ...
                if (totalRewards > 0) { //include reflection
                    uint256 percentRewards = _amount.div(pool.runningTotal).mul(100); //get % of share out of 100
                    uint256 reflectAmount = percentRewards.mul(totalRewards).div(100); //get % of reflect amount
                    pool.runningTotal = pool.runningTotal.sub(_amount);
                    _amount = _amount.add(reflectAmount);
                    pool.lpToken.safeTransfer(address(msg.sender), _amount);
                    emit Withdraw(msg.sender, _pid, _amount);
                }               
    ...
    function getRunningTotal(uint256 _pid) external view returns (uint256) { 
         PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdrawRewardsOnly(uint256 _pid) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accDirtyPerShare).div(1e12);
        emit WithdrawRewardsOnly(msg.sender, _pid, pending);
    }
