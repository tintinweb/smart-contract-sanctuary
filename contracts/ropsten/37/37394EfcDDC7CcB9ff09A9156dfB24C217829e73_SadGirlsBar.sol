...
contract SadGirlsBar is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  uint public constant MAX_GIVEAWAY_TOKENS = 666;
  ...
  uint internal nextGiveawayTokenId = 0;
  uint internal nextTokenId = 666;
  ...
  function mint(uint256 numTokens) external payable nonReentrant {
    ...
    require(numTokens > 0 && numTokens <= 100, "Must mint from 1 to 100 NFTs");
    // ...and not try to get more tokens than TOTALLY allowed...
    require(totalSupply().add(numTokens) <= MAX_TOKENS, "Can't get more than 10000 NFTs");
  ...
  for thanks to people who support this project */
  function mintGiveaway(uint256 numTokens) public onlyOwner {
    require(nextGiveawayTokenId < MAX_GIVEAWAY_TOKENS);
    require(nextGiveawayTokenId.add(numTokens) < MAX_GIVEAWAY_TOKENS, "Only 666 max");
    uint256 index;
    // Reserved for people who helped this project and giveaways
    for (index = 0; index < numTokens; index++) {
      nextGiveawayTokenId++;
      _safeMint(owner(), nextGiveawayTokenId);
  ...
  function getMintedCount() public view returns(uint) {
    return nextTokenId.sub(MAX_GIVEAWAY_TOKENS).add(nextGiveawayTokenId);
  }
