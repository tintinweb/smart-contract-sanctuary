...
contract PuppyNFT is Context, ERC721 {
    ...
    function setSaleDetails(uint256 puppyId, uint256 sellingPrice, address operator) public {
        require(
            _puppyDetails[puppyId].purchasePrice*2 > sellingPrice,
            "Re-selling price is more than 200%"
        );
...
contract Market {
    ...
    constructor(Token token, CrateNFT crate, PuppyNFT puppy) {
        _token = token;
        _crate = crate;
		_puppy = puppy;
    ...
    function stimulateCrate(uint256 crateId, uint256 dogType) public {
        ...
        _puppy.transferPuppy(seller, dogType);
    }
	
	// Purchase the dog from the secondary market hosted by organiser
    function secondaryPurchase(uint256 dogId) public {
        address seller;
        uint sellingPrice;
