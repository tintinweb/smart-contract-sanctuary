...
contract TokenSale is Ownable {
    ...
    IERC20 public tokenAccecpt;
    uint256 public price;         
    bool public isActive = false;

    event Sold(address buyer, uint256 amount);

    constructor(IERC20 _tokenSale, uint256 _price, bool _isActive, IERC20 _tokenAccecpt) {
        require(_price > 0);

        tokenSale = _tokenSale;
        price = _price;
        tokenAccecpt = _tokenAccecpt;
        isActive = _isActive;
    }
    ...
    function buyTokens(uint256 numberOfTokens) public {
        ...
        require(tokenAccecpt.allowance(msg.sender,address(this)) >= numberOfTokens*price);

        require(tokenAccecpt.transferFrom(msg.sender, owner(), (numberOfTokens*price)));
