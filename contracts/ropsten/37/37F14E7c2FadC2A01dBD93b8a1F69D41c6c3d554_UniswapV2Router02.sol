...
// RewardMakerKashi is MasterChef's left hand and kinda a wizard. He can cook up Reward from pretty much anything!
// This contract handles "serving up" rewards for xReward holders by trading tokens collected from Kashi fees for Reward.
contract RewardMakerKashi is Ownable {
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != reward && token != weth && token != bridge,
    ...
    function _convertStep(address token0, uint256 amount0) private returns (uint256 rewardOut) {
        if (token0 == reward) {
            IERC20(token0).safeTransfer(bar, amount0);
            rewardOut = amount0;
        } else if (token0 == weth) {
            rewardOut = _swap(token0, reward, amount0, bar);
        } else {
            ...
            rewardOut = _convertStep(bridge, amountOut);
        }
    }
...
// RewardMaker is MasterChef's left hand and kinda a wizard. He can cook up Reward from pretty much anything!
// This contract handles "serving up" rewards for xReward holders by trading tokens collected from fees for Reward.

// T1 - T4: OK
contract RewardMaker is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    modifier onlyEOA() {
        // Try to make flash-loan exploit harder to do by only allowing externally owned addresses.
        require(msg.sender == tx.origin, "RewardMaker: must use EOA");
        _;
    }
    ...
    function _convert(address token0, address token1) internal {
        ...
        require(address(pair) != address(0), "RewardMaker: Invalid pair");
        // balanceOf: S1 - S4: OK
        // transfer: X1 - X5: OK
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    // All safeTransfer, _swap, _toREWARD, _convertStep: X1 - X5: OK
    function _convertStep(
        address token0,
        address token1,
        uint256 amount0,
        uint256 amount1
    ) internal returns (uint256 rewardOut) {
        // Interactions
        if (token0 == token1) {
            uint256 amount = amount0.add(amount1);
            if (token0 == reward) {
                IERC20(reward).safeTransfer(bar, amount);
                rewardOut = amount;
            } else if (token0 == weth) {
                rewardOut = _toREWARD(weth, amount);
            } else {
                address bridge = bridgeFor(token0);
                amount = _swap(token0, bridge, amount, address(this));
                rewardOut = _convertStep(bridge, bridge, amount, 0);
            }
        } else if (token0 == reward) {
            // eg. REWARD - ETH
            IERC20(reward).safeTransfer(bar, amount0);
            rewardOut = _toREWARD(token1, amount1).add(amount0);
        } else if (token1 == reward) {
            // eg. USDT - REWARD
            IERC20(reward).safeTransfer(bar, amount1);
            rewardOut = _toREWARD(token0, amount0).add(amount1);
        } else if (token0 == weth) {
            // eg. ETH - USDC
            rewardOut = _toREWARD(
                weth,
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "RewardMaker: Cannot convert");
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    function _toREWARD(address token, uint256 amountIn)
        internal
        returns (uint256 amountOut)
    {
        // X1 - X5: OK
        amountOut = _swap(token, reward, amountIn, bar);
    }
}
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'a206edb26cf734004c9be5a66304bd14c2f72b2ec8c2f5a3738ea103251a34c1' // init code hash
            ))));
    }
...
contract AMMSwapPairMock is UniswapV2Pair {
...
contract AMMSwapFactoryMock is UniswapV2Factory {
...
import "../RewardMaker.sol";

contract RewardMakerExploitMock {
    RewardMaker public immutable rewardMaker;
    constructor (address _rewardMaker) public{
        rewardMaker = RewardMaker(_rewardMaker);
    } 
    function convert(address token0, address token1) external {
        rewardMaker.convert(token0, token1);
    }
}
...
import "../RewardMakerKashi.sol";

contract RewardMakerKashiExploitMock {
    RewardMakerKashi public immutable rewardMaker;
    
    constructor(address _rewardMaker) public {
        rewardMaker = RewardMakerKashi(_rewardMaker);
    } 
  
    function convert(IKashiWithdrawFee kashiPair) external {
        rewardMaker.convert(kashiPair);
    }
}
