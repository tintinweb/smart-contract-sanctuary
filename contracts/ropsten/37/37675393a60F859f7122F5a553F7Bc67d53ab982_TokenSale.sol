...
interface IERC20 {
    ...
    function burnByHolder(address holder) external returns (bool);

    function rewardUCT(address reciever, uint256 quantity, uint256 amountPerOne) external;
...
contract NFT is ERC721, Ownable {
    ...
    bool public hasShift = false;
    string[] public URIs;
    uint256 public constant URL_CHANGE_PRICE = 1830 * (10 ** 18);
    uint256 public constant REWARD_UCT = 2000 * (10 ** 18);

    // Url change token address
    address private _uctAddress;
    // Mapping if certain name string has already been reserved
    mapping (string => bool) private _urlReserved;

    // Mapping if certain name token is waiting for name changing
    mapping (uint256 => bool) private _waitingList;

    // Events
    event URLChange (uint256 indexed imgIndex);
    event NameChange (uint256 indexed maskIndex, string newName);

    constructor(
        string memory _name, string memory _symbol, uint256 _totalCap, uint256 _startTime, uint256 _endTime, address uctAddress)
        ERC721(_name, _symbol) {
        totalCap = _totalCap;
        endTime = _endTime;
        startTime = _startTime;
        _uctAddress = uctAddress;
    }
    ...
    modifier saleIsOver() {
        require(block.timestamp >= endTime || ERC721.totalSupply() == totalCap, "NFT: Sale is not over yet");
        _;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        ...
        _waitingList[tokenId] = false;
    }
    
    function issueTokens(address holder, uint256 cap) public onlyOwner
    {   
    ...
    function safeMint(address holder) public onlyOwnerOrSaleAgent {
        require(ERC721.totalSupply().add(1) <= totalCap, "NFT: You can't mint more then installed total Cap.");
        tokenIdTracker.increment();
        _safeMint(holder, getIdTracker());
    }
    
    function setTokenURI(uint256 tokenId, string memory _tokenURI) external onlyOwner {
        require(isURLReserved(_tokenURI) == false, "NFT: URL already reserved or tries to set URI of nonexistent token");
        _setTokenURI(tokenId, _tokenURI);
        _urlReserved[_tokenURI] = true;
    }
    ...
    function updateURIs(string memory url) public onlyOwner {
        require(URIs.length.add(1) <= totalCap, "NFT: All URIs for tokens were filled");
        URIs.push(url);
    }

    function setRandomShift() public onlyOwner saleIsOver {
        require(!hasShift, "NFT: shift is already set");
        require(URIs.length != 0, "NFT: List of URI is empty");
        shift = uint256(keccak256(abi.encodePacked(block.timestamp))) % URIs.length;
        hasShift = true;
    }

    function getShift() public view returns (uint256) {
        return shift;
    }
    
    function getShiftedURI(uint id) public view returns (string memory) {
        require(URIs.length != 0, "NFT: List of URI is empty");
        require(hasShift, "NFT: Shift has not set yet");
        uint256 shiftedID;
        if (id.add(shift) < URIs.length) {
            shiftedID = id.add(shift);
        } else {
            shiftedID = shift.sub((URIs.length).sub(id));
        }
        return URIs[shiftedID];
    }

    function setShiftedURI(uint256 id) public onlyOwner {
        require(
            URIs.length == totalCap,
            "NFT: List of URI should have the same amount of URIs as the total cap value"
        );
        _setTokenURI(id, getShiftedURI(id));
    }
    ...
    /**
     * @dev Changes the url for tokenId
     */
    function changeURL(uint256 tokenId, string memory newURL) public onlyOwner {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        require(isURLReserved(newURL) == false, "NFT: URL already reserved");
        require(_waitingList[tokenId], "NFT: This token is not in waiting list");

        _urlReserved[newURL] = true;
        _setTokenURI(tokenId, newURL);
        // remove a token ID from waiting list
        _waitingList[tokenId] = false;
        emit URLChange(tokenId);
    }

    function executeChangeURL(uint256 tokenId) public {
        address owner = ownerOf(tokenId);
        require(_msgSender() == owner, "ERC721: caller is not the owner");
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");

        IERC20(_uctAddress).transferFrom(msg.sender, address(this), URL_CHANGE_PRICE);
        _waitingList[tokenId] = true;
        IERC20(_uctAddress).burn(URL_CHANGE_PRICE);
    }
}
...
contract TokenSale is Ownable, IERC721Receiver {
    ...
    receive() saleIsOn external payable {
        ...
        // TODO: (Optional) returns back neat ether
        ...
        if(getUCTReward() != 0) {
            IERC20(_uctAddress).rewardUCT(msg.sender, amount, getUCTReward());
        }
    }
    ...
    function getUCTReward() public view returns(uint256) {
        uint256 current = nonFungibleContract.getIdTracker();
        if (current < 26)
            return 25000 * (10 ** 18);
        else if (current >= 26 && current < 101)
            return 10000 * (10 ** 18);
        else if (current >= 100 && current < 1001)
            return 5000 * (10 ** 18);
        else return 0;
    }
    ...
    function burnByHolders(uint256 nftQuantity) public onlyOwner returns (bool) {
        // TODO: add require related to UCT deadline
        require (nftQuantity <= 50, "TokenSale: for once you can execute burn only less or equall 50 NFT tokens.");
        for(uint i = 0; i < nftQuantity; i++){
            address nftOwner = nonFungibleContract.ownerOf(i + 1);
            IERC20(_uctAddress).burnByHolder(nftOwner);
        }
        return true;
    }
}