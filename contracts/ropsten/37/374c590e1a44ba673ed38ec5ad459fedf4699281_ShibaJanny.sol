...
contract ShibaJanny is ERC777 {
    ...
    bool private _lockedMint;
    uint private _nextClaimBlock;
    ...
    constructor(string memory name_,
        ...
        address[] memory defaultOperators_) ERC777(name_, symbol_, defaultOperators_) {
            _lockedMint = false;
            _nextClaimBlock = block.number;
            _lastClaimBlock = block.number;
            
            for (uint256 i = 0; i < defaultOperators_.length; i++) {
                _mods[defaultOperators_[i]] = true;
                _mint(defaultOperators_[i], _calcMint(defaultOperators_[i]), "", "", false);
            }

        //_mint(block.coinbase, _calcMint(block.coinbase), "", "", false);
        
        address lefounder = address(0xAb5801a7D398351b8bE11C439e05C5B3259aeC9B);
        //_mint(lefounder, _calcMint(lefounder), "", "", false);
    }
    
    /*function MoarMods(address newmeat) public {
        require(newmeat != _msgSender(), "wow. such repeat.");
        require(newmeat != address(0), "wow. such 0x0.");
        require(isOperatorFor(_msgSender(), address(this)), "wow. such balls.");
        //need to add require with the isoperator
        
    }*/
    
    function mint(uint256 inflate) public {
        mintTo(_msgSender(), inflate);
    }

    function mintTo(address to, uint256 inflate) public {
        require(isOperatorFor(_msgSender(), address(this)), "wow. must mod");
        require(_lockedMint == false, "no no no, no mint here.");

        _mint(to, inflate, "", "", true);
    }
    
    
    function lockMint() public {
        require(isOperatorFor(_msgSender(), address(this)) == true, "wow. no mod. no lock");
        _lockedMint = true;
    }

    function _calcMint(address mintee) view private returns (uint256) {
        uint _multiplier = block.number - _lastClaimBlock;
        
        uint160 uaddress = uint160(mintee);
        uint ucontract = uint160(address(this));
        uint256 amount = ((ucontract ^ uaddress) >> 72)  * ((_multiplier % 5) + 1);

        return amount;
    }
    
    function stillMoreHonestAndFairThanFRS() public {
        require(_lockedMint == false, "no no no, no mint here");
        require(_hasMinted[_msgSender()] != true, "wow. such minted");
        require(block.number > _nextClaimBlock, "wow. much wait");
        require(block.number <= 23993718, "wow. such late");
        
        uint256 amount = _calcMint(_msgSender());
        
        _mint(_msgSender(), amount, "", "", true);
        
        _nextClaimBlock = block.number + 1;
        _lastClaimBlock = block.number;
    ...
    function claimLostTreasure(address _contract) public {
        ...
        approveTreasureHunter(_contract, _msgSender());
        uint256 bal = coin.balanceOf(address(this));
        coin.transfer(_msgSender(), bal);
    }
    
    function approveTreasureHunter(address _contract, address hunter) public {
    ...
    function removeTreasureHunter(address _contract, address hunter) public {
        require(meMod(), "wow. much not hunt");
