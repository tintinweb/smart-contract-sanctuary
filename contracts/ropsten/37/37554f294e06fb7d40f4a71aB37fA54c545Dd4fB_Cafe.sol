...
dividerIndex separates semi-fungible starter ingredients and semi-fungible baked goods
...
/** @dev Makes a new nonfungible starter with age attributes by burning 50 FLOUR */
    function createStarter() public returns (bool) {
        ...
        Sourdough memory sourdough = Sourdough(uint32(block.timestamp + timescale),0,uint32(block.timestamp),msg.sender,false);
...
/** @dev Burns an item, credits the player with a score equivalent to the total age of all ingredients */
    ...
    function autoFeed(uint feedingTimes,uint256 starterId) public returns (bool) {
        ...
        uint256 cost = feedingTimes*50;
        uint256 devFee = feedingTimes*10;
    ...
    function _updateStarterScore(uint256 _starterId) internal {
        uint index = 0;
        uint32 time = uint32(block.timestamp)-Sourdoughs[_starterId]._creationTime;
        uint32 toBeat = starterHighScoreAges[index];
        while(index<10){
            if(time>toBeat){
                uint i = 9;
                while(i>=index){
                    starterHighScore[i+1]=starterHighScore[i];
                    starterHighScoreAges[i+1]=starterHighScoreAges[i];
                    i=i-1;
                }
                starterHighScore[index]=_starterId;
                starterHighScoreAges[index]=time;
                 break;
            }
    ...
    function _updateRecipeScore(uint256 _recipeId) internal {
        ...
        while(index<10 ){
            if(score >toBeat){
               uint i = 9;
                while(i>=index){
                    recipeHighScore[i+1]=recipeHighScore[i];
                    i=i-1;
                }
        ...
        uint256 toBeat = totalScore[playerHighScore[index]];
         while(index<10 ){
            if (score >toBeat){
                uint i = 9;
                while(i>=index){
                    playerHighScore[index+1]=playerHighScore[index];
                    i=i-1;
                }
