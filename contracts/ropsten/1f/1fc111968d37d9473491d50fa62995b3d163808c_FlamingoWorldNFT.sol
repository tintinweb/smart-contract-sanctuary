...
contract FlamingoWorldNFT is ERC721, Ownable {
    
    using SafeMath for uint256;

    // Basic //
    string public flamingoProvenance = ""; // IPFS URL WILL BE ADDED WHEN ALL SOLD OUT
    uint256 public constant maxCollectiblePerPurchase = 20; // 1-20 tokens per transaction
    uint256 public constant maxCollectibles = 10000;
    uint256 public presalePurchaseLimit = 10; // PRESALE: 10 tokens per wallet maximum.
    uint256 public purchaseLimit = 20; // PUBLIC SALE: 20 tokens per wallet maximum.
    uint256 public collectibleReserve = 150; // FREE MINT FOR OWNER
    ...
    if (totalSupply() <= 1000) {
      return 0.03 ether;
    } else if (totalSupply() > 1000 && totalSupply() <= 3000) {
      return 0.04 ether;
    } else if (totalSupply() > 3000 && totalSupply() <= 5000) {
      return 0.05 ether;
    } else if (totalSupply() > 5000 && totalSupply() <= 7000) {
      return 0.06 ether;
    } else if (totalSupply() > 7000 && totalSupply() <= 9000) {
      return 0.07 ether;
    } else if (totalSupply() > 9000 && totalSupply() <= 10000) {
      return 0.08 ether;
    }
  }
    ...
    function withdraw() external {
        ...
        _withdraw(devAddress, balance.mul(15).div(100));
        _withdraw(caten, balance.mul(15).div(100));
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner notLocked {
        flamingoProvenance = provenanceHash;
    }
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        require(msg.value >= getMintPrice().mul(numberOfTokens), "Ether value sent is not correct.");
