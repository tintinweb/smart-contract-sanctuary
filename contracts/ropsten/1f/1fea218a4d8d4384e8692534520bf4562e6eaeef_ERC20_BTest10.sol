...
contract ERC20_BTest10 is Context, IERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 private uniswapV2Router;
    address public uniswapV2Pair;

    string private constant _name = "BTest-10"; 
    string private constant _symbol = "BTest-10";
    uint8 private constant _decimals = 9;  
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000000000000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _tFeeTotal;
    uint256 public _BTest10Burned;
    ...
    uint256 private _BTest10Burn = 2;    
    ...
    function removeAllFee() private {
        if (_reflection == 0 && _contractFee == 0 && _BTest10Burn == 0) return;
        _reflection = 0;
        _contractFee = 0;
        _BTest10Burn = 0;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = _BTest10EthBurn(amount);
    ...
    function _BTest10EthBurn(uint amount) private returns (uint, uint) {  
        uint orgAmount = amount;
        uint256 currentRate = _getRate();
        uint256 tBurn = amount.mul(_BTest10Burn).div(100);
        uint256 rBurn = tBurn.mul(currentRate);
        _tTotal = _tTotal.sub(tBurn);
        _rTotal = _rTotal.sub(rBurn);
        _BTest10Burned = _BTest10Burned.add(tBurn);
