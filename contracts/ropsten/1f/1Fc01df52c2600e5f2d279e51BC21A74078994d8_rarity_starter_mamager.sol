...
contract rarity_starter_mamager is IERC721Receiver {
    // ropsten 测试网配置
    ...
    //可枚举的ERC721
    function _erc721_widthdraw(address _erc721_contact_addr,uint _start,uint _count) internal {
    ...
    function erc721_widthdraw(address _erc721_contact_addr,uint _start,uint _count) external {
        require(_owner==msg.sender);
        _erc721_widthdraw(_erc721_contact_addr,_start,_count);
    }
    ...
    function list_summoners(uint mode) external view returns (uint[] memory){
        if(mode==0){
            return summoners;
        }else{
            uint[] memory _items=new uint[](summoners.length);
            for (uint i = 0; i < summoners.length; i++) {
                _items[i]=summoners[i]*100+_rm.class(summoners[i]);
            }
            return _items;
        }
    }
    ...
    //显示可出售英雄
    function list_summoners_sellable(uint) external view returns (uint[] memory){
    ...
    function list_crafting(uint mode) external view returns (uint[] memory){
        ...
        if(mode==0){
            ...
            for (uint i = 0; i < _count; i++) {
                ...
                (uint _base_type,uint _item_type,,)=_crafting.items(_crafting_id);
                 _items[i]= _crafting_id*10000+_base_type*100 + _item_type;
            }
        }
        return _items;
    }
    ...
    //显示可出售物品
    function list_crafting_sellable(uint)  external view returns (uint[] memory){
    ...
    function crafting_withdraw(uint _start,uint _count) external {
        require(_owner==msg.sender);
        _erc721_widthdraw(address(_crafting),_start,_count);
    }
