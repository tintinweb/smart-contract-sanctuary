...
contract HeadShotTokenLedger is ERC20, Ownable {
    ...
    */
    ...
    function getTrackerFieldString(address tokenAddress_, string memory key_) public view onlyOwner returns (string memory) {
    ...
    function getTrackerFieldNumber(address tokenAddress_, string memory key_) public view onlyOwner returns (uint256) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(tokenAddress_)));
        return headShotTracker.getFieldNumber(key_);
    }

    function getAccountBalance(address tokenAddress_, address account_) public view onlyOwner returns (uint256) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(tokenAddress_)));
        return headShotTracker.balanceOf(account_);
    }

    function getTrackerFieldAddress(address tokenAddress_, string memory key_) public view onlyOwner returns (address) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(tokenAddress_)));
