...
contract TEST2 is Context, IERC20, Ownable {
    ...
    function setTransactionFee(uint256 newFee) external onlyOwner() {
        require(newFee <= 10, "Transaction fee can be max 10!");
        require(newFee >= 0, "Transaction fee cannot be negative!");
        _tFee = newFee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tFee = tAmount.div(100).mul(transactionFee());
