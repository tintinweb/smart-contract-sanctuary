...
contract PotLuck is Context, Ownable, usingProvable {
    ...
    uint256 public oraclizeFee = 4000000000000000;
    bool private isAvailable = false;
    bool public isProvableEnabled = false;
    struct ResultInfo {
        address address1;
        address addaddress2;
        uint256 amount1;
    ...
    function setOraclizeFee(uint256 amount) external onlyOwner {
        oraclizeFee = amount;
    }
    function start() external onlyOwner payable {
    ...
    function enableProvable() external onlyOwner {
        isProvableEnabled = true;
        provable_query(roundInterval, "URL", "json(https://www.therocktrading.com/api/ticker/BTCEUR).result.0.last");
    }
    function disableProvable() external onlyOwner {
        isProvableEnabled = false;
    }
    ...
    function __callback(bytes32 qid, string memory result) public  override {
        ...
        pickWinner();
        if (isProvableEnabled)
            provable_query(roundInterval, "URL", "json(https://www.therocktrading.com/api/ticker/BTCEUR).result.0.last");
        //emit LogRoundUpdated(result);
    }
    function pickWinner() public onlyOwner returns(address, address, uint256, uint256) {
        ...
        resetPlayers();
        isAvailable = true;
        lastResult = ResultInfo(players[winner1Index].player, players[winner2Index].player, winner1Amount, winner2Amount);
