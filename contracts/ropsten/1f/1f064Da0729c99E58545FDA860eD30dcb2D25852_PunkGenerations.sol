...
contract PunkGenerations {
    ...
    function borderImage2(bytes memory pixels, bytes4 color) internal pure returns (bytes memory) {
        ...
        for (uint y = 0; y < 24; y++) {
            bool previousOn = false;
            bool previousBorderColor = false;
            for (uint x = 0; x < 24; x++) {
                uint p = (y * 24 * 4) + (x * 4);
                bool on = (pixels[p] != 0x0) || (pixels[p+1] != 0x0) || (pixels[p+2] != 0x0) || (pixels[p+3] != 0x0);
                bool borderColor = (pixels[p] == 0x0) && (pixels[p+1] == 0x0) && (pixels[p+2] == 0x0) && (pixels[p+3] == 0xFF);
                if (previousOn && !on && previousBorderColor && (x > 0)) {
                    result[p-4] = color[0];
                    result[p-3] = color[1];
                    result[p-2] = color[2];
                    result[p-1] = color[3];
                }
                if (!previousOn && on && borderColor) {
                    result[p] = color[0];
                    result[p+1] = color[1];
                    result[p+2] = color[2];
                    result[p+3] = color[3];
        ...
        for (uint x = 0; x < 24; x++) {
            ...
            for (uint y = 0; y < 24; y++) {
                ...
                && ((pixels[p] != color[0]) || (pixels[p+1] != color[1]) || (pixels[p+2] != color[2]) || (pixels[p+3] != color[3]));
                bool borderColor = (pixels[p] == 0x0) && (pixels[p+1] == 0x0) && (pixels[p+2] == 0x0) && (pixels[p+3] == 0xFF);
                if (previousOn && !on && previousBorderColor && (y > 0)) {
                    result[p-(24*4)] = color[0];
                    result[p-(24*4)+1] = color[1];
                    result[p-(24*4)+2] = color[2];
                    result[p-(24*4)+3] = color[3];
                } else if (!previousOn && on && borderColor) {
                    result[p] = color[0];
                    result[p+1] = color[1];
                    result[p+2] = color[2];
                    result[p+3] = color[3];
