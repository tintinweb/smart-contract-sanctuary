...
contract Polymon is ERC721Enumerable, Ownable{
    ...
    uint256 public constant NFT_PRICE = 100000000000000000;//80000000000000000; // 0.08 ETH
    uint256 public constant NFT_PRICE_PRESALE = 75000000000000000;//80000000000000000; // 0.075 ETH
    uint256 public constant MAX_NFT_PURCHASE = 1000; //20
    uint256 public RESERVED_TOKENS = 200;
    uint256 public MAX_EGG_SUPPLY = 10000;
    uint256 public MAX_PRESALE_SUPPLY = RESERVED_TOKENS + 500; //(500 supply, 200 reserved tokens)
    ...
    function polyPresaleMint(uint numberOfTokens) public payable {
    ...
    function polyMint(uint numberOfTokens) public payable {
        uint256 totalSupply = totalSupply();
    ...
    function getCreatureProperties(uint256 tokenId) public view returns (uint256[] memory)
    {
        uint256[] memory props = new uint256[](2);
        props[0] = uint256(getGender(tokenId));
        props[1] = uint256(getGeneration(tokenId));
        return props;
    }
    ...
    function withdrawShareHolder(address _shareHolder, uint256 _share) public payable onlyOwner {
        uint balance = address(this).balance;
        
        payable(_shareHolder).transfer(balance * (_share / 100));
    }
