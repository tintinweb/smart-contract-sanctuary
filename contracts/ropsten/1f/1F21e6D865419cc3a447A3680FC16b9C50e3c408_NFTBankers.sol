...
abstract contract Sale is ERC721, Ownable{
   ...
   // mapping (uint256 => address) public _tokenOwner;
    ...
    function orderPlace(address from, uint256 tokenId, uint256 _price) public{
        require( from == _owners[tokenId], "Is Not a Owner");
        Order memory order;
    ...
    function acceptBId(address token,address from, address admin, uint256 amount, uint256 tokenId) public{
        ...
        tokenTransfer(_owners[tokenId], from, tokenId);
    }
    function checkTokenApproval(address app, uint256 tokenId) internal view returns (bool result){
        require(_operatorApprovals[_creator[tokenId]][app], "Token Not approved");
        require(_tokenApprovals[tokenId] == app, "Token Not approved");
        return true;
    }
    function tokenTransfer(address from, address to, uint256 tokenId) internal virtual{
    ...
    function saleToken(address payable admin,uint256 tokenId, uint256 amount) public payable{
        ...
        tokenTransfer(_owners[tokenId], msg.sender, tokenId);
    }
...
contract NFTBankers is ERC721,Sale{
    ...
    function mint(string memory _name, string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal) public returns (bool){
