...
contract Marketplace {
    ...
    mapping(address => mapping(uint256 => Royalty)) NFTRoyalty;
    ...
    function setRoyalty(address _tokenAddress, uint256 _tokenId,uint256[] memory _royalty,address[]  memory _royaltyAddress) public  {

        require(_tokenAddress == msg.sender, "Only the contract can set the royalty for its tokens");

        NFTRoyalty[_tokenAddress][_tokenId] = Royalty({
            royalty:_royalty,
    ...
    function executeTrade(uint256 _trade) public payable tradeExists(_trade) {
        ...
        Royalty memory  royaltyObj = NFTRoyalty[address(trade.tokenContract)][trade.tokenId];
        uint sendAmount = 0;
