...
interface ILiquidityMigrator {
    ...
    * @param dmmRouter Kyber DMM router address
...
/**
* @dev Liquidity Migrator contract to help migrating liquidity from other sources to Kyber DMM Pool
*/
contract LiquidityMigrator is ILiquidityMigrator, Ownable {
    ...
    */
    function manualApproveAllowance(IERC20[] calldata tokens, address[] calldata spenders, uint256 allowance)
        external onlyOwner
    {
    ...
    */
    function migrateLpToDmmPoolWithPermit(
        ...
        (amountA, amountB, addedLiquidity) = migrateLpToDmmPool(
            dmmRouter, uniPair, tokenA, tokenB, liquidity,
    ...
    */
    function migrateLpToDmmPool(
        ...
        (amountA, amountB, addedLiquidity) = _addLiquidityToDmmPool(
            dmmRouter, tokenA, tokenB, amountA, amountB,
            amountAMin, amountBMin, poolInfo, deadline
        );
    ...
    /** @dev Allow the Owner to withdraw any funds that have been 'wrongly' transferred to the migrator contract
    */
    function withdrawFund(IERC20 token, uint256 amount) external onlyOwner {
        if (token == IERC20(0)) {
            (bool success, ) = owner().call { value: amount }("");
            require(success, 'can not withdraw eth');
        } else {
    ...
    */
    ...
    {
        // safe approve tokens if haven't given allowances
        ...
        if (poolInfo.poolAddress == address(0)) {
            ...
            (amountA, amountB, liquidity) = _addLiquidityNewPool(
                dmmRouter, tokenA, tokenB, amountADesired, amountBDesired,
    ...
    */
    function _addLiquidityExistingPool(
        ...
        returns (uint256 amountA, uint256 amountB, uint256 liquidity)
    {
        (amountA, amountB, liquidity) = dmmRouter.addLiquidity(
    ...
    */
    function _addLiquidityNewPool(
        ...
        returns (uint256 amountA, uint256 amountB, uint256 liquidity)
    {
        (amountA, amountB, liquidity) = dmmRouter.addLiquidityNewPool(
