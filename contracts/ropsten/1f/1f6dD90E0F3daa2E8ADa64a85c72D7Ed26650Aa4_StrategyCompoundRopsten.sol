...
contract StratManager is Ownable, Pausable {
    ...
    address public borgFeeRecipient;
     ...
     * @param _borgFeeRecipient address where to send borg's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
        address _unirouter,
        address _vault,
        address _borgFeeRecipient
    ) public {
        keeper = _keeper;
        strategist = _strategist;
        unirouter = _unirouter;
        vault = _vault;
        borgFeeRecipient = _borgFeeRecipient;
    }
    ...
    /**
     * @dev Updates borg fee recipient.
     * @param _borgFeeRecipient new borg fee recipient address.
     */
    function setBorgFeeRecipient(address _borgFeeRecipient) external onlyOwner {
        borgFeeRecipient = _borgFeeRecipient;
    }
...
abstract contract FeeManager is StratManager {
    ...
    uint constant public FEE_DENOM = 10000;
    ...
    uint public borgFee = MAX_FEE - STRATEGIST_FEE - callFee;

    function setCallFee(uint256 _fee) external onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");

        callFee = _fee;
        borgFee = MAX_FEE - STRATEGIST_FEE - callFee;
    }
...
contract StrategyCompoundRopsten is StratManager, FeeManager {
    ...
    constructor(
        ...
        address _strategist,
        address _borgFeeRecipient
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _borgFeeRecipient) public {
        want = _want;
        ...
        applyFees = true;
        rewardToken = 0xf76D4a441E4ba86A923ce32B89AFF89dBccAA075;
        wrapped_native_token = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
        cToken = 0x2973e69b20563bcc66dC63Bde153072c33eF37fe;

        comptroller = Comptroller(0xcfa7b0e37f5AC60f3ae25226F5e39ec59AD26152);
    ...
    /**
     * @dev Withdraws funds and sends them back to the vault. It deleverages from Compound first,
    ...
    function withdraw(uint256 _amount) external {
        require(msg.sender == vault, "caller is not authorised vault");
    ...
    function updateExchangeRate() public {
        currentExchangeRate = CToken(cToken).exchangeRateCurrent();
    }
    ...
    //// ------- VIEW FUNCTIONS -------
    // Total underlying assets held by strat. Expressed in 'want'
    function balanceOf() public returns (uint256) {
        return balanceOfWant().add(balanceOfPool());
    }


    // Returns amount of idle 'want' currently held in this contract
    ...
    function userReserves() public returns (uint256, uint256) {

        uint256 supplyBal = CToken(cToken).balanceOfUnderlying(address(this));
        uint256 borrowBal = CToken(cToken).borrowBalanceCurrent(address(this));
    ...
    function setFeeApplication(bool _status) external onlyManager {
        applyFees = _status;
    }
    ...
    function chargeFees(uint256 amount) internal {
        if(applyFees) {
            uint256 callFeeAmount = amount.mul(callFee).div(FEE_DENOM);
            IERC20(wrapped_native_token).safeTransfer(msg.sender, callFeeAmount);

            uint256 borgFeeAmount = amount.mul(borgFee).div(FEE_DENOM);
            IERC20(wrapped_native_token).safeTransfer(borgFeeRecipient, borgFeeAmount);

            uint256 strategistFee = amount.mul(STRATEGIST_FEE).div(FEE_DENOM);
            IERC20(wrapped_native_token).safeTransfer(strategist, strategistFee);
        }
    }


    // Approve tokens for transfers by lending pool and dex
    function _giveAllowances() internal {
        IERC20(rewardToken).safeApprove(unirouter, uint256(-1));
        IERC20(want).safeApprove(cToken, uint256(-1));
        IERC20(wrapped_native_token).safeApprove(unirouter, uint256(-1));        
    }

    // Revoke approvals for token transfers by lending pool and dex
    function _removeAllowances() internal {
        IERC20(want).safeApprove(cToken, 0);
        IERC20(wrapped_native_token).safeApprove(unirouter, 0);
    ...
    function _leverage(uint256 _amount) internal {
        ...
        for (uint i = 0; i < borrowDepth; i++) {
            require(CToken(cToken).mint(_amount) == 0, "Could not mint cTokens");
            _amount = _amount.mul(borrowRate).div(100);

            require(CToken(cToken).borrow(_amount) == 0, "Could not borrow. Got enough collateral?");
        }
    ...
    function _deleverage() internal {
        ...
        while (wantBal < borrowBal) {
            require(CToken(cToken).repayBorrow(wantBal) == 0, "Could not repay borrowed token. Transfer approved?");

            (supplyBal, borrowBal) = userReserves();
            uint256 targetSupply = borrowBal.mul(100).div(borrowRate);

            require(CToken(cToken).redeemUnderlying(supplyBal.sub(targetSupply)) == 0, "Could not redeem cToken");

            wantBal = IERC20(want).balanceOf(address(this));
        }

        (supplyBal, borrowBal) = userReserves();
        require(CToken(cToken).repayBorrow(borrowBal) == 0, "Could not repay borrowed token. Transfer approved?");
        require(CToken(cToken).redeemUnderlying(supplyBal) == 0, "Could not redeem cToken");
     ...
     * Calculate interest fee on earning from rewardToken.
    ...
    function _claimComp() internal {
        ...
        if (amt != 0) {
            uint256 wantBeforeCompClaim = IERC20(want).balanceOf(address(this)); 
            IUniswapRouterV2 uniswapRouter = IUniswapRouterV2(unirouter);
            address[] memory path = _getPath(rewardToken, address(want));
            ...
            if (amountOut != 0) {
                ...
                uniswapRouter.swapExactTokensForTokens(amt, 1, path, address(this), now + 10);
                uint256 wantFromCompClaim = IERC20(want).balanceOf(address(this)).sub(wantBeforeCompClaim);
                chargeFees(wantFromCompClaim);
            }
        }
    }

    function _getPath(address _from, address _to) internal view returns (address[] memory) {
        address[] memory path;
        if (_from == wrapped_native_token || _to == wrapped_native_token) {
            path = new address[](2);
            path[0] = _from;
            path[1] = _to;
        } else {
