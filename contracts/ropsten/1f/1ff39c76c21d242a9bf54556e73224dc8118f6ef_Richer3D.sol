...
contract Richer3D {
    ...
    function getBounsWithRoundID(uint256 _roundID) private view returns(uint256 _bouns) {
        ...
        DataModal.RoundInfo memory roundInfo = rInfoXrID[_roundID];
        for(uint256 d=1;d<=roundInfo.totalDay;d++){
            DataModal.DayInfo memory dayinfo = rInfoXrID[_roundID].dayInfoXDay[d];
    ...
    function getUserBalance(address _userAddress) private view returns(uint256 _balance) {
        ...
        for(uint256 i=1;i<=roundNumber;i++) {
            DataModal.RoundInfo memory roundInfo = rInfoXrID[i];
            for(uint256 j=1;j<roundInfo.totalDay;j++) {
