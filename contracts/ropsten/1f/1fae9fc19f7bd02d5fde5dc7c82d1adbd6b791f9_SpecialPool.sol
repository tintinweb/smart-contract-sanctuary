...
contract SpecialPool is Ownable {
    ...
    function calculateRewards(uint256 stakeID) public view returns(uint256 reward) {
        ...
        if (block.timestamp < stake.intialTimestamp + 3 minutes) {
            reward = stake.tokenQuantity * 1 / 100;
        }
        
        if (block.timestamp >= stake.intialTimestamp + 3 minutes && block.timestamp < stake.intialTimestamp + 5 minutes) {
            reward = (stake.tokenQuantity.mul(ENTRY_RATE))/100E18;
        }
        
        if (block.timestamp >= stake.intialTimestamp + 5 minutes && block.timestamp < stake.intialTimestamp + 8 minutes) {
            reward = stake.tokenQuantity * DAYS30_RATE /  100E18;
        }
        
        if (block.timestamp >= stake.intialTimestamp + 8 minutes) {
            reward = stake.tokenQuantity * 10**18 / 100**18;
        }
