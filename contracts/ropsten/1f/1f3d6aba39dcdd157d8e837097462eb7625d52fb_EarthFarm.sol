...
contract EarthFarm is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. EARTH tokens to distribute per block.
        uint256 lastRewardBlock; // Last block number that EARTH tokens distribution occurs.
        uint256 accEarthPerShare; // Accumulated EARTH tokens per share, times 1e12. See below.
    }

    EarthToken public immutable earth; // The EARTH ERC-20 Token.
    uint256 private earthPerBlock; // EARTH tokens distributed per block. Use getEarthPerBlock() to get the updated reward.

    PoolInfo[] public poolInfo; // Info of each pool.
    mapping(uint256 => mapping(address => UserInfo)) public userInfo; // Info of each user that stakes LP tokens.
    uint256 public totalAllocPoint; // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public startBlock; // The block number when EARTH token mining starts.

    uint256 public blockRewardUpdateCycle = 1 days; // The cycle in which the earthPerBlock gets updated.
    uint256 public blockRewardLastUpdateTime = block.timestamp; // The timestamp when the block earthPerBlock was last updated.
    uint256 public blocksPerDay = 6525; // The estimated number of mined blocks per day.
    uint256 public blockRewardPercentage = 1; // The percentage used for earthPerBlock calculation.
    ...
    constructor(
        ...
        require(address(_earth) != address(0), "EARTH address is invalid");
    ...
    modifier updateEarthPerBlock() {
        (uint256 blockReward, bool update) = getEarthPerBlock();
        if (update) {
            earthPerBlock = blockReward;
            blockRewardLastUpdateTime = block.timestamp;
        }
        _;
    }

    function getEarthPerBlock() public view returns (uint256, bool) {
        if (block.number < startBlock) {
            return (0, false);
        }

        uint256 poolReward = earth.balanceOf(address(this));
        if (poolReward == 0) {
            return (0, earthPerBlock != 0);
        }

        if (block.timestamp >= getEarthPerBlockUpdateTime() || earthPerBlock == 0) {
            return (poolReward.mul(blockRewardPercentage).div(100).div(blocksPerDay), true);
        }

        return (earthPerBlock, false);
    }

    function getEarthPerBlockUpdateTime() public view returns (uint256) {
    ...
    // Update the given pool's EARTH token allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending EARTH tokens on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            (uint256 blockReward, ) = getEarthPerBlock();
            uint256 earthReward = multiplier.mul(blockReward).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public updateEarthPerBlock {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        // no minting is required, the contract should have EARTH token balance pre-allocated
        // accumulated EARTH per share is stored multiplied by 10^12 to allow small 'fractional' values
        pool.accEarthPerShare = pool.accEarthPerShare.add(earthReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to EarthFarming for EARTH token allocation.
    ...
    // Withdraw LP tokens from EarthFarming
    ...
    // Safe EARTH token transfer function, just in case if
    // rounding error causes pool to not have enough EARTH tokens
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 balance = earth.balanceOf(address(this));
        uint256 amount = _amount > balance ? balance : _amount;
        earth.transfer(_to, amount);
    }
