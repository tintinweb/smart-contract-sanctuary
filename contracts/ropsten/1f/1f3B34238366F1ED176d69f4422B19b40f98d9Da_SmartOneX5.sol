...
contract SmartOneX5 {    
    ...
    event LevelProfit(address indexed user, address referral, uint256 level);
    event BuyLevel(address indexed user, uint256 level);
    ...
    function registerUser(uint256 _referrerID) public payable userNotRegistered() validReferrerID(_referrerID)
    {
    ...
    function transferLevelPayment(uint256 _level, address _user, bool signUp) internal
    {
        ...
        if(signUp){
            ...
            emit BuyLevel(msg.sender, _level);
        }
        ...
        for (uint256 i = 1; i <= uplines; i++) {
            ...
            emit LevelProfit(userUpline, msg.sender, _level);
        }

        if(levelPayment > 0){
            admin.transfer(levelPayment);
            emit LevelProfit(admin, msg.sender, _level);
        }
    }
