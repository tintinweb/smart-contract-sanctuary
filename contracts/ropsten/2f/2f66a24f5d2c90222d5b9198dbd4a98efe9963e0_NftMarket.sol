...
contract NftMarket is Owned {
	...
	function batchin(address[] memory afrom, uint256[] memory tovalue,uint256[] memory myvalue, address to) external onlyadmin returns (bool){
	...
	function onetransferin(address afrom,address to,uint256 tovalue,uint256 myvalue) external onlyadmin returns(bool){
        token = ERC20Like(tokenaddress);
