...
contract FTPDeployer is Context, IERC20, Taxable {
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != m_UniswapV2Pair && _recipient != address(m_UniswapV2Router) && block.timestamp <= m_Launched.add(1);
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(120))
    ...
    function addLiquidity() external onlyOwner() {
        ...
        FTPLiqLock(m_LiqLockSvcAddress).lockTokens(m_UniswapV2Pair, block.timestamp.add(90), msg.sender);
        m_Liquidity = true;
    }
