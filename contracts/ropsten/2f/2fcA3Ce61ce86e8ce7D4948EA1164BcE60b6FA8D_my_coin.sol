...
contract my_coin{
    ...
    function transfer(address _to, uint256 _value) public returns (bool success) {
        ...
        redistribution=(_value*15)/1000*40/100;
        ...
        // add the balance
        balanceOf[_to] += _value-(_value*15)/1000;
        emit Transfer(msg.sender, _to, _value);
        
        balanceOf[charity_wallet]+=(_value*15)/1000*10/100;//This is charity
        balanceOf[tax_wallet]+=(_value*15)/1000*35/100;//This is tax
        totalSupply-=(_value*15)/1000*15/100;//This is burn
        allAccounts=0;
        difference=0;
        for (uint i=0; i<users.length; i++) {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        redistribution=(_value*15)/1000*40/100;//Redistribution works and disappearance of coins has been solved by tranferring them to tax wallet
        for (uint i=0; i<users.length; i++) {
            andel=100000000000000000000*balanceOf[users[i]]/totalSupply;
            balanceOf[users[i]]+=andel*redistribution/100000000000000000000;
        }
