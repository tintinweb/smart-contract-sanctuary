...
the more you earn – it really is that simple. auto distribution to all $CASHDOGE Holders hourly.
*/
...
contract CashDoge is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 private uniswapV2Router;
    address private uniswapV2Pair;

    address private _dividendToken = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43;
    ...
    bool private buyBackRandomEnabled = true;

    CASHDOGEDividendTracker private dividendTracker;

    address private buyBackWallet;
    
    uint256 private maxBuyTranscationAmount = 3000000000000 * (10**18);
    uint256 private maxSellTransactionAmount = 1000000000000000 * (10**18);
    uint256 private swapTokensAtAmount = 20000000000 * (10**18);
    uint256 private maxWalletToken = 50000000000000 * (10**18); // 5% of total supply

    uint256 private dividendRewardsFee;
    ...
    uint256 private _buyBackMultiplier = 100;
    ...
    mapping (address => bool) private _isExcludedMaxSellTFransactionAmount;
    ...
    constructor() ERC20("Cash Doge", "CASHDOGE") {
            	dividendTracker = new CASHDOGEDividendTracker();
    ...
    function doConstructorStuff() external onlyOwner{
    	...
    	buyBackWallet = 0x6a2bEedDA95D21E10801D993a38EE63D567b9a2E;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "CASHDOGE: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "CASHDOGE: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "CASHDOGE: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CASHDOGE: The dividend tracker already has that address");

        CASHDOGEDividendTracker newDividendTracker = CASHDOGEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CASHDOGE: The new dividend tracker must be owned by the CASHDOGE token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CASHDOGE: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CASHDOGE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CASHDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CASHDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CASHDOGE: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "CASHDOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CASHDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CASHDOGE: Cannot update gasForProcessing to same value");
...
contract CASHDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CASHDOGE_Dividend_Tracker", "CASHDOGE_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CASHDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CASHDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CASHDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CASHDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CASHDOGE_Dividend_Tracker: Cannot update claimWait to same value");
