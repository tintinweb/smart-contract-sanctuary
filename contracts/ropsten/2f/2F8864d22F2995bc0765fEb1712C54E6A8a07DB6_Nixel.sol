...
contract Nixel is ERC721Enumerable, Ownable {
    ...
    function buyBlock(int256 x_pos, int256 y_pos) public payable {
        ...
        uint256 toLiquidity = msg.value / 100;
        uint256 toSeller = msg.value - toLiquidity;
        uint256 liqAmount = msg.value / 100 / 10 ** 18;
        uint256 ethAmount = msg.value / 10 ** 18;
        uint256 mintAmount = nixelPixelMint / 10 ** 18;
        payable(_sellerAddress[x_pos][y_pos]).transfer(toSeller);
        payable(address(pixelContract)).transfer(toLiquidity);
        pixelContract.mint(_msgSender(), mintAmount * liqAmount * 10**18);
        pixelContract.mint(
            _sellerAddress[x_pos][y_pos],
            mintAmount * liqAmount * 10**18
        );
        pixelContract.mint(
            address(pixelContract),
            (ethAmount * mintAmount / 100 * 2 * 20) * 10 ** 18
        );
