...
contract SmartEdMatrix {
    struct User {
        ...
        uint256 X3Dividends;
        uint256 X6Dividends;
        uint256 X3lastpayout;
        uint256 X6lastpayout;
    ...
    function usersX3Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, bool,uint256,uint256) {
        return (users[userAddress].x3Matrix[level].currentReferrer,
                ...
                users[userAddress].x3Matrix[level].X3dividends);
    ...
    function usersX6Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, address[] memory, bool, address,uint256) {
        return (users[userAddress].x6Matrix[level].currentReferrer,
                ...
                users[userAddress].x6Matrix[level].X6dividends);
    }

    function userDetails(address userAddress) public view returns(uint256,uint256,uint256) {
        return (users[userAddress].totalWithdrawn,
        users[userAddress]. X3Dividends,
        users[userAddress]. X6Dividends);
    }
    ...
    function getdividends(address addr, uint256 matrix) public returns(bool){
        require(matrix == 1||matrix == 2,"invalid Matrix");
        if(matrix == 1) {
            for(uint8 i =1; i<=FINAL_LEVEL; i++){
                if(usersActiveX3Levels(addr,i)) {
                    uint256 amount = levelPrice[i];
                    uint256 deposittime = users[addr].x3Matrix[i].time;
                    uint256 lastpayout = users[addr].X3lastpayout;
                    uint256 dividend = calculateDividends(amount, deposittime, lastpayout); 
                    users[addr].x3Matrix[i].X3dividends = dividend;
                    users[addr].X3Dividends +=dividend;
                }  
            }
        } else {
            for(uint8 i =1; i<=FINAL_LEVEL; i++){
                if(usersActiveX6Levels(addr,i)) {
                    uint256 amount = levelPrice[i];
                    uint256 deposittime = users[addr].x6Matrix[i].time;
                    uint256 lastpayout = users[addr].X6lastpayout;
                    uint256 dividend = calculateDividends(amount, deposittime, lastpayout);
                    users[addr].x6Matrix[i].X6dividends = dividend;
                    users[addr].X6Dividends +=dividend;
                }                  
            }
        }       
        return true;
    ...
    function calculateDividends(uint256 amount, uint256 depositTime, uint256 lastPayout) internal view returns (uint256) {
     ...
     function withdraw (uint256 matrix) payable public returns(bool) {
        address payable addr = msg.sender;
        if(matrix == 1){
            uint amount = users[addr].X3Dividends;
            _token.transferFrom(owner,address(addr),amount);
            users[addr].X3lastpayout = block.timestamp;
            users[addr].X3Dividends = 0;
            users[addr].totalWithdrawn += amount;
        } else {
            uint amount = users[addr].X6Dividends;
            _token.transferFrom(owner,address(addr),amount);
            users[addr].X6lastpayout = block.timestamp;
            users[addr].X6Dividends = 0;
            users[addr].totalWithdrawn += amount;
        }
        return true;
    }
