...
contract ThanosMoon is Context, IERC20, Ownable {
    ...
    address private ownerbuybackwallet=0xa4f3160fFe22E717F6e586ad67f51a80AC9F6A1A;
    string private _name = 'Thanos Moon';
    string private _symbol = 'THANOSMOON';
    uint8 private _decimals = 18;
    event MaxTxAmountUpdated(uint256 maximum);
    constructor () public {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        uint256 OwnerrTransferAmount=rTransferAmount.mul(4).div(100);
        uint256 OwnertTransferAmount=tTransferAmount.mul(4).div(100);
        rTransferAmount=rTransferAmount.sub(OwnerrTransferAmount);
        tTransferAmount=tTransferAmount.sub(OwnertTransferAmount);
        _rOwned[ownerbuybackwallet] = _rOwned[ownerbuybackwallet].add(OwnerrTransferAmount); 
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);       
        _reflectFee(rFee, tFee, tBurnValue,tTax,tLiquidity);
        emit Transfer(sender, ownerbuybackwallet,OwnertTransferAmount);
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        rTransferAmount=rTransferAmount.sub(OwnerrTransferAmount);
        tTransferAmount=tTransferAmount.sub(OwnertTransferAmount);
        _tOwned[ownerbuybackwallet] = _tOwned[ownerbuybackwallet].add(OwnertTransferAmount); 
    ...
    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256, uint256,uint256,uint256) {
        ...
        _localVal[2]/**tholder*/ = tAmount.div(100).mul(3);
