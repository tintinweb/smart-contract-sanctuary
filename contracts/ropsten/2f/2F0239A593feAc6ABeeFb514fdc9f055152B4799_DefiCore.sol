...
contract DefiCore is IDefiCore {
    ...
    function getUserAssets(address _userAddr)
        external
        view
        returns (
            bytes32[] memory _availableAssets,
        ...
        )
    {
        ...
        _availableAssets = new bytes32[](
            _allAssetsCount - _supplyAssetsCount - _borrowAssetsCount
        );

        for (uint256 i = 0; i < _allAssetsCount; i++) {
            bytes32 _currentAsset = _allAssets[i];

            if (
                !_supplyAssets[_userAddr].contains(_currentAsset) &&
                !_borrowAssets[_userAddr].contains(_currentAsset)
            ) {
                _availableAssets[_currentIndex++] = _currentAsset;
            }
        }
    }
    ...
    function addLiquidity(bytes32 _assetKey, uint256 _liquidityAmount) external returns (uint256) {
        ...
        require(
            !_borrowAssets[msg.sender].contains(_assetKey),
            "DefiCore: It is impossible to add liquidity to an asset that is borrowed."
        );
    ...
    function borrow(bytes32 _assetKey, uint256 _borrowAmount) external returns (uint256) {
        ...
        require(
            !_supplyAssets[msg.sender].contains(_assetKey),
            "DefiCore: Unable to borrow an asset into which liquidity was deposited."
        );
...
contract LiquidityPool is ERC20 {
    ...
    function getAmountInUSD(uint256 _assetAmount) public view returns (uint256) {
        ...
        return (_assetAmount * _parameters.getAssetPrice(assetKey)) / 10**decimals();
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            borrowInfos[to].borrowAmount == 0,
            "LiquidityPool: Unable to send tokens to an address that is in arrears."
        );

        if (from != address(0) && !_defiCore.disabledCollateralAssets(from, assetKey)) {
