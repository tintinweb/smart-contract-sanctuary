...
interface IDistributor {
    ...
    function getInfoIndex(address _recipient) external view returns (bool, uint256);
...
contract OlympusStaking is OlympusAccessControlled {
    ...
    function setEpochLength(uint256 _epochLength) onlyGovernor external {
    ...
    function stake(
        ...
        if (_claim && warmupPeriod == 0) {
            distributor.setRecipient(msg.sender, _amount, true);// add or update stake info
    ...
    function distributeProfit(address _recipient, uint256 _amount) external {
        require(address(distributor) != address(0) && msg.sender == address(distributor), "Only distributor");
        sOHM.transfer(_recipient, _amount);
    }
...
//    }
    ...
    function unstake(
        ...
        bool have;
        uint256 infoIndex;
        (have, infoIndex) = distributor.getInfoIndex(_to);
        require(have, "Not exist staked");

        distributor.setRecipient(msg.sender, _amount, false);
        distributor.removeRecipient(infoIndex);
