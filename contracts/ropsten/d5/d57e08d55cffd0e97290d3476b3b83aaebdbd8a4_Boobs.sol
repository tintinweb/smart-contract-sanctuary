...
contract Boobs is Context, IERC20, Ownable {
    ...
    uint256 internal _tokenTotal = 640_00000000e9;
    ...
    //@dev 3%
    uint256 public _feeDecimal = 3;
    //@dev The tax fee contains two decimal places so 200 = 2%
    uint256 public _taxFee = 200;
    //@dev The liquidityFee fee contains two decimal places so 300 = 3%
    uint256 public _liquidityFee = 300;
    //@dev this is the total cost 5%
    ...
    //@dev set the min tokens for transaction to be sucess
    uint256 public minTokensBeforeSwap = 5000;
    uint256 public minEthBeforeSwap = 5000;
    
    uint256 public liquidityAddedAt;

    uint256 public lastRebalance = now ;
    uint256 public rebalanceTaxInterval = 120 minutes;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "BOOBS: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'BOOBS: The contract it self cannot be excluded');
        require(!_isExcluded[account], "BOOBS: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "BOOBS: Account is already included");
    ...
    function _transfer(
        ...
        //@dev Limit the transfer to 20000000000 tokens for first 5 minutes
        require(now > liquidityAddedAt + 5 minutes  || amount <= 20000000000e9, "You cannot transfer more than 20000000000 tokens.");
        ...
        if(!inSwapAndLiquify && sender != uniswapV2Pair) {
            ...
            //@dev Buy tokens
            if(now > lastRebalance + rebalanceTaxInterval 
                && rebalanceEnalbed 
    ...
    function setrebalanceTaxInterval(uint256 interval) public onlyOwner {
        rebalanceTaxInterval = interval;
    }
