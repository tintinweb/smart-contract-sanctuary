...
/*
 * Crypto stamp Bridge: Bridged ERC-721 Token
 * Prototype contract for all ERC-721 tokens deployed by bridging an original
 * ERC-721 token from the other side of the bridge
 *
...
// File: contracts/BridgedERC721.sol

/*
 * A generic bridged non-fungible token, to be deployed by the bridge when a
 * token from the other side is exiting on this side of the bridge.
 * Implements ERC 721 NFT standard: https://eips.ethereum.org/EIPS/eip-721
 */
...
contract BridgedERC721 is ERC721Enumerable, BridgedERC721I {
    ...
    ERC721("Bridged ERC721 Prototype", "BrProto")
    {
    ...
    function initialRegister(address _bridgeDataAddress,
                             string memory _symbol, string memory _name,
                             string memory _originalChainName, address _originalChainAddress)
    external override
    requireActive
    {
    ...
    function setMetadata(string memory _newSymbol, string memory _newName)
    external
    requireActive
    onlyBridgeControl
    {
        symbolStore = _newSymbol;
        nameStore = _newName;
    }

    function setSymbol(string memory _newSymbol)
    external
    requireActive
    onlyBridgeControl
    {
    ...
    function _baseURI() internal view override returns (string memory) {
        return string(abi.encodePacked(bridgeData.tokenURIBase(), uint256(uint160(address(this))).toHexString(20), "/meta/"));
    }

    /*** Implement mintable and exists interfaces ***/

    // Safely mint a token. Calls onERC721Received() on the `_to` address if it's a contract.
