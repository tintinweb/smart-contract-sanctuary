...
contract BC_BIM_Contract_v7 {
    string public name = 'PIER-BIM-v7';
    ...
    mapping(uint256 => string[][]) public excelData;
    ...
    function drawingPublishReceivers(uint _updateId) public view returns (address [] memory) {
        return drawingPublishs[_updateId].receiver;
    }
    
    function correspondenceReceivers(uint _updateId) public view returns (address [] memory) {
        return correspondences[_updateId].receiver;
    }
    
    function modelUpdateReceivers(uint _updateId) public view returns (address [] memory) {
        return modelUpdates[_updateId].receiver;
    }
    ...
    function newModelUpdate(address[] memory _receiver, string memory _modelNumber, string memory _revision, string memory _revisionDetails, string memory _modelTitle, string memory _fileIpfsHash, string[] memory _data) checkForWallet(msg.sender) public {
         // Make sure the file hash exists
        require(bytes(_fileIpfsHash).length > 0);
        // Make sure drawing number exists
        require(bytes(_modelNumber).length > 0);
        
        updatesCount ++;
        modelUpdates[updatesCount] = ModelUpdate(updatesCount, msg.sender, _receiver, block.timestamp, _modelNumber, _revision, _revisionDetails, _modelTitle, _fileIpfsHash);
        excelData[updatesCount].push(_data);    
    }
    
    function getExcelData(uint _updateId) checkForWallet(msg.sender) public view returns(string[][] memory) {
        return excelData[_updateId];
    }
