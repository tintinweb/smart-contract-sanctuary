...
contract Gabriel is Ownable {
    ...
    function unstake(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            uint256 tStaked = pool.totalStaked.sub(pool.forfeited); // Here we finally subtract forfeited from totalStaked to get the actual total staked.
            if (tStaked == 0) {
                // Calculate the LOR.
                uint256 tsTr = tStaked.add(pool.rewardsInPool);
                ...
                if (poolBal > tsTr) {
                    ...
                    if (LOR > 0) {
                        ...
                        // Transfer Half to devaddr, and otherHalf to growthFundAddr.
