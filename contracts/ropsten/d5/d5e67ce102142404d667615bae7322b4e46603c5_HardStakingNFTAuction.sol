...
contract HardStakingNFTAuction is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    event RescueAuctionToken(address indexed to, uint auctionId, uint tokenId, bool wasAuctionFinished);
    ...
    function getStakeTokenEquivalentAmount(uint burnTokenAmount) public view returns (uint) { 
        if (isSetPrice) {
            return burnTokenAmount.mul(stakeTokenToBurnTokenRate) / 1e18;
        } else {
    ...
    function getBurnTokenEquivalentAmount(uint stakeTokenAmount) public view returns (uint) { 
        if (isSetPrice) {
            return stakeTokenAmount.mul(1e18) / stakeTokenToBurnTokenRate;
        } else {
    ...
    function processSuccesfullAuction(uint auctionId) public { 
        ...
        if (winner == address(0)) {
            ...
            if (successAuctionFeePercentage != 0) {
                successFee = actualStakeAmount.mul(successAuctionFeePercentage) / 100;
                bidFinalAmount = actualStakeAmount.sub(successFee);
            ...
            _totalSupply = _totalSupply.sub(actualStakeAmount);
            _balances[winner] = _balances[winner].sub(actualStakeAmount);
    ...
    function _stake(uint amount, uint auctionId, address user, bool isBurnToken, bool isRestake) private nonReentrant {
        ...
        uint nextBid = auction.topBidder != address(0)
            ? auction.topRelativeBid.mul(nextBidStepPercentage) / 100
