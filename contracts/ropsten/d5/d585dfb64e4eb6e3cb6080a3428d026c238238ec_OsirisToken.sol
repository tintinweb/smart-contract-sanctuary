...
contract OsirisToken is ERC20Upgradeable, AccessControlUpgradeable, OwnableUpgradeable, Pauseable {
        ...
        returns (bool)
    {
        ...
        uint256 leftOver = tokenTax(msg.sender, recipient, amount);
        swap(msg.sender, recipient);

        return super.transfer(recipient, leftOver);
    }
    ...
    function transferFrom(
        ...
        uint256 leftOver = tokenTax(sender, recipient, amount);
        swap(sender, recipient);

        return super.transferFrom(sender, recipient, leftOver);
    }
    ...
    */
    function getTax(
        ...
        if (pairs[sender] == true) {
            // Buy
            taxed = (amount * buyTax) / 100;
            amountLeft = amount - taxed;
        } else if (pairs[recipient] == true) {
            // Sell
            taxed = (amount * sellTax) / 100;
            amountLeft = amount - taxed;
        }
    }
