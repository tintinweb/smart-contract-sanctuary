...
contract GovernorAlpha is GovernorStorage, GovernorEvents {
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(ausc.balanceOf(msg.sender) >= createVotes(), "ITS UNDER 9000!! Proposer balance below create threshold");
    ...
    function approve(uint256 proposalId)
        public
    {
        require(state(proposalId) == ProposalState.Created, "GovernorAlpha::add: proposal already approved");
        require(ausc.getPriorVotes(msg.sender, SafeMath.sub(block.number, 1)) >= approveVotes(), "GovernorAlpha::propose: approver votes below approval threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || msg.sender == proposal.proposer || ausc.balanceOf(proposal.approver) < approveVotes(), "GovernorAlpha::cancel: proposer above threshold");
    ...
    function _setVotingDelay(uint newVotingDelay) external {
        ...
        require(newVotingDelay >= MIN_VOTING_DELAY && newVotingDelay <= MAX_VOTING_DELAY, "GovernorAlpha::_setVotingDelay: invalid voting delay");
    ...
    function _setVotingPeriod(uint newVotingPeriod) external {
        ...
        require(newVotingPeriod >= MIN_VOTING_PERIOD && newVotingPeriod <= MAX_VOTING_PERIOD, "GovernorAlpha::_setVotingPeriod: invalid voting period");
    ...
    function _setApprovePercent(uint newApprovePercent) external {
        require(msg.sender == admin, "GovernorAlpha::_setProposalThreshold: admin only");
        require(newApprovePercent >= MIN_APPROVE_THRESHOLD && newApprovePercent <= MAX_APPROVE_THRESHOLD, "GovernorAlpha::_setProposalThreshold: invalid approve threshold");
    ...
    function _setQuorumPercent(uint newQuorum) public {
        ...
        require(newQuorum >= MIN_QUORUM_THRESHOLD && newQuorum <= MAX_QUORUM_THRESHOLD, "GovernorAlpha::_setQuorumPercent: invalid quorum percent");
    ...
    function _acceptAdmin() external {
        ...
        require(msg.sender == pendingAdmin && msg.sender != address(0), "GovernorAlpha:_acceptAdmin: pending admin only");
