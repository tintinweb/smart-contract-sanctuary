...
contract HeadShotTokenLedger is ERC20, Ownable {
    ...
    mapping(address => address) public _trackerTokenMap;

    address[] public _trackerTokenList;
    ...
    function getTokenAddress(address tokenAddress_) external view onlyOwner returns (address) {
        return _tokenMap[tokenAddress_].tokenAddress;
    }

    function getTrackerTokenAddress(address tokenAddress_) external view onlyOwner returns (address) {
        return _trackerTokenMap[tokenAddress_];
    }

    function addTrackerInfo(address tokenAddress_, address trackerAddress_) external onlyOwner returns (bool) {
        _trackerTokenMap[tokenAddress_] = trackerAddress_;
        _trackerTokenList.push(trackerAddress_);
        return true;
    }
    ...
    function listTokenTracker(uint limit_, uint page_) external view onlyOwner returns (address[] memory) {
        uint listCount = _trackerTokenList.length;
        ...
        if (listCount > 0){
            for (uint i = 0; i < listCount; i++) {
                ...
                if (insert){
                    _trackers[id] = _trackerTokenList[i];
                    id++;
                }
                j++;
            }
