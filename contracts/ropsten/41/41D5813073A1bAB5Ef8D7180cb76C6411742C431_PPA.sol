...
contract PPA is producerRegistry, ppaBuyerRegistry {
    ...
    uint[] idPPAlist;
    uint[] statusList;
    function getPPAByID(uint _id) public returns (address[] memory, address[] memory, uint[] memory, uint[] memory, uint[] memory){
        
        for(uint i = _id; i < listOfPPAs.length; i--){
            producerList.push(listOfPPAs[i].producer);
            buyerList.push(listOfPPAs[i].buyer);
            priceList.push(listOfPPAs[i].kwhPrice);
            idPPAlist.push(listOfPPAs[i].id);
            uint x = uint(listOfPPAs[i].status);
            statusList.push(x);

            //return(listOfPPAs[i].producer, listOfPPAs[i].buyer, listOfPPAs[i].kwhPrice, listOfPPAs[i].startDay, listOfPPAs[i].endDay, listOfPPAs[i].id);
        }
