...
contract PRESALETEST is Ownable {
  ...
  function setTESTPrice( uint256 newTESTPrice_ ) external onlyOwner() returns ( uint256 ) {
    TESTPrice = newTESTPrice_;
    return TESTPrice;
  }
  ...
  function _calculateAmountPurchased( uint256 amountPaid_ ) internal returns ( uint256 ) {
    return amountPaid_.mul( TESTPrice );
  }

  function buyTEST( uint256 amountPaid_ ) external returns ( bool ) {
    require( approvedBuyers[msg.sender], "Buyer not approved." );
    uint256 TESTAmountPurchased_ = _calculateAmountPurchased( amountPaid_ );
    dai.safeTransferFrom( msg.sender, _saleProceedsAddress, amountPaid_ );
    TEST.safeTransfer( msg.sender, TESTAmountPurchased_ );
    return true;
  }
