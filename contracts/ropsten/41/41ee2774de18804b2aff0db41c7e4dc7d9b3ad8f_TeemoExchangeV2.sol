...
// File: contracts/standards/HasSecondarySaleFees.sol
...
// File: contracts/TransferProxyForDeprecated.sol
...
contract TeemoExchangeV2 is Ownable, ExchangeDomain {
    using SafeMath for uint;
    ...
    ITransferReferralContract public referralContract;


    constructor(
        ITransferReferralContract _referralContract,
    ...
    function exchange(
        ReferralOrder calldata referralOrder
    ) payable external {
        Order  memory order = referralOrder.order;
        Sig memory sig = referralOrder.sig;
        Sig memory buyerFeeSig = referralOrder.buyerFeeSig;
        uint amount = referralOrder.amount;
        uint buyerFee = referralOrder.buyerFee;
        address buyer = referralOrder.buyer;
        ...
        require(order.key.sellAsset.assetType != AssetType.BNB, "BNB is not supported on sell side");
        if (order.key.buyAsset.assetType == AssetType.BNB) {
        ...
        referralContract.transferReferral(order.key.salt, referralOrder.referrals, referralOrder.refarralSig);
    }
    ...
    function transfer(Asset memory asset, uint value, address from, address to) internal {
        if (asset.assetType == AssetType.BNB) {
