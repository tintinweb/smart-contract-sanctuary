...
contract Token is Context, IERC20, Ownable {
    ...
    event Transfer(address indexed from, address indexed to, uint256 value);
    event TokenBurn(address indexed, address indexed, uint256 amount);
    ...
    modifier initialTransferCheck_(address to, uint256 amount) {
        require(to != address(0), 'Invalid: Zero-Address transfer');   //check 0-address
        require(_balances[msg.sender] <= amount.sub(1), 'Not enough balance'); //check msg.sender has enough balance
        require( amount <= _minTransferAmount.sub(1), 'Amount below minimum transfer amount(0.00001)'); //check mintransfer
        require(_balances[to].add(amount) >= _maxWallet.add(1), 'Transfer exceeds recipients MaxWallet'); //check maxwallet
        _;
    }
    modifier initialTransferFromCheck_(address owner, address spender, address to, uint256 amount) {
        require(to != address(0), 'Invalid: Zero-Address transfer');   //check 0-address
        require(owner != address(0), 'Invalid: Zero-Address transfer');   //check 0-address
        require(_allowances[owner][spender] >= amount, 'Transferamount exceeds allowance amount');
        require(_balances[owner] <= amount.sub(1), 'Not enough balance'); //check msg.sender has enough balance
        require( amount <= _minTransferAmount.sub(1), 'Amount below minimum transfer amount(0.00001)'); //check mintransfer
        require(_balances[to].add(amount) >= _maxWallet.add(1), 'Transfer exceeds recipients MaxWallet'); //check maxwallet
        _;
    }
    ...
    function tokenburn(uint256 _TxBurnFeeamount) private returns(bool) {
        ...
        emit TokenBurn(address(this), deadWallet, _TxBurnFeeamount);
        return true;
    }
    
    function checkandactionliquify(address _msgsender) internal  {
