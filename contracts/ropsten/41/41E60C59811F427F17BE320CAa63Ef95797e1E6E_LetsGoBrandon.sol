...
contract LetsGoBrandon is Context, IERC20, Ownable {
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if(!automatedMarketMakerPairs[from]) 
		{
			...
			if(automatedMarketMakerPairs[to]){
				...
				if(blockTime < firstTransTime + 7 minutes){
					extraFees = 15;
				}
				else if(blockTime < firstTransTime + 10 minutes){
					extraFees = 10;
				}
				else if(blockTime < firstTransTime + 15 minutes){
					extraFees = 5;
				}
	...
	function getExtraSalesFee(address account) public view returns (string memory) {
		uint256 extraFees = 0;
		...
		string memory ts = uint2str(firstTransTime);
        string memory ef = uint2str(extraFees);
        return string(abi.encodePacked(ts, " ", ef));
	}
