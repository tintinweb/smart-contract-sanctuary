...
// File: contracts/vendors/libraries/TxStorage.sol
...
contract TxStorage is GovernanceOwnable{	
    ...
    function closedTransaction(address to, uint i) internal {
         transactionsHistory[to][i].closed = true;
    }
    ...
    function getTransaction(address to, uint i) public view returns (uint ammount, uint price, uint timestamp, uint expireTimeStamp, bool closed) {
       require(transactionsHistory[to][i].timestamp != 0 , "INDEX OUT OF RANGE");
       ammount = transactionsHistory[to][i].ammount;
       price =transactionsHistory[to][i].price;
       timestamp = transactionsHistory[to][i].timestamp;
       expireTimeStamp = transactionsHistory[to][i].expireTimeStamp;
       closed = transactionsHistory[to][i].closed;
    }


    function checkTrransaction(address to, uint i) internal view{
       Transaction memory transaction = transactionsHistory[to][i];
...
contract BasePool is Whitelist, TxStorage {
 ...
 constructor (
        ...
        _oracleAddress = 0x7BAbda58ae792588E7216205DE03Ab8ac9BBeFd7;
        _PACT = 0xb72261Db70864B468159Db2A0238D728A60E3722;
	...
	function calculateAB(uint amountIn) public view returns (uint amountOut) {
        uint WETHPrice = IUniswapOracle(_oracleAddress).consultAB(amountIn);
        amountOut = WETHPrice.div(_price).mul(1e18);
	}


	function calculateBA(uint amountIn) public view returns (uint amountOut) {
        uint usd = amountIn.mul(_price);
        usd = usd.div(1e18);
        amountOut = IUniswapOracle(_oracleAddress).consultBA(usd);
	}


    function deposit() public onlyWhitelisted payable {
        uint amountIn = msg.value;
        IUniswapOracle(_oracleAddress).update();
        uint tokensAmount = calculateAB(amountIn);
    ...
    function withdraw(uint index) external onlyWhitelisted {
        IERC20 PACT = IERC20(_PACT);
