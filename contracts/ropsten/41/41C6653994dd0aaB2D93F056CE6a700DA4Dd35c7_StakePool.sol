...
contract StakePool is ReentrancyGuard, Ownable, Pausable {
    ...
    function redeemRewards() public nonReentrant whenNotPaused redemptionAllowed(msg.sender) {
        ...
        console.log("stakedToken", stakedToken.totalSupply());
        console.log("rewardsToken", rewardsToken.totalSupply());
    ...
    function updateRewardsDistributionRate(uint _rate) external onlyOwner updatePool {
      require(_rate >= 0, "Rate cannot be less than 0");

      rewardsDistributionRate = _rate;
    }

    function updateUnlockTime(uint _minutes) external onlyOwner {
      unlockTime = _minutes;
    }

    function updateRelockTime(uint _minutes) external onlyOwner {
      relockTime = _minutes;
    }
}
