...
contract EpicMeta_Test is ERC721URIStorage, Ownable {
    ...
    constructor(string memory name, string memory symbol, string memory metadata, uint16 num_mint, string memory rarity, uint256 price, address content_creator) ERC721(name,symbol) {
        ...
        for (uint i = 0; i < num_mint; i++){
            mint(content_creator,i+1);
        }
    }
    
    event show_URI(string uri);
...
import "./EpicMeta_Test.sol";
import "./EpicMeta_Token_Test.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract EpicMeta_Test_Factory is Ownable{

    mapping(string => uint256) public max_distribution;
    mapping(address => bool) content_creators;
    mapping(string => bool) deployed;
    address public utility_token_address;
    address public epicmeta_address;
    EpicMeta_Token_Test utility_token_ledger;

    event NFTCreated(address nft_Address);

    constructor() {
        max_distribution["Favourite"] = 10;
        max_distribution["Winning"] = 25;
        max_distribution["Fan"] = 50;
        epicmeta_address = msg.sender;
        whitelist(msg.sender);
    }

    function set_ledger(string memory erc20Name, string memory erc20Symbol, uint256 max_supply) public onlyOwner{
        utility_token_ledger = new EpicMeta_Token_Test(erc20Name,erc20Symbol,max_supply);
        utility_token_address = address(utility_token_ledger);
    }
    
    function whitelist(address content_creator) public onlyOwner{
        content_creators[content_creator] = true;
    }

    function createNFT(string memory name, string memory symbol, string memory metadata, uint16 num_mint, string memory rarity, uint256 price) public returns (address nftAddress){
        require(num_mint <= max_distribution[rarity] && content_creators[msg.sender] == true && deployed[metadata] == false); 
        deployed[metadata] = true;
        EpicMeta_Test moment = new EpicMeta_Test(name, symbol, metadata,num_mint,rarity,price, msg.sender);
        address nft_address = address(moment);
        emit NFTCreated(nft_address);
        return nft_address;
    }

    function mint_tokens(address to,uint256 num_tokens) public onlyOwner{
        utility_token_ledger.mint(to, num_tokens);
    }
...
contract EpicMeta_Token_Test is ERC20Burnable, Ownable {
    uint256 immutable public _max_supply;
    mapping (address => uint256) private _balances;
    ...
    constructor(string memory name_, string memory symbol_, uint256 max_supply) ERC20(name_,symbol_){
        require(max_supply > 0, "Max_supply cannot be 0");
        _name = name_;
        _symbol = symbol_;
    ...
    function mint(address account, uint256 amount) public onlyOwner{
        require(totalSupply() + amount <= _max_supply, "ERC20Capped: cap exceeded");
     ...
     */
    function transfer(address sender, address recipient, uint256 amount) public virtual returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        require(senderBalance > 0, "Sender Balance is 0");
