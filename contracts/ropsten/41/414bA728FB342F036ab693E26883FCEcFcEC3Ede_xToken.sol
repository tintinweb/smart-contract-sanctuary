...
abstract contract AgentManager is AdminManager {
  ...
  function isAgentBool(address _agent) public view returns(bool) {
    return !equalStrings(agents[msg.sender], "");
  }
  ...
  function transferAgentsOwnership(string memory _groupName, address _newAgent) external onlyOwner("Issuer") {
...
interface IExternalList {
  function contains(address _member) external view returns(bool);
}
...
abstract contract MembersManager is AgentManager {
  ...
  function isMemberOfExternalList(string memory _groupName, address _member) external view returns(bool) {
    return IExternalList(externalList[_groupName]).contains(_member);
  }
...
contract xToken is ERC20, CompoundRateKeeper {
  ...
  function _requestMint(address _destination) onlyActiveState private returns(uint256) {
  ...
  function approveMint(uint256 _mintRequestID, uint256 _amount) onlyIssuer external {
    ...
    onlyAuthorizedHolderF(_info.destination);
  ...
  function _safeguardUnstake(uint256 _amount) onlyAuthorizedHolder(msg.sender) private {
