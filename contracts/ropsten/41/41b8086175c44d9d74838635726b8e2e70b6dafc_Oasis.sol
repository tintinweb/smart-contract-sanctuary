...
contract Oasis{
	...
	*/
	        ...
	        if(activedtime > 0 && activedtime < now) {
	            ...
	            if(lid > 0 && allprize[p][0] > allprize[p][1]){
	                if(now  < activedtime  + sysday){
	                    ...
	                    uint d = getyestoday();
	                    if(userlevelsnum[lid][d] > 0) {
	                        ps = (allprize[p][0] - allprize[p][1])/userlevelsnum[lid][d];
	                    }
	...
	function setlevel(address user) private returns(bool) {
	    ...
	    if(my[user].mylevelid > 0) {
	        uint acttime = prizeactivetime[my[user].mylevelid - 1];
	        if(acttime > 0 && now > acttime) {
	...
	function sell(uint256 amount) onlySystemStart() public returns(bool success) {
		...
		//require(tg[tags].userethnumused + amount <= tg[tags].userethnum/2);
		if(dayseths[t] > dayseths[d]){
		    require((daysysdraws[d] + amount) < (dayseths[t]*subper/100));
		}else{
		    require((daysysdraws[d] + amount) < (dayseths[d]*subper/100));
		}
