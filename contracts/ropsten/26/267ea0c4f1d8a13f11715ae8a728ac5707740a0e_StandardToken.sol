...
contract CollateralDetails is Ownable {
    ...
    event record(uint position, string document, string book, uint256 bags, uint256 tokens, bytes32 executionType);
    ...
    function getLastRecord() public view returns(uint256 id,string memory document, string memory book, uint256 bags, uint256 tokens ,bytes32  execType){
        Collateral storage c = _collateral[_collateralList.length-1];
        return (
            _collateralList.length-1, 
            c.cda_wa_document, 
            c.book, 
            c.bags, 
            c.tokenAmount, 
    ...
    function getRecords(uint indexes_) public view returns(string [] memory documents, string[] memory books, uint256[] memory bags, uint256[] memory tokensAmount ,bytes32[] memory executionTypes){
     require(indexes_ <= _collateralList.length, "Invalid indexes, value is greater than the _collateralList!");
       ...
       uint256 [] memory _tokensAmout = new uint256[](indexes_);
       bytes32 [] memory _executionTypes = new bytes32[](indexes_);
       ...
       for( uint i = 0; i < indexes_; i++){
           ...
           _books[i] = c.book;
           _bags[i] = c.bags;
           _tokensAmout[i] = c.tokenAmount;
           _executionTypes[i] = c.executionType;
       }
       return(_documents,_books,_bags,_tokensAmout,_executionTypes);
    }
    
    function getCollateral(uint position_) public view returns(string memory cda_wa_document, string memory book, uint256 bags, uint256 tokenAmount ,bytes32 executionType){
    ...
    function recordCollateral(string memory cda_wa_document_, string memory book_, uint256 bags_, uint256 tokenAmount_, bytes32  executionType_) internal onlyOwner{
