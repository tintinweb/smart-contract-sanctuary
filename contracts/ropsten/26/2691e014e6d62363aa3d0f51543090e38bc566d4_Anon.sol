...
contract Anon is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && recipient != owner() && recipient != _teamWalletAddress && recipient != _marketingWalletAddress) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }

        // Require buyers to wait at least 30 seconds between transactions.
        if (cooldownEnabled && sender == uniswapV2Pair && recipient != address(uniswapV2Router) && !_isExcluded[recipient]) {
        ...
        // higher fees.
        if (cooldownEnabled && sender != uniswapV2Pair && sender != address(uniswapV2Router)) {
        ...
        if (!inSwap && swapEnabled && overMinTokenBalance && sender != uniswapV2Pair) {
            ...
            sendETHToFee(contractTokenBalance);
