...
// Mapping from owner to operator approvals
    ...
    modifier onlyTokenContract() {
        require(msg.sender == _tokenContract, "Sender not allowed to call this function");
        _;
    }

    function setTokenContract(address currentTokenContract) public onlyOwner {
        _tokenContract = currentTokenContract;
    }
    ...
    function getTokenOwnerOf(uint256 tokenId) public view returns (address) {
        return arts[tokenId].owner;
    }

    function getTokenURI(uint256 tokenId) public view returns (string memory) {
        return arts[tokenId].ipfsMetadataHash;
    }

    function getBalance(address _owner) public view returns (uint256 count) {
        return ownerToNFTokenCount[_owner];
    }

    function mintToken(address to, bytes32 _rfid, string memory tokenURI) public onlyTokenContract whenNotPaused returns (uint256) {
        _tokenIds.increment(); 
        uint256 tokenId =  _tokenIds.current();     
        arts[tokenId].ipfsMetadataHash = tokenURI;
        arts[tokenId].owner = to;
        rfids[_rfid] = true;
        ownerToNFTokenCount[to] = ownerToNFTokenCount[to] + 1;
        return tokenId;
    } 

    function burnToken(uint256 tokenId) public onlyTokenContract whenNotPaused {
        address owner = arts[tokenId].owner;
        setTokenApprovals(address(0), tokenId);
        arts[tokenId].ipfsMetadataHash = '';
        arts[tokenId].owner = address(0);
        // rfids[_rfid] = true;
        ownerToNFTokenCount[owner] = ownerToNFTokenCount[owner] - 1;
    }
    
    function transferToken(address to, uint256 tokenId) public onlyTokenContract whenNotPaused {
        address currentOwner = arts[tokenId].owner;
        arts[tokenId].owner = to;
        ownerToNFTokenCount[currentOwner] = ownerToNFTokenCount[currentOwner] - 1;
        ownerToNFTokenCount[to] = ownerToNFTokenCount[to] + 1;
    }

    function setTokenApprovals(address to, uint256 tokenId) public onlyTokenContract whenNotPaused returns (address) {
        return tokenApprovals[tokenId] = to;
    }

    function getTokenApprovals(uint256 tokenId) public view returns (address) {
        require(tokenExists(tokenId), "ERC721: approved query for nonexistent token");
        return tokenApprovals[tokenId];
    }

    function setOperatorApprovalForAll(address sender, address operator, bool approved) public onlyTokenContract whenNotPaused returns (bool) {
...
contract RtistiqToken is Context, ERC165, IERC721, IERC721Metadata, Ownable, Pausable {
    ...
    function balanceOf(address owner) public view virtual override returns (uint256) {
        ...
        return rtistiqBase.getBalance(owner);
    }
    ...
    */
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        //TODO - This function should not be called from outside
        require(RtistiqToken.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own"); // internal owner
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        //TODO - below condition is required??
        require(_msgSender() == owner || RtistiqToken.isApprovedForAll(owner, _msgSender()),
    ...
    function _setApprovalForAll(address from, address operator, bool _approved) internal {
        rtistiqBase.setOperatorApprovalForAll(from, operator, _approved);
        emit ApprovalForAll(from, operator, _approved);
    }

    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        return rtistiqBase.getOperatorApprovalForAll(owner, operator);
    }
...
contract RtistiqCore is RtistiqToken {
  ...
  function createArtworkToken(bytes24 _artId,
    ...
    require(rtistiqBase.rfidExists(rfid) != true, "The given RFID already exists!");
    ...
    uint256 tokenId = _mint(recipient,rfid,metadata);
    return tokenId;
  }

  function transferArtworkOwnership(uint256 tokenId, bytes32 _rfid, address from, address to, bytes memory _signature) public onlyOwner whenNotPaused {
      //hashing of _rfid & _newOwner
      bytes32 _hash = keccak256(abi.encodePacked(_rfid, to));
      //Recover signer address
      address sigOwner = _hash.recover(_signature);
      //signature verification
      require(from == sigOwner, "Signature verification Failed");

      _transfer(from, to, tokenId);   
  }

  //TODO - Approve
  function artworkApprove(uint256 tokenId, bytes32 _rfid, address from, address to, bytes memory _signature) public onlyOwner whenNotPaused {
    //hashing of _rfid
      bytes32 _hash = keccak256(abi.encodePacked(_rfid));
      //Recover signer address
      address sigOwner = _hash.recover(_signature);
      //signature verification
      require(from == sigOwner, "Signature verification Failed");
  ...
  //TODO - ApproveForAll
  function artworkApproveForAll(address from, address to, bytes memory _signature) public onlyOwner whenNotPaused {
    //hashing of to address
      bytes32 _hash = keccak256(abi.encodePacked(to));
      //Recover signer address
      address sigOwner = _hash.recover(_signature);
      //signature verification
      require(from == sigOwner, "Signature verification Failed");
  ...
  function burnArtworkToken(uint256 tokenId, address from, bytes memory _signature) public onlyOwner whenNotPaused {
     //hashing of to address
