...
contract SuperRichCoin is ERC20Pausable, Ownable {
	...
	function transferToLockedBalances(address[] memory receivers, uint256[] memory values) whenNotPaused public {
	...
	function airDropToLockedBalances(address[] memory receivers, uint256[] memory values) whenNotPaused public {
		require(receivers.length != 0);
		require(receivers.length == values.length);
