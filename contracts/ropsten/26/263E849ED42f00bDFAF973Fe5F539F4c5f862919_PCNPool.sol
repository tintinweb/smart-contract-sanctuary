...
// File: contracts/farms/PCN/common/IERC20Metadata.sol
...
// File: contracts/farms/PCN/PCNpool.sol
...
contract PCNPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
...
//        uint256 _poolLimitPerUser,
    ...
    /* PCN Admin functions to setup pool */

    function PCNStartIn(uint256 _blocks) public onlyOwner {
        PCNSetStart(block.number.add(_blocks));
    }

    function PCNSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function PCNSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        PCNCalcRewardPerBlock();
    }

    function PCNCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function PCNSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
