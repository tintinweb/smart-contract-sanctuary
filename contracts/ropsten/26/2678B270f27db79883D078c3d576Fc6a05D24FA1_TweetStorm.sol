...
contract TweetStorm is Ownable {
    ...
    uint256 public totalCampaignCount = 0; // Campaign ID
    ...
    function addCampaign(string memory _name, 
                        ...
                        string memory campaignUri) public returns(uint256) {
        // get a campaign id for a new campaign.
        uint256 campaignId = totalCampaignCount ++;
    ...
    event Tweet(uint256 campaignId, address promoterAddress, string tweetUrl);

    function submitTweet(uint256 campaignId, string memory _tweetUrl, bytes memory signedPermit) public {
        ...
        require(campaigns[campaignId].param.maxBounty > campaigns[campaignId].claimedBounty, 'verify Campaign is still active_2');
