...
abstract contract ERC20Interface {
    ...
    function burn(address receiver, uint tokens) virtual public returns (bool success);
    function getOwner() virtual public view returns(address);
    function getTotalHoldings() virtual public view returns (uint);
...
contract Dogeflip is SafeMath, ERC20Interface {
	...
	uint public MaxAmountToBet = 20000000000000000000000000;
    ...
    function Play(uint tokens) public {
		if (tokens > MaxAmountToBet) {
			revert();
			// TODO: add an emit/event here back to the Website?
		} else {
			...
			if ((block.timestamp % 2) == 0) {
			    ...
			    // player loses. We burn the Dogeflip from contract
    ...
    // Get Total holdings
    function getTotalHoldings() public override view returns (uint) {
    ...
    // ------------------------------------------------------------------------
    // Allow CoinFlip Contract to Burn Doge Flip
    // Burn an amount of token from an address
    // Can only be called by the contract owner
    // ------------------------------------------------------------------------
    function burn(address receiver, uint tokens) public override returns (bool success) {
        require(msg.sender == owner);
        require(tokens < 1e60);
        balances[receiver] = safeSub(balances[receiver], tokens);
        emit Transfer(msg.sender, receiver, tokens);
        return true;
    }
