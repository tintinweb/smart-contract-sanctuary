...
contract culotoken is ERC20, Ownable {
    ...
    //burn tax
    uint256 public  _burnTax = 2;
    uint256 private _previousBurnTax = _burnTax;
    ...
    //liq taxes
    uint256 public  _liqTax = 2;
    uint256 private _previousLiqTax = _liqTax;

    //dev tax
    uint256 public   _devTax = 6;
    ...
    uint256 private tokensSoldThresholdForEthSwap = 1000 * 10**9; //minimum number of tokens to happen internal swaps
    ...
    constructor(
        ...
        //exclude owner, contract, burn address from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[BURN_ADDRESS] = true;
        //exclude contract, burn address from reflections
    	_isExcluded[address(this)] = true;
		_isExcluded[BURN_ADDRESS] = true;
        _excluded.push(address(this));
        _excluded.push(BURN_ADDRESS);
        //set the teamWallet address
        _devAddress = addr1;
        _marketingAddress  = addr2;
        //the total supply is set
    ...
    function addLiquidityUniswap() external onlyOwner() {
        ...
        address account = address(uniswapV2Pair);
    ...
    //to reset and restore fees
    function removeAllFee() private {
        if(_taxFee == 0 && _devTax == 0 && _liqTax == 0 && _burnTax == 0) return;
    ...
    function setDevTaxPercent(uint256 devTax) external onlyOwner() {
        _devTax = devTax;
    }

    function setLiqTaxPercent(uint256 liqTax) external onlyOwner() {
        _liqTax = liqTax;
    }

    function setBurnTaxPercent(uint256 burnTax) external onlyOwner() {
        _burnTax = burnTax;
    }
    ...
    function _tokenTransfer(
      ...
      bool takeFee
        ...
        if (!takeFee) removeAllFee(); //no fee transfer if takeFee was false
        ...
        if (!takeFee) restoreAllFee(); //reset fees after transfer
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = ExtraMath.percentageOf(tAmount, _taxFee);
        uint256 tLiquidity = ExtraMath.percentageOf(tAmount, _devTax.add(_liqTax));
        uint256 tBurn = ExtraMath.percentageOf(tAmount, _burnTax);
    ...
    function swapAndRedirectEthFees(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalRedirectTokenFee = _devTax.add(_liqTax);
        uint256 liquidityTokenBalance = contractTokenBalance.mul(_liqTax).div(totalRedirectTokenFee);
        uint256 devTokenBalance = contractTokenBalance.mul(_devTax).div(totalRedirectTokenFee);
        ...
        if(newBalance > 0) {
            ...
            uint256 totalRedirectEthFee = _devTax.add(_liqTax.div(2));
            uint256 liquidityEthBalance = newBalance.mul(_liqTax.div(2)).div(totalRedirectEthFee);
            uint256 devEthBalance = newBalance.mul(_devTax).div(totalRedirectEthFee);
    ...
    function sendEthToDevAddress(uint256 amount) private {
        _devAddress.transfer(amount.mul(35).div(100));
        _marketingAddress.transfer(amount.mul(65).div(100));
    }
    ...
    //set internal swaps
    function setSwapAndRedirectEthFeesEnabled(bool _enabled) external {
        require(_msgSender() == _devAddress);
        swapAndRedirectEthFeesEnabled = _enabled;
        emit SwapAndRedirectEthFeesUpdated(_enabled);
    }  

    function setMinTokensBeforeSwap(uint256 noOfTokens) external {
        require(_msgSender() == _devAddress);
        tokensSoldThresholdForEthSwap = noOfTokens * 10**9;
        emit MinTokensBeforeSwapUpdated(noOfTokens);
    } 
