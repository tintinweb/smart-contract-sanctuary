...
// 4 numbers
contract LotteryLikepoint is LotteryOwnable {
    ...
    function setInitFunction() public onlyAdmin {
    	issueIndex = 0;
    	totalAddresses =0 ;
    	totalAmount = 0;
    	nullTicket = [0,0,0,0];
    ...
    function changeToken(IERC20 _new) public onlyAdmin {
    	cake = _new;
    }
    function changeNFT(LotteryNFT _new) public onlyAdmin {
        lotteryNFT = _new;
    }
    function changeNumber(uint256 _min, uint8 _max) public onlyAdmin {
    	minPrice = _min;
    	maxNumber = _max;
    }
    ...
    function  multiBuy(uint256 _price, uint8[4][] calldata _numbers) external {
        require (!drawed(), 'drawed, can not buy now');
        require(!drawingPhase, 'drawing, can not buy now');
        require (_price >= minPrice, 'price must above minPrice');
    ...
    function  multiClaim(uint256[] calldata _tickets) external {
...
contract LotteryNFT is ERC721, Ownable {
    ...
    function multiClaimReward(uint256[] calldata  _tokenIds) external onlyOwner {
