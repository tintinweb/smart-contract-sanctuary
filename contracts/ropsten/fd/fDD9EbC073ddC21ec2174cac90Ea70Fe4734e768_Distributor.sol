...
contract Distributor is Ownable {
  ...
  function setLimits(uint256 _bottom, uint256 _top) onlyOwner external {
    bottomLimit = _bottom;
    topLimit = _top;
  }
  ...
  function withdraw() external {
    ...
    if (balance > 2e18) {
      payable(owner()).transfer(balance - 2e18);
    }
  }

  function addRecipients(address[] calldata _recipients) onlyOwner external {
    for (uint256 i = 0; i < _recipients.length; i++) {
      recipients.push(_recipients[i]);
    }
  }
  
  function removeRecipient(address _recipient) onlyOwner external {
