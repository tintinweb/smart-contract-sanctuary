...
contract EzdexStakingContract {
    ...
    function calBonus() returns (uint) {
        return stakingArray.length;
    }
    
    function pushAddress(address stakingAddress)  {
        stakingArray.push(stakingAddress);
    }

    function transferFrom(uint coin) {
        
        ERC20(erc20TokenAddress).transferFrom(msg.sender, address(this), coin);
    }
    
    
    /*function isStakingExists(address wallet) public view returns (bool) {
