...
contract NFTCreature is ERC721, ERC721Enumerable , Ownable {
    ...
    function setUserInfo(uint256 id,string memory name, string memory value) public{
        require(ERC721.ownerOf(id) == _msgSender(), "You must be token owner to update user storage");
    ...
    function rescueNFT(address _nft,uint256 tokenId) public onlyOwner{
        require(address(this) != _nft , "Can't resque own NFTs. use transfer");
        ERC721(_nft).transferFrom(address(this), msg.sender, tokenId);
    }
