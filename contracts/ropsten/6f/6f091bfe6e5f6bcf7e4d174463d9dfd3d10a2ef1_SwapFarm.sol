...
contract SwapFarm is Configurable, ContextUpgradeSafe, Constants {
    ...
    function __SwapFarm_init(address governor_, address WETH_, address swapFactory_, address factory_, address rewardsToken_, address distributor_, address ecoAddr_) external initializer {
        __Governable_init_unchained(governor_);
        __SwapFarm_init_unchained(WETH_, swapFactory_, factory_, distributor_, rewardsToken_, ecoAddr_);
    }
    
    function __SwapFarm_init_unchained(address WETH_, address swapFactory_, address factory_, address rewardsToken_, address distributor_, address ecoAddr_) public governance {
        WETH        = WETH_;
    ...
    function _swapFarming(address currency, uint fee) internal returns (uint reward) {
        ...
        rewards[ecoAddr][currency] = rewards[ecoAddr][currency].add(rwdEco);
    ...
    function getRewardA(address payable acct, address currency) public {
        uint reward = rewards[acct][currency];
        if (reward > 0) {
            ...
            IERC20(rewardsToken).safeTransferFrom(distributor, acct, reward);
