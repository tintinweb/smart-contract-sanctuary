...
contract XCGDMETA is ERC721, ERC721Enumerable, Ownable {
    using SafeMath for uint256;

    uint256 public constant MAX_WHITE_LIST_MINT = 2;
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedUri) ERC721("XCGDMETA", "GGSA") {
    ...
    function mintWhiteList(uint256 numTokens, bytes32[] calldata proof) external payable {
        require(Address.isContract(msg.sender) == false, "Cannot mint from a contract");

        uint256 ts = totalSupply();
        require(_verify(_leaf(msg.sender), proof), "Address is not on allowlist");
        require(whiteListSaleActive, "The pre-sale is not active");
        require(_whiteListClaimed[msg.sender].add(numTokens) <= MAX_WHITE_LIST_MINT, "Purchase would exceed max pre-sale tokens");
        require(ts.add(numTokens) <= TOKEN_LIMIT, "Purchase would exceed max tokens");
        require(msg.value >= TOKEN_PRICE.mul(numTokens), "Ether value sent is below the required price");

        _whiteListClaimed[msg.sender] = _whiteListClaimed[msg.sender].add(numTokens);
        for (uint256 i = 0; i < numTokens; i++) {
            _safeMint(msg.sender, ts.add(i));
        }
    }
