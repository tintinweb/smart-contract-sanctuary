...
contract PortalNft is ERC1155Bouncer {
  ...
  */
  function voucherMint(NFTVoucher calldata voucher, uint numberOfTokens) external payable {
    require(premintTxs[activelyMintingNft][msg.sender] < 1 , "You can only early mint once");
    ...
    require(totalSupply(activelyMintingNft) + numberOfTokens <= maxSupply, "Quantity unavailable");
  ...
  */
  function publicMint(uint numberOfTokens) external payable {
    ...
    require(walletTxs[activelyMintingNft][msg.sender] < 2 , "You can only mint twice");
