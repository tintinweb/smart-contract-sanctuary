...
contract Ticket is ERC721Full, IERC721Creator, Ownable {
    using SafeMath for uint256;
    ...
    ITicket private oldTicket;

    // Event indicating metadata was updated.
    event TokenURIUpdated(uint256 indexed _tokenId, string  _uri);

    event TokenMinted(uint256 indexed _tokenId, string _name);

    address public LOM_ADDRESS = 0xedc3b1d1819Ea39e35D425af2d2d6a79Ce4103dD;
     ...
     */
    function addNewToken(string _uri, string _name) public {
      Token(LOM_ADDRESS).transferFrom(msg.sender, owner(), price);
      _createToken(_uri, msg.sender, _name);
    }
    ...
    function updateNameAndTransfer(uint256 _tokenId, address _receiver, string _name) public onlyTokenOwner(_tokenId) {
    ...
    */
    ...
    function changeName(uint256 _tokenId ,string _name)
      public
      onlyTokenOwner(_tokenId)
    {
        _setName(_tokenId, _name);
    ...
    function _setName(uint256 _tokenId, string _name) internal {
        name[_tokenId] = _name;
    }
     ...
     * @param _creator address of the creator of the token.
     */
    function _createToken(string _uri, address _creator, string _name) internal returns (uint256) {
      uint256 newId = idCounter;
      idCounter++;
      _mint(_creator, newId);
      _setTokenURI(newId, _uri);
      _setTokenCreator(newId, _creator);
      _setName(newId, _name);
      emit TokenMinted(newId, _name);
      return newId;
    }
}