...
contract Gulag is IERC20, Context, Ownable {
    ...
    event BlockedPrisonerSell(uint256 amount);
    event BlockedWhale(uint256 amount);
    event BlockedDump(uint256 amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (amount == 0 || isInternalTransaction) {
    ...
    function _gulagTransfer(address sender, address recipient, uint256 amount) private {
        bool isSellTransaction = recipient == uniswapV2Pair || recipient == address(_uniswapV2Router);
        bool isBuyTransaction = sender == uniswapV2Pair || sender == address(_uniswapV2Router);
    ...
    function _sell(address sender, address recipient, uint256 amount) private {
        ...
        if (block.timestamp < freeDate) {
            emit BlockedPrisonerSell(amount);
        }
    ...
    function _buy(address sender, address recipient, uint256 amount) private {
        ...
        if (currentBalance + amount > _balanceLimit) {
            emit BlockedWhale(amount);
        }
    ...
    // TODO: remove these helper test functions

    // Toggle prison time to test selling, dump protection and lottery
    function testReleasePrisoner(address account) public onlyOwner {
        delete _prisoners[account];
    }

    // Toggle liquidity release time downwards to test LP removal
    function testSetLiquidityReleaseTime(uint256 releaseTime) public onlyOwner {
        liquidityReleaseTime = releaseTime;
    }
}