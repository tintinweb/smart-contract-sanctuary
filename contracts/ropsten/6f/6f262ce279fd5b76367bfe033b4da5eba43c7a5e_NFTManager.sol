...
contract NFTManager is Initializable, OwnableUpgradeSafe, ERC721UpgradeSafe {
    ...
    function initialize(address _yeld, address _yeldies, address _devTreasury, address _chi, string memory baseUri_) public initializer {
        __Ownable_init();
    ...
    function setCHI(address _chi) public onlyOwner {
        chi = _chi;
    }
    ...
    function setYELDIES(address _yeldies) public onlyOwner {
        yeldies = _yeldies;
    }
    ...
    function createBlueprint(string memory _tokenURI, uint256 _maxMint, uint256 _yeldCost, uint256 _yeldiesCost) public onlyOwner {
        uint256[4] memory blueprint = [_maxMint, 0, _yeldCost, _yeldiesCost];
        blueprints[_tokenURI] = blueprint;
        blueprintExists[_tokenURI] = true;
        tokenURIs.push(_tokenURI);
    }

    // Same function as stakeYELD() but with CHI gastoken discount
    function discountedStakeYELD(uint256 _amount) public discountCHI {
        stakeYELD(_amount);
    }

    // Stacking YELD RESETS the staking time
    function stakeYELD(uint256 _amount) public {
        // Check allowance
        uint256 allowance = IERC20(yeld).allowance(msg.sender, address(this));
    ...
    function discountedReceiveYELDIES() public discountCHI {
        receiveYELDIES();
    }

    function receiveYELDIES() public {
        require(amountStaked[msg.sender] > 0, 'You must have YELD staked to receive YELDIES');
        uint256 yeldiesGenerated = getGeneratedYELDIES();
        timeStaked[msg.sender] = block.number;
        IYELDIES(yeldies).mint(msg.sender, yeldiesGenerated);
    }

    function discountUnstakeYELDAndReceiveYELDIES(uint256 _amount) public discountCHI {
        unstakeYELDAndReceiveYELDIES(_amount);
    }
    ...
    function safeMint(string memory _tokenURI) public {
        ...
        uint256 allowanceYELDIES = IERC20(yeldies).allowance(msg.sender, address(this));
        require(allowanceYELD >= blueprints[_tokenURI][2], 'NFTManager: You have to approve the required token amount of YELD to stake');
        require(allowanceYELDIES >= blueprints[_tokenURI][3], 'NFTManager: You have to approve the required token amount of YELDIES to stake');

        uint256 cost = blueprints[_tokenURI][2];
        uint256 dev = blueprints[_tokenURI][2].mul(treasuryPercentage).div(1e20);

        // Payment
        IERC20(yeld).transferFrom(msg.sender, address(this), cost.sub(dev));
        IERC20(yeld).transferFrom(msg.sender, devTreasury, dev);
        IERC20(yeldies).transferFrom(msg.sender, address(this), blueprints[_tokenURI][3]);
    ...
    /// @notice Returns the generated YELDIES after staking YELD but doesn't send them
    function getGeneratedYELDIES() public view returns(uint256) {
        uint256 blocksPassed = block.number.sub(timeStaked[msg.sender]);
        uint256 yeldiesGenerated = amountStaked[msg.sender].mul(blocksPassed).div(oneDayInBlocks);
        return yeldiesGenerated;
    }
