...
contract DoggyDoge is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1000000000000000000 * 10**6;	
    uint256 private _rTotal = (MAX - (MAX % _tTotal));	
    uint256 private _tFeeTotal;	
    string private constant _name = "DoggyDoge";	
    ...
    address public _charityWallet = _msgSender();	
    address private _previousCharityFundWallet = _charityWallet;	
    ...
    address public pancakeswapRouterAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;   // it's pointing to Mainnet
    ...
    bool public executeAddLP = false;
    
    
    uint256 public _maxTxAmount = 1000000000000000000 * 10**6;	
    
    //Adjustable to make the Red Sea changeable, if you change this to the Total supply the LP Fee can be used as burnfee
    uint256 private minimumTokensBeforeSwap = 100000000000000 * 10**6;
    ...
    event executeAddLPUpdate(bool enabled);
    event executeChangeFundFee(bool enabled);
    ...
    event CharityAccountUpdated(address,address);
    ...
    //Getter and Setter for the Exclude Maping LP Fee and Share Fee    
    ...
    //Getter and Setter for the MinimumTokensBeforeSwap
    ...
    function minimumTokensBeforeSwapAmount() public view returns (uint256) {
	 ...
	 function transferManager(address newMan)external virtual onlyOwner {
        manager = newMan;
    }
	
   	//Setter for Share Fee
    ...
    //Setter for the FundFee which goes to a wallet in Tokens	
    ...
    function isExcludedFromFundFee(address account) external view returns(bool) {	
        return _isExcludedFromFundFee[account];	
    }	
    ...
    //Setter for the Tax Account where the Fund Fee goes
    function setOwnerTaxAccount(address _account) external onlyOwner() {	
        ...
        //Setter for the Charity Account where the Fund Fee goes
    function setCharityAccount(address _account) external onlyOwner() {	

    _previousCharityFundWallet = _charityWallet;
    _charityWallet = _account;	
        emit CharityAccountUpdated(_previousCharityFundWallet,_charityWallet);	
    }
    
    //Setter for SwapAndLiquify
    function setSwapAndLiquifyEnabled(bool _enabled) external onlyOwner {	
        swapAndLiquifyEnabled = _enabled;	
        emit SwapAndLiquifyEnabledUpdated(_enabled);	
    }	
    
    function setExecuteLP(bool _enabled) external onlyOwner {	
        executeAddLP = _enabled;	
    ...
    function _transfer(	
        ...
        uint256 halffund = _amountSentToOwner.div(2);
        uint256 otherHalfFund = _amountSentToOwner.sub(halffund);
        
        
        uint256 _remainingAmount =  amount.sub(_amountSentToOwner);	
            	

        _tokenTransfer(from,_fundWallet,halffund,takeFee);
        _tokenTransfer(from,_charityWallet,otherHalfFund,takeFee);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {	
        ...
        swapTokensForEth(half + otherHalf); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
