...
contract MDTOKEN is Context, IERC20, Ownable {
    //library methods
    ...
    string private _name = "MobyDick Token";
    string private _symbol = "MDToken";
    ...
    address payable public antiWhaleAddress = 0x739e4CaA9819C696f7FDBE0c40cCF43Cc6386646;
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!_isExcluded.contains(sender), "Excluded addresses cannot call this function");
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 4, "Exceed limit");
        _taxFee = taxFee;
    }

    function setNumTokensSellToAddToLiquidity(uint256 newNumTokensSellToAddToLiquidity) external onlyOwner() {
        require(newNumTokensSellToAddToLiquidity <= 500 * 10**6 * 10**9, "Exceed limit");
        numTokensSellToAddToLiquidity = newNumTokensSellToAddToLiquidity;
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 4, "Exceed limit");
        _liquidityFee = liquidityFee;
    }

    function setAntiWhaleAddress(address payable newAntiWhaleAddress) external onlyOwner() {
        antiWhaleAddress = newAntiWhaleAddress;
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee.contains(from) || _isExcludedFromFee.contains(to)){
            takeFee = false;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        antiWhaleAddress.transfer(address(this).balance);
