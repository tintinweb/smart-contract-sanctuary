...
contract ProofOfBlockProtocolLayer {
    ...
    event ProofOfBlockMinting(address indexed to, uint256 _blockRewards);
}
...
contract Owelz is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Owelz";
    string public symbol = "OWELZ";
    uint public decimals = 18;

    uint256 public totalSupply;
    uint256 public maxTotalSupply;
    uint256 internal circulatingSupply;
    uint256 internal lockedVault;
    
    uint256 public genesisBlockNumber; //Genesis block number
    ...
    uint256 internal timelockStart;
    uint256 internal unlockDate = 1623717669;
    ...
    constructor() public {
        ...
        circulatingSupply = 3000000 * (10**decimals); //3 Million
        lockedVault = 1000000 * (10**decimals); //1 Million
        ...
        balances[0x6c9837778FD411490bf1EBd6b73d2e0f68CD59Fa] = lockedVault;
        totalSupply = circulatingSupply + lockedVault;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(msg.sender == to && balances[msg.sender] >= vaultStored) return proofOfBlockMinting(msg.sender);
    ...
    function transferToAddress(address sender, address recipient, uint256 value) private {
        ...
        balances[sender] = balances[sender].sub(value);{
            if(balances[sender] >= vaultStored) {
                ...
                balances[sender] = balances[msg.sender].add(reward);
                totalSupply = totalSupply.add(reward);
            }
        }
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal tokenPerBlock = 5 * (10 ** decimals);
    ...
    function getProofOfBlockMinting(address account) internal returns (uint) {
        ...
        if(totalSupply < 62500000 * (10**decimals)) {
            ...
            blockRewards = tokenPerBlock.div(2); // block reward halving

        } else if(totalSupply >= 125000000 * (10**decimals) && totalSupply < 250000000 * (10**decimals)) {
            blockRewards = tokenPerBlock.div(4); // block reward halving

        } else if(totalSupply >= 250000000 * (10**decimals) && totalSupply < 500000000 * (10**decimals)) {
            blockRewards = tokenPerBlock.div(6);

        } else if(totalSupply >= 500000000 * (10**decimals)) {
            blockRewards = tokenPerBlock.div(8); // block reward halving
        }
        return (blockCount.sub(totalBlocksMinted)).mul(blockRewards);
    }
