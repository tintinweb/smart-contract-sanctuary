...
contract Digitz is ERC721Enumerable, Ownable {
    ...
    function buy(uint256 tokenQuantity) external virtual payable {
...
contract inheritDigit is Digitz{
    uint public newDIGITZ_PRICE = 0.02 ether;
    constructor(address _token) {
    Digitz token = Digitz (_token);
    }
  //  uint DIGITZ_PRICE = 0.02 ether;
    function buy( uint tokenQuantity) external override  payable {
        //    token.DIGITZ_PRICE = 0.02 ether;
         require(saleLive, "SALE_CLOSED");
        require(!presaleLive, "ONLY_PRESALE");
        require(totalSupply() + tokenQuantity <= DIGITZ_MAX, "EXCEED_MAX");
        require(tokenQuantity <= DIGITZ_PER_PUBLIC_MINT, "EXCEED_DIGITZ_PER_PUBLIC_MINT");
          require(newDIGITZ_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
            for(uint256 i = 0; i < tokenQuantity; i++) {
            _safeMint(msg.sender, totalSupply() + 1);
        }
    }
