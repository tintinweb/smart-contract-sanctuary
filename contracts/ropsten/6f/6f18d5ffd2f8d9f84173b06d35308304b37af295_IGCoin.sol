...
contract IGCoin is DeaultERC20 {
    ...
    uint256 public constant FIXED_3 = 0x07fffffffffffffffffffffffffffffff;//0x03fffffffffffffffffffffffffffffff;//0x07fffffffffffffffffffffffffffffff;
    ...
    */
    ...
    function sell(uint256 amount) public returns (uint256 revenue){
        ...
        (p, ps) = power(1000008,1000000,(uint32)(1e8+1e8*_totalSupply/SU),1e8); // Calculate exponent
        p=(S*p)>>ps;

        uint256 p2 = 0;
        uint8 ps2 = 0;

        (p2, ps2) = power(1000008,1000000,(uint32)(1e8+1e8*a/SU),1e8); // Calculate exponent
        p2=(S*p2)>>ps2;
