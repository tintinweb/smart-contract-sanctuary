...
interface IMonsterBudsV2 {
    ...
    */
    ...
    /**
     * @dev user can buy the minted token and transfer the token from owner to buyer.
     *
     * Returns
     * - boolean.
     *
    ...
    */
    ...
    function buyCollectiable(uint256 token_id) external payable returns(bool);
    ...
    */
    ...
    function selfBreedCollectiable( uint256 req_token_id, uint256 accept_token_id) external payable returns (uint256);
...
import "./imx_upgradable/utils/MintingUpgradable.sol";
...
contract MonsterbudsV2 is MonsterBudHolders, ERC721URIStorageUpgradeable, IMonsterBudsV2{
    ...
    */
    ...
    function list(uint256 token_id, uint256 price, uint256 expiry_time) external returns(uint256){
       ...
       tokenListDetails storage data = tokenListInfo[token_id];
    ...
    function delist(uint256 token_id) external{        
       ...
       require(ownerOf(token_id) == msg.sender, "$MONSTERBUDS: You are not owner of token");
       require(tokenListInfo[token_id].tokenId != 0, "$MONSTERBUDS: token is not listed");
       delete tokenListInfo[token_id];
       emit ListTokenDetails(ownerOf(token_id), token_id, 0, 0);
    }
    ...
    */
     ...
     * Requirements:
     * - `token_id` must exist.
     * - owner must not buy owned token.
     *
     * Returns
     * - boolean.
     *
    ...
    */
    ...
    function buyCollectiable(uint256 token_id) external override payable returns(bool){
        address payable owner = payable(ownerOf(token_id));

        require(owner != msg.sender, "$MONSTERBUDS: Cannot buy owned bud");
        require(tokenListInfo[token_id].tokenId != 0, "$MONSTERBUDS: token is not listed");
        require(tokenListInfo[token_id].expiryTime < block.timestamp, "$MONSTERBUDS: listed token is expired");
        require(tokenListInfo[token_id].price  == msg.value && tokenListInfo[token_id].price != 0, "$MONSTERBUDS: Price is incorrect");
        ...
        owner.transfer(remainAmount); // transfer remaining 90% ethers of msg.value to owner of token
    ...
    */
     ...
     * 
     * @param req_token_id token Id of msg.sender.
     * @param accept_token_id token Id of msg.sender.
     *
     * Returns
     * - new token count.
     *
    ...
    */
    ...
    function selfBreedCollectiable(uint256 req_token_id, uint256 accept_token_id) external override payable returns (uint256) {
        require(breedValue == msg.value, "$MONSTERBUDS: Amount is incorrect");
        require(req_token_id >= 10 && accept_token_id >= 10, "$MONSTERBUDS: PPP Monsters cannot breed");  // 865
        address owner_req = (ownerOf(req_token_id));
        address owner_accept = (ownerOf(accept_token_id));

        require(selfBreedStatus == true,"$MONSTERBUDS: Breeding is closed");
        require(owner_req == owner_accept && owner_req == msg.sender && req_token_id != accept_token_id, "$MONSTERBUDS: Cannot Self Breed");
        require(breedingInfo[req_token_id].breedCount < 2 && breedingInfo[accept_token_id].breedCount < 2, "$MONSTERBUDS: Exceeds max breed count");
        require(block.timestamp >= breedingInfo[req_token_id].timstamp && block.timestamp >= breedingInfo[accept_token_id].timstamp,"$MONSTERBUDS: cannot breed now");
        ...
        uint countOfReq = breedingInfo[req_token_id].breedCount;
        uint countOfAccept = breedingInfo[accept_token_id].breedCount;
        ...
        tokenCounter = tokenCounter + 1;
        breedInfomation1 storage req_data = breedingInfo[req_token_id];
        req_data.tokenId = req_token_id;
        req_data.breedCount = countOfReq + 1;
        req_data.timstamp = block.timestamp + 300;

        breedInfomation1 storage accept_data = breedingInfo[accept_token_id];
        accept_data.tokenId = accept_token_id;
    ...
    */
    ...
    function MinitableInitialization(address _imx) external onlyOwner {
        imx = _imx;
    }
    ...
    ) external payable onlyIMX {
        require(quantity == 1, "Mintable: invalid quantity");
        require(msg.value == tokenValue, "Monsterbuds: price is incorrect");
        (uint256 id, bytes memory blueprint) = MintingUpgradable.split(mintingBlob);
...
import "./BytesUpgradable.sol";

library MintingUpgradable {
    ...
    {
        int256 index = BytesUpgradable.indexOf(blob, ":", 0);
        require(index >= 0, "Separator must exist");
        // Trim the { and } from the parameters
        uint256 tokenID = BytesUpgradable.toUint(blob[1:uint256(index) - 1]);
