...
contract MockCheapTask is Ownable2, MockProviderModule {
    ...
    function cancelTasks(TaskReceipt[] calldata _taskReceipts)
        external
        payable
    {
        try
            IGelatoCore(gelatoCore).multiCancelTasks(_taskReceipts)
        {} catch Error(string memory error) {
            revert(string("MockCheapTask.cancelTasks:").suffix(error));
        } catch {
            revert("MockCheapTask.cancelTasks:undefined");
        }
    }
