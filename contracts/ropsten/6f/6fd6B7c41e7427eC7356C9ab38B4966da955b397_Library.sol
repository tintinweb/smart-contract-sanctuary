...
contract Library {
    ...
    function addKnjiga(string memory ISBN, string memory avtor, string memory naslov) public{
        knjige[stevecKnjig] = Knjiga(stevecKnjig, ISBN, avtor, naslov, true, int(-1), uint(1), uint(1), uint(0));
        stetjeKnjig();
    }
    ...
    //Ko nekdo knjigo prevzame
    event podaljsanjeKnjige (int Oseba, string Knjiganaslov);
    function podaljsanje(int OsebaID, string memory naslovKnjige) public jeIzposojena(naslovKnjige){
        for(uint i; i<= stevecKnjig; i++){
            if(keccak256(abi.encodePacked(knjige[i].naslov)) == keccak256(abi.encodePacked(naslovKnjige))){
               if(knjige[i].moznostIzposoje == false && knjige[i].stPodaljsanj < 3){
                   knjige[i].datumVrnitve += 259200;
                   emit podaljsanjeKnjige(OsebaID, naslovKnjige);
                   knjige[i].stPodaljsanj += 1;
               }
            }    
        }
    }
