...
contract Distribute is Ownable  {
    /**
       ...
       */
    function doDistribute(address _tokenAddr, address[] memory dests, uint256 values) onlyOwner public
    returns (uint256) {
        uint256 i = 0;
        while (i < dests.length) {
            ERC20(_tokenAddr).transferFrom(msg.sender, dests[i], values * (10 ** 18));
            i += 1;
        }
        return(i);
    }
