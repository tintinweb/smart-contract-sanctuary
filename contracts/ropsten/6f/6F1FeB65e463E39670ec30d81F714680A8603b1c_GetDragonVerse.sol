...
contract GetDragonVerse is ERC721, Ownable {
    ...
    uint public maxItemsPerPreSale = 2; // Mutable by owner
    uint public maxItemsPerSale = 2; // Mutable by owner
    string public _baseTokenURI;
    bool public preMintPaused = true;
    bool public publicMintPaused = true;
    mapping(address => uint) public whitelistedPreSale; //Array of address qualified to mint pre sale
    mapping(address => uint) public whitelistedSale; //Array of address qualified to mint public sale
    ...
    function preMint() external payable {
        ...
        require(amount <= whitelistedPreSale[msg.sender], "preMint: Amount greater than allocation");
        preMintCount += amount;
        whitelistedPreSale[msg.sender] -= amount;
    ...
    function publicMint() external payable {
        ...
        require(amount <= maxItemsPerSale, "publicMint: Max 2 per tx");

        require(totalMinted + amount <= maxItems, "publicMint: Surpasses cap");
        require(amount <= whitelistedSale[msg.sender], "publicMint: Amount greater than allocation");
        whitelistedSale[msg.sender] -= amount;
        _mintWithoutValidation(msg.sender, amount);
    }
    ...
    function addToWhitelistPreSale(address[] memory toAdd) external onlyOwner {
        for(uint i = 0; i < toAdd.length; i++) {
            whitelistedPreSale[toAdd[i]] = maxItemsPerPreSale;
        }
    }
    
     function addToWhitelistSale(address[] memory toAdd) external onlyOwner {
        for(uint i = 0; i < toAdd.length; i++) {
            whitelistedSale[toAdd[i]] = maxItemsPerPreSale;
        }
    }
    ...
    function setmaxItemsPerSale(uint _maxItemsPerSale) external onlyOwner {
        maxItemsPerSale = _maxItemsPerSale;
    }

    function setmaxItemsPerPreSale(uint _maxItemsPerPreSale) external onlyOwner {
