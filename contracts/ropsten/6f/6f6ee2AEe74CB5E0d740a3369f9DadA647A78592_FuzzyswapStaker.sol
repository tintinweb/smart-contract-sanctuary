...
contract FuzzyswapVirtualPool is IFuzzyswapVirtualPool{
    ...
    uint32 public override globalTimeSpent;
    ...
    function finish(uint32 _endTimestamp, uint32 previousTimestamp) external override onlyPool{
        ...
        globalSecondsPerLiquidityCumulative = globalSecondsPerLiquidityCumulative +
                    ((uint160(_endTimestamp-previousTimestamp) << 128) / (prevLiquidity > 0 ? prevLiquidity : 1));
        globalTimeSpent = globalTimeSpent + (_endTimestamp - previousTimestamp);
    }
    ...
    function processSwap(
        ...
        previousTimestamp = previousTimestamp < initTimestamp ? initTimestamp : previousTimestamp;
        globalSecondsPerLiquidityCumulative = globalSecondsPerLiquidityCumulative +
                    ((uint160(currentTimestamp-previousTimestamp) << 128) / (prevLiquidity > 0 ? prevLiquidity : 1));
        prevLiquidity = currentLiquidity;
        globalTimeSpent = globalTimeSpent + (currentTimestamp - previousTimestamp);
    }
...
library PoolAddress {
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x71dd59e800258eacc0dd4c3aebfbd204978d13df86428f526f81bd49c3c488cc;
...
interface IFuzzyswapVirtualPool{
    ...
    function globalTimeSpent() external returns(uint32);
    ...
    function finish(uint32 _endTimestamp, uint32 previousTimestamp) external;
