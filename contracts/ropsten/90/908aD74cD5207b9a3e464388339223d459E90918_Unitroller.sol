...
contract Unitroller is UnitrollerAdminStorage, ComptrollerErrorReporter {
    ...
    constructor(address registry_) public {
        ...
        registry = RegistryForComptroller(registry_);
    }
    ...
    function _upgradeLnSystemVersion(uint256 newSystemVersion, bytes calldata becomeImplementationData) external returns (uint) {
        ...
        // Update Version
        bool updateSuccessful = RegistryForComptroller(registry).updateLnVersion(newSystemVersion);
        require(updateSuccessful, "Version update failed");

        // First, update the implementation used by the unitroller
        address comptrollerImplementation = RegistryForComptroller(registry).getImplementationForLn(address(this), unitrollerContractHash);
...
import "../../../Peripheral/RainMaker/RainMakerInterface.sol";
import "../../../Peripheral/RainMaker/RainMaker.sol";

interface RegistryForComptroller {
...
contract UnitrollerAdminStorage {
    ...
    */
    RegistryForComptroller public registry;
...
contract ComptrollerV5Storage is ComptrollerV4Storage {
    /// OLA_ADDITIONS : All below this line

    /// @note Borrow requests for less than this USD amount will not be approved.
    uint public minBorrowAmountUsd;

    // TODO : CRITICAL : Change to address
    RainMaker public rainMaker;

    /// @note Time period (in seconds) in which liquidation is to be limited (e.g: 60 for one minute)
//    uint public freshLiquidationLimitedPeriod;

    /// @note The liquidators that are allowed to liquidate while still in the 'freshLiquidationLimitedPeriod'
...
contract ComptrollerStorageOlaV0_02 is ComptrollerV5Storage {
    // Underlying asset -> contractNameHash -> deployed oTokens
    mapping(address => mapping(bytes32 => address)) public existingMarketTypes;

    // TODO : CRITICAL : Change to 'bouncer'
    // An address to turn to in order to check if an account is approved for specific actions.
...
import "../../OlaSystem/InterestRateModels/InterestRateModel.sol";
...
interface RegistryForOToken {
    ...
    function olaFactor() external view returns (uint);
}
...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function _reduceReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        // OLA_ADDITIONS : Transfer also to Ola Bank.
        // doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
        doTransferOut(admin, adminPart);
        doTransferOut(olaBankAddress, olaPart);

        emit ReservesReduced(admin, adminPart, olaBankAddress, olaPart, totalReservesNew);
    ...
    /**
     * OLA_ADDITIONS: This function
     * @dev Fetches the ola reserves factor.
     */
    function fetchOlaReserveFactorMantissa() internal returns (uint) {
        return RegistryForOToken(comptroller.getRegistry()).olaFactor();
...
interface Erc20ForRainMaker {
    function balanceOf(address _owner) external view returns (uint256 balance);
    function transfer(address _to, uint256 _value) external returns (bool success);
}

contract RainMaker is RainMakerStorage, RainMakerInterface, ExponentialNoError {
    ...
    function grantCompInternal(address user, uint amount) internal returns (uint) {
        Erc20ForRainMaker lnToken = Erc20ForRainMaker(getLnIncentiveTokenAddress());
    ...
    /**
     * @notice Begins transfer of admin rights. The newPendingAdmin must call `_acceptAdmin` to finalize the transfer.
     * @dev Admin function to stop the accumulation of extra incentives in this rainmakers
     */
...
contract CErc20StorageV0_01 is CErc20Interface {}

contract CErc20InterfaceV0_01 is CErc20StorageV0_01 {}

contract CErc20InterfaceV0_02 is CErc20StorageV0_01 {}

contract CEtherInterface {}

contract CEtherStorageV0_01 is CEtherInterface {}

contract CEtherInterfaceV0_01 is CEtherStorageV0_01 {}

contract CEtherInterfaceV0_02 is CEtherStorageV0_01 {}
...
contract RainMakerStorage {
    ...
    */
    ...
    /// @note The address of the erc20 token used by the network for participation incentive.
