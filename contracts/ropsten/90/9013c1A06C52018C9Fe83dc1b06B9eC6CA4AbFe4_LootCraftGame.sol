...
interface IERC721Metadata is IERC721 {
     ...
     */
    function tokenURI(uint256 ennemyId, uint256 lootId) external view returns (string memory);
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function tokenURI(uint256 ennemyId, uint256 lootId) public view virtual override returns (string memory) {
        require(_exists(ennemyId), "ERC721Metadata: URI query for nonexistent token");

        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, ennemyId.toString(),lootId.toString())) : "";
    }
...
contract LootCraftGame is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getEnnemies(uint256 ennemyId) public view returns (string memory) {
        return pluck(ennemyId, "V2MONSTER2", ennemies);
    }
    ...
    function battleEnnemies(uint256 ennemyId, uint256 lootId) public view returns (string memory) {
       if (randomizeBattle(ennemyId, lootId)) {
    ...
    function tokenURI(uint256 ennemyId, uint256 lootId) override public view returns (string memory) {
        string[39] memory parts;
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base"';
        parts[1] = '';
        parts[2] = getRarity(random(string(abi.encodePacked("ENNEMIES", toString(ennemyId)))));
        parts[3] = battleEnnemies(ennemyId, lootId);
        parts[4] = '';
        parts[5] = '</text><text x="10" y="40" class="base">';
        parts[6] = " with Loot #";
        parts[7] = toString(lootId);
        parts[8] = '</text><text x="10" y="60" class="base">';
        parts[9] = "You win Bag #";
        parts[10] = toString(ennemyId);
        parts[11] = ' : </text><text x="10" y="80" class="base"';
        parts[12] = getRarity(random(string(abi.encodePacked("NATURE", toString(ennemyId)))));
        parts[13] = getNature1(ennemyId);

        parts[14] = '</text><text x="10" y="100" class="base"';
        parts[15] = getRarity(random(string(abi.encodePacked("NATURE2", toString(ennemyId)))));
        parts[16] = getNature2(ennemyId);
        
        parts[17] = '</text><text x="10" y="120" class="base"';
        parts[18] = getRarity(random(string(abi.encodePacked("NATURE3", toString(ennemyId)))));
        parts[19] = getNature3(ennemyId);

        parts[20] = '</text><text x="10" y="140" class="base"';
        parts[21] = getRarity(random(string(abi.encodePacked("MINERAL", toString(ennemyId)))));
        parts[22] = getMineral1(ennemyId);

        parts[23] = '</text><text x="10" y="160" class="base"';
        parts[24] = getRarity(random(string(abi.encodePacked("MINERAL2", toString(ennemyId)))));
        parts[25] = getMineral2(ennemyId);
        
        parts[26] = '</text><text x="10" y="180" class="base"';
        parts[27] = getRarity(random(string(abi.encodePacked("MINERAL3", toString(ennemyId)))));
        parts[28] = getMineral3(ennemyId);

        parts[29] = '</text><text x="10" y="200" class="base"';
        parts[30] = getRarity(random(string(abi.encodePacked("MONSTER", toString(ennemyId)))));
        parts[31] = getMonster1(ennemyId);

        parts[32] = '</text><text x="10" y="220" class="base"';
        parts[33] = getRarity(random(string(abi.encodePacked("MONSTER2", toString(ennemyId)))));
        parts[34] = getMonster2(ennemyId);
        
        parts[35] = '</text><text x="10" y="240" class="base"';
        parts[36] = getRarity(random(string(abi.encodePacked("MONSTER3", toString(ennemyId)))));
    ...
    //Claim
    //function play(uint256 ennemyId) public payable nonReentrant {
    function play(uint256 ennemyId, uint256 lootId) public payable nonReentrant {
        require(ennemyId > 0 && ennemyId < 2000, "Ennemy ID is invalid");
        //(msg.value>=10000000000000000, "Must send minimum value to purchase (0.01 E) !");
        //require(battleEnnemies(ennemyId, lootId), "You lost");
        
        if (randomizeBattle(ennemyId, lootId)) {
             require(msg.value>=20000000000000000, "You win ! But you must send minimum value to claim this Bag (0.02 E) !");
            _safeMint(_msgSender(), ennemyId);
        } else {
            battleEnnemies(ennemyId, lootId);
        }
    }
    ...
    constructor() ERC721("LootCraft Game", "CRAFT") Ownable() {}
}
