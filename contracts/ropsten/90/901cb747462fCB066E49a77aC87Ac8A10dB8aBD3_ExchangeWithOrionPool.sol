...
import "./Exchange.sol";
import "./utils/orionpool/periphery/interfaces/IOrionPoolV2Router02.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/SafeERC20.sol";
...
*/
    ...
    function fillThroughOrionPool(
        ...
        require(filledAmounts[orderHash]==0, "filledAmount already has some value");
        //updateFilledAmount
        filledAmounts[orderHash] += filledAmount; //it is safe to add ui112 to each other to get i192
        require(filledAmounts[orderHash] <= order.amount, "E12B");
        }
        ...
        if(order.buySide==1){ /* NOTE BUY ORDER ************************************************************/
            ...
            if(!orionpoolAllowances[order.quoteAsset]) {
                IERC20(order.quoteAsset).safeIncreaseAllowance(_orionpoolRouter, 2**256-1);
                orionpoolAllowances[order.quoteAsset] = true;
            }
            uint[] memory amounts = IOrionPoolV2Router02(_orionpoolRouter)
            ...
            // Update User's balances
            updateOrderBalance(order, filledAmount, amountQuote, true);
            ...
            if(!orionpoolAllowances[order.baseAsset]) {
                IERC20(order.baseAsset).safeIncreaseAllowance(_orionpoolRouter, 2**256-1);
                orionpoolAllowances[order.baseAsset] = true;
            }
            ...
            // Update User's balances
            updateOrderBalance(order, filledAmount, amountQuote, false);
...

{
  ...
  "evmVersion": "istanbul",
  "libraries": {
    "/C/work/orion-13/orion-exchange-private/contracts/libs/LibValidator.sol": {
      "LibValidator": "0xfF4437662e8F4634eB806984C30Cf3Cc9e45d839"
    },
    "/C/work/orion-13/orion-exchange-private/contracts/libs/LibUnitConverter.sol": {
      "LibUnitConverter": "0x21dA232a1D63A63468579Bb2c53263a38745C94f"
    },
    "/C/work/orion-13/orion-exchange-private/contracts/libs/MarginalFunctionality.sol": {
