...
contract ReputationSystem is IReputationSystem, Initializable, AbstractDependant {
    ...
    uint256 public constant LEAST_TRUSTED_VOTER_REPUTATION = 15; // 1.5
    uint256 public constant MINIMUM_TRUSTED_VOTERS = 5;
    
    address public claimVoting;
    
    uint256 internal _trustedVoterReputationThreshold; // 1.5    
    ...
    modifier onlyClaimVoting() {
        require (claimVoting == msg.sender, "ReputationSystem: Caller is not a ClaimVoting contract");
        _;
    }

    function __ReputationSystem_init() external initializer {
        _trustedVoterReputationThreshold = 15;
    ...
    function setNewReputation(
    ...
    ) external override onlyClaimVoting {
        ...
        if (_reputation[voter] == 0) {
            _votedOnceCount++;
            _roundedReputations[REPUTATION_PRECISION / 3]++;
            _reputation[voter] = PRECISION;
        }
    ...
    function _recalculateTrustedVoterReputationThreshold() internal {        
        uint256 trustedVotersAmount = 
            Math.max(MINIMUM_TRUSTED_VOTERS, _votedOnceCount.mul(PERCENTAGE_OF_TRUSTED_VOTERS).div(PERCENTAGE_100));
        uint256 votersAmount;
        uint256 halfReputationPrecision = REPUTATION_PRECISION / 2;

        for (uint8 i = REPUTATION_PRECISION - 1; i > halfReputationPrecision; i--) {
            uint256 roundedReputation = _roundedReputations[i];
            votersAmount = votersAmount.add(roundedReputation);

            if (votersAmount >= trustedVotersAmount) {
                if (votersAmount >= trustedVotersAmount.mul(3).div(2) && 
                    votersAmount > roundedReputation) {
                    i++;
                }
    ...
    function getNewReputation(address voter, uint256 percentageWithPrecision) 
        ...
        returns (uint256) 
    {                
        return getNewReputation(_reputation[voter] == 0 ? PRECISION : _reputation[voter], percentageWithPrecision);
    }

    function getNewReputation(
        uint256 voterReputation,
        uint256 percentageWithPrecision
    ) public pure override returns (uint256) {
        require (percentageWithPrecision <= PERCENTAGE_100, "ReputationSystem: Percentage can't be more than 100%");
        ...
        if (percentageWithPrecision >= PRECISION.mul(50)) {
            return Math.min(MAXIMUM_REPUTATION, voterReputation.add(percentageWithPrecision.div(100).div(20)));
        } else {
            ...
            return Math.max(MINIMUM_REPUTATION, voterReputation.sub(fraction));
        }
    }
    ...
    /// @dev this function will count voters as trusted that have initial reputation >= 1.5
    ...
    function reputation(address user) public view override returns (uint256) {
