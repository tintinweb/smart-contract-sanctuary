...
interface Market {
    function placeOrder(address _creator, uint256 _tokenId, uint256 _editions, uint256 _pricePerNFT, uint256 _saleType, uint256 _startTime, address _paymentToken)  external returns (bool);
    function importOrder(address _creator, uint256 _tokenId, uint256 _editions, uint256 _pricePerNFT, uint256 _saleType, uint256 _startTime, address _paymentToken)  external returns (bool);
}
...
contract SeedifyLaunchpadNFT is ERC1155, Ownable, Pausable, ERC1155Burnable {
    ...
    {        
        ...
        Market(marketPlaceAddress).placeOrder(account, tokenIDs, _editions, _pricePerNFT, _saleType, _startTime, _paymentToken);    
    }
    ...
    {        
        ...
        require(checkTokenAddress(_paymentToken), "Invalid token address"); 
        _setTokenUri(tokenIDs, _uri);
        Market(marketPlaceAddress).importOrder(_creator, _tokenId, _editions, _pricePerNFT, _saleType, _startTime, _paymentToken);    
    }
