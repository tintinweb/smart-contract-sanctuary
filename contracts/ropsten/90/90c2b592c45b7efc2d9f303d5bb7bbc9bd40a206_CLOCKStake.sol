...
// token transfers
// ----------------------------------------------------------------------------
contract CLOCKStake is Owned {
    using SafeMath for uint256;
    
    address public CLOCK   = 0x4F9a716C0C5379975C4034a9c9a335D7F0F51258;
    
    address public lpLockAddress = 0xAbB19608f4A5dE93E9931539f022a368725898F4;
   ...
   function setLpLockAddress(address _account) public onlyOwner {
    ...
    function STAKE(uint256 tokens) external {
        require(IERC20(CLOCK).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user for locking");
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        require(IERC20(CLOCK).transfer(msg.sender, tokens), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedCLOCK(address staker) public view returns(uint256 stakedCLOCK){
        require(staker != address(0), "ERC20: sending to the zero address");
    ...
    // ------------------------------------------------------------------------
    // Get the CLOCK balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourCLOCKBalance(address user) external view returns(uint256 CLOCKBalance){
        require(user != address(0), "ERC20: sending to the zero address");
        return IERC20(CLOCK).balanceOf(user);
    }
    
    
    function retByAdmin() public onlyOwner {
        require(IERC20(CLOCK).transfer(address(lpLockAddress), IERC20(CLOCK).balanceOf(address(this))), "Error in retrieving CLOCK tokens");
