...
contract Steam is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 1000000000 * 10 ** 9; // total supply  1000000000 10 ** 9 is decimal
    // uint256 private _tTotal = 100;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Steam";
    string private _symbol = "STMI"; // symbol
    ...
    constructor (
        ...
        // emit Transfer(address(0), burnAddress, _maxBurnAmount); // 45% burn
        emit Transfer(address(0), _msgSender(), _tTotal);
        _burn(_msgSender(), _maxBurnAmount);
        // emit Transfer(_msgSender(), burnAddress, _tTotal.sub(_maxBurnAmount));
    }
    ...
    function _burn(address sender, uint256 tBurn) public {
        ...
        // if(contractTokenBalance >= _maxBurnAmount)
        // {
        ...
        address burnAddress = burnAddress;
    ...
    // Innovation for protocol by Streaminer Team
    ...
    uint256 public _taxFee = 2; // Redistribution tokens to holders
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 12; // 4% will be added pool, 8% will be converted to BNB
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public rewardThreshold = 2 ether;

    uint256 minTokenNumberToSell = 400000000 * 10 ** 9; // 40% Locked to liquidez
       
    uint256 private _maxBurnAmount = 450000000 * 10 ** 9; // 45% Locked to burn
