...
contract bettingInu is ERC20("WOOINU used for betting", "BINU"), ERC20Snapshot{
    ...
    */
    function checkForLock(address from, uint256 amount) internal view {
        ...
        unchecked{
        require(balanceOf(from) - fromLock.amount >= amount, "BINU: You can only transfer unLocked BINU's");
        }
        }
    }
    ...
    */
    function collectRewards(uint256 index) external exists(index){
        ...
        _bet memory bet_ = _bets[index]; 
        require (block.timestamp < bet_.end, "BINU: The betting period has ended");
        require (bets_[sender][index].amount != 0, "BINU: You haven't voted on that yet");
