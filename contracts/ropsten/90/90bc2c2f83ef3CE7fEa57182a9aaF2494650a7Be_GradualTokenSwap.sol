...
contract GradualTokenSwap is  ReentrancyGuard {
    ...
    event tokensSwapped(uint royaToken);

 

    constructor (IERC20 _mROYA, IERC20 _ROYA,address _wallet) public {
        mROYA =_mROYA;
        ROYA = _ROYA;
        owner=_wallet;
    }
    ...
    function swapTokens(uint _amount) external nonReentrant() whenNotPaused(){
        uint royaAmount;
        mROYA.safeTransferFrom(msg.sender,address(this),_amount);
        royaAmount=_amount.add(_amount.mul(ratio).div(DENOMINATOR));
        ROYA.safeTransfer(msg.sender,royaAmount);
        emit tokensSwapped(royaAmount);
    }
    ...
    function changeRatio(uint _ratio) external onlyOwner{
        ratio=_ratio;
    }
