...
contract YELDIES is Initializable, OwnableUpgradeSafe, ERC20UpgradeSafe {
    using SafeMath for uint256;
    mapping (address => bool) public isFrozen;
    address public nftManager;

    modifier onlyManager {
        require(msg.sender == nftManager || msg.sender == owner(), 'YELDIES: Only executable by the NFTManager contract or owner');
        _;
    }
    
    function initialize(address _nftManager) public initializer {
        __Ownable_init();
        __ERC20_init('YELDIES', 'YELDIES');
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(!isFrozen[msg.sender], 'YELDIES: Your transfers are frozen');
        require(sender != address(0), "YELDIES: ERC20: transfer from the zero address");
        require(recipient != address(0), "YELDIES: ERC20: transfer to the zero address");
        _beforeTokenTransfer(sender, recipient, amount);
        _balances[sender] = _balances[sender].sub(amount, "YELDIES: ERC20: transfer amount exceeds balance");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }
