...
contract Factory is Configurable, ContextUpgradeSafe, Constants {
    ...
    function _transfer(address payable sender, address callOrPut, address undOrCur, int vol, int max, address[] memory path) internal {
        ...
        uint deadline = config[_deadline_];
    ...
    function _checkMistakeETH(address payable sender, address underlying, address currency, int dUnd, int dCur, address[] calldata pathUnd, address[] calldata pathCur) internal {
        address WETH = address(config[_WETH_]);
        if(!(msg.value == 0
            || (pathUnd.length > 0 && pathUnd[0] == WETH && dUnd > 0)
            || (pathCur.length > 0 && pathCur[0] == WETH && dCur > 0)
            || (pathUnd.length == 0 && underlying == WETH && dUnd > 0)
            || (pathCur.length == 0 && currency == WETH && dCur > 0))
        )
            sender.transfer(msg.value);
    }
    ...
    function swapRoute(address[2] calldata und_cur, uint[2] memory floor_cap, int[2] calldata dCallPut, int[2] calldata maxes, bytes calldata data, address[][2] calldata paths) external payable returns (address[2] memory call_put, int[2] memory dUndCur) {
        ...
        (dUndCur[0], dUndCur[1], floor_cap[0], floor_cap[1]) = calcDelta(floor_cap[0], floor_cap[1], Call(call_put[0]).totalSupply(), Put(call_put[1]).totalSupply(), dCallPut[0], dCallPut[1]);
        ...
        _checkMistakeETH(_msgSender(), und_cur[0], und_cur[1], dUndCur[0], dUndCur[1], paths[0], paths[1]);
        ...
        if(dUndCur[0] < 0)
            _transfer(_msgSender(), call_put[0], und_cur[0], dUndCur[0], maxes[0], paths[0]);
        if(dUndCur[1] < 0)
            _transfer(_msgSender(), call_put[1], und_cur[1], dUndCur[1], maxes[1], paths[1]);
        
        if(data.length > 0)
            IFlashSwapCallee(_msgSender()).onFlashSwap(_msgSender(), call_put[0], call_put[1], dCallPut[0], dCallPut[1], dUndCur[0], dUndCur[1], data);
        ...
        emit Swap(_msgSender(), und_cur[0], und_cur[1], Call(call_put[0]).priceFloor(), Call(call_put[0]).priceCap(), dCallPut[0], dCallPut[1], dUndCur[0], dUndCur[1], data);
    }
/*
...
/*
    ...
    function calcDeltaRoute(uint priceFloor, uint priceCap, uint totalCall, uint totalPut, int dCall, int dPut, uint slippage, address[] calldata pathUnd, address[] calldata pathCur) public view returns (int undMax, int curMax, uint totalUnd, uint totalCur) {
        (undMax, curMax, totalUnd, totalCur) = calcDeltaWithFeeAndSlippage(priceFloor, priceCap, totalCall, totalPut, dCall, dPut, slippage);
        undMax = calcAmountRoute(undMax, pathUnd);
        curMax = calcAmountRoute(curMax, pathCur);
    }
    ...
    function calcAmountRoute(int amount, address[] calldata path) public view returns (int) {
        ...
        if(path.length > 0)
            if(amount > 0)
                ...
                return -int(rounter.getAmountsOut(uint(-amount), path)[path.length-1]);
        return amount;
    }
