...
contract Test {
    ...
    function getTokens() external {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            JsmnSolLib.Token memory token = tokens[i];
            emit Token(token.start, token.startSet, token.end, token.endSet, token.size);
        }
    }
    ...
    event Token(uint start, bool startSet, uint end, bool endSet, uint8 size);
}