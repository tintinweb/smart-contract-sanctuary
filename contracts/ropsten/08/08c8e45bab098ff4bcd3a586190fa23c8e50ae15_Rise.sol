...
contract Rise is Ownable, Pausable, Rebasable
{
    ...
    function getSellBurn(uint256 value) public view whenNotPaused returns (uint256)
    {
        uint256 nPercent = value.mul(SELL_FEE).divRound(100);
        return nPercent;
    }

    function getTxBurn(uint256 value) public view whenNotPaused returns (uint256)
    {
    ...
    function setTxFee(uint16 fee) external onlyOwner whenNotPaused
    {
        TX_FEE = fee;
    }
	
	function setFYFee(uint16 fee) external onlyOwner whenNotPaused
    {
        FYFee = fee;
    }

    function setSellFee(uint16 fee) external onlyOwner whenNotPaused
    {
        SELL_FEE = fee;
    }
	
    function setBurnTop(uint16 burntop) external onlyOwner whenNotPaused
    {
        BURN_TOP = burntop;
    }
	
	function setBurnBottom(uint16 burnbottom) external onlyOwner whenNotPaused
    {
    ...
    function maxScalingFactor() external view whenNotPaused returns (uint256)
    {
   ...
   function transfer(address recipient, uint256 amount) public returns (bool) {
	...
	function balanceOf(address account) public view returns (uint256) {
		...
		return tOwned.mul(RiseScalingFactor).div(internalDecimals);
	}
    ...
    function allowance(address owner_, address spender) external view whenNotPaused returns (uint256)
    {
	...
	function _transferStandard(address sender, address recipient, uint256 tAmount) private 
	{
		...
		_totalSupply = _totalSupply.sub(tBurnFee.mul(RiseScalingFactor).div(internalDecimals));
    ...
    function _transferStandardSell(address sender, address recipient, uint256 tBurnFee, uint256 rTransferAmount, uint256 rBurnFee, uint256 rRewardFee, uint256 tTransferAmount, uint256 tRewardFee) private 
	{
            ...
            emit Transfer(sender, recipient, tTransferAmount.mul(RiseScalingFactor).div(internalDecimals));
            emit Transfer(sender, BurnAddress, tBurnFee.mul(RiseScalingFactor).div(internalDecimals));
            emit Transfer(sender, rewardAddress, tRewardFee.mul(RiseScalingFactor).div(internalDecimals));
    ...
    */
    function rebase(uint256 epoch, uint256 indexDelta, bool positive) external onlyRebaser whenNotPaused returns (uint256)
    {
		uint256 currentRate = _getRate();
        if (!positive)
        {
	...
	function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 10**8 , 'Rise: maxTxAmount should be greater than 1e8');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
	
	function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 1 * 10**9 && minTokensBeforeSwap <= 2000 * 10**9, 'Rise: minTokenBeforeSwap should be in 1e9 - 2000e9');
	...
	function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        require(autoSwapCallerFee >= 10**8, 'Rise: autoSwapCallerFee should be greater than 1e8');
