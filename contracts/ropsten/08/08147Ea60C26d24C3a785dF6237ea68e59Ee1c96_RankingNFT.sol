...
contract RankingNFT is ERC721, Ownable {
  ...
  function buyTokenAfterMinting(uint256 _tokenId) public onlyOwner returns(bool) {
  ...
  function buyToken(uint256 _tokenId) public returns(bool) {
    ...
    uint256 beforeBalanceForCompany = token.balanceOf(owner());
    token.approve(owner(), companyPortion);
    ...
    token.approve(transHistory.currentOwner, currentOwnerPortion);    
