...
contract NFT is ERC721Enumerable, Ownable {
    ...
    event TokenCreated(uint256 indexed tokenId, address to);
    ...
    function mint(
        ...
        Token memory token = Token(name, collection, imageUrl);
        tokens[createdTokenId] = token;

        registry.addApplicationToken(application, createdTokenId);

        if (royaltyFee > 0) {
            address beneficiary = registry.getRoyaltyBeneficiary(msg.sender);
            if (beneficiary == address(0)) {
                registry.addRoyalty(createdTokenId, msg.sender, royaltyFee);
            } else {
                registry.addRoyalty(createdTokenId, beneficiary, royaltyFee);
            }
        }

        if (isForSale) {
            address marketplace = registry.getMarketplace();
            if (!isApprovedForAll(msg.sender, marketplace)) {
                setApprovalForAll(marketplace, true);
            }
            registry.offerForSale(createdTokenId, price);
        }

        emit TokenCreated(createdTokenId, msg.sender);
    }
