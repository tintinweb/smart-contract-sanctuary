...
contract Hornies is ERC721, Ownable {
    ...
    constructor(string memory baseURI) ERC721("Hornies", "Hornies") {
        setBaseURI(baseURI);
    }
    ...
    function changePrice(uint256 _newPublicPrice) external onlyOwner {
    ...
    function airdrop(address[] calldata _recipients) external onlyOwner {
        ...
        require(
            supply + _recipients.length <= MAX_SUPPLY,
            "Airdrop will exceed maximum supply of Hornies"
        );
        ...
        payable
    {
        ...
        require(
            _count > 0 && _count <= 6,
            "Minimum 1 and Maximum 6 Hornies can be minted per transaction"
        );
        require(
            supply + _count <= MAX_SUPPLY,
            "Exceeds maximum supply of Hornies"
        );
        require(
