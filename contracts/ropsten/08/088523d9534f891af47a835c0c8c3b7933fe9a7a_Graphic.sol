...
contract Graphic is Ownable {
    ...
    mapping(uint256 => FileMetadata) filesMetadataIndex;
    ...
    function addFile(string[] memory metadata) public onlyOwner returns (uint256) {
        ...
        filesMetadataIndex[size].file_number = dataConvert(metadata[0]);
        filesMetadataIndex[size].title = dataConvert(metadata[1]);
        filesMetadataIndex[size].album = dataConvert(metadata[2]);
        filesMetadataIndex[size].website = dataConvert(metadata[3]);
        (filesMetadataIndex[size].ipfs_hash[0], filesMetadataIndex[size].ipfs_hash[1]) = splitIpfsHash(bytes(metadata[4]));
        filesMetadataIndex[size].comment = dataConvert(metadata[5]);
		filesMetadataIndex[size].copyright = dataConvert(metadata[6]);
        filesMetadataIndex[size].submission_date = dataConvert8(metadata[7]);
		filesMetadataIndex[size].md_hash  = dataConvert(metadata[8]);
        (uint year, uint month, uint day) = timestampToDate(block.timestamp);
        filesMetadataIndex[size].blockchain_date = dataConvert8(concat( convertVaalue(day),  ".",  convertVaalue(month), ".", convertVaalue(year) ));



        filesByNumber[filesMetadataIndex[size].file_number].push(size);
        bytes32 _hashId = keccak256(abi.encodePacked(metadata[4]));
        filesByIpfsHash[_hashId].push(size);
        filesByMDHash[filesMetadataIndex[size].md_hash].push(size);
    ...
    function findFilesByKey(int key, string memory hash) view internal returns (FileOutput[] memory) {
        ...
        for (uint256 index = 0; index < len; index++){
            ...
            outputsCollection.file_number[index] = filesMetadataIndex[id].file_number;
            outputsCollection.title[index] = filesMetadataIndex[id].title;
            outputsCollection.album[index] = filesMetadataIndex[id].album;
            outputsCollection.website[index] = filesMetadataIndex[id].website;
            outputsCollection.ipfs_hash_part1[index] = filesMetadataIndex[id].ipfs_hash[0];
            outputsCollection.ipfs_hash_part2[index] = filesMetadataIndex[id].ipfs_hash[1];
            outputsCollection.comment[index] = filesMetadataIndex[id].comment;
            outputsCollection.copyright[index] = filesMetadataIndex[id].copyright;
            outputsCollection.submission_date[index] = filesMetadataIndex[id].submission_date;
            outputsCollection.blockchain_date[index] =  filesMetadataIndex[id].blockchain_date;
			outputsCollection.md_hash[index] = filesMetadataIndex[id].md_hash;	
