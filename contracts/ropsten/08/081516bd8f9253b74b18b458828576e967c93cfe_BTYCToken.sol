...
contract BTYCToken is ERC20Interface, Owned {
	...
	uint public sysPrice1; //挖矿的衡量值
	uint public sysPer1; //挖矿的增量百分比 /100
	/*
	...
	*/
	...
	constructor() public {
		...
		sysPrice = 3 ether;//test
		sysPer = 225; //挖矿的增量百分比 /100
		//sysPrice1 = 3000 ether; //挖矿的衡量值
		sysPrice1 = 6 ether;//test
		sysPer1 = 500; //挖矿的增量百分比 /100
		//candyper = 1 ether;
	...
	function _transfer(address from, address to, uint tokens) private returns(bool success) {
		...
		if(sendPer > 0 && sendPer <= 100 && topuser1 != address(0) && topuser1 != from) {
			uint subthis = 0;
				//上家分润
			...
			//如果存在第二层
		    if(sendPer2 > 0 && sendPer2 <= 100 && fromaddr[topuser1] != address(0) && fromaddr[topuser1] != from) {
				...
				//如果存在第三层
				if(sendPer3 > 0 && sendPer3 <= 100 && fromaddr[topuser2] != address(0) && fromaddr[topuser2] != from) {
	 ...
	 * @param {Object} uint
	 */
	function setPrices(uint newonceaddtime, uint newonceouttime, uint newBuyPrice, uint newSellPrice, uint systyPrice, uint sysPermit, uint systyPrice1, uint sysPermit1, uint syssendfrozen, uint syssendper1, uint syssendper2, uint syssendper3) public {
		require(admins[msg.sender] == true);
	 ...
	 * 获取系统设置
	 */
	function getprice() public view returns(uint addtime, uint outtime, uint bprice, uint spice, uint sprice, uint sper, uint sprice1, uint sper1, uint sdfrozen, uint sdper1, uint sdper2, uint sdper3) {
		addtime = onceAddTime;
		outtime = onceOuttime;
		bprice = buyPrice;
		spice = sellPrice;
	...
	function mint() public {
		...
		if(balances[user] >= sysPrice1) {
		    mintAmount = balances[user] * sysPer1 / 10000;
		}
