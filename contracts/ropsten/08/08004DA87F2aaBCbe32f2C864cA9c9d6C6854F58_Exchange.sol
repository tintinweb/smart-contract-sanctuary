...
contract OrderBook is Ownable{
    ...
    struct Order {
        ...
        /* order Type */
        uint orderType; // 1.sell , 2.buy, 3. bid, 4. lazy
    }
...
interface IShizukaniMintableStore{

    function mintWithSignature(address from, address to, uint256 id, uint256 blockExpiry, uint8 v, bytes32 r, bytes32 s) external returns(bool) ;
}
...
contract Exchange is OrderState, TransferSafe {
    ...
    IShizukaniMintableStore private ShizukaniMintableStore;
    address public weth;
    ...
    constructor (address payable beneficiary, address buyerfeesigner, uint256 beneficiaryfee, address dirtyPunkStore, address wethAddr) public {
        beneficiaryAddress = beneficiary;
        buyerFeeSigner = buyerfeesigner;
        beneficiaryFee = beneficiaryfee;
        ShizukaniMintableStore = IShizukaniMintableStore(dirtyPunkStore);
        weth = wethAddr;
    }
    
    function sell(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, address buyer, uint256 royaltyFee, address payable royaltyReceipt) payable external {
       require( ( order.expiryTime == 0 ) || ( block.timestamp<= order.expiryTime ), "signature expired" );
       require(order.orderType == 1, "invalid order");

       validateOrderSignature(order, sig);
       validateBuyerFeeSig(order, royaltyFee, royaltyReceipt, buyerFeeSig);
        
       transferSellFee(order, royaltyReceipt, royaltyFee, buyer);
       setCompleted(order,true);
       transferToken(order, buyer);
       emitBuy(order, buyer);
    }
    
    function buy(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, address buyer, uint256 royaltyFee, address payable royaltyReceipt) payable external {
       require( ( order.expiryTime == 0 ) || ( block.timestamp<= order.expiryTime ), "signature expired" );
    ...
    function transferToken(Order calldata order, address buyer) internal {
        if(order.key.sellAsset.assetType == AssetType.ERC721){
            if(order.orderType == 1) {
                ...
                erc721safeTransferFrom(IERC721(order.key.buyAsset.token), order.key.owner, buyer,order.key.buyAsset.tokenId);
            }
        }
    ...
    function mintableStoreExchange(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, address buyer,uint256 royaltyFee, address payable royaltyReceipt, uint256 expiry, uint8 v, bytes32 r, bytes32 s) external payable  {
        require( ( order.expiryTime == 0 ) || ( block.timestamp<= order.expiryTime ), "signature expired" );
        require(order.orderType == 1, "invalid order");
        ...
        setCompleted(order,true);
        transferSellFee(order, royaltyReceipt, royaltyFee, buyer);
        ShizukaniMintableStore.mintWithSignature(royaltyReceipt, buyer,order.key.sellAsset.tokenId, expiry, v, r, s);
        emitBuy(order, buyer);
    }
    ...
    function transferBuyFee(Order calldata order,  address payable royaltyReceipt, uint256 royaltyFee, address buyer) internal {
        if(order.key.sellAsset.token == address(0x00)){
            require(msg.value == order.buying, "msg.value is invalid");
            transferEthFee(order.buying, order.key.owner, royaltyFee, royaltyReceipt);
        } else if(order.key.sellAsset.token == weth){
            transferWethFee(order.buying, order.key.owner, buyer, royaltyFee, royaltyReceipt);
        } else {
            transferErc20Fee(order.key.sellAsset.token,order.buying, order.key.owner, buyer,royaltyFee, royaltyReceipt);
        }
    }
    ...
    function transferEthFee(uint256 amount, address payable _seller, uint256 royaltyFee, address payable royaltyReceipt ) internal{
        ...
        uint256 fee = (amount.mul(royaltyFee)).div(1000);
        if( (fee > 0) && (royaltyReceipt!= address(0x00)) ){
                royaltyReceipt.transfer(fee);
        }
        
        uint256 value = amount.sub(protocalfee.add(fee));
        if(value > 0){
            _seller.transfer(value);
        }
    }

    function transferWethFee(uint256 amount, address payable _seller, address buyer,uint256 royaltyFee, address payable royaltyReceipt ) internal{
        uint256 protocalfee = (amount.mul(beneficiaryFee)).div(1000);
        if(protocalfee > 0){
        ...
        uint256 fee = (amount.mul(royaltyFee)).div(1000);
        if( (fee > 0) && (royaltyReceipt!= address(0x00)) ){
                require(IWETH(weth).transferFrom(buyer,royaltyReceipt,fee),"invalid 2");
        }
        
        uint256 value = amount.sub(protocalfee.add(fee));
        if(value > 0){
            require(IWETH(weth).transferFrom(buyer,_seller,value),"invalid 3");
        }
    }
    
    function transferErc20Fee(address token, uint256 amount, address payable _seller, address buyer,uint256 royaltyFee, address payable royaltyReceipt ) internal{
        ...
        uint256 fee = (amount.mul(royaltyFee)).div(1000);
        if( (fee > 0) && (royaltyReceipt!= address(0x00)) ){
                require(IERC20(token).transferFrom(buyer,royaltyReceipt,fee),"invalid 2");
        }
        
        uint256 value = amount.sub(protocalfee.add(fee));
        if(value > 0){
            require(IERC20(token).transferFrom(buyer,_seller,value),"invalid 3");
        }
    }
    

    function transferFeeView(uint256 amount, uint256 royaltyFee ) public view returns(uint256,uint256,uint256){
        uint256 protocalfee = (amount.mul(beneficiaryFee)).div(1000);
