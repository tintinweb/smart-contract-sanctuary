...
contract RegisteringContract is IRegisteringContract, Ownable {
	...
	mapping(bytes32 => NameInfo) public nameHashes;

	uint256 lockingAmount = 1;
	uint256 ExpireDuration = 1000;
	uint256 feePerByte = 1;
	
	modifier checkName(bytes32 _nameHash) {
		NameInfo storage nameHash = nameHashes[_nameHash];
		UserInfo storage user = userInfo[_msgSender()];
		if ( nameHash.isRegistered && block.timestamp - nameHash.activeTime < ExpireDuration) {
			nameHash.isActive = false;
	...
	*/
	...
	function bookNameWithHash(bytes32 _nameHash, bytes memory _signature) external payable checkName(_nameHash){
		NameInfo storage nameHash = nameHashes[_nameHash];

		require(verify(_msgSender(), _nameHash, _signature), "bookNameWithHash: invalid signature.");
		require(!nameHash.isBooked, "bookNameWithHash: name already booked!");
		require(!nameHash.isActive, "bookNameWithHash: name already registered!");
		...
		nameHash.isBooked = true;
	}
	function registerName(string memory _name, bytes memory _signature) external checkName(getNameHash(_name)){
		NameInfo storage nameHash = nameHashes[getNameHash(_name)];

		require(!nameHash.isActive, "registerName: name already registered");
		require(verify(_msgSender(), getNameHash(_name), _signature), "registerName: invalid signature.");
		require(nameHash.isBooked, "registerName: you didnt booked the name yet.");

		nameHash.isRegistered = true;
		nameHash.isActive = true;
		nameHash.activeTime = block.timestamp;
	...
	function renew(string memory _name, bytes memory _signature) external payable checkName(getNameHash(_name)) {
		...
		require(nameHash.isBooked, "renew: you didnt book the name yet.");
		require(!nameHash.isActive, "renew: your name still active now");
