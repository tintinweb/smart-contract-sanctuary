...
contract Extras {
    ...
    function Safety(uint8 position, address from) public returns (bool success);
    function GetSafety() public returns (uint _safety);
    
    event MintingDisabled(string message);
    event BurningDisabled(string message);
}
...
contract SailtheStars is ERC20Interface, SafeMath, Extras {
     ...
     function DisableMinting() public returns (bool success)
    {
        if(msg.sender == _sailthestars && MintingAllowed ==  true && SafetyKey == 0) {
            ...
            emit MintingDisabled("Minting of SailStars permanently disabled.");
            SafetyKey=1;
            return true;
        } else { revert(); }
    }
    ...
    function DisableBurning() public returns (bool success)
    {
        if(msg.sender == _sailthestars && BurningAllowed ==  true && SafetyKey == 0) {
            ...
            emit BurningDisabled("Burning of SailStars permanently disabled.");
            SafetyKey=1;
            return true;
        } else { revert(); }
    }
