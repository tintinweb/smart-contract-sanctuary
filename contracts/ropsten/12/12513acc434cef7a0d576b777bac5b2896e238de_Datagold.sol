...
contract Datagold is ERC20Interface, SafeMath{
   ...
   // datagold coins
     ...
     */
    constructor(address DATcontract) public {
        DAT = ERC20Interface(DATcontract);
    ...
    */
    ...
    function swapDGCForDAT(uint dgcAmt)
    external
    returns (bool success)
    {
        require(balances[msg.sender] >= dgcAmt, "You do not have enough DGC balance for this swap.");
        transferFrom(msg.sender, address(this), dgcAmt);
        uint datAmt = safeMul(dgcAmt, 100);
        require(DAT.balanceOf(address(this)) >= datAmt, "The DGC contract does not have enough DAT balance for this swap.");
        DAT.transfer(msg.sender, datAmt);
         
        return true;
    }
    
    
    function swapDATForDGC(uint datAmt)
    external 
    returns (bool success)
    {
        return swapDATForDGCImp(datAmt);
    }
    
    function swapDATForDGCImp(uint datAmt)
    private 
    returns (bool success)
    {
        require(DAT.balanceOf(msg.sender) >= datAmt, "You do not have enough DAT balance for this swap.");
        DAT.transferFrom(msg.sender, address(this), datAmt);
        
        uint dgcAmt = safeDiv(datAmt, 100);
        require(balanceOf(address(this)) >= dgcAmt, "The DGC contract does not have enough DGC balance for this swap.");
        transfer(msg.sender, dgcAmt);
