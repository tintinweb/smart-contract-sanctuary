 ...
 */
 contract RegisterMerkleRoot {
  ...
  function isDeviceMintable(bytes32 hardwareHash
  )
    public
    view
    returns (bool)
  ...
  function getMintableDevices(bytes32 root
  )
    public
    view
    returns (uint256)
  {
    ...
    return r.totalDevices;
  }

  function getMintableKong(bytes32 root
  )
    public
    view
    returns (uint256)
  {
    ...
    return r.totalMintableKong;
  }

  /**
   * @dev Return Kong amount for a registered root.
   */
  ...
  /**
   * @dev Return proofUri for a registered root.
   */
  function getProofUri(bytes32 root
  )
    public
    view
    returns (bytes32)
  ...
  // function getDeviceContractAddress(bytes32[] memory proof, bytes32 root, bytes32 hardwareHash
  // )
  //   public
  //   view
  //   returns (address)
  // {
  //   // TODO: use create2 to get a contract address for escrow contract (?); create2 can include the address of another contract
  // }
