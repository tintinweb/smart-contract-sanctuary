...
contract WZyxBridge is Ownable {
    using SafeMath for *;
    ...
    function newTransfer(Swap calldata swapInfo) public onlyOracle supportedToken(swapInfo.token) {
    ...
    function depositToken(address token, uint256 amount, int chainIdTo) public supportedToken(token) {
    ...
    /**** Admin functions  ****/
    function addCoin(address token, uint256 minSwap) public onlyOwner {
        listOfSupportedTokens[token] = true;
        minLimitForSwap[token] = minSwap;
    }

    function removeCoin(address token) public onlyOwner {
        delete listOfSupportedTokens[token];
        delete minLimitForSwap[token];
    }
