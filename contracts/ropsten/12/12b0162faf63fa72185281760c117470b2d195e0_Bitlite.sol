...
contract Bitlite is ERC20, CoinRateMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "Bitlite";
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public coinRate = 10 * (10**decimals);
    uint256 public intervalRate = 180 * (10**decimals);
    ...
    function mintingCoinRate() StrongHolderMinter public returns (bool) {
        ...
        uint256 coinRateMinting = getCoinRateMinting();
    ...
    function getCoinRateMinting() internal returns (uint) {
        require((now >= coinRateStartTime) && (coinRateStartTime > 0));
        currentBlockNumber = block.number; lastBlockNumber = currentBlockNumber;
        newBlockNumber = lastBlockNumber;
        uint256 blockMinting = lastBlockNumber.sub(newBlockNumber);
        uint256 blockReward = blockMinting.add(coinRate);
        uint256 coinsReward = blockReward.mul(intervalRate);
        return coinsReward;
    }
