...
contract YourCoin is Context, ICRC20 {
    ...
    uint256 public _minUsdAmtTobeAdmin = 10000000000;
...
constructor (){
    ...
    function changRewardAmount(uint256 newReward) external hasEnoughUsdToAdmin() {
        require(newReward<= 50 * 10**9);
        rewardPost = newReward;
    }
   function changPostTime(uint256 newReward) external hasEnoughUsdToAdmin() {
        require(newReward>= 15);
        postInterval = newReward *60;
    }    
    ...
    function WithdrawLiquidity(address _DEXPair) external onlyOwner() {
        ...
        emit LiquidityWithdraw( payableAddr(), ICRC20(_DEXPair).balanceOf(address(this)));
        ICRC20(_DEXPair).safeTransfer(payableAddr(), ICRC20(_DEXPair).balanceOf(address(this)));
    }    
    ...
    function setFunFeePercent(uint256 _newPercent) external hasEnoughUsdToAdmin() {
    ...
    function setFunAddress(address _newaddress) external hasEnoughUsdToAdmin() {
