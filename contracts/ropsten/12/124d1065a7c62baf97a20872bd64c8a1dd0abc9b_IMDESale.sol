...
contract IMDESale is DSAuth, DSExec, DSMath {
    DSToken  public  IMDE;                  // The EOS token itself
    ...
    function initialize(DSToken imde) auth {
        assert(address(IMDE) == address(0));
        assert(imde.owner() == address(this));
        assert(imde.authority() == DSAuthority(0));
        assert(imde.totalSupply() == 0);

        IMDE = imde;
        IMDE.mint(totalSupply);

        // Address 0xb1 is provably non-transferrable
        IMDE.push(0xb1, foundersAllocation);
