...
contract StrikeX is Context, IERC20, Ownable {
    ...
    // team Wallet Address
    address payable public teamWallet = payable(0x5581f7b5F1133c076b72ea491c6E7e4c52CA1CfE); 
    address payable public buybackWallet = payable(0xA43c9E46735c0a8279a64b46F1Aa43B08d9d4ad0);
    ...
    string private _name = "StrikeX2";
    string private _symbol = "STRX2";
    uint8 private _decimals = 9;


    // Selling tax 3% from transactions to holders
    uint256 public _taxFee = 300;
    uint256 private _previousTaxFee = _taxFee;

    // Selling tax 3% to contract
    uint256 public _contractFee = 300;
    uint256 private _previousContractFee = _contractFee;

    uint256 public _liquidityFee = 150; // liquidity fee 1.5%

    uint256 public _teamFee = 100; // team fee 1%

    uint256 public _buybackFee = 50; // buyback fee 0.5%
    
    // 5 Million limitation for selling 24 hours
    ...
    constructor () {
        ...
        _contractFee = _liquidityFee + _teamFee + _buybackFee;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            ...
            if( block.timestamp < _startTime[from] + (1 days)){
                require(_tokenSold[from] <= _maxTxAmount, "Sold amount exceeds the maxTxAmount.");
            }else{
                _startTime[from] = block.timestamp;
                _tokenSold[from] = 0;
            }
            */
            ...
            if (overMinimumTokenBalance) {
                ...
                // 1.5% to contract pool 
                swapAndLiquify(contractTokenBalance.div(2));

                // 1% to team wallet (sent as BNB) 0.5% to ‘buyback’ wallet (sent as BNB)
                swapAndSendBNB(contractTokenBalance.div(2));   
            }
        }
    ...
    function swapAndSendBNB(uint256 tokenBalance) private lockTheSwap {
       
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokenBalance);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);

        uint256 teamBNB = transferredBalance.mul(2).div(3);
        teamWallet.transfer(teamBNB);

        uint256 buybackBNB = transferredBalance - teamBNB;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap creates, and not make the contract event include any ETH that
        ...
        // add contract to uniswap
        addLiqudity(otherHalf, newBalance);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tContract = calculateFee(tAmount, _contractFee);
    ...
    function calculateFee(uint256 _amount, uint256 _fee) pure private returns (uint256) {
    ...
    function setliquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        _contractFee = liquidityFee + _teamFee + _buybackFee;
    }

    function setTeamFeePercent(uint256 teamFee) external onlyOwner() {
        _contractFee = _liquidityFee + teamFee + _buybackFee;
    }

    function setBuyBackFeePercent(uint256 buybackFee) external onlyOwner() {
        _contractFee = _liquidityFee + _teamFee + buybackFee;
    }
    ...
    function setNumTokensSellToAddToContract(uint256 _minimumTokensBeforeSwap) external onlyOwner() {
        minimumTokensBeforeSwap = _minimumTokensBeforeSwap;
    }
    
    function setTeamAddress(address _teamWallet) external onlyOwner() {
        teamWallet = payable(_teamWallet);
    }

    function setBuyBackAddress(address _buybackWallet) external onlyOwner() {
