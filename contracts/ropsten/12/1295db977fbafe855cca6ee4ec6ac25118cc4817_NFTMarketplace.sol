  ...
  ReentrancyGuardUpgradeable
{
  ...
  event NftListed(uint256 listingId, address nftTokenAddress, uint256 nftTokenId, address listedBy, uint256 paymentType, uint256 paymentAmount, address paymentTokenAddress);
  ...
  event WithdrawnErc20Premiums(address indexed recipient, IERC20 erc20Token, uint256 amount);
  ...
  function initialize(
    ...
    EIP712MetaTransactionUpgradeable._initialize("CoinviseNftMarketplace", "1");
  ...
  function _calculateCut(uint256 amount) internal view returns (uint256) {
    return amount.mul(premiumPercentage).div((10**(premiumPercentageDecimals.add(2))));
  }
   ...
   */
  function getCurrentNftListingIds() external view returns (uint256[] memory) {
  ...
  function listNftInEth(
    ...
    uint256 _nftTokenId,
    uint256 _priceInWei
  ) external nonReentrant pausable payable {
    ...
    emit NftListed(nftListing.listingId, nftListing.nftTokenAddress, nftListing.nftTokenId, nftListing.listedBy, nftListing.paymentType, nftListing.paymentAmount, nftListing.paymentTokenAddress);
  }
   ...
   */
  function unlistNftByListingCreator(uint256 _nftListingId) external pausable {
    address _sender = msgSender();

    require(_sender == nftListingById[_nftListingId].listedBy, "UNAUTHORIZED");
  ...
  function buyNftInEth(uint256 _nftListingId) external payable pausable {
    ...
    uint256 expectedAmountWei =
      listingAmountWei.add(_calculateCut(listingAmountWei));
  ...
  function buyNftInErc20Tokens(uint256 _nftListingId) external pausable {
    ...
    uint256 expectedAmountInTokens =
      listingAmountInTokens.add(_calculateCut(listingAmountInTokens));
    IERC20 erc20Token = IERC20(nftListing.paymentTokenAddress);
    require(
      erc20Token.allowance(_sender, address(this)) >= expectedAmountInTokens,
    ...
    erc20Token.safeTransferFrom(_sender, address(this), expectedAmountInTokens);
  ...
  function likeNftListing(uint256 _nftListingId) external pausable {
    ...
    require(_listedNftTokenIds.contains(_nftListingId), "ALREADY_DELISTED");
    require(!hasUserLikedNftListing[_sender][_nftListingId], "ALREADY_LIKED");
