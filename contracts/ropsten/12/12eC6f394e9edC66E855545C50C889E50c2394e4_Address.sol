...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function randomColorHSL(uint256 tokenId) public pure returns (ColorHSL memory) {
        return toColorHSL(randomColorRGB(tokenId));
    }

    function randomColorRGB(uint256 tokenId) public pure returns (ColorRGB memory) {
        return ColorRGB(randomColor(tokenId, "RED"), randomColor(tokenId, "GREEN"), randomColor(tokenId, "BLUE"));
    }

    function randomHue(uint256 tokenId) internal pure returns (uint256) {
        return random(toString(tokenId)) % 360;
    }

    function randomPercent(uint256 tokenId, string memory keyPrefix) internal pure returns (uint256) {
        return 10 + random(string(abi.encodePacked(keyPrefix, toString(tokenId)))) % 91;
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[1] = string(abi.encodePacked('<rect x="0" y="0" height="20%" width="100%" fill="', hslToString(colors[0]) ,'"/>'));
        parts[2] = string(abi.encodePacked('<rect x="0" y="70" height="20%" width="100%" fill="', hslToString(colors[1]) ,'"/>'));
        parts[3] = string(abi.encodePacked('<rect x="0" y="140" height="20%" width="100%" fill="', hslToString(colors[2]) ,'"/>'));
        parts[4] = string(abi.encodePacked('<rect x="0" y="210" height="20%" width="100%" fill="', hslToString(colors[3]) ,'"/>'));
        parts[5] = string(abi.encodePacked('<rect x="0" y="280" height="20%" width="100%" fill="', hslToString(colors[4]) ,'"/>'));
        parts[6] = '</svg>';

        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6]));
        
        // string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Palette #', toString(tokenId), '", "description": "Color Loot is randomized color palette generated and stored on chain. Feel free to use Color Loot in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function toColorRGB(ColorHSL memory colorHSL) public pure returns (ColorRGB memory) {
        uint256 r;
        uint256 g;
        uint256 b;
    ...
    function toColorHSL(ColorRGB memory colorRGB) public pure returns (ColorHSL memory) {
    ...
    constructor() ERC721("Color Loot", "COLOOT") Ownable() {
