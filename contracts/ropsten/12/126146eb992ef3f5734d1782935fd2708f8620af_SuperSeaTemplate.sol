...
// File: SuperSeaTokenSubscripton..sol
...
contract SuperSeaTemplate is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    event Subscription(uint256 tokenId, SubscriptionPlan plan, uint256 timestamp, uint256 expiresAt, uint256 subscriptionCounter);
    ...
    //addred uint256 tokenId as argument and tokenId to _hasActive subscription.check
    ...
    function subscribe(uint planIndex, uint256 tokenId) whenNotPaused public payable {
	...
	// Get the current time
        ...
        if(_hasActiveSubscription(tokenId)) {
	    // If they do, we override the starting date with the active plan's expiry date ***
            startingDate = subscriptionExpiration[tokenId];
        }   
	...
	// Assign the expiry date to the address
        subscriptionExpiration[tokenId] = expiresAt;
        _subscriptionCounter.increment();

        emit Subscription(tokenId, plan, block.timestamp, expiresAt, _subscriptionCounter.current());
    }

    function _hasActiveSubscription(uint256 tokenId) private view returns (bool) {
	// Get the expiry date from the map. If it's greater than the current time, then subscription is active.
        ...
        return subscriptionExpiration[tokenId] > block.timestamp;    
    }

    function hasActiveSubscription(uint256 tokenId) external view returns (bool) {
        return _hasActiveSubscription(tokenId);
    }

    function getSubscription(uint256 tokenId) external view returns (uint256) {
        return subscriptionExpiration[tokenId];
    }
