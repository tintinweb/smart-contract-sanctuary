...
/**
 * @dev Stixex main smart contract for receiving investments (only in USDT)
 */
contract Stixex_USDT is BlackList, Pausable {
    using ECDSA for bytes32;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IERC20 private _usdt;                               // interface of usdt token
    uint256 private _minAllowedAmount = 2 ether;        // 2 USDT default min deposit amount
    ...
    modifier amountAllowed (uint256 amount) {
        require(amount >= _minAllowedAmount, "allowedAmount: deposited amount is smaller than minimum allowed limit");
        _;
    }
    ...
    constructor (IERC20 usdt) public {
        require(address(usdt) != address(0), "constructor: invalid usdt token address");
        _usdt = usdt;
    }
    ...
    function deposit(address sender, uint256 amount) public isNotBlocked whenNotPaused amountAllowed(amount) {
        // transfer tokens from user to contract address
        _usdt.safeTransferFrom(sender, address(this), amount);

        // close previues session if its exist
        _closeCurrentSession(sender);

        // init new session
        _initNewSession(sender, amount);

        // update total funded amount of sender
        _updateUserTotalFunds(sender, amount);
    ...
    function withdrawAdmin(uint256 amount) public onlyOwner {
        _withdrawUsdt(msg.sender, amount);
    }

    function depositAdmin(uint256 amount) public onlyOwner {
    ...
    function _withdrawUsdt(address receiver, uint256 amount) private {
        _usdt.safeTransfer(receiver, amount);
