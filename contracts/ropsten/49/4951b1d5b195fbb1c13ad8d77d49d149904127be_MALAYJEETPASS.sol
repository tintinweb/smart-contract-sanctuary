...
contract MALAYJEETPASS is ERC721Enumerable, Ownable {
    ...
    function tokenTier(uint256 tokenId) external view returns (uint16){
        require(mintBlock[tokenId] + 1 < block.number);
    ...
    function upgradeTier2to1(uint256 upgradeTokenID,uint256 burnTokenID1,uint256 burnTokenID2) external payable{
        ...
        tier2 = tier2-3;
        tier1++;
        this.transferFrom(msg.sender, address(0xDead), burnTokenID1);
        this.transferFrom(msg.sender, address(0xDead), burnTokenID2);
    ...
    function upgradeTier3to1(uint256 upgradeTokenID) external payable{
        require(tier1 < 50);
        require(msg.value >= 0.0000025 ether);
        require(msg.sender == ownerOf(upgradeTokenID));
