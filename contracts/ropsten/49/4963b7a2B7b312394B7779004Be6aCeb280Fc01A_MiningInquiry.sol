...
// File: contracts/MiningInquiry.sol
...
contract MiningInterface{
    ...
    function claim_multi_reward_for(address rec, uint256[] memory ratio) public;
}
...
contract MiningInquiry is Ownable{
  using SafeMath for uint256;
  address[] public minters;
  uint256[] public ratios;

  constructor(address[] memory _minters, uint256[] memory _ratios) public{
    minters = _minters;
    ratios = _ratios;
  }
  
  function reset_minter(address[] memory _minters) public onlyOwner{
    minters = _minters;
  }
  function reset_ratio(uint256[] memory _ratios) public onlyOwner{
    ratios = _ratios;
  }
  function get_minter_reward(address addr, address mint) public view returns(uint256){
    return MiningInterface(mint).get_multi_claimable_reward(ratios, addr);
  }
  function get_minter_reward_with_ratio(address addr, address mint, uint256[] memory _ratios) public view returns(uint256){
    return MiningInterface(mint).get_multi_claimable_reward(_ratios, addr);
  }
  function get_total_reward(address addr) public view returns(uint256){
    uint amount = 0;
    for (uint i = 0; i < minters.length; i++){
      amount = amount.safeAdd(get_minter_reward(addr, minters[i]));
    }
    return amount;
  }
  function get_total_reward_with_ratio(address addr, uint256[] memory _ratios) public view returns(uint256){
    uint amount = 0;
    for (uint i = 0; i < minters.length; i++){
      amount = amount.safeAdd(get_minter_reward_with_ratio(addr, minters[i], _ratios));
    }
    return amount;
  }
  function claim_all_for(address rec, uint256[] memory _ratio) public{
    for (uint i = 0; i < minters.length; i++){
      MiningInterface(minters[i]).claim_multi_reward_for(rec, _ratio);
    }
  }
  function claim_all(uint256[] memory _ratio) public{
    claim_all_for(msg.sender, _ratio);
  }
}

contract MiningInquiryFactory {
  event NewMiningInquiry(address addr);
  function createMiningInquiry(address[] memory _minters, uint256[] memory _ratios) public returns(address){
    MiningInquiry h = new MiningInquiry(_minters, _ratios);
    h.transferOwnership(msg.sender);
    emit NewMiningInquiry(address(h));
    return address(h);
  }
}