...
contract Zetal is ERC20, ProofOfAgeProtocol, ProofOfStakeProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;

    string public name = "Zetal";
    string public symbol = "ZETAL";
...
//--------------------------------------------------------------------------------------
    ...
    uint public constant stakingLockTime = 1 days;
    ...
    function getProofOfStakeMinting(address _holder) public view returns (uint) {
        ...
        } else if(totalParticipant >= 2500) {
            stakingRate = stakeRate.mul(1); //50%
        } else if(totalParticipant >= 5000) {
            stakingRate = stakeRate.div(2); //25%
        } else if(totalParticipant >= 10000) {
            stakingRate = stakeRate.div(4); //12.5%
        } else if(totalParticipant >= 25000) {
...
//--------------------------------------------------------------------------------------
    ...
    function setTimelock(uint256 timestamp) public onlyOwner {
        require(now.sub(timelockStart) < unlockDate);
