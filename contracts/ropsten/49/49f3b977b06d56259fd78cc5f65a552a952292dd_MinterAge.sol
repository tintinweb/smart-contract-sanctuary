...
contract MinterAge is ERC20, PoHASMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "MinterAge";
    string public symbol = "MINAGE";
...
//------------------------------------------------------------------------------
    ...
    function mint() PoHASMinter public returns (bool) {
        require(balances[msg.sender] >= coinHold);
    ...
    function getCoinMinting(address _address) internal view returns (uint) {
        ...
        uint _now = now;
        uint _mintingAge = getMintingAge(_address, _now);
