...
// File contracts/IWETHUpgradable.sol
...
// File contracts/MarketConsts.sol
...
library IntentFlags {
    ...
    // 0b0010 auction
    // 0b0100 buy
    uint256 constant SELL = 0x1;
    uint256 constant AUCTION = 0x2;
    uint256 constant BUY = 0x4;
}
    ...
    PausableUpgradeable
{
    ...
    function _run(
        ...
        } else if (detail.op == Consts.Op.REFUND_AUCTION) {
            require(inventoryStatus[itemHash] == Consts.InvStatus.AUCTION);
            OngoingAuction storage auc = ongoingAuctions[itemHash];

            if (auc.netPrice > 0) {
                _transferTo(item.currency, order.user, auc.netPrice);
    ...
    function _distributeFeeAndProfit(
        ...
        for (uint256 i = 0; i < sd.fees.length; i++) {
            ...
            uint256 amount = fee.percentage * price;
            payment -= amount;
