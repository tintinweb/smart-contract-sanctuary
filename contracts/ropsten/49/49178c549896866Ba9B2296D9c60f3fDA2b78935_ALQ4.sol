...
contract ALQ4 is IERC20, Auth {
	using SafeMath for uint256;
	string constant _name = "ALQ4";
	string constant _symbol = "ALQ4";
	...
	function _swapTaxAndLiquify() internal {
		...
		// uint256 _taxSwapMin = _totalSupply * 25 / 10000; //to be replaced with a variable changeable by tax operator
		if (_taxTokensAvailable > _taxSwapMin) {
		    ...
		    if (_autoLPShares > 0) {
		    	...
		    	uint256 _ethWeiAmount = _ethSwapped * _autoLPShares / _totalShares / 2;
