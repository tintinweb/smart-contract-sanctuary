...
contract PhuketStakeContract is Ownable, Pausable {
    ...
    mapping(address => mapping(uint256 => StakerData)) public StakerDataAddress;
    ...
    constructor(uint256 _ratio,address _phuketToken,uint256 _maxStkQty,uint256 _stakeDays,address _rewardAddress) public {
        ratio = _ratio;
     ...
     */
    function stakeALT(uint256 _altQuantity)
        public
        whenNotPaused
    {
        require(_altQuantity <= maxStakedQuantity && _altQuantity > 0,"Invalid Phuket quantity"); // To verify Phuket quantity
        ...
        if (StakerDataAddress[msg.sender][_timestamp].staker != address(0)) {
            _timestamp = _timestamp.add(1);
        }
        
        StakerDataAddress[msg.sender][_timestamp] = StakerData(
            _altQuantity,
            _timestamp,
            msg.sender,
            0,
    ...
    {
        uint256 fee = 0;
        uint256 withdrawAmount = 0; uint _drawnAmount;
        uint256 rewardAmount = 0;

        for (uint256 i = 0; i < _expiredTimestamps.length; i = i.add(1)) {
             require(!(StakerDataAddress[msg.sender][_expiredTimestamps[i]].isCompleted),"Already Unstaked");
             require(StakerDataAddress[msg.sender][_expiredTimestamps[i]].staker == msg.sender,"give a correct time");
                    _drawnAmount = StakerDataAddress[msg.sender][_expiredTimestamps[i]].altQuantity;
                    
                    if(withdrawAmount.add(_drawnAmount) > _amount){
                        _drawnAmount = _amount.sub(withdrawAmount);
                        StakerDataAddress[msg.sender][_expiredTimestamps[i]].altQuantity = StakerDataAddress[msg.sender][_expiredTimestamps[i]].altQuantity.sub(_drawnAmount);
                    }
                    else{
                        StakerDataAddress[msg.sender][_expiredTimestamps[i]].isCompleted=true;
                        StakerDataAddress[msg.sender][_expiredTimestamps[i]].altQuantity = StakerDataAddress[msg.sender][_expiredTimestamps[i]].altQuantity.sub(_drawnAmount);
                    }
                    
                    uint256 _curentTime = block.timestamp;
                    uint256 _days = _curentTime.sub(_expiredTimestamps[i]).div(30 seconds);
                    
                    if(_days >= stakeDays) { _days = stakeDays; }
                    else if(_days < stakeDays) {
                        fee = _drawnAmount.mul(3).div(10);
                        _drawnAmount = _drawnAmount.mul(7).div(10); 
                    ...
                    if(_days > 0) {
                        rewardAmount = rewardAmount.add(_calculateReward(_drawnAmount,ratio,_days));
                        StakerDataAddress[msg.sender][_expiredTimestamps[i]].reward = StakerDataAddress[msg.sender][_expiredTimestamps[i]].reward.add(rewardAmount);
                    }
        ...
        IERC20(PhuketTokenAddress).transfer(msg.sender, withdrawAmount.add(rewardAmount));
        if(fee > 0){ IERC20(PhuketTokenAddress).transfer(rewardAddress, fee); }
