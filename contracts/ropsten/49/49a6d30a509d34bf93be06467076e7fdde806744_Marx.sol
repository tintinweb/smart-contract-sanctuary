...
contract Marx is Context, Ownable, IERC20, IERC20Metadata {
    ...
    uint256 numTokensCirculating;
    uint256 private _lastPrice = 100000000 wei;
    uint256 private _tick = 22567 wei;
    ...
    mapping (uint256 => Buy) controlBuys;
    
    address[] communistArray;
    uint256[] transactionsArray;

    function _calculateBuy(uint256 value) public view returns (uint256) {
        uint256 averagePrice = ((_tick.div(value)).add(_lastPrice)).mul(2);
        uint256 amountBuy = value.div(averagePrice);
        
        return amountBuy;
    }
    
    function buy() external payable returns (uint256) {
        uint256 valueBuy = msg.value;
        uint256 totalAmount = _calculateBuy(valueBuy).mul(10000);
        _transfer(address(this), _msgSender(), totalAmount);
        numTokensCirculating += totalAmount;
        _lastPrice += totalAmount.mul(_tick);
        return totalAmount;
        
    }
    
    function verifyInitialPrice() public view returns (uint256){
        return _lastPrice;
    }
    
    function verifyTokensCirculating() public view returns (uint256) {
        return numTokensCirculating;
    }
    
    function _calculateSell(uint256 amount) public view returns (uint256) {
        uint256 priceSell = (_lastPrice.sub((_tick.mul(amount))).mul(2)).mul(amount);
        return priceSell;
    }
    
    function sell(uint256 amount) external {
        uint256 totalWei = _calculateSell(amount);
        //msg.sender or address this?
        payable(msg.sender).transfer(totalWei);
        _transfer(_msgSender(),address(this), amount.mul(10000));
    }
