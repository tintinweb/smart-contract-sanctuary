...
contract TradeMint is ITradeMint,Context,Owned{
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(poolInfo.lastSettlementBlock.add(settlementBlock) <= block.number  && poolInfo.unSettlementAmount >0){
            ...
            uint256 summaReward = getMultiplier(form, to).mul(poolInfo.rewardShare).div(totalRewardShare);
            poolInfo.easterEgg += summaReward.mul(easterEggPoint).div(100);//彩蛋抽成
            settlementTrade(poolInfo.poolAddress,(summaReward.sub(summaReward.mul(easterEggPoint).div(100))).div(tradeShare));//结算交易
            settlementPoolNewLiquidityIncentiveGrowth(poolInfo.poolAddress);
        }else if(block.number.div(settlementBlock).mul(settlementBlock) > poolInfo.lastSettlementBlock){
            poolInfo.easterEgg+= getMultiplier(poolInfo.lastSettlementBlock, block.number.div(settlementBlock).mul(settlementBlock)).mul(poolInfo.rewardShare).div(totalRewardShare);
        }
    }
        ...
        for (uint256 pid = 0; pid < length; ++pid) { 
            ...
            if(userInfo.lastTradeBlock != 0){
                if(userInfo.lastTradeBlock < poolInfo.lastSettlementBlock){ 
                    amount += FullMath.mulDiv( userInfo.tradeUnSettlementedAmount, poolInfo.tradeSettlementAmountGrowth[(userInfo.lastTradeBlock.div(settlementBlock).add(1)).mul(settlementBlock)],FixedPoint128.Q128);
                    ...
                    uint256 summaReward = getMultiplier(form, to).mul(poolInfo.rewardShare).div(totalRewardShare);
                    uint256 tradeReward = (summaReward.sub(summaReward.mul(easterEggPoint).div(100))).div(tradeShare);
                    uint256 quotient =  FullMath.mulDiv(summaReward, FixedPoint128.Q128, poolInfo.unSettlementAmount);
                    amount += FullMath.mulDiv(quotient,userInfo.tradeUnSettlementedAmount,  FixedPoint128.Q128);
                } 
    ...
    function getEasterEgg(address poolAddress) public view returns(uint256){
        ...
        if(block.number.div(settlementBlock).mul(settlementBlock) > poolInfo.lastSettlementBlock){
            easterEgg+= getMultiplier(poolInfo.lastSettlementBlock, block.number.div(settlementBlock).mul(settlementBlock)).mul(poolInfo.rewardShare).div(totalRewardShare);
        }
    }
    function getPoolReward(address  poolAddress) internal view returns (uint256) {
        ...
        uint256 reward =  multiplier.mul(poolInfo.rewardShare).mul(uint256(100).sub(easterEggPoint)).div(100).div(totalRewardShare).div(tradeShare).mul(tradeShare.sub(1));    
        return reward;
    }
    function getLiquidityIncentiveGrowthInPosition(int24 _tickLower,int24 _tickUpper,address  poolAddress) internal view returns (uint256) {
        ...
        if(tickLower.liquidityVolumeGrowthOutside != 0){ 
            newLowerLiquidityIncentiveGrowthOutside = newLowerLiquidityIncentiveGrowthOutside +
            FullMath.mulDiv(
        ...
        if(tickUpper.liquidityVolumeGrowthOutside != 0){
            newUpLiquidityIncentiveGrowthOutside = newUpLiquidityIncentiveGrowthOutside +
            FullMath.mulDiv(
    ...
    function settlementLiquidityIncentiveGrowthInPosition(int24 _tickLower,int24 _tickUpper,address  poolAddress) internal returns (uint256) {
        ...
        if(poolInfo.blockSettlementVolume[tickLower.settlementBlock] >0 && tickLower.liquidityVolumeGrowthOutside>0){
            tickLower.liquidityIncentiveGrowthOutside = tickLower.liquidityIncentiveGrowthOutside+
            FullMath.mulDiv(
                tickLower.liquidityVolumeGrowthOutside,
                poolInfo.blockSettlementVolume[tickLower.settlementBlock],
                FixedPoint128.Q128);
        ...
        if(poolInfo.blockSettlementVolume[tickUpper.settlementBlock] >0 && tickUpper.liquidityVolumeGrowthOutside >0){
            tickUpper.liquidityIncentiveGrowthOutside = tickUpper.liquidityIncentiveGrowthOutside+
            FullMath.mulDiv(
                tickUpper.liquidityVolumeGrowthOutside,
                poolInfo.blockSettlementVolume[tickLower.settlementBlock],
                FixedPoint128.Q128);
    ...
    function settlementPoolNewLiquidityIncentiveGrowth(address  poolAddress) internal returns (uint256) {
        ...
        poolInfo.blockSettlementVolume[poolInfo.lastSettlementBlock.add(settlementBlock)] =
        FullMath.mulDiv(
                reward,
                FixedPoint128.Q128,
    ...
    function settlementTrade(address tradeAddress,address  poolAddress,uint256 summaReward) internal{
        ...
        PoolInfo storage poolInfo = poolInfoByPoolAddress[poolAddress];
        poolInfo.tradeSettlementAmountGrowth[poolInfo.lastSettlementBlock.add(settlementBlock)] += FullMath.mulDiv(summaReward, FixedPoint128.Q128, poolInfo.unSettlementAmount);
        userInfo.tradeSettlementedAmount += FullMath.mulDiv( userInfo.tradeUnSettlementedAmount, poolInfo.tradeSettlementAmountGrowth[(userInfo.lastTradeBlock.div(settlementBlock).add(1)).mul(settlementBlock)],FixedPoint128.Q128);
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if(userInfo.lastTradeBlock != 0){
                if(userInfo.lastTradeBlock < poolInfo.lastSettlementBlock){
                    ...
                    uint256 summaReward = getMultiplier(form, to).mul(poolInfo.rewardShare).div(totalRewardShare);
                    poolInfo.easterEgg += summaReward.mul(easterEggPoint).div(100);//彩蛋抽成
                    poolInfo.tradeSettlementAmountGrowth[to] += FullMath.mulDiv((summaReward.sub(summaReward.mul(easterEggPoint).div(100))).div(tradeShare), FixedPoint128.Q128, poolInfo.unSettlementAmount);
                    userInfo.tradeSettlementedAmount += FullMath.mulDiv( userInfo.tradeUnSettlementedAmount, poolInfo.tradeSettlementAmountGrowth[to],FixedPoint128.Q128);
                    userInfo.tradeUnSettlementedAmount = 0;
                }
    ...
    function cross(int24 _tick,int24 _nextTick) external override{
        ...
        if(isReward[_msgSender()]){
            ...
            if(tick.liquidityVolumeGrowthOutside >0 ){
                tick.liquidityIncentiveGrowthOutside = tick.liquidityIncentiveGrowthOutside+
                FullMath.mulDiv(
            ...
            tick.settlementBlock = (block.number.div(settlementBlock).add(1)).mul(settlementBlock);
    ...
    function snapshot(address tradeAddress,int24 tick,uint256 liquidityVolumeGrowth,uint256 tradeVolume) external override {
        ...
        if(isReward[_msgSender()]){
            if(poolInfo.lastSettlementBlock.add(settlementBlock) <= block.number  && poolInfo.unSettlementAmount >0){
                ...
                uint256 summaReward = getMultiplier(form, to).mul(poolInfo.rewardShare).div(totalRewardShare);
                poolInfo.easterEgg += summaReward.mul(easterEggPoint).div(100);//彩蛋抽成
                settlementTrade(tradeAddress,_msgSender(),(summaReward.sub(summaReward.mul(easterEggPoint).div(100))).div(tradeShare));
                settlementPoolNewLiquidityIncentiveGrowth(_msgSender());
                if(easterEggEnable && poolInfo.easterEgg>0){
                    if(rand(luckNum) == 0){
                        uint256 eggAmount = poolInfo.easterEgg.mul(easterEggReward).div(100);
    ...
    function getPoolReward(address poolAddress,uint256 blockNum) external view returns (uint256 lpReward,uint256 tradeReward){ 
         PoolInfo storage poolInfo = poolInfoByPoolAddress[poolAddress];
        uint256 form = blockNum.sub(settlementBlock);
        uint256 to =blockNum;
        uint256 summaReward = getMultiplier(form, to).mul(poolInfo.rewardShare).div(totalRewardShare);
        tradeReward = (summaReward.sub(summaReward.mul(easterEggPoint).div(100))).div(tradeShare);
