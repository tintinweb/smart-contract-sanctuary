...
contract DDAO_vault is AccessControl,Ownable
{
	...
	function TokenApprove(address token, address spender,uint256 amount)public onlyAdmin
	{
		IToken(token).approve(spender,amount);
	}
	function TokenAllowance(address token,address spender)public view returns(uint256 amount)
	{
		amount = IToken(token).allowance(address(this),spender);
	}
	function TokenBalance(address token)public view returns(uint256 amount)
	{
