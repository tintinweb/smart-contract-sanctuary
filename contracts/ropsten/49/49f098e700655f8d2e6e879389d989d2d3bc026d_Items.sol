...
contract Items {
    ...
    function createItem(string _name, uint256 _price) public {
        require(msg.sender == owner);
    ...
    function updateItem(uint _id, string _name, uint256 _price, bool _forSale) public {
        require(msg.sender == owner);
        items[_id] = Item(_name, _price, _forSale);
    }

    function buyItem(uint _id) public {
        token.transfer(itemOwner[_id], items[_id].price);
