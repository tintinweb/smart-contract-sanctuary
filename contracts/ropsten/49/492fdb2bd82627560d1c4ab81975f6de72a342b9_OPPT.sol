 ...
 * @title OPPTOwnable
 *
 * @dev Due to ownable change in zeppelin, the authorities in OPPTOwnable includeowner 
 *      Each authority has different roles.
 */
contract OPPTOwnable {
    ...
    event OPPT_RoleTransferred(
        Role indexed ownerType,
        address indexed previousOwner,
        address indexed newOwner
    );
    
    event OPPT_SetOperator(address indexed operator); 
    event OPPT_DeletedOperator(address indexed operator);
    ...
    */
    function transferOwnership(address newOwner) public onlyOwner {
        emit OPPT_RoleTransferred(Role.owner, owner, newOwner);
        owner = newOwner;
    }
 ...
 */
contract OPPTPausable is OPPTOwnable {
    event OPPT_Pause();
    event OPPT_Unpause();
 ...
 */
contract OPPTBlacklist is OPPTOwnable {
    mapping(address => bool) blacklisted;
    
    event OPPT_Blacklisted(address indexed blacklist);
    event OPPT_Whitelisted(address indexed whitelist);
 ...
 * @title HasNoEther
 */
contract HasNoEther is OPPTOwnable {
...
/**
 * @title OPPTBaseToken - Major functions such as authority setting on tockenlock are registered.
 */
contract OPPTBaseToken is StandardToken, OPPTPausable, OPPTBlacklist, HasNoEther {
    ...
    event OPPT_SetSuperInvestor(address indexed SuperInvestor); 
    event OPPT_DeleteSuperInvestor(address indexed SuperInvestor);
    
    event OPPT_SetInvestor(address indexed investor); 
    event OPPT_DeleteInvestor(address indexed investor);
    
    event OPPT_TransferFrom(address indexed owner, address indexed spender, address indexed to, uint256 value);
    event OPPT_Burn(address indexed burner, uint256 value);
    ...
    */
    function setSuperInvestor(address _super) external onlyOwner {
        superInvestor[_super] = true;
        
        emit OPPT_SetSuperInvestor(_super);
    }

    /** 
    * @param _super Remove authorities of the address as a superinvestor  
    */
    function delSuperInvestor(address _super) external onlyOwner {
        superInvestor[_super] = false;
        
        emit OPPT_DeleteSuperInvestor(_super);
    }

    /**
    * @param _addr  Remove authorities of the address as a investor .   
    */
    function delInvestor(address _addr) onlyOwner public {
        investorList[_addr] = false;
        searchInvestor[_addr] = investor(0,0,0);
        emit OPPT_DeleteInvestor(_addr);
    }
    ...
    */
    ...
    whenPermitted(msg.sender) whenPermitted(_to) whenNotPaused onlyOwner
    returns (bool) {   
    ...
    */
    ...
    public returns(bool ret) {
        ...
        if (!ret) {
            ...
            emit OPPT_TransferFrom(_from, msg.sender, _to, _value);
        }
    }
...
contract OPPT is OPPTBaseToken {
    string public constant name = "Open Proprietary Protoco";
    string public constant symbol = "OPPT";
