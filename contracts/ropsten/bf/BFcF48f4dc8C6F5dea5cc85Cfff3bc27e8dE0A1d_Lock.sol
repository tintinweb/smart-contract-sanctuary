...
contract Lock is Ownable {
    ...
    modifier canClaim(uint256 id) {
        ...
        // for(uint8 index = 0; index <= _userVsLockIds[msg.sender].length; index++ ) {
        //     if(_userVsLockIds[msg.sender][index] == id) break;
        // }
        // require(
        //     _userVsLockIds[msg.sender].status == Status.OPEN,
        //     "Lock: Unauthorized access!!"
        // );
        _;
    }
    ...
    function lock(
        ...
        uint256 remValue =
            _lock(tokenAddress, amount, feesInEth, duration, amountThreshold, msg.value);
        ...
        whenNotPaused
    {
        ...
        newAmount = msg.value - fee;
        
        
        if (ETH_ADDRESS == lockedAsset.token) 
        {
            _checkValidValue(msg.value, amount, true, fee, feesInEth);
            _transferFee(fee, feesInEth);
        }
        else
        {   
            _checkValidValue(msg.value, amount, false, fee, feesInEth);
            _transferFee(fee, feesInEth);
    ...
    function claimable(uint256 id) public view returns (bool) {
        ...
        if (
            ...
            (asset.endDate <= block.timestamp ||
                ...
                (asset.amountThreshold > 0 &&
                    asset.amount >= asset.amountThreshold))
        ) {
            return true;
        }
        return false;
    ...
    function _lock(
        ...
        if (ETH_ADDRESS == tokenAddress) {
            newAmount = value - fee;
            _lockETH(amount, fee, feesInEth, endDate, amountThreshold, value);

            remValue = remValue.sub(amount);
        } else {
            //newAmount = amount - fee;
            _lockERC20(tokenAddress, amount, fee, feesInEth, endDate, amountThreshold, value);
        }
    ...
    /**
        ...
        msg.value - 1.05 (inETH)
        msg.value - 1 (in COIN)(approved - 0.05)
     */
    ...
    function _lockETH(
        ...
        bool amountinEth = true;
        _checkValidValue(value, amount, amountinEth, fee, feesInEth);
    ...
    // Removed extra function parameter
    ...
    function _checkValidValue(uint256 value, uint256 amount,bool amountinEth, uint256 fee,bool feesInEth )
    pure
    private
    {   
