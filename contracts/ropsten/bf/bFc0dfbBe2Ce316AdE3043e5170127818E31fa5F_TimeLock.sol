...
contract TimeLock is Ownable {
    ...
    uint constant public UNLOCK_DURATION = 7200; // 7,200 blocks = 4 hours
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 depositFeePercent; // 10000 = 100%
    }
    // THE POLLEN TOKEN!!!!
    ...
    uint256 public harvestDelayEndBlock;

    uint256 public HARVEST_DELAY = 7200; // 7,200 blocks = 4 hours
    ...
    constructor(
        ...
        POLLEN.mint(msg.sender, 420000 ether);

        depositFeeAddress = _depositFeeAddress;
        pollenPerBlock = _pollenPerBlock;
        bonusEndBlock = _bonusEndBlock;
        startBlock = _startBlock;
        harvestDelayEndBlock = startBlock.add(302400); // 302,400 blocks = 1 week
    }
    ...
    function add(
        ...
        require(_depositFeePercent <= 400, 'deposit fee must be less than 4%');
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0 && (block.number >= user.harvestUnlockBlock || block.number >= harvestDelayEndBlock)) {
            uint256 pending =
        ...
        uint256 depositFee = _amount.mul(pool.depositFeePercent).div(10000);
        ...
        uint256 amountAfterFees = _amount.sub(depositFee);
        
        user.amount = user.amount.add(amountAfterFees);
        user.rewardDebt = user.amount.mul(pool.accPollenPerShare).div(1e12);
        user.harvestUnlockBlock = (block.number).add(HARVEST_DELAY);
        pool.depositAmount = pool.depositAmount.add(amountAfterFees);
        emit Deposit(msg.sender, _pid, amountAfterFees);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (block.number >= user.harvestUnlockBlock || block.number >= harvestDelayEndBlock) {
            uint256 pending =
                user.amount.mul(pool.accPollenPerShare).div(1e12).sub(
                    user.rewardDebt
                );
    ...
    function updateDepositFeeAddress(address _depositFeeAddress) public {
        require(msg.sender == depositFeeAddress, "update: not called from depositFeeAddress");
        depositFeeAddress = _depositFeeAddress;
    }
}
...
contract PollenToken is ERC20("POLLEN Token", "POLLEN"), Ownable {
    /// @notice Can only be called by the owner (MasterChef).
