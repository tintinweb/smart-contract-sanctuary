...
contract ELONAndMARVIN is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isBlockedAddress;
    ...
    string private _name = 'ELON&MARVIN';
    string private _symbol = 'E&M';
    ...
    uint256 private _maxBalance = 30000000000e9;
    ...
    function setBlockAccount(address account) external onlyOwner() {
        require(!_isBlockedAddress[account], "Account is already blocked");
        _isBlockedAddress[account] = true;
    }
    
    function removeBlockAccount(address account) external onlyOwner() {
        require(_isBlockedAddress[account], "Account is not blocked");
        _isBlockedAddress[account] = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!_isBlockedAddress[sender] && !_isBlockedAddress[recipient], "sender or recipient is blocked");
        ...
        if(recipient != owner())
            require((balanceOf(recipient).add(amount)) <= _maxBalance, "Recipient's balance should be below than maxBalance");
    ...
    function _getMaxBalance() private view returns(uint256) {
        return _maxBalance;
    }
