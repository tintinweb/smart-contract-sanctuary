...
contract CallAgent is Ownable {
    ...
    function init(address owner, address adm) public {
...
contract DeployFactory is Ownable {
    ...
    event SetImplementation(address oldImplementation, address newImplementation);

    function setImplementation(address implementation) public onlyOwner {
        emit SetImplementation(currentImplementation, implementation);
    ...
    function newFilter(address admin, bool reCreate) public returns(address) {
        ...
        ERC1967Proxy c = new ERC1967Proxy(currentImplementation, abi.encodeWithSignature("init(address,address)", msg.sender, admin));
