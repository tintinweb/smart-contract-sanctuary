...
// File: contracts\TokenTroveTestToken.sol
...
contract TokenTroveTestToken is ERC721, Ownable {
    event TokenCreated(uint256 uuid, address to);
    ...
    constructor(string memory baseTokenURI) ERC721("TokenTroveTestToken", "TTTT") {
        _baseTokenURI = baseTokenURI;
        proxyMinter = _msgSender();
    }

    function createToken(uint256 tokenId, address to) public onlyOwner {
    ...
    function mintFor(address to, uint256 amount, bytes memory mintingBlob) public {
        ...
        (uint256[] memory tokenIds) = parseMintingBlob(mintingBlob, amount);
        for (uint i; i < tokenIds.length; i++) {
            createToken(tokenIds[i], to);
        }
    }
    ...
    // Parses a mintingBlob with {tokenId1}:{metadata}{tokenId2}:{metadata} and then returns them
    // Example: {1}:{rarity:epic, quality:diamond}{2}:{rarity:rare, quality:shadow}
    function parseMintingBlob(bytes memory mintingBlob, uint256 amount) internal pure returns (uint256[] memory) {
        uint256 offset = 0;
        ...
        for (uint i; i < mintingBlob.length; i++) {
            if (mintingBlob[i] == "{") {
                if (isTokenId) {
                    ...
                    (bytes32 uuid, uint256 nextItem) = parseMintingBlobMetadata(mintingBlob, i + 1);
                    isTokenId = true;
                    offset++;
                    i = nextItem;
                }
    ...
    function parseMintingBlobMetadata(bytes memory mintingBlob, uint start) internal pure returns (bytes32, uint256) {
