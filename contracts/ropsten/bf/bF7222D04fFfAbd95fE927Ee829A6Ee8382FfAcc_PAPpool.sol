...
contract InfoBundler {
    function getPoolsInfo(address[] memory pools)
        external
        view
        returns (PAPpool.Pool[][] memory)
    {
        uint256 len = pools.length;
        PAPpool.Pool[][] memory infos = new PAPpool.Pool[][](len);
        for (uint256 i = 0; i < len; i++) {
            infos[i] = new PAPpool.Pool[](3);
        }
        for (uint256 i = 0; i < len; i++) {
            (
                PAPpool.Pool memory tier1,
                PAPpool.Pool memory tier2,
                PAPpool.Pool memory tier3
            ) = PAPpool(pools[i]).getPapPoolInfo();
            infos[i][0] = tier1;
            infos[i][1] = tier2;
            infos[i][2] = tier3;
        }
