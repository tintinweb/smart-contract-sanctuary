...
abstract contract PreSale is Context{
    ...
    function preSaleBuyBag(address pInvestor, uint256 pSize) public returns(bool){
        ...
        require(allowance >= _preSaleBagPrice[pSize], "[XTKN061325::ERROR] you have to authorize the transfer of your BUSD first or you do not have enough BUSD!");
        require(_preSaleBagsAvailableCheck(pSize), "[XTKN061325::ERROR] no more of these bags available!");
        require(!_preSaleInvestorHasThisBag(pInvestor, pSize), "[XTKN061325::ERROR] you can only buy one of this bag!");
        require(_preSaleBuyBag(pInvestor, pSize), "[XTKN061325::ERROR] could not buy the bag!");
        return(true);
    }
    ...
    function _preSaleAddBag(uint256 pSize, uint256 pMax, uint256 pPrize) private{
        uint256 index = _preSaleToWei(pSize);
        _preSaleBagsAvailable[index] = pMax;
        _preSaleBagPrice[index] = _preSaleToWeiToken(pPrize);
    }
...
contract IXTKN061325Liquidity{
    function init() public{}
    function getPair() public returns(address){}
}


contract IXTKN061325Project{
    function init() public{}
...
*/
contract XTKN061325 is Context, IERC20, Ownable, RobinHood, PreSale{
    // public properties
        // lib
            using SafeMath for uint256;
            using Address for address;
        ...
        // interfaces
            ...
            IXTKN061325Liquidity private _liquidity;
            IXTKN061325Project private _project;
            IXTKN061325Staking private _staking;
    ...
    // private properties
        ...
        // how our token is setup, standard one billion tokens, but with 9 decimals instead of the 18
            string private _name = "XTKN061325"; // the name of our project
            string private _symbol = "XTKN061325";
    ...
    function initLiquidity(address contractAddress) public onlyOwner{
        if(!_contractLiquidityActive){
            ...
            if(ADDRESS_LIQUIDITY.balance > 0){
                ...
                // init contract
                _liquidity = IXTKN061325Liquidity(ADDRESS_LIQUIDITY);
                _liquidity.init();
    ...
    function initProject(address contractAddress) public onlyOwner{
        if(!_contractProjectActive){
            ...
            // init contract
            _project = IXTKN061325Project(ADDRESS_PROJECT);
            _project.init();
    ...
    function initStaking(address contractAddress) public onlyOwner{
        if(!_contractStakingActive){
            ...
            // init contract
            _staking = IXTKN061325Staking(ADDRESS_STAKING);
            _staking.init();
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[XTKN061325::ERROR] approve from the zero address");
        require(spender != address(0), "[XTKN061325::ERROR] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[XTKN061325::ERROR] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[XTKN061325::ERROR] transfer amount exceeds allowance"));
        return(true);
    }
