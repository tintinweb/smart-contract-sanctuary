...
contract ProofOfStakeToken {
    ...
    event ContinuousMint(address indexed _address, uint _tokenMinted);
}
...
contract ETH419v5 is ERC20, ProofOfStakeToken, Ownable {
    using SafeMath for uint256;

    string public name = "ETH419v5";
    string public symbol = "ETH419v5";
...
//------------------------------------------------------------------------------
    ...
    function continuousMint() internal returns (uint256) {
        uint tokenMinted = getMintReward(msg.sender);
        Mint(msg.sender, tokenMinted);
        ContinuousMint(msg.sender, tokenMinted);
        return tokenMinted;
    }
