...
contract PriceCalculatorMatic is IGovernable, Initializable, Controllable {
  using SafeERC20 for IERC20;
  using Address for address;
  using SafeMath for uint256;

  uint256 constant public precisionDecimals = 18;
  ...
  function getPrice(address token, address outputToken) external view returns (uint256) {
    ...
    if (isSwapPlatform(token)) {
      ...
      for (uint256 i = 0; i < 2; i++) {
        uint256 priceToken = computePrice(tokens[i], outputToken);
        if (priceToken == 0) {
          return 0;
        }
        uint256 tokenValue = priceToken * amounts[i] / 10 ** precisionDecimals;
        price += tokenValue;
      }
      return price;
    } else {
      return computePrice(token, outputToken);
    }
  }
  ...
  function computePrice(address token, address outputToken) public view returns (uint256) {
    uint256 price;
    if (token == outputToken) {
      return 10 ** precisionDecimals;
    ...
    (address keyToken, , uint256 factoryIdx,) = getLargestPoolForSwaps(token, keyTokens);
    if (keyToken == address(0)) {
      return 0;
    }

    uint256 lpPrice = getPriceFromLp(
      swapFactories[factoryIdx],
      token,
      keyToken
    );
    ...
    uint256 keyTokenPrice = computePrice(keyToken, outputToken);
    price = lpPrice * keyTokenPrice / 10 ** precisionDecimals;
    return price;
  }

  // Gives the LP with largest liquidity for a given token
  // and a given tokenset (either keyTokens or pricingTokens)
  function getLargestPoolForSwaps(address token, address[] memory tokenList)
  public view returns (address, uint256, uint256, address) {
    ...
    for (uint256 i = 0; i < tokenList.length; i++) {
      ...
      for (uint256 j = 0; j < swapFactories.length; j++) {
        (uint256 poolSize, address lp) = getPoolSizeForFactory(swapFactories[j], token, tokenList[i]);
    ...
    return (largestKeyToken, largestLpSize, largestPlatformIdx, lpAddress);
  }

  function getPoolSizeForFactory(address _factory, address token0, address token1)
  ...
  function getPriceFromLp(address _factory, address token0, address token1) public view returns (uint256) {
    address pairAddress = IUniswapV2Factory(_factory).getPair(token0, token1);
    IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
