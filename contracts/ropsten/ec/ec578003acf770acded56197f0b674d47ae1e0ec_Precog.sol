...
contract Precog {
    ...
    uint256 public feeWithdraw = 2e6; // 2 USDC
    uint256 public feeTradingEth = 1e17; // 0.1 ETH
    ...
    address[] tradeUsers;
    mapping(address => uint256) public tradeUserIndex;
    ...
    function setFeeWithdraw(uint256 fee) external onlySetFeeAdmin{
        feeWithdraw = fee;
    }
    ...
    function withdraw(uint256 amount, address to) external {
        require( availableAmount[msg.sender] >= amount + feeWithdraw , "PrecogV2: AVAILABLE_AMOUNT_NOT_ENOUGH");
    ...
    function _wthdraw(uint256 amount) internal {
        availableAmount[msg.sender] -= amount + feeWithdraw;
        emit Withdraw(msg.sender, amount, feeWithdraw, block.timestamp);
    }
    ...
    function withdrawLiquidate(address futureToken, address to) external {
        uint amount = _liquidate(futureToken) - feeWithdraw;
        IERC20(usdc).transfer(to, amount);
        emit Withdraw(msg.sender, amount, feeWithdraw, block.timestamp);
    ...
    function _swapTradingFee() internal {
        ...
        (address exchange, uint256 feeTradingUsdc) = _selectBestPriceExchange(pair, feeTradingEth);
        if (address(exchange) != address(0)) {
            IExchangeRouter(exchange).swapTokensForExactETH(feeTradingEth, feeTradingUsdc, pair, tradingService, deadline);
        }
    }
    ...
    function maxProfitable(address user) public view returns (
        ...
        uint256 feeTradingUsdc = _convertEthToUsdc(feeTradingEth);
