...
contract BleedFomo is Ownable, Settings{
    ...
    event onDebug(string str, address addr, uint256 v1, uint256 v2, uint256 v3, uint256 v4);
        ...
        if (sharePotUnTreate > 0){
            for (i=0; i<executeCount; i++){
                ...
                if (checkoutCurr >= userkeys.length) {
                    ...
                    uint64 userkeysnum = isactivate ? userkeysnum = userkeys[checkoutCurr].num_pending : userkeys[checkoutCurr].num;
                    if (userkeysnum > 0){
                        address uaddr = userkeys[checkoutCurr].useraddr;
                        tmpval = sharenumEveryKey.mul(userkeysnum) / decimalPercent;
                        emit onDebug("shareone", uaddr, tmpval, sharenumEveryKey, userkeysnum, 0);
                        if (tmpval > 0){
