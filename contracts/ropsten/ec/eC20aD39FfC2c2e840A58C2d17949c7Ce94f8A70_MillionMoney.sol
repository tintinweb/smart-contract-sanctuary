...
contract MillionMoney {
    ...
    constructor() public {
        ...
        // users[ownerWallet].refferral = new address[](0);
    ...
    function regUser(uint _referrerID) public payable {
        ...
        users[msg.sender].levelExpired[1] = now + PERIOD_LENGTH; //now = block.timestamp
    ...
    function buyLevel(uint _level) public payable {
        ...
        if(_level == 1) {
            ...
            require(users[msg.sender].levelExpired[l] >= now, 'Buy the previous level');  //now = block.timestamp
    ...
    function payForLevel(uint _level, address _user) internal {
        ...
        bool sent = false;
        if(users[referer].levelExpired[_level] >= now) { //now = block.timestamp
