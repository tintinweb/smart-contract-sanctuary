...
contract AngryContract is ReentrancyGuard{
    ...
    event OrderConfirm(address _userAddr, uint256 _orderIdx, uint256 _amountOutMin, uint8 _orderStatus);
    ...
    function processPrePurchaseOrder(address _addr, uint256 _orderIdx, uint256 _amountOutMin) nonReentrant private {
        ...
        repurchase = pcInfo.paymentAmount;
    ...
    function confirmPrePurchaseOrder(address _addr, uint256  _orderIdx, uint256 _amountOutMin) onlyExecutor public {
        PrePurchaseInfo[] storage purchases = prePurchaseList[ _addr ];
        require( purchases.length > _orderIdx, "OR" );
        PrePurchaseInfo storage pcInfo = purchases[ _orderIdx ];
        require( pcInfo.status == 0, "US" );
        pcInfo.status = 3;
        processPrePurchaseOrder(_addr, _orderIdx, _amountOutMin);
        emit OrderConfirm(_addr, _orderIdx, _amountOutMin, pcInfo.status);
    }
