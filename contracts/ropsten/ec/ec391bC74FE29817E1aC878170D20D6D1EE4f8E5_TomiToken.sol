...
contract TomiToken is ERC20Burnable {
    ...
    uint256 private constant TOTAL_SUPPLY = 1500 * 10**(6 + 18);
    ...
    constructor(
        ...
        _transfer(_owner, community, _pools_amount[COMMUNITY_INDEX]);
        _minted_pool[COMMUNITY_INDEX] = true;

        _transfer(_owner, develop, _pools_amount[DEVELOPMENT_INDEX]);
        _minted_pool[DEVELOPMENT_INDEX] = true;

        _transfer(_owner, liquidity, _pools_amount[LIQUIDITY_INDEX]);
    ...
    function addLocker(uint8 pool_index, address pool_address) external {
        ...
        _transfer(_owner, pool_address, _pools_amount[pool_index]);
