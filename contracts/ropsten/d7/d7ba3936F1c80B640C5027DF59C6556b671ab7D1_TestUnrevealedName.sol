...
contract TestUnrevealedName is ERC721, Mintable {
    ...
    // I guess I can add any native ERC721 Contract custom functions here!
    // Returning tokenURI to be compatible with something like opensea standard
    string baseTokenURI;
    string baseTokenURI_EXT;
    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        require(_exists(tokenId_), "Unminted token!");
        return string(abi.encodePacked(baseTokenURI, Strings.toString(tokenId_), baseTokenURI_EXT));
    }
    function setTokenURIs(string memory uri_, string memory ext_) public onlyOwner {
