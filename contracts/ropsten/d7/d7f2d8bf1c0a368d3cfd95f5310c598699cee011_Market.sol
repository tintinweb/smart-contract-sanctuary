...
// File: contracts/ReputationToken.sol

contract ReputationToken is Ownable, StandardToken {
  string public constant name = "ReputationToken";
  string public constant symbol = "MREP";
  uint8 public constant decimals = 0;

  mapping (address => uint) blocked;

  function inflateToken(address target, uint amount)
  onlyOwner public {
    balances[target] = balances[target].add(amount);
    Transfer(0, target, amount);
  }

  function burnToken(address target, uint amount)
  onlyOwner {
    if(amount > balances[target]) amount = balances[target];
    balances[target] = balances[target].sub(amount);
    Transfer(target, 0, amount);
  }

  function blockTokens(address target, uint amount)
  onlyOwner public {
    blocked[target] = blocked[target].add(amount);
  }

  function unblockTokens(address target, uint amount)
  onlyOwner public {
    blocked[target] = blocked[target].sub(amount);
  }

  function transfer(address _to, uint256 _value) public returns (bool) {
    require(balances[msg.sender].sub(blocked[msg.sender]) >= _value);
    return super.transfer(_to, _value);
  }

  function transferFrom(address _from, address _to, uint256 _value)
  public returns (bool) {
    require(balances[_from].sub(blocked[_from]) >= _value);
...
contract Market is Ownable, RBAC {
  string public constant ROLE_WHITELIST = "whitelist";
  string public constant ROLE_ARBITER = "arbiter";
  ...
  function setRole(address addr, string role, bool set)
  onlyRole(ROLE_ADMIN) public {
    if(set) {
      addRole(addr, role);
    } else {
      if(addr == owner) require(msg.sender == owner, "only owner can remove roles from owner");
      removeRole(addr, role);
    }
  }

  function addOffer(string product, uint price, address arbiter)
  onlyRole(ROLE_WHITELIST)
  public returns (uint) {
    checkRole(arbiter, ROLE_ARBITER);
  ...
  inState(id, Status.OFFERED)
  onlyRole(ROLE_WHITELIST) public
  payable {
    Offer storage offer = offers[id];
    ...
    reputation.blockTokens(offer.creator, offer.price);
  }
  ...
  function finalize(uint id) internal {
    ...
    reputation.inflateToken(offer.creator, offer.price);
  ...
  public {
    ...
    if(delivered) {
      ...
      if(burn) {
        reputation.burnToken(offer.creator, offer.price);
        reputation.unblockTokens(offer.creator, offer.price);
      }
    }
  }
