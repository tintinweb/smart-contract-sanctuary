...
contract StormSender is OwnedUpgradeabilityStorage, Claimable {
    ...
    event MultiSendedToContract(uint256 total, address tokenAddress);
...
//        require(_newStep != 0);
    ...
    function multisendEther(address[] _contributors, uint256[] _balances) public payable{
        ...
        setTxCount(msg.sender, txCount(msg.sender).add(1));
        MultiSendedToContract(msg.value, 0x000000000000000000000000000000000000bEEF);
    }

    function multiSendToContract(address contractAddress, uint256 times) public payable{

        uint256 i = 0;
        for (i; i < times; i++) {
            contractAddress.transfer(0);
        }
