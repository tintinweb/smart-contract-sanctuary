...
contract Octopus is Ownable {
    ...
    function sell(address _optionAddress, uint256 _weiNotional) payable public {
        ...
        IOption(_optionAddress).mint(address(this), ethNotional);
    ...
    //Alice / Seller
    function sell2(address _optionAddress, uint256 _weiNotional) payable public {
        require(options[_optionAddress].isValid, "Invalid option");
        require(IOption(_optionAddress).getMaturity() < block.timestamp, "Expired option");
        ...
        //We sell the tokens for USDT in Uniswap, which is sent to the user
        uint256 premium = 0; //sellTokensInUniswap(_optionAddress, _weiNotional); //TODO: Substract the fee before
    ...
    //Alice / Seller
    function sell3(address _optionAddress, uint256 _weiNotional) payable public {
        require(options[_optionAddress].isValid, "Invalid option");
        require(IOption(_optionAddress).getMaturity() < block.timestamp, "Expired option");
    ...
    function sell4(address _optionAddress, uint256 _weiNotional) payable public {
        (uint256 usdCollateral, uint256 weiCollateral) = calculateCollateral(_optionAddress, _weiNotional);
        ...
        //We emit an event to be able to send KiboTokens offchain, according to the difference against the theoretical Premium
        emit OptionPurchase(_optionAddress, msg.sender, _weiNotional, usdCollateral, weiCollateral, 0);
    }
    
    function sell5(address _optionAddress, uint256 _weiNotional) payable public {
        (uint256 usdCollateral, uint256 weiCollateral) = calculateCollateral(_optionAddress, _weiNotional);
    ...
    function sell6(address _optionAddress, uint256 _weiNotional) payable public {
        require(options[_optionAddress].isValid, "Invalid option");
        require(IOption(_optionAddress).getMaturity() < block.timestamp, "Expired option");
    ...
    function calculateCollateral(address _optionAddress, uint256 _notionalInWei) public view returns (uint256, uint256) {
        ...
        uint256 notionalInEth = _notionalInWei.div(1e18);
        
        //Collateral = Strike * Notional (in ETH, not WEI)
        uint256 collateralInUSD = IOption(_optionAddress).getStrike().mul(notionalInEth);
    ...
    function claimCollateralAtMaturityForSellers(address _optionAddress) public {
        ...
        require(usdtToken.transferFrom(address(this), msg.sender, totalToReturn), "Transfer failed");
    }
    ...
    function claimCollateralAtMaturityForBuyers(address _optionAddress, uint256 _numberOfTokens) public {
        ...
        require(ERC20(usdtToken).transferFrom(address(this), msg.sender, totalToReturn), "Transfer failed");
    }
    ...
    function createPairInUniswap(address _optionAddress, uint256 _totalTokens, uint256 _totalUSDT) public returns (uint amountA, uint amountB, uint liquidity) {
        uint256 allowance = usdtToken.allowance(address(this), address(uniswapRouter));
        if (allowance == 0) {
            usdtToken.approve(address(uniswapRouter), _totalUSDT);
        }
    ...
    function _activateOption(address _optionAddress, uint256 _uniswapInitialUSDT, uint256 _uniswapInitialTokens) public payable onlyOwner {
        require(IOption(_optionAddress).getMaturity() < block.timestamp, "Expired option");
    ...
    function _activateOption2(address _optionAddress, uint256 _uniswapInitialUSDT, uint256 _uniswapInitialTokens) public payable onlyOwner {
        require(IOption(_optionAddress).getMaturity() < block.timestamp, "Expired option");
        require(!options[_optionAddress].isValid, "Already activated");
        require(msg.value > 0, "Collateral cannot be zero");
    ...
    function _setEthFinalPriceAtMaturity(address _optionAddress) public onlyOwner {
        ...
        uint256 optionWorth = IOption(_optionAddress).getStrike().sub(usdPriceOfEth);
