...
contract Richer3D {
    ...
    function calculateTarget() public {
        ...
        if(increaseBalance >= targetBalance) {
            ...
            rInfoXrID[roundNumber].increaseETH = rInfoXrID[roundNumber].increaseETH.sub(getETHNeedPay(roundNumber,dayNumber.sub(1)));
    ...
    function getBounsWithRoundID(uint256 _roundID) private view returns(uint256 _bouns) {
        _bouns = _bouns.add(rInfoXrID[_roundID].bounsInitNumber).add(rInfoXrID[_roundID].increaseETH.mul(891).div(1000));
        uint256 totalDay = rInfoXrID[_roundID].totalDay;
        uint256 todayNeedToPay = getETHNeedPay(_roundID,totalDay);
        _bouns = _bouns.sub(todayNeedToPay);
        return(_bouns);
    }
    
    function getETHNeedPay(uint256 _roundID,uint256 _dayID) private view returns(uint256 _amount) {
        if(_dayID >=2) {
            uint256 mineTotal = rInfoXrID[_roundID].totalMine.sub(rInfoXrID[_roundID].dayInfoXDay[_dayID-1].totalMine).sub(rInfoXrID[_roundID].dayInfoXDay[_dayID-2].totalMine);
            _amount = mineTotal.mul(getTransformRate()).div(10000);
        } else {
            uint256 mineTotal2 = rInfoXrID[_roundID].totalMine.sub(rInfoXrID[_roundID].dayInfoXDay[_dayID-1].totalMine);
            _amount = mineTotal2.mul(getTransformRate()).div(10000);
        }
        return(_amount);
    }
