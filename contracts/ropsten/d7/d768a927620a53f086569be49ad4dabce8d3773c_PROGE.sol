...
contract PROGE is Context, IERC20, Ownable {
    ...
    string private constant _name = "The Protector Roge";
    ...
    function manualSwapAndLiquify() public onlyOwner() {
        uint uniswapBalance = balanceOf(uniswapV2Pair);
        uint threePercentOfUNI = uniswapBalance.mul(3).div(100);
        uint contractTokenBalance = balanceOf(address(this));
        if (contractTokenBalance > threePercentOfUNI) {
            contractTokenBalance = threePercentOfUNI;
        }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) { //exlude buy sell code from owner
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                uint animalSanctuaryBalance = balanceOf(animalSanctuary);
                uint maxBuy = (_tTotal.sub(animalSanctuaryBalance)).div(100);
                require (amount <= maxBuy, "!> 1% circ supply");
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) { 
                ...
                uint uniswapBalance = balanceOf(uniswapV2Pair);
                uint threePercentOfUNI = uniswapBalance.mul(3).div(100);
                require (amount <= threePercentOfUNI, "!> 3% market impact");
