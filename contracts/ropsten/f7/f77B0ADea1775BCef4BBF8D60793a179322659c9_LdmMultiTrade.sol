...
contract LdmMultiTrade {
    using SafeERC20 for IERC20;
    
    // Bancor
    IBancorNetwork private constant bancorNetwork = IBancorNetwork(0xb3fa5DcF7506D146485856439eb5e401E0796B5D);
    address private constant BANCOR_ETH_ADDRESS = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE; //weth地址
    ...
    constructor() {
        ...
        IERC20(USDC).safeApprove(address(uniswapRouter), type(uint256).max);
    ...
    event Earn(uint256 _old,uint256 _new,uint256 earn);
    ...
    function _tradeOnUniswap(uint256 amountIn, uint256 amountOutMin, uint256 deadline) private {
        address tokenIn = INJ;
        address tokenOut = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
        uint24 fee = 3000;
    ...
    function _sushiTradeETH(uint256 amountIn, uint256 amountOutMin,uint256 deadline) private {
        address[] memory path =  new address[](2);
        path[0] = INJ;
        path[1] = 0xc778417E063141139Fce010982780140Aa0cD5Ab;

        address recipient = msg.sender;
        sushiRouter.swapExactTokensForETH(amountIn, amountOutMin,path, recipient, deadline);
    }
    ...
    function multiSwapPreview() external payable returns(uint256) {
        ...
        _sushiTradeETH(IERC20(BNT).balanceOf(address(this)), amountOutMinSushiSwap, deadline);
        // _uniswapTradeETH(IERC20(INJ).balanceOf(address(this)), amountOutMinUniswap, deadline);
        //这里换直接换为ETH
        ...
        uint ethEndBalance = msg.sender.balance;
        uint256 result = ethEndBalance - ethBalance;


        emit Earn(ethBalance,ethEndBalance,result);
        return result;
    }
