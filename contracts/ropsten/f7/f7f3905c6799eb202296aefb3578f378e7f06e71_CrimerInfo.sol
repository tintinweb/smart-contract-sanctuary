...
contract CrimerInfo is Ownable {
  ...
  function crimerList(address[] memory _crimers, address stealer, uint8 crimerCount) public view returns(uint256[] memory cash, uint256[] memory power, bool[] memory isProtected, uint256[] memory totalStolenCount) {
    for(uint8 i=0;i<crimerCount;i++) {
      ...
      isProtected[i] = crimers[_crimers[i]].isProtected;
