...
modifier onlyOwner(){
    ...
    ****************************************/
    ...
    function _harvest(address sender) internal{
        ...
        uint256 timeNow = now;
        uint256 locktime = timeNow.sub(lockingTimeStarts[sender]);
    ...
    function myReward(address sender) external view returns (uint256) {
        require(lockedAmount[sender] >= 0, 'you have not locked anything');
        uint256 timeNow = now;
        uint256 timeStart = lockingTimeStarts[sender];
        uint256 locktime = timeNow.sub(timeStart);
        uint256 reward = lockedAmount[sender].mul(lockRate).mul(locktime).div(2592000);
        return reward;
    }
    ...
    function myLockedPeriod(address sender) external view returns (uint256) {
        ...
        uint256 timeNow = now;
        uint256 timeStart = lockingTimeStarts[sender];
        uint256 locktime = timeNow - timeStart;
        return locktime;
    }
    
    function myTimeStarts(address sender) external view returns (uint256) {
        require(lockedAmount[sender] >= 0, 'you have not locked anything');
        return lockingTimeStarts[sender];
    }
