...
contract HardStakingNFTAuction is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    bool public allowAnyToken;

    uint public lastAuctionId;
    mapping(uint => Auction) public auctions;
    mapping(uint => CollectionToken) public auctionCollectionToken;
    mapping(address => mapping(uint => uint)) public tokenLastAuctionId;
    ...
    event Staked(address indexed user, uint stakeAmount, uint totalRelativeBid, uint indexed auctionId, address tokenAddress, uint indexed tokenId, bool isRestake);
    event NewAuction(uint indexed auctionId, address indexed tokenAddress, uint indexed tokenId);
    event AuctionInited(uint indexed auctionId, uint auctionEnd, address indexed tokenAddress, uint indexed tokenId);
    event NewTopBid(uint indexed auctionId, address tokenAddress, uint indexed tokenId, uint relativeBidAmount, uint actualBidAmount, address indexed bidder);
    event Withdraw(address indexed user, uint burnAmount);
    event BurnForToken(uint burnAmount, uint receiveAmount);
    event AuctionTokenWithdraw(address indexed winner, address previousOwner, uint bidAmount, uint fee, address tokenAddress, uint indexed tokenId, uint indexed auctionId);
    ...
    event RescueAuctionToken(address indexed to, uint indexed auctionId, address tokenAddress, uint indexed tokenId, bool wasAuctionFinished);
    event UpdateCollectionTokens(address indexed tokenAddress, bool indexed isAllowed);
    ...
    constructor(
        ...
        require(Address.isContract(_burnToken) && Address.isContract(_stakeToken) && Address.isContract(_auctionToken), "HardStakingNFTAuction: Not contract(s)");
        burnToken = IBurnable(_burnToken);
        stakeToken = IERC20Permit(_stakeToken);
        isAllowedCollection[_auctionToken] = true;
    ...
    function relativeBidAmount(uint auctionId, uint bid) public view returns (uint) {
        ...
        return bid * (auction.auctionEnd - block.timestamp) / auction.auctionRoundDuration;
    }
    ...
    function getNextBidMinAmount(uint auctionId) external view returns (uint) {
        ...
        if (auction.auctionEnd != 0 && auction.auctionEnd > block.timestamp) {
            ...
            return nextBid * auction.auctionRoundDuration / (auction.auctionEnd - block.timestamp);
    ...
    function withdrawAndGetRewardByTokenId(address tokenAddress, uint tokenId) external {
        uint auctionId = tokenLastAuctionId[tokenAddress][tokenId];
        getReward();
        withdraw(auctionId);
    }

    function withdrawByTokenId(address tokenAddress, uint tokenId) external {
        uint auctionId = tokenLastAuctionId[tokenAddress][tokenId];
        withdraw(auctionId);
    }
    ...
    function processSuccesfullAuction(uint auctionId) public { 
        ...
        IERC721Metadata auctionToken = IERC721Metadata(auction.tokenAddress);
        if (winner == address(0)) {
            auctionToken.transferFrom(address(this), winner, tokenId);
            emit RescueAuctionToken(winner, auctionId, address(auctionToken), tokenId, true);
        } else {
            _getReward(winner);
            ...
            emit AuctionTokenWithdraw(auction.topBidder, auction.owner, bidFinalAmount, successFee, address(auctionToken), tokenId, auctionId);
        }
    }

    function startNewAuctions(address[] memory tokenAddresses, uint[] memory tokenIds, uint[] memory startBidAmounts, uint[] memory roundDurations, string[] memory descriptions) external { 
        require(tokenIds.length == startBidAmounts.length, "HardStakingNFTAuction: Wrong lengths");
        require(tokenIds.length == tokenAddresses.length, "HardStakingNFTAuction: Wrong lengths");
        require(tokenIds.length == roundDurations.length, "HardStakingNFTAuction: Wrong lengths");
        require(tokenIds.length == descriptions.length, "HardStakingNFTAuction: Wrong lengths");
        for (uint i; i < tokenIds.length; i++) {
            startNewAuction(tokenAddresses[i], tokenIds[i], startBidAmounts[i], roundDurations[i], descriptions[i]);
        }
    }

    function startNewAuction(address tokenAddress, uint tokenId, uint startBidAmount, uint roundDuration, string memory description) public {
        require(roundDuration >= MIN_AUCTION_DURATION, "HardStakingNFTAuction: Auction duration is too short");
        require(allowAnyToken || isAllowedCollection[tokenAddress], "HardStakingNFTAuction: Not supported collection");
        ...
        auctionCollectionToken[auctionId] = CollectionToken(tokenAddress, tokenId);
        tokenLastAuctionId[tokenAddress][tokenId] = auctionId;
        
        auctions[auctionId].tokenAddress = tokenAddress;
        ...
        auctions[auctionId].auctionRoundDuration = roundDuration;

        emit NewAuction(auctionId, tokenAddress, tokenId);
    }

    function rescueUnbiddenTokenByTokenId(address tokenAddress, uint tokenId) external {
        uint auctionId = tokenLastAuctionId[tokenAddress][tokenId];
    ...
    function rescueUnbiddenToken(uint auctionId) public { 
        ...
        IERC721Metadata(tokenAddress).transferFrom(address(this), msg.sender, auction.tokenId);
        emit RescueAuctionToken(msg.sender, auctionId, tokenAddress, auction.tokenId, false);
    }
    ...
    function _stake(uint amount, uint auctionId, address user, bool isBurnToken, bool isRestake) private nonReentrant {
        ...
        uint totalRelativeBid;
        {
        uint currentRelativeBid = amount * (auction.auctionEnd - block.timestamp) / auction.auctionRoundDuration;
        totalRelativeBid = userRelativeBidAmounts[user][auctionId] + currentRelativeBid;
        userRelativeBidAmounts[user][auctionId] = totalRelativeBid;

        emit Staked(user, amount, totalRelativeBid, auctionId, auction.tokenAddress, auction.tokenId, isRestake);
        }
        ...
        if (totalRelativeBid >= nextBid ) {
            ...
            emit NewTopBid(auctionId, auction.tokenAddress, auction.tokenId, totalRelativeBid, amount, user);
        }
    }
    ...
    function _initAuction(uint auctionId) private {
        uint auctionEnd = block.timestamp + auctions[auctionId].auctionRoundDuration;
        auctions[auctionId].auctionEnd = auctionEnd;
        emit AuctionInited(auctionId, auctionEnd, auctions[auctionId].tokenAddress, auctions[auctionId].tokenId);
    }
    ...
    function updateCollectionTokens(address tokenAddress, bool isAllowed) external onlyOwner {
        if (isAllowed) require(Address.isContract(tokenAddress), "HardStakingNFTAuction: Not a contract");
        isAllowedCollection[tokenAddress] = isAllowed;
    }
