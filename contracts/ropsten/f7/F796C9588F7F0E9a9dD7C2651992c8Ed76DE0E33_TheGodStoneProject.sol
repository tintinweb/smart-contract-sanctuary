...
contract TheGodStoneProject is ERC721, ERC721Enumerable, Ownable {
    ...
    */
    ...
    // }
    function getActiveStoneSupply(uint256 _indx) public view returns(uint256 totalSupply, uint256 activeStones){
        
        Stone[] memory stones = mapStoneClass[_indx].arrayOfStones;
        uint256 activeStoneCount = 0;

        for (uint256 i = 0; i < stones.length; i ++) {
            if (stones[i].isUsed == false) {
                activeStoneCount = activeStoneCount + 1;
            }
        }
        return (stones.length, activeStoneCount);
    }



    /**
    * GET PROPERTY OF A STONE FOR A TOKENID
    * RETURNS:
    * Stone
    */
    function getStoneProperty(uint256 tokenId) public view returns (Stone memory stone){
        // There are 8 classes of stones (1-8)
        uint256 class = 1;
        for (class; class <= 8; class++) {
            Stone[] memory stones = mapStoneClass[class].arrayOfStones;
            for (uint256 x = 0; x < stones.length; x ++) {
                if (stones[x].stoneId == tokenId) {
                    return stones[x];
                }
            }
        }  
...
/**
* RETURNS:
* Array of Stones(Struct) for a given array of tokenIds
*/
    function getAllStoneProperties (uint256[] memory arrayTokenIds) public view returns (Stone[] memory stones) {

        Stone[] memory _stones = new Stone[](arrayTokenIds.length);
        for (uint i = 0; i < arrayTokenIds.length; i ++) {
            _stones[i] = getStoneProperty(arrayTokenIds[i]);
        }
    ...
    // /**
    // * RETURN:
    // * ARRAY OF TOKEN URI FOR ALL THE TOKENS AN OWNER POSSESSES
    // */
    // function getAllTokenURI (address _addres) public view returns (address[] memory arrayOfTokenURI) {
    //     uint256[] allTokenIdsOwned = 
