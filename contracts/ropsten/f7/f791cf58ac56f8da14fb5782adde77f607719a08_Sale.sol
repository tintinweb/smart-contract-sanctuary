...
contract Sale {
    ...
    function getTimeLeft() public view returns(uint256) {
        ...
        if (timePassed >= maxTimeInHours) {
            return 0;
        }
        return maxTimeInHours.sub(timePassed);
    }
    ...
    function distribute(address referredAddress) private {
        if (players[msg.sender].addr == address(0)) {
            ...
            arrPlayers.push(players[msg.sender]);
        }
        ...
        if (referredAddress != address(0) && referredAddress != msg.sender)
        {
            ...
            uint256 tokenBonusByReferral = msg.value.mul(referrerBonus).div(100);
        ...
        uint256 timePassed = now.sub(timestamp);
        if (maxTimeInHours.sub(timePassed) > maxTimeInHours) {
            timestamp = maxTimeInHours;
        }
        ...
        uint256 total = totalSaled.add(TotalYUMToDistribute);
        require(total<=maxSale, "Cannot distribute YUM token more than its maxsale");
