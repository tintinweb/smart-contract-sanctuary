...
/**
 *
 * NameChangeToken Contract (The native token of CosmoMasks)
 * @dev Extends standard ERC20 contract
 */
contract NameChangeToken is Context, IERC20Burnable {
    ...
    constructor(uint256 emissionStartTimestamp_) {
        _name = "NameChangeToken";
        _symbol = "NCT";
        _decimals = 18;
        emissionStart = emissionStartTimestamp_;
        emissionEnd = emissionStartTimestamp_ + (1 * 365 * 10);
    }
    ...
    /**
     * @dev When accumulated NCTs have last been claimed for a CosmoMask index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(
            ICosmoMasks(_masksAddress).ownerOf(tokenIndex) != address(0),
            "Owner cannot be 0 address"
        );
        require(
            tokenIndex < ICosmoMasks(_masksAddress).totalSupply(),
    ...
    /**
     * @dev Accumulated NCT tokens for a CosmoMask token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment =
                ICosmoMasks(_masksAddress).isMintedBeforeReveal(tokenIndex) ==
                    true
    ...
    /**
     * @dev Claim mints NCTs and supports multiple CosmoMask token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenIndices.length; i++) {
            ...
            require(
                tokenIndices[i] < ICosmoMasks(_masksAddress).totalSupply(),
                "NFT at index has not been minted yet"
            );
            ...
            require(
                ICosmoMasks(_masksAddress).ownerOf(tokenIndex) == msg.sender,
...
interface ICosmoMasks is IERC721Enumerable {
