...
contract SatoshiART1155Marketplace is AccessControl {
    ...
    function putOnSale(uint256 tokenId, uint256 price) external {
        require(
            ...
            "Put on sale: caller cannot be zero address"
        );
        require(
            satoshiART1155.balanceOf(msg.sender, tokenId) >= 1,
            "Put on sale: you are trying to sell more than you have"
        );
    ...
    function setAsDropOfTheDay(
        ...
        require(
            ...
            "Caller is not a drop of the day creator"
        );
        require(
            satoshiART1155.balanceOf(itemOwner, tokenId) >= 1,
    ...
    function setAsAuction(
        ...
        require(
            msg.sender != address(0),
            "Set as auction: caller cannot be zero address"
        );
        require(
            _listings[tokenId][msg.sender].status == 0x02,
            "auction: token already on sale"
        );
        require(
            satoshiART1155.balanceOf(msg.sender, tokenId) >= 1,
            "auction: owner is trying to sell more than they have"
        );
    ...
    function setAsDropOfTheDayAuction(
        uint256 tokenId,
        address itemOwner,
        uint256 price,
        uint256 startTime,
        uint256 endTime,
        uint256 auctionCommission
    ) external {
        require(
            _listings[tokenId][itemOwner].status == 0x02,
            "drop of the day auction: token already on sale"
        );
        require(
            hasRole(DROP_OF_THE_DAY_CREATOR_ROLE, msg.sender),
            "drop of the day auctio: caller is not an drop of the day creator"
        );
        require(
            satoshiART1155.balanceOf(itemOwner, tokenId) >= 1,
            "drop of the day auctio: owner is trying to sell more than they have"
        );
        require(
            auctionCommission >= 0 && auctionCommission <= 3000,
            "drop of the day auctio: commission is too high"
        );
    ...
    function auctionEnd(uint256 tokenId, address itemOwner) public {
        ...
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "Auction end: caller is not default admin"
        );
        require(
            _listings[tokenId][itemOwner].status == 0x02,
    ...
    // function putOnHold(uint256 tokenId) external {
    //     require(
    //         _listings[tokenId][msg.sender].status != 0x00,
    //         "Put on hold: item is already on hold."
    //     );
    //     require(
    //         !_listings[tokenId][msg.sender].isDropOfTheDay,
    //         "Put on hold: cannot put drop of the day item on hold."
    //     );
    //     require(
    //         _listings[tokenId][msg.sender].highestBidder == address(0),
    //         "Put on hold: cannot put on hold as bid exists."
    //     );
    //     require(
    //         _listings[tokenId][msg.sender].startTime > block.timestamp ||
    //             _listings[tokenId][msg.sender].endTime < block.timestamp,
    //         "Put on hold: cannot put on hold during auction."
    //     );

    //     _listings[tokenId][msg.sender].status = 0x00;
    // }

    // function putDropOfTheDayItemOnHold(uint256 tokenId) external {
    //     require(
    //         hasRole(DROP_OF_THE_DAY_CREATOR_ROLE, msg.sender),
    //         "Put drop of the day item on hold: caller is not drop of the day creator."
    //     );
    //     require(
    //         _listings[tokenId][msg.sender].status != 0x00,
    //         "Put drop of the day item on hold: item is already on hold."
    //     );
    //     require(
    //         !_listings[tokenId][msg.sender].isDropOfTheDay,
    //         "Put drop of the day item on hold: cannot put drop of the day item on hold."
    //     );
    //     require(
    //         _listings[tokenId][msg.sender].highestBidder == address(0),
    //         "Put drop of the day item on hold: cannot put on hold as bid exists."
    //     );
    //     require(
    //         _listings[tokenId][msg.sender].startTime > block.timestamp ||
    //             _listings[tokenId][msg.sender].endTime < block.timestamp,
    //         "Put drop of the day item on hold: cannot put on hold during auction."
    //     );
        ...
        external
    {
        ...
        require(
            block.timestamp < _listings[tokenId][msg.sender].endTime,
