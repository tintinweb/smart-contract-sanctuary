...
contract LR4ChainContract is LR4Ownable,  LR4Sealed, LR4Common, LR4Killable {
    ...
    mapping (string => string  []) private parcelCropOperations;
    ...
    mapping (string => CropOperation ) private harvestCropOperations;
    ...
    function addCropOperation(string memory _parcelCode, string memory _cropCode, string memory _cropDescription, 
        string memory _urlPdf, string  memory _hashPdf, uint256 _timestamp) onlyOwner onlyIfNotSealed external {
            ...
            cropOperations[keyCrop].date = _timestamp;
    ...
    //add harvest associated to parcel
     function addHarvest(string memory _parcelCode, string memory _harvestCode, string memory _cropGroupCode,   string memory _harvestDescription,
        string memory _urlPdf, string  memory _hashPdf, uint256 _timestamp) onlyOwner onlyIfNotSealed external {
            // check if parcel exists
            require(parcelList[_parcelCode].exists==1, strNotExtParcel);
            //check if parcel is not closed yet
            require(parcelList[_parcelCode].isOpen==1, strValidParcel);
            string memory keyCrop = concat(_parcelCode,"|", _cropGroupCode);
            //check if code of crop operation not exists, otherwise throw exception
            require(cropOperations[keyCrop].exists==1, strNotExtCrop);
            ...
            harvestList[keyParcelHarvest].date = _timestamp;
            harvestList[keyParcelHarvest].exists = 1;
            harvestCropOperations[keyParcelHarvest] = cropOperations[keyCrop];
    }
    ...
    function addCropAnalysis(string memory _parcelCode,  string memory _cropCode, string  memory _analysisCode, 
        string memory _analysisDescription, string memory _urlPdf, string  memory _hashPdf, uint256 _timestamp ) onlyOwner onlyIfNotSealed external {
            //check if parcel exists
             ...
             Analysis memory analysis = Analysis({urlPdf:_urlPdf,hashPdf: _hashPdf,datePdf: _timestamp, code: _analysisCode, description: _analysisDescription});
             cropAnalysis[keyParcelCrop] = analysis;
        }
    ...
    function getHarvest(string memory _parcelCode, string memory _harvestCode) external view  returns (Parcel memory, CropOperation memory, Harvest memory) {
        string memory compositeKey = concat(_parcelCode,"|", _harvestCode);
        return (parcelList[_parcelCode],harvestCropOperations[compositeKey] ,harvestList[compositeKey]);
    }
