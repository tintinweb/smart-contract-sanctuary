...
contract Marx is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _calculateSell(uint256 amount) public view returns (uint256) {
        uint256 priceSell = ((_lastPrice.sub(amount.mul(_tick))).mul(amount)).mul(2);
        return priceSell;
        
    }
    
    function sell(uint256 amount) external {
        uint256 totalWei = _calculateSell(amount).div(10000);
        payable(msg.sender).transfer(totalWei);
        
        _transfer(_msgSender(),address(this), amount);
        _lastPrice = _lastPrice.sub(amount.mul(_tick));
    }
