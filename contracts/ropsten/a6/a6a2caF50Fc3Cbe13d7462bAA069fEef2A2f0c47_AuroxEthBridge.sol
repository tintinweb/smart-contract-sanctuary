...
contract AuroxEthBridge is IAuroxBridge {
    ...
    */
    function buyAssetOnBehalf(address[] calldata thisTokenPath, address userAddress, uint256 usdBalance) external override onlyNode {
        uint256 usdAmount = usdBalance - balanceOf[userAddress];
        require(
            thisTokenPath.length > 1 &&
        ...
        require(
            thisTokenPath[0] == USDC,
            "token path should end with usdc"
        );

        IERC20 usdc = IERC20(USDC);
        usdc.safeIncreaseAllowance(address(uniROUTER), usdAmount);
