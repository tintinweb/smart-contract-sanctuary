...
contract CosmoMasks is Ownable, CosmoMasksERC721, ICosmoMasks {
    ...
    string public constant PROVENANCE =
        "67c9815f30768bdaa4d8f016295ebe0339326587ab9a5dc423191c5aee4ce532";
    ...
    ) public CosmoMasksERC721("CosmoMasks", "COSMAS") {
    ...
    function getPrice() public view returns (uint256) {
        require(
            ...
            "CosmoMasks: sale has not started"
        );
        require(
            totalSupply() < MAX_SUPPLY,
            "CosmoMasks: sale has already ended"
        );
    ...
    function mint(uint256 numberOfMasks) public payable {
        ...
        require(
            numberOfMasks <= 20,
            "CosmoMasks: You may not buy more than 20 CosmoMasks at once"
        );
        require(
            totalSupply().add(numberOfMasks) <= MAX_SUPPLY,
            "CosmoMasks: Exceeds MAX_SUPPLY"
        );
        require(
            getPrice().mul(numberOfMasks) == msg.value,
            "CosmoMasks: Ether value sent is not correct"
        );
    ...
    function finalizeStartingIndex() public {
        require(
            startingIndex == 0,
            "CosmoMasks: starting index is already set"
        );
    ...
    function changeName(uint256 tokenId, string memory newName) public {
        ...
        require(
            _msgSender() == owner,
            "CosmoMasks: caller is not the token owner"
        );
        require(
            validateName(newName) == true,
            "CosmoMasks: not a valid new name"
        );
        require(
            sha256(bytes(newName)) != sha256(bytes(_tokenName[tokenId])),
            "CosmoMasks: new name is same as the current one"
        );
        require(
            isNameReserved(newName) == false,
            "CosmoMasks: name already reserved"
        );

        IERC20BurnTransfer(_cmpAddress).transferFrom(
            msg.sender,
            address(this),
            NAME_CHANGE_PRICE
        );
    ...
    function setProvenanceExtra(string memory provenanceExtra)
        public
        onlyOwner
    {
        require(
            provenanceExtraSetted == false,
            "CosmoMasks: provenanceExtra has already setted"
        );
    ...
    function setCosmoToken(address token) public onlyOwner {
        require(
            _cosmoToken == address(0),
            "CosmoMasks: CosmosToken has already setted"
        );
        require(
            token != address(0),
            "CosmoMasks: CosmoToken is the zero address"
        );
        _cosmoToken = token;
    }
