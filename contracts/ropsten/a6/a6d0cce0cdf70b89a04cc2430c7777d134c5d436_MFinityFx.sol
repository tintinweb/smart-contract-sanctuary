...
contract MFXToken is ERC20 {
    ...
    //          STATE VARIABLES
    // ------------------------------------------------------------------------
    string public name = "MFinityFx Token";
    string public symbol = "MFX";
    uint8 public decimals = 5;
    address public owner;
    uint public totalSupply = 500000000 * (10 ** 5);
    uint public currentSupply = 300000000 * (10 ** 5); // 60% of total supply
    ...
    // ------------------------------------------------------------------------
    constructor () public {
        owner = address(0xDA3F3AD64A228586F6B079f8e93515eaF3251d4a);
    }
...
contract MFinityFx is MFXToken {
    using SafeMath for uint;
    // ------------------------------------------------------------------------
    //          STATE VARIABLES  00000000
    // ------------------------------------------------------------------------
    // 60% of totail coins will be sold in ico
    uint public tokenSaleAllocation = 300000000 * (10 ** 5);
    // 2% of total supply goes for bounty 
    uint public bountyAllocation = 10000000 * (10 ** 5); 
    //13% of total tokens is reserved for founders and team
    uint public founderAllocation =  65000000 * (10 ** 5); 
    //5% of total tokens is reserved for partners
    uint public partnersAllocation = 25000000 * (10 ** 5); 
    // 10% of total tokens is for Liquidity reserve
    uint public liquidityReserveAllocation = 50000000 * (10 ** 5); 
    //5% of total tokens is reserved for advisors
    uint public advisoryAllocation = 25000000 * (10 ** 5); 
    //5% of total tokens in reserved for pre-seed Inverstors
    uint public preSeedInvestersAllocation = 25000000 * (10 ** 5); 
    ...
    constructor () public {
        ...
        rateDeno = 100000000;
        softCap = 5000  ether;
        hardCap = 30005019135500000000  wei;
        tokenSaleStart = now;
        tokenSaleEnds = now;
        balances[this] = currentSupply;
        isIcoSuccessful = true;
        icoSuccessfulTime = now;
        beneficiary = address(0xDA3F3AD64A228586F6B079f8e93515eaF3251d4a);
    ...
    function calculateDiscount() internal view returns(uint) {
        if (totalCoinsSold < 12500000000000) {
            return 115;   // 15 % discount
        } else if (totalCoinsSold < 18750000000000) {
            return 110;   // 10 % discount
        } else if (totalCoinsSold < 25000000000000) {
