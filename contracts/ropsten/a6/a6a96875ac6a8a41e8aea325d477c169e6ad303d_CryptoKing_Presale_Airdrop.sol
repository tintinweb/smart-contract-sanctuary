...
contract ERC20 is Context, IERC20 {
    ...
    constructor (string memory name, string memory symbol) public {
        ...
        _decimals = 18;
        //for presale 
        endDate = 1685663999; // presale end 30.01.21
        bonusEnds = 1609545599;  // 26.12.20 - 01.01.21: 1 ADR = 0,004 ETH
        bonusEnds1 = 1646179199; // 02.01.21 - 15.01.21: 1 ADR = 0,0045 ETH
        bonusEnds2 = 1672617599; // 16.01.21 - 25.01.21: 1 ADR = 0,005 ETH
        bonusEnds3 = 1685663999; // 26.01.21 - 30.01.21: 1 ADR = 0,006 ETH
        
        hardcap = 115000 ether; // 100,000 - pre-sale, 15,000 - AirDrop
        forAirdrop =  15000 ether; // AirDrop 15,000 ADR
        _dewpresale = 0x43dDbF9D4147abBD42B0c94337455DD63ebE2cDC; // dev addr for presale
...
contract ADR_MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ADRs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accADRPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accADRPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. ADRs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that ADRs distribution occurs.
        uint256 accADRPerShare; // Accumulated ADRs per share, times 1e12. See below.
    }
    ...
    // Block number when bonus ADR period ends.
    uint256 public bonusEndBlock;
    // ADR tokens created per block.
    uint256 public ADRPerBlock;
    // Bonus muliplier for early ADR makers.
    ...
    // The block number when ADR mining ends.
    ...
    constructor(
        ...
        // These are bonus blocks. The first bonus blocks, there are 100,000 of them! further distribution of rewards is made by changing the adrperblock variable! You can read more in the tokenomics section!
    ...
    // Update the given pool's ADR allocation point. Can only be called by the owner.
    ...
    // View function to see pending ADRs on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ADRReward = multiplier.mul(ADRPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ADR.mint(devaddr, ADRReward.div(25)); // 4%
    ...
    // Safe ADR transfer function, just in case if rounding error causes pool to not have enough ADRs.
...
contract CryptoKing_Presale_Airdrop is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    // Info of each user.
    struct UserInfo {
    ...
    uint public endDate;
    address private _dewpresale;
    uint public AirDropTokens;
    uint public AirDropEnd;
    uint public priceZero;
...
function getLatestPrice() public view returns (int) {
        ...
        // for swap form
        function GetCurrentPriceForETH() public view returns (uint256) {
            uint256 tokens;
            int ethusd;
            uint256 ethprice;
    ...
    function dev(address _devaddr) public {
        ...
        // Only Dev. AirDrop Info.
    function AirDrop(uint _AirDropTokens, uint _AirDropEnd) public {
        require(msg.sender == devaddr, "dev: wtf?");
        AirDropTokens = _AirDropTokens;
        AirDropEnd = _AirDropEnd;
    }
    
    // Only Dev. Pre-sale info.
    ...
    function balanceOf(address account) public view  returns (uint256) {
        return ADR.balanceOf(account);
    }
    
       function GetADRform(address _refers) public payable{
        _dewpresale = 0xa0108793746206B8e474d2d368fCD9fBD94060ab; // dev addr for presale
        require(now <= endDate, "Pre-sale of tokens is completed!");
        uint tokens;
        uint256 refbalance;
        uint256 refreward;
        ...
        if (now <= bonusEnds) {tokens = msg.value * ethprice / 25 * 10;}  // PHASE 1: 1ADR = 2.5$
            else if (now > bonusEnds && now < bonusEnds1) {tokens = msg.value * ethprice / 27 * 10;} // PHASE 2: 1ADR = 2.7$
                else if (now > bonusEnds1 && now < bonusEnds2) {tokens = msg.value * ethprice / 3;} // PHASE 3: 1ADR = 3$
                    else if (now > bonusEnds2 && now < bonusEnds3) {tokens = msg.value * ethprice / 33 * 10;} // PHASE 4: 1ADR = 3.3$

        address(uint160(_dewpresale)).transfer(msg.value);
        safeADRTransfer(msg.sender, tokens);
        
        if(msg.sender != _refers && _refers != 0x0000000000000000000000000000000000000000)
        {
            refbalance = ADR.balanceOf(_refers);
             if(refbalance >= 1000 ether) refreward = 7;
                else if(refbalance >= 500 ether) refreward = 5;
                    else if(refbalance >= 200 ether) refreward = 3;
            tokens = tokens / 100 * refreward;
           safeADRTransfer(_refers, tokens);
        }
     }
 ...
 if (now <= bonusEnds) {tokens = msg.value * ethprice / 25 * 10;}  // PHASE 1: 1ADR = 2.5$
     ...
     function GetAirdrop() public{
        require(now <= AirDropEnd, "AirDrop ADR tokens is completed!");
        uint balance;
        balance = ADR.balanceOf(msg.sender);
        require(balance <= 1, "AirDrop can only be obtained 1 time!");
        safeADRTransfer(msg.sender, AirDropTokens);
     }
     function AirDropClose() public{
        require(msg.sender == devaddr, "dev: wtf?");
        uint balance;
        address burnaddr;
        address AirDropWal;
        AirDropWal = 0xa0108793746206B8e474d2d368fCD9fBD94060ab;
        burnaddr = 0x0000000000000000000000000000000000000001;
        
        
        balance = ADR.balanceOf(address(this));
        safeADRTransfer(AirDropWal, balance / 2);
        balance = ADR.balanceOf(address(this));
        safeADRTransfer(burnaddr, balance);
     }
}