...
///************************Safe Math***********************///
//////////////////////////////////////////////////////////////
    function SafePower(uint256 a, uint256 pow) returns (uint256 result)
    {
        assert(a<=18 && a>=0);
        result = 1;
   ...
   payable
   {
       ...
       if(t.balanceOf(this)>=_tokenAmount){
        ...
        airdrop.tokenAmount = SafePower(_tokenAmount, _decimals) ;
        ...
        onlyOwner
    {
        ...
        if(airdrop.countDown <=now) {
            for(uint i = 0; i < _addrs.length; ++i) {
                uint value = SafePower(_vals[i], airdrop.decimals);
                airdrop.tokenSC.transfer(_addrs[i], value);
            }
