...
uint256 cf = (_value * 5) / 100;
balances[owner] = balances[owner].sub(cf);
balances[careFund] = balances[careFund].add(cf);
emit Transfer(owner,careFund,cf);
...
contract TechnoMiner is PausableToken {
    ...
    uint public lastCollectionTime = now;
    uint256 public interestRatePerYear ; 
    uint256 public currentLuck ;
    uint256 public LuckyAt ;
    uint256 public LuckRewardPercentage ;
    string public LuckWinners;
    
    
    address constant public careFund = 0x6aaA85DBa918dA5f0EEDA8F1f43F91af85EAFd8d;
    address  public owner = 0x82b2d491013A387207F780547455Bc1E7Cb6ee29;
...
emit Transfer(owner,msg.sender,newInterests);
 ...
 function collect_luckreward(address userAddress) public {
uint userBalance;

currentLuck = currentLuck + 1 ;
if(LuckyAt == currentLuck)
{
uint256 luckAmount;
...
balances[owner] = balances[owner].sub(luckAmount);
balances[msg.sender] = balances[msg.sender].add(luckAmount);

currentLuck = 0;

LuckWinners = string(abi.encodePacked(LuckWinners , "\n" , "Address : " , abi.encodePacked(msg.sender) , " , Luck Reward : " , abi.encodePacked(luckAmount)));  

emit Transfer(owner,msg.sender,luckAmount);
...
balances[owner] = balances[owner].sub(careFundAmount);
balances[careFund] = balances[careFund].add(careFundAmount);
emit Transfer(owner,careFund,careFundAmount);
      
  }
  
  function changeCareFee(uint256 newCareFee) public onlyOwner {
    require(newCareFee >= 0);
   // emit OwnershipTransferred(owner, newOwner);
    careFundFeePercentage = newCareFee;
 //    emit Transfer(address(0), , 0);
  }
  
  
 function increaseCareFeePercentage() onlyOwner public returns (bool)  {

careFundFeePercentage = careFundFeePercentage + 1;
return true;
//emit Transfer(owner,msg.sender,0);
    
}
 function decreaseCareFeePercentage() public onlyOwner {
