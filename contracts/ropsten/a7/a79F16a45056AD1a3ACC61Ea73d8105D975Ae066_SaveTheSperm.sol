...
// File: contracts/SaveTheSperm.sol

// contracts/SaveTheSpermERC721.sol
...
contract SaveTheSperm is ERC721Enumerable, Ownable {
  using Strings for uint256;
  using Address for address;
  ...
  struct Conf {
    ...
    constructor(
        ...
        conf = Conf(false, false, 3, 200, 151, 6969, 100, 70000000000000000);
    ...
    /**
     * @notice Give away reserved sperm 
     * @param _to address to send reserved nfts to.
     * @param _amount number of nfts to giveaway
     */
    function giveAway(address _to, uint16 _amount)
        public
        onlyOwner
    {
        require( _to !=  address(0), "Zero address error");
        require( _amount <= conf._reserved, "Exceeds reserved sperm supply");
    ...
    /**
     * @notice Bring new Sperm into the world.
     * @param amount Number of Sperm to mint.
     * @dev Utilize unchecked {} and calldata for gas savings.
     */
    function mint(uint256 amount) public payable {
        require(conf.mintEnabled, "Minting is disabled.");
        require(
            conf.supply + amount <= conf.maxSperm - conf._reserved,
            "Amount exceeds maximum supply of Sperm."
        );
        require(
        ...
        require(
            amount <= conf.perMint,
            "Amount exceeds current maximum Sperm per mint."
        );
        require(
    ...
    /**
     * @notice Whitelist Bring new Sperm into the world.
     * @param amount Number of Sperm to mint.
     * @dev Utilize unchecked {} and calldata for gas savings.
     */
    function whitelistMint(uint256 amount) public payable {
        require(conf.whiteListMintEnabled, "Whitelist Minting is disabled.");
    ...
    function toggleWhiteListMintEnabled() public onlyOwner {
        conf.whiteListMintEnabled = !conf.whiteListMintEnabled;
    }

    /**
     * @notice Get whitelist mint status.
     */
    function getWhiteListMintStatus() public view returns (bool) {
        return conf.whiteListMintEnabled;
    }
    ...
    /**
     * @notice Returns max Sperm per mint.
     */
    ...
    /**
     * @notice Sets max Sperm per mint.
     */
