...
contract RastaNFT {
    ...
    mapping(uint256 => NFTInfo) public _NFTs;
    mapping(address => uint256[]) public _ownedNFTs;
    mapping(uint256 => uint256) public _ownedNFTIndex;
    ...
    modifier updateReward(uint256 tokenId) {
        ...
        if(totalSupply > 0) {
            ...
            uint256 pending = nft.price.mul(accPerShare).mul(rewardRate).div(1e12*(10**decimalsDiff)).div(totalSupply) ;
    ...
    function buyNFT(uint256 tokenId, uint256 amount) external {
        ...
        require(amount == nft.price, "Price is not match");
    ...
    function sellNFT(uint256 tokenId) external {
        ...
        claim(tokenId); // claim nft reward
        ...
        if (feeAmount > 0) stakingToken.safeTransfer(_owner, feeAmount);
    ...
    function claimable(uint256 tokenId) external view returns (uint256) {
        ...
        if(totalSupply > 0) {
            uint256 accPerShare = block.timestamp.sub(nft.lastUpdateTime);
            uint256 pending = nft.price.mul(accPerShare).mul(rewardRate).div(1e12*(10**decimalsDiff)).div(totalSupply) ;
            return nft.pendingRewards.add(pending);
        } else {
    ...
    function _safeTransferRewards(address to, uint256 amount) internal returns (uint256) {
        ...
        if (feeAmount > 0) rewardToken.safeTransfer(_owner, feeAmount);
