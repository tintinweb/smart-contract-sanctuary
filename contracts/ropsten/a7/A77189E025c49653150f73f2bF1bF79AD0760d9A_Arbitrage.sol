...
contract Arbitrage is Ownable {
    ...
    function uniToBehodler(uint256 expectETH, uint deadline, uint256 passes) public payable onlyOwner {
        ...
        // transfer fee
        (uint256 transferFee, uint256 burnFee,) = behodler.getConfiguration();
        uint256 feeComponent = transferFee.mul(scxAmount).div(1000);
        uint256 burnComponent = burnFee.mul(scxAmount).div(1000);
        uint256 scxAmountAfterBurn = scxAmount.sub(feeComponent.add(burnComponent));

        uint256 ethInPool = IERC20(behodler.Weth()).balanceOf(_behodler);
        uint256 outputETHAmount = behodler.withdrawLiquidityFindSCX(behodler.Weth(), ethInPool.div(2), scxAmountAfterBurn, passes);
        require(outputETHAmount >= 0, "Arbitrage: Lost");
        emit Log(scxAmountAfterBurn, ethInPool, outputETHAmount);
        // has assert in this function
