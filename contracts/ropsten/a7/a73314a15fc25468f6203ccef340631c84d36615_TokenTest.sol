...
contract TokenTest is Context, IERC20, Ownable {
    ...
    string private constant _name = "Token Test";
    string private constant _symbol = "TTEST";
  ...
  //  uint256 private gasPriceLimit = 750 * 1 gwei; // do not allow over 750 gwei for launch
   ...
   /* function setGasPriceLimit(uint256 gas) external onlyOwner {
    ...
    function setgasPriceBuyers(uint256 gasBuyer) public onlyOwner{
        require(gasBuyer >= 0);
        gasPriceBuyers = gasBuyer * 1 gwei;
    }

        function setgasPriceSellers(uint256 gasSeller) public onlyOwner{
        require(gasSeller >= 0);
        ...
        if(limitsInEffect){
            if (
                ...
                if (gasLimitActive && automatedMarketMakerPairs[from]) {
                    require(tx.gasprice <= gasPriceBuyers, "Gas price exceeds limit."); //New Gas Price buyer
                }
