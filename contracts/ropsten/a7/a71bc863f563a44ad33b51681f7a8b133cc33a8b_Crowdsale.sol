...
mapping (uint256 => tokenInfo) public tokenBook;
  ...
  bool public Paused;
    ...
    */
    ...
    function endPublicSale() public onlyOwner returns(uint256){
    ...
    bytes memory data  = abi.encodeWithSignature(("burn(uint256)"), remainingTokens);
    (bool success, bytes memory returnedData) = tokenContractAddress.delegatecall(data);
    require(!success, "tokens not burned");
    // token.burn(remainingTokens);
    crowdSaleStarted = false;
    emit Test(returnedData);
