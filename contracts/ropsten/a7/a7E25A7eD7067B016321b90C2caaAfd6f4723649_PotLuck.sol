...
contract PotLuck {
    ...
    function random() private view returns(uint){
         return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, addresses)));
    }
    function getPlayersCountInPot() public view returns(uint256)  {
        return players.length;
    }
    function getWinners() public returns (uint, uint, uint256, uint256) {
        uint winner1Index = random() % players.length;
        uint winner2Index = random() % players.length;
    ...
    function pickWinner() public returns (address, address, uint256) {
        require(players.length >= 2, "Should be two and more people in the pot");
        uint winner1Index = random() % players.length;
        uint winner2Index = random() % players.length;
        address payable winner1Addr = payable(players[winner1Index].player);
        address payable winner2Addr = payable(players[winner2Index].player);
        ...
        uint256 winner1Amount = totalPotAmount.mul(winnerPercent).div(100).mul(players[winner1Index].amount).div(players[winner1Index].amount.add(players[winner2Index].amount));
        uint256 winner2Amount = totalPotAmount.mul(winnerPercent).div(100).mul(players[winner2Index].amount).div(players[winner1Index].amount.add(players[winner2Index].amount));
        // (bool success, ) = winner1Addr.call{value: winner1Amount}("");
        // (success, ) = winner2Addr.call{value: winner2Amount}("");
        ...
        return (players[winner1Index].player, players[winner2Index].player, totalPotAmount);
    }
