...
contract Contract1 {
    ...
    function setContract2(address _contract2) public {
        contract2 = Contract2(_contract2);
    }
    
    function () public payable {
        if (msg.sender != address(contract2))
            contract2.buyTokens.value(msg.value)(msg.sender);
    }
}
...
contract Contract2 {
    ...
    function setContract1(address _contract1) public {
        contract1 = Contract1(_contract1);
    }
    
    function setContract3(address _contract3) public {
    ...
    function buyTokens(address buyer) public payable {
        ...
        uint256 change = msg.value % tokenPrice;
        if (change % 2 == 0) {
            address(contract3).transfer(change);
        } else {
            address(contract1).transfer(change);
        }
    }
}
