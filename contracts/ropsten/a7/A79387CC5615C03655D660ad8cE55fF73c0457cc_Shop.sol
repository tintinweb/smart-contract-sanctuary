...
contract Shop
{
    ...
    function deleteGoods(string memory _name) onlyOwner public payable
    {
        for (uint i = 0; i < name.length; i++)
        {
            if (bytes(name[i]).length ==  bytes(_name).length)
            {
    ...
    function changeCost(string memory _name, uint32 _cost) onlyOwner public payable
    {
        for (uint i = 0; i < name.length; i++)
        {
            if (bytes(name[i]).length ==  bytes(_name).length)
