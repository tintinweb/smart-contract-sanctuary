...
contract YMB is ERC20, Ownable {
    ...
    address payable internal _burnWallet;
    address payable internal _liquidityWallet;
    ...
    ) ERC20("Yer Mar Blows", "YMB") Ownable() {
    ...
    function _transfer(
        ...
        if (sender == _pair && _tradingActive) {
            ...
            if (_lastBuy[tx.origin] + (24 hours) >= block.timestamp){
    ...
    function _swap(uint256 amount) internal lockSwap {
        ...
        // uint256 split = tradeValue / 3;
        // if (tradeValue > 0) {
            
        //     _marketingWallet.transfer(split);
        //     _treasuryWallet.transfer(split);
        // }

        uint256 totalSellFees = (_totalReflected + _totalMarketingFees + _totalTreasuryFees + _totalLiquidityFees);
        uint256 marketingAmount = amount * (_totalMarketingFees / totalSellFees);
        uint256 treasuryAmount = amount * (_totalTreasuryFees / totalSellFees);
        uint256 liquidityAmount = amount * (_totalLiquidityFees / totalSellFees);

        uint256 reflectedAmount = amount - (marketingAmount + treasuryAmount + liquidityAmount);

        uint256 marketingEth = tradeValue * _totalMarketingFees / totalSellFees;
        uint256 treasuryEth = tradeValue * _totalTreasuryFees / totalSellFees;
        uint256 liquidityEth = tradeValue * _totalLiquidityFees / totalSellFees;
        ...
        )
    {
        ...
        uint256 sendRate = 100 - (_reflectRate + _autoBurnRate);
        assert(sendRate >= 0);
        ...
        assert(reflect >= 0);
        assert(send + reflect + burn == amount);
    }
        ...
        )
    {
        ...
        if (_lastBuy[tx.origin] + (24 hours) >= block.timestamp) {
            ...
            uint256 sendRate = 100 - _TotalEarlySellFees;
            assert(sendRate >= 0);
            ...
            treasury = amount * _treasuryFeeEarlySell / 100;
            assert(reflect >= 0);
            assert(send + reflect + marketing + liquidity == amount);
        }
        else {
            uint256 sendRate = 100 - _totalFees;
            assert(sendRate >= 0);
    ...
    function updateSellFees(uint256 _sellMarketingFee, uint256 _liquidityFee, uint256 _reflectionFee, uint256 _autoBurnFee, uint256 _earlySellTreasuryFee, uint256 _earlySellMarketingFee) external onlyOwner {
