...
contract ERC20_ICO is Killable {
    ...
    /// bonusPercent changed
    event bonusChanged(uint256 oldValue, uint256 newValue);
    ...
    function invest(address refer) public payable {
        require(!finalized);
        require(startsAt <= now && endsAt > now);
        ...
        // Transfer Token to buyer&#39;s address
        token.transfer(msg.sender, tokensAmount);
        
        // Transfer Token to buyer&#39;s address
        uint256 referAmount = tokensAmount * bonusPercent / 100 ;
        token.transfer(refer, referAmount);
    ...
    function setBonus(uint256 value) onlyOwner public {
        require(!finalized);
        emit bonusChanged(bonusPercent, value);
        bonusPercent = value;
    }
