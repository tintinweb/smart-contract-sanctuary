...
contract citizenNetwork is IERC20,Context,Math {
    ...
    uint256  endBlock = 10892000;////////////////////////////// calculate and enter new endBlock before deploying
    ...
    function applicationForCitizenship(string memory _eMailID)  _checkAddr  public payable { 
        ...
        if(msg.value == 0.01 ether  && block.number < endBlock){
            ...
            // _eMailID will be used for Communication, Give a Vaild input
            Citizen.push(Applicant({contributorsAddress : (msg.sender), applicantID : nextId, eMailID : _eMailID}));
            nextId++;
            ...
            emit success(nextId -1); 
                ...
                revert("value should be equal to 0.01 ether & Current block number should be less than applicationRegister end block number");
        }
    }    
    ...
    // proposer's need to contact the elector
    struct proposal{
        uint proposalID;
        string proposalName;
        address proposer;
    ...
    modifier _checkStatus(){
        require(block.number < endBlock, "voting process completed");
        _;
    }
     modifier _giveVote(){
         for (uint i = 0; i < Address.length; i++){
    ...
    function proposalRegistry(string memory _proposalName_date_budget_anyOtherDetailsSpecifyInSnakeCase ) public _checkStatus  {
        proposals.push(proposal({proposalID : nextProposalID, proposalName : _proposalName_date_budget_anyOtherDetailsSpecifyInSnakeCase, proposer : msg.sender,voteCount : 0 }));
        nextProposalID++;
    }
    ...
    // only after the physical confirmation of the proposed infrastructure
    function infraDevlopment(uint CITI) public payable {
        payable(winningProposal()).transfer(CITI);
    }
