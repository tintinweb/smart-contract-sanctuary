...
contract MainMappedEx is MainMappedBase {
    ...
    function updateReward_(address account) virtual public {
        rewardPerTokenStored = rewardPerToken();
...
contract MainMapped is MainMappedBase, Proxy {
    ...
    function withdrawSend(uint256 amount, uint toChainId, address to) virtual public payable nonReentrant updateReward(msg.sender) returns (uint nonce) {
        require(amount > 0, "Cannot withdraw 0");
        factorDelay[toChainId][to][nonce] = calcFactorDelay(msg.sender, amount);
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        //stakingToken.safeTransfer(msg.sender, amount);
        emit Withdrawn(msg.sender, amount);
    ...
    function calcFactorDelay(address from, uint volume) public view returns (uint) {
    ...
    function calcDelayReward(uint vol, uint fee, uint factor) virtual public view returns (uint reward) {
        if(factor == 0 || msg.sender != tx.origin || msg.sender.isContract())
            return 0;
        uint bal = IERC20(token).balanceOf(address(this)).sub(vol >> 1);
        uint supply = Math.max(bal, _totalSupply.add(1));
        uint maxFactor = Factory(factory).getConfig(_maxFactor_);
        factor = Math.max(1e18, factor.mul(bal).div(supply));
        factor = Math.min(maxFactor, factor.sub(1e18));
        reward = fee.mul(factor).div(1e18);
    ...
    function _settleDelayReward(address to, uint vol, uint fee, uint factor) virtual internal returns (uint reward) {
    ...
    function calcTxReward(address to) virtual public view returns (uint) {
        this;
        to;
        return 0;
    }
    function _settleTxReward(address to) virtual internal returns (uint reward) {
    ...
    function updateReward_(address account) virtual public {
        (bool success, bytes memory data) = address(_implementation()).delegatecall(abi.encodeWithSelector(this.updateReward_.selector, account));
        assembly {
            switch success
                case 0  { revert(add(data, 32), returndatasize()) }        // delegatecall returns 0 on error.
                default { return(add(data, 32), returndatasize()) }
...
contract Factory is ContextUpgradeSafe, Configurable, Constants {
    ...
    function setPath(address swapFactory, address[] calldata pathDelayReward, address[] calldata pathTVL, address[] calldata pathAPY) virtual external governance {
