...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 public maxSellPerDay = 11000000000000;
    uint256 internal minTokensToSwap = 50000 * 10**9;
    ...
    feeRatesStruct public sellFees =
        feeRatesStruct({
            taxFee: 0 ,
            burnFee: 5000,
            airdropFee: 2000,
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        antiwhale_measures[0] = antiwhale({
            selling_threshold: (110000000000 * 25),
            extra_tax: 1
        }); //0.025% of initial supply
        antiwhale_measures[1] = antiwhale({
            selling_threshold: (110000000000 * 50),
            extra_tax: 2
        }); //0.05% of initial supply
        antiwhale_measures[2] = antiwhale({
            selling_threshold: (110000000000 * 75),
            extra_tax: 3
        }); //0.075% of initial supply
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[sender] && recipient == pancakePair) {
            if (block.timestamp < _firstSell[sender] + 24 * 1 hours) {
                require(
                    _totSells[sender] + amount <= maxSellPerDay,
                    "You can't sell more than maxSellPerDay"
                );
                _totSells[sender] += amount;
            } else {
                require(
                    amount <= maxSellPerDay,
                    "You can't sell more than maxSellPerDay"
                );
    ...
    function _transfeTokens(
        ...
        unchecked {
            _balances[sender] = _balances[sender] - s.tTransferAmount;
        }
        ...
        returns (valuesFromGetValues memory to_return)
    {
        ...
        to_return.tFee =
            (_amount * appliedFees.totFees * appliedFees.taxFee) / 1000000;
        to_return.tSwap =
            (_amount * appliedFees.totFees * appliedFees.swapFee) / 1000000;
    ...
    function getAntiwhaleFee(uint256 amount)
        internal
        view
        returns (uint256 sell_tax)
    {
        if (amount < antiwhale_measures[0].selling_threshold) {
            ...
            sell_tax = antiwhale_measures[2].extra_tax;
        }
...
contract DoxyFinance_V3 is ERC20, Ownable { 
    ...
    constructor() ERC20("Doxy Finance", "DOXY_V3") {
        _mint(liquidityWallet, 5500000000000000);
        _mint(privateSaleWallet, 359700000000000);
        ...
        _mint(burnWallet, 502700000000000);
    ...
    function setSellFees(
        ...
        sellFees.swapFee =
            ...
            sellLiquidityFee;
        require(
            sellFees.swapFee + sellFees.taxFee == 10000,
            "sum of all percentages should be 10000"
        );
    }
    ...
    function setAntiwhaleMeasure(
        uint256[3] memory selling_thresholds,
        uint256[3] memory extra_taxes
    ) external onlyOwner {
        //values of selling_threshold (are values in input)/1000 % of total supply , extra taxes are expressed in %
        antiwhale_measures[0] = antiwhale({
            selling_threshold: (110000000000 * selling_thresholds[0]),
            extra_tax: extra_taxes[0]
        });
        antiwhale_measures[1] = antiwhale({
            selling_threshold: (110000000000 * selling_thresholds[1]),
            extra_tax: extra_taxes[1]
        });
        antiwhale_measures[2] = antiwhale({
            selling_threshold: (110000000000 * selling_thresholds[2]),
            extra_tax: extra_taxes[2]
        });
    } 
