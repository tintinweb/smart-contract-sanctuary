...
contract TestUniswapFlashSwap is IUniswapV2Callee {
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external override {
        ...
        // do stuff here
        IERC20(tokenBorrow).approve(ROUTER, amountBorrow);
        uint amountReceived = IUniswapV2Router(ROUTER).swapExactTokensForTokens(amountBorrow, 1, path, address(this), block.timestamp)[1];
