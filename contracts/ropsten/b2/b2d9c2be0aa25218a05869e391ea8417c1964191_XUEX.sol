...
contract XUEX is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "XUEX";
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "XUE: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'XUE: The contract it self cannot be excluded');
        require(!_isExcluded[account], "XUE: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "XUE: Account is already included");
