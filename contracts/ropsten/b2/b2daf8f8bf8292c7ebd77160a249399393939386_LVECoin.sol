...
contract Ownable {
    ...
    // TODO: 2018/10/11 - 服務合約用
    event Trust(address _from);
    event Distrust(address _from);
    ...
    modifier onlyServices() {
        require(services[msg.sender] == true, "");
        _;
    }
    function trust(address _newService) public isOwner {
        require(_newService != address(0), "");
        require(services[_newService] == false, "");
        services[_newService] = true;
        emit Trust(_newService);
    }
    function distrust(address _service) public isOwner {
        require(_service != address(0), "");
        require(services[_service] == true, "");
        services[_service] = false;
        emit Distrust(_service);
    }
...
contract LVECoin is ERC20Token, Pausable {
    ...
    // TODO: 2018/10/11 - 服務合約用
    function pay(address _from, uint256 _tokenAmt) public onlyServices returns(bool){
        require(_from != address(0), "");
        require(balances[_from] >= _tokenAmt, "");

        balances[_from] = balances[_from].sub(_tokenAmt);
        balances[msg.sender] = balances[msg.sender].add(_tokenAmt);
        emit Transfer(_from, msg.sender, _tokenAmt);
        emit Pay(_from, msg.sender, _tokenAmt);
        return true;
    }


    // TODO: 加上approveAndCall => event ApproveAndCall 未加
