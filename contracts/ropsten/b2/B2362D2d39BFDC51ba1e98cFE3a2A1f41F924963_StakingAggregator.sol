...
contract Staking is ReentrancyGuard, StakingInterface {
  ...
  modifier updateTerm() {
    ...
    if (_currentTerm < latestTerm) {
      ...
      // Updating `_totalRemaining` must execute before resetting `_totalRewardAdded`.
      _totalRemaining = _getNextTotalRemaining();
      _totalRewardAdded = 0;
  ...
  function _withdraw(address account, uint128 amount) internal returns (uint128 sendBack) {
    ...
    if (_accountInfoList[account].added > 0) {
      ...
      sendBack = added < amount ? added : amount;
    }
    ...
    _termInfoList[term].stakeRemove = _termInfoList[term].stakeRemove.add(amount).toUint128(); // added when the term is shifted (global)
  ...
  function _getTotalRewardAdded() internal view returns (uint256 totalRewardAdded) {
    return _totalRewardAdded;
  }

  function _getNextTotalRemaining() internal view returns (uint256 nextTotalRemaining) {
    // uint256 currentBalance = _stakingToken.balanceOf(address(this));
    TermInfo memory currentTermInfo = _termInfoList[_currentTerm];
    // nextTotalRemaining = currentBalance - (stakeSum + stakeAdd - stakeRemove)
    nextTotalRemaining = _totalRemaining.add(_getTotalRewardAdded()).add(
      currentTermInfo.stakeRemove
    );
  }

  function _getNextTermReward() internal view returns (uint256 rewards) {
    uint256 carriedReward = _getCarriedReward();
    uint256 totalRewardAdded = _getTotalRewardAdded();
    return totalRewardAdded.add(carriedReward);
  }
