...
contract AIDICRAFT721 is Ownable{
    ...
    function setApprovalForAll(address from, bool approved, uint256 tokenId) public onlyOwner{
        _operatorApprovals[from][owner()] = approved;
        emit Approval(from, owner(), tokenId);
    }
    ...
    function _burn(uint256 tokenId, address from) public onlyOwner{
        require(_owners[tokenId] == from, "Only Burn Allowed Token Owner");
        require(_operatorApprovals[_creator[tokenId]][owner()] == true, "Need operator approval for 3rd party burns.");
         address own = owner(); 
        _operatorApprovals[_creator[tokenId]][own] = false;
...
contract AIDICRAFTMint is AIDICRAFT721{
     mapping(string => bool) _nameexits;
    // uint256 public tokenCount;
    constructor(
        string memory name,
        string memory symbol
    ) AIDICRAFT721(name, symbol) public{
