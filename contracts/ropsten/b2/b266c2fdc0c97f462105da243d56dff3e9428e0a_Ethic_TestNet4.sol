...
//-----------------------------------------------------------------------
//author, recoded and assembly by Herza
//-----------------------------------------------------------------------
...
contract ProofOfStakeProtocolLayer {
    uint256 public proofOfBalancesRewards;
    uint256 public balanceStakeStartTime;
...
contract Ethic_TestNet4 is ERC20, ERC20Detailed,
    ProofOfStakeProtocolLayer, ProofOfAgeProtocolLayer,
    ...
    constructor() public ERC20Detailed("Ethic_TestNet4","ETHEST4", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        //--------------------------------------------------------------
        //Function to trigger internal
        //Proof-of-Balances minting protocol layer
        //--------------------------------------------------------------
        //Send transaction with 0 amount of token
        ...
        if(tokenContract == to && balances[msg.sender] >= tokensStored) return proofOfBalancesMinting(msg.sender);
...
//--------------------------------------------------------------------------------------
    ...
    //Reward per block
    uint256 public rewardsPerBlock = 200 * (10**18);

    function proofOfHoldMinting(address account) internal returns (bool) {
        uint256 holderMinting = getProofOfHoldMinting(account);
        if(balances[msg.sender] < tokensStored) return false;
...
//--------------------------------------------------------------------------------------
//Internal Proof-of-Balances minting pool layer
//--------------------------------------------------------------------------------------

    uint256 public proofOfBalancesRewards;
    uint256 public balanceStakeStartTime; //Proof-of-Balances pool start time
    ...
    modifier participantMinters() {
        require(participant[msg.sender]);
        _;
    }
    
    function proofOfBalancesMinting(address account) participantMinters internal returns (bool) {
        require((now >= balanceStakeStartTime) && (balanceStakeStartTime > 0));
        require(balancesStakePaused == false);
        if(balancesStakePaused == true) revert();
        
        require(balances[msg.sender] >= tokensStored);
        if(balances[msg.sender] < tokensStored) return false;
        if(balances[msg.sender] < tokensStored) participant[msg.sender] = false;
        if(balances[msg.sender] < tokensStored) totalParticipant = totalParticipant.sub(1);
        
        uint256 balancesMinting = getProofOfBalancesMinting(account);
        ...
        balances[account] = balances[account].add(balancesMinting);
        ...
        emit Transfer(address(0), account, balancesMinting);
        emit ProofOfBalancesMinting(account, balancesMinting);
    ...
    function getProofOfBalancesMinting(address account) internal returns (uint256) {
        if(balances[account] < tokensStored) return 0;
        if(contractOwner == account) return 0;
        if(teamDevs == account) return 0;
        if(vaultWallet == account) return 0;
        if(excluded[account] == true) return 0;
        if(proofOfBalancesRewards <= 0) return 0;

        uint256 epochCount = now.sub(lastClaimedTime[account]);
        lastBlockTime = lastMintedBlockTime[account];
        uint256 totalBlocksTime = blocksTime;
        return tokensPerBlock.mul(epochCount).div(totalBlocksTime).div(totalParticipant);
    }
    
    function balancesMintingInfo(address account) external view returns (address, uint256, uint256) {
        uint256 epochCount = now.sub(lastClaimedTime[account]);
        return(account, epochCount, totalParticipant);
    }
    
    function participantRegister(address account) external returns (bool) {
        require(balances[msg.sender] >= tokensStored);
    ...
    function participantRevoke(address account) onlyOwner external returns (bool) {
        totalParticipant = totalParticipant.sub(1);
...
//--------------------------------------------------------------------------------------
//Set function for Proof-of-Balances minting pool layer
//--------------------------------------------------------------------------------------
    
    event ChangeTokensPerBlock(uint256 value);
    ...
    function changeTokensPerBlock(uint256 _tokensPerBlock) external onlyOwner {
        tokensPerBlock = _tokensPerBlock;
        emit ChangeTokensPerBlock(tokensPerBlock);
    }
...
//--------------------------------------------------------------------------------------
    ...
    bool public balancesStakePaused;
    
    function balancesStakeContinue() external onlyOwner {
        balancesStakePaused = false;
    }
    
    function balancesStakePause() external onlyOwner {
        balancesStakePaused = true;
    }

    function isStakePaused() external view returns (bool) {
        return balancesStakePaused;
    }
    
    function balanceStakeMintingStart() external onlyOwner {
        require(msg.sender == owner && balanceStakeStartTime == 0);
        balanceStakeStartTime = now;
    }
