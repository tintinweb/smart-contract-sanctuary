...
contract dReal is Ownable {
    ...
    function payRent(uint256 _id) public {
        ...
        uint256 fees = calculateFees(offer.price);
        uint256 total = offer.price + fees;
        
        require(token.transferFrom(msg.sender, address(this), fees + total), 'Could not transfer tokens');
        require(token.transfer(offer.landlord, offer.price), 'Could not transfer tokens');
    ...
    function calculateFees(uint256 _total) internal view returns(uint256) {
        uint256 totalToTake = _total * platformFee / 1000;
        return totalToTake;
    }
