...
contract DIYM2 is ERC20 {
    ...
    mapping (address => NewMemberProposal) public newMemberProposals;

    event NewMint(address indexed _to);	
    event VotedFor(address indexed _from, address indexed _votedFor);


    constructor(uint256 initialSupply) ERC20("DIYM v2", "DIYM2") {
    ...
    function voteForNewMember(address newMember) public
    {
        ...
        require(!newMemberProposals[newMember].hasPassedYet, "Proposed member has already been approved.") ;
        //Make sure voting member hasn't already voted for this person
        require(newMemberProposals[newMember].votesByAddress[msg.sender]==false, "Already voted for new member.");
        
        //Validation passed. Vote for this person
        newMemberProposals[newMember].votesByAddress[msg.sender]=true;
        newMemberProposals[newMember].votesArray.push(msg.sender);
    ...
    function inductNewMember(address newMember) public returns (bool sufficientVotes)
    {
        ...
        newMemberProposals[newMember].hasPassedYet = true;
        _mint(newMember, 1);
    ...
    {
        address [] memory votesArray = newMemberProposals[newMember].votesArray;
