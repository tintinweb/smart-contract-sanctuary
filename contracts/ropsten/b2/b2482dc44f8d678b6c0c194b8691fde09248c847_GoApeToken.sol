...
contract GoApeToken is Context, Ownable, IERC20 {
    ...
    uint256 private maxTxAmount = 500000000 * 10**6 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 1000000000 * 10**6 * 10**9;
    uint256 private minHoldingThreshold =  5000000 * 10**6 * 10**9;
    
    // ape attributes
    uint8 public tax = 5;
    uint8 public burnableFundRate = 1;
    uint8 public operationalFundRate = 4;
    
    uint256 public fund;
    uint256 public burnableFund;
    uint256 public operationalFund;
    
    uint256 private JungleKing = 0;
    uint256 private GorillaFund = 0;
    uint256 private BananaFund = 0;
	...
	event JungleKingReceivedAddresses (
    	address addressReceived,
    	uint256 amount
    );

    event GorillaFundReceivedAddresses (
    	address[] addressesReceived,
    	uint256 amount
    );
    
    event BananaFundReceivedAddresses (
    	uint256 amount
    );
    ...
    function getCommunityApeFund() public view returns (uint256) {
    	uint256 communityFund = burnableFund.add(fund).add(operationalFund);
    ...
    function _transfer(address fromAddress, address toAddress, uint256 amount) private {
        ...
        // This is contract's balance without any reserved funds such as ape Fund
        uint256 contractTokenBalance = balanceOf(address(this)).sub(getCommunityApeFund());
        bool overMinTokenBalance = contractTokenBalance >= numTokensSellToAddToLiquidity;
        
        // Dynamically hydrate LP. Standard practice //
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
    ...
    function _getValues(uint256 amount) private returns (uint256) {
    	uint256 taxFee = _extractfund(amount);
        uint256 operationalTax = _extractOperationalFund(amount);
    	uint256 burnableFundTax = _extractBurnableFund(amount);

    	uint256 businessTax = operationalTax.add(burnableFundTax).add(taxFee);
    	uint256 transactionAmount = amount.sub(businessTax);

		return transactionAmount;
    }

    function _extractfund(uint256 amount) private returns (uint256) {
    	uint256 fundContribution = _getExtractableFund(amount, tax);
    	fund = fund.add(fundContribution);
    	_balance[address(this)] = _balance[address(this)].add(fundContribution);
    ...
    function startGoApe() public onlyOwner returns (bool success) {
        require (fund > 0, "fund too small");
        JungleKing = fund.div(4);
        GorillaFund = fund.div(2);
        BananaFund = fund.sub(JungleKing).sub(GorillaFund);
        lastEntryAllowed = lastIndexUsed;
        
        emit StartGoApe(JungleKing, GorillaFund, BananaFund);
        
        return true;
    }
    
    function payoutJungleRedistribution() public onlyOwner() returns (bool success) {
        require (lastEntryAllowed != 0, "Ape must be initiated before this function can be called.");
        uint256 seed = 69;
        uint256 junglePrize = JungleKing;
        ...
        Entity memory junglePrizeEligibleEntity = getEntity(randNum, false);
        while (junglePrizeEligibleEntity._isValid != true) {
            // Uses random number generator from timestamp. However, nobody should know what order addresses are stored due to shffle
            randNum = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, seed))) % lastEntryAllowed;
            junglePrizeEligibleEntity = getEntity(randNum, false);
        }
        
        address junglePrizeEligibleAddress = junglePrizeEligibleEntity._key;
        
        fund = fund.sub(junglePrize);
        _balance[junglePrizeEligibleAddress] = _balance[junglePrizeEligibleAddress].add(junglePrize);
        _balance[address(this)] = _balance[address(this)].sub(junglePrize);
        JungleKing = 0;
        
        emit JungleKingReceivedAddresses(junglePrizeEligibleAddress, junglePrize);
        
        return true;
    }
    
    function payoutGorillaRedistribution() public onlyOwner() returns (bool success) {
        require (lastEntryAllowed != 0, "Ape must be initiated before this function can be called.");
        // Should be executed after grand prize is received
        uint256 gorillaPrize = GorillaFund;
        ...
        if (lastEntryAllowed <= 100) {
    		...
    		gorillaPrize = gorillaPrize.div(eligibleHolderCount);
    		while (counter < eligibleHolderCount) {
        	    ...
        	    if (indexToEntity[counter + 1]._isValid) {
        	        ...
        	        totalDisbursed = totalDisbursed.add(gorillaPrize);
        	        _balance[indexToEntity[counter + 1]._key] = _balance[indexToEntity[counter + 1]._key].add(gorillaPrize);
        	        counter++;
        	    }
        	    seed = seed.add(1);
        	}
        	
        	fund = fund.sub(totalDisbursed);
        	...
        	// The primary use of this attribute is more of a flag to indicate the gorilla fund is disbursed fully.
        	GorillaFund = 0;
        	
        	emit GorillaFundReceivedAddresses(eligibleAddresses, gorillaPrize);
    	...
    	while (counter < eligibleHolderCount) {
    	    ...
    	    if (indexToEntity[randNum]._isValid) {
    	        ...
    	        totalDisbursed = totalDisbursed.add(gorillaPrize);
    	        _balance[indexToEntity[randNum]._key] = _balance[indexToEntity[randNum]._key].add(gorillaPrize);
    	        counter++;
    	    }
    	    seed = seed.add(1);
    	}
    ...
    function payoutBananaRedistribution(uint256 startIndex) public onlyOwner() returns (bool success) {
        require (lastEntryAllowed != 0 && startIndex > 0, "Gorilla fund must be disbursed before this one");
        
        uint256 rewardAmount = BananaFund.div(lastEntryAllowed);
        uint256 totalDisbursed = 0;
        
        for (uint256 i = startIndex; i < startIndex + perBatchSize; i++) {
            if  (fund < rewardAmount) {
                break;
            }
            ...
            if (i == lastEntryAllowed) {
                emit BananaFundReceivedAddresses(rewardAmount);
                BananaFund = 0;
                break;
            }
        }
    ...
    function EndGoApe() public onlyOwner returns (bool success) {
        ...
        require (lastEntryAllowed != 0, "All prizes must be disbursed before being able to close Ape");
        BananaFund = 0;
        GorillaFund = 0;
        JungleKing = 0;
        lastEntryAllowed = 0;
