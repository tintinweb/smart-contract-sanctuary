...
contract InitialCoinOffering is  Ownable, Pausable, ReentrancyGuard, Initializable {
    using SafeBEP20 for IBEP20;
    
    event Deposit( address indexed user, uint indexed pSale, uint indexed _saleID, uint amount, uint qty, uint purchaseTime);
    event Withdraw( address indexed user, uint indexed pSale, uint indexed _saleID, uint releasedAmount, uint claimTime);
    ...
    // The block number when offering starts
    uint256[2] public startBlock;
    // The block number when offering ends
    uint256[2] public endBlock;
    // price of offering token
    uint256[2] public offeringPrice = [0.00001e18, 0.00002e18];
    ...
    uint[2] public claimRelease = [50 /* launch date */,10 /* month release */];
    // Number of current sale
    uint[2] public currentSale;
    
    // Active status of the offering
    ...
    modifier onlyPresales( uint _pSale) {
        require((_pSale == 0) || (_pSale == 1), "InitialCoinOffering :: onlyPresales : _pSale must be zero or one");
        _;
    }
     ...
     * @dev Initializes the contract setting the deployer as the initial owner.
     */
    function _initialize(IBEP20 _offeringToken) external initializer {
        require(_offeringToken != IBEP20(address(0)), "InitialCoinOffering :: initialize : offeringToken should not be zero address");
        offeringToken = _offeringToken;
    }
    
    function startNextSaleOne() external onlyOwner {
        uint256 _saleCloseBlock = launchBlock[0] + (unlockBlock[0] + closeBlock[0]);
        require(block.timestamp > _saleCloseBlock, "InitialCoinOffering :: startNextSaleOne : current time should be greater than close time");
    ...
    function startNextSaleTwo() external onlyOwner {
        ...
        require(block.timestamp > _saleCloseBlock, "InitialCoinOffering :: startNextSaleTwo : current time should be greater than close time");
    ...
    function setLaunchBlockSaleOne( uint256 _launchBlock) external onlyOwner {
        require(_launchBlock >= block.timestamp, "InitialCoinOffering :: setLaunchBlockSaleOne : _launchBlock should be greater than current time");
    ...
    function setSaleOneConfig(uint256 _startBlock, uint256 _endBlock, uint256 _offeringAmount ) external onlyOwner {
        require(_startBlock > block.timestamp, "InitialCoinOffering ::  start block should > current time");
        require(_startBlock < _endBlock, "InitialCoinOffering :: startBlock should < endBlock");
        require(_offeringAmount > 0, "InitialCoinOffering :: offeringAmount > 0");
    ...
    function setUnlockBlockSaleOne( uint256 _unlockBlock, uint _releaseOfferingOnEvery,  uint256 _closeBlock, uint _totalOfferRelease) external onlyOwner {
        require(_unlockBlock > 0, "InitialCoinOffering :: unlockBlock > 0");
        require(_releaseOfferingOnEvery > 0, "InitialCoinOffering :: _releaseOfferingOnEvery should > 0");
        require(_totalOfferRelease > 0, "InitialCoinOffering :: _totalOfferRelease should > 0");
    ...
    function setUnlockBlockSaleTwo( uint256 _unlockBlock, uint256 _releaseOfferingOnEvery,  uint256 _closeBlock, uint _totalOfferRelease) external onlyOwner {
        ...
        require(_releaseOfferingOnEvery > 0, "InitialCoinOffering :: _releaseOfferingOnEvery should > 0");
        require(_totalOfferRelease > 0, "InitialCoinOffering :: _totalOfferRelease should > 0");
        require(_closeBlock == (_releaseOfferingOnEvery * _totalOfferRelease), "InitialCoinOffering :: total offer release should equal to close block");
    ...
    function setOfferingAmountSaleOne(uint256 _offerAmount) external onlyOwner {
        require (block.timestamp < startBlock[0], "InitialCoinOffering :: setOfferingAmountSaleOne : cant change offering amount after start block.");
    ...
    function setOfferingAmountSaleTwo(uint256 _offerAmount) external onlyOwner {
        require (block.timestamp < startBlock[1], "InitialCoinOffering :: setOfferingAmountSaleTwo : cant change offering amount after start block.");
    ...
    function unsoldTokens( uint _pSale, uint _saleID ) external onlyOwner onlyPresales( _pSale) {
        require(!isPresaleActive[_pSale], "InitialCoinOffering :: getUnsoldTokens : cannot get tokens until the presale is closed.");
        require(_saleID <= currentSale[_pSale], "InitialCoinOffering :: _saleID < currentSale");
        require((offeringAmount[_pSale][_saleID] > totalSoldTokens[_pSale][_saleID]) && (offeringToken.balanceOf(address(this)) >= offeringAmount[_pSale][_saleID]), "Presale :: unsoldTokens : no unsold tokens left or presale balance < offer Amount");
     ...
     */
    function deposit( uint _pSale, uint _qty) external payable whenNotPaused onlyPresales( _pSale) {
        require(isPresaleActive[_pSale], "InitialCoinOffering :: buyOffer : presale is closed");
        require((block.timestamp >= startBlock[_pSale]) && (startBlock[_pSale] != 0), "InitialCoinOffering :: buyOffer : cannot buy token until start block");
        require(block.timestamp <= endBlock[_pSale], "InitialCoinOffering :: buyOffer : presale is closed, cannot buy token after end block");
        require(msg.value == calcualteOfferInBNB( _pSale, _qty), "InitialCoinOffering :: buyOffer : invalid amount");
        ...
        emit Deposit( _msgSender(), _pSale, currentSale[_pSale], msg.value, _qty, block.timestamp);
    }
     ...
     */    
    function withdraw( uint _pSale, uint _saleID) external onlyPresales( _pSale) whenNotPaused nonReentrant {
        require(isClaimActive[_pSale], "InitialCoinOffering :: claim : not active");
        require(block.timestamp >= launchBlock[_pSale], "InitialCoinOffering :: claim : wait until claim period starts");
        require(_saleID <= currentSale[_pSale], "InitialCoinOffering :: _saleID < currentSale");
        
        UserInfo storage _usrInfo = userInfo[_msgSender()][_pSale][_saleID];
        Variables memory _var;
        
        require(!_usrInfo.isClaimedAll, "InitialCoinOffering :: claim : user has claimed all his offerings");
        require(_usrInfo.balances > 0, "InitialCoinOffering :: claim : user has no available offerings");
        ...
        if((_var._releaseAmount > 0) && (offeringAmount[_pSale][_saleID] >= _var._releaseAmount)) {
            ...
            emit Withdraw( _msgSender(), _pSale, _saleID, _var._releaseAmount, block.timestamp);
        }
        else revert("InitialCoinOffering :: claim : No offering token user has to wait till next release");
