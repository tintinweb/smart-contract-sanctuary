...
contract RewardsGenerator is IRewardsGenerator, OwnableUpgradeable, AbstractDependant {
    ...
    uint256 internal _totalPoolStaked; // includes 5 decimal places
        ...
        )
    {
        ...
        _totalPoolStaked > 0
            ? rewardPerBlockPolicyBook = info
                .rewardMultiplier
                .mul(info.totalStaked)
                .mul(rewardPerBlock)
                .mul(PRECISION)
                .div(_totalPoolStaked)
            : rewardPerBlockPolicyBook = 0;
    ...
    function updatePolicyBookShare(uint256 newRewardMultiplier) external override onlyPolicyBooks {
        ...
        _totalPoolStaked = _totalPoolStaked.sub(totalStaked.mul(info.rewardMultiplier));
        _totalPoolStaked = _totalPoolStaked.add(totalStaked.mul(newRewardMultiplier));
    ...
    function stake(
        ...
        _totalPoolStaked = _totalPoolStaked.add(amount.mul(info.rewardMultiplier));
    ...
    /// @dev returns APY% in DAI multiplied by 10**25
    function getPolicyBookAPY(address policyBookAddress)
        ...
        returns (uint256)
    {
        (uint256 rewardPerBlockPolicyBook, , ) = _getNewAverage(policyBookAddress);
        uint256 totalStakedPolicyBook = _policyBooksRewards[policyBookAddress].totalStaked;

        if (totalStakedPolicyBook == 0 || rewardPerBlockPolicyBook == 0) {
            return 0;
        }

        uint256 rewardPerBlockPolicyBookDAI = priceFeed.howManyDAIsInBMI(rewardPerBlockPolicyBook);

        uint256 rewardMultiplierPolicyBook =
            _policyBooksRewards[policyBookAddress].rewardMultiplier;

        uint256 dailyReward =
            rewardPerBlockPolicyBookDAI
                .mul(BLOCKS_PER_DAY)
                .mul(totalStakedPolicyBook)
                .mul(rewardMultiplierPolicyBook)
                .div(_totalPoolStaked);

        return dailyReward.mul(365).div(totalStakedPolicyBook);
    }
    ...
    function _withdraw(address policyBookAddress, uint256 nftIndex) internal returns (uint256) {
        ...
        _totalPoolStaked = _totalPoolStaked.sub(amount.mul(info.rewardMultiplier));
    ...
    function getDAIStake(uint256 nftIndex) external view override returns (uint256) {
...
interface IRewardsGenerator {
    ...
    /// @notice returns staked DAI equivalent (used in BMIDAIStaking)
    function getDAIStake(uint256 nftIndex) external view returns (uint256);
