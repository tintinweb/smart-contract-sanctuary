...
contract Exchange {
    ...
    uint constant ORDER_LIFE_TIME = 5 minutes;
    ...
    modifier canBeClosed(uint _index) {
        emit Log(block.timestamp, orders[_index].createdAt + ORDER_LIFE_TIME);
        uint time = orders[_index].createdAt + ORDER_LIFE_TIME;
