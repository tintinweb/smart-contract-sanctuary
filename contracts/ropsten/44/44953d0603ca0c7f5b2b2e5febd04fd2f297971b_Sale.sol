...
contract Sale is Owned {
    ...
    uint256 saleStart;
    uint256 saleEnd = 0;
    uint256 minLimit = 100000000000000000; // 0.1 in terms of eths
    ...
    receive() external payable saleIsOpen {
        ...
        purchaseToken(tokens);
        totalInvestment = totalInvestment.add(msg.value);
    }
    
    function _preValidatePurchase() private {
        users[msg.sender].investment = users[msg.sender].investment.add(msg.value);
        require(users[msg.sender].investment >= minLimit, "Purchase is lower than min limit");
        require(users[msg.sender].investment <= maxLimit, "Purchase limit exceeds");
    } 
    
    // tokens purchased can be claimed after the release date
    function purchaseToken(uint256 _amount) private {
        users[msg.sender].tokens = users[msg.sender].tokens.add(_calculateTokens(_amount));
        emit TokenPurchased(msg.sender, _amount, _calculateTokens(_amount));
    }
    ...
    function claimInvestment(uint256 claimAmount) external onlyOwner{
        require(claimAmount <= totalInvestment);
