...
contract MSG {
    ...
    uint256 public id;
    event Message(uint Id, address writer, string message, string name, uint time);
...
function read(uint256 _id) public view returns (address writer, string memory message, string memory _name, uint _time) {
        return (data[_id].writer, data[_id].message, names[data[_id].writer], data[id].time);
    }
