...
contract BulletLoans is ERC721 {
    ...
    function createLoan(
        ...
        loans[instrumentId] = LoanMetadata(_underlyingToken, Status.Issued, principal, totalDebt, 0);
    ...
    function repay(uint256 instrumentId, uint256 amount) public {
        ...
        if (loan.amountRepaid >= loan.totalDebt) {
            loan.status = Status.FullyRepaid;
        }
        loan.underlyingToken.transferFrom(msg.sender, ownerOf(instrumentId), amount);
        emit LoanRepaid(instrumentId, amount);
    }

    function getStatus(uint256 instrumentId) public view returns (Status) {
        require(_exists(instrumentId), "BulletLoans: Cannot get status of non-existent loan");
        return loans[instrumentId].status;
    }

    function getRemainingDebt(uint256 instrumentId) public view returns (uint256) {
        require(_exists(instrumentId), "BulletLoans: Cannot get remaining debt for a non-existent loan");
        if (loans[instrumentId].amountRepaid > loans[instrumentId].totalDebt) {
            return 0;
        }
        return loans[instrumentId].totalDebt - loans[instrumentId].amountRepaid;
    }
}
...
library BPMath {
    ...
    function div(BP a, uint256 b) internal pure returns (BP) {
        return BP.wrap((BP.unwrap(a) / b));
    }
...
contract ManagedPortfolio is IERC721Receiver, ERC20, Manageable {
    ...
    uint256 public latestLoanDefaultDate;
    ...
    function createBulletLoan(
        ...
        uint256 loanDefaultDate = block.timestamp + loanDuration;
        require(loanDefaultDate <= endDate, "ManagedPortfolio: Loan end date is greater than Portfolio end date");
        if (loanDefaultDate > latestLoanDefaultDate) {
            latestLoanDefaultDate = loanDefaultDate;
        }
        BP protocolFee = protocolConfig.protocolFee();
        uint256 managersPart = managerFee.mul(principalAmount).mul(loanDuration).div(YEAR).normalize();
        uint256 protocolsPart = protocolFee.mul(principalAmount).mul(loanDuration).div(YEAR).normalize();
        ...
        uint256 loanId = bulletLoans.createLoan(underlyingToken, principalAmount, repaymentAmount);
        _loans.push(loanId);
    ...
    function setEndDate(uint256 newEndDate) external onlyManager {
        ...
        require(newEndDate >= latestLoanDefaultDate, "ManagedPortfolio: End date cannot be less than max loan default date");
        endDate = newEndDate;
