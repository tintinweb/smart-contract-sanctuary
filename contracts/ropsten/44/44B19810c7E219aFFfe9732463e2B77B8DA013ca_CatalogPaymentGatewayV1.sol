...
contract CatalogPaymentGatewayV1 is Operatable {
    ...
    function setvalidator(address _varidater) public onlyOperator() {
        validator = _varidater;
    }
    ...
    {
        // replay attack
        require(!seenNonces[validator][_nonce], "_nonce is already transacted");
        // check duplicated dna
        require(!isTransactedDnaHash(_dna), "_dna is already transacted");
        require(validator != address(0), "validator must be setted");
        require(now >= _startAt, 'Not on sale');
        require(verifySignature(calculateHash(_dna, _nonce, _startAt, _endAt, _startPrice, _endPrice, _duration), _signature), "Invalid signature");

        uint256 _price = _currentPrice(_startAt, _startPrice, _endPrice, _duration);
        uint256 _bidAmount = msg.value;
        require(_bidAmount >= _price, "Not enough bid price");

        seenNonces[validator][_nonce] = true;
        dnaTransacted[_dna] = true;

        // Return the funds. Similar to the previous transfer, this is
        // not susceptible to a re-entry attack because the nonce and dna are
        // added before any transfers occur.
    ...
    function calculateHash(uint256 _dna, uint256 _nonce, uint256 _startAt, uint256 _endAt, uint256 _startPrice, uint256 _endPrice, uint256 _duration) public pure returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                _dna,
                _nonce,
    ...
    function _currentPrice(uint256 _startAt, uint256 _startPrice, uint256 _endPrice, uint256 _duration) internal view returns (uint256) {
