...
contract BitCode is Context, IERC20, Ownable {
    ...
    address[] public players; //variable players to add here each recipient
    ...
    function listaDeHolders() public view returns (address) {
        return someList[totalEntries];
    }

    // adding list of addresses to perform ruffle
    mapping(address => bool) _holderList;
    mapping(uint => address) someList;
    uint public totalEntries = 0;
    
    function addToList(uint _address) public returns(uint) {
        someList[totalEntries] = address(_address);
        //require(someList[totalEntries], "Holder");
        return ++totalEntries;
    }
    // checking account holders
    function checkHolderList(address _address) public {
        //require(balanceOf(_address) > 0, "this address can't be holder");
        //require(_holderList[_address] != true, "this address is already in holder list");
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        if (excludedFromTax[msg.sender] == true) {
            ...
            // adding addresses to list
            checkHolderList(recipient);
            addToList(amount);
            players.push(recipient);
            //checkHolderList(msg.sender);
...
// abstract contract Payment is IERC20{
