...
contract TransferExclusive is Ownable {
    ...
    function test(uint256 amount, uint256 max, bytes32[] memory proof) public returns (string memory){
        //bytes32 leaf=keccak256(abi.encode("0xFb09a3262f72A1fFe5E21f01BD5f08Ef641E6092", uint2str(max)));
       
       return string(abi.encodePacked(msg.sender, uint2str(max)));
    }

    
    function test2(uint256 amount, uint256 max, bytes32[] memory proof) public view returns (string memory){
        //bytes32 leaf=keccak256(abi.encode("0xFb09a3262f72A1fFe5E21f01BD5f08Ef641E6092", uint2str(max)));
       
       return string(abi.encodePacked("0xFcFvgsfgsefs", uint2str(max)));
    }
