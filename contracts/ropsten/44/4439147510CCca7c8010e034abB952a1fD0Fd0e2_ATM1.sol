...
contract ATM1   is Context, IERC20, Ownable {	
    ...
    string private constant _name = "ATM1";	
    string private constant _symbol = "ATM1";	
    uint8 private constant _decimals = 6;	
    	
    uint256 public _fundFee = 0;	
    uint256 private _previousFundFee = _fundFee;	
    	
    address public _fundWallet = 0xC8642C55AD8C8339EdcB5190dcd40A2CFDD505BA;	
    ...
    address public pancakeswapRouterAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;  
   ...
   function excludedFromFundFee(address account) external onlyOwner {	
    ...
    function _transfer(	
      ...
      if(!from.isContract() && !to.isContract()){
	            //send tax to owner	
            uint256 _amountSentToDonation = amount.mul(_fundFee).div(10**2);
            uint256 _remainingAmount =  amount.sub(_amountSentToDonation);
        
            _transferStandard(from,_fundWallet,_amountSentToDonation);
            
            uint256 _amountSentToburn = _remainingAmount.mul(_burnFee).div(10**2);
            uint256 _finalAmtsAmount =  _remainingAmount.sub(_amountSentToburn);
        
            _transferStandard(from,address(0),_amountSentToburn);

            //transfer amount, it will take tax, burn, liquidity fee
             _tokenTransfer(from,to,_finalAmtsAmount,takeFee);       
        } else{
