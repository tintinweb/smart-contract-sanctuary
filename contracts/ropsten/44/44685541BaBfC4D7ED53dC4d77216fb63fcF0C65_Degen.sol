...
abstract contract Router {
    ...
    function getBestQuote(address[] memory path, uint256 amountIn, OrderType orderType) public view returns (uint, uint256,uint256) {
        ...
        // if(sushiAmounts[0]>uniAmounts[0]) {
        //     return(2,sushiAmounts[0]);
        // } else {
        //     return(1,uniAmounts[0]);
        // }
        
        return (0,sushiAmounts[0],uniAmounts[0]);
...
contract Degen is Router {
    ...
    function executeSwap(OrderType orderType, address[] memory path, uint256 assetInOffered, uint256 fees) external payable{
        ...
        (uint dexId, uint256 minAmountExpected,uint256 ac) = getBestQuote(path, assetInOffered, orderType);
        if(dexId == 1){
