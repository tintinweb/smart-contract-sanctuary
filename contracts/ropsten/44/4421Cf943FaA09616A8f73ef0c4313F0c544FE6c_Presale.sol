...
contract Presale is Ownable {
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount) internal view {
        require(isOpen(), "Presale: sale is not open");
        require(!hasEnded(), "Presale: sale is over");
        require(weiAmount >= minContribution, "Presale: min contribution criteria not met");
        require(contributions[beneficiary].add(weiAmount) <= maxContribution, "Presale: max contribution criteria not met");
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "Presale: presale is not over");
        require(softCapReached(), "Presale: soft cap not reached, refund is available");
