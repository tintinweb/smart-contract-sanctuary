...
contract Go9tro {
    ...
    event MissedTokenReceive(address indexed receiver, address indexed _from, uint8 indexed matrix, uint8 level,uint amount, uint time);
    ...
    function registrationExt(address referrerAddress) isLock external {
        registration(msg.sender, referrerAddress);
    }
    
    function buyNewLevel(uint8 matrix, uint8 level) isLock external  {
        require(isUserExists(msg.sender), "user is not exists. Register first.");
        require(matrix == 1 || matrix == 2, "invalid matrix");
        require(Token.balanceOf(msg.sender) >= levelPrice[level], "Insufficient token balance");
    ...
    function registration(address userAddress, address referrerAddress) isLock private {
        require(Token.balanceOf(userAddress) >= levelPrice[1].mul(2), "Insufficient token balance");
        ...
        Token.transferFrom(userAddress,address(this), levelPrice[1].mul(2));
        ...
        if (matrix == 1) {
            while (true) {
                if (users[receiver].G3Matrix[level].blocked) {
                    emit MissedTokenReceive(receiver, _from, 1, level, levelPrice[level], now);
            ...
            while (true) {
                if (users[receiver].G4Matrix[level].blocked) {
                    emit MissedTokenReceive(receiver, _from, 2, level, levelPrice[level], now);
                    isExtraDividends = true;
    ...
    function balanceOf(address userAddr) public view returns(uint256 user_token){
