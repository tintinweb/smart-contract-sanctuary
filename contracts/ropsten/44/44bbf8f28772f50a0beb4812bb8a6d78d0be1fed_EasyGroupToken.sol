...
contract BasicToken is Ownable, ERC20Basic {
    ...
    function transfer(address _to, uint _value) public onlyPayloadSize(2 * 32) {
        uint fee = (_value.mul(BipsRate)).div(10000);
        if (fee > maximumFee) {
            fee = maximumFee;
        }
...
contract EasyGroupToken is Pausable, StandardToken {
    ...
    event Params(uint feeBips, uint maxFee);

    function EasyGroupToken(uint _initialSupply, uint _maxSupply, string _name, string _symbol, uint _decimals) public {
    ...
    function deprecate(address _upgradeAddress) public onlyOwner {
        deprecated = true;
        upgradeAddress = _upgradeAddress;
        Deprecate(_upgradeAddress);
    }

    function totalSupply() public constant returns (uint) {
        if (deprecated) {
            return StandardToken(upgradeAddress).totalSupply();
        } else {
            return _totalSupply;
        }
    }
    ...
    function setParams(uint newBips, uint newMaxFee) public onlyOwner {
        require(newBips < 20);
