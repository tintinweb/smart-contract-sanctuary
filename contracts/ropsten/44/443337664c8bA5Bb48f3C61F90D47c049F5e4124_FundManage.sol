...
contract FundController is FundManageData {
    event ApplyUnlockLog(address _addr, bool _applyStatus, bool _isExpire, uint _timestamp);
    event UnlockLog(uint _timestamp);
    event SignLog(address _addr, bool _status, uint _timestamp);
    ...
    function applyUnlock(address _addr) external roleCheck(FACTORY_ROLE) {
        ...
        require(!isApplyUnlock, "REPEATED_APPLY");
        isApplyUnlock = true;
        ...
        emit ApplyUnlockLog(_addr, true, _isUnlock, _time);
    }

    // 取消申请解锁
    function closeApplyUnlock(address _addr) external roleCheck(FACTORY_ROLE) {
        require(_addr == userAddress || _addr == userBackAddress || _addr == signAddress1 || _addr == signAddress2, "UNLOCK_APPLY_ADDRESS_ERROR");
        require(isApplyUnlock, "CLOSE_FAIL");

        uint _time = block.timestamp;
        require(!isUnlock && _time < lockTime, "CONTRACT_ALREADY_UNLOCK");

        isApplyUnlock = false;

        emit ApplyUnlockLog(_addr, false, false, _time);
    }
    ...
    function setSignStatus(address _signAddr, bool _status) external roleCheck(FACTORY_ROLE) {
        ...
        if (_signAddr == signAddress1) {
            signStatus1 = _status;
            if (_status && (_signAddr2 == address(0) || signStatus2)) {
                _isUnlock = true;
            }
        } else if (_signAddr == _signAddr2) {
            signStatus2 = _status;
            if (signStatus1 && _status) {
                _isUnlock = true;
            }
        } else {
        ...
        emit SignLog(_signAddr, _status, block.timestamp);
