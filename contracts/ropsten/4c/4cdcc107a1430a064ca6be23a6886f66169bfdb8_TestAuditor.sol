...
contract TestAuditor{
    ...
    function getONG_AUDIT(bytes32 id)  public returns ( bytes32 i, string ong, string audit, string audit_des, uint256 date,  bool activo ) {
    ...
    function getONG(bytes32 id)  public returns ( bytes32 i, string ong_name, string ong_des, uint256 date,  bool activo ) {
        return (mapaONG[id].id, mapaONG[id].ong_name, mapaONG[id].ong_desc, mapaONG[id].date, mapaONG[id].activo);
    }
    
    function addONG_AUDIT(bytes32 id, string audit,  string audit_des)  public onlyOWNER(msg.sender) {
        
        if(!mapaONG_AUDIT[id].activo){
            mapaONG_AUDIT[id] = ONG_AUDIT(id, mapaONG[id], audit, audit, block.timestamp, true);
