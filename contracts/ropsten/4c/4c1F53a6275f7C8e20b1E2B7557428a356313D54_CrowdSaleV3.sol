...
interface ICrowdSale{
    ...
    function referrals(address _user,uint256 _index) external view returns(address);
    function getReferralLength(address referral) external view returns(uint256);
    function transferOwnership(address newOwner) external; 
}

contract CrowdSaleV3 is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 constant public PRICE_CAP = 1.9 ether;  // 1.9 usdt
    uint256 constant public INITIAL_PRICE = 1.400027339 ether;   // 1.3 usdt
    uint256 constant public TOTAL_ROB_TO_BE_SOLD = 6700000 ether;
    uint256  public PRICE_INCREASE;
    ...
    constructor(address _usdt, address _rob,ICrowdSale _crowdSaleV1){
        ...
        PRICE_INCREASE = (PRICE_CAP.sub(INITIAL_PRICE)).mul(1e18).div(TOTAL_ROB_TO_BE_SOLD);
    }
    ...
    function validateAndManageReferrals(uint256 _usdt_amount, address referee) internal {
        require(_usdt_amount > 0, "CrowdSaleV3: Cannot buy for 0");
        require(referee != address(0), "CrowdSaleV3: referee can not be a 0 address");
        require(referee != msg.sender, "CrowdSaleV3: self reference.");
    ...
    function transferTokensWithReferrals(uint256 _amount, IERC20 token) internal {
        ...
        while(counter < 5){
            referee = CROWDSALEV1.direct_referee(referee);
    ...
    function newPrice(uint256 _tokenReceive) internal view returns(uint256){
        uint256 priceIncrease = (price.add(_tokenReceive.mul(PRICE_INCREASE).div(1e18)));
 ...
 // A(referee) => B(referral), A refers B
    ...
    function calculateUsdtTokens(uint256 _rob_amount) public view returns(uint256){
        uint256 usdtToBePaid =((_rob_amount.mul(2).mul(price)).add(_rob_amount.mul(_rob_amount.sub(1e18)).mul(PRICE_INCREASE).div(1e18))).div(2).div(1e18);
        return usdtToBePaid;
    }
    
    function calculateRobTokens(uint256 _usdt_amount) public view returns(uint256){
        // n = (sqrt(b^2+4ac)-(a-d))/2d
        uint256 a = ((price.mul(2)).sub(PRICE_INCREASE));
        uint256 b = sqrt((a.mul(a)).add(_usdt_amount.mul(8).mul(PRICE_INCREASE)));
        uint256 c = (price.mul(2)).sub(PRICE_INCREASE);
        uint256 d = b.sub(c);
        uint256 receivingRobTokens = d.mul(1e18).div(2).div(PRICE_INCREASE);
        return receivingRobTokens;
    }
    ...
    function transferOwnershipV1(address _newOwner) public onlyOwner{
        CROWDSALEV1.transferOwnership(_newOwner);
    }
