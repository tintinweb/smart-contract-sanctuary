...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    constructor (address payable owner_) {
        ...
        _Softcap_deadline = contractcreationtime + 7889229;
        _burningtime = contractcreationtime+ 34186659;
    }
	...
	//If total sold reaches hardcap, buy function will not mint tokens 
     ...
     function Buy()payable public returns(bool){
        ...
        require(block.timestamp <= contractcreationtime + 31556916 ,"sale finished ");
     ...
     function FinalizeSale()payable public returns(bool){
         
        require(msg.sender == _owner , "NOT Accessable");
        uint256 amount = _HardCapTokens - _TotalSold;
        require (amount>0, "hardcap reached");
        require(block.timestamp > contractcreationtime + 31556916 ,"sale not finished ");
        require (_TotalSold <=78812e8, "sold out");
	...
	//if user refunds, tokens will be burned and deducted from the total supply
     ...
     function RefundNBurn (address payable recipient) external{
        ...
        require(block.timestamp > contractcreationtime + 7889229 ,"time limit not reached wait until softcap deadline");
	...
	//address 0x3737373737373737373737373737373737373737 will be used as a Charon
	//if a user wants to burn tokens, they will send tokens to Charon
	//ones a month, this function will called and burn tokens inside
    function Charon(uint256 amount) public virtual {
     require(block.timestamp > _burningtime ,"Burning time not reached, tokens are staying with Charon");
    _burn(0x7a2315E6894EC79329b18B61d708Eb13FD020EE4, amount);
    _burn(0x3737373737373737373737373737373737373737, amount);
    ...
    function ClaimLocked()public returns(bool success){
        ...
        if(msg.sender == 0x7a2315E6894EC79329b18B61d708Eb13FD020EE4 ){
            ...
            require(block.timestamp >=contractcreationtime + 15778458  && !developers,"time limit not reached ");
            _mint(msg.sender,275e8);
            developers = true;
            return(true);
        }else if(msg.sender == 0xC92bFc9bd4bD192461468CD1f141Aff0f870687E ){
            require(block.timestamp >= contractcreationtime + 23667687 && !seed_investors,"time limit not reached ");
            _mint(msg.sender,150e8);
            seed_investors = true;
            return(true);
        }
        else if(msg.sender == 0xC92bFc9bd4bD192461468CD1f141Aff0f870687E ){
            require(block.timestamp >= contractcreationtime + 31556916 && !wei_dai,"time limit not reached ");
            _mint(msg.sender,920e8);
            wei_dai = true;
            return(true);
        }
        else if(msg.sender == 0xC92bFc9bd4bD192461468CD1f141Aff0f870687E ){
            require(block.timestamp >= contractcreationtime + 63113832 && !founder,"time limit not reached ");
