...
contract Apyfier is Ownable {
    ...
    */
    ...
    function usersAA(address _address,uint256 _id) public view returns(uint256){
       uint256 amount = users[_address].strategyShares[_id];
       return amount;
    }
    
    function testUpdate(uint256 _sid, uint256 _amount)external onlyEOA{
        _updateStrategy(_sid, _amount, true);
    }
    
    function testDeposit(uint256 _amount) external onlyEOA{
        require(_amount > 0, '_amount = 0');
        
        token.transferFrom(msg.sender, address(this), _amount);
        
        emit Deposit(msg.sender, 12111111111, _amount);
    }
    
    function returnStatus(uint256 _sid) public view returns(uint256 _stauts){
        Strategy memory _strategy  = strategies[_sid];
        if(_strategy.status == StrategyStatus.Active ){
            return 0;
        }else if (_strategy.status == StrategyStatus.Inactive){
            return 1;
        }else{
            return 2;
        }
    ...
    function testGetApyBalance(uint256 _sid) public view returns(uint256 _strategyBalance){
        Strategy memory _strategy  = strategies[_sid];
        _strategyBalance = _strategy.status == StrategyStatus.Active ? _strategy.instance.balance(): 123123;
        return _strategyBalance;
    }
    
    function strategiesAAA(uint256 _id) public view returns(uint256 _lastBalance, uint256 _shareValue, uint256 _shareTotal, uint256 _pendingBalance, uint256 _lockedBalance, StrategyStatus _status){
       Strategy memory strateg = strategies[_id];
        _lastBalance =  strateg.lastBalance;
        _shareValue = strateg.shareValue;
        _shareTotal = strateg.shareTotal;
        _pendingBalance = strateg.pendingBalance;
        _lockedBalance = strateg.lockedBalance;
        _status = strateg.status;
       return (_lastBalance,_shareValue,_shareTotal,_pendingBalance,_lockedBalance,_status);
    }
    ...
    */
    ...
    function _calculateShareValue(Strategy storage _strategy) private view returns (uint256 _strategyBalance, uint256 _balance, uint256 _shareValue, uint256 _gain) {
        _strategyBalance = _strategy.status == StrategyStatus.Active ? _strategy.instance.balance() : 0;//10
