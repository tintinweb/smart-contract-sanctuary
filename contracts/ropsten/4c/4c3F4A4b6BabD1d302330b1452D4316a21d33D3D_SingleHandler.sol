...
contract InstanceBase is ERC721Full
{
    using Address for address;
	
	uint public maxTokensCount;	
...
contract SingleHandler is InstanceBase
{
	string public baseTokenIPFSHash;
    ...
    constructor(
		...
		address _checkAccessContract
	) InstanceBase(_name, _symbol, _maxTokensCount, _tokenURI, _checkAccessContract)
	{
		baseTokenIPFSHash = _tokenIPFSHash;
    }

	function tokenIPFSHash(uint256 tokenId) public view returns (string memory hash)
	{
		require(_exists(tokenId), "query for nonexistent token");
		return baseTokenIPFSHash;
	}
	
	function issueToken(address addr, uint256 tokenId) internal
	{
		require( tokenId!=0, "tokenId must not be zero" );
		...
		{
			require( totalSupply() < maxTokensCount, "exceeding the limit of tokens");
			_mint(addr, tokenId);
		}
	}
	
	function giveTokenTo(address to, uint tokenId) public onlyAdmin
	{
		issueToken(to, tokenId);
	}

	function giveTokens(address[] memory addrs, uint[] memory idTokens) public onlyAdmin
	{
		...
		{
			issueToken(addrs[i], idTokens[i]);
        }
    }

	function takeToken(uint256 tokenId, uint64 nonce, bytes32 r, bytes32 s, uint8 v) public
	{
		bytes memory prefix = "\x19Ethereum Signed Message:\n32";
		bytes32 hash = keccak256( abi.encodePacked(address(this), _msgSender(), nonce, tokenId) );
