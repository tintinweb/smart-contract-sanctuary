...
// File: contracts\StkBMIToken.sol
...
contract StkBMIToken is ERC20 {
	...
	constructor(address _stakingContract) ERC20("Staking BMI", "stkBMI") {
		stakingContract = _stakingContract;
	}

	function mint(address account, uint256 amount) public onlyBMIStaking {
		_mint(account, amount);
	}

	function burn(address account, uint256 amount) public onlyBMIStaking {
...
contract BMIStaking is Ownable {
	...
	event StakedBMI(uint256 stakedBMI, uint256 mintedStkBMI, address recipient);
	event WithdrawnBMI(
		uint256 withdrawnBMI,
		uint256 burnedStkBMI,
		address recipient
	);
	event StakingFundsWithdrawn(address to);
	...
	function getStakedBMI(address _address) public view returns (uint256) {
		return _convertToBMI(stkBMIToken.balanceOf(_address));
	}
