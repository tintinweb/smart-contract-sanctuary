...
* 3% operation/dev/marketing/research wallet
*
*/
...
// File: contracts/HeadShootToken.sol

pragma solidity >=0.6.0 <0.8.0;
// SPDX-License-Identifier: None

contract HeadShoot is Context, IERC20, Ownable {
    ...
    string private constant _name = "HeadShoot";
    string private constant _symbol = "$HST";
    uint8 private constant _decimals = 9;
    uint8 private constant _millions = 1;
    uint8 private constant _thousands = 6;
    uint8 private constant _tokenSellPercents = 1;
    uint8 private constant _maxTxAmountPercents = 5;

    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = _millions * 10**_thousands * 10**_decimals;
    ...
    uint256 public _maxTxAmount = _maxTxAmountPercents * 10**2  * 10**_decimals;
    uint256 public numTokensSellToAddToLiquidity = _tokenSellPercents / 1000 * _tTotal;   //0.1%
    ...
    */
    function setRfiRatesPercents(uint8 _rfi, uint8 _lp, uint8 _dev) public onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tLiquidity).sub(s.tDev);
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi) {
        ...
        rTransferAmount = rAmount.sub(rRfi).sub(rLiquidity).sub(rDev);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        if (_rOwned[recipient] == 0) {numOfHolders++;}
        ...
        reflectDevandResearchFee(s.tDev);

        emit Transfer(sender, recipient, s.tTransferAmount);
    }

    function reflectDevandResearchFee(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDevelopment =  tDev.mul(currentRate);
        _tDevelopmentTotal = _tDevelopmentTotal.add(tDev);
        _rOwned[devWallet] = _rOwned[devWallet].add(rDevelopment);
    }
