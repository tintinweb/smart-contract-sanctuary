...
contract CareToken is ERC20 {
  ...
  // Only to be used once all sources of generating CARE are to be in a closed ecosystem
  bool public _freezeApprovedMintersList = false;

  // v0 pet owner staking variables
  ...
  constructor() ERC20('Spirit Orb Pets Care Token', 'CARE') {
    _mint(msg.sender, 1000000 * 10 ** 18); // Initial tokens minted for drops and contests
    _owner = msg.sender;
  }
  ...
  /**
  * @dev in case more is needed for contests
  */
  ...
  */
  ...
  /**
  * @dev If the community decides that minting should only occur to the SOPV1 contract
  * @dev then this will close that loop permanently.
  */
  function closeMintToOwner() external onlyOwner {
    require(!_closedMintToOwner, "This can only be set to true then never set again.");
    _closedMintToOwner = true;
  }

  /**
  * @dev This adds new contracts to the list that is allowed to generate new tokens
  * @dev for use in the CARE token system
  */
