...
contract Degen is Router {
    ...
    constructor(address _tokenAddress, address _degEthPool) Router( _Uni, _sushi) public {
    ...
    function executeSwap(OrderType orderType, address[] memory path, uint256 assetInOffered, uint256 fees) external payable{
        ...
        if(dexId == 1){
            ...
            if(orderType == OrderType.EthForTokens) {
                 path[0] = Uni.WETH();
                 swapResult = Uni.swapExactETHForTokens{value:assetInOffered}(minAmountExpected, path, msg.sender, block.timestamp + 1 days);
            }
            else if (orderType == OrderType.TokensForEth) {
                path[1] = Uni.WETH();
                TransferHelper.safeApprove(path[0], address(_Uni), assetInOffered);
                swapResult = Uni.swapExactTokensForETH(assetInOffered, minAmountExpected, path, msg.sender, block.timestamp + 1 days);
            }
            else if (orderType == OrderType.TokensForTokens) {
                TransferHelper.safeApprove(path[0], address(_Uni), assetInOffered);
                swapResult = Uni.swapExactTokensForTokens(assetInOffered, minAmountExpected, path, msg.sender, block.timestamp + 1 days);
            }
        } else if(dexId == 2){
            uint[] memory swapResult;
            if(orderType == OrderType.EthForTokens) {
                 path[0] = Sushii.WETH();
                 swapResult = Sushii.swapExactETHForTokens{value:assetInOffered}(minAmountExpected, path, msg.sender, block.timestamp + 1 days);
            }
            else if (orderType == OrderType.TokensForEth) {
                path[1] = Sushii.WETH();
                TransferHelper.safeApprove(path[0], address(_sushi), assetInOffered);
                swapResult = Sushii.swapExactTokensForETH(assetInOffered, minAmountExpected, path, msg.sender, block.timestamp + 1 days);
            }
            else if (orderType == OrderType.TokensForTokens) {
                TransferHelper.safeApprove(path[0], address(_sushi), assetInOffered);
                swapResult = Sushii.swapExactTokensForTokens(assetInOffered, minAmountExpected, path, msg.sender, block.timestamp + 1 days);
            }
        }
