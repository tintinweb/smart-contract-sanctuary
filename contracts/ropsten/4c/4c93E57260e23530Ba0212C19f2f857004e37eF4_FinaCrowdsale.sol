...
contract HoldErc20TokenCrowdsaleHelper {
    ...
    /**
     * @param tokenContract_ ERC20 token contract address
     * @param minTokenAmount_ minimum amount of token required to hold
     */
    constructor(address tokenContract_, uint256 minTokenAmount_) {
        require(
            tokenContract_ != address(0),
            "HoldErc20TokenCrowdsaleHelper: zero token address"
        );

        require(
            minTokenAmount_ > 0,
            "HoldErc20TokenCrowdsaleHelper: zero min token amount"
        );

        tokenContract = tokenContract_;
        minTokenAmount = minTokenAmount_;
    }

    modifier holdsSufficientTokens(address account) {
        require(
            IERC20(tokenContract).balanceOf(account) >= minTokenAmount,
            "HoldErc20TokenCrowdsaleHelper: account less than min"
        );
        _;
    }
}
