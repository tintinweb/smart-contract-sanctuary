...
contract MPChannels {
    ...
    struct Sender {
        mapping (uint => bytes) messages; //indexed by the number of messages
    }
    mapping(string => Sid) sids;
    function sendMessage(string memory sid, bytes memory _message, uint _receivingPid, uint _sendingPid, uint _msgNum) public {
        if (sids[sid].participants[_sendingPid] != msg.sender) revert("Wrong sender");
      if (sids[sid].receivers[_receivingPid].senders[_sendingPid].messages[_msgNum].length != 0) revert ("Message existing");
        sids[sid].receivers[_receivingPid].senders[_sendingPid].messages[_msgNum] = _message;
    }
    function readMessage(string memory sid, uint _receivingPid, uint _sendingPid, uint _msgNum) public view returns (bytes memory, uint b) {
