...
contract InTokenTest10{
    ...
    bool  movePermissionStat = false;
    bool  isLockAllAccount = false;
    ...
    function InTokenTest10(uint256 initialSupply, 
                          string tokenName, 
                          uint8 decimalUnits, 
    ...
    modifier canTransfer{
        require(!isLockAllAccount && !lockOf[msg.sender] );
        _;
    }

    /* Change contract name */
    function changeTokenName(string _tokenName) nolyOwner public returns (bool){
        name = _tokenName;
        return true;
    }

    /* Change contract symbol */
    function changeSymbol(string tokenSymbol)  nolyOwner public returns (bool){
         symbol = tokenSymbol;
    }
    
    /* Add supply symbol  */
    function addSupply(uint256 _addSupply)  nolyOwner public returns (bool){
    ...
    /* burn symbol */
    function burnSupply(uint256 supply) nolyOwner public returns (bool){
    ...
    function setMovePermissionStat(bool status) nolyOwner public {
       movePermissionStat = status;
    }

    /* move  permissions */
    function movePermission(address to) nolyOwner public returns (bool){
    ...
    function freeze(address to,uint256 _value) nolyOwner public returns (bool) {
    ...
    /* unFreeze value  */
    function unFreeze(address to,uint256 _value) nolyOwner public returns (bool) {
       require(to != 0x0 && to != owner && _value > 0);
    ...
    function lockAccount(address to) nolyOwner public returns (bool){
       lockOf[to] = true;
       return true;
    }

    function unlockAccount(address to) nolyOwner public returns (bool){
       lockOf[to] = false;
       return true;
    }

    function lockAllAccount() nolyOwner public returns (bool){
       isLockAllAccount = true;
       return true;
    }

    function unlockAllAccount() nolyOwner public returns (bool){
       isLockAllAccount = false;
       return true;
    }
    ...
    function transferFrom(address _from, address _to, uint256 _value)  public returns (bool) {
        require(_to != 0x0 && _value > 0);
        require( !isLockAllAccount && !lockOf[_from] && balanceOf[_from] >= _value && _value <= allowance[_from][msg.sender]);
    ...
    function kill() nolyOwner  public {
        selfdestruct(owner);
    }
