...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    constructor (string memory name_, string memory symbol_) {
        ...
        _symbol = symbol_;
        _owner = msg.sender;
        finalSupply = 100000000 * 10 ** uint256(decimals());
    }
    ...
    function setBurnRatio(uint256 _r) public onlyOwner {
        require((_r + rewardRatio) < 100, "100 ??");
        burnRatio = _r;
    }
    
    function setRewardRatio(uint256 _r) public onlyOwner {
        require((_r + burnRatio) < 100, "100 ??");
        rewardRatio = _r;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(rewardRatio > 0 && _totalSupply > finalSupply){
            rewardAmount = amount * rewardRatio / 100;
        }
        ...
        _balances[recipient] += (amount - rewardAmount - burnAmount);
        depositLP(recipient, (amount - rewardAmount - burnAmount));
        if(burnAmount > 0){
