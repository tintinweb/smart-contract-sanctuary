...
contract StakingRewards4CRE8 is StakingTokenWrapper4CRE8, Ownable { 
    ...
    // uint256 public immutable LOCK_DURATION = 30 days; 
    uint256 public immutable DURATION = 1 minutes; 
    uint256 public immutable LOCK_DURATION = 1 minutes; 

    // uint256 public immutable rewardsPerDay = 100 * 1000; // 100k per day
    uint256 public immutable rewardsPerDay = 100; // 100k per day
    ...
    */
    function withdraw(uint256 _amount, uint256 entryIndex) external {
        ...
        for (i = fromDay; i < toDay; i++) {
            ...
            additionalRewards = additionalRewards + rewardsPerDay / stakedPerDay[i] * stakingEntries[msg.sender][entryIndex].value;
        }        
        ...
        claimAmt = stakingEntries[msg.sender][entryIndex].claimableRewards / stakingEntries[msg.sender][entryIndex].value * _amount;
        ...
        stakedPerDay[uint256(block.timestamp / DURATION)] = totalSupply();
    }
    ...
    */
    function claimReward(uint256 entryIndex, uint256 _amount) public {
        ...
        stakingEntries[msg.sender][entryIndex].fullDaysStaked = uint256(block.timestamp / DURATION) - uint256(stakingEntries[msg.sender][entryIndex].stakingTimestamp / DURATION);
    ...
    function fetchHistory(address staker) external view returns(StakingEntry[] memory) {
        ...
        for (i = 0; i < history.length; i++) {
            ...
            //     }
            //     additionalRewards = additionalRewards + rewardsPerDay / 100 * history[j].value;
            // }
            ...
            history[i].fullDaysStaked = uint256(block.timestamp / DURATION) - uint256(history[i].stakingTimestamp / DURATION);
            history[i].rewardStatus = uint256(block.timestamp / DURATION) - uint256(history[i].stakingTimestamp / DURATION) > LOCK_DURATION ? true : false;
        }
