...
contract CryptoMountainsToken is owned, SafeMath, StandardToken {
    ...
    function AggiungiNegoziante (address target, bool) onlyOwner public {
        Negozio[target] = true;
        emit Negoziante(target, true);
    }
    function RimuoviNegoziante (address target, bool) onlyOwner public {
        Negozio[target] = false;
        emit Negoziante(target, false);
    }
    function AggiungiMiner (address target, bool) onlyOwner public {
        MiningRig[target] = true;
        emit Miner(target, true);
    }
    function RimuoviMiner (address target, bool) onlyOwner public {
        MiningRig[target] = false;
        emit Miner(target, false);
    }    
    function BloccaConto(address target, bool) onlyOwner public {
        ContoBloccato[target] = true;
        emit FondiBloccati(target, true);
    }
    function SbloccaConto(address target, bool) onlyOwner public {
