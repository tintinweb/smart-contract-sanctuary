...
contract bettingInu is ERC20("WOOINU used for betting", "BINU"), ERC20Snapshot{
    ...
    * @param from: The user to check for
    ...
    */
    function checkForLock(address from, uint256 amount) internal view {
        ...
        if (fromLock.amount != 0){
        unchecked{
        require(balanceOf(from) - fromLock.amount >= amount, "BINU: You can only transfer unlocked BINU");
        }
        }
    }
    ...
    */
    function deposit(uint256 amount) external {
        address sender = _msgSender();
        // Unlocks if it hasn't already. This would ensure that its not renewing already unlocked tokens
    ...
    */
    function unlock() external {
        address sender = _msgSender();
        if (_locks[sender].endDate <= block.timestamp && _locks[sender].endDate != 0){
            delete _locks[sender];
        }else {
            revert("BINU: Nothing to unlock");
        }
    }
