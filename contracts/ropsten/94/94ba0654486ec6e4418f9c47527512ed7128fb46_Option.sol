...
contract Option is IOption, Initializable, ReentrancyGuard, Ownable {
  ...
  bytes4 private constant ZOOMERC20_INIT_SIGNITURE = bytes4(keccak256("initialize(string,uint8)"));
  ...
  function initialize (
      ...
      option.optionTokenDecimals = _optionTokenDecimals;
  ...
  function _createZoomToken(
    ...
    bytes memory initData = abi.encodeWithSelector(
        ZOOMERC20_INIT_SIGNITURE,
        string(abi.encodePacked(option.name, "_", _suffix)),
        _decimals
    );
