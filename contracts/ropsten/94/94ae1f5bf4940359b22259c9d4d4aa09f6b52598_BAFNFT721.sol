...
contract BAFNFT721 is ERC721 {
    ...
    function verifySign(address sender, Sign memory sign) internal view {
        bytes32 hash = keccak256(abi.encodePacked(this,sender));
    ...
    function createCollectible( uint256 fee, uint256 numToken , Sign memory sign ) public payable returns(bool) {
        uint256 newItemId = tokenCounter;
        require(!sale , "sale has not started");
        require(supply < totalSupply_, "All tokens have been sold already.");
        require(supply + newItemId <= totalSupply_,"There aren't this many tokens left.");
        verifySign(msg.sender, sign);
        uint256 costForMinting = getCostForMinting(newItemId);
        require(msg.value >= costForMinting,"Too little sent, please send more eth.");
    ...
    function _mint(uint256 _numToMint, uint256 fee) internal {
        
        require(_numToMint <= MAX_MINTABLE_AT_ONCE, "Minting too many at once.");

        uint256 updatedNumAvailableTokens = remainingSupply;
        for (uint256 i = 0; i < _numToMint; i++) {
            uint256 newTokenId = useRandomAvailableToken(_numToMint, i);
            _safeMint(msg.sender, newTokenId, fee);
            //_setTokenURI(newTokenId, tokenURI);
            updatedNumAvailableTokens--;
        }
        remainingSupply = updatedNumAvailableTokens;
    }

    function getCostForMinting(uint256 tokenId) public  view returns (uint256) {
        
        require( supply + tokenId <= totalSupply_, "There aren't this many tokens left.");

        uint256 _cost;
        uint256 _index;

        for (_index; _index < tokenId; _index++) {
          _cost += getCostForMintingPhunk(totalSupply_ + _index);
        }
        return _cost;
    }
    
    function getCostForMintingPhunk(uint256 _numMinted) private pure returns (uint256){
    ...
    returns (uint256)
  {
    ...
    uint256 randomIndex = randomNum % remainingSupply;
