...
contract ERC20 is IERC20, Ownable {
    ...
    function internalReleaseDate(address _account) public view returns(uint) {
        require(_account == _msgSender() || isOwner(), "You dont have right to use this function");
        return internalRecordKeeper.getLockup(_account);
    }
    ...
    function availableForTransfer(address _owner) public view returns (uint256) {
        ...
        if( now < getReleaseTimeStampForDays(internalReleaseDate(_owner))) {
 ...
 */
contract XPLLTokenCrowdsale is MintedCrowdsale, TimedCrowdsale {
    constructor (
        uint256 openingTime,
        uint256 closingTime,
        uint256 rate,
