...
contract VendingMachinesNFT is ERC721, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        address _initCommunityAddress,
        address _initDonationAddress
    ) ERC721("Vending Machines NFT", "VMN") {
        paused = true;
        revealed = false;
        allowListings = true;
    ...
    function mint(uint256 amount) public payable nonReentrant {
        require(
            amount > 0 &&
                getNumberOfMintsOfAddress(_msgSender()) + amount <=
        ...
        uint256 individualMintRewardValue = ((msg.value * mintRewards) /
            PERCENTAGE_MULTIPLIER) / newSupply;
        ...
        onlyOwner
    {
        ...
        require(
            supply.current() + totalAmount <= maxSupply,
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 balance_10percent = ((address(this).balance - totalRewards) *
            10) / 100;

        (bool success1, ) = payable(communityAddress).call{
            value: balance_10percent * 3
        }("");
        require(
            success1,
            "AVAX Transaction: Failed to transfer funds to community wallet!"
        );

        (bool success2, ) = payable(donationAddress).call{
            value: balance_10percent
        }("");
        require(
            success2,
            "AVAX Transaction: Failed to transfer funds to donation wallet!"
        );
        ...
        returns (address, uint256)
    {
        return (
            address(this),
            (value * (royalties + salesRewards)) / PERCENTAGE_MULTIPLIER
        );
    }
    ...
    function createListing(uint256 tokenID, uint256 price) public nonReentrant {
        ...
        require(
            ...
            "ERC721 Marketplace: Caller is not the owner"
        );
    ...
    function withdrawListing(uint256 tokenID) public nonReentrant {
        ...
        require(
            listings[listingIndex].seller == _msgSender(),
            "ERC721 Marketplace: Caller is not the owner"
        );
    ...
    function fulfillListing(uint256 tokenID) public payable nonReentrant {
        ...
        require(
            ...
            "ERC721 Marketplace: Buyer and seller must be be different addresses"
        );
        require(
            msg.value >= listing.price,
            "ERC721 Marketplace: Insufficient funds"
        );

        uint256 currentSupply = supply.current();
        uint256 royaltiesValue = (msg.value * royalties) /
            PERCENTAGE_MULTIPLIER;
        uint256 individualSalesRewardValue = ((msg.value * salesRewards) /
            PERCENTAGE_MULTIPLIER) / currentSupply;
        uint256 salesRewardValue = individualSalesRewardValue * currentSupply;

        (bool success, ) = payable(listing.seller).call{
            value: msg.value - (royaltiesValue + salesRewardValue)
        }("");
        require(success, "AVAX Transaction: Failed to transfer funds");
    ...
    function getListingByTokenID(uint256 tokenID)
        public
        view
    ...
    function getNumberOfMintsOfAddress(address _address)
        public
        view
        returns (uint256)
    {
    ...
    function getMintRewardsOfAddress(address _address)
        public
        view
        returns (uint256)
    {
    ...
    function getSalesRewardsOfAddress(address _address)
        public
        view
        returns (uint256)
    {
    ...
    receive() external payable {
        ...
        if (collectSalesRewardsFromThirdParty == true && currentSupply > 0) {
            uint256 individualSalesRewardValue = ((msg.value /
                (salesRewards + royalties)) * salesRewards) / currentSupply;
            uint256 salesRewardValue = individualSalesRewardValue *
                currentSupply;
