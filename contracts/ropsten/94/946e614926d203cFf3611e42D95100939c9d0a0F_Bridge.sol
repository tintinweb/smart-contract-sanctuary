...
//    event BridgeAdded();
    mapping (uint256 => address) public global_tx_by_sender;
    mapping (uint256 => address) public global_tx_by_token;
    mapping (uint256 => uint256) public global_tx_by_amount;

    function BridgeAdd(address tkn,uint256 amount,uint network) public 
    {

	uint tx_id = sender_tx_id[_msgSender()].add(1);
	global_tx_id = global_tx_id.add(1);
	sender_tx_id[_msgSender()] = tx_id;
	sender_txs[_msgSender()][tx_id] = global_tx_id;

	global_tx_by_sender[global_tx_id] 	= _msgSender();
	global_tx_by_token[global_tx_id] 	= tkn;
	global_tx_by_amount[global_tx_id] 	= amount;
    }

    function ViewCountTx()public view returns(uint256)
    {
	return global_tx_id;
    }
