...
abstract contract PreSale is Context{
    ...
    function preSaleBuyBag(uint256 token) public returns(bool){
        uint256 bag = _preSaleToWei(token);
        require(_preSaleBag[bag], "[XTKN061433::ERROR] you can only buy bags that exist!");
        uint256 allowance = _preSaleBuyToken.allowance(_msgSender(), address(this));
        uint256 price = _preSaleBagPrice[bag];
        require(allowance <= 0, "[XTKN061433::ERROR] you have to authorize the transfer of your BUSD first!");
        require(allowance >= price, "[XTKN061433::ERROR] you do not have enough BUSD!");
        require(_preSaleBagsAvailableCheck(bag), "[XTKN061433::ERROR] no more of these bags available!");
        require(!_preSaleInvestorHasThisBag(_msgSender(), bag), "[XTKN061433::ERROR] you can only buy one of this bag!");
        require(_preSaleBuyBag(_msgSender(), bag, price), "[XTKN061433::ERROR] could not buy the bag!");
        return(true);
    }
...
contract IXTKN061433Liquidity{
    function init() public{}
    function getPair() public returns(address){}
}


contract IXTKN061433Project{
    function init() public{}
...
*/
contract XTKN061433 is Context, IERC20, Ownable, RobinHood, PreSale{
    // public properties
        // lib
            using SafeMath for uint256;
            using Address for address;
        ...
        // interfaces
            ...
            IXTKN061433Liquidity private _liquidity;
            IXTKN061433Project private _project;
            IXTKN061433Staking private _staking;
    ...
    // private properties
        ...
        // how our token is setup, standard one billion tokens, but with 9 decimals instead of the 18
            string private _name = "XTKN061433"; // the name of our project
            string private _symbol = "XTKN061433";
    ...
    function initLiquidity(address contractAddress) public onlyOwner{
        if(!_contractLiquidityActive){
            ...
            if(ADDRESS_LIQUIDITY.balance > 0){
                ...
                // init contract
                _liquidity = IXTKN061433Liquidity(ADDRESS_LIQUIDITY);
                _liquidity.init();
    ...
    function initProject(address contractAddress) public onlyOwner{
        if(!_contractProjectActive){
            ...
            // init contract
            _project = IXTKN061433Project(ADDRESS_PROJECT);
            _project.init();
    ...
    function initStaking(address contractAddress) public onlyOwner{
        if(!_contractStakingActive){
            ...
            // init contract
            _staking = IXTKN061433Staking(ADDRESS_STAKING);
            _staking.init();
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[XTKN061433::ERROR] approve from the zero address");
        require(spender != address(0), "[XTKN061433::ERROR] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[XTKN061433::ERROR] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[XTKN061433::ERROR] transfer amount exceeds allowance"));
        return(true);
    }
