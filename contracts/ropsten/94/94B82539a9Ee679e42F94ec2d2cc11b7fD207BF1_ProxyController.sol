...
contract ProxyController is AccessControl, IProxyController {
    ...
    /**
     * @dev See {IProxyController-startOrderBehalf}.
     *
     */
    function startOrderBehalf(address borrower, uint256 amountIn, uint256 priceA, uint256 priceB, OrderInput memory input) public virtual {
        address borrower_ = _msgSender();
        OrderInput memory input_ = input;

        if (borrower != address(0)) {
            input_.borrower = borrower_;
        }

        startOrder(amountIn, priceA, priceB, input_);
    }
