...
contract Campaign is Ownable, SafeMath {
  ...
  mapping(address => string) rewardedRecords;
  ...
  function claimReward(address _owner, string _reason) public isOpening returns (bool) {
    ...
    if (result) {
      ...
      safeAdd(claimedAmount, rewardAmountPerClaim);
