...
import "./utils/BuyBackToken.sol";
import "./utils/SwapWithLP.sol";

contract DT is Context, Ownable, IERC20, LockableSwap, EthReflectingToken, BuyBackToken {
...
//    event UpdateTransferFees(Fees oldFees, Fees newFees);
//    event UpdateBuyerFees(Fees oldFees, Fees newFees);
    event UpdateSellerFees(SellFees oldSellFees, SellFees nesSellFees);
...
//    SellFees private zeroFees = SellFees(0, 0, 0, 0);
    ...
    uint256 sellToProject = 40;
    uint256 sellToEthReflections = 40;

    uint256 public _minimumTimeFee = 5;
    uint256 public _minimumSizeFee = 5;

    // Fees are ordered highest to lowest, Level1 represents the highest fees possible and most impactful boundaries
    function initSellFees() external onlyOwner {
    ...
    string public name = "Dynamics Token";
    string public constant symbol = "DYNA";
    uint256 public constant decimals = 18;

    bool public sniperDetection = true;
//    uint256 private sniperMaxTxLimit = 1000 * 10 ** _decimals;
...
//        initSellFees(); // TODO - This guy is too much for the constructor
...
//        }
    ...
    function removeAllFee() private {
        ...
        buyFees = Fees(0, 0, 0, 0, 0, 0);
        transferFees = Fees(0, 0, 0, 0, 0, 0);
        sellFees[FeeLevels.LEVEL1] = SellFees(0, 0, 0, 0);
        sellFees[FeeLevels.LEVEL2] = SellFees(0, 0, 0, 0);
        sellFees[FeeLevels.LEVEL3] = SellFees(0, 0, 0, 0);
        sellFees[FeeLevels.LEVEL4] = SellFees(0, 0, 0, 0);
        sellFees[FeeLevels.LEVEL5] = SellFees(0, 0, 0, 0);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router)) {  // Buy
                (rAmount, tTransferAmount, rTransferAmount) = calculateBuy(from, to, amount);
            } else if(from != uniswapV2Pair && from != address(uniswapV2Router) && to == uniswapV2Pair) {  // Sell
                (rAmount, tTransferAmount, rTransferAmount) = calculateSell(from, to, amount);
            } else {  // Transfer
                (rAmount, tTransferAmount, rTransferAmount) = calculateTransfer(from, to, amount);
            }
            if(!inSwapAndLiquify && from != uniswapV2Pair)
                selectSwapEvent();

        } else {
            (rAmount, tTransferAmount, rTransferAmount) = calculateForOwner(from, to, amount);
        }
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(!distributionRoundCompleted || nextRoundStartTime > block.timestamp){
            // TODO
        }

    }

    // This function is kinda redundant considering how fees are handled but it solves stack too deep compilation errors
    // so rather than rewrite the _transfer function in its entirety it is easier to add this and keep the calculations
    // consistent with how the buy/sell/transfer ones are written.
    function calculateForOwner(address from, address to, uint256 amount) private view returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
    ...
    function calculateBuy(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
    ...
    function calculateSell(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        if(block.timestamp > trader[from].lastBuy.add(sellFees[FeeLevels.LEVEL5].saleCoolDownTime)) {
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees[FeeLevels.LEVEL1].saleCoolDownTime)) {
            timeBasedFee = sellFees[FeeLevels.LEVEL1].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees[FeeLevels.LEVEL2].saleCoolDownTime)) {
            timeBasedFee = sellFees[FeeLevels.LEVEL2].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees[FeeLevels.LEVEL3].saleCoolDownTime)) {
            timeBasedFee = sellFees[FeeLevels.LEVEL3].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees[FeeLevels.LEVEL4].saleCoolDownTime)) {
            timeBasedFee = sellFees[FeeLevels.LEVEL4].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees[FeeLevels.LEVEL5].saleCoolDownTime)) {
            timeBasedFee = sellFees[FeeLevels.LEVEL5].saleCoolDownFee;
        }

        uint256 finalSaleFee = _minimumSizeFee;
        uint256 poolSize = amountInPool();
        if(amount < poolSize.mul(sellFees[FeeLevels.LEVEL5].saleSizeLimitPercent).div(100)){
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(amount > poolSize.mul(sellFees[FeeLevels.LEVEL1].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees[FeeLevels.LEVEL1].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees[FeeLevels.LEVEL2].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees[FeeLevels.LEVEL2].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees[FeeLevels.LEVEL3].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees[FeeLevels.LEVEL3].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees[FeeLevels.LEVEL4].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees[FeeLevels.LEVEL4].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees[FeeLevels.LEVEL5].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees[FeeLevels.LEVEL5].saleSizeLimitPrice;
        }

        if (finalSaleFee < timeBasedFee) {
            finalSaleFee = timeBasedFee;
        }
        (tTransferAmount, tFee, tLiquidity, tOther) = _getTValues(amount, 0, 0, finalSaleFee);
        (rAmount, rTransferAmount, rFee) = _getRValues(amount, tFee, tLiquidity, tOther, _getRate());
        ...
        uint256 buyBackQty = saleFeeQty.sub(teamQty.add(ethRedisQty));
        _takeProjectFees(teamQty, 0);
        _takeEthBasedFees(ethRedisQty, buyBackQty);
    }

    function calculateTransfer(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        uint256 tFee; uint256 tLiquidity; uint256 tOther; uint256 rFee;
        trader[to].lastBuy = block.timestamp;
    ...
    function _transferStandard(address sender, address recipient, uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount) private {
        ...
        _registerTransfer(sender, rAmount, recipient, rTransferAmount);
    ...
    function updateBuyerFees(uint256 reflectionFee, uint256 projectFee, uint256 liquidityFee, uint256 burnFee, uint256 marketingFee, uint256 ethReflectionFee) public onlyOwner {
        ...
        buyFees = Fees({
            ...
            ethReflection: ethReflectionFee
        });
    ...
    function updateSellerFees(uint8 _level, uint256 upperTimeLimitInHours, uint256 timeLimitFeePercent, uint256 saleSizePercent, uint256 saleSizeFee ) public onlyOwner {
        ...
        FeeLevels level = FeeLevels(_level - 1);
        SellFees memory oldSellFees = sellFees[level];
        ...
        emit UpdateSellerFees(oldSellFees, sellFees[level]);
    }
...
//        emit SwapAndLiquifyEnabledUpdated(_enabled);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal {
        ...
        // swap tokens for ETH
        SwapWithLP.swapTokensForEth(uniswapV2Router, address(this), half); // <- this breaks the ETH ->  swap when swap+liquify is triggered
        ...
        SwapWithLP.addLiquidity(uniswapV2Router, address(this), otherHalf, newBalance);

        emit SwapAndLiquify(half, newBalance, otherHalf);
    }

    function swapEthBasedFees(uint256 reflections, uint256 buyback) private returns(uint256 ethForReflections, uint256 ethForBuyback){
        uint256 initialBalance = address(this).balance;
        _approve(address(this), address(uniswapV2Router), reflections.add(buyback));
        // swap tokens for ETH
        SwapWithLP.swapTokensForEth(uniswapV2Router, address(this), reflections.add(buyback)); // <- this breaks the ETH ->  swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        if(buyback > 0){
            ethForReflections = newBalance.mul(reflections).div(buyback);
            ethForBuyback = newBalance.sub(ethForReflections);
        } else {
    ...
    function updateRouter(address newAddress) public onlyOwner {
        ...
        emit SwapWithLP.UpdateRouter(newAddress, address(uniswapV2Router));
    ...
    event BuyBackRangeUpdated(uint256 oldMinBuyback, uint256 newMinBuyback, uint256 oldMaxBuyback, uint256 newMaxBuyback);
    ...
    function buyBackTokens(uint256 amount) internal {
        if (amount > minBuyBack && amount < address(this).balance) {
            if(amount > maxBuyBack)
                amount = maxBuyBack;
            SwapWithLP.swapETHForTokens(uniswapV2Router, address(this), amount, buybackSinkAddress);
...
import "./AutomatedExternalReflector.sol";
import "./Ownable.sol";

abstract contract EthReflectingToken is Ownable, ISupportingExternalReflection {

    AutomatedExternalReflector public reflectionContract;
    bool internal distributionRoundCompleted = true;
    uint256 internal nextRoundStartTime;
    uint256 internal roundDelay;

    function takeEthReflection(uint256 amount) internal virtual {
        if(amount > 0)
            takeEthReflection(amount);
    }

    function _takeEthReflection(uint256 amount) private {
        if(address(this).balance >= amount)
            reflectionContract.depositEth{value: amount}();
    ...
    function setReflectorAddress(address payable _reflectorAddress) external override onlyOwner {
        ...
        reflectionContract = AutomatedExternalReflector(_reflectorAddress);
    }

    function _registerTransfer(address from, uint256 senderAmount, address to, uint256 recipientAmount) internal {
        reflectionContract.logTransactionEvent(from, senderAmount, to, recipientAmount);
    }

    function _updateTotalSupply(uint256 newTotalSupply) internal {
        reflectionContract.updateTotalSupply(newTotalSupply);
    }

    function updateRoundDelay(uint256 delayTimeInHrs) external override onlyOwner {
        require(delayTimeInHrs != roundDelay, "Delay already set to this time");
        roundDelay = delayTimeInHrs * 1 hours;
    }
}
...
import "./Ownable.sol";
import "./SwapWithLP.sol";

contract BuyBackToken is Ownable {
...
interface ISupportingExternalReflection {
    ...
    function updateRoundDelay(uint256 delayTimeInHrs) external;
}
...
import "../interfaces/ISupportingExternalReflection.sol";
import "../interfaces/IExternalReflection.sol";

abstract contract  AutomatedExternalReflector is IExternalReflection {

    receive() external payable {}

    function depositEth() external payable override {}

    function depositToken(address tokenAddress, uint256 amount) external override {

    }

    function logTransactionEvent(address from, uint256 senderAmount, address to, uint256 recipientAmount) external override {}
    function reflectRewards() external override returns (bool allComplete) {
        allComplete = false; // PlaceHolder
    }
    function getRemainingPayeeCount() external view override returns(uint256 count) {
        count = 0; // PlaceHolder
    }
    function enableReflections(bool enable) external override {}
    function setReflectedToken(address tokenAddress) external override {}
    function setReflectedTokenToEth() external override {}
    function isExcludedFromDividend() external view override returns(bool excluded) {
        excluded = false; // PlaceHolder
    }
    function excludedFromDividend(address target, bool excluded) external override {}
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

interface IExternalReflection {
    function depositEth() external payable;
    function depositToken(address tokenAddress, uint256 tokenAmount) external;
    function logTransactionEvent(address from, uint256 senderAmount, address to, uint256 recipientAmount) external;
    ...
    function setReflectedToken(address tokenAddress) external;
    function setReflectedTokenToEth() external;
    function isExcludedFromDividend() external view returns(bool excluded);
    function excludedFromDividend(address target, bool excluded) external;
