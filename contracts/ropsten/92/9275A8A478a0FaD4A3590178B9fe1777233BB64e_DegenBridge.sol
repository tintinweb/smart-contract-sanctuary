...
contract DegenBridge is Ownable {
    ...
    */
    ...
    function swap(address tokenA, address tokenB, uint256 amount, address user, uint8 crossOrderType, uint8 dexId, uint256[] memory distribution) 
    external payable notPaused returns (bool) {
        ...
        _nonce = _nonce+1;
        emit SwapRequest(tokenA, tokenB, user, amount, crossOrderType, _nonce, dexId, distribution);
        return true;
    }
    ...
    internal returns(bool) {
        ...
        emit ClaimApprove(path[0], path[path.length-1], user, amount, crossOrderType, dexId, distribution);
