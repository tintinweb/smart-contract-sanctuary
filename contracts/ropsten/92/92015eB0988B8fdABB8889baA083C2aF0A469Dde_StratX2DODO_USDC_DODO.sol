...
// File contracts/interfaces/IDODOExchange.sol 
interface IDODOV2 {
...
//Arbi: 0x88CBf433471A0CD8240D2a12354362988b4593E5
...
//Arbi: 0x36E5238B4479d1ba0bFE47550B0B8e4f4f500EAA
interface IDODODspProxy{
...
interface IDODOERC20Mine{ 
    ...
    function getIdByRewardToken(address rewardToken) external view returns(uint256);
...
// File contracts/StratX2DODO.sol
contract StratX2DODO is StratX2 {
    address  public dodoToken;
    address  public dodoApprove;
    address  public dodoSwapProxy;
    address  public dodoDspProxy;
     
    address[]  dodoToUSDCPath;
    uint256 constant public dodoSwapSlippage = 5;
    bool    public isDodoStablePool; 

    function _initDodo(address _dodoToken, address _approve,address _swapProxy,address _dspProxy,address _dodoUsdc) internal{
        dodoToken = _dodoToken;
        dodoApprove = _approve;
        dodoSwapProxy = _swapProxy;
        dodoDspProxy = _dspProxy;
        dodoToUSDCPath.push(_dodoUsdc);
    }
    function _farm() internal override {
        ...
        IDODOERC20Mine(farmContractAddress).deposit(wantAmt);
    }

    function _unfarm(uint256 _wantAmt) internal override {
        IDODOERC20Mine(farmContractAddress).withdraw(_wantAmt);   
    }

    function inCaseClaimGetStuck() public virtual onlyAllowGov{
       IDODOERC20Mine(farmContractAddress).claimAllRewards();   
    }

    function _dodoSwap(address tokenIn,address tokenOut,address[] memory path,uint256 inAmount) internal returns(uint256){
        require(path.length == 1,"!path==1");
        address dodoV2Pool = path[0];
        //2-Approve DODO to dodoApprove
        IERC20(tokenIn).safeApprove(dodoApprove, 0);
        IERC20(tokenIn).safeIncreaseAllowance(dodoApprove, inAmount);
        //3-Calc minReturnAmount  
        uint256 _minReturnAmount = 1;//DODOV2Proxy02: RETURN_AMOUNT_ZERO

        //4-Swap In Pool 
        address baseToken = IDODODVM(dodoV2Pool)._BASE_TOKEN_();//DVM OR DSP
        //directions: If sellBaseToken represents 0, sellQuoteToken represents 1. 
        uint256 directions = tokenIn == baseToken ? 0 : 1; 
        uint256 deadline = block.timestamp + 60 * 10;
        uint256 returnAmount = IDODOV2Proxy02(dodoSwapProxy).dodoSwapV2TokenToToken(
            tokenIn,
            tokenOut,
            inAmount,
            _minReturnAmount,
    ...
    function _claimRewardToEarnedToken(address rewardToken,address[] memory path) internal returns(uint256){
        IDODOERC20Mine _mine = IDODOERC20Mine(farmContractAddress);
        uint256 _tokenId = _mine.getIdByRewardToken(rewardToken);
        //1-claim  reward token
        uint256 _beforeAmt = IERC20(rewardToken).balanceOf(address(this));
        _mine.claimReward(_tokenId);
        uint256 _afterAmt = IERC20(rewardToken).balanceOf(address(this));
        if(_afterAmt > _beforeAmt){
            uint256 _rewardAmt = _afterAmt.sub(_beforeAmt);
            return _dodoSwap(rewardToken,earnedAddress, path,_rewardAmt);
        }else{
            return 0;
        } 
    }                           

    function _claimToEarnedToken() internal virtual returns(uint256) {
    
    }


    function _addDVMLiquidity(address pool,address token0,address token1,uint256 amount0,uint256 amount1) internal virtual{
        uint256 deadline = block.timestamp + 60 * 10;
        address baseToken  = IDODODVM(pool)._BASE_TOKEN_();
        address quoteToken = IDODODVM(pool)._QUOTE_TOKEN_();
        require((token0 == baseToken && token1 == quoteToken)||
                (token1 == baseToken && token0 == quoteToken),"!token0,!token1");
        uint256 baseAmount  = baseToken == token0 ? amount0 : amount1;
        uint256 quoteAmount = quoteToken == token0 ? amount0 : amount1;
        IDODOV2Proxy02(dodoSwapProxy).addDVMLiquidity(pool,baseAmount,quoteAmount,0,0,0,deadline);
    }

    function _addDSPLiquidity(address pool,address token0,address token1,uint256 amount0,uint256 amount1) internal virtual{
        uint256 deadline = block.timestamp + 60 * 10;
        address baseToken  = IDODODSP(pool)._BASE_TOKEN_();
        address quoteToken = IDODODSP(pool)._QUOTE_TOKEN_();
        require((token0 == baseToken && token1 == quoteToken)||
                (token1 == baseToken && token0 == quoteToken),"!token0,!token1");
        uint256 baseAmount  = baseToken == token0 ? amount0 : amount1;
        uint256 quoteAmount = quoteToken == token0 ? amount0 : amount1;
        IDODODspProxy(dodoDspProxy).addDSPLiquidity(pool,baseAmount,quoteAmount,0,0,0,deadline);
    }

    
    function _addLiquidity(address token0,address token1,uint256 amount0,uint256 amount1) internal virtual{       
        if(isDodoStablePool == false){
            address dvmAddress = wantAddress;
            _addDVMLiquidity(dvmAddress,token0,token1,amount0,amount1);
        }else{
            address dspAddress = wantAddress; 
            _addDSPLiquidity(dspAddress,token0,token1,amount0,amount1);
        }
    }

    function earn() public override nonReentrant whenNotPaused {
       uint256 earnedAmt =   _claimToEarnedToken();
       require(earnedAmt>0,"!earnedAmt>0");
       earnedAmt = distributeFees(earnedAmt);
       earnedAmt = buyBack(earnedAmt);
       // Swap half earned to token0
       if (earnedAddress != token0Address) {
         _dodoSwap(earnedAddress,token0Address,earnedToToken0Path,earnedAmt.div(2));
       }
       // Swap half earned to token0
       if (earnedAddress != token1Address) {
         _dodoSwap(earnedAddress,token1Address,earnedToToken1Path,earnedAmt.div(2));
       }
        // Get want tokens, ie. add liquidity
        uint256 token0Amt = IERC20(token0Address).balanceOf(address(this));
        uint256 token1Amt = IERC20(token1Address).balanceOf(address(this));
        if (token0Amt > 0 && token1Amt > 0) {
            IERC20(token0Address).safeIncreaseAllowance(dodoApprove,token0Amt);
            IERC20(token1Address).safeIncreaseAllowance(dodoApprove,token1Amt);
            _addLiquidity(token0Address,token1Address,token0Amt,token1Amt);
        }

        lastEarnBlock = block.number;
       // Deposits want tokens
       _farm();
    } 
...
// File contracts/StratX2DODO_USDC_DODO.sol
/**
    wantAddress: 0x6a58c68FF5C4e4D90EB6561449CC74A64F818dA5          //DODO-USDC DLP
    farmContractAddress: 0x38Dbb42C4972116c88E27edFacD2451cf1b14255  //IDODOERC20Mine
    earnedAddress: 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8        //USDC： ---DODO--->USDC
*/
contract StratX2DODO_USDC_DODO is StratX2DODO {

    constructor(address _dodoToken, address _approve,address _swapProxy,address _dspProxy,address _dodoUsdc) public{
        govAddress = msg.sender;
        _initDodo(_dodoToken,_approve,_swapProxy,_dspProxy,_dodoUsdc);
    }
    ...
    function _claimToEarnedToken() internal override returns(uint256) { 
       uint256 earnedAmt = _claimRewardToEarnedToken(dodoToken,dodoToUSDCPath); 
       return earnedAmt;
    } 
}