...
contract SigmoidLoan {
    ...
    function getWalletInfo(uint256 _auctionType,address _cur) view public returns(AUCTION[] memory) {
        uint256 len2=0;
        for (uint256 i = 0; i < auctionList.length; i++) {
        ...
        uint256 f = 0;
            for (uint256 j = 0; j < myLoan.length; j++) {
                if(_auctionType == myLoan[j].auctionType && myLoan[j].seller == _cur){
