...
contract Nft is AssetContract, ERC721, Ownable, ERC721Burnable {
    using Address for address;
    using Strings for uint256;
...
/*
    ...
    function prepareTransfer(address from, address to, uint256 tokenId, string memory message) public {
        ...
        _transferPending[tokenId] = TransferStruct(from, to, _msgSender(), message, false, false);
    }

    function acceptTransfer(uint256 tokenId) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId) || _msgSender() == _transferPending[tokenId].to, "NFT7");
