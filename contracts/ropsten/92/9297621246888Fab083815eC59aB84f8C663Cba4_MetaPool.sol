    ...
    Ownable
{
    ...
    event Minted(address minter, uint256 mintAmount, uint256 amount0In, uint256 amount1In);

    event Burned(address burner, uint256 burnAmount, uint256 amount0Out, uint256 amount1Out);

    constructor(string memory _name, IUniswapV3Pool _pool, int24 _initialLowerTick, int24 _initialUpperTick, address _gelato) ERC20(_name) {
        pool = _pool;
    ...
    function mint(uint128 newLiquidity) external returns (uint256 mintAmount) {
        ...
        if (extraAmount0 > 0) {
          TransferHelper.safeTransferFrom(address(token0), msg.sender, address(this), extraAmount0);
        }
        ...
        if (extraAmount1 > 0) {
          TransferHelper.safeTransferFrom(address(token1), msg.sender, address(this), extraAmount1);
        }
        ...
        emit Minted(msg.sender, mintAmount, amount0.add(extraAmount0), amount1.add(extraAmount1));
    }
        ...
        )
    {
        ...
        uint256 extraAmount0 = uint256(burnAmount).mul(token0.balanceOf(address(this))) / _totalSupply;
        if (extraAmount0 > 0) {
          TransferHelper.safeTransfer(address(token0), msg.sender, extraAmount0);
        }
        uint256 extraAmount1 = uint256(burnAmount).mul(token1.balanceOf(address(this))) / _totalSupply;
        if (extraAmount1 > 0) {
          TransferHelper.safeTransfer(address(token1), msg.sender, extraAmount1);
        }
    ...
    function _adjustCurrentPool(
        ...
        if (
            currentLowerTick != newLowerTick ||
            currentUpperTick != newUpperTick
        ) {
