...
contract Lottery is AccessControl {
    ...
    uint ticketPrice = 0.005 ether;  // $20 @ ETH 4k 
    ...
    uint public opsFee = 200;   // marketing wallet
    uint public lFee = 200; // liquidity fee
    uint public affiliateFee = 100;

    uint public n = 1000;   // normalizer

    uint ticketValue;   // dynamic via price feed ~ $20
    ...
    event TicketBought(address indexed user, uint[6] ticket);
    event AutoLiquify(uint256 amountBNB, uint256 amountVICE);
    event SwapBackSuccess(uint256 amount);
    event SwapBackFailed(string message);
    event Winner(address indexed winner, uint[6] indexed ticket, string winType);
    event winnerDrawn (winningTicket ticket);
    event LotteryStarted(uint openTime);

    // min amount of vicex in contract before an LP swap
    uint public decimals = 9;
    uint public viceXSupply = 1000 * 10 ** 9 * (10 ** decimals); // 10 billion;
    uint256 public _maxTxAmount = viceXSupply / 100000; // 0.1%
    uint256 public swapThreshold = viceXSupply / 500000; // 0.02%

    bool public swapEnabled = true;

    constructor (address token, address _dev /*, address _router*/) {
    ...
    // start lotto 
    function start(uint openTime) external onlyAdmin {
        require(!open, 'lotto is closed');
        delta = openTime;
        startTime = block.timestamp;
        open = true;
        emit LotteryStarted(openTime);
    }
    // generate randomness
    // @dev - static for now
    function randomness() external returns(uint8[6] memory){
        uint8[6] memory winner = [0,1,2,3,4,5];
        emit winnerDrawn(winningTicket(winner, epoch));
        return winner;
    }
    ...
    // probably want a min claim for gas
    ...
    function takeFeesBasic(uint amount) internal returns (uint) { // swapping
    ...
    function takeFeesAffiliate(uint amount, address affiliate) internal swapping returns (uint) {
        ...
        uint liquidityFee = amount.mul(lFee);
        // uint divAmount = amount.mul(divFee).div(n);
        // swap rewards amount and send to the staking contract
        // pay dev
        if (balances[dev] == 0) {
    ...
    function shouldSwapBack() internal view returns (bool) {
        return !inSwap
        && swapEnabled
        && vicex.balanceOf(address(this)) >= swapThreshold;
    }

    function swapBack() internal swapping {
        uint256 amountToSwap = swapThreshold.div(2);

        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WETH;

        uint256 balanceBefore = vicex.balanceOf(address(this));

        try router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountToSwap}(
            0,
            path,
            address(this),
            block.timestamp
        ) {
            uint256 amountTokens = vicex.balanceOf(address(this)).sub(balanceBefore);

            try router.addLiquidityETH{ value: amountToSwap }(
                vicexToken,
                amountTokens,
                0,
                0,
                dev,    // LP to dev wallet
                block.timestamp
            ) {
                emit AutoLiquify(amountToSwap, amountTokens);
            } catch {
    ...
    function buyTicket(uint[6] memory ticket) public payable {
        require(msg.value >= ticketPrice, 'Not enough ether sent to buy a ticket');
        require(startTime + delta >= block.timestamp, 'sale is over');
        require(open, 'lottory has not started yet');
        require(ticket.length == 6, 'Incorrect ticket format');   
        require(msg.value >= ticketPrice, 'Not enough ETH sent for ticket fee');
        // add to users
        users.push(msg.sender);
        // @dev - add fee check conditions
        uint jackpotAmount = takeFeesBasic(msg.value);

        tickets[_msgSender()].push(ticket);

        balances[address(this)] = balances[address(this)].add(jackpotAmount);
        emit TicketBought(_msgSender(), ticket);
    }
    ...
    // TODO - add events on wins
    function findWinner(address ticketOwner, uint[6] memory wTicket, uint[6] memory uTicket) internal {
    ...
    function findWinners(uint[6] memory wTicket) external onlyAdmin {   // need better role system
        require(startTime + delta >= block.timestamp, 'sale has not ended');
        require(open, 'lottory has not started yet');
        require(wTicket.length == 6, 'Incorrect ticket format');
        require(!processing, 'Paying winners...please try again later');
        // set lottery to closed while winners are calculated, the lottery has then ended
        open = false;
        ...
        for (uint i = 0; i < users.length - 1; i++) {
            ...
            for (uint j = 0; j < userTickets.length - 1; j++) {
                findWinner(users[i], wTicket, userTickets[i]);
            }
        }
    ...
    function rollOver() external onlyAdmin {
        require(startTime + delta >= block.timestamp, 'sale has not ended');
