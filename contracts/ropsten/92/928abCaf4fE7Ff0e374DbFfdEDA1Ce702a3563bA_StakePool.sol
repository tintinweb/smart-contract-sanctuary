...
contract StakePool is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public totalStaked; // was private before. do we need?
    ...
    struct UserInfo {
        ...
        uint256 rewardsDebt; // variable used to calculate rewardsDebt on user action
        uint256 rewardsAccrued; // variable used to store the rewards accrued and ready for redemption, but not redeemed yet
        uint256 rewardsRedeemed; // variable used to store the total rewards redeemed by user
        uint256 unlockTime; // in hours
        uint256 relockTime; // in hours
        uint256 lastUpdatedAt;
        // string stakingState; // Locked, cooldown time, redemption time..
    }
    ...
    modifier redemptionAllowed(address _user) {
    ...
    modifier updatePool() {
        if (totalStaked > 0) {
            ...
            // console.log("rewardsPerStakedToken", rewardsPerStakedToken);
            // console.log("rewardsDistributionRate", rewardsDistributionRate);
            // console.log("block.number.sub(lastUpdatedAt)", block.number.sub(lastUpdatedAt));
            // console.log("---");
    ...
    function rewardsEarned(address _account) public view returns (uint256) {
        ...
        console.log("userInfo[_account].stakedAmount", userInfo[_account].stakedAmount);
        console.log("rewardsPerStakedToken", rewardsPerStakedToken);
        console.log("userInfo[_account].rewardsDebt", userInfo[_account].rewardsDebt);
        console.log("userInfo[_account].rewardsAccrued", userInfo[_account].rewardsAccrued);
        console.log("userInfo[_account].rewardsRedeemed", userInfo[_account].rewardsRedeemed);

        if (userInfo[_account].stakedAmount > 0) {
            return userInfo[_account].stakedAmount
    ...
    function stake(uint256 _amount) external nonReentrant whenNotPaused updatePool {
        ...
        userInfo[msg.sender].lastUpdatedAt = block.number;
    ...
    function withdraw(uint256 _amount) public nonReentrant whenNotPaused redemptionAllowed(msg.sender) updatePool {
    ...
    function redeemRewards() public nonReentrant whenNotPaused redemptionAllowed(msg.sender) {
