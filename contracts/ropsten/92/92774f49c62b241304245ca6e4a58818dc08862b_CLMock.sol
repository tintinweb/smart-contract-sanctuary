  ...
  Ownable
{
  ...
  string public baseURI = "http://abc.com/index.html?tokenId=";

  uint256 public maxSupply = 100000000;

  string public constant name = "CL MAXX";
  ...
  function mint(
    ...
    onlyOwner
  {
    require(tokens.length < maxSupply,"Exceeds max supply");

    string memory _uri;
    if(bytes(_uri).length == 0){
      _uri = string(abi.encodePacked(baseURI, _uint2str(_tokenId)));
    } 
    super._mint(_to, _tokenId);
  ...
  /**
   * @dev repalce baseURI for a NFT from owner.
   * @param _uri String representing RFC 3986 URI.
   */
  ...
  /**
   * @dev repalce maxSupply for a NFT from owner.
   * @param _maxSupply String representing RFC 3986 URI.
   */
  function setMaxSupply(uint256 _maxSupply) public  onlyOwner  {
   ...
   /**
   * @dev repalce uri for a NFT from owner.
   * @param _tokenId Id for which we want URI.
   * @param _uri String representing RFC 3986 URI.
   */
