...
interface PinkslipNFT is IERC721{
...
contract NftUtils {
    ...
    function tokensOfOwner(PinkslipNFT _token, address _owner, uint256 _limit, uint256 _offset)
        external
        view
        returns(
        ...
        if (tokenCount == 0) {
            ...
            for (tokenId = _offset; tokenId <= maxId; tokenId++) {
                if (_token.ownerOf(tokenId) == _owner) {
                    uint256 carType = PinkslipNFT(_token).carType(tokenId);
