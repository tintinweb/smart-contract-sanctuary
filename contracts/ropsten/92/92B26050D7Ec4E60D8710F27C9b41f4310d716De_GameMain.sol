...
contract GameERC721 is Initializable, ContextUpgradeSafe, ERC165UpgradeSafe, IERC721, IERC721Metadata, IERC721Enumerable, ERC721UpgradeSafe,Governable {
    ...
    function __GameERC721_init(address governor_,string memory name, string memory symbol) external initializer {
		__Governable_init_unchained(governor_);
        __Context_init_unchained();
        __ERC165_init_unchained();
        __GameERC721_init_unchained(name, symbol);
        maxId=0;
    }

    function __GameERC721_init_unchained(string memory name, string memory symbol)  public governance {
        __ERC721_init_unchained(name,symbol);
    }
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view override returns (bool) {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        address owner = ownerOf(tokenId);
        return (spender == owner || spender ==governor|| getApproved(tokenId) == spender || isApprovedForAll(owner, spender));
    }
    

    function mintAuto(address to) public virtual governance {
        maxId++;
        mint(to,maxId);
    }
    

    function mint(address to, uint256 tokenId) public virtual governance {
        maxId = tokenId;
        _safeMint(to, tokenId, "");
    }
...
contract GameMain is Governable,ContextUpgradeSafe,IERC721Receiver {    
   ...
   function __GameMain_init(address governor_) external initializer {
    ...
    function testCreateNFT() public governance {
        ...
        GameERC721 NFT = new GameERC721();
        if (index==0)
            NFT.__GameERC721_init(address(this),"NFT0","NFT0");
        else if (index==1)
            NFT.__GameERC721_init(address(this),"NFT1","NFT1");
        else if (index==2)
            NFT.__GameERC721_init(address(this),"NFT2","NFT2");
        else if (index==3)
            NFT.__GameERC721_init(address(this),"NFT3","NFT3");
        else if (index==4)
            NFT.__GameERC721_init(address(this),"NFT4","NFT4");
        else if (index==5)
            NFT.__GameERC721_init(address(this),"NFT5","NFT5");
        else if (index==6)
            NFT.__GameERC721_init(address(this),"NFT6","NFT6");
        else if (index==7)
            NFT.__GameERC721_init(address(this),"NFT7","NFT7");
        else if (index==100)
            NFT.__GameERC721_init(address(this),"comNFT","NCom");
        if (index<8)
