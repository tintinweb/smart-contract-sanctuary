...
contract NFTToken is ERC721Enumerable, Ownable{
    ...
    function updateLogoURI(string memory _logoURI) public onlyOwner
    {
        logoURI = _logoURI;
    }
    
    function updateFeaturedURI(string memory _featuredURI) public onlyOwner
    {
        featuredURI = _featuredURI;
    }
    
    function updateBannerURI(string memory _bannerURI) public onlyOwner
    {
        bannerURI = _bannerURI;
    }
    
    function updateCollectionName(string memory _collectionName) public onlyOwner
    {
        collectionName = _collectionName;
    }
    
    function updateCustomURI(string memory _customURI) public onlyOwner
    {
        customURI = _customURI;
    }
    ...
    function mint_NFT(string memory tokenUri,string memory tokenName, string memory externalLink, string memory description) public payable returns (uint256){
        _tokenIds.increment();
...
contract collections {
    mapping(address=>address[]) public allCollections;
    constructor(){}
    function createCollection(string memory _logoURI, string memory _featuredURI, string memory _bannerURI, string memory _collectionName, string memory _customURI) external returns (address collection){
        ...
        bytes memory bytecode = type(NFTToken).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_logoURI, _featuredURI, _bannerURI, _collectionName, _customURI));
        assembly {
            collection := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        allCollections[msg.sender].push(collection);
