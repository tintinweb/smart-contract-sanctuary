...
contract GencoinCapital is Context, IERC20, Ownable {
    ...
    uint256 public maxEthSellLaunch = 1 * 10**18;
    ...
    function _transfer(
        ...
        if(!_isMaxWalletExempt[to] && from != owner()){
            if(block.timestamp <= launchedAtTime + 1 hours) {
                ...
                require(amount + balanceOf(to) <= _Launch_Phase2maxWalletToken, "Total holding is limited to 2%");
            }else {
                require(amount + balanceOf(to) <= _maxWalletToken, "Total holding is limited to 4%");
            }
        }
        ...
        if(tradingOpen && to == uniswapV2Pair && from != owner()) { //sell
            ...
            if(block.timestamp <= launchedAtTime + 2 hours) {
                ...
                currenttotalFee = _sellLiquidityFee.add(_sellTreasuryFee).add(_sellMarketingFee).add(_sellTeamFee); //12%
                if(maxEthSellTradeEnabled){
    ...
    function setMaxEthSellLimit(uint256 sellVal) external onlyOwner {
        require(maxEthSell >= 5 * 10**9);
        maxEthSell = sellVal * 10**9;
    }    
