...
contract Lunareum is IBEP20, LPSwapSupport, BuyBack {
    ...
    constructor (uint256 _supply, address _routerAddress, address _tokenOwner, address _marketingAddress) BuyBack() public payable {
        ...
        minTokenSpendAmount = _tTotal.div(1000000);

        _rOwned[_tokenOwner] = _rTotal; // TODO
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if(from == pancakePair){ // Buy
                ...
                (rAmount, tTransferAmount, rTransferAmount) = takeFees(amount, isWhaleSell(amount));
                shouldDoBuyback = shouldAutoBuyback();
            } else {
...
abstract contract LPSwapSupport is Ownable {
    ...
    function swapCurrencyForTokensUnchecked(address tokenAddress, uint256 tokenAmount, address destination) internal {
        _swapCurrencyForTokensAdv(tokenAddress, tokenAmount, destination);
    }
...
abstract contract BuyBack is LPSwapSupport {
    ...
    constructor() internal LPSwapSupport() {
        ...
        buybackWalletPercent = 1; // TODO - For testing
        buybackWalletPercentDivisor = 1;
    }
    ...
    function updateBuybackCriteria(bool basicBuybackEnabled, uint256 buybackTrigger) external onlyOwner { // TODO - Basic buyback only
        isDumbBuyback = basicBuybackEnabled;
        minBuybackTrigger = buybackTrigger;
    }

    function updateWhaleCriteria(uint256 sellPercentage, uint256 percentageDivisor) external onlyOwner {
    ...
    function isWhaleSell(uint256 amount) internal view returns(bool) {
    ...
    function shouldAutoBuyback() internal view returns(bool) {
        return autoBuybackEnabled && ((isDumbBuyback && address(this).balance >= minBuybackTrigger)); // TODO - || !isDumbBuyback && smartBuyback());
    }
    ...
    function smartBuyback() private returns(bool){ // TODO
        return false;
    }

    function autoBuyback() internal { // TODO - Add advanced features here if req.
