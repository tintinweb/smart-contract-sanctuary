...
function requestSwap(address payable _requestor, uint256 _amount) public nonReentrant poolActive
{
    ...
    require(tokenContractAddress.balanceOf(_requestor) >= (_amount * (10 ** uint256(tokenContractAddress.decimals()))));
...
function swapToken(ERC20TokenBurnable _tokenaddr, uint256 _amount) public virtual
{
    require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
    require(_tokenaddr.allowance(msg.sender,address(PoolsRecords[address(_tokenaddr)].tokenPool)) >= _amount);
