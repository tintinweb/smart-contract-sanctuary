...
contract nftMarket is ERC721PresetMinterAutoId {
    ...
    function __offerTokenForSale(uint256 tokenId, uint256 minSalePriceInWei) public {
        //msg.sender must be owner or approved by the owner
        require(_isApprovedOrOwner(msg.sender, tokenId), "market: caller is not owner nor approved");
    ...
    // function __offerTokenForSaleToAddress(uint256 tokenId, uint256 minSalePriceInWei, address toAddress) public {
    //     //msg.sender must be owner or approved by the owner
    ...
    function __tokenNoLongerForSale(uint256 tokenId) public {
        //msg.sender must be owner or approved by the owner
        require(_isApprovedOrOwner(msg.sender, tokenId), "ERC721: caller is not owner nor approved");
    ...
    function __buyToken(uint tokenId) public payable {
        ...
        require(offer.seller == ownerOf(tokenId), "seller no longer owner of token");

        address seller = offer.seller;

        _transfer(seller, msg.sender, tokenId);
        pendingWithdrawals[seller] += msg.value;
        __tokenNoLongerForSale(tokenId);
    ...
    function __enterBidForToken(uint tokenId) public payable {
        require(_exists(tokenId), "market: bid for nonexistent token");
        require(ownerOf(tokenId) != msg.sender, "you already owned this token");
    ...
    // function __withdrawBidForToken(uint tokenId) public {
    //     require(_exists(tokenId), "market: bid for nonexistent token");
    //     require(ownerOf(tokenId) != msg.sender, "you already owned this token");
    //     Bid memory bid = tokenBids[tokenId];
