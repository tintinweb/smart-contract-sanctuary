...
interface IRewardPool  {
    function stakeFor(address holder, uint256 amount)external;

    function pushReward(address recipient, uint256 amount) external;
...
// File: contracts/controllers/RewardPool/RewardPool.sol
...
contract NoMintRewardPool  is IRewardPool, Controllable {
    using SafeMath for uint256;
    ...
    modifier onlyManager() {
        require(msg.sender == manager, "Not a controller");
        _;
    }
    ...
    function stakeFor(address holder, uint256 amount)public updateReward(holder) onlyManager{
    ...
    function withdrawOf(address holder, uint256 amount) internal {
    ...
    // todo： 增加调用约束条件
    function pushReward(address recipient, uint256 amount) public updateReward(recipient) onlyManager{
        withdrawOf(recipient, amount);
