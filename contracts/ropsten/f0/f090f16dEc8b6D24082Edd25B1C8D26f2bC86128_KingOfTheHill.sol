...
contract KOTHPresale is Context, Ownable, Pausable, ReentrancyGuard {
    ...
    function setOriginalReferrerPercentage(uint256 percentage)
        public
        onlyOwner()
    {
        require(
    ...
    function grantReferrer(address account) public onlyOwner() {
        require(
            account != address(0),
            "KOTHPresale: zero address can not be a referrer"
        );
        _referrer[account] = Referrer(true, address(0));
    }

    function migrate(address[] memory addresses, address[] memory parents)
        public
        onlyOwner()
    {
        for (uint256 index = 0; index < addresses.length; index++) {
            _referrer[addresses[index]] = Referrer(true, parents[index]);
        }
    }

    function mintReferrer(address account) public {
        require(
            _referrer[account].isActive == true,
    ...
    function getKOTHAmount(uint256 weiAmount)
        public
        view
        onlyKOTHRegistered()
        returns (uint256)
    ...
    function getPurchasePrice(uint256 tokenAmount)
        public
        view
        onlyKOTHRegistered()
        returns (uint256)
    ...
    // buy with a referrer
    function buyKOTHWithReferrer(address referrer)
        public
        payable
        whenNotPaused()
        nonReentrant()
    ...
    {
        require(
            _referrer[referrer].isActive == true,
            "KOTHPresale: account is not a referrer"
        );
        ...
        require(msg.value > 0, "KOTHPresale: purchase price can not be 0");
        uint256 nbKOTH = getKOTHAmount(msg.value);
        uint256 nbKOTHWithBonus =
            nbKOTH.add(percentageToAmount(nbKOTH, _kothBonusPercentage));
    ...
    function _forwardFunds(address referrer) private {
        ...
        if (isOriginalReferrer(referrer)) {
            parentReward = percentageToAmount(
                msg.value,
                _originalReferrerPercentage
            );
            payable(referrer).transfer(parentReward);
        } else if (isChildReferrer(referrer)) {
            childReward = percentageToAmount(
                msg.value,
                _childReferrerPercentage
            );
            parentReward = percentageToAmount(
                msg.value,
                _originalReferrerPercentage.sub(_childReferrerPercentage)
            );
