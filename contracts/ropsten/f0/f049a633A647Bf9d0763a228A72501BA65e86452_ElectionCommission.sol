...
contract ElectionCommission {
    ...
    function insertNewCandidate(
        ...
        string calldata _lname,
        string calldata _party_name,
        string calldata _party_symbol,
        address candidate_address
    ) external is_ECAdmin {
        candidateContract.insertCandidateData(
            _fname,
            _lname,
            _party_name,
            _party_symbol,
...
contract Candidate {
    ...
    function insertCandidateData(
        ...
        require(
            msg.sender == electionCommissionAddress,
            "Incoming call is not from Election Commission's contract"
        );
    ...
    function getCandidateDataByPartyName(string calldata _party_name)
        public
        view
        returns (candidate_template_data memory _candidate)
    {
        for (uint256 loop = 0; loop < candidateData.length; loop++) {
            if (
                keccak256(bytes(candidateData[loop].party_name)) ==
                keccak256(bytes(_party_name))
            ) {
    ...
    function getCandidateAddressByPartyName(string calldata _party_name)
        public
        view
        returns (address _candidate_address)
    {
        for (uint256 loop = 0; loop < candidateData.length; loop++) {
            if (
                keccak256(bytes(candidateData[loop].party_name)) ==
