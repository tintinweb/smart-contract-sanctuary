...
contract luckDog {
    ...
    string public constant symbol = "LDOG2";
    ...
    function allMemberBonus(
        address payable[] memory _allUsers)
    public
    returns (bool)
    {
        require(_allUsers.length == 0,"not same length");
        ...
        for(uint8 i = 0; i < _allUsers.length; i++){
            ...
            emit Transfer(msg.sender, _allUsers[i],bounsNumber);
        }
        return true;
    }
