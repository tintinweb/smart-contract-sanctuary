...
contract MilkPricer {
    ...
    constructor(
        ...
        require(_bot != address(0), "MilkPricer: Cannot set 0 address as bot");
        require(_oracle != address(0), "MilkPricer: Cannot set 0 address as oracle");
        require(_aggregator != address(0), "MilkPricer: Cannot set 0 address as aggregator");
     ...
     */
    modifier onlyBot() {
        require(msg.sender == bot, "MilkPricer: unauthorized sender");
     ...
     */
    function setExpiryPriceInOracle(uint256 _expiryTimestamp,uint256 price) external onlyBot {
        require(now > _expiryTimestamp, "Not expired");
        oracle.setExpiryPrice(asset, _expiryTimestamp, uint256(price));
    }
 
    function getPrice() external  view returns (uint256) {
        (, int256 answer, , , ) = aggregator.latestRoundData();
        require(answer > 0, "MilkPricer: price is lower than 0");
