    ...
    */
    ...
    function inviteCompetition(address from, address to) public {
        ...
        if (relations[from] == address(0) && !hasRole(ANGEL_ROLE, from)) {
            IERC20Sumswap(summa).transferFrom(from,address(this), invite_amount);
    ...
    function batchAddNode(address[] memory _addr,address[] memory _relation,uint256[] memory _invitor,bool[] memory _angel_role,bool[] memory _initial_role,bool[] memory _invite_role,bool[] memory _node_role,bool[] memory _public_role,bool[] memory _trans_role)   public {
