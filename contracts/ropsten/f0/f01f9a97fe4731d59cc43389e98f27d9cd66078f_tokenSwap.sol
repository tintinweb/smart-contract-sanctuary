   ...
   function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, address _to) external {
    ...
    if (_tokenIn == WETH || _tokenOut == WETH) {
      ...
      path[2] = _tokenOut;
        ...
        IUniswapV3Router(UNISWAP_V3_ROUTER).swapExactTokensForTokens(_amountIn, _amountOutMin, path, _to, block.timestamp);
    }
     ...
     function getAmountOutMin(address _tokenIn, address _tokenOut, uint256 _amountIn) external view returns (uint256) {
       ...
       //the if statement below takes into account if token in or token out is WETH.  then the path is only 2 addresses
        ...
        uint256[] memory amountOutMins = IUniswapV3Router(UNISWAP_V3_ROUTER).getAmountsOut(_amountIn, path);
