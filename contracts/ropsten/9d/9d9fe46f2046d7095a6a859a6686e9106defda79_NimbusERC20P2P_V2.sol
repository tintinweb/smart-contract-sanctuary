...
contract NimbusERC20P2P_V2 is IERC721Receiver {    
    ...
    function createTradeNFToNFT(address[] calldata proposedAsset, uint[] calldata proposedTokenId, address[] calldata askedAsset, uint[] calldata askedTokenId, uint deadline) external returns (uint tradeId) {
    ...
    function createTradeETH(address askedAsset, uint askedAmount, uint deadline) payable external returns (uint tradeId) {
        ...
        tradeId = _createTrade(address(NBU_WETH), msg.value, askedAsset, askedAmount, deadline, 0);   
    }
    function createTradeETHtoNFT(address askedAsset, uint askedAmount, uint deadline) payable external returns (uint tradeId) {
        require(Address.isContract(askedAsset), "NimbusERC20P2P_V1: Not contract");
        NBU_WETH.deposit{value: msg.value}();
        tradeId = _createTrade(address(NBU_WETH), msg.value, askedAsset, askedAmount, deadline, 0);   
    }
    function createTradeNFTtoETH(address proposedAsset, uint tokenId, uint askedAmount, uint deadline) external returns (uint tradeId) {
        require(Address.isContract(proposedAsset), "NimbusERC20P2P_V1: Not contracts");
        IERC721(proposedAsset).safeTransferFrom(msg.sender, address(this), tokenId);
        tradeId = _createTrade(proposedAsset, tokenId, address(NBU_WETH), askedAmount, deadline, 3);   
    }   
    ...
    function supportTradeNFToNFT(uint tradeId) external lock {
    ...
    function supportTradeNFToERC20(uint tradeId) external lock {
        require(tradeCount >= tradeId && tradeId > 0, "NimbusERC20P2P_V1: invalid trade id");
        Trade storage trade = trades[tradeId];
        require(trade.status == 0 && trade.deadline > block.timestamp, "NimbusERC20P2P_V1: not active trade");
        TransferHelper.safeTransferFrom(trade.askedAsset, msg.sender, trade.initiator, trade.askedAmount);
    ...
    function supportTradeETHtoNFT(uint tradeId) external lock {
        require(tradeCount >= tradeId && tradeId > 0, "NimbusERC20P2P_V1: invalid trade id");
        Trade storage trade = trades[tradeId];
        require(trade.status == 0 && trade.deadline > block.timestamp, "NimbusERC20P2P_V1: not active trade");
        IERC721(trade.askedAsset).safeTransferFrom(msg.sender, trade.initiator, trade.askedAmount);
    ...
    function supportTradeNFTtoETH(uint tradeId) payable external lock {
        require(tradeCount >= tradeId && tradeId > 0, "NimbusERC20P2P_V1: invalid trade id");
        Trade storage trade = trades[tradeId];
        require(trade.status == 0 && trade.deadline > block.timestamp, "NimbusERC20P2P_V1: not active trade");
        require(msg.value >= trade.askedAmount, "NimbusERC20P2P_V1: Not enough ETH sent");
    ...
    function cancelTrade(uint tradeId) external lock { 
        ...
        require(trade.status == 0 && trade.deadline > block.timestamp, "NimbusERC20P2P_V1: not active trade");
        trade.status = 2;
        if (trade.proposedAsset != address(NBU_WETH) && trade.tradeType != 2 && trade.tradeType != 3) {
