...
contract DekaApp is IDekaReceiver {
    ...
    address public constant DEKA_TOKEN = 0xcc043fF110ec01beB2e33C2713Af5924A19aB723; // deka.finance - token
    address public constant DEKA_PROTOCOL = 0x0757733c26f1364Ae096E6F21eeE6281789ab7cD; // deka.finance - protocol
...
contract Pool is PoolERC20, IPool {
    ...
    uint256 public reserveDekaAmount;
    ...
    function swapWithFeeRewardDistribution(
        ...
        IERC20(DEKA_TOKEN).safeTransfer(_staker, result);
    }
    ...
    {
        (amountDEKA, amountALT) = _addLiquidity(_amountDEKA, _amountALT, _amountDEKAMin, _amountALTMin);
        liquidity = mintLiquidityTokens(_maker, amountDEKA, amountALT);
        calcNewReserveAddLiquidity(amountDEKA, amountALT);
    }
    ...
    function removeLiquidity(address _maker)
        ...
        override
        onlyFactory
        returns (uint256 amountDEKA, uint256 amountALT)
    {
        (amountDEKA, amountALT) = burn(_maker);
    }
    ...
    function getAPYStake(uint256 _amountIn) public view returns (uint256 result) {
        ...
        uint256 den = (reserveDekaAmount.mul(1000)).add(amountInWithFee);
        result = num.div(den);
    }

    function getAPYSwap(uint256 _amountIn) public view returns (uint256 result) {
        uint256 amountInWithFee = _amountIn.mul(getLPFee());
        uint256 num = amountInWithFee.mul(reserveDekaAmount);
        uint256 den = (reserveAltAmount.mul(1000)).add(amountInWithFee);
        result = num.div(den);
    }

    function getLPFee() public view returns (uint256) {
        uint256 dpy = IDekaProtocol(DEKA_PROTOCOL).getDPY(0);
        return uint256(1000).sub(dpy.div(5e15));
    }
    ...
    function burn(address to) private lock returns (uint256 amountDEKA, uint256 amountALT) {
        uint256 balanceDEKA = IERC20(DEKA_TOKEN).balanceOf(address(this));
        uint256 balanceALT = IERC20(token).balanceOf(address(this));
        uint256 liquidity = balanceOf[address(this)];

        amountDEKA = liquidity.mul(balanceDEKA) / totalSupply;
        amountALT = liquidity.mul(balanceALT) / totalSupply;

        require(amountDEKA > 0 && amountALT > 0, "Pool:: INSUFFICIENT_LIQUIDITY_BURNED");

        _burn(address(this), liquidity);

        IERC20(DEKA_TOKEN).safeTransfer(to, amountDEKA);
        IERC20(token).safeTransfer(to, amountALT);

        balanceDEKA = balanceDEKA.sub(IERC20(DEKA_TOKEN).balanceOf(address(this)));
        balanceALT = balanceALT.sub(IERC20(token).balanceOf(address(this)));

        calcNewReserveRemoveLiquidity(balanceDEKA, balanceALT);
    }
    ...
    function _addLiquidity(
        ...
        uint256 _amountDEKAMin,
        uint256 _amountALTMin
    ) private view returns (uint256 amountDEKA, uint256 amountALT) {
        if (reserveAltAmount == 0 && reserveDekaAmount == 0) {
            (amountDEKA, amountALT) = (_amountDEKA, _amountALT);
        } else {
            uint256 amountALTQuote = quote(_amountDEKA, reserveDekaAmount, reserveAltAmount);
            if (amountALTQuote <= _amountALT) {
                require(amountALTQuote >= _amountALTMin, "Pool:: INSUFFICIENT_B_AMOUNT");
                (amountDEKA, amountALT) = (_amountDEKA, amountALTQuote);
            } else {
                uint256 amountDEKAQuote = quote(_amountALT, reserveAltAmount, reserveDekaAmount);
                require(
                    (amountDEKAQuote <= _amountDEKA) && (amountDEKAQuote >= _amountDEKAMin),
                    "Pool:: INSUFFICIENT_A_AMOUNT"
                );
                (amountDEKA, amountALT) = (amountDEKAQuote, _amountALT);
            }
        }
    }
    ...
    function mintLiquidityTokens(
        ...
        if (totalSupply == 0) {
            liquidity = SafeMath.sqrt(_dekaAmount.mul(_altAmount)).sub(MINIMUM_LIQUIDITY);
            _mint(address(0), MINIMUM_LIQUIDITY);
        } else {
            liquidity = SafeMath.min(
                _dekaAmount.mul(totalSupply) / reserveDekaAmount,
    ...
    function calcNewReserveStake(uint256 _amountIn, uint256 _amountOut) private {
        reserveDekaAmount = reserveDekaAmount.add(_amountIn);
        reserveAltAmount = reserveAltAmount.sub(_amountOut);
    }

    function calcNewReserveSwap(uint256 _amountIn, uint256 _amountOut) private {
        reserveDekaAmount = reserveDekaAmount.sub(_amountOut);
        reserveAltAmount = reserveAltAmount.add(_amountIn);
    }

    function calcNewReserveAddLiquidity(uint256 _amountDEKA, uint256 _amountALT) private {
        reserveDekaAmount = reserveDekaAmount.add(_amountDEKA);
        reserveAltAmount = reserveAltAmount.add(_amountALT);
    }

    function calcNewReserveRemoveLiquidity(uint256 _amountDEKA, uint256 _amountALT) private {
        reserveDekaAmount = reserveDekaAmount.sub(_amountDEKA);
...
//   - dekaMint() - allows to dekaMint an arbitrary amount of DEKA, with the
...
contract PoolERC20 is IERC20 {
    ...
    // bytes32 private constant NAME_HASH = keccak256("DEKA-ALT-LP Token")
    bytes32 private constant NAME_HASH = 0x94b421e6fb7fb3faaad6c4daa2587719b888696e074d1f6809a3139939dad676;
    ...
    string public constant name = "DEKA-ALT-LP Token";
    string public constant symbol = "DKA-ALT-LP";
    ...
    function _validateSignedData(
        ...
        require(recoveredAddress != address(0) && recoveredAddress == signer, "DEKA-ALT-LP Token:: INVALID_SIGNATURE");
    }
    ...
    function _transfer(
        ...
        require(to != address(0), "DEKA-ALT-LP Token:: RECEIVER_IS_TOKEN_OR_ZERO");
        // Balance is implicitly checked with SafeMath's underflow protection
        balanceOf[from] = balanceOf[from].sub(value);
        balanceOf[to] = balanceOf[to].add(value);
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "DEKA-ALT-LP Token:: AUTH_EXPIRED");
    ...
    function transferWithAuthorization(
        ...
        require(block.timestamp > validAfter, "DEKA-ALT-LP Token:: AUTH_NOT_YET_VALID");
        require(block.timestamp < validBefore, "DEKA-ALT-LP Token:: AUTH_EXPIRED");
        require(!authorizationState[from][nonce], "DEKA-ALT-LP Token:: AUTH_ALREADY_USED");
