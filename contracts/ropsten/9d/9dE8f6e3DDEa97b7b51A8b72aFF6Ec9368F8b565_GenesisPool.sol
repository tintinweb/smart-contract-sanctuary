...
contract GenesisPool is MemeTokenWrapper, Ownable {
  ...
  constructor(ERC1155Tradable _memesAddress, IERC20 _memeAddress)
    public
  ...
  function addCards(uint256[] memory cardIds, uint256[] memory amounts)
    public
    onlyOwner
  {
    for (uint256 i = 0; i < cardIds.length; i++) {
      addCard(cardIds[i], amounts[i]);
    }
  }
  ...
  function stake(uint256 amount) public updateReward(msg.sender) {
    require(
      amount.add(balanceOf(msg.sender)) <= 500000000,
      "Cannot stake more than 5 meme"
    );
  ...
  function redeem(uint256 card) public updateReward(msg.sender) {
    ...
    require(
      memes.totalSupply(card) < memes.maxSupply(card),
      "Max cards minted"
    );
