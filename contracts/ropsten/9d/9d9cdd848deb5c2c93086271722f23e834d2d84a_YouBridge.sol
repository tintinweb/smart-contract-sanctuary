...
contract YouBridge is Management {
    ...
    uint256 _nonce = 0;
    address private constant _youToken = 0x1093EcdBACa4168F136f3BEfd17a261bfbbeDdA3;
    ...
    function cancelOrder(uint256 orderId) public lock whenPaused returns (bool)  {
        Order storage order = orders[orderId];
        require(order.state == 1, 'YouSwap:FORBIDDEN');
        require(msg.sender == order.sender, 'YouSwap: MUST_BE_THE_OWNER_OF_THIS_ORDER');
        order.state = 101;
    ...
    function _burnFrom(address account, uint256 amount) private {
        ...
        (bool success, bytes memory data) = _youToken.call(abi.encodeWithSelector(methodId, account, amount));
