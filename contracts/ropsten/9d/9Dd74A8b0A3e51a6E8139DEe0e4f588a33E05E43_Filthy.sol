...
library Minting {
    ...
    function deserializeMintingBlob(bytes calldata blob)
        internal
        pure
        returns (uint256, uint256)
    {
        ...
        uint256 dna = Bytes.toUint(blob[uint256(index) + 1:blob.length - 1]);
        return (tokenID, dna);
    }
}
...
abstract contract Mintable is Ownable, IMintable {
    address public imx;
    mapping(uint256 => uint256) filthyDna;
    ...
    function mintFor(
        ...
        (uint256 tokenId, uint256 dna) = Minting.deserializeMintingBlob(mintingBlob);
        _mintFor(to, tokenId, dna);
        filthyDna[tokenId] = dna;
        emit FilthMinted(to, tokenId, dna);
    }
...
// File: contracts/Filthy.sol


contract Filthy is ERC721, Mintable {
    constructor(
        address _owner,
        string memory _name,
