...
contract GencoinCapital is Context, IERC20, Ownable {
    ...
    address payable private devWallet = payable (0x00126516AFB10D3c7a820192532EbFe44e970289); // Main tax wallet
    ...
    //uint256 public _Launch_Phase1maxWalletToken = _tTotal.div(1000).mul(10); //1%
    //uint256 public _maxWalletToken = _tTotal.div(1000).mul(20); //2% 

    uint256 public _Launch_Phase1maxWalletToken = _tTotal.div(1000).mul(200); //test
    uint256 public _maxWalletToken = _tTotal.div(1000).mul(200); //test

    address[] path;
    bool public maxEthSellTradeEnabled = true;
    uint256 public maxEthSellLaunch = 1 * 10**18; //Max 1 ETH sell for the first 24 hours.
    uint256 public maxEthSell = 5 * 10**18; //Max 5 ETH (24 hours after launch)
    ...
    uint256 public swapThreshold = _tTotal.div(1000).mul(5); //0.1%
    ...
    function _transfer(
        ...
        //max wallet holding
        if(!_isMaxWalletExempt[to] && from != owner()){
            if(block.timestamp <= launchedAtTime + 1 hours) {
                require(amount + balanceOf(to) <= _Launch_Phase1maxWalletToken, "Total holding is limited to 1%");
            }else {
                require(amount + balanceOf(to) <= _maxWalletToken, "Total holding is limited to 2%.");
            }
        }
        ...
        if(tradingOpen && to == uniswapV2Pair && from != owner()) { //sell
            reflectionFee = _sellReflectionFee;

            //get total ETH out
            uint256 _ethBalance = uniswapV2Router.getAmountsOut(amount, path)[1];

            //anti-dump structure for the first 24 hours (Max ETH sell && Higher fee).
            if(block.timestamp <= launchedAtTime + 2 hours) {
                currenttotalFee = 30; //30%
                if(maxEthSellTradeEnabled){
                    require(_ethBalance <= maxEthSellLaunch); //Max sell 1 ETH
                }
                ...
                if(maxEthSellTradeEnabled){
                    require(_ethBalance <= maxEthSell); //Max sell 5 ETH
                }
            }
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path2, address(this), block.timestamp);      
        emit SwapTokensForETH(tokenAmount, path2);
    }
    ...
    function setMaxEthSellEnabled(bool maxEthTrades) external onlyOwner {
        maxEthSellTradeEnabled = maxEthTrades;
    }

    function setMaxEthSellLimit(uint256 sellVal) external onlyOwner {
        require(maxEthSell >= 5 * 10**18);
    ...
    function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
        _maxWalletToken = _tTotal.div(1000).mul(maxWallPercent);
    }
    ...
    function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _marketingFee, uint256 _teamFee) external onlyOwner {
        require(_reflectionFee <= staticTax.maxReflection && _liquidityFee <= staticTax.maxLiquidity && _marketingFee <= staticTax.maxMarketing && _teamFee <= staticTax.maxTeamfee);
