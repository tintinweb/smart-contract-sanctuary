...
contract DTWhitelisted is Context {
    ...
    WhitelistRound[] public _DTWhitelistRounds;

    uint256 public _DTTimestamp;
    address public _DTPairAddress;
    ...
    /*
     * createDTWhitelist - Call this after initial Token Generation Event (TGE)
     *
    ...
    function createDTWhitelist(
        ...
        _DTPairAddress = pairAddress;

        if (durations.length > 0) {
            delete _DTWhitelistRounds;

            for (uint256 i = 0; i < durations.length; i++) {
                WhitelistRound storage wlRound = _DTWhitelistRounds.push();
    ...
    /*
     * modifyDTWhitelistAddresses - Define what addresses are included/excluded from a whitelist round
     *
     * index - 0-based index of round to modify whitelist
     * duration - period in seconds from DT event or previous whitelist round
     * amountMax - max amount (TOKEN decimals) for each whitelist round
     *
     */

    function modifyDTWhitelist(
        uint256 index,
        uint256 duration,
        uint256 amountMax,
        ...
        require(index < _DTWhitelistRounds.length, "Invalid index");
        require(amountMax > 0, "Invalid amountMax");

        if (duration != _DTWhitelistRounds[index].duration)
            _DTWhitelistRounds[index].duration = duration;

        if (amountMax != _DTWhitelistRounds[index].amountMax)
            _DTWhitelistRounds[index].amountMax = amountMax;

        for (uint256 i = 0; i < addresses.length; i++) {
            _DTWhitelistRounds[index].addresses[addresses[i]] = enabled;
        }
    }
    ...
    {
        if (_DTTimestamp > 0) {
            uint256 wlCloseTimestampLast = _DTTimestamp;

            for (uint256 i = 0; i < _DTWhitelistRounds.length; i++) {
                WhitelistRound storage wlRound = _DTWhitelistRounds[i];
    ...
    /*
     * _applyDTWhitelist - internal function to be called initially before any transfers
     *
     */

    function _applyDTWhitelist(
        address sender,
        address recipient,
        uint256 amount
    ) internal {
        if (_DTPairAddress == address(0) || _DTWhitelistRounds.length == 0)
            return;

        if (
            _DTTimestamp == 0 &&
            sender != _DTPairAddress &&
            recipient == _DTPairAddress &&
            amount > 0
        ) _DTTimestamp = block.timestamp;

        if (sender == _DTPairAddress && recipient != _DTPairAddress) {
            //buying

            (uint256 wlRoundNumber, , , , , ) = getDTWhitelistRound();

            if (wlRoundNumber > 0) {
                WhitelistRound storage wlRound = _DTWhitelistRounds[
                    wlRoundNumber.sub(1)
                ];

                require(
                    wlRound.addresses[recipient],
                    "DT - Buyer is not whitelisted"
                );
                ...
                require(
                    ...
                    "DT - Amount exceeds whitelist maximum"
                );
