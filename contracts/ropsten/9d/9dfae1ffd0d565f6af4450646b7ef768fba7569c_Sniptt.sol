...
//import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/1c1ebd765d712977d028450708e06cbff7051b7d/contracts/token/ERC20/ERC20.sol";
...
/**
 * @title ISnipttTokensSender
 * @dev ISnipttTokensSender token transfer hook interface
 */
interface ISnipttTokensSender {
    /**
...
/**
 * @title ISnipttTokensRecipient
 * @dev ISnipttTokensRecipient token transfer hook interface
 */
interface ISnipttTokensRecipient {
    /**
...
/**
 * @notice Partition strategy validator hooks for Sniptt
 */
interface ISnipttPartitionStrategyValidator {
...
library PartitionUtils {
    ...
    {
        return string(abi.encodePacked("SnipttPartitionStrategyValidator", _prefix));
    }
}

/**
 * @title ErrorCodes
 * @notice Sniptt error codes.
 */
contract ErrorCodes {
...
/**
 * @title Sniptt
 * @notice Sniptt is an ERC20 compatible collateral token designed to support
 * multiple classes of collateralization systems.
 * @dev The Sniptt token contract includes the following features:
 *
 * Partitions
 ...
 *
 * Operators
 *   Inspired by ERC777, Sniptt allows token holders to assign "operators" on
 ...
 *
 * Transfers with Data
 *   Inspired by ERC777, Sniptt transfers can include arbitrary data, as well as
 ...
 *   Inspired by ERC777, Sniptt uses the ERC1820 Registry to allow collateral
 ...
 *   Sniptt is able to define certain sets of partitions, identified by a 4 byte
 ...
 * with collateral systems directly through the Sniptt contract.
 */
contract Sniptt is IERC20, ERC1820Client, ERC1820Implementer, ErrorCodes, Ownable {
    ...
    /**
     * @dev SnipttToken interface label.
     */
    string internal constant SNIPTT_INTERFACE_NAME = "SnipttToken";
    ...
    /**
     * @dev SnipttTokensSender interface label.
     */
    string internal constant SNIPTT_TOKENS_SENDER = "SnipttTokensSender";

    /**
     * @dev SnipttTokensRecipient interface label.
     */
    string internal constant SNIPTT_TOKENS_RECIPIENT = "SnipttTokensRecipient";

    /**
     * @dev SnipttTokensChecker interface label.
     */
    string internal constant SNIPTT_TOKENS_CHECKER = "SnipttTokensChecker";

    /**************************************************************************/
    /*************************** Token properties *****************************/

    /**
     * @dev Token name (Sniptt).
     */
    string internal _name;

    /**
     * @dev Token symbol (SNIPTT).
     */
     ...
     * forwarded to upon successfully minting Sniptt.
     */
    address
    ...
    /**
     * @notice Indicates tokens swapped for Sniptt.
     * @dev The tokens that are swapped for Sniptt will be transferred to a
     * graveyard address that is for all practical purposes inaccessible.
     * @param operator Address that executed the swap.
     * @param from Address that the tokens were swapped from, and Sniptt minted for.
     * @param value Amount of tokens swapped into Sniptt.
     */
    ...
    /**
     * @notice Initialize Sniptt, initialize the default partition, and register the
    ...
    constructor(
        ...
        ERC1820Client.setInterfaceImplementation(SNIPTT_INTERFACE_NAME, address(this));
        ERC1820Client.setInterfaceImplementation(ERC20_INTERFACE_NAME, address(this));

        // Indicate token verifies Sniptt and ERC20 interfaces
        ERC1820Implementer._setInterface(SNIPTT_INTERFACE_NAME);
     ...
     * that due to other functionality in Sniptt, this figure should not be used
    ...
    /**************************************************************************/
    /** EXTERNAL FUNCTIONS (SNIPTT) **********************************************/
    /**************************************************************************/

    /******************************** Swap  ***********************************/

    /**
     * @notice Swap tokens to mint SNIPTT.
    ...
    /***************************** Token metadata *****************************/
    /**
     * @notice Get the name of the token (Sniptt).
    ...
    /**
     * @notice Get the symbol of the token (SNIPTT).
    ...
    /********************************* Hooks **********************************/
    /**
     * @notice Check for and call the 'SnipttTokensSender' hook on the sender address
     * (`_from`), and, if `_fromPartition` is within the scope of a strategy,
     * check for and call the 'SnipttPartitionStrategy.tokensFromPartitionToTransfer'
    ...
    function _callPreTransferHooks(
        ...
        senderImplementation = interfaceAddr(_from, SNIPTT_TOKENS_SENDER);
        if (senderImplementation != address(0)) {
            ISnipttTokensSender(senderImplementation).tokensToTransfer(
                msg.sig,
                _fromPartition,
                _operator,
                _from,
        ...
        if (_isPartitionStrategy[fromPartitionPrefix]) {
            ...
            if (fromPartitionValidatorImplementation != address(0)) {
                ISnipttPartitionStrategyValidator(fromPartitionValidatorImplementation)
    ...
    /**
     * @dev Check for 'SnipttTokensRecipient' hook on the recipient and call it.
    ...
    function _callPostTransferHooks(
        ...
        if (_isPartitionStrategy[toPartitionPrefix]) {
            ...
            if (partitionManagerImplementation != address(0)) {
                ISnipttPartitionStrategyValidator(partitionManagerImplementation)
        ...
        recipientImplementation = interfaceAddr(_to, SNIPTT_TOKENS_RECIPIENT);

        if (recipientImplementation != address(0)) {
            ISnipttTokensRecipient(recipientImplementation).tokensReceived(
                msg.sig,
                _toPartition,
                _operator,
                _from,
    ...
    function _callPartitionStrategyOperatorHook(
        ...
        if (strategyValidatorImplementation != address(0)) {
            return
                ISnipttPartitionStrategyValidator(strategyValidatorImplementation)
