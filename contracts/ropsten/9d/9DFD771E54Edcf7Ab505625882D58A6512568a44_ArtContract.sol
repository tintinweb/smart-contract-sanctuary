...
// https://ropsten.etherscan.io/address/0xDad6a6B4816E4006552446c9205f0a8569DC1a9c#code
pragma solidity ^0.8.0;
...
contract ArtContract is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint16[] memory eyes_p, uint8[] memory eyes_a) ERC721("Art", "ART") ERC721Enumerable() Ownable()   ReentrancyGuard() {
        ...
        nonce = 12;
        numPromos = 0;
        splitterAddress = 0xB78F471f963B492Fe0Fe887d0D41eB524c10883E;
    ...
    function startSale() public onlyOwner {
		...
		// tested
    function artificialMint(uint numberOfTokens) public onlyOwner {
        _mintToken(numberOfTokens, owner());
    }

		// tested
    function mintPromo() public onlyOwner {
        _safeMint(owner(), totalSupply());
        createRare(totalSupply() - 1, ++numPromos + NUM_SUPERS);
    }

    function _mintToken(uint numberOfTokens, address sender) internal {
    ...
    function mintToken(uint numberOfTokens) public payable {
        ...
        _mintToken(numberOfTokens, msg.sender);
    }

    function mintThreeTokens() public payable {
        require(saleIsActive, "Sale must be active to mint Duck");
        ...
        _mintToken(3, msg.sender);
    }

    function mintTenTokens() public payable {
        require(saleIsActive, "Sale must be active to mint Duck");
    ...
    function getTraits(uint tokenId) public view returns (uint, uint, uint, uint, uint, uint){
        ...
        require(bytes(IPFS_CIDs[tokenId]).length > 0, "Unauthorized");
        return (duckTraits[tokenId][0], duckTraits[tokenId][1], duckTraits[tokenId][2], duckTraits[tokenId][3], duckTraits[tokenId][4], duckTraits[tokenId][5]);
    }

	  function getHiddenTraits(uint tokenId) public view onlyOwner returns (uint, uint, uint, uint, uint, uint){
        require(_exists(tokenId), "ERC721Metadata: Trait query for nonexistent token");
        return (duckTraits[tokenId][0], duckTraits[tokenId][1], duckTraits[tokenId][2], duckTraits[tokenId][3], duckTraits[tokenId][4], duckTraits[tokenId][5]);
    }
    ...
    function createRare(uint tokenId, uint superId) internal {
        for(uint8 i = 0; i < NUM_TRAITS; i++){
