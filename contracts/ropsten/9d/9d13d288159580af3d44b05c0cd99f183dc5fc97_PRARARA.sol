...
contract PRARARA is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "PRARARA";
    string private constant _symbol = "PRAR";
    ...
    address payable private _marketingAddress = payable(0xf88A39DbCe4Fa69E6ff2Be81412C45bf942bC061);
    ...
    // keeps the bots away
    uint256 public _maxTxAmount = 300000000000 * 10**9; // 0.03%
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !preTrader[from] && !preTrader[to]) {
            ...
            // Trade start check
            if (!tradingOpen) {
                require(preTrader[from], "TOKEN: This contract cannot send tokens, until trading was enabled");
            }
              
            require(amount <= _maxTxAmount, "TOKEN: Maximal transaction limit");
    ...
    // Set maximal transaction amount
