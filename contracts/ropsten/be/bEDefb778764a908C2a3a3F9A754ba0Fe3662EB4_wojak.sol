...
contract wojak is ERC721Enumerable, Pausable, PaymentSplitter {
    ...
    function getPrice() public view returns (uint256) {
        ...
        } else if (block.timestamp >= _saleConfig.startTime + 1 hours) {
            _price = 70000000000000000; //0.07 ETH
			
        } else if (
     		(block.timestamp > _saleConfig.startTime ) &&
            (block.timestamp < _saleConfig.startTime + 1 hours)
        ) {
		 uint256 _minutes=(block.timestamp-_saleConfig.startTime)/60;
		 uint256 _decrease= _minutes*15500000000000000;
            _price = 1000000000000000000-_decrease; 
        } else {
            _price = 70000000000000000; //0.7 ETH
        }
        return _price;
    }
