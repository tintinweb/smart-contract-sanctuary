...
contract TummyToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address charityWalletAddress = 0x012164001DED7AdcFd40dF29AFE3e76D7e23B8E7;  // Needs to be changed
    ...
    string private _name = "TUMMY";
    string private _symbol = "TUMMY";
    ...
    event charityWalletAddressUpdated(address oldAddress, address newAddress);
...
//---------------------------------//
    ...
    function setCharityWalletAddress(address _newAddress) external onlyOwner() {
        address _oldAddress = charityWalletAddress;
        charityWalletAddress = _newAddress;
        emit charityWalletAddressUpdated(_oldAddress, charityWalletAddress);
    }
...
//---------------------------------//
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(tCharity).sub(tBurn);
        return (tTransferAmount, tFee, tLiquidity, tCharity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 currentRate) private view returns (uint256, uint256, uint256) {
