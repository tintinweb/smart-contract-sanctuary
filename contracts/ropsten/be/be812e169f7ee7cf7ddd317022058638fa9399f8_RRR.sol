...
contract RRR is Context, IERC20, Ownable {
    ...
    uint256 public _liquidityFee = 200; // 2% buy fee
    uint256 public _liquidityFeeSell = 300; //3% sell fee
    uint256 public _devFee = 2000; //20% of the distributeed reward
    ...
    uint256 public minBnbBeforeSwap = 2;
    
    uint256 public liquidityAddedAt;

    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address payable public _stackingSCAddress = 0x38c586D61A4440bec6e20E125eA906AF8336Ebea;
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify && sender != uniswapV2Pair) {
            ...
            if (overMinTokenBalance && swapAndLiquifyEnabled) {
                ...
                address payable self = address(this);
                uint256 balance = self.balance;
                if (minBnbBeforeSwap < balance) { //always leave 0.1 BNB
                    uint256 devFee = minBnbBeforeSwap.mul(_devFee).div(10**(_feeDecimal + 2));
                    _owner.transfer(devFee);
                    _stackingSCAddress.transfer(minBnbBeforeSwap.sub(devFee));
                }
            }
        }
    ...
    function swapTokensForBnb() private lockTheSwap {
        ...
        // //collect dev fee when sending
        // bool overMinBnbBalance = address(this).balance > minBnbBeforeSwap;
        // if (overMinBnbBalance) {
        //     uint256 devFee = minBnbBeforeSwap.mul(_devFee).div(10**(_feeDecimal + 2));
        //     uint256 rewards = minBnbBeforeSwap.sub(devFee);
        //     _owner.transfer(devFee);
        //     _stackingSCAddress.transfer(rewards); 
        // }
    }

    function setStackingSCAddress(address payable account) public virtual onlyOwner {
        _stackingSCAddress = account;
    }
    ...
    function setMinBnbBeforeSwap(uint256 amount) public onlyOwner {
        minBnbBeforeSwap = amount;
    }
