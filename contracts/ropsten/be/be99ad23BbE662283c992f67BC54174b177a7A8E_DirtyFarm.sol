    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (_pid == 1) { //$Dirty tokens
                ...
                uint256 tempRewards = pendingRewards(_pid, _msgSender());
                user.rewardDebt = tempRewards; //this puts pending rewards to 0 when rewards are moved from pending to actual
                //user.amount.mul(pool.accDirtyPerShare).div(1e12).sub(tempRewards);
