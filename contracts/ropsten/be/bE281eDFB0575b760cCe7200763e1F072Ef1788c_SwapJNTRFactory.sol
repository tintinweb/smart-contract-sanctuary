...
contract SwapJNTRFactory is Ownable {
    ...
    event SwapRequest(address indexed tokenA, address indexed tokenB, address indexed user, uint256 amount, uint256 nonce);
    ...
    */
    ...
    function swap(address tokenA, address tokenB, uint256 amount, address user) external payable notPaused returns (bool) {
        ...
        _nonce = _nonce+1;
        emit SwapRequest(tokenA, tokenB, user, amount,_nonce);
            
        // }
        return true;
    }

    function _claim(address tokenA, address tokenB, address user, uint256 amount) internal {
        address payable pair = getPair[tokenA][tokenB];
        require(pair > address(9), 'PAIR_NOT_EXISTS');
        // IValidator(validator).checkBalance(pair, foreignPair[pair], user);
        balanceCallback(pair,user,amount);
        emit ClaimRequest(tokenA, tokenB, user,amount);
    }
    // amountB - amount of foreign token to swap
    function claimTokenBehalf(address tokenA, address tokenB, address user,uint256 amount, uint256 nonce) external onlySystem notPaused returns (bool) {
        require(!nonceProcessed[nonce], "Exchange already processed");
        nonceProcessed[nonce] = true;
        _claim(tokenA, tokenB, user,amount);
        return true;
    }
    ...
    function balanceCallback(address payable pair, address user, uint256 balanceForeign) public returns(bool) {
        // require (validator == msg.sender, "Not validator");
        // require (false, "balance Callback called");
        ...
        (tokenA, tokenB) = SwapJNTRPair(pair).claimApprove(user, balanceForeign);
        // if (canMint[tokenA])
        //     IBEP20(tokenA).mint(user, amount);
        // else
        
        // require(false,"error in GatewayVault");
        // IGatewayVault(gatewayVault).vaultTransfer(USDT, system, amount);        
        IGatewayVault(gatewayVault).vaultTransfer(USDT, system, balanceForeign);        
        // emit ClaimApprove(tokenA, tokenB, user, amount);
        emit ClaimApprove(tokenA, tokenB, user, balanceForeign);
        return true;
    }
}