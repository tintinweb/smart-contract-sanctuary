    ...
    function sendReward(address payable to) public afterVotingPeriod {
        ...
        if (winnerVotes > 0) {
            ...
            if (referrer1 != address(0)) {
                _ = reward / 100 * 2;  // 2%
                forwarder.forward.value(_)(referrer1, to);
                cryptoVersusPrize -= _;
                emit ReferrerRewardWasPaid(to, referrer1, _);
            }
            if (referrer2 != address(0)) {
                _ = reward / 1000 * 2;  // 0.2%
                forwarder.forward.value(_)(referrer2, to);
                cryptoVersusPrize -= _;
                emit ReferrerRewardWasPaid(to, referrer2, _);
            }
        ...
        if (loserVotes > 0) {
            if (referrer1 != address(0)) {
                _ = loserVotes / 100 * 1;  // 1%
                forwarder.forward.value(_)(referrer1, to);
                cryptoVersusPrize -= _;
                emit ReferrerRewardWasPaid(to, referrer1, _);
            }
            if (referrer2 != address(0)) {
                _ = loserVotes / 1000 * 1;  // 0.1%
                forwarder.forward.value(_)(referrer2, to);
                cryptoVersusPrize -= _;
                emit ReferrerRewardWasPaid(to, referrer2, _);
            }
