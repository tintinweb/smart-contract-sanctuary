...
contract ConstellePackage is ERC721Enumerable, Member {
    using Strings for uint256;
    ...
    string public uriPrefix =
        "https://storageapi.fleek.co/9ea7501d-0e4d-47e8-b48f-c2c4482bac9a-bucket/blind/";
    ...
    function open(uint256 packageId) external {
        ...
        ConstelleCard card = ConstelleCard(manager.members("card"));
    ...
    function batchOpen(uint256 packageId) external {
        ...
        ConstelleCard(manager.members("card")).batchMint(
            msg.sender,
            cardIdPres
        );
    ...
    {
        return (
            uint32(uint32(tokenId >> 224) ^ uint32(1 << 31)),
            uint64(tokenId >> 160),
            uint16(tokenId >> 144),
            uint40(tokenId >> 104),
            uint40(tokenId >> 64)
...
interface IShop {
    ...
    // function getTypeWeights(uint256 packageId)
    //     external
    //     view
