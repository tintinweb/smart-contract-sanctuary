...
// hevm: flattened sources of src/strategies/strategy-bsgs-dai-lp.sol
...
abstract contract StrategyBase {
    ...
    // send build to this treasury
    address public treasury = 0x5A6eBeB61A80B2a2a5e0B4D893D731358d888583;
...
////// src/strategies/strategy-gold-farm-base.sol

/* pragma solidity ^0.6.7; */

/* import "./strategy-staking-rewards-base.sol"; */

interface BsgsStaking {
    function notifyReward(uint256) external;
}

abstract contract StrategyGoldFarmBase is StrategyStakingRewardsBase {
    // Token addresses
    address public bsgs = 0x99A99C90C71343C3e9D947b97D7bBf7ead721B9D; // mainnet: 0xA9d232cC381715aE791417B624D7C4509D2c28DB;
    address public dai = 0x20854C69Aead08Bc21970B316494f504f99C0619; // mainnet: 0x6B175474E89094C44Da98b954EedeAC495271d0F;
    address public build = 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984;// mainnet: 0x6e36556B3ee5Aa28Def2a8EC3DAe30eC2B208739;

    // DAI/<token1> pair
    address public token1;

    // How much BSGS tokens to keep?
    uint256 public keepBSGS = 1500;
    uint256 public constant keepBSGSMax = 10000;

    // Uniswap swap paths
    address[] public bsgs_build_path;
    address[] public bsgs_dai_path;
    address[] public dai_token1_path;
        ...
        )
    {
        ...
        bsgs_dai_path = new address[](2);
        bsgs_dai_path[0] = bsgs;
        bsgs_dai_path[1] = dai;

        dai_token1_path = new address[](2);
        dai_token1_path[0] = dai;
        dai_token1_path[1] = token1;

        bsgs_build_path = new address[](4);
        bsgs_build_path[0] = bsgs;
        bsgs_build_path[1] = dai;
        bsgs_build_path[2] = weth;
        bsgs_build_path[3] = build;
    }
    ...
    function harvest() public override onlyBenevolent {
        ...
        uint256 _bsgs = IERC20(bsgs).balanceOf(address(this));
        if (_bsgs > 0) {
            // 12% is streamed to staking contract
            uint256 _keepBSGS = _bsgs.mul(keepBSGS).div(keepBSGSMax);
            _swapUniswapWithPath(bsgs_build_path, _keepBSGS);
            // If stakingContract is not set, send to treasury
            IERC20(build).safeTransfer(
                treasury,
                IERC20(build).balanceOf(address(this))
            );

            if (token1 == bsgs) {
                _swapUniswapWithPath(bsgs_dai_path, _bsgs.sub(_keepBSGS).div(2));
            } else {
                _swapUniswapWithPath(bsgs_dai_path, IERC20(bsgs).balanceOf(address(this)));
            }
        }
        ...
        if (token1 != bsgs) {
            ...
            if (_dai > 0) {
                _swapUniswapWithPath(dai_token1_path, _dai.div(2));
            }
        }
...
////// src/strategies/strategy-bsgs-dai-lp.sol

/* pragma solidity ^0.6.7; */

/* import "./strategy-gold-farm-base.sol"; */

contract StrategyBsgsDaiLp is StrategyGoldFarmBase {
    // Token addresses
    address public bsgs_rewards = 0x61f7c36C76Bd0DD5c47a4C48B6Ae97285894D6D3; //mainnet: 0x3B871056E9f13aA3BA5b4dC3f71f00f7dc652199;
    address public uni_bsgs_dai_lp = 0x10b4D1853e570401fb54fd8B45bD8d376103F46E; //mainnet: 0x980a07E4F64d21a0cB2eF8D4AF362a79b9f5c0DA;
    address public token_bsgs = 0x99A99C90C71343C3e9D947b97D7bBf7ead721B9D; // mainnet: 0xA9d232cC381715aE791417B624D7C4509D2c28DB;

    constructor(address _strategist)
        public
        StrategyGoldFarmBase(
            token_bsgs,
            bsgs_rewards,
            uni_bsgs_dai_lp,
            _strategist
