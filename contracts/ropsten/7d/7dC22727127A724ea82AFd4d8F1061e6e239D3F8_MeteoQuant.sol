...
contract MeteoQuant is AccessControl, Initializable {
    ...
    bytes32 public constant MARTIAN = keccak256("MARTIAN");
    ...
    uint24 public voteSupportRate;//default = protocol fee * 16;
    ...
    function setMartin(address _martian) external {
        require(hasRole(MARTIAN, msg.sender) && _martian!=address(0) && !hasRole(MARTIAN, _martian), "MUST MARTIAN");
        _setupRole(MARTIAN, _martian);
    }

    function setDao(address _dao) external {
        require(hasRole(DAO, msg.sender) && _dao!=address(0) && !hasRole(DAO, _dao), "MUST DAO");
        _setupRole(DAO,  _dao);
    }

    ///@dev List the strategy and add the number of support strategies for the strategy
    ///@param _strategyId strategy id
    ///@param _proposalId strategy's proposal id
    function addStrategies(uint256 _strategyId, uint256 _proposalId) external {
        //must timelock or admin
        require(msg.sender == timelock || hasRole(MARTIAN, msg.sender), "MUST TIMELOCK OR MARTIAN");
    ...
    function strategyRewards(uint256 _strategyId, address _voter) external view returns( uint256 rewards){
        ...
        rewards = strategy.rewardsPerVote * forVotes - rewardsDebts[_voter][_strategyId];
    }
    ...
    function claimStrategyRewards( uint256 _strategyId ) external {
        ...
        uint256 rewards = strategy.rewardsPerVote * forVotes - rewardsDebts[msg.sender][_strategyId];

        /// update rewardDebt
        rewardsDebts[msg.sender][_strategyId] = strategy.rewardsPerVote * forVotes;
    ...
    function endProject(uint256 _projectId, uint256 _totalFee, uint256 _strategyFee) external {
        require(hasRole(MARTIAN, msg.sender), "MUST MARTIAN");
        Project storage project = projects[_projectId];
    ...
    function protocolFees(uint256 _amount, Project storage _project) internal{
        ...
        if(strategy.proposalId>0 && strategy.forVotes>0){
            ...
            strategy.rewardsPerVote += _amount * voteSupportRate / 100 / strategy.forVotes;
        }else{
    ...
    function settlement(uint256 _projectId, uint256 _totalFee, uint256 _strategyFee, bool _useForegift, bool _endProjct) public {
        require(hasRole(MARTIAN, msg.sender) || msg.sender == address(this), "MUST MARTIAN");
    ...
    function withdrawTreasury(uint256 _amount, address _receipt) external {
        require(hasRole(DAO, msg.sender), "MUST DAO");
        require(_amount>0 && _receipt != address(0), "INVALID PARAM");
        require( treasuryBalances > 0, "BALANCE ZERO");
    ...
    function withdrawLabs(uint256 _amount, address _receipt) external {
        require(hasRole(DAO, msg.sender), "MUST DAO");
        require(_amount>0 && _receipt != address(0), "INVALID PARAM");
        require( mteLabsBalances > 0, "BALANCE ZERO");
    ...
    function withdrawVoteAgainstRewards(uint256 _amount, address _receipt) external {
        require(hasRole(DAO, msg.sender), "MUST DAO");
        require(_amount>0 && _receipt != address(0), "INVALID PARAM");
        require( voteAgainstRewards > 0, "BALANCE ZERO");
    ...
    /// @dev DAO handles with rewards generated by strategies that did not in voting
    function withdrawVoteSupportRewards(uint256 _amount, address _receipt) external {
        require(hasRole(DAO, msg.sender), "MUST DAO");
        require(_amount>0 && _receipt != address(0), "INVALID PARAM");
        require( voteSupportRewards > 0, "BALANCE ZERO");
...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV1, GovernorBravoEvents {
    ...
    uint public constant MAX_PROPOSAL_THRESHOLD = 10e8;//100000e18; //100,000 Comp
    ...
    uint public constant quorumVotes = 100e8;//400000e18; // 400,000 = 4% of Comp
    ...
    function cancel(uint proposalId) external {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == proposal.proposer || mte.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold, "GovernorBravo::cancel: proposer above threshold");
    ...
    function unlockVoteMte(uint256 proposalId) external returns (uint96){
      ...
      require(vstate != ProposalState.Pending 
        && vstate != ProposalState.Active 
        && vstate != ProposalState.Succeeded 
        && vstate != ProposalState.Queued, 
        "GovernorBravo::castVoteInternal: voting isnot closed");
    ...
    function _initiate() external {
        ...
        proposalCount = 1; //GovernorAlpha(governorAlpha).proposalCount();
