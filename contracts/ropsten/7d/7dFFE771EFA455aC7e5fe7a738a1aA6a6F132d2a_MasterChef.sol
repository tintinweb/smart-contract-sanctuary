...
contract MasterChef is Ownable, ReentrancyGuard, IMasterChef, Constants {
    ...
    bool public wasAdjustedPre;
    bool public wasAdjustedPost;
    bool public wasSaved;
    ...
    */
    ...
    function calculateRewardPool() external override {
        ...
        //test
        wasSaved = false;
        wasAdjustedPost = false;
        wasAdjustedPre = false;
        ...
        for(uint32 i=0; i < totalUserLength; i++){
            ...
            for(uint8 j=0; j < length; j++){
                ...
                if(user.stakedSince > 0){
                    ...
                    if(adjustedAmount > 0 && !wasAdjustedPre){
                        wasAdjustedPre = true;
                    }
                }
            }
            if(adjustedAmount > 0 && !wasAdjustedPost){
                        wasAdjustedPost = true;
                    }
            rewardPoolShare[currentRewardIteration][rewardUser] = adjustedAmount;
            if(rewardPoolShare[currentRewardIteration][rewardUser] > 0 && !wasSaved){
                        wasSaved = true;
                    }
        }
