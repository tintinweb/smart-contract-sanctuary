...
contract IdleFinanceStrategy is  IStrategy, RewardTokenProfitNotifier {
 using SafeMath for uint256;
  ...
  address public multiSig = address(0xF83F4adcA4e5B5E21472C69375b556c827Da68F1);
  ...
  //modifier updateVirtualPrice() {
   // if (protected) {
     // require(virtualPrice <= idleTokenHelper.getRedeemPrice(cToken), "virtual price is higher than needed");
   // }
    //_;
    //virtualPrice = idleTokenHelper.getRedeemPrice(cToken);
 // }
  constructor(
    address _storage,
    address _underlying,
  ...
  */
  function investAllUnderlying() public restricted {
    ...
    CErc20Interface(cToken).mint(balance);
  }
  /**
  ...
  */
  function withdrawAll() internal {
    uint256 balance = IERC20(cToken).balanceOf(address(this));
    // this automatically claims the crops
   // IIdleTokenV3_1(cTokenUnderlying).redeemIdleToken(balance);
    //liquidateRewards();
    CErc20Interface(cToken).redeem(balance);
  }
  function withdrawToVault(uint256 amountUnderlying) public restricted {
    ...
    /**
    uint256 totalcTokenLpTokens = IERC20(cTokenUnderlying).balanceOf(address(this));
    uint256 totalUnderlyingBalance = totalcTokenLpTokens.mul(virtualPrice).div(1e18);
    uint256 ratio = amountUnderlying.mul(1e18).div(totalUnderlyingBalance);
    uint256 toRedeem = totalcTokenLpTokens.mul(ratio).div(1e18);*/
    uint256 balanceBefore = underlying.balanceOf(address(this));
    uint256 decimalOfUnderlying=6;
    uint256 base=10;
    uint256 decimal=18;
    uint256 exchangeRate =base**((decimal.sub(8)).add(decimalOfUnderlying));
    CErc20Interface(cToken).redeem(amountUnderlying.mul((CTokenInterface(cToken).exchangeRateCurrent()).div(exchangeRate)));
 ...
 // function claim() internal {
   // IIdleTokenV3_1(cToken).redeemIdleToken(0);
  ...
  */
  function investedUnderlyingBalance() public view returns (uint256) {
   ...
   // uint256 invested = IERC20(cToken).balanceOf(address(this)).mul(virtualPrice).div(1e18);
