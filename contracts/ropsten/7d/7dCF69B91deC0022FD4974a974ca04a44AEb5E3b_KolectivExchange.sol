...
contract KolectivExchange is Ownable, Pausable, ReentrancyGuard {
    ...
    event Deposit(address indexed account, uint256 amount, uint256 exchangeId);
    event Withdraw(address indexed account, uint256 amount, uint256 exchangeId);
    mapping (uint256 => bool) private depositComplete;
    mapping (uint256 => bool) private withdrawComplete;
    ...
    function deposit(uint256 exchangeId) public payable whenNotPaused {
        ...
        require(depositComplete[exchangeId] == false, "That deposit has already been completed");
        depositComplete[exchangeId] = true;
    ...
    function withdraw(address payable account, uint256 amount, uint256 exchangeId) public onlyOwner nonReentrant whenNotPaused {
        ...
        require(withdrawComplete[exchangeId] == false, "That withdraw has already been completed");
        require(address(this).balance - amount >= reservedBalance, "That withdraw will use reserved funds.");
        withdrawComplete[exchangeId] = true;
