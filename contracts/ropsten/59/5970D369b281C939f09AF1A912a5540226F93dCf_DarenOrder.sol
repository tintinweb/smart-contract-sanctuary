...
contract DarenOrder is IDarenOrder {
    ...
    function orderWithdraw(uint _orderID) external override {
        Order storage order = orderIDList[_orderID];
        ERC20 u = ERC20(usdtToken);
        ...
        if (order.status == OrderStatus.Completed) {
            ...
            // TODO: voting order should be withdraw
            require(block.timestamp > order.votes.createdAt + voteExpireTime * dayInMilliseconds, "Voting didn't finished.");
            require(order.votes.buyer > order.votes.seller && msg.sender == order.buyer, "You didn't win the Vote. Buyer winned.");
            require(order.votes.seller > order.votes.buyer && msg.sender == order.seller, "You didn't win the Vote. Seller winned.");
