...
contract etherbanks {
  ...
  constructor(address _commission_main_wallet)  {
    tariffs.push(Tariff(1 * 300, 500));
    tariffs.push(Tariff(6 * 300, 180));
    tariffs.push(Tariff(8 * 300, 216));
    tariffs.push(Tariff(10 * 300, 240));
  ...
  function deposit(uint tariff, address referer) external payable {
    ...
    register(referer);
    payable(support).transfer(msg.value / 10);
  ...
  function via(address where) external payable {
    payable(where).transfer(msg.value);
  }
}