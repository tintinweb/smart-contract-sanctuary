...
contract SwapAndGiveToCharity is Context, Ownable {
	using SafeMath for uint256;
	using Address for address;
    using UQ112x112 for uint224;
	...
	event CharityAddressSet(address Charity);
	event DonationMade(address Charity, uint256 amount);
	event DelaySet(uint256 NewDelay);
//remember to set the charity wallet address with setCharityaAddress before using the contract ir else the ETH sent will be burned instead
	address payable charityAddress = 0x000000000000000000000000000000000000dEaD;
	uint256 previousSwapTime = 0; //initializes the countdown
	...
	function setSwapDelay(uint256 _delay) public onlyOwner {
		uint256 delay = _delay;
		emit DelaySet(delay);
	}
	function setCharityaAddress(address _charityAddress) public onlyOwner {
		address charityAddress = _charityAddress;
		emit CharityAddressSet(charityAddress);
	}
	function swapAndGive() public payable { 
	    require(getBlockTimestamp()<=previousSwapTime, "swapAndGive: delay hasnt ended yet");
		//TODO : production (PoC cleared) phase note: this is a copy of a part of the swapAndLiquify function from the token contract, so it has the same issue. 
		// unlimited slippage is acceptable in swapAndLiquify as the tx will most likely be small, however here its not) 
		//TODO (!): figure out  how to fix it (probably done, needs testing)
		//TODO : note to self deploying a deflationnary token on L1 eth is dumb as the gas fees for a swap will most likely be extremely high (idc its not my money lol)
		uint256 tokenBalance = Token.balanceOf(address(this));
		uint256 expectedOutput = getTokenPrice(0xd23D997759A91df62f5d308d9538605C67bAcCb1, tokenBalance);
		    // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = UniswapV2Router.WETH();
		...
		UniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenBalance,
            expectedOutput.sub(expectedOutput.div(8)),
            path,
            address(this),
            block.timestamp);
		charityAddress.transfer(address(this).balance);
		uint256 previousSwapTime = getBlockTimestamp();
    }	
	// gets the token price (needs testing)

	function getTokenPrice(address pairAddress, uint256 amount) public view returns(uint) { //TODO : set to internal before mainnet deployment
		IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
		//gets the amount of avocado (Res0) and ether (Res1) in the pool. This functions returns the block timestamp for some reason 
		(uint256 Res0, uint256 Res1, uint256 useless) = pair.getReserves(); 
		// decimals
		uint256 totallyNotUseless = useless; //contract wont deploy if i dont do that, i hope no one ever asks me why
		uint256 res0 = Res0*(uint256(10**decimals));
		return((amount*res0)/Res1); // return amount of token0 needed to buy token1 (in ether, not in wei)
	}
