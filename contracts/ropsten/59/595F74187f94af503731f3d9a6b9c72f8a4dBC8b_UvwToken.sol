    ...
    ERC20Votes
{
    ...
    uint8 constant internal DECIMAL_PLACES = 10;
    ...
    function burnToken(address account, uint256 amount) external onlyRole(BURNER_ROLE) {
    ...
    function takeSnapShot() public returns(uint256 currentID) {
    ...
    /// @param account account whose votes will removed 
    function _slashVotes(address account, uint256 amount) private whenNotPaused {
        _burn(account, amount);
    } 
    ...
    function setRole(string memory role, address add) external onlyRole(DEFAULT_ADMIN_ROLE) {
        bytes32 _role = stringToBytes32(role);
        roles[role] = _role;
        _setupRole(_role, add);
    }

    function revokeRole(string memory role, address revoke) external onlyRole(DEFAULT_ADMIN_ROLE) {
        bytes32 _role = stringToBytes32(role);
        roles[role] = _role;
