...
contract ATST6 is IERC20, Auth {
	using SafeMath for uint256;
	string constant _name = "Test ATST6";
	string constant _symbol = "ATST6";
...
// 	constructor (uint32 _vtr, uint32 _vrs) Auth(msg.sender) {      
    ...
    function _convertCollectedTax(address sender, address recipient) internal {
        uint256 taxTokenBalance = balanceOf(address(this));
		if (inTaxSwap == false) {
			if (_taxSwapOnBuy && (taxTokenBalance > (_totalSupply * _swapTaxOnBuyLimit / 100)) && sender == _uniLpAddr ) {
				_swapTaxTokensForEth(taxTokenBalance);
			} else if (_taxSwapOnSell && taxTokenBalance > 0 && recipient == _uniLpAddr) {
				_swapTaxTokensForEth(taxTokenBalance);
	...
	function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        require(_checkTradingOpen(recipient), "Trading is not open yet");
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
	    _convertCollectedTax(sender, recipient);
        if ( sender != address(this) && recipient != address(this) ) { require(_checkLimits(recipient, amount), "TX exceeds current limits"); }
