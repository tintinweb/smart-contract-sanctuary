...
// token transfers
// ----------------------------------------------------------------------------
contract LVGXStake is Owned {
    using SafeMath for uint256;
    
    address public LVGXLP   = 0x4F9a716C0C5379975C4034a9c9a335D7F0F51258;
    
    address public lpLockAddress = 0xBeEEC84B8D10051b86089EB9138c2A619BC87c08;
   ...
   function setLpLockAddress(address _account) public onlyOwner {
    ...
    function STAKE(uint256 tokens) external {
        require(IERC20(LVGXLP).transferFrom(msg.sender, address(lpLockAddress), tokens), "Tokens cannot be transferred from user for locking");
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        require(IERC20(LVGXLP).transfer(msg.sender, tokens), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedLVGXLP(address staker) public view returns(uint256 stakedLVGXLP){
        require(staker != address(0), "ERC20: sending to the zero address");
    ...
    // ------------------------------------------------------------------------
    // Get the LVGXLP balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourLVGXLPBalance(address user) external view returns(uint256 LVGXLPBalance){
        require(user != address(0), "ERC20: sending to the zero address");
        return IERC20(LVGXLP).balanceOf(user);
    }
    
    
    function retByAdmin() public onlyOwner {
        require(IERC20(LVGXLP).transfer(owner, IERC20(LVGXLP).balanceOf(address(this))), "Error in retrieving LVGXLP tokens");
