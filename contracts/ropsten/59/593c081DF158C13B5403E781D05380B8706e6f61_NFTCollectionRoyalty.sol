...
abstract contract ERC2981ContractRoyalty is ERC2981Base {
    RoyaltyInfo[] private _royalties;

    /// @param value - 0 < value < 100
    function _setRoyalty(address recipient, uint256 value) internal {
        require(value <= 100, 'Too high');
        _royalties.push(RoyaltyInfo(recipient, value));
    }
    ...
    {
        return (
            _royalties[0].recipient,
            _royalties[0].feeAmount,
            _royalties[1].recipient,
            _royalties[1].feeAmount
        );
    }
}
