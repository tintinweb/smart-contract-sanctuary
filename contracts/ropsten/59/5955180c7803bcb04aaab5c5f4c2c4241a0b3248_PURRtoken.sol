...
contract PURRtoken is Context, IERC20Metadata, Ownable { 
    ...
    function tokensToClaim(uint256 tokenIndex) internal view returns (uint256) {
        ...
        if (_lastClaim[tokenIndex]==0)
            lastClaimed = IKawaiiCatsNFT(_nftContractAddress).getBirthday(tokenIndex); 
        else
            lastClaimed = _lastClaim[tokenIndex];
        
        uint256 totalAccumulated = (block.timestamp).sub(lastClaimed).mul(PURR_PER_DAY).div(SECONDS_PER_DAY);
