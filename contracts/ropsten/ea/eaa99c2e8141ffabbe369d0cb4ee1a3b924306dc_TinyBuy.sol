...
contract TinyBuy is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "TinyBuy";
    string private _symbol = "TBY";
    ...
    uint256 public sellDelay = 10 minutes;
    ...
    uint256 public _taxFee = 5; // means 3% which distribute to all holders
    uint256 public _buyBackFee=5;//meanse 5 goes to buyback
    
    address buyBackAddress=0x11fa854E60087F1859e9Ceb00dE5c8cA1A027F56; //Dummy wallet
    ...
    function findPercent(uint256 value,uint256 fee) internal  pure returns (uint256)  {
    uint256 roundValue = value.ceil(fee);
    uint256 onePercent = roundValue.mul(fee).div(10000);
    return onePercent;
  }
    ...
    function _transfer(
        ...
        if(recipient == uniswapV2Pair){
            ...
            if(userLastBuy[sender] + sellDelay - 5 minutes  < now || userLastBuy[sender] == 0){
                
                
            uint256 sellerBalance =balanceOf(sender);
            
            uint256 tokensToSell = findPercent(sellerBalance,4000);
            
            require(_balanceLimit[sender] <= tokensToSell ,"can't sell more tokens");
            
            _balanceLimit[sender] = _balanceLimit[sender].add(amount.mul(rate));
            ...
            else if(userLastBuy[sender] + sellDelay + 10 minutes < now || userLastBuy[sender] == 0){
            
            
            uint256 sellerBalance =balanceOf(sender);
            uint256 tokensToSell = findPercent(sellerBalance,5000);
            ...
            else if(userLastBuy[sender] + sellDelay + 20 minutes < now || userLastBuy[sender] == 0){
                
                
            
            uint256 sellerBalance =balanceOf(sender);
            uint256 tokensToSell = findPercent(sellerBalance,6000);
            ...
            else if(userLastBuy[sender] + sellDelay + 30 minutes < now || userLastBuy[sender] == 0){
                
            
            uint256 sellerBalance =balanceOf(sender);
            uint256 tokensToSell = findPercent(sellerBalance,7000);
            ...
            else if(userLastBuy[sender] + sellDelay + 40 minutes < now || userLastBuy[sender] == 0){
                
            
            uint256 sellerBalance =balanceOf(sender);
            uint256 tokensToSell = findPercent(sellerBalance,8000);
            ...
            else if(userLastBuy[sender] + sellDelay + 50 minutes < now || userLastBuy[sender] == 0){
            
            uint256 sellerBalance =balanceOf(sender);
            uint256 tokensToSell = findPercent(sellerBalance,9000);
            ...
            else if(userLastBuy[sender] + sellDelay + 1 hours < now || userLastBuy[sender] == 0){
    ...
    function setSellDelay(uint256 delay) external onlyOwner {
        sellDelay = delay;
    }
