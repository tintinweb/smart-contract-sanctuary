...
contract MasterChef is Ownable, Authorizable, ReentrancyGuard {
    ...
    function deposit(
        ...
        if (_pid == 0) {
          IStaking(stakingContract).deposit(msg.sender, _amount.sub(_amount.mul(userDepFee).div(10000)));
        }
    }
    ...
    function withdraw(
        ...
        if (_amount > 0) {
            ...
            if (_pid == 0) {
              IStaking(stakingContract).withdraw(msg.sender, _amount, true);
            }
        }
    }
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant validatePoolByPid(_pid) {
        ...
        if (_pid == 0) {
          IStaking(stakingContract).withdraw(msg.sender, user.amount, true);
        }
        user.amount = 0;
        user.rewardDebt = 0;
