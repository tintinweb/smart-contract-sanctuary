...
contract ShareMarket is IMinterReceiver, AccessControl {
    ...
    event AddListing(address supplier, uint40 stakeId, uint72 shares);
    event SharesUpdate(address updater, uint40 stakeId, uint72 sharesAvailable);
    ...
    function claimableShares(uint40 stakeId, address shareOwner)
        public
        view
        returns (uint72 shares)
    {
        return shareListings[stakeId].shareOwners[shareOwner];
    }
    ...
    function sharesMinted(
        ...
        require(hasRole(MINTER_CONTRACT, msg.sender), "CALLER_NOT_MINTER");
        ...
        emit AddListing(supplier, stakeId, stakeShares);
    }
    ...
    function multiBuyShares(ShareOrder[] memory orders) external lock {
        ...
        for (uint256 i = 0; i < orderCount; i++) {
            ...
            uint40 stakeId = order.stakeId;
            uint72 sharesPurchased = order.sharesPurchased;
            ...
            listing.shareOwners[msg.sender] += sharesPurchased;
            emit SharesUpdate(msg.sender, stakeId, listing.sharesAvailable);

            listing.supplierHeartsOwed += uint72(heartsOwed);
            emit BuyShares(stakeId, heartsOwed, sharesPurchased);

            totalHeartsOwed += heartsOwed;
        }

        hexContract.transferFrom(msg.sender, address(this), totalHeartsOwed);
    }

    function buyShares(uint40 stakeId, uint72 sharesPurchased) external lock {
        require(sharesPurchased != 0, "INSUFFICIENT_SHARES_PURCHASED");
    ...
    function claimEarnings(uint40 stakeId) external lock {
        ...
        if (msg.sender == listing.supplier) {
            ...
            emit SharesUpdate(msg.sender, stakeId, 0);
        }
        ...
        emit ClaimEarnings(stakeId, ownedShares, heartsOwed);
    }
    ...
    function supplierWithdraw(uint40 stakeId) external lock {
        ...
        emit SupplierWithdraw(msg.sender, stakeId, heartsOwed);
    }
}