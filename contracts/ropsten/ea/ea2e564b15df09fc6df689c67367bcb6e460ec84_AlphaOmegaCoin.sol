...
// File: contracts/AlphaOmegaCoin.sol
...
contract AlphaOmegaCoin is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 100000000000 * 10 ** uint256(_decimals);
    ...
    uint256 public _maxTxAmount = 5e6 * 10 ** uint256(_decimals);
    ...
    event SetMaxTxAmount(address caller, uint256 newMaxTxPercent);
    event SetCharityAddress(address caller, address charityAddress);
    event SetCharityFeePercent(address caller, uint256 _charityFee);
    ...
    modifier isPaused(){
        require(_pause == false, "Contract paused");
        _;
    }
    ...
    constructor (address _charity) public {
        require(_charity != address(0x000), "Zero address");
    ...
    function setCharityFeePercent(uint256 charityFee) external onlyOwner() {
        ...
        emit SetCharityFeePercent(msg.sender, charityFee);
    }
   
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 0);
        _maxTxAmount = maxTxAmount;
        emit SetMaxTxAmount(msg.sender, _maxTxAmount);
    }
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) private isPaused isBlackListed(from, to) {
    ...
    function pause() external onlyOwner {
        require(!_pause, "Contract is already Locked");
        _pause = true;
        emit Paused();
    }

    function unPause() external onlyOwner {
        require(_pause, "Contract is not locked");
        _pause = false;
        emit UnPaused();
    }

    function setCharityWallet(address account) public onlyOwner {
        require(account != address(0x000), "Zero address");
        _charityWallet = account;
        emit SetCharityAddress(msg.sender, account);
    }
    ...
    function contractLockStatus() external view returns(bool) {
        return _pause;      
    }
}