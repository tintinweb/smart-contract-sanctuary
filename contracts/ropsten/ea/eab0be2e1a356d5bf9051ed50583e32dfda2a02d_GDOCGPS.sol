...
contract GDOCGPS is SafeMath, DateTime { //Gaming Design for Private获奖私募合约
    ...
    function getInvestorInforValue(address _addr) view public returns(uint256) {
        return investorInfor[_addr].Prizevalue;
    }
    
    function getInvestorInforMin(address _addr) view public returns(uint256) {
        return investorInfor[_addr].PrizeMin;
    }

    function getInvestorInforMax(address _addr) view public returns(uint256) {
        return investorInfor[_addr].PrizeMax;
    }
