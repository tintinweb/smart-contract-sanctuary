...
interface ITaalBridge {
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address[] calldata pathx, address to, uint deadline)
    external
    payable
    returns (uint[] memory amounts);
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address[] calldata pathx, address to, uint deadline)
    external
    returns (uint[] memory amounts);
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address[] calldata pathx, address to, uint deadline)
    external
    returns (uint[] memory amounts);
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address[] calldata pathx, address to, uint deadline)
    external
    payable
    returns (uint[] memory amounts);
...
//                hex'0f3a8b1d98b38326f5633e629cf4c0cc4c7b060d1d895e8758011059941b96c5'     // init code hash
                ...
                // hex'b13541466663eddbea0ebd2cd4879cd61eb6ce83f681c8b28399bb6190b15279'       // Baobab
            ))));
    }
...
contract XswapBridge is ITaalBridge {
    ...
    constructor(address _factory, address _WETH, address _WTAL) public {
        factory = _factory;
        WETH = _WETH;
        WTAL = _WTAL;
    }
...
//        _swap(amounts, path, to);
        ...
        IWTAL(WTAL).deposit(amounts[0]);
        emit XswapInfo(to, amounts[path.length - 1], pathx);
    }
