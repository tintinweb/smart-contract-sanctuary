...
contract Comrade {
    ...
    string public privateDataCS;
    string public CBPKey;
    string public CSPKey;
    bool public contractDie;
    bool public CBPayment;
    bool public filledData;
    bool public dataSent;
    bool public dataMatch;
    bool public lockBalances;
    bytes32 public privateDataHash;
    uint256 public timeStartDeal;
    address payable public addressCS;
    address payable public addressCB;
    ...
    function acceptDeal() public returns(string memory) {
        if (!contractDie && ((CBPayment && addressCB == msg.sender && !compareStrings(status, "not happened from combuyer")) || (addressCS==msg.sender && block.timestamp >= timeStartDeal.add(30 days)))) {
                ...
                transfer(addressCS, costVacancy);
                //addressComShop.transfer(feeComShop);
                transfer(addressComShop, feeComShop);
                //addressCB.transfer();
                transfer(addressCB, balances[addressCB].sub(costVacancy.add(feeComShop)));
    ...
    function disableContract()  public {
        ...
        if (balances[addressCB] > 0) {
            ...
            transfer(addressCS, balances[addressCS]);
           // addressCB.transfer(balances[addressCB]);
            transfer(addressCB, balances[addressCB]);
            balances[addressCB] = 0;
            CBPayment = false;
            contractDie = true;
            status = "disabled";
    ...
    function transfer (address _address, uint _amount) internal returns (bool) {
    ...
    function cancelDeal() public returns (bool) {
        if (!contractDie && (msg.sender == addressCB || msg.sender == addressCS)) {
            if (!dataSent || msg.sender == addressCS) {
                //addressCB.transfer(balances[addressCB]);
                transfer(addressCB, balances[addressCB]);
                balances[addressCB] = 0;
                addressCB = address(0);
    ...
    function getPrivateData() public view returns(string memory) {
        ...
        require(msg.sender == addressCB || msg.sender == addressCS, "Only deal participant can see it");
        if (msg.sender == addressCS) {
            return privateDataCS;
        } else {
