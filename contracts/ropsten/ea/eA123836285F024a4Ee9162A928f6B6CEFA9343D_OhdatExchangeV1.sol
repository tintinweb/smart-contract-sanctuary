...
contract OhdatExchangeV1 is AccessControl, Pausable {
    ...
    event PurchaseSuccessful(address indexed buyer,uint256 indexed amount,uint256[] indexed nftTokenIds,uint256 totalPrice);
...
//        require(_mintNft(mintNftTokenIds),"NFT mint failed");
        ...
        emit PurchaseSuccessful(msg.sender,_amount,mintNftTokenIds,totalPrice);
    }

    // nftTokenIds = 5   _arrayLength = 3
    function _getNftTokenIds(uint256 _arrayLength) internal view whenNotPaused returns(uint256[] memory){
        ...
        //i = 2                                  2
        for (uint256 i = nftTokenIds.length; i >= (nftTokenIds.length - _arrayLength); i--){
            resultNftTokenIds[(nftTokenIds.length - i)] = nftTokenIds[i - 1];
        }
    ...
    function _mintNft(uint256[] memory mintNftTokenIds)internal whenNotPaused returns(bool) {
        ...
        if (mintNftTokenIds.length == 1){
            OhdatNftContract.mint(msg.sender,mintNftTokenIds[0],1,abi.encode(address(this)));

        }else{
            amountArray = _generateAmountArray(mintNftTokenIds.length);
            OhdatNftContract.mintBatch(msg.sender,mintNftTokenIds,amountArray,abi.encode(address(this)));
        }
        return true;
    }
