...
//    event UpdateBuyerFees(Fees oldFees, Fees newFees);
    event UpdateSellerFees(FeeLibrary.SellFees oldSellFees, FeeLibrary.SellFees newSellFees);
...
//        uint8 level = FeeLibrary.FeeLevels(_level - 1);
        FeeLibrary.SellFees memory oldSellFees = sellFees.level[_level];
        sellFees.level[_level].setTo(upperTimeLimitInHours * 1 hours, timeLimitFeePercent, saleSizePercent, saleSizeFee);
...
//    }
    ...
    function getAll(SellFeeLevels storage leveledFees) public view returns (SellFees memory, SellFees memory, SellFees memory, SellFees memory, SellFees memory) {
        return(leveledFees.level[1], leveledFees.level[2], leveledFees.level[3], leveledFees.level[4], leveledFees.level[5]);
    }
...

{
  ...
  "libraries": {
    "contracts/libs/FeeLibrary.sol": {
      "FeeLibrary": "0xde9a16ce7a6a196829a70697fce14ed60eb1b765"
    },
    "contracts/libs/SwapWithLP.sol": {
      "SwapWithLP": "0x9e7170814087edc4cb0defc0ea8687cfe22583e5"
    }
  }
}