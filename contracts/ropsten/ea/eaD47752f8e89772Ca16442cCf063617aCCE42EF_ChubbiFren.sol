...
contract ChubbiAuction is ChubbiReserve {
    ...
    function maxAuctionSupply() public view returns (uint256) {
        return maxSupply - (reserved + claimed);
    }
...
contract ChubbiReserve is ChubbiBase {
    ...
    address[] internal reservedAddresses;
    ...
    function claim() external whenNotPaused {
        ...
        reserved = reserved.sub(numberOfTokens);
    ...
    function stopReservations() public onlyOwner whenNotPaused {
        for (uint256 i = reservedAddresses.length; i > 0; i--) {
            delete reservations[reservedAddresses[i - 1]];
            reservedAddresses.pop();
        }
        assert(reservedAddresses.length == 0);
        reserved = 0;
        _isClaimingActive = false;
    }
    ...
    function reserve(address _address, uint256 _amount) internal {
        uint256 previousAmount = reservations[_address];
        uint256 newReserved = reserved.sub(previousAmount).add(_amount);
        require(newReserved + claimed < maxSupply, "Invalid reservation");

        reservations[_address] = _amount;
        reservedAddresses.push(_address);
