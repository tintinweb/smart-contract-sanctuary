...
### MINT INFORMATION ###
mint_price_1: public(uint256)
mint_price_4: public(uint256)
mint_price_10: public(uint256)
...
### PRESALE INFORMATION ###
presaled: public(HashMap[address, uint256])

PRESALE_PRICE: constant(uint256) = 10 * 10 ** 15  # 0.01 ETH
PRESALE_LIMIT: constant(uint256) = 2  # Limit for 1 account
PRESALE_AMOUNT: constant(uint256) = 1000  # Amount of tokens participated in presale
...
def __init__():
    ...
    self.mint_price_1 = 60 * 10 ** 15  # 0.06 ETH
    self.mint_price_4 = 55 * 10 ** 15  # 0.055 ETH
    self.mint_price_10 = 50 * 10 ** 15  # 0.05 ETH

    self.admin = msg.sender
    # Premint, will be revealed later
...
def safeTransferFrom(
    ...
    self._transfer_from(_from, _to, _token_id, msg.sender)
    if _to.is_contract:
        returnValue: bytes32 = ERC721Receiver(_to).onERC721Received(msg.sender, _from, _token_id, _data)
...
@internal
def _mint(_number_of_bees: uint256, _to: address):
    Pollen(self.pollen).settle(_to)
...
def mint(_number_of_bees: uint256 = 1, _to: address = msg.sender) -> bool:
    ...
    price: uint256 = self.mint_price_1
    if _number_of_bees >= 10:
        price = self.mint_price_10
    elif _number_of_bees >= 4:
        price = self.mint_price_4
    assert msg.value == _number_of_bees * price  # dev: incorrect ETH amount

    self._mint(_number_of_bees, _to)
    return True
...
def presale(_number_of_bees: uint256, _signature: Bytes[65], _to: address = msg.sender) -> bool:
    ...
    assert self.totalSupply + _number_of_bees <= PRESALE_AMOUNT  # dev: presale tokens ended
    assert self.presaled[msg.sender] + _number_of_bees <= PRESALE_LIMIT  # dev: too many tokens for you
    assert msg.value == PRESALE_PRICE * _number_of_bees  # dev: incorrect ETH amount

    self._mint(_number_of_bees, _to)
    self.presaled[msg.sender] += _number_of_bees
    return True
...
@external
def set_new_mint(_price_1: uint256, _price_4: uint256, _price_10: uint256):
    """
    @notice Set new prices for minting new tokens
    @param _price_1 Price for minting each token
    @param _price_4 Price for minting each token in a pack of 4+
    @param _price_10 Price for minting each token in a pack of 10+
    """
    assert msg.sender == self.admin  # dev: admin only

    self.mint_price_1 = _price_1
    self.mint_price_4 = _price_4
    self.mint_price_10 = _price_10


@external
def reveal_premint():
    """
    @notice Reveal the tokens preminted for the team
    """
    assert msg.sender == self.admin  # dev: admin only
    assert not self.revealed  # dev: already revealed
    Pollen(self.pollen).settle(msg.sender)
