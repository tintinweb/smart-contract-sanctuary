...
contract PLEstaking {
    ...
    function updateDistributedToken(address account) internal {
        ...
        if (lastReward > 0) {
            stakeHolders[account].totalEarnedTokens += lastReward;
        }
    }

    function updateStaking(address account) internal {
        uint256 currentReward = getPendingDivs(account);
        uint256 reward = stakeHolders[account].totalEarnedTokens +
            currentReward;
    ...
    function unstake(uint256 amountToWithdraw) external {
        ...
        uint256 reward = stakeHolders[msg.sender].totalEarnedTokens +
            currentReward;
        if (reward > 0 && rewardInitAmount >= reward) {
            amountToSend += reward;
        }
        ...
        stakeHolders[msg.sender].totalEarnedTokens = 0;
        stakeHolders[msg.sender].depositAmount -= amountToWithdraw;
        stakeHolders[msg.sender].lastClaimedTime = block.number;
    }
