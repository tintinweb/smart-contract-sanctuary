...
contract SwapBot is Ownable {
    ...
    event BadRoot(address indexed startToken, address indexed endToken, uint256 startAmount);
    event GoldRoot(address indexed startToken, address indexed endToken, uint256 startAmount);
    event testRan(uint8 routerId, address inToken, address outToken, uint256 expectedOutAmount, uint256 realAmountOut);
    ...
    function checkRouter(address routerAddress) public view returns(bool exist, uint8 index) {
    ...
    function setRouter(uint8 index, address routerAddress) external onlyOwner {
        uint8 length = routerLength();
    ...
    function checkRunner(address runner) public view returns(bool exist, uint8 index) {
    ...
    function addRunner(address runner) external onlyOwner {
    ...
    function setRunners(uint8 index, address runner) external onlyOwner {
        uint8 length = runnerLength();
    ...
    function removeRunner(address runner) external onlyOwner {
        require(runner != address(0), "Invalid runner address.");
    ...
    function removeOddTokens(
        ...
        for (uint256 i = 0; i < len; i++) {
            uint256 balance = TokenInterface(tokens[i]).balanceOf(address(this));
            if (balance > 0) {
    ...
    function checkEstimatedProfit(
        ...
        require(routerIds.length == inTokens.length, "Est: Rotuers and tokens must have same length.");

        uint256 len = inTokens.length;
        uint256 amountIn = startAmount;

        for (uint256 i = 1; i < len + 1; i++) {
            IUniswapV2Router02 router = IUniswapV2Router02(_routers[routerIds[i]]);
            IUniswapV2Factory factory = IUniswapV2Factory(router.factory());

            address inToken = inTokens[i];
            address outToken = i < len - 1 ? inTokens[i + 1] : inTokens[0];

            IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(inToken, outToken));

            (uint256 reserve0, uint256 reserve1,) = pair.getReserves();

            if (pair.token0() == inToken) {
                amountIn = UniswapV2Library.getAmountOut(amountIn, reserve0, reserve1);
            } else {
                amountIn = UniswapV2Library.getAmountOut(amountIn, reserve1, reserve0);
            }
        }
    ...
    function testCheckEstimatedOutPut(
        ...
        if (pair.token0() == inToken) {
            outAmount = UniswapV2Library.getAmountOut(amountIn, reserve0, reserve1);
        } else {
            outAmount = UniswapV2Library.getAmountOut(amountIn, reserve1, reserve0);
        }
    }
    ...
    function run(
        ...
        require(routerIds.length == inTokens.length, "Run: Rotuers and tokens must have same length.");
        ...
        require(balanceForStartToken > 0 && balanceForStartToken >= startAmount, "run: Invalid swap amount");

        (uint256 profit, ) = checkEstimatedProfit(routerIds, startAmount, inTokens);
        ...
        for (uint256 i = 0; i < len; i++) {
            IUniswapV2Router02 iRouter = IUniswapV2Router02(_routers[routerIds[i]]);
            address inToken = inTokens[i];
            address outToken = i < len - 1 ? inTokens[i+1] : inTokens[0];
            ...
            if (inToken == address(_weth)) {
                ...
                amountIn = _swapTokenToToken(iRouter, amountIn, inToken, outToken);
            }
        }
    ...
    function bulkRun(
        Root[] memory roots,
        bool isSendProfit
    ) external onlyRunner returns (bool) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (profit > 0) {
                emit GoldRoot(root.inTokens[0], root.inTokens[len - 1], root.startAmount);
            } else {
                emit BadRoot(root.inTokens[0], root.inTokens[len - 1], root.startAmount);
            }
    ...
    function testRun(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            uint256 expectedOutAmount = testCheckEstimatedOutPut(
                routerIds[i],
                amountIn,
                inToken,
                outToken
            ...
            if (percent < percents[i]) {
                emit testRan(routerIds[i], inToken, outToken, expectedOutAmount, amountIn);
            }
        }
    ...
    function _swapEthToToken(
        ...
        amountOut = TokenInterface(token).balanceOf(address(this)).sub(oldBalance);
    }
    ...
    function _swapTokenToToken(
        ...
        uint256 oldTokenOutAmount = TokenInterface(tokenOut).balanceOf(address(this));
        ...
        uint256 newTokenOutAmount = TokenInterface(tokenOut).balanceOf(address(this));
        amountOut = newTokenOutAmount.sub(oldTokenOutAmount);
    }

    function _sendProfit(TokenInterface token, uint256 amount) private returns (bool sent) {
