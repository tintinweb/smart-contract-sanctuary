 ...
 * @author Remco Bloemen <<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="14667179777b5426">[email&#160;protected]</a>π.com>, Eenae <<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="a9c8c5ccd1ccd0e9c4c0d1cbd0ddccda87c0c6">[email&#160;protected]</a>>
...
contract SignkeysCrowdsale is Ownable, ReentrancyGuard {
    ...
    event BuyTokens(address indexed buyer, uint256 tokenPrice, uint256 amount, uint256 indexed customerId);
...
contract SignkeysToken is ERC20, ERC20Detailed, Ownable {
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        bool stakingTransfer = from == address(stakingContract) || to == address(stakingContract);

        if (!stakingTransfer && // pass vesting conditions for stake/unstake operations
        from != address(saleContract) && // sale contract may transfer tokens bypassing vesting schedule restrictions
        from != tokenOwner && // pass vesting condition if owner need to transfer own tokens
        to != tokenOwner) {// pass vesting condition if user returns tokens to owner
            vestingContract.checkTransfer(from, value);
        }
    ...
    function burnFrom(address from, uint256 value) external onlyOwner {
        require(address(vestingContract) != 0x0, "Token should have vesting contract for burn operation");
        _burnFrom(from, value);
        vestingContract.decrementIncome(from, value);
    }
    ...
    function mint(address to, uint256 value) external onlyOwner returns (bool) {
        require(address(vestingContract) != 0x0, "Token should have vesting contract for mint operation");
        _mint(to, value);
        vestingContract.registerIncome(to, value);
        return true;
    }
}
...
contract SignkeysVesting is Ownable {
    ...
    uint256 public INITIAL_PERCENTAGE_TO_SELL_BEFORE_CLIFF_DATE = 50; // 50%

    // TODO test setters/getters
    // The token to which we add the vesting restrictions
    address public signkeysToken;
    ...
    // TODO test setters/getters
    // the percentage of tokens available to sell before cliff date
    uint256 public percentageAvailableToSell;

    // TODO test setters/getters
    /* The amount of tokens user bought */
    mapping(address => uint256) private _income;

    // TODO test setters/getters
    /* The amount of tokens user sold */
    ...
    constructor() public{
        ...
        percentageAvailableToSell = INITIAL_PERCENTAGE_TO_SELL_BEFORE_CLIFF_DATE;
    }
    ...
    function getIncome(address tokenHolder) external view returns (uint256) {
        return _income[tokenHolder];
    }

    function getOutcome(address tokenHolder) external view returns (uint256) {
        return _outcome[tokenHolder];
    }
    ...
    function setPercentageAvailableToSell(uint256 percentage) external onlyOwner {
        require(percentage >= 0 && percentage <= 100, "Percentage must be in range [0, 100]");
        percentageAvailableToSell = percentage;
    }
    ...
    modifier onlyToken() {
        require(msg.sender == signkeysToken, "Only token can call this method");
        _;
    }

    event OutcomeChanged(address indexed from, uint value, uint updatedOutcome);
    event IncomeChanged(address indexed to, uint value, uint updatedIncome);

    // TODO add test to call this method not via token
    function registerTransfer(address _from, address _to, uint256 _value) public onlyToken {
        _outcome[_from] = _outcome[_from].add(_value);
        _income[_to] = _income[_to].add(_value);
        emit OutcomeChanged(_from, _value, _outcome[_from]);
        emit IncomeChanged(_to, _value, _income[_to]);
    }

    // TODO add test to call this method not via token
    function registerIncome(address to, uint256 value) public onlyToken {
        _income[to] = _income[to].add(value);
        emit IncomeChanged(to, value, _income[to]);
    }

    // TODO add test to call this method not via token
    function decrementIncome(address to, uint256 value) public onlyToken {
        _income[to] = _income[to].sub(value);
        emit IncomeChanged(to, value, _income[to]);
    }
}