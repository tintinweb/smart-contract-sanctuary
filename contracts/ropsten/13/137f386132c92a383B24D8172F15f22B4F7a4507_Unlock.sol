...
library VestingLibrary {
    ...
    uint constant PERCENTAGE_SCALING_FACTOR = 10000;
    uint constant DECIMAL_SCALING_FACTOR = 10**18;

    struct Data {
        // uint32 in seconds = 136 years 
        uint16 cliffPercentage;  
    ...
    function initialize(
        ...
        require(cliffPercentage <= PERCENTAGE_SCALING_FACTOR, "VestingLibrary: CLIFF");
        if (vestingDuration == 0 || vestingInterval == 0) {
            // vesting disabled
            require(vestingDuration == 0 && vestingInterval == 0, "VestingLibrary: VESTING");
            // when vesting is disabled, cliff must unlock 100% of funds
            require(cliffPercentage == PERCENTAGE_SCALING_FACTOR, "VestingLibrary: CLIFF");
        } else {
            require(vestingInterval > 0 && vestingInterval <= vestingDuration, "VestingLibrary: VESTING");
        }
    ...
    function availableInputAmount(Data storage self, uint max, uint input, uint saleEnd) internal view returns (uint) {
        // input = amount_unlocked + amount_vested
        uint cliffEnd = saleEnd.add(self.cliffDuration);
        if (now < cliffEnd) {
            return 0; // no unlock or vesting yet
        }
        uint cliff = max.mul(self.cliffPercentage) / PERCENTAGE_SCALING_FACTOR;
        uint maxVested = max.sub(cliff);
        if (input == 0) {
            return _vested(self, 0, maxVested, cliffEnd).add(cliff);
        } else {
            // amount_vested = input - amount_unlocked
            uint vested = input.sub(cliff);
            return _vested(self, vested, maxVested, cliffEnd);
        }
    }

    function availableOutputAmount(Data storage self, uint max, uint output, uint saleEnd) internal view returns (uint) {
        ...
        if (output == max) {
            ...
            uint vested = max.sub(output).sub(cliff);
            return _vested(self, vested, maxVested, cliffEnd);
        }
    }
    ...
    function _vested(
        ...
        uint vestedPerInterval = maxVested.mul(self.vestingInterval) / self.vestingDuration;
        ...
        uint vestingEnd = (maxVested / vestedPerInterval).mul(self.vestingInterval).add(cliffEnd);
        if (now >= vestingEnd) {
            return maxVested.sub(vested);
        } else {
            uint lastVesting = (vested / vestedPerInterval).mul(self.vestingInterval).add(cliffEnd);
            return now.sub(lastVesting).div(self.vestingInterval).mul(vestedPerInterval);
        }
    }
}
