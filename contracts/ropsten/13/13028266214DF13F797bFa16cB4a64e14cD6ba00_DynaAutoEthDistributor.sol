...
contract DynaAutoEthDistributor is AutomatedExternalReflector {
    ...
    function updateRouter(address newAddress) public onlyOwner (){
        ...
        isExcludedFromReflections[address(uniswapV2Router)];
        isExcludedFromReflections[uniswapV2Pair];
    }
...
//        dynaHodler[newUserAddress][currentRound] = dynaHodler[newUserAddress][currentRound+1];
    ...
    function _reflectRewards() private lockTheSwap returns(bool allComplete) {
        ...
        if(currentQueueIndex >= queue.length() || poolForRound[currentRound] == 0){
            currentQueueIndex = 0;
            allComplete = true;
            nextRoundStart = block.timestamp.add(timeBetweenRounds * 1 minutes);
...
//        emit IERC20.Transfer(address(this), hodler, reward);
    ...
    function updateTokenAddress(address token) public {
        ...
        isExcludedFromReflections[pair];
    }
