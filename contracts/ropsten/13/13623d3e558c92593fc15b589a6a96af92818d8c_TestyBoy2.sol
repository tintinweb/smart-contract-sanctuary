...
contract TestyBoy2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("TestyBoy2", "TB2") {
        ...
        if(airDropLimitInEffect){ // Check if Limit is in effect
            if(airDropLimitLiftDate <= block.timestamp){
                ...
                if(_isAirdoppedWallet[from] && senderBalance.sub(amount) < _airDropTokensRemaining[from]){
                    ...
                    require(_airDropAddressNextSellDate[from] <= block.timestamp && block.timestamp >= airDropLimitLiftDate.sub(5 days), "_transfer:: Please read the contract for your next sale date.");
