...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0xfe92A1532232d356DD7fD0ee398D9967001d3AE5), 5000);
        m_DidDeploy = true;
    }
...
contract Insano is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000 * 10**9;
    string private m_Name = "Insano";
    string private m_Symbol = "INSANO";
    ...
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(17 minutes))
    ...
    function addLiquidity() external onlyOwner() {
        ...
        FTPLiqLock(m_LiqLockSvcAddress).lockTokens(m_UniswapV2Pair, block.timestamp.add(16 days), msg.sender);
