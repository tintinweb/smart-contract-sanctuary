...
interface IRadRouter is IERC721Receiver {
   ...
   * @dev Emitted when buyer `buyer` deposits or withdraws ETH from this escrow for asset `assetId` in ledger `ledger`. `escrowed` is true for deposits and false for withdrawals
   */
  event EscrowFulfill(address indexed ledger, uint256 indexed assetId, address seller, address buyer);  
  ...
  /**
   * @dev Sets the minimum price for an asset
   *
   * Requirements:
   *
   * - `_ledger` cannot be the zero address.
   ...
   *
   * - `_ledger` cannot be the zero address.
   * - `msg.sender` must be the escrowed buyer for asset `_assetId` in ledger `_ledger`
   *
   * Emits a {BuyerEscrowChange} event.
   */
...
library RevenueSplitMapping {
    function get(RevMap storage map, address key) internal view returns (uint256) {
        return map.values[key].value;
    }

    function getKeyAtIndex(RevMap storage map, uint256 index) internal view returns (address) {
        return map.keys[index];
    }

    function size(RevMap storage map) internal view returns (uint256) {
        return map.keys.length;
    }

    function set(RevMap storage map, address key, uint256 val) internal {
    ...
    function remove(RevMap storage map, address key) internal {
        if (!map.values[key].inserted) {
            return;
        }
    ...
    function contains(RevMap storage map, address key) internal view returns(bool) {
...
contract RadRouter is IRadRouter, ERC721Holder {
   ...
   */
  function setRevenueSplit(address _ledger, address payable _stakeholder, uint256 _share, bool _retail) external onlyBy(administrator_) virtual override returns (bool success) {
    ...
    require(_share > 0 && _share <= 100, "Stakeholder share must be greater than 0% and at most 100%");
   ...
   */
  function setAssetMinPrice(address _ledger, uint256 _assetId, uint256 _minPrice) external virtual override returns (bool success) {
    require(_ledger != address(0), "Asset ledger cannot be the zero address");
    ...
    Ledgers[_ledger].Assets[_assetId].owner = ledger.ownerOf(_assetId);
   ...
   */
  function sellerEscrowDeposit(address _ledger, uint256 _assetId) external virtual override returns (bool success) {
    require(_ledger != address(0), "Asset ledger cannot be the zero address");
    IERC721 ledger = IERC721(_ledger);
    address owner = ledger.ownerOf(_assetId);
  ...
  function sellerEscrowDepositBatch(address _ledger, uint256[] calldata _assetIds) external virtual override returns (bool success) {
    ...
    for (uint256 i = 0; i < _assetIds.length; i++) {
      ...
      address owner = ledger.ownerOf(assetId);
      require(msg.sender == owner || msg.sender == administrator_, "Only the asset owner or Rad administrator can change asset ownership");
      require(ledger.isApprovedForAll(owner, address(this)) || ledger.getApproved(assetId) == address(this), "Must set Rad Router as an operator for all assets before depositing to escrow");

      Ledgers[_ledger].Assets[assetId].owner = owner;

      ledger.safeTransferFrom(owner, address(this), assetId);

      if (Ledgers[_ledger].Assets[assetId].buyer.wallet != address(0)) {
        _fulfill(_ledger, assetId);
      }

      emit SellerEscrowChange(_ledger, assetId, owner, true);
    }
  ...
  function buyerEscrowWithdraw(address _ledger, uint256 _assetId) external virtual override returns (bool success) {
    require(msg.sender == Ledgers[_ledger].Assets[_assetId].buyer.wallet, "Only the escrowed buyer can withdraw their funds");
    require(_ledger != address(0), "Asset ledger cannot be the zero address");
    Ledgers[_ledger].Assets[_assetId].buyer.wallet = address(0);
    Ledgers[_ledger].Assets[_assetId].buyer.amountEscrowed = 0;
    payable(msg.sender).transfer(Ledgers[_ledger].Assets[_assetId].buyer.amountEscrowed);
  ...
  function _fulfill(address _ledger, uint256 _assetId) internal virtual returns (bool success) {
    ...
    emit EscrowFulfill(_ledger, _assetId, Ledgers[_ledger].Assets[_assetId].owner, Ledgers[_ledger].Assets[_assetId].buyer.wallet);
