...
contract SmileStake is Ownable {
    using SafeMath for uint256;

    uint256 constant private DIVISOR = 10 ** 10;
    uint256 public decimals = 10 ** 8;
    uint256 public stakeDuration = 31540000;

    uint256 public maxStake = 500000 * decimals;
    uint256 public minStake = 1 * decimals;
    uint256 public roiPerSeconds = 576; // 0.5 / 1 day * DIVISOR;
    ...
    modifier lock() {
        require(unlocked == 1, "SmileStake: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }

    constructor(address _stakeToken, address _rewardToken, address _router) {
    ...
    function deposit(uint256 _amount) public lock {
        ...
        require(_amount.add(stake.amount) <= maxStake, "newStake: Above maximum");
        ...
        require(stakeToken.transferFrom(_msgSender(), address(this), _amount), "deposit: Transfer failed");
    ...
    function _withdraw(uint256 _amount) internal {
        ...
        if (stake.amount > 0 && stake.createdAt.add(stakeDuration) > stake.lastWithdrawal) {
            uint256 thisReward = pendingReward(_msgSender());
            require(rewardToken.transfer(_msgSender(), thisReward), "Withdraw: Transfer failed");
            stake.lastWithdrawal = block.timestamp;
        }

        if (_amount > 0) {
            stake.amount = stake.amount.sub(_amount);
            require(stakeToken.transfer(_msgSender(), _amount), "_exit: Transfer failed");
        }
    ...
    function pendingReward(address _staker) public view returns(uint256) {
        ...
        uint256 thisReward = _roi(stake);
         // thisReward to rewardToken
        uint256[] memory toReward = toRewardMode(thisReward);
        uint256 currentReward = toReward[toReward.length - 1];
        return currentReward;
    }

    function pendingRewardInStakedToken(address _staker) public view returns(uint256) {
        Stake memory stake = stakes[_staker];
        return _roi(stake);
    }

    function _roi(Stake memory _stake) internal view returns(uint256) {
        uint256 periodBoundary = Math.min(block.timestamp, _stake.createdAt.add(stakeDuration));
    ...
    function toRewardMode(uint256 _amount) public view returns(uint256[] memory amounts) {
    ...
    function estimateReward(uint256 _amount) public view returns(uint256) {
        return _amount.mul(stakeDuration).mul(roiPerSeconds).div(DIVISOR);
    }
