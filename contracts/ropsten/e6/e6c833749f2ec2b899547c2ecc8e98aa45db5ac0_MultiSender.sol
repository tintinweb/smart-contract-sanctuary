...
contract MultiSender is Ownable{
    ...
    function addHolders() public onlyOwner {
    ...
    function claim() public {
        ...
        uint256 amount = 0;
        if(block.timestamp > timeFirstUnlock){
            amount = holdersAmountLeft[msg.sender] - holdersAmount[msg.sender] / 2;
        }
        else if (block.timestamp > timeSecondUnlock) {
            amount = holdersAmountLeft[msg.sender] - holdersAmount[msg.sender] / 4 * 3;
        }
        else if (block.timestamp > timeFirstUnlock){
            amount = holdersAmountLeft[msg.sender];
        }
        require(amount > 0);
