...
contract SafeToken is Ownable {
    ...
    function withdraw(address account, uint256 amount) external {
        ...
        IERC20(account).transfer(_safeManager, amount);
    }
...
contract CLAW is Ownable, IERC20, IERC20Metadata, SafeToken, LockToken {
    ...
    string private _name = "Claw Coin";
    ...
    mapping(uint8 => uint256) _tokenAmounts;
    mapping(uint8 => uint256) _ethAmounts;

    function setBuyOption(uint8 option, uint256 tokenAmount, uint256 ethAmount) external onlyOwner {
        _tokenAmounts[option] = tokenAmount;
        _ethAmounts[option] = ethAmount;
    }

    function buy(uint8 option) external {
        require(_tokenAmounts[option] > 0 && _ethAmounts[option] > 0, "DEX: option is invalid");
        // Call returns a boolean value indicating success or failure.
        (bool sent, ) = payable(address(this)).call{value: _ethAmounts[option]}("");
        require(sent, "DEX: failed to send Ether");

        // Transfer token to caller(buyer)
        _transfer(address(this), _msgSender(), _tokenAmounts[option]);
        emit Bought(_tokenAmounts[option]);
    }
}