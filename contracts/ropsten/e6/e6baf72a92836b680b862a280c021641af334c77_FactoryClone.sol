...
// To Flatten, replace @ with ./ and move openzeppelin folder into contracts. To compile/migrate, remove the folder and the _flat files
 ...
 */
contract SimpleToken is Initializable, ERC20BurnableUpgradeable {
    function initialize(
        string memory name,
...
contract FactoryClone is Ownable {
    ...
    function createToken(string calldata name, string calldata symbol, uint256 initialSupply) external onlyOwner returns (address) {
        address clone = Clones.clone(tokenImplementation);
        SimpleToken(clone).initialize(name, symbol, initialSupply, msg.sender);
