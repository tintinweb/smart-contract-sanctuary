...
contract AngryContract{
    ...
    uint256 public constant MINE_REWARD_PER_DAY = 63920050;  //每天释放6392.005万枚币用于愤怒挖矿
    ...
    uint256 public mineRewardsWithdrawAmount;
    ...
    mapping(string => uint8) public invoiceStatus;
    ...
    event MineRemainingBurn(uint256 _amount);
    ...
    function withdrawMiningRewards(string calldata _invoiceId, uint256 _amount, bytes memory _sig) public {
        require( invoiceStatus[_invoiceId] == 0, "Can't withdraw twice!" );
        ...
        mineRewardsWithdrawAmount = mineRewardsWithdrawAmount + _amount;
        angryToken.safeTransfer(msg.sender, _amount);
        invoiceStatus[_invoiceId] = 1;
        emit MineRewardsWithdraw(msg.sender, _invoiceId, _amount);
    }
    
    function getTotalMiningRewardAmount() view public returns(uint256) {
        uint256 timeEclapsed = block.timestamp - startTime;
        uint256 ds = timeEclapsed / (1 days);
        uint256 left = timeEclapsed % (1 days);
        if(left > 0){
        ...
        return ds * MINE_REWARD_PER_DAY * 10 ** angryTokenDecimals;
    }
    
    function burnMineRemainingTokens(uint256 _amount) public onlyExecutor {
        uint256 maxBurnAmount = getTotalMiningRewardAmount() - mineRewardsWithdrawAmount;
        require( _amount <= maxBurnAmount, "" );
        IBurnable(ANGRY_TOKEN_ADDRESS).burn(_amount);
        emit MineRemainingBurn(_amount);
    }
}