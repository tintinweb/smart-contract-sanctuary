...
//       require(block.timestamp < 1574711999, "LameFaces sale has completed. They are now only available on the secondary market. ");
   ...
   function integerToString(uint _i) internal pure 
    ...
    function mintFace(uint numberOfTokens) public payable {
        require(numberOfTokens > 0 && numberOfTokens <= 20, "Can only mint 20 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= tokenLimit, "Purchase would exceed max supply of Punks");
        require(msg.value >= facePrice.mul(numberOfTokens), "Ether value sent is not correct");

        for(uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply() < tokenLimit) {
                createFace(1);
            }
        }
