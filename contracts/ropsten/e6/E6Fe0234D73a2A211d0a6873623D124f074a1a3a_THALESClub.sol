...
contract THALESClub is Ownable {
 ...
 function purchaseTokens(address _customerAddress, uint256 _incomingSTAT) internal returns(uint256) {
  if (getInvested() == 0) {
   totalHolder++;
  }
  ...
  uint256 _dividends = SafeMath.div(SafeMath.mul(_incomingSTAT, entryFee_calc), 100000);

  uint256 _amountOfTokens = SafeMath.sub(_incomingSTAT, _dividends);
  ...
  emit onTokenPurchase(_customerAddress, _incomingSTAT, _amountOfTokens, now);
