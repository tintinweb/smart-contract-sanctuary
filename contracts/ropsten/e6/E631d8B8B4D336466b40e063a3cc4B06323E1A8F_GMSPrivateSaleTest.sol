...
contract GMSPrivateSaleTest {
    ...
    constructor(
        ...
        require(_owner != address(0), "GMSPrivateSaleTest: Owner is a zero address");
        require(_cap > 0, "GMSPrivateSaleTest: Cap is 0");
        require(_duration > 0, "GMSPrivateSaleTest: Duration is 0");
        require(_startTime + _duration > block.timestamp, "GMSPrivateSaleTest: Final time is before current time");
    ...
    modifier onlyWhitelisted() {
        require(!whitelistedOnly || whitelisted[msg.sender], "GMSPrivateSaleTest: Account is not whitelisted");
        _;
    }

    modifier isOngoing() {
        require(isLive(), "GMSPrivateSaleTest: Sale is not active");
        _;
    }

    modifier onlyOwner() {
        require(msg.sender == owner, "GMSPrivateSaleTest: Only for contract Owner");
        _;
    }

    modifier isEnded() {
        require(_ended || block.timestamp > startTime + duration, "GMSPrivateSaleTest: Not ended");
        _;
    }
    ...
    function buyWith(address stableCoinAddress, uint256 amount) external isOngoing onlyWhitelisted {
        require(stableCoins.contains(stableCoinAddress), "GMSPrivateSaleTest: Stable coin not supported");
        require(amount > 0, "GMSPrivateSaleTest: Amount is 0");
        require(_isBalanceSufficient(amount), "GMSPrivateSaleTest: Insufficient remaining amount");
        require(amount + balances[msg.sender] >= minPerAccount, "GMSPrivateSaleTest: Amount too low");
        require(maxPerAccount == 0 || balances[msg.sender] + amount <= maxPerAccount, "GMSPrivateSaleTest: Amount too high");

        uint8 decimals = IERC20(stableCoinAddress).safeDecimals();
        uint256 stableCoinUnits = amount * (10**(decimals-2));

        // solhint-disable-next-line max-line-length
        require(IERC20(stableCoinAddress).allowance(msg.sender, address(this)) >= stableCoinUnits, "GMSPrivateSaleTest: Insufficient stable coin allowance");
    ...
    function endPresale() external onlyOwner {
        require(collected >= cap, "GMSPrivateSaleTest: Limit not reached");
        _ended = true;
    }
    ...
    function changeOwner(address _newOwner) external onlyOwner {
        require(_newOwner != address(0), "GMSPrivateSaleTest: New Owner is a zero address");
        newOwner = _newOwner;
    }

    function acceptOwnership() external {
        require(msg.sender == newOwner, "GMSPrivateSaleTest: Only new Owner");
        newOwner = address(0);
    ...
    receive() external payable {
        require(allowETH, "GMSPrivateSaleTest: Revert on all ETH transfers");
    }
}
