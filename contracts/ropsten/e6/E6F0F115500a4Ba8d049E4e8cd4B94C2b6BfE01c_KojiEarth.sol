    ...
    function process() internal {
        ...
        //while(gasUsed < gas && iterations < shareholderCount) {
          while(iterations < shareholderCount) {
            ...
            balance = IBEP20(address(this)).balanceOf(address(shareholders[currentIndex]));
            if (balance < minHoldAmountForRewards) {shares[shareholders[currentIndex]].amount = 0;}
            else {shares[shareholders[currentIndex]].amount = balance;}
...
contract KojiEarth is IBEP20, Auth {
    ...
    function viewHolderInfo(address _address) external view returns (uint256, uint256, uint256) {
        return distributor.holderInfo(_address);
    }
 
    function viewMathInfo() external view returns (uint256, uint256, uint256, uint256, uint256) {
