...
contract PonziToken is BEP20('Ponzi Token', 'PONZI') {
    ...
    external
    {
        ...
        require(signatory != address(0), "REVA::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "REVA::delegateBySig: invalid nonce");
        require(now <= expiry, "REVA::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "REVA::getPriorVotes: not yet determined");
    ...
    internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying REVA (not scaled);
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "REVA::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts/PonziMinter.sol
