...
// File: contracts/deftifyVesting.sol
...
contract Vesting is Ownable {
    ...
    uint256 public constant WEEKS = 60 seconds;
    uint256 public constant MONTHS = 240 seconds;

    uint256 public startTimestamp;
    uint256 public usersSupply;
    ...
    event Withdrawn(uint256 time, address sender, uint256 amount);
...
//            console.log('_amount:', _amount[i]);
//            console.log('balance before transfer', token.balanceOf(address(this)));
            token.transferFrom(msg.sender,address(this),_amount[i]);
//            console.log('balance after transfer', token.balanceOf(address(this)));
            TotalTokenAllocations[_round][_accounts[i]] = _amount[i];
            usersSupply += _amount[i];
        }
    }
    ...
    function withdraw() public onlyOwner {
        require(token.balanceOf(address(this)) > usersSupply, "There is no more allowed to withdraw tokens");
        uint256 amount = token.balanceOf(address(this)) - usersSupply;
        token.transfer(msg.sender, amount);
        emit Withdrawn(block.timestamp, msg.sender, amount);
    }
    /**
