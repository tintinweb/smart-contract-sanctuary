...
contract Cypher is ERC721 {
	...
	constructor(uint256 tokensLimitInit, address payable desAddressOneInit, address payable desAddressTwoInit) public ERC721("Cypher","CYPHER") {
	...
	function mintToken(address to, string memory uri) 
		...
		returns (uint256) 
	{
		require(msg.value >= 10000000000,"Not enough ETH sent");
	...
	function mintFiveTokens(address to, string memory uri,string memory uriTwo,string memory uriThree,string memory uriFour,string memory uriFive) 
		...
		returns (uint256) 
	{
		require(msg.value >= 50000000000,"Not enough ETH sent");
