...
contract KoreanLiberationDayToken is StandardToken, Ownable {
  ...
  function speech(uint16 tokenId) public view returns (string) {
      return bytes32ToString(tokens[tokenId]);
  }
  ...
  function firstTokenId(address _owner) internal view returns (uint16) {
  ...
  function transfer(address _to, uint256 _value) public returns (bool) {
    ...
    for (uint16 i = 0; i < _value; i++) {
      transferTokenOwnership(msg.sender, _to, firstTokenId(msg.sender));
  ...
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
    ...
    for (uint16 i = 0; i < _value; i++) {
      transferTokenOwnership(_from, _to, firstTokenId(_from));
  ...
  function transferBatch(address [] _to) public returns (bool) {
    ...
    for (uint16 i = 0; i < _to.length; i++) {
      transferTokenOwnership(msg.sender, _to[i], firstTokenId(msg.sender));
