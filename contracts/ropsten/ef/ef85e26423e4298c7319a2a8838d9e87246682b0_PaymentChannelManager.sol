...
contract PaymentChannelManager {
    ...
    event ChannelOpen(address client, uint channelCounter);
    event ChannelCharge(address client, uint channelNumber, uint tokens);
    event ChannelClosing(address client, uint channelCounter);
    event ChannelClose(address client, uint channelCounter);
    event ChannelChallenge(address client, uint channelCounter);
    ...
    function closeChannel(bytes _receipt, bytes _clientSignature, bytes _serverSignature) public {
        ...
        if (receiptStruct.close) {
            ...
            emit ChannelClosing(receiptStruct.account, receiptStruct.channelNumber);
        }
    }
    ...
    function closeChannelWithoutReceipt() public {
        ...
        channel.state = ChannelState.CLOSING;
        emit ChannelClosing(msg.sender, channelNumber);
    }
    ...
    function challengeChannel(bytes _receipt, bytes _clientSignature, bytes _serverSignature) public {
        ...
        emit ChannelChallenge(receiptStruct.account, receiptStruct.channelNumber);
