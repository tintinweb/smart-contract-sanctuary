...
contract SmolBrain is ERC721Enumerable, Ownable  {
    ...
    function mintSmolBrainAndLand() external payable {
        ...
        function mintBulk(uint256 numberOfTokens) public payable {
        require(numberOfTokens != 0, "You need to mint at least 1 token");
        require(numberOfTokens <= maxMint, "You can only mint 20 tokens at a time");
        require(totalSupply() + numberOfTokens <= LAST_FEMALE, "Minting would exceed max. supply");
        require(_price * numberOfTokens <= msg.value, "Not enough Ether sent.");
