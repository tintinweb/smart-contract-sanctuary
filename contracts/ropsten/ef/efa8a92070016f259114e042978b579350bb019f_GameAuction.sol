...
contract GameAuction {
    ...
    modifier onlyIfBidsNotFull(string memory team){
        if (((keccak256(abi.encodePacked(team))) == ((keccak256(abi.encodePacked("P"))))) && (numberOfBidsP >= 2)) revert BidsFull(team);
        if (((keccak256(abi.encodePacked(team))) == ((keccak256(abi.encodePacked("E"))))) && (numberOfBidsE >= 2)) revert BidsFull(team);
        _;
    }
        ...
        onlyIfTeamIsValid(_teamName)
    {
        ...
        if ((numberOfBidsP >=2) && (numberOfBidsE >=2)) { // TESTINGTODO:Change numbers accordingly
            stage = 2;
        ...
        onlyBefore(revealEnd)
    {
        ...
        if ((numberOfRevealsP >= 2) && (numberOfRevealsE >=2)) { // TESTINGTODO:Change numbers accordingly
            stage = 0;
            auctionEnd();
        }
    }
