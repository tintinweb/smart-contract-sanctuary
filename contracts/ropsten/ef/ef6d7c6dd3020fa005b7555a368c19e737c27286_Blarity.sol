...
contract Blarity {
  ...
  function donateToken(KyberNetworkProxy network, ERC20 src, uint srcAmount, uint maxDestAmount, uint minConversionRate, address walletId) public payable {
    uint amount = network.trade.value(msg.value)(src, srcAmount, ACCEPT_DAI_ADDRESS, address(this), maxDestAmount, minConversionRate, walletId);
    for(uint i = 0; i < donators.length; i++) {
      if (donators[i].addr == msg.sender) {
        donators[i].amount += amount;
        Donated(msg.sender, amount);
