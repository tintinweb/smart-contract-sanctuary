    ...
    AbstractDependant
{
    ...
    event DAIInvested(
        address _referralLink,
        address _daiInvestor,
        uint256 _tokensAmount,
        uint256 _newTotalGroupAmount
    );
    ...
    function joinTheTeam(address _referralLink) external override {
        ...
        usersTeamInfo[msg.sender] = UserTeamInfo(
            usersTeamInfo[_referralLink].teamName,
            _referralLink,
            0,
            false
        );
    ...
    function investDAI(uint256 _tokensAmount, address _policyBookAddr) external override {
        ...
        require(_userTeamInfo.teamAddr != address(0), "LM: User is without a team");
    ...
    function getReward() external override returns (uint256) {
        ...
        countsOfRewardedMonth[_teamAddr][msg.sender] = countsOfRewardedMonth[_teamAddr][msg.sender]
            .add(_getAvailableMonthForReward(msg.sender));
    ...
    function _getAvailableMonthForReward(address _userAddr) internal view returns (uint256) {
        ...
        uint256 _countOfRewardedMonth =
            countsOfRewardedMonth[usersTeamInfo[_userAddr].teamAddr][_userAddr];
...
interface IPolicyBook {
    ...
    function withdrawalsInfo(address _userAddr)
        external
        view
        returns (uint256 _withdrawalAmount, uint256 _readyToWithdrawDate);
    ...
    function buyPolicyFor(address _buyer, uint256 _epochsNumber, uint256 _coverTokens) external returns (uint256);
...
interface IPolicyBookRegistry {
    ...
    function buyPolicyBatch(
        ...
        external         
        returns (uint256[] memory _allowances);
