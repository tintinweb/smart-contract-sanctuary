...
contract KittyClub is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 300000000000000000000000;
    ...
    constructor(address devWallet, address tradingAgent_) {
        _rOwned[_msgSender()] = _rTotal;
        emit Transfer(address(0), _msgSender(), _tTotal);
        teamDevAddress = payable(devWallet);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(
            teamDevAddress,
            transferredBalance.div(_liquidityFee).mul(splitDivisor)
        );
    }
