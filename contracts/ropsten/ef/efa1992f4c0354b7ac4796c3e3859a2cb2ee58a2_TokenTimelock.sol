...
contract TokenTimelock {
    ...
    public
  {
    ...
    for (uint256 i = 0; i < _releaseTimes.length; i++) {
      require(_releaseTimes[i] > block.timestamp);
    }    
    token = _token;
  ...
  function release() public {
    // solium-disable-next-line security/no-block-members
    require(block.timestamp >= releaseTimes[count]);

    uint256 amount = token.balanceOf(this);
    require(amount > 0);

    for (uint256 i = 0; i < beneficiaries.length; i++) {
      token.safeTransfer(beneficiaries[i], amount);
      releasedTokens[beneficiaries[i]] = releasedTokens[beneficiaries[i]].add(1*10**18);
    }
  }
}