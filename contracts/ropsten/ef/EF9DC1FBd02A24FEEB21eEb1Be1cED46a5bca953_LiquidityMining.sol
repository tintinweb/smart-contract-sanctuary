    ...
    AbstractDependant
{
    ...
    function getTopTeams() external view override returns (TopTeamInfo[] memory teams) {
        uint256 leaderboradSize = leaderboard.length;

        teams = new TopTeamInfo[](leaderboradSize);

        for (uint256 i = 0; i < leaderboradSize; i++) {
            teams[i].teamDetails = _getTeamDetails(leaderboard[i]);
            teams[i].totalReward = _getTeamReward(i);
        }
    }

    function getTopUsers() external view override returns (TopUserInfo[] memory users) {
        uint256 topUsersSize = topUsers.length;

        users = new TopUserInfo[](topUsersSize);

        for (uint256 i = 0; i < topUsersSize; i++) {
            address _currentUserAddr = topUsers[i];

            users[i].userInfo = UserInfo(
                _currentUserAddr,
                teamInfos[usersTeamInfo[_currentUserAddr].teamAddr].name,
                usersTeamInfo[_currentUserAddr].stakedAmount
            );
            users[i].mainNFT = checkMainNFTReward(_currentUserAddr);
            users[i].platinumNFT = checkPlatinumNFTReward(_currentUserAddr);
        }
    }
...
interface ILiquidityMining {
    ...
    function getTopTeams() external view returns (TopTeamInfo[] memory teams);

    function getTopUsers() external view returns (TopUserInfo[] memory users);
