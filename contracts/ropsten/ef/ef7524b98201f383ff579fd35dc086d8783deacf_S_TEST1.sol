...
contract S_TEST1 is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    mapping (address => bool) public _isExcludedFromTimelock;
    mapping (address => bool) public _isExcludedFromAntiWhale;
    mapping (address => uint256) public lastTransfer;

    address[] private _excluded;

    uint256 public _taxFee = 1; //in percentage
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 6; // 50% will be added pool, 50% will be converted to BNB
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint public _charityFee = 1;
    ...
    uint256 public threshHoldTopUpRate = 20; // 20 percent
    mapping(address => uint256) public nextAvailableClaimDate;
    bool public swapAndLiquifyEnabled = false; // should be true after launch
    uint256 public disableEasyRewardFrom = 0;
    uint256 public rewardThreshold = 1 ether; //threshold after which 20% of the BNB is taken to buy tokens, these tokens get burned

    uint256 minTokenNumberToSell = _tTotal.mul(1).div(100000); // 0.001% max tx amount will trigger swap and add liquidity. Should always be lower than maxTxAmount!
    ...
    string private _name = "S_TEST1 Token";
    string private _symbol = "STEST1";
    uint8 private _decimals = 9;

    address public charityWallet = address(0x68793Bd9E38BA703A8efd3E2d06b63CE8eEFb4C6); //to be changed!!
    ...
    uint256 public _timeDelta;
    uint256 public _maxBalancePrct = 10; //10%
    uint256 public _maxTotalSupplyPrct = 10; // 1/1000 = 0.1%
    uint256 public _minBalanceToClaim = 5 * 10**6 * 10**9; //5M tokens needed to be able to claim BNB
    ...
    function setChariftyFee(uint256 charityFee) public onlyOwner {
    ...
    function setMaxTotalSupply(uint256 maxTotalSupplyPrct) public onlyOwner {
        _maxTotalSupplyPrct = maxTotalSupplyPrct;
    }
    ...
    function _takeCharity(uint256 tCharity) private {
        ...
        _rOwned[charityWallet] = _rOwned[address(this)].add(rCharity);
        if (_isExcludedFromRFI[charityWallet])
            _tOwned[charityWallet] = _tOwned[address(this)].add(tCharity);
    }
    ...
    function activateContract() public onlyOwner {
        ...
        setMaxTotalSupply(1); // uses factor 10 000
