...
library SafeMath {
    ...
    function toUint8(uint256 _value) internal pure returns (uint8 z){
        require(_value < 2**8, "toUint8() overflow!");
        return uint8(_value);
    }
}
...
contract EPARA {
    ...
    function approveTransferTo(address _multisign, address _to, uint _value) public returns(bool success) 
    {
        ...
        if (true == isMultiSignActivated(_multisign)
            && score >= multiSign[_multisign].passScore
            &&_value <= multiSign[_multisign].multiSignBalance){//check if reach passScore, execute recall
