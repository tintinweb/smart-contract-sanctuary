...
contract Oasis{
	...
	//释放 
	mapping(address => mapping(uint => uint[])) public mycan; //时间
	//mapping(address => uint[]) public mycanmoney; //金额
	//上线释放
	mapping(address => mapping(uint => uint[])) public myrun; //时间
	...
	function addmoney(address _addr, uint256 _amount, uint256 _money, uint _day) private returns(bool){
		eths[_addr] += _money;
		mycan[_addr][0].push(_amount);
		mycan[_addr][1].push(_money);
		if(_day > 0){
		    mycan[_addr][2].push(0);
		}else{
		    mycan[_addr][2].push(now);
		}
	...
	function addrunmoney(address _addr, uint256 _amount, uint256 _money, uint _day) private {
		runs[_addr] += _money;
		myrun[_addr][0].push(_amount);
		myrun[_addr][1].push(_money);
		if(_day > 0){
		    myrun[_addr][2].push(0);
		}else{
		    myrun[_addr][2].push(now);
		}
	...
	function getcanuse(address user) public view returns(uint _left) {
		...
		if(mycan[user][0].length > 0) {
		    for(uint i = 0; i < mycan[user][0].length; i++) {
    			uint stime = mycan[user][2][i];
    			if(stime == 0) {
    			    _left += mycan[user][0][i];
    			}else{
    			    if(_now - onceOuttime >= stime) {
    			        uint smoney = mycan[user][0][i] * (per/ 1000) * ((_now - stime)/sysday);
    			        if(smoney <= mycan[user][1][i]){
    			            _left += smoney;
    			        }
    			    }
		...
		else if(_left > eths[user]) {
			_left = eths[user];
		}else{
		    _left = _left - used[user];
		}
	...
	function getcanuserun(address user) public view returns(uint _left) {
		uint256 _now = now;
		if(myrun[user][0].length > 0) {
		    for(uint i = 0; i < myrun[user][0].length; i++) {
    			uint stime = myrun[user][2][i];
    			if(stime == 0) {
    			    _left += myrun[user][0][i];
    			}else{
    			    
    			    if(_now - onceOuttime >= stime) {
    			        uint smoney = myrun[user][0][i] * (per/ 1000) * ((_now - stime)/sysday);
    			        if(smoney <= myrun[user][1][i]){
    			            _left += smoney;
    			        }
    			    }
		...
		if(_left < runused[user]) {
			_left = 0;
		}
		else if(_left > runs[user]) {
			_left = runs[user];
		}else{
		    _left = _left - runused[user];
		}
