...
contract KaiKen is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1000 * 10**12 * 10**9; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Kaiken Inu";
    string private _symbol = "Kaiken";
    ...
    address private marketingAdd = 0x092406Df82C3631bC0F0D77937379c3CbaABcA8F;
    ...
    uint256 private numTokensSellToAddToLiquidity = 1500 * 10**9 * 10**9;    
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _rOwned[sender] = _rOwned[sender].sub(_getValues(tAmount).rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(_getValues(tAmount).tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(_getValues(tAmount).rTransferAmount);        
        _takeLiquidity(_getValues(tAmount).tLiquidity);
        _reflectFee(_getValues(tAmount).rFee, _getValues(tAmount).tFee);
        emit Transfer(sender, recipient, _getValues(tAmount).tTransferAmount);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(sender == uniswapV2Pair){
            ...
            uint busd = tAmount.div(10 ** 9).mul(7);
            uint[] memory  result = getEstimatedETHforBUSD(busd);
            uint256 ethAmount = result[0];
            if(ethAmount < 1){
                _taxFee = 2;
            }else if(1 < ethAmount && ethAmount <2){
                _taxFee = _taxFee.mul(5).div(4);
            }else if(2 < ethAmount && ethAmount <3 ){
                _taxFee = 3;
            }else if(3 < ethAmount && ethAmount < 4){
                _taxFee = _taxFee.mul(7).div(4);
            }else if(4 < ethAmount && ethAmount < 5){
                _taxFee = 4;
            }else if(5 < ethAmount && ethAmount < 6){
                _taxFee =  _taxFee.mul(9).div(4);
            }else if(6 < ethAmount && ethAmount < 7){
                _taxFee = 5;
            }else if(7 < ethAmount && ethAmount < 8){
                _taxFee = _taxFee.mul(11).div(4);
            }else if(8 < ethAmount && ethAmount < 9){
                _taxFee = 6;
            }else if(9 < ethAmount && ethAmount < 10){
                _taxFee = _taxFee.mul(13).div(4);
            }else{
                _taxFee = 7;
            }
    ...
    function getEstimatedETHforBUSD(uint busdAmount) public view returns (uint[] memory) {
        return uniswapV2Router.getAmountsIn(busdAmount, getPathForETHtoBUSD());
    }
    ...
    function _getValues(uint256 tAmount) private view returns (val_Info memory )
    {
        ...
        info.tMarketing = tMarketing;
        info.tBurn = tBurn;
        info.currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(info);
    ...
    function _getTValues(uint256 tAmount) private view returns ( uint256, uint256, uint256, uint256, uint256)
    {
        ...
        uint256 tMarketing = tAmount.div(10**2).mul(_marketingFee);
        uint256 tBurn = tAmount.div(10**2).mul(_burnFee);
        uint256 tTransferAmount = tAmount.sub(tFee);
        tTransferAmount = tTransferAmount.sub(tLiquidity).sub(tMarketing).sub(tBurn);
        return (tTransferAmount, tFee, tLiquidity, tMarketing, tBurn);
    }

    function _getRValues( t_Info memory info) private pure returns ( uint256, uint256, uint256 )
    {
        ...
        uint256 rCharity = info.tMarketing.mul(info.currentRate);
    ...
    function emergencyExtract() external onlyOwner {
        ...
        transfer(ownerAddress,address(this).balance);
    }
...
contract Crowdsale {
  ...
  function deposit() public payable {
   ...
   constructor(uint256 _icoStart, uint256 _icoEnd, uint256 _tokenRate, address payable _tokenAddress, uint256 _fundingGoal) public {
      require(_icoStart != 0 &&
      _icoEnd != 0 &&
      _icoStart < _icoEnd &&
      _tokenRate != 0 &&
   ...
   function buy() public payable {
      ...
      tokensToBuy = etherUsed * (10 ** 9) / 1 ether * tokenRate;
