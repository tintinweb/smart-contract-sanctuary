...
contract ESG is IERC20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        if(sender==address(this)) { return _basicTransfer(sender, recipient, amount); }
    ...
    function swapBack(uint256 _feeAmount) internal swapping {
        require(balanceOf(address(this))>=_feeAmount, "swapBack : Insufficient balance");
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(swapInToken) {
            ...
            if(msg.sender!=address(pair)&&!inSwap) { swapBack(feeAmount); }
        }
