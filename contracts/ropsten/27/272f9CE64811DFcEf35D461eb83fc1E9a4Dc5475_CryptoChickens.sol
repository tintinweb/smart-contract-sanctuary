...
contract CryptoChickens is ERC721Full, Ownable {
  ...
  uint256 public constant MAX_RAREBREED = 130;
  uint256 public constant MAX_COMMONPACK = 300;
  uint256 public constant MAX_RARECOMMON = 30;
  uint256 public constant reveal_timestamp = 1631074000;
  ...
  uint256 public RAREBREED = 0;
  uint256 public COMMONPACK = 0;
  uint256 public RARECOMMON = 0;
  ...
  function mintRareBreed(address _to, string memory _tokenURI) public payable {
    require(msg.value >= BREEDERFEE);
    require(RAREBREED < MAX_RAREBREED);
    uint _tokenId = totalSupply().add(1);
    _mint(_to, _tokenId);
    _setTokenURI(_tokenId, _tokenURI);
    RAREBREED++;
  ...
  function mintRareCommon(address _to, string memory _tokenURI) public payable {
    require(msg.value >= BREEDERFEE);
    require(RARECOMMON < MAX_RARECOMMON);
    uint _tokenId = totalSupply().add(1);
    _mint(_to, _tokenId);
    _setTokenURI(_tokenId, _tokenURI);
    RARECOMMON++;
