...
def __init__(_name: String[64], _symbol: String[32], _decimals: uint256, _credits: int128, _cls: int128, _dataVersion: int128):
    ...
    self.AdminSecurityNum[self.name] = [100, 200, 300]
...
@external
def RegisterNewAdmin(_AdminName: String[100], _0xAdminName: Bytes[100], _sec1: uint256, _sec2: uint256, _sec3:uint256):
    
    # validate legitimate admin
    assert _sec1 > 20, "Wrong first registration digit "
    assert _sec2 > 20, "Wrong second registration digit "
    assert _sec3 > 20, "Wrong third registration digit "
    ...
    _AdminAddress: address = self.CreateAddres(_0xAdminName, _sec1, _sec2, _sec3)
...
@external
def UpdateDatabase(_AdminName1: String[100],_sec11:uint256, _sec12:uint256, _sec13:uint256, _AdminName2: String[100],_sec21:uint256, _sec22:uint256, _sec23:uint256, _data: int128[l], _cost: int128[l]): 
    """
    ...
    assert _Num1[0] == _sec11, "Wrong first digit"
    assert _Num1[1] == _sec12, "Wrong second digit"
    assert _Num1[2] == _sec13, "Wrong third digit"
    
    # Check the validity of the Admin2
    _Num2: uint256[3] = self.AdminSecurityNum[_AdminName2]
    
    assert _Num2[0] == _sec21, "Wrong first digit"
    assert _Num2[1] == _sec22, "Wrong second digit"
    assert _Num2[2] == _sec23, "Wrong third digit"
...
@external
def RegisterNewClient(_Name: String[100], _0xName: Bytes[100], _sec1: uint256, _sec2: uint256, _sec3:uint256):
    """
    @dev allows new user to register 
    ...
    _address: address = self.CreateAddres(_0xName, _sec1, _sec2, _sec3)
...
@external
def CheckClientStatus(_Name: String[100], _sec1: uint256, _sec2:uint256, _sec3: uint256,  _id: int128[idx],):
    """
    ...
    assert _Num[0] == _sec1, "Wrong first digit"
    assert _Num[1] == _sec2, "Wrong second digit"
    assert _Num[2] == _sec3, "Wrong third digit"
...
################################ Admin security numbers  ###################
@view
@external
def AdminSecNum( _AdminName1:String[100], _sec1:uint256, _sec2:uint256, _sec3:uint256, _AdminName2:String[100],) -> (uint256[3]):
    
    # Check Admin validity
    assert self.AdminRegistry[ _AdminName1] != ZERO_ADDRESS, "Unauthorized user"
    assert self.AdminRegistry[ _AdminName2] != ZERO_ADDRESS, "Unauthorized user"
    
    # Check the validity of the client
    _Num: uint256[3] = self.AdminSecurityNum[_AdminName1]
    ...
    return self.AdminSecurityNum[ _AdminName2]

################################ View Database ################################

@view
@external
def SystemDataBase(_version: int128) -> (int128[l], int128[l]):
...
@view
@external
def ClientActivityStats(_ClientName: String[100]) -> (int128,int128[l], int128[l]):
    
    # Check user validity
    assert self.ClientRegistry[_ClientName] != ZERO_ADDRESS, "Unauthorized user"
    
    _id:int128[l] = empty(int128[l])
    for i in range(l):
        _id[i] = i
    return self.Client_Frequency[_ClientName], _id,  self.Activity_Statistics[_ClientName]

################################ User security numbers  ###################
@view
@external
def ClientSecNum(_AdminName:String[100], _sec1:uint256, _sec2:uint256, _sec3:uint256, _ClientName: String[100]) -> (uint256[3]):
    ...
    # Check the validity of the client
    _Num: uint256[3] = self.AdminSecurityNum[_AdminName]
    ...
    return self.ClientSecurityNum[ _ClientName]
...
@external
def TransferToClient(_toName : String[100], _sec1: uint256, _sec2: uint256, _sec3: uint256, _value : uint256) -> bool:
    """
...
@external
def TransferClient1toClient2(_fromName : String[100],  _sec11: uint256, _sec12: uint256, _sec13: uint256, _toName : String[100],  _sec21: uint256, _sec22: uint256, _sec23: uint256, _value : uint256) -> bool:
    """
    @dev Transfer tokens from one address to another.
...
@external
def AssignTokenToName( _AdminName: String[100], _sec11: uint256, _sec12: uint256, _sec13: uint256,  _toName: String[100], _sec21: uint256, _sec22: uint256, _sec23: uint256, _value: uint256):
    """
    @dev Mint an amount of the token and assigns it to an account.
...
@external
def DeleteTokensFrom(_toName: String[100], _sec1: uint256, _sec2: uint256, _sec3: uint256, _value: uint256):
    """
    Parameters
    ----------
    _to : address
