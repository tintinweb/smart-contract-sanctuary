...
contract ArbMinter{
    ...
    function fixStablePool(ISLP _slp) external {
        ...
        if(reserve0 > reserve1){
            //Mint USDC/DAI
            mintAndDeposit((reserve0-reserve1),_slp.token0(),_slp.token1());
        }
        else{
            //Mint bUSD
            mintAndDeposit((reserve1-reserve0),_slp.token1(),_slp.token0());
        }
    ...
    function mintAndDeposit(uint256 _amount, address _sellToken, address _buyToken) internal{
        ...
        Router.swapExactTokensForTokens(_amount,0,router,0xC189Ca9C9168004B3c0eED5409c15A88B87a0702,block.timestamp);
    }
}