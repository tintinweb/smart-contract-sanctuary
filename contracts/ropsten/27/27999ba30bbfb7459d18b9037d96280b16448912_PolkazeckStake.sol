...
contract PolkazeckStake is Ownable {
    ...
    function toRewardMode2(uint256 _amount, address _token) public view returns(uint256[] memory amounts) {
        address weth = router.WETH();
        address[] memory path1 = new address [](2);
        address[] memory path2 = new address [](3);
        ...
        if (_token == weth) {
            path1[0] = address(stakeToken);
            path1[1] = weth;
            amounts = router.getAmountsOut(_amount, path1);
            
        } else {
            path2[0] = address(stakeToken);
            path2[1] = weth;
            path2[2] = _token;
            amounts = router.getAmountsOut(_amount, path2);
        }
    }
