...
contract DirtyFarm is Ownable, Authorizable, ReentrancyGuard {
    ...
    function updateVariablePoolReward() private {
        ...
        runningTotal1 = runningTotal1.div(210000000000000000000000); //divide by 210,000 to get Dirty converted to LP ratio

        uint256 rTotal = runningTotal0.add(runningTotal1); //add LP tokens + dirty tokens in LP ratio

        if (rTotal > 10000000000000000000000) { //make sure rTotal is > 10k since we are dividing by 10k in next step
            
            rTotal = rTotal.div(10000000000000000000000); //divide that total by 10,000 (default pool reward) to get multiplier

            poolReward = rTotal.mul(10000000000000000000000); // 10k x multiplier to get new poolReward

        } else {
            poolReward = 10000000000000000000000; // if rTotal is < 10k, that means not many stakers left, set poolReward back to 
                                                  // default of 10k
        }
