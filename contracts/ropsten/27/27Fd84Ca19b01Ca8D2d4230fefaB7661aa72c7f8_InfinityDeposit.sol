...
contract InfinityDeposit is RetrieveTokensFeature, Package {
    ...
    event LogWithdrawal(uint256 Id, uint256 IndexPackage, address SentToAddress, uint256 AmountTransferred);
    event LogDeposit(uint256 Id, uint256 IndexPackage, address WithdrawalAddress, uint256 Amount);
    ...
    function makeDeposit(address _withdrawalAddress, uint256 _idxPackage, uint256 _amount) public canBuy(_idxPackage, _amount) returns(uint256 _id) {
        ...
        emit LogDeposit(_id, _idxPackage, _withdrawalAddress, tensetFixedBalance);
    }
...
contract Package is Ownable {
    ...
    function showPackageDetail(uint16 _index) public view returns(string memory, uint256, uint256, uint256) {
