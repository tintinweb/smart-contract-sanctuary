...
contract Arbitrage is FlashLoanReceiverBase {
    ...
    function _tradeOnUniswapV3(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOutMin) public {
        uint24 fee = 3000;
    ...
    function _tradeOnSushi(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOutMin) public {
        address recipient = msg.sender;
        uint256 deadline = block.timestamp + 300;
        ...
        if (executionType >= 2) {
            for (uint i=0; i<tradePaths.length; i++) {
                ...
                if (keccak256(abi.encodePacked(tradePath.exchange)) == keccak256(abi.encodePacked('uniswapv3'))) {
                    _tradeOnUniswapV3(tradePath.tokenIn, tradePath.tokenOut, tradePath.amountIn, tradePath.amountOutMinimum);
                }
                else if (keccak256(abi.encodePacked(tradePath.exchange)) == keccak256(abi.encodePacked('sushiswap'))) {
                    _tradeOnSushi(tradePath.tokenIn, tradePath.tokenOut, tradePath.amountIn, tradePath.amountOutMinimum);
                }
            }
        }
