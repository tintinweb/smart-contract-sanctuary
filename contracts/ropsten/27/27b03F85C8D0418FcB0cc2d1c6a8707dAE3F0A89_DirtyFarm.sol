...
contract DirtyFarm is Ownable {
    ...
    uint256 public blocksPerDay = 3333; // The estimated number of mined blocks per day, lowered so rewards are halved to start.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (_pid == 1) { //$Dirty tokens
                ...
                pool.runningTotal = pool.runningTotal.add(_amount).mul(95).div(100);
                user.amount = user.amount.add(_amount).mul(95).div(100); //5% tax on deposit for 
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (_pid == 1) { //$Dirty tokens
                ...
                uint256 lpSupply = pool.lpToken.balanceOf(address(this));
                uint256 totalRewards = lpSupply.sub(pool.runningTotal);
                if (totalRewards == 0) {
                    pool.lpToken.safeTransfer(address(msg.sender), _amount);
                } 
                if (totalRewards > 0) { //include reflection
                    uint256 percentRewards = _amount.div(pool.runningTotal);
                    uint256 reflectAmount = percentRewards.mul(totalRewards);
