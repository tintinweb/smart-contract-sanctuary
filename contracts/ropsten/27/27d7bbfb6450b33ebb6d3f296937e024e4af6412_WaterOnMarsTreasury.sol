...
contract WaterOnMarsTreasury is Ownable {
    ...
    uint public sWOMQueue; // Delays change to sWOM address
    ...
    //    isLiquidityToken[ _WOMDAI ] = true;
    //    liquidityTokens.push( _WOMDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for WOM
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint WOM needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( WOM ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn WOM for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IWOMERC20( WOM ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( MEMOries ).balanceOf( msg.sender ); // Can only borrow against sWOM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with WOM
        @param _amount uint
     */
    function repayDebtWithWOM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IWOMERC20( WOM ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, WOM, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( WOM ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( WOM ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns WOM valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match WOM decimals
            value_ = _amount.mul( 10 ** IERC20( WOM ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SWOM ) { // 9
            sWOMQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
