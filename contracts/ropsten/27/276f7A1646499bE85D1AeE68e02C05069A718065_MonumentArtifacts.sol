...
contract MonumentArtifacts is NFT, Taxes, ReentrancyGuard {
  ...
  payable
  {
    ...
    _buildMonument("https://monument.app/monuments/0.json");

    // Build Genesis Artifact and Zero Token
    _mintArtifact(0, "https://monument.app/artifacts/0.json", 1);
  }
  ...
  Counters.Counter public totalMonuments;
  ...
  mapping(address => uint256[]) public getMonumentIDsByFounder; // `getMonumentIDsByFounder` logs all the Monument IDs that the address was once a Founder of. If you want to track if that address is the founder of the same Monument currently as well, use `Monument.founder`.
    ...
    returns(address)
  {
    ...
    getMonumentIDsByFounder[_newFounder].push(_monumentID);
    emit MonumentPermissionsModified (
      msg.sender, 
      _newFounder, 
  ...
  function _buildMonument(
    ...
    returns(uint256)
  {
    uint256 newId = totalMonuments.current();
    totalMonuments.increment();
...
contract Splits {
    ...
    constructor(address[] memory _splitters, uint256[] memory _permyriads) {
