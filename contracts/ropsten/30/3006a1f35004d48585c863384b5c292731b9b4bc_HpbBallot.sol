   ...
   */
   ...
   /**
	        ...
	        for(uint m=0;m<candidateIndexArray.length;m++){
	        	 voteStages[_voteStageIndex].candidateArray.push(
	        	     Candidate(
	        	         ...
	        	         voteStages[0].candidateArray[candidateIndexArray[m]].name,
	        	         voteStages[0].candidateArray[candidateIndexArray[m]].facilityId,
	        	         0,new address[](0)
	        	 	 )
	        	 );
    ...
    function getVoteResult(
        ...
        for(uint m=0;m<cl;m++){
             _addrs[m]=voteStages[0].candidateArray[candidateIndexArray[m]].candidateAddr;
             _facilityIds[m]=stringToBytes32(voteStages[0].candidateArray[candidateIndexArray[m]].facilityId);
             _nums[m]=voteStages[0].candidateArray[candidateIndexArray[m]].numberOfVotes;
        }
        return (_addrs,_facilityIds,_nums);
    }
    function getCandidateIndexArray(
    ) onlyVoteAfterStart public constant returns(
        uint[] nums
    ){
        return candidateIndexArray;
    }
    function getCandidateIndexArrayLength(
    ) onlyVoteAfterStart public constant returns(
        uint nums
    ){
        return candidateIndexArray.length;
    }
    ...
    function calVoteResult(
        ...
        for(uint n=0;n<_addrs.length;n++){
           candidateIndexArray.push(voteStages[0].candidateIndexMap[_addrs[n]]);
        }
        return (_addrs,_facilityIds,_nums);
    }
