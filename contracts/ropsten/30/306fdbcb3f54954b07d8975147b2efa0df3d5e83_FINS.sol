...
contract FINS is ERC20 {
    ...
    event Claim(address claimAddress, uint256 amount, uint256 timestamp);

    constructor() {
        //for team
        uint256 teamAmount = (totalSupply() * 100) / 1000;
        uint256 teamFirstUnlockTime = block.timestamp * (86400 * 90);

        _claimData[0x2A3d33EF0b2CdA54A77b4bf8D33622Fb77E6a555] = [
            [teamFirstUnlockTime, teamAmount / 12, 0],
            [teamFirstUnlockTime + (86400 * (30 * 1)), teamAmount / 12, 0],
            [teamFirstUnlockTime + (86400 * (30 * 2)), teamAmount / 12, 0],
            [teamFirstUnlockTime + (86400 * (30 * 3)), teamAmount / 12, 0],
        ...
        //for reserve
        uint256 reserveAmount = (totalSupply() * 50) / 1000;
        uint256 reserveFirstUnlockTime = block.timestamp * (86400 * 90);

        _claimData[0x8cB760fD4A49C792bbA8395C5a6ee798e37758C7] = [
            [reserveFirstUnlockTime, reserveAmount / 12, 0],
            [
                reserveFirstUnlockTime + (86400 * (30 * 1)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 2)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 3)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 4)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 5)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 6)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 7)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 8)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 9)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 10)),
                reserveAmount / 12,
                0
            ],
            [
                reserveFirstUnlockTime + (86400 * (30 * 11)),
                reserveAmount / 12,
                0
            ]
        ];
        ...
        uint256 NISFirstUnlockTime = block.timestamp * (86400 * 90);
        ...
        uint256 APEPFirstUnlockTime = block.timestamp * (86400 * 90);
        ...
        uint256 CBFirstUnlockTime = block.timestamp * (86400 * 90);
        ...
        uint256 RFFirstUnlockTime = block.timestamp * (86400 * 90);
    ...
    function getClaimData(address yourAddress)
        public
        view
        returns (uint256[3][12] memory)
    {
        return _claimData[yourAddress];
    }

    function getCurrentClaimData(address yourAddress)
        internal
        view
        returns (uint256[3] memory, uint256)
    {
        return _getCurrentClaimData(yourAddress);
    }

    function _getCurrentClaimData(address yourAddress)
        internal
        view
        returns (uint256[3] memory result, uint256 i)
    {
        uint256[3][12] memory claimData = _claimData[yourAddress];
        require(claimData.length > 0, "You cannot claim!");
        for (i = 0; i < claimData.length; i++) {
            if (claimData[i][2] == 0) {
                if (claimData[i][0] >= block.timestamp) {
                    result = claimData[i];
                    break;
                }
            }
    ...
    function claim() public returns (bool) {
        (
            ...
            uint256 index
        ) = _getCurrentClaimData(_msgSender());
        require(currentClaimData.length > 0, "You cannot claim anymore!");
        require(
            currentClaimData[0] >= block.timestamp,
            "You cannot claim yet!"
        );

        _transfer(address(this), _msgSender(), currentClaimData[1]);
        _claimData[_msgSender()][index][2] = 1;

        emit Claim(_msgSender(), currentClaimData[1], currentClaimData[0]);
        return true;
    }
}