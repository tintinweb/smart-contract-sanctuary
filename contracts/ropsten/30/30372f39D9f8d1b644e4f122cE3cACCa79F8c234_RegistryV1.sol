...
contract RegistryV1 is RegistryV1Storage, RegistryV1Interface {
    ...
    /// @notice Emitted when a LeN has upgraded it's system version
    event OTokenDeployed(address indexed len, address indexed underlying, address oTokenAddress, address admin);
    ...
    function deployOToken(address underlying,
        ...
        // Only the Unitroller can ask for cToken deployment
        require(isLnRegistered(lnUnitroller), "Not Registered");
        ...
        require(oTokensFactory != address(0));

        address deployedOToken =  OTokensFactoryForRegistry(oTokensFactory).deployODelegator(underlying, contractNameHash, params, lnUnitroller, interestRateModel, admin, becomeImplementationData);

        emit OTokenDeployed(lnUnitroller, underlying, deployedOToken, admin);
...
contract RegistryV0Storage is UnistryAdminStorage {
    ...
    mapping(uint => mapping(bytes32 => address)) public implementations;
...
contract RegistryV1Storage is RegistryV0Storage {
    ...
    mapping(uint => address) public tokenFactories;
}
...
contract Ministry is UnistryAdminStorage {
    ...
    /**
      * @notice Emitted when pendingRegistryImplementation is changed
      */
    event NewPendingImplementation(address oldPendingImplementation, address newPendingImplementation);

    /**
      * @notice Emitted when pendingRegistryImplementation is accepted, which means Registry implementation is updated
      */
...
//        }
        ...
        require(msg.sender == pendingRegistryImplementation && pendingRegistryImplementation != address(0), "Not the EXISTING registry implementation");
