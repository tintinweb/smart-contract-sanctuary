...
contract EthTokenToSmthSwaps {
  ...
  event Withdraw();
  // Set target wallet
  function setTargetWallet(address participantAddress, address newTargetWallet) public returns (address) {
    Swap memory swap = swaps[msg.sender][participantAddress];

	swaps[msg.sender][participantAddress].targetWallet = newTargetWallet;
	return swaps[msg.sender][participantAddress].targetWallet;
  }
