...
contract Celan {
    ...
    // By using this modifier in function only owner invokes otherwise revert
    modifier onlyOwner() {
        require(msg.sender == owner, "Only Owner");
        _;
    }

    // By using this modifier it will revert if there is any function in lock
    modifier isLock() {
        require(lockStatus == false, "Contract Locked");
        _;
    }

    // By using this modifier other Contract should not access our functions
    ...
    */
    function register(uint _referid, uint _level) public isLock isContractcheck(msg.sender) payable {
        ...
        require(_level == 1, "For registration level should be given as 1");
    ...
    */
    function buyLevel(uint _level) public isLock payable {
        require(users[msg.sender].isExist == true, "User not register yet");
        require(msg.value == levelPrice[_level], "Incorrect level price");
        require(users[msg.sender].currentLevel + 1 == _level, "Level should be next to current level");
    ...
    function _paylevel(address _referer, uint _level, uint _amount) internal {
        ...
        if (loopCheck[msg.sender] < referalUpline[_level]) {
            ...
            require(address(uint160(_referer)).send(_amount), "Level commission failed");
    ...
    */
    function poolPercent() public onlyOwner {
        require(poolMembers.length != 0, "No users in pool members");
        uint _poolmember = poolMembers.length.mul(0.1 ether);
        require(poolBonus >= _poolmember, "Not sufficient amount for all members");

        if (poolMembers.length > 0) {
            for (uint i = 0; i < poolMembers.length; i++) {
                if (pollStatus[poolMembers[i]] == false && poolMembers[i] != address(0)) {
                    require(address(uint160(poolMembers[i])).send(poolBonus.div(poolMembers.length)), " Pool transaction failed");
