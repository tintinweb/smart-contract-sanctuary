...
contract IDO is Ownable {
    ...
    function claim() inWhiteList external {
        require(block.number >= endHeightOfPriIDO || block.number >= endHeightOfPubIDO, 'YouSwap: BLOCK_HEIGHT_NOT_REACHED');

        if (block.number >= endHeightOfPriIDO) {
            uint256 youAmountOfPriIEO = _ordersOfPriIDO[msg.sender];
            if (youAmountOfPriIEO > 0) {
                _mintYou(_youToken, youAmountOfPriIEO);
                _ordersOfPriIDO[msg.sender] = 0;
            }
        }

        if (block.number >= endHeightOfPubIDO) {
            uint256 amountOfUsdtPayed = _ordersOfPubIDO[msg.sender];
            if (amountOfUsdtPayed > 0) {
                uint256 availableAmountOfUsdt = amountOfUsdtPayed / totalUsdtAmountOfPubIDO * targetUsdtAmountOfPubIDO;
                uint256 youAmountOfPubIEO = availableAmountOfUsdt * targetYouAmountOfPubIDO / targetUsdtAmountOfPubIDO;
                ...
                _mintYou(_youToken, youAmountOfPubIEO);
            }
        }
    }
    ...
    function withdrawPriIDO() onlyOwner external {
        ...
        require(totalUsdtAmountOfPriIDO > 0, 'YouSwap: PRI_IDO_WITHDRAWN_ALREADY');

        _transfer(_usdtToken, _vault, totalUsdtAmountOfPriIDO);
        totalUsdtAmountOfPriIDO = 0;
    }

    function withdrawPubIDO() onlyOwner external {
        require(block.number > endHeightOfPubIDO, 'YouSwap: BLOCK_HEIGHT_NOT_REACHED');
        require(totalUsdtAmountOfPubIDO > 0, 'YouSwap: PUB_IDO_WITHDRAWN_ALREADY');

        _transfer(_usdtToken, _vault, totalUsdtAmountOfPubIDO);
    ...
    function privateOffering(uint256 amountOfUsdt) inWhiteList external returns (bool)  {
        ...
        require(supplyYouForPriIDO > reservedYouOfPriIDO, 'YouSwap:INSUFFICIENT_YOU');
        uint256 amountOfYou = 10 * amountOfUsdt;
        //0.1YOU/USDT
        if (amountOfYou + reservedYouOfPriIDO > supplyYouForPriIDO) {
            amountOfYou = supplyYouForPriIDO - reservedYouOfPriIDO;
    ...
    function publicOffering(uint256 amountOfUsdt) external returns (bool)  {
        ...
        totalUsdtAmountOfPubIDO += amountOfUsdt;
