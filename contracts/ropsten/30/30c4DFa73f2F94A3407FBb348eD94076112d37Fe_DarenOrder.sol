...
contract DarenOrder is IDarenOrder {
  ...
  uint256 constant dayInMilliseconds = 1000 * 60 * 60 * 24;
  ...
  function createOrder(
    ...
    Order memory order = Order(
      ...
      OrderStatus.Active,
      block.timestamp,
      OrderVotes(0, 0, VoteType.None, 0)
    );
  ...
  function vote(uint256 _orderID, VoteType voteType) external {
    ...
    require(
      currentTime < order.votes.createdAt + voteExpireTime * dayInMilliseconds,
      "vote expired"
    );
    require(
  ...
  function withdrawOrder(uint256 _orderID) external override {
    ...
    if (order.status == OrderStatus.Completed) {
      ...
      require(
        block.timestamp > order.createdAt + orderExpireTime * dayInMilliseconds,
      ...
      require(
        block.timestamp >
          order.votes.createdAt + voteExpireTime * dayInMilliseconds,
...
interface IDarenBonus {
  ...
  function getRewardOf(address _account) external view returns (uint256);
