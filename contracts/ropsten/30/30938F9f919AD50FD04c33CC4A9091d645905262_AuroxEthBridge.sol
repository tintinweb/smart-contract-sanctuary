...
contract AuroxEthBridge is IAuroxBridge {
    ...
    address public constant USDC = address(0x68ec573C119826db2eaEA1Efbfc2970cDaC869c4);
    ...
    */
    function registerSwap(address[] calldata thisTokenPath, address[] calldata targetTokenPath,
        ...
        token.safeIncreaseAllowance(address(uniROUTER), amountIn);
...
interface IAuroxBridge {
    function registerSwap(address[] memory path, address[] memory secondPath, uint256 amountIn, uint minAmountOut) external;
    function buyAssetOnBehalf(address[] memory path, address userAddress, uint256 usdcAmount) external;
    function issueOwnershipNft(address userAddress, address token, uint256 amount) external;

    event BuyAssetOnBehalf(address, address, uint, uint);
