...
contract TokenRecipient {
    ...
    event FallbackEther(address indexed sender, uint amount, string message);
    ...
    fallback () external payable {
        emit FallbackEther(msg.sender, msg.value, "Fallback");
    }
}
...
contract AuthenticatedProxy is TokenRecipient, OwnedUpgradeabilityStorage {
        ...
        returns (bool result)
    {
        ...
        if (howToCall == HowToCall.Call) {
            (bool res, bytes memory ret_data) = dest.call(call_data);
            result = res;
        } else if (howToCall == HowToCall.DelegateCall) {
            (bool res, bytes memory ret_data) = dest.delegatecall(call_data);
            result = res;
        } else {
          result = false;
        }
...
contract OwnedUpgradeabilityProxy is Proxy {
  ...
  */
  ...
  function upgradeToAndCall(address implementation, bytes memory data) payable public onlyProxyOwner {
    ...
    (bool res, bytes memory ret_data) = address(this).delegatecall(data);
    require(res == true);
  }
}
...
contract OwnableDelegateProxy is OwnedUpgradeabilityProxy {
    ...
    constructor(address owner, address initialImplementation, bytes memory call_data)
    {
        ...
        (bool res, bytes memory ret_data) = initialImplementation.delegatecall(call_data);
...
/**
 * @title NFTMinter
 * NFTMinter - ERC721 contract that whitelists a trading address, and has minting functionality.
 */
contract NFTMinter is ERC721, ContextMixin, NativeMetaTransaction, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    ) ERC721("MysticBets Jersey Token", "MBJT") {
     ...
     * @param _to address of a new fee recipient
     */
    function setFeeRecipient(address _to) public {
        feeRecipient = _to;
    }
     ...
     */
    function mintTo(address _to, string memory uri) public payable nonReentrant {
        ...
        require(msg.value >= 25 * (10 ** 15), "fee is not sufficient to mint");

        uint256 remnant = msg.value - (25 * (10 ** 15));

        address payable fr = payable(feeRecipient);
        fr.transfer(25 * (10 ** 15));

        fr = payable(_msgSender());
        fr.transfer(remnant);
    }
    ...
    function burn(uint256 _tokenId) public {
        require(_msgSender() == ownerOf(_tokenId) || _msgSender() == owner());
        _burn(_tokenId);
    }
