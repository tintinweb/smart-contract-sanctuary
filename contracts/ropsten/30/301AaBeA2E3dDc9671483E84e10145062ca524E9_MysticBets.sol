...
contract MysticBets is ERC721, Ownable {
    ...
    event Minted(address indexed minter, uint price, uint nftNum);
    ...
    function mint(uint _nftNum) public payable {
        require(msg.value >= mbjPrice * _nftNum, "Insufficient funds");
        require(totalSupply() + _nftNum < MAX_MBJS, "Fulfilled limit");

        for (uint i; i < _nftNum; i ++) {
            
            uint _id = totalSupply() + 1;
            price[_id] = mbjPrice / _nftNum;
            listedMap[_id] = true;
        ...
        emit Minted(msg.sender, mbjPrice / _nftNum, _nftNum);
    }
    ...
    function getTokenURI() private view returns(string memory URI){
        URI = string(abi.encodePacked("https://ipfs.io/ipfs/QmPBGhBbRjd8TMkZ6fwr3ukDMaEvjCUMvt7gRfvTnfMWoa/", totalSupply().toString() , ".json" ));
    }
}