...
contract DT is Context, Ownable, IERC20, LockableSwap, FeeLibrary, EthReflectingToken {
    ...
    constructor (string memory NAME, uint256 _supply, uint256 _MAXTXAMOUNT, uint256 ADDTOLIQUIDITYTHRESHOLD,
        address tokenOwner, address projectWallet, address marketingWallet) payable {
        ...
        emit Transfer(address(0), tokenOwner, _tOwned[tokenOwner]);
...
//        emit BuyBackRangeUpdated(minBuyBack, _minBuyBackEther, maxBuyBack, _maxBuyBackEther);
    ...
    function init(address routerAddress, address reflectorAddress) public onlyOwner{
        require(runOnce == 1, "This function can only ever be called once");
        emit UpdateRouter(routerAddress, address(uniswapV2Router));
...
//        try reflectionContract.reflectRewards() {} catch {}
    }
