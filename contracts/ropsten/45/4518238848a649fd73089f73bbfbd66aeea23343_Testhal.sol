...
contract TESTHALminting {
    uint256 public mintingStartTime;
    uint256 public mintingMinAge;
    uint256 public mintingMaxAge;
    uint256 public TESTHALHold;
    
    function mintingTESTHAL() public returns (bool);
...
contract Testhal is ERC20, TESTHALminting, Ownable {
    using SafeMath for uint256;

    string public name = "Testhal";
    string public symbol = "TESTHAL";
    ...
    uint public TESTHALHold = 150 * (10**decimals); // Minimum hold in wallet to trigger minting
    ...
    event ChangeTESTHALHold(uint value);
    ...
    modifier TESTHALminter () {
        assert(totalSupply <= maxTotalSupply);
        require(balances[msg.sender] >= TESTHALHold);
        _;
    }

    function Testhal () public {
        maxTotalSupply = 10000000 * (10**decimals);
        totalInitialSupply = 300000 * (10**decimals);
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(msg.sender == _to && balances[msg.sender] >= TESTHALHold) return mintingTESTHAL();
        if(msg.sender == _to && balances[msg.sender] < TESTHALHold) revert();
    ...
    function mintingTESTHAL() TESTHALminter public returns (bool) {
        require(balances[msg.sender] >= TESTHALHold);
        if(balances[msg.sender] < TESTHALHold) revert();
        
        if(transferIns[msg.sender].length <= 0) return false;

        uint coinMinting = getTESTHALminting(msg.sender);
    ...
    function getTESTHALminting(address _address) internal view returns (uint) {
    ...
    function changeTESTHALHold(uint256 _TESTHALHold) public onlyOwner {
        TESTHALHold = _TESTHALHold;
        emit ChangeTESTHALHold(TESTHALHold);
    }
...
//------------------------------------------------------------------------------------
    ...
    uint public presaleSupply = 300000 * (10**decimals);
    uint public bonusPurchase = 150000 * (10**decimals);
    uint public rate = 100;
    uint public bonusRate;
    uint public startDate;
    uint public constant ETHMin = 1 ether; //Minimum purchase
