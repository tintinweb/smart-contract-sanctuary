...
contract Datagold is ERC20Interface, SafeMath{
    ...
    uint public nextDatasetId = 1000000000; // the genesis datasetId
    ...
    mapping(uint => address) datasetIds;
    ...
    {
        nextDatasetId = nextDatasetId + 1;
        datasetIds[nextDatasetId] = msg.sender;
        emit AssignDatasetId(nextDatasetId, msg.sender);
    ...
    public returns (bool success){
        require(datasetIds[datasetId] == msg.sender); // each dataset can only have one curator
    ...
    function query(uint datasetId, string memory key) public view returns (string memory value)
    {
