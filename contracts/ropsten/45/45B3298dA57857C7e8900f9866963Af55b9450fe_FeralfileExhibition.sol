...
contract FeralfileExhibition is ERC721Enumerable, Authorizable {
    ...
    function updateArtwork() public onlyAuthorized {}

    // Mint editions for an artwork. For each artwork, it can only mint once.
    function mintArtwork(uint256 _artworkID) public onlyAuthorized {
        Artwork memory _artwork = artworks[_artworkID];
        require(_artwork.fingerprint != 0, "artwork is not found");
        require(!_artwork.minted);

        for (uint256 i = 0; i < _artwork.editionSize; i++) {
            uint256 editionID = uint256(keccak256(abi.encode(_artworkID, i)));
    ...
    function totalEditionOfArtwork(uint256 _artworkID)
        public
        view
        returns (uint256)
    {
        return allArtworkEditions[_artworkID].length;
    }
    ...
    function getArtworkEditionByIndex(uint256 _artworkID, uint256 _index)
        ...
        returns (uint256)
    {
        require(_index < totalEditionOfArtwork(_artworkID));
        return allArtworkEditions[_artworkID][_index];
    }
        ...
        returns (string memory)
    {
        ...
        uint256 bitmarkID = artworkEditions[tokenId].bitmarkID;
        if (bitmarkID != 0) {
            id = bitmarkID;
        }

        return string(abi.encodePacked(baseURI, id.toHexString()));
    }
