...
contract Management is Ownable {
    ...
    (
        ...
        creator = _transactions[transactionId].creator;
        teamId = _transactions[transactionId].teamId;
        teamAccount = _transactions[transactionId].teamAccount;
        newTeamAccount = _transactions[transactionId].newTeamAccount;
        teamAgent = _transactions[transactionId].agent;
        newTeamAgent = _transactions[transactionId].newAgent;
...
contract TokenDispatch is Management {
    ...
    function reviewTeam(uint8 teamId)  external view returns
    (
        address account,
        address agent,
        uint256 reserved,
    ...
    function claim(uint8 teamId) public {
    ...
    function nextClaimHeight(uint8 teamId) public view returns (uint256) {
        return _teams[teamId].lastClaimedHeight + blocksOfMonth;
    }

    function claimed(uint8 teamId) public view returns (uint256) {
        return _teams[teamId].claimed;
    }

    function balanceOf(uint8 teamId) public view returns (uint256) {
        require(_teams[teamId].agent == msg.sender, 'YouSwap: NOT_ALLOWED');
