...
library Arrays {
    function inArray(uint256[] memory array, uint256 element)
        internal
        pure
        returns (bool)
    {
...
contract LordAngels is ERC721, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("LordAngels", "ANG") {
    ...
    function mint(uint256[] memory _tokenIds) public {
        ...
        require(
            ...
            "Sorry, NFT was already claimed for this token(s)."
        );
    ...
    function validateTokenIds(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (!_existTokenIds.inArray(_tokenIds[i])) {
                return false;
            }
        }
