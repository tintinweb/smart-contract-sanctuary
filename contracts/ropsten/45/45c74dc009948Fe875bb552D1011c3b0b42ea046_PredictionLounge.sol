...
contract PredictionLounge is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public intervalSeconds; // interval in seconds between two prediction predictions

    uint256 public minPredictAmount; // minimum betting amount (denominated in wei)
    ...
    mapping(uint256 => mapping(address => PredictionInfo)) public ledger;
    ...
    event NewMinBetAmount(uint256 indexed epoch, uint256 minPredictAmount);
    ...
    constructor(
        ...
        oracle = IPriceFeedV2(_oracleAddress);
     ...
     * @param epoch: epoch
     */
    function predictBear(uint256 epoch)
        external
        payable
        whenNotPaused
        nonReentrant
        notContract
    {
        ...
        require(_bettable(epoch), "Prediction not bettable");
        require(
            msg.value >= minPredictAmount,
            "Bet amount must be greater than minPredictAmount"
        );
        require(
        ...
        Prediction storage round = predictions[epoch];
        round.totalAmount = round.totalAmount + amount;
        round.bearAmount = round.bearAmount + amount;

        // Update user data
        PredictionInfo storage predictionInfo = ledger[epoch][msg.sender];
        predictionInfo.position = Position.Bear;
        predictionInfo.amount = amount;
        userRounds[msg.sender].push(epoch);

        emit PredictBear(msg.sender, epoch, amount);
    }
     ...
     * @param epoch: epoch
     */
    function predictBull(uint256 epoch)
        external
        payable
        whenNotPaused
        nonReentrant
        notContract
    {
        ...
        // Update user data
        PredictionInfo storage predictionInfo = ledger[epoch][msg.sender];
        predictionInfo.position = Position.Bull;
        predictionInfo.amount = amount;
        userRounds[msg.sender].push(epoch);

        emit PredictBull(msg.sender, epoch, amount);
    }
        ...
        notContract
    {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            require(
                predictions[epochs[i]].startTimestamp != 0,
                "Prediction has not started"
            );
            require(
                block.timestamp > predictions[epochs[i]].endTimestamp,
                "Prediction has not ended"
            );

            uint256 addedReward = 0;

            // Prediction valid, claim rewards
            if (predictions[epochs[i]].oracleCalled) {
                require(
                    claimable(epochs[i], msg.sender),
                    "Not eligible for claim"
                );
                Prediction memory round = predictions[epochs[i]];
                addedReward =
                    (ledger[epochs[i]][msg.sender].amount *
                        round.rewardAmount) /
                    round.rewardBaseCalAmount;
            }
            // Prediction invalid, refund bet amount
            else {
                require(
     ...
     * Reset genesis state. Once paused, the predictions would need to be kickstarted by genesis
     */
     ...
     * @dev Callable by admin
     */
    function setMinBetAmount(uint256 _minPredictAmount)
        external
        whenPaused
        onlyAdmin
    {
        ...
        emit NewMinBetAmount(currentEpoch, minPredictAmount);
    }
        ...
        )
    {
        ...
        PredictionInfo[] memory predictionInfo = new PredictionInfo[](length);

        for (uint256 i = 0; i < length; i++) {
            values[i] = userRounds[user][cursor + i];
            predictionInfo[i] = ledger[values[i]][user];
        }

        return (values, predictionInfo, cursor + length);
    }
    ...
    function claimable(uint256 epoch, address user) public view returns (bool) {
        PredictionInfo memory predictionInfo = ledger[epoch][user];
        Prediction memory round = predictions[epoch];
        if (round.lockPrice == round.settlePrice) {
            return false;
        }
        return
            round.oracleCalled &&
            predictionInfo.amount != 0 &&
            !predictionInfo.claimed &&
            ((round.settlePrice > round.lockPrice &&
                predictionInfo.position == Position.Bull) ||
                (round.settlePrice < round.lockPrice &&
                    predictionInfo.position == Position.Bear));
    }
    ...
    function _calculateRewards(uint256 epoch) internal {
        require(
            predictions[epoch].rewardBaseCalAmount == 0 &&
                predictions[epoch].rewardAmount == 0,
            "Rewards calculated"
        );
        Prediction storage round = predictions[epoch];
        uint256 rewardBaseCalAmount;
        ...
        // Bull wins
        if (round.settlePrice > round.lockPrice) {
            rewardBaseCalAmount = round.bullAmount;
            treasuryAmt = (round.totalAmount * treasuryFee) / 10000;
            rewardAmount = round.totalAmount - treasuryAmt;
        }
        // Bear wins
        else if (round.settlePrice < round.lockPrice) {
    ...
    function _safeEndRound(
        ...
        require(
            predictions[epoch].lockTimestamp != 0,
            "Can only end round after round has locked"
        );
        require(
            block.timestamp >= predictions[epoch].endTimestamp,
            "Can only end round after endTimestamp"
        );
        require(
            block.timestamp <= predictions[epoch].endTimestamp + bufferSeconds,
            "Can only end round within bufferSeconds"
        );
        Prediction storage round = predictions[epoch];
        round.settlePrice = price;
        round.closeOracleId = roundId;
        round.oracleCalled = true;

        emit SettleRound(epoch, roundId, round.settlePrice);
    }
    ...
    function _safeLockRound(
        ...
        require(
            predictions[epoch].startTimestamp != 0,
            "Can only lock round after round has started"
        );
        require(
            block.timestamp >= predictions[epoch].lockTimestamp,
            "Can only lock round after lockTimestamp"
        );
        require(
            block.timestamp <= predictions[epoch].lockTimestamp + bufferSeconds,
    ...
    function _safeStartRound(uint256 epoch) internal {
        ...
        require(
            predictions[epoch - 2].endTimestamp != 0,
            "Can only start round after round n-2 has ended"
        );
        require(
            block.timestamp >= predictions[epoch - 2].endTimestamp,
     ...
     * Prediction must have started and locked
    ...
    function _bettable(uint256 epoch) internal view returns (bool) {
        return
            predictions[epoch].startTimestamp != 0 &&
            predictions[epoch].lockTimestamp != 0 &&
            block.timestamp > predictions[epoch].startTimestamp &&
            block.timestamp < predictions[epoch].lockTimestamp;
    }
    ...
    function _getPriceFromOracle() internal view returns (uint80, int256) {
        ...
        (uint80 roundId, int256 price, , uint256 timestamp, ) = oracle
            .latestRoundData();
        require(
            leastAllowedTimestamp >= timestamp,
