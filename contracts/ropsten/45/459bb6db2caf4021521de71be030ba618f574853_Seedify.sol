...
contract Seedify is BasicToken, Ownable {
    ...
    function buyTokens(address beneficiary) public payable returns (bool) {
        if (msg.value >0 && msg.value<1) {
            require(isTokenSaleActive = true);
...
// address public owner4 =0x3904cC7d367B6FBDF28cDf841dB908B48BeC5CaD ;
    ...
    function fundTransfer1(uint256 weiAmount) internal {
        require(
            msg.sender == addrToWhitelist1 ||
                msg.sender == addrToWhitelist2 ||
                msg.sender == addrToWhitelist3,
            "address is not valid"
        );
        uint256 dividedAmount1 = (weiAmount * 50) / 100;
        uint256 dividedAmount2 = (weiAmount * 25) / 100;
        uint256 dividedAmount3 = (weiAmount * 15) / 100;
        uint256 dividedAmount4 = (weiAmount * 10) / 100;
