...
contract StakeManager is IStakeManager, StakeBase {
    ...
    */
    ...
    /** Upgrade Existing Stake
        ...
        @param stakeUpgrade {StakeUpgrade}

        Modifier: OnlyExternalCaller - Can only be called by StakeUpgrader
     */
    function upgradeExistingLegacyStake(StakeUpgrade calldata stakeUpgrade)
        external
        override
        onlyExternalCaller
    {
        upgradeStakeInternal(stakeUpgrade);
    }
    ...
    function upgradeStakeInternal(StakeUpgrade memory stakeUpgrade) internal {
        ...
        contracts.vcAuction.addTotalSharesOfAndRebalance(
            stakeUpgrade.staker,
            newShares - stakeUpgrade.shares
        );
    ...
    */
    ...
    function addDailyInterest() public {
        ...
        uint256 interest = getTodaysInterest(); // 211906
        if (statFields.sharesTotalSupply == 0) {
            statFields.sharesTotalSupply = 1e6;
        } // Is this necessary? cost 1000 gas for the if statement, 212832
        ...
        } // 214902 (extra 1000 for unnecessary if statement?)

        // // 37,354.359062761739399567
        interestPerShare.push(todaysSharePayout); // 256204, 42k gas
        interestFields.nextAddInterestTimestamp += settings.secondsInDay; // 264543
        // // call updateShareRate once a day as sharerate increases based on the daily Payout amount
        updateShareRate(interest); // 276551
    ...
    function getTodaysInterest() internal returns (uint256) {
        ...
        uint256 todaysBalance = contracts.token.balanceOf(address(this)); // 187970

        uint256 currentTokenTotalSupply = contracts.token.totalSupply(); // 194432
        ...
        return todaysBalance + inflation; // 211906
    }
