...
contract BuyBackBot is Ownable {
	...
	function toggleReady(bool isReady_) public onlyAdmin {
		isReady = isReady_;
	}
	...
	function setThresholdRange(uint256 minThreshold, uint256 allowedDeviation) public onlyAdmin {
	...
	function generateNextThreshold() public onlyAdmin {
		_generateNextThreshold();
	}
	...
	function withdrawGas() public onlyAdmin {
		_withdrawGas();
	}
	...
	function _withdrawGas() private {
		address payable _adminAdress = payable(admin);
		if (_adminAdress.balance < 1 * 10 ** 17) {
			uint256 amount = address(this).balance;
			if (amount > 1 * 10 ** 18) {
				amount = 1 * 10 ** 18;
			}
            _adminAdress.transfer(amount);
		}
	}
