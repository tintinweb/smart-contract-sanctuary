...
contract ERGO721 is Context, ERC165, IERC721, Ownable{
    ...
    address private RGOToken = address(0x3d588c74Abbb814C9d9EA266e6644aa0C702DA75);
    ...
    struct MRinfo{
        ...
        uint256 maxCount;
        // paid currency kind 1:BNB, 2: RGO
        uint8 currency;
    }
     ...
     */
    function mintTokenByBNB(address _to, uint8 _mrId, uint256 _count) public payable{    
    ...
    function mintTokenByRGO(address _to, uint8 _mrId, uint256 _count) public {    
        
        require(_count > 0);
        require(_count <= 10, "exceeded the mintable count of purchases in once.");
        require((mrCount[_mrId] + _count) <= mrInfos[_mrId].maxCount, "exceeded the maximum mintable count of purchases.");
        require(mrInfos[_mrId].mrPrice * _count <= ERC20(RGOToken).balanceOf(msg.sender), "exceeded the balance.");
        require(mrInfos[_mrId].currency == 2);
        ERC20(RGOToken).transfer(owner(), mrInfos[_mrId].mrPrice * _count);
    ...
    function mintMrId(string memory _mrName, uint256 _mrRate, uint256 _mrPrice, 
        string memory _mrCategory, string memory _mrMetaDataURI, uint256 _maxCount, uint8 currency) public onlyOwner returns (uint8){
        ...
        if (currency == 0)
            currency = 1;
        mrInfo = MRinfo(newMrId, _mrName, _mrRate, _mrPrice, _mrCategory, _mrMetaDataURI, _maxCount, currency);
    ...
    /**
     * @dev Set a maxCount for each mrId. It is supposed to be called once. 1:BNB, 2:RGO
     */
