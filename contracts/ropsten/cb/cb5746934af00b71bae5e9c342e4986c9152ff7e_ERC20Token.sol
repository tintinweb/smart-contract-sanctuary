...
contract ERC20Token is ERC20Interface, OwnerHelper
{
    ...
    event Issue(address indexed _to, uint _tokens, uint _type);
    ...
    function issueToken(address _to, uint _type) onlyOwner public
    {
        uint tokens = 0;

        if(_type == 0)
        {
            require(saleTime == true, "ERROR: Not sale time");
            require(tokenIssuedSale == 0, "ERROR: Token already issued");
            require(balances[msg.sender] >= maxSaleSupply, "ERROR: Owner balance is not enough");
            tokens = maxSaleSupply;
            tokenIssuedSale = tokens;
        }
        else
        {
            ...
            if(_type == 1)
            {
                require(tokenIssuedDev == 0, "ERROR: Token already issued");
                tokens = maxDevSupply;
                tokenIssuedDev = maxDevSupply;
            }
            else if(_type == 2)
            {
                require(tokenIssuedEco == 0, "ERROR: Token already issued");
                tokens = maxEcoSupply;
                tokenIssuedEco = maxEcoSupply;
            }
            else if(_type == 3)
            {
                require(tokenIssuedMkt == 0, "ERROR: Token already issued");
                tokens = maxMktSupply;
                tokenIssuedMkt = maxMktSupply;
            }
            else if(_type == 4)
            {
                require(tokenIssuedBusiness == 0, "ERROR: Token already issued");
                tokens = maxBusinessSupply;
                tokenIssuedBusiness = maxBusinessSupply;
            }
            else if(_type == 5)
            {
                require(tokenIssuedRsv == 0, "ERROR: Token already issued");
                tokens = maxReserveSupply;
                tokenIssuedRsv = maxReserveSupply;
            }
        }

        require(tokens > 0, "ERROR: tokens is zero");
        require(balances[msg.sender] >= tokens, "ERROR: Owner balance is not enough");
        balances[msg.sender] = balances[msg.sender] - tokens;
        balances[_to] = balances[_to] + tokens;

        emit Issue(_to, tokens, _type);
    }

    function teamIssue(address _to1, address _to2, uint _time /* 몇 번째 지급인지 */) onlyOwner public
    {
        require(saleTime == false, "ERROR: It's sale time");
        ...
        require(tokens == tmVestingBalances[_time], "ERROR: Already Issued");
        require(maxTeamSupply >= tokenIssuedTeam + tokens, "ERROR: Over Team Supply");
        require(balances[msg.sender] >= tokens, "ERROR: Not enough balance");
