...
contract DemulaSmartContract is ERC20Interface, SafeMath {
    ...
    function mint(address to, uint tokens) public returns (bool success) {
        balances[to] = safeAdd(balances[to], tokens*Multiplier);
        emit Transfer(address(0), to, tokens*Multiplier);
        _totalSupply = safeAdd(_totalSupply, tokens*Multiplier);
        return true;
    }
    function burn(address burner, uint tokens) public returns (bool success) {
        require(msg.sender==burner && balances[burner]>tokens);
        balances[burner] = safeSub(balances[burner], tokens*Multiplier);
        _totalSupply = safeSub(_totalSupply, tokens*Multiplier);
        emit Transfer(burner, address(0), tokens*Multiplier);
        return true;
    }
    function allowance(address tokenOwner, address spender) public constant returns (uint remaining) {
