...
contract rarity_attributes {
    ...
    uint constant POINT_BUY = 32;//买点
    rarity constant rm = rarity(0xc4B6C3E745313384072Cc0CcaC56ad2a40459855);
    
    struct ability_score {//属性值
        uint32 strength;//力量
        uint32 dexterity;//灵巧
        uint32 constitution;//体质
        uint32 intelligence;//智力
        uint32 wisdom;//智慧
        uint32 charisma;//魅力
    }
    
    mapping(uint => ability_score) public ability_scores;//属性值
    mapping(uint => uint) public level_points_spent;//花费的等级属性点数
    mapping(uint => bool) public character_created;//创建的角色
    ...
    function point_buy(uint _summoner, uint32 _str, uint32 _dex, uint32 _const, uint32 _int, uint32 _wis, uint32 _cha) external {
        require(_isApprovedOrOwner(_summoner));//被批准或所有者
        require(!character_created[_summoner]);//创建的角色
        require(calculate_point_buy(_str, _dex, _const, _int, _wis, _cha) == POINT_BUY);//计算积分购买
        character_created[_summoner] = true;
        
        ability_scores[_summoner] = ability_score(_str, _dex, _const, _int, _wis, _cha);//能力分数
        emit Created(msg.sender, _summoner, _str, _dex, _const, _int, _wis, _cha);//已创建
    }
    ...
    function _increase_base(uint _summoner) internal {
        require(_isApprovedOrOwner(_summoner));//被批准或所有者
        require(character_created[_summoner]);//创建的角色
        uint _points_spent = level_points_spent[_summoner];//花费的点数
        require(abilities_by_level(rm.level(_summoner)) - _points_spent > 0);//能力等级
        level_points_spent[_summoner] = _points_spent+1;//花费的等级点数
    }
    
    //增加力量
    function increase_strength(uint _summoner) external {
        _increase_base(_summoner);//增加基数
        ability_score storage _attr = ability_scores[_summoner];//属性值
        _attr.strength = _attr.strength+1;//力量+1
        //水平
    ...
    function increase_dexterity(uint _summoner) external {
        _increase_base(_summoner);//增加基数
        ability_score storage _attr = ability_scores[_summoner];//属性值
        _attr.dexterity = _attr.dexterity+1;//灵巧+1
        //水平
        emit Leveled(msg.sender, _summoner, _attr.strength, _attr.dexterity, _attr.constitution, _attr.intelligence, _attr.wisdom, _attr.charisma);
    }
    ...
    function increase_constitution(uint _summoner) external {
        _increase_base(_summoner);//增加基数
        ability_score storage _attr = ability_scores[_summoner];//属性值
        _attr.constitution = _attr.constitution+1;//体质+1
        //水平
        emit Leveled(msg.sender, _summoner, _attr.strength, _attr.dexterity, _attr.constitution, _attr.intelligence, _attr.wisdom, _attr.charisma);
    }
    ...
    function increase_intelligence(uint _summoner) external {
        _increase_base(_summoner);//增加基数
        ability_score storage _attr = ability_scores[_summoner];//属性值
        _attr.intelligence = _attr.intelligence+1;//智力+1
        //水平
        emit Leveled(msg.sender, _summoner, _attr.strength, _attr.dexterity, _attr.constitution, _attr.intelligence, _attr.wisdom, _attr.charisma);
    }
    ...
    function increase_wisdom(uint _summoner) external {
        _increase_base(_summoner);//增加基数
        ability_score storage _attr = ability_scores[_summoner];//属性值
        _attr.wisdom = _attr.wisdom+1;//智慧+1
        //水平
        emit Leveled(msg.sender, _summoner, _attr.strength, _attr.dexterity, _attr.constitution, _attr.intelligence, _attr.wisdom, _attr.charisma);
    }
    ...
    function increase_charisma(uint _summoner) external {
        _increase_base(_summoner);//增加基数
        ability_score storage _attr = ability_scores[_summoner];//属性值
        _attr.charisma = _attr.charisma+1;//魅力+1
        //水平
        emit Leveled(msg.sender, _summoner, _attr.strength, _attr.dexterity, _attr.constitution, _attr.intelligence, _attr.wisdom, _attr.charisma);
    }
