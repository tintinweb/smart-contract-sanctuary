...
// File: contracts/CashfreeToken.sol
...
contract CashfreeToken is MintableToken, PausableToken, StandardBurnableToken, DetailedERC20 {
  ...
  /**
   * @dev Initialize the CashfreeToken and transfer the initialBalance to the
   *      initialAccount.
   */
  constructor(address initialAccount, uint256 initialBalance)
    DetailedERC20("Cashfree", "CASH", 18)
    public
  {
  ...
  modifier canTransfer() {
    require(
      transferable
      || (crowdsale != address(0) && crowdsale == msg.sender)
      || (owner == msg.sender)
    );
    _; 
  }
