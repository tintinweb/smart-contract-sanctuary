...
contract KandyTreasury is Ownable {
    ...
    address public immutable Kandy;
    ...
    address public sKANDY;
    uint public sKANDYQueue; // Delays change to sKANDY address
    ...
    //    isLiquidityToken[ _KANDYDAI ] = true;
    //    liquidityTokens.push( _KANDYDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for KANDY
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) public returns ( uint send_ ) {
        ...
        // mint KANDY needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Kandy ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn KANDY for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IKANDYERC20( Kandy ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sKANDY ).balanceOf( msg.sender ); // Can only borrow against sKANDY held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with KANDY
        @param _amount uint
     */
    function repayDebtWithKANDY( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IKANDYERC20( Kandy ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Kandy, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Kandy ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Kandy ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns KANDY valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match KANDY decimals
            value_ = _amount.mul( 10 ** IERC20( Kandy ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SKANDY ) { // 9
            sKANDYQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
