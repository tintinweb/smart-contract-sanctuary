...
contract Payment is Ownable {
    ...
    function addCustomer(address _customerAddress) private onlyOwner isCustomerNotAdded(_customerAddress) checkSecondaryAddress(_customerAddress) returns (bool){
    ...
    function addCustomerTokenAmount(address _customerAddress, uint _tokenAmount) private onlyOwner isCustomerAdded(_customerAddress) returns (bool){
    ...
    function getCustomerInfo(address _customerAddress) private view onlyOwner isCustomerAdded(_customerAddress) returns(bool, uint, uint){
        Customer memory c = customers[_customerAddress];
        return (c.isLocked, c.weiBalance, c.tokenBalance);
    }
    
    function addSecondaryAddress(address _customerAddress, address _secondaryAddress) private onlyOwner isCustomerAdded(_customerAddress) 
    ...
    function deleteSecondaryAddress(address _customerAddress, address _secondaryAddress) private onlyOwner isCustomerAdded(_customerAddress)
    ...
    function getLinkedAddress(address _address) private view onlyOwner isValidAddress(_address) returns(address){
    ...
    function transferETHtoProviders(address _customerAddress, address _providerCA, uint _providerFee, address _creoCA, uint _creoFee )
        private onlyOwner isCustomerAdded(_customerAddress) isProviderAdded(_providerCA) isProviderAdded(_creoCA) returns(bool){
    ...
    function setProvider(address _providerCA, uint _weiLimit, uint _tokenLmit) private onlyOwner isContract(_providerCA) 
        isProviderNotAdded(_providerCA) returns(bool){
            providers[_providerCA] = Provider(_providerCA, false, 0, _tokenLmit, 0, _weiLimit);
            return true;
    }
    
    function setProviderLimits(address _providerCA, uint _weiLimit, uint _tokenLmit) private onlyOwner isContract(_providerCA) 
    ...
    function getProviderInfo(address _providerCA) private view onlyOwner isContract(_providerCA)
    ...
    function lockedProvider(address _providerCA) private onlyOwner isProviderAdded(_providerCA) isUnLockedProvider(_providerCA) returns(bool){
        Provider storage p = providers[_providerCA];
        p.isLocked = true;
        return true;
    }
    
    function unlockedProvider(address _providerCA) private onlyOwner isProviderAdded(_providerCA) isLockedProvider(_providerCA) returns(bool){
        Provider storage p = providers[_providerCA];
        p.isLocked = false;
        return true;
    }
    ...
    function refundETHToCustomer(address _customerAddress, uint _refundAmount) private onlyOwner isCustomerAdded(_customerAddress) isAvailableETHBalance(_refundAmount) returns(bool){
        Customer storage c = customers[_customerAddress];
    ...
    function refundTokenToCustomer(address _customerAddress, uint _refundAmount) private onlyOwner isCustomerAdded(_customerAddress) isAvailableTokenBalance(_refundAmount) returns(bool){
        Customer storage c = customers[_customerAddress];
    ...
    function payProviderFee(address _providerWallet) private isProviderAdded(msg.sender) isValidAddress(_providerWallet){
