...
contract Soup is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() public ERC721("Soup", "SOUP") {}
        ...
        string memory json = encode(
            string(
                abi.encodePacked(
                    '{"name": "Plot #',
                    toString(tokenId),
                    '", "description": "Soup makes soup", "image": "data:image/svg+xml;base64,',
                    encode(output),
                    '"}'
                )
            )
    ...
    function getPlot(uint256 tokenId) internal pure returns (string memory) {
