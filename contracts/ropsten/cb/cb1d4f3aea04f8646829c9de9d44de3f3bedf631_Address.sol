...
/**
*Rug Defense Foundation
*/

/**
  Telegram: https://t.me/+0nejVhLtOPM1Nzlh
*/
...
contract RugDefenseFoundation  is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0xa86AD65959fcA5fd92b78cb601aE6c2626415C84;
    address public devWallet = 0xc79dD9547Cf58c10deaddaC2334c7582469F5C7D;
    address public donationWallet = 0x4D1de6d2ea988AA81A6196BFCe82a1a40bcD092e;


    string private constant _name = "RugDefenseFoundation" ;
    string private constant _symbol = "RDF";
    ...
    Taxes public taxes = Taxes(1, 1, 4, 1, 1);
    Taxes public sellTaxes = Taxes(1, 5, 5, 1, 1);
    ...
    //@dev kept original reflections naming -> "reward" as in reflection
    ...
    function setTaxes(uint256 _reflections, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _donation) public onlyOwner {
       taxes = Taxes(_reflections,_marketing,_liquidity,_dev,_donation);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _reflections, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _donation) public onlyOwner {
       sellTaxes = Taxes(_reflections,_marketing,_liquidity,_dev,_donation);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rreflections, to_return.rMarketing, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
        (to_return.rDev,to_return.rdonation) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.treflections-s.tMarketing-s.tLiquidity-s.tDev-s.tdonation;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rreflections,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rreflections-rMarketing-rLiquidity-rDev-rdonation;
        return (rAmount, rTransferAmount, rreflections,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rDev, uint256 rdonation) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tDev + s.tdonation);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.dev + temp.donation) * 2;
