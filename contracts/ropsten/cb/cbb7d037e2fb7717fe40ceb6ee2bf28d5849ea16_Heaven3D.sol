...
contract Heaven3D is modularLong {
	...
	PlayerBookInterface constant private PlayerBook = PlayerBookInterface(0x7579BB8F9b718EF68bDb99d119db0BD1661f40Dc);
//==============================================================================
//     _ _  _  |`. _     _ _ |_ | _  _  .
...
// FLOW CONTROL
//****************	
	bool private endRoundDecisionResult = false;
	address private address_of_last_rand_gen_source = address(0);	
//****************
// PLAYER DATA 
//****************
...
// TEAM FEE DATA 
//****************
    mapping (uint256 => H3Ddatasets.TeamFee) public fees_;          // (team => fees) fee distribution by team
    mapping (uint256 => H3Ddatasets.PotSplit) public potSplit_;     // (team => fees) pot split distribution by team
//==============================================================================
...
//==============================================================================
        ...
        public
    {
        ...
        // (H3D, P3D) + (Pot , Referrals, Community)
            // Referrals / Community rewards are mathematically designed to come from the winner&#39;s share of the pot.
        fees_[0] = H3Ddatasets.TeamFee(30,6);   //50% to pot, 10% to aff, 2% to com, 1% to pot swap, 1% to air drop pot // Whale
        fees_[1] = H3Ddatasets.TeamFee(43,0);   //43% to pot, 10% to aff, 2% to com, 1% to pot swap, 1% to air drop pot	// Bear
        fees_[2] = H3Ddatasets.TeamFee(56,10);  //20% to pot, 10% to aff, 2% to com, 1% to pot swap, 1% to air drop pot // Snek
        fees_[3] = H3Ddatasets.TeamFee(43,8);   //35% to pot, 10% to aff, 2% to com, 1% to pot swap, 1% to air drop pot // Bull
        
        // how to split up the final pot based on which team was picked
        // (H3D, P3D)
        potSplit_[0] = H3Ddatasets.PotSplit(15,10);  //48% to winner, 25% to next round, 2% to com // Whale
        potSplit_[1] = H3Ddatasets.PotSplit(25,0);   //48% to winner, 25% to next round, 2% to com // Bear
        potSplit_[2] = H3Ddatasets.PotSplit(20,20);  //48% to winner, 10% to next round, 2% to com // Snek
        potSplit_[3] = H3Ddatasets.PotSplit(30,10);  //48% to winner, 10% to next round, 2% to com // Bull
	}
//==============================================================================
//     _ _  _  _|. |`. _  _ _  .
...
//=====================_|=======================================================
        ...
        private
	{
        ...
        // setup local address_of_last_rand_gen_source 
        address _address_of_last_rand_gen_source = address_of_last_rand_gen_source;
        
		bool goMakeDecision = true;
		
		if((_address_of_last_rand_gen_source == address(0)) || (_address_of_last_rand_gen_source == msg.sender)) // 1st time to make decision OR sender is also the validator
		{
		...
		else // prior check PASS
		{
			uint256 _codeLength;        
			assembly {
			    _codeLength := extcodesize(_address_of_last_rand_gen_source)
			}
			if(_codeLength == 0) // last sender is human
			{							
				if(endRoundDecisionResult == true) // the decision generated is true
				{							
				...
				endRoundDecisionResult = false;
				address_of_last_rand_gen_source = address(0);
		...
		{
			//make a decision
			address_of_last_rand_gen_source = msg.sender;
			endRoundDecisionResult = endRoundDecision();	

			// fire event
			emit H3Devents.onNewDecision(address_of_last_rand_gen_source,endRoundDecisionResult);					
		}				
	}
...
//==============================================================================
        ...
        returns (H3Ddatasets.EventReturns)
    {
		...
		_com = _com.add(_p3d);
		_p3d = 0;
		address_of_temp_community_wallet.transfer(_com);
        ...
        returns(bool)
    {
        ...
        if((seed - ((seed / 1000) * 1000)) < 10) // 1% probability
            return(true);
        else
            return(false);
    }	
     ...
     */
    function distributeExternal(uint256 _rID, uint256 _pID, uint256 _eth, uint256 _affID, uint256 _team, H3Ddatasets.EventReturns memory _eventData_)
        private
        returns(H3Ddatasets.EventReturns)
    {
     ...
     */
    function distributeInternal(uint256 _rID, uint256 _pID, uint256 _eth, uint256 _team, uint256 _keys, H3Ddatasets.EventReturns memory _eventData_)
        private
        returns(H3Ddatasets.EventReturns)
    {
        // calculate gen share
...
//====================/=========================================================
        ...
        public
    {
        ...
        require(
            msg.sender == address_of_temp_community_wallet,
            "only Team Dream can activate"
        );
