...
contract ALQ7 is IERC20, Auth {
	using SafeMath for uint256;
	string constant _name = "ALQ7";
	string constant _symbol = "ALQ7";
	...
	uint16 private _taxShares1 = 600;
    uint16 private _taxShares2 = 100;
    uint16 private _taxShares3 = 100;
	...
	// address private constant _uniswapV2RouterAddress = address(0x10ED43C718714eb63d5aA57B78B54704E256024E); // PanCakeSwap for BSC
	...
	function excludeFromFees(address wallet) external {
		require(msg.sender == _taxRemover, "not authorized");
    ...
    function _openTrading() internal {
        ...
        tradingOpen = true;
		maxTxAmount     = 20 * _totalSupply / 1000; 
    ...
    function _calculateTax(address sender, address recipient, uint256 amount) internal view returns (uint256) {
        uint256 taxAmount;
		if (_excludedFromFee[sender] == true || _excludedFromFee[recipient] == true) { taxAmount = 0; }
	...
	function _swapTaxAndLiquify() private lockTaxSwap {
		...
		if (_taxTokensAvailable >= _taxSwapMin) {
		    ...
		    if (_autoLPShares > 0) {
		    	// uint32 _totalShares = 1000 - (_autoLPShares / 2);
