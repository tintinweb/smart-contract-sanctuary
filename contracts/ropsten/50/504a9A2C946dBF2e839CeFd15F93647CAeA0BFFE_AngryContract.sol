...
contract AngryContract is ReentrancyGuard{
    ...
    function prePurchase(uint256 _expectedPrice, uint256 _startTime, uint256 _expiredTime, string calldata _invoiceId, bytes memory _sig) public payable {
        ...
        if(msg.value > 0){
            ...
            uint256 lowEthPrice = ethPrice * expectedPriceMultipleLo * (100 - expectedPriceFloatValLo) / 1000;
            uint256 highEthPrice = ethPrice * expectedPriceMultipleHi * (100 + expectedPriceFloatValHi) / 1000;
            require( _expectedPrice > ethPrice && _expectedPrice >= lowEthPrice && _expectedPrice <= highEthPrice, "Invalid expectedPrice!" );
            ...
            uint256 lowUsdtPrice = usdtPrice * expectedPriceMultipleLo * (100 - expectedPriceFloatValLo) / 1000;
            uint256 highUsdtPrice = usdtPrice * expectedPriceMultipleHi * (100 + expectedPriceFloatValHi) / 1000;
            require( _expectedPrice > usdtPrice && _expectedPrice >= lowUsdtPrice && _expectedPrice <= highUsdtPrice, "Invalid expectedPrice!" );
