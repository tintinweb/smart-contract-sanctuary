...
contract EversifyTEST is Context, IERC20, Ownable {
    ...
    string private constant _name = "Eversify Test3";
    string private constant _symbol = "EVE_TEST3";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !_noTaxMode) {
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && initialLimitDuration > block.timestamp) {
                ...
                require(amount.add(walletBalance) <= _tTotal.mul(2).div(100)); //This is the initial max %
            }
