...
contract Trade is Ownable {
    ...
    function createNew(uint256 id, address _payer, address _seller, address _agent,
        ...
        id == id++;
        deals[id] = Deal(_payer, _seller, _agent, _value, _commision, _persent, _endtime, false, false, DealState.Created); // Статус в true для ускорения тестирования
    ...
    /**
     * @dev Перечисление залога
     */
    function pay(uint256 _dealId) public payable { // Залог
    ...
    /**
     * @dev Отмена сделки и возврат денег покупателю
     */
