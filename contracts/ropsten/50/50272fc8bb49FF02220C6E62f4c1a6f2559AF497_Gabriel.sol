...
contract Gabriel is Ownable {
    ...
    function setStartTime(uint256 _time) public onlyOwner {
        require(_time > block.timestamp, "Time must be a future time");
        startTime = _time;
        canStake = true;
    }

    function setEndTime(uint256 _time) public onlyOwner {
        require(_time > block.timestamp && _time > startTime, "End Time must be greater than Start Time");
        endTime = _time;
    }

    function setTsecs() public onlyOwner {
        require(startTime >0 && endTime >0);
        tSecs = endTime.sub(startTime);
    }

    function setTotalRewards(uint256 _reward) public onlyOwner {
