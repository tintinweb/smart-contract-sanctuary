...
contract ERC20 is Context, IERC20, Inventory {
   ...
   function setprice(uint256 NewRate) private returns(uint256) {
   ...
   function getPrice () public view returns(uint256){
    ...
    function basket( address _address, uint _small_sizes, uint _medium_sizes, uint _large_sizes, uint _xlarge_sizes, uint _xxlarge_sizes) public payable {
        ...
        uint cbought = (_small_sizes + _medium_sizes + _large_sizes + _xlarge_sizes + _xxlarge_sizes);
        uint csupply = (smallSizes_.amount + mediumSizes_.amount + largeSizes_.amount + xlargeSizes_.amount + xxlargeSizes_.amount);
        BondCurve storage _Rate = rate;
        
        percentageIncrease = _Rate.Rate * ( cbought /csupply);
        
        
        _Rate.Rate = 0 ether + _Rate.Rate + percentageIncrease;
        
        NewRate = 0 ether + (Rate) + percentageIncrease;
        TotalBasketAmount  = 0 ether + (cbought) * NewRate;
        
        
        
        emit bought(TotalBasketAmount);
        
        uint balsend = msg.sender.balance;
            
        require(balsend >= ((TotalBasketAmount/ 1 ether) * 1 ether), "You don't have enough Ether: Aborted");
        require(msg.value >= ((TotalBasketAmount / 1 ether) * 1 ether), "Your not sending the correct amount to cover your balance");
