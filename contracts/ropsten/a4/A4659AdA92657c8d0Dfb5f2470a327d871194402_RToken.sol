...
contract RToken is IRToken, ERC20, ReserveFormula {
    // Declare state variables
    mapping(address => bool) private approvedToken;  // Only parent contract may buy reserve token
    bool private setApprovedTokenOnce = false;  // Admin can only set the approved token once
    address private admin;  // Contract admin
    uint256 private fixedPrice;  // Price of the Reserve Token during the fixed price phase
    address private wallet01; // Address of stakeholder's wallet
    address private wallet02; // Address of stakeholder's wallet
    ...
    uint256 internal constant PHASE_1_END_SUPPLY = 65e6 * 1e18; // supply of reserve tokens at the end of phase 1
    uint256 internal constant PHASE_3_START_SUPPLY = 68333333333333333333333333; // supply of reserve tokens at the start of phase 3
    uint256 internal constant PRICE_FLOOR = 1e14; // minimum price for reserve token

    /**  @dev Reserve Token Constructor
     *
     *   @param _admin Address of the contract admin
     *   @param _fixedPrice Price of the Reserve Token during the fixed price phase
     *   @param _wallet01 Address of stakeholder's wallet
     *   @param _wallet02 Address of stakeholder's wallet
     */     
    constructor(
        address _admin,
        uint _fixedPrice,
        address _wallet01,
        address _wallet02
    ) ERC20("RToken001", "RTKN001") payable {
        admin = _admin;
        
        // Mint 30M tokens
        _mint(_wallet01, 20e6 * 1e18); // Mint 20M tokens to stakeholder wallet
        _mint(_wallet02, 10e6 * 1e18); // Mint 10M tokens to stakeholder wallet
    ...
    /**  @dev Buy tokens through minting.  Assumes the buyer is the Reserve Token owner.
     *
     *   @param minTokensRequested The minimum amount of reserve tokens to mint.  If this
     ...
     *  the total supply of the reserve token (_supply), the balance of Ether in the reserve 
    ...
    /**  @dev Calculates the effective price (wei) of the reserve token
     *
    ...
    /** @dev Calculates the average price (in wei) per Reserve Token issued for some
     * some amount of Ethers.
     *   @param amount Amount of Ethers (in wei)
     *   @return avgPriceForTokens Average price per Reserve Token (in wei)
     */
    ...
    /** @dev Calculates the cost (in Wei) to buy some amount of Reserve Tokens
     *   @param amount Amount of Reserve Tokens
     *   @param originalBalance The amount of ethers in this contract with msg.value subtracted
     *   @return price Price total price to purchase (amount) of Reserve Tokens
     */
