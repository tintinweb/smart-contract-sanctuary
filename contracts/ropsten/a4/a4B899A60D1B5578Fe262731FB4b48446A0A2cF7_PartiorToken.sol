...
contract PartiorToken is Context, IERC20, Ownable {
    ...
    string private _name = "Partior Token";
    string private _symbol = "PARTIOR";
    ...
    address public donationWallet = 0x81aB176915340b5228C0BFe861862ee6fc5eaD06;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Calculate burn amount and donation amount
        uint256 burnAmt = amount.mul(_burnFee).div(1000);
        uint256 donationAmt = amount.mul(_donationFee).div(100);

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(donationAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(donationAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(donationAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(donationAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(donationAmt)));
        }

        //Temporarily remove fees to transfer to burn address and donation wallet
        _taxFee = 0;
        _liquidityFee = 0;
