...
contract ShibaBTC is ERC20, Ownable {
   ...
   SHIBABTCDividendTracker public dividendTracker;
    ...
    uint256 public liquidateTokensAtAmount = 50000 * (10**18);
    ...
    constructor() ERC20("Shiba-BTC (https://t.me/shibabtc)", "SHIBA-BTC") {
    ...
    receive() external payable {
      ...
      function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SHIBA-BTC: The Uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SHIBA-BTC: Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "SHIBA-BTC: Account is already excluded from fees");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "SHIBA-BTC: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            ...
            if(!openForPresale){
                require(canTransferBeforeTradingIsEnabled[from], "SHIBA-BTC: This account cannot send tokens until trading is enabled");
            }
        }
...
contract SHIBABTCDividendTracker is DividendPayingToken, Ownable {
    ...
    function withdrawDividend() pure public override {
        require(false, "SHIBABTC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shiba-BTC contract.");
    }
    ...
    function excludeFromDividends(address account) external onlyOwner {
      ...
      function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "SHIBABTC_Dividend_Tracker: Cannot update gasForTransfer to same value");
        emit GasForTransferUpdated(newGasForTransfer, gasForTransfer);
        gasForTransfer = newGasForTransfer;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "SHIBABTC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
