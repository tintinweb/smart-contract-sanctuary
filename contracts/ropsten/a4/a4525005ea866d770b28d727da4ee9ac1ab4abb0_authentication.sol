...
contract authentication{
    ...
    //生成新TS
    function newTS(bytes32 pk,uint id) internal returns(uint number){
        uint len = getTScount();
        len++;
        TS[len].PK_TS = pk;
        TS[len].ID_TS = id;
    ...
    //返回TS的公钥，失败返回2
    function getTS_PK(uint value) internal returns(bytes32 key){
...
*/
    ...
    returns(bool flag){
        ...
        if(checkTicketstate(id_uav,state_temp)){
            ...
            return uav_add.send(10);
        }
        
    }
     function ini(){
       newTS(0x2a936931e41bded0ccedd983e00d2226,1);
   ...
   */
   ...
   function showhashv(uint a ,uint b)
