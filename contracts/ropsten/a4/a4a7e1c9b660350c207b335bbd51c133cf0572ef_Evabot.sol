...
contract Evabot {
  ...
  */
  ...
  function startCycle() onlyAdmin() public {
      ...
      for(uint256 i = 0; i < pendingUserlists.length; i++) {
          ...
          uint j = 0;
          for (uint k=0; k<activeUserlists.length; k++) {
            if(activeUserlists[k] == pendingUserlists[i]) {
              j++;
            }
          }
          if (j == 0)
  ...
  function stopCycle() onlyAdmin() public {
      ...
      for(uint256 i = 0; i < activeUserlists.length; i++) {
          ...
          if(isAutoInvest[activeUserlists[i]]) {
            ...
            uint j = 0;
            for (uint k=0; k<pendingUserlists.length; k++) {
              if(pendingUserlists[k] == activeUserlists[i]) {
                  j++;
              }
            }
            if (j == 0)
 ...
 // pending userlist
 function setPendingUserListLength(uint256 _len) onlyAdmin() public {
   pendingUserlists.length = _len;
 }
 
  // pending userlist empty
 function setActiveUserListLength(uint256 _len) onlyAdmin() public {
   pendingUserlists.length = _len;
 }
 ...
 // active user list array control
  ...
  function removePendingArray(address _user) onlyAdmin() public {
    _premoveByValue(_user);
  }
  
  function removeActiveArray(address _user) onlyAdmin() public {
      _aremoveByValue(_user);
  }
}