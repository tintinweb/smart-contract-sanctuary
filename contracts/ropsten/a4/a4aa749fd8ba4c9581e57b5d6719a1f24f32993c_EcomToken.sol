...
contract EcomToken {
  ...
  using SafeERC20 for EcomToken;

  string public name = "Ecom Token";
  ...
  function reclaimToken(EcomToken token) onlyOwner external {
...
library SafeERC20 {
  ...
  function safeTransfer(EcomToken token, address to, uint256 value) internal {
    assert(token.transfer(to, value));
  }

  function safeTransferFrom(EcomToken token, address from, address to, uint256 value) internal {
    assert(token.transferFrom(from, to, value));
  }

  function safeApprove(EcomToken token, address spender, uint256 value) internal {
