...
contract ERC20Token is ERC20Capped, Ownable {
    ...
    constructor(
        ...
        uint256 _rate
    )ERC20(_name, _symbole) ERC20Capped(_maxTotalSupply * (10**18))  {
    
        setCurrentRate(_rate);
        uint256 tokenSupply = _initialSupply * (10**decimals());
    ...
    receive() external payable {
        buyTokenUptoCapped(msg.value);
    }

    // buy abount in wei.
    function buyTokenUptoCapped(uint256 buyAmount) public payable {
        uint256 totalPayableToken = (buyAmount * _tokenRate) / 1 ether;
        ...
        emit paybaletoken(totalPayableToken * (10**decimals()), msg.sender);
    }
