...
contract XTKN031711Liquidity is Context, Ownable{
    // lib
    ...
    // events
        ...
        event CreatedLiquidityPair(address pair);
        event FailedCreatingLiquidityPair(uint256 tokens, uint256 native);
    ...
    constructor(address pToken){
        ...
        if(ADDRESS_PAIR == address(0) && ADDRESS_TOKEN != address(0)){
            // setup uniswap
            router = IUniswapV2Router02(ADDRESS_ROUTER);
            ADDRESS_PAIR = IUniswapV2Factory(router.factory()).createPair(ADDRESS_TOKEN, router.WETH());            
            _token.approve(address(router), 2**256 - 1);
            emit CreatedLiquidityPair(ADDRESS_PAIR);
        }
    ...
    function init() public onlyOwner{
        if(ADDRESS_PAIR != address(0) && ADDRESS_TOKEN != address(0)){
            // get the contracts tokens and native balance to add to liquidity (all tokens)
            uint256 tokens = _token.balanceOf(address(this));
            uint256 native = address(this).balance;

            if(tokens > 0 && native > 0){
                // if we can add liquidity, lock contract
                _addLiquidity(tokens, native);
                _lock();
            }else{
                emit FailedCreatingLiquidityPair(tokens, native);
            }
        }
    }
    ...
    function _addLiquidity(uint256 pTokens, uint256 pNative) private{
        // create liquidity
        router.addLiquidityETH{value:pNative}(
            ADDRESS_TOKEN,
            pTokens,
            0,
            0,
