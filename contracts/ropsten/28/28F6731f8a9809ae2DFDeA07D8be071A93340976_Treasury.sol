...
contract Treasury {
    ...
    function claim(address recipient) public returns (uint256){
        ...
        // IERC20(erc20BetrustAddress).balanceOf(address(this));
...
contract ERC721Minter is ERC721URIStorage, ERC721Enumerable{
    ...
    uint256 constant public erc20PriceTier2 = 2 ether;
    ...
    function getTierPrice(uint256 tier) public pure returns (uint256){
        if(tier == 1){
            return priceTier1;
        }
        else return priceTier2;
    }
    function getTierERC20Price(uint256 tier) public pure returns (uint256){
        if(tier == 1){
            return erc20PriceTier1;
        }
        else return erc20PriceTier2;
    }
    function calculateERC20Value(address owner) public returns (uint256){
        ...
        for(uint256 index = 0; index < limit; index++){
            ...
            value+=getTierERC20Price(tokenTiers[tokenId]);
        }
        emit Value(value);
        return value;
    }
