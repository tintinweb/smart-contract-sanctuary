...
contract CosmoMasksPower is Ownable, CosmoMasksPowerERC20 {
    ...
    uint256 public constant emissionStart = 1615630000; // "2021-03-14T15:00:00.000Z"
    ...
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(
            ICosmoMasksShort(cosmoMasksAddress).ownerOf(tokenIndex) !=
                address(0),
            "CosmoMasksPower: owner cannot be 0 address"
        );
        require(
            tokenIndex < ICosmoMasksShort(cosmoMasksAddress).totalSupply(),
            "CosmoMasksPower: CosmoMasks at index has not been minted yet"
        );
    ...
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        require(
            ...
            "CosmoMasksPower: emission has not started yet"
        );
        require(
            ICosmoMasksShort(cosmoMasksAddress).ownerOf(tokenIndex) !=
                address(0),
        ...
        uint256 accumulationPeriod =
            block.timestamp < emissionEnd ? block.timestamp : emissionEnd;
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment =
                ICosmoMasksShort(cosmoMasksAddress).isMintedBeforeReveal(
                    tokenIndex
                ) == true
    ...
    function setCosmoMasksAddress(address masksAddress) public onlyOwner {
        require(
            cosmoMasksAddress == address(0),
            "CosmoMasks: CosmoMasks has already setted"
        );
        require(
            masksAddress != address(0),
            "CosmoMasks: CosmoMasks is the zero address"
        );
    ...
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenIndices.length; i++) {
            ...
            require(
                tokenIndices[i] <
                    ICosmoMasksShort(cosmoMasksAddress).totalSupply(),
                "CosmoMasksPower: CosmoMasks at index has not been minted yet"
            );
            ...
            for (uint256 j = i + 1; j < tokenIndices.length; j++)
                require(
                    ...
                    "CosmoMasksPower: duplicate token index"
                );
            ...
            require(
                ICosmoMasksShort(cosmoMasksAddress).ownerOf(tokenIndex) ==
                    msg.sender,
                "CosmoMasksPower: sender is not the owner"
            );
