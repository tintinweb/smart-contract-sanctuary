...
contract Staker {
    ...
    /*---------Variables & constructor----------*/
    address private carchain;
    address private carNC;
    ...
    /*---------Modifiers----------*/
    modifier onlyCarchain {
        require(msg.sender == carchain);
        _;
    }

    /*---------Methods----------*/
    function deposit(address _provider, uint256 _amount) public onlyCarchain{
        balances[_provider] = balances[_provider].add(_amount);
    }

    function withdraw(address _provider, uint256 _amount) public onlyCarchain{
        require(balances[_provider] >= _amount);
        CarNCToken _carNc = CarNCToken(carNC);
        _carNc.transfer(_provider, _amount);
    }

    function balanceOf(address _staker) view public returns(uint256){
        return balances[_staker];
    }
    /*---------Events----------*/

    event Deposit(address indexed staker, uint256 _amount);
    event Withdraw(address indexed staker, uint256 _amount);
