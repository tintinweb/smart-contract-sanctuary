...
contract Freezable is Ownable {
        ...
        event MultiFreezed(address[] targets);
        event MultiUnfreezed(address[] targets);
        ...
        function multiFreeze(address[] memory _targets)
                public
                onlyOwner
                returns (bool)
        {
                ...
                emit MultiFreezed(_targets);
                return true;
        }

        function multiUnfreeze(address[] memory _targets)
                public
                onlyOwner
                returns (bool)
        {
                ...
                emit MultiUnfreezed(_targets);
                return true;
        }
}

contract MTCT08 is StandardToken, Pausable, Freezable {
        using SafeMath for uint256;

        string  public  name = "MTCT08";
        string  public  symbol = "MTCT08";
