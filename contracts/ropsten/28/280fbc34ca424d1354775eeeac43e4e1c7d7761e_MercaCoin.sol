...
uint public constant MAX_UNU = 814;
  ...
  constructor() public {
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] memory) {
        uint tokenCount = super._getOwnerNFTCount(_owner);

        uint256[] memory tokensId = new uint256[](tokenCount);
        for(uint i = 0; i < tokenCount; i++){
            tokensId[i] = ownerToIds[_owner][i];//ERC721Enumerable.tokenOfOwnerByIndex(_owner, i);
        }
