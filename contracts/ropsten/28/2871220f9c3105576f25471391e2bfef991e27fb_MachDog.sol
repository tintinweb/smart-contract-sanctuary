...
contract MachDog is MachDogDevents {
	...
	function createDog(uint256 dogId, address owner)
        olnyAdmin()
        public
    {
	    dogTxcode_[dogId] = 0;
		dog_[dogId].owner = owner;
		dog_[dogId].status = 0;
		// 
		emit MachDogDevents.Create (
			dogId,
			owner
		);
	}
        ...
        public
    {
		...
		uint256 _now = now;
		//
		require(dogTxcode_[dogId] == code && f == 1 && _now < endtime);
		//
		dogTxcode_[dogId] = 0;
