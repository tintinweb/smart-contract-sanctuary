...
contract Generator is Construction {
    ...
    mapping(address => Details) queue;
    event Activated(address indexed _vaultAddress, address indexed _proxyAddress);
    ...
    function generate(address _admin, address _owner) public returns(address _vault, address _proxy) {
        ...
        queue[_vault].proxyAddress = _proxy;
        queue[_vault].ownerAddress = _owner;
        emit Generated(_admin, _vault, _proxy);
        return (_vault, _proxy);
    }
    function activate(address _vault) public {
        if (!Alpha(_vault).setting(queue[_vault].proxyAddress, queue[_vault].ownerAddress))
        revert();
        emit Activated(_vault, queue[_vault].proxyAddress);
    }
}