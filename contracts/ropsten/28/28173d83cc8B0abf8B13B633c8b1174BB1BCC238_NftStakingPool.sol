...
contract NftStakingPool is Ownable
{
	...
	event StakedNft(address staker,address token, uint256 id);

	/// @notice Event records info about withdrawed nft from this pool.
	/// @param staker - address of nft staker.
	/// @param token - address of nft contract.
	/// @param id - id of staked nft.
	event WithdrawedNft(address staker, address token, uint256 id);
	...
	/// @notice mapping records nft user staked in pool.
	mapping (address => stakedNft) public nftStaked;
	...
	function stakeNft(address token, uint256 id) public
	{
		...
		nftStaked[msg.sender].token = token;
		nftStaked[msg.sender].id = id;
