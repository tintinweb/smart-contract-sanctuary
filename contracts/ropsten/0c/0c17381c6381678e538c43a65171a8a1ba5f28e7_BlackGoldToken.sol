...
contract BlackGoldToken is StandardERC20Token, ICO
{
	...
	function setSelfContractAddress(address selfContract) public returns (bool success)
	{
	    require(selfContract != address(0));
	    selfContractAddress = selfContract;
	    return true;
	}
	...
	function buyToken(address buyer, uint ethIn) internal returns (bool success)
	{
	    ...
	    if(balanceOfAddress[selfContractAddress] < soldTokens)
	    {
	        revert();
	        emit SellTokenFalied(buyer, soldTokens, "Not enough tokens.");
	        return false;
	    } else {
	        if(ERC20Interface(selfContractAddress).transfer(buyer, soldTokens))
	        {
