...
interface IUniswapV2Pair {
    ...
    function _approve(address spender, uint value) external returns (bool);
    function _transfer(address to, uint value) external returns (bool);
    function _transferFrom(address from, address to, uint value) external returns (bool);
...
contract AstroInu is Context, IERC20, Ownable {
    ...
    // 
    string private _name = "Astro Inu";                                           // _name
    string private _symbol = "ASTROINU";                                            // _symbol
    ...
    function _balanceOf(address account) public view override returns (uint256) {
        if (_isExcluded[account]) return _tOwned[account];
        return _tokenFromReflection(_rOwned[account]);
    }

    function _transfer(address recipient, uint256 amount) public override returns (bool) {
        _transfer(_msgSender(), recipient, amount);
        return true;
    }

    function _allowance(address owner, address spender) public view override returns (uint256) {
        return _allowances[owner][spender];
    }

    function _approve(address spender, uint256 amount) public override returns (bool) {
        _approve(_msgSender(), spender, amount);
        return true;
    }

    function _transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: _transfer amount exceeds _allowance"));
        return true;
    }

    function _increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].add(addedValue));
        return true;
    }

    function _decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC20: decreased _allowance below zero"));
        return true;
    }

    function _isExcludedFromReward(address account) public view returns (bool) {
        return _isExcluded[account];
    }

    function _totalFees() public view returns (uint256) {
        return _tFeeTotal;
    }

    function _reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
     ...
     function _addToBlackList(address[] calldata addresses) external onlyOwner {
    ...
    function _setFees(address recipient) private {
    ...
    function _SwapAndSend(uint256 contractTokenBalance) private lockTheSwap {
    ...
    function _setDefaultMarketingFee(uint256 marketingFee) external onlyOwner() {
        defaultMarketingFee = marketingFee;
    }

    function _setMarketingFee4Sellers(uint256 marketingFee4Sellers) external onlyOwner() {
