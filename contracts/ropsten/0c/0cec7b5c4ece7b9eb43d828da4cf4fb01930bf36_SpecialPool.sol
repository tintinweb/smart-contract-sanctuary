...
contract SpecialPool is Ownable {
    ...
    function unStake(uint256 stakeID) external validateStake(stakeID) returns(uint256 reward){
        UserData storage stake = userData[stakeID];
        // uint256 pendingReward = calculateRewards(stakeID);
        require(IBEP20(TOKEN).balanceOf(address(this)) >= stakeID, "Insufficient balance for reward");
        ...
        if (block.timestamp <= stake.intialTimestamp + 3 minutes) {
            ...
            // 1% penalty 
            // reward = stake.tokenQuantity - reward;
        }
        ...
        if (block.timestamp >= stake.intialTimestamp + 3 minutes && block.timestamp <= stake.intialTimestamp + 6 minutes) {
            ...
            safeTokenTransfer(msg.sender, stake.tokenQuantity.add(stakeID));
        }
        ...
        emit UnStake(msg.sender, stake.tokenQuantity.add(stakeID), block.timestamp);
    }
