...
contract Autoglyphs {
    ...
    function _drawHelper(uint a, uint c, bytes5 symbols, bytes output) internal view returns (bytes) {
        int x = 0;
        int y = 0;
        uint v = 0;
        uint value = 0;
    ...
    function draw(uint id) public view returns (string) {
        ...
        string memory result = string(_drawHelper(a, c, symbols, output));
        return result;
    }
