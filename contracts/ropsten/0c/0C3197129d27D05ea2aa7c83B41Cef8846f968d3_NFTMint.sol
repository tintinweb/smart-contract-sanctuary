...
library SafeMath {
    function mulfees(uint256 a,uint256 b) internal pure returns (uint256) {
        b=b/100;
        b=b*a;
...
contract Sale is ERC721, Ownable{
    ...
    mapping (uint256 => uint256) public _rayal;
    ...
    function saleToken(address payable admin,uint256 tokenId) public payable{
        ...
        uint256 fee = msg.value.mulfees(5);
        //service fees
        admin.transfer(fee);
        uint256 value = msg.value - fee;
        uint256 roy = value.mulfees(_rayal[tokenId]);
        //royalty
        create.transfer(roy);
        //owner
        value = value - roy;
...
contract NFTMint is ERC721,Sale{
    ...
    function mintwithsale(string memory _name, string memory tokenuri, uint256 value, uint256 tokenId, uint256 rayal) public {
        require(!_nameexits[_name]);
        _mint(msg.sender, tokenId);
        _creator[tokenId]=msg.sender;
        _tokenOwner[tokenId]=msg.sender;
        _rayal[tokenid]=rayal;
        _nameexits[_name] = true;
        _setTokenURI(tokenId, tokenuri);
