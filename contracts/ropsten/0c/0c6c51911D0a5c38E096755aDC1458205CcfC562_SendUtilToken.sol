...
contract SendUtilToken is PausableToken {
    ...
    function singleInvokeWith2Args(address _contractAddr, string methodName, address _to, uint256 _value) public returns (uint256[]){
        uint256 [] memory values;
        address[1] memory _tos =  [_to];
        uint256[1] memory _values = [_value];
        if (_tos.length != _values.length) {
            return values;
        }
        bytes memory methodNameWithSign = concat(methodName, "(address,uint256)");
