...
contract LiquidityMiningStaking is OwnableUpgradeable, ReentrancyGuard, AbstractDependant {
	...
	function rewardPerToken() public view returns (uint256) {
		...
		uint256 accumulatedReward = blocksWithRewardsPassed().mul(rewardPerBlock).mul(DECIMALS).div(totalStaked);
	...
	function rewards(address staker) external view returns (uint256) {
		...
		return reward.sub(reward.mul(getSlashingPercentage()).div(PERCENTAGE_100));
	}
	...
	function getReward() public nonReentrant updateReward(_msgSender()) {
		...
		if (reward > 0) {
			...
			uint256 bmiStakingProfit = reward.mul(getSlashingPercentage()).div(PERCENTAGE_100);
	...
	function restake() external nonReentrant updateReward(_msgSender()) {
		uint256 reward = _rewards[_msgSender()];
	...
	function setRewards(
		...
		require(rewardTokensLocked <= rewardsToken.balanceOf(address(this)),
		...
		returns (uint256)
	{
		...
		if (block.number < _from) 
			return _to.sub(_from).add(1);
