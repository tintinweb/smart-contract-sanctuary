...
contract LylzToken is ERC721BasicToken {
    ...
    */
     ...
     */
    function createToken(address _to, uint256 _id, string memory _metadataPath) public {
        require(msg.sender == owner);
        tokenIdCounter = tokenIdCounter.add(1);
        addTokenTo(_to, tokenIdCounter);
     ...
     * @param _metadataPath string the url metadata path
     */
    function createMultipleToken(address _to, uint256 _id, string[] memory _metadataPath) public {
        require(msg.sender == owner);
        for (uint32 i = 0; i < _metadataPath.length; i++) {
            createToken(_to, _id, _metadataPath[i]);
        }
    }
