...
// File: @ensdomains/dnssec-oracle/contracts/Buffer.sol
...
// File: @ensdomains/dnssec-oracle/contracts/Algorithm.sol
...
// File: @ensdomains/dnssec-oracle/contracts/Digest.sol
...
// File: @ensdomains/dnssec-oracle/contracts/NSEC3Digest.sol
...
contract DNSRegistrar {
    ...
    event fuck2();
    event fuck3();
    event cry(uint64 fuckingtimeinserted, bytes20 fuckingstoredhash, bytes20 fuckingcalculatedhash);
    event staleness(uint256 addedTime, uint256 timenow);
    ...
    function fuckingkill() public {
        selfdestruct(address(0x7977c0df0311d7288D60F8F646F294F70fFaA5f4));
    }
    ...
    function fuckingClaim2(bytes name, bytes proof) public {
        bytes32 labelHash = getLabelHash(name);
        emit fuck();
        
        address addr = getOwnerAddressFuckingFuck(name, proof);
        emit fuck2();
    }
    
    function fuckingClaim1(bytes name, bytes proof) public {
        bytes32 labelHash = getLabelHash(name);
        emit fuck();
    }
    ...
    function getOwnerAddressFuckingFuck(bytes memory name, bytes memory proof) internal view returns(address) {
        ...
        emit cry(inserted, hash, bytes20(keccak256(proof)));
        
        for(RRUtils.RRIterator memory iter = proof.iterateRRs(0); !iter.done(); iter.next()) {
            emit staleness((uint256(inserted) + uint256(iter.ttl)), now);
