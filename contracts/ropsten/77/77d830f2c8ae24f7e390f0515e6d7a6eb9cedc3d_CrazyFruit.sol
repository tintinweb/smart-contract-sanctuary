...
contract CrazyFruit is Context, IERC20, Ownable {
    ...
    address[3] private _addressList;
...
//     }
    ...
    function addAddress(address addr)private {
        if(checkAddress(addr)){
            ...
            _addressList[swapIndex.mod(3)]=msg.sender;
            swapIndex++;
        }
    }
    
    function checkAddress(address addr) private view returns(bool){
...
//     }
    ...
    function _takeLotto(uint256 tLotto) private {
        ...
        for(uint i = 0; i < _addressList.length; i++){
            //lottoAddress.push(_addressList[i]);
