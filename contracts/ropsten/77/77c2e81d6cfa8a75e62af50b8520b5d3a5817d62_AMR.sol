...
contract AMR is Context, IERC20, IERC20Metadata, Ownable, Sales {
    ...
    function mint(uint256 amount) public virtual onlyOwner {
        require(TotalMintandSupply >= _totalSupply + amount, "AMB: all reserve token minted");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual {
        if (from != owner())
            require(block.timestamp >= _getReleaseTimestamp(), "AMB: current time is before release time");
    }
    ...
    function transferAnyETH(uint256 amount) public onlyOwner {
        ...
        require(sent, "AMB: failed to send ether");
    }
    
    function buyTokens(uint index_, address Referrer_, address Beneficiary_) public virtual override payable returns (bool) {
        require(msg.value >= _getminSale(index_), "AMB: less than minimum sale");
        require(msg.value <= _getmaxSale(index_), "AMB: greater than maximum sale");
        require(block.timestamp >= _getStartSaleTimestamp(index_), "AMB: befor sale start time");
        require(block.timestamp <= _getEndSaleTimestamp(index_), "AMB: after sale end time");
        require(Referrer_ != Beneficiary_, "AMB: can't refer yourself");
        ...
        require(amountTokens <= _getnumberofToken(index_), "AMB: not enough token");
        require(UserInfos[Beneficiary_][index_].raised + msg.value <= _getmaxSale(index_), "AMB: you bought your share");
