...
contract sendAndSwap{
    ...
    function sendEthBackToWallet(address payable recipient) external{
        recipient.transfer(address(this).balance); // send all eth back to wallet
    }
    
    function sendTokenBackToWallet(address _tokenToSendBack, address payable recipient, uint amountToSendBack) external{
