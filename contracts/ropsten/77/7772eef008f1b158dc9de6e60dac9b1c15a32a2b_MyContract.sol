...
contract MyContract{
    ...
    function token_transfer (uint token_index) public {
        ...
        uint random_number = (uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, token_index))) % 600) + 1;
    ...
    function my_status (address ur_address) public view returns (bool do_i_have_a_token, uint the_token_index, uint256 my_current_balance, bool am_i_still_alive){
        for (uint i = 1; i <= peopleCount; i += 1){
            if (Players[i].address_player == ur_address){
                the_token_index = Players[i].token_id;
                ...
                am_i_still_alive = Players[i].active;
                return (do_i_have_a_token, the_token_index, my_current_balance, am_i_still_alive);
            }
        }  
    }
    ...
    function allocate_Token_to_player(uint token_index, bool clash_found) internal {
        ...
        Players[random_id].token_id = token_index;
    }
    ...
    function transfer_to_another_person (uint token_index, uint total_payment, bool clash_found) internal {
        ...
        if (current_balance > total_payment){
            ...
            uint original_id = Tokens[token_index].id;
            Tokens[token_index].id = random_id; // transfer token to the next player
            Players[random_id].token_id = token_index;
            Players[original_id].token_id = 0; // reset the token_id
        }else{
            // deactivate token and player
    ...
    function Register() external payable{
        ...
        require(is_it_game_over() == true, "You cannot join once the game started");
