...
contract GUniRouter is IGUniRouter, IUniswapV3SwapCallback {
        ...
        )
    {
        ...
        if (amount0In > 0 && !wethToken0) {
            pool.token0().safeTransferFrom(
                msg.sender,
                address(this),
                amount0In
            );
        } else if (amount0In > 0 && wethToken0) {
            require(
                amount0In == msg.value,
                "mismatching amount of ETH forwarded"
            );
        ...
        if (amount1In > 0 && !wethToken0) {
            require(
                amount1In == msg.value,
                "mismatching amount of ETH forwarded"
            );
            weth.deposit{value: amount1In}();
        } else if (amount1In > 0 && wethToken0) {
            pool.token1().safeTransferFrom(
                msg.sender,
                address(this),
                amount1In
        ...
        (amount0Use, amount1Use, mintAmount) = _getAmountsAndRefundETH(
            pool,
            amount0Max,
            amount1Max,
            wethToken0
    ...
    function _swap(
        ...
        bytes memory callback = _encodeCallback(pool);
        pool.pool().swap(
            address(this),
            zeroForOne,
            swapAmount,
    ...
    function _getAmountsAndRefundETH(
        IGUniPool pool,
        uint256 amount0Max,
        uint256 amount1Max,
        bool wethToken0
    ...
    function _encodeCallback(IGUniPool pool)
        internal
        view
