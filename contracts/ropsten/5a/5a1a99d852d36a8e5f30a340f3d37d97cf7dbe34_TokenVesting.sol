...
contract TokenVesting is Ownable, ReentrancyGuard {
  ...
  struct LockParams {
    ...
    uint256 endEmission; // the unlock data as a unix timestamp (in seconds)
   ...
   * user address(0) for no condition
  ...
  function transferLockOwnership (uint256 _lockID, address payable _newOwner) external nonReentrant {
    ...
    // transferredLock.sharesDeposited = 0; // gas refund
    // transferredLock.sharesWithdrawn = 0; // gas refund
    transferredLock.sharesWithdrawn = transferredLock.sharesDeposited;
    emit onTransferLock(_lockID, msg.sender, _newOwner);
  }
