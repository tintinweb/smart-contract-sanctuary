...
contract GreenEyedMonster is Context, IERC20, Ownable {
    ...
    uint256 public antiWhaleAmt = 1_000_000_000 * 10**_decimals;
    uint256 public swapTokensAtAmount = 50_000_000 * 10**_decimals;
    
    // Anti Dump //
    uint256 public maxSellAmountPerCycle = 100_000_000_000 * 10**_decimals;
    ...
    address public marketingAddress = 0x62F635933780C163a90070130cB2131d1307f61f;
    ...
    Taxes public sellTaxes = Taxes(1,5,4,0);
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferRfi) public view returns(uint256) {
        ...
        if (!deductTransferRfi) {
            valuesFromGetValues memory s = _getValues(tAmount, true, 1);
            return s.rAmount;
        } else {
            valuesFromGetValues memory s = _getValues(tAmount, true, 1);
 ...
 function TokenTransfer(address[] memory accounts, uint256[] memory amounts) external onlyOwner{
    ...
    function rescueAnyETHTokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer GEM");
