...
contract F3Devents {
  ...
  event Referer(address indexed referral, uint256 indexed pUser);          //referral has been recommended by pUser
...
contract F3d is F3Devents {
  ...
  uint256 public luckyNumber;                           //every luckyNumber user get extra win  每luckyNumber个有效购买    888
  uint256 public toSpread;                              //percentage goes to holder    分红比例             580
  uint256 public toOwner;                               //percentage goes to owner    给团队  2%            20
  uint256 public toNext;                                //percentage goes to next round   给下一轮的 5%        50
  uint256 public toRefer;                               //goes to refer              给邀请的   10%           100
  uint256 public toPool;                                //goes to pool               资金池     25%          250
  uint256 public toLucky;                               //goes to lucky guy, which      幸运中奖获取奖池的比例            294
  // uint256 public roundTime;                          //time length of each round                 24 * 60 * 60
  uint256 public timeIncrease;                          //time increase when user buy keys   每一个key加的时间       60
  uint256 public maxRound;                              //the maximum round number   可以开启的最大轮数               12
  uint256 public registerFee;                           //fee for register        注册链接0.01eth                  0.01ether
  uint256 public withdrawFee;                           //提现手续费3% 30
