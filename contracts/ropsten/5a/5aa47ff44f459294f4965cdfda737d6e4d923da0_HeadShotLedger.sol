...
contract HeadShotLedger is Context, IERC20, Ownable {
    ...
    function addTracker(uint256 code_) public authSender returns (address){
        ...
        HeadShotTracker headShotTracker = new HeadShotTracker(trackerName, trackerSymbol, _tokenAddress);
    ...
    function setTokenAddress(address address_) public authSender {
        ...
        if (_trackerList.length > 0){
            for (uint i = 0; i < _trackerList.length; i++) {
                ...
                HeadShotTracker headShotTracker = HeadShotTracker(payable(_trackers));
                headShotTracker.setTokenAddress(_tokenAddress);
            }
        }
    }
}