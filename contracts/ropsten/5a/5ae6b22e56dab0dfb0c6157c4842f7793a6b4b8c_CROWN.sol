...
contract CROWN is Ownable, ERC20 {
    ...
    uint256 public stableCoinPriceIn100Unit = 100; //--Stable coin price (USD) for calculate dividend amount--- (Use in 100 unit bacause of decimal price)
    uint256 public crownPriceIn100Unit = 100; //--CWT token price (USD) for caculate dividend amount--- (Use in 100 unit bacause of decimal price)
    uint256 public dividendRateIn100Unit = 400; //---Dividend rate 4% of stake amount--- (Use in 100 unit bacause of decimal rate)
    ...
    event DividenPaid(address indexed to, uint256 amount, uint256 timestamp);
...
//-------------------------------- Staking Functions ---------------------------------------------
   ...
   function addStake(address stakeHolder, uint256 stakeAmount) public validAddress(stakeHolder) {
       ...
       _transfer(stakeHolder, address(this), stakeAmount);
   ...
   function calculateReward(address stakeHolder) internal view validAddress(stakeHolder) returns (uint256) {
   ...
   function priceFeeding(uint256 _stableCoinPriceIn100Unit, uint256 _crownPriceIn100Unit) public onlyOwner {
       require(_stableCoinPriceIn100Unit >0 && _crownPriceIn100Unit>0,"price must be more than 0");
   ...
   function calculateRewardToStableCoin(address stakeHolder) internal view returns (uint256) {
   ...
   function distributeRewards() public onlyOwner {
        ...
        for (uint256 i = 0; i < stakeholders.length; i += 1) {
           ...
           uint256 dividend = dividends[stakeholder];
           if (reward > 0) {
               uint256 rewardStableCoin = dividend;
               IERCToken(stableCoinAddress).transfer(stakeholder, rewardStableCoin);
               string memory tokenSymbol = IERC20Metadata(stableCoinAddress).symbol();
               rewards[stakeholder] -= reward;
               dividends[stakeholder] -= dividend;
               removeStake(stakeholder, stakes[stakeholder]);
               dividendList.push(Dividend(address(this), stakeholder, reward, tokenSymbol, block.timestamp));
               dividendHistory[stakeholder].push(Dividend(address(this), stakeholder, reward, tokenSymbol, block.timestamp));
   ...
   function withdrawReward(address stakeHolder) public validAddress(stakeHolder) {
       ...
       require(block.timestamp >= endDate, "Please wait until the reward removal date.");
       require(stableCoinAddress != address(0), "Not specified stable coin's contract yet!");
       require(rewards[stakeHolder] > 0,"You don't have the reward to withdraw.");
       require(msg.sender == owner() || msg.sender == stakeHolder, "Just an admin & stakeHolder can remove stake!");
       uint256 reward = rewards[stakeHolder];
       uint256 dividend = dividends[stakeHolder];
       if (reward > 0) {
