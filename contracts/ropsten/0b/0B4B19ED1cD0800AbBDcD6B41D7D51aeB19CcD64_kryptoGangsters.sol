...
contract kryptoGangsters is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    using Strings for *;
    Counters.Counter public _tokenIDs;
	Counters.Counter public _uriIDs;
    ...
    mapping(address => bool) public isTeamListed;
    mapping(address => uint256) public tokenCounters;

    string public imageUri = '';
    bool public isSetRevealOpenSea = false;
    uint256 private mintAmount = 120000000000000000;
    uint256 private mintWhitelistAmount = 100000000000000000;
    ...
    function createItem(uint256 amount) public payable returns (uint256) {
        require(
            isTeamListed[msg.sender] == false,
            "You are team member. You have already NFT items."
        );
        require(_tokenIDs.current() + amount < 10000, "Max mint amount is reached");
        if (isTeamListed[msg.sender]) {
            require(
                tokenCounters[msg.sender] <= 5,
                "Team member can only 5 NFT tokens at max"
            );
        }
        if (!isTeamListed[msg.sender]) {
            require(
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            Items[newItemID] = Item(
                newItemID,
                msg.sender,
                imageUri.toSlice().concat('0.json'.toSlice()),
                false
            );
            tokenCounters[msg.sender] = tokenCounters[msg.sender] + 1;
        }
        return 1;
    }

    function createTeamItem(address owner) private returns (uint256) {
        for (uint256 i = 0; i < 5; i++) {
            ...
            tokenCounters[owner] = tokenCounters[owner] + 1;
        }
        return 1;
    }
    ...
    function whiteListAddress(address userAddress) public onlyAdmin {
    ...
    function teamListAddress(address userAddress) public onlyAdmin {
        require(userAddress != address(0), "  userAddress is the zero address");
        isTeamListed[userAddress] = true;
    }

    function removeTeamListAddress(address userAddress) public onlyAdmin {
        require(userAddress != address(0), "  userAddress is the zero address");
        isTeamListed[userAddress] = false;
    }

    function setRevealOpenSea() public onlyAdmin {
        isSetRevealOpenSea = true;
        for (uint256 i = 1; i <= _tokenIDs.current(); i++) {
            if (Items[i].setOpenSea == false) {
                _setTokenURI(Items[i].id, imageUri.toSlice().concat(uintToString(Items[i].id).toSlice()).toSlice().concat('.json'.toSlice()));
                Items[i] = Item(Items[i].id, Items[i].creator, Items[i].uri, true);
            }
        }
    }
