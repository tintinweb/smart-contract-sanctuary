...
contract Voyagerz is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public constant MAX_MINT_COUNT = 2500;
    ...
    function resetMyJourney(uint256 tokenId) public {
        address owner = ownerOf(tokenId);
        require(msg.sender == owner, "not the owner");
        voyage_count[tokenId] = 0;
        n_sum_first[tokenId] = 0;
    }
    ...
    function mintPriceForMe() public view returns (uint256) {
        uint256 curMintPrice = mintPrice;

        if (!_test_chain_mode) {
            if ((ribonzContract.balanceOf(msg.sender) != 0 && spacetimeContract.balanceOf(msg.sender) != 0)) {
                // Discount
                curMintPrice = mintPriceSpecialOfferForRZHolders;
            }
        }
    ...
    function mintVoyagerzWithRZ(uint256 num_to_mint) public payable virtual nonReentrant {
        ...
        console.log('LOG -- mint voygagerz with RZ');
        require(ribonzContract.balanceOf(msg.sender) > 0, "Need to hold RIBONZ: Genesis for special offer");
        console.log('LOG -- mint voygagerz with RZ -- pass balance check');


        console.log('LOG -- mint voygagerz with RZ -- Start Spacetime balanceOF');


        require(spacetimeContract.balanceOf(msg.sender) > 0, "Need to hold RIBONZ: Spacetime for special offer");

        console.log('LOG -- mint voygagerz with RZ -- Start Spacetime balanceOF - PASSED');
        ...
        console.log('LOG -- mint voygagerz with RZ -- pass tokensOfOwner =', stTokensOfOwner.length);
        

        //
        //
        //
        require (num_to_mint <= stTokensOfOwner.length, "Not enough Spacetimes owneed to mint that number");

        console.log('LOG -- mint voygagerz with RZ -- pass enough ST');
        console.log('LOG -- mint voygagerz with RZ -- pass enough ST - len = ', stTokensOfOwner.length);

        // At this point we know they want to mint less than the number of ST's they have but we need to see how
        // many free reservation slots exis too
        uint256 availableMintSlots = 0;
        for (uint256 i= 0; i<stTokensOfOwner.length; i++) {
            if (spaceTimeMintSlots[stTokensOfOwner[i]] == 0) {
                console.log('ST Mint Slot available at', stTokensOfOwner[i]);
                availableMintSlots++;
            }
        }
        console.log('ST Mint Slot TOTAL =  ', availableMintSlots);

        require( availableMintSlots >= num_to_mint, "you've minted your available Spacetime special offer slots");
    ...
    function getSvg(uint256 tokenId) internal view returns (string memory, uint) {
        //
        // Generate SVG
        //
        ...
        {
            uint256 radius = min(voyage_count[tokenId] * 4 + 50 + n_sum_first[tokenId], 180);
            ...
            buf = string(
                abi.encodePacked(
                    buf,
                    '" stroke="hsl(255,50%,50%)" fill="none"/>'
                    "</defs>"
        ...
        return (buf, rp.bgindex);
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        (string memory svgBuf, uint bgindex) = getSvg(tokenId);
...
library Base64 {
    ...
    // Note changed to URL std chars
    bytes private constant base64urlchars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

    function encode(string memory _str) internal pure returns (string memory) {
        bytes memory _bs = bytes(_str);
        //console.log('base64 str len = %s', _bs.length);
