...
contract OnChainShelly is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    bool internal cooldownEnabled = true;

    bool internal gamePaused = false;

    uint88 public whitelistMintStartTime = 1634537645;
    uint88 public publicMintStartTime = 1634537645;
    ...
    mapping(uint8 => uint8[]) public fullSets;
    ...
    constructor() ERC721("OnChainShellies", "SNAIL") {
        ...
        pools[3] = LootPool({cost: 6, total: 20301, minTier: 2, tier1: 3048, tier2: 15222, tier3: 2031, tier4: 0});
        pools[4] = LootPool({cost: 12, total: 13635, minTier: 3, tier1: 6819, tier2: 6816, tier3: 0, tier4: 0});
        pools[5] = LootPool({cost: 24, total: 1665, minTier: 4, tier1: 834, tier2: 831, tier3: 0, tier4: 0});
        pools[6] = LootPool({cost: 48, total: 3333, minTier: 5, tier1: 3328, tier2: 5, tier3: 0, tier4: 0});

        fullSets[0] = [23, 16, 27, 13, 36];
        fullSets[1] = [25, 27, 29, 14, 37];
        fullSets[2] = [30, 29, 23, 29, 38];
        fullSets[3] = [27, 20, 20, 25, 40];
        fullSets[4] = [26, 30, 15, 27, 39];

        bodyToEyesMouth[1] = [4, 6, 19, 8, 16, 20];
        bodyToEyesMouth[2] = [4, 22, 27, 5, 16, 23];
        bodyToEyesMouth[3] = [6, 23, 33, 2, 14, 23];
        bodyToEyesMouth[4] = [22, 23, 33, 5, 8, 16];
        bodyToEyesMouth[5] = [4, 6, 27, 20, 22, 23];
        bodyToEyesMouth[6] = [8, 13, 27, 5, 12, 16];
        bodyToEyesMouth[7] = [4, 26, 33, 3, 8, 18];
        bodyToEyesMouth[8] = [2, 6, 26, 3, 8, 18];
        bodyToEyesMouth[9] = [15, 22, 30, 3, 4, 17];
        bodyToEyesMouth[10] = [19, 26, 32, 5, 19, 20];
        bodyToEyesMouth[11] = [5, 10, 18, 9, 15, 16];
        bodyToEyesMouth[12] = [1, 16, 30, 3, 7, 19];
        bodyToEyesMouth[13] = [3, 8, 17, 10, 15, 20];
        bodyToEyesMouth[14] = [2, 19, 31, 2, 5, 21];
        bodyToEyesMouth[15] = [1, 4, 14, 10, 19, 21];
        bodyToEyesMouth[16] = [6, 10, 29, 1, 9, 16];
        bodyToEyesMouth[17] = [5, 8, 19, 9, 19, 21];
        bodyToEyesMouth[18] = [1, 26, 27, 4, 7, 19];
        bodyToEyesMouth[19] = [10, 13, 19, 1, 8, 15];
        bodyToEyesMouth[20] = [1, 7, 33, 8, 9, 10];
        bodyToEyesMouth[21] = [2, 12, 19, 6, 9, 21];
        bodyToEyesMouth[22] = [1, 3, 29, 9, 13, 18];
        bodyToEyesMouth[23] = [27, 28, 34, 4, 10, 19];
        bodyToEyesMouth[24] = [7, 26, 29, 8, 9, 15];
        bodyToEyesMouth[25] = [8, 14, 29, 1, 6, 7];
        bodyToEyesMouth[26] = [1, 3, 26, 9, 10, 22];
        bodyToEyesMouth[27] = [1, 21, 29, 1, 10, 19];
        bodyToEyesMouth[28] = [7, 9, 26, 1, 7, 16];
        bodyToEyesMouth[29] = [3, 10, 24, 15, 19, 23];
        bodyToEyesMouth[30] = [7, 28, 34, 6, 9, 21];

        // // LEAF
        // leaf = new Leaf();
        // leaf.grantRole(leaf.DEFAULT_ADMIN_ROLE(), msg.sender);
    }
    ...
    function mintWhitelist(
        ...
        if (msg.sender != owner() && tokenId > 100) {
    ...
    function claimableLeaf(uint256 id) public view returns (uint256 _claimable) {
        ...
        _claimable = (24 * 6 * (diff * (leafModifier(id)) * 1 ether)) / 1 days;
    ...
    function ownerSnailsCount(address _owner) public view returns (uint256) {
        uint256 numIds = 0;
        Activity memory activity;
        for (uint256 i = 1; i <= _tokenIdCounter.current(); i++) {
            activity = snailActivities[i];
            if (activity.user == _owner) numIds++;
        }
        return numIds;
    }

    function ownerSnailByIndex(address _owner, uint256 index) public view returns (uint256) {
        uint256 numIds = 0;
        Activity memory activity;
        for (uint256 i = 1; i <= _tokenIdCounter.current(); i++) {
            activity = snailActivities[i];
            if (activity.user == _owner) {
    ...
    function lootForSnail(
        ...
        snailActivities[id].lastRoll = block.timestamp;
    ...
    function canLoot(uint256 id, uint8 rollTier) public view returns (bool) {
        Activity memory currentActivity = snailActivities[id];
        return block.timestamp - currentActivity.lastRoll > _cooldownForTier(rollTier);
    }
    ...
    function _checkFullSet(uint256 id) internal {
        ...
        for (uint8 i = 0; i < 5; i++) {
            ...
            uint8[] memory set = fullSets[i];
            if (snail.background == set[0] && snail.body == set[1] && snail.shell == set[2] && snail.trail == set[3]) {
                snail.accessory = set[4];
                setClaimed[i] = true;
                snails[id] = snail;
                break;
            }
    ...
    function tierUnlocked(uint8 tier) public view returns (bool) {
        ...
        return block.timestamp > (startTime + 1 * uint88(tier - 1) * 5 minutes);
    }
    ...
    function _mintSnail() internal {
        ...
        snailActivities[id] = Activity({user: msg.sender, since: block.timestamp, state: ActivityState.UNSTAKED, lastRoll: 0});
    }
