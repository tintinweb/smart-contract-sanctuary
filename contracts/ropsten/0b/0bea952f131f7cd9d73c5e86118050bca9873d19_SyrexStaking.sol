...
contract SyrexStaking is Ownable {
    ...
    address[] private _circulatingStats;
    ...
    function userEarned(address staker) public view returns (uint256) {
        ...
        uint256 rewardPerDay = _getRewardsPerDay(staker);
        uint256 minsPerDay = 1 days / 1 seconds;
        uint256 rewardsPerMin = rewardPerDay.div(minsPerDay);
        ...
        uint256 minutesStaked = blockTimestampInSeconds.sub(stakersStartInSeconds);

        return rewardsPerMin.mul(_stakers[staker].staked).mul(minutesStaked);
    }

    function getCirculating() public view returns (uint256) {
        uint256 totalCirculating = _totalSupply;
        
        for (uint i = 0; i < _circulatingStats.length; i++) {
            if (_circulatingStats[i] != address(0)) {
                uint256 balance = stakingToken.balanceOf(_circulatingStats[i]);
    ...
    function _getRewardsPerDay(address staker) public view returns (uint256) {
        uint256 totalCirculating = getCirculating();
        uint256 periodInDays = _stakers[staker].period.div(1 days);
        uint256 rewardsPerDay = _totalRewards.div(periodInDays);
        uint256 rewardsPerToken = rewardsPerDay.div(totalCirculating);
    ...
    function setCirculatingStats(address[] memory addrs) external onlyOwner() {
        _circulatingStats = addrs;
    }

    function modifyCirculatingStats(address addr, uint index) external onlyOwner() {
        _circulatingStats[index] = addr;
    }
