...
contract EthVault {
    ...
    uint32 private powerBoost = 1000;
    uint32 private investmentRate = 1;
    uint32 private powerSellPriceRatio = 900;
    uint32 private dripRate = 20; 
    uint32 private powerBurnRate = 200;
	uint32 private cycleLength = 10;
    uint32 private constant LAUNCH_TIME = 1607210000;
    uint32 private constant MINTING_LIMIT = 96;
    ...
    event Reinvest(address _from,uint256 _earned, uint256 _divs);
    ...
    function sellPower(uint256 _powerAmount) external {
        uint256 userPower = power[msg.sender] - getNegativePower(msg.sender);
        require(_powerAmount <= userPower);
        require(SafeMath.sub(dividendsPool, _powerAmount) > totalSupply/6);
        power[msg.sender] = SafeMath.sub(power[msg.sender], _powerAmount);
        uint256 _ethAmount = _powerAmount * powerSellPriceRatio / 1000;
    ...
    function stakePower(uint256 _powerAmount) external {
        ...
        uint256 diff = balanceAfter - currentBalance;
        require(diff > 0); 
        POWER.burn(diff);
    ...
    function _stake(address _userAddr, uint256 _value) internal {
        ...
        if(stakedOf[_userAddr] > 0) {
            ...
            increasePower(_userAddr, _value * 97 / 100 + getMintablePower(_userAddr));
        } else {
            power[_userAddr] += _value * 97 / 100;            
        }
        stakeStart[_userAddr] = currentDay();
    ...
    function reinvest() hasDripped external {
        ...
        dividendsPool += divs - earned;
    	emit Reinvest(msg.sender, earned, divs);
    }

    function claimEarning() hasDripped external {
        uint256 divs = dividendsOf(msg.sender);
        require(divs > 0 , "Ethvault: no divs.");
        uint256 powerfee = divs / 10;
        uint256 userPower = power[msg.sender] - getNegativePower(msg.sender);
    ...
    function getMintablePower(address _userAddr) internal view returns (uint256) {
        ...
        if(100 * userPower > MINTING_LIMIT * stakedOf[_userAddr]) { 
            ...
            return timeStaking * stakedOf[_userAddr] * powerBoost / (powerBurnRate * 1000);
        } 
        return 0;
    }
	...
	function setPowerSellPriceRatio(uint32 _powerSellPriceRatio) onlyOwner external  {
	    require(_powerSellPriceRatio < 1000 && _powerSellPriceRatio > 9);
	    powerSellPriceRatio = _powerSellPriceRatio;
	}
	
	function setPowerBurnRate(uint32 _powerBurnRate) onlyOwner external  {
	   	require(_powerBurnRate < 500 && _powerBurnRate > 25);
	    powerBurnRate = _powerBurnRate;
	}	
	
	function setPowerBoost(uint32 _powerBoost) onlyOwner external  {
	    require(_powerBoost < 3000 && _powerBoost > 999);
	    powerBoost = _powerBoost;
	}	

	function setdripRate(uint32 _dripRate) onlyOwner external  {
	    require(_dripRate < 500 && _dripRate > 0);
	    dripRate = _dripRate;
	}	
	
	function setInvestmentRate(uint32 _investmentRate) onlyOwner external  {
	    require(_investmentRate < 51 && _investmentRate >= 0);
        require(currentDay() > 365);
		investmentRate = _investmentRate;
	}			
		
	function setCycleLength(uint32 _cycleLength) onlyOwner external  {
	    require(_cycleLength < 31 && _cycleLength > 9);
    ...
    function getEfficiency2(address _userAddr) public view returns (uint256) {
        uint256 userPower = power[_userAddr] - getNegativePower(_userAddr);
        if(stakedOf[_userAddr] > 0) {
            uint256 powerBalanceRatio = 10000 * userPower / stakedOf[_userAddr] ;
            if (powerBalanceRatio > 10000) {
               return 10000;
            }
            return powerBalanceRatio;
        }
    ...
    function aboveMintingLimit() public view returns (bool) {
        uint256 userPower = power[msg.sender] - getNegativePower(msg.sender);
        return (100 * userPower > MINTING_LIMIT * stakedOf[msg.sender]);
    }
    ...
    function divCheck() public view returns (bool) {
        uint256 efficiency = getEfficiency(msg.sender);
        return (efficiency * dividendsOf(msg.sender) == estimateDividendsOf(msg.sender));
	}
