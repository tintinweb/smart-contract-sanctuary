...
//    }
    ...
    function getListTokens(KyberNetwork network) public view returns (address[] memory oReserveTokens, address[] memory conversionRates) {
        ...
        conversionRates = new address[](numConversionRates);
        ...
        for(rID = 0; rID < numReserves; rID++) {
            ...
            if (network.reserveType(reserve) == KyberNetwork.ReserveType.PERMISSIONLESS) {
                ...
                if (ConversionRates(KyberReserve(reserve).conversionRatesContract()) != address(0)) {
                    conversionRates[numConversionRates] = ConversionRates(KyberReserve(reserve).conversionRatesContract());
                    numConversionRates += 1;
                }
            }
        }
