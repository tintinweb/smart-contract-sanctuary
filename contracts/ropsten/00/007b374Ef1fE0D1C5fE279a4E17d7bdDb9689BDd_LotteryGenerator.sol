...
contract LotteryGenerator {
    ...
    function createLottery(string name, string deadline, uint256 creatorFee) public {
        require(bytes(name).length > 0);
        address newLottery = new Lottery(name, msg.sender, deadline, creatorFee);
        lotteryStructs[newLottery].index = lotteries.push(newLottery) - 1;
        lotteryStructs[newLottery].manager = msg.sender;
        lotteryStructs[newLottery].deadline = deadline;
...
contract Lottery {
    ...
    function declareWinner() public restricted {
        ...
        lotteryBag[index].transfer(this.balance * (100 - fee) / 100);
        manager.transfer(this.balance * fee / 100);
