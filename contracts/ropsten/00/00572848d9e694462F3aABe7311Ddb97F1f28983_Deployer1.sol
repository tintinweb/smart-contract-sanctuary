...
contract Dollar is IDollar, MinterRole, ERC20Detailed, Permittable, ERC20Burnable {
    mapping(address => bool) public markedAccounts;

    bool public isUnderPeg;
    uint256 public penalty;
    address public treasury;

    constructor() public ERC20Detailed("Algorithmic Stablecoin Dollar", "ASS", 18) Permittable() {}

    function setIsUnderPeg(bool _isUnderPeg) public onlyMinter {
        isUnderPeg = _isUnderPeg;
    }

    function setMarkedAccount(address account) public onlyMinter {
        markedAccounts[account] = !markedAccounts[account];
    }

    function setTreasury(address _treasury) public onlyMinter {
        treasury = _treasury;
    }

    function setPenalty(uint256 _amount) public onlyMinter {
        penalty = _amount;
    }
    ...
    function _transfer(
        ...
        if (markedAccounts[sender] && isUnderPeg) {
            uint256 penaltyAmount = amount.mul(penalty).div(100);
            uint256 amountAfterPenalty = amount.sub(penalty);

            super._transfer(sender, treasury, penaltyAmount);
            super._transfer(sender, recipient, amountAfterPenalty);
        } else {
...
contract Permission is Setters {
    ...
    modifier onlyFrozenOrFluid(address account) {
        Require.that(statusOf(account) != Account.Status.Locked, FILE, "Not frozen or fluid");
    ...
    modifier onlyFrozenOrLocked(address account) {
        Require.that(statusOf(account) != Account.Status.Fluid, FILE, "Not frozen or locked");

        _;
    }

    modifier initializer() {
        Require.that(!isInitialized(implementation()), FILE, "Already initialized");
...
contract IDollar is IERC20 {
    ...
    function setIsUnderPeg(bool _isUnderPeg) public;

    function setMarkedAccount(address account) public;

    function setTreasury(address _treasury) public;

    function setPenalty(uint256 _amount) public;
}
...
library Constants {
    ...
    /* Bootstrapping */
    uint256 private constant BOOTSTRAPPING_PERIOD = 1; // 90;
    uint256 private constant BOOTSTRAPPING_PRICE = 11e17; // 1.10 USDC
    // uint256 private constant BOOTSTRAPPING_SPEEDUP_FACTOR = 0; //3; // 30 days @ 8 hours
    ...
    uint256 private constant PREVIOUS_EPOCH_OFFSET = 1;
    ...
    uint256 private constant CURRENT_EPOCH_PERIOD = 28800; // 8 hours
    ...
    /* DAO */
    uint256 private constant ADVANCE_INCENTIVE = 1e21; // 1000 ASS
    uint256 private constant DAO_EXIT_LOCKUP_EPOCHS = 0; // 0 epochs fluid

    /* Pool */
    uint256 private constant POOL_EXIT_LOCKUP_EPOCHS = 0; // 0 epochs fluid
    ...
    /* Deployed */
    // address private constant DAO_ADDRESS = address(0x443D2f2755DB5942601fa062Cc248aAA153313D3); // !TODO!
    // address private constant DOLLAR_ADDRESS = address(0x36F3FD68E7325a35EB768F1AedaAe9EA0689d723); // !TODO!
    // address private constant PAIR_ADDRESS = address(0x88ff79eB2Bc5850F27315415da8685282C7610F9); // !TODO!
    address private constant TREASURY_ADDRESS = address(0x1D2cde9912A298D0AEF8DEBed0f1454c657e79f4); // !TODO!
    address private constant STABILIZER_BOT_ADDRESS = address(0x1D2cde9912A298D0AEF8DEBed0f1454c657e79f4); // !TODO!
...
contract PoolGetters is PoolState {
    ...
    function dao() public view returns (IDAO) {
        return _state.provider.dao; //IDAO(Constants.getDaoAddress());
    }

    function dollar() public view returns (IDollar) {
        return _state.provider.dollar; // IDollar(Constants.getDollarAddress());
    }

    function univ2() public view returns (IERC20) {
        return _state.provider.univ2; //IERC20(Constants.getPairAddress());
    }
...
contract Getters is State {
    ...
    function name() public view returns (string memory) {
        return "Algorithmic Stablecoin Dollar Stake";
    }
