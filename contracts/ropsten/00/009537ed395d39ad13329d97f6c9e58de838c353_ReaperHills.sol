...
contract MerkleProof is Ownable {
     ...
     * @param proof hashes to validate
     */
    function isProofValid(bytes32[] memory proof) public returns (bool) {
    ...
    IERC2981
{
    ...
    // part of royalty that is being distributed to the team
    uint256 public constant ROYALTY_SELL_PERCENT_TEAM = 50; // 5% of sell price
    ...
    string public nftIPFSHashProof = "";
    // the interval in which royalty gets collected
    uint256 public royaltyInterval = 2 weeks;

    // address team to receive the minting value and part of royalties
    address public royaltyReceiverTeam;
    ...
    mapping(uint256 => NFTDetails) public ownedTokensDetails;
    // mapping from owner address to the stage index to start collecting royalties
    mapping(address => uint256) private ownerRoyaltyStageIndex;
    ...
    constructor(
        ...
        royaltyReceiverTeam = royaltyReceiver_;
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721, ERC165Storage, ERC721Enumerable, IERC2981)
        returns (bool)
    {
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        royaltyAmount = (_salePrice * ROYALTY_SELL_PERCENT) / 100;
    }
     ...
     */
    function setIPFSHashProof(string memory hashProof) public onlyOwner {
        require(bytes(hashProof).length > 0, "hash proof is empty");
        nftIPFSHashProof = hashProof;
    }
    ...
    function changeRoyaltyInterval(uint256 royaltyInterval_) public onlyOwner {
        require(
            ...
            "The royalty interval is the same"
        );
        royaltyInterval = royaltyInterval_;
    }
    ...
    /**
     * @dev Get the index of current royalty stage to start collecting royalties for sender
     */
    function getCurrentRoyaltyStageIndex() public view returns (uint256) {
        return ownerRoyaltyStageIndex[msg.sender];
    }
    ...
    function _mint(uint256 _count) internal {
        ...
        royaltyStages[getLastRoyaltyStageIndex()].totalSupply += _count;

        for (uint256 i = 1; i <= _count; i++) {
            uint256 id = randomIndex(); //totalSupply + i;
            uint256 currentIndex = ownerTokenList[msg.sender].length;
            ownerTokenList[msg.sender].push(id);
            ownedTokensDetails[id] = NFTDetails(
                id,
                currentIndex,
                block.timestamp
            );
        ...
        _splitAmount(royaltyReceiverTeam);
    }

    /**
     * @dev The contract gets royalties from the token sell
     * @param _to address to wich to send the remaining token sell value
     */
    function _splitAmount(address _to) internal {
        if (msg.value == 0) {
            return;
        }
        uint256 royaltyAmount = (msg.value * ROYALTY_SELL_PERCENT) / 100;

        if (royaltyAmount > 0) {
            uint256 royaltyAmountTeam = (royaltyAmount *
                ROYALTY_SELL_PERCENT_TEAM) / 100;

            royaltyStages[getLastRoyaltyStageIndex()].amount +=
                royaltyAmount -
                royaltyAmountTeam;

            address payable receiverTeam = payable(
                address(royaltyReceiverTeam)
            );
            (bool successToTeam, ) = receiverTeam.call{
                value: royaltyAmountTeam
            }("");
            require(successToTeam, "Transfer failed for team.");

            address payable receiverContract = payable(address(this));
            (bool successToContract, ) = receiverContract.call{
                value: royaltyAmount - royaltyAmountTeam
            }("");
            require(successToContract, "Transfer failed for contract.");
        }

        address payable to = payable(_to);
        uint256 valueToSendToSeller = msg.value - royaltyAmount;
        (bool successToSeller, ) = to.call{value: valueToSendToSeller}("");
        require(successToSeller, "Transfer failed for seller.");
    }
    ...
    function withdrawRoyalty() external {
        uint256 currentIndex = getCurrentRoyaltyStageIndex();
        uint256 lastIndex = getLastRoyaltyStageIndex();
        require(
            currentIndex < lastIndex,
            "Can't withdraw royalty for the current royalty stage"
        );

        uint256 royaltyAmount = 0;
        for (uint256 i = currentIndex; i < lastIndex; i++) {
            uint256 lastStartDate = royaltyStages[i + 1].startDate;
            RoyaltyStage memory stage = royaltyStages[i];
            if (stage.amount == 0) {
                continue;
            }
            uint256 eligibleTokenCount = 0;
            uint256[] memory ownerTokens = ownerTokenList[msg.sender];
            for (uint256 j = 0; j < ownerTokens.length; j++) {
                NFTDetails memory details = ownedTokensDetails[ownerTokens[j]];
                if (
                    stage.startDate <= details.timestamp &&
                    details.timestamp < lastStartDate
                ) {
                    eligibleTokenCount++;
                }
            }
        ...
        require(royaltyAmount > 0, "There is not enough royalty to withdraw");

        ownerRoyaltyStageIndex[msg.sender] = lastIndex;

        address payable to = payable(msg.sender);
        (bool success, ) = to.call{value: royaltyAmount}("");
        require(success, "Transfer failed.");
    }

    /**
     * @dev Withdraw royalties for a specific tokenId for all the royalty stages the token was hold by the sender
     * @param tokenId for which to collect royalties
     */
    function _withdrawRoyaltyOfToken(address from, uint256 tokenId) internal {
        uint256 currentIndex = getCurrentRoyaltyStageIndex();
        uint256 lastIndex = getLastRoyaltyStageIndex();

        if (currentIndex < lastIndex) {
            NFTDetails memory details = ownedTokensDetails[tokenId];
            uint256 royaltyAmount = 0;
            for (uint256 i = currentIndex; i < lastIndex; i++) {
                RoyaltyStage memory stage = royaltyStages[i];
                if (stage.amount == 0) {
                ...
                if (stage.startDate >= details.timestamp) {
                    royaltyAmount += stage.amount / stage.totalSupply;
                }
            }
            if (royaltyAmount > 0) {
                address payable currentTokenOwner = payable(from);
                (bool success, ) = currentTokenOwner.call{value: royaltyAmount}(
                    ""
                );
                require(success, "Transfer failed.");
            }
    ...
    function nextRoyaltyStage() public onlyOwner {
        require(canChangeRoyaltyStage(), "The stage can't be changed");
        _nextRoyaltyStage();
    }
    ...
    /**
     * @dev Create a new royalty stage if it's possible
    ...
    function _nextRoyaltyStage() internal {
        royaltyStages.push(RoyaltyStage(block.timestamp, 0, totalSupply()));
    }

    /**
     * @dev Change token owner details mappings
     */
    function _changeTokenOwner(
        address from,
        address to,
        uint256 tokenId
    ) internal {
        ...
        ownedTokensDetails[tokenId] = NFTDetails(
            tokenId,
            currentIndex,
            block.timestamp
        );
    ...
    function safeTransferFrom(
        ...
        _withdrawRoyaltyOfToken(from, tokenId);
        _changeTokenOwner(from, to, tokenId);
