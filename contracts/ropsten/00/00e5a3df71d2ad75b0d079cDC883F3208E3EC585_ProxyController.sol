...
contract ProxyController is AccessControl, IProxyController {
     ...
     *
     */
    function startOrder(uint256 amountIn, OrderInput memory input) public virtual onlyRole(EXECUTOR_ROLE) {
        ...
        CTokenInterface cTokenA = CTokenInterface(input.cTokenA);
        CTokenInterface cTokenB = CTokenInterface(input.cTokenB);

        // oracle price
        uint256 priceA = IPriceOracle(_oracle).getUnderlyingPrice(cTokenA);
        uint256 priceB = IPriceOracle(_oracle).getUnderlyingPrice(cTokenB);
