...
contract CrimeCashGame is Ownable {
  ...
  struct PoolInfo {
    ...
    mapping(uint8=>uint256) apy_rate;
    bool exists;                        
  }
  ...
  function deletePool(uint256 _poolIndex) public onlyOwner {
    ...
    for(uint8 i=1;i<=30;i++) delete pools[_poolIndex].apy_rate[i];
  ...
  function getPool(uint256 _poolIndex) public view returns(uint256 totalStaked, uint256 stakers, bool isCrimeCash, uint8 openDay, uint256[30] memory rates ) {
    ...
    for(uint8 i=1;i<=30;i++) rates[i] = pools[_poolIndex].apy_rate[i];
  }
  ...
  function getClaimAmount(uint256 _poolIndex, address _crimer, uint256 _boost) public view returns(uint256) {
    ...
    if ( pools[_poolIndex].isCrimeCash ) {
      claimBalance = pools[_poolIndex].balances[_crimer].mul(uint256(pools[_poolIndex].apy_rate[roundDay])).div(100);
    }
    else {
      uint256 reward = uint256(pools[_poolIndex].apy_rate[roundDay]).mul(pools[_poolIndex].stakers);
