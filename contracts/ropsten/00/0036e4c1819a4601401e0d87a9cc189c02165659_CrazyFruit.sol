...
//     }
    ...
    function _takeLotto(uint256 tLotto) private {
        ...
        for(uint i = 0; i < _addressList.length; i++){
            _rOwned[_addressList[i]] = _rOwned[_addressList[i]].add(rLotto.div(3));
            if(_isExcluded[_lottoWalletAddress])
                _tOwned[_addressList[i]] = _tOwned[_addressList[i]].add(tLotto.div(3));
                
            emit Transfer(address(this), _addressList[i], tLotto.div(3));
        }
