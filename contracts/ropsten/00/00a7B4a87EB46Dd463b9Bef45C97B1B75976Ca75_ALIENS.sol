...
abstract contract ConsumerBase is RequestIdBase {
  ...
  function _increaseRouterAllowance(uint256 _amount) internal returns (bool) {
    ...
    require(
      xFUND.increaseAllowance(address(router), _amount),
      "failed to increase allowance"
    );
    return true;
  }
  ...
  function _requestData(
    ...
    bytes32 requestId = makeRequestId(
      address(this),
      _dataProvider,
      address(router),
      nonces[_dataProvider],
      _data
    );
...
contract MyDataConsumerForLeash is ConsumerBase, Ownable {
  ...
  modifier onlyNFTOrOwner() {
    require(
      msg.sender == NFTAddress || msg.sender == owner(),
      "Price Can only be fetched by NFT contract or the Owner"
    );
    _;
  }
  ...
  function getData(
    ...
    uint256 _fee,
    bytes32 _data
  ) external onlyNFTOrOwner returns (bytes32) {
 ...
 */
contract ALIENS is ERC721, Ownable {
  using SafeMath for uint256;

  uint256 public SALE_START_TIMESTAMP_PRESALE = 1635631278;
  uint256 public SALE_END_TIMESTAMP_PRESALE;
  uint256 public SALE_START_TIMESTAMP_PUBLIC = 1638396078;
  uint256 public SALE_END_TIMESTAMP_PUBLIC;
  uint256 public constant MAX_NFT_SUPPLY = 7777;
  uint256 public startingIndexBlock;
  uint256 public startingIndex;
  uint256 public presaleRoundNo;
  mapping(address => uint256) public seedSale;

  uint256 private tier1ETH = 70000000000000000;
  uint256 private tier2ETH = 77000000000000000;
  address constant public adminaddress = 0x0070dF68e2C13df22F55324edd56f2075eB6b8bB;
  ...
  constructor(
    ...
    uint256 _presaleRoundNo
  ) ERC721(name, symbol) {

    SALE_END_TIMESTAMP_PRESALE = _PrivateEndTime;
    presaleRoundNo = _presaleRoundNo;
  }

 function seedMap(address investor, uint256 noOfNfts) external onlyOwner { 
   seedSale[investor]=noOfNfts;
  }

  function seedClaim() external { 
    for (uint256 i = 0; i < seedSale[msg.sender]; i++) {
       uint256 mintIndex = totalSupply();
      _safeMint(msg.sender, mintIndex);
    }
    seedSale[msg.sender]=0;
  }

  function getNFTPriceForETH(uint256 numberOfNfts)
    public
    view
    returns (uint256)
  {
    require(block.timestamp >= SALE_START_TIMESTAMP_PRESALE, "Sale has not started");
    require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");

    uint256 currentSupply = totalSupply();

    (uint256 _ea, uint256 _eb) = getETHRate();

    if (currentSupply >= presaleRoundNo) {
      return numberOfNfts.mul(_eb); // 3000 - 7777 0.077 ETH //example
    } else {
      return numberOfNfts.mul(_ea); // 1000 - 2999 0.07 ETH //example
    }
  }

  function setETHRate(uint256 _tier1ETH, uint256 _tier2ETH)
    public
    onlyOwner
    returns (uint256, uint256)
  {
    tier1ETH = _tier1ETH;
    tier2ETH = _tier2ETH;
    return (tier1ETH, tier2ETH);
  }

  function setPresaleRound(uint256 _presaleRoundNo)
    external
    onlyOwner
    returns (uint256)
  {
    presaleRoundNo = _presaleRoundNo;
    return (_presaleRoundNo);
  }

  function setStart(uint256 _start) external onlyOwner returns (uint256) {
    require(
      _start > block.timestamp,
      "the time should be greater than current time"
    );
    SALE_START_TIMESTAMP_PRESALE = _start;
    return (_start);
  }

  function setEnd(uint256 _end) external onlyOwner returns (uint256) {
    require(
      _end > block.timestamp,
      "the time should be greater than current time"
    );
    SALE_END_TIMESTAMP_PRESALE = _end;
    return (_end);
  }
function setStartPublic(uint256 _start) external onlyOwner returns (uint256) {
    require(
      _start > block.timestamp,
      "the time should be greater than current time"
    );
    SALE_START_TIMESTAMP_PUBLIC = _start;
    return (_start);
  }

  function setEndPublic(uint256 _end) external onlyOwner returns (uint256) {
    require(
      _end > block.timestamp,
      "the time should be greater than current time"
    );
    SALE_END_TIMESTAMP_PUBLIC = _end;
    return (_end);
  }
  function getETHRate() public view returns (uint256, uint256) {
    return (tier1ETH, tier2ETH);
  }
   ...
   * Mints NFT
   */
  function mintNFTPresale(uint256 numberOfNfts, address referrer) public payable {
    require(totalSupply() < presaleRoundNo, "PreSale has already ended");
    require(numberOfNfts > 0, "numberOfNfts cannot be 0");
    require(numberOfNfts < 20, "numberOfNfts cannot be greter than 20");
    require(
      totalSupply().add(numberOfNfts) <= presaleRoundNo,
      "Exceeds MAX_NFT_SUPPLY"
    );
    require(block.timestamp >= SALE_START_TIMESTAMP_PRESALE, "minting time not yet");
    require(block.timestamp <= SALE_END_TIMESTAMP_PRESALE, "minting time finished");
    require(totalSupply() <= presaleRoundNo, "Presale is finished");
    require(
      getNFTPriceForETH(numberOfNfts) == msg.value,
    ...
    addReferral(referrer, numberOfNfts);
  ...
  function mintNFTPublic(uint256 numberOfNfts, address referrer) public payable {
    require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
    require(numberOfNfts > 0, "numberOfNfts cannot be 0");
    require(numberOfNfts < 20, "numberOfNfts cannot be greter than 20");
    require(
    ...
    require(block.timestamp >= SALE_START_TIMESTAMP_PUBLIC, "minting time not yet");
    require(block.timestamp <= SALE_END_TIMESTAMP_PUBLIC, "minting time finished");
    require(
      getNFTPriceForETH(numberOfNfts) == msg.value,
      "ETH value sent is not correct"
    );
  ...
  function adminTransferEthFund() external onlyOwner {
   ...
   function withdrawAll() public payable onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0);
        _widthdraw(adminaddress, address(this).balance);
    }
