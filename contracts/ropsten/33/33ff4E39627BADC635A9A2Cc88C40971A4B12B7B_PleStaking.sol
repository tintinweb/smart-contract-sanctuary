...
contract PleStaking {
    ...
    // limit staking reward amount
    uint256 public limitToRewards =10000000000000000000000000;
    ...
    constructor(address _tokenAddress, address _tokenFeeAddress,address _tokenRewardAddress) public {
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivs > 0) {
            ...
            require(
               tokenContract.transferFrom(tokenRewardAddress,account, pendingDivs),
    ...
    function getPendingDivs(address _holder) public returns (uint256) {
        if (!holders.contains(_holder)) return 0;
        if (depositedTokens[_holder] == 0) return 0;
        if (totalClaimedRewards >= limitToRewards) {
        ...
        uint256 pendingDivs =
            stakedAmount.mul(rewardRate).mul(timeDiff).div(rewardInterval).div(
                1e4
            );
