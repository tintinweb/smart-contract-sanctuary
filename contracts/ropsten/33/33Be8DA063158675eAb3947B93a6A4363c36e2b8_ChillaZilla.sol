...
contract ChillaZilla is ChillaZillaERC721 {
    ...
    event ZillaRevealed(uint256 zillaId); //ipfs ada listener buat event ini buat ganti image url baby zilla
...
contract ChillaZillaERC721 is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private baseURI; //maybe buat pisahin yg reveal dan not revealed di opensea?

    uint256 public maxSupply;
    uint256 public maxGenCount; //max genesis zilla count
    uint256 public hatchlingCount = 0;
    ...
    mapping (address => uint256) public presaleWhitelist; // max 2 NFT, whitelist spot 1000
    ...
    function editWhitelist(address[] calldata whitelistAddreses, uint256[] calldata amount) external onlyOwner {
        for(uint256 i; i < whitelistAddreses.length; i++){
            presaleWhitelist[whitelistAddreses[i]] = amount[i];
        }
    }
