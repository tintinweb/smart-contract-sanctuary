...
contract PPSwap is ERC20Interface, SafeMath{
    ...
    */
    ...
    function safeSwap(address accountA, 
        ...
        // The swap fee is shared by both parties, half for each
        balances[accountA] = safeSub(balances[accountA], swapfeePerTrans/2);
        balances[accountB] = safeSub(balances[accountB], swapfeePerTrans/2);
    ...
    /* If customers send ETH to this contract by accident, let's tranfer to the contractOwner to refund them back to the customers. */
    function withDraw()
    external
    onlyOwner
    {
        safeWithdrawByContract();
    }
     
    function safeWithdrawByContract()
    private
    {
        trustAccount.transfer(msg.value);
    }
