...
contract MamaCoco is IBEP20, LockableFunction, MamaCocoNoBSSupport, LPSwapSupport, AntiLPSniper{
    ...
    constructor(string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, address routerAddress, address noBSRouterAddress,
        ...
        minTokenSpendAmount = maxWalletSize.div(10000);
    ...
    function performSwap() private lockTheSwap {
        ...
        if(taxAllocations.reflectionToken1 >= taxAllocations.reflectionToken2){
    ...
    function openTrading() external onlyOwner {
        ...
        swapsEnabled = true;
        taxAllocations.reflectionToken1 = _balances[address(this)];
    }
}
...
abstract contract MamaCocoNoBSSupport is NoBSAdvSupport {
    ...
    function setDistributionCriteria(uint256 minPeriod, uint256 minDistribution) external onlyOwner {
        noBSReflectors[0].setDistributionCriteria(minPeriod, minDistribution);
        noBSReflectors[1].setDistributionCriteria(minPeriod, minDistribution);
    }
}
...
abstract contract NoBSAdvSupport is NoBSInit, Ownable, LockableFunction {
    ...
    function updateReflectors(address from, uint256 fromBal, address to, uint256 toBal) internal {
        for(uint256 i = 0; i < totalReflectors; ++i){
            noBSReflectors[i].setShare(from, fromBal);
            noBSReflectors[i].setShare(to, toBal);
        }
    }
    ...
    function distributeRewards(uint256 gasToUse) public {
        if(!locked){
            _distributeRewards(gasToUse);
        }
    }

    function _distributeRewards(uint256 gasToUse) private lockFunction {
        try getAReflector().process{gas: gasToUse}(gasToUse) {} catch {}
    }

    function _distributeRewardsForReflector(uint256 reflectorNo, uint256 gasToUse) private lockFunction {
        try noBSReflectors[reflectorNo].process{gas: gasToUse}(gasToUse) {} catch {}
    }
    ...
    function _manualClaim() private lockFunction {
        for(uint256 i = 0; i < totalReflectors; ++i){
            noBSReflectors[i].claimDividendFor(_msgSender());
        }
    }
    ...
    function excludeFromRewards(address hodler, bool shouldExclude) public onlyOwner {
        for(uint256 i = 0; i < totalReflectors; ++i){
            noBSReflectors[i].excludeFromReward(hodler, shouldExclude);
        }
    }
