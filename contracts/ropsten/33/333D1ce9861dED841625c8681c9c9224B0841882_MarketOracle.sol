...
/**
 * @title SUREMOON price Oracle
 *      This Oracle calculates the average USD price of SUREMOON based on the BNB-SUREMOON and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private suremoonBnbPrice0CumulativeLast;
    uint private suremoonBnbPrice1CumulativeLast;
    uint32 private suremoonBnbBlockTimestampLast;
    ...
    address private constant _busd = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F;

    IUniswapV2Pair private _suremoon_bnb;
    ...
    constructor(
        address __suremoon_bnb,   // Address of the suremoon-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _suremoon_bnb = IUniswapV2Pair(__suremoon_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        suremoonBnbPrice0CumulativeLast = _suremoon_bnb.price0CumulativeLast();
        suremoonBnbPrice1CumulativeLast = _suremoon_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, suremoonBnbBlockTimestampLast) = _suremoon_bnb.getReserves();
    ...
    // Get the average price of 1 suremoon in the smallest BNB unit (18 decimals)
    function getsuremoonBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_suremoon_bnb));

        require(_blockTimestamp != suremoonBnbBlockTimestampLast, "SUREMOON Last and current are equal");

        FixedPoint.uq112x112 memory suremoonBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - suremoonBnbPrice0CumulativeLast) / (_blockTimestamp - suremoonBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, suremoonBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint suremoonBnbAverage;
        uint busdBnbAverage;

        (suremoonBnbPrice0CumulativeLast, suremoonBnbPrice1CumulativeLast, suremoonBnbBlockTimestampLast, suremoonBnbAverage) = getsuremoonBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, suremoonBnbAverage) = getsuremoonBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = suremoonBnbAverage*(1e6) / busdBnbAverage;
