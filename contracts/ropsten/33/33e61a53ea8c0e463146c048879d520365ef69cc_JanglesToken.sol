    ...
    Pausable
{
    ...
    /** Staking portal will have the ability to mint. */
    ...
    function mintForIds(uint256[] calldata ids) external {
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            address owner = bojangles.ownerOf(id);
            uint256 claimed = lastClaim(id);
            uint256 owed = (block.timestamp - claimed) * janglesPerSecond;
            _mint(owner, owed);
            lastClaimOnNft[id] = block.timestamp;
        }
    }
