...
abstract contract Ownable is Context {
 ...
 contract swapverse is IERC20Metadata, Context, Ownable {
     ...
     mapping (address => bool) internal _isExcludedTxlimit;
     ...
     constructor () {
         ...
         _isExcludedTxlimit[owner()] = true;
         _isExcludedTxlimit[burnFeeReceiver] = true;
    ...
    function decimals() external pure override returns (uint8) {
     ...
     function setTxlimit(uint256 _maxTxAmount) external onlyOwner() {
    ...
    function excludeFromFee(address account) public onlyOwner() {
     ...
     function excludeFromTxLimit(address account) public onlyOwner() {
         _isExcludedTxlimit[account] = true;
     }
     
     function includeFromTxLimit(address account) public onlyOwner() {
         _isExcludedTxlimit[account] = false;
     }
     ...
     function isExcludedFromTxLimit(address account) public view returns(bool) {
         return _isExcludedTxlimit[account];
     }
     ...
     function _transfer(address from, address to, uint256 amount) internal {
         ...
         if(!_isExcludedTxlimit[from] && !_isExcludedTxlimit[to])
