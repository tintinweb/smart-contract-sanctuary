...
contract HpbPay{
    ...
    struct Merchant{
        ...
        string desc;//商户描述
        uint[] orderIndexs;//该商户的订单列表
        mapping (string => uint) indexMap;//订单号=》订单列表下标
    }
    /**
     * 付款者详细信息结构体
     */
     ...
     *查看付款者的订单,供内部调用
     */
    function _fetchOrderByIdWithWithPayer(
        string orderId,
        address payerAddr
    ...
    function fetchOrderByIdWithWithPayer(
        ...
        return _fetchOrderByIdWithWithPayer(orderId,msg.sender);
    }
    /**
     *管理员查询付款者的某个付款订单
     */
    function fetchOrderByIdWithWithPayerByAdmin(
        string orderId,
        address payerAddr
    )onlyAdmin public constant returns (
        address _from,
        ...
        return _fetchOrderByIdWithWithPayer(orderId,payerAddr);
    }
