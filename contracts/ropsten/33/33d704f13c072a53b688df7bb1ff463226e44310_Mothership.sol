...
contract Mothership {
    ...
    mapping(address => bool) private _activeContracts;

    constructor(){
        owner = msg.sender;
    }
    
    function getBalance(address _addy) public view returns (bytes[] memory) {
        bytes[] memory foundTokens;
        uint256 arrayIndex = 0;
        for (uint256 i=0;i<_contractAddresses.length;i++){
            uint256 addressBalance = ERC721(_contractAddresses[i]).balanceOf(_addy);
            ERC721Enumerable enumCon = ERC721Enumerable(_contractAddresses[i]);
            if (addressBalance > 0) {
                for (uint256 ii = 0;ii<addressBalance;ii++){
                    foundTokens[arrayIndex] = (abi.encodePacked(_contractAddresses[i],enumCon.tokenOfOwnerByIndex(_addy,ii)));
                    arrayIndex ++;
                }
            }
        }
    ...
    function activateContract(address _contract) public {
    ...
    function viewMothershipContracts() public view returns (address[] memory){
