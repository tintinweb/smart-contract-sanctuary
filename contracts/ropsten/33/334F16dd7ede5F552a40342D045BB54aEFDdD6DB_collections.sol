...
contract collections {
    ...
    function createCollection(string memory _logoURI, string memory _collectionName) external returns (address collection){
        
        collection = address(new Terraworld(_collectionName, _logoURI));
        myCollections[msg.sender].push(collection);
        allCollections.push(collection);

        emit CollectionCreated(msg.sender, collection);
        
        Ownable(collection).transferOwnership(msg.sender);
    }
    
    function getMyCollectionCount(address owner) public view returns(uint256 count){
        count = myCollections[owner].length;
    }
}