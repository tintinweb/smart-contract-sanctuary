...
library Parsing {
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        int256 indexB1 = Bytes.indexOf(blueprint, ",", 0);
        require(index >= 0, "Separator must exist");

        uint256 generation = Bytes.toUint(blueprint[0:uint256(indexB1)-1]);

        int256 indexB2 = Bytes.indexOf(blueprint[uint256(indexB1) + 1:blueprint.length], ",", 0);

        uint256 pirate_type = Bytes.toUint(blueprint[uint256(indexB1)+1:uint256(indexB2)-1]);

        return (tokenID, generation, pirate_type);
    }
