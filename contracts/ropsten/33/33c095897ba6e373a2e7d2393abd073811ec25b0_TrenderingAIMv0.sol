...
contract TrenderingAIMv0 is ERC20Burnable, Roles, TokenRecover {
    ...
    constructor(
        ...
        IERC20 _TRND_token, 
        IERC20 _xTRND_token, 
        IERC20 _ETH_TRND_LP_token, 
        IERC20 _DAI_TRND_LP_token, 
        IERC20 _DAI_xTRND_LP_token
        ...
        TRND_token = _TRND_token;
        xTRND_token = _xTRND_token;
        
        ETH_TRND_LP_token = _ETH_TRND_LP_token;
        DAI_TRND_LP_token = _DAI_TRND_LP_token;
        DAI_xTRND_LP_token = _DAI_xTRND_LP_token;
    ...
    function startVote() public {
        ...
        last_vote_deadline = block.timestamp + 7 days;
        vote_active = true;
    ...
    function stake_DAI_LPs(uint256 DAI_TRND_LP_amount, uint256 DAI_xTRND_LP_amount) public {
        ...
        if (user.DAI_TRND_LP_amount > 0 && user.DAI_TRND_LP_time > 0 && DAI_fees > 0) {
            ...
            uint256 DAI_reward_part = DAI_fees.div(2).mul(user_poolshare).mul(user_timeshare);
                    DAI_reward = DAI_reward.add(DAI_reward_part);
                    DAI_fees = DAI_fees.sub(DAI_reward_part);

            
            user.DAI_TRND_LP_time = this_time;
        }
        if (user.DAI_xTRND_LP_amount > 0 && user.DAI_xTRND_LP_time > 0 && DAI_fees > 0) {
