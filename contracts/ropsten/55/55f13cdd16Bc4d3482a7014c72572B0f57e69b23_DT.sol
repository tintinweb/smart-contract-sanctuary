...
contract DT is Context, Ownable, IERC20, LockableSwap, SwapWithLP, FeeLibrary, EthReflectingToken {
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(automatedReflectionsEnabled && (!distributionRoundCompleted || nextRoundStartTime > block.timestamp)){
            reflectRewards();
        }
    }
...
//    function calculateForOwner(uint256 amount) private view returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
...
//    }
    ...
    function calculateSell(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        if(block.timestamp > trader[from].lastBuy.add(sellFees.level[5].saleCoolDownTime)) {
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[1].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[1].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[2].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[2].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[3].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[3].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[4].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[4].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[5].saleCoolDownTime)) {
    ...
    function _transferStandard(address sender, address recipient, uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount) private {
        ...
        if(automatedReflectionsEnabled)
            reflectionContract.logTransactionEvent(sender, rAmount, recipient, rTransferAmount);
    ...
    function burn(uint256 amount) public {
        ...
        if(automatedReflectionsEnabled)
            reflectionContract.logTransactionEvent(_msgSender(), amount, address(0), 0);
        _burn(amount);
    }
...
contract EthReflectingToken is Ownable, ISupportingExternalReflection {
    ...
    function reflectRewards() internal {
        distributionRoundCompleted = reflectionContract.reflectRewards();
    }
...
//    event SwapETHForTokens(uint256 amount, address[] path);
    ...
    function swapTokensForEth(IUniswapV2Router02 uniswapV2Router, address destination, uint256 tokenAmount) internal {
    ...
    function swapETHForTokens(IUniswapV2Router02 uniswapV2Router, uint256 amount, address destination) internal {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
    ...
    function addLiquidity(IUniswapV2Router02 uniswapV2Router, address destination, uint256 tokenAmount, uint256 ethAmount) internal {
