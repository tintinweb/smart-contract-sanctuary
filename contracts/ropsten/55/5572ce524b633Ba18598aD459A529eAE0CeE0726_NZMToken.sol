 ...
 */
contract NZMToken is ERC20Detailed, ERC20Capped, ERC20Burnable {
    /**
    ...
    /**
     * buy amount tokens by using address
     *
     */
    function buy(uint256 amount) external payable returns (bool) {
        require(msg.sender != address(0), "ERC20: mint to the zero address");
        require(price.mul(amount).div(ONE) <= msg.value, "Ether value sent is not correct");
        _buy(msg.sender, amount);
        return true;
    }

    function setPrice(uint256 _price) external onlyMinter returns (bool) {
        require(_price != 0, "NZM: Can't set the price of token as Zero");
        price = _price;
        return true;
    }
