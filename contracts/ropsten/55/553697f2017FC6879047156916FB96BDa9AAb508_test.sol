            ...
            revert(
                string(
                    abi.encodePacked(
                        ...
                        Strings.toString(uint160(account)), //Strings.toHexString(uint160(account), 20),
                        " is missing role ",
                        Strings.toString(uint256(role)) //Strings.toHexString(uint256(role), 32)
                    )
                )
            );
        }
 ...
 */
contract test is ERC721, Ownable, PaymentSplitter, AccessControl {
    ...
    uint256 public tokenPrice = .05 ether;

    uint256 public constant maxTokens = 3500;

    uint256 public devTokenCount = 3500;

    string private _contractURI;

    uint256 public maxMintsPerTx = 5;

    bytes32 public constant whitelistRole = keccak256("WHITELIST_ROLE");
    ...
    function addWhitelisted(address[] memory accounts) public onlyOwner {
        for (uint256 account = 0; account < accounts.length; account++) {
            grantRole(whitelistRole, accounts[account]);
        }
    }

    function whitelistMint(uint256 quantity) external payable {
        require(
            hasRole(whitelistRole, msg.sender) == true,
