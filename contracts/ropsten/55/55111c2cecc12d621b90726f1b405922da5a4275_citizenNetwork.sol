...
contract citizenNetwork is IERC20,Context,Math {
   ...
   /** string name;
    ...
    /**
     * @dev alloocates the totalsupply to creator
     */
    ...
    uint256 number = 10900000;
    uint256  endBlock = 10869999;
    ...
    function applicationForCitizenship(string memory _eMailID)  _checkAddr  public payable { 
        // send exactly 0.05 ether to register the details
    ...
    event winner(uint proposalID,string  proposalName,address proposer,uint budget,uint voteCount);
    
    function setEnd(uint _endReg) public {
        endReg = _endReg;
    }
    
    function proposalRegistry(string memory _proposalName, uint _budget) public  {
        proposals.push(proposal({proposalID : nextProposalID, proposalName : _proposalName, proposer : msg.sender,budget : _budget,voteCount : 0 }));
        nextProposalID++;
    }
    ...
    function winningProject() external returns (address winningProposalAddress) {
        uint winningVoteCount = 0;
        for (uint i = 0; i < proposals.length; i++) {
            if (proposals[i].voteCount > winningVoteCount) {
                winningVoteCount = proposals[i].voteCount;
                winningProposalAddress = proposals[i].proposer;
                emit winner(proposals[i].proposalID,proposals[i].proposalName,proposals[i].proposer,proposals[i].budget,proposals[i].voteCount);
            }
        }
    ...
    function fundProject() public payable{
        require(voters[msg.sender].voted == true,"not eligible for funding");
        /**address addr = winningProject();
        payable(addr).transfer(msg.value);
        
        uint Contribution;
        Contribution = Contribution + msg.value;
        uint projectPerc = perc(msg.value, Contribution); 
        */
