...
contract NFTContract is ERC721, ERC721URIStorage, Ownable {
    ...
    function createNFT(address nftOwner, string memory metaData)
        public
        returns (uint256)
    {
        _tokenIds.increment();
    ...
    function buyNFT(uint256 _tokenId) public payable {
        ...
        require(tokenOwner != msg.sender, "You are already the owner");
        tokenInfo memory tokenInfoById = allTokensInfo[_tokenId];
        require(tokenInfoById.forSale, "NFT not available for sale");
        require(msg.value >= tokenInfoById.price, "Value sent is not correct");
    ...
    function changeTokenPrice(uint256 _tokenId, uint256 _newPrice) public {
        ...
        require(ownerOf(_tokenId) == msg.sender, "Token does not exist");
        tokenInfo memory tokenInfoById = allTokensInfo[_tokenId];
        tokenInfoById.price = _newPrice;
        allTokensInfo[_tokenId] = tokenInfoById;
    }
    function changeSellingStatus(uint256 _tokenId, bool status) public payable {
        require(msg.sender != address(0), "Invalid caller");
        require(_exists(_tokenId), "Token does not exist");
        require(ownerOf(_tokenId) == msg.sender, "You are not the owner");
        tokenInfo memory tokenInfoById = allTokensInfo[_tokenId];
        tokenInfoById.forSale = status;
        allTokensInfo[_tokenId] = tokenInfoById;
    }
}