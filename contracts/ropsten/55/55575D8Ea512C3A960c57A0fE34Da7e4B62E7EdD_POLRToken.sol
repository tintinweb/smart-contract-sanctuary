...
// File contracts/ERC20/POLRToken.sol

// contracts/GameItem.sol

pragma solidity ^0.8.0;
/// @author X89Lab
contract POLRToken is ERC20, ReentrancyGuard, Ownable {
    uint256 public constant DECIMAL_MULTIPLIER = 10**18;
    uint256 public constant TOTAL_SUPPLY = 1 * (10**11) * DECIMAL_MULTIPLIER;
    uint256 public constant SEED_SALE_SUPPLY = (12**9) * DECIMAL_MULTIPLIER;
    uint256 public constant PRIVATE_SALE_SUPPLY = (25**9) * DECIMAL_MULTIPLIER;

    uint256 public constant PUBLIC_SALE_SUPPLY = (3**9) * DECIMAL_MULTIPLIER;
    uint256 public constant FOUNDATION_SUPPLY = (20**9) * DECIMAL_MULTIPLIER;
    uint256 public constant MINT_REWARD_SUPPLY = (30**9) * DECIMAL_MULTIPLIER;
    uint256 public constant LIQUIDITY_SUPPLY = (3**9) * DECIMAL_MULTIPLIER;

    uint256 public constant ADVISOR_SUPPLY = (7**9) * DECIMAL_MULTIPLIER;

    uint256 public constant BLACKLIST_LOCK_DAYS = 50;
    uint256 public constant BLACKLIST_EFFECTIVE_DURATION = 30 days;

    uint256 public totalSeedFundReleased = 0;
    uint256 public totalPrivateSaleReleased = 0;
    ...
    constructor(string memory name, string memory symbol) ERC20(name, symbol) {
        ...
        _blacklistEffectiveEndtime =
            block.timestamp +
            BLACKLIST_EFFECTIVE_DURATION;
    }
    ...
    function getBlacklistByUser(address user)
        external
        view
        returns (BlacklistInfo memory)
    {
    ...
    function addToBlacklist(address user) external {
        require(
            block.timestamp < _blacklistEffectiveEndtime,
            "POLR: Force lock time ended"
        );
        _blacklist[user] = BlacklistInfo(
            true,
            block.timestamp,
            balanceOf(user)
        );
    ...
    function _getUnlockedBalance(address user)
        internal
        view
        returns (uint256 unlockedBalance)
    {
        ...
        if (info.locked && daysPassed < BLACKLIST_LOCK_DAYS) {
            unlockedBalance =
                (daysPassed * info.initLockedBalance) /
                (BLACKLIST_LOCK_DAYS);
        } else {
    ...
    function addVestingToken(
        ...
        require(beneficiary != address(0), "POLR: Zero address");
        require(!_vestingList[beneficiary].isActive, "POLR: Invalid vesting");
    ...
    function revokeVestingToken(address user) external onlyOwner {
        require(_vestingList[user].isActive, "POLR: Invalid beneficiary");
        uint256 claimableAmount = _getVestingClaimableAmount(user);
        require(
            (totalSupply() + claimableAmount) <= TOTAL_SUPPLY,
            "POLR: Max supply exceeded"
        );
        ...
        returns (uint256 claimableAmount)
    {
        ...
        uint256 roundsPassed = ((block.timestamp - releaseTime)) /
            (1 days) /
            (info.daysPerRound);
        ...
        if (roundsPassed >= info.releaseTotalRounds) {
            ...
            releasedAmount =
                (info.amount - roundsPassed) /
                (info.releaseTotalRounds);
        }
        claimableAmount = 0;
    ...
    function claimVestingToken() external nonReentrant returns (uint256) {
        require(
            ...
            "POLR: Not in vesting list"
        );
        uint256 claimableAmount = _getVestingClaimableAmount(_msgSender());
        require(claimableAmount > 0, "POLR: Nothing to claim");
        require(
            (totalSupply() + claimableAmount) <= TOTAL_SUPPLY,
            "POLR: Max supply exceeded"
        );
    ...
    function _beforeTokenTransfer(
        ...
        if (_blacklist[from].locked) {
            ...
            require(
                ...
                "POLR BLACKLIST: Cannot transfer locked balance"
            );
        }
    }

    function withdrawERC20(address token, uint256 amount) external onlyOwner {
        require(amount > 0, "POLR: Amount must be greater than 0");
        require(
            IERC20(token).balanceOf(address(this)) >= amount,
            "POLR: ERC20 not enough balance"
        );
        require(
            IERC20(token).transfer(_msgSender(), amount),
            "POLR: transfer ERC20 failed"
        );
    }
