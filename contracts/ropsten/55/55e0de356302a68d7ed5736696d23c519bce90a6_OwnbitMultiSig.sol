...
contract OwnbitMultiSig {
  ...
  function _validSignature(address erc20Contract, address destination, uint256 value, uint8[] vs, bytes32[] rs, bytes32[] ss) private returns (bool) {
    require(vs.length == rs.length,'vs length not equal to the rs');
    require(rs.length == ss.length, 'rs length not equal to the ss');
    require(vs.length <= owners.length, 'vs length not equal to the owners length');
    require(vs.length >= getRequiredWithoutInactive(), 'vs length  is not bigger than the requireWithoutActive');
    ...
    require(_distinctOwners(addrs), 'the distinct owners error');
