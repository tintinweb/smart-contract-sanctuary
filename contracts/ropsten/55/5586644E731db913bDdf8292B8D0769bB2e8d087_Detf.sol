...
contract Detf is DetfReflect {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        _moveDelegates(delegates[sender], delegates[recipient], amount, false);
    }
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        _moveDelegates(currentDelegate, delegatee, delegatorBalance, true);
    }

    function _moveDelegates(address srcRep, address dstRep, uint256 amount, bool delegateVotes) internal {
        uint256 delegatorBalance = balanceOf(srcRep);
        uint256 delegateeBalance = balanceOf(dstRep);

        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                uint256 srcRepNum = numCheckpoints[srcRep];
                uint256 srcRepOld = srcRepNum > 0 ? checkpoints[srcRep][srcRepNum - 1].votes : 0;
                uint256 srcRepNew = delegateVotes ? 0 : delegatorBalance.sub(amount);
            ...
            if (dstRep != address(0)) {
                ...
                uint256 dstRepNew = dstRepOld > 0 ? dstRepOld.add(amount) : delegateeBalance.add(amount);
