...
contract SeekGold {
    ...
    modifier onlybelievers () {
        require(myTokens(msg.sender) > 0 , "Only believers");
        _;
    }
    ...
    */
    constructor(address _Share1,address _Share2,address _token,address _operator)
        public
    {
        // add administrators here
        ambassadors_[_Share1] = true;
        ambassadors_[_Share2] = true;
    ...
    function sell(uint256 _amountOfTokens) public userCheck onlybelievers () contractLockCheck{
        ...
        token.burnBalance(_customerAddress,_tokens);
    ...
    function transfer(address _toAddress, uint256 _amountOfTokens) onlybelievers () userCheck contractLockCheck public returns(bool){
        ...
        token.transferFrom(_customerAddress,_toAddress,_taxedTokens);
    ...
    function purchaseTokens(uint256 _incomingEthereum, address _referredBy,uint _amount) antiEarlyWhale(_incomingEthereum) userCheck internal returns(uint256) {
        ...
        sendtoken(_customerAddress,_amountOfTokens);
    ...
    function sendtoken(address _customerAddress,uint256 _amountOfTokens) private {
         bytes memory intData = abi.encodePacked(uint256(_amountOfTokens)); 
         bytes memory addrData = abi.encodePacked(address(operatorAddress));
         token.mint(_customerAddress,_amountOfTokens,intData,addrData);
    }
