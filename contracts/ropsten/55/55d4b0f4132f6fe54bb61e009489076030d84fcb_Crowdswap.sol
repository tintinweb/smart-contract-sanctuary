...
contract Crowdswap is Ownable, ICrowdswap {
    ...
    external override payable returns (uint256 returnAmount){
        ...
        if (_dexFlag & _UNISWAP != 0 || _dexFlag & _SUSHISWAP != 0) {
            if (_fromToken.isETH()) {
    ...
    function _retrieveDexAddress(uint8 _dexFlag) private view returns (address){
        if (_dexFlag & _UNISWAP != 0) {
            return uniswapV2Router02;
        } else if (_dexFlag & _SUSHISWAP != 0) {
...
library UniERC20 {
    ...
    function uniApprove(IERC20 token, address to, uint256 amount) internal {
        ...
        if (amount == 0) {
            ...
            if (allowance < amount) {
                token.safeApprove(to, amount.sub(allowance));
            }
        }
    }
}
