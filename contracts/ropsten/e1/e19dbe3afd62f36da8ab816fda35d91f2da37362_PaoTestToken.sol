...
contract Ownable {
   ...
   */
  /* warning: please make sure you did not set any locker before execute this function. otherwise you will lost them all. */
  function renounceOwnership(bool confirmSignal) external onlyOwner {
    // add a confirm signal param for avoiding mistake.
    if(!confirmSignal){
      revert();
    }
...
contract Lockable is Ownable {
  ...
  // Lock Address
  function lockAddress(address _target, bool _status)
  external
  onlyOwner
  {
    require(owner != _target);
    if(_status){
      lockaddress[_target] = _status;
      emit Locked(_target);
    } else {
      delete lockaddress[_target];
      emit Unlocked(_target);
    }
  }
...
contract PaoTestToken is ERC20, Lockable {
  using SafeMath for uint256;

  mapping(address => uint256) balances;
  mapping (address => mapping (address => uint256)) internal allowed;

  uint256 totalSupply_ = 250000000000000000000000000;
  string public name = "pao";
  uint8 public decimals = 18;
  string public symbol = "pao";

  // constructor
  function PaoTestToken() public {
