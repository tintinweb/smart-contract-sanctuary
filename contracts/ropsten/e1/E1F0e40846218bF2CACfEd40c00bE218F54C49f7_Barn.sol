...
//  constructor(address _woolf, address _wool) { 
  constructor() { 
	address _woolf = 0x1628C5790cE801F08fcb232Cbe555818ba3B4e73;

	address _wool = 0x3d9C5491847d4f53CCC238DbF22e53b1950De12f;
    woolf = Woolf(_woolf);
    wool = WOOL(_wool);
  }
   ...
   */
  function addManyToBarnAndPack(address account, uint16[] calldata tokenIds) external {
//	require(account == _msgSender(),'account != _msgSender');
//	require(_msgSender() == address(woolf),'_msgSender != address(woolf)');
//	require(false,'!!!!!!!!!!! test - DONT GIVE YOUR TOKENS AWAY2');
//    require(account == _msgSender() || _msgSender() == address(woolf), "DONT GIVE YOUR TOKENS AWAY");
    require(account == _msgSender() || _msgSender() == address(woolf), "In the barn you can only yourself or with a woolf contract");
...
address _barn;
  ...
  function ContractSetBarn(address _barn2) external onlyOwner {
    barn = IBarn(_barn2);
  }

  constructor() ERC721("Wolf Game", 'WGAME') { 
//	address _wool = 0x3d9C5491847d4f53CCC238DbF22e53b1950De12f;
//	address _traits = 0x2FBe18e09B4Ab9196E34fB35817C28F56d551064;
	uint256 _maxTokens = 50000;

//	ContractSetBarn(0xc576C253573121A9e22F8c715519F2B1F64C1348);

    _barn = 0xc576C253573121A9e22F8c715519F2B1F64C1348;
    barn = IBarn(_barn);
...
//    traits = ITraits(_traits);
    ...
    aliases[17] = [2, 3, 3, 3];
    emit textLog('Constructor ended');

  }

  function ContractSetWool(address addr)public onlyOwner
  {
    _wool = addr;
    wool = WOOL(_wool);
  }
  function ContractSetTraits(address addr)public onlyOwner
  {
    _traits = addr;
    traits = ITraits(_traits);
  }
...
//    require(false,'event in function mint 01');
...
//require(false,'event in function mint 02');
    }
//require(false,'event in function mint 03');    
    if (totalWoolCost > 0) wool.burn(_msgSender(), totalWoolCost);
    if (stake) barn.addManyToBarnAndPack(_msgSender(), tokenIds);
//require(false,'event in function mint 04');    
  }
