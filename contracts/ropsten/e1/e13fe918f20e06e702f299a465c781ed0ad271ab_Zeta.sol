...
contract Zeta is ERC20, ProofOfAgeProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;

    string public name = "Zeta";
    string public symbol = "ZETA";
    ...
    uint256 internal unlockDate = 1825 days; //5 years
    uint256 internal timelockStart;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if((vaultWallet == msg.sender && now.sub(block.timestamp) < unlockDate)) revert();
...
//--------------------------------------------------------------------------------------
    ...
    function setTimelock(uint timestamp) public onlyOwner {
