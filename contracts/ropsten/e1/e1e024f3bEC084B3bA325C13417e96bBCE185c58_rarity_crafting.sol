...
//ropsten-standard:0x672dF7ee4C09F5a596D797A707619DfC4719ae23
contract rarity_crafting is ERC721Enumerable {
    ...
    rarity_attributes constant _attr = rarity_attributes(0xf4537e2B4F2C1E5ee8a1fa57620D2808F716AB00);
    ...
    codex_items_goods constant _goods = codex_items_goods(0x722b70520BB865a7122e6bA52229D75BCd50bc5A); //standard:0x722b70520BB865a7122e6bA52229D75BCd50bc5A
    codex_items_armor constant _armor = codex_items_armor(0x0096f482FE7DDD9fEa64db099A3eF4952fF4C26A);
    codex_items_weapons constant _weapons = codex_items_weapons(0x899f8aC580B1c4cB6F5515B8800338a4a91d2973);
    ...
    function getResult(uint val) external view returns(bool crafted,int check,uint cost,uint dc){
        if(val>0){
            return (_last_crafted,_last_check,_last_cost,_last_dc);
        }
    }
    ...
    function craft_t1(uint _summoner, uint8 _base_type, uint8 _item_type, uint _crafting_materials) external {
        ...
        //require(_summoner != SUMMMONER_ID, "hax0r");
        //require(isValid(_base_type, _item_type), "!valid");
        _last_dc = get_dc(_base_type, _item_type);
        // if (_crafting_materials >= 10) {
        //     require(_craft_i.transferFrom(SUMMMONER_ID, _summoner, SUMMMONER_ID, _crafting_materials), "!craft");
        //     _last_dc = _last_dc - (_crafting_materials / 10);
        // }
        (_last_crafted, _last_check) = craft_skillcheck(_summoner, _last_dc);
        _last_cost = get_item_cost(_base_type, _item_type);
        
        //if (_last_crafted) {
            //require(_gold.transferFrom(SUMMMONER_ID, _summoner, SUMMMONER_ID, _last_cost), "!gold");
            items[next_item] = item(_base_type, _item_type, uint32(block.timestamp), _summoner);
            _safeMint(msg.sender, next_item);
            //emit Crafted(msg.sender, uint(_last_check), _summoner, _base_type, _item_type, _last_cost, _crafting_materials);
            next_item++;
        //}
        //_rm.spend_xp(_summoner, craft_xp_per_day);
    }
    
    function spend_xp_test(uint _summoner)  external {
        _rm.spend_xp(_summoner, 0);
    }

    // function craft(uint _summoner, uint8 _base_type, uint8 _item_type, uint _crafting_materials) external {
    //     craft_t0(_summoner,_base_type,_item_type,_crafting_materials);
    //     craft_t1(_summoner,_base_type,_item_type,_crafting_materials);
    // }
