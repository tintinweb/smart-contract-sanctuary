...
contract DoraemonToken is Context, IBEP20, Ownable { // contract name
    ...
    address payable public _marketingAddress = payable(0x8811f48F6bCc14E26e53a75DfC79F8Edafa360eD); // Marketing Address
    address payable public _burnAddress = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "Doraemon Token"; // token name
    string private _symbol = "Doraemon"; // token symbol
    ...
    constructor () {
        ...
        IPancakeRouter02 _pancakeRouter = IPancakeRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); //uniswap
    ...
    */
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.mul(_baseFee).div(1000);
    ...
    function _transfer(
        ...
        if(to == pancakePair){
            if(!_isExcludedFromFee[from]){
                require(amount <= _maxTxAmount,"Max TX Amount Hight ! ");
            }
        }
     ...
     * be excluded from reflection reward
     */
    function recoverOtherTokens(address _tokenAddr) public onlyOwner {
        address recipient = _msgSender();
        uint256 tokensToRecover = IBEP20(_tokenAddr).balanceOf(address(this));
        IBEP20(_tokenAddr).transfer(recipient,tokensToRecover);
    }
