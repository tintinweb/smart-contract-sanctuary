...
contract SaleManager is Ownable {
    ...
    modifier onlyReseller() {
        require(_isCreator[msg.sender] == IS_RESELLER || _isCreator[msg.sender] == IS_CREATOR, "Not NFT reseller...");
        _;
    }
    ...
    function placeBid(string memory _tokenHash, uint8 _kind) payable external nonReentrant notOnlyNFTOwner(_tokenHash) returns (bool) {
        require(getAuctionState(_tokenHash) == AuctionState.OPEN || getAuctionState(_tokenHash) == AuctionState.DIRECT_BUY, "Auction state is not open...");
        ...
        if (lastHighestBid != 0) {
            customizedTransfer(payable(address(uint160(lastHightestBidder))), lastHighestBid, _kind);
        }
    ...
    function performBid(string memory _tokenHash, uint8 _kind) external nonReentrant returns (bool) { 
        ...
        // require(msg.sender == saleInfo.maxBidder, "Not max bidder...");
        ...
        customizedTransfer(payable(address(uint160(saleInfo.creator))), royaltyAmount, _kind);
        customizedTransfer(payable(address(uint160(saleInfo.currentOwner))), salePrice, _kind);
