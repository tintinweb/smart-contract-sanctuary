...
contract yoruToken {
    ...
    uint public Nights = 1001;
    uint public exchangeRatio;
    uint public feePerc;
    ...
    event FeeChange(address indexed changer, uint256 Percentage);
    ...
    constructor()    {
        ...
        balanceOf[address(this)] = totalSupply;                 // xD
    ...
    function ethContract () public view returns (uint256){
    ...
    function melt(uint256 _amount) public returns (bool success) {
        ...
        uint _ether = _amount/exchangeRatio*feePerc/100;
        require(balanceOf[address(this)] >= _ether);
    ...
    function priceAvailability(uint256 _ratio, bool _canMint) public onlyOwner returns (uint256) {
        require( 0 <= _ratio);
        uint256 mustInContractEth = balanceOf[address(this)]*95/100*_ratio;
        require(address(this).balance >= (address(this).balance-mustInContractEth));
        exchangeRatio = _ratio;
        mintable = _canMint;
        emit PriceChange(_ratio, _canMint);
        return mustInContractEth;
    }
    
    function changeFeePerc(uint256 _fee) public onlyOwner{
        require( 0 <= _fee);
        require( _fee <= 10);
        uint256 mustInContractEth = balanceOf[address(this)] * feePerc /100 /exchangeRatio;
        require (mustInContractEth >= address(this).balance);
        feePerc = _fee;
        emit FeeChange(msg.sender, feePerc);
    }

    function withdrawAdmin(uint256 _amount) public onlyOwner returns (uint256) {
        uint256 mustInContractEth = balanceOf[address(this)]*feePerc/100*exchangeRatio;
        require( (address(this).balance - _amount) >= mustInContractEth );
