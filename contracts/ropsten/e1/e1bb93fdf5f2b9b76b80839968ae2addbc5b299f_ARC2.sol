...
contract CoinEraMintableStandard {
    uint256 public coinEraStartTime;
    ...
    function coinEra() external view returns (uint);
...
contract ARC2 is ERC20, CoinAgeMintableStandard, CoinEraMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "ARC2";
    string public symbol = "ARC2";
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public coinEraStartTime; //CoinEra Minting start time
    
    uint256 public coinEraRate = 1 * (10**decimals);
    
    uint public minInterval = 1 days; //Minimum interval day to minting : 1 day
    uint public maxInterval = 180 days; //Maximum interval day to minting : 180 days
    ...
    function mintingCoinEra() StrongHolderMinter public returns (bool) {
        ...
        if(balances[msg.sender] < onlyStrongHolder) return false;
        if(transferIns[msg.sender].length <= 0) return false;
        
        uint256 coinEraMinting = getCoinEraMinting(msg.sender);
        ...
        //Function to reset CoinEra to zero after receive minting coin
        //and user must hold for certain of time again before minting coin
    ...
    function getCoinEraMinting(address _address) internal view returns (uint) {
        require((now >= coinEraStartTime) && (coinEraStartTime > 0));
        uint _now = now; uint _intervalRate = getCoinEra(_address, _now);
        uint mintingEra = _intervalRate;
        if(_intervalRate <= 0) return 0;
        uint rewardEraRate = coinEraRate;
        if((_now.sub(coinEraStartTime)).div(1 years) == 0) {
            rewardEraRate = coinEraRate.mul(5);
        } else if((_now.sub(coinEraStartTime)).div(1 years) == 1) {
            rewardEraRate = coinEraRate.mul(4);
        } else if((_now.sub(coinEraStartTime)).div(1 years) == 2) {
            rewardEraRate = coinEraRate.mul(3);
        } else if((_now.sub(coinEraStartTime)).div(1 years) == 3) {
            rewardEraRate = coinEraRate.mul(2);
        }
        return rewardEraRate * mintingEra;
    }
    
    function coinEra() external view returns (uint myCoinEra) {
        myCoinEra = getCoinEra(msg.sender,now);
    }

    function getCoinEra(address _address, uint _now) internal view returns (uint _intervalRate) {
        if(transferIns[_address].length <= 0) return 0;
        for (uint i = 0; i < transferIns[_address].length; i++){
            if(_now < uint(transferIns[_address][i].time).add(minInterval)) continue;
            uint intervalDay = _now.sub(uint(transferIns[_address][i].time));
            if(intervalDay > maxInterval) intervalDay = maxInterval;
            _intervalRate = _intervalRate.add(uint(transferIns[_address][i].amount) * intervalDay.div(1 days));
        }
    }
