...
contract ActualGenesis is ERC721, Ownable {
  ...
  mapping(uint256 => bool) public tokenIdToIsArtworkPrivate;
  mapping(uint256 => address) public tokenIdToAddressArtworkIsLentTo;
  ...
  /**
   * @dev Sets the address where funds are withdrawn to.
   *
   * @param newWithdrawalAddress address to withdraw to
   */
  ...
  /**
   * @dev Withdraws funds to `withdrawalAddress`.
   */
   ...
   * used in conjuction with makeArtworkPublic/Private
   *
   * Requirements:
   ...
   */
  function makeArtworkPublic(uint256 tokenID) public {
   ...
   */
  function makeArtworkPrivate(uint256 tokenID) public {
   ...
   */
  function getArtworkPrivateState(uint256 tokenID) public view returns (bool) {
    if (!_exists(tokenID)) revert ArtworkDoesNotExist();
  ...
  /**
   * @dev Computes a signed message based on keccak256 hash of
   * `account` and `tokenID`
   *
   * @param account msg.sender of minting transaction
   * @param newTokenId artwork to be minted
   *
   * @return an Ethereum Signed Message, created from a `hash`
   * corresponding to https://eth.wiki/json-rpc/API#eth_sign
   */
    ...
    returns (bytes32)
  {
    return
      ECDSA.toEthSignedMessageHash(
        keccak256(abi.encodePacked(newTokenId, account))
      );
  }

  /**
   * @dev Compares a signed message based on keccak256 hash of
