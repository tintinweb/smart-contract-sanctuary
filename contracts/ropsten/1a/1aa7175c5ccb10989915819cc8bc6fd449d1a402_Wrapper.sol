...
//    }
    ...
    function getNumberKyberReserves(KyberNetwork network) public view returns (uint numReserves) {
        numReserves = 0;
        for(uint rID = 0; rID < network.getNumReserves(); rID++) {
    ...
    function getKyberReserveAtIndex(KyberNetwork network, uint index) public view returns(KyberReserve kyberReserve) {
        uint counter = 0;
        for(uint rID = 0; rID < network.getNumReserves(); rID++) {
            KyberReserveInterface reserve = network.reserves(rID);
            if (network.reserveType(reserve) != KyberNetwork.ReserveType.PERMISSIONLESS) {
    ...
    function getListKyberReserves(KyberNetwork network) public view returns(KyberReserve[] memory kyberReserves) {
        KyberReserveInterface reserve;
        KyberReserve kReserve;
        uint rID;
        uint resultCount = 0;
    ...
    function getListConversionRates(KyberNetwork network) public view returns(ConversionRatesInterface[] memory conversionRates) {
        KyberReserveInterface reserve;
        KyberReserve kReserve;
        ...
        for(rID = 0; rID < network.getNumReserves(); rID++) {
            ...
            if (network.reserveType(reserve) != KyberNetwork.ReserveType.PERMISSIONLESS) {
                kReserve = KyberReserve(reserve);
                if (kReserve != address(0)) {
                    conversionRate = kReserve.conversionRatesContract();
                    if (conversionRate != address(0)) { resultCount += 1; }
                }
            }
        }
        conversionRates = new ConversionRatesInterface[](resultCount);
        resultCount = 0;
        for(rID = 0; rID < network.getNumReserves(); rID++) {
            reserve = network.reserves(rID);
            if (network.reserveType(reserve) != KyberNetwork.ReserveType.PERMISSIONLESS) {
                ...
                if (kReserve != address(0)) {
                    ...
                    if (conversionRate != address(0)) {
                        conversionRates[resultCount] = conversionRate;
                        resultCount += 1;
                    }
                }
            }
    ...
    function getListTokensAndKyberReserves(KyberNetwork network) public view returns (ERC20[] memory orderReserveTokens, KyberReserve[] memory kyberReserves) {
        ...
        for(rID = 0; rID < network.getNumReserves(); rID++) {
            ...
            if (network.reserveType(reserve) == KyberNetwork.ReserveType.PERMISSIONLESS) {
                ...
                if (oReserve != address(0)) {
                    ...
                    if (token != address(0)) { numOrderReserveTokens += 1; }
                }
            } else {
                kReserve = KyberReserve(reserve);
                if (kReserve != address(0)) { numKyberReserves += 1; }
            }
        }
        orderReserveTokens = new ERC20[](numOrderReserveTokens);
        kyberReserves = new KyberReserve[](numKyberReserves);
        ...
        for(rID = 0; rID < network.getNumReserves(); rID++) {
            ...
            if (network.reserveType(reserve) == KyberNetwork.ReserveType.PERMISSIONLESS) {
                ...
                if (oReserve != address(0)) {
                    (knc, token, fee, addr, median, orderList) = oReserve.contracts();
                    if (token != address(0)) {
                        orderReserveTokens[numOrderReserveTokens] = token;
                ...
                kReserve = KyberReserve(reserve);
                if (kReserve != address(0)) {
                    kyberReserves[numKyberReserves] = kReserve;
                    numKyberReserves += 1;
                }
            }
        }
        return (orderReserveTokens, kyberReserves);
    }
}