...
contract NftFactoryManager {
    ...
    using MomentState for MomentState.PackStore;

    NftFactory public nftFactory;
    CreatorPool public creatorPool;
    
    MomentState.PackStore public packStore;
    ...
    mapping(uint => mapping(uint => bool)) public momentPacked;
    ...
    event PackMinted(uint indexed packID, address indexed creator);
    event PackSold(uint indexed packID, address indexed creator, address indexed to);
    ...
    function publishMoment(uint _momentID) internal returns(uint mintNumber) {
        ...
        momentPacked[_momentID][mintNumber] = true;
   ...
   function createPack (
       ...
       uint _price
        ...
        uint packID = MomentState.getNextPackID(packStore, creator);
    ...
    function transferPack(
        ...
        require(_packID > 0 && _packID <= packStore.nextPackID[_from], "Can only buy a pack that's minted.");
        require(packStore.packs[_from][_packID].forSale, "Can only buy a pack not already bought.");
        require(packStore.packs[_from][_packID].owner == _from, "Cannot sell a pack if you don't own it.");
        require(msg.value == packStore.packs[_from][_packID].price, "Value sent must be at least as great as the pack price.");

        // Update application state
        MomentState.onPackPurchase(
            packStore,
            _packID,
            _from,
            _to
        ...
        // Call NFT Factory
        transferBatchOfMoments(_from, _to, packStore.packs[_from][_packID].momentIDs, packStore.packs[_from][_packID].mintNumbers, "");

        // Call creator pool
        creatorPool.onPackPurchase{value: packStore.packs[_from][_packID].price}(packStore.packs[_from][_packID].price, payable(_from), _to);
    ...
    function transferBatchOfMoments(
        ...
        for(uint i = 0; i < _momentIDs.length; i++) {
            ...
            if(nftFactory.balanceOf(_from, _momentIDs[1]) == 0) {
                EnumerableSet.remove(momentIDsOwned[_from], _momentIDs[i]);
            }
            EnumerableSet.remove(momentMintNumbers[_from][_momentIDs[i]], _mintNumbers[i]);

            // Updating new owners' moment state
            if(nftFactory.balanceOf(_to, _momentIDs[i]) == 0) {
                EnumerableSet.add(momentIDsOwned[_to], _momentIDs[i]);
            }
            EnumerableSet.add(momentMintNumbers[_to][_momentIDs[i]], _mintNumbers[i]);
    ...
    function setMomentSaleStatus(
        ...
        require(!momentStore[_momentID].momentPacked[_mintNumber], "Cannot change the status of moments that are packed.");
        require(_momentID < nextMomentID, "Can only change the status of an existing moment token.");
    ...
    function getMoment(uint _momentID, uint _mintNumber) public view returns (MomentState.Moment memory moment) {
        moment = momentStore[_momentID].mintNumber_to_moment[_mintNumber];
    }
    ...
    function getMintNumbersOwned(address _holder, uint  _momentID) public view returns (uint[] memory mintNumbers) {
        ...
        uint len = EnumerableSet.length(momentMintNumbers[_holder][_momentID]);
        mintNumbers = new uint[](len);

        for(uint i = 0; i < len; i++) {
            mintNumbers[i] = EnumerableSet.at(momentMintNumbers[_holder][_momentID], i);
        }
    }
    ...
    function getPack(uint _packID, address _creator) public view returns(MomentState.Pack memory pack) {
        pack = packStore.packs[_creator][_packID];
    }

    function getMomentIDsInPack(uint _packID, address _creator) public view returns (uint[] memory momentIDs) {
        momentIDs = packStore.packs[_creator][_packID].momentIDs;
    }

    function getMintNumbersInPack(uint _packID, address _creator) public view returns (uint[] memory mintNumbers) {
        mintNumbers = packStore.packs[_creator][_packID].mintNumbers;
    }

    function getNextPackID(address _creator) public view returns (uint nextPackID) {
        nextPackID = packStore.nextPackID[_creator];
    }

    function getTotalPackCreators() public view returns (uint totalCreators) {
        totalCreators = packStore.totalCreators;
    }

    function getIsMomentPacked(uint _momentID, uint _mintNumber) public view returns (bool isPacked) {
        isPacked = momentStore[_momentID].momentPacked[_mintNumber];
    }
}
...
library MomentState {
    ...
    struct MomentStore {
        ...
        mapping(uint => bool) momentPacked;
        uint nextMintNumber;
    }
    ...
    struct PackStore {
        ...
        mapping(address => uint) nextPackID;
        mapping(address => mapping(uint => Pack)) packs;

        mapping(address => bool) isInPackStore;
    }
    ...
    function onPublishMoment(
        ...
        self.momentPacked[mintNum] = true;
    }

    function getNextPackID(
        PackStore storage self,
        address _creator
    ) public  returns (uint nextPackID) {
        if(!self.isInPackStore[_creator]) {
            self.totalCreators += 1;
            self.creators[self.totalCreators] = _creator;

            self.isInPackStore[_creator] = true;
        }

        self.nextPackID[_creator] = self.nextPackID[_creator] + 1;
        nextPackID = self.nextPackID[_creator];
    }
    ...
    function onPackMint(
        ...
        self.packs[_creator][_packID].owner = _creator;
        self.packs[_creator][_packID].id = _packID;
        self.packs[_creator][_packID].momentIDs = _momentIDs;
        self.packs[_creator][_packID].mintNumbers = _mintNumbers;
        self.packs[_creator][_packID].price = _price;
        self.packs[_creator][_packID].forSale = true;
    ...
    function onPackPurchase(
        ...
        self.packs[_from][_packID].owner = _to;
        self.packs[_from][_packID].forSale = false;
    }
    ...
    function onSingleMomentTransfer(
        ...
        self.momentPacked[_mintNumber] = false;
    }
