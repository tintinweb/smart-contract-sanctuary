...
contract Vesting is Ownable, IVesting {
    ...
    constructor(
        ...
        _initialized = false;
        referenceDate = referenceDate_;
        emit ReferenceDateSet(referenceDate_);
        _transferOwnership(owner);
    }

    // need to be changed for the production implementation !!
    // need to add a check on initialized
    ...
    function updateTreasuryWallet(address newTreasury) external override onlyOwner {
        ...
        require(newTreasury != treasury(), "address already set");
    ...
    function updateTeamWallet(address newTeam) external override onlyOwner {
        ...
        require(newTeam != team(), "address already set");
    ...
    function updateEcosystemFundWallet(address newEcosystemFund) external override onlyOwner {
        ...
        require(newEcosystemFund != ecosystemFund(), "address already set");
    ...
    function updateLongTermLockUpWallet(address newLongTermLockUp) external override onlyOwner {
        ...
        require(newLongTermLockUp != longTermLockUp(), "address already set");
    ...
    function deposit(
        ...
        require(
            ...
            _to == longTermLockUp(), 'invalid address parameter');
        require(_amount > 0, 'Value must be positive');
        require(_vestingData[_to].totalVestedAmount != 0, 'this is initial deposit, use the correct function');
        // transfer tokens on the vesting contract
        _token.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function _getAvailableBalance(address _holder) internal view returns (uint256) {
        ...
        // Divide with 10^4 resolution
        uint256 tokensPerTick = (_vestingData[_holder].totalVestedAmount * (10000 / _vestingData[_holder].vesting)) / 10000;
