...
contract UniswapExchangeAdapter {
    ...
    function _getPair(address from, address to) internal returns(IUniswapV2Pair) {
        address pair = factory.getPair(from, to);
        require (pair != address(0), "ExchangeAdapter: pair is missing");
        return IUniswapV2Pair(pair);
    }

    function exchangeTokens(Erc20Asset from, uint256 fromAmount, Erc20Asset to, uint256 toAmount) internal {
        ...
        path[0] = fromAddr;
        path[1] = toAddr;
        bytes memory call = abi.encodeWithSelector(sig, toAmount, fromAmount, path, address(to), block.timestamp);

        from.execute(address(router), call, fromAmount);
    }

    function exchangeTokenForNative(Erc20Asset from, uint256 fromAmount, NativeAsset to, uint256 toAmount) internal {
        address fromAddr = address(from.assetAddress());
    ...
    function exchangeNativeForToken(NativeAsset from, uint256 fromAmount, Erc20Asset to, uint256 toAmount) internal {
        address toAddr = address(to.assetAddress());
        ...
        path[1] = toAddr;
        bytes memory call = abi.encodeWithSelector(sig, toAmount, path, address(to), block.timestamp);
