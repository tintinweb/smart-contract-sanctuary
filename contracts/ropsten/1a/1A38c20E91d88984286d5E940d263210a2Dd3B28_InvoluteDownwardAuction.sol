...
// Events
event AuctionStarted(bytes16 auctionId, bytes32 auctionChecksum, uint256 endTime, uint256 minDecrease);
event LowestBidDecreased(bytes16 auctionId, address bidder, uint256 lowestBid);
event AuctionEnded(bytes16 auctionId, address winner, uint256 lowestBid);
...
uint256 _minDecrease) internal returns(bool) {
...
/// @dev All information about the bid is already in the message (value and sender).
// The only argument is therefore the uint256 auctionId.
...
stableCoinContract.transferFrom(msg.sender, address(this), _amount);
...
stableCoinContract.increaseAllowance(msg.sender, amount);
...
// 3. Interaction
stableCoinContract.increaseAllowance(beneficiary, a.lowestBid);
...
bytes32 _auctionChecksum,
uint256 _biddingTime,
uint256 _minDecrease) onlyOwner external returns(bool) {

return _createAuction(_auctionId, _auctionChecksum, _biddingTime, _minDecrease);
...
function withdraw() onlyAllowed external returns(bool){
return _withdraw();
}
