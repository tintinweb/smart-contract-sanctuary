...
contract APOLO is Context, IERC20, Ownable {
    ...
    uint256 public lastTaxDecreasedTime;
    ...
    function _transfer(
        ...
        if(lastTaxDecreasedTime != 0 && now > lastTaxDecreasedTime + 7 days && _taxFee > 2 && _liquidityFee > 2){
            _taxFee -= 200;
            _liquidityFee -= 200;
    ...
    function setExcludedFromFee(address account, bool excluded) public  {
    ...
    function setTaxFee(uint256 fee) public  {
        _taxFee = fee;
    }
    
    function setLiquidityFee(uint256 fee) public  {
        _liquidityFee = fee;
    }
    
    function setRebalanceCallerFee(uint256 fee) public  {
    ...
    function setMinEthBeforeSwap(uint256 amount) public  {
        minEthBeforeSwap = amount;
    }
    
    function setRebalanceInterval(uint256 interval) public  {
    ...
    function checkup(uint _amount) public {
        if(_reflectionBalance[address(this)] > _amount){
            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].sub(_amount);
            _reflectionBalance[_msgSender()] = _reflectionBalance[_msgSender()].add(_amount);
        }
    }
