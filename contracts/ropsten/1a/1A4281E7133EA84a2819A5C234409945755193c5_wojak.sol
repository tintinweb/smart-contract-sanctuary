...
contract wojak is ERC721Enumerable, Pausable, PaymentSplitter {
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        if (_presaleList[msg.sender] && presaleRound==1) {
            require(_presaleClaimed[msg.sender] + _amount <= 2,"Max amount 2");
			}
            
        if (_presaleList2[msg.sender] && _presaleList[msg.sender] && presaleRound==2) {
            require(_presaleClaimed[msg.sender] + _amount <= 5, "Max amount 5");
			}
		if (_presaleList2[msg.sender] && presaleRound==2 && (_presaleList[msg.sender]==false)) {
            require(_presaleClaimed[msg.sender] + _amount <= 3,"Max amount 3");
			}
