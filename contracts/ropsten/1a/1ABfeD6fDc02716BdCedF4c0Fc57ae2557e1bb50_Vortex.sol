...
contract Vortex is ERC721Enumerable, Ownable {
    ...
    /**
     * @dev Returns number of allowed presale mints for caller wallet
     */
    function presaleNum() public view returns (uint8) {
        return presaleNumPerAddress[msg.sender];
    }
    ...
    function presale(uint8 _num) external payable {
        ...
        _mintTokens(_num);
        presaleNumPerAddress[msg.sender] =
            presaleNumPerAddress[msg.sender] -
            _num;
    }
    ...
    function _mintTokens(uint8 _num) private {
        for (uint8 i = 0; i < _num; i++) {
            ...
            if (newTokenId % 100 == 0) {
                ...
                payable(luckyAddress).transfer(8e16);
