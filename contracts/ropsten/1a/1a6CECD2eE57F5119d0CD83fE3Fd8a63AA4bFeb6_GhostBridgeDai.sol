...
contract GhostBridge is AccessControl {
     ...
     * @params token IERC20 based token, GHOST token
     */
    ...
    function mintTx(uint32 _blockNumber, uint32 _transactionIndex, address _to, uint256 _amount) public transactionNotExists(_blockNumber, _transactionIndex) returns (bool) {
        ...
        require(_token.transfer(_to, _amount), "GhostBridge: token transfer failed");
	 ...
	 */
    function rescueFunds(IERC20 _rescueToken, address payable _to, uint256 _amount) external returns (bool) {
