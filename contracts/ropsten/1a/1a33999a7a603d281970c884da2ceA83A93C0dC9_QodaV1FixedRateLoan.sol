...
contract QodaV1FixedRateLoan is IQodaV1FixedRateLoan, ErrorCode {
  ...
  mapping(bytes => FixedRateLoan) outstandingLoans;
  ...
  /// @notice Mapping of whether a nonce has already been used for a given user. Returns true if used and false otherwise
  mapping(address => mapping(uint => bool)) noncesUsed;
  ...
  constructor(address _principalTokenAddress) public {
  ...
  function repayBorrow(bytes memory signature, uint amount) external returns(uint){
    FixedRateLoan storage loan = outstandingLoans[signature];
    
    // Read the current unpaid amount
    uint unpaid = loan.principalPlusInterest - loan.amountRepaid;
  ...
  function getOutstandingLoan(bytes memory signature) external view returns(
                                                                            uint,
                                                                            uint,
                                                                            uint,
                                                                            uint,
    ...
    FixedRateLoan memory fixedRateLoan = outstandingLoans[signature];
    return (
            fixedRateLoan.startBlock,
            fixedRateLoan.endBlock,
            fixedRateLoan.principal,
            fixedRateLoan.principalPlusInterest,
            fixedRateLoan.amountRepaid,
            fixedRateLoan.lender,
            fixedRateLoan.borrower
            );
  }
  ...
  function _createFixedRateLoan(
    ...
    // Adding `fixedRateLoan` to storage and pointers to `fixedRateLoan` for `accountBorrows` and `accountLends`
    outstandingLoans[signature] = fixedRateLoan;
