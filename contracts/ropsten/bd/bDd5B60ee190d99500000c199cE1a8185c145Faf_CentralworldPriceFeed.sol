...
interface ICentralworldAddressRegistry {
    function tokenRegistry() external view returns (address);
}

interface ICentralworldTokenRegistry {
...
contract CentralworldPriceFeed is Ownable {
    /// @notice keeps track of oracles for each tokens
    mapping(address => address) public oracles;

    /// @notice Centralworld address registry contract
    ...
    function registerOracle(address _token, address _oracle)
        ...
        onlyOwner
    {
        ICentralworldTokenRegistry tokenRegistry = ICentralworldTokenRegistry(
            ICentralworldAddressRegistry(addressRegistry).tokenRegistry()
        );
