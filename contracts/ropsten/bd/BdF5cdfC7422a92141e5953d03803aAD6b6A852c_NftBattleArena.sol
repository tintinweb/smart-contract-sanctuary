...
contract NftBattleArena is Ownable
{
	...
	function getNftsInEpoch(uint256 epoch, uint256 i) public view returns (NftRecord memory)
	{
	...
	function truncateAndPair() public returns (bool success)
	{
		...
		while (nftsInEpoch[currentEpoch].length != 0)                                     // Get pairs of nft until where are zero left in list.
		{
			...
			pairsInEpoch[currentEpoch].push(NftPair(token1, id1, token2, id2, false, false));  // Push pair.
	...
	{	
		...
		numberOfPlayedPairsInEpoch[currentEpoch]++;
		pairsInEpoch[currentEpoch][i].playedInEpoch = true;
