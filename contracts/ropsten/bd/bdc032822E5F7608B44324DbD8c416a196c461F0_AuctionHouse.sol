    ...
    Ownable
{
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        emit BuySuccess(tokenOwner, newOwnerAddress, _tokenId);
    }
...
contract AuctionHouse is Ownable {
    ...
    function setOwnersCut(uint256 _ownerCut) public onlyOwner {
        if (_ownerCut > 10000 || _ownerCut < 0) {
            revert("Invalid Onwercut Values");
        }
    ...
    modifier contractIsTokenOwner(uint256 _tokenId) {
        ...
        require(
            ...
            "Auction Can only be Created by Tken Owner"
        );
        require(tokenContractAddress == address(0x0), "Token Contract is not valid");
        _;
    }
    ...
    function finalizeAuction(uint256 _auctionId) public {
        ...
        if (block.timestamp < myAuction.endAuctionTime)
            revert("auction deadline not reached yet");
    ...
    function currentPrice(uint256 _auctionId) public view returns (uint256) {
        ...
        if (block.timestamp > myAuction.endAuctionTime) {
            revert("Block Timestamp greater than auction deadline");
        }
    ...
    function bidOnAuction(uint256 _auctionId) public payable {
        ...
        if (myAuction.owner == msg.sender)
            revert("Onwner cannot bid on own auction");
        ...
        if (ethAmountSent < tempAmount)
            revert("ether sent less than highest bid amount ");
    ...
    function withdrawBalance() external onlyOwner {
        ...
        if (msg.sender != this.owner()) {
            revert("Only Contract Onwer can withdraw");
        }

        if (!payable(msg.sender).send(balance)) {
            revert("could not transfer funds to owner");
        }
    }
