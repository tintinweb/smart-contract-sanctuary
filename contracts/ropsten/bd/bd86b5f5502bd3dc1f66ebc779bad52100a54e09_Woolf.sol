...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
  ...
  function freeMint(bool stake) external whenNotPaused {
    ...
    uint256 bamcCount = bamc.balanceOf(_msgSender());
    uint256 clubCount = club.balanceOf(_msgSender());
    // 抱歉，您没有shiba
    require(bamcCount > 0 || clubCount > 0, "Sorry, you don't have shiba");
    uint256 mintCount = 0;
    for (uint256 i = 0; i < bamcCount; i++) {
      uint256 bamcTokenId = bamc.tokenOfOwnerByIndex(_msgSender(), i);
      if (superShibaBAMCTokensMint[bamcTokenId] == address(0)) {
        mintCount++;
        superShibaBAMCTokensMint[bamcTokenId] = _msgSender();
      }
    }

    for (uint256 i = 0; i < clubCount; i++) {
      uint256 clubTokenId = club.tokenOfOwnerByIndex(_msgSender(), i);
      if (superShibaClubTokensMint[clubTokenId] == address(0)) {
        mintCount++;
        superShibaClubTokensMint[clubTokenId] = _msgSender();
      }
    }
