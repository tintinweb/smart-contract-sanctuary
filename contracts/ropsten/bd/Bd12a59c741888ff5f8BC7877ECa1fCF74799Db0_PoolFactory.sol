...
contract PoolFactory is Ownable {
    ...
    function deployPool(
        ...
        address _admin
    ) external onlyOwner returns (address smartChefAddress) {
        require(_stakedToken.totalSupply() >= 0);
        require(_rewardToken.totalSupply() >= 0);
        // require(_stakedToken != _rewardToken, "Tokens must be be different");

        bytes memory bytecode = type(TangleSmartChef).creationCode;
        ...
        TangleSmartChef(smartChefAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
...
contract TangleSmartChef is Ownable, ReentrancyGuard {
