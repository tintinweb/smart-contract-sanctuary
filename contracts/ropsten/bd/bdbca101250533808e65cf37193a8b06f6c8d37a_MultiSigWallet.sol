...
contract MultiSigWallet {
    ...
    uint256 constant internal ERR_CODE_TRANSFER_EXPIRED = 11;
        ...
        hasApproved(_transactionId, msg.sender)
    {
        ...
        if (isTransferConfirmed(_transactionId)) 
        {
            ...
            if (txn.destination.call.value(txn.value)(txn.data))
                emit EventExecution(msg.sender, _transactionId, now, ERR_CODE_SUCC);
            else 
            {
                emit EventExecution(msg.sender, _transactionId, 
        ...
        returns (uint256 transactionId)
    {
        ...
        transactionCount += 1;
        emit EventSubmission(msg.sender, transactionId, now);
    }
