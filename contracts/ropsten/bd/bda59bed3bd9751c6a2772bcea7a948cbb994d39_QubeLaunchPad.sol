...
contract SignerManager is Ownable  {
    ...
    // keccak256("signer_manager.signer.address")
    bytes32 internal constant SIGNER_STORAGE_SLOT = 0x4a204f620c8c5ccdca3fd54d003badd85ba500436a431f0cbda4f558c93c34c8;
...
contract QubeLaunchPad is Ownable,Pausable,SignerManager{
    ...
    function signDecodeParams(bytes memory params) public pure returns (singParams memory res) {
        return abi.decode(params,(singParams));
    }
    ...
    function ico(uint256 id,uint256 amount,bytes memory signStore) public payable {
        ...
        while(vars.endTime[vars.currentTier] < block.timestamp && !vars.tierStatus){
            if(vars.currentTier != vars.startTime.length) {
                ...
                if(vars.currentTier == vars.normalSaleStartTier + 1) vars.tierStatus = true;
            }
            
            if(!vars.signOff && vars.normalSaleStartTier == vars.currentTier) {
                vars.signOff = true;
            }
        }
        ...
        if(address(vars.quoteToken) == address(0)){
           ...
           uint256 getAmountOut = getAmountIn.mul(getPrice(vars.salePrice[vars.currentTier],vars.quotePrice[vars.currentTier],18)).div(1e18);
           ...
           uint256 getAmountOut = amount.mul(getPrice(vars.salePrice[vars.currentTier],vars.quotePrice[vars.currentTier],decimal)).div(10 ** decimal);
