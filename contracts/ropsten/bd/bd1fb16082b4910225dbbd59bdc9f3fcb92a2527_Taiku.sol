...
- swapExactTokensForTokensSupportingFeeOnTransferTokens for generic trades ??
...
contract Taiku is Ownable {
	...
	/*function getRequestInfo(uint id) public view returns (uint, address, address, uint, string memory, string memory, address[] memory, bool) {
		return (allRequests[id].id, allRequests[id].applicant, allRequests[id].tokenToBuy, allRequests[id].buyAmount, allRequests[id].routerName, allRequests[id].message, allRequests[id].approvals, allRequests[id].approved);
	}*/
	...
	*/
	...
	function isWhitelisted(address userAddress) public view returns (int) {
	...
	function addNewRouter(string memory routerName, address routerAddress) public onlyOwner {
	...
	function swapRouter(uint buyAmount, address tokenToBuy, string memory routerName) private {
		...
		for (uint i = 0; i < getNbWhitelist(); i++)
		{
			if (getAddressBalance(getWhitelistAddress(i)) >= buyAmount) {
				// higher gas cost but no need to check if fees exist or no
				getRouter(routerName).swapExactTokensForTokensSupportingFeeOnTransferTokens(buyAmount, 0, path, getWhitelistAddress(i), block.timestamp + 300);
