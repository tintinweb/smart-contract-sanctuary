    ...
    // XXXX DO NOT ADD FURHTER BASES WITH STATE VARS HERE XXXX
{
        ...
        nonReentrant
    {
        ...
        if (_adminBalanceToken1 > 0) {
            ...
            token0.safeTransfer(_treasury, amountToSend1);
        }
    }

    function autoWithdrawAdminBalance(
        address feeToken,
        uint256 feeAmount
    ) external gelatofy(feeAmount, feeToken) {
        uint256 amount0;
        uint256 amount1;
        if (feeToken == address(token0)) {
            require(
              _adminBalanceToken0 * _maxWithdrawFeeBPS / 10000 >= feeAmount,
            ...
            require(
              _adminBalanceToken1 * _maxWithdrawFeeBPS / 10000 >= feeAmount,
              "only withdraw after enough fees accrued"
            );
        ...
        )
    {
        ...
        if (amount0Current == 0 && amount1Current > 0) {
            ...
            uint256 amount1Mint =
                amount1Current > 0
                    ...
                    : 0;
            if (amount0Mint > 0 && amount0Mint == amount1Mint) {
                return (amount0Max, amount1Max, amount0Mint);
            }
            useAmount0Max = amount0Mint < amount1Mint;
            mintAmount = useAmount0Max ? amount0Mint : amount1Mint;
        }
        ...
        if (useAmount0Max) {
            ...
            amount1 = amount1RU > amount1Max ? amount1Max : amount1RU;
        } else {
            amount1 = amount1Max;
            uint256 amount0RU =
                FullMath.mulDivRoundingUp(
            ...
            amount0 = amount0RU > amount0Max ? amount0Max : amount0RU;
        }
    }
    ...
    function _adjustCurrentPool(
        ...
        uint256 reinvest1;
        {
            ...
            reinvest0 = _paymentToken == address(token0)
                ? token0.balanceOf(address(this)) - _adminBalanceToken0 - _feeAmount
                : token0.balanceOf(address(this)) - _adminBalanceToken0;

            reinvest1 = _paymentToken == address(token1)
                ? token1.balanceOf(address(this)) - _adminBalanceToken1 - _feeAmount
                : token1.balanceOf(address(this)) - _adminBalanceToken1;
        }
    ...
    function _withdraw(
        ...
        _adminBalanceToken0 += (amountFeesAccrued0 * _adminFeeBPS) / 10000;
        _adminBalanceToken1 += (amountFeesAccrued1 * _adminFeeBPS) / 10000;
    }
    ...
    // XXXX DONT MODIFY ORDERING XXXX
{
    ...
    function updateTreasury(address account) external onlyOwner {
        emit UpdateTreasury(_treasury, account);
        _treasury = account;
    }
