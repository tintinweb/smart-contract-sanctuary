...
contract Crowdsale is Ownable { 
  ...
  uint256 public tokensForPublicSale = 14850000000000000000000000;
  
  uint256 public preSaleTokenSold; 
  uint256 public publicSaleTokenSold = 0;

  //Sale minimum maximum values
  uint256 public minimumTokensInPreSale = 500000000000000000000;
  uint256 public MaximumTokensInPreSale = 10000000000000000000000000;
  uint256 public minimumTokensInPublicSale = 500000000000000000000;
  uint256 public MaximumTokensInPublicSale = 14850000000000000000000000;

   //tokens per ETH in each sale 
  uint256 public preSaleTokensPerETH = 3200;
  uint256 public publicSaleTokensPerETH = 2700;
...
mapping (uint256 => tokenInfo) public tokenBook;
  ...
  address public tokenContractAddress = address(0x25D0B32277ba1e80493bE4D258e407DA2D65051c);
  ...
  bool public Paused;
    ...
    */
    ...
    require(currentStage == Stages.PreSaleStart, "Pre sale not started");
    ...
    require(currentStage == Stages.PublicSaleStart, "Public sale not started");
   ...
   */
   function buyTokens(address beneficiary) CrowdsaleStarted public payable {
    ...
    publicSaleTokenSold = publicSaleTokenSold + tokens;   
    }
   ...
   */
   function _preValidatePurchase(uint256 _ETH) internal view { 
     ...
     if (currentStage == Stages.PreSaleStart) {
        require(_ETH >= minimumTokensInPreSale);
        require(_ETH <= MaximumTokensInPreSale);

      }
      else if (currentStage == Stages.PublicSaleStart) {
        require(_ETH >= minimumTokensInPublicSale);
        require(_ETH <= MaximumTokensInPublicSale);
