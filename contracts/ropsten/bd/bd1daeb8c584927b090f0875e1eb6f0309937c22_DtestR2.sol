...
contract DtestR2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private charityWallet = payable(0xa312C1FbF5556D245647567Ba72157216894A532); // charity  Wallet
    address payable private ecosystemWallet = payable(0x5Df410078F25B1281D0C44109756e507d06580d2); // Ecosystem Wallet
    address payable private marketingWallet = payable (0x5aFE0827bbD00f34D5dD688F57023BFe36B1DaC3); // marketing Wallet
    ...
    string private _name = "DtestR2";
    string private _symbol = "DtR2";
    ...
    uint256 private totalFee =
        liquidityFee.add(charityFee).add(marketingFee).add(ecosystemFee);
    uint256 private currenttotalFee = totalFee;
    
    uint256 public swapThreshold = _tTotal.div(10000).mul(200); //2%
    ...
    function _transfer(
        ...
        if(tradingOpen && to == uniswapV2Pair) { //sell
            currenttotalFee= _sellLiquidityFee.add(_sellcharityFee).add(_sellmarketingFee);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Send to charity wallet and marketing wallet
    ...
    function setWallets(address _charityWallet,address _marketingWallet) external onlyOwner() {
    ...
    function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
     ...
     function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _charityFee,uint256 _marketingFee) external onlyOwner {
        ...
        totalFee = liquidityFee.add(charityFee).add(marketingFee).add(ecosystemFee);

    }

    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _charityFee,uint256 _marketingFee) external onlyOwner {
