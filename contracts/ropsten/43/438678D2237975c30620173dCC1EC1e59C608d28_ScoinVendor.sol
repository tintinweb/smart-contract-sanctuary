...
contract Scoin is ERC20, ERC20Burnable, Ownable {
    // Assumes we'll be splitting up the full batch into 2 addresses, pass them in here.
    constructor(address address1, address saleContactAddress) ERC20("Scoin", "SPLAY") {
      // We're starting with a max quantity of one million.
      // To start that will be split out to various parties,
      // most going to the sales contractor for later sales.

      // 500000 (half) to the bosses wallet:
      _mint(address1, 500_000 * 10 ** decimals());

      // The other half goes to the sales contract.
      _mint(saleContactAddress, 500_000 * 10 ** decimals());
    }

    // Leave a way for us to create more if necessary.
