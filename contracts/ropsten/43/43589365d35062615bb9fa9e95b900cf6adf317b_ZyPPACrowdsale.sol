...
contract usingZyPPAConsts {
    ...
    address constant TEAM_ADDRESS = 0xBA1d3447400103a408075E197EfCb1CD0fF93094;
    address constant BOUNTY_ADDRESS = 0x236B914A56d7D91881D3DA78226163a5a807F789;
...
contract ZyPPAToken is usingZyPPAConsts, MintableToken, BurnableToken {
    /**
...
contract ZyPPARateProvider is usingZyPPAConsts, ZyPPARateProviderI, Ownable {
...
contract ZyPPACrowdsale is usingZyPPAConsts, FinalizableCrowdsale {
    ZyPPARateProviderI public rateProvider;

    function ZyPPACrowdsale(
            uint _startTime,
            uint _endTime,
            uint _hardCapTokens
    ) public
        ...
        ZyPPAToken(token).addExcluded(TEAM_ADDRESS);
        ZyPPAToken(token).addExcluded(BOUNTY_ADDRESS);
        ZyPPAToken(token).addExcluded(PREICO_ADDRESS);

        ZyPPARateProvider provider = new ZyPPARateProvider();
    ...
    function setRateProvider(address _rateProviderAddress) onlyOwner {
        ...
        rateProvider = ZyPPARateProviderI(_rateProviderAddress);
    }
    ...
    function addExcluded(address _address) onlyOwner notFinalized {
        ZyPPAToken(token).addExcluded(_address);
    }
    ...
    function finalization() internal {
        super.finalization();
        token.finishMinting();
        ZyPPAToken(token).crowdsaleFinished();
