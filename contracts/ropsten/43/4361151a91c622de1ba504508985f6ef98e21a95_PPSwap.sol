...
// ----------------------------------------------------------------------------
contract SafeMath {
    function safeAdd(uint56 a, uint56 b) public pure returns (uint56 c) {
        c = a + b;
        require(c >= a);
    }
    function safeSub(uint56 a, uint56 b) public pure returns (uint56 c) {
        require(b <= a); 
        c = a - b; 
    } 
        
    function safeMul(uint56 a, uint56 b) public pure returns (uint56 c) { 
        c = a * b; 
        require(a == 0 || c / a == b); 
    } 
        
    function safeDiv(uint56 a, uint56 b) public pure returns (uint56 c) { 
        require(b > 0);
        c = a / b;
    }
}
...
contract PPSwap is ERC20Interface, SafeMath {
    ...
    mapping(address => uint56) balances;       // two column table: owneraddress, balance
    mapping(address => mapping(address => uint56)) allowed; // three column table: owneraddress, spenderaddress, allowance
     ...
     */
    constructor() public {
        balances[msg.sender] = uint56(_totalSupply);
    ...
    // called by the owner
    function approve(address spender, uint rawAmt) public returns (bool success) {
        require(rawAmt < 2**56, "Approve amount is too big.");
        allowed[msg.sender][spender] = uint56(rawAmt);
        emit Approval(msg.sender, spender, uint56(rawAmt));
        return true;
    }

    function transfer(address to, uint rawAmt) public returns (bool success) {
        require(rawAmt < 2**56, "Transfer amount is too big.");
        balances[msg.sender] = safeSub(balances[msg.sender], uint56(rawAmt));
    ...
    function transferFrom(address from, address to, uint rawAmt) public returns (bool success) {
        require(rawAmt < 2**56, "Transfer amount is too big.");
        balances[from] = safeSub(balances[from], uint56(rawAmt));
        allowed[from][msg.sender] = safeSub(allowed[from][msg.sender], uint56(rawAmt));
        balances[to] = safeAdd(balances[to], uint56(rawAmt));
