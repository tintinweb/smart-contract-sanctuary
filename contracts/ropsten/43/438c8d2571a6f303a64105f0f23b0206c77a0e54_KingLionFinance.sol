...
contract KingLionFinance is Context, IERC20, Ownable {
    ...
    function setTradeEnabled(bool value) external onlyOwner() {
        _tradeEnabled = value;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam) = _getTValues(tAmount, _distributionPercentage, _teamFee);
        uint256 currentRate =  _getRate();
    ...
    function _reflectFee(uint256 rFee, uint256 tFee, uint256 feeAmount) private {
        _rTotal = _rTotal.sub(rFee).sub(feeAmount.mul(_getRate()));
        _tFeeTotal = _tFeeTotal.add(tFee).add(feeAmount);
    }

    function _takeTeam(uint256 tTeam) private {
        uint256 currentRate =  _getRate();
        uint256 rTeam = tTeam.mul(currentRate);
        _rOwned[_feeAddress] = _rOwned[_feeAddress].add(rTeam);
        if(_isExcluded[_feeAddress])
            _tOwned[_feeAddress] = _tOwned[_feeAddress].add(tTeam);
    }
