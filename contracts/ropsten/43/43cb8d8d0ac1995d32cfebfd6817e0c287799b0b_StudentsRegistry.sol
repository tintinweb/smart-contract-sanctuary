...
// File: contracts/StudentsRegistry.sol
...
contract StudentsRegistry is IStudentsRegistry{
    ...
    function changeRegisterTime(uint256 _maxRegisterTime) public onlyOwner{ // this function can only be called by the owner. 
        maxRegisterTime = _maxRegisterTime;
    }
    ...
    // view means this function does not change any status and also does not spend any gas.
    // we add override because we put it into interface, so since it cannot be in two places, we simiply add override here.
