...
contract DogeV3 is Ownable, DogeV3Interface, IERC20 {
  ...
  modifier onlyLegends() {
    require(
      msg.sender == addresses[LEGENDS.VITALIK] ||
        msg.sender == addresses[LEGENDS.CZ] ||
        msg.sender == addresses[LEGENDS.SAM] ||
        msg.sender == addresses[LEGENDS.UNISWAP_DEPLOYER],
      "ERROR: You are not legend"
    );
    _;
  }
    ...
    onlyOwner
  {
    ...
    IERC20(address(this)).approve(address(LiquidityManager), uint256(-1));
  ...
  function incrementStage() external override {
    ...
    (uint256 tokenId, uint256 tokenId2) = _mint(upperTick, lowerTick, tick);
    ...
    )
  {
    ...
    if (isReversed) {
      lowerPrice = uint256(10**16).div(
        calcs.getPriceE8FromTick(stage[stageID].upperTick)
      );
      upperPrice = uint256(10**16).div(
        calcs.getPriceE8FromTick(stage[stageID].lowerTick)
      );
    } else {
      upperPrice = calcs.getPriceE8FromTick(stage[stageID].upperTick);
      lowerPrice = calcs.getPriceE8FromTick(stage[stageID].lowerTick);
    }
    return (
      isReversed,
      ...
      upperPrice,
      lowerPrice,
      calcs.getPriceE8FromSQRTPrice(price)
    );
  }
