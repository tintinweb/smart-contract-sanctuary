...
contract RealPrimary {
   ...
   function setReal(address payable _owner_1, uint256  _value_1, uint _id, bytes16 _md5_hash_picture, uint _certificate) public { // register a new real estate.
        ...
        for (uint i = 0; i < newReals.length; i++) {// the propetry could be added twice or more, so this algorithm will check several aspects as below.
            ...
            if((newReals[i].getTheCertificate() == _certificate || newReals[i].getId() == _id) && (newReals[i].getValid() == true)){ // if the certificate or the id of the property has been added by the blockchain database, and this property is valid, then the algorithm will check additional aspects as below. 
            ...
            if (newReals[i].getAvailable() == true){ // if the property satisfies the above conditions, then the algorithm will check if the property has been sold or not. 
                    ...
                    require(newReals[i].getOwner() == _owner_1 && newReals[i].getValue() != _value_1,'This is offer has already been added'); // if the property has not been sold before, and the actual owner wants to change the price, then the smart contract set the previous offer as unvalid offer. After the aforementioned point, a new offer will be created by the parameters that have been sent by the owner.
    ...
    function buy(uint _id) payable public  { 
        ...
        require(newReals[realListFromAddressToArrayId[realListFromIdToaddress[_id]]].getAvailable() == true, 'The property is sold, or it is not available'); // second condition, the real estate must be available.
   ...
   function cancelOffer(uint _id) public { // cancel this offer.
    require(realListFromIdToTheValidation[_id] == true, 'The offer has already been canceled');
    require(realListFromIdToTheAvailability[_id] == true,' The offer can not be canceled owing to the offer has already been sold!');
