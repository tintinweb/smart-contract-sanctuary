...
contract CrimeCashGame is Ownable {
  ...
  //value=>cost
  mapping(uint256=>uint256) Attack;
  mapping(uint256=>uint256) Defense;
  mapping(uint256=>uint256) Boost;
  mapping(uint256=>uint256) Protect;
  ...
  event eventUpdatedAttack(uint8 assetType, uint256 assetIndex, uint256 assetValue, uint256 assetCost);
  ...
  function setGlobal() public onlyOwner {
  }
  ...
  function setSupplyGold(uint256 _goldSupply, uint256 _rate) public onlyOwner {
    require(isRoundOpening == false, "Error: There is an opened round currently");
    goldSupply = _goldSupply;
    goldForTeam = goldSupply.mul(_rate).div(100);
    goldForCrimer = goldSupply.sub(goldForTeam);
  }
  ...
  function addAttack(uint256 _index, uint256 _value, uint256 _cost) public onlyOwner {
    require(isRoundOpening == false, "Error: There is not an opened round currently");
    Attack[_value] = _cost;
    emit eventUpdatedAttack(1, _index, _value, _cost);
  }
  function removeAttack(uint256 _value) public onlyOwner {
    require(isRoundOpening == false, "Error: There is not an opened round currently");
    delete Attack[_value];
  }
  function getAttack(uint256 _value) public view returns(uint256) {
    return Attack[_value];
  }
  
  function addDefense(uint256 _index, uint256 _value, uint256 _cost) public onlyOwner {
    require(isRoundOpening == false, "Error: There is not an opened round currently");
    Defense[_value] = _cost;
    emit eventUpdatedAttack(2, _index, _value, _cost);
  }
  function removeDefense(uint256 _value) public onlyOwner {
    require(isRoundOpening == false, "Error: There is not an opened round currently");
    delete Defense[_value];
  }
  function getDefense(uint256 _value) public view returns(uint256) {
    return Defense[_value];
  }
  
  function addBoost(uint256 _index, uint256 _value, uint256 _cost) public onlyOwner {
    require(isRoundOpening == false, "Error: There is not an opened round currently");
    Boost[_value] = _cost;
    emit eventUpdatedAttack(3, _index, _value, _cost);
  }
  function removeBoost(uint256 _value) public onlyOwner {
    require(isRoundOpening == false, "Error: There is not an opened round currently");
    delete Boost[_value];
  }
  function getBoost(uint256 _value) public view returns(uint256) {
    return Boost[_value];
  }
  
  function addProtect(uint256 _index, uint256 _value, uint256 _cost) public onlyOwner {
    require(isRoundOpening == false, "Error: There is not an opened round currently");
    Protect[_value] = _cost;
    emit eventUpdatedAttack(4, _index, _value, _cost);
  }
  function removeProtect(uint256 _value) public onlyOwner {
    require(isRoundOpening == false, "Error: There is not an opened round currently");
    delete Protect[_value];
  }
  function getProtect(uint256 _value) public view returns(uint256) {
    return Protect[_value];
  }
  ...
  function setApyRate(uint256 _poolIndex, uint256[] memory _rates) public onlyOwner {
    pools[_poolIndex].apy_rate = _rates;  
  }
  ...
  function getPool(uint256 _poolIndex) public view returns(uint256 totalStaked, uint256 stakers, bool isCrimeCash, uint8 openDay, uint256[] memory apyRates ) {
    ...
    apyRates    = pools[_poolIndex].apy_rate;
  ...
  function stakeCash(uint256 _poolIndex, address _crimer, uint256 _amount) public {
  ...
  function unstake(uint256 _poolIndex, uint8 roundDay, uint256 _amount) public onlyCrimer {
  ...
  function getClaimAmount(uint256 _poolIndex, address _crimer, uint256 _boost, uint8 _roundDay) public view returns(uint256) {
    uint256 claimBalance = 0;
    if ( pools[_poolIndex].isCrimeCash ) {
      claimBalance = pools[_poolIndex].balances[_crimer].mul(uint256(pools[_poolIndex].apy_rate[_roundDay-1])).div(100);
    }
    else {
      uint256 reward = uint256(pools[_poolIndex].apy_rate[_roundDay-1]).mul(pools[_poolIndex].stakers);
...
contract CrimerInfo is Ownable {
  ...
  event stakeLpToken(uint256 poolIndex, address crimer, uint256 amount);
  event eventClearProtectCrimer(address crimer);
  ...
  function createNewCrimer(address _crimer) public onlyCrimer {
    _createNewCrimer(_crimer);
    emit eventAddNewCrimer(_msgSender());
  }
  function createNewCrimerByReferral(address _crimer, address byReferral) public onlyOwner {
    require(byReferral!=address(0), "Error: Invalid referral user");
    require(_crimer != byReferral, "Error: Something went wrong");
  ...
  function _createNewCrimer(address crimerAddress) private {
    if (isNewCrimer(crimerAddress)) {
      ...
      crimers[crimerAddress].isProtected = true;
    }
    crimecashToken.mint(address(this), initBank.add(initCash));
  }
  function crimerInfo(address _crimer) public view returns(bool exist, uint256 bank, uint256 cash, uint256 attack, 
                                                           uint256 defense, uint256 boost, uint256 referrals, bool isProtected, uint256[] memory boosts) {
    if (!isNewCrimer(_crimer)) {
      exist = crimers[_crimer].exist;
      bank = crimers[_crimer].bank;
      cash = crimers[_crimer].cash;
      attack = crimers[_crimer].attack;
      defense = crimers[_crimer].defense;
      boost = crimers[_crimer].boost;
      referrals = crimers[_crimer].referrals;
      isProtected = crimers[_crimer].isProtected;
      boosts = crimers[_crimer].boosts;
    }
  }
  function crimerCashBalance(address _crimer) public view returns (uint256) {
    return crimers[_crimer].cash;
  }
  function crimerbankBalance(address _crimer) public view returns (uint256) {
    return crimers[_crimer].bank;
  }
  ...
  function buyAsset(uint8 assetType, uint256 assetIndex, uint256 assetValue, uint8 _count) public onlyCrimer {
    require(_count <= maxBuyLimit, "Error: Something went wrong. unallocated crimer");
    uint256 _cost_amount;
    if ( assetType == 1 ) {
      uint256 asset_cost = gameEngine.getAttack(assetValue);
      _cost_amount = asset_cost.mul(uint256(_count));
      require(crimers[_msgSender()].cash >= _cost_amount, "Error: There is not enough balance to buy asset");
      crimers[_msgSender()].attack = crimers[_msgSender()].attack.add(assetValue.mul(uint256(_count)));
    }
    else if ( assetType == 2 ) {
      uint256 asset_cost = gameEngine.getDefense(assetValue);
      _cost_amount = asset_cost.mul(uint256(_count));
      require(crimers[_msgSender()].cash >= _cost_amount, "Error: There is not enough balance to buy asset");
      crimers[_msgSender()].defense = crimers[_msgSender()].defense.add(assetValue.mul(uint256(_count)));
    }
    else if ( assetType == 3 ) {
      uint256 asset_cost = gameEngine.getBoost(assetValue);
      _cost_amount = asset_cost;
      require(crimers[_msgSender()].cash >= _cost_amount, "Error: There is not enough balance to buy asset");
      require(isAlreadyHasBoost(_msgSender(), assetValue) == false, "Error: This boost is already exist");
      crimers[_msgSender()].boost = crimers[_msgSender()].boost.add(assetValue);
      crimers[_msgSender()].boosts.push(assetValue);
    }
    else {
      uint256 asset_cost = gameEngine.getProtect(assetValue);
      _cost_amount = asset_cost;
      require(crimers[_msgSender()].cash >= _cost_amount, "Error: There is not enough balance to buy asset");
      crimers[_msgSender()].isProtected = true;
    }
  ...
  function stealCash(address _crimer) public onlyCrimer {
    require(crimers[_crimer].isProtected == false, "Error: This crimer is being at protected now");
  ...
  function claim(uint256 _poolIndex, uint8 _payout_mode, uint8 _roundDay) public onlyCrimer {
    require(_roundDay >= 1 && _roundDay <= 30, "Error: Invalid round day");
    require(gameEngine.getPoolOpenDay(_poolIndex) <= _roundDay, "Pool doesn't open yet");
    uint256 _amount = gameEngine.getClaimAmount(_poolIndex, _msgSender(), crimers[_msgSender()].boost, _roundDay);
  ...
  function clearProtectCrimer(address _crimer) public onlyOwner {
    require(gameEngine.isRoundOpening() == true, "Error: There is not an opened round currently");
    require(crimers[_crimer].exist == true, "Error: Something went Wrong.");
    crimers[_crimer].isProtected = false;
    emit eventClearProtectCrimer(_crimer);
  }
  ...
  function initCrimers() public onlyOwner {
    for(uint256 i=0;i<criminals;i++) {
      ...
      crimers[crimer_addresses[i]].isProtected = false;
