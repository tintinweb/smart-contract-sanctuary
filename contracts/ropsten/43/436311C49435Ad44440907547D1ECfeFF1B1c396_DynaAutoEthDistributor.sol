...
contract DynaAutoEthDistributor is AutomatedExternalReflector {
    ...
    constructor(address tokenAddress) public {
        ...
        isExcludedFromReflections[0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D]; // Uniswap V2 Routers (Mainnet and Ropsten)
...
abstract contract  AutomatedExternalReflector is Context, LockableSwap, Ownable, IAutomatedExternalReflector {
    ...
    event TransactionLogged(address indexed from, uint256 senderAmount, address indexed to, uint256 recipientAmount);
    ...
    uint256 public totalExcludedTokenHoldings;
...
//    EnumerableSet.AddressSet private excludedList;
    ...
    EnumerableSet.AddressSet private queue;
    ...
    function depositEth() external payable override returns(bool success) {
        totalEthDeposits = totalEthDeposits.add(msg.value);
        success = true;
    }

    function addNewUser(address newUserAddress, uint256 bal) private {
        dynaHodler[newUserAddress][currentRound+1] = User(
            {
                tokenBalance: bal,
                totalPayouts: 0,
                pendingPayouts: 0,
                ...
                exists: true,
                //            excluded: false,
                lastPayoutRound: currentRound
            });
        dynaHodler[newUserAddress][currentRound] = dynaHodler[newUserAddress][currentRound+1];
    }

    function logTransactionEvent(address from, uint256 senderAmount, address to, uint256 recipientAmount) external override {
        if(from != address(0)) {
            logUserTransaction(from, senderAmount, true);
        }
        if(to != address(0)) {
            logUserTransaction(to, recipientAmount, false);
        }
        emit TransactionLogged(from, senderAmount, to, recipientAmount);
    }

    function logUserTransaction(address user, uint256 value, bool isSender) private {
        queue.add(user);

        if(!dynaHodler[user][currentRound+1].exists){
            if(dynaHodler[user][currentRound].exists){
                dynaHodler[user][currentRound+1] = dynaHodler[user][currentRound];
                dynaHodler[user][currentRound+1].tokenBalance = value;
            } else {
...
//            if(isSender){
//                totalExcludedTokenHoldings = totalExcludedTokenHoldings.add(value);
//            } else {
//                totalExcludedTokenHoldings = totalExcludedTokenHoldings.sub(value);
//            }
//        }
    }
    ...
    function _reflectRewards() private lockTheSwap returns(bool allComplete) {
        ...
        uint256 endGas = maxGas.sub(gasleft());

        while(gasleft() > minGas && gasleft() > endGas && currentQueueIndex < stopProcessingAt && currentQueueIndex < queue.length()){
            address hodler = queue.at(currentQueueIndex);
            if(isExcludedFromReflections[hodler]){
                currentQueueIndex++;
                continue;
            }
            _sendEthTo(hodler);
        ...
        if(currentQueueIndex >= queue.length() || poolForRound[currentRound] == 0){
            currentQueueIndex = 0;
            allComplete = true;
            nextRoundStart = block.timestamp.add(timeBetweenRounds + 0 hours);
            currentRound = currentRound.add(1);
            poolForRound[currentRound] = address(this).balance;
        }
    }

    function _sendEthTo(address hodler) private {
        if(!dynaHodler[hodler][currentRound].exists){
            dynaHodler[hodler][currentRound] = dynaHodler[hodler][currentRound - 1];
        }
        if(!dynaHodler[hodler][currentRound].exists || dynaHodler[hodler][currentRound].tokenBalance == 0) { return; }
        if(dynaHodler[hodler][currentRound].lastPayoutRound == currentRound){ return; }
        uint256 reward = dynaHodler[hodler][currentRound].pendingPayouts;
        reward = reward.add(simplePayoutCalc(hodler));
        if(allowLowLevelCalls){
            payable(hodler).send(reward);
        } else {
            payable(hodler).transfer(reward);
        }
    }
    ...
    function enableReflections(bool enable) external override { //onlyOwner {
        require(enable != reflectionsEnabled, "Reflections already set to this value");
        reflectionsEnabled = enable;
    }

    function excludeFromReflections(address target, bool exclude) external override { // onlyOwner {
        require(isExcludedFromReflections[target] != exclude, "This address is already included/excluded");
...
//        }
    ...
    function updateTokenAddress(address token) external onlyOwner{
        ...
        isExcludedFromReflections[token] = true;
    }

    function updateTotalSupply(uint256 newTotal) external override onlyOwner {
        totalCirculatingTokens = newTotal;
    }

    function collectShare() external override {}

    function allowUnsafeTransactions(bool allow) external {
        allowLowLevelCalls = allow;
    }

    function updateGasRange(uint256 _minGas, uint256 _maxGas) public {
        minGas = _minGas;
        maxGas = _maxGas;
    }
}
...
interface ISupportingExternalReflection is IERC20 {
