...
/*
  ...
  function withdrawStakedTokens(uint256 stakingId) public failSafe returns(bool){
    ...
    require(block.timestamp > _stakingStartTime[stakingId].add(2592000), "30 days staking time is not over");
 ...
 * @dev Modifier to check if a user account is blocked
 */
    modifier onlyBySubmitter(uint256 proposalID) {
      require(msg.sender == proposals[proposalID].submitter);
      _;
    }
