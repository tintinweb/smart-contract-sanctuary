...
contract DTF is ERC20, ERC20Burnable, Ownable {
    uint256 private constant INITIAL_SUPPLY = 1000 * 10**(6 + 18); // 1000M tokens
    AntiBotInterface antiBot = AntiBotInterface(address(0));
    constructor()
    ERC20("DTF Game", "DTF") 
    {
    ...
    function setAntiBot(address _address) public onlyOwner{
        antiBot = AntiBotInterface(_address);
    }
    ...
    function _transfer(
        ...
        if (antiBot != AntiBotInterface(address(0)) && !antiBot._transfer(sender,recipient,amount)) {
            revert("Anti Bot");
        }
