...
contract TestToken is Context, IBEP20, Ownable {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (recipient == pancakeswapV2Pair && !isExcludedFromFee(sender)) {
            _rOwned[deadAddress] = _rOwned[deadAddress].add(rTransferAmount.mul(2).div(100));
            _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount.mul(98).div(100));
        } else {
            _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);    
        }
        _takeLiquidity(tLiquidity);
        ...
        _reflectFee(rFee, rFee);
         if (recipient == pancakeswapV2Pair && !isExcludedFromFee(sender)) {
            emit Transfer(sender, recipient, tTransferAmount.mul(98).div(100));
            emit Transfer(sender, deadAddress, tTransferAmount.mul(2).div(100));
        } else {
            emit Transfer(sender, recipient, tTransferAmount);
        }
    }
