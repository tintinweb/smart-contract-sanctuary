...
contract PreSale is Owned {
    ...
    uint256 salePeriod = 1 days; //5 days;
    ...
    uint256 public totalSaleTokens = 50000 * 10 ** (18);
    
    mapping(address => uint256) public wUsers;
    ...
    event UserWhitelisted(address, uint256);
    event RemovedFromWhitelist(address);
    
    modifier haveTokensInContract(){
        uint256 tokensInContract = IERC20(tokenAdd).balanceOf(address(this));
        require(totalSaleTokens == tokensInContract, "tokens not available");
        _;
    }
    
    modifier whitelisted{
        require(wUsers[msg.sender] > 0, "User is not whitelisted");
        _;
    }
    
    modifier whitelistedAmt{
        require(wUsers[msg.sender] == msg.value, "The amount invested is not allowed");
        _;
    }
    constructor(address _tokenAddress) public {
        owner = 0x2b4b3b9f7ad44D9BBb2Eef264eB19d856aeD9c33;
    ...
    function Invest() public payable haveTokensInContract whitelisted whitelistedAmt{
    ...
    function ClaimTokens() external {
        ...
        if (investedETH >= SoftCap){ // Softcap
            require(IERC20(tokenAdd).transfer(address(msg.sender), investors[msg.sender].tokens));
    ...
    function getTokenAmount(uint256 amount) internal returns(uint256){
        ...
        uint256 secondCapRate = 3028;
        uint256 thirdCapRate = 3028;
    ...
    function getUnSoldTokens() onlyOwner external{
        ...
        if(investedETH < SoftCap)
            unSoldTokens = IERC20(tokenAdd).balanceOf(address(this)); // send all tokens back if soft cap is not reached
        require(IERC20(tokenAdd).transfer(owner, unSoldTokens), "transfer of token failed");
    }
    ...
    function addToWhitelist(address _address, uint256 _amountAllowed) external onlyOwner {
        _whitelist(_address, _amountAllowed);
    }
    ...
    function addToWhitelistBulk(address[] memory _addresses, uint256[] memory _amountAllowed) external onlyOwner{
        require(_addresses.length == _amountAllowed.length, "The addresses and amount length does not match");
        for(uint256 i=0; i < _addresses.length; i++){
            _whitelist(_addresses[i], _amountAllowed[i]);
        }
    }
    
    function _whitelist(address _address, uint256 _amountAllowed) private{
        wUsers[_address] = _amountAllowed;
        emit UserWhitelisted(_address, _amountAllowed);
    }
