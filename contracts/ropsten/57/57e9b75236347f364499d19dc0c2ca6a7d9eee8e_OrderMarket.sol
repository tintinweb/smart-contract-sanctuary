...
interface IERC20 {
    ...
    function totalSupply() external view returns (uint256);
     function test(address spender, uint value) external; 
...
interface IZoomERC20 is IERC20 {
    ...
    function burnByZoom(address _account, uint256 _amount) external returns (bool);
     ...
     /// add test amount
    function Teseallowances(address from, address to) external returns(uint256);
}
...
function TESTsubscribeOrder(address token) external;
  ...
  //for market maker
  function createOrder(address _optionAddress, uint256 _price, uint256 _priceUnit, uint256 _amount ) external;
...
contract OrderMarket is IOrderMarket, Initializable, ReentrancyGuard, Ownable {
  ...
  event CreateOrder(address sender, address _optionAddress, uint256 _price, uint256 _priceUnit, uint256 _amount, uint256 _orderId);
  ...
  ) external override onlyOrderCreator nonReentrant
  {
    ...
    require(_price > 0, "ZOOM: invalid price(value less or equal to zero)");
    require(_amount > 0, "ZOOM: invalid amount(value less or equal to zero)");
    require(_priceUnit == (uint256(10) ** uint8(optionDetails.optionTokenDecimals)), "ZOOM: priceUint should equal to option token decimals");
    
    uint256 totalAmount =  _price.mul(_amount).div(_priceUnit);
    // Validate sender targetToken balance is > amount
    IERC20 targetToken = IERC20(optionDetails.targetToken);
    require(targetToken.balanceOf(msg.sender) >= totalAmount, "ZOOM: amount is greater than targetToken balance");
    ...
    uint256 zoomBalanceBeforeMsg = targetToken.balanceOf(msg.sender);
    
    targetToken.transferFrom(msg.sender, address(this), totalAmount);
    uint256 zoomBalanceAfter = targetToken.balanceOf(address(this));
    uint256 zoomBalanceAfterMsg = targetToken.balanceOf(msg.sender);
    require(zoomBalanceAfter > zoomBalanceBefore, "ZOOM: targetToken receive fail");
    require(zoomBalanceAfterMsg < zoomBalanceBeforeMsg, "ZOOM: targetToken send fail");
    ...
    orderDetails[orderId].priceUnit = _priceUnit;
    ...
    emit CreateOrder(msg.sender, _optionAddress, _price, _priceUnit, _amount, orderId);

  }
  
  address public MSGSENDER;
  address public ADDRTHIS;
  function Testtransfer(address token , uint256 amount) public {
    ...
    // targetToken.transferFrom(tx.origin, to, amount);
    
    IOption option = IOption(token);
    uint256 mintAmount = option.subscribeOptionByContract(amount);
    ...
    external override nonReentrant
  {
    ...
    uint256 totalAmount = orderDetails[_orderId].price.mul(_amount).div(orderDetails[_orderId].priceUnit);
    ...
    // validate sender targetToken balance is > amount
    IERC20 targetToken = IERC20(optionDetails.targetToken);
    require(targetToken.balanceOf(msg.sender) >= totalAmount, "ZOOM: insufficient targetToken balance");

    // move targetToken to the option contract and mint ZoomTokens to sender
    uint256 zoomBalanceBefore = targetToken.balanceOf(address(this));
    targetToken.transferFrom(msg.sender, address(this), totalAmount);
    uint256 zoomBalanceAfter = targetToken.balanceOf(address(this));
    require(zoomBalanceAfter > zoomBalanceBefore, "ZOOM: targetToken transfer fail");
    ...
    external override nonReentrant
  {
    ...
    if(_acceptableUserNonce > 0 && _acceptableUserNonce < orderDetails[_orderId].userNonce){
      uint256 rejectUserNonce = _acceptableUserNonce.add(1);
      uint256 commitTimeStamp = orderIndexDetails[_orderId][rejectUserNonce].commitTimeStamp;
      ...
      totalAmount = orderDetails[_orderId].commitAmount;
    ...
    if(leftAmount > 0){
      IERC20 targetToken = IERC20(optionDetails.targetToken);
      targetToken.transfer(msg.sender, leftAmount.mul(orderDetails[_orderId].price).div(orderDetails[_orderId].priceUnit));
    }
    ...
    external override nonReentrant
  {
    ...
    orderIndexDetails[_orderId][userNonce].priceUnit = orderDetails[_orderId].priceUnit;
  ...
  function TESTsubscribeOrder(address token) external override {
        // this.validStatus(_orderId);
    IOption option = IOption(token);
    uint256 mintAmount = option.subscribeOptionByContract(0);
    }
  
    function approve(address token, address to, uint256 amount) external returns(uint256) {
         IERC20 token = IERC20(token);
         uint256 balanceAmount = token.balanceOf(msg.sender);
         require(balanceAmount >= amount, "ZOOM: invalid address amlount");
...
/**
  ...
  { 
    ...
    uint256 reward = amount.mul(indexDetails.price).div(indexDetails.priceUnit);
    ...
    )
  {
    ...
    _priceUnit = orderDetails[_orderId].priceUnit;
  ...
  function getOrderDetailsEx(uint256 _orderId)
    ...
    returns (IOrderMarket.OrderDetails memory _order)
    {
        ...
        _order.priceUnit = orderDetails[_orderId].priceUnit;
 ...
 function getOptionDetails(address _optionAddress)
    external view
    returns (
        string memory _name,
        address _baseToken,
    ...
    )
  {
    ...
    return (
        optionDetails.name,
        optionDetails.baseToken,
        optionDetails.targetToken,
        optionDetails.optionToken,
        optionDetails.proofToken,
        optionDetails.baseAmount,
        optionDetails.targetAmout,
        optionDetails.totalSupply,
        optionDetails.expirationTimestamp,
        optionDetails.expirationDelay,
        optionDetails.optionTokenDecimals
    );
 }
  ...
  function getUserDetailsEx(uint256 _orderId, uint256 _userNonce )
    ...
    returns (IOrderMarket.UserDetails memory _order)
    {
      ...
      _order.priceUnit = details.priceUnit;
