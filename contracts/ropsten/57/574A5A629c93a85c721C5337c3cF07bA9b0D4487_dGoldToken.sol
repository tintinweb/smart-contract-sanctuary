...
contract dGoldToken is ERC20, Ownable {
    ...
    constructor() ERC20 ("Devious Licks dGOLD", "dGOLD11") {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool res) {
        res = _transferFrom(sender, recipient, amount);
    ...
    function getRewardDivision(bool isBuying) external view returns (RewardDivision[] memory rewardForTx) {
        rewardForTx = rewardSetting[isBuying];
    }

    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool){
        bool isExempt = isExemptAccount[sender] || isExemptAccount[recipient];
        if(isExempt || (sender != pair && recipient != pair) || IERC20(pair).totalSupply() == 0 || isSwapping) {
            _transfer(sender, recipient, amount);
            if(recipient != address(0) && recipient != address(pair) && !dGoldTokenHolders.contains(recipient))
        ...
        for(uint i = 0; i <  rewardForTx.length; i++){
            ...
            if(rewardForTx[i].account == address(this)){
                ...
                {
                    uint256 feeToHolders = amount * rewardForTx[i].percentage / (feeDenominator * countHolders);
                    _transfer(sender, dGoldTokenHolders.at(j), feeToHolders);
                    totalFee += feeToHolders;
                }
            }
            else if(rewardForTx[i].account == pair) {
                _transfer(sender, address(this), amount * rewardForTx[i].percentage / feeDenominator);
        ...
        _transfer(sender, recipient, (amount - totalFee));
