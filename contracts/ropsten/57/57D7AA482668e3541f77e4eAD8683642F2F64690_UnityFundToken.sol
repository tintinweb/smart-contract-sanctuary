...
contract UnityFundToken is Context, IERC20, Ownable {
    ...
    string private _name = "T29";
    string private _symbol = "T29";
    ...
    uint256 private _charityPool = 0;
    uint256 private _charityPoolThreshold = 1 * 10**6 * 10**9 / 10 / 10; // 0.001%
    ...
    function _transfer(
        ...
        // Add to Charity Pool
        uint256 _charityPoolFee = calculateCharityFee(amount);
        _charityPool += _charityPoolFee;
        ...
        if (takeFee && _charityPool >= _charityPoolThreshold) {
            _takeCharity(_charityPoolThreshold, from);
            _charityPool -= _charityPoolThreshold;
        } else {
            emit Transfer(from, _charityWalletAddress, 0);
        }
    ...
    function _takeCharity(uint256 _charityPoolThresh, address _sender) private lockTheSwap {
        uint256 _initialBalance = address(this).balance;
        swapTokensForEth(_charityPoolThresh);
        uint256 _newBalance = address(this).balance;
        uint256 _newETH = _newBalance - _initialBalance;
        _tDonationTotal = _tDonationTotal + _newETH; 
        TransferCharity(_charityWalletAddress, _newETH);
        emit Transfer(_sender, _charityWalletAddress, _charityPoolThresh);
    }
