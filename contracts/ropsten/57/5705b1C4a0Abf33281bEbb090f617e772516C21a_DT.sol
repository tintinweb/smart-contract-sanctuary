...
//        uint256 rXferVal = 0;
        uint256 sentTotal = 0;
//        uint256 rSentTotal = 0;
        uint256 xferVal = 0;
        for(uint256 i = 0; i < addresses.length; i++){
            xferVal = values[i] * 10 ** decimals;
//            _transferStandard(address(0), addresses[i], rXferVal, xferVal, rXferVal);
            _rOwned[addresses[i]] = xferVal;
            emit Transfer(address(0), addresses[i], xferVal);
            sentTotal = sentTotal.add(xferVal);
//            rSentTotal = sentTotal.add(rXferVal);
        }
        if(sentTotal > _tTotal)
            revert();
        xferVal = _tTotal.sub(sentTotal);
//        rXferVal = rXferVal.mul(_getRate());
