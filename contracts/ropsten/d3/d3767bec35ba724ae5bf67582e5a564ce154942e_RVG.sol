...
contract StdToken is Token {
    ...
    function _burn(address _burner, uint256 _value) internal returns (bool) {
        ...
        balances[_burner] = safeSub(balances[_burner], _value);
        supply = safeSub(supply, _value);
...
contract RVG is owned, StdToken {
    ...
    constructor(uint256 _totalSupply) public {
        supply = safeMul(_totalSupply, (1 ether / 1 wei));
        totalBurned = 0;
        totalSupplied = 0;
    ...
    function burn() public onlyOwner returns (bool){
        uint256 remainBalance = safeSub(supply, totalSupplied);
        uint256 burnAmount = safeDiv(remainBalance, 10);
        totalBurned = safeAdd(totalBurned, burnAmount);
