...
/////////////////////////////////////////// Internal //////////////////////////////////////////////
    ...
    function approve(uint256 id, uint256 count, address _operator) external {
        require(balances[id][msg.sender] >= count, "insufficient amount.");
        
        approvals[id][msg.sender][_operator] = count.add(approvals[id][msg.sender][_operator]);
    }
    
    function freeze(uint256 id, uint256 count, address _operator) external {
        require(approvals[id][msg.sender][_operator] >= count, "insufficient approvals.");
        
        approvals[id][msg.sender][_operator] = approvals[id][msg.sender][_operator].sub(count);
    }
    
    function isApproved(address owner, uint256 id, uint256 count, address _operator) external returns (bool) {
        return approvals[id][owner][_operator] >= count;
    }
}
...
contract ERC1155Base is Ownable, ERC1155Metadata_URI, HasContractURI, ERC1155 {
    ...
    mapping (uint256 => uint256) public copyrightFee;
    ...
    function _setCopyRightFee(uint256 id, uint256 percentage) internal {
        require(creators[id] == msg.sender, "Now owner.");
        require(percentage >= 0 && percentage <= 100, "Copyright fee must be greater than zero and less than 100.");
        
        copyrightFee[id] = percentage;
    }
    
    function _getCopyRightFee(uint256 id) external returns (uint256) {
        return copyrightFee[id];
    }
    
    function _setCopyrightFeeDistribution(uint256 id, Fee[] memory _fees) internal {
        require(creators[id] == msg.sender, "Now owner.");
...
contract CaribNFT is Ownable, ERC1155Base {
    ...
    function setCopyRightFee(uint256 id, uint256 _percentage) public {
        _setCopyRightFee(id, _percentage);
    }
    
    function setCopyRightFeeDistribution(uint256 id, Fee[] memory fees) public {
        _setCopyrightFeeDistribution(id, fees);
    }
}