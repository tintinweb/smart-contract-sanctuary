...
contract artToken {
    ...
    modifier onlyAdmin {
        require(msg.sender==admin,"Only admin is allowed to access");
        _;
    }
    ...
    function setUri(uint256 _tokenId, string memory _artName ) public onlyAdmin {
    ...
    mapping(address=> mapping(uint256=>uint256)) auctionPrice;  //address=>tokenId=>price
    ...
    function auction(uint256 _tokenId) public payable{
    ...
    function result(uint256 _tokenId) public payable onlyAdmin returns(address,uint256) {
        require(now>=endTime,"Auction is still going");
        require(nft._exists(_tokenId),"Token Id does not exist");
        ...
        nft.transferFrom(admin,highestBidder,_tokenId);
