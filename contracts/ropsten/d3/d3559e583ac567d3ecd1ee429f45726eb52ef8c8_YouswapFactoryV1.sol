...
interface IYouswapFactoryV1 {
    ...
    self：Sender地址
     */
    event InviteRegister(address self);
...
contract YouswapFactoryV1 is IYouswapFactoryV1 {
    ...
    function computeReward(uint256 _pool) internal {
        ...
        if ((0 < poolInfo.totalPower) && (poolInfo.rewardProvide < poolInfo.rewardTotal)) {
            ...
            poolInfo.rewardPerShare = poolInfo.rewardPerShare.add(reward.mul(1e13).div(poolInfo.totalPower));
    ...
    function provideReward(uint256 _pool, uint256 _rewardPerShare, address _lp, address _user, address _upper1, address _upper2) internal {
        ...
        if ((0 < userInfo.invitePower) || (0 < userInfo.pledgePower)) {
            inviteReward = userInfo.invitePower.mul(_rewardPerShare).sub(userInfo.inviteRewardDebt).div(1e13);
            pledgeReward = userInfo.pledgePower.mul(_rewardPerShare).sub(userInfo.pledgeRewardDebt).div(1e13);
        ...
        if (address(0) != _upper1) {
            ...
            if ((0 < upper1Info.invitePower) || (0 < upper1Info.pledgePower)) {
                inviteReward = upper1Info.invitePower.mul(_rewardPerShare).sub(upper1Info.inviteRewardDebt).div(1e13);
                pledgeReward = upper1Info.pledgePower.mul(_rewardPerShare).sub(upper1Info.pledgeRewardDebt).div(1e13);
            ...
            if (address(0) != _upper2) {
                ...
                if ((0 < upper2Info.invitePower) || (0 < upper2Info.pledgePower)) {
                    inviteReward = upper2Info.invitePower.mul(_rewardPerShare).sub(upper2Info.inviteRewardDebt).div(1e13);
                    pledgeReward = upper2Info.pledgePower.mul(_rewardPerShare).sub(upper2Info.pledgeRewardDebt).div(1e13);
    ...
    function pendingReward(uint256 _pool, address _user) override external view returns (uint256) {
        ...
        if (address(0) != poolInfo.lp && (poolInfo.startBlock <= block.number)) {
            ...
            if (0 < poolInfo.totalPower) {
                ...
                rewardPerShare = reward.mul(1e13).div(poolInfo.totalPower);
            }
            rewardPerShare = rewardPerShare.add(poolInfo.rewardPerShare);

            UserInfo memory userInfo = pledgeUserInfo[_pool][_user];
            totalReward = userInfo.pendingReward;
            totalReward = totalReward.add(userInfo.invitePower.mul(rewardPerShare).sub(userInfo.inviteRewardDebt).div(1e13));
            totalReward = totalReward.add(userInfo.pledgePower.mul(rewardPerShare).sub(userInfo.pledgeRewardDebt).div(1e13));
        }
