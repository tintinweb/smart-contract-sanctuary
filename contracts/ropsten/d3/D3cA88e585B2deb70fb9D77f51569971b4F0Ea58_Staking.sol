...
contract Staking is Ownable {
    ...
    struct PoolInfo {
        address stakeToken;
        address rewardToken;
        uint256 rewardRate; // Reward rate multiplied by 10^18
    ...
    function emergencyWithdraw(address token) external onlyOwner {
        ...
        require(
            IERC20(token).transfer(
                msg.sender,
                IERC20(token).balanceOf(address(this)).sub(tokenAmounts[token])
            ),
    ...
    function deposit(uint256 pid, uint256 amount) external {
        ...
        uint256 rewardAmount = amount.mul(pool.rewardRate).div(1e18);
        require(
            rewardAmount <= getRemainingRewards(pool.rewardToken),
        ...
        tokenAmounts[pool.stakeToken] = tokenAmounts[pool.stakeToken].add(
            amount
        );
        tokenAmounts[pool.rewardToken] = tokenAmounts[pool.rewardToken].add(
            rewardAmount
        );
    ...
    function withdraw(uint256 stakeId) external {
        ...
        tokenAmounts[info.stakeToken] = tokenAmounts[info.stakeToken].sub(
            info.amount
        );
        tokenAmounts[info.rewardToken] = tokenAmounts[info.rewardToken].sub(
            info.rewardAmount
        );
        ...
        if (info.endTime < block.timestamp) {
            ...
            emit Claim(msg.sender, info.rewardToken, info.rewardAmount);
        }
    ...
    function getRemainingRewards(address token) public view returns (uint256) {
        uint256 totalBalance = IERC20(token).balanceOf(address(this));

        if (totalBalance < tokenAmounts[token]) {
            return 0;
        }
        return totalBalance.sub(tokenAmounts[token]);
    }
