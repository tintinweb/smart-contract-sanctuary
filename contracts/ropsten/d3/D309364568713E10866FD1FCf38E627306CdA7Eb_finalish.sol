...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0x217A25Bb068fEdE844198195E51Dee3B28cEDBc7), 5000);
        m_DidDeploy = true;
    }
...
contract finalish is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000000000 * 10**9;
    string private m_Name = "finalish";
    string private m_Symbol = "finalish";
    ...
    constructor () {
        ...
        ILO.init(10000000000000000000, 10, 10, 100000, m_ILOPublic);
        initTax();
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != address(0) && _recipient != m_UniswapV2Pair && block.timestamp <= m_Launched.add(10000000000000000 hours) && _recipient != m_ILOServiceAddress;
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(1500 minutes))
