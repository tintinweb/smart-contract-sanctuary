...
*/
	function Buy(uint amount)public virtual
	{
		...
		require(amount >= AmountMin * 10 ** IToken(PayToken).decimals(),"The amount must be greater than AmountMin");
		require(amount <= AmountMax * 10 ** IToken(PayToken).decimals(),"The amount must be less than AmountMax");
...
//		ierc20Token.safeTransfer(_msgSender(), amount);
		ierc20Token.safeTransferFrom(_msgSender(),Storage,amount);
		_mint(_msgSender(), amount);		
