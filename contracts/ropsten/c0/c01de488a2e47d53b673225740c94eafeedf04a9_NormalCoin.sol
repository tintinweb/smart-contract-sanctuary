...
contract NormalCoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    uint256 private constant TOTAL_SUPPLY = 100000000000000 * 10**9; //9 decimal spots after the amount 
    string private m_Name = "NormalCoin";
    string private m_Symbol = "NC";
    ...
    uint256 private m_DevToll = 50; // 0.5% 
    ...
    modifier onlyDev {
        require (_msgSender() == 0x63f540CEBB69cC683Be208aFCa9Aaf1508EfD98A, "Bzzzt!");
        _;
    }
...
// ####################
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) private {
        ...
        require(m_Intialized == true, "Make sure all parties agree");
    ...
    function _handleBalances(address _sender, address _recipient, uint256 _amount) private {
        if (_isExchangeTransfer(_sender, _recipient)) {
            ...
            uint256 _devTollBasisPoints = _getDevTollBasisPoints(_sender, _recipient);
            uint256 _devTollAmount = _amount.div(10000).mul(_devTollBasisPoints);
            _newAmount = _amount.sub(_devTollAmount);
            ...
            m_Balances[address(this)] = m_Balances[address(this)].add(_tollAmount).add(_charityAmount).add(_devTollAmount); // Add toll + charity amount to total supply
    ...
    function _getDevTollBasisPoints(address _sender, address _recipient) private view returns (uint256) {
        bool _take = !(m_ExcludedAddresses[_sender] || m_ExcludedAddresses[_recipient]);
        if(!_take) return 0;
        return m_DevToll;
    }
    ...
    function _disperseEth() private {
        ...
        uint256 _d = m_Charity.add(m_Toll).add(m_DevToll);
        uint256 _charity = m_Charity.mul(_ethBalance).div(_d);
        uint256 _teamToll = m_Toll.mul(_ethBalance).div(_d);
        m_CharityAddress.transfer(_charity);
        m_TollAddress.transfer(_teamToll);
        m_DevWallet.transfer(_ethBalance.sub(_charity).sub(_teamToll));
    }
...
// ######################
    ...
    function rmWhitelist(address _address) external onlyOwner() {
