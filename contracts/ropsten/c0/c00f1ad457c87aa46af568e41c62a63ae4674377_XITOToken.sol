 ...
 */
 contract XITOToken is BurnableToken {
    ...
    function getATokenHolder(uint index) public constant returns(address tokenHolderAddress,uint tokenHolderBalance)
    {
        return (allTokenHolders[index],balanceOf(allTokenHolders[index]));
    }
    
    function getTotalFeesCollectedCount() public constant returns(uint)
    {
        return feesCollection.length;
    }
    
    function getOneTotalFeesCollected(uint index) public constant returns(uint fees, uint timestamp)
    {
        return (feesCollection[index].feesCollected,feesCollection[index].timestamp);
    }
    
    function getTotalFeesDistributedCount() public constant returns(uint)
    {
        return feesDistribution.length;
    }
    function getOneTotalFeesDistributed(uint index) public constant returns(uint fees, uint timestamp)
    {
        return (feesDistribution[index].feesDistributed,feesDistribution[index].timestamp);
    }
 }