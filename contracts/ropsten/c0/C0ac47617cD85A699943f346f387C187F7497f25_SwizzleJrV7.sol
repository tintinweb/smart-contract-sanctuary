...
contract SwizzleJrV7 is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) private _oddsArray;
    ...
    function initOdds() external onlyOwner {
        _initOdds();
    }
        ...
        returns (string memory)
    {
        ...
        return string(abi.encodePacked(base, ipfsUri, tokenPartUri, tokenSlot.toString(), tokenExtension));
    }

    function mintSwizzles(uint256 count) external payable returns (uint256) {
        ...
        uint256 initialIndex = _mintSwizzle(msg.sender, count);
    ...
    function _initOdds() private {
        uint256 nextFlavor;
        uint256 lastBit = 0;

        // Init odds array.
        for (uint256 i = 0; i < maxNftsForSale; i++) {
            nextFlavor = (i % numberOfFlavors) + 1; // Distribute equally.

            if(lastBitEnabled) {
                lastBit = (i % 100) > 61 ? 1 : 0; // Distribute based on golden ratio.
            }

            _oddsArray[i] = (nextFlavor * 100) + lastBit;
        }
    }

    function _mintSwizzle(address minter, uint256 count)
        private
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            uint256 newItemId = _coinMintCounter.current();
            _safeMint(minter, newItemId);

            // More to do here.
            uint256 oddsIdx = uint256(
                keccak256(abi.encodePacked(newItemId, block.number))
            ) % maxNftsForSale;
            tokenFlavor = _oddsArray[oddsIdx];

            require(tokenFlavor > 0, "Flavor invalid");
