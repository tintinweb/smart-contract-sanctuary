...
contract Chrono is ERC20, EpochMintableToken, Ownable {
    using SafeMath for uint256;

    string public name = "Chrono";
    string public symbol = "CHRONO";
    ...
    uint public epochRate = 2 * (10**decimals);
...
//------------------------------------------------------------------------------
    ...
    function getMintingReward(address _address) internal returns (uint) {
        ...
        uint256 rewardInterval = 1 years;
        uint256 coinbase = balances[msg.sender] * epochEra;
        uint256 rewardPerBlock = coinbase.div(epochRate * rewardInterval);
