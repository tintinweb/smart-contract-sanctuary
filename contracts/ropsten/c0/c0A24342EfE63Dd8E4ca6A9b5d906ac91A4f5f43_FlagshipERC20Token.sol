...
/**
 * @notice It is solidity ^0.8.0 safeMath.sol from openzeppelin
 * NOTE : Inclue Absolute Subtraction.
 */
...
contract FlagshipERC20Token is ERC20Permit, Ownable {
  ...
  /* ========== STATE VARIABLES ========== */
  address public immutable _FexP;

  bool public allTaxOrExempt = false;

  event TaxedTransfer(address indexed _from, address indexed _to, uint256 _afterTaxedAmount, uint256 _taxes);
  event TaxExemptTransfer(address indexed _from, address indexed _to, uint256 _amount);
  ...
  constructor(address _fuelExchangePermit) ERC20("Flagship Fuel", "FF", 18) {
    require(_fuelExchangePermit != address(0), "Zero address: fuelExchangePermit");
    _FexP = _fuelExchangePermit;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal override {
    ...
    uint256 taxes;
    IERC721 fuelExchangePermit = IERC721(_FexP);
    if (fuelExchangePermit.balanceOf(sender) == 0 && !allTaxOrExempt) { // no fuelExchangePermit, has to be taxed; adjust amount
    ...
    emit TaxExemptTransfer(sender, recipient, amount);
  }
  ...
  function _taxTxn(address _sender, uint256 _amount) internal returns (uint256 _afterTaxedAmount, uint256 _taxes) {
    ...
    _taxes = _taxes.add(_checkPortion(_amount));
  ...
  function _checkTransferRecords(address _sender, uint256 _amount) internal returns (uint256 taxAmount) {
    ...
    if (block.timestamp.absSub(_transferRecords[_sender]._lastTxnTime) >= 1 hours) { // one hour time reached; reset
      ...
      _transferRecords[_sender]._numTxnWithinInterval++;
    }
    ...
    if (_transferRecords[_sender]._numTxnWithinInterval >= 2 && _transferRecords[_sender]._numTxnWithinInterval <= 4) {
      taxAmount = (_amount / 50) * (_transferRecords[_sender]._numTxnWithinInterval.sub(1)); // add 2% for each;
    } else if (_transferRecords[_sender]._numTxnWithinInterval >= 5) {
      taxAmount = (_amount / 10); // 10%;
    } else {
      taxAmount = _amount / 100; // it is the minimum taxAmount; 1%
    }
  } 

  function _checkPortion(uint256 _amount) internal view returns (uint256) {
    // check percentage of _totalSupply; adjust _amount
    uint256 one_percentile = _totalSupply / 100;
    uint256 two_percentile = _totalSupply / 50;
    uint256 five_percentile = _totalSupply / 20;
    if (_amount > five_percentile) {
      return (_amount - five_percentile) / 2; // add 50% of that is more than five_percentile
    } else if (_amount > two_percentile) {
      return (_amount - two_percentile) / 5; // add 20% of that is more than five_percentile
    } else if (_amount > one_percentile) {
      return (_amount - one_percentile) / 10; // add 10% of that is more than five_percentile
    }
  ...
  /* ========== PERMIT FUNCTIONS ========== */
  function setallTaxOrExempt(bool _allTaxOrExempt) public onlyOwner() {
    require(_allTaxOrExempt != allTaxOrExempt, "State will not be changed!");
    allTaxOrExempt = _allTaxOrExempt;
  }
