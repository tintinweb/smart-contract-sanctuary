...
contract SmartOneX5 {    
    ...
    uint256 private constant LEVEL_HIGHER_FOUR_EXPIRE_TIME = 180 days;
    
    mapping(uint256 => address) public userAddresses;    
    mapping(address => User) public users;
     
    address payable private owner;
    address payable private admin = payable(0x3cEF2B8940fcDf64F440e6bc8BF5a4D7A069252a);
    ...
    uint256 private constant signUpAmount = 0.11 ether;
    ...
    event LevelProfit(address indexed user, address referral, uint256 level, uint256 value);
    event BuyLevel(address indexed user, uint256 level, uint256 value);
    ...
    constructor(){
        ...
        users[owner].referrals.push(admin);
        
        for (uint256 i = 0; i < MAX_LEVEL; i++) {
            users[owner].levelExpiresAt[i] = 1 << 37; // Never expire
        }

        last_uid++;
        User storage userTwo = users[admin];
        userTwo.id = last_uid;
        userTwo.referrerID = 1;
        userTwo.referrals =  new address[](0);

        userAddresses[last_uid] = admin;

        for (uint256 i = 0; i < MAX_LEVEL; i++) {
            users[admin].levelExpiresAt[i] = 1 << 37; // Never expire
        }
    }

    function registerUser() public payable userNotRegistered()
    {
        uint256 _referrerID = 2;

        require(levelPrice[0].add(signUpAmount) >= msg.value, "Invalid signup amount sent");
        ...
        transferLevelPayment(_level, msg.sender, true);
    }


    function buyLevel(uint256 _level) public payable userRegistered() validLevel(_level) validLevelAmount(_level)
    {
        require(getUserLevelExpiresAt(msg.sender, 0) >= block.timestamp, "Buy previous level first");
        ...
        transferLevelPayment(_level, msg.sender, false);
    }
    ...
    function getUserUpline(address _user, uint256 height) internal view returns (address)
    {
        ...
        for (uint256 i = 0; i < height; i++) {
            if(userAddresses[users[upline].referrerID] == address(0)){
                break;
            }
            else{
                upline = userAddresses[users[upline].referrerID];
            }
        }
    ...
    function transferLevelPayment(uint256 _level, address _user, bool signUp) internal
    {
        address userUpline;
        uint256 uplines = 3; 
        ...
        if(signUp){
            levelPayment = levelPayment.sub(signUpAmount);
            admin.transfer(signUpAmount);
        }
        else{
            emit BuyLevel(_user, _level, levelPayment);
        }

        uint256 fee = levelPayment.mul(100).div(DIVIDER); // Fee 10% of level price
        uint256 userPayment = levelPayment.sub(fee).div(uplines); // User payment 90% of level price
                
        for (uint256 i = 1; i <= uplines; i++) {
            userUpline = getUserUpline(_user, i);

            // If referrer exist and has and inactive plan
            if (userUpline != address(0) && (users[_user].levelExpiresAt[_level] == 0 || getUserLevelExpiresAt(userUpline, _level) < block.timestamp)) {
                continue;
            }
            ...
            payable(userUpline).transfer(userPayment);
            emit LevelProfit(userUpline, msg.sender, _level, userPayment);
        }

        if(levelPayment > 0){
            admin.transfer(levelPayment);
            emit LevelProfit(admin, msg.sender, _level, levelPayment);
        }
    }
    ...
    function getUserReferrals(address _user) internal view returns (address[] memory)
    {
        return users[_user].referrals;
    }

    function getUserReferralsID(address _user) internal view returns (uint256[2] memory)
    {
    ...
    function getUserBranch(address _user) internal view returns(Level[4] memory){
        ...
        for (uint256 k = 0; k < 4; k++) {
            ...
            for (uint256 j = 0; j < branch[k].referrals.length; j++) {
                last_id = last_id + 1;

                branch[index + j] = Level({
                    user: branch[k].referrals[j],
                    id: last_id,
                    parent_id: branch[k].id,
                    referrals: getUserReferrals(branch[k].referrals[j]),
                    referrals_id: getUserReferralsID(branch[k].referrals[j])
                });
            }
        }
    ...
    function getUserData(address _user, bool _branch) public view
        returns (
            uint256,
            uint256,
            address[] memory,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < MAX_LEVEL; i++) {
            levelExpires[i] = user.levelExpiresAt[i];
        }

        Level[4] memory branch;

        if(_branch){
         branch = getUserBranch(_user);
        }
