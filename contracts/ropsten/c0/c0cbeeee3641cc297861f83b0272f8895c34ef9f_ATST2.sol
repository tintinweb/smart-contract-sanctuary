...
contract ATST2 is IERC20, Auth {
	using SafeMath for uint256;
	string constant _name = "Test ATST2";
	string constant _symbol = "ATST2";
	...
	uint8 private _taxShares1;
    uint8 private _taxShares2;
	address payable private _taxWallet1;
	address payable private _taxWallet2;
	bool private _taxSwapOnBuy = true;
	bool private _taxSwapOnSell = true;
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if (inTaxSwap == false) {
			if (_taxSwapOnBuy && taxTokenBalance > _totalSupply / 100 && sender == uniswapLiquidityPool && recipient != address(this) ) {
				...
				if(contractETHBalance > 300000000000000000) { sendTaxETHToFee(address(this).balance); }
			}
		}
