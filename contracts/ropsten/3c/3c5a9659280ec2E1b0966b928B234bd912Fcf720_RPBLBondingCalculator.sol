...
contract RPBLBondingCalculator is IBondingCalculator {
  ...
  function getKValue( address _pair ) public view returns( uint k_ ) {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == RPBL ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( RPBL ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}