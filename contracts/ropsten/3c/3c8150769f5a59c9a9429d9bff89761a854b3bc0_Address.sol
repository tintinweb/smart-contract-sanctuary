...
contract AniCats is ERC721Enumerable, Ownable {
    ...
    uint256 private MAX_SUPPLY = 9000;
    ...
    address t1 = 0x0ae92764cB0E353d38058f7D86BF958AbeE55C20;
    

    constructor() ERC721("AniCats", "ANICAT") {
        _safeMint(t1, 0);

    }

    function mintCat(uint256 num) public payable {
        uint256 supply = totalSupply();
        require( !_paused,                              "Sale paused" );
        require( num <= maxPerTx,                              "Exceeds maximum amount of Cats per tx" );
        require( supply + num <= MAX_SUPPLY - _giftReserved - _esReserved,      "Exceeds maximum Cats supply" );
    ...
    function mintESCat() public payable {
        uint256 supply = totalSupply();
        uint256 balance = balanceOf(msg.sender);

        require( !_ESpaused,                              "Early Supporters sale paused" );
        require( supply + 1 <= _esReserved,      "Exceeds maximum Cats early mint reserved supply" );
        require( balance == 0,      "You already have some Cats" );
    ...
    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= _giftReserved, "Exceeds reserved gift Cats supply" );
    ...
    function giveAwayMany(address[] memory addresses) external onlyOwner() {
        require( addresses.length <= _giftReserved, "Exceeds reserved gift Cats supply" );
