...
contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
		//allow to effectively renounce ownership AND fee collector as well
...
contract DeflationaryERC20 is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
		...
		if (uniswapPair == address(0)) //feeless transfer before pool initialization
		{	//send full amount
			...
			// 2: subtract owner fee, keep amount for further fees to be subtracted later
    ...
    function swapBufferTokens() private {
		...
		if (contractTokenBalance <= _totalSupply.div(1e5)) //only swap reasonable amounts
			return;
...
contract OfficialTestToken is DeflationaryERC20 {
    constructor()  DeflationaryERC20("OfficialTestToken1", "OTT1", 0) {
