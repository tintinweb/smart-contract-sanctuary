...
contract Game is ExchangeAdmin {
    ...
    function dish(string winner, address profit) public onlyAdmin payable returns (address[10], uint[10]) {
        ...
        for (uint i = 0; i<sideData[StringYokes.zint_convert(winner)].usedAddresses.length; i++) {
            ...
            uint winAddition = (970*1000*contribAmount*totalGameContrib*(totalGameContrib-totalSideContrib))/(1000000*totalSideContrib);
            // Returns bet if won
...
contract BEthy is ExchangeAdmin {
    ...
    function endGame(uint gameId, string winningSide) public onlyAdmin returns (address[10], uint[10]) {
        //current[gameId].value(2);
        //
        require(current[gameId].isNotExpired());
        return current[gameId].dish(winningSide, owner);
    }
    /*function endGame(uint gameId, uint winningId, address profit) public onlyAdmin returns (string) {
        endGame(gameId, current[gameId].getStrFromId(winningId));
    }*/
}