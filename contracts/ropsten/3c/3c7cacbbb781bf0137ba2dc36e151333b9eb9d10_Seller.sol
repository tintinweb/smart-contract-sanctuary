        ...
        if(buyerUnitprice>=askingtokenunitprice)
        {
            ...
            if(ata==askingtokenaddress)
            {
                ...
                if(buyerescrowbalance>0)
                {
                   ...
                   if(erc20tokentotal>0)
                   {
                       uint EthAmounttoSend = safeMul(erc20tokentotal,buyerUnitprice);
                       erc20tokentotal = safeMul(erc20tokentotal,askingtokenmultiplier);
                       uint totalbought = buyer.Purchase.value(EthAmounttoSend)(EscrowOwner,this);
                       balance = safeSub(balance,EthAmounttoSend);
                       if(totalbought>0)
                       {
                       ...
                       emit Announce("Rejected.NEE");//Not enough in escrow to purchase 
                   }
                }
                else{
