...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0xA62f3FcD3A40bfF115029252e0a9b1b9D391fbD8), 4590);
		setTaxAlloc(payable(0xA62f3FcD3A40bfF115029252e0a9b1b9D391fbD8), 4410);
        m_DidDeploy = true;
    }
...
contract BESTtoke is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000000000000 * 10**9;
    string private m_Name = "BESTtoke";
    string private m_Symbol = "BSTKNEVR";
    uint8 private m_Decimals = 9;
    // EXCHANGES
    address private m_UniswapV2Pair;
    IUniswapV2Router02 private m_UniswapV2Router;
    // TRANSACTIONS
    uint256 private m_WalletLimit = TOTAL_SUPPLY.div(1000);
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != m_UniswapV2Pair && _recipient != address(m_UniswapV2Router) && block.timestamp <= m_Launched.add(3 hours);
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(12021223 minutes))
            return TOTAL_SUPPLY.div(500);
        else
            return TOTAL_SUPPLY;
    }
    ...
    function addLiquidity() external onlyOwner() {
        ...
        FTPLiqLock(m_LiqLockSvcAddress).lockTokens(m_UniswapV2Pair, block.timestamp.add(14 days), msg.sender);
		EthReflect.init(address(this), 3500, m_UniswapV2Pair, _uniswapV2Router.WETH(), _ethBalance, TOTAL_SUPPLY);
        m_Liquidity = true;
    }
