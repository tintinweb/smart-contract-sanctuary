...
contract BasicToken is ERC20 {
    ...
    modifier nonZeroEth(uint256 _value) {
        require(_value > 0);
        _;
    }
    ...
    function transferFrom(
        ...
        address _to,
        uint256 _value
    ) public nonZeroEth(_value) onlyPayloadSize returns (bool) {
        if (
...
contract Seedify is BasicToken, Ownable {
  ...
  uint256 public totalSupply ; // total supply of SDF Tokens
 ...
 // get the two owner on the run time
  constructor(string _name,string _symbol,uint256 _totalSupply,address _owner2,address _owner3,address _owner4 ) public {
  ...
  function startTokenSale(uint8 _days) public onlyOwner returns(bool) {
  ...
  function calcToken(uint256 value_At_least_1ETH)internal view returns(uint256 amount) {
 ...
 // function to transfer the funds to founders account
  function fundTransfer(uint256 weiAmount) internal {
      uint dividedAmount1 = weiAmount*50 / 100 ;
      uint dividedAmount2 = weiAmount*25 / 100 ;
      uint dividedAmount3 = weiAmount*15 / 100 ;
      uint dividedAmount4 = weiAmount*10 / 100 ;
 ...
 //for white listing the customer tier1
  ...
  function firstTierWhitelistAddr(address Cust_Addr) public {
      require(msg.sender == owner, "Only owner has the right to perform this action");
      addrToWhitelist1 = Cust_Addr;
  }
  function readFirstTierAddr(address _Address) public view returns(bool){
  ...
  function secondTierWhitelistAddr(address Cust_Addr) public {
      require(msg.sender == owner, "Only owner has the right to perform this action");
      addrToWhitelist2 = Cust_Addr;
  }
  function readSecondTierAddr(address _Address) public view returns(bool){
 ...
 //for white listing the customer tier3
  ...
  function thirdTierWhitelistAddr(address Cust_Addr) public {
      require(msg.sender == owner, "Only owner has the right to perform this action");
      addrToWhitelist3 = Cust_Addr;
  }
  function readThirdTierAddr(address _Address) public view returns(bool){
  ...
  function buyTokens(address beneficiary) public payable returns(bool) {
      ...
      if (msg.value <= 1000000000000000000 ) {
          require(msg.sender == addrToWhitelist1);

          fundTransfer(msg.value) ;
          
          
     } else if (msg.value >= 1000000000000000000  &&  msg.value <= 3000000000000000000   ) {
          require(msg.sender == addrToWhitelist2);
     
          fundTransfer(msg.value) ;
          return true;

    
     }else if (msg.value >= 3000000000000000000  &&  msg.value <= 5000000000000000000   ) {
          require(msg.sender == addrToWhitelist3);
