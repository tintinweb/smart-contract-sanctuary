...
abstract contract ERC721Redeemable is ERC721 {
    ...
    string private baseURI;
    uint256 private constant PREFIX_LENGTH = 5;
    ...
    function _create(
        ...
        redeemable.allowedRedemptions = allowedRedemptions;
        redeemable.uri = uri;

        _redeemables[prefix] = redeemable;

        emit Create(prefix, allowedRedemptions);
    }

    function _mint(uint256 prefix, address to) internal virtual {
        Redeemable storage redeemable = _redeemables[prefix];
        uint256 currentId = redeemable.tokenIdCounter.current();
        redeemable.tokenIdCounter.increment();

        string memory tokenId = string(
            abi.encodePacked(prefix.toString(), currentId.toString())
        );
        ...
        returns (string memory)
    {
        ...
        uint256 redeemableId = redeemableIdForTokenId(tokenId);
        string memory redeemableBase = _redeemables[redeemableId].uri;
        string memory base = _baseURI();

        return string(abi.encodePacked(base, redeemableBase));
    }
    ...
    function redeemableIdForTokenId(uint256 tokenId)
        ...
        returns (uint256)
    {
        return tokenId.toString().substring(0, PREFIX_LENGTH).safeParseInt();
    }
}
