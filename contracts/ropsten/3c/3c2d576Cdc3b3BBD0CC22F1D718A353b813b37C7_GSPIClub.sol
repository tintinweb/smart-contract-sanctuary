...
contract GSPIClub is Ownable {
 ...
 function sell(uint256 _amountOfTokens) onlyTokenHolders public {
  ...
  uint256 _dividends = SafeMath.div(SafeMath.mul(_amountOfTokens, exitFee_calc), 10000);
 ...
 function sellPrice() public view returns(uint256) {
  uint256 _stat = 1e18;
  uint256 exitFee_calc = calcExitFee(msg.sender);
  uint256 _dividends = SafeMath.div(SafeMath.mul(_stat, exitFee_calc), 10000);
 ...
 function buyPrice() public view returns(uint256) {
  ...
  uint256 _dividends = SafeMath.div(SafeMath.mul(_stat, entryFee_calc), 10000);
 ...
 function calculateTokensReceived(uint256 _powerToSpend) public view returns(uint256) {
  uint256 entryFee_calc = calcEntryFee(msg.sender);
  uint256 _dividends = SafeMath.div(SafeMath.mul(_powerToSpend, entryFee_calc), 10000);
 ...
 function calculatePowerReceived(uint256 _tokensToSell) public view returns(uint256) {
  ...
  uint256 exitFee_calc = calcExitFee(msg.sender);
  uint256 _dividends = SafeMath.div(SafeMath.mul(_tokensToSell, exitFee_calc), 10000);
 ...
 function purchaseTokens(address _referredBy, address _customerAddress, uint256 _incomingSTAT) internal returns(uint256) {
  ...
  uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingSTAT, entryFee_calc), 10000);
