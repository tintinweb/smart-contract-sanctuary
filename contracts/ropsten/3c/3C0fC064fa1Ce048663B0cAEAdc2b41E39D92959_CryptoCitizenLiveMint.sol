...
{   
    ...
    bytes32 public immutable _MINTER_ROLE = keccak256("MINTER_ROLE");                                          // Minter Role
    bytes32 public immutable _DEACTIVATED_ROLE = keccak256("DELISTED_ROLE");                                   // Deactivated Role
    ...
    /**
     * @dev Mapping ticketTokenID => address of redeemer
     **/
    mapping (uint256 => address) public mintingWhitelist;

    /**
     * @dev Mapping ticketTokenID => bool
     **/
    ...
    /**
     * @dev Mapping bookingSlot => amount of bookings 
     **/
    ...
    /**
     * @dev Mapping address => booking slot 
     **/
    mapping (address => uint256) private userSelectedSlot;
    ...
    /**
     * @dev Emitted When `redeemer` Changes Booked Time-Slot To `slot`.
     **/
    ...
    function ChangeTimeslot(uint256 ticketTokenID, uint256 newSlot) public nonReentrant whenNotPaused
    {
        ...
        uint256 oldSlot = userSelectedSlot[msg.sender];
        bookings[oldSlot] += 1;
        userSelectedSlot[msg.sender] = newSlot;
        bookings[newSlot] -= 1;
    ...
    /**
     * @dev Mints CryptoCitizen NFT & Returns `citizenTokenID`
     **/
    function mintArtBlocks() private returns (uint citizenTokenID) { return IArtBlocks(_ArtBlocksMintingContractAddress).purchase(_ArtBlocksProjectID); }

    /**
     * @dev Sends Crypto Citizen Corresponding to the `tokenID` to the `to` Address Specified
     **/
