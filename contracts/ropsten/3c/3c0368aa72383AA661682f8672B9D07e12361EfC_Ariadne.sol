...
contract ExtranetToken is ERC20, Ownable, SignatureValidator {
    ...
    mapping (bytes32 => bool) public mintTransactions;
    ...
    function mint(uint256 aFarmTokenAmount, bytes32 homenetTx) public onlyOwner {
        require(!mintTransactions[homenetTx], "tx already minted");
        mintTransactions[homenetTx] = true;
    ...
    function sell(uint256 aFarmTokenAmount, uint256 quoteTokenAmount, uint32 deadline, bytes calldata signature)
        public
        validSignature(signature, abi.encodePacked(aFarmTokenAmount, quoteTokenAmount, deadline))
    {
        require(!paused, "paused");
        ...
        require(aFarmTokenAmount <= balanceOf(custodian) / 2 - 1, "purchase less than half");

        IERC20(quoteToken).safeTransferFrom(msg.sender, custodian, quoteTokenAmount);
        _transfer(custodian, msg.sender, aFarmTokenAmount);
    }

    function purchase(uint256 aFarmTokenAmount, uint256 quoteTokenAmount, uint32 deadline, bytes calldata signature)
        public
        validSignature(signature, abi.encodePacked(aFarmTokenAmount, quoteTokenAmount, deadline))
    {
        require(block.timestamp < deadline, "expired");
        ...
        IERC20(quoteToken).safeTransferFrom(custodian, msg.sender, quoteTokenAmount);
    }

    function availableSupply() public view returns (uint256) {
        return balanceOf(custodian);
    }
...
abstract contract SignatureValidator is Ownable {
    modifier validSignature(bytes calldata signature, bytes memory packed) {
        bytes32 hash = keccak256(packed);
        address recovered = recoverFromStringSignature(toEthSignedMessageHash(hash), signature);

        require(recovered == owner(), "signature");
...
// FIXME burn the rest of LP tokens left after all liquidity withdrawn?

uint256 constant DIVISION_PRECISION = 10**18;

/**
 * @dev Main class for Uniswap-like farms. It is an ERC20 token itself.
 */
abstract contract AFarmUniswapBase is ERC20, Ownable {
    ...
    address public immutable quoteToken;
    address public immutable correspondentToken;
    ...
    /** @dev address collecting protocol fees and dust after each runInvestmentQueue/runWithdrawQueue */
    address public feeTo;

    /** @dev hash with enumeration, basically */
    mapping (address => uint256) public pendingInvestmentAmount;
    address[] public pendingInvestmentAddresses;
    ...
    uint256 public minSingleWithdrawAFarmTokenAmount = 0;
    ...
    // These are internal because external contracts and code should not
    // use any notion of what is "main" and "secondary" for AFarmUniswapBase
    address internal mainToken;
    address internal secondaryToken;


    address[] internal swapPathQuoteTokenToMainToken;
    address[] internal swapPathMainTokenToQuoteToken;
    ...
    uint8 internal lpTokenMultiplier = 1;
     ...
     * In case no direct path exists to swap `quoteToken`s for `mainTokens`, then also `correspondentTokenAddress` is supplied
     * so that `quoteToken`s are first swapped to `correspondentToken`s and then to `mainToken`s.
     *
     * - `routerAddress` must be swap's UniswapRouter02 address
     * - `quoteTokenAddress` what token do we sell for, like USDT
     * - `mainTokenAddress` main token address in pair we're creating
     * - `quoteTokenAddress` the other token address in pair we're creating
     * - `correspondentTokenAddress` token thru which we swap quote amount, can be zero
     * - `name` farm's ERC20 name
     * - `symbol` farm's ERC20 symbol
     */
    constructor(
        ...
        address correspondentTokenAddress,
        string memory name,
        string memory symbol,
        uint8 _lpTokenMultiplier
    ) ERC20(
        name,
        symbol
    ) {
        ...
        correspondentToken = correspondentTokenAddress;
        quoteToken = quoteTokenAddress;
        feeTo = msg.sender;
        lpTokenMultiplier = _lpTokenMultiplier;
        ...
        // no correspondent token
        if (correspondentTokenAddress == address(0)) {
            // directly investing one of the tokens
            if (quoteTokenAddress == mainTokenAddress) {
                isQuoteTokenMain = true;

            // we will have to swap the quote token in full
            } else {
                isQuoteTokenMain = false; // explicitly

                swapPathQuoteTokenToMainToken = new address[](2);
                swapPathQuoteTokenToMainToken[0] = quoteTokenAddress;
                swapPathQuoteTokenToMainToken[1] = mainTokenAddress;

                swapPathMainTokenToQuoteToken = new address[](2);
                swapPathMainTokenToQuoteToken[0] = mainTokenAddress;
                swapPathMainTokenToQuoteToken[1] = quoteTokenAddress;
            }

        } else {
            swapPathQuoteTokenToMainToken = new address[](3);
            swapPathQuoteTokenToMainToken[0] = quoteTokenAddress;
            swapPathQuoteTokenToMainToken[1] = correspondentTokenAddress;
            swapPathQuoteTokenToMainToken[2] = mainTokenAddress;

            swapPathMainTokenToQuoteToken = new address[](3);
            swapPathMainTokenToQuoteToken[0] = mainTokenAddress;
            swapPathMainTokenToQuoteToken[1] = correspondentTokenAddress;
            swapPathMainTokenToQuoteToken[2] = quoteTokenAddress;
        }
    ...
    function invest(uint256 quoteTokenAmount) public {
        ...
        require(pendingInvestmentAddresses.length < maxQueueSize, "full");
        ...
        if (pendingInvestmentAmount[msg.sender] == 0) {
            pendingInvestmentAddresses.push(msg.sender);
        }

        pendingInvestmentAmount[msg.sender] += quoteTokenAmount;

        emit PendingInvest();
    }

    function getPendingInvestmentAddresses() public view returns (address[] memory) {
        return pendingInvestmentAddresses;
    }

    function cancelInvestment(uint256 atIndex) public {
        require(pendingInvestmentAddresses[atIndex] == msg.sender, "address != sender");

        uint256 quoteTokenRefundAmount = pendingInvestmentAmount[msg.sender];

        delete pendingInvestmentAmount[msg.sender];

        if (pendingInvestmentAddresses.length == 1) {
            delete pendingInvestmentAddresses;

        } else {
            pendingInvestmentAddresses[atIndex] = pendingInvestmentAddresses[pendingInvestmentAddresses.length-1];
            pendingInvestmentAddresses.pop();

            // FIXME make sure that the above deleting code is correct and delete the snippet below
            // for (uint i = atIndex; i<pendingInvestmentAddresses.length-1; i++) {
            //     pendingInvestmentAddresses[i] = pendingInvestmentAddresses[i+1];
            // }
            // pendingInvestmentAddresses.pop();
        }
    ...
    function cancelAllInvestments() public onlyOwner {
        ...
        require(pendingInvestmentAddresses.length > 0, "empty");

        for (uint i=0; i<pendingInvestmentAddresses.length; i++) {
            address account = pendingInvestmentAddresses[i];

            uint256 refundQuoteTokenAmount = pendingInvestmentAmount[account];
            delete pendingInvestmentAmount[account];
    ...
    function pendingInvestmentLength() public view returns (uint256) {
        return pendingInvestmentAddresses.length;
    }

    /******************** Investment Implementation Methods ********************/

    function swapQuoteTokensToMainTokens(uint256 quoteTokenAmount) internal {
        uint mainTokenAmountOut = getAmountOut(quoteTokenAmount, swapPathQuoteTokenToMainToken);

        IUniswapV2Router02(router).swapExactTokensForTokens(
            quoteTokenAmount,
            mainTokenAmountOut,
            swapPathQuoteTokenToMainToken,
            address(this),
    ...
    function mintInvestmentQueueTokens(uint256 quoteTokenBalance, uint256 liquidity) internal {
        uint256 multiplierP = liquidity * lpTokenMultiplier * DIVISION_PRECISION / quoteTokenBalance;

        for (uint256 i=0; i<pendingInvestmentAddresses.length; i++) {
            address account = pendingInvestmentAddresses[i];

            uint256 aFarmTokenAmountToMint = pendingInvestmentAmount[account] * multiplierP / DIVISION_PRECISION;
    ...
    function runInvestmentQueue() public {
        require(pendingInvestmentAddresses.length > 0, "empty");

        uint256 quoteTokenBalance = IERC20(quoteToken).balanceOf(address(this));
        require(quoteTokenBalance > 0, "balance <= 0");

        if (!isQuoteTokenMain) {
            swapQuoteTokensToMainTokens(quoteTokenBalance);
        }
        ...
        if (!isQuoteTokenMain) {
            assert(IERC20(quoteToken).balanceOf(address(this)) == 0);
        }

        assert(
            IERC20(mainToken).balanceOf(address(this)) == 0 ||
            IERC20(secondaryToken).balanceOf(address(this)) == 0
        );

        mintInvestmentQueueTokens(quoteTokenBalance, liquidity);
    ...
    function withdraw(uint256 aFarmTokenAmount) public {
        require(pendingWithdrawAddresses.length < maxQueueSize, "full");
        require(aFarmTokenAmount > 0, "aFarmTokenAmount <= 0");
        require(minSingleWithdrawAFarmTokenAmount == 0 || aFarmTokenAmount >= minSingleWithdrawAFarmTokenAmount, "too low");

        _transfer(msg.sender, address(this), aFarmTokenAmount);

        if (pendingWithdrawAmount[msg.sender] == 0) {
            pendingWithdrawAddresses.push(msg.sender);
        }

        pendingWithdrawAmount[msg.sender] += aFarmTokenAmount;

        emit PendingWithdraw();
    }

    function getPendingWithdrawAddresses() public view returns (address[] memory) {
        return pendingWithdrawAddresses;
    }

    function cancelWithdrawal(uint256 atIndex) public {
        require(pendingWithdrawAddresses[atIndex] == msg.sender, "address != sender");

        uint256 quoteTokenRefundAmount = pendingWithdrawAmount[msg.sender];

        delete pendingWithdrawAmount[msg.sender];

        if (pendingWithdrawAddresses.length == 1) {
            delete pendingWithdrawAddresses;

        } else {
            pendingWithdrawAddresses[atIndex] = pendingWithdrawAddresses[pendingWithdrawAddresses.length-1];
            pendingWithdrawAddresses.pop();
        }

        if (quoteTokenRefundAmount > 0) {
            _transfer(address(this), msg.sender, quoteTokenRefundAmount);
        }
    }

    function pendingWithdrawLength() public view returns (uint256) {
        return pendingWithdrawAddresses.length;
    }
    ...
    function swapMainTokensToQuoteTokens() internal {
        IUniswapV2Router02(router).swapExactTokensForTokens(
            IERC20(mainToken).balanceOf(address(this)),
            0, // FIXME oracle
            swapPathMainTokenToQuoteToken,
            address(this),
            block.timestamp + swapDeadlineSeconds
        );
    }
    ...
    function transferQuoteTokensToWithdrawingAccounts(uint256 aFarmTokenPendingWithdrawTotalAmount) internal {
        ...
        for (uint256 i=0; i<pendingWithdrawAddresses.length; i++) {
            address account = pendingWithdrawAddresses[i];

            uint256 multiplierP = pendingWithdrawAmount[account] * DIVISION_PRECISION / aFarmTokenPendingWithdrawTotalAmount;
            ...
            delete pendingWithdrawAmount[account];
        }
    ...
    function runWithdrawQueue() public {
        require(pendingWithdrawAddresses.length > 0, "empty");

        require(pendingInvestmentAddresses.length == 0, "investments");
        require(IERC20(quoteToken).balanceOf(address(this)) == 0, "quoteToken != 0");

        if (!isQuoteTokenMain) {
            require(IERC20(mainToken).balanceOf(address(this)) == 0, "mainToken != 0");
        }

        require(IERC20(secondaryToken).balanceOf(address(this)) == 0, "secondaryToken != 0");

        uint256 aFarmTokenPendingWithdrawTotalAmount = balanceOf(address(this));

        uint256 multiplierP = aFarmTokenPendingWithdrawTotalAmount * DIVISION_PRECISION / totalSupply();
        uint256 lpTokensToWithdraw = lpTokenBalance() * multiplierP / DIVISION_PRECISION;
        require(lpTokensToWithdraw > 0, "lp == 0");

        unstakeTokens(lpTokensToWithdraw); // FIXME require zero LP tokens on balance so that removed liquidity always == lpTokensToWithdraw?

        removeLiquidity(IERC20(pair).balanceOf(address(this))); // FIXME and then use no balance?

        swapSecondaryTokensToMainTokens();

        if (!isQuoteTokenMain) {
            swapMainTokensToQuoteTokens();
        }

        transferQuoteTokensToWithdrawingAccounts(aFarmTokenPendingWithdrawTotalAmount);

        // FIXME aFarm dust will be left...
        _burn(address(this), aFarmTokenPendingWithdrawTotalAmount);
    }
    ...
    function collectDust() public {
        require(pendingInvestmentAddresses.length == 0, "investments");
        require(pendingWithdrawAddresses.length == 0, "withdrawal");
    ...
    function _collectDust() private {
        ...
        if (correspondentToken != address(0)) {
            _collectDustFromToken(correspondentToken);
        }
    ...
    function _collectDustFromToken(address tokenAddress) private {
    ...
    function runInvestmentThenWithdrawQueue() public {
        runInvestmentQueue();
        runWithdrawQueue();
    }

    function queueInfo() public view returns (
        uint256 pendingWithdrawAddressesLength,
        uint256 aFarmTokenPendingWithdrawTotalAmount,

        uint256 pendingInvestmentAddressesLength,
        uint256 quoteTokenPendingInvestmentTotalAmount
    ) {
        pendingWithdrawAddressesLength = pendingWithdrawAddresses.length;
        aFarmTokenPendingWithdrawTotalAmount = balanceOf(address(this));

        pendingInvestmentAddressesLength = pendingInvestmentAddresses.length;
        quoteTokenPendingInvestmentTotalAmount = IERC20(quoteToken).balanceOf(address(this));
    }
    ...
    function calcMainTokenAmountForTokens(uint256 aFarmTokenAmount) public view returns (uint256) {
        if (aFarmTokenAmount == 0) {
            return 0;
        }


        (uint256 reserveMain,) = UniswapV2LibraryModified.getReserves(pairCodeHash, IUniswapV2Router02(router).factory(), mainToken, secondaryToken);

        uint256 aFarmShareMultiplierP = aFarmTokenAmount * DIVISION_PRECISION / totalSupply();
        uint256 lpTokensAmount = lpTokenBalance() * aFarmShareMultiplierP / DIVISION_PRECISION;

        uint256 lpTokensShareP = lpTokensAmount * DIVISION_PRECISION / IERC20(pair).totalSupply();
        return reserveMain * lpTokensShareP / DIVISION_PRECISION;
    }

    function quote(uint256 fromTokenAmount, address fromToken, address toToken) public view returns (uint256 toTokenAmount) {
        if (fromTokenAmount == 0) {
            return 0;
        }

        (uint256 reserveFrom, uint256 reserveTo) = UniswapV2LibraryModified.getReserves(pairCodeHash, IUniswapV2Router02(router).factory(), fromToken, toToken);

        return UniswapV2LibraryModified.quote(fromTokenAmount, reserveFrom, reserveTo);
    }

    function getPriceInQuoteToken(uint256 aFarmTokenAmount) public view returns (uint256) {
        if (aFarmTokenAmount == 0) {
            return 0;
        }

        uint256 mainTokenAmount = calcMainTokenAmountForTokens(aFarmTokenAmount);

        if (isQuoteTokenMain) {
            return mainTokenAmount * 2;
        }

        if (correspondentToken != address(0)) {
            uint256 correspondentTokenAmount = quote(mainTokenAmount * 2, mainToken, correspondentToken);
            return quote(correspondentTokenAmount, correspondentToken, quoteToken);
        }

        return quote(mainTokenAmount * 2, mainToken, quoteToken);
    }
    ...
    function setLimits(
        ...
        uint256 _minSingleWithdrawAFarmTokenAmount
    ) public onlyOwner {
        ...
        minSingleWithdrawAFarmTokenAmount = _minSingleWithdrawAFarmTokenAmount;
    }
    ...
    function _removeAllLiquidity() internal {
        uint256 totalTokens = IUniswapV2Pair(pair).balanceOf(address(this));
        if (totalTokens > 0) {
            removeLiquidity(totalTokens);
        }
    }

    function shutdown() public onlyOwner {
        require(paused, "must be paused");
        require(pendingInvestmentAddresses.length == 0, "investment");
        require(pendingWithdrawAddresses.length == 0, "withdraw");
...
contract AFarmUniswap is AFarmUniswapBase {
    constructor(
        ...
        string memory name,
        string memory symbol,
        uint8 lpTokenMultiplier
    ) AFarmUniswapBase(
        routerAddress,
        quoteTokenAddress,
        mainTokenAddress,
        secondaryTokenAddress,
        correspondentTokenAddress,
        name,
        symbol,
        lpTokenMultiplier
    ) {
...
interface AFarmUniswapLookupInterface {
    ...
    function getPriceInQuoteToken(uint256 aFarmTokenAmount) external view returns (uint256);
    function pendingInvestmentAmount(address account) external view returns (uint256);
    function pendingWithdrawAmount(address account) external view returns (uint256);
...
contract AFarmLookup is Ownable {
    function lookupFarm(address aFarmAddress) public view returns (
        ...
        lpTotalSupply = IUniswapV2Pair(pairAddress).totalSupply();
        lpTokenBalance = AFarmUniswapLookupInterface(aFarmAddress).lpTokenBalance();

        (uint112 _reserve0, uint112 _reserve1, ) = IUniswapV2Pair(pairAddress).getReserves();
        reserve0 = _reserve0;
        reserve1 = _reserve1;

        totalSupply = IERC20(aFarmAddress).totalSupply();
        totalValueInQuoteToken = AFarmUniswapLookupInterface(aFarmAddress).getPriceInQuoteToken(totalSupply);
    }

    function lookupAccount(address aFarmAddress, address account) public view returns (
        uint256 myAFarmContractBalance,
        uint256 myValueInQuoteToken,
        uint256 pendingInvestmentAmount,
        uint256 pendingWithdrawAmount
    ) {
        myAFarmContractBalance = IERC20(aFarmAddress).balanceOf(account);
        myValueInQuoteToken = AFarmUniswapLookupInterface(aFarmAddress).getPriceInQuoteToken(myAFarmContractBalance);
        pendingInvestmentAmount = AFarmUniswapLookupInterface(aFarmAddress).pendingInvestmentAmount(account);
        pendingWithdrawAmount = AFarmUniswapLookupInterface(aFarmAddress).pendingWithdrawAmount(account);
    }

    function getPricesInQuoteToken(address aFarmAddress, uint256[] memory prices) public view returns (uint256[] memory pricesInQuoteToken) {
        pricesInQuoteToken = new uint256[](prices.length);

        for (uint i=0; i<prices.length; i++) {
            pricesInQuoteToken[i] = AFarmUniswapLookupInterface(aFarmAddress).getPriceInQuoteToken(prices[i]);
        }
    }

    function lookupForExtranet(
        address aFarmAddress,
        uint256 extranetTotalSupply,
        uint256 extranetAvailableSupply
    ) public view returns (
        uint256 homenetTotalSupply,
        uint256 homenetTotalSupplyQuoteToken,
        uint256 extranetTotalSupplyQuoteToken,
        uint256 extranetAvailableSupplyQuoteToken
    ) {
        homenetTotalSupply = IERC20(aFarmAddress).totalSupply();
        homenetTotalSupplyQuoteToken = AFarmUniswapLookupInterface(aFarmAddress).getPriceInQuoteToken(homenetTotalSupply);

        extranetTotalSupplyQuoteToken = AFarmUniswapLookupInterface(aFarmAddress).getPriceInQuoteToken(extranetTotalSupply);
        extranetAvailableSupplyQuoteToken = AFarmUniswapLookupInterface(aFarmAddress).getPriceInQuoteToken(extranetAvailableSupply);
    }

    function lookupExtranet(address extranetToken) public view returns (
        uint256 totalSupply,
        uint256 availableSupply,
        uint256 availableLiquidityQuoteToken
    ) {
        totalSupply = IERC20(extranetToken).totalSupply();
        availableSupply = ExtranetTokenLookupInterface(extranetToken).availableSupply();
        ...
        availableLiquidityQuoteToken = IERC20(quoteToken).balanceOf(extranetTokenCustodian);
    }
...
contract Ariadne is ERC20 {
  constructor() ERC20("Test AR", "tttA") {
...
contract AFarmSushiswap is AFarmUniswapBase {
    ...
    address[] private swapPathSushiToMain;

    address public immutable sushiToken;
    address public immutable masterChef;

    uint256 public pairPid = 0;
    ...
    constructor(
        ...
        if (correspondentTokenAddress == address(0)) {
            swapPathSushiToMain = [_sushiToken, mainTokenAddress];
        } else {
            swapPathSushiToMain = [_sushiToken, correspondentTokenAddress, mainTokenAddress];
        }

        // FIXME move out swap paths out of contracts at all
    ...
    function stakeAllTokens() internal override {
        ...
        ISushiswapMasterChef(masterChef).deposit(pairPid, lpTokens);
    }

    function unstakeTokens(uint256 lpTokensToWithdraw) internal override {
        ISushiswapMasterChef(masterChef).withdraw(pairPid, lpTokensToWithdraw);
    }

    function lpTokenBalance() public view override returns (uint256) {
        (uint256 amount, ) = ISushiswapMasterChef(masterChef).userInfo(pairPid, address(this));
        return amount;
    }

    // FIXME wrong calc?
    // function pendingSushi() public view returns (uint256) {
    //     return ISushiswapMasterChef(masterChef).pendingSushi(pairPid, address(this));
    // }

    function harvest() public onlyOwner {
        ISushiswapMasterChef(masterChef).deposit(pairPid, 0);
    }
    ...
    function compound() public onlyOwner {
        ...
        IUniswapV2Router02(router).swapExactTokensForTokens(
            sushiTokenAmount,
            0, //FIXME calculate amountOutMin
            swapPathSushiToMain,
            address(this),
            block.timestamp + swapDeadlineSeconds
        );
