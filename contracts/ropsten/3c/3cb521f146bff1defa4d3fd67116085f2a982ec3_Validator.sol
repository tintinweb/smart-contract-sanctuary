...
contract Custodian is Ownable {
    ...
    function addAuthorized(address addr) public onlyOwner validAddress(addr) {
        require(_authorized[addr] == false, "Already authorized");
        _authorized[addr] = true;
    }

    function removeAuthorized(address addr) public onlyOwner validAddress(addr) {
        require(_authorized[addr], "Already de-authorized");
        _authorized[addr] = false;
    }

    function isAuthorized(address addr) public view validAddress(addr) returns (bool) {
...
abstract contract ValidatorOwner is Ownable {
    ...
    function whitelistAdd(address addr) public onlyOwner validAddress(addr) {
    ...
    function whitelistRemove(address addr) public onlyOwner validAddress(addr) {
    ...
    function setInflationRateBase(uint16 value) public onlyOwner {
        inflationRateBase = value;
        emit InflationRateBaseUpdated(value);
    }

    function setInflationRateTarget(uint16 value) public onlyOwner {
        inflationRateTarget = value;
        emit InflationRateTargetUpdated(value);
    }

    function setSlotsMax(uint256 value) public onlyOwner {
        slotsMax = value;
...
contract Validator is ValidatorOwner {
    ...
    event RewardWithdrawn(address addr);
    ...
    function stakeOf(address addr) public view validAddress(addr) returns (uint256) {
        if (whitelist[addr]) {
            return balances[addr] >= stakeMin ? (balances[addr] <= stakeMax ? balances[addr] : stakeMax) : 0;
        } else {
            return 0;
        }
    }
