...
contract CivInvestment is ICivInvestment
{
    ...
    //         uint128 tokensOwed1
    //     ) = v3Nft.positions(_id);
    //     return TradeInfo(_id, token0, 0, 0, 0);
    // }
    ...
    function civTradeList(
        ...
        tradeIDs = new uint256[](v3Nft.balanceOf(_owner));
        for (uint i = 0; i < tradeIDs.length; i++) {
            tradeIDs[i] = v3Nft.tokenOfOwnerByIndex(_owner, i);

            // trades[tradeIDs[i]] = this.getTradeInfo(tradeIDs[i]);
        }
        return tradeIDs;
    }
