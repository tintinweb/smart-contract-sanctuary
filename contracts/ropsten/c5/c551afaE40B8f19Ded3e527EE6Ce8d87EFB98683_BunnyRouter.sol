...
contract MangaNFT is ERC721Tradable {
    ...
    /// @dev Assigns a new address to act as the Router. Only available to the current Owner.
    ...
    function _createFadeAwayBunny(
        ...
        returns (uint256)
    {
        uint256 nextTokenId = mintTo(_owner);
    ...
    function contractURI() public pure returns (string memory) {
        return "https://api.trophee.co/api/contract/";
    }
}
...
contract BunnyRouter is Ownable {
    ...
    /// @dev Update the address of the manga nft contract, can only be called by the Owner.
    /// @param _address An address of a MangaNFT contract instance to be used from this point forward.
    function setMangaNFT(address _address) external onlyOwner {
