...
contract PhuketStakeContract is Ownable, Pausable {
    ...
    function earnedReward(uint256 _id, uint256 _amount) public view returns(uint256)
    {
        uint256 rewardAmount;
         if(stakerData[_id].endTime <= block.timestamp){
             uint256 exactDays = (stakerData[_id].exactdate).mul(ONE_DAY);
              rewardAmount = _calculateReward(_amount, ratio, exactDays);
         }
         else
         {
             uint256 days_remaining = (block.timestamp).sub(stakerData[_id].initiationTimestamp);
             rewardAmount = _calculateReward(_amount, ratio, days_remaining);
         }
         return rewardAmount;
    }
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < _expiredTimestamps.length; i = i.add(1)) {
                ...
                uint256 timestamp = block.timestamp;
                 ...
                 uint256 initiate_time = stakerData[_expiredTimestamps[i]].initiationTimestamp;
                  require(withdrawAmount >=_amount, "Amount Invalid");
                  require(stakerData[_expiredTimestamps[i]].withdrawable <= withdrawAmount,"Invalid operation");
                  
            if(stakerData[_expiredTimestamps[i]].endTime <= block.timestamp){
             uint256 exactDays = (stakerData[_expiredTimestamps[i]].exactdate).mul(ONE_DAY);
            ...
            else if(withdrawAmount > _amount){
               rewardAmount = _calculateReward(_amount, ratio, exactDays);
                IERC20(PhuketTokenAddress).transfer(msg.sender, rewardAmount);
            }
          ...
          stakerData[_expiredTimestamps[i]].withdrawable=(_amount).add(stakerData[_expiredTimestamps[i]].withdrawable);
          
            stakerBalance[msg.sender] = stakerBalance[msg.sender].sub(_amount);

        totalStakedTokens = totalStakedTokens.sub(_amount);

        IERC20(PhuketTokenAddress).transfer(msg.sender, _amount);
                    
            
        }
        else {
            uint256 days_remaining = timestamp.sub(initiate_time);
             rewardAmount = _calculateReward(_amount, ratio, days_remaining);
             IERC20(PhuketTokenAddress).transfer(msg.sender, rewardAmount);
             uint256 fee_amt = (_amount.mul(FEE)).div(10000);
             IERC20(PhuketTokenAddress).transfer(fee_address, fee_amt);
             uint256 _final_amount = _amount.sub(fee_amt);
        ...
        IERC20(PhuketTokenAddress).transfer(msg.sender, _final_amount);
        ...
        if(stakerData[_expiredTimestamps[i]].withdrawable == stakerData[_expiredTimestamps[i]].altQuantity)
        {
              stakerData[_expiredTimestamps[i]].isCompleted=true;
        }
    }
