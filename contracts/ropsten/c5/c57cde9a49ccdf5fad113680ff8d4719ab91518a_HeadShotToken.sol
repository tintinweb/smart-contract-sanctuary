...
contract HeadShotToken is Ownable, IERC20 {
    ...
    */
    ...
    /*
    HeadShotTicket public createTicket;
    HeadShotTicket public verifyTicket;
    HeadShotTicket public voteTicket;
    */
    constructor() {
        ...
        /*
        createTicket = new HeadShotTicket(_ticketCreateName, _ticketCreateSymbol);
        verifyTicket = new HeadShotTicket(_ticketVerifyName, _ticketVerifySymbol);
        voteTicket = new HeadShotTicket(_ticketVoteName, _ticketVoteSymbol);
        */
    ...
    function buyTicket(uint256 _mode, uint256 _count) public returns (bool) {
        if (_mode == 0){
            _transfer(_msgSender(), salesWallet, _count * _realPriceOfRegister);
            _createTicketBalances[_msgSender()] += _count;
            //createTicket.increaseBalance(payable(_msgSender()), _count * 10);
        } else if (_mode == 1){
            _transfer(_msgSender(), salesWallet, _count * _realPriceOfVerify);
            _verifyTicketBalances[_msgSender()] += _count;
            //verifyTicket.increaseBalance(payable(_msgSender()), _count * 10);
        } else if (_mode == 2) {
            _transfer(_msgSender(), salesWallet, _count * _realPriceOfVote);
            _voteTicketBalances[_msgSender()] += _count;
            //voteTicket.increaseBalance(payable(_msgSender()), _count * 10);
        }
        airdropVoteTicket(_msgSender());
        return true;
    }

    function useTicket(uint256 _mode, uint256 _count) public returns (bool) {
        if (_mode == 0){
            _transfer(_msgSender(), salesWallet, _count * _realPriceOfRegister);
            _createTicketBalances[_msgSender()] -= _count;
            //createTicket.decreaseBalance(payable(_msgSender()), _count * 10);
        } else if (_mode == 1){
            _transfer(_msgSender(), salesWallet, _count * _realPriceOfVerify);
            _verifyTicketBalances[_msgSender()] -= _count;
            //verifyTicket.decreaseBalance(payable(_msgSender()), _count * 10);
        } else if (_mode == 1){
            _transfer(_msgSender(), salesWallet, _count * _realPriceOfVote);
            _voteTicketBalances[_msgSender()] -= _count;
            //voteTicket.decreaseBalance(payable(_msgSender()), _count * 10);
        }
        return true;
    }
