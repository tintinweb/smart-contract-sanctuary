...
contract DateTime {
    struct _DateTime { uint16 year; uint8 month; uint8 day; }
...
contract WannaPanda is ERC721, Ownable, DateTime {
    using SafeMath for uint256;
    ...
    struct userMintInfo {
        uint16 year; uint8 month; uint8 day; uint minted;   
    }
    mapping(address => userMintInfo) userMints;
    event MintedIDs(uint[] array);
    constructor() ERC721("WannaPanda", "WP") {}
    function checkUserMinting(address userAddress, _DateTime memory dt) internal view returns(uint) {
        userMintInfo storage userMint = userMints[userAddress];
        if(dt.year == userMint.year && dt.month == userMint.month && dt.day == userMint.day) {
    ...
    function setUserMinting(address userAddress, uint minted, _DateTime memory dt) public onlyOwner {
        userMintInfo storage userMint = userMints[userAddress];
        userMint.minted = minted;
        userMint.year = dt.year;
        userMint.month = dt.month;
        userMint.day = dt.day;
    }
    ...
    function mintPandas(uint numberOfTokens, string memory revealingImage) public payable {
        ...
        _DateTime memory dateParts = parseTimestamp(block.timestamp);
        uint todayAlreadyMinted = checkUserMinting(msg.sender, dateParts);
        ...
        if(CHECK_WHITELISTING) {
            ...
            if(maxMintsAllowed == 10) {
                message = "Can not mint more than 10 tokens per day";
            }
            else if(maxMintsAllowed == 25) {
                message = "Can not mint more than 25 tokens per day";
            }
            require(numberOfTokens <= maxMintsAllowed, message);
            require(numberOfTokens.add(todayAlreadyMinted) <= maxMintsAllowed, message);
        }
        else {
        ...
        setUserMinting(msg.sender, numberOfTokens.add(todayAlreadyMinted), dateParts);
