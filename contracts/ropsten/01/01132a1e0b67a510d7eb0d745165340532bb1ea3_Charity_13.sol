...
contract Charity_13{
   ...
   /* function Withdraw(address _adr) internal {
    ...
    function CheckPersonsNeedisEnough() external  {
        require(Aperson[persons[0]].exists, "Nobody is exist");

        for(uint i=0; i< persons.length; i++){
                if(someval >= Aperson[persons[i]].needval){ 
                        emit CheckPersonsforWithdrawEvent(Aperson[persons[i]].adrs, Aperson[persons[i]].needval , Aperson[persons[i]].natcode);
                }else{
                    break;
                }
            }
