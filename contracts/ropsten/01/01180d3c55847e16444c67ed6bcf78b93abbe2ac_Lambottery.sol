...
abstract contract Pancake is Ownable {
    ...
    bool inSwap;
    modifier swapping() {
        require(swapEnabled, "Swapping feature is disabled");
        inSwap = true;
        _;
        inSwap = false;
    }
    ...
    function shouldSwapBack(uint256 balance) internal view returns (bool) {
     ...
     */
    function swapTokensForBnb(uint256 tokenAmount) internal swapping pcsInitialized returns(uint256) {
    ...
    function liquify(uint256 tokens, uint256 bnb) internal {
        if(tokens > 0 && bnb > 0){
...
abstract contract TxLimits is Ownable {
    ...
    uint256 public maxSellForWhale;
    uint256 public minBalanceForBeWhale;
    ...
    constructor(uint256 whaleMaxSell, uint256 whaleMinBalance) {
        setWhaleLimits(whaleMaxSell, whaleMinBalance);
...
abstract contract AutoBuyback is Pancake {
    ...
    uint256 autoBuybackBnbThreshold;
    ...
    function tryAutoBuyback(address to) internal returns(bool) {
        if(shouldAutoBuyback(to)) {
            triggerAutoBuyback();
            return true;
        }
        return false;
    ...
    function shouldAutoBuyback(address to) private view returns (bool) {
        return !liquidityPools[msg.sender]
        && liquidityPools[to]
        && !inSwap
        && autoBuybackEnabled
...
abstract contract LotteryHelper is Pancake {
    ...
    uint256 public lotterySwapThresholdInBNB = 1 ether / 1000; // 0.01 BNB
    ...
    // getters
    function getWinnerHourly(address account) external view returns(uint256 amount, uint256 time, address winner) {
        return hourly.getWinner(account);
    }

    function getWinnerDaily(address account) external view returns(uint256 amount, uint256 time, address winner) {
        return daily.getWinner(account);
    }
    ...
    function shouldSwapAndSendBUSDToLottery() private view returns (bool) {
        return !liquidityPools[msg.sender]
        && !inSwap
        && swapEnabled
    ...
    function trySwapAndSendBUSDToLottery() internal returns (bool) {
        if(shouldSwapAndSendBUSDToLottery()) {
    ...
    // settings
    // function setLotteryRewardToken(address tokenAddress) external onlyOwner {
    //     require(!liquidityPools[tokenAddress] && tokenAddress != WBNB, "This token address is not allowed");
    //     require(tokenAddress != address(RewardToken), "Same address as current");
    //     require(IUniswapV2Factory(router.factory()).getPair(tokenAddress, WBNB) != address(0), "There is no lp in current router");
    //     RewardToken = IBEP20(tokenAddress);
    //     hourly.setRewardToken(tokenAddress);
    //     daily.setRewardToken(tokenAddress);
    //     weekly.setRewardToken(tokenAddress);
    //     emit UpdateLotteryRewardToken(address(RewardToken), tokenAddress);
    // }
    ...
    function setLotteryDrawThreshold(uint256 _hourlyBUSDThreshold, uint256 _dailyBUSDThreshold) external onlyOwner {
    ...
    // event UpdateLotteryRewardToken(address oldToken, address newToken);
...
LotteryHelper {
    ...
    string constant _name = "Lambottery";
    string constant _symbol = "LAMBO";
    ...
    uint256 public separateBuybackFee = 0;
    uint256 public liquidityFee = 0;
    uint256 public lotteryFee = 2000;
    uint256 public marketingAndOperationsFee = 0;
    ...
    address public marketingAndOperationsFeeReceiver = 0xBC3C2C6e7BaAeB7C7EA2ad4B2Fa8681a91d47Ccd;
    address public weeklyLottoWallet = 0xBC3C2C6e7BaAeB7C7EA2ad4B2Fa8681a91d47Ccd;

    constructor (uint256 whaleMaxSell, uint256 whaleMinBalance, address _weeklyAddress, address _daily, address _hourly, address _rewardToken)
    TxLimits(whaleMaxSell, whaleMinBalance)
    LotteryHelper(_weeklyAddress, _daily, _hourly, _rewardToken) {
        ...
        initDEXRouter(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // testnet ropsten
        _approveMax(address(this), routerAddress);

        marketingAndOperationsFeeReceiver = msg.sender;

        // setSwapEnabledAndThreshold(false, 100 * (10 ** _decimals));
        // setAutoBuybackEnabledAndThreshold(false, 0);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        bool done;
        done = tryProcessHourlyLottery();

        if (!done) {
            done = tryProcessDailyLottery();
        }

        if (!done) {
            tryAutoBuyback(recipient);
            trySwapAndSendBUSDToLottery();
            if(shouldSwapBack(_balances[address(this)])){
    ...
    function swapBack(uint256 amount) internal swapping pcsInitialized {
        uint256 tokenAmountToLiquify = amount.mul(liquidityFee).div(2).div(totalFee);
        uint256 amountToSwap = amount.sub(tokenAmountToLiquify);

        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;

        uint256 amountBNB = swapTokensForBnb(amountToSwap);

        uint256 totalSwappedFee = totalFee.sub(liquidityFee.div(2));
        ...
        uint256 bnbLiquidity = amountBNB.mul(liquidityFee).div(2).div(totalSwappedFee);
        uint256 bnbLottery = amountBNB.mul(lotteryFee).div(2).div(totalSwappedFee);
        uint256 bnbMarketingAndOperations =
        amountBNB.sub(bnbBuyback).sub(bnbSeparateBuyback)
        .sub(bnbLiquidity).sub(bnbLottery);

        accumulatedBNBForBuyback = accumulatedBNBForBuyback.add(bnbBuyback);
        accumulatedBNBForLottery = accumulatedBNBForLottery.add(bnbLiquidity);
        ...
        if (bnbLiquidity > 0) {
            liquify(tokenAmountToLiquify, bnbLiquidity);
        }
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _separateBuybackFee, uint256 _marketingFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        marketingAndOperationsFee = _marketingFee;
        separateBuybackFee = _separateBuybackFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_separateBuybackFee).add(_marketingFee);
        require(totalFee < 5100);
    }

    function setFeeReceivers(address _marketingFeeReceiver, address _weeklyLottoWallet, address _separateBuybackReceiver) external onlyOwner {
        marketingAndOperationsFeeReceiver = _marketingFeeReceiver;
        weeklyLottoWallet = _weeklyLottoWallet;
