...
contract Edworm is Ownable, ERC721Enumerable {
	...
	function transfer(
		...
		require(
			// require that the address isnt zero — not strictly necessary, but:
	...
	function getArt(uint256 _tokenId) public view returns (string memory) {
		...
		for (uint i = 0; i < layers.length; i++) {
			if (layers[i] == bytes8('token_id')) {
				acc = strConcat(acc, _tokenId.toString());
			} else {
				acc = strConcat(acc, svg[layers[i]]);
			}
		}
	...
	function getMeta(uint256 _tokenId) public view returns (string memory) {
		...
		if (_tokenId == 0) {
			...
			name = strConcat('The Worm Hologram #', _tokenId.toString());
			desc = meta['descHolo'];
		}
