...
/**
 * This is the same as the openzeppelin implementation, the only difference is that
 * we make _balances and totalSupply internal
 */
...
interface IERC20Upgradable is IERC20 {
    // We need these functions to pass the original sender. If we don't do this,
    // msg.sender will be the old contract.
    // YOU NEED TO VERIFY AND CONFIRM THESE FUNCTIONS CAN ONLY BE CALLED BY THE OLD CONTRACT
 ...
 */
contract ERC20 is Upgradable, Context, IERC20Upgradable, IERC20Metadata {
    ...
    function totalSupply() external view virtual override returns (uint256) {
        if (deprecated) {
            return IERC20Upgradable(upgradedAddress).totalSupply();
        }
        return _totalSupply;
    }
    ...
    {
        if (deprecated) {
            return IERC20Upgradable(upgradedAddress).balanceOf(account);
        }
        ...
        returns (bool)
    {
        if (deprecated) {
            return
                IERC20Upgradable(upgradedAddress).transferLegacy(
                    _msgSender(),
                    recipient,
                    amount
                );
    ...
    {
        if (deprecated) {
            return IERC20Upgradable(upgradedAddress).allowance(owner, spender);
        }
        ...
        returns (bool)
    {
        if (deprecated) {
            return
                IERC20Upgradable(upgradedAddress).approveLegacy(
                    _msgSender(),
                    spender,
                    amount
                );
    ...
    function transferFrom(
        ...
        if (deprecated) {
            return
                IERC20Upgradable(upgradedAddress).transferFromLegacy(
                    _msgSender(),
                    sender,
                    recipient,
                    amount
        ...
        returns (bool)
    {
        if (deprecated) {
            return
                IERC20Upgradable(upgradedAddress).increaseAllowanceLegacy(
                    _msgSender(),
                    spender,
                    addedValue
                );
        ...
        returns (bool)
    {
        if (deprecated) {
            return
                IERC20Upgradable(upgradedAddress).decreaseAllowanceLegacy(
                    _msgSender(),
                    spender,
                    subtractedValue
                );
...
contract FeeCalculator is IFeeCalculator {
    ...
    function calculate(uint256 amount)
        ...
        returns (uint256)
    {
        uint256 fee = amount.mul(_percentageRate).div(100);
        if (fee > _maximumFee) {
            fee = _maximumFee;
        }
        return fee;
