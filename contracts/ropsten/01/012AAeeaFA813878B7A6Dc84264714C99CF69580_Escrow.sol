...
contract Escrow {
    ...
    function check_parties_exist() public {
        if(seller != address(0) && buyer != address(0)) {
            ...
            emit notify("Both parties have been set within the contract");
        } else {
            emit notify("Waiting on all parties to be defined");
        }
    }
    
    // update the product details
    function b_Product_details(address payable _seller, address payable _buyer, string memory _b_name,uint units,string memory _date,uint p_p_u) public
    {
        buyer = _buyer;
        seller = _seller;
