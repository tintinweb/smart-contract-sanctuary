...
contract Evaluation {
     ...
     * Evaluator functions
     */
    function registerSelfEvaluator(uint _id, string memory _email) external {
    ...
    function validateEvaluator() external view returns (uint _level, uint _id, string memory _email) {
        require(evaluators[msg.sender].level > 0, "Invalid Evaluator");
        return (evaluators[msg.sender].level, evaluators[msg.sender].id, evaluators[msg.sender].email);
    }

    function registerEvaluator(address _address, uint _id, uint _level, string memory _email) external isModerator {
        // max level
    ...
    function changeEvaluatorLevel(address _address, uint _level) external {
        // max level
        uint max = evaluators[msg.sender].level;
