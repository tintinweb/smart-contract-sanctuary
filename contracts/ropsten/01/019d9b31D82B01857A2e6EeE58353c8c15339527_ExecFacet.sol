...
/// @author hilmarx
contract ExecFacet is FacetBase {
    ...
    function exec(address _service, bytes calldata _data) external {
        require(
            canExecutorExec(_service, msg.sender),
            "ExecFacet exec: PreExec Revert"
        );
    ...
    function addExecutor(address _executor) external onlyOwner {
        ...
        require(
            !es.executors.contains(_executor),
            "ExecFacet: Executor already whitelisted"
        );
        es.executors.add(_executor);
    }
    ...
    function removeExecutor(address _executor) external onlyOwner {
        ...
        require(
            es.executors.contains(_executor),
            "ExecFacet: Executor not whitelisted"
        );
        es.executors.remove(_executor);
    }

    // ################ View funds ################
    ///@dev change to querien the Task Registry Storage
        ...
        returns (bool)
    {
        ...
        return
            srs.executorPerUseCase[_service].contains(_executor) &&
    ...
    function isExecutor(address _executor) public view returns (bool) {
        LibExecutor.ExecutorStorage storage es = LibExecutor.executorStorage();
        return es.executors.contains(_executor);
    }
}
