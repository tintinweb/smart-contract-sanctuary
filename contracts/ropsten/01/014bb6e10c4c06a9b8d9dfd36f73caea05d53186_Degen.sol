...
abstract contract Router {
    ...
    uint8 public constant totalDEX = 3;            // Total no of DEX aggregators or exchanges used
    ...
    constructor(address _Uni, address _sushi, address _oneSplit/* address _xToken, address _airSwap, address _paraSwap */) payable {
...
contract Degen is Router, Ownable {
   ...
   modifier onlySystem() {
    ...
    function executeSwap(
        ...
        _swap(orderType, path, assetInOffered, minExpectedAmount, msg.sender, msg.sender, dexId, distribution);
   
        processFee(gasleft(), receivedFees, licenseeVault, msg.sender);
    ...
    function executeCrossExchange(
        ...
        if(orderType == OrderType.EthForTokens){
            require(msg.value >= (assetInOffered + fees + proccessingFee), "Payment = assetInOffered + price[0] + proccessingFee");
            receivedFees = receivedFees + msg.value - assetInOffered;
        } else {
        ...
        if(path[0] == USDT) {
            ...
            uint256 minAmountExpected = _swap(orderType, path, assetInOffered, minExpectedAmount, msg.sender, address(this), dexId, distribution);
    ...
    function callbackCrossExchange( 
        ...
        _swap(orderType,path,assetInOffered,uint256(0),user,user, dexId, distribution);
        return true;
    }
