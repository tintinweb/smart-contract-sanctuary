...
//      ///////////////////////////  Functions (Write) ///////////////////
    ...
    function deposit_to_contract() public inState(State.contract_created) {
        ...
        if(msg.sender == buyer) {
            ...
            depositors_balances[msg.sender].add(buyer_share);
            ...
            depositors_balances[msg.sender].add(seller_share);
        ...
        if(
            ...
            (Token.balanceOf(address(this)) >= SafeMath.add(seller_share , buyer_share))
       ){
            //platform_fee = platform_commission_percent * total_amount / PERCENTAGE_RATE;
            require(Token.transfer(platform, platform_fee), "Deposit_to_contract: Token broker transfer failed!");
    ...
    function inspector_fee_payment (uint _inspector_fee) public only_seller inState(State.inspector_assigned) {
    ...
    function assigning_inspector (address _inspector) public only_seller inState(State.payment_done) {
    ...
    function forwarder_fee_payment (uint _forwarder_fee) public only_buyer inState(State.forwarder_assigned) {
    ...
    function assigning_forwarder (address _forwarder) public only_buyer inState(State.inspector_assigned) {
