...
contract BabyGiraffe is ERC721Enumerable, Ownable {
    ...
    address public gleafAddress = 0x08E33377D7D523884851f1486dBEc99a8e6D93d0;
    address nullAddress = 0x0000000000000000000000000000000000000000;
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
    address private ownerAddress;
    string _currentBaseURI = "http://api.giraffetowernft.com/babies/";
    ...
    constructor() ERC721("BabyGiraffe", "BABYGIRAFFE") {
    ...
    function setGiraffeName(uint256 tokenId, string memory name) public {
        ...
        IERC20(gleafAddress).transferFrom(msg.sender, address(this), nameChangePrice);
        if (pr != address(this)) {
    ...
    function initiateBreeding(
        ...
        uint256 incubation_period;
        uint256 wait_period;
        if (_parentIdowner[0] != nullAddress && _parentIdowner[1] != nullAddress) {
        ...
        if (_parentIdrentfee[0] > 0) {
            IERC20(gleafAddress).transfer(_parentIdowner[0], _parentIdrentfee[0]);
        }
        if (_parentIdrentfee[1] > 0) {
            IERC20(gleafAddress).transfer(_parentIdowner[1], _parentIdrentfee[1]);
        }
    ...
    function processBreeding(uint256 collection_id,uint256[] memory parent_ids, uint256[] memory others) internal{
         if (
