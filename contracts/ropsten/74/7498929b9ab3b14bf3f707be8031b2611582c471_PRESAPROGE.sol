...
contract PRESAPROGE is Context, IBEP20, Ownable {
    ...
    address payable rogeTreasury; // 0x2ADf3ddd9A238E8c0afea647137aE4F534405388;
    address public ROGE = 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984; //ropsten UNI 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984  eth roge 0x45734927fa2f616fbe19e65f42a0ef3d37d1c80a
    address public animalSanctuary = 0x50D52032bC4eeBa4D415A52d12F80c40FaEA68Aa; //ropsten 0x50D52032bC4eeBa4D415A52d12F80c40FaEA68Aa eth animalSanctuary 0x4A462404ca4b7caE9F639732EB4DaB75d6E88d19
    ...
    constructor (address payable _presa, address payable _rogeTreasury)  {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // eth router  0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D bsc pancakev2 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient == address(uniswapV2Pair) || recipient == address(uniswapV2Router) ) { 
            ...
            uint sinceLaunch = nowTime.sub(firstBlock);                   
            if(sinceLaunch <= 7 days) {
                feeFactor = 2;
            }
        }

        bool feeOnTransfer; 
        if(sender == address(this) || sender == owner() || sender == address(rogeTreasury) || recipient == address(rogeTreasury) ) {
            feeOnTransfer = false;
        } else {
            feeOnTransfer = true;
        }
        ...
        if (feeOnTransfer) {            
            maxTxAmount = _rTotal.sub(balanceOf(animalSanctuary)).div(100);
            require(amount <= maxTxAmount, "Cannot exceed 1% of Circ Supply");

            uint contractTokenBalance = balanceOf(address(this));
            bool overMinTokenBalance = (contractTokenBalance >= _tokensAmountToLiquify);
            uint contractETHBalance = address(this).balance;

            uint halfMaxTxAmount = maxTxAmount.div(2);
            if(contractTokenBalance > halfMaxTxAmount) {
                contractTokenBalance = halfMaxTxAmount;
            }
            ...
            if (_isExcluded[sender] && !_isExcluded[recipient]) {
                ...
                _transferStandard(sender, recipient, amount, feeFactor);
            }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, uint feeFactor) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount, feeFactor);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount, uint feeFactor) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee) = _getTValues(tAmount, feeFactor);
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee);
    }

    function _getTValues(uint256 tAmount, uint feeFactor) private pure returns (uint256, uint256) {
        uint256 tFee = tAmount.div(100).mul(2).mul(feeFactor);
