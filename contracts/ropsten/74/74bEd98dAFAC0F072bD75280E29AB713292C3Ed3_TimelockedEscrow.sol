 ...
 * tokens after a given release time. The payer can recover back the tokens
...
contract TimelockedEscrow {
    ...
    constructor(
        ...
        require(
            ...
            "TimelockedEscrow: release time is not after current time"
        );
        // solhint-disable-next-line not-rely-on-time
        require(
            recoveryTime_ > releaseTime_,
            "TimelockedEscrow: recovery time is not after release time"
        );
        _token = token_;
    ...
    function release() public virtual {
        ...
        require(
            ...
            "TimelockedEscrow: current time is before release time"
        );

        require(
            address(msg.sender) == _beneficiary,
            "TimelockedEscrow: only the beneficiary can release the funds"
        );

        uint256 amount = token().balanceOf(address(this));
        require(amount > 0, "TimelockedEscrow: no tokens to release");
    ...
    function rescue() public virtual {
        ...
        require(
            ...
            "TimelockedEscrow: current time is before recovery time"
        );
