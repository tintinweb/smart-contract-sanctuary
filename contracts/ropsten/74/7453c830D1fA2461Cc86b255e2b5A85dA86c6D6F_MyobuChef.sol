...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
    ...
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn, uint256 amountETHMin, uint256 amountTokenMin, uint256 lockTime) public payable nonReentrant {
        ...
        require(msg.value > 0);
        require((lockTime >= 600 && lockTime <= 31200) || user.lockedUntil >= block.timestamp, "addLiquidityNoFeeAndStake : Can't lock tokens for less than 1 week");
        updatePool(0);
    ...
    function buyAndStakeETH(uint256 amountOutMin, uint256 lockTime) public payable {
        UserInfo storage user = userInfo[1][msg.sender];
        require(msg.value > 0);
        require((lockTime >= 600 && lockTime <= 31200) || user.lockedUntil >= block.timestamp, "buyAndStakeETH : Can't lock tokens for less than 1 week");
        updatePool(1);
        if (user.weightedBalance > 0) {
            _addToClaimable(1, msg.sender);
        }
    ...
    function addToClaimable(uint32 _pid) public {
...
//--------------------------------------------------------INTERNAL FUNCTIONS --------------------------------------------------------
    function _deposit(address sender, uint256 _pid, uint256 _amount, uint256 lockTime) internal {
        ...
        if (_amount > 0) {
            require((lockTime >= 600 && lockTime <= 31200) || user.lockedUntil > block.timestamp, "deposit : Can't lock tokens for less than 1 week");
    ...
    function lockTokens(uint256 _pid, uint256 lockTime) internal {
        ...
        userInfo[_pid][msg.sender].lockTimeBoost = uint32(2 * 1000 * (lockTime-600) / 30600); // 0 - 2000 
        updateUserWeightedBalance(_pid, msg.sender);
	...
	function updateUserWeightedBalance(uint256 _pid, address user) public {
		...
		if (userInfo[_pid][user].lockedUntil < block.timestamp) {
		    userInfo[_pid][user].weightedBalance = 0;
