...
contract LatticeStakingPool is ReentrancyGuard {
    ...
    event Deposit(address indexed _user, uint256 indexed _projectId, uint256 indexed _poolId, uint256 _amount);
    event Withdraw(address indexed _user, uint256 indexed _projectId, uint256 indexed _poolId, uint256 _amount);
    ...
    function addProject(string memory _name, uint256 _startBlock, uint256 _endBlock) external {
        ...
        require(_startBlock >= block.number, "addNewProject: startBlock is less than the current block number.");
        require(_startBlock < _endBlock, "addNewProject: startBlock is greater than or equal to the endBlock.");
        require(isProjectNameTaken[_name] == false, "addNewProject: project name already taken.");
        ...
        projectNameToProjectId[_name] = projects.length.sub(1);
        isProjectNameTaken[_name] = true;
        
        emit ProjectAdded(projects.length.sub(1), _name);
    }
    ...
    function deposit (uint256 _projectId, uint256 _poolId, uint256 _amount) external nonReentrant {
        require(projectIdToWhitelistedAddress[_projectId][msg.sender] == true, "deposit: Address is not whitelisted for this project.");
        ...
        require(block.number <= projects[_projectId].endBlock, "deposit: Staking no longer permitted for this project.");
        require(block.number >= projects[_projectId].startBlock, "deposit: Staking is not yet permitted for this project.");
        ...
        if(stakingPoolInfo[_projectId][_poolId].maxStakingAmountPerUser > 0){
            require(_userStakedAmount.add(_amount) <= stakingPoolInfo[_projectId][_poolId].maxStakingAmountPerUser, "deposit: Cannot exceed max staking amount per user.");
        }
        ...
        projects[_projectId].totalAmountStaked = projects[_projectId].totalAmountStaked.add(_amount);
        stakingPoolInfo[_projectId][_poolId].totalAmountStaked = stakingPoolInfo[_projectId][_poolId].totalAmountStaked.add(_amount);
        userStakedAmount[_projectId][_poolId][msg.sender] = userStakedAmount[_projectId][_poolId][msg.sender].add(_amount);
    ...
    function withdraw (uint256  _projectId, uint256 _poolId) external nonReentrant {
        require(projectIdToWhitelistedAddress[_projectId][msg.sender] == true, "withdraw: Address is not whitelisted for this project.");
        ...
        require(didUserWithdrawFunds[_projectId][_poolId][msg.sender] == false, "withdraw: User has already withdrawn funds for this pool.");
    ...
    function whitelistAddresses( uint256 _projectId, address[] memory _newAddressesToWhitelist) external {
        ...
        for (uint i=0; i < _newAddressesToWhitelist.length; i++) {
            if(projectIdToWhitelistedAddress[_projectId][_newAddressesToWhitelist[i]] == false){
    ...
    function getWhitelistedAddressesForProject( uint256 _projectId) external view returns(address[] memory){
    ...
    function isAddressWhitelisted(uint256 _projectId, address _address) external view returns(bool){
    ...
    function getTotalStakingInfoForProject(uint256 _projectId)external view returns (UserInfo[] memory){
        require(msg.sender == owner, "getTotalStakingInfoForProject: Caller is not the owner.");
        require(_projectId < projects.length, "getTotalStakingInfoForProject: Invalid project ID.");
        
        uint256 _usersCounter = 0;
        
        for(uint256 i=0;i<projects[_projectId].numberOfPools;i++){
            _usersCounter = _usersCounter + stakingPoolInfo[_projectId][i].usersStaked.length;
        }
        
        UserInfo[] memory _result = new UserInfo[](_usersCounter);
        uint256 _resultIndex = 0;
        
        for(uint256 i=0;i<projects[_projectId].numberOfPools;i++){
            for(uint256 j=0;j<stakingPoolInfo[_projectId][i].usersStaked.length;j++){
                UserInfo memory _userInfo;
                _userInfo.userAddress = stakingPoolInfo[_projectId][i].usersStaked[j];
                _userInfo.poolId = i;
                _userInfo.percentageOfTokensStakedInPool = getPercentageAmountStakedByUserInPool(_projectId,i,_userInfo.userAddress);
                _userInfo.amountOfTokensStakedInPool = getAmountStakedByUserInPool(_projectId,i,_userInfo.userAddress);
    ...
    function getTotalAmountStakedInProject(uint256 _projectId) external view returns (uint256) {
        require(_projectId < projects.length, "getTotalAmountStakedInProject: Invalid project ID.");
        
        return projects[_projectId].totalAmountStaked;
    }
    
    function getTotalAmountStakedInPool(uint256 _projectId,uint256 _poolId) external view returns (uint256) {
        require(_projectId < projects.length, "getTotalAmountStakedInPool: Invalid project ID.");
        require(_poolId < projects[_projectId].numberOfPools, "getTotalAmountStakedInPool: Invalid pool ID.");
        
        return stakingPoolInfo[_projectId][_poolId].totalAmountStaked;
    }
    
    function getAmountStakedByUserInPool(uint256 _projectId,uint256 _poolId, address _address) public view returns (uint256) {
        require(_projectId < projects.length, "getAmountStakedByUserInPool: Invalid project ID.");
        require(_poolId < projects[_projectId].numberOfPools, "getAmountStakedByUserInPool: Invalid pool ID.");  
        
        return userStakedAmount[_projectId][_poolId][_address];
    }
    
    function getPercentageAmountStakedByUserInPool(uint256 _projectId,uint256 _poolId, address _address) public view returns (uint256) {
        require(_projectId < projects.length, "getPercentageAmountStakedByUserInPool: Invalid project ID.");
        require(_poolId < projects[_projectId].numberOfPools, "getPercentageAmountStakedByUserInPool: Invalid pool ID.");  
        
        return userStakedAmount[_projectId][_poolId][_address].mul(1e10).div(stakingPoolInfo[_projectId][_poolId].totalAmountStaked);
    }
}