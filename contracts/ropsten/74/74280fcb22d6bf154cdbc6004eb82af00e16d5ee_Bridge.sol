...
contract Bridge {
    ...
    function getAvailableTokens(uint64 searchBlock) public view returns(address[] memory, address[] memory, uint256[] memory) {
        ...
        for (uint i=n-1; i>=0; i--) {
            if (tokenIdxMatchBlock[i] <= searchBlock) {
                retSrcContracts[items] = srcContracts[i];
                retDestContracts[items] = destContracts[i];
                retMinTokenAmounts[items] = minTokenAmounts[i];
                items++;
