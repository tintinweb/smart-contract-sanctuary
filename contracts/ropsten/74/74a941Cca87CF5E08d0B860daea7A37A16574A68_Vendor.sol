...
contract Vendor is Ownable {
    ...
    function buyTokens() external payable {
        if (yourToken.transfer(msg.sender, msg.value * tokensPerEth)) {
            emit BuyTokens(msg.sender, msg.value, msg.value * tokensPerEth);
        }
    }

    // function sellTokens2() external payable {
    //     require(
    //         msg.value * tokensPerEth >= 1,
    ...
    //     require(
    //         userBalance >= msg.value * tokensPerEth,
    //         "Your balance is lower than the amount of tokens you want to sell"
    //     );

    //     // Check that the Vendor's balance is enough to do the swap
    //     uint256 amountOfETHToTransfer = msg.value / tokensPerEth;
    ...
    //     // yourToken.approve(msg.sender, msg.value / tokensPerEth);
    //     if (
    ...
    function sellTokens(uint256 tokens) public {
        ...
        uint256 amountETH = tokens / tokensPerEth;
        uint256 ownerETHBalance = address(this).balance;
        require(
            ownerETHBalance >= amountETH,
            "Vendor has not enough funds to accept the sell request"
        );

        yourToken.transferFrom(msg.sender, address(this), tokens);
        emit SellTokens(msg.sender, amountETH, tokens);
    }
