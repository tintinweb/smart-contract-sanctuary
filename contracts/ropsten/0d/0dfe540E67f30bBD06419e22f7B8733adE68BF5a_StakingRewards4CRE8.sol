...
contract StakingRewards4CRE8 is StakingTokenWrapper4CRE8, Ownable { 
    ...
    */
    function withdraw(uint256 _amount, uint256 entryIndex) external {
        ...
        claimReward(entryIndex, stakingEntries[msg.sender][entryIndex].claimableRewards / stakingEntries[msg.sender][entryIndex].value * _amount); // should be confirmed
    ...
    */
    function claimReward(uint256 entryIndex, uint256 _amount) public {
        ...
        for (i = fromDay; i < toDay; i++) {
            if (stakedPerDay[i] == 0) stakedPerDay[i] = stakedPerDay[i - 1];
            additionalRewards = additionalRewards + rewardsPerDay / stakedPerDay[i] * stakingEntries[msg.sender][entryIndex].value;
        }
        stakingEntries[msg.sender][entryIndex].totalRewards = stakingEntries[msg.sender][entryIndex].totalRewards + additionalRewards;
        stakingEntries[msg.sender][entryIndex].claimableRewards = stakingEntries[msg.sender][entryIndex].claimableRewards + additionalRewards;
    ...
    function fetchHistory() external view returns(StakingEntry[] memory) {
        StakingEntry[] memory history = new StakingEntry[](stakingEntries[msg.sender].length);
        uint256 i;
        uint256 j;
        uint256 stakedDay;
        ...
        for (i = 0; i < history.length; i++) {
            history[i].initialValue = stakingEntries[msg.sender][i].initialValue;
            history[i].value = stakingEntries[msg.sender][i].value;
            history[i].stakingTimestamp = stakingEntries[msg.sender][i].stakingTimestamp;
            history[i].lastClaimedTime = stakingEntries[msg.sender][i].lastClaimedTime;
            history[i].totalRewards = stakingEntries[msg.sender][i].totalRewards;
            history[i].claimableRewards = stakingEntries[msg.sender][i].claimableRewards;
            history[i].fullDaysStaked = stakingEntries[msg.sender][i].fullDaysStaked;
            history[i].rewardStatus = stakingEntries[msg.sender][i].rewardStatus;
        }
        ...
        for (i = 0; i < history.length; i++) {
            ...
            for (j = fromDay; j < toDay; j++) {
                if (stakedPerDay[i] == 0) stakedDay = stakedPerDay[i - 1];
                else stakedDay = stakedPerDay[i];
                additionalRewards = additionalRewards + rewardsPerDay / stakedDay * history[j].value;
            }
