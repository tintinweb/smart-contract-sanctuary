...
contract CodeTest_1 is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    mapping (uint256 => string ) private metadataGames;
    ...
    function safemint(address to, string memory metadata ,string memory metadataGame) public {
        ...
        metadataGames[tokenID] = metadataGame;
    ...
    function traitOfowner(uint256 tokenId) public view virtual returns(uint256 , uint256 ,uint256 ){
        if(NFTtrackList[tokenId].buy_time + revealThreeDays <= block.timestamp ){
             return  (idWithTraits[tokenId].speed , idWithTraits[tokenId].weight ,idWithTraits[tokenId].power);
        }else{
    ...
    function tokenURI(uint256 tokenId)public view override(ERC721, ERC721URIStorage) returns (string memory)
    {
     ...
     function tokenURIGame(uint256 tokenId) public  view  returns (string memory)
    {
        if(NFTtrackList[tokenId].buy_time + revealThreeDays <= block.timestamp ){
            return metadataGames[tokenId];
        }else{
            if( count >= 666 && lastNFTBuy + revealTwelveHour <= block.timestamp ){
               return metadataGames[tokenId];
            }else{
