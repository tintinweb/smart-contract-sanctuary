...
contract HeadShotSalesLedger is ERC20, Ownable {
    ...
    constructor() ERC20("HeadShotSalesLedger", "HSSL") {}
    ...
    */
    ...
    function getTrackerFieldString(uint256 code_, string memory key_) public view onlyOwner returns (string memory) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.getFieldString(key_);
    }

    function getTrackerFieldNumber(uint256 code_, string memory key_) public view onlyOwner returns (uint256) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.getFieldNumber(key_);
    }

    function getAccountBalance(uint256 code_, address account_) public view onlyOwner returns (uint256) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.balanceOf(account_);
    }

    function getTrackerFieldAddress(uint256 code_, string memory key_) public view onlyOwner returns (address) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(code_)));
