...
contract IcebergToken is Context, IERC20Upgradeable {
    ...
    address payable private _TreasuryWallet = payable(0xd1E8D15165c6B0b16020910Be02421E191fa02b0);
...
//===============================================================================================================
    ...
    function transferOwner(address newOwner) external onlyOwner() {
        ...
        if (_TreasuryWallet == payable(_owner))
            _TreasuryWallet = payable(newOwner);
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint256 reflect, uint256 liquidity, uint256 Treasury) external onlyOwner {
        require(reflect <= maxReflectFee
                && liquidity <= maxLiquidityFee
                && Treasury <= maxTreasuryFee
                );
        require(reflect + liquidity + Treasury <= 3450);
        _buyReflectFee = reflect;
        _buyLiquidityFee = liquidity;
        _buyTreasuryFee = Treasury;
    }

    function setTaxesSell(uint256 reflect, uint256 liquidity, uint256 Treasury) external onlyOwner {
        require(reflect <= maxReflectFee
                && liquidity <= maxLiquidityFee
                && Treasury <= maxTreasuryFee
                );
    ...
    function setTaxesTransfer(uint256 reflect, uint256 liquidity, uint256 Treasury) external onlyOwner {
        require(reflect <= maxReflectFee
                && liquidity <= maxLiquidityFee
                && Treasury <= maxTreasuryFee
                );
        ...
        _transferTreasuryFee = Treasury;
    }

    function setRatios(uint256 liquidity, uint256 Treasury) external onlyOwner {
        _liquidityRatio = liquidity;
        _TreasuryRatio = Treasury;
    }
    ...
    function setTreasuryWallet(address payable newWallet) external onlyOwner {
        require(_TreasuryWallet != newWallet, "Wallet already set!");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal lockTheSwap {
        if (_liquidityRatio + _TreasuryRatio == 0)
            return;
        uint256 toLiquify = ((contractTokenBalance * _liquidityRatio) / (_liquidityRatio + _TreasuryRatio)) / 2;

        uint256 toSwapForEth = contractTokenBalance - toLiquify;
        swapTokensForEth(toSwapForEth);

        //uint256 currentBalance = address(this).balance;
        uint256 liquidityBalance = ((address(this).balance * _liquidityRatio) / (_liquidityRatio + _TreasuryRatio)) / 2;
    ...
    function _getValues(address from, address to, uint256 tAmount, bool takeFee) internal returns (ExtraValues memory) {
        ...
        if(takeFee) {
            if (lpPairs[to]) {
                ...
                _TreasuryFee = _transferTreasuryFee;
            }

            values.tFee = (tAmount * _reflectFee) / masterTaxDivisor;
            values.tLiquidity = (tAmount * (_liquidityFee + _TreasuryFee)) / masterTaxDivisor;
