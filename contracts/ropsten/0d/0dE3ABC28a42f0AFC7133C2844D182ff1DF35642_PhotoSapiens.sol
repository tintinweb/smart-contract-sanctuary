...
contract PhotoSapiens is ERC721Enumerable {
    ...
    uint32 private _whitelistTime; // (in seconds)

    address private _whitelistSignerAddress;
    address private _backendSignerAddress;

    constructor(
        uint32 launchDate,
        uint32 whitelistTime, // NOTE: in hours!
        address adminAddress,
        address whitelistSignerAddress,
        address backendSignerAddress
    ) ERC721("PhotoSapiens", "PS") {
        _launchDate = launchDate;
        _whitelistTime = (whitelistTime * 1 hours);
    ...
    // The one and only admin address.
    ...
    // getWhitelistPeriod returns the start and end of the whitelist period.
    function getWhitelistPeriod() external view returns (uint32, uint32) {
        return (_launchDate, _launchDate + _whitelistTime);
    }

    // transferBalance transfers the given amount to the admin address.
