...
contract OpenAuction {
  ...
  function bid() public payable {
    ...
    uint newValue;
    if (penddingReturns[msg.sender] != 0) {
      newValue = penddingReturns[msg.sender] + msg.value;
    } else {
      newValue = msg.value;
    }
    require(newValue >= minValue, "Value smaller than min value.");
    require(newValue > highestBid, "There already is a higher bid.");
