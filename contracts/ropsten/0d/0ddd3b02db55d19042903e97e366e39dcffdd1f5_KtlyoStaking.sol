...
contract KtlyoStaking {
	...
	uint256 public amtRewardRemainingBalance1;
	uint256 public amtRewardRemainingBalance2;
	uint256 public totalStaked1;
	uint256 public totalStaked2;
	uint256 public totalRedeemed1;
	uint256 public totalRedeemed2;
	uint256 public openRewards1;
	uint256 public openRewards2;
    ...
    function activate() onlyOwner public {
		...
		uint256 rewardsForToken1= tokenOne.balanceOf(address(this));
		uint256 rewardsForToken2= tokenTwo.balanceOf(address(this));
		require (rewardsForToken1>=rewardAmt1,"Activation error. Insufficient balance of rewards for token1");
		require (rewardsForToken2>=rewardAmt2,"Activation error. Insufficient balance of rewards for token2");
		//activate staking
		stakingStarted = true;
	...
	returns (uint256 returnAmount, uint256 returnAmount2) 
	{
		...
		if (transactions[requestor][indexId].token==token1)
		{
			...
			if (transactions[requestor][indexId].stakeEnd < block.timestamp)
			{
	...
	function redeemEarly(uint256 indexId) public {
	   ...
	   require(transactions[msg.sender][indexId].redeemed==false && transactions[msg.sender][indexId].stakeEnd>block.timestamp ,"Stake is already redeemed or end_date is in the past");
