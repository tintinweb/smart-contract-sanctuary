...
contract GameResult {
    ...
    function getrecords() public view returns (string memory ss) {
        ...
        for (uint i = 0; i < records.length; i ++) {
            append(ss, "^", "[", records[i].id);
            append(ss, ",", string(abi.encodePacked(records[i].submitter)), ",");
            append(ss, "", uint2str(records[i].submit_nonce), "]");
            append(ss, uint2str(records[i].winner_mask), "(", uint2str(records[i].winner_score));
            append(ss, ",", uint2str(records[i].loser_score), ")");
        }
    }
