...
contract IRSwap {
  ...
  constructor(address _tokenAddress, address _cTokenAddress, uint8 _initLeverage, uint8 _maintLeverage) public {
  ...
  function createSwap(
    ...
    require(fundingPayer >= notional.div(initLeverage), "payer funding below initMargin");
    require(fundingReceiver >= notional.div(initLeverage), "receiver funding below initMargin");
    require(checkBalance(addressPayer, fundingPayer), "payer balance below funding");
    require(checkBalance(addressReceiver, fundingReceiver), "receiver balance below funding");
    require(checkApproval(addressPayer, fundingPayer), "payer must approve contract spend");
    require(checkApproval(addressReceiver, fundingReceiver), "receiver must approve contract spend");
    
    // Transfer token balances from counterparties to contract as escrow
    IERC20 token = IERC20(tokenAddress);
    token.transferFrom(addressPayer, address(this), fundingPayer);
    token.transferFrom(addressReceiver, address(this), fundingReceiver);
    
    (uint256 initCTokenRate, ) = getCTokenRate();
    IRSwapTrade memory swap = IRSwapTrade(block.number,
                                 endBlock,
                                 notional,
                                 initCTokenRate,
                                 fixedRatePerBlock,
  ...
  function accrueInterest(uint256 i) public {
    ...
    IRSwapTrade storage swap = swaps[i];
  ...
  //INTERNAL FUNCTIONS
  function checkBalance(address userAddress, uint256 amount) internal view returns(bool){
  ...
  function checkApproval(address userAddress, uint256 amount) internal view returns(bool){
  ...
  //READ FUNCTIONS
  function getSwapTrade(uint256 i) public view returns(
                                                      uint256,
                                                      uint256,
                                                      uint256,
                                                      uint256,
    ...
    require(i < swaps.length, "index out of bounds");
    IRSwapTrade memory swap = swaps[i];
