...
contract DecentralizedSmartGames{
    ...
    function getAccountData() public view 
    ...
    function getReferrer() public view returns(address first, address second, address third)
    {
    ...
    function calculateEthereum(uint256 incomingTokens) public view returns(uint256){
        uint256 ethereum = ((2 * tokenPrice.mul(1e18) - tokenPriceStep * (incomingTokens + 1e18)) / 2 / 1e18) * incomingTokens / 1e18;
        return ethereum;
    }
    function calculateTokens(uint256 incomingEthereum) public view returns(uint256){
