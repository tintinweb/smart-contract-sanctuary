...
contract TokenMapped is AuthQuota, ContextUpgradeSafe {
    using SafeERC20 for IERC20;
...
contract MappableToken is ERC20UpgradeSafe, TokenMapped, IPermit {
	...
	function __MappableToken_init(address governor_, string memory name_, string memory symbol_, uint8 decimals_) external initializer {
		...
		__Governable_init_unchained(governor_);
		__TokenMapped_init_unchained(address(this));
...
contract MATTER is ERC20UpgradeSafe, Configurable {
	...
	function __TokenMapped_init_unchained() public governance {
        token = address(this);
