...
// File: EFExchanger.sol

contract EFExchanger is Ownable, TrustListTools{
    ...
    // agent[approved]  --from_token->  DEX  --target_token->  msg.sender
    function handle_token(address from_token, address target_token, uint256 amount, uint min_amount) public is_trusted(msg.sender) {
        uint256 maxOut = 0;
        uint256 fpi = 0;

        for(uint pi = 0; pi < path_indexes.length; pi ++){
            if(path_from_addr(pi) != from_token || path_to_addr(pi) != target_token){
                continue;
            }
        ...
        IERC20(from_token).safeTransferFrom(agent_addr, address(this), amount);
        IERC20(from_token).safeApprove(dex, amount);
        if (target_token == weth) {
            ...
            msg.sender.transfer(target_amount);  // ???
        } else {
            ...
            require(target_amount >= min_amount, "slippage screwed you");
            IERC20(target_token).transfer(address(msg.sender), target_amount);
        }
    }
    ...
    /* ----- admin ----- */
    function set_agent(address _new) public onlyOwner {
        agent_addr = _new;
    }

    event AddPath(bytes32 hash, address dex, address[] path);
    function add_path(address dex, address[] memory path) public onlyOwner{
    ...
    function remove_path(address dex, address[] memory path) public onlyOwner{
        bytes32 hash = keccak256(abi.encodePacked(dex, path));
        remove_path_by_hash(hash);
    }

    function remove_path_by_hash(bytes32 hash) public onlyOwner{
