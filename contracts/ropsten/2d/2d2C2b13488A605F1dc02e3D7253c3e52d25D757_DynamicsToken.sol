...
contract DynamicsToken is Context, IERC20, Ownable {
    ...
    string private constant _name = "Dynamics Token";
    ...
    uint256 public _maximumSizeFee = 10;
    ...
    constructor (uint256 _supply, uint256 _MAXTXAMOUNT, uint256 ADDTOLIQUIDITYTHRESHOLD,
                 address routerAddress, address tokenOwner, address projectWallet, address marketingWallet) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isBlacklisted[_msgSender()], "Address is blacklisted");
        ...
        if(from != owner() && to != owner()) {
            ...
            if(!trader[msg.sender].exists) {
                trader[msg.sender] = User(0,0,0,true);
            }
...
//                _transferStandard(from,to, rAmount, tTransferAmount, rTransferAmount);
                ...
                // sell
            } else if(!inSwapAndLiquify && from != uniswapV2Pair && from != address(uniswapV2Router)) {
                ...
                uint256 sizeBasedFee = _maximumSizeFee;
...
//                _transferStandard(from,to, rAmount, tTransferAmount, rTransferAmount);
                ...
                require(to != uniswapV2Pair && to != address(uniswapV2Router) && from != uniswapV2Pair && from != address(uniswapV2Router));
...
//                _transferStandard(from,to, rAmount, tTransferAmount, rTransferAmount);
    ...
    function updateFallbackFees(uint256 minimumTimeBasedFee, uint256 maximumSizeBasedFee) public onlyOwner {
        _minimumTimeFee = minimumTimeBasedFee;
        _maximumSizeFee = maximumSizeBasedFee;
    }
}
