...
contract MarketSale is IMarketSale, ERC721HolderUpgradeable, OwnableUpgradeable {
    ...
    function safeTransMyToken(address to, uint256 tokenId) external onlyOwner {
        IERC721Upgradeable(_nftContract).safeTransferFrom(address(this), to, tokenId);
    }

    function safeTransMyTo(address to, uint256 tokenId) external onlyOwner {
        _safeTransferToken(address(this), to, tokenId);
    }
