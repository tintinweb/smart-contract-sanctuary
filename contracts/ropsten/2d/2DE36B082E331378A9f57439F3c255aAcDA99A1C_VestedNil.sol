...
contract VestedNil is AccessControl, ERC20, ReentrancyGuard, IVestedNil {
    ...
    function multiMint(address[] calldata accounts, uint256[] calldata amounts) external override onlyMinter {
    ...
    function mint(address account, uint256 amount) public override onlyMinter {
        require(totalSupply() + nil.totalSupply() + amount <= nil.MAX_SUPPLY(), "VestedNil:SUPPLY_OVERFLOW");
    ...
    function _claim(address account) private returns (uint128 claimable) {
        ...
        uint256 amount = balance + claimed;
        uint256 vested = _totalVestedOf(amount, block.timestamp);
        claimable += uint128(Math.min(vested - claimed, balance));
        claimed = claimed + claimable;
        claimedByAccount[account] = claimed;
    ...
    function claimAndStake() external override nonReentrant {
        ...
        nilStaking.mintAndStake(msg.sender, claimed);
    }
    ...
    function _totalVestedOf(uint256 initialAllocation, uint256 currentTime) internal view returns (uint256 total) {
        ...
        uint256 currentYear = elapsed / timePeriodInSeconds;
        uint256 elapsedInCurrentYear = elapsed % timePeriodInSeconds;
        uint256 cumulativeMultiplierPast = 0;

        if (currentYear > 0) {
            cumulativeMultiplierPast = _cumulativeScheduleInBPS[currentYear - 1];
            total = (initialAllocation * cumulativeMultiplierPast) / 10000;
        }

        if (elapsedInCurrentYear > 0) {
            /**
                currentYear can never go out of bounds because after the last year elapsedInCurrentYear will always be 0
                so we will not enter this block
             */
            uint256 currentMultiplier = _cumulativeScheduleInBPS[currentYear] - cumulativeMultiplierPast;
            uint256 yearAllocation = (initialAllocation * currentMultiplier) / 10000;
            total += (yearAllocation * elapsedInCurrentYear) / timePeriodInSeconds;
        }
    }

    function _vestingSnapshot(address account) internal view returns (uint256, uint256) {
        uint128 claimed = claimedByAccount[msg.sender];
        uint256 amount = balanceOf(account) + claimed;
        return (_totalVestedOf(amount, block.timestamp), claimed);
    }
    ...
    function claimableOf(address account) external view override returns (uint256) {
        (uint256 vested, uint256 claimed) = _vestingSnapshot(account);
