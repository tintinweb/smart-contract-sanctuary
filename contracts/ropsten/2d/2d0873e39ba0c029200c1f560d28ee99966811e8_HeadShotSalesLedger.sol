...
contract HeadShotSalesLedger is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using ConvertString for uint256;
    using Address for address;

    string private constant _name = "HeadShotSalesLedger";
    string private constant _symbol = "HSSL";
    ...
    function listSalesPart1(uint limit_, uint page_) public view onlyOwner returns (uint256[] memory, uint256[] memory, string[] memory) {
    ...
    function listSalesPart2(uint limit_, uint page_) public view onlyOwner returns (uint256[] memory, string[] memory, uint256[] memory) {
        uint listCount = _salesList.length;
