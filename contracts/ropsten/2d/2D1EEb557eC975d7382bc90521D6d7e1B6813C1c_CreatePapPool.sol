...
contract PAPpool {
    ...
    function participate(uint256 _amount) public {
        ...
        require(block.timestamp >= _pool.timeEnd, 'participate: pool not started');
        require(_pool.timeStart >= block.timestamp, 'participate: pool already ended');
    ...
    function getUserInfo(address user) external view returns (infoParticipants memory) {
