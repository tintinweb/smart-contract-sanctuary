...
contract UniswapV2Factory is IUniswapV2Factory {
    ...
    {
        require(block.timestamp - _delay >= lockTime, "current time is before release time");
    }
...
interface IUniswapV2Pair is IUniswapV2ERC20 {
    ...
    function swapWithTreasury(uint amount0Out, uint amount1Out, address to) external;
...
interface IUniswapV2Factory {
    ...
    function test_timeStamp() external view returns (uint);
    function test_releaseTime() external view returns (uint);
    function test_Diff() external view returns (uint);
    function test_locktime() external view returns (uint);
    function test_addressZero() external view returns (address);
    function test_Require(uint) external;
