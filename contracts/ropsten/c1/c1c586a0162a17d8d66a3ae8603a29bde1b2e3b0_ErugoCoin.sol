...
contract ERC20Token {
    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        ...
        emit Approval(msg.sender, from, allowed_[from][msg.sender]);
    ...
    function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
        ...
        allowed_[msg.sender][spender] = allowed_[msg.sender][spender].add(addedValue);
        emit Approval(msg.sender, spender, allowed_[msg.sender][spender]);
        return true;
    }
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        require(spender != address(0));

        allowed_[msg.sender][spender] = allowed_[msg.sender][spender].sub(subtractedValue);
        emit Approval(msg.sender, spender, allowed_[msg.sender][spender]);
        return true;
    }
...
contract ErugoCoin is Ownable, ERC20Token {
    ...
    address constant company = 0xbBF8a9B66f2eFA67564f542BC96C09eD574C4678;
    ...
    function transferFrom(address from, address to, uint256 value) public lockedSender(from) returns (bool) {
