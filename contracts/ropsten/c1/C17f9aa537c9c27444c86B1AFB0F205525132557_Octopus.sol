...
contract Octopus is Ownable {
    ...
    function sell(address _optionAddress, uint256 _weiNotional) payable public {
        ...
        if (IOption(_optionAddress).isPut()) {
            (usdCollateral, weiCollateral) = calculateCollateral(_optionAddress, _weiNotional);
            
            require(msg.value >= weiCollateral, 'Invalid collateral');
            difference = msg.value - weiCollateral;
        ...
        if (IOption(_optionAddress).isPut()) {
            feesToCollect = weiCollateral / 100;
    ...
    function _activateETHOption(address _optionAddress, uint256 _uniswapInitialUSDT, uint256 _uniswapInitialTokens, bool _createPair) public payable onlyOwner {
        require(IOption(_optionAddress).getExpiresOn() > block.timestamp, "Expired option");
