...
// File: contracts/ICommonStakeERC721Earnkdm.sol

// SPDX-License-Identifier: MIT

pragma solidity =0.6.6;

interface ICommonStakeERC721Earnkdm {
    ...
    event EmergencyUnstakeAllFromkdm(address indexed user, uint256 amount);

    function getStakingPower(uint256 _tokenId) external view returns (uint256);

    function pendingkdm(address _user) external view returns (uint256);
    ...
    function emergencyUnstakeAllFromkdm(uint256 _amount) external;
...
// File: contracts/IGetStakingPower.sol
...
// File: contracts/CommonStakeERC721Earnkdm.sol
...
contract CommonStakeERC721EarnKDM is ICommonStakeERC721Earnkdm, ERC20, Ownable, ERC721Holder, Pausable {
    ...
    uint256 public constant acckdmPerShareMultiple = 1E20;
    uint256 public lastRewardBlock;
    // total has stake to kdmMaster stakingPower
    uint256 public totalStakingPower;
    IERC721 public immutable erc721;
    address public constant kdmToken = 0x8a61951c879fD53b6Caf4CD60310EA969B220A26;
    ICommonMaster public immutable kdmMaster;
    ...
    constructor(
        ...
        kdmMaster = ICommonMaster(_kdmMaster);
    ...
    function pendingkdm(address _user) external view override returns (uint256) {
        UserInfo memory userInfo = _userInfoMap[_user];
        uint256 _acckdmPerShare = acckdmPerShare;
        if (totalStakingPower != 0) {
            uint256 totalPendingkdm = kdmMaster.pendingToken(address(this), address(this));
            _acckdmPerShare = _acckdmPerShare.add(
                totalPendingkdm.mul(acckdmPerShareMultiple).div(totalStakingPower)
            );
        }
        return userInfo.stakingPower.mul(_acckdmPerShare).div(acckdmPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updateStaking() public override {
        ...
        (, uint256 lastRewardDebt) = kdmMaster.poolUserInfoMap(address(this), address(this));
        kdmMaster.stake(address(this), 0);
        (, uint256 newRewardDebt) = kdmMaster.poolUserInfoMap(address(this), address(this));
        acckdmPerShare = acckdmPerShare.add(
            newRewardDebt.sub(lastRewardDebt).mul(acckdmPerShareMultiple).div(totalStakingPower)
        );
        lastRewardBlock = block.number;
    }
    ...
    function _harvest(UserInfo storage userInfo) internal {
        ...
        if (userInfo.stakingPower != 0) {
            uint256 pending = userInfo.stakingPower.mul(acckdmPerShare).div(acckdmPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending != 0) {
                safekdmTransfer(_msgSender(), pending);
    ...
    function harvest() external override {
        ...
        _harvest(userInfo);
        userInfo.rewardDebt = userInfo.stakingPower.mul(acckdmPerShare).div(acckdmPerShareMultiple);
    }
    ...
    function stake(uint256 _tokenId) public override whenNotPaused {
        ...
        kdmMaster.stake(address(this), stakingPower);
    ...
    function unstake(uint256 _tokenId) public override {
        ...
        kdmMaster.unstake(address(this), stakingPower);
    ...
    function _approveToMasterIfNecessary(uint256 amount) internal {
        uint256 currentAllowance = allowance(address(this), address(kdmMaster));
        if (currentAllowance < amount) {
            _approve(address(this), address(kdmMaster), 2**256 - 1 - currentAllowance);
        }
    }
    ...
    function emergencyUnstakeAllFromkdm(uint256 _amount) external override onlyOwner whenPaused {
        kdmMaster.emergencyUnstake(address(this), _amount);
        emit EmergencyUnstakeAllFromkdm(_msgSender(), _amount);
    }

    function safekdmTransfer(address _to, uint256 _amount) internal {
        uint256 kdmBal = IERC20(kdmToken).balanceOf(address(this));
        if (_amount > kdmBal) {
            IERC20(kdmToken).transfer(_to, kdmBal);
        } else {
            IERC20(kdmToken).transfer(_to, _amount);
        }
    }
