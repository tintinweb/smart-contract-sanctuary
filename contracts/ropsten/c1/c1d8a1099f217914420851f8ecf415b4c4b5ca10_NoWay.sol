...
contract NoWay is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "NoWay";
    ...
    uint256 public sellDelay = 2 minutes;
    ...
    function findPercent(uint256 value,uint256 fee) internal  view returns (uint256)  {
   
    uint256 onePercent = value.mul(fee).div(10**(_feeDecimal + 2));
    return onePercent;
  }
    ...
    function _transfer(
        ...
        if(recipient == uniswapV2Pair){
            ...
            _balanceLimit[sender] = _balanceLimit[sender].add(amount);
