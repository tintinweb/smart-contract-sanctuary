...
contract NYM is Context, Ownable, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    function mint() external payable {
        ...
        uint mintIndex = nextId ++;
        _safeMint(msg.sender, mintIndex);

        if (_rewardToken != address(0) && 0 < _rewardAmount) {
            TransferHelper.safeTransfer(_rewardToken, msg.sender, _rewardAmount);
        }
    }
    ...
    */
     ...
     */
    function getIPFSHashHex(uint256 tokenId) internal view returns (bytes memory)
    {
        _getIPFSHashHex(tokenId);
    }
    ...
    function _getIPFSHashHex(uint256 tokenId) internal view returns (bytes memory)
    {
        ...
        bytes32 ipfsHashHex = 0x36f52df6ca044f8e6af1f77b7a056abe12ec1a8a29c5209e0fbfec2ce105fbd1;
        if (ipfsHashHex == "") {
            return bytes("");
        }
