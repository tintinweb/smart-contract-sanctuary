...
library PlusCodes {
    ...
    bytes20 constant _PLUS_CODES_DIGITS = bytes20("23456789CFGHJMPQRVWX");
    ...
    ///                    additional area
    function getParent(uint256 childCode)
        internal
        pure
        returns (uint256 parentCode)
    {
...
abstract contract RandomDropVending {
    ...
    function purchaseTokensAndReveal(uint32 benevolence) payable external {
    ...
    function dropStatistics() view external returns (uint256 inventoryForSale, uint256 queueCount, uint256 setAside) {
        return (
            _inventoryForSale(),
            _commitQueue.count(),
    ...
    function _revealCallback(address recipient, uint256 allocation) virtual internal;
...
abstract contract AreaNFT is ERC721, Ownable {
    ...
    function split(uint256 tokenId) payable external {
    ...
    function setPriceToSplit(uint256 newPrice) onlyOwner external {
    ...
    function setBaseURI(string calldata baseURI) onlyOwner external {
        _baseTokenURI = baseURI;
    }
