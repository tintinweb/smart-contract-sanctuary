...
contract XTKN031649Project is Context, Ownable{
    // lib
    ...
    // storage
        ...
        uint256 private _unclaimedTaxes;
    ...
    function tax(uint256 pAmount) public{
        _unclaimedTaxes = _unclaimedTaxes.add(pAmount);
    }

    function distribute() public{
        uint256 reward;
        uint256 contractBalance = _token.balanceOf(address(this));
        if(contractBalance >= _unclaimedTaxes){
            for(uint256 i=0; i<_wallets.length; i++){
                reward = _unclaimedTaxes.mul(_share[_wallets[i]]).div(10**3);
                if(reward >= _unclaimedTaxes){
                    _token.transfer(_wallets[i], reward);
                    _unclaimedTaxes = _unclaimedTaxes.sub(reward);
    ...
    function getUnclaimedTaxes() public view onlyOwner returns(uint256 taxes){
        return(_unclaimedTaxes);
    }
