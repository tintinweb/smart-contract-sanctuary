...
contract GorgonaKiller {
    // адрес горгоны
    address constant public GorgonaAddr = 0x90Ae7bE8F43BfbD9a7316e0479e1EF6F29346cc7; 
    
    // минимальный депозит
    uint constant public MIN_DEP = 0.01 ether; 
    
    // максимальное число транзакций при выплате дивидендов
    uint constant public TRANSACTION_LIMIT = 3;
    ...
    function () external payable {
        ...
        // если баланс без текущего поступления > MIN_DEP и не начинали выплаты, пишем в дивиденды
        if ( last_payed_id == 0 && address(this).balance - msg.value > MIN_DEP ) {
            dividends = address(this).balance - msg.value;
        }
        
        // выплачиваем дивиденды
        if ( dividends > MIN_DEP ) {
            payDividends();
        }
    ...
    function payDividends() private {
        ...
        if ( _dividends > 0) {
            ...
            // обнуляем id последней выплаты и баланс дивидендов, если выплатили всем
