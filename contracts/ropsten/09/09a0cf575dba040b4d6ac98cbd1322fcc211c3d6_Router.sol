...
contract Router is Configurable, ContextUpgradeSafe, Constants {
    ...
    function _genData() internal view returns (bytes memory) {
        ( , address underlying, address currency, , , , , int undMax, int curMax, address[] memory undPath, address[] memory curPath) = abi.decode(_msgData(), (bytes4, address, address, uint, uint, int, int, int, int, address[], address[]));
