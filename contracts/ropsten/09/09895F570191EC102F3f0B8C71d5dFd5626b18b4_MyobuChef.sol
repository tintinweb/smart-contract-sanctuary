...
//--------------------------------------------------------VIEW FUNCTIONS --------------------------------------------------------
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256, uint256) {
        ...
        if (block.number > pool.lastRewardBlock && weightedBalance != 0) {
            ...
            uint256 rwtReward = multiplier.mul(rwtPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accRwtPerShare = accRwtPerShare.add(rwtReward.mul(1e12).div(weightedBalance));
            uint256 ETHReward = multiplier * ETHPerBlock * pool.allocPoint / totalAllocPoint;
            accETHPerShare = accETHPerShare.add(ETHReward.mul(1e12).div(weightedBalance));
        }
		
	uint256 PendingRWT = (userWeightedAmount.mul(accRwtPerShare).div(1e12)).sub(user.rewardDebt);
    uint256 PendingETH = (userWeightedAmount.mul(accETHPerShare).div(1e12)).sub(user.ETHrewardDebt);
...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(tokenReleased) {
            uint256 rwtReward = (multiplier.mul(rwtPerBlock).mul(pool.allocPoint)).div(totalAllocPoint);
            rwt.mint(rwtReward);
            pool.accRwtPerShare = pool.accRwtPerShare.add(rwtReward.mul(1e12).div(lpSupply));
        }
        uint256 ETHReward = (multiplier.mul(ETHPerBlock).mul(pool.allocPoint)).div(totalAllocPoint);
        ETHLeftUnclaimed = ETHLeftUnclaimed.add(ETHReward);
        ETHLeftUnshared = ETHLeftUnshared.sub(ETHReward);
        pool.accETHPerShare = pool.accETHPerShare.add(ETHReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = uint64(block.number);
    }
    ...
    function withdraw(uint32 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if(tokenReleased) {
            uint256 pending = (user.weightedBalance.mul(pool.accRwtPerShare).div(1e12)).sub(user.rewardDebt);
            if (pending > 0) {
                safeRWTTransfer(msg.sender, pending);
            }

        }
        uint256 ETHPending = (user.weightedBalance.mul(pool.accETHPerShare).div(1e12)).sub(user.ETHrewardDebt);
        if (ETHPending > 0) { 
        ...
        user.ETHrewardDebt = user.weightedBalance.mul(pool.accETHPerShare).div(1e12);
        user.rewardDebt = user.weightedBalance.mul(pool.accRwtPerShare).div(1e12);
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant onlyEmergency {
        ...
        pool.weightedBalance = pool.weightedBalance.sub(user.weightedBalance);
        user.amount = 0;
    ...
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn, uint256 amountETHMin, uint256 amountTokenMin, uint256 lockTime) public payable nonReentrant {
        ...
        if (userInfo[0][msg.sender].weightedBalance > 0) {
            if(tokenReleased) {
                uint256 pending = userInfo[0][msg.sender].weightedBalance.mul(poolInfo[0].accRwtPerShare).div(1e12).sub(userInfo[0][msg.sender].rewardDebt);
                
                if (pending > 0) {
                safeRWTTransfer(msg.sender, pending);
                }
                userInfo[0][msg.sender].rewardDebt = userInfo[0][msg.sender].weightedBalance.mul(poolInfo[0].accRwtPerShare).div(1e12);

            }
            uint256 ETHPending = userInfo[0][msg.sender].weightedBalance.mul(poolInfo[0].accETHPerShare).div(1e12).sub(userInfo[0][msg.sender].ETHrewardDebt);
            if (ETHPending > 0) { 
                safeETHTransfer(msg.sender, ETHPending);
                userInfo[0][msg.sender].ETHrewardDebt = userInfo[0][msg.sender].weightedBalance.mul(poolInfo[0].accETHPerShare).div(1e12);
        ...
        userInfo[0][msg.sender].amount =  userInfo[0][msg.sender].amount.add(numLiquidityAdded);
        ...
        //@dev suboptimal, calls updateUserWeightedBalance twice if lockTime > 0,
        if (lockTime > 0) {
    ...
    function reinvestETHRewards(uint256 amountOutMin) public nonReentrant {
            ...
            if(tokenReleased) {
                uint256 pending = (user.weightedBalance.mul(poolInfo[1].accRwtPerShare).div(1e12)) - user.rewardDebt;
                if (pending > 0) {
                safeRWTTransfer(msg.sender, pending);
                userInfo[1][msg.sender].rewardDebt = (user.weightedBalance.mul(poolInfo[1].accRwtPerShare) / 1e12);
                }
            }
            address[] memory path = new address[](2);
            path[0] = WETH;
            path[1] = address(Myobu);
            uint256 ETHPending = (user.weightedBalance.mul(poolInfo[1].accETHPerShare).div(1e12)).sub(user.ETHrewardDebt);
            if (ETHPending > 0) { 
                ...
                uint256 amountSwapped = Myobu.balanceOf(address(this)).sub(balanceBefore);
                userInfo[1][msg.sender].amount = userInfo[1][msg.sender].amount.add(amountSwapped);
                updateUserWeightedBalance(1, msg.sender);
                userInfo[1][msg.sender].ETHrewardDebt = user.weightedBalance.mul(poolInfo[1].accETHPerShare).div(1e12);
...
//--------------------------------------------------------RESTRICTED FUNCTIONS --------------------------------------------------------	
    ...
    function boostWithNFT(uint256 _pid, address NFTContract, uint tokenID) public nonReentrant {
        ...
        // All other public functions are accessible and devs are more than welcomed to build on top of our contracts.
...
//--------------------------------------------------------INTERNAL FUNCTIONS --------------------------------------------------------
    ...
    function lockTokens(uint256 _pid, uint256 lockTime) internal {
        ...
        userInfo[_pid][msg.sender].lockedUntil = block.timestamp.add(lockTime);
        // .mul wont work with constants using this syntax (correct syntax would be mul(constant, variable)), but variable.mul(variable2) works, so here's a stupid fix : 
        uint256 two = 2; 
        uint256 oneThousand = 1000;
        uint256 oneYear = 31449600;
        userInfo[_pid][msg.sender].lockTimeBoost = two.mul((oneThousand.mul(lockTime)).div(oneYear)); // 0 - 2000 
        updateUserWeightedBalance(_pid, msg.sender);
		emit TokensLocked(msg.sender, block.timestamp, lockTime);
    }
    // @dev safe to leave it public, but useless. 
	...
	function updateUserWeightedBalance(uint256 _pid, address user) public {
		...
		uint256 poolBalance = poolInfo[_pid].weightedBalance.sub(userInfo[_pid][user].weightedBalance);
		...
		// @dev Stupid fix again
		uint256 oneThousand = 1000;
		uint256 oneMillion = 1000000;
		userInfo[_pid][user].weightedBalance = ((amount.mul((oneThousand).add(userInfo[_pid][user].lockTimeBoost))).mul((oneThousand).add(userInfo[_pid][user].boostPointsBP))).div(oneMillion);
		poolInfo[_pid].weightedBalance = poolBalance.add(userInfo[_pid][user].weightedBalance);
    ...
    function updateETHRewards() internal {
        ...
        //@dev massUpdatePools MUST be before updateETHRewards;
        massUpdatePools();
        // @dev ...
        uint256 oneDay = 5760;
        uint256 seven = 7;
        ETHPerBlock = ETHLeftUnshared.div(oneDay.mul(seven));
