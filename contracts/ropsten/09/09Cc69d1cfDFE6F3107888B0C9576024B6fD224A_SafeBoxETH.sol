...
// File interfaces/ICEther.sol



pragma solidity 0.6.12;

// dforce protocol iToken
interface ICEther {
  ...
  function redeem(address _from, uint256 redeemTokens) external;
...
contract SafeBoxETH is Governable, ERC20, ReentrancyGuard {
  ...
  ICEther public immutable cToken;
  ...
  constructor(
    ...
    cToken = _cToken;
    relayer = msg.sender;
    // IWETH(_weth).approve(address(_cToken), uint(-1));
  }
  ...
  function deposit() external payable nonReentrant {
    ...
    cToken.mint{value: msg.value}(address(this));
  ...
  function withdraw(uint amount) public nonReentrant {
    ...
    cToken.approve(address(cToken), uint256(-1));
    cToken.redeem(address(this), uint256(amount));
    uint ethBalanceAfter = address(this).balance;
    uint ethAmount = ethBalanceAfter.sub(ethBalanceBefore);
