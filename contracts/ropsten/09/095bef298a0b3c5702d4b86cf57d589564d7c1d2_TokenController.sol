...
contract TokenController is OwnerHelper
{
    ...
    function createPool(address _contract, uint _cap, uint _min, uint _max, uint _maxInvestPerPerson, uint _fee, uint _tokenPerEth) public
    {
        ...
        poolInfoData[poolNumber].cap = _cap;
        poolInfoData[poolNumber].total = 0;
        poolInfoData[poolNumber].min = _min;
        poolInfoData[poolNumber].max = _max;
        poolInfoData[poolNumber].maxInvestPerPerson = _maxInvestPerPerson;
    ...
    function joinToPool(uint _poolNum, uint _value) public
    {
        ...
        depositEth[msg.sender] = depositEth[msg.sender].sub(_value);
        poolInvestData[_poolNum][msg.sender] = poolInvestData[_poolNum][msg.sender].add(_value);
        poolInfoData[_poolNum].total = poolInfoData[_poolNum].total.add(_value);
    }
    ...
    function showPoolNumber(address _poolOwner, address _contract) constant public returns (uint)
    {
        return poolNumbers[_poolOwner][_contract];
    }
    
    function showPoolMembers(uint _poolNum) constant public returns (address[])
    {
        return poolInfoData[_poolNum].poolMembers;
    }
    
    function showInvestEther(uint _poolNum) constant public returns (uint)
    {
        return poolInvestData[_poolNum][msg.sender];
    }
    
    function showDepositEther() constant public returns (uint)
    {
        return depositEth[msg.sender];
    }
    
    function showPoolInfo(uint _poolNum) internal view returns (poolInfo)
    {
        return poolInfoData[_poolNum];
    }
    
    function withDrawEther(uint _value) onlyOwner public
    {
