...
//            stats.unpausedBlock = 0;
    ...
    function updateSlot(uint poolId, uint slotId) public initializedPoolOnly(poolId) {
        ...
        if (rewards1 > 0) {
            ...
            IERC20(pulse.rewardToken1).transfer(slot.owner, rewards1);
        }

        if (rewards2 > 0) {
            IERC20(pulse.rewardToken2).transfer(slot.owner, rewards2);
        }
        ...
        if (burn > 0) {
            ...
            // pay the tax!
            IERC20(pool.liquidityToken).transfer(pool.taxAddress, burn);
        }
    }
    ...
    function claimSlot(uint poolId, uint slotId, uint newBurnRate, uint newDeposit) external {
        ...
        if (slot.owner == address(0)) {
            ...
            if (slot.depositWei > 0) {
                IERC20(pool.liquidityToken).transfer(slot.owner, slot.depositWei);
            }
        }
        ...
        // transfer the tokens!
        if (newDeposit > 0) {
            IERC20(pool.liquidityToken).transferFrom(msg.sender, address(this), newDeposit);
        }
    }
    ...
    function withdrawFromSlot(uint poolId, uint slotId) external initializedPoolOnly(poolId) {
        ...
        // if there's any deposit left,
        if (slot.depositWei > 0) {
            IERC20(pool.liquidityToken).transfer(owner, slot.depositWei);
        }
    }
