...
contract PunkGenerations is ERC721Enumerable, Ownable {
    ...
    uint16 private totalCap;
    uint8 private generationCap;
    ...
    constructor() ERC721("Punk Generations", "PG") {
        // Rinkeby
    ...
    function composite(bytes1 index, bytes1 yr, bytes1 yg, bytes1 yb, bytes1 ya) internal view returns (bytes4 rgba) {
        rgba = assets.composite(index, yr, yg, yb, ya);
    }
    ...
    function breedPunkAssets(uint16 fatherIndex, uint16 motherIndex) internal returns (bytes memory childAssets) {
        require(fatherIndex >= 0 && fatherIndex < 10000 + punksCount, "Unknown father");
        require(motherIndex >= 0 && motherIndex < 10000 + punksCount, "Unknown mother");
        ...
        if ((fa == 10) || (fa == 11)) { // alien or ape
            ...
            require(fa >= 1 && fa < 5);
            require(ma >= 5 && ma < 9);
            info.isMale = (random() % 2) == 0;
            uint8 low = (ma - 5) < (fa - 1) ? (ma - 5) : (fa - 1);
            uint8 high = (ma - 5) < (fa - 1) ? (fa - 1) : (ma - 5);
            info.punkType = (info.isMale ? 1 : 5) + uint8(low + (random() % (high + 1 - low)));
        }
    ...
    function mintChildPunks(uint16 fatherIndex, uint16 motherIndex) external {
        require(punksCount < totalCap, "Total cap reached");
        ...
        require(childGen < generationCap, "Generation cap reached");
