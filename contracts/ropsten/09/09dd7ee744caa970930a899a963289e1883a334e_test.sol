...
contract test is Context, IERC20, Ownable {
    ...
    uint256 public _aggregateFee; //(liquidity fee + marketing fee)
    uint256 private _previousAggregateFee = _aggregateFee;
    ...
    constructor () public {
        ...
        marketingWallet  = payable(0x9Aca76481EE939ccf9fD4C61ae1A8Cd8DE5FebEB);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingFeeAmt = Balance.div(_marketingFee);
        uint256 liquidityFeeAmt = Balance.sub(marketingFeeAmt);
        marketingWallet.transfer(marketingFeeAmt);

        // add liquidity to uniswap
        addLiquidity(otherHalf, liquidityFeeAmt);
        
        emit SwapAndLiquify(half, liquidityFeeAmt, otherHalf);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_aggregateFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _aggregateFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousAggregateFee = _aggregateFee;
    ...
    function restoreAllFee() private {
        ...
        _aggregateFee = _previousAggregateFee;
    }
    ...
    function setAggregateFee(uint256 aggFee) external onlyOwner() {
        _aggregateFee = aggFee;
    }

    function setMarketingFee(uint256 mktFee) external onlyOwner() {
