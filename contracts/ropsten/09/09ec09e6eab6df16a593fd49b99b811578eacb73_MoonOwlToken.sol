...
interface IBEP20 {
     ...
     *
     * Returns a boolean value indicating whMATICer the operation succeeded.
     *
...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - see https://github.com/MATICereum/solidity/issues/2691
        return msg.data;
    }
}
...
library Address {
     ...
     *
     * https://eips.MATICereum.org/EIPS/eip-1884[EIP1884] increases the gas cost
...
contract MoonOwlToken is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    
    address constant blackholeAddress = 0x000000000000000000000000000000000000dEaD;
    address public charityAddress = 0x41f1F4276922e5fa0743b8aEbf5Ee6FAF8613722;
    bool public liquidityAdded;
    bool public moonMissionStarted;
    ...
    string private _name = "Moon Owl";
    string private _symbol = "MOWL";
    uint8 private _decimals = 9;

    IQuickSwapRouter02 immutable public quickSwapRouter;
    ...
    constructor (
        ...
        IQuickSwapRouter02 _quickSwapRouter = IQuickSwapRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
       ...
       _approve(address(this), address(_quickSwapRouter), 2 ** 256 - 1);
    ...
    function _calculateMATICReward(
    ...
    //to recieve MATIC from pancakeRouter when swaping
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !moonMissionStarted){
            revert();
        }
    ...
    //this mMATICod is responsible for taking all fee, if takeFee is true
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 forCharity = 0;
        uint256 forBurn = amount.mul(burnRate).div(100);
       
        
         if (_isExcluded[sender] && !_isExcluded[blackholeAddress]) {
            _transferFromExcluded(sender, blackholeAddress, forBurn);
        } else if (!_isExcluded[sender] && _isExcluded[blackholeAddress]) {
            _transferToExcluded(sender, blackholeAddress, forBurn);
        } else if (!_isExcluded[sender] && !_isExcluded[blackholeAddress]) {
            _transferStandard(sender, blackholeAddress, forBurn);
        } else if (_isExcluded[sender] && _isExcluded[blackholeAddress]) {
            _transferBothExcluded(sender, blackholeAddress, forBurn);
        } else {
            _transferStandard(sender, blackholeAddress, forBurn);
        }
        
         if (sender != address(this) && liquidityAdded){
               if (recipient == quickSwapPair){ 
               uint256 balanceBefore = balanceOf(address(this));
               forCharity = amount.div(100);
                      if (_isExcluded[sender] && !_isExcluded[address(this)]) {
                        _transferFromExcluded(sender, address(this), forCharity);
                    } else if (!_isExcluded[sender] && _isExcluded[address(this)]) {
                        _transferToExcluded(sender, address(this), forCharity);
                    } else if (!_isExcluded[sender] && !_isExcluded[address(this)]) {
                        _transferStandard(sender, address(this), forCharity);
                    } else if (_isExcluded[sender] && _isExcluded[address(this)]) {
                        _transferBothExcluded(sender, address(this), forCharity);
                    } else {
                        _transferStandard(sender, address(this), forCharity);
                    }
                    uint256 balanceAfter = balanceOf(address(this));
                    if(balanceAfter > balanceBefore)
                        swapTokensForMATICForCharity(balanceAfter.sub(balanceBefore));
            }
         }
        
          uint256 forTransfer = amount.sub(forBurn).sub(forCharity);

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, forTransfer);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, forTransfer);
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, forTransfer);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, forTransfer);
        } else {
            _transferStandard(sender, recipient, forTransfer);
        }
    ...
    uint256 public disruptiveCoverageFee = 2000 ether; // antiwhale
    ...
    uint256 public _liquidityFee = 8; // 4% will be added pool, 4% will be converted to MATIC
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public rewardThreshold = 1000 ether; // 1000 MATIC
    ...
    function claimMATICReward() isHuman nonReentrant public {	
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own MOWL to claim reward');	
    ...
    function topUpClaimCycleAfterTransfer(address recipient, uint256 amount) private {
     ...
     function swapTokensForMATICForCharity(
        uint256 tokenAmount
    ) public {

        // generate the pancake pair path of token -> wMATIC
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = quickSwapRouter.WETH();
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(quickSwapPair)) // swap 1 time
            ...
            _addLiquidity(owner(), otherPiece, MATICToBeAddedToLiquidity);

            emit SwapAndLiquify(piece, deltaBalance, otherPiece);
        }
    }

    function changeBurnRate(uint256 n) external onlyOwner{
        require(n <= 5, "burnRate can not be more than 5%");
        require(n >= 1, "burnRate can not be less than 1%");
        burnRate = n;
    }
    ...
    function activateContract() public onlyOwner {
        ...
        // protocol
        disruptiveCoverageFee = 1000 ether; //1000 MATIC
        disruptiveTransferEnabledFrom = block.timestamp;
        setMaxTxPercent(1); // 0.1% per transaction
        liquidityAdded = true;
