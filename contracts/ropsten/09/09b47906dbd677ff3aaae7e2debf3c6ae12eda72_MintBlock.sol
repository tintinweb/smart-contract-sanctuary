...
contract ProofOfStakeMineableToken {
    ...
    event Mine(address indexed _address, uint _tokensMining);
}
...
contract MintBlock is ERC20, ProofOfStakeMineableToken, Ownable {
    using SafeMath for uint256;

    string public name = "MintBlock";
    string public symbol = "MIBL";
...
//------------------------------------------------------------------------------
    ...
    function mine() PoSMiner public returns (bool) {
        ...
        if(tokensMining <= 0) return false;
        ...
        emit Mine(msg.sender, tokensMining);
    ...
    function getMiningReward() internal returns (uint) {
        ...
        uint256 blockReward = (currentBlockNumber.sub(lastBlockNumber)).add(tokensPerBlock);
        return blockReward;
    }
