...
contract OwnbitMultiSig {
  ...
  // includes this contract's address and a nonce for replay protection.
  // One option to independently verify: https://leventozturk.com/engineering/sha3/ and select keccak
  function generateMessageToSign(address test,address erc20Contract, address destination, uint256 value) public  view returns (bytes32) {
    //the sequence should match generateMultiSigV2 in JS
    bytes32 message = keccak256(abi.encodePacked(test, erc20Contract, destination, value, spendNonce));
    return message;
  }


function generateWithPrefix(bytes32 message,  string prefix)public view returns(bytes32){
...
function testEncodeOneParameter(string addr)public view returns (bytes32){
bytes32 message = keccak256(abi.encodePacked(addr));
return message;
}
  


function testTwoOneParameter(string addr, uint256 value)public view returns (bytes32){
bytes32 message = keccak256(abi.encodePacked(addr,value));
return message;
}
  


function testTwoOneParameter2(address addr, uint256 value)public view returns (bytes32){
bytes32 message = keccak256(abi.encodePacked(addr,value));
return message;
}
}