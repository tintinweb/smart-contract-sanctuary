...
// File: contracts\token\TeddyToken.sol

// contracts/token/TeddyToken.sol
...
contract TeddyToken is Context,
                     AccessControlEnumerable,
                     ERC20Burnable {

    constructor(
        address owner
    ) ERC20("Teddy Token", "TED") {
        _setupRole(DEFAULT_ADMIN_ROLE, owner);

        _mint(owner, 1000000000000000000 * (10**14));
    }

    function mint(address to, uint256 amount) public virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "TeddyToken: must have admin role to mint");
        _mint(to, amount);
    }

    function burn(address from, uint256 amount) public virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "TeddyToken: must have admin role to burn");
        _burn(from, amount);
    }
