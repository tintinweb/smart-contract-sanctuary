...
interface IMetasaurs {
  ...
  function customThing ( uint256 nftID, uint256 id, string memory what ) external;
  ...
  function presaleBuy ( bytes32 hash, bytes memory sig, uint256 qty, string memory nonce ) external;
  function presaleLive (  ) external view returns ( bool );
  function pricePerToken (  ) external view returns ( uint256 );
  function publicBuy ( bytes32 hash, bytes memory sig, string memory nonce ) external;
