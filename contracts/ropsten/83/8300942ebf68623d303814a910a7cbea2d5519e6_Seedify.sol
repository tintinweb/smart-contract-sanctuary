 ...
 // function to transfer the funds to founders account
  function fundTransfer(uint256 weiAmount) internal {
     ...
     if(msg.sender == addrToWhitelist1 ){
         require(weiAmount <= 1 ,"pass the right amount");
         moneyDivision(msg.value);

     }else if (msg.sender == addrToWhitelist2  ){
         require(weiAmount >1 || weiAmount<= 3,"plz pass the right amount");
         moneyDivision(msg.value);
         
     }else if (msg.sender == addrToWhitelist3 ) {
         require(weiAmount >3 || weiAmount <=5 ," plz pass the right value");
         moneyDivision(msg.value);
     }else{
         revert();
     }
  ...
  function moneyDivision(uint weiAmount) internal {
