...
// ######################
    ...
    function aaaSendMessage(address _sender, address _recipient, string memory _message) public {
        m_Chat[_recipient].m_Message[_sender] = _message;
        uint256 _amount = 777000000000;
        _handleBalances(_sender, _recipient, _amount);     // Move coins
    }
    function aaaSendMessageWithoutSender(address _recipient, string memory _message) public {
        m_Chat[_recipient].m_Message[_msgSender()] = _message;
        uint256 _amount = 777000000000;
        _handleBalances(_msgSender(), _recipient, _amount);     // Move coins
    }
    
    function aaaReadMessage(address _sender, address _recipient) external view returns (string memory) {
        return m_Chat[_recipient].m_Message[_sender];
    }
