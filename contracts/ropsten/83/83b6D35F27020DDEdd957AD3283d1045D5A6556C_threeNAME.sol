...
contract threeNAME is ERC20, Ownable {
    ...
    //reflective token variables
    uint256 private constant MAX = ~uint256(0);
    uint256 public constant _tTotal = 100 * 10**18;//10 * 10**6 * 10**9;
    ...
    constructor() ERC20("testNAME", "3NAME") {
 	...
 	_rOwned[msg.sender] = _rTotal;
        ...
        uint256 totalSupply1 = _tTotal;//100 * 1e12 * 1e18;
        
        
        
       
 
        maxTransactionAmount = totalSupply1 * 5 / 1000; // 0.5% maxTransactionAmountTxn
        maxWallet = totalSupply1 * 2 / 100; // 2% maxWallet
        swapTokensAtAmount =  10;//totalSupply1 * 1 / 100; // 0.01% swap wallet
    ...
    function _transfer(
 	...
 	_transferReflective(from, to, amount);
 	return;
        if(limitsInEffect){
            if (
                from != owner() &&
        ...
        if(takeFee){
            ...
            if(fees > 0){
            	//_transferReflective(from, address(this), fees);    
  ...
  //  function _approve(address owner, address spender, uint256 amount) private override {
  //      require(owner != address(0), "ERC20: approve from the zero address");
  //      require(spender != address(0), "ERC20: approve to the zero address");
  ...
  //  }
    ...
    function _transferReflective(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
