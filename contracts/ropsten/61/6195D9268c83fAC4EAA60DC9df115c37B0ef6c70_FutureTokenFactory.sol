...
contract FutureToken is IFutureToken, ERC20 {
    ...
    constructor(address tokenA, address tokenB, uint256 _expiryDate) ERC20("FutureToken", "FUT", 18) {
        require(_expiryDate >= block.timestamp, "Future Token: EXPIRY_DATE_INVALID");
    ...
    modifier canMint() {
        require(block.timestamp < expiryDate, "Future Token: MINT_AFTER_EXPIRED");
        _;
    }

    modifier canBurn() {
        require(block.timestamp >= expiryDate, "Future Token: BURN_BEFORE_EXPIRED");
        _;
    }
...
contract FutureTokenFactory is IFutureTokenFactory, Ownable {
    ...
    function createFutureToken(
        ...
        require(getFutureToken[token0][token1][deadline] == address(0), "PrecogV2: FUTURETOKEN_EXISTS");
