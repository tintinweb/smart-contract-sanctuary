...
contract TradeMint is ITradeMint, Context, Owned {
        ...
        if (isReward[poolAddress]) {
            ...
            if (userLastReward == 0) {
                userLastReward = userInfo[userAddress][poolAddress]
                    ...
                    .add(
                        FullMath.mulDiv(
                            userInfo[userAddress][poolAddress]
                                .lastRewardVolumeGrowth,
            ...
            if (liquidityIncentiveGrowthInPosition > userLastReward) {
                uint256 liquidityIncentiveGrowthInPosition = liquidityIncentiveGrowthInPosition
                        .sub(userLastReward);
                amount += FullMath.mulDiv(
        ...
        returns (uint256 feeGrowthInside, uint256 blockSettlementVolume)
    {
        ...
        if (
            poolInfo.blockSettlementVolume[
                userInfo[userAddress][poolAddress].lastRewardSettlementedBlock
            ] != 0
        ) {
    ...
    function settlementByTokenId(uint256 tokenId, address userAddress)
        internal
        returns (uint256)
    {
        (
        ...
        if (isReward[poolAddress]) {
            ...
            uint256 userLastReward = getLastReward(userAddress, poolAddress,tokenId);
    ...
    function getLastReward(address userAddress, address poolAddress,uint256 tokenId)
        internal
        view
    ...
    function snapshotLiquidity(
        ...
        if (isReward[_msgSender()]) {
            ...
            userInfo.lastRewardVolumeGrowth = poolInfo.liquidityVolumeGrowth;
            userInfo.lastRewardSettlementedBlock = (
                block.number.div(settlementBlock).add(1)
            ).mul(settlementBlock);
