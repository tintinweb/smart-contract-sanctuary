...
contract Seller is ISeller, Ownable, WeekManaged {
    ...
    function getPendingBasket(address who_, uint8 category_, uint256 week_) public view returns(uint16[] memory) {
        BasketRequest storage request = basketRequestMap[who_][week_][category_];
        return request.assetIndexes;
    }

    function getLatestPendingBasket(address who_, uint8 category_) external view returns(uint16[] memory) {
        uint256 week = getCurrentWeek();

        for (uint i = 2; i >= 0; --i) {
            if (basketRequestMap[who_][week + i][category_].time > 0) {
                return getPendingBasket(who_, category_, week + i);
            }
        }
