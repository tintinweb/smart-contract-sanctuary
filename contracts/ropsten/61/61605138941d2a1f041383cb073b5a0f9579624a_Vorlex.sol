...
contract Vorlex  is ERC1155, ERC1155Holder,  AccessControl  {
    ...
    */
    ...
    event Minted(uint256 indexed vorlex_id, string ipfs, address indexed custodian, uint256 amount);
    ...
    event Burnt(uint256 block_no, string metadata);
    ...
    */
	function mint( string memory ipfs, address custodian, uint256 amount) public virtual {
        ...
        uint256 new_vorlex = _minted_vorlexes[ipfs];
         if(new_vorlex == 0){
            ...
            vorlexes[new_vorlex] = Vorlexy(ipfs, custodian, amount);
        }
        _mint(custodian, new_vorlex, amount, "");
        emit Minted(new_vorlex, ipfs, custodian, amount);
    } 
    ...
    */
    ...
    function batchMove(uint256[] memory vorlex_ids, uint256[] memory amounts, string memory metadata, address from, address to, bool is_swap) public virtual payable {
        ...
        if(to == address(0)){ // If operation is a burn operation
            ...
            emit Burnt(block.number, metadata);
        }
        if(is_swap){
