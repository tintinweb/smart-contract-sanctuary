...
contract Amaber is Ownable
{
    using AmaberSafeMath for uint256;
    ...
    string public name     = "Amaber";
    string public symbol   = "Amaber";
    uint8  public decimals = 9;

    address payable public MarketingAddress = payable(0xc9F24BbE418d9637f19d518f50a2785701037cBD); // Marketing Address
    ...
    uint256 private AmaberScalingFactor  = BASE;
    ...
    uint256 initSupply = 1*10**6 * 10**9;
    uint256 _totalSupply = 1*10**6 * 10**9;
	...
	uint256 public _minTokensBeforeSwap = 1*10**4 * 10**9;
    ...
    {
		require(fee < 95, 'Amaber: Transaction fee should be less than 95%');
        TX_FEE = fee;
    }
    ...
    {
		require(fee < 95, 'Amaber: Sell fee should be less than 95%');
        SELL_FEE = fee;
    }
   ...
   function transfer(address recipient, uint256 amount) public returns (bool) {
	...
	function balanceOf(address account) public view returns (uint256) {
        ...
        if (_isExcluded[account]) return _tOwned[account].mul(AmaberScalingFactor).div(internalDecimals);
	...
	function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "Amaber: approve from the zero address");
        require(spender != address(0), "Amaber: approve to the zero address");
    	...
    	function reflect(uint256 tAmount) private 
	{
        ...
        uint256 TAmount = tAmount.mul(internalDecimals).div(AmaberScalingFactor);
	...
	function _transfer(address sender, address recipient, uint256 amount) private 
	{
		...
		require(sender != address(0), "Amaber: cannot transfer from the zero address");
        require(recipient != address(0), "Amaber: cannot transfer to the zero address");
        require(amount > 0, "Amaber: Transfer amount must be greater than zero");
		
		if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {
            require(amount <= _maxTxAmount, "Amaber: Transfer amount exceeds the maxTxAmount.");
            if((_msgSender() == currentPoolAddress || _msgSender() == address(_uniswapV2Router)) && !tradingEnabled)
                require(false, "Amaber: trading is disabled.");
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Send to Marketing and Developmment contract
	...
	{
		uint256 scaledAmount = amount.mul(AmaberScalingFactor).div(internalDecimals);
		return(scaledAmount);
	}
    ...
    function setBuyBackEnabled(bool _enabled) private onlyOwner {
	...
	function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 10**8 , 'Amaber: maxTxAmount should be greater than 0.1 Amaber');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
	
	function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 1 * 10**9 && minTokensBeforeSwap <= 1*10**6 * 10**9, 'Amaber: minTokenBeforeSwap should be between 1 and 3*10**8 Amaber');
