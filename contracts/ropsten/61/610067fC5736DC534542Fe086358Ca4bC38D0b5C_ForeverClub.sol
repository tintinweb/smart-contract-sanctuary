...
contract ForeverClub is ERC721URIStorage {
    ...
    function _validateTransfer(uint256 tokenId) private {
        ...
        if(block.timestamp > (_transferTimestamp[tokenId] + 10 minutes)) {
            _burn(tokenId);
        }
        require(block.timestamp <= (_transferTimestamp[tokenId] + 10 minutes), "Time expired. No transfers possible.");
    }
