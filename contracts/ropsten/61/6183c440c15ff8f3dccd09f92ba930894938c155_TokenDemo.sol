...
contract TokenDemo is Token {    
    ...
    function TokenDemo(uint256 _initialAmount,address sq,address sm,address yy,address group) public {
	   ...
	   team=group;
	   balances[team]=totalSupply/10*2;//团队账户
	   startTime=now;
    }
    function transfer(address _to, uint256 _value) public returns (bool success) {
		...
		if(msg.sender==simu){//记录私募投资人账户的收币时间与初始额度
			record(_to,_value);
		}
		if(time[msg.sender]>0){//判断发起交易的是否为投资人或组员账户
			judge(_value,msg.sender);//判断发起的交易是否满足时间限制
		}
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        require(balances[_from] >= _value && allowed[_from][msg.sender] >= _value);//判断溢出情况和缺失
