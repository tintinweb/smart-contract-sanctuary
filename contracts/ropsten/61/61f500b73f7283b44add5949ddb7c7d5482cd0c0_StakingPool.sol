...
contract StakingPool is Owned {
    ...
    */
    ...
    function stake(uint256 amount) external {
        ...
        userTotalStaking[msg.sender].lastAccountingTimestampSec = now;
    ...
    function updateAccounting() public returns (
        uint256 unlockedRewards, uint256 userStakingShareSeconds, uint256 totalStakingShareSeconds, uint256 userRewards, uint256 currentTimeStamp) {
        ...
        // User Accounting
        UserTotals storage totals = userTotalStaking[msg.sender];
        uint256 newUserStakingShareSeconds =
            now
            .sub(totals.lastAccountingTimestampSec)
            .mul(totals.totalStaking);
    ...
    //compute rewards
    function computeNewReward(uint256 currentRewardTokens, uint256 stakingShareSeconds) private view returns (uint256 _reward) {
        uint256 rewardsUnlocked = totalUnlocked();
        
        if (rewardsUnlocked > 0) {
            uint256 userRewards = totalUnlocked()
            .mul(stakingShareSeconds)
            .div(_totalStakingShareSeconds);
            
            return currentRewardTokens.add(userRewards);
        } else {
            return 0;
        }
    }
    ...
    function withdrawStaking(uint256 amount) external {
        ...
        while (sharesLeftToBurn > 0) {
            ...
            if (lastStake.amount <= sharesLeftToBurn) {
                ...
                newStakingShareSecondsToBurn = lastStake.amount.mul(stakeTimeSec);
        ...
        totals.totalStaking = totals.totalStaking.sub(amount);
    ...
    function withdrawRewardsOnly() external {
        (uint256 unlockedRewards, uint256 userStakingShareSeconds, uint256 totalStakingShareSeconds, uint256 userRewards, uint256 currentTimeStamp) = updateAccounting();
        require(userRewards > 0, "You do not have enough rewards");
        ...
        while (i > 0) {
            ...
            // fully redeem a past stake
            newStakingShareSecondsToBurn = userStake.amount.mul(stakeTimeSec);
            rewardAmount = computeNewReward(rewardAmount, newStakingShareSecondsToBurn);
            stakingShareSecondsToBurn = stakingShareSecondsToBurn.add(newStakingShareSecondsToBurn);
    ...
    //get totalUnlocked rewards value
    function totalUnlocked() public view returns (uint256 _totalUnlocked) {
        uint256 rewardPerSecond = totalRewards.mul(1 ether);
        if (rewardPerSecond != 0 ) {
            rewardPerSecond = rewardPerSecond.div(poolDuration);
        }
        uint256 totalTimeSec;
        //check if current time is more than pool ending time
        if (now > poolEndTime) {
            totalTimeSec = poolDuration;
        } else {
            totalTimeSec = now.sub(poolStartTime);
        }
        return rewardPerSecond.mul(totalTimeSec).div(1 ether);
    }
