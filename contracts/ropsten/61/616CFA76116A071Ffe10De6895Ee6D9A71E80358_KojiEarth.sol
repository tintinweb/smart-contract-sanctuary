...
interface IDividendDistributor {
    ...
    function subShare(address shareholder, uint256 amount) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    function addShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        if(amount > 0){ 
           ...
           shares[shareholder].amount = amount;
            ...
            if (shares[shareholder].amount < minHoldAmountForRewards) {
    ...
    function subShare(address shareholder, uint256 amount) external override onlyToken {
    ...
    function process() external override {
        ...
        while(iterations < shareholderCount) {
            ...
            balance = IERC20(address(this)).balanceOf(address(shareholders[currentIndex]));
            if (balance < minHoldAmountForRewards) {
                shares[shareholders[currentIndex]].amount = 0;
            } else {
                shares[shareholders[currentIndex]].amount = balance;
            }
    ...
    function shouldDistribute(address shareholder) internal view returns (bool) {
        return getUnpaidEarnings(shareholder) > minDistribution && shares[shareholder].amount >= minHoldAmountForRewards;
    }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            //address payable _holder = payable(shareholder);
    ...
    function holderInfo(address _holder) external view returns (uint256, uint256, uint256) {
        return (shares[_holder].amount, shares[_holder].totalExcluded, shares[_holder].totalRealised);
    }
...
contract KojiEarth is IBEP20, Auth {
    ...
    function _tF(address s, address r, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[s]){ try distributor.subShare(s, _balances[s]) {} catch {} }
        if(!isDividendExempt[r]){ try distributor.addShare(r, _balances[r]) {} catch {} }
    ...
    function swapBack() internal swapping {
        ...
        if (addToLiquid) {
            ...
            if (balance > 0) {IWETH(WETH).transfer(pair, balance);}
        }
    ...
    function _setIsDividendExempt(address holder, bool exempt) internal {
        ...
        if(exempt){
            distributor.subShare(holder, 0);
        }else{
            distributor.addShare(holder, _balances[holder]);
        }
    }
    ...
    function viewHolderInfo(address _address) external view returns (uint256 amount, uint256 excluded, uint256 realised) {
