...
contract MarketSale is IMarketSale, ERC721HolderUpgradeable, OwnableUpgradeable {
    ...
    function _buySale(uint256 tokenId) internal {
        ...
        SaleSucceed(tokenId, price, seller, msg.sender);
    }
    ...
    function withdrawNFTBalance() external onlyOwner {
        _withdrawSaleBalance(_nftContract);
    }

    function getNFTBalance() external view onlyOwner returns (uint256) {
        return _getSaleBalance(_nftContract);
    }

    function _getSaleBalance(address seller) internal view returns (uint256) {
        return _saleBalance[seller];
    }

    function getSaleBalance(address seller) external view override returns (uint256) {
        return _getSaleBalance(seller);
    }

    function withdrawSellerSaleBalance(address seller) external {
        _withdrawSaleBalance(seller);
    }

    function withdrawSaleBalance() external override {
        _withdrawSaleBalance(msg.sender);
    }

    function _withdrawSaleBalance(address seller) internal {
        uint256 balance = _saleBalance[seller];
        require(balance >= _minFee, "Too little balance");
        _saleBalance[seller] = 0;
