...
/**
    @title GenerativeEdition
    @author jongold.eth
    @dev Generative NFT series with pseudo-random hashes & births

    based on Iain Nash's NFT Editions
    https://github.com/ourzora/nft-editions
*/

contract GenerativeEdition is ERC721, IEdition, IERC2981, Ownable {
    using Counters for Counters.Counter;
    using Strings for string;

    mapping(uint256 => bytes32) tokenIdToHash;
    mapping(uint256 => uint256) tokenIdToBirth;
    ...
    /**
      ...
      @param _baseUri Address of the metadata server
     ...
     */
    function owner() public view override(Ownable, IEdition) returns (address) {
        return super.owner();
    }
        ...
        returns (uint256)
    {
        ...
        while (atEditionId.current() <= endAt) {
            ...
            _updateEntropy();
            tokenIdToHash[atEditionId.current()] = entropyBase;

            tokenIdToBirth[atEditionId.current()] = block.number;
    ...
    */
    ...
    function tokenInfo(uint256 tokenId)
        public
        view
        returns (bytes32 hash, uint256 birth)
    {
        require(_exists(tokenId), "No token");

        return (tokenIdToHash[tokenId], tokenIdToBirth[tokenId]);
    }
