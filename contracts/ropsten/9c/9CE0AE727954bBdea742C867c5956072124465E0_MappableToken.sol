...
contract MappingTokenFactory is ContextUpgradeSafe, Configurable, Constants {
    ...
    function increaseAuthQuota(address mappingTokenMapped, address signatory, uint increment) virtual public onlyAuthorty returns (uint quota) {
    ...
    function decreaseAuthQuota(address mappingTokenMapped, address signatory, uint decrement) virtual public onlyAuthorty returns (uint quota) {
    ...
    function chainIdMappingTokenMappeds(address tokenOrMappingToken) virtual public view returns (uint[] memory chainIds, address[] memory mappingTokenMappeds_) {
    ...
    function _registerMapping(uint mainChainId, address token, uint[] memory chainIds, address[] memory mappingTokenMappeds_) virtual internal {
        ...
        for(uint i=0; i<chainIds.length; i++) {
            ...
            require(_mainChainIdTokens[mappingTokenMappeds_[i]] == 0, 'mainChainIdTokens exist already');
    ...
    function registerMapping(uint mainChainId, address token, uint[] memory chainIds, address[] memory mappingTokenMappeds_, Signature[] memory signatures) virtual external payable {
        ...
        for(uint i=0; i<N; i++) {
            bytes32 structHash = keccak256(abi.encode(REGISTER_TYPEHASH, mainChainId, token, signatures[i].signatory));
            bytes32 digest = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, structHash));
            address signatory = ecrecover(digest, signatures[i].v, signatures[i].r, signatures[i].s);
            require(signatory != address(0), "invalid signature");
            require(signatory == signatures[i].signatory, "unauthorized");
            _decreaseAuthCount(signatures[i].signatory, 1);
            emit Authorize(mainChainId, token, signatory);
        }
        _registerMapping(mainChainId, token, chainIds, mappingTokenMappeds_);
    }
    event Authorize(uint indexed mainChainId, address indexed token, address indexed signatory);

    function certifiedCount() public view returns (uint) {
    ...
    function allCertifiedTokens() public view returns (string[] memory symbols, uint[] memory chainIds, address[] memory tokens) {
    ...
    function registerCertified(string memory symbol, uint mainChainId, address token) external governance {
        ...
        if(mainChainId == 1 || mainChainId == 3)
    ...
    function createTokenMapped(address token) public payable returns (address tokenMapped) {
        _chargeFee();
    ...
    function createMappableToken(string memory name, string memory symbol, uint8 decimals, uint totalSupply) public payable returns (address mappableToken) {
        _chargeFee();
    ...
    function createMappingToken(uint mainChainId, address token, string memory name, string memory symbol, uint8 decimals, uint cap) public payable returns (address mappingToken) {
        _chargeFee();
