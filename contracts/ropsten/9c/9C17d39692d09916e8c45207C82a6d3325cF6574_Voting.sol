...
//TODO add ipfs voting information for voting strategy lambda
contract Voting {
  struct Option {
    address callTarget;
    bytes callData;
  ...
  IVotingStrategy public immutable votingStrategy;
  Roadmap public immutable roadmap;
  uint256 public immutable timeLockBeforeVoting;
  uint256 public immutable votingDuration;
  uint256 public immutable timeLockBeforeExecution;
  mapping(uint256 => Proposal) public proposals;

  event AddProposal(uint256 indexed id);
  event Vote(
  ...
  event Execute(uint256 indexed proposalId, uint256 indexed optionId);
  ...
  constructor(
    ...
    Roadmap _roadmap,
    uint256 _timeLockBeforeVoting,
    uint256 _votingDuration,
    uint256 _timeLockBeforeExecution
  ) {
  ...
  function vote(
    ...
    uint256 previousOptionId = proposal.selectedOptions[msg.sender];
    require(
      optionId > 0 && optionId < proposal.options.length + 1,
      "Invalid option id"
    );
    require(previousOptionId != optionId, "Already voted for this option");

    if (previousOptionId != 0) {
      Option storage previousOption = getOptionById(proposal, previousOptionId);
    ...
    proposal.selectedOptions[msg.sender] = optionId;
    Option storage option = getOptionById(proposal, optionId);
    option.votingPower += votingPower;
    option.votingPowerByAddress[msg.sender] = votingPower;

    emit Vote(proposalId, optionId, votingPower, ipfsHash);
  }
  ...
  {
    require(
      optionId == maxVotingPowerOption(proposalId),
    ...
    if (optionId == 1) {
      ...
      emit Execute(proposalId, optionId);
    } else {
      //TODO rewrite to NonReentrant
      proposal.executed = true;
      //TODO check validity of such call method
      (bool success, bytes memory data) =
        option.callTarget.call(option.callData);
      if (success) {
        emit Execute(proposalId, optionId);
      } else {
        proposal.executed = false;
        emit ExecutionFailure(proposalId, optionId, getRevertMsg(data));
      }
    }
  }

  //TODO add checks
  function getOptionVotingPower(uint256 proposalId, uint256 optionId)
    external
    view
    returns (uint256)
  {
    return getOptionById(proposals[proposalId], optionId).votingPower;
  }
  ...
  function maxVotingPowerOption(uint256 proposalId)
    ...
    returns (uint256)
  {
    //TODO add handling of two options with the same voting power
    Proposal storage proposal = proposals[proposalId];
    uint256 optionsCount = proposal.options.length;
    uint256 maxVotingPower = 0;
    uint256 maxOptionId = 1;
    for (uint256 i = 1; i <= optionsCount; i++) {
      Option storage option = getOptionById(proposal, i);
  ...
  function getOptionById(Proposal storage proposal, uint256 id)
    private
    view
    returns (Option storage)
  {
    //offset by 1 so the default zero value of a selectedOptions could be distinguished from zero-index option
    return proposal.options[id - 1];
  }
