...
contract FootballBetting {
  ...
  event BetAccepted(address creator, uint id, address acceptor,uint ratio,uint amount);
  ...
  function acceptBet(address betCreator, uint betId, uint ratio) public payable {
    ...
    emit BetAccepted(betCreator, betId, betAcceptor, ratio, totalAmount);
  }
