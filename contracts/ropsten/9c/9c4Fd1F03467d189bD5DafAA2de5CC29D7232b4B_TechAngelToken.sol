...
contract TechAngelToken is ERC20Detailed, ERC20Capped {
    using SafeMath for uint256;
    
    string constant private NAME = "TechAngelToken";
    string constant private SYMBOL = "TAT";
    uint8 constant private DECIMALS = 18;
    uint256 constant private CAP = 999999999 * (10 ** uint256(DECIMALS));

    // Event of transfer of ether from a smart contract to a recipient
    event WagerEther(address recipient, uint256 amount);

    // Count wagers token to recipient
     ...
     */
    function transferTo(address payable to, uint256 amount) public onlyMinter returns (bool) {
        require(to != address(0), "TokenManager: Address Not Found");

        to.transfer(amount);
        _wagers[to] = _wagers[to].add(amount);

        emit WagerEther(to, amount);
        return true;
    }

    /**
     * @dev Ban on mint and transfer tokens.
     * @return A boolean value indicating whether the operation succeeded.
     */
    function setPauseContract() public onlyMinter returns (bool) {
        _pause();
        return true;
    }

    /**
     * @dev Allow on mint and transfer tokens.
     * @return A boolean value indicating whether the operation succeeded.
     */
    function setUnpauseContract() public onlyMinter returns (bool) {
        _unpause();
        return true;
    }

    /**
    * @dev Gets the wager of the specified address.
    * @param owner The address to query the wager of.
    * @return A uint256 representing the amount wagered by the passed address.
    */
    function wagerOf(address owner) public view returns (uint256) {
        return _wagers[owner];
    }
}
...
contract ERC20Capped is ERC20Mintable {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override(ERC20Mintable) {
