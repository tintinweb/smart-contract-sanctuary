...
contract ClaimVoting is IClaimVoting, Initializable, AbstractDependant {
    ...
    function initializeVoting(
        ...
        _votings[claimIndex] = VotingResult(
            coverTokens, 
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (_allClaimsInfo[i - offset].finalVerdict == IClaimingRegistry.ClaimStatus.ACCEPTED) {
                _allClaimsInfo[i - offset].finalClaimAmount = 
                    _votings[claimInfo.index].votedAverageWithdrowalAmount;
            }
        ...
        ) 
    {
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (_myClaimsInfo[i - offset].finalVerdict == IClaimingRegistry.ClaimStatus.ACCEPTED) {
                _myClaimsInfo[i - offset].finalClaimAmount = _votings[index].votedAverageWithdrowalAmount;
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (_myVotesInfo[i - offset].allClaimInfo.finalVerdict == IClaimingRegistry.ClaimStatus.ACCEPTED) {
                _myVotesInfo[i - offset].allClaimInfo.finalClaimAmount = 
                    _votings[myVote.claimIndex].votedAverageWithdrowalAmount;
            }
    ...
    function _calculateAverages(
        ...
        if (votedFor) {           
            ...
            uint256 votedSuggestedPrice = info.votedAverageWithdrowalAmount.mul(
            ...
            info.votedAverageWithdrowalAmount = votedSuggestedPrice.add(voterSuggestedPrice).div(PERCENTAGE_100);
    ...
    function _addAnonymousVote(
        ...
        _allVotesByIndexInst[_voteIndex] = VotingInst(
            claimIndex,
            finalHash,
            encryptedVote,
            voter,
            reputationSystem.reputation(voter),
            0, 0, false, // default values
            VoteStatus.ANONYMOUS_PENDING
        );
    ...
    function exposeVoteBatch(
        ...
        require (claimIndexes.length == suggestedClaimAmounts.length &&
            claimIndexes.length == signatureOfVoteHashes.length, 
            "CV: Length mismatches");
        ...
        for (uint32 i = 0; i < claimIndexes.length; i++) {
            ...
            bytes32 hashOfVote = keccak256(abi.encodePacked(claimIndex, "#", suggestedClaimAmounts[i]));
            bytes32 finalHash = keccak256(abi.encodePacked(hashOfVote, signatureOfVoteHashes[i]));

            require (_allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].finalHash == finalHash,
                "CV: Data mismatches");
            require (_votings[claimIndex].withdrowalAmount >= suggestedClaimAmounts[i], 
            ...
            if (vBMI.totalSupply() == 0 || 
                ...
                if (_votings[claimIndex].votedYesPercentage >= APPROVAL_PERCENTAGE) {
                    ...
                    IPolicyBook(claimInfo.policyBookAddress).commitClaim(
                        claimInfo.claimer, 
                        _votings[claimIndex].votedAverageWithdrowalAmount
                    );
