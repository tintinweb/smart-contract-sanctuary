...
contract SwampWater is ERC20, Ownable {
    ...
    constructor() ERC20("SwampWater", "SWPv4") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(amount > 0);
        bool takeFee;
        if (from != owner() && to != owner() && to != address(this)) {
            ...
            if(contractTokenBalance >= swapTokensAtAmount && !swapping && from != uniswapV2Pair) {
                ...
                uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(contractTokenBalance,0,path,owner(),block.timestamp);
                swapping = false;
            }
        }
        if(takeFee) {
