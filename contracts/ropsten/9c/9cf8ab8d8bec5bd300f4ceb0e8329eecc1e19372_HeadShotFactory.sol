...
contract HeadShotFactory is Context, Ownable {
    ...
    constructor () {
        _salesLedgerAddress = 0xc230CC6f841aB90097B60926e8797b3943aD4485; // in Ropsten
        headShotSalesLedger = IHeadShotSalesLedger(_salesLedgerAddress);

        _tokenLedgerAddress = 0xEEf407496f468A974D9527f9Fe49b639571AFf92; // in Ropsten
    ...
    function addToken(uint256 code_, address address_, string memory network_,
        ...
        if (creator_ == owner()){
            ...
            isSell = headShotSalesLedger.spend(creator_, code_, 1);
        }
        uint256 created_ = block.timestamp;
        if (isSell){
            return headShotTokenLedger.addTokenInfo(
                address_, network_, name_, symbol_, decimals_, created_, creator_
            );
        }
        return false;
    }

    function checkAddToken(uint256 code_, address address_)
    public view onlyOwner returns (address, address, address) {
        require(code_ != 0, "ERR: Zero code");
        require(address_ != address(0), "ERR: Zero address");
        require(headShotTokenLedger.getTokenAddress(address_) != address_, "ERR: Token already exist");
        address creator_ = _msgSender();
        //bool isSell = false;
        return (creator_, owner(), address_);
        /*
        if (creator_ == owner()){
            isSell = true;
        } else {
