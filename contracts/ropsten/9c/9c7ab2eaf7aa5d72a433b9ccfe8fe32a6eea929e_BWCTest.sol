...
contract BWCTest is ERC721, Ownable {
	...
	function isValidData(bytes memory sig) internal view returns(bool){
       bytes32 message = keccak256(abi.encodePacked(msg.sender));
       return (recoverSigner(message, sig) == authorizedSigner);
	}
    ...
    function presale(uint amount, bytes memory signature) external payable overallSupplyCheck {
		...
		require(isValidData(signature), "Wallet not authorized to mint during presale");
    ...
    function withdraw() external onlyTeam {
        ...
        payable(developer).transfer(splitBalance);
    }
