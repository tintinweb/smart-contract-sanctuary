...
contract StakingAggregator is StakingAggregatorInterface {
   ...
   * @notice Stake the staking token for the token to be paid as reward.
   */
  function stake(uint128 amount) external override isEnabled {
    _stakingToken.safeTransferFrom(msg.sender, address(this), amount);
    uint256 minUntilNextTerm;
    ...
    for (uint256 i = 0; i < _startTimestampList.length; i++) {
      ...
      if (untilNextTerm < minUntilNextTerm) {
        minUntilNextTerm = untilNextTerm;
        nextStakingContractIndex = i;
      }
    }

    _stakingContracts[nextStakingContractIndex].stakeViaAggregator(msg.sender, amount);
  }

  /**
   * @notice Withdraw the staking token for the token to be paid as reward.
   */
  function withdraw(uint128 amount) external override isEnabled {
    require(amount != 0, "withdrawing amount should be positive number");
    ...
    for (uint256 i = 0; i < _startTimestampList.length; i++) {
      // assert(_startTimestampList[i] <= block.timestamp);
      // untilNextTerm := (int256(_startTimestampList[i]) - int256(block.timestamp)) % _termInterval
      uint256 untilNextTerm =
        _termInterval - ((block.timestamp - _startTimestampList[i]) % _termInterval);
      if (untilNextTerm < minUntilNextTerm) {
        minUntilNextTerm = untilNextTerm;
        nextStakingContractIndex = i;
      }
    ...
    for (
      ...
      (, , , , , uint128 depositAmount, ) = ithStakingContract.getAccountInfo(msg.sender);
      uint128 ithAmount = (amount < depositAmount) ? amount : depositAmount;
      // assert(amount >= ithAmount);
      amount -= ithAmount;

      ithStakingContract.withdrawViaAggregator(msg.sender, ithAmount);
    }
   ...
   * @notice Receive the reward for your staking in the token.
   */
  function receiveReward() external override isEnabled returns (uint256 rewards) {
    for (uint256 i = 0; i < _stakingContracts.length; i++) {
      rewards += _stakingContracts[i].receiveRewardViaAggregator(msg.sender);
    }
  }
