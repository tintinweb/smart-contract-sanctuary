...
contract CreateERC20Minter {
    ...
    function deployERC20Minter(string memory name, string memory symbol, uint8 decimals) public onlyOwner {
    ...
    function computeAddress(string memory name, string memory symbol, uint8 decimals, bytes32 codeHash) public view returns (address) {
        bytes32 salt = keccak256(abi.encodePacked(name, symbol, decimals));
