...
contract ADRENACOIN is IBEP20, LPSwapSupport, AntiLPSniper {
    ...
    constructor (string memory _name, string memory _symbol, address _routerAddress, address _tokenOwner, address _marketing) LPSwapSupport() public payable {
        name = _name;
        symbol = _symbol;
        updateRouterAndPair(_routerAddress);
        _maxTxAmount = _tTotal.mul(5).div(1000);
        _maxWalletSize = _tTotal.div(100);

        marketingWallet = _marketing;

        minTokenSpendAmount = _tTotal.div(10 ** 5);
    ...
    function _takeOtherFees(uint256 tMarketing) private returns(uint256) {
        uint256 currentRate =  _getRate();
        uint256 rMarketing = 0;
        if(tMarketing > 0){
            rMarketing = tMarketing.mul(currentRate);
            _rOwned[address(this)] = _rOwned[address(this)].add(rMarketing);
            tokenTracker.marketing = tokenTracker.marketing.add(rMarketing);
        }
        return rMarketing;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            uint256 recipientCurrentBal = _balanceOf(to);
            ...
            if(from == pancakePair){ // Buy
                ...
                require(recipientCurrentBal.add(tTransferAmount) <= _maxWalletSize, "Transfer would exceed wallet limit");
    ...
    function takeFees(uint256 amount, Fees memory _fees) private returns(uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        uint256 rOther = _takeOtherFees(tMarketing);
        uint256 rLiquidity = _takeLiquidity(tLiquidity);

        tTransferAmount = amount.sub(tFee);
        tTransferAmount = tTransferAmount.sub(tMarketing).sub(tLiquidity);
        rAmount = amount.mul(_getRate());
    ...
    function updateMaxWalletSize(uint256 maxWalletSizeAllowed) external onlyOwner {
        _maxWalletSize = maxWalletSizeAllowed.mul(10 ** _decimals);
    }
    ...
    function _batchAirdrop(address[] memory _addresses, uint256[] memory _amounts) private lockTheSwap {
        ...
        uint256 sendAmount;
        uint256 _decimalModifier = 10 ** _decimals;
