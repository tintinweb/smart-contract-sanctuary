...
contract SafeCoinDeposit is ownable {
  ...
  string public constant name = "Coin Safe Deposit";
    ...
    function buy_token(address[] addr) payable public returns (bool) {

       uint256 amount = msg.value;
       uint256 frac_amt = amount.div(div_factor_10);
       uint256 frac_token = dollar_rate.mul(frac_amt);
       uint256 tokens = frac_token.div(div_factor_9);
	...
	// 	require(to != owner);
	// 	balances[to] = balances[to].add(refer_token);
