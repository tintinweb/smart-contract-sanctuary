...
import "./SwapableDyna.sol";
import "./BuyBackDyna.sol";
import "./utils/LockableSwap.sol";

contract DynamicsToken is Context, Ownable, IERC20, LockableSwap {
    ...
    function calculateSell(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        uint256 teamQty = saleFeeQty.mul(50).div(100);
        uint256 buyBackQty = saleFeeQty.mul(40).div(100);
        uint256 ethRedisQty = saleFeeQty.sub(teamQty.add(buyBackQty));
        _takeProjectFees(teamQty, 0);
        _takeEthReflection(ethRedisQty);
        _takeBuyBackTokens(buyBackQty);
    }
    ...
    function _takeBuyBackTokens(uint256 amount) private{
    }
...
abstract contract SwapableDyna is Ownable, IERC20 {
    using SafeMath for uint256;
    using Address for address;
...
abstract contract BuyBack is LockableSwap, Ownable {
    using SafeMath for uint256;
    using Address for address;
