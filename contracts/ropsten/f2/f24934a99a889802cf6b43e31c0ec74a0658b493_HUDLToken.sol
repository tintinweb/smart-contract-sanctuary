...
contract HUDLToken is Ownable, ERC20{
    ...
    /* Max pool supply scaled 100 = 1% */
    uint256 poolMaxSize = 100;
    /* Min pool supply scaled 1 = 0.01% */
    uint256 poolMinSize = 1;
    /* Timestamp for tracking */
    uint256 lastPoolTimeStamp = block.timestamp;
    /* Distributor contract address */
    address distributorAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    /* HUDL pool contract address */
    address hudlPoolAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    ...
    function distributorMint() external onlyOwner{
    ...
    function hudlCreatePool(uint256 size) external onlyOwner{
        ...
        if(block.timestamp - lastPoolTimeStamp > 24 hours){
            ...
            tempPool = poolMaxSize + ((100-poolMaxSize)*((block.timestamp - lastPoolTimeStamp) / 24 hours));
        }
        ...
        if(((size * scalar) / amountInReserve) < poolMinSize){
            ...
            if(poolMaxSize - ((size * scalar) / amountInReserve) > 30){
                poolMaxSize = poolMaxSize - (((size * scalar) / ((amountInReserve * 70) / 100)));
            }
            else{
                poolMaxSize = 30;
            }
