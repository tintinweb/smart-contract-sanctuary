...
interface ICRD is IERC20 {
    ...
    function masterAddr() external view returns (address);
...
contract CRDStaking is Context, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_stakingToken != address(0), "CRDStaking: ZERO_STAKING_ADDRESS");
        require(_crdToken != address(0), "CRDStaking: ZERO_CRDTOKEN_ADDRESS");
        require(_startClaimBlockIndex >= _startStakingBlockIndex, "CRDStaking: INVALID_START_CLAIM_BLOCK_INDEX");
    ...
    function stake(uint amount) external nonReentrant _logs_ {
        require(!_stakingPaused, "CRDStaking: STAKING_PAUSED");
        require(amount > 0, "CRDStaking: INVALID_STAKING_AMOUNT");

        require(_msgSender() != ICRD(CRDToken).masterAddr(), "CRDStaking: masterAddr can not be staked");
    ...
    function unstake(uint amount) external nonReentrant _logs_ {
        require(!_withdarawPaused, "CRDStaking: UNSTAKE_PAUSED");
        require(block.number >= startUnstakeBlockIndex, "CRDStaking: UNSTAKE_NOT_STARTED");

        StakingRecord storage rec = _stakingRecords[_msgSender()];

        require(amount > 0, "CRDStaking: ZERO_UNSTAKE_AMOUNT");
        require(amount <= rec.staked, "CRDStaking: UNSTAKE_AMOUNT_EXCEEDED");
    ...
    function claimProfit() external nonReentrant _logs_ {
        require(!_claimProfitPaused, "CRDStaking: CLAIM_PROFIT_PAUSED");
        require(block.number >= startClaimBlockIndex, "CRDStaking: CLAIM_NOT_STARTED");

        uint totalProfit = _getTotalProfit(_msgSender());
        require(totalProfit > 0, "CRDStaking: ZERO_PROFIT");
    ...
    function collect(address token, address to) external nonReentrant onlyOwner _logs_ {
        require(token != StakingToken, "CRDStaking: COLLECT_NOT_ALLOWED");
