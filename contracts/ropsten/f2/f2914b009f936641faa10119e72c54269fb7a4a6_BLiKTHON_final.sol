...
contract BLiKTHON_final is Log, Ownable {
    ...
    // OnlyInterViewer
    function SetEvaluate(uint32 _ApplicantNumber, uint8 _round, uint64 _score) external onlyInterviewer {
        require(ApplicantList[_ApplicantNumber].score.length >= _round);
        
        ApplicantList[_ApplicantNumber].score[_round] = _score;
    }
    
    function GetEvaluate(uint32 _ApplicantNumber, uint8 _round) external view returns(uint64) {
