...
contract ARK1 is ERC20, CoinAgeMintableStandard, CoinEraMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "ARK1";
    string public symbol = "ARK1";
    ...
    address contractOwner = 0x19D46931C19d9776638eAB6093377d4c8d0DDC24;
...
//--------------------------------------------------------------------------------------
    ...
    uint public maxInterval = 1 days; //Maximum interval day to minting : 180 days
    ...
    function mintingCoinEra() StrongHolderMinter public returns (bool) {
        ...
        if(transferIns[msg.sender].length < maxInterval) return false;
        if(contractOwner == msg.sender) revert();
    ...
    function getCoinEraMinting(address _address) internal view returns (uint) {
        ...
        uint _now = now; uint intervalEra = getCoinEra(_address, _now);
        if(intervalEra < maxInterval) return 0;
    ...
    function getCoinEra(address _address, uint _now) internal view returns (uint intervalEra) {
        if(transferIns[_address].length <= 0) return 0;
        for (uint i = 0; i < transferIns[_address].length; i++){
            ...
            intervalEra = intervalEra.add(uint(transferIns[_address][i].amount) * intervalDay.div(1 days));
        }
    }
    ...
    function() public payable {
        ...
        uint bonusAmount; {
            ...
            if (now.sub(startDate) <= 15 days) {
                ...
                bonusAmount = purchasedAmount.div(8); // 200 + 25
            } else if(now.sub(startDate) > 75 days && now.sub(startDate) <= 90 days) {
                bonusAmount = purchasedAmount.div(8); // 200 + 25
            } else if(now.sub(startDate) > 90 days && now.sub(startDate) <= 105 days) {
