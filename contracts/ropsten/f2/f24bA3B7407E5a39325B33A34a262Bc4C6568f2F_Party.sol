...
contract Party {
    ...
    function fillQuote(
        ...
        require(sellTokenAddress.approve(spender, type(uint256).max),"Approval invalid");
    ...
    function leaveParty(
        ...
        require(
            ...
            "Leave Party platform signature invalid"
        );
    ...
    function kickParty(
        ...
        require(
            ...
            "Kick party platform signature invalid"
        );
        require(member[quittingMember], "User Address doesn't exist.");
        uint256 balanceBeforeSwap = getPartyBalance();
        if(tokenData.length > 0){
            swapPlaceHolder(tokenData); 
        }
    ...
    function swapPlaceHolder(SwapWithoutRSV[] memory tokenData) internal isAlive returns (uint256) {
        uint256 fee = 0;
        for (uint256 index = 0; index < tokenData.length; index++) {
            fee = (tokenData[index].buyAmount * 5) / 1000;
            uint256 outAmount = tokenData[index].sellAmount;
    ...
    function closeParty(
        ...
        require(
            ...
            "Close party platform signature invalid"
        );
        if(tokenData.length > 0){
            swapPlaceHolder(tokenData); 
        }
    ...
    function calculatePenalty (uint256 _userBalance, uint256 n) private pure returns (uint256){
        uint256 penalty;
        if (n != 0) {
            uint256 _temp = 17 * 10**8 + (n - 1) * 4160674157;
