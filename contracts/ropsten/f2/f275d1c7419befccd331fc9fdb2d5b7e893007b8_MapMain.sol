...
contract Order {
    ...
    function addOwnerInOrder(uint256 _idOrder, address _buyer, uint256 _amount) public returns (bool){
        ...
        require(orders[_idOrder].status, "Order: Status is false");
        ...
        orders[_idOrder].child[owner] -= _amount;
        return true;
    }
    ...
    function detailOrder(uint256 _idOrder) public view returns (NodeRes[] memory){
        ...
        for(uint i = 0; i < length; i++){
            ...
            datas[i].tmpStatus = orders[_idOrder].status;
        }
        return datas;
    }
...
contract ERC20 is IERC20, Context{
    ...
    function transferFrom(address _sender, address _recipient, uint256 _amount) public override returns (bool){
        ...
        uint256 currentAllowance = allowances[_sender][_recipient];
        require(currentAllowance >= _amount, "ERC20: transfer amount exceeds allowance");
        unchecked {
            _transfer(_sender, _recipient, _amount);
            // _approve(_sender, _msgSender(), currentAllowance - _amount);
            _approve(_sender, _recipient, currentAllowance - _amount);
        }
        
        return true;
    }
    
    function allowance(address _owner, address _sender) public view override returns (uint256){
...
contract MapMain is ERC20, Order{
    ...
    function buyOrder(uint256 _idOrder, address _sender, uint256 _amount) public returns(bool){
        ...
        approveFrom(_sender, adminAddress, tmpAllowances);
    ...
    function getBalancesByUsers(address[] memory _owners) public view returns (BalanceUser[] memory){
    ...
    function disbursement(uint256 _idOrder, address _owner) public {
        Node storage item = orders[_idOrder];
        uint256 amount = item.child[_owner];
        uint256 budget = item.budget;
        require(_owner == item.owner, "Order: You don't have owner");
        require(amount == 0, "Order: You can't disbursement. Because your amount is not zero");
        approveFrom(adminAddress, _owner, budget);
        transferFrom(adminAddress, _owner, budget);
    }
    
    function cancelOrder(uint256 _idOrder, address _owner) public {
        Node storage item = orders[_idOrder];
        address[] storage buyers = item.members;
        address nodeOwner = item.owner;
        require(_owner == nodeOwner, "Order: You don't have owner");
        if(buyers.length == 0){
            orders[_idOrder].status = false;
        }else{
            for(uint i = 0; i < buyers.length; i++){
                uint256 currentAmount = item.child[buyers[i]];
                if(currentAmount > 0){
                    transfer(buyers[i], currentAmount);
                    emit Transfer(adminAddress, buyers[i], currentAmount);
                }
            }
        }
    }
