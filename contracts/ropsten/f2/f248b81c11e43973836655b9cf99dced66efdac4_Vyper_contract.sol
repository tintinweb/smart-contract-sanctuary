...
INITIAL_SUPPLY: constant(uint256) = 1023912500
...
def decimals() -> uint256:
    ...
    """
    return 3  # amount is denominated in mg
...
def _adjusted_ts(_ts: uint256) -> uint256:
    if _ts >= self.last_ts.ts:
        return self.last_ts.adjusted
    power: uint256 = 1
    for i in range(MINT_PERIODS):
        if self.adjusted_ts[i].ts + HALVING_PERIOD > _ts:
            return self.adjusted_ts[i].adjusted + (_ts - self.adjusted_ts[i].ts) / power
...
def _to_settle(_owner: address, settlement_ts: uint256) -> uint256:
    ...
    return period * rate / COLLECT_PERIOD
