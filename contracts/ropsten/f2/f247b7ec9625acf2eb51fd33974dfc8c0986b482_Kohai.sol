...
contract Kohai is IERC20, Context {
    ...
    uint8 public stakeRate;
    uint8 public lockRate;
    string private _symbol;
    string private _name;
    uint256 private _decimals = 18;
    uint256 public cap = 42 * 1e18;
...
modifier onlyOwner(){
    ...
    ****************************************/
    ...
    function _harvest(address sender) internal{
        ...
        //require (block.timestamp >= lockingTimeEnds[msg.sender], 'Locking time still remains');
        uint256 time = lockingTimeEnds[sender].sub(lockingTimeStarts[sender]);
        uint256 reward = lockedAmount[sender].mul(lockRate).mul(time).mul(lockReward).div(totalLocked);
        lockingTimeStarts[sender] = 0;
        lockingTimeEnds[sender] = 0;
        lockedAmount[sender] = 0;
