...
contract GGRT is IERC20, Ownable {
    ...
    string private _name = "Gratia Governance Token";
    string private _symbol = "GGRT";
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "GGRT::deliver: excluded addresses cannot call this function");
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns (uint256) {
        require(tAmount <= _tTotal, "GGRT::reflectionFromToken: amount must be less than supply");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns (uint256) {
        require(rAmount <= _rTotal, "GGRT::tokenFromReflection: amount must be less than total reflections");
        
        return _tokenFromReflection(rAmount, _getCurrentRate());
    }

    function excludeFromReward(address account) public onlyOwner {
        require(!_isExcluded[account], "GGRT::excludeFromReward: account is already excluded");
    ...
    function includeInReward(address account) external onlyOwner {
        require(_isExcluded[account], "GGRT::includeInReward: account is already included");
	...
	function setSwapAndLiquifyAddress(address newAddress) public onlyOwner {
        ...
        require(priviousAddress != newAddress, "GGRT::setSwapAndLiquifyAddress: same address");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "GGRT::_approve: approve from the zero address");
        require(spender != address(0), "GGRT::_approve: approve to the zero address");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "GGRT::_transfer: transfer from the zero address");
        require(to != address(0), "GGRT::_transfer: transfer to the zero address");
        require(amount > 0, "GGRT::_transfer: amount must be greater than zero");
        require(_isExcludedFromMaxTxAmount[from] || _isExcludedFromMaxTxAmount[to] || amount <= maxTxAmount, "GGRT::_transfer: transfer amount exceeds the maxTxAmount.");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(signatory != address(0), "GGRT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "GGRT::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "GGRT::delegateBySig: signature expired");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldROwned, uint256 oldTOwned, uint256 newROwned, uint256 newTOwned) private {
        uint32 blockNumber = safe32(block.number, "GGRT::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function _writeRateCheckpoint(uint32 nCheckpoints, uint256 oldRate, uint256 newRate) private {
        uint32 blockNumber = safe32(block.number, "GGRT::_writeRateCheckpoint: block number exceeds 32 bits");
