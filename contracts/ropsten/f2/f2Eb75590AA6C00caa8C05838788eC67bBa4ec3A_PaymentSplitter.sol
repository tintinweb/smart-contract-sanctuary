...
contract PaymentSplitter {
    ...
    mapping(address => uint) assignedSplliters;
    mapping(address => bool) enabledAutoAddresses;
    ...
    function createSplitter(address[] memory addresses, uint[] memory percentages) public returns (uint) {
        ...
        require(percentagesSum == 100000, "percentages sum must be 100%");
    ...
    function pay(uint id) public payable {
        ...
        uint length = splitterAddresses[id].length;
        for (uint i = 0; i < length; i++) {
            payable(splitterAddresses[id][i]).transfer(msg.value * splitterPercentages[id][i] / 100000);
        }
    }
    
    function assignSplitterToAddress(uint splitterId) public {
        require(splitterId < totalSplitters, "splitter does not exists");
        
        enabledAutoAddresses[msg.sender] = true;
        assignedSplliters[msg.sender] = splitterId;
    }
    
    
    receive() external payable {
        require(enabledAutoAddresses[msg.sender], "no splitter assigned to this address");
        
        uint id = assignedSplliters[msg.sender];
        uint length = splitterAddresses[id].length;
        for (uint i = 0; i < length; i++) {
            payable(splitterAddresses[id][i]).transfer(msg.value * splitterPercentages[id][i] / 100000);
        }
