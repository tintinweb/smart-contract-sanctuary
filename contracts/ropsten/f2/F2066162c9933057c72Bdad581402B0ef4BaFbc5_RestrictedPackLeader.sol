...
contract RestrictedPackLeader is Ownable, ReentrancyGuard, EIP712 {
    ...
    function getUserMultiplier(address _user, uint256 _to) public view returns (uint256) {
        uint256 _from = escrow.getSellerLastTxnTime(_user);

        uint256 day = 1 days;
        uint256 fromToDiff = _to.sub(_from);

        // returns a multiplier based on the user's last txn
        // if it was over than a day ago, multiplier is 0

        if (fromToDiff <= day) {
            return day.sub(fromToDiff).div(day);
        } else {
            return 0;
        }
    }
    ...
    function payOrLockupPendingAuro(uint256 _pid) internal {
        ...
        uint256 userMul = getUserMultiplier(_msgSender(), block.timestamp);

        uint256 pending = user.amount.mul(pool.accAuroPerShare).div(1e12).sub(user.rewardDebt).mul(userMul);
...
interface IEscrowController {
    ...
    function getSellerScore(address seller) external view returns(uint256);
    function updateSellerScore(address seller, uint256 amount) external;
