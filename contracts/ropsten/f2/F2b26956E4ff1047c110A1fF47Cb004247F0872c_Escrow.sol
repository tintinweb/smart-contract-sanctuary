...
contract Escrow{
   ...
   // Defining function modifier 'onlyBuyer'
    ...
    function createEscrow(address payable seller) public payable{
        ...
        pair[1] = seller;
        escrowPair[lastIndex] = pair;
        escrowState[lastIndex] = State.await_delivery;
        escrowAmount[lastIndex] = msg.value;
        lastIndex++;
    }
    ...
    function confirmDelivery(uint32 ind) onlyBuyer(ind) instate(State.await_delivery) public{
        require(address(this).balance > (escrowAmount[ind] / 100 * (100 - arbiterFee)), "Balance is lower than the amount to transfer!");
        escrowPair[ind][1].transfer(escrowAmount[ind] / 100 * (100 - arbiterFee));
    ...
    function returnPayment(uint32 ind) onlySeller(ind) instate(State.await_delivery)public{
        require(address(this).balance > (escrowAmount[ind] / 100 * (100 - arbiterFee)), "Balance is lower than the amount to withdraw!");
        escrowPair[ind][0].transfer(escrowAmount[ind] / 100 * (100 - arbiterFee));
        escrowState[ind] = State.incomplete;
    }
