...
contract TEST {
    ...
    function getExitFee() public view returns (uint8) {
        ...
        } else if (lifetime > 6 && lifetime <= 14) {
            return exitFeeD0_ - 5; // 25%
        } else if (lifetime > 14 && lifetime <= 21) {
            return exitFeeD0_ - 10; //20%
        } else if (lifetime > 21 && lifetime <= 28) {
            return exitFeeD0_ - 15; // 15%
        } else {
            return exitFee_ + getExitFee2(); // 4%
        }
    }
    ...
    function purchaseTokens(uint256 _incomingEthereum, address _r1, address _r2, address _r3, address _r4, address _r5) internal {
        ...
        if ( investment <= 1 ether) { 
            ...
            discountedFee = entryFee_ - 1; // 9%
        } else if ( investment > 2 ether && investment <= 3 ether) {
            discountedFee = entryFee_ - 2; // 8%
        } else if ( investment > 3 ether && investment <= 4 ether) {
            discountedFee = entryFee_ - 3; // 7%
        } else if ( investment > 4 ether && investment <= 5 ether) {
            discountedFee = entryFee_ - 4; // 6%
        } else if ( investment > 5 ether) {
            discountedFee = entryFee_ - 5; // 5%
        }
