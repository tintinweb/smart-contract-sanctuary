...
contract ShibaStake is WhitelistAdminRole {
    ...
    function STAKEbsc(uint256 tokens) external {
        require(IERC20(SHIB).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function addSHIB(address _SHIB, address _Weth) public onlyWhitelistAdmin {
        require(SHIB == address(0), 'Cannot set twice.');
        SHIB = _SHIB; 
        wETH = _Weth;
    }

    function ADDFUNDS(uint256 tokens) external {
        require(IERC20(wETH).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function CLAIMREWARDbsc() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(wETH).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
   ...
   function WITHDRAWbsc(uint256 tokens) external {
       ...
       uint256 owing = pendingReward(msg.sender);
        ...
        require(IERC20(SHIB).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
   ...
   function yourStakedSHIB(address staker) external view returns(uint256 stakedSHIB){
        return stakers[staker].stakedTokens;
    }
        
     function yourSHIBBalance(address user) external view returns(uint256 SHIBBalance){
        return IERC20(SHIB).balanceOf(user);
    }
}