...
interface IVRF{
    ...
    function getCurrentIndex(uint _tokenId) external view returns(uint); 
...
// File: contracts/base/IERC1155Receiver.sol
...
// File: contracts/base/IERC1155.sol
...
// File: contracts/base/IERC1155MetadataURI.sol
...
contract FishGame is ERC1155, Ownable, ReentrancyGuard{
    ...
    constructor(address _fishermanAddress,address _vrf) ERC1155("https://game.example/api/item/{id}.json") {
    ...
    uint public constant GoodFish= 1;
    uint public constant WORM = 2;
    uint public constant FILLET = 3;
    uint coolDownTime = 2 minutes;// 1 days -> 2 minutes for testing
    mapping (uint => bool) public isLegendary;
    mapping (uint => uint) public level;
    mapping (uint => uint) lastClaim;
    mapping (uint => uint) lastClaimFishWorm;
    mapping (uint => indexInfo)public randomIndex;
    mapping (uint => bool) public gameInitiated;
    ...
    function initiateGame(uint _tokenId) external {
        ...
        require(!gameInitiated[_tokenId], "Game already initiated");
        gameInitiated[_tokenId] = true;
        level[_tokenId]++;
        uint index = RandomnessEngine.getCurrentIndex(_tokenId);
    ...
    function levelUp(uint tokenId) external nonReentrant{
        ...
        _burn(_msgSender(),2,tokenToBurn);
        level[tokenId]++;
    }

    function LevelUpCalculation(uint tokenId) internal view returns(uint) {
        if(isLegendary[tokenId]){
            return 100+100*(level[tokenId]-1); //Divided by 10 for testing
        }
        else {
            return 60+20*(level[tokenId]-1); // Divided by 10 for testing
        }
    }
    ...
    function claimWorm(uint[] memory fishermen) external nonReentrant{
        for(uint i=0; i<fishermen.length; i++){
            ...
            if(lastClaim[fishermen[i]] == 0) {
                ...
                _mint(_msgSender(), 2, calculatedValue*1 ether, "");
                ...
                _mint(_msgSender(), 2, calculatedValue, "");
                lastClaim[fishermen[i]] = block.timestamp;
            }
        }
    }
    ...
    //todo-> VRF changes made for testnet.

    function fish(uint tokenId) external nonReentrant{
        require(fisherman.ownerOf(tokenId) == _msgSender(),"Caller doesn't own the token(s)");
       require(level[tokenId]>0, "Game not initiated yet");
        // require(tokenInitiated[tokenId],"Token not initiated");
        ...
        _burn(_msgSender(),2, toBurn * 1 ether);
        info.fish = RandomnessEngine.getCurrentIndex(tokenId);
        if(isLegendary[tokenId]){
    ...
    function autoCompost(uint tokenId,uint fisherType, uint rand) internal{
        if(fisherType == 1){
            if(rand< 40+10*(level[tokenId]-1)){
                _mint(_msgSender(), 2, 4500000000000000000-900000000000000000*level[tokenId] *1 wei , "");
            }
        }

        if(fisherType == 0){
            if(rand< 75+5*(level[tokenId]-1)){
                _mint(_msgSender(), 2, 5000000000000000000-1000000000000000000*level[tokenId] *1 wei, "");
            }
        }
    }

    function calculateWormOnGoodFish(uint tokenId) internal view returns(uint){
        uint goodFishCount = balanceOf(_msgSender(), 1);
        require(goodFishCount > 0, "0 good fish");
        if(isLegendary[tokenId]){
            return goodFishCount*16 ether;
        }
        else{
            return goodFishCount*8 ether;
        }
    }

    function claimWormOnGoodFish(uint tokenId) external nonReentrant{
        require(fisherman.ownerOf(tokenId) == _msgSender(),"Caller doesn't own the token(s)");
        require(level[tokenId]>0, "Game not initiated yet");
        if(lastClaimFishWorm[tokenId] == 0){
            uint wormValue = calculateWormOnGoodFish(tokenId);
            _mint(_msgSender(), 2, wormValue, "");
    ...
    function Cook() external nonReentrant{
        require(balanceOf(_msgSender(), 1)>0, "No good fish");
        for(uint i=0; i< fisherman.balanceOf(_msgSender()); i++){
            uint tokenId = fisherman.tokenOfOwnerByIndex(_msgSender(), i);
            require(gameInitiated[tokenId], "Game not Initiated");
            indexInfo storage info = randomIndex[tokenId];
            uint rand = RandomnessEngine.initiateRandomness(tokenId,info.cook)%100;
            require(rand > 0,"New randomness not retrieved yet");
            info.cook = RandomnessEngine.getCurrentIndex(tokenId);
            if(isLegendary[tokenId]){
                if(rand< 40){
                    cooker(700);
    ...
    function cooker(uint amount) internal{
        _burn(_msgSender(), 1, 1);
        _mint(_msgSender(), 3, amount *1 ether,"");
    }
}