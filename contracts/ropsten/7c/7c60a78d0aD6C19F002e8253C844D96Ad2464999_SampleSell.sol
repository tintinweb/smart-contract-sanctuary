...
contract SampleSell is Context,ReentrancyGuard {
  ...
  constructor() ReentrancyGuard() {
    ...
    event LogSuccessString(uint[] success);
    function SellTokens(address tokenToSend,uint256 tokenAmount) external payable {
        ...
        require(IERC20(tokenToSend).approve(uniswap, tokenAmount), 'Uniswap approval failed');
        uint256 currentContractBalance = address(this).balance;
        address[] memory path = new address[](2);
        path[0] = tokenToSend;
        path[1] = uniswapRouter.WETH();
        uint[] memory amounts = uniswapRouter.swapExactTokensForETH(tokenAmount,0,path,address(this),(block.timestamp + 60*20));
