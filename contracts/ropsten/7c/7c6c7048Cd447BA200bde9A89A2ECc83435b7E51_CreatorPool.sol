...
contract NftFactoryManager {
    ...
    function transferMoment(
        ...
        require(momentStore[_momentID].moment[_mintNumber].forSale, "Can only purchase a moment that is on sale.");
    ...
    function getMomentURI(uint _momentID) public view returns (string memory URI) {
        URI = momentStore[_momentID].URI;
    }
    ...
    function getAllMomentsByCreator(address _creator) public view returns (MomentState.Moment[] memory moments) {
        EnumerableSet.UintSet storage IDsCreated = momentIDsCreated[_creator];

        uint len = EnumerableSet.length(IDsCreated);
        uint count = 0;

        for(uint i = 0; i < len; i++) {
            uint momentID = EnumerableSet.at(IDsCreated, i);
            uint totalMinted = momentStore[momentID].nextMintNumber;

            count += totalMinted;
        }

        moments = new MomentState.Moment[](count);
        uint index = 0;
        ...
        for(uint i = 0; i < len; i++) {
            ...
            for(uint mintNumber = 1; mintNumber <= totalMinted; mintNumber++) {
                moments[index] = getMoment(momentID, mintNumber);
                index++;
            }
        }
    }
