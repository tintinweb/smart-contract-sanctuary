...
contract Token is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        _balances[to] = _balances[to].add(amount).sub(marketingAmt).sub(liquidityAmt);
        
        if (marketingAmt != 0) {
            _marketingFee = 0;
            _transfer(from, marketingWallet, marketingAmt);
            _marketingFee = _previousmarketingFee;
        }
        if (liquidityAmt != 0 && swapEnabled) {
            _liquidityFee = 0;
            swapAndLiquify(liquidityAmt);
