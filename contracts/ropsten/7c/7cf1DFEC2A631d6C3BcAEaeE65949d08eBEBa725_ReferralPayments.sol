...
contract ReferralPayments is ReentrancyGuard {
    ...
    constructor(SupportedToken[] memory supportedTokens) {
        ...
        for (uint256 counter = 0; counter < supportedTokens.length; counter++) {
    ...
    function _addToken(address token, string memory symbol) internal {
        require(
            bytes(tokensSymbols[token]).length == 0,
            "ReferralPayments: token already exists"
        );
    ...
    function addClaimableAmount(
        ...
        totalUnclaimed[tokenAddress] = totalUnclaimed[tokenAddress].add(amount);
    ...
    function claim(address owner) external nonReentrant {
        for (uint256 counter = 0; counter < tokens.length; counter++) {
            ...
            _claim(owner, SupportedToken(tokenSymbol, tokenAddress));
        }
    }

    function _claim(address owner, SupportedToken memory supportedToken)
        internal
    {
        uint256 amountToClaim = claimableAmounts[supportedToken.symbol][owner];
