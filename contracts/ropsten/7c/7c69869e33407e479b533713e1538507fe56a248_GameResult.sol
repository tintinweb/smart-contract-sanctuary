...
contract GameResult {
    ...
    function getrecords() public view returns (string memory ss) {
        ...
        for (uint i = 0; i < records.length; i ++) {
            ...
            ss = append(ss, ",", records[i].submitter, ",");
            ss = append(ss, "", records[i].submit_nonce, "]");
            ss = append(ss, records[i].winner_mask, "(", records[i].winner_score);
            ss = append(ss, ",", records[i].loser_score, ")");
        }
    }
     ...
     */
    function appendRecord(uint _nonce, string memory _id, string memory _submitter, string memory _submit_nonce, string memory _winner_score, string memory _loser_score, string memory _winner_mask) public returns (uint next_nonce) {
