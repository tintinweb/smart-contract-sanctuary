...
contract MappingTokenFactory is ContextUpgradeSafe, Configurable, Constants {
    ...
    function _createMappingToken(uint mainChainId, address token, string memory name, string memory symbol, uint8 decimals, uint cap) internal returns (address mappingToken) {
        ...
        InitializableProductProxy(payable(mappingToken)).__InitializableProductProxy_init(address(this), _MappingToken_, abi.encodeWithSignature('__MappingToken_init(uint256,address,address,string,string,uint8,uint256)', mainChainId, token, _msgSender(), name, symbol, decimals, cap));
