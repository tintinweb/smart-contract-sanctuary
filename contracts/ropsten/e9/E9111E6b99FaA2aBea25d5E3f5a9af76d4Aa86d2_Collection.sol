...
interface IAuction {
  ...
  function setWinner(address newWinner) external;
...
contract DutchAuction is IAuction {
  ...
  function setState(uint newState) external override {
    auctionState = State(newState);
  }
  ...
  function setWinner(address newWinner) external override {
    winner = newWinner;  
  }
  
  // Calculates the current auction price based on parameters and crnt block
  function calculatePrice(uint crntBlock) private correctTime returns(uint256) {
  ...
  function getCurrentPrice() external correctTime override returns(uint256) {
  ...
  // Dutch implementation of 'finalise'. Only needed to mark NFT sale as done
