...
/**
 * @title TNWGSale contract
 * @author https://grox.solutions
 */
contract TNWGSale is Ownable {
    ...
    constructor(address USDTAddr, address TNWGAddr, address initialOwner, address initialFeeWallet, address initialFundWallet) public Ownable(initialOwner) {
        require(USDTAddr != address(0) && TNWGAddr != address(0) && initialFeeWallet != address(0) && initialFundWallet != address(0));

        USDT = IUSDT(USDTAddr);
        TNWG = IERC20(TNWGAddr);
