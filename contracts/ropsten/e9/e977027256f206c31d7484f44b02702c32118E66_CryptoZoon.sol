...
contract CryptoZoon is ZoonERC20, ReentrancyGuard {
    ...
    function swapTokens(uint256 contractTokenBalance) private nonReentrant {
        ...
        transferToAddressETH(payable(marketingAddress), address(this).balance);
    }
...
contract ZoonERC20 is Ownable, ERC20 {
    using SafeMath for uint256;

    uint256 public amountPlayToEarn = 10 * 10**6 * 10**18;
    uint256 public playToEarnReward;
    uint256 internal amountFarm = 30 * 10**6 * 10**18;
    uint256 private farmReward;

    ManagerInterface public manager;

    address public marketingAddress;
    uint256 public sellFeeRate = 4;
