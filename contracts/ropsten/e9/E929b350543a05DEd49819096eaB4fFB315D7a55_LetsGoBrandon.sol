...
contract LetsGoBrandon is Context, IERC20, Ownable {
	...
	function _getTValues(uint256 tAmount, bool takeFee, uint256 extraFee) private view returns (ReflectValueSet memory set) {
		...
		set.tOtherFee = tAmount * (fees.marketingFee + fees.liquidityFee + extraFee) / 100;
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if(!automatedMarketMakerPairs[from]) 
		{
			...
			if(automatedMarketMakerPairs[to]){
				//user is swapping his LETS to eth == Selling/withdrawing
				if(block.timestamp < launchedAtTime + 7 days){
					extraFees = 25;
				}
				else if(block.timestamp < launchedAtTime + 14 days){
					extraFees = 10;
				}
	...
	function afterHowManyDays() public view returns (uint256) {
		if(block.timestamp < launchedAtTime + 7 days){
			return 25;
		}
		else if(block.timestamp < launchedAtTime + 14 days){
