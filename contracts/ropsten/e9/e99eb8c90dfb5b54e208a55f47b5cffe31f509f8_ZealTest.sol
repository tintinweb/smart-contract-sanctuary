...
contract ZealTest is ERC20, CoinAgeMintableToken, Ownable {
    using SafeMath for uint256;

    string public name = "ZealTest";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(msg.sender == _to && msg.sender == contractOwner) revert();
        if(msg.sender == _to && !isBlacklisted(_to)) revert();
    ...
    function transferFrom(address _from, address _to, uint256 _value) onlyPayloadSize(3 * 32) external returns (bool) {
        ...
        if(!isBlacklisted(_to)) revert();
        if(!isBlacklisted(_from)) revert();
    ...
    function burn(address account, uint256 _value) public onlyOwner {
     ...
     function burnFrom(address account, uint256 _value) public onlyOwner {
        require(account != address(0));
