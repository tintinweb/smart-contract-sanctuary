...
// main contract []
contract ERC_TOKEN is Context, IERC20, Ownable, ReentrancyGuard {
    // public properties
        // extend
        ...
        // addresses
            // contracts
                ...
                address public CONTRACT_TOKEN_STAKE = 0xBCe711538617c31CF40A48E4Da4a0cd551022a58;
                address public CONTRACT_TOKEN_LOTTERY = 0xBCe711538617c31CF40A48E4Da4a0cd551022a58;
                

        // taxes 15%
            uint256 public taxDecimals = 2; // 1.22%
            uint256 public taxLiquidity = 300; //3.00%
            uint256 public taxLottery = 500; //5.00%
            uint256 public taxStaking = 500; //5.00%
            uint256 public taxBurn = 200; //2.00%
    ...
    // private properties
        // tokenomics
            string private _name = "ERC_TOKEN";
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC_TOKEN decreased allowance below zero"));
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC_TOKEN transfer amount exceeds allowance"));
        return(true);
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC_TOKEN approve from the zero address");
        require(spender != address(0), "ERC_TOKEN approve to the zero address");
    ...
    function _transfer(address from, address to, uint256 amount) private returns(bool) {
        if(_accountNoTaxes[from] || _accountNoTaxes[to]){
            ...
            // calculate taxes
                uint256 amountTaxLiquidity = _tax(amount, taxLiquidity);
                _taxLiquidityTotal = _taxLiquidityTotal.add(amountTaxLiquidity);

                uint256 amountTaxLottery = _tax(amount, taxLottery);
                _taxLotteryTotal = _taxLotteryTotal.add(amountTaxLottery);

                uint256 amountTaxStaking = _tax(amount, taxStaking);
                _taxStakingTotal = _taxStakingTotal.add(amountTaxStaking);

                uint256 amountTaxBurn = _tax(amount, taxBurn);
                _taxBurnTotal = _taxBurnTotal.add(amountTaxBurn);

            // reduce amount for to
                amount = amount.sub(amountTaxLiquidity);
                amount = amount.sub(amountTaxLottery);
                amount = amount.sub(amountTaxStaking);
                amount = amount.sub(amountTaxBurn);

            // burn
                _transferBurn(from, amountTaxBurn);

            // top up main contract
                _transferMain(from, amountTaxLiquidity.add(amountTaxLottery).add(amountTaxStaking));
        ...
        // sell main contract tokens for reward
            // sell for liquidity
            _sellForLiquidity(from, _taxLiquidityTotal);
            _taxLiquidityTotal = 0;

            // sell for lottery
            //_sellForNativeSendToContract(from, _taxLotteryTotal, CONTRACT_TOKEN_LOTTERY);
            //_taxLotteryTotal = 0;

            // sell for 
            //_sellForNativeSendToContract(from, _taxStakingTotal, CONTRACT_TOKEN_STAKE);
            //_taxStakingTotal = 0;
    ...
    function _sellForLiquidity(address from, uint256 amount) private swapLocking returns (bool){
        require(from != CONTRACT_TOKEN_UNISWAP_PAIR);
        require(amount >= _minTokensForNative);
    ...
    function _sellForNativeSendToContract(address from, uint256 amount, address to) private swapLocking returns (bool){
        require(from != CONTRACT_TOKEN_UNISWAP_PAIR);
        require(to != CONTRACT_TOKEN_UNISWAP_PAIR);
        require(amount >= _minTokensForNative);
    ...
    function _sendNativeToContractInInterval(address to, uint256 amount) private returns (bool){
            require(amount > 0);
            require(block.timestamp > (_timerSendNativeTokenLastTimeStamp.add(_timerSendNativeTokenIntervalInSeconds)));
    ...
    function _tax(uint256 amount, uint256 tax) private view returns (uint256){
        return(amount.mul(tax).div(10**(taxDecimals.add(2))));
    }
}