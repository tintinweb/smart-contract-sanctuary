...
abstract contract ChainStakePoolBase is IPool, ReentrancyGuard {
    ...
    struct User {
        ...
        // @dev last reward distribution to user .
    ...
    */
    ...
    for (uint256 i=0 ; i < user.deposits.length ; i++){
     ...
     //unstake deposit with isYeild flag set to false
    ...
    function updateStakeLock(
        ...
        _sync();
        ////check
        _processRewards(msg.sender, false, false);
        // delegate call to an internal function
    ...
    function processRewards() external virtual override {
        // delegate call to an internal function
        _processRewards(msg.sender, true, false);
    }
    ...
    function _stake(
        ...
        if (user.tokenAmount > 0) {
            _processRewards(_staker, false, true);
        }
...
//stake allowed
            ...
            //throw error 
           emit   ProcessRewardCalled(msg.sender , _isStake ," insideerror throw");
            require( vestingStatus ,'Vesting period error : wait untill vesting veriod get over');
            return 0 ;
         }
...
event CalledCalculatePendingReward( address indexed staker , uint256 userLastRewardIndex);
    ...
    internal returns (uint256 totalPendingRewardToTransfer){

    User storage user=users[_staker];

    for(uint256 i= user.userLastRewardIndex ; i < user.rewards.length ; i++){

     Reward storage reward=user.rewards[i] ;

     if( reward.rewardTaken==false){
...
event ProcessRewardCalled(uint256 indexed timestamp , bool istake);
    ...
    function _processRewards(
        ...
        pendingYield = super._processRewards(_staker, _withUpdate, _isStake);
