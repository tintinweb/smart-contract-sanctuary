...
// control KDM inflation by assigning rules 
...
interface DarkMatter is IERC20 {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event FeeAddressUpd (address indexed user, address indexed newAddress);
    event BossAddressUpd(address indexed user, address indexed newAddress);
    ...
    //Notice in case of duplicate LP tokens.
   ...
   // Return reward multiplier over the given _from to _to block.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 KDMReward = multiplier.mul(KDMPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        KDM.mint(BossAddress, KDMReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        require (_pid != 0, 'deposit DarkMatter by staking');
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require (_pid != 0, 'withdraw DarkMatter by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Withdraw DarkMatter tokens from STAKING.
    ...
    function setBossAddress (address _BossAddress) public {
        require(msg.sender == BossAddress, "setBossAddress: FORBIDDEN");
        require(_BossAddress != address(0), "setBossAddress: ZERO");
        BossAddress = _BossAddress;
        emit BossAddressUpd(msg.sender, _BossAddress);
    }
    ...
    function setFeeAddress(address _feeAddress) public{
        ...
        emit FeeAddressUpd(msg.sender, _feeAddress);
    }

    //** has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
