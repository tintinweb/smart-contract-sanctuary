...
contract Referral is Declaration, Ownable {
    ...
    /**
     * @dev Withdraw corresponding amount of ETH to _addr and transfer _value tokens to referral contract
     * @param _amount amount of tokens to sell
     */
    function withdrawToken(uint256 _amount) public {
        require(token.balanceOf(msg.sender) >= _amount);

        uint256 etherValue = _amount * token.price() / ethUsdRate;
        token.transfer(this, _amount);
