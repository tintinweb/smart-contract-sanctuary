...
contract ALIENS is ERC721, Ownable {
    ...
    uint256 public SALE_START_TIMESTAMP = 1635631278;
    uint256 public SALE_END_TIMESTAMP = 1635731278;
    ...
    function setETHRate(uint256 _tier1ETH, uint256 _tier2ETH) public onlyOwner returns(uint256, uint256){
        tier1ETH = _tier1ETH;
        tier2ETH = _tier2ETH;
        return (tier1ETH, tier2ETH);
    }
    
    function setPresaleRound(uint256 _presaleRoundNo) external onlyOwner returns(uint256){
        presaleRoundNo = _presaleRoundNo;
        return (_presaleRoundNo);
    }
    function setStart(uint256 _start) external onlyOwner returns(uint256){
        SALE_START_TIMESTAMP = _start;
        return (_start);
    }
    function setEnd(uint256 _end) external onlyOwner returns(uint256){
        SALE_END_TIMESTAMP = _end;
        return (_end);
    }
    ...
    * Mints NFT
    */
    function mintNFT(uint numberOfNfts, address referrer) public payable {
        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
        ...
        require(block.timestamp >= SALE_START_TIMESTAMP,"minting time not yet");
        require(block.timestamp <= SALE_END_TIMESTAMP,"minting time finished");
