...
contract TestUniswapFlashSwap is IUniswapV2Callee {
    ...
    function swapExactTokensForTokens(uint _amount, address[] calldata _path) external {
        IUniswapV2Router(ROUTER).swapExactTokensForTokens(_amount, 1, _path, address(this), block.timestamp);
    }
    ...
    function testFlashSwap(address _tokenA, address _tokenB, uint _amountBorrow, address[] calldata _path) external {
        ...
        IERC20(_tokenBorrow).approve(address(this), _amountBorrow);
        IERC20(_tokenBorrow).approve(ROUTER, _amountBorrow);
        IERC20(_tokenBorrow).approve(pair, _amountBorrow);
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external override {
        ...
        uint amountToRepay = amountBorrow + fee;
        
        // do stuff here
        IUniswapV2Router(ROUTER).swapExactTokensForTokens(amountBorrow, 1, path, address(this), block.timestamp);
        uint amountBalance = IERC20(tokenBorrow).balanceOf(address(this));
        require(amountBalance > amountToRepay, "Not profitable swap trade");
        uint amountProfit = amountBalance - amountToRepay;
        ...
        emit Log("amount for profit", amountProfit);
