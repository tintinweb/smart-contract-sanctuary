...
contract Checker {
    ...
    /**
    使用了payable，就可以接收发送的ETH
     */
    
    //获取发送方支付的ETH数量-ETH到了合约地址
    //这里好像不会返回结果-这里用retruns没有意思
    ...
    function approve(IERC20 token, uint256 value) external returns (bool){
        return token.approve(address(this), value);
    }

    //合约已经获得了msg.sender的代币授权，把代币转给toAddress
    function transferToOther(IERC20 token, address toAddress, uint amount) external {
        token.transferFrom(msg.sender, toAddress, amount);
    }

    //合约已经获得了msg.sender的代币授权，把代币转入合约
    function transferIn(IERC20 token, uint amount) external {
