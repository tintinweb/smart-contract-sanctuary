...
contract ProofOfStakeProtocolLayer {
    ...
    function deposit(address account, uint256 amountToStake) public;
    function withdraw(address account, uint256 amountToWithdraw) public;
    ...
    event ProofOfStakeMinting(address indexed account, uint256 indexed amount);
}
...
contract ProofOfHoldProtocolLayer {
    ...
    event ProofOfHoldMinting(address indexed to, uint256 _holderMinting);
}
...
contract ProofOfAgeProtocolLayer {
    ...
    event ProofOfAgeMinting(address indexed _address, uint256 _tokenAgeMinting);
}
...
contract Ethic_Testnet is ERC20, ERC20Detailed,
    ...
    constructor() public ERC20Detailed("Ethic_Testnet","ETHIN", 18){
        owner = msg.sender;
        ...
        lastRewardBlock = block.number > genesisBlockNumber ? block.number : genesisBlockNumber;
...
//--------------------------------------------------------------------------------------
    ...
    function proofOfAgeMinting() tokenMinter public returns (bool) {
...
//--------------------------------------------------------------------------------------
    ...
    function tokenAgeInfo(address account) external view returns (address, uint myTokenAge) {
        myTokenAge = getProofOfAge(msg.sender, now);
        return (account, myTokenAge);
    }
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public rewardsPerBlock = 500 * (10**18);
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal tokensPerBlock = 200 * (10**18);
    uint256 internal totalPoolStake = 0;
    
    uint256 internal rewardStartBlock;
    uint256 internal lastRewardBlock;
    uint256 internal stakeMinting;
    
    event Stake(address indexed sender, address indexed recipient, uint256 indexed amount);
    event UnStake(address indexed sender, address indexed recipient, uint256 indexed amount);

    struct MinterInfo{uint256 stake; uint256 rewardBalance; uint256 rewardPending;}

    mapping (address => uint) public depositedTokens;
    mapping(address => MinterInfo) private minterInfo;
    mapping(address => bool) minters;
    
    modifier stakeMinter() {
        require(minters[msg.sender]);
        _;
    }
    
    function stake(address account, address stakingPool, uint256 amountToStake) external returns (bool) {
        require((now >= stakeStartTime) && (stakeStartTime > 0));
        require(amountToStake > 0, "Cannot deposit zero amount");
        
        account = msg.sender;
        stakingPool = address(tokenContract);
        
        farm(account);
        
        MinterInfo storage minter = minterInfo[account];
        
        depositedTokens[account] = depositedTokens[account].add(amountToStake);
        
        balances[account] = balances[account].sub(amountToStake);
        balances[stakingPool] = balances[stakingPool].add(amountToStake);
        
        minter.stake = minter.stake.add(amountToStake);
        minter.rewardPending = minter.stake.mul(stakeMinting).div(1e12);
        
        totalPoolStake = totalPoolStake.add(amountToStake);
        
        emit Stake(msg.sender, stakingPool, amountToStake);
        emit Transfer(msg.sender, stakingPool, amountToStake);
        
        return true;
    }
    
    function unStake(address account, address stakingPool, uint256 amountToWithdraw) external returns (bool) {
        ...
        depositedTokens[account] = depositedTokens[account].add(amountToWithdraw);
        balances[account] = balances[account].add(amountToWithdraw);
        balances[stakingPool] = balances[stakingPool].sub(amountToWithdraw);
        
        minter.stake = minter.stake.sub(amountToWithdraw);
        minter.rewardPending = minter.stake.mul(stakeMinting).div(1e12);
        
        totalPoolStake = totalPoolStake.sub(amountToWithdraw);
        
        emit UnStake(stakingPool, msg.sender, amountToWithdraw);
        emit Transfer(stakingPool, msg.sender, amountToWithdraw);
        
        return true;
    }
    
     function deposit(address account, uint256 amountToStake) public stakeMinter {
        require((now >= stakeStartTime) && (stakeStartTime > 0));
        require(amountToStake > 0, "Cannot deposit zero amount");
        ...
        depositedTokens[account] = depositedTokens[account].add(amountToStake);
        balances[account] = balances[account].sub(amountToStake);
        balances[address(tokenContract)] = balances[address(tokenContract)].add(amountToStake);
        ...
        emit Deposit(account, amountToStake);
        emit Transfer(account, address(tokenContract), amountToStake);
    }

    function withdraw(address account, uint256 amountToWithdraw) public stakeMinter {
        require(depositedTokens[account] >= amountToWithdraw, "Invalid amount to withdraw");
        farm(account);
        MinterInfo storage minter = minterInfo[account];
        
        depositedTokens[account] = depositedTokens[account].sub(amountToWithdraw);
        balances[account] = balances[account].add(amountToWithdraw);
        balances[address(tokenContract)] = balances[address(tokenContract)].sub(amountToWithdraw);
        ...
        emit Transfer(address(tokenContract), account, amountToWithdraw);
    }
    
    function updatePool() public stakeMinter {
        if(block.number > lastRewardBlock) {
            stakeMinting = getProofOfStakeMinting(msg.sender);
            lastRewardBlock = block.number;
        }
    }

    function farm(address account) internal {
        updatePool();
        MinterInfo storage minter = minterInfo[account];
        if(minter.stake > 0) {
            uint256 reward = minter.stake.mul(stakeMinting).div(1e12);
            if (reward > minter.rewardPending) {
                uint256 pending = reward.sub(minter.rewardPending);
                minter.rewardBalance = minter.rewardBalance.add(pending);
            }
        }
    }

    function getProofOfStakeMinting(address account) public view returns (uint256) {
        require((now >= stakeStartTime) && (stakeStartTime > 0));
        require(stakePaused == false);
        if(stakePaused == true) revert();
        
        uint256 blocksCount = block.number.sub(genesisBlockNumber);
        uint256 lastMintedBlock = blocksCount.sub(lastRewardBlock);
        uint256 tokensRewardsReleases = lastMintedBlock.mul(tokensPerBlock);
        
        if(totalSupply < mintingTier.mul(1)) {
            tokensPerBlock.mul(1);

        } else if(totalSupply >= mintingTier.mul(1) && totalSupply < mintingTier.mul(2)) {
            tokensPerBlock.mul(1);

        } else if(totalSupply >= mintingTier.mul(2) && totalSupply < mintingTier.mul(4)) {
            tokensPerBlock.div(2); //Tokens per block halving

        } else if(totalSupply >= mintingTier.mul(4) && totalSupply < mintingTier.mul(8)) {
            tokensPerBlock.div(2);

        } else if(totalSupply >= mintingTier.mul(8) && totalSupply < mintingTier.mul(16)) {
            tokensPerBlock.div(4); //Tokens per block halving
            
        } else if(totalSupply >= mintingTier.mul(16)) {
            tokensPerBlock.div(4);
        }
        
        uint256 totalBlocksTime = blocksTime;
        uint256 stakingAmount = depositedTokens[account];
        uint256 tokenRewards = tokensRewardsReleases.div(totalBlocksTime);
        return stakingAmount.mul(tokenRewards).div(totalPoolStake);
    }

    function pendingReward(address account) public view returns (uint256) {
        MinterInfo storage minter = minterInfo[account];
        uint256 reward = minter.stake.mul(getProofOfStakeMinting(msg.sender)).div(1e12);
        uint256 pending = 0;
        if(reward > minter.rewardPending) {
            pending = reward.sub(minter.rewardPending);
        }
        return minter.rewardBalance.add(pending);
    }

    function getStakingRewards(address account) stakeMinter public returns (uint256 stakingRewards) {
        farm(account);
        MinterInfo storage minter = minterInfo[account];
        assert(stakingRewards <= proofOfStakeRewards);
        if(proofOfStakeRewards == 0) return 0;
        stakingRewards = minter.rewardBalance;
        minter.rewardBalance = 0;
        minter.rewardPending = minter.stake.mul(stakeMinting).div(1e12);
        totalBlocksMinted++;
        emit ProofOfStakeMinting(account, stakingRewards);
        emit Transfer(address(0), account, stakingRewards);
    }
    
    function mintersInfo(address account) public view returns (address, uint256, uint256, uint256, uint256, uint256) {
        uint256 blocksCount = block.number.sub(rewardStartBlock);
        uint256 lastMintedBlock = blocksCount.sub(lastRewardBlock);
        uint256 stakingAmount = depositedTokens[account];
        return(account, stakingAmount, blocksCount, lastMintedBlock, tokensPerBlock, totalPoolStake);
    }
    
    function setRewardStartBlock(uint256 _rewardsStartBlock) external onlyOwner {
        rewardStartBlock = _rewardsStartBlock;
        rewardStartBlock = block.number;
        rewardStartBlock = now;
    }
    
    function getRewardStartBlockNumber() external view returns (uint blockNumber) {
        blockNumber = rewardStartBlock;
    }

//--------------------------------------------------------------------------------------
//Set function for Proof-of-Stake pool
//--------------------------------------------------------------------------------------
    ...
    function setProofOfStakeRewards(uint256 value) external onlyOwner {
        require(totalSupply <= maxTotalSupply);
        if(totalSupply == maxTotalSupply) revert();
        proofOfStakeRewards = proofOfStakeRewards.add(value);
    }
    
    function cutProofOfStakeRewards(uint256 value) external onlyOwner {
...
//--------------------------------------------------------------------------------------
//Pause and continue Proof-of-Stake minting pool
//--------------------------------------------------------------------------------------
