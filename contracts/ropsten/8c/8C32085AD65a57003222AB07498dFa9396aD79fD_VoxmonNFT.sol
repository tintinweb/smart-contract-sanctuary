...
contract VoxmonSignature {
    ...
    bytes32 constant VOXMON_VOUCHER_TYPEHASH = keccak256(
        "NFTVoucher(uint256 minPrice,uint256 tokenId,string uri,address forWallet)"
    );
    ...
    function recover(NFTVoucher memory voucher, uint8 v, bytes32 r, bytes32 s) internal view returns (address) {
    ...
    function test(uint8 v1, bytes32 r1, bytes32 s1) public payable returns (bool) {

        NFTVoucher memory voucher = NFTVoucher({
            minPrice: 1,
            tokenId: 0,
            uri: "www.google.com",
            forWallet: 0x7c4D0a5FC1AeA24d2Bd0285Dd37a352b6795b78B
        });

        //Example sig components
        // uint8 v = 28;
        ...
        console.logBytes32(hash(voucher));
        console.logAddress(recover(voucher, v1,r1,s1)); //this should equal the address of the signer
        assert(recover(voucher, v1, r1, s1) == 0x7c4D0a5FC1AeA24d2Bd0285Dd37a352b6795b78B);
        return true;
    }
}
