...
contract ElementCrystals is ERC721 {
  ...
  uint256 public scorpionFinanceCounter;
  ...
  function mintElementCrystal(string memory _name, string memory _tokenURI, uint256 _price) external {
    // check if thic fucntion caller is not an zero address account
    require(msg.sender != address(0));
    // increment counter
    scorpionFinanceCounter ++;
    // check if a token exists with the above token id => incremented counter
    require(!_exists(scorpionFinanceCounter));
    ...
    // mint the token
    _mint(msg.sender, scorpionFinanceCounter);
    // set token URI (bind token id with the passed in token URI)
    _setTokenURI(scorpionFinanceCounter, _tokenURI);
    ...
    allElementCrystals[scorpionFinanceCounter] = newElementCrystal;
  }
