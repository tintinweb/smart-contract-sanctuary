...
contract RewardDistributionManager is Ownable {
    address rewardDistributionManager;

    modifier onlyRewardDistributionManager() {
        require(_msgSender() == rewardDistributionManager, "Caller is not reward distribution manager");
        _;
    }
...
contract dTrancheToken is ERC20, Ownable {
    ...
    constructor(uint256 initialSupply) ERC20("dTrancheToken", "TRA") payable {
        _mint(msg.sender, initialSupply);
    }

    function mint(address _account, uint256 _amount) external {
        require(minters[msg.sender], "MASK Token: You are not the minter");
...
contract JTAndTRAV2PairTokenWrapper {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IERC20 public JTAndTRAV2PairToken;
    
    uint256 private _totalSupply;
    mapping(address => uint256) private _balances;

    constructor (address pairTokenAddress) {
        JTAndTRAV2PairToken = IERC20(pairTokenAddress);
    }
    ...
    function stake(uint256 amount) virtual public {
        uint256 _before = JTAndTRAV2PairToken.balanceOf(address(this));
        JTAndTRAV2PairToken.safeTransferFrom(msg.sender, address(this), amount);
        uint256 _after = JTAndTRAV2PairToken.balanceOf(address(this));
    ...
    function withdraw(uint256 amount) virtual public {
        ...
        JTAndTRAV2PairToken.safeTransfer(msg.sender, amount);
    }
}

contract JTAndTRALPTokenStakeRewardPool is JTAndTRAV2PairTokenWrapper, RewardDistributionManager {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Math for uint256;
    using SafeERC20 for dTrancheToken;

    dTrancheToken public TRAToken;
    uint256 public constant DURATION = 12 hours;

    uint256 public constant startTime = 1604066400; 
    ...
    constructor(address pairToken, address TRAToken_) JTAndTRAV2PairTokenWrapper(pairToken) {
        TRAToken = dTrancheToken(TRAToken_);
    }
