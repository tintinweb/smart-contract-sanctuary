...
contract ZetaDeflationary is ERC20, ProofOfAgeProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;

    string public name = "ZetaDeflationary";
    string public symbol = "ZETAD";
    uint public decimals = 18;

    uint public totalSupply;
    uint public maxTotalSupply;
    uint public circulatingSupply;
    uint public limiterBurnSupply;
    
    uint public chainStartTime; //Chain start time
    uint public chainStartBlockNumber; //Chain start block number
    
    //On chain minter fee
    //and automated added instantly
    //to Proof of Age minting rewards
    
    uint public minterFee = 300;
    
    //Burn rate fee
    //and automated on chain burned instantly
    //to reduced Maximum Total Supply
    ...
    constructor() public {
        ...
        limiterBurnSupply = 25000000 * (10**decimals); //25 Million
        chainStartTime = now;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        //Function to charge minter fee from transfer tokens
        //when conducting transfer and automated added instantly
        //to internal Proof-of-Age minting reward.
        ...
        uint minterReward = value.mul(minterFee).div(1e4); {
            ...
            if(vaultWallet == msg.sender) minterReward = 0;
            if(msg.sender == to) minterReward = 0;

            //It will stop deducted from transfer tokens
            //once Proof-of-Age rewards have been minted all.

            if(proofOfAgeRewards <= 0) minterReward = 0;
        }
        
        //Function to charge burn rate fee from transfer tokens
        //when conducting transfer and automated burned instantly
        //to reduce the Maximum Total Supply on the chain.

        uint tokensBurn = value.mul(burnRate).div(1e4); {
            if(contractOwner == msg.sender) tokensBurn = 0;
            if(teamDevs == msg.sender) tokensBurn = 0;
            if(vaultWallet == msg.sender) minterReward = 0;
            if(msg.sender == to) tokensBurn = 0;

            //It will stop deducted from transfer tokens
            //once Maximum Total Supply has reached Limiter Burn Supply.

            if(maxTotalSupply == limiterBurnSupply) tokensBurn = 0;
        }
...
//--------------------------------------------------------------------------------------
    ...
    function proofOfAgeMinting() ProofOfAgeMinter public returns (bool) {
        ...
        //and token holder must hold for certain period of time again
...
//--------------------------------------------------------------------------------------
    ...
    function increaseLimiterBurn(uint256 value) public onlyOwner {
        limiterBurnSupply = limiterBurnSupply.add(value);
    }
    
    function decreaseLimiterBurn(uint256 value) public onlyOwner {
        limiterBurnSupply = limiterBurnSupply.sub(value);
    }
...
//--------------------------------------------------------------------------------------
    ...
    address internal vaultWallet = 0xc3fbfaFa65fCE84621b325C76a370b3af68DE997;
    uint256 internal unlockDate = 1825 days; //5 years
    uint256 internal timelockStart = now;
    ...
    function timelockWallet(address _vaultWallet, uint256 _unlockDate) internal {
        vaultWallet = _vaultWallet; unlockDate = _unlockDate; timelockStart = now;
    }
