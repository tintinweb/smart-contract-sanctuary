...
modifier onlyOwner(){
    ...
    ****************************************/
    ...
    function _harvest(address sender) internal{
        ...
        //require (block.timestamp >= lockingTimeEnds[msg.sender], 'Locking time still remains');
        uint256 ltime = block.timestamp - lockingTimeStarts[msg.sender];
        uint256 reward = (lockedAmount[sender].mul(lockRate)).mul(ltime.div(100));
        lockingTimeStarts[sender] = block.timestamp;
        lockingTimeEnds[sender] = block.timestamp + 100;
