...
contract Token is ERC20, DateTime, Ownable {
    ...
    function setTokenIssuer(address _addressIssuerFees) public onlyOwner {
        tokenIssuer = _addressIssuerFees;
    }
    ...
    //Mint tokens to each beneficiary
    function mints(address[] calldata _recipients, uint256[] calldata _values) external payable onlyOwner {
    ...
    //Burn tokens to each beneficiary
    ...
    function withdrawEther(uint256 amount) public payable onlyOwner {
        address payable ownerPayable = payable(address(uint160(Ownable.owner())));
