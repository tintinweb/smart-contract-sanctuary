...
contract ShkoobyStake is Ownable {
    ...
    function getStakePoolDetails(uint256 _stakePoolId) public view returns(stakePool memory){
        stakePool memory stakePoolDetails = stakePools[_stakePoolId];
        return (stakePoolDetails);
    }
    ...
    function getUserStakeDetails(uint256 _stakeId) public view returns(userStake memory, stakePool memory){
        userStake memory userStakeDetails = userStakes[_stakeId];
        stakePool memory stakePoolDetails = stakePools[userStakeDetails.stakePoolId];
        
        return (userStakeDetails, stakePoolDetails);
    }
    ...
    function getFlexyPoolDetails (address _tokenAddress) external view returns (uint256[] memory) {
        ...
        totalFlexyPoolStakedBalance = stakePoolDetails.stakedTokens + totalFlexyPoolStakedBalance;
        totalFlexyPoolClaimedLockedRewards = stakePoolDetails.claimedLockedRewards + totalFlexyPoolClaimedLockedRewards;
        totalFlexyPoolsClaimedUnlockedRewards = stakePoolDetails.claimedUnlockedRewards + totalFlexyPoolsClaimedUnlockedRewards;
        totalFlexyPoolWeight = stakePoolDetails.dpr + totalFlexyPoolWeight;
        totalFlexyPoolAPR = stakePoolDetails.apr + totalFlexyPoolAPR;
    ...
    */
    ...
    function claimAndLockRewards(uint256 _stakeId) external returns (bool){
        ...
        lockedRewardDetails.unlockTime = block.timestamp + (86400 * 365); //Claimed rewards locked for 1 year
    ...
    function getUnlockTime(uint256 _lockDays) public view returns (uint256){
       uint256 unlockTime = block.timestamp + (86400 * _lockDays);
       return unlockTime;
    }
    ...
    function getUserLockedRewardDetails(uint256 _lockedRewardId) public view returns(lockedReward memory, stakePool memory){
        lockedReward memory lockedRewardDetails = lockedRewards[_lockedRewardId];
        stakePool memory stakePoolDetails = stakePools[lockedRewardDetails.stakePoolId];
        
        return (lockedRewardDetails, stakePoolDetails);
    }
    ...
    function getUserDetailsByAddress(address _address) external view returns (user memory){
