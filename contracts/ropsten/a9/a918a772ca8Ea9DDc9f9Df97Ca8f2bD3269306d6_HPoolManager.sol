...
interface IHPoolFactory {
    function deployHPool(uint256 hPoolId, address championAddress) external returns (address);
}
...
contract HPoolManager is ERC1155HolderUpgradeable, HordUpgradable {
    ...
    /**
     * @notice          Function which setting champion address for exact HPool.
     */
    function setChampionAddress(address _championAddress, uint256 poolId) external {
        ...
        for(uint256 i = 0; i < championAddressToHPoolIds[msg.sender].length-1; i++) {
            if(championAddressToHPoolIds[msg.sender][i] == poolId || check) {
                check = true;
                championAddressToHPoolIds[msg.sender][i] = championAddressToHPoolIds[msg.sender][i+1];
            }
        }

        championAddressToHPoolIds[msg.sender].pop();
    }
    ...
    */
    ...
    returns (uint256, uint256)
    {
        return (platformFee.feesCollected, platformFee.feesWithdrawn);
    }
