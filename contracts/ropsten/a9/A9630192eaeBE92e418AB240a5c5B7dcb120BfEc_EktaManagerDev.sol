...
contract EktaManagerDev is AccessControl, Pausable, ReentrancyGuard {
    ...
    function withdrawNativeFromEkta() payable external {
        require(msg.value <= (msg.sender).balance, "Bridge: Amount exceeds balance");
        address payable _user = payable(address(this));
        _user.transfer(msg.value);
        emit NativeWithdraw(msg.sender, address(this), msg.value);
    }
