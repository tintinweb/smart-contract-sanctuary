...
contract TeazersTron {

  struct Account {
    uint id;
    uint[] activeLevel;
    address sponsor;
    mapping(uint => T1) x31Positions;
    mapping(uint => T2) x22Positions;
  }
  ...
  event PlacementT1(address member, address sponsor, uint level, uint8 placement, bool passup, uint orderId);
  event PlacementT2(address member, address sponsor, uint level, address placedUnder, uint8 placementSide, bool passup, uint orderId);
  ...
  constructor(address _addr, address _holder) public {
    ...
    levelCost[1] = Level({cost: 1 ether, commission: 1 ether, fee: 0});
    topLevel = 1;

    lastId++;

    createAccount(lastId, _addr, _addr, true);
    handlePositionT1(_addr, _addr, _addr, 1, true);
    handlePositionT2(_addr, _addr, _addr, 1, true);
  }
  ...
  function preRegistration(address _addr, address _sponsor) internal {
    ...
    handlePositionT1(_addr, _sponsor, _sponsor, 1, false);
    handlePositionT2(_addr, _sponsor, _sponsor, 1, false);
  ...
  function handleLevel(uint _matrix, uint _level) internal {
    ...
    if (_matrix == 1) {
      handlePositionT1(msg.sender, sponsor, activeSponsor, _level, false);
    } else {
      handlePositionT2(msg.sender, sponsor, activeSponsor, _level, false);
    }
  ...
  function handlePositionT1(address _addr, address _mainSponsor, address _sponsor, uint _level, bool _initial) internal {
    Account storage member = members[_addr];

    member.activeLevel[0] = _level;
    member.x31Positions[_level] = T1({sponsor: _sponsor, placement: 0, passup: 0, reEntryCheck: 0});
    ...
    sponsorPlaceT1(_addr, _sponsor, _level, false);
  }

  function sponsorPlaceT1(address _addr, address _sponsor, uint _level, bool passup) internal {
    T1 storage position = members[_sponsor].x31Positions[_level];

    emit PlacementT1(_addr, _sponsor, _level, (position.placement + 1), passup, orderId);
    ...
    if (position.placement >= 2) {
      ...
      if (_sponsor != idToMember[1]) {
        ...
        sponsorPlaceT1(_sponsor, position.sponsor, _level, true);
      }
    } else {
      position.placement++;
    }
  ...
  function handlePositionT2(address _addr, address _mainSponsor, address _sponsor, uint _level, bool _initial) internal {
    Account storage member = members[_addr];
    
    member.activeLevel[1] = _level;
    member.x22Positions[_level] = T2({sponsor: _sponsor, passup: 0, cycle: 0, reEntryCheck: 0, placementSide: 0, placedUnder: _sponsor, placementFirstLevel: new address[](0), placementLastLevel: 0});
    ...
    sponsorPlaceT2(_addr, _sponsor, _level, false);
  }

  function sponsorPlaceT2(address _addr, address _sponsor, uint _level, bool passup) internal {
    T2 storage member = members[_addr].x22Positions[_level];
    T2 storage position = members[_sponsor].x22Positions[_level];
    ...
    emit PlacementT2(_addr, _sponsor, _level, member.placedUnder, member.placementSide, passup, orderId); 
 
    if (position.placementFirstLevel.length < 2) {
      position.placementFirstLevel.push(_addr);

      positionPlaceLastLevelT2(_addr, _sponsor, position.placedUnder, position.placementSide, _level);
    }
    ...
    if ((position.placementLastLevel & 15) == 15) {
      ...
      if (_sponsor != idToMember[1]) {
        ...
        sponsorPlaceT2(_sponsor, position.sponsor, _level, true);
      }
    }
  }

  function positionPlaceLastLevelT2(address _addr, address _sponsor, address _position, uint8 _placementSide, uint _level) internal {
    T2 storage position = members[_position].x22Positions[_level];
    ...
    if ((position.placementLastLevel & 15) == 15) {
      ...
      if (_position != idToMember[1]) {
        ...
        sponsorPlaceT2(_position, position.sponsor, _level, true);
      }
    }
  }
  ...
  function handleReEntryT1(address _addr, uint _level) internal {
    T1 storage member = members[_addr].x31Positions[_level];
    bool reentry = false;
  ...
  function handleReEntryT2(address _addr, uint _level) internal {
    T2 storage member = members[_addr].x22Positions[_level];
    bool reentry = false;
  ...
  function findPayoutReceiver(address _addr, uint _matrix, uint _level) internal returns (address) {
    ...
    if (_matrix == 0) {
      ...
      while (true) {
        T1 storage member = members[receiver].x31Positions[_level];
      ...
      while (true) {
        T2 storage member = members[receiver].x22Positions[_level];
  ...
  function processCompLevel(address _addr, uint _matrix, uint _level) internal {
    ...
    for (uint num = (activeLevel + 1);num <= _level;num++) {
      ...
      if (matrix == 0) {
        handlePositionT1(_addr, sponsor, activeSponsor, num, false);
        handlePayout(_addr, 0, num, false);
      } else {
        handlePositionT2(_addr, sponsor, activeSponsor, num, false);
  ...
  function addLevel(uint _levelPrice, uint _levelCommission, uint _levelFee) external isOwner(msg.sender) {
    ...
    handlePositionT1(idToMember[1], idToMember[1], idToMember[1], topLevel, true);
    handlePositionT2(idToMember[1], idToMember[1], idToMember[1], topLevel, true);
  }
