...
contract Zink is ERC20, proofOfAgeProtocol, Ownable {
    using SafeMath for uint256;

    string public name = "Zink";
    string public symbol = "ZINK";
    ...
    //Default pool fee is 2%, and it send directly to internal Proof of Age minting pool
    uint public poolFee = 200;
    
    //Default burn rate is 2%, and it send directly to address 0
    uint public burnRate = 200;
    
    //Default share tax is 1%, and it send directly to every token holder
    uint public shareTax = 100;
    ...
    //Share receiver address
    address public tokenHolder = address(owner);
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint poolSupply = balances[msg.sender].mul(poolFee).div(1e4);
        uint shareFund = balances[msg.sender].mul(shareTax).div(1e4);
        uint tokensBurn = getTokensBurn();
        
        uint valueAfterFee = value.sub(poolSupply).sub(shareFund).sub(tokensBurn);
        
        balances[msg.sender] = balances[msg.sender].sub(value).sub(valueAfterFee);
        balances[to] = balances[to].add(valueAfterFee);
        
        proofOfAgeRewards = proofOfAgeRewards.add(poolSupply);
        maxTotalSupply = maxTotalSupply.sub(tokensBurn);
        totalSupply = totalSupply.sub(tokensBurn);

        emit Transfer(msg.sender, to, valueAfterFee);
        emit Burn(msg.sender, address(0), tokensBurn);
        emit ShareTransfer(msg.sender, address(tokenHolder), shareFund);
    ...
    function getTokensBurn() internal view returns (uint tokensBurn) {
        tokensBurn = balances[msg.sender].mul(burnRate).div(1e4);{
            //If tokens burn has reach 80,000,000, it will stop 
            //deducting from user balances when transfer
            if(tokensBurn >= 80000000 * (10**decimals))
            tokensBurn = balances[msg.sender].mul(0);
        }
    }
...
//--------------------------------------------------------------------------------------
    ...
    function annualMintingRate() internal view returns (uint mintingRate) {
        ...
        } else if(totalSupply > 1000000 * (10**decimals) && totalSupply <= 2500000 * (10**decimals)) {
            mintingRate = (500 * baseRate).div(100);
        //annual minting rate 25%
        } else if(totalSupply > 2500000 * (10**decimals) && totalSupply <= 5000000 * (10**decimals)) {
            mintingRate = (250 * baseRate).div(100);
        //annual minting rate 12,5%
        } else if(totalSupply > 5000000 * (10**decimals) && totalSupply <= 7500000 * (10**decimals)) {
            mintingRate = (125 * baseRate).div(100);
        //annual minting rate 10%
        } else if(totalSupply > 7500000 * (10**decimals) && totalSupply <= 10000000 * (10**decimals)) {
            mintingRate = (100 * baseRate).div(100);
        //annual minting rate 5%
        } else if(totalSupply > 10000000 * (10**decimals) && totalSupply <= 25000000 * (10**decimals)) {
...
//--------------------------------------------------------------------------------------
    ...
    function changePoolFee(uint256 _poolFee) public onlyOwner {
     ...
     function changeShareTax(uint256 _shareTax) public onlyOwner {
        shareTax = _shareTax;
