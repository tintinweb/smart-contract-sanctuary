...
contract PuppyNFT is Context, ERC721 {
     ...
     * Update puppy details
     */
    function secondaryTransferPuppy(address buyer, uint256 salePuppyId, uint256 dogType)
        public
    {
        ...
        removePuppyFromSale(salePuppyId, dogType);
     ...
     */
    function setSaleDetails(uint256 puppyId, uint256 dogType, uint256 sellingPrice, address operator) public {
        uint256 purchasePrice = _puppyDetails[puppyId].purchasePrice;
        ...
        if (!isSalePuppyAvailable(puppyId, dogType)) {
            puppysForSale[dogType].push(puppyId);
        }
    ...
    function getPuppysForSale(uint256 dogType) public view returns (uint256[] memory) {
        return puppysForSale[dogType];
    }
    ...
    function isSalePuppyAvailable(uint256 puppyId, uint256 dogType)
        internal
        view
        returns (bool)
    {
        for (uint256 i = 0; i < puppysForSale[dogType].length; i++) {
            if (puppysForSale[dogType][i] == puppyId) {
                return true;
            }
        }
        return false;
    ...
    function removePuppyFromSale(uint256 puppyId, uint256 dogType) internal {
        uint256 numOfPuppys = puppysForSale[dogType].length;
		uint256 label = 0;
        for (uint256 i = 0; i < numOfPuppys; i++) {
            if (puppysForSale[dogType][i] == puppyId) {
                label = i + 1;
		...
		if (label != 0){
			for (uint256 j = label; j < numOfPuppys; j++) {
				puppysForSale[dogType][j - 1] = puppysForSale[dogType][j];
			}
			puppysForSale[dogType].pop();
		}
    }
}