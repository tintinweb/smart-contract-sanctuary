...
contract testbinopt {
    ...
    function addGameInQueue() private {
        Games.push(Game(0, 0, 0, 0, 0, false, block.timestamp+intervalSeconds, block.timestamp+intervalSeconds+intervalSeconds, 0, 0, false));
    }
    ...
    function NextCurrentGame() external {
        ...
        require(game1.rewardCalculated == false);
        
        uint256 currentprice = getCurrentPrice();
        game1.priceEnd = currentprice;
        
        updateTreasury(currentGameId);
        require(game1.rewardCalculated, 'rewardCalculated failed');
        
        addGameInQueue();
        currentGameId += 1;
        Game storage game2 = Games[currentGameId];
        game2.priceStart = game1.priceEnd;
        game2.startTimestamp = block.timestamp+intervalSeconds;
        game2.endTimestamp = game2.startTimestamp + intervalSeconds + intervalSeconds;
