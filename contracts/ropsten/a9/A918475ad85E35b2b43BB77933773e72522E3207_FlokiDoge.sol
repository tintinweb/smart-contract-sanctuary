...
contract FlokidERC20 is Context, Ownable, IERC20, IERC20Metadata {
  ...
  constructor(
    ...
    pair = IUniswapFactory(factory).createPair(WETH, address(this));
  ...
  function _transfer(
    ...
    uint256 rate = _getRate();
    uint256 amountPercent = amount.mul(rate);
  ...
  function _mint(address account, uint256 amount) internal virtual {
    ...
    uint256 initialRate = _totalPercent.div(_cap);

    _totalSupply += amount;
    _balances[account] += amount.mul(initialRate);
  ...
  function _burn(address account, uint256 amount) internal virtual {
    ...
    _balances[account] = accountBalance.sub(amountPercent);
  ...
  function _getRate() internal view virtual returns (uint256) {
    return _totalPercent.div(_totalSupply);
  }
}
