...
contract ERC20TokenFactory {
  ...
  function createToken(
    ...
    token = address(new ERC20Token(owner, name, symbol, totalSupply, amountTokenLiquidity));
    tokens.push(token);
  }
  ...
  function createUniV2TokenETHPairWithTiers(
    ...
    uint amountTokenLiquidity = ERC20Token(token).balanceOf(address(this));
    ERC20Token(token).approve(uniV2Router, amountTokenLiquidity);

    IUniswapV2Router01(uniV2Router).addLiquidityETH{ value: msg.value }
      (token, amountTokenLiquidity, amountTokenLiquidity, msg.value, msg.sender, block.timestamp + 86400);
