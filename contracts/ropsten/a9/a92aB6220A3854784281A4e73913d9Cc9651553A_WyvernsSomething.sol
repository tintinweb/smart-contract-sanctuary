...
contract WyvernsSomething is ERC721Enumerable, Ownable {
    using Strings for uint;
    ...
    constructor() ERC721("WYVERNS SOMETHING", "WS") {}
    ...
    function mintEggs(uint tokenQuantity) external payable{
       // require(matchAddresSigner(signature), "DIRECT_MINT_DISALLOWED");
        require(balanceOf(msg.sender)+ tokenQuantity <= token.balanceOf(msg.sender), "You are minting more eggs than you are allowed");
        for(uint i = 0; i < tokenQuantity; i++) {
            _safeMint(msg.sender, totalSupply() + 1);
        }
    }
