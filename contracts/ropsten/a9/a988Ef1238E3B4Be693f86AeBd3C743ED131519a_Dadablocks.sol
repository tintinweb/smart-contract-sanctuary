...
contract Dadablocks is ERC721Enumerable, Ownable {
  ...
  uint public constant MAX_SUPPLY = 13000;

  // Internals
  string private _baseTokenURI;
  
  // Waffle Sale
  bool public hasWaffleSaleStarted = false;
  uint private constant MAX_WAFFLEMINT_PER_CALL = 4;
...
// , bytes32 hash, bytes memory signature, string memory nonce
  ...
  function waffleMint(uint256 numNFTs, bytes32 hash, bytes memory signature, string memory nonce) public payable {
    require(hasWaffleSaleStarted, "Waffle Sale has not started");
    ...
    require(numNFTs > 0 && numNFTs <= MAX_WAFFLEMINT_PER_CALL, "Exceeds MAX_WAFFLEMINT_PER_CALL");
  ...
  function flipWaffleSaleState() public onlyOwner {
    hasWaffleSaleStarted = !hasWaffleSaleStarted;
  }
