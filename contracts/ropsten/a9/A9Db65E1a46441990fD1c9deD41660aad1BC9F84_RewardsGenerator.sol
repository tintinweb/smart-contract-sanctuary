...
contract RewardsGenerator is IRewardsGenerator, OwnableUpgradeable, AbstractDependant {
    ...
    function getPolicyBookAPY(address policyBookAddress)
        ...
        returns (uint256)
    {
        uint256 policyBookRewardMultiplier = _policyBooksRewards[policyBookAddress]
        .rewardMultiplier;
        uint256 totalStakedPolicyBook = _policyBooksRewards[policyBookAddress].totalStaked.add(
            APY_TOKENS
        );

        uint256 rewardPerBlockPolicyBook = policyBookRewardMultiplier
        ...
        uint256 rewardPerBlockPolicyBookSTBL = priceFeed.howManyUSDTsInBMI(
            rewardPerBlockPolicyBook
        );
