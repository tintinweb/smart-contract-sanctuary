...
/// @author AL GORITHM
contract Namelessfolks is ERC721Enumerable, Ownable, Mintable {

  using SafeMath for uint256;

  uint256 public constant FOLK_GIVEAWAY = 498;// 300;  // UPDATE BEFORE GO LIVE
  uint256 public constant FOLK_PRIVATE = 2;
  uint256 public constant FOLK_PUBLIC = 4000;// 11698;  // UPDATE BEFORE GO LIVE
  uint256 public constant FOLK_MAX = FOLK_GIVEAWAY + FOLK_PRIVATE + FOLK_PUBLIC;

  uint256 FOLK_PRICE = 0.02 ether;

  uint256 public constant EFS_FOLK_MAX_PER_MINT = 5; // UPDATE BEFORE GO LIVE
  ...
  constructor(string memory baseURI, address _owner, address _imx)
    ERC721("Nameless Folks IMX", "FOLKIMX") Mintable(_owner, _imx) {
      setBaseURI(baseURI);
  }
  ...
  function verifiedWhiteListMint(uint256 quantity) external payable {
    ...
    require(msg.value >= FOLK_PRICE * quantity, "PAYMENT: Insufficient ETH value");
    require(publicMinted + quantity <= FOLK_PUBLIC, "SUPPLY: Nameless Folks Presale Sold Out");
    require(presalePurchases[msg.sender] + quantity <= verifiedWhiteListPurchaseLimit, "TRANSACTION: Exceeded Max Early Folk Supporter Allocation");
