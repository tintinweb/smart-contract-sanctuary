...
contract NFTTest is ERC721Burnable, ERC721Pausable, RandomlyAssigned, WithSaleStart, WithContractMetaData, WithUnreveal, WithRecover {
    ...
    */
    ...
    function editWhitelist(address[] memory _beneficiaries, uint8 _amount) external onlyOwner {
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            whitelist[_beneficiaries[i]] = _amount;
        }
    }
}
...
/// @title Handle Unreveal -> Reveal functionality
abstract contract WithUnreveal is WithIPFSMetaData {
    ...
    /// @param cid_ the content identifier for the token metadata.
    ...
    function freezeMetadata() external onlyOwner {
        ...
        emit MetadataFinalized(cid);
    }

    function tokenURI(uint256 tokenId) public view virtual override(WithIPFSMetaData) returns (string memory) {
        return super.tokenURI(tokenId);
    }

    function _baseURI() internal view virtual override(WithIPFSMetaData) returns (string memory) {
        return super._baseURI();
    }
}
