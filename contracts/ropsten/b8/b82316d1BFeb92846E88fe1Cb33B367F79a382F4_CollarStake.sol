...
contract CollarStake is Ownable, ReentrancyGuard, Pausable {
    ...
    function poolCreation(poolInfo calldata poolParams) external onlyOwner whenNotPaused {
        currentPool++;
        poolDetails[currentPool] = poolParams;
    }


    function stakeCollar(uint256 _poolID,uint256 _tokenAmount, uint256 _stakeDays) external nonReentrant whenNotPaused {
        currentPool++;
        require( _tokenAmount > 0 && _tokenAmount < stakeLimit,"incorrect token amount");
        poolInfo storage pool = poolDetails[currentPool];
        pool.poolStakeID++;
        UserInfo storage user = userDetails[currentPool][pool.poolStakeID];
    ...
    function unstakeCollar(uint256 _poolID,uint256 _stakeID) external nonReentrant whenNotPaused {
        UserInfo storage user = userDetails[_poolID][_stakeID];
        require(user.stakeTime > 0 , "Invalid stake ID");
        require(user.stakeTime.add(user.stakingDays.mul(86400)) <= block.timestamp,"");
    ...
    function withdraw(uint256 _poolID,uint256 _stakeID) external {
        ...
        require(!user.claimed, "This ID already withdrawed");
