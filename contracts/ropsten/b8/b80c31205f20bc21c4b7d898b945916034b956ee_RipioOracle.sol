...
contract Oracle is Ownable {
    ...
    */
    function addCurrency(string ticker) external onlyOwner returns (bool) {
...
contract RipioOracle is Oracle, Delegable, BytesUtils {
    ...
    address[] public altOracles;
    mapping(address => uint256) public altOraclesIndex;
    ...
    */
    function setExpirationTime(uint256 time) external onlyOwner returns (bool) {
        expiration = time;
        return true;
    }
    ...
    */
    function setFallback(Oracle _fallback) external onlyOwner returns (bool) {
        fallback = _fallback;
        return true;
    }
    ...
    */
    function invalidateCache(bytes32 currency) external onlyOwner returns (bool) {
        delete cache[currency].timestamp;
        return true;
    }
    
    function getAltOracles() external view returns (address[]) {
        return altOracles;
    }
    
    function addAltOracle(address _oracle) external onlyOwner returns (bool) {
        require(altOraclesIndex[_oracle] == 0, "Alt oracle already exists");
        altOraclesIndex[_oracle] = altOracles.push(_oracle);
        return true;
    }
    
    function delAltOracle(address _oracle) external onlyOwner returns (bool) {
        require(altOraclesIndex[_oracle] != 0, "Alt oracle does not exists");
        uint256 i = altOraclesIndex[_oracle];
        address last = altOracles[altOracles.length - 1];
        altOracles[i - 1] = last;
        altOraclesIndex[last] = i;
        altOracles.length--;
        return true;
    }
    ...
    */
    function getRate(bytes32 currency, bytes data) external returns (uint256, uint256) {
        ...
        if (rateCache.timestamp >= timestamp && !isExpired(rateCache.timestamp)) {
            ...
            address signer = ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(this, currency, rate, decimals, timestamp)))),v,r,s);

            if(!isDelegate(signer)) {
                signer = tryAltOracles(currency, rate, decimals, timestamp, r, s, v);
            }
    ...
    function tryAltOracles(
        ...
        bytes32 hash;
        uint256 t = altOracles.length;
        for(uint256 i = 0; i < t; i++) {
            hash = keccak256(abi.encodePacked(altOracles[i], currency, rate, decimals, timestamp));
            signer = ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash)),v,r,s);
            if (isDelegate(signer)) return signer;
        }
