...
contract OrderMarket is IOrderMarket, Initializable, ReentrancyGuard, Ownable {
  ...
  ) external override onlyOrderCreator nonReentrant
  {
    ...
    targetToken.safeTransferFrom(msg.sender, address(this), totalAmount);
    ...
    external override nonReentrant
  {
    ...
    if(_acceptableUserNonce == 0 && orderDetails[_orderId].userNonce == 0){
      totalAmount = 0;
      _acceptableUserNonce = 0;
    }
    else if(_acceptableUserNonce >= 0 && _acceptableUserNonce < orderDetails[_orderId].userNonce){
      //if _acceptableUserNonce == 0 here, will reject all order index
      ...
      //if _acceptableUserNonce is greater or equal to userNonce, it will collect all option token
