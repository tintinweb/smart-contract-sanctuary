...
//  IERC20 constant lpToken = IERC20(0x54d5230d16033eb03ffbaB29FCc09Ac54Df6F812);

  IERC20 constant eldToken = IERC20(0xa15e1b87e754c388A8999f613ad6276B1c63D29A);
  ILPToken constant lpToken = ILPToken(0x4A9eaFE2D1727dad99f923E8E677183f867Bc526);
  uint constant blockPerDay = 5760;

  address admin;
  uint eldPrice = 900; // decimal 3
  ...
  function deposit(uint _lpTokenAmount) external {
    ...
    if (user.usdtAmount > 0 && now > user.lastReward) {
      uint rewardedBlock = now - user.lastReward;
      user.lastReward = now;
  ...
  function getPendingReward(uint _rewardedBlock, uint _usdtAmount) public view returns (uint) {
    return _rewardedBlock * _usdtAmount * 1e12 / eldPrice * 1000 * 5 / 1000 / blockPerDay;
  }
  ...
  function emergencyWithdraw() public {
    ...
    user.usdtAmount = 0;
    user.lpTokenAmount = 0;
    safeLPTokenTransfer(msg.sender, user.lpTokenAmount);
