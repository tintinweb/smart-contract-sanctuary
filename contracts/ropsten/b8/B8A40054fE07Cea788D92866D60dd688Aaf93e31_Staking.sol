...
contract Staking is Ownable, Multicall, IERC677Receiver {
    ...
    function onTokenTransfer(address _sender, uint _amount, bytes calldata _data) external override {
        require(msg.sender == address(rewardToken), "onTokenTransfer: only supports rewardToken");
        require(msg.sender == address(lpToken[0]), "onTokenTransfer: pool 0 needs to be a rewardToken pool");
        if (_amount > 0) {
            deposit(0, _amount, _sender);
        }
    }
}
