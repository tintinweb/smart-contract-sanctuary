...
contract AirNFTStaking is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function stake() external nonReentrant whenNotPaused updateReward(false) updateUserList {
        ...
        for (uint i = 0; i < balance; i++) {
            uint256 _tokenId = parentNFT.tokenOfOwnerByIndex(msg.sender, i);
            amount += parentNFT.price(_tokenId);
            parentNFT.safeTransferFrom(
                msg.sender,
                address(this),
                _tokenId
        ...
        require (totalSupply.add(amount) <= maxCap, 'exceeded maximum cap');
    ...
    function unstake() public nonReentrant updateReward(true) updateUserList {
        UserInfo storage user = userInfo[msg.sender];
        ...
        for (uint i = user.tokenIds.length -1; i >= 0 ; i--) {
