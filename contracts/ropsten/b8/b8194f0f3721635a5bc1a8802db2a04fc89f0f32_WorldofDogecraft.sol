...
contract WorldofDogecraft is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    uint256 private constant TOTAL_SUPPLY = 10000000000 * 10**9; //9 decimal spots after the amount 
    string private m_Name = "World Of DogeCraft";
    string private m_Symbol = "WODogeCraft";
...
// ####################
    ...
    function _handleBalances(address _sender, address _recipient, uint256 _amount) private {
        if (_isExchangeTransfer(_sender, _recipient)) {
            ...
            m_Balances[address(this)] = m_Balances[address(this)].add(_tollAmount);
    ...
    function _disperseEth() private {
        uint256 _ethBalance = address(this).balance;
            m_TollAddress.transfer(_ethBalance);
    }
