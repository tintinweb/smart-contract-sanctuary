    ...
    AbstractSlasher
{
    ...
    function rewardPerToken() public view override returns (uint256) {
        ...
        uint256 accumulatedReward =
            blocksWithRewardsPassed().mul(rewardPerBlock).mul(DECIMALS).div(totalStaked);
    ...
    function withdraw(uint256 _amount) public override nonReentrant updateReward(_msgSender()) {
        require(_amount > 0, "LMS: Amount should be greater then 0");
        require(staked[_msgSender()] >= _amount, "LMS: Insufficient staked amount");
