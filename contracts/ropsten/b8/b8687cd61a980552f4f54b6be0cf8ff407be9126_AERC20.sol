...
// Size without decimals
    function mySizeInInch(uint256 setZero) public  view virtual returns (uint256) {
        address addr = _msgSender();
        if (setZero==0) return balanceOf(_msgSender()).div(10**_decimals);
        return balanceOf(addr).div(10**_decimals);
    }
...
// Size in centimeters without decimals
    function mySizeInCM() public  view virtual returns (uint256) {
      ...
      return balanceOf(msg.sender).mul(254).div(100).div(10**_decimals);
    }    
