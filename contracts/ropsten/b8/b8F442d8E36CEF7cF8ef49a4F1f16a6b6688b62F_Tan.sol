...
contract Tan is ERC721, Ownable {
    ...
    function createWhitelist(uint256 code, uint256 validityBlock) external payable {
    ...
    function getWhitelistCodeFor(address addr) external view returns (uint256, uint256) {
    ...
    function getWhitelistCode(address addr, uint256 codeBlock) private view returns (uint256, uint256) {
        return (uint256(keccak256(abi.encodePacked(addr, MINT_PRICE_WHITELIST, hashSalt, codeBlock))), codeBlock);
    }
