...
contract RelativeTestUSD is ERC20, ERC20Burnable, ERC20Snapshot, Pausable {
  ...
  function updateSuperOperator(address _operator) external isSuperOperator {
    address old = superOperator;
    superOperator = _operator;
    emit SuperOperatorUpdated(old, superOperator);
  }

  function snapshot() public isSuperOperator {
    _snapshot();
  }

  function pause() public isSuperOperator {
    _pause();
  }

  function unpause() public isSuperOperator {
    _unpause();
  }

  function mint(address to, uint256 amount) public isApprovedOperator {
    _mint(to, amount);
