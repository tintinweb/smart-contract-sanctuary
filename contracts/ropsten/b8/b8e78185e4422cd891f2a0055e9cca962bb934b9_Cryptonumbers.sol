...
contract CreateCryptonumbers is Ownable {
  ...
  event NewNumber(uint numberId, uint tokenId);
  ...
  function _createNumber(uint _numberId, uint _hashNumber) internal {
    uint id = numbers.push(Number(_numberId, _hashNumber)) - 1;
    numberToOwner[id] = msg.sender;
    ownerNumberCount[msg.sender] = ownerNumberCount[msg.sender].add(1);
    emit NewNumber(id, _hashNumber);
  }

  function _generateRandomhashNumber(uint _str) private view returns (uint) {
