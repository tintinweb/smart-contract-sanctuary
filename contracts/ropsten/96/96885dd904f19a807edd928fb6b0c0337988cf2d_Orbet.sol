...
contract Orbet {
    ...
    uint16 public winnerPercentage;
    uint16 public callerPercentage;
    uint16 public initiatorPercentage;
    uint16 public burnPercentage;
    uint16 public vaultPercentage;
    ...
    struct BetStruct {
        ...
        uint Threshold;
        uint ExpiryDate;
        uint TotalThresholdBetAmount;
        uint TotalNoThresholdBetAmount;
    ...
    constructor() {
        owner = msg.sender;
        winnerPercentage = 900;
        callerPercentage = 10;
        initiatorPercentage = 10;
        vaultPercentage = 10;
        burnPercentage = 10;
    }
    ...
    function betDeposit(uint betStructId, bool position, uint betAmount) public {
        ...
        require(betStruct.Positions[msg.sender].BetAmount == 0, "Position Already Exists");
        if (!position)
            betStruct.TotalNoThresholdBetAmount += betAmount;
        if (position)
            betStruct.TotalThresholdBetAmount += betAmount;
        IERC20(currencyToken).transferFrom(msg.sender, address(this), betAmount);
        betStruct.Positions[msg.sender].Position = position;
        betStruct.Positions[msg.sender].BetAmount = betAmount;
    ...
    function betWithdraw(uint betStructId) public {
        ...
        BetStruct storage betStruct = BetStructs[betStructId];
        require(betStruct.BetStatus == 1, "Bet Not Active");
        require(betStruct.Positions[msg.sender].BetAmount > 0, "No Position");
        if (!betStruct.Positions[msg.sender].Position)
            betStruct.TotalNoThresholdBetAmount -= betStruct.Positions[msg.sender].BetAmount;
        if (betStruct.Positions[msg.sender].Position)
            betStruct.TotalThresholdBetAmount -= betStruct.Positions[msg.sender].BetAmount;
        IERC20(currencyToken).transfer(msg.sender, betStruct.Positions[msg.sender].BetAmount);
        betStruct.Positions[msg.sender].Position = false;
        betStruct.Positions[msg.sender].BetAmount = 0;
    ...
    function betAdjust(uint betStructId, uint adjustToAmount) public {
        ...
        require(betStruct.BetStatus == 1, "Bet Not Active");
        require(betStruct.Positions[msg.sender].BetAmount > 0, "No Position");
        require(betStruct.Positions[msg.sender].BetAmount != adjustToAmount, "No Change");
        if (betStruct.Positions[msg.sender].BetAmount > adjustToAmount) {
            uint betAmountDiff = betStruct.Positions[msg.sender].BetAmount - adjustToAmount;
            IERC20(currencyToken).transfer(msg.sender, betAmountDiff);
            if (!betStruct.Positions[msg.sender].Position)
                betStruct.TotalNoThresholdBetAmount -= betAmountDiff;
            if (betStruct.Positions[msg.sender].Position)
                betStruct.TotalThresholdBetAmount -= betAmountDiff;
        } else {
            uint betAmountDiff = adjustToAmount - betStruct.Positions[msg.sender].BetAmount;
        ...
        betStruct.Positions[msg.sender].BetAmount = adjustToAmount;
    ...
    function claimReward(uint betStructId) public {
        ...
        require(betStruct.Positions[msg.sender].BetAmount > 0, "No Bet Amount");
        bytes16 betAmountDouble = ABDKMath.fromUInt(betStruct.Positions[msg.sender].BetAmount);
        ...
        finalCurrencyTokenReward = ABDKMath.toUInt(ABDKMath.mul(baseCurrencyTokenReward, ABDKMath.div(ABDKMath.fromUInt(uint(winnerPercentage)), ABDKMath.fromUInt(1000))));
    ...
    function endBet(uint betStructId) public {
        ...
        if (betStruct.TotalNoThresholdBetAmount == 0) {
            betStruct.ThresholdNoThresholdRatio = bytes16(0);
        } else {
            betStruct.ThresholdNoThresholdRatio = ABDKMath.div(ABDKMath.fromUInt(betStruct.TotalThresholdBetAmount), ABDKMath.fromUInt(betStruct.TotalNoThresholdBetAmount));
        }
        ...
        for (uint i = 0; i < teamAddresses.length; i++) {
            IERC20(currencyToken).transfer(teamAddresses[i], ABDKMath.toUInt(ABDKMath.mul(ABDKMath.div(ABDKMath.fromUInt(teamPercentages[i]), ABDKMath.fromUInt(1000)), ABDKMath.fromUInt((betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount)))));
        }
        totalVaultRewards = ABDKMath.add(totalVaultRewards, ABDKMath.div(ABDKMath.mul(ABDKMath.fromUInt(betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount), ABDKMath.div(ABDKMath.fromUInt(vaultPercentage), ABDKMath.fromUInt(1000))), ABDKMath.fromUInt(totalVaultDeposits)));
        IERC20(currencyToken).transfer(msg.sender, ABDKMath.toUInt(ABDKMath.mul(ABDKMath.div(ABDKMath.fromUInt(callerPercentage), ABDKMath.fromUInt(1000)), ABDKMath.fromUInt((betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount)))));
        IERC20(currencyToken).transfer(betStruct.Initiator, ABDKMath.toUInt(ABDKMath.mul(ABDKMath.div(ABDKMath.fromUInt(initiatorPercentage), ABDKMath.fromUInt(1000)), ABDKMath.fromUInt((betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount)))));
        IERC20(currencyToken).transfer(address(0), ABDKMath.toUInt(ABDKMath.mul(ABDKMath.div(ABDKMath.fromUInt(burnPercentage), ABDKMath.fromUInt(1000)), ABDKMath.fromUInt((betStruct.TotalThresholdBetAmount + betStruct.TotalNoThresholdBetAmount)))));
    ...
    function addTeamContract(address teamMemberAddress, uint teamMemberPercent) external onlyOwner {
        require(!frozen, "Frozen!");
        require(teamMemberPercent > 0 && teamMemberPercent < 1000, "Invalid Percent");
