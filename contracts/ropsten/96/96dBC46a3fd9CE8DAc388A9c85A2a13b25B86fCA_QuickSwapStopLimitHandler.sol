...
contract QuickSwapStopLimitHandler is UniswapV2Router02Handler {
    constructor(IUniswapV2Router02 _quickswapRouter, address _weth)
        UniswapV2Router02Handler(_quickswapRouter, _weth)
    ...
    function canHandle(
        ...
        address inToken = address(_inToken);
        address outToken = address(_outToken);
        ...
        if (inToken == WETH || inToken == ETH) {
            ...
            uint256 bought = _getAmountOut(_amountIn - fee, path);
            ...
            uint256 bought = _getAmountOut(_amountIn - inTokenFee, path);
            return bought <= _maxReturn && bought >= _minReturn;
        }
    }
}
