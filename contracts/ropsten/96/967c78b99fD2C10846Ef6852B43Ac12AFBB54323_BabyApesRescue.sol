...
string baseTokenURI;
   ...
   // Migration stuff
    mapping(uint256 => bool) public alreadyMigrated;
    ...
    function setSignerAddress(address _addr) external onlyOwner{
     ...
     function migrateMint(uint256[] memory _id, bytes calldata _sig) external {
        for (uint256 i = 0; i < _id.length; i++) {
            require(_id[i] <= 2500, "Can only migrate Baby Apes OG");
        }
        for (uint256 i = 0; i < _id.length; i++) {
            require(
                keccak256(abi.encodePacked(_id[i]))
                    .toEthSignedMessageHash()
                    .recover(_sig) == SIGNER,
                "Invalid Signature"
            );
            require(
                !alreadyMigrated[_id[i]],
                "Token Id has already been migrated"
            );
            alreadyMigrated[_id[i]] = true;
