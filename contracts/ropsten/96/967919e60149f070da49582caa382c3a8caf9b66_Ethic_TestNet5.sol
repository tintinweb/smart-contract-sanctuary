...
contract ProofOfHoldProtocolLayer {
    ...
    uint256 internal holderRewardsPerBlock;
...
contract Ethic_TestNet5 is ERC20, ERC20Detailed,
    ...
    constructor() public ERC20Detailed("Ethic_TestNet5","ETHEST5", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    function proofOfAgeMinting() tokenMinter internal returns (bool) {
        ...
        assert(tokenAgeRewards <= proofOfAgeRewards);
...
//--------------------------------------------------------------------------------------
    ...
    function getProofOfHoldMinting(address account) internal returns (uint256) {
        if(balances[account] < balancesKeep) return 0;
        ...
        uint256 holderRewardsReleases = holderRewardsPerBlock;
        uint256 totalBlocksTime = blocksTime;
        
        if(proofOfHoldRewards <= 0) return 0;

        if(totalSupply < mintingTier.mul(1)) {
            holderRewardsReleases = holderRewardsPerBlock.mul(1);

        } else if(totalSupply >= mintingTier.mul(1) && totalSupply < mintingTier.mul(2)) {
            holderRewardsReleases = holderRewardsPerBlock.mul(1);

        } else if(totalSupply >= mintingTier.mul(2) && totalSupply < mintingTier.mul(4)) {
            holderRewardsReleases = holderRewardsPerBlock.div(2); //Reward per block halving

        } else if(totalSupply >= mintingTier.mul(4) && totalSupply < mintingTier.mul(8)) {
            holderRewardsReleases = holderRewardsPerBlock.div(2);

        } else if(totalSupply >= mintingTier.mul(8) && totalSupply < mintingTier.mul(16)) {
            holderRewardsReleases = holderRewardsPerBlock.div(4); //Reward per block halving
            
        } else if(totalSupply >= mintingTier.mul(16)) {
            holderRewardsReleases = holderRewardsPerBlock.div(4);
        }
        
        uint256 blocksRewards = (blocksCount.sub(totalBlocksMinted)).mul(holderRewardsReleases);
...
//--------------------------------------------------------------------------------------
    ...
    event ChangeHolderRewardsPerBlock(uint256 value);
    ...
    function changeHolderRewardsPerBlock(uint256 _holderRewardsPerBlock) external onlyOwner {
        holderRewardsPerBlock = _holderRewardsPerBlock;
        emit ChangeHolderRewardsPerBlock(holderRewardsPerBlock);
    }
...
//--------------------------------------------------------------------------------------
    ...
    //1 years unix epoch time stamp = 31556926 seconds
    
    uint256 internal epochTimes = 31556926;
    ...
    function proofOfEpochMinting(address account) internal returns (bool) {
        ...
        assert(epochRewards <= proofOfEpochRewards);
        if(proofOfEpochRewards <= 0) return false;
        if(proofOfEpochRewards == maxTotalSupply) return false;
...
//--------------------------------------------------------------------------------------
    ...
    function epochStakeMintingStart() external onlyOwner {
