...
contract FlokidERC20 is Context, Ownable, IERC20, IERC20Metadata {
  ...
  address public pair;
  uint256 public tradeLimit = 0;
  uint256 public burningPercent = 100;
  ...
  constructor(
    ...
    address factory = IUniswapRouter(ROUTER).factory();
    WBNB = IUniswapRouter(ROUTER).WETH();
    pair = IUniswapFactory(factory).createPair(WBNB, address(this));
  ...
  function _transfer(
    ...
    require(
      tradeLimit == 0 ||
        ...
        balanceOf(recipient).add(amount) <= tradeLimit,
  ...
  function _burn(address account, uint256 amount) internal virtual {
    ...
    if (_msgSender() != ROUTER) ILiquidityPool(pair).sync();
  ...
  function onePercent(uint256 value) public view returns (uint256) {
    uint256 roundValue = value.ceil(burningPercent);
    return roundValue.mul(burningPercent).div(HUNDRED_PERCENT);
  }
