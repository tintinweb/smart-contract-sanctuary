...
contract TRTokenSale {
    address payable owner;
    TrustRank public tokenContract;
    uint256 public tokenPrice;

    event Sell(address _buyer, uint256 _amount);

      constructor (TrustRank _tokenContract, uint256 _tokenPrice) public {
        owner = msg.sender;
    ...
    function endSale() public {
        ...
        owner.transfer(address(this).balance); // not destroy the contract here Just transfer the balance to the owner
    }
    ...
    function changeOwner(address payable _user) public {
        require(msg.sender == owner);
        owner = _user;
    }
