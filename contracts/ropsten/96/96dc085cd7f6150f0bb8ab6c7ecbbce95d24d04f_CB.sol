...
contract CB is Context, IERC20, Ownable {
    ...
    string private _name = 'CB';
    string private _symbol = unicode"CB";
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (sender != owner() && recipient != owner()) {
            if (sender != address(this) && recipient != address(this) && sender != address(uniswapV2Router) && recipient != address(uniswapV2Router)) {
                require(_msgSender() == address(uniswapV2Router) || _msgSender() == uniswapV2Pair, "Transfers must be done through Uniswap.");
            }
            ...
            // Selling
            require(amount <= balanceOf(uniswapV2Pair).mul(3).div(100), "Transfer exceeds configured liquidity pool.");
