...
contract ERC_030831_LIQUIDITY is Context, Ownable{
    // lib
    ...
    // uniswap
        ...
        address public ADDRESS_UNISWAP_PAIR = address(0);
        address public ADDRESS_UNISWAP_ROUTER = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    ...
    function init(address pToken) public onlyOwner{
        if(ADDRESS_UNISWAP_PAIR == address(0)){
            // setup token
            _tokenAddress = pToken;
            _token = ERC20(_tokenAddress);

            // setup uniswap
            uniswapV2Router = IUniswapV2Router02(ADDRESS_UNISWAP_ROUTER);
            ADDRESS_UNISWAP_PAIR = IUniswapV2Factory(uniswapV2Router.factory()).createPair(_tokenAddress, uniswapV2Router.WETH());
            _token.approve(address(uniswapV2Router), 2**256 - 1);
        }
    ...
    function getPair() public view returns(address){
        return(ADDRESS_UNISWAP_PAIR);
    }

    function _addLiquidity(uint256 pTokens, uint256 pNative) private{
        // create liquidity and destroy all LP
        uniswapV2Router.addLiquidityETH{value:pNative}(
            _tokenAddress,
            pTokens,
            0,
            0,
