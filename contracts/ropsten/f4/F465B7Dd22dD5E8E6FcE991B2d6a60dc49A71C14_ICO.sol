...
contract ICO is Ownable  {
    ...
    function getTokenUri(uint256 tokenIdUri) public view virtual returns(string memory ,string memory) {
        return  (token.tokenURI(tokenIdUri) , token.tokenURIGame(tokenIdUri));
    }
     function getTotalSale() public view virtual returns(uint256) {
        return  token._getTotalSale();
    }
    
    function _mintNFT(string  memory metadata , string memory metadataGame ) private {
        require(!finalized, "Strings: finalized error");
        require(token_id < 222, "Strings: 222 nft is minted");
        token_id++;
        token.safemint(owner(), metadata , metadataGame);
    }
    
    function mintNFT(string memory _ipfs ,string memory _ipfsGame) onlyOwner public {
        _mintNFT(_ipfs , _ipfsGame);
    }
