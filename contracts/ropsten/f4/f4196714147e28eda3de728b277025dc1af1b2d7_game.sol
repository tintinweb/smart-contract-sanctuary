...
//        lotteryTime = now + 12 hours;
    ...
    function _computePayMoney(uint256 _ticketCount,address _addr) private view returns(bool){
        ...
        if(_p.earnings >= (_tmpMoney.add(_tmpMoney2))){
            return true;
        }
        return false;
    }
...
//        if(totalTicketCount < 10000){
//            lotteryTime = now + 12 hours;
//
//        }else{
//            lotteryTime = now + 30 minutes;
//        }
        lotteryTime = now + 10 minutes;
    }
    ...
    function updateGameMoney(uint256 _money,uint256 _spaceshipNo,uint256 _ticketCount,uint256 arrayPid) private {
        ...
        for(uint i = 0; i<players.length; i++){
            if(arrayPid != i){
                ...
                _tmpP.dividendEarnings = _tmpP.dividendEarnings.add(_tmpP.xzTicketCount.mul(_xzFMoney));
                //韭菜分红
                _tmpP.earnings =  _tmpP.earnings.add(_tmpP.jcTicketCount.mul(_jcFMoney));
                _tmpP.dividendEarnings = _tmpP.dividendEarnings.add(_tmpP.jcTicketCount.mul(_jcFMoney));
            }
        }
    ...
    function makeMoney() public {
        ...
        //剩余的奖池
        uint256 _lastMoney = moonPrizePool.sub(_pMoney);

        // 2%用于下一轮奖池
        uint256 _nextMoonPrizePool = _lastMoney.div(100).mul(2);
        //奖励船长5%
        uint256 _luckyCaptainMoney = _lastMoney.div(100).mul(5);
        //其余其他人分享 43%
        uint256 _luckyCrewMoney = _lastMoney.sub(_nextMoonPrizePool).sub(_luckyCaptainMoney);
        //找出当前速度最快的飞船
