...
contract Staking {
  ...
  */
  ...
  function setWithdrawTime(uint256 withdrawTime) external onlyOwner returns(bool){
    require(withdrawTime != 0, "Zero Time not Supported, Please Try Again!!!");
  ...
  */
  ...
  function withdrawStakedTokens(uint256 stakingId) public returns(bool){
    ...
    require(now >= _tokenStakingStartTime[stakingId] + _withdrawTime, "Unable to Withdraw Stake, Please Try Again Later!!!");
  ...
  function withdrawStakedBNB(uint256 stakingId) public returns(bool){
    ...
    require(_bnbTransactionstatus[stakingId] != true,"Either tokens are already withdrawn or blocked by admin");
      if(_bnbTotalDays[stakingId] == 30){
            require(now >= _bnbStakingStartTime[stakingId] + _withdrawTime, "Unable to Withdraw Stake before 15 days of staking start time, Please Try Again Later!!!");
           ...
           require(now >= _bnbStakingStartTime[stakingId] + _withdrawTime, "Unable to Withdraw Stake before 30 days of staking start time, Please Try Again Later!!!");
           _bnbTransactionstatus[stakingId] = true;
            if(now >= _bnbStakingEndTime[stakingId]){
                _finalBNBStakeWithdraw[stakingId] = _usersBNB[stakingId].add(getBNBRewardDetailsByStakingId(stakingId));
                _bnbStakingAddress[stakingId].transfer(_finalBNBStakeWithdraw[stakingId]);
           ...
           require(now >= _bnbStakingStartTime[stakingId] + _withdrawTime, "Unable to Withdraw Stake before 45 days of staking start time, Please Try Again Later!!!");
           _bnbTransactionstatus[stakingId] = true;
           if(now >= _bnbStakingEndTime[stakingId]){
               _finalBNBStakeWithdraw[stakingId] = _usersBNB[stakingId].add(getBNBRewardDetailsByStakingId(stakingId));
               _bnbStakingAddress[stakingId].transfer(_finalBNBStakeWithdraw[stakingId]);
    ...
    // require(now >= _bnbStakingStartTime[stakingId] + _withdrawTime, "Unable to Withdraw Stake, Please Try Again Later!!!");
    // _bnbTransactionstatus[stakingId] = true;
    // if(now >= _bnbStakingEndTime[stakingId]){
    //     _finalBNBStakeWithdraw[stakingId] = _usersBNB[stakingId].add(getBNBRewardDetailsByStakingId(stakingId));
    //     _bnbStakingAddress[stakingId].transfer(_finalBNBStakeWithdraw[stakingId]);
