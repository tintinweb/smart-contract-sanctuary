...
contract MoonKnight is IMoonKnight, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    // Total supply of MoonKnights on distribution sale phase.
    uint private constant TOTAL_SALE_KNIGHTS = 20000;
    ...
    function setFloorPriceCap(uint value) external onlyOwner {
        require(value > 0, "MoonKnight: invalid value");
        floorPriceCap = value;
    }
    ...
    function claimMoonKnight(uint amount) external override payable {
        ...
        require(balanceOf(account) + amount <= 10, "MoonKnight: 10 at most");
        require(_knights.length + amount <= TOTAL_SALE_KNIGHTS, "MoonKnight: sold out");
        require(value == _salePrice * amount, "MoonKnight: incorrect value");
        ...
        (bool isSuccess,) = owner().call{value: value - (floorPrice * amount)}("");
        require(isSuccess, "MoonKnight: transfer failed");

        if (startingIndex == 0 && (_knights.length == TOTAL_SALE_KNIGHTS || block.timestamp >= _revealTime)) {
    ...
    function changeKnightName(
        ...
        require(_validateStr(newName, false), "MoonKnight: invalid name");
    ...
    function attachSymbolToKnight(
        uint knightId,
        string memory symbol
    ) external override onlyKnightOwner(knightId) collectTokenAsFee(serviceFeeInToken, owner()) {
        require(_validateStr(symbol, true), "MoonKnight: invalid symbol");
        ...
        _knights[knightId].symbol = symbol;
    ...
    function addFloorPriceToKnight(uint knightId) external override payable {
        ...
        Knight storage knight = _knights[knightId];
        uint newFloorPrice = knight.floorPrice + increasedAmount;

        require(newFloorPrice <= floorPriceCap, "MoonKnight: cannot add more");

        knight.floorPrice = newFloorPrice;

        acceptedToken.safeTransferFrom(msg.sender, owner(), feeInToken);

        emit KnightPriceIncreased(knightId, newFloorPrice, increasedAmount);
    }
    ...
    function offer(uint knightId) external override payable {
        ...
        knightsWithOffers[knightId][buyer] = offeredValue;

        emit KnightOffered(knightId, buyer, offeredValue);
    }
    ...
    function _finalizeStartingIndex() private {
        startingIndex = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % TOTAL_SALE_KNIGHTS;
...
interface IMoonKnight {
    ...
    event SymbolChanged(uint indexed knightId, string symbol);
    ...
    event StartingIndexFinalized(uint startingIndex);

    /**
     * @notice Claims moon knights when it's on presale phase.
     */
    function claimMoonKnight(uint amount) external payable;
     ...
     * - Does not attach to any `symbol`.
     * - Only 5 knights have the same `symbol`.
     * - Token required: `serviceFeeInToken`
     */
    ...
    /**
     * @notice Operators can generate a Knight
     */
    function generateKnight(address account) external;
