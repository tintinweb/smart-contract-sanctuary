...
contract LunchMoney is Ownable {
    ...
    function stake(uint256 _quantity) public {
        ...
        require(isWhitelist[msg.sender], "LMY: Address Not whitelisted");
    ...
    // Safe LunchMoney withdraw function by admin
    function safeLunchMoneyWithdraw(address _to, uint256 _amount) onlyOwner external {
        uint256 LunchMoneyBalalance = IBEP20(TOKEN).balanceOf(address(this));
        if (_amount > LunchMoneyBalalance) {
            IBEP20(TOKEN).transfer(_to, LunchMoneyBalalance);
        } else {
