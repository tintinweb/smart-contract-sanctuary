...
contract DynamicsBuyBack is AuthorizedList, LockableSwap, IBuyBack {
    ...
    function buyBackTokens(address destination) external override authorized {
        if(!inSwapAndLiquify)
            _buyBackTokens(address(this).balance, destination);
    }
    ...
    function swapETHForTokens(uint256 amount, address destination) private {
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{gas: gasleft(), value: amount}(
            ...
            path,
            destination,
            block.timestamp.add(450)
        );
    }

    function buybackForce(uint256 amount) external authorized {
        require(amount * 1 ether >= address(this).balance, "Balance too low for this transaction");
        _buyBackTokens(amount, address(this));
    }
