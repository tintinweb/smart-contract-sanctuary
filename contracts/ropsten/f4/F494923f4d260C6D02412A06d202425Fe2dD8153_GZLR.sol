...
contract GZLR is Context, IERC20, Ownable {
    ...
    function updateFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner {
        ...
        _swapFee = _liquidityFee + _marketingFee;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateFee(tAmount, _swapFee);
    ...
    function _transfer(
        ...
        if(!isExcludedMxWalletSize(from) && !isExcludedMxWalletSize(to)) {
            if( to != _uniswapV2Pair && to != address(_uniswapV2Router) ) 
                require( balanceOf(address(to)) + amount <= _maxWalletSize, "Balance exceeds the maxWalletSize.");
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        if (marketingFeeETH > 0) {
            ...
            emit MarketingFeeSent(_marketingWallet, marketingFeeETH);
        }
