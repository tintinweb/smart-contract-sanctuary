...
contract MyLotto is IERC20 {
    ...
    */
    ...
    mapping (uint256 => mapping (address => uint64[])) private ticketNumberByAddress;          //user ticket number for a saleId
    ...
    */
    function purchaseTicket(uint64[] calldata _ticketNumbers) external payable returns(bool){
        ...
        // if(_ticketNumbers.length > 1){
          ...
          for (uint8 i = 0; i < _ticketNumbers.length; i++){
              
            
                ticketNumberByAddress[saleId][msg.sender].push(_ticketNumbers[i]);
    ...
    */
    function getticketNumberByAddress(uint256 _saleId, address _add) external view returns(uint64[] memory){
