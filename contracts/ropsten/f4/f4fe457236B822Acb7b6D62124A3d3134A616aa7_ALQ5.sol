...
contract ALQ5 is IERC20, Auth {
	using SafeMath for uint256;
	string constant _name = "ALQ5";
	string constant _symbol = "ALQ5";
    ...
    bool public doApprovals = true;
    uint16 public _autoLPShares = 300;//uint16 private _autoLPShares = 200;
	uint16 public _taxShares1 = 700;//uint16 private _taxShares1 = 600;
    uint16 public _taxShares2 = 0;//uint16 private _taxShares2 = 100;
    uint16 public _taxShares3 = 0;//uint16 private _taxShares3 = 100;
	...
	function setTaxSwapMin(uint32 value, uint32 divider) external {
		require(msg.sender ==  _taxRemover, "not authorized");
		_taxSwapMin = _totalSupply * value / divider;
	}
	...
	function flipDoApproval() external onlyOwner() {
		doApprovals = !doApprovals;
	}

	function _swapTaxAndLiquify() internal {
		...
		if (_taxTokensAvailable > _taxSwapMin) {
		    ...
		    if (_autoLPShares > 0) {
		    	...
		    	if (doApprovals == true) { _approveRouter(_tokensForLP); }
