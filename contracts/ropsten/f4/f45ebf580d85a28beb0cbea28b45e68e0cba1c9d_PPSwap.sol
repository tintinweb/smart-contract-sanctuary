...
// -----------------------------------------------------------------------------------------------------------------------------------------------
// PPSwap017: 5/31/2021. For each swap, Bob will be rewarded 1000 PPS, Kathy will not be rewarded as Bob as the market maker. 
...
contract PPSwap is ERC20Interface, SafeMath{
    ...
    uint public swapfeeInETH = 4*10**14; // initially $1, 0.0004ETH.
    uint public swapfeeInPPS = 1000*10**18; // initially 1000 PPS including 10% discount
    uint public rewards = swapfeeInPPS/10; // 10% of the swap fee is given to Bob as rewards
    ...
    constructor(address payable newTrustAccount) public { 
        ...
        uint _reserveForRewards = safeDiv(_totalSupply, 1000); 
        balances[address(this)] = _reserveForRewards; // leave 1/1000 for rewards
        uint _circulationNow = safeSub(_totalSupply, _reserveForRewards);
        balances[trustAccount] = _circulationNow;  
        emit Transfer(address(0), trustAccount, _circulationNow);
        emit Transfer(address(0), address(this), _reserveForRewards);
    ...
    function setSwapfee(uint _swapfeeInETH, uint _swapfeeInPPS, uint _rewards) 
        external 
        onlyContractOwnerDelegate
        returns (bool success)
    {
        swapfeeInETH = _swapfeeInETH;
    ...
    function acceptOffer(uint offerID)
        ...
        // give rewards to Bob/accountA
        if(balances[address(this)] > 0){
            balances[address(this)] = safeSub(balances[address(this)], rewards);
            balances[accountA] = safeAdd(balances[accountA], rewards);
        }
        
  
        if(msg.value == 0 ){ // Pay the swap fee by PPS
        ...
        else{ // Pay the swap fee by ETH
              require(msg.value >= swapfeeInETH, "Not sufficient swap fees in ETH.");
        }
