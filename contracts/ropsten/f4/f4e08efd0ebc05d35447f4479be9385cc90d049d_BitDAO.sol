...
library Arrays {
   /**
     ...
     * repeated elements.
     */
    function findUpperBound(uint256[] storage g_array, uint256 g_element) internal view returns (uint256) {
        if (g_array.length == 0) {
            return 0;
        }

        uint256 low = 0;
        uint256 high = g_array.length;
        ...
        if (low > 0 && g_array[low - 1] == g_element) {
            return low - 1;
        } else {
            return low;
        }
...
library Counters {
    ...
    function current(Counter storage g_counter) internal view returns (uint256) {
        return g_counter._value;
    }

    function increment(Counter storage g_counter) internal {
        // The {SafeMath} overflow check can be skipped here, see the comment at the top
        g_counter._value += 1;
    }

    function decrement(Counter storage g_counter) internal {
        g_counter._value = g_counter._value.sub(1);
    }
}
...
contract BitDAO is ERC20 {
	...
	constructor(address _admin) public ERC20("BitDAO", "BIT") {
		admin = _admin;
