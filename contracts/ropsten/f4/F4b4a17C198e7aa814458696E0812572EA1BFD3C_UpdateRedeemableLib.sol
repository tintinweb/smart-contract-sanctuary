...
library ComputeCyclesHeldLib {
  ...
  function _computeCyclesHeld(
    ...
    if (cycleEnd == 0) cycleEnd = currentTimestamp + interval;
      ...
      uint256 cyclesMissed = currentTimestamp.sub(cycleEnd).div(interval).add(1);
...
library UpdateRedeemableImplLib {
    ...
    function _updateCumulativeRewards(StakingControllerLib.Isolate storage isolate, address _user) internal {
      ...
      if (isolate.currentDay > user.lastDaySeen) {
        user.cumulativeRewardWeight = user.cumulativeRewardWeight.add(isolate.currentDay.sub(user.lastDaySeen).mul(user.weight));
    ...
    function _updateRedeemable(
        ...
        if (locals.lastDayChanged < isolate.currentDay) {
            locals.dayDifference = isolate.currentDay.sub(
                locals.lastDayChanged
            );
/*
...
*/  
            ...
            uint256 denominator = Math.max(Math.min(isolate.cumulativeTotalWeight.sub(user.cumulativeTotalWeight), isolate.totalWeight.mul(locals.dayDifference)), uint256(1 ether));
            
            redeemable =
                locals.dayDifference.mul(isolate.inflateBy).mul(user.cumulativeRewardWeight).div(denominator);

            if (multiplier > uint256(1 ether))
                bonuses = redeemable.mul(multiplier.sub(uint256(1 ether))).div(multiplier);
            else bonuses = 0;
        }
    }
    ...
    function _determineMultiplier(
        ...
        for (uint256 i = isolate.currentCycle; i > 0; i--) {
            ...
            if (locals.minimum < locals.tier.minimum) {
                if (
                    isolate.lockCommitments[user] == locals.tierIndex &&
                    penaltyChange
                ) {
    ...
    function _updateDailyStatsToLast(
        ...
        if (user.start == 0) init = true;
        {
            ...
            if (user.cyclesHeld > 0 && user.cyclesHeld > cycleChange) {
                for (uint256 i = user.cyclesHeld; i > cycleChange; i--) {
                    weightChange.changes[i] = user.weight.mul(1 ether).div(user.multiplier);
	    ...
	    user.cumulativeRewardWeight = 0;
            isolate.dailyBonusesAccrued[sender] = isolate.dailyBonusesAccrued[
                sender
            ]
                .add(bonuses);
	...
	*/
    ...
    function _computeNewRedeemablePrincipalSplit(
        ...
        uint256 total =
            isolate.dailyBonusesAccrued[user]
                ...
                .div(uint256(1 ether));
        StakingControllerLib.DailyUser storage dailyUser =
            isolate.dailyUsers[user];
	...
	//StakingControllerLib.User storage dailyUser = cycle.users[sender];
        ...
        if (oldBalance != newBalance) {
            if (locals.currentWeight == oldBalance)
                user.currentWeight = newBalance;
            else
       ...
       // _recalculateDailyWeights(isolate, sender, newBalance.mul(dailyUser.multiplier).div(uint256(1 ether)), penalty);
        ...
        if (
            user.daysClaimed - cycle.day - 1 > 0 &&
            locals.previousMinimumWeight > 0
        ) {
            locals.totalInflated;
            locals.daysToRedeem;
            if (cycle.day - 1 > user.daysClaimed)
                locals.daysToRedeem = uint256(cycle.day - 1).sub(
                    user.daysClaimed
                );
            ...
            if (locals.totalInflated > 0) {
                ...
                if (locals.previousMultiplier > 1 ether) {
                    ...
                    isolate.bonusesAccrued[sender] = isolate.bonusesAccrued[
                        sender
                    ]
                        .add(locals.bonus);
                }
        ...
        locals.minimumWeight = Math.min(
            user.minimumWeight,
            locals.currentWeight
        );
        ...
        locals.currentTotalWeight = cycle
            .totalWeight
            .add(
            locals.minimumWeight.mul(locals.multiplier).div(uint256(1 ether))
        )
            .sub(
        ...
        cycle.totalRawWeight = cycle
            ...
            .sub(
            locals.currentWeight.mul(locals.previousMultiplier).div(1 ether)
        );
