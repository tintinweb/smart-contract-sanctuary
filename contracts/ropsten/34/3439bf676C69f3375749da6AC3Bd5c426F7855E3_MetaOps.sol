 ...
 * @title nft contract
 */
contract MetaOps is Mintable, ERC721, ERC721Enumerable {
    string private _baseURIPath;
    uint256 public constant tokenPrice = 10000000000000000; // 0.05 ETH
    uint public maxTokenPurchase = 1;
    uint public maxTokenPerWallet = 3;
    uint256 public MAX_TOKENS = 2061;
    // token purchase per wallet
    mapping (address => uint) private _numberOfWallets;
    mapping(uint256 => bool) private _isOccupiedId;
    // minted token id list
    ...
    // Called before minting token in IMX L2 to pay token price.
    function buy(uint256 _id) public payable {
        require(_numberOfWallets[msg.sender] + 1 <= maxTokenPerWallet, "Exceeded max token purchase per wallet");
        ...
        require(_isOccupiedId[_id] == false, "MINT: Those ids already have been used for other customers");
