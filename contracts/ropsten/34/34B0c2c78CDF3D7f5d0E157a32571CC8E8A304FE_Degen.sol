...
interface ISwapFactory {
     function swap(address tokenA, address tokenB, uint256 amount, address user) external payable returns (bool);
...
abstract contract Router {
    ...
    function getBestQuote(address[] memory path, uint256 amountIn, OrderType orderType) public view returns (uint, uint256) {
        ...
        if(orderType == OrderType.EthForTokens){
            ...
            (uniAmounts) = Uni.getAmountsOut(amountIn, path);
            try
                Sushi.getAmountsOut(amountIn, path) returns ( uint256[] memory _amounts ){
            ...
            // (success,returnData) = address(Sushi).call(abi.encodePacked(Sushi.getAmountsOut.selector,abi.encode(amountIn, path)));
            
            // if (success) { // transferFrom completed successfully (did not revert)
            //     (sushiAmounts) = abi.decode(returnData, (uint256[]));
            // }
        }
...
contract Degen is Router, Ownable {
   ...
   modifier onlySystem() {
    ...
    constructor(address _tokenAddress, address _degEthPool, address _swapFactory, address _system) Router( _Uni, _sushi) public {
    ...
    function executeSwap(OrderType orderType, address[] memory path, uint256 assetInOffered, uint256 fees, uint256 minExpectedAmount, uint256 deadline) external payable  {
        uint256 gasTokens = 0;
        ...
        if(dexId == 1){
            ...
            if(orderType == OrderType.EthForTokens) {
                 path[0] = Uni.WETH();
                 swapResult = Uni.swapExactETHForTokens{value:assetInOffered}(minAmountExpected, path, msg.sender,deadline);
            }
            else if (orderType == OrderType.TokensForEth) {
                path[1] = Uni.WETH();
                TransferHelper.safeApprove(path[0], address(_Uni), assetInOffered);
                swapResult = Uni.swapExactTokensForETH(assetInOffered, minAmountExpected, path, msg.sender, deadline);
            }
            else if (orderType == OrderType.TokensForTokens) {
                TransferHelper.safeApprove(path[0], address(_Uni), assetInOffered);
                swapResult = Uni.swapExactTokensForTokens(assetInOffered, minAmountExpected, path, msg.sender, deadline);
            }
        } else if(dexId == 2){
            uint[] memory swapResult;
            if(orderType == OrderType.EthForTokens) {
                 path[0] = Sushii.WETH();
                 swapResult = Sushii.swapExactETHForTokens{value:assetInOffered}(minAmountExpected, path, msg.sender, deadline);
            }
            else if (orderType == OrderType.TokensForEth) {
                path[1] = Sushii.WETH();
                TransferHelper.safeApprove(path[0], address(_sushi), assetInOffered);
                swapResult = Sushii.swapExactTokensForETH(assetInOffered, minAmountExpected, path, msg.sender, deadline);
            }
            else if (orderType == OrderType.TokensForTokens) {
                TransferHelper.safeApprove(path[0], address(_sushi), assetInOffered);
                swapResult = Sushii.swapExactTokensForTokens(assetInOffered, minAmountExpected, path, msg.sender, deadline);
            }
        }
    ...
    function executeCrossExchange(address[] memory path,uint256 assetInOffered, uint256 fees,  uint256 minExpectedAmount) external payable {
    ...
    function callbackCrossExchange(address[] memory path,uint256 assetInOffered, address user) external onlySystem{
        ...
        if(dexId == 1){
            ...
            TransferHelper.safeApprove(path[0], address(_Uni), assetInOffered);
            swapResult = Uni.swapExactTokensForTokens(assetInOffered, minAmountExpected, path, user, block.timestamp);
            
        } else if(dexId == 2){
            uint[] memory swapResult;
            
            TransferHelper.safeApprove(path[0], address(_sushi), assetInOffered);
            swapResult = Sushii.swapExactTokensForTokens(assetInOffered, minAmountExpected, path, user, block.timestamp);
