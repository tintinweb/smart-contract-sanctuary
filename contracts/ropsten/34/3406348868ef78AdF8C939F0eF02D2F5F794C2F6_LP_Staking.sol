...
contract LP_Staking {
  ...
  uint256 initial_epoch_output;

  // prelaunch_precision == 100%
  // prelaunch_precision/2 == 50%
  function init(address[] calldata user_list, uint32[] calldata initial_epoch_standing, uint256 _initial_epoch_output) public {
    require(initial_epoch_output == 0, "already initialized");
    require(user_list.length == initial_epoch_standing.length, "array length mismatch");
    initial_epoch_output = _initial_epoch_output;
    for(uint8 user_idx = 0; user_idx < user_list.length; user_idx++){
      initial_standing[user_list[user_idx]] = initial_epoch_standing[user_idx];
    }
  }

  mapping(address => uint32) initial_standing;
  ...
  uint256 public constant precision = 2**24;
  uint256 public constant prelaunch_precision = 10000000;
  Epoch[] public epochs;
  ...
  function stake(uint256 value) public {
    ...
    require(user.balance == 0, "user must unstake before adding more");

    IERC20(lp_token_address).transferFrom(msg.sender, address(this), value);
    uint8 current_epoch_number = get_current_epoch_number();
    require(current_epoch_number < final_epoch, "this staking contract has finished.");
    user.balance = value;
    user.last_epoch_withdrawn = current_epoch_number;
    Epoch storage latestEpoch = epochs[epochs.length-1];
    // push new epoch
    if(latestEpoch.epoch_number < current_epoch_number) {
      epochs.push(Epoch(latestEpoch.balance + value, latestEpoch.balance, current_epoch_number));
      return;
    }

    latestEpoch.balance += value;
  }
  ...
  function unstake() public {
    ...
    User storage user = users[msg.sender];
    Epoch storage latestEpoch = epochs[epochs.length-1];
    latestEpoch.balance -= user.balance;
    if(latestEpoch.balance < latestEpoch.minimum_balance){
      latestEpoch.minimum_balance = latestEpoch.balance;
    }

    IERC20(lp_token_address).transfer(msg.sender, user.balance);
    delete(users[msg.sender]);
  }


  function get_available_reward(address account) public view returns(uint256) {
    uint256 final_award = (initial_standing[account] * initial_epoch_output)/prelaunch_precision;
    uint8 epoch_span;
    ...
    for(;epoch_idx >= 0 ; epoch_idx--) {
      ...
      if(current_epoch.epoch_number == current_epoch_number){
        ...
        if(current_epoch.minimum_balance == 0){
          break;
        }
  ...
  function withdraw_rewards() public {
    ...
    uint8 current_epoch_number = get_current_epoch_number();
    if(users[msg.sender].last_epoch_withdrawn == current_epoch_number
      || current_epoch_number == 0
      || users[msg.sender].balance == 0) {
      //only init can have a value here
      if(initial_standing[msg.sender] > 0) {
        delete(initial_standing[msg.sender]);
      }
      if(final_award > 0){
