...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _mint(address account, uint256 amount) internal virtual {
     ...
     function refund() external {
        uint256 refundedAmount = _balances[msg.sender];
        require((_TotalSaled < 98e8) , "cap reached");
        require(block.timestamp >= contractcreationtime + 120 && !first,"time limit not reached wait 6 minutes after contract deployment ");
        _balances[msg.sender] = 0;
        transfer(msg.sender, refundedAmount);
    ...
    function GetPrice(uint256 TokenQuantity)public pure returns (uint256){
     ...
     function Total_Sales_Token()public view returns (uint256){
    ...
    function Withdraw (uint256 amount) public returns(bool){
        require(msg.sender == _owner , "NOT Accessable");
        require((_TotalSaled >= 98e8) , "cap not reached");
