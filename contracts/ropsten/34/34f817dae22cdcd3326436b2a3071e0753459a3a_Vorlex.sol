...
contract Vorlex  is ERC1155, ERC1155Holder,  AccessControl  {
    ...
    */
    ...
    event InFunded(bytes32 indexed metadata);
    ...
    event SaleSetup(bytes32 indexed sale_id, uint256 indexed amount, 
            uint256 indexed worth, uint256 bid_span, sale_status status);
    ...
    event SaleClosed(bytes32 indexed sale_id);
	...
	mapping(string => uint256) _minted_vorlexes;
    ...
    */
    function inFundContract(bytes32 metadata) external payable {
    ...
    */
    ...
    /**
        ...
        * @param new_custodian Wallet Address of New Ancestor
     */
    ...
    function createSale(
        ...
        sales[sale_id] = Sale(new_status, vorlex_id, amount, 0, worth, bid_span, 0, msg.sender, address(0));
        safeTransferFrom(msg.sender, address(this), vorlex_id, amount, "");
        emit SaleSetup(sale_id, amount, worth, bid_span, new_status);
    } 
     ...
     */
    function placeBid(bytes32 sale_id) public payable {
     ...
     */
    function cancelSale(bytes32 sale_id) public payable {
    ...
    /**
        ...
        * @param sale_id Auction ID of ESCROW PURCHASE
    */
    function closeSale(bytes32 sale_id) public payable onlyAdmin {
        require(sales[sale_id].status == sale_status.CANC_ESCROW_BUY ||
                sales[sale_id].status == sale_status.ACTIVE_BID , 
                "INVALID_CLOSE");
        if(sales[sale_id].status == sale_status.ACTIVE_BID){
            ...
            payable(vorlexes[sales[sale_id].vorlex_id].custodian).transfer(sales[sale_id].worth/10); // anc Royalty
            payable(sales[sale_id].seller).transfer(sales[sale_id].worth * 4/5);// Owner's Payout
            sales[sale_id].status = sale_status.CLOSED_BID;
        } else {
            uint256 payout = sales[sale_id].worth * sales[sale_id].purchased_amount;
            payable(vorlexes[sales[sale_id].vorlex_id].custodian).transfer(payout/10); // anc Royalty
            payable(sales[sale_id].seller).transfer(payout * 4/5);// Owner's Payout
            sales[sale_id].status = sale_status.CLOSED_ESCROW_BUY;
        }
        emit SaleClosed(sale_id);
    }
    ...
    */
    ...
    function batchMove(
        uint256[] memory vorlex_ids, uint256[] memory amounts, 
        string memory metadata, address from, address to, bool is_swap
    ...
    /**
        * @dev Vorlexes can only be transferred to this Contract or From this Contract by NON-ADMIN
