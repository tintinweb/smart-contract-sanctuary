...
contract SwapJNTRFactory is Ownable {
    ...
    */
    ...
    function claimTokenBehalf(address tokenA, address tokenB, address user,uint256 amount,uint8 crossOrderType, uint256 nonce) external onlySystem notPaused returns (bool) {
        require(!nonceProcessed[nonce], "Exchange already processed");
        nonceProcessed[nonce] = true;
        _claim(tokenA, tokenB, user,amount,crossOrderType);
        return true;
    }

    function _claim (address tokenA, address tokenB, address user, uint256 amount, uint8 crossOrderType) internal returns(bool) {
        ...
        if(tokenA == USDT) {
            ...
            emit ClaimApprove(tokenA, tokenB, user, amount, crossOrderType);
        }
