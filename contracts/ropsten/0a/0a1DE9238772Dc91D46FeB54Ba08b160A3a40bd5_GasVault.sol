 ...
 */
contract GasVault is IGasVault, Initializable, OwnableUpgradeable, UUPSUpgradeable   {
     ...
     * maxGasPerMethod is total acceptable amount of gas to be sent in one Method.
    ...
    function initialize(address controller, address stratRegistry) public initializer   {
        __Ownable_init();
        __UUPSUpgradeable_init();

        orchestrator = IOrchestrator(controller);
        strategyRegistry = IStrategyRegistry(stratRegistry);
    }

    function _authorizeUpgrade(address) internal override onlyOwner {}

    modifier onlyOrchestrator   {
        require(msg.sender == address(orchestrator), "Only orchestrator can call this");
        _;
    }
     ...
     */
    function deposit(address targetAddress)
        external
        payable
        override
    {
     ...
     */
    function transactionsRemaining(address targetAddress, uint256 gasCostEstimate) 
    external 
    view 
    override
    returns(uint256)   {
        gasInfo memory thisGasInfo = gasAccounts[targetAddress];
        IStrategyRegistry.RegisteredStrategy memory info = strategyRegistry.strategyDetails(thisGasInfo.tokenID);
        if (gasCostEstimate > info.maxGasCost)   {
            return 0;
        }
        uint256 totalGasPerMethod = info.maxGasPerMethod * info.maxMethods * gasCostEstimate * gasDecimals;
     ...
     * @param actionsLength is number of actions orchestrator will be performing
     * @return uint256 gasAvailable (representing amount of gas available per Method).
     */
    function setAsideGas(address targetAddress, uint256 actionsLength) 
    external 
    onlyOrchestrator 
    override 
    returns(uint256)   {
        IStrategyRegistry.RegisteredStrategy memory info = strategyRegistry.strategyDetails(gasAccounts[targetAddress].tokenID);
        //Ensure requested gas use is acceptable
        require(tx.gasprice <= info.maxGasCost, "Gas too expensive.");
        require (actionsLength <= info.maxMethods, "Too many methods");
        uint256 gasAvailable = info.maxGasPerMethod; //Represents gas available for EACH method call TODO: find more efficient way requiring fewer state updates
        gasAccounts[targetAddress].ethBalance -= (tx.gasprice * gasAvailable) * actionsLength;
        ...
        require (gasleft() >= (gasAvailable * actionsLength) + 50000, "Insufficient gas allotted to job (node-side error)");
    ...
    onlyOrchestrator   {
        ...
        gasAccounts[targetAddress].ethBalance += (originalGas - gasUsed) * tx.gasprice;
...
interface IGasVault   {
     ...
     * @return uint256 gasAvailable (representing amount of gas available per Method).
     */
    function setAsideGas(address targetAddress, uint256 actionsLength) external returns(uint256);
...
interface IOrchestrator {
    enum actionState { PENDING, APPROVED, COMPLETE }
    /**
     ...
     */
    function createAction(bytes4[] memory _methods, bytes[] memory _args, address targetAddress) external returns (uint256 actionId);
     ...
     *
     */
    function getActionInfo(uint256 _actionId) external view 
    returns (bytes4[] memory methods, bytes[] memory args, address targetAddress, actionState state);
     ...
     */
    event ActionStateChanged(uint256 _actionId, address _from, actionState _state);

    event vote(uint256 _actionId, address _from, bool _approved);

    event actionFailed(uint256 _actionId, uint256 _method);
...
interface IStrategyRegistry is IERC721Upgradeable{

    function getStrategyAddress (uint tokenID) external view returns (address);
    function getStrategyOwner (uint tokenID) external view returns (address);
    ...
    function supportsInterface(bytes4 interfaceId) external
        view
        override//(AccessControlEnumerableUpgradeable, IERC721Upgradeable, ERC721EnumerableUpgradeable)
        returns (bool);

    function tokenURI(uint256 tokenId) external view returns (string memory);

    function strategyDetails(uint256 tokenId) external view returns (RegisteredStrategy memory);
     ...
     */
    function setGasParameters(uint256 _tokenID, uint128 _maxGasCost, uint128 _maxGasPerMethod, uint16 _maxMethods) external;
