...
contract WorldOfNorja is ERC721URIStorage, Ownable{
 
    event MintOrcs (address indexed minter, uint256 startWith, uint256 times);
    
    
    uint256 public totalOrcs;
    ...
    uint256 public releaseTime = 1630447200; // works KEKW
    ...
    modifier canWhitelistMint() {
        require(block.timestamp >= presaleRelease, "Wait");
        require(whiteListStart, "Not started");
    ...
    function mintOrc(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "Mint wrong number");
        require(totalOrcs + _times <= totalCount, "Too much");
        require(msg.value == _times * price, "Value error");
        payable(owner()).transfer(msg.value);
        emit MintOrcs(_msgSender(), totalOrcs+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalOrcs++);
        }
    } 
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalOrcs + _times <= totalCount, "Mint amount will exceed total collection amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalOrcs + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintOrcs(_addr, totalOrcs+1, 1);
        _mint(_addr, 1 + totalOrcs++);
    } 
