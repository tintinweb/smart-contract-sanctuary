...
contract TimedCrowdsale is Crowdsale {
    ...
    function getCurrentRate() public view virtual returns (uint256) {
        require(rates.length > 0, "Rates length");

        // solhint-disable-next-line not-rely-on-time
        uint256 currentTime = block.timestamp;
        uint256 currentRate = rates[0][0];
        for (uint8 i = 0; i < rates.length; i++) {
            if (currentTime >= rates[i][1] && currentTime <= rates[i][2]) {
                currentRate = rates[i][0];
            }
        }
        ...
        returns (uint256)
    {
        ...
        return weiAmount.div(currentRate) * 10**decimals;
    }
    ...
    function hasFinalClosed() public view returns (bool) {
        return block.timestamp > rates[rates.length][2];
    }
...
contract PostDeliveryCrowdsale is TimedCrowdsale {
    ...
    event TokenPurchased(string description, address indexed _from, uint256 _n);
    ...
    constructor(
        ...
        address payable wallet_,
        IERC20 token_
    ) TimedCrowdsale(decimals, rates, wallet_, token_) {
    ...
    function withdrawTokens(address beneficiary) public {
        require(hasFinalClosed(), "PostDeliveryCrowdsale: not closed");
