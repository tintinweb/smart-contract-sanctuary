...
contract FastExchange is Owned, SafeMath {
    ...
    uint public constant minEth = 0.05 ether;
    uint public constant maxEth = 5 ether;
    uint public lockedEth;
    ...
    function setTokenInfo(string _symbol, address _address) public returns (string, address){
        tokenAddresses[_symbol] = _address;
        return (_symbol, _address);
    }

    function getTransactionListLength() public constant returns (uint length){
        length = transactions.length;
    }

    function getTransactionDetail(uint _transactionId) public constant ValidTransactionId(_transactionId) returns (address, uint, uint, uint, uint, uint){
    ...
    function getUserTransactionIndexes(address _user) public constant returns(uint[]) {
        return transactionBook[_user];
    }

    function getLastPendingTransaction() public constant returns(uint, bool) {
