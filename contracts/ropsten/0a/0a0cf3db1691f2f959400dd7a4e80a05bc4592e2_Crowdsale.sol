...
contract Crowdsale is Ownable {
  ...
  function Crowdsale() public {
    accountAddress = 0xb11D7cd4D759493DbCF32505054c0363d37773aA;	// public sale address
    rate = 2000000000000;
    start = 1614304428; /* 03/26/2021 @ 09:53am (UTC) */
    period = 5 * 365;
    per_p_sale = 5;
    per_sale = 365;
    sale_pre_sale = 50;
    ...
    presaleTokens    = 800000000 * 1 ether;
  ...
  function setSale(uint _sale_pre_sale, uint _sale_1_week, uint _sale_2_week, uint _sale_3_week, uint _sale_4_week, uint _sale_5_week) public onlyOwner {
  ...
  function DeliverTokens() SaleOn payable public {

    uint tokens = rate.mul(msg.value).div(1 ether);
    uint bonusTokens = 0;
    start_ico = start + per_p_sale * 1 days; 
    accountAddress.transfer(msg.value);    
	
    if(now < start_ico) 
    { 
     if(address(this).balance >= ini_supply.sub(presaleTokens))
       {
