...
contract LiquidityRegistry is ILiquidityRegistry, AbstractDependant {
    ...
    function getLiquidityInfos(
        ...
        for (uint256 i = _offset; i < _to; i++) {
            ...
            (uint256 _lockedAmount, , ) = IPolicyBook(_currentPolicyBookAddr).withdrawalsInfo(
                _userAddr
            );
            uint256 _availableAmount = IERC20(address(_currentPolicyBookAddr)).balanceOf(
                _userAddr
            );
        ...
        returns (uint256 _arrLength, WithdrawalRequestInfo[] memory _resultArr)
    {
        ...
        for (uint256 i = _offset; i < _to; i++) {
            ...
            IPolicyBook.WithdrawalStatus _currentStatus = _currentPolicyBook.getWithdrawalStatus(
                _userAddr
            );
            ...
            (uint256 coverTokens, uint256 liquidity) = _currentPolicyBook
    ...
    function getWithdrawalSet(
        ...
        for (uint256 i = _offset; i < _to; i++) {
            ...
            (uint256 _requestAmount, , bool withdrawalAllowed) = _currentPolicyBook
            .withdrawalsInfo(_userAddr);
