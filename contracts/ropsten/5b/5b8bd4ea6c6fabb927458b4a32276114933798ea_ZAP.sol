...
contract ZAP is Context, IERC20, Ownable {
   ...
   // address payable public _devWalletAddress;
    ...
    function rebasePlus(uint256 _amount) private {
        //uint256 _totalMe = _amount.div(7);
        
        //_devWalletAddress.transfer(_amount.div(25));
         _totalSupply = _totalSupply.add(_amount.div(7));
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if(!_live)
                ...
                require(balanceOf(from) <= txLimitAmount, 'ERC20: current balance exceeds the max limit.');
