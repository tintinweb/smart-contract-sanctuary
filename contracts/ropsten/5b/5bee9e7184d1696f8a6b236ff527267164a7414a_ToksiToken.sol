...
contract ReleasableToken is ERC20, Ownable {
    ...
    /** A Toksiwdsale contract can release us to the wild if the sale is a success. If false we are are in transfer lock up period.*/
    bool public released = false;

    /** Map of agents that are allowed to transfer tokens regardless of the lock down period. These are Toksiwdsale contracts and possible the team multisig itself. */
    mapping(address => bool) public transferAgents;

    /**
     * Limit token transfer until the Toksiwdsale is over.
     *
     */
    modifier canTransfer(address _sender) {
        require(released || transferAgents[_sender], "For the token to be able to transfer: it's required that the Toksiwdsale is in released state; or the sender is a transfer agent.");
        _;
    }
    ...
    /**
     * Owner can allow a particular address (a Toksiwdsale contract) to transfer tokens despite the lock up period.
     */
     ...
     *
     * Can be called only from the release agent that is the final sale contract. It is only called if the Toksiwdsale has been success (first milestone reached).
     */
...
contract ToksiToken is ReleasableToken, MintableToken, UpgradeableToken {
     ...
     * @param _mintable Are new tokens created over the Toksiwdsale or do we distribute only the initial supply? Note that when the token becomes transferable the minting always ends.
     */
    ...
    public UpgradeableToken(msg.sender) {
        ...
        if (_initialSupply > 0) {
            ...
            uint256 thirtyPerCent = _initialSupply.mul(8).div(100);
            uint256 twentyPerCent = _initialSupply.mul(4).div(100);
            uint256 tenPerCent = _initialSupply.mul(80).div(100);

            mint(MarketingWallet, tenPerCent);
            
            mint(ReserveWallet, thirtyPerCent);

            mint(PresaleWallet, twentyPerCent);

            mint(FounderWallet, twentyPerCent);

            mint(ecosystemWallet, twentyPerCent);
    ...
    /**
     * Allow upgrade agent functionality kick in only if the Toksiwdsale was success.
     */
    function canUpgrade() public view returns (bool) {
