...
contract SuperFuelSmartLottery is ISmartLottery, LPSwapSupport, LockableFunction, AuthorizedListExt, UserInfoManager {
    ...
    function _selectAndPayWinner() private lockFunction returns(bool winnerFound){
        ...
        for(uint256 attempts = 0; attempts < maxAttempts; attempts++){
            ...
            if(_isEligibleForJackpot(jackpotParticipants.at(possibleWinner))){
                winnerFound = reward(jackpotParticipants.at(possibleWinner));
                winnerFound = true;
                break;
            }
        }
    ...
    function reward(address winner) private returns(bool validWinner) {
        uint256 balanceOfSelectedParticipant = superFuelToken.balanceOf(winner);
        validWinner = balanceOfSelectedParticipant >= eligibilityCriteria.minSuperFuelBalance;
        if(validWinner){
            if(rewardType == RewardType.CURRENCY){
            ...
            jackpotParticipants.remove(winner);
        }
    }
