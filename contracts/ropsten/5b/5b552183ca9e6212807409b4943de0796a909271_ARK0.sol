...
contract ARK0 is ERC20, CoinAgeMintableStandard, CoinEraMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "ARK0";
    string public symbol = "ARK0";
...
//--------------------------------------------------------------------------------------
    ...
    function getCoinEraMinting(address _address) internal view returns (uint) {
        ...
        uint _now = now;
        uint intervalEra = getCoinEra(_address, _now);
        if(intervalEra < minInterval) return 0;
        uint256 coinsReward = coinEraRate * intervalEra;
        return coinsReward;
    }
    ...
    function getCoinEra(address _address, uint _now) internal view returns (uint intervalEra) {
        if(transferIns[_address].length <= 0) return 0;
        uint i = 0; i < transferIns[_address].length;
        uint intervalDay = _now.sub(uint(transferIns[_address][i].time));
        intervalEra = intervalEra * intervalDay.div(1 days);
    }
