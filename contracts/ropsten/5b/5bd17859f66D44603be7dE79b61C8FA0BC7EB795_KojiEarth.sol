...
contract DividendDistributor is IDividendDistributor {
    ...
    function getUnpaidEarnings(address shareholder) public view returns (uint256 unpaidAmount) {
        if(shares[shareholder].amount == 0 && shares[shareholder].unpaidDividends == 0){ return 0; }

        if(shares[shareholder].amount == 0 && shares[shareholder].unpaidDividends > 0){ return shares[shareholder].unpaidDividends; }

        if(shares[shareholder].amount > 0 && shares[shareholder].unpaidDividends >= 0){
