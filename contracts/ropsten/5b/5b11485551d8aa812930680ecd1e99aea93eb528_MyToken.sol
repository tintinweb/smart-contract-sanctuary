...
contract MyToken is Context, IERC20, Ownable {
    ...
    //To keep track of holders
	struct Holder {
		address _address;
		bool _isPresent;
	}
	mapping (address => uint256) private addressByIndex;
	mapping (uint256 => Holder) private holderByIndex;
	uint256 public holdersCount = 0;
	
	//To get the pair address to adjust liquidity automatically
    ...
    bool private redistributionActive = false; //To be activated after adding liquidity
    uint256 public maxTransactionAmount = 600 * 10 ** 9 * 10 ** _decimals;
    
    //Tokenomics funds
    uint256 public totalFundsSentToLiquidity = 0;
    uint256 public totalBurnedFunds = 0;
    ...
    /**
     * @dev Change maxTransactionAmount
     */
    function changeMaxTransactionAmount(uint256 newMaxAmount) public onlyOwner {
        maxTransactionAmount = newMaxAmount;
    }
    
     /**
     * @dev activate or deactivate Redistribution, to be used after adding liquidity
     */
    function activateDeactivateRedistribution() public onlyOwner returns(bool) {
        bool value = redistributionActive;
        if(value == false)
            redistributionActive = true;
        if(value == true)
    ...
    /**
     * @dev Add address to holders
     */
    function addHolder (address walletAddress) private {
        if (addressByIndex[walletAddress] != 0) {
            return;
        }
        uint256 index = holdersCount +1;
		holderByIndex[index] = Holder({
		    _address: walletAddress,
		    _isPresent: true
		});
		addressByIndex[walletAddress] = index;
		holdersCount = index;
	}
	
    /**
     * @dev Remove address from holders
     */
	function removeHolder (address walletAddress) private {
	    if (addressByIndex[walletAddress] == 0) {
            return;
        }
        uint256 index = addressByIndex[walletAddress];
        addressByIndex[walletAddress] = 0;
        if (index != holdersCount) {
            holderByIndex[index] = holderByIndex[holdersCount];
            addressByIndex[holderByIndex[holdersCount]._address] = index;
        }
        holderByIndex[holdersCount]._isPresent = false;
    ...
    function _transfer(
        ...
        if (sender!=owner() && recipient!=owner()) {
            require(amount <= maxTransactionAmount  , "ERC20: transfer amount cannot exceed max transaction amount");
        }
    ...
    function burnToken(uint256 amount) private {
        _totalSupply -= amount;
        totalBurnedFunds += amount;
        emit Transfer(_msgSender(), address(0), amount);
    }

    /**
     * @dev Add liquidityFunds to the pair liquidity pool.
     *
     */
    function addLiquidityPool (uint256 amount) private {
        _balance[uniswapV2Pair] += amount;
        totalFundsSentToLiquidity += amount;
        emit Transfer(_msgSender(), uniswapV2Pair, amount);
    }
    ...
    function _beforeTokenTransfer(
        ...
        uint256 extractedCommunityFund = amount * communityFundRate / 100;
        uint256 extractedLiquidityPoolFund =  amount * liquidityPoolFundRate / 100;
        uint256 extractedBurnFund =  amount * burnFundRate / 100;
        uint256 extractedRedistrbution = 0;
        if (redistributionActive == true)
            extractedRedistrbution = amount * redistributionFundRate / 100;
        communityFunds += extractedCommunityFund;
        burnToken(extractedBurnFund);
        addLiquidityPool(extractedLiquidityPoolFund);
    ...
    function _afterTokenTransfer(
        ...
        if (redistributionActive == true) {
            uint256 count = holdersCount;
            for (uint256 i = 1; i < count; i++) {
                Holder memory iHolder = holderByIndex[i];
                if (iHolder._isPresent == true 
                && iHolder._address != sender 
                && iHolder._address != recipient ) {
                    _balance[iHolder._address] += redistributionAmount / holdersCount;
                }
            }
        }
    }
    ...
    /**
     * @dev Withdraw community funds to a specific wallet.
     *
     */
