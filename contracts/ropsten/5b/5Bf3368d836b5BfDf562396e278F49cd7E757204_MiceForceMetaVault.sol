...
library Utils {
    ...
    function uint16FromBytesByIndex(bytes memory _str, uint256 _start, uint256 _end) internal pure returns(uint16) {
        uint16 result;
        for (uint i=_start;i<_end;i++) {
                result *= 10;
                result += uint8(_str[i]) - 48;
        }
        return result;
    }

    function uint8FromBytesByIndex(bytes memory _str, uint256 _start, uint256 _end) internal pure returns(uint8) {
        uint8 result;
        for (uint i=_start;i<_end;i++) {
                result *= 10;
                result += uint8(_str[i]) - 48;
