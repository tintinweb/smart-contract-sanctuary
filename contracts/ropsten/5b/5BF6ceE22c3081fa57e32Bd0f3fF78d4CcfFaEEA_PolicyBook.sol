...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    function buyPolicyFor(
        ...
        uint256 _coverTokens
    ) external override onlyPolicyBookRegistry returns (uint256) {
        return _buyPolicy(_buyer, _epochsNumber, _coverTokens);
    }
    ...
    function buyPolicy(uint256 _epochsNumber, uint256 _coverTokens)
        ...
        returns (uint256)
    {
        return _buyPolicy(_msgSender(), _epochsNumber, _coverTokens);
    }
    ...
    function _buyPolicy(
        ...
        uint256 _epochsNumber,
        uint256 _coverTokens
    ) internal withPremiumsDistribution updateBMIDAIStakingReward returns (uint256) {
        ...
        uint256 _totalSeconds =
            secondsToEndCurrentEpoch().add(_epochsNumber.sub(1).mul(EPOCH_DURATION));
    ...
    {
        WithdrawalInfo storage withdrawalInfo = withdrawalsInfo[_userAddr];

        if (withdrawalInfo.readyToWithdrawDate == 0) {
            return WithdrawalStatus.NONE;
        }

        if (block.timestamp < withdrawalInfo.readyToWithdrawDate) {
            return WithdrawalStatus.PENDING;
        }

        if (
            !withdrawalInfo.withdrawalAllowed &&
            block.timestamp >= withdrawalInfo.readyToWithdrawDate.add(READY_TO_WITHDRAW_PERIOD)
        ) {
    ...
    function unlockTokens() external override {
        ...
        require(_lockedAmount > 0, "PB: Amount to unlock must be greater than zero");
    ...
    /// @dev _annualProfitYields is multiplied by 10**25
        ...
        )
    {
        return (
            ...
            totalCoverTokens.add(10000) <= totalLiquidity
                ? policyQuote.getQuote(SECONDS_IN_THE_YEAR, 10000, address(this))
                : 0
        );
    }
