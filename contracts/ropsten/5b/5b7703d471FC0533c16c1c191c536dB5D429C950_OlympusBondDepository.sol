...
interface IStaking {
    function stake(address _recipient, uint _amount, bool _claim ) external returns ( uint256 );
}
...
contract OlympusBondDepository is Ownable {
    ...
    enum PARAMETER { VESTING, PAYOUT, FEE, DEBT, CONTROLVARIABLE, TOTALDEBT }
    /**
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        ...
        } else if ( _parameter == PARAMETER.TOTALDEBT ) { // 5
            totalDebt = _input;
        }
    }
    ...
    function deposit(
        ...
        uint fee = payout.mul( terms.fee ).div( 1000000 );
        ...
        index++;
        ids[index] = _depositor;

        // depositor info is storedre
    ...
    function query(uint id) external view returns(Bond memory) {
        address addr = ids[id];
        Bond memory bond = bondInfo[addr];
        return bond;
    }
    ...
    function redeem( address _recipient, bool _stake ) external returns ( uint ) {
        ...
        if ( percentVested >= 10000 ) { // if fully vested
            ...
            return stakeOrSend( _recipient, _stake, info.payout, true ); // pay user everything due
            ...
            return stakeOrSend( _recipient, _stake, payout, true );
        }
    }
     ...
     *  @return uint
     */
    function stakeOrSend( address _recipient, bool _stake, uint _amount, bool _claim ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( OHM ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            IERC20( OHM ).approve( staking, _amount );
            IStaking( staking ).stake(_recipient, _amount, _claim );
        }
        return _amount;
    }
