...
contract ERC1155NafitiBasic is ERC1155 {
    uint256 public constant NAFI_MULT = 0;
    uint256 public totalTokens = 0;
    
    event TokenMinted(uint256 tokenType, uint256 quantity, address minter);

    constructor() public ERC1155("https://game.example/api/item/{id}.json") {
        _mint(msg.sender, NAFI_MULT, 10**18, "");
    }

    function mint(uint quantity) public returns (uint256){
        uint256 token_type = totalTokens + 1;
        _mint(msg.sender, token_type, quantity, "");
        totalTokens++;
        TokenMinted(token_type, quantity, msg.sender);
        return token_type;
    }
}
...
event CreateERC721Nafiti(address owner, string name, string symbol);
...
contract ERC721Nafiti is ERC721Base {


    function __ERC721Nafiti_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) external initializer {
        _setBaseURI(baseURI);
         __ERC721Lazy_init_unchained();
        //_RoyaltiesV2Upgradeable_init_unchained();
        //__Context_init_unchained();
        //__ERC165_init_unchained();
        __Ownable_init_unchained();
        //__ERC721Burnable_init_unchained();
        ...
        emit CreateERC721Nafiti(_msgSender(), _name, _symbol);
    }
    uint256[50] private __gap;
}
...
contract ERC721NafitiBasic is ERC721 {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() public ERC721("Nafiti", "NAFI") {}
...
import "./ERC721Lazy.sol";
import "../HasContractURI.sol";
import "./ERC721Nafiti.sol";

contract ERC721NafitiUser is ERC721Base {

    // function __ERC721NafitiUser_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) external initializer {
    //     _setBaseURI(baseURI);
    //     __ERC721Lazy_init_unchained();
    //     __Context_init_unchained();
    //     __ERC165_init_unchained();
    ...
    //     }
    //     emit CreateERC721NafitiUser(_msgSender(), _name, _symbol);
    // }
