...
contract RENOVI is ERC721Enumerable, Mintable {
  ...
  function getToken(uint256 tokenId) public view returns(TokenInfoDetailed memory ) {
    ...
    return TokenInfoDetailed(tokenId, _tokenURIs[tokenId], creator, users[creator], owner, users[owner], _tokenStatus[tokenId], _tokenPrice[tokenId]);
  }
  ...
  function update(
    ...
    require(msg.sender != address(0) && msg.sender == _tokenCreator[tokenId] && msg.sender == ownerOf(tokenId), "Not creator/owner.");
    _setTokenURI(tokenId, tokenUri);
    _tokenStatus[tokenId] = status;
    _tokenPrice[tokenId] = price;
    return tokenId;
  ...
  function updateStatus(uint256 tokenId, SaleStatus status, uint256 price) external {
    require(_exists(tokenId));
