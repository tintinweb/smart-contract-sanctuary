...
contract MyContract{
    ...
    function my_status (uint input_1_to_check_ur_status_do_not_be_sneaky) public view returns (bool do_i_have_a_token, uint the_token_index, uint256 my_current_balance){
        if (input_1_to_check_ur_status_do_not_be_sneaky == 1){
            for (uint i = 1; i <= tokenCount; i += 1){
                if (Players[i].address_player == msg.sender){
                    the_token_index = Players[i].token_id;
