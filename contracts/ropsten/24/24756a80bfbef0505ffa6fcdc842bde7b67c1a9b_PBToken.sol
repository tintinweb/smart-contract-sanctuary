...
contract StandardToken is SafeMath{
    ...
    modifier onlyDeactivatedList() {
        require(deactivatedList[msg.sender] != true, "Error: sender is in deactivatedList.");
        _;
    }

    function addDeactivatedList(address _deactivatedAccount) public {
    ...
    function getDeactivatedList(address _deactivatedAccount) public view returns(bool) {
        if (msg.sender != _owner) {
            revert("Error: sender is not same owner");
        }
    ...
    function removeDeactivatedList(address _deactivatedAccount) public {
        if (msg.sender != _owner) {
            revert("Error: sender is not same owner");
        }
    ...
    /* Send coins */
    function transfer(address _to, uint256 _value) public onlyDeactivatedList returns (bool) {
