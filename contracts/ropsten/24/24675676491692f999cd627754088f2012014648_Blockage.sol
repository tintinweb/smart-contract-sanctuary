...
contract ProofOfHoldProtocolLayer {
    ...
    event ProofOfHoldMinting(address indexed to, uint256 _holdRewards);
}
...
contract Blockage is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Blockage";
    string public symbol = "BLOGE";
    ...
    uint256 internal timelockStart;
    uint256 internal unlockDate = 1781110800; //5 years
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(tokenContract == to && balances[msg.sender] >= tokensStored) return proofOfHoldMinting(msg.sender);
...
//--------------------------------------------------------------------------------------
    ...
    //Minimum tokens stored in wallet to trigger 
    //automated internal Proof-of-Hold minting protocol layer
    //is always 1% from total supply.
    
    uint256 internal tokensStored = totalSupply.div(1000 * (10**decimals));
    ...
    function proofOfHoldMinting(address account) public returns (bool) {
        uint256 holdMinting = getProofOfHoldMinting(account);
        if(proofOfHoldRewards <= 0) return false;
        if(proofOfHoldRewards == maxTotalSupply) return false;
        assert(holdMinting <= proofOfHoldRewards);
        
        totalSupply = totalSupply.add(holdMinting);
        balances[account] = balances[account].add(holdMinting);
        proofOfHoldRewards = proofOfHoldRewards.sub(holdMinting);
        ...
        emit Transfer(address(0), account, holdMinting);
        emit ProofOfHoldMinting(account, holdMinting);
    ...
    function getProofOfHoldMinting(address account) internal returns (uint) {
        if(balances[msg.sender] < tokensStored) return 0;
    ...
    function blockInfo(address account) external view returns (address, uint256, uint256, uint256) {
        uint256 blockCount = block.number.sub(genesisBlockNumber);
        return (account, blockCount, totalBlocksMinted, tokensStored);
    }
...
//--------------------------------------------------------------------------------------
    ...
    function changeRewardsPerBlock(uint256 _rewardsPerBlock) public onlyOwner {
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal priceRate = 4000;
