...
contract CHAltar is CHArena {
  ...
  event NewAltarRecord(uint256 _id, uint256 _ethereum);
  event ChickenToAltar(address indexed _user, uint256 _id, uint256 _chicken);
  event EthereumFromAltar(address indexed _user, uint256 _id, uint256 _ethereum);
...
contract CHCommittee is CHAltar {
  ...
  event SetDepot(uint256 _ethereum, uint256 _max);
  ...
  event SetNameAndSymbol(string _name, string _symbol);
    ...
    onlyCommittee
  {
    ...
    pets.push(
      Pet(_huntingPower, _offensePower, _deffense, _chicken, _ethereum, _max)
    );
    ...
    onlyCommittee
  {
    ...
    require(_pet.max > 0 && _max >= _pet.max);
    require(_max == uint256(uint32(_max)));
  ...
  function setNameAndSymbol(string _name, string _symbol) public onlyCommittee {
    name = _name;
    symbol = _symbol;
...
contract ChickenHunt is CHCommittee {
    ...
    )
  {
    ...
    _hunter = [_house.hunter.strength, _house.hunter.dexterity];
    _barrier = [_house.barrier.structure, _house.barrier.material];
    _pets = _house.pets;
