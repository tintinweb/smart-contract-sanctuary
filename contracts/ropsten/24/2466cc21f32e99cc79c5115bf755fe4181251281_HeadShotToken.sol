...
interface ILedger {
    ...
    function getTrackerBalance(address address_, address account_) external returns (uint256);
...
contract HeadShotToken is Context, IERC20, Ownable {
    ...
    */
    ...
    function collect(address ledger_, uint256 code_, uint256 count_) public returns (uint256){
        ILedger iLedger = ILedger(ledger_);
        address tracker_ = iLedger.getTrackerAddress(code_);
        ...
        uint256 _subTotal = _price.mul(_count).mul(10 ** _decimals);
        uint256 _totalPay = _subTotal;

        uint256 _tokenTax = _ecosystemFee.add(_liquidityFee).add(_rewardFee);
        if (_tokenTax > 0){
        ...
        _transferStandard(_account, payable(ledger_), _totalPay);

        emit CollectItem(ledger_, code_, _count, _totalPay);
    ...
    function spend(address ledger_, uint256 code_, uint256 count_) public returns (uint256) {
        ...
        return iLedger.decreaseBalance(tracker_, _msgSender(), count_);
    }
    function withdraw(address ledger_, uint256 code_) public onlyOwner {
        ...
        //uint _amount = _balance.mul(_price).mul(10 ** _decimals);
        ...
        _transfer(ledger_, _pool, _amount);
        emit Withdraw(ledger_, _pool, _amount);
    }
}