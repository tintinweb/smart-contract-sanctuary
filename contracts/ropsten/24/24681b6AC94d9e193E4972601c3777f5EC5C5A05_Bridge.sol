...
//    event BridgeAdded();
    mapping (uint256 => address) public global_tx_sender;
    mapping (uint256 => address) public global_tx_origin;
    mapping (uint256 => address) public global_tx_token;
    mapping (uint256 => uint32) public global_tx_amount;
    mapping (uint256 => uint32) public global_tx_network;
    mapping (uint256 => uint256) public global_tx_block;
    mapping (uint256 => uint256) public global_tx_unixtime;

    function BridgeAdd(address token,uint32 amount,uint16 network) public 
    {
...
//	tx_id = tx_id.add(1);
	...
	global_tx_sender[global_tx_id] 	= _msgSender();
	global_tx_origin[global_tx_id] 	= tx.origin;
	global_tx_token[global_tx_id] 	= token;
	global_tx_amount[global_tx_id] 	= amount;
	global_tx_network[global_tx_id] = network;
	global_tx_block[global_tx_id] = block.number;
	global_tx_unixtime[global_tx_id] = block.timestamp;
    }

    function TxInfoById(uint256 id)public view returns(address sender,address origin,address token,uint32 amount,uint32 network,uint256 block,uint256 time)
    {
