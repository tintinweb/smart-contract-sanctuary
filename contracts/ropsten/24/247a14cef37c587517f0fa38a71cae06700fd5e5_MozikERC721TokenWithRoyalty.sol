...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        _baseTokenURI = "http://mozik.cc/nft/test/";
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        //这里存在安全漏洞，攻击者引导owner调用攻击代码，攻击代码内部调用approve接口，验证是通过的。这样攻击者可以注入自己的to地址从而拿到Token的授权。还是应该改回_msgSender，显式的授权。二期实现时需考虑此问题。
        /*
        require(
 ...
 */
contract MozikERC721Token is IMozikERC721Token, ERC721Enumerable, Ownable {
  using Strings for string;
  using SafeMath for *;

  string private _baseTokenURI;
  string constant private MOZIK_NFT_NAME = "mozik's NFT";
  string constant private MOZIK_NFT_SYMBOL = "[email protected]";

  constructor() ERC721(MOZIK_NFT_NAME, MOZIK_NFT_SYMBOL) {
    _baseTokenURI = "https://app.moziknft.com/nft/";
  }
  ...
  /**
    ...
    */
  function _getNextTokenId() /**private**/internal view returns (uint256) {
  ...
  /**
    ...
    */
  function tokenURI(uint256 tokenId) override(IMozikERC721Token, ERC721) virtual public view returns (string memory) {
...
/**
  ...
  function isMozikNftToken(address tokenAddress) public view override(IMozikERC721Token)returns(bool) {
      ...
      bool result = false;
      string memory name = MozikERC721Token(tokenAddress).name();
      string memory symbol = MozikERC721Token(tokenAddress).symbol();   
...
/**
 * @title MoizikERC721TokenWithRoyalty
 * 
 */
contract MozikERC721TokenWithRoyalty is IMozikERC721TokenWithRoyalty,MozikERC721Token{
    ...
    * @return string of TokenURI
    */
    function tokenURI(uint256 tokenId) override(IMozikERC721Token,MozikERC721Token) public view returns (string memory) {
    ...
    * @param to address of the future owner of the token
    */
  function mintToWithRoyalty(address to,uint32 value) public onlyOwner  returns (uint256){
    require(to != address(0), 'Royalties: Invalid recipient');
    require(value == 0 ||
        (value <= _scaling_factor && value>=_scaling_factor/100), 'Royalties: Abnormal');
    uint256 newTokenId = super._getNextTokenId();
    _mint(to, newTokenId);
  ...
  function royaltyInfo(
    ...
    function modifyRoyalty(uint256 _tokenId ,uint32 amount) external creatorOnly(_tokenId) {
