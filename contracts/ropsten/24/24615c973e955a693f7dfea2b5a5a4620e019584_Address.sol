...
*/
contract DartToken is ERC1155("D-ART Token"){
    struct NFT{
        string _name;
        string _symbol;
        uint _Id;
    ...
    mapping(uint=>NFT) public idToNFT ;
     mapping(address=>uint[]) public ownersToken  ;
        function mint(string memory name, string memory symbol,string memory uri, address account, uint256 id, uint256 amount, bytes memory data) public virtual {
            NFT memory token = NFT(name,symbol,id,uri,amount);
            idToNFT[id]=token;
            _mint( account,  id,  amount,  data);
            ownersToken[account].push(id);
