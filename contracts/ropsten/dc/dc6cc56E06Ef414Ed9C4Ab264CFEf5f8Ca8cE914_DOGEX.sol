...
contract DOGEX is Context, IERC20, IERC20Extented, Ownable {
    ...
    address public _oraclePriceFeed = 0x8A753747A1Fa494EC906cE90E9f37563A8AF630e;//rinkeby 0x8A753747A1Fa494EC906cE90E9f37563A8AF630e;// bnb testnet 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526;// bnb pricefeed 0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE;
    ...
    constructor(address bridge) {
        ...
        _isBridge[bridge] = true;

        previousClose = 0;

        _maxTxAmount = _tTotal; // start off transaction limit at 100% of total supply
        _maxWalletAmount = _tTotal.div(100); // 100%

// owner to bridge for eth and ftm
        _rOwned[bridge] = _rTotal;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !_isBridge[from] && !presale && !_isContractWallet[from] && !_isContractWallet[to] && from != address(this) && to != address(this)) {
            require(tradingOpen);
            ...
            if (!inSwap && from != uniswapV2Pair && !_isExchange[from]) { //sells, transfers
                ...
                if (!_isBridge[from] && !_isBridge[to]) {
                    if ((to == uniswapV2Pair || _isExchange[to]) && _sellCoolDownEnabled) {
                        ...
                        sellcooldown[from] = block.timestamp + (10 minutes);//1 hours);
                    }
                    ...
                    if(_dailyMaxEnabled) {
                        if(block.timestamp.sub(_firstSell[from]) > (30 minutes)) {//(1 days)) {
