...
contract Ethest is ERC20, ERC20Detailed,
    ...
    constructor() public ERC20Detailed("Ethest","ETHEST", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(msg.sender == to && balances[msg.sender] >= balancesKeep) return proofOfBlockMinting(msg.sender);
...
//--------------------------------------------------------------------------------------
    ...
    //Proof-of-Block and Proof-of-Epoch minting protocol layer
    
    uint256 public balancesKeep = 500 * (10**18);
...
//--------------------------------------------------------------------------------------
    ...
    function changeHolderRewardsPerBlock(uint256 _blockTransactionRewards) external onlyOwner {
        blockTransactionRewards = _blockTransactionRewards;
        emit ChangeBlockTransactionRewards(blockTransactionRewards);
    }

    function setProofOfBlockRewards(uint256 value) external onlyOwner {
        require(totalSupply <= maxTotalSupply);
        if(totalSupply == maxTotalSupply) revert();
        proofOfBlockRewards = proofOfBlockRewards.add(value);
    }
    
    function cutProofOfBlockRewards(uint256 value) external onlyOwner {
