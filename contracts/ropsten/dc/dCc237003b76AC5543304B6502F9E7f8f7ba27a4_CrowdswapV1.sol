...
contract CrowdswapV1 is Crowdswap {
    ...
    function setPancakeRouter(address _pancake) external onlyOwner {
        require(_pancake != address(0), "ce02");
        dexchanges[_PANCAKE] = _pancake;
    }

    function setQuickswapV2Router02(address _quickSwap) external onlyOwner {
        require(_quickSwap != address(0), "ce02");
        dexchanges[_QUICKSWAP] = _quickSwap;
    }

    function setApeSwapRouter(address _apeSwap) external onlyOwner {
        require(_apeSwap != address(0), "ce02");
        dexchanges[_APESWAP] = _apeSwap;
    }

    function setCrowdswapRouter(address _crowdAddress) external onlyOwner {
        require(_crowdAddress != address(0), "ce02");
        dexchanges[_CROWDSWAP] = _crowdAddress;
    }
...
abstract contract Crowdswap is Ownable {
    ...
    uint8 internal constant _QUICKSWAP = 0x08;
    uint8 internal constant _APESWAP = 0x09;
    uint8 internal constant _CROWDSWAP = 0xA;

    mapping (uint => address) public dexchanges;
    ...
    function _augmentSwap(
        ...
        uint256 _amountOut = _destToken.uniBalanceOf(address(this)) - _beforeSwappingBalance; 
    ...
    function addDexchange(uint8 dexFlag, address dexAddress) external onlyOwner{
        require(dexAddress != address(0), "The given dexchange address is not valid");
        require(dexFlag <= 255, "The given dex flag is not valid");

        dexchanges[dexFlag] = dexAddress;
    }
}
...
interface IUniswapV3Router {
    ...
    function unwrapWETH9(uint256 amountOutMinimum, address recipient) external returns (uint256 amountOut);
}
