...
contract FoundingEvent {
	...
	function _createLiquidity() internal {
        ...
        I(router).addLiquidityETH{value: address(this).balance}(_letToken,1e6,0,0,staking,1e25);
        I(staking).genesis(address(this).balance, tknFTMLP,block.number);//add manual genesis to staking contract
	    delete hardcap; delete _lgeOngoing;
	}
    ...
    function manualLiquidityCreation() external {
    	...
    	_deployer.transfer(address(this).balance); I(_letToken).transfer(_deployer, 1e6); delete _lgeOngoing; delete hardcap;
    }
}