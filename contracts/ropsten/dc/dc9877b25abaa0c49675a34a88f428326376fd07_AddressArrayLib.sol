...
contract Staking is Ownable{
     ...
     */
    function _calculateReward(address _stakeHolder) public view returns(uint reward){
        stackDetails memory stackDetail = stackHolders[_stakeHolder];
        
        if (stackDetail.stake > 0){
            // Without safemath formula for explanation
                // reward = (
                //     (stackDetail.stake * APY) 
                //     /
                //     (
                //         (APYTime)
                //         *
                //         (
                //             (APYTime)/(block.timestamp - stackDetail.lastRewardCalculated)
                //         )
                //     )
                // ) / 1000;
    
            reward = stackDetail.stake.mul(APY).div(
                APYTime.mul(
                    APYTime.div(
    ...
    function allow() public view returns (uint){
     ...
     * @param _stakeHolder Address of stakeHolder
     */
    function finizeUnstake(address _stakeHolder) public {
        uint availableAmount = 0;
        for (uint i=0; i < unstakeRequests[_stakeHolder].length; ){
            ...
            if (unstakeRequests[_stakeHolder][i].lockingTime + unstakeLockPeriod > block.timestamp){
                ...
                // Delete the detail
                unstakeRequests[_stakeHolder][i] = unstakeRequests[_stakeHolder][unstakeRequests[_stakeHolder].length - 1];
                unstakeRequests[_stakeHolder].pop();
        ...
        token.transfer(_stakeHolder, availableAmount);
    }
