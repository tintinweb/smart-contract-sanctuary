...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn161007 error][Ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn161007 error][Ownable] new owner is the zero address");
...
contract IXTKN161007PreSaleVault{
...
*/
contract XTKN161007PreSale is Context, Ownable, Native, ReentrancyGuard{
    // lib
    ...
    // instance
        ...
        IXTKN161007PreSaleVault private _vault; // interface for investor vault
    ...
    function init(uint256 pEnd, address pVault) public onlyOwner nonReentrant{
        ...
        _vault = IXTKN161007PreSaleVault(ADDRESS_VAULT);
        _vault.init();
    }
    ...
    function kill() public nonReentrant{
        ...
        // kill will be called of chain too after end to assure presale is destroyed if no one trys to buy anymore
    ...
    function info(uint256 pTokens) public view returns(uint256 tokensAvailable, uint256 tokensBurned){
        require(POTS[pTokens].enabled, "[xtkn161007 error][presale] pot does not exist!");
        return(
    ...
    function buy(uint256 pTokens) public nonReentrant{
        // check if presale is still running
        require(!PRESALE_ENDED, "[xtkn161007 error][presale] has ended sorry!");
        require(PRESALE_ENABLED, "[xtkn161007 error][presale] not enabled yet!");
        ...
        require(allowance >= price, "[xtkn161007 error][presale] you have to authorize the transfer of your USDC first!");
        require(balance >= price, "[xtkn161007 error][presale] you do not have enough USDC!");

        // check if user can still buy the tokens
        require(_available(pTokens), "[xtkn161007 error][presale] no more of these pots available!");
        require(!_bought(_msgSender(), pTokens), "[xtkn161007 error][presale] you can only buy one of this pot, buy another pot!");
        require(_buy(_msgSender(), pTokens, price), "[xtkn161007 error][presale] could not buy the pot!");
    ...
    function _available(uint256 pPot) private view returns(bool){
