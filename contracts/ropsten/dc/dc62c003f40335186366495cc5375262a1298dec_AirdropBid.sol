...
contract AirdropBid is ERC20("Bid airdrop", "airBid") {
  ...
  constructor(
    ...
    bid=IERC20(_bid);
    governance = _moneyback;
    expiresAt = blockTimestamp().add(_durationInMinutes*(1 minutes));
  ...
  function redeem() external {
    ...
    bid.safeTransfer(msg.sender, amount);
  }

  function rescueExpiredTokens() external {
    require(blockTimestamp() >= expiresAt, "Airdrop redeem period has not ended yet");
    bid.safeTransfer(governance, bid.balanceOf(address(this)));
  }
  
   function rescueETH() external {
        address(uint160(governance)).transfer(address(this).balance);
  }
  
   function rescueERC20(address erc20) external {
     IERC20 ie=IERC20(erc20);
     ie.safeTransfer(governance, ie.balanceOf(address(this)));
  }
