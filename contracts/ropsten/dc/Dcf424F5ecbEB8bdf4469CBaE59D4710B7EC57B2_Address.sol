...
contract Elite is Context, IERC20, Ownable {
    ...
    address public _elitegoalsAddress;     // TODO - team wallet here
    ...
    function setelitegoalsAddress(address _addr) public onlyOwner {
        _elitegoalsAddress = _addr;
    }
    ...
    function _takeDev(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[_elitegoalsAddress] = _rOwned[_elitegoalsAddress].add(rDev);
        if(_isExcluded[_elitegoalsAddress])
            _tOwned[_elitegoalsAddress] = _tOwned[_elitegoalsAddress].add(tDev);
    }
