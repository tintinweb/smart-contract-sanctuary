...
contract TokenContract is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    uint256 public feeDevelopment = 4;
    uint256 public feeMarketing = 4;
    uint256 public feeStaking = 2;
    ...
    constructor (address _walletMarketing, address _walletDevelopment, address _walletStaking) ERC20(_name, _symbol) {
    ...
    function updateWalletStaking(address _walletStaking) external onlyOwner() {
        walletStaking = _walletStaking;
    }
    ...
    function setFeesPercent(uint256 devFee, uint256 marketingFee, uint256 stakingFee) public onlyOwner {
        feeDevelopment = devFee;
        feeMarketing  = marketingFee;
        feeStaking = stakingFee;
        require(devFee + marketingFee + stakingFee <= 20, "Check fee limit");
    }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = accumulatedFee >= _tokenThresholdToSwap;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
        ...
        if(takeFee) {
            uint256 totalFeesPercent = feeDevelopment.add(feeMarketing).add(feeStaking);
        	uint256 fees = amount.mul(totalFeesPercent).div(100);
        	finalTransferAmount = amount.sub(fees);
            accumulatedFee = accumulatedFee.add(fees);
    ...
    function swapAndSendFees() private lockTheSwap {
        ...
        swapTokensForEth(accumulatedFee);
        uint256 amount = address(this).balance.sub(initialBalance);

        uint256 ethDevelopment = amount.mul(feeDevelopment).div(feeDevelopment.add(feeMarketing).add(feeStaking));
        uint256 ethMarketing = amount.mul(feeMarketing).div(feeDevelopment.add(feeMarketing).add(feeStaking));
        uint256 ethStaking = amount.sub(ethDevelopment).sub(ethMarketing);
        ...
        if(ethMarketing > 0)
            payable(walletMarketing).transfer(ethMarketing);
        if(ethStaking > 0)
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(enableRewardSystem, "Staking hasnt started yet");

        _updatePool();

        if (user.amount > 0) {
            uint256 pendingToken = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardTokenDebt);
            uint256 pendingEth = user.amount.mul(accEthPerShare).div(PRECISION_FACTOR).sub(user.rewardEthDebt);
            if (pendingToken > 0) {
        ...
        if (_amount > 0) {
            ...
            super._transfer(address(msg.sender), address(this), _amount);
        }
    ...
    function withdraw() external nonReentrant {
        ...
        require(_amount > 0, "No amount in Staking");
        _updatePool();
        ...
        super._transfer(address(this), address(msg.sender), _amount);
    }
    ...
    function pendingReward(address _user) external view returns (uint256, uint256) {
        ...
        if (block.timestamp > lastRewardTime && totalStakedAmount != 0) {
            ...
            pendingToken =  user.amount.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardTokenDebt);
            pendingEth =  user.amount.mul(adjustedEthPerShare).div(PRECISION_FACTOR).sub(user.rewardEthDebt);
        } else {
            pendingToken = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardTokenDebt);
            pendingEth = user.amount.mul(accEthPerShare).div(PRECISION_FACTOR).sub(user.rewardEthDebt);
        }
    ...
    function _updatePool() internal {
        ...
        if (enableRewardSystem == false || totalStakedAmount == 0) {
            lastRewardTime = block.timestamp;
            return;
        }
