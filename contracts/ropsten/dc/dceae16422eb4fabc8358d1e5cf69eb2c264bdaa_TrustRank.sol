...
contract TrustRank {   
    ...
    function addStory(bytes12 _storyId, bytes12 _channelId, uint _sponserTokens, uint expiryTime) public {
        require(tokenContract.transferFrom(msg.sender,address(this),_sponserTokens),"Insufficient No of Trust Rank Tokens");
    ...
    function addClaim(bytes12 _storyId, bytes12 _claimId) public {   
        require(tokenContract.transferFrom(msg.sender, address(this),CREATE_CLAIM_TOKENS),"Insufficient No of Trust Rank Tokens");
    ...
    function upVote(bytes12 _storyId, bytes12 _claimId) public {   
        require(tokenContract.transferFrom(msg.sender, address(this),UP_VOTE_TOKENS),"Insufficient No of Trust Rank Tokens");
        require (now < storiesDetail[_storyId].expiry,"Story has been expired");
    ...
    function downVote(bytes12 _storyId, bytes12 _claimId) public {   
        require(tokenContract.transferFrom(msg.sender, address(this),DOWN_VOTE_TOKENS),"Insufficient No of Trust Rank Tokens");
        require (now < storiesDetail[_storyId].expiry,"Story has been expired");
