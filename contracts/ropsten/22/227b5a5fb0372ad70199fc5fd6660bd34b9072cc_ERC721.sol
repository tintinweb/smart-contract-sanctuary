...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    // Fee
    function setCreatorFeePercent(uint256 percent) public onlyOwner() {
    ...
    function createRate(uint256 tokenId) public payable {
        ...
        if (auction.auctionType == timeAuction) {
            require(msg.value > auction.minBidPrice, "Cost should be greater than min price.");
            require(auction.endTime > block.timestamp && auction.startTime < block.timestamp, 'The auction time error');

            auctionOfToken[tokenId].minBidPrice = msg.value;

            _setRateToAuction(auction.id, _msgSender(), msg.value);
        }

        emit CreateRate(auction.id, _msgSender(), msg.value);
        ...
        if (auction.auctionType == simpleAuction) {
            ...
            emit CloseAuction(auction.tokenId, _msgSender(), msg.value, auction.id);
        }
    }
    ...
    // Token name
    string private _name = 'Realic';

    // Token symbol
    string private _symbol = 'RLC';
