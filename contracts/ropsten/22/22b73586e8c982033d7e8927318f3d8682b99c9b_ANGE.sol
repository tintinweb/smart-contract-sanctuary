...
contract ANGE is IBEP20, HasForeignAsset {
    ...
    function amountInToken(address addr, uint256 amount) public view returns(uint256[] memory) {
    ...
    function amountInETH(uint256 amount) public view returns(uint256[] memory) {
        // uint256 inToken = balanceOf(account);
    ...
    function balanceOfInToken(address token,address account) public view returns(uint256[] memory) {
        uint256 inToken = balanceOf(account);
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = pancakeRouter.WETH();
        path[2] = token;
        uint256[] memory inBUSD = pancakeRouter.getAmountsOut(inToken, path);
        return inBUSD;
    }
