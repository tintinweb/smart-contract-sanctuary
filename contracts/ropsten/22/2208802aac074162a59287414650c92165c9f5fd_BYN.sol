...
contract BYN is Variable, Event, Get, Set, Admin, manageAddress
{
  ...
  {
    require(allowedAddress[_from] || transferLock == false);
    require(!blockedAddress[_from] && !blockedAddress[_to] && !blockedAddress[msg.sender]);
    require(lockTimeAddress[_to] == false);
    require(_value <= allowed[_from][msg.sender]);

    if(lockTimeAddress[_from] == false)
    {
