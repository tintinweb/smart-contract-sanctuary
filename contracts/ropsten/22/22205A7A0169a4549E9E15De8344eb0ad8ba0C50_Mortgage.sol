...
contract Mortgage is AccessControl, Pausable,Utils{
    ...
    function mortgageNFT(uint256[] memory mortgageTokenIds,bytes32 hash, bytes memory signature,string memory nonce) public{
        ...
        mortgageNFTContract.safeBatchTransferFrom(msg.sender,address(this),mortgageTokenIds,_amounts,abi.encode(msg.sender));
        //记录Map
    ...
    /**
     * @dev  用户取回NFT
     * @param recaptureTokenIds 要取回的tokenIds
     * @param hash 交易hash
     * @param signature 账户签名
     * @param nonce 交易随机数
     */
    function recaptureNFT(uint256[] memory recaptureTokenIds,bytes32 hash, bytes memory signature,string memory nonce)public{
        //验证是否已经抵押
        require(mortgageMap[msg.sender].length != 0,"You dont't have a mortgage!");
        //验证hash
        require(hashRecaptionTransaction(recaptureTokenIds,msg.sender,nonce) == hash,"Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash,signature),"Invalid signature!");
    ...
    function _recapture(uint256[] memory recaptureTokenIds,address sender)private returns(bool){
        ...
        mortgageNFTContract.safeBatchTransferFrom(address(this),sender,recaptureTokenIds,_amounts,abi.encode(msg.sender));
        //删除Map
     ...
     * @param _tokenIds 要被取回的NFTtokenId数组
     * @param sender 交易触发者
     * @param nonce 交易随机数
     * @return 交易的hash值
     */
    function hashRecaptionTransaction(uint256[] memory _tokenIds,address sender, string memory nonce) private pure returns (bytes32) {
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(_tokenIds,sender,nonce))
