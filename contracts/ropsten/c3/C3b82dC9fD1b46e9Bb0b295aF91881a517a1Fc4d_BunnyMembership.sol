...
contract BunnyMembership is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl {
    ...
    function _configureDiscountTable( uint _mType, uint _mp0,  uint _mp1,  uint _mp2,  uint _mp3,  uint _mp4 ) internal onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   // ----------------------------------------------------------------------------------------------
    ...
    function mintMembership( uint _mType, uint _quanity, address _to, string memory _uri ) public onlyRole(MINTER_ROLE) {
        ...
        require( isAvailableForSale(_mType), "M0: not available for sale" );

        // if premint is on make sure the address is allowed to buy
        if ( _premintIsOn )
            require( isOnWhitelist(_to), "M1: not on whitelist" );

        // let's get their price for this membership
        //uint256 _mPrice = 
        // make sure they have enough ETH for the purchase
        // TODO

        // BS statement for now
        if ( _quanity > 1 )
            require( isOnWhitelist(_to), "M1: not on whitelist" );
        // if they are minting a free membership they can only get one

        // TODO lots more to finish here
