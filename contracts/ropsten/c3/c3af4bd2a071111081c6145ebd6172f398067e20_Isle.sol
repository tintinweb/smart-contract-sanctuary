...
contract Isle is Ownable, IERC721Receiver, Pausable {
  ...
  function mynft(address _address) public view returns (uint16[] memory, uint80[] memory, address[] memory) {
    uint16[] memory tokenId = new uint16[](totalNFT);
    uint80[] memory value = new uint80[](totalNFT);
    address[] memory owner = new address[](totalNFT);
    uint counter=0;
    for (uint i=0; i<totalNFT;i++) {
      Stake storage stake = isle[i];
      if (stake.owner==_address) {
        tokenId[counter]=stake.tokenId;
        value[counter]=stake.value;
        owner[counter]=stake.owner;
        counter = counter+1;
      }
      

    }
    return(tokenId,value,owner);
  }
}