...
// File: contracts/interfaces/IPmfc.sol
...
contract Orbit is ERC20, Ownable {
    ...
    // Tue Apr 06 2032 05:00:00 GMT+0000
    uint256 public constant END = 1964840400;

    bool rewardPaused = true;
    ...
    constructor(address _pmfcAddress) ERC20("Orbits", "ORBITS") {
        Pmfc = IPmfc(_pmfcAddress);
        allowedAddresses[_pmfcAddress] = true;
    }
    ...
    */
    ...
    // To grant/ revoke access to call 'updateReward' and 'claimReward' function along with burn access
    ...
    function updateReward(
        ...
        require(allowedAddresses[msg.sender], "Unauthorized caller");

        // Prevent rewards generation until our start is hit.
        if (rewardGenerationStartTime != 0) {
            if (from != address(0)) {
                rewards[from] += getPendingReward(from, reward);
                lastUpdate[from] = block.timestamp;
            }
            if (to != address(0)) {
                rewards[to] += getPendingReward(to, reward);
    ...
    function claimReward(address user, uint256 reward) external {
        require(allowedAddresses[msg.sender], "Unauthorized caller");
        require(!rewardPaused, "Reward claiming is turned off");
        ...
        _mint(user, rewards[user] + getPendingReward(user, reward));
        rewards[user] = 0;
        lastUpdate[user] = block.timestamp;

        emit RewardPaid(user, rewards[user] + getPendingReward(user, reward));
    }

    function getTotalClaimable(address user, uint256 reward)
        external
        view
        returns (uint256)
    {
        ...
        if (rewardGenerationStartTime == 0) {
            ...
            return rewards[user] + getPendingReward(user, reward);
        }
    }
    ...
    function getPendingReward(address user, uint256 reward)
        private
        view
        returns (uint256)
    {
