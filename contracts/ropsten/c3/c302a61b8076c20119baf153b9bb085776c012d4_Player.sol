...
contract Player is Character,Weapon{
    ...
    function attack(address _add)public onlyOwner{
        ...
        if(opp.characterHp() == 0){
            ...
            emit battleEvent(address(this), _add, playerLevel, playerPoints);
        }
        emit atackEvent(address(this), _add, weaponDamage, opp.characterHp());
    }
    ...
    function setHealth(uint8 _harm) external override{
        if(_harm >= characterHp){
            ...
            emit battleEvent(msg.sender, address(this), Player(msg.sender).playerLevel(), Player(msg.sender).playerPoints());
        }else{
            characterHp -= _harm;
        }
        emit atackEvent(msg.sender, address(this), Player(msg.sender).weaponDamage(), characterHp);
    }
