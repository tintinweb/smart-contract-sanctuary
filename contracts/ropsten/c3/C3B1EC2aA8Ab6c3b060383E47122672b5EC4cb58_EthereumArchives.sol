...
contract EthereumArchives is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function mint(uint256 blockNumber, string memory name) public payable nonReentrant {
        require(msg.value >= MINTING_FEE, "Not enough ether sent!");
    ...
    function tokenURI(uint256 blockNumber) override public view returns (string memory) {
        ...
        string memory blockName = getName(blockNumber);
    ...
    function _validateName(string memory name) pure private {
        require(bytes(name).length > 0, "Name cannot be empty.");
        require(bytes(name).length < 46, "Name cannot extend 45 characters");
    }
}
