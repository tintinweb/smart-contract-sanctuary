...
contract AnimalHouse is ERC1155, AbstractAnimalHouseEntity {
  ...
  */
  ...
  function createContent(string memory ipfsAddress, NftDataType dataType, uint256 amount, uint256 price, string memory title, string memory desc) external {
  ...
  function getContent(uint256 id) external view validContentId(id) returns (address, NftDataType, string memory, uint, uint, string memory, string memory, uint256) {
    NftContent memory content = _idToNftContent[id];
    address owner = content.owner;
    return (owner, content.dataType, content.ipfsAddress, content.price, content.amount, content.title, content.desc, content.timestamp);
  }
  ...
  function getMyNFTContents(address payable userWallet) public view returns (NftContent[] memory) {
      
      uint256 nSize = 0;
      for(uint i = 0; i < _contentId; i++) {
          if (balanceOf(userWallet, i+1) > 0) {
              nSize++;
          }
      }
      ...
      uint256 nIndex = 0;
      uint nLen = _contentId;
      NftContent[] memory contents = new NftContent[](nSize);
      
      for(uint i = 0; i < _contentId; i++) {
          uint amt = balanceOf(userWallet, i+1);
          if (amt > 0) {
              NftContent storage content = _idToNftContent[i+1];
              contents[nIndex] = content;
              contents[nIndex].amount  = amt;
              nIndex++;
          }
      }
      return contents;
  ...
  function buyNFT(uint256 contentId, address to, uint256 amount) external {
    ...
    _idToNftContent[contentId].amount = _idToNftContent[contentId].amount - amount;
