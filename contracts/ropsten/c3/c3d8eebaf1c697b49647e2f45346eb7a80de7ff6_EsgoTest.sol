        ...
        if(fundTXs[txId].funded==esgoTXs[txId].txVal) {
          ...
          //  uint useraward = esgoTXs[txId].award-awardallocate;
            ...
            for(uint iu = 0;iu<ul;iu++){
                ...
                for(uint iu1 = 0;iu1<users.length;iu1++){
                address fuser2 = users[iu1];
              
                for(uint iutx1 = 0;iutx1<userTXs[fuser2].length;iutx1++){
                    if(userTXs[fuser][iutx].participant == 2 && userTXs[fuser2][iutx].txId == txId){
                        if(disputeTxs[txId].disputed==false){
                            balances[fuser2] = balances[fuser2] + userTXs[fuser2][iutx].txVal + ((userTXs[fuser2][iutx].txVal*awardallocate)/(1000000000000000000));
                        }else{
                            balances[fuser2] = balances[fuser2] + esgoTXs[txId].txVal;
                        }
                        //fuser.send(userTXs[fuser][iu].txVal);
                        break;
                    }
