...
//------------------------------------------------------------//
    ...
    function harvestICO(address payable _to) onlyOwner public {
...
//------------------------------------------------------------//  
    ...
    function pendingDebt(address _user) public view returns (uint256) {
        ...
        if (userTier == maxTier || _balances[_user] == 0 || isExcluded(_user)) {
            ...
            uint256 pending = _balances[_user].mul(tierInfo[userTier].dailyHoldingFee).div(100).mul(duration);

            if (pending.div(86400) > _balances[_user]) return _balances[_user];
            else return pending.div(86400);
        }
    }
...
//------------------------------------------------------------//  
    ...
    function _transferStandard(address sender, address recipient, uint256 debtAmount) private {
        uint256[4] memory pending = [pendingReward(msg.sender), pendingDebt(msg.sender), pendingReward(recipient), pendingDebt(recipient)];
