...
contract charity{
   ...
   // Событие вызываемое в момент пополнения счета.
    ...
    function donation() public payable notEnoughSum(balanceView()) onlyBefore(date) {
...
//Если сумма не= заявленной И дата просрочена, то деньги переводятся на адрес благотворительной организации
  function send() public  onlyOwner(msg.sender) enoughSum(amount){
      getter.transfer(amount);
      } 
 function sendIfFail() public  onlyAfter(date) onlyOwner(msg.sender) notEnoughSum(amount){
