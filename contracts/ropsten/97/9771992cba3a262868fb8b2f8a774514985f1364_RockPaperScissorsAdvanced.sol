...
contract RockPaperScissorsAdvanced is SafeMath ,FeeControl, CustomArray {
    ...
    event JoinGameRandomEv(bool isSuccess, address player2, int gameid);
    ...
    function joinGameRandom(uint8 choice) public payable  returns (int){
        if (getArrayLength()==0) {
            ...
            emit JoinGameRandomEv(false,msg.sender,-6);
            return -6;
        }
        uint gameRandomIndexID = random( getArrayLength()-1 );
        ...
        if (game.player != address(0) || game.closed !=false 
            ...
            emit JoinGameRandomEv(false,msg.sender,-5);
            return -5;
        }
        else {
            emit JoinGameRandomEv(true,msg.sender, int(gameRandomID) );
    ...
    //// generates a number in [0 .. size_] range
