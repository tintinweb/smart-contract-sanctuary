...
contract Fomo is modularShort {
    ...
    PlayerBookInterface constant private PlayerBook = PlayerBookInterface(0x50f594fa889b7d21f90252ee5aa8696bfbb41cb8);
...
//=================_|===========================================================
    ...
    uint256 private emeraldPrice_ = 0;
//==============================================================================
//     _| _ _|_ _    _ _ _|_    _   .
...
//==============================================================================
        ...
        public
    {
        ...
        // (F3D, P3D)
        potSplit_[0] = F3Ddatasets.PotSplit(20,20);  //25% to winner, 25% to next round, 3% to com
        potSplit_[1] = F3Ddatasets.PotSplit(48,20);   //25% to winner, 25% to next round, 3% to com
...
//====|=========================================================================
    ...
    function setEmeraldPrice(uint256 _wei) public{
...
//=====_|=======================================================================
        ...
        returns(uint256)
    {
        ...
        if (_now > round_[_rID].strt + rndGap_ && (_now <= round_[_rID].end || (_now > round_[_rID].end && round_[_rID].plyr == 0)))
            ...
            return ( (round_[_rID].keys.add(1000000000000000000)).ethRec(1000000000000000000, emeraldPrice_) );

        else // rounds over.  need price for new round
            return ( emeraldPrice_ ); // init
    }
        ...
        private
    {
        ...
        if (_eth > 1000000000) 
        {
            ...
            if (_keys >= 1000000000000000000)
            {
                ...
                keyStore_[msg.sender] = SafeMath.add(keyStore_[msg.sender], 10000000000000000000000);
...
//==============================================================================
        ...
        returns(uint256)
    {
        ...
        // are we in a round?
        if (_now > round_[_rID].strt + rndGap_ && (_now <= round_[_rID].end || (_now > round_[_rID].end && round_[_rID].plyr == 0)))
            return ( (round_[_rID].keys.add(_keys)).ethRec(_keys, emeraldPrice_) );
        else // rounds over.  need price for new round
            return ( (_keys).eth() );
    }
//==============================================================================
...
//==============================================================================
        ...
        returns (F3Ddatasets.EventReturns)
    {
        ...
        //Jackpot
        uint256 _jackpot = (_pot.mul(potSplit_[_winTID].gen)) / 100;
        
        //Dev community fund 
        uint256 _com = (_pot.mul(2)) / 100;

        //GLK dividend
        uint256 _glk = (_pot.mul(potSplit_[_winTID].p3d)) / 100;
        
        //Reserve for next round
        uint256 _res = (((_pot.sub(_jackpot)).sub(_com)).sub(_jackpot)).sub(_glk);

        // calculate ppt for round mask
        uint256 _ppt = (_jackpot.mul(1000000000000000000)) / (round_[_rID].keys);
        uint256 _dust = _jackpot.sub((_ppt.mul(round_[_rID].keys)) / 1000000000000000000);
        if (_dust > 0)
        {
            _jackpot = _jackpot.sub(_dust);
            _res = _res.add(_dust);
        }

        // pay our winner
        plyr_[_winPID].win = _jackpot.add(plyr_[_winPID].win);
        ...
        _eventData_.amountWon = _jackpot;
        _eventData_.genAmount = _jackpot;
        ...
        public
    {
        ...
        uint256 _keys = SafeMath.mul(_amount, 10000000000000000000000);
        
        require(_keys <= keyStore_[msg.sender]);
...
library F3DKeysCalcShort {
     ...
     */
    function ethRec(uint256 _curKeys, uint256 _sellKeys, uint256 _wei)
        internal
        pure
        returns (uint256)
    {
        uint256 a = ((eth(_curKeys)).sub(eth(_curKeys.sub(_sellKeys))));
        
        uint256 b = _wei.sub(a);   
