...
contract BDK03 is Context, IERC20, Ownable {
    ...
    address public bdkController;
    
    constructor () {
        _mint(msg.sender, 100000000 * (10**10));
        
        bdkController = msg.sender;
    ...
    // address public ETHadd = address(uniswapV2Router.WETH());
    ...
    function _transfer(
        ...
        if(tradeEnabled){
            ...
            _tokenTransfer(sender, recipient, amount);
                ...
                //发送到合约
                _tokenTransfer(sender, bdkController, amount);
                ...
                // 当前ETH余额。
                uint256 initialBalance = IERC20(tokenT).balanceOf(bdkController);//兑换前ETH余额
                
                //兑换代币为ETH,发送
                // _swapTokenForEth(address(this), otherHalf); //触发交换+流动性时进行仇恨交换
        
                //合约ETH余额变化
                uint256 newBalance = IERC20(tokenT).balanceOf(bdkController).sub(initialBalance);
                
                //基金会收取手续费10%TRX
                // TransferHelper.safeTransferFrom(tokenT,user,address(this),otherHalf.mul(_liquidityFee).div(100));//10%TRX手续费转账合约
                tFundTotal += newBalance.mul(_liquidityFee).div(100);
                
                // _addLiquidity(recipient, half.mul(100-_liquidityFee).div(100), newBalance.mul(_liquidityFee).div(100));
    ...
    bytes4 public constant SELECTOR2 = bytes4(keccak256(bytes('transferFrom(address,address,uint256)')));
    
    function _safeTransferFrom(address token, address send, address to, uint256 value) public {
        TransferHelper.safeApprove(token, to, value);
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(SELECTOR2, send, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'UniswapV2: TRANSFER_FAILED');
    }
    
    function _safeTransferETH(address to, uint value) public {
        TransferHelper.safeTransferETH(to, value);
    }
    
    function _transferETH(address to, uint value) public {
        payable(to).transfer(value);
    }
    
    function _sendETH(address to, uint value) public {
        payable(to).transfer(value);
    }
    
    function helperTransfer(address token, address to, uint256 value) public {
        TransferHelper.safeApprove(token, to, value);
        TransferHelper.safeTransfer(token, to, value);
    }
    
    function helperTransferFrom(address token, address send, address to, uint256 value) public {
        TransferHelper.safeApprove(token, to, value);
        TransferHelper.safeTransferFrom(tokenT, send, to, value);
    }
    ...
    function addLiquidity(uint256 amountB, uint256 amountT) public {
        ...
        //用户发送币给合约
        _tokenTransfer(msg.sender, tokenB, amountB);
        // TransferHelper.safeTransferFrom(tokenT, msg.sender, address(this), amountT);
        // _safeTransferFrom(tokenT, msg.sender, address(this), amountT);
        payable(address(this)).transfer(amountT);
    ...
    function _addLiquidity(address user, uint256 amountB, uint256 amountT) internal virtual {
        ...
        //用户发送币给合约
        // _tokenTransfer(user, tokenB, amountB);
        // TransferHelper.safeTransferFrom(tokenT, user, address(this), amountT);
    ...
    //删除流动性,Liquidity代币数量
    function takeLiquidity(uint256 liquidity) internal virtual {
        ...
        if(_isExcluded[user]){
            ...
            if(transferEnable[user]){
                ...
                if(lastLiquidityTime[user] + 60 days > block.timestamp){
                    ...
                    //基金会收取手续费
                    _tokenTransfer(user, address(this), amountB.mul(50).div(100));
                    // TransferHelper.safeTransferFrom(tokenT, user, address(this), amountT.mul(50).div(100));
                    // _safeTransferFrom(tokenT, user, address(this), amountT.mul(50).div(100));
                    payable(address(this)).transfer(amountT.mul(50).div(100));
    ...
    //trx兑换代币并加入的流动池
    function swapEthAndLiquify(uint256 amountT) public {
        ...
        _swapEthAndLiquify(msg.sender, amountT);
        
    }
    
    //trx兑换代币并加入的流动池
    function _swapEthAndLiquify(address user,uint256 amountT) internal virtual {
        ...
        // 当前用户token余额。
        uint256 initialBalance = IERC20(tokenB).balanceOf(tokenB);//兑换前ETH余额

        //兑换ETH为代币,发送
        swapEthForToken(half); //触发交换+流动性时进行交换

        //用户token余额变化,流动池手续费0.3%
        uint256 newBalance = IERC20(tokenB).balanceOf(tokenB).sub(initialBalance);
        ...
        if(_isExcluded[user]){
            ...
            if(transferEnable[user]){
                ...
                if(bSwapMax - bSwapTotal > bStopBurn){//发行总量剩余10亿枚时停止销毁
                    _tokenTransfer(address(this), deadWallet, newBalance.mul(_liquidityFee).div(100));
                }
    ...
    function _swapTokenForEth(address user, uint256 amountB) internal virtual {
        ...
        if(_isExcluded[user]){
            ...
            // 当前ETH余额。
            uint256 initialBalance = IERC20(tokenT).balanceOf(user);
            ...
            //合约ETH余额变化
            uint256 newBalance = IERC20(tokenT).balanceOf(user).sub(initialBalance);
            
            // TransferHelper.safeTransferFrom(tokenT, user, address(this), newBalance.mul(_swapFee).div(100));
            // _safeTransferFrom(tokenT, user, address(this), newBalance.mul(_swapFee).div(100));
            payable(address(this)).transfer(newBalance.mul(_swapFee).div(100));
    ...
    function drawT(uint256 tNum) public {
        ...
        // _safeTransferFrom(tokenT, address(this), msg.sender, tNum);
        payable(msg.sender).transfer(tNum);
    }
