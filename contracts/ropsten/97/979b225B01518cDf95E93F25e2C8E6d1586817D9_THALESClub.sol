...
/*
    ...
    THALES token 0xB42e1c3902b85b410334f5fff79cDc51fBeE6950
...
contract THALESClub is Ownable {
 ...
 string public name = "THALES Staked";
 string public symbol = "sTHALES";
 uint8 constant public decimals = 18;
 uint256[] internal entryFee_ = [ 1000, 750, 500, 250, 125 ];
 uint256 internal transferFee_ = 0;
 uint256[] internal exitFee_ = [ 1000, 750, 500, 250, 125 ];
 uint256[] internal steps = [ 100, 1000, 5000, 10000 ];
 uint256 internal referralFee_ = 0;
 ...
 function buyFor(uint256 _amount, address _customerAddress, address _referredBy) public returns(uint256) {
  checkAndTransferSTAT(_amount);
  stakedAmount[_customerAddress] = stakedAmount[_customerAddress].add(_amount);
 ...
 function withdraw() onlyDivis public {
  ...
  if(totalHolder > 0) {
    bep20.transfer(_customerAddress, _dividends);
      ...
      bep20.transfer(_customerAddress, bep20.balanceOf(address(this)));
  }
  stakedAmount[msg.sender] = stakedAmount[msg.sender].sub(_dividends);
 ...
 function sell(uint256 _amountOfTokens) onlyTokenHolders public {
  ...
  uint256 exitFee_calc = calcExitFee(msg.sender);

  uint256 _dividends = SafeMath.div(SafeMath.mul(_amountOfTokens, exitFee_calc), 100000);
 ...
 function sellPrice() public view returns(uint256) {
  uint256 _stat = 1e18;
  uint256 exitFee_calc = calcExitFee(msg.sender);
  uint256 _dividends = SafeMath.div(SafeMath.mul(_stat, exitFee_calc), 100000);
 ...
 function buyPrice() public view returns(uint256) {
  uint256 _stat = 1e18;
  uint256 entryFee_calc = calcEntryFee(msg.sender);
  uint256 _dividends = SafeMath.div(SafeMath.mul(_stat, entryFee_calc), 100000);
 ...
 function calculateTokensReceived(uint256 _powerToSpend) public view returns(uint256) {
  uint256 entryFee_calc = calcEntryFee(msg.sender);
  uint256 _dividends = SafeMath.div(SafeMath.mul(_powerToSpend, entryFee_calc), 100000);
 ...
 function calculatePowerReceived(uint256 _tokensToSell) public view returns(uint256) {
  ...
  uint256 exitFee_calc = calcExitFee(msg.sender);
  uint256 _dividends = SafeMath.div(SafeMath.mul(_tokensToSell, exitFee_calc), 100000);
 ...
 function purchaseTokens(address _referredBy, address _customerAddress, uint256 _incomingSTAT) internal returns(uint256) {
  ...
  uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingSTAT, entryFee_calc), 100000);
 ...
 function calcExitFee(address account) public view returns(uint256 fee) {
     fee = exitFee_[0];
     for(uint8 i = 0; i < steps.length; i++)
     {
         if(stakedAmount[account] >= steps[i].mul(1e18))
         {
             fee = exitFee_[i+1];
         }
     }
 ...
 function calcEntryFee(address account) public view returns(uint256 fee){
     fee = entryFee_[0];
     for(uint8 i = 0; i < steps.length; i++)
     {
         if(stakedAmount[account] >= steps[i].mul(1e18))
