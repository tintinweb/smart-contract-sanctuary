...
contract Bridge {
    ...
    function processPack(/*pack header*/uint8 networkId,uint64[2] memory blockScanRange,  /*pack body*/uint256[] memory txHashes, address[] memory tokens, address[] memory recipients, uint256[] memory amounts, uint8[11] memory v, bytes32[11] memory r, bytes32[11] memory s) public returns(bytes32) {
   ...
   function encodeAddressArray(address[] memory addresses) internal pure returns(bytes memory data){
    ...
    function processSigners(uint64 epochBlock, address[21] memory signers, uint8[11] memory v, bytes32[11] memory r, bytes32[11] memory s) public returns(bytes32 /*bytes32*/) {
        //follow eip191 standard https://eips.ethereum.org/EIPS/eip-191
        bytes32 msgHash = keccak256(abi.encodePacked(bytes1(0x19), bytes1(0), address(0),epochBlock,encodeSignersArray(signers)));
