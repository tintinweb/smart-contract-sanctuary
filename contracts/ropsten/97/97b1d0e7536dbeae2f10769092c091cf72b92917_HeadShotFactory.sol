...
contract HeadShotFactory is Context, Ownable {
    ...
    function testBuy1() public {
        ...
        uint256 count_ = 1;
        uint256 _totalPay = _price.mul(count_).mul(10 ** _tokenDecimals);
        headShotToken.allowance(msg.sender, address(this));
        bool approved = headShotToken.approve(address(this), _totalPay);
        require(approved, "Not approved");
        headShotToken.transfer(address(this), _totalPay);
        //payable(msg.sender).transfer(_totalPay);
        emit Deposit(_totalPay);
    }

    //running
    function testBuy2() public {
        uint256 _price = 10000;
        uint256 count_ = 1;
        uint256 _subTotal = _price.mul(count_).mul(10 ** _tokenDecimals);
        uint256 _totalTax = _subTotal.mul(10).div(100);
        uint256 _totalPay = _subTotal.add(_totalTax);
        //headShotToken.allowance(msg.sender, address(this));
        headShotToken.allowance(msg.sender, address(this));
        headShotToken.transfer(_salesWalletAddress, _totalPay);
        //payable(msg.sender).transfer(_totalPay);
        emit Deposit(_totalPay);
    }
    ...
    function testBuy3() public {
        ...
        uint256 _totalPay = _subTotal.add(_totalTax);
        //headShotToken.allowance(msg.sender, address(this));
        headShotToken.approve(address(this), _totalPay);
        headShotToken.transfer(_salesWalletAddress, _totalPay);
        //payable(msg.sender).transfer(_totalPay);
        emit Deposit(_totalPay);
    }
