...
contract TheTreasureTest {
     ...
     */
    modifier onlyOwner {
        require(msg.sender == owner, "TheTreasureTest: Only the owner can call this function.");
        _;
    }
    ...
    modifier onlyOwnerOrPending {
        require(msg.sender == owner || msg.sender == pendingOwner,
			"TheTreasureTest: Only the owner or pending owner can call this function."
		);
        _;
    }
    ...
    constructor() {
		...
		_name = "TheTreasure.io X";
        _symbol = "TRSR X";
        _decimals = 18;
        baseNumber = 20000000000000000; // 0.02 token
		blocktime = 20; // ~ Every minute

		// @dev Exclude the owner, treasury addresses from winning and paying fees.
    ...
    function _transfer(address sender_, address recipient_, uint256 amount_) internal {
        require(sender_ != address(0), "TheTreasureTest: Transfer from the zero address.");
	...
	function _approve(address owner_, address spender_, uint256 amount_) internal {
        require(owner_ != address(0), "TheTreasureTest: Approve from the zero address.");
        require(spender_ != address(0), "TheTreasureTest: Approve to the zero address.");
	...
	function changeBaseNumber(uint newBaseNumber_) external onlyOwner {
		...
		require(baseNumber != newBaseNumber_, "TheTreasureTest: Base number is already as requested.");
	...
	function changeBlocktime(uint newBlocktime_) external onlyOwner {
		...
		require(blocktime != newBlocktime_, "TheTreasureTest: Base number is already as requested.");
	 ...
	 */
	function exclude(address excluded_) external onlyOwner {
		isExcluded[excluded_] = true;
		emit Exclude(excluded_);
	}
    ...
    function withdrawTokens(address token_, uint amount_) external onlyOwner {
		if(token_ == address(0)) {
			...
			require(success_, "TheTreasureTest: Transfer failed.");
		}
		else {
    ...
    function changeOwner(address newOwner) external onlyOwner {
        require(newOwner != address(0), "TheTreasureTest: Owner can not be the zero address.");
    ...
    function transfer(address recipient_, uint256 amount_) public returns (bool) {
        require(amount_ > 0, "TheTreasureTest: Amount should be more than zero.");
