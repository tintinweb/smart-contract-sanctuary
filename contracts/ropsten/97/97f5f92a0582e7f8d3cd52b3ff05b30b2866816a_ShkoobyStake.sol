...
contract ShkoobyStake is Ownable {
    ...
    constructor() {
        address baseTokenAddress = 0x0d30991dF49088C5be4F3d9f3755E816CF220109; // Your ERC20 token address (Shkooby Inu Token)
        addStakePool(
            baseTokenAddress,
    ...
    function getUnclaimedRewards(uint256 _stakeId) public view returns (uint256){
        ...
        unclaimedRewards = (userStakeDetails.stakeBalance * 1000) / 10000;
    ...
    function getUserFlexyPoolsDetails (address _tokenAddress) external view returns (uint256[] memory) {
        uint256[] memory stakePoolIds = getStakePoolIds(_tokenAddress);
        uint256[] memory userStakeDetails;
        uint256 userTotalFlexyPoolsStakedBalance;
        uint256 userTotalFlexyPoolsClaimedLockedRewards;
        uint256 userTotalFlexyPoolsClaimedUnlockedRewards;

        (userStakeDetails,) = getUserStakeDetails(stakePoolIds[0]);
        userTotalFlexyPoolsStakedBalance = userStakeDetails[2] + userTotalFlexyPoolsStakedBalance;
        userTotalFlexyPoolsClaimedLockedRewards = userStakeDetails[3] + userTotalFlexyPoolsClaimedLockedRewards;
        userTotalFlexyPoolsClaimedUnlockedRewards = userStakeDetails[4] + userTotalFlexyPoolsClaimedUnlockedRewards;
        
        uint [] memory userFlexyPoolsDetailsArray = new uint[](5);
        
        userFlexyPoolsDetailsArray[0] = userTotalFlexyPoolsStakedBalance;
        userFlexyPoolsDetailsArray[1] = userTotalFlexyPoolsClaimedLockedRewards;
    	userFlexyPoolsDetailsArray[2] = userTotalFlexyPoolsClaimedUnlockedRewards;
    	
        return userFlexyPoolsDetailsArray;
    }

    function getUserLockedPoolsDetails (address _tokenAddress) external view returns (uint256[] memory) {
        uint256[] memory stakePoolIds = getStakePoolIds(_tokenAddress);
        uint256[] memory userStakeIds = getStakePoolIds(_tokenAddress);
        uint256[] memory userStakeDetails;
        uint256 userTotalLockedPoolsStakedBalance;
        uint256 userTotalLockedPoolsClaimedLockedRewards;
        uint256 userTotalLockedPoolsClaimedUnlockedRewards;

        for(uint i = 1; i < stakePoolIds.length; i++){
            userStakeIds = getUserStakeIds();
            for(uint j = 0; j < stakePoolIds.length; j++){
                (userStakeDetails,) = getUserStakeDetails(stakePoolIds[j]);
                userTotalLockedPoolsStakedBalance = userStakeDetails[2] + userTotalLockedPoolsStakedBalance;
                userTotalLockedPoolsClaimedLockedRewards = userStakeDetails[3] + userTotalLockedPoolsClaimedLockedRewards;
                userTotalLockedPoolsClaimedUnlockedRewards = userStakeDetails[4] + userTotalLockedPoolsClaimedUnlockedRewards;
            }
        }
        
        uint [] memory userLockedPoolsDetailsArray = new uint[](5);
        
        userLockedPoolsDetailsArray[0] = userTotalLockedPoolsStakedBalance;
        userLockedPoolsDetailsArray[1] = userTotalLockedPoolsClaimedLockedRewards;
    	userLockedPoolsDetailsArray[2] = userTotalLockedPoolsClaimedUnlockedRewards;
    	
        return userLockedPoolsDetailsArray;
    }
    ...
    function getUserLockedRewardDetails(uint256 _lockedRewardId) public view returns(uint256[] memory, uint256[] memory){
        ...
        uint256 [] memory lockedRewardDetailsArray = new uint[](8);
        ...
        lockedRewardDetailsArray[5] = lockedRewardDetails.createdTime;
