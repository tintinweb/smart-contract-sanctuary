...
// File: contracts/NFTittiesERC721.sol
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    //Allow bidding with TITS
    bool public allowTokenBids;
    //Enable createAuction function
    bool public enableCreateAuction;
    ...
    function setTokenBids(bool value) external onlyOwner {
        allowTokenBids = value;
    }

    function setCreateAuction(bool value) external onlyOwner {
        enableCreateAuction = value;
    }

    function approveAuction(uint256 tokenId, uint256 serverBidPrice, address creatorAdd, address tokenAddress) external onlyAdmin {
    ...
    function simpleApproveAuction(uint256 tokenId, uint256 serverBidPrice) external onlyAdmin {
    ...
    function emergencyCloseAuction(uint256 tokenId) external onlyOwner {
        Auction memory auction = auctionOfToken[tokenId];
    ...
    function createTokenAndAuction(uint256 minBidPrice, address tokenAddress) public {
    ...
    function createAuction(uint256 tokenId, uint256 minBidPrice, address creatorAddress, address tokenAddress) public payable {
        require(enableCreateAuction, "createAuction not enabled");
        require(auctionOfToken[tokenId].id == 0, "Auction already exists");
        require(minBidPrice >= minBid, "Your bid is too low");
        ...
        if (_owners[tokenId] == address(0)) {
            ...
            if (isEthTransaction)
                require(msg.value >= minBid, "Not enough ETH for minimum bid");

            if (!isEthTransaction) {
                require(allowTokenBids, "Token bids not allowed yet");
                require(
    ...
    function startAuction(uint256 tokenId) external payable {
        ...
        require(msg.value >= minBid, "ETH does not meet minimum bid");

        _setAuctionToMap(tokenId, msg.value, address(0));

        auctionOfToken[tokenId].creatorAddress = systemAddress;

        _setRateToAuction(auctionOfToken[tokenId].id, _msgSender(), msg.value, tokenId);

        emit CreateAuction(tokenId, _msgSender(), msg.value, auctionOfToken[tokenId].id);
    }

    function startAuctionTest(uint256 tokenId) external payable {
        require(auctionOfToken[tokenId].id == 0, "Auction already exists");
        require(!isPreMinting, "Pre-minting is active");
        require(_owners[tokenId] == address(0), "Token already owned");
        require(msg.value >= minBid, "ETH does not meet minimum bid");
    ...
    function placeBid(uint256 tokenId, uint256 tokensAmount) external payable {
        ...
        if (isEthTransaction)
            tokensAmount = msg.value;

        require(tokensAmount >= (maxRate.cost > auction.minBidPrice ? maxRate.cost : auction.minBidPrice).mul(rateStep + 100).div(100),
            "Your bid should be greater than previous bid"
        );
        ...
        emit PlaceBid(auction.id, _msgSender(), tokensAmount);
    }
    ...
    function claimToken(uint256 tokenId) external {
        ...
        require(auction.canClaim, "Auction has not been approved yet");
    ...
    function _setAuctionToMap(uint256 _tokenId, uint256 _minBidPrice, address tokenAddress) private {
