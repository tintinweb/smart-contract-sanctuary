...
** Using FTPEthReflect to give 0.50% of ALL transactions to holders. **
...
contract test3lfg is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000000000000 * 10**9;
    string private m_Name = "test3lfg";
    string private m_Symbol = "test3lfg";
    uint8 private m_Decimals = 9;
    // EXCHANGES
    address private m_UniswapV2Pair;
    IUniswapV2Router02 private m_UniswapV2Router;
    // TRANSACTIONS
    uint256 private m_WalletLimit = TOTAL_SUPPLY.div(909);
    ...
    // LP ADD
    IWETH private WETH;
    uint256 private m_LiqAlloc = 6900;
    // MISC
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != m_UniswapV2Pair && _recipient != address(m_UniswapV2Router) && block.timestamp <= m_Launched.add(23 hours);
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(12 minutes))
            return TOTAL_SUPPLY.div(909);
        else
            return TOTAL_SUPPLY;
    }
    ...
    function addLiquidity() external onlyOwner() {
        ...
        FTPLiqLock(m_LiqLockSvcAddress).lockTokens(m_UniswapV2Pair, block.timestamp.add(2 days), msg.sender);
		EthReflect.init(address(this), 500, m_UniswapV2Pair, m_UniswapV2Router.WETH(), _ethBalance, TOTAL_SUPPLY);
