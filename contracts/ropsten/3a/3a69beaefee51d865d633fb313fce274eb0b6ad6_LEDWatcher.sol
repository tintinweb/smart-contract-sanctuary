...
contract LEDWatcher is Initializable {
    ...
    function getState(uint256 _deviceId) public view returns (bool) {
        return stateOfDeviceId[bytes32(_deviceId)];
    }

    function getLastIndex(uint256 _deviceId) public view returns (bytes4) {
        return lastIndexOfDeviceId[bytes32(_deviceId)];
    }

    function getDepositBalance(address _address) public view returns (uint256) {
        return depositBalances[_address];
    }

    function getRequest(address _address) public view returns (bytes32, bool, bytes4) {
        return (requests[_address].deviceId, requests[_address].state, requests[_address].indexOfdevice);
    }

    function getIsRequesting(address _address) public view returns (bool) {
        return isRequesting[_address];
    }
    ...
    function issueRequest(uint256 _deviceId, bool _state) public payable {
        bytes32 deviceId = bytes32(_deviceId);
        require(radoa.isHealthDevice(deviceId),"code 0 in recordState");
        require(msg.value > 0,"code 1 in recordState");
        require(!isRequesting[msg.sender],"code 2 in recordState");
        bytes4 index = radoa.getLastIndex(deviceId);
        ...
        emit NewRequest(msg.sender, deviceId, _state);
    }
    ...
    function completeRequest(address _requester) public onlyOwner {
        ...
        uint256 depositBalance = depositBalances[_requester];
        depositBalances[_requester] = 0;
        isRequesting[_requester] = false;
        owner.transfer(depositBalance);
