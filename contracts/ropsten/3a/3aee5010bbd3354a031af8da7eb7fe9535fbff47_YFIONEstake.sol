...
contract YFIONEstake is Context {
    ...
    function claimPoolToDev() public virtual onlyOwner{
        uint256 yfobal = IERC20(_YFIONEtoken).balanceOf(address(this));
        uint256 yfopoolbal = yfobal.sub(devBalance[_YFIONEtoken]);
        IERC20(_YFIONEtoken).safeTransfer(_owner, yfopoolbal);
    }
