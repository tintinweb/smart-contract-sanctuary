...
contract TinVilleToken is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    string private _name = "TinVille-Token";
    string private _symbol = "TINV";
    ...
    uint256 private _tTotal = 250 * 10**3 * 10**18;
...
function _transfer(
        ...
        // also, don't swap & Reward if sender is the pancake pair.
        ...
        if (to != uniswapV2Pair) tokensToLock = 0;
        bool overMinTokenBalance = contractTokenBalance >= minTokensBeforeAddToLP;
        bool interactWithpancake = to == uniswapV2Pair; 
        if (
            overMinTokenBalance &&
            !inSwapAndAbsorb &&
            interactWithpancake &&
    ...
    if (tokensToLock > 0 && interactWithpancake){
    ...
    */
    ...
    function updateFeesAndSwapsEnabled(uint8 FeeDecimals, uint32 FeePercentage, uint256 FeeDivider, bool _enabled)
        public
        onlyOwner
        nonReentrant
    {
        require(FeeDivider >= 5, "The fee cannot be more than 5%");
        require(FeeDivider <= 1000, "The fee cannot be less than 0.1%");
        require(FeePercentage <= 15, "The fee cannot be more than 15%");
        feeDivider = FeeDivider;
        feeDecimals = FeeDecimals;
        feePercentage = FeePercentage;
        emit FeeUpdated(FeeDecimals, FeePercentage);
    ...
    */
    ...
    function burnLiq(address _token, address _to, uint256 _amount) public onlyOwner nonReentrant {
        ...
        IUniswapV2Pair token = IUniswapV2Pair(_token);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 aAmount, uint256 aTransferAmount, uint256 aFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = _getValues(tAmount);
        uint256 aBurn =  tBurn.mul(currentRate);
        _aOwned[sender] = _aOwned[sender].sub(aAmount);
        _aOwned[recipient] = _aOwned[recipient].add(aTransferAmount);       
        _allocationFee(aFee, aBurn, tFee, tBurn);
    ...
    function _allocationFee(uint256 aFee, uint256 aBurn, uint256 tFee, uint256 tBurn) private {
        _aTotal = _aTotal.sub(aFee).sub(aBurn);
        _tFeeTotal = _tFeeTotal.add(tFee);
        _tBurnTotal = _tBurnTotal.add(tBurn);
        _tTotal = _tTotal.sub(tBurn);
    }
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = __getTValues(tAmount, _burnFee);
        uint256 currentRate =  _getRate();
        (uint256 aAmount, uint256 aTransferAmount, uint256 aFee) = _getAValues(tAmount, tFee, tBurn, currentRate);
        return (aAmount, aTransferAmount, aFee, tTransferAmount, tFee, tBurn);
    }
    ...
    function __getTValues(uint256 tAmount, uint256 burnFee) private view returns (uint256, uint256, uint256) {
    ...
    function _getAValues(uint256 tAmount, uint256 tFee,uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 aAmount = tAmount.mul(currentRate);
        uint256 aFee = tFee.mul(currentRate);
        uint256 aBurn = tBurn.mul(currentRate);
        uint256 aTransferAmount = aAmount.sub(aFee).sub(aBurn);
