...
contract Rise is Ownable, Rebasable
{
	...
	uint256 public _autoSwapCallerFee = 15 * 10**8;
	...
	function removefuturePool(uint index) external onlyOwner
	{
		require(index < futurePools.length, "index out of bounds");
		if(index < futurePools.length - 1)
		{
			futurePools[index] = futurePools[futurePools.length - 1];
		}
