...
contract MasterChef is Ownable, ReentrancyGuard, IMasterChef, Constants {
    ...
    uint16 public currentRewardIteration = 0;
    ...
    function pendingRewards(address _user) external view returns (uint128 _lotl, uint128 _busd) {
        uint128 rewardPoolShareUser = rewardPoolShare[currentRewardIteration][_user];
        if(rewardPoolShareUser > 0){
        uint128 busdPending = rewardInfo.amountBUSD * rewardPoolShareUser / 1e12;
        uint128 lotlPending = rewardInfo.amountLOTL * rewardPoolShareUser / 1e12;
    ...
    function withdraw(uint8 _pid, uint128 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint128 taxedAmount = _amount * (1e4 - pool.unstakingFeeBP) / 1e4;
    ...
    function withdrawReward() public nonReentrant {
        uint128 rewardPoolShareUser = rewardPoolShare[currentRewardIteration][msg.sender];
        require(rewardPoolShareUser > 0, "withdraw: not good");
        uint128 busdPending = rewardInfo.amountBUSD * rewardPoolShareUser / 1e12;
        uint128 lotlPending = rewardInfo.amountLOTL * rewardPoolShareUser / 1e12;
            ...
            uint16 userLength = uint16(pool.poolUser.length);
                for(uint32 j; j < userLength; j++){
                    ...
                    if(user.stakedSince > 0){
                        adjustedLiquidity[i] += uint128(user.amount * 1e12 * calculateTimeRewards(user.stakedSince));
                    }
                }
            }   
        for(uint32 i; i < totalUserLength; i++){
            address rewardUser = rewardInfo.poolUser[i];
            uint128 adjustedAmount;
            for(uint8 j; j < length; j++){
                UserInfo memory user = userInfo[j][rewardUser];
                if(user.stakedSince > 0){
                    PoolInfo memory pool = poolInfo[i];
                    adjustedAmount += uint128(user.amount * 1e12 * calculateTimeRewards(user.stakedSince) / adjustedLiquidity[j] / pool.allocPoint / totalAllocPoint);
                }
            }
