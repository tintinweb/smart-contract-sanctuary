...
contract nft is ERC721Enumerable, Ownable {
    ...
    uint256 private mintCost_ = 120000000 gwei;
    ...
    constructor() ERC721('Horgi', 'H') {
        //this uri will only be used once revealed is on
        _currentBaseURI = 'ipfs://QmNR3tEnd5XBsdQqpYqRmEuATNURhqkMie37YVnNTADX1n/';
        //start off with zero tokens and max in total collection will be 1k
        numberOfTokens = 0;
        maxTokens = 1000;

        //reserved mints values
        reservedMints = 0;
        maxReservedMints = 110;
    ...
    function tokenId() internal view returns(uint256) {
        uint currentId = numberOfTokens + 1 + maxReservedMints - reservedMints;
    ...
    function mint(uint number) payable public {
        ...
        require(msg.value >= mintCost_ * number, 'Insufficient Funds, Price of each NFT is 0.12 BNB');
    ...
    function reservedMint(uint256 number, address recipient) public onlyOwner {
        ...
        for (uint256 i = 0; i < number; i++) {
            _safeMint(recipient, reservedMints + 1);
