...
contract CrowdswapV1 is Ownable, ICrowdswap {
    ...
    external override payable returns (uint256 returnAmount){
        ...
        address _dexAddress;
        (_beforeSwappingBalance, _dexAddress) = _prepareSwap(_fromToken, _destToken, _desc.amountIn, _dexFlag);

        //TODO: prevent the “sandwich” attack
        uint256[] memory dexResult;
        if (_dexFlag == _UNISWAPV2 || _dexFlag == _SUSHISWAP) {
            if (_fromToken.isETH()) {
                revert(toAsciiString(_dexAddress));
                try IUniswapV2Router02(_dexAddress).swapExactETHForTokens{value : msg.value}(
                    _desc.amountOutMin,
                    _desc.path,
                    address(this),
                    _desc.deadline
