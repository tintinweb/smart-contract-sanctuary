...
contract MadDogsNFT is ERC721Enumerable, Ownable {
    ...
    function mintDog(address _to, uint256 num) public payable {
        uint256 supply = totalSupply();
        if(msg.sender != owner()) {
        require( !_paused,                                 "Sale paused" );
        require( num < (_maxMint + 1),                     "You can mint a maximum of 20 dogs per call" );
        require( supply + num < MAX_SUPPLY - _reserved,    "Not enough dogs" );
        require( msg.value >= _price * num,                "Incorrect ether value" );
        }
