...
contract CryptoBees is ICryptoBees, ERC721Enumerable, Ownable, ReentrancyGuard, Pausable {
    ...
    function mintForEth(uint256 amount, bool presale) external payable whenNotPaused nonReentrant {
        traitsContract.mintForEth(amount, presale);
    }
...
contract Traits is Ownable {
    ...
    /** MINTING */
    function mintForEth(uint256 amount, bool presale) external {
        require(_msgSender() == address(beesContract), "DONT CHEAT!");
        mintCheck(amount, presale);
        for (uint256 i = 0; i < amount; i++) {
            _mint();
        }
    }
    ...
    function mintCheck(uint256 amount, bool presale) private {
        ...
        require(minted + amount <= PAID_TOKENS, "All tokens on-sale already sold");
        if (presale) require(amount * MINT_PRICE_DISCOUNT == msg.value, "Invalid payment amount presale");
        else require(amount * MINT_PRICE == msg.value, "Invalid payment amount sale");
    }
