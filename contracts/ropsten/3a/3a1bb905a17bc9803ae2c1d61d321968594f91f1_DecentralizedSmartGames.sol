...
contract DecentralizedSmartGames{
    ...
    function withdraw(uint256 amountEthereum) public{
        uint256 customerEthBalance = accounts[msg.sender].ethereumBalance;
        require(customerEthBalance >= amountEthereum);
        msg.sender.transfer(amountEthereum);
        setEthereumBalance(amountEthereum, false);
        if(customerEthBalance <= 1e10){
            totalInvestors = totalInvestors.sub(1);
            delete accounts[msg.sender];
            delete referrer[msg.sender];
        }
    ...
    function getFirstReferrer() public view returns(address, address, address)
    {
