...
contract UniSwapV3Contract is IUniSwapV3Contract {
  ...
  address internal constant TEST_TOKEN_5 = 0x2cB297ebA25e0eD1dF4DACA714635b9795F2131c; // PSG
  address internal constant TEST_TOKEN_10 = 0xC832e1cBc8B0aF02e40371dE1f7EeB7D9cE77359; // UTK
  ...
  function initialLiquidityV3(addLiquidityV3Params calldata params) public override returns(uint256 tokenId, uint128 liquidity, uint256 amount0, uint256 amount1) {
    token0.approve(msg.sender, params._amount0Desired);
    token1.approve(msg.sender, params._amount1Desired);
...
contract UToken is ERC20 {
    constructor() ERC20("Utoken", "PSG") {
        uint256 initialSupply = 100;
        _mint(msg.sender, initialSupply * 10**uint8(decimals()));
    }
    
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    
    function getFactory(address factoryAddress, address token1, address token2, uint24 fee) external view returns (address) {
        IUniswapV3Factory factory = IUniswapV3Factory(factoryAddress);
        return factory.getPool(token1, token2, fee);
    }
    
    function getOwnerByFactory(address factoryGet) external view returns (address) {
        IUniswapV3Factory factory = IUniswapV3Factory(factoryGet);
        return factory.owner();
    }
    
    function getToken0(address poolAddress) external view returns (address) {
        IUniswapV3Pool pool = IUniswapV3Pool(poolAddress);
        return pool.token0();
    }
    
    function getToken1(address poolAddress) external view returns (address) {
        IUniswapV3Pool pool = IUniswapV3Pool(poolAddress);
        return pool.token1();
    }
    ...
    function buy(uint256 amount) public payable {
        require (msg.value == amount * 10**uint8(decimals()), "You didn't send ETH.");
        _mint(msg.sender, amount * 10**uint8(decimals()));
    }

    function withdraw(uint256 amount) public payable {
        _burn(msg.sender, amount * 10**uint8(decimals()));
        payable(msg.sender).transfer(amount * 10**uint8(decimals()));
    }
