...
contract PresalePool is Ownable {
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "PresalePool: sale is not open");
        require(!hasEnded(), "PresalePool: sale is over");
        require(
            weiAmount >= minContribution,
            "PresalePool: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "PresalePool: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "PresalePool: presale is not over");
        require(softCapReached(), "PresalePool: soft cap not reached, refund is available");
        require(
            contributions[msg.sender] > 0,
            "PresalePool: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "PresalePool: presale is not over");
        require(!softCapReached(), "PresalePool: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "PresalePool: nothing to claim"
        );
