...
contract ZooGame  {
        ...
        for(uint i=0;i<animals.length;i++){
            ...
            if(animalInRound.totalAmount>0){
                ...
                if(animalInRound.lastPrice>=animalInRound.firstPrice){
                    animalInRound.range = animalInRound.lastPrice.sub(animalInRound.firstPrice).div(animalInRound.firstPrice);
                }
                else{
                    animalInRound.range = animalInRound.firstPrice.sub(animalInRound.lastPrice).div(animalInRound.firstPrice);
                }
    ...
    function getTokenPrice(address _pairAddress,address _token) public view returns(uint)
    {
    ...
    function claimReward() public lock{
        ...
        for(uint i=0;i<animals.length;i++){
            totalRewardPening = totalRewardPening.add(calRewardPending(msg.sender,animals[i]));            
        }
