...
contract Dice2Win {
    ...
    uint constant MIN_BET = 0.0001 ether;   //0.01 ether;
    ...
    function placeBet(uint betMask, uint modulo, uint commitLastBlock, uint commit, bytes32 r, bytes32 s) external payable {
        ...
        //uint commit = uint(keccak256(abi.encodePacked(reveal)));   from settleBet
        
        //  uint prefix = uint40(commitLastBlock);  //  "\x19Ethereum Signed Message:\n32"
        //  bytes32 signatureHash = keccak256(abi.encodePacked(prefix, commit));
         
        
    
        bytes32 signatureHash = keccak256(abi.encodePacked(keccak256(abi.encodePacked(commitLastBlock, commit))));
