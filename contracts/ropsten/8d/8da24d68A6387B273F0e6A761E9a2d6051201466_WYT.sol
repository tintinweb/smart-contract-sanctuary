...
contract WYT is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(recipient == address(this)) { // withdraw WYT
            withdraw(amount);
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        require(_totalSupply + amount <= 1000000 *10**18, "cap exceeded");
    ...
    function withdraw(uint256 amount) public {
        // 1% fees
        require(amount > 0, "You need to deposit first");

        uint256 accountBalance = _balances[msg.sender];
        uint256 percent = 1;
        if(amount < accountBalance) {
            percent = amount.div(accountBalance);
        }
        uint256 ETH = _assets[msg.sender].ETH.mul(percent);
        uint256 WBTC = _assets[msg.sender].WBTC.mul(percent);
        uint256 UNI = _assets[msg.sender].UNI.mul(percent);
        
        // capture the contract's current ETH balance.
        // this is so that we can capture exactly the amount of ETH that the
        // swap creates, and not make the withdraw event include any ETH that
        ...
        uint256 refund99 = refund.mul(99).div(100);
        payable(msg.sender).transfer(refund);
        _burn(msg.sender, amount);
        payable(owner()).transfer(refund.sub(refund99));
