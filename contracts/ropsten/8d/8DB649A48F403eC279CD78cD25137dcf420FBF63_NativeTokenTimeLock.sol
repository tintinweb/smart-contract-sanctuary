...
contract NativeTokenTimeLock is Ownable {
    ...
    function () payable external {
        ...
        LockTime memory item = LockTime({amount: msg.value, releaseDate: now + _releaseAfterTime});
        lockList[msg.sender].push(item);

        _token.mint(_token.owner(), rate * msg.value / 1 ether);

        emit ERC20Minted(rate, rate * msg.value / 1 ether, msg.value);
    }
