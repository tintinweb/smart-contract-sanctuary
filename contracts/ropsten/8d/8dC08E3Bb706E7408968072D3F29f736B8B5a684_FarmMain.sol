...
contract FarmMain is IFarmMain, ERC1155Receiver {
    ...
    function _mintFarmTokenAmount(address uniqueOwner, uint256 amount, uint256 setupIndex) private returns(uint256 objectId) {
        if (_farmTokenCollection == address(0)) {
            (_farmTokenCollection,) = IEthItemOrchestrator(_orchestrator).createNative(abi.encodeWithSelector(INativeV1(address(0)).init.selector, _toString(), _toString(), true, IFarmFactory(_factory).getFarmTokenCollectionURI(), address(this), ""), "");
        }
        if (_setups[setupIndex].objectId == 0) {
            (objectId,) = INativeV1(_farmTokenCollection).mint(amount, "", "", IFarmFactory(_factory).getFarmTokenURI(), true);
    ...
    function _toString() private pure returns(string memory) {
        return "a";
    }
