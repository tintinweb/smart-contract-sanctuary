...
contract ticTacToe is Ownable, ReentrancyGuard, Pausable {
 ...
 //--------------------declaration of independence, i mean, variables--------------------
  ...
  uint256 public entryFee = 0.11 ether;//* 10**18; //default AVAX (maybe MOVR, later) amount required to play
 ...
 //--------------------------------------constructor, misc-------------------------------------
  ...
  function claimDonation(address payable _charityAddress) external nonReentrant whenNotPaused {
    ...
    //(bool success, ) = _charityAddress.call{ value: donation }(""); require(success, "donation claim fail");
  ...
  function ownerClaimRewards() external onlyOwner whenNotPaused {
    require(percSum<=100, "Check claim distribution percentages and correct such that they add to less than 100");
    uint256 ownerClaimAmount = ownerClaimableRewards;
    ownerClaimableRewards = 0;
    //(bool success, ) = msg.sender.call{ value: ownerClaimAmount }(""); require(success, "owner claim fail");
    payable(msg.sender).transfer(ownerClaimableRewards);
    updateClaimable();
  }
  ...
  function withdraw(uint256 _withdrawAmount) external onlyOwner whenNotPaused {   
    ...
    //(bool success, ) = msg.sender.call{ value: _withdrawAmount }(""); require(success, "withdraw fail");
