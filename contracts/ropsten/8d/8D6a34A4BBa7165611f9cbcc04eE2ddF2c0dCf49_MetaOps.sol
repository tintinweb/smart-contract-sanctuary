...
contract MetaOps is Mintable, ERC721, ERC721Enumerable {
    ...
    uint public maxTokenPurchase = 3;
    ...
    constructor(address _imx)
        ERC721("MetaOps Battle Pass", "MOBP")
        Mintable(_imx) { _isWhiteListed[msg.sender] = true; }
    ...
    function buy(uint256[] memory _ids) public payable {
        ...
        require(_isWhiteListed[msg.sender], "You are not allowed to mint token");
        require(_numberOfWallets[msg.sender] + numberOfTokens <= maxTokenPerWallet, "Exceeded max token purchase per wallet");
        require(_occupiedList.length + numberOfTokens <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
