...
contract NFTTT is ERC721, ERC721Pausable, Ownable {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override (ERC721, ERC721Pausable) {
     ...
     function removeTokenFromOwner(address from, uint256 tokenId) private {
        uint256 lastTokenIndex = _minted[from];
        uint256 tokenIndex = _mintedIds[from][tokenId];
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _mintedIds[from][lastTokenIndex];
            _mintedIds[from][tokenIndex] = lastTokenId; 
            _mintedIds[from][lastTokenId] = tokenIndex; 
        }

        delete _mintedIds[from][lastTokenIndex];
    }
