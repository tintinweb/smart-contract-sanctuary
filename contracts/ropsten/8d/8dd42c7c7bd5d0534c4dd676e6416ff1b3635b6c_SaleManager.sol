...
contract SaleManager is Ownable {
    ...
    function createSale(string memory _tokenHash, uint _interval, uint _startPrice, uint24 _royalty, uint8 _kind) external onlyReseller returns (bool) {
    ...
    function destroySale(string memory _tokenHash) external onlyReseller returns (bool) {
    ...
    function placeBid(string memory _tokenHash, uint8 _kind) payable external notOnlyNFTOwner(_tokenHash) returns (bool) {
        require(getAuctionState(_tokenHash) == AuctionState.OPEN || getAuctionState(_tokenHash) == AuctionState.DIRECT_BUY, "Auction state is not open...");
    ...
    function performBid(string memory _tokenHash, uint8 _kind) external returns (bool) { 
        require(getAuctionState(_tokenHash) == AuctionState.ENDED || getAuctionState(_tokenHash) == AuctionState.DIRECT_BUY, "Auction state is not correct...");
