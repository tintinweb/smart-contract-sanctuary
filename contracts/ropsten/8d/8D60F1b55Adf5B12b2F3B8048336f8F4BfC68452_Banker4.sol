...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "Banker4";
    string internal constant SYMBOL = "BNKR4";
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        _takeFees( amount, currentRate, sumOfFees, sender, recipient);
        emit Transfer(sender, recipient, tTransferAmount);
    }
    
    function _takeFees(uint256 amount, uint256 currentRate, uint256 sumOfFees, address sender, address recepient ) private {
        if ( sumOfFees > 0 && !isInPresale ){
            _takeTransactionFees(amount, currentRate, sender, recepient);
        }
    }
     ...
     */
    function _takeTransactionFees(uint256 amount, uint256 currentRate, address sender, address recepient) internal virtual;
}
...
abstract contract SafeToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    function _beforeTokenTransfer(address sender, address recepient, uint256 , bool ) internal override {
         bool isWallet = !(Address.isContract(sender) && Address.isContract(recepient));
        if ( !isInPresale  && isWallet){
            uint256 contractTokenBalance = balanceOf(address(this));
            liquify( contractTokenBalance, sender );
        }
    }

    function _takeTransactionFees(uint256 amount, uint256 currentRate, address sender, address recepient) internal override {
