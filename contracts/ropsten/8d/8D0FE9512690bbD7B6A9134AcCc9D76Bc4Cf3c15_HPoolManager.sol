...
contract HPoolManager is ERC1155HolderUpgradeable, HordUpgradable {
    ...
    function setChampionAddress(address _championAddress, uint256 poolId) external {
        ...
        require(_championAddress != address(0), "Champion address can not be 0x0 address");
        require(msg.sender == hp.championAddress, "msg.sender is not champion.");
        ...
        for(uint256 i = 0; i < championAddressToHPoolIds[msg.sender].length; i++) {
            if(championAddressToHPoolIds[msg.sender][i] == poolId) {
                championAddressToHPoolIds[msg.sender][i] = championAddressToHPoolIds[msg.sender][championAddressToHPoolIds[msg.sender].length - 1];
                break;
            }
        }
        ...
        onlyMaintainer
    {
        ...
        require(block.timestamp >= hp.endPrivateSubscriptionPhase || numberOfTicketsUsed[poolId] < maxTicketsToUse, "Time is up or all tickets have been used.");
