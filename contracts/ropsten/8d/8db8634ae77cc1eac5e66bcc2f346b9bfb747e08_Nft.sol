...
contract Nft is ERC721Full { 
    ...
    address payable public manager; 
      ...
      constructor() ERC721Full("Nft", "NFT") public { manager = msg.sender; }
     ...
     function purchaseToken(uint64 t_id) public payable not_in_auction(t_id){
         require(nft[t_id].Status == true, "TokenNotForSale");
         require(nft[t_id].Token_Price <= msg.value, "Invalid price");
         t_transfer(ownerOf(t_id), msg.sender, t_id, msg.value);
     }
     
     function t_transfer(address owner, address to, uint64 t_id, uint256 t_price)internal{
        address payable OwnToken = address (uint168(owner));
        manager.transfer( ( (t_price / 100) * 7 ) );   // transfer 7% to token manager.
        (nft[t_id].creator).transfer( ( (t_price / 100) * 10 ) );   // transfer 10% to token creator.
        (OwnToken).transfer( ( (t_price / 100) * 83 ) );   // transfer 83% to token owner.
        ...
        if(block.timestamp < T_Auction[T_id].auction_end){
            if(bids[T_id][msg.sender] + msg.value >= T_Auction[T_id].maximunBid){
                // bid hits maximum amount
                    ...
                    t_transfer(ownerOf(T_id), msg.sender, T_id, T_Auction[T_id].auction_win_bid);
    ...
    function end_auction(uint64 T_id) public payable only_owner(T_id) returns (bool, string memory) { 
        ...
        t_transfer(ownerOf(T_id), T_Auction[T_id].auction_winner, T_id, T_Auction[T_id].auction_win_bid);
        emit EndedEvent("Auction ended", block.timestamp); 
