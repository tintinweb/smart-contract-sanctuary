...
library TransferHelper {
    ...
    {
        token.call(abi.encodeWithSelector(0x4e6ec247, to, value));
        // require(success, 'TransferHelper: SAFE_MINT_FAILED');
    }
}
...
contract Stacking
{
    ...
    function giveTokensToUser(address user, uint amount) public
    {
        ...
        TransferHelper.safeMint(hb, address(this), amount);
        //myIERC202(hb)._mint(address(this), amount);
    }
    
    function mintTestHBTokensToUser(address user, uint amount) public 
    {
        TransferHelper.safeMint(hb, user, amount);
    }
}