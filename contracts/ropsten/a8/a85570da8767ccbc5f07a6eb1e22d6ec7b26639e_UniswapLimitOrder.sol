...
import {WETH} from "../../constants/CTokens.sol";
    ...
    function swapUniswap(
        ...
        if (buyToken == ETH_ADDRESS) {
            ...
            returns (uint256[] memory buyAmounts) {
                ...
                revert(
                    "ActionUniswapV2Trade.action: swapExactTokensForETH reverted"
                );
            }
        } else {
            try
            ...
            returns (uint256[] memory buyAmounts) {
                ...
                revert(
                    "ActionUniswapV2Trade.action: swapExactTokensForTokens reverted"
                );
            }
        }
...
contract UniswapLimitOrder is ActionUniswapV2Trade, SimpleServiceStandard {
    ...
    constructor(
        ...
        address _gelato
    ) ActionUniswapV2Trade(_uni) SimpleServiceStandard(_gelato) {
    ...
    function exec(
        ...
        external
        gelatofy(
            _outTokenFee ? _o.path[_o.path.length - 1] : _o.path[0],
            _o.user,
            abi.encode(
                _o.user,
                _o.path[0],
        ...
        )
    {
        ...
        swapUniswap(
            _o.path,
            _outTokenFee ? _o.amountIn : _o.amountIn - _fee,
            _outTokenFee ? _o.amountOut + _fee : _o.amountOut,
            _outTokenFee ? address(this) : _o.user
        );
...
contract SimpleServiceStandard is TaskStorage {
    ...
    modifier gelatofy(
        ...
        // Pay Gelato
        if (_swapRate == 0) _transferEthAndToken(gelato, _feeToken, _fee);
