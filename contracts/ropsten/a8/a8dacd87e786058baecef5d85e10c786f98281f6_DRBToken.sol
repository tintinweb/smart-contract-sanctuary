...
contract DRBToken is ERC20("DRBToken", "DRB"), Ownable {
    using SafeMath for uint256;

    uint256 public NUMBER_BLOCKS_PER_DAY;
    uint256 public cap = 80000000e18 + 1e18;
    address public DRBMaster;
    address public DRBMasterPending;
    uint public setDRBMasterPendingAtBlock;
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (from == address(0)) {
            require(totalSupply().add(amount) <= cap, "DRBToken: cap exceeded");
        }
    }

    function setPendingMaster(address _DRBMaster) public onlyOwner {
        DRBMasterPending = _DRBMaster;
        setDRBMasterPendingAtBlock = block.number;
    }

    function confirmMaster() public onlyOwner {
        require(block.number - setDRBMasterPendingAtBlock > 2 * NUMBER_BLOCKS_PER_DAY, "DRBToken: cannot confirm at this time");
        DRBMaster = DRBMasterPending;
    }

    function setMaster(address _DRBMaster) public onlyOwner {
        require(DRBMaster == address(0x0), "DRBToken: Cannot set master");
        DRBMaster = _DRBMaster;
    }

    function mint(address _to, uint256 _amount) public {
        require(msg.sender == DRBMaster, "DRBToken: only master farmer can mint");
        require(seedPoolAmount > 0, "DRBToken: cannot mint for pool");
        require(seedPoolAmount >= _amount, "DRBToken: amount greater than limitation");
    ...
    function mintForPartnership(address _to) public onlyOwner {
        ...
        require(block.number >= startAtBlock + 7 * NUMBER_BLOCKS_PER_DAY, "DRBToken: Cannot mint at this time");
        require(partnershipAmount > 0, "DRBToken: Cannot mint more token for partnership");
    ...
    function mintForTeam(address _to) public onlyOwner {
        ...
        require(block.number >= startAtBlock + 7 * NUMBER_BLOCKS_PER_DAY, "DRBToken: Cannot mint at this time");
        require(teamAmount > 0, "DRBToken: Cannot mint more token for team");
        // This first minting
    ...
    function mintForFutureFarming(address _to) public onlyOwner {
        require(block.number >= startAtBlock + 56 * NUMBER_BLOCKS_PER_DAY, "DRBToken: Cannot mint at this time");
        require(futureAmount > 0, "DRBToken: Cannot mint more token for future farming");
