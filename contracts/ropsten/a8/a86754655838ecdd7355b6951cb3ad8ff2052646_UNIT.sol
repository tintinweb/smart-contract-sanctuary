...
contract UNIT is IERC20, Ownable, Pausable {
    ...
    uint private _totalSupply = 15_000_000e18;
    string private constant _name = "UNIT";
    string private constant _symbol = "UNIT";
    uint8 private constant _decimals = 18;

    uint private vestingFirstPeriod = 30 days;
    uint private vestingSecondPeriod = 90 days;
    ...
    function transferFrom(address sender, address recipient, uint amount) external override whenNotPaused returns (bool) {
        ...
        _approve(sender, msg.sender, _allowances[sender][msg.sender].sub(amount, "UNIT::transferFrom: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function permit(address owner, address spender, uint amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external whenNotPaused {
        ...
        require(signatory != address(0), "UNIT::permit: invalid signature");
        require(signatory == owner, "UNIT::permit: unauthorized");
        require(block.timestamp <= deadline, "UNIT::permit: signature expired");
    ...
    function decreaseAllowance(address spender, uint subtractedValue) external returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].sub(subtractedValue, "UNIT::decreaseAllowance: decreased allowance below zero"));
        return true;
    }

    function unvest() external whenNotPaused returns (uint unvested) {
        require (_vestingNonces[msg.sender] > 0, "UNIT::unvest:No vested amount");
    ...
    function vest(address user, uint amount) external {
        require (vesters[msg.sender], "UNIT::vest: not vester");
        _vest(user, amount, 1);
    }

    function burnTokens(uint amount) external returns (bool success) {
        require(amount <= _unfrozenBalances[msg.sender], "UNIT::burnTokens: exceeds available amount");
        _unfrozenBalances[msg.sender] = _unfrozenBalances[msg.sender].sub(amount, "UNIT::burnTokens: transfer amount exceeds balance");
        _totalSupply = _totalSupply.sub(amount, "UNIT::burnTokens: overflow");
    ...
    function _approve(address owner, address spender, uint amount) private {
        require(owner != address(0), "UNIT::_approve: approve from the zero address");
        require(spender != address(0), "UNIT::_approve: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint amount) private {
        require(sender != address(0), "UNIT::_transfer: transfer from the zero address");
        require(recipient != address(0), "UNIT::_transfer: transfer to the zero address");

        _unfrozenBalances[sender] = _unfrozenBalances[sender].sub(amount, "UNIT::_transfer: transfer amount exceeds balance");
    ...
    function multisend(address[] memory to, uint[] memory values) external onlyOwner returns (uint) {
        ...
        _unfrozenBalances[owner] = _unfrozenBalances[owner].sub(sum, "UNIT::multisend: transfer amount exceeds balance");
        for (uint i; i < to.length; i++) {
            _unfrozenBalances[to[i]] = _unfrozenBalances[to[i]].add(values[i], "UNIT::multisend: transfer amount exceeds balance");
    ...
    function multivest(address[] memory to, uint[] memory values) external onlyOwner returns (uint) { 
        ...
        _unfrozenBalances[owner] = _unfrozenBalances[owner].sub(sum, "UNIT::multivest: transfer amount exceeds balance");
        for (uint i; i < to.length; i++) {
