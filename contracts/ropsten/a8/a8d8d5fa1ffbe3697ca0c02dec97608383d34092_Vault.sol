...
contract Vault is Ownable, ReentrancyGuard
{
    ...
    address private vault_token_address;
    uint256 private vault_token_decimals;
    ...
    modifier validTermID(uint256 _term_id)
    {
        ...
        require(
            0 <= _term_id && _term_id < (number_of_terms - 1),
            "Invalid term ID"
        );
    ...
    //
    // @param _amount:
    //  - The number of tokens to recover. This number is scaled automatically
    //    to the number of decimals used by the associated token so that usage
    //    is intuitive. However, as a consequence, decimal amounts are not
    //    supported.
    function recoverVaultTokens(uint256 _amount) public onlyOwner
    {
        ...
        // Scales the given token amount to match the number of decimals it
        // uses.
        _amount = _scaleTokenAmount(_amount);
    ...
    //
    // @param _amount:
    //  - The number of tokens to deposit. This number is scaled automatically
    //    to the number of decimals used by the associated token so that usage
    //    is intuitive. However, as a consequence, decimal amounts are not
    //    supported.
    function deposit(
        uint256 _term_id,
        uint256 _amount
    ) public payable nonReentrant validTermID(_term_id)
    {
        ...
        _deposit.reward    = _calculateReward(
            _amount,
