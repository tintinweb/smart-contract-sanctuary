...
// Modified to work in the OLIV system

// all votes work on underlying _olivBalances[address], not balanceOf(address)
...
//    Since OLIV supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match OLIV as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "OLIV Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(oliv.initSupply(), 4), 100); } // 4% of OLIV

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(oliv.initSupply(), 100); } // 1% of OLIV
    ...
    constructor(address timelock_, address oliv_) public {
        timelock = TimelockInterface(timelock_);
        oliv = OLIVInterface(oliv_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(oliv.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || oliv.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = oliv.getPriorVotes(voter, proposal.startBlock);
