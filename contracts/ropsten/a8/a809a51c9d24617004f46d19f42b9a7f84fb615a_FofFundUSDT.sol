...
//母基金合约，区分不同资金币种。一个资金币种一个合约实例。
contract FofFund is DSPausable
{
    using SafeMath for uint256;
    using SafeERC20 for ERC20;
    using SafeERC20 for DSToken;

    DSToken public fdsToken; //母基金token合约
    uint256 public maxIssueAmount; //最大发行金额（单位为资金币种数量）
    uint256 public currentIssueAmount; //当前已经发行量（单位为资金币种数量）

    IVault public vault;  //对应的机枪池
    ERC20 public capitalToken; //资金币token合约

    //DSToken public structToken; //项目代币

    event IssueAmountChanged(uint256 currentIssueAmount_,uint256 maxIssueAmount_);
    event Purchased(address user,uint256 amount,uint256 fdsCount);
    event Redeemed(address user,uint256 amount);

    constructor(address vaultAddress,uint256 maxIssueAmount_,address fdsTokenAddress
                , address ctrlPanel) public{
        require(vaultAddress!=address(0));
        require(fdsTokenAddress!=address(0));
        vault =IVault(vaultAddress);
        require(vault.capitalTokenAddress()!=address(0));
        capitalToken =ERC20(vault.capitalTokenAddress());
        fdsToken =DSToken(fdsTokenAddress);
        maxIssueAmount =  maxIssueAmount_;

        //structToken=DSToken(CtrlPanel(ctrlPanel).structToken());
    }
    ...
    //最大发行金额
    function setMaxCirculation(uint256 maxIssueAmount_) external onlyOwner {
        maxIssueAmount =maxIssueAmount_;
    }

//    function setVault(address vaultAddress) external onlyOwner{
//        require(vaultAddress!=address(0));
//        uint256 totalTokenBalance= vault.tokenAmount(address(this));
//        vault.redeem(totalTokenBalance);
//
//
//        //TODO:根据当前资金量买入新机枪池
//        IVault vaultNew=IVault(vaultAddress);
//        uint256 totalBalance= capitalToken.balanceOf(address(this));
//        vaultNew.purchase(totalBalance);
//
//        vault =vault;
//    }

//    //得到购买tokenAmount份数的基金需要付出的资金token的数量。
//    function queryPurchaseCost(uint256 tokenAmount) view public returns(uinty256){
//        uint256 vaultTokenPrice=_vault.queryPrice();
//        uint256 amount=tokenAmount.mul(vaultTokenPrice);
//        return amount;
//    }

    //申购(存入资金token（approve方式），得到基金token)
    function purchase(uint256 amount) external whenNotPaused{
        require(amount>0,"purchase amount ==0 ");
        require(capitalToken.balanceOf(_msgSender())>=amount,"Insufficient capital available");
        require(currentIssueAmount.add(amount) <= maxIssueAmount,"Exceeding the maximum issue amount");

        //从用户的allowance份额转入当前合约
        capitalToken.safeTransferFrom(_msgSender(),address(this),amount);
        //从当前合约approve到机枪池，由机枪池的purchase转入机枪池。
        capitalToken.safeApprove(address(vault),amount);

        uint256 vaultTokenCount = vault.purchase(amount);//当前合约到机枪池,返回新得到的机枪池token的数量

        //产生母基金token
        fdsToken.mint(_msgSender(), vaultTokenCount);//1:1 机枪池token换为此基金token给用户。
        currentIssueAmount = currentIssueAmount.add(amount);

        emit Purchased(_msgSender(),amount,vaultTokenCount);
        emit IssueAmountChanged(currentIssueAmount, maxIssueAmount);
    }

    //赎回（存入基金token（approve方式），得到资金token）
    function redeem(uint256 tokenCount) external whenNotPaused{
        require(tokenCount >0,"redeem amount should be > 0");
        require(fdsToken.balanceOf(_msgSender())>= tokenCount,"Insufficient amount");

        //收回母基金代币
        fdsToken.burnFrom(_msgSender(), tokenCount);

        //从机枪池赎回资金，到当前合约
        IERC20 vaultToken=IERC20(vault.tokenAddress());
        vaultToken.approve(address(vault), tokenCount);

        uint256 capitalBalance=capitalToken.balanceOf(address(this));
        vault.redeem(tokenCount); //用purchase操作得到的vaultToken，换回资金token到本合约。
        uint256 capitalBalanceNew=capitalToken.balanceOf(address(this));

        require(capitalBalanceNew>=capitalBalance,"Emergency: capitalBalanceNew < capitalBalance"); //亏的话，但不会小于，最多本金全部亏完。此时相等

        uint256 profitAmount=capitalBalanceNew.sub(capitalBalance);

        if (profitAmount>0){
            //从当前合约转出资金token到用户
            capitalToken.safeTransfer(_msgSender(),profitAmount);
            require(currentIssueAmount >= tokenCount, "Insufficient total amount");
            currentIssueAmount = currentIssueAmount.sub(tokenCount);
        }
        //TODO: 给用户项目代币STRUCT奖励
        emit Redeemed(_msgSender(), tokenCount);
        emit IssueAmountChanged(currentIssueAmount, maxIssueAmount);
    }
...
// File: instances/FofFundUSDT.sol

pragma solidity ^0.6.12;


contract FofFundUSDT is FofFund {
    constructor(address vault, address fdsToken)
        public
        FofFund(
            vault,
