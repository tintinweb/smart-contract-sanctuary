...
contract RiseUp is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x102E7d9988Ee5c68709Fa8eeE38c813241854b58); // Marketing Address
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != uniswapV2Pair) {
           ...
           require(balanceOf(to).add(amount) <= _maxWallet, "Transaction will exceed Maxmimum allowed wallet balance");
            if (!tradingOpen) {
                if (!(from == address(this) || from == address(this)
                || from == address(owner()) || from == address(owner()))) {
