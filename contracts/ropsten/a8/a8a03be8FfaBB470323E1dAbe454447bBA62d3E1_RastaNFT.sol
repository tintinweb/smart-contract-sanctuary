...
contract RastaNFT {
    ...
    function mint(uint256 tokenId, uint256 price, string calldata _tokenURI) external returns (bool) {
        ...
        _ownedNFTIndex[tokenId] = _ownedNFTs[ownerAddr].length - 1;
    ...
    function burn(uint256 tokenId) external returns (bool) {
        require(msg.sender == ownerAddr, "!owner");
        require(_NFTs[tokenId].owner == ownerAddr, "!owner");

        reduceNFT(tokenId, ownerAddr);
