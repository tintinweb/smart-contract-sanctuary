...
contract GameAuction {
    ...
    mapping(string => int256) private playerPositions;
    uint256 totalMovesByPAndE; // NOTE: this is increased via the increaseTurnNumber method in an external class
    string public winner;
    uint256 public MAXIMUM_TURN_ROUNDS = 10;
    ...
    mapping(uint => mapping(uint => mapping(address => Blinded_Bid[]))) public bids_team_p; // submitted team b bids in the running auction
    mapping(uint => mapping(uint => mapping(address => Blinded_Bid[]))) public bids_team_e; // submitted team e bids in the running auction

    mapping(uint => address) public highestBidders_p; // round => address, 
    mapping(uint => address) public highestBidders_e; // round => address, 
    mapping(address => uint) public stakes_p; // address => money, stakes which an address has in team p at current round
    mapping(address => uint) public stakes_e; // address => money, stakes which an address has in team e at current round

    mapping(uint8 => address[]) public teamMembers;
    mapping(address => uint8) public teamMembership;

    uint public numberOfBidsP;
    uint public numberOfBidsE; 
    uint public numberOfRevealsP;
    uint public numberOfRevealsE;
    address public highestBidder_p;
    address public highestBidder_e;
    uint public highestBid_p;
    uint public highestBid_e;
    uint public nextMove_p;
    uint public nextMove_e;
    uint public stakesPot_p;
    uint public stakesPot_e;
    ...
    bool public ended;
    uint public stage; // stage 0: Closed for bids and reveals, stage 1: accepting bids. Stage 2: Accepting reveals
        ...
        onlyBefore(revealEnd)
    {
        ...
        if (teamMembership[msg.sender] == 1){
            ...
            for (uint i = 0; i < length; i++) {
                ...
                (uint value, uint move, bytes32 salt) = (_values[i], _moves[i], _salts[i]);
        ...
        onlyIfAuctionCanBeEnded
    {
        ...
        // a) pursuer
        if (nextMove_p == 1) { // TODO: check if move is valid in the conditions
            movePursuerRight();
            emit MovePursuer("Move pursuer right");
        } else if (nextMove_p == 2) {// TODO: check if move is valid in the conditions
            movePursuerUpRight();
            emit MovePursuer("Move pursuer up right");
        } else if (nextMove_p == 3) {// TODO: check if move is valid in the conditions
            movePursuerUp();
            emit MovePursuer("Move pursuer up");
        } else if (nextMove_p == 4) {// TODO: check if move is valid in the conditions
            movePursuerUpLeft();
            emit MovePursuer("Move pursuer up left");
        } else if (nextMove_p == 5) {// TODO: check if move is valid in the conditions
            //movePursuerLeftt();
            emit MovePursuer("Move pursuer left");
        } else if (nextMove_p == 6) {
            //movePursuerDownLeft();
            emit MovePursuer("Move pursuer down left");
        } else if (nextMove_p == 7) {
            //movePursuerDown();
            emit MovePursuer("Move pursuer down");
        } else if (nextMove_p == 8) {
        ...
        } else if (nextMove_p == 3) {
            //moveEvaderUp();
            emit MoveEvader("Move evader up");
        } else if (nextMove_p == 5) {
            //moveEvaderLeft();
    ...
    // derived contracts).
    function placeBid_p(address bidder, uint value, uint move) internal
    ...
    function placeBid_e(address bidder, uint value, uint move) internal
            returns (bool success)
    {
    ...
    function Auctioneer(uint _stage) 
        public 
    ...
    function endGame()
        internal
        {
            ...
            if (keccak256(abi.encodePacked(winner)) == keccak256(abi.encodePacked("P"))){
                for (uint i = 0; i < MAXIMUM_TURN_ROUNDS; i++) {
    ...
    function assignTeam(address _address, uint8 _team) private{
            teamMembers[_team].push(_address);
            teamMembership[_address] = _team;
    }

    function resetTeamMemberships() 
        public // this must be internal and only called after a game!
        {
  ...
  */
    ...
    function getTurnNumber() public view returns (uint256) {
        return totalMovesByPAndE;
    }

    function increaseTurnNumber() public {
        totalMovesByPAndE = totalMovesByPAndE + 1;
    }

    function resetTurnNumber() public {
        totalMovesByPAndE = 0;
    }
  ...
  */
    function isValidIndex(int256 idx) private pure returns (bool) {
  ...
  */
    function getCurrentPursuerIndex() public view returns (int256) {
  ...
  */
    function getCurrentEvaderIndex() public view returns (int256) {
        return playerPositions["E"];
    }

    function setCurrentPursuerIndex(int256 newPursuerIdx) private {
        playerPositions["P"] = newPursuerIdx;
    }

    function setCurrentEvaderIndex(int256 newEvaderIdx) private {
    ...
    function movePursuerNone() public {
    }

    function isValidMovePursuerUpRight() public view returns (bool) {
        int256 currentIdx = getCurrentPursuerIndex();
        return
    ...
    function movePursuerUpRight() public { //TODO: must be private (called by auction)
        int256 currentIdx = getCurrentPursuerIndex();
        require(
            (currentIdx == playerPositions["P"]),
    ...
    function moveEvaderNone() public {
    }

    function isValidMoveEvaderUp() public view returns (bool) {
        int256 currentIdx = getCurrentEvaderIndex();
        return
    ...
    function moveUp(int256 currentIdx) private {
        require(
            ((currentIdx + 10) < 100),
    ...
    function moveDown(int256 currentIdx) private {
        require(
            ((currentIdx - 10) >= 0),
    ...
    function moveLeft(int256 currentIdx) private {
        require(
            ((currentIdx % 10) > 0),
            "Move Failed: Attemped to move out of the allowed left barrier"
        );
    ...
    function moveRight(int256 currentIdx) private {
        require(
            ((currentIdx % 10) < 9),
            "Move Failed: Attemped to move out of the allowed right barrier"
        );
  ...
  */
    function moveFromTo(int256 previousIdx, int256 newIdx) private {
        require(
