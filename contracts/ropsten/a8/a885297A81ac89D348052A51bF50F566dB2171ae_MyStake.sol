...
contract MyStake is Ownable,Pausable,ReentrancyGuard {
    ...
    function uploadInfo(string memory cpu,string memory gpu,string memory str,string memory mem) public {
        ...
        stake[msg.sender].AllUploadCount += 1;
        stake[msg.sender].rewardUpload += 1;
        if(stake[msg.sender].rewardUpload >= 3){
            stake[msg.sender].rewardUpload = 0;
            stake[msg.sender].AllRewardAmount += stake[msg.sender].deposit_amount * rewardPercent / 10000;
            stake[msg.sender].deposit_amount += stake[msg.sender].deposit_amount * rewardPercent / 10000;
        }
