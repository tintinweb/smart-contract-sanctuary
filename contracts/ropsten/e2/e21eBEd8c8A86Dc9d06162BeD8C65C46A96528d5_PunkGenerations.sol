...
contract PunkGenerations {
    ...
    function breedPunkAssets(uint16 fatherIndex, uint16 motherIndex) internal view returns (bytes memory childAssets) {
        ...
        bytes memory parentAssets = info.isMale ? motherAssets : fatherAssets;
        for (uint8 j = 1; j < 8; ++j) {
            fa = uint8(parentAssets[j]);
            if (fa > 0) {
                ma = getMappedAsset(fa, info.isMale);
                if (ma > 0) {
                    traits[getAssetType(ma)] = info.assetIndex;
                    childAssets[info.assetIndex++] = byte(ma);
                }
            }
        }
        
        for (uint8 j = 1; j < 8 && info.assetIndex < 8; ++j) {
            fa = uint8(info.isMale ? fatherAssets[j] : motherAssets[j]);
            if (fa > 0) {
                uint8 assetType = getAssetType(fa);
                uint8 previousAssetIndex = traits[assetType];
                if ((previousAssetIndex == 0) || (rand % 2 == 0)) {
                    if ((fa == 61) || (fa == 125)) { info.earringIndex = info.assetIndex; }
                    if ((fa == 19) || (fa == 115)) { info.cigaretteIndex = info.assetIndex; }
                    if (previousAssetIndex > 0) {
                        childAssets[previousAssetIndex] = byte(fa);
                    } else {
                        traits[assetType] = info.assetIndex;
                        childAssets[info.assetIndex++] = byte(fa);
                    }
                }
                rand = random(rand);
            }
            info.isDifferent = info.isDifferent || (childAssets[j] != byte(fa));
        }
