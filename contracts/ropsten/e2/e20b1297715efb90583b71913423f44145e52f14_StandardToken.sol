...
/*
  ...
  function getRewardsDetailsOfUserById(uint256 id) public view returns(uint256){
      ...
      return (((block.timestamp - _stakingStartTime[id])/86400) * (_rewardPercentage) * _usersTokens[id])/1000;
      } else if (_stakingEndTime[id] < block.timestamp){
        return (((_stakingEndTime[id] - _stakingStartTime[id])/86400) * (_rewardPercentage) * _usersTokens[id])/1000;
      } else{
      return 0;
    }
  }
  ...
  function withdrawStakedTokens(uint256 stakingId) public failSafe returns(bool){
    ...
    require(_stakingEndTime[stakingId] > _stakingStartTime[stakingId].add(2629743), "30 days staking time is not over");
 ...
 */
  ...
  uint256 _tokenVoteValue = 1000000000000000000;
  struct Proposal {
...
event VotesSubmitted (uint256 _proposalID, uint256 votesReceived, bool proposalPassed);
 ...
 */
  function blacklistAddresses(bool status, address _account) external onlyOwner {
   ...
   /**
   * @dev Function for setting votes with respect to tokens by owner
   * @param tokenVoteValue vote value
   */
  function setVoteValue(uint256 tokenVoteValue) public onlyOwner returns(bool){
    require(tokenVoteValue > 0, "Invalid value");
    _tokenVoteValue = tokenVoteValue;
    return true;
  }

  /**
   * @dev Function for getting vote value
   */
  function setVoteValue() public view returns(uint256){
    return _tokenVoteValue;
  }
 ...
 */
  function submitProposal(uint256 maxVotesNeeded, uint256 voteLength, uint256 voteTokens) public whenNotBlocked(msg.sender) returns (uint proposalID) {
    uint256 votesReceived = voteTokens;
    transferFrom(msg.sender, _owner, voteTokens);
    _burn(_owner, voteTokens);
...
// /**
//  * @dev modifier to check eligible voters
//  */ 
 ...
 * @param voteLength time limit for the voting
 */
  function addProposal(uint256 _votesReceived, address Submitter, uint256 maxVotesNeeded, uint256 voteLength) internal returns (uint proposalID) {
 ...
 */  
  function submitVote(uint256 _proposalID, uint256 voteTokens) whenNotBlocked(msg.sender) public returns (bool){
