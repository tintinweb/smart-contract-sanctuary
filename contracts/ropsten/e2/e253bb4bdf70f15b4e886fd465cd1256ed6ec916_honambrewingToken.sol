...
mapping (address => uint256) public lockup_time;

mapping (address => uint256) public lockup_amount;
...
lockup_amount[_addr] = lockup_amount[_addr] + amount;

lockup_time[_addr] = _releasetime;

}


function My_Lockup() public{

require(lockup_time[_msgSender()] <= block.timestamp, "Lockup time has not been released yet.");

require(lockup_amount[_msgSender()] != 0, "Locked token quantity is Zero.");

IERC20(this).transfer(msg.sender,lockup_amount[msg.sender]);

lockup_amount[_msgSender()] = 0;

lockup_time[_msgSender()] = 0;
