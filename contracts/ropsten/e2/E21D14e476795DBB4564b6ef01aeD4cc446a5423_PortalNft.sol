...
abstract contract ERC1155Bouncer is ERC1155, OffchainBouncer, ERC1155Supply {
  string name_;
  string symbol_; 
...
abstract contract OffchainBouncer is AccessControl, Ownable, EIP712 {
  ...
  function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
    return _hashTypedDataV4(keccak256(abi.encode(
      keccak256("NFTVoucher(address minter,uint mintLimit,uint256 start,uint256 end,uint phaseCap)"),
        voucher.minter,
        voucher.mintLimit,
        voucher.start,
        voucher.end,
