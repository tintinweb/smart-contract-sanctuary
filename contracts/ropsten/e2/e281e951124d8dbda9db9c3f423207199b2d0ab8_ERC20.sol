...
library SafeMath {
  function add(uint a, uint b) internal pure returns (uint c) {c = a + b;
    require(c >= a);}

  function sub(uint a, uint b) internal pure returns (uint c) {require(b <= a);
    c = a - b;}

  function mul(uint a, uint b) internal pure returns (uint c) {c = a * b;
    require(a == 0 || c / a == b);}

  function div(uint a, uint b) internal pure returns (uint c) {require(b > 0);
    c = a / b;}
}
...
contract ERC20 is ERC20Interface, Owned {
  ...
  constructor(string memory _name, string memory _symbol, string memory _baseURI, uint8 _decimals, uint _supply) {
    symbol = _symbol;
    name = _name;
    decimals = _decimals;
    baseURI = _baseURI;
