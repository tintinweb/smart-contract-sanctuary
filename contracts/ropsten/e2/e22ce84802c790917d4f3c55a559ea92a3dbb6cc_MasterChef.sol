...
contract MasterChef is Ownable {
    ...
    // Max MobiFi Supply
    uint256 constant MaxSupply = 75e23;
    ...
    function safeMobiFiTransfer(address _to, uint256 _amount) internal {
        ...
        // Total  current Supply Of Token.
        uint256 CurrentPoolBalance = TotalBalanceDeposited;
        // Current Balance Of (Deposited Minus Rewards That Minted) Pool.
        uint256 TotalRewardBalance =
            CurrentPoolBalance <= MobiFiBal
                ? MobiFiBal.sub(CurrentPoolBalance)
                : 0;
        // if Current Balance Be More Than CurrentPoolBalance Then We Reduce CurrentPoolBalance From MobiFiBal To Get How Much Of Contract Token Balance Is Reward And How Much Is User Deposited Token And of Reward Be Nothing Then We Will Return 0.
        if (
            MobiFiSupply.add(_amount) > MaxSupply &&
            TotalRewardBalance >= _amount
        ) {
            //
            MobiFi.transfer(_to, _amount);
        } else if (MobiFiSupply.add(_amount) > MaxSupply) {
