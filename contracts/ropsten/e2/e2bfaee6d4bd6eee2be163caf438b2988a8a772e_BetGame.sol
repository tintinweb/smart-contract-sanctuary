...
*/
    ...
    {
        return ViewSecretBet(msg.sender);
    }
    
    function ViewSecretBet(address addr)  public view returns(bytes32[],uint[])
    {
        return (players[addr][round].secertBet,players[msg.sender][round].betNumbers);
    }
   
    function ViewBet(address addr,uint n)  public view returns(uint)
    {
        return (players[msg.sender][round].betNumbers[n]);
    }
