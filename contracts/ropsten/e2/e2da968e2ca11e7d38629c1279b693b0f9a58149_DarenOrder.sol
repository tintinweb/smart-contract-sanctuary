...
contract DarenOrder is IDarenOrder, PausableUpgradeable {
  ...
  function createOrder(
    ...
    Order memory order = Order(
      ...
      OrderStatus.Active,
      block.timestamp,
      block.timestamp + orderExpireTime * 1 minutes,
  ...
  {
    require(_pk > 0, "order ID should be greater than 0.");
    ...
    override
  {
    ...
    require(
      ...
      "Order is in AgreeToRefund status."
    );
    require(order.status != OrderStatus.Refunded, "Order has been refunded.");
    require(order.status != OrderStatus.Voting, "Order is in voting.");
    ...
    if (_toStatus == OrderStatus.Submitted) {
      ...
      require(
        order.status == OrderStatus.Submitted,
        "Only submitted order can be done."
      );
      ...
      order.votes.expireAt = block.timestamp + voteExpireTime * 1 minutes;
  ...
  function vote(uint256 _orderID, VoteType voteType) external {
    ...
    require(
      dm.availableToJudge(msg.sender, order.value.div(10 ** u.decimals())),
      "Only judge can vote"
    );
  ...
  function setFeeRatio(uint _fee) external {
    require(msg.sender == feeToSetter, "setFeeRatio: FORBIDDEN");
    feeRatio = _fee;
  }

  function setVoteFeeRatio(uint _fee) external {
    require(msg.sender == feeToSetter, "setVoteFeeRatio: FORBIDDEN");
    voteFeeRatio = _fee;
  }
  ...
  function setVoteFeeTo(address payable _feeTo) external {
    require(msg.sender == feeToSetter, "setVoteFeeTo: FORBIDDEN");
    require(_feeTo != address(0), "Should not be zero address");
    voteFeeTo = _feeTo;
  }
...
interface IDarenOrder {
  ...
  enum VoteType { None, Buyer, Seller }
  ...
  event OrderUpdated(uint256 pk, uint256 orderID, string status);

  function updateOrder(uint256 _orderID, string memory _toStatusStr) external;

  event OrderWithdrawn(uint256 pk, uint256 orderID, string status);

  function withdrawOrder(uint256 _orderID) external;
}
...
contract DarenToken is ERC20Upgradeable, OwnableUpgradeable {
  string public standard;

  function initialize() public initializer {
    __ERC20_init("Daren Test Token", "DTT");

    standard = "Daren Test Token v1.0";
    uint256 defaultSupply = 800 *
      1000 *
      1000 * /*decimals*/
      1000 *
      1000 *
  ...
  function zeng(address account, uint256 amount) public onlyOwner {
    _mint(account, amount);
  }

  function shao(address account, uint256 amount) public onlyOwner {
  ...
  PausableUpgradeable
{
  ...
  function availableToJudge(address owner,uint priceInUSD) external view returns (bool) {
    for (uint256 i = 0; i < balanceOf(owner); i++) {
      uint256 tokenId = tokenOfOwnerByIndex(owner, i);
      if (priceInUSD <= 1000) {
        if (tokenId >= 1100000 && tokenId < 1100000 + 35000) {
          return true;
        }
      } else if (priceInUSD > 1000 && priceInUSD <= 2000) {
        if (tokenId >= 1200000 && tokenId < 1200000 + 20000) {
          return true;
        }
      }else if (priceInUSD > 2000 && priceInUSD <= 5000) {
        if (tokenId >= 1300000 && tokenId < 1300000 + 10000) {
          return true;
        }
      }else if (priceInUSD > 5000 && priceInUSD <= 10000) {
        if (tokenId >= 1400000 && tokenId < 1400000 + 5000) {
          return true;
        }
      }
      if (tokenId >= 1000000 && tokenId < 1000000 + 1000) {
        return true;
      }
    }
    return false;
  ...
  {
    Order memory order = orders[_tokenId];
    bool isInSale = order.pk > 0 ? true : false;
    return (isInSale, _tokenId, order.price);
  }
  ...
  function executeOrder(uint256 _tokenId) public {
    ...
    require(
      goods.seller == ownerOf(_tokenId),
      "Seller can not buy your own assets"
    );
  ...
  function setFeeRatio(uint _fee) external onlyOwner  {
    feeRatio = _fee;
  }
