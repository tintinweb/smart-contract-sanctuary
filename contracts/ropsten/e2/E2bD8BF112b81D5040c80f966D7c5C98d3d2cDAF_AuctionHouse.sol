    ...
    Ownable
{
    ...
    // switch between set for sale and set not for sale
    function toggleForAuction(uint256 _tokenId, address _owner) public {
        // require that token should exist
        require(_exists(_tokenId));
        // get that token from all businesstwins mapping and create a memory of it defined as (struct => businesstwin)
        Businesstwin memory businesstwin = businesstwinNFTs[_tokenId];
        //check if token owner
        require(businesstwin.currentOwner == _owner);
        // if token's forSale is false make it true and vice versa
        if (businesstwin.forAuction) {
            businesstwin.forAuction = false;
        } else {
            businesstwin.forAuction = true;
        }
        // set and update that token in the mapping
        businesstwinNFTs[_tokenId] = businesstwin;
    }
