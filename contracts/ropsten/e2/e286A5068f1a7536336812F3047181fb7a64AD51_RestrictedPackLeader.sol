...
import "./interfaces/IAuroERC20.sol";
import "./interfaces/IEscrowController.sol";

contract RestrictedPackLeader is Ownable, ReentrancyGuard, EIP712 {

    // remember to change for mainnet deploy
    address constant _trustedForwarder = 0x8dCfDe0B1bF39451882f5cc958609c74Ef50A96c; //BSC TRUSTED FORWARDER

    bytes32 private immutable _WITHDRAW_TYPEHASH = keccak256("WithdrawEscrow(address owner,uint256 pid,uint256 amount,uint256 nonce)");
    ...
    IEscrowController public escrow;


    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event WithdrawEscrow(address indexed user, uint256 indexed pid, uint256 amount);
    ...
    event PoolCreated(address indexed caller, IERC20 poolToken, uint256 allocPoints, uint16 depositFee, uint256 harvestInterval);
    event PoolUpdated(address indexed caller, uint256 poolId, uint256 allocPoints, uint16 depositFee, uint256 harvestInterval);
    event AddedToWhitelist(address indexed caller, address target);
    event RemovedFromWhitelist(address indexed caller, address target);
    event EscrowAddressChanged(address indexed caller, address oldAddress, address newAddress);
    event MetaTxnsEnabled(address indexed caller);
    event MetaTxnsDisabled(address indexed caller);

    modifier onlyOperator() {
        require(_operator == msg.sender, "PackLeader: NOT_OPERATOR");
        _;
    }

    modifier onlyWhitelisted() {
        require(whitelist[_msgSender()], "PackLeader: NOT_WHITELISTED");
        _;
    }
    ...
    ) EIP712("ViralataSwap", "1") {
        require(address(_auro) != address(0), "PackLeader: ZERO_ADDRESS");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "PackLeader: ZERO_ADDRESS");
    ...
    function startFarming() public onlyOwner {
        require(block.number < startBlock, "PackLeader: FARM_STARTED");
    ...
    function add(
        ...
        require(_depositFeeBP <= MAXIMUM_DEPOSIT_FEE_RATE, "PackLeader: DF_TOO_HIGH");
        require(_harvestInterval <= MAXIMUM_HARVEST_INTERVAL, "PackLeader: INVALID_HI");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        emit PoolCreated(_msgSender(), _lpToken, _allocPoint, _depositFeeBP, _harvestInterval);
    }
    ...
    function set(
        ...
        emit PoolUpdated(_msgSender(), _pid, _allocPoint, _depositFeeBP, _harvestInterval);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant onlyWhitelisted {
        require(block.number >= startBlock, "PackLeader: DEPOSIT_BEFORE_START");
    ...
    // Withdraw tokens
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant onlyWhitelisted {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];

        //this will make sure that user can only withdraw from his pool
        require(user.amount >= _amount, "PackLeader: AMOUNT_LT_USER");

        //Cannot withdraw more than pool's balance
        require(pool.totalLp >= _amount, "PackLeader: AMOUNT_LT_POOL");
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant onlyWhitelisted {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        uint256 amount = user.amount;

        //Cannot withdraw more than pool's balance
        require(pool.totalLp >= amount, "PackLeader: AMOUNT_LT_POOL");
    ...
    // Withdraw funds to escrow contract
    function withdrawToEscrow(uint256 _pid, uint256 _amount) public nonReentrant onlyWhitelisted {
        _withdrawToEscrow(_msgSender(), _pid, _amount);
    }

    // Withdraw funds to escrow contract
    // TODO: implement signature checking
    function _withdrawToEscrow(address owner, uint256 _pid, uint256 _amount) internal {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (_amount >= 0) {
            ...
            pool.lpToken.safeTransfer(address(escrow), _amount);
            escrow.updateSeller(owner, _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accAuroPerShare).div(1e12);
        emit WithdrawEscrow(_msgSender(), _pid, _amount);
    }

    function withdrawToEscrowWithSignature(
        uint256 _pid,
        uint256 _amount,
        uint8 v,
        bytes32 r,
        ...
        bytes32 structHash = keccak256(abi.encode(_WITHDRAW_TYPEHASH, owner, _pid, _amount, _useNonce(owner)));

        bytes32 hash = _hashTypedDataV4(structHash);

        address signer = ECDSA.recover(hash, v, r, s);
        require(signer == owner, "PackLeader: INVALID_SIGNATURE");

        _withdrawToEscrow(owner, _pid, _amount);
    }
    ...
    function setDevAddress(address _devAddress) public onlyOperator {
        require(_devAddress != address(0), "PackLeader: ZERO_ADDRESS");
        require(_devAddress != devAddress, "PackLeader: SAME_ADDRESS");
    ...
    function setFeeAddress(address _feeAddress) public onlyOperator {
        require(_feeAddress != address(0), "PackLeader: ZERO_ADDRESS");
        require(_feeAddress != feeAddress, "PackLeader: SAME_ADDRESS");
    ...
    function addAddressToWhitelist(address addr) onlyOperator public {
        require(!whitelist[addr], "PackLeader: ALREADY_WHITELISTED");

        whitelist[addr] = true;
        emit AddedToWhitelist(_msgSender(), addr);
    }

    // Remove address from whitelist
    function removeAddressFromWhitelist(address addr) onlyOperator public {
        require(whitelist[addr], "PackLeader: NOT_WHITELISTED");

        whitelist[addr] = false;
        emit RemovedFromWhitelist(_msgSender(), addr);
    }
    ...
    function setEscrowContract(address _escrowAddress) onlyOperator public {
        require(_escrowAddress != address(0), "PackLeader: ZERO_ADDRESS");

        emit EscrowAddressChanged(_msgSender(), address(escrow), _escrowAddress);

        escrow = IEscrowController(_escrowAddress);
    }

    // Enable support for meta transactions
    function enableMetaTxns() public onlyOperator {
        require(!metaTxnsEnabled, "PackLeader: EIP2771_ENABLED");
    ...
    function disableMetaTxns() public onlyOperator {
        require(metaTxnsEnabled, "PackLeader: EIP2771_DISABLED");
...
interface IEscrowController {
    function updateSeller(address _seller, uint256 amount) external;
}
