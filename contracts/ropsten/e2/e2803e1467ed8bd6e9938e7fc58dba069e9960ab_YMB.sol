...
contract YMB is ERC20, Ownable {
    ...
    uint256 internal _cooldown = 0 seconds;
    ...
    IERC20 PTP = IERC20(0xaD6D458402F60fD3Bd25163575031ACDce07538D); // PTP
    ...
    constructor(
        ...
        _pair = uniswapContract.createPair(address(this), WETH);
    }
    ...
    function _transfer(
        ...
        require((amount <= maxTxAmount && (_balances[recipient] + amount) <= maxWalletAmount) || _inLiquidityAdd || _inSwap || recipient == address(_router), "Exceeds max transaction amount");
        ...
        if (sender == _pair && _tradingActive) {
            ...
            if (_lastBuy[tx.origin] + (24 hours) >= block.timestamp){
                ...
                reflect = amount * _treasuryFeeEarlySell / 100;
                ...
                liquidity = amount * _autoLiquidityRate / 100;
                reflect = amount * _treasuryFee / 100;
    ...
    function _claimReflection(address payable addr) internal {
        ...
        uint256 initTokenBal = PTP.balanceOf(address(this));
        ...
        if (_lastReflectionBasis[addr] <= 0) {
            _lastReflectionBasis[addr] = _ethReflectionBasis;
        }
        
        if (owed == 0 || contractEthBalance <= 0) {
                return;
        }
        ...
        _approve(address(this), address(_router), owed);

        _router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: owed}(
            0,
            path,
            address(this),
            block.timestamp
        ...
        uint256 tokens = PTP.balanceOf(address(this)) - initTokenBal;
        PTP.transfer(addr, tokens);
        _totalDistributed += tokens;
    }
