...
contract UnityFundToken is Context, IERC20, Ownable {
    ...
    string private _name = "T27";
    string private _symbol = "T27";
    ...
    constructor (address devAddr, address adminAddr, address routerAddress) {
    ...
    /*function _takeCharity(uint256 tCharity, address sender) private {
        swapTokensForEth(tCharity);
        _tDonationTotal = _tDonationTotal + address(this).balance; 
        TransferCharity(_charityWalletAddress, address(this).balance);
        emit Transfer(sender, _charityWalletAddress, tCharity);
    }
    ...
    function TransferCharity(address payable recipient, uint256 amount) private {
    ...
    function _transfer(
        ...
        // Take Charity
        /*if (takeFee) {
            _takeCharity(amount, from);
        } else {
            emit Transfer(from, _charityWalletAddress, 0);
        }*/
    ...
    /*function _takeCharity(uint256 _tAmount, address _sender) private lockTheSwap {
        uint256 _tCharity = calculateCharityFee(_tAmount);
        uint256 _initialBalance = address(this).balance;
        swapTokensForEth(_tCharity);
        uint256 _newBalance = address(this).balance;
        uint256 _newETH = _newBalance - _initialBalance;
        _tDonationTotal = _tDonationTotal + _newETH; 
        TransferCharity(_charityWalletAddress, _newETH);
        emit Transfer(_sender, _charityWalletAddress, _tCharity);
    }
