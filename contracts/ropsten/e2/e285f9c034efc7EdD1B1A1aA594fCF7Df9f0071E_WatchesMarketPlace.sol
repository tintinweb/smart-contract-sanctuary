...
contract WatchesTokenWhitelist is Pausable, Ownable {
    ...
    Ownable
{
    ...
    function getAllTokens() public view returns (RenderToken[] memory) {
        uint256 lastId = _tokenIdCounter.current();
        uint256 counter = 0;
    ...
    function getAllTokensOfOwner(address owner)
        public
        view
        returns (RenderToken[] memory)
    {
        ...
        uint256 counter = 0;
        RenderToken[] memory res = new RenderToken[](balance);
        for (uint256 i = 0; i < balance; i++) {
            uint256 tokenId = tokenOfOwnerByIndex(owner, i);
            string memory uri = tokenURI(tokenId);
            res[i] = RenderToken(tokenId, uri);
            counter++;
        }
        return res;
    }
