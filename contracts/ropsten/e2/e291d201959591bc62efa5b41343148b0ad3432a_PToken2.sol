...
// This software is a subject to Plysafe License Agreement.
// No use or distribution is allowed without written permission from Plysafe.
...
contract Ply2Enabled {
    Ply2 ply2;

    modifier onlyRole(bytes32 _role) {
        if (address(ply2) != 0x0 && ply2.hasRole(this, _role, msg.sender)) {
            _;
        }
    }

    // Perform only after claiming the node, or claim in the same tx.
    function setupPly2(Ply2 _ply2) returns(bool) {
        if (address(ply2) != 0x0) {
            return false;
        }
...
contract Ply2EnabledFull is Ply2Enabled {
    // Setup and claim atomically.
    function setupPly2(Ply2 _ply2) returns(bool) {
        if (address(ply2) != 0x0) {
            return false;
        }
        if (!_ply2.claimFor(this, msg.sender) && !_ply2.isOwner(this, msg.sender)) {
            return false;
        }
 ...
 * @title PToken2.
 *
 * The official Plysafe assets platform powering all kinds of tokens.
 * PToken2 uses EventsHistory contract to keep events, so that in case it needs to be redeployed
 ...
 * didn't happen yet.
 */
contract PToken2 is Ply2EnabledFull {
