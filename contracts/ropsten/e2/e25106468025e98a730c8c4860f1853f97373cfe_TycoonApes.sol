...
contract TycoonApes is ERC721, Pausable, Ownable {
    ...
    bool[] public allowedCollections = [true, false, false, false];
    ...
    function _collectionIndex(uint256 tokenId) internal pure token(tokenId) returns(uint collectionId) {
        if(tokenId < 10){
            return 0;
        } else if(tokenId < 100){
            return 1;
