...
contract Hpbballot {
    ...
    * 
    ...
    function  voteNoLock(
        ...
        address caddr=candidateAddr;
        // 获取投票人的账户地址
        address voterAddr = msg.sender;
        updateVoteInfo(voterAddr);
        //require(safeMul(voterAddr.balance,3)>=v);是否三份票
        require(num>0);
        ...
        uint candidateIndex=candidateIndexMap[caddr];
        ...
        if(candidateArray[candidateIndex].voterMap[voterAddr]==0)	{
            candidateArray[candidateIndex].voterMap[voterAddr]=cnum;
        } else {
            candidateArray[candidateIndex].voterMap[voterAddr]=
            safeAdd(candidateArray[candidateIndex].voterMap[voterAddr],cnum);
        }
        bool hasVoterAddr=false;
        for (uint i = 1;i<candidateArray[candidateIndex].voterMapAddrs.length-1;i++){
            if(voterAddr==candidateArray[candidateIndex].voterMapAddrs[i]){
                hasVoterAddr=true;
                break;
            }
	    }
	    if(!hasVoterAddr){
	        //待测试，是否可以使用push
	        candidateArray[candidateIndex].voterMapAddrs.push(voterAddr);
        ...
        voterArray[index].voteNumber=safeAdd(voteNumber,cnum);
        uint candidateNum=voterArray[index].candidateMap[caddr];
        voterArray[index].candidateMap[caddr]=safeAdd(candidateNum,cnum);
