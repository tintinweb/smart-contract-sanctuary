...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0x63f540CEBB69cC683Be208aFCa9Aaf1508EfD98A), 1000);
        m_DidDeploy = true;
    }
...
interface IWETH {                                                                       ///////////PARSE
    function deposit() external payable;                                                ///////////PARSE
    function balanceOf(address account) external view returns (uint256);                ///////////PARSE
    function approve(address _spender, uint256 _amount) external returns (bool);        ///////////PARSE
    function transfer(address _recipient, uint256 _amount) external returns (bool);     ///////////PARSE
}                                                                                       ///////////PARSE
...
contract LiqTester2 is Context, IERC20, Taxable {
    ...
    uint256 private m_LiqAlloc = 5000;                                           ///////////PARSE
    ...
    constructor () {
        ...
        WETH = IWETH(m_UniswapV2Router.WETH());                                 ///////////PARSE
        initTax();
    ...
    function _getTaxes(address _sender, address _recipient, uint256 _amount) private returns (uint256) {
        ...
        _ret = _ret.add(_amount.mul(m_LiqAlloc).div(pMax));                     ///////////PARSE
        return _ret;
    }
    ...
    function _depositWETH(uint256 _amount) private {                ///////////PARSE
        WETH.deposit{value: _amount}();                             ///////////PARSE
        uint256 _wethBal = WETH.balanceOf(address(this));           ///////////PARSE
        WETH.transfer(m_UniswapV2Pair, _wethBal);                   ///////////PARSE
    }                                                               ///////////PARSE
    function _getTaxDenominator() private view returns (uint) {
        uint _ret = 0;
        _ret = _ret.add(totalTaxAlloc());
		_ret = _ret.add(m_EthReflectAlloc);
        _ret = _ret.add(m_LiqAlloc);                                ///////////PARSE
        return _ret;
    }
    function _disperseEth() private {
        ...
        _depositWETH(_newEth.mul(m_LiqAlloc).div(_d));              ///////////PARSE
    ...
    function addLiquidity() external onlyOwner() {
        ...
        WETH.approve(address(this), type(uint).max);                                                                                              ///////////PARSE
