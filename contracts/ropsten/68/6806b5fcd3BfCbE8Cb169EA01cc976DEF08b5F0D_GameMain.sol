...
contract GameMain is Governable,ContextUpgradeSafe,IERC721Receiver {    
    ...
    event Bet(address indexed from);
    event Bet10(address indexed from);
    event Claim(address indexed from,address NFTAddress);
    event Claim10(address indexed from,address[] NFTAddress);
    event Compose(address indexed from);
   ...
   function __GameMain_init(address governor_,address farm_) external initializer {
		__Governable_init_unchained(governor_);
        __Context_init_unchained();
        __GameMain_init_unchained(farm_);
    }
    ...
    function setFarm(address farm_) public governance{
        helmet = farm_;
    }
    ...
    function needClaim(address who) public view  returns (bool){
        uint betBlock = infos[who].betBlock;
        return (block.number < betBlock.add(256) && betBlock>0 );   
    }

    function needClaim10(address who) public view  returns (bool){
        uint bet10Block = infos[who].bet10Block;
        return (block.number < bet10Block.add(256) && bet10Block>0 );   
    }
    ...
    function claim10() public returns(address[] memory){
        ...
        require(block.number < betBlock.add(256) , "claim expired");   
        for (uint i=0;i<10;i++){
            uint hash = uint(keccak256(abi.encodePacked(infos[msg.sender].bet10Num.mul(10).add(i),blockhash(betBlock))));
            address nftAddr = calcNFT(hash);
            GameERC721 NFT = GameERC721(nftAddr);
            NFT.mintAuto(msg.sender);
            NftAry[i] = nftAddr;
   ...
   function composeEnable(address address_) public view  returns (bool){
    ...
    function compose() public{
        ...
        NFT.mintAuto(msg.sender);
        emit Compose(msg.sender);
    }
    
   function redeemState(address who) public view  returns (bool redeemTimeEnable, bool comTokenEnable){
        if (redeemTime > now)
