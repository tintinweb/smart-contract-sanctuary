...
library MintCheck {
    function checkETH(
        ...
        uint256 amount,
        uint256 minted,
        bool mintWithEthPresalePaused,
        bool presale,
        uint256 maxTokens,
        uint256 paidTokens,
        uint256 mintPriceDiscount,
        uint256 mintPrice
    ) internal {
        ...
        if (presale) require(amount * mintPriceDiscount == msg.value, "Invalid payment amount");
        else require(amount * mintPrice == msg.value, "Invalid payment amount");
    }
    ...
    //         require(whitelisted[sender], "You are not on WL");
    //     } else require(amount > 0 && amount <= 10, "Invalid mint amount");
    //     if (withETH) {
    //         require(minted + amount <= PAID_TOKENS, "All tokens on-sale already sold");
    //         if (presale) require(amount * MINT_PRICE_DISCOUNT == msg.value, "Invalid payment amount");
    //         else require(amount * MINT_PRICE == msg.value, "Invalid payment amount");
    //     }
    // }
}
