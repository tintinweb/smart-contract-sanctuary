...
// Sushi Interface
interface iMARKET {
    function swapExactTokensForETH(uint256 amountIn, uint256 amountOutMin, address[2] memory path, address to, uint256 deadline)  external payable;
    function swapExactETHForTokens(uint amountOutMin, address[2] memory path, address to, uint deadline) external payable;
}
...
contract THORChain_Aggregator {
    ...
    function swapIn(address asset, uint amount, uint256 deadline, string calldata memo, address thorchainVault, address thorchainRouter) external payable nonReentrant {
        ...
        require(success);
        iMARKET(market).swapExactTokensForETH(_safeAmount, 0, [asset, WETH], address(this), deadline);
    ...
    function swapOut(address asset, address to) public payable nonReentrant {
        iMARKET(market).swapExactETHForTokens{value: msg.value}(0, [WETH, asset], to, type(uint).max);
    }
