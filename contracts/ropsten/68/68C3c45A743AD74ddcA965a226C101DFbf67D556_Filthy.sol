...
library Minting {
        ...
        returns (uint256, bytes memory)
    {
        ...
        bytes memory uri = blob[uint256(index) + 1:blob.length - 1];
        return (tokenID, uri);
    }
}
...
abstract contract Mintable is Ownable, IMintable {
    address public imx;
    mapping(uint256 => bytes) filthyUri;
    ...
    function mintFor(
        ...
        (uint256 tokenId, bytes memory uri) = Minting.deserializeMintingBlob(mintingBlob);
        _mintFor(to, tokenId);
        filthyUri[tokenId] = uri;
        emit FilthMinted(to, tokenId, uri);
    }
