...
contract BuilderShop {
   ...
   //nBoss registry is hard coded
   address nBossRegistryContract = 0x9BF6b068667e07f98A0482B56ABA2be6eeCA30d5;
   ...
   function isValidBuilderShop(address builder_shop) public view returns (bool isValid) {
       //public function, allowing anyone to check if a contract address is a valid nBoss contract
...
contract NBossBuilderInstance is ERC721Full {
   ...
   modifier onlyValidSender() {
       NBossRegistry nftg_registry = NBossRegistry(nBossRegistryContract);
       bool is_valid = nftg_registry.isValidNBossSender(msg.sender);
...
//   string public creatorName;
   ...
   //nBoss registry contract
   address public nBossRegistryContract = 0x9BF6b068667e07f98A0482B56ABA2be6eeCA30d5;

   //master builder - ONLY DOES STATIC CALLS
   address public masterBuilderContract = 0xa2284985c553C05cD89C01167bf38E45817E6B4a;
   ...
   mapping (uint => Counters.Counter) public _numNBossMinted;
   mapping (uint => uint) public _numNBossPermitted;
   mapping (uint => uint) public _nBossPrice;
   mapping (uint => string) public _nBossIPFSHashes;
   mapping (uint => bool) public _IPFSHashHasBeenSet;

   //EVENTS

   //purchase + creation events
   event NBossPurchased(address _buyer, uint256 _amount, uint _tokenId);
   event NBossCreated(address new_owner, uint _nBossType, uint _tokenId);
   ...
   constructor(
       ...
       //offset starts at 1 - there is no nBossType of 0
       for (uint i=0; i<(num_nifties); i++) {
           _numNBossPermitted[i+1] = nBoss_quantities[i];
       }
   }
   
   function setNBossIPFSHash(uint nBossType, 
                            string memory ipfs_hash) onlyValidSender public {
        //can only be set once
        if (_IPFSHashHasBeenSet[nBossType] == true) {
            revert("Can only be set once");
        } else {
            _nBossIPFSHashes[nBossType] = ipfs_hash;
            _IPFSHashHasBeenSet[nBossType]  = true;
        }
    }

   function isNBossSoldOut(uint nBossType) public view returns (bool) {
       if (nBossType > numNiftiesCurrentlyInContract) {
           return true;
       }
       if (_numNBossMinted[nBossType].current() > _numNBossPermitted[nBossType]) {
           return (true);
       } else {
           return (false);
       }
   ...
   function giftNBoss(address collector_address, 
                      uint nBossType) onlyValidSender public {
       //master for static calls
       BuilderMaster bm = BuilderMaster(masterBuilderContract);
       _numNBossMinted[nBossType].increment();
       //check if this nBoss is sold out
       if (isNBossSoldOut(nBossType)==true) {
           revert("NBoss sold out!");
       }
       //mint a nBoss
       uint specificTokenId = _numNBossMinted[nBossType].current();
       uint tokenId = bm.encodeTokenId(contractId, nBossType, specificTokenId);
       string memory tokenIdStr = bm.uint2str(tokenId);
       string memory tokenURI = bm.strConcat(baseURI, tokenIdStr);
       string memory ipfsHash = _nBossIPFSHashes[nBossType];
       //mint token
       _mint(collector_address, tokenId);
       _setTokenURI(tokenId, tokenURI);
       _setTokenIPFSHash(tokenId, ipfsHash);
       //do events
       emit NBossCreated(collector_address, nBossType, tokenId);
   }
...
contract BuilderMaster {
   ...
   function getNBossTypeId(uint tokenId) public view returns (uint);
   function getSpecificNBossNum(uint tokenId) public view returns (uint);
   function encodeTokenId(uint contractId, uint nBossType, uint specificNBossNum) public view returns (uint);
...
/**
* Contracts and libraries below are from OpenZeppelin, except nBoss builder instance
**/
