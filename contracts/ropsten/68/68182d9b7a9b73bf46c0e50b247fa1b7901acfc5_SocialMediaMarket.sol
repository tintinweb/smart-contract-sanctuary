 ...
 * Author: Mikhail Vladimirov <<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="f39e9a989b929a9fdd859f92979a9e9a819c85b3949e929a9fdd909c9e">[email&#160;protected]</a>>
 */
 ...
 * Author: Mikhail Vladimirov <<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="6e030705060f07024018020f0a0703071c01182e09030f0702400d0103">[email&#160;protected]</a>>
 */
pragma solidity ^0.4.20;
...
contract SocialMediaMarket is Ownable {
    ...
    function initiateEscrow(uint64 id, uint256 amount, uint256 fee_amount, address adv_address, address inf_address) onlyOwner public returns (bool success) {
        if (items[id].amount > 0) {
            return false;
        }
        uint256 approveAmount = amount.add(fee_amount);
        if (_token.allowance(adv_address, address(this)) < approveAmount) {
            return false;
        }
        if (fee_amount == 0) {
            return false;
    ...
    function updateEscrow(uint64 id, uint256 amount) onlyOwner public returns (bool success){
        if (items[id].amount > 0) {
            if (amount != items[id].amount) {
                uint256 currentAmount = amount.add(items[id].fee_amount);
                if (currentAmount <= items[id].amountTransferred) {
                    items[id].amount = amount;
                }
                else {
                    uint256 approveAmount = currentAmount - items[id].amountTransferred;
                    if (_token.allowance(items[id].adv_address, address(this)) >= approveAmount) {
                        items[id].amount = amount;
    ...
    function withdraw(uint64 id, address[] addresses, uint256 percentBack) onlyOwner public returns (bool success) {
        if (items[id].amount > 0) {
            if (items[id].status == 0) {
                if (percentBack <= items[id].percent) {
                    ...
                    if (addresses.length > 0) {
                        if (items[id].fee_amount / addresses.length == 0) {
                            return false;
                        }
                        for (currentFee = addresses.length - 1; currentFee >= 0; currentFee -= 1) {
                            require(_token.transfer(addresses[currentFee], items[id].fee_amount / addresses.length));
                        }
    ...
    function payback(uint64 id, address[] addresses) onlyOwner public returns (bool success) {
        if (items[id].amount > 0) {
            if (items[id].status == 0) {
    ...
    function changePercent(uint256 percentPayout) onlyOwner public returns (bool success) {
        if (percentPayout == percent) {
            return false;
        }
        else if (percentPayout == 0 || percentPayout >= 100) {
            return false;
        }
        percent = percentPayout;
        return true;
