...
contract ProductHub is IProductHub, Ownable {
    ...
    mapping(uint256 => uint256) /*tokenId*/ /*timestamp*/
        internal _tokenSubscriptionStartTimes;
    mapping(uint256 => uint256) /*tokenId*/ /*timestamp*/
        ...
        ) 
    {
        ...
        uint[]    memory subscriptionStartTime = new uint[](tokens.length());
        uint[]    memory paymentAmounts = new uint[](tokens.length());
        uint[]    memory payedAmounts = new uint[](tokens.length());

        for (uint i = 0; i < tokens.length(); i++) {
            uint token = tokens.at(i);
            addrs[i] = nft.ownerOf(token);
            subscriptionStartTime[i] = _tokenSubscriptionStartTimes[token];
            lastPeriodStartTime[i] = _tokenLastPeriodStartTimes[token];
            paymentAmounts[i] = product.price * ((block.timestamp - lastPeriodStartTime[i]) / product.period);
            payedAmounts[i] = (product.price * ((block.timestamp - subscriptionStartTime[i]) / product.period)) - paymentAmounts[i];
        }

        return (addrs, lastPeriodStartTime, subscriptionStartTime, paymentAmounts, payedAmounts);
    }
    ...
    function subscribe(string memory productId) external override {
        ...
        _tokenSubscriptionStartTimes[tokenId] = block.timestamp;
...
interface IProductHub {
    ...
    function getProductSubscribers(string memory productId) external view returns (
        address[] memory,
        uint[] memory,
        uint[] memory,
        uint[] memory,
