...
contract claimToken is Verify, Oracled{
    ...
    //         chainId := mload(add(add(sigData, 0x1), 36))
    //         toAddress := mload(add(add(sigData, 0x14), 37))
    //         //TODO - @Fuzzy add correct bytecode pos to load from 0x?
    //         // tokenAddress := mload(add(add(sigData, 0x14), 51))
    //     }
    //     td.id = Endian.reverse64(id);
    //     td.ts = Endian.reverse32(ts);
    //     td.fromAddr = Endian.reverse64(fromAddr);
    ...
    // address tokenAddress
    {
        ...
        // return (td.id ,td.toAddress, td.tokenAddress, td.quantity);
        // if (_tokenAddress == address(weth)) {
        //     pegOutWeth(payable(_tokenRecipient), _tokenAmount, "");
        // } else {
        //     IERC20(_tokenAddress).safeTransfer(_tokenRecipient, _tokenAmount);
...
// contract instantiateClaim is claimToken {
//     function verifySigDataFrom(bytes memory sigData)
//         private
//         returns (TeleportData memory)
//     {
//         return(this.verifySigData(sigData));
//     }
// }
