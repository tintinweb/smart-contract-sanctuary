...
contract ShitcoinRoulette is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                _cooldown[to] = block.timestamp + (60 seconds);
            }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        uint256 team = amount.mul(_teamFee).div(100);
        uint256 jackpot = amount.mul(_jackpotFee).div(100);
        _transferStandard(sender, _FeeAddress, team);
        _transferStandard(sender, address(this), jackpot);
        _transferStandard(sender, recipient, amount.sub(team).sub(jackpot));
    }

    function _transferStandard(address _from, address _to, uint256 _value) private
        returns (bool)
    {
