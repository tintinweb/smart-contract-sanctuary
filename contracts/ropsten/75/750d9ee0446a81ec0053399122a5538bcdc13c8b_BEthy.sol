...
contract Game is ExchangeAdmin {
    ...
    bool expired;
    bool verified; //to add a game it must be verifiedby admin
    ...
    constructor (string gameName, uint gameExpiry, bytes32[] gameSides) public {
        ...
        else{verified = false;} //games are by default not verified
        allSidesPledged = 0;
    }
    ...
    function isNotExpired() view public returns (bool) {
        return ((now < expiry) && !expired && verified == true); //to not be expired game must also be verified
    }
    ...
    function verifyGame(bool valid) public onlyAdmin{ //function to allow admin to verify a game at given ID
      verified = valid;
    }
...
contract BEthy is ExchangeAdmin {
    ...
    function verifyGame(uint i, bool valid) public onlyAdmin{ //function to allow admin to verify a game at given ID
