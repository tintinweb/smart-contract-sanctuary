...
contract ChubbiFren is ChubbiAuction {
    ...
    ) ChubbiAuction(_name, _symbol, _proxyRegistryAddress, 8888, 1805) {
...
contract ChubbiAuction is ChubbiReserve {
    ...
    function getCurrentPrice() public view returns (uint256) {
        ...
        return _getCurrentPrice(startBlock, block.number, maxPrice, minPrice);
    }

    /**
     * @dev Get the current price of a token.
     * We make this virtual so we can override it in tests.
     * @param _startBlock the starting block number.
     * @param _currentBlock the current block number.
    ...
    function _getCurrentPrice(
        ...
        // Every 20 blocks reduce the price by 0.05 ether
        uint256 priceDiff = _currentBlock.sub(_startBlock).div(20).mul(
            0.05 ether
        );
...
contract ChubbiReserve is ChubbiBase {
    ...
    event Reserved(address indexed to, uint256 allocation);
    ...
    constructor(
        ...
        require(
            ...
            "Reserved should be less than the supply"
        );
        reserved = _reserved;
        claimed = 0;
        allocated = 0;
    ...
    /**
     * @dev Get the allocations for a given address.
     * @param _for address of the owner of the tokens.
     */
    function getAllocations(address _for) external view returns (uint256) {
        return allocations[_for];
    }
     ...
     * @param _amount amount of tokens to reserve.
     */
    function reserve(address _to, uint256 _amount)
        public
        onlyOwner
        whenNotPaused
    {
        ...
        uint256 oldAmount = allocations[_to];
        uint256 newAllocation = allocated.sub(oldAmount).add(_amount);
        uint256 maxAllocation = reserved.sub(claimed);
        require(newAllocation <= maxAllocation, "Cannot allocate more tokens");

        allocated = newAllocation;
        allocations[_to] = _amount;

        emit Reserved(_to, _amount);
    }

    /**
     * @dev Reserve tokens for many addresses.
     * _amonuts[i] will be reserved to _addresses[i]
     *
     * @param _addresses an array of addresses.
     * @param _amounts an array of reservation amounts.
     */
    function reserveMany(address[] memory _addresses, uint256[] memory _amounts)
        external
        onlyOwner
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            reserve(_addresses[i], _amounts[i]);
        }
    }
    ...
    function claim() external whenNotPaused {
        ...
        uint256 numberOfTokens = allocations[msg.sender];
        delete allocations[msg.sender];
        claimed = claimed.add(numberOfTokens);
        allocated = allocated.sub(numberOfTokens);
