...
contract Party {
    ...
    function placeHolder(SwapWithoutRSV[] memory tokenData) internal isAlive returns (uint256) {
        uint256 fee = 0;
        for (uint256 index = 0; index < tokenData.length; index++) {
            fee = (tokenData[index].buyAmount * 5) / 1000;
            uint256 outAmount = tokenData[index].sellAmount;
    ...
    function closeParty(
        ...
        for (uint256 index = 0; index < weightArray.length; index++) {
            ...
            emit LeavePartyEvent(weightArray[index].weightAddress, weightArray[index].weightPercentage, address(this), value, fee, 0);
        }
