...
/*
Auction interface that implements functionality to auction an NFT from the 
parent Collection.sol class.
*/
interface IAuction {
  ...
  function setWinner(address payable newWinner) external;

  function setCurrentPrice(uint newPrice) external;
...
/*
DutchAuction implementation of IAuction.sol
*/
contract DutchAuction is IAuction {
  enum State {Active, Cancelled, Resolved, Flushed}
  ...
  // Initialises auction with required parameters. 
  // Ensures that the auction creator is recorded
  constructor(
  ...
  // Ensures that any call to 'calculatePrice' is done whilst auction is active
  ...
  function getOwner() external view override returns(address payable) {
    return owner;
  }
  
  function getWinner() external view override returns(address payable) {
    return winner;
  }
  
  function setWinner(address payable newWinner) external override {
    winner = newWinner;  
  }
  
  // Calculates the current auction price based on parameters and crnt block
  function calculatePrice(uint crntBlock) private correctTime returns(uint) {
    while(crntBlock > startBlock +dropInterval) {
  ...
  function getCurrentPrice() external correctTime override returns(uint) {
    return calculatePrice(block.number);
  }

  function setCurrentPrice(uint newPrice) external override {}
  
  // Owner cancells auction
  function cancel() public override {
    require(msg.sender == owner);
    auctionState = State.Cancelled;
  }
  
  // When auction owner can receive funds
  function finalise() public override {
    auctionState = State.Resolved;
  }
    
  // When the auction is flushed, it cannot be used
  function flush() public override {
    auctionState = State.Flushed;
  }  
}
...
contract EnglishAuction is IAuction {
  ...
  // Initialises auction with required parameters. Ensures that the auction 
  // creator is recorded
  constructor (
    address payable creator, 
    uint numWeeks, 
  ...
  function setWinner(address payable newWinner) external override {
    highestBidder = newWinner;  
  }
  
  function getCurrentPrice() external view correctTime override returns(uint) {
    return highestBindingBid;
  }
  
  function setCurrentPrice(uint newPrice) external override {
    highestBindingBid = newPrice;  
  }
  ...
  // Marks the auction as flushed; it can no longer be used
  function flush() public override {
    auctionState = State.Flushed;
  }
}
