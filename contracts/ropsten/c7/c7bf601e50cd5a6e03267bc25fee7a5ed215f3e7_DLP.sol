...
contract DLP is Ownable {
    ...
    event ClaimedRef(address indexed to,uint256 tokenAmount);
    event WdTicket(address indexed to,uint256 tokenAmount);
    event Buy(address indexed to,uint256 buyTicket, uint256 totalTicket);
    ...
    function buy(uint256 _tickets, uint256 refId) external payable {
        ...
        devFeePending += fee;
        uint256 total = currentTicketAmount + _tickets;
        emit Buy(msg.sender, _tickets, total);
    }
    
    function getRefId(address referrer) external view returns(uint256){
        return dLRNFT.getRefId(referrer);
    }
    ...
    function claimRef() external {
        ...
        if(amount>0){
            ...
            emit ClaimedRef(msg.sender,amount);
        }
    }
