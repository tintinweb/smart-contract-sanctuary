...
contract NFTTest is ERC721, Ownable {
    ...
    function mintForWhiteListed(uint8 numberToMint) external payable {
        require(isWhiteListSaleActive, "WhiteList sale is not active");
        require(numberToMint <= _whiteList[msg.sender], "Exceeded max available to purchase");
        require(TOTAL_NFT_MINTED + numberToMint <= MAX_SUPPLY, "Purchase would exceed max tokens");
        require(msg.value >= MINT_COST * numberToMint, "Not enough ether sent");
    ...
    function maxMintAllowedPerWallet() public view returns (uint256) {
