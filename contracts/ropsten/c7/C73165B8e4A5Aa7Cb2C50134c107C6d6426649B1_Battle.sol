...
contract Battle is Ownable {
    ...
    struct RoundInfo {
        ...
        uint range;
        RangeType ry;
        uint targetPriceUnder;
        uint targetPriceSuper;
    ...
    constructor(IERC20 _collateral, IOracle _oracle, string memory _trackName) {
    ...
    function init(uint amount, uint _spearPrice, uint _shieldPrice, uint _range, RangeType _ry, uint _startTS, uint _endTS) external {
        ...
        uint price = oracle.getPrice("BTC");
    ...
    function buySpear(uint amount) external {
        ...
        if(isBigger) {
            ...
            collateralShield[currentRoundId] = spearBalanceOf[currentRoundId][address(this)].sub(collateralSpear[currentRoundId]).divideDecimal(spearBalanceOf[currentRoundId][address(this)].multiplyDecimal(shieldBalanceOf[currentRoundId][address(this)]));
        }
        spearBalanceOf[currentRoundId][address(this)] = spearBalanceOf[currentRoundId][address(this)].sub(spearOut);
    ...
    function buyShield(uint amount) external {
        ...
        if(isBigger) {
            ...
            collateralSpear[currentRoundId] = shieldBalanceOf[currentRoundId][address(this)].sub(collateralShield[currentRoundId]).divideDecimal(shieldBalanceOf[currentRoundId][address(this)].multiplyDecimal(spearBalanceOf[currentRoundId][address(this)]));
        }
        shieldBalanceOf[currentRoundId][address(this)] = shieldBalanceOf[currentRoundId][address(this)].sub(shieldOut);
    ...
    function removeLiquility(uint amount) public  {
        ...
        uint dletaSpear = deltaCollateral.multiplyDecimal(collateralSpear[currentRoundId]).divideDecimal(lpTotalSupply[currentRoundId]);
        uint dletaShield = deltaCollateral.multiplyDecimal(collateralShield[currentRoundId]).divideDecimal(lpTotalSupply[currentRoundId]);
        ...
        spearBalanceOf[currentRoundId][address(this)] = spearBalanceOf[currentRoundId][address(this)].sub(dletaSpear);
        shieldBalanceOf[currentRoundId][address(this)] = shieldBalanceOf[currentRoundId][address(this)].sub(dletaShield);
...
contract DToken is ERC20, IDToken {

    address public admin;
    constructor(string memory name, string memory symbol, address _admin, address account, uint amount) ERC20(name, symbol) {
       admin = _admin;
       _mint(account, amount);
    }

    function mint(address account, uint amount) external override onlyAdmin {
        _mint(account, amount);
    }

    function burn(address account, uint amount) external override onlyAdmin {
...
interface IOracle {
   function getPrice(string memory symbol) external returns(uint); 
}
