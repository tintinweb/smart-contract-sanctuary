...
contract Economi is ERC721 {
  ...
  mapping (uint256 => uint256) public basicNote;
  ...
  function generateBasicNote(uint256 _value, uint256 _mintPrice) public {
    require(msg.sender == owner, "Only the owner can generate notes.");
    basicNote[_value] = _mintPrice;
  } 

  function mintNote(uint256 _value) public payable {
    require(basicNote[_value] > 0, "Note does not exist.");
    uint256 supply = totalSupply().add(1);
    Note storage note = notes[supply];
  ...
  function executeTrade(uint256 _id) public payable {
    Note storage note = notes[_id];
    require(note.forSale, "Note not for sale.");
    address payable _owner = address(uint160(note.owner));
    _owner.transfer(note.price);
  ...
  function cancelTrade(uint256 _id) public {
    Note storage note = notes[_id];
    require(msg.sender == note.owner, "Cannot cancel trade. You do not own the item");
    require(!note.forSale, "Cannot cancel trade that is not open.");
    note.forSale = false;
    transferFrom(address(this), note.owner, _id);
  }
  // -------------------------
}