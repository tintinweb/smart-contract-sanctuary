...
contract KolectivToken is ERC721, ERC721Pausable, Ownable {
    ...
    address private proxyMinter; // only account that can call mintFor()
    ...
    function parseMintingBlob(bytes memory mintingBlob, uint256 amount) internal pure returns (uint256[] memory, bytes32[] memory) {
        ...
        bool isTokenId = true;
        for (uint i = 1; i < mintingBlob.length; i++) {
            if (isTokenId) {
                (uint256 tokenId, uint256 nextItem) = parseMintingBlobTokenId(mintingBlob, i);
                tokenIds[offset] = tokenId;
                isTokenId = false;
                i = nextItem;
            } else {
                (bytes32 uuid, uint256 nextItem) = parseMintingBlobUUID(mintingBlob, i);
                uuids[offset] = uuid;
                isTokenId = true;
                offset++;
                i = nextItem;
    ...
    function parseMintingBlobUUID(bytes memory mintingBlob, uint start) internal pure returns (bytes32, uint256) {
        ...
        for (uint i = start; i < mintingBlob.length; i++) {
            if (mintingBlob[i] == "}") {
                ...
                return (result, i + 1); // i + 1 since we want to skip over the closing bracket
            }
            if (mintingBlob[i] == "-") {
                continue;
            }
