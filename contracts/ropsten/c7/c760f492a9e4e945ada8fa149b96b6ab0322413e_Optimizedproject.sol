...
contract Optimizedproject {
    ...
    function reqvehreg(address _hash,string _aadhar,string _vin,string _numberplate, string _licensenumber)public{
        require(_hash==msg.sender);
        reqvehregcount = reqvehregcount +1;
        userdetails[_hash].vehreqnums.push(reqvehregcount);
    ...
    function vehicleregistration(address _hash,string _vin,string _numberplate,string _licensenumber)public returns(bool){
        require(_hash == msg.sender);
        require(userdetails[_hash].loginstatus==true);
    ...
    function displayvehicledetails(address _hash, uint256 _number)public view returns(uint256,string,string,string){
        require(_hash == msg.sender);
        require(userdetails[_hash].loginstatus==true);
        require(vehicleFound(_hash,_number));
        return(_number,uservehicles[_hash][_number].vin,uservehicles[_hash][_number].numberplate,uservehicles[_hash][_number].licensenumber);
    }
    ...
    function request(uint256 _departmentNo,address _from, address _to,bool _variety)public validmember(_departmentNo,_from) {
        ...
        if(checkapproval(_departmentNo,department[_departmentNo].request[i].member,_variety)){
            department[_departmentNo].request[i].status=true;
            if(_variety){
    ...
    function respond(uint256 number, address _from, address _to,bool _variety,bool _decision)public validmember(number,_from) returns(bool){
        ...
        if(checkapproval(number,_to,_variety)){
            department[number].request[index].status=true;
            if(_variety){
    ...
    function departmentrequest(address hash,string _name,bool _opinion)public validmember(1,hash){
        ...
        if(deptcheckapproval(_name)){
            reqdepartment[depreqcount].status= true;
            departmentscount = departmentscount +1;
    ...
    function deptcheckrespond(address hash)public validmember(1,hash) view returns(uint256,uint256,address,string){
        ...
        if(i<depreqcount){
            return(i+1,depreqcount,reqdepartment[i+1].member,reqdepartment[i+1].name);
        }else{
            return(0,0,0x0,"");
        }
    }
    
    function deptrespond(address _from,address _to,string _name,bool _opinion)public validmember(1,_from)returns(bool){
        ...
        department[1].departmentmember[pos].deptdecision[index].opinion = _opinion;
        if(deptcheckapproval(_name)){
            reqdepartment[index].status= true;
            departmentscount = departmentscount +1;
            department[departmentscount].name = _name;
            department[departmentscount].memberscount = 1;
            department[departmentscount].presentmemberscount = 1;
    ...
    function getdeptmembershipstatus(uint256 _departmentNo,address _hash)public view returns(bool){
        if((_hash==msg.sender)&&(userdetails[_hash].loginstatus==true)&&(exist(_departmentNo,_hash))){
            return true;
        }else{
            return false;
        }
    ...
    function vehreqexist(uint256 num,address _hash)public view returns(bool){
        require(msg.sender==_hash);
        uint256 count=userdetails[_hash].vehreqnums.length;
        uint256 i;
        for(i=1;i<=count;i++){
            if(userdetails[_hash].vehreqnums[i]==num){
                return true;
            }
        }return false;
    }
    
    function getvehreqdetails(uint256 num,address _hash)public view returns(string,bool){
        require(_hash==msg.sender);
        require(vehreqexist(num,_hash));
        return(vehiclereq[num].vin,vehiclereq[num].status);
    }
    
    function getdeptreqscount()public view returns(uint256){
        return depreqcount;
    }
    
    function getdeptreqdetails(uint256 i,address _hash)public validmember(1,_hash) view returns(address,string,bool){
        require((i>=1)&&(i<=depreqcount));
        return (reqdepartment[i].member,reqdepartment[i].name,reqdepartment[i].status);
    }
    
    function getmemreqscount(uint256 dno)public view returns(uint256){
        return (department[dno].requestscount);
    }
    
    function getmemreqdetails(uint256 dno,uint256 i)public validmember(dno,msg.sender) view returns(address,bool,bool){
        require((i>=1)&&(i<=department[dno].requestscount));
        return (department[dno].request[i].member,department[dno].request[i].variety,department[dno].request[i].status);
    }
}