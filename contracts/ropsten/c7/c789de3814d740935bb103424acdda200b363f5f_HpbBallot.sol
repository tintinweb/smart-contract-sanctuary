   ...
   */
   ...
   /**
	...
	function cancelVoteForCandidateByStage(
        ...
        if(voteStageIndex==0){
            require(_voteStageBlock==voteStages[0].blockNumber);
        }
    ...
    function fechAllCandidatesByStage(
        ...
        for(uint i=0;i<voteStages[voteStageIndex].candidateArray.length;i++){
            ...
            _names[i]=stringToBytes32(voteStages[voteStageIndex].candidateArray[i].name);
            _facilityIds[i]=stringToBytes32(voteStages[voteStageIndex].candidateArray[i].facilityId);
        }
    ...
    /**
     * 在指定的区块号获取所有候选人的投票奖励情况
     */
    function fechAllSumNumForEncourage(
        uint _block
    ) onlyVoteAfterStart public constant returns (
        address[] addr,
        uint[] nums
    ){
        uint _stageIndex=voteStageIndexMap[currentVoteStageBlock];
        require(_stageIndex>=1);
        if(_block<=currentVoteStageBlock){
            require(_stageIndex>=2);
            require(voteStages[_stageIndex-2].blockNumber<_block);
            return fechAllSumNumForCandidateByStage(voteStages[_stageIndex-2].blockNumber);
        }else{
            return fechAllSumNumForCandidateByStage(voteStages[_stageIndex-1].blockNumber);
        }
    }
    /**
     * 获取指定阶段每个候选人的累计投票情况
