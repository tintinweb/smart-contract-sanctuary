...
contract dReal is Ownable {
    ...
    uint256 public tokenPrice = 5000; //Price in euros for 10,000 tokens. Initial price: 1 REAL = 0.5 EUR
    
    uint256 public platformFee = 49; //Equivalent to 0.49%
    ...
    function createRent(uint256 _price, uint256 _collateral, uint256 _repetitions, string memory _street, string memory _description, uint256 _surface, bool _isApartment) public {
    ...
    function acceptRent(uint256 _id) public {
        ...
        uint256 fees = calculateFees(offer.price);
        uint256 total = offer.collateral + offer.price + fees;

        require(token.transferFrom(msg.sender, address(this), total), 'Could not transfer tokens');
        require(token.transfer(offer.landlord, offer.price), 'Could not transfer tokens');
    ...
    function payRent(uint256 _id) public {
        ...
        require(token.transferFrom(msg.sender, address(this), offer.price + fees), 'Could not transfer tokens');
        require(token.transfer(offer.landlord, offer.price), 'Could not transfer tokens');
    ...
    function calculateFees(uint256 _total) internal view returns(uint256) {
        uint256 totalToTake = _total * platformFee / 10000;
        return totalToTake;
    }
    ...
    function withdrawFees() public onlyOwner {
        ...
        totalFees = 0;
        require(token.transfer(msg.sender, total), 'Could not transfer tokens');
    }
