...
contract Celan {
    ...
    event ReferalCommission(address indexed to, uint level, uint value,uint time);
    ...
    event PoolPerson(address indexed to, uint amount,uint time);
    // Registration event
    event Registration(address indexed from, address indexed to, uint level, uint value,
    uint time, address indexed directrefer);
    // Buy level event
     ...
     */
    function register(uint _referid, uint _level) public payable isLock isContractCheck(msg.sender) {
        ...
        address invitor = userList[_referid];
        ...
        _userRegister(msg.sender, _referid, _level, msg.value, directAddress,invitor);
    }

    function _userRegister(address _user, uint _referid, uint _level, uint _amount, address _directAddress,address _invitor) internal {
        UserStruct memory userstruct;
        userstruct = UserStruct({
            isExist: true,
            id: currentId,
        ...
        require(address(uint160(_invitor)).send(directBonus), "Celan: Direct bonus failed");
        uint amount = _amount.sub(poolPercentage.add(directBonus));
        _paylevel(userList[_referid], _level, amount.div(referalUpline[_level]));
        emit DirectReferCommission(_user,_invitor, directBonus,block.timestamp);
    ...
    function _directShare(address _user, address _refer, uint _level, uint _amount) internal {
        if (users[_refer].activeLevel[_level] != true) {
            ...
            emit DirectReferCommission(_user, _refer, _amount,block.timestamp);
        }
    }
    ...
    function _paylevel(address _referer, uint _level, uint _amount) internal {
        ...
        if (loopCheck[msg.sender] < referalUpline[_level]) {
            ...
            emit ReferalCommission(_referer, _level, _amount,block.timestamp);
    ...
    function poolPercent() public onlyOwner {
        ...
        if (poolMembers.length > 0) {
            for (uint i = 0; i < poolMembers.length; i++) {
                if (poolStatus[poolMembers[i]] == false && poolMembers[i] != address(0)) {
                    require(address(uint160(poolMembers[i])).send(poolBonus.div(poolMembers.length)),
                    "Celan: Pool transaction failed");
                    poolStatus[poolMembers[i]] = true;
                    emit PoolPerson(poolMembers[i], poolBonus.div(poolMembers.length),block.timestamp);
                }
            }
            poolBonus = 0;
        }
    ...
    /**
     * @dev updatePrice: Update level price function invokes by owner
     */
    function updatePrice(uint _level,uint _amount) public onlyOwner {
       levelPrice[_level] = _amount;
    }
