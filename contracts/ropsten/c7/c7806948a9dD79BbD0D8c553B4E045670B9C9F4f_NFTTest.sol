...
contract Ticket is ERC721Burnable, ERC721Enumerable, RandomlyAssigned, WithContractMetaData, WithIPFSMetaData {
    using SafeMath for uint256;

    event MintedTickets(uint256 indexed amount);

    constructor(uint256 maxElements_, string memory contractUri_, string memory cid_)
    ERC721("Ticket", "T")
    WithContractMetaData(contractUri_)
    WithIPFSMetaData(cid_)
    RandomlyAssigned(maxElements_, 0) // Max. 1k NFTs available; Start counting from 0
    {
    }

    function mintTickets(uint256 amount) external onlyOwner() {
        for (uint256 i = 0; i < amount; i++) {
            uint id = nextToken();
            _safeMint(_msgSender(), id);
        }
        emit MintedTickets(amount);
    }
