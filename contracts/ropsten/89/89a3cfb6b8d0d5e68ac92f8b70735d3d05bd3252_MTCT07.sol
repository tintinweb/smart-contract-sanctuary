...
contract MTCT07 is StandardToken, Pausable, Freezable {
        using SafeMath for uint256;

        string  public  name = "MTCT07";
        string  public  symbol = "MTCT07";
        ...
        function multiTransfer(address[] memory _recipients, uint256[] memory _values)
                ...
                returns (bool[] memory)
        {
                uint256 _sumOfTransfer = 0;
                bool[] memory result = new bool[](_recipients.length);

                for (uint256 i = 0; i < _recipients.length; i++) {
                    result[i] = super.transfer(_recipients[i], _values[i]);
                    _sumOfTransfer += _values[i];
                }
                emit MultiTransfer(_recipients, _values, _sumOfTransfer, result);
                return result;
        }
