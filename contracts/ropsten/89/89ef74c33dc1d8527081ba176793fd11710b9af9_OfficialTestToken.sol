...
contract DeflationaryERC20 is Context, IERC20, Ownable {
    ...
    // set pool address, enforce deflationary features
    function setUniswapPair(address pairAddress) external onlyOwner {
        require(uniswapPair == address(0), "Pool: Address immutable once set");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
		...
		if (inSwapAndLiquify || isExcludedFromFee(sender) || isExcludedFromFee(recipient) || uniswapPair == address(0)) //feeless transfer before pool initialization and in liquify swap
		{	//send full amount
			...
			//any sell/liquify must occur before main transfer
			swapBufferTokens();
    ...
    function excludeFromFee(address account) public onlyOwner {
        if (account.isContract() && account != uniswapPair && account != uniswapV2RouterAddr)
...
contract OfficialTestToken is DeflationaryERC20 {
    constructor()  DeflationaryERC20("OfficialTestToken2", "OTT2", 0) {
