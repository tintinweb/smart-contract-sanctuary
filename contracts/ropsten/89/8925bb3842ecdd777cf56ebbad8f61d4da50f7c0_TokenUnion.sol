...
///-=-=-=-=-=-=-=-=-=-=-=-=-CONSTRUCTOR-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\\\
     ...
     * @return balance uint256 balance of specified token for given user
     */
     ...
     * @dev simply calls userTokenBalances() using msg.sender as input and
    ...
    function getMyTokenBalances() public view returns (uint256[], uint256[]) {
        address _user = msg.sender;
        return (userTokenBalances(_user), userWalletTokenBalances(_user));
    }
