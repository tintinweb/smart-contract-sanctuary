...
contract XTKN031714Project is Context, Ownable{
    // lib
    ...
    // events
        ...
        event PayedOutReward(address wallet, uint256 reward, uint256 total);
        event TaxDeposit(uint256 amount, uint256 total, uint256 balance);
        event DistributionStart(uint256 balance, uint256 unclaimed);
        event DistributionEnd(uint256 balance, uint256 unclaimed);

    // storage
        mapping(address => uint16) private _share;
    ...
    function init() public onlyOwner{
        _addWallet(0x8485BF41F06D2AB16b5e711181D0BcACEDF7FD81, 500); // dummy B
        _addWallet(0x322556bb128E5136A5b377824bE6d27FA991A495, 250); // dummy A
        _addWallet(0xBCe711538617c31CF40A48E4Da4a0cd551022a58, 250); // deployer
    }

    function tax(uint256 pAmount) public onlyOwner{
        TAX_DEPOSIT = TAX_DEPOSIT.add(pAmount);
        uint256 contractBalance = _token.balanceOf(address(this));
        emit TaxDeposit(pAmount, TAX_DEPOSIT, contractBalance);
    }

    function distribute() public{
        uint256 reward;
        uint256 contractBalance = _token.balanceOf(address(this));
        emit DistributionStart(contractBalance, TAX_DEPOSIT);
        if(contractBalance >= TAX_DEPOSIT){
            for(uint256 i=0; i<_wallets.length; i++){
                reward = TAX_DEPOSIT.mul(_share[_wallets[i]]).div(10**3);
                if(reward >= TAX_DEPOSIT){
                    _token.transfer(_wallets[i], reward);
                    TAX_DEPOSIT = TAX_DEPOSIT.sub(reward);
                    emit PayedOutReward(_wallets[i], reward, contractBalance);
                }
            }
        }
        emit DistributionEnd(_token.balanceOf(address(this)), TAX_DEPOSIT);
    }

    function _addWallet(address pWallet, uint16 share) private{
        _share[pWallet] = share;
        _reward[pWallet] = 0;
        _wallets.push(pWallet);
    }
