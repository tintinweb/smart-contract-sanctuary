...
contract TokenFactory is Ownable {
    ...
    event FahrenheitTokenCreated(string name, string symbol, uint8 decimals, address contractAddress);

    // We store a list of all the Token ERC20 contracts we've created
    mapping(string => address) fahrenheitTokens;

    // constructor () {
    // }

    // To create a new token we need to pass name, symbol, number of decimals
    // Initial supply is assumed to be 0 always, we need to use/call mintForCustomer() in order
    // to increase the supply
    function createFahrenheitToken(
        string calldata name, 
        string calldata symbol,
        uint8 decimals
    ) public onlyOwner returns (address) {

        // Check that we don't already have a token with the same ticker
        require(fahrenheitTokens[symbol] == address(0), "TokenFactory: ticker already in use!");

        Token token = new Token(name, symbol, decimals);
        fahrenheitTokens[symbol] = address(token);
        // transfer ownership to the owner of TokenFactory, instead of TokenFactory contract itself
        token.transferOwnership(this.owner());

        emit FahrenheitTokenCreated(name, symbol, decimals, address(token));
        return address(token);
    }

    // Reads address of a given ticker's ERC20 contract
    function getTokenAddress(string memory symbol) public view returns (address) {
        return fahrenheitTokens[symbol];
    }
