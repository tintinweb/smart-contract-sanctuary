...
contract BTCZ is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, Mintable  {
    ...
    constructor(address _owner, address _imx,address payable fundsTo_, uint256 maxSupply_, uint256 maxPerTx_, uint256 pricePer_, uint256 pricePerPre_) ERC721("BTCZ", "BTCZ") Mintable(_owner, _imx){
        imx = _imx;
        require(_owner != address(0), "Owner must not be empty");
        transferOwnership(_owner);
        fundsTo = fundsTo_;
    ...
    function setMaxPerTX(uint __maxPerTX) external onlyOwner {
        maxPerTx = __maxPerTX;
    }
    ...
    function transfer(uint256 quantity) external payable {
        ...
        require(totalSupplied + quantity <= maxSupply, "Purchase would exceed max tokens for presale");
         require(mintedLog[msg.sender] + quantity <= maxWallet, "Purchase would exceed max tokens per wallet");
    ...
    function transferPreSale(uint256 quantity) external payable {
        ...
        require(totalSupplied + quantity <= maxPreMint, "Purchase would exceed max tokens for presale");
         require(mintedLog[msg.sender] + quantity <= maxWallet, "Purchase would exceed max tokens per wallet");
        require(!Address.isContract(msg.sender), "Contracts are not allowed to mint");
