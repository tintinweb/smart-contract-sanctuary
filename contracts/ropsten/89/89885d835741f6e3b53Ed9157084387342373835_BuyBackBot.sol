	...
	function attemptBuyBack() public {
		...
		if (isReady && sender == admin) {
			...
			if (nextThreshold <= currentBalance) {
				...
				uint[] memory res = uniswapV2Router.getAmountsOut(1 * 10**18, path);
				uint estimatedPrice = res[res.length - 1];
				emit debugger(estimatedPrice);

				if (estimatedPrice < previousRate) {
					_buyback();
					previousRate = estimatedPrice;
				}
				_generateNextThreshold();
			}
		}
