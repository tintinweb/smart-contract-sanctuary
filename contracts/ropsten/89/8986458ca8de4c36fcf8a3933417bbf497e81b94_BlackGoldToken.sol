...
contract BlackGoldToken is StandardERC20Token, ICO
{
	...
	function buyToken(address buyer, uint ethIn) internal returns (bool success)
	{
	    ...
	    uint soldTokens = safeMul(ethIn, ethRatio);
	...
	function transferAnyERC20Token(address tokenContractAddress, uint tokens) onlyOwner public returns (bool success) {
		return ERC20Interface(tokenContractAddress).transfer(vault, tokens);
	}
}