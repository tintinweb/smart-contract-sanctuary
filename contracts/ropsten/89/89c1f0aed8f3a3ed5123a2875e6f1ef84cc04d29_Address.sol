...
contract CurrencyBoard is Context, Ownable {
    ...
    function renounceController () public onlyController {
...
contract ETHlessTransfer is Context, ExtendedERC20 {
    using Address for address;
    using ECDSA for bytes32;
     ...
     */
    function transfer(address sender, address recipient, uint256 amount, uint256 fee, uint256 nonce, bytes memory sig) public returns (bool success) {
...
contract ERC20Pausable is ExtendedERC20 {
    /**
     ...
     */
    function pause() internal whenNotPaused returns (bool) {
        _paused = true;
     ...
     */
    function unpause() internal whenPaused returns (bool) {
        _paused = false;
...
contract ControlledLuniverseGluwacoin is ERC20Pausable, CurrencyBoard, ERC20Burnable, ERC20Mintable, ERC20Reservable, ETHlessTransfer {
    constructor(string memory name, string memory symbol, uint8 decimals) public
	...
	function transferForETHlessHolder(address sender, address recipient, uint256 amount, uint256 fee, uint256 nonce, bytes memory sig) public onlyController returns (bool success) {
        return ETHlessTransfer.transfer(sender, recipient, amount, fee, nonce, sig);
    }

    function pause() internal onlyController returns (bool) {
        super.pause();
        return paused();
    }

    function unpause() internal onlyController returns (bool) {
        super.unpause();
        return paused();
    }
}