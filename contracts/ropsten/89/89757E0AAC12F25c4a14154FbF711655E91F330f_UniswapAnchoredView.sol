...
contract UniswapAnchoredView is UniswapConfig {
    ...
    function priceInternal(TokenConfig memory config) internal view returns (uint) {
        if (config.priceSource == PriceSource.REPORTER || config.priceSource == PriceSource.UNISWAP) return prices[config.symbolHash];
    ...
    function getUnderlyingPrice(address cToken) external view returns (uint) {
        ...
        // OLA_ADDITIONS : Using underlying instead of cToken
        config = getTokenConfigByUnderlying(CErc20ForUniswapConfig(cToken).underlying());
    ...
    function postPrices(bytes[] calldata messages, bytes[] calldata signatures, string[] calldata symbols) external {
        ...
        for (uint i = 0; i < messages.length; i++) {
            // OLA_ADDITIONS : Added 'if' condition,
            // TODO : only call 'put' for "REPORTER" price source
    ...
    function postPriceInternal(string memory symbol, uint ethPrice) internal {
        TokenConfig memory config = getTokenConfigBySymbol(symbol);
        require(config.priceSource == PriceSource.REPORTER || config.priceSource == PriceSource.UNISWAP, "only reporter or uniswap prices get posted");
