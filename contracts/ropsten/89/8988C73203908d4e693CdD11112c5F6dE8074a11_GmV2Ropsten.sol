...
import "hardhat/console.sol"; // DO NOT FORGET TO REMOVE THIS!!!!
    ...
    ReentrancyGuard
{
    ...
    uint256 public maxSupply = 29; // Total of all normal and special 1/1 tokens minus 100 burned tokens
    uint256 public maxNormalTokens = 16; // 836 normal tokens in circulation
    uint256 public maxSpecialTokens = 13; // 63 possible special 1/1 tokens
    ...
    function upgradeToken() external migrationActive nonReentrant {
        require(
            (_nextNormalTokenId.current() + _nextSpecialTokenId.current()) <
                maxSupply,
            "MAX_TOTAL_SUPPLY"
        );
        require(
            _nextNormalTokenId.current() < maxNormalTokens,
            "MAX_NORMAL_SUPPLY"
        );
    ...
    function upgradeBatch(uint256 qty) external onlyOwner nonReentrant {
        require(
            ((_nextNormalTokenId.current() + qty) +
                _nextSpecialTokenId.current()) < maxSupply,
            "MAX_TOTAL_SUPPLY"
        );
        require(
            (_nextNormalTokenId.current() + qty) < maxNormalTokens,
            "MAX_NORMAL_SUPPLY"
        );
    ...
    function onERC1155Received(
        ...
        console.log("Inside onERC1155Received");
        require(
