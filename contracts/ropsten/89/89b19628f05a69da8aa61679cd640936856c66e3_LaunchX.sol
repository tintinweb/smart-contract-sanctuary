...
contract LaunchX is Ownable {
  ...
  function buyTokens(uint256 acceptedAmount) external {
    ...
    // Should check trigger external function carefully
    // Snapshot contract accepted balance
    address contractAddress = getContractAddress();
    uint256 acceptedBalSnapshot = _acceptedToken.balanceOf(contractAddress);
    // Transfer tokens into contract
    require(_acceptedToken.transferFrom(senderAddress, address(this), acceptedAmount));
    // Verify transferFrom correctly
    uint256 acceptedBalCurrent = _acceptedToken.balanceOf(contractAddress);
    require(acceptedBalCurrent == acceptedBalSnapshot + acceptedAmount, "Transfer accepted token is not correct");
