...
contract LotteryV1 is AccessControl, Pausable {
    ...
    function _raffle(uint256 _eventId,uint256 _amount,uint256 _payType,bytes32 hash, bytes memory signature,string memory nonce)internal {
        ...
        //验证hash
        require(hashRaffleTransaction(msg.sender,amount,nonce,_payType) == hash,"Invalid hash!");
        //验证签名
        ...
        if (_payType == 1){
            ...
            if (_payType == 3){
                ...
                if (ticketNumber >= burnTicketNumber){
                    ...
                    emit Raffle(EventId,msg.sender,amount,burnTicketNumber,3,mintNftTokenIds,_Nonce);
                }
            }
        }
        //增加该场次的已购买的NFT数量
    ...
    function withdrawNFT(uint256 _tokenId,address _withdrawNFTContractAddress,uint256 _withdrawNFTTokenID,address _withdrawNFTAddress,bytes32 hash, bytes memory signature,string memory nonce)public{
        //验证hash
        require(hashWithdrawNFTTransaction(_tokenId,_withdrawNFTAddress,_withdrawNFTTokenID,_withdrawNFTAddress,msg.sender,1,nonce) == hash,"Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash,signature),"Invalid signature!");
        //验证是否拥有该资产NFT
    ...
    function mortgageNFT2ETH(uint256 _tokenId,uint256 _ETHAmount ,bytes32 hash, bytes memory signature,string memory nonce)public{
        //验证hash
        require(hashMortgageNFTTransaction(_tokenId,msg.sender,_ETHAmount,nonce,"ETH") == hash,"Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash,signature),"Invalid signature!");
        //验证是否拥有该资产NFT
        require(_validateOwnership(_tokenId),"You don't have this NFT!");
    ...
    function mortgageNFT(uint256 _tokenId,uint256 _token1Amount ,bytes32 hash, bytes memory signature,string memory nonce)public{
        //验证hash
        require(hashMortgageNFTTransaction(_tokenId,msg.sender,_token1Amount,nonce,"NFT") == hash,"Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash,signature),"Invalid signature!");
        //验证是否拥有该资产NFT
        require(_validateOwnership(_tokenId),"You don't have this NFT!");
        ...
        a[0] = 1;
        //铸造
        _mintNft(a,_token1Amount,assetsContractAddress);
        //销毁奖券
        _burnNFT(_tokenId);
     ...
     * @return 交易的hash值
     */
    function hashRaffleTransaction(address sender, uint256 qty, string memory nonce,uint256 _payType) private pure returns (bytes32) {
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(sender, qty, nonce,_payType))
            )
        );
        return hash;
    }
     ...
     * @return 交易的hash值
     */
    function hashWithdrawNFTTransaction(uint256 _tokenId,address _withdrawNFTContractAddress,uint256 _withdrawNFTTokenID,address _withdrawNFTAddress,address sender, uint256 qty, string memory nonce) private pure returns (bytes32) {
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(_tokenId,_withdrawNFTContractAddress,sender,_withdrawNFTTokenID,_withdrawNFTAddress,qty, nonce))
            )
        );
        return hash;
    }
     ...
     * @param qty 交易数量
     * @param nonce 交易随机数
     * @param mortgageType 抵押类型
     * @return 交易的hash值
     */
    function hashMortgageNFTTransaction(uint256 tokenId,address sender, uint256 qty, string memory nonce,string memory mortgageType) private pure returns (bytes32) {
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(tokenId,sender, qty, nonce,mortgageType))
            )
        );
        return hash;
    }
...
//      emit TransferETH(_toAddress,_amount);
    ...
    function getPurchasedNumber(uint256 _eventId)public view returns(uint256){

        uint256 purchasedNumber = EventInfoMap[_eventId].purchasedNumber;
