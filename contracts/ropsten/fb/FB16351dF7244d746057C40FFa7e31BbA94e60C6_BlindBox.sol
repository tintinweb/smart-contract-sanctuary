...
contract ERC721Metadata is ERC165, ERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        infoUrl = strConcat('https://800dapp.com/v1/', uint2str(tokenId));
        return infoUrl;
    }
}
...
contract BlindBox is ERC721Full, ERC721Mintable, Ownable {
    using SafeMath for uint256;
    ...
    function batchMint(address _to,uint256 _n) public onlyMinter{
        for (uint i = 0; i < _n; i++) {
            _mint(_to, i);
        }
    }
