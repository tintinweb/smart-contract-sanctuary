...
contract EPARA {
    ...
    function transfer(address _to, uint256 _value) public returns (bool success){
        ...
        if (balanceOf[msg.sender] < _value.safeAdd(theLockValue(msg.sender))) revert("insufficient balance or lock detected"); 
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        if (balanceOf[_from] < _value.safeAdd(theLockValue(_from))) revert("insufficient balance or lock detected");
    ...
    /*get locked balance*/
    function theLockValue(address addr) internal returns (uint256 amount){
    ...
    /*get locked balance*/
    function getLockValue(address addr) public view returns (uint256 amount){
	    lock[addr].lockTime > block.timestamp ? amount = lock[addr].lockValue : amount = 0;
    }
    ...
    function updateLockTime(address addr, uint256 _days)public returns (bool success) {
        require(theLockValue(addr) > 0,"NO lock detected");
    ...
    function transferForMultiAddresses(address[] memory _addresses, uint256[] memory _amounts) public returns (bool) {
       ...
       for (uint i = 0; i < _addresses.length; i++) {
           ...
           if (balanceOf[msg.sender] < _amounts[i].safeAdd(theLockValue(msg.sender))) revert("insufficient balance or lock detected"); 
