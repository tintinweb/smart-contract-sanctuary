...
contract MetaZoos is ERC721Enumerable {
  ...
  uint256 public MetazooMinted;
  string private MetazooURI; 
  event MetazooBought(address buyer, uint256 MetazooBought, uint256[10] metazooIndexes);
  ...
  function mintMetazooFamily(uint256[] memory toMint) public {
    require(msg.sender == owner, "Only owner can mint before sale");
    MetazooMinted += toMint.length;
  ...
  function buy(uint256 amountToBuy) public payable {
    ...
    emit MetazooBought(msg.sender, tokensToMint, tokenIdsToMint);
    MetazooMinted += tokensToMint;
...
// review onlyowner and tostring declaration errors
