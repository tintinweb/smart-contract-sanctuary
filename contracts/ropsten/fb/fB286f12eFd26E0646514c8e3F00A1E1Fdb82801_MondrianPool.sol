/**
 *change the stake time from month to seconds
*/
...
contract vBid {
    ...
    uint256 constant MAX_STAKE_TIME=12*30 days;
    ...
    function vstake(uint256 bidAmount,uint256 stakeTime) internal {
        require(stakeTime<=MAX_STAKE_TIME,"stakeTime not in range");
        require(_balances[msg.sender]==0,"this should be called with zero stake");
        
        uint256 muliplier=stakeTime/30 days;
        uint256 amount=bidAmount.mul(muliplier);
    ...
    function increaseStakeAndTime(uint256 bidAmount,uint256 stakeTime1) internal {
        ...
        uint256 amountNew=bidAmount.mul(_startTimes[msg.sender]+newStakeTime-block.timestamp).div(duration_cycle);
        uint256 amountOri=_bidBalances[msg.sender].mul(stakeTime1).div(duration_cycle);
