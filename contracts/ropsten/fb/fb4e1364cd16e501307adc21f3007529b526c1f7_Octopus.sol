...
contract Octopus is Ownable {
    ...
    function _activateEthPutOption(address _optionAddress, uint256 _usdtCollateral, uint256 _uniswapInitialUSDT, uint256 _uniswapInitialTokens, bool _createPair) external onlyOwner {
        require(IOption(_optionAddress).getExpiresOn() > block.timestamp, "Expired option");
    ...
    function _setEthFinalPriceAtMaturity(address _optionAddress) external onlyOwner {
        require(options[_optionAddress].isValid, "Invalid option");
    ...
    function getOption(address _optionAddress) external view returns (bool _isValid, bool _isPut, uint256 _etherPriceInUSDTAtMaturity, uint256 _optionWorth) {
