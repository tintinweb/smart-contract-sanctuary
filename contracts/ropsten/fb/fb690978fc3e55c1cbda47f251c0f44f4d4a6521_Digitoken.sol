...
contract Digitoken is ERC20Interface, SafeMath {
    ...
    mapping(address => representatives )  public registeredReps;
    ...
    constructor() {
        ...
        name = "Digitrade";
        decimals = 0;
        _totalSupply = 900_000_000;
        daoSupply = 50_000_000;
        devFund = 50_000_000;
        digidao_address = msg.sender;
        devFund_address = msg.sender;
        current_rate = div(1,90);
        mintCap = 10;
        minimumTimeBetweenMints = 1 days * 90;
        balances[msg.sender] = _totalSupply;
        emit Transfer(address(0), digidao_address, _totalSupply);
        lockMint = true;
        rateLock = true;
    ...
    modifier onlyVested(uint _proposal) {
       ...
       //require(!proposals[_proposal].alreadyVoted== true);
      _;
    }
    ...
    function registerRep(address _rep) public {
      ...
      uint _unlockBlock = block.number + 155200;  //unlocks after 30 days or so
      registeredReps[_rep] = representatives(_rep,block.number, _unlockBlock);
    }
