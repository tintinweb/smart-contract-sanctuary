...
contract modularLong is FuwaGameEvent, Ownable{}
...
*
*/
contract FuwaGame is modularLong {
    using SafeMath for *;
    using NameFilter for string;
    using FuwaKeysCalcLong for uint256;
    ...
    mapping (uint256 => Fuwadatasets.Player) public players;   // (pID => data) player data
    mapping (uint256 => mapping (uint256 => Fuwadatasets.PlayerRounds)) public playerRounds;    // (pID => rID => data) player round data by player id & round id

    /*  round data */
    uint256 public currendRoundId = 0;
    mapping (uint256 => Fuwadatasets.Round) public rounds;   // (rID => data) round data
        ...
        payable
   {
      ...
      onlyOwner
    {
        if(communityBalance != 0) {
              ...
              communityBalance = communityBalance.sub(send);
        }
    }
        ...
        private
    {
      ...
      if (_eth > 1000000000)
      {
          ...
          emit FuwaGameEvent.onBuy(msg.sender, _eth, _keys);
      }
    }
    ...
    */
        ...
        public
    {
        ...
        if (_eth > 0) {
            ...
            emit FuwaGameEvent.onWithdraw(_playerId, msg.sender, _eth, _now);
        }
    }
        ...
        private
    {
        ...
        emit FuwaGameEvent.onRoundGameOver(
            _currendRoundId,
    ...
    *
        ...
        returns( bool, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, address, uint256, uint256)
    {
        ...
        return (
            ...
            rounds[_currendRoundId].pot.add(rounds[_currendRoundId].bonus),
...
library FuwaKeysCalcLong {
    ...
    function keys(uint256 _eth)
        ...
        returns(uint256)
    {
        // 由于fomo3d 的价格曲线，走势缓慢，我们对 key 放大了100倍处理，用以拉动kkey的价格快速增长，结束本轮游戏.
        return (((  (   ((_eth).mul(1000000000000000000)).mul(312500000000000000000000000)) .add(5624988281256103515625000000000000000000000000000000000000000000)).sqrt()).sub(74999921875000000000000000000000)) / (156250000) / 100;
    }
    ...
    function eth(uint256 _keys)
        ...
        returns(uint256)
    {
        return ((78125000).mul(_keys.sq().mul(10000)).add(((149999843750000).mul(_keys.mul(1000000000000000000).mul(100))) / (2))) / ((1000000000000000000).sq());
    }
}
