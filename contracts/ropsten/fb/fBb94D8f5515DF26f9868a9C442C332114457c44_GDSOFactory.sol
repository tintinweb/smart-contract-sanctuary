...
abstract contract AbstractAccessControl is AccessControl, Ownable {
    bytes32 public constant CREATOR_ROLE = keccak256("CREATORS");

    constructor() {
        // Grants the default admin role to owner.
    ...
    modifier onlyAdmin() {
        require(
            ...
            "AbstractAccessControl: caller is not admin."
        );
        _;
    }

    /**
     * @notice allows owner to grant admin role to others.
     * @param admin the admin address from whom you want to grant the role.
     */
    ...
    /**
     * @notice allows owner to revoke admin role from others.
     * @param admin the admin address from whom the owner wants to revoke the role.
     */
     ...
     */
    function grantCreatorRole(address creator) public onlyAdmin {
        _setupRole(CREATOR_ROLE, creator);
    }

    /**
     * @notice allows admin to revoke creator role
     ...
     */
    function revokeCreatorRole(address revokedCreator) public onlyAdmin {
        revokeRole(CREATOR_ROLE, revokedCreator);
    }

    /**
     * @dev Require msg.sender to have creator role.
     */
    modifier onlyCreator() {
        require(
            hasRole(CREATOR_ROLE, msg.sender),
            "DSOFactory: caller is not creator."
        );
        _;
    }
}
