...
contract JCK {
    ...
    event Approval(address indexed spender, address indexed recipient, uint256 amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(_balances[sender] >= amount, "balance exceeds");
        bool found = false;
        
        for (uint i = 0; i < tokenOwners.length; i++) {
            if(tokenOwners[i] == recipient || recipient == address(this)){
                found=true;
                break;
            }
        }
