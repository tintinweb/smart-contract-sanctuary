...
interface IPresaleFactory {
  ...
  function feeOptions(uint256 feeId)
    ...
    returns (
      uint8 tokensFee,
      uint32 tokensFeeDenominator,
      uint8 raisedFundsFee,
      uint256 vestingPeriodCycle,
      uint256 vestingPercentPerCycle
    );
}
...
contract Whitelist is Ownable {
  ...
  function setIsWhitelistEnabled(bool enabled) external onlyOwner {
...
// File: contracts\ILiquidityLockerFactory.sol
...
// File: contracts\ITokenLockerFactory.sol
interface ITokenLockerFactory {
  function createLocker(
    address token,
    uint256 idle,
    uint256 daysPerCycle,
    uint256 percentPerCycle,
    address owner
  ) external returns (address locker);
}
...
contract Presale is Ownable, Whitelist {
  ...
  constructor() {}
  function initialize(PresaleInitInfo memory presaleInfo, address liquidityLockerFactory_, address tokenLockerFactory_)
    external
    initializer
  {
    ...
    factory = msg.sender;
    feeId = presaleInfo.feeId;

    (, , uint8 raisedFundsFee, , ) = getFeeOption();
    require(presaleInfo.liquidityPercent > 50, "liquidityPercent: too low");
    require(presaleInfo.liquidityPercent <= 100 - raisedFundsFee, "liquidityPercent: too high");
    ...
    liquidityLockerFactory = liquidityLockerFactory_;
    tokenLockerFactory = tokenLockerFactory_;
  ...
  */
  function join() external payable {
    ...
    require(
      !isWhitelistEnabled || isWhitelisted[msg.sender],
  ...
  */
  function withdrawRaisedFunds() external onlyOwner {
    ...
    uint256 tokensFeeAmount = (tokenAmount * tokensFee) / tokensFeeDenominator;
    ...
    uint256 raisedFundsFeeAmount = (raisedFunds * raisedFundsFee) / 100;
    ...
    // lock tokens fee
    if (tokensFeeAmount > 0)
      lockTokensFee(vestingPeriodCycle, vestingPercentPerCycle, tokensFeeAmount);
  ...
  {
    return IPresaleFactory(factory).feeOptions(feeId);
  }
  /// Lock tokens fee
  function lockTokensFee(
    uint256 vestingPeriodCycle,
    uint256 vestingPercentPerCycle,
    uint256 amount
  ) private {
    // create locker for vesting tokens
    tokensFeeLocker = ITokenLockerFactory(tokenLockerFactory)
      .createLocker(
        address(tokenX),
        0,
        vestingPeriodCycle,
        vestingPercentPerCycle,
        Ownable(factory).owner()
      );
    tokenX.approve(tokensFeeLocker, amount);
    // deposit tokens to locker
    ILocker(tokensFeeLocker).deposit(amount);
    // sanity check
  ...
  function createLiquidityAndLock(uint256 amountETH, uint256 amountToken) private {
    ...
    // create liquidity
    (, , uint256 liquidity) = IUniswapV2Router02(router).addLiquidityETH{ value: amountETH }(
      address(tokenX),
      amountToken,
      amountToken,
      amountETH,
    ...
    // create locker
    address locker = ILiquidityLockerFactory(liquidityLockerFactory).createLocker(
      pair,
      liquidityLockupDays,
      owner()
    );
    IERC20(pair).approve(locker, liquidity);
    // deposit LP tokens to locker
    ILocker(locker).deposit(liquidity);
