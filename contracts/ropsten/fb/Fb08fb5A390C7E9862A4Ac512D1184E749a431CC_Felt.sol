...
library Minting {
    ...
    function split(bytes calldata blob) internal pure returns (uint256, uint16, uint8, string memory) {
        int256 index = Bytes.indexOf(blob, ":", 0);
        require(index >= 0, "Separator must exist");
        int256 index2 = Bytes.indexOf(blob, ",", uint256(index));
        require(index2 > index, "First parameter must exist");
        int256 index3 = Bytes.indexOf(blob, ",", uint256(index2) + 1);
        require(index3 > index2, "Second parameter must exist");

        uint256 tokenId = Bytes.toUint(blob[1:uint256(index) - 1]);
        uint16 feltId = Bytes.toUint16(blob[uint256(index) + 1:uint256(index2)]);
        uint8 feltRarity = Bytes.toUint8(blob[uint256(index2) + 1:uint256(index3)]);
        uint256 endIndex = blob.length - 1;
        string memory feltHash = Bytes.substring(blob, uint256(index3) + 1, endIndex);

        return (tokenId, feltId, feltRarity, feltHash);
    }
}
...
library Bytes {
    ...
    function toUint8(bytes memory b) internal pure returns (uint8) {
        uint8 result = 0;
