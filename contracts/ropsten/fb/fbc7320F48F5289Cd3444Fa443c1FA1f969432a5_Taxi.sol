...
pragma solidity 0.8.11;
import "./TaxiOracle.sol";

contract HasTaxiOracle {
    address oracle;
    modifier onlyTaxi() {
        require(
            TaxiOracle(oracle).isClaimer(msg.sender),
            "Not a verified claimer"
        );
        _;
    }

    function isTaxi() internal view returns (bool) {
        return TaxiOracle(oracle).isClaimer(msg.sender);
    }
}
...
contract TaxiOracle {
    ...
    constructor(address[] memory claimers) {
        for (uint256 i = 0; i < claimers.length; i++) {
            _claimers[claimers[i]] = true;
        }
    }

    function addClaimer(address claimer) external onlyClaimers {
        _claimers[claimer] = true;
    }

    function removeClaimer(address claimer) external onlyClaimers {
        _claimers[claimer] = false;
    }

    function addClaimerBatch(address[] memory claimers) external onlyClaimers {
        for (uint256 i = 0; i < claimers.length; i++) {
            _claimers[claimers[i]] = true;
        }
    }
