...
contract StakerMultitoken is Ownable {
    ...
    function f9Address() external view returns (address) {
        return address(f9token);
    }

    function ShibaInuAddress() external view returns (address) {
        return address(shibatoken);
    }
    ...
    function stake(address tokenAddress, uint256 value) external notHalted {
        require(value > 0, "Staking Contract: stake value should be greater than 0");
        require(tokenAddress == address(f9token) || tokenAddress == address(shibatoken), "Invalid stake token");
        IERC20 stakeToken = IERC20(tokenAddress);
        stakeToken.transferFrom(_msgSender(), address(this), value);
        _tokenBalances[tokenAddress][_msgSender()] = _tokenBalances[tokenAddress][_msgSender()].add(value);
