...
contract DebtToken is ERC20, Ownable {
    ...
    uint256 private _totalSupply = 1 * 10**6 * 10**DECIMALS;
    uint256 private _initialLiquidity = _totalSupply.mul(95).div(100);
...
//------------------------------------------------------------//
    ...
    function changeThresholdOfTier(uint256 _tier, uint256 _newThreshold)
        ...
        onlyOwner
    {
        tierInfo[_tier].threshold = _newThreshold;
    }
    ...
    function manageTier(address _user, uint256 _newBalance, uint256 _oldBalance) internal {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (boostBalance < tiers[i].threshold) {
                ...
                if (oldTier != i) {
                    user.currentTier = i;
                    if (_newBalance != 950000000000000000000000) tierB.tierSupply = tierB.tierSupply.add(_newBalance);  // Prevents the initial liquidity to count in the upper tier
