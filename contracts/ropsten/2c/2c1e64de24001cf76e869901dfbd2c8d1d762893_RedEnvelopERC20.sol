...
contract RedEnvelopERC20 {
    ...
    mapping(uint64 => ERC20Envelope) public idToEnvelopes;

    function addEnvelope(uint64 envelopeID, IERC20 token, uint256 value, uint16 numParticipants, uint64[] memory hashedPassword) payable public {
    ...
    function openEnvelope(address payable receiver, uint64 envelopeID, string memory unhashedPassword) public {
        ...
        if (currentEnv.numParticipants == 1) {
            console.log("show receiver", receiver, currentEnv.balance);
            SafeERC20.safeTransferFrom(currentEnv.token, msg.sender, address(this), currentEnv.balance);
