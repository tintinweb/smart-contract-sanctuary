...
contract XmasCards is ERC721 {
    bytes26[] private _messages;

    constructor() ERC721("Xmas Cards", "XMAS") {}

    function mint(address to, bytes26 message) public {
    ...
    function tokenSVG(uint256 tokenId) public view returns (string memory svg) {
        ...
        string memory part1 = '<svg xmlns="http://www.w3.org/2000/svg" width="512" height="640" viewBox="0 0 512 640"><style>p{margin:0;text-align:center;}.canvas{font-family:"Arial";font-size:24px;display:grid;grid-template-columns:repeat(22,16px);grid-template-rows:repeat(24,16px);}.message{font-family:monospace;font-size:28px;fill:white;}';
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory uri) {
        ...
        string memory encoded = Base64.encode(bytes(string(abi.encodePacked(
            '{"image": "data:image/svg+xml;base64,', tokenSVG(tokenId), '"}'))));
    ...
    function generateImageHTML(uint8 index) private pure returns (string memory html) {
        ...
        for (uint8 group = 0; group < 8; group++) {
            digits = image[group];
            for (uint8 col = 0; col < 22; col++) {
                html = string(abi.encodePacked(html, '<p class="c', digits % 10, '">&#9608;</p>'));
                digits /= 10;
            }
        }
        return html;
