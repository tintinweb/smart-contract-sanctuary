...
contract Library {
    ...
    //Iskanje knjige
    event iskanjeKnjige(string naslovKnjige, string author, string ISBN);
    function search_Knjiga(string memory search) public{
        for(uint i; i<= stevecKnjig; i++){
            if(knjige[i].moznostIzposoje == true){
                if(keccak256(abi.encodePacked(knjige[i].naslov)) == keccak256(abi.encodePacked(search))){
                    emit iskanjeKnjige(knjige[i].naslov, knjige[i].avtor, knjige[i].ISBN);
                } else if(keccak256(abi.encodePacked(knjige[i].ISBN)) == keccak256(abi.encodePacked(search))){
                    emit iskanjeKnjige(knjige[i].naslov, knjige[i].avtor, knjige[i].ISBN);
                } else if(keccak256(abi.encodePacked(knjige[i].avtor)) == keccak256(abi.encodePacked(search))){
                    emit iskanjeKnjige(knjige[i].naslov, knjige[i].avtor, knjige[i].ISBN);
                }
            }
        }
