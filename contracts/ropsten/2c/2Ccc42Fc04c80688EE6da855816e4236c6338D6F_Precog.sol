...
contract Precog {
    ...
    mapping(address => mapping(address => uint256)) public tradingAmountOnFutureToken;
    mapping(address => mapping(address => uint256)) public profitAmountOnFutureToken;
    ...
    function _removeTradingUser(address _user) internal {
    ...
    function maxProfitable(address user) public view returns (
        ...
        if (availableAmount[user] > feeTrading) {
            amount = availableAmount[user] - feeTrading;
    ...
    function invest(
        ...
        require(availableAmount[user] >= amount + feeTrading, "PrecogV2: AVAILABLE_AMOUNT_NOT_ENOUGH");
        ...
        investAmount[user] += amount + feeTrading;
        tradingAmount[user] += amount + feeTrading;
        tradingAmountOnFutureToken[user][address(futureToken)] += amount + feeTrading;
        profitAmount[user] += profit;
        profitAmountOnFutureToken[user][address(futureToken)] += profit;
        availableAmount[user] -= amount + feeTrading;
        
        _swapTradingFee();
        _removeTradingUser(user);
        
        emit Trade(user, address(futureToken), expiryDate, amount, profit, feeTrading, block.timestamp);
    }
}
