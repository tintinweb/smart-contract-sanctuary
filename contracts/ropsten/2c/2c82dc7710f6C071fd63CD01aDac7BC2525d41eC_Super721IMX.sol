...
contract Super721IMX is PermitControl, ERC165Storage, IERC721 {
  ...
  */
  modifier hasItemRight(uint256 _id, bytes32 _right) {
    ...
    if (_msgSender() == owner()) {
      ...
      revert("hasItemRight: _msgSender does not have the right to perform that action");
    }
  }
  ...
  */
  function ownerOf(uint256 tokenId) public view override returns (address) {
      return _tokenOwners.get(tokenId, "ownerOf: owner query for nonexistent token");
  }
  ...
  function approve(address to, uint256 tokenId) public virtual override {
      ...
      require(_msgSender() == owner || isApprovedForAll(owner, _msgSender()),
          "approve: approve caller is not owner nor approved for all"
      );
  ...
  */
  function setURI(string calldata _uri) external virtual
    ...
    require(!uriLocked,
      "setURI: the collection URI has been permanently locked");
  ...
  function setContractURI(string calldata _uri) external virtual
    hasValidPermit(UNIVERSAL, SET_URI) {
      require(!contractUriLocked,
        "setContractURI: the contract URI has been permanently locked");
  ...
  */
  ...
  returns (uint256) {
    require(_owner != address(0),
      "balanceOf: balance query for the zero address");
  ...
  */
  function balanceOfBatch(address[] calldata _owners, uint256[] calldata _ids)
    ...
    require(_owners.length == _ids.length,
      "balanceOfBatch: accounts and ids length mismatch");
  ...
  */
  function setApprovalForAll(address _operator, bool _approved) external
    ...
    require(_msgSender() != _operator,
      "balanceOf: setting approval status for self");
    ...
    if (_to.isContract()) {
      try IERC721Receiver(_to).onERC721Received(_operator, _from, _id,
        ...
        if (response != IERC721Receiver(_to).onERC721Received.selector) {
          revert("_doSafeTransferAcceptanceCheck: ERC721Receiver rejected tokens");
        }
      } catch Error(string memory reason) {
        revert(reason);
      } catch {
        revert("_doSafeTransferAcceptanceCheck: transfer to non ERC721Receiver implementer");
      }
    }
  }
  ...
  */
  ...
  function _safeTransferFrom(address _from, address _to, uint256 _id,
    ...
    require(_to != address(0),
      "_safeTransferFrom : transfer to the zero address");
    require(_from == _msgSender() || isApprovedForAll(_from, _msgSender()),
      "_safeTransferFrom : caller is not owner nor approved");
    ...
    require(balances[_id][_from] >= 1, "_safeTransferFrom: insufficient balance for transfer");
  ...
  */
  ...
  function safeBatchTransferFrom(address _from, address _to,
    ...
    external virtual {
    require(_to != address(0),
      "safeBatchTransferFrom: transfer to the zero address");
    require(_from == _msgSender() || isApprovedForAll(_from, _msgSender()),
      "safeBatchTransferFrom: caller is not owner nor approved");
    ...
    for (uint256 i = 0; i < _ids.length; ++i) {
      ...
      // Update all specially-tracked group-specific balances.
      require(balances[_ids[i]][_from] >= 1, "safeBatchTransferFrom: insufficient balance for transfer");
  ...
  */
  function configureGroup(uint256 _groupId, ItemGroupInput memory _data)
    ...
    require(_groupId != 0,
      "configureGroup: group ID 0 is invalid");

    // If the collection is not locked, we may add a new item group.
    if (!itemGroups[_groupId].initialized) {
      require(!locked,
        "configureGroup: the collection is locked so groups cannot be created");
    ...
    // Edit an existing item group. The name may always be updated.
      ...
      if (itemGroups[_groupId].supplyType == SupplyType.Capped) {
        require(_data.supplyType == SupplyType.Capped,
          "configureGroup: you may not uncap a capped supply type");
        require(_data.supplyData <= itemGroups[_groupId].supplyData,
          "configureGroup: you may not increase the supply of a capped type");
      ...
      require(_data.supplyData >= itemGroups[_groupId].circulatingSupply,
        "configureGroup: you may not decrease supply below the circulating amount");
  ...
  */
  function _mintChecker(uint256 _id) private view returns (uint256) {
    ...
    require(itemGroups[groupId].initialized,
      "_mintChecker: you cannot mint a non-existent item group");
    ...
    require(!_tokenOwners.contains(_id),
      "_mintChecker: token already exists");
    ...
    if (itemGroups[groupId].supplyType != SupplyType.Uncapped) {
      require(currentGroupSupply + 1 <= itemGroups[groupId].supplyData,
        "_mintChecker: you cannot mint a group beyond its cap");
    }
  ...
  function mintBatch(address _recipient, uint256[] memory _ids,
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      require(_hasItemRight(_ids[i], MINT),
        "mintBatch: you do not have the right to mint that item");
  ...
  */
  function mintFor(address _to, uint256 quantity, bytes calldata _blueprint) external {
    ...
    require(quantity == 1);
    uint256 id = Bytes.split(_blueprint);
  ...
  */
  function _burnChecker(uint256 _id) private view
    ...
    uint256 groupId = shiftedGroupId >> 128;
    require(itemGroups[groupId].initialized,
      "_burnChecker: you cannot burn a non-existent item group");

    // If the item group is non-burnable, then revert.
    if (itemGroups[groupId].burnType == BurnType.None) {
      revert("_burnChecker: you cannot burn a non-burnable item group");
    }
    ...
    // If we can burn items, then we must verify that we do not exceed the cap.
      ...
      require(itemGroups[groupId].burnCount + 1
        ...
        "_burnChecker you may not exceed the burn limit on this item group");
    }
  ...
  */
  ...
  //     "burn: burn from the zero address");
  //
  ...
  //     .sub(_amount,
  //     "burn: burn amount exceeds balance");
  ...
  function burnBatch(address _burner, uint256[] memory _ids) external virtual {
    ...
    for (uint i = 0; i < _ids.length; i++) {
      require(_hasItemRight(_ids[i], BURN),
        "burnBatch: you do not have the right to burn that item");
      ...
      // Update storage of special balances and circulating values.
      require(balances[burntItemId][_burner] >= 1, "burn: burn amount exceeds balance");
  ...
  */
  ...
  function getApproved(uint256 tokenId) public view override returns (address) {
      require(_tokenOwners.contains(tokenId), "getApproved: approved query for nonexistent token");

      return _tokenApprovals[tokenId];
  }

  function _isApprovedOrOwner(address spender, uint256 tokenId) internal view returns (bool) {
      require(_tokenOwners.contains(tokenId), "getApproved: operator query for nonexistent token");
      address owner = ownerOf(tokenId);
  ...
  function transferFrom(address from, address to, uint256 tokenId) public virtual override {
      ...
      require(_isApprovedOrOwner(_msgSender(), tokenId), "transferForm: transfer caller is not owner nor approved");
      safeTransferFrom(from, to, tokenId);
      //
      // require(ownerOf(tokenId) == from, "transferForm: transfer of token that is not own");
      // require(to != address(0), "transferForm: transfer to the zero address");
      //
