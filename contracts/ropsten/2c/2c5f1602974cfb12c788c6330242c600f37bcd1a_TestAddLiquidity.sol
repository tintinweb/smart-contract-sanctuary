...
contract TestAddLiquidity {
    ...
    uint deadline) public {
        require(IERC20(token).approve(UNISWAP_ROUTER_ADDRESS, amountTokenDesired),"Approve Failed");
        require(IERC20(token).transferFrom(msg.sender, address(this), amountTokenDesired),"Transfer Failed");
        uniswap.addLiquidityETH(token, amountTokenDesired, amountTokenMin, amountETHMin,address(this), deadline);
    }
