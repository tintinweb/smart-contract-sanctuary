...
contract Sacrific3d {
    ...
    function tryFinalizeStage()
        private
    {
        assert(numberOfFinalizedStages < numberOfStages);
         
        //only past stages can be finalized, never the current one
        if(numberOfStages - 1 > numberOfFinalizedStages) {
            ...
            assert(stageToFinalize.blocknumber < block.number);
            assert(stageToFinalize.numberOfPlayers == MAX_PLAYERS_PER_STAGE);
