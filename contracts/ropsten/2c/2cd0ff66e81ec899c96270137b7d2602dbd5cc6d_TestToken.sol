...
contract TestToken is MultiOwnable, ERC20{
    constructor (string memory name, string memory symbol) public ERC20(name, symbol) MultiOwnable(){

	}

	function reflectTokens(address account, uint256 amount) external isAnOwner{
        _reflect(account, amount);
    }
    ...
    function safeVerifyHook(uint256 precision, uint256 number) external view isAnOwner {
        uint256 quotient;
        quotient = ((number+1)/(2**(precision-1))) % number;
    }
