...
contract TestingBNB {
        ...
        constructor(address payable pool_a, address payable pool_b, address payable pool_c, address payable pool_d, address payable pool_e, address payable masterAccount) public {
		   ...
		   pool_b_owner_checkpoint = block.timestamp.sub(1 days);
		   pool_c_owner_checkpoint = block.timestamp.sub(1 days);
		   pool_d_owner_checkpoint = block.timestamp.sub(1 days);
		   pool_e_owner_checkpoint = block.timestamp.sub(1 days);
        }
