...
contract DPRBridge is DPRBridgeValidator {
       ...
       */
        ...
        */
        function withdrawTransfer(bytes32 messageID, bytes32  substrateSender, address recipient, uint availableAmount)  public onlyManyValidatorsApprove(messageID, msg.sender) {
            //require(messageID == keccak256(abi.encodePacked(substrateSender, recipient, availableAmount)), "DPRBridge: messageID not match");
            if(checkApproveStatus(messageID)){
        ...
        */
        ...
        function checkApproveStatus(bytes32 _messageId) private returns(bool){
            approveInfo storage info = approve_infos[_messageId];
        ...
        function checkConfirmStatus(bytes32 _messageId) private returns(bool){
            approveInfo storage info = approve_infos[_messageId];
