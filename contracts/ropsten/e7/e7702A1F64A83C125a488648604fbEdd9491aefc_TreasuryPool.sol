    ...
    function addLiquidity(uint256 amount) external override nonReentrant {
        ...
        if (length > 0) {
            ...
            for (uint256 i = 0; i < length; i++) {
                ...
                uint256 ltokenAmountItem =
                    _divExchangeRate(amount, borrowerInterestEarning);
                ltokenAmount = ltokenAmount.add(ltokenAmountItem);
            }
        } else {
            ltokenAmount = amount;
        }
    ...
    function removeLiquidity(uint256 amount) external override nonReentrant {
        ...
        if (length > 0) {
            ...
            for (uint256 i = 0; i < length; i++) {
                ...
                uint256 borrowerInterestEarning =
                    farmingPool.computeBorrowerInterestEarning();
                uint256 underlyingAssetAmountItem =
                    _mulExchangeRate(amount, borrowerInterestEarning);
                underlyingAssetAmount = underlyingAssetAmount.add(
                    underlyingAssetAmountItem
                );
            }
        } else {
