...
contract TokenWrap {
  ...
  constructor() public payable{
   ...
   function setup(IERC20 contract_address) public returns(bool){
       require(address(contract_address).isContract());
       token = contract_address;
       return true;
   }
  function staticInvest(uint8 invent_level,address  payable ahead_address) public payable returns(bool){
     ...
     require(dynamic_map[msg.sender]>=invent_level);
      sendValue(ahead_address,msg.value.mul(84).div(100));
      sendValue(top1,msg.value.mul(10).div(100));
      sendValue(top2,msg.value.mul(6).div(100));
     _sendTokenEtb(ahead_address,msg.value.mul(16).div(100).mul(10000));
     return true;
  }
  function dynamicInvest(uint8 invent_level,address payable  referr_address ) public payable returns(bool){
