...
contract Lottery {
    ...
    uint256 public lotteryDifficulty;

    constructor(uint256 difficulty) {
        owner = msg.sender;
        lotteryDifficulty = difficulty;
    }

    function random() private view returns (uint) {
        return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, lotteryDifficulty)));
    }
