...
/**
    Welcome to CronoBlades one of the first play to earn projects on Cronos Chain!
    Join Official telegram Group:
    https://t.me/CronoBlades
    Website: 
 ...
 * 
 *
 * The CronoBlades contract itself has been heavily annotated to allow all frens
 ...
 * or dump from the CronoBlades. wallets:
 * 
...
// File: CronoBlades.sol


pragma solidity 0.8.6;

contract CronoBlades is Context, ICRC20 {
    ...
    string private constant TOKEN_NAME = "CronoBlades";
    
    string private constant TOKEN_TICKER = "CroBlades";
...
constructor (){
        ...
        //Sets Usdc address
        Usdc = 0x110a13FC3efE6A245B50102D2d79B3E76125Ae83;
        // Sets CronoBlades wallets.
        ...
        BattleChest = 3; 
        previousFunFeePercent = BattleChest;
        ...
        autoLiquidityThreshold = 30000 * 10**9;
        ...
        // Creates the USDC-CroBlades pair on the DEX.
        DEXPair = IDEXFactory(
     ...
     *  (but not buy or receive) CroBlades even if their balance exceeds the max wallet limit.
     ...
     *  CroBlades from a pool if the withdrawal would violate the max Tx or max wallet limits.
     ...
     * It sells half of the CroBlades queued for LP conversion when the threshold is reached, triggered by sells.
     * It then grabs the amount of CRO the swap creates.
     * Finally it combines the CRO with CroBlades to add liquidity to the main WCRO-CroBlades pool to the DEX.
     * Over time, it will cause a build-up of CroBlades in this contract address.
     * This happens because swapping half the CroBlades into CRO reduces the price a bit.
     * In order to mitigate any centralization risk, CroBlades built up in the contract will be recovered and locked
    ...
    function getTaxValues(uint256 transferAmount) private view returns (uint256[6] memory) {
        ...
        reflectLiqCharityBurnFunFeeArray[4] = transferAmount.mul(BattleChest).div(10**2);   
    ...
    function restoreAllFee() private {
        ...
        BattleChest = previousFunFeePercent;
    ...
    /** Returns the total CroBlades supply in uint256 format - 1000000000000000 i.e. 1,000,000 tokens. */
    ...
    /** Returns the token ticker - CroBlades. */
    function symbol() external pure override returns (string memory) {
        return TOKEN_TICKER;   
    }

    /** Returns the token name - CronoBlades. */
    function name() external pure override returns (string memory) {
        return TOKEN_NAME;   
    }

    /** Returns the CroBlades balance of the address queried in uint256 format e.g. 5000000000000 = 5,000 tokens. */
    ...
    function setRouterAddress(address _newAddress) external onlyOwner() {
        ...
        IDEXRouter02 DEXRouterLocal = IDEXRouter02(DEXRouterAddress);      
        DEXPair = IDEXFactory(DEXRouterLocal.factory()).createPair(address(this), Usdc);     
        DEXRouter = DEXRouterLocal;   
    }
     ...
     *  taxes. In this case, the charity mechanism could be achieved by mobilizing CroBlades or
     ...
     * bound is higher at 5%. CroBlades can be deflationary or not - frens decide.
     * 
    ...
    function setFunFeePercent(uint256 _newPercent) external onlyOwner() {
        ...
        require(_newPercent != BattleChest, "Value already set");
        emit FunFeePercentChanged(BattleChest, _newPercent);
        previousFunFeePercent = BattleChest;
        BattleChest = _newPercent;
    }
     ...
     *  that may arise, such as being unable to withdraw CroBlades from a token
     ...
     *  which would result in a significant build-up of CroBlades in this contract address
     *  due to the tax and reflections gained. The CroBlades could then be recovered from
    ...
    /**
     * Function: Allows the owner to set the amount of CroBlades in the token contract
     ...
     *  a number of reasons: at launch when the market cap is low and the number of CroBlades
     ...
     *  low, at 50 for example, CroBlades would build up over time and then as the market cap
     ...
     *  CroBlades in the average Tx decreases relative to the max supply.
     * 
     * Risk statement: Owner could set threshold as low as possible in order to increase the
     *  number of CroBlades that accumulate in the CroBlades contract, then recover them from them
     ...
     *  because they hold most of the CroBlades. The burn address is also excluded for The
     ...
     *  deemed to threaten the future of the project by holding more CroBlades than intended,
     ...
     *  in reflections.
     * 
     * Risk statement: Owner could include the main CroBlades LP address in reflections to
     ...
     *  
     * Justification: This function exists primarily to whitelist CroBlades pools so that 
     ...
     *  issues: a fren may be unable to unstake CroBlades from a pool, harvest from a farm or 
     *  break LP if the Tx would take them over the max wallet limit; a fren may be unable 
     *  to receive CroBlades from giveaways and competitions. These isues are only expected to
     ...
     *  to unstake CroBlades or break LP due to it violating the max Tx limit. It can also be used
    ...
    /**
     * Function: Allows the owner to recover a specific amount of CroBlades from the CroBlades 
     *  contract.
     * 
     * Justification: Someone ending CroBlades to the contract address will almost certainly happen 
     *  at some point. This function allows the owner to recover the CroBlades lost so they can be 
     *  returned to the sender.
     * 
     * Risk statement: It is not expected that the contract will hold CroBlades unless someone
     *  accidentally sends them. Owner could choose not to return the CroBlades but there would
     *  be little incentive to do so.
     * 
     * Mitigation: N/A.
     */
    function recoverCronoBlades(uint256 _amount) external onlyOwner() {
        require (_amount <= balanceOf(address(this)), "Not enough CroBlades in contract");
        emit CronoBladesRecovered(payableAddr(), _amount);
     ...
     *  from the CroBlades contract.
     *  
     * Justification: As above but for any token. The owner does not preclude the possibility of 
     *  taking some of this LP, breaking it, distributing the CroBlades back to the community via 
     ...
     * 
     * Risk statement: As above but for any token. Could be called to withdraw CroBlades awaiting
    ...
    /**
     * Function: Allows the owner to withdraw all CRO from the CroBlades contract.
     *  
     * Justification: The reason this is called a recover function is that its intended
     *  use is to recover CRO sent accidentally to the CroBlades contract address. This is
     ...
     *  in CroBlades is swapped for CRO and combined to form LP, the swap itself marginally reduces 
     *  the price of CroBlades. The autoLiquidity function does not account for this the next 
     *  time it is called.
     * 
    ...
    /** Gets the address to which tokens can be recovered from the CroBlades contract - the owner wallet. */
