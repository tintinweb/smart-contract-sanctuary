...
contract NFT is ERC721 
{
	...
	uint public limit_supply; // 총 토큰 발행 제한 개수

    Token[] public tokens; // tokens의 index는 0부터 시작
    address public owner; // 컨트랙트의 소유자

    constructor(string memory _name, string memory _symbol, uint _limit_supply) public
	{
		name = _name;
		symbol = _symbol;
		limit_supply = _limit_supply;
        owner = msg.sender; // 새 token을 생성할 수 있는 NFT 컨트랙트의 소유자
    }

    function mint(string memory title, string memory description, string memory image, address account) public
	{
		require(total_supply + 1 <= limit_supply, "limit_supply of mints already reached");
        require(owner == msg.sender); // 소유자만이 token을 생성할 수 있습니다
        uint256 tokenId = tokens.length; // 유일한 token ID
        tokens.push(Token(title, description, image));  // 신규 발행하는 token 정보를 추가
		total_supply += 1; // 총 토큰 발행 개수 + 1
