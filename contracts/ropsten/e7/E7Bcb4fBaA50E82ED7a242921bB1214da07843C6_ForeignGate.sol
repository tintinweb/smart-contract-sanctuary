...
contract OperatorHub is Ownable {
  ...
  constructor(uint8 requiredOperators_, address[] memory initialOperators) {
    require(requiredOperators_ != 0, "should provide the number of required operators");
    require(initialOperators.length >= requiredOperators_, "should provide more operators");
  ...
  function operatorLocations() public view returns (bytes32[] memory) {
    bytes32[] memory res = new bytes32[](operatorList.length);
    for (uint i = 0; i < res.length; i++) {
      res[i] = stringToBytes32(operatorLocation[operatorList[i]]);
    }
