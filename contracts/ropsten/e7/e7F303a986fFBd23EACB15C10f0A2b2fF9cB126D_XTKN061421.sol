...
abstract contract PreSale is Context{
    ...
    event DebugPreSaleTransaction(address owner, uint256 amount);
    ...
    function preSaleBuyBag(uint256 pSize) public returns(bool){
        require(_preSaleBag[pSize], "[XTKN061421::ERROR] you can only buy bags that exist!");
        uint256 allowance = _preSaleBuyToken.allowance(_msgSender(), address(this));
        uint256 price = _preSaleBagPrice[pSize];
        require(allowance <= 0, "[XTKN061421::ERROR] you have to authorize the transfer of your BUSD first!");
        require(allowance >= price, "[XTKN061421::ERROR] you do not have enough BUSD!");
        require(_preSaleBagsAvailableCheck(pSize), "[XTKN061421::ERROR] no more of these bags available!");
        require(!_preSaleInvestorHasThisBag(_msgSender(), pSize), "[XTKN061421::ERROR] you can only buy one of this bag!");
        require(_preSaleBuyBag(_msgSender(), pSize, price), "[XTKN061421::ERROR] could not buy the bag!");
        return(true);
    }

    function _preSaleBuyBag(address pInvestor, uint256 pSize, uint256 pPrice) private returns(bool){
        ...
        if(status){
            ...
            _preSaleBagsAvailable[pSize] = _preSaleBagsAvailable[pSize].sub(1);
            emit PreSaleBuyBag(pInvestor, pSize, pPrice, _preSaleInvestors[pInvestor][pSize], _preSaleBagsAvailable[pSize]);
            return(true);
        }
        return(false);
    }
    function _preSaleBagsAvailableCheck(uint256 pSize) private returns(bool){
        if(_preSaleBagsAvailable[pSize] > 0){
            return(true);
        }
        emit PreSaleBuyFailCanOnlyBuyOneBag(_preSaleBagsAvailable[pSize]);
        return(false);
    }

    function _preSaleInvestorHasThisBag(address pInvestor, uint256 pSize) private returns(bool){
        if(_preSaleInvestors[pInvestor][pSize]){
            return(true);
        }
        emit PreSaleBuyFailOnlyOneBag(_preSaleInvestors[pInvestor][pSize]);
        return(false);
    }
...
contract IXTKN061421Liquidity{
    function init() public{}
    function getPair() public returns(address){}
}


contract IXTKN061421Project{
    function init() public{}
...
*/
contract XTKN061421 is Context, IERC20, Ownable, RobinHood, PreSale{
    // public properties
        // lib
            using SafeMath for uint256;
            using Address for address;
        ...
        // interfaces
            ...
            IXTKN061421Liquidity private _liquidity;
            IXTKN061421Project private _project;
            IXTKN061421Staking private _staking;
    ...
    // private properties
        ...
        // how our token is setup, standard one billion tokens, but with 9 decimals instead of the 18
            string private _name = "XTKN061421"; // the name of our project
            string private _symbol = "XTKN061421";
    ...
    function initLiquidity(address contractAddress) public onlyOwner{
        if(!_contractLiquidityActive){
            ...
            if(ADDRESS_LIQUIDITY.balance > 0){
                ...
                // init contract
                _liquidity = IXTKN061421Liquidity(ADDRESS_LIQUIDITY);
                _liquidity.init();
    ...
    function initProject(address contractAddress) public onlyOwner{
        if(!_contractProjectActive){
            ...
            // init contract
            _project = IXTKN061421Project(ADDRESS_PROJECT);
            _project.init();
    ...
    function initStaking(address contractAddress) public onlyOwner{
        if(!_contractStakingActive){
            ...
            // init contract
            _staking = IXTKN061421Staking(ADDRESS_STAKING);
            _staking.init();
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[XTKN061421::ERROR] approve from the zero address");
        require(spender != address(0), "[XTKN061421::ERROR] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[XTKN061421::ERROR] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[XTKN061421::ERROR] transfer amount exceeds allowance"));
        return(true);
    }
