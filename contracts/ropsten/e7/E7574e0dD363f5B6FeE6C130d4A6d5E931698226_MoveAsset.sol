...
contract MoveAsset is IERC1155Receiver {
  ...
  function _calculateFee()   public view returns (uint256){
      (uint128 value, uint128 timestamp)  = DIAOracleV2(gasOracle).getValue("GAS_ETH");
  ...
  function _isFeeRequired(address _nftAddress,  uint256 _nftID) public view returns(bool){
  ...
  function deposit(uint256 _nftID, address _nftAddress) external payable  {
    ...
    if (_isFeeRequired(_nftAddress,_nftID)){
    ...
    IERC1155(_nftAddress).safeTransferFrom(
     ...
     emit Deposit(_nftAddress, msg.sender, _nftID,ethdropped[_nftAddress][_nftID],msg.value);
  }
