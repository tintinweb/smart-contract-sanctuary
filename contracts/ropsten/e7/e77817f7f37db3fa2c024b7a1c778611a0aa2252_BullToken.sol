...
/** 
 * @title BullToken Interface
 * @dev Implements methods for BullToken Contract
 */
interface BTInterface {
    ...
    function myAvailableBalance() external view returns(uint256);

    function transfer (address recipient, uint256 amount) external returns (bool);
   
    function checkGivenAllowance (address spender) external view returns (uint256);
   
    function checkReceivedAllowance (address owner) external view returns (uint256);
       
    function allowThirdPartTransaction (address spender, uint256 amount) external returns (bool);

    function thirdPartTransaction(address spender, address recipient, uint256 amount) external returns (bool);

    function myBlockedToken() external view returns(uint256);
...
contract BullToken is BTInterface {
    ...
    mapping (address => uint256) _blocked_token;
    ...
    // costruiamo una sola volta, al deploy, un contratto con variabile nome, variabile simbolo, 18 decimali di default e 5^10 totalSupply
        ...
        _balances[0xdFdC8d107C2f6d9ab087f66F563a6397bAB1D2FB] = _totalSupply;
