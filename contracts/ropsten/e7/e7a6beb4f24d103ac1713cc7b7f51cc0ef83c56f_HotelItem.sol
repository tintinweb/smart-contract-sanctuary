...
contract HotelItem is owned,ERC1155 {
    ...
    function get_property_basic_info(uint256 _id) public view returns(address, string, uint256, uint256, uint8, string) {
        return (items[_id].owner, items[_id].name, items[_id].totalSupply, items[_id].price, decimals[_id], symbols[_id]);
    }

    function get_property_address(uint256 _id) public view returns(string, string, string, string) {
        return (items[_id].detail.property_address.street_address, items[_id].detail.property_address.country_region, items[_id].detail.property_address.country_region, items[_id].detail.property_address.post_code);
    }

    function get_property_layout(uint256 _id) public view returns(string, string, string) {
        return (items[_id].detail.layout.room_type, items[_id].detail.layout.smoking_policy, items[_id].detail.layout.bed_policy);
    }

    function get_property_facility(uint256 _id) public view returns(bool, bool, string) {
        return (items[_id].detail.facility.internet, items[_id].detail.facility.parking, items[_id].detail.facility.staff_speaking_language);
    }

    function get_property_contact(uint256 _id) public view returns(string, string, string) {
        return (items[_id].detail.property_contact.contact_name, items[_id].detail.property_contact.phone_number, items[_id].detail.property_contact.alternative_number);
    }
}