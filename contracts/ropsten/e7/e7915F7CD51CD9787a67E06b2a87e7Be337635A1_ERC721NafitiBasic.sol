...
contract ERC721NafitiBasic is ERC721 {
    ...
    constructor(address erc20TokenAddress) public ERC721("Nafiti", "NAFI") {
        erc20TokenContract = IERC20(erc20TokenAddress);
    }
    ...
    event AuctionCreated(address seller, AuctionType auctionType, uint256 tokenId, uint256 amount);
    event BidPlaced(address bidder, uint256 tokenId);
    event BidClosed(address highestBidder, uint256 highestBid, address seller);
    event TokenPurchased(address seller, address buyer, uint256 amount);
    ...
    function CreateAuction(uint256 _tokenId, AuctionType _auctionType, uint256 _duration, uint256 _startingPrice) public payable
    {
        //Check that the seller is the owner of the collectible
        ...
        //Require that the auction is not currently open
        require(auctions[_tokenId].auctionStatus != AuctionStatus.Open);

        Bid memory bid = Bid(msg.sender, _startingPrice, AuctionStatus.Open, _duration, msg.sender, 1, _auctionType);
        AuctionCreated(msg.sender, _auctionType, _tokenId, _startingPrice);
    }

    function PlaceBid(uint256 _tokenId, uint _amount) public
    { 
        Bid storage bid = auctions[_tokenId];

        //Require that the auction is not currently open
        require(bid.auctionStatus == AuctionStatus.Open);

        //Require that the bid to be placed is higher than the highest bid placed 
        require(bid.highestBid < _amount, "Bidding amount must be higher than the current highest bid");
        ...
        bid.highestBid = _amount;
        bid.highestBidder = msg.sender;
        BidPlaced(msg.sender, _tokenId);
    }

    function CloseBid(uint256 _tokenId) public
    {
        Bid storage bid = auctions[_tokenId];
        require(msg.sender == bid.seller, "Only Seller Can close the bid");
        require(bid.auctionType == AuctionType.Unlimited, "Only Free Bids can be closed");

        //Transfer the money to the seller
        erc20TokenContract.approve(address(this), bid.highestBid);
        erc20TokenContract.transferFrom(address(this), bid.seller, bid.highestBid);

        safeTransferFrom(bid.seller, bid.highestBidder, _tokenId);
        BidClosed(bid.highestBidder, bid.highestBid, bid.seller);
    }
    ...
    {
        Bid storage bid = auctions[_tokenId];
        require(bid.auctionType == AuctionType.Fixed, "Bid must be fixed");
        erc20TokenContract.approve(address(this), bid.highestBid);
        erc20TokenContract.transferFrom(msg.sender, bid.seller, bid.highestBid);
        _transfer(bid.seller, msg.sender, _tokenId);
        emit TokenPurchased(bid.seller, msg.sender, bid.highestBid);
    }

    function createSellOrder(uint256 _tokenId, uint256 _price) public
    {
        //Check that the seller is the owner of the collectible
        address ownerOfToken = ownerOf(_tokenId);
        ...
        //Sell
        Bid memory bid = Bid(msg.sender, _price, AuctionStatus.Open, 0, msg.sender, 1, AuctionType.Fixed);
        auctions[_tokenId] = bid;
        emit AuctionCreated(msg.sender, AuctionType.Fixed, _tokenId, _price);
    }
    ...
    function cancelBid(uint256 _tokenId) public
    {
        ...
        Bid memory bid = auctions[_tokenId];

        if(bid.highestBidder != msg.sender)
        {
            erc20TokenContract.transferFrom(address(this), bid.highestBidder, bid.highestBid);
        }
        bid.auctionStatus = AuctionStatus.Cancelled;
    }
