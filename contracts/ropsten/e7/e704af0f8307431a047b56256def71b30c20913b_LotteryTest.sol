...
contract LotteryTest {
    ...
    function play() payable public returns (bool) {
        return placeBet(uint(keccak256(abi.encodePacked(msg.sender,block.number))));
    }    

    function placeBet(uint _hash) public payable returns (bool) {
        uint _wei = msg.value;
        assert(_wei == 1000);
    ...
    function drawWinner() public onlyOwner returns (address) {
        ...
        winners[_winner] = 1000 * betaddrs.length;
        cumulativeHash = bytes32(0);
        delete betaddrs;
        return _winner;
    }
