...
contract TrueNFT is ERC721 {
  ...
  function createTrueAsset(string memory uri, uint8 royalty,string memory didToken,string memory socialUri) public returns(uint256){
    ...
    TrueAssets[newTrueAssetId] = TrueAsset(newTrueAssetId, payable(msg.sender), address(this), uri, royalty,didToken,socialUri);

    return newTrueAssetId;
  }

function createTrueAssetBundle(string memory uri, uint8 royalty, uint256 quantity, string memory didToken, string memory socialUri) public returns(uint256[] memory){
    require(royalty > 0, "Royalty cannot be zero or smaller than zero");
  ...
  function getDidToken(uint256 tokenId) external virtual view returns (string memory){
    require(_exists(tokenId), "ERC721Metadata: didToken query for nonexistent token");

    return TrueAssets[tokenId].didToken;
  }

  function getSocialUri(uint256 tokenId) external virtual view returns (string memory){
    require(_exists(tokenId), "ERC721Metadata: SocialUri query for nonexistent token");

    return TrueAssets[tokenId].socialUri;
  }
