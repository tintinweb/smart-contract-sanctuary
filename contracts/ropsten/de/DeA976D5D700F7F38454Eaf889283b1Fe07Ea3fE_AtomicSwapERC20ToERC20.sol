...
contract AtomicSwapERC20ToERC20 {
  ...
  function open(bytes32 _swapID, uint256 _openValueTokenId, address _openContractAddress, uint256 _closeValue, address _closeTrader, address _closeContractAddress) public onlyInvalidSwaps(_swapID) {
    ...
    require(address(this) == openERC721Contract.getApproved(_openValueTokenId));
    // require(openERC20Contract.safeTransferFrom(msg.sender, address(this), _openValueTokenId));
    openERC721Contract.safeTransferFrom(msg.sender, address(this), _openValueTokenId); // TODO: before in example it's return bool as result, how I could handle that transfer was success?

    // Store the details of the swap.
    Swap memory swap = Swap({
      openValue: _openValueTokenId,
  ...
  function close(bytes32 _swapID) public onlyOpenSwaps(_swapID) {
    ...
    IERC721 openERC721Contract = IERC721(swap.openContractAddress);
    // require(openERC721Contract.safeTransferFrom(swap.closeTrader, swap.openValue));
    openERC721Contract.safeTransferFrom(address(this), swap.closeTrader, swap.openValue); // TODO: before in example it's return bool as result, how I could handle that transfer was success?
  ...
  function expire(bytes32 _swapID) public onlyOpenSwaps(_swapID) {
    ...
    IERC721 openERC721Contract = IERC721(swap.openContractAddress);
    // require(openERC721Contract.safeTransferFrom(swap.openTrader, swap.openValue));
    openERC721Contract.safeTransferFrom(address(this), swap.openTrader, swap.openValue); // TODO: before in example it's return bool as result, how I could handle that transfer was success?
