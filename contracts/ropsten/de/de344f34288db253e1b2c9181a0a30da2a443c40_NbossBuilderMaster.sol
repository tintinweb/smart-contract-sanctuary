...
contract NbossBuilderMaster {
    ...
    function getSpecificNbossNum(uint tokenId) public view returns (uint) {
         uint top_level = getContractId(tokenId);
         uint mid_level = getNbossTypeId(tokenId);
         return uint(tokenId - (topLevelMultiplier*top_level) - (mid_level*midLevelMultiplier));
    }
    
    function encodeTokenId(uint contractIdCalc, uint nbossType, uint specificNbossNum) public view returns (uint) {
        return ((contractIdCalc * topLevelMultiplier) + (nbossType * midLevelMultiplier) + specificNbossNum);
    }
