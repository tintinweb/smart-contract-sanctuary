...
contract MasterPool is Ownable {
  ...
  function add(uint _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
    ...
    require(pidByToken[address(_lpToken)].added == false, "MasterPool: already added");
    ...
    pidByToken[address(_lpToken)] = PoolPosition({
  ...
  function pendingRewards(uint _pid, address _user) external view returns (uint) {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint rewardTokenReward = multiplier * rewardTokenPerBlock * pool.allocPoint / totalAllocPoint;
