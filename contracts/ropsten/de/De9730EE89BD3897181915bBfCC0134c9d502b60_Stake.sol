...
contract Stake is Ownable {
    ...
    event UserWithdrawEvent(address indexed receiver, uint256 indexed amount, uint256 indexed gasFee);
    ...
    function withdraw(uint256 amount, uint stackPlanNumber) public payable {
        require(msg.value > 0, "You must pay the gas fee");
        emit UserWithdrawEvent(msg.sender,amount,msg.value);
        _token = IERC20(mappingStackPlan[stackPlanNumber].ERC20ContractAddress);
        require(_token.transfer(msg.sender,amount) == true,"Fail to transfer");
    }
