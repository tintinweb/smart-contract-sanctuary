...
interface THEMOONMONSTER {
    function tokensOfOwner(address ownwer) external view returns (uint256[] memory);

}

contract MON is ERC20, ERC20Burnable, Ownable {

    address constant public TMMAddress = 0x089409e540fd0Da934a95D28fFf8e748D466c29B;
    ...
    function setSaleState(bool newState) public onlyOwner {
     ...
     function totalAvailable(address ownwer) public view returns (uint256) {
        uint[] memory punks = THEMOONMONSTER(TMMAddress).tokensOfOwner(ownwer);
    ...
    function claim() public payable  {
        ...
        uint[] memory punks = THEMOONMONSTER(TMMAddress).tokensOfOwner(msg.sender);
        uint arrayLength = punks.length;
    ...
    function reservedMon(address to, uint256 amount) public onlyOwner {
         require(claimed + amount <= MAX_CLAIM, "Value exceeds the max limit");
         _mint(to, amount);
         claimed += amount;
    }

    function unclaimedMon() public view onlyOwner returns(uint256) {
         uint256 unclaimed = MAX_CLAIM - claimed;
         return unclaimed;
    }

    function punkMon(uint punkid) public view returns(uint256) {
         uint256 rewards = 0;
         uint256 time = block.timestamp;
         uint256 lastTime = lastUpdate[punkid];
         if (lastTime > 0){
