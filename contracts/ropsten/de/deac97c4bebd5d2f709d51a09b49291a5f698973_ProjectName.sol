...
contract ProjectName is AdminRole, VerifySignature{
  ...
  constructor () public {
    ...
    _token_price = 500000000000000000; //0.5 USDT * (10**18) = 500000000000000000 wei, where 18 is decimal of USDT

    // set the address that stores Tokens and signs data from the white list
    _signer_address = address(0x4dda2fF0A7D2B631C8EA212Dec750e5B73976266);
    ...
    project_token_address = address(0x562aC31bBa0F9C83F1f34cC4e854DBEe16ad2Bc4);
    _project_token = Token_interface(project_token_address);

    // set administrators
    _addAdmin(address(0xBa16E9E4c8718221050a50b62b7B5A33C2AF51De));
    _addAdmin(address(0x8882616bdf9095311b418Ce7c7Ab931632bA2E1B));
    _addAdmin(address(0xD58168dccCBB6BCb8C6B1337781E133E87F91D3D));
