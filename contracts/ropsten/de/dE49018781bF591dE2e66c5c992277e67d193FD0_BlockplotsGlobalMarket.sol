...
contract BlockplotsGlobalMarket is AccessControl, Initializable {
    ...
    // A record of the highest Blockplots bid
    // tileIndex => Bid
    mapping (uint => Bid) public bids;
    mapping (uint => GlobalBid) public globalbids;
    mapping (address => uint) public pendingWithdrawals;

    event BlockplotsTransfer(uint indexed index, address from, address to);
    ...
    function initialize() public initializer {
        ...
        mapSize = 33;
        blockplots = Blockplots(0x3d4a05d979567265A0ce3AA873bA96E78601Dc24);
    ...
    function globalbid() public payable {
    ...
    function withdrawGlobalBid(uint globalbidid) public {
    ...
    function acceptBid(uint8 col, uint8 row, uint minPrice) public {
        ...
        emit BlockplotsTransfer(index, msg.sender, bid.bidder);
    }
    
    function acceptGlobalBid(uint8 col, uint8 row, uint globalbidid) public {
        require(blockplots.getOwner(col, row) == msg.sender, "ACCEPT_BID::Only owner can accept bid");
        uint index = _index(col, row);
        GlobalBid memory globalbid = globalbids[globalbidid];
        ...
        emit BlockplotsTransfer(index, msg.sender, globalbid.bidder);
    }
