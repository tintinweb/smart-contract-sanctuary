...
contract SuperFuel is IBEP20, AntiLPSniper, LockableFunction, SmartBuyback {
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(tokenTracker.reward >= tokenSwapThreshold){
            ...
            address(reflectorContract).call{value: swappedCurrency}("");
            ...
            swapTokensForCurrency(toSwap);
            uint256 swappedCurrency = address(this).balance.sub(contractBalance);
            address(lotteryContract).call{value: swappedCurrency}("");
    ...
    function jackpot() external view returns(string memory jackpotReward, uint256 jackpotAmount, uint256 currentDrawNumber){
        jackpotReward = lotteryContract.rewardCurrency();
        jackpotAmount = lotteryContract.jackpotAmount();
        currentDrawNumber = lotteryContract.draw();
        return (jackpotReward, jackpotAmount, currentDrawNumber);
    }
