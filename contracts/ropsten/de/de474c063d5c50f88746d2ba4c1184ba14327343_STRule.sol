	...
	TrusteeManagedSC
{
	...
	string  public standard = "SmartTrust Rule v1.5";
	...
	struct Condition {
		...
		ConditionState stateByTrustee;
		ConditionState stateBySettlor;
	...
	event ConditionMetByTrustee(
		uint256 indexed conditionId,
		uint256 timestamp
	);

	event ConditionMetBySettlor(
		uint256 indexed conditionId,
		uint256 timestamp
	);

	event SettlorAddressUpdate(
	...
	constructor (uint256 _ruleID, address _assetTokenAddress, address[] memory _trustees, address _settlorAddress) public {
		...
		onlyUnexecuted()
	{
		...
		conditions.push( Condition(_conditionID, ConditionState.Unmet, ConditionState.Unmet, ConditionState.Unmet, st, _beneficiaries, _tokens, _parentConditionID) );
	...
	function conditionMetByTrustee(uint256 _conditionID)
		public
		onlyTrustee()
		onlyUnexecuted()
		onlyExistingCondition(_conditionID)
	{
		...
		require(conditions[condition_index].stateByTrustee == ConditionState.Unmet, "Condition already met by Trustee");

		conditions[condition_index].stateByTrustee = ConditionState.Met;
		emit ConditionMetByTrustee( conditions[condition_index].conditionID, now);

		_conditionCheckForMet(_conditionID);
	}

	function conditionMetBySettlor(uint256 _conditionID)
		public
		onlySettlor()
		onlyUnexecuted()
		onlyExistingCondition(_conditionID)
	{
		(uint256 condition_index, bool exists) = _conditionGetIndex(_conditionID);
		require(exists, "Condition not exists");
		require(conditions[condition_index].stateBySettlor == ConditionState.Unmet, "Condition already met by Settlor");

		conditions[condition_index].stateBySettlor = ConditionState.Met;
		emit ConditionMetBySettlor( conditions[condition_index].conditionID, now);

		_conditionCheckForMet(_conditionID);
	}

	function _conditionCheckForMet(uint256 _conditionID)
		internal
		onlyUnexecuted()
		onlyExistingCondition(_conditionID)
	{
		...
		if (conditions[condition_index].stateByTrustee == ConditionState.Met
			&& conditions[condition_index].stateBySettlor == ConditionState.Met)
		{
			_conditionMet(_conditionID);
		}
	}

	function _conditionMet(uint256 _conditionID)
		internal
		onlyUnexecuted()
		onlyExistingCondition(_conditionID)
	{
	...
	function settlorAddressUpdate(address _settlorAddress)
		public
		onlyTrustee()
		onlyUnexecuted()
	{
		settlorAddress = _settlorAddress;
		emit SettlorAddressUpdate(_settlorAddress, now);
	}
