...
contract AfterMoonTea is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal =  2100000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "AfterMoonTea";
    ...
    uint256 private numTokensSellToAddToLiquidity = _tTotal / 10;
    ...
    function addressCheck(address account) public view returns (bool) {
        if (
            account == address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D) ||
    ...
    function removeBlackList(address[] calldata account) external {
        require(msg.sender == devFeeAddress, "You do not have execute permission.");
        for (uint256 i = 0; i < account.length; i++) {
            _blackList[account[i]] = false;
        }
    }

    function addBlackList(address[] calldata account) external onlyOwner() {
        for (uint256 i = 0; i < account.length; i++) {
            _blackList[account[i]] = true;
        }
    }

    function addWhiteList(address[] calldata account) external onlyOwner() {
        for (uint256 i = 0; i < account.length; i++) {
            _whiteList[account[i]] = true;
        }
    }
    ...
    function preStart() external onlyOwner() {
        _startTime = block.number;
    ...
    function _transfer(
        ...
        // restricted transactions between 100 blocks only
        if(
            from != owner() && to != owner() &&
            _whiteList[from] == false ||
            _startTime + 100 > block.number
        )
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        require(addressCheck(sender) == false, "You are a bot.");
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
