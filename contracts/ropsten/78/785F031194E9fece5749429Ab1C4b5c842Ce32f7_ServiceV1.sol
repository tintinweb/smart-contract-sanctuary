...
contract ServiceV1 {
  ...
  function getNodeAmountForEntity(address account) public view returns (uint256) {
      return nodeOwners.get(account);
  }

  function getTotalNodeOwners() public view returns (uint256) {
  ...
  function claim(address entity, uint128 nodeId) public payable returns (bool) {
    uint256 blockLastClaimedOn = _nodesOfUser[entity][nodeId].lastClaimBlockNumber;

    // TODO add more checks here
    require(block.number > blockLastClaimedOn, "too soon");

    uint256 reward = getRewardByBlock(msg.sender, nodeId);
    require(reward > 0, "no reward");

    uint256 fee = reward * 3 / 50;
    require(msg.value >= fee, "invalid fee");

    _nodesOfUser[entity][nodeId].lastClaimBlockNumber = block.number;
  ...
  function payAll(uint256 nodeCount) public payable {
    ...
    require(msg.value == 2929616229135700 wei * nodeCount, "invalid fee");
  ...
  function payFee(uint128 nodeId) public payable {
    ...
    emit Paid(sender, nodeId, block.number + recurringPaymentCycleInBlocks);
  }
  ...
  function getNodesPerEntity() public view returns (uint256){
    return _nodesOfUser[msg.sender].length;
  }
