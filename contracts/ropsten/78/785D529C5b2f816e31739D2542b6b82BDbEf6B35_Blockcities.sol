...
contract Blockcities is ERC20, ERC20Burnable, Ownable{
    uint private _rate;
    uint private _investLimit = 50000;
    uint private _buyLimit = 50000;
    
   
    mapping (address => uint) securityTokens;
    event InvestEvent(address indexed _investor, string _assetDetails, uint _amount,string _duration, string _roi, uint256 _timestamp);
    event ReserveEvent(address indexed _depositor, string _assetDetails, uint _amount, uint256 _timestamp);
    event BuyEvent(address indexed assetOwnerAddress, string assetTitle, uint256 initailAssetTokenAmount, uint taxTokenAmount, uint feeTokenAmount, string unitNumber);

        // Public construtor of Token to be developed
    constructor() 

        // Passing ERC20 token details to the constructor
        ERC20("Blockcities", "BLKCT") {

            // Adding total supply in multiplication with desired decimal places <Total supply * (10 ** decimals)>
            uint totalSupply = 1000000000 * (10**2);
    ...
    function setInvestLimit(uint investlimit) onlyOwner public  {
    ...
    function setBuyLimit(uint buylimit) onlyOwner public  {
            _buyLimit = buylimit;
    }
    ...
    function getBuyLimit() public view returns(uint )  {
        return(_buyLimit);
    }
    ...
    function getSecurityToken(address investorAddress) public view returns(uint)  {
    ...
    function buyBlockcities(uint _amount) public {
    ...
    function invest(uint _amount,address _assetowner, string memory _duration, string memory _roi,string memory _assetDetails) public {
          require(_amount >= _investLimit, "Not enough balance");
          require(balanceOf(_msgSender()) >= _amount, "Not enough balance");
          _transfer(_msgSender(),_assetowner,_amount);
          securityTokens[_msgSender()] = securityTokens[_msgSender()] + _amount;
          emit InvestEvent(_msgSender(),_assetDetails, _amount, _duration, _roi, block.timestamp);
    }
    ...
    /** @dev func reserve()
     ...
     function reserve(uint _amount, address _receiver, string memory _assetDetails) public {
          require(balanceOf(_msgSender()) >= _amount, "Not enough balance");
          _transfer(_msgSender(),_receiver,_amount);
          emit ReserveEvent(_msgSender(), _assetDetails, _amount, block.timestamp);
    }
    ...
    function claimReturns(uint _amount,address _assetowner,address _investor, uint _initialAmount) public {
          require(balanceOf(_assetowner) >= _amount, "Not enough balance");
          _transfer(_assetowner,_investor,_amount);
    ...
    /** @dev func buyAsset()
     ...
     function buyAsset(uint256 initailAssetTokenAmount,address assetOwnerAddress,
     uint256 feeTokenAmount, uint256 taxTokenAmount, string calldata assetTitle, string calldata unitNumber)external returns(string memory, string memory) {
         _transfer(msg.sender,assetOwnerAddress,initailAssetTokenAmount);
         _transfer(msg.sender,owner(),feeTokenAmount);
         _transfer(msg.sender,owner(),taxTokenAmount);
         emit BuyEvent(_msgSender(),assetTitle,initailAssetTokenAmount,taxTokenAmount,feeTokenAmount,unitNumber);
         return (assetTitle, unitNumber);
     }
}