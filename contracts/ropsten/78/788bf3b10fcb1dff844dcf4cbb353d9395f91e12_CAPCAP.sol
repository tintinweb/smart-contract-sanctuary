...
contract CAPCAP is Context, IERC20, Ownable {
    ...
    uint256 private totalFee = liquidityFee.add(devFee);
    ...
    function _transfer(
        ...
        if(tradingOpen && to == uniswapV2Pair) { //sell
            currenttotalFee= _sellLiquidityFee.add(_sellDevFee);
        ...
        if (!inSwap && tradingOpen && to == uniswapV2Pair) {
            ...
            SCTokenBalance = balanceOf(address(this));       
        }
    ...
    function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
     ...
     function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        ...
        devFee = _devFee;
        totalFee = liquidityFee.add(devFee);

    }

    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
