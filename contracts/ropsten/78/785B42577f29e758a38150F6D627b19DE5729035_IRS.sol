...
contract IRS {
  ...
  function getCTokenExchangeRate() public view returns(uint256, uint256) {
  ...
  function createSwap(
                      uint256 notional,
                      uint256 addressPayer,
                      uint256 fundingPayer,
                      uint256 addressReceiver,
  ...
  //READ FUNCTIONS  
  function getPaymentFloating() public view returns(uint256) {
    (uint256 cTokenRate, ) = getCTokenExchangeRate();
    return swap.notional.mul(cTokenRate).div(swap.initCTokenRate);
  }

  function getSwapStartBlock() public view returns(uint256){
    return swap.startBlock;
  }

  function getSwapInitCTokenRate() public view returns(uint256){
    return swap.initCTokenRate;
  }

  function getSwapNotional() public view returns(uint256){
