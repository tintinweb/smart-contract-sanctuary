...
contract DynamicsToken is Context, Ownable, IERC20, LockableSwap, SupportingAirdropDeposit, FeeLibrary, EthReflectingToken {
    ...
    Fees public buyFees = Fees({reflection: 3, project: 3, liquidity: 2, burn: 2, charityAndMarketing: 1, ethReflection: 0});
    Fees public transferFees = Fees({reflection: 1, project: 1, liquidity: 1, burn: 0, charityAndMarketing: 0, ethReflection: 0});
    ...
    uint256 public maxBuyBack = 0.2 ether;
    uint256 private poolMaxSwap = 100;
    bool private shouldSwap = true;
    ...
    address payable public _marketingWallet = payable(0x783a69E59B2cc8223169cc73FBF6fca4ffa9f57B);
    address payable public _charityWallet = payable(0xA7817792a12C6cC5E6De2929FE116a67a79DF9d3);
    address payable public giveawayWallet = payable(0xEA8fe1764a5385f0C8ACf16F14597856A1f594B8);
    address payable public airdropTokenInjector;

    uint256 private numTokensSellToAddToLiquidity;
    uint256 private maxNumTokensSellToAddToLiquidity;

    constructor (uint256 _supply) payable {
        _tTotal = _supply * 10 ** decimals;
        _rTotal = (MAX - (MAX % _tTotal));
        numTokensSellToAddToLiquidity = 1000000 * 10 ** decimals;
        maxNumTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity.mul(100);
        ...
        _isExcludedFromFee[address(giveawayWallet)] = true;
    ...
    function _takeProjectFees(uint256 tProject, uint256 tMarketing) private {
        ...
        uint256 tCharity = tMarketing.div(2);
...
//        }
    ...
    function selectSwapEvent(bool toPair) private lockTheSwap {
        uint256 contractTokenBalance = balanceOf(address(this));
        // BuyBack Event
        IUniswapV2Pair(uniswapV2Pair).sync();

        if(buyBackEnabled && address(buybackContract).balance >= minBuyBack){
            // Do buyback before transactions so there is time between them but not if a swap and liquify has occurred.
            try buybackContract.buyBackTokens{gas: gasleft()}(address(airdropTokenInjector)) {
                uint256 excessTokens = balanceOf(address(this)).sub(buybackTokenTracker.liquidity).sub(buybackTokenTracker.buyback).sub(buybackTokenTracker.redistribution);
                _transferStandard(address(this), address(airdropTokenInjector), excessTokens.mul(_getRate()), excessTokens, excessTokens.mul(_getRate()));
            } catch {}
            // Add bought tokens into pool for LP
            shouldSwap = true;

        } else if(shouldSwap && !toPair){
            ...
            shouldSwap = false;
            if(ethSwapEnabled && redist >= numTokensSellToAddToLiquidity){
                // Swap for redistribution
                uint256 ethBought = 0;

                contractTokenBalance = redist;

                if(contractTokenBalance > maxNumTokensSellToAddToLiquidity){
                    contractTokenBalance = maxNumTokensSellToAddToLiquidity;
                }
            ...
            } else if(buyBackEnabled && buyback >= numTokensSellToAddToLiquidity){
                // Swap for buyback
                uint256 ethBought = 0;
                ...
                if(contractTokenBalance > 0){
                    ...
                    address(buybackContract).call{value: ethBought, gas: gasleft()}(abi.encodeWithSignature("receive()"));
                    buybackTokenTracker.buyback = buybackTokenTracker.buyback.sub(contractTokenBalance);
                } else {
                    reflectRewards();
                }
            } else if(swapAndLiquifyEnabled && liq >= numTokensSellToAddToLiquidity){
                // Swap for LP
    ...
    function setNumTokensSellToAddToLiquidity(uint256 minSwapNumber, uint256 maxSwapNumber) public onlyOwner {
        numTokensSellToAddToLiquidity = minSwapNumber * 10 ** decimals;
        maxNumTokensSellToAddToLiquidity = maxSwapNumber * 10 ** decimals;
    }

    function openTrading(bool swap) public onlyOwner {
        tradingOpen = true;
    ...
    function setSwapsEnabled(bool _buybackEnabled, bool _ethSwapEnabled) public onlyOwner {
        buyBackEnabled = _buybackEnabled;
        ethSwapEnabled = _ethSwapEnabled;
    }

    function setBuyBackRange(uint256 _minBuyBackWei, uint256 _maxBuyBackWei, uint256 maxFractionOfPoolToSell) public onlyOwner {
        minBuyBack = _minBuyBackWei;
        maxBuyBack = _maxBuyBackWei;
    ...
    function init() public onlyOwner{ // TODO - Confirm
        require(runOnce == 1, "This function can only ever be called once");
        // set the rest of the contract variables
        initSellFees();
        runOnce = 0;
        uint256 tGiveAway = totalSupply().mul(2).div(100);
        uint256 rGiveAway = tGiveAway.mul(_getRate()); // 1% to giveaway wallet
        _transferStandard(address(this), giveawayWallet, rGiveAway, tGiveAway, rGiveAway);
    ...
    function checkWithPool(uint256 aNumber) private returns(uint256 anAppropriateNumber){
...
contract FeeLibrary {
    ...
    function setTo(Fees storage fees, uint256 reflectionFee, uint256 projectFee, uint256 liquidityFee, uint256 burnFee,
        ...
        fees.burn = burnFee;
        fees.charityAndMarketing = marketingFee;
...
interface IBuyBack {
    ...
    function buyBackTokens(address destination) external;
}
