...
contract CivInvestment is ICivInvestment
{
    ...
    function newTrade(
        ...
        emit NewLogInt24(tick, "first tick");

        // emit NewLogInt24(pool.tickSpacing(), "tick spacing");
...
interface ICivInvestment is ICivDevEvent {
    ...
    */
    ...
    event NewTradeOpened(address pool, address token0, address token1, uint256 fee, uint256 nftId);

    /// @notice Returns the current owner of the CivInvestment
    /// @dev Can be changed by the current owner via SwapContractOwner
    /// @return The address of the factory owner
    function owner() external view returns (address);

    /**
     * Will swap the owner, the owner will be the only one that will have the ability to 
     * call the actions to swap ETH from the Holder contract, to the Uniswap V3 of interest
     */
    function swapContractOwner(address newOwner) external;
