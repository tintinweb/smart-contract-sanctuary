...
contract UBXTStaking is Ownable {
    ...
    /**
     * this method will be called when performance fee is distributed
     */
    function distributeToken(
        address developer, uint256 amountForDeveloper, 
        address reserved, uint256 amountForReserved,
        address burning, uint256 amountForBurning,
        address pool, uint256 amountForPool) public  {
        safeTokenTransfer(developer, amountForDeveloper);
        // IERC20(token).transferFrom(treasuryAddress, reserved, amountForReserved);
        // IERC20(token).transferFrom(treasuryAddress, burning, amountForBurning);
        // IERC20(token).transferFrom(treasuryAddress, pool, amountForPool);
