    ...
    Multicall
{
    ...
    modifier reportersOnly() {
        require(
            hasRole(REPORTER_ROLE, _msgSender()),
            "must have reporter role"
        );
        _;
    }
    ...
    constructor() {
        ...
        _setupRole(REPORTER_ROLE, _msgSender());
        _baseURI = string(
            abi.encodePacked(
...
// Replace with with upstream version when https://github.com/OpenZeppelin/openzeppelin-contracts/pull/2608 is merged
    ...
    ERC1155Pausable
{
    ...
    AgreementsBeacon public agreementsBeacon;

    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `MINTER_ROLE`, and `PAUSER_ROLE` to the account that
     * deploys the contract.
     */
    constructor(string memory uri, AgreementsBeacon beacon) ERC1155(uri) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    ...
    function mint(
        ...
        require(
            agreementsBeacon.getBeaconCreator(address(this), id) ==
                _msgSender(),
    ...
    function mintBatch(
        ...
        require(
            agreementsBeacon.getBeaconCreator(address(this), ids[0]) ==
                _msgSender(),
            "minter must own the beacon"
        );
    ...
    /**
     * @dev Hook that fires before tokens move wallets.
     */
    function _beforeTokenTransfer(
        address operator,
        address from,
        ...
        if (from != address(0)) {
            require(
                // TODO: build bulk receiver in AgreementsBeacon
                agreementsBeacon.getAgreementStatus(
                    address(this),
                    ids[0],
                    to
