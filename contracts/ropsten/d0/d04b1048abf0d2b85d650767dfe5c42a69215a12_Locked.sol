...
contract Locked is ERC20, ProofOfAgeProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;

    string public name = "Locked";
    string public symbol = "LOCKED";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(vaultWallet == msg.sender && block.timestamp < unlockDate) revert();{
            if(vaultWallet == msg.sender && block.timestamp > unlockDate) return true;
        }
