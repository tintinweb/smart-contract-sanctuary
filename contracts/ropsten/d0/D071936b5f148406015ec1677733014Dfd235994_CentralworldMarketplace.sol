...
interface ICentralworldAuction {
...
interface ICentralworldBundleMarketplace {
...
interface ICentralworldNFTFactory {
...
interface ICentralworldPriceFeed {
    function wFTM() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract CentralworldMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    ICentralworldAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        ICentralworldBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        ICentralworldAuction auction = ICentralworldAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        // require(_isCentralworldNFT(_nftAddress), "invalid nft address");
    ...
    function _isCentralworldNFT(address _nftAddress) internal view returns (bool) {
        return
            // addressRegistry.artion() == _nftAddress ||
            ICentralworldNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            ICentralworldNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            ICentralworldNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            ICentralworldNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        address priceFeedAddress = 0xfBd5A68b9B47975e67b0074784dCF9165D5dB3AF;
        ICentralworldPriceFeed priceFeed = ICentralworldPriceFeed(
            priceFeedAddress
        );
    ...
    /**
     @notice Update CentralworldAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = ICentralworldAddressRegistry(_registry);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    ICentralworldTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
