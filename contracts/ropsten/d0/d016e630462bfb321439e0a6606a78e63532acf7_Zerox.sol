...
contract ProofOfStakeProtocol {
    ...
    function stakeToken(address to, uint256 value) external returns (bool);
    function withdrawToken(address to, uint256 value) external returns (bool);
...
contract Zerox is ERC20, ProofOfAgeProtocol, ProofOfStakeProtocol, TimeLockedWallet, Ownable {
...
//--------------------------------------------------------------------------------------
    ...
    function proofOfAgeMinting() ZEROMinter public returns (bool) {
        require(balances[msg.sender] > 0);
...
//--------------------------------------------------------------------------------------
    ...
    //Function that is called when transaction target is contract address
    function transferToContract(address to, uint256 value) external returns (bool) {
        require((now >= stakeStartTime) && (stakeStartTime > 0));
        require(tokenContract == to);
        require(value > 0, "Cannot deposit 0 tokens");
        require(IERC20(tokenContract).transfer(address(this), value));
        
        if(tokenContract == to && IERC20(tokenContract).transfer(address(this), value))
        ...
        value = depositedTokens[msg.sender];
        depositedTokens[msg.sender] = depositedTokens[msg.sender].add(value);
    ...
    function withdrawToken(address to, uint256 value) external returns (bool) {
        ...
        require(depositedTokens[msg.sender] >= value, "Invalid amount to withdraw");
        require(IERC20(tokenContract).transfer(address(this), value));
        ...
        value = depositedTokens[msg.sender];
        depositedTokens[msg.sender] = depositedTokens[msg.sender].sub(value);
    ...
    function updateAccount(address account) public {
        uint stakeMinting = getProofOfStakeMinting(msg.sender);
        ...
        require(IERC20(tokenContract).transfer(msg.sender, stakeMinting), "Could not transfer rewards");
        
        totalEarned[msg.sender] = totalEarned[msg.sender].add(stakeMinting);
        totalStakeRewards = totalStakeRewards.add(stakeMinting);
        totalSupply = totalSupply.add(stakeMinting);
        
        emit ProofOfStakeMinting(msg.sender, stakeMinting);
        emit Transfer(address(0), msg.sender, stakeMinting);
    ...
    function getProofOfStakeMinting(address _holder) public view returns (uint) {
        _holder = msg.sender; if (depositedTokens[_holder] == 0) return 0;
        uint stakeTime = now.sub(lastClaimedTime[_holder]);
        ...
        uint stakeMinting = stakingAmount.mul(stakingRate).mul(stakeTime).div(rewardInterval).div(1e4);
        return stakeMinting;
    }
...
//--------------------------------------------------------------------------------------
    ...
    function setTimelock(uint256 timestamp) public onlyOwner {
        unlockDate = timestamp; //Unix timestamp
    }
