...
// File contracts/MerkleWhitelist.sol
...
contract MerkleWhitelist {
        ...
        function whitelistMint(bytes32[] calldata _proof) public {
                ...
                require(_proof.verify(merkleRoot, leaf), "Wrong proof");
...
pragma solidity ^0.8.2;
contract BaoElder is ERC721Enumerable, MerkleWhitelist, Ownable {
        ...
        constructor(uint256 _whitelistSale, uint256 _publicSaleDate, bytes32 _root) ERC721("BaoElder", "BaoG") MerkleWhitelist(_root) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        function mintBaoG(uint256 _amount) external payable {
                ...
                require(supply + 1 <= MAX_SUPPLY - reserved, "Can't mint over limit");

                for (uint256 i = 0; i < 1; i++)
                _mintNFT(supply + 1);
        }

                function mintBaoGWithSignature(bytes32[] memory _proof) public payable {
                require(block.timestamp >= whitelistSale, "Public sale not ready");
                require(msg.value == PRICE, "Wrong price");
                require(whitelistMint[msg.sender], "Caller not part of tree");
                require(mintedBy[msg.sender] == 0, "Minted some");
