...
contract TokenYou is Ownable,ITokenYou {
    ...
    uint256 private _max_supply = 2*10**14;
   ...
   */
    ...
    function mint(address recipient, uint256 amount) external override isMinter{
        require(_totalSupply.add(amount) <= _max_supply, 'YouSwap: EXCEED_MAX_SUPPLY');
    ...
    function resetMaxSupply(uint256 newValue) external override onlyOwner{
        require(newValue > _max_supply, 'YouSwap: NOT_ALLOWED');
        emit MaxSupplyChanged(_max_supply,newValue);
        _max_supply = newValue;
    }
}