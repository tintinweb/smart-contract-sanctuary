...
contract Game is ExchangeAdmin {
    ...
    function placeBet(address a, uint value, string betSide) public payable {
        sideData[StringYokes.zint_convert(betSide)].totalPledged+=value;
        allSidesPledged+=value;
        if (sideData[StringYokes.zint_convert(betSide)].contribDb[a].used) {
            value+=sideData[StringYokes.zint_convert(betSide)].contribDb[a].contribAmount;
        }
        else {
            sideData[StringYokes.zint_convert(betSide)].usedAddresses.push(a);
            sideData[StringYokes.zint_convert(betSide)].contribDb[a].used=true;
        }
        sideData[StringYokes.zint_convert(betSide)].contribDb[a].contribAmount+=value;
    }
    ...
    function dish(string winner, address profit) public onlyAdmin payable returns (address[10], uint[10]) {
        address[10] memory a;
        uint[10] memory b;
        expired = true;
...
contract BEthy is ExchangeAdmin {
    ...
    function getSides(string str, uint j) view public returns (string) {
        return StringYokes.zint_bytes32ToString(current[references[StringYokes.zint_convert(str)]].getGameSides()[j]);
    }
    ...
    function endGame(uint gameId, string winningSide, address beneficiary) public onlyAdmin returns (address[10], uint[10]) {
        //current[gameId].value(2);
        //
        require(current[gameId].isNotExpired());
        return current[gameId].dish(winningSide, beneficiary);
    }
    function endGame(uint gameId, uint winningId, address profit) public onlyAdmin returns (string) {
