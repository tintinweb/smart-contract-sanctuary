...
contract LostPunks is ERC721Enumerable, Ownable {
    ...
    function mintLostPunks(uint16 fatherIndex, uint16 motherIndex) external {
        ...
        require(child1Mapping[fatherIndex] == 0, "Previously minted children for father");
        require(child1Mapping[motherIndex] == 0, "Previously minted children for mother");
        require(siblingMapping[motherIndex] != fatherIndex, "Cannot mint children for siblings");
        ...
        _mint(msg.sender, child1Index);
        _mint(msg.sender, child2Index);
    }
    ...
    function tokenURI(uint256 index) public view override returns (string memory)
    {
        ...
        string memory json = base64Encode(bytes(string(abi.encodePacked(
            '{"name": "LostPunk #',
            toString(index), 
            '", "description": "LostPunks are the next generation descendants of the original CryptoPunks. Metadata and images 100% generated and stored on-chain. Inspired by but not affiliated with Larva Labs.", "image": "data:image/svg+xml;base64,', 
    ...
    function punkAssetsAttributes(bytes memory punkAssets, uint16 index) internal view returns (string memory text) {
        ...
        if (index < punksCount) {
            ...
            text = string(abi.encodePacked(text, ", Father: ", sibling > 0 ? toString(fatherMapping[index]) : "-"));
            text = string(abi.encodePacked(text, ", Mother: ", sibling > 0 ? toString(motherMapping[index]) : "-"));
    ...
    function appendAttribute(string memory text, string memory key, string memory value, bool asString, bool append) internal pure {
        string memory quote = asString ? '"' : '';
        string memory attribute = 
            string(abi.encodePacked('{ "trait_type": "', key, '", "value": ', quote, value, quote, ' }'));
        if (append) {
            text = string(abi.encodePacked(text, ', ', attribute));
        } else {
            text = attribute;
        }
    }
    ...
    function metadataAttributes(bytes memory punkAssets, uint16 index) internal view returns (string memory text) {
        ...
        for (uint8 j = 0; j < 8; j++) {
            ...
            if (asset > 0) {
                if (j > 0) {
                    ++accessoryCount;
                    appendAttribute(text, "Accessory", getAssetName(asset), true, true);
                } else {
                    appendAttribute(text, "Type", getAssetName(asset), true, false);
                }
            } else {
                break;
            }
        }
        appendAttribute(text, "# Traits", toString(accessoryCount), false, true);
        appendAttribute(text, "Generation", toString(generationMapping[index]), false, true);
        uint16 sibling = siblingMapping[index]; 
        if (sibling > 0) {
            appendAttribute(text, "Father", toString(fatherMapping[index]), false, true);
            appendAttribute(text, "Mother", toString(motherMapping[index]), false, true);
        }
    }
    ...
    function colorForGeneration(uint16 index) internal pure returns (bytes4) {
        uint32[9] memory colors = [
            0x00000000, 0xFAD121FF, 0xA070DDFF,
    ...
    string private constant SVG_HEADER = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 360 360">';
    ...
    function punkAssetsImageSvg(bytes memory punkAssets, uint16 index) internal view returns (string memory svg) {
        ...
        if (bgColor[3] > 0) {
            svg = string(abi.encodePacked(svg, rectSvg(0, 0, 360, 360, bgColor)));
        }
        for (uint y = 0; y < 24; y++) {
            for (uint x = 0; x < 24; x++) {
                uint p = (y * 24 + x) * 4;
                if (uint8(pixels[p + 3]) > 0) {
                    ...
                    svg = string(abi.encodePacked(svg, rectSvg(x * 15, y * 15, 15, 15, color)));
                }
            }
        }
