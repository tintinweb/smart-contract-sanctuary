...
contract Detf is DetfReflect {
    ...
    function _moveDelegates(address srcRep, address dstRep, uint256 amount, bool delegateVotes) internal {
        uint256 srcRepBalance = balanceOf(srcRep);

        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                uint256 srcRepNum = numCheckpoints[srcRep];
                uint256 srcRepOld = srcRepNum > 0 ? checkpoints[srcRep][srcRepNum - 1].votes : 0;
                // uint256 srcRepNow = srcRepBalance > srcRepOld ? 0 : 0;
                uint256 srcRepNew = delegateVotes ? 0 : srcRepBalance; //= srcRepOld.sub(amount);
