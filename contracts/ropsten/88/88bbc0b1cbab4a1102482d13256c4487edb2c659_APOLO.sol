...
contract APOLO is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "APOLLO Protocol_TEST";
    string private _symbol = "ASPO2a";
    ...
    uint256 internal _tokenTotal = 100_000e21; // 100_000_000e18  
    ...
    uint256 public lastTaxDecreasedTime = block.timestamp;
    ...
    constructor(address burnWallet_, address _dev) {
        ...
        _burnWalet = burnWallet_;
        
        balancer = address(new Balancer());
        
        isExcludedFromFee[_msgSender()] = true;
        isExcludedFromFee[_burnWalet] = true;
        ...
        _reflectionBalance[_dev] = _reflectionTotal;
        
        emit Transfer(address(0), _dev, _tokenTotal);
    }
        ...
        returns (uint256)
    {
        ...
        if (!deductTransferFee) {
            ...
            return
                tokenAmount.sub(tokenAmount.mul(_taxFee).div(10000)).mul(_getReflectionRate());
        }
    }
    ...
    function excludeAccount(address account) external  {
        require(
            ...
            "APOLO: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'APOLO: The contract it self cannot be excluded');
        require(!_isExcluded[account], "APOLO: Account is already excluded");
    ...
    function includeAccount(address account) external  {
        require(_isExcluded[account], "APOLO: Account is already included");
    ...
    function _transfer(
        ...
        require(block.timestamp > liquidityAddedAt + 7 minutes  || amount <= 1200e18, "You cannot transfer more than 1200 tokens.");
        
        //@dev check if 7 days has passed since lastTaxDecreasedTime and decrease the tax fee by 2%, limit of tax fee decrement is set to 2%
        if(tradingEnabled && lastTaxDecreasedTime != 0 && block.timestamp > lastTaxDecreasedTime + 7 days && _taxFee > 200 && _liquidityFee > 200){
            _taxFee = _taxFee.sub(200);
            _liquidityFee = _liquidityFee.sub(200);
            lastTaxDecreasedTime = block.timestamp;
        }
     ...
     */
    function _transf(address sender, address recipient, uint256 amount) internal {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");

        _tokenBalance[sender] = _tokenBalance[sender].sub(amount, "ERC20: transfer amount exceeds balance");
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_taxFee != 0){
            ...
            uint _burnAmount;
            uint _liquidityReward;
            // Here the Fees are collected [holders, lp uni, dev]
            // DevWallet = 4%, LPholders 3%, Tholders 1%
            
            if(_taxFee == 800){
                _taxFeeTotal = _taxFeeTotal.add(taxFee.div(8));
                _burnAmount = taxFee.div(2);
                _liquidityReward = taxFee.mul(375).div(1000);
            }
            else if(_taxFee == 600){
                _burnAmount = taxFee.div(3);
                _taxFeeTotal = _taxFeeTotal.add(taxFee.div(6));
                _liquidityReward = taxFee.div(2);
            }
            else{
                _taxFeeTotal = _taxFeeTotal.add(taxFee.div(4));
                _liquidityReward = taxFee.div(2);
                _burnAmount = taxFee.div(4);
            }
            
            // Reward LP holders
            _rewardLiquidityProviders(_liquidityReward);
            // Burn Wallet
    ...
    function buyAndBurnToken(uint256 contractBalance) private lockTheSwap {
        ...
        uint256 rewardForCaller = swapedTokens.mul(_rebalanceCallerFee).div(10000);
    ...
    function _rewardLiquidityProviders(uint256 liquidityRewards) private {
        ...
        _transf(address(this), uniswapV2Pair, liquidityRewards);
    ...
    // ------------------------------------------------------------------------
    function missedTokens(address _tokenAddress) public onlyOwner returns(bool success) {
        uint _value = IERC20(_tokenAddress).balanceOf(address(this));
