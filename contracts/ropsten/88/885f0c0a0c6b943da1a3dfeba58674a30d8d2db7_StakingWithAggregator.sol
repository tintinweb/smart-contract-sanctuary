...
contract Staking is ReentrancyGuard {
  ...
  struct TermInfo {
    int128 stakeAdd; // the total added amount of stake which will be merged to stakeSum at the term+1
  ...
  function _stakeOrUnstake(address account, int128 amount) internal {
    ...
    _termInfoList[term].stakeAdd = _termInfoList[term].stakeAdd.add(amount).toInt128(); // added when the term is shifted (global)
  }

  function _stake(address account, uint128 amount) internal {
    _stakeOrUnstake(account, amount.toInt128());
  ...
  function _getNextStakeSum() internal view returns (uint128 nextStakeSum) {
    ...
    return addDiff(currentTermInfo.stakeSum, currentTermInfo.stakeAdd).toUint128();
  }
  ...
  function _getNextTotalRemaining() internal view returns (uint256 nextTotalRemaining) {
    ...
    TermInfo memory currentTermInfo = _termInfoList[_currentTerm];
    // nextTotalRemaining = currentBalance - (stakeSum + stakeAdd)
    //                    = currentBalance - (stakeSum + (stakeAdd > 0 ? stakeAdd : 0) + (stakeAdd < 0 ? stakeAdd : 0)
    nextTotalRemaining = addDiff(
      _totalRemaining + _getTotalRewardAdded(),
      (currentTermInfo.stakeAdd < 0 ? -currentTermInfo.stakeAdd : 0)
    );
  }
   ...
   */
  function getStakingTokenAddress() external view returns (address stakingTokenAddress) {
    ...
    )
  {
    ...
    nextTermStaking = addDiff(termInfo.stakeSum, termInfo.stakeAdd).toUint128();
  }
    ...
    )
  {
    ...
    currentReward = accountInfo.remaining;
    uint256 nextLatestTermRewards = _getNextTermReward();
    depositAmount = addDiff(accountInfo.stakeAmount, accountInfo.added).toUint128();
    withdrawableStakingAmount = depositAmount;

    TermInfo memory termInfo = _termInfoList[_currentTerm];
    uint256 nextStakeSum = addDiff(termInfo.stakeSum, termInfo.stakeAdd);
    nextLatestTermUserRewards = nextStakeSum == 0
      ? 0
      : nextLatestTermRewards.mul(depositAmount) / nextStakeSum;
  }
