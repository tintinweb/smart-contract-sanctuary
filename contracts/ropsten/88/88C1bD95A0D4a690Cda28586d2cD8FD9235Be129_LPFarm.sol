...
abstract contract Distribution is TwoStageOwnable {
    ...
    struct Account {
        ...
        uint256 withdrawalStartsAt;
        uint256 withdrawalEndsAt;
    ...
    function estimateWithdrawableRewards(address address_) public view returns (uint256) {
        Account storage account = _accounts[address_];
        if (account.claimedRewards == 0) return account.withdrawableRewards;
        uint256 timestamp = getTimestamp();
        uint256 unlockedRewards = account.withdrawalEndsAt <= timestamp
            ? account.claimedRewards
            : (account.claimedRewards * (timestamp - account.withdrawalStartsAt)) / withdrawalPeriod;
        uint256 newWithdrawableRewards = unlockedRewards - account.unlockedRewards;
        return account.withdrawableRewards + newWithdrawableRewards;
    }
    ...
    function claim(uint256 amount) external onlyInitialized returns (bool success) {
        ...
        unlockRewards();
        uint256 timestamp = getTimestamp();
        account.withdrawalStartsAt = timestamp;
        account.withdrawalEndsAt = timestamp + withdrawalPeriod;
    ...
    function unlockRewards() public onlyInitialized returns (bool success) {
        address caller = msg.sender;
        Account storage account = _accounts[caller];
        if (account.claimedRewards == 0) return true;
        uint256 timestamp = getTimestamp();
        uint256 unlockedRewards = account.withdrawalEndsAt <= timestamp
            ? account.claimedRewards
            : (account.claimedRewards * (timestamp - account.withdrawalStartsAt)) / withdrawalPeriod;
        uint256 newWithdrawableRewards = unlockedRewards - account.unlockedRewards;
        if (newWithdrawableRewards == 0) return true;
        account.withdrawableRewards += newWithdrawableRewards;
        emit RewardsUnlocked(caller, newWithdrawableRewards);
        account.unlockedRewards = unlockedRewards;
        if (account.unlockedRewards == account.claimedRewards) {
            account.claimedRewards = 0;
            account.unlockedRewards = 0;
        }
        return true;
    }
    ...
    function withdraw(uint256 amount) external onlyInitialized returns (bool success) {
        ...
        address caller = msg.sender;
        Account storage account = _accounts[caller];
        require(account.withdrawableRewards >= amount, "Requested amount exceeds claimed amount");
        account.withdrawableRewards -= amount;
