...
import {ERC20Interface} from "./interfaces/external/ERC20Interface.sol";
import {
    Actions,
...
contract Opeth is ERC20Upgradeable {
    ...
    uint public underlyingDecimals;
        ...
        external
    {
        ...
        __setGovernance(_governance);

        // has initializer modifier
        __ERC20_init_unchained(_name, _symbol); // initializes _decimals=18
        ...
        require(now < _expiryTimestamp, "OTOKEN_EXPIRED");
        require(isPut, "NOT_PUT");

        controller = ControllerInterface(AddressBookInterface(_addressBook).getController());
    ...
    function mintFor(address _destination, uint _amount) public {
        ...
        underlyingAsset.safeTransferFrom(
            msg.sender,
            address(this),
            oTokenToUnderlyingQuantity(_oToken)
        );
        uint fee = _amount.mul(mintFee).div(FEE_PRECISION);
        accumulatedFee = accumulatedFee.add(fee);
        _mint(_destination, _amount.sub(fee));
    }
    ...
    function isSettlementAllowed() public view virtual returns (bool) {
        return controller.isSettlementAllowed(address(oToken));
    }

    function getOpethDetails(uint _oToken) external view returns(IERC20, address, uint) {
        return (underlyingAsset, address(oToken), oTokenToUnderlyingQuantity(_oToken));
    }
    ...
    function collectFee() external { // publicly callable
    ...
    function setFee(uint fee) external onlyGovernance {
        require(fee <= FEE_PRECISION, "FEE_INVALID");
        mintFee = fee;
    }
    ...
    function __setGovernance(address _governance) internal {
        require(_governance != address(0), "NULL_GOVERNANCE");
