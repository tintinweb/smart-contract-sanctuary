...
interface BlockList{
    function isblockAddr(address _account) external view  returns(bool);
}
...
contract Bridge is Ownable{
    ...
    event SetAdressParms(address _storehouse, address _signatoryAddres, address _WETH,address _blocklist);
    event SetControllerAddr(address _cctoken, address _controller);
    constructor(address _WETH, address _signatoryAddres, address _configurationController, BlockList _blocklist) {
        WETH = _WETH;
    ...
    //修改仓库地址,签名地址,WETH地址
    function setAdressParms(address _storehouse, address _signatoryAddress, address _WETH, BlockList _blocklist) public onlyConfigurationController{
        ...
        require(address(_blocklist) != address(0), "the blacklist address is zero address");
        storehouse = _storehouse;
        signatoryAddres = _signatoryAddress;
        WETH = _WETH;
        blocklist = _blocklist;
        emit SetAdressParms(_storehouse, _signatoryAddress, _WETH, address(_blocklist));
    }
    ...
    function myencode(uint16 a, uint16 b, uint64 c) public pure returns (bytes memory) {
     ...
     function exchange(exchangeInfo memory info) public payable returns(bool){
        ...
        require(!blocklist.isblockAddr(info._tokenA) && !blocklist.isblockAddr(info._to), "the caller or to address is blacklist address");
    ...
    function transferAndMint(transferAndMintInfo memory info) public onlyConfigurationController returns(bool){
        ...
        require(!blocklist.isblockAddr(info._to), "the caller or to address is blacklist address");
