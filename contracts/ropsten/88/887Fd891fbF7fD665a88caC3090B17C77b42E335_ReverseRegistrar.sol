...
contract ReverseRegistrar is Ownable, Controllable {
    ...
    /**
     * @dev Constructor
     * @param mystAddr The address of the MYST registry.
     * @param resolverAddr The address of the default reverse resolver.
     */
    constructor(MYST mystAddr, NameResolver resolverAddr) {
        myst = mystAddr;
        defaultResolver = resolverAddr;

        // Assign ownership of the reverse record to our deployer
        ReverseRegistrar oldRegistrar = ReverseRegistrar(
            myst.owner(ADDR_REVERSE_NODE)
        );
    ...
    modifier authorised(address addr) {
        require(
            addr == msg.sender ||
                ...
                myst.isApprovedForAll(addr, msg.sender) ||
                ownsContract(addr),
    ...
    /**
     * @dev Transfers ownership of the reverse MYST record associated with the
     *      calling account.
     * @param owner The address to set as the owner of the reverse record in MYST.
     * @return The MYST node hash of the reverse record.
     */
    ...
    /**
     * @dev Transfers ownership of the reverse MYST record specified with the
     *      address provided
     * @param addr The reverse record to set
     * @param owner The address to set as the owner of the reverse record in MYST.
     * @param resolver The address of the resolver to set; 0 to leave unchanged.
    ...
    /**
     * @dev Sets the `name()` record for the reverse MYST record associated with
    ...
    function setNameForAddr(
        ...
        defaultResolver.setName(node, name);
        myst.setSubnodeOwner(ADDR_REVERSE_NODE, sha3HexAddress(addr), owner);
        return node;
    }

    // test getNames
    /**
     * Read only function to return myst name only if both forward and reverse resolution are set     *
     */
    /* function getNames(address[] calldata addresses) external view returns (string[] memory r) {
        ...
        for(uint i = 0; i < addresses.length; i++) {
            ...
            address resolverAddress = myst.resolver(node);
            if(resolverAddress != address(0x0)){
                ...
                address forwardResolverAddress = myst.resolver(namehash);
