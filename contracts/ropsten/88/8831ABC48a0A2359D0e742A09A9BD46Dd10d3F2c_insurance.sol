...
contract insurance {
    ...
    function ApplyClaim(address payable _claimant ,uint _cost, string memory _description) public payable {
     ...
     //checks if the claim description matches any from validClaims
      if(checkClaimDescription(tolower(_description)) != true){
          claims[_claimant].description = string(abi.encodePacked(claims[_claimant].description,", is an invalid claim."));
          claims[_claimant] = TreatmentClaim(_cost,_description,"Rejected");
      }
      else if(balance[owner] < _cost * 1 ether){
          claims[_claimant].description = string(abi.encodePacked(claims[_claimant].description,", insufficient balance"));
          claims[_claimant] = TreatmentClaim(_cost,_description,"Rejected");
      }
      else{
     ...
     */
    function ViewYourClaim(address _claimant) view public returns (uint, string memory, string memory) {
      require(msg.sender == _claimant,"You can only view your own claims");
      return(claims[_claimant].cost, claims[_claimant].description, claims[_claimant].approval);
    }
    
    /**
     * Allows the insurance company to view any claim
     */
    function InsurerViewClaimDetail(address ins) view public returns (uint, string memory, string memory) {
