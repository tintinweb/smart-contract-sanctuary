...
library SafeMath {// это просто нужная для работы библиотека
...
contract Token is Context, IERC20, IERC20Metadata { //собственно сам контракт токена
    ...
    string public _name = "GOG"; //Полное имя токена
    
    string public _symbol= "GOG";//краткое имя токена
    
    bool public allowanceEnabled = false; // переменная блокирующая трансфер токенов между кошельками
...
contract Presale is Token {
    ...
    uint256 public SoftCap       =         2*10**18; //софт кап минимальное токенов BNB не набрав которое мы не сможем вывести деньги
   ...
   constructor ()  {
    ...
    function bayback() public payable  {
            ...
            uint256 tokensBNB =  _balances[msg.sender] / Price; //узнаем сколько выплатить BNB по балансу запросившего
