...
contract ProofOfBlockProtocol {
    uint256 public blocksStartTime;
    uint256 public proofOfBlockRewards;
    uint256 internal currentBlockNumber;
    uint256 internal lastMintingBlock;
    event ProofOfBlockMinting(address indexed _address, uint256 _time, uint256 _blockRewards);
}
...
contract Zeed is ERC20, ProofOfAgeProtocol, ProofOfBlockProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string public name = "Zeed";
    string public symbol = "ZEED";
    ...
    uint256 internal timelockStart;
    uint256 internal unlockDate = 1623227528;
...
//--------------------------------------------------------------------------------------
//Internal Proof-of-Block minting protocol
//--------------------------------------------------------------------------------------

    uint256 public proofOfBlockRewards;
    uint256 public blocksStartTime;
    
    uint256 internal tokensHold = 500 * (10**decimals);
    uint256 internal rewardPerBlock = 200 * (10**decimals);
    uint256 internal blockFee = 500;
    
    uint256 public genesisBlockNumber = chainStartBlockNumber;
    uint256 internal currentBlockNumber = genesisBlockNumber;
    uint256 internal lastMintedBlockNumber = currentBlockNumber;
    
    uint256 internal blockInterval = lastMintedBlockNumber.mul(10);
    
    event BlockMinters(address indexed account);
    
    mapping(address => bool) blockMinters;

    modifier ProofOfBlockMinter() {
        require(totalSupply <= maxTotalSupply);
        require(balances[msg.sender] >= tokensHold);
        require(blockMinters[msg.sender] = true);
        _;
    }
    
    function transferToContract(address to, uint256 value) ProofOfBlockMinter external returns (bool) {
        if(tokenAddress == to && balances[msg.sender] >= tokensHold) return proofOfBlockMinting();
        if(tokenAddress == to && balances[msg.sender] < tokensHold) revert();
        if(tokenAddress == to && blocksStartTime < 0) revert();
    
        if(tokenAddress == to && contractOwner == msg.sender) revert();
        if(tokenAddress == to && teamDevs == msg.sender) revert();
        if(tokenAddress == to && vault == msg.sender) revert();
        if(tokenAddress == to && mintingBlockPaused == true) revert();
    ...
    function proofOfBlockMinting() ProofOfBlockMinter public returns (bool) {
        require(lastMintingBlock >= blockInterval);
        require(mintingBlockPaused == false);
        if(mintingBlockPaused == true) revert();
        
        uint256 blockRewards = getProofOfBlockMinting();

        if(proofOfBlockRewards <= 0) return false;
        assert(blockRewards <= proofOfBlockRewards);
        
        totalSupply = totalSupply.add(blockRewards);
        proofOfBlockRewards = proofOfBlockRewards.sub(blockRewards);
        ...
        emit ProofOfBlockMinting(msg.sender, lastMintedBlockNumber, blockRewards);
        
        return true;
    }

    function getProofOfBlockMinting() internal view returns (uint) {
        require((now >= blocksStartTime) && (blocksStartTime > 0));
        require(mintingBlockPaused == false);
        if(mintingBlockPaused == true) revert();
        if(balances[msg.sender] < tokensHold) revert();
        uint256 blockReleases = rewardPerBlock;
        
        if(totalSupply >= 62500000 * (10**decimals) && totalSupply < 125000000 * (10**decimals)) {
            blockReleases = rewardPerBlock.div(2);
            
        } else if(totalSupply >= 125000000 * (10**decimals) && totalSupply < 250000000 * (10**decimals)) {
            blockReleases = rewardPerBlock.div(4);
            
        } else if(totalSupply >= 250000000 * (10**decimals) && totalSupply < 500000000 * (10**decimals)) {
            blockReleases = rewardPerBlock.div(8);
            
        } else if(totalSupply >= 500000000 * (10**decimals)) {
            blockReleases = rewardPerBlock.div(16);
        }
        
        uint256 lastBlockNumber = lastMintedBlockNumber;
        uint256 blockTime = currentBlockNumber.sub(lastBlockNumber);
        uint256 blockRewards = blockTime.mul(blockReleases);
        return blockRewards;
    }
    
    function addBlockMinters(address account) public {
        require(balances[msg.sender] > tokensHold);
        blockMinters[account] = true;
        emit BlockMinters(account);
    }

//--------------------------------------------------------------------------------------
//Set function for Proof-of-Block mining protocol
//--------------------------------------------------------------------------------------
    ...
    function setTokensHold(uint256 _tokensHold) public onlyOwner {
        tokensHold = _tokensHold;
    }

    function blockInfo(address account) public view returns(address, uint, uint, uint) {
        return (account, currentBlockNumber, lastMintedBlockNumber, blockInterval);
    }

//--------------------------------------------------------------------------------------
//Function to pause and continue internal Proof-of-Block mining protocol
//--------------------------------------------------------------------------------------

    bool public mintingBlockPaused;
    
    function miningStart() public onlyOwner {
        require(msg.sender == owner && blocksStartTime == 0);
        blocksStartTime = now;
    }
    
    function mintingBlockContinue() public onlyOwner {
        mintingBlockPaused = false;
    }
    
    function mintingBlockPause() public onlyOwner {
        mintingBlockPaused = true;
    }

    function isMintingBlockPaused() public view returns (bool) {
        return mintingBlockPaused;
    }
