...
contract Freaks is ERC721, Ownable {
    using SafeMath for uint256;
    uint256 public constant MAX_FREAKS = 10000;
    ...
    constructor(string memory baseURI) ERC721("Freaks", "FREAKS") {
        setBaseURI(baseURI);
    }
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_FREAKS, "Sale has already ended");
    ...
    function calculatePriceForToken(uint256 _id) public view returns (uint256) {
        require(_id < MAX_FREAKS, "Sale has already ended");
    ...
    function adoptChubby(uint256 numFreaks) public payable {
        require(totalSupply() < MAX_FREAKS, "Sale has already ended");
        require(
            numFreaks > 0 && numFreaks <= 20,
            "You can adopt minimum 1, maximum 20 chubbies"
        );
        require(
            totalSupply().add(numFreaks) <= MAX_FREAKS,
            "Exceeds MAX_CHUBBIES"
        );
        require(
            msg.value >= calculatePrice().mul(numFreaks),
            "Ether value sent is below the price"
        );

        for (uint256 i = 0; i < numFreaks; i++) {
    ...
    function reserveGiveaway(uint256 numFreaks) public onlyOwner {
        uint256 currentSupply = totalSupply();
        require(
            totalSupply().add(numFreaks) <= 30,
            "Exceeded giveaway supply"
        );
        require(hasSaleStarted == false, "Sale has already started");
        uint256 index;
        // Reserved for people who helped this project and giveaways
        for (index = 0; index < numFreaks; index++) {
