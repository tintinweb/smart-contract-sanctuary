...
contract LPTOKENERC20 {
    ...
    string public constant name = 'CES - LP';
    string public constant symbol = 'CESXLP';
...
contract SwapLPPair is LPTOKENERC20 {
    using SafeMathUniswap  for uint;
...
contract CESLPFactory {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(SwapLPPair).creationCode);
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        bytes memory bytecode = type(SwapLPPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        SwapLPPair(pair).initialize(token0, token1);
