...
contract InvestorsStorage {
  ...
  // function notContains(address addr) public view returns (bool) {
  //   return !(s.data[addr].keyIndex > 0);
  // }
...
library ToAddress {
  ...
  // todo testing with addr where latest symbol is "0"
...
contract Revolution is Accessibility, PaymentSystem {
  ...
  // percents 
  Percent.percent private m_dividendsPercent = Percent.percent(333, 10000); // 333/1000*100% = 3.33%
  ...
  function doInvest(address[3] refs) public payable notOnPause balanceChanged {
    ...
    if (!m_referrals[msg.sender]) {
      ...
      if (notZeroNotSender(refs[0]) && m_investors.contains(refs[0])) {
        ...
        assert(m_investors.addRefBonus(refs[0], reward)); // referrer bonus
  ...
  function payout() public notOnPause onlyAdmin(AccessRank.Payout) atPaymode(Paymode.Push) balanceChanged {
    ...
    // if m_paysys.keyIndex == m_investors.iterStart() then payout NOT in process and we must check latest time of payment.
  ...
  // function setMaxBalance(uint newMaxBalance) public onlyAdmin(AccessRank.Full) {
  //   maxBalance = newMaxBalance;
  // }

  // function setMinInvesment(uint newMinInvesment) public onlyAdmin(AccessRank.Full) {
  //   minInvesment = newMinInvesment;
  // }

  // function setAdminPercent(uint num, uint den) public onlyAdmin(AccessRank.Full) {
  //   den.requireNotZero();
  //   m_adminPercent = Percent.percent(num, den);
  // }

  // function setPayerPercent(uint num, uint den) public onlyAdmin(AccessRank.Full) {
  //   den.requireNotZero();
  //   m_payerPercent = Percent.percent(num, den);
  // }

  // function setDividendsPercent(uint num, uint den) public onlyAdmin(AccessRank.Full) {
  //   den.requireNotZero();
  //   m_dividendsPercent = Percent.percent(num, den);
  // }
