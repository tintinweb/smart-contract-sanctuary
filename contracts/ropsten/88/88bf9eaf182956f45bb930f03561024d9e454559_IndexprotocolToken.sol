...
contract BasicToken is ERC20Basic {
  ...
  */
  ...
  // do ledger for dividend transfer
  function handle_dividend(address _from, address _to) internal{
      if(dividends.length > 0){
          ...
          // Go in if dividend is claimed from the sender
          if(dividend_claimed_from > 0){
              // Go in if sender is transfering tokens that have claimed dividend
              if(from_balance <  dividend_claimed_from){
                  uint256 token_claimed_dividend = dividend_claimed_from.sub(from_balance);
                  // Add token claimed entry to dividend map
...
contract IndexprotocolToken is StandardToken, Ownable {
  ...
  // function to add dividend only owner
  ...
  // to calim dividend for latest dividend payout
  ...
  // to calim dividend for specific dividend payout
  function claim_devidend_for(uint256 dividend_index) internal {
    ...
    //update specific dividend entry with calculated token balance
  ...
  // function to check specific wallet dividend
  ...
  // Owner can withdraw unclaimed balance for previous devidends
  function withdraw_unclaimed_balance(uint256 dividend_index) onlyOwner public {
      require(dividends.length > 0);
      // always withdraw older unclaimed dividends
