...
contract AngryContract is ReentrancyGuard{
    ...
    uint256 public expectedPriceFloatVal = 10;
    ...
    function prePurchase(uint256 _expectedPrice, uint256 _startTime, uint256 _expiredTime, string calldata _invoiceId, bytes memory _sig) public payable {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender,_invoiceId));
        ...
        if(msg.value > 0){
            ...
            if(maxPriceMultiple != 0){
                uint256 lowEthPrice = ethPrice * maxPriceMultiple * (100 - expectedPriceFloatVal) / 100;
                uint256 highEthPrice = ethPrice * maxPriceMultiple * (100 + expectedPriceFloatVal) / 100;
                require( _expectedPrice >= lowEthPrice && _expectedPrice <= highEthPrice, "Invalid expectedPrice!" );
            }
            require( _expectedPrice > ethPrice, "Invalid expectedPrice!" );
            ...
            if(maxPriceMultiple != 0){
                uint256 lowUsdtPrice = usdtPrice * maxPriceMultiple * (100 - expectedPriceFloatVal) / 100;
                uint256 highUsdtPrice = usdtPrice * maxPriceMultiple * (100 + expectedPriceFloatVal) / 100;
                require( _expectedPrice >= lowUsdtPrice && _expectedPrice <= highUsdtPrice, "Invalid expectedPrice!" );
            }
            require( _expectedPrice > usdtPrice, "Invalid expectedPrice!" );
        ...
        emit PrePurchase(msg.sender, purchases.length-1, currAmount, pcInfo.paymentAmount, pcInfo.price, pcInfo.expectedPrice, pcInfo.startTime, pcInfo.expiredTime, pcInfo.paymentType, pcInfo.status);
    }
