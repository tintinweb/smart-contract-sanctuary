...
contract StandardToken is Token {
  ...
  function burn(uint256 _amount) public override returns (bool success) {
...
contract GenerateToken is StandardToken {
  ...
  */
  string public name;                   //fancy name: eg Tic Token
  ...
  //make sure this function name matches the contract name above. So if you're token is called TutorialToken, make sure the //contract name above is also TutorialToken instead of Token

  constructor(string memory _name, string memory _symbol) public {
    balances[msg.sender] = 10e27; // Give the creator all initial tokens (100000 for example)
    _totalSupply = 10e27;          // Update total supply (100000 for example)
