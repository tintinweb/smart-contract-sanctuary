...
contract NFTMarket is ReentrancyGuard {
    ...
    address payable[] private operators;

    constructor(address[] memory _operators) {
        owner = payable(msg.sender);
        for(uint i = 0; i < _operators.length; i++)
        {
            operators.push(payable(_operators[i]));
        }
    }
    ...
    function acceptOffer(uint256 itemId, uint256 offerIndex) public nonReentrant{
        ...
        {
            uint256 tFee = SafeMath.div(fees,10);
            operators[i].transfer(tFee);
            operatorFee += tFee;
        }
        owner.transfer(fees - operatorFee);
