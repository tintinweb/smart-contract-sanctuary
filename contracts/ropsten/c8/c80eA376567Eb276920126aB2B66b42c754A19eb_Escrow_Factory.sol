...
contract Escrow_Factory {
    ...
    function buyerCancel(uint256 _id, bool _cancel) public {
        ...
        require((block.timestamp - e.getBuyerConfirmedTimeStamp()) > 3 days, 'Please allow 3 days for the seller to accept before cancelling');

        // Make sure contract is still awaiting seller acceptence
        require(e.getState() == contractState.AWAITNG_SELLER_ACCEPTANCE, 'Contract can only be cancelled if it is still awaiting acceptance from seller');
