...
contract LiquifiV2Portfolio is Portfolio {
    ...
    function smartTransferFrom(address token, address from, address to, uint value) internal returns(bool) {
    ...
    function _deposit(address token, uint amount, address from) private returns (uint liquidityOut){
        ...
        require(smartTransferFrom(token, from, reserve, amount), "ERROR: Deposit failed");
    ...
    function deposit(address token, uint amount) external override returns (uint liquidityOut) {
        return _deposit(token, amount, msg.sender);
    }

    function _withdraw(address token, uint liquidity, address to) private returns (uint amount){
    ...
    function withdraw(address token, uint liquidity) external override returns (uint amount) {
        return _withdraw(token, liquidity, msg.sender);
    }
    ...
    function withdraw(address[] memory tokens, uint[] memory liquidities) external override returns (uint[] memory amounts) {
        ...
        for(uint i = 0; i < tokens.length; i++) {
            amounts[i] = _withdraw(tokens[i], liquidities[i], msg.sender);
        }
    }
    ...
    function _swap(address tokenIn, uint amountIn, address tokenOut, uint minAmountOut, address from) 
        ...
        require(smartTransferFrom(tokenIn, from, reserveAddress1, amountIn), "ERROR: Transfer from failed");
        require(
