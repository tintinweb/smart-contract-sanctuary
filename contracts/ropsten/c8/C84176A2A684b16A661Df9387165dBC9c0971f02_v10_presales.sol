...
interface ISOHO {
    ...
    function setPreSalesEnded() external;
}
...
contract v10_presales is Context, Ownable {
...
// -- post sale --
    ...
    function concludeAndAddLiquidity() external onlyOwner {
        ISOHO(address(token)).setPreSalesEnded();
        ...
        uint256 nbOfTokenPerBNBAfterSale = 180; //price to add liquidity: 1BNB per 180B tokens, 10**9 decimals vs 10**18 decimals
        ...
        ISOHO(address(token)).activateContract(); //sets fees and antiwhale
