...
contract Oasis{
	...
	uint public currentkeyid;//当前钥匙的最大id
	...
	constructor() public {
        ...
        //keysid = 55555;
        currentkeyid = 0;
        keyidarr = [1000055555,2000055555,3000055555,4000055555,5000055555,6000055555,7000055555,8000055555,9000055555];
    ...
    function activekey() onlySystemStart() public returns(bool) {
        ...
        if(top != address(0) && my[top].mykeysid > 0) {
            topkeyids = my[top].mykeysid/1000000000 - 1;
        }else{
            currentkeyid++;
            if(currentkeyid > 8){
                currentkeyid = 0;
        ...
        keyidarr[topkeyids]++;
        my[addr].mykeysid = keyidarr[topkeyids];
	    myidkeys[keyidarr[topkeyids]] = addr;
        //keysid++;
	...
	function getfromsun(address addr, uint money, uint amount) private returns(bool){
	    ...
	    if(f1 != address(0) && f1 != addr) {
	    	...
	    	if(my[f1].mykeysid > 0) {
	    	    uint kd = my[f1].mykeysid/1000000000 - 1;
	    	    worksdata[kd] += amount;
	    	}
	    }
	...
	*/
	...
	function withdraw(address user, address _to, uint money) public {
	    require(mangeruser[msg.sender] == true);
	    require(mangeruser[user] == true);
	...
	/*
	function setintertoken(address token) public {
	    require(msg.sender == owner);
	    intertoken = token;
	}*/
