...
contract Party {
     ...
     */
    function withdraw(address partyAddress, uint256 amount) external onlyMember {
        require(
    ...
    function checkDifferenceInBalance() external view returns (int) {
    ...
    function joinParty(
        ...
        uint256 amount
    ) external notAMember reqDeposit(amount) {
        require(
            verifySigner(
                PLATFORM_ADDRESS,
                messageHash(
    ...
    function approve(
        ...
        registeredProposal[proposalId] = payable(address(0));
        uint cut = tempBalance * 5 / 1000;
        uint sent = tempBalance - cut;
        balanceParty -= tempBalance;
        token.safeTransfer(PLATFORM_ADDRESS, cut);
     ...
     */
    function profitDeposit (uint256 amount, address[] memory nonPayers) external {
    ...
    function leaveParty(address quittingMember) external onlyMember {
        ...
        uint cut = _tempBalance * 5 / 1000;
        uint sent = _tempBalance - cut;
