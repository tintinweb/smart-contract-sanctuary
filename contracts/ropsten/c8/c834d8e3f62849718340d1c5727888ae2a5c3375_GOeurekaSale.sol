...
contract gotTokenSaleConfig is GoConfig {
    ...
    uint public constant PRESALE_START = 1528820787; // Friday, June 15, 2018 12:00:46 PM GMT+08:00
...
contract GOeurekaSale is Claimable, gotTokenSaleConfig, Pausable, Salvageable {
    ...
    function whatIsCurrentCap() external view returns (uint256){
        return currentCap;
    }
    ...
    function calculateRates() internal {
        presaleCap =              uint(300 * DECIMALSFACTOR).div(basicRate);
        week1Cap = presaleCap.add(uint(300 * DECIMALSFACTOR).div(basicRate));
        week2Cap = week1Cap.add(uint(300 * DECIMALSFACTOR).div(basicRate));
        week3Cap = week2Cap.add(uint(300 * DECIMALSFACTOR).div(basicRate));
