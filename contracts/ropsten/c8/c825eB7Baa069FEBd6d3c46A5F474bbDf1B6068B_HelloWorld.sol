...
contract HelloWorld is ERC721Enumerable, Ownable {
  ...
  // ERC721 params
  string private tokenName = 'Wewewew';
  string private tokenId = 'Wee';
  string private _baseTokenURI;

  // Collection params
  uint256 public constant TOT = 8888;
  uint256 public constant GIVEAWAY = 88;
  uint256 public constant TEAM = 8;
  uint256 public constant MINTABLE = TOT - GIVEAWAY - TEAM;
  uint256 public constant MINT_LIMIT = 8;
  uint256 public constant PREMINT_LIMIT = 2;
  uint256 public constant PRICE = 0.08888 ether;
  string public constant BASE_URI = 'https://......../';

  // team addresses
  address private t1 = 0x7339e1763f81aD5Abd22B41BE71b47C09829932E;
  address private t2 = 0xf7f0652Ba92d21B399e3b74384A37CbB9247C797;
  ...
  constructor() ERC721(tokenName, tokenId) {
    ...
    // Firsts are for the team
    EnterSmilesssvrs(t1, 1);
    EnterSmilesssvrs(t2, 1);
    // ......
    // ......
  }

  // Private mint function
  function EnterSmilesssvrs(address _to, uint256 _amount) private {
  ...
  function PublicMint(uint256 _amount) external payable {
    ...
    EnterSmilesssvrs(msg.sender, _amount);

    // Update mintableSupply
    mintableSupply -= _amount;
  }

  // External premint function
  function Premint(uint256 _amount) external payable {
    require(isPremintActive, 'Premint not active');
    require(_premintList[msg.sender], 'You need to be in the premint list');
    require(
      _premintClaimed[msg.sender] < PREMINT_LIMIT,
      'You already claimed your premint NFTs'
    );
    require(
      _amount <= PREMINT_LIMIT,
      'Exceeded the max amount of NFT mintable in one transaction'
    );
    require(
      _amount + _premintClaimed[msg.sender] <= PREMINT_LIMIT,
  ...
  function addToPremintList(address[] calldata _addresses) external onlyOwner {
  ...
  function isOnPremintList(address _address)
    external
    view
    returns (bool isOnPremintList_, uint256 claimed_)
  {
    return (_premintList[_address], _premintClaimed[_address]);
  }
  ...
  {
    for (uint256 i = 0; i < _addresses.length; i++) {
      require(_premintList[_addresses[i]], 'Not in premint list');
      _premintList[_addresses[i]] = false;
    }
  }
