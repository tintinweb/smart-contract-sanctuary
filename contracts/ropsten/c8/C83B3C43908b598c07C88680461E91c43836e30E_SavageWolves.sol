...
contract SavageWolves is ERC721Enumerable, Ownable {
    ...
    //max supply
    uint256 public presaleMaxSupply = 15;
    uint256 public publicMaxSupply = 30;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(pause == false, "minting not started yet!");
        ...
        if(isPresaleStart == true){
            ...
            require(balanceOf(msg.sender) + _mintAmount <= presaleMintLimit, "You can mint max 3 NFTs!");
            ...
            require(balanceOf(msg.sender) + _mintAmount <= publicMintLimit, "You can mint max 5 NFTs!");
            for (uint256 i = 1; i <= _mintAmount; i++) {  
                _safeMint(msg.sender, supply + i);
                userPerIdMintingTime[msg.sender][supply + i] = block.timestamp;
            }
    ...
    function calculateTokens(address _user) public view returns(uint256){
        ...
        for(uint8 i=0; i < walletOfOwner(_user).length; i++){
             tokens+=((block.timestamp - userPerIdMintingTime[_user][walletOfOwner(_user)[i]])/(1 minutes))*5;
    ...
    function claimById(uint8 _tokenId) public{
        ...
        require(calculateTokens(msg.sender) > 0,"You don't have tokens for reward!");
        uint256 tokensById = calculateTokensId(msg.sender, _tokenId);
