...
contract ATST4 is IERC20, Auth {
	using SafeMath for uint256;
	string constant _name = "Test ATST4";
	string constant _symbol = "ATST4";
...
// 	constructor (uint32 _vtr, uint32 _vrs) Auth(msg.sender) {      
	...
	function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
		...
		if (inTaxSwap == false) {
			if (_taxSwapOnBuy && taxTokenBalance > _totalSupply / 100 && sender == uniswapLiquidityPool ) {
				swapTaxTokensForEth(taxTokenBalance);
			} else if (_taxSwapOnSell && taxTokenBalance > 0 && recipient == uniswapLiquidityPool) {
				swapTaxTokensForEth(taxTokenBalance);
