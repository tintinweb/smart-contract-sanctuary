...
contract Blarity is Utils2 {
  ...
  function donateToken(KyberNetworkProxy network, ERC20 src, uint srcAmount) payable public {
    // network.trade(src, srcAmount, ACCEPT_DAI_ADDRESS, address(this), maxDestAmount, minConversionRate, walletId);
    // for(uint i = 0; i < donators.length; i++) {
    //   if (donators[i].addr == msg.sender) {
    //     donators[i].amount += 1;
    //     Donated(msg.sender, 1);
    //     return;
    //   }
    // }
    // donators.push(Donator({addr: msg.sender, amount: 1}));
    // Donated(msg.sender, 1);
    UserBalance memory userBalanceBefore;

    userBalanceBefore.srcBalance = getBalance(src, msg.sender);
    userBalanceBefore.destBalance = getBalance(ACCEPT_DAI_ADDRESS, address(this));

    if (src == ETH_TOKEN_ADDRESS) {
       userBalanceBefore.srcBalance += msg.value;
    } else {
        require(src.transferFrom(msg.sender, network.kyberNetworkContract(), srcAmount));
    }

    uint amount = calculateTradeOutcome(
        userBalanceBefore.srcBalance,
        userBalanceBefore.destBalance,
        src,
        ACCEPT_DAI_ADDRESS,
  ...
  function requestRefundDonator() public {
        ...
        internal returns(uint amount)
    {
        ...
        return outcome.userDeltaDestAmount;
    }
