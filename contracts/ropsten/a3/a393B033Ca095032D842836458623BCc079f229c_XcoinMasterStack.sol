...
contract WaultSwapToken is ERC20("WaultSwap", "XCOIN", 18, 0, 750000000), Ownable, Mintable {
...
contract XcoinMasterStack is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accXcoinPerShare;   
    }

    WaultSwapToken public xcoin;
    uint256 public xcoinPerBlock;
    ...
    constructor(
        WaultSwapToken _xcoin,
        uint256 _xcoinPerBlock,
        uint256 _startBlock
    ) public {
        xcoin = _xcoin;
        xcoinPerBlock = _xcoinPerBlock;
    ...
    function pendingXcoin(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accXcoinPerShare = pool.accXcoinPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 xcoinReward = multiplier
                .mul(xcoinPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accXcoinPerShare = accXcoinPerShare.add(
                xcoinReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accXcoinPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingRewards);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 xcoinReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        xcoin.mint(address(this), xcoinReward);
        pool.accXcoinPerShare = pool.accXcoinPerShare.add(
            xcoinReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, bool _withdrawRewards) public {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                if (_withdrawRewards) {
                    safeXcoinTransfer(msg.sender, user.pendingRewards);
        ...
        user.rewardDebt = user.amount.mul(pool.accXcoinPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount, bool _withdrawRewards) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accXcoinPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            user.pendingRewards = user.pendingRewards.add(pending);

            if (_withdrawRewards) {
                safeXcoinTransfer(msg.sender, user.pendingRewards); // to change in th waultswap token
                emit Claim(msg.sender, _pid, user.pendingRewards);
                user.pendingRewards = 0;
            }
        }
    ...
    function safeXcoinTransfer(address _to, uint256 _amount) internal {
        uint256 xcoinBal = xcoin.balanceOf(address(this));
        if (_amount > xcoinBal) {
            xcoin.transfer(_to, xcoinBal);
        } else {
            xcoin.transfer(_to, _amount);
        }
    }

    function setXcoinPerBlock(uint256 _xcoinPerBlock) public onlyOwner {
        require(_xcoinPerBlock > 0, "!xcoinPerBlock-0");
        xcoinPerBlock = _xcoinPerBlock;
    }
