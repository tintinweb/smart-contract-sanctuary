...
contract ArtWork is ERC721URIStorage, Ownable {
    ...
    function mint(string memory tokenURI, uint256 startingBidValue) external onlyOwner {
    ...
    _biddingTime[_nextTokenId] = block.timestamp + 1 minutes;
    ...
    function biddingTimeRemaining(uint256 tokenId) external view returns(uint256) {
        require(_exists(tokenId), " ArtWork: query for nonexistent token");
    ...
    function bid(uint256 tokenId, uint256 amount) external biddingEnabled(tokenId) {
    ...
    function claim(uint256 tokenId) external payable {
        require(_biddingTime[tokenId] < block.timestamp, "bidding is still goining");
        require(_maxBid[tokenId].bidder == _msgSender(), "caller is not the highest bidder");
