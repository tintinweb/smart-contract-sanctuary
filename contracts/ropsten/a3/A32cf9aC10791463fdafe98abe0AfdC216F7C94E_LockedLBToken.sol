...
*/
abstract contract PoolDepositable is AccessControl, Poolable, Depositable {
 ...
 */
abstract contract Tierable is AccessControl, Depositable, ITierable {
    ...
    constructor(uint256[] memory tiersMinAmount) {
...
*/
abstract contract Poolable is AccessControl {
    using SafeMath for uint256;
    ...
    modifier whenPoolOpened(uint256 poolIndex) {
        require(poolIndex < _pools.length, "Poolable: Invalid poolIndex");
    ...
    function addPool(Pool calldata pool) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _pools.push(
            Pool({lockDuration: pool.lockDuration, opened: pool.opened})
        );

        uint256 poolIndex = _pools.length - 1;
