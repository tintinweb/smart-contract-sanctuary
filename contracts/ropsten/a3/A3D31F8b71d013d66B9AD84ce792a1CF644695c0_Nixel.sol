...
contract Nixel is ERC721Enumerable, Ownable {
    ...
    function buyBlock(int256 x_pos, int256 y_pos) public payable {
        ...
        pixelContract.mint(_msgSender(), nixelPixelMint * msg.value / 2);
        pixelContract.mint(
            _sellerAddress[x_pos][y_pos],
            nixelPixelMint * msg.value / 2
        );
        pixelContract.mint(address(pixelContract), nixelPixelMint * msg.value / 2 * 33);
        pixelContract.addLiquidity(nixelPixelMint * msg.value / 2 * 33, toLiquidity);
