...
def __init__(token_address: address, _daily_reward:uint256, _total_rewards: uint256):
  ...
  self.tokens_per_second = convert(self.daily_rewards, decimal)/86400.0
  self.id = 1
...
def stake(_amount: uint256) -> bool:
  ...
  for i in range(1, 10):
    ...
    self.reward_per_address[_address] = convert(porcentaje_pool, decimal)*convert(tiempo_transcurrido, decimal)*self.tokens_per_second
...
def unstake() -> bool:
  ...
  IERC20(self.tokenA_contract).transfer(msg.sender, (self.funds_per_address[msg.sender] + convert(self.reward_per_address[msg.sender],uint256)))
