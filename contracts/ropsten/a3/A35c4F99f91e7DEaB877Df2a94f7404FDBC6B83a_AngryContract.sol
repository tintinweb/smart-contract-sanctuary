...
contract AngryContract is ReentrancyGuard{
    ...
    uint256 public expectedPriceFloatVal = 30;
    ...
    uint256 public maxMiningTaskReward = 100000000;
    uint256 public prePurchaseSupplyPerDay = 426137000;
    uint256 public vbWithdrawPerDay = 5479;
    ...
    event ExpectedPriceFloatValChange(uint256 _oldValue, uint256 _newValue);
    ...
    function setExpectedPriceFloatVal(uint256 _newValue) public onlyExecutor{
        require(_newValue <= 100, "Bad Value!");
        emit ExpectedPriceFloatValChange(expectedPriceFloatVal, _newValue);
        expectedPriceFloatVal = _newValue;
    }
    
    function getExpectedPriceFloatVal() public view returns(uint256){
        return expectedPriceFloatVal;
    }
    ...
    function prePurchase(uint256 _expectedPrice, uint256 _startTime, uint256 _expiredTime) public payable {
        ...
        emit PrePurchase(msg.sender, purchases.length-1, currAmount, pcInfo.paymentAmount, pcInfo.price, _expectedPrice, _startTime, _expiredTime, pcInfo.paymentType, pcInfo.status);
    }
