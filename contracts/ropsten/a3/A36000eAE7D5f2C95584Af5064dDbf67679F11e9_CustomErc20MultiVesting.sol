...
abstract contract Erc20MultiVesting is Context, Ownable {
    ...
    uint256 public totalRevokedClaim;
    ...
    function takeRevoked(address account) public onlyOwner {
        ...
        uint256 maxVestedForNow = _calculateVestedAmount(totalSupply, 0);
        uint256 reallyVestedForNow = _calculateVestedAmount(totalSupply - totalAllocatedSupply, 0);

        for (uint i = 0; i < _beneficiaries.length; i++) {
            reallyVestedForNow += _vestedFor(_beneficiaries[i]);
        }

        require(maxVestedForNow > reallyVestedForNow, "takeRevoked: No revoked tokens");

        uint256 revokedForNow = maxVestedForNow - reallyVestedForNow;
        require(revokedForNow > totalRevokedClaim, "takeRevoked: All revoked token are claimed");

        uint256 revokedForClaim = revokedForNow - totalRevokedClaim;
        totalRevokedClaim += revokedForClaim;
    ...
    function _calculateVestedAmount(uint256 totalAmount, uint256 revokedAmount) private view returns (uint256) {
        ...
        uint256 tgeAmount = _percent(totalAmount, tgePercent);
        uint256 intervalAmount = _percent(totalAmount, intervalPercent);
