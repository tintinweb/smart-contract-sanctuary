...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
    ...
    function buyAndStakeETH(uint256 amountOutMin, uint256 lockTime) public payable nonReentrant{
        ...
        if (_amount > 0) {
            ...
            user.amount = user.amount + uint112(_amount);
        }
...
//--------------------------------------------------------RESTRICTED FUNCTIONS --------------------------------------------------------	
    ...
    function depositFor(address sender, uint256 _pid, uint112 amount, uint256 lockTime) public onlyAuthorized {
...
//--------------------------------------------------------INTERNAL FUNCTIONS --------------------------------------------------------
    function _deposit(address sender, uint256 _pid, uint112 _amount, uint256 lockTime) internal {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function updateETHRewards() internal {
        // ETHPerBlock = ETHLeftUnshared / nb of blocks in 2 days. 
        // The contact assumes it's not going to receive any ether for another 2 days and computes the rewards for this period accordingly.
        // Refilling early isnt an issue, any unshared ETH left will make it into the next rewards cycle
        ETHPerBlock = ETHLeftUnshared / 5760 * 2;
        massUpdatePools();
    }
