...
//     _marketingWalletAddress.transfer(amount);
    ...
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 1 && taxFee <= 499, 'taxFee should be in 1 - 49.9');
        _taxFee = taxFee;
    }

    function _setMarketingFee(uint256 marketingFee) external onlyOwner() {
        require(marketingFee >= 1 && marketingFee <= 499, 'marketingFee should be in 1 - 49.9');
        _marketingFee = marketingFee;
    }

    function _setLiquidityFee(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee >= 1 && liquidityFee <= 499, 'liquidityFee should be in 1 - 49.9');
