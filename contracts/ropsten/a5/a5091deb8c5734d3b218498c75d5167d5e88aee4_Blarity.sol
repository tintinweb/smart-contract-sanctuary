...
contract Blarity is ReentrancyGuard {
  ...
  function creatorClaimFundTransfer(uint id) public onlyOwner {
    ...
    for(uint i = 0; i < votes.length; i++) {
      if (votes[i].isApproved) { positive += votes[i].weight; }
      else { negavtive += votes[i].weight; }
    }
    // positive or negative is more than 50%, immediate update the request
    if (positive > targetedMoney / 2) {
    ...
    } else if (negavtive > targetedMoney / 2) {
      requestFunds[id].isEnded = true;
  ...
  function contribute(KyberNetworkProxyInterface network, ERC20 src, uint srcAmount, address _delegator) public payable {
    ...
    if (src != ETH_TOKEN_ADDRESS) {
        // approve this contract to use this src amount
        require(src.approve(address(this), srcAmount));
    }
    if (src == acceptedToken) {
      ...
      paidAmount = doContributeWithKyber(network, src, srcAmount, targetedMoney - currentBalance, minConversionRate, walletId);
    }
    currentBalance += paidAmount;
    // add paid amount to Contributor
    bool contributorExist = false;
    for(uint i0 = 0; i0 < contributors.length; i0++) {
      if (contributors[i0].ownerAddress == msg.sender) {
        require(contributors[i0].delegator == _delegator);
        contributors[i0].amount += paidAmount;
  ...
  function doContributeWithKyber(KyberNetworkProxyInterface network, ERC20 src, uint srcAmount, uint maxDestAmount, uint minConversionRate, address walletId) internal returns (uint paidAmount) {
    if (src != ETH_TOKEN_ADDRESS) {
      require(src.transferFrom(msg.sender, address(this), srcAmount));
      require(src.approve(network, 0));
      require(src.approve(network, srcAmount));
    }
    bytes memory hint;
    paidAmount = network.tradeWithHint.value(msg.value)(src, srcAmount, acceptedToken, address(this), maxDestAmount, minConversionRate, walletId, hint);
    if (src != ETH_TOKEN_ADDRESS) {
      require(src.approve(network, 0));
    }
    return paidAmount;
