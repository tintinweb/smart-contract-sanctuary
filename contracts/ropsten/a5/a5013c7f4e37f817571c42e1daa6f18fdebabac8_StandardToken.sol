...
contract StandardToken is IERC20 {
  ...
  event PurchaseToken(address indexed purchaser, uint256 amount);
 ...
 */
function purchaseTokens() external failSafe payable payableCheck{
    ...
    emit PurchaseToken(msg.sender, msg.value * _tokenPriceETH);
  }
...
/*
  ...
  function withdrawStakedTokens(uint256 stakingId) public failSafe returns(bool){
    ...
    _TokenTransactionstatus[stakingId] = true;
        if (block.timestamp > _stakingStartTime[_stakingCount].add(2592000)){
 ...
 */
  function submitProposal(uint256 voteLength) public whenNotBlocked(msg.sender) returns (uint256) {
