...
contract DAKPoolV1 is ERC20 {
    ...
    function initialize(address _lp, address _team, address _dead) external payable {        
        ...
        StartTimestamp = block.timestamp + 360;
    ...
    // 存款结算 自己挖的奖励和团长的邀请奖励 ，提款结算自己的奖励和团长的奖励
    // 自己下属的奖励咋办？只能等待下属提款吗？

    function stake(uint _amount, address _inviter) external {
        require(_amount >1e12 || _amount == 0 , "Error : Stake Amount too low");
        ...
        //就算存0个，依然要记录他的已领取奖励
        if (_accTokenPerShare != 0) {
            user.RewardAmount = user.DepositAmount.mul(_accTokenPerShare).div(1e12);
        }
        ...
        if (_amount > 0 && user.inviter != address(0) && user.inviter != address(DEAD)) {
            ...
            // 重新统计团队信息，再次计算已领走的邀请奖励
        uint _leaderTotalDepositAmount2;
        for (uint i = 0; i < invitation.length; i++) {
            _leaderTotalDepositAmount2 +=  invitation[i].DepositAmount;
        }

        if(_inviterAccTokenPerShare != 0) {
            users[user.inviter].inviterRewardAmount = _leaderTotalDepositAmount2.mul(_inviterAccTokenPerShare).div(1e12);
        }
    ...
    function withdraw(uint _amount) external {
        ...
        require(user.inviter !=address(0), "Inviter can't equal 0x00");
