...
interface IReflectERC20 is IERC20 {
    /**
    * @dev Works exactly the same as transferFrom but always applies the fee which
    * allows us to make sure that the fee gets applied whichever gateway we transfer through.
    */
    function transferFromForceFees(address sender, address recipient, uint256 amount) external returns (bool);

    /**
    * @dev Calculates the fee from `amount` and deducts it from the sender's balance before redistributing it.
    */
    function distributeOnlyFees(uint256 amount) external returns (bool);
}
...
contract PeggedGateway is Gateway {
    ...
    */
    function transferNativeIn(address gw, address to, uint256 amount) override public {
        require(token.transferFrom(msg.sender, address(this), amount), "Failed to transfer in");

        lockedTokens = lockedTokens.add(amount);

        emit TransferNativeIn(gw, to, block.number, amount);
    }
}
...
contract R3FItoken is Context, IReflectERC20, Ownable {
    using SafeMath for uint256;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        if (_isGateway[account]) return _tOwned[account];
    ...
    /// @dev called by gateway to enforce fee distribution
    function transferFromForceFees(address sender, address recipient, uint256 amount) public override returns (bool) {
        _transferForceFees(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function _transferForceFees(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
    ...
    /**
    * @dev only apply the reflection fees and deduct them from the sender's balance
    */
    function distributeOnlyFees(uint256 tAmount) override public returns (bool) {
...
contract pR3FIToken is ERC20 {
    constructor() ERC20("pRXX", "pRXX") {
        _setupDecimals(9);
