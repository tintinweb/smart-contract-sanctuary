...
contract Privilege is Ownable {
    ...
    function limitMoneyPrivilegeAccount(uint256 tradeMoney) internal{
        ...
        if(block.timestamp/86400000 > currentDay){
            currentDay = block.timestamp/86400000;
            privilegeAccountStruct.dayMoney = privilegeAccountStruct.dayMoneyLimit;
        }
        require(
            privilegeAccountStruct.dayMoney >= tradeMoney,
            "The amount exceeds the limit for days"
        );
        require(
            privilegeAccountStruct.totalMoney >= tradeMoney,
            "The amount exceeds the total limit"
        );
        privilegeAccountStruct.dayMoney -= tradeMoney;
        privilegeAccountStruct.totalMoney -= tradeMoney;
    }

    function setPrivilegeAccount(address addr,uint256 dayMoneyLimit,uint256 addTotalMoney) external onlyOwner(){
        ...
        privilegeAccountMap[addr].totalMoney += addTotalMoney;
    }
...
contract BridgeGateway is Privilege {
    ...
    function sendTokenList(
        ...
        for (uint256 index = 0; index < txIdList.length; index++) {
