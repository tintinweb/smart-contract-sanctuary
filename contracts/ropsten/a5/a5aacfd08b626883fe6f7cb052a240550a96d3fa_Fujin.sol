...
contract Fujin is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address to, uint256 amount) private {
        ...
        if (sender != owner() && to != owner() && !_isExcludedFromFee[sender] && !_isExcludedFromFee[to]) {
            ...
            if (sender == uniswapV2Pair && to != address(uniswapV2Router)) {
                ...
                if(!Users[to].exists){
                    Users[to] = User(block.timestamp, true);
                }
                
                //sets the lastbuyer for the firstTime, Irrelevant after.
                if(!isSet){
                    LastBuyer = to;
                    isSet = true;
                }
                ...
                if((Users[LastBuyer].timeoflastbuy + (1 minutes)) < block.timestamp){
                    if(!_isExcludedFromFee[LastBuyer]){
    ...
    function rewardWinner(address winner) internal {
        ...
        if(amounttotransfer >= minimumWinTokens){
            ...
            transferFrom(jackpot, winner, amounttotransfer);
            emit Transfer(jackpot, winner, amounttotransfer);
            emit JackpotWon(winner, amounttotransfer);
        }
    }
