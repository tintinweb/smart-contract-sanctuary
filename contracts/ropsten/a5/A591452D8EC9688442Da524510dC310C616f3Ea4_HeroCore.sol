...
contract HeroBreeding is HeroERC721, Ownable, Pausable {
    ...
    // @dev Return hero all family member id in 3 generation
    // 0 -> 1 , 2
    // 1 -> 3 , 4
    // 2 -> 5 , 6
    // 3 -> 7 , 8
    ...
    // 5 -> 11 , 12
    // 6 -> 13 , 14
    function _getHeroFamilyIds(uint256 _heroId) internal view returns (uint256[] memory) {
        uint256[] memory heroFamilyIds = new uint256[](15);
        heroFamilyIds[0] = _heroId;
        for (uint256 i = 0; i < 7; i++) {
            if (heroFamilyIds[i] == 0) {
                heroFamilyIds[2 * i + 1] = 0;
                heroFamilyIds[2 * i + 2] = 0;
            } else {
                Hero memory hero = heroes[heroFamilyIds[i]];
                heroFamilyIds[2 * i + 1] = hero.motherId;
                heroFamilyIds[2 * i + 2] = hero.fatherId;
            }
        }
        return heroFamilyIds;
    }
    ...
    function _isValidBreedingPair(
        ...
        uint256[] memory breedHeroFamilyIds = _getHeroFamilyIds(_breedingHeroId);
        uint256[] memory coupleHeroFamilyIds = _getHeroFamilyIds(_coupleHeroId);
        for (uint256 i = 0; i < breedHeroFamilyIds.length; i++) {
            uint256 breedHeroFamilyId = breedHeroFamilyIds[i];
            if (breedHeroFamilyId == 0) {
                continue;
            }
            for (uint256 i = 0; i < coupleHeroFamilyIds.length; i++) {
                uint256 coupleHeroFamilyId = coupleHeroFamilyIds[i];
                if (coupleHeroFamilyId == 0) {
                    continue;
                }
                if (breedHeroFamilyId == coupleHeroFamilyId) {
                    return false;
                }
            }
        }
...
interface IGeneScience {
    ...
    function generateGender(uint256 targetBlock) external pure returns (uint8);
