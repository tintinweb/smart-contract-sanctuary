...
contract Staking is Ownable {
  ...
  function stake(uint256 stakeAmount) public {
    ...
    require(stakeAmount <= maxLeftToStake(), "Staked cannot be > 50% of total holdings");
  ...
  function currentRewards(address addy) public view returns (uint256) {
    return _accured[msg.sender] + _calculateReward(addy);
  }

  function maxLeftToStake() public view returns (uint256) {
      if(_stakingToken.balanceOf(msg.sender) <= _stakedAmount[msg.sender])
        return 0;
      return _maxStakeAmount().sub(_stakedAmount[msg.sender]);
  }

  function _maxStakeAmount() public view returns (uint256) {
      return (_stakingToken.balanceOf(msg.sender).add(_stakedAmount[msg.sender])).div(2);
  }
