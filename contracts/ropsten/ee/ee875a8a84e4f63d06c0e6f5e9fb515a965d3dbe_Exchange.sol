...
contract TransferSafe {
    ...
    function erc1155safeMintTransferFrom(IDirtyPunkStore token, address from, address to, uint256 id, uint256 value,
...
contract Exchange is OrderState, TransferSafe {
    ...
    IDirtyPunkStore private DirtyPunkStore;
    address public weth;
        ...
        if(order.key.sellAsset.assetType == AssetType.ERC721){
            if(order.orderType == 1 || order.orderType == 3) {
                if(!isStore){
                    ...
                    erc1155safeMintTransferFrom(IDirtyPunkStore(order.key.sellAsset.token), order.key.owner, buyer, order.key.sellAsset.tokenId, 1, 
                ...
                if(!isStore){
                    ...
                    erc1155safeMintTransferFrom(IDirtyPunkStore(order.key.buyAsset.token), buyer, order.key.owner, order.key.buyAsset.tokenId, 1,
                                                storeParams.blockExpiry,storeParams.v,storeParams.r, storeParams.s, storeParams.uri);
            ...
            if(order.orderType == 1 || order.orderType == 3) {
                if(!isStore){
                    ...
                    erc1155safeMintTransferFrom(IDirtyPunkStore(order.key.sellAsset.token), order.key.owner, buyer, order.key.sellAsset.tokenId, order.selling,
                                                storeParams.blockExpiry,storeParams.v,storeParams.r, storeParams.s, storeParams.uri);
                ...
                if(!isStore){
                    ...
                    erc1155safeMintTransferFrom(IDirtyPunkStore(order.key.buyAsset.token), buyer, order.key.owner, order.key.buyAsset.tokenId, order.buying,
                                                storeParams.blockExpiry,storeParams.v,storeParams.r, storeParams.s, storeParams.uri);
    ...
    function bid(Order calldata order, Sig calldata sig,Sig calldata buyerSig, Sig calldata buyerFeeSig, address buyer, uint256 buyingAmount, 
        uint256 royaltyFee, address payable royaltyReceipt, bool isStore, mintParams memory storeParams) payable external {
        require((block.timestamp <= order.expiryTime ), "Signature expired" );
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        DirtyPunkStore = IDirtyPunkStore(newMintableStore);
