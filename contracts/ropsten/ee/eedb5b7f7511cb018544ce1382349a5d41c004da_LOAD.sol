...
contract LOAD is Context, IERC20, Ownable {
    ...
    address payable public walletAddress = payable(0x441b4E0C78E944Ee9125f0A932F9128B9d0B923f);
    ...
    uint256 internal _tokenTotal = 100000e9;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "LOAD: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'LOAD: The contract it self cannot be excluded');
        require(!_isExcluded[account], "LOAD: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "LOAD: account is already included");
    ...
    //charity: takes a percentage of swapped balance and donates to charity    
        uint256 charityBasis = 10; //ex: 1% in this case we use 1000
        uint256 charityAmount = ethAmount.mul(charityBasis).div(1000);
        walletAddress.call{value: charityAmount}("");
    }
