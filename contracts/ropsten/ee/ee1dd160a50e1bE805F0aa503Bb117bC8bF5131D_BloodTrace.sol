...
contract BloodTrace {
   // address public owner;
    //address [] public bcsusers;
    //address [] public pcsusers;
    //address [] public tcsusers;
    //address [] public hospsusers;
    address [] public donorusers;
    //constructor () public {
      //  owner=msg.sender;
    //}
    struct Blood {
   ...
   struct Bloodprovided {
    ...
    function addDonor(string memory _dname, address _daddress, string memory _dgender, uint _dage, uint _dweight, string memory _bloodtype) public 
    {
                 
              donorCount ++;
              donors[donorCount]=Donor(donorCount, _dname, _daddress, _dgender, _dage, _dweight, _bloodtype,false);
              donorusers.push(_daddress);
    }
   ...
   function update(uint _pcid, uint _did, string memory _bloodtype, uint _plasmaunit, uint _redbloodcell, uint _platelet) public 
   {
                ...
                bloodinfo[_bloodtype].bloodtype=_bloodtype;
   ...
   function getinfo(string memory _bloodtype) public view returns(uint, uint, uint, uint)  
   {
    ...
    function Viewdonorpersonaldetails(uint _did) public view returns( string memory _dname, address _daddress, string memory _dgender, uint _dage, uint _dweight, string memory _dbloodtype, bool _ddonated)
    {
              _dname=donors[_did].dname;
              _daddress=donors[_did].daddress; 
