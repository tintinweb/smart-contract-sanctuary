...
contract MaxProfitHub {
    ...
    function ProfitRate() public view returns(uint) {
        ...
        if (contractBalance < 150 ether) {
            return (100);
        }
        if (contractBalance >= 150 ether && contractBalance < 500 ether) {
            return (70);
        }
    ...
    function payoutAmount() public view returns(uint256) {
        ...
        uint256 withdrawalAmount = rate.mul(different).mul(6).sub(ProfitWithdraw[msg.sender]);
