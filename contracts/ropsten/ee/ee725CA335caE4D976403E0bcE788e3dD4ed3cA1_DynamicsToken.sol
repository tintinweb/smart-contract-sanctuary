...
//    bool enableLowLevelCalls = false; // TODO - Remove
    ...
    constructor (uint256 _supply, uint256 _MAXTXAMOUNT, uint256 ADDTOLIQUIDITYTHRESHOLD,
        ...
        //exclude owner and this contract from fee TODO - Fix this for prod also.
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if(!inSwapAndLiquify && to != uniswapV2Pair && swapAndLiquifyEnabled){
                selectSwapEvent();
            }
        } else {
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(buyBackEnabled && address(this).balance >= minBuyBack){
            ...
            uint256 redist = tokenFromReflection(buybackTokenTracker.redistribution);
            uint256 buyback = tokenFromReflection(buybackTokenTracker.buyback);
            uint256 liq = tokenFromReflection(buybackTokenTracker.liquidity);

            if(redist >= buyback && redist >= liq){
                // Swap for redistribution
                emit SwapAndLiquify(1,1,1);
                uint256 ethBought = 0;
                ...
                buybackTokenTracker.redistribution = buybackTokenTracker.redistribution.sub(contractTokenBalance.mul(_getRate()));
                totalEthSentToPool = totalEthSentToPool.add(ethBought);
            } else if(buyback >= redist && buyback >= liq){
                // Swap for buyback
                emit SwapAndLiquify(2,2,2);
                ...
                buybackTokenTracker.buyback = buybackTokenTracker.buyback.sub(contractTokenBalance.mul(_getRate()));
            } else {
                // Swap for LP
                emit SwapAndLiquify(3,3,3);
                ...
                buybackTokenTracker.liquidity = buybackTokenTracker.liquidity.sub(contractTokenBalance.mul(_getRate()));
            }
...
//        }
    ...
    function depositTokens(uint256 liquidityDeposit, uint256 redistributionDeposit, uint256 buybackDeposit) external override {
        ...
        buybackTokenTracker.liquidity = buybackTokenTracker.liquidity.add(liquidityDeposit.mul(currentRate));
        buybackTokenTracker.buyback = buybackTokenTracker.buyback.add(buybackDeposit.mul(currentRate));
        buybackTokenTracker.redistribution = buybackTokenTracker.redistribution.add(redistributionDeposit.mul(currentRate));
    }
