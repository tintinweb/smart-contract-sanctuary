...
contract QToken is Authorization {
    ...
    event ChangeName(string _name);
    event ChangeSymbol(string _name);
    
    constructor(){
        name = "QToken";
        symbol= "QT";
        decimals = 18;
        _totalSupply = _totalSupply.add(100000 * 10** uint(decimals));
    ...
    function changeName(string memory _name) public onlyOwner returns (bool success){
        name = _name;
    ...
    function changeSymbol(string memory _symbol) public onlyOwner returns (bool success){
        symbol = _symbol;
        
        emit ChangeSymbol(_symbol);
    ...
    function mint(address _to, uint _value) public whenNotPaused onlyOwner returns (bool success){
        balances[_to] = balances[_to].add(_value);
