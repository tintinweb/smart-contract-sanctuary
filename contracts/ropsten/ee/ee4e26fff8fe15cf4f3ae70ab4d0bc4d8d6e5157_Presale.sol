...
contract CommonTokensale is MultiOwnable, Pausable {
    ...
    function weiToTokens(uint _amountWei) public view returns (uint256) {
        ...
        // Calculamos bono
        //Tablas de bonos
        if(valueUSD > 500 ) bono = 10;
        if(valueUSD > 1000 ) bono = 20;
        if(valueUSD > 2500 ) bono = 30;
        if(valueUSD > 5000 ) bono = 40;
        if(valueUSD > 10000 ) bono = 50;

        uint256 total = valueUSD.add(valueUSD.mul(bono) / 100);

        uint256 tokens = total.mul(tokensPerUSD());

        return tokens;
    }

    function tokensPerUSD() public view returns (uint256) {
