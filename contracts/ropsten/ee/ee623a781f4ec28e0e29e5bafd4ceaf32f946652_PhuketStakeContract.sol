            ...
            require(stakerData[_expiredTimestamps[i]].endTime <= block.timestamp,"Not End staking Period");
             require(!(stakerData[_expiredTimestamps[i]].isCompleted),"Already Unstaked");
                    ...
                    if(withdrawAmount > _amount){
                        withdrawAmount = withdrawAmount.add(_amount.sub((withdrawAmount.sub(stakerData[_expiredTimestamps[i]].altQuantity))));
                        emit Unstake(
                            msg.sender,
                            PhuketTokenAddress,
                            address(this),
                            _amount.sub((withdrawAmount.sub(stakerData[_expiredTimestamps[i]].altQuantity))),
                            _expiredTimestamps[i]
                        );
                        
                        stakerData[_expiredTimestamps[i]].altQuantity = stakerData[_expiredTimestamps[i]].altQuantity.sub(_amount.sub((withdrawAmount.sub(stakerData[_expiredTimestamps[i]].altQuantity))));
                    }
                    else{
                        emit Unstake(
                            msg.sender,
