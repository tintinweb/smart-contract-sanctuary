...
contract WIDPresale is Ownable {
    ...
    modifier tokensUnlocked() {
      require(block.timestamp > unlockDate, "WIDPresale: tokens not unlocked yet");
      _;
    }

    modifier afterClosedSale() {
      require(isClosed, "WIDPresale: Presale is not closed.");
      _;
    }
    ...
    function initializePresale(
      ...
      require(_unlockDate > _endDate || _unlockDate == 0, "WIDPresale: invalid unlockDate");
      ...
      widToken.transfer(_msgSender(), _tokensForLiquidity);
    ...
    function getTeamBalance() public view returns( address[] memory members, uint256[] memory balances ) {
      for(uint256 i=0;i<teamMembers.length;i++) {
        members[i] = teamMembers[i];
        balances[i] = teamBalances[teamMembers[i]];
      }
    }

    function setCanClaim(bool canClaim) external onlyOwner {
    ...
    function claimTokens() external tokensUnlocked {
      require(isClosed, "WIDPresale: sale not closed");
    ...
    function withdrawAllWid() public onlyOwner tokensUnlocked {
      require(isClosed, "WIDPresale: sale not closed");
      require(canClaimTokens, "WIDPresale: Claiming is not allowed yet!");
      require(tokensPurchased[_msgSender()] > 0, "WIDPresale: no tokens to claim");
    ...
    function disperseWIDtoTeam() private {
      for(uint256 i=0;i<teamMembers.length;i++) {
        widToken.transferFrom(address(this), teamMembers[i], teamBalances[teamMembers[i]]);
      }
    }
    ...
    function withdrawETH() external onlyOwner tokensUnlocked {
