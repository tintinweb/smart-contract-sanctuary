...
contract Spud3D {
    ...
    function GetSpud(address MN) public payable
    {
        ...
        if(blocknr - 256 > Spudgame[round][inroundindex].blocknumber && RNGresult != 1)
        {
            //win
            // won payout
            Pot += 1 finney;
            ...
            base = Pot.div(10);
            p3dContract.buy.value(base)(rotated);
            Spudgame[round][inroundindex].player.transfer(base.mul(5));
            emit payout(Spudgame[round][inroundindex].player , base.mul(5));
            Pot = Pot.sub(base.mul(6));
            // ifpreviouswon => new round
            nextround = curround++;
            Spudgame[round+1][0].player = sender;
            Spudgame[round+1][0].blocknumber = blocknr;
            newirindex = 0;
            round++;
            RNGdeterminator = 6;
        }
