...
interface IERC20 {
 ...
 contract gambling {
    ...
    function BetGame(uint _gameId, uint _option, uint _amount) public {
    ...
    function addResult(uint _gameId, uint _optionWinner, string memory _description) public {
        uint _now;
        _now = block.timestamp;
        uint _gameEnd;
    ...
    function collectMyEarnings() public {
        require(!lock);
        lock=true;
        uint _playerCounterBet;
        _playerCounterBet = playerCounterBet[msg.sender];
    ...
    function ActiveGames() public view returns(game[] memory) {
