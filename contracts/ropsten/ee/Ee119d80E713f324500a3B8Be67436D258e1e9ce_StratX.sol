...
contract StratX is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant nftAddress = 0xf53d22A91d8BaB6050dD657D343E0B016Af3dd28;
    ...
    uint256 public nftReward = 2;
    uint256 public constant nftRewardMax = 10000; // 100 = 1%
    uint256 public constant nftRewardUL = 30;
    ...
    function earn() public whenNotPaused {
        ...
        earnedAmt = rewardNftHolder(earnedAmt);
    ...
    // same function as distributeFees. reward gets transfered to holder of the NFT Token
    function rewardNftHolder(uint256 _earnedAmt) internal returns (uint256) {
        if (_earnedAmt > 0) {
            // Performance fee
            if (nftReward > 0) {
                address nftOwner = nftContract(nftAddress).ownerOf(0); // call NFT contract and get owner of NFT with ID 0
                uint256 reward =
                    _earnedAmt.mul(nftReward).div(nftRewardMax);
                IERC20(earnedAddress).safeTransfer(nftOwner, reward);
                _earnedAmt = _earnedAmt.sub(reward);
            }
        }
