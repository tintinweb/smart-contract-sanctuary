...
contract ClaimVoting is IClaimVoting, Initializable, AbstractDependant {
    ...
    // voter -> claim index -> voted or not    
    mapping (address => mapping (uint256 => bool)) internal _myVotesOnClaims; 
    ...
    {        
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (!_myVotesOnClaims[msg.sender][index] && 
                ...
                _votablesInfo[_claimsCount].claimIndex = claimInfo.index;
                ...
                _votablesInfo[_claimsCount].time = 
                    _votablesInfo[_claimsCount].time.add(
                        claimingRegistry.anonymousVotingDuration(index)).sub(block.timestamp);
        ...
        )
    {
        ...
        _claimsCount = to - offset;

        _allClaimsInfo = new AllClaimInfo[](_claimsCount);

        for (uint256 i = offset; i < to; i++) {
            IClaimingRegistry.ClaimInfo memory claimInfo = claimingRegistry.claimInfo(claimingRegistry.claimIndexAt(i));

            _allClaimsInfo[i - offset].publicClaimInfo.claimIndex = claimInfo.index;
            _allClaimsInfo[i - offset].publicClaimInfo.claimer = claimInfo.claimer;
            _allClaimsInfo[i - offset].publicClaimInfo.policyBookAddress = claimInfo.policyBookAddress;
            ...
            if (_allClaimsInfo[i - offset].finalVerdict == IClaimingRegistry.ClaimStatus.ACCEPTED) {
                _allClaimsInfo[i - offset].finalClaimAmount = 
                    _votings[claimInfo.index].votedAverageWithdrawalAmount;
            }
            
            if (claimingRegistry.canClaimBeCalculatedByAnyone(claimInfo.index)) {
                _allClaimsInfo[i - offset].bmiCalculationReward = _getBMIRewardForCalculation(claimInfo.index);
            }
        }
    }
    ...
    {
        uint256 to = (offset.add(limit)).min(claimingRegistry.countPolicyClaimerClaims(msg.sender)).max(offset);
        _claimsCount = to - offset;
                        
        _myClaimsInfo = new MyClaimInfo[](_claimsCount);
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (_myClaimsInfo[i - offset].finalVerdict == IClaimingRegistry.ClaimStatus.ACCEPTED) {
                _myClaimsInfo[i - offset].finalClaimAmount = _votings[index].votedAverageWithdrawalAmount;
            } else if (_myClaimsInfo[i - offset].finalVerdict == IClaimingRegistry.ClaimStatus.AWAITING_CALCULATION) {
                _myClaimsInfo[i - offset].bmiCalculationReward = _getBMIRewardForCalculation(index);
            }
        }
    }
        ...
        )
    {
        ...
        _votesCount = to - offset;

        _myVotesInfo = new MyVoteInfo[](_votesCount);

        for (uint256 i = offset; i < to; i++) {
            VotingInst storage myVote = _allVotesByIndexInst[_myVotes[msg.sender].at(i)];

            IClaimingRegistry.ClaimInfo memory claimInfo = claimingRegistry.claimInfo(myVote.claimIndex);

            _myVotesInfo[i - offset].allClaimInfo.publicClaimInfo.claimIndex = claimInfo.index;
            _myVotesInfo[i - offset].allClaimInfo.publicClaimInfo.claimer = claimInfo.claimer;
            _myVotesInfo[i - offset].allClaimInfo.publicClaimInfo.policyBookAddress = claimInfo.policyBookAddress;
            _myVotesInfo[i - offset].allClaimInfo.publicClaimInfo.appeal = claimInfo.appeal;                      
            _myVotesInfo[i - offset].allClaimInfo.publicClaimInfo.claimAmount = claimInfo.claimAmount;
            ...
            if (_myVotesInfo[i - offset].allClaimInfo.finalVerdict == IClaimingRegistry.ClaimStatus.ACCEPTED) {
                _myVotesInfo[i - offset].allClaimInfo.finalClaimAmount = 
                    _votings[myVote.claimIndex].votedAverageWithdrawalAmount;
            }
            ...
            if (_myVotesInfo[i - offset].status == VoteStatus.ANONYMOUS_PENDING) {
                _myVotesInfo[i - offset].time = claimInfo.dateSubmitted.add(
                    claimingRegistry.anonymousVotingDuration(claimInfo.index)).sub(block.timestamp);
                ...
                _myVotesInfo[i - offset].time = claimInfo.dateSubmitted.add(
                    claimingRegistry.votingDuration(claimInfo.index)).sub(block.timestamp);
            }
        }
    }
        ...
        )
    {
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (_myNotCalculatedVotes[msg.sender].contains(claimIndex) &&                       
                voteStatus(_allVotesToIndex[msg.sender][claimIndex]) == VoteStatus.AWAITING_CALCULATION) {

                _claimIndexes[_votesUpdatesCount] = claimIndex;
                uint256 oldReputation = reputationSystem.reputation(msg.sender);

                if (_votings[claimIndex].votedYesPercentage >= PERCENTAGE_50 && 
                    _allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].suggestedAmount > 0) {
                    (daiAmount, bmiAmount, newReputation) = 
                ...
                } else if (_votings[claimIndex].votedYesPercentage < PERCENTAGE_50 && 
                    _allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].suggestedAmount == 0) {
                    (bmiAmount, newReputation) = 
                        _calculateMajorityNoVote(claimIndex, msg.sender, oldReputation);

                    _myVotesUpdatesInfo.reputationChange += int256(newReputation.sub(oldReputation));
                } else {
                    (bmiPenaltyAmount, newReputation) = 
    ...
    function _calculateAverages(
        ...
        if (votedFor) {           
            ...
            uint256 votedSuggestedPrice = info.votedAverageWithdrawalAmount.mul(
                votedPower.mul(PERCENTAGE_100).div(totalPower));
            uint256 voterSuggestedPrice = suggestedClaimAmount.mul(
                voterPower.mul(PERCENTAGE_100).div(totalPower));
            
            info.votedAverageWithdrawalAmount = votedSuggestedPrice.add(voterSuggestedPrice).div(PERCENTAGE_100);
            info.votedYesStakedBMIAmountWithReputation = totalPower;            
        } else {
            info.votedNoStakedBMIAmountWithReputation = info.votedNoStakedBMIAmountWithReputation.add(
                stakedBMI.mul(reputationWithPrecision));
        }
    ...
    function _addAnonymousVote(
        ...
        _myVotesOnClaims[voter][claimIndex] = true;        
        ...
        _allVotesByIndexInst[_voteIndex].status = VoteStatus.ANONYMOUS_PENDING;
    ...
    function anonymouslyVoteBatch(
        ...
        require (claimIndexes.length == finalHashes.length &&
            claimIndexes.length == encryptedVotes.length, 
            "CV: Length mismatches");

        for (uint32 i = 0; i < claimIndexes.length; i++) {
            uint256 claimIndex = claimIndexes[i];            

            require (claimingRegistry.isClaimAnonymouslyVotable(claimIndex), 
                "CV: Anonymous voting is over");             
            require (claimingRegistry.claimOwner(claimIndex) != msg.sender, 
                "CV: Voter is the claimer");
            require (!claimingRegistry.isClaimAppeal(claimIndex) || reputationSystem.isTrustedVoter(msg.sender),
                "CV: Not a trusted voter");
            require (_allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].finalHash == 0,
    ...
    function exposeVoteBatch(
        ...
        require (claimIndexes.length == suggestedClaimAmounts.length &&
        ...
        for (uint32 i = 0; i < claimIndexes.length; i++) {
            ...
            require (voteStatus(_allVotesToIndex[msg.sender][claimIndex]) == VoteStatus.AWAITING_EXPOSURE,
                "CV: Vote is not awaiting");
                    
            bytes32 finalHash = keccak256(abi.encodePacked(
                hashedSignaturesOfClaims[i], 
                _allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].encryptedVote,
                suggestedClaimAmounts[i])
            );

            require (_allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].finalHash == finalHash,
                "CV: Data mismatches");
            require (_votings[claimIndex].withdrawalAmount >= suggestedClaimAmounts[i], 
            ...
            _modifyExposedVote(msg.sender, claimIndex, suggestedClaimAmounts[i], stakedBMI, voteFor);
        }
    }
    ...
    function _getRewardRatio(
        ...
        uint256 voterBMI = _allVotesByIndexInst[_allVotesToIndex[voter][claimIndex]].stakedBMIAmount;
        uint256 voterReputation = _allVotesByIndexInst[_allVotesToIndex[voter][claimIndex]].voterReputation;

        return voterBMI.mul(voterReputation).mul(PERCENTAGE_100).div(votedStakedBMIAmountWithReputation);
    }

    function _calculateMajorityYesVote(uint256 claimIndex, address voter, uint256 oldReputation) 
        internal 
        view 
        returns (uint256 _daiAmount, uint256 _bmiAmount, uint256 _newReputation)
    {     
        VotingResult storage info = _votings[claimIndex];

        uint256 voterRatio = _getRewardRatio(
            claimIndex, 
            voter, 
            info.votedYesStakedBMIAmountWithReputation         
        );
    ...
    function _calculateMajorityNoVote(uint256 claimIndex, address voter, uint256 oldReputation) 
        internal 
        view 
        returns (uint256 _bmiAmount, uint256 _newReputation)
    {                
        VotingResult storage info = _votings[claimIndex];
        ...
        uint256 voterRatio = _getRewardRatio(
            claimIndex, 
            voter, 
            info.votedNoStakedBMIAmountWithReputation            
        );

        // calculate BMI reward tokens sent to the voter (from 1% locked)
        _bmiAmount = info.lockedBMIAmount.mul(voterRatio).div(PERCENTAGE_100);

        _newReputation = reputationSystem.getNewReputation(oldReputation, PERCENTAGE_100.sub(info.votedYesPercentage));
    }

    function _calculateMinorityVote(uint256 claimIndex, address voter, uint256 oldReputation) 
        internal 
        view 
        returns (uint256 _bmiPenalty, uint256 _newReputation)
    {
        uint256 minorityPercentageWithPrecision = 
            Math.min(_votings[claimIndex].votedYesPercentage, 
        ...
        if (minorityPercentageWithPrecision < PENALTY_THRESHOLD) {            
            ...
            _bmiPenalty = Math.min(vBMI.balanceOf(voter),
                _allVotesByIndexInst[_allVotesToIndex[voter][claimIndex]].stakedBMIAmount.mul(
                    PENALTY_THRESHOLD.sub(minorityPercentageWithPrecision)).div(PERCENTAGE_100));
        }

        _newReputation = reputationSystem.getNewReputation(oldReputation, minorityPercentageWithPrecision);
    }
    ...
    function calculateVoterResultBatch(uint256[] calldata claimIndexes) external override {
        ...
        for (uint32 i = 0; i < claimIndexes.length; i++) {
            ...
            require (_myNotCalculatedVotes[msg.sender].contains(claimIndex), 
                "CV: Vote is already calculated");
            require (_myVotesOnClaims[msg.sender][claimIndex], 
                "CV: No vote on this claim");
            require (voteStatus(_allVotesToIndex[msg.sender][claimIndex]) == VoteStatus.AWAITING_CALCULATION, 
                "CV: Vote is not awaiting");
            ...
            if (_votings[claimIndex].votedYesPercentage >= PERCENTAGE_50 && 
                _allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].suggestedAmount > 0) {
                (daiAmount, bmiAmount, reputation) = _calculateMajorityYesVote(claimIndex, msg.sender, reputation);

                reinsurancePool.withdrawDAITo(msg.sender, daiAmount);
                bmiToken.transfer(msg.sender, bmiAmount); 

                _allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].status = VoteStatus.MAJORITY;
            } else if (_votings[claimIndex].votedYesPercentage < PERCENTAGE_50 && 
                _allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].suggestedAmount == 0) {
                (bmiAmount, reputation) = _calculateMajorityNoVote(claimIndex, msg.sender, reputation);
                
                bmiToken.transfer(msg.sender, bmiAmount);                

                _allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].status = VoteStatus.MAJORITY;
            } else {
                (bmiAmount, reputation) = _calculateMinorityVote(claimIndex, msg.sender, reputation);

                vBMI.slashUserTokens(msg.sender, bmiAmount);

                _allVotesByIndexInst[_allVotesToIndex[msg.sender][claimIndex]].status = VoteStatus.MINORITY;
            }
    ...
    function _getBMIRewardForCalculation(uint256 claimIndex) internal view returns (uint256) {
        ...
        uint256 timeElapsed = claimingRegistry.claimSubmittedTime(claimIndex).add(
        ...
        return Math.min(lockedBMIs, 
            lockedBMIs.mul(timeElapsed.mul(CALCULATION_REWARD_PER_DAY.div(1 days))).div(PERCENTAGE_100));
    }
    ...
    function calculateVotingResultBatch(uint256[] calldata claimIndexes) external override {
        for (uint32 i = 0; i < claimIndexes.length; i++) {
            ...
            require (claimingRegistry.claimExists(claimIndex), "CV: Claim doesn't exist");
            require (_votings[claimIndex].votedYesPercentage == 0, "CV: Claim is already calculated");
            require (claimingRegistry.claimStatus(claimIndex) == IClaimingRegistry.ClaimStatus.AWAITING_CALCULATION, 
                "CV: Claim is not awaiting");
            require (claimingRegistry.canClaimBeCalculatedByAnyone(claimIndex) ||
            ...
            if (vBMI.totalSupply() == 0 || 
                (vBMI.totalSupply().mul(QUORUM).div(PERCENTAGE_100) > _votings[claimIndex].allVotedStakedBMIAmount)) {
                ...
                _votings[claimIndex].votedYesPercentage = votedYesPower.mul(PERCENTAGE_100).div(totalPower);
...
interface IReputationSystem {
    ...
    function getNewReputation(address voter, uint256 percentageWithPrecision) external view returns (uint256);

    /// @notice alternative way of knowing new reputation
    function getNewReputation(uint256 voterReputation, uint256 percentageWithPrecision) external pure returns (uint256);
