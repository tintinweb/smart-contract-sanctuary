...
contract NoopStrategy is StrategyBase {
  ...
  function isZeroAddressPool() override internal pure returns (bool) {
    return false;
  }
  ...
  function liquidateReward() override internal {
    // noop
  }
...
abstract contract StrategyBase is IStrategy, Controllable {
  ...
  // *********************** EVENTS *******************
  event ProfitAndBuybackLog(address token, uint256 profitAmount, uint256 feeAmount, uint256 timestamp);
  ...
  function rewardBalance(uint256 rewardTokenIdx) public view returns (uint256) {
    return IERC20(rewardTokens[rewardTokenIdx]).balanceOf(address(this));
  }
 ...
 */
  ...
  function distributeRewards(uint256 _rewardBalance, address _rewardToken) internal {
    if (_rewardBalance > 0) {
      ...
      if (feeAmount > 0) {
        targetTokenEarned += IFeeRewardForwarder(forwarder).notifyPsPool(_rewardToken, feeAmount);
      }
      if (buybackAmount > 0) {
        targetTokenEarned += IFeeRewardForwarder(forwarder).notifyCustomPool(_rewardToken, _smartVault, buybackAmount);
      }
  ...
  function liquidateRewardDefault() internal {
    for (uint256 i; i < rewardTokens.length; i++) {
      ...
      distributeRewards(rewardBalance(i), rewardTokens[i]);
    }
  }
  ...
  function rewardPoolBalance() public virtual view returns (uint256 bal);

  function depositToPool(uint256 amount) virtual internal;

  function withdrawAndClaimFromPool(uint256 amount) virtual internal;

  function emergencyWithdrawFromPool() virtual internal;

  function liquidateReward() virtual internal;

  function isZeroAddressPool() virtual internal view returns (bool);
