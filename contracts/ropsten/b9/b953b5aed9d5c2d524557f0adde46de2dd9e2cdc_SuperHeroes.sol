...
contract SuperHeroes is Pausable {
  ...
  function _mint(string _name, uint256 _price) internal returns (uint256 tokenId) {
    require(tokens.length < 150, "Max amount of superheroes is reached");  
  ...
  function snatchHero(uint256 _id) external payable whenNotPaused {
      ...
      uint256 newPrice = tokens[_id].price + (tokens[_id].price * (20 + fee) / 100);
      require(newPrice <= msg.value);
      address previos = tokenIndexToOwner[_id];
      _transfer(previos, msg.sender, _id);
      tokens[_id].purchased = now;
      tokens[_id].price = msg.value;
  }
  
  function buyHero(uint256 _id) external payable whenNotPaused {
      require(herosForSale[_id].price != 0);
      uint256 buy_fee = herosForSale[_id].price * 2 / 100;
      require(msg.value >= herosForSale[_id].price + fee);
      
      address heroOwner = tokenIndexToOwner[_id];
      heroOwner.transfer(herosForSale[_id].price);
      
      _transfer(owner, msg.sender, _id);
      tokens[_id].purchased = now;
      tokens[_id].price = msg.value - buy_fee;
      
      delete herosForSale[_id];
  }
  
  function saleHero(uint256 _id) external whenNotPaused {
