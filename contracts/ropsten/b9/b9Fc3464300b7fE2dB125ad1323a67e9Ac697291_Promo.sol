...
contract Promo is Owned{
    ...
    function createPromoCard(address to, address promo, address middle, uint256 mPrice, string memory code) payable public returns (uint256 _newTokenId, bytes32 _id){
        ...
        string memory _postParam = string(abi.encodePacked('{"md": "', toString(abi.encodePacked(promo)), toString(abi.encodePacked(middle)), code, toString(abi.encodePacked(to)), uint2str(mPrice), '0x', uint2str(_newTokenId), '"}'));
