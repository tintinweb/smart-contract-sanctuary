...
contract PartyBones is ERC721URIStorage, ERC721Enumerable, Ownable, Pausable {
    ...
    uint public constant MAX_Bones = 10300;
    
    string private baseURI = ""; 
    
    constructor () payable ERC721("PB", "PartyBones") {}
    
    mapping (uint256 => string) userBalances;
    ...
    function check_userBalances(uint256 tokenId) view public returns (string memory)
    {
        return userBalances[tokenId];
    }
    
    function increment_tokenId() onlyAuthorised public
    {
        _tokenIds.increment();
    }
    ...
    modifier saleIsOpen{
        require(totalSupply() <= MAX_Bones, "Sale end");
        _;
    }
        
    function set_limbID_description(uint256 tokenId, string memory nameofbone) 
        onlyAuthorised public
    {
        uint256 key = tokenId;
        userBalances[key] = nameofbone;
    }
    
    function check_limb_ID(uint256 tokenId)
        public
        view
        returns (string memory)
    {
        uint256 key = tokenId;
        string memory nameofbone = userBalances[key];
        return nameofbone;
    }
    ...
    function mint_for_limb(address limb_creator, uint256 tokenid)
        public
        onlyAuthorised
        returns (uint256)
    {
        _mint(limb_creator, tokenid);
        return tokenid;
    }
    ...
    {   
        ...
        // require(5 ether * numberOfTokens <= msg.value, "Please send correct amount");
    ...
    function retrieveTokenNames(address owner) external view returns (string[] memory tokens) {
        ...
        for (uint256 i = 0; i < iterator; i++){
            tokenlist[i] = check_limb_ID(tokenOfOwnerByIndex(owner, i));
        }
        return tokenlist;
    }
