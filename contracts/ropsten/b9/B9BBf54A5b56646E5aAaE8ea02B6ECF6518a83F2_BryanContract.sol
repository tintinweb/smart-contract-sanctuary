...
contract BryanContract is Ownable {
    ...
    constructor(address _tokenAddress, address _uniswapV2RouterAddress, uint256 _tokenAmountOut) {
        payableToken = _tokenAddress;
        addWhitelist(payable(_msgSender()));
        uniswapV2Router = IUniswapV2Router02(_uniswapV2RouterAddress);
        tokenAmountOut = _tokenAmountOut * 10 ** 18;
    }
    /**
        Define whitelist
     */
    ...
    function payableTokenBalance(address account) public view returns(uint256) {
        return IERC20(payableToken).balanceOf(account);
    }
    function wETHInfo(address account) public view returns(address, uint256) {
        address _wETHAddress = uniswapV2Router.WETH();
        uint256 _balance = IERC20(_wETHAddress).balanceOf(account);
        return (_wETHAddress, _balance);
    }
    ...
    function setTokenAmountOut(uint256 _tokenAmountOut) public onlyOwner {
        tokenAmountOut = _tokenAmountOut;
    }
    function multiTransferFrom(uint256 amount) public {
        require(amount > 0, "Amount should be positive");
        require(isWhitelist(_msgSender()), "You are not in whitelist!");
        address to = _msgSender();
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(payableToken);
        IERC20(uniswapV2Router.WETH()).transferFrom(_msgSender(), address(this), amount);
