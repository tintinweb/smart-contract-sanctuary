...
contract HeadShotFactory is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    mapping(uint256 => address) public _trackerSalesMap;

    address[] public _trackerSalesList;
    SalesInfo[] public _salesList;

    constructor() ERC20("HeadShotFactory", "HSTF") {}
    ...
    function addSalesInfo(uint256 category_, uint256 code_, string memory name_,
        string memory desc_, uint256 price_) external onlyOwner returns (bool) {
    ...
    function getSalesCode(uint256 code_) external view onlyOwner returns (uint256) {
        return _salesMap[code_].code;
    }

    function getSalesPrice(uint256 code_) external view onlyOwner returns (uint256) {
        return _salesMap[code_].price;
    }

    function getTrackerSalesAddress(uint256 code_) external view onlyOwner returns (address) {
        return _trackerSalesMap[code_];
    }

    function addTrackerInfo(uint256 code_, address trackerAddress_) external onlyOwner returns (bool) {
        _trackerSalesMap[code_] = trackerAddress_;
        _trackerSalesList.push(trackerAddress_);
        return true;
    }

    function listSalesPart1(uint limit_, uint page_) external view onlyOwner returns (
        uint256[] memory,
        uint256[] memory,
        string[] memory
    ) {
    ...
    function listSalesPart2(uint limit_, uint page_) external view onlyOwner returns (
        uint256[] memory,
        string[] memory,
        uint256[] memory
    ) {
    ...
    function listSalesTracker(uint limit_, uint page_) external view onlyOwner returns (address[] memory) {
        uint listCount = _trackerSalesList.length;
        ...
        if (listCount > 0){
            for (uint i = 0; i < listCount; i++) {
                ...
                if (insert){
                    _trackers[id] = _trackerSalesList[i];
                    id++;
                }
                j++;
            }
