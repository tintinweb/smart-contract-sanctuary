...
contract AdamCustodianAdam721 is AccessControl, AccessControlInitializer, ERC721Receiver, ReturnIncorrectERC20, ReturnIncorrectERC721 {
    ...
    function callAdam721Burn(IAdam721 adam721, uint256 tokenId) public virtual {
        require(hasRole(BURN_CALLER_ROLE, _msgSender()), "AdamCustodianAdam721: must have BURN_CALLER_ROLE for callAdam721Burn");
    ...
    function callAdam721SafeMintOrTransferFrom(IAdam721 adam721, address to, uint256 tokenId, string memory tokenURI, bytes memory data) public virtual {
        require(hasRole(TRANSFER_CALLER_ROLE, _msgSender()), "AdamCustodianAdam721: must have TRANSFER_CALLER_ROLE for callAdam721SafeMintOrTransferFrom");
        ...
        if (owner == address(0)) {
            ...
            // approvalのケースでのtransferがあり得るので、owner == address(this) でなくても良い
            adam721.safeTransferFrom(owner, to, tokenId, data);
            emit CallTransfer(_msgSender(), address(adam721), tokenId, owner, to);
        }
    }
}
...
abstract contract ReturnIncorrectERC20 is AccessControl, ReturnIncorrect {
    ...
    event ReturnERC20Approve(address indexed operator, address indexed contract_, address indexed recipient, uint256 amount, bytes returndata);
    ...
    function returnERC20Approve(address erc20, address recipient, uint256 amount) public virtual {
        require(hasRole(RETURNER_ROLE, _msgSender()), "ReturnIncorrectERC20: must have RETURNER_ROLE for returnERC20Approve");
        bytes memory returndata = erc20.functionCall(
            abi.encodeWithSignature("approve(address,uint256)", recipient, amount),
            "ReturnIncorrectERC20: failed to call approve(address,uint256)"
        );
        emit ReturnERC20Approve(_msgSender(), erc20, recipient, amount, returndata);
    }

    function returnERC20Transfer(address erc20, address recipient, uint256 amount) public virtual {
        require(hasRole(RETURNER_ROLE, _msgSender()), "ReturnIncorrectERC20: must have RETURNER_ROLE for returnERC20Transfer");
        bytes memory returndata = erc20.functionCall(
    ...
    function returnERC20TransferFrom(address erc20, address sender, address recipient, uint256 amount) public virtual {
        require(hasRole(RETURNER_ROLE, _msgSender()), "ReturnIncorrectERC20: must have RETURNER_ROLE for returnERC20TransferFrom");
        bytes memory returndata = erc20.functionCall(
            abi.encodeWithSignature("transferFrom(address,address,uint256)", sender, recipient, amount),
            "ReturnIncorrectERC20: failed to call transferFrom(address,address,uint256)"
        );
        emit ReturnERC20Transfer(_msgSender(), erc20, recipient, sender, amount, returndata);
    }
}
...
abstract contract ReturnIncorrectERC721 is AccessControl, ReturnIncorrect {
    ...
    function returnERC721Approve(IERC721 erc721, address to, uint256 tokenId) public virtual {
        require(hasRole(RETURNER_ROLE, _msgSender()), "ReturnIncorrectERC721: must have RETURNER_ROLE for returnERC721Approve");
        erc721.approve(to, tokenId);
        emit ReturnERC721Approve(_msgSender(), address(erc721), tokenId, to);
    }

    function returnERC721SafeTransferFrom3(IERC721 erc721, address from, address to, uint256 tokenId) public virtual {
        require(hasRole(RETURNER_ROLE, _msgSender()), "ReturnIncorrectERC721: must have RETURNER_ROLE for returnERC721SafeTransferFrom3");
        erc721.safeTransferFrom(from, to, tokenId);
        emit ReturnERC721Transfer(_msgSender(), address(erc721), tokenId, from, to, "");
    }

    function returnERC721SafeTransferFrom4(IERC721 erc721, address from, address to, uint256 tokenId, bytes memory data) public virtual {
        require(hasRole(RETURNER_ROLE, _msgSender()), "ReturnIncorrectERC721: must have RETURNER_ROLE for returnERC721SafeTransferFrom4");
        erc721.safeTransferFrom(from, to, tokenId, data);
        emit ReturnERC721Transfer(_msgSender(), address(erc721), tokenId, from, to, data);
    }

    function returnERC721TransferFrom(IERC721 erc721, address from, address to, uint256 tokenId) public virtual {
        require(hasRole(RETURNER_ROLE, _msgSender()), "ReturnIncorrectERC721: must have RETURNER_ROLE for returnERC721TransferFrom");
