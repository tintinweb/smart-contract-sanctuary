...
contract Unfoldrian is UnfoldrianPricing, EIP712 {
    ...
    function test(
        bytes memory data
    ) public view returns(uint256[2] memory, uint256, bytes memory) {
        (uint256[2] memory tokens, uint256 expiry, bytes memory signature) = abi.decode(data,(uint256[2], uint256, bytes));
