...
contract WiseGuys is Context, IERC20, Ownable {
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !_preTrader[from] && !_preTrader[to]) {
            
            //Trade start check
            if (!_tradingOpen) {
                require(_preTrader[from], "TOKEN: This account cannot send tokens until trading is enabled");
            }
              
            require(amount <= MaxTxAmount, "TOKEN: Max Transaction Limit");
            require(!_bots[from] && !_bots[to], "TOKEN: Your account has been blacklisted");
