...
// File: contracts/LicenseRegistrar/NSXTPerpetual.sol

pragma solidity ^0.8.0;



contract NSXTPerpetual is Ownable, LNFT {
    string public name = "VMware NSX Transformer Perpetual License";
    string public symbol = "NSXTP";
    ...
    /// Only contract owner (VMware) is approved to mint new type of perpetual license
    function _isSemiTypeMintApproved(address _operator, address _to, uint256 _licenseType, uint256 _value, bytes memory _data) internal override onlyOwner returns (bool) {
    ...
    /// Always never expire for perpetual license
    function _setExpireTime(uint256 _licenseType, uint256 _licenseId, uint256, bytes memory _data) internal override {
        super._setExpireTime(_licenseType, _licenseId, TIME_INFINITE, _data);
    }

    function expireOn(uint256 _licenseId) public view override returns (uint256) {
        require(_licenseExists(_licenseId), "LNFT: _licenseId should exist");
