...
contract ProxyController is AccessControl, IProxyController {
     ...
     *
     */
    function startOrder(address borrower, uint256 amountIn, uint256 priceA, uint256 priceB, OrderInput memory input) public virtual {
        address borrower_ = _msgSender();
        ...
        bytes32 orderId = keccak256(abi.encode(borrower_, amountIn, input.tokenA, input.tokenB, block.timestamp));
        ...
        emit Swapped(orderId, borrower_, input.tokenA, input.tokenB, amounts[0], amounts[1]);
        emit OrderCreated(orderId, borrower_, input.tokenA, input.tokenB, amounts[0], input.limit, input.stop, input.expiry, input.autoRepay);
        ...
        orders[orderId] = Order(orderTokenA, orderTokenB, borrower_, input.limit, input.stop, input.expiry, input.autoRepay);
