...
contract LpStaking is ReentrancyGuard{
    ...
    function calculateLpStakingIncomeRate(uint256 _incomeRate) internal view returns(uint256) {
        ...
        uint256 _totolSupply = IERC20(hashRateToken).totalSupply().mul(OneThousand.sub(feeRate)).div(OneThousand);
    ...
    function pendingFeeIncome() internal view returns(uint256 pendingFee) {
        ...
        uint256 OneThousand = 1000;
        uint256 _totolSupply = IERC20(hashRateToken).totalSupply().mul(feeRate).div(OneThousand);
        uint256 feeIncomeRate = _totolSupply.mul(incomeRate).div(1e18);
        pendingFee = block.timestamp.sub(_incomeLastUpdateTime).mul(feeIncomeRate);
    }
    ...
    function getAccumulateFee() external view returns (uint256) {
        ...
        uint256 pendingFee = pendingFeeIncome();
        totalFee = totalFee.add(pendingFee);
    ...
    modifier updateIncome(address account)  {
        ...
        if (block.timestamp > startMiningTime) {
            ...
            uint256 lastIncomePerToken = incomePerTokenStored;
            uint256 currentIncomePerToken = incomePerToken();
            uint256 incomeAmount = currentIncomePerToken.sub(lastIncomePerToken).mul(totalSupply).div(1e18);
            emit IncomeCounted(startTime, currentTime, incomeAmount);

            uint256 pendingFee = pendingFeeIncome();
            accumulateFee = accumulateFee.add(pendingFee);
            incomePerTokenStored = currentIncomePerToken;
