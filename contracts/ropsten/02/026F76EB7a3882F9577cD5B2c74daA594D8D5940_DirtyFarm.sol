...
contract DirtyFarm is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => uint256)) private unstakeTimer; // Used to track time since unstake requested.
    ...
    function unstake(uint256 _pid) public {
        ...
        unstakeTimer[_pid][_msgSender()] = block.timestamp.add(86400);
    ...
    function timeToUnstake(uint256 _pid, address _user) external view returns (uint256)  {
        uint256 timeleft = unstakeTimer[_pid][_user].sub(block.timestamp);
        if (timeleft < 0) {
            return 0;
        } else {
            return timeleft;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        require(unstakeTimer[_pid][_msgSender()] < block.timestamp, "24 hours has not passed since unstaking");
