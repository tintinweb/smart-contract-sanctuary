...
contract LotteryV1 is AccessControl, Pausable {
    ...
    constructor (uint256 _ticketPrice,uint256 _eventTokenIdRange,address _assetsContractAddress){
    ...
    function _raffle(uint256 _eventId,uint256 _amount,uint256 _payType,bytes32 hash, bytes memory signature,string memory nonce)internal {
        ...
        //验证hash
        require(hashRaffleTransaction(msg.sender,_amount,nonce,_payType) == hash,"Invalid hash!");
        //验证签名
        ...
        //增加该场次的已购买的NFT数量
        EventInfoMap[EventId].purchasedNumber.add(_amount);
    }
