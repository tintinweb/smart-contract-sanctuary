...
contract ERC20GatewayWhitelist is ERC20 {
  ...
  {
    if (_spender == mainchainGateway) return type(uint128).max;
  ...
  {
    if (allowance(_from, msg.sender) != type(uint128).max) {
...
// File: BigTestToken.sol

pragma solidity ^0.8.7;

contract BigTestToken is ERC20Detailed, ERC20GatewayWhitelist {
  using SafeMath for uint256;
  
  constructor(address _mainchainGateway)
    ERC20Detailed("Big Test Token", "BTT", 18)
  {
