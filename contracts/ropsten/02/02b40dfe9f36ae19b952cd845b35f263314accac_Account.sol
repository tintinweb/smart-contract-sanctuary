...
// Additional methods available for USDC
...
contract FlashLoanTemplate is ICallee {
    // The USDC token contract, since we're assuming we want a loan in USDC
    ...
    constructor() {
        // Give infinite approval to dydx to withdraw USDC on contract deployment,
    ...
    function flashLoan(uint loanAmount) external {
        ...
        The loan must be given as an ERC-20 token, so USDC is used instead of ETH. Other
    ...
    function callFunction(address sender, Account.Info memory accountInfo, bytes memory data) external override {
        ...
        // We now have a USDC balance of loanAmount. The logic for what we
        ...
        // Leave just enough USDC to pay back the loan, and convert the rest to ETH
