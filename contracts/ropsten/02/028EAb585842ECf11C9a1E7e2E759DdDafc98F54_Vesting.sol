...
contract Vesting is IVesting {
        ...
        returns (uint256 vestedAmount)
    {
        ...
        uint256 scheduleEndTimestamp = cliffEndTimestamp.add(
            intervalSeconds.mul(vestingSchedule.numberOfIntervals)
        );
        ...
        if (vestingSchedule.releaseMethod == ReleaseMethod.IntervalEnd) {
            ...
            uint256 secondsInInterval = block.timestamp.sub(
                cliffEndTimestamp.add(intervalSeconds.mul(intervalNumber))
            );
