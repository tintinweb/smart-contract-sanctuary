...
contract PulaToken is StandardToken {
    string public constant name = "PulaToken";
...
contract Crowdsale is Ownable {
    ...
    PulaToken public token = new PulaToken(myAddress);
    ...
    constructor() public {
        ...
        giveTokens(address(teamAddress), 500000);
    ...
    function calcBonus () public view returns(uint256) {
        ...
        if (amountToken < 200000 * (1 ether)){ 
            actualBonus = 45;    
        }
        if (amountToken >= 200000 * (1 ether) && amountToken < 300000 * (1 ether)){
            actualBonus = 40;
        }
        if (amountToken >= 300000 * (1 ether) && amountToken < 400000 * (1 ether)){
            actualBonus = 30;
        }
        if (amountToken >= 400000 * (1 ether) && amountToken < 5500000 * (1 ether)){
            actualBonus = 20;
        }
        if (amountToken >= 550000 * (1 ether) && amountToken < 6000000 * (1 ether)){
            actualBonus = 10;
        }
