...
//   8_888_888_888
  uint256 private constant _totalPercent = 1e64;

  uint256 public TRADE_LIMIT = 0;
  uint256 public BURNING_PERCENT = 100;
  uint256 public constant HUNDRED_PERCENT = 10000;
//   8_888_888_888
  uint256 public constant cap = 1 * 1_000_000_000;
  ...
  /**
   * @dev calculate burned amount.
   */
  function burnedAmount() public view returns (uint256) {
    return cap.sub(_totalSupply);
  }
  ...
  function _transfer(
    ...
    require(senderBalance >= amount.mul(rate), "ERC20: transfer amount exceeds balance");
    unchecked {
      _balances[sender] = senderBalance - amount.mul(rate);
    }
    _balances[recipient] += amount.mul(rate);
  ...
  function _mint(address account, uint256 amount) internal virtual {
    ...
    require(_totalSupply < cap, "ERC20: Token is on the cap");
    uint256 rate = _totalPercent.div(cap);
  ...
  function _burn(address account, uint256 amount) internal virtual {
    ...
    require(accountBalance >= amount.mul(rate), "ERC20: burn amount exceeds balance");
    unchecked {
      _balances[account] = accountBalance - amount.mul(rate);
    }
    _totalSupply -= amount;
