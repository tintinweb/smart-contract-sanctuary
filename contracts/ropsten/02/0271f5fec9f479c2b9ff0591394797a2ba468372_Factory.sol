...
contract Factory is Configurable, ContextUpgradeSafe, Constants {
    ...
    function mintCall4(address underlying, address currency, uint priceFloor, uint priceCap, uint volCall, uint undInMax, uint curInMax) public returns (address call, int dUnd, int dCur) {
        (call, , dUnd, dCur) = swap(underlying, currency, priceFloor, priceCap, int(volCall), 0, int(undInMax), int(curInMax));
    }
    function mintCall(address call, uint volCall, uint undInMax, uint curInMax) public returns (int dUnd, int dCur) {
        (, , dUnd, dCur) = swap(Call(call).underlying(), Call(call).currency(), Call(call).priceFloor(), Call(call).priceCap(), int(volCall), 0, int(undInMax), int(curInMax));
    }
    function mintCall_(address sender, address call, uint volCall, uint undInMax, uint curInMax) public returns (int dUnd, int dCur) {
        require(msg.sender == call, 'Only Call');
        (, , dUnd, dCur) = _swap(sender, Call(call).underlying(), Call(call).currency(), Call(call).priceFloor(), Call(call).priceCap(), int(volCall), 0, int(undInMax), int(curInMax));
    }

    function mintPut4(address underlying, address currency, uint priceFloor, uint priceCap, uint volPut, uint undInMax, uint curInMax) public returns (address put, int dUnd, int dCur) {
        (, put, dUnd, dCur) = swap(underlying, currency, priceFloor, priceCap, 0, int(volPut), int(undInMax), int(curInMax));
    }
    function mintPut(address put, uint volPut, uint undInMax, uint curInMax) external returns (int dUnd, int dCur) {
        (, , dUnd, dCur) = swap(Put(put).underlying(), Put(put).currency(), Put(put).priceFloor(), Put(put).priceCap(), 0, int(volPut), int(undInMax), int(curInMax));
    }
    function mintPut_(address sender, address put, uint volPut, uint undInMax, uint curInMax) external returns (int dUnd, int dCur) {
        require(msg.sender == put, 'Only Put');
        (, , dUnd, dCur) = _swap(sender, Put(put).underlying(), Put(put).currency(), Put(put).priceFloor(), Put(put).priceCap(), 0, int(volPut), int(undInMax), int(curInMax));
    }
...
contract Call is ERC20UpgradeSafe {
    ...
    function mint(uint volume, uint undInMax, uint curInMax) external returns (int dUnd, int dCur) {
        return Factory(factory).mintCall_(_msgSender(), address(this), volume, undInMax, curInMax);
    }
...
contract Put is ERC20UpgradeSafe, Constants {
    ...
    function mint(uint volume, uint undInMax, uint curInMax) external returns (int dUnd, int dCur) {
        return Factory(factory).mintPut_(_msgSender(), address(this), volume, undInMax, curInMax);
    }
