...
contract ComptrollerV0_02 is ComptrollerStorageOlaV0_02, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    event NewRainMaker(RainMaker oldRainMaker, RainMaker newRainMaker);

    // OLA_ADDITIONS : This event
    /// @notice Emitted when the Whitelister is changed
    event NewWhitelister(address oldWhitelister, address newWhitelister);
    ...
    function getRegistry() public returns (address) {
    ...
    */
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external returns (uint) {
        ...
        if (hasRainMaker()) {
            rainMaker.updateCompSupplyIndex(cToken);
            rainMaker.distributeSupplierComp(cToken, minter);
        }
    ...
    function redeemAllowed(address cToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        if (hasRainMaker()) {
            // Keep the flywheel moving
            rainMaker.updateCompSupplyIndex(cToken);
            rainMaker.distributeSupplierComp(cToken, redeemer);
        }
        ...
        if (!markets[cToken].accountMembership[borrower])
{
        ...
        if (hasRainMaker()) {
            ...
            rainMaker.updateCompBorrowIndex(cToken, borrowIndex);
            rainMaker.distributeBorrowerComp(cToken, borrower, borrowIndex);
        }
    ...
    function seizeAllowed(
        ...
        if (hasRainMaker()) {
            // Keep the flywheel moving
            rainMaker.updateCompSupplyIndex(cTokenCollateral);
            rainMaker.distributeSupplierComp(cTokenCollateral, borrower);
            rainMaker.distributeSupplierComp(cTokenCollateral, liquidator);
        }
    ...
    function transferAllowed(address cToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        if (hasRainMaker()) {
            // Keep the flywheel moving
            rainMaker.updateCompSupplyIndex(cToken);
            rainMaker.distributeSupplierComp(cToken, src);
            rainMaker.distributeSupplierComp(cToken, dst);
        }
     ...
     */
    function _setRainMaker(RainMaker newRainMaker) external returns (uint) {
        ...
        if (hasRainMaker()) {
            rainMaker.retireRainMaker();
        }

        // Track the old rain maker for the Comptroller
        RainMaker oldRainMaker = rainMaker;
    ...
    /**
     * @notice Sets a new Whitelister for the Comptroller
     * @dev Admin function to set a new Whitelister
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setWhitelister(address newWhitelister) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_WHITELISTER_OWNER_CHECK);
        }

        // Track the Whitelister for the Comptroller
        address oldWhitelister = whitelister;

        // Set Comptroller's whitelister to newWhitelister
        whitelister = newWhitelister;

        emit NewWhitelister(oldWhitelister, newWhitelister);
    ...
    function _supportNewMarket(address underlying,
        ...
        address payable adminForMarket,
        ...
        address deployedCToken = registry.deployOToken(underlying, contractNameHash, params, interestRateModel, adminForMarket, becomeImplementationData);
    ...
    function _addMarketInternal(address cToken) internal {
        ...
        if (hasRainMaker()) {
            rainMaker._supportMarket(cToken);
        }
    }
    ...
    function updateDelegatedImplementations(bytes calldata becomeImplementationData) external {
        ...
        for (uint i = 0; i < allMarkets.length; i ++) {
            ...
            require(CDelegatorInterface(address(oToken)).updateImplementationFromRegistry(false, becomeImplementationData), "Update failed");
        }
    }
    ...
    function getOracleForCToken(address cToken) view public returns (PriceOracle) {
        PriceOracle oracle = PriceOracle(registry.getOracleForAsset(CToken(cToken).underlying()));
        return oracle;
    }
    ...
    function hasWhitelister() view public returns (bool) {
        return address(whitelister) != address(0);
    }
    ...
    function isAccountApprovedInternal(address account) view internal returns (bool) {
        if (hasWhitelister()) {
            return IBouncerForComptroller(whitelister).isAccountApproved(account);
        } else {
            return false;
        }
    }
