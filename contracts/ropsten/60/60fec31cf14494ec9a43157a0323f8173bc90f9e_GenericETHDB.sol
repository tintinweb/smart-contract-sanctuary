...
contract GenericETHDB {
    ...
    function countDataset() public view returns (uint256) {
    ...
    function isValidNode(address nodeAddress) public view returns (bool) {
        for (uint256 i = 0; i < validNodes.length; i++) {
            if (
                validNodes[i].nodeAddress == nodeAddress &&
                validNodes[i].isValid
            ) {
                return true;
            }
    ...
    function activateNode(address nodeAddress) public isOwner {
        for (uint256 i = 0; i < validNodes.length; i++) {
            if (
                validNodes[i].nodeAddress == nodeAddress
            ) {
                if (validNodes[i].isValid) {
                    revert("Node is already activated.");
                } else {
                    validNodes[i].isValid = true;
                    return;
                }
            }
        }

        validNodes.push(Node(nodeAddress, true));
    }

    function deactivateNode(address nodeAddress) public isOwner {
        for (uint256 i = 0; i < validNodes.length; i++) {
            if (
                validNodes[i].nodeAddress == nodeAddress
            ) {
                if (!validNodes[i].isValid) {
                    revert("Node is already deactivated.");
                } else {
                    validNodes[i].isValid = false;
                    return;
                }
            }
        }

        revert("Node wasn't registered & activated before.");
    }

    function changeDBName(string calldata newName) public isOwner {
        DBName = newName;
    }
