...
// File: contracts/Funganomics-NFT.sol



contract FunganomicsNFT is ERC721, Ownable{
  ...
  struct Lot {
    ...
    constructor() ERC721("Funganomics", "FUNGNFT")  {
  ...
  function mint(string memory _tokenURI, address _toAddress, uint _price) public returns (uint) {
    ...
    function buyCharacter(uint _id) external payable {
        _validate(_id);
    ...
    // Buy Clan function implemented by DR
    
    function buyClan(uint256[] memory _ids,uint256[] memory  _amounts) external payable {
        
        for (uint i = 0; i < _ids.length; i++) {
          uint _tokenId =totalSupply() + 1;
          price[_tokenId] = _amounts[i];
          require(_amounts.length == _ids.length);
          address payable _previousOwner = payable (ownerOf(_ids[i]));
          address payable _newOwner = payable (msg.sender);
          _validate(_ids[i]);
          _transfer(_previousOwner, _newOwner, _ids[i]);
         _previousOwner.transfer(_amounts[i]);
         emit Purchase(_previousOwner, _newOwner, price[_ids[i]], _ids[i], tokenURI(_ids[i]));
      }
    ...
    function mintBatch(address _toAddress, uint256 numberofNFT,string calldata _tokenURI,uint256[] memory  _price) public   {
      ...
      for (uint i = 0; i < numberofNFT; i++) {
        ...
        _safeMint(_toAddress, _tokenId);
        _setTokenURI(_tokenId, _tokenURI);
        emit Minted(_toAddress, _price[i], _tokenId, _tokenURI);
      }
    ...
    function _validate(uint _id) internal {
        ...
        require (!_lot.active, 'An Auction is ongoing on this Token you cannot buy this NFT');
