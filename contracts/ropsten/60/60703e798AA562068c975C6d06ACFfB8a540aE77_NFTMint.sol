...
abstract contract Sale is ERC721, Ownable{
    ...
    // function acceptBId(address token,address admin,address from, uint256 amount,uint256 tokenId) public{
    ...
    //     //creator
    //     amount = amount - fee;
    //     uint256 roy = amount.mul(_royal[tokenId]).div(100);
    ...
    // }
      function acceptBId(address token,address admin,address from, uint256 amount) public{
        //require(checkTokenApproval(admin, tokenId));
        ERC20 t = ERC20(token);
        //admin
        //uint256 fee = amount.mul(5).div(100);
        t.transferFrom(from,admin,amount);
        //creator
        // amount = amount - fee;
        // uint256 roy = amount.mul(_royal[tokenId]).div(100);
        // address creator = _creator[tokenId];
    ...
    function saleToken(address payable admin,uint256 tokenId) public payable{
        ...
        //service fees
        admin.transfer(fee);
        uint256 price= order_place[owner][tokenId].price;
