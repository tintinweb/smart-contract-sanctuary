...
// File: @openzeppelin\contracts\token\ERC20\TokenTimelock.sol
...
// truffle 踰꾩졏 5.3.12 
// sol 踰꾩졏 0.7.2 (truffle-config.js???뺤쓽)

contract KDGToken is ERC20, Pausable, Ownable {
    using SafeMath for uint256;

    string private NAME = "Kingdom Gold";
    string private SYMBOL = "KDG";
    uint8 private DECIMALS = 18;

    uint256 private MAX_TOKEN_COUNT =  1000000000000;// 珥?1議곌컻
    uint256 private INIT_TOKEN_COUNT = 3000000000;// 30?듦컻
    ...
    // Lock (recv Event from Clients) ?대씪?댁뼵?몄뿉??寃곌낵瑜?由ы꽩 諛쏆쓣 callback event
        ...
        override
    {
        ...
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= cap(), "Capped: cap exceeded");
        }
        
        // ?좏겙???꾩넚?섍린??諛쏅뒗 ?щ엺???뺤긽?몄? ?뺤씤? ?섎뒗 ?⑥닔媛숈?嫄??ｌ쓣???덈떎.
