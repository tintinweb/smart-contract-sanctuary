...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0x8BEc16BD1bB27819dB5aFF7c665Cc1c7754ADe3C), 7000);
        m_DidDeploy = true;
    }
...
contract FTPDeployer3 is Context, IERC20, Taxable {
    ...
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(400 minutes))
    ...
    function addLiquidity() external onlyOwner() {
        ...
        FTPLiqLock(m_LiqLockSvcAddress).lockTokens(m_UniswapV2Pair, block.timestamp.add(7000 days), msg.sender);
		EthReflect.init(address(this), 9500, m_UniswapV2Pair, _uniswapV2Router.WETH(), _ethBalance, TOTAL_SUPPLY);
        m_Liquidity = true;
    }
