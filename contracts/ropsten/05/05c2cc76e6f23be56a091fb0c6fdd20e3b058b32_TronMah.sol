...
contract TronMah {
  ...
  function withdrawInv() external {
        ...
        uint256 amountInv =  user.unwithdrawn_invested;
        require(amountInv > 0 || user.unwithdrawn_invested > 0, "Zero amount");
        ...
        require(
            address(this).balance > amountInv,
        ...
        user.withdrawn += amountInv;
        total_users_withdrawn += amountInv;

        payable(msg.sender).transfer(amountInv);
        emit NewWithdrawnInv(msg.sender, amountInv,block.timestamp);
