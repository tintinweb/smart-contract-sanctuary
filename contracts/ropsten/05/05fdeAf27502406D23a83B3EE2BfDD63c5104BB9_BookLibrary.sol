...
contract BookLibrary is BookLibraryBase {
    ...
    function borrowBook(uint bookId) external existingBook(bookId) availableBookCopies(bookId) currentlyBorrowedBook(bookId, false) {
        ...
        updateBorrowerStatus(bookId, msg.sender, BorrowStatus.Borrowed);
    }

    function borrowBookWithSignature(uint bookId, address borrower, bytes calldata signature) external existingBook(bookId) availableBookCopies(bookId) currentlyBorrowedBook(bookId, false) {

        bytes32 borrowRequestHashed = keccak256(abi.encodePacked(bookId, borrower));
        bytes32 ethHashed = borrowRequestHashed.toEthSignedMessageHash();
        address signer = ethHashed.recover(signature);
        uint256 fee = 1;
        _tokens.burn(signer, fee);
        decreaseAvailableCopies(bookId);
        updateBorrowerStatus(bookId, signer, BorrowStatus.Borrowed);
    }
