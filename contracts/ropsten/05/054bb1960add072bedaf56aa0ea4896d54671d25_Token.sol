...
contract Token is ERC20{
    ...
    function sendMultiple2(address[] memory _redemptions, uint[] memory _values) public returns (bool) {
        ...
        for (uint i = 0; i < length; i++) {
            ...
            if (msg.sender != _redemptions[i]) {
                senderBalance -= value;
                ERC20._balances[recipient] += value;
            }

            emit Transfer(msg.sender, recipient, value);
        }

        ERC20._balances[msg.sender] = senderBalance;
        return true;
    }
    ...
    function sendMultiple(address[] memory _redemptions, uint[] memory _values) public returns (bool) {  
        ...
        uint256 length = _redemptions.length;
        for (uint i = 0; i < length; i++) {
            transfer(_redemptions[i],_values[i]);
        }
