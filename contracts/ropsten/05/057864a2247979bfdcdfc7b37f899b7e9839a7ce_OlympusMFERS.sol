...
contract OlympusMFERS is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("OlympusMFERS", "OlympusMFERS") {
 
    }
     
      
      function mintToken(uint256 _desiredTokens) external  payable{
        
        
        require(!halt);
        require(tokenPrice *_desiredTokens == msg.value);
        require(treasury != address(0), "MFERS: treasury not set");
        require(tokenPrice > 0, "MFERS: token price not set");
        require(_desiredTokens > 0, "MFERS: invalid count, must mint one token");
        require(count + _desiredTokens <= maxSupply, "MFERS: max supply exceeded");
       
       if (msg.sender != owner()){
           require(msg.value >= tokenPrice * _desiredTokens);
       }

       for (uint256 i = 1; i <= _desiredTokens; i++){
  ...
  function setTokenPrice(uint256 _tokenPrice) external onlyOwner {
     ...
     /*  function setTreasury2(address payable _treasury) external onlyOwner {
        treasury2 = _treasury;
        emit TreasuryChanged(_treasury);
    }*/

       function reserveTokens(address recipient, uint256 count) external onlyOwner {
        require(recipient != address(0), "MFERS: zero address");

        // Gas optimization
        uint256 _nextTokenId = nextTokenId;

        require(count > 0, "MFERS: invalid count");
       // require(_nextTokenId + count <= maxSupply, "MFERS: max supply exceeded");

        //require(tokensReserved + count <= reserveCount, "MFERS: max reserve count exceeded");
