...
contract VotingChallenge {
    ...
    event NewVotesFor(address _participant, uint _candidate, uint _votes, uint _coefficient);  // in
    ...
    function currentCoefficient() public view returns (uint) {  // in 1/1000000
        return 1000000 - 500000 * (now - challengeStarted) / challengeDuration;
    }
    ...
    function voteForCandidate(uint candidate) public payable inVotingPeriod {
        ...
        uint coef = currentCoefficient();
        uint weighted = msg.value * coef / 1000000;
        votes[candidate] += weighted;

        // Change the votes distribution
        userVotesDistribution[msg.sender][candidate] += weighted;

        // Fire the event
        emit NewVotesFor(msg.sender, candidate, msg.value, coef);
    }
    ...
    function voteForCandidateViaProxy(uint candidate, address sender) public payable inVotingPeriod {
        ...
        // Change the votes distribution
        userVotesDistribution[sender][candidate] += weighted;

        // Fire the event
        emit NewVotesFor(sender, candidate, weighted, coef);
    }
    ...
    function sendCryptoVersusReward() public afterVotingPeriod {
        emit CryptoVersusRewardWasPaid(cryptoVersusWallet, address(this).balance);
        selfdestruct(cryptoVersusWallet);
    }
}
