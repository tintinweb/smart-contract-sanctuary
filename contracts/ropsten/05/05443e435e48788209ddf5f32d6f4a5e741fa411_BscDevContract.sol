...
contract BscDevContract is ERC20, Ownable {

    // Events
    event SetMaxWallet(uint256 maxWalletToken);
    event SetFees(uint256 DevFee);
    event SetSwapBackSettings(bool enabled, uint256 Threshold, uint256 SwapSize);
    ...
    // Basic Contract Info
    string constant _name = "Name"; // Add Name -- TO DO
    string constant _symbol = "Symbol"; // Add Symbol -- TO DO
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 1000000000 * (10 ** _decimals); // 1,000,000,000 Tokens
    
    // Max wallet
    uint256 public _maxWalletSize = (_totalSupply * 5) / 100;  // 5% MaxWallet - 50,000,000 Tokens

    // Fee receiver
    uint256 DevFee = 5;
    
    // Fee receiver & Dead Wallet
    address private DevFeeReceiver = 0x84b4030578A3Ba7A0a28deD695b643A86e9C9e5f; // Add Wallet -- TO DO
...
// External Functions
    ...
    function setSwapBackSettings(bool _enabled, uint256 _threshold, uint256 _swapSize) external onlyOwner {
        require(_threshold >= 1, "Can't set SwapThreshold to ZERO");
        swapEnabled = _enabled;
        swapThreshold = _threshold;
        maxSwapSize = _swapSize;
        emit SetSwapBackSettings(swapEnabled, swapThreshold, maxSwapSize);
    }
