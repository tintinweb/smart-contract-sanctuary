...
contract HAT is ERC721, Ownable, Pausable
{
	...
	mapping(uint256 => ticket_data) public Tickets;
	mapping(address => mapping(uint256 => uint256)) public TicketMyByType;
	...
	function Buy(address to, uint8 param_id,uint256 time)public
	{
		...
		TicketMyByType[to][param_id]++;
		require(((Params[param_id].disposable == true && TicketMyByType[to][param_id]<2) || (Params[param_id].disposable == false)),"A one-time token of the same type cannot be bought again.");
		
		IERC20(PayToken).safeTransferFrom(_msgSender(),address(this),amount);

		_safeMint(to, TicketsCount, "");

		if(time > block.timestamp)
		Tickets[TicketsCount].start = time;
		else
		Tickets[TicketsCount].start = block.timestamp;

		Tickets[TicketsCount].end = Tickets[TicketsCount].start + Params[param_id].interval;
	...
	{
		require(false,'Transfer function disable');
	...
	{
		require(false,'Transfer function disable');
		emit transferLog(_msgSender(),to,amount);
	}
}
