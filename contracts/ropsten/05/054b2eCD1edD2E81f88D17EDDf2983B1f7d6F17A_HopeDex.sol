...
contract HopeDexV0 is Initializable, AccessControl{
    ...
    bytes32 constant public PRELAUNCH_ROLE = keccak256("Prelaunch Role");
    bytes32 constant public PRELAUNCH_MANAGER_ROLE = keccak256("Prelaunch admin Role");
    ...
    function addPrelaunchUser(address user) public{
        require(hasRole(PRELAUNCH_MANAGER_ROLE, msg.sender), "Access denied");
        _setupRole(PRELAUNCH_ROLE, user);
    }

    function updateP(address user, uint p) public{
        require(hasRole(PRELAUNCH_MANAGER_ROLE, msg.sender), "Access denied");
        if(p < 1 days){
            p = 1 days;
        }
    ...
    function buyHOPE(uint amountETH, uint256 parentId) external accrueByTime ifRunning{
        require(launched || hasRole(PRELAUNCH_ROLE, msg.sender), "Not launched");
        ...
        if(!launched){
            _revokeRole(PRELAUNCH_ROLE, msg.sender);
        }
    ...
    function init(IHOracle _oracle, IETHDistributor _ETHDis) external initializer() {
        ...
        _setupRole(PRELAUNCH_ROLE, msg.sender);
        _setupRole(PRELAUNCH_MANAGER_ROLE, msg.sender);
