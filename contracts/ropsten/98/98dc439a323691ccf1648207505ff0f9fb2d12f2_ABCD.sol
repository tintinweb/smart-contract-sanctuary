...
contract ABCD is Context, IERC20, Ownable {
    ...
    //// ERC-20 init
    string private constant _name = "v201";
    string private constant _symbol = "v201";//TODO
    uint8 private constant _decimals = 9;
    uint256 private constant _tTotal = 625 * 10**6 * 10**9; //TODO
    address private constant _devAddress = 0x9f64c4F22F6D29E584C7DD151eFb402c237C2eD2;  //TODO
    address public constant _swapRouter = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;  //TODO
    // tokens own
    ...
    uint256 private constant _level1MinReferredToken = 7 * 10**6 * 10**9; //TODO
    uint256 private constant _level2MinReferredToken = 15 * 10**6 * 10**9; //TODO
    uint256 private constant _level3MinReferredToken = 60 * 10**6 * 10**9; //TODO
    bool private referralProgramIsOn = true;
    ...
    //// liquidity pool 
    uint256 private numTokensSellToAddToLiquidity = 5 * 10**6 * 10**9;  //TODO
    // time lock
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!takeFee) {
            removeAllFee();
        } else if(referralProgramIsOn == true && sender == uniswapV2Pair && referrer[recipient] != address(0)) {
        ...
        if(!takeFee) {
            restoreAllFee();
        } else if(referralProgramIsOn == true && sender == uniswapV2Pair && referrer[recipient] != address(0)) { //the sender has a referrer
    ...
    function calculateTaxFee(uint256 _amount, address to) private view returns (uint256) {
        ...
        uint256 dayCount = (block.timestamp - launchDate) / 60 / 60;
        uint256 startRate = 23;
        if (dayCount < 7) {
    ...
    function setReferrer(address account) public {
        require(referralProgramIsOn == true, "Referral program is disabled.");
        require(referrer[_msgSender()] == address(0), "Referrer existing.");
        require(account != _msgSender(), "Referrer cant be same as referree.");
    ...
    function disableReferralProgram() external onlyOwner {
        require(block.timestamp > contractReleaseTime || block.timestamp < contractInitializeDeadline, "Owner can't modify contract until release time");
        referralProgramIsOn = false;
        _referrerFee = 0;
        _referreeFee = 0;
    }
    // update dev fee
    function setDevFeePercent(uint256 newDevFee) external onlyOwner {
        require(referralProgramIsOn == false, "Please turn off referral program first before adjusting dev fee percent.");
        require(block.timestamp > contractReleaseTime || block.timestamp < contractInitializeDeadline, "Owner can't modify contract until release time");
        require(newDevFee < _devFee, "New dev fee must be less than current reflection fee");
        _devFee = newDevFee;
    }
    ...
    function removeLiquidity(uint256 liquidity) external onlyOwner {
        ...
        (amountToken, amountBNB) = uniswapV2Router.removeLiquidityETH(
            ...
            0, 
            0, 
            payable(0x57A02dD9Ac2D58D2323835FAA51022767cA41beB),  //TODO: can be another contract has it's own lock machenism 
            block.timestamp + 360
        );
        // update unlocked liquidity
    ...
    function updateMinTokensBeforeSwap(uint256 _numTokensSellToAddToLiquidity) external onlyOwner {
        numTokensSellToAddToLiquidity = _numTokensSellToAddToLiquidity;
        emit MinTokensBeforeSwapUpdated(_numTokensSellToAddToLiquidity);
    }
