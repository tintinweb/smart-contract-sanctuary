...
contract LowEffortSmileeez is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Pausable, Ownable {
    ...
    mapping(address => uint8) mintersQuantity;

    address proxyRegistryAddress = 0xa5409ec958C83C3f309868babACA7c86DCB077c1;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("Low Effort Smileeez V2", "SMLE2") { } 
    ...
    function myPrice(uint8 qty) public view returns (uint256) {
        ...
        // Discounted when on testnet
        if (getChainId() != 1) {
            networkPrice = 0.001 ether;
        }

        // First mint is free for everyone
        if (mintersQuantity[msg.sender] == 0) {
        ...
        payable
    {
        ...
        require(msg.value >= myPrice(qty), "Insufficient payable value");

        for (uint8 i = 0; i<qty; i++) {
            uint256 newId = _tokenIdCounter.current();
            _safeMint(msg.sender, newId);
            _setTokenURI(newId, string(abi.encodePacked(cids[i])));
            _tokenIdCounter.increment();

            mintersQuantity[msg.sender] = mintersQuantity[msg.sender] + 1;
        }
    }
