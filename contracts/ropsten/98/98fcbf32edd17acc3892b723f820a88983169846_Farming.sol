...
contract Farming is Ownable, ReentrancyGuard {
    ...
    uint256 public reserveDistributionSchedule = 4 days;
    uint256 public lastReserveDistributionTimestamp;
    uint256 public depositPeriod = 48 hours;
    ...
    // x * reserveDistributionSchedule + (0,depositPeriod) => x+1
    // x * reserveDistributionSchedule + (depositPeriod,reserveDistributionSchedule) => x+2
    // x >= pool.deposits.length => pool.deposits.length + 1;
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (farmAmount != 0) {
            ...
            _deposit(_pid, farmAmount, currentRound);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (canPullRewards()) {
            pullRewards();
            if (rewards.length > 0) updatePool(_pid);
        }

        uint256 pendingRewards = pendingMoney(_pid, msg.sender);
        uint256 currentRound = getCurrentRoundId(_pid);
        uint256 withdrawFor = pool.deposits.length - 1;
        ...
        if (_amount > 0) {
            ...
            _deposit(_pid, user.amount, currentRound);
    ...
    function _deposit(
        ...
        if (pool.deposits.length == 0) {
            pool.deposits.push(_farmAmount);
        } else if (_currentRound > pool.deposits.length) {
            pool.deposits.push(
                _farmAmount.add(pool.deposits[pool.deposits.length.sub(1)])
            );
        } else {
            pool.deposits[_currentRound.sub(1)] = pool
                .deposits[_currentRound.sub(1)]
                .add(_farmAmount);
        }
    }

    function canPullRewards() internal view returns (bool) {
        if (
            lastReserveDistributionTimestamp.add(reserveDistributionSchedule) >
            block.timestamp
        ) return false;

        if (IReserve(reserve).canWithdraw()) return true;
        return false;
    }
