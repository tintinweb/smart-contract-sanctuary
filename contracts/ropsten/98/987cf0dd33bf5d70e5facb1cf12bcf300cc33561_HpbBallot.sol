...
contract HpbBallot is Ownable ,AdminInterface,VoteInterface,FetchVoteInterface {
    ...
    event UpdateCandidateAddr(
        address indexed _oldCandidateAddr,
        address indexed _newCandidateAddr
    );

    event SetRewardAddr(
        address indexed _candidateAddr,
        address indexed _rewardAddr
    );
    // 投票
    ...
    modifier onlyApproved(
        ...
        require(isApproved(addr, msg.sender));
        _;
    }
    ...
    function isApproved(
        ...
        if (addr == operator) {
            return true;
        } else if (_useCustomOperator[addr]) {
    ...
    constructor () payable public {
        ...
        // 设置第一位置
        // Set the first position.
        candidateArray.push(Candidate(msg.sender, msg.sender, 0, 0, new address[](0)));
    }
    ...
    function addCandidate(
        ...
        require(isRunUpStage()); // 必须是竞选准备阶段
        uint index =candidateIndexMap[_candidateAddr];
        // 必须候选人地址还未使用
        require(index == 0);
        // 添加候选人
        index = candidateArray.push(Candidate(_candidateAddr,_rewardAddr, _name, 0, new address[](0)))-1;
    ...
    function updateCandidateAddr(
        address _oldCandidateAddr, 
        address _newCandidateAddr
    ) onlyApproved(_oldCandidateAddr) public {
    ...
    function setRewardAddr(
        address _candidateAddr, 
        address _rewardAddr
    ) onlyApproved(_candidateAddr) public {
        uint index =candidateIndexMap[_candidateAddr];
        require(index != 0);
        candidateArray[index].rewardAddr = _rewardAddr;
        emit SetRewardAddr(_candidateAddr, _rewardAddr);
    }
    ...
    function fetchAllVoteResult() public view returns (
        ...
        address[] memory _candidateAddrs=new address[](vcl);
        address[] memory _rewardAddrs=new address[](vcl);
