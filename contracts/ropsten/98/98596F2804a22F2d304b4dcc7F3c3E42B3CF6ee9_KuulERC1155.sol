...
contract KuulERC1155 is ERC1155, Ownable {
  ...
  function buySingleToken(uint256 id, uint256 amount) public {
    ...
    if(!_info.tokenType) {
      _info.payToken.transfer(_info.owner, _price);
    } else {
      uint256 _realPrice = _price.mul(9).div(10);
      _info.payToken.transfer(_info.owner, _realPrice);
      for(uint256 i = 0; i < _info.subTokenIds.length; i++) {
        ...
        _info.payToken.transfer(_subInfo.owner, _royaltyPrice);
      }
    }
