...
contract Splitter is Killable {
    ...
    constructor(
        ...
        Payee memory owner = Payee(ownerAddress, ownerShare);
        payees.push(owner);
    ...
    function split(uint256 amount) internal {
        ...
        for (uint256 i = 0; i < payees.length; i++) {
            ...
            payee.transfer(balance.div(fullPercentage).mul(payees[i].share)); // transfer percentage share
        }
        if(address(this).balance > 0){
            address payable owner = payees[0].payeeAddress;
