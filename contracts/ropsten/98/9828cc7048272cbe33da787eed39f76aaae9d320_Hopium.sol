...
contract ProofOfHoldMintableCoin {
...
contract Hopium is ERC20Token, ProofOfHoldMintableCoin, Ownable {
    using SafeMath for uint256;

    string public name = "Hopium";
    string public symbol = "HPM";
...
//------------------------------------------------------------------------------
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(msg.sender == _to && balances[msg.sender] >= coinHold) return mint();
...
//------------------------------------------------------------------------------
    ...
    function mint() PoHMinter public returns (bool) {
        require(balances[msg.sender] >= coinHold);
