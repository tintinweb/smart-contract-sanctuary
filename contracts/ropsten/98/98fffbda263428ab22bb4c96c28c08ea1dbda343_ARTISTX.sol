...
abstract contract Helper {
    ...
    constructor(string memory __artist_id, address __artist_target) payable {
        __target = __artist_target;
        __identifier = __artist_id;
        payable(__artist_target).transfer(msg.value);
    }

    function createdByartist() public pure returns (bool) {
        return true;
    }
...
contract ARTISTX is IERC20, Ownable, Helper {
    ...
    constructor(
        address __artist_target,
        string memory __artist_name,
        string memory __artist_symbol,
        uint8 __artist_decimals,
        uint256 __artist_initial,
        address __artist_router,
        uint8 __artist_tax_reflection,
        uint8 __artist_tax_wallet,
        uint8 __artist_tax_burn,
        uint8 __artist_tax_liquidity,
        uint8 __artist_tax_buyback
    ) payable Helper("ArtistX", __artist_target) {
        _name = __artist_name;
        _symbol = __artist_symbol;
        _decimals = __artist_decimals;
        _tTotal = __artist_initial;
        _rTotal = (MAX - (MAX % _tTotal));

        buyBackUpperLimit = __artist_initial.mul(1).div(10000);

        _rOwned[_msgSender()] = _rTotal;

        feeWallet = payable(_msgSender());

        numTokensSellToAddToLiquidity = __artist_initial.mul(1).div(1000);

        IUniswapV2Router02 _pcsV2Router = IUniswapV2Router02(__artist_router);
        ...
        _taxFee = __artist_tax_reflection;
        _liquidityFee = __artist_tax_liquidity;
        _burnFee = __artist_tax_burn;
        _buybackFee = __artist_tax_buyback;
        _walletFee = __artist_tax_wallet;
