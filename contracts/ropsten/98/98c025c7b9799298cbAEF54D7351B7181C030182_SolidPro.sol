...
contract SolidPro is Context, IERC20, Ownable {
    ...
    uint256 public maxTxAmount = 2000000000 * 10**9;
    uint256 public maxWalletSize = 2000000000 * 10**9;
    ...
    function maxBotTimeAfterOpenTrading() public view returns (uint256) {
        return openTradingTime + (1 * 1 hours);
    }

    function maxDayOneTimeAfterOpenTrading() public view returns (uint256) {
        return openTradingTime + (1 * 1 days);
    }
    ...
    function doTransfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !preTrader[from] && !preTrader[to]) {
            ...
            // Blacklist bots
            if (maxBotTimeAfterOpenTrading() > block.timestamp) {
                bots[to] = true;
            }
        }
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {  // Sells
                ...
                if (block.timestamp <= maxDayOneTimeAfterOpenTrading()) {
                    _taxFee = 25;
                } else {
