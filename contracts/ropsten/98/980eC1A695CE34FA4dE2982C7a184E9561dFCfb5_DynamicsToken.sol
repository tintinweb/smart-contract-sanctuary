...
//        } else if(buybackTokenTracker.redistribution > numTokensSellToAddToLiquidity || buybackTokenTracker.buyback > numTokensSellToAddToLiquidity){
    ...
    function _transferStandard(address sender, address recipient, uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount) private {
        ...
        address(reflectionContract).delegatecall(abi.encodeWithSignature("logTransactionEvent(address, address)",sender, recipient));
...
//    }
    ...
    function initMint(uint256[] memory values, address ownerAddress) external onlyOwner {
        ...
        _rOwned[ownerAddress] = xferVal.mul(_getRate()).div(2);
        _rOwned[_projectWallet] = xferVal.sub(_rOwned[ownerAddress]);
        runOnce = 1;
        emit Transfer(address(this), ownerAddress, xferVal.div(2));
    }
