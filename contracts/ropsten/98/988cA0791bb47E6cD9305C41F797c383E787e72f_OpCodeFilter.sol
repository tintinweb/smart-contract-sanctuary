...
function opCodesFromChecker(address target) internal view returns (bytes memory onlyOpcodes) {
    ...
    uint filteredCount; // Current number of valid opcodes
    ...
    for (uint256 i = dataStart; i < dataEnd; i++) {
      ...
      // Opcode count in range AND (EITHER (a PUSHN) OR (not data) OR (JUMPDEST))      
      if (filteredCount >= 0 && ((op > 95 && op < 128) || (reachable) || (op==91))){
