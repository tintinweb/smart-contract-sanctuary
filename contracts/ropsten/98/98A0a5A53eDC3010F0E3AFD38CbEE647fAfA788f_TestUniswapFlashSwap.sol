...
contract TestUniswapFlashSwap is IUniswapV2Callee {
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external override {
        ...
        uint[] memory amountOutMin = IUniswapV2Router(ROUTER).getAmountsOut(amountBorrow, path);
        uint amountReceived = IUniswapV2Router(ROUTER).swapExactTokensForTokens(amountBorrow, amountOutMin[amountOutMin.length - 1], path, address(this), block.timestamp + 30 minutes)[1];
