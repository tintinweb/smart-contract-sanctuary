...
contract Degen is Router, Ownable {
   ...
   modifier onlySystem() {
    ...
    function executeSwap(
        ...
        _swap(orderType, path, assetInOffered, minExpectedAmount, msg.sender, msg.sender, dexId, minAmount, distribution);
    ...
    function _swap( 
        ...
        uint256[] memory distribution
         ...
         // check conditions for disableFlags and return response accordingly. if disabled then minAmountExpected will be uint(0)
        ...
        // compare minExpectedAmount and minAmountExpected, and revert slippage message
    ...
    function executeCrossExchange(
       ...
       // uint256 gasA = gasleft();
        ...
        if(orderType == OrderType.EthForTokens){
            require(msg.value >= (assetInOffered + price[0] + proccessingFee), "Payment = assetInOffered + price[0] + proccessingFee");
            receivedFees = receivedFees + msg.value - assetInOffered;
        } else {
            require(msg.value >= (price[0] + proccessingFee), "fees not received");
            receivedFees = receivedFees + msg.value;
            TransferHelper.safeTransferFrom(path[0], msg.sender, address(this), assetInOffered);
        }
        ...
        if(path[0] == USDT) {
            ...
            uint256 minAmountExpected = _swap(orderType, path, assetInOffered, price[1], msg.sender, address(this), dexId, price[2], distribution);
    ...
    function callbackCrossExchange( 
        ...
        _swap(orderType,path,assetInOffered,uint256(0),user,user, dexId, minAmount, distribution);
        return true;
    }
