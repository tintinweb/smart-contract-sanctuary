...
contract rLoan is ReentrancyGuard{
    ...
    function requestLoan(uint256[N_COINS] calldata amounts) external returns(uint256 loanID) {
      ...
      function approveLoan(uint _loanID) external onlyWallet() {
        uint256[N_COINS] memory royaleBalance=royale.getTotalPoolBalance();
        for(uint8 i=0;i<N_COINS;i++) {
            require(totalApprovedLoan[i].add(transactions[_loanID].tokenAmounts[i]).sub(totalLoanTaken[i])<royaleBalance[i],"Can not approve that much amount");
        }
