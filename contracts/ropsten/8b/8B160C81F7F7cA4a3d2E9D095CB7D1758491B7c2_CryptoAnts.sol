...
contract CryptoAnts is ERC721, ICryptoAnts, Ownable {
  ...
  uint256 public hatchCooldown = 1 minutes;
  uint256[] public allAntsIds;
  mapping(uint256 => address) public antToOwner;
  mapping(uint256 => uint32) hatchReadyTimes;
  ...
  function hatchEggs(uint256 _antId) external lock(_antId) {
    require(antToOwner[_antId] == msg.sender, 'Unauthorized');
    uint256 random = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, randNonce))) % 100;
    randNonce++;
  ...
  function notLocked(uint256 _antId) external view override returns (bool) {
    return hatchReadyTimes[_antId] <= block.timestamp;
  }
  ...
  modifier lock(uint256 _antId) {
    ...
    //locked = true;
    require(hatchReadyTimes[_antId] <= block.timestamp, 'Sorry, you are not allowed to re-enter here :)');
    _;
    //locked = notLocked;
    hatchReadyTimes[_antId] = uint32(block.timestamp + hatchCooldown);
  }
}
