...
contract MyLotto is IERC20 {
    ...
    */
    function purchaseTicket(uint256 _ticketNumbers, uint256 _ticketCount) public payable returns(bool){
        ...
        //     _ticketNumbers.length == 36 || _ticketNumbers.length == 42 || _ticketNumbers.length == 48 || _ticketNumbers.length == 54 || _ticketNumbers.length == 60) , "Invalid Value");
            require(msg.value >= _ticketCount.mul(_ticketPrice), "Insufficient eth value");
        // if(_ticketNumbers.length > 1){
            if((saleStartTimeStamp[saleIdNow] + elapsedTime) > now){
              ...
              for (uint256 i = 0; i < _ticketCount.mul(uint256(6)); i++){
                  
                ticketNumberByAddress[saleIdNow][msg.sender].push(_ticketNumbers%100);
                _ticketNumbers = _ticketNumbers.div(100);
              }
               if(_ticketNumbers == 60){
