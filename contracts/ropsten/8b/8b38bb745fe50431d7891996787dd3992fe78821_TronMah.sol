...
contract TronMah {
    ...
    uint256 constant min_user_deposit = 0.0001 ether;
    ...
    function deposit(uint16 _days, address _referrer) external payable {
        ...
        user.unwithdrawn_invested += msg.value; // new
        ...
        payable(commission_main_wallet).transfer(commissionAmount);
  ...
  function withdrawInv() external {
        ...
        uint256 amount =  user.unwithdrawn_invested;
        require(amount > 0 || user.unwithdrawn_invested > 0, "Zero amount");
        ...
        user.bonused += user.unwithdrawn_invested;
        user.unwithdrawn_invested = 0;
