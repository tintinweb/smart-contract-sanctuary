...
contract Arena is IArena, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
    ...
    ArenaInfo KombatIsland = ArenaInfo("Kombat Island", 8, 4, 6, 2, 2);
    ArenaInfo hideAndSeek = ArenaInfo("Hide and Seek", 2, 4, 6, 9, 8);
    ArenaInfo captureTheFlag = ArenaInfo("Capture the Flag", 6, 4, 8, 3, 6);
    ArenaInfo freezeDance = ArenaInfo("Freeze Dance", 2, 1, 6, 9, 8);
    ...
    function gettestArray() external view onlyOwner returns (uint256[] memory) {
        return testArray;
    }

    function gettestArrayObj() external view onlyOwner returns (uint256) {
        return testArray[1];
    }

    function gettestArray2Obj() external view onlyOwner returns (uint256) {
        return testArray2[1];
    }

    function gettestArray2()
        external
        view
        onlyOwner
...
interface IToken is IERC721Enumerable {
    ...
    function playerMint(
        ...
        uint256 wisdom,
        uint256 intelligence,
        uint256 endurance) external payable;
