...
contract StakingFactory is AccessControl, ReentrancyGuard, IStakingFactory {
    ...
    event NewStakingDeploy(address newStaking, address stakedToken);

    event NewRewardAdded(uint256 total, address token);
    ...
    function deposit(address token, uint256 amount) external {
        ...
        address sender = _msgSender();
        emit NewRewardAdded(amount, pools[token]);
    ...
    function createStaking(address token) public onlyManagerOrAdmin {
        ...
        emit NewStakingDeploy(newStaking, token);
    }

    function inputReward(uint256 amount, Reward[] calldata rewards) external onlyManagerOrAdmin {
        require(amount != 0, "Not valid");
        uint256 totalPercent;
        uint256 totalReward;
        for (uint256 i = 0; i < rewards.length; i++) {
            totalPercent += rewards[i].percent;
            totalReward += (amount * rewards[i].percent) / 10000;
            IStaking(rewards[i].token).addReward((amount * rewards[i].percent) / 10000);
        }
        require(totalPercent == 10000, "Invalid distribution");
        require(amount - totalReward < 1000000000, "Invalid sum");
        AssetLib.safeTransferFrom(YDR_TOKEN, _msgSender(), amount);
    }
    ...
    function claimReward() external {
        ...
        uint256 total;
        for (uint256 i = 0; i < users[sender].length; i++) {
            total += IStaking(users[sender][i]).claimDividends(sender);
        }
    ...
    function getAvailToClaim() external view returns (uint256) {
        address sender = _msgSender();
        uint256 total;
        for (uint256 i = 0; i < users[sender].length; i++) {
            total += IStaking(users[sender][i]).availDividends(sender);
        }
        return total;
    }
