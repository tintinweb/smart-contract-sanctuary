...
contract Vault is Constant, Configurable {
    ...
    IERC20 public aETHb;
    uint public totalAETHbEthValue;
    ...
    function __Vault_init_unchained(address _oneMinter, ONE _one, ONS _ons, address _onb, IAETH _aETHc, address _WETH, uint _begin, uint _span) public governance {
    ...
    function resetParameters(IAETH _aETHb) public governance {
        config[_burnOneThreshold_]  = 1.02 ether;
        aETHb = _aETHb;
    }
    ...
    function mintONEaETHc(address from, uint amt) public {
        receiveAETHcFrom(from, amt);
        receiveOnsInPercentage(from, amt);
        one.mint_(from, amt);
    }

    function mintONEaETHb(address from, uint amt) public {
    ...
    function interestsAETHc() public view returns (uint) {
