...
abstract contract CreditsStakeContract is BasicStakeContract, Ownable {
     ...
     */
    function creditBalanceOf(address _user) public view returns (uint256) {
     ...
     */
    function spendCredits(address _user, uint256 _amount) public onlyOwner {
        require(creditBalances[_user] >= _amount, "Insufficient balance");

        creditBalances[_user] = creditBalances[_user] - _amount;
    }
    ...
    function stake(uint256 _amount, bytes memory _data) public override {
        super.stake(_amount, _data);

        updateCreditBalance(msg.sender, _amount, defaultLockInDuration);
    }
    ...
    function stakeFor(
        ...
        updateCreditBalance(_user, _amount, defaultLockInDuration);
    }
    ...
    function stakeForDuration(
        ...
        super.createStake(_user, _amount, _lockInDuration, _data);

        updateCreditBalance(_user, _amount, _lockInDuration);
    }
    ...
    function updateCreditBalance(
        ...
        require(_amount >= divisor, "Insufficient amount");
        ...
        uint256 creditsAwarded = (_amount * rewardMultiplier) / divisor;
        creditBalances[_user] = creditBalances[_user] + creditsAwarded;
    }
}
...
import "../ERC900/CreditsStakeContract.sol";

/**
 * @title CreditsStakeContractMock
 */
contract CreditsStakeContractMock is CreditsStakeContract {
    /**
     * @dev Constructor function
     ...
     */
    constructor(ERC20 _stakingToken, uint256 _lockInDuration)
