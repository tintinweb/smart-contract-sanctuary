...
contract DirtyFarm is Ownable, Authorizable, ReentrancyGuard {
    ...
    function updateVariablePoolReward() private {
        ...
        uint256 stakelpratio;

        uint256 ratio;
        uint256 multiplier;
        uint256 rewardSegment = poolReward / 2;

        if (runningTotal0 >= maxLPStake) {
            lpratio = runningTotal0 / maxLPStake;
        } else {
            lpratio = maxLPStake / maxLPStake;
        }

        if (runningTotal1 >= maxDirtyStake) {
             stakelpratio = runningTotal1 / maxDirtyStake; 
             stakelpratio = stakelpratio / maxLPStake;
        } else {
            stakelpratio = maxDirtyStake / maxDirtyStake;
        }   

        multiplier = lpratio + stakelpratio;
        
        poolReward = rewardSegment * multiplier;

        if (stakelpratio == lpratio) { //ratio of pool rewards should remain the same (65 lp, 35 stake)
            set(0, 65, true);
            set(1, 35, true);
        }

        if (stakelpratio > lpratio) {
            ratio = stakelpratio / lpratio;
