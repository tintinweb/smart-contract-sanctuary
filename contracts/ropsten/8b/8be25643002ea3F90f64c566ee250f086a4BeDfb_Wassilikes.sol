...
contract Wassilikes is Ownable, ERC721URIStorage {
    ...
    ) public ERC721("Wassilikes v0.1.2", "WASSI") {
        require(_royaltyRecipients[0] != address(0), "Invalid address");  
  
        _prefixURI = initialBaseURI;
    ...
    // When we switch to production testing we use the above to override original tokenURI
    // That way we can provide json output for opensea to use
    //function _tokenURI(uint256 tokenId) public view returns (string memory) { // preserving this
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(_prefixURI, tokenId.toString(), ".json"));
    }
