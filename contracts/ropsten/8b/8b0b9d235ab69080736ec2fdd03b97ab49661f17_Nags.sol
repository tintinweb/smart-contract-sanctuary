...
contract Nags is Ownable, ERC721Enumerable, ReentrancyGuard {
  ...
  uint256 private _price = 0.06 ether; // 60000000000000000
  
  string public NagcontractURI;
  
  constructor() ERC721("Nags", "Nag") {}
  ...
  mapping(address => uint256) public freeRollNags;

  uint256 private _lastTokenIdMintedInInitialSet = 10000;

  function numTotalNags() public view virtual returns (uint256) {
    return 10000;
  }

  function freeRollMint() public nonReentrant() {
    require(freeRollNags[msg.sender] > 0, "You don't have any free rolls!");
    uint256 toMint = freeRollNags[msg.sender];
    freeRollNags[msg.sender] = 0;
    uint256 remaining = numTotalNags() - totalSupply();
    if (toMint > remaining) {
      toMint = remaining;
    }
    _mint(toMint);
  ...
  function getNumFreeRollNags(address owner) public view returns (uint256) {
    return freeRollNags[owner];
  }
  ...
  function mint(uint256 _numToMint) public payable nonReentrant() {
    ...
    require(
      totalSupply + _numToMint <= numTotalNags(),
      "There aren't this many Nags left."
    );
    uint256 costForMintingNags = _price * _numToMint;
    require(
      msg.value >= costForMintingNags,
      "Too little sent, please send more eth."
    );
    if (msg.value > costForMintingNags) {
      payable(msg.sender).transfer(msg.value - costForMintingNags);
    }
  ...
  function contractURI() public view returns (string memory){
    return NagcontractURI;
  }


  function getCostForMintingNags(uint256 _numToMint)
    public
    view
    returns (uint256)
  {
    require(
      totalSupply() + _numToMint <= numTotalNags(),
      "There are not this many Nags left."
    );
    require(
      _numToMint <= MAX_MINTABLE_AT_ONCE,
      "You cannot mint that many Nags."
    );
    return _numToMint * _price;  
  }

  function getNagsBelongingToOwner(address _owner)
    external
    view
    returns (uint256[] memory)
  {
    uint256 numNags = balanceOf(_owner);
    if (numNags == 0) {
      return new uint256[](0);
    } else {
      uint256[] memory result = new uint256[](numNags);
      for (uint256 i = 0; i < numNags; i++) {
  ...
  function giveFreeRoll(address receiver, uint256 numRolls) public onlyOwner {
    ...
    require(freeRollNags[receiver] + numRolls < 21, "Cannot exceed 20 unused free rolls!");
    uint256 freeRolls = freeRollNags[receiver];
    freeRollNags[receiver] = freeRolls + numRolls;
