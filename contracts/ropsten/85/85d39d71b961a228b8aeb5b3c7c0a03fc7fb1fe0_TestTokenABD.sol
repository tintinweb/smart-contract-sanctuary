...
contract DeflationaryERC20 is Context, IERC20, Ownable {
    ...
    uint8 public txFeeOwner = 6; // to owner1 (team)
    uint8 public txFeeCollector = 4; // to owner2 (lotto)
    ...
    constructor (string memory __name, string memory __symbol, uint8 __decimals)  {
        ...
        //_isExcludedFromFee[msg.sender] = true; //disabled for testing fees, enable in production for feeless liquidity add
    ...
    function calculateAmountsAfterFee(
		...
		uint256 feeOwner = amount.mul(txFeeOwner).div(100); //6%
		uint256 feeCollector = amount.mul(txFeeCollector).div(100); //4%
        ...
        if (sender.isContract() || recipient.isContract()) {
			return (amount.sub(fee).sub(feeOwner).sub(feeCollector), fee.sub(feeBurn),feeOwner.add(feeCollector),feeBurn,false);
        } else { // p2p 
    ...
    function swapBufferTokens() private {
        ...
        uint256 half = contractETHBalance.div(txFeeOwner+txFeeCollector).mul(txFeeOwner);
        uint256 otherHalf = contractETHBalance.sub(half);
		payable(owner() == address(0)? feeCollector():owner()).transfer(half);
		payable(feeCollector()).transfer(otherHalf);
    ...
    function setTxOwnerFeePercent(uint8 _txFeeOwner) external onlyOwner() {
		require(_txFeeOwner < 10,'txFeeOwner above 10%');
        txFeeOwner = _txFeeOwner;
    }
    function setTxCollectorFeePercent(uint8 _txFeeCollector) external onlyCollector() {
		require(_txFeeCollector < 10,'txFeeCollector above 10%');
        txFeeCollector = _txFeeCollector;
    }
...
contract TestTokenABD is DeflationaryERC20 {
    constructor()  DeflationaryERC20("TestTokenABD", "ABD", 6) {
