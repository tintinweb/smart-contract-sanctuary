...
contract Erc20Vault is Withdrawable, IERC777Recipient {
    ...
    function pegIn(
        ...
        string memory _tokenSymbol, //tokensymbol raw uint64 instead?
        uint8 _tokenDecimals,
        uint64 _poolID,
        uint8 _fromChainID,
        uint8 _toChainID
    ...
    function pegInEth(string memory _destinationAddress, 
        string memory _tokenSymbol, //tokensymbol raw uint64 instead?
        uint8 _tokenDecimals,
        uint64 _poolID,
        uint8 _fromChainID,
    ...
    function claim( bytes calldata sigData,  bytes[] calldata signatures, address tokenAddress)
        external{
            (address toAddress,uint64 quantity) = claimAction.claim(sigData, signatures);
            pegOut(payable(toAddress), tokenAddress, quantity);
        }
}
...
// Claim
// ----------------------------------------------------------------------------
contract claimToken is Verify, Owned, Oracled {
    using SafeMath for uint256;
    ...
    function verifySigData(bytes memory sigData) private returns (TeleportData memory) {
        ...
        requiredSymbolRaw = uint64(bytes8(stringToBytes32(claimToken.symbol)));
            ...
            uint64 quantity)
    {
        ...
        return (td.toAddress, td.quantity);
    }
