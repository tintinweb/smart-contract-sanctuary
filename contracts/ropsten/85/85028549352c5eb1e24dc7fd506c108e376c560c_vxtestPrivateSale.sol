...
contract vxtestPrivateSale {
    ...
    address public beneficiary = address(0x6C4764b1fA4B7AC3F8B49afb5F472DaE57e364EE); // Send ETH to the address after the sale
    uint public fundingEthGoal = 10 * 1 ether; // Goal funding ethereum amount
    uint public pricePerEther = 60000;         // Price of token per ethereum
    
    uint public minimumEthAmt = 1 ether; // Minimum deposit amount
    uint public raisedEthAmt;            // Funded ethereum amout
    uint public totalSoldTokenCount;     // Sold total token count
    
    Token public tokenReward;            // VectorX address
	WhiteList public whiteListMge;       // VectorX whitelist contract address
	
    bool public enableWhiteList = true;  // check whitelist flag
    bool public saleProceeding = false;  // Whether ico is in progress
    ...
    event StartICO(address owner, bool isStart);               // Sale start event
	event CloseICO(address recipient, uint totalAmountRaised); // Sale close event
    event ReturnExcessAmount(address funder, uint amount);     // When the goal investment amount exceeds the investment re-transfer event
    event RaisedWithdrawal(address beneficiary, uint balance, bool isGoal); // Send ETH to the beneficiary address event
    /**
    ...
    function () public payable {
        ...
        require(msg.value >= minimumEthAmt); // Minimum deposit amount
        if (enableWhiteList) {
        ...
        if (tokenReward.transferSoldToken(address(this), msg.sender, tokenCount)) {
            ...
            if (returnAmt > 0) {
                ...
                saleProceeding = false; // ICO close
     ...
     *
     * Checks if the goal or time limit has been reached and ends the sale
     */
