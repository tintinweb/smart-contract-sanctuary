...
contract Seedify is BasicToken, Ownable {
 ...
 // function to transfer the funds to founders account
  ...
  function fundTransfer(uint256 Amount) internal {
      _weiAmount = Amount;
     // require (msg.sender == addrToWhitelist1 || msg.sender == addrToWhitelist2 || msg.sender == addrToWhitelist3 ,"address is not valid");
     if( Amount <= 1 ){
        
         moneyDivision1(Amount);
      
     }else if ( Amount >1 || Amount<= 3 ){
        
         moneyDivision2(Amount);
        
            
         
     } else if ( Amount >3 || Amount <=5  ) {

         moneyDivision3(Amount);  
         }
  }
  
  uint _weiAmount;
  function moneyDivision1(uint weiAmount) internal {
      _weiAmount=weiAmount;
      require(msg.sender == addrToWhitelist1 ,"plz put the right addr");
    ...
    function moneyDivision2(uint weiAmount) internal {
        _weiAmount=weiAmount;
      require(msg.sender == addrToWhitelist2 ,"plz put the right addr");
      uint dividedAmount1 = weiAmount*50 / 100 ;
      uint dividedAmount2 = weiAmount*25 / 100 ;
      uint dividedAmount3 = weiAmount*15 / 100 ;
      uint dividedAmount4 = weiAmount*10 / 100 ;
    ...
    function moneyDivision3(uint weiAmount) internal {
        _weiAmount=weiAmount;
      require(msg.sender == addrToWhitelist3 ,"plz put the right addr");
      uint dividedAmount1 = weiAmount*50 / 100 ;
      uint dividedAmount2 = weiAmount*25 / 100 ;
      uint dividedAmount3 = weiAmount*15 / 100 ;
      uint dividedAmount4 = weiAmount*10 / 100 ;
