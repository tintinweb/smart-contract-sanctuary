...
contract MALAYJEETPASS is ERC721Enumerable, Ownable {
    ...
    uint256 private WhitelistPRICE = 0 ether;
    uint256 private auctionStartPrice = 0.0 ether;
    uint256 private auctionEndPrice = 0.0 ether;
    uint256 private auctionDuration = 14400;
    uint256 private auctionStartTime;


    struct MalayJeetPass{
        uint8 tier;
        uint8 character;
    }

    mapping(uint256 => uint256) mintBlock;
    mapping(uint256 => MalayJeetPass) public tokenTraits;
    mapping (address => bool) claimed;

    bool publicSaleMode = false;
    ...
    constructor() ERC721("MalayJeetPass", "MalayJeetPass") {
    ...
    require(publicSaleMode, "Public Sale not yet enabled");
    ...
    require(whitelistMode, "Whitelist minting is disabled");
    ...
    function generate(uint256 tokenId, uint256 seed) internal returns (MalayJeetPass memory t) {
        t = selectTraits(seed);
    ...
    function selectTier(uint256 seed) internal view returns (uint8){
        uint256 tiercalc = (seed & 0xFFFF) % 100;
        if(tiercalc == 0 && tier1 < 50){return 0;} else if(tiercalc <= 4 && tier2 < 500){return 1;} else {
	    return 2;
	    }
    }

    function selectTraits(uint256 seed) internal view returns (MalayJeetPass memory t){
        t.tier = selectTier(seed);
        seed >>= 16;
        t.character = selectElement(uint16(seed & 0xFFFF));
    }

    function structToHash(MalayJeetPass memory g) internal pure returns (uint256) {
	return uint256(bytes32(abi.encodePacked(g.tier, g.character)));
    }

    function random(uint256 seed) internal view returns (uint256) {
    return uint256(keccak256(abi.encodePacked(tx.origin,blockhash(block.number - 3),block.timestamp,seed, blockhash(block.number - 2))));
    }

    function getTokenTraits(uint256 tokenId) external view  returns (MalayJeetPass memory)
    ...
    function numTiersMinted() external view  returns (uint8 Tier1,uint8 Tier2,uint8 Tier3){
    ...
    function upgradeTier3to2(uint256 upgradeTokenID,uint256 burnTokenID) external {
        require(tier2 < 500);
        ...
        tier3 = tier3-2;
        tier2++;
        _transfer(msg.sender, address(0xDead), burnTokenID);
        tokenTraits[upgradeTokenID].tier = 1;
  }


    function upgradeTier2to1(uint256 upgradeTokenID,uint256 burnTokenID1,uint256 burnTokenID2) external payable{
        require(tier1 < 50);
        require(msg.value >= 0.5 ether);
        require(upgradeTokenID != burnTokenID1 && upgradeTokenID != burnTokenID2 && burnTokenID1 != burnTokenID2 );    
        require(msg.sender == ownerOf(upgradeTokenID) && msg.sender == ownerOf(burnTokenID1) && msg.sender == ownerOf(burnTokenID2));
        require(tokenTraits[upgradeTokenID].tier == 1 && tokenTraits[burnTokenID1].tier == 1 && tokenTraits[burnTokenID2].tier == 1);
        tier2 = tier2-3;
        tier1++;
        _transfer(msg.sender, address(0xDead), burnTokenID1);
        _transfer(msg.sender, address(0xDead), burnTokenID2);
