...
contract EnglishAuction is IAuction {
  ...
  function cancel() public override onlyOwner {
    require(highestBindingBid < reserve);
  ...
  function placeBid() public payable override notOwner correctTime {
    ...
    require(msg.value > highestBindingBid + bidIncrement, "Bid not high enough.");
    
    // repay previous bidder
    highestBidder.transfer(highestBindingBid);
    
    // adjust bids and variables as needed above
    highestBidder = payable(msg.sender);
    highestBindingBid = msg.value;
  }
  
  function finalise() public override {
    require(block.number > endBlock, "Too soon to end auction.");
    
    if(highestBindingBid >= reserve) {
...
contract DutchAuction is IAuction {
  ...
  function getCurrentPrice() external correctTime returns(uint) {
  ...
  function placeBid() public payable override notOwner correctTime {
    ...
    require(
      ...
      "Bid exceeds current price by over 0.05 ETH."
    );
