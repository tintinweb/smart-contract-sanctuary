...
contract MoonLenderTest is ERC20, Ownable {
    ...
    uint256 private _disallowedTimeLimit = 86400;
    ...
    event DisallowChange(address indexed holder, bool indexed status);
    ...
    function mintToAddress(uint256 amountToMint, address addressToSendTokens) public onlyOwner {
        ...
        require(!isTradingEnabled, "MoonLenderTest: Cannot mint while trading is enabled");
    ...
    function disallowAccount(address account) public onlyOwner {
        uint256 currentTimestamp = _getNow();
        require(!_isDisallowed[account], "MoonLenderTest: Account is already disallowed");
        require(currentTimestamp.sub(_launchStartTimestamp) < _disallowedTimeLimit, "MoonLenderTest: Time to disallow accounts has expired");
        _isDisallowed[account] = true;
        emit DisallowChange(account, true);
    }
    function allowAccount(address account) public onlyOwner {
        require(_isDisallowed[account], "MoonLenderTest: Account is already allowed");
        _isDisallowed[account] = false;
        emit DisallowChange(account, false);
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            require(!_isDisallowed[to], "MoonLenderTest: Account is not allowed to trade");
            require(!_isDisallowed[from], "MoonLenderTest: Account is not allowed to trade");
    ...
    // returns an equivalent amount of an asset given the amount of an other asset and pair reserves 
