...
contract YsecPresale is Ownable, ReentrancyGuard{
    ...
    address private constant _uniswapFactoryAddress = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f;
    ...
    function CreatePresale(PresaleSettings memory settings) external returns(uint256 presaleId){
        ...
        require(settings.StartDate > now, "Start in future");
    ...
    function Contribute(uint256 presaleId) nonReentrant() public payable{
        ...
        Presales[presaleId].State.RaisedFeeEth = Presales[presaleId].State.RaisedFeeEth.add(amountRecieved.mul(5).div(100));//5% is fee
    ...
    function AddUniswapLiquidity(uint256 presaleId) nonReentrant() external{
        ...
        IERC20(Presales[presaleId].Addresses.TokenAddress).approve(_uniswapRouterAddress, Presales[presaleId].TokenLiqAmount);//approve unirouter
        uint256 amountOfEth = Presales[presaleId].State.ContributedEth.sub(Presales[presaleId].State.RaisedFeeEth).mul(Presales[presaleId].LiqPercentage).div(100);
        if(Presales[presaleId].PermalockLiq)//permanently locked liq
        {
            IUniswapV2Router02(_uniswapRouterAddress).addLiquidityETH{value : amountOfEth}(address(Presales[presaleId].Addresses.TokenAddress), Presales[presaleId].TokenLiqAmount, 0, 0, address(0x000000000000000000000000000000000000dEaD), block.timestamp.add(1 days));
        }
        else// use allocation for locking
        {
            IUniswapV2Router02(_uniswapRouterAddress).addLiquidityETH{value : amountOfEth}(address(Presales[presaleId].Addresses.TokenAddress), Presales[presaleId].TokenLiqAmount, 0, 0, address(this), block.timestamp.add(1 days));
            address pairAddress = IUniswapV2Factory(_uniswapFactoryAddress).getPair(IUniswapV2Router02(_uniswapRouterAddress).WETH(), Presales[presaleId].Addresses.TokenAddress);
    ...
    //step 3 -> distribute eth to presale host and fees to ysecTODOTODO
    ...
    function EthDistributable(uint256 presaleId) public view returns(bool){
        return Presales[presaleId].State.ContributedEth.sub(Presales[presaleId].State.RetrievedEthAmount) > 0;
    }
