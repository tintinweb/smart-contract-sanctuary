...
import "../prices/TestChainlinkPriceSource.sol";

contract TestSynthetixPriceSource is Ownable, ISynthetixExchangeRates
{
    ...
    function setPriceSourcesForCurrencyKeys(
        ...
        require(_currencyKeys.length == _aggregators.length && _rateAssets.length == _aggregators.length);

        for (uint256 index = 0; index < _currencyKeys.length; index++) {
            currencyKeyToAggregator[_currencyKeys[index]] = AggregatorInfo({
                aggregator: _aggregators[index],
                rateAsset: _rateAssets[index]
            });
        }
    }
    ...
    function rateAndInvalid(bytes32 _currencyKey) 
        external view override returns (uint256 rate_, bool isInvalid_)
    {
        ...
        if (storedRate != 0) {
            ...
            AggregatorInfo memory aggregatorInfo = currencyKeyToAggregator[_currencyKey];//getAggregatorFromCurrencyKey(_currencyKey);
            ...
            uint256 decimals = TestChainlinkPriceSource(aggregator).decimals();
            rate_ = uint256(TestChainlinkPriceSource(aggregator).latestAnswer()).mul(
                10**(uint256(18).sub(decimals))
            );

            if (aggregatorInfo.rateAsset == RateAsset.ETH) {
                uint256 ethToUsd = uint256(
                    TestChainlinkPriceSource(
                        currencyKeyToAggregator[_currencyKey].aggregator
                    ).latestAnswer()
                );
    ...
    // function getAggregatorFromCurrencyKey(bytes32 _currencyKey) public view returns (AggregatorInfo memory _aggregator) 
    // {
...
contract TestChainlinkPriceSource {
    event AnswerUpdated(int256 indexed currentAnswer, uint256 indexed roundId, uint256 timestamp);
