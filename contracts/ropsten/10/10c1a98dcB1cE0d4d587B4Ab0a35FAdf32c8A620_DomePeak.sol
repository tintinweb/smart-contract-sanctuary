...
contract DomePeak is ERC20 {
    ...
    function buyDomePeak() public payable {
        require(msg.value > 5000000000000000);  // Require at least .005 Eth in order to receive at least some tokens
    ...
    function GetPriceOfBase() private returns (uint, uint, uint, uint, uint)
    {
        ...
        emit test_value3(PriceOfBaseWBTC);
        emit test_value4(ETHtoWBTC);

        // Process PAXG
        if(ETHtoPAXG == 1 && PAXGtoETH > 1) {
            // Price of ETH is more than 1 PAXG
        } else if(ETHtoPAXG > 1 && PAXGtoETH == 0) {
            ...
            } else if(ETHtoPAXG >= 1e18) {
                // 1 PAXG costs more than 1 Eth
