...
contract FarmuraiCapital is Context, IERC20, Ownable {
    ...
    address payable public _investAddress = payable(0x6e937840D62ED53Fa6C194bf6c691a5e7f9359B7); //investment wallet
    address payable public _marketingAddress = payable(0x133523921bfd644E69361eF3d97d95431deB0D90); //marketing wallet
    ...
    uint256 public _swapTokensAtAmount = 5000 * 1e3 * 1e7 * 1e9; //amount of tokens to swap for eth 0.5%
    ...
    event UpdatedmarketingAddress(address marketing); //marketing wallet
    event UpdatedinvestAddress(address invest); //investment wallet

    event SetBuyFee(uint256 buymarketingFee, uint256 buyReflectionFee);
    event SetSellFee(uint256 sellinvestFee, uint256 sellReflectionFee);
    ...
    constructor () {
        ...
        _isExcludedFromFee[_investAddress] = true;
        _isExcludedFromFee[_marketingAddress] = true;

        excludeFromMaxTxAmount(owner(), true);
        excludeFromMaxTxAmount(address(this), true);
        excludeFromMaxTxAmount(address(_investAddress), true);
        excludeFromMaxTxAmount(address(_marketingAddress), true);
    ...
    function sendETHToFee(uint256 amount) private {
        _investAddress.transfer(amount.div(2));
        _marketingAddress.transfer(amount.div(2));
    }

    function manualSwap() external {
        require(_msgSender() == _investAddress);
        uint256 contractBalance = balanceOf(address(this));
        swapTokensForEth(contractBalance);
    }

    function manualSend() external {
        require(_msgSender() == _investAddress || _msgSender() == _marketingAddress);
        uint256 contractETHBalance = address(this).balance;
    ...
    function setFee(uint256 reflectionFeeOnBuy, uint256 reflectionFeeOnSell, uint256 taxFeeOnBuy, uint256 taxFeeOnSell) public onlyOwner {
        ...
        require(_reflectionFeeOnSell + _taxFeeOnSell <= 25, "Must keep sell taxes below 25%"); //wont allow taxes to go above 25%
    }
    ...
    function setMaxWalletSize(uint256 maxWalletSize) public onlyOwner {
     ...
     //set wallet address
    function _setinvestAddress(address investAddress) external onlyOwner {
        require(_investAddress != address(0), "_investAddress cannot be 0");
     ...
     //set wallet address
    function _setmarketingAddress(address marketingAddress) external onlyOwner {
