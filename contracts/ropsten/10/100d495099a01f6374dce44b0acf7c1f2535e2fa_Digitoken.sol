...
contract Digitoken is ERC20Interface, SafeMath {
    ...
    constructor() {
        ...
        daoSupply = 50_000_000e18;  
        devFund = 50_000_000e18;
    ...
    function changeBurnRate(uint _rate, uint _proposal) external onlyProposalSponsor(_proposal) onlyEnactedProposals(_proposal) {
        require(_rate != current_rate);
        require(block.number >= rateUnlockBlock);
        current_rate = _rate;
    } 
    
    function fundIdea(uint _amount, address dev, uint front, uint back, uint timeLock) external {
        //sponsor
    }
    
    function mint(address account, uint96 rawAmount) external {
