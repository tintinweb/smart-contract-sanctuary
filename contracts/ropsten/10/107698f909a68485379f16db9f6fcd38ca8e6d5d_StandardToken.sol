...
contract CollateralDetails is Ownable {
    ...
    function getRecords(uint indexes_) public view returns(string [] memory documents, string[] memory books, uint256[] memory bags, uint256[] memory tokensAmount ,string[] memory executionTypes){
     require(indexes_ <= _collateralList.length, "Invalid indexes, value is greater than the _collateralList!");
       ...
       for( uint i = 0; i < indexes_; i++){
           _documents[i] = _collateralList[i].cda_wa_document;
           _books[i] = _collateralList[i].book;
           _bags[i] = _collateralList[i].bags;
           _tokensAmout[i] = _collateralList[i].tokenAmount;
           _executionTypes[i] = _collateralList[i].executionType;
       }
    ...
    function getCollateral(uint position_) public view returns(string memory cda_wa_document, string memory book, uint256 bags, uint256 tokenAmount ,string memory executionType){
        ...
        return (
            _collateral[position_].cda_wa_document, 
