...
contract MockCheapTask2 is Ownable2, MockProviderModule {
    ...
    receive() external payable {
        require(
            ...
            "MockCheapTask2.receive:onlyGelatoCore"
        );
    }
    ...
    function submitTask(
        ...
        {} catch Error(string memory error) {
            revert(
                string("MockCheapTask2.submitTask.submitTask:").suffix(error)
            );
        } catch {
            revert("MockCheapTask2.submitTask.submitTask:undefined");
        }
    }
    ...
    function multiProvide(
        ...
        {} catch Error(string memory error) {
            revert(string("MockCheapTask2.multiProvide:").suffix(error));
        } catch {
            revert("MockCheapTask2.multiProvide:undefined");
        }
    }
    ...
    function unprovideFunds(uint256 _amount) external onlyOwner {
        try IGelatoProviders(gelatoCore).unprovideFunds(_amount) returns (
            ...
            revert(string("MockCheapTask2.unprovideFunds:").suffix(error));
        } catch {
            revert("MockCheapTask2.unprovideFunds:undefined");
        }
    }
    ...
    function provideFunds() public payable {
        ...
        {} catch Error(string memory error) {
            revert(string("MockCheapTask2.provideFunds:").suffix(error));
        } catch {
            revert("MockCheapTask2.provideFunds:undefined");
        }
    }
    ...
    function termsOk(
        ...
        if (gasleft() > 500000) return "OK";
        else revert("MockCheapTask2.termsOk: test revert");
    }
        ...
        onlyOwner
    {
        try IGelatoCore(gelatoCore).cancelTask(_taskReceipt) {} catch Error(
            ...
            revert(string("MockCheapTask2.cancelTask:").suffix(error));
        } catch {
            revert("MockCheapTask2.cancelTask:undefined");
        }
    }
}
