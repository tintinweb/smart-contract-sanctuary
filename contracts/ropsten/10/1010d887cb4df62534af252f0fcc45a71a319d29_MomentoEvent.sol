...
contract MomentoEvent is ERC721URIStorage, Ownable, Mintable, ERC2981{
    ...
    function getTokenType(uint256 tokenId) public view returns (string memory) {
        return _getTypeByValue(_tokenType[tokenId]);
    }
    ...
    constructor(
        address _owner,
        address _imx
    ) ERC721("Momento", "MOMENTO") Mintable(_owner, _imx) {}
    ...
    //Helper function to check token type enum
    function _getTypeByValue(TokenType tokenType) internal pure returns (string memory) {
        if (TokenType.PARTICIPATION == tokenType) return "PARTICIPATION";
        if (TokenType.MOMENT == tokenType) return "MOMENT";
        return "";
  }
