...
contract AlPankNft is Ownable, ERC721
{
    ...
    */
    ...
    function getByImgId(string memory _imgId) public view returns (string memory imgId, uint256 tokenIdToPrice, address tokenIdToTokenAddress, string memory title, string memory url) {
        uint256 count_nft = getCounterLast();
        
        imgId = "";
        tokenIdToPrice = 0;
        tokenIdToTokenAddress = address(0);
        title = "";
        url = "";
        ...
        for (uint256 i=1; i <=count_nft; i++) {
            if (id_to_date[i].tokenIdToTokenAddress != address(0)) {
                ...
                bool isId = (keccak256(abi.encodePacked((nft_data.imgId))) == keccak256(abi.encodePacked((_imgId))));
                if (isId) {
                    imgId = nft_data.imgId;
                    tokenIdToPrice = nft_data.tokenIdToPrice;
                    tokenIdToTokenAddress = nft_data.tokenIdToTokenAddress;
    ...
    function isImgId(string memory imgId) public view returns (bool) {
        bool _isImgId = false;
        uint256 count_nft = getCounterLast();
        for (uint256 i=1; i <=count_nft; i++) {
    ...
    function getCounterLast() public view returns (uint256) {
