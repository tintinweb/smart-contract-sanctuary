...
contract MasterChef is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pendingSushi = user.amount.mul(pool.accSushiPerShare).div(1e12).sub(user.rewardDebtSushi);
            uint256 pendingKing = user.amount.mul(pool.accKingPerShare).div(1e12).sub(user.rewardDebtKing);
            safeSushiTransfer(msg.sender, pendingSushi);
            safeKingTransfer(msg.sender, pendingKing);
        }
    ...
    function harvest(uint256 _pid) public {
        ...
        emit Harvest(msg.sender, _pid, pendingSushi, pendingKing);
    }
