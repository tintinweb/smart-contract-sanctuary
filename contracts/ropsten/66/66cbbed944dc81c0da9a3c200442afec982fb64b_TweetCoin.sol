...
contract Pausable {
  ...
  /**
   * @dev Called by the pauser (initially set to the contract creator), changes
   * the pauser. It allows to freeze the methods protected by whenPaused by calling pause()
   * and then setPause(0x0), which will effectively prevent unpausing.
   */
