...
contract FoundingTeam is Pausable {
    ...
    uint256 private tokenSoldThreshold      = totalToken.mul(50).div(1000);
    // token supply amount
    uint256 public tokenSupplyQuota         = 0;

    // online
    // lock end time
    // uint256 public tokenLockEndTime         = 1609516799;
    
    // DEBUG - S
    // // lock end time
    // uint256 public tokenLockEndTime         = 0;
    
    // DEBUG - F
    // lock end time
    uint256 public tokenLockEndTime         = now + 125 minutes;
    ...
    function transferTokenAndLock(address _beneficiary, uint256 _amount) public onlyOwner isCanSold isOnSale freezeable(_beneficiary){
