...
contract AXLPresale is ReentrancyGuard {
    ...
    function userWithdrawTokens () public nonReentrant {
        require(presaleStatus() == 2, "Not Succed"); // Sucess
        BuyerInfo storage buyer = buyers[msg.sender];
    ...
    function remainingBurn() public onlyOwner {
        require(presaleStatus() == 2, "Not Succed"); // Sucess
        require(presale_info.hardcap * presale_info.token_rate > status.sold_amount * tokeninfo.decimal, "Nothing to burn");
        uint256 rushTokenAmount = IERC20(presale_info.sale_token).balanceOf(address(this)) - status.sold_amount;
        TransferHelper.safeTransfer(address(presale_info.sale_token), address(0), rushTokenAmount);
    }
}