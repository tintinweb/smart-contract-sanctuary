...
contract Test{  
   ...
   // IRouter public pancakeRouter;
    ...
    function getRate1(uint256 ip) public view  returns (uint[] memory amounts){ 
    ...
    function getRate2(uint ip) public view  returns (uint[] memory amounts){ 
        address[] memory path = new address[](2);
        path[1] = address(0x110a13FC3efE6A245B50102D2d79B3E76125Ae83);
        path[0] = pancakeRouter.WETH();
        return pancakeRouter.getAmountsOut(ip,path) ; 
    } 

    function getRate(uint toks) public view returns(uint256 , uint256){
      uint[] memory res1 = getRate1(toks) ; 
      uint[] memory res2 = getRate2(res1[1]) ;  

      return (res2[1] / 1000000  , res2[1]); 
