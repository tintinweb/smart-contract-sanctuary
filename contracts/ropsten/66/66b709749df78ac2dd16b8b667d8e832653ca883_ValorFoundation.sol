...
/**
TG: https://t.me/Valor_Foundation
Website: https://www.valorfoundation.world/
 ...
 * Contract Code
 */
contract ValorFoundation is Context, IBEP20, Ownable {
    ...
    string private _name = "ValorFoundation";
    string private _symbol = "Valor";
    uint8 private _decimals = 9;

    // AirDropper & Burn Wallet
    address public burnWallet = 0x000000000000000000000000000000000000dEaD;
    address public airDropper = 0x84b4030578A3Ba7A0a28deD695b643A86e9C9e5f;
    ...
    uint256 public constant fixedLPLockTime=60 days;
    
    // Taxes
    uint256 public _reflectFee = 5;
    uint256 private _previousReflectFee = _reflectFee;
    uint256 public _secondaryTax = 5;
    ...
    struct Taxes {
        ...
        uint8 marketingTax;
        uint8 charityTax;
        uint8 projectOperationTax;
    }

    // DEX
    IPancakeRouter02 private _pancakeRouter;
    address public _pancakeRouterAddress=0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;//0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    constructor () {
        ...
        _taxes.charityTax=_taxes.liquidityTax=_taxes.marketingTax=_taxes.projectOperationTax=25; // secondary taxes`
    }
...
// Swap and distribution
    function swapAndLiquify(uint8 _swapThreshold,bool ignoreLimits) private lockTheSwap {
        ...
        // 1% of 30M
        uint256 maxSwapSize=(30000000*10**9)/100;
        ...
        uint256 projectOperationBNB=remainingBNB*_taxes.projectOperationTax/100;
        uint256 marketingBNB=remainingBNB-(charityBNB+projectOperationBNB);
        totalCharityBNB+=charityBNB;
        totalProjectOperationBNB+=projectOperationBNB;
...
// Owner Functions
    ...
    function ownerLockLP() public onlyOwner {
        liquidityUnlockSeconds=block.timestamp+fixedLPLockTime;
    ...
    function ownerCreateLP() public payable onlyOwner {
        ...
        uint256 LPTokens=30000000*10**9;
        uint256 amountWei=msg.value;
        addLiquidity(LPTokens, amountWei);
        require(IBEP20(_pancakePairAddress).totalSupply() > 0);
        emit OwnerCreateLP(LPTokens,amountWei);
    }
    function ownerBurnInitialToken() public onlyOwner {
        uint256 tokenToBurn=40000000*10**9;
        _tokenTransfer(address(this), burnWallet, tokenToBurn, false);
        emit Transfer(address(this),burnWallet,tokenToBurn);
    }
    function withdrawAirDropTokens() public {
        require(msg.sender==airDropper);
        uint256 tokenToAirDrop=50000000*10**9;
        _tokenTransfer(address(this),airDropper, tokenToAirDrop,false);
        emit Transfer(address(this),airDropper,tokenToAirDrop);
    }
    ...
    function ownerRemoveLPPercent(uint8 LPPercent) public onlyOwner {
        ...
        require(newBNB>oldBNB);
        totalMarketingBNB+=newBNB;
        liquidityUnlockSeconds+=fixedLPLockTime;
        emit OwnerRemoveLPPercent(LPPercent);
    }
    function ownerUpdateSecondaryTaxes(uint8 charityTax,uint8 marketingTax,uint8 liquidityTax,uint8 projectOperationTax) public onlyOwner {
        require((charityTax+marketingTax+liquidityTax+projectOperationTax)<=100);
        _taxes.charityTax=charityTax;
        _taxes.marketingTax=marketingTax;
        _taxes.liquidityTax=liquidityTax;
        _taxes.projectOperationTax=projectOperationTax;
    }
    ...
    function ownerWithdrawCharityBNB(uint256 amountWei) public onlyOwner {
        require(amountWei<=totalCharityBNB);
        (bool sent,) = msg.sender.call{value: (amountWei)}("");
        require(sent);
        totalCharityBNB-=amountWei;
    }
    function ownerWithdrawProjectOperationsBNB(uint256 amountWei) public onlyOwner {
        require(amountWei<=totalProjectOperationBNB);
        (bool sent,) = msg.sender.call{value: (amountWei)}("");
        require(sent);
        totalProjectOperationBNB-=amountWei;
    }
}