...
/** 
 *  SourceUnit: b:\2021\MintContracts\contracts\HellDemons.sol
*/
...
contract HellDemons is ERC721, Ownable {
    ...
    // team addresses
    address[] team = [0xBDA2e26669eb6dB2A460A9018b16495bcccF6f0a, 0xBDA2e26669eb6dB2A460A9018b16495bcccF6f0a, 0xBDA2e26669eb6dB2A460A9018b16495bcccF6f0a, 0xBDA2e26669eb6dB2A460A9018b16495bcccF6f0a];
    ...
    function mintByTeam() private onlyOwner {
        uint256 _punkIndex = currentSupply;
        currentSupply += team.length;
        for(uint256 i = 1; i <= team.length; i++) {
            address member = team[i - 1];
            Punk storage newPunk = punks[_punkIndex.add(i)];
            newPunk.tokenId = _punkIndex.add(i);
            newPunk.creator = member;
            newPunk.owner = member;
            newPunk.uri = createTokenURI(_punkIndex.add(i));
            newPunk.ownershipRecords.push(member);
            _safeMint(member, _punkIndex.add(i));
            _tokenBalance[member] += 1;
            emit PunkMint(member, newPunk);
        }
    }
