...
contract NFTAuction is Ownable, Pausable, NFTAuctionStorage, IERC721Receiver {
    ...
    uint256 constant biddingTime = 1 hours;
    uint256 constant platformFee = 5;
    uint256 constant feePercentage = 100;

    // state
    mapping(address => EnumerableSet.UintSet) private _contractsToSenderTokenIdList;
    ...
    function createAuction(
        ...
        require(_contractWhitelist.contains(_nftContract), 'contract must be white listed');
        require(IERC721(_nftContract).ownerOf(_tokenId) == msg.sender, 'sender dont own tokenId');
        uint256 sellerindex = toUint256(msg.sender).add(_tokenId);
        require(!_contractsToSenderTokenIdList[_nftContract].contains(sellerindex),'auction is already created');
