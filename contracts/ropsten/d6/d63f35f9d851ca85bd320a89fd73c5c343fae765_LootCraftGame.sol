...
contract LootCraftGame is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getEnnemiesStrength(uint256 ennemyId) public view returns (uint256) {
        uint256 strength = calcEnnemiesStrength(ennemyId, "V2ENNEMIESCALC1");
        strength *= calcEnnemiesQuantity(ennemyId, "V2ENNEMIESCALC2");
        strength *= calcEnnemiesLevel(ennemyId, "V2ENNEMIESCALC3");
    ...
    function tokenURI(uint256 ennemyId, uint256 lootId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base" style="fill: #64D98A;">';
        parts[1] = 'VICTORY';
        parts[2] = '</text><text x="10" y="40" class="base">You defeated </text><text class="base" x="40" y="120" ';
