...
contract CliqStaking {
  ...
  uint256 constant public WITHDRAW_TIME_SILVER     = 45 * 1 days;   // variable constant to manage withdraw time lock up for silver
  uint256 constant public WITHDRAW_TIME_GOLD       = 90 * 1 days;   // variable constant to manage withdraw time lock up for gold
  uint256 constant public WITHDRAW_TIME_PLATINUM   = 135 * 1 days;  // variable constant to manage withdraw time lock up for platinum
  uint256 public TOKEN_REWARD_PERCENT_SILVER       = 10000000;      // variable constant to manage token reward percentage for silver
  uint256 public TOKEN_REWARD_PERCENT_GOLD         = 4735920;       // variable constant to manage token reward percentage for gold
  uint256 public TOKEN_REWARD_PERCENT_PLATINUM     = 11217430;      // variable constant to manage token reward percentage for platinum
  uint256 public TOKEN_PENALTY_PERCENT_SILVER      = 870042;        // variable constant to manage token penalty percentage for silver
  uint256 public TOKEN_PENALTY_PERCENT_GOLD        = 2367960;       // variable constant to manage token penalty percentage for gold
  uint256 public TOKEN_PENALTY_PERCENT_PLATINUM    = 5608715;       // variable constant to manage token penalty percentage for platinum
  ...
  */
  ...
  uint256 public totalStakedTokenInSilverPool = 0;
  
  // variable for total stake token in a pool
  uint256 public totalStakedTokenInGoldPool = 0;
  
  // variable for total stake token in a pool
  uint256 public totalStakedTokenInPlatinumPool = 0;
  ...
  */
  ...
  function stakeToken(uint256 tokens, uint256 time) public tokenStakeCheck(tokens, time) returns(bool){
    ...
    if(time == PERIOD_SILVER){
        require(totalStakedTokenInSilverPool.add(tokens) <= 1000000000000000000, "Pool Limit Reached");
    ...
    } else if (time == PERIOD_GOLD) {
        require(totalStakedTokenInSilverPool.add(tokens) <= 25000000000000000000000000, "Pool Limit Reached");
        _tokentime = now + (time * 1 days);
        _tokenStakingCount = _tokenStakingCount +1;
        _tokenTotalDays[_tokenStakingCount] = time;
        _tokenStakingAddress[_tokenStakingCount] = msg.sender;
