...
contract TestAndLearn {
    ...
    uint public constant PRICE = 800 finney;

    // The beneficiary is 350.org
    address public constant BENEFICIARY = 0xA4f58588f83f9AFF6F1de51Ae4A1E89DC8d9b9e5;
    ...
    string internal nftName = "Test&Learn";
    ...
    function draw(uint id) public view returns (string) {
        ...
        for (int i = int(0); i < SIZE; i++) {
            ...
            for (int j = int(0); j < SIZE; j++) {
                test[0] = (2 * (j - HALF_SIZE) + 1);
                if (a % 2 == 1) {
                    test[0] = abs(test[0]);
                }
                test[0] = test[0] * int(a);
                v = uint(test[0] * test[1] / ONE) % mod;
                if (v < 5) {
