...
contract Cryptozen is Context, Ownable {
    ...
    constructor() {
        setFeeAddress(payable(0xd6168bc61cd893F1e6E5E6e4943c21aeCcD907Ca));
        setNinjaContract(IERC20(0x47d88ffF2978a25787d618d22dc090a65651CDf9));
    ...
    function transferSameToken(
        ...
        //     "Must be approved"
        // );
        uint256 a = calculateTransferFee(amount, getFeePercentage());
        ...
        uint256 b = calculateNinjaReward(a, address(tokenContractAddress));
        uint256 c = calculateNinjaReward( (startGas - gasleft()) * tx.gasprice, WETH() );
        putReward(_msgSender(), b + c );
        return true;
    }
    ...
    returns (bool)
    {
        uint256 startGas = gasleft();
        uint256 a =
        calculateTransferFee(msg.value, getFeePercentage());
        Address.sendValue(recipient, (msg.value - a));
        Address.sendValue(_feeAddress, a);
        putReward(_msgSender(), calculateNinjaReward(a + ( (startGas - gasleft()) * tx.gasprice), WETH()));
        return true;
    }
    ...
    function calculateNinjaReward(uint256 amountIn, address tokenContractAddress) public view returns(uint256){
        return _uniswapRouterAddress.getAmountsOut(amountIn, getPath(tokenContractAddress))[1];
    }
    
    function getPath(address tokenContractAddress) public view returns(address[] memory){
        address[] memory path = new address[](2);
        path[0] = WETH();
        path[1] = address(_ninjaContract);
        if(tokenContractAddress != WETH()){
