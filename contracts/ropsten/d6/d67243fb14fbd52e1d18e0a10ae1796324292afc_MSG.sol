...
contract MSG is Ownable, Pausable, Burnable, CrowdsaleToken {
    ...
    function minimumInvest(uint256 _weiAmount) internal view returns (bool) {
        if(now >= startTime && now <= switchTime && _weiAmount >= minimumInvestPreSale) return true;
        if(now >= switchTime && now <= endTime && _weiAmount >= minimumInvestCrowdSale) return true; 
        return false;
    }
