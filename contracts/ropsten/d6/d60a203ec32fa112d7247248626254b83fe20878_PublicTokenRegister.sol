...
contract PublicTokenRegister is IPublicTokenRegister, Ownable {
	...
	function removeCoin(string _name) public onlyOwner {
		bytes4 _coin = convertStringToBytes(_name);
		require(coin[_coin].isReg == true);
		coin[_coin].isReg = false;
	}
	...
	function getHexSymbol(string _coin) public view returns(bytes4 _hexSymbol) {
   	...
   	function getCoinAddress(string _coin) public view returns(address _coinTokenContract) {
		return coin[convertStringToBytes(_coin)].coinTokenContract;
	}
	
	function getIsCoinReg(string _coin) public view returns(bool _isCoinReg) {
		return coin[convertStringToBytes(_coin)].isReg;
	}
	
	function getCoinInfo(string _coin) public view returns(string _name, string _symbol, address coinAddress, bytes4 _hexSymbol, bool _isReg) {
		return (getName(_coin),getSymbol(_coin),getCoinAddress(_coin),getHexSymbol(_coin),getIsCoinReg(_coin));
	}
