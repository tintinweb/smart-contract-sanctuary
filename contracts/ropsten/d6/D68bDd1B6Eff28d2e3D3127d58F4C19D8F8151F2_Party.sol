...
contract Party {
    ...
    function checkDifferenceInBalance() external view isAlive returns (int256) {
        return int256(getPartyBalance()) - int256(balanceParty);
    }
    ...
    function getAllowance() external view isAlive returns (uint256) {
    ...
    function getMemberList() external view isAlive returns (address[] memory) {
        return memberList;
    }
     ...
     */
    function sell(uint256 amount) external isAlive {
        // distribute the amount to the payers
        uint256 _receive = 0;
        for (uint256 i = 0; i < memberList.length; i++) {
            uint256 _temp = ((amount * weight[memberList[i]]) / 10**7);
            balance[memberList[i]] = balance[memberList[i]] + _temp;
            _receive += _temp;
        }
        token.safeTransferFrom(msg.sender, address(this), _receive);
        emit ProfitDepositEvent(_receive, memberList);
    }
