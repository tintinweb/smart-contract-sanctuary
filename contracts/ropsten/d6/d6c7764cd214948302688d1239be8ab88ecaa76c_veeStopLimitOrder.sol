...
contract veeStopLimitOrder {
    ...
    struct stopLimitOrder{
     ...
     event OnCreateOroder(bytes32 indexed orderIDHash,
     ...
     event OnCompleteOroder(    bytes32 indexed orderIDHash, bytes32 orderID,
                                uint uniswapExchangeNum);

     //3. 挂单过期后的EVENT,3个参数为indexed用于过滤和检索
     event OnCancelOroder(bytes32 indexed orderIDHash, bytes32 orderID);
    ...
    function CreateOrder(address orderOwner,address tokenFrom,address tokenTo,uint amount,uint maxPrice,uint minPrice, uint expiryDate,bool isAutoRepay) external returns (bytes32) {
    ...
    function CancelOroder(bytes32 orderid)external returns(bool){
        emit OnCancelOroder(orderid, orderid);
        return true;
    }
