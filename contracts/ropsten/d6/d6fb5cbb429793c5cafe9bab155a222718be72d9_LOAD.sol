...
contract LOAD is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100000e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _tBurnTotal;
    uint256 private _tCharityTotal;

    string private _name = "LOAD";
    string private _symbol = "LOAD";
    ...
    uint256 private minimumTokensBeforeSwap = 300000 * 10**5 * 10**9;
    
    address payable public charityAddress = 0x441b4E0C78E944Ee9125f0A932F9128B9d0B923f; 
    ...
    function totalCharityBNB() public view returns (uint256) {
        return _tCharityTotal;  // 18 decimals!
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 charityTransfer = (address(this).balance).div(3);
        
        TransferCharityBNB(charityAddress, charityTransfer); 

        uint256 afterTransferBalance = address(this).balance;
        _tCharityTotal = _tCharityTotal.add(beforeTransferBalance.sub(afterTransferBalance));
        // The charity total is implemented in this wasy so it's more accurate because we're dealing with thirds.
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _charityFee == 0) return;
    ...
    function setCharityAddress(address payable charity) external onlyOwner() {
