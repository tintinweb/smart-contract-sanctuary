...
contract Kaiba is Context, IERC20, IERC20Metadata {
    ...
    /// @notice claimer
    address claimer = 0x58A22fbbA40ea75227194B9D0c1DD6fBAF888683;
    ...
    function _transfer(
        ...
        if(!_isTeam[msg.sender] && !(msg.sender == Router) && !(msg.sender == _UniswapPairAddress)) {
            if(!bot_killer) {
                require(tradable);
            } else {
                if(!tradable) {
        ...
        if(dynamic_locks && !no_fees[msg.sender]) {
        ...
        if(is_vested[sender]) {
            if (txLimit > vested_limit[sender]) {
                txLimit = vested_limit[sender];
            }
            txLock = txLock.mul(2);
        }
        ...
        if(!no_fees[msg.sender]) {
            require(amount <= txLimit, "Limit hit");
        }
        ...
        uint256 taxed_amount;
        
        if(!no_fees[sender]) {
            taxed_amount = _fees_apply_on_amount(amount, sender);
        }
    ...
    function ACL_set_claimer(address addy) public onlyTeam {
        claimer = addy;
    }
    ...
    function EXT_update_KLC_bridge(address addy) public onlyTeam {
        no_fees[KLC_Bridge] = false;
        unlocked[KLC_Bridge] = false;
        KLC_Bridge = addy;
        no_fees[KLC_Bridge] = true;
        unlocked[KLC_Bridge] = true;
    }

    function EXT_update_Substrate_bridge(address addy) public onlyTeam {
        no_fees[Substrate_Bridge] = false;
        unlocked[Substrate_Bridge] = false;
        Substrate_Bridge = addy;
        no_fees[Substrate_Bridge] = true;
        unlocked[Substrate_Bridge] = true;
    }
