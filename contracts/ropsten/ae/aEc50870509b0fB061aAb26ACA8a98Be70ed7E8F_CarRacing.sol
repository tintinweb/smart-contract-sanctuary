...
pragma solidity ^0.8.0;
import "./CarLevelUp.sol";

contract CarHelper is CarLevelUp {

  uint levelUpFee = 0.001 ether;

  //Actions
  modifier aboveLevel(uint _level, uint _carId) {
    require(cars[_carId].carLevel >= _level);
    _;
  }
  ...
  function levelUp(uint _carId) external payable {
    ...
    cars[_carId].carLevel = cars[_carId].carLevel++;
  }

  function changeName(uint _carId, string calldata _newName) external aboveLevel(2, _carId) onlyOwnerOf(_carId) {
    cars[_carId].name = _newName;
  }

  function changeDna(uint _carId, uint _newDna) external aboveLevel(20, _carId) onlyOwnerOf(_carId) {
...
contract CarLevelUp is CarFactory {
  ...
  function _isReady(Car storage _car) internal view returns (bool) {
      return (_car.readyTime <= block.timestamp);
  }

  function UpgradeCar(uint _carId, uint _targetDna) internal onlyOwnerOf(_carId) {
    Car storage myCar = cars[_carId];
    require(_isReady(myCar));
    _targetDna = _targetDna;
    uint newDna = (myCar.dna + _targetDna);
    cars[_carId].dna = newDna;
    //UPDATE CAR will come here
    _triggerCooldown(myCar);
  }

  function levelUpCar(uint _carId, uint _targetDna) public {
    UpgradeCar(_carId, _targetDna);
  }
...
contract CarFactory is Ownable{
    ...
    event NewCar(uint carId, string name, uint dna, uint engineLevel, uint carHP, uint carFuel, uint endurance, uint maxSpeed, uint carLevel);
    ...
    function _createCar( uint _carId, string memory _name, uint _dna,
     ...
     uint _maxSpeed, uint _carLevel) private {
        ...
        emit NewCar(_carId, _name, _dna, _engineLevel, _carHP, _carFuel, _endurance, _maxSpeed, _carLevel);
    }


    function createRandomCar(string memory _name, uint packType) payable public {
