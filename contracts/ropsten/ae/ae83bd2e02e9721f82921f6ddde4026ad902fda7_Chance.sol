...
contract Chance {
    ...
    constructor(address ownerAddress,address tokenAddress) public {
        ...
        for (uint8 j = 1; j <= LAST_LEVEL; j++) {
            users[ownerAddress].activeO3Levels[j] = true;
            users[ownerAddress].activeO6Levels[j] = true;
        }
    ...
    function donate(uint256 amount) public {
          uint256 convertamount = amount * 10000000000000000;
          if(token.balanceOf(msg.sender) >= convertamount){
             if(convertamount <= token.allowance(msg.sender,address(this))){
                 token.transferFrom(msg.sender,address(this),convertamount);
                 token.transfer(owner,convertamount);
             }else{
                 revert("Error allowance");
             }
        }else{
            revert("Error Balance");
        }
     }

      function donateUser(uint256 amount,address useraddress) public {
          uint256 convertamount = amount * 10000000000000000;
          if(token.balanceOf(msg.sender) >= convertamount){
             if(convertamount <= token.allowance(msg.sender,address(this))){
                 token.transferFrom(msg.sender,address(this),convertamount);
                 token.transfer(useraddress,convertamount);
             }else{
                 revert("Error allowance");
             }
        }else{
    ...
    function buyNewLevel(uint8 matrix, uint8 level,uint256 amount) public {
        require(isUserExists(msg.sender), "user is not exists. Register first.");
        require(matrix == 1 || matrix == 2, "invalid matrix");
        uint256 convertamount = amount * 10000000000000000;
        require(convertamount == levelPrice[level], "invalid price");
    ...
    function registration(address userAddress, address referrerAddress) private {
        //require(msg.value == 0.002, "registration cost 2");
        ...
        lastUserId++;
         totalearnedtrx = totalearnedtrx+(levelPrice[1] * 2);
    ...
    function updateO6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        address[] memory O6mat = users[users[referrerAddress].O6Matrix[level].currentReferrer].O6Matrix[level].firstLevelReferrals;
    ...
    function sendTronDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        uint levelpricefinal = levelPrice[level];
        if(matrix==1)
        {
        ...
        else if(matrix==2)
        {
                ...
                users[userAddress].O6Income += levelpricefinal ;
        }


        if(token.balanceOf(_from) >= levelpricefinal){
             if(levelpricefinal <= token.allowance(msg.sender,address(this))){
                 token.transferFrom(msg.sender,address(this),levelPrice[level]);
                 token.transfer(userAddress,levelPrice[level]);
             }else{
                 revert("Error allowance");
             }
        }else{
