...
import "./BlucaEggAirdrop.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721URIStorage.sol";

contract BlucaEggOwnership is ERC721URIStorage, BlucaEggAirdrop {
    constructor() ERC721("BlucaEgg", "BLUCA EGG") {}

    function claimBlucaEgg() external payable onlyAirdropWhitelist(msg.sender) {
        require(!isClaimed[msg.sender], "this address have claimed already");
        require(msg.value == airdropFee, "value not equal airdrop fee");
        AirdropEggDetail memory eggDetail = whitelistEggDetail[msg.sender];
        spawnBlucaEgg(eggDetail.rarity, eggDetail.element, eggDetail.id);
        _safeMint(msg.sender, eggDetail.id);
        _setTokenURI(eggDetail.id, eggDetail.tokenUri);
        updateIsClaimed(msg.sender);
    }
}
