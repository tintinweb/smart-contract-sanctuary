...
abstract contract ERC20 is IERC20{
  ...
  function transferFrom(address from, address to, uint256 amount) public override returns (bool success){
    if(from != msg.sender){
      ...
      require(allowanceAmount >= amount,"transder amount exceed allowance");
  ...
  function _transfer(address from , address to,uint amount) internal {
    ...
    require(_balances[from] >= amount,"amount is enough");
...
contract LCNC is ERC20 {
  constructor() ERC20("Lucky Number Coin","LCNC"){}
  ...
  function withdraw(uint amount) public {
    require(amount > 0 && _balances[msg.sender] >= amount , "withdraw amount exceeds balance.");
