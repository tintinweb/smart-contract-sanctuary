...
contract NapXDafTopicsArena {
  ...
  event LogNewTopic   (bytes15 indexed topicName, uint index, address topicAddress, string longDescription, string assetClass);
  event LogUpdateTopic(bytes15 indexed topicName, uint index, address topicAddress, string longDescription, string assetClass);
  ...
  function getTopic(bytes15 topicName)
    public
    constant
    returns(address topicAddress, uint index, string longDescription, string assetClass)
  {
    if(!isTopic(topicName)) revert();
    return(
  ...
  function updateTopicDescription(bytes15 topicName, string longDescription)
    onlyOwner
    public
    returns(bool success)
  {
  ...
  function updateTopicAssetClass(bytes15 topicName, string assetClass)
    onlyOwner
    public
    returns(bool success)
  {
