...
contract PrivateSale is Pausable {
    ...
    function getContractTokenBalance() internal returns(bool){
    ...
    function _isSoldOut() public view returns (bool _rsoldOut) {
        ...
        return (tokensSold >= tokenSupplyQuota);
    }
    ...
    function setNewlockAccount(address _lockAddr, uint256 _endTime, uint256 _lockAmount) internal returns(bool){
        ...
        // TODO:測試時再拉出來徹底檢驗
        // 判斷是否已在locker list
        if(investorMap[_lockAddr].addr != address(0) && investorMap[_lockAddr].addr != _lockAddr){
