...
contract UserInterface is Ownable{
    ...
    modifier updateAccount(address addr) {
      ...
      if(balanceToAdd > 0) {
        ...
        accounts[addr].lastDividendPoints = actualminer.totalDividendPoints();
      }
      _;
    }
    ...
    function getReferralCode() external view returns(bytes8){
    ...
    function dividends(address addr) private view returns(uint256) {
      uint256 newDividend = (actualminer.totalDividendPoints().sub(accounts[addr].lastDividendPoints)).mul(accounts[addr].Tokens);
      return newDividend;
    }
    ...
    returns(uint256)
    {
        ...
        uint256 token = ethAmount.div(PriceNow);
        uint256 dividendShift = actualminer.totalDividendPoints().sub(accounts[addr].lastDividendPoints)*(accounts[addr].Tokens.div(accounts[addr].Tokens.add(token)));
        accounts[addr].lastDividendPoints = actualminer.totalDividendPoints().sub(dividendShift);
    ...
    isActualMiner()
    {
        ...
        if(referralMap[affCode]!=address(0)){
            ...
            //Affiailate function
            accounts[referralMap[affCode]].AffiliateBalance += token.mul(PriceNow).mul(ReferralRatio).div(uint256(10000)); 
        }
        
        if(ReferralThreshold > accounts[addr].Tokens && ReferralThreshold <= (accounts[addr].Tokens.add(token)))
            ReferralActivated(addr);
        //
        accounts[addr].lastDividendPoints = actualminer.totalDividendPoints().sub((actualminer.totalDividendPoints().sub(accounts[addr].lastDividendPoints))*(accounts[addr].Tokens.div(accounts[addr].Tokens.add(token))));
        accounts[addr].Tokens += token;
    ...
    function checkExpired() external{
        ...
        for (; i<arrayLength; i++) {
          if((timeNow-salesRecord[i].Time)>ExpiredTime){
            ...
            accounts[salesRecord[i].addr].lastDividendPoints = actualminer.totalDividendPoints().sub((actualminer.totalDividendPoints().sub(accounts[salesRecord[i].addr].lastDividendPoints)).mul(accounts[salesRecord[i].addr].Tokens).div(accounts[salesRecord[i].addr].Tokens.sub(salesRecord[i].Tokens)));
