...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    uint256 public constant NEEDED_TIME_AFTER_LM = 2 weeks;
    ...
    modifier onlyLiquidityMining() {
        require(_msgSender() == address(liquidityMining), "PB: Not an LM");
        _;
    }
    ...
    modifier onlyPolicyBookAdmin() {
        require(_msgSender() == policyBookAdmin, "PB: Not a PBAdmin");
        _;
    }
        ...
        updateBMIDAIXStakingReward
    {
        ...
        epochAmounts[holder.endEpochNumber] = epochAmounts[holder.endEpochNumber].sub(
            holder.coverTokens
        );
    ...
    function buyPolicy(uint256 _epochsNumber, uint256 _coverTokens)
        external
        override
        ...
        updateBMIDAIXStakingReward
    {
        ...
        require(
            !policyRegistry.isPolicyActive(_msgSender(), address(this)),
            "PB: The holder already exists"
        );
        require(
            !claimingRegistry.isClaimPending(
                claimingRegistry.claimIndex(_msgSender(), address(this))
            ),
    ...
    function _distributePremiums() internal {
        ...
        if (currentEpoch > lastEpoch) {
            ...
            uint256 lastDistributionEpoch =
                currentEpoch.min(lastEpoch + MAX_PREMIUM_DISTRIBUTION_EPOCHS + 1);
    ...
    function updateEpochsInfo() public override {
        (uint256 _lastEpochUpdate, uint256 _currentEpochNumber, uint256 _totalCoverTokens) =
            getUpdatedEpochsInfo();
    ...
    function secondsToEndCurrentEpoch() external view returns (uint256) {
        uint256 epochNumber = block.timestamp.sub(epochStartTime).div(EPOCH_DURATION).add(1);
    ...
    function addLiquidityFromLM(address _liquidityHolderAddr, uint256 _liquidityAmount)
        external
        override
        onlyLiquidityMining
    {
    ...
    function isExitFromQueuePossible(address _userAddr)
        external
        view
        override
    ...
    function updateWithdrawalQueue(uint256 _lastIndexToUpdate) external override {
        ...
        uint256 _usersToUpdateCount =
            withdrawalQueue.length().min(MAX_USERS_TO_UPDATE).min(_lastIndexToUpdate.add(1));
    ...
    function leaveQueue() external override {
        require(
            getWithdrawalStatus(_msgSender()) == WithdrawalStatus.IN_QUEUE,
            "PB: User not in a queue"
        );
    ...
    function _requestWithdrawal(uint256 _tokensToWithdraw) internal withPremiumsDistribution {
        require(_tokensToWithdraw > 0, "PB: Zero amount");

        WithdrawalStatus _status = getWithdrawalStatus(_msgSender());
        require(
            _status == WithdrawalStatus.NONE || _status == WithdrawalStatus.EXPIRED,
        ...
        uint256 _availableDaiBalance =
            convertDAIXtoDAI(
                balanceOf(_msgSender()).add(withdrawalsInfo[_msgSender()].withdrawalAmount)
            );
    ...
    function unlockTokens() external override {
        require(
            getWithdrawalStatus(_msgSender()) == WithdrawalStatus.EXPIRED,
            "PB: Withdrawal not expired"
        );
    ...
    function _withdrawLiquidity(address _userAddr, uint256 _tokensToWithdraw)
        internal
        updateBMIDAIXStakingReward
    {
        uint256 _daiTokensToWithdraw = convertDAIXtoDAI(_tokensToWithdraw);
    ...
    function triggerPremiumsDistribution() external {
        uint256 epochBefore = lastPremiumDistributionEpoch;
        _distributePremiums();

        emit DistributionManuallyTriggered(
            _msgSender(),
            lastPremiumDistributionEpoch.sub(epochBefore)
        );
    }
    ...
    {
        return (name(), insuranceContractAddress, contractType, whitelisted);
    }
}
...
library Queue {
    ...
    function remove(UniqueAddressQueue storage baseQueue, address addrToRemove)
        internal
        returns (bool)
    {
    ...
    function next(UniqueAddressQueue storage baseQueue, address addr)
        internal
        view
        returns (address)
    {
    ...
    function at(UniqueAddressQueue storage baseQueue, uint256 index)
        internal
        view
        returns (address)
    {
    ...
    function contains(UniqueAddressQueue storage baseQueue, address addr)
        internal
        view
        returns (bool)
    {
        ...
        if (
            baseQueue.HEAD != addr &&
            currentNode.next == address(0) &&
            currentNode.prev == address(0)
        ) {
            return false;
        }
