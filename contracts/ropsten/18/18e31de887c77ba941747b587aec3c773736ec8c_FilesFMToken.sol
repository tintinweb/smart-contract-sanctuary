...
contract FilesFMToken is TokenRecoverable, ERC777ERC20BaseToken {
    ...
    function mint(address _tokenHolder, uint256 _amount, bytes _operatorData) public hasMintPermission canMint {
        doMint(_tokenHolder, _amount, _operatorData);
    }

    function mintToken(address _tokenHolder, uint256 _amount) public hasMintPermission canMint {
        doMint(_tokenHolder, _amount, "");
    }

    function mintTokens(address[] _tokenHolders, uint256[] _amounts) public hasMintPermission canMint {
    ...
    function sendWithSignature(address _to, uint256 _amount, uint256 _fee, bytes _data, uint256 _nonce, bytes _sig) public canTransfer returns (bool) {
    ...
    /// @param _sig concatenated r,s,v values
    /// @return `true` if the token transfer is success, otherwise should fail
    function transferWithSignature(address _to, uint256 _amount, uint256 _fee, bytes _data, uint256 _nonce, bytes _sig) public canTransfer returns (bool) {
