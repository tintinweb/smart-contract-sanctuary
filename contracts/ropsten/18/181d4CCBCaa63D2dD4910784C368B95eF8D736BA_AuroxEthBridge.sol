...
contract AuroxEthBridge is IAuroxBridge {
    ...
    */
    function registerSwap(address[] memory path, uint256 amountIn, uint256 minAmountOut) external override {
        ...
        emit RegisterSwap(msg.sender, path[path.length - 1], usdcAmount[0]);
    }
    /**
    ...
    */
    ...
    function buyAssetOnBehalf(address[] memory path, address userAddress, uint usdcAmount) external override onlyNode {
        ...
        uint[] memory amounts = uniROUTER.swapTokensForExactTokens(usdcAmount, type(uint256).max, path, address(this), block.timestamp + 1000, false);
...
interface IAuroxBridge {
    function registerSwap(address[] memory path, uint256 amountIn, uint minAmountOut) external;
