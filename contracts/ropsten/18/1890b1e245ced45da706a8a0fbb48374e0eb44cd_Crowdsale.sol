...
contract Crowdsale is Ownable { 
  ...
  uint256 public tokensForSale = 10000000000000000000000000;
  uint256 public tokensForPrivateSale = 100000000000000000000;
  uint256 public tokensForPreSale = 100000000000000000000;
  uint256 public tokensForPublicSale = 100000000000000000000;
  ...
  uint256 public minimumETHInPrivateSale = 100000000000000000;
  uint256 public MaximumETHInPrivateSale = 10000000000000000000;
  ...
  uint256 public privateSaleTokensPerETH = 100000000000000000;
  uint256 public preSaleTokensPerETH = 100000000000000000;
  uint256 public publicSaleTokensPerETH = 100000000000000000;
   ...
   */
   function _preValidatePurchase(uint256 _ETH) internal view { 
      ...
      if (currentStage == Stages.PrivateSaleStart) {
        require(_ETH >= minimumETHInPrivateSale);
        require(_ETH <= MaximumETHInPrivateSale);
      }
   ...
   */
   function _deliverTokens(address _beneficiary, uint256 _tokenAmount) internal {
    ...
    if (currentStage == Stages.PrivateSaleEnd) {
        require(token.transferFrom(owner(),_beneficiary,_tokenAmount));  
      }
      
      else if (currentStage == Stages.PreSaleEnd) {
        require(token.transferFrom(owner(),_beneficiary,_tokenAmount));  
   ...
   */
    ...
    */
    function _getTokenAmount(uint256 _ETH) CrowdsaleStarted internal view returns (uint256) {
      uint256 tokens;
      if (currentStage == Stages.PrivateSaleStart) {
         tokens = _ETH.mul(privateSaleTokensPerETH);
      }
      else if (currentStage == Stages.PreSaleStart) {
         tokens = _ETH.mul(privateSaleTokensPerETH);
      }
