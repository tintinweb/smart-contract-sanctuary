...
contract MultiLot is AbstractLot {
  ...
  function withdrawRefund(uint256 _lotId) external {
    ...
    if (size < lot.totalDepositPoolA && lot.userDepositPoolA[msg.sender] > 0) {
      uint256 refundRatio = (ratioPrecision * (lot.totalDepositPoolA - size)) /
      ...
      uint256 refundRatio = (ratioPrecision * (lot.totalDepositPoolB - size)) /
  ...
  function withdrawClaim(uint256 _lotId) external returns (bool isSuccessful) {
    ...
    if (lot.userDepositPoolA[msg.sender] > 0) {
      uint256 claimRatio = (lot.userDepositPoolA[msg.sender] * ratioPrecision) /
        lot.totalDepositPoolA;
      claimAmount = (claimRatio * lot.totalClaimPoolA) / ratioPrecision;
    } else {
      uint256 claimRatio = (lot.userDepositPoolB[msg.sender] * ratioPrecision) /
        lot.totalDepositPoolB;
...
abstract contract AbstractLot is Ownable {
  ...
  function resolveLot(uint256 lotId) external virtual;
