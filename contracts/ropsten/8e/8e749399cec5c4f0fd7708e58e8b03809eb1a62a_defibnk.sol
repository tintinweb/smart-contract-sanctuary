...
contract defibnk {
    ...
    function becomeInvestor(address affiliate) public payable returns (bool) {
        ...
        if (lastInvestment + THREE_DAYS < block.timestamp) {
            ...
            else if (amount >= 15 ** 16 && amount <= 5 ** 17 ) {
            ...
            else if (amount >= 5 ** 17 && amount <= 2 ** 18 ) {
                lastInvestment = block.timestamp;
    ...
    // All money goes to charities and NGOs accross the world
    function investInTheSystem() public payable {
        bnkfee.transfer(msg.value);
    }

    // Several investments run using the money invested into the bank
