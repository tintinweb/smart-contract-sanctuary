...
contract NFTTest is ERC721, Ownable, ReentrancyGuard {
    ...
    */
     ...
     */
    function premintMasks(uint256 _indexStart) external onlyOwner {
        require(totalSupply() < MAX_MASK_SUPPLY, "Sale over");

        for (uint i = _indexStart; i < _indexStart + 100; i++) {
...
interface INFTRugs {
    function safeRugTransferFrom(address _from, address _to, uint256 _tokenId) external;
    function ownerOf(uint256 tokenId) external view returns (address);
    function totalSupply() external view returns (uint256);
}

/**
 * @title Rug Marketplace Contract for https://carpet.finance
 */
contract RugMarketplace is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    ...
    // Max supply of Rugs
    uint256 public constant MAX_RUG_SUPPLY = 10000;

    // The NFTRugs contract address
    address public nftRugsContract;
    ...
    uint256 public marketplaceTransactionFee = 1000;


    // A record of Rugs that are offered for sale at a specific minimum value, and perhaps to a specific person
    mapping (uint => Offer) public rugsOfferedForSale;

    // A record of the highest Rug bid
    mapping (uint => Bid) public rugBids;
    ...
    // Events
    event RugTransfer(address indexed _from, address indexed _to, uint256 _rugIndex);
    event RugOffered(uint indexed _rugIndex, uint _minValue, address indexed _toAddress);
    event RugBidEntered(uint indexed _rugIndex, uint _value, address indexed _fromAddress);
    event RugBidWithdrawn(uint indexed _rugIndex, uint _value, address indexed _fromAddress);
    event RugBought(uint indexed _rugIndex, uint _value, address indexed _fromAddress, address indexed _toAddress);
    event RugNoLongerForSale(uint indexed _rugIndex);

    /**
     * @dev Contract constructor
     */
    constructor(address _nftRugsContract) public {
        devAddr = msg.sender;
        nftRugsContract = _nftRugsContract;
    }
    ...
    */
    ...
    /**
     * @dev Transfer ownership of a Rug to another user without requiring payment
     */
    function transferRugWithoutPayment(address _to, uint _rugIndex) public {
        requireChecks(_rugIndex);

        if (rugsOfferedForSale[_rugIndex].isForSale) {
            rugNoLongerForSale(_rugIndex);
        }

        INFTRugs(nftRugsContract).safeRugTransferFrom(msg.sender, _to, _rugIndex);

        // Check for the case where there is a bid from the new owner and refund it.
        // Any other bid can stay in place.
        Bid memory bid = rugBids[_rugIndex];

        if (bid.bidder == _to) {
            // Kill bid and refund value
            pendingWithdrawals[_to] += bid.value;
            rugBids[_rugIndex] = Bid(false, _rugIndex, address(0), 0);
        }
    }

    /**
     * @dev Set a Rug's offer to sell to be false
     */
    function rugNoLongerForSale(uint _rugIndex) public {
        requireChecks(_rugIndex);

        rugsOfferedForSale[_rugIndex] = Offer(false, _rugIndex, msg.sender, 0, address(0));
        emit RugNoLongerForSale(_rugIndex);
    }

    /**
     * @dev Offer a Rug for sale with a minimum price
     */
    function offerRugForSale(uint _rugIndex, uint _minSalePriceInWei) public {
        requireChecks(_rugIndex);

        rugsOfferedForSale[_rugIndex] = Offer(true, _rugIndex, msg.sender, _minSalePriceInWei, address(0));
        emit RugOffered(_rugIndex, _minSalePriceInWei, address(0));
    }

    /**
     * @dev Offer to sell a Rug to a specific address
     */
    function offerRugForSaleToAddress(uint _rugIndex, uint _minSalePriceInWei, address _toAddress) public {
        requireChecks(_rugIndex);

        rugsOfferedForSale[_rugIndex] = Offer(true, _rugIndex, msg.sender, _minSalePriceInWei, _toAddress);
        emit RugOffered(_rugIndex, _minSalePriceInWei, _toAddress);
    }

    /**
     * @dev The Rug buying function
     */
    function buyRug(uint _rugIndex) payable public nonReentrant {
        require(INFTRugs(nftRugsContract).totalSupply() == MAX_RUG_SUPPLY, "Sale over");

        Offer memory offer = rugsOfferedForSale[_rugIndex];

        require(_rugIndex <= MAX_RUG_SUPPLY, "No rug");
        ...
        require(offer.seller == INFTRugs(nftRugsContract).ownerOf(_rugIndex), "Seller not owner");
        ...
        INFTRugs(nftRugsContract).safeRugTransferFrom(seller, msg.sender, _rugIndex);

        emit RugNoLongerForSale(_rugIndex);

        pendingWithdrawals[seller] += paymentAfterFeeRemoved;

        emit RugBought(_rugIndex, msg.value, seller, msg.sender);
    ...
    /**
     * @dev Enter a bid for a Rug
     */
    function enterBidForRug(uint _rugIndex) payable public nonReentrant {
        requireChecks(_rugIndex);
        require(INFTRugs(nftRugsContract).ownerOf(_rugIndex) != address(0), "No owner");
        require(msg.value != 0, "0 bid");

        Bid memory existing = rugBids[_rugIndex];
        ...
        rugBids[_rugIndex] = Bid(true, _rugIndex, msg.sender, msg.value);
        emit RugBidEntered(_rugIndex, msg.value, msg.sender);
    }

    /**
     * @dev Allows Rug owner to accept a bid
     */
    function acceptBidForRug(uint _rugIndex, uint _minPrice) public nonReentrant {
        requireChecks(_rugIndex);
        ...
        INFTRugs(nftRugsContract).safeRugTransferFrom(seller, bid.bidder, _rugIndex);

        rugsOfferedForSale[_rugIndex] = Offer(false, _rugIndex, bid.bidder, 0, address(0));
        ...
        rugBids[_rugIndex] = Bid(false, _rugIndex, address(0), 0);
        pendingWithdrawals[seller] += paymentAfterFeeRemoved;
        emit RugBought(_rugIndex, bid.value, seller, bid.bidder);
    }

    /**
     * @dev Withdraw a bid for a Rug
     */
    function withdrawBidForRug(uint _rugIndex) public nonReentrant {
        requireChecks(_rugIndex);
        require(INFTRugs(nftRugsContract).ownerOf(_rugIndex) != address(0), "No owner");
        ...
        emit RugBidWithdrawn(_rugIndex, bid.value, msg.sender);
        rugBids[_rugIndex] = Bid(false, _rugIndex, address(0), 0);
     ...
     */
    function requireChecks(uint256 _rugIndex) internal view {
        require(INFTRugs(nftRugsContract).totalSupply() == MAX_RUG_SUPPLY, "Sale over");
        require(INFTRugs(nftRugsContract).ownerOf(_rugIndex) == msg.sender, "Not owner");
        require(_rugIndex <= MAX_RUG_SUPPLY, "No rug");
    }
}
