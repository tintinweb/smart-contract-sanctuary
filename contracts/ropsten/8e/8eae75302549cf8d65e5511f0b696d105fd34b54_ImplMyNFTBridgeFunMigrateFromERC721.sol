...
contract ImplMyNFTBridgeFunMigrateFromERC721 is
    ImplMemoryStructure,
    MyNFTBridgeERC721toERC721Arrival
{
    ...
    function migrateFromIOUERC721ToERC721(
        ...
        try ERC721(_destinationWorld).ownerOf(_destinationTokenId) returns (
            address _currOwner
        ) {
            if (_currOwner == address(0x0)) {
                ...
                ERC721(_destinationWorld).safeTransferFrom(
                    address(0x0),
                    _destinationOwner,
                    _destinationTokenId
                );
    ...
    function emitIOU(
        ...
        emit MigrationArrivalRegisteredERC721IOU(
            _originUniverse,
            _originWorld,
            _originTokenId,
            _originOwner,
    ...
    function generateMigrationHashArtificialLocalIOUIncomingEx(
        bytes32 _originUniverse,
        bytes32 _originBridge,
        bytes32 _originWorld,
        bytes32 _originTokenId,
        ...
        return
            generateMigrationHashArtificial(
                ...
                bytes32(uint256(uint160(_destinationWorld))),
                bytes32(_destinationTokenId),
                bytes32(uint256(uint160(_destinationOwner))),
                bytes32(uint256(uint160(_signee))),
                _height
            );
    }
    ...
    bytes32 public constant MESSAGE_TYPEHASH =
        keccak256("HashStruct(bytes32 escrowHash)");

    /// TODO : Change to sign V4, see implementation in gasless cryptograph
    function msgSign(bytes32 escrowHash) external pure returns (bytes32) {
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    DOMAIN_SEPARATOR,
                    keccak256(
                        abi.encode(MESSAGE_TYPEHASH, hashStruct.escrowHash)
                    ) //The escrowHash emitted by the departure bridge is hashed with the current relay public address
                )
            );
    }
    ...
    function checkEscrowSignature(
        ...
        require(
            recoverSigner(message, _relayedMigrationHashSigned) == _signee,
            "The migration data signed by the signee do not match the inputed data"
        );
    }
    ...
    function recoverSignerEx(bytes32 message, bytes memory sig)
        external
        pure
        returns (address)
    {
