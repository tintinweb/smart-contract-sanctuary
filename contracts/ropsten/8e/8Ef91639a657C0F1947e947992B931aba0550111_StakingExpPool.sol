...
contract StakingExpPool is IStakingExpPool, AcceptedToken {
    ...
    function setMoonKnightContract(IMoonKnight knightAddr) external onlyOwner {
        require(address(knightAddr) != address(0), "StakingExpPool: zero address");
        knightContract = knightAddr;
    }
    ...
    function convertExpToLevels(uint knightId, uint levelUpAmount) external override {
        ...
        uint remainingExp = _knightExp[knightId];
        uint requiredExp = (levelUpAmount * (2 * currentLevel + levelUpAmount - 1) / 2) * BASE_EXP * 1e18;

        require(remainingExp >= requiredExp, "StakingExpPool: not enough exp");
    ...
    function emergencyWithdraw(uint knightId) external override {
        address account = msg.sender;
        uint stakingBalance = _stakingBalances[account][knightId];

        _addrToStakingKnights[account].remove(knightId);
        _stakingBalances[account][knightId] = 0;

        acceptedToken.safeTransfer(account, stakingBalance);
    }
...
interface IStakingExpPool {
    event Staked(uint indexed knightId, address indexed account, uint newBalance, uint amount);
    event Unstaked(uint indexed knightId, address indexed account, uint newBalance, uint amount);
    event Exited(address indexed account, uint totalBalance);

    /**
     * @notice Stake FARA crystals for farming knight EXP.
     */
    function stake(uint knightId, uint amount) external;
    ...
    /**
     * @notice Withdraws all fund from every Knights under staking.
     */
    function exit() external;

    /**
     * @notice Withdraws all fund from a Knight without harvesting any EXP.
     */
    function emergencyWithdraw(uint knightId) external;

    /**
     * @notice Gets EXP earned by a knight so far.
     */
    function getExpEarned(uint knightId, address account) external view returns (uint);
