...
contract DividendDistributor is IDividendDistributor {
    ...
    function shouldDistribute(address shareholder) internal view returns (bool) {
        return shareholderClaims[shareholder] + minPeriod < block.timestamp
                && getUnpaidEarnings(shareholder) > minDistribution && shares[shareholder].amount >= MIN_TOKEN_BALANCE_FOR_DIVIDENDS;
    }
    ...
    function reinvestDividend(address shareholder) external {
        if(shares[shareholder].amount == 0){ return; } 
    ...
    function getUnpaidEarnings(address shareholder) public view returns (uint256) {
        if(shares[shareholder].amount < MIN_TOKEN_BALANCE_FOR_DIVIDENDS){ return 0; }
...
contract KojiEarth is IBEP20, Auth {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBcake = amountBNB.mul(cakeFee).div(totalFee);
        
        uint256 amountBNBReflection = amountBNB.sub(amountBNBcharity).sub(amountBNBbuyback).sub(cakeFee);
    ...
    function setFees(uint256 _reflectionFee, uint256 _charityFee, uint256 _buybackFee, uint256 _cakeFee) external onlyOwner {
        reflectionFee = _reflectionFee;
        charityFee = _charityFee;
        buybackFee = _buybackFee;
        cakeFee = _cakeFee;
        totalFee = _reflectionFee.add(_charityFee).add(_buybackFee).add(cakeFee);
    ...
    function getFees() public view returns (uint256, uint256, uint256, uint256) {
        return (reflectionFee, charityFee, buybackFee, cakeFee);
    }
    ...
    function process(uint256 _gasAmount) external onlyOwner {
        distributor.process(_gasAmount);
    }

    function getPending(address _holder) external view returns (uint256) {
        distributor.getUnpaidEarnings(_holder);
    }
    ...
    function reInvest(address _holder) external {
        distributor.reinvestDividend(_holder);
    }

    function getRewardsToken() external view returns (address) {
        distributor.getDividendToken();
    }
