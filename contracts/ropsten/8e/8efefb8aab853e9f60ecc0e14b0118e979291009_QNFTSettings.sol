...
contract QNFTSettings is IQNFTSettings, ContextUpgradeable {
    ...
    // constants
    uint32 public constant NFT_SALE_DURATION = 1209600; // 2 weeks
    ...
    function initialize(address _settings) external initializer {
        __Context_init();
        qstkPrice = 0.000005 ether; // qstk price = 0.000005 ether
        nonTokenPriceMultiplier = 100; // non token price multiplier = 100%;
        tokenPriceMultiplier = 100; // token price multiplier = 100%;
        upgradePriceMultiplier = 1000; // upgrade price multiplier = 1000%;
        ...
        )
    {
        ...
        nonTokenPrice = (nonTokenPrice * nonTokenPriceMultiplier) / 100;
...
interface IQNFTSettings {
    ...
    function characterPrices(uint32 characterId)
        external
        view
        returns (uint256);
