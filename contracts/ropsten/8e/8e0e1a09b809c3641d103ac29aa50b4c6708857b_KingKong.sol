...
contract KingKong is Context, Ownable, IERC20 {
    ...
    uint256 private _total = 1000000000000 * 10**9;
    uint256 private maxTxAmount = 500000000000 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 345000 * 10**8 * 10**9;
    uint256 private minHoldingThreshold = 100 * 10**8 * 10**9;
    ...
    uint256 private BigApeFund = 0;
    uint256 private GorillaFund = 0;
    uint256 private ChimpFund = 0;
    ...
    function _transfer(address fromAddress, address toAddress, uint256 amount) private {
        ...
        // Add and remove wallet address from Ape eligibility
    ...
    // --- Section 4 --- : Ape functions. 
    // Off-chain bot calls payoutLargeRedistribution, payoutMediumRedistribution, payoutSmallRedistribution in order
    
    function startApe() public onlyOwner returns (bool success) {
        require (fund > 0, "fund too small");
        BigApeFund = fund.div(4);
        GorillaFund = fund.div(2);
        ChimpFund = fund.sub(BigApeFund).sub(GorillaFund);
        lastEntryAllowed = lastIndexUsed;
        
        emit StartApe(BigApeFund, GorillaFund, ChimpFund);
    ...
    function payoutMediumRedistribution() public onlyOwner() returns (bool success) {
        ...
        uint256 mediumPrize = GorillaFund;
    ...
    function payoutSmallRedistribution(uint256 startIndex) public onlyOwner() returns (bool success) {
        ...
        uint256 rewardAmount = ChimpFund.div(lastEntryAllowed);
        uint256 totalDisbursed = 0;
    ...
    function EndApe() public onlyOwner returns (bool success) {
