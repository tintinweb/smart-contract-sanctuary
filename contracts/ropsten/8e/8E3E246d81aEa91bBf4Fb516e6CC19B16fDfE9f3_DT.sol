...
contract DT is Context, Ownable, IERC20, LockableSwap, SwapWithLP, FeeLibrary, EthReflectingToken {
    ...
    function calculateBuy(address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        if(sniperDetection && !tradingOpen){ // Pre-launch snipers get nothing but a blacklisting
            ...
            // TODO - Clean this up
            _burn(amount.mul(buyFees.burn).div(100).mul(_getRate()));
    ...
    function calculateTransfer(address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        _takeLiquidity(tLiquidity);
        _burn(amount.mul(transferFees.burn).div(100).mul(_getRate()));
    ...
    function _transferStandard(address sender, address recipient, uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount) private {
        ...
        if(automatedReflectionsEnabled)
            try reflectionContract.logTransactionEvent(sender, _rOwned[sender], recipient, _rOwned[recipient]){} catch {}
    ...
    function _burn(uint256 amount) private {
        _rOwned[deadAddress] = _rOwned[deadAddress].add(amount);
...
abstract contract EthReflectingToken is Ownable, ISupportingExternalReflection {
    ...
    function takeEthReflection(uint256 amount) internal {
        if(amount > 0 && address(this).balance >= amount){
            ...
            if(automatedReflectionsEnabled){
                try reflectionContract.depositEth{value: amount}() {} catch {}
            } else {
    ...
    function reflectRewards() internal returns(bool complete) {
        complete = false;
        try reflectionContract.reflectRewards() { complete = true; } catch {}
    }
...
contract FeeLibrary {
    ...
    // TODO - Use this to simplify calculating fees?
