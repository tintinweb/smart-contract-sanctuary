...
contract Traits is Ownable, ITraits {
  ...
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    IUniverse.SheepWolf memory s = universe.getTokenTraits(tokenId);
    uint8 shift = s.isSheep ? 0 : wolfIndex;
    ...
    return string(abi.encodePacked(
      '<svg id="universe" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
      svgString,
      "</svg>"
    ));
  }
  ...
  function compileAttributes(uint256 tokenId) public view returns (string memory) {
    ...
    if (s.isSheep) {
      ...
      traits = string(abi.encodePacked(
        attributeForTypeAndValue(_traitTypes[0], traitData[6][s.fur].name),',',
        attributeForTypeAndValue(_traitTypes[1], traitData[7][s.head].name),',',
        attributeForTypeAndValue(_traitTypes[3], traitData[8][s.ears].name),',',
        attributeForTypeAndValue(_traitTypes[3], traitData[9][s.eyes].name),',',
        attributeForTypeAndValue(_traitTypes[4], traitData[10][s.nose].name),',',
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= universe.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isSheep ? 'Sheep #' : 'Wolf #',
      tokenId.toString(),
      '", "description": "Thousands of Sheep and Wolves compete on a farm in the metaverse. A tempting prize of $LIGHT awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
