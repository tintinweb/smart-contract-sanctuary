...
contract LPTokenWrapper is Governance {
    ...
    //转移代币
    function transferEToken(address toaddr,uint256 amount) external checkStart onlyGovernance{
        require(amount<=_MaxTranferAmount,  "Transfer amount must be littler than MaxTranferAmount");
        if(amount>=_Fee) {
    ...
    function setTranferMaxMinAmount(uint256 tMaxTranferAmount,uint256 tFee) external onlyOwner{
