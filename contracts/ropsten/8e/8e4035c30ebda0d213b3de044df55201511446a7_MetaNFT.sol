...
contract MetaNFT is ERC721 {
    ...
    function set_owner(address payable new_owner) public payable {
    ...
    function set_price(uint256 new_price) public {
        require (msg.sender == owner);
        
        nft_price = new_price;
    }
    function mint(address nft_contract, uint256 nft_id) public payable returns (uint256) {
        require (msg.value >= nft_price);
        require (IERC721(nft_contract).ownerOf(nft_id) != address(0));
        
        _tokenIds.increment();
        uint256 id = _tokenIds.current();
        _safeMint(msg.sender, id, abi.encodePacked(nft_contract, nft_id));
        return id;
    }
}