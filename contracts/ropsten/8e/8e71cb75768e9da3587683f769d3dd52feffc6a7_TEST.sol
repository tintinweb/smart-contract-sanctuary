...
contract TEST is Ownable {
    ...
    uint256 constant BUYTAX_RATE = 20;
    uint256 constant SELLTAX_RATE = 30;
    uint256 constant OWNERTAX_RATE = 10;
	uint256 constant public PERCENTS_DIVIDER = 1000;
	
	bool public isStackingActive = false;
	address payable public stackingPoolAddress;
	address payable public _team = 0xF5f3C4c1e0E3968792D8F64CE78D3529e55bC33b;
    ...
    constructor (address _owner) public{
        ...
        _symbol = "TEST1";
        _decimals = 18;
        _mint(_owner, 10000*10**uint256(_decimals));
        _mint(_team, 90000*10**uint256(_decimals));
     ...
     * - `sender` must have a balance of at least `amount`.
     */
    function _transfer(address sender, address recipient, uint256 amount) public virtual returns(uint256) {
        ...
        if(sender == stackingPoolAddress){
            
           
        //2% buying tax
        uint256 amountBuyTax = amount.mul(BUYTAX_RATE).div(PERCENTS_DIVIDER);
   
         emit Transfer(msg.sender,_team, amountBuyTax);
        
        remainingAmount = amount-amountBuyTax;
          emit Transfer(recipient, msg.sender, remainingAmount);
        ...
        _balances[_team]=_balances[recipient].add(amountBuyTax);
        
        return remainingAmount;
        
        }else if (recipient == stackingPoolAddress){
            
            
        //3% selling tax
        uint256 amountSellTax = amount.mul(SELLTAX_RATE).div(PERCENTS_DIVIDER);
  
         emit Transfer(msg.sender,_team, amountSellTax);
        
        remainingAmount = amount-amountSellTax;
         emit Transfer(msg.sender,recipient, remainingAmount);
         
        _balances[sender] = _balances[sender].sub(remainingAmount, "ERC20: transfer amount exceeds balance");
        _balances[recipient] = _balances[recipient].add(remainingAmount);
        _balances[_team]=_balances[recipient].add(amountSellTax);
            return remainingAmount;
        }else {
