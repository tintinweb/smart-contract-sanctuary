...
contract Oasis{
	...
	mapping(address => mapping(uint => uint)) public daysuserlucky;
	...
	function showlvzhou(address user) public view returns(
	    ...
	    total = userethnum[tags];//0
	    mykeyid = mykeysid[user];//1
	    mytzs = tzs[user];//2
	    daysget = usereths[user]*per/1000;//3
	    prizeget = prizecount[user];//4
	    //myeths = eths[user];//5
	...
	function showteam(address user) public view returns(
	    ...
	    daysnum1 = mysunsdaynum[user][t];//5
	    dayseth1 = myprizedayget[user][t];//6
	    man1 = mysuns[user].length;//2
	    man2 = mysecond[user].length;//3
	    man3 = mythird[user].length;//4
	    myruns = runs[user];//6
	...
	function showleveldetail(address user) public view returns(
	    ...
	    uint t = getyestoday();
	    uint d = gettoday();
	    daysnum = mysunsdaynum[user][t];//5
	    dayseth = myprizedayget[user][t];//6
	    uint levelid = getlevel(user);//7
	    uint mylevel = mylevelid[user];//8
