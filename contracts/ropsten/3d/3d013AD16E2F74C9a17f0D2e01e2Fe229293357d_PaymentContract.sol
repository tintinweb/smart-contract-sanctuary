...
contract PaymentContract is AccessControl {
    ...
    address[] private owners = [0x2569e7C63F9D9380DceE23A4a2a2aB2405428a68, 0x5c1C906e89b59569e020a84b7958014f210cA255, 0xe10E9a58B3139Fe0EE67EbF18C27D0C41aE0668C];
    address private marketing = 0x40Fe68980068909E98d204A152759251961CC788;
    uint256 private payedOut = 0;
    uint256 private totalBalance = 0;
    uint256 private totalPercent = 100;

    // mapping will allow us to create a relationship of investor to staking balance; investor: stakingBalance
    mapping( address => uint256 ) public _currentBalance;

    //create a collection of addresses that have staked 
    ...
    bool public hasReceivedPayment = false;

    uint256 public lastReceivedAmt = 0;
    ...
    function withdrawBalanceOwner() public {
        ...
        if(_currentBalance[msg.sender] > 0){
            ...
            if(hasRole(OWNERS, msg.sender)){
                ...
                payedOut = payedOut.add(amountToPay);

                withdrawee.send(amountToPay);
            }
        }
    ...
    function checkBalance() external returns (uint256 balance){
        uint256 balance = _currentBalance[msg.sender];
        return balance;

    }

    function checkTotalBalance() external returns (uint256 totalBalance) {
    ...
    function checkTotalPayouts() external returns (uint256 payouts) {
        if(hasRole(OWNERS, msg.sender) || hasRole(MARKETING, msg.sender)){
    ...
    function withdrawMarketing() public {
        if(hasRole(MARKETING, msg.sender)){
            ...
            if(amountToPay > 0){
                ...
                _currentBalance[msg.sender] = _currentBalance[msg.sender].sub(amountToPay);
                totalBalance = totalBalance.sub(amountToPay);
                payedOut = payedOut.add(amountToPay);
                address payable marketingPayable = payable(marketing);
                marketingPayable.send(amountToPay);
            }
        }
