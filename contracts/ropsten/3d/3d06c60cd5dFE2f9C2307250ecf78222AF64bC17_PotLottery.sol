...
contract PotLottery {
    ...
    event EnterPot(string indexed tokenName, uint256 amount);
    ...
    function adminEnterPot(
        ...
        emit EnterPot(_tokenName, _amount);
    }
    ...
    function removeAccumulatedFees(address _address) public onlyAdmin {
        for (uint256 index = 0; index < adminFeeToken.length; index++) {
            payAccount(
                adminFeeToken[index],
                _address,
