...
contract Property is ERC1155, ERC1155Holder, ERC1155Burnable {
  ...
  /// @dev Called when a sell order is issued (Value is in ETH)
  /// @notice The Value & Fee is in ETH
  event SellOrder(uint256 value, uint256 fees, uint256 tokenId);
  /// @dev Called when a buy order is issued
  /// @notice The Value & Fee is in the token
  event BuyOrder(uint256 value, uint256 fees, uint256 tokenId);
  ...
  function mintProperty(uint256 _shares, string memory _uri) public {
    ...
    totalSupply_[_id] = _shares;
    emit MintProperty(_id);
  }

  /// @dev Used to get the total supply of a token
  function totalSupply(uint256 _id) public view returns (uint256) {
    return totalSupply_[_id];
  }
  ...
  function purchaseTokens(uint256 _shares, uint256 _id) public payable {
    ...
    uint256 fee = getNativeFee(_id, _shares);
    emit BuyOrder(_shares, fee, _id);
    nativeFees_ = nativeFees_ + fee;
  ...
  function sellTokens(uint256 _shares, uint256 _id) public {
    ...
    uint256 fee = getTokenFee(_id, _shares);
    tokenFees_ = tokenFees_ + fee;
    ...
    emit SellOrder(transferAmount, fee, _id);
