...
library SafeMath {
  ...
  /**
    * @dev Умножает два числа, выбрасывает при переполнении.
    */
    function mul(uint256 a, uint256 b) internal pure returns (uint256 c) {
        // Оптимизация газа: это дешевле, чем утверждать, что 'a' не равно нулю, а
        // выгода теряется, если также проверяется 'b'.
   ...
   /**
    * @dev Целочисленное деление двух чисел с усечением частного.
    */
    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        // assert (b> 0); // Solidity автоматически выбрасывается при делении на 0
        // uint256 c = a / b;
   ...
   /**
    * @dev Вычитает два числа, вызывает переполнение (т.е. если вычитаемое больше, чем minuend).
    */
    ...
    /**
    *@dev Складывает два числа, выбрасывает при переполнении.
    */
...
/**
    Примечание: простой контракт для использования в качестве основы для константных значений
*/
...
/**
    Примечание. Идентификатор ERC-165 для этого интерфейса - 0x4e2312e0.
*/
interface ERC1155TokenReceiver {
    /**
         @notice Обработка получения одного типа токена ERC1155.
        @dev Смарт-контракт, совместимый с ERC1155, ДОЛЖЕН вызывать эту функцию в контракте получателя токена в конце safeTransferFrom после обновления баланса.
        Эта функция ДОЛЖНА возвращать `bytes4 (keccak256 (" onERC1155Received (address, address, uint256, uint256, bytes) "))` (т.е. 0xf23a6e61), если она принимает передачу.
        Эта функция ДОЛЖНА быть восстановлена, если она отклоняет передачу.
        Возврат любого другого значения, кроме предписанного значения, сгенерированного keccak256, ДОЛЖЕН привести к отмене транзакции вызывающей стороной.
        @param _operator Адрес, который инициировал передачу (например, msg.sender)
        @param _from Адрес, который ранее владел токеном
        @param _id Идентификатор передаваемого токена
        @param _value Количество переводимых токенов
        @param _data Дополнительные данные без указанного формата
        @return `bytes4 (keccak256 (" onERC1155Received (адрес, адрес, uint256, uint256, байты) "))`
    */
    function onERC1155Received(address _operator, address _from, uint256 _id, uint256 _value, bytes calldata _data) external returns(bytes4);

    /**
       @notice Обработка получения нескольких типов токенов ERC1155.
        @dev Смарт-контракт, совместимый с ERC1155, ДОЛЖЕН вызывать эту функцию в контракте получателя токена в конце safeBatchTransferFrom после обновления балансов.
        Эта функция ДОЛЖНА возвращать `bytes4 (keccak256 (" onERC1155BatchReceived (адрес, адрес, uint256 [], uint256 [], байты) "))` (т.е. 0xbc197c81), если она принимает передачу (и).
        Эта функция ДОЛЖНА быть восстановлена, если она отклоняет передачу (и).
        Возврат любого другого значения, кроме предписанного значения, сгенерированного keccak256, ДОЛЖЕН привести к отмене транзакции вызывающей стороной.
        @param _operator Адрес, который инициировал пакетную передачу (например, msg.sender)
        @param _from Адрес, который ранее владел токеном
        @param _ids Массив, содержащий идентификаторы каждого передаваемого токена (порядок и длина должны соответствовать массиву _values)
        @param _values Массив, содержащий количество каждого передаваемого токена (порядок и длина должны соответствовать массиву _ids)
        @param _data Дополнительные данные без указанного формата
        @return `bytes4 (keccak256 (" onERC1155BatchReceived (адрес, адрес, uint256 [], uint256 [], байты) "))`
    */
