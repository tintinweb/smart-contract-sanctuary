...
contract DragonKing is Destructible {
  ...
  uint16 public constant maxCharacters = 600;
  ...
  uint8 public constant MAX_CHARACTER_TYPE = 27;
  ...
  uint64 public constant eruptionThreshold = 28800;
  ...
  uint8 public constant percentageToKill = 10;
  ...
  function DragonKing(address teleportTokenAddress,
                      ...
                      address xperTokenAddress) public onlyOwner {
    nextId = 1;
  ...
  function exit() public {
    ...
    for (uint16 i = 0; i < nchars; i++) {
      if (characters[ids[i]].owner == msg.sender 
          ...
          && (characters[ids[i]].characterType < BALLOON_MIN_TYPE 
          || characters[ids[i]].characterType > BALLOON_MAX_TYPE)) {
  ...
  function fight(uint32 characterID, uint16 characterIndex) public {
    ...
    require(cooldown[characterID] + CooldownThreshold <= now);
    ...
    assert(adversaryIndex < maxCharacters);
    ...
    if (character.value == adversary.value) {
      ...
      if (adversaryPower > characterPower) {
        base_probability -= uint16((100 * adversary.value) / character.value / fightFactor);
      }
    } else if (characterPower > adversaryPower) {
        base_probability += uint16((100 * character.value) / adversary.value / fightFactor);
    }
    ...
    if (dice >= base_probability) {
      ...
      NewFight(adversaryID, characterID, value, base_probability, dice);
    } else {
      // character won
      ...
      NewFight(characterID, adversaryID, value, base_probability, dice);
    }
  ...
  */
  ...
  function sellCharacter(uint32 characterId) public {
    ...
    require(characters[characterId].characterType < BALLOON_MIN_TYPE 
    || characters[characterId].characterType > BALLOON_MAX_TYPE);
  ...
  function receiveApproval(address sender, uint256 value, address tokenContract, bytes callData) public {
    ...
    if (msg.sender == address(teleportToken)) {
      ...
      if(cType <= KNIGHT_MAX_TYPE || (cType >= ARCHER_MIN_TYPE && cType <= ARCHER_MAX_TYPE)){
        lifePrice = ((cType % 6) + 1) * protectionPrice;
        max = 3;
      } else if (cType >= BALLOON_MIN_TYPE && cType <= BALLOON_MAX_TYPE) {
        lifePrice = (((cType+3) % 6) + 1) * protectionPrice * 2;
        max = 6;
  ...
  function teleportKnight(uint32 id) internal {
    ...
    require(knight.characterType >= KNIGHT_MIN_TYPE); //this also makes calls with non-existent ids fail
  ...
  function getNumDragons() constant public returns(uint16 numDragons) {
    for (uint8 i = 0; i < DRAGON_MAX_TYPE; i++)
