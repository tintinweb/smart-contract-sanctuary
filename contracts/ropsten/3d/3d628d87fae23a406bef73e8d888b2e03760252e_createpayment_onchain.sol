...
contract createpayment_onchain {  
  ...
  modifier isReiceiver(uint _id){
    require(msg.sender==payments[_id].receiver);
    _;
  } 
  modifier notcanceled (uint _id){
    require(payments[_id]. iscanceled== false);
    _;
  } 
  ...
  function createpayment (string memory _name,uint256 _total_am,string[] memory _payer_list) public returns (uint) {
    uint _id=counter;
  ...
  function cancelpayment(uint _id) public isReiceiver(_id)  returns(bool){
    require(payments[_id].isfinished==false,"only ongoing payment can be canceled");
  ...
  function pay(uint _id, string memory _payer)payable public checkValue(_id)   checkfinished (_id) notcanceled(_id) returns(bool){
    ...
    require(ispayer==true,"the payer name should have in payer list");
    ...
    require (paid ==false,"should not pay twice");
    uint256 price=payments[_id].total_am/payments[_id].payer_list.length;
    require(msg.value>=price,"should pay enough money");
    payments[_id].paid_payer_list.push(_payer);
    payments[_id].received_am=payments[_id].received_am+price;
    payments[_id].balance=payments[_id].balance+price;
    emit Logpay(_id,_payer,msg.sender,price,payments[_id].received_am);
    return true ;
  }

  function withdraw(uint _id,uint256 _withdraw_amount)public isReiceiver(_id)  {
    require(payments[_id].balance>=_withdraw_amount,"should have enough balance");
  ...
  function fetch_payment (uint _id) public view returns(string memory name,address receiver,uint256  total_am,string[] memory payer_list,string[] memory paid_payer_list,uint256 received_am,uint256 balance,bool iscanceled,bool isfinished){
    ...
    return (name,receiver,total_am,payer_list,paid_payer_list,received_am,balance,iscanceled,isfinished);
  }
