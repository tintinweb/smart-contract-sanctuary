...
{   
    ...
    //Sends GoldenTicket & Whitelists Address to receive NFT
    function _whitelistGT(uint256 tokenID) public nonReentrant
    {
        require(IERC721(_GoldenTicketAddress).ownerOf(tokenID) == msg.sender);
        IERC721(_GoldenTicketAddress).safeTransferFrom(msg.sender, _BRTMULTISIG, tokenID);
