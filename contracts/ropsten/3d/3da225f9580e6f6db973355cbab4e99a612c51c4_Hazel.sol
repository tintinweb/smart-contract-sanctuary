...
contract Hazel is ERC721Enumerable, ReentrancyGuard, Ownable{
	...
	// Token ID 8,001 - 18993
	function mintWithMoreLoot(uint256 mlootId, uint256[] calldata traits) public payable nonReentrant {
		require(isSaleActive, "SALE_NOT_ACTIVE");
		require(_countTokenMLoot.current() < _supplyMLoot - _supplyWhitelist, "SOLD_OUT");
		require(mlootId > 8000, "TOKEN_ID_OUT_OF_RANGE");
		require(mlootContract.ownerOf(mlootId) == msg.sender, "MUST_OWN_TOKEN_ID");
	...
	function mintGiveaway(uint256 lootId, uint256[] calldata traits) public nonReentrant onlyOwner {
		require(isSaleActive, "SALE_NOT_ACTIVE");
