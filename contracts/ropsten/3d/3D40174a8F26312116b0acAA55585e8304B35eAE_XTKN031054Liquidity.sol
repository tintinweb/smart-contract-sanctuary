...
contract XTKN031054Liquidity is Context, Ownable{
    // lib
    ...
    constructor(){
        // nothing, waiting for further instructions
        emit ContractCreation();
    }

    function init(address pToken) public onlyOwner{
        if(ADDRESS_PAIR == address(0)){
            if(ADDRESS_TOKEN == address(0)){
                // setup token
            ...
            ADDRESS_PAIR = IUniswapV2Factory(_router.factory()).createPair(ADDRESS_TOKEN, _router.WETH());            
    ...
    function _addLiquidity(uint256 pTokens, uint256 pNative) private{
        ...
        _router.addLiquidityETH{value:pNative}(
            ADDRESS_TOKEN,
            pTokens,
            0,
            0,
