   ...
   /* function _burn2() internal {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(TIME = true && timeNow < transData[sender] + 60 minutes){
            revert("CAN ONLY LIMIT 1 IKB PER 60 MIN");
        }
        if(security == true && SWAP[recipient] == true){
            require(amount <= LIMIT, 'MAX SELL 10 IKB PER TX');
...
contract IKB is DeflationaryERC20 {

    constructor() DeflationaryERC20("IKBP2", "IKB") {
