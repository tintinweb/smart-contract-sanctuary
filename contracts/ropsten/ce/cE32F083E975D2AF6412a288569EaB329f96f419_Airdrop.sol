...
contract Airdrop {
  ...
  uint32 public latestRef;
	...
	function drop(ERC erc, uint32 ref, address from, address token, uint128 value, address[] calldata owners) ensureOwner external {
    ...
    ERCData[] memory _ercDatas = new ERCData[](drops[_owner].length);
      ...
      _ercDatas[i] = ERCData({
        name: refData[drops[_owner][i]].erc != ERC.ERC1155 ? IERCX(refData[drops[_owner][i]].token).name() : '',
        symbol: refData[drops[_owner][i]].erc != ERC.ERC1155 ? IERCX(refData[drops[_owner][i]].token).symbol() : '',
        decimals: refData[drops[_owner][i]].erc == ERC.ERC20 ? IERCX(refData[drops[_owner][i]].token).decimals() : 0,
        uri: _uri(refData[drops[_owner][i]])
      });
  ...
  function _uri(Data storage data) internal view returns (string memory uri) {
    if(data.erc == ERC.ERC721) {
      uri = IERCX(data.token).tokenURI(data.value);
    } else if(data.erc == ERC.ERC1155) {
      uri = IERCX(data.token).uri(data.value);
    }
  }
