...
contract Atom is ERC721Enumerable, Pausable, Ownable {
    ...
    function setWithdrawAddress(address _originAddress) public onlyOwner {
        require(_originAddress != address(0), "Cannot be null address");
        originAddress = _originAddress;
    }

    function withdrawAll() public onlyOwner {
        require(originAddress != address(0) , "Withdrawal address not set");
        uint256 balance = address(this).balance;
        require(balance > 0, "Insufficent balance");
        _widthdraw(originAddress, address(this).balance);
    }

    function _widthdraw(address _address, uint256 _amount) private {
        require(payable(_address).send(_amount), "Failed to widthdraw Ether");
    }
    ...
    /**
     * @dev In case price needs to re-adjust due to eth unpredictable change
     */
    ...
    /**
     * @dev In case number of reserved tokens needs to be adjusted
     */
    function setReservedCount(uint256 _newReservedCount) public onlyOwner() {
        require(reservedClaimed < _newReservedCount, "Already claimed more reserved tokens");
        RESERVED_TOKENS = _newReservedCount;
    }
