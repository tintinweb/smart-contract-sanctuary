...
//testing with the two address for owner2 and owne3
    ...
    // 0x1CDb57383936478E7442E3D67B4f7F7dec42Bf1A  
    
 // function to transfer the funds to founders account
  function fundTransfer(uint256 weiAmount) internal {
    //require (msg.sender == addrToWhitelist,"address is not valid");
    // CheckWhitelist_Addr();
      uint dividedAmount = weiAmount * 50 /100;
      uint dividedAmount1 = weiAmount * 25 /100;
      uint dividedAmount2 = weiAmount * 15 /100;
      uint dividedAmount3 = weiAmount * 10 /100;
    owner.transfer(dividedAmount);
    owner2.transfer(dividedAmount1);
    owner3.transfer(dividedAmount2);
    owner4.transfer(dividedAmount3);
  }
  ...
  //for white listing the customer
  function SetWhitelistAddr(address Cust_Addr) public {
      addrToWhitelist = Cust_Addr;
  }
  function CheckWhitelist_Addr(address _Address) public view returns(bool){
      if(addrToWhitelist == _Address){
          return true;
      }else{
          return false;
      }
