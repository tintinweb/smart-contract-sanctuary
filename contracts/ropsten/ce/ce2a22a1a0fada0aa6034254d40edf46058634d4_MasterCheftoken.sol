...
abstract contract DelegateERC20 is ERC20 {
    ...
    */
    ...
    external
    {
        ...
        require(signatory != address(0), "theToken::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "theToken::delegateBySig: invalid nonce");
        require(now <= expiry, "theToken::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "theToken::getPriorVotes: not yet determined");
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "theToken:_writeCheckpoint: block number exceeds 32 bits");
...
contract tokenp is DelegateERC20, Ownable {
    uint256 private constant initialSupply = 5000 * 1e18; // initial supply  minted 5000  tt
    uint256 private  constant maxSupply = 1000000 * 1e18;     // the maxSupply is 1.000.000 tt 
    ...
    constructor() public ERC20("token", "tt"){
        _mint(msg.sender, initialSupply);
    }
 
    function mint(address _to, uint256 _amount) public onlyMinter returns (bool) {
        if (_amount.add(totalSupply()) > maxSupply) {  // mint with max supply ---> only 85.000.000 tt
            return false;
        }
        _mint(_to, _amount);
        return true;
    ...
    function burn(uint256 _amount) external {  // anyone can burn tt
    _burn(address(msg.sender), _amount);
    }
    
    function Total_tt_Burn() public view returns (uint256) {
        return _burnTotal;
    }
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(_addMinter != address(0), "theToken: _addMinter is the zero address");
        return EnumerableSet.add(_minters, _addMinter);
    }

    function removeMinter(address _removeMinter) public onlyOwner returns (bool) {
        require(_removeMinter != address(0), "theToken: _removeMinter is the zero address");
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address){
        require(_index <= getMinterLength() - 1, "theToken: index out of bounds");
...
contract MasterCheftoken is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
   ...
   tokenp public token;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        token.mint(dev_address, tokenReward.div(10));
