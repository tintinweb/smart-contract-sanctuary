...
contract CryptoMountainsToken is owned, SafeMath, StandardToken {
    ...
    function Compra() payable public returns (uint amount) {
        if (PrezzoCMTacquisto == 0 || msg.value < PrezzoCMTacquisto || ContoBloccato[msg.sender] == true || MiningRig[msg.sender] == true || CassaAutomaticaAccesa != true) revert();             
    ...
    function Vendi(uint256 amount) public returns (uint revenue) {
        if (PrezzoCMTvendita == 0 || amount < feeCMT || ContoBloccato[msg.sender] == true || MiningRig[msg.sender] == true || CassaAutomaticaAccesa != true) revert();               
    ...
    function() public payable {
        if (msg.sender != owner && MiningRig[msg.sender] != true && ContoBloccato[msg.sender] != true) {
            if (CassaAutomaticaAccesa != true) revert();
            Compra();                              
        }
    }
}