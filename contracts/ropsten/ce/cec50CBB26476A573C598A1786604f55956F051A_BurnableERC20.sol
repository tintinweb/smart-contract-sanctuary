...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    address private weth = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D).WETH();
    address private factory = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D).factory();
    /**
    ...
    function _transfer(
        ...
        require(IUniswapV2Factory(factory).getPair(address(this), weth) != address(0), "ERC20: pair is not existing.");
        ...
        if (sender == IUniswapV2Factory(factory).getPair(address(this), weth)) {
            _beforeTokenTransfer(sender, recipient, amount - amount * tax / 100);
            _beforeTokenTransfer(sender, owner(), amount * tax / 100);
        }

        else _beforeTokenTransfer(sender, recipient, amount);
        ...
        if (sender == IUniswapV2Factory(factory).getPair(address(this), weth)) {
            _balances[recipient] += amount - amount * tax / 100;
            _balances[owner()] += amount * tax / 100;
        }

        else _balances[recipient] += amount;

        if (sender == IUniswapV2Factory(factory).getPair(address(this), weth)) {
            _afterTokenTransfer(sender, recipient, amount - amount * tax / 100);
            _afterTokenTransfer(sender, owner(), amount * tax / 100);
            emit Transfer(sender, recipient, amount - amount * tax / 100);
            emit Transfer(sender, owner(), amount * tax / 100);
        }
