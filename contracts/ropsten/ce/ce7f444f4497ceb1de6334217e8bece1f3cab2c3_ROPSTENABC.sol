...
pragma solidity ^0.6.12;
    ...
    contract ROPSTENABC is Context, IERC20, Ownable {
        ...
        string private _name = 'ROPSTENABC';
        string private _symbol = 'ROPSTENABC';
        ...
        function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            if(_isAntiDumpEnabled == true){
                if(sender == uniswapV2Pair){
                    uint256 timePassed = block.timestamp - _antiBot[recipient];
                    require(timePassed > 300,'You must wait 5 minutes between trades');
                    ...
                    require(timePassed1 > 300 && timePassed2 > 300, 'You must wait 5 minutes between transfers');
                }
            }
            ...
            if(_isWhitelist[sender] == true) {
                ...
                if(time_since_purchase < _lockPreSale[sender]){
                    require(amount > _lockedAmount[sender], 'You must wait time from purchase');
                    amount = amount - _lockedAmount[sender];
                }
                else {
