...
contract NiftyCollection is Ownable, AccessControl, ERC721Full, IERC721Receiver {
    /// @dev Byte32 object to store MINTER_ROLE keccak256
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    string public description;

    /// @dev Struct to store creator and royaltyPercentage of Token
    struct TokenRoyaltyData {
        address creator;
        uint256 royaltyPercent;
    }

    /// @dev maps tokenId with its creator and royalty percentage
    mapping(uint256 => TokenRoyaltyData) public royaltyDataOf;

    /// @dev create a ERC721Full collection contract
    /// @param name defines name of the collection
    /// @param symbol defines symbol of collection
    /// @param storeDescription description of store
    constructor(
        string memory name,
    ...
    /// @notice Mint tokens of a collection
    /// @dev Mints `tokenId` and transfers it to `minter` of contract
    /// @param tokenId is a integer which determine id of token
    /// @param tokenURI is metadata which contain actual location of asset
    /// @param creatorAddress is an address which is the `owner` of the asset
    /// @param royaltyPercent is a integer denotes percent of royalty is transferred to `creator`
    function mint(
        uint256 tokenId,
        ...
        royaltyDataOf[tokenId].creator = creatorAddress;
        royaltyDataOf[tokenId].royaltyPercent = royaltyPercent;
