...
contract FroyoCat is ERC721, Ownable, Pausable {
    struct Cat {
        address owner;
        bool claimed;
        uint256 time;
    ...
    constructor(FroyoCatMachine _froyoCatMachine)
        ERC721("FroyoCat", "FroyoCat")
    {
    ...
    function adminMint(address[] calldata _toAddresses, string[] calldata _urls)
        external
        onlyOwner
        whenNotPaused
    {
        require(_toAddresses.length == _urls.length, "Invalid input");
        for (uint256 i = 0; i < _toAddresses.length; i++) {
            require(_toAddresses[i] != address(0), "Address invalid");
            uint256 _nftId = totalSupply();

            _mint(address(froyoCatMachine), _nftId);
            _setTokenURI(_nftId, _urls[i]);
