...
contract PreSale is ERC20, Ownable {
    ...
    IERC20Metadata public TOKEN = IERC20Metadata(address(0));
    ...
    uint256 public RATE = TOKEN_AMOUNT.mul(10**10).div(MIM_AMOUNT);
    uint256 public MAX_CAP = 2000*10**18;
    ...
    function getTokenToMint(uint mimAmount) view internal returns (uint amount) {
        return mimAmount.mul(RATE).div(10**10);
    }

    function presale(uint256 mimAmount) external {
        require(presaleActive, "presale is not active");
        require(caps[msg.sender] + mimAmount <= MAX_CAP, "MIM Amount exceed");
    ...
    function setRedeemEnabled(bool enabled, address token) external onlyOwner() {
        redeemEnabled = enabled;
        TOKEN = IERC20Metadata(token);
    }

    function withdrawMIM(uint256 amount) external onlyOwner() {
