...
contract TokenMapped is ContextUpgradeSafe, AuthQuota {
    ...
    bytes32 public constant WITHDRAW_TYPEHASH = keccak256("Withdraw(uint256 fromChainId,address to,uint256 index,uint256 volume,address authorizer)");
    ...
    function deposit(uint toChainId, address to, uint volume) virtual external {
        IERC20(token).safeTransferFrom(_msgSender(), address(this), volume);
        emit Deposit(_msgSender(), toChainId, to, volume);
    }
    event Deposit(address indexed from, uint indexed toChainId, address indexed to, uint volume);
...
contract MappableTokenTest /*is ERC20UpgradeSafe, AuthQuota, IPermit*/ {
    function recv(Signature[] memory signatures) virtual external {
