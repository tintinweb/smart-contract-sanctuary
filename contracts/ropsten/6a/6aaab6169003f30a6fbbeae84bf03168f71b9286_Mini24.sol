...
Contract by DeFiSCI - built on others previous work w/ a splash of DeFiSCI magic...
...
contract Mini24 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x519B269Ee2aCc6C1a999291016e40a14Bde0C6dE); // Marketing Address
    ...
    string private _name = "Mini24";
    string private _symbol = "MINI24";
    ...
    uint256 private buyBackSellLimit = 1;
    uint256 public _buyBackBuyLimit = 1;
    ...
    bool public buyBackBuyEnabled = true;

    
    event RewardLiquidityProviders(uint256 tokenAmount);
    event BuyBackEnabledUpdated(bool enabled);
    event BuyBackBuyEnabledUpdated(bool enabled);
    ...
    constructor () {
       ...
       // transferOwnership(0xD0F5EE2A747b429360b53A95aDAb7bF71D1fA843);
    ...
    function buyBackBuyLimitAmount() public view returns (uint256) {
        return _buyBackBuyLimit;
    }
    ...
    function _transfer(
        ...
        //Buy back buy and sell limit to wei
        uint256 _bBBLimit = _buyBackBuyLimit.mul(_ether).div(100);
        uint256 _bBSLimit = buyBackSellLimit.mul(_ether).div(100);

        //buy
        if (buyBackBuyEnabled && balance > _bBBLimit && from == uniswapV2Pair) {
            buyBackTokens(_bBBLimit);
        }
        else{
            //sell
            if (buyBackEnabled && balance > _bBSLimit && to == uniswapV2Pair) {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            // if send account have special fee 
            if(_addressFees[from].enable){
                ...
                //buy
                if(from == uniswapV2Pair){
                    _taxFee = _addressFees[from]._buyTaxFee;
                    _liquidityFee = _addressFees[from]._buyLiquidityFee;
                }
                //sell
                if(to == uniswapV2Pair){
    ...
    function setBuyBackBuyLimit(uint256 buyBackBuyLimit) external onlyOwner {
        _buyBackBuyLimit = buyBackBuyLimit;
    }
    ...
    function setBuyBackBuyEnabled(bool _enabled) public onlyOwner {
        buyBackBuyEnabled = _enabled;
        emit BuyBackBuyEnabledUpdated(_enabled);
    }
