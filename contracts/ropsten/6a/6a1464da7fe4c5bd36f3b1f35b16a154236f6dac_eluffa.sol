...
contract eluffa {
    ...
    uint8 public tokendebit; // If disable its wont debit
    ...
    constructor() public {
        ...
        min_withdrawal = 100000000000000000000;
    ...
    function initDeposit() external  {
        ...
        uint _amount = tc.allowance(msg.sender, address(this));
    ...
    function withdraw() external payable {
        ...
        tc.transfer(
            msg.sender,
            users[msg.sender].profitpayout.div(100).mul(95)
        );
        tc.transfer(
            companyaddress,
            users[msg.sender].profitpayout.div(100).mul(5)
        );

        users[msg.sender].total_withdrawl = users[msg.sender]
            .total_withdrawl
            .add(users[msg.sender].profitpayout);

        total_withdraw = total_withdraw.add(users[msg.sender].profitpayout);

        current_profit = current_profit.sub(users[msg.sender].profitpayout);
        ...
        users[msg.sender].profitpayout = 0;
    }

    function investStaking(uint256 amount) external payable {
    ...
    function payMerchant(address _addr, uint256 _amount) external payable {
