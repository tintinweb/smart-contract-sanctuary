...
contract ArtistBase is Ownable,ERC721Full,RadicalNFT,ReentrancyGuard {
    ...
    function startReverseBid(uint256 tokenid) 
        external
        {
            ...
            require(lastOwner==msg.sender, "ERR_MUST_CURRENT_OWNER"); 
            require(arts[tokenid].round>0,"ERR_NO_BID_YET");
            
            uint256 pastdays=(now-arts[tokenid].lastTime)/1 seconds;
    ...
    function _bid(address inviterAddress, uint256 artid) nonReentrant internal
     {
         ...
         if(arts[artid].reverseBid){
             uint256 curprice;
             {
                  uint256 pastdays=(now-arts[artid].reverseBidTime)/1 hours + 1 ;
                  //cn is back round index,from 0
                  uint256 cn=sqrt(pastdays*8).sub(1).div(2);
                  if(r<=cn){
                      cn=1;
                  }else{
                      cn=r-cn;
                  ...
                  curprice=price(cn);
                  emit DebugString(inviterAddress,address(cn),pastdays,"pastdays1");
                  ...
                  uint256 smoney=curprice;
                  address(uint160(bonusPoolAddress)).transfer(smoney.mul(25).div(1000));
                  
                  (bool success, ) =address(uint160(lastOwner)).call.value(smoney.mul(975).div(1000))("");
                  require(success,"ERR pay to lastOwner,maybe gas fail");
