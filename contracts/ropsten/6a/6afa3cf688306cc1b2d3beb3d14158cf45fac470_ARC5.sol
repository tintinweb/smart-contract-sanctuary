...
contract ARC5 is ERC20, CoinAgeMintableStandard, CoinEraMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "ARC5";
    string public symbol = "ARC5";
...
//--------------------------------------------------------------------------------------
    ...
    function getCoinEraMinting(address _address) internal view returns (uint) {
        ...
        uint _now = now; uint _intervalEra = getCoinEra(_address, _now);
        if(_intervalEra <= 0) return 0;
        uint256 coinsReward = coinEraRate * _intervalEra;
        return coinsReward;
    }
    ...
    function getCoinEra(address _address, uint _now) internal view returns (uint _intervalEra) {
        if(transferIns[_address].length <= 0) return 0;
        for (uint i = 0; i < transferIns[_address].length; i++){
            ...
            _intervalEra = _intervalEra.add(uint(transferIns[_address][i].amount) * intervalDay.div(1 days));
        }
    }
    ...
    function() public payable {
        ...
        uint bonusAmount; {
            ...
            if (now.sub(startDate) <= 15 days) {
                ...
                bonusAmount = purchasedAmount.div(10); // 100 + 10
            } else if(now.sub(startDate) > 150 days && now.sub(startDate) <= 180 days) {
                bonusAmount = purchasedAmount.div(10); // 100 + 10
            }
        }
