...
contract IndonesiaCitizen {
    ...
    modifier onlyAdminister() {
        bool isAdminister = administers[msg.sender];
        require(isAdminister == true, 'This method can only be accessed by the Administer.');
        _;
    }
    ...
    function assignAdminister(address _administerAddress) public
    onlyProprietor
    returns(bool isAssigned) {
        administers[_administerAddress] = true;

        return administers[_administerAddress];
    }

    function revokeAdminister(address _administerAddress) public
    onlyProprietor
    returns(bool isAssigned) {
        administers[_administerAddress] = false;
