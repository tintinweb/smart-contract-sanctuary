...
contract ClaimingRegistry is IClaimingRegistry, Initializable, AbstractDependant {
    ...
    uint256 internal constant ANONYMOUS_VOTING_DURATION_CONTRACT = 600;
    uint256 internal constant ANONYMOUS_VOTING_DURATION_EXCHANGE = 900;

    uint256 internal constant EXPOSE_VOTE_DURATION = 600;
    uint256 internal constant PRIVATE_CLAIM_DURATION = 300;
    ...
    function submitClaim(
         ...
         */
        bool canSubmitNewClaim = !appeal && active && status == ClaimStatus.CAN_CLAIM;

        bool canSubmitNewClaimOrAppeal =
            canSubmitNewClaim || (appeal && status == ClaimStatus.REJECTED_CAN_APPEAL);

        bool canSubmitClaimOrAppealOrOneIsExpiredOrNewPolicyIsBought =
            canSubmitNewClaimOrAppeal ||
                (!appeal &&
                    active &&
        ...
        require(
            canSubmitClaimOrAppealOrOneIsExpiredOrNewPolicyIsBought,
