...
contract PooInu is Context, IERC20, Ownable {
    ...
    string private constant _name = "PooInu";
    ...
    uint256 private _minBuyAmount = 0;
    ...
    constructor () {
        _devWallet = payable(0x74Eb2d8a86a344465e6A2a7C1BF1a042dFB7D171);
        _ATHWallet = payable(0x74Eb2d8a86a344465e6A2a7C1BF1a042dFB7D171);
        _rOwned[_msgSender()] = _rTotal;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_devWallet] = true;
        _isExcludedFromFee[_ATHWallet] = true;
		_ATHPrice = 0;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            } else if (to == uniswapV2Pair && from != address(uniswapV2Router) && ! _isExcludedFromFee[to]) {
				// Selling
				require(amount >= _minBuyAmount, "Minimum Buy Amount Not Reached");
				require(cooldown[from] < block.timestamp, "Selling before cooldown expiry");
				
				uint256 currentPrice = _getPrice();
				if (currentPrice > _ATHPrice) {
					// new ATH has been reached 
					_ATHWallet = payable(from);
    ...
    function openTrading() external onlyOwner() {
        ...
        _minBuyAmount = 25000000000000000; //0.025 eth
        tradingOpen = true;
	...
	function _getPrice () private view returns (uint) {
	  (uint left, uint right,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
	  (uint tokenReserves, uint ethReserves) = (address(this) < WETH) ?
