...
contract AutoInvest is Ownable {
    ...
    function deposit(uint256 _amount) external {
        ...
        if (investors.inserted[msg.sender]) {
            ...
            investors.set(msg.sender, _amount + oldAmount);
        } else {
    ...
    function getMyRewardAmount(address _address) public view returns (uint256) {
        ...
        if (totalStaked == 0) { // if the contract has not yet staked anything
            ...
            uint256 stakeShare = investors.get(_address) * 10**18 / totalStaked;
    ...
    //     require(investors.inserted[msg.sender]); // Not participated in staking
