...
contract NftBattleArena is Ownable, ERC721
{
	...
	function pairNft(uint256 stakingPositionId) external
	{
		...
		require(numberOfNftsWithNonZeroVotes / 2 > nftsInGame / 2, "incorrect pair"); // Requires enough nft for pairing.
	...
	function voteWithZoo(uint256 votingPositionId, uint256 amount) public returns (uint256 votes)
	{
		...
		// require(typeOfPositions[votingPositionId] == PositionType.VoterPosition, "Wrong position type"); // Requires correct position type.
	...
	{
		require(getCurrentStage() == Stage.FirstStage, "Wrong stage!");             // Requires to be at first stage.
		require(ownerOf(votingPositionId) == msg.sender && typeOfPositions[votingPositionId] == PositionType.VoterPosition);               // Requires to be owner of position.
	...
	function recomputeZooVotes(uint256 votingPositionId) external
	{
		...
		require(newZooVotes > oldZooVotes && newZooVotes <= votingPositions[votingPositionId].daiInvested);// Requires for new votes amount to be bigger than before and less than votes from dai.
