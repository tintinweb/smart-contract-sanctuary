...
contract OffchainRolling {
    ...
    //    require(msg.sender == address(this));
        ...
        // replay attacks btfo 
        txCount[Gambler] = txCount[Gambler]+1; //5k 
        // tx.origin is an allowed notary 
    ...
    function roll_normal(address Gambler, uint Nonce,uint Value,bytes32 UserSeed,bytes32 ServerSeed, bytes Recover, bytes32 unHashedSeed, uint Rolls) public {
        uint gas = gasleft();
        /*bool completed = address(this).call(
            abi.encodeWithSignature("_roll(address,uint256,uint256,bytes32,bytes32,bytes,bytes32,uint256)", 
            Gambler, 
            Nonce, 
            Value, 
            UserSeed, 
