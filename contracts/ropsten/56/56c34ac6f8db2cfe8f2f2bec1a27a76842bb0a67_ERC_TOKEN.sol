...
contract ERC_TOKEN is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function _transfer(address from, address to, uint256 amount) private returns(bool) {
        if(_accountNoTaxes[from] || _accountNoTaxes[to]){
            ...
            amount = amount.sub(amountTaxLiquidity).sub(amountTaxLottery).sub(amountTaxRedistribution).sub(amountTaxBurn);

            _transferBurn(from, amountTaxBurn);
            emit TransferWithTaxes(from, to, amountTaxLiquidity, amountTaxLottery, amountTaxRedistribution, amountTaxBurn);
        }
    ...
    function _uniswapSwapAndLiquidity(uint256 contractTokenBalance) private swapLocking {
        uint256 half = contractTokenBalance.div(2);
        uint256 otherHalf = contractTokenBalance.sub(half);
        uint256 initialBalance = address(this).balance;
        _uniswapSwapTokensForETH(half);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        _uniswapAddLiquidity(otherHalf, newBalance);
    }

    function _uniswapSwapTokensForETH(uint256 tokenAmount) private {
    ...
    function _uniswapAddLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
