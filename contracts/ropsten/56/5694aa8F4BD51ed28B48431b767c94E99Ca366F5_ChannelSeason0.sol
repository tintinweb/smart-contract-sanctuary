...
contract ChannelSeason0 is ERC721, Ownable {
  ...
  function updateRolfes(address payable _ROFLES) public onlyOwner {
    rolfes = _ROFLES;
  }
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    return string(abi.encodePacked(baseURI, Strings.toString(tokenId), extension));
  }

  function _mintRequires(uint256 numToMint) private {
    require(numToMint <= MAX_MINT);
    require(
      ((tokenIds.current() - 1) + numToMint + (TOTAL_DEV_MINTS - devMintCount.current())) <
        TOTAL_SUPPLY,
      'max supply reached'
    ); // - 1 because tokenIdTracker.current starts at 1
  }

  function _payableMint(address receiver, uint256 numToMint) private {
    require(msg.value >= PRICE * numToMint, 'must pay mint fee for each');
    uint256 multisigFee = msg.value - ROYALTY_PRICE;
    rolfes.call{value: ROYALTY_PRICE}('');
    multisig.call{value: multisigFee}('');
    for (uint256 i = 0; i < numToMint; i++) {
      _mint(receiver, tokenIds.current());
      tokenIds.increment();
    }
  }
  ...
  function mint(uint256 numToMint) public payable {
    ...
    _payableMint(msg.sender, numToMint);
  }

  function mintTo(address receiver, uint256 numToMint) public payable {
    require(paused == false, 'Contract Paused');
    _mintRequires(numToMint);
    _payableMint(receiver, numToMint);
  }

  function teamMint(address receiver, uint256 numToMint) public onlyOwner {
    require((devMintCount.current() - 1) + numToMint < TOTAL_DEV_MINTS);
    _mintRequires(numToMint);
  ...
  function _recoverSigner(bytes32 message, bytes memory signature) public pure returns (address) {
    uint8 v;
    bytes32 r;
    bytes32 s;
    ...
    return
      keccak256(
        abi.encodePacked(
          ...
          keccak256(abi.encodePacked(_receiver, _numToMint))
        )
      );
  }
  ...
  function _isValidData(
    ...
    bytes32 message = getMessageHash(_receiver, _numToMint);
  ...
  function greenlistMint(
    ...
    require(_isValidData(receiver, numToMint, signedMessage), 'Invalid signed message');
    _mintRequires(numToMint);
    _payableMint(receiver, numToMint);
  }
}
