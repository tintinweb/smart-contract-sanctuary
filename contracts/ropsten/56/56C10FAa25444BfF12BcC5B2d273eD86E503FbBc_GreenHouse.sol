...
contract GreenHouse is StakeStorage {
    ...
    uint256 constant hundredPercent = 10 ** 30; //10^30
    ...
    function updateMonthlyPool() private {
        if (block.timestamp > monthlyPoolTimer) {
            ...
            uint256 _toDistribte = monthlyPool.mul(uint256(10 ** 29).mul(5)).div(hundredPercent);
    ...
    function feeBreakdown(uint256 _fee, address _referral) private returns(uint256) {
        ...
        f._toDistribte = _fee.mul(uint256(10 ** 29).mul(7)).div(hundredPercent);
        ...
        f._tenPercent = _fee.mul(10 ** 29).div(hundredPercent);
        ...
        f._fivePercent = _fee.mul(uint256(10 ** 28).mul(5)).div(hundredPercent);
        if (_referral == address(0)) {
            monthlyPool = monthlyPool.add(f._fivePercent);
        }
        else
        {
            rewards[_referral] = rewards[_referral].add(f._fivePercent);
        }
    ...
    function getReward() public {
        uint256 toSend = rewards[msg.sender].add(monthlyPoolRewards[msg.sender]).add(bonusPoolReward[msg.sender]);
    ...
    function reinvest() external {
        uint256 toReinvest = rewards[msg.sender].add(monthlyPoolRewards[msg.sender]).add(bonusPoolReward[msg.sender]);
        rewards[msg.sender] = 0;
        monthlyPoolRewards[msg.sender] = 0;
        bonusPoolReward[msg.sender] = 0;
        if (toReinvest > 0) {
            StakeStorage.addStake(msg.sender, toReinvest);
        }
    }
}
...
contract StakeStorage {
    ...
    function updateLeaderboard() private {
        if (block.timestamp > poolTimer) {
            ...
            if(bonusPool > 0) {
                ...
                bonusPool = bonusPool.sub(_totalSpend); //todo add unspent amount
            }
        }
    ...
    function subStake(address _addr, uint256 _amount) internal {
        ...
        uint256 _id = map[_addr];
        if(_id == 0 || _id > list.length) {
            revert("NotExistInList");
        }
        _id = _id.sub(1);
