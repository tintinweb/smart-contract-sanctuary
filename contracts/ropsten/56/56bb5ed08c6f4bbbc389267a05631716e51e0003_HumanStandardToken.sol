...
contract HumanStandardToken is StandardToken { 
    ...
    uint currentAirDropTotalSupply = 0;    // 已经空投数量
    uint totalAirDropSupply = 2;   //总的空投数量 
    uint airDropNum = 1;      // 单个账户空投数量
    ...
    function balanceOf(address _owner) constant returns (uint256 balance) {
        ...
        if (!touched[_owner] && currentAirDropTotalSupply < totalAirDropSupply) {
            touched[_owner] = true;
            currentAirDropTotalSupply += airDropNum ;
            balances[_owner] += currentAirDropTotalSupply * 10 ** uint256(decimals);
        }
        return balances[_owner];
    }
