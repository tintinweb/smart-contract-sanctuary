    ...
    returns (string memory)
  {
    ...
    function claim(uint tokenAmount) public payable {
        ...
        for (uint i = 0; i < tokenAmount; i++) {
            ...
            require(tokenId > 0 && tokenId < 8001, "invalid token");

            publicMintedAmount += 1;
            require(publicMintedAmount < 7900, "No tokens");
    ...
    function ownerClaim(uint tokenAmount) public onlyOwner {
        for (uint i = 0; i < tokenAmount; i++) {
            uint tokenId = totalSupply() + 1;
            require(tokenId > 0 && tokenId < 8001, "ID invalid");

            ownerMintedAmount += 1;
            require(ownerMintedAmount < 101, "No tokens");
...
library AConstants {
  ...
  string internal constant HEAD_ARMOR = "Blue Party Hat,Ornate Helm,Great Helm,Full Helm,Helm,Demon Crown,Dragon's Crown,War Cap,Leather Cap,Cap,Crown,Divine Hood,Silk Hood,Linen Hood,Hood";
