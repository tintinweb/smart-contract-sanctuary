    ...
    Pausable
{
    ...
    constructor(
        ...
        CapInfo memory capInfo,
        Timeframe memory timeframe,
        ContractInfo memory contractInfo
    )
        Crowdsale(wallet_, tokenSelling_, paymentTokensInfo)
        VestedCrowdsale(contractInfo.vestingContract)
        CappedTokenSoldCrowdsaleHelper(capInfo.tokenCap)
        IndividuallyCappedCrowdsaleHelper(capInfo.weiCap)
        TimedCrowdsaleHelper(timeframe)
        WhitelistCrowdsaleHelper(contractInfo.whitelistContract)
    {
    ...
    function pause() external onlyBy(crowdsaleAdmin) {
        _pause();
    }

    function unpause() external onlyBy(crowdsaleAdmin) {
        _unpause();
    }
 ...
 */
abstract contract CappedTokenSoldCrowdsaleHelper {
 ...
 */
abstract contract IndividuallyCappedCrowdsaleHelper {
    using SafeMath for uint256;
    ...
    /**
     * @param weicap Wei limit for each benificiary contribution
     */
    constructor(uint256 weicap) {
        require(weicap > 0, "IndividuallyCappedCrowdsaleHelper: zero cap");
        _weicap = weicap;
    }

    modifier beneficiaryCapNotExceeded(address beneficiary, uint256 weiAmount) {
        require(
            _contributions[beneficiary].add(weiAmount) <= _weicap,
    ...
    /**
     * @return weicap Cap for beneficiary in wei
     */
    function getBeneficiaryCap() external view returns (uint256 weicap) {
        weicap = _weicap;
    }
     ...
     * @return contribution Beneficiary contribution so far
     */
    function getBeneficiaryContribution(address beneficiary)
        external
        view
        returns (uint256 contribution)
    {
        ...
        contribution = _contributions[beneficiary];
    }
     ...
     * @param weiAmount Amount in wei of ERC20 payment token
     */
    function _updateBeneficiaryContribution(
        address beneficiary,
        uint256 weiAmount
    ) internal {
 ...
 */
abstract contract PausableCrowdsaleHelper is Pausable {
 ...
 */
abstract contract TimedCrowdsaleHelper {
    using SafeMath for uint256;
 ...
 */
abstract contract WhitelistCrowdsaleHelper {
...
contract Crowdsale is ReentrancyGuard, ICrowdsale {
    ...
    // Lot size of token being sold for each ERC20 payment token
    mapping(address => uint256) private _lotSizes;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < paymentTokensInfo.length; i++) {
            ...
            uint256 lotSize_ = paymentTokensInfo[i].lotSize;
            require(lotSize_ > 0, "Crowdsale: zero lot size");
            ...
            _lotSizes[paymentToken] = lotSize_;
        }
    }
    ...
    /**
     * @param paymentToken ERC20 payment token address
     * @return lotSize_ lot size of token being sold for specified ERC20 payment token
     */
    function lotSize(address paymentToken)
        external
        view
        override
        returns (uint256 lotSize_)
    {
        ...
        lotSize_ = _lotSize(paymentToken);
    }
     ...
     */
    function getTokenAmount(address paymentToken, uint256 lots)
        external
        view
        override
        returns (uint256 tokenAmount)
    {
        ...
        tokenAmount = _getTokenAmount(paymentToken, lots);
    }
    ...
    function _buyTokensFor(
        ...
        uint256 tokenAmount = _getTokenAmount(paymentToken, lots);
    ...
    /**
     * @param paymentToken ERC20 payment token address
     * @return lotSize_ lot size for token being sold
     */
    function _lotSize(address paymentToken)
        internal
        view
        virtual
        returns (uint256 lotSize_)
    {
        lotSize_ = _lotSizes[paymentToken];
    }
     ...
     */
    function _getTokenAmount(address paymentToken, uint256 lots)
        internal
        view
        virtual
        returns (uint256 tokenAmount)
    {
        uint256 lotSize_ = _lotSize(paymentToken);
        tokenAmount = lots.mul(lotSize_).mul(TOKEN_SELLING_SCALE);
    }
