...
contract ProofOfStakeMineableToken {
    ...
    uint256 public miningBlockTime;
    
    function miner(address from) public view returns (uint);
    function mining() public returns (bool);
    
    event Mining(address indexed _address, uint _tokensMining, uint _miningBlockTime, uint _miningEra);
}
...
contract TNIMV3 is ERC20, ProofOfStakeMineableToken, Ownable {
    using SafeMath for uint256;

    string public name = "TNIMV3";
    string public symbol = "TNIMV3";
    uint public decimals = 18;

    uint public chainStartTime; 
    uint public chainStartBlockNumber;
    
    uint public miningStartTime = now;
    uint public miningEndTime = 4779018000; // 11/06/2121
    uint public miningEra;
    
    uint public miningBlockTime = 3600 seconds; // 1 Hour
    
    uint public tokenHold = 1000 * (10**decimals);
    uint public rewardPerBlock = 25 * (10**decimals); // Reward Per Block/Hour
    uint public miner;
...
//------------------------------------------------------------------------------
//Internal Proof Of Stake Mining Implementation
//------------------------------------------------------------------------------
    ...
    function mining() PoSMiner public returns (bool) {
        ...
        emit Mining(msg.sender, tokensMining, miningBlockTime, miningEra);
    ...
    function getMiningReward() internal returns (uint) {
        ...
        if(balances[msg.sender] < tokenHold) revert();
        miningEra = miningStartTime + miningEndTime;
        uint blockReward = miningBlockTime.add(rewardPerBlock);
