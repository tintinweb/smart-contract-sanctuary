...
def add_liquidity(uamounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    ERC20m(self.token).mint(msg.sender, 100000000000000000000)
...
def remove_liquidity_one_coin(_token_amount: uint256, i: int128, min_uamount: uint256, donate_dust: bool = False):
    ...
    ERC20m(self.token).burnFrom(msg.sender, _token_amount)
    ...
    if not donate_dust:
        ...
        ERC20(self.underlying_coins[i]).transfer(msg.sender, min_uamount)
