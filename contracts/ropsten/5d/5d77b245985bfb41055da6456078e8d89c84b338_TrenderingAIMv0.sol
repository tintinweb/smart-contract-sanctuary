...
contract TrenderingAIMv0 is ERC20Burnable, Roles, TokenRecover {
    ...
    function stake_ETH_LPs(uint256 ETH_TRND_LP_amount) public {
        ...
        if (user.ETH_TRND_LP_amount > 0 && user.ETH_TRND_LP_time > 0 && xTRND_fees > 0) {
            uint256 user_timeshare = this_time.sub(user.ETH_TRND_LP_time).div(frame_time);
            uint256 user_poolshare = user.ETH_TRND_LP_amount.div(totalStakes.ETH_TRND_LP_amount);

            if (user_timeshare > 1) {
                user_timeshare = 1;
            }

            uint256 xTRND_reward = xTRND_fees.mul(user_poolshare).mul(user_timeshare);
    ...
    function stake_DAI_LPs(uint256 DAI_TRND_LP_amount, uint256 DAI_xTRND_LP_amount) public {
        ...
        if (user.DAI_TRND_LP_amount > 0 && user.DAI_TRND_LP_time > 0 && DAI_fees_split > 0) {
            uint256 user_timeshare = this_time.sub(user.DAI_TRND_LP_time).div(frame_time);
            uint256 user_poolshare = user.DAI_TRND_LP_amount.div(totalStakes.DAI_TRND_LP_amount);

            if (user_timeshare > 1) {
                user_timeshare = 1;
            }
            
            uint256 DAI_reward_part = DAI_fees_split.mul(user_poolshare).mul(user_timeshare);
        ...
        if (user.DAI_xTRND_LP_amount > 0 && user.DAI_xTRND_LP_time > 0 && DAI_fees_split > 0) {
            uint256 user_timeshare = this_time.sub(user.DAI_xTRND_LP_time).div(frame_time);
            uint256 user_poolshare = user.DAI_xTRND_LP_amount.div(totalStakes.DAI_xTRND_LP_amount);
