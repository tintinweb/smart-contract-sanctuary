...
contract MetaNFT is ERC721 {
    ...
    function setOwner(address payable new_owner) public payable {
    ...
    function getId(address nft_contract, uint256 nft_id) public pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(nft_contract, nft_id)));
    }
    function ownerOf(address nft_contract, uint256 nft_id) public view returns (address) {
        return ownerOf(getId(nft_contract, nft_id));
    }
    function mint(address nft_contract, uint256 nft_id) public payable {
        // We want our money
        require (msg.value >= nft_price);
        // One can only issue tokens of existing tokens
        require (IERC721(nft_contract).ownerOf(nft_id) != address(0));
        
        _safeMint(msg.sender, getId(nft_contract, nft_id));
    }
}