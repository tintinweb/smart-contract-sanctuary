...
contract FatCat is ERC721, Ownable {
    using SafeMath for uint256;

    string public FATCAT_PROVENANCE = ""; // IPFS URL WILL BE ADDED WHEN BANANAS ARE ALL SOLD OUT

    string public LICENSE_TEXT = ""; // IT IS WHAT IT SAYS

    bool licenseLocked = false; // TEAM CAN'T EDIT THE LICENSE AFTER THIS GETS TRUE

    uint256 public constant fatCatPrice = 50000000000000000; // 0.05 ETH

    uint256 public constant maxFatCatPurchase = 20;

    uint256 public constant MAX_FATCATS = 10000;
    ...
    // Reserve 150 fat cats for team - Giveaways/Prizes etc
    uint256 public fatCatReserve = 150;

    event licenseisLocked(string _licenseText);

    constructor() ERC721("Fat Cats", "FATCATS") {}
    ...
    function reserveFatCats(address _to, uint256 _reserveAmount)
        public
        onlyOwner
    {
        uint256 supply = totalSupply();
        require(
            _reserveAmount > 0 && _reserveAmount <= fatCatReserve,
            "Not enough reserve left for team"
        );
        for (uint256 i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, supply + i);
        }
        fatCatReserve = fatCatReserve.sub(_reserveAmount);
    }

    function setProvenanceHash(string memory _provenanceHash) public onlyOwner {
        FATCAT_PROVENANCE = _provenanceHash;
    }
    ...
    function tokenLicense(uint256 _id) public view returns (string memory) {
        require(_id < totalSupply(), "CHOOSE A FAT CAT WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function mintFatCat(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Banana");
        require(
            numberOfTokens > 0 && numberOfTokens <= maxFatCatPurchase,
            "Can only mint 20 tokens at a time"
        );
        require(
            totalSupply().add(numberOfTokens) <= MAX_FATCATS,
            "Purchase would exceed max supply of fat cats"
        );
        require(
            msg.value >= fatCatPrice.mul(numberOfTokens),
