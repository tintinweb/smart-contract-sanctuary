   ...
   */
    ...
    */
    ...
    function transferFrom(address _from, address _to, uint256 _value) public whenNotPaused returns (bool) {
        ...
        if (timelockList[_from].length > 0) {
            _autoUnlock(_from);            
        }
    ...
    mapping(address => LockInfo[]) internal timelockList;

    event Lock(address indexed holder, uint256 value, uint256 releaseTime);
    event Unlock(address indexed holder, uint256 value);

    function balanceOf(address _holder) public view returns (uint256 balance) {
        uint256 totalBalance = super.balanceOf(_holder);
        if( timelockList[_holder].length > 0 ){
            for(uint i=0; i<timelockList[_holder].length;i++){
                totalBalance = totalBalance.add( _unlockableBalance(_holder,i) );
            }
        }        
        return totalBalance;
    }
    ...
    function lock(address _holder, uint256 _value, uint256 _releaseStart, uint256 _termOfRound, uint256 _unlockAmountPerRound) public onlyOwner returns (bool) {
        _lock(_holder, _value, _releaseStart, _termOfRound, _unlockAmountPerRound);   
        return true;
    }
    ...
    function unlock(address _holder, uint256 idx) public onlyOwner returns (bool) {
        require( timelockList[_holder].length > idx, "There is not lock info.");
        _unlock(_holder,idx);
        return true;
    }

    function transferWithLock(address _holder, uint256 _value, uint256 _releaseTime) public onlyOwner returns (bool) {
        _transfer(msg.sender, _holder, _value);
        _lock(_holder, _value, _releaseTime, 1, _value);
        return true;
    }

    function transferWithSliceLock(address _holder, uint256 _value, uint256 _releaseStart, uint256 _termOfRound, uint256 _unlockAmountPerRound) public onlyOwner returns (bool) {
        _transfer(msg.sender, _holder, _value);
        _lock(_holder, _value, _releaseStart, _termOfRound, _unlockAmountPerRound);
        return true;
    }

    function _lock(address _holder, uint256 _value, uint256 _releaseTime, uint256 _termOfRound, uint256 _unlockAmountPerRound) internal returns(bool) {
        require(_balances[_holder] >= _value,"There is not enough balances of holder.");
        _balances[_holder] = _balances[_holder].sub(_value);
        timelockList[_holder].push( LockInfo(_releaseTime, _value, _value, _termOfRound, _unlockAmountPerRound) );
    ...
    function _autoUnlock(address _holder) internal returns(bool) {
        for(uint256 idx =0; idx < timelockList[_holder].length ; idx++ ) {
            if (timelockList[_holder][idx]._releaseTime <= now) {
                // If lockupinfo was deleted, loop restart at same position.
                if( _releaseTimeLock(_holder, idx) ) {
                    idx -=1;
                }
            }
        }
