...
contract NFTSale is ERC721, Ownable {
    ...
    constructor() public ERC721("NFTSale", "NFT") {
    }
    ...
    function mintNFT(string[] memory metaDatas) public payable {
    ...
    function reserveNFT(string[] memory metaDatas) public onlyOwner {
        uint numberOfTokens = metaDatas.length;
        require(saleIsActive, "Sale must be active to mint NFT");
        require(totalSupply().add(numberOfTokens) <= MAX_NFT, "Purchase would exceed max supply of NFT");
        for(uint i = 0; i < numberOfTokens; i++) {
