...
contract BlahNFTScore is BlahNFTNFT {
    ...
    function hasUploadedMetadata(BlahNFTTypes.City memory city) internal view returns (bool) {
        return keccak256(abi.encodePacked(city.image)) != keccak256(abi.encodePacked(placeHolderImage));
    }
...
contract BlahNFTNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 private constant FOUNDERS_RESERVE_AMOUNT = 15;
    ...
    constructor() ERC721("City Clash", "Unite factions!") {
    ...
    function mint(uint256 _tokenId) internal {
        BlahNFTTypes.City memory city;// = BlahNFTTypes.City(1, "Seattle", "USA", "image", 10, 0, 1);
        city.id = _tokenId;
        city.image = getPlaceHolderImage();
