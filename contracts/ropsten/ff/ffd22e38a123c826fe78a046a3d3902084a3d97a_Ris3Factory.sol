...
contract Ris3Factory is Ownable {
    ...
    address governmentAddress = 0x56489aA400D20537A196007522AF6DFD8393790D;
    ...
    address public lpTokenAddress = 0x89acd6C8C6313d1D6923BE4b8e13DADC9b8Cebda;
    ...
    function calculateRewardTesting(address userAddress) public view returns (uint256 _percnt, uint256 _diff, uint256 _userRewardPerSecond) {
        ...
        if (percnt != 0 ) {
            percnt = percnt.div(totalLiquidity);
        }
        ...
        uint256 rewardsEverySecond = totalRewards / 5 / 24 / 60 / 60; //get rewards every second
        uint256 userRewardPerSecond = rewardsEverySecond.mul(percnt);
                userRewardPerSecond = userRewardPerSecond.div(100000000);
    ...
    function calculateReward(address userAddress) public view returns (uint256 _reward, uint256 _tax) {
        ...
        if (now < government.getWithdrawingStartTime() || userRewardInfo[userAddress].lastWithdrawTime >= govElectionStartTime || percnt == 0){
            return ( 0, 0 );
        } else {
            //if not withdrawn on current cycle yet
            if (userRewardInfo[userAddress].lastWithdrawTime < cycleStartTime) {
            ...
            amount = userRewardPerSecond.mul(diff);
