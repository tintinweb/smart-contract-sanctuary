...
contract Settings {
    /// percent decimal
    uint256 internal constant decimalPercent = 10000;
    /// percent of pot
    uint256 internal constant percentBigwin = 4400;
    uint256 internal constant percentShare = 5000;
    uint256 internal constant percentNextround = 250;
    uint256 internal constant percentCost = 350;
    
    
    // percent of share everytime
    uint256 internal constant percentShareOnetime = 2000;
    
    
    // percent of Extend
    uint256 internal constant percentExtendMain = 9000;
    uint256 internal constant percentExtendUpper = 800;
    uint256 internal constant percentExtendUpperUpper = 200;
...
contract BleedFomo is Ownable, Settings{
    ...
    // time setting
    uint256 private constant checkOutinterval =  5 minutes;  
    uint256 private constant endTimeSettingInit = 10 minutes;
    uint256 private constant endTimeSettingMax = 10 minutes;
    uint256 private constant endTimeSettingMin = 5 minutes;
    uint256 private constant endTimeSettingIncrease1 = 10 minutes;
    uint256 private constant endTimeSettingIncrease2 = 5 minutes;
    uint256 private constant nextroundInterval = 5 minutes; //from gameend to gamestart
    
    uint256 private constant keyPriceInit = 0.00046 ether;
    uint256 private constant timeKeyPriceIncrease = 10 minutes;
    uint256 private constant percentKeyPriceIncrease = 500;
    uint256 private constant conditionKeyPriceIncreaseInit = 5;
    uint256 private constant conditionKeyPriceAttenuation = 1;
    ...
    function _calcCheckoutTime(uint256 _now) private {
        uint256 _flagtime = 1538006400;  // GMT+8 2018/9/11/16:00
        checkoutTime = _flagtime > checkoutTime ? _flagtime : checkoutTime;
        if(_now > checkoutTime){
            uint256 _t = ((_now - checkoutTime) / checkOutinterval);
            checkoutTime = checkoutTime + ((_t+1) * checkOutinterval);
        }
    }
    ...
    function _setUserInfo(address _useraddr, uint64 _nkeys, uint256 _now, address _upper) private {
        ...
        //emit onDebug("userinfo", _useraddr, userkeysindex[_useraddr], userkeys[userkeysindex[_useraddr]-1].num, userkeys[userkeysindex[_useraddr]-1].num_pending, userkeys.length);
    }
