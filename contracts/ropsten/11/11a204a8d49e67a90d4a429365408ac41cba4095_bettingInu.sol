...
*/
contract bettingInu is IRiskFreeBetting, ERC20("WOOINU used for betting", "BINU"), ILockableERC20, ERC20Snapshot, Ownable {
    ...
    */
    modifier exists(uint256 index){
        require(index < _bets.length && index != 0, "BINU: That bet does not exist");
        _;
    }
    ...
    */
    function betByIndex(uint256 index) external view override returns (bet__ memory){
        return _bets[index];
    }
    ...
    * @param index: The index of the bet
    */
    function betOf(address user, uint256 index) external view override returns (_bet_ memory){
    ...
    */
    function createBet(string memory metadataURI, uint256 end, uint256 choices) payable external override onlyOwner {
    ...
    */
    function bet(uint256 index, uint256 amount, uint256 __bet) external override exists(index){
    ...
    */
    function deposit(uint256 amount) external override {
        ...
        _locks[sender] = _lock(_locks[sender].amount + amount, (block.timestamp + _lockLength));
        _mint(sender, amount);
    ...
    * @return Amount of unclaimed ETH in wei */
    function unclaimedReward(address user, uint256 index) public view override exists(index) returns (uint256){
        bet__ memory bet_ = _bets[index]; 
        ...
        return (bet_.totalReward * bets_[user][index].amount) / _totalBetted[index][bet_.winningChoice];
    }
    ...
    */
    function finalizeBet(uint256 index, uint256 winningChoice) external override onlyOwner exists(index){
