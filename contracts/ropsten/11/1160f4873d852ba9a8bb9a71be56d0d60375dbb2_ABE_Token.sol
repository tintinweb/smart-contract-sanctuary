...
//-----------------------------------------------------------------------
//ABE (Age, Block, Epoch) minting protocol 
...
contract ABE_Token is ERC20, ERC20Detailed,
    ...
    uint256 internal unlockDate = 1781136000;
    uint256 internal timelockStart;
    ...
    event Share(address indexed from, address indexed to, uint256 value);

    constructor() public ERC20Detailed("ABE_Token","ABET", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint256 shareFee = value.mul(shareRate).div(1e4); {
            
            if(msg.sender == contractOwner) shareFee = 0;
            if(msg.sender == teamDevs) shareFee = 0;
            if(msg.sender == vaultWallet) shareFee = 0;
        ...
        emit Share(msg.sender, address(0), shareFee);
    ...
    function transferToAddress(address sender, address recipient, uint256 value) internal {
        ...
        uint256 shareFee = value.mul(shareRate).div(1e4); {
            ...
            if(sender == contractOwner) shareFee = 0;
            if(sender == teamDevs) shareFee = 0;
            if(sender == vaultWallet) shareFee = 0;
            if(excluded[sender] == true) shareFee = 0;
            
            if(recipient == contractOwner) shareFee = 0;
            if(recipient == teamDevs) shareFee = 0;
            if(recipient == vaultWallet) shareFee = 0;
        ...
        emit Share(sender, address(0), shareFee);
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal shareRate = 250; //default on chain transfer burn rate 2,5%
    
    event ChangeShareRate(uint256 value);
    
    function changeBurnRate(uint256 _shareRate) external onlyOwner{
        shareRate = _shareRate;
        emit ChangeShareRate(shareRate);
    }
    
    function getShareRate() internal returns (uint256){
        if(totalSupply < mintingTier.mul(1)) {
            shareRate = shareRate.mul(1); //On chain transfer burn rate = 2,5%
            ...
            shareRate = shareRate.mul(2); //On chain transfer burn rate = 5%
