...
contract GalaxyPadStake{
    ...
    uint256 [4] public tier_pool_weight = [0,10,40,140];
    uint256 [4] public tier_pool_member = [0,0,0,0];
    uint256 [4] public tier_pool_amount = [0,100000,300000,900000];
    ...
    constructor(){
        ...
        Token = IERC20(0xfcbD7E6E35A758062202a599EE6500507DD51668);
    ...
    function stake(uint256 val) public{
        ...
        if(user.totalstakeduser >= tier_pool_amount[1].mul(10**Token.decimals()) && user.totalstakeduser < tier_pool_amount[2].mul(10**Token.decimals()))
        {
            user.currenttire = 1;
            tier_pool_member[user.currenttire] +=1;
            user.tirein[1] = true;
        }else if(user.totalstakeduser >= tier_pool_amount[2].mul(10**Token.decimals()) && user.totalstakeduser < tier_pool_amount[3].mul(10**Token.decimals()))
        {
            checktier(msg.sender,2);
            user.tirein[2] = true;
            user.currenttire = 2;
            tier_pool_member[user.currenttire] +=1; 
        }else if(user.totalstakeduser >= tier_pool_amount[3].mul(10**Token.decimals()))
        {
