...
function flatten(
    ...
    uint output_length = dim1 * dim2 * dim3;
...
contract MNIST8x8NeuralNet {
    function conv1(
        ...
        for (uint i = 0; i < weights.length; ++i) {
            ...
            for (uint j = 0; j < filters_i.length; ++j) {
                for (uint k = 0; k < filters_i[0].length; ++k) {
    ...
    function get_ex_prediction() external view returns (uint) {
        return predict(ex_image);
    }
}