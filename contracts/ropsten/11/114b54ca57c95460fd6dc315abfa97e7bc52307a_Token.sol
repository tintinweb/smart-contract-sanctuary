...
contract Token is Context, Ownable, IERC20 {
    ...
    function getCommunityTokenFund() public view returns (uint256) {
    ...
    function withdrawOperationFund(uint256 amount, address walletAddress, string memory reason) public onlyOwner() {
        require(amount < operationalFund, "You cannot withdraw more funds that you have in token fund");
    ...
    function _transfer(address fromAddress, address toAddress, uint256 amount) private {
        ...
        // This is contract's balance without any reserved funds such as token Fund
        uint256 contractTokenBalance = balanceOf(address(this)).sub(getCommunityTokenFund());
