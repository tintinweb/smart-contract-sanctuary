...
contract Auction is Context, Ownable {
	...
	function bid(uint256 _tokenId) public payable{
	    ...
	    if(pending_claim[msg.sender][_tokenId] > 0){
	...
	function claim(uint256 _tokenId) public {
	    ...
	    if(auction[_tokenId].highestBidder == msg.sender){
    		...
    		payable(token_seller[_tokenId]).transfer(auction[_tokenId].highestBid);
	    }	
	    else{
	...
	function buy(uint256 _tokenId) public payable  {
	     ...
	     nft_token.transferFrom(address(this), msg.sender, _tokenId);
	     payable(token_seller[_tokenId]).transfer(msg.value);
