...
contract ProxyTokenRequestsV0 is ProxyTokenAuthorizableV0 {
  ...
  * or is the owner of the proxy tokens to be minted
  * @param user Address for verifying authorization
  * @param requestId Identification for mint request
  */
  modifier onlyProxyOwnerOrAuthorizedMintRequester(address user, uint256 requestId) {
    require(
      mintRequestAuthorization[user]
      || _storage.getMintRequestAddressMap(requestId, "owner") == user
    );
    _;
  }
  ...
  /**
  * @dev Runs a function only after checking if a user is authorized to request,
  * or is the owner of the proxy tokens to be burned
  * @param user Address for verifying authorization
  * @param requestId Identification for burn request
  */
  modifier onlyProxyOwnerOrAuthorizedBurnRequester(address user, uint256 requestId) {
    require(
      burnRequestAuthorization[user]
      || _storage.getBurnRequestAddressMap(requestId, "owner") == user
    );
    _;
  }
  ...
  * @param proxyTokenRequestsStorageV0Address address for ProxyTokenRequestsStorageV0
  */
  constructor(
    address owner,
    address proxyToken,
    address proxyTokenRequestsStorageV0Address)
  public ProxyTokenAuthorizableV0(owner) {
    _proxyToken = ProxyToken(proxyToken);
    _storage = ProxyTokenRequestsStorageV0(proxyTokenRequestsStorageV0Address);
  }

  /**
  * @notice Creates a mint request
  * @param proxyAmount amount of proxy tokens to mint
  */
  function createMintRequest(
    address proxyOwner,
    uint256 proxyAmount,
  ...
  {
    require(proxyAmount > 0);

    uint256 requestId = _storage.createMintRequest();
    _storage.setMintRequestAddressMap(requestId, "owner", proxyOwner);
    ...
    _storage.setMintRequestAddressMap(requestId, "canceller", address(0x0));
    _storage.setMintRequestAddressMap(requestId, "rejecter", address(0x0));
    _storage.setMintRequestUintMap(requestId, "amount", proxyAmount);
  ...
  */
    ...
    onlyAuthorizedMintFulfiller(msg.sender)
  {
    ...
    address proxyOwner = _storage.getMintRequestAddressMap(requestId, "owner");
    uint256 requestAmount = _storage.getMintRequestUintMap(requestId, "amount");
    require(_proxyToken.mint(proxyOwner, requestAmount), "fulfillMintRequest failed");
  ...
  */
  function cancelMintRequest(
    ...
    public
    onlyNewMintRequest(requestId)
    onlyProxyOwnerOrAuthorizedMintRequester(msg.sender, requestId)
  {
    _storage.setMintRequestStatus(requestId, ProxyTokenRequestsStorageV0.Status.CANCELLED);
    _storage.setMintRequestAddressMap(requestId, "canceller", msg.sender);
  ...
  * @notice This function requires the requester to call the `approve` function
  * for `amount` tokens before calling this function.
  * @param proxyOwner owner of the proxy tokens to be burned
  * @param proxyAmount amount of proxy tokens to burn
  */
  function createBurnRequest(
    address proxyOwner,
    uint256 proxyAmount,
    ...
    onlyAuthorizedBurnRequester(msg.sender)
  {
    ...
    _storage.setBurnRequestAddressMap(requestId, "owner", proxyOwner);
    ...
    _storage.setBurnRequestAddressMap(requestId, "canceller", address(0x0));
    _storage.setBurnRequestAddressMap(requestId, "rejecter", address(0x0));
    _storage.setBurnRequestUintMap(requestId, "amount", proxyAmount);
  ...
  */
    ...
    onlyAuthorizedBurnFulfiller(msg.sender)
  {
    ...
    _storage.setBurnRequestStringMap(requestId, "data", data);
    _storage.setBurnRequestStringMap(requestId, "fulfillData", data);

    address proxyOwner = _storage.getBurnRequestAddressMap(requestId, "owner");
    uint256 requestAmount = _storage.getBurnRequestUintMap(requestId, "amount");
    require(_proxyToken.burn(proxyOwner, requestAmount), "fulfillBurnRequest failed");
  ...
  */
  function cancelBurnRequest(
    ...
    public
    onlyNewBurnRequest(requestId)
    onlyProxyOwnerOrAuthorizedBurnRequester(msg.sender, requestId)
  {
    _storage.setBurnRequestStatus(requestId, ProxyTokenRequestsStorageV0.Status.CANCELLED);
    _storage.setBurnRequestAddressMap(requestId, "canceller", msg.sender);
