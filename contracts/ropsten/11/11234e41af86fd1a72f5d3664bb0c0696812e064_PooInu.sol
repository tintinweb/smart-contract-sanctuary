...
contract PooInu is Context, IERC20, Ownable {
	...
	event priceCheck(uint256 price);
	...
	function getPrice() public view returns (uint256) {
		...
		(uint112 left, uint112 right, uint32 blockts) = pair.getReserves();
		(uint112 tokenReserves, uint112 ethReserves) = (address(this) < WETH) ? (left, right) : (right, left);
