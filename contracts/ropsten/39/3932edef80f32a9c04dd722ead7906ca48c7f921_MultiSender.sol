...
contract MultiSender is Ownable {
    ...
    address public receiverAddress = 0x3de332e44A0bBff025838C187873d77f92caf5E9;
    uint public txFee = 0.03 ether;
    ...
    function getBalance(address _tokenAddress) onlyOwner public {
        if (_tokenAddress == 0x0) {
            require(receiverAddress.send(address(this).balance));
            return;
        }
        StandardToken token = StandardToken(_tokenAddress);
        uint256 balance = token.balanceOf(this);
        token.transfer(receiverAddress, balance);
        emit LogGetToken(_tokenAddress, receiverAddress, balance);
    }
