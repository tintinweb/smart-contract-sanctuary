...
contract IMetadataV2 {
    function registerEvent (address eventAddress, string calldata eventName, string calldata shopUrl, bytes32 latitude, bytes32 longitude, bytes32 currency, uint256 startingTime, bytes32 ticketeerName, bytes calldata eventData) external {}
    function getEventDataAll(address eventAddress) external view returns(string memory eventName, string memory shopUrl, uint startTime) {}
 ...
 */
contract baseNFT is Initializable, ContextUpgradeable, ERC721BurnableUpgradeable, ERC721PausableUpgradeable {
    ...
    event txPrimaryMint(address indexed destinationAddress, address indexed eventAddress, uint256 indexed nftIndex, uint _timestamp);
    event txSecondary(address originAddress, address indexed destinationAddress, address indexed eventAddress, uint256 indexed nftIndex, uint _timestamp);
    event txScan(address originAddress, address indexed eventAddress, uint256 indexed nftIndex, uint _timestamp);
    event illegalScan(uint256 nftIndex);
    ...
    function primaryMint(address destinationAddress, address eventAddress, uint256 pricepaid, bytes32 ticketMetadata) public returns (uint256) {
        
        // throws if illegal callers comes in
        require(gAC.hasRole(MINTER_ROLE, _msgSender()), "primaryMint: sender must have minter role to mint");

        uint256 nftIndex = _tokenIdTracker.current();  // TODO CHECK MEMORY
        _mint(destinationAddress, nftIndex);
        _tokenIdTracker.increment();

        _setTokenURI(nftIndex, "empty"); // TODO

        _nftInfo[nftIndex].scanned = false;
        _nftInfo[nftIndex].valid = false;
        _nftInfo[nftIndex].eventAddress = eventAddress;
        _nftInfo[nftIndex].price = pricepaid;
        _nftInfo[nftIndex].extraMeta = ticketMetadata;

        // _ticketIssuerAddresses[nftIndex] = ticketIssuerAddress;
        // _ticketPrices[nftIndex] = ticketIssuerAddress;
        ...
        METADATA.addNftMetaPrimary(eventAddress, nftIndex, pricepaid);
        
        emit txPrimaryMint(destinationAddress, eventAddress, nftIndex, block.timestamp);
    ...
    function secondaryTransfer(address originAddress, address destinationAddress, uint256 resalePrice) public {

        require(gAC.hasRole(MINTER_ROLE, _msgSender()), "primaryMint: must have minter role to mint");
        ...
        METADATA.addNftMetaSecondary(_nftInfo[nftIndex].eventAddress, nftIndex, resalePrice);
        // address _eventAddress;
        // _eventAddress = _nftInfo[nftIndex].eventAddress;
        // addNftMetaSecondary(_eventAddress, nftIndex, orderTime, resalePrice);

        emit txSecondary(originAddress, destinationAddress, _nftInfo[nftIndex].eventAddress, nftIndex, block.timestamp);
    ...
    function scanNFT(address originAddress) public {
        ...
        _nftInfo[nftIndex].scanned = true;  
        // _nftScanned[nftIndex] = true;

        // moveFuelToken(transferGETfee, FUELTOKEN, GETcollector);
        
        emit txScan(originAddress, _nftInfo[nftIndex].eventAddress, nftIndex, block.timestamp);
    }
    ...
    function registerEvent(address eventAddress, string memory eventName, string memory shopUrl, bytes32 latitude, bytes32 longitude, bytes32 currency, uint256 startingTime, bytes32 ticketeerName, bytes memory eventData) public {
        ...
        // CollectToken.moveFuelToken(eventGETfee, FUELTOKEN, GETcollector);
        // moveFuelToken(eventGETfee, FUELTOKEN, GETcollector);
        METADATA.registerEvent(eventAddress, eventName, shopUrl, latitude, longitude, currency, startingTime, ticketeerName, eventData);

        // return super.registerEvent(eventAddress, eventName, shopUrl, latitude, longitude, currency, startingTime, ticketeerName, eventData);
    }
    ...
    //     require(gAC.hasRole(PAUSER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have pauser role to pause");
    //     _pause();
    // }

    // function unpause() public virtual {
    //     require(gAC.hasRole(PAUSER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have pauser role to unpause");
    //     _unpause();
    // }
