...
contract Test is Ownable {
  ...
  {
    require(bundleNumber < totalBundle);
  ...
  function getBundleExpiredTimeOfEmail(string email, uint8 bundleNumber) public
  returns(uint256)
  {
    if (bundleExpriredTimeOfEmail[email].length < totalBundle) return 0;
    return bundleExpriredTimeOfEmail[email][bundleNumber];
  }
}