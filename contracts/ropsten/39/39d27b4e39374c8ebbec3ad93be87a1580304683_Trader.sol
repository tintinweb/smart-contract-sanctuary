...
contract Trader {
    ...
    // function getBalance(address tokenAddress) public constant onlyOwner returns (uint256) {
        ...
        return ETH_TOKEN_ADDRESS.balanceOf(this);
    }

    function testBalance(address tokenAddress) public constant onlyOwner returns (uint256) {
        ERC20 token = ERC20(tokenAddress);
    ...
    function displayParamAndCaller(string param) public constant returns (string, address) {
        return (param, msg.sender);
    }
}