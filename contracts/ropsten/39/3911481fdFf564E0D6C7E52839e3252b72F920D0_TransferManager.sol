...
contract TransferManager {
    ...
    event WithdrawItem(address indexed sender, address indexed receiver, uint256 indexed tokenId, string userId);
    ...
    function getMessageHash(address _from, address _to, string memory _userId, uint256 _tokenId)
        public pure returns (bytes32)
    {
        return keccak256(abi.encodePacked(_from, _to, _userId, _tokenId));
    }
    ...
    function verify(address _from, address _to, string memory _userId, 
        uint256 _tokenId, bytes memory signature
    )
        public view returns (bool)
    {
        bytes32 messageHash = getMessageHash(_from, _to, _userId, _tokenId);
    ...
    function transferItem(address _to, string memory _userId, uint256 _tokenId, 
            bytes memory signature) public payable {
                
        require(verify(msg.sender, _to, _userId, _tokenId, signature) == true, "Invalid signature");
        ...
        emit WithdrawItem(msg.sender, _to, _tokenId, _userId);
    }
