...
contract Harvest is Ownable {
    ...
    /**
      * @dev set vesting periods
      * @param periodEndDate end dates of periods
      * @param rewardToken address of reward token
      */

    function setPeriods(
            uint[] memory periodEndDate, 
            address rewardToken
            ) public onlyOwner {
        for(uint i=0;i<periodEndDate.length;i++) {
            endStageDate[rewardToken][i] = periodEndDate[i];
        }
    }

    /**
      * @dev set flag true/false for users, claimed or not
      * @param _users list of users address
      * @param _rewardToken address of reward token
      * @param _stage number of claim stage
      * @param _value claimed or not
      */
    function setClaimFlag(
        ...
        for(uint i = 0; i < _users.length; i++) {
            claimedInStage[_rewardToken][_stage][_users[i]]=_value[i];
        }
