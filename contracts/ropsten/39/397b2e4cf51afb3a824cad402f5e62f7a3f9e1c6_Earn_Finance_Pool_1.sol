...
contract Earn_Finance is BEP20, Ownable {
    ...
    function _swapTokensForEth(uint256 tokenAmount) public onlyMinter {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
...
contract Earn_Finance_Pool_1 is Ownable {
    ...
    function updateAccount(address account) private {
        ...
        uint256 _devFee = pendingDivs.mul(devFee).div(100);
        
        if (pendingDivs > 0) {
            EARN.mint(address(EARN), pendingDivs);
            
            EARN._swapTokensForEth(pendingDivs);
        ...
        if (_devFee > 0) {
            ...
            emit RewardsTransferred(account, _devFee);
        }
    ...
    function deposit(uint256 amountToStake) public {
        ...
        EARN.swapAndLiquify(amountToStake);
