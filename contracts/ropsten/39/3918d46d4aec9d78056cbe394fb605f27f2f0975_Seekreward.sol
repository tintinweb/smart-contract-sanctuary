...
contract Seekreward{
    ...
    function _refAmount(address _addr,address _upline,uint amount)private{
        ...
        for(uint8 i = 0; i < 20; i++) {
            ...
            users[up].referalBonus = users[up].referalBonus.add(amount.mul(refBonuses).div(100e18)); 
             emit Referalamount(_addr,up,amount.mul(refBonuses).div(100e18),block.timestamp);
    ...
    function _matchBonus(address _user,uint amount)private{
        address up = users[_user].upline;
        for(uint i = 1;i <= 21;i++){
        if(up == address(0))break;
           if(i<=3){
               users[up].matchBonus = users[up].matchBonus.add(amount);
                emit Matchbonus(_user,up,amount,block.timestamp);
           }
           else if(i <= 6){
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].depositPayouts < max_payout) {
            payout =  ((users[_addr].depositAmount.mul(1e18).div(100e18)) * ((block.timestamp.sub(users[_addr].depositTime)).div(1 days))) - users[_addr].depositPayouts;
