...
contract S_Presales is Context, Ownable {
...
// -- post sale --
    ...
    function concludeAndAddLiquidity() external onlyOwner {
        ...
        pancakeSwapV2Router.addLiquidityETH{value: BNB_for_pool}(
            address(token),
            balance_token,
            balance_token, // sLiPpaGe iS uNaVoIdAbLe --> JVP: Initial liquidity, no slippage
            BNB_for_pool, // sLiPpaGe iS uNaVoIdAbLe --> JVP: Initial liquidity, no slippage
            owner(),
            block.timestamp
        );
    ...
    // gives complete access to the owner, to all funds on the presales!
    // HOWEVER: owner would get the LP token representing the BNB & tokens anyway...
