...
contract LostPoets is ReentrancyGuard, AdminControl, ERC721, ILostPoets {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AdminControl, ERC721, IERC165)
        returns (bool)
    {
        return
     ...
     */
    function finalizePoets(bool value, uint256[] memory tokenIds)
        external
        override
        adminRequired
    {
     ...
     */
    function getWordCount(uint256 tokenId)
        external
        view
        override
        returns (uint8)
    {
        require(
            _exists(tokenId),
            "ERC721: word count query for nonexistent token"
        );
        if (_addWordsLastBlock[tokenId] == block.number) {
     ...
     */
    function updateERC721BurnAddress(address erc721BurnAddress)
        external
        override
        adminRequired
    {
    ...
    function _onERC1155Received(
        ...
        if (action == 0) {
            require(
                redemptionEnabled && block.timestamp <= redemptionEnd,
                "Redemption inactive"
            );
            ...
            require(
                !finalized[tokenId],
                "Cannot modify words of finalized poet"
            );
    ...
    function onERC721Received(
        ...
        (uint256 action, uint256 tokenId) = abi.decode(
            data,
            (uint256, uint256)
        );
