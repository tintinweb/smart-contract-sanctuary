...
contract Distributor is Ownable {
    ...
    // This function will allow you to add only one token to the pool.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            // Dynamic WETHReward and WETHPerBlock
            uint256 WETHBal = WETH.balanceOf(address(this));
            uint256 WETHReward =
                WETHBal.div(1000).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
    ...
    **/
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = this.pendingWETH(_pid, msg.sender);
    ...
    /// This function allows you to harvest your pending ETH reward without Un-staking your PROFIT tokens.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of WETH rewards.
    ...
    /// @notice Withdraw PROFIT tokens from Distributor and harvest pending ETH rewards.
    /// @param _pid The index of the pool. See `poolInfo`.
