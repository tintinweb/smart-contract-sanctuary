...
contract EscrowToken is Admins {
	...
	event FreezeWithdraw(address user, uint amountWithdraw, uint amountFreeze, bool status, uint balanceOfUser, uint freezeBalanceOfUser);
	...
	function 	freezeWithdraw(
		...
		notNullAddress(user)
		notZero(amountFreeze)
		onlyModerator()
		public
	{
		require(amountTransfer == 0 && status != false);

		if (status == true) { // user is win
			_balanceOf[user] = _balanceOf[user].add(amountTransfer);
			_balanceOf[_liquidatePool] = _balanceOf[_liquidatePool].sub(amountTransfer);
		}
		_freezeBalanceOf[user] = _freezeBalanceOf[user].sub(amountFreeze);
		emit FreezeWithdraw(user, amountTransfer, amountFreeze, status, _balanceOf[user], _freezeBalanceOf[user]);
	}
