...
contract TokenSale is ERC20Interface, Owned {
    ...
    function buyTokens(address referrer) onlyWhenRunning public payable {
        ...
        address feferrerv1 = getReferrer(msg.sender);
        
        if (feferrerv1 != address(0)) {
            uint256 commissionAmount = ethAmount.mul(referralCommissionRate).div(10000);
            ethAmount = ethAmount.sub(commissionAmount);
