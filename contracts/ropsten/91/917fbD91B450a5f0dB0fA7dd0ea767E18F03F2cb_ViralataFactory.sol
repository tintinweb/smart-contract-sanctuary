...
contract ViralataFactory is IViralataFactory {
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        if (address(token0) == address(_AURO) || address(token1) == address(_AURO)) {
            IAURO(_AURO).grantRole(IAURO(_AURO).getFeeRole(), pair);
        }
    ...
    function setTokenAddress(address token) external override {
        require(msg.sender == feeToSetter, 'ViralataSwap: FORBIDDEN');
        _AURO = token;
    }
