...
contract PLithToken is ERC20('PRE-LITHIUM', 'PLITHIUM'), ReentrancyGuard {
    ...
    event endBlockChanged(uint256 newEndBlock);

    constructor(uint256 _startBlock, address _feeAddress) {
    ...
    function setEndBlock(uint256 _newEndBlock) external onlyOwner {
        require(block.number < _newEndBlock, "cannot set end block in the past");
