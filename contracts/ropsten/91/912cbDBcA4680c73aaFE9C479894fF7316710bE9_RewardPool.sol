...
contract RewardPool is Ownable, Constants, IRewardPool {
    ...
    function emergencySwapToBusd(IERC20 _inputToken, uint256 _amount) public onlyOwner {
    ...
    function emergencyRemoveLiquidity(IERC20 _lpToken) public onlyOwner {
    ...
    function swapToLotl() public onlyOwner {
        require (!hasSwappedToLotlThisCycle);
    ...
    function distributeRewardPool () public onlyOwner {
        chef.calculateRewardPool();
        emit DistributeRewardPool(msg.sender);
    }
    ...
    function removeAllLiquidity () public onlyOwner {
        for(uint8 i; i < lptoken.length; i++)
        {
            ...
            {
                emergencyRemoveLiquidity(lptoken[i]);
            }
        }
    }
    ...
    function swapAllToBUSD () public onlyOwner {
        for(uint8 i; i < tokens.length; i++){
            if(tokens[i].balanceOf(address(this)) > 0 && tokens[i] != IERC20(busdAddr)){
                emergencySwapToBusd(tokens[i], tokens[i].balanceOf(address(this)));
            }
        }
    }
