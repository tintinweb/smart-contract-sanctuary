...
contract MoonLenderTestV2 is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    string private _name = "MoonLenderTestV2";
    string private _symbol = "MDLRV2";
    ...
    function changeLiquidtyFormulaToV1(bool value) public onlyOwner {
        require(useV1 != value, "MoonLenderTestV2: useV1 already set to that value");
        emit LiquidityFormulaChanged(value, useV1);
        useV1 = value;
    }
    function changeBurnStatus(bool value) external onlyOwner {
        require(burnEnabled != value, "MoonLenderTestV2: burnEnabled already set to that value");
    ...
    function mintToAddress(uint256 amountToMint, address addressToSendTokens) public onlyOwner {
        require(!_swapping, "MoonLenderTestV2: Cannot mint while in swap and liquify");
        require(!isTradingEnabled, "MoonLenderTestV2: Cannot mint while trading is enabled");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MoonLenderTestV2: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;
        emit AutomatedMarketMakerPairChange(pair, value);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFee[account] != excluded, "MoonLenderTestV2: Account is already the value of 'excluded'");
        _isExcludedFromFee[account] = excluded;
        emit ExcludeFromFeesChange(account, excluded);
    }
    function disallowAccount(address account) public onlyOwner {
        uint256 currentTimestamp = _getNow();
        require(!_isDisallowed[account], "MoonLenderTestV2: Account is already disallowed");
        require(currentTimestamp.sub(_launchStartTimestamp) < _disallowedTimeLimit, "MoonLenderTestV2: Time to disallow accounts has expired");
        _isDisallowed[account] = true;
        emit DisallowChange(account, true);
    }
    function allowAccount(address account) public onlyOwner {
        require(_isDisallowed[account], "MoonLenderTestV2: Account is already allowed");
    ...
    function setLiquidityWallet(address newAddress) public onlyOwner {
        require(liquidityWallet != newAddress, "MoonLenderTestV2: The liquidityWallet is already that address");
    ...
    function setInsuranceWallet(address newAddress) public onlyOwner {
        require(insuranceWallet != newAddress, "MoonLenderTestV2: The insuranceWallet is already that address");
        _isExcludedFromFee[newAddress] = true;
    ...
    function setDevWallet(address newAddress) public onlyOwner {
        require(devWallet != newAddress, "MoonLenderTestV2: The devWallet is already that address");
        _isExcludedFromFee[newAddress] = true;
    ...
    function setBuyFees(uint256 _liquidityFeeOnBuy, uint256 _insuranceFeeOnBuy, uint256 _devFeeOnBuy) public onlyOwner {
        require(!_swapping, "MoonLenderTestV2: Cannot change fees in swap and liquify");
    ...
    function setBurnOnSell(uint256 newValue) public onlyOwner {
        require(burnOnSell != newValue, "MoonLenderTestV2: The burnOnSell is already that amount");
        emit BurnOnSellChange(newValue, burnOnSell);
        burnOnSell = newValue;
    }
    function setUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MoonLenderTestV2: The router already has that address");
        emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function setMinimumTokensBeforeSwap(uint256 newValue) public onlyOwner {
        require(newValue != minimumTokensBeforeSwap, "MoonLenderTestV2: Cannot update minimumTokensBeforeSwap to same value");
        emit MinTokenAmountBeforeSwapChange(newValue, minimumTokensBeforeSwap);
        minimumTokensBeforeSwap = newValue;
    }
    function claimExcess(uint256 amount, address wallet) external onlyOwner {
        require(amount < address(this).balance, "MoonLenderTestV2: Cannot send more than contract balance");
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            require(isTradingEnabled, "MoonLenderTestV2: Trading is currently disabled.");
            require(!_isDisallowed[to], "MoonLenderTestV2: Account is not allowed to trade");
            require(!_isDisallowed[from], "MoonLenderTestV2: Account is not allowed to trade");

            if(currentTimestamp.sub(_launchStartTimestamp) <= 300 && isBuyFromLp) {
                require(currentTimestamp.sub(_buyTimesInLaunch[to]) > 60, "MoonLenderTestV2: Cannot buy more than once per min in first 5min of launch");
            }
        }
