...
contract Lottery49 {
  ...
  /**
    ...
    function removeAuthorized(address _toRemove) onlyOwner public {
  	...
  	function getNow() constant external returns (uint) {
        return now;
    }
    ...
    function getPeriods() constant external returns (uint32) {
        return Periods;
    }
    /**
     * @dev Set betclose
    ...
    function placeBet(int8 _type, int8 _hash, uint32 _periods, uint _tik) public payable returns (bool) {
        ...
        require(!betclose && _periods == Periods && results[_periods].status && now <= CloseTime);
        uint _wei = msg.value;
    ...
    function getPoolbyGtype(int8 _type) constant external returns (int8,uint){
        Pool storage rec= pools[_type];
        return (_type,rec.Amount);
    }
    function getAllbetByGtype(int8 _btype) constant external returns (int8,uint) {
        tGnum storage rec = tgums[Periods];
        tGtype storage rec1 = rec.tgtypes[_btype];
        return (_btype,rec1.bets);        
    }
    function getAllbetByNum(int8 _btype,int8 _bnum) constant external returns (int8,int8,uint) {
        tGnum storage rec = tgums[Periods];
        tGtype storage rec1 = rec.tgtypes[_btype];
        tNum storage rec2=rec1.tnums[_bnum];
        return (_btype,_bnum,rec2.bets);        
    }
    function getbetData(int8 _btype,int8 _bnum,address _owner) constant external returns (int8,int8,address,uint) {
        tGnum storage rec = tgums[Periods];
        tGtype storage rec1 = rec.tgtypes[_btype];
        tNum storage rec2=rec1.tnums[_bnum];
    ...
    function getResult(uint32 _periods,int8 _btype) constant external returns (int8,int8,bool){
    ...
    function getDrawStatus(uint32 _periods) constant external returns(bool){
        GamePeriods storage rec=results[_periods];
        return rec.draws;
    }
