...
contract StakingV2 is IStakingV2, Initializable, AccessControlUpgradeable {
    ...
    event AccountRegistered(address indexed account, uint256 indexed totalShares);
    ...
    modifier onlyExternalStaker() {
        require(hasRole(EXTERNAL_STAKER_ROLE, _msgSender()), 'Caller is not a external staker');
        _;
    }
    ...
    modifier pausable() {
        require(paused == false || hasRole(MIGRATOR_ROLE, _msgSender()), 'Contract is paused');
        _;
    }
    ...
    function stakeInternal(
        ...
        if (block.timestamp >= nextPayoutCall) makePayout();
        ...
        //time of staking start is block.timestamp
        uint256 start = block.timestamp;
        //time of stake end is block.timestamp + number of days * stepTimestamp which is 24 hours
        uint256 end = block.timestamp.add(stakingDays.mul(stepTimestamp));
    ...
    function calculateStakingInterest(
        ...
        for (uint256 i = firstPayout; i < lastIndex; i++) {
            uint256 payout = payouts[i].payout.mul(shares).div(payouts[i].sharesTotalSupply);
    ...
    function unstake(uint256 sessionId) external pausable {
        ...
        uint256 actualEnd = block.timestamp;
    ...
    function unstakeV1(uint256 sessionId) external pausable {
        ...
        // Unstaked already
        require(session.shares == 0 && session.withdrawn == false, 'Staking: Stake withdrawn');
    ...
    function getAmountOutAndPenalty(
        ...
        uint256 secondsStaked = block.timestamp.sub(start);
        uint256 daysStaked = secondsStaked.div(stepTimestamp);
        uint256 amountAndInterest = amount.add(stakingInterest);

        // Early
        if (stakingDays > daysStaked) {
            uint256 payOutAmount = amountAndInterest.mul(secondsStaked).div(stakingSeconds);
            ...
            return (amountAndInterest, 0);
            /** Remove late penalties for block.timestamp */
    ...
    function makePayout() public {
        require(block.timestamp >= nextPayoutCall, 'Staking: Wrong payout time');
        ...
        emit MakePayout(payout, sharesTotalSupply, block.timestamp);
    }
    ...
    function _getPayout() internal returns (uint256) {
        ...
        uint256 currentTokenTotalSupply =
            (IERC20Upgradeable(addresses.mainToken).totalSupply()).add(globalPayin);

        IToken(addresses.mainToken).burn(address(this), amountTokenInDay);
        //we add 8% inflation
        uint256 inflation =
            uint256(8).mul(currentTokenTotalSupply.add(totalStakedAmount)).div(36500);
    ...
    function _getShareRate(
        ...
        uint256 numerator = (amount.add(stakingInterest)).mul(uint256(1819).add(stakingDays));
    ...
    function unstakeInternalCommon(
        ...
        uint256 stakingInterest = calculateStakingInterest(firstPayout, lastPayout, shares);
        ...
        if (stakingDays >= basePeriod) {
            ...
            uint256 bpdAmount = calcBPD(start, actualEnd < end ? actualEnd : end, shares);
        ...
        if (penalty != 0) {
            ...
            IAuctionV2(addresses.auction).callIncomeDailyTokensTrigger(penalty);
        }
    ...
    function withdrawDivTokenInternal(address tokenAddress, uint256 _totalSharesOf) internal {
        ...
        if (tokenAddress != address(0xFFfFfFffFFfffFFfFFfFFFFFffFFFffffFfFFFfF)) {
            ...
            payable(msg.sender).transfer(tokenInterestEarned);
        }
    ...
    function rebalance(address staker, uint256 oldTotalSharesOf) internal {
        for (uint8 i = 0; i < divTokens.length(); i++) {
            ...
            if (
                totalSharesOf[staker].mul(tokenPricePerShare[divTokens.at(i)]) < tokenInterestEarned
            ) {
    ...
    function updateTokenPricePerShare(
        ...
        uint256 amountForDivs = amountBought.sub(amountForOrigin).sub(amountForBidder); //remaining is the actual amount that was used to buy the token
        ...
        if (tokenAddress != address(0xFFfFfFffFFfffFFfFFfFFFFFffFFFffffFfFFFfF)) {
            ...
            payable(bidderAddress).transfer(amountForBidder);
            payable(originAddress).transfer(amountForOrigin);
        }
    ...
    function calculateStepsFromStart() public view returns (uint256) {
        return block.timestamp.sub(startContract).div(stepTimestamp);
    }
    ...
    function findBPDs(uint256 starttime, uint256 endtime) internal view returns (uint16[2] memory) {
        uint16[2] memory bpdInterval;
        uint256 denom = stepTimestamp.mul(350);
        bpdInterval[0] = uint16(MathUpgradeable.min(5, starttime.sub(startContract).div(denom))); // (starttime - t0) // 350
    ...
    // Comment out the following or else the contract size will be to large.
    ...
    function maxShare(uint256 sessionId) external pausable {
        ...
        (uint256 newStart, uint256 newEnd, uint256 newAmount, uint256 newShares) =
            maxShareUpgrade(
                session.firstPayout,
                session.lastPayout,
                session.shares,
        ...
        addBPDMaxShares(session.shares, session.start, session.end, newShares, newStart, newEnd);
    ...
    function maxShareV1(uint256 sessionId) external pausable {
        ...
        maxShareInternal(sessionId, shares, newShares, amount, newAmount, newStart, newEnd);
    ...
    {
        require(maxShareEventActive == true, 'STAKING: Max Share event is not active');
        require(
        ...
        require(newShares > shares, 'STAKING: New shares are not greater then previous shares');
    ...
    function maxShareInternal(
        ...
        uint256 oldTotalSharesOf = totalSharesOf[msg.sender];
        totalSharesOf[msg.sender] = totalSharesOf[msg.sender].add(newShares - oldShares);
