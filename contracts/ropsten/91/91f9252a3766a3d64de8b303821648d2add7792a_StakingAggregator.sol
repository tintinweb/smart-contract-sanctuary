...
contract Staking is ReentrancyGuard, StakingInterface {
  ...
  modifier updateReward(address account) {
    ...
    for (uint256 term = startTerm; term < _currentTerm && term < startTerm + MAX_TERM; term++) {
      ...
      if (termInfo.stakeSum != 0) {
        ...
        uint256 rewardsAdded = termInfo.rewardSum.mul(accountInfo.stakeAmount).div(
          termInfo.stakeSum
        );
  ...
  function _getRewardAdded() internal view returns (uint256 rewards) {
    ...
    uint256 nextRewardSum = currentBalance.sub(
      nextStakeSum.add(_totalRemainingRewards),
      "system error"
    );
    ...
    )
  {
    ...
    currentTermRewards = _termInfoList[currentTerm].rewardSum;
...
interface StakingAggregatorInterface {
  ...
  function getAccountInfo(address account)
    ...
    returns (
      ...
      uint128[] memory nextAddedStakeAmountList,
      uint256[] memory currentRewardList,
      uint256[] memory nextLatestTermUserRewardsList,
      uint128[] memory depositAmountList,
...
contract StakingAggregator is StakingAggregatorInterface {
  ...
  function withdraw(uint128 amount) external override isEnabled {
    ...
    for (uint256 i = 0; i < _startTimestampList.length; i++) {
      ...
      uint256 untilNextTerm = _termInterval -
        ((block.timestamp - _startTimestampList[i]) % _termInterval);
      if (untilNextTerm < minUntilNextTerm) {
        minUntilNextTerm = untilNextTerm;
        nextStakingContractIndex = i;
      }
  ...
  {
    uint256 numOfStakingContracts = _stakingContracts.length;
    stakeAddList = new uint128[](numOfStakingContracts);
    stakeSumList = new uint128[](numOfStakingContracts);
    rewardSumList = new uint256[](numOfStakingContracts);
    for (uint256 i = 0; i < numOfStakingContracts; i++) {
      (uint128 stakeAdd, uint128 stakeSum, uint256 rewardSum) = _stakingContracts[i].getTermInfo(
        term
      );
    ...
    )
  {
    ...
    nextAddedStakeAmountList = new uint128[](numOfStakingContracts);
    currentRewardList = new uint256[](numOfStakingContracts);
    nextLatestTermUserRewardsList = new uint256[](numOfStakingContracts);
    depositAmountList = new uint128[](numOfStakingContracts);
    ...
    for (uint256 i = 0; i < numOfStakingContracts; i++) {
      ...
      currentRewardList[i] = currentReward;
      nextLatestTermUserRewardsList[i] = nextLatestTermUserRewards;
      depositAmountList[i] = depositAmount;
