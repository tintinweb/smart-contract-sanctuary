...
contract CryptoBees is ICryptoBees, ERC721Enumerable, Ownable, ReentrancyGuard, Pausable {
    ...
    function mintForEth(uint256 amount, bool presale) external payable whenNotPaused nonReentrant {
        traitsContract.mintForEth(_msgSender(), amount, presale, msg.value);
    }

    // function mintForHoney(uint256 amount) external whenNotPaused nonReentrant {
    //     // mintCheck(amount, false, false);
    //     MintCheck.checkETH(_msgSender(), amount, minted, false, mintWithEthPresalePaused, MAX_TOKENS, PAID_TOKENS, MINT_PRICE_DISCOUNT, MINT_PRICE);
    ...
    //         mint(minted);
    //     }
    //     woolContract.transferFrom(msg.sender, owner(), totalWoolCost);
    // }
    ...
    function mintCost(uint256 tokenId) public pure returns (uint256) {
        if (tokenId <= 20000) return 3000 ether;
