...
contract VeeProxyController is IVeeProxyController, VeeSystemController, Initializable{    
   ...
   IUniswapV2Router02 private _router;
    ...
    /**
     * @dev constructor for initializing UNISWAP router and CETH
     */
     ...
     * @return true: success; 
     */
    function excuteOrderERC20ToERC20(Order memory order,bytes32 orderId) internal onlyExecutor nonReentrant veeLock(uint8(VeeLockState.LOCK_EXECUTEDORDER)) returns (bool){  
     ...
     * @return true: success; 
     */
    function executeOrderETHToERC20(Order memory order,bytes32 orderId) internal onlyExecutor nonReentrant veeLock(uint8(VeeLockState.LOCK_EXECUTEDORDER)) returns (bool){
     ...
     * @return true: success; 
     */
    function executeOrderERC20ToETH(Order memory order,bytes32 orderId) internal onlyExecutor nonReentrant veeLock(uint8(VeeLockState.LOCK_EXECUTEDORDER)) returns (bool){
        uint256[] memory amounts = swapETHToERC20(order.tokenB, order.maxSlippage);
        emit OnTokenSwapped(orderId, order.orderOwner, order.tokenB, order.tokenA, amounts[0], amounts[1]);
     ...
     *
     */ 
    function swapETHToERC20(address tokenB,uint256 maxSlippage) internal returns (uint256[] memory){
...
contract VeeSystemController is AccessControl {
    ...
    address internal VETH = address(1);
...
interface IVeeProxyController {  
    ...
    /**
     * @notice set executor role by administrator.
     */
    function setExecutor(address newExecutor) external;

    /**
     * @notice  remove an executor role from the list by administrator.
     */
