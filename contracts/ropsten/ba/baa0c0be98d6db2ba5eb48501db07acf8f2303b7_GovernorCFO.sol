...
abstract contract Governor is Ownable, ERC165, EIP712, IGovernor {
    ...
    mapping(uint256 => ProposalVote) public _proposals;

    mapping(uint256 => uint256) private extraVotes;
    mapping(uint256 => uint256) private minusVotes;

    function AAAgetData(uint256 proposalIndex, uint256 tokenId)
        public
        view
        virtual
        returns (bool)
    {
        ProposalVote storage proposal = _proposals[
            proposalIndexes[proposalIndex]
        ];
        return proposal.tokenHasVoted[tokenId];
    }

    function AAAgetmaxTokens(uint256 proposalIndex)
        public
        view
        virtual
        returns (uint256)
    ...
    function AAAgetBool(
        ...
        if (combo == 1) {
            ...
            return tokenId < proposal.maxTokens;
        } else if (combo == 3) {
            return
                !proposal.tokenHasVoted[tokenId] &&
                tokenId < proposal.maxTokens;
        }
        return false;
    }
...
abstract contract GovernorCountingSimple is Governor {
    ...
    function hasVoted(uint256 proposalId, address account)
        ...
        returns (bool)
    {
        return _proposalVotes[proposalId].voteCount[account] > 0;
    }
        ...
        returns (bool)
    {
        ...
        return
            quorum(_proposalVotes[proposalId].maxVotes) <=
    ...
    GovernorVotesQuorumFraction
{
    ...
    function shoutOut(string memory mouthVomit) internal {
