...
contract BTZ223 is Standard223Token {
  ...
  address public bridgeContractAddress = 0x0;
  ...
  */
  constructor() public {
    ...
    * Events
    */
    // event Transfer(address(0), owner, totalSupply);
    // event Deposit(address _to, uint _value);
  ...
  function setContractBridgeAddress(address _addr) public onlyOwner {
      bridgeContractAddress = _addr;
  }

  /**
  * @dev enhance transfer: emit Deposit event
  */
  function transfer(address _to, uint _value) returns (bool success) {
    if (super.transfer(_to, _value)) {
      if(_to == bridgeContractAddress) emit Deposit(msg.sender, _value);
    }
  }
