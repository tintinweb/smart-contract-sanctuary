...
contract ArtisLifeNFTMartketplace {
    ...
    function newOffer(
        ...
        require(royalty < 10000, "Cannot set royalty higher than 100%");
        require(
            token.allowance(msg.sender, address(this)) >= price,
            "Not approved for that amount."
        );
        currentOfferCount[nftAddress][id] = currentOfferCount[nftAddress][id].add(1);
    ...
    function setForSale(
        ...
        currentSaleCount[nftAddress][id] = currentSaleCount[nftAddress][id].add(1);
        currentSaleCountByUser[nftAddress][id][msg.sender] = currentSaleCountByUser[nftAddress][id][
            msg.sender
        ].add(1);
        ...
        offerIsNotCanceled(offerId)
    {
        ...
        uint256 royalty = salePrice.mul(nftsOffer[saleId].fee).div(10000);
        ...
        currentOfferCount[nftAddress][id] = currentOfferCount[nftAddress][id].sub(1);
        currentOfferCountByUser[settlementToken][
            owner
        ] = currentOfferCountByUser[settlementToken][owner].sub(salePrice);
        emit NFTOfferAccepted(saleId, msg.sender, salePrice, royalty);
    }
        ...
        saleIsNotCanceled(saleIdentification)
    {
        ...
        uint256 royalty = salePrice.mul(nftsForSale[saleId].fee).div(10000);
        uint256 marketFee = salePrice.mul(marketplaceFee).div(10000);
        salePrice = salePrice.sub(royalty).sub(marketFee);
        ...
        token.transferFrom(msg.sender, owner, salePrice);
        ...
        currentSaleCount[nftAddress][nftID] = currentSaleCount[nftAddress][nftID].sub(1);
        currentSaleCountByUser[nftAddress][nftID][owner] = currentSaleCountByUser[nftAddress][nftID][
            owner
        ].sub(1);
        ...
        saleIsNotCanceled(saleId)
    {
        ...
        currentSaleCount[nftAddress][nftID] = currentSaleCount[nftAddress][nftID].sub(1);
        currentSaleCountByUser[nftAddress][nftID][msg.sender] = currentSaleCountByUser[nftAddress][
            nftID
        ][msg.sender].sub(1);
        ...
        offerIsNotCanceled(offerId)
    {
        ...
        currentOfferCount[nftAddress][nftID] = currentOfferCount[nftAddress][nftID].sub(1);
        currentOfferCountByUser[settlementToken][
            msg.sender
        ] = currentOfferCountByUser[settlementToken][msg.sender].sub(
            nftsOffer[offerId].price
        );
        ...
        offerIsNotCanceled(offerId)
    {
        ...
        nftsOffer[offerId].canceled = true;
        currentOfferCount[nftAddress][nftID] = currentOfferCount[nftAddress][nftID].sub(1);
        currentOfferCountByUser[nftsOffer[offerId].settlementToken][
            owner
        ] = currentOfferCountByUser[nftsOffer[offerId].settlementToken][owner]
            .sub(price);
        emit OfferCanceled(offerId);
    }
    ...
    {
        require(price > 0, "Price must be greater than zero.");
        IERC20 token = IERC20(nftsOffer[offerId].settlementToken);
        uint256 oldPrice = nftsOffer[offerId].price;
        if (token.balanceOf(msg.sender) > (price + nftsOffer[offerId].price)) {
    ...
    function updateOfferFee(
        ...
        require(fee < 10000, "Fee cannot be 100% or higher.");
        nftsOffer[offerId].fee = fee;
    ...
    function emergencyNFTWithdraw(
        ...
        require(erc == 721 || erc == 1155, "NFT Not Supported.");
        if (erc == 1155) {
            ERC1155 token = ERC1155(nftAddress);
