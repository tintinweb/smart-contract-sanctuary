...
library LibERC721LazyMint {
    ...
    bytes4 constant public ERC721_LAZY_PACKAGE = bytes4(keccak256("ERC721_LAZY_PACKAGE"));
...
abstract contract TransferManager is OwnableUpgradeable, TransferExecutor {
    ...
    function setDefaultFeeReceiver() external view onlyOwner returns (address) {
    ...
    function doTransfers(
        ...
        doTransfersWithFees(order.makeAsset.value, order.maker, order.taker, order.makeAsset.assetType, order.takeAsset.assetType);
    }
    ...
    function doTransfersWithFees(
        ...
        totalAmount = calculateTotalAmount(amount, protocolFee);
        ...
        transfer(LibAsset.Asset(matchCalculate, rest), from,  to);
        transfer(LibAsset.Asset(matchNft, 1), to,  from);
    }
    ...
    function transferProtocolFee(
        ...
        if (fee > 0) {
            transfer(LibAsset.Asset(matchCalculate, fee), from, defaultFeeReceiver);
        }
        return rest;
    }
    ...
    function transferRoyalties(
        ...
        LibPart.Part[] memory fees = getRoyalties(matchNft.data);

        (uint result, uint totalRoyalties) = transferFees(matchCalculate, rest, amount, fees, from);
        require(totalRoyalties <= 5000, "Royalties are too high (>50%)");
        return result;
    }

    function getRoyalties(bytes memory matchNftData) internal pure returns (LibPart.Part[] memory) {
        (, LibERC721LazyMint.Mint721Data memory data) = abi.decode(matchNftData, (address, LibERC721LazyMint.Mint721Data));
    ...
    function transferFees(
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            ...
            if (feeValue > 0) {
                transfer(LibAsset.Asset(matchCalculate, feeValue), from,  fees[i].account);
            }
        }
    }
...
abstract contract TransferExecutor {
    ...
    uint constant MINT_REQUEST_SIZE = 1282;
    ...
    function transfer(
        ...
        if (asset.assetType.assetClass == LibAsset.ERC20_ASSET_CLASS) {
            address erc20address = abi.decode(asset.assetType.data, (address));
            IERC20Upgradeable(erc20address).transferFrom(from, to, asset.value);
            emit Transfer(asset, from, to);
        } else if (asset.assetType.assetClass == LibERC721LazyMint.ERC721_LAZY_ASSET_CLASS) {
            (address erc721address, LibERC721LazyMint.Mint721Data memory data) = abi.decode(asset.assetType.data, (address, LibERC721LazyMint.Mint721Data));
            IERC721LazyMint(erc721address).transferFromOrMint(data, from, to);
            emit Transfer(asset, from, to);
        } else if(asset.assetType.assetClass == LibERC721LazyMint.ERC721_LAZY_PACKAGE) {
            for (uint i = 0; i < asset.value; i++) {
                bytes memory currentData = asset.assetType.data.slice(i * MINT_REQUEST_SIZE, MINT_REQUEST_SIZE); 
                (address erc721address, LibERC721LazyMint.Mint721Data memory data) = abi.decode(currentData, (address, LibERC721LazyMint.Mint721Data));
                IERC721LazyMint(erc721address).transferFromOrMint(data, from, to);
                emit Transfer(asset, from, to);
            } 
        }
...
contract OrderValidator is Initializable, ContextUpgradeable, EIP712Upgradeable {
    ...
    function __OrderValidator_init() public initializer {
        __EIP712_init("Market721", "1");
    }
...
library LibOrder {
    ...
    bytes32 constant ORDER_TYPEHASH = keccak256(
        "Order(address maker,Asset makeAsset,address taker,Asset takeAsset)Asset(AssetType assetType,uint256 value)AssetType(bytes4 assetClass,bytes data)"
    );
