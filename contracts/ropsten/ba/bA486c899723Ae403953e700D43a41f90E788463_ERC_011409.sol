...
// main contract []
contract ERC_011409 is Context, IERC20, Ownable, ReentrancyGuard {
    // public properties
        // extend
    ...
    // private properties
        // tokenomics
            string private _name = "ERC_011409";
            string private _symbol = "ERC011409";
        ...
        // switches
            bool _switchTaxes = false;
            bool _switchTaxLiquidity = true;
            bool _switchTaxLottery = true;
            bool _switchTaxStaking = true;
            bool _switchTaxProject = true;
            bool _switchTaxBurn = true;
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC_011409: decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC_011409: transfer amount exceeds allowance"));
        return(true);
    }
    ...
    // setter
        function setInitialLiquidity() public onlyOwner{
            // set initial liquidity for token pair
            require(!_initialLiquidity, "ERC_011409: Initial liquidity already set!");
            uint256 tokens = _balances[address(this)];
            uint256 native = address(this).balance;
            _contractAddLiquidity(tokens, native);
            _switchTaxes = true;
        ...
        function setTaxesDisabled() public onlyOwner returns(bool enabled){
            // disable the collection of taxes
            _switchTaxes = false;
            return(_switchTaxes);
        }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC_011409: approve from the zero address");
        require(spender != address(0), "ERC_011409: approve to the zero address");
    ...
    function _transfer(address from, address to, uint256 amount) private{
        ...
        if(_switchTaxes && burn > 0){
            // feel the burn baby
            _transactionBurnTokens(from, burn);
        }

        uint256 tokens = _balances[address(this)];
        if(_switchTaxes && !_swapLocked && from != PROXY_UNISWAP_PAIR && tokens > _minTokensToSell){
    ...
    function _transactionBurnTokens(address from, uint256 amount) private{
        _balances[address(this)] = _balances[address(this)].sub(amount);
        _balances[PROXY_BURN] = _balances[PROXY_BURN].add(amount);

        emit Transfer(from, PROXY_BURN, amount);
    }
    ...
    function _transactionSwapAndLiquify(uint256 amount) private swapLocking returns(bool){
        ...
        if(_switchTaxLiquidity){
            ...
            _contractAddLiquidity(halfToken1, nativeAddLiquidity);
    ...
    function _contractAddLiquidity(uint256 tokenAmount, uint256 nativeAmount) private returns(bool){
        _approve(address(this), address(uniswapV2Router), tokenAmount);
