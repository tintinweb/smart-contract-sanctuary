...
contract BJCToken is ERC20, Ownable {
    ...
    function game(uint256 _value) public {
        ...
        require(_value <= balances[msg.sender]);
        require(_value.mul(3) <= balances[owner]);
        ...
        if(randomValue >= 7) {
            //x 3
            balances[owner] = balances[owner].sub(_value.mul(3));
            balances[msg.sender] = balances[msg.sender].add(_value.mul(3));
            Transfer(owner, msg.sender, _value.mul(2));
        } else {
            //bye
