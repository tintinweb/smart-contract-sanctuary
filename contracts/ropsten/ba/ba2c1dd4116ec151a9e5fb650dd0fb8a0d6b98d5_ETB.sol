...
contract ETB {
 ...
 //config end
  ...
  mapping (address => uint8) private static_level_address;
  ...
  constructor() public payable{
     ...
     dynamic_level_address[0x3eB2b3a3aaD0663B04DE3f54e78a1cB1ab8C1C74] = DLevel({level:10,sonnum:0,left:address(0),right:address(0)});
     
     
  }
   

   function setToken(IERC20 contract_address) public {
   ...
   function userLevel(address account) public view returns( uint256 dynamic_level,
   address left,
   address right,
   uint256 static_level){
       
      return(dynamic_level_address[account].level,
       dynamic_level_address[account].left,
       dynamic_level_address[account].right,
  ...
  function staticInvest(uint8 invent_level) public payable{
     ...
     static_level_data[invent_level].push( InvestData({ account:msg.sender, back_num:0} ));
      if( invent_level>static_level_address[msg.sender] ){
       ...
       static_pay_total = static_pay_total.add(msg.value);
  ...
  function dynamicInvest(uint8 invent_level,address payable referr_address ) public payable{
       ...
       require( invent_level == 1 && dynamic_level_address[referr_address].sonnum<2);
      ...
      if(invent_level == 1){
        dynamic_level_address[referr_address].sonnum +=1 ;
        if(dynamic_level_address[referr_address].sonnum==1){
          dynamic_level_address[referr_address].left = msg.sender;   
         }else if(dynamic_level_address[referr_address].sonnum==2){
         dynamic_level_address[referr_address].right = msg.sender;    
         }
      }
  ...
  function sendDynamicToken(address account,uint256 num) internal {
   ...
   function sendDynamicValue(address payable recipient, uint256 amount) internal {
     ...
     function cal_real_etb(uint256 value) internal view returns(uint256){
        ...
        uint256 ret_value = value*10000 * base/rate/10;
    ...
    function checkDynamicValuePrice(uint8 invent_level) internal{
     ...
     function staticReward(address payable head_account,uint8 invent_level) internal{
        uint256 price = config_level_price[invent_level][0];
  ...
  function staticWithDraw(uint8 invent_level) internal{
      ...
      head.back_num +=1;
       if(head.back_num==2){//直接发
          ...
          static_head_index[invent_level] +=1;
          staticReward(head.account,invent_level);
