...
contract Butter is ERC721Enumerable, Ownable {
    ...
    function listedTokens() public view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](nftIndex);
        uint256 index;

        for (index = 0; index <= nftIndex; index++) {
            if(tradeStatus[index] == true){
                result[index] = index;
            }
        }
        return result;
...
contract ButterManager is Ownable {

    mapping (address => bool) public approvedContracts;

    function swap(address _recipient, uint256 _tokenID) public {
        require(approvedContracts[msg.sender] == true, "ButterManager: Run setApprovedContract for NFT Contract");
        Butter(msg.sender).butterManagerPerformSwap(_recipient, _tokenID);
    }

    function setApprovedContract(address addr, bool status) public onlyOwner {
