...
abstract contract Sale is ERC721, Ownable{
   ...
   // mapping (uint256 => address) public _tokenOwner;
    ...
    function acceptBId(address token,address from, address admin, uint256 amount, uint256 tokenId) public{
        ...
        uint256 ser=uint256(5).div(2);
        uint256 val = amount.mul(ser).div(100);
        uint256 or_am = amount.sub(val);
        uint256 roy = or_am.mul(_royal[tokenId]).div(100);
        amount = or_am.sub(roy);
        ERC20 t = ERC20(token);
        t.transferFrom(from,admin,fee);
        t.transferFrom(from,_creator[tokenId],roy);
        t.transferFrom(from,_owners[tokenId],amount);
    ...
    function saleToken(address payable admin,uint256 tokenId) public payable{
        require(msg.value > order_place[_owners[tokenId]][tokenId].price , "Insufficent found");
        ...
        //service fees
        admin.transfer(fee);
        uint256 price= order_place[_owners[tokenId]][tokenId].price;
        uint256 roy = price.mul(_royal[tokenId]).div(100);
        //royalty
        create.transfer(roy);
        //owner
        uint256 net_per=_royal[tokenId].add(5);
        uint256 net = value.mul(net_per).div(100);
        value = value.sub(net);
...
contract NFTMint is ERC721,Sale{
    ...
    function mint(string memory _name, string memory tokenuri, uint256 tokenId) public {
