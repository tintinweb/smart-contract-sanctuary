...
contract XOASIS is ERC721Enumerable, Ownable {
    ...
    function isInWhiteliste(address owner) public view returns (bool) {
    ...
    function mint(uint256 amount) public payable {
        if (publicOpen) {
            ...
            require(who > 0, "Public sale is not open");
            require(who == 1, "You have already participated in the pre-sale");
            require(amount == 1, "Can only mint 1 token in pre sale");
    ...
    function gift(address to, uint256 amount) public onlyOwner {
        uint256 supply = totalSupply();
        require(supply + amount <= maxSupply, "Purchase would exceed max supply");
        require(amount <= giftLimit, "Gift reserve exceeded with provided amount.");
