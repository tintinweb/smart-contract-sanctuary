...
contract BMIStaking is IBMIStaking, OwnableUpgradeable, AbstractDependant {
    ...
    uint256 internal constant WITHDRAWING_COOLDOWN_DURATION = 10 days;
    ...
    modifier onlyStaking() {
        require(
            _msgSender() == bmiDaiStakingAddress || _msgSender() == liquidityMiningStakingAddress,
            "BMIStk: Not a staking contract"
        );
        _;
    }
    ...
    function stakeFor(address _user, uint256 _amountBMI) external override onlyStaking {
        require(_amountBMI > 0, "BMIStk: can't stake 0 tokens");
    ...
    function unlockTokensToWithdraw(uint256 _amountBMIUnlock) external override {
        require(_amountBMIUnlock > 0, "BMIStk: can't unlock 0 tokens");

        uint256 _amountStkBMIUnlock = _convertToStkBMI(_amountBMIUnlock);
        require(
            stkBMIToken.balanceOf(_msgSender()) >= _amountStkBMIUnlock,
            "BMIStk: not enough BMI to unlock"
        );

        require(isBMIRewardUnlocked(), "BMIStk: lock up time didn't end");
    ...
    function withdraw() external override updateRewardPool {
        ...
        require(_whenCanWithdrawBMIReward != 0, "BMIStk: unlock not started/exp");
        require(_whenCanWithdrawBMIReward <= block.timestamp, "BMIStk: cooldown not reached");

        uint256 amountBMI = withdrawalsInfo[_msgSender()].amountBMIRequested;
        delete withdrawalsInfo[_msgSender()];

        require(bmiToken.balanceOf(address(this)) >= amountBMI, "BMIStk: !enough BMI tokens");

        uint256 _amountStkBMI = _convertToStkBMI(amountBMI);

        require(
            stkBMIToken.balanceOf(_msgSender()) >= _amountStkBMI,
            "BMIStk: !enough stkBMI to wthdrw"
        );
    ...
    function revokeUnusedRewardPool() external override onlyOwner updateRewardPool {
        ...
        require(contractBalance > totalPool, "BMIStk: No unused tokens revoke");
