...
contract RC3 is ERC721PresetMinterPauserAutoId {
    ...
    function mint(address to, uint _price, uint _isMusic, uint _isEnabled) external {

        _cards[id].price = _price;
        
        if(_isMusic == 1) _cards[id].category = Category.Music;
        else _cards[id].category = Category.Painting;

        if(_isEnabled == 1) _cards[id].state = TokenState.ForSale;
