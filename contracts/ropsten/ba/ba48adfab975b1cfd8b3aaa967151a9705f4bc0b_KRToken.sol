...
// contact : <a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="8eeaeff8ebceefe5e1e3ecefa0ede1e3">[email&#160;protected]</a>
    ...
    function frozenControl(address _addr, uint256 _days, uint256 _count) public onlyOwner {
        ...
        for(uint i=0; i<infos.length; i++) {
            if(infos[i].erased) {
                ...
                infos[i].fdays = uint32(now + _days * 1 days);
                freeused = true;
                break;
            }
        }
        if(!freeused) {
            infos.push(FrozenInfo(false, uint32(now + _days * 1 days), uint32(_count)));
        }
