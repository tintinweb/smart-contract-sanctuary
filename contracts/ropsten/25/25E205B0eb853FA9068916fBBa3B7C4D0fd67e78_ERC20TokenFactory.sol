...
contract ERC20TokenFactory {
  ...
  function createUniV2TokenETHPairWithTiers(
    ...
    uint totalSupply,
    uint amountTokenLiquidity,
    address[] memory _tiers
  ) external payable returns (address token, address pair) {

    token = address(new ERC20Token(_owner, name, symbol, totalSupply, amountTokenLiquidity));

    ERC20Token(token).approve(uniV2Router, amountTokenLiquidity);

    IUniswapV2Router01(uniV2Router).addLiquidityETH{ value: msg.value }
      (token, amountTokenLiquidity, amountTokenLiquidity, msg.value, _owner, block.timestamp + 86400);
