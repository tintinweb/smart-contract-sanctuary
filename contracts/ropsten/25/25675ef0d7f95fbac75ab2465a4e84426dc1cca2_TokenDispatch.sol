...
contract Management is Ownable {
    ...
    function deleteTransaction(uint transactionId) public isManager {
        require(pendingTransactions[transactionId].state == 1, 'YouSwap:NOT_EXIST');
        uint8 replace = 0;
        for (uint i = 0; i < pendingTxIds.length; i++) {
            if (1 == replace) {
                pendingTxIds[i - 1] = pendingTxIds[i];
            }

            if (transactionId == pendingTxIds[i]) {
                replace = 1;
            }
        }
        delete pendingTxIds[pendingTxIds.length - 1];
        pendingTxIds.length--;
...
contract TokenDispatch is Management {
    ...
    function updateTeam(uint8 teamId, address newAccount, address newAgent) isManager external lock returns (uint256) {
        ...
        Transaction storage transaction =  pendingTransactions[transactionId];
    ...
    function claim(uint8 teamId) external lock {
        ...
        require(teams[teamId].agent == msg.sender || teams[teamId].account == msg.sender, 'YouSwap: CALLER_IS_NOT_THE_OWNER_OR_AGENT');
