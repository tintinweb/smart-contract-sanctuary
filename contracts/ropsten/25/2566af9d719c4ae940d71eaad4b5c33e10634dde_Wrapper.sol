...
contract Wrapper is Utils {
    ...
    function testing_third(KyberNetwork network, ERC20 token) public view returns(address[]) {
        uint numReserves = network.getNumReserves();
        address[] memory result = new address[](2 * numReserves);
        for(uint i = 0; i < numReserves; i++) {
            result[i] = network.reservesPerTokenSrc(token, i);
            result[i + numReserves] = network.reservesPerTokenDest(token, i);
        }
        return result;
    }

    // Return list src and dest reserves for a given token
    function getListReserves(KyberNetwork network, ERC20 token) public view returns(uint, address, address, address[], address[]) {
        uint numReserves = network.getNumReserves();
        uint i = 0;
