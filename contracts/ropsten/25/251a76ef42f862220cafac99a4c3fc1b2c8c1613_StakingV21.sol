...
contract StakingV21 is IStakingV21, Initializable, AccessControlUpgradeable {
    ...
    mapping(address => mapping(uint256 => Session)) public override sessionDataOf;
    ...
    function transferTokens(address vcAuction, address stakeManager) external onlyMigrator {
        for (uint8 i = 0; i < divTokens.length(); i++) {
            if (divTokens.at(i) != address(0xFFfFfFffFFfffFFfFFfFFFFFffFFFffffFfFFFfF)) {
                IERC20Upgradeable token = IERC20Upgradeable(divTokens.at(i));

                token.transfer(vcAuction, token.balanceOf(address(this)));
            } else {
                payable(vcAuction).transfer(address(this).balance);
            }
        }

        IERC20Upgradeable axn = IERC20Upgradeable(addresses.mainToken);
        axn.transfer(stakeManager, axn.balanceOf(address(this)));
    }
    ...
    // only needed for v3
    function resolveTotalSharesOf(address account) external view override returns (uint256) {
        uint256 totalShares;
        ...
        for (uint256 i = 0; i < sessionsOfAccount.length; i++) {
            ...
            totalShares = totalShares.add(sessionDataOf[account][sessionsOfAccount[i]].shares); //sum total shares
        }
        ...
        for (uint256 i = 0; i < v1SessionsOfAccount.length; i++) {
            ...
            (, , , uint256 shares, ) = stakingV1.sessionDataOf(account, v1SessionsOfAccount[i]);
    ...
    function getTotalSharesOf(address account) external view override returns (uint256) {
    ...
    function getIsVCARegistered(address staker) external view override returns (bool) {
    ...
    function getTokenPricePerShare(address tokenAddress) external view override returns (uint256) {
        return tokenPricePerShare[tokenAddress];
    }
