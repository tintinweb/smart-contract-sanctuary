...
contract LiquidityBalancer is IGovernable, Controllable {
  ...
  uint256 constant internal PRECISION_DECIMALS = 18;
  ...
  function changeLiquidity(address _token, address _lp, uint256 _sellAmount, uint256 _remAmount)
  public onlyHardWorkerOrGovernance {
    ...
    (address oppositeToken, uint256 oppositeTokenStacked, uint256 price) = getLpInfo(_lp, _token);
    uint256 currentTvl = oppositeTokenStacked.mul(2);

    bool needToSell = _sellAmount > 0 && tokenBalance >= _sellAmount && price > priceTargets[_token];
    bool needToRemove = _remAmount > 0 && lpBalance >= _remAmount && currentTvl > lpTvlTargets[_lp];

    if (needToRemove) {
      removeLiquidity(_lp, _remAmount, _token, oppositeToken);
      // buy target tokens
  ...
  internal view returns (address oppositeToken, uint256 oppositeTokenStacked, uint256 price) {
    ...
    return (oppositeToken, oppositeTokenStacked, price);
  }
