...
// 2% will go to contract to pay gas fees.
// 3% will go to social cause.
...
contract LotteryContract is Context, Ownable {
    ...
    uint256 _defaultLotteryPrice;
    uint256 _winingTimeInSeconds;
    uint256 _defaultMaxWinners;
    event LotteryBought(
    ...
    function buyLottery(address token) external {
        ...
        require(lotteryAmount > 0, "lottery amount must be grater then zero");
    ...
    function checkRules(address token) public {
        if (_currentLotNumber[token] == 0) {
            ...
            lotteryForToken[token].lots[_currentLotNumber[token]]
                .maxParticipant = _defaultMaxWinners; // TODO : set dynamic participant and time and amount
        }
        if (
            lotteryForToken[token].lots[_currentLotNumber[token]]
        ...
        if (
            ...
            lotteryForToken[token].lots[_currentLotNumber[token]]
                .participants
                .length >=
            lotteryForToken[token].lots[_currentLotNumber[token]].maxParticipant
        ) {
            pickWinner(token);
        }
    }
    ...
    function pickWinner(address token) private {
        ...
        setLotteryRules(token);
    }

    function setLotteryRules(address token) private {
        if (
            lotteryForToken[token].lots[_currentLotNumber[token] - 1]
                .maxParticipant >=
            lotteryForToken[token].lots[_currentLotNumber[token] - 1]
                .participants
                .length
            ...
            lotteryForToken[token].lots[_currentLotNumber[token]]
                .maxParticipant = lotteryForToken[token].lots[
                _currentLotNumber[token] - 1
            ]
                .maxParticipant
                .add(1);
        } else {
...
//TODO:
//user can transfer balance if they changed their mind
//user can buy lottery by sending ether