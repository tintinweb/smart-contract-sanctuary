...
contract NFTContract is ERC721, ERC721URIStorage, Ownable {
    address wbnbAddress;
    constructor(string memory tokenName, string memory tokenSymbol, address _wbnb) ERC721(tokenName, tokenSymbol){
        wbnbAddress = _wbnb;
    }
    ...
    function transferByAcceptOffer(uint256 tokenId, address newOwner, bytes32 hash, bytes memory signature, uint payThrough) external payable onlyNftOwner(tokenId) {
        address signer = hash.recover(signature);
        // require(signer == msg.sender, "Invalid User signature");
        ...
        transferwbnbToOwner(newOwner, amountToTransfer, msg.sender);
        emit OfferAccepted(tokenId,bidsOffers[tokenId][newOwner],msg.sender,newOwner);
    }
    function transferByBid(uint256 tokenId, address newOwner, bytes32 hash, bytes memory signature, uint payThrough) external payable onlyNftOwner(tokenId) {
        address signer = hash.recover(signature);
        // require(signer == msg.sender, "Invalid User signature");
    ...
    // fallback function to receive direct payments/bnb sent by metamask (for testing)
    ...
    function transferwbnbToOwner(address buyer, uint256 amount, address ownerOfNFT) private {
        WBNB wbnb = WBNB(wbnbAddress);
        uint256 balance = wbnb.balanceOf(buyer);
        require(balance >= amount, "insufficient balance" );
        wbnb.transferFrom(buyer, ownerOfNFT, amount);
    }
}