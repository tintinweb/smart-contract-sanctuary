...
function checkerAndActivator() public returns(string memory)
{
     if(poolBaseCurrencyFXP.balanceOf(address(this)) >= proposedFXPInVault)
     {
...
*/
function requestSwap(address  _requestor, uint256 _amount) public nonReentrant poolActive
{
    ...
    proposedFXPInVault = tokenContractAddress.balanceOf(address(this)) - swapedsufxpamount;
    ...
    /* not using burn function as it may not be possible for those BEP20 tokens which don't have burn functions defined
...
//checker and activator --- call function from pool
 function poolCheckerAndActivator(BEP20Token _tokenaddr) external returns(string memory)
 {
     require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
     return(PoolsRecords[address(_tokenaddr)].tokenPool.checkerAndActivator());
 }
...
function getTokenPoolName(BEP20Token _tokenaddr) external view returns(string memory)
{
     require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
