...
contract AstropenGatekeeper is Ownable {
    address internal constant DEAD_ADDRESS =
        0x000000000000000000000000000000000000dEaD;

    ERC1155Tradable public astropenNFT;
    mapping(address => mapping(uint256 => bool)) public ownerToUsedToken;

    event AstropenNFTChanged(address newAddress);
    event AstropenNFTUsed(address user, uint256 tokenId);

    constructor(address _astropenNFTAddress) {
        astropenNFT = ERC1155Tradable(_astropenNFTAddress);
    }

    function setAstropenNFT(address _astropenNFTAddress) external onlyOwner() {
        astropenNFT = ERC1155Tradable(_astropenNFTAddress);
        emit AstropenNFTChanged(_astropenNFTAddress);
    }

    function useToken(uint256 _tokenId) external {
        require(
            ownerToUsedToken[msg.sender][_tokenId] == false,
            "Token was used before"
        );
        require(
            astropenNFT.balanceOf(msg.sender, _tokenId) > 0,
            "Balance is not enough"
        );
        astropenNFT.safeTransferFrom(msg.sender, DEAD_ADDRESS, _tokenId, 1, "");
        ownerToUsedToken[msg.sender][_tokenId] = true;
        emit AstropenNFTUsed(msg.sender, _tokenId);
    }
}
