...
contract Precog is IPrecog {
    ...
    function _liquidate(address futureContract) internal returns(uint256 amount) {
        ...
        amount = tradingAmountOnFutureContract[msg.sender][futureContract] + profitAmountOnFutureContract[msg.sender][futureContract];
        ...
        IERC20(tokenA).approve(futureExchange, type(uint256).max);
        IERC20(tokenB).approve(futureExchange, type(uint256).max);
        
        IFutureExchangeRouter(futureExchange).closeFuture(tokenInvest, usdc, expiryDate, address(this), amount);
        
        tradingAmount[msg.sender] -= tradingAmountOnFutureContract[msg.sender][futureContract];
        profitAmount[msg.sender] -= profitAmountOnFutureContract[msg.sender][futureContract];
        tradingAmountOnFutureContract[msg.sender][futureContract] = 0;
        profitAmountOnFutureContract[msg.sender][futureContract] = 0;
    ...
    function invest(
        ...
        tradingAmountOnFutureContract[user][address(futureContract)] += amount + feeTradingUsdc;
        profitAmount[user] += profit;
        profitAmountOnFutureContract[user][address(futureContract)] += profit;
