...
contract TokenERC20 {
    ...
    mapping (uint => address payable) shareholders;
...
function payDividends() external onlyOwner {
        require(Balance4Dividends > 0 && totalShareholders > 0);
            for (uint i = 1; i <= totalShareholders; i++) {
                ...
                if (currentBalance > 0) {
                    uint256 amount = Balance4Dividends * currentBalance / shareholdersBalance;
                    shareholders[i].transfer(amount/10**18);
                }
            }
        }
receive() external payable {
     require(msg.value/10**18 > 0 && msg.value/10**18 >= _tokenPrice);
        ...
        if (registeredShareholders[msg.sender] == false) { 
            totalShareholders += 1;
            shareholders[totalShareholders] = payable(msg.sender);
