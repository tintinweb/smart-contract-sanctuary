...
contract Mycontract 
{
    ...
    Userdetails[peoplecount].initialpayment= 0;
     ...
     function getfunction(uint _ID,uint[] memory _Id,uint[]memory _booktakentime,uint[] memory _bookstaken)public  {
        
        
         for(uint i=0; i<5; i++) {
          Userdetails[_ID].Bookid.push(_Id[i]);
          Userdetails[_ID].booktakentime.push(_booktakentime[i]);
         
          Userdetails[_ID].bookstaken.push(_bookstaken[i]);  
          Totalbooks.push(_bookstaken[i]);
          numbercount = numbercount - _bookstaken[i];
         }
    ...
    function ReturnBook(uint _ID, uint[] memory _Id, uint[] memory _Booksreturn, uint _initialpayment)public{
        ...
        for(uint i=0; i<5; i++) {
        Userdetails[_ID].Bookid.push(_Id[i]);
        Userdetails[_ID].Booksreturn.push(_Booksreturn[i]);
        Userdetails[_ID].initialpayment =Userdetails[_ID].initialpayment - fine;
        numbercount = numbercount + _Booksreturn[i];
