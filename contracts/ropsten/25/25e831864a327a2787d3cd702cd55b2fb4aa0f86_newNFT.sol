...
contract newNFT is Ownable, ERC721{
    ...
    function setRustyK(address rusty) external onlyOwner {
        rustyK = rusty;
    }
    ...
    function mintNewNFT( uint256 tokenID, address _contract) external  {
       ...
       // (bool success, bytes memory data) = _contract.call(bytes32(keccak256("burn(uint256)")),tokenID);
        mtrac = success;
        dta = data;
         if(success){
            totalSupply++;
    ...
    function mintNFT( uint256 tokenID, address msgsender) external {
        require(mintIsActive, "Minting has not commenced or has ended");
        require(totalSupply < MAX_SUPPLY, "All Claimed!");
        require(msg.sender == rustyK, "Unauthorised Mint!");
        totalSupply++;
