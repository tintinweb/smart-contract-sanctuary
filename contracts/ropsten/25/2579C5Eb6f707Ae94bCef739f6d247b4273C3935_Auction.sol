...
contract Auction is Ownable {
    ...
    event Bidded(address indexed user, address indexed asset, uint256 amount);
    event CanceledAll(address indexed user, address indexed asset, uint256 amount);
    event Refund(address indexed user, address indexed asset, uint256 amount);
    event KeepersExported(uint256 amount, address asset, address[] keepers);
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _assets.length; i++) {
            ...
            assetMeta[_assets[i]] = AssetMeta(i, 10**(decimals - BTC_DECIMALS));
        }
        assetList = _assets;
    }
    ...
    function bid(address _asset, uint256 _amount) external inState(State.Bid) {
        ...
        require(meta.divisor != 0, "unknown asset");
        ...
        if (userBids.asset == address(0)) {
            userBids.asset = _asset;
            bidderCount += 1;
        } else {
            require(userBids.asset == _asset, "bidding multiple assets is not allowed");
        }
        userBids.uniAmount += uniAmount;

        require(ERC20(_asset).transferFrom(msg.sender, address(this), _amount));
        emit Bidded(msg.sender, _asset, _amount);
    }
    ...
    function exportKeepers() external inState(State.ChallengeEnded) onlyOwner {
        ...
        for (uint256 i = 0; i < assetList.length; i++) {
            ...
            uint256 amount = collateralAmount * assetMeta[asset].divisor;
            address[] memory assetKeepers = new address[](keepers.length);
            uint256 total;

            for (uint256 j = 0; j < keepers.length; j++) {
                address keeper = keepers[j];
                UserBids storage userBids = allUserBids[keeper];
                if (asset == userBids.asset) {
                    assetKeepers[j] = keeper;
                    userBids.uniAmount -= collateralAmount;
                    total += amount;
                }
            }

            if (total > 0) {
                require(ERC20(asset).approve(address(keeperImport), total));
                keeperImport.importKeepers(amount, asset, assetKeepers);

                emit KeepersExported(amount, asset, assetKeepers);
            }
        }
    }
    ...
    function cancelAllOrRefund(bool _isCancel) private {
        UserBids storage userBids = allUserBids[msg.sender];
        address asset = userBids.asset;
        uint256 amount = userBids.uniAmount * assetMeta[asset].divisor;

        if (amount > 0) {
            userBids.uniAmount = 0;
            require(ERC20(asset).transfer(msg.sender, amount));

            if (_isCancel) {
                emit CanceledAll(msg.sender, asset, amount);
            } else {
                emit Refund(msg.sender, asset, amount);
            }
        }
    }
