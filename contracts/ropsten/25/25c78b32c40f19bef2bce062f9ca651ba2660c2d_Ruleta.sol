...
// (En caso de que salgo cero se maracara con una salida igual a 37 salvo para la apostar_en_numero):
//   - apostar_en_numero
//      - input: entero (0, ... ,36)
...
contract Ruleta {
    ...
    function apostar_en_color(uint color) payable public returns(string memory, uint, uint){
        ...
        if (num_ruleta == 0){
            casino.balance += apuesta_jugador;
            return ('Lo siento has perdido ha salido 0', color, 37);
        }
        
        uint color_ruleta; // 0 = rojo , 1 = negro
    
        if (num_ruleta == 1 || num_ruleta == 3 || num_ruleta == 5 || num_ruleta == 7 || num_ruleta == 9 || num_ruleta == 12 || num_ruleta == 14 || num_ruleta == 16 
            || num_ruleta == 18 || num_ruleta == 19 || num_ruleta == 21 || num_ruleta == 23 || num_ruleta == 25 || num_ruleta == 27 || num_ruleta == 30 
            || num_ruleta == 32 || num_ruleta == 34 || num_ruleta == 36) {
            color_ruleta = 0;
        } else {
            color_ruleta = 1;
        }

        if (color == color_ruleta) {
    ...
    function apostar_en_par_impar(uint impar) payable public returns(string memory, uint, uint){
        ...
        if (num_ruleta == 0){
            casino.balance += apuesta_jugador;
            return ('Lo siento has perdido ha salido 0', impar, 37);
        }
    ...
    function apostar_en_docena(uint docena) payable public returns(string memory, uint, uint){
        ...
        if (num_ruleta == 0){
            casino.balance += apuesta_jugador;
            return ('Lo siento has perdido ha salido 0', docena, 37);
        }
    ...
    function apostar_en_bajo_alto(uint alto) payable public returns(string memory, uint, uint){
        ...
        if (num_ruleta == 0){
            casino.balance += apuesta_jugador;
            return ('Lo siento has perdido ha salido 0', alto, 37);
        }
