...
contract Sign {
    ...
    function encode(string memory uri, uint256 len) public pure returns(bytes memory) {
        return abi.encodePacked("\x19Ethereum Signed Message:\n", len, uri);
    } 
    function hash(bytes memory value) public pure returns (bytes32) { 
        return keccak256(value);
    }
    function recoverAddress(bytes32 h, uint8 v, bytes32 r, bytes32 s) public pure returns(address) { 
