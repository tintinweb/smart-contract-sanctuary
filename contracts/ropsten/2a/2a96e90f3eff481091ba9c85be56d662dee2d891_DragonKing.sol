...
contract DragonKing is mortal {
	...
	uint64 constant public OneDayInMillis = 25 * 60 * 60 * 1000;
	...
	function exit() public {
		...
		for (uint16 i = 0; i < nchars; i++) {
			if (characters[ids[i]].owner == msg.sender 
					&& characters[ids[i]].purchaseTimestamp + OneDayInMillis < now
				...
				while (nchars > 0 
						...
						&& characters[ids[nchars - 1]].purchaseTimestamp + OneDayInMillis < now
	...
	*/
	...
	function sellCharacter(uint32 characterId) public {
		...
		require(characters[characterId].purchaseTimestamp + OneDayInMillis < now);
	 ...
	 * */
	function getCharacter(uint32 characterId) constant public returns(uint8, uint128, address, uint64) {
		return (characters[characterId].characterType, characters[characterId].value, characters[characterId].owner, characters[characterId].purchaseTimestamp);
	}
