...
contract rarity_manager_v2 is IERC721Receiver {
    ...
    function onERC721Received(address ,address ,uint256 _tokenId,bytes calldata) public virtual override  returns (bytes4){
        IERC721 _erc721=IERC721(msg.sender);
        _erc721.transferFrom(address(this), _owner, _tokenId); //收到直接发送到owner
    ...
    //批量制造装备
    function crafting(uint8 _base_type, uint8 _item_type,uint _maxCount) external{
        require(_owner==msg.sender);
    ...
    //汇总金币
    function gold_summary_batch(uint _to,uint _start,uint _count,uint _max_gold) external {
        require(_owner==msg.sender);
        ...
        for (uint i = _start; i<_len; i++) {
            ...
            if(_max_gold>_gold.balanceOf(_from)){
