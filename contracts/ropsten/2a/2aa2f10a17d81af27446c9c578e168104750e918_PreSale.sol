...
// File: contracts/PreSale/PreSaleBy1155.sol

contract PreSale is ERC1155, Ownable {

	using SafeMath for uint256;
	
	/**
	 * init token general proof
	 */
	
	uint256 public constant PFC = 0; // General proof

    uint256 public constant THOR = 1; // thor
	...
	Token private BUSDToken;
  	...
  	mapping(uint => BoxInfo) public productinfo;
	...
	constructor(Token usdt_, uint presaletime, uint time) ERC1155("https://contract.trade.rest/ERC1155/{id}.json") {
		 maker = msg.sender;
		 releaseTime = time;
		 preSaleTime = presaletime;
		 _mint(msg.sender, PFC, 10**18, "");
		 usdt = address(usdt_);
		 BUSDToken = usdt_;
	}

	// Add PreSale product
	function AddProduct(uint[] memory _boxId,uint[] calldata _price, uint[] calldata _number, string[] calldata name, string[] calldata tokenURI)
    	public  
    	virtual 
    	returns (bool) 
	{
		require(address(maker) == address(msg.sender), "Only maker can Add Product");
		require(_boxId.length == _price.length, "Data parameter length error");
		require(_price.length == _number.length, "Data parameter length error");
		for(uint i=0;i< _price.length; i=i.add(1)) {
			uint boxId = _boxId[i];
			uint Price = _price[i];
			...
			BoxInfo memory infos = BoxInfo(name[i], tokenURI[i]);
			// update product info
			productinfo[boxId] = infos;

			Stock memory productInfo = Stock(boxId, Price, Number);
			for(uint s=0;s < PlayerAddress.length; s=s.add(1)){
				if(Player[boxId][PlayerAddress[i]].weight > 0 && Player[boxId][PlayerAddress[s]].attend){
					buyed = true;
				}
			}
			if(!buyed){
				// If exists. update or add
				for(uint p=0; p < stocks.length;p=p.add(1)) {
					if(stocks[p].boxId == boxId){
						hasStocks = true;
						stocks[p].number = Number;
						productNum = productNum.sub(1);
					}
				}
				if(!hasStocks){
					stocks.push(productInfo);
	...
	function OffRackBlindBox(uint boxId_) public returns (bool) {
		...
		bool buyed = false;
		for(uint s=0;s < PlayerAddress.length; s=s.add(1)){
			if(Player[boxId_][PlayerAddress[s]].weight > 0 && Player[boxId_][PlayerAddress[s]].attend){
				buyed = true;
			}
		}
		if(!buyed){
	...
	function BuyBlindBox(uint boxId_, uint num_) public payable virtual returns (bool) {
		...
		for(uint i=0; i < stocks.length;i=i.add(1)) {
			if(stocks[i].boxId == boxId_){
				stocks[i].number = stocks[i].number.sub(num_);
			}
		}
		stock[boxId_].number = stock[boxId_].number.sub(num_);
		// Update Billboard
		Billboard[msg.sender] = Billboard[msg.sender].add(price);
	...
	function refund(uint boxId_) public virtual returns (bool) {
		...
		for(uint i=0; i < stocks.length;i=i.add(1)) {
			if(stocks[i].boxId == boxId_){
				stocks[i].number = stocks[i].number.add(buyer.weight);
			}
		}
		stock[boxId_].number = stock[boxId_].number.add(buyer.weight);
		Billboard[msg.sender] = Billboard[msg.sender].sub(buyer.amount);
	...
	function LaunchWithdrawal() onlyOwner public virtual returns(bool) {
		state = ProductLaunchState.Launch;
		uint256 amount = totalPreSalePrice;
// 		super.safeTransferFrom(address(this), address(msg.sender), ids, amount, "0x0");
		BUSDToken.transfer(address(this), amount);
