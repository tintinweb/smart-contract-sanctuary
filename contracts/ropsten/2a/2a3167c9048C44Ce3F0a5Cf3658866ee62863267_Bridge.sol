...
contract Bridge is IMinter, IAssetLocker, Ownable, ReentrancyGuard {
    ...
    function _unlockOrMint(
        ...
        if (_canonAsset != address(0x0)) {
            {
                uint256 decimals = IERC20Metadata(_canonAsset).decimals();
...

            {
                ...
                uint256 _canonTransfer = Math.min(_bal, _amount);
                if (_canonTransfer != 0) {
                    if (_canonAsset != nativeWrapper) {
                        IERC20(_canonAsset).safeTransfer(
                            _recipient,
                            _canonTransfer
                        );
                    } else {
                        INativeWrapper(nativeWrapper).withdraw(_canonTransfer);
                        payable(_recipient).transfer(_canonTransfer);
                    }
                    emit Unlock(
                        _srcChainId,
                        _dstChainId,
