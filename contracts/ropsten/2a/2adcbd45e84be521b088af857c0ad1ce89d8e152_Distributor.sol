...
contract Distributor is Policy {
    ...
    constructor( address _treasury, address _wom, uint32 _epochLength, uint32 _nextEpochTime ) {        
        require( _treasury != address(0) );
        treasury = _treasury;
        require( _wom != address(0) );
        WOM = _wom;
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return IERC20( WOM ).totalSupply().mul( _rate ).div( 1000000 );
    }
