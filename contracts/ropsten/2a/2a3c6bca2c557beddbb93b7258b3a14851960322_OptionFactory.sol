...
contract OptionFactory is IOptionFactory, Initializable, ReentrancyGuard, Ownable {
  ...
  ) external override onlyOptionCreator nonReentrant
  {
    ...
    require(_baseAmount > 0, "ZOOM: invalid _baseAmount(amount less or equal to zero)");
    require(_targetAmout > 0, "ZOOM: invalid _targetAmout(amount less or equal to zero)");
    require(block.timestamp < _expirationTimestamp, "ZOOM: invalid expiration date");
    require(_expirationDelay > 0, "ZOOM: invalid _expirationDelay(value less or equal to zero)");
    require(_optionTokenDecimals > 0, "ZOOM: invalid _optionTokenDecimals(value less or equal zero)");
    require(_totalSupply > 0, "ZOOM: invalid _totalSupply(value less or equal to zero)");
  ...
  function validOptionStatus(address _optionAddress) external view override {
    require(optionAddressToNonce[_optionAddress] > 0,"ZOOM: invalid optionToken address");
    require(activeOptions[_optionAddress],"ZOOM: optionToken is inactived");
  }
  ...
  function setActive(address _optionAddress, bool _active) external  override onlyGovernance returns (bool) {
    require(optionAddressToNonce[_optionAddress] > 0,"ZOOM: invalid optionToken address");
    require(activeOptions[_optionAddress] != _active, "ZOOM: the same option status to set!" );
