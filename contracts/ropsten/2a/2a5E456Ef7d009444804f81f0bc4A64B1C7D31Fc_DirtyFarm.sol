...
contract DirtyFarm is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accDirtyPerShare; // Accumulated DIRTYCASH tokens per share, times 1e6. See below.
    }
    ...
    function getDirtyPerBlock() public view returns (uint256, bool) {
        ...
        uint256 poolReward = dirty.balanceOf(address(this));
        if (poolReward == 0) {
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accDirtyPerShare = accDirtyPerShare.add(dirtyReward.mul(1e6).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public updateDirtyPerBlock {
        ...
        // accumulated DIRTYCASH per share is stored multiplied by 10^6 to allow small 'fractional' values
        pool.accDirtyPerShare = pool.accDirtyPerShare.add(dirtyReward.mul(1e6).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function setBlocksPerDay(uint256 _blocksPerDay) external onlyOwner {
        require(_blocksPerDay >= 0 && _blocksPerDay <= 14000, "Value is outside of range 6200-14000");
