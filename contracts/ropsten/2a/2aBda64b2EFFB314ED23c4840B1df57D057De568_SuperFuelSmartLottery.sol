...
contract SuperFuelSmartLottery is ISmartLottery, LPSwapSupport, LockableFunction, AuthorizedListExt, UserInfoManager {
    ...
    function rewardCurrency() external view override returns(string memory){
        return string(abi.encodePacked(rewardTokenInfo.name));
    }
    ...
    function setJackpotToCurrency() external virtual override authorized {
    ...
    function resetToToken(address _tokenAddress) private lockTheSwap {
        ...
        if(rewardType == RewardType.TOKEN){
            ...
            if(contractBalance > lotteryToken.decimals())
                swapTokensForCurrencyAdv(address(lotteryToken), contractBalance, address(this));
        }
...
abstract contract LPSwapSupport is Ownable {
    ...
    uint256 public minSpendAmount = 0.01 ether;
