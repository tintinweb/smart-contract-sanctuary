...
contract Asset is InitialAsset, IAsset {
    ...
    function __Asset_init(
        ...
        address[4] memory oracleDexRouterDexFactoryAndZVault,
        ...
        address weth_ = IPancakeRouter02(oracleDexRouterDexFactoryAndZVault[1]).WETH();
        InitialAsset.__InitialAsset_init(
            nameSymbol[0],
            nameSymbol[1],
            oracleDexRouterDexFactoryAndZVault[0],
            oracleDexRouterDexFactoryAndZVault[2],
            imeTimeInfo[0],
            imeTimeInfo[1],
            _tokenWhitelist,
            weth_
        ...
        dexRouter = oracleDexRouterDexFactoryAndZVault[1];
        zVault = oracleDexRouterDexFactoryAndZVault[3];

        AssetLib.checkIfTokensHavePair(_tokensInAsset, oracleDexRouterDexFactoryAndZVault[2], weth_);
        ...
        nonReentrant
    {
        ...
        AssetLib.checkIfTokensHavePair(newTokensInAsset, dexFactory, _weth);
    ...
    function setDescription(string memory newValue) external onlyManagerOrAdmin {
...
contract InitialAsset is ERC20Upgradeable, ReentrancyGuard {
    ...
    function __InitialAsset_init(
        ...
        require(_imeStartTimestamp < _imeEndTimestamp, "Wrong ime time");
        imeStartTimestamp = _imeStartTimestamp;
        imeEndTimestamp = _imeEndTimestamp;

        AssetLib.checkAndWriteWhitelist(_tokenWhitelist, [_dexFactory, weth_], _whitelistIndexes, isTokenWhitelisted);
...
library AssetLib {
    ...
    function checkIfTokensHavePair(
        address[] memory tokens,
        address dexFactory,
        address weth
    ) public view {
    ...
    function checkAndWriteWhitelist(
        ...
        address[2] memory dexFactoryAndWeth,
        mapping(address => uint256) storage _whitelistIndexes,
        mapping(address => bool) storage isTokenWhitelisted
    ) external {
        checkIfTokensHavePair(tokenWhitelist, dexFactoryAndWeth[0], dexFactoryAndWeth[1]);
    ...
    function changeWhitelist(
        ...
        if (value == false && oldValue == true) {
            ...
            temp[0] = token;
            checkIfTokensHavePair(temp, dexFactoryAndWeth[0], dexFactoryAndWeth[1]);
...

{
  ...
  "libraries": {
    "/Users/tepnik/Desktop/ydragon-contracts/contracts/lib/AssetLib.sol": {
      "AssetLib": "0x7Ed95731690dA8218C91E39Bb883823a79605348"
    },
    "/Users/tepnik/Desktop/ydragon-contracts/contracts/lib/AssetLib2.sol": {
      "AssetLib2": "0x009b6c5326D6fcD268105823C7671E3960d8541a"
    }
  },
  "outputSelection": {
    "*": {
