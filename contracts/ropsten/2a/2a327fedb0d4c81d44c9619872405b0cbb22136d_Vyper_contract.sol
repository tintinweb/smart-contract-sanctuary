# Slinky - An ultra deflationary token inspired on eip1559, deflationary and inflationary by periods.
#  
# Slinky automatically burns, mints, and sent airdrops to buyers
# features a perpetual dynamic supply range between 100,000 Slinky and 100 Slinky
#
# Find out more about SlinkyFinance @ http://slinkyfinance.xyz/
#
# Official Telegram @ https://t.me/slinkyfinance
...
def __init__(_name: String[64], _symbol: String[32], _decimals: uint256, _supply: uint256, _min_supply: uint256, _max_supply: uint256):
    ...
    self.burn_pct = convert(0.0250 * deciCalc, uint256)
...
def _macro_contraction_bounds() -> bool:
    if self.isBurning == True:
        self.min_supply = self.min_supply / (1 + (3/100))
    else:
        self.max_supply = self.max_supply / (1 + (3/100))
    return True
...
def _turn() -> bool:
    ...
    if self.turn == 1 and self.firstrun == False:
        ...
        self.treasury_pct = convert(0.0050 * deciCalc, uint256)
        self.macro_contraction = True
    if self.turn >= 2 and self.turn <= 464:
        self._macro_contraction_bounds()
        self.macro_contraction = True
    elif self.turn >= 465 and self.turn <= 928:
...
def manager_killswitch() -> bool:
    # Anyone can take the manager controls away on Sunday, March 30, 2021 23:59:59  GMT
    assert msg.sender != ZERO_ADDRESS
    assert block.timestamp > 1617148799
...
def transfer(_to : address, _value : uint256) -> bool:
    ...
    if msg.sender != self.owner:
        if self.botThrottling == True:
            if self.tx_n < 70:
                assert _value < 1000 * 10 ** self.decimals, "Maximum amount allowed is 1000 Slinky until the 70th transaction."
