...
contract ProxyController is AccessControl, IProxyController {
    ...
    function startOrder(OrderInput memory input) public virtual onlyRole(EXECUTOR_ROLE) {
        ...
        emit OrderCreated(orderId, input.tokenA, input.tokenB, borrower, input.limit, input.stop, input.expiry, input.autoRepay);
     ...
     * - `orderId` must exist
     */
    function checkOrder(bytes32 orderId) public virtual onlyRole(EXECUTOR_ROLE) returns (uint256) {
...
interface IProxyController {
     ...
     * @dev Order created.
     */
    event OrderCreated(bytes32 indexed orderId, address tokenA, address tokenB, address indexed borrower, uint256 limit, uint256 stop, uint256 expiry, bool autoRepay);
