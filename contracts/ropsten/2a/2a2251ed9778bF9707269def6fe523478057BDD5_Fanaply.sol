...
contract Fanaply 
{
	...
	event ContractCreated(address new_contract_address, string name, uint contractId);
	...
	function createSingleHandler(
		...
		string memory _tokenIPFSHash,
		address _checkAccessContract
	) public returns (SingleHandler tokenAddress) 
	{
		CheckAccess check = CheckAccess(_checkAccessContract);
		require( check.isAdmin(msg.sender), "wrong admin" );

		tokenAddress = new SingleHandler( _name, _symbol, _maxTokensCount, _tokenURI, _tokenIPFSHash, _checkAccessContract );

		address externalId = address(tokenAddress);

		contracts.push(externalId);

		emit ContractCreated(externalId, _name, contractId++);
	}
}