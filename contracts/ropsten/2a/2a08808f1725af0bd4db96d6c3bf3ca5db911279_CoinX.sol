...
contract CoinX is ERC20, COINXminting, Ownable {
    using SafeMath for uint256;

    string public name = "CoinX";
    string public symbol = "COINX";
    ...
    uint public COINXHold = 150 * (10**decimals); // Minimum COINX hold in wallet to trigger minting
    ...
    event ChangeCOINXHold(uint value);
    ...
    modifier COINXminter () {
        assert(totalSupply <= maxTotalSupply);
        require(balances[msg.sender] >= COINXHold);
        _;
    }
...
//------------------------------------------------------------------------------------
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(msg.sender == _to && balances[msg.sender] >= COINXHold) return mintingCOINX();
        if(msg.sender == _to && balances[msg.sender] < COINXHold) revert();
...
//------------------------------------------------------------------------------------
    ...
    function mintingCOINX() COINXminter public returns (bool) {
        require(balances[msg.sender] >= COINXHold);
        if(balances[msg.sender] < COINXHold) revert();
        
        if(transferIns[msg.sender].length <= 0) return false;

        uint coinMinting = getCOINXminting(msg.sender);
    ...
    function getCOINXminting(address _address) internal view returns (uint) {
    ...
    function changeCOINXHold(uint256 _COINXHold) public onlyOwner {
        COINXHold = _COINXHold;
        emit ChangeCOINXHold(COINXHold);
    }
...
//------------------------------------------------------------------------------------
    ...
    event ChangeBonusPurchase(uint256 _value);
    ...
    uint public constant BNBMin = 1 ether; //Minimum purchase
    uint public constant BNBMax = 100 ether; //Maximum purchase

    function () public payable {
        require(startDate > 0 && now.sub(startDate) <= 90 days);
        uint purchasedAmount = msg.value * rate;
        uint bonusAmount = purchasedAmount.div(bonusRate);
        ...
        require(!closed);
        require(msg.value >= BNBMin && msg.value <= BNBMax);
