...
/////////////////String Library/////////////////////////////
    ...
    function getBalanceDecimal(uint currBalance, uint decimalCount) internal pure returns (string memory) {
      ...
      // for (uint i=decimalCount-1; i>=0 ; i--){
      //     if (i<=neededDecimals){
      //         bstr[i] = bytes1(uint8(48 + currBalance % 10));
      //     } 
      //     currBalance /= 10;
      // }
      return string(bstr);
...
/////////////////String Library/////////////////////////////
    ...
    {
        uint _r = uint(unit_price);
        return convertTokenBalance(_r,token_symbol,2);
    }
    ...
    {
        uint _r = depositors_balances[seller];
        return convertTokenBalance(_r,token_symbol,2);
    }
    ...
    {
        uint _r = depositors_balances[buyer];
        return convertTokenBalance(_r,token_symbol,2);
    }
    
    function get_balance(address depositor_address) public view returns(uint)
    {
        // uint _r = depositors_balances[depositor_address];
        // return convertTokenBalance(_r,token_symbol,2);
    ...
    {
        uint _r = uint(unit_price * demand);
        return convertTokenBalance(_r,token_symbol,2);
    }

    function token_decimal() public view returns (uint) {
