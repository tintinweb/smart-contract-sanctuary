...
contract RankingNFT is ERC721, Ownable {
  ...
  function buyTokenAfterMinting(uint256 _tokenId) public payable onlyOwner returns(bool) {
    ...
    emit OnBuyTokenAfterMinting(_tokenId, msg.sender, msg.value, transHistory.numberOfTransfers);
  ...
  function buyToken(uint256 _tokenId) public payable returns(bool) {
    ...
    //company's balance
    IERC20 token = IERC20(CURRENCY_RANKING_ERC20_TOKEN_ADDRESS);
    uint256 beforeBalanceForSender = token.balanceOf(msg.sender); //sender's balance
    uint256 beforeBalanceForCompany = token.balanceOf(owner());
    ...
    //sender's balance
    uint256 afterBalanceForSender = token.balanceOf(msg.sender);
