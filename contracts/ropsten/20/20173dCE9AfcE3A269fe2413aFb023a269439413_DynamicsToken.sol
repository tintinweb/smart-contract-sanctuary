...
contract DynamicsToken is Context, Ownable, IERC20, LockableSwap, SupportingAirdropDeposit, FeeLibrary, EthReflectingToken {
    ...
    constructor (uint256 _supply) payable {
        ...
        maxNumTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity.mul(10);
...
//        }
        ...
        if(!inSwapAndLiquify && tradingOpen && from != uniswapV2Pair) {
            //            selectSwapEvent();
            if(to != address(uniswapV2Router) || from != address(uniswapV2Router)){
...
//        IUniswapV2Pair(uniswapV2Pair).sync();
        ...
        if(!routerInvolved && buyBackEnabled && address(buybackContract).balance >= minBuyBack){
