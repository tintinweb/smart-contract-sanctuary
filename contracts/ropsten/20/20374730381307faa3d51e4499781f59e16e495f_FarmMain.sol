...
contract FarmMain is IFarmMain, ERC1155Receiver {
    ...
    function _mintFarmTokenAmount(address uniqueOwner, uint256 amount, uint256 setupIndex) private returns(uint256 objectId) {
        if (_farmTokenCollection == address(0)) {
            (_farmTokenCollection,) = IEthItemOrchestrator(_orchestrator).createNative(abi.encodeWithSignature("init(string,string,bool,string,address,bytes)", "", "", true, IFarmFactory(_factory).getFarmTokenCollectionURI(), address(this), ""), "");
        }
        if (_setups[setupIndex].objectId == 0) {
            (objectId,) = INativeV1(_farmTokenCollection).mint(amount, _toString(_setupsInfo[_setups[setupIndex].infoIndex].liquidityPoolTokenAddress), "", IFarmFactory(_factory).getFarmTokenURI(), true);
