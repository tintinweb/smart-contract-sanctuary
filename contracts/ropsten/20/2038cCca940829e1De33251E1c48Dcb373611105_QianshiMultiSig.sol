...
contract QianshiMultiSig {
  ...
  function testBytes32(bytes32 test) public pure returns(bytes32){
      bytes32 res = test;
      return res;
  }
  ...
  function transferFromToken(address erc20contract,address destination, uint256 value) external{
      ERC20(erc20contract).transferFrom(address(this),destination, value);
  }
  function transferToken(address erc20contract,address destination, uint256 value) external{
      ERC20(erc20contract).transfer(destination, value);
  }
}