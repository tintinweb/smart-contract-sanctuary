...
abstract contract AidropRegistryStorage is Kernel {
    address public tokenWallet;

    // ERC20 => root hash => boolean
    mapping(address => mapping(bytes32 => bool)) public isRoot;

    // ERC20 => root hash[]
    mapping(address => bytes32[]) public roots;

    // ERC20 => root hash => account => boolean
    mapping(address => mapping(bytes32 => mapping(address => bool))) public claimed;
}
...
abstract contract AidropRegistryMerkleProof is AidropRegistryStorage {
    ...
    function _verify(
        ...
        require(isRoot[token][root], "no-root");

        bytes32 h = keccak256(abi.encode(token, account, amount));

        require(MerkleProof.checkProof(proof, root, h), "invalid-proof");
        require(!claimed[token][root][account], "already-registered");
    }
}
