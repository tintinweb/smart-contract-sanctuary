...
contract Ownable {
	...
	function setSkyDuckFoundationOwner (address foundation) public returns (bool);
}
...
contract MintAndBurnToken is TokenForge, CappedToken, LockableProtocol {
	...
	function totalSkyDuckSupply() public view returns(uint256) {
...
contract SkyDuckToken is LockableToken {
	string  public constant name     = "SkyDuck";
	...
	function setSkyDuckFoundationOwner (address newFoundationOwner) public onlyOwner returns (bool) {
...
contract SkyDuckTokenUpgrade is SkyDuckToken {
	event UpgradeStart();
	event UpgradeStop();
