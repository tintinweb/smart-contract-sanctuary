...
abstract contract NiftyGenerator is ERC721URIStorage, Ownable, PaymentSplitter {
  using SafeMath for uint256;

  uint256 public NFT_PRICE; //0.08 ETH
  uint public constant maxPurchase = 20;
  uint256 public MAX_NFTS;

  string private _baseURIextended; // Overriding ERC721URIStorage base

  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;

  constructor (string memory _name, string memory _symbol, uint256 maxNftSupply, uint256 unitPrice) ERC721(_name, _symbol) {
  ...
  function mint(uint numberOfTokens) public payable
  {
    ...
    require(_tokenIds.current().add(numberOfTokens) <= MAX_NFTS, "Purchase would exceed total supply");
  ...
  // Add details specified here https://docs.opensea.io/docs/contract-level-metadata
  // Then go here after deployed: https://opensea.io/get-listed
}
