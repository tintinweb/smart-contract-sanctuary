...
contract Yumo {
    ...
    uint256 public maxTimeInHours = 24 hours;
    ...
    address[] public renownedArray; // ether distributed for the main game
    ...
    function getPrice(uint256 val) public pure returns (uint256) {
        uint256 value = val.div(1000).mul(1000);
        return value.mul(value).mul(999 * 10 ** 7).add(10 ** 15);
    }
    ...
    function gameOver() public {
        ...
        rounds[currentRound].lastPersonParticipated = creator;
        rounds[currentRound].lastPersonInvested = 0;
        rounds[currentRound].checkpoint = now;
    ...
    function distribute(uint256 albums, uint team, address referredAddress) private {
        ...
        rounds[currentRound].lastPersonInvested = albums;
