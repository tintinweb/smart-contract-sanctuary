...
contract Bridge {
    ...
    mapping(uint8 => uint256[]) allChargePercentages;
    ...
    function tokens(uint64 searchBlock) public view returns(uint8[] memory networkIds, address[][] memory tokens, address[][] memory crossTokens, uint256[][] memory minAmounts, uint256[][] memory chargePercentages) {
        ...
        uint256[][] memory retChargePercentages;
        ...
        retChargePercentages = new uint256[][](allNetworkIds.length);
        ...
        for (uint i=0;i< allNetworkIds.length;i++) {
            ...
            retChargePercentages[i] = new uint256[](retItems);
       ...
       return (retNetworkIds,retTokens,retCrossTokens,retMinAmounts, retChargePercentages);
    }
   ...
   function encodeAddressArray(address[] memory addresses) internal pure returns(bytes memory data){
    ...
    function addTokens(uint8 _networkId, address _token,address _crossToken,uint256 _minAmount, uint64 _futureBlock, uint256 chargePercentage) public {
        bool hasNetwork = false;
        for(uint i=0;i<allNetworkIds.length;i++) {
        ...
        allChargePercentages[_networkId].push(chargePercentage);
    }
