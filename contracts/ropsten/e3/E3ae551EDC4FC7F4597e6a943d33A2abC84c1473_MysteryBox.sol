...
contract MysteryBox is OwnableUpgradeable {
        ...
        payable
    {
        ...
        uint256[] memory sold_id_list = new uint256[](amount);
        ...
        if (box.sell_all) {
            ...
            for (uint256 i = 0; i < amount; i++) {
                ...
                if (creator == IERC721(nft_address).ownerOf(token_id)) {
                    ...
                    IERC721(nft_address).safeTransferFrom(creator, msg.sender, token_id);
                    box.purchased_nft[msg.sender].push(token_id);
                    sold_id_list[nft_transfered] = token_id;
                    nft_transfered++;
                }
                else {
        ...
        {
            uint256[] memory final_sold_id_list = new uint256[](amount);
            for (uint256 i = 0; i < amount; i++) {
                final_sold_id_list[i] = sold_id_list[i];
            }
            emit OpenSuccess(box_id, msg.sender, nft_address, final_sold_id_list);
        }
    }
