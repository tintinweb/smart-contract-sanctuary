...
contract SlotMarket is Initializable {
    ...
    function claimSlot(uint8 slot, uint128 bid, address delegate) external {
        console.log("Bid:", bid);
        ...
        console.log("Existing bid:", existingBidAmount);
        uint128 existingSlotBalance = slotBalance(slot);
        console.log("Existing slot balance:", existingSlotBalance);
        require(bid > existingBidAmount || existingSlotBalance == 0, "bid must be > current bid");
        console.log("Bid diff:", bid - existingBidAmount);
        ...
        slotExpiration[slot] = uint64((uint256(taxDenominator) * 8640000000 / uint256(taxNumerator) + block.timestamp * 100000) / 100000);
    ...
    function slotBalance(uint8 slot) public view returns (uint128 balance) {
        ...
        if (currentBid.bidAmount == 0) {
            ...
            // TODO: review math - proabably can simplify - doublecheck for overflow/underflow -> min bid to help with this?
            return uint128((uint256(currentBid.bidAmount) * 100000 - ((uint256(currentBid.bidAmount) * (block.timestamp - currentBid.periodStart) * currentBid.taxNumerator * 100000) / (uint256(currentBid.taxDenominator) * 86400))) / 100000);
        }
    }
