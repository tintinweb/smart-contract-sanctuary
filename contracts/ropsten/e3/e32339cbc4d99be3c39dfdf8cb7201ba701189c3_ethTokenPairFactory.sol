...
contract EthTokenSwapPoolContract is ReentrancyGuarded {
    ...
    using SafeERC20 for ERC20Token; //SafeERC20 library used for safe transfer of tokens
    ...
    event Swapped(address,uint256); // Swapped event triggered once tokens are swapped
     ...
     ERC20Token public tokenContractAddress; // The unique token contract address of the token pool
     uint256 public proposedWeiInVault; // the ether limit of the vault in wei which is changed when swapped or more ethers are added
    ...
    constructor(
    ERC20Token _tokenaddress, // takes the token address as parameter
    uint256 _weiiinvault      // inittialize with ether limit at beiginning in wei
  )
    
  {
    
    tokenContractAddress = _tokenaddress; // initialize with token address
    proposedWeiInVault = _weiiinvault;  // inittialize with ether limit ata beiginning which needed to deposited
...
function swapRate() public view  poolActive returns(uint256 _ramount)
{
    ...
    return uint256((proposedWeiInVault * (10 ** uint256(tokenContractAddress.decimals()))) / poolTokenSupply); // swap rate of 1 Token in wei
}
...
function requestSwap(address payable _requestor, uint256 _amount) public nonReentrant poolActive
{
    ...
    //update pools ether balance limit
    proposedWeiInVault = address(this).balance - swapedweiamount;
    
    //burn the amount of tokens from requestors tokenbalance, deduct the supply first before transferring the tokens to burner to avois redundancy
    poolTokenSupply -= smallestTokenUnits;
    
    /* not using burn function as it may not be possible for those erc20 tokens which don't have burn functions defined
     * burning by creating a self destructive contract and sending tokens to that contract
     */
    ...
    SafeERC20.safeTransfer(tokenContractAddress,address(lit), smallestTokenUnits); // transfer the swapped tokens to the burner
    lit.letBurn(); // Burn 
...
*/
contract ethTokenPairFactory is Ownable, ReentrancyGuarded{
...
function createTokenPoolContract(ERC20Token _tokenaddr, uint256 etherInPool) private returns(EthTokenSwapPoolContract)
{
    uint256 weiiinpool = etherInPool;
...
function buildTokenPool(ERC20Token _tokenaddr, uint256 etherInPool) public nonReentrant
{
...
function tokenPoolExist(ERC20Token _tokenaddr) public view returns(bool)
{
    return(PoolsRecords[address(_tokenaddr)].exist);
}

//get the token pool address if the token pool already exist
function getTokenPoolAddress(ERC20Token _tokenaddr) external view returns(EthTokenSwapPoolContract _tokenPool)
{
...
function getTokenSwapRate(ERC20Token _tokenaddr) external view returns(uint256 _swaprate)
{
     require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
     return PoolsRecords[address(_tokenaddr)].tokenPool.swapRate();
}

//get the amount of ether limit in the pool
function getTokenPoolEtheLimit(ERC20Token _tokenaddr) external view returns(uint256 _balanceINwei)
{
     require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
     return PoolsRecords[address(_tokenaddr)].tokenPool.proposedWeiInVault();
}

//get the amount of token supply in pool
function getTokenPoolSupply(ERC20Token _tokenaddr) external view returns(uint256 _balanceINwei)
{
     require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
...
function getTokenPoolName(ERC20Token _tokenaddr) external view returns(string memory)
{
     require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
     return PoolsRecords[address(_tokenaddr)].tokenPool.poolName();
}
...
//swap token 
function swapToken(ERC20Token _tokenaddr, uint256 _amount) public nonReentrant 
{
    require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
    ...
    SafeERC20.safeTransferFrom(_tokenaddr, msg.sender, address(PoolsRecords[address(_tokenaddr)].tokenPool),smallestTokenUnits);
