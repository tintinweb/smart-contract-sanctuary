...
contract HeadShotFactory is Context, Ownable {
    ...
    function testBuy() public {
        ...
        uint256 depositAllowance = headShotToken.allowance(msg.sender, address(this));
        require(depositAllowance >= _totalPay, "Check the token allowance");
        headShotToken.transferFrom(msg.sender, address(this), _totalPay);
        payable(msg.sender).transfer(_totalPay);
        emit Deposit(_totalPay);

        headShotToken.transfer(_salesWalletAddress, _totalPay);
        emit Withdraw(_totalPay);
    }
