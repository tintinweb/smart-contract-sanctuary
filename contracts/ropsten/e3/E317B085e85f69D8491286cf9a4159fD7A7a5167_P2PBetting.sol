...
contract P2PBetting is Ownable, ERC165 {
    ...
    event claimFee(address player, uint256 claimFee);
    ...
    function joinSlot(uint256 _pairID, uint256 creatorID, address[] memory _NFTaddress, uint256[] memory _NFTids, uint256[] memory _type ) external payable authTokens(_NFTaddress) {
        playerID++;
        require(_playerDetails[creatorID].playerAddress != msg.sender,"creator and joiner are not same");
        require(_NFTaddress.length == _NFTids.length,"Incorrect parameters");
    ...
    function claimTokens(uint256 _playerID, uint256 _pairID, uint256 blockTime, uint256 flag,  uint8 v, bytes32 r, bytes32 s) external payable {
        require((flag == 0) || (flag == 1),"Invalid flag value");
        PlayingPair storage pair = playingPairDetails[_pairID];
        PlayerDetails storage player1 = _playerDetails[_playerID];
        PlayerDetails storage player2 = _playerDetails[pair.creatorID];
        require(player1.playerAddress == msg.sender,"caller is not a playerID owner");
        require(!pair.pass,"Winner not announced");
        require(pair.creatorID == _playerID || pair.joinerID == _playerID,"player not in this pair");
        require(blockTime >= block.timestamp,"signature expiry");
        bytes32 msgHash = toSigEthMsg(msg.sender, _playerID, _pairID, blockTime, flag);
        ...
        if(flag == 0){
            ...
            pair.pass = true;
            pair.winnerID = _playerID;
            pair.winnerAddress = _playerDetails[_playerID].playerAddress;
        ...
        emit claimFee(msg.sender, msg.value);
    }
    ...
    function toSigEthMsg(address _user, uint256 _playerID, uint256 _pairID, uint256 _blockTime, uint256 _flag)internal view returns(bytes32){
        bytes32 hash = keccak256(abi.encodePacked(abi.encodePacked(_user, _playerID, _pairID, _blockTime, _flag),address(this)));
        return keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash));
    }

    function emergencyNFTSafe(address _tokenAddress,address _to, uint256 _tokenId, uint256 _type) external onlyOwner {
        if(_type == 1){
