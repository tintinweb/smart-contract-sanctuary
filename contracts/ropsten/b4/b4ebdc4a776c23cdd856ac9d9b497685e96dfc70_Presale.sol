...
abstract contract BasicToken is IERC20, Context, Ownable {
    ...
    uint256 public startTime = 1614254076;                                  // ------| Deploy Timestamp |--------
...
abstract contract StandardToken is BasicToken {
    ...
    function _transferSpecial( address sender, address recipient,  uint256 amount) internal virtual {
        require( sender != address(0), "ERC20 Special: transfer from the zero address");
        require( recipient != address(0),   "ERC20 Special: transfer to the zero address");

        balances_[sender] = balances_[sender].sub(amount,
    ...
    function _transfer( address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(block.timestamp >= startTime.add(unlockDuration), "Tokens not unlocked yet");
        ...
        if (transferCounter == 3) { //20 for mainnet
        ...
        if (block.timestamp > rewardDispatchStartTime.add(3 minutes)) { //7 days  //-|Transfer rewards to RewardCollector every 7 days |--
            balances_[rewardCollector] = balances_[rewardCollector].add(StakingContractFee);

            realStakingContractFee = StakingContractFee;
            StakingContractFee = 0;
            rewardDispatchStartTime = block.timestamp;

            emit Transfer( address(this), rewardCollector, realStakingContractFee );
        }
...
contract PreSaleToken is StandardToken, Configurable { 
    ...
    receive() external payable {
        require( currentPhase == Phases.start, "The presale has not started yet");
        ...
        require(ethBalances[msg.sender] <= 3e18, "You are exceeding max 3 ETH of purchase");
        require(ethBalances[address(this)] <= 60e18, "Sorry! target amount of 60 ETH has been achieved");
        ...
        balances_[owner()] = balances_[owner()].sub(tokens,
...
contract Presale is PreSaleToken {
    string public name = "CZ Finance";
    string public symbol = "WDCS";
