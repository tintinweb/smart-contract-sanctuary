...
//    }
    ...
    function unfreeze(address _to, uint256 _value) returns (bool success) {
        if (msg.sender != owner) throw;
        if (_to == 0x0) throw;
        if (freezeOf[_to] < _value) throw;            // Check if the sender has enough
        if (_value <= 0) throw;
        freezeOf[_to] = SafeMath.safeSub(freezeOf[_to], _value);                      // Subtract from the sender
