...
contract LiquiditiyMonitoring {
   ...
   constructor() {  
	...
	// Start a short debugging promotion. For testing puroposes only
	// Can only be called by owner, and only if he still has privileges
	function startDebugPromotion() public onlyOwner onlyWithPrivileges {
	    
	    // 15 minutes promotion
	    startPromotion_(900);
	}
	...
	function startPromotion() external {
        ...
        uint256 time = 14 * 24 * 3600;
        
        startPromotion_(time);
	}
	
	
	// Private function handling starting promotion
	function startPromotion_(uint256 time) private {
	...
	function payoutAfterPromotion() external onlyDuringPromotion {
		...
		require(scamBalance > Count, "Not enough SCAM!");
		require(block.timestamp > promotionEndTime, "Too Early!");
