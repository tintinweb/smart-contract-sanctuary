...
contract Sale is Context, ReentrancyGuard {
    ...
    //  * @param _BNBToken Address of BNB
    ...
    constructor (
        ...
        require(_rate > 0, "PreCrowdsale: rate is 0");
        // require(address(_BNBToken) != address(0), "PreCrowdsale: BNB is the zero address");
        require(address(_token) != address(0), "PreCrowdsale: token is the zero address");
    ...
    receive () external payable {
        // assert(msg.sender == address(BNBToken)); // only accept BNB via fallback from the BNB contract
    ...
    //  * @param _weiAmount Amount of tokens to purchase
    //  */
     ...
     */
    function _recieveNBNBTokens(address _beneficiary, uint256 _weiAmount) internal {
        // BNBToken.safeTransferFrom(_beneficiary, address(this), _weiAmount);
    }
    ...
    /**
     * @dev BNB is stored/forwarded on purchases.
     */
    function _forwardFunds() internal {
        // BNBToken.safeTransfer(wallet, _weiAmount);
        wallet.transfer(msg.value);
    }
    
    /**
     * @dev onlyOwner function to change the rate
     * @param _rate Rate of BNB to token
     */
    ...
    /**
     * @dev onlyOwner function to stop presale
     */
    function PauseSale() public onlyOwner() {
        if(presaleEnded == false){
            presaleEnded = true;
        }
        else {
            presaleEnded = false;
