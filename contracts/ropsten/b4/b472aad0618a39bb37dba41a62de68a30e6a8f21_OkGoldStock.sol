...
contract OkGoldStock is StandardToken {
    string public name = &#39;OkGoldStock&#39;;
    string public symbol = &#39;OGS&#39;;
    uint public decimals = 10;
    uint public INITIAL_SUPPLY = 27.7 * 100000000 * (10 ** decimals);
    address owner;
    ...
    {
        require(balances[msg.sender] >= a_coinAmount);
        balances[msg.sender] = balances[msg.sender].sub(a_coinAmount);
        totalSupply_ = totalSupply_.sub(a_coinAmount);
        emit EventBurnCoin(msg.sender, a_coinAmount);
    }

    function addCoin(uint a_coinAmount) external isOwner
    {
        balances[msg.sender] = balances[msg.sender].add(a_coinAmount);
        totalSupply_ = totalSupply_.add(a_coinAmount);
        emit EventAddCoin(a_coinAmount, totalSupply_);
    }
