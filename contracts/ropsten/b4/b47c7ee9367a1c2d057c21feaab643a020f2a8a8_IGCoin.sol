...
contract IGCoin is DeaultERC20 {
    ...
    uint256 constant private R = 125;  // matlab R=1.008, this R=1/(1.008-1)
    uint256 constant private P = 10; // precision
    uint256 constant private lnR = 125; // 1/ln(R)   (matlab R)
    uint256 constant private S = 1e3; // s.t. S*R = integer
    uint256 constant private RS = 8; // 1.008*S-S=8
    uint256 constant private lnS = 14; // ln(S) = 18
    uint256 constant private lnRS = 147498573695997371659474392890123382422685;// FIXED_3 * ln(1e3)/ln(1.008) // ln(S)/ln(R) (matlab R)
    uint256 public refund = 0;
    uint256 constant private R1000 = 2888; // (matlab R)^1000
    ...
    constructor() public {
        ...
        quoteAsk();
        quoteBid();        
        debugVal = 1175293658221324600208007724055171054601*FIXED_3/1355713814124746533949216473472641271;
        debugVal2 = ln_fixed3(1e3,1);
        debugVal3 = ln_fixed3(1008,1)-ln_fixed3(1000,1);
        debugVal4 = (2**100 * debugVal2)/debugVal3;
    ...
    function buy() public payable returns (uint256 amount){
        ...
        if(initialSaleComplete)
        {
            ...
            //debugVal = fracExp(S, R, (1+_totalSupply/1e16),P);
            //debugVal2 = RS*msg.value/1e16 + fracExp(S, R, (1+_totalSupply/1e16),P);
            //debugVal3 = ln_fixed3(debugVal2,1);
            //debugVal4 = ((ln_fixed3_lnr(debugVal2,1))-lnRS-FIXED_3)/FIXED_3;
            
            
            b = ((ln_fixed3_lnr_18( RS*msg.value/1e16 + fracExp(S, R, (1+_totalSupply/1e16),P),1))-1e18*lnRS-1e18*FIXED_3)/FIXED_3;
            debugVal = b;
            
            refund = msg.value - (msg.value/1e16)*1e16;
            amount = b*1e16/1e18-_totalSupply;
            //debugVal2 = (msg.value/1e16)*1e16;
            //debugVal3 = refund;
            //debugVal4 = amount;

            reserveAddress.transfer((msg.value/1e16)*1e16); // send the ask amount to the reserve
    ...
    /// @param _denominator is a value between 1 and 2 ^ (256 - MAX_PRECISION) - 1
    /// @return is a value between 0 and floor(ln(2 ^ (256 - MAX_PRECISION) - 1) * 2 ^ MAX_PRECISION)
    function ln_fixed3(uint256 _numerator, uint256 _denominator) internal pure returns (uint256) {
        assert(_numerator <= MAX_NUM);
    ...
    function ln_fixed3_lnr(uint256 _numerator, uint256 _denominator) internal pure returns (uint256) {
        ...
        return (((res * LN2_MANTISSA) >> LN2_EXPONENT)*125499336)/1000000;
    }    
    ...
    function ln_fixed3_lnr_18(uint256 _numerator, uint256 _denominator) internal pure returns (uint256) {
        ...
        return (((res * LN2_MANTISSA) >> LN2_EXPONENT)*125499336*1e18)/1000000;
    }       
