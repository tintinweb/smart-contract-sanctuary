...
contract NFT is ERC721, Ownable, ReentrancyGuard {
  ...
  mapping (uint256 => uint256) private parts;
  mapping (uint256 => uint256) private participation;
  
  //mapping (uint256 => uint8) private participation;
  uint256 private totalcounts;
  ...
  */
  ...
  function PlaySaveTheGalaxy(uint256 tokenId) public virtual returns (string memory) {
    ...
    require(totalcounts <= 300, "More than 300 Ape Heroes are gathered");
    address owner = ERC721.ownerOf(tokenId);
    require(_msgSender() == owner,"ERC721: approve caller is not owner nor approved for all");

    if (parts[tokenId] >= 0){
        //tl[tokenId] = block.timestamp - ta[tokenId];
        //require(tl[tokenId] >= 60, "Play Once a day");
        
        //require(tl[tokenId] >= 60*60*24*2, "Play Once a day");
        //ta[tokenId] = block.timestamp;
        parts[tokenId]++;
    }
  ...
  function StartSaveTheGalaxy() public onlyOwner{
    totalcounts=0;
  }
  ...
  nonReentrant{
    ...
    require(owner() == msg.sender && parts[tokenId] == 5, "Need more parts to claim GalacticApe Hero");
    _mint(msg.sender);
    }
