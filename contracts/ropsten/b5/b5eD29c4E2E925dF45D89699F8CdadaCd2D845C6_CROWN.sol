...
contract CROWN is Ownable, ERC20 {
    ...
    event DividendWithdrawal(address indexed from, address indexed to, uint256 amount, uint256 timestamp);
...
//-------------------------------- Main Staking Functions ---------------------------------------------
   ...
   function withdrawDividend(address stakeHolder, address toAddress, string memory currency, uint256 amount) public validAddress(stakeHolder) onlyAfterPeriod {
       assert(keccak256(abi.encodePacked(currency)) == keccak256(abi.encodePacked("USDC")) || keccak256(abi.encodePacked(currency)) == keccak256(abi.encodePacked("USDT")));
       ...
       emit DividendWithdrawal(stakeHolder, toAddress, dividendAmount, block.timestamp);
   }
}