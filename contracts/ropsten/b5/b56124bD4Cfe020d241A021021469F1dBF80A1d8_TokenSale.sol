...
contract TokenSale is Ownable {
    ...
    constructor(address USDTAddr, address TOKENAddr, address initialOwner, address initialFeeWallet, address initialFundWallet, uint256 _rateMul, uint256 _rateDiv, bool _rateChangeable) public Ownable(initialOwner) {
    ...
    function setReferrer(address ref, uint256 refPercent) public onlyOwner {
        require(ref != address(0) && refPercent <= FEE_PERCENT + REF_PERCENT);
        users[ref].refPercent = refPercent;
    }

    function setRate(uint256 newRateMul, uint256 newRateDiv) public onlyOwner {
