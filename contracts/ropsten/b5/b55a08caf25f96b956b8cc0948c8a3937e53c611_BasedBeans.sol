...
/// @title Based Beans for bloot holders!
/// Original author: Will Papper <https://twitter.com/WillPapper>
/// @notice This contract mints Based Beans for bloot holders and provides
/// administrative functions to the bloot DAO. It allows:
/// * bloot holders to claim Based Beans
/// * A DAO to set seasons for new opportunities to claim Based Beans
/// * A DAO to mint Based Beans for use within the bloot ecosystem
/// @custom:unaudited This contract has not been audited. Use at your own risk.
contract BasedBeans is Context, Ownable, ERC20 {
    ...
    // Give out 10,000 Based Beans for every bloot Bag that a user holds
    uint256 public BasedBeansPerTokenId = 10000 * (10**decimals());
    ...
    */
    ...
    constructor() Ownable() ERC20("Based Beans", "BEAN") {
    ...
    /// @notice Claim Based Beans for a given bloot ID
    ...
    /// @notice Claim Based Beans for all tokens owned by the sender
    ...
    /// @notice Claim Based Beans for all tokens owned by the sender within a
    /// given range
    ...
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        // Check that Based Beans have not already been claimed this season
        // for a given tokenId
        require(
            !seasonClaimedByTokenId[season][tokenId],
            "BEANS_CLAIMED_FOR_TOKEN_ID"
        );

        // Effects

        // Mark that Based Beans has been claimed for this season for the
        // given tokenId
        seasonClaimedByTokenId[season][tokenId] = true;

        // Interactions

        // Send Based Beans to the owner of the token ID
        _mint(tokenOwner, BasedBeansPerTokenId);
    }
    ...
    /// @notice Allows the DAO to set a season for new Based Beans claims
    /// @param season_ The season to use for claiming bloot
    function daoSetSeason(uint256 season_) public onlyOwner {
        season = season_;
    }

    /// @notice Allows the DAO to set the amount of Based Beans that is
    /// claimed per token ID
    /// @param BasedBeansDisplayValue The amount of bloot a user can claim.
    ...
    /// 100 * 10^18.
    function daoSetBasedBeansPerTokenId(uint256 BasedBeansDisplayValue)
        public
        onlyOwner
    {
        BasedBeansPerTokenId = BasedBeansDisplayValue * (10**decimals());
    }

    /// @notice Allows the DAO to set the season and Based Beans per token ID
    /// in one transaction. This ensures that there is not a gap where a user
    /// can claim more Based Beans than others
    /// @param season_ The season to use for claiming bloot
    /// @param BasedBeansDisplayValue The amount of bloot a user can claim.
    /// This should be input as the display value, not in raw decimals. If you
    /// want to mint 100 bloot, you should enter "100" rather than the value of
    ...
    /// BasedBeans variables directly. It is better practice for security,
    ...
    function daoSetSeasonAndBasedBeansPerTokenID(
        uint256 season_,
        uint256 BasedBeansDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetBasedBeansPerTokenId(BasedBeansDisplayValue);
    }
}