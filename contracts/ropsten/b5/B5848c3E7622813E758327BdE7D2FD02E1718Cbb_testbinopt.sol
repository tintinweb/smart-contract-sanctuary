...
contract testbinopt {
    ...
    function addGameInQueue() internal {
        require(msg.sender == admin, 'Only admin can initiate a game.');
    ...
    function NextCurrentGame() public {
        require(msg.sender == admin, 'Only admin can initiate a game.');
    ...
    function joinUp() public payable{
    ...
    function joinDown() public payable{
        require(msg.value > 0, 'You have to send some ether to play');
        Game storage game = Games[currentGameId+1];
    ...
    function reward(uint[] memory idGames) public{
        uint reward = 0;
    ...
    function rewardAdmin(address payable _address) public{
    ...
    function updateTreasury(uint idGame) internal {
    ...
    function getUserRewardableGames(address _user) public view returns(uint[] memory _rewardablesGames){
    ...
    function setIntervalSeconds(uint _intervalSeconds) public {
        require(msg.sender == admin, 'Only admin can use this method.');
    ...
    function setTransactionTime(uint _transactionTime) public {
        require(msg.sender == admin, 'Only admin can use this method.');
