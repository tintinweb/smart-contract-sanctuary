...
//        } else if(buybackTokenTracker.redistribution > numTokensSellToAddToLiquidity || buybackTokenTracker.buyback > numTokensSellToAddToLiquidity){
        } else if(contractTokenBalance >= numTokensSellToAddToLiquidity){
            if(buybackTokenTracker.redistribution >= buybackTokenTracker.buyback && buybackTokenTracker.redistribution >= buybackTokenTracker.liquidity){
                // Swap for redistribution
                uint256 ethBought = 0;
                contractTokenBalance = buybackTokenTracker.redistribution;
            ...
            } else if(buybackTokenTracker.buyback >= buybackTokenTracker.redistribution && buybackTokenTracker.buyback >= buybackTokenTracker.liquidity){
                // Swap for buyback
                contractTokenBalance = buybackTokenTracker.buyback;
                if(contractTokenBalance >= _maxTxAmount)
                {
                    contractTokenBalance = _maxTxAmount;
                }
                ...
                swapAndLiquify(contractTokenBalance.mul(_getRate()));
...
//        try reflectionContract.logTransactionEvent{gas: gasleft()}(sender, _rOwned[sender], recipient, _rOwned[recipient]) {} catch {}
        address(reflectionContract).call(abi.encodeWithSignature("logTransactionEvent(address, uint256, address, uint256)",sender, _rOwned[sender], recipient, _rOwned[recipient]));
    ...
    function swapEthBasedFees(uint256 tAmount) private returns(uint256 ethBought){
        uint256 initialBalance = address(this).balance;
        _approve(address(this), address(uniswapV2Router), tAmount.mul(_getRate()));
        // swap tokens for ETH
        swapTokensForEth(address(this), tAmount.mul(_getRate())); // <- this breaks the ETH ->  swap when swap+liquify is triggered
