...
contract MyERC721 is ERC721Token, Ownable {
    ...
    function getDesign( uint _designId ) public view returns(string hash, string author, string comment){
        Design memory _design = designs[_designId];
    ...
    function mint(string _hash, string _author, string _comment) public {
        Design memory _design = Design({ hash: _hash, author: _author, comment: _comment });
        uint _designId = designs.push(_design) - 1;
    ...
    function createAuction (uint256 _tokenId, uint128 _price) external {
      ...
      transferFrom(msg.sender, this, _tokenId);
    ...
    function bid( uint256 _tokenId ) external payable {
      ...
      uint128 fee = (price * 5)/100;
      uint128 sum = price - fee;
      address dt = 0xF8CBBB0Ed1563a87490D0CD0f0F6B8DF613bAEC3;

      seller.transfer(sum);
      dt.transfer(fee);
      this.approve(msg.sender, _tokenId);
      transferFrom(this, msg.sender, _tokenId);
    }
