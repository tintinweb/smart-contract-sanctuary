...
import "./token/BaseNFT.sol";
import "./lib/Operatorable.sol";

abstract contract BaseSale is Operatorable {
  BaseNFT public immutable baseNFT;
  // Selling enabled status flag
  bool public selling;
  // Token price in ETH
  uint256 public price;
  // Maximum mint amount in sale
  uint256 public maxMint;
  // Currently minted amount in sale
  uint256 public minted;

  event LogEthReceived(address indexed account, uint256 amount);
  event LogEthSent(address indexed account, uint256 amount);
  event LogPurchased(address indexed account, uint256 amount);

  constructor(address _baseNFT) {
    require(_baseNFT != address(0), "BaseSale: BASE_NFT_ADDRESS_INVALID");
    baseNFT = BaseNFT(_baseNFT);
  }

  receive() external payable {
    revert("BaseSale: receive not supported");
  }
  ...
  /**
    * @dev Set `maxMint`
    * Only `owner` can call
    * '_maxMint' can be zero
    */
  function setMaxMint(uint256 _maxMint) external onlyOwner {
    require(_maxMint > 0, "BaseSale: MAX_MINT_AMOUNT_INVALID");
    maxMint = _maxMint;
  }

  /**
   * @dev Mint unsold tokens
   * Only `owner` can call
   * `_amount` must not be zero
   * Total minted amount must not exceed `maxMint`
   */
  function mintUnsoldTokens(uint256 _amount) external onlyOwner {
    require(!selling, "BaseSale: SALE_NOT_FINISHED");
    require(_amount > 0, "BaseSale: MINT_UNSOLD_AMOUNT_INVALID");
    uint256 _minted = minted;
    require(_amount + _minted <= maxMint, "BaseSale: MAX_MINT_LIMIT_EXCEED");

    for (uint8 i = 0; i < _amount; i++) {
      baseNFT.mint(owner(), "");
    }

    minted = _amount + _minted;
  }

  /**
   * @dev Purchase `_amount` of tokens
   */
  function purchase(uint8 _amount) public virtual payable;

  /**
   * @dev Withdraw Eth balance to `_to`
   * Only `owner` can call
   * `_to` must not be zero address
   */
  function withdrawEth(address _to) external onlyOwner {
    require(_to != address(0), "BaseSale: RECEIVER_ADDRESS_INVALID");
    uint256 balance = address(this).balance;
    (bool sent, ) = payable(_to).call{value: balance}("");
    require(sent, "BaseSale: ETH_TRANSFER_FAILED");
  ...
  function _purchase(
    ...
    require(!_isContract(_account), "BaseSale: CALLER_CONTRACT");
    require(selling, "BaseSale: SALE_NOT_ENABLED");
    require(_amount > 0, "BaseSale: PURCHASE_AMOUNT_INVALID");
    // check if total minted amount exceeds `maxMint` limit
    uint256 _minted = minted;
    require(_minted + _amount <= maxMint, "BaseSale: MAX_MINT_LIMIT_EXCEED");

    // check if Eth is sufficient
    uint256 _price = price;
    require(msg.value >= _amount * _price, "BaseSale: INSUFFICIENT_FUNDS");

    for (uint8 i = 0; i < _amount; i++) {
      baseNFT.mint(_account, "");
    }

    minted = _minted + _amount;

    // return change if any
    uint256 change = msg.value - _amount * _price;

    if (change > 0) {
      (bool sent, ) = payable(_account).call{value: change}("");
      require(sent, "BaseSale: ETH_TRANSFER_FAILED");
    }

    emit LogPurchased(_account, _amount);
  }
}
