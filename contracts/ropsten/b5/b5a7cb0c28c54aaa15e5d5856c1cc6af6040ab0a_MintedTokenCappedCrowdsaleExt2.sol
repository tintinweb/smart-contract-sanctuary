...
Println(0);
    if(isWhiteListed) {
      ...
      if(tokenAmountOf[receiver] == 0) { //tokenAmount < token.minCap() && 
        throw;
      }
    }
 ...
 *
 */
contract MintedTokenCappedCrowdsaleExt2 is CrowdsaleExt2 {

  /* Maximum amount of tokens this crowdsale can sell. */
  uint public maximumSellableTokens;
  address public salespool;

  function MintedTokenCappedCrowdsaleExt2(
    string _name, 
    address _token, 
    PricingStrategy _pricingStrategy, 
   ...
   */
  function assignTokens(address receiver, uint tokenAmount) private {
    FractionalERC20Ext erc20token = FractionalERC20Ext(token);
    erc20token.transferFrom(salespool, receiver, tokenAmount);
  }
}