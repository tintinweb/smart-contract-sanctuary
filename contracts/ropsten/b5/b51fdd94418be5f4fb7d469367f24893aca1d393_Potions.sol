...
contract Potions is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getAcid(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "ACID", acid);
    }
    ...
    function getMight(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "MIGHT", might);
    }
    
    function getLightning(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "LIGHTNING", lightning);
    }
    ...
    function getRegen(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "REGENERATION", regen);
    }
    ...
    function pluck(
        ...
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("SYMBOL") && greatness >= 14) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="purple" /><text x="10" y="20" class="base">';
        ...
        parts[7] = getNightvision(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">';
        
        parts[9] = getRegen(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';
        
        parts[11] = getMindcontrol(tokenId);

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = getLightning(tokenId);
