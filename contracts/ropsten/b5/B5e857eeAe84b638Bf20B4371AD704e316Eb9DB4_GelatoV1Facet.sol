...
import {LibGelatoV1} from "../libraries/LibGelatoV1.sol";
import {
    GelatoTaskReceipt
...
contract GelatoV1Facet is BFacetOwner {
    ...
    using LibGelatoV1 for IGelatoV1;
    ...
    function withdrawExcessExecutorStake(
        ...
        (uint256 amount, string memory error) =
            _gelatoCore.withdrawExcessExecutorStakeVerbose(_withdrawAmount);
        if (amount == 0)
            error.revertWithInfo("GelatoV1Facet.withdrawExcessExecutorStake:");
    ...
    function providerRefund(
        ...
        (bool success, string memory error) =
            _gelatoCore.providerRefund(_provider, _amount);
        if (!success) error.revertWithInfo("GelatoV1Facet.providerRefund:");
    }
...
library LibGelatoV1 {
    ...
    function withdrawExcessExecutorStakeVerbose(
        IGelatoV1 _gelatoCore,
        uint256 _withdrawAmount
    ) internal returns (uint256, string memory) {
        try _gelatoCore.withdrawExcessExecutorStake(_withdrawAmount) returns (
            uint256 realAmount
        ) {
    ...
    function providerRefund(
        ...
        address _provider,
        uint256 _amount
    ) internal returns (bool, string memory error) {
        try _gelatoCore.provideFunds{value: _amount}(_provider) {
            return (true, "");
