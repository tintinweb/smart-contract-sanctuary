...
contract Buyer {
    function Purchase(address,address)public  payable  returns (uint) {}
...
contract Seller is SafeMath {
    ...
    uint private start;
     ...
     event EscrowFailed();
     event ReceivedETH(address indexed _from, uint _value);
           ...
           // address ata = buyer.GetSellingTokenContractAddress();
                ...
                if(buyerescrowbalance>0)
                {
                      uint erc20tokentotal = safeDiv(balance ,buyerUnitprice);// This will return in whole number
                       ...
                       uint totalbought = buyer.Purchase.value(balance)(EscrowOwner,this);
                       balance = safeSub(balance,balance);
                       if(totalbought>0)
                       {
