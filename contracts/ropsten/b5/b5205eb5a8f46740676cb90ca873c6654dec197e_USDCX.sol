...
contract USDCX is ERC223Interface {
    ...
    uint256 public TotalSupply =  2 * (10 ** 9) * (10 ** 18); // 2 billion
    ...
    function mint() {
        require(msg.sender==contractOwner);
        require(now-nowtime > 365 days);
        
        uint256 twoMillion = 2 * (10 ** 9) * (10 ** 18); 
        TotalSupply=TotalSupply+twoMillion;
        tokenRemained=tokenRemained+twoMillion;
        nowtime=now;
