...
contract ProofOfBlockProtocolLayer {
    ...
    event BlockNumberMinting(address _by, uint256 _time, uint256 _blockNumber);
}
...
contract Zael is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Zael";
    string public symbol = "ZAEL";
    ...
    uint256 internal timelockStart;
    uint256 internal unlockDate = 1623541071;
    ...
    constructor() public {
        ...
        lastMintingBlock[msg.sender] = totalBlocksMinted;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        proofOfBlockRewards = proofOfBlockRewards.add(transferFee);

        emit Transfer(msg.sender, to, valueAfterFee);
        emit FeeTransfer(msg.sender, transferFee);
        
        //Function to reset coin age to zero for "msg.sender"
        ...
        //Automated triggering internal Proof-of-Block minting protocol layer
...
//--------------------------------------------------------------------------------------
    ...
    function proofOfAgeMinting() ProofOfAgeMinter public returns (bool) {
        ...
        emit BlockNumberMinting(msg.sender, lastBlockTime, lastBlock);
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public storedToken = 3000 * (10**decimals);
    uint256 internal blockRate = 1000; //Block rate
    
    uint256 internal totalBlocksMinted = 1;
    uint256 internal lastBlockTime;
    uint256 internal lastBlock;
    
    mapping (address => uint256) internal lastMintingBlock;

    function proofOfBlockMinting(address account) public returns (bool) {
        require(balances[account] > storedToken);
        require(lastMintingBlock[account] < totalBlocksMinted);
        require(blockMinterPaused == false);
        if(blockMinterPaused == true) revert();
        
        if(balances[account] <= storedToken) revert();
        
        //Excluded addresses from triggering
        //internal Proof-of-Block protocol layer

        if(contractOwner == account) revert();
        if(teamDevs == account) revert();
        if(vault == account) revert();
        if(blacklist[account] == true) revert();
        ...
        uint256 minterFee = blockMinting.mul(minterTax).div(1e4);
        uint256 mintingAfterFee = blockMinting.sub(minterFee);
        
        totalSupply = totalSupply.add(blockMinting);
        balances[account] = balances[account].add(mintingAfterFee);
        
        proofOfBlockRewards = proofOfBlockRewards.sub(blockMinting);
        proofOfAgeRewards = proofOfAgeRewards.add(minterFee);
        ...
        emit Transfer(address(0), account, mintingAfterFee);
        emit MinterFeeTransfer(address(0), minterFee);
        emit ProofOfBlockMinting(account, mintingAfterFee);
        emit BlockNumberMinting(account, lastBlockTime, lastBlock);
        
        return true;
    }

    function annualBlockRate() internal view returns (uint blockRewardRate) {
        blockRewardRate = blockRate;

        if(totalSupply < 31250000 * (10**decimals)) {
            blockRewardRate = blockRate.mul(1);

        } else if(totalSupply >= 31250000 * (10**decimals) && totalSupply < 62500000 * (10**decimals)) {
            blockRewardRate = blockRate.div(2); //block reward rate halving (500)

        } else if(totalSupply >= 62500000 * (10**decimals) && totalSupply < 125000000 * (10**decimals)) {
            blockRewardRate = blockRate.div(4); //block reward rate halving (250)

        } else if(totalSupply >= 125000000 * (10**decimals) && totalSupply < 250000000 * (10**decimals)) {
            blockRewardRate = blockRate.div(8); //block reward rate halving (125)

        } else if(totalSupply >= 250000000 * (10**decimals) && totalSupply < 500000000 * (10**decimals)) {
            blockRewardRate = blockRate.div(16); //block reward rate halving (62,5)

        } else if(totalSupply >= 500000000 * (10**decimals)) {
            blockRewardRate = blockRate.div(32); //block reward rate halving (31,25)
        }
    }

    function getProofOfBlockMinting(address account) internal view returns (uint) {
        require((now >= blockMinterStartTime) && (blockMinterStartTime > 0));
        require(blockMinterPaused == false);
        if(blockMinterPaused == true) revert();
        if(lastMintingBlock[account] == totalBlocksMinted) return 0;
        uint blockRewardRate = blockRate;
        uint256 lastMintingTime = lastMintingBlock[account];
        ...
        return ((totalBlocksMinted.sub(lastMintingTime)) * blockRewardRate).mul(balances[account]).div(totalSupply);
    }
    
    function blockMinterInfo(address account) external view returns (address, uint256, uint256) {
        uint256 lastMintingTime = lastMintingBlock[account];
...
//--------------------------------------------------------------------------------------
    ...
    event ChangeBlockRate(uint256 value);
    event ChangeStoredToken(uint256 value);
    ...
    function changeBlockRate(uint256 _blockRate) public onlyOwner {
        blockRate = _blockRate;
        emit ChangeBlockRate(blockRate);
    }
    
    function changeStoredToken(uint256 _storedToken) public onlyOwner {
        storedToken = _storedToken;
        emit ChangeStoredToken(storedToken);
    }

//--------------------------------------------------------------------------------------
//Pause and continue internal Proof-of-Block minting protocol layer function
//--------------------------------------------------------------------------------------

    bool public blockMinterPaused;
    
    function blockMintingStart() public onlyOwner {
        require(msg.sender == owner && blockMinterStartTime == 0);
        blockMinterStartTime = now;
    }
    
    function blockMinterContinue() public onlyOwner {
        blockMinterPaused = false;
    }
    
    function blockMinterPause() public onlyOwner {
        blockMinterPaused = true;
    }

    function isBlockMinterPaused() public view returns (bool) {
        return blockMinterPaused;
    }
...
//--------------------------------------------------------------------------------------
    ...
    event MinterFeeTransfer(address indexed from, uint256 value);
    event ChangeMinterTax(uint256 value);
    
    uint256 public minterTax = 250; //Minter fee is 2,5%
    
    function changeMinterTax(uint256 _minterTax) public onlyOwner {
        minterTax = _minterTax;
        emit ChangeTransferTax(minterTax);
    }
