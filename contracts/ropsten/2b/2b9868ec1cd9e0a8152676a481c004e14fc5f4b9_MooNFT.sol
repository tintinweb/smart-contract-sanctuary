...
contract RoyaltiesV2Impl is Ownable, AbstractRoyalties, RoyaltiesV2 {
    uint96 _royaltyFee = 1000;
    function getRaribleV2Royalties(uint256) override external view returns (LibPart.Part[] memory) {
        LibPart.Part[] memory _royalties = new LibPart.Part[](1);
        _royalties[0].value = _royaltyFee;
        _royalties[0].account = payable(address(owner()));
        return _royalties;
        //return royalties[id];
    }
...
contract MooNFT is ERC721URIStorage, EIP712, Ownable, RoyaltiesV2Impl, AccessControl {
  ...
  function withdraw() public {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Only authorized admins can withdraw");
