...
contract OVM_FraudVerifier is Lib_AddressResolver, Abs_FraudContributor, iOVM_FraudVerifier {
     ...
     * @param _preReceipt Receipt before the fraudulent transaction.
     * @param _postReceipt Receipt after the fraudulent transaction.
     * @param _transaction OVM transaction claimed to be fraudulent.
     */
    function initializeFraudVerification(
        Lib_OVMCodec.Receipt memory _preReceipt,
        Lib_OVMCodec.Receipt memory _postReceipt,
        Lib_OVMCodec.Transaction memory _transaction
    )
        override
        public
        contributesToFraudProof(_preReceipt.stateRoot, Lib_OVMCodec.hashTransaction(_transaction))
    {
        bytes32 _txHash = Lib_OVMCodec.hashTransaction(_transaction);

        if (_hasStateTransitioner(_preReceipt.stateRoot, _txHash)) {
            return;
        }
        ...
        require(
            ...
            "Hash of OVM transaction has to be same as the one provided by receipt."
        );

        require (
            _preReceipt.index + 1 == _postReceipt.index,
            "Pre-state root global index must equal to the transaction root global index."
        );

        _deployTransitioner(_preReceipt.stateRoot, _txHash, _preReceipt.index);

        emit FraudProofInitialized(
            _preReceipt.stateRoot,
            _preReceipt.index,
            _txHash,
            msg.sender
        );
        ...
        contributesToFraudProof(_preReceipt.stateRoot, Lib_OVMCodec.hashTransaction(_transaction))
    {
        ...
        iOVM_StateTransitioner transitioner = getStateTransitioner(_preReceipt.stateRoot, _txHash);
        iOVM_StateCommitmentChain ovmStateCommitmentChain = iOVM_StateCommitmentChain(resolve("OVM_StateCommitmentChain"));
        ...
        require (
            _postReceipt.index == _preReceipt.index + 1,
        ...
        require(
            _postReceipt.stateRoot != transitioner.getPostStateRoot(),
        ...
        transitioners[keccak256(abi.encodePacked(_preReceipt.stateRoot, _txHash))] = iOVM_StateTransitioner(0x0000000000000000000000000000000000000000);
...
interface iOVM_CanonicalTransactionChain {
     ...
     * @return True if the ovm transaction hash equals to the one embedded in receipt.
     */
    function verifyTransaction(
        Lib_OVMCodec.Transaction memory _transaction,
        Lib_OVMCodec.Receipt memory _receipt
    )
        external
        view
        returns (
...
interface iOVM_StateCommitmentChain {
    ...
    function insideFraudProofWindow(
        Lib_OVMCodec.Transaction memory _ovmTransaction
    )
        external
        view
        returns (
...
interface iOVM_FraudVerifier {
    ...
    function initializeFraudVerification(
        Lib_OVMCodec.Receipt calldata _preReceipt,
        Lib_OVMCodec.Receipt calldata _postReceipt,
