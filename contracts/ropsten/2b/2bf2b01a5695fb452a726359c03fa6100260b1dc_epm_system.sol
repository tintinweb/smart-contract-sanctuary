...
contract epm_system  {
	...
	function paySmartContract(bytes32 smarthash,address[] addr, uint[] visits,bytes32 signDelegate) public payable returns (bool status, uint costForVisit, uint fee) {
	    if (!isEpocum()) revert();
