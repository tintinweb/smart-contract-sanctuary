...
abstract contract LotteryWinner {
    ...
    WinnerInfo[] internal _lastWinners;
    ...
    function addWinner(address winner, uint256 amount, uint256 time, uint256 currentIndex) internal returns (uint256 index) {
        ...
        if (_lastWinners.length > 2) {
            _lastWinners[0] = _lastWinners[1];
            _lastWinners[1] = _lastWinners[2];
            _lastWinners[2] = winnerInfo;
        } else {
            _lastWinners.push(winnerInfo);
        }
    }
    ...
    function getLastWinners() external view returns(WinnerInfo[] memory) {
        return _lastWinners;
    }
}
