...
contract ArtisLifeNFTMartketplace {
        ...
        offerIsNotCanceled(offerId)
    {
        ...
        IERC20 token = IERC20(nftsOffer[saleId].settlementToken);
        uint256 salePrice = nftsOffer[saleId].price_in_eth;
        ...
        offerIsNotCanceled(offerId)
    {
        ...
        if (token.balanceOf(msg.sender) > price) {
            nftsOffer[offerId].price_in_eth = price;
    ...
    modifier offerIsNotCompleted(uint256 offerId) {
        require(
            nftsOffer[offerId].completed == false,
            "Offer has settled."
        );
        _;
    }
