...
import "./interfaces/MigrationPools.sol";

/// @author  umb.network
contract StakingRewardsMigratable is MigrationPools, StakingRewards {
    ...
    function migrateRewardToken(
        StakingRewardsMigratable _newPool,
    ...
    function migrateTokenCallback(address _token, address _user, uint256 _amount, bytes calldata)
        external
        override
        onlyPool
    {
        require(_token == address(stakingToken), "migration not possible");

        _stake(_user, _amount, true);
    }
}
...
abstract contract MigrationPools is Ownable {
