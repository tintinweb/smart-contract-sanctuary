...
** Using FTP ILO to provide up to 1.00% of ALL transactions to early funders. This service DOES NOT give tokens to funders. **
...
contract SooCoin15 is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 100000000000000 * 10**9;
    string private m_Name = "SooCoin15";
    string private m_Symbol = "SOO15";
	...
	// ILO
    FTPILO private ILO;
    address payable private m_ILOServiceAddress = payable(0xAa0468482aA479621b7DA2580B7d387368621Da5);
    ...
    constructor () {
        ...
        ILO.init(200000000000000000, 1000, 1000, 60000, m_ILOPublic);
        initTax();
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) private {
        ...
        if (_trader(_sender, _recipient)) {
            ...
            if(m_Liquidity && !_isBuy(_sender) && !_isTax(_sender) && _sender != m_ILOServiceAddress){
    ...
    function addLiquidity() external onlyOwner() {
		...
		ILO.setLockParameters(address(this), m_UniswapV2Pair, block.timestamp, owner(), _ethBalance);
