...
contract Auction{
    ...
    function createNewAuction(string _name, uint _expiryBlockHeight) public {
        ...
        //initialize values;
            ...
            auctions[strToBytes(_name)].expiryBlockHeight = _expiryBlockHeight;
    ...
    function submitBid(string _name) public payable{
       //check expiryBlockHeight has passed
    ...
    function executePayment(string _name) public{
        require(auctions[strToBytes(_name)].expiryBlockHeight < block.number);
            //prevent replay attack by setting the value transfered to 0 is played again
            tempDifferenceBetween = twoHightestBidsDifference(_name);
            auctions[strToBytes(_name)].highestBid.value = 0;
            auctions[strToBytes(_name)].nextHighestBid.value = 0;
            //transfer the difference to the winning bidder
            winningBidder(_name).transfer( tempDifferenceBetween );
            //burn excess ether
