...
contract Token is ERC721, Ownable {
    ...
    function playerMint(
        uint256 strength,uint256 magic,uint256 dexterity,
        uint256 wisdom,uint256 intelligence,uint256 endurance) external payable {
        uint256 totalPoints = strength + magic + dexterity + wisdom + intelligence;
        require(!paused);
        ...
        _tokenDetails[nextId] = Pet(strength,magic,dexterity,wisdom,intelligence,block.timestamp,endurance);
        _safeMint(msg.sender, nextId);
     ...
     */
    function sendToArena() external onlyOwner {
        payable(address(arena)).transfer(address(this).balance);
    }
    // REMOVE
