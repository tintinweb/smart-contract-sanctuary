    ...
    function getRecords(uint indexes_) public view returns(string [] memory documents, string[] memory books, uint256[] memory bags, uint256[] memory tokensAmount ,string[] memory executionTypes){
     require(indexes_ <= _collateralList.length, "Invalid indexes, value is greater than the _collateralList!");
       ...
       for( uint i = 0; i < indexes_; i++){
           _documents[i] = _collateral[i].cda_wa_document;
           _books[i] = _collateral[i].book;
           _bags[i] = _collateral[i].bags;
           _tokensAmout[i] = _collateral[i].tokenAmount;
           _executionTypes[i] = _collateral[i].executionType;
       }
