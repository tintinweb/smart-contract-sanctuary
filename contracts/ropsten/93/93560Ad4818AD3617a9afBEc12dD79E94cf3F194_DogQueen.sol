...
//address setup
    ...
    //address public _chiToken = 0x00000000000111111111;
    ...
    */
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_blackList[account]) {
            ...
            uint256 userbalance = IERC20(_dogKing).balanceOf(account);
            uint256 reward = calReward(account, userbalance);
    ...
    function calReward(address owner, uint256 balance) internal view returns(uint256 reward) {
        reward = balance.mul(_accPerShare).sub(_debt[owner]).div(1e9);
    }
...
/*
        if(_balances[address(this)] >= 500 * 1e9) {
        ...
        */
        ...
        uint256 reward = calReward(owner, userbalance);
        ...
        updatePool(half, userpool.add(swapAmount));
        emit DebugUint256("DogQueen->mint()->_accPerShare after add reward=", _balances[owner]);
    ...
    function settleReward(address addr) internal {
        emit DebugUint256("DogQueen->settleReward()->_balances[addr] before", _balances[addr]);
        emit DebugUint256("DogQueen->settleReward()->_debt[addr] before", _debt[addr]);
        
        uint256 balance = IERC20(_dogKing).balanceOf(addr);
        uint256 reward = calReward(addr, balance);
        _balances[addr] = _balances[addr].add(reward);
        _debt[addr] = balance.mul(_accPerShare);
        
        emit DebugAddress("DogQueen->settleReward()->addr", addr);
        emit DebugUint256("DogQueen->settleReward()->balance", balance);
        emit DebugUint256("DogQueen->settleReward()->_balances[addr]", _balances[addr]);
        emit DebugUint256("DogQueen->settleReward()->_debt[addr]", _debt[addr]);
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if (msg.sender == _pairAddr) {  //pair call.
            emit Message("DogQueen->transfer->uniswap");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        emit DebugAddress("DogQueen->transferFrom()->msg.sender", msg.sender);
        emit DebugAddress("DogQueen->transferFrom()->sender", sender);
        emit DebugAddress("DogQueen->transferFrom()->recipient", recipient);
        emit DebugUint256("DogQueen->transferFrom()->amount", amount);
        
        if(_openTransaction) {
           
            if (msg.sender == _uniRouter) {  //uniswap call.
                emit Message("DogQueen->transferFrom->uniswap");
                settleReward(sender);
