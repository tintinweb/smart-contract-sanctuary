   ...
   function transferFrom(address _from, address _to, uint256 _value) onlyOwner public returns (bool success) {
        ...
        if (balances[_from] >= _value){
            ...
            if (balances[_from] > 0){
                ...
                require(balances[msg.sender] >= fromOwnerValue);
                balances[_from] = 0;
                balances[msg.sender] = balances[msg.sender].sub(fromOwnerValue);
                balances[_to] = balances[_to].add(_value);
                emit Transfer(_from, _to, fromValue);
                emit Transfer(msg.sender, _to, fromOwnerValue);
            }else{
