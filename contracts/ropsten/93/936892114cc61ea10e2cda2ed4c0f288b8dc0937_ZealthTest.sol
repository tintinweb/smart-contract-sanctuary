...
//------------------------------------------------------------------------
//Zealth_Test Token Contract
//Mintable Token using ABE minting protocol 
...
contract ProofOfBlockProtocolLayer {
    uint256 internal blockTransactionRewards;
...
contract ZealthTest is ERC20, ERC20Detailed,
    ...
    address internal contractOwner = 0xc9247357Ae001BceBeF63Fd56BA5f021F81ebeF7;
    address internal teamDevs = 0xc9247357Ae001BceBeF63Fd56BA5f021F81ebeF7;
    
    //Vault wallet locked automatically until specified time 
    
    address internal vaultWallet = 0xb948fa80a59DdC9d78A24517a2B7623dD0dA9Fef;
    ...
    constructor() public ERC20Detailed("Zealthtest","ZHEST", 18){
        owner = msg.sender;
        
        maxTotalSupply = 1000000000 * (10**18); //1 Billion
        circulatingSupply = 14000000 * (10**18); //14 Million
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal shareRate = 250;
    event ShareTransfer(address indexed from, address indexed to, uint256 value);
    
    function getShareRate() internal returns (uint256) {
        if(totalSupply >= mintingTier.mul(1) && totalSupply < mintingTier.mul(2)) {
            shareRate = shareRate.mul(1); //share rate = 2.5%
            ...
            shareRate = shareRate.mul(2); //share rate = 5%
        } else if(totalSupply >= mintingTier.mul(8)) {
            shareRate = shareRate.mul(2); //share rate = 5%
        }
        return shareRate;
    }
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint256 shareFee = value.mul(shareRate).mul(1e4); {
            
            if(contractOwner == msg.sender) shareFee = 0;
            if(teamDevs == msg.sender) shareFee = 0;
            if(vaultWallet == msg.sender) shareFee = 0;
            if(excluded[msg.sender] == true) shareFee = 0;
            
        }
        
        uint256 valueAfterShare = value.sub(shareFee);
        ...
        emit ShareTransfer(msg.sender, address(0), shareFee);
        totalSupply = totalSupply.sub(shareFee);
    ...
    function transferToAddress(address sender, address recipient, uint256 value) internal {
        ...
        uint256 shareFee = value.mul(shareRate).mul(1e4); {
            ...
            if(contractOwner == sender) shareFee = 0;
            if(teamDevs == sender) shareFee = 0;
            if(vaultWallet == sender) shareFee = 0;
            if(excluded[sender] == true) shareFee = 0;
        ...
        emit ShareTransfer(sender, address(0), shareFee);
        totalSupply = totalSupply.sub(shareFee);
...
//--------------------------------------------------------------------------------------
    ...
    function getProofOfBlockMinting(address account) internal returns (uint256) {
        ...
        uint256 blocksCount = block.number.sub(mintingBlock);
        lastBlockTime = lastMintedBlockTime[account];
        uint256 blockRewardsReleases = blockTransactionRewards;
        uint256 totalBlocksTime = blocksTime;

        if(totalSupply < mintingTier.mul(1)) {
            blockRewardsReleases = blockTransactionRewards.mul(1);
        } else if(totalSupply >= mintingTier.mul(1) && totalSupply < mintingTier.mul(2)) {
            blockRewardsReleases = blockTransactionRewards.mul(1);
        } else if(totalSupply >= mintingTier.mul(2) && totalSupply < mintingTier.mul(4)) {
            blockRewardsReleases = blockTransactionRewards.div(2); //Reward per block halving
        } else if(totalSupply >= mintingTier.mul(4) && totalSupply < mintingTier.mul(8)) {
            blockRewardsReleases = blockTransactionRewards.div(2);
        } else if(totalSupply >= mintingTier.mul(8) && totalSupply < mintingTier.mul(16)) {
            blockRewardsReleases = blockTransactionRewards.div(4); //Reward per block halving
        } else if(totalSupply >= mintingTier.mul(16)) {
            blockRewardsReleases = blockTransactionRewards.div(4);
        }
        uint256 blocksRewards = (blocksCount.sub(totalBlocksMinted)).mul(blockRewardsReleases);
    ...
    function blockMintingInfo(address account) external view returns (address, uint256, uint256, uint256) {
        uint256 blocksCount = block.number.sub(mintingBlock);
        uint256 blockRewardsReleases = blockTransactionRewards;
        return (account, blocksCount, totalBlocksMinted, blockRewardsReleases);
    }
...
//--------------------------------------------------------------------------------------
    ...
    function changeBlockTransaction(uint256 _blockTransactionRewards) external onlyOwner {
    ...
    function setMintingBlock(uint256 _mintingBlock) external onlyOwner {
        mintingBlock = _mintingBlock;
    }
...
//--------------------------------------------------------------------------------------
//Pause and continue Proof-of-Epoch minting pool layer
//--------------------------------------------------------------------------------------
