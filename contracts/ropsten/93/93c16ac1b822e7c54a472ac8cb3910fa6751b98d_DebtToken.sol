...
contract DebtToken is ERC20, Ownable {
    ...
    uint256 private initialLiquidity = 37700000 * 10 ** 18;
    uint256 private totalTokensICO; 
    uint256 private maxBuyICO;
	uint256 public unclaimedFees;
    address public debtUniswapLPContract;
    mapping (address => uint256) private _balances;  
    ...
    constructor() public ERC20("Debt Protocol", "DEBT") { 
        ...
        transferPauseExemptList[address(this)] = true;
...
//------------------------------------------------------------//
    ...
    function setDebtUniswapLPContract(address _newDebtUniswapLPContract)
        ...
        onlyOwner
    {
        debtUniswapLPContract = _newDebtUniswapLPContract;
    ...
    function manageTier(address _user, uint256 _newBalance, uint256 _oldBalance) internal {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (boostBalance < tiers[i].threshold) {
                ...
                if (oldTier != i) {
                    user.currentTier = i;
                    if (_newBalance != initialLiquidity) tierB.tierSupply = tierB.tierSupply.add(_newBalance);  // Prevents the initial liquidity to count in the upper tier
