...
contract Charity_5{
    ...
    function PayCharity() payable public returns (bool) {
        ...
        if(Aperson[persons[0]].exists){
            while( someval >= Aperson[persons[0]].needval ){
                Withdraw(persons[0]);
            }
        }
        else{
  ...
  /*  function removearr(uint64 _index) internal return(address[]) {
        //if (index >= array.length) return;
        require(_index < persons.length);
        for (uint64 i = _index; i < persons.length-1; i++){
            persons[i] = persons[i+1];
        }
        delete persons[persons.length-1];
        return 
       // return array;
    }*/
