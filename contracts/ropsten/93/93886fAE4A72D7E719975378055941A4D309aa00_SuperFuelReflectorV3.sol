...
abstract contract NoBSDynamicReflector is ISuperFuelReflector, AuthorizedListExt, LPSwapSupport, LockableFunction {
    ...
    constructor(address _lpRouter, address _controlToken, address _rewardsToken) public payable {
    ...
    function _process(uint256 gas) private lockFunction returns(string memory, uint256, uint256) {
        ...
        uint256 iterationIndex = currentIndex < shareholders.length ? currentIndex : 0;
        uint256 startIndex = iterationIndex;

        while(gasUsed < gas && iterationIndex < shareholderCount) {
            rewardsSent = rewardsSent.add(distributeDividend(shareholders[currentIndex]));
            iterationIndex++;
            gasUsed = startGas.sub(gasleft());
        }
        string memory rewardName = rewardTokenInfo.name;
        (currentIndex, iterationIndex) = (iterationIndex >= shareholderCount ? 0 : iterationIndex, iterationIndex.sub(startIndex));
...
interface ISuperFuelReflector is INoBSDynamicReflector, IAuthorizedListExt, IAuthorizedList {
