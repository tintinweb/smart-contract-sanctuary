...
contract EthTokenSwapPoolContract is ReentrancyGuarded {
    ...
    event BurnedFrom(address,uint256);
    event Swapped(address,uint256);
    
   
     ERC20Token public tokenContractAddress;
...
function requestSwap(address payable _requestor, uint256 _amount) public nonReentrant poolActive
{
    ...
    SafeERC20.safeTransferFrom(tokenContractAddress, _requestor, address(0),smallestTokenUnits);
     emit BurnedFrom(_requestor, _amount);
...
function createTokenPoolContract(ERC20Token _tokenaddr, uint256 etherInPool) internal returns(EthTokenSwapPoolContract)
{
...
function buildTokenPool(ERC20Token _tokenaddr, uint256 etherInPool) public
{
...
function getTokenPoolAddress(ERC20Token _tokenaddr) public view returns(EthTokenSwapPoolContract _tokenPool)
{
...
//get the token pool address if the token pool already exist
function getTokenSwapRate(ERC20Token _tokenaddr) public view returns(uint256 _swaprate)
{
     require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
     return PoolsRecords[address(_tokenaddr)].tokenPool.swapRate();
}

function swapToken(ERC20Token _tokenaddr, uint256 _amount) public
{
    require(PoolsRecords[address(_tokenaddr)].exist, "The token pool does not exist");
