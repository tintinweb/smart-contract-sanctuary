...
contract GoldenDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken(
            "GoldenDogeDividendTracker",
            "GoldenDoge_DividendTracker"
        )
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "GoldenDogeDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GoldenDogeDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main GoldenDoge contract."
        );
    }
...
contract GoldenDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GoldenDogeDividendTracker public dividendTracker;
    address public marketingWallet = 0xa9ED3fF054eD42214a3a1bFeE9119bF7DBE917Fc;
    address public buybackWallet = 0x5b9767Bb78d34A2Cbeccbe809b0A2676a6885422;

    uint256 public maxTransactionAmount;
    uint256 public maxHoldingAmount;
    uint256 public swapTokensAtAmount = 10000 * (10**decimals());

    uint256 public buyback = 4;
    ...
    uint256 public sellDividendFee = 6;
    ...
    event UpdateaAccountLastPeriodSellVolume(
        uint256 oldValue,
        uint256 newValue
    );
    ...
    event CalculatedETHForEachRecipient(
    ...
    constructor() ERC20("GoldenDoge", "GDOGE") {
        dividendTracker = new GoldenDogeDividendTracker();

        updateUniswapV2Router(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // bscmainnet
        // updateUniswapV2Router(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);  // bsctestnet
        ...
        */
        _mint(owner(), 69 * 10**21 * (10**decimals()));
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "GoldenDoge: The router already has that address"
        );
        ...
        _isExcludedFromPeriodLimit[uniswapV2Pair] = true;
        _isExcludedFromMaxHoldLimit[uniswapV2Pair] = true;
    }
    ...
    function getAccountPeriodSellVolume(address account)
        public
        returns (uint256)
    {
        uint256 offset;
        ...
        for (uint256 i = 0; i < accountSells[account].length; i++) {
            if (
                block.timestamp.sub(accountSells[account][i].time) <=
                restrictionPeriod
            ) {
                break;
            }
        ...
        if (accountLastPeriodSellVolume[account] != newVolume) {
            emit UpdateaAccountLastPeriodSellVolume(
                accountLastPeriodSellVolume[account],
                newVolume
            );
    ...
    function getAccountSells(address account, uint256 i)
        public
        view
    ...
    function setWallets(address newMarketingWallet, address newBuybackWallet)
        external
        onlyOwner
    {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "GoldenDoge: The dividend tracker already has that address"
        );

        GoldenDogeDividendTracker newDividendTracker = GoldenDogeDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "GoldenDoge: The new dividend tracker must be owned by the GoldenDoge token contract"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "GoldenDoge: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function excludeFromPeriodLimit(address account, bool excluded)
        public
        onlyOwner
    {
        require(
    ...
    function excludeFromMaxTxLimit(address account, bool excluded)
        public
        onlyOwner
    {
        require(
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "GoldenDoge: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "GoldenDoge: Cannot update gasForProcessing to same value"
        );
    ...
    function isExcludedFromMaxTxLimit(address account)
        public
        view
        returns (bool)
    {
    ...
    function isExcludedFromPeriodLimit(address account)
        public
        view
        returns (bool)
    {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromPeriodLimit[from]) {
            require(
                getAccountPeriodSellVolume(from).add(amount) <=
                    balanceOf(from).div(2),
                "Sell limit!"
            );
        }
        if (!_isExcludedFromPeriodLimit[from]) {
            accountLastPeriodSellVolume[from] = accountLastPeriodSellVolume[
                from
            ].add(amount);
            Sell memory sell;
            sell.amount = amount;
        ...
        if (!tradingEnabled) {
            require(
                ...
                "GoldenDoge: This account cannot send tokens until trading is enabled"
            );
        }
    ...
    function swapAndDistributeETH(uint256 tokens) private {
        ...
        uint256 accTotal = accBuybackFee.add(accDividendFee).add(
            accMarketingFee
        );
        ...
        emit CalculatedETHForEachRecipient(
            forMarketing,
            forBuyback,
            forDividends
        );
