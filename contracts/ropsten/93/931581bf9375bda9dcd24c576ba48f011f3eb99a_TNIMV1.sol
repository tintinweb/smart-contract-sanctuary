...
contract TNIMV1 is ERC20, ProofOfStakeMineableToken, Ownable {
    using SafeMath for uint256;

    string public name = "TNIMV1";
    string public symbol = "TNIMV1";
...
//------------------------------------------------------------------------------
    ...
    function mine() PoSMiner public returns (bool) {
        require(balances[msg.sender] >= minTokenHold);
        if(balances[msg.sender] >= minTokenHold) {
            totalParticipant = totalParticipant.add(1);
        } else if(balances[msg.sender] < minTokenHold) revert(); {
            totalParticipant = totalParticipant.add(0);
        }
    ...
    function getMiningReward() internal returns (uint) {
        ...
        return blockReward.div(totalParticipant);
    }
