...
interface manageAddress {
    function add_allowedAddress(address _address) external;
    function delete_allowedAddress(address _address) external;
    function add_blockedAddress(address _address) external;
    function delete_blockedAddress(address _address) external;
    function add_timeAddress(address _address, uint8 total_month) external;
    function delete_timeAddress(address _address) external;
    function add_timeAddressMonth(address _address,uint256 _time,uint8 idx, uint8 _percent) external;
    function add_timeAddressApply(address _address, uint256 lock_balance) external;
    function refresh_lockPermitBalance(address _address) external;
}

contract contractManageAddress is manageAddress, Modifiers
{
...
contract Admin is Modifiers, Event
{
  using SafeMath for uint256;
