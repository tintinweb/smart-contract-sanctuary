...
// File: contracts/MetacryptGeneratorInfo.sol
...
// File: contracts/MetacryptHelper.sol
...
// File: contracts/Metacrypt_B_TR_TAX_X.sol
...
contract Metacrypt_B_TR_TAX_X is IERC20, ERC20Ownable, MetacryptHelper, MetacryptGeneratorInfo {
    ...
    uint8 public maxFee = 50;
    uint8 public minMxTxPercentage = 2;
    uint8 public minMxWalletPercentage = 2;
    ...
    uint8 public _taxFee = 25; // Fee for Reflection
    uint8 private _previousTaxFee = _taxFee;

    uint8 public _liquidityFee = 20; // Fee for Liquidity
    ...
    uint8 public _charityFee = 20; // Fee to charity wallet
    uint8 private _previousCharityFee = _charityFee;

    uint8 public _marketingFee = 35; // Fee to marketing wallet
    ...
    address payable public marketingFeeWallet; // Wallet for marketing fee
    address payable public charityFeeWallet; // Wallet for Charity fee
    address public burnWallet; // wallet for burning
    address public buyBackWallet; // wallet for buyback
    ...
    constructor(
        ...
        marketingFeeWallet = payable(_msgSender());
        charityFeeWallet = payable(_msgSender());
        burnWallet = address(this);
    ...
    // function for setting marketing fee wallet
    function setMarketingFeeWallet(address payable newMarketingFeeWallet) external onlyOwner {
        require(newMarketingFeeWallet != address(0), "ZERO ADDRESS");
        excludeFromReward(newMarketingFeeWallet);
        marketingFeeWallet = newMarketingFeeWallet;
    }

    // function for setting charity fee wallet
    function setCharityFeeWallet(address payable newCharityFeeWallet) external onlyOwner {
        require(newCharityFeeWallet != address(0), "ZERO ADDRESS");
        excludeFromReward(newCharityFeeWallet);
        charityFeeWallet = newCharityFeeWallet;
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        ...
        return _amount.mul(_liquidityFee + _burnFee + _charityFee + _marketingFee + _buybackFee).div(10**3);
    }

    function removeAllFee() private {
        // if (_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _walletFee == 0 && _buybackFee == 0) return;
        if (_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _charityFee == 0 &&  _marketingFee == 0 && _buybackFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint8 totFee = _burnFee + _charityFee + _marketingFee + _liquidityFee + _buybackFee;
        ...
        if (_marketingFee != 0) {
            ...
            _tokenTransferNoFee(address(this), marketingFeeWallet, spentAmount);
            totSpentAmount = totSpentAmount + spentAmount;
        }

        if (_charityFee != 0) {
            spentAmount = contractTokenBalance.div(totFee).mul(_charityFee);
            _tokenTransferNoFee(address(this), charityFeeWallet, spentAmount);
            totSpentAmount = totSpentAmount + spentAmount;
        }
