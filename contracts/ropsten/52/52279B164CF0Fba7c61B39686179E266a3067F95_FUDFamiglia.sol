...
contract FUDFamiglia is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    function mintMafia(uint amount) internal{
  
        require(totalSupply().add(amount) <= (_fudfSupply + 1 - _airdrop_giveaway_amount), "Minting would exceed max supply");
    ...
    {
        require(totalSupply().add(hashed_code.length) <= _fudfSupply,"Codes would exceed max supply");
    ...
    function createAirDropCode(bytes32 code) public onlyOwner {
        ...
        require(totalSupply().add(1) <= _fudfSupply,"Codes would exceed max supply");
    ...
    function revealAttribute(string memory baseURI) public onlyOwner {
        _setBaseURI(baseURI);
        _baseURI = baseURI;
    }
