...
contract StoreBase is ERC165, Ownable, ERC1155Metadata_URI,  ERC1155Base {
    ...
    mapping (uint256 => uint256) public limitedEdition;
...
// File: contracts/DirtyPunkStore.sol
...
contract DirtyPunkStore is Ownable, SignerRole, StoreBase {
    string public name;
    ...
    function addToBid( uint256 _id, uint _qty) public onlyOwner {
        require(_qty > 0, "_qty must not be zero");
        creators[_id] = owner();
        limitedEdition[_id] = _qty;
    }
    
    function removeFromBid( uint256 _id) public onlyOwner {
        limitedEdition[_id] = 0;
    }
    ...
    function executeBid(
        ...
        require(limitedEdition[id] > 0, "closed or doesnt exist");   
        require(sellForValue > 0, "sellForValue must be greater than zero");
        require(
            blockExpiry >= block.number &&
            blockExpiry <= (block.number + blockLimit),
            "signature expired"
        ...
        if(sellForCrypto == address(0)){
            require( msg.value == sellForValue, "invalid value");
            payable(owner()).transfer(sellForValue);
        }
        else {
            require(msg.value == 0, "Should not pass value");
            require(IERC20(sellForCrypto).transferFrom(msg.sender, owner(),sellForValue), "sellForCrypto transfer failed");
        }
        
        messageHash[msgHash] = true;
        limitedEdition[id] = limitedEdition[id].sub(1);
        
         _mint( owner(), to, id, 1, uri);
    }
