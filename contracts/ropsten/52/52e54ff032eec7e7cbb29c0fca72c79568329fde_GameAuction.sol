...
contract GameAuction {
    ...
    modifier onlyIfBidsNotFull(string memory team){
        ...
        if (((keccak256(abi.encodePacked(team))) == ((keccak256(abi.encodePacked("E"))))) && (numberOfBidsE >= 1)) revert BidsFull(team);
        _;
    }
        ...
        onlyIfTeamIsValid(_teamName)
    {
        ...
        if ((numberOfBidsP >=2) && (numberOfBidsE >=1)) { // TESTINGTODO:Change numbers accordingly
            stage = 2;
        ...
        onlyBefore(revealEnd)
    {
        ...
        if ((numberOfRevealsP >= 2) && (numberOfRevealsE >=1)) { // TESTINGTODO:Change numbers accordingly
            stage = 0;
            auctionEnd();
        }
    }
