...
/**
 * @title A Presale contract made with ❤️ for the AUG folks.
 * @dev   Enable whitelisted investors to contribute to the AUG presale.
 */
contract AUGPresale {
    using SafeMath for uint256;

    uint256 constant ETH_PRICE_BASE = 1 ether;
    string constant ERROR_ADDRESS = "AUG > Invalid address";
    string constant ERROR_PRICE = "AUG > Invalid price";
    string constant ERROR_CAP = "AUG > Invalid cap";
    string constant ERROR_ARRAY = "AUG > Invalid array [too long]";
    string constant ERROR_OPENED = "AUG > Presale opened";
    string constant ERROR_NOT_OPENED = "AUG > Presale not opened yet";
    string constant ERROR_OVER = "AUG > Presale over";
    string constant ERROR_NOT_OVER = "AUG > Presale not over yet";
    string constant ERROR_NOT_WHITELISTED = "AUG > Address not whitelisted";
    string constant ERROR_ALREADY_WHITELISTED =
        "AUG > Address already whitelisted";
    string constant ERROR_GLOBAL_CAP = "AUG > Global cap reached";
    string constant ERROR_INDIVIDUAL_CAP = "AUG > Individual cap reached";
    string constant ERROR_ERC20_TRANSFER = "AUG > ERC20 transfer failed";
    ...
    modifier protected() {
        require(msg.sender == admin, "AUG > Protected operation");
        _;
    }
    ...
    /**
     * @dev          Deploy and initialize the AUG presale contract.
     * @param _admin The address of the admin allowed to perform protected operations.
     * @param _bank  The address to which received ETH and remaining AUG tokens are gonna be sent once the presale closes.
     * @param _token The address of the AUG token.
    ...
    /**
     * @dev Close the presale. Close buys and open whithdrawal operations. Withdraw received ETH and remaining AUG tokens.
     */
    ...
    /**
     * @dev Transfer any remaining AUG tokens hold by this contract to the bank.
     */
    ...
    receive() external payable isRunning {
        ...
        require(invested[msg.sender] <= INDIVIDUAL_CAP, ERROR_INDIVIDUAL_CAP);
        // transfer token
        // PLEASE MAKE SURE AUG ERC20 returns true on success.
   ...
   function _setPrice(uint256 _ETHPrice) private {
        ETH_PRICE = _ETHPrice;
        TOKEN_PRICE = _ETHPrice.mul(ETH_PRICE_BASE).mul(uint256(3)).div(
            uint256(1000)
        );
