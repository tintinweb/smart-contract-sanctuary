...
contract LibertyNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    event MintedToken(address indexed owner, uint256 indexed tokenId, uint256 index, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength, uint256 createTime, uint256 updateTime);
    event TokenURIUpdated(uint256 indexed tokenId, string tokenURI, uint256 updateTime);
    event BurnedToken(address indexed owner, uint256 indexed tokenId);
    
    event FeeAddressTransferred(address indexed previousOwner, address indexed newOwner);
    event SetPricePerPixel(uint256 oldPricePerPixel, uint256 newPricePerPixel);
    event SetModifyFeeAmount(uint256 oldModifyFeeAmount, uint256 newModifyFeeAmount);
    ...
    constructor(
        ...
        emit SetPricePerPixel(0, pricePerPixel);
    }
    ...
    function mint(address to, string memory _tokenURI, uint256 index, uint256 startX, uint256 startY, uint256 xLength, uint256 yLength) public returns (uint256 tokenId) {
        ...
        uint256 mintFee = pricePerPixel.mul(xLength).mul(yLength);
        TransferHelper.safeTransferFrom(mintFeeTokenAddr, _msgSender(), feeAddr, mintFee);
        tokenId = nextTokenId;
        _mint(to, tokenId);
        nextTokenId++;
        ...
        emit MintedToken(_msgSender(), tokenId, index, startX, startY, xLength, yLength, block.timestamp, block.timestamp);
    }
    ...
    function burn(uint256 tokenId) public {
        ...
        emit BurnedToken(_msgSender(), tokenId);
    }
    ...
    function setTokenURI(uint256 tokenId, string memory tokenURI) public whenNotPaused {
        require(_isApprovedOrOwner(_msgSender(), tokenId), 'caller is not owner nor approved');
        TransferHelper.safeTransferFrom(modifyFeeTokenAddr, _msgSender(), feeAddr, modifyFeeAmount);
        string memory oldTokenURI = tokenURI;
        _setTokenURI(tokenId, tokenURI);
        Liberty storage liberty = nftLibertyMap[tokenId];
        liberty.updateTime = block.timestamp;
        emit TokenURIUpdated(tokenId, tokenURI, block.timestamp);
    }
    ...
    function setPricePerPixel(uint256 _pricePerPixel) public onlyOwner {
        require(pricePerPixel != _pricePerPixel, 'Not need update');
        emit SetPricePerPixel(pricePerPixel, _pricePerPixel);
        pricePerPixel = _pricePerPixel;
    }
    ...
    function setModifyFeeAmount(uint256 _modifyFeeAmount) public onlyOwner {
        ...
        emit SetModifyFeeAmount(modifyFeeAmount, _modifyFeeAmount);
        modifyFeeAmount = _modifyFeeAmount;
    }
}