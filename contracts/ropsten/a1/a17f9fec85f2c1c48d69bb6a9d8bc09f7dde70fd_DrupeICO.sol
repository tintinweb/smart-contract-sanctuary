...
contract DrupeICO {
	...
	event Referrer(address referrer, address refContract);
	...
	// Modifier to ensure that a function is only called after the ico:
	modifier afterIcoOnly() {
	...
	// (It is assumed that this function is called during the ico)
	...
	function() public payable icoOnly {
		...
		uint tokens = rawTokens.mul(bonus.n).div(bonus.d);
	...
	function purchase(address payout, address referrer) public payable icoOnly returns (uint tokens) {
		...
		uint rawTokens = msg.value.mul(_basePrice.d).div(_basePrice.n);
		// Calculate the amount of tokens including bonus:
		tokens = rawTokens.mul(bonus.n).div(bonus.d);
	...
	function burnUnsoldTokens() public ownerOnly afterIcoOnly {
