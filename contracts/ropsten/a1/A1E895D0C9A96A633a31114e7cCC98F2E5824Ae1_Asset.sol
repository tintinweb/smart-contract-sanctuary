...
abstract contract Mintable is Ownable, IMintable {
    ...
    mapping(uint256 => bytes) public nameValues;
    ...
    function mintFor(
        ...
        (uint256 id, bytes memory blueprint, bytes memory nameValue) = Minting.split(mintingBlob);
        _mintFor(user, id, blueprint);
        blueprints[id] = blueprint;
        nameValues[id] = nameValue;
...
library Minting {
        ...
        returns (uint256, bytes memory, bytes memory)
    {
        ...
        if (blueprintLength == 0) {
            return (tokenID, bytes(""), bytes(""));
        }
        bytes calldata blueprint = blob[uint256(index) + 2:blob.length - 2];
        
        int256 indexComma = Bytes.indexOf(blueprint, ",", 0);
        require(indexComma >= 0, "Separator must exist");
        bytes calldata nameKV = blueprint[1:uint256(indexComma) - 1];
        
        int256 indexCN = Bytes.indexOf(nameKV, ":", 0);
        require(indexCN >= 0, "Separator must exist");
        
        bytes calldata nameValue = nameKV[uint256(indexCN) + 1: nameKV.length];
        
        return (tokenID, blueprint, nameValue);
    }
}
