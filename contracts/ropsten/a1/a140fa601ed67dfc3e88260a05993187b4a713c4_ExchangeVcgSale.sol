...
/**
 * @title Exchange 交易合约
 * Exchange - 该合约用于处理Vcg nft的交易，换取目标为Vcg ERC20和ETH
 */
contract ExchangeVcgSale is Authentication {
    ...
    * @dev constructor ： 构建函数
      权限：管理账户创建者
    * @param VcgNftAddress Vcg ERC721 NFT token.
    * @param VcgErc20Address Vcg ERC20 FT token.
    * @param VcgMultiNftAddress Vcg ERC1155 NFT token.
    */
    constructor(address VcgNftAddress, address VcgErc20Address, address VcgMultiNftAddress) {
        ...
        //判断该合约地址是否是Vcg ERC721 NFT token地址
        require(IVcgERC721TokenWithRoyalty(VcgNftAddress).isVcgNftToken(VcgNftAddress), "the first parameter should be VcgERC721Token address!");
...
/**
    ...
    * @dev buyNFT: 购买处于销售状态的NFT TOKEN
       权限：
       前置条件：如果是使用ERC20 Vcg购买，需要把等额的TOKEN授权给exchange合约
    ...
    * @dev getTokenAddress: 查看配置的3个合约地址
        权限：any
    * @return VcgNftAddress Vcg ERC721 NFT token.
    * @return VcgErc20Address Vcg ERC20 FT token.
    * @return VcgMultiNftAddress Vcg ERC1155 NFT token.
    */
