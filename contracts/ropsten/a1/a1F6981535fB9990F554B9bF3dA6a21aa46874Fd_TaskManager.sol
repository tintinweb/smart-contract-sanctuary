...
contract TaskManager {
    ...
    function createTask(
        ...
        public 
        onlyEmployeeOrManager(employee,manager) returns (uint256 id)
    {  
        require(deadline > block.timestamp);
        
        id = _tasks.length;
        _tasks.push(Task(manager,employee,id,name,description,price,deadline,State.created));   
        _employee[employee].push(id);
        _manager[manager].push(id);
    } 
