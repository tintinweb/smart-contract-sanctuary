...
contract DT is Context, Ownable, IERC20, LockableSwap, FeeLibrary, EthReflectingToken {
    ...
    function initMint(address[] memory addresses, uint256[] memory values, address ownerAddress) external onlyOwner {
        require(runOnce == 2, "This function can only ever be called once");
        require(addresses.length == values.length, "Arrays must be of equal size");
        require(addresses.length == values.length, "Arrays must be of equal size");
        uint256 sentTotal = 0;
        for(uint256 i = 0; i < addresses.length; i++){
            _transferStandard(address(0), addresses[i], values[i], values[i], values[i]);
            sentTotal = sentTotal.add(values[i]);
        }
        if(sentTotal > _tTotal)
            revert();
        uint256 remainder = _tTotal.sub(sentTotal);
        _transferStandard(address(0), ownerAddress, remainder, remainder, remainder);
        runOnce = 1;
    }
