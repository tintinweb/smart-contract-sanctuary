...
contract MasterChef1 is Ownable {
    ...
    function pendingSushi(uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 sushiReward = 0;
        if (lpSupply != 0) {
            sushiReward = (sushiPerBlock * pool.allocPoint) / totalAllocPoint;
        }
    ...
    function getReward(uint256 _pid) public {
        uint256 sushiReward = pendingSushi(_pid, msg.sender);
        sushi.mint(address(this), sushiReward);
        sushi.transfer(msg.sender, sushiReward);
    }
}