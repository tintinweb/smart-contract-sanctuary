...
contract epocum is decentralizedNetwork {
    ...
    function getAcceptance(bytes32 _smartHash,address addr) public constant returns(string dLink,string web,uint nConn) {
	    dLink = acceptByAddress[addr][_smartHash].dLink;
	    web = acceptByAddress[addr][_smartHash].website;
    }
    
    function getMyAcceptance(address addr,uint y) public constant returns(string dLink,bytes32 smartHash) {
       dLink =  myAcceptance[msg.sender][y].dLink;
       smartHash = myAcceptance[msg.sender][y].smartHash;
    }
