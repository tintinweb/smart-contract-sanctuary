...
contract Inventory{
    ...
    event subtractingSmallSizeInventory(uint amount);
    event subtractingMediumSizeInventory(uint amount);
    event subtractingLargeSizeInventory(uint amount);
    event subtractingxLargeSizeInventory(uint amount);
    event subtractingxxLargeSizeInventory(uint amount);
...
function subSmallSizeInventory(uint _amount) public {
        require(msg.sender == owner, "this function is restricted to the owner");
             sizes storage smallSizes_ = smallSizes;
             
        if (smallSizes_.amount < _amount){
            revert("Amount is Greater Than Supply");
            
        } else {
        smallSizes_.amount -= _amount; 
        emit subtractingSmallSizeInventory(_amount);
        }
        
    }

    
function subMediumSizeInventory(uint _amount) public {
        require(msg.sender == owner, "this function is restricted to the owner");
        sizes storage mediumSizes_ = mediumSizes;
        
          if (mediumSizes_.amount < _amount){
            revert("Amount is Greator Than Supply");
            
            
        } else {
        mediumSizes_.amount -= _amount;
        emit subtractingMediumSizeInventory(_amount);
        }
        
    }

function subLargeSizeInventory(uint _amount) public {
        require(msg.sender == owner, "this function is restricted to the owner");
        sizes storage largeSizes_ = largeSizes;
          if (largeSizes_.amount < _amount){
            revert("Amount is Greater Than Supply");
            
            
        } else {
        largeSizes_.amount -= _amount;
        emit subtractingLargeSizeInventory(_amount);
        }
    }
    
function subxLargeSizeInventory(uint _amount) public {
        require(msg.sender == owner, "this function is restricted to the owner");
        sizes storage xlargeSizes_ = xlargeSizes;
          if (xlargeSizes_.amount < _amount){
            revert("Amount is Greater Than Supply");
            
        } else {
        xlargeSizes_.amount -= _amount;
        emit subtractingxLargeSizeInventory(_amount);
        }
    }

function subxxLargeSizeInventory(uint _amount) public {
        require(msg.sender == owner, "this function is restricted to the owner");
        sizes storage xxlargeSizes_ = xxlargeSizes;
          if (xxlargeSizes_.amount < _amount){
            revert("Amount is Greater Than Supply");
...
contract ERC20 is Context, IERC20, Inventory {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual { }
       ...
       function basket(address _address, uint _small_sizes, uint _medium_sizes, uint _large_sizes, uint _xlarge_sizes, uint _xxlarge_sizes) public {
        
        OrderInfo storage buyer = order_info[_address];
        
        
         subSmallSizeInventory(_small_sizes);
        ...
        subMediumSizeInventory(_medium_sizes);
        ...
        subxxLargeSizeInventory(_xxlarge_sizes);
