...
// Note that this pool has no minter key of CHIPS (rewards).
// Instead, the governance will call CHIPS distributeReward method and send reward to this pool at the beginning.
contract CHIPSRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accCHIPSPerShare; // Accumulated CHIPS per share, times 1e18. See below.
    ...
    // The block number when CHIPS mining starts.
    uint256 public startBlock;

    // The block number when CHIPS mining ends.
    ...
    uint256 public rewardDuration = 10; // in days
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "CHIPSRewardPool: caller is not the operator");
        _;
    }
    
    function startMint()public onlyOperator {       // it can be called by operator only once.
        
        require(isMintStarted == false, "Chipshop: Minting started");
    ...
    function checkPoolDuplicate(IERC20 _lpToken) internal view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].lpToken != _lpToken, "CHIPSRewardPool: existing pool?");
        }
    }

    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate, uint256 _lastRewardBlock) public onlyOperator {
    ...
    // View function to see pending CHIPS on frontend.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            uint256 FeeToDAO = 0;
            if(_pid < 4){                   // in case BNB, BUSD, BTD, MDO pool,users have to pay 1% fee when deposit
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            uint256 FeeToDAO = 0;
            if(_pid == 4){                          // in case CHIP/BNB pool, users have to pay 3% fee when withdraw
