    ...
    ReentrancyGuardUpgradeable
{
    ...
    event UpgradeNftCharacter(
        address indexed user,
        uint256 indexed nftId,
        uint256 oldCharacterId,
    ...
    uint256 public constant EMOTION_COUNT = 5;
    ...
    /**
     * @dev updates nft character of a given nft
     */
    function upgradeNftCharacter(uint256 _nftId, uint256 _characterId)
        external
        payable
    {
        require(
        ...
        require(
            nftSettings.characterCount() >= _characterId,
            "QNFT: invalid character id"
        );
        ...
        uint256 mintPrice = nftSettings.characterPrices(_characterId);
        require(
            msg.value >= mintPrice,
            "QNFT: insufficient character upgrade price"
        );

        // transfer remaining to user
        payable(msg.sender).transfer(msg.value - mintPrice);

        uint256 oldCharacterId = data.characterId;
    ...
    function _mintNft(
        ...
        require(
            _defaultEmotionIndex < EMOTION_COUNT,
            "QNFT: invalid emotion index"
        );
        ...
        require(
            getTokenId(_characterId, _bgImageId, _favCoinId, lockDuration) ==
                uint256(0),
            "QNFT: nft already exists"
        );
