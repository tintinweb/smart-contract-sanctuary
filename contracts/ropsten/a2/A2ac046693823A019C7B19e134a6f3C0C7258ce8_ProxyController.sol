...
import "./interfaces/compound/SimplePriceOracle.sol";
...
contract ProxyController is AccessControl, IProxyController {
    ...
    function startOrder(uint256 amountIn, OrderInput memory input) public virtual onlyRole(EXECUTOR_ROLE) {
        ...
        // oracle price
        uint256 priceA = SimplePriceOracle(_oracle).getUnderlyingPrice(CTokenInterface(input.cTokenA));
        // uint256 priceB = SimplePriceOracle(_oracle).getUnderlyingPrice(CTokenInterface(input.cTokenB));
...
contract SimplePriceOracle is IPriceOracle {
    ...
    function getUnderlyingPrice(CTokenInterface cToken) public view override returns (uint) {
        if (compareStrings(CTokenInterface(address(cToken)).symbol(), "cETH")) {
            return 1e18;
        } else {
