...
contract RedEnvelopeMerkleERC20 is IEnvelope, Ownable {
    ...
    function returnEnvelope(string calldata envelopeID) public {
        ...
        // FIXME: check if the token is created by someone else, can I still call approve here
        SafeERC20.safeApprove(token, address(this), env.balance);
        SafeERC20.safeTransferFrom(token, address(this), env.creator, env.balance);
    }
    ...
    function openEnvelope(
        ...
        if (currentEnv.numParticipants == 1) {
            // FIXME: check if I can still call approve if I am not the owner
            SafeERC20.safeApprove(currentEnv.token, address(this), currentEnv.balance);
            SafeERC20.safeTransferFrom(currentEnv.token, address(this), receiver, currentEnv.balance);
