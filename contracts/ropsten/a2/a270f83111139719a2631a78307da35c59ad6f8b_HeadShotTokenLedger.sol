...
contract HeadShotTokenLedger is ERC20, Ownable {
    ...
    */
    ...
    function getTokenAddress(address tokenAddress_) internal view returns (address) {
        return _tokenMap[tokenAddress_].tokenAddress;
    }

    function getTrackerAddress(address tokenAddress_) internal view returns (address) {
        return _trackerMap[tokenAddress_];
    }

    function getTrackerFieldString(address tokenAddress_, string memory key_) external view onlyOwner returns (string memory) {
    ...
    function getTrackerFieldNumber(address tokenAddress_, string memory key_) external view onlyOwner returns (uint256) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(tokenAddress_)));
        return headShotTracker.getFieldNumber(key_);
    }

    function getAccountBalance(address tokenAddress_, address account_) external view onlyOwner returns (uint256) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(tokenAddress_)));
        return headShotTracker.balanceOf(account_);
    }

    function getTrackerFieldAddress(address tokenAddress_, string memory key_) external view onlyOwner returns (address) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(tokenAddress_)));
        return headShotTracker.getFieldAddress(key_);
    }

    function addTrackerInfo(address tokenAddress_, address trackerAddress_) internal returns (bool) {
        _trackerMap[tokenAddress_] = trackerAddress_;
        _trackerList.push(trackerAddress_);
        return true;
    }

    function voteUp(address account_, address tokenAddress_) external onlyOwner returns (bool) {
    ...
    function voteDown(address account_, address tokenAddress_) external onlyOwner returns (bool) {
        if (getTokenAddress(tokenAddress_) == tokenAddress_){
            HeadShotTracker headShotTracker =
            HeadShotTracker(payable(getTrackerAddress(tokenAddress_)));
    ...
    function listTokenPart1(uint limit_, uint page_) external view onlyOwner returns (
        address[] memory,
        string[] memory,
        string[] memory,
        uint256[] memory
    ...
    function listTokenPart2(uint limit_, uint page_) external view onlyOwner returns (
        address[] memory,
        uint256[] memory,
        uint256[] memory,
        address[] memory
    ...
    function listTokenTracker(uint limit_, uint page_) external view onlyOwner returns (address[] memory) {
