...
contract Farming is Ownable {
    ...
    // The block number when EXO mining starts.
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        for (uint256 periodId = 0; periodId < 4; ++periodId) {
            FarmPeriod memory fp = FarmPeriod({
                ...
                accExosPerShare: 0,
                depositBlock: 0,
                periodInBlocks: periodInDays[periodId]*dayInBlocks,
                withdrawBlock: 0,
    ...
    function updatePool(uint256 _pid, uint256 _fpid) public {
        
        FarmPeriod memory farmPeriod = getFarmPeriod(_pid,_fpid);
        if (block.number <= farmPeriod.lastRewardBlock) {
            return;
        }
        uint256 lpSupply = farmPeriod.lpToken.balanceOf(address(this));
        if (lpSupply == 0) {
    ...
    function deposit(uint256 _pid, uint256 _fpid, uint256 _amount) public {
        ...
        farmPeriod.depositBlock = block.number;
        farmPeriod.withdrawBlock = farmPeriod.depositBlock + farmPeriod.periodInBlocks;

        emit Deposit(msg.sender, _pid, _fpid, _amount, farmPeriod.depositBlock, farmPeriod.withdrawBlock );
    }
    ...
    function withdraw(uint256 _pid, uint256 _fpid, uint256 _amount) public {
        ...
        FarmPeriod memory farmPeriod = getFarmPeriod(_pid,_fpid);
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(block.number > farmPeriod.withdrawBlock, "withdraw: still in lock period");
    ...
    function emergencyWithdraw(uint256 _pid, uint256 _fpid) public onlyOwner {

        FarmPeriod memory farmPeriod = getFarmPeriod(_pid,_fpid);
        UserInfo storage user = userInfo[_pid][msg.sender];
        farmPeriod.lpToken.safeTransfer(address(msg.sender), user.userPeriod[_fpid].amount);
        emit EmergencyWithdraw(msg.sender, _pid, _fpid, user.userPeriod[_fpid].amount);
