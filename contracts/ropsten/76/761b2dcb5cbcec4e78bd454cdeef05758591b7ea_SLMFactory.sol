...
// File: contracts/ISLMFactory.sol

/*
SITHToken factory interface

This defines the SITHToken factory interface
...
interface ISLMFactory {
    // events
    event SITHTokenCreated(address indexed user, address sithtoken);

    /**
     * @notice create a new SITHToken
...
// File: contracts/SLMFactory.sol

/*
SITHToken Factory

This implements the SITHToken factory contract which allows any user to
easily configure and deploy their own SITHToken

*/

pragma solidity ^0.6.12;

contract SLMFactory is ISLMFactory {
    // fields
    ...
    /**
     * @param sith_ address of SITH token
     */
    constructor(address sith_) public {
        _sith = sith_;
    }

    /**
     * @inheritdoc ISLMFactory
     */
    function create(
        address stakingToken,
        address rewardToken,
        ...
        // create
        SITHToken sithtoken = new SITHToken(
            stakingToken,
            rewardToken,
            bonusMin,
            bonusMax,
        ...
        sithtoken.transferOwnership(msg.sender);

        // bookkeeping
        map[address(sithtoken)] = true;
        list.push(address(sithtoken));

        // output
        emit SITHTokenCreated(msg.sender, address(sithtoken));
        return address(sithtoken);
    }

    /**
     * @return total number of SITHTokens created by the factory
     */
