...
contract Gleaf is ERC20Burnable, Ownable {
    ...
    function claimByTokenId(uint256 tokenId) public {
        ...
        tokenIdToTimeStamp[tokenId] = CLAIM_START_TIME;
        }
    ...
    function claimAll() public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(tokenIdToTimeStamp[tokenIds[i]] == 0){
                ...
                tokenIdToTimeStamp[tokenIds[i]] = CLAIM_START_TIME;
            }
            totalRewards =
                totalRewards +
    ...
    function getAllRewards(address _address) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if(tokenIdToTimeStamp[tokenIds[i]] == 0){
                ...
                totalRewards = totalRewards + ((block.timestamp - CLAIM_START_TIME) * EMISSIONS_RATE);
            }else{
