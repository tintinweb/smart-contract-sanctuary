...
contract Vorlex  is ERC1155, ERC1155Holder,  AccessControl  {
    ...
    */
    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, msg.sender), "ADM");
        _;
    }
    ...
    struct Sale {
        uint256 buy_or_bid; // One-time Purchase or Bidding :: 1 for Buying :: 2 for Bidding
        uint256 is_escrow; // 1 is Escrow :: 2 is not Escrow
        uint256 vorlex_id;
        uint256 amount;
        ...
        uint256 status; // 1 => Open for Buy/Bid :: 2 => Closed for Buy/Bid/Escrow_Settlement :: 3 => Open for Escrow_Settlement
    }


    event InFunded(bytes32 indexed metadata);
    event OutFunded(uint256 indexed amount, address indexed receiver, bytes32 indexed metadata);
    event Minted(uint256 indexed vorlex_id, string ipfs, address indexed custodian, uint256 amount, string vorlex_data);
    event CustodianChanged(uint256 indexed vorlex_id, address indexed new_custodian);
    event SaleSetup(bytes32 indexed sale_id);
    event Purchased(bytes32 indexed sale_id, uint256 indexed amount, address indexed buyer);
    event Bidded(bytes32 indexed sale_id );
    event SaleCancelled(bytes32 indexed sale_id);
    event SaleClosed(bytes32 indexed sale_id, bool indexed is_refund);
    ...
    */
    ...
    function outFundContract(uint256 amount, address receiver, bytes32 metadata) public payable onlyAdmin {
    ...
    */
    ...
    function setMinSale(uint256 new_min_sale) public virtual onlyAdmin {
    ...
    */
	function mint( string memory ipfs, address custodian, uint256 amount, string memory vorlex_data) public virtual {
     ...
     */
    function setCustodian( uint256 vorlex_id, address new_custodian) public virtual onlyAdmin {
        require(vorlexes[vorlex_id].custodian != address(0), "NILTOK");
    ...
    /**
        ...
        * @param bid_or_buy True for Sale Setup, False for Auction Setup
        * @param vorlex_id Vorlex id
     ...
     */
    function createSale(bytes32 sale_id, bool bid_or_buy, uint256 vorlex_id, uint256 amount, uint256 worth, uint256 bid_span, bool is_escrow) public virtual {
        require(sales[sale_id].vorlex_id == 0, "DUPL" );
        require(bid_span >= 1 hours && bid_span <= 3 days, "BSPAN"); // * Check if the Bid Span is Valid
        require((bid_or_buy ? worth : worth/amount) >= MIN_SALE, "LOWSALE");
        is_escrow = vorlexes[vorlex_id].amount > 1 ? false : is_escrow;
        sales[sale_id] = Sale(bid_or_buy ? 1 : 2, is_escrow ? 1 : 2, vorlex_id, amount, worth, bid_span, 0, msg.sender, address(0), 1);
        safeTransferFrom(msg.sender, address(this), vorlex_id, amount, "");
        emit SaleSetup(sale_id);
    } 
     ...
     */
    function purchaseToken( bytes32 sale_id, uint256 amount) public payable {
        require(sales[sale_id].status == 1, "XED");
        require (sales[sale_id].buy_or_bid == 1, "FORBID");
        require(sales[sale_id].amount >= amount, "LOWTOK");
        require(msg.value/amount == sales[sale_id].worth, "LOWBAL");
        sales[sale_id].amount -= amount;
        if(sales[sale_id].is_escrow == 2){ // Transfer Payment and Token if not Escrow
            this.safeTransferFrom( address(this), msg.sender, sales[sale_id].vorlex_id, amount, "");
            uint256 _payment = msg.value;
            payable(vorlexes[sales[sale_id].vorlex_id].custodian).transfer(_payment /10); // custodianRoyalty
            payable(sales[sale_id].seller).transfer(_payment * 4/5);
        } else {
            sales[sale_id].highest_bidder = msg.sender;
            sales[sale_id].status = 3;
        }
        emit Purchased(sale_id, amount, msg.sender);
    }
    ...
    function placeBid(bytes32 sale_id) public payable {
        require(sales[sale_id].status == 1, "XED");
        require (sales[sale_id].buy_or_bid == 2, "FORSALE");
        require ((msg.value - sales[sale_id].worth) >= (sales[sale_id].worth /100), "LOWBAL" );
    ...
    function cancelSale(bytes32 sale_id) public payable {
        require(msg.sender == sales[sale_id].seller, "NOWN");
        require(sales[sale_id].highest_bidder == address(0), "ACTV");
        this.safeTransferFrom( address(this),  msg.sender,  sales[sale_id].vorlex_id, sales[sale_id].amount, "");
        sales[sale_id].status = 2;
    ...
    */
    function closeSale(bytes32 sale_id, bool is_refund) public payable onlyAdmin {
        require(sales[sale_id].status == 3, "NOSCROW");
        if(!is_refund){
            if(sales[sale_id].buy_or_bid == 2){
                require(sales[sale_id].expiry < block.timestamp , "NOEXP" );
            }
            this.safeTransferFrom( address(this),  sales[sale_id].highest_bidder,  sales[sale_id].vorlex_id, sales[sale_id].amount, "");
            ...
            payable(sales[sale_id].highest_bidder).transfer(sales[sale_id].worth);// Fully Refund the buyer
        }
        sales[sale_id].status = 2;
        emit SaleClosed(sale_id, is_refund);
    }


    /**
        *@dev Updates Vorlex Amount when Burnt
     */
    function _burnBatch( uint256[] memory ids, uint256[] memory amounts) internal virtual {

        for (uint256 i = 0; i < ids.length; i++) {
            vorlexes[ids[i]].amount -= amounts[i];
        }
    ...
    */
    ...
    function batchMove(uint256[] memory vorlex_ids, uint256[] memory amounts, string memory metadata, address from, address to, bool is_swap) public virtual payable {
        safeBatchTransferFrom(from, to, vorlex_ids, amounts, "");
        if(to == address(0)){ // If operation is a burn operation
            is_swap = false;
            _burnBatch(vorlex_ids, amounts);
            emit Burnt(metadata);
        }
        if(is_swap){
        ...
        internal virtual override(ERC1155)
    {
        ...
        // ADMIN/CONTRACT can initiate any Transfers :: Vorlexers can only transfer to ADMIN/CONTRACT
        require (hasRole(ADMIN_ROLE, from) || hasRole(ADMIN_ROLE, to) || hasRole(ADMIN_ROLE, operator), "INVALID TRX");
