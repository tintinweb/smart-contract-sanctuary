...
contract CashBox {
    ...
    function deposit(address _token, uint _deposit_amount) public returns (bool) {
        // TODO Do we need white list/black list of account to deposit?
        // TODO Do we need to set deposit limit?
        ...
        uint totalAssets = IERC20(Token).balanceOf(address(this));
        uint shares;
        if (totalSupply_ > 0) {
        ...
        user.deposit_amount = SafeMath.add(user.deposit_amount, _deposit_amount);
    ...
    function getWithdrawAmount(uint _ptoken_amount) public view returns (uint) {
        uint freeFunds = IERC20(Token).balanceOf(address(this));
        uint value = SafeMath.div(SafeMath.mul(_ptoken_amount, freeFunds), totalSupply_);
    ...
    function withdraw(uint _withdraw_amount) public returns (bool) {
        ...
        uint value = getWithdrawAmount(_withdraw_amount);
        
        // Update user info
        UserInfo storage user = userinfo[msg.sender];
        user.deposit_amount = SafeMath.sub(user.deposit_amount, _withdraw_amount);
