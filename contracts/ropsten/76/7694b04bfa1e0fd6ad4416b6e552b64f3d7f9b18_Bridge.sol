...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    /**
        ...
        @param dataHash Hash of data to be provided when deposit proposal is executed.
     */
    function getProposal(uint8 originDomainID, uint64 depositNonce, bytes32 dataHash) external view returns (ProposalStatus proposalStatus, uint200 yesVotes, uint8 yesVotesTotal, uint40 proposedBlock) {
        uint72 nonceAndID = (uint72(depositNonce) << 8) | uint72(originDomainID);
        Proposal memory proposal = _proposals[nonceAndID][dataHash];
        proposalStatus = proposal._status;
        yesVotes = proposal._yesVotes;
        yesVotesTotal = proposal._yesVotesTotal;
    ...
    function adminWithdraw(
        ...
        IERCHandler handler = IERCHandler(handlerAddress);
        handler.withdraw(token, recipient, amount);
    }
