...
//
// Symbol      : DBC
// Name        : Debit Coin Token
// Total supply: Starting from zero. and minted as specified stable coins comes in, making 1:1 ratio
// Decimals    : 4
// 
// (c) by A. Valamontes with Blockchain Ventures / Geopay.me Inc 2013-2019. 
// "SPDX-License-Identifier: Affero GPLv3 Licence."
// ----------------------------------------------------------------------------------------------
...
contract DebitCoinToken is owned {
    ...
    string constant private _name = "Debit Coin Token";
    string constant private _symbol = "DBC";
    ...
    mapping(address => bool) public custodianTokens;
     ...
     */
    function issueToken(address paymentTokenAddress, uint256 dbcTokenAmount)  external returns(string memory) {
        
        //assign local variables
        address msgSender = msg.sender;
        
        //checking conditions
        require(!safeguard, 'safeguard was placed');
        require(dbcTokenAmount <= maxTransactionAmount, 'max Transaction Amount');
		require(custodianTokens[paymentTokenAddress], 'Invalid payment Token Address');
        
        //first deduct the payment tokens. User has to approve this smart contract as spender to be able to do this.
        uint256 paymentTokenDecimals = IERC20(paymentTokenAddress).decimals();
        uint256 paymentTokenAmount = dbcTokenAmount * (10**paymentTokenDecimals) / (10**_decimals);
        IERC20(paymentTokenAddress).transferFrom(msgSender, address(this), paymentTokenAmount);
        
        //issue DBC tokens
        _balanceOf[msgSender] = _balanceOf[msgSender].add(dbcTokenAmount);
        _totalSupply = _totalSupply.add(dbcTokenAmount);
        emit Transfer(address(0), msgSender, dbcTokenAmount);
        
        return "DBC tokens issued successfully";
    }
     ...
     */
    function destroyToken(address receivingTokenAddress, uint256 dbcTokenAmount) external returns (string memory) {
        ...
        require(!safeguard, 'safeguard was placed');
        require(dbcTokenAmount <= maxTransactionAmount, 'max Transaction Amount');
		require(custodianTokens[receivingTokenAddress], 'Invalid payment Token Address');
        
        //now destroy DBC tokens
        _balanceOf[msgSender] = _balanceOf[msgSender].sub(dbcTokenAmount);  // Subtract from the sender
        _totalSupply = _totalSupply.sub(dbcTokenAmount);                      // Updates totalSupply
        emit Burn(msgSender, dbcTokenAmount);
        emit Transfer(msgSender, address(0), dbcTokenAmount);
        
        //send receiving tokens
        uint256 receivingTokenDecimals = IERC20(receivingTokenAddress).decimals();
        uint256 receivingTokenAmount = dbcTokenAmount * (10**receivingTokenDecimals) / (10**_decimals);
        IERC20(receivingTokenAddress).transfer(msgSender, receivingTokenAmount);
        
        return "DBC tokens destroyed successfully";
    }
     ...
     */
    function addCustodialToken(address tokenContract) external onlyOwner returns(string memory){
        custodianTokens[tokenContract] = true;
        return "Token contract added successfully";
    }
    ...
    function changeMaxTransactionAmount(uint256 _maxTransactionAmount) external onlyOwner returns(string memory){
        require(_maxTransactionAmount != 0, "Amount can not be zero");
        maxTransactionAmount = _maxTransactionAmount;
        return "maxTransactionAmount updated successfully";
    }
