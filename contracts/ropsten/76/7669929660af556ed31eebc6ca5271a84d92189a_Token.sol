...
contract Token is IERC20, Ownable {
    ...
    function _transferFee(address from, uint256 tax) private returns (bool _success) {
        ...
        if(taxPercentages[uint256(taxAddressType.taxAddressDevelopment)] > 0)  taxAmountDevelopment = (tax * taxPercentages[uint256(taxAddressType.taxAddressDevelopment)]) / (100);
        if(taxPercentages[uint256(taxAddressType.taxAddressNewMovies)] > 0)  taxAmountNewMovies = (tax * taxPercentages[uint256(taxAddressType.taxAddressNewMovies)]) / (100);
        if(taxPercentages[uint256(taxAddressType.taxAddressProfit)] > 0)  taxAmountProfit = (tax * taxPercentages[uint256(taxAddressType.taxAddressProfit)]) / (100);
        if(taxPercentages[uint256(taxAddressType.taxAddressUserCredit)] > 0)  taxAmountUserCredit = (tax * taxPercentages[uint256(taxAddressType.taxAddressUserCredit)]) / (100);
        ...
        // Transfer Amount
        if(taxAmountDevelopment > 0) _balances[taxAddresses[uint256(taxAddressType.taxAddressDevelopment)]] += taxAmountDevelopment;
        if(taxAmountNewMovies > 0) _balances[taxAddresses[uint256(taxAddressType.taxAddressNewMovies)]] += taxAmountNewMovies;
        if(taxAmountProfit > 0) _balances[taxAddresses[uint256(taxAddressType.taxAddressProfit)]] += taxAmountProfit;
        if(taxAmountUserCredit > 0) _balances[taxAddresses[uint256(taxAddressType.taxAddressUserCredit)]] += taxAmountUserCredit;
