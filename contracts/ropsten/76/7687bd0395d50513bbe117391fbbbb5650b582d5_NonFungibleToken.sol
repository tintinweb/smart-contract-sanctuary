...
// NonFungibleToken SMART CONTRACT
contract NonFungibleToken is ERC721 {
    ...
    /**
     * Get Holder Token URIs
     *
     * @param holder - Holder of the Tokens
     */
    function getHolderTokenURIs(address holder)
        public
        view
        returns (string[] memory)
    {
        ...
        uint256 id_index;
        for (id_index = 0; id_index <= count - 1; id_index++) {
            tokenIds[id_index] = tokenOfOwnerByIndex(holder, id_index);
        }
        string[] memory result = new string[](count);
        uint256 uri_index;
        for (uri_index = 0; uri_index <= count - 1; uri_index++) {
            result[uri_index] = tokenURI(tokenIds[uri_index]);
        }
        return result;
    }

    /**
     * Get Holder Token IDs & URIs
     *
     * @param holder - Holder of the Tokens
     */
        ...
        returns (uint256[] memory, string[] memory)
    {
        ...
        uint256 uri_index;
        for (uri_index = 0; uri_index <= count - 1; uri_index++) {
            tokenURIs[uri_index] = tokenURI(tokenIds[uri_index]);
        }
        return (tokenIds, tokenURIs);
    }

    /**
     * Get Holder Token IDs & URIs - By Range
     *
     * @param holder - Holder of the Tokens
     * @param start - Range Start
     * @param end - Range End
     */
    function getHolderTokensByRange(
        address holder,
        uint256 start,
        uint256 end
        ...
        for (id_index = 0; id_index < count; id_index++) {
            tokenIds[id_index] = tokenOfOwnerByIndex(holder, temp);
            temp++;
        }
        string[] memory tokenURIs = new string[](count);
        uint256 uri_index;
        for (uri_index = 0; uri_index < count; uri_index++) {
            tokenURIs[uri_index] = tokenURI(tokenIds[uri_index]);
        }
        return (tokenIds, tokenURIs);
    }
