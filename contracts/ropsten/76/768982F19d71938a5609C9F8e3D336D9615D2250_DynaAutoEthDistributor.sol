...
//        dynaHodler[newUserAddress][currentRound] = dynaHodler[newUserAddress][currentRound+1];
    ...
    function logTransactionEvent(address from, uint256 senderAmount, address to, uint256 recipientAmount) public override returns (bool) {
    ...
    function logUserTransaction(address user, uint256 value, bool isSender) private {
        queue.add(user);
        if(user.isContract() && !contractWhitelist[user]){
        ...
        uint256 prevUserBal = dynaHodler[user][currentRound+1].tokenBalance;
    ...
    function reflectRewards() public override returns (bool) {
        require(gasleft() > minGas, "More gas is required for this function");
        if(!inSwapAndLiquify)
    ...
    function _sendEthTo(address payable hodler) private {
        ...
        if(allowLowLevelCalls){
            (bool success) = hodler.send(reward);
        } else {
            hodler.transfer(reward);
        }
//        emit IERC20.Transfer(address(this), hodler, reward);
    }
