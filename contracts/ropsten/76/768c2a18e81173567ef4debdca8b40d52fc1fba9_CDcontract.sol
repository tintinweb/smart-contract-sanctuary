...
contract CDcontract {
    ...
    function registration(address userAddress, address referrerAddress, uint area) private {
        ...
        if (area == 1) {
            if (tract[referrerAddress].left == address(0)) {
                ...
                leftIdToAddress[leftLastUserId] = userAddress;
                leftLastUserId++;
            }
        } else {
            if (tract[referrerAddress].right == address(0)) {
                ...
                rightIdToAddress[rightLastUserId] = userAddress;
                rightLastUserId++;
            }
        }

        if (tract[place].left != address(0) && tract[place].right != address(0)) {
    ...
    function pointCommission(address userAddress, uint8 matrix, uint8 level, uint price) private {
        ...
        for (i = 1; i <= 24; i++) {
            address referrerAddress = tract[selectAddress].place;
            if (referrerAddress != address(0)) {
    ...
    function bumpCommission(address userAddress, address placeAddress, uint price) private {
        if (placeAddress != address(0)) {
            uint placeLeftAmount = users[tract[placeAddress].left].amount;
            uint placeRightAmount = users[tract[placeAddress].right].amount;
            uint minAreaAmount = placeLeftAmount < placeRightAmount ? placeLeftAmount : placeRightAmount;
