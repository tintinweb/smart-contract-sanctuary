...
contract Factory {
    GraJek[] public contracts; //to store the created contract address into this array
    uint disabledCount;
    event newContractCreated(address ContractAddress); //just to see whats the latest contract created i guess
    

    function createContract(uint _durationMin, uint distanceKm, uint _startPostal, uint _endPostal) external {
        GraJek newContract = new GraJek(_durationMin, distanceKm, _startPostal, _endPostal);
    ...
    // function getRiders() external view returns(GraJek[] memory _riders){
    //   _riders = new GraJek[](contracts.length - disabledCount);
    //   uint count;
    //   for(uint i=0;i<contracts.length; i++){
    //       if(contracts[i].isEnabled()){
    //          _riders[count] = contracts[i];
    //          count++;
    //       }
    //     }
    //  }
