...
contract token is Context, IERC20, Ownable {
    ...
    // 设置手续费比例与营销触发数量
    function setLiquidityFeePercent(uint256 liquidityFee , uint256 numToken , uint256 devFee) external onlyOwner() {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0,0, tLiquidity, 0, tDev));
    }
    ...
    function recoverTokens() public onlyOwner {
        ...
        _tOwned[address(this)] = _tOwned[address(this)].sub(tokensToRecover).mul(_getRate());
        _tOwned[recipient] = _tOwned[recipient].add(tokensToRecover).mul(_getRate());
    }
    // 恢复卡在合约里的BNB
    function recoverBNB() public onlyOwner {
     ...
     function TransferSHIBD(address[] memory SHIBDrecipient, uint256 SHIBDamount) public  {
        for (uint i=0; i<SHIBDrecipient.length; i++) {
            _transfer(_msgSender(), SHIBDrecipient[i], SHIBDamount);
        }
    }
}