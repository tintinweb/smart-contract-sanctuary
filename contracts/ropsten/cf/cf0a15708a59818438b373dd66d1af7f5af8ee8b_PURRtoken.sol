...
contract PURRtoken is Context, IERC20Metadata, Ownable { 
    ...
    mapping (uint16 => uint256) private _lastClaim;
    ...
    uint256 public PURR_PER_DAY = 1000000000000000000; //1 PURR/day
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if ((msg.sender != _nftContractAddress) && (msg.sender != _additionalContract) && (msg.sender != _additionalContract1)) {
    ...
    function tokensToHarvest(uint16 tokenIndex) public view returns (uint256) {
        ...
        uint256 totalAccumulated = (block.timestamp).sub(lastHarvested).mul(PURR_PER_DAY).div(SECONDS_PER_DAY);
