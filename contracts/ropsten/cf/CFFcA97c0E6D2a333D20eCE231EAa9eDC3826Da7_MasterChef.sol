...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. LOTLs to distribute per block.
    ...
    struct Rewards {
        uint256 totalTimeAlloc;     // Total time factor for all stakes.
    ...
    // The block number when LOTL mining starts, renewed after each halving process.
    ...
    // TODO Whenever function called add the LP tokens to the RewardPool Contract.
    /*
    function add(uint256 _allocPoint, IBEP20 _lpToken, IBEP20 _tokenA, IBEP20 _tokenB, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        require(_depositFeeBP <= 10000, "add: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
        address[] storage emptyUser;
        rewardAddress.addLpToken(_lpToken, _tokenA, _tokenB);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, IERC20 _tokenA, IERC20 _tokenB, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        require(_depositFeeBP <= 10000, "add: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's LOTL allocation point and deposit fee. Can only be called by the owner.
    ...
    // Return block distance over the given _from to _to block.
    function getBlockDistance(uint256 _from, uint256 _to) public view returns (uint256) {
        return _to - _from;
    }

    // View function to see pending LOTLs on frontend.
    function pendingLotl(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid + 1][_user];
        uint256 accLotlPerShare = pool.accLotlPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getBlockDistance(pool.lastRewardBlock, block.number);
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0 && _amount > 0){
            ...
            if(user.amount > _amount){
                user.stakedSince = user.stakedSince + _amount * 1e12 / user.amount * getBlockDistance(user.stakedSince, block.number) / 1e12;
            }

            if(user.amount == _amount){
                user.stakedSince = block.number - getBlockDistance(user.stakedSince, block.number) / 2;
            }

            if(user.amount < _amount){
                user.stakedSince = block.number - user.amount * 1e12 / _amount * getBlockDistance(user.stakedSince, block.number) / 1e12;
            }
        }
    ...
    function safeBusdTransfer(address _to, uint256 _amount) internal {
        ...
        bool transferSuccess = false;
        if (_amount > busdBal) {
            transferSuccess = busd.transfer(_to, busdBal);
        } else {
            transferSuccess = busd.transfer(_to, _amount);
        }
    ...
    // Automated in contract minting rate adjustment was discarded due to unneccessary Gas cost increase for every transaction
    ...
    */
    ...
    function calculateRewardPool() public onlyAdmins {
        uint256 length = poolInfo.length;
        uint256 rewardUserlength = rewardInfo.poolUser.length;
        rewardInfo.amountBUSD = busd.balanceOf(address(this));
        ...
        for(uint256 i; i< rewardUserlength; i++){
        ...
        for (uint128 i=0; i < length; i++){
            ...
            uint256 lpSupply = pool.lpToken.balanceOf(address(this));
            uint256 userLength = pool.poolUser.length;
            ...
            for(uint256 j; j< userLength; j++){
                ...
                if(user.stakedSince > 0){
                    ...
                    uint256 timeReward = calculateTimeRewards(user.stakedSince);
    ...
    function currentHoldingFactor(uint16 _pid) public view returns(uint256 holdFactor){
