...
contract NFTMarket is ERC721Holder, ERC1155Holder, AccessControl {
    ...
    event AuctionEnded(uint256, address, AuctionStatus, uint256, uint256, uint256);
    ...
    function launchAuction(
        ...
        if (_tokenType == TokenType.ERC721) {
            ...
            require(token.isApprovedForAll(msg.sender, address(this)), "Approved not found");
            ...
            token.safeTransferFrom(msg.sender, address(this), _tokenID, 1, DEFAULT_MESSAGE);
        }
        
        if (_contractAddr != defaultCon) {
            require(_creatorProfit == 0, "Contract address didn't support creator profit.");
        }
    ...
    function getAuctionId(address _contractAddr, uint256 _tokenID) public view returns (uint256) {
        return auctions[_contractAddr][_tokenID];
    }
    
    function getAuctionContractInfo(uint256 _auctionID) public view returns (address contractAddr, uint256 tokenID) {
    ...
    function getAuctionInfo(uint256 _auctionID) public view returns (address seller, TokenType tokenType, uint256 startPrice, uint16 creatorProfit, uint16 storeProfit, uint256 auctionStartTime, uint256 auctionEndTime, address highestBidder, uint256 highestBid, uint256 lastBidTime, uint256 totalBids, AuctionStatus status)
    {
        AuctionInfo memory auction = auctionInfo[_auctionID];
    ...
    function buyOrBid(uint256 _auctionID) payable public returns (bool) {
        ...
        if (isDirectBuy(auction.auctionStartTime, auction.auctionEndTime)) {
            // Direct buy
            require(msg.value == auction.startPrice, "Please make check your price.");
            directBuy = true;
        } else {
            // Bid
            require(getNow() >= auction.auctionStartTime, "Auction not start yet");
            require(!isOverExtraEndTime(auction.auctionEndTime, auction.lastBidTime), "Auction already finished.");

            require(msg.value > auction.startPrice, "Your bid didn't higher than start price.");
            require(msg.value > auction.highestBid, "Your bid didn't higher than highest price");
        }

        // Refund to previous winner
        if (auction.totalBids != 0) {
            // Usually use transfer
            require(payable(auction.highestBidder).send(auction.highestBid), "Refound failed");
        }
        ...
        emit NewBidReceived(_auctionID, auction.highestBidder, auction.highestBid);
        return true;
    }
    ...
    function finalizeAuction(uint256 _auctionID) public {
        ...
        // Check auction time
        if (!isDirectBuy(auction.auctionStartTime, auction.auctionEndTime)) {
            // Bid time needs over 15 minutes
            require(isOverExtraEndTime(auction.auctionEndTime, auction.lastBidTime), "Auction not end.");
        }

        if (auction.totalBids == 0) {
            auction.status = AuctionStatus.Unsold;
            emit AuctionEnded(_auctionID, auction.highestBidder, auction.status);
        } else {
            // Caculate profit
            uint256 creatorProfit = auction.highestBid * (auction.creatorProfit / BASE_PERCENT);
            uint256 storeProfit = auction.highestBid * (auction.storeProfit / BASE_PERCENT);
            ...
            if (creatorProfit != 0) {
                payable(callContractRetAddr(auction.contractAddr, "creatorOf(uint256)", auction.tokenID)).transfer(creatorProfit);
            }
            ...
            if (auction.tokenType == TokenType.ERC721) {
                IERC721 token = IERC721(auction.contractAddr);
                token.safeTransferFrom(address(this), auction.highestBidder, auction.tokenID);
            } else {
                IERC1155 token = IERC1155(auction.contractAddr);
                token.safeTransferFrom(address(this), auction.highestBidder, auction.tokenID, 1, DEFAULT_MESSAGE);
            }
            
            // Finalize Auction
            auction.status = AuctionStatus.Sold;
            emit AuctionEnded(_auctionID, auction.highestBidder, auction.status, amount, creatorProfit, storeProfit);
        }
    }
    ...
    function isDirectBuy(uint256 _auctionStratTime, uint256 _auctionEndTime) private pure returns (bool) {
        return _auctionStratTime == _auctionEndTime && _auctionStratTime == 0;
    }

    function isOverExtraEndTime(uint256 _auctionEndTime, uint256 _lastBidTime) private view returns (bool) {
        uint256 nowTime = getNow();
        if (_lastBidTime != 0) {
            uint256 extraEndTime = _lastBidTime + 15 minutes;
    ...
    function setStoreData(address _storeAddress, uint16 _storeProfit, address _defaultCon) public onlyRole(CONFIGURATOR_ROLE) {
        checkProfit(_storeProfit);
        
        storeAddress = _storeAddress;
        storeDefaultProfit = _storeProfit;
        defaultCon = _defaultCon;
    }
    ...
    function getDefaultCon() public view returns (address) {
        return defaultCon;
    }
    ...
    function checkStatus(AuctionStatus _status, AuctionStatus _expect) private pure {
    ...
    function transferToContract(address _contractAddr, uint256 _tokenID) external {
        IERC721 token = IERC721(_contractAddr);
        require(token.isApprovedForAll(msg.sender, address(this)), "Approved not found");
        token.safeTransferFrom(msg.sender, address(this), _tokenID);
    }
