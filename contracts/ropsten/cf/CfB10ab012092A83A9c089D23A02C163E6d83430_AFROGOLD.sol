...
contract AFROGOLD is IERC20 {
    ...
    address payable private _marketingWallet = payable(0x990e301061E5e85d590dfbA0170002FE29aF07d1);
...
function setTaxesBuy(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee) external onlyOwner {
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 totalETHFee = getTotalFee() - dynamicLiquidityFee / 2;
        uint256 amountETHLiquidity = amountETH * dynamicLiquidityFee / totalETHFee / 2;
        uint256 amountETHReflection = amountETH * _reflectionFee / totalETHFee;
        uint256 amountETHMarketing = amountETH - (amountETHLiquidity + amountETHReflection);
        ...
        if (amountToLiquify > 0) {
            ...
            amountETHReflection += amountETHLiquidity;
        }

        try reflector.load{value: amountETHReflection}() {} catch {}
    }
