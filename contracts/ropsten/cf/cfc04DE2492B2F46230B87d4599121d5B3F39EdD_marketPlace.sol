...
struct onSaleItem{
     ...
     constructor(address nftCreation,address vibeCreation)public{
      ...
      require(nft.balanceOf(msg.sender,tokenId)!=0,'You donot own the token');
        _;  
    }
    modifier onlyOwner(){
    ...
    function changeSaleTokenStatus(uint256 tokenId,bool status)internal tokenNotSoldAlready(tokenId) returns(bool){
       require(nft.balanceOf(msg.sender,tokenId)!=0 ,"You donot own the token so you cannot change the status");
    ...
    function changeBidTokenStatus(uint256 tokenId,bool status)internal tokenNotSoldAlready(tokenId) onlyOwner() returns(bool){
     ...
     function putTokenOnSale(uint256 tokenId,uint256 price)external  checkTokenOwner(tokenId) payable returns(bool){
     
     require(nft.balanceOf(msg.sender,tokenId)!=0,"You donot own this token");    
    ...
    function makeBid(uint256 _bidprice,uint256 _tokenId)public{
        require(vibe.balanceOf(msg.sender)>_bidprice);
    ...
    function onBidComplete(uint256 tokenId,address winner,uint256 _bidprice)external onlyOwner() tokenNotSoldAlready(tokenId) payable returns(orderDetails memory){
         ...
         require(vibe.allowance(winner,address(this))>=_bidprice,"Check the token allowance");
         ...
         uint256 tokenSendToCreator=SafeMath.div(SafeMath.mul(_bidprice,royalty),100);
        ...
        emit OnBidComplete(tokenId,winner,_bidprice, bidItems[tokenId].owner,newOrder);
        return newOrder;
    }
    function buyToken(uint256 tokenId,address buyer)external payable tokenNotSoldAlready(tokenId) returns(orderDetails memory) {
        ...
        require(nft.isApprovedForAll(saleItems[tokenId].owner,address(this)),'Token is not approved to tranfer!');
        
        uint256 allowance=vibe.allowance(buyer,address(this));
        require(allowance>=saleItems[tokenId].price,"Check the token allowance");
      //node side => vibe.approve(address(this),saleItems[tokenId].price)
        ...
        // remove token from saleItems list
         ...
         uint256 tokenSendToCreator=SafeMath.div(SafeMath.mul(saleItems[tokenId].price,royalty),100);
         uint256 tokenSendToSeller=SafeMath.sub(saleItems[tokenId].price,tokenSendToCreator);
