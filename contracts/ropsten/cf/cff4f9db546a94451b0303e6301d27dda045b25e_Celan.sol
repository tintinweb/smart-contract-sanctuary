...
contract Celan {
    ...
    event ReferalCommission(address indexed from,address indexed to, uint level, uint value,uint time);
    ...
    function _userRegister(address _user, uint _referid, uint _level, uint _amount, address _directAddress,address _invitor) internal {
        ...
        _paylevel(msg.sender,userList[_referid], _level, amount.div(referalUpline[_level]));
    ...
    function buyLevel(uint _level) public isLock payable {
        ...
        require(users[msg.sender].currentLevel + 1 == _level && _level <= 10, "Celan: Level should be next to current level");
        ...
        _paylevel(msg.sender,userList[users[msg.sender].referer], _level, amount.div(referalUpline[_level]));
    ...
    function _paylevel(address _user,address _referer, uint _level, uint _amount) internal {
        ...
        if (loopCheck[msg.sender] < referalUpline[_level]) {
            ...
            emit ReferalCommission(_user,_referer, _level, _amount,block.timestamp);
            loopCheck[msg.sender] = loopCheck[msg.sender].add(1);
            address ref = userList[users[_referer].referer];
            _paylevel(_user,ref, _level, _amount);
        }
    }
        ...
        if (poolMembers.length > 0) {
            for (uint i = 0; i < poolMembers.length; i++) {
                if (poolStatus[poolMembers[i]] == false && poolMembers[i] != address(0)) {
                    ...
                    users[poolMembers[i]].poolEarned = users[poolMembers[i]].poolEarned.add(poolBonus.div(poolMembers.length));
