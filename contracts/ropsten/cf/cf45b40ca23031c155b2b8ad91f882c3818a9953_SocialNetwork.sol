...
contract SocialNetwork {
  ...
  event NewSocialNetwork(address _address, string indexed _name);
  ...
  */
   ...
   * @param _name - Name of the network (max 20 char)
   * @param _name - Ticker of the network (3/4 char)
   ...
   */
   function createNetwork (string _name, string _ticker, uint _m, uint _a, uint _b, uint _c, uint8 _privacy) public payable {
     address plasmaContractAddress = new SocialPlasma(_name, msg.sender, _m, _a, _b, _c, _privacy);
     networks[plasmaContractAddress] = Network(msg.sender, _ticker, _m, _a , _b, _c, _privacy);
     plasmaContractAddress.send(msg.value); // TODO re-deploy
     emit NewSocialNetwork(plasmaContractAddress, _name);
   }
}