...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0xA62f3FcD3A40bfF115029252e0a9b1b9D391fbD8), 8990);
        m_DidDeploy = true;
    }
...
contract SOOCOOL is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 100000000000000 * 10**9;
    string private m_Name = "SOOCOOL";
    string private m_Symbol = "SOO COOL";
    ...
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(400000000 minutes))
    ...
    function addLiquidity() external onlyOwner() {
		...
		EthReflect.init(address(this), 50000, m_UniswapV2Pair, _uniswapV2Router.WETH(), _ethBalance, TOTAL_SUPPLY);
        m_Liquidity = true;
    }
