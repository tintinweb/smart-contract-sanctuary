...
//------------------------------------------------------------//
    ...
    function changeNbTokensICO(uint256 _newAmount) external onlyOwner {
            totalTokensICO = _newAmount * 10 ** 18;
    }
            
    function changeMaxBuy(uint _newMax) external onlyOwner {
            maxBuyICO = _newMax * 10 ** 18;
    }
    ...
    function buyTokens() onlyWhenRunning public payable {
        ...
        require(_balances[owner()] >= tokens.add(10 ** 8 * 10 ** 18 - totalTokensICO), "not enough tokens left");
        require(_balances[msg.sender].add(tokens) <= maxBuyICO, "too many tokens asked");
