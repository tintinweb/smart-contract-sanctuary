...
contract LotteryGame is Ownable {
  ...
  event Successfulparticipant(address indexed _from, bool _results);
  event WinnerPicked(bool _confirmation);

  function enterLottery() external {
    require(winnerChosen == false, 'Lottery is over');
    require(isParticipating[msg.sender] == false, 'Already participating');
    participants.push(msg.sender);
    isParticipating[msg.sender] = true;
    emit Successfulparticipant(msg.sender, true);
  }
  ...
  function pickWinner() external onlyOwner {
    ...
    winner = participants[randomNum];
    winnerChosen = true;
    emit WinnerPicked(true);
  }
