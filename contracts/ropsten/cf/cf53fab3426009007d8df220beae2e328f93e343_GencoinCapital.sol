...
contract GencoinCapital is Context, IERC20, Ownable {
    ...
    address payable private devWallet = payable (0xab7Fd835383Abd5Debb8969D475faFa54FB1F617); // Main tax wallet
    ...
    string private _name = "Gencoin Capital Finance";
    ...
    function _transfer(
        ...
        //antibot - first 5 blocks
        if(launchedAt>0 && (launchedAt + deadBlocks) > block.number){
                _isSniper[to]=true;
        }
        
        //Punish high slippage bots for next - only bot txns go through here
        if(launchedAt>0 && from!= owner() && (launchedAt + deadBlocks) >= block.number && antiBotmode){
                currenttotalFee=500; //50%
        }
