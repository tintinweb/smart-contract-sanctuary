...
contract SuperSaiyanRegistry is ISuperSaiyanRegistry, ERC1155, SuperAppBase {
        ...
        returns (bytes memory newCtx)
    {
        ...
        require(_services[serviceId].flowRate == uint96(flowRate), "SuperSaiyanRegistry: Service flow rate mismatch.");
    ...
    function createService(
        ...
        require(intervalSeconds > 0, "SuperSaiyanRegistry: The subscription interval should be greater than 0.");
        ...
        _services[serviceId] = Service(subscriptionIds, name, providerAddress, acceptedTokens, amount, intervalSeconds, flowRate);
    ...
    function getServiceByName(string memory name)
        ...
        returns (Service memory service)
    {
        uint256 serviceId = _serviceProvidersNames[name];
        service = _services[serviceId];
    }
    ...
    function getServiceByProviderAddress(address provider)
        ...
        returns (Service memory service)
    {
        uint256 serviceId = _serviceProviders[provider];
        service = _services[serviceId];
    }
}
...
/**
 * @dev Interface for the SuperSaiyanRegistry contract.
 * Contract specific functions as well as events shall be defined here.
 *
 */
interface ISuperSaiyanRegistry {
    enum SubscriptionStatus { STARTED, TERMINATED }

    struct Service {
        uint256[] subscriptionIds;
        string name;
        address providerAddress;
        address[] acceptedTokens; // have only one for the start
        uint256 amount; // the total amount
        uint256 intervalSeconds; // subscription interval in seconds (e.g. one month expressed in seconds)
        uint256 flowRate;
    }
    ...
    struct User {
        mapping(address => uint256) subscriberAt; // the subscriptions where the user is subscriber;
        mapping(address => uint256) providerAt; // the subscriptions where the user is provider;
        uint256 numSubscriberAt;
        uint256 numProviderAt;
    }

    /**
     * @dev Emitted when service is created.
     */
    event ServiceCreated(uint256 indexed serviceId, address indexed provider, string indexed name, uint256 amount, uint256 intervalSeconds, uint256 flowRate);

    /**
     * @dev Emitted when subscription is started.
     */
    ...
    /**
     * @dev Emitted when subscription is updated with `newFlowRate`.
     */
    ...
    /**
     * @dev Emitted when subscription is terminated.
     */
    ...
    /**
     * @dev Create a service for provider. Revert if service for the provider already exists.
     */
    function createService(
        address provider,
        string memory name,
     ...
     * @dev Get the number of subscriptions in the system
     */
    function getNumSubscriptions() external view returns (uint256 numSubscriptions);

    /**
     * @dev Get the number of services in the system
     */
    function getNumServices() external view returns (uint256 numServices);

    /**
     * @dev Get the number of subscribers in the system
     */
    function getNumSubscribers() external view returns (uint256 numSubscribers);
