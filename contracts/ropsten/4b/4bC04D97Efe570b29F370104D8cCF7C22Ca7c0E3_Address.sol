...
contract Guardian is Context, IERC20, Ownable {
    ...
    function setRouter(address routerAddress) public onlyOwner() { 
        //require(uniswapV2Router == IUniswapV2Router02(address(0)), 'Router already set!');   
    ...
    function setBurnRate(uint amount) public onlyOwner() {
        require (amount <= 1000, "Burn too high");    // max burn of 10%
        _burnRate = amount;
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] || !_isExcludedFromFee[to]){
            require(!_swapMode, 'Dev mode enabled');
        }
