...
contract NftBattleArena is Ownable, ERC721, VRFConsumerBase
{
	...
	constructor (
		...
		address _link
		) Ownable() VRFConsumerBase(_vrfCoordinator, _link) ERC721("ZooBattle", "zBat")
	{
	...
	function voteWithZoo(uint256 votingPositionId, uint256 amount) public returns (uint256 votes)
	{
		...
		require(amount <= votingPositions[votingPositionId].daiInvested);           // Requires for zoo invested  to be less than dai invested.
	...
	{
		require(getCurrentStage() == Stage.FirstStage, "Wrong stage!");                   // Requires to be at first stage.
		require(ownerOf(votingPositionId) == msg.sender && typeOfPositions[votingPositionId] == PositionType.VoterPosition);// Requires to be owner of position and voter type.
	...
	uint256 lastEpochNumber)
	{
		...
		for (uint256 i = votingPositions[positionId].lastRewardedEpoch; i < lastEpochNumber; i++)
		{
