...
// QubeToken with Governance.
contract QubeToken is ERC20("QubeToken", "Qube"), Ownable {
...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to QubeSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // QubeSwap must mint EXACTLY the same amount of QubeSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterChef is the master of Qube. He can make Qube and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Qube is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accQubePerShare) - user.rewardDebt
        //
    ...
    // Qube tokens created per block.
    uint256 public QubePerBlock;
    // Bonus muliplier for early qube makers.
    ...
    constructor(
        QubeToken _qube,
        address _devaddr,
        uint256 _QubePerBlock,
        uint256 _startBlock
    ) public {
        qube = _qube;
        devaddr = _devaddr;
        QubePerBlock = _QubePerBlock;
    ...
    function pendingQube(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 QubeReward = multiplier.mul(QubePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accQubePerShare = accQubePerShare.add(QubeReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accQubePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 QubeReward = multiplier.mul(QubePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        qube.mint(devaddr, QubeReward.div(10));
        qube.mint(address(this), QubeReward);
        pool.accQubePerShare = pool.accQubePerShare.add(QubeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Qube allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accQubePerShare).div(1e12).sub(user.rewardDebt);
            safeQubeTransfer(msg.sender, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accQubePerShare).div(1e12);
