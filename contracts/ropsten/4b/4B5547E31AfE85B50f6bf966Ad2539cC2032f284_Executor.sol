...
contract Executor is ProposalValidator, IExecutor {
  ...
  uint256 public override EXECUTION_PERIOD;
  uint256 public override MINIMUM_DELAY;
  uint256 public override MAXIMUM_DELAY;
  ...
  function executeTransaction(
    ...
    require(block.timestamp <= executionTime.add(EXECUTION_PERIOD), 'EXECUTION_PERIOD_FINISHED');
  ...
  function createNewGame(
      ...
      uint256 _callingReward) 
  public onlyTimelock {
    address newGame = address(new LotteryGameMock(name, _participationFee, _callingReward));
    lotteryGames.push(newGame);
  }
    ...
    returns (bool)
  {
    ...
    return (block.timestamp > proposal.executionTime.add(EXECUTION_PERIOD));
  }
...
interface IExecutor {
   ...
   **/
  function EXECUTION_PERIOD() external view returns (uint256);
  /**
...
contract ProposalValidator is IProposalValidator {
   ...
   * @param votingDuration duration in seconds of the voting period
  ...
  function isPropositionPowerEnough(
    ...
    return
      currentGovernanceStrategy.getVotingPower(user)>=
    ...
    returns (uint256)
  {
    ...
    return
      currentGovernanceStrategy
        .getTotalVotingSupply() * PROPOSITION_THRESHOLD / ONE_HUNDRED_WITH_PRECISION;
  }
    ...
    returns (bool)
  {
    ...
    return (proposal.forVotes * ONE_HUNDRED_WITH_PRECISION / votingSupply >
      proposal.againstVotes  * ONE_HUNDRED_WITH_PRECISION / votingSupply + VOTE_DIFFERENTIAL);
  }
}
...
contract LotteryGameMock is Ownable {
    ...
    constructor(string memory _name, uint256 fee, uint256 reward) Ownable(){
        NAME = _name;
