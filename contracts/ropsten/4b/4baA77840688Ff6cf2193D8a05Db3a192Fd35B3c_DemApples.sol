...
contract DemApples is ERC721, ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor() ERC721("Dem Apples", "NOM") {
  }

  // SET AND RETRIEVE URIS
  // Set revealed metadata base URI
  function setBaseURI(string memory baseURI_) external onlyOwner {
    baseURI = baseURI_;
  }

  // Retrieve revealed metadata base URI
  function _baseURI() internal view virtual override returns (string memory) {
    return baseURI;
  }

  // Set not-revealed metadata base URI
  function setNotRevealedURI(string memory notRevealedURI_) external onlyOwner {
    notRevealedURI = notRevealedURI_;
  }

  // Retrieve not-revealed metadata base URI
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if(revealed == false) {
      string memory currentNotRevealedURI_ = _notRevealedURI();
      return string(abi.encodePacked(currentNotRevealedURI_, tokenId.toString()));
    }
  ...
  // Mint only if the address is in the whitelist
  function mintPrivateSale(uint8 numberOfTokens) external payable {
    require(!paused);
    uint256 ts = totalSupply();

    require(privateSale, "Private sale is not active");
    require(isWhitelisted(msg.sender), "You are not on the whitelist. Please come back when our Public Sale is active");
    require(numberOfTokens > 0, "Can't mint 0 tokens");
  ...
  function mintPublicSale(uint256 numberOfTokens) public payable {
    require(!paused);
    uint256 ts = totalSupply();
  ...
  // Reveal base URI of actual metadata
  function reveal() public onlyOwner {
      revealed = true;
  }

  // Pause all transactions on the smart contract
  function pause(bool _state) public onlyOwner {
    paused = _state;
  }

  // Set the status of the private sale
  function setPrivateSale(bool _newState) public onlyOwner {
    privateSale = _newState;
  }  
  ...
  // Set the array of whitelisted addresses
  function setWhitelist(address[] calldata addresses) external onlyOwner {
    delete whitelistedUsers;
    whitelistedUsers = addresses;
  }
