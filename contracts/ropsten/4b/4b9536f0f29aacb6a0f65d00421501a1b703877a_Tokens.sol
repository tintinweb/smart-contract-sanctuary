...
contract Tokens {
    ...
    function tokenMetadata(uint256 _id) public view returns (string, string, string, address) {
        return (tokens[_id].name, tokens[_id].iamge, tokens[_id].description, tokens[_id].owner);
    }
    
    function createtoken(string _name, string _iamge, string _description, address _owner) public returns (bool success) {
        tokens[id] = token(_name, _iamge, _description, _owner);
