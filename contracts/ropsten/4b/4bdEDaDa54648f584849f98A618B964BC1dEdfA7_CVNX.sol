...
contract CVNXGovernance is ICVNXGovernance, Ownable {
    ...
    event PollCreate(uint256 indexed pollNum);

    /// @notice Emit when address vote in poll.
    event PollVote(
        address voterAddress,
        VoteType indexed voteType,
        uint256 indexed voteWeight
    );
    ...
    modifier onlyWithBalanceNoLess(uint256 _minimalBalance) {
        require(
            cvnx.balanceOf(msg.sender) > _minimalBalance,
            "[E-34] - Your balance is too low."
        );
        _;
    }
    ...
    function createProposalPoll(uint64 _pollDeadline, string memory _pollInfo)
        external
        override
    {
    ...
    /// @param _pollDeadline Poll deadline
    /// @param _pollInfo Info about poll
    function createExecutivePoll(uint64 _pollDeadline, string memory _pollInfo)
        external
        override
        onlyOwner
    {
    ...
    /// @param _pollDeadline Poll deadline
    /// @param _pollInfo Info about poll
    function createEventPoll(uint64 _pollDeadline, string memory _pollInfo)
        external
        override
        onlyOwner
    {
    ...
    function createPrivatePoll(
        ...
        require(
            _verifiedAddressesCount > 1,
            "[E-35] - Verified addresses not set."
        );

        uint256 _pollNum =
            _createPoll(PollType.PRIVATE, _pollDeadline, _pollInfo);
    ...
    function vote(
        ...
        require(
            polls[_pollNum].pollStopped > block.timestamp,
            "[E-37] - Poll ended."
        );

        if (polls[_pollNum].pollType == PollType.PRIVATE) {
            require(
                verifiedToVote[_pollNum][msg.sender] == true,
                "[E-38] - You are not verify to vote in this poll."
            );
        }
        ...
        voted[_pollNum][msg.sender].voteWeight =
            _voterVoteWeightBefore +
            _voteWeight;
        ...
        emit PollVote(msg.sender, _voteType, _voteWeight);
    }
    ...
    function unlockTokensInPoll(uint256 _pollNum) external override {
        require(
            polls[_pollNum].pollStopped <= block.timestamp,
            "[E-81] - Poll is not ended."
        );
        require(
            isTokenLockedInPoll[_pollNum][msg.sender] == true,
            "[E-82] - Tokens not locked for this poll."
        );
    ...
    function getPollStatus(uint256 _pollNum)
        external
        view
        override
        returns (uint256, PollStatus)
    {
    ...
    function getPollExpirationTime(uint256 _pollNum)
        external
        view
        override
        returns (uint64)
    ...
    function getPollStopTime(uint256 _pollNum)
        external
        view
        override
        returns (uint64)
    ...
    function getPollHistory(address _voter)
        external
        view
        override
        returns (bool[] memory)
    ...
    function getPollInfoForVoter(uint256 _pollNum, address _voter)
        external
        view
        override
        returns (Vote memory)
    ...
    function getIfUserHasVoted(uint256 _pollNum, address _voter)
        external
        view
        override
        returns (bool)
    ...
    /// @param _voter Voter address
    /// @return Poll number
    function getLockedAmount(address _voter)
        external
        view
        override
        returns (uint256)
    ...
    function getPollLockedAmount(uint256 _pollNum, address _voter)
        external
        view
        override
        returns (uint256)
    ...
    function _createPoll(
        ...
        require(
            _pollDeadline > block.timestamp,
            "[E-41] - The deadline must be longer than the current time."
        );
...
/// @notice ICVNXGovernance interface.
