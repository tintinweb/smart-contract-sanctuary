...
/**
 * @dev Required interface of a CryptoAnts compliant contract.
 */
interface ICryptoAnts is IERC721 {
  // / @dev Emitted when `user` buys `amount` Eggs.
  event EggsBought(address user, uint256 amount);

  /// @dev Emitted when a new Ant was created.
  event AntCreated();

  /// @dev Emitted when an Ant was sold.
  event AntSold();
  ...
  function antPrice() external view returns (uint256);
  ...
  /// @dev Occurs when trying to create Ants without Eggs.
  error NoEggs();

  /// @dev Occurs when sending a tx without funds.
  error NotEnoughContractBalance();

  /// @dev Occurs when trying to create an Ant with an existing Id.
  error AlreadyExists();

  /// @dev Occurs when zero address is not permited as param.
  error NoZeroAddress();

  /// @dev Occurs when expected ETH is not equal to msg.value.
  error WrongEtherSent();

  /// @dev Occurs when trying to execute a function not permitted.
  error Unauthorized();

  /// @dev Occurs when call tx fails.
  error CallFailed();
}
