...
contract Dogenstein is Context, IERC20, Ownable {
    ...
    uint256 public _liquidityFee = 6; //(4% liquidityAddition + 1% developmentFund + 1% marketingFund)
    ...
    uint256 private minTokensBeforeSwap = 12;
    ...
    // @dev set Router
    function setRouterAndPair(address _newPancakeRouter) external onlyOwner {
    ...
    function _transfer(
        ...
        if(limit ==  true && from != owner() && to != owner()){
            if(to != pancakePair){
                require(((balanceOf(to).add(amount)) <= 500000000 ether));
            }
            require(amount <= 100000000 ether, 'Transfer amount must be less than 100000000 tokens');
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 forLiquidity = contractTokenBalance.mul(2).div(3);
        uint256 treasuryFund = contractTokenBalance.div(3);
        // split the liquidity
        ...
        // swap tokens for ETH
        swapTokensForEth(half.add(treasuryFund)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 Balance = address(this).balance.sub(initialBalance);
        uint256 oneHalf = Balance.div(2);
        treasury.transfer(oneHalf);

        // add liquidity to pancake
        addLiquidity(otherHalf, oneHalf);
        
        emit SwapAndLiquify(half, oneHalf, otherHalf);
    }
