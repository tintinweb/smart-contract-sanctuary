...
contract DigitalAsset is ERC721PresetMinterPauserAutoId {
    string[] public certificates;
    mapping(string => bool) _certificateExists;
    mapping(string => address) _ownerOfCertificate;
    ...
    function mint(string memory _certificate) public {
        ...
        //Add certificate to the NFTs
        certificates.push(_certificate);

        uint256 _id = certificates.length;
        //Mint the NFT
        _mint(msg.sender, _id);

        //Track the certificate
        _certificateExists[_certificate] = true;
        _ownerOfCertificate[_certificate] = msg.sender;
    }

    // see also ownerOf(uint256 tokenId)
    function ownerOfCertificate(string memory _certificate)
        public
        view
        returns (address)
    {
        return _ownerOfCertificate[_certificate];
    }
}