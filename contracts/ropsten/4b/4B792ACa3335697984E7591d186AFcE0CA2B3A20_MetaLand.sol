...
import "./NoBSHelpers/NoBSSupport.sol";

contract MetaLand is IBEP20, LockableFunction, NoBSSupport, LPSwapSupport, AntiLPSniper{
    ...
    address public buybackAddress = 0xf94ae1187D3572Ba64aE659C858DB200B7b65F2d;
    ...
    constructor(string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, address routerAddress,
            address tokenOwner) LPSwapSupport() NoBSAdvSupport() public {
        ...
        transferFees.liquidity = 3;
        transferFees.marketing = 4;
        transferFees.buyback = 2;
        transferFees.reflectionToken1 = 3;
        transferFees.reflectionToken2 = 2;
        transferFees.divisor = 100;

        buyFees.liquidity = 3;
        buyFees.marketing = 4;
        buyFees.buyback = 2;
        buyFees.reflectionToken1 = 3;
        buyFees.reflectionToken2 = 2;
        buyFees.divisor = 100;

        sellFees.liquidity = 3;
        sellFees.marketing = 4;
        sellFees.buyback = 2;
        sellFees.reflectionToken1 = 3;
        sellFees.reflectionToken2 = 2;
        sellFees.divisor = 100;

        maxWalletSize = _totalSupply.div(50);
        ...
        _previousOwner = tokenOwner;
    }

    function finalize(address reflector1, address reflector2) external onlyOwner init {
        setReflectorAddress(0, reflector1);
        setReflectorAddress(1, reflector2);
        updateRouterAndPair(address(pancakeRouter));
        updateReflectors(address(this), _balances[address(this)], _owner, _balances[_owner]);
        excludeFromRewards(pancakePair, true);
    ...
    function updateTransferFees(uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee, uint256 reflector1Fee, uint256 reflector2Fee, uint256 feeDivisor) external onlyOwner {
        transferFees = Fees({
    ...
    function updateSellFees(uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee, uint256 reflector1Fee, uint256 reflector2Fee, uint256 feeDivisor) external onlyOwner {
        _updateFees(true, liquidityFee, marketingFee, buybackFee, reflector1Fee, reflector2Fee, feeDivisor);
    }

    function updateBuyFees(uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee, uint256 reflector1Fee, uint256 reflector2Fee, uint256 feeDivisor) external onlyOwner {
        _updateFees(false, liquidityFee, marketingFee, buybackFee, reflector1Fee, reflector2Fee, feeDivisor);
    }

    function _updateFees(bool updateSellFee, uint256 _liquidity, uint256 _marketing, uint256 _buyback, uint256 _reflector1, uint256 _reflector2, uint256 _divisor) private {
        if(updateSellFee){
            sellFees = Fees({
        ...
        if(from != owner() && to != owner() && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if(!isWhitelisted[from] && !isWhitelisted[to]) {
                if(automatedMarketMakerPairs[from] && antiSniperEnabled && !tradingIsEnabled){
                    ...
                    taxAllocations.liquidity = taxAllocations.liquidity.add(amount);
                    return;
                } else {
    ...
    function takeFees(address from, uint256 amount, bool isSell) private returns(uint256){
        ...
        thisFee = amount.mul(txFees.buyback).div(divisor);
        totalFees = totalFees.add(thisFee);
        taxAllocations.buyback = taxAllocations.buyback.add(thisFee);
    ...
    function performSwap() private lockTheSwap {
        ...
        if(taxAllocations.liquidity >= minTokenSpendAmount){
            ...
            taxAllocations.buyback = taxAllocations.buyback.sub(swapTokensForCurrencyAdv(address(this), taxAllocations.buyback, buybackAddress));
        }
...
import "./NoBSAdvSupport.sol";

abstract contract NoBSSupport is NoBSAdvSupport {
    ...
    function setDistributionCriteriaReflector1(uint256 minDistribution) external onlyOwner {
        noBSReflectors[0].setDistributionCriteria(minDistribution);
    }

    function setDistributionCriteriaReflector2(uint256 minDistribution) external onlyOwner {
        noBSReflectors[1].setDistributionCriteria(minDistribution);
    }
}
...
abstract contract NoBSAdvSupport is Ownable, LockableFunction {
    ...
    modifier init(){
        require(!isInit, "Contract has been initialized already");
        isInit = true;
        _;
    }
    ...
    function updateReflectors(address from, uint256 fromBal, address to, uint256 toBal) internal {
        for(uint256 i = 0; i < totalReflectors; ++i){
            noBSReflectors[i].setShares(from, fromBal, to, toBal);
        }
    }
    ...
    function setReflectorAddress(uint8 index, address reflector) public onlyOwner {
