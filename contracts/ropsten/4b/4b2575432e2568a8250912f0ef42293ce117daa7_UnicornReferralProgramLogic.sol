...
contract UnicornReferralProgramLogic is Ownable {
    ...
    constructor(address pun, address unicornUsersReferral)  {
        levels = [50, 30, 20];
        PUN = IERC20(pun);
    ...
    function processFee(address user, uint amount) external {
        require(PUN.balanceOf(address(this)) >= amount, "Unicorn Referral: Not correct amount");
        ...
        for (uint i; i < levels.length; i++) {
            ...
            if (sponsor == address(0)) {
                PUN.transfer(unicornUsers.userAddressById(2), unspent);
                break;
            } else {
                uint transferAmount = amount.mul(levels[i]) / 100;
                PUN.transfer(sponsor, transferAmount);
