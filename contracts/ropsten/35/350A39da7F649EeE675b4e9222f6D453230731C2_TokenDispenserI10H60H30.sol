...
contract TokenDispenserBase is Context, Ownable {
    ...
    modifier onlyInTransferWhitelist(address account) {
        require(transferWhitelist[account], "not in transfer whitelist");
        _;
    }
    ...
    function transferAllocation(address recipient, uint amount) external onlyTransferNotPaused onlyInTransferWhitelist(msg.sender) updateVested(msg.sender) updateVested(recipient) returns (bool) {
...
contract TokenDispenserI10H60H30 is TokenDispenserBase {
    ...
    constructor (address kine_, uint startTime_) public {
        ...
        transferPaused = false;
        hour1EndTime = startTime_.add(1 hours);
        hour2EndTime = startTime_.add(2 hours);
    }
