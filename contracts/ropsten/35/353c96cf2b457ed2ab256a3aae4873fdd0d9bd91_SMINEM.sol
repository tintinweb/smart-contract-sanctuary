...
abstract contract ERC20 is IERC20 {
    uint256 internal constant _totalSupply = 1e6;
    string _name;
    string _symbol;
...
abstract contract MaxBuyDynamic is ERC20 {
    ...
    function checkMaxBuyLimit(address to, uint256 amount) internal {
        // increment maxBuy
...
abstract contract TradableErc20 is MaxBuyDynamic, FirstBlockBots, Ownable {
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_isExcludedFromFee[to]) {
            ...
            // max buy limit
            if (!isAutoBanBots()) checkMaxBuyLimit(to, amount);
            trySetBot(to);
            // calculate fee
...
contract SMINEM is TradableErc20, Withdrawable {
    constructor() TradableErc20("SMINEM", "SMINEM") {
