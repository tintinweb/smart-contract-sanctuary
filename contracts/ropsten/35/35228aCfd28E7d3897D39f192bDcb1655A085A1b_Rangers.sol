...
interface CamoInterface {
    function _ownerAddress(address) external view returns(uint256[] memory);
}
...
contract Rangers is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string public _baseURIextended = "TBD";
    ...
    function mintwithCAMO() public nonReentrant {
        uint256[] memory ids = CamoInterface(contractAddress)._ownerAddress(msg.sender);
    ...
    function mintwithCAMOTest() public nonReentrant {
        // uint256[] memory ids = CamoInterface(contractAddress)._ownerAddress(msg.sender);
        uint256[] memory ids;
        ids[0] = 5664;
        ids[1] = 309;
