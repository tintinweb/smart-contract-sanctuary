...
interface IOracle{
    function quote() external returns (uint amountB, uint reserve0, uint reserve1);
}
...
contract MemeCoin is ERC20 {
  ...
  uint256 private MAX_ALLOWED_SUPPLY = ~uint256(0);
  // means 1%
  uint8 private targetRateMultiplier = 101;
  ...
  constructor(address _owner, address _controller) ERC20("MemeCoin16", "MEMC16"){
    controller = _controller;
    _totalSupply = 100000000000000000000000;
    _gonsPerFragment = 20;
    _totalGons = _gonsPerFragment * _totalSupply;
    _gonsPerOwner[_owner] = _totalGons;

    rebaseCooldown = 2 minutes;
  ...
  function setRebaseCooldown(uint8 _newCooldown) onlyController public {
    rebaseCooldown = _newCooldown;
  }

  function setTargetRatePercentage(uint8 _rateMultiplier) onlyController public{
    targetRateMultiplier = _rateMultiplier;
  }

  function setOracle(address _oracleAddress) onlyController public{
    oracle = IOracle(_oracleAddress);
    (uint amountB, uint reserve0, uint reserve1) = oracle.quote();
    baseRate = amountB;
  ...
  function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
    ...
    require(_gonsPerOwner[sender] >= valueInGons, "Insufficient funds");
  ...
  function getRebaseValues() public returns (uint256, uint256, int256){
    ...
    int256 supplyDelta = int256(-1) * int256(_totalSupply.mul(rateDiff).div(targetRate, "Failed at supply delta target rate division"));
    return (
      exchangeRate, 
      targetRate, 
      supplyDelta
  ...
  function rebase() external returns(uint256) {
    ...
    _gonsPerFragment = _totalGons.div(_totalSupply, "Failed at gons per fragment total supply division");

    assert(_totalSupply <= MAX_ALLOWED_SUPPLY);

    baseRate = targetRate.mul(targetRateMultiplier).div(100, "Failed at new base rate setting");
