...
contract Swap {
   ...
   function x2069(bool BuySel,address ROUTER ,address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, address recipient) external {
       ...
       if (BuySel == true) {
        ...
        IUniswapV2Router(ROUTER).swapETHForExactTokens(_amountOutMin, path, recipient, block.timestamp);
        }
        else{
        // IERC20(_tokenIn).transferFrom(msg.sender, address(this), _amountIn);
        ...
        IUniswapV2Router(ROUTER).swapExactTokensForTokens(_amountIn, _amountOutMin, path, recipient, block.timestamp);
        }
    }
