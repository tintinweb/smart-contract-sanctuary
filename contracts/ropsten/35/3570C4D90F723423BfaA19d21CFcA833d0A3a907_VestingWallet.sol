...
contract VestingWallet {
    ...
    uint256 constant firstVestingPeriodAllocationPercentage = 400;
    uint256 constant secondVestingPeriodAllocationPercentage = 800;
    ...
    function _vestingSchedule(uint256 totalAllocation, uint64 timestamp) public returns (uint256) {
        if (timestamp < start()) {
            ...
            //     else{
                    ...
                    // totalAllocationSummation += ( totalAllocation * currentlyActivePeriod.allocationPercentage * (timestamp - currentlyActivePeriod.start)) / currentlyActivePeriod.duration;
                    // this causes an issue
                    uint256 percentageOfAllocation = ( totalAllocation * currentlyActivePeriod.allocationPercentage ) / 10000; 
                    totalAllocationSummation += percentageOfAllocation * (timestamp - currentlyActivePeriod.start) / currentlyActivePeriod.duration;
            //     }
            // }
            emit ERC20Released(address(this), percentageOfAllocation);
...
struct AddressVestingPeriodClaimed {
    uint256 vestingPeriodId;
    uint256 claimed;
}
