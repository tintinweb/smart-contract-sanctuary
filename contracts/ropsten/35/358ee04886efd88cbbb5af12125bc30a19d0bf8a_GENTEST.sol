...
contract GENTEST is Context, IERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "GENTEST Token";
    string private _symbol = "GENTEST";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ECR20: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ECR20: decreased allowance below zero"));
        return true;
    }
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ECR20: approve from the zero address");
        require(spender != address(0), "ECR20: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "ECR20: transfer from the zero address");
        require(to != address(0), "ECR20: transfer to the zero address");
    ...
    uint256 public _liquidityFee = 14; // 5% will be added pool, 9% will be converted to ETH
    ...
    function claimETHReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own GENTEST to claim reward');
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(uniswapPair)) // swap 1 time
            ...
            uint256 piece = contractTokenBalance.sub(pooledETH).mul(65).div(100); // 9/14 %
            uint256 otherPiece = pooledETH.sub(piece); //5/14 %
