...
//https://bscscan.com/address/0xca937f076285583ec607218f1d1b1163646277bc#code
contract Reinvestor {
    ...
    function reinvest(
        ...
        uint256 liquidityMinted = mintLpTokens(
            maxClaimTokenAmount,
            deadline,
            claimV,
            claimR,
        ...
        uint256 reinvestAmount = Math.min(permitAmount, liquidityMinted);
        ...
        StakingService(stakingService).stakeFrom(
            msg.sender,
            uint128(reinvestAmount)
        );
    }

    function mintLpTokens(
        uint128 signedClaimTokenAmount,
        ...
        uint128 claimTokenAmount;
        uint128 pairedTokenAmount;
        (claimTokenAmount, pairedTokenAmount) = getTokenAmount(
            signedClaimTokenAmount,
            maxPairedTokenAmount
        );
        ...
        IERC20(token).safeTransfer(pair, claimTokenAmount);
        IERC20(pairedToken).safeTransferFrom(
            msg.sender,
            pair,
            pairedTokenAmount
        );
        return IPancakePair(pair).mint(msg.sender);
    }

    function getTokenAmount(uint128 tokenAmount, uint128 pairedTokenAmount)
        private
        view
        returns (uint128 requiredTokenAmount, uint128 requiredPairedTokenAmount)
    {
        (uint256 tokenReserve, uint256 pairedTokenReserve) = PancakeLibrary
            .getReserves(factory, token, pairedToken);
        uint256 quotedTokenAmount = PancakeLibrary.quote(
            pairedTokenAmount,
            pairedTokenReserve,
            tokenReserve
        );
        if (quotedTokenAmount <= tokenAmount) {
            return (uint128(quotedTokenAmount), pairedTokenAmount);
        }
        uint256 requiredPairedTokenAmount_ = PancakeLibrary.quote(
            tokenAmount,
            tokenReserve,
            pairedTokenReserve
        );
        return (tokenAmount, uint128(requiredPairedTokenAmount_));
    }
}
