...
contract Bridge is
    ...
    uint256[] public override destChainWhiteList;
    ...
    function addChainWhiteList(uint256 chainId) external override onlyOwner {
        require(!isChainIdWhiteListed(chainId), "Bridge: chain already whitelisted");
        destChainWhiteList.push(chainId);
    }
    function removeChainWhiteList(uint256 chainId) external override onlyOwner {
        uint256[] memory l = destChainWhiteList;
        for (uint256 i = 0; i < l.length; i++ ) {
            if (l[i] == chainId) {
                destChainWhiteList[i] = destChainWhiteList[destChainWhiteList.length - 1];
                destChainWhiteList.pop();
                return;
            }
        }
        revert("Bridge: chainId not found in whitelist");
    }

    function isChainIdWhiteListed(uint256 chainId) internal view returns(bool) {
        uint256[] memory l = destChainWhiteList;
        for (uint256 i = 0; i < l.length; i++ ) {
            if (l[i] == chainId) return true;
        }
        return false;
    }
    ...
    function bridgeTokenAt(
        ...
        require(isChainIdWhiteListed(dstChainId_), "Bridge: chainId not whitelisted");
...
interface IBridgeOutbound {
    function destChainWhiteList(uint256 chainId) external view returns (uint256);
    function addChainWhiteList(uint256 chainId) external;
    function removeChainWhiteList(uint256 chainId) external;
