...
contract VestingWallet is IVestingWallet {
     ...
     */
    function start() public override returns (uint256) {
     ...
     */
    function currentlyActivePeriod() public override returns (VestingPeriod memory){
     ...
     */
    function released(address recipient) public override returns (uint256) {
     ...
     */
    function vestedAmount(uint64 timestamp, address recipient,uint256 totalAllocation) public override returns (uint256) {
     ...
     */
    function _vestingSchedule(uint256 totalAllocation, uint64 timestamp) public override returns (uint256) {
        if (timestamp < start()) {
            return 0;
