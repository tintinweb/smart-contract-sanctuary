...
// new log with the previous strategy price
    ...
    function bibatchbacktest(uint32[]dates, uint64[2][] prices, int64[2][] positions) public onlyAuthorized {
      require(positions[0].length == positions[1].length);
      uint batchlength = positions[0].length;
      // otherwise standard advance
      require(batchlength >= 2);
      require(dates.length==batchlength && prices[0].length==batchlength && prices[1].length==batchlength);
      ...
      require(strategyInitPosition[0]==positions[0][0]);
      require(strategyInitPosition[1]==positions[1][0]);

      require(strategyInitDate==dates[0]);

      uint64 strategyPrice = StrategyLib.libbibatchbacktest(strategyInitPrice,dates,prices,positions);
      ...
      explicitState.position[0] = positions[0][batchlength-1];
      explicitState.position[1] = positions[1][batchlength-1];
...
library StrategyLib{
  ...
  *stratPrice: strategy price of the day (taken the previous day)
  ...
  -------------------------------------------------------------------------*/
  function libbibatchbacktest(uint64 strategyInitPrice, uint32[]dates, uint64[2][] prices, int64[2][] positions) public returns (uint64) {
