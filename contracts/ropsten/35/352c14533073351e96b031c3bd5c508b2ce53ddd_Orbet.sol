...
contract Orbet {
    ...
    function endBet(uint betStructId) public {
        ...
        if (vaultPercentage > 0) {
            totalVaultRewards = ABDKMath.add(totalVaultRewards, ABDKMath.div(ABDKMath.mul(ABDKMath.div(combinedOrbitBetAmountBytes, ABDKMath.fromUInt(orbitsPerOrbi)), vaultPercentageBytes), ABDKMath.fromUInt(_totalVaultDeposits)));
        }
    ...
    function vaultCheckRewards() external view returns (uint) {
        ...
        return ABDKMath.toUInt(ABDKMath.mul(ABDKMath.sub(totalVaultRewards, VaultAccounts[msg.sender].T0TotalVaultRewards), ABDKMath.fromUInt(VaultAccounts[msg.sender].StoredLiquidity)));
    }
    ...
    function setWinnerPercentage(uint16 newWinnerPercentage) external onlyOwner {
        require(!frozen, "Frozen!");
        winnerPercentage = newWinnerPercentage;
        winnerPercentageBytes = ABDKMath.div(ABDKMath.fromUInt(winnerPercentage), 0x4012e848000000000000000000000000);
    }
    
    function setCallerPercentage(uint16 newCallerPercentage) external onlyOwner {
        require(!frozen, "Frozen!");
        callerPercentage = newCallerPercentage;
        callerPercentageBytes = ABDKMath.div(ABDKMath.fromUInt(callerPercentage), 0x4012e848000000000000000000000000);
    }
    
    function setInitiatorPercentage(uint16 newInitiatorPercentage) external onlyOwner {
        require(!frozen, "Frozen!");
        initiatorPercentage = newInitiatorPercentage;
        initiatorPercentageBytes = ABDKMath.div(ABDKMath.fromUInt(initiatorPercentage), 0x4012e848000000000000000000000000);
    }
    
    function setBurnPercentage(uint16 newBurnPercentage) external onlyOwner {
        require(!frozen, "Frozen!");
        burnPercentage = newBurnPercentage;
        burnPercentageBytes = ABDKMath.div(ABDKMath.fromUInt(burnPercentage), 0x4012e848000000000000000000000000);
    }
    
    function setVaultPercentage(uint16 newVaultPercentage) external onlyOwner {
        require(!frozen, "Frozen!");
        vaultPercentage = newVaultPercentage;
        vaultPercentageBytes = ABDKMath.div(ABDKMath.fromUInt(vaultPercentage), 0x4012e848000000000000000000000000);
    }
    
    function addTeamContract(address teamMemberAddress, uint teamMemberPercent) external onlyOwner {
        require(!frozen, "Frozen!");
