    ...
    AbstractDependant
{
    ...
    uint256 public constant LM_DURATION = 1 days;
    ...
    function createTeam(string calldata _teamName) external override {
        require(isLMStarted() && !isLMEnded(), "LM: LME didn't start or finished");
        require(
    ...
    function checkAvailableNFTReward(address _userAddr) external view override returns (bool) {
        if (!isLMEnded() || usersTeamInfo[msg.sender].isNFTDistributed) {
            return false;
        }
    ...
    function isLMStarted() public view override returns (bool) {
        return startLiquidityMiningTime != 0;
    }

    function isLMEnded() public view override returns (bool) {
        return isLMStarted() && getEndLMTime() < block.timestamp;
    }
    ...
    function _sendNFT(uint256 _index, address _userAddr) internal {
        ...
        if (_index == 0) {
            _nftIndex = GOLD_NFT_ID;
        } else if (_index < 4) {
            _nftIndex = SILVER_NFT_ID;
        } else {
            _nftIndex = BRONZE_NFT_ID;
        }
