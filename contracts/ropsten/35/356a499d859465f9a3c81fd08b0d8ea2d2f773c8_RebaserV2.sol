...
contract RebaserV2 {
    ...
    function rebaseOrbi(int supplyDelta) external onlyOwner returns (bool) {
        ...
        if (supplyDelta > 0)
            orbiSupplyTotal = orbiSupplyTotal + uint(supplyDelta);
        if (supplyDelta < 0)
            orbiSupplyTotal = orbiSupplyTotal - uint(-supplyDelta);
    }
    ...
    function setOrbiSupplyTotal(uint newOrbiSupplyTotal) external onlyOwner returns (bool) {
        orbiSupplyTotal = newOrbiSupplyTotal;
        return true;
    }
    ...
    function rebase() external returns (bool) {
        ...
        require(orbiNewSupply != orbiSupplyTotal, "SUPPLY_UNCHANGED");
