...
contract LootCraftGame is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 ennemyId, uint256 lootId) override public view returns (string memory) {
        ...
        parts[1] = getRarity(random(string(abi.encodePacked("NATURE", toString(ennemyId)))));
        parts[2] = getNature1(ennemyId);

        parts[3] = '</text><text x="10" y="40" class="base"';
        parts[4] = getRarity(random(string(abi.encodePacked("NATURE2", toString(ennemyId)))));
        parts[5] = getNature2(ennemyId);
        
        parts[6] = '</text><text x="10" y="60" class="base"';
        parts[7] = getRarity(random(string(abi.encodePacked("NATURE3", toString(ennemyId)))));
        parts[8] = getNature3(ennemyId);

        parts[9] = '</text><text x="10" y="80" class="base"';
        parts[10] = getRarity(random(string(abi.encodePacked("MINERAL", toString(ennemyId)))));
        parts[11] = getMineral1(ennemyId);

        parts[12] = '</text><text x="10" y="100" class="base"';
        parts[13] = getRarity(random(string(abi.encodePacked("MINERAL2", toString(ennemyId)))));
        parts[14] = getMineral2(ennemyId);
        
        parts[15] = '</text><text x="10" y="120" class="base"';
        parts[16] = getRarity(random(string(abi.encodePacked("MINERAL3", toString(ennemyId)))));
        parts[17] = getMineral3(ennemyId);

        parts[18] = '</text><text x="10" y="140" class="base"';
        parts[19] = getRarity(random(string(abi.encodePacked("MONSTER", toString(ennemyId)))));
        parts[20] = getMonster1(ennemyId);

        parts[21] = '</text><text x="10" y="160" class="base"';
        parts[22] = getRarity(random(string(abi.encodePacked("MONSTER2", toString(ennemyId)))));
        parts[23] = getMonster2(ennemyId);
        
        parts[24] = '</text><text x="10" y="180" class="base"';
        parts[25] = getRarity(random(string(abi.encodePacked("MONSTER3", toString(ennemyId)))));
        parts[26] = getMonster3(ennemyId);
        
        parts[27] = '</text><text x="10" y="220" class="base"';
        parts[28] = getRarity(random(string(abi.encodePacked("ENNEMIES", toString(ennemyId)))));
        parts[29] = battleEnnemies(ennemyId, lootId);
        parts[30] = ' with Loot #';
        parts[31] = toString(lootId);
    ...
    function play(uint256 ennemyId, uint256 lootId) public nonReentrant {
        require(lootId > 0 && lootId < 7779, "Loot Token ID invalid");
