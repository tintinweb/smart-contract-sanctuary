...
contract Clone_Factory {
  event Album_Core_Launched(address indexed new_instance, address indexed owner, string artist, string uri);
  function launch_album_core(string memory artist, string memory uri) public {
    //create
    Album_Core_ETH new_instance = new Album_Core_ETH(artist, uri);
    //set owner to sender
    new_instance.change_owner(msg.sender);

    emit Album_Core_Launched(address(new_instance), msg.sender, artist, uri);
  }
}