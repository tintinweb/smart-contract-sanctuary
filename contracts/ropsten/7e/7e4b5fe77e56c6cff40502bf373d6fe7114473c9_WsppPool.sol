...
//        uint256 _poolLimitPerUser,
    ...
    function oliveStartIn(uint256 _blocks) public onlyOwner {
        oliveSetStart(block.number.add(_blocks));
    }

    function oliveSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function oliveSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        oliveCalcRewardPerBlock();
    }

    function oliveCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function oliveSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
