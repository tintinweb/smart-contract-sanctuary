...
contract Escrow {
    ...
    function addOrder(address seller, address buyer) public onlyOwner returns(bool){
    ...
    function createOrder(address seller, string memory orderName) public onlySeller returns(bool){
        require(seller != address(0), "Invalid Address");
    ...
    function createPayment(uint orderId, address buyer, uint value) onlyBuyer external payable payableCheck {
    ...
    function setNewOrderAnalytic(uint256 temperature, uint256 humidity) external onlyCarrier returns(bool){
        require(temperature != 0, "Invalid Temperature");
        require(humidity !=0, "Invalid Humidity");
        _temperature = temperature;
        _humidity = humidity;
