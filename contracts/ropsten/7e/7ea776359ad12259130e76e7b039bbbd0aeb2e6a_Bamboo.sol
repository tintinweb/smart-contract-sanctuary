...
contract Bamboo is ERC20("Bamboo Token", "$Bamboo") {
	using SafeMath for uint256;

    uint256 public cliamUnitPerPeriod = 10;
    uint256 public cliamPeriod = 5 minutes;
	uint256 public cliamEndTime = now + 365 * 10 days;
	...
	event CliamedReward(address indexed user, uint256 tokenId, uint256 reward);
    ...
    function _checkCliamTime(uint256 _tokenId) internal view returns (bool) {
        if (lastClaimedTime[_tokenId] + cliamPeriod <= now ) {
            return true;
        } else {
            return false;
        }        
    ...
    function _getAccmulatedDays(uint256 _tokenId, address _holder) internal view returns (uint256) {
         if (lastClaimer[_tokenId] == _holder ) {
             return now.sub(lastClaimedTime[_tokenId]).div(cliamPeriod);  
         } else {
             return 1;
         } 
    }
    
    function _cliamReward(uint256 _tokenId, address _holder) internal {
        if (_checkCliamTime(_tokenId)){
            uint256 howManyDaysAccumulated = _getAccmulatedDays(_tokenId, _holder);
            uint256 claimAmount = howManyDaysAccumulated * cliamUnitPerPeriod * (uint(10)**decimals());
            _mint(_holder, claimAmount);
            lastClaimedTime[_tokenId] = now;
            lastClaimer[_tokenId] = _holder;
            emit CliamedReward(_holder, _tokenId, claimAmount);
        }
    }
    ...
    function cliam() external {
       require(now <= cliamEndTime, "Cliam period is ended!");    
       ...
       for (uint256 i=0; i < tokenCount; i++){
           ...
           _cliamReward(tokenId, msg.sender);
       }
       _unlock(msg.sender);
    }
}