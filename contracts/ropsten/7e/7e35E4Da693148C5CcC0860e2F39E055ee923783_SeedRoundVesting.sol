...
contract SeedRoundVesting is Erc20MultiVesting {
    constructor() Erc20MultiVesting(
        ...
        _allocate(0xDB31278bCBCcb6FbB4118bB23B0A4190644bD97e, 100000000);
    }
}
...
abstract contract Erc20MultiVesting is Context {
    ...
    constructor (
        ...
        require(startTime_ > block.timestamp, "Erc20MultiVesting: Start time is before current time!");
    ...
    function _vestedAmount(address beneficiary) private view returns (uint256) {
        ...
        uint256 timeLeftAfterStart = currentTime - startTime;
        if (timeLeftAfterStart < cliffDuration) {
            return _min(beneficiaryAvailable, tgeAmount);
        }

        uint256 timeLeftAfterCliff = currentTime - (startTime + cliffDuration);
