...
luckyWinners = string(abi.encodePacked(luckyWinners  , "Address : " , addr , " , Luck Reward : " , val, "\n"));  
...
//uint256 interestPerDay= (interestRatePerYear );

////principal = add (principal, mulDiv (ratio, principal, 10^18));

    uint256  newInterests = daysSinceLastCollect * (interestRatePerYear) * (balanceOf(_stakeholder));

       return newInterests / 365;
