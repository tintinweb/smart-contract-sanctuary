...
abstract contract NFT721Creator is Initializable, AccountMigration, ERC721Upgradeable {
   ...
   */
  function isWhitelistedCreator(address _creator) public returns (bool) {
      return creatorsPermittedToMint[_creator];
  }
...
abstract contract NFT721Mint is Initializable, AccountMigration, ERC721Upgradeable, NFT721Creator, NFT721Market, NFT721Metadata {
   ...
   */
  function mintAndTransfer(string memory _tokenIPFSPath, address creator, bytes memory signature) public returns (uint256) {
    ...
    uint256 _tokenId = _mint(creator, _tokenIPFSPath);
    _safeTransfer(creator, msg.sender, _tokenId, "");
  }
  /**
   * @notice Allows a creator to mint an NFT.
   */
  function mint(string memory _tokenIPFSPath, uint256 timestamp, bytes memory signature) public returns (uint256) {
    ...
    bytes memory data = abi.encodePacked(
        "I authorized the market to mint ",
        _tokenIPFSPath,
        " but not after ",
  ...
  function _mint(string memory tokenIPFSPath) internal returns (uint256 tokenId) {
    _mint(msg.sender, tokenIPFSPath);
  }
...
abstract contract AccountMigration is MarketOperatorRole {
  ...
  function _toEthSignedMessage(bytes memory message) public pure returns (bytes32) {
