...
contract MysticBets is ERC721 {
  ...
  address public feeRecipient = 0x5F08C5804DdD7A75b260929eddAA9c289AfaB3f5;
  ...
  uint256 public constant sellerFeeBasisPoints = 5;
  ...
  function mintMysticBets() external {
  ...
  function buyToken(uint256 _tokenId) public payable {
    ...
    address payable feeSendTo = payable(feeRecipient);
    // send token's worth of ethers to the owner
    sendTo.transfer(msg.value - msg.value * sellerFeeBasisPoints / 100);
    feeSendTo.transfer(msg.value * sellerFeeBasisPoints / 100);
    // update the token's previous owner
    mysticbet.previousOwner = mysticbet.currentOwner;
    // update the token's current owner
    mysticbet.currentOwner = msg.sender;
    // update the how many times this token was transfered
    mysticbet.numberOfTransfers ++;
  ...
  function toggleForSale(uint256 _tokenId) public {
    ...
    if (mysticbet.forSale)  mysticbet.forSale = false;
    else mysticbet.forSale = true;
    // set and update that token in the mapping
    allMysticBets[_tokenId] = mysticbet;
  }
