...
contract Radoa is DeviceRegister, UpdateMsgVerifier {
    ...
    function isOpenedIndex(uint64 _index) public view returns (bool) {
        return timer.isOpenedIndex(_index);
    }

    function computeOpenTime(uint64 _index) public view returns (uint64) {
    ...
    function initDevice(bytes8 _timestamp) public {
        ...
        lastMsgOfDeviceId[deviceId].index = bytes4(timer.getLastClosedIndex());
...
contract UpdateMsgVerifier is SignatureVerifier, VDFHashManager {
    ...
    function attestMsgValidity(UpdateMsg memory _oldMsg, UpdateMsg memory _newMsg, Signature memory _updateSignature) internal view {
        require(uint64(_oldMsg.timestamp).add(timer.updatePeriod()) <= uint64(_newMsg.timestamp) && uint64(_newMsg.timestamp) <  uint64(_oldMsg.timestamp).add(2*timer.updatePeriod()),"invalid timestamp");
...
contract VDFHashManager is VerifyVDFForRoot, VDFNodeManager {
    ...
    uint constant VDF_SUBMISSION_LATENCY_SEC = 10;
...
interface ITimer {
    ...
    function isOpenedIndex(uint64 _index) external view returns (bool);
    function computeOpenTime(uint64 _index) external view returns (uint64);
...
contract Timer is ITimer {
    ...
    uint64 constant UPDATE_PERIOD = 60;/*120;*/
    ...
    function isOpenedIndex(uint64 _index) public view override returns (bool) {
        return getBlockTimestamp() >= computeOpenTime(_index);
    }

    function computeOpenTime(uint64 _index) public view override returns (uint64) {
        return uint64(deployedTime.add(UPDATE_PERIOD.mul(_index)));
    }
