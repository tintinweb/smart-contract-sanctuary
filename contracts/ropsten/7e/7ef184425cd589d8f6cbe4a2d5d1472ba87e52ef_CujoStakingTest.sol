...
contract CujoStakingTest is Ownable {
    ...
    function stakeDay() public view returns (uint256) {
        return block.timestamp / 1 minutes - stakeStart / 1 minutes;
    }

    function userStakeDay(address staker) public view returns (uint256) {
        return block.timestamp / 1 minutes - _stakers[staker].start / 1 minutes;
    }

    function twoWeek(address staker) public view returns (uint256) {
        return _stakers[staker].start / 1 minutes - stakeStart / 1 minutes;
    }

    function _isLocked(address staker) public view returns (bool) {
        bool isLocked = false;

        uint256 _stakeDay = _stakers[staker].start / 1 minutes;
        if (_stakeDay - stakeStart / 1 minutes < 14) {
           if (block.timestamp / 1 minutes - _stakeDay < 14) {
               isLocked = true;
           }
        }
