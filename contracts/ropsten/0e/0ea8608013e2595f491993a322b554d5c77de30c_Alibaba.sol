...
contract Alibaba is Ownable {
	...
	event DepositDai(address userAddress, uint amount);
	...
	constructor(address daiAddress, address routerAddress) {
		daiToken = IERC20(daiAddress);
		router = IUniswapV2Router02(routerAddress);
	}

	// user has to approve spending first on the DAI smart contract
	function depositDai(uint amount) public onlyWhitelist {
		daiToken.transferFrom(_msgSender(), address(this), amount);
		setAddrBalance(_msgSender(), getAddrBalance(_msgSender()) + amount);
		emit DepositDai(_msgSender(), amount);
	}

	function swapRouter(uint buyAmount) private {
		require(daiToken.approve(address(router), buyAmount), "Approve failed.");
		address[] memory path = new address[](2);
		path[0] = address(daiToken);
		path[1] = router.WETH();
		router.swapExactTokensForTokens(buyAmount, 0, path, address(this), block.timestamp); // + 300 ?
	}
	...
	*/
	...
	// REDO THIS FUNCTION IN ORDER TO WITHDRAW DAI
	/*
	...
	*/
	...
	function finalizeRequestAndBuy(uint id) public onlyWhitelist onlyValidId(id) {
		...
		swapRouter(allRequests[id].buyAmount);
		// transfer with router
