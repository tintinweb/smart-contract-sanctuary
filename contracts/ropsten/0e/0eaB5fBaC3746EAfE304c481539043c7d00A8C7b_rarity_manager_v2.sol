...
contract rarity_manager_v2 is IERC721Receiver {
   ...
   // struct summoner_info {
    ...
    function class_list(uint _start,uint _count,uint unit) external view returns (uint[] memory _classes) { 
        if(unit==0) unit=1e18;
        ...
        if(_eof>_start){
            _count=_eof-_start; //更新count
            _classes=new uint[](_count);
            for (uint i = _start; i < _eof; i++) {
                _classes[i-_start]=uint32(_rm.class(summoners[i])/unit);
            }
        }
    }
    function level_list(uint _start,uint _count,uint unit) external view returns (uint[] memory _levels) { 
        if(unit==0) unit=1e18;
        ...
        if(_eof>_start){
            ...
            for (uint i = _start; i < _eof; i++) {
                _levels[i-_start]=uint32(_rm.level(summoners[i])/unit);
            }
        }
    }
    ...
    function xp_list(uint _start,uint _count,uint unit) external view returns (uint[] memory _xps) { 
        ...
        if(_eof>_start){
            ...
            for (uint i = _start; i < _eof; i++) {
                _xps[i-_start]=uint32(_rm.xp(summoners[i])/unit);
            }
        }
    }
    ...
    function gold_list(uint _start,uint _count,uint unit) external view returns (uint[] memory _gold_balances) { 
        ...
        if(_eof>_start){
            ...
            for (uint i = _start; i < _eof; i++) {
                _gold_balances[i-_start]=uint32(_gold.balanceOf(summoners[i])/unit);
            }
        }
    }

    function materials_1_list(uint _start,uint _count,uint unit) external view returns (uint[] memory _xps) { 
        if(unit==0) unit=1e18;
        ...
        if(_eof>_start){
            ...
            _xps=new uint[](_count);
            for (uint i = _start; i < _eof; i++) {
                _xps[i-_start]=uint32(_materials_1.balanceOf(summoners[i])/unit);
            }
        }
    }
    ...
    function _find_crafter_ready(uint8 _base_type, uint8 _item_type,uint _start) internal view returns(uint _summonerId,int _next,int _check,uint _cost,uint _dc) {
        ...
        for(uint _i=_start;_i<_len;_i++ ){
            ...
            bool _crafted;
            (_crafted,_check,_cost,_dc)= _crafting.simulate(_summoner, _base_type, _item_type, 0);
            if(_crafted && (_rm.xp(_summoner)>=250e18) && (_gold.balanceOf(_summoner)>= _cost) ){
                return (_summoner,int(_i+1),_check,_cost,_dc);
            }
        }
    ...
    function find_crafter_ready(uint8 _base_type, uint8 _item_type,uint _start) external view returns(uint _summonerId,int _next,int _check,uint _cost,uint _dc) {
