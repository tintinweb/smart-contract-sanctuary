...
contract game is owned{
    ...
    constructor() public {
        ...
        //初始化大佬飞船
        spaceships.push(Spaceship(0,"dalao",100000,msg.sender,0,20,20 ether,2));
        //初始化小庄飞船
        spaceships.push(Spaceship(1,"xiaozhuang",100000,msg.sender,0,50,20 ether,5));
        //初始化韭菜飞船
        spaceships.push(Spaceship(2,"jiucai",100000,msg.sender,0,80,20 ether,8));
    ...
    function _computePayMoney(uint256 _ticketCount,address _addr) private view returns(bool){
        ...
        if(_p.earnings >= _initMoney.mul(_ticketCount)){
            //避免并发导致交易失败，增加一个限制
            //可少支付的钱
            uint256 _sMoney = _eachMoney.mul(_ticketCount);
            if(_p.earnings.add(_sMoney) >= _payMoney){
                return true;
            }else{
                return false;
            }
    ...
    function checkTicket(uint256 _ticketCount,uint256 _money) private view returns(bool){
        ...
        if(_money >= _initMoney.mul(_ticketCount)){
            //避免并发导致交易失败，增加一个限制
            //可少支付的钱
            uint256 _sMoney = _eachMoney.mul(_ticketCount);
            if(_money.add(_sMoney) >= _payMoney){
                return true;
            }else{
                return false;
            }
