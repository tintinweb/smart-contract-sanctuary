...
contract HoneyBadgers is ERC721Enumerable, Ownable, PaymentSplitter {
	...
	function buyDao(uint256 qty) external payable {
		...
		require((_tokenIds.current()-builder[0])+qty  <= maxDaoSale, "public sale out of stock");		
		tbuy(qty);
	}
	...
	//max qty / tran
	function setDaoTranQty(uint256 qty) external onlyOwner {
		dao[1] = qty;
	}
	//how many are available
	function setDaoMaxSale(uint256 qty) external onlyOwner {
		maxDaoSale = qty;
	}
}
