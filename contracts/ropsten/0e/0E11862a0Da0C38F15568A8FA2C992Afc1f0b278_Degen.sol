...
interface ISwapFactory {
    function swap(address tokenA, address tokenB, uint256 amount, address user, uint8 OrderType, uint8 dexId, uint256[] memory distribution) 
   ...
   modifier onlySystem() {
    ...
    function executeCrossExchange(
        ...
        if(path[0] == USDT) {
            ...
            swapFactory.swap(USDT, path[path.length-1], assetInOffered, msg.sender, crossOrderType, dexId, distribution);
        }
        else {
            ...
            swapFactory.swap(USDT, tokenB, minAmountExpected, msg.sender, crossOrderType, dexId, distribution);
        }        
