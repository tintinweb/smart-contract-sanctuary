...
contract Perseverance is PerseverancePricing {
    ...
    DerivativeParameters params = DerivativeParameters(false, false, 0, 500, 5);
    ...
    // TBD
    uint32 preSaleLaunchTime = 1635379200;
    // TBD
    uint32 publicSaleLaunchTime = 1635381000;
    ...
    function mintWithN(address recipient, uint256[] calldata tokenIds, uint256 paid) public override virtual nonReentrant {
        uint256 maxTokensToMint = tokenIds.length;
        require(isPreSaleActive(), "Perseverance:PRE_SALE_NOT_ACTIVE");
        require(maxTokensToMint <= totalMintsAvailable(), "NilPass:MAX_ALLOCATION_REACHED");
        require(balanceOf(recipient) + maxTokensToMint <= params.maxMintAllowance, "NilPass:MINT_ABOVE_MAX_MINT_ALLOWANCE");
    ...
    function mint(address recipient, uint8 amount, uint256 paid) public override virtual nonReentrant {
        require(isPublicSaleActive(), "Perseverance:PUBLIC_SALE_NOT_ACTIVE");
        ...
        require(balanceOf(recipient) + amount <= MAX_PUBLIC_MINT, "NilPass:MINT_ABOVE_MAX_MINT_ALLOWANCE");
    ...
    function canMint(address account) public virtual override view returns (bool) {
        ...
        if(isPreSaleActive() && (totalMintsAvailable() > 0) && (nOwnerResolver.balanceOf(account) > 0) && (balance < params.maxMintAllowance)) {
            return true;
        }
