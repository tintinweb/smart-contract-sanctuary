...
contract Tokens {
    ...
    function tokenURI(uint256 _id) public view returns (string) {
        require(tokenExists[_id]);
        return tokens[_id]._tokenURI;
    }
    ...
    function tokenMetadata(uint256 _id) public view returns (string, string, string, string, address) {
        return (tokens[_id].name, tokens[_id].horseId, tokens[_id].dna, tokens[_id]._tokenURI, tokenOwners[_id]);
    }
    
    function createtoken(string _name, string _horseId, string _dna, string tmp_tokenURI) public returns (uint256) {
        uint256 tokenId = id;
        tokens[tokenId] = token(_name, _horseId, _dna, tmp_tokenURI);
