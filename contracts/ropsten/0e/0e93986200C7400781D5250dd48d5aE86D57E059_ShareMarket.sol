...
contract ShareMarket is MinterReceiver {
    ...
    function _hash(uint40 stakeId, address addr) private pure returns (bytes32) {
    ...
    function _buyShares(
        ...
        emit SharesUpdate(stakeId, msg.sender, uint256(uint72(sharesBalance)) | (uint256(uint72(heartsBalance)) << 72));
    ...
    function supplierWithdraw(uint40 stakeId) external lock {
        ...
        if (heartsEarned != 0) {
            ...
            if (sharesBalance != 0) {
                ...
                shareOwners[supplier].sharesOwned += uint72(sharesBalance);
