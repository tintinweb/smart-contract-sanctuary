...
interface IStreamingFeeModule {
  function feeStates(ISetToken _setToken)
    external
    view
    returns (StreamingFeeModule.FeeState memory);
...
contract ERC20Viewer {
  ...
  function batchFetchAllowances(
    ...
    // Cycle through contract addresses array and fetching the balance of each for the owner
    for (uint256 i = 0; i < addressesCount; i++) {
      allowances[i] = ERC20(address(_tokenAddresses[i])).allowance(
        _ownerAddresses[i],
        _spenderAddresses[i]
      );
    }
...
contract SetTokenViewer {
  ...
  function batchFetchManagers(ISetToken[] memory _setTokens)
    external
    view
  ...
  function batchFetchModuleStates(ISetToken[] memory _setTokens, address[] calldata _modules)
    public
    view
  ...
  function batchFetchDetails(ISetToken[] memory _setTokens, address[] calldata _moduleList)
    public
    view
  ...
  function getSetDetails(ISetToken _setToken, address[] calldata _moduleList)
    external
    view
    returns (SetDetails memory)
  {
