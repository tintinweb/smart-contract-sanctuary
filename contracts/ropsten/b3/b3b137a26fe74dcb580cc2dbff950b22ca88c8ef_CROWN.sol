   ...
   function distributeRewards() public onlyOwner {
        ...
        for (uint256 i = 0; i < stakeholders.length; i += 1) {
           ...
           if (reward > 0) {
               ...
               if (address(stableCoinAddress) == address(usdcAddress)) {
                   IERC20(stableCoinAddress).transfer(stakeholder, rewardStableCoin);
               } else {
                   IUSDT(stableCoinAddress).transfer(stakeholder, rewardStableCoin);
               }
               ...
               dividendList.push(Dividend(address(this), stakeholder, rewardStableCoin, tokenSymbol, block.timestamp));
               dividendHistory[stakeholder].push(Dividend(address(this), stakeholder, rewardStableCoin, tokenSymbol, block.timestamp));
               emit DividenPaid(stakeholder, reward, block.timestamp);
           }
        }
   }
   
   function withdrawReward(address stakeholder, string memory symbol) public validAddress(stakeholder) {
       require(startDate > 0 && endDate > 0, "Invalid stake end date!");
       require(block.timestamp >= endDate, "Please wait until the reward removal date.");
       require(rewards[stakeholder] > 0,"You don't have the reward to withdraw.");
       require(msg.sender == owner() || msg.sender == stakeholder, "Just an admin & stakeHolder can remove stake!");
       assert(keccak256(abi.encodePacked(symbol)) == keccak256(abi.encodePacked("USDC")) || keccak256(abi.encodePacked(symbol)) == keccak256(abi.encodePacked("USDT")));
       uint256 reward = rewards[stakeholder];
       uint256 dividend = dividends[stakeholder];
       string memory tokenSymbol = IERC20Metadata(usdtAddress).symbol();
       if (reward > 0) {
           uint256 rewardStableCoin = dividend;
           if (keccak256(abi.encodePacked(symbol)) == keccak256(abi.encodePacked("USDC"))) {
                IERC20(usdcAddress).transfer(stakeholder, rewardStableCoin);
                tokenSymbol = IERC20Metadata(usdcAddress).symbol();
           } else {
                IUSDT(usdtAddress).transfer(stakeholder, rewardStableCoin);
           }
           rewards[stakeholder] -= reward;
           dividends[stakeholder] -= dividend;
           removeStake(stakeholder,stakes[stakeholder]);
