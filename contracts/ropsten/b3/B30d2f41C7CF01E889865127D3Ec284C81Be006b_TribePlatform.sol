...
contract ERC20Metadata {
    ...
    string public tokenURIPrefix = &#39;https://backend.tribe.wtf/meta/tribe/&#39;;
 ...
 */
contract Tribe is StandardToken, OwnableChild, ERC20Metadata
{
    // ======================================================================================
    // ======================================================================================
    // ======================================================================================
    ...
    /*function setIpfsImage(string _ipfsHash) public onlyOwner returns(bool) {
    	ipfsHash = _ipfsHash;
    	return true;
    }*/
 ...
 * Token Factory
 */
contract TribePlatform is Ownable
{
    // TRIBES
    mapping(string => address) internal tribes;
    string[] public tribeSymbols;
    address public tribeProducts;

    /**
     * constructor
     * TODO: check if we need this. For protection / security we should always include a constructor, as far as I remember
     */
    function TribePlatform() public {
        //tribeProducts = address(new TribeProducts());
    }

    /**
     * update products contract address
     */
    function updateTribeProducts(address _tribeProducts) public onlyOwner{
        tribeProducts = _tribeProducts;
    }

    /**
     * create a new ERC-20 contract / Tribe Token
     * TODO: reinsert requirements (removed for testing) or make the contract killable and kill after testing
     */
    function newTribe(string _name, string _symbol) public returns(address) {
        // caller must be a real user //TODO: reactivate!!
        //if(msg.sender != tx.origin) revert();

        // unique symbol //TODO: reactivate!!
        //if(tribes[_symbol] != address(0x0)) revert();

        // create new tribe
        Tribe tribe = new Tribe(msg.sender, _name, _symbol, address(this));
        
        tribes[_symbol] = address(tribe);
        tribeSymbols.push(_symbol);
     ...
     * only by symbol
     */
    function getTribe(string _symbol) public view returns(address){
        return tribes[_symbol];
    }
}