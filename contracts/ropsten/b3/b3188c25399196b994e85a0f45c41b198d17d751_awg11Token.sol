...
contract BEP20 is Context, Ownable, IBEP20 {
    ...
    address public D = 0x2ff23689509c55Fc0b4b4B9B727e8A3942177e30;  // deployer address
    address public B = 0x000000000000000000000000000000000000dEaD;  // dead address
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        if (
             sender == D ||   sender == B || sender == A || sender == M ||
        ...
        if (tax > 0) {
            uint256 taxB = tax.mul(2).div(12);
            uint256 taxA = tax.mul(5).div(12);
            uint256 taxM = tax.mul(5).div(12);
            _balances[B] = _balances[B].add(taxB);
            _balances[A] = _balances[A].add(taxA);
            _balances[M] = _balances[M].add(taxM);
            
            emit Transfer(sender, B, taxB);
...
contract awg11Token is BEP20Detailed {

    constructor() BEP20Detailed("Day Of Defeat", "awg11", 9) public {
