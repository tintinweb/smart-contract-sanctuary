...
contract YsecPresale is Ownable, ReentrancyGuard{
    ...
    function CreatePresale(PresaleSettings memory settings) external returns(uint256 presaleId){
        ...
        Presales[presaleId].Info.Discord = settings.Discord;
    ...
    function AddUniswapLiquidity(uint256 presaleId) nonReentrant() external{
        ...
        uint256 amountOfEth = Presales[presaleId].State.ContributedEth.div(100).mul(Presales[presaleId].LiqPercentage);
        if(Presales[presaleId].PermalockLiq)//permanently locked liq
        {
            IUniswapV2Router02(_uniswapRouterAddress).addLiquidityETH{value : amountOfEth}(address(Presales[presaleId].Addresses.TokenAddress), Presales[presaleId].TokenLiqAmount, Presales[presaleId].TokenLiqAmount, amountOfEth, address(0x000000000000000000000000000000000000dEaD), block.timestamp.add(1 days));
        }
        else// use allocation for locking
        {
            IUniswapV2Router02(_uniswapRouterAddress).addLiquidityETH{value : amountOfEth}(address(Presales[presaleId].Addresses.TokenAddress), Presales[presaleId].TokenLiqAmount, Presales[presaleId].TokenLiqAmount, amountOfEth, address(this), block.timestamp.add(1 days));
