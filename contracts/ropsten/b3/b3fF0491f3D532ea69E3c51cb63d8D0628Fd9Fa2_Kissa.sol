    ...
    ContextMixin
{
    ...
    // Initially minted tokens.
    uint256 constant private _launchSupply = 0;
    ...
    address constant private mittenzToken = 0x900DcF4cC494cD44bb921e3E96c471c344C7108C;  //Ropsten
    // address constant private mittenzToken = 0xb73F00feEAFc232C247516AA180261fEc0E909fc;  //Ethereum Mainnet
    ...
    constructor(
        address mintableERC20PreicateProxy
    ) KissaBase(_launchSupply) {
        _setupContractId("Kissa");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(PREDICATE_ROLE, mintableERC20PreicateProxy);
     ...
     * @dev Locks Mittenz.
     */
    function exchangeMittenzForKissa(address user, uint256 amountInMittenz)
        external
    {
        // TEST/DEBUG on Ropsten
        ...
        uint256 mittenzAllowance = IERC20(mittenzToken).allowance(_msgSender(), address(this));
        ...
        IERC20(mittenzToken).safeTransferFrom(user, address(this), amountInMittenz);
        _mint(user, amountInKissa);
        _floatingReserve += amountInKissa;
        emit Exchange(user, mittenzToken, amountInMittenz, address(this), amountInKissa);
    }
    ...
    function exchangeKissaForMittenz(uint256 amountInKissa) external {
        ...
        IERC20(mittenzToken).safeTransfer(withdrawer, amountInMittenz);
        _floatingReserve -= amountInKissa;
        emit Exchange(withdrawer, address(this), amountInKissa, mittenzToken, amountInMittenz);
    }
...
contract KissaBase is 
    ...
    // Initially minted tokens.
    uint256 constant private _launchSupply = 4000000 * 10**18;

    string constant private _name = "KISSA";
    string constant private _symbol = "KISSA";

    /**
     * @dev Mints {launchSupply} tokens.
     */
    constructor (uint256 launchSupply) {
        _mint(msg.sender, launchSupply);
    }
