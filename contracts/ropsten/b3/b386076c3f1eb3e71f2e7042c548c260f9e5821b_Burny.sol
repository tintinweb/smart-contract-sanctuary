...
contract Burny is Context, IERC20, Ownable {
    ...
    event Burned (address indexed sender, uint amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_totalSupply <= burnDisabledAtAmount) {
            burnEnabled = false;
        }
        ...
        if (burnEnabled) {
            ...
            uint _burnAmount = amount.mul(100).div(15);
            uint _transferFee = _charityAmount.add(_burnAmount);
            uint _amountToTransfer = _amount.sub(_transferFee);
            
            _balances[sender] = senderBalance - _transferFee;
            _totalSupply -= _burnAmount;
            
            _balances[charityWallet] += _charityAmount;
            
            amount = _amountToTransfer;
            
            emit Burned (sender, _burnAmount);
        }
