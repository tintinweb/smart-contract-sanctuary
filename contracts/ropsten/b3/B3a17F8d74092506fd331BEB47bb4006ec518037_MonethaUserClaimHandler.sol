...
/**
 *  @title MonethaUserClaimStorage
 *
 *  MonethaUserClaimStorage is a storage contract. 
 *  It will be used by MonethaUserClaimHandler to update and delete user claim. 
 */
contract MonethaUserClaimStorage is Contactable {
...
/**
 *  @title MonethaUserClaimHandler
 *
 *  MonethaUserClaimHandler contract is a calling contract,
 *  which is used to update the storage contract MonethaUserClaimStorage.
 */
contract MonethaUserClaimHandler is Contactable {   
    MonethaUserClaimStorage public storageContract;

    event StorageContractOwnerChanged(address indexed _newOwner);

    constructor(address _storageAddr) public {
        storageContract = MonethaUserClaimStorage(_storageAddr);
    }

    /**
     *  claimTokens calls updateUserClaim function of MonethaUserClaimStorage contract to update user&#39;s token claim status and assign tokens to user.
    ...
    /**
     *  claimTokensInBulk calls updateUserClaim function of MonethaUserClaimStorage contract to update multiple users token claim status and assign tokens to user.
     */
    ...
    /**
     *  changeOwnerOfMonethaUserClaimStorage changes ownership
     *  @param _newOwner address of new owner
     */
    function changeOwnerOfMonethaUserClaimStorage(address _newOwner) external onlyOwner {
