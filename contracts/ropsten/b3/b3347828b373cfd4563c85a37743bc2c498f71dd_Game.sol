...
contract Game {
    ...
    //****************
    uint8 constant distributionForInvition = 20;
    uint8 constant distributionForInvition2 = 5;
    uint8 constant distributionForHolder = 25;
    ...
    function getCurrentRoundInfo() public view returns(uint256 _id,uint256 _totalJackpot,uint256 _jackpot4Winner,uint256 _startTime,uint256 _endTime,uint256 _myKeys,uint256 _myHoldEarnings,uint256 _playerNumber,uint256 _totalKeys){
        _id = totalRoundNumber;
        _totalJackpot = getBounsInRound(totalRoundNumber);
        _jackpot4Winner = _totalJackpot.mul(distributionForWinner).div(100);
        _startTime = roundInfoXRound[_id].startTime;
        _endTime = roundInfoXRound[totalRoundNumber].endTime;
        (_myKeys,_myHoldEarnings,) = getPlayerInfoInRound(totalRoundNumber);
        _totalKeys = roundInfoXRound[totalRoundNumber].keysNumber;
        _playerNumber = roundInfoXRound[totalRoundNumber].playerNumber;
        return(
            _id,
            _totalJackpot,
            _jackpot4Winner,
    ...
    function buyCore(address _playerAddress,address _inviteAddress,uint256 _amount) private {
        uint256 keybought = playerInfoXAddress[_playerAddress].propertyXRound[totalRoundNumber].keyCount;
        uint256 keyWillBuy = _amount.mul(1 ether).div(getBuyPrice());
        require(keybought.add(keyWillBuy) <= 100000 ether,"You have reached our purchase limit");
        require(_amount>=0.001 ether,"You need to pay 0.001 ether at least");
        ...
        playerInfoXAddress[_playerAddress].propertyXRound[totalRoundNumber].buyInfoXIndex[payIndex] = GameModel.BuyInfo(keysExist,_amount.mul(1 ether).div(getBuyPrice()));
