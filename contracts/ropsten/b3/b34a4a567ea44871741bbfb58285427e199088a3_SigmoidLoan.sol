...
contract SigmoidLoan {
    ...
    function createLoan(uint256 _auctionType, address _seller,uint256 _startingPrice,uint256 _endingPrice,uint256 _auctionDuration,uint256 _interestRate,uint256 _loanDuration,uint256 _assetsName,uint256 _assetsAmount) public {
    ...
    function hanldeRepay(uint256 _auctionTimestamp,address _seller) public {
    ...
    function getDEXInfo(uint256 _auctionType) view public returns(AUCTION[] memory targetAuction) {
        uint256 len=0;
        for (uint256 i = 0; i < auctionList.length; i++) {
        ...
        targetAuction.length - 1;
    }

    function getWalletInfo(uint256 _auctionType) view public returns(AUCTION[] memory walletAuction) {
        uint256 len=0;
        for (uint256 i = 0; i < myLoan.length; i++) {
            if(_auctionType == myLoan[i].auctionType){
                walletAuction[len] = myLoan[i];
                len++;
            }
        }
        walletAuction.length - 1;
    }
