...
contract Oasis{
	...
	*/
	...
	constructor() public {
	    ...
	    keyconfig.basekeysub = 500 ether;//500
		...
		*/
		...
		prizelevelmans = [4,7,10];//test
		prizelevelsunsday = [1,3,5];//test
	...
	/*
	function getddd(uint tid) public view returns(uint,uint,uint,uint) {
	    uint d = gettoday();
	    uint pp = tid - 1;
	    return(userlevelsnum[tid][d],allprize[pp][0],allprize[pp][1],gettruelevel(msg.sender,d));
	}*/
	...
	function setper(uint onceOuttimes,uint8 perss,uint runpers,uint pubpers,uint subpers,uint luckypers,uint lastpers,uint sellkeypers,uint sellpers,uint selluppers,uint lasttimes,uint sysdays,uint cksysdays) public {
	    require(msg.sender == owner);
