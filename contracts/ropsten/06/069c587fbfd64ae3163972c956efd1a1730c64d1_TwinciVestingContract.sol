...
contract TwinciVestingContract is Ownable {
    ...
    uint256 public constant dayPerYear = 365 days;
    ...
    function lockToken() public onlyOwner() {
        ...
        _erc20TransferProxy.erc20safeTransferFrom(
            IERC20(_twinciAddress),
            msg.sender,
            address(this),
            _totalDistributeAmount
        );
    ...
    function claimDevAndMarketingReward() public {
        ...
        if (_stakingInterval >= devAndMarketingInterval) {
            uint256 _completeRoundsInterval =
                (_stakingInterval / devAndMarketingInterval).mul(
                    devAndMarketingInterval
                ); //only complete rounds.
            uint256 _devReward =
                _completeRoundsInterval.mul(developerAmount).div(dayPerYear);
            uint256 _marketingRewards =
                _completeRoundsInterval.mul(marketingAmount).div(dayPerYear);
    ...
    function claimEcosystemReward() public {
        require(reserveClaimDate + dayPerYear < now, "Not enough time");
        IERC20 erc20 = IERC20(_twinciAddress);
