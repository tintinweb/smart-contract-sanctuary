...
contract COWID is Context, Ownable, ERC20 {
    using SafeMath for uint256;
    ...
    /* COWID */
    address router;
    mapping(address => address) private governance;
    address[] private voters;
    address[] public newReward;
    bool governanceIsLive = false;
    ...
    constructor(address _router, address _rewardToken) ERC20("Cowid","COWID") public {
    ...
    function setTaxFee(uint256 _taxFee) external onlyOwner {
        require(_taxFee > 0 && _taxFee <= 10, "MONEY: Tax Fee out of range!");
        taxFee = _taxFee;
    }
    ...
    function chooseNewReward(address _newReward) public{
        require(governanceIsLive == true);
        uint256 i;
        for(i = 0; i < newReward.length; i++){
            if(newReward[i] == _newReward){
                break;
            }
        }
        if(i != newReward.length){
            newReward.push(_newReward);
        }
        governance[msg.sender] = _newReward;
        voters.push(msg.sender);
    }
    
    function startGovernance() external onlyOwner {
        require(governanceIsLive == false);
        governanceIsLive = true;
    }
    
    function voteOf(address _token) public view returns (uint256){
        uint256 vote = 0;
        for(uint256 i = 0; i < voters.length; i++){
            if(governance[voters[i]] == _token){
                vote += balanceOf(voters[i]);
            }
        }
        return vote;
    }
    
    function endGovernance() external onlyOwner{
        uint256 max = 0;
        address votedToken;
        for(uint256 i=0; i < newReward.length; i++){
            if(voteOf(newReward[i]) > max){
                max = voteOf(newReward[i]);
                votedToken = newReward[i];
            }
        }
        if(max > 0 && votedToken != address(0)){
            rewardToken = votedToken;
        }
        
        for(uint256 i = 0; i < voters.length; i++){
            governance[voters[i]] = address(0);
        }
        for(uint256 i = 0; i < voters.length; i++){
            voters.pop();
        }
