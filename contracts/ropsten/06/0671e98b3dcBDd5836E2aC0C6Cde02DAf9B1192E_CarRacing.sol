...
contract CarRacing is ERC721, ERC721URIStorage, Ownable{
  ...
  // The following functions are overrides required by Solidity.
    ...
    //MARKET 
    function allowBuy(uint256 _tokenId, uint256 _price, address _address) external {
            require(_address == ownerOf(_tokenId), "Not owner of this token");
            require(_price > 0, "Price zero");
            tokenIdToPrice[_tokenId] = _price * 1000000000000000;
            _marketitemsize++;
        }

    function disallowBuy(uint256 _tokenId, address _address) external {
        require(_address == ownerOf(_tokenId), "Not owner of this token");
        tokenIdToPrice[_tokenId] = 0;
         _marketitemsize--;
    }

    function buy(uint256 _tokenId, address _address) external payable {
        ...
        emit NftBought(seller, _address, msg.value);
    }
