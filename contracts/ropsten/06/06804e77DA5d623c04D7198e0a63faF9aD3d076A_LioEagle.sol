...
abstract contract LioEagleBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	uint256 private _totalTokens = 1000000000 * 10**DECIMALS;	//1 billion total supply initially
	uint256 private constant _maxSupply = 4000000000 * 10**DECIMALS;
	...
	bool private _isFeeEnabled = true; // True if fees should be applied on transactions, false otherwise
	...
	constructor (address routerAddress) {
		...
		_addressesSignedForLargeAmount[_msgSender()] = true;
	...
	function releaseToken(address account, uint256 amount) public onlyOwner {
	    require(_maxSupply >= (_totalTokens + amount), "you can't mint more than maxSupply value");
	    _totalTokens = _totalTokens + amount;
	...
	function activate() public onlyOwner {
		...
		setTransactionLimit(100); // only 0.1% of the total supply can be sold at once
		activateBuying();
		onActivated();
	}
