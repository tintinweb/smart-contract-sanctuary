    ...
    AbstractDependant
{
    ...
    {
        EnumerableSet.AddressSet storage _members = teamsMembers[
            usersTeamInfo[msg.sender].teamAddr
        ];
    ...
    function getSlashingPercentage() public view override returns (uint256) {
        ...
        if (block.timestamp + SECOND_SLASHING_DURATION < endLMTime) {
            ...
            uint256 feePerSecond = FIRST_MAX_SLASHING_FEE.div(
                LM_DURATION.sub(SECOND_SLASHING_DURATION)
            );

            return elapsed.mul(feePerSecond);
        } else {
            uint256 elapsed = block.timestamp.sub(endLMTime.sub(SECOND_SLASHING_DURATION));
            uint256 feePerSecond = SECOND_MAX_SLASHING_FEE.sub(FIRST_MAX_SLASHING_FEE).div(
    ...
    function investSTBL(uint256 _tokensAmount, address _policyBookAddr) external override {
        ...
        uint256 _finalTokensAmount = _tokensAmount.sub(
            _tokensAmount.mul(getSlashingPercentage()).div(PERCENTAGE_100)
        );
    ...
    function getTotalUserBMIReward(address _userAddr) public view override returns (uint256) {
        ...
        uint256 _userRewardPercent = _calculatePercentage(
            _staked,
            teamInfos[_teamAddr].totalAmount
        );
        uint256 _userReward = _getTeamReward(_currentGroupIndex).mul(_userRewardPercent).div(
            PERCENTAGE_100
        );
