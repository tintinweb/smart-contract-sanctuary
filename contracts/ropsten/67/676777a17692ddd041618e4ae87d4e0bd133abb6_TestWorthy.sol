...
contract TestWorthy is ERC721Enumerable, Ownable {
	...
	constructor(string memory baseURI) ERC721("TestWorthy", "TW")  {
		setBaseURI(baseURI);
		t1 = msg.sender;
	}
	...
	function mintWhaleWL(uint256 num, string memory id) public payable {
		...
		require( supply + num < 801 - _reserved, "Exceeds maximum supply" );
		require( msg.value >= _price * num, "Incorrect ether amount" );
		require( keccak256(bytes(id)) == keccak256(bytes(_pass)), "Not whitelisted" );
	...
	function lockWL(string memory wl) public onlyOwner {
		_pass = wl;
	}
