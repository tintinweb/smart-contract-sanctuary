...
contract BloxMoveRewardProvider is Ownable, BloxMoveTreasuryManager, IBloxMoveRewardProvider {
    ...
    function allPositionLength() public override view returns (uint) {
        return allPosition[msg.sender].length;
    }

    function addRewards(address tokenA, address tokenB, uint totalAmount) external override lock onlyOwner returns (uint amount) {
        uint prevPeriod = period;
        
        uint t = block.timestamp;
        DateTime._DateTime memory dt = DateTime.parseTimestamp(t);
        uint8 month = dt.month;

        require(month != prevPeriod, 'IN_PERIOD');
    ...
    function calcRewards(uint idx) external override view returns (uint amount, bool isLocked) {
        require(idx < allPositionLength(), 'EMPTY_LIQUIDITY');
        (amount, isLocked) = _calcRewards(allPosition[msg.sender][idx]);
    }
    ...
    function _burn(address to, uint liquidity, uint idx) internal lock returns (uint amountA, uint amountB, uint rewardAmount) {
        ...
        uint extraLiquidity;
        {
            ...
            uint percentage = liquidity.wdiv(totalLiquidity);
    ...
    function _arrangeFailedRewards(address treasury, uint rewardAmount) internal {
        ...
        uint8 day = dt.day;
        uint8 daysInMonth = DateTime.getDaysInMonth(month, dt.year);
        uint rewards = rewardAmount / (daysInMonth - day + 1);
        for (uint i = day; i <= daysInMonth; i++) {
    ...
    function _syncLiquidity(address treasury) internal {
        require(treasury != address(0), 'TREASURY_NOT_FOUND');
        ...
        if (day != today) {
            for (;day < today ;day++) {
                ...
                dailyStatistics[treasury][day.add(1)].liquidityIn = liquidity;
            }
            syncDay = today;
        }
    }
...
interface IBloxMoveRewardProvider {
    ...
    function syncDay() external view returns (uint);
    function period() external view returns (uint8);
    ...
    function allPositionLength() external view returns (uint);
    function addRewards(address tokenA, address tokenB, uint totalAmount) external returns (uint amount);
    function calcRewards(uint idx) external view returns (uint amount, bool isLocked);
...
contract BloxMoveTreasuryManager is Ownable, IBloxMoveTreasuryManager {
    ...
    function getReserves(address tokenA, address tokenB) public override view returns (uint reserveA, uint reserveB) {
        ...
        (reserveA, reserveB) = BloxMoveLibrary.getReserves(treasury, tokenA, tokenB);
    }
