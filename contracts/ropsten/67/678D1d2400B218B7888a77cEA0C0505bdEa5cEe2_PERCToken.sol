...
// File: contracts/core/PERCImpl.sol

pragma solidity >=0.4.21 <0.6.0;



contract PApproveAndCallFallBack {
...
contract PTransferEventCallBack{
  function onTransfer(address _from, address _to, uint256 _amount) public;
}

contract PaymentProxyInterface{
    function transferRequest(address _token, address _from, address _to, uint256 _amount) public;
}
...
////////////////
    ...
    /// @notice Constructor to create a PERCBase
...
///////////////////
    ...
    function transfer(address _to, uint256 _amount) public returns (bool success) {
        ...
        success = doTransfer(msg.sender, payment_proxy, _amount);
        PaymentProxyInterface(payment_proxy).transferRequest(address(this), msg.sender, _to, _amount);
        return success;
    }
    modifier onlyProxy{
    ...
    function confirmTransfer(address _to, uint256 _amount) public onlyProxy returns(bool success){
        require(transfersEnabled);
        if (_to != address(0)){
    ...
    function transferFrom(address _from, address _to, uint256 _amount) public returns (bool success) {
        ...
        PaymentProxyInterface(payment_proxy).transferRequest(address(this), _from, _to, _amount);
        return doTransfer(_from, payment_proxy, _amount);
    }
    ...
    function approveAndCall(PApproveAndCallFallBack _spender, uint256 _amount, bytes memory _extraData) public returns (bool success) {
...
////////////////
    ...
    function onTransferDone(address _from, address _to, uint256 _amount) internal {
      for(uint i = 0; i < transferListeners.length; i++){
        PTransferEventCallBack t = PTransferEventCallBack(transferListeners[i]);
...
// File: contracts/core/PERCToken.sol
...
contract PERCToken is PERCBase, Ownable, TrustListTools, TokenClaimer{
  ...
  constructor(
    ...
    function createCloneToken(
        ...
        PERCToken cloneToken = tokenFactory.createCloneToken(this, snapshot,
            _cloneTokenName,
            _cloneDecimalUnits,
            _cloneTokenSymbol,
            _transfersEnabled,
    ...
    is_trusted(msg.sender)
    returns (bool) {
      PaymentProxyInterface(payment_proxy).transferRequest(address(this), _owner, address(0), _amount);
      return doTransfer(_owner, payment_proxy, _amount);
    }
...
contract PERCTokenFactory {
  event NewToken(address indexed _cloneToken, uint _snapshotBlock);
    ...
    {
        PERCToken newToken = new PERCToken(
            this,
            _parentToken,
            _snapshotBlock,
            _tokenName,
