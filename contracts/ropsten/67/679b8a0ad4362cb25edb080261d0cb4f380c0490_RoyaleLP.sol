...
contract RoyaleLP is RoyaleLPstorage,ReentrancyGuard {
  ...
  modifier validAmount(uint amount){
   ...
   modifier onlyAuthorized(address _caller) {
    ...
    function depsoitInRoyale(uint256[N_COINS] calldata amounts) external nonReentrant{
        for(uint8 i=0;i<N_COINS;i++){
            if(amounts[i]!=0){
    ...
    function deposit() onlyWallet() external nonReentrant{
    ...
    //Withdraw from Pool
    function withdraw() onlyWallet() external nonReentrant{
        uint8 counter = 0;
        for(uint8 i=0; i<N_COINS; i++) {
    ...
    function withdrawAll() public onlyWallet() nonReentrant{
    ...
    function rebalance() onlyWallet() external nonReentrant{
