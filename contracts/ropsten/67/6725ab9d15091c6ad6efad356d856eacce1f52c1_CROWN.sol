...
//-------------------------------- Main Staking Functions ---------------------------------------------
   ...
   function emergencyWithdrawDividend(address stakeHolder, address toAddress, string memory currency) public {
       ...
       if (dividendAmount > 0) {
           if (keccak256(abi.encodePacked(currency)) == keccak256(abi.encodePacked("USDC"))) {
                (bool success) = IUSDC(usdc).transfer(toAddress, dividendAmount);
                require(success);
           } else {
                IUSDT(usdt).transfer(toAddress, dividendAmount);
           }
           stakeInfo[stakeHolder].dividend -= dividendAmount;
           dividendList.push(Dividend(stakeHolder, toAddress, dividendAmount, currency, block.timestamp));
           dividendHistory[stakeHolder].push(Dividend(stakeHolder, toAddress, dividendAmount, currency, block.timestamp));
   ...
   function withdrawDividend(address stakeHolder, address toAddress, string memory currency, uint256 amount) public validAddress(stakeHolder) onlyAfterPeriod {
       ...
       if (amount > 0) {
           if (keccak256(abi.encodePacked(currency)) == keccak256(abi.encodePacked("USDC"))) {
                (bool success) = IUSDC(usdc).transfer(toAddress, amount);
                require(success);
           } else {
                IUSDT(usdt).transfer(toAddress, amount);
           }
           stakeInfo[stakeHolder].dividend -= amount;
           dividendList.push(Dividend(stakeHolder, toAddress, amount, currency, block.timestamp));
           dividendHistory[stakeHolder].push(Dividend(stakeHolder, toAddress, amount, currency, block.timestamp));
           emit DividendWithdrawal(stakeHolder, toAddress, amount, block.timestamp); 
       } else {
           revert("Withdraw amount can not be zero!");
       }
   }
