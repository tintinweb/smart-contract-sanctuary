...
contract RPC20 is Context, IRPC20, Ownable {
    ...
    constructor(string memory name, string memory symbol) public {
		...
		_MaxSupply = 10000000000 * 10 ** uint256(18);   //Adjust here2
    }
...
contract RPC20TokenTEST is RPC20('Reward Point Coin TEST', 'RPC TEST') {      //Adjust here3
    ...
    external
    {
        ...
        require(signatory != address(0), "FlashDEX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FlashDEX::delegateBySig: invalid nonce");
        require(now <= expiry, "FlashDEX::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "FlashDEX::getPriorVotes: not yet determined");
    ...
    internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FlashDEXs (not scaled);
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "FlashDEX::_writeCheckpoint: block number exceeds 32 bits");
