...
contract Staking {
  ...
  uint256 constant public WITHDRAW_TIME_SILVER     = 45 ;   // variable constant to manage withdraw time lock up for silver
  uint256 constant public WITHDRAW_TIME_GOLD       = 90 ;   // variable constant to manage withdraw time lock up for gold
  uint256 constant public WITHDRAW_TIME_PLATINUM   = 135;  // variable constant to manage withdraw time lock up for platinum
  uint256 public TOKEN_REWARD_PERCENT_SILVER       = 10000000;      // variable constant to manage token reward percentage for silver
  uint256 public TOKEN_REWARD_PERCENT_GOLD         = 20000000;       // variable constant to manage token reward percentage for gold
  uint256 public TOKEN_REWARD_PERCENT_PLATINUM     = 30000000;      // variable constant to manage token reward percentage for platinum
  ...
  */
  ...
  function stakeToken(uint256 tokens, uint256 time) public tokenStakeCheck(tokens, time) returns(bool){
    ...
    if(time == PERIOD_SILVER){
        require(totalStakedTokenInSilverPool.add(tokens) <= TOKEN_POOL_CAP, "Pool Limit Reached");
        _tokentime = now + (time);
        ...
        require(totalStakedTokenInGoldPool.add(tokens) <= TOKEN_POOL_CAP, "Pool Limit Reached");
        ...
        require(totalStakedTokenInPlatinumPool.add(tokens) <= TOKEN_POOL_CAP, "Pool Limit Reached");
        _tokentime = now + (time * 1 days);
        _tokenStakingCount = _tokenStakingCount +1;
        _tokenTotalDays[_tokenStakingCount] = time;
        _tokenStakingAddress[_tokenStakingCount] = msg.sender;
  ...
  function withdrawStakedTokens(uint256 stakingId) public returns(bool) {
    ...
    if(_tokenTotalDays[stakingId] == PERIOD_SILVER){
          ...
          if(now >= _tokenStakingEndTime[stakingId]){
              ...
              totalStakedTokenInSilverPool = totalStakedTokenInSilverPool.sub(_usersTokens[stakingId]);
          } else {
          ...
          if(now >= _tokenStakingEndTime[stakingId]){
              ...
              icliq.transferFrom(address(this), msg.sender,_finalTokenStakeWithdraw[stakingId]);
              totalTokenStakesInContract = totalTokenStakesInContract.sub(_usersTokens[stakingId]);
              totalStakedTokenInGoldPool = totalStakedTokenInGoldPool.sub(_usersTokens[stakingId]);
          } else {
              _finalTokenStakeWithdraw[stakingId] = _usersTokens[stakingId].add(getTokenPenaltyDetailByStakingId(stakingId));
              icliq.transferFrom(address(this), msg.sender,_finalTokenStakeWithdraw[stakingId]);
              totalTokenStakesInContract = totalTokenStakesInContract.sub(_usersTokens[stakingId]);
          ...
          if(now >= _tokenStakingEndTime[stakingId]){
              ...
              icliq.transferFrom(address(this), msg.sender,_finalTokenStakeWithdraw[stakingId]);
              totalTokenStakesInContract = totalTokenStakesInContract.sub(_usersTokens[stakingId]);
              totalStakedTokenInPlatinumPool = totalStakedTokenInPlatinumPool.sub(_usersTokens[stakingId]);
          } else {
              _finalTokenStakeWithdraw[stakingId] = _usersTokens[stakingId].add(getTokenPenaltyDetailByStakingId(stakingId));
              icliq.transferFrom(address(this), msg.sender,_finalTokenStakeWithdraw[stakingId]);
              totalTokenStakesInContract = totalTokenStakesInContract.sub(_usersTokens[stakingId]);
