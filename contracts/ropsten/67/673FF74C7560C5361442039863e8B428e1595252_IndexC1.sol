...
contract IndexC1 is Ownable {
  ...
  function enterMarket() public payable {    
    ...
    //   uint[] memory returnedAmounts = uniswapRouter.getAmountsIn(tokenEtherAmount, path);
    // 
    //   emit SwapInit(tokenAddress, tokenEtherAmount, tokenEtherAmount, returnedAmounts);
  ...
  function exitMarket() public {
    ...
    for (uint i = 0; i < allocationBalances[msg.sender].length; i++) {
      ...
      try uniswapRouter.swapExactTokensForETH( 
        ...
        coinEtherV = coinEtherV.add(amountInExt);
        
        emit SwapSuccess(tokenAddress, amountInExt, tokenAmounts[0], tokenAmounts[1]);
      } catch Error(string memory _err) {
        emit SwapFailureString(tokenAddress, _err);
        continue;
      } catch (bytes memory _err) {
  ...
  function custodialExit(address recipient) public {
