...
/**
 *
 * NameChangeCar Contract (The native token of Hashcars)
 * @dev Extends standard ERC20 contract
 */
contract NameChangeCar is Context, IERC20 {
    using SafeMath for uint256;

    // Constants
    uint256 public SECONDS_IN_A_DAY = 86400;

    uint256 public constant INITIAL_ALLOTMENT = 1111 * (10 ** 18);
    ...
    /**
     * @dev When accumulated NCCs have last been claimed for a Hashcars index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(ICars(_carsAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < ICars(_carsAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    /**
     * @dev Accumulated NCC tokens for a Hashcar token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = ICars(_carsAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
     ...
     */
    function setCarsAddress(address carsAddress) public {
        require(_carsAddress == address(0), "Already set");
        
        _carsAddress = carsAddress;
    }
    
    /**
     * @dev Claim mints NCCs and supports multiple Hashcar token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < ICars(_carsAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(ICars(_carsAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
        ...
        require(totalClaimQty != 0, "No accumulated NCC");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the HAshCars contract. For better UX.
        if (msg.sender != _carsAddress) {
