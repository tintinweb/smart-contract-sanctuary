...
contract sOlympus is IsOHM, ERC20Permit {
    ...
    modifier onlyStakingContract() {
        require(msg.sender == stakingContract || msg.sender == stakingDistributor, "StakingContract:  call is not staking contract");
        _;
    }
    ...
    // do this last
    function initialize(address _stakingContract, address _stakingDistributor, address _treasury) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");

        require(_stakingContract != address(0), "Staking");
        stakingContract = _stakingContract;

        require(_stakingDistributor != address(0), "Staking");
        stakingDistributor = _stakingDistributor;

//        _gonBalances[stakingContract] = TOTAL_GONS;
        _gonBalances[stakingContract] = INITIAL_FRAGMENTS_SUPPLY;
    ...
    function transferStaking(address to, uint256 value) public onlyStakingContract returns (bool) {
        require(_gonBalances[msg.sender] >= value, "Debt: cannot transfer amount");
        //        uint256 gonValue = value.mul(_gonsPerFragment);

        _gonBalances[msg.sender] = _gonBalances[msg.sender].sub(value);
        _gonBalances[to] = _gonBalances[to].add(value);
        //_gonBalances[to] = _gonBalances[to].add(gonValue);
    ...
    function transferFromStaking(
        ...
        address to,
        uint256 value
    ) public onlyStakingContract returns (bool) {
        require(_allowedValue[from][msg.sender]  >= value, "Debt: cannot transfer amount");
        require(_gonBalances[from] >= value, "Debt: cannot transfer amount");
    ...
    function gonsForBalance(uint256 amount) public pure override returns (uint256) {
        //        return amount.mul(_gonsPerFragment);
        return amount;
    }

    function balanceForGons(uint256 gons) public pure override returns (uint256) {
