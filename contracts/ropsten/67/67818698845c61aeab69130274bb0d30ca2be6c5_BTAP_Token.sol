...
contract BTAP_Token is Context, IERC20, Ownable {
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev Tax fee
        if (_tokenTotal <= 50000000 *10**18 && _taxFee != 0) {
            transferAmount = transferAmount.sub(_taxFeeAfter50millionSupply);
            _reflectionTotal = _reflectionTotal.sub(taxFee.mul(rate));
            _taxFeeTotal = _taxFeeTotal.add(taxFee);
            emit RewardsDistributed(taxFee);
            
        } else if(_tokenTotal > 50000000 *10**18 && _taxFee != 0){
        ...
        //@dev liquidity fee
        if(_tokenTotal >= 50000000 *10**18 + burnFee && _burnFee != 0){
