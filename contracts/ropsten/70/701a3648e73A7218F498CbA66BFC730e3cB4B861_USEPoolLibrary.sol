...
// File: contracts\USE\Pools\USEPoolLibrary.sol
// License: MIT



library USEPoolLibrary {
    ...
    function calcMintFractionalUSE(MintFU_Params memory params) internal pure returns (uint256, uint256) {
        ...
        uint256 shares_dollar_value_d18;
        uint256 c_dollar_value_d18;
        // Scoping for stack concerns
        {    
            // USD amounts of the collateral and the FXS
            shares_dollar_value_d18 = params.shares_amount.mul(params.shares_price_usd).div(1e6);
    ...
    // Must be internal because of the struct
    function calcBuyBackShares(BuybackShares_Params memory params) internal pure returns (uint256) {
...
abstract contract USEPool is ContractGuard,AccessControl {
    using SafeMath for uint256;
    /* ========== STATE VARIABLES ========== */
    ...
    // Bonus rate on Shares minted during recollateralizeUSE(); 6 decimals of precision, set to 0.75% on genesis
    uint256 public bonus_rate = 7500;
    // Number of blocks to wait before being able to collectRedemption()
    uint256 public redemption_delay = 1;
    uint256 public global_use_supply_adj;
    // AccessControl Roles
    ...
    function updateOraclePrice() public { 
        ...
        _useDaiOracle.update();
        _useSharesOracle.update(); 
    }
    ...
    function mint1t1USE(uint256 collateral_amount, uint256 use_out_min) external onlyOneBlock notMintPaused { 
        ...
        (uint256 use_amount_d18) = USEPoolLibrary.calcMint1t1USE(
            getCollateralPrice(),
    ...
    function mintFractionalUSE(uint256 collateral_amount, uint256 shares_amount, uint256 use_out_min) external onlyOneBlock notMintPaused {
        ...
        uint256 collateral_amount_d18 = collateral_amount * (10 ** missing_decimals);
        USEPoolLibrary.MintFU_Params memory input_params = USEPoolLibrary.MintFU_Params(
            share_price,
            getCollateralPrice(),
            shares_amount,
            collateral_amount_d18,
        ...
        (uint256 mint_amount, uint256 shares_needed) = USEPoolLibrary.calcMintFractionalUSE(input_params);
    ...
    function redeem1t1USE(uint256 use_amount, uint256 COLLATERAL_out_min) external onlyOneBlock notRedeemPaused {
        ...
        (uint256 collateral_needed) = USEPoolLibrary.calcRedeem1t1USE(
            getCollateralPrice(),
    ...
    function recollateralizeUSE(uint256 collateral_amount, uint256 shares_out_min) external onlyOneBlock {
        ...
        uint256 global_collateral_ratio = USE.global_collateral_ratio();
        uint256 global_collat_value = USE.globalCollateralValue();
        (uint256 collateral_units, uint256 amount_to_recollat) = USEPoolLibrary.calcRecollateralizeUSEInner(
            collateral_amount_d18,
            getCollateralPrice(),
            global_collat_value,
            use_total_supply,
    ...
    function buyBackShares(uint256 shares_amount, uint256 COLLATERAL_out_min) external onlyOneBlock {
        ...
        updateOraclePrice();
        uint256 share_price = USE.share_price();
        USEPoolLibrary.BuybackShares_Params memory input_params = USEPoolLibrary.BuybackShares_Params(
            availableExcessCollatDV(),
            share_price,
            getCollateralPrice(),
            shares_amount
        );
        (uint256 collateral_equivalent_d18) = (USEPoolLibrary.calcBuyBackShares(input_params)).mul(uint(1e6).sub(buyback_tax)).div(1e6);
