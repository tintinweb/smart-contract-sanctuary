...
contract GraJekFactory{
    ...
    event RiderCreated(address riderAddress, uint _durationMin, uint distanceKm, uint _startPostal, uint _endPostal);

     function createRider(uint _durationMin, uint distanceKm, uint _startPostal, uint _endPostal) external{
       Rider rider = new Rider( _durationMin,  distanceKm,  _startPostal,  _endPostal, riders.length);
       riders.push(rider);
       emit RiderCreated(address(rider),  _durationMin,  distanceKm,  _startPostal,  _endPostal);
     }
...
contract Rider {
    ...
    constructor (uint _durationMin, uint distanceKm, uint _startPostal, uint _endPostal, uint _index) payable { 
