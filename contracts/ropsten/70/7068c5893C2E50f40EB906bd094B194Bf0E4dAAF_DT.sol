...
//    }
    ...
    function calculateSell(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        if(block.timestamp > trader[from].lastBuy.add(sellFees.level[uint8(FeeLevels.LEVEL5)].saleCoolDownTime)) {
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[uint8(FeeLevels.LEVEL1)].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[uint8(FeeLevels.LEVEL1)].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[uint8(FeeLevels.LEVEL2)].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[uint8(FeeLevels.LEVEL2)].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[uint8(FeeLevels.LEVEL3)].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[uint8(FeeLevels.LEVEL3)].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[uint8(FeeLevels.LEVEL4)].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[uint8(FeeLevels.LEVEL4)].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[uint8(FeeLevels.LEVEL5)].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[uint8(FeeLevels.LEVEL5)].saleCoolDownFee;
        }

        uint256 finalSaleFee = _minimumSizeFee;
        uint256 poolSize = amountInPool();
        if(amount < poolSize.mul(sellFees.level[uint8(FeeLevels.LEVEL5)].saleSizeLimitPercent).div(100)){
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(amount > poolSize.mul(sellFees.level[uint8(FeeLevels.LEVEL1)].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[uint8(FeeLevels.LEVEL1)].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[uint8(FeeLevels.LEVEL2)].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[uint8(FeeLevels.LEVEL2)].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[uint8(FeeLevels.LEVEL3)].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[uint8(FeeLevels.LEVEL3)].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[uint8(FeeLevels.LEVEL4)].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[uint8(FeeLevels.LEVEL4)].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[uint8(FeeLevels.LEVEL5)].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[uint8(FeeLevels.LEVEL5)].saleSizeLimitPrice;
        }
...
//    }
    ...
    function updateSellerFees(uint8 _level, uint256 upperTimeLimitInHours, uint256 timeLimitFeePercent, uint256 saleSizePercent, uint256 saleSizeFee ) public onlyOwner {
        ...
        FeeLibrary.SellFees memory oldSellFees = sellFees.level[uint8(level)];
        sellFees.level[uint8(level)].setTo(upperTimeLimitInHours * 1 hours, timeLimitFeePercent, saleSizePercent, saleSizeFee);
        emit UpdateSellerFees(oldSellFees, sellFees.level[uint8(level)]);
    }
