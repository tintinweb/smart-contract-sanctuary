...
contract SnailFront is Ownable {
   ...
   ISwapFactory public uniFactory;
    ...
    constructor(IUniswapV3CrossPoolOracle _pp, ISwapRouter _r,
        ...
        factory = _factory;
        uniFactory = ISwapFactory(ISwapRouter(router).factory());
    }
    ...
    // event Debug0(address indexed who);
    // event Debug1(string indexed who);
    // event Debug2(string indexed who);
    // event Debug3(address indexed who);
    // event Debug4(uint24 indexed who);

    function newVixPool(address _tok1) public {
        address uniPool = _tok1;
        // emit Debug4(defaultFee);
        require(uniPool != address(0), "up");
        require(snailPoolsTypeV[uniPool] == address(0), "p");
        // emit Debug0(uniPool);
        string memory name = concatenate(IERC20Metadata(dai).symbol(),
                                         IERC20Metadata(_tok1).symbol());
        // emit Debug1(name);
        string memory fullName = concatenate("SnailVix", name);
        // emit Debug2(fullName);
        address snailPool = ISnailPoolFactory(factory).newVixPool(fullName, dai);
        // emit Debug3(snailPool);
        snailPoolsTypeV[uniPool] == snailPool;
        emit NewVixPool(uniPool, snailPool);
    }

    function newCitadelPool(address _tok1) public {
        address uniPool = _tok1;
