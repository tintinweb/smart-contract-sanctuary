...
contract Chibis is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    event Minted(address indexed sender, uint256 startWith, uint256 times);
    ...
    function initialize(
        ...
        for (uint256 i; i < factions; i++) {
            factionPrice[i] = price;
            factionTotal[i] = total;
        }
    }
    ...
    function addFactions(uint256 amount, uint256 price, uint256 total) public onlyOwner {
        ...
        while (amount-- != 0) {
            factionPrice[totalFactions - amount] = price;
            factionTotal[totalFactions - amount] = total;
        }
    }

    function getFactionPrice(uint256 faction) view public returns (uint256) {
        return factionPrice[faction];
    }

    function getFactionTotal(uint256 faction) view public returns (uint256) {
        return factionTotal[faction];
    }
    ...
    function mint(uint256 faction, uint256 times) payable public {
        require(faction >= totalFactions, "Faction doesn't exist.");
        require(times > 0 && times <= 10, "Too many.");
        require(totalMinted + times <= factionTotal[faction], "Exceeds minting limit.");
        require(msg.value == times * factionPrice[faction], "value error, please check price.");
        payable(owner()).transfer(msg.value);
        emit Minted(_msgSender(), totalMinted + 1, times);
