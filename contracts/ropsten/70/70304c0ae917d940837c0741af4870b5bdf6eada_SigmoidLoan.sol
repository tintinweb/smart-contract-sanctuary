...
contract SigmoidLoan {
    ...
    function getWalletInfo() view public returns(AUCTION[] memory) {
        /* uint256 len=0;
        for (uint256 i = 0; i < auctionList.length; i++) {
        ...
        // return len;
        AUCTION[] memory walletAuction = new AUCTION[](len);
        uint256 f = 0;
            for (uint256 j = 0; j < myLoan.length; j++) {
                if(myLoan[j].auctionType == _auctionType2 && myLoan[j].seller == msg.sender && myLoan[j].auctionStatus == false ){
