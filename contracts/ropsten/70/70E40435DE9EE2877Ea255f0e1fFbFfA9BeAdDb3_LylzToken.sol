...
abstract contract ERC721BasicToken is ERC165, ERC721Basic, ERC721Metadata, ERC721Enumerable, ERC721Receiver, Pausable {
    ...
    bytes4 constant internal InterfaceSignature_ERC165 = 0x150b7a02;
    ...
    /* bytes4(keccak256('name()')) ^
     ...
     bytes4(keccak256('setApprovalForAll(address,uint256)')) ^
     bytes4(keccak256('isApprovedForAll(address,uint256)')) ^
...
contract LylzToken is ERC721BasicToken {
    ...
    // Token symbol
    string internal symbol_ = "LYLZ_TEST2";
    ...
    */
     ...
     */
    function createToken(string memory _metadataPath) public {
        require(msg.sender == owner);
        tokenIdCounter = tokenIdCounter.add(1);
        addTokenTo(msg.sender, tokenIdCounter);
        _setTokenURI(tokenIdCounter, _metadataPath);
    }

    /**
       * public function to create token and transfer to user
       * @param _metadataPath string the url metadata path
       */
    function createTokenToAddress(address _owner, string memory _metadataPath) public {
        require(msg.sender == owner);
     ...
     * @param _metadataPath string the url metadata path
     */
    function createMultipleToken(string[] memory _metadataPath) public {
        require(msg.sender == owner);
        for (uint32 i = 0; i < _metadataPath.length; i++) {
            createToken(_metadataPath[i]);
        }
    }
