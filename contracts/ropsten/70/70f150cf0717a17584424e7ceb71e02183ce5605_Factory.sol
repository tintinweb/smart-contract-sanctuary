...
contract Factory is ERC20("BXFactory", "bWETH"), Ownable {
    ...
    function calculateRewards(address _address, uint256 _apy) public view returns (uint256) {
        ...
        if (user.amount > 0 && user.lastStakeTime > 0) {
            rewards = (user.amount*(_apy/100)/(365*24*60*60)*(block.timestamp - user.lastStakeTime)).div(1e18);
        }

        return rewards;
    }

    function calculateRewardsTest(address _address, uint256 _apy) public view returns (uint256) {
        UserInfo storage user = userInfo[_address];
        uint256 rewards = 0;
