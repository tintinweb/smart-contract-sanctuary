...
contract Exchange is Minter, IERC1155TokenReceiver {
    ...
    /**
    @dev Only astropenWallet (the owner) should call this function after getting aproval from creator.
    That helps to reduce complexity for creator when creating an order. Fee for this transaction will be backed up by creator fund
     */
    function createSellOrder(
        ...
        require(
            astropenNFT.balanceOf(_seller, _tokenId) >= _amount,
            "Token reserve is not enough"
        );
        require(
            creatorFund.fundOf(_seller) >= createSellOrderFee,
            "Creator fund is not enough to pay fee"
        );
        FixedPriceOrder memory order =
            FixedPriceOrder(_seller, _tokenId, _price, _amount);
        currentOrderId++;
        fixedPriceOrders[currentOrderId] = order;
        creatorFund.useFund(_seller, msg.sender, createSellOrderFee);
...
import "./CreatorFund.sol";

contract Minter is Ownable {
    ERC1155Tradable public astropenNFT;
    CreatorFund public creatorFund;
    IERC20 public paymentToken;

    /**
    @dev mintFee should be updated later
     */
    ...
    constructor(address _paymentToken) {
        ...
        // This contract is the owner of CreatorFund, so it restricts access to creator funds
        creatorFund = new CreatorFund(_paymentToken);
    }

    /** @dev Only astropenWallet can call this function and pay the gas fee, 
    but _initialOwner is the one who receives tokens. A mint fee will be taken from the creatorFund to make up for that gas fee.
    _initialOwner needs to make a call to setApproveForAll later to delegate the right to use tokens to this contract so that
    other funcionalities can work. 
    */
    function mint(
        ...
        creatorFund.useFund(_initialOwner, msg.sender, mintFee);
        astropenNFT.create(_initialOwner, _initialSupply, _uri, _data);
    }
}
