...
contract RandomToken is ERC20UpgradeSafe, OwnableUpgradeSafe {
    ...
    function initialize() public initializer {
        __ERC20_init("RandomToken", "RANDOM");
    ...
    function setTransferFeePercent(uint256 tFeePercent) external onlyOwner {
        require(
            now >= (_tFeeTimestamp + 12 hours),
            "Transfer fee changes timelocked for 12 hours"
        );
    ...
    function rebase(int256 supplyDelta) external returns (uint256) {
        require(
            _msgSender() == owner() || _msgSender() == _rebaser,
        ...
        uint256 uSupplyDelta = (supplyDelta < 0 ? -supplyDelta : supplyDelta)
            .toUint256();
    ...
    function refractionFromToken(uint256 tAmount, bool deductTransferFee)
        public
        view
        returns (uint256)
    {
    ...
    function tokenFromRefraction(uint256 rAmount)
        public
        view
        returns (uint256)
    {
        require(
            rAmount <= _rTotal,
            "Amount must be less than total refractions"
        );
    ...
    function _transferStandard(
        ...
        (
            ...
            uint256 tFee
        ) = _getValues(tAmount, tFeePercent);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount, uint256 tFeePercent)
        private
        view
        returns (
            uint256,
    ...
    function _getTValues(uint256 tAmount, uint256 tFeePercent)
        private
        pure
