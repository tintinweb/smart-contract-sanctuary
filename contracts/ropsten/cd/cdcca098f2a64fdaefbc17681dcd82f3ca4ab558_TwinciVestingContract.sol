...
contract TwinciVestingContract is Ownable {
    ...
    uint256 public constant devAndMarketingInterval = 3 minutes;
    uint256 public constant ecosystemInterval = 65 minutes;
    uint256 public constant dayPerYear = 35;
    ...
    uint256 public ecosystemClaimedAmount;
    ...
    modifier onlyInvestor() {
        require(_investor == msg.sender, "Investor: caller is not the investor");
        _;
    }
    ...
    /**
     *  Get Developer and Marketing completed round for claiming reward
     */
    function getCompleteRound() public view returns (uint256){
         uint256 stakingInterval = now.sub(developerAndMarketingClaimDate);
        return stakingInterval.div(devAndMarketingInterval);
    }
    
     /**
     *  Anyone can call this method to claim tokens for developer and marketing
     */
    function claimDevAndMarketingReward() public {
        require(developerAndMarketingClaimDate + devAndMarketingInterval <= now, "Wait for the next round");
        uint256 stakingInterval = now.sub(developerAndMarketingClaimDate);
        IERC20 erc20 = IERC20(_twinciAddress);
        require(stakingInterval >= devAndMarketingInterval, "Insufficient time.");
        uint256 completeRoundsInterval = stakingInterval.div(devAndMarketingInterval); //only complete rounds.
        uint256 devReward = completeRoundsInterval.mul(developerAmount).div(dayPerYear);
        uint256 marketingRewards = completeRoundsInterval.mul(marketingAmount).div(dayPerYear);

        if(devReward > developerAmount - devClaimedAmount) {
            // remaining tokens due to indivisible to 365
            devReward = developerAmount - devClaimedAmount;
        }
        
        if(marketingRewards > marketingRewards - martketingClaimedAmount) {
            // remaining tokens due to indivisible to 365
            marketingRewards = marketingRewards - martketingClaimedAmount;
        }
        if (devReward > 0 && devClaimedAmount + devReward <= developerAmount) {
            erc20.approve(address(_erc20TransferProxy), devReward);
            _erc20TransferProxy.erc20safeTransferFrom(erc20, address(this),_developerAddress, devReward);
            devClaimedAmount += devReward;
            emit Claimed(_twinciAddress, _developerAddress, devReward);
        }

        if (marketingRewards > 0 && martketingClaimedAmount + marketingRewards <= marketingAmount) {
            erc20.approve(address(_erc20TransferProxy), marketingRewards);
            _erc20TransferProxy.erc20safeTransferFrom(erc20, address(this), _marketingAddress, marketingRewards);
            martketingClaimedAmount += marketingRewards;
            emit Claimed(_twinciAddress, _marketingAddress, marketingRewards);
        }
        developerAndMarketingClaimDate = now;
    ...
    /**
     *  Anyone can call this method to claim tokens for ecosystem
     */
    function claimEcosystemReward() public {
        require(reserveClaimDate + ecosystemInterval <= now, "Insufficient time.");
        require(ecosystemClaimedAmount == 0, "Already claimed: ecosystem");
        IERC20 erc20 = IERC20(_twinciAddress);
        ...
        reserveClaimDate = now;
        ecosystemClaimedAmount = ecosystemReserveAmount;
        emit Claimed(
            _twinciAddress,
