...
contract PlasmaMgr {
    ...
    function submitBlockHeaders(bytes headers) public {
        for (uint256 i = 0; i < headers.length/blockHeaderLength; i++) {
            submitBlockHeader(headers,i*blockHeaderLength);
        }
    }

	function submitBlockHeader(bytes header,uint offset) public returns (bool success) {
        ...
        bytes1 sigV;
        assembly {
            let currentdata := add(header,offset)
            let data := add(currentdata,0x20)
