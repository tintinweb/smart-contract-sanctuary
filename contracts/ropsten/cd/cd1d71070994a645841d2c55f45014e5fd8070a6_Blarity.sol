...
contract Blarity {
  ...
  function donateToken(KyberNetworkInterface network, ERC20 src, uint srcAmount, uint maxDestAmount, uint minConversionRate, address walletId) payable public {
    bytes memory hint;
    uint amount = network.tradeWithHint(msg.sender, src, srcAmount, ACCEPT_DAI_ADDRESS, address(this), maxDestAmount, minConversionRate, walletId, hint);
    for(uint i = 0; i < donators.length; i++) {
      if (donators[i].addr == msg.sender) {
        donators[i].amount += amount;
        Donated(msg.sender, amount);
