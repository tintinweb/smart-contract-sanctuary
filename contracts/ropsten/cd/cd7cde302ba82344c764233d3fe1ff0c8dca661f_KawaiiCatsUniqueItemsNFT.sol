...
contract KawaiiCatsUniqueItemsNFT is Ownable, ERC721, ERC721Enumerable {
    ...
    uint256 public CUSTOM_ANIM_PRICE = 50000000000000000; //0.05 ETH
     ...
     */
    function requestCustomOutfit (string memory _picURL, uint16 _assignedNFTId, string memory _description, bool _animation) public payable {
        ...
        requestsArray.push(CustomRequest(_picURL, uint32(block.timestamp), _description, _animation, msg.sender, _assignedNFTId, getCustomPrice(_animation), false)); 
        queue ++;
    } 
     ...
     */
    function requestCustomOutfitPURR (string memory _picURL, uint16 _assignedNFTId, string memory _description, bool _animation) public {
        require(nftInterface.balanceOf(msg.sender) > 0, "Must own at least a kawaii cat");
        require(requestCustomPurr == true, "Function inactive"); 
        require(token.getBalanceOf(msg.sender) > CUSTOM_PURR_PRICE, "Not enough PURR");
        
        requestsArray.push(CustomRequest(_picURL, uint32(block.timestamp), _description, _animation, msg.sender, _assignedNFTId, 0, false)); 
