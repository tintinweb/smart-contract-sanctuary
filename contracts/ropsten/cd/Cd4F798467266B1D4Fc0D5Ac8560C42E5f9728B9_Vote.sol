...
contract Vote is Ownable {
    ...
    function create(uint256 expirationDate, string memory title, string memory description) external onlyOwner {
        ...
        if(counter > 0) {
            require(getProposalStatus(counter) == ProposalStatus.CLOSED, "");
        }
    ...
    function voting(Choice _choice) external {
        require(getProposalStatus(counter) == ProposalStatus.OPEN, "");
        ...
        require(votes[counter][msg.sender].count == 0, "");
        require(counter > 0, "");
    ...
    function closeProposal() external view onlyOwner {
        proposals[counter].adminStatus == AdminStatus.CLOSED;
    }
    
    function getActive() public view returns(uint256, string memory, string memory, uint256, uint256, uint256, ProposalStatus) {
        return getItem(counter);
    }
    
    function getHistory(uint256 index) public view returns(uint256, string memory, string memory, uint256, uint256, uint256, ProposalStatus) {
        require(index > 0, "");
