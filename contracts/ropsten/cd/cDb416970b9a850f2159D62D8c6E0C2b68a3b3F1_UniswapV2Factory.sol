...
{   
	...
	function SetCaller(address _address, bool _enabled) external;
}
...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function initialize(address _token0, address _token1) external {
		...
		name = string(abi.encodePacked(IERC20(_token0).name()," - ",IERC20(_token1).name()," Hero LP"));
		symbol = name;
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private {
		...
		//uint liquidity = uint(_reserve0).mul(_reserve1).sub(MINIMUM_LIQUIDITY);
		uint liquidity = 0;
		if(totalSupply == 0) liquidity = Math.sqrt(amount0.mul(amount1)).sub(MINIMUM_LIQUIDITY);
		else liquidity = Math.min(amount0.mul(totalSupply) / _reserve0, amount1.mul(totalSupply) / _reserve1);
		
		uint feeAmount = IDistributor(feeTo).CalculateFee(liquidity);
		if(feeAmount == 0)
			return;

		IDistributor(feeTo).OnFee(address(this), feeAmount);
...
contract UniswapV2Factory is IUniswapV2Factory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
		...
		IDistributor(feeTo).SetCaller(pair, true);
