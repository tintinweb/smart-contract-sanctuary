...
contract TESTSER is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x029bC8dc9B872fa3003c6AA92De93224b0911246); // Marketing Address
    ...
    string private _name = "TESTSER";
    string private _symbol = "TSER";
    ...
    uint256 public _maxWallet = 1500 * 10**9;
    ...
    function _transfer(
        ...
        if(!_isFree[from]) {
            require(balanceOf(from) < _maxWallet);
        }
        ...
        if (to == uniswapV2Pair && balanceOf(uniswapV2Pair) > 0) {
            ...
            sellHistory.mvrAmount = _getSellmvrAmount(amount);
                    ...
                    if (_isAutoBuyBack) {
                        ...
                        for (uint i = 0; i < _sellHistories.length; i ++) {
                            ...
                            if (_sellHistories[i].time >= startTime) {
                                summvrAmount = summvrAmount.add(_sellHistories[i].mvrAmount);
                                cnt = cnt + 1;
                            }
                        }

                        if (cnt > 0 && _buyBackDivisor > 0) {
                            _bBSLimitMax = summvrAmount.div(cnt).div(_buyBackDivisor);
                        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 percentToken = contractTokenBalance/100;
        uint256 tokenToLiq = percentToken * _liquidityFee;
        swapTokensForEth(contractTokenBalance-tokenToLiq-10);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        uint256 percentBalance = transferredBalance/100;
        uint256 liqAmount = percentBalance * _liquidityFee;
        addLiquidity(liqAmount, tokenToLiq);
        uint256 marketingTax = (transferredBalance.mul(marketingDivisor).div(100))-liqAmount;
        if (marketingTax >= address(this).balance) {
            marketingTax = address(this).balance;
        }
        // Send to Marketing address
        transferToAddressETH(marketingAddress, marketingTax);
    ...
    function _getSellmvrAmount(uint256 tokenAmount) private view returns(uint256) {
        address[] memory path = new address[](2);
    ...
    function _removeOldSellHistories() private {
        ...
        for (uint256 j = 0; j < _sellHistories.length; j ++) {
            ...
            if (_sellHistories[j].time >= maxStartTimeForHistories) {
                ...
                _sellHistories[i].mvrAmount = _sellHistories[j].mvrAmount;
