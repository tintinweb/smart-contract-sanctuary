...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "LOOP: caller is not the owner");
        _;
    }
     ...
     * - The contract must not be paused.
     */
    function pause() internal onlyOwner() virtual whenNotPaused {
        _paused = true;
     ...
     * - The contract must be paused.
     */
    function unpause() internal onlyOwner() virtual whenPaused {
        _paused = false;
...
contract LOOP is Context, IERC20, Ownable {
    ...
    uint256 private constant _currentSupply = MAX / 1.987654321e56; // initialize uint256 to 10^20
    uint256 private constant _totalSupply = 1e12 * 1e8; // total supply
    uint256 private _rTotal = (MAX - (MAX % _currentSupply));
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, address _Owner, uint256 _CurrentSupply) {
        ...
        _rOwned[_Owner] = _rOwned[_Owner].add(_CurrentSupply.mul(currentRate));
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) private view returns(uint256) {
        ...
        require(tAmount <= _currentSupply, "Amount must be less than supply");
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        uint256 tSupply = _currentSupply;      
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_rTotal, _currentSupply);
            rSupply = rSupply.sub(_rOwned[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        
        if (rSupply < _rTotal.div(_currentSupply)) return (_rTotal, _currentSupply);
