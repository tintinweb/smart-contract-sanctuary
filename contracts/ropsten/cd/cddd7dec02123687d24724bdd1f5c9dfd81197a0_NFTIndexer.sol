...
contract NFTIndexer {
    ...
    //IERC20 constant public immuable NFT = IERC20(address(0xcB8d1260F9c92A3A545d409466280fFdD7AF7042));
    IERC20 public immutable NFT;

    // Propose a new instruction
    event ContractProposed(address proposer, address contractAddress, string uri);

    // DAO approves an instruction into the index
    event ContractApproved(string checksum, address contractAddress, string uri);

    // TODO: reject proposal (because it's spam) and keep the fee
    event ContractRejected(address proposer, address contractAddress, string uri);

    // TODO: deferr proposal (because it was voted against) and return fee to proposer
    event ContractDeferred(address proposer, address contractAddress, string uri);

    // TODO: remove proposal (because it's spam) and keep the fee
    event ContractRemoved(address proposer, address contractAddress);
    ...
    function approveContract(address contractAddress, string memory uri, string memory checksum) public {
        require(msg.sender == multisig, "Only multisig can approve proposals");
        emit ContractApproved(checksum, contractAddress, uri);
    }

    function proposeContract(address contractAddress, string memory uri) public {
        ...
        emit ContractProposed(msg.sender, contractAddress, uri);
    }

    function rejectContract(address contractAddress, string memory uri) public {
        require(msg.sender == multisig, "Only multisig can reject a proposal");
        // TODO: increment a counter of deposited tokens kept
        emit ContractRejected(msg.sender, contractAddress, uri);
    }

    function deferContract(address contractAddress, string memory uri) public {
        require(msg.sender == multisig, "Only multisig can defer a proposal");
        // TODO: return proposer's deposit
        emit ContractDeferred(proposers[contractAddress][uri], contractAddress, uri);
    }

    function removeContract(address contractAddress) public {
        require(msg.sender == multisig, "Only multisig can reject a proposal");
        // TODO: increment a counter of deposited tokens kept
        emit ContractRemoved(msg.sender, contractAddress);
    }
