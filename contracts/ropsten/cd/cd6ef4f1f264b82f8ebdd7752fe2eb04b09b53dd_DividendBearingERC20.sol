...
contract DividendBearingERC20 is ERC20 {
    ...
    function _transfer(
        ...
        if (sender == _donation) {
            _dividendsClaimed[recipient] += (amount * _getRewardsForAccount(sender))/(balanceOf(sender));
        }
    ...
    /**
     * @dev burns from _donation wallet.
     */
    function burn(uint256 amount) internal virtual {
        _burn(_donation, amount);
    }
    ...
    function donate(uint256 amount, address donationAddress) internal virtual {
        _transfer(_donation, donationAddress, amount);
    }
