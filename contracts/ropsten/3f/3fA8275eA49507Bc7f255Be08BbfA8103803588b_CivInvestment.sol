...
contract CivInvestment is ICivInvestment
{
    ...
    function newTrade(
        ...
        uint160 minPrice = - limitPrice * 10 / 9;
        uint160 maxPrice = - limitPrice * 11 / 9;
        int24 lowerTick = TickMath.getTickAtSqrtRatio(minPrice);
        int24 upperTick = TickMath.getTickAtSqrtRatio(maxPrice);

        pool.mint(msg.sender, lowerTick, upperTick, amount0, new bytes(0));

        emit NewTradeOpened(pool.token0(), pool.token1(), fee, 0);
        // return pool.token0();
    }
...
interface ICivInvestment {
    ...
    */
    ...
    event NewTradeOpened(address token0, address token1, uint256 fee, uint256 nftId);
