...
contract Carchain is ERC721PresetMinterPauserAutoId {
    ...
    event CarCreated(uint256 indexed _id, address indexed sender, string hash, string make, string model, uint256 year, string vin, string engine, string colour, string plate, uint256 km);
    event KMUpdated(uint256 indexed _id, address indexed sender, uint256 km, string hash);
    ...
    function mintAndSet(address _to, string memory _hash, string memory _make, string memory _model, uint256 _year, string memory _vin, string memory _engine, string memory _colour, string memory _plate, uint256 _km) public {
        ...
        emit CarCreated(id, _msgSender(), _hash, _make, _model, _year, _vin, _engine, _colour, _plate, _km);
    }
    ...
    function updateKm(uint256 _id, uint256 _km, string memory _hash) public onlyOwner(_id) {
        require(metadata[_id].km < _km, "Invalid number");
        
        metadata[_id].km = _km;
        metadata[_id].hash = _hash;

        emit KMUpdated(_id, _msgSender(), _km, _hash);
    }
