...
abstract contract PressChild {
     ...
     */
    function deposit(uint256 _pid, uint256 _amount, address _user) external virtual;
     ...
     */
    function withdraw(uint256 _pid, uint256 _amount, address _user) external virtual;
}


/******************************************/
/*       PressMaster starts here          */
/******************************************/

pragma solidity ^0.6.12;

contract PressMaster is Ownable {
    ...
    PressChild[] public pressChildren;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pressChildArrayLength = pressChildren.length;
        for (uint256 i = 0; i < pressChildArrayLength; i++) 
        {
            if (address(pressChildren[i]) != address(0)) {
                pressChildren[i].deposit(_pid, _amount, msg.sender);
            }           
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        {
            if (address(pressChildren[i]) != address(0)) {
                pressChildren[i].withdraw(_pid, _amount, msg.sender);
            }           
        }
    ...
    function transfer(address _to, uint256 _value) external onlyOwner returns (bool) {
        return MARK.transfer(_to, _value);
    }
     ...
     */
    function setMarkPerBlock(uint256 _markPerBlock) external onlyOwner returns (uint256) {
    ...
    /**
     * @dev Add a press child. 
     * @param _pressChild Address of the press child contract.
     */
    function addPressChild(address _pressChild) external onlyOwner {
        pressChildren.push(PressChild(_pressChild));
    }

    /**
     * @dev Remove a press child. 
     * @param _index Index of the press child contract.
     */
    function removePressChild (uint256 _index) public onlyOwner {
        delete pressChildren[_index];
    }
