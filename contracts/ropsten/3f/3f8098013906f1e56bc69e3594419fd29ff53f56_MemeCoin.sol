...
contract MemeCoin is Context, IERC20, Ownable {
    ...
    event Balances(uint256 currTotalBalance, uint256 accBalance);
    event Rewards(uint256 reward, address accAddress);
    event TotalRewardBalance(uint256 balance, address accAddress);
        ...
        for (uint i=0 ; i < releaseTimeList.length ; i++){
            ...
            if (tokenlock.accounts.length != 0){
                ...
                for (uint j=0; j < tokenlock.accounts.length; j++) {
                    ...
                    emit Balances(currentTotalBalance, balanceOf(tokenlock.accounts[j]));
                }
                ...
                for (uint j=0; j< tokenlock.accounts.length; j++) {
                    ...
                    emit TotalRewardBalance(currentTotalBalance, tokenlock.accounts[j]);
                    uint256 reward = (balanceOf(tokenlock.accounts[j]).mul(10**2)).div(currentTotalBalance);
                    emit Rewards(reward, tokenlock.accounts[j]);
