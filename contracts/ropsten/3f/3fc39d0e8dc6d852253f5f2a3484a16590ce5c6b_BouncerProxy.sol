...
contract BouncerProxy {
  ...
  function returnAllRSV(bytes _signature)public view returns(bytes32,bytes32,uint){
    bytes32 r;
    bytes32 s;
    uint8 v;
    assembly {
  ...
  function retrurnAddress(bytes _signature,bytes32 _hash)public view returns(address){
    bytes32 r;
    bytes32 s;
    uint8 v;
    assembly {
    ...
    address abc = ecrecover(keccak256(
        abi.encodePacked("\x19Ethereum Signed Message:\n32", _hash)
      ), v, r, s);
      return abc;
  }
  
  function returnA(bytes32 hash) public view returns(bytes32){
    bytes32 abc =  keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash));
    return abc;   
  }
