...
contract PiccoloInu is Context, IERC20, Ownable {
    ...
    //Penalty Fee
    uint256 private _penaltyFee = 30;

    //Penalty Fee Time
    uint256 private _penaltyFeeTime = 86400;
    
    mapping(address => bool) public bots;
    mapping (address => bool) public preTrader;
    mapping(address => uint256) private cooldown;
    
    address payable private _marketingAddress = payable(0xB13F833c4E7aB8AC3b9ce126f126F5DE8Ac02d8A);
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                if (_buyMap[from] != 0 && (_buyMap[from] + _penaltyFeeTime >= block.timestamp)) {
    ...
    // Set Penalty Fee
    function setPenaltyFee(uint256 fee) public onlyOwner {
        _penaltyFee = fee;
    }

    // Get Penalty Fee
    function getPenaltyFee() public view onlyOwner returns (uint256) {
        return _penaltyFee;
    }
    ...
    // Get Penalty Fee Time
    function getPenaltyFeeTime() public view onlyOwner returns (uint256) {
        return _penaltyFeeTime;
    }

    // Remove Penalty Fee Time
    function removePenaltyFeeTime() public onlyOwner {
        _penaltyFeeTime = 0;
    }
}