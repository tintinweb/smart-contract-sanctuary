...
contract Celan{
    ...
    event directReferer(address indexed refer);
    event registration(address indexed _from,address indexed to,uint level,uint value,uint time);
    ...
    function register(uint _referid,uint level)public isLock payable{
        ...
        require(level <= _lastlevel,"invalid level");
        
        if(users[userList[_referid]].referals.length >= referLimit){
            emit directReferer(userList[_referid]);
        }
         if(users[userList[_referid]].referals.length >= referLimit)
        ...
        emit registration(msg.sender,userList[_referid],level,msg.value,block.timestamp);
    ...
    function _poolPercent()public onlyOwner{
