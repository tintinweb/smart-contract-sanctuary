...
contract LilHustlaz is ERC721Enumerable, Ownable {
    ...
    function mintLilHustlaz(uint256 _count) external payable {
        require(
            totalSupply() + _count <= MAX_HUSL,
            "Exceeds maximum supply of Lil Hustlaz"
        );
        address user = msg.sender;
        if (msg.sender != owner()) {
            require(saleOpen || presaleOpen, "Sale is not open yet");
            require(
            ...
            if (presaleOpen && !saleOpen) {
                require(
                    totalSupply() + _count <= 2200,
                    "Purchase will exceed maximum Lil Hustlaz available for presale"
                );
                require(
                    whitelistAddr[user] == false,
                    "Address has already minted two Lil Hustlaz"
                );
                require(
                    _count > 0 && _count <= 2,
                    "Minimum 1 and Maximum 2 Lil Hustlaz can be minted per address in presale"
                );
                require(
                ...
                require(
                    _count > 0 && _count <= 7,
                    "Minimum 1 & Maximum 7 Lil Hustlaz can be minted per transaction"
                );
            }
        }
