...
contract Marx is IERC20, IERC20Metadata, Context, Ownable{
    ...
    Communist[] public communists;

    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (uint256 => address) public communistToOwner;
    mapping (address => uint256) ownerCommunistCount;
    
    
    //Mais Valia = surplus value 
    
   function withdrawDividends(uint256 _communistId, address communist) public isCommunist(msg.sender) {
        ...
        require(communist == communistToOwner[_communistId]);
        Communist storage myCommunist = communists[_communistId];
        require(_isReadySurplusValue(myCommunist)); 
        ...
        _limitSurplusValue(myCommunist);
        
        emit DividendPayment(communist, myDividends);
    }
    
    
    function _createCommunist(address _communist) private returns (uint id) {
        id = _idCommunist.add(1);
        communists.push(Communist(id, _communist, uint256(block.timestamp.add(surplusValueTime))));
        communistToOwner[id] = msg.sender;
        ownerCommunistCount[msg.sender].add(1);
