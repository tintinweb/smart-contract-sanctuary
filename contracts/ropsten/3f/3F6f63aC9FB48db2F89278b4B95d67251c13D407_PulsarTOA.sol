...
import {IERC20, SafeERC20} from "./vendor/openzeppelin/contracts/token/ERC20/SafeERC20.sol";
import {ReentrancyGuard} from "./vendor/openzeppelin/contracts/utils/ReentrancyGuard.sol";
...
import {IOracleAggregator} from "./interfaces/pulsar/IOracleAggregator.sol";
import {IPulsar} from "./interfaces/pulsar/IPulsar.sol";
import {ITaskStorage} from "./interfaces/pulsar/ITaskStorage.sol";
import {IPulsarRouter} from "./interfaces/pulsar/IPulsarRouter.sol";
import {IUniswapV2Router02} from "./interfaces/uniswap/IUniswapV2Router02.sol";
import {_to18Decimals} from "./pulsar/functions/FToken.sol";
import {SimpleServiceStandard} from "./pulsar/standards/SimpleServiceStandard.sol";
import {_transferEthOrToken} from "./pulsar/functions/FPayment.sol";
import {ETH} from "./pulsar/constants/CTokens.sol";
import {Fee} from "./pulsar/structs/SPulsar.sol";

contract PulsarTOA is SimpleServiceStandard, ReentrancyGuard, Utils {
    ...
    IPulsarRouter public immutable pulsarRouter;
    ...
    event LogTOATrade(uint256 indexed id, ExecOrder order, uint256 outAmount);
    ...
    constructor(
        ...
        IPulsarRouter _pulsarRouter,
        IUniswapV2Router02 _uniRouterV2,
        IUniswapV2Router02 _sushiRouterV2,
        address _pulsarDiamond
    ) SimpleServiceStandard(_pulsarDiamond) {
        kyberProxy = _kyberProxy;
        pulsarRouter = _pulsarRouter;
        ...
        returns (ExecOrder memory order, uint256 id)
    {
        if (_order.inToken == ETH) {
            uint256 value = _isSubmitAndExec
                ? _order.amountPerTrade * (_order.numTrades + 1)
                : _order.amountPerTrade * _order.numTrades;
            require(
                msg.value == value,
                "PulsarTOA.submit: mismatching amount of ETH deposited"
            );
        }
        order = ExecOrder({
            user: msg.sender,
    ...
    function submitAndExec(
        ...
        require(
            _order.numTrades > 1,
            "PulsarTOA.submitAndExec: cycle must have 2 or more trades"
        );
        ...
        } else if (_protocol == Dex.PULSAR) {
            received = _doPulsarTrade(
                _tradePath,
                _order.amountPerTrade,
                _minReturnOrRate,
                payable(msg.sender),
        ...
        emit LogTOATrade(id, order, received);
    }
        ...
        nonReentrant
    {
        ...
        if (_order.inToken == ETH) {
            ...
            require(success, "PulsarTOA.cancel: Could not refund ETH");
        }
    ...
    function claimPlatformFees(
        ...
        for (uint256 i = 0; i < _platformWallets.length; i++) {
            for (uint256 j = 0; j < _tokens.length; j++) {
                uint256 fee = platformWalletFees[_platformWallets[i]][
                    _tokens[j]
                ];
                if (fee > 1) {
        ...
        )
    {
        ...
        emit LogTOATrade(_id, _order, outAmount);
    }
    ...
    {
        // 4. Rate Check
        (uint256 idealReturn, ) = IOracleAggregator(
            IPulsar(pulsar).getOracleAggregator()
        ...
        require(
            idealReturn > 0,
            "PulsarTOA.getMinReturn: idealReturn cannot be 0"
        );

        // check time (reverts if block.timestamp is below execTime)
        uint256 timeSinceCanExec = block.timestamp - // solhint-disable-next-line not-rely-on-time
            (_order.lastExecutionTime + _order.delay);
    ...
    function _actionPulsar(
        ...
        (
            ...
            uint256 minReturn,
            address payable receiver
        ) = _preExec(_order, _fee, _outTokenFee, Dex.PULSAR);

        require(
            _order.inToken == _tradePath[0] &&
                _order.outToken == _tradePath[_tradePath.length - 1],
            "PulsarTOA.action: trade path does not match order."
        );
    ...
    function _doPulsarTrade(
        ...
        address actualOutToken;
        {
            ...
            if (_tradePath[0] == address(ETH)) {
                _tradePath[0] = pulsarRouter.WETH();
            }
            if (_tradePath[tradeLen - 1] == address(ETH)) {
                _tradePath[tradeLen - 1] = pulsarRouter.WETH();
            }
        ...
        if (actualInToken == ETH) {
            amounts = pulsarRouter.swapExactETHForTokens{
                value: actualSellAmount
            }(_minReturn, _tradePath, _receiver, _MAX_AMOUNT);
        } else {
            if (actualOutToken == address(ETH)) {
                amounts = pulsarRouter.swapExactTokensForETH(
                    actualSellAmount,
                    _minReturn,
                    _tradePath,
                    _receiver,
                ...
                amounts = pulsarRouter.swapExactTokensForTokens(
                    actualSellAmount,
                    _minReturn,
                    _tradePath,
                    _receiver,
    ...
    function _storeOrder(ExecOrder memory _order, bool _isSubmitAndExec)
        private
        returns (uint256 id)
    {
    ...
    function _getKyberRate(
        ...
        uint256 newAmountIn = _to18Decimals(
            _inToken,
            _amountIn,
            "PulsarTOA:_getKyberRate: newAmountIn revert"
        );
        uint256 newMinReturn = _to18Decimals(
            _outToken,
            _minReturn,
            "PulsarTOA:_getKyberRate: newMinReturn revert"
        );
    ...
    function getExpectedReturnPulsar(
        uint256 _inAmount,
        address[] calldata _tradePath,
        uint256 _platformFee
    ) external view returns (uint256 outAmount, uint256 expectedRate) {
        ...
        try pulsarRouter.getAmountsOut(srcAmountAfterFee, _tradePath) returns (
            uint256[] memory amounts
        ) {
...
contract Utils {
    ...
    IERC20 internal constant USDT_TOKEN_ADDRESS =
        IERC20(0xdAC17F958D2ee523a2206206994597C13D831ec7);
    IERC20 internal constant DAI_TOKEN_ADDRESS =
        IERC20(0x6B175474E89094C44Da98b954EedeAC495271d0F);
    IERC20 internal constant USDC_TOKEN_ADDRESS =
        IERC20(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48);
    IERC20 internal constant WBTC_TOKEN_ADDRESS =
        IERC20(0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599);
    IERC20 internal constant KNC_TOKEN_ADDRESS =
    ...
    function getSetDecimals(IERC20 token)
        internal
...
import {TaskReceipt} from "pulsar-protocol/contracts/core/interfaces/IPulsarCore.sol";
import {IPulsarV1} from "./IPulsarV1.sol";
...
interface IPulsar {
    ...
    function stakeExecutor(IPulsarV1 _pulsarCore) external payable;

    function unstakeExecutor(IPulsarV1 _pulsarCore, address payable _to)
        external;

    function multiReassignProviders(
        IPulsarV1 _pulsarCore,
...
interface IPulsarRouter {
    ...
    function termswapTokensAndTokens(
        ...
        address user,
        uint256 deadline
    ) external returns (uint256 termamountAOut, uint256 termamountBOut);
    ...
    function getTermAmountAOut(
        ...
        uint256 reserveA,
        uint256 reserveB
    ) external view returns (uint256 termamountAOut);
    ...
    function getTermAmountBOut(
        ...
        uint256 reserveA,
        uint256 reserveB
    ) external view returns (uint256 termamountBOut);
}
...
import {SafeERC20} from "../../vendor/openzeppelin/contracts/token/ERC20/SafeERC20.sol";
...
import {_transferEthOrToken, _swapTokenToEthTransfer} from "../functions/FPayment.sol";
import {_getExpectedReturnAmount} from "../functions/FPulsar.sol";
import {ETH} from "../constants/CTokens.sol";

abstract contract SimpleServiceStandard is TaskStorage {
    address public immutable pulsar;
    ...
    modifier pulsarfy(
        ...
        // Check only Pulsar is calling
        require(
            address(pulsar) == msg.sender,
            "SimpleServiceStandard: Caller is not pulsar"
        );
        ...
        // Pay Pulsar
        if (_swapRate == 0)
            _transferEthOrToken(payable(pulsar), _creditToken, _fee);
        else if (
            _getExpectedReturnAmount(_creditToken, ETH, _fee, pulsar) == 0
        ) {
            _swapTokenToEthTransfer(pulsar, _creditToken, _fee, _swapRate);
        }

        emit LogExecSuccess(_id, tx.origin, _fee, _swapRate, _creditToken);
    }

    constructor(address _pulsar) {
        pulsar = _pulsar;
    }
...
import {IPulsarProviderModule} from "../../provider-modules/IPulsarProviderModule.sol";
import {IPulsarCondition} from "../../conditions/IPulsarCondition.sol";

struct Provider {
    address addr;  //  if msg.sender == provider => self-Provider
    IPulsarProviderModule module;  //  can be IPulsarProviderModule(0) for self-Providers
}

struct Condition {
    IPulsarCondition inst;  // can be AddressZero for self-conditional Actions
...
struct Task {
    ...
    Action[] actions;
    uint256 selfProviderGasLimit;  // optional: 0 defaults to pulsarMaxGas
...
struct TaskReceipt {
    ...
    Task[] tasks;
    uint256 expiryDate;
    uint256 cycleId;  // auto-filled by PulsarCore. 0 for non-cyclic/chained tasks
    uint256 submissionsLeft;
}

interface IPulsarCore {
    event LogTaskSubmitted(
    ...
    /// @param _provider Pulsar Provider object: provider address and module.
    ...
    /// @param _provider Pulsar Provider object: provider address and module.
    /// @param _task A Pulsar Task object: provider, conditions, actions.
    ...
    /// @notice A Pulsar Task Cycle consists of 1 or more Tasks that automatically submit
    ...
    /// @dev PulsarCore checks this during execution, in order to safeguard the Conditions
    /// @param _TR TaskReceipt, consisting of user task, user proxy address and id
    /// @param _gasLimit Task.selfProviderGasLimit is used for SelfProviders. All other
    ///  Providers must use pulsarMaxGas. If the _gasLimit is used by an Executor and the
    ///  tx reverts, a refund is paid by the Provider and the TaskReceipt is annulated.
    /// @param _execTxGasPrice Must be used by Executors. Gas Price fed by pulsarCore's
    ///  Gas Price Oracle. Executors can query the current pulsarGasPrice from events.
    ...
    ///   successfully executed, or when the execution failed, despite of pulsarMaxGas usage.
...
import {Action, Provider, Task, DataFlow, TaskReceipt} from "pulsar-protocol/contracts/core/interfaces/IPulsarCore.sol";

// TaskSpec - Will be whitelised by providers and selected by users
struct TaskSpec {
    IPulsarCondition[] conditions; // Address: optional AddressZero for self-conditional actions
    Action[] actions;
...
interface IPulsarV1 {
    ...
    /// @notice Stake on Pulsar to become a whitelisted executor
    /// @dev Msg.value has to be >= minExecutorStake
    function stakeExecutor() external payable;

    /// @notice Unstake on Pulsar to become de-whitelisted and withdraw minExecutorStake
    ...
    /// @param _userProxy userProxy passed by PulsarCore during submission and exec
    /// @param _provider Pulsar Provider object: provider address and module.
    /// @param _task Task defined in IPulsarCore
    /// @return Expected to return "OK"
    ...
    /// @notice Validate if selected TaskSpec is whitelisted by provider and that current pulsarGasPrice is below GasPriceCeil
    /// @dev If gasPriceCeil is != 0, Task Spec is whitelisted
    /// @param _userProxy userProxy passed by PulsarCore during submission and exec
    /// @param _provider Pulsar Provider object: provider address and module.
    /// @param _task Task defined in IPulsarCore
    /// @param _pulsarGasPrice Task Receipt defined in IPulsarCore
    /// @return res Expected to return "OK"
    ...
    /// @param _taskSpecs Task Receipt List defined in IPulsarCore
    ...
    function addProviderModules(IPulsarProviderModule[] calldata _modules)
        external;

    /// @notice De-Whitelist new provider Module(s)
    /// @param _modules Addresses of the modules which will be removed
    function removeProviderModules(IPulsarProviderModule[] calldata _modules)
        external;
    ...
    function multiProvide(
        ...
        IPulsarProviderModule[] calldata _modules
    ) external payable;

    /// @notice De-Whitelist TaskSpec(s), Module(s) and withdraw funds from pulsar in one tx
    ...
    /// @param _pulsarMaxGas Current pulsarMaxGas
    /// @param _pulsarGasPrice Current pulsarGasPrice
    ...
    /// @param _provider Address of provider
    /// @param _pulsarMaxGas Currentt pulsarMaxGas
    /// @param _pulsarGasPrice Current pulsarGasPrice
    ...
    /// @notice Check if executor has sufficient stake on pulsar
    ...
    /// @notice Constant used to specify the highest gas price available in the pulsar system
    ...
    function isModuleProvided(address _provider, IPulsarProviderModule _module)
        external
        view
        returns (bool);
    ...
    function providerModules(address _provider)
        external
        view
        returns (IPulsarProviderModule[] memory);
    ...
    function setPulsarGasPriceOracle(address _newOracle) external;
    ...
    function setPulsarMaxGas(uint256 _newMaxGas) external;
    ...
    /// @notice Addess of current Pulsar Gas Price Oracle
    function pulsarGasPriceOracle() external view returns (address);
    ...
    function pulsarMaxGas() external view returns (uint256);
    ...
    /// @param _gasPrice Current pulsar gas price
...
/// @title IPulsarCondition - solidity interface of PulsarConditionsStandard
/// @notice all the APIs of PulsarConditionsStandard
/// @dev all the APIs are implemented inside PulsarConditionsStandard
interface IPulsarCondition {
    /// @notice PulsarCore calls this to verify securely the specified Condition securely
    /// @dev Be careful only to encode a Task's condition.data as is and not with the
    ///  "ok" selector or _taskReceiptId, since those two things are handled by PulsarCore.
    /// @param _taskReceiptId This is passed by PulsarCore so we can rely on it as a secure
...
/// @notice all the APIs and events of PulsarActionsStandard
/// @dev all the APIs are implemented inside PulsarActionsStandard
interface IPulsarAction {
    /// @notice Providers can use this for pre-execution sanity checks, to prevent reverts.
    /// @dev PulsarCore checks this in canExec and passes the parameters.
...
interface IPulsarProviderModule {
    /// @notice Check if provider agrees to pay for inputted task receipt
    /// @dev Enables arbitrary checks by provider
    /// @param _userProxy The smart contract account of the user who submitted the Task.
    /// @param _provider The account of the Provider who uses the ProviderModule.
    /// @param _task Pulsar Task to be executed.
    ...
    /// @param _taskReceiptId Unique ID of Pulsar Task to be executed.
    /// @param _userProxy The smart contract account of the user who submitted the Task.
    /// @param _provider The account of the Provider who uses the ProviderModule.
    /// @param _task Pulsar Task to be executed.
    ...
    /// @notice Called by PulsarCore.exec to verifiy that no revert happend on userProxy
    /// @dev If a caught revert is detected, this fn should revert with the detected error
    /// @param _proxyReturndata Data from PulsarCore._exec.userProxy.call(execPayload)
...
import {Action, Task} from "../core/interfaces/IPulsarCore.sol";
...
import {PULSAR_GAS_PRICE_ORACLE} from "../constants/CPulsar.sol";
import {IChainlinkOracle} from "../../interfaces/chainlink/IChainlinkOracle.sol";
import {ETH} from "../constants/CTokens.sol";
...
function _getPulsarGasPrice(address _gasPriceOracle) view returns (uint256) {
    return uint256(IChainlinkOracle(_gasPriceOracle).latestAnswer());
}

// Pulsar Oracle price aggregator
...
function _getExpectedReturnAmount(
    ...
    (buyAmt, ) = IOracleAggregator(IPulsar(_pulsar).getOracleAggregator())
        .getExpectedReturnAmount(_amt, _inToken, _outToken);
}

function _getPulsarFee(
    uint256 _gasOverhead,
    uint256 _gasStart,
    address _payToken,
    address _pulsar
) view returns (uint256 pulsarFee) {
    pulsarFee =
        (_gasStart - gasleft() + _gasOverhead) *
        _getCappedGasPrice(IPulsar(_pulsar).getGasPriceOracle());

    if (_payToken == ETH) return pulsarFee;

    // returns purely the ethereum tx fee
    (pulsarFee, ) = IOracleAggregator(IPulsar(_pulsar).getOracleAggregator())
        .getExpectedReturnAmount(pulsarFee, ETH, _payToken);
}

function _getCappedGasPrice(address _gasPriceOracle) view returns (uint256) {
    uint256 oracleGasPrice = _getPulsarGasPrice(_gasPriceOracle);
...
IChainlinkOracle constant PULSAR_GAS_PRICE_ORACLE = IChainlinkOracle(
