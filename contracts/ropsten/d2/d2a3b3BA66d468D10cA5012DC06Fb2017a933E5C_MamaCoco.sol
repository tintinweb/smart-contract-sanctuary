...
contract MamaCoco is IBEP20, LockableFunction, MamaCocoNoBSSupport, LPSwapSupport, AntiLPSniper{
    ...
    function initialize(address reflectionToken1, address reflectionToken2) external onlyOwner init {
        initReflectors([reflectionToken1, reflectionToken2]);
...
abstract contract MamaCocoNoBSSupport is NoBSAdvSupport {
    ...
    function initReflectors(address[2] memory rewardsTokens) public onlyOwner init {
        noBSReflectors.push(INoBSDynamicReflector(noBSRouter.createAdditionalDynamicReflectorWithToken(rewardsTokens[0])));
        noBSReflectors.push(INoBSDynamicReflector(noBSRouter.createAdditionalDynamicReflectorWithToken(rewardsTokens[1])));
    }
}
...
abstract contract NoBSAdvSupport is NoBSInit, Ownable, LockableFunction {
    ...
    function initReflectors(address[] memory rewardsTokens) public virtual onlyOwner init {
        for(uint256 i = 0; i < rewardsTokens.length; ++i){
            noBSReflectors.push(INoBSDynamicReflector(noBSRouter.createAdditionalDynamicReflectorWithToken(rewardsTokens[i])));
            totalReflectors++;
        }
    }
