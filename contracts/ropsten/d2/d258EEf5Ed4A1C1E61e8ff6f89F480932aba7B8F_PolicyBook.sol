...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    uint256 public constant MINUMUM_COVERAGE = 100 * DECIMALS; // 100 STBL
    ...
    uint256 public constant override READY_TO_WITHDRAW_PERIOD = 600;
    ...
    function getClaimApprovalAmount(address user) external view override returns (uint256) {
        return priceFeed.howManyBMIsInUSDT(policyHolders[user].coverTokens.div(100));
    }
    ...
    function _submitClaimAndInitializeVoting(string memory evidenceURI, bool appeal) internal {
        ...
        uint256 virtualEndEpochNumber = policyHolders[_msgSender()].endEpochNumber +
            VIRTUAL_EPOCHS;
    ...
    function commitClaim(
        ...
        if (status == IClaimingRegistry.ClaimStatus.ACCEPTED) {
            ...
            stblToken.transfer(claimer, claimAmount);
            ...
            uint256 endUnlockEpoch = Math.max(
            ...
            uint256 virtualEndEpochNumber = policyHolders[claimer].endEpochNumber.add(
                VIRTUAL_EPOCHS
            );
    ...
    function _buyPolicy(
        ...
        stblToken.transferFrom(_buyer, reinsurancePoolAddress, _reinsurancePrice);
        stblToken.transferFrom(_buyer, address(this), _price);
    ...
    function updateEpochsInfo() public override {
        ...
        uint256 _newDistributionEpoch = Math.min(
            getEpoch(block.timestamp),
            _lastDistributionEpoch + MAXIMUM_EPOCHS
        );
        ...
        updateBMICoverStakingReward
    {
        ...
        if (_msgSender() != policyBookFabricAddress) {
            stblToken.transferFrom(_liquidityHolderAddr, address(this), _liquidityAmount);
        }
    ...
    function _getUserAvailableSTBL(address _userAddr) internal view returns (uint256) {
        uint256 availableSTBL = convertBMIXToSTBL(
        ...
        updateBMICoverStakingReward
    {
        ...
        uint256 _tokensToWithdraw = Math.min(
            _currentWithdrawalAmount,
            convertSTBLToBMIX(liquidity.sub(totalCoverTokens))
        );

        uint256 _stblTokensToWithdraw = convertBMIXToSTBL(_tokensToWithdraw);
        stblToken.transfer(_msgSender(), _stblTokensToWithdraw);
