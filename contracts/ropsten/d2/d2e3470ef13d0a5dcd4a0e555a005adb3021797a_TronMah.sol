...
contract TronMah {
    ...
    uint256 constant min_user_deposit = 0.01 ether;
    ...
    event NewDeposit(address indexed user, uint256 amount, uint16 invest_deposit_days);
    ...
    function rewardReferrers(address _addr, uint256 _amount) private {
        ...
        for (uint8 i = 0; i < count_referral; i++) {
            ...
            emit ReferralBonusPaid(ref, _addr, i + 1, _amount, bonus);
    ...
    function deposit(uint16 _days, address _referrer) external payable {
        ...
        emit CommissionPaid(msg.sender, msg.value, commissionAmount, _days);
