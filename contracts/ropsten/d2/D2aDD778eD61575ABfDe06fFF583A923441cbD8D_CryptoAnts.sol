...
interface ICryptoAnts is IERC721 {
  /**
   * @dev Mints `_amount` eggs to the user for `_eggPrice`.
   *
   * Requirements:
   * - `_amount` should be bigger than 0.
   * - The amount of eth should be enough for the price of the egg.
   *
   * Emits a {EggsBought} event.
   */
  function buyEggs(uint256 _amount) external payable;

  /**
   * @dev Sells `_antId` to the CryptoAnts contract for a price less than `_eggPrice`.
   *
   * Requirements:
   * - The contract should have enough funds.
   * - The sender should own the `_antId`.
   *
   */
  function sellAnt(uint256 _antId) external;

  /**
   * @dev creates ants to the sender.
   *
   * Requirements:
   * - The sender should have an egg.
   * - The sender needs to allow the egg tokens to be transfered by the contract.
   *
   * Emits a {AntCreated} event.
   */
  ...
  /**
   * @dev Returns the amount of ants created.
   */
  function getAntsCreated() external view returns (uint256);

  /**
   * @dev Returns the actual egg price.
   */
  function getEggPrice() external view returns (uint256);

  /**
   * @dev Sets the actual egg price.
   *
   * Requirements:
   * - Only governance can execute this call.
   */
  function setEggPrice(uint256 _newPrice) external;

  /**
   * @dev Returns the Ant price.
   *
   * Requirements:
   * - Should be less than egg price
   */
  function getAntPrice() external view returns (uint256);

  /**
   * @dev The ant will hatch an egg.
   *
   * Requirements:
   * - Sender should be owner of the ant.
   * - An ant can only hatch an egg once every 10 minutes.
   */
  function hatch(uint256 _antId) external;

  /**
   * @dev Emitted when eggs are being bought.
   */
  event EggsBought(address buyer, uint256 antId);
  /**
   * @dev Emitted when eggs are being hatched.
   */
  event EggsHatched(address caller, uint256 antId, uint256 eggstToHatch);
  /**
   * @dev Emitted when an ant is sold.
   */
  event AntSold(address seller, uint256 antId);
  /**
   * @dev Emitted when an ant is created.
   */
  event AntCreated(address owner, uint256 antId);
  /**
   * @dev Emitted when an ant dies when hatching.
   */
  event AntDead(uint256 antId);
  /**
   * @dev Emitted when sender wants to create an ant and has no eggs.
   */
  error NoEggs();
}
