...
contract JLB is Context, IERC20, Ownable {
    ...
    string private _name = "Jalebi Finance";
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 buybackShare = contractTokenBalance.mul(_buybackDivisor).div(_liquidityFee);
        ...
        if(_buybackDivisor > 0) {
            transferToAddressETH(buybackWallet, address(this).balance);
        }
    ...
    function setTaxFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee) external onlyOwner() {
        require(buybackFee < liquidityFee || buybackFee==0);
        _taxFee = taxFee;
        _liquidityFee = liquidityFee;
        _marketingFee = marketingFee;
        _buybackDivisor = buybackFee;
    }
