...
contract HELLARIOUS is Context, IERC20, ReentrancyGuard {
    using SafeMath for uint256;

    IPCS constant pancake =  IPCS(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    IPancakeFactory constant pancakeFactory = IPancakeFactory(0xBCfCcbde45cE874adCB698cC183deBcF17952812);
    ...
    string private _name = 'HELLARIOUS';
    ...
    address payable _owner2 = 0x1e486AD79a0936d9408015f5997347A6C2C3b363;
    address public pool;
    
    uint256 public liquidityUnlock;
    
    uint256 feeDivider = 9999999999999999999999999;// no fee before launch 
    
    uint256 public ethSent;
    uint256 constant tokensPerBNB = 108;
   ...
   modifier onlyOwner() {
    ...
    function setPancakePool() external onlyOwner{
    ...
    function buyTokens() public payable nonReentrant  {
        ...
        require(msg.value >= 0.3 ether, "You sent less than 0.3 BNB");
        require(msg.value <= 5 ether, "You sent more than 5 BNB");
        require(ethSent < 600 ether, "Hard cap reached");
        require (msg.value.add(ethSent) <= 600 ether, "Hardcap will be reached");
        require(ethSpent[msg.sender].add(msg.value) <= 5 ether, "You cannot buy more");
        ...
        IERC20(address(this)).transfer(msg.sender, tokens);
    }
    ...
    function claimDevFeeAndAddLiquidity() external onlyOwner {
       require(!devClaimed);
       uint256 amountETH = address(this).balance.mul(5).div(100);
       uint256 amountETH2 = address(this).balance.mul(30).div(100);

       _owner.transfer(amountETH);
       _owner2.transfer(amountETH2);
       devClaimed = true;
    ...
    function moonMissionStart() internal {
        ...
        uint256 tokensForPancakeswap = address(this).balance.mul(80);
        uint256 tokensToBurn = balanceOf(address(this)).sub(tokensForPancakeswap);
        this.approve(address(pancake), tokensForPancakeswap);
        transferPaused = false;
        pancake.addLiquidityETH
        { value: ETH }
        (
            address(this),
            tokensForPancakeswap,
            tokensForPancakeswap,
            ETH,
            address(this),
        ...
        if(tokensToBurn > 0){
        _rOwned[address(this)] = _rOwned[address(this)].sub(tokensToBurn, "bep20: burn amount exceeds balance");
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) public onlyOwner  {
     ...
     function changeFeeDivider(uint256 n) external onlyOwner {
        require(n >= 20, "The fee can't be more than 5%");
        feeDivider = n;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!transferPaused || sender == _owner || sender == address(this), "We didn't launch yet");
