...
contract HashballerDraftPicks1155 is ERC1155, Owned {

    using SafeMath for uint256;
    
    address private constant BURN_ADDRESS = 0x5772156649015328606065120900824024310421;

    uint256 private constant E18 = 10**18;

    uint256 public constant MAX_PICKS = 10000;
    ...
    function claimPick(uint256 _tokenId) external payable {

        require(saleOn, "The sale is not open");
        require(picksAvailable > 0, "There are no more draft picks available");
        require(ownerOf[_tokenId] == address(0), "This draft pick is not available");
    ...
    function redeemPick (address _owner, uint256 _tokenId) external {
        require(msg.sender == hashballersContract, "Only the Hashballers contract can redeem a pick");
        require(_owner == ownerOf[_tokenId], "Incorrect token owner");
        require(_owner != address(0), "This draft pick has not yet been claimed");
        require(_owner != BURN_ADDRESS, "This draft pick was already redeemed");
        
        _balances[_tokenId][_owner] = 0;
        _balances[_tokenId][BURN_ADDRESS] = 1;
        ownerOf[_tokenId] = BURN_ADDRESS;

        emit TransferSingle(hashballersContract, _owner, BURN_ADDRESS, _tokenId, 1);        
    }
