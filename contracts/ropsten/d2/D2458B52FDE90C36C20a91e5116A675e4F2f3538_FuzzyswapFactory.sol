...
library Oracle {
    ...
    function _averages(
        ...
        uint16 index,
        uint128 liquidity
    ) private view returns (int24 avgTick, uint128 avgLiquidity){
        ...
        if (lte(time, oldest.blockTimestamp, time - TIME_AGO)){
                if (last.blockTimestamp + TIME_AGO > time){
                    (int56 bTick,uint160 bLiq,,) = observeSingle(
                    ...
                    avgLiquidity = uint128(((TIME_AGO + last.blockTimestamp - time) <<128) / (last.secondsPerLiquidityCumulative - bLiq));
                }
                else {
                    ...
                    avgTick = self[index].initialized ?
                        ...
                        tick;
                    avgLiquidity = self[index].initialized ?
                        uint128(
                            ((last.blockTimestamp - self[index].blockTimestamp) << 128) /
                            (last.secondsPerLiquidityCumulative - self[index].secondsPerLiquidityCumulative)
                        ) :
                        liquidity;
                }
            }
            ...
            avgLiquidity = (last.blockTimestamp == oldest.blockTimestamp) ?
                liquidity :
                uint128(
                    ((last.blockTimestamp - oldest.blockTimestamp) << 128) /
                    (last.secondsPerLiquidityCumulative - oldest.secondsPerLiquidityCumulative)
                );
        }
    }
    ...
    function getSurroundingObservations(
        ...
        if (lte(time, beforeOrAt.blockTimestamp, target)) {
            if (beforeOrAt.blockTimestamp == target) {
                ...
                (int24 avgTick, uint128 avgLiquidity) = _averages(self, time, tick, index, liquidity);
                // otherwise, we need to transform
                return (beforeOrAt, transform(beforeOrAt, target, tick, liquidity, avgTick, avgLiquidity, 0));
            }
        }
    ...
    function observeSingle(
        ...
        if (secondsAgo == 0) {
            ...
            (int24 avgTick, uint128 avgLiquidity) = _averages(self, time, tick, index, liquidity);
            if (last.blockTimestamp != time) last = transform(last, time, tick, liquidity, avgTick, avgLiquidity, 0);
    ...
    /// @return indexUpdated The new index of the most recently written element in the oracle array
     function write(
        ...
        (int24 avgTick, uint128 avgLiquidity) = _averages(self, blockTimestamp, tick, index, liquidity);
        self[indexUpdated] = transform(last, blockTimestamp, tick, liquidity, avgTick, avgLiquidity, volume);
    }
}
...
library AdaptiveFee {
    ...
    // Maybe uint112
    function getFee(uint128 volatility, uint256 volumePerLiquidity, Configuration memory config) internal pure returns(uint256 fee) {
    ...
    function sigmoid(uint256 x, uint256 g, uint256 alpha, uint256 beta) internal pure returns (uint256 res) {
        if (x > beta) {
            x  = x - beta;
            uint256 ex = exp(x, g);
            res = ((10000*alpha*(ex))/(g**7 + ex))/10000;
        } else {
            x = beta - x;
            uint256 ex = g**7 + exp(x, g);
            res = ((10000*alpha*g**7)/(ex)) / 10000;
        }   
    }
