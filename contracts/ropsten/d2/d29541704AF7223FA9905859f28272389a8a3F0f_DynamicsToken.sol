...
import "./utils/SwapableERC20.sol";

contract DynamicsToken is Context, Ownable, SwapableERC20, BuyBack {
    ...
    // Yes, this transfer function is overly large.
    // Given the quantity of fees to calculate, and that solidity imposes limits on private variables -> function depth
    // it was necessary to calculate each tax for every even inside this code block rather than propagate it to
    // sub functions as the compiler would not run with so many variables too deep in sub functions.
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            } else if(buyBackEnabled && address(this).balance >= minBuyBack){
    ...
    function calculateSell(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        if(block.timestamp < trader[from].lastBuy.add(sellFees[FeeLevels.LEVEL1].saleCoolDownTime)) {
        ...
        if(amount > poolSize.mul(sellFees[FeeLevels.LEVEL1].saleSizeLimitPercent).div(100)) {
        ...
        //                uint256 salesTokenQuantity = amount.mul(finalSaleFee).div(100);
        uint256 saleFeeQty = amount.mul(finalSaleFee).div(200);
        _takeProjectFees(saleFeeQty, 0);
        //                _reflectFee(rFee, tFee);
        swapTokensForEth(saleFeeQty.mul(_getRate()));
    }
    ...
    function updateBuyerFees(uint256 reflectionFee, uint256 projectFee, uint256 liquidityFee, uint256 burnFee, uint256 marketingFee) public onlyOwner {
        ...
        emit UpdateBuyerFees(oldBuyFees, buyFees);
    }
    ...
    function updateTransferFees(uint256 reflectionFee, uint256 projectFee, uint256 liquidityFee, uint256 burnFee, uint256 marketingFee) public onlyOwner {
        ...
        emit UpdateTransferFees(oldTransferFees, transferFees);
    }
...
import "./Ownable.sol";
import "./SwapableERC20.sol";

abstract contract BuyBack is Ownable, SwapableERC20 {
    using SafeMath for uint256;
    using Address for address;
    ...
    function buyBackTokens(uint256 amount) internal lockTheSwap {
    ...
    function setBuyBackRange(uint256 _minBuyBackEther, uint256 _maxBuyBackEther) public onlyOwner {
        uint256 oldMin = minBuyBack;
        uint256 oldMax = maxBuyBack;
        minBuyBack = _minBuyBackEther * 1 ether;
        maxBuyBack = _maxBuyBackEther * 1 ether;
        emit BuyBackRangeUpdated(oldMin, minBuyBack, oldMax, maxBuyBack);
    }
...
abstract contract SwapableERC20 is IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    function _approve(address ad, address routerAddress, uint256 tokenAmount) internal virtual;
    function balanceOf(address account) public view override virtual returns (uint256);

    string internal _name = "Dynamics Token";
    string internal constant _symbol = "DYNA";
    ...
    function swapETHForTokens(uint256 amount, address destination) internal {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
