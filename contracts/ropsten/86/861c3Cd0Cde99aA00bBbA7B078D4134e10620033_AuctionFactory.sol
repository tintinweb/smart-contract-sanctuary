...
contract AuctionFactory is Ownable {
    ...
    address[] public availableLots;
    mapping(address => bool) public lotSaleAllowance;

    struct AuctionDetails {
        address lotTokenAddress;
        uint256 lotTokenAmount;
        address auctionAddress;
    }
    uint256 public lotId;
    mapping(uint256 => AuctionDetails) public bidding;

    event CreateAuction(uint256 indexed _lotId, uint256 _timeBeforeAuctionStarts, uint256 _beginPrice, uint256 _auctionStep, uint256 _bidTime, address indexed _lot);

    function createAuction(
        uint256 _timeBeforeAuctionStarts, //time before auction starts (amount of seconds. example: 600 s -> 10 m)
        uint256 _beginPrice, //price value (wei)
        uint256 _auctionStep, //step value (wei)
        uint256 _minTimer, //minimal time (amount of seconds)
        uint256 _bidTime, //bid time (amount of seconds)
        address _lotTokenAddress, //lot address (address)
        address payable _serviceWallet,
        uint256 _lotAmount //amount of tokens to sale (wei)
    ) onlyOwner public returns(address) {
        require(lotSaleAllowance[_lotTokenAddress], "Lot is not available");
        require(_beginPrice >= _auctionStep, "Begin price must be higher then auction step or equal to it");
        lotId++;
        address auction = address(new Auction(lotId, _timeBeforeAuctionStarts, _beginPrice, _auctionStep, _minTimer, _bidTime, _lotTokenAddress, _lotAmount, _serviceWallet));
        bidding[lotId] = AuctionDetails(
            {
                lotTokenAddress: _lotTokenAddress,
                lotTokenAmount: _lotAmount,
                auctionAddress: auction
            });
        emit CreateAuction(lotId, _timeBeforeAuctionStarts, _beginPrice, _auctionStep, _bidTime, _lotTokenAddress);

        return auction;
    }

    /**
     * Set available BEP-20 tokens addresses
     */
    function setAvailableLots(address _lotTokenAddress) onlyOwner public returns(bool) {
        require(_lotTokenAddress.isContract(), "this is not the address of the contract");
        require(!lotSaleAllowance[_lotTokenAddress], "lot address already available");
        availableLots.push(_lotTokenAddress);
        lotSaleAllowance[_lotTokenAddress] = true;

        return true;
    }

    /**
     * Returns available BEP-20 tokens addresses
     *
     * @return Array of addresses.
     */
    function getAvailableLots() public view returns (address[] memory) {
