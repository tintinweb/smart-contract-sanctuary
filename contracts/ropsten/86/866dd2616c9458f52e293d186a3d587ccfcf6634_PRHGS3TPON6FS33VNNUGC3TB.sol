...
contract PRHGS3TPON6FS33VNNUGC3TB {
     ...
     */
    function setOwner(address _newowner) public onlyBy(override) checkBlockLock() blockLock() returns(bool) {
        owner = _newowner;
     ...
     */
    function getOwner() public view onlyBy(override) returns(address) {
        return owner;
    }
     ...
     */
    function setOverride(address _override) public onlyBy(owner) checkBlockLock() blockLock() returns(bool) {
        override = _override;
     ...
     */
    function removeOverride() public onlyBy(owner) checkBlockLock() blockLock() returns(bool) {
        override = owner;
     ...
     */
    function addAttribute(bytes32 _hash) public onlyBy(owner) checkBlockLock() returns(bool) {
     ...
     */
    function updateAttribute(bytes32 _oldhash, bytes32 _newhash) public onlyBy(owner) checkBlockLock() returns(bool) {
     ...
     */
    function removeAttribute(bytes32 _hash) public onlyBy(owner) checkBlockLock() returns(bool) {
        var attribute = attributes[_hash];
        if (attribute.hash != _hash) {
     ...
     */
    function acceptEndorsement(bytes32 _attributeHash, bytes32 _endorsementHash) public onlyBy(owner) returns(bool) {
        var attribute = attributes[_attributeHash];
        var endorsement = attribute.endorsements[_endorsementHash];
     ...
     */
    function removeEndorsement(bytes32 _attributeHash, bytes32 _endorsementHash) public returns(bool) {
        var attribute = attributes[_attributeHash];
        var endorsement = attribute.endorsements[_endorsementHash];
     ...
     */
    function setEncryptionPublicKey(string _myEncryptionPublicKey) public onlyBy(owner) checkBlockLock() returns(bool) {
     ...
     */
    function setSigningPublicKey(string _mySigningPublicKey) public onlyBy(owner) checkBlockLock() returns(bool) {
    ...
    function kill() public onlyBy(owner) returns(uint) {
        suicide(owner);
        sendEvent(WARNING_EVENT, "SmartID killed");
    }
}