...
interface ITHEMOONMONSTER is IERC721Enumerable {
    function isMintedBeforeReveal(uint256 index) external view returns (bool);
}

/**
 *
 * Moni Contract (The native token of THEMOONMONSTER)
 * @dev Extends standard ERC20 contract
 */
contract Moni is Context, IERC20 {
    ...
    address private _THEMOONMONSTERAddress;
    ...
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(ITHEMOONMONSTER(_THEMOONMONSTERAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < ITHEMOONMONSTER(_THEMOONMONSTERAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = ITHEMOONMONSTER(_THEMOONMONSTERAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
     ...
     */
    function setTHEMOONMONSTERAddress(address THEMOONMONSTERAddress) public {
        require(_THEMOONMONSTERAddress == address(0), "Already set");
        
        _THEMOONMONSTERAddress = THEMOONMONSTERAddress;
    }
    ...
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < ITHEMOONMONSTER(_THEMOONMONSTERAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(ITHEMOONMONSTER(_THEMOONMONSTERAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the THEMOONMONSTER contract. For better UX.
        if (msg.sender != _THEMOONMONSTERAddress) {
