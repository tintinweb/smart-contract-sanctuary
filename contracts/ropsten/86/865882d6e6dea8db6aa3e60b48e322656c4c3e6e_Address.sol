...
//    _      ____ _____   __        __    _   ___   _     ____  ___   _   ____  _      ___   __  
//   | |\/| | |_   | |   / /\      / /`_ | | | |_) | |   | |_  | |_) | | | |_  | |\ | | | \ ( (` 
//   |_|  | |_|__  |_|  /_/--\     \_\_/ |_| |_| \ |_|__ |_|   |_| \ |_| |_|__ |_| \| |_|_/ _)_) 
//
...
contract MetaGirlFriends is ERC721Enumerable, Ownable {
    ...
    uint public immutable reserveCount = 50;    
    ...
    mapping(address => uint8) public whitelistCounts;
    ...
    function setDev(address newDev) external onlyOwner{
        require(newDev!=address(0), "New dev is the null address");
        dev = newDev;
    } 
    ...
    function setWhitelist(address[] calldata newAddresses, uint8[] calldata newCounts) external devOrOwner {
        ...
        for (uint256 i = 0; i < newAddresses.length; i++){
            ...
            require(!isWhitelisted(newaddress), "Duplicate address");
            whitelistCounts[newAddresses[i]] = newCounts[i];
        }
    }

    function removeWhitelist(address[] calldata currentAddresses) external devOrOwner {
        for (uint256 i = 0; i < currentAddresses.length; i++){
            address curraddress = currentAddresses[i];
            require(curraddress!=address(0),"Address is the zero address");
            whitelistCounts[curraddress]=0;
        }
    }

    function isWhitelisted(address buyer) public view returns (bool){
        return whitelistCounts[buyer]>0;
    }

    function getWhitelistCount(address buyer) external view returns (uint8){
        require(isWhitelisted(buyer), "Not whitelisted");
        return whitelistCounts[buyer];
    }    
    ...
    function mintNFTs(uint256 quantity) external payable {
        ...
        require(_msgSender() == tx.origin, "Minter must be EOA");        
        ...
        if(_msgSender()!=treasury){
            ...
            if (whitelistEnabled){
                ...
                require(balanceOf(_msgSender()).add(quantity) <= whitelistCounts[_msgSender()], "White list mint limit exceeded");
            }
            require(quantity <= buyLimit, "Buy limit exceeded");
        }else{
            // treasury can mint a maximum of reserveCount tokens for giveaways
            require(tokensReserved.add(quantity) <= reserveCount, "Max reserve count exceeded");
