...
contract FreezableToken is StandardToken, Ownable {
    ...
    function toKey(address _addr, uint _release) internal pure returns (bytes32 result) {
        result = 0x7543857400000000000000000000000000000000000000000000000000000000;
        assembly {
    ...
    function toKey2(address _addr1, address _addr2, uint _release) internal pure returns (bytes32 result) {
        bytes32 key1 = 0x7543857400000000000000000000000000000000000000000000000000000000;
        bytes32 key2 = 0x8947698456784657453478560000000000000000000000000000000000000000;
        assembly {
...
contract Consts {
    ...
    string public constant TOKEN_NAME = "ZeusCoin";
    string public constant TOKEN_SYMBOL = "ZEUS";
    uint public constant INITIAL_SUPPLY = 1000000000 * TOKEN_DECIMAL_MULTIPLIER;
}


contract ZeusCoin is Consts, BurnableToken, Pausable, MintableToken, FreezableToken
{
    uint256 startdate;
    ...
    function init() private {
        ...
        // Start date is April 01, 2021
        startdate = 1617206400;


        beneficiary1 = 0xb11D7cd4D759493DbCF32505054c0363d37773aA; // Public sale
        beneficiary2 = 0xcdc095a8073c7dFd45B37d52b43b87d58AA2D4b7; // Team 
        beneficiary3 = 0x4fdF40DF018A0D5D1e439E2B854324f07f6309f1; // Bounty
        beneficiary4 = 0xBEE715f8e40A58056F8a879d51d5bE05777359c4; // Foundation

        // Public sale (80%)
        _transfer(address(this), beneficiary1, totalSupply().mul(80).div(100));

        // DevTeam tokens (10%) are frozen and will be released every 6 months after SEO.
        ...
        // Bounty of (3%) tokens will be released.
        _transfer(address(this), beneficiary3, totalSupply().mul(3).div(100));
		
		// Foundation receives (7%) tokens that will be frozen on Zeus Foundation smart-contract for 2 years.
