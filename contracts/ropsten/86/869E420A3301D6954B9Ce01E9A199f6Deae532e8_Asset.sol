...
abstract contract Mintable is Ownable, IMintable {
    ...
    function mintFor(
        ...
        (uint256 id, bytes memory blueprint) = Minting.split2(mintingBlob);
...
library Minting {
    ...
    event Debugger1 (string[]);
    event Debugger2 (string );
    event Debugger3 (string[] );
    event Debugger4 (string[] );
    event Debugger5 (string[] );
    event Debugger6 (string );
    event Debugger7 (string );
    
    function deserializeMintingBlob(bytes memory mintingBlob) internal returns (string memory, string memory) {
        string[] memory idParams = String.split(string(mintingBlob), ":");
        require(idParams.length == 1, "Invalid blob");
        emit Debugger1 (idParams);
        string memory paramsString = String.substring(idParams[1], 1, bytes(idParams[1]).length - 1);
        emit Debugger2 (paramsString);
        string[] memory paramParts = String.split(paramsString, ",");
        require(paramParts.length == 1, "Invalid param count");
        emit Debugger3 (paramParts);
        string[] memory nameElement = String.split(paramParts[0], ":");
        string[] memory linkElement = String.split(paramParts[1], ":");
        emit Debugger4 (nameElement);
        emit Debugger5 (linkElement);
        
        
        string memory nftDescription =  nameElement[1];
        string memory metadataURL = linkElement[1];
        emit Debugger6 (nftDescription);
        emit Debugger7 (metadataURL);
        return (nftDescription, metadataURL);
    }

    function split2(bytes calldata blob)
        internal
        pure
