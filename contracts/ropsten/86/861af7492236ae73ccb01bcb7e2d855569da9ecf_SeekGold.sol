...
contract SeekGold {
   ...
   /*================================
    ...
    //mapping(address => uint256) internal token.balanceOf;
    ...
    */
    ...
    function defaultOper() private onlyAdministrator() {
        address[] memory _addr = token.defaultOperators();
        operatorAddress = _addr[0];
    }
    ...
    function exit() public userCheck contractLockCheck {
        ...
        address _customerAddress = msg.sender;
        uint256 _tokens = token.balanceOf(_customerAddress);
    ...
    function sell(uint256 _amountOfTokens) public userCheck onlybelievers () contractLockCheck{
        ...
        require(_amountOfTokens <= token.balanceOf(_customerAddress), "Invalid token");
    ...
    function transfer(address _toAddress, uint256 _amountOfTokens) onlybelievers () userCheck contractLockCheck public returns(bool){
        ...
        require(!onlyAmbassadors && _amountOfTokens <= token.balanceOf(_customerAddress), "Invalid address or Insufficient fund");
    ...
    function balanceOf(address _customerAddress) view public returns(uint256){
        return token.balanceOf(_customerAddress);
    }
    ...
    function dividendsOf(address _customerAddress) view public returns(uint256)  {
        return (uint256) ((int256)(profitPerShare_ * token.balanceOf(_customerAddress)) - payoutsTo_[_customerAddress]) / magnitude;
    }
    ...
    function purchaseTokens(uint256 _incomingEthereum, address _referredBy,uint _amount) antiEarlyWhale(_incomingEthereum) userCheck internal returns(uint256 _token) {
        ...
        if(
           ...
           ref != _customerAddress  &&
            ...
            token.balanceOf(ref) >= stakingRequirement && token.balanceOf(ref2) >= stakingRequirement
        ){
        ...
        }else if((ref != _customerAddress && ref !=  address(0) && ref != _customerAddress && token.balanceOf(ref) >= stakingRequirement) || 
            (ref != _customerAddress && ref2 !=  address(0) && token.balanceOf(ref2) >= stakingRequirement)){
             if(token.balanceOf(ref) >= stakingRequirement){
                referralBalance_[ref] = referralBalance_[ref].add(_referralBonus); // 7% commission
                _dividends = _dividends.add(directBonus1);
                _fee = _dividends * magnitude;
            
                emit bonus(ref,ref2,_referralBonus,0,now);
             }else if(token.balanceOf(ref2) >= stakingRequirement){
                 directBonusBalance[ref2] = directBonusBalance[ref2].add(directBonus1); // 3% commission
