...
/**
Multi Farmurai Capital: $MFC

Tokenomics (LAUNCH):
5% of tax goes to existing holders.
20% of tax goes into multi-chain farming to add to the treasury and buy back MFC tokens.

Tokenomics (AFTER LAUNCH IS COMPLETED):
5% of tax goes to existing holders.
10% of tax goes into multi-chain farming to add to the treasury and buy back MFC tokens.

Telegram:
https://t.me/multifarmuraicapital
...
contract MultiFarmuraiCapital is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "MultiFarmuraiCapital";
    ...
    address payable public _mfcAddress = payable(0x6e937840D62ED53Fa6C194bf6c691a5e7f9359B7); //investment wallet
    address payable public _mktgAddress = payable(0x133523921bfd644E69361eF3d97d95431deB0D90); //marketing wallet
    ...
    event UpdatedMfcAddress(address mfc); //investment wallet
    ...
    function sendETHToFee(uint256 amount) private {
        _mfcAddress.transfer(amount.div(2));
        _mktgAddress.transfer(amount.div(2));
    }

    function manualSwap() external {
        require(_msgSender() == _mfcAddress);
        uint256 contractBalance = balanceOf(address(this));
        swapTokensForEth(contractBalance);
    }

    function manualSend() external {
        require(_msgSender() == _mfcAddress || _msgSender() == _mktgAddress);
        uint256 contractETHBalance = address(this).balance;
    ...
    function setMaxWalletSize(uint256 maxWalletSize) public onlyOwner {
     ...
     //set wallet address
    function _setMFCAddress(address mfcAddress) external onlyOwner {
        require(_mfcAddress != address(0), "_mfcAddress cannot be 0");
        _isExcludedFromFee[mfcAddress] = false;
        mfcAddress = payable(_mfcAddress);
        _isExcludedFromFee[mfcAddress] = true;
        emit UpdatedMfcAddress(_mfcAddress);
    }
