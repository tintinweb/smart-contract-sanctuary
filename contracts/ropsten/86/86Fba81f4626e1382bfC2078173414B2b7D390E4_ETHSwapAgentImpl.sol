...
contract ETHSwapAgentImpl is Context, Initializable {
    ...
    function fillBSC2ETHSwap(
        ...
        for (uint i = 0; i < bscTxHashArr.length; i++) {
            require(!filledBSCTx[bscTxHashArr[i]], "bsc tx filled already");

            filledBSCTx[bscTxHashArr[i]] = true;
            IERC20(registeredERC20).safeTransfer(toAddressArr[i], amountArr[i]);

            emit SwapFilled(registeredERC20, bscTxHashArr[i], toAddressArr[i], amountArr[i]);
        }
