...
contract WeirdElves is ERC721Enumerable, Ownable, CybermonValues {
    ...
    uint256 public constant NFT_PRICE_PUBLIC = 30000000000000000; // 0.025 ETH
    uint256 public constant NFT_PRICE_WHITELIST = 25000000000000000; // 0.025 ETH
    uint256 public constant MAX_NFT_PURCHASE = 20;
    uint256 public constant MAX_NFT_PRESALE = 5;
    ...
    mapping(address => bool) _presaleList;
   
    constructor() ERC721("WeirdElves","WRDELVS"){
        cybermonContract = 0x321b3E9C6fdd8CfaAAcc96a6F6E15394a1b08719;
    }
    ...
    function mintChecks(uint256 numberOfTokens, uint256 msgVal, address sender, bool whiteList) private view {
        ...
        uint256 ethVal = whiteList ? NFT_PRICE_WHITELIST * numberOfTokens : NFT_PRICE_PUBLIC * numberOfTokens;
        require(ethVal == msgVal, "Sent ether val is incorrect");

        if(whiteList) {
            require(isWhitelisted(sender), "You are not whitelisted");
            require((MAX_NFT_PRESALE - balanceOf(msg.sender)) >= numberOfTokens, "Cant mint that much");
        }
        else{
            require(publicSalePhase, "You are not able to mint");
            require(numberOfTokens <= MAX_NFT_PURCHASE, "Max max 20 tokens per trx");            
        }
    }
    ...
    function presaleMint(uint256 numberOfTokens) public payable {
        uint256 totalSupply = totalSupply();
        mintChecks(numberOfTokens, msg.value, msg.sender, true);
    ...
    function mint(uint256 numberOfTokens) public payable {
        uint256 totalSupply = totalSupply();
        mintChecks(numberOfTokens, msg.value, msg.sender, false);        
    ...
    function isWhitelisted(address sender) public view returns (bool) {
        if((_whiteList[sender] && whitelistPhase ) || (_presaleList[sender] && presalePhase))
            return true;
    ...
    function addToWhiteList(address[] memory accounts, bool isBackup) external onlyOwner{
        require(accounts.length > 0, "Invalid input");
        for(uint256 index = 0; index < accounts.length; index++){
            if(isBackup)
                _presaleList[accounts[index]] = true;
            else 
                _whiteList[accounts[index]] = true;
        }
    }
    
    function removeFromWhiteList(address[] memory accounts, bool isBackup) external onlyOwner{
        require(accounts.length > 0, "Invalid input");
        for(uint256 index = 0; index < accounts.length; index++){
            if(isBackup)
                _presaleList[accounts[index]] = false;
            else 
