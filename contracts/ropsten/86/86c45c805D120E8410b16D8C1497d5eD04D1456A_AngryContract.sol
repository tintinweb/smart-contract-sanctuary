...
contract AngryContract{
    ...
    function prePurchase(uint256 _expectedPrice, uint256 _startTime, uint256 _expiredTime) public payable {
        ...
        uint256 highestEthPrice = 0;
        uint256 highestUSDTPrice = 0;
        (highestEthPrice, highestUSDTPrice) = getHighestExpectedPrice();
        if(msg.value > 0){
            if(highestEthPrice > 0){
            ...
            pcInfo.price = 10 ** angryTokenDecimals * msg.value / currAmount;
            pcInfo.paymentAmount = msg.value;
            pcInfo.paymentType = 1;
        }else{
            if(highestUSDTPrice > 0){
            ...
            pcInfo.price = 10 ** angryTokenDecimals * allowance / currAmount;
    ...
    function getHighestExpectedPrice() public view returns(uint256 _ethPrice, uint256 _usdtPrice){
        ...
        _ethPrice = amounts[1] * maxPriceMultiple * (100 + expectedPriceFloatVal) / 100;
        _usdtPrice = amounts[2] * maxPriceMultiple * (100 + expectedPriceFloatVal) / 100;
    }
