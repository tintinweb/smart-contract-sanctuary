...
//                dynaHodler[user][currentRound+1].exists = true;
//                dynaHodler[user][currentRound+1].lastPayoutRound = currentRound;
//            } else {
...
//        }
    ...
    function snapshot() private {
        uint256 queueLength = queue.length();
        for(uint256 i = 0; i < queueLength; i++){
            try IERC20(address(tokenContract)).balanceOf(queue.at(i)) returns (uint256 bal) {
                dynaHodler[queue.at(i)][currentRound].tokenBalance = bal;
            } catch {}
        }
    }
    ...
    function _sendEthTo(address payable hodler) private returns(uint256) {
        ...
        // TODO - Clean this up for better gas
        dynaHodler[hodler][currentRound+1].totalPayouts = dynaHodler[hodler][currentRound].totalPayouts.add(reward);
