...
contract MetaBoom is ComposableTopDown, Ownable {
    ...
    uint256 public constant maxSupply = 30; //max 5000 minus 300 airdrops
    //uint256 public constant maxSupply = 4700; //max 5000 minus 300 airdrops
    uint256 public constant price = 0.05 ether;
    uint256 public constant airDropMaxSupply = 10;
    ...
    uint256 public publicSaleTime = 1635596072;
        ...
        returns (childContract[] memory)
    {
        ...
        if (childCount == 0) {
            return new childContract[](0);
        } else {
            childContract[] memory childContracts = new childContract[](
                childCount
            );

            uint256 index;
            for (index = 0; index < childCount; index++) {
                address childAddr = childContractByIndex(_tokenId, index);
                childContracts[index].contracts = childAddr;
                uint256 tokenCount = totalChildTokens(_tokenId, childAddr);
                uint256 indexToken;
                for (indexToken = 0; indexToken < tokenCount; indexToken++) {
                    childContracts[index].tokens[
                        indexToken
                    ] = childTokenByIndex(_tokenId, childAddr, indexToken);
                }
            }
