...
import "./IGratiaERC721.sol";
import "./Stringstrings.sol";
import "./IERC20.sol";
import "./SafeERC20.sol";

interface IGratiaWallet {
 ...
 * {ERC721Enumerable}.
 */
contract GratiaERC721 is ERC165, IERC721, IERC721Metadata, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    address public ggrtToken;
    address public nygToken;

    IGratiaWallet public gratiaWallet;
    ...
    function balanceOf(address owner) public view virtual override returns (uint256) {
        require(owner != address(0), "GratiaERC721: balance query for the zero address");
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        return _tokenOwners.get(tokenId, "GratiaERC721: owner query for nonexistent token");
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = GratiaERC721.ownerOf(tokenId);
        require(to != owner, "GratiaERC721: approval to current owner");

        require(_msgSender() == owner || isApprovedForAll(owner, _msgSender()),
            "GratiaERC721: approve caller is not owner nor approved for all"
        );
    ...
    function getApproved(uint256 tokenId) public view virtual override returns (address) {
        require(exists(tokenId), "GratiaERC721: approved query for nonexistent token");
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(operator != _msgSender(), "GratiaERC721: approve to caller");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "GratiaERC721: transfer caller is not owner nor approved");
     ...
     * are aware of the GratiaERC721 protocol to prevent tokens from being forever locked.
     *
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory _data) internal virtual {
        _transfer(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, _data), "GratiaERC721: transfer to non ERC721Receiver implementer");
    }
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        require(exists(tokenId), "GratiaERC721: operator query for nonexistent token");
        address owner = GratiaERC721.ownerOf(tokenId);
    ...
    function _safeMint(address to, uint256 tokenId, bytes memory _data) internal virtual {
        _mint(to, tokenId);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), "GratiaERC721: transfer to non ERC721Receiver implementer");
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        require(to != address(0), "GratiaERC721: mint to the zero address");
        require(!exists(tokenId), "GratiaERC721: token already minted");
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        // Prevent burner's GratiaWallet assets to be transferred together	
        gratiaWallet.withdrawAll(address(this), tokenId);
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(GratiaERC721.ownerOf(tokenId) == from, "GratiaERC721: transfer of token that is not own");
        require(to != address(0), "GratiaERC721: transfer to the zero address");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(GratiaERC721.ownerOf(tokenId), to, tokenId);
    }
        ...
        private returns (bool)
    {
        if (to.isContract()) {
            try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("GratiaERC721: transfer to non ERC721Receiver implementer");
                } else {
    ...
    /**
     * @dev Returns name of the Gratia at index.
     */
    ...
    /**
     * @dev Returns if the Gratia has been minted before reveal phase
     */
    function isMintedBeforeReveal(uint256 index) public view virtual returns (bool) {
        return _mintedBeforeReveal[index];
    }

    function initGratiaWalletAddress(address newGratiaWallet) public virtual onlyOwner {
        gratiaWallet = IGratiaWallet(newGratiaWallet);
    }
    ...
    function changeName(uint256 tokenId, string memory newName) public virtual {
        ...
        IERC20(nygToken).safeTransferFrom(_msgSender(), address(this), NAME_CHANGE_PRICE);
        ...
        IERC20(nygToken).burn(NAME_CHANGE_PRICE);
...
interface IGratiaERC721 is IERC721Enumerable {
