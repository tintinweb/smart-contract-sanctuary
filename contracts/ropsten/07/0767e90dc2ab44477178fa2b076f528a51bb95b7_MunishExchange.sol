...
contract MunishExchange {
  ...
  function MunishExchange(address feeAccount_) {
    owner = msg.sender;
  ...
  function depositToken(address token, uint256 amount) {
    ...
    if (!MunishToken(token).transferFrom(msg.sender, this, amount)) throw;
  ...
  function withdraw(address token, uint256 amount) returns (bool success) {
    ...
    if (token == address(0)) {
      ...
      if (!MunishToken(token).transfer(msg.sender, amount)) throw;
    }
  ...
  function adminWithdraw(address token, uint256 amount, address user, uint256 nonce, uint8 v, bytes32 r, bytes32 s, uint256 feeWithdrawal) onlyAdmin returns (bool success) {
    ...
    if (token == address(0)) {
      ...
      if (!MunishToken(token).transfer(user, amount)) throw;
    }
