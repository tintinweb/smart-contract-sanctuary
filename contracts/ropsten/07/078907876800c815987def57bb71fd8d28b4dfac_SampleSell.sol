...
contract SampleSell is Context,ReentrancyGuard {
  ...
  constructor() ReentrancyGuard() {
    ...
    event LogSuccessString(bool success);
    
    function SellTokens(address tokenToSend,uint256 tokenAmount) nonReentrant() external payable {
		require(IERC20(tokenToSend).transferFrom(_msgSender(), address(this), tokenAmount), 'transferFrom failed');
        require(IERC20(tokenToSend).approve(address(uniswap), tokenAmount), 'Uniswap approval failed');
        uint256 currentContractBalance = address(this).balance;
        address[] memory path = new address[](2);
        path[0] = tokenToSend;
        path[1] = uniswapRouter.WETH();
        // uniswapRouter.swapExactTokensForETH(tokenAmount,0,path,address(this),block.timestamp);
        // try uniswapRouter.swapExactTokensForETH(tokenAmount,0,path,address(this),block.timestamp) returns (uint[] memory amounts) {
        //     emit LogSuccessString(amounts);
        // } catch {
