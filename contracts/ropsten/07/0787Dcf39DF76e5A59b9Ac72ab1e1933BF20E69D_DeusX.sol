...
contract DeusX is ERC721Enumerable, Ownable {
  ...
  */
  ...
  constructor(string memory _name, string memory _symbol, bytes32[] memory _roots, uint256[] memory _startTimes) ERC721(_name, _symbol) {
    roots = _roots;
  ...
  function purchase(uint256 numberOfTokens, uint256[] calldata factions) external payable {
    ...
    require(block.timestamp > startTimes[3], 'Not started');
  ...
  function setTimes(uint256[] calldata _startTimes) external onlyOwner {
      startTimes = _startTimes;
  }
