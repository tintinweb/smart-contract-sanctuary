...
contract StandardToken is IERC20 {
  ...
  bool crowdSaleStarted = true;                                                 // crowdsale status
  uint256 public airdropcountOfMMM = 0;                                         // Variable to keep track on number of airdrop
  uint256 tokensForMMM = 20000000000000000000000000;                            // total tokens for MMM
  ...
  */
  ...
  function totalSupply() external override view returns (uint256) {
   ...
   /** 
   * @dev Total number of tokens in MMM.
   */
  function totalSupplyOfMMM() public view returns (uint256) {
    return tokensForMMM;
  }
  ...
  function airdropByOwnerForMMM(address[] memory _addresses, uint256[] memory _amount) public onlyOwner returns (bool){
    ...
    uint256 count = _addresses.length;
    for (uint256 i = 0; i < count; i++){
      require (tokensForMMM > _amount[i], "tokens are less to transfer");
