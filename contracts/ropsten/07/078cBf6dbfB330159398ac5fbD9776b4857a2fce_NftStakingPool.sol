...
contract NftStakingPool {
	...
	{
		require(tokenStakedBy[token][id] == msg.sender, "Must be staked by you!"); // Requires for token to be staked by msg.sender.
	...
	function isStaker(address who) public view returns (bool result) {
		
		return nftStaked[who] > 0;
...
contract YieldFarm is Ownable, ERC721
{
	...
	struct BalanceRecord
	{
		...
		uint256 endEpoch;                             // Epoch of withdrawal.
	}
	...
	{
		require(block.timestamp > lastUpdateDate + 7 days, "seven days have not passed!");// Require for date to be 7 days after last function call.
	...
	function _withdraw(address who, uint256 id) internal
	{
		...
		positions[id].endEpoch = currentEpoch;                          // Records epoch when withdraw.
		...
		emit Withdrawal(who, id, value, currentEpoch);                  // Records in Withdrawal event.
	}
