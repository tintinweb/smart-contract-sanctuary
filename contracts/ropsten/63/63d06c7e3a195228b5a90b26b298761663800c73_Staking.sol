...
contract Staking {
  ...
  uint256 constant public WITHDRAW_TIME_EQUILIBRIUM   = 90 ;    // variable constant to manage withdraw time lock up for gold
  uint256 constant public WITHDRAW_TIME_TRANQUILLITY  = 135;     // variable constant to manage withdraw time lock up for platinum
  ...
  */
  ...
  function setContractAddress(address tokenContractAddress) external onlyOwner returns(bool){
    itoken = ICliq(tokenContractAddress);
    return true;
  }
  ...
  */
  ...
  function withdrawAddedTokenReward(uint256 token) external onlyOwner returns(bool){
    ...
    itoken.transferFrom(address(this), msg.sender, token);
    return true;
  }
  ...
  function withdrawStakedTokens(uint256 stakingId) public returns(bool) {
    ...
    if(_tokenTotalDays[stakingId] == PERIOD_SERENITY){
          ...
          if(now >= _tokenStakingEndTime[stakingId]){
              ...
              itoken.transferFrom(address(this), msg.sender,_finalTokenStakeWithdraw[stakingId]);
