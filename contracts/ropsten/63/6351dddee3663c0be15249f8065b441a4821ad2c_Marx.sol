...
contract Marx is IERC20, IERC20Metadata, Context, Ownable{
    ...
    string private _name = "Marx12";
    string private _symbol = "MARX12";
    uint8 private _decimals = 4;
    uint256 private _totalSupply;
    uint256 public _totalCommunists = 0;
   ...
   function withdrawDividends(address communist) public isCommunist(msg.sender) {
    ...
    function _updateCommunist(address _communist) private view {
        if(balanceOf(_communist) == 0) {
            _totalCommunists.add(1);
        } else if(balanceOf(_communist) > 0){
            _totalCommunists.add(2);
        }
    }
    
    function _prepareCommunists() private view {
        _totalCommunists.sub(1);
    }
