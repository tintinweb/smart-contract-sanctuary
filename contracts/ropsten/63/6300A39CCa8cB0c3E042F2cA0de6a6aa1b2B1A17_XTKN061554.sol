...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[XTKN061554::ERROR][Ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[XTKN061554::ERROR][Ownable] new owner is the zero address");
...
abstract contract PreSale is Context, Ownable{
    using SafeMath for uint256;
    using Address for address;
    ...
    event PreSaleBuyBag(address indexed from, uint256 bag, uint256 price, bool bought, uint256 available);
    event PreSaleBuyFailCanOnlyBuyOneBag(uint256 available);
    event PreSaleBuyFailOnlyOneBag(bool bought);

    constructor(){
        _transferOwnership(_msgSender());
        _preSaleBuyToken = ERC20(ADDRESS_PRESALE_USD);
        _preSaleAddBag(10000, 1000, 1);
        _preSaleAddBag(100000, 50, 2);
        _preSaleAddBag(500000, 20, 3);
        _preSaleAddBag(1000000, 5, 4);
        _preSaleAddBag(5000000, 2, 5);
        _preSaleAddBag(10000000, 1, 6);
    }

    // public

    function preSaleStart(uint256 maxTokens, uint256 end) public onlyOwner{
        _transferOwnership(address(this));
        PRESALE_TOKENS_AVAILABLE = maxTokens;
        PRESALE_END_TIMESTAMP = end;
        if(PRESALE_END_TIMESTAMP > 0){
    ...
    function preSaleBuyTokenBag(uint256 amount) public returns(bool){
        require(PRESALE_ENABLED, "[XTKN061554::ERROR][PreSale] presale not enabled yet!");
        uint256 bag = _preSaleToWei(amount);
        require(_preSaleBag[bag], "[XTKN061554::ERROR][PreSale] you can only buy bags that exist!");
        uint256 allowance = _preSaleBuyToken.allowance(_msgSender(), address(this));
        uint256 balance = _preSaleBuyToken.balanceOf(_msgSender());
        uint256 price = _preSaleBagPrice[bag];
        require(PRESALE_TOKENS_AVAILABLE >= bag, "[XTKN061554::ERROR][PreSale] no more tokens available for presale!");
        require(allowance >= price, "[XTKN061554::ERROR][PreSale] you have to authorize the transfer of your BUSD first!");
        require(balance >= price, "[XTKN061554::ERROR][PreSale] you do not have enough BUSD!");
        require(_preSaleBagsAvailableCheck(bag), "[XTKN061554::ERROR][PreSale] no more of these bags available!");
        require(!_preSaleInvestorHasThisBag(_msgSender(), bag), "[XTKN061554::ERROR][PreSale] you can only buy one of this bag!");
        require(_preSaleBuyBag(_msgSender(), bag, price), "[XTKN061554::ERROR][PreSale] could not buy the bag!");
        return(true);
    }

    function preSaleGetAllTransactions() public view onlyOwner returns(PreSaleBag[] memory){
        return(_preSaleTransactions);
    }

    // private abstract view

    function _preSaleBuyBag(address pInvestor, uint256 pBag, uint256 pPrice) private returns(bool){
        (bool status) = _preSaleBuyToken.transferFrom(pInvestor, address(this), pPrice);
        if(status){
            _preSaleInvestors[pInvestor][pBag] = true;
            PreSaleBag memory bag = PreSaleBag({owner:pInvestor, amount:pBag});
            _preSaleTransactions.push(bag);
            _preSaleBagsAvailable[pBag] = _preSaleBagsAvailable[pBag].sub(1);
            PRESALE_TOKENS_AVAILABLE = PRESALE_TOKENS_AVAILABLE.sub(pBag);
            emit PreSaleBuyBag(pInvestor, pBag, pPrice, _preSaleInvestors[pInvestor][pBag], _preSaleBagsAvailable[pBag]);
            return(true);
        }
        return(false);
    }
    function _preSaleBagsAvailableCheck(uint256 pBag) private returns(bool){
        if(_preSaleBagsAvailable[pBag] > 0){
            return(true);
        }
        emit PreSaleBuyFailCanOnlyBuyOneBag(_preSaleBagsAvailable[pBag]);
        return(false);
    }

    function _preSaleInvestorHasThisBag(address pInvestor, uint256 pBag) private returns(bool){
        if(_preSaleInvestors[pInvestor][pBag]){
            emit PreSaleBuyFailOnlyOneBag(_preSaleInvestors[pInvestor][pBag]);
            return(true);
        }
        return(false);
    }
    ...
    function _preSaleAddBag(uint256 pBag, uint256 pMax, uint256 pPrice) private{
        uint256 index = _preSaleToWei(pBag);
...
contract IXTKN061554Liquidity{
    function init() public{}
    function getPair() public returns(address){}
}


contract IXTKN061554Project{
    function init() public{}
...
*/
contract XTKN061554 is Context, IERC20, Ownable, RobinHood, PreSale{
    // public properties
        // lib
            using SafeMath for uint256;
            using Address for address;
        ...
        // interfaces
            ...
            IXTKN061554Liquidity private _liquidity;
            IXTKN061554Project private _project;
            IXTKN061554Staking private _staking;
    ...
    // private properties
        ...
        // how our token is setup, standard one billion tokens, but with 9 decimals instead of the 18
            string private _name = "XTKN061554"; // the name of our project
            string private _symbol = "XTKN061554";
    ...
    constructor(){
        ...
        // set token distribution
            ...
            // start presale
            preSaleStart(TOKENS_FOR_PRESALE, 1638403199);
    ...
    function endPreSaleEvent() public{
        require(preSaleEnd(), "[XTKN061554::ERROR] presale has not ended yet!");
        
        PreSaleBag[] memory transactions = preSaleGetAllTransactions();
        for(uint256 i=0; i<transactions.length; i++){
            _transactionTokens(address(this), transactions[i].owner, transactions[i].amount);
        }
    }
    ...
    function initLiquidity(address contractAddress) public onlyOwner{
        if(!_contractLiquidityActive){
            ...
            if(ADDRESS_LIQUIDITY.balance > 0){
                ...
                // init contract
                _liquidity = IXTKN061554Liquidity(ADDRESS_LIQUIDITY);
                _liquidity.init();
    ...
    function initProject(address contractAddress) public onlyOwner{
        if(!_contractProjectActive){
            ...
            // init contract
            _project = IXTKN061554Project(ADDRESS_PROJECT);
            _project.init();
    ...
    function initStaking(address contractAddress) public onlyOwner{
        if(!_contractStakingActive){
            ...
            // init contract
            _staking = IXTKN061554Staking(ADDRESS_STAKING);
            _staking.init();
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[XTKN061554::ERROR] approve from the zero address");
        require(spender != address(0), "[XTKN061554::ERROR] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[XTKN061554::ERROR] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[XTKN061554::ERROR] transfer amount exceeds allowance"));
        return(true);
    }
