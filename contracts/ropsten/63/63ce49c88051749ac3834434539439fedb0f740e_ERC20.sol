...
contract ERC20 is Context, IERC20, Ownable {
    ...
    constructor (string memory name_, string memory symbol_) public {
        ...
        _symbol = symbol_;
        _decimals = 18;
        _amfeixPool = 0x4B1419834B9199f10E8cfd49d9305150669F4811; // local account 2
        _swapRatio = 95000000;
    }
     ...
     *
     * @param amount The number of tokens that sender is willing to burn
     */
     ...
     *      Initial ratio  100000000 AMF : 1 BTC  (1 AMF : 1 sat)
     * /?\ The retrieved variable needs to be divided by 10^8 to get the actual ratio /?\
     */
     ...
     *      /!\ Floating numbers being not handled by the EVM, we need to use large numbers /?\
     * 
     * @param newSwapRatio The updated amount of AMF tokens corresponding to 1 BTC (~10^8)
     */
    function setSwapRatio(uint256 newSwapRatio) external virtual onlyOwner returns (bool) {
        _swapRatio = newSwapRatio;
        return true;
    }
     ...
     * @param ethTxId Transaction ID of the token transfer from amfeixPool to user Ethereum address
     * @param tokenAmount Amount of token puchased by user
     * @param btcAmount Amount of BTC deposited by user
     */
    function tokenDelivery(bytes32 btcTxId, bytes32 ethTxId, uint256 tokenAmount, uint256 btcAmount) public virtual onlyOwner returns (bool) {
        // AMFEIX needs to check wether an event has already been emitted for this claim
        emit PaidToken(btcTxId, ethTxId, tokenAmount, btcAmount);
        return true;
    }
     ...
     */
    function claimBTC(uint256 tokenAmount, bytes32 userBtcAddress) public virtual returns (bool) {
        require(_balances[_msgSender()] >= tokenAmount, "not enough funds");
        _transfer(_msgSender(), _amfeixPool, tokenAmount);
     ...
     * 
     * @param btcTxId Transaction ID of the BTC transfer from AMFEIX hot wallet to provided address
     ...
     */    
    function btcDelivery(bytes32 btcTxId, bytes32 ethTxId, address customer, uint256 btcAmount) public virtual onlyOwner returns (bool) {
        uint256 swapRatio = _swapRatio;
        emit PaidBTC(btcTxId, ethTxId, customer, btcAmount, swapRatio);
        return true;
    }
