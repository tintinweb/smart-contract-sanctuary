...
contract LotteryV2 is AccessControl, Pausable {
    ...
    event Raffle(address indexed buyer,uint256 indexed amount,uint256 indexed ticketNumber,uint256 eventId,uint256 payment,string nonce);
    event RaffleInfo(address indexed buyer,address indexed contracrAddress,uint256 indexed tokenId,string  nonce);
...
//    event TransferETH(address indexed _toAddress,uint256 indexed _amount);
     ...
     * @param nonce 交易随机数
     * @param raffleInfo 编码的raffle详情数组
     * @param expireBlockNumber 限制的区块高度
     */
    function raffleAll(uint256 _eventId,uint256 _payType,bytes32 hash, bytes memory signature,string memory nonce, bytes[] memory raffleInfo,uint256 expireBlockNumber)public payable{
        _raffle(_eventId,0,_payType,hash,signature,nonce,raffleInfo,expireBlockNumber);
    }
     ...
     * @param expireBlockNumber 限制的区块高度
     */
    function raffle(uint256 _eventId,uint256 _payType,bytes32 hash, bytes memory signature,string memory nonce,bytes[] memory raffleInfo,uint256 expireBlockNumber)public payable{
        _raffle(_eventId,raffleInfo.length,_payType,hash,signature,nonce,raffleInfo,expireBlockNumber);
    }
     ...
     * @param expireBlockNumber 限制的区块高度
     */
    function _raffle(uint256 _eventId,uint256 _amount,uint256 _payType,bytes32 hash, bytes memory signature,string memory nonce,bytes[] memory raffleInfo,uint256 expireBlockNumber)internal {
        ...
        //验证区块高度是否合法
        assert(block.number < expireBlockNumber);
        //计算剩余的NFT数量
        ...
        //验证hash
        //TODO: add raffleInfo.
        require(hashTransaction(msg.sender,amount,nonce,raffleInfo) == hash,"Invalid hash!");
        //验证签名
        ...
        if (_payType == 1){
            ...
            require(_mintNFTByEncodeBytes(raffleInfo,nonce),"NFT mint failed");
            payable(address(this)).transfer(totalPrice);
            if (msg.value > totalPrice){
                payable(msg.sender).transfer(msg.value - totalPrice);
            }
            emit Raffle(msg.sender,amount,0,EventId,1,Nonce);
        }else{
            //查询拥有的奖券
            ...
            if (_payType == 2){
                ...
                emit Raffle(msg.sender,amount,burnTicketNumber,EventId,2,Nonce);
            }
            //混合支付
            if (_payType == 3){
                ...
                if (ticketNumber >= burnTicketNumber){
                    require(_mintNFTByEncodeBytes(raffleInfo,Nonce),"NFT mint failed");
                    ticketContract.burn(msg.sender,1,burnTicketNumber);
                    emit Raffle(msg.sender,amount,burnTicketNumber,EventId,3,Nonce);
                }else{
        ...
        //增加该场次的已购买的NFT数量
        EventInfoMap[EventId].purchasedNumber.add(amount);
    }
    ...
    /**
     * @dev  通过编码的raffleInfo参数铸造nft，内部封装方法
     * @param raffleInfo 编码的raffle详情数组
     * @return 是否铸造成功
     */
    //铸造NFT
    function _mintNFTByEncodeBytes(bytes[] memory raffleInfo,string memory nonce)internal returns(bool){
        uint256 success;
        for (uint256 i = 0; i < raffleInfo.length; i++) {
            address _address;//合约地址
            uint256 _randomId;//干扰值
            uint256 _TokenId;//NFT-TokenId
            string  memory _randomString;//干扰字符串
            (_address,_randomId,_TokenId,_randomString) = abi.decode(raffleInfo[i],(address, uint256, uint256, string));
            //判断合约类型
            if (_checkProtocol(_address) == 2){
                Common721NFT Common721NFTContract = Common721NFT(_address);
                Common721NFTContract.mint(msg.sender,_TokenId);
                success++;
                emit RaffleInfo(msg.sender,_address,_TokenId,nonce);
            }
            if (_checkProtocol(_address) == 1){
                Common1155NFT Common1155NFTContract = Common1155NFT(_address);
                Common1155NFTContract.mint(msg.sender,_TokenId,1,abi.encode(msg.sender));
                success++;
                emit RaffleInfo(msg.sender,_address,_TokenId,nonce);
            }
        }
        if (success == raffleInfo.length){
            return true;
        }else{
            return false;
        }
    ...
    function withdrawNFT(uint256 _tokenId,address _withdrawNFTContractAddress,uint256 _withdrawNFTTokenID,address _withdrawNFTAddress,bytes32 hash, bytes memory signature,string memory nonce)public{
        ...
        a[0] = abi.encode(msg.sender);
        //验证hash
        require(hashTransaction(msg.sender,1,nonce,a) == hash,"Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash,signature),"Invalid signature!");
        //验证是否拥有该资产NFT
    ...
    function mortgageNFT2ETH(uint256 _tokenId,uint256 _ETHAmount ,bytes32 hash, bytes memory signature,string memory nonce)public{
        ...
        a[0] = abi.encode(msg.sender);
        //验证hash
        require(hashTransaction(msg.sender,_ETHAmount,nonce,a) == hash,"Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash,signature),"Invalid signature!");
        //验证是否拥有该资产NFT
        require(_validateOwnership(_tokenId),"You don't have this NFT!");
    ...
    function mortgageNFT(uint256 _tokenId,uint256 _token1Amount ,bytes32 hash, bytes memory signature,string memory nonce)public{
        ...
        a[0] = abi.encode(msg.sender);
        //验证hash
        require(hashTransaction(msg.sender,_token1Amount,nonce,a) == hash,"Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash,signature),"Invalid signature!");
        //验证是否拥有该资产NFT
        require(_validateOwnership(_tokenId),"You don't have this NFT!");
     ...
     * @param qty 交易数量
     * @param nonce 交易随机数
     * @param bytesArray 传入的编码的bytes数组
     * @return 交易的hash值
     */
    function hashTransaction(address sender, uint256 qty, string memory nonce,bytes[] memory bytesArray) public pure returns (bytes32) {
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encode(sender, qty, nonce , bytesArray))
            )
        );
        return hash;
    }
