...
/**
 * @title TMONSale contract
 * @author https://grox.solutions
 */
contract TMONSale is Ownable {
    ...
    constructor(address USDTAddr, address TMONAddr, address initialOwner, address initialFeeWallet, address initialFundWallet) public Ownable(initialOwner) {
        require(USDTAddr != address(0) && TMONAddr != address(0) && initialFeeWallet != address(0) && initialFundWallet != address(0));

        USDT = IUSDT(USDTAddr);
        TMON = IERC20(TMONAddr);
