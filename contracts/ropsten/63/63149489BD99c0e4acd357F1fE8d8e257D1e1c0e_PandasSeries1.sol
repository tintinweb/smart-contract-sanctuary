...
contract PandasSeries1 is ERC721Tradable, ReentrancyGuard {
  ...
  constructor(address proxyRegistryAddress, uint256 launchTimestamp, uint256 publicSaleTimestamp)
    ERC721Tradable("PandasSeries1", "OHPS1", proxyRegistryAddress) {
  ...
  function mintGiveaway(address to) external onlyOwner {
    ...
    require(_totalSalesCount < _totalPandasCount && _tierSales[5] < _tierAmounts[5], "Minting exceeded");

    uint256 tokenId = _nextTokenId.current();

    mintTo(to);

    _tokenTiers[tokenId] = 5; //Giveaway
    _totalSalesCount += 1;
    _tierSales[5] += 1;

    emit PandaSeries1Minted(to, tokenId, 5);
  }
