...
contract TeamTokenFactory is Ownable{
    ...
    uint256 public feePercentage; // percentage should be in 3 decimal

    event TeamTokenCreated(address token, string name, string symbol, uint8 decimals, uint256 supply);

    constructor(address _feeWallet, uint256 _feePercentage) public{
        feeWallet = _feeWallet;
        feePercentage = _feePercentage;
    }

    function setFeeWallet(address _feeWallet) external onlyOwner{
        require(_feeWallet != address(0),'address is 0');
        feeWallet = _feeWallet; 
    }

    function setFeePercentage(uint256 _feePercentage) external onlyOwner{
        require(_feePercentage > 0,'address is 0');
        feePercentage = _feePercentage; 
    }
    ...
    function createTeamToken(string memory _name, string memory _symbol, uint8 _decimals, uint256 _supply) external returns (address teamTokenAddr) {
        ...
        TeamToken token = new TeamToken(_name, _symbol, _decimals, _supply, msg.sender, feeWallet, feePercentage);
