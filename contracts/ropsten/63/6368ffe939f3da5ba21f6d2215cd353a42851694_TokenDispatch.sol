...
contract TokenDispatch is Management {
    ...
    mapping(uint8 => team) public _teams;
    ...
    function reviewTeam(uint8 teamId) isManager external view returns
    (
        address,
        address,
        uint256,
    ...
    // function getTeam(uint8 teamId) isManager external view returns ( team memory ) {
    //     require(_teams[teamId].approved, 'YouSwap: NOT_EXIST');
    ...
    function claimed(uint8 teamId) public view returns (uint256) {
        //require(_beneficiaries[teamId].agent == msg.sender, 'YouSwap: NOT_ALLOWED');
