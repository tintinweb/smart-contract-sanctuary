...
contract PAPpool {
    ...
    // 0 for tier3  1 for tier2 2 for tier1
    // Pool infomation
    struct Pool {
        uint256 poolID;
        uint256 timeStart;
    ...
    mapping(string => uint256) totalSupply;
    ...
    constructor(
        ...
        totalSupply['tierOne'] = _Tier1.tokensAmount;
        totalSupply['tierTwo'] = _Tier2.tokensAmount;
        totalSupply['tierThree'] = _Tier3.tokensAmount;
    }
    ...
    function participate(uint256 _amount) public {
        ...
        uint256 amountInLotteryTokens = (_amount * pool[_tier].swapPriceNumerator) /
        ...
        require(
            ...
            'Amount is bigger than total supply!'
        );
    ...
    function getPapPoolInfo() external view returns (PapPoolInfo memory info) {
        info = PapPoolInfo({
            pools: [pool['tierOne'], pool['tierTwo'], pool['tierThree']],
            supplies: [
                totalSupply['tierOne'],
                totalSupply['tierTwo'],
                totalSupply['tierThree']
            ],
    ...
    function getUserInfo(address user) external view returns (infoParticipants memory) {
        participationRegistration storage participant = didParticipate[user];
        return participationBook[participant.participationId];
    }
}
...
contract CreatePapPool is Ownable {
    ...
    */
    ...
    function getUserInfo(address[] memory pools, address user)
        external
        view
        returns (PAPpool.infoParticipants[] memory)
    {
        uint256 len = pools.length;
        PAPpool.infoParticipants[] memory userInfos = new PAPpool.infoParticipants[](len);
        for (uint256 i = 0; i < len; i++) {
            userInfos[i] = PAPpool(pools[i]).getUserInfo(user);
        }
        return userInfos;
    }
}
