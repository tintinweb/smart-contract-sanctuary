...
abstract contract StandardToken  {
...
contract Controlled is Owned{
    ...
    //设置lockFlag值
    function disableLock(bool _locked) public onlyOwner returns(bool success){
        lockFlag = _locked;
        return true;
    }
    // 把_addr加到锁定账户里，拉黑名单。
    function addLocked(address _addr) public onlyOwner returns(bool success){
...
contract XinxinToken is StandardToken,Controlled{
    
    //使用SafeMath
    using SafeMath for uint256;
    
    //账户集合0x63a8074bC53a971bEE69580e0fd1A651b7c62D03
	mapping (address => uint256) public balanceOf;
	mapping (address => mapping (address => uint256)) internal allowed;
	constructor()  {
        totalSupply = 200000;//10亿
        name = "Xinxin Token";
        symbol = "xinxin";
        decimals = 2;
