...
contract Amas is ERC20, CoinAgeMintableStandard, CoinRateMintable, Ownable {
    using SafeMath for uint256;

    string public name = "Amas";
    string public symbol = "AMAS";
...
//--------------------------------------------------------------------------------------
    ...
    function mintingCoinRate() StrongHolderMinter CoinAgeMinter public returns (bool) {
        ...
        //Function to reset CoinRate to zero after receive minting coin
        //and user must hold for certain of time again before minting coin
    ...
    function getCoinRateMinting(address _address) internal view returns (uint) {
        ...
        uint _now = now; uint _periodInterval = getPeriodRate(_address, _now);
        if(_periodInterval <= 0) return 0;
        uint mintingEra = _periodInterval;
        uint coinRateMinting = coinRate * mintingEra;
    ...
    function getPeriodRate(address _address, uint _now) internal view returns (uint _periodInterval) {
        ...
        for (uint i = 0; i < transferIns[_address].length; i++){
            ...
            _periodInterval = _periodInterval * periodTime.div(1 days);
        }
    }
