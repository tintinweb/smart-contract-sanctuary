...
contract Auction {
    ...
    //nft小数位
    mapping(string => uint256) public n_decimal;
    //nft合约地址
    ...
    function push(string memory nft, uint256 lowestPrice, uint256 time, address settle, uint256 timeBuffer, uint256 percentage, uint256 decimal, address addr) public isOwner {
        ...
        n_decimal[nft] = decimal;
        n_addr[nft] = addr;
    }

    //解锁/锁定NFT
    function lock(string memory nft, uint8 lockType) public isOwner isExpired(nft) {
        if (lockType == 1) {
            n_lock[nft] = true;
        }
        if (lockType == 0) {
    ...
    //修改NFT截止时间
    function modify(string memory nft, uint256 time) public isOwner isLock(nft) isExpired(nft) {
        endTime[nft] = time;
    }

    //添加/移除黑名单
    function lockAddr(string memory nft, address addr, uint8 processType) public isOwner isLock(nft) isExpired(nft) {
        if (processType == 1) {
    ...
    function bid(string memory nft, uint256 amount) public payable isLock(nft) isExpired(nft) {
        ...
        if (bidder != address(0)) {
            if (n_settle[nft] == address(0)) {
                ...
                IERC20(n_settle[nft]).transfer(bidder, oldPrice * (10 ** n_decimal[nft]));
            }
        }
        if (n_settle[nft] != address(0)) {
            IERC20(n_settle[nft]).transferFrom(msg.sender, address(this), amount * (10 ** n_decimal[nft]));
        }
    ...
    function AuctionEnd(string memory nft) public payable {
        ...
        if (n_price[nft] != 0 && last_bidder[nft] != address(0)) {
            //转账
            IERC721(n_addr[nft]).safeTransferFrom(address(this), last_bidder[nft], nft);
        }
    }

    function withDraw(address contractAddr, address _to, uint256 amount, uint256 decimal) public payable isOwner {
        if (contractAddr == address(0)) {
            payable(_to).transfer(amount);
        } else {
            IERC20(contractAddr).transfer(_to, amount * (10 ** decimal));
        }
    }
}