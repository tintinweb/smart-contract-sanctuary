...
contract EscrowContract is BasicToken {
    ...
    function EscrowContract (address creator,address beneficiary,uint date) {
        creator_ = creator;
        beneficiary_ = beneficiary;
        date_ = date;
    }
    
    function executeBeneficiary(uint amount_) onlyBeneficiary onlyMatureEscrow {
        BasicToken.transfer(beneficiary_,amount_);
    }
    
    function executeCreator(uint amount_) onlyBeneficiary onlyMatureEscrow {
        BasicToken.transfer(creator_,amount_);
    }
...
contract Bittwatt is Token,Claimable, PausableToken {
    ...
    function createEscrow(address _beneficiary, uint _amount, uint _date) public {
        address escrowContract = new EscrowContract(msg.sender,_beneficiary,_date);
