...
library AddressM {
    ...
    function sendValue(address payable recipient, uint256 amount) internal {
        require(address(this).balance >= amount, "AddressM: insufficient balance");

        // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
        (bool success, ) = recipient.call{ value: amount }("");
        require(success, "AddressM: unable to send value, recipient may have reverted");
    }
    ...
    function functionCallWithValue(address target, bytes memory data, uint256 value) internal returns (bytes memory) {
        return functionCallWithValue(target, data, value, "AddressM: low-level call with value failed");
    }
    ...
    function functionCallWithValue(address target, bytes memory data, uint256 value, string memory errorMessage) internal returns (bytes memory) {
        require(address(this).balance >= value, "AddressM: insufficient balance for call");
...
library SafeERC20M {
    ...
    function safeTransfer(IERC20M token, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(IERC20M token, address from, address to, uint256 value) internal {
     ...
     */
    function safeApprove(IERC20M token, address spender, uint256 value) internal {
    ...
    function safeIncreaseAllowance(IERC20M token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).add(value);
        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(IERC20M token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).sub(value, "SafeERC20M: decreased allowance below zero");
        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }
    ...
    function _callOptionalReturn(IERC20M token, bytes memory data) private {
        ...
        bytes memory returndata = address(token).functionCall(data, "SafeERC20M: low-level call failed");
        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "SafeERC20M: ERC20 operation did not succeed");
        }
    }
}
...
contract OwnableM is ContextM {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableM: new owner is the zero address");
...
contract ERC20M is ContextM, IERC20M {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC20M: decreased allowance below zero"));
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(recipient != address(0), "ERC20M: transfer to the zero address");

        _beforeTokenTransfer(sender, recipient, amount);

        _balances[sender] = _balances[sender].sub(amount, "ERC20M: transfer amount exceeds balance");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _balances[account] = _balances[account].sub(amount, "ERC20M: burn amount exceeds balance");
...
contract MasterChef is OwnableM {
    ...
    function add(uint256 _allocPoint, IERC20M _lpToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
