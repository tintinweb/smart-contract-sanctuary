...
contract MyStake is Ownable,Pausable,ReentrancyGuard {
    ...
    mapping (address => ClientInfo[]) public clientInfos;
    ...
    function returnToken(uint _amount) public nonReentrant{
        ...
        require(
            ERC20Interface.transfer(staker, _amount) == true,
    ...
    function getRewardPercent() public view returns (uint){
    ...
    function uploadInfo(string memory cpu,string memory gpu,string memory str,string memory mem) public {
        ClientInfo memory clientInfo;
        clientInfo.cpu = cpu;
        clientInfo.gpu = gpu;
        clientInfo.str = str;
        clientInfo.mem = mem;
        clientInfos[msg.sender].push(clientInfo);
    }
