...
contract Crowdsale is Ownable { 
  ...
  uint256 public hardCap;//USD 
  uint256 public softCap;//USD  
  uint256 public tokensForSale;
  uint256 public tokensForPrivateSale;
  uint256 public tokensForPreSale;
  uint256 public tokensForPublicSale;
  
   //tokens per dollar in each sale 
  uint256 public privateSaleTokensPerDollar;
  uint256 public preSaleTokensPerDollar;
  uint256 public publicSaleTokensPerDollar;
  
  //Sale minimum maximum values
  uint256 public minimumUsdCentsInPrivateSale;
  uint256 public MaximumUsdCentsInPrivateSale;
  uint256 public minimumUsdCentsInPreSale;
  uint256 public MaximumUsdCentsInPreSale;
  uint256 public minimumUsdCentsInPublicSale;
  uint256 public MaximumUsdCentsInPublicSale;
  ...
  bool public Paused;
    ...
    */
    function authorizeKyc(address[] calldata addrs) public onlyOwner returns (bool success) {
   ...
   */
   function _preValidatePurchase(uint256 _usdCents) internal view { 
      ...
      if (currentStage == Stages.PrivateSaleStart) {
        require(_usdCents >= minimumUsdCentsInPrivateSale);
        require(_usdCents <= MaximumUsdCentsInPrivateSale);
      }
      else if (currentStage == Stages.PreSaleStart) {
        require(_usdCents >= minimumUsdCentsInPreSale);
        require(_usdCents <= MaximumUsdCentsInPreSale);

      }
      else if (currentStage == Stages.PublicSaleStart) {
        require(_usdCents >= minimumUsdCentsInPublicSale);
        require(_usdCents <= MaximumUsdCentsInPublicSale);
   ...
   */
    ...
    */
    function _getTokenAmount(uint256 _usdCents) CrowdsaleStarted internal view returns (uint256) {
      uint256 tokens;
      if (currentStage == Stages.PrivateSaleStart) {
         tokens = _usdCents.mul(privateSaleTokensPerDollar * 1 ether);
      }
      else if (currentStage == Stages.PreSaleStart) {
         tokens = _usdCents.mul(privateSaleTokensPerDollar * 1 ether);
      }
