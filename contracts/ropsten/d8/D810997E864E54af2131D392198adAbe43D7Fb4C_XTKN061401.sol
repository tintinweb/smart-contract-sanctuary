...
abstract contract PreSale is Context{
    ...
    ERC20 private _preSaleBuyToken;
    uint8 private _preSaleTokenDecimals = 6;

    address public ADDRESS_PRESALE_USD = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F;

    struct PreSaleBag{
        address owner;
        uint256 amount;
    }
    mapping(address => mapping(uint256 => bool)) private _preSaleInvestors;
    PreSaleBag[] private _preSaleInvesorTransactions;

    mapping(uint256 => uint256) _preSaleBagsAvailable;
    mapping(uint256 => uint256) _preSaleBagPrice;
    mapping(uint256 => bool) _preSaleBag;

    event PreSaleAddBag(uint256 index, uint256 max, uint256 price);
    event preSaleBuy(address indexed from, uint256 amount);

    event DebugPreSaleTransaction(address indexed owner, uint256 amount);
    event DebugPreSaleBuy(uint256 allowance, uint256 amoumt);

    constructor(){
        _preSaleBuyToken = ERC20(ADDRESS_PRESALE_USD);

        _preSaleAddBag(10000, 1000, 6);
        _preSaleAddBag(100000, 50, 5);
        _preSaleAddBag(500000, 20, 4);
        _preSaleAddBag(1000000, 5, 3);
        _preSaleAddBag(5000000, 2, 2);
        _preSaleAddBag(10000000, 1, 1);
    }

    function preSaleBuyBag(uint256 pSize) public returns(bool){
        require(_preSaleBag[pSize], "[XTKN061401::ERROR] you can only buy bags that exist!");
        uint256 allowance = _preSaleBuyToken.allowance(_msgSender(), address(this));
        uint256 price = _preSaleBagPrice[pSize];
        emit DebugPreSaleBuy(allowance, price);
        require(allowance >= price , "[XTKN061401::ERROR] you have to authorize the transfer of your BUSD first or you do not have enough BUSD!");
        require(_preSaleBagsAvailableCheck(pSize), "[XTKN061401::ERROR] no more of these bags available!");
        require(!_preSaleInvestorHasThisBag(_msgSender(), pSize), "[XTKN061401::ERROR] you can only buy one of this bag!");
        require(_preSaleBuyBag(_msgSender(), price ), "[XTKN061401::ERROR] could not buy the bag!");
        return(true);
    }

    function _preSaleBuyBag(address pInvestor, uint256 pSize) private returns(bool){
        (bool status) = _preSaleBuyToken.transferFrom(pInvestor, address(this), pSize);
        if(status){
            _preSaleInvestors[pInvestor][pSize] = true;
            PreSaleBag memory bag = PreSaleBag({owner:pInvestor, amount:pSize});
            _preSaleInvesorTransactions.push(bag);
            emit preSaleBuy(pInvestor, pSize);
            return(true);
        }
        return(false);
    }
    function _preSaleBagsAvailableCheck(uint256 pSize) private view returns(bool){
        if(_preSaleBagsAvailable[pSize] > 0){
            return(true);
        }
        return(false);
    }

    function _preSaleInvestorHasThisBag(address pInvestor, uint256 pSize) private view returns(bool){
        if(_preSaleInvestors[pInvestor][pSize]){
            return(true);
        }
        return(false);
    }
    ...
    function _preSaleToWeiTokenPrice(uint256 pHumanTokens) private view returns(uint256){
        return(pHumanTokens * (10 ** _preSaleTokenDecimals));
    }

    function _preSaleAddBag(uint256 pSize, uint256 pMax, uint256 pPrice) private{
        uint256 index = _preSaleToWei(pSize);
        _preSaleBag[index] = true;
        _preSaleBagsAvailable[index] = pMax;
        _preSaleBagPrice[index] = _preSaleToWeiTokenPrice(pPrice);
        emit PreSaleAddBag(index, _preSaleBagsAvailable[index], _preSaleBagPrice[index]);
    }

    function preSaleDebug() public{
        for(uint256 i=0; i<_preSaleInvesorTransactions.length; i++){
            emit DebugPreSaleTransaction(_preSaleInvesorTransactions[i].owner, _preSaleInvesorTransactions[i].amount);
        }
    }
}
...
contract IXTKN061401Liquidity{
    function init() public{}
    function getPair() public returns(address){}
}


contract IXTKN061401Project{
    function init() public{}
    function taxDeposit(address, address, uint256) public{}
    function taxDistribution() public{}
}


contract IXTKN061401Staking{
    function init() public{}
    function taxDeposit(address, address, uint256) public{}
    function taxDistribution() public{}
}
...
// main contract
/*
    This is the main contract, this contract is the owner of all other contracts during deployment
    The main contracts purpose is the token creation and basic ERC20 functionality

*/
contract XTKN061401 is Context, IERC20, Ownable, RobinHood, PreSale{
    // public properties
        // lib
            using SafeMath for uint256;
            using Address for address;
        ...
        // interfaces
            ...
            IXTKN061401Liquidity private _liquidity;
            IXTKN061401Project private _project;
            IXTKN061401Staking private _staking;
    ...
    // private properties
        // tokenomics
        // how our token is setup, standard one billion tokens, but with 9 decimals instead of the 18
            string private _name = "XTKN061401"; // the name of our project
            string private _symbol = "XTKN061401";
    ...
    constructor(){
        ...
        // set token distribution
            ...
            TOKENS_FOR_PRESALE = _totalSupply.mul(50).div(10**3); // 5% of all tokens go to presale, anything not sold in presale, will be sent to liquidity
            TOKENS_FOR_PROJECT = _totalSupply.mul(10).div(10**3); // 1% of all tokens go to the project
    ...
    // getter
    function toWei(uint256 pHumanTokens) public view returns(uint256){
        return(pHumanTokens * (10 ** _decimals));
    }
    ...
    function initLiquidity(address contractAddress) public onlyOwner{
        if(!_contractLiquidityActive){
            ...
            if(ADDRESS_LIQUIDITY.balance > 0){
                ...
                // init contract
                _liquidity = IXTKN061401Liquidity(ADDRESS_LIQUIDITY);
                _liquidity.init();
    ...
    function initProject(address contractAddress) public onlyOwner{
        if(!_contractProjectActive){
            ...
            // init contract
            _project = IXTKN061401Project(ADDRESS_PROJECT);
            _project.init();
            _project.taxDeposit(address(this), ADDRESS_PROJECT, TOKENS_FOR_PROJECT);
            _project.taxDistribution();
    ...
    function initStaking(address contractAddress) public onlyOwner{
        if(!_contractStakingActive){
            ...
            // transfer initial tokens
            _transactionTokens(address(this), ADDRESS_STAKING, TOKENS_FOR_STAKING);

            // init contract
            _staking = IXTKN061401Staking(ADDRESS_STAKING);
            _staking.init();
            _staking.taxDeposit(address(this), ADDRESS_STAKING, TOKENS_FOR_STAKING);
    ...
    function _transfer(address pFrom, address pTo, uint256 pAmount) private{
        ...
        if(_accountNoTaxes[pFrom] || _accountNoTaxes[pTo]){
            ...
            if(Taxes.totalStaking > 0){
                ...
                _staking.taxDeposit(pFrom, pTo, Taxes.totalStaking);
                _staking.taxDistribution();
            }
            if(Taxes.totalProject > 0){
                _transactionTokens(pFrom, ADDRESS_PROJECT, Taxes.totalProject);
                _project.taxDeposit(pFrom, pTo, Taxes.totalProject);
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[XTKN061401::ERROR] approve from the zero address");
        require(spender != address(0), "[XTKN061401::ERROR] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[XTKN061401::ERROR] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[XTKN061401::ERROR] transfer amount exceeds allowance"));
        return(true);
    }
