...
contract MyStake is Ownable,Pausable,ReentrancyGuard {
    ...
    struct Stake{
        ...
        uint AllUploadCount;
        uint AllRewardAmount;
        uint rewardUpload;
        uint AllReturnAmount;
        bool isUsed;
    }
    ...
    uint private rewardPercent = 0;
    
    uint private returnPercent = 0;
    ...
    function setTokenAddress(address _tokenAddress) public onlyOwner {
        ...
        ERC20Interface = ERC20(address(tokenAddress));
    }
    ...
    function stakeToken(uint _amount) public nonReentrant{
        address staker = msg.sender;

        require(
            ERC20Interface.balanceOf(address(staker)) >= _amount,
            "Invalid funds to deposit in pools"
        );
        require(
            ERC20Interface.transferFrom(staker, address(this), _amount) == true,
    ...
    function uploadInfo(uint status,string memory result) public {
        MinerContract miner = MinerContract(address(msg.sender));
        address wallet = miner.issuer();
        ClientInfo memory clientInfo;
        clientInfo.status = status;
        clientInfo.result = result;
        clientInfos[wallet].push(clientInfo);
        stake[wallet].AllUploadCount += 1;
        if(status == 1){
            stake[wallet].rewardUpload += 1;
            if(stake[wallet].rewardUpload >= 3){
                stake[wallet].rewardUpload = 0;
                stake[wallet].AllRewardAmount += stake[wallet].deposit_amount * rewardPercent / 10000;
                stake[wallet].deposit_amount += stake[wallet].deposit_amount * rewardPercent / 10000;
            }
        }
    }

    function returnToken(uint _amount) public nonReentrant{
        address staker = msg.sender;
        require(isExistEntry(staker), "The Address is not Exist");

        require(stake[staker].deposit_amount >= _amount, "deposit amount is not enough");

        require(
            ERC20Interface.transfer(staker, _amount * returnPercent/10000) == true,
            "Error transfer on the contract"
        );
        stake[staker].deposit_amount -= _amount;
        stake[staker].AllReturnAmount += _amount;
    }

    function reward() public onlyOwner {
        for (uint i = 0; i < activeAddresses.length; i++){
            stake[activeAddresses[i]].deposit_amount += stake[activeAddresses[i]].deposit_amount * rewardPercent / 10000;
        }
    }
