...
/**
* @title XYZB ICO Contract
* @dev XYZB is an ERC-20 Standar Compliant Token
*/
...
contract XYZBICO is admined {
    ...
    uint256 public ICO1Start = PRESALEStart.add(30 days);
    uint256 public ICO2Start = ICO1Start.add(30 days);
    uint256 public SaleDeadline = ICO2Start.add(30 days);
    ...
    */
    function contribute(address _target, uint256 _value) public notFinished payable {
        require(now > PRESALEStart && USDPriceInWei !=0); //Current time must be equal or greater than the start time
    ...
    */
    function tokenBuyCalc(uint _value) internal view returns (uint sold,uint remaining) {
      ...
      uint256 tempPrice = USDPriceInWei; //0.0001$ in wei

      if (state == State.PRESALE){

        tempPrice = tempPrice.mul(8500); //0.85$
        sold = _value.div(tempPrice);

        return (sold,0);

      } else if (state == State.ICO1){

        tempPrice = tempPrice.mul(9000); //0.90$
        sold = _value.div(tempPrice);

        return (sold,0);

      } else if (state == State.ICO2){

        tempPrice = tempPrice.mul(9500); //0.95$
        sold = _value.div(tempPrice);
    ...
    */
    function checkIfFundingCompleteOrExpired() public {
        ...
        } else if(state == State.PRESALE && now > ICO1Start){

          state = State.ICO1;

        } else if(state == State.ICO1 && now > ICO2Start){
    ...
    */
    function successful() public {
        ...
        tokenReward.transfer(creator,temp); //Try to transfer tokens
