...
*/        
    ...
    function _swap(address payable sender, address underlying, address currency, uint priceFloor, uint priceCap, int dCall, int dPut, int undMax, int curMax, bytes memory data) internal nonReentrant returns (address call, address put, int dUnd, int dCur) {
        ...
        //_checkMistakeETH(sender, underlying, currency, dUnd, dCur);
        {
            ...
            if(dUnd < 0)
                Call(call).withdraw_(tos[2], uint(-dUnd).mul(uint(1e18).sub(feeRate)).div(1e18));
            if(dCur < 0)
                Put( put ).withdraw_(tos[3], uint(-dCur).mul(uint(1e18).sub(feeRate)).div(1e18));
        }
        if(data.length > 0)
            _onFlashSwap(sender, call, put, dCall, dPut, dUnd, dCur, data);
        else {
    ...
    function _onFlashSwap(address payable sender, address call, address put, int dCall, int dPut, int dUnd, int dCur, bytes memory data) internal {
        IFlashSwapCallee(_msgSender()).onFlashSwap{value: msg.value}(sender, call, put, dCall, dPut, dUnd, dCur, data);
    }

    function _transferFrom(address sender, address callOrPut, address undOrCur, int vol) internal {
        IERC20(undOrCur).safeTransferFrom(sender, callOrPut, uint(vol).mul(feeRate.add(uint(1e18))).div(1e18));
    }
/*
...
interface IFlashSwapCallee {
    function onFlashSwap(address sender, address call, address put, int dCall, int dPut, int dUnd, int dCur, bytes memory data) external payable;
}
...
contract Router is Configurable, ContextUpgradeSafe, Constants {
    ...
    function calcAmountRoute(int amount, address[] memory path) public view returns (int) {
        if(path.length > 0)
            if(amount > 0)
                return int(UniswapV2Library.getAmountsIn(swapFactory, uint(amount), path)[0]);
            else if(amount < 0)
                return -int(UniswapV2Library.getAmountsOut(swapFactory, uint(-amount), revertPath(path))[path.length-1]);
        return amount;
    }
    ...
    function _routeIn( uint amountOut, uint amountInMax, address[] memory path, address to, uint deadline) internal ensure(deadline) returns (uint[] memory amounts) {           // virtual override 
        address WETH_ = WETH;
        amounts = UniswapV2Library.getAmountsIn(swapFactory, amountOut, path);
        ...
        if(msg.value > 0 && path[0] == WETH_) {
            ...
            if(msg.value > amounts[0])
                _msgSender().transfer(msg.value - amounts[0]);
            IERC20(path[0]).safeTransfer(pair0, amounts[0]);
        } else
            IERC20(path[0]).safeTransferFrom(_msgSender(), pair0, amounts[0]);
        _route(amounts, path, to);
    }
    
    function _routeOut(uint amountIn, uint amountOutMin, address[] memory path, address to, uint deadline) internal ensure(deadline) returns (uint[] memory amounts) {           // virtual override 
    ...
    function _transfer(address payable sender, address callOrPut, address undOrCur, int vol, int max, address[] memory path) internal {
        ...
        if(vol > 0) {
            ...
            if(path.length == 0) {
                ...
                require(path[path.length - 1] == undOrCur, 'INVALID_PATH');
                _routeIn(v, uint(-max), path, callOrPut, deadline);
            }
        } else if(vol < 0) {
            uint v = uint(-vol).sub(fee);
            if(path.length == 0)
                ...
                path = revertPath(path);
                require(path[0] == undOrCur, 'INVALID_PATH');
                _routeOut(v, uint(-max), path, (path[path.length-1] == WETH_ ? address(this) : sender), deadline);
            }
            if(path.length == 0 && undOrCur == WETH_ || path.length > 0 && path[path.length-1] == WETH_) {
                IWETH(WETH_).withdraw(v);
                sender.transfer(v);
            }
        }
    ...
    function _genData() internal view returns (bytes memory data) {
        ...
        address undTo = undPath.length > 0 ? IUniswapV2Factory(swapFactory).getPair(undPath[undPath.length-1], undPath[undPath.length-2]) : _msgSender();
        address curTo = curPath.length > 0 ? IUniswapV2Factory(swapFactory).getPair(curPath[curPath.length-1], curPath[curPath.length-2]) : _msgSender();
    ...
    function swap(address underlying, address currency, uint priceFloor, uint priceCap, int dCall, int dPut, int undMax, int curMax, address[] memory undPath, address[] memory curPath) external payable returns (address call, address put, int dUnd, int dCur) {
        _checkMistakeETH(_msgSender(), underlying, currency, dUnd, dCur, undPath, curPath);
    ...
    function onFlashSwap(address payable sender, address call, address put, int dCall, int dPut, int dUnd, int dCur, bytes memory data) external payable {
        require(_msgSender() == factory, 'only Factory');
...
library UniswapV2Library {
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        ...
        amountIn = (numerator / denominator).add(uint(1));
    }
