...
contract IronTradeRoute is Ownable, IERC721Receiver, Pausable {
  ...
  uint256 public ironPerAlpha = 0; 

  // merchant earn 10000 resource per day
  uint256 public constant DAILY_IRON_RATE = 200000 ether;
  ...
  uint256 public constant IRON_CLAIM_TAX_PERCENTAGE = 20;
  // there will only ever be (roughly) 2.4 billion resources earned through staking
  uint256 public constant MAXIMUM_GLOBAL_IRON = 2400000000 ether;

  // amount of resources earned so far
  uint256 public totalIronEarned;
   ...
   * @param _iron reference to the $IRON token
   */
  constructor(address _pirate, address _iron) { 
  ...
  /**
   * adds Merchants and Pirates to the Trade Route 
  ...
  function _addPirateToFleet(address account, uint256 tokenId) internal {
    ...
    fleet[alpha].push(Stake({
      owner: account,
      tokenId: uint16(tokenId),
      value: uint80(ironPerAlpha)
    })); // Add the pirate to the Fleet
    emit TokenStaked(account, tokenId, ironPerAlpha);
  }

  /** CLAIMING / UNSTAKING */

  /**
   * realize $IRON earnings and optionally unstake tokens from the Route / Fleet
   * to unstake a Merchant it will require it has 2 days worth of $IRON unclaimed
  ...
  function _claimMerchantFromRoute(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (totalIronEarned < MAXIMUM_GLOBAL_IRON) {
      owed = (block.timestamp - stake.value) * DAILY_IRON_RATE / 1 days;
    } else if (stake.value > lastClaimTimestamp) {
      owed = 0; // resource production stopped already
    } else {
      owed = (lastClaimTimestamp - stake.value) * DAILY_IRON_RATE / 1 days; // stop earning additional resource if it's all been earned
    }
    if (unstake) {
      ...
      _payPirateTax(owed * IRON_CLAIM_TAX_PERCENTAGE / 100); // percentage tax to staked pirates
      owed = owed * (100 - IRON_CLAIM_TAX_PERCENTAGE) / 100; // remainder goes to Merchants owner
      tradeRoute[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
  ...
  function _claimPirateFromFleet(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    require(stake.owner == _msgSender(), "SWIPER, NO SWIPING");
    owed = (alpha) * (ironPerAlpha - stake.value); // Calculate portion of tokens based on Alpha
    if (unstake) {
  ...
  function _payPirateTax(uint256 amount) internal {
    ...
    ironPerAlpha += (amount + unaccountedRewards) / totalAlphaStaked;
  ...
  function randomPirateOwner(uint256 seed) external view returns (address) {
    ...
    uint256 cumulative;
    seed >>= 32;
    // loop through each bucket of Pirates with the same alpha score
...
contract Pirate is IPirate, ERC721Enumerable, Ownable, Pausable {
  ...
  mapping(uint256 => MerchantPirate) public tokenTraits;
  // mapping from hashed(tokenTrait) to the tokenId it's associated with
  // used to ensure there are no duplicates
  mapping(uint256 => uint256) public existingCombinations;

  // list of probabilities for each trait type
  // 0 - 9 are associated with Merchants, 10 - 18 are associated with Pirates
  uint8[][18] public rarities;
  // list of aliases for Walker's Alias algorithm
  // 0 - 9 are associated with Merchants, 10 - 18 are associated with Pirates
  uint8[][18] public aliases;

  // reference to the TradeRoute for choosing random Pirate thieves
  ITradeRoute public ironTradeRoute;
   // reference to the TradeRoute for choosing random Pirate thieves
  ITradeRoute public woodTradeRoute;
   // reference to the TradeRoute for choosing random Pirate thieves
  ITradeRoute public linenTradeRoute;
  // reference to $WOOL for burning on mint
//   WOOL public wool;
   // reference to $WOOD for burning on mint
  WOOD public wood;
   // reference to $IRON for burning on mint
  IRON public iron;
   // reference to $LINEN for burning on mint
  LINEN public linen;
  // reference to Traits
   ...
   */
  constructor(address _iron, address _wood, address _linen, address _traits, uint256 _maxTokens) ERC721("Pirate Game", 'PGAME') { 
    iron = IRON(_iron);
    wood = WOOD(_wood);
    linen = LINEN(_linen);
  ...
  /** 
   * mint a token - 90% Merchant, 10% Pirates
   * The first 20% are free to claim, the remaining cost $WOOL
   */
  function mint(uint256 amount, uint256 stakeDestination) external payable whenNotPaused {
    require(tx.origin == _msgSender(), "Only EOA");
    require(minted + amount <= MAX_TOKENS, "All tokens minted");
    require(amount > 0 && amount <= 10, "Invalid mint amount");
    require(stakeDestination > 0 && stakeDestination < 4, "Invalid destination");
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      address recipient = selectRecipient(seed, stakeDestination);
      if (stakeDestination == 0 || recipient != _msgSender()) {
        _safeMint(recipient, minted);
      } else if (stakeDestination == 1) {
        _safeMint(address(ironTradeRoute), minted);
        tokenIds[i] = minted;
      } else if (stakeDestination == 2) {
        _safeMint(address(linenTradeRoute), minted);
        tokenIds[i] = minted;
      } else if (stakeDestination == 3) {
        _safeMint(address(woodTradeRoute), minted);
        tokenIds[i] = minted;
      } else {
        _safeMint(recipient, minted);
      }
      totalResourceCost += mintCost(minted);
    }
    
    if (totalResourceCost > 0) {
        wood.burn(_msgSender(), totalResourceCost);
        iron.burn(_msgSender(), totalResourceCost);
        linen.burn(_msgSender(), totalResourceCost);
    } 
    if (stakeDestination == 1) {
        ironTradeRoute.addManyToTradeRouteAndFleet(_msgSender(), tokenIds);
    } else if (stakeDestination == 2) {
        linenTradeRoute.addManyToTradeRouteAndFleet(_msgSender(), tokenIds);
    } else if (stakeDestination == 3) {
        woodTradeRoute.addManyToTradeRouteAndFleet(_msgSender(), tokenIds);
    }
  }

  /** 
   * the first 20% are paid in ETH
   * the next 20% are 8000 resources 
   * the next 40% are 16000 resources
   * the final 20% are 32000 resources
   * @param tokenId the ID to check the cost of to mint
   * @return the cost of the given token ID
   */
  function mintCost(uint256 tokenId) public view returns (uint256) {
    if (tokenId <= PAID_TOKENS) return 0;
    if (tokenId <= MAX_TOKENS * 2 / 5) return 8000 ether;
    if (tokenId <= MAX_TOKENS * 4 / 5) return 16000 ether;
    return 32000 ether;
  }
  ...
  function transferFrom(
    ...
    // Hardcode the TradeRoutes's approval so that users don't have to waste gas approving
    if (_msgSender() != address(ironTradeRoute)
        || _msgSender() != address(woodTradeRoute)
        || _msgSender() != address(linenTradeRoute))
   ...
   */
  function generate(uint256 tokenId, uint256 seed) internal returns (MerchantPirate memory t) {
    t = selectTraits(seed);
   ...
   */
   function selectRecipient(uint256 seed, uint256 route) internal view returns (address) {
    if (route == 1) {
        return selectIronRecipient(seed);
    } else if (route == 2) {
        return selectLinenRecipient(seed);
    } else {
        return selectWoodRecipient(seed);
    }
  }

  function selectIronRecipient(uint256 seed) internal view returns (address) {
    if (minted <= PAID_TOKENS || ((seed >> 245) % 10) != 0) return _msgSender(); // top 10 bits haven't been used
    address thief = ironTradeRoute.randomPirateOwner(seed >> 144); // 144 bits reserved for trait selection
    if (thief == address(0x0)) return _msgSender();
    return thief;
  }

  function selectLinenRecipient(uint256 seed) internal view returns (address) {
    if (minted <= PAID_TOKENS || ((seed >> 245) % 10) != 0) return _msgSender(); // top 10 bits haven't been used
    address thief = linenTradeRoute.randomPirateOwner(seed >> 144); // 144 bits reserved for trait selection
    if (thief == address(0x0)) return _msgSender();
    return thief;
  }

  function selectWoodRecipient(uint256 seed) internal view returns (address) {
    if (minted <= PAID_TOKENS || ((seed >> 245) % 10) != 0) return _msgSender(); // top 10 bits haven't been used
    address thief = woodTradeRoute.randomPirateOwner(seed >> 144); // 144 bits reserved for trait selection
    if (thief == address(0x0)) return _msgSender();
    return thief;
  }
   ...
   */
  function selectTraits(uint256 seed) internal view returns (MerchantPirate memory t) {    
   ...
   */
  function structToHash(MerchantPirate memory s) internal pure returns (uint256) {
  ...
  function getTokenTraits(uint256 tokenId) external view override returns (MerchantPirate memory) {
  ...
  /**
   * called after deployment so that the contract can get random pirate thieves
   * @param _tradeRoute the address of the Trade route
   */
  function setIronRoute(address _tradeRoute) external onlyOwner {
    ironTradeRoute = ITradeRoute(_tradeRoute);
  }

  function setLinenRoute(address _tradeRoute) external onlyOwner {
    linenTradeRoute = ITradeRoute(_tradeRoute);
  }

  function setWoodRoute(address _tradeRoute) external onlyOwner {
    woodTradeRoute = ITradeRoute(_tradeRoute);
  }
...
contract LINEN is ERC20, Ownable {

  // a mapping from an address to whether or not it can mint / burn
  mapping(address => bool) controllers;
  
  constructor() ERC20("LINEN", "LINEN") { }

  /**
   * mints $LINEN to a recipient
   * @param to the recipient of the $LINEN
   * @param amount the amount of $LINEN to mint
   */
  function mint(address to, uint256 amount) external {
    require(controllers[msg.sender], "Only controllers can mint");
    _mint(to, amount);
  ...
  /**
   * burns $LINEN from a holder
   * @param from the holder of the $LINEN
   * @param amount the amount of $LINEN to burn
   */
  function burn(address from, uint256 amount) external {
    require(controllers[msg.sender], "Only controllers can burn");
    _burn(from, amount);
...
interface IPirate {
  ...
  function getTokenTraits(uint256 tokenId) external view returns (MerchantPirate memory);
}
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts v4.4.0-rc.1 (token/ERC721/extensions/IERC721Enumerable.sol)
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts v4.4.0-rc.1 (token/ERC721/extensions/ERC721Enumerable.sol)
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts v4.4.0-rc.1 (token/ERC20/extensions/IERC20Metadata.sol)
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts v4.4.0-rc.1 (token/ERC20/ERC20.sol)
