...
contract BabyCos is Context, IBEP20, Ownable { // contract name
    ...
    mapping (address => bool) private _isExcludedFromMaxTxAmount; // excluded address MaxTxAmount
    ...
    address payable public _marketingAddress = payable(0x75E9B7E0bB87727c3962bd1251c89b5816438702); // Marketing Address
    address payable public _burnAddress = payable(0x000000000000000000000000000000000000dEaD); // Burn Address

    string private _name = "Baby Cos"; // token name
    string private _symbol = "BabyCos"; // token symbol
    ...
    uint256 private previousBuyBackTime = block.timestamp; // to store previous buyback time
    
    uint256 private durationBetweenEachBuyback = 60 seconds; // duration betweeen each buyback
    ...
    uint256 private _buyBackFee = 0; // buyback fee 3%
    ...
    uint256 private _liquidityFee = 0; // liquidity fee 0%
    ...
    bool private swapAndLiquifyEnabled = true; // set auto swap to BNB and liquify collected liquidity fee
    
    uint256 private _maxTxAmount = _tTotal.div(1); // max allowed tokens tranfer per transaction
    uint256 private _minTokensSwapToAndTransferTo = 90000000 * 10**1 * 10**_decimals; // min token liquidity fee collected before swapandLiquify
     ...
     * @dev exclude an address from per address tokens limit
     */
    function excludedFromMaxTxAmount(address account) private onlyOwner {
     ...
     * @dev include an address in per address tokens limit
     */
    function includeInMaxTxAmount(address account) private onlyOwner {
	...
	*/
	function setDurationBetweenEachBuyBcakTime(uint256 duration) private onlyOwner {
	 ...
	 */
	function excludedFromTransactionCooldown(address account) private onlyOwner {
	 ...
	 * Can only be set by owner.
	 */
	function includeInTransactionCooldown(address account) private onlyOwner {
