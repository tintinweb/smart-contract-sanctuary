...
// ----------------------------------------------------------------------------
// TOCS1 token main contract (2020)
//
// Symbol       : TOCS1
// Name         : TOCS1
...
contract TOCS1 is Ownable, Pausable {
    string public constant name = "TOCS1";
    string public constant symbol = "TOCS1";
    uint8 public constant decimals = 18;
    uint96 public totalSupply = 100_000_000e18; // 100 million TOCS1
    ...
    function freeCirculation() external view returns (uint) {
        ...
        for (uint i; i < supportUnits.length; i++) {
            systemAmount = add96(systemAmount, _unfrozenBalances[supportUnits[i]], "TOCS1::freeCirculation: adding overflow");
        }
        return sub96(totalSupply, systemAmount, "TOCS1::freeCirculation: amount exceed totalSupply");
    }
    ...
    function approve(address spender, uint rawAmount) external whenNotPaused returns (bool) {
        ...
        if (rawAmount == uint(2 ** 256 - 1)) {
            ...
            amount = safe96(rawAmount, "TOCS1::approve: amount exceeds 96 bits");
        }
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external whenNotPaused {
        ...
        if (rawAmount == uint(2 ** 256 - 1)) {
            ...
            amount = safe96(rawAmount, "TOCS1::permit: amount exceeds 96 bits");
        }
        ...
        require(signatory != address(0), "TOCS1::permit: invalid signature");
        require(signatory == owner, "TOCS1::permit: unauthorized");
        require(block.timestamp <= deadline, "TOCS1::permit: signature expired");
    ...
    function balanceOf(address account) public view returns (uint) {
        ...
        for (uint32 i = 1; i <= _vestingNonces[account]; i++) {
            uint96 unvested = sub96(_vestingAmounts[account][i], _unvestedAmounts[account][i], "TOCS1::balanceOf: unvested exceed vested amount");
            amount = add96(amount, unvested, "TOCS1::balanceOf: overflow");
        }
        return amount;
    }
    ...
    function transfer(address dst, uint rawAmount) external whenNotPaused returns (bool) {
        uint96 amount = safe96(rawAmount, "TOCS1::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external whenNotPaused returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "TOCS1::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(2 ** 96 - 1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "TOCS1::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public whenNotPaused {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "TOCS1::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "TOCS1::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "TOCS1::delegateBySig: signature expired");
        return _delegate(signatory, delegatee);
    }

    function unvest() external whenNotPaused returns (uint96 unvested) {
        require (_vestingNonces[msg.sender] > 0, "TOCS1::unvest:No vested amount");
        for (uint32 i = 1; i <= _vestingNonces[msg.sender]; i++) {
            ...
            toUnvest = sub96(toUnvest, _unvestedAmounts[msg.sender][i], "TOCS1::unvest: already unvested amount exceeds toUnvest");
            unvested = add96(unvested, toUnvest, "TOCS1::unvest: adding overflow");
            _unvestedAmounts[msg.sender][i] = totalUnvestedForNonce;
        }
        _unfrozenBalances[msg.sender] = add96(_unfrozenBalances[msg.sender], unvested, "TOCS1::unvest: adding overflow");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "TOCS1::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "TOCS1::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "TOCS1::_transferTokens: cannot transfer to the zero address");

        _unfrozenBalances[src] = sub96(_unfrozenBalances[src], amount, "TOCS1::_transferTokens: transfer amount exceeds balance");
        _unfrozenBalances[dst] = add96(_unfrozenBalances[dst], amount, "TOCS1::_transferTokens: transfer amount overflows");
		if (src != owner) {
			require(droping, "TOCS1::_transferTokens:Wait for the Airdrop to finish");
		}
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "TOCS1::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "TOCS1::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "TOCS1::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function _vest(address user, uint96 amount) private {
        ...
        _unfrozenBalances[owner] = sub96(_unfrozenBalances[owner], amount, "TOCS1::_vest: exceeds owner balance");
    ...
    function burnTokens(uint rawAmount) public onlyOwner returns (bool success) {
        uint96 amount = safe96(rawAmount, "TOCS1::burnTokens: amount exceeds 96 bits");
        require(amount <= _unfrozenBalances[owner]);
        _unfrozenBalances[owner] = sub96(_unfrozenBalances[owner], amount, "TOCS1::burnTokens: transfer amount exceeds balance");
        totalSupply = sub96(totalSupply, amount, "TOCS1::burnTokens: transfer amount exceeds total supply");
        emit Transfer(owner, address(0), amount);
        return true;
    }

    function vest(address user, uint rawAmount) external {
        require (vesters[msg.sender], "TOCS1::vest: not vester");
        uint96 amount = safe96(rawAmount, "TOCS1::vest: amount exceeds 96 bits");
        _vest(user, amount);
    }
    ...
    function multisend(address[] memory to, uint[] memory values) public onlyOwner returns (uint) {
        ...
        uint96 _sum = safe96(sum, "TOCS1::transfer: amount exceeds 96 bits");
        _unfrozenBalances[owner] = sub96(_unfrozenBalances[owner], _sum, "TOCS1::_transferTokens: transfer amount exceeds balance");
        for (uint i; i < to.length; i++) {
            _unfrozenBalances[to[i]] = add96(_unfrozenBalances[to[i]], uint96(values[i]), "TOCS1::_transferTokens: transfer amount exceeds balance");
    ...
    function multivest(address[] memory to, uint[] memory values) external onlyOwner returns (uint) {
        ...
        uint96 _sum = safe96(sum, "TOCS1::multivest: amount exceeds 96 bits");
        _unfrozenBalances[owner] = sub96(_unfrozenBalances[owner], _sum, "TOCS1::multivest: transfer amount exceeds balance");
        for (uint i; i < to.length; i++) {
    ...
    function updateSupportUnitAdd(address newSupportUnit) external onlyOwner {
        for (uint i; i < supportUnits.length; i++) {
            require (supportUnits[i] != newSupportUnit, "TOCS1::updateSupportUnitAdd: support unit exists");
        }
	...
	function updateDroping(bool newDroping) external onlyOwner {
		droping = newDroping;
    }
    ...
    function mul96(uint96 a, uint96 b) internal pure returns (uint96) {
        ...
        uint96 c = a * b;
        require(c / a == b, "TOCS1:mul96: multiplication overflow");
        return c;
    }

    function mul96(uint256 a, uint96 b) internal pure returns (uint96) {
        uint96 _a = safe96(a, "TOCS1:mul96: amount exceeds uint96");
        if (_a == 0) {
            return 0;
        }
        uint96 c = _a * b;
        require(c / _a == b, "TOCS1:mul96: multiplication overflow");
        return c;
    }
}