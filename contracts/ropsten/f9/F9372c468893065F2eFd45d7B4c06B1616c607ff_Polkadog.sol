...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 private constant maxTokensToSell = 10 * 10**18;
    ...
    constructor (address[] memory marketing_address_) {
        ...
        require(marketing_address_.length == 10, "Address cant be zero address");
    ...
    function transferToFeeWallet() public {
        ...
        swapTokensForEth(_marketingFeeBalance, marketing_address[marketing_address_index]);
        if(marketing_address_index == 9) {
    ...
    function _swapAndLiquify(address from) internal {
        if(_marketingFeeBalance >= maxTokensToSell) {
            transferToFeeWallet();
        }
