...
/**
 * @title Staking
 * @dev   Staking Contract for LP token staking
 */
contract Staking {
  ...
  */
  ...
  uint256 public TOKEN_REWARD_PERCENT_SILVER       = 6791140;       // variable constant to manage token reward percentage for silver
  uint256 public TOKEN_REWARD_PERCENT_GOLD         = 18720669;      // variable constant to manage token reward percentage for gold
  uint256 public TOKEN_REWARD_PERCENT_PLATINUM     = 35790606;      // variable constant to manage token reward percentage for platinum
  uint256 public TOKEN_PENALTY_PERCENT_SILVER      = 3395570;       // variable constant to manage token penalty percentage for silver
  uint256 public TOKEN_PENALTY_PERCENT_GOLD        = 6552234;       // variable constant to manage token penalty percentage for silver
  uint256 public TOKEN_PENALTY_PERCENT_PLATINUM    = 8947651;       // variable constant to manage token penalty percentage for silver
  ...
  function withdrawStakedTokens(uint256 stakingId) public returns(bool) {
    ...
    if(_tokenTotalDays[stakingId] == PERIOD_SILVER){
          ...
          if(now >= _tokenStakingEndTime[stakingId]){
              ...
              itoken.transferFrom(address(this), msg.sender,_usersTokens[stakingId]);
              totalTokenStakesInContract = totalTokenStakesInContract.sub(_usersTokens[stakingId]);
            }
