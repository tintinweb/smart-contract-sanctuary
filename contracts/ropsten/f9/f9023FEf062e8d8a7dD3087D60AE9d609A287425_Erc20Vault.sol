...
contract Erc20Vault is Withdrawable, IERC777Recipient, claimToken {
        ...
        returns ( bool)
    {
        ...
        require(supportedTokens.contains(td.tokenAddress), "Token is not supported");
        // TODO require the toAddress is the same as the one in the event
        // TODO change quantity to wei amount with correct decimals
        uint precision = 18 - ERC20(td.tokenAddress).decimals();
        // uint precision = 18 - 8;
        uint quantity = td.quantity * 10**precision;
        // uint balance = IERC20(td.tokenAddress).balanceOf(address(this));
        // //Receive funds
        if (td.tokenAddress == address(weth)) {
            pegOutWeth(payable(td.toAddress), quantity, "");
        } else {
            IERC20(td.tokenAddress).safeTransfer(td.toAddress, td.quantity);
        }
...
contract claimToken is Verify, Oracled {
    ...
    //     uint256 chainId
    // );
    event Claimed(uint64 id, address toAddress, address tokenAddress, uint256 amount);
        ...
        returns (TeleportData memory)
    {
        ...
        assembly {
            ...
            tokenAddress := mload(add(add(sigData, 0x14), 59))
        ...
        returns ( TeleportData memory)
    {
        ...
        // verify signatures
        require(sigData.length == 77, "Signature data is the wrong size");
        require(
