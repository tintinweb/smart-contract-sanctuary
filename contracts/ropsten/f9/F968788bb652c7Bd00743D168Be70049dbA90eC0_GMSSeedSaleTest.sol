...
contract GMSSeedSaleTest {
    ...
    constructor(
        ...
        require(_owner != address(0), "GMSSeedSaleTest: Owner is a zero address");
        require(_cap > 0, "GMSSeedSaleTest: Cap is 0");
        require(_duration > 0, "GMSSeedSaleTest: Duration is 0");
        require(_startTime + _duration > block.timestamp, "GMSSeedSaleTest: Final time is before current time");
    ...
    modifier onlyWhitelisted() {
        require(!whitelistedOnly || whitelisted[msg.sender], "GMSSeedSaleTest: Account is not whitelisted");
        _;
    }

    modifier isOngoing() {
        require(isLive(), "GMSSeedSaleTest: Sale is not active");
        _;
    }
    ...
    modifier isEnded() {
        require(_ended || block.timestamp > startTime + duration, "GMSSeedSaleTest: Not ended");
        _;
    }
    ...
    function buyWith(address stableCoinAddress, uint256 amount) external isOngoing onlyWhitelisted {
        require(stableCoins.contains(stableCoinAddress), "GMSSeedSaleTest: Stable coin not supported");
        require(amount > 0, "GMSSeedSaleTest: Amount is 0");
        require(_isBalanceSufficient(amount), "GMSSeedSaleTest: Insufficient remaining amount");
        require(amount + balances[msg.sender] >= minPerAccount, "GMSSeedSaleTest: Amount too low");
        require(maxPerAccount == 0 || balances[msg.sender] + amount <= maxPerAccount, "GMSSeedSaleTest: Amount too high");

        uint8 decimals = IERC20(stableCoinAddress).safeDecimals();
        uint256 stableCoinUnits = amount * (10**(decimals-2));

        // solhint-disable-next-line max-line-length
        require(IERC20(stableCoinAddress).allowance(msg.sender, address(this)) >= stableCoinUnits, "GMSSeedSaleTest: Insufficient stable coin allowance");
    ...
    function endPresale() external onlyOwner {
        require(collected >= cap, "GMSSeedSaleTest: Limit not reached");
        _ended = true;
    }
    ...
    function changeOwner(address _newOwner) external onlyOwner {
        require(_newOwner != address(0), "GMSSeedSaleTest: New Owner is a zero address");
        newOwner = _newOwner;
    }

    function acceptOwnership() external {
        require(msg.sender == newOwner, "GMSSeedSaleTest: Only new Owner");
        newOwner = address(0);
    ...
    receive() external payable {
        require(allowETH, "GMSSeedSaleTest: Revert on all ETH transfers");
    }
}
