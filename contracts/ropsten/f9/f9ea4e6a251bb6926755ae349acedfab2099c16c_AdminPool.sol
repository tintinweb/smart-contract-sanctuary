...
contract AdminPool {
    ...
    function startPool() public payable {
        ...
        //todo.. if adminContribution > 0 so pay back to admin first...as pool is restarted
    ...
    function claimReward () public {
        ...
        for(uint i = 0; i < count; i++) {
            if(msg.sender == leaderboard[i].user) {
                ...
                // else {
                //     require(leaderboard[i].isClaimed == false, "Already claimed. Take care");
                // }
            }
        }
        // if (userFound == 0) {
        //     require(userFound == 1, "User score not found in pool");
        // }
    }
}