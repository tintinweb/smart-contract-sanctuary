...
contract MatchLog is WhitelistAdminRole, Pausable {
    ...
    public onlyWhitelistAdmin whenNotPaused
  {
    ...
    ////validateMatches(_matches);

    // Calculate sum of matches
    ////uint64 sum = calSum(_matches);
  ...
  /**
    ...
    */
  function addDataMulti(Datas[] memory datas)
    public onlyWhitelistAdmin whenNotPaused
  {
    for (uint256 i = 0; i < datas.length; i++) {
      addData(datas[i]._supplyPointId, datas[i]._yearMonth, datas[i]._totalWh, datas[i]._hash);
    }
  }
  ...
  /**
    ...
    */
  function reviseDataMulti(Datas[] memory datas)
    public onlyWhitelistAdmin whenNotPaused
  {
    for (uint256 i = 0; i < datas.length; i++) {
      reviseData(datas[i]._supplyPointId, datas[i]._yearMonth, datas[i]._totalWh, datas[i]._hash);
    }
  }
  ...
  /*function validateMatches(uint32 [] memory _matches) internal {
    require(_matches.length != 0 && _matches.length % 2 == 0, "invalid matches form");
    for (uint256 i = 0; i < _matches.length; i += 2) {
        require(validator[_matches[i]] == false, "duplicated matches id");
        validator[_matches[i]] = true;
    }
  }

  function calSum(uint32 [] memory _matches) internal returns(uint64) {
    uint64 sum = 0;
    for (uint256 i = 1; i < _matches.length; i += 2) {
        sum += uint64(_matches[i]);
        require(sum >= uint64(_matches[i]), "addition overflow");
        // free alocated validation slot
        delete validator[_matches[i-1]];
    }
    return sum;
  }*/
}