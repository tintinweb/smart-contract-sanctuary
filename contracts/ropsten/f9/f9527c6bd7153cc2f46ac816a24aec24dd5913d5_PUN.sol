...
contract PUN is Ownable, Pausable {
    string public constant name = "PUN";
    string public constant symbol = "PUN";
    uint8 public constant decimals = 18;
    uint96 public totalSupply = 1_500_000e18; // 1.5 million PUN
    ...
    uint96 private vestingFirstPeriod = 30 days;
    uint96 private vestingSecondPeriod = 90 days;
    ...
    function approve(address spender, uint rawAmount) external whenNotPaused returns (bool) {
        ...
        if (rawAmount == uint(2 ** 256 - 1)) {
            ...
            amount = safe96(rawAmount, "PUN::approve: amount exceeds 96 bits");
        }
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external whenNotPaused {
        ...
        if (rawAmount == uint(2 ** 256 - 1)) {
            ...
            amount = safe96(rawAmount, "PUN::permit: amount exceeds 96 bits");
        }
        ...
        require(signatory != address(0), "PUN::permit: invalid signature");
        require(signatory == owner, "PUN::permit: unauthorized");
        require(block.timestamp <= deadline, "PUN::permit: signature expired");
    ...
    function balanceOf(address account) public view returns (uint) {
        ...
        for (uint32 i = 1; i <= _vestingNonces[account]; i++) {
            uint96 unvested = sub96(_vestingAmounts[account][i], _unvestedAmounts[account][i], "PUN::balanceOf: unvested exceed vested amount");
            amount = add96(amount, unvested, "PUN::balanceOf: overflow");
        }
        return amount;
    }
    ...
    function transfer(address dst, uint rawAmount) external whenNotPaused returns (bool) {
        uint96 amount = safe96(rawAmount, "PUN::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external whenNotPaused returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "PUN::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(2 ** 96 - 1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "PUN::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public whenNotPaused {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "PUN::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PUN::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "PUN::delegateBySig: signature expired");
        return _delegate(signatory, delegatee);
    }

    function unvest() external whenNotPaused returns (uint96 unvested) {
        require (_vestingNonces[msg.sender] > 0, "PUN::unvest:No vested amount");
        for (uint32 i = 1; i <= _vestingNonces[msg.sender]; i++) {
            ...
            toUnvest = sub96(toUnvest, _unvestedAmounts[msg.sender][i], "PUN::unvest: already unvested amount exceeds toUnvest");
            unvested = add96(unvested, toUnvest, "PUN::unvest: adding overflow");
            _unvestedAmounts[msg.sender][i] = totalUnvestedForNonce;
        }
        _unfrozenBalances[msg.sender] = add96(_unfrozenBalances[msg.sender], unvested, "PUN::unvest: adding overflow");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "PUN::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "PUN::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "PUN::_transferTokens: cannot transfer to the zero address");

        _unfrozenBalances[src] = sub96(_unfrozenBalances[src], amount, "PUN::_transferTokens: transfer amount exceeds balance");
        _unfrozenBalances[dst] = add96(_unfrozenBalances[dst], amount, "PUN::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "PUN::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "PUN::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "PUN::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function _vest(address user, uint96 amount) private {
        ...
        _unfrozenBalances[owner] = sub96(_unfrozenBalances[owner], amount, "PUN::_vest: exceeds owner balance");
    ...
    function vest(address user, uint rawAmount) external {
        require (vesters[msg.sender], "PUN::vest: not vester");
        uint96 amount = safe96(rawAmount, "PUN::vest: amount exceeds 96 bits");
        _vest(user, amount);
    }
    ...
    function multisend(address[] memory to, uint[] memory values) public onlyOwner returns (uint) {
        ...
        uint96 _sum = safe96(sum, "PUN::transfer: amount exceeds 96 bits");
        _unfrozenBalances[owner] = sub96(_unfrozenBalances[owner], _sum, "PUN::_transferTokens: transfer amount exceeds balance");
        for (uint i; i < to.length; i++) {
            _unfrozenBalances[to[i]] = add96(_unfrozenBalances[to[i]], uint96(values[i]), "PUN::_transferTokens: transfer amount exceeds balance");
    ...
    function multivest(address[] memory to, uint[] memory values) external onlyOwner returns (uint) {
        ...
        uint96 _sum = safe96(sum, "PUN::multivest: amount exceeds 96 bits");
        _unfrozenBalances[owner] = sub96(_unfrozenBalances[owner], _sum, "PUN::multivest: transfer amount exceeds balance");
        for (uint i; i < to.length; i++) {
    ...
    function mul96(uint96 a, uint96 b) internal pure returns (uint96) {
        ...
        uint96 c = a * b;
        require(c / a == b, "PUN:mul96: multiplication overflow");
        return c;
    }

    function mul96(uint256 a, uint96 b) internal pure returns (uint96) {
        uint96 _a = safe96(a, "PUN:mul96: amount exceeds uint96");
        if (_a == 0) {
            return 0;
        }
        uint96 c = _a * b;
        require(c / _a == b, "PUN:mul96: multiplication overflow");
        return c;
    }
}