...
contract mortgage {
    ...
    function cal_mortgage (uint _id )  public {
        if (user_details1[_id].deposited_asset == AsssetType.AGCoin ) {
            ...
            user_details1[_id].max_mortgage =  user_details2[_id].deposited_value*70/100;
        }
    ...
    function cal_intrest(uint _id)public{
        require(user_details1[_id].m_state == mortgage_state.provided, " mortgage is not provided yet");
        user_details2[_id].interest = user_details2[_id].mortgage_amount*((1+user_details1[_id].rate)**user_details1[_id].time)-user_details2[_id].mortgage_amount;
