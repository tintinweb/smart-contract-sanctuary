...
/**
 * @title NotRandom - ERC721 with infinite supply 
 * NotRandom - kinda random but 100% deterministic so gl
 */
contract NotRandom is ERC721Enumerable {

  // Store python code that these tokens are designed for on chain
  // Currently runs on a backend hosted at api.3-body.xyz/solve and takes json in the request body in the format of {"hash": <hash>} where hash is the tokenHash
  ...
  function hashOf(uint256 tokenId) public view returns (bytes32) { return tokens[tokenId].hash; }
  function nameOf(uint256 tokenId) public view returns (string memory) { return tokens[tokenId].name; }
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    ...
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', tokens[tokenId].name, '", "description": "NotRandom provides a keccak256 hash of the token name which is given during mint. This hash can be used for anything but is designed to be used with SOLVER_CODE to provide initial values to create a three body simulation.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
