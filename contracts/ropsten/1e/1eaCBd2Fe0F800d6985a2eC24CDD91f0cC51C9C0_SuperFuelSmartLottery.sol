...
contract SuperFuelSmartLottery is UserInfoManager, ISmartLottery, LPSwapSupport, LockableFunction, AuthorizedListExt {
    ...
    function getInfo() external view override returns(string memory){
        return string(abi.encodePacked("Jackpot: ", jackpotAmount(), " ", rewardTokenInfo.name, " Current Draw: ", draw));
    }
}
...
//    function winnersByRound(uint256 round) external view returns(WinnerLog);
}
