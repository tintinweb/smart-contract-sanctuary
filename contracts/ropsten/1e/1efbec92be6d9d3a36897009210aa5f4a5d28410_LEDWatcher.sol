...
contract LEDWatcher is Initializable {
    using SafeMathUpgradeable for uint256;

    event RegisteredState (
    ...
    event CompleteOfRequest (
        address indexed requester,
        bytes32 indexed deviceId,
        bytes4 indexed completeIndex
    );

    struct Request {
        bytes32 deviceId;
        bool state;
        bytes4 indexOfdevice;
    }

    address owner;
    Radoa radoa;
    mapping (bytes32=>bool) stateOfDeviceId;
    mapping (bytes32=>bytes4) lastIndexOfDeviceId;
    mapping (address=>uint256) depositBalances;
    mapping (address=>Request) requests;
    mapping (address=>bool) isRequesting;

    uint constant onHash = 0x4bf5122f344554c53bde2ebb8cd2b7e3d1600ad631c385a5d7cce23c7785459a;
    uint constant offHash = 0x6e340b9cffb37a989ca544e6bb780a2c78901d3fb33738768511a30617afa01d;
    ...
    function recordState(bool _state) public {
        ...
        if(_state){
            ...
            expectedHash = offHash;
        }
        require(uint(stateHash)==expectedHash,"code 2 in recordState");
        stateOfDeviceId[deviceId] = _state;
        lastIndexOfDeviceId[deviceId] = lastAuthedIndex;
        emit RegisteredState(deviceId,lastAuthedIndex,_state);
    }

    function issueRequest(bytes32 _deviceId, bool _state) public payable {
        require(radoa.isHealthDevice(_deviceId),"code 0 in recordState");
        require(msg.value > 0,"code 1 in recordState");
        require(!isRequesting[msg.sender],"code 2 in recordState");
        bytes4 index = radoa.getLastIndex(_deviceId);
        Request memory request;
        request.deviceId = _deviceId;
        request.state = _state;
        request.indexOfdevice = index;
        depositBalances[msg.sender] = depositBalances[msg.sender].add(msg.value);
        requests[msg.sender] = request;
        isRequesting[msg.sender] = true;
        emit NewRequest(msg.sender, _deviceId, _state);
    }

    function completeRequest(address _requester) public onlyOwner {
        require(isRequesting[_requester],"code 0 in completeRequest");
        bytes32 deviceId = requests[_requester].deviceId;
        bytes4 lastIndex = lastIndexOfDeviceId[deviceId];
        require(lastIndex > requests[_requester].indexOfdevice, "code 1 in completeRequest");
        require(stateOfDeviceId[deviceId] == requests[_requester].state, "code 2 in completeRequest");
        depositBalances[msg.sender] = 0;
        isRequesting[msg.sender] = false;
        emit CompleteOfRequest(_requester, deviceId, lastIndex);
    }
}