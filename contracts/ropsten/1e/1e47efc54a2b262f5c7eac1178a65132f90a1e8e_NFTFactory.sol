...
/*
 ___  ___  ___  ___  _____ ______   ________  ___               ________   ________ _________   
...
contract NFTFactory {
    ...
    function setNftTokenValues(address owner, address payable admin, uint256 commissionRate, string memory name, string memory symbol, bool anyoneMints) public returns(address _contract){
        address newNftContract = address(new HUMBLNFT(owner, admin, commissionRate, name, symbol, anyoneMints));
...
contract HUMBLNFT is ERC721Full, ReentrancyGuard {
    ...
    function buy(uint256 tokenId) public payable nonReentrant {
        ...
        // transfer funds
        require(msg.value >= sellBidPrice[tokenId], "Not enough funds");
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {
        ...
        require(sellBidPrice[tokenId]==0, "HUMBLNFT: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "HUMBLNFT: The selected NFT already has an auction");
        require(ownerOf(tokenId)==msg.sender, "HUMBLNFT: Only owner can auction this item");
    ...
    function auctionFinalize(uint256 tokenId) public nonReentrant {
        ...
        require(auctions[tokenId].open, "There is no auction opened for this tokenId");
