...
contract LendingPoolConfigurator is VersionedInitializable {
  ...
  address public aTokenProxyAddress;
  ...
  function initReserve(address aTokenImpl,uint8 underlyingAssetDecimals) public {
    aTokenProxyAddress = _initTokenWithProxy(aTokenImpl, underlyingAssetDecimals);
  }
  ...
  function _initTokenWithProxy(address implementation, uint8 decimals) internal returns (address) {
    ...
    //bytes memory params = abi.encodeWithSelector(bytes4(keccak256(bytes('initialize(uint8,string,string)'))), decimals, IERC20Detailed(implementation).name(), IERC20Detailed(implementation).symbol());
