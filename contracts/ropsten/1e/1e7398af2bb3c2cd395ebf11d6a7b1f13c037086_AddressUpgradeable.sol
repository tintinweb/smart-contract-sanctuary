...
contract EquipNFT is ERC721URIStorageUpgradeable,ERC721EnumerableUpgradeable,Governable {
    ...
    uint private constant ELECTSAW = 5;
    uint private constant VAN = 6;
    uint private constant TRUCK = 7;

    
    address[3] public item;//food,wood,gold erc20 address
    ...
    function setItemToken(address[3] calldata token)  public governance {
            item[0] = token[0];
            item[1] = token[1];
            item[2] = token[2];
    }
    ...
    function transferFrom(
        ...
        require(curPropertys[tokenId].curDurability >= property.totalDurability,"Must full durability");
    ...
    function setDur(uint id,uint dur) public governance  {
...
contract AllowListNftPool is Governable{
    ...
    uint public curValue;

    function __AllowListNftPool_init(address governor_, uint nftTypeId_, address token_, uint begin_,uint span_,string memory  uri_,uint price_,uint maxValue_,address globalInfo_) external initializer {
		__Governable_init_unchained(governor_);
		__AllowListNftPool_init_unchained(nftTypeId_,token_, begin_,span_,uri_,price_,maxValue_,globalInfo_);
	}
	
    function __AllowListNftPool_init_unchained( uint nftTypeId_, address token_, uint begin_,uint span_,string memory  uri_,uint price_,uint maxValue_,address globalInfo_) public governance {
    ...
    function setMaxValue(uint maxValue_) external governance{
        maxValue= maxValue_;
    }
    ...
    function buy() external payable  {
        ...
        curValue += 1;
        require(curValue<=maxValue,"no NFT");
...
contract NftPool is Governable{
    ...
    function __NftPool_init(address governor_, uint nftTypeId_,address token_, uint begin_, uint span_, string memory  uri_,uint price_,uint maxValue_,address globalInfo_) external initializer {
		__Governable_init_unchained(governor_);
		__NftPool_init_unchained(nftTypeId_,token_, begin_, span_, uri_,price_,maxValue_,globalInfo_);
	}
	
    function __NftPool_init_unchained(uint nftTypeId_,address token_, uint begin_,uint span_, string memory  uri_,uint price_,uint maxValue_,address globalInfo_) public governance {
        nftTypeId = nftTypeId_;
        nftAddr = token_;
        begin = begin_;
        span =span_;
...
contract DaoFarmer is Governable{
    ...
    mapping(address => bool) public powerInits;   //address>init 2000
    mapping(address => uint) public powers;   //address>power
    ...
    function __DaoFarmer_init(address governor_, uint begin_, uint span_) external initializer {

	}
	
    function __DaoFarmer_init_unchained(uint begin_,uint span_) public governance {
    ...
    event Farm(address nft,uint id,address erc20,uint value,uint time);
    
    
    function costFoodForFull() public view returns(uint foodValue) {
    ...
    function costGoldForRepairFull(uint propId,uint tokenId) public {
