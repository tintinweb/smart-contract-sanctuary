...
contract animalMain is  ERC721,Ownable {
  ...
  function createAnimal() public payable {
      ...
      //TO DO 
      //使用亂數來產生DNA, 星級, 動物種類
      dna = keccak256(abi.encodePacked(block.timestamp, block.coinbase, block.difficulty, now));
      star = uint(dna)+uint(now) % 5 +1;
      roletype = uint(dna)+uint(block.difficulty) % 5 +1 ;
