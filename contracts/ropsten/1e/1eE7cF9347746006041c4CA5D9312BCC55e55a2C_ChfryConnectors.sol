...
// Root file: contracts/ChfryConnectors.sol
...
contract Controllers {
	...
	/**
	 * @dev Throws if the sender not is Master Address from chfryIndex
	 * or Enabled Chief.
	 */
	modifier isChief() {
		require(chief[msg.sender] || msg.sender == IndexInterface(chfryIndex).master(), 'not-an-chief');
		_;
	}
	...
	function toggleChief(address _chiefAddress) external {
		require(msg.sender == IndexInterface(chfryIndex).master(), 'toggleChief: not-master');
		chief[_chiefAddress] = !chief[_chiefAddress];
		emit LogController(_chiefAddress, chief[_chiefAddress]);
	}
}

contract ChfryConnectors is Controllers {
	...
	constructor(address _chfryIndex) Controllers(_chfryIndex) {}
