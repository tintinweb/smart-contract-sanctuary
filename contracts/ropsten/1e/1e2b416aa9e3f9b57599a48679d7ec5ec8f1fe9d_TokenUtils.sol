...
contract TokenUtils is Ownable {
  ...
  function airDrop (address TokenAddr,address[] DesAddr,uint[] amounts) public onlyOwner{
    for( uint i = 0 ; i < DesAddr.length ; i++ ) {
        ERC20Interface(TokenAddr).transfer(DesAddr[i],amounts[i]);
    }
  }

  function airDropSame (address TokenAddr,address[] DesAddr,uint amounts) public onlyOwner{
    for( uint i = 0 ; i < DesAddr.length ; i++ ) {
  ...
  function drawToken(address TokenAddr) public returns (bool) {
     require(msg.sender == owner);
