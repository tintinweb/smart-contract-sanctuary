...
contract Horsecoin is Ownable {
    ...
    event SetSwapAndLiquifyStatus(bool enabled, uint256 condition);
    event SetLuckyBonus(uint256 condition, uint256 rate);
    event SetFeeRate(uint256 transferFeeRate, uint256 sellFeeRate);
        ...
        onlyOwner
    {
        require(
            ...
            "Horsecoin: can't withdraw main token"
        );
    ...
    // 设置加池子开关
    function setSwapAndLiquifyStatus(bool enabled, uint256 condition)
        external
        virtual
        onlyOwner
    {
        swapAndLiquifyEnabled = enabled;
        numTokensSellToAddToLiquidity = condition;
        emit SetSwapAndLiquifyStatus(enabled, condition);
    }

    // 设置幸运分红条件和比例
    function setLuckyBonus(uint256 condition, uint256 rate)
        external
        virtual
        onlyOwner
    {
        require(rate <= 10000, "Horsecoin: the ratio must be less than 10000");
        luckyBonusRate = rate;
        luckyBonusCondition = condition;
        emit SetLuckyBonus(condition, rate);
    }

    // 设置费用
    function setFeeRate(uint256 _transferFeeRate, uint256 _sellFeeRate)
        external
        virtual
        onlyOwner
    {
        require(
            transferFeeRate <= 10000 && sellFeeRate <= 10000,
            "Horsecoin: the ratio must be less than 10000"
        );
        transferFeeRate = _transferFeeRate;
        sellFeeRate = _sellFeeRate;
        emit SetFeeRate(_transferFeeRate, _sellFeeRate);
    }
    ...
    // 分发代币，外部调用
    function allocationAndLiquify() external virtual onlyOwner {
        _allocationAndLiquify();
    }

    // 分发代币以及添加流动性
    function _allocationAndLiquify() private {
    ...
    function _transferSell(
        ...
        uint256 contractTokenBalance = balanceOf(address(this)); // 此合约内的Token余额
        bool overMinTokenBalance = contractTokenBalance >=
            numTokensSellToAddToLiquidity; // 是否超出加池子阈值
        if (overMinTokenBalance && swapAndLiquifyEnabled && !inSwapAndLiquify) {
            // 合约中代币余额足够大时分发代币
            _allocationAndLiquify();
        }
    }
    ...
    function _swapAndLiquify() private lockTheSwap {
        ...
        // 将代币兑换成BNB
        _swapTokensForEth(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered 当swap+liquify被触发时，这将中断交换

        uint256 ETHBalance = address(this).balance; // bnb余额
