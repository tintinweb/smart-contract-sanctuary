...
contract ExchangeWithAtomic is ExchangeWithOrionPool {
    ...
    function claimAtomic(bytes calldata secret, bytes calldata matcherSignature) public nonReentrant {
        ...
        swap.used = true;
        _updateBalance(swap.receiver, swap.asset, swap.amount);
    }
...
contract ExchangeWithOrionPool is Exchange, IPoolSwapCallback {
    ...
    function fillThroughOrionPool(
        ...
        {   //  STack too deep workaround
            ...
            LibPool.updateFilledAmount(order, uint112(tmp.filledBase), filledAmounts);
        }
    ...
    function swapThroughOrionPool(
        ...
        if (isSentETHEnough) {
            LibPool.refundChange(amountOut);
