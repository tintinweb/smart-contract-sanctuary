...
contract SpecialPool is Ownable {
    ...
    function unStake(uint256 stakeID) external {
        ...
        // require (stakeID != 0, 'withdraw CAKE by unstaking');
        ...
        if (block.timestamp <= stake.intialTimestamp.add(30 minutes)) {
            ...
            emit UnStake(msg.sender, pendingReward, block.timestamp);
        }
    ...
    function calculateRewards(uint256 stakeID) public view returns(uint256 reward) {
        ...
        if (block.timestamp <= stake.intialTimestamp + 10 minutes) {
            uint256 pendingReward = stake.tokenQuantity * PENALTY / 100E18;
            reward = stake.tokenQuantity - pendingReward;
        ...
        if (block.timestamp >= stake.intialTimestamp + 10 minutes && block.timestamp <= stake.intialTimestamp + 20 minutes) {
            uint256 pendingReward = (stake.tokenQuantity.mul(ENTRY_RATE))/100E18;
            reward = (pendingReward.div(60)).mul(day);
            require(IBEP20(TOKEN).balanceOf(address(this)) >= reward, "Insufficient balance for reward");
        }
        
        if (block.timestamp >= stake.intialTimestamp + 20 minutes && block.timestamp <= stake.intialTimestamp + 30 minutes) {
            uint256 pendingReward = (stake.tokenQuantity.mul(DAYS30_RATE))/100E18;
            reward = (pendingReward.div(60)).mul(day);
            require(IBEP20(TOKEN).balanceOf(address(this)) >= reward, "Insufficient balance for reward");
        }
        
        if (block.timestamp >= stake.intialTimestamp + 30 minutes) {
