...
contract FtmPricePrediction is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  uint256  bufferSeconds; // number of seconds for valid execution of a prediction round
  uint256  intervalSeconds; // interval in seconds between two prediction rounds

  uint256  minBetAmount; // minimum betting amount (denominated in wei)
  uint256  treasuryFee; // treasury rate (e.g. 200 = 2%, 150 = 1.50%)
  uint256  treasuryAmount; // treasury amount that was not claimed

  uint256  currentEpoch; // current epoch for prediction round

  uint256  oracleLatestRoundId; // converted from uint80 (Chainlink)
  uint256  oracleUpdateAllowance; // seconds
  uint256  MAX_TREASURY_FEE = 1000; // 10%
  {
   ...
   * @param epoch: epoch
   */
  function betDown(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
   ...
   * @param epoch: epoch
   */
  function betUp(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
    require(epoch == currentEpoch, "Bet is too early/late");
    require(_bettable(epoch), "Round not bettable");
    require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
    require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
  ...
  function claimTreasury() external nonReentrant onlyAdmin {
    ...
    treasuryAmount = 0;
    _safeTransferFTM(adminAddress, currentTreasuryAmount);
  ...
  function _calculateRewards(uint256 epoch) internal {
    ...
    // Up wins
    if (round.closePrice > round.lockPrice && round.upAmount != 0) {
      rewardBaseCalAmount = round.upAmount;
      treasuryAmt = (round.totalAmount * treasuryFee) / 10000;
      rewardAmount = round.totalAmount - treasuryAmt;
    }
    // Down wins
    else if (round.closePrice < round.lockPrice && round.downAmount != 0) {
