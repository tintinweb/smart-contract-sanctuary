...
// File: contracts/registry/ITokitRegistry.sol


pragma solidity ^0.6.12;

/**
 * @title An interface for a registry contract
 */
...
// File: contracts/breaker/IBreakerFund.sol


pragma solidity ^0.6.12;

/**
 * @title Fund contract interface
 */
...
// File: contracts/breaker/BreakerAsset.sol
...
contract BreakerAsset {
  ...
  uint public assetId;
  uint public constant SUM_OF_FRACTIONS = 10000;
...
// File: contracts/breaker/BreakerAssetWithInvestor.sol
...
contract BreakerAssetWithInvestor is BreakerAsset {
  ...
  function increaseInvestorEthThreshold(address _recepient, uint _ethThreshold) public onlyOwner {
    if (_recepient == salesAgent.recepient) {
      ...
      salesAgent.ethThreshold = salesAgent.ethThreshold.add(_ethThreshold);
      return;
    }
    ...
    for (uint i = 0; i < ethAmountInvestors.length; ++i) {
      if (ethAmountInvestors[i].recepient == _recepient) {
        ...
        ethAmountInvestors[i].amountToBePaid = ethAmountInvestors[i].amountToBePaid.add(_ethThreshold);
        break;
      }
    }
  }
  ...
  function increaseInvestorCoinThreshold(address _recepient, uint _coinThreshold) public onlyOwner {
    if (_recepient == salesAgent.recepient) {
      ...
      salesAgent.coinThreshold = salesAgent.coinThreshold.add(_coinThreshold);
      return;
    }
    ...
    for (uint i = 0; i < coinAmountInvestors.length; ++i) {
      if (coinAmountInvestors[i].recepient == _recepient) {
        ...
        coinAmountInvestors[i].amountToBePaid = coinAmountInvestors[i].amountToBePaid.add(_coinThreshold);
        break;
      }
    }
  }
...
// File: contracts/deployer/AssetDeployerWithManyInvestors.sol
