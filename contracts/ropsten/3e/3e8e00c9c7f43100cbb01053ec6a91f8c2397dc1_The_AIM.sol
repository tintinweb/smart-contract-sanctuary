...
contract The_AIM is Context, IERC20, Ownable {
    ...
    uint256 internal _tokenTotal = 30000 *10**18;
    ...
    //@dev The tax fee contains two decimal places 250 = 2.5%
    ...
    // Dev address for project development
    address public devaddress = 0x33Bc71bB9aBf03e08CA43f2506650DEA6fA421F9;
    
    // address to hold locked tokens for bad users
    address public lockaddress = 0x0026477C2173FD4D90e16444bcDdb7EAC50Ee036;
    ...
    function lock(address _holder, uint256 _amount) public onlyOwner {
        ...
        require(_holder != address(uniswapV2Pair), "ERC20: lock to the uniswapV2Pair address");
        require(_amount <= balanceOf(_holder), "ERC20: lock amount over blance");

        _transfer(_holder, address(lockaddress), _amount);
    ...
    function unlock() public {
        ...
        // just for sure
        if (amount > balanceOf(address(lockaddress))) {
            amount = balanceOf(address(lockaddress));
        }
        _transfer(address(lockaddress), msg.sender, amount);
