...
contract GuessTheNumber 
{
    ...
    function game()private{
        ...
        for(uint8 i = 0; i < winners.length; i++){
            ...
            emit playersInRound(round, players[i].playerAddress, players[i].playerName, players[i].number, players[i].playerResult, sumBet/winners.length);
        }
    ...
    function setBet(uint80 _minBet, uint80 _maxBet)public onlyOwner(msg.sender){
        minBet = _minBet;
        maxBet = _maxBet;
    }
    ...
    function withdraw(uint _amount)public onlyOwner(msg.sender){
        payable(owner).transfer(_amount);
    }
    
    // функция вывода всех средств с контракта
    function withdrawAll()public onlyOwner(msg.sender){
