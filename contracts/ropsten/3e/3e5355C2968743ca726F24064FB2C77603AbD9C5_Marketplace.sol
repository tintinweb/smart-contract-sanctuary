...
contract Marketplace {
    ...
    mapping(uint256 => address) public heighstBid;
    ...
    function makeTradeOffer(uint256 _price,uint256 _tokenId,address _tokenAddress ) public payable{
    ...
    function cancelOffer(uint256 _tokenId, uint256 _offer,address _tokenAddress ) public payable{
        ...
        emit offerCancled(_offer, _tokenAddress, _tokenId);
    }
    ...
    function makeAuctionBid(uint256 _auction, uint256 _price) public payable auctionExists(_auction){
        ...
        heighstBid[_auction] = msg.sender;
    ...
    function claimAuctionNFT(uint256 _auction) public payable auctionExists(_auction){
        Auction memory auction = auctions[_auction];


        Bid memory bid = auctionsBids[_auction][auction.offerCount-1];

        require(block.timestamp > auctions[_auction].endtime, "Auction not end yet");
        require(msg.sender == bid.poster, "You are not the winner of the auction");
