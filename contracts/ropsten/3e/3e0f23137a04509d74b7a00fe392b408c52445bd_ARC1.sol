...
contract ARC1 is ERC20, CoinAgeMintableStandard, CoinEraMintableStandard, Ownable {
    using SafeMath for uint256;

    string public name = "ARC1";
    string public symbol = "ARC1";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        //user will trigger both CoinAge and CoinEra minting simultaneously.
        
        if(msg.sender == _to && balances[msg.sender] >= coinAgeHolder) return mintingCoinAge();
        if(msg.sender == _to && balances[msg.sender] >= onlyStrongHolder) return mintingCoinEra();
...
//--------------------------------------------------------------------------------------
    ...
    //How to trigger internal PoHASM minting :
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public coinEraRate = 10 * (10**decimals);
    uint256 public intervalRate = 180;
    ...
    function mintingCoinEra() StrongHolderMinter public returns (bool) {
        ...
        if(block.timestamp < timeInterval) return false;
