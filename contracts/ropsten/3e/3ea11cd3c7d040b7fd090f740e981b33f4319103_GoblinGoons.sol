...
contract GoblinGoons is Context, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    bool private paused = false; //TODO: Start as TRUE
    ...
    constructor() ERC721("Goblin Goons", "GOGS") {
        _baseTokenURI = "websiteaddress.com/";
    }
    ...
    function mint(uint amount) public payable {
        ...
        require(msg.value == mintPrice*amount, "It costs 0.03 eth to mint a Goblin.");
    ...
    function mintGoblin(uint id) private {
        _mint(msg.sender, id);
        uint randomCoins = randomCoinAmount();
    ...
    function randomCoinAmount() private view returns(uint) {
        uint randomNumber =  uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, totalSupply())));
        
        if(randomNumber % 100 < 57) {return 1;}
        if(randomNumber % 100 < 81) {return 2;}
        if(randomNumber % 100 < 92) {return 3;}
        if(randomNumber % 100 < 98) {return 4;}
        return 5;
    }
}