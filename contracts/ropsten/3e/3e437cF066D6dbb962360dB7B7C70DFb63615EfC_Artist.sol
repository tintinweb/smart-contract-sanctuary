...
contract ArtistBase is Ownable,ERC721Full,RadicalNFT,ReentrancyGuard {
    ...
    mapping(uint256=>bool) public taxFreeArt;
    uint256 taxFreeCount;
    uint256 maxTaxFree;
    //do NOT change memory map
    function setTaxFreeArt(uint256 tokenid,bool state) 
        external
        onlyOwner
    {
        //this is not accurate,but should be ok
        if(state==true){
            if(taxFreeArt[tokenid]==true){
                return;
            }
    ...
    function setBidEndTime(uint256 tokenid,uint64 endtime) 
        external
        onlyCLevel
        {
            require(tokenid<curid, "ERR_ARTID_TOOBIG");
            arts[tokenid].bidEndTime=endtime;
        }
    ...
    function _bid(address inviterAddress, uint256 artid) nonReentrant internal
     {
         ...
         if(r==0){
             ...
             require(msg.value>=payprice, "ERR_NOT_ENOUGH_MONEY");
              //return extra
              (bool success, ) =address(uint160(msg.sender)).call.value(msg.value.sub(payprice))("");
              require(success,"ERR refund to payer,maybe gas fail");
              //pay artist
              (bool success1, ) =address(uint160(owner)).call.value(payprice)("");
            ...
            emit LOG_AUCTION_V2(artid, payprice,payprice,x,lastOwner,msg.sender,inviterAddress,0,0 );
            return;
         }
        ...
        require(msg.value>=payprice, "ERR_NOT_ENOUGH_MONEY");
         ...
         {//only for stack deep issue
            ...
            require(success2,"ERR refund to payer,maybe gas fail");
             ...
             uint256 pasthours=(now-arts[artid].lastTime)/1 hours;
        ...
        arts[artid].lastTime=uint64(now);
        emit LOG_AUCTION_V2(artid, curprice,payprice,x,lastOwner,msg.sender,inviterAddress,artistdivident,tax );
