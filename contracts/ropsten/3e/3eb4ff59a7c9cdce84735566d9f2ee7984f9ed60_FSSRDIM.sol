...
// constructor is only executed during the creation of the contract
    constructor() ERC721("FissuresDims", "FSSRDIM") {
    
    }

    function setFissuresAddr(address _fissuresContract) public {
    ...
    function amIdaOwner(uint256 tokenId) public view returns (bool) {
        address tokenOwner = IFissure(FissuresAddr).ownerOf(tokenId);
        if(tokenOwner == tx.origin){
          return true;
        }
        return false;
