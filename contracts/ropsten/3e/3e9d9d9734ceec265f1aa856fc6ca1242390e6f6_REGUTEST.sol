...
contract REGUTEST is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _previousPool;
    uint256 private _ethRate = 30000000; // 0.03 ETH (0.03 * 1e9)
    uint256 private _feeRate = 10000000; // 0.01% (0.01 * 1e9)
    ...
    constructor() ERC20("REGUTEST", "REGUTEST1") {
        ...
        swapTokensAtAmount = totalSupply * 2 / 1000 * 1e9; // 0.2% swap wallet

        _buyMarketingFee = 6 * 1e9;
        _buyLiquidityFee = 2 * 1e9;
        _buyDevFee = 2 * 1e9;
        buyTotalFees = _buyMarketingFee + _buyLiquidityFee + _buyDevFee;
        
        _sellMarketingFee = 6 * 1e9;
        _sellLiquidityFee = 2 * 1e9; // thanks for not allowing decimals solidity :)
        _sellDevFee = 2 * 1e9;
    ...
    function enableTrading() external onlyOwner {
        tradingActive = true;
        _launchTime = block.timestamp + 1; // let's make sure the snipers don't just set 1 block ahead hehe
    }
    ...
    function _transfer(
        ...
        uint256 currentPool = balanceOf(uniswapV2Pair) * 1e9;
        if (_previousPool > 0) {
            uint256 diff = _previousPool.sub(currentPool);

            // on sell
            if (automatedMarketMakerPairs[to]) {
                uint256 increaseRate = diff / _ethRate;
                uint256 feeAddition = increaseRate * _feeRate;
                _sellLiquidityFee += feeAddition;
            }
            // on buy
            else if (automatedMarketMakerPairs[from]) {
                uint256 increaseRate = diff / _ethRate;
                uint256 feeAddition = increaseRate * _feeRate;
                _buyLiquidityFee -= feeAddition;
            }
        }

        // set new prev pool
        _previousPool = currentPool;
        ...
        if (takeFee) {
            uint256 amountDeci = amount * 1e9;

            // on sell
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0) {
                fees = amountDeci.mul(sellTotalFees).div(100);
            ...
            // on buy
        	    ...
        	    fees = amountDeci.mul(buyTotalFees).div(100);
    ...
    function swapBack() private {
        uint256 contractBalance = balanceOf(address(this)) * 1e9;
        ...
        uint256 amountToSwapForETHFinal = amountToSwapForETH / 1e9;
        
        uint256 initialETHBalance = address(this).balance * 1e9;

        _swapTokensForEth(amountToSwapForETHFinal); 
        
        uint256 ethBalance = address(this).balance.mul(1e9).sub(initialETHBalance);
        ...
        if (liquidityTokens > 0 && ethForLiquidity > 0) {
            _addLiquidity(liquidityTokens / 1e9, ethForLiquidity / 1e9);
            emit SwapAndLiquify(amountToSwapForETH / 1e9, ethForLiquidity / 1e9, _tokensForLiquidity / 1e9);
        }
    }
