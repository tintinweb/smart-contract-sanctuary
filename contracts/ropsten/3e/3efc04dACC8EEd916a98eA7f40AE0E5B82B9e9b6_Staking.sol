...
contract Staking is Ownable {
    ...
    IERC20 public stakeToken; // Rakucoin
    IERC20 public rewardToken; // Rakugold
    uint256 public conversionRate; // default 458000, 1 Rakugold = 458000 Rakucoin
    ...
    function addPool(uint256 _rewardRate, uint256 _lockupDuration)
        external
        onlyOwner
    {
        poolInfo.push(
    ...
    function deposit(uint256 pid, uint256 amount) external {
        ...
        require(amount >= getPendingRewards(), "Not enough rewards remaining");
    ...
    function withdraw(uint256 pid, uint256 stakeId) external {
        ...
        require(
            stakeToken.transfer(msg.sender, stakeInfo.amount),
            "Withdrawl failed"
        );
        emit Withdraw(msg.sender, pid, stakeInfo.amount);

        if (stakeInfo.depositTime.add(pool.lockupDuration) > block.timestamp) {
            uint256 rewardAmount =
                getRewardAmount(stakeInfo.amount, pool.rewardRate);
            require(
    ...
    function getPendingRewards() private view returns (uint256) {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            pendingRewards = pendingRewards.add(
                poolInfo[i].depositedAmount.mul(poolInfo[i].rewardRate).div(
                    10**6
                )
            );
        }
    ...
    function getRemainingRewards() public view returns (uint256) {
        uint256 totalBalanceInStakeToken =
            rewardToken.balanceOf(address(this)).mul(conversionRate);
    ...
    function getRewardAmount(uint256 amount, uint256 rewardRate)
        ...
        returns (uint256)
    {
        return amount.mul(rewardRate).div(10**6).div(conversionRate);
    }
}