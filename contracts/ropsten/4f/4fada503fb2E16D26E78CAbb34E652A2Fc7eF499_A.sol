...
contract B {
    ...
    //всегда reverted, тк staticcall работает только с view, pure функциями,
    //иначе выбрасывает исключение, как в функции данной ниже
    function staticCallSecond(address payable _contract) public {
        (bool success,) = _contract.staticcall(
            abi.encodeWithSignature("forStaticCallSecond()")
        );
        require(success,"This function reverts");  //success всегда false
    }
}
