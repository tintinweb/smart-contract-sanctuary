...
contract PrayToPotatoStatue {
    ...
    function pray() public payable {
        ...
        if (dice < lucky.div(5)) {
            IERC20(0x63c62d5BDFe08ee4294B7ed11cd0A53a17b4bcc9).transfer(msg.sender, prize.mul(10));
            emit Pray(msg.sender, prize.mul(5));
        }
        else if (dice < lucky.mul(3).div(5)) {
            IERC20(0x63c62d5BDFe08ee4294B7ed11cd0A53a17b4bcc9).transfer(msg.sender, prize);
    ...
    function getInfo() public view returns (uint sff, uint lucky, uint _prize) {
        ...
        uint TWO_HOURS = 7200;
        lucky = t >= TWO_HOURS? BASE : BASE - BASE * (t-TWO_HOURS)**2 / TWO_HOURS**2;
