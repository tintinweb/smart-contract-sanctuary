...
contract OpenOceanCaller is IOpenOceanCaller, DistributionCaller, SafeERC20Extension, UniswapV2LikeCaller {
...
abstract contract UniswapV2LikeCaller is IUniswapV2LikeCaller {
    ...
    function uniswapV2Swap(
        ...
        (uint256 inReserve, uint256 outReserve) = reverse ? (reserve1, reserve0) : (reserve0, reserve1);
        uint256 inAmount = from.balanceOf(pair).sub(inReserve);
        uint256 outAmount = calculateOutAmount(inReserve, outReserve, inAmount, uint96(toWithFee >> 160));

        (uint256 amount0Out, uint256 amount1Out) = reverse ? (outAmount, uint256(0)) : (uint256(0), outAmount);
        if (noCalldata) {
            IUniswapV2NoCalldataPair(pair).swap(amount0Out, amount1Out, address(toWithFee));
        } else {
            IUniswapV2Pair(pair).swap(amount0Out, amount1Out, address(toWithFee), new bytes(0));
        }
    }
    ...
    function calculateOutAmount(
        ...
        uint48 feeNumerator = uint48(fee >> 48);
        uint48 feeDenominator = uint48(fee);

        uint256 inAmountWithFee = amount.mul(feeDenominator - feeNumerator);
...
interface IOpenOceanState is IBalancerV2State {}
...
contract OpenOceanState is IOpenOceanState, BalancerV2State {}
