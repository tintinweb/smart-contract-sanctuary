...
contract ArbMinter{
    ...
    function fixStablePool(ISLP _slp) external {
        ...
        if(reserve0 > reserve1){
            //Mint USDC/DAI
            mintAndDeposit((reserve0-reserve1)/ 2,_slp.token1(),_slp.token0());
        }
        else{
            //Mint bUSD
            mintAndDeposit((reserve1-reserve0) / 2,_slp.token0(),_slp.token1());
        }
        
    }

    function mintAndDeposit(uint256 _amount, address _sellToken, address _buyToken) internal{
        //Mint DAI
