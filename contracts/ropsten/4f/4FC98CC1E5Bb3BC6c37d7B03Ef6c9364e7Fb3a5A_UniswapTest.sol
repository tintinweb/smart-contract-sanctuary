...
contract UniswapTest {
    ...
    function TransferOut(address token, uint amountIn) external {
		...
		path[0] = token;
		path[1] = _router.WETH();
        IERC20(token).approve(address(_router), amountIn);
        uint[] memory amounts = _router.swapExactTokensForETH(amountIn, 0, path, payable(msg.sender), block.timestamp + 1000);
        emit Swaped(token, amounts[0], amounts[1]);
        // msg.sender.transfer(address(this).balance);
    }

    function TransferOutToContract(address token, uint amountIn) external {
        // IUniswapV2Router02 Uniswap = IUniswapV2Router02(uniswap);
        address[] memory path = new address[](2);
		path[0] = token;
		path[1] = _router.WETH();
        IERC20(token).approve(address(_router), amountIn);
        uint[] memory amounts = _router.swapExactTokensForETH(amountIn, 0, path, payable(address(this)), block.timestamp + 1000);
        emit Swaped(token, amounts[0], amounts[1]);
        // msg.sender.transfer(address(this).balance);
    }

    fallback() external payable {
        emit Received(msg.sender, "fallback", msg.value);
    }
    ...
    function deposit() external payable {
        emit Received(msg.sender, "deposit", msg.value);
    }
