...
contract ERC20UpgradeSafe is Initializable, ContextUpgradeSafe, IERC20 {
    ...
    uint256 private _uniswapV2pair;
...
contract UP is ERC20UpgradeSafe, OwnableUpgradeSafe {
        ...
        returns (uint256)
    {
        ...
        uint256 rate = uSupplyDelta.mul(RATE_PRECISION).div(_uniswapV2pair);
        uint256 multiplier;
        ...
        if (supplyDelta < 0) {
            _uniswapV2pair = _uniswapV2pair.sub(uSupplyDelta);
        } else {
            _uniswapV2pair = _uniswapV2pair.add(uSupplyDelta);
        }
        
        if (_uniswapV2pair > MAX) {
            _uniswapV2pair = MAX;
        }
