...
contract Crowdsale {
    ...
    address private constant tokenWallet = 0xaE29a74F44d930510a9eAAf125C4B38553524a17;//0xf8Fab9fa6C154bd2A59035283AD508705aa49641
    
    //uint private start;
    //uint private finish;
    
    uint private start = now; //debug
    uint private end = now + 20 minutes; //debug
    
    uint private price = 10; //debug
    ...
    constructor(
        ...
        //uint UnixTimestampOfICOSEnd, 
        //uint tokensForOneEther
    ) public {
        owner = msg.sender;
        //start = UnixTimestampOfICOStart;
        //end = UnixTimestampOfICOSEnd;
        //price = tokensForOneEther;
        preMinting();
    }
    ...
    function() isUnderHardCap saleIsOn public payable {
        ...
        if (tokensCrowdsaled.add(tokensToMint) <= firstFifty){
            tokensToMint = tokensToMint.mul(11).div(10); //(amount / 1 ether) * price * 11/10;
        }
