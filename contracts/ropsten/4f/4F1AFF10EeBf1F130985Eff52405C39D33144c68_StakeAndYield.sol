...
contract StakeAndYield is Ownable {
    ...
    function depositFor(address _user, uint256 amount, uint256 stakeType) updateReward(_user, stakeType) public {
        ...
        require(user.balance == 0 || user.stakeType==stakeType);
    ...
    function userInfo(address account) public view returns(
        ...
        numbers[7] = rewardPerToken(STAKE).mul(365 days).div(PERIOD);
        numbers[8] = rewardPerToken(YIELD).mul(365 days).div(PERIOD);
