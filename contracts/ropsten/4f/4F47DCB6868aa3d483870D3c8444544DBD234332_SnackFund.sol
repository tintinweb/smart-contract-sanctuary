...
contract SnackFund {
  ...
  constructor(address _nucTokenAddress, address _fundHolderAddress, address _oldMemberListAddress) {
    ...
    if (_oldMemberListAddress != address(0)) {
      ...
      for (uint i = 0; i < oldMemberListCount; i++) {
        uint oldId = i + 1; // old member list went from 1 :-/
        (, string memory name, address creator) = oldMemberList.members(oldId);
        members[i].id = i;
        members[i].name = name;
        members[i].creator = creator;
        members[i].disabled = false;
        memberCount++;

        // due to bad design in MemberListContract, only can migrate payment in 2021-12
        string memory paymentId = string(abi.encodePacked(uint2str(oldId), "_1638291600000"));
        (address paidBy, uint paidAt, uint blockNumber) = oldMemberList.payments(paymentId);
        if (paidAt != 0) {
          Payment storage payment = payments[i][1638291600000];
          payment.paidAt = paidAt;
          payment.paidBy = paidBy;
          payment.blockNumber = blockNumber;
          payment.paidFor = members[i];
          payment.paidForTime = 1638291600000;
          payment.paidAmount = 5000000000000000000; // it's always 5 NUC from the old contract

          allPayments[paymentCount] = payment;
          paymentCount++;
        }
      }
    }
  ...
  function pay(uint _memberId, uint _timePayFor, uint _amount) public {
    Payment storage payment = payments[_memberId][_timePayFor];
    ...
    payment.paidAt = block.timestamp;
    payment.blockNumber = block.number;
    payment.paidFor = members[_memberId];
    payment.paidForTime = _timePayFor;
