...
contract Market is IMarket,Ownable {
    ...
    function listOnAuction(uint256 tokenId, uint256 price, CURRENCY currency, uint256 _days ) public override returns (Auction memory) {
        require(ChilliSwap(ERC721_TOKEN_ADDRESS).ownerOf(tokenId) == msg.sender, "EA");
        ...
        ChilliSwap(ERC721_TOKEN_ADDRESS).transferFrom(msg.sender, address(this), tokenId);
    ...
    function claimNft(uint256 _tokenId) public override returns(uint256) {
        ...
        if (auctionItem.owner == address(0)) {
            ChilliSwap(ERC721_TOKEN_ADDRESS).transferFrom(address(this), auctionItem.seller, _tokenId);
        } else {
            ChilliSwap(ERC721_TOKEN_ADDRESS).transferFrom(address(this), auctionItem.owner, _tokenId);

            ChilliSwap.Artwork memory artwork = ChilliSwap(ERC721_TOKEN_ADDRESS).getArtwork(_tokenId);
