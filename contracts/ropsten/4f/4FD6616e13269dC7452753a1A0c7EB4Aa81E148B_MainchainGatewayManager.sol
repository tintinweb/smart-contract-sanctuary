...
contract MainchainGatewayManager is MainchainGatewayStorage {
  ...
  function depositBulkFor(
    ...
    whenNotPaused
  {
    require(_tokens.length == _tokenNumbers.length, "length error");
  ...
  function getSigner(
    ...
    address _signer = _hash.recover(_signatures, 0);
    ...
    onlyMappedToken(_token, 20)
  {
    ...
    require(verifySignatures(_hash, _signatures), "not enough signatures");
    ...
    returns (bool)
  {
    ...
    require(_signatureCount > 0, "signature count is zero");
    ...
    for (uint256 i = 0; i < _signatureCount; i++) {
      ...
      require(_signer > _lastSigner, "duplicated signer");
      _lastSigner = _signer;
    }
    ...
    returns (uint256 _depositId)
  {
    ...
    require(_standard == _tokenStandard, "token standard not match");
...
library ECVerify {
  ...
  function recover(bytes32 _hash, bytes memory _signature, uint256 _index) internal pure returns (address _signer) {
    require(_signature.length >= _index + 66, "signature length invalid");
