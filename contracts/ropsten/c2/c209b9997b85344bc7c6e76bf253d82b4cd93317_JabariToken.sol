...
contract JabariToken is ERC20Interface, Owned, Lock {
    ...
    uint public hardCap = 2000000;
    uint public tokeSaleStartTime;
    uint public tokenSaleEndTime;
    uint public Minimum_contribution = 100 ;
    bool public isTokenSaleActive = false;
    ...
    constructor( address _tokensaleContract) public {
           ...
           _totalSupply = 280*10**6 *10**uint(decimals);
     ...
     * @param _reason The reason to query the lock tokens for
     */
    function tokensLocked(address _of, bytes32 _reason)  onlyOwner 
    public
    view
     ...
     * @param _reason The reason to query the unlockable tokens for
     */
    function tokensUnlockable(address _of, bytes32 _reason)  onlyOwner 
    public
    view
    returns(uint256 amount) {
        require(isTokenSaleActive != false);
     ...
     * @param _of Address of user, claiming back unlockable tokens 
     */
    function unlock(address _of)  onlyOwner 
    public 
    ...
    function startTokenSale(uint _timeStamp) public
     onlyOwner 
    returns (bool)
    {
            tokeSaleStartTime =  _timeStamp;                            
            isTokenSaleActive = !isTokenSaleActive;
            return true;
    }
    
      // function to end the crowdfund
    function endTokenSale(uint _timeStamp) public onlyOwner returns(bool) {
    if (isTokenSaleActive = true) {
         tokenSaleEndTime = _timeStamp ; 
      return true;
    }
    return false;
  }
  ...
  // -------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------
     function () external payable {
         require(isTokenSaleActive != false);
         require(msg.value >Minimum_contribution ,"minimum 100$ required");
         revert();
     }
// -------------------------------------------------------------------
