...
contract TestUniswapFlashSwap is IUniswapV2Callee {
    ...
    function swapExactTokensForTokens(address _startToken, uint _amount, address[] calldata _path) external {
        IERC20(_startToken).approve(address(ROUTER), _amount);
        IUniswapV2Router(ROUTER).swapExactTokensForTokens(_amount, 1, _path, address(this), block.timestamp + 30 minutes);
    }
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external override {
        ...
        uint amountReceived = IUniswapV2Router(ROUTER).swapExactTokensForTokens(amountBorrow, 1, path, address(this), block.timestamp + 30 minutes)[1];
