...
contract TwinciVestingContract is Ownable {
    ...
    uint256 public constant ecosystemInterval = 60 minutes;
    uint256 public constant dayPerYear = 60;
    ...
    function getStakingInterval() public view returns (uint256){
        return now.sub(developerAndMarketingClaimDate);
    }
    ...
    function claimDevAndMarketingReward() public {
        require(developerAndMarketingClaimDate + devAndMarketingInterval <= now, "Wait for the next interal");
        uint256 stakingInterval = now.sub(developerAndMarketingClaimDate);
        IERC20 erc20 = IERC20(_twinciAddress);
        require(stakingInterval >= devAndMarketingInterval, "Not enough time for 1 round");
    ...
    function claimEcosystemReward() public {
        require(reserveClaimDate + ecosystemInterval <= now, "Not enough time.");
        IERC20 erc20 = IERC20(_twinciAddress);
