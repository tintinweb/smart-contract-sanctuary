...
contract Shit2 is BEP20, Ownable {
  ...
  constructor() {
    ...
    _symbol = "SHT2v1";
    _decimals = 9;
    _totalSupply = 5000000 * 10**6 * 10 ** uint256(_decimals);
  ...
  */
  ...
  function _transferWithFees(address sender, address recipient, uint256 amount) internal {
    ...
    (uint256 toBurn, uint256 toLp, uint256 toMgt) = _calculateFees(amount);
  ...
  function _calculateFees(uint256 amount) internal view returns (uint256 burnAmount, uint256 lpAmount, uint256 mgtAmount) {
    burnAmount = 0;
