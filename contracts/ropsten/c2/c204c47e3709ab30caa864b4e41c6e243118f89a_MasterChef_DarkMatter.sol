...
contract DarkMatter is  ERC20, Ownable {
    uint256 private constant initialSupply = 5000000 * 1e18; // initial supply  minted 5.000.000 KDM
    ...
    event setMasterChef (address indexed _address);
  ...
  function setDeflationController(address _address ) external { //add deflation controller contract address.
...
interface IMasterChef  {
    function pendingKDM(uint256 pid, address user) external view returns (uint256);
...
contract MasterChef_DarkMatter is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function dev(address _dev_address) public {
