...
contract HPoolManager is ERC1155HolderUpgradeable, HordUpgradable {
    ...
    struct hPool {
        ...
        address championAddress;
        uint256 createdAt;
        uint256 ticketSaleDurationSecs;
        uint256 privateSubscriptionDurationSecs;
        uint256 publicSubscriptionDurationSecs;
        uint256 nftTicketId;
        bool isValidated;
        ...
        onlyMaintainer
    {
        ...
        hp.ticketSaleDurationSecs = block.timestamp.add(hordConfiguration.ticketSaleDurationSecs());
        ...
        onlyMaintainer
    {
        ...
        require(block.timestamp > hp.ticketSaleDurationSecs, "Ticket sale phase is not finished yet.");
        hp.poolState = PoolState.PRIVATE_SUBSCRIPTION;
        hp.privateSubscriptionDurationSecs = block.timestamp.add(hordConfiguration.privateSubscriptionDurationSecs());
    ...
    function privateSubscribeForHPool(uint256 poolId) external payable {
        ...
        require(block.timestamp <= hp.privateSubscriptionDurationSecs, "The time has elapsed for the private subscription phase.");
        ...
        onlyMaintainer
    {
        ...
        require(block.timestamp > hp.privateSubscriptionDurationSecs || numberOfTicketsUsed[poolId] < maxTicketsToUse, "Time is up or all tickets have been used.");
        ...
        hp.publicSubscriptionDurationSecs = block.timestamp.add(hordConfiguration.publicSubscriptionDurationSecs());
    ...
    function publicSubscribeForHPool(uint256 poolId) external payable {
        ...
        require(block.timestamp <= hp.publicSubscriptionDurationSecs, "The time has elapsed for the public subscription phase.");
        ...
        onlyMaintainer
    {
        ...
        IHPool hpContract = IHPool(hPoolFactory.deployHPool(poolId, hp.bePoolId, hp.championAddress, uniswapRouter));
        ...
        uint256 amountUSD = uint256(getLatestPrice()).mul(amountEth).div(getDecimalsReturnPrecision());
    ...
    //TODO: add separate for sending HORD as well
    /**
     * @notice          Function to add more protocol fees
     */
    function addProtocolFee(uint256 amount) external {
        platformFee.feesAvailable = platformFee.feesAvailable.add(amount);
    }
    ...
    function getDecimalsReturnPrecision() public view returns (uint8) {
        return chainLinkOracle.decimals();
    }
    ...
    */
        ...
        )
    {
        ...
        return (
            ...
            hp.treasuryFeePaid,
            hp.ticketSaleDurationSecs,
            hp.privateSubscriptionDurationSecs,
            hp.publicSubscriptionDurationSecs
        );
    }
}