...
/**
 * @title Gateway Starport
 * @author Compound Finance
 * @notice Contract to link Ethereum to Gateway
 */
contract Starport {
    ...
    /**
     * @notice Transfer an asset to Gateway via locking it in the Starport
     ...
     * @param recipient The recipient of the asset in Gateway
     */
    ...
    /*
     * @notice Transfer Eth to Gateway via locking it in the Starport
     ...
     * @notice Transfer Eth to Gateway via locking it in the Starport
     * @param chain The chain of the recipient, e.g. "ETH" for Ethereum
     * @param recipient The recipient of the Eth on Gateway
     * @dev Use `lock` to lock CASH or collateral assets.
     */
     ...
     * @dev Locking CASH will burn the CASH (as it's being transfer to Gateway)
    ...
    /**
     * @notice Executes governance proposal on Gateway
     * @dev This must be called from the admin, which should be the Compound Timelock
     * @param extrinsics SCALE-encoded extrinsics that can execute on Gateway
     */
     ...
     * @dev Notices are generated by certain actions from Gateway and signed by validators.
     * @param notice The notice generated by Gateway, encoded for Ethereum.
     * @param signatures Signatures from a quorum of validator nodes from Gateway.
     ...
     * @dev This must be called from `invoke` via passing in a signed notice from Gateway.
     ...
     * @dev This must be called from `invoke` via passing in a signed notice from Gateway or by the admin.
