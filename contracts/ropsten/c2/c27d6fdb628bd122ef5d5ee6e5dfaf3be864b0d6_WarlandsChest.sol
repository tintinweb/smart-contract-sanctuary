...
contract WarlandsChest  {
    ...
    event withdrawing(address indexed _from,address indexed _to, uint256 withdrawnAmount); 
    ...
    modifier shouldPay(uint256 _cost) {
        ...
        function setSilverChest(uint newSilverChest) public
    {
        require(msg.sender == owner, "Only owner can change this");
        SilverCost = newSilverChest;
    }

    function setGoldChest(uint newGoldChest) public
    {
        require(msg.sender == owner, "Only owner can change this");
        GoldCost = newGoldChest;
    }

    function setDiamondChest(uint newDiamondChest) public
    {
        require(msg.sender == owner, "Only owner can change this");
