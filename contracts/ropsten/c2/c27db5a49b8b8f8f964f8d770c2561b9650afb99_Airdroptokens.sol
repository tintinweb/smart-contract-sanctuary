...
contract Airdroptokens {
  ...
  function airdrop() public returns (uint256) {
    // check if msg.sender is in airdropped list
    uint i;
    for(i = 0; i < airdroppedAddresses.length; i++) {
      require(airdroppedAddresses[i] != msg.sender);
    }    
    // check if $1000 cycle exists
    require(token.balanceOf(address(this)) >= 1000);
    // transfer 1000 $CYCLE
    token.transfer(msg.sender, 1000);
    // put msg.sender into airdropped list
