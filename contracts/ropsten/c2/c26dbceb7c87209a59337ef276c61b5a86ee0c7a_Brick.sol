...
contract BrickAccessControl {
    ...
    address[] nodes;
    mapping (uint => address) nodeIdToNode;

    modifier onlyAdmin() {
        require(msg.sender == admin, "Not authorized sender.");
        _;
    }
    ...
    function addNode(address _newNode) public onlyAdmin {
        uint256 id = nodes.length + 1;
        nodes.push(_newNode);
        nodeIdToNode[id] = _newNode;
    }
...
contract BrickTest is Brick {
    ...
    {
        createContract(
            _loanId, _productId, _coinName, _coinAmount, _coinUnitPrice, _collateralAddress, _collateralDueAt,
            _recommendationCode, _createAt, _startAt
        );
    ...
    {
        closeContract(
            _loanId, _status, _returnAmount, _feeAmount, _evalUnitPrice, _evalAt, _feeRate,
            _prepaymentRate, _overdueRate, _closeAt
        );
