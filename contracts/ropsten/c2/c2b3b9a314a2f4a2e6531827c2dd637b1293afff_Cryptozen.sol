...
contract Cryptozen is Context, Ownable {
	...
	IUniswapV2Router01 public _uniswapRouterAddress;

	constructor() {
		setFeeAddress(payable(0x47d88ffF2978a25787d618d22dc090a65651CDf9));
		setNinjaContract(IERC20(0x5c648210aBB0E77160daE48F5c3a20BC195295cc));
	...
	function getTierByAmount(uint256 amount)
	public
	view
	...
	function getTier() public view returns (uint256[3] memory){
		return getTierByAmount(getNinjaBalance());
	}

	function getNinjaBalance() public view returns(uint256){
		return _ninjaContract.balanceOf(_msgSender());
	}
	...
	returns (uint256)
	{
		require(SafeMath.mul(amount, percent) >= 10000);
		return SafeMath.div(SafeMath.mul(amount, percent), 10000);
	}
	...
	function transferSameToken(
		...
		uint256 a =
		calculateTransferFee(amount, getFeePercentage());
	...
	function transferSameEther(address payable recipient, uint256 amount)
	public
	payable
	returns (bool)
	{
		...
		uint256 a =
		calculateTransferFee(amount, getFeePercentage());
		Address.sendValue(recipient, (amount - a));
		Address.sendValue(_feeAddress, a);
		putReward(_msgSender(), calculateNinjaReward(amount - a, _uniswapRouterAddress.WETH()));
		return true;
	}

	function putRewards(address[] memory recipients, uint256[] memory amounts) public onlyOwner returns(bool){
		for (uint i=0; i<recipients.length; i++) {
			putReward(recipients[i], amounts[i]);
		}
	}

	function putReward(address recipient, uint256 amount) public onlyOwner returns(bool){
	...
	function calculateNinjaReward(uint256 amountIn, address tokenContractAddress) public returns(uint256){
		address[] memory path;
		path[0] = address(_uniswapRouterAddress.WETH());
		path[1] = address(_ninjaContract);
		if(address(tokenContractAddress) !=  address(_uniswapRouterAddress.WETH())){
			if(address(tokenContractAddress) != address(_ninjaContract)){
				path[0] = address(tokenContractAddress);
				path[1] = address(_uniswapRouterAddress.WETH());
				path[2] = address(_ninjaContract);
			}
		}

		return _uniswapRouterAddress.getAmountsOut(amountIn, path)[1];
	}
