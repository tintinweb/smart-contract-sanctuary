...
//------------------------------------------------------------------------
//ABE Minting Protocol 
//Author, constructed, recoded and assembly by Azreh Ahargun
//Donate : 0x14Dfa531FafEb382971A834A68a26C9af77De655
//------------------------------------------------------------------------
...
contract ProofOfEpochProtocolLayer {
    ...
    event LastMintedTime(address indexed account, uint256 time);
}
...
contract ABE_MintingProtocol is ERC20, ERC20Detailed,
    ...
    constructor() public ERC20Detailed("ABE_MintingProtocol","AMP", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint256 lastMintedTime = now.sub(block.timestamp);
        lastClaimedTime[to] = lastMintedTime;
        
        emit Transfer(msg.sender, to, valueAfterShare);
        emit ShareTransfer(msg.sender, address(0), shareFee);
        emit LastMintedTime(to, lastMintedTime);
