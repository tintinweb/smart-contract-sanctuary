...
contract DOGUEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DOGUE_Dividend_Tracker", "DOGUE_Dividend_Tracker") {
        claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DOGUE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DOGUE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DOGUE contract.");
    }
    ...
    function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "DOGUE_Dividend_Tracker: Cannot update gasForTransfer to same value");
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DOGUE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DOGUE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DOGUE is ERC20, Ownable {
    ...
    DOGUEDividendTracker public dividendTracker;
    ...
    function activate() public onlyOwner {
        require(!tradingEnabled, "DOGUE: Trading is already enabled");
        _swapEnabled = true;
        tradingEnabled = true;
    }
    ...
    constructor() ERC20("Dogue Life", "DOGUE \xF0\x9F\x90\xB6") {
        _marketingWallet = payable(0xa89146d9d72E4Aa7bd2c10599D9f45bebAA74F39);
        dividendTracker = new DOGUEDividendTracker();
        ...
        excludeFromFees(address(0xA5025FABA6E70B84F74e9b1113e5F7F4E7f4859f)); //Multisender.app
    ...
    receive() external payable {
      ...
      function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DOGUE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DOGUE: Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "DOGUE: Account is already excluded from fees");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        // Need to make gas fee customizable to future-proof against Ethereum network upgrades.
        require(newValue != gasForProcessing, "DOGUE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DOGUE: This account cannot send tokens until trading is enabled");
        }
