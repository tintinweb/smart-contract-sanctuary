...
contract MiniGame is Ownable {
    ...
    function cancelMatch(uint256 _matchId) public onlyOwner {
        ...
        mt.allJoinedAddresses = new address[](0);
...
contract InvestmentContract is Ownable {
    ...
    mapping(address => mapping(IERC20 => uint256)) public pendingRewards;
    mapping(address => mapping(IERC20 => uint256)) public totalRewards;
    ...
    function safePull() payable external onlyOwner  {
    ...
    function deposit(uint256 _amount) external {
        ...
        balance[msg.sender][erc20] += _amount;
    }

    function withdraw(uint256 _amount) external {

        /* Confirm transfered tokens is no more than needed */
        require(balance[msg.sender][erc20] >= _amount, "Not enough balance");

        /* Transfer Funds */
        require(erc20.transfer(msg.sender, _amount), "Failed ERC20 token transfer");
           balance[msg.sender][erc20] -= _amount;

    }

    function addRewards(IERC20 token, address[] memory recipients, uint256[] memory values) external onlyOwner {
        require(recipients.length == values.length, "Invalid length");

        for (uint256 i = 0; i < recipients.length; i++) {
            pendingRewards[recipients[i]][token] += values[i];
            totalRewards[recipients[i]][token] += values[i];
        }
    }
