...
/* V2.2
    ...
    function _swap(address payable sender, address underlying, address currency, uint priceFloor, uint priceCap, int dCall, int dPut, int undMax, int curMax, bytes memory data) internal nonReentrant returns (address call, address put, int dUnd, int dCur) {
        ...
        (dUnd, dCur, priceFloor, priceCap) = calcDelta(priceFloor, priceCap, Call(call).totalSupply(), Put(put).totalSupply(), dCall, dPut);
        require(dUnd.add_(undMax = int(Math.abs(dUnd).mul(feeRate).div(1e18))) <= undMax && dCur.add_(Math.abs(dCur).mul(feeRate).div(1e18)) <= curMax, _slippage_too_high_);
...
// V2.4
    function swap(address underlying, address currency, uint priceFloor, uint priceCap, int dCall, int dPut, int undMax, int curMax, bytes memory data) external nonReentrant returns (address call, address put, int dUnd, int dCur) {
        ...
        (dUnd, dCur, priceFloor, priceCap) = calcDelta(priceFloor, priceCap, Call(call).totalSupply(), Put(put).totalSupply(), dCall, dPut);
        require((undMax = int(Math.abs(dUnd).mul(feeRate).div(1e18))) <= undMax.sub_(dUnd) && (curMax = int(Math.abs(dCur).mul(feeRate).div(1e18))) <= curMax.sub_(dCur), _slippage_too_high_);
