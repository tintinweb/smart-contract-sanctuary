	...
	TrusteeManagedSC
{
		...
		onlyUnexecuted()
	{
        ...
        for (uint256 i = 0; i < _conditionsIDforAdd.length; i++) {
            ...
            (uint256 condition_index, bool condition_exists) = _conditionGetIndex(_conditionsIDforAdd[i]);
            if (condition_exists && condition_index > 0) {
                if (tokens_sum > 0) {
                    conditionUpdate(_conditionsIDforAdd[i], _beneficiariesAddresses, tokens, _parentConditionsID[i]);
                } else {
                    address[] storage emptyAddresses;
                    uint256[] storage emptyTokens;
                    conditionUpdate(_conditionsIDforAdd[i], emptyAddresses, emptyTokens, _parentConditionsID[i]);
                }
            } else {
                if (tokens_sum > 0) {
		...
		onlyUnexecuted()
	{
		...
		conditions.push( Condition(_conditionID, ConditionState.Unmet, st, _beneficiaries, _tokens, _parentConditionID) );
    ...
    function conditionUpdate(uint256 _conditionID, address[] memory _beneficiaries, uint256[] memory _tokens, uint256 _parentConditionID)
		public
		onlyTrustee()
		onlyUnexecuted()
	{
		require(conditionExists(_conditionID), "Condition not exists");
        (uint256 condition_index, bool condition_exists) = _conditionGetIndex(_conditionID);
        require(condition_exists, "Condition not exists");

		if (_beneficiaries.length > 0) {
		    conditions[condition_index].subtype = ConditionSubtype.Beneficiaries;
		}

        conditions[condition_index].beneficiaryAddresses = _beneficiaries;
        conditions[condition_index].beneficiaryTokens = _tokens;

		if (_parentConditionID > 0) _conditionSetSubtype(_parentConditionID, ConditionSubtype.Subconditions);

		emit ConditionUpdate(_conditionID, now, _beneficiaries, _tokens, _parentConditionID);
	}
