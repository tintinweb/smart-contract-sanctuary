...
contract Umsfere {
    ...
    function FILL_Problem(uint Problem_ID) payable public {
        ...
        problem.Total_Weis = problem.Total_Weis + msg.value;
        ...
        if (TOTAL_S_Fillings != 0) {
            ...
            for (i=1; i<=TOTAL_S_Fillings; i++) {
                ...
                if (s_filling.Problem_ID == Problem_ID) {
                    ...
                    PS_weis = PS_weis + s_filling.Weis;
            ...
            for (j=1; j<=TOTAL_S_Fillings; j++) {
                ...
                if (solution.Problem_ID == Problem_ID) {
                    ...
                    payable(address(solution.Posted_By)).transfer((s_filling.Weis*((s_filling.Weis*1000000000000000000000000000000000000)/PS_weis))/1000000000000000000000000000000000000);
        ...
        payable(address(problem.Posted_By)).transfer((msg.value*((msg.value*1000000000000000000000000000000000000)/PS_weis))/1000000000000000000000000000000000000);
        
        P_Filler storage p_filler = P_FILLER[msg.sender];
        p_filler.Filler        = msg.sender;
        p_filler.Total_Filled  = p_filler.Total_Filled + msg.value;
        
        Posted_By storage posted_by = POSTED_BY[problem.Posted_By];
        posted_by.Total_Weis  = posted_by.Total_Weis + msg.value;
    ...
    function FILL_Solution(uint Solution_ID) payable public {
        ...
        if (TOTAL_S_Fillings != 0) {
            ...
            for (i=1; i<=TOTAL_S_Fillings; i++) {
                ...
                if (_s_filling.Problem_ID == problem.ID) {
                    ...
                    PS_weis = PS_weis + _s_filling.Weis;
            ...
            for (j=1; j<=TOTAL_S_Fillings; j++) {
                ...
                if (_solution.Problem_ID == problem.ID) {
                    ...
                    payable(address(_solution.Posted_By)).transfer((s_filling.Weis*((s_filling.Weis*1000000000000000000000000000000000000)/PS_weis))/1000000000000000000000000000000000000);
        ...
        payable(address(problem.Posted_By)) .transfer((msg.value*((msg.value*500000000000000000000000000000000000)/PS_weis))/1000000000000000000000000000000000000);
        payable(address(solution.Posted_By)).transfer((msg.value*((msg.value*500000000000000000000000000000000000)/PS_weis))/1000000000000000000000000000000000000);
        
        S_Filler storage s_filler = S_FILLER[msg.sender];
        s_filler.Filler        = msg.sender;
        s_filler.Total_Filled  = s_filler.Total_Filled + msg.value;
