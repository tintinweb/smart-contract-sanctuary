...
contract Godsmoneytest {
    /* This creates an array with all balances */
    mapping (address => uint256) public balanceOf;

    string public name = "Godsmoneytest";
    string public symbol = "GSMT";
    uint256 public max_supply = 37500000000000;
    uint256 public unspent_supply = 0;
    uint256 public spendable_supply = 0;
    uint256 public circulating_supply = 0;
    uint256 public decimals = 6;
    uint256 public reward = 48230000;
    ...
    function updateSupply() internal returns (uint256) {
      ...
      if (now - timeOfLastHalving >= 1576800 minutes) {
        reward /= 2;
        timeOfLastHalving = now;
      }

      if (now - timeOfLastIncrease >= 180 seconds) {
        uint256 increaseAmount = ((now - timeOfLastIncrease) / 180 seconds) * reward;
