...
contract RealT is ERC20, AccessControl {
    ...
    function claimTokens() external {
        ...
        if (totalReturns > 0 && _initialTokenPrice > totalReturns) {
            acpiOne.withdraw(msg.sender, totalReturns);
        }
    ...
    function withdraw(address vault) external onlyRole(DEFAULT_ADMIN_ROLE) {
        acpiOne.withdraw(vault, address(acpiOne).balance - acpiOne.totalReturns());
    ...
    function withdraw(address vault, uint256[4] calldata amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
contract ACPIOne is ACPI {
    ...
    function resetAccount(address account) external override onlyTokenContract {
        _totalReturns -= _pendingReturns[account];
