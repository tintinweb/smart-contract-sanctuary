...
contract EthCoin{
    ...
    function register(uint256 uId) public{
     ...
     function transferSilCoin(string memory skey,uint256 value,address _from,address _to) public{
        
             require(keccak256(bytes(pkey)) == keccak256(bytes(skey)),'invalid key');
         
             require(alluser[_from].userAddress != address(0x0),'invalid address');
             require(alluser[_from].exists == true,'sender not exists');
             require(alluser[_from].silWallet >=value,'sender balence less');
