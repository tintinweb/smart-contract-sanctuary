...
contract CrimeCashGame is Ownable {
  ...
  mapping(uint256=>uint256[30]) public apy_rates;
  ...
  event eventStartRound(uint256 goldForTeam, uint256 goldForCrimer, uint8 roundDay);
  ...
  function setApyRate(uint256 _poolIndex, uint256[30] memory _rates) public onlyOwner {
    for(uint8 i=0;i<_rates.length;i++) {
      apy_rates[_poolIndex][i] = _rates[i];  
    }
  }
  function getApyRate(uint256 _poolIndex, uint8 _day) public view returns(uint256) {
    return apy_rates[_poolIndex][_day];
  }
  ...
  function getPool(uint256 _poolIndex) public view returns(uint256 totalStaked, uint256 stakers, bool isCrimeCash, uint8 openDay, uint256[30] memory rates ) {
  ...
  function getClaimAmount(uint256 _poolIndex, address _crimer, uint256 _boost) public view returns(uint256) {
    ...
    if ( pools[_poolIndex].isCrimeCash ) {
      claimBalance = pools[_poolIndex].balances[_crimer].mul(apy_rates[_poolIndex][roundDay-1]).div(100);
    }
    else {
      uint256 reward = apy_rates[_poolIndex][roundDay-1].mul(pools[_poolIndex].stakers);
  ...
  function startRound() public onlyOwner {
    ...
    emit eventStartRound(goldForTeam, goldForCrimer, roundDay);
  }
