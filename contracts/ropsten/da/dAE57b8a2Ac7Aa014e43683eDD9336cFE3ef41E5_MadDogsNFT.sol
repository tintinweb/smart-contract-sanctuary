...
contract MadDogsNFT is ERC721Enumerable, Ownable {
    ...
    uint256 private RESERVED_TOKENS = 100;
    uint256 public MINT_FEE_PER_TOKEN = 4 * 10**16;
    ...
    function mintDog(address _to, uint256 num) public payable {
        ...
        require( num < (MAX_MINT + 1),                     "You can mint a maximum of 20 dogs per call" );
        require( supply + num < MAX_SUPPLY - RESERVED_TOKENS,    "Not enough dogs" );
        require( msg.value >= MINT_FEE_PER_TOKEN * num,                "Incorrect ether value" );
        }
    ...
    function mintWhitelistDog(address _to, uint256 num) public payable {
        uint256 supply = totalSupply();
        if(msg.sender != owner()) {
        require( !whitelistPaused,                         "Whitelist Sale not active" );
        require( num < (MAX_WHITELIST_MINT + 1),            "You can mint a maximum of 20 dogs per call" );
        require( supply + num < MAX_SUPPLY - RESERVED_TOKENS,    "Not enough dogs" );
        require( msg.value >= MINT_FEE_PER_TOKEN * num,                "Incorrect ether value" );
        }
    ...
    function giveawayDog(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= RESERVED_TOKENS, "Exceeds reserved dogs supply" );
