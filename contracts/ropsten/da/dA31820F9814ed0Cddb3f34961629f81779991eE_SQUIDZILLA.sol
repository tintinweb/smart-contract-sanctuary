...
contract SQUIDZILLA is IBEP20, LPSwapSupport, AntiLPSniper {
    ...
    constructor (address _routerAddress, address _tokenOwner, address _charity) LPSwapSupport() public payable {
    ...
    function _takeOtherFees(uint256 tCharity) private returns(uint256) {
        uint256 currentRate =  _getRate();
        uint256 rCharity = 0;
        if(tCharity > 0){
    ...
    function takeFees(uint256 amount, Fees memory _fees) private returns(uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        uint256 rOther = _takeOtherFees(tCharity);
    ...
    function updateBuyFees(uint256 reflectionFee, uint256 liquidityFee, uint256 charityFee, uint256 newFeeDivisor) external onlyOwner {
        buyFees = Fees({
    ...
    function updateSellFees(uint256 reflectionFee, uint256 liquidityFee, uint256 charityFee,uint256 newFeeDivisor) external onlyOwner {
        sellFees = Fees({
            reflection: reflectionFee,
            liquidity: liquidityFee,
            charity: charityFee,
    ...
    function updateTransferFees(uint256 reflectionFee, uint256 liquidityFee, uint256 charityFee, uint256 newFeeDivisor) external onlyOwner {
        transferFees = Fees({
            reflection: reflectionFee,
            liquidity: liquidityFee,
            charity: charityFee,
