...
contract IXTKN100017PreSaleLocked{
...
/*
    ...
    - Will send all liquidity ($$$) to main contract and end of presale
*/
contract XTKN100017PreSale is Context, Ownable, Native{
    // lib
    ...
    // address
        ...
        address public ADDRESS_LOCKED; // address of locked contract

    // instance
        IUniswapV2Router02 public router; // interface for uniswap router
        ERC20 private _token; // interface for parent token
        ERC20 private _preSaleBuyToken; // interface for usd token
        IXTKN100017PreSaleLocked private _locked; // interface for investor lock

    // pre sale token
        address public ADDRESS_PRESALE_USD = 0x07865c6E87B9F70255377e024ace6630C1Eaa37F; // contract address of presale usd token
        uint256 private _preSaleTokenDecimals; // decimals of presale usd token
    ...
    // pre sale pots
        ...
        mapping(uint256 => PotOfGold) public POTS;
        uint256[] public POTS_INDEX;
        ...
        uint256 public PRESALE_END_TIMESTAMP; // end timestamp of presale, last buy will trigger this
    ...
    // events
        ...
        event PreSaleAddPot(uint256 index, uint256 max, uint256 price);
        event PreSaleBuyPot(address indexed from, uint256 pot, uint256 price, bool bought, uint256 available);
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _preSaleTokenDecimals = _preSaleBuyToken.decimals();
    ...
    function init(uint256 pEnd, address pLocked) public onlyOwner{
        // this function is called by main contract to enable this sub contract
        ...
        // enable lock
        ADDRESS_LOCKED = pLocked;
        _locked = IXTKN100017PreSaleLocked(ADDRESS_LOCKED);
        _locked.init();
    }
    ...
    function info(uint256 pTokens) public view returns(bool enabled, uint256 price, uint256 tokens, uint256 max, uint256 available){
        require(POTS[pTokens].enabled, "[xtkn100017 error][presale] no more of these pots available!");
        return(
    ...
    function buy(uint256 pTokens) public{
        ...
        require(!PRESALE_ENDED, "[xtkn100017 error][presale] has ended sorry!");
        require(PRESALE_ENABLED, "[xtkn100017 error][presale] not enabled yet!");

        // check for valid user input
        uint256 allowance = _preSaleBuyToken.allowance(_msgSender(), address(this)); // get allowance of user to trade for $$$$
        uint256 balance = _preSaleBuyToken.balanceOf(_msgSender()); // get how many $$$ tokens the user has
        uint256 price = POTS[pTokens].price; // get the price in $$$ of the pot user wants to buy
        require(allowance >= price, "[xtkn100017 error][presale] you have to authorize the transfer of your USDC first!");
        require(balance >= price, "[xtkn100017 error][presale] you do not have enough USDC!");

        // check if user can still buy the tokens
        require(_available(pTokens), "[xtkn100017 error][presale] no more of these pots available!");
        require(!_bought(_msgSender(), pTokens), "[xtkn100017 error][presale] you can only buy one of this pot, buy another pot!");
        require(_buy(_msgSender(), pTokens, price), "[xtkn100017 error][presale] could not buy the pot!");
    ...
    function _buy(address pInvestor, uint256 pPot, uint256 pPrice) private returns(bool){
        ...
        if(status){
            ...
            // transfer tokens to presale lock, but twice the amount bought as reward!
            _token.transfer(ADDRESS_LOCKED, pPot.mul(2));
            _locked.deposit(pInvestor, pPot);

            emit PreSaleBuyPot(pInvestor, pPot, pPrice, _preSaleInvestors[pInvestor][pPot], POTS[pPot].available);
            return(true);
        }
        return(false);
    }
    ...
    function _swap() private returns(bool){
        ...
        (bool sent, ) = to.call{value:native}("");
        return(sent);
    }
