...
contract FumaFinance is Context, Ownable, IERC20 {
    ...
    constructor() {
        ...
        antiwhale_measures[0] = antiwhale({
            selling_threshold: (_tTotal * 25) / 100000,
            extra_tax: 1
        }); //0.025% of initial supply
        antiwhale_measures[1] = antiwhale({
            selling_threshold: (_tTotal * 50) / 100000,
            extra_tax: 2
        }); //0.05% of initial supply
        antiwhale_measures[2] = antiwhale({
            selling_threshold: (_tTotal * 75) / 100000,
            extra_tax: 3
        ...
        returns (valuesFromGetValues memory s)
    {
        ...
        s.tSwap =
            (tAmount * appliedFees.totFees * appliedFees.swapFee) /
            1000000;
    ...
    function _takeSwapFees(uint256 rSwap, uint256 tSwap) private {
        ...
        if (_isExcluded[address(this)]) _tOwned[address(this)] += tSwap;
    }
    ...
    function setAntiwhaleMeasure(
        ...
        antiwhale_measures[0] = antiwhale({
            selling_threshold: (_tTotal * selling_thresholds[0]) / 100000,
            extra_tax: extra_taxes[0]
        });
        antiwhale_measures[1] = antiwhale({
            selling_threshold: (_tTotal * selling_thresholds[1]) / 100000,
            extra_tax: extra_taxes[1]
        });
        antiwhale_measures[2] = antiwhale({
            selling_threshold: (_tTotal * selling_thresholds[2]) / 100000,
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && to == pancakePair) {
            if (block.timestamp < _firstSell[from] + 24 * 1 hours) {
                require(
                    _totSells[from] + amount <= maxSellPerDay,
        ...
        if (
            overMinTokenBalance && !inSwap && from != pancakePair && swapEnabled
        ) {
