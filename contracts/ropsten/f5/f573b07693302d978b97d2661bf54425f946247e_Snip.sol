...
/**
 * @title ISnipTokensSender
 * @dev ISnipTokensSender token transfer hook interface
 */
interface ISnipTokensSender {
    /**
...
/**
 * @title ISnipTokensRecipient
 * @dev ISnipTokensRecipient token transfer hook interface
 */
interface ISnipTokensRecipient {
    /**
...
/**
 * @notice Partition strategy validator hooks for Snip
 */
interface ISnipPartitionStrategyValidator {
...
library PartitionUtils {
    ...
    {
        return string(abi.encodePacked("SnipPartitionStrategyValidator", _prefix));
    }
}

/**
 * @title ErrorCodes
 * @notice Snip error codes.
 */
contract ErrorCodes {
...
/**
 * @title Snip
 * @notice Snip is an ERC20 compatible collateral token designed to support
 * multiple classes of collateralization systems.
 * @dev The Snip token contract includes the following features:
 *
 * Partitions
 ...
 *
 * Operators
 *   Inspired by ERC777, Snip allows token holders to assign "operators" on
 ...
 *
 * Transfers with Data
 *   Inspired by ERC777, Snip transfers can include arbitrary data, as well as
 ...
 *   Inspired by ERC777, Snip uses the ERC1820 Registry to allow collateral
 ...
 *   Snip is able to define certain sets of partitions, identified by a 4 byte
 ...
 * with collateral systems directly through the Snip contract.
 */
contract Snip is IERC20, ERC1820Client, ERC1820Implementer, ErrorCodes, Ownable {
    ...
    /**
     * @dev SnipToken interface label.
     */
    string internal constant SNIP_INTERFACE_NAME = "SnipToken";
    ...
    /**
     * @dev SnipTokensSender interface label.
     */
    string internal constant SNIP_TOKENS_SENDER = "SnipTokensSender";

    /**
     * @dev SnipTokensRecipient interface label.
     */
    string internal constant SNIP_TOKENS_RECIPIENT = "SnipTokensRecipient";

    /**
     * @dev SnipTokensChecker interface label.
     */
    string internal constant SNIP_TOKENS_CHECKER = "SnipTokensChecker";
     ...
     * forwarded to upon successfully minting Snip.
     */
    address
    ...
    /**
     * @notice Indicates tokens swapped for Snip.
     * @dev The tokens that are swapped for Snip will be transferred to a
     * graveyard address that is for all practical purposes inaccessible.
     * @param operator Address that executed the swap.
     * @param from Address that the tokens were swapped from, and Snip minted for.
     * @param value Amount of tokens swapped into Snip.
     */
    ...
    /**
     * @notice Initialize Snip, initialize the default partition, and register the
    ...
    constructor(
        ...
        ERC1820Client.setInterfaceImplementation(SNIP_INTERFACE_NAME, address(this));
        ERC1820Client.setInterfaceImplementation(ERC20_INTERFACE_NAME, address(this));

        // Indicate token verifies Snip and ERC20 interfaces
        ERC1820Implementer._setInterface(SNIP_INTERFACE_NAME);
     ...
     * that due to other functionality in Snip, this figure should not be used
    ...
    /**************************************************************************/
    /** EXTERNAL FUNCTIONS (SNIP) **********************************************/
    /**************************************************************************/

    /******************************** Swap  ***********************************/

    /**
     * @notice Swap tokens to mint SNIP.
    ...
    /***************************** Token metadata *****************************/
    /**
     * @notice Get the name of the token (Snip).
    ...
    /**
     * @notice Get the symbol of the token (SNIP).
    ...
    /********************************* Hooks **********************************/
    /**
     * @notice Check for and call the 'SnipTokensSender' hook on the sender address
     * (`_from`), and, if `_fromPartition` is within the scope of a strategy,
     * check for and call the 'SnipPartitionStrategy.tokensFromPartitionToTransfer'
    ...
    function _callPreTransferHooks(
        ...
        senderImplementation = interfaceAddr(_from, SNIP_TOKENS_SENDER);
        if (senderImplementation != address(0)) {
            ISnipTokensSender(senderImplementation).tokensToTransfer(
                msg.sig,
                _fromPartition,
                _operator,
                _from,
        ...
        if (_isPartitionStrategy[fromPartitionPrefix]) {
            ...
            if (fromPartitionValidatorImplementation != address(0)) {
                ISnipPartitionStrategyValidator(fromPartitionValidatorImplementation)
    ...
    /**
     * @dev Check for 'SnipTokensRecipient' hook on the recipient and call it.
    ...
    function _callPostTransferHooks(
        ...
        if (_isPartitionStrategy[toPartitionPrefix]) {
            ...
            if (partitionManagerImplementation != address(0)) {
                ISnipPartitionStrategyValidator(partitionManagerImplementation)
        ...
        recipientImplementation = interfaceAddr(_to, SNIP_TOKENS_RECIPIENT);

        if (recipientImplementation != address(0)) {
            ISnipTokensRecipient(recipientImplementation).tokensReceived(
                msg.sig,
                _toPartition,
                _operator,
                _from,
    ...
    function _callPartitionStrategyOperatorHook(
        ...
        if (strategyValidatorImplementation != address(0)) {
            return
                ISnipPartitionStrategyValidator(strategyValidatorImplementation)
