...
abstract contract  AutomatedExternalReflector is Context, LockableSwap, Ownable, IAutomatedExternalReflector {
    ...
    event NO(address indexed sender);
...
//        bool excluded;
    ...
    bool public inDistroMode = false;
    ...
    function addToQueue(address newUserAddress, uint256 bal, bool isSender) private {
        uint256 currentBalance = IERC20(tokenContract).balanceOf(newUserAddress);
        if(isSender){
            bal = currentBalance.sub(bal);
        } else {
            bal = currentBalance.add(bal);
        }
...
//            excluded: false,
        ...
        dynaHodler[newUserAddress][currentRound] = newUser;
        dynaHodler[newUserAddress][currentRound+1] = newUser;
        queue.add(newUserAddress);
    }
    ...
    function logUserTransaction(address user, uint256 value, bool isSender) private {
        if(!queue.contains(user)){
            addToQueue(user, value, isSender);
        } else {
            ...
            if(isSender){
                if(isExcludedFromReflections[user]){
                    totalExcludedTokenHoldings.sub(value);
                }
                dynaHodler[user][currentRound+1].tokenBalance.sub(value);
            } else {
                if(isExcludedFromReflections[user]){
                    totalExcludedTokenHoldings.add(value);
                }
                dynaHodler[user][currentRound+1].tokenBalance.add(value);
            }
        }
    }

    function reflectRewards() external override returns (bool) {
        if(!inSwapAndLiquify && block.timestamp > nextRoundStart && address(this).balance > 0){
    ...
    function _reflectRewards() private lockTheSwap returns(bool allComplete) {
        ...
        while(gasleft() > minGas && startingGas.sub(gasleft()) < maxGas && currentQueueIndex < stopProcessingAt && currentQueueIndex < queue.length()){
    ...
    function updateTokenAddress(address token) public onlyOwner{
        tokenContract = ISupportingExternalReflection(token);
    }

    function updateTotalSupply(uint256 newTotal) public override onlyOwner {
        totalCirculatingTokens = newTotal;
    }

    function collectShare() public override {}
