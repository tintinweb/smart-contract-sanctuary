...
contract MultiToolsCapital is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0xB01D713eBb927924a3fc830CC2d5bD24d866E056); // Marketing Address
        
    address payable public MTCAddress =
        payable(0x87476a657F0fc9A0F7325A0d6bBDBFcA02312932); // MTC Address
    ...
    string private constant _name = "MultiToolsCapital";
    string private constant _symbol = "MTC";
    ...
    uint256 private _MTCTokensToSwap;
    
    bool private gasLimitActive = true;
    uint256 private gasPriceLimit = 280 * 1 gwei; 
    ...
    event UpdatedMTCAddress(address buyback);
    ...
    constructor() {
        ...
        _isExcludedFromFee[MTCAddress] = true;
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _MTCTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyMTCFee;
                buyOrSellSwitch = BUY;
            } 
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellMTCFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_MTCTokensToSwap).add(_marketingTokensToSwap);
        ...
        uint256 bnbForMTC = bnbBalance.mul(_MTCTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForMTC);

        _liquidityTokensToSwap = 0;
        _marketingTokensToSwap = 0;
        _MTCTokensToSwap = 0;
        
        (bool success,) = address(MTCAddress).call{value: bnbForMTC}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _MTCTokensToSwap += tLiquidity * _buyMTCFee / _liquidityFee;
            ...
            _MTCTokensToSwap += tLiquidity * _sellMTCFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyMTCFee)
        external
        onlyOwner
    {
        ...
        _buyMTCFee = buyMTCFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyMTCFee <= 20, "Must keep taxes below 20%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buyMTCFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellMTCFee)
        external
        onlyOwner
    {
        ...
        _sellMTCFee = sellMTCFee;
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellMTCFee <= 30, "Must keep taxes below 30%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellMTCFee);
    }
    ...
    function setMTCAddress(address _MTCAddress) external onlyOwner {
        require(_MTCAddress != address(0), "_MTCAddress address cannot be 0");
        MTCAddress = payable(_MTCAddress);
        _isExcludedFromFee[MTCAddress] = true;
        emit UpdatedMTCAddress(MTCAddress);
    }
