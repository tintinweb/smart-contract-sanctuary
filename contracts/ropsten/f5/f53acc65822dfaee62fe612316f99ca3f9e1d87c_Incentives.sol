...
// File: contracts/interfaces/iIncentives.sol
...
interface IIncentives {
    ...
    */
     ...
     */
      ...
      */
     function createnNewIncentivesToken(string memory name, string memory symbol) external returns (address);
...
contract Incentives is IIncentives{
    ...
    constructor(address _governanceTokens,uint256 Supply,bool _entered) {
        owner = msg.sender;
        entered = _entered;
    ...
    function getAddress(string memory name, string memory symbol) public view returns (address) {
    ...
    function createnNewIncentivesToken(string memory name, string memory symbol) external onlyOwner override returns (address contractAddress){
        bytes memory bytecode = type(IncentivesToken).creationCode;
        bytecode = abi.encodePacked(bytecode, abi.encode(name, symbol, initialSupply));
        bytes32 salt = keccak256(abi.encodePacked(name, symbol));
    ...
    function withdrawGovernance(uint256 amount) external override{
        ...
        require(amount>0 
        && users[msg.sender].govTokenBalances.sub(users[msg.sender].maxBorrowed.amount) >= amount,"available amount exceeded");
    ...
    function withdrawIncentives(address tokenAddress, uint256 amount) external override{
        require(users[msg.sender].id>0,"user not exist");
        require(amount>0 
        && users[msg.sender].govTokenBalances.sub(users[msg.sender].borrowedList[tokenAddress]) >= amount,"available amount exceeded");
    ...
    function putIncentivesTokens(address tokenAddress, uint256 amount) external override{
        require(users[msg.sender].id>0,"user not exist");
        require(amount>0 
        && users[msg.sender].govTokenBalances.sub(users[msg.sender].borrowedList[tokenAddress]) >= amount,"bad amount or tokenAddress");
        require(
    ...
    function userState(address user) view external override returns(address[] memory, uint256[] memory){
        ...
        balances[allTokens.length] = users[user].govTokenBalances;
