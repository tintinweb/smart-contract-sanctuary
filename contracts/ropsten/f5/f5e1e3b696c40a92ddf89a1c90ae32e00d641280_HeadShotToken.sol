...
contract HeadShotToken is Context, IERC20, Ownable {
    ...
    */
    ...
    function getPurchases(address buyer) public view returns (uint256[] memory, string[] memory, string[] memory, uint256[] memory, uint256[] memory) {
        uint256[] memory codes = new uint256[](_salesList.length);
        string[] memory names = new string[](_salesList.length);
        string[] memory descs = new string[](_salesList.length);
        uint256[] memory prices = new uint256[](_salesList.length);
        uint256[] memory balances = new uint256[](_salesList.length);
        if (_salesList.length > 0){
            for (uint index = 0; index < _salesList.length; index++) {
