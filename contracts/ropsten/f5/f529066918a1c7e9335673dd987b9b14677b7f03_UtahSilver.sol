...
contract UtahSilver {
    ...
    event ReceievedToken(address indexed receiver, address indexed _from, uint8 indexed matrix, uint8 level,uint amount, uint time);
    ...
    function registration(address userAddress, address referrerAddress)  private isLock {
        require(omgToken.balanceOf(msg.sender) >= levelPrice[1].mul(2),"Insufficient token balance");
        ...
        emit Registration(userAddress, referrerAddress, users[userAddress].id,users[referrerAddress].id,levelPrice[1].mul(2),now);
    }
    ...
    function sendTokenDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (isExtraDividends) {
            emit SentExtraTokenDividends(_from, receiver, matrix, level, levelPrice[level], now);
             ...
             emit ReceievedToken(receiver, _from, matrix, level, levelPrice[level], now);
        }
    }
