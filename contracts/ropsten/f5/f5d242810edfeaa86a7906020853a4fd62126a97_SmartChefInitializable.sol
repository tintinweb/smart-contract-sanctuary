...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        lockTime = _lockTime;
        endLockTime = startBlock + lockTime;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                require(
                    block.number >= endLockTime ||
                        block.number >= bonusEndBlock,
                    "Lock still in affect, or pool hasn't ended yet."
                );
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (pending > 0) {
            require(
                block.number >= endLockTime || block.number >= bonusEndBlock,
                "Lock still in affect, or pool hasn't ended yet."
            );
            rewardToken.safeTransfer(address(msg.sender), pending);
        }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare.add(
                SOKUReward.mul(PRECISION_FACTOR).div(stakedTokenSupply)
            );
            return
                user
                    .amount
