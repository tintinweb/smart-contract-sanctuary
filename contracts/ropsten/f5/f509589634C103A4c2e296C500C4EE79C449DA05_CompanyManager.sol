...
library Roles {
    ...
    * Compare two roles
    */
    function isSame(bytes32 _a, bytes32 _b) public pure returns (bool) {
        return _a == _b;
    }
    ...
    */
    function isValid(bytes32 _a) public pure returns (bool) {
        return isSame(_a, ALL_ROLE) || isSame(_a, CREATE_ROLE)
               || isSame(_a, UPDATE_ROLE) || isSame(_a, DELETE_ROLE)
               || isSame(_a, TRANSFER_ROLE);
    }

    function hasCreateRole(bytes32 _a) public pure returns (bool) {
        return isSame(_a, ALL_ROLE) || isSame(_a, CREATE_ROLE);
    }

    function hasUpdateRole(bytes32 _a) public pure returns (bool) {
        return isSame(_a, ALL_ROLE) || isSame(_a, UPDATE_ROLE);
    }

    function hasDeleteRole(bytes32 _a) public pure returns (bool) {
        return isSame(_a, ALL_ROLE) || isSame(_a, DELETE_ROLE);
    }

    function hasTransferRole(bytes32 _a) public pure returns (bool) {
        return isSame(_a, ALL_ROLE) || isSame(_a, TRANSFER_ROLE);
    }
}
...
interface ICompanyManager {
    function addCompany(string calldata _idCPMY, string calldata _name, address _owner, address _nfts) external returns (address);
...
contract InventoryManager is Adminable, ERC1155Holder {
    ...
    string id;
    mapping(uint256 => nfts) public nftsItems;

    enum State {Actived, Blocked, ReadOnly}
    State public inventoryState = State.Actived;

    // Events to emit
    event ItemAdded(uint256 _id, address _erc1155, uint256 _amount, bytes payload);


    /**
    * Modifier to verify if the inventory is actived
    */
    modifier isActived() {
        require(inventoryState == State.Actived);
        _;
    }
    
    /**
    * Modifier to verify if writte permissions are enable
    */
    modifier isWritteEnable() {
        require(inventoryState != State.Blocked, "The inventory is blocked at this moment");
        require(inventoryState != State.ReadOnly, "The inventory was set with read only permissions");
        _;
    }
    ...
    */
    ...
    /**
    * Used to set and change linked NFTs (ERC1155) smartcontract
    * Also keeps all ERC1155 in an array to future verification of balances and assets
    */
    function setTraceablesContract(address _nftcontract) external onlyOwnerOrAdmin isActived {
        nftcontract = Traceables(_nftcontract);
    }

    /**
    * Default function to verify if supports an interface like ERC1155 and AccessControl
    */
    ...
    /**
    * Used to add new item/traceables (NFT) to the current inventory and company
    * This function call the mint function on a default OpenZeppelin ERC1155 contract, set by setTraceablesContract function
    */
    function addItem(uint256 _id, bytes memory _payload, uint256 _amount) external onlyOwnerOrAdmin {
        require(nftsItems[_id].exists != true, "The NFT for the passed id already exists in this inventory");
        nftcontract.mint(address(this), _id, _amount, _payload);
        nftsItems[_id] = nfts(address(nftcontract), nftcontract.uri(_id), true, address(0));
        emit ItemAdded(_id, nftsItems[_id].nftAddress, _amount, _payload);
    }

    /**
    * Used to block all functions inside the contract, except the change state function
    * Like "activate", "readOnly", "blocks"
    */
    function blocks() external onlyOwnerOrAdmin {
        inventoryState = State.Blocked;
    }

    /**
    * Used to enable all functions inside the contract
    */
    function activate() external onlyOwnerOrAdmin {
        inventoryState = State.Actived;
    }

    /**
    * Used to disable all writte functions inside the contract
    * But keep read functions enabled
    */
    function readOnly() external onlyOwnerOrAdmin {
        inventoryState = State.ReadOnly;
    }

    /**
    * Used to update inventory
    */
    function onReceiveNFT(uint256 _id, address _nftcontract, address _from) external isActived returns (bool) {
        nftsItems[_id] = nfts(_nftcontract, Traceables(_nftcontract).uri(_id), true, _from);
        return true;
    }

    /**
    * Check if inventory is actived
    */
    function isActiveToReceive() external view returns (bool) {
        return inventoryState == State.Actived;
    }

    /**
    * Transfer an NFT/Traceable to another inventory
    */
    function transfer(uint256 _id, address _to, uint256 _amount, bytes memory _payload) isActived external {
        InventoryManager toContract =  InventoryManager(_to);
        require(toContract.isActiveToReceive() == true, 'The To invetory is not actived');
        nftcontract.safeTransferFrom(address(this), _to, _id, _amount, _payload);
        delete nftsItems[_id];
        toContract.onReceiveNFT(_id, address(nftcontract), address(this));
    }
}
