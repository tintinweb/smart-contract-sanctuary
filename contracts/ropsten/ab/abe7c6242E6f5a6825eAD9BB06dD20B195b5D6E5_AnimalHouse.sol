...
contract AnimalHouse is ERC1155, AbstractAnimalHouseEntity {
  ...
  */
  ...
  function getAllContents() public view returns (NftContent[] memory) {
      uint nLen = _contentId;
      NftContent[] memory contents = new NftContent[](nLen);
      
      for(uint i = 0; i < nLen; i++) {
          NftContent storage content = _idToNftContent[i+1];
          contents[i] = content;
      }
  ...
  /* Transfer NFT by id */  
  function buyNFT(uint256 contentId, address to, uint256 amount) external {
      
    uint capacity = balanceOf(address(this), contentId);
    
    require(capacity > amount  , "You don't have capacity");
