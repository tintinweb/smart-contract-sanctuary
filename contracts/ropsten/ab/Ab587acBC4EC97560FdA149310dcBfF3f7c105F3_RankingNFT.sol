...
contract RankingNFT is ERC721, Ownable {
    ...
    event OnMintNFT(address indexed _actualMinter, string indexed _tokenURI, uint256 _newItemId);
    ...
    event OnBuyTokenAfterMinting(uint256 indexed _tokenId, address indexed _minter, uint256 indexed _price, uint256 _numberOfTransfers);
    ...
    function mintNFT(string memory _tokenURI, uint256 _initERC20Price) public returns (uint256) {
        ...
        PurchaseInfo memory initPurchase = PurchaseInfo(owner(), address(0), _initERC20Price, 0, block.timestamp, block.timestamp);
        ...
        newHistory.currentOwner = address(0); //mint person or last buyer 
        ...
        emit OnMintNFT(msg.sender, _tokenURI, newItemId);

        // returns the id for the newly created token
        return newItemId;
  }

  function buyTokenAfterMinting(uint256 _tokenId) public payable onlyOwner returns(bool) {
    ...
    // This token will sell only to the minter
    ...
    require(beforeBalanceForOwner >= transHistory.purchaseInfoList[transHistory.numberOfTransfers].goalPrice);

    token.transferFrom(msg.sender, owner(), transHistory.purchaseInfoList[transHistory.numberOfTransfers].goalPrice);
    ...
    emit OnBuyTokenAfterMinting(_tokenId, msg.sender, transHistory.purchaseInfoList[transHistory.numberOfTransfers].finalPrice, transHistory.numberOfTransfers);
  ...
  function buyToken(uint256 _tokenId) public payable returns(bool) {
    ...
    require(beforeBalanceForBuyer >= transHistory.purchaseInfoList[transHistory.numberOfTransfers].goalPrice);

    uint256 beforeBalanceForCompany = token.balanceOf(owner());
    token.transferFrom(msg.sender, owner(), companyPortion);
    uint256 afterBalanceForCompany = token.balanceOf(owner());    
    ...
    //buyer's balance
    uint256 afterBalanceForBuyer = token.balanceOf(msg.sender);
