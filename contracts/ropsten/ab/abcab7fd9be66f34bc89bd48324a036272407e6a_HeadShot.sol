...
contract HeadShot is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        _salesLedgerAddress = 0xec86379EbeC996DC0684a1C07181905DabE10303; // in Ropsten
        headShotSalesLedger = IHeadShotSalesLedger(_salesLedgerAddress);
        //headShotSalesLedger.transferOwnership(address(this));

        _tokenLedgerAddress = 0xd8f93385Cb1d3e2D753b9308ce8c842358DfBd7d; // in Ropsten
        ...
        _ecosystemWalletAddress = 0xD7c891f24eEEeE63BB420B177E4cB52b1123FEEd;
    }
    ...
    */
    ...
    function collect(uint256 code_, uint256 count_) public returns (bool) {
        if (headShotSalesLedger.getSalesCode(code_) == code_){
            ...
            if (_maxBuy > 0){
                ...
                uint256 maxCount_ = (_maxBuy - _balanceAfter) + count_;
                if (count_ > maxCount_){
                    count_ = maxCount_;
                }
            }
            ...
            _transfer(_account, _salesWalletAddress, _totalPay);
            bool trx = headShotSalesLedger.buy(_account, code_, count_);
            return trx;
        }
        return false;
    }
    ...
    function getTokenTrackerFieldString(uint256 tokenAddress_, string memory key_) public view returns (string memory){
        return headShotTokenLedger.getTrackerFieldString(tokenAddress_, key_);
    }
    function getTokenTrackerFieldNumber(uint256 tokenAddress_, string memory key_) public view returns (uint256){
        return headShotTokenLedger.getTrackerFieldNumber(tokenAddress_, key_);
    }
    function getTokenTrackerFieldAddress(uint256 tokenAddress_, string memory key_) public view returns (address){
