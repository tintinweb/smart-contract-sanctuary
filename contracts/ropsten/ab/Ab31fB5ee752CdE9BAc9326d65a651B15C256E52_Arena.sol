...
contract Arena is IArena, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
    ...
    function startGame() external view onlyOwner returns (uint256) {
        ...
        uint256 rndArenaIndex3 = ((randomNum & 0xFFFFFFFF) % arenas.length) - 1; // choose a value from 0 to the number of arenas
        // int rndIndex = Random(0, areas.Length);
        // ArenaInfo memory currentArena = arenas[rndArenaIndex];
        return rndArenaIndex3;
        //return currentArena;
    ...
    function rndArenaIndex(uint256 num) external view returns (uint256) {
        uint256 randomNum = random(num);
        uint256 rndArenaIndex2 = ((randomNum & 0xFFFFFFFF) % arenas.length); // choose a value from 0 to the number of arenas

        return rndArenaIndex2;
    }

    function getRandom(uint256 num) external view returns (uint256) {
        uint256 randomNum = random(num);
        //uint256 rndArenaIndex = ((randomNum & 0xFFFFFFFF) % arenas.length); // choose a value from 0 to the number of arenas
