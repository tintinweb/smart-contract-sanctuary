...
contract Azell is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Azell";
    string public symbol = "AZELL";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint256 networkFee = transferFee.div(2);
        uint256 blockFee = transferFee.div(2);
        
        proofOfAgeRewards = proofOfAgeRewards.add(networkFee);
        proofOfBlockRewards = proofOfBlockRewards.add(blockFee);
        
        emit Transfer(msg.sender, to, valueAfterFee);
        emit NetworkFee(msg.sender, networkFee);
        emit BlockPoolFee(msg.sender, blockFee);
    ...
    function transferFrom(address sender, address recipient, uint256 value) onlyPayloadSize(3 * 32) external returns (bool) {
        ...
        emit BlockPoolFee(sender, blockFee);
...
//--------------------------------------------------------------------------------------
    ...
    event BlockPoolFee(address indexed from, uint256 value);
