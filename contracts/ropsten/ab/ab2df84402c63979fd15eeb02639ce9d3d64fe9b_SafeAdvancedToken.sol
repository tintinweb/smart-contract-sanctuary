...
contract SafeAdvancedToken is TokenERC20 {
    ...
    uint256 public preICOstartTime = now;//UNIX time stamp that determines when will the funding start
    uint256 public preICOendTime = now + durationInyears  * 1 years;//UNIX time stamp that determines when will the funding end
    uint256 public ICOstartTime = now;//UNIX time stamp that determines when will the funding start
    uint256 public ICOendTime = now + durationInyears  * 1 years;//UNIX time stamp that determines when will the funding end
