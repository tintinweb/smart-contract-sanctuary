...
abstract contract ERC20 is Ownable {
  ...
  function mint(address account, uint256 amount) onlyOwner public {
     ...
     function mintInterest(address account, uint256 amount)  internal {

        totalSupply = totalSupply.add(amount);
        balances[account] = balances[account].add(amount);
        emit MintInterest(address(0), account, amount);
        emit Transfer(address(0), account, amount);
    }
...
require(newInterests <=0 ,"Interest will be available after 1 Day.");
    ...
    mintInterest(msg.sender,newInterests);
...
function multiply(uint x, uint y) internal pure returns (uint z) {
    ...
    /*
function testme(uint256 buyTokens1) public view returns(uint256 rate1,uint256 val,uint256 val2,uint256 val3)
{
        uint256 weiAmount = buyTokens1;
    require(weiAmount > 0, "You need to send some ether");

//uint256 rate = getRateOfToken();
    // calculate token amount to be created
    uint256  t1 =  multiply(weiAmount, (10**decimals));
    uint256 t2 = 1 ether;
    
   // uint256 tokens = ((weiAmount  * (rate)) / (1* 10**decimals)) ;
    
   // uint256 tokenWeiAmount = (tokens * (1* 10**decimals));
    
 uint256 tokenWeiAmount = ((multiply(weiAmount, (10**decimals)) / 1 ether) * CPNPrice);
   uint256 tokensToBuy = (tokenWeiAmount / (1* 10**decimals));

    
    return(t1,tokenWeiAmount,tokensToBuy,t2);
...
//tokens = (tokens) / (10 ** decimals);
 ...
 emit Bought(msg.sender,tokenWeiAmount);
  }
