...
contract Presale is ReentrancyGuard, Ownable {
  ...
  constructor (address _arcAddress, uint256 _startingTimestamp) public {
    arc = IBEP20(_arcAddress);
    isSaleActive = true;
  ...
  function buy (address beneficiary) public payable nonReentrant {
    ...
    require (_bnbSent <= 0.5 ether, "BNB is greater than max value");
    require (bnbReceived <= 2 ether, "Hardcap reached");
    ...
    require(tokensOwned[_buyer] <= 1000 ether, "Can't buy more than 0.5 BNB worth of tokens");
  ...
  function getArcTokensLeft () external view returns (uint256) {
  ...
  function claimTokens () external {
    ...
    require (tokensOwned[msg.sender] > 0, "User should own some arc tokens");
    require (tokensUnclaimed[msg.sender] > 0, "User should have unclaimed arc tokens");
    require (arc.balanceOf(address(this)) >= tokensOwned[msg.sender], "There are not enough arc tokens to transfer");
    ...
    arc.transfer(msg.sender, tokensOwned[msg.sender].div(10));
  ...
  function withdrawUnsoldArc() external onlyOwner {
...
contract PresaleFix is ReentrancyGuard, Ownable {
  ...
  constructor(address _arcAddress, address payable _presaleAddress) public {
  ...
  function claimTokens () external {
    ...
    "User should own some arc tokens");
    require (arc.balanceOf(address(this)) >= tokensOwned[msg.sender], 
    "There are not enough arc tokens to transfer");
