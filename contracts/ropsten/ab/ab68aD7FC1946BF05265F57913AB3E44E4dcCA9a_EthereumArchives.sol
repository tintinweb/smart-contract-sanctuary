...
contract EthereumArchives is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    event Mint(uint256 indexed timestamp, address indexed sender, uint256 tokenId, uint256 indexed blockNumber, string blockName);
    ...
    function ownerMint(uint256 tokenId, uint256 blockNumber, string memory blockName) public onlyOwner {
        _checkIfMintStarted();

        require(tokenId > lastTokenId - OWNER_RESERVATION_COUNT && tokenId < lastTokenId, "Token id is invalid");
        _mint(tokenId, blockNumber, blockName);
    }

    function mint(uint256[] memory blockNumbers, string[] memory blockNames) public payable nonReentrant {
        _checkIfMintStarted();
        ...
        for (uint256 i = 0; i < blockNumbers.length; i++) {
            ...
            string memory blockName = blockNames[i];

            _mint(tokenId, blockNumber, blockName);
        }
    }

    function burnAndMint(uint256 oldBlockNumber, uint256 newBlockNumber, string memory newBlockName) public nonReentrant {
        ...
        _mint(newTokenId, newBlockNumber, newBlockName);
    }

    function updateName(uint256 blockNumber, string memory newName) public {
        ...
        _changeName(blockNumber, newName);
    }
    ...
    function _mint(uint256 tokenId, uint256 blockNumber, string memory blockName) private {
        _validateBlockNumber(blockNumber);
        _validateName(blockName);
        ...
        _changeName(blockNumber, blockName);
        _safeMint(_msgSender(), tokenId);

        emit Mint(block.timestamp, msg.sender, tokenId, blockNumber, blockName);
    }
    ...
    function _validateName(string memory name) pure private {
        require(bytes(name).length <= 42, "Name cannot extend 42 characters");
    }

    function _changeName(uint256 blockNumber, string memory newName) private {
