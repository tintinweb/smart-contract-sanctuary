...
contract DT is Context, Ownable, IERC20, LockableSwap, EthReflectingToken {
    ...
    using SwapWithLP for IUniswapV2Router02;
...
//    }
    ...
    FeeLibrary.FeeLevels public FeeLevels;
...
//    }
    ...
    function calculateSell(address from, address to, uint256 amount) private returns(uint256 rAmount , uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        if(block.timestamp > trader[from].lastBuy.add(sellFees.level[FeeLibrary.FeeLevels.LEVEL5].saleCoolDownTime)) {
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLibrary.FeeLevels.LEVEL1].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL1].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLibrary.FeeLevels.LEVEL2].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL2].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLibrary.FeeLevels.LEVEL3].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL3].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLibrary.FeeLevels.LEVEL4].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL4].saleCoolDownFee;
        } else if(block.timestamp < trader[from].lastBuy.add(sellFees.level[FeeLibrary.FeeLevels.LEVEL5].saleCoolDownTime)) {
            timeBasedFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL5].saleCoolDownFee;
        }

        uint256 finalSaleFee = _minimumSizeFee;
        uint256 poolSize = amountInPool();
        if(amount < poolSize.mul(sellFees.level[FeeLibrary.FeeLevels.LEVEL5].saleSizeLimitPercent).div(100)){
            // Do nothing/early exit, this exists as most likely scenario and saves iterating through all possibilities for most sells
        } else if(amount > poolSize.mul(sellFees.level[FeeLibrary.FeeLevels.LEVEL1].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL1].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[FeeLibrary.FeeLevels.LEVEL2].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL2].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[FeeLibrary.FeeLevels.LEVEL3].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL3].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[FeeLibrary.FeeLevels.LEVEL4].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL4].saleSizeLimitPrice;
        } else if(amount > poolSize.mul(sellFees.level[FeeLibrary.FeeLevels.LEVEL5].saleSizeLimitPercent).div(100)) {
            finalSaleFee = sellFees.level[FeeLibrary.FeeLevels.LEVEL5].saleSizeLimitPrice;
        }
...
//    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal {
        ...
        // swap tokens for ETH
        uniswapV2Router.swapTokensForEth(address(this), half); // <- this breaks the ETH ->  swap when swap+liquify is triggered
        ...
        uniswapV2Router.addLiquidity(address(this), otherHalf, newBalance);
    ...
    function swapEthBasedFees(uint256 reflections, uint256 buyback) private returns(uint256 ethForReflections){
        ...
        // swap tokens for ETH
        uniswapV2Router.swapTokensForEth(address(this), reflections.add(buyback)); // <- this breaks the ETH ->  swap when swap+liquify is triggered
    ...
    function buyBackTokens(uint256 amount) internal {
        if (amount > minBuyBack && amount < address(this).balance) {
            if(amount > maxBuyBack)
                amount = maxBuyBack;
            uniswapV2Router.swapETHForTokens(amount, buybackSinkAddress);
        }
...

{
  ...
  "libraries": {
    "contracts/libs/FeeLibrary.sol": {
      "FeeLibrary": "0x27a6d4421b0cb9612c70fcf57b4bb11161d65c77"
    },
