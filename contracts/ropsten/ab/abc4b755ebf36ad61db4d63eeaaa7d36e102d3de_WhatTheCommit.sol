    ...
    ERC721Pausable
{
    ...
    constructor() ERC721("WhatTheCommit", "WTC") {
        _tokenIdsOwner._value = MAX_TOKENS - TOKENS_PRESERVED_FOR_OWNER; // counter for public should start before owner count
    }
    ...
    function ownerMint(uint256 numTokens) public onlyOwner {
        require(_tokenIdsOwner.current() + numTokens < TOKENS_PRESERVED_FOR_OWNER, "Owner has not enough supply left");
        
        for (uint i = 0; i < numTokens; i++){
            tokenLockedFromTimestamp[_tokenIds.current()] = TOKEN_LOCK_TIMESTAMP;
            
            _mint(msg.sender, _tokenIdsOwner.current());
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "What the commit #', toString(tokenId), '", "description": "What the commit is a NFT project about random, sometimes hilarious, commit messages on the blockchain.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
