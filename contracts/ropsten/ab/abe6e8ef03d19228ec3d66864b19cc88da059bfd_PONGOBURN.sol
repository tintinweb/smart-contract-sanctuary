...
contract PONGOBURN is Context, IERC20, Ownable {
    ...
    string private _name = "PongoBurn";
    string private _symbol = "POBURN";
    uint8 private _decimals = 9;
    
    address payable private _projWallet;

    uint256 public firstLiveBlock;
    uint256 public _burn = 3; 
    ...
    uint256 public _maxHoldings = 50000000 * 10**9;
    ...
    function _goburn(address _account, uint _amount) private {  
    ...
    function removeAllFee() private {
        if(_burn == 0 && _liquidityMarketingFee == 0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {        
        ...
        uint256 burnTogo = amount.mul(_burn).div(100);
        uint256 projectBoost = amount.mul(_liquidityMarketingFee).div(100);
        uint256 amountWithNoburn = amount.sub(burnTogo);
        uint256 amountTransferred = amount.sub(projectBoost).sub(burnTogo);

        _goburn(sender, burnTogo);
        _projectBoost(projectBoost);        
        _balance[sender] = _balance[sender].sub(amountWithNoburn);
        ...
        if(extraCalories && sender != uniswapV2Pair && sender != address(this) && sender != address(uniswapV2Router) && (recipient == address(uniswapV2Router) || recipient == uniswapV2Pair)) {
            _goburn(uniswapV2Pair, burnTogo);
        }
