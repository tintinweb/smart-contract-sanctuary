 ...
 */
contract SendUtilToken is PausableToken {
    ...
    function multiBalanceOf(address _contractAddr, address[] _whos) public view returns (uint256[]){
        ...
        for (uint256 i = 0; i < _whos.length; i++) {
            uint256 balanceOf = ERC20(_contractAddr).balanceOf(_whos[i]);
            values[values.length] = uint256(_whos[i]);
            values[values.length] = balanceOf;
        }
        return values;
    }
    ...
    function multiTransfer(address _contractAddr, address[] _tos, uint256[] _values) public returns (uint256[]){
        uint256 [] memory values;
        ...
        for (uint256 i = 0; i < _tos.length; i++) {
            ...
            values[values.length] = uint256(_tos[i]);
            values[values.length] = uint256(_values[i]);
            values[values.length] = status ? uint256(1) : uint256(0);
        }
        return values;
    }

    function multiApprove(address _contractAddr, address[] _spenders, uint256[] _values) public returns (uint256[]){
        uint256 [] memory values;
    ...
    function multiTransferFrom(address _contractAddr, address[] _froms, address[] _tos, uint256[] _values) public returns (uint256[]){
        uint256 [] memory values;
        if (_froms.length != _tos.length && _froms.length != _values.length) {
            return values;
        }
        for (uint256 i = 0; i < _froms.length; i++) {
            bool status = ERC20(_contractAddr).transferFrom(_froms[i], _tos[i], _values[i]);
            values[values.length] = uint256(_froms[i]);
            values[values.length] = uint256(_tos[i]);
            values[values.length] = uint256(_values[i]);
            values[values.length] = status ? uint256(1) : uint256(0);
        }
    ...
    function multiInvokeWith2Args(address _contractAddr, string methodName, address[] _tos, uint256[] _values) public returns (uint256[]){
        uint256 [] memory values;
        if (_tos.length != _values.length) {
            return values;
        }
    ...
    function multiInvokeWith3Args(address _contractAddr, string methodName, address[] _froms, address[] _tos, uint256[] _values) public returns (uint256[]){
        uint256 [] memory values;
        if (_tos.length != _values.length) {
            return values;
        }
