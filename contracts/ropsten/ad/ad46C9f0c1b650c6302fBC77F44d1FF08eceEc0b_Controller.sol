...
contract Controller is IController, Controllable {
  ...
  event FeeRewardForwarderChanged(address value);
  event GovernanceChanged(address value);
  event BookkeeperChanged(address value);
  ...
  event PSNumeratorDenominatorChanged(uint256 numerator, uint256 denominator);

  // ************ VARIABLES **********************
  string public constant VERSION = "0";
  uint256 public override profitSharingNumerator = 100;
  uint256 public override profitSharingDenominator = 1000;
  ...
  function setPSNumeratorDenominator(uint256 numerator, uint256 denominator) external onlyGovernance {
    require(numerator <= denominator, "invalid values");
    require(denominator != 0, "cannot divide by 0");
    profitSharingNumerator = numerator;
    profitSharingDenominator = denominator;
    emit PSNumeratorDenominatorChanged(numerator, denominator);
  }
  ...
  function isAllowedUser(address _adr) external view override returns (bool) {
    return (_adr == tx.origin) // sender is not smart contract
    || whiteList[_adr]
    || isGovernance(_adr)
    || isHardWorker(_adr)
