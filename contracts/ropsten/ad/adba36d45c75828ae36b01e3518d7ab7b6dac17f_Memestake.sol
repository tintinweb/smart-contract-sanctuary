...
contract Memestake is Ownable {
    ...
    struct UserInfo{
        ...
        // claimable DGB by a user is:
        //
        //   pending reward = (user.amount * pool.accDgbPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDgbPerShare` (and `lastRewardedBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock; // Last block number where DGB got distributed.
        uint256 accMfundPerShare; // Accumulated DGB per share.
        uint256 stakedAmount; // Total staked tokens for this pool.
    }
    ...
    constructor(Memefund _mFundToken,
                ...
                require(address(_mFundToken) != address(0), "constructor: _dgbToken must not be zero address!");
                require(_totalMfund > 0, "constructor: _totalDgb must be greater than 0");
    ...
    /// @param _stakedAmount For this pool, maximum amount per user that can be staked
    /// @param _withUpdate Set to true for updating all pools before adding this one
    function add(uint256 _allocPoint, IERC20 _tokenContract, uint256 _stakedAmount, bool _withUpdate) public onlyOwner {
        ...
        require(_stakedAmount > 0, "add: _stakedAmount must be greater than zero");
    ...
    /// @param _stakedAmount Maximum amount that a user can deposit into the far
    /// @param _withUpdate Set to true if you want to update all pools before making this change - it will checkpoint those rewards
    function set(uint256 _pid, uint256 _allocPoint, uint256 _stakedAmount, bool _withUpdate) public onlyOwner {
        require(block.number < endBlock, "set: must be before end");
        require(_pid < poolInfo.length, "set: invalid _pid");
        require(_stakedAmount> 0, "set: _stakedAmount must be greater than zero");
        ...
        poolInfo[_pid].stakedAmount= _stakedAmount;
    }

    /// @notice View function to see pending and unclaimed Dgbs for a given user
    /// @param _pid ID of the pool where a user has a stake
    /// @param _user Account being queried
    /// @return Amount of DGB tokens due to a user
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        require(_pid < poolInfo.length, "pendingDgb: invalid _pid");
    ...
    function claimRewards(uint256 _pid) public{
        ...
        safeDgbTransfer(msg.sender, pending);
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        require(user.amount.add(_amount) <= pool.stakedAmount, "deposit: can not exceed max staking amount per user");
    ...
    /// @dev Safe DGB transfer function, just in case if rounding error causes pool to not have enough LPTs.
    /// @param _to Whom to send DGB into
    /// @param _amount of DGB to send
    function safeDgbTransfer(address _to, uint256 _amount) internal {
        uint256 dgbBal = mFundToken.balanceOf(address(this));
        if (_amount > dgbBal) {
            mFundToken.transfer(_to, dgbBal);
        } else {
