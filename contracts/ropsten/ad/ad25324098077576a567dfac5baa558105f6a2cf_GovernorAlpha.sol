...
contract GovernorAlpha is GovernorStorage, GovernorEvents {
    ...
    uint public constant MIN_PROPOSAL_THRESHOLD = 50; // 0.5% voting power

    /// @notice The maximum setable proposal threshold
    uint public constant MAX_PROPOSAL_THRESHOLD = 500; // 5% voting power
    ...
    /// @notice Percentage of votes needed for a proposal to approved
    uint public proposalThreshold = 100; // 1% voting power
    
    /// @notice The delay before voting on a proposal may take place, once proposed, in blocks
    uint public votingDelay = 1; // 1 block

    /// @notice The duration of voting on a proposal, in blocks
    uint public votingPeriod =  20; // ~3 days in blocks (assuming 15s blocks)

    /// @notice The duration of the approval phase of a proposal
    uint public approvalPeriod = 20; // ~1 day in blocks (assuming 15s blocks)
    ...
    /// @notice The number of Tokens required in order for a voter to escalate a proposal
    function approveThreshold() public view returns (uint256) {
        uint256 minimumVotes =  SafeMath.div(ausc.initSupply(), 10000); // Approval requirement is always a multiple of 0.01% of AUSC supply
        return SafeMath.mul(minimumVotes, proposalThreshold); 
    }

    /// @notice The number of Tokens required in order for a voter to create a proposal
    function createThreshold() public pure returns (uint256) { return 9000; }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(ausc.balanceOf(msg.sender) >= createThreshold(), "ITS UNDER 9000!! Proposer balance below create threshold");
    ...
    function approve(uint256 proposalId)
        public
    {
        require(state(proposalId) == ProposalState.Created, "GovernorAlpha::add: proposal already escalated");
        require(ausc.getPriorVotes(msg.sender, SafeMath.sub(block.number, 1)) >= approveThreshold(), "GovernorAlpha::propose: approver votes below approval threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || msg.sender == proposal.proposer || ausc.balanceOf(proposal.approver) < approveThreshold(), "GovernorAlpha::cancel: proposer above threshold");
    ...
    /**
      ...
      * @param newQuorum new proposal threshold
      */
    function _setQuorumPercent(uint newQuorum) public {
        require(msg.sender == admin, "GovernorAlpha::_setQuorumPercent admin only");
        require(newQuorum >= MIN_QUORUM_THRESHOLD && newQuorum <= MAX_QUORUM_THRESHOLD, "GovernorBravo::_setQuorumPercent: invalid quorum percent");
        uint oldPercent = quorumPercent;
        quorumPercent = newQuorum;
        emit QuorumPercentSet(oldPercent, quorumPercent);
    }
        /**
    ...
    function _acceptTimelockAdmin()
        public
    {
        require(msg.sender == guardian, "GovernorAlpha::_acceptTimelockAdmin: sender must be gov guardian");
