...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSQUAD }
    ...
    address public sSQUAD;
    uint public sSQUADQueue; // Delays change to sSQUAD address
    ...
    constructor (
       ...
       //isLiquidityToken[ _SQUADDAI ] = true;
       //liquidityTokens.push( _SQUADDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for SQUAD
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SQUAD needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SQUAD ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SQUAD for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        ISQUADERC20( SQUAD ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSQUAD ).balanceOf( msg.sender ); // Can only borrow against sSQUAD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SQUAD
        @param _amount uint
     */
    function repayDebtWithSQUAD( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISQUADERC20( SQUAD ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SQUAD, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SQUAD ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SQUAD ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SQUAD valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SQUAD decimals
            value_ = _amount.mul( 10 ** IERC20( SQUAD ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSQUAD ) { // 9
            sSQUADQueue = block.number.add( blocksNeededForQueue );
        } else return false;
