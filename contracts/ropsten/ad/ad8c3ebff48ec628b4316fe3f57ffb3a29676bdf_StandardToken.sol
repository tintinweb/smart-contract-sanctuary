...
contract CollateralDetails is Ownable {
    ...
    function getLastRecords(uint indexes_) public view returns(Collateral[] memory){
       Collateral[] memory items = new Collateral[](indexes_);
       for( uint i = 0; i < indexes_; i++){
           Collateral storage item = _collateral[i];
           items[i] = item;
       }
       return items;
    }
    
    function getCollateral(uint position_) public view returns(string memory document, string memory book, uint256 bags, uint256 tokenAmount ,string memory executionType){
        return (
            _collateral[position_].document, 
            _collateral[position_].book, 
            _collateral[position_].bags, 
            _collateral[position_].tokenAmount, 
            _collateral[position_].executionType
            );
    }
    
    function getLastRecordId() public view returns(uint256){
        return _collateralList.length-1;
        
    }
    
    function recordCollateral(string memory document_, string memory book_, uint256 bags_, uint256 tokenAmount_, string memory executionType_) internal onlyOwner{
        _collateralList.push(Collateral(document_,book_,bags_,tokenAmount_,executionType_));
        _collateral[_collateralList.length-1] = Collateral(document_,book_,bags_,tokenAmount_,executionType_);
        emit record(_collateralList.length-1, document_,book_,bags_,tokenAmount_,executionType_);
    }
...
contract StandardToken is IERC20, Ownable, Pausable, CollateralDetails{
    ...
    constructor (string memory name_, string memory symbol_, uint256 totalSupply_, uint8 decimals_, string memory document_, string memory book_, uint256 bags_)  {
        ...
        recordCollateral(document_, book_, bags_, totalSupply_, "INIT");
    ...
    */
    function burn(uint256 amount_, string memory document_, string memory book_, uint256 bags_) public onlyOwner whenNotPaused{
        ...
        recordCollateral(document_, book_, bags_, amount_, "BURN");
    ...
    */
    function mint(uint256 amount_, string memory document_, string memory book_, uint256 bags_)public onlyOwner whenNotPaused{
        ...
        recordCollateral(document_, book_, bags_, amount_, "MINT");
