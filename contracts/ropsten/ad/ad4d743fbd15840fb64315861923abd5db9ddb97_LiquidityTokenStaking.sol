...
/**
 * @title IUNI
 * @dev   Contract interface for Uniswap liquidity Pool Token contract 
 */
...
/**
 * @title LiquidityTokenStaking
 * @dev   LiquidityTokenStaking Contract for Liquidity Token staking
 */
contract LiquidityTokenStaking {
  ...
  */
  ...
  function setTokenContractAddress(address tokenContractAddress) external onlyOwner returns(bool){
    iuni = IUni(tokenContractAddress);
    return true;
  }
  ...
  */
  ...
  function addTokenReward(uint256 token) external onlyOwner returns(bool){
    ...
    iuni.transferFrom(msg.sender, address(this), _ownerTokenAllowance);
    return true;
  }
  ...
  function withdrawAddedTokenReward(uint256 token) external onlyOwner returns(bool){
    ...
    iuni.transferFrom(address(this), msg.sender, _ownerTokenAllowance);
    return true;
  }
  ...
  */
  ...
  /*
  * ---------------------------------------------------------------------------------------------------------------------------
  * Functionality for Add Liquidity in Token
  * ---------------------------------------------------------------------------------------------------------------------------
  */
  ...
  */
  ...
  function stakeToken(uint256 tokens, uint256 time) public tokenStakeCheck(tokens, time) returns(bool){
    ...
    iuni.transferFrom(msg.sender, address(this), tokens);
    return true;
  }
  ...
  function withdrawStakedTokens(uint256 stakingId) public returns(bool) {
    ...
    if(_tokenTotalDays[stakingId] == PERIOD_SILVER){
          ...
          if(now >= _tokenStakingEndTime[stakingId]){
              ...
              iuni.transferFrom(address(this), msg.sender,_finalTokenStakeWithdraw[stakingId]);
              ...
              iuni.transferFrom(address(this), msg.sender,_usersTokens[stakingId]);
              totalTokenStakesInContract = totalTokenStakesInContract.sub(_usersTokens[stakingId]);
            }
