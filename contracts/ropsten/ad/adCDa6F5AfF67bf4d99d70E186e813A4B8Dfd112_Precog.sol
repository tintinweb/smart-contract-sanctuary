...
// File: contracts/precog-v2/interfaces/IExchangeRouter.sol
...
// File: contracts/precog-v2/interfaces/IPrecog.sol
...
// File: contracts/future-exchange/interfaces/IFutureExchangeRouter.sol
...
// File: contracts/future-token/interfaces/IFutureToken.sol
...
// File: contracts/future-token/interfaces/IFutureTokenFactory.sol
...
// File: contracts/precog-v2/Precog.sol
...
contract Precog is IPrecog {
    ...
    function withdrawLiquidate(address futureToken, address to) external {
        uint amount = _liquidate(futureToken) - feeWithdrawByUSDC;
        IERC20(usdc).transfer(to, amount);
        emit Withdraw(msg.sender, amount, feeWithdrawByUSDC, block.timestamp);
    }

    function reinvest(address futureToken) external {
        uint amount = _liquidate(futureToken);
        availableAmount[msg.sender] += amount;
        _addTradingUser();
    }
    ...
    function _liquidate(address futureToken) internal returns(uint256 amount) {
        ...
        uint256 expiryDate = IFutureToken(futureToken).expiryDate();
    ...
    function maxProfitable(address user) public view returns (
        uint amount, uint profit, address exchange, address futureExchange, address futureToken
    ) {
            ...
            for (uint k = 0; k < futureExchanges.length; k++) {
                ...
                for (uint j = 0; j < futureTokens.length; j++) {
                    ...
                    for (uint i = 0; i < exchanges.length; i++) {
                        ...
                        (uint _profit,,) = _calculateProfit(user, amount, _exchange, _futureExchange, _futureToken);
                        if (_profit > profit) {
                            profit = _profit;
    ...
    function _calculateProfit(
        ...
        uint256 expiryDate = futureToken.expiryDate();
    ...
    function invest(
        ...
        uint256 feeTradingUsdc = _convertEthToUsdc(feeTradingByEth);
        require(availableAmount[user] >= amount + feeTradingUsdc, "PrecogV2: AVAILABLE_AMOUNT_NOT_ENOUGH");

        (uint256 profit, address[] memory pairs, uint256 tradeAmount) = _calculateProfit(user, amount, exchange, futureExchange, futureToken);
        ...
        tradingAmountOnFutureToken[user][address(futureToken)] += amount + feeTradingUsdc;
        profitAmount[user] += profit;
        profitAmountOnFutureToken[user][address(futureToken)] += profit;
        availableAmount[user] -= amount + feeTradingUsdc;

        _swapTradingFee();
        _removeTradingUser(user);

        emit Trade(user, address(futureToken), expiryDate, amount, profit, feeTradingUsdc, block.timestamp);
    }
}