...
interface blockCitiesInterface is IERC20 {
...
contract Assets is Ownable, BaseContract {
    ...
    function buyBlockcities(uint256 _amount) public {
        ...
        address getsAddress = manager.getAddress("BLCKT");
        blockCitiesInterface BLCKT = blockCitiesInterface(getsAddress);
        BLCKT.transferPrice(owner(), _msgSender(), _amount);
    }
    ...
    function invest(
        ...
        require(BLCKT.balanceOf(_msgSender()) >= _amount, "Not enough balance");
        BLCKT.transferPrice(_msgSender(), _assetowner, _amount);
    ...
    function reserve(
        ...
        blockCitiesInterface BLCKT = blockCitiesInterface(getsAddress);
        require(BLCKT.balanceOf(_msgSender()) >= _amount, "Not enough balance");
        BLCKT.transferPrice(_msgSender(), _receiver, _amount);
        emit ReserveEvent(
            _msgSender(),
            _assetDetails,
            _amount,
    ...
    function claimReturns(
        ...
        address getsAddress = manager.getAddress("BLCKT");
        blockCitiesInterface BLCKT = blockCitiesInterface(getsAddress);
        require(BLCKT.balanceOf(_assetowner) >= _amount, "Not enough balance");
        BLCKT.transferPrice(_assetowner, _investor, _amount);
   ...
   //  * @param unitNumber : alphaNumeric value of the asset
    // */
      ...
      function buyAsset(
        ...
        // string memory unitNumber
    ) external{
        ContractManagerInterface manager = ContractManagerInterface(
            managerAddress
        );
        address getsAddress = manager.getAddress("BLKCT");
        blockCitiesInterface BLKCT = blockCitiesInterface(getsAddress);
         require(
            BLKCT.balanceOf(msg.sender) >= 0,
             "ERROR : Not enough token to buy asset"
        );
        // uint256 amount = initialAssetTokenAmount;
        uint256 fee = (initialAssetTokenAmount*feePercentage)/100;
        uint256 amountAfterAddingFee = initialAssetTokenAmount + fee;
        
        uint256 Discount = (amountAfterAddingFee*discountPercentage)/100;
        uint256 amountAfterAddingDiscount = amountAfterAddingFee - Discount;
        
        uint256 Tax = (amountAfterAddingDiscount*taxPercentage)/100;
        uint256 amountAfterAddingTax = amountAfterAddingDiscount + Tax;
        
        uint256 amount = amountAfterAddingTax - (fee + Tax);
        BLKCT.transferPrice(msg.sender, assetOwnerAddress, amount);
        BLKCT.transferPrice(msg.sender, owner(), fee + Tax);
       
        
        emit BuyEvent(initialAssetTokenAmount, feePercentage, discountPercentage, taxPercentage, assetOwnerAddress);
