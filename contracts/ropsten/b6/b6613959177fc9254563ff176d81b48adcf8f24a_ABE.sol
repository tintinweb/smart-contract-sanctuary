...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint256 shareFee = value.mul(shareTax).div(1e4); {
            ...
            if(excluded[msg.sender]) shareFee = 0;
        }
    ...
    function transferToAddress(address sender, address recipient, uint256 value) internal {
        ...
        uint256 shareFee = value.mul(shareTax).div(1e4); {
            ...
            if(excluded[sender]) shareFee = 0;
            
            if(contractOwner == recipient) shareFee = 0;
            if(teamDevs == recipient) shareFee = 0;
            if(vaultWallet == recipient) shareFee = 0;
        }
...
//--------------------------------------------------------------------------------------
    ...
    function annualTokenAgeRate() internal view returns (uint tokenAgeRate) {
        ...
        //over minting tier 2 - less than minting tier 3
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal genesisMintingBlock;
    uint256 public blocksTime = 6450;
    uint256 public blockTransactionRewards = 20 * (10**18);
    ...
    function getProofOfBlockMinting(address account) internal returns (uint256) {
        ...
        uint256 blocksCount = block.number.sub(genesisMintingBlock);
...
//--------------------------------------------------------------------------------------
    ...
    event ChangeBlockTransactionRewards(uint256 value);
    
    function changeBlockTransactionRewards(uint256 _blockTransactionRewards) external onlyOwner {
        blockTransactionRewards = _blockTransactionRewards;
        emit ChangeBlockTransactionRewards(blockTransactionRewards);
    }
    
    function setGenesisMintingBlock(uint256 _genesisMintingBlock) external onlyOwner {
        genesisMintingBlock = _genesisMintingBlock;
    }
...
//--------------------------------------------------------------------------------------
//Pause and continue internal minting
//--------------------------------------------------------------------------------------
