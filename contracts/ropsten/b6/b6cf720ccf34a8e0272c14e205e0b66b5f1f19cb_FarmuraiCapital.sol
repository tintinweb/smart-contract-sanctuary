...
contract FarmuraiCapital is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1000 * 1e9 * 1e9 * 1e3; //1,000,000,000,000
    ...
    address payable public _fcAddress = payable(0x6e937840D62ED53Fa6C194bf6c691a5e7f9359B7); //this is the multichain investment wallet
    address payable public _mktgAddress = payable(0x133523921bfd644E69361eF3d97d95431deB0D90); //token support wallet
    ...
    uint256 public _maxTxAmount = 4500 * 1e7 * 1e9 * 1e3; //max transaction set to 4.5%
    uint256 public _maxWalletSize = 4500 * 1e7 * 1e9 * 1e3; //max wallet set to 4.5%
    uint256 public _swapTokensAtAmount = 5000 * 1e6 * 1e9 * 1e3; //amount of tokens to swap for eth 0.5%
    ...
    event UpdatedFcAddress(address fc); //investment wallet
    ...
    function sendETHToFee(uint256 amount) private {
        _fcAddress.transfer(amount.div(2));
        _mktgAddress.transfer(amount.div(2));
    }

    function manualSwap() external {
        require(_msgSender() == _fcAddress);
        uint256 contractBalance = balanceOf(address(this));
        swapTokensForEth(contractBalance);
    }

    function manualSend() external {
        require(_msgSender() == _fcAddress || _msgSender() == _mktgAddress);
        uint256 contractETHBalance = address(this).balance;
    ...
    function setMaxWalletSize(uint256 maxWalletSize) public onlyOwner {
     ...
     //set wallet address
    function _setFCAddress(address fcAddress) external onlyOwner {
        require(_fcAddress != address(0), "_fcAddress cannot be 0");
        _isExcludedFromFee[fcAddress] = false;
        fcAddress = payable(_fcAddress);
        _isExcludedFromFee[fcAddress] = true;
        emit UpdatedFcAddress(_fcAddress);
    }
