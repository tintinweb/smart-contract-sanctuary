...
contract FUN is ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable, Pausable { 
    ...
    function mintWithTokenURI(address to, string memory _tokenURI) external virtual onlyOwner returns (uint256 tokenId) {
        tokenId = _mint(to);
        _tokenURIs[tokenId] = _tokenURI;
    }
    

    function safeMintWithTokenURI(address to, string memory _tokenURI, bytes memory _data) external virtual onlyOwner returns (uint256 tokenId) {
    ...
    function multiSafeMintWithTokenURI(address[] memory to, string[] memory _tokenURI, bytes[] memory _data) external virtual onlyOwner returns (uint256 lastTokenId) {
        require(to.length == _tokenURI.length, "FUN: arrays have different lengths");
        require(to.length == _data.length, "FUN: arrays have different lengths");
        uint tokenId;
        for (uint i; i < to.length; i++) {
    ...
    function multiMintWithTokenURI(address[] memory to, string[] memory _tokenURI) external virtual onlyOwner returns (uint256 lastTokenId) {
        require(to.length == _tokenURI.length, "FUN: arrays have different lengths");
        uint tokenId;
        for (uint i; i < to.length; i++) {
            tokenId = _mint(to[i]);
