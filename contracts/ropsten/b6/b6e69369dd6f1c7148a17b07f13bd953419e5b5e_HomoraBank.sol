...
interface ICErc20 {
  ...
  function mint(address _recipient, uint256 mintAmount) external;
...
contract HomoraBank is Governable, ERC1155NaiveReceiver, IBank {
  ...
  function initialize(IOracle _oracle, uint _feeBps, address _weth) external onlyGov {
    oracle = _oracle;
  ...
  function accrue(address token) public override {
    ...
    uint debt = uint(ICErc20(bank.cToken).borrowBalanceCurrent(address(this)));
    if (debt > totalDebt) {
  ...
  function addBank(ISafeBox safeBox) external onlyGov {

    address token = safeBox.uToken();
    address cToken = safeBox.cToken();
    // approve safeBox withdraw cToken from bank
    ICErc20(cToken).approve(address(safeBox), uint256(-1));
  ...
  function borrow(address token, uint amount) external override inExec poke(token) {
    ...
    uint borrowAmt = doBorrow(token, amount);
    if (token == weth) {
      // wrap eth -> weth
      IWETH(weth).deposit{value: borrowAmt}();
    }
    IERC20(token).safeTransfer(msg.sender, borrowAmt);
  ...
  function doBorrow(address token, uint amountCall) internal returns (uint) {
    ...
    ICErc20(bank.cToken).borrow(amountCall);
  ...
  function doRepay(address token, uint amountCall) internal returns (uint) {
    ...
    if (token == weth) {
    // weth -> eth
      IWETH(weth).withdraw(amountCall);
      ICEther cToken = ICEther(bank.cToken);
      cToken.repayBorrow{value: amountCall}();
      uint newDebt = uint(cToken.borrowBalanceStored(address(this)));
      bank.totalDebt = newDebt;
    } else {
      ICErc20 cToken = ICErc20(bank.cToken);
      cToken.repayBorrow(uint256(amountCall));
      uint newDebt = uint(cToken.borrowBalanceStored(address(this)));
      bank.totalDebt = newDebt;
    }
    return oldDebt.sub(bank.totalDebt);
  }
