...
contract CryptoBees is ERC721, Ownable, Pausable {
  ...
  function getTokenType(uint256 tokenId) external view returns (string memory) {
    return getTokenTextType(tokenId);
  }

  function getTokenNumType(uint256 tokenId) private view returns (uint256) {
    require(_exists(tokenId), 'ERC721Metadata: Nonexistent token');
  ...
  function getTokenTextType(uint256 tokenId) private view returns (string memory) {
    require(_exists(tokenId), 'ERC721Metadata: Nonexistent token');
    uint256 numType = getTokenNumType(tokenId);
    string memory textType;
    if (numType == 0) textType = 'BEEKEEPER';
    else if (numType == 1) textType = 'BEARR';
    else textType = 'BEE';
    return textType;
  }
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    uint256 numType = getTokenNumType(tokenId);
    string memory textType = getTokenTextType(tokenId);
    ...
    string memory metadata = string(
      abi.encodePacked(
        ...
        '","image": "QmVPMv3Kxg94vAJo4fQY2FGnYTYp4RM1dq7anwr9psbz9P"}'
      )
    );
