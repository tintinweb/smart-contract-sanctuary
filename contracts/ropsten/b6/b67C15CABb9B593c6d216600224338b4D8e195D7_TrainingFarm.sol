...
contract TrainingFarm is Ownable {
    ...
    function claimPool(uint256 idCharacter) public returns(uint256){
        ...
        for (uint256 index = 0; index < usesTechnique.length; index++) {
            if(usesTechnique[index]>0){
                ...
                if(amountUses <= infoTechniques[idTech].countUses){
    ...
    function pendingReward(uint256 idCharacter) view public returns (uint256 reward,uint256 kiRest,uint256[] memory auxRepeat) {
        ...
        while((kiNow>0) && (totalRepeat>0)){
            for (uint256 index = 0; index < profitArray.length; index++) {
                if((repeatArray[index] > 0)&&(kiNow >= burnKiArray[index] )){
                    reward= reward.add(baseReward.mul(profitArray[index]));
                    kiNow= kiNow.sub(burnKiArray[index]);
                    repeatArray[index]--;
                    auxRepeat[index]++;
                    totalRepeat--;
        ...
        reward=reward.div(knowledgeContract.getDiv());
        if(currentLevel < LEVEL_WITH_MORE_CHARACTERS){
            reward= reward.div(2);
        }
    return (reward,kiResta.sub(kiNow),auxRepeat);
    }
    //reward= 
    function profitByTechnique(uint256 idTechnique) view public returns (uint256 profit,uint256 repeat,uint256 burnKi) {
    ...
    function upKi(uint256 idCharacter,uint256 amount,bool levelUp) public  returns(uint256 newAmountKi){
        require(ownerCharacter[idCharacter] == _msgSender());
        claimPool(idCharacter);
        kiTokenContract.burnFrom(_msgSender(),amount);
...
contract KICharacter is ERC721Enumerable , ERC721Burnable, Ownable {
    ...
    function addCharacterRule(uint256 idRarity,string memory name, uint256 baseReward,
        uint256 innerPower,uint256 valueBaseKiBurnNewLevel,uint256 kiBurnByBlock) public onlyOwner {
    ...
    function upKi(uint256 idCharacter,uint256 newAmountKi,uint256 dificultEx,bool levelUp) public onlyOwner returns(uint256 ki) {
       if(levelUp){
    ...
    function mintCopy(address _to, uint256 _characterId,uint256 _amount) public onlyOwner {
        ...
        mint(_character.idCharacterRule,_character.currentLevel,_character.ki,_character.lastUpKi,_to,_amount);
    }

    function mint( uint256 idCharacterRule,uint256 currentLevel,uint256 ki,uint256 lastUpKi, address _to,uint256 _amount) public onlyOwner {
        Character memory _character= Character(idCharacterRule,currentLevel,ki,lastUpKi);
