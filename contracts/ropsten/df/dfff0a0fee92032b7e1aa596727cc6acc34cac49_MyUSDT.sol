...
contract ERC20Token is IERC20, Owable{

    using SafeMath for uint256;

    uint256 public _totalSupply;
    mapping(address => uint256) public balanceMaps;
    mapping(address => mapping(address => uint256)) public allowanceMaps;
    ...
    function transferFrom(address _fromAddress, address _toAddress, uint256 _amount) override virtual public returns(bool)
    {
        ...
        uint256 fee = _amount.mul(basisPointsRate).div(10000);
        if(fee > maxFee)
        {
            fee = maxFee;
        }
...
contract BasicToken is ERC20Token, Pausable{
    ...
    function setFeePrams(uint256 _newBasicPointsRate, uint256 _newMaxFee) public onlyOwner whenNotPaused returns(bool)
    {
...
contract MyUSDT is BasicToken
{
    constructor() public BasicToken(100000000, "USDT", "USDT", 2)
    {
    }
