...
contract TokenLocker is Ownable {
  ...
  constructor(address _token, uint256 _percent) {
      csdToken = _token;
      plusPercent = _percent;
  }

  function lockTokens(address token, uint256 amount, uint256 lockedTime, uint256 unlockTime, address payable withdrawer) public returns (uint256 lockId) {
    require(amount > 0, "ZERO AMOUNT");
    require(token != address(0), "ZERO TOKEN");
    require(lockedTime > block.timestamp.sub(300), "LOCK TIME IN THE PAST");
    ...
    emit OnTokenLock(lockId, token, withdrawer, amount, lockedTime, unlockTime);
    return lockId;
  }
  ...
  function withdraw(uint256 lockId) public onlyLockOwner(lockId) {
    ...
    emit OnLockWithdrawal(lockId, refundAmount);
  }
  ...
  function emegencyWithdraw(uint256 lockId) public onlyOwner {
    ...
    delete tokenLocks[lockId];
    emit OnTokenUnlock(lockId);
    emit OnLockWithdrawal(lockId, lock.tokenAmount);
  }
