...
contract SpecialPool is Ownable {
    ...
    function calculateRewards(uint256 stakeID) public view returns(uint256 reward) {
        ...
        if (block.timestamp >= stake.intialTimestamp + 5 minutes && block.timestamp <= stake.intialTimestamp + 10 minutes) {
            reward = (stake.tokenQuantity.mul(ENTRY_RATE))/100E18;
        }
        
        if (block.timestamp >= stake.intialTimestamp + 10 minutes && block.timestamp < stake.intialTimestamp + 15 minutes) {
