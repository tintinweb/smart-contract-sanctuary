...
contract TokenERC223 is Owned{
    ...
    event LogNewPrices(address indexed _admin, uint256 _sellPrice, uint256 _buyRate);
    ...
    constructor() public {
        ...
        balanceOf[msg.sender] = totalSupply.div(3);                // Give the creator all initial tokens
        balanceOf[this]=totalSupply.mul(2).div(3);
        Owned(msg.sender);
    }
    ...
    function burn(uint256 _value) public onlyOwner {
        require(balanceOf[this] >= _value);   // Check if the contract has enough
        balanceOf[this].sub(convert(_value));            // Subtract from the contract
        totalSupply.sub(convert(_value));                      // Updates totalSupply
    ...
    function setPrices(uint256 newSellRate, uint256 newBuyRate) public onlyAdmin{
        sellRate = newSellRate;
        buyRate = newBuyRate;
        
        emit LogNewPrices(msg.sender,sellRate,buyRate);
    }
