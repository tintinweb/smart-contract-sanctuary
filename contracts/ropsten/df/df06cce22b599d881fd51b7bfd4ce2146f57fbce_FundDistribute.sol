...
contract FundDistribute {
    ...
    function get_Addresses () public view returns (address[] memory, uint[] memory, address){
        require(tokenAddress != address(0),"Token should set at first place.");
        return (addList,valList,tokenAddress);
    }

    function getAddresses () public only_owner returns (address[] memory, uint[] memory, address){
        require(tokenAddress != address(0),"Token should set at first place.");
        Distributed = false;
        return (addList,valList,tokenAddress);
    }
    ...
    event AddressesArranged (address[] _addresses, uint[] _values);
    event AddressesChanged (address[] _addresses, uint[] _values);
    event CleanUp_Done (bool cleanupResult);
    event FundDistributed (bool[] _distributionResults);
