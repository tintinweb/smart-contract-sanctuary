...
contract TestnetToken is ERC721URIStorage, Ownable{

    event WakeUpDevils (address indexed summoner, uint256 startWith, uint256 times);
    ...
    function wakeUpDevil(uint256 _times) payable public {
        require(started, "not started");
        require(_times >0 && _times <= maxBatch, "wake wrong number");
        require(totalDevils + _times <= totalCount, "wake too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit WakeUpDevils(_msgSender(), totalDevils+1, _times);
