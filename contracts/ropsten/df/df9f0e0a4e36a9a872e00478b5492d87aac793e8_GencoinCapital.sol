...
contract GencoinCapital is Context, IERC20, Ownable {
    ...
    uint256 public _buyTreasuryFee = 40; //4%
    uint256 public _buyMarketingFee = 40; //4%
    uint256 public _buyTeamFee = 20; //2%

    uint256 public SCTokenBalance = 0;

    uint256 public _sellLiquidityFee = 20; //2%
    uint256 public _sellReflectionFee = 10; //1%
    uint256 public _sellTreasuryFee = 40; //4%
    uint256 public _sellMarketingFee = 40; //4%
    uint256 public _sellTeamFee = 20; //2%
    ...
    function _transfer(
        ...
        if(tradingOpen && to == uniswapV2Pair) { //sell
            ...
            if(block.timestamp <= launchedAtTime + 2 hours) {
                currenttotalFee = 300; //30%
            }else if (block.timestamp <= launchedAtTime + 12 hours) {
                currenttotalFee = 250; //25%
            }else if (block.timestamp <= launchedAtTime + 24 hours) {
                currenttotalFee = 200; //20%
            }else{
                currenttotalFee = _sellLiquidityFee.add(_sellTreasuryFee).add(_sellMarketingFee).add(_sellTeamFee);
            }
        }
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(reflectionFee).div(10**3);
    }
    
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(currenttotalFee).div(10**3);
    }
    ...
    function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _marketingFee, uint256 _teamFee) external onlyOwner {
        ...
        require(total_buy_fee <= 1300); //Max buy fee 13%
    ...
    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _treasuryFee, uint256 _marketingFee, uint256 _teamFee) external onlyOwner {
        ...
        require(total_sell_fee <= 1300); //Max sell fee 13%
