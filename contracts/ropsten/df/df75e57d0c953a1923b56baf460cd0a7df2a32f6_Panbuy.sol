...
contract Panbuy is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeCast for int256;
  
    string private _name = "Panbuy";
    string private _symbol = "PanB";
    ...
    uint256 public _taxFee = 0; // means 3% which distribute to all holders
    uint256 public _buyBackFee=500;//meanse 5 goes to buyback wallet
    ...
    mapping(address => uint256) public cooldownCycleStart;
    ...
    uint256 public initialSellLimit = 4000; // 40%
    ...
    constructor() public {
        ...
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506); // Sushiswap Router
        ...
        buyBackAddress = 0x69F25875A05F88195B1bDb5E8b05582f354f8f31; // Dummy wallet
    ...
    function _transfer(
        ...
        if(cooldownEnabled && sender != uniswapV2Pair && msg.value == 0 && !isTaxless[sender]) {
            if(cooldownCycleStart[sender] == 0){
                cooldownCycleStart[sender] = block.timestamp;
            }
    ...
    function calculateCooldownAmountLimit(address sender) public view returns (uint256) {
        ...
        if(cooldownCycleStart[sender] + (7 * cooldownTime) < block.timestamp) return currentBalance;
        uint256 limitMultiplier = (block.timestamp - cooldownCycleStart[sender]) / cooldownTime; // this calculates how many days since listed
        uint256 amountLimit = initialSellLimit.add(limitMultiplier * 1000); // increase the sell limit each day
