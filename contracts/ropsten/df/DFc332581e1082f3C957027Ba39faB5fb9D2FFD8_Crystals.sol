    ...
    Pausable
{
    ...
    uint32 private constant registrationThreshold = 5;
        ...
        nonReentrant
    {
        ...
        if (registeredCrystals < registrationThreshold) {
            ...
            if (tokenId > 8000) {
                IMANA(manaAddress).burn(_msgSender(), crystalsMap[tokenId].level * 10);
            } else {
                IMANA(manaAddress).burn(_msgSender(), crystalsMap[tokenId].level * 100);
            }   
        }
