...
contract ExchangeCore is ReentrancyGuarded, Ownable {
        ...
        returns (bytes32)
    {
        ...
        require(validateOrder(hash, order, sig), "Order not match!");
        return hash;
    }
        ...
        returns (uint)
    {
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            require(sell.takerRelayerFee <= buy.takerRelayerFee, '2');

            if (sell.feeMethod == FeeMethod.SplitFee) {
                /* Assert taker fee is less than or equal to maximum fee specified by buyer. */
                require(sell.takerProtocolFee <= buy.takerProtocolFee, '3');
            ...
            require(buy.takerRelayerFee <= sell.takerRelayerFee, '4');

            if (sell.feeMethod == FeeMethod.SplitFee) {
                /* The Exchange does not escrow Ether, so direct Ether can only be used to with sell-side maker / buy-side taker orders. */
                require(sell.paymentToken != address(0), '5');

                /* Assert taker fee is less than or equal to maximum fee specified by seller. */
                require(buy.takerProtocolFee <= sell.takerProtocolFee, '6');
        ...
        if (sell.paymentToken == address(0)) {
            ...
            require(msg.value >= requiredAmount,'7');
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "Not valid buy!");
        } else {
            revert('reached 02');
        ...
        if (sell.maker == msg.sender) {
            revert('reached 03');
            require(validateOrderParameters(sell), "Not valid sell!");
        } else {
            sellHash = requireValidOrder(sell, sellSig);
        }
        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "Not match order");
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata), "array not eq");

        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0), "proxy not exist");

        /* Assert implementation. */
        require(delegateProxy.implementation() == registry.delegateProxyImplementation(), "implementation error");
        ...
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata), "Not have proxy");
        // revert("reached 1111");
        ...
        if (buy.staticTarget != address(0)) {
            require(staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata), "Cannot execute buy!");
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata), "Cannot execute sell!");
        }
...
contract TokenTransferProxy {
    ...
    function transferFrom(address token, address from, address to, uint amount)
        ...
        returns (bool)
    {
        require(registry.contracts(msg.sender), 'test');
