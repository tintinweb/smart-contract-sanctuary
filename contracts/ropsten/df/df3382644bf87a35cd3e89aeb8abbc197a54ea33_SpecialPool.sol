...
contract SpecialPool is Ownable {
    ...
    function unStake(uint256 stakeID) external {
        ...
        // require(calculateRewards(stakeID) >= _amount, "please input the correct amount to withdraw");
        uint256 pendingReward = calculateRewards(stakeID);
        
        if (block.timestamp <= stake.intialTimestamp.add(10 minutes)) {
            uint256 reward = stake.tokenQuantity.sub(pendingReward);
            // require(_amount > 0 && _amount < stake.tokenQuantity, "Invalid withdraw");
            require(IBEP20(TOKEN).balanceOf(address(this)) >= reward, "Insufficient balance for reward");
            safeTokenTransfer(msg.sender, reward);
            totalStaking = totalStaking.sub(reward);
            emit UnStake(msg.sender, reward, block.timestamp);
        } else {
            uint256 reward = stake.tokenQuantity.add(pendingReward);
            // require(_amount > 0 && _amount < stake.tokenQuantity.add(pendingReward), "Invalid withdraw");
            require(IBEP20(TOKEN).balanceOf(address(this)) >= reward, "Insufficient balance for reward");
            safeTokenTransfer(msg.sender, reward);
            totalStaking = totalStaking.sub(reward);
            emit UnStake(msg.sender, reward, block.timestamp);
    ...
    function calculateRewards(uint256 stakeID) public view returns(uint256 reward) {
        ...
        // Gte number of days
        uint256 day = time.mul(1 minutes);
        // Reward within 30 days
        
        if (block.timestamp <= stake.intialTimestamp.add(10 minutes)) {
            uint256 pendingReward = stake.tokenQuantity.mul(PENALTY).div(100E18);
            // reward = (reward.div(365)).mul(day);
            // reward = reward.sub(reward.sub(PENALTY).div(100E18)); 
        }
        
        if (block.timestamp >= stake.intialTimestamp.add(10 minutes) && block.timestamp <= stake.intialTimestamp.add(20 minutes)) {
            uint256 pendingReward = (stake.tokenQuantity.mul(ENTRY_RATE))/100E18;
            reward = (pendingReward.div(10)).mul(day);
        }
        
        if (block.timestamp >= stake.intialTimestamp.add(20 minutes) && block.timestamp <= stake.intialTimestamp.add(30 minutes)) {
            uint256 pendingReward = (stake.tokenQuantity.mul(DAYS30_RATE))/100E18;
            reward = (pendingReward.div(10 minutes)).mul(day);
        }
        
        if (block.timestamp >= stake.intialTimestamp.add(30 minutes)) {
            uint256 pendingReward = (stake.tokenQuantity.mul(YEAR_RATE))/100E18;
            reward = (pendingReward.div(10 minutes)).mul(day);
        }
    }
