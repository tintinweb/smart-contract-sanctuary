...
contract StakingV4 is NamedContract, StakingStorageV3, StakingEventV3 {
    ...
    */
    ...
    /**
     * @notice Batch approves the provided addresses to claim the provided amounts.
     *
     * @param toAddresses The addresses can claim reward
     * @param amounts The amounts to claim
     */
    function batchApproveClaim(address[] calldata toAddresses, uint256[] calldata amounts) external {
        ...
        for (uint i = 0; i < toAddresses.length; i++) {
            require(
                _rewardPoolAmount >= amounts[i],
                "Insufficient reward pool"
            );
    
            _claimNonce = _claimNonce.add(1);
            _approvedClaimMap[toAddresses[i]][_claimNonce] = amounts[i];
