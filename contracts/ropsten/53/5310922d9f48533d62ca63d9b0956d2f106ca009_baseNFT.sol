 ...
 */
contract baseNFT is Initializable, ContextUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, ERC721PausableUpgradeable {

    IERC20 public FUELTOKEN;
    // IMETA public METADATA; 
    ...
    function initialize() public virtual initializer {
        ...
        // METADATA = new 
        // __initialize_metadata(0xCf3728Ff7857507243feB557B6ffB30d788d98f6);
    }
    ...
    function changeFuel(IERC20 _fuelAddress) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "changeFuel: must have admin role to change fuel");
        FUELTOKEN = IERC20(_fuelAddress);
    }

    function primaryMint(address destinationAddress, address ticketIssuerAddress, address eventAddress, string memory ticketMetadata) public returns (uint256) {
        
        require(hasRole(MINTER_ROLE, _msgSender()), "primaryMint: must have minter role to mint");

        // _tokenIdTracker.increment();
        uint256 nftIndex = _tokenIdTracker.current();  // CHECK MEMORY
        _mint(destinationAddress, nftIndex);
        _tokenIdTracker.increment();

        _setTokenURI(nftIndex, ticketMetadata);
        _ticketIssuerAddresses[nftIndex] = ticketIssuerAddress;
        _eventAddresses[nftIndex] = eventAddress;
        _nftScanned[nftIndex] = false;
        _nftInvalidated[nftIndex] = false;
    ...
    function secondaryTransfer(address originAddress, address destinationAddress) public {
        ...
        emit txSecondary(originAddress, destinationAddress, _ticketIssuerAddresses[nftIndex], nftIndex, block.timestamp);
    ...
    function scanNFT(address originAddress) public {
        ...
        _nftScanned[nftIndex] = true;

        // moveFuelToken(transferGETfee, FUELTOKEN, GETcollector);
        
        emit txScan(originAddress, _ticketIssuerAddresses[nftIndex], nftIndex, block.timestamp);
    }


    // function mint(address to) private virtual {
