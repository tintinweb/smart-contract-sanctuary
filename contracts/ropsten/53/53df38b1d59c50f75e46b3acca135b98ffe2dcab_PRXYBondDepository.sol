...
contract PRXYBondDepository is Ownable {
    ...
    IERC20 public immutable PRXY; // token given as payment for bond
    ...
    function deposit( 
        ...
        uint balanceBefore = PRXY.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            PRXY.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == PRXY.balanceOf(address(this)), "Not enough PRXY to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            PRXY.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                PRXY.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                PRXY.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return PRXY.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
