...
/******************************************/
/*       PressChild starts here           */
/******************************************/

pragma solidity ^0.6.12;

contract PressChild is Ownable {
    ...
    struct PoolInfo 
    {
        ...
        uint256 lpSupply;               // Supply of LP tokens associated with this press child.
    ...
    modifier onlyPressMaster() {
        require(msg.sender == pressMaster, "Caller is not the PressMaster");
        _;
    }

    constructor(IERC20 _REWARD, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _endBlock, address _pressMaster) public {
        REWARD = _REWARD;
        rewardPerBlock = _rewardPerBlock;
        startBlock = _startBlock;
        endBlock = _endBlock;
        pressMaster = _pressMaster;
    }
     ...
     * @param _withUpdate Whether to update all LP token contracts. Should be true if REWARD distribution has already begun.
     */
    ...
    /**
     * @dev Update the given pool's REWARD allocation point. Can only be called by the owner.
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(reward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accRewardPerShare = pool.accRewardPerShare.add(reward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }

    /**
     * @dev Deposit LP tokens to Faucet for REWARD allocation.
     * @param _pid ID of a specific LP token pool. See index of PoolInfo[].
     * @param _amount Amount of LP tokens to deposit.
     */
    function deposit(uint256 _pid, uint256 _amount, address _user) public onlyPressMaster {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        updatePool(_pid);
        if (user.amount > 0) {
     ...
     */
    function withdraw(uint256 _pid, uint256 _amount, address _user) public onlyPressMaster {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
    ...
    /**
     * @dev Safe REWARD transfer function, just in case if rounding error causes faucet to not have enough MARK.
    ...
    /**
     * @dev Views total number of REWARD tokens deposited for rewards.
     * @return REWARD token balance of the faucet.
     */
    ...
    /**
     * @dev Transfer REWARD tokens.
     * @return Success.
     */
    function transfer(address to, uint256 value) external onlyOwner returns (bool) {
        return REWARD.transfer(to, value);
    }

    /**
     * @dev Define last block on which REWARD reward distribution occurs.
