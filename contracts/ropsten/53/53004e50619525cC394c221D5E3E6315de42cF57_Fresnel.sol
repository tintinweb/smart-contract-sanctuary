...
// - 1% is reflected proportionally to all holders.
...
contract Fresnel is ERC20, Ownable {
    ...
    uint256 private _ratedTotalSupply;
    uint256 private _liquifyThreshold;

    mapping (address => bool)    private _excludedFromFee;
    mapping (address => bool)    private _excludedFromReflection;
    mapping (address => uint256) private _excludedFromReflectionBalances;
    address[]                    private _excludedFromReflectionAddresses;
    ...
    constructor () ERC20("Fresnel", "FRS", 8) {
        ...
        excludeFromReflection(address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D)); // ETH
        // excludeFromReflection(address(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F)); // BSC
    ...
    function balanceOf            (address account) public view override returns (uint256) {
        ...
        if (_excludedFromReflection[account]) 
            return _excludedFromReflectionBalances[account];
    ...
    function excludeFromReflection(address account) public onlyOwner {
        require(!_excludedFromReflection[account], "Account is already excluded.");
        
        _excludedFromReflection        [account] = true;
        _excludedFromReflectionBalances[account] = balanceOf(account);
        _excludedFromReflectionAddresses.push(account);
    }
    function includeInReflection  (address account) public onlyOwner {
        require( _excludedFromReflection[account], "Account is already included.");

        for (uint256 i = 0; i < _excludedFromReflectionAddresses.length; i++) {
            if (_excludedFromReflectionAddresses[i] == account) {
                _excludedFromReflection        [account] = false;
                _excludedFromReflectionBalances[account] = 0;

                _excludedFromReflectionAddresses[i] = _excludedFromReflectionAddresses[_excludedFromReflectionAddresses.length - 1];
                _excludedFromReflectionAddresses.pop();
                break;
            }
        }
    }
    ...
    function _transfer            (address from, address to, uint256 amount) internal override {
        ...
        if (_excludedFromReflection[from         ]) _excludedFromReflectionBalances[from         ] = _excludedFromReflectionBalances[from         ].sub(amount);
        if (_excludedFromReflection[to           ]) _excludedFromReflectionBalances[to           ] = _excludedFromReflectionBalances[to           ].add(tokensToTransfer);
        if (_excludedFromReflection[address(0)   ]) _excludedFromReflectionBalances[address(0)   ] = _excludedFromReflectionBalances[address(0)   ].add(tokensToBurn); 
        if (_excludedFromReflection[address(this)]) _excludedFromReflectionBalances[address(this)] = _excludedFromReflectionBalances[address(this)].add(tokensToLiquify); 
    ...
    function _swapAndLiquify      (uint256 amount) private lockTheSwap {
        ...
        uniswapV2Router.addLiquidityETH{value: newBalance}(address(this), otherHalf, 0, 0, address(this), block.timestamp);
    }
    function _computeFees         (address from, address to, uint256 amount) private view returns (uint256 tokensToBurn, uint256 tokensToLiquify, uint256 tokensToReflect, uint256 tokensToTransfer, uint256 ratedAmount, uint256 ratedTokensToBurn, uint256 ratedTokensToLiquify, uint256 ratedTokensToReflect, uint256 ratedTokensToTransfer) {
    ...
    function _getRate             () private view returns(uint256) {
        ...
        uint256 ratedSupply = _ratedTotalSupply;

        for (uint256 i = 0; i < _excludedFromReflectionAddresses.length; i++) {
            if (_excludedFromReflectionBalances[_excludedFromReflectionAddresses[i]] > supply ||
                _balances                      [_excludedFromReflectionAddresses[i]] > ratedSupply)
                return _ratedTotalSupply.div(_totalSupply);
            
            supply      = supply     .sub(_excludedFromReflectionBalances[_excludedFromReflectionAddresses[i]]);
            ratedSupply = ratedSupply.sub(_balances                      [_excludedFromReflectionAddresses[i]]);
        }
        
        if (ratedSupply < _ratedTotalSupply.div(_totalSupply)) 
            return _ratedTotalSupply.div(_totalSupply);

        return ratedSupply.div(supply);
    }
