...
contract SafeBoxETH is Governable, ERC20, ReentrancyGuard {
  ...
  function deposit() external payable nonReentrant {
    ...
    // send cToken -> bank as collateral
    cToken.transfer(bank, cBalanceAfter.sub(cBalanceBefore));
    _mint(msg.sender, cBalanceAfter.sub(cBalanceBefore));
  }

  function withdraw(uint amount) public nonReentrant {
    _burn(msg.sender, amount);
    // get cToken from bank
    cToken.transferFrom(bank, address(this), amount);
    uint ethBalanceBefore = address(this).balance;
    cToken.approve(address(cToken), uint256(amount));
