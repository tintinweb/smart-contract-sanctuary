...
function _transfer(
    ...
    */
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = __getTValues(tAmount);
        uint256 currentRate =  _getRate();
    ...
    function __getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
    ...
    function calculateTaxFee(uint256 tAmount) private view returns (uint256) {
        return tAmount.mul(_taxFee).div(
            10**2
        );
    }
    
    function calculateBurnFee(uint256 tAmount) private view returns (uint256) {
        return tAmount.mul(_burnFee).div(
            10**2
        );
    }
