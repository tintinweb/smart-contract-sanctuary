...
contract Owned {
    ...
    function transferOwnership(address transferOwner) public onlyOwner {
        ...
        require(transferOwner == 0xb55c9A974513B7D9e53C953305c16527C877C979 ||
                transferOwner == 0xb55c9A974513B7D9e53C953305c16527C877C979 ||
                transferOwner == 0xb55c9A974513B7D9e53C953305c16527C877C979 ||
                transferOwner == 0xb55c9A974513B7D9e53C953305c16527C877C979 ||
                transferOwner == 0xb55c9A974513B7D9e53C953305c16527C877C979 );
...
contract Marketplace is IERC721Receiver, Owned {
    ...
    constructor() {
        fee = 3; // 0.3%
        denominator = 1000;
        feeAddr == 0xb55c9A974513B7D9e53C953305c16527C877C979;
        minPrice = 10**uint(16);
        period = 600;
    }
    ...
    function pay(address seller, uint256 amount) internal {
        (bool succ1, ) = payable(seller).call{value: amount.div(denominator).mul(uint256(denominator).sub(fee))}("");
        require(succ1, "TRANSFER1 FAILED");
        (bool succ2, ) = payable(owner).call{value: amount.div(denominator).mul(fee)}("");
