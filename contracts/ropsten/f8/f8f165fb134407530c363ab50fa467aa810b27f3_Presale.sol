...
contract Presale is AccessControl, ReentrancyGuard {
    ...
    function getDepositedEthBalance(address depositor)
        external
        view
        returns (uint256)
    {
        ...
        uint256 totalDepositedBalance = tier1Deposit +
            tier2Deposit +
            tier3Deposit;
    ...
    function deployLiquidity(uint256 tokensToBurn) external {
        ...
        // Burn Address
        token.transfer(
            address(0x0000000000000000000000000000000299792458),
            (tokensToBurn * 10**6)
        );
        totalPresalePoolAmount =
            totalPresalePoolAmount -
            (tokensToBurn * 10**6);
    ...
    function claimTokens() external nonReentrant {
        ...
        uint256 buyerAmountAvailable = (presaleBuyers[msg.sender]
            .weightedBalance * totalPresalePoolAmount) /
            totalPresaleWeightedBalance;
