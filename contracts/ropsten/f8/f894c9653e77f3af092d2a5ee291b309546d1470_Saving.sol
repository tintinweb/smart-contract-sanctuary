...
contract Saving is TokenWraper {
    ...
    mapping(address => uint256) public savedTimestamp;
    ...
    function rewardEarned(address account) public view returns(uint256) {
        ...
        if(claimRewardTimestamp[account] == savedTimestamp[account]) {
            return _rewardEarned;
        }else {
            uint256 _cRangeTime = _currentTime.sub(claimRewardTimestamp[account]);
            uint256 _cRewardEarned = _rewardEarnedPerSecond(account).mul(_cRangeTime);
            _rewardEarned = _rewardEarned.sub(_cRewardEarned);
            return _rewardEarned;
        }
    }
