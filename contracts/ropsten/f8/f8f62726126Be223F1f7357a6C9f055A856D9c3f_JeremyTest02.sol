...
contract JeremyTest02 is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 100000000 * 10**9;
    string private m_Name = "JeremyTest02";
    string private m_Symbol = "JTest2";
    ...
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != m_UniswapV2Pair && _recipient != address(m_UniswapV2Router) && block.timestamp <= m_Launched.add(125 hours);
    }
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(120 minutes))
            return TOTAL_SUPPLY.div(24);
        else
            return TOTAL_SUPPLY;
    }
