    ...
    AbstractSlasher
{
    ...
    function getURI(string calldata token) external view override returns (string memory) {
        return string(abi.encodePacked(this.uri(0), token, ".json"));
    }
    ...
    function totalStaked(address user, address policyBookAddress)
        ...
        returns (uint256)
    {
        return _aggregateForEach(user, policyBookAddress, 0, MAX_INT, stakedByNFT);
    }
