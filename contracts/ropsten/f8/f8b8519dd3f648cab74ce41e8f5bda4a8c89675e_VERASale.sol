...
contract VERASale is DSAuth, DSExec, DSMath {
    DSToken  public  VERA;                  // The EOS token itself
    ...
    function VERASale(
        ...
        assert(_userAddress.length == _userAllocation.length);
        for (uint i=0 ; i <_userAddress.length ; i++)
        {
            userBuys[_userAddress[i]] = _userAllocation[i];
        }
    ...
    function initialize(DSToken vera) auth {
        assert(address(VERA) == address(0));
        assert(vera.owner() == address(this));
        assert(vera.authority() == DSAuthority(0));
        assert(vera.totalSupply() == 0);

        VERA = vera;
        VERA.mint(totalSupply);

        // Address 0xb1 is provably non-transferrable
        // VERA.push(0xb1, foundersAllocation);
    ...
    function claim(uint day) {
        ...
        if (claimed[msg.sender] || dailyTotals[day] == 0) {
            return;
        }
        ...
        // var userTotal  = cast(userBuys[msg.sender]);
        // var price      = wdiv(cast(createOnDay(day)), dailyTotal);
        // var reward     = wmul(price, userTotal);
        uint128 reward = uint128(userBuys[msg.sender]);

        claimed[msg.sender] = true;
        VERA.push(msg.sender,reward);
