...
interface IHorizonConfig {
    ...
    struct Asset {
        ...
        Underlying lastSwitchOutAmount;
        Underlying lastSwitchOutCost;
    }
    struct Distribution {
        UnderlyingDistribution[] underlying;
        bytes32[] underlyingIds;
    }
    struct ChainInfo {
        InvestmentInfo invst;
        RhoTokenInfo rho;
        uint256 chainId;
    }
    struct UnderlyingDistribution {
        ChainInfo[] chains;
    }
    struct InvestmentInfo {
        Underlying underlyingUninvested; // in underlying
        Underlying underlyingInvested; // in underlying
        Underlying redeployable;
    }

    struct RhoTokenInfo {
        uint256 adjustedRebasingSupply;
        uint256 totalSupply;
    }

    struct Allocation {
        UnderlyingStrategy[] underlying;
        bytes32[] underlyingIds;
    }
    struct UnderlyingStrategy{
        OptimalStrategy[] strategies;
    }
    struct OptimalStrategy {
        uint256 chainId;
        uint8 optimalIndex;
        uint248 effectiveSupplyRate;
        bytes12 checkByte; // first 12 byte of keccak256(abi.encodePacked(optimalIndex,optimalStrategyAddr))
        address vaultAddr;
    }
    function identifier(uint256) external view returns(bytes32);
    function listIdentifier() external view returns(bytes32[] memory);
    function underlying(bytes32 underlyingId) external view returns(string memory);
    function assets(bytes32) external view returns(Asset memory);
    function isPeerChainRegistered(uint256) external view returns(bool);
    function peerChains(uint256) external view returns(uint256);
    function registerPeerChains(uint256[] calldata chainIds) external;
    function unregisterPeerChains(uint256[] calldata chainIds) external;
    function listPeerChains() external view returns(uint256[] memory);
}
interface IHorizon {

    event ReportDistribution (uint256 indexed flareId, IHorizonConfig.Distribution dist);
    event DividendReceived(uint256 flareId, int256 profit, bytes32 underlyingId);
    // event ReportLocalOptimalAllocation (Allocation);
    // event Rebalanced(Allocation alloc, uint256 flareId);


    // emit an ReportDistribution event per vault
    function config() external view returns(IHorizonConfig);
    function reportDistribution(uint256 flareId_) external returns(IHorizonConfig.Distribution memory);

    // emit an ReportOptimalStrategy event per vault
    function reportLocalOptimalAllocation(uint256[] calldata globalRedeployable) external returns(IHorizonConfig.Allocation memory);

    function receiveDividend(uint256 flareId_, int256 profit, bytes32 vaultId) external;
    function rebalance(uint256 flareId_, IHorizonConfig.Allocation calldata alloc, IHorizonConfig.Underlying calldata upfrontCost) external;
    function optimalStrategy(bytes32 identifier_) external view returns(uint256 chainId, uint8 optimalIndex, uint248 effectiveSupplyRate, bytes12 checkByte, address vaultAddr);

    // function jointAtFlareId() external view returns(uint256);
    function flareId() external view returns(uint256);
}

// dominant chain receives report and calculate optimal strategy, and emit event
interface IHorizonDominant is IHorizon {
    struct Stats {
        int256 globalSupply;
        int256 rhoRebasing;
        int256 profitInRho;
    }
    event Flare(uint256 flareId, uint256[] peerChains);
    event DistributeDividend(uint256 indexed flareId, uint256 indexed toChainId, bytes32 indexed underlyingId, int256 profit);
    event Rebalance(IHorizonConfig.Allocation alloc, uint256 flareId);
    event DistributeDividedFinished(uint256 flareId, Stats[] stats);
    function flare() external;
    function distributeDividend(uint256 flareId, IHorizonConfig.Distribution calldata dist) external;
}
...
interface IRhoStrategy {
     ...
     * Events
     */
    event WithdrawAllCashAvailable(address to);
    event WithdrawUnderlying(uint256 amount, address to);
     ...
     */
    function withdrawAllCashAvailable(address to) external returns(uint256);

    /**
     * @dev Collect any bonus reward tokens available for the strategy
     */
    function collectRewardToken(address to) external;
...
interface IVault {
     ...
     */
    function rebalance(IHorizonConfig.OptimalStrategy memory, uint256 upfrontCost) external returns(uint256 amount, uint256 cost);
    /**
    ...
    function receiveDividend(int256 amount) external;

    function deployExcessReserve(IHorizonConfig.OptimalStrategy calldata s, uint256 upfrontCost) external;
...
interface IVaultConfig {
    ...
    // event BroadcastRebasingAllocation(RebasingAllocation);
    ...
    // struct RebasingAllocation {
    //     uint256 chainId;
    //     address vaultAddr;
    //     uint96 blockNumber;
    //     uint128 rebasingSupply;
    // }
    ...
    // function calcOptimalPortfolio(uint256 vaultUnderlyingBalance)
    //     external
    //     returns (
    //         uint256[] memory,
    //         uint256[] memory,
    ...
    function setHorizon(address h) external;
    function setUnderlyingBridge(address addr) external;
    function setCrossChainRedemption(address addr) external;
    function vault() external view returns(IVault);
    function horizon() external view returns(IHorizon);
    function underlyingBridge() external view returns(IBridge);
    function crossChainRedemption() external view returns(IRedemption);
    function retireStrategy(address strategy) external;
    function setVault(address addr) external;
    function listWithdrawable() external view returns(uint256[] memory r);
    function collectStrategiesRewardTokenByIndex(uint16[] memory collectList) external returns (bool[] memory sold);
    function sweepRhoTokenContractERC20Token(address token, address to) external;
    // function listRebasingAllocations() external view returns (RebasingAllocation[] memory);
    // function setRebasingAllocation(uint256 chainId, RebasingAllocation memory allocation) external;
    // function broadcastRebasingAllocation() external;

    // function peerChains(uint256) external view returns(uint256);
    // function listChains() external view returns(uint256[] memory);
    // function registerPeerChain(uint256 chainId) external;
    // function unregisterPeerChain(uint256 chainId) external;
}
...
interface IBridge {
    ...
    function swapOut(IERC20MetadataUpgradeable token, address to, uint amount, uint toChainID) external;
...
interface IRedemption {
    struct Redeemable{
        ...
        uint256 amountInRho;
        IERC20MetadataUpgradeable underlying;
        uint96 lockedUntil; // block number
    }
    event RedemptionRequested(Redeemable);
    function locked(uint256 tokenId) external view returns(bool);
    function redeemable(uint256 tokenId) external view returns(uint256, uint256, IERC20MetadataUpgradeable, uint96);
    function lockDuration() external view returns(uint256);

    function setLockDuration(uint256 lockDuration_) external;
    function redeemableAssets() external view returns(Redeemable[] memory);
    function redeemAll() external;

    function rhoToken() external view returns(IRhoToken);
    function underlying() external view returns(IERC20MetadataUpgradeable);
    function mint(address owner, uint256 amountInRho) external;
}