...
contract RewardsGenerator is IRewardsGenerator, OwnableUpgradeable, AbstractDependant {
        ...
        returns (uint256)
    {
        ...
        uint256 totalPoolStaked = _totalPoolStaked;

        rewardPerBlockPolicyBook = rewardPerBlockPolicyBook.div(10**20); // leaving only 5 precision decimals

        if (totalPoolStaked == 0 || rewardPerBlockPolicyBook == 0) {
            return 0;
        }
        ...
        return
            rewardPerBlockPolicyBookDAI
                .mul(BLOCKS_PER_DAY)
                .mul(_policyBooksRewards[policyBookAddress].rewardMultiplier)
                .div(totalPoolStaked)
                .mul(365);
    }
