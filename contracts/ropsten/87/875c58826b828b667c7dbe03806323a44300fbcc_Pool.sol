 ...
 * @title Pool
 *
 * @notice this implements the CRC core Pool contract. It supports generalized
...
contract Pool is IPool, IEvents, ReentrancyGuard, OwnerController {
     ...
     * @inheritdoc IPool
     */
    function crcBalance() external view override returns (uint256) {
        return _crcVested;
    }
    ...
    function withdraw(uint256 amount) external override {
        requireController();
        require(amount > 0, "p1");
        require(amount <= _crcVested, "p2");

        // do transfer
        _crc.safeTransfer(msg.sender, amount);

        _crcVested = _crcVested - amount;

        emit CrcWithdrawn(amount);
    }
    ...
    /**
     * @dev private method to process Crc spending and vesting
     * @param spent number of tokens to unstake
     * @param vested data passed to staking module
     */
    function _processCrc(uint256 spent, uint256 vested) private {
        // spending
        if (spent > 0) {
            _crc.safeTransferFrom(msg.sender, address(this), spent);
        }
        ...
        if (vested > 0) {
            ...
            if (fee > 0) {
                _crc.safeTransfer(_factory.treasury(), fee);
            }
            _crcVested = _crcVested + vested - fee;
        }
    }
}