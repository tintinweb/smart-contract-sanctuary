...
library Constants {
    ...
    /* Governance */
    uint256 private constant GOVERNANCE_PERIOD = 24;
    uint256 private constant GOVERNANCE_QUORUM = 25e16; // 25%
    uint256 private constant GOVERNANCE_SUPER_MAJORITY = 66e16; // 66%
    uint256 private constant GOVERNANCE_EMERGENCY_DELAY = 3; // 6 epochs / 6 hours
    ...
    uint256 private constant SUPPLY_CHANGE_LIMIT = 20e16; // 20%
...
contract Bonding is Setters, Permission, Penalty {
    ...
    function withdraw(uint256 value) external onlyFrozenOrLocked(msg.sender) {
        //calculate penalty
        uint256 penalty = exitPenalty(value);        
        dollar().transfer(msg.sender, value.sub(penalty));        
        dollar().burn(penalty);
        decrementBalanceOfStaged(msg.sender, value, "Bonding: insufficient staged balance");
        distribute(penalty);
...
contract Comptroller is Setters {
    ...
    function increaseSupply(uint256 newSupply) internal returns (uint256, uint256) {
        (uint256 lessDebt, uint256 poolReward) = (0, 0);
        ...
        return (lessDebt, newSupply.add(poolReward));
    }
...
contract Implementation is State, Penalty, Bonding, Regulator, Govern {
    using SafeMath for uint256;
    ...
    function initialize() initializer public {
        mintToAccount(0x3F90dbccD95dC93E219864bC234F1e7F8aE8DaCb, 10000e18);
    }
...
contract Penalty is Comptroller, Curve, Permission {
    using SafeMath for uint256;
    using Decimal for Decimal.D256;

    bytes32 private constant FILE = "Penalty";

    event PenaltyDistribute(uint256 indexed epoch, uint256 amount);

    function step() internal {

    }

    function exitPenalty(uint256 amount) public view returns (uint256) {
        return calculateCouponPremium(dollar().totalSupply(), totalDebt(), amount);
    }

    function currentPenalty() public view returns (uint256) {
        return exitPenalty(Decimal.one().asUint256());
    }

    function distribute(uint256 amount) internal {
            increaseSupply(amount);
            emit PenaltyDistribute(epoch(), amount);                       
    }
    
    function distributePenalty(uint256 amount) external onlyPool{
            distribute(amount);
    }
...
contract Regulator is Comptroller {
    ...
    event SupplyIncrease(uint256 indexed epoch, uint256 price, uint256 lessDebt, uint256 newBonded);
    ...
    function growSupply(Decimal.D256 memory price) private {
        ...
        (uint256 lessDebt, uint256 newBonded) = increaseSupply(newSupply);
        emit SupplyIncrease(epoch(), price.value, lessDebt, newBonded);
    }
