...
contract ERC20_BTest14 is Context, IERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 private uniswapV2Router;
    address public uniswapV2Pair;

    string private constant _name = "BTest-14"; 
    string private constant _symbol = "BTest-14";
    uint8 private constant _decimals = 9;  
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000000000000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _tFeeTotal;
    uint256 public _BTest14Burned;
    ...
    uint256 private _BTest14Burn = 2;    
    ...
    address public targetToken = 0xaD6D458402F60fD3Bd25163575031ACDce07538D; //dai
    ...
    function removeAllFee() private {
        if (_reflection == 0 && _contractFee == 0 && _BTest14Burn == 0) return;
        _reflection = 0;
        _contractFee = 0;
        _BTest14Burn = 0;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = _BTest14EthBurn(amount);
    ...
    function _BTest14EthBurn(uint amount) private returns (uint, uint) {  
        uint orgAmount = amount;
        uint256 currentRate = _getRate();
        uint256 tBurn = amount.mul(_BTest14Burn).div(100);
        uint256 rBurn = tBurn.mul(currentRate);
        _tTotal = _tTotal.sub(tBurn);
        _rTotal = _rTotal.sub(rBurn);
        _BTest14Burned = _BTest14Burned.add(tBurn);
