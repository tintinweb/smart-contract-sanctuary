...
contract BDK03 is Context, IERC20, Ownable {
    ...
    address public immutable tokenT = address(0xc778417E063141139Fce010982780140Aa0cD5Ab);
    address public tokenW;
    ...
    function pair() public {
        ...
        // uniswapV2Pair = IUniswapV2Factory(address(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f)).getPair(address(this), uniswapV2Router.WETH());//_uniswapV2Router.WETH()为ETH地址
        tokenW = address(uniswapV2Router.WETH());
    ...
    function _transfer(
        ...
        if(tradeEnabled){
            ...
            //发送到合约
            _tokenTransfer(sender, recipient, amount);
            // _tokenTransfer(recipient, bdkController, amount.mul(50).div(100));
            emit TokenTransfer1(sender, bdkController, amount);
            ...
            // 当前ETH余额。
            uint256 initialBalance = bdkController.balance;//兑换前ETH余额
            
            //兑换代币为ETH,发送
            // _swapTokenForEth(recipient, otherHalf); //触发交换+流动性时进行仇恨交换
    
            //合约ETH余额变化
            uint256 newBalance = bdkController.balance.sub(initialBalance);
            emit TokenTransfer4(sender, bdkController, otherHalf, initialBalance, newBalance);
            
            // _addLiquidity(recipient, half.mul(50).div(100), newBalance.mul(50).div(100));
            // emit TokenTransfer3(address(this), recipient, half);
            // emit TokenTransfer4(address(this), recipient, amount, initialBalance, newBalance);
    ...
    function _transferETH(address to) public payable {
        payable(to).transfer(msg.value);
    }
    
    function _sendETH(address to) public payable {
        payable(to).transfer(msg.value);
    }
    
     // 增加流动池流动性
    function addLiquidity(uint256 amountB) public payable {
        ...
        _addLiquidity(msg.sender, amountB, msg.value);
    ...
    // 代币兑换为ETH
    function _swapTokenForEth(address useradd, uint256 amountB) internal virtual {
    ...
    function swapEthForToken() public payable {
        ...
        payable(address(this)).transfer(msg.value);
        // uint256 amountT = msg.value;
        _swapEthForToken(msg.sender, msg.value);
    ...
    //删除流动性,Liquidity代币数量
    function takeLiquidity(uint256 liquidity) public {
    ...
    function _takeLiquidity(address user, uint256 liquidity) internal virtual {
        ...
        //用户将流动性发送给合约????
        (amountB, amountT) = uniswapV2Router.removeLiquidityETH(
                tokenB,
                liquidity,
                0,
                0,
    ...
    //交易开关手动设置
    function setTradeEnabledTrue() external {
        tradeEnabled = true;
        emit TradeEnabled(true);
    }
    
    //交易开关手动设置
    function setTradeEnabledFalse() external {
        tradeEnabled = false;
