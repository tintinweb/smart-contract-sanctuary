...
//    _barn = 0xc576C253573121A9e22F8c715519F2B1F64C1348;
  address _pointer = 0x6cea8Aa96B1e5898F256AEcAf863cF87cc73DA25;
  pointer = IPointer(_pointer);
    _barn = pointer.FindByName('barn');
    barn = IBarn(_barn);
    
    //wool = WOOL(_wool);
    _traits = pointer.FindByName('traits');
    traits = ITraits(_traits);


    _wool = pointer.FindByName('wool');
    wool = WOOL(_wool);
//    traits = ITraits(_r);
  ...
  function ContractSetBarn(address addr) external onlyOwner {
    barn = IBarn(addr);
  }
...
//    _traits = addr;
  ...
  function ContractSetFromPointerBarn() external onlyOwner {
    address addr = pointer.FindByName('barn');
    barn = IBarn(addr);
  }
  function ContractSetFromPointerWool()public onlyOwner
  {
    address addr = pointer.FindByName('wool');
    wool = WOOL(addr);
  }
  function ContractSetFromPointerTraits()public onlyOwner
  {
    address addr = pointer.FindByName('traits');
    traits = ITraits(addr);
  }
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    require(amount > 0 && amount <= 100, "Invalid mint amount (less 1 or mor 100");
