...
contract Horse is Ownable {
    ...
    function checkActualHorseStable(string memory _id, string memory _stableName) public dataExist(_id, _stableName) returns (bool) {
        return keccak256(abi.encodePacked(horses[_id].stables[horses[_id].stables.length - 1])) == keccak256(abi.encodePacked(_stableName));
    }

    function horseHistory(string memory _id) public returns (string[] memory){
