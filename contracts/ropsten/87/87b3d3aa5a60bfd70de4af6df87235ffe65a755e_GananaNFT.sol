...
contract GananaNFT is ERC721, Ownable, Whitelist, ReentrancyGuard {
 ...
 // A mapping of all of the auctions currently running.
    ...
    constructor()  ERC721("Ganana", "NANAN")  {
    ...
    function buy(uint _id) external payable {
        ...
        address payable  _previousOwner = payable(ownerOf(_id));
        ...
        if (currency[_id] == 1) {
            transferETHOrWETH(_previousOwner, _sellerValue);
        } else {
            require(IERC20(paymentTokenAddress).transferFrom(_newOwner, _previousOwner, _sellerValue));
            require(IERC20(paymentTokenAddress).transferFrom(_newOwner, address(this), _commissionValue));
        }
    ...
    */
    function setPaymentToken(address newERC20Contract) external onlyOwner {
        ...
        auctionComplete(tokenId)
    {
        ...
        address fundsRecipient = auctions[tokenId].fundsRecipient;
        ...
        if (CreatorFeePercent > 0) {
            ...
            transferPaymentToken(Creator, CreatorAmount);
        ...
        address payable nftCreator =
            payable(
                address(IMediaModified(address(this)).tokenCreators(tokenId))
            );
        ...
        if (nftCreator == fundsRecipient) {
            transferPaymentToken(nftCreator, amount);
        } else {
            ...
            uint256 creatorAmount =
                ...
                IMarket(IMediaModified(address(this)).marketContract())
                    .splitShare(
            ...
            transferPaymentToken(nftCreator, creatorAmount);
            // Send the remainder of the amount to the funds recipient.
            transferPaymentToken(fundsRecipient, amount.sub(creatorAmount));
        }
