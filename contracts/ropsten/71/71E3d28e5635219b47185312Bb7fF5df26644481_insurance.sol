...
contract insurance {
     ...
     */
    function ViewYourClaim(address _claimant) view public returns (string memory, string memory) {
      require(msg.sender == _claimant,"You can only view your own claims");
      return(claims[msg.sender].description, claims[msg.sender].approval);
    }
    
    /**
     * Allows the insurance company to view any claim
     */
    function InsurerViewClaimDetail(address ins) view public returns (string memory, string memory) {
      require(msg.sender == owner,"Insurer only function");
      return(claims[ins].description, claims[ins].approval);
    }
}