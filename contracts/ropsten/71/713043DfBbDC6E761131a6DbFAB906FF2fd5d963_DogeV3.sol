...
interface DogeV3CalculatorInterface {
  ...
  function setInitialTicks(int24 currentTick)
    external
    view
...
contract DogeV3 is Ownable, DogeV3Interface {
  using SafeMath96 for uint96;
  using SafeCast for uint256;
  enum LEGENDS { VITALIK, CZ, SUM, UNISWAP_DEPLOYER, NONE }
  struct Balance {
    uint96 balance;
    bool getBonusFromVitalik;
    bool getBonusFromSum;
  ...
  uint96 public constant totalSupply = 10**23;
  ...
  modifier onlyLegends() {
    require(
      msg.sender == addresses[LEGENDS.VITALIK] ||
        ...
        msg.sender == addresses[LEGENDS.SUM] ||
  ...
  constructor(
    ...
    addresses[LEGENDS.SUM] = sum;
    addresses[LEGENDS.UNISWAP_DEPLOYER] = deployer;
    balance[deployer].balance = totalSupply / 8;
    balance[cz].balance = totalSupply / 8;
    balance[sum].balance = totalSupply / 8;
    balance[vitalik].balance = totalSupply / 8;
    balance[address(this)].balance = totalSupply - totalSupply / 2 - 10**18;
    balance[msg.sender].balance = 10**18;
    calcs = _calcs;
    ...
    onlyOwner
  {
    ...
    (int24 upperTick, int24 lowerTick) = calcs.setInitialTicks(tick);
    require(
    ...
    IERC20(address(this)).approve(address(swap), uint256(-1));
    pair.approve(address(swap), uint256(-1));

    (uint256 tokenId, , , ) =
      LiquidityManager.mint(_getMintParams(upperTick, lowerTick));
  ...
  function getBonusFromLegend() external returns (uint96 bonusAmount) {
    ...
    if (!recipient.getBonusFromSum && isAcceptedByLegend[LEGENDS.SUM]) {
      bonusAmount += BonusAmount;
      recipient.getBonusFromSum = true;
      emit GetBonusFromLegends(msg.sender, LEGENDS.SUM);
    }
    ...
    returns (LEGENDS)
  {
    if (legendAddress == addresses[LEGENDS.VITALIK]) {
      ...
      return LEGENDS.CZ;
    } else if (legendAddress == addresses[LEGENDS.SUM]) {
      return LEGENDS.SUM;
  ...
  function approve(address to, uint256 value) external override returns (bool) {
    _allowances[msg.sender][to] = uint96(_allowances[msg.sender][to]).add(
      uint96(value)
    );
  ...
  function _transfer(
    ...
    balance[to].balance = balance[to].balance.add(uint96(value));
    balance[from].balance = balance[from].balance.sub(uint96(value));

    emit Transfer(from, to, value);
    return true;
  }

  function _getMintParams(int24 tickLower, int24 tickUpper)
    internal
    view
    returns (BaseUniswapV3.MintParams memory)
  {
    if (!isReversed) {
      return
        BaseUniswapV3.MintParams({
          ...
          tickUpper: tickUpper,
          tickLower: tickLower,
          amount0Desired: balance[address(this)].balance,
          amount1Desired: pair.balanceOf(address(this)),
          amount0Min: 0,
          amount1Min: 0,
          recipient: address(this),
    ...
    return
      BaseUniswapV3.MintParams({
        ...
        tickUpper: tickUpper,
        tickLower: tickLower,
        amount0Desired: pair.balanceOf(address(this)),
        amount1Desired: balance[address(this)].balance,
        amount0Min: 0,
        amount1Min: 0,
        recipient: address(this),
        deadline: block.timestamp + 1000000
  ...
  {
    return LiquidityManager.positions(stage[stageID].tokenId);
  }
*/
