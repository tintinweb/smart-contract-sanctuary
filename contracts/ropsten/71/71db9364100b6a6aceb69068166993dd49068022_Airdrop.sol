...
contract Airdrop is Configurable {
	...
	function __Airdrop_init(address governor_, address token_, address distributor_, uint expiry_) external initializer {
		__Governable_init_unchained(governor_);
		__Airdrop_init_unchained(token_, distributor_, expiry_);
	}
	
	function __Airdrop_init_unchained(address token_, address distributor_, uint expiry_) public governance {
		token = IERC20(token_);
	...
	function claim() public {
		...
		token.safeTransferFrom(distributor, msg.sender, quota_.sub(done_));
		done[msg.sender] = quota_;
		emit Claim(msg.sender, quota_.sub(done_));
	}
