...
contract Chubbies is ERC721, Ownable {
    using SafeMath for uint256;
    uint256 public constant MAX_CHUBBIES = 10000;
    bool public hasSaleStarted = false;
    ...
    function adoptChubby(uint256 numChubbies) public payable {
        require(totalSupply() < MAX_CHUBBIES, "Sale has already ended");
        require(
            numChubbies > 0 && numChubbies <= 20,
            "You can adopt minimum 1, maximum 20 chubbies"
        );
        require(
            totalSupply().add(numChubbies) <= MAX_CHUBBIES,
            "Exceeds MAX_CHUBBIES"
        );
        require(
            msg.value >= calculatePrice().mul(numChubbies),
            "Ether value sent is below the price"
        );

        for (uint256 i = 0; i < numChubbies; i++) {
    ...
    function reserveGiveaway(uint256 numChubbies) public onlyOwner {
        uint256 currentSupply = totalSupply();
        require(
            totalSupply().add(numChubbies) <= 30,
