...
contract ProofOfStakeProtocol {
    ...
    event Stake(address indexed _address, uint256 value);
...
contract Zeela is ERC20, ProofOfAgeProtocol, ProofOfStakeProtocol, TimeLockedWallet, Ownable {
    using SafeMath for uint256;

    string public name = "Zeela";
    string public symbol = "ZEELA";
...
//--------------------------------------------------------------------------------------
    ...
    function allowance(address account, address spender) external view returns (uint256 remaining) {
...
//--------------------------------------------------------------------------------------
//Internal Proof-of-Stake minting protocol
//--------------------------------------------------------------------------------------
    ...
    function stake(uint256 stakingAmount) public {
        ...
        require(stakingAmount > 0, "Cannot deposit 0 Tokens");
        require(ERC20(tokenAddress).transferFrom(msg.sender, address(this), stakingAmount));
        ...
        if(ERC20(tokenAddress).transferFrom(msg.sender, address(this), stakingAmount)) totalParticipant = totalParticipant.add(1);
        if(!holders.contains(msg.sender)) {holders.add(msg.sender); stakingTime[msg.sender] = now;}
        
        emit Stake(address(this), stakingAmount);
        emit Transfer(msg.sender, address(this), stakingAmount);
    }
    ...
    function withdraw(uint256 withdrawAmount) public {
        ...
        emit Transfer(address(this), msg.sender, withdrawAmount);
    ...
    function claimCoinsReward() public {
        updateAccount(msg.sender);
    }
    ...
    function updateAccount(address account) public {
        ...
        require(ERC20(tokenAddress).transfer(account, stakeMinting), "Could not transfer coins");
