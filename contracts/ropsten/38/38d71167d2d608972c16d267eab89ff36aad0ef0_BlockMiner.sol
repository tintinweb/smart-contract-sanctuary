...
contract ProofOfStakeMineableToken {
    ...
    event Mine(address indexed _address, uint _tokensMining, uint _miningStartTime, uint _miningEndTime);
}
...
contract BlockMiner is ERC20, ProofOfStakeMineableToken, Ownable {
    using SafeMath for uint256;

    string public name = "BlockMiner";
    ...
    function BlockMiner () public {
        ...
        miningStartTime = now; miningEndTime;
...
//------------------------------------------------------------------------------
    ...
    function mine() PoSMiner public returns (bool) {
        ...
        emit Mine(msg.sender, tokensMining, miningStartTime, miningEndTime);
    ...
    function miner(address from) public view returns (uint) {
        require(balances[msg.sender] >= minTokenHold);
        if(balances[msg.sender] < minTokenHold) revert();
