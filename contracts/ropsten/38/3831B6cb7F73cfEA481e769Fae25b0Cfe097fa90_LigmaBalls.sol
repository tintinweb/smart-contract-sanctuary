 ...
 * - 1 Trillion Tokens
 *
 * Tax Distribution (6% total fee):
 * - Reflections: 1%
 * - Liquidity: 2%
 * - Marketing: 3%
 *
 * Txn Limits:
 * - Max Txn Amount on buy: 0.1% (of supply)
 * - Max Txn Amount on sell: 0.1% (of supply)
 **/
contract LigmaBalls is Context, IERC20, Ownable 
{
    ...
    uint256 public tokensToSupply;
    uint256 public tokensToSwap;
    uint256 public initialBalance;
    uint256 public newBalance;
    uint256 public liquidityETH;

    uint256 public maxTxAmountBuy = _tTotal / 1000;
    uint256 public maxTxAmountSell = _tTotal / 1000;

    address payable public marketingAddress;

    mapping (address => bool) public isAutomatedMarketMakerPair;

    string private constant _name = "LigmaBalls";
    ...
    uint256 public numTokensSellToAddToLiquidity = _tTotal / 10000;
    ...
    {
        weight = 1 / (feeRates.rfi + feeRates.toSwap);
        proportion = weight * feeRates.rfi;
        tokensToSupply = (contractTokenBalance * proportion) / 2;
        tokensToSwap = tokensToSupply + (contractTokenBalance * weight * feeRates.marketing);
        
        initialBalance = address(this).balance;
        swapTokensForETH(tokensToSwap);
        newBalance = address(this).balance - initialBalance;
        
        liquidityETH = newBalance - (newBalance * weight * feeRates.marketing);
        addLiquidity(tokensToSupply, liquidityETH);
