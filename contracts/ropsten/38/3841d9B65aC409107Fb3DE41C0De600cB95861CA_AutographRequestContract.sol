...
contract AutographRequestContract is Ownable {
    ...
    // Events
    event RequestCreated(uint id, address from, address to, uint price, uint responseTime, uint created);
    event RequestDeleted(uint id, address from, address to, uint price, uint responseTime, uint created);
    event RequestSigned(uint id, address from, address to, uint price, uint responseTime, uint created, string nftHash, string metadata);
    event Withdrawal(address addr, uint amount, uint balance);
