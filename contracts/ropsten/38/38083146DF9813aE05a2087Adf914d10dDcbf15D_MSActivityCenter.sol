...
contract NoahNFT is ERC721Preset {
    ...
    uint256 public maxBreedLimit = 0;
    ...
    function setMaxBreedLimit(uint256 _maxBreedLimit) public {
        require(
            hasRole(MINTER_ROLE, _msgSender()),
            "NoahNFT: must have minter role to set maxBreedLimit"
        );
        maxBreedLimit = _maxBreedLimit;
    }
    ...
    function getTokenBreedLimit(uint256 tokenId) public view returns (uint256) {
        if (maxBreedLimit <= tokenData[tokenId].breedLimit) {
            return 0;
        } else {
            return maxBreedLimit - tokenData[tokenId].breedLimit;
        }
    }
    ...
    {
        TokenMetadata memory _data = tokenData[tokenId];
        if (maxBreedLimit <= _data.breedLimit) {
            _data.breedLimit = 0;
        } else {
            _data.breedLimit = maxBreedLimit - _data.breedLimit;
        }
        return _data;
    }
    ...
    function mint(uint256 activityId, address to) public returns (uint256) {
        ...
        super.mint(to);
        tokenData[tokenId] = TokenMetadata(birthDate, 0, 0, 0);

        emit Mint(activityId, tokenId, to, birthDate);
        return tokenId;
    }
    ...
    function breed(
        ...
        require(
            hasRole(MINTER_ROLE, _msgSender()),
            "NoahNFT: must have minter role to breed"
        );
        require(matronId != sireId, "NoahNFT: matronId is equal to sireId");
        require(
            ownerOf(matronId) == owner,
            "NoahNFT: Matron token is not belong to owner"
        );
        require(
            tokenData[matronId].breedLimit < maxBreedLimit,
            "NoahNFT: Matron cannot breed beyond limit"
        );
        require(
            ownerOf(sireId) == owner,
            "NoahNFT: Sire token is not belong to owner"
        );
        require(
            tokenData[sireId].breedLimit < maxBreedLimit,
            "NoahNFT: Sire cannot breed beyond limit"
        );

        uint256 tokenId = _tokenIdTracker.current();
        uint256 birthDate = block.timestamp;
        super.mint(owner);
        tokenData[tokenId] = TokenMetadata(birthDate, matronId, sireId, 0);
        tokenData[matronId].breedLimit += 1;
        tokenData[sireId].breedLimit += 1;

        emit Breed(activityId, matronId, sireId, tokenId, owner, birthDate);
        return tokenId;
    }
}
