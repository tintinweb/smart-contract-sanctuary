...
import {MockProviderModule} from "./abstract/MockProviderModule.sol";
...
contract MockCheapTaskExternalProvider is Ownable2, MockProviderModule {
    ...
    address public constant externalProvider =
        address(0x9386CdCcbf11335587F2C769BB88E6e30685945e);
    ...
    receive() external payable {
        require(
            ...
            "MockCheapTaskExternalProvider.receive:onlyGelatoCore"
        );
    }
    ...
    function submitTask(Log _log) external {
        Provider memory provider =
            Provider({
                addr: externalProvider,
                module: IGelatoProviderModule(address(this))
            });
        ...
        {} catch Error(string memory error) {
            revert(
                string("MockCheapTaskExternalProvider.submitTask.submitTask:")
                    .suffix(error)
            );
        } catch {
            revert(
                "MockCheapTaskExternalProvider.submitTask.submitTask:undefined"
            );
        }
    }

    function hashTaskSpec(bool _logCanExecFailed)
        external
        view
        returns (bytes32)
    {
        NoDataAction[] memory noDataActions = new NoDataAction[](1);
        NoDataAction memory noDataAction =
            NoDataAction({
                addr: address(this),
                operation: Operation.Call,
                dataFlow: DataFlow.None,
                value: false,
                termsOkCheck: _logCanExecFailed
            });
        noDataActions[0] = noDataAction;
        return keccak256(abi.encode(new Condition[](0), noDataActions));
    }
    ...
    function tRevert() external pure {
        revert("MockCheapTaskExternalProvider.tRevert");
    }
    ...
    function termsOk(
        ...
        else revert("MockCheapTaskExternalProvider.termsOk: test revert");
    }

    function cancelTask(TaskReceipt memory _taskReceipt) external payable {
        try IGelatoCore(gelatoCore).cancelTask(_taskReceipt) {} catch Error(
            string memory error
        ) {
            revert(
                string("MockCheapTaskExternalProvider.cancelTask:").suffix(
                    error
                )
            );
        } catch {
            revert("MockCheapTaskExternalProvider.cancelTask:undefined");
        }
    }
}
...
abstract contract MockProviderModule is GelatoProviderModuleStandard {
    function execPayload(
        uint256,
        address,
        address,
