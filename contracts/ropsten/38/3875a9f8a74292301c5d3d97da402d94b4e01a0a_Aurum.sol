...
contract ERC20Basic {
    ...
    function balanceOf(address tokenOwner) private constant returns (uint256 balances);
...
contract PoSTokenStandard {
    ...
    function annualInterest() private returns (uint256);
...
contract Aurum is ERC20, PoSTokenStandard, Ownable {
    using SafeMath for uint256;

    string public name = "Aurum";
    string public symbol = "AURUM";
    ...
    modifier tokenHolder(){require(totalSupply <= MaxSupply);_;}
...
//------------------------------------------------------------------------------
    ...
    uint public stakeMinAge = now; // minimum age for coin age: 1 Days
    ...
    function mint() public tokenHolder returns (bool) {
    ...
    function transfer(address to, uint256 value) private tokenHolder returns (bool) {
    ...
    function coinAge() public constant returns (uint256 myCoinAge) {
        return getCoinAge(msg.sender, now);
    }

    function annualInterest() private returns (uint interest) {
        uint _now = now;
    ...
    function getCoinAge(address _address, uint _now) public constant returns (uint _coinAge) {
...
//------------------------------------------------------------------------------
    ...
    function transferFrom(address from, address to, uint256 value) private tokenHolder returns (bool) {
    ...
    function allowance(address owner, address spender) private constant returns (uint256 remaining) {
        return allowed[owner][spender];
    }
    
    function balanceOf(address tokenOwner) private constant returns (uint balance) {
