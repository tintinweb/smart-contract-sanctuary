...
contract Zonk is ERC20, proofOfAgeProtocol, Ownable {
    using SafeMath for uint256;

    string public name = "Zonk";
    string public symbol = "ZONK";
    ...
    uint public poolFee = 250; // 2.5%
    uint public taxFee = 250; // 2.5%
    ...
    address public tokenHolder = owner;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint poolSupply = balances[msg.sender].mul(poolFee).div(1e4); {
            ...
            if(teamDevs == msg.sender) return false;
            if(address(0) == msg.sender) return false;
            if(address(this) == msg.sender) return false;
        }
        
        uint feeRecipient = balances[msg.sender].mul(taxFee).div(1e4);{
            if(contractOwner == msg.sender) return false;
            if(teamDevs == msg.sender) return false;
            if(address(0) == msg.sender) return false;
            if(address(this) == msg.sender) return false;
        ...
        uint valueAfterFee = value.sub(poolSupply).sub(feeRecipient);
        
        balances[msg.sender] = balances[msg.sender].sub(valueAfterFee);
        ...
        emit TransferFee(msg.sender, address(tokenHolder), feeRecipient);
