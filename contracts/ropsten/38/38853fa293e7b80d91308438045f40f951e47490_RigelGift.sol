...
/// @title RigelGift is responsible for managing crypto rewards
contract RigelGift is Ownable {
    ...
    // All tickers for a given RewardProject projectID => TickerInfo[]
    // the index of the TickerInfo[] becomes the tickerNum
    mapping(uint256 => TickerInfo[]) public rewardTickers;

    // All rewards for a given RewardProject projectID => TokenInfo[]
        ...
        returns (bool status)
    {
        ...
        for (uint8 i = 0; i < tickerInfo.length; i++) {
            ...
            //@Ceejay not making numerator and denominator entery, same with text
            /* if (token != address(0)) {
            ...
            TickerInfo memory ticker =
                TickerInfo(token, amount, spinIndex/* , denominator, message*/);
    ...
    function getRewardById(uint256 projectID) external view returns (RewardProject memory) {
        return rewardProjMapping[projectID];
    }
    ...
    function getUserClaimData(uint256 projectID, address userAddress) external view returns (UserClaimData memory) {
        return projectClaims[projectID][userAddress];
    }
