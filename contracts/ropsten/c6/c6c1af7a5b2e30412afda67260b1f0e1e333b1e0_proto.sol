...
*/
    ...
    address public ELIGIBLE_NFT_ADDRESS;
    // In this version the loan amount is a fixed amount:
    uint64  public LOAN_AMOUNT = 100000000000000000;
    // Term in days:
    uint16  public TERM_IN_DAYS = 90;
    // Reposession address - this is the address that NFTs will be send to on the expiry
    // of the loan term.
    address public REPO_ADDRESS;
    ...
    uint64  public SERVICE_FEE = 10000000000000000;
...
*/
    ...
    event loanAdvanced          (uint256 id, uint256 tokenId, address tokenAddress, address borrower, uint256 loanAmount, uint256 effectiveDate);
    event paymentReceived       (uint256 id, uint256 tokenId, address tokenAddress, address borrower, uint256 repaymentAmount, uint256 effectiveDate);
    event itemRedeemed          (uint256 id, uint256 tokenId, address tokenAddress, address borrower, uint256 effectiveDate);
    event itemExtended          (uint256 id, uint256 tokenId, address tokenAddress, address borrower, uint256 effectiveDate);
    event itemRepossesed        (uint256 id, uint256 tokenId, address tokenAddress, address borrower, uint256 currentBalance, uint256 effectiveDate);
...
*/
    ...
    modifier OnlyItemBorrower(uint256 id){
        require(itemsUnderLoan[id].borrower == msg.sender, "Payments can only be made by the borrower");
        _;
    }
    ...
    modifier IsCurrentlyCustodied(uint256 id){
        IERC721 tokenContract = IERC721(itemsUnderLoan[id].tokenAddress);
        require(tokenContract.ownerOf(itemsUnderLoan[id].tokenId) == address(this));
        _;
    }

    // Check that we have a loan for this item:
    modifier ItemExists(uint256 id){
        require(id < itemsUnderLoan.length && itemsUnderLoan[id].id == id, "Could not find item");
        _;
    }

    // Check to see if the array item returned is no longer current:
    modifier IsUnderLoan(uint256 id){
        require(itemsUnderLoan[id].isCurrent == true, "Item is not currently under loan");
        _;
    }
...
*/
    ...
    function setTermInDays(uint16 _TERM_IN_DAYS) public onlyOwner returns (bool) {
...
*/
    ...
    function advanceLoan(uint16 tokenId, address tokenAddress) OnlyItemOwner(tokenAddress,tokenId) HasTransferApproval(tokenAddress,tokenId) external returns (uint256){
        // Check this isn't already under loan!
        require(activeLoans[tokenAddress][tokenId] == false, "Item is already under loan");
        // The id is the length of the current array as this is the next item:
        uint64 newItemId = uint64(itemsUnderLoan.length);
        // Add this to the array:
        itemsUnderLoan.push(LoanItem(newItemId, true, tokenAddress, payable(msg.sender), uint32(block.timestamp), uint32(block.timestamp) + (TERM_IN_DAYS * 60 * 60 * 24), TERM_IN_DAYS, tokenId, LOAN_AMOUNT, SERVICE_FEE, LOAN_AMOUNT + SERVICE_FEE, LOAN_AMOUNT + SERVICE_FEE));

        
        activeLoans[tokenAddress][tokenId] = true;
        assert(itemsUnderLoan[newItemId].id == newItemId);
        // Custody the asset to this contract:
        IERC721(tokenAddress).safeTransferFrom(msg.sender, address(this), tokenId);
        // Send the borrower their ETH:
        payable(msg.sender).transfer(LOAN_AMOUNT);
        emit loanAdvanced(newItemId, tokenId, tokenAddress, msg.sender, LOAN_AMOUNT, block.timestamp);
        return newItemId;
    }
    ...
    // borrower:
    function receivePayment(uint64 id) payable public ItemExists(id) IsUnderLoan(id) OnlyItemBorrower(id) IsCurrentlyCustodied(id){
        require(itemsUnderLoan[id].currentBalance >= msg.value, "Payment exceeds current balance");
        // Reduce the balance outstanding by the amount of ETH received:
        itemsUnderLoan[id].currentBalance = (itemsUnderLoan[id].currentBalance - uint64(msg.value));
        // Emit this payment event: 
        emit paymentReceived(id, itemsUnderLoan[id].tokenId, itemsUnderLoan[id].tokenAddress, msg.sender, msg.value, block.timestamp);

        // See if this payment remains the loan is done and we can return the asset:
        if (itemsUnderLoan[id].currentBalance == 0)
        {   
            // Well done borrower! Paid on time - here is your NFT back:
            IERC721(itemsUnderLoan[id].tokenAddress).safeTransferFrom(address(this), msg.sender, itemsUnderLoan[id].tokenId);
            itemsUnderLoan[id].isCurrent = false;
            activeLoans[itemsUnderLoan[id].tokenAddress][itemsUnderLoan[id].tokenId] = false;
            emit itemRedeemed(id, itemsUnderLoan[id].tokenId, itemsUnderLoan[id].tokenAddress, msg.sender, block.timestamp);
        }
    }
    ...
    function extendLoan(uint256 id) payable public ItemExists(id) IsUnderLoan(id) OnlyItemBorrower(id) IsCurrentlyCustodied(id){
        ...
        itemsUnderLoan[id].endDate      = (itemsUnderLoan[id].endDate + (TERM_IN_DAYS * 60 * 60 * 24));
        itemsUnderLoan[id].termInDays   = (itemsUnderLoan[id].termInDays + TERM_IN_DAYS);
        // Emit this extension event: 
        emit itemExtended(id, itemsUnderLoan[id].tokenId, itemsUnderLoan[id].tokenAddress, msg.sender, block.timestamp);
    }

    // This function is called when an item is repossessed. This is ONLY possible when the 
    // loan has lapsed.
    function repossessItem(uint256 id) public ItemExists(id) IsUnderLoan(id) onlyOwner IsCurrentlyCustodied(id){
        require(itemsUnderLoan[id].endDate < block.timestamp, "Loan still current, what you trying to do?!");
        // Shouldn't be possible, but double-check this isn't a loan with a 0 outstanding balance:
        require(itemsUnderLoan[id].currentBalance > 0, "Loan balance is 0, cannot repossess");
        
        // repo the item to the repo address:
        IERC721(itemsUnderLoan[id].tokenAddress).safeTransferFrom(address(this), REPO_ADDRESS, itemsUnderLoan[id].tokenId);
        itemsUnderLoan[id].isCurrent = false;
        activeLoans[itemsUnderLoan[id].tokenAddress][itemsUnderLoan[id].tokenId] = false;
        emit itemRepossesed(id, itemsUnderLoan[id].tokenId, itemsUnderLoan[id].tokenAddress, itemsUnderLoan[id].borrower, itemsUnderLoan[id].currentBalance, block.timestamp);
    }

    // Repossess all eligible items:
    function repossessItems() public onlyOwner{
        for (uint i =1; i <= itemsUnderLoan.length; i++) {
            if (itemsUnderLoan[i].endDate < block.timestamp)
            {
                repossessItem(itemsUnderLoan[i].id);
            }
        }
    }
}