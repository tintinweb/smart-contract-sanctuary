...
// TokenFarm is the master of TKN. He can make TKN and he is a fair cat.
//
// Note that it's ownable and the owner wields tremendous power. The owner will set
// the governance contract and will burn its keys once TKN is sufficiently
// distributed.
//
...
contract TokenFarm is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accTKNPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accTKNPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accTKNPerShare; // Accumulated TKNs per share, times 1e12. See below.
    }
    ...
    // Block number when bonus TKN period ends.
    uint256 public bonusEndBlock;
    // TKN tokens created per block.
    uint256 public TKNPerBlock;
    // Bonus muliplier for early TKN makers.
    ...
    // The block number when TKN mining ends.
    ...
    constructor(
        Token _TKN,
        address _devaddr,
        uint256 _TKNPerBlock, // 100000000000000000000
    ...
    // Update the given pool's TKN allocation point. Can only be called by the owner or governance contract.
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        //TKN minting ocurrs only until endBLock
        if(_to > endBlock){
            _to = endBlock;
        }
    ...
    function pendingTKN(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 TKNReward = multiplier.mul(TKNPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTKNPerShare = accTKNPerShare.add(TKNReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accTKNPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        //TKN.mint(devaddr, TKNReward.div(100));
        TKN.mint(address(this), TKNReward);
        pool.accTKNPerShare = pool.accTKNPerShare.add(TKNReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to TokenFarm for TKN allocation.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTKNPerShare).div(1e12).sub(user.rewardDebt);
            safeTKNTransfer(msg.sender, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accTKNPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from TOkenFarm.
    ...
    function devFee() public {
        ...
        TKN.mint(devaddr, TKN.totalSupply().div(200));
