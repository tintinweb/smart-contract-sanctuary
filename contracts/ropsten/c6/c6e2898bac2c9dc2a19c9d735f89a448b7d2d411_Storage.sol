...
contract Storage {
    ...
    */
    ...
    function transferERC721(address collection, address destination, uint256 tokenId) public {
        ...
        require(isApproved(erc721, tokenId), "you must approve the contract to send tokens");

        erc721.safeTransferFrom(msg.sender, destination, tokenId);
    }

    /**
     * Checks that this token is approved by the sender to be transferred by this contract.
     */
    function isApproved(IERC721 erc721, uint256 tokenId) view internal returns (bool) {
