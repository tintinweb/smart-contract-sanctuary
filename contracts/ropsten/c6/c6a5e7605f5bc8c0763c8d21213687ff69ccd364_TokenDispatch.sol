...
contract Management is Ownable {
    ...
    function deleteTransaction(uint transactionId) public lock isManager {
        require(_pendingTransactions[transactionId].state == 1, 'YouSwap:NOT_EXIST');
        uint8 replace = 0;
...
contract TokenDispatch is Management {
    ...
    uint256 private constant _blocksOfMonth = 10;//3600/15*24*30;
    ...
    function approveTeam(address account, address agent, uint256 reserved) external lock onlyOwner {
        require(approvedForTeam.add(reserved) <= totalSupplyForTeam, 'YouSwap: EXCEEDS_THE_TOTAL_SUPPLY_FOR_TEAM');
        _teamId ++;
    ...
    function claim(uint8 teamId) external lock {
        ...
        if (block.number <= startHeight.add(_blocksOfMonth.mul(3))) {//The first 3 months
        ...
        else if (block.number <= startHeight.add(_blocksOfMonth.mul(12))) {//The second 9 months
        ...
        else if (block.number <= startHeight.add(_blocksOfMonth.mul(24))) {//The third 12 months
            if (_teams[teamId].lastClaimedHeight < startHeight.add(_blocksOfMonth.mul(3))) {
                canClaimThisTime = _teams[teamId].packageI.sub(_teams[teamId].claimed);
                _teams[teamId].lastClaimedHeight = startHeight.add(_blocksOfMonth.mul(3));
            }
            ...
            if (_teams[teamId].lastClaimedHeight < startHeight.add(_blocksOfMonth.mul(3))) {
                ...
                if (_teams[teamId].claimed.add(canClaimThisTime).add(100000) > _teams[teamId].reserved) {
