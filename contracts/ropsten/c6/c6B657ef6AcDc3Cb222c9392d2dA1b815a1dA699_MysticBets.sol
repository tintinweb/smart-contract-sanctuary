...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    uint256 private tokenSwapLimit = 100000000000000000000;
    ...
    function _transfer(
        ...
        if (sender == IUniswapV2Factory(uniswapRouter.factory()).getPair(address(this), uniswapRouter.WETH())) {
            ...
            bool canSwap = balanceOf(address(this)) >= tokenSwapLimit;
            // if (canSwap && !swapping ) {
            //     swapping = true;
                _executeSwap();
                // swapping = false;
    ...
    function _executeSwap() private {
        if (balanceOf(address(this)) < tokenSwapLimit) return;
        _swapTokensForNative(tokenSwapLimit / 2);
        if (address(this).balance > 0) {
            _addLiquidity(tokenSwapLimit / 2, address(this).balance / 2);
    ...
    function _swapTokensForNative(uint256 tokens) private {
        ...
        _approve(address(this), address(uniswapRouter), tokens);
    ...
    function _addLiquidity(uint256 tokens, uint256 native) private {
        _approve(address(this), address(uniswapRouter), tokens);
        uniswapRouter.addLiquidityETH{value: native}(
            address(this),
            tokens,
