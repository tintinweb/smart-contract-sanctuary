...
// DhoardBar with Governance.
contract DhoardBar is BEP20('DhoardBar Token', 'DHOARD') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    function safeoriTransfer(address _to, uint256 _amount) public onlyOwner {
...
//     // Perform LP token migration from legacy DragonSwap to ori.
...
contract MasterChef is Ownable {
    ...
    uint256 public constant maxtokenperblock = 5*10**18;// 30 token per block
    ...
    function updatePool(uint256 _pid) public {
        ...
        ori.mint(address(dhoard), oriReward);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            uint256 amount2 = _amount.div(100).mul(stablebscpools_w_fee);
            uint256 amount3 = _amount.div(100).mul(lotteryfund_w_fee);
            uint256 amount4 = _amount.div(100).mul(dexrenewable_w_fee);
            uint256 amount5 = _amount.div(100).mul(teamfund_w_fee);
            uint256 amount6 = _amount.div(100).mul(project_w_fee);
            uint256 amount0 = _amount.sub(amount2).sub(amount3).sub(amount4);
            uint256 amount1 = amount0.sub(amount5).sub(amount6);
    ...
    // Stake ori tokens to MasterChef
    function enterStaking(uint256 _amount) public {
        ...
        dhoard.mint(msg.sender, _amount);
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        dhoard.burn(msg.sender, _amount);
    ...
    function safeoriTransfer(address _to, uint256 _amount) internal {
        dhoard.safeoriTransfer(_to, _amount);
    }
    ...
    //change withdraw fee
    function changeWithdrawFee(uint256 _newstablebscpools_w_fee, uint256 _newlotteryfund_w_fee, uint256 _newdexrenewable_w_fee, uint256 _newteamfund_w_fee, uint256 _newproject_w_fee) public onlyOwner {
        require(_newstablebscpools_w_fee <= maxwithdrawfee, "too high value");
        require(_newlotteryfund_w_fee <= maxwithdrawfee, "too high value");
        require(_newdexrenewable_w_fee <= maxwithdrawfee, "too high value");
        require(_newteamfund_w_fee <= maxwithdrawfee, "too high value");
        require(_newproject_w_fee <= maxwithdrawfee, "too high value");
        require(_newstablebscpools_w_fee >= minwithdrawfee, "too low value");
        require(_newlotteryfund_w_fee >= minwithdrawfee, "too low value");
        require(_newdexrenewable_w_fee >= minwithdrawfee, "too low value");
        require(_newteamfund_w_fee >= minwithdrawfee, "too low value");
        require(_newproject_w_fee >= minwithdrawfee, "too low value");
        stablebscpools_w_fee = _newstablebscpools_w_fee;
        lotteryfund_w_fee = _newlotteryfund_w_fee;
