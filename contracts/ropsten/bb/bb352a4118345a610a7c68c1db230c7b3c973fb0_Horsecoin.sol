...
contract Horsecoin is Ownable {
    ...
    event SetLuckyBonusRate(uint256 rate);
    event SetLuckyBonusCondition(uint256 amount);
    event SetSellFeeRate(uint256 rate);
    event SetTransferFeeRate(uint256 rate);
        ...
        onlyOwner
    {
        require(
            ...
            "Horsecoin: can't set the same status"
        );
        swapAndLiquifyEnabled = _swapAndLiquifyEnabled;
        ...
        onlyOwner
    {
        ...
        emit SetLuckyBonusCondition(_luckyBonusCondition);
    }
