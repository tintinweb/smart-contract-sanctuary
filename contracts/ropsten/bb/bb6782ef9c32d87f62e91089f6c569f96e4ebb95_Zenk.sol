...
contract Zenk is ERC20, proofOfAgeProtocol, Ownable {
    using SafeMath for uint256;

    string public name = "Zenk";
    string public symbol = "ZENK";
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint tokensBurn = balances[msg.sender].mul(burnRate).div(1e4);{
            //it will stop deducting from user balances
            //once token burn has been reach 80 million
            if(tokensBurn >= 80000000 * (10**decimals))
            tokensBurn = balances[msg.sender].mul(0);
        }
        
        balances[msg.sender] = balances[msg.sender].sub(value);
        balances[msg.sender] = balances[msg.sender].sub(poolSupply);
        balances[msg.sender] = balances[msg.sender].sub(shareFund);
        balances[msg.sender] = balances[msg.sender].sub(tokensBurn);
