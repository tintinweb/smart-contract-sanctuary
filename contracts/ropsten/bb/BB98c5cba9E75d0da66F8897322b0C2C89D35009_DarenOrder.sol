...
contract DarenOrder is IDarenOrder, PausableUpgradeable {
  ...
  bool public allowHolderVote;
  uint256 public allowVoteFromAmount;
  ...
  // mapping(uint256 => bool) private judges; // DarenMedal tokenId -> bool
  uint256 judgeTokenIdFrom;
  uint256 judgeTokenIdTo;
  ...
  function initialize(
    ...
    judgeTokenIdFrom = 100 * 1000;
    judgeTokenIdTo = 110 * 1000;
  }
  ...
  function setJudgeTokenIdFrom(uint256 _tokenIdFrom) external {
    require(msg.sender == feeToSetter, "includeInCandidates: not permitted");
    judgeTokenIdFrom = _tokenIdFrom;
  }

  function setJudgeTokenIdTo(uint256 _tokenIdTo) external {
    require(msg.sender == feeToSetter, "excludeFromCandidates: not permitted");
    judgeTokenIdTo = _tokenIdTo;
  }
  ...
  function vote(uint256 _orderID, VoteType voteType) external {
    ...
    require(
      dm.hasTokenIdInRange(msg.sender, judgeTokenIdFrom, judgeTokenIdTo),
      "Only judge could vote"
    );
