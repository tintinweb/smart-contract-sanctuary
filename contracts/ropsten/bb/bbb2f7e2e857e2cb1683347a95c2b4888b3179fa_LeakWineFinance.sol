...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    function _rewardLiquidityProviders(uint256 liquidityRewards) private {
        if(Xleakwine != address(0)) {
            super._transfer(address(this), Xleakwine, liquidityRewards);
            IUniswapV2Pair(Xleakwine).sync();
            emit SupplyXleakwine(liquidityRewards);
        }
        else {
...
contract Balancer {
    using SafeMath for uint256;    
    LeakWineFinance token;
    address public burnAddr = 0x351aF1631Aa5eA1cA62aD8a4e3cd87128D4D9108;
    address payable public treasury;
    
    constructor(address payable treasury_) public {
        token = LeakWineFinance(msg.sender);
        treasury = treasury_;
    }
    function () external payable {}
...
contract LeakWineFinance is 
    ERC20(1250000 ether), 
    ERC20Detailed("Leak Wine Finance", "Leak", 18), 
    ERC20Burnable, 
    ...
    WhitelistAdminRole 
{
    function setUniswapV2Router(address _uniswapV2Router) public onlyWhitelistAdmin {
        require(uniswapV2Router == address(0), "LeakToken::setUniswapV2Router: already set");
        uniswapV2Router = _uniswapV2Router;
    }

    function setUniswapV2Pair(address _uniswapV2Pair) public onlyWhitelistAdmin {
        require(uniswapV2Pair == address(0), "LeakToken::setUniswapV2Pair: already set");
        uniswapV2Pair = _uniswapV2Pair;
    }

    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public onlyWhitelistAdmin {
        if (_liquidityLockDivisor != 0) {
            require(_liquidityLockDivisor >= 10, "LeakToken::setLiquidityLockDivisor: too small");
        }
    ...
    function setRebalanceDivisor(uint256 _rebalanceDivisor) public onlyWhitelistAdmin {
        if (_rebalanceDivisor != 0) {
            require(_rebalanceDivisor >= 10, "LeakToken::setRebalanceDivisor: too small");
        }        
        rebalanceDivisor = _rebalanceDivisor;
    }
    
    function setXleakSwap(address _3) public onlyWhitelistAdmin {
      Xleakwine = _3;
    }
    ...
    function setCallerRewardDivisior(uint256 _rewardDivisor) public onlyWhitelistAdmin {
        if (_rewardDivisor != 0) {
            require(_rewardDivisor >= 10, "LeakToken::setCallerRewardDivisor: too small");
        }        
