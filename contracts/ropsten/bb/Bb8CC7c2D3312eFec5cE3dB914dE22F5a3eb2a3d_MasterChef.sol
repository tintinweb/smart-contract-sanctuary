...
// File contracts/libs/IBrewReferral.sol
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 1 BREW per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.1 BREW per block.
    ...
    function pendingBrew(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accBrewPerShare = accBrewPerShare.add(brewReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 brewReward = multiplier.mul(brewPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        brew.mint(devAddress, brewReward.div(10));
        brew.mint(address(this), brewReward);
        pool.accBrewPerShare = pool.accBrewPerShare.add(brewReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBrewPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(brew)) {
    ...
    function updateEmissionRate() public {
        ...
        require(brewPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = brewPerBlock;
        ...
        if (newEmissionRate >= brewPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = brewPerBlock;
        brewPerBlock = newEmissionRate;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(brewReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                brew.mint(referrer, commissionAmount);
