...
contract QubeLaunchPad is Ownable,Pausable,SignerManager{
    ...
    uint256 public instantRoi = 60;
    uint256 public installmentRoi = 40;
    uint256 public installmentMonths = 8;
    ...
    function installmentMonthsUpdate(uint256 _installmentMonths) public onlyOwner {
    ...
    function initICO(inputStore memory vars) public onlyOwner {
        ...
        IBEP20(vars.saleToken).safeTransferFrom(_msgSender(),address(this),vars.IdoAmount);
    ...
    function ico(uint256 id,uint256 amount,bytes memory signStore) public payable {
        ...
        if(!vars.signOff){
            singParams memory sign = signDecodeParams(signStore);
        ...
        require(vars.totalAmountOut <= vars.IdoAmount, "amount exceeds");

        vars.saleToken.safeTransfer(user,getAmountOut.mul(instantRoi).div(1e2));
        userLockIdInfo[user].add(internalLockTickets);
        userLockInfo[internalLockTickets] = userData({
            userAddress: user,
            saleToken: vars.saleToken,
            idoID: id,
            lockedAmount: getAmountOut.mul(installmentRoi).div(1e2),
            releasedAmount: 0,
            lockedDuration: block.timestamp,
            lastClaimed: 0,
            unlockCount: 0,
            installmentMonths: installmentMonths,
            distributeROI: uint256(1e3).div(installmentMonths)     
        });
        ...
        emit _ico(
            ...
            internalLockTickets,
            getAmountOut,
            getAmountOut.mul(instantRoi).div(1e2),
            getAmountOut.mul(installmentRoi).div(1e2),
            block.timestamp
        );
    }

    function claim(uint256 lockId) public whenNotPaused {
        ...
        uint256 reward = store.lockedAmount * (store.distributeROI) / (1e3);
        uint256 diff = block.timestamp - store.lockedDuration;
        uint256 duration_ = monthDuration;
        uint given = store.unlockCount;

        store.unlockCount = 0;
        
        for(uint8 i=1;i<=installmentMonths;i++){
            if(store.unlockCount == installmentMonths){
                break;
            }
            if(diff > duration_){
                store.unlockCount = store.unlockCount + 1;
                duration_ = duration_ + monthDuration;
            }else {
                break;
            }
        }
    ...
    function ReserveInfos(uint256 id) public view returns (dataStore memory) {
        dataStore memory vars = reserveInfo[id];
