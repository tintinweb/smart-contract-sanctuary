...
contract animalMain is  ERC721,Ownable {
  ...
  function transfer(address _to, uint256 _tokenId) public {
    ...
    //增加受贈者的擁有動物數量
    ownerAnimalCount[_to] = ownerAnimalCount[_to].add(2);
    //減少轉出者的擁有動物數量
    ownerAnimalCount[msg.sender] = ownerAnimalCount[msg.sender].sub(2);
    //動物所有權轉移
  ...
  function createAnimal() public payable{
      ...
      //使用亂數來產生DNA, 星級, 動物種類
      dna = keccak256(abi.encodePacked(msg.sender, owner));
      star = uint(dna)%5;
      roletype = uint(dna)% 10 + 1;
