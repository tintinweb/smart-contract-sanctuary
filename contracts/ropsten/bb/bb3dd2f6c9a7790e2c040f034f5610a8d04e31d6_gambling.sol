...
interface IERC20 {
 ...
 contract gambling {
    ...
    //Mapping para saber si aposto o no en un juego y evitar recorrer todos
    mapping(address=>mapping(uint=>bool)) public IsinTheGame;
    ...
    function BetGame(uint _gameId, uint _option, uint _amount) external {
        ...
        //Lo marco como que aposto en el juego seleccionado, si ya esta marcado no vuelvo a reescribir
        if (IsinTheGame[msg.sender][_gameId]==false) {
            IsinTheGame[msg.sender][_gameId]=true;
        }
    ...
    function GetMyTicketsInaGame(uint _gameId) external view returns(Ticket[] memory) {     
        uint j;      
        uint _tmpCounter;
        _tmpCounter = playerCounterBet[msg.sender];

        Ticket[] memory result = new Ticket[](_tmpCounter);
        for (uint i=0; i<_tmpCounter; i++) {
                if (result[i].gameId==_gameId) {
                    result[j] = tickets[msg.sender][i];
                    j++;
                }
        }
    ...
    function GetMyTickets(address _address) external view returns(uint[] memory, uint[] memory) {     
        uint[] memory returnA;
        uint[] memory returnB;
        for (uint i=0; i<playerCounterBet[msg.sender]; i++) {
           if (tickets[msg.sender][i].generatedBy==_address) {
               returnA[i]=tickets[msg.sender][i].gameId;
               returnB[i]=i;
           }
                
        }
        //Ticket[] memory result = new Ticket[](counter.length);
