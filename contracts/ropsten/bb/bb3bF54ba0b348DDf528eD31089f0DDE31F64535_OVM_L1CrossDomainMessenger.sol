        ...
        ReentrancyGuardUpgradeable
{
    ...
    {
        return (
            _verifyStateRootProof(_proof, _ovmTransaction, _receipt)
        ...
        )
    {
        ...
        iOVM_ExecutionManager ovmExecutionManager = iOVM_ExecutionManager(
            resolve("OVM_ExecutionManager")
        );

        bytes32 ovmTxHash = Lib_OVMCodec.hashTransaction(_ovmTransaction);

        require (
            ovmTxHash == _receipt.ovmTransactionHash,
            "Hash of OVM transaction has to be same as the one provided by receipt"
        );

        return (
            //instead verify the operator signature 
            
            ovmStateCommitmentChain.insideFraudProofWindow(_ovmTransaction) == false
