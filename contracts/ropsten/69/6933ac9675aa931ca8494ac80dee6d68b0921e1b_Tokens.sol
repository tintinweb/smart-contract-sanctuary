...
contract Tokens {
    ...
    function tokenMetadata(uint256 _id) public view returns (string, string, string, string, string, string) {
        return (tokens[_id].name, tokens[_id].for_sale, tokens[_id].allow_breeding, tokens[_id].location, tokens[_id].sale_price, tokens[_id].breeding_fee);
    }
    
    function createtoken(string _name, string _for_sale, string _allow_breeding, string _location, string _sale_price, string _breeding_fee) public returns (bool success) {
        tokens[id] = token(_name, _for_sale, _allow_breeding, _location, _sale_price, _breeding_fee);
