...
contract Staker is Ownable {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount; // How many rewards tokens the user has provided.
    ...
    // Info of each pool.
    struct PoolInfo {
        IBEP20 stakingToken; // Address of StakingToken.
        IBEP20 rewardToken; // Address of rewardToken
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = 
                user.amount.mul(pool.accStakerPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function safeRewardTransfer(IBEP20 rewardToken, address _to, uint256 _amount) internal {
