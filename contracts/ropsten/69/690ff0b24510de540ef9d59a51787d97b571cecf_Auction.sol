...
contract Auction is Ownable {
    ...
    event MakeBid(address, TYPES, uint8);
    event CloseAuction(Lot[], Lot[], Lot[]);
    ...
    function makeBid(TYPES _type, uint8 id) public {
        for(uint16 i=0; i<highAuction.length; ++i) {
            require(highAuction[i].buyer != _msgSender(), 'You are already participating in this auction');
        }
      
        for(uint16 i=0; i<middleAuction.length; ++i) {
            require(middleAuction[i].buyer != _msgSender(), 'You are already participating in this auction');
        }
        
        for(uint16 i=0; i<lowAuction.length; ++i) {
            require(lowAuction[i].buyer != _msgSender(), 'You are already participating in this auction');
        }
        ...
        emit MakeBid(_msgSender(), _type, id);
    }
