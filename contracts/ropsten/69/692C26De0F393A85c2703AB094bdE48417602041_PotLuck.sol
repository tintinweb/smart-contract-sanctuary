...
contract PotLuck {
    ...
    function random() public view returns(uint256){
         return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, addresses)));
    }
    ...
    function getWinners() public view returns (uint256, uint256, uint256, uint256) {
        uint winner1Index = random().mod(players.length);
        uint winner2Index = random().mod(players.length);
