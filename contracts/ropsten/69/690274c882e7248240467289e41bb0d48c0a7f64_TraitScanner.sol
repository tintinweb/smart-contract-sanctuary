...
contract TraitScanner is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Box Labs TraitScanner", "BLTS") {
        subscriptionPlans.push(Subscription(basePrice * 3, 15));
        subscriptionPlans.push(Subscription(basePrice * 5, 30));
        subscriptionPlans.push(Subscription(basePrice * 8, 60));
    }

    function register(uint _id) public payable {
        require(msg.value == getPlanPrice(_id), "Incorrect ETH value");
        require(getActiveSubscribersCount() < maxUsers, "Max users reached");
        require(subscriptionPlans[_id]._days > 0, "Subscription plan does not exist");
        require(userToTimeRegistered[msg.sender] == 0, "User already registered");
        require(whitelisted[msg.sender] == false, "User already whitelisted");
    ...
    function updateSubscription(uint _id) public payable{
        ...
        require(getActiveSubscribersCount() < maxUsers || expireTime > timestamp, "Max users reached");
    ...
    function getActiveSubscribersCount() public view returns(uint) {
        uint activeSubCount;
        uint256 timestamp = block.timestamp;
    ...
    function getAllActiveSubscribers() external view returns (address[] memory) {
        uint count = getActiveSubscribersCount();
    ...
    function getPlanPrice(uint _id) public view returns(uint256) {
        return subscriptionPlans[_id]._price;
    }

    function setPlanPrice(uint _planId, uint256 _price) public onlyOwner {
    ...
    function removeTrial(address _address) external onlyOwner {
        trialToTime[_address] = 0;
    }
