...
contract BulkSender is Ownable {
     ...
     * @param amounts - uint256[]
     */
    function distributeTokens(address[] calldata addresses, uint256[] calldata amounts) onlyWhiteListed external payable {
        _checkValues(addresses, amounts);
        for (uint256 i; i < addresses.length; i++) {
            BaseERC20(tokenAddress).transfer(addresses[i], amounts[i]);
        }
    }
     ...
     */
    function _checkValues(address[] calldata addresses, uint256[] calldata amounts) internal returns (uint256) {
