...
function pluckEnnemies(uint256 ennemyId, string memory keyPrefix, string[] memory sourceArray) internal view returns (string memory) {
    uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(ennemyId))));
    string memory output = string(abi.encodePacked(quantity[rand % quantity.length], " ", sourceArray[rand % sourceArray.length], " ", level[rand % level.length]));
    uint256 greatness = rand % 21;
    if (greatness > 18) {
        output = string(abi.encodePacked(prefixes[rand % prefixes.length], " ", output));
    }
    ...
    //min score mob : 1, max: 100. Rarity (has a prefix) = 2x stronger mobs
    ...
    function tokenURI(uint256 ennemyId, uint256 lootId) override public view returns (string memory) {
        ...
        parts[1] = '';
        parts[2] = getEnnemyRarity(ennemyId);
