...
contract CrimeCashIOFarm is Ownable {
  ...
  event eventDepositBank(address _from, address _to, uint256 _amount);
  event eventClaim(address claimer, uint256 _poolIndex, uint8 payout_mode, uint256 _amount);
  event eventWithdrawBank(address _from, address _to, uint256 _amount);
  event eventLockedCrimecash(uint256 lockedCrimeCash);
  ...
  function addPool(uint256 _poolIndex, address _lpToken) public onlyOwner {
  ...
  function getPool(uint256 _poolIndex) public view returns(uint256 _totalSupply, uint256 _max_players ) {
  ...
  function claim(uint256 _poolIndex, uint8 _payout_mode, uint256 _amount) public onlyCrimer {
    ...
    require(_amount>0, "Error: amount should be greater than zero.");
    crimecashToken.mint(address(this), _amount);
    
    emit eventClaim(_msgSender(), _poolIndex, _payout_mode, _amount);
  }
  function isExistsPlayerInPool(uint256 _poolIndex, address _player) internal view returns (bool) {
  ...
  function withdraw(uint256 amount) public onlyCrimer {
    require(isRoundOpening == true, "Error: There is not an opened round currently.");
    require(crimecashToken.balanceOf(address(this))>0, "Error: admin balance insufficient");
    uint256 _amount;
    if ( crimecashToken.balanceOf(address(this)) < amount )
      _amount = crimecashToken.balanceOf(address(this));
    else
      _amount = amount;
      
    crimecashToken.transfer(address(_msgSender()), _amount);
    
    emit eventWithdrawBank(address(this), _msgSender(), _amount);
  }
  
  function distributeCrimeGold(address _crimer, uint256 amount) public onlyOwner returns (bool) {
    require(isRoundOpening == true, "Error: There is not an opened round currently.");
    require(crimegoldToken.balanceOf(address(this))>0, "Error: admin balance insufficient");
    uint256 _amount;
  ...
  function buyCcash(uint256 amount_) public onlyCrimer payable {
    require(isRoundOpening == true, "Error: There is not an opened round currently.");
    require(amount_>0, "zero amount doesn't allow to mine");
    if (msg.value > 0) {
      crimecashToken.mint(_msgSender(), amount_);
