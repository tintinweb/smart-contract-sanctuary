...
contract Lean_Hog {
    ...
    string public constant name = "Lean Hog Coin";
    string public constant symbol = "LEANHOG";
    ...
    /*
    uint256 totalSupply_ = 10 ** (7 + 18);
	uint256 preMine_ =  1 * (10 ** (7 + 18));
	uint airdropSize_ = 1 ** (6 + 18);
   ...
   constructor() public {  
	...
	function LaunchToken() external {
		...
		require(preMine_ <= balances[address(this)]);
		
		this.transfer(owner_, preMine_);
	}
