...
contract Staking is Initializable, OwnableUpgradeable {
    ...
    /**
     * @notice Sets the rewardToken per block to be distributed. Can only be called by the owner.
     * @dev Its decimals count is ACC_REWARD_PRECISION
     * @param _rewardPerBlock The amount of rewardToken to be distributed per second.
     */
    function setRewardPerBlock(uint256 _rewardPerBlock) public onlyOwner {
        require(_rewardPerBlock >= 0.5 ether, "Check minimum rewardToken per block");
        updatePool();
        ...
        returns (uint256)
    {
        ...
        if (block.number > lastRewardBlock && totalStaking != 0) {
    ...
    function updatePool() public {
        if (block.number > lastRewardBlock) {
            ...
            lastRewardBlock = block.number;
            emit LogUpdatePool(lastRewardBlock, totalStaking, accRewardPerShare);
        }
    }
