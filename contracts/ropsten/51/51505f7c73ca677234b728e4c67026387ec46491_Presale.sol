...
contract CommonTokensale is MultiOwnable, Pausable {
    ...
    function() public payable {
        ...
        uint valueUSD = rateUSDETH.mul(_amountWei);
    ...
    function weiToTokens(uint _amountWei) public view returns (uint) {
        ...
        uint ethers = _amountWei / 10e18;

        uint valueUSD = rateUSDETH.mul(ethers);
        ...
        uint total = valueUSD.add(valueUSD.mul(bono) / 100);

        uint tokens = total.mul(tokensPerUSD());
