...
contract Numbers is ERC721Enumerable, NumbersDNAFactory, Ownable {
 ...
 function claimAll(address _owner, uint _salt) public {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      if (tokenIds[i] != -1) {
        claim(uint256(tokenIds[i]), 1, _salt);
      }
    }
  }

  function claim(uint256 _tokenId, uint256 _numnumbers, uint _salt) public {
...
//regular sale
 function mint(uint256 _numnumbers, uint _salt) public payable {
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    return string(abi.encodePacked(_baseURI(), "/", _tokenId,"-", numbers[_tokenId],".json"));
  }   
