...
contract ethBuying is Context, Ownable {
    ...
    constructor(address _gftAddress) {
        _GFT = _gftAddress;
    ...
    function remainingGFT() public view returns (uint256) {
        return _balance;
    }
    ...
    function buyUSDT(uint256 _amount) public virtual returns (bool) {
        ...
        uint256 _gftAmount = getUSDPrice(_amount*1e12);
        _buy(_USDT, _amount, _gftAmount);
        return true;
    }
    ...
    function buyUSDC(uint256 _amount) public virtual returns (bool) {
        require(_started == true, "Buying not yet started.");
        uint256 _gftAmount = getUSDPrice(_amount*1e12);
        _buy(_USDC, _amount, _gftAmount);
        return true;
    }

    function buyLINK(uint256 _amount) public virtual returns (bool) {
        require(_started == true, "Buying not yet started.");
        uint256 _gftAmount = getLINKPrice(_amount);
        _buy(_LINK, _amount, _gftAmount);
        return true;
    }
    ...
    function buyETH() public payable returns (bool) {
        ...
        uint256 _gftAmount = getETHPrice(_amount);
        require(_gftAmount <= _balance, "Not enough GFT in contract.");
        IERC20(_GFT).transfer(_msgSender(), _gftAmount);
        _balance = _balance.sub(_gftAmount);
        return true;
    }
