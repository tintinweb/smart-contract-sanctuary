...
uint256 constant APY_TOKENS = 1000 * DECIMALS;
...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    uint256 public constant ANNUAL_COVERAGE_TOKENS = MINUMUM_COVERAGE;
    ...
    uint256 public constant PREMIUM_DISTRIBUTION_EPOCH = 1 hours;
    ...
    uint256 public constant override EPOCH_DURATION = 600;
    uint256 public constant MAXIMUM_EPOCHS = SECONDS_IN_THE_YEAR / EPOCH_DURATION;
    uint256 public constant VIRTUAL_EPOCHS = 2;

    uint256 public constant WITHDRAWAL_PERIOD = 600;
    uint256 public constant override READY_TO_WITHDRAW_PERIOD = 300;
    ...
    function _submitClaimAndInitializeVoting(address claimer, bool appeal) internal {
        ...
        uint256 virtualEndEpochNumber = policyHolders[claimer].endEpochNumber + VIRTUAL_EPOCHS;

        /// @dev "lock" claim and appeal tokens
        if (!appeal) {
            epochAmounts[virtualEndEpochNumber] = epochAmounts[virtualEndEpochNumber].sub(cover);
        } else {
            uint256 claimIndex = claimingRegistry.claimIndex(claimer, address(this));
        ...
        claimVoting.initializeVoting(
            ...
            address(this),
            cover,
            policyHolders[claimer].paid.mul(PROTOCOL_PERCENTAGE).div(PERCENTAGE_100),
            appeal
        );
    }
    ...
    function commitClaim(
        ...
        if (status == IClaimingRegistry.ClaimStatus.ACCEPTED) {
            totalCoverTokens = totalCoverTokens.sub(claimAmount);
    ...
    function _updateBMIDAIStakingReward() internal {
        uint256 rewardMultiplier = MINIMUM_REWARD;
    ...
    function _buyPolicy(
        ...
        emit PolicyBought(_buyer, _coverTokens, _price, _totalCoverTokens);
    ...
    function updateEpochsInfo() public override {
        uint256 _newDistributionEpoch = Math.min(getEpoch(block.timestamp), MAXIMUM_EPOCHS);
    ...
    function addLiquidityAndStake(uint256 _liquidityAmount, uint256 _bmiDAIxAmount)
        external
        override
    {
    ...
    /// @dev _annualProfitYields is multiplied by 10**5
...
interface ILiquidityMining {
    ...
    /// @notice checks if liquidity mining event is started (startLiquidityMining() has been called)
    /// @return true if LM is started, else false
    function isLMStarted() external view returns (bool);
