...
contract ProofOfStakeMineableToken {
    ...
    function continousMine() internal returns (bool);
    event Mine(address indexed _address, uint _tokensMining);
    event ContinousMine(address indexed _address, uint _tokensMining);
}
...
contract MintBlockV3 is ERC20, ProofOfStakeMineableToken, Ownable {
    using SafeMath for uint256;

    string public name = "MintBlockV3";
    string public symbol = "MIBL3";
    ...
    uint public tokensPerBlock = 10 * (10**decimals);
...
//------------------------------------------------------------------------------
    ...
    function mine() PoSMiner public returns (bool) {
        ...
        continousMine(); return true;
    }
    
    function continousMine() PoSMiner internal returns (bool) {
        require(balances[msg.sender] >= minTokenHold);
        if(balances[msg.sender] < minTokenHold) return false;
        ...
        emit ContinousMine(msg.sender, tokensMining);
        emit Transfer(address(0), msg.sender, tokensMining);
