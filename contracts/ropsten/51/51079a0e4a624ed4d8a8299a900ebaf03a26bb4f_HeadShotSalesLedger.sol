...
contract HeadShotSalesLedger is ERC20, Ownable {
    ...
    */
    ...
    function addSalesInfo(uint256 category_, uint256 code_, string memory name_,
        string memory desc_, uint256 price_, uint256 maxBuy_) external onlyOwner returns (bool) {
    ...
    function getTrackerFieldString(uint256 code_, string memory key_) external view onlyOwner returns (string memory) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.getFieldString(key_);
    }

    function getTrackerFieldNumber(uint256 code_, string memory key_) external view onlyOwner returns (uint256) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.getFieldNumber(key_);
    }

    function getAccountBalance(uint256 code_, address account_) external view onlyOwner returns (uint256) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.balanceOf(account_);
    }

    function getTrackerFieldAddress(uint256 code_, string memory key_) external view onlyOwner returns (address) {
        HeadShotTracker headShotTracker =
        HeadShotTracker(payable(getTrackerAddress(code_)));
        return headShotTracker.getFieldAddress(key_);
    }

    function buy(address account_, uint256 code_, uint256 count_) external onlyOwner returns (bool) {
    ...
    function spend(address account_, uint256 code_, uint256 count_) external onlyOwner returns (bool) {
        if (getSalesCode(code_) == code_){
            HeadShotTracker headShotTracker =
            HeadShotTracker(payable(getTrackerAddress(code_)));
    ...
    function getSalesCode(uint256 code_) internal view returns (uint256) {
        return _salesMap[code_].code;
    }

    function getSalesPrice(uint256 code_) internal view returns (uint256) {
        return _salesMap[code_].price;
    }

    function getTrackerAddress(uint256 code_) internal view returns (address) {
        return _trackerMap[code_];
    }

    function addTrackerInfo(uint256 code_, address trackerAddress_) internal returns (bool) {
