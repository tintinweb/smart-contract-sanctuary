...
contract SN_PRE is Context, Ownable {
    ...
    // -- presale --
    function setPriceAndCap(uint256 _individualBnbCap, uint256 _nbOfTokenPerBNB, uint256 _priceHike, uint256 _preSaleDelay, uint256 _minPurchaseBNB, uint256 _maxTotalAllocationBNB) external onlyOwner beforeSale {
        individualBnbCap = _individualBnbCap; //use 18 decimals
        nbOfTokenPerBNB = _nbOfTokenPerBNB; //use 9 decimals
        priceHike = _priceHike; //use %
        preSaleDelay = _preSaleDelay; //use seconds!
    ...
    function setTotalAllocationBNB(uint256 _newMaxTotalAllocationBNB) external onlyOwner {
        maxTotalAllocationBNB = _newMaxTotalAllocationBNB;
    }

    function addToWhiteList(address[] calldata whiteListedAddresses) external onlyOwner {
    ...
    function startPreSale() external onlyOwner {
         ...
         prePreSaleStartedOn = block.timestamp;
    }
    ...
    function buy() external payable duringSale {
         require(whiteListed[msg.sender] == true || block.timestamp >= (prePreSaleStartedOn + preSaleDelay), "You are not whitelisted, the whitelisted presale is still ongoing!");
    ...
    function _buy(address sender, uint256 amountBNB) internal {
         ...
         uint256 _nbOfTokenPerBNB = (whiteListed[msg.sender] == true && block.timestamp < (prePreSaleStartedOn + preSaleDelay)) ? nbOfTokenPerBNB : nbOfTokenPerBNB.mul((100-priceHike)).div(100);
         uint256 amountToken = amountBNB.mul(_nbOfTokenPerBNB);
    ...
    function getAmountOfTokens(address addressToCheck, uint256 inputBnbInWei) public view returns(uint256) {
        uint256 _nbOfTokenPerBNB = (whiteListed[addressToCheck] == true && block.timestamp < (prePreSaleStartedOn + preSaleDelay)) ? nbOfTokenPerBNB : nbOfTokenPerBNB.mul((100-priceHike)).div(100);
        return inputBnbInWei.mul(_nbOfTokenPerBNB);
    }   

    function isWhiteListed(address addressToCheck) public view returns(bool) {
        return whiteListed[addressToCheck];
    }
