...
contract OfmERC20 is IERC20, AccessControl {
   ...
   *
   * @dev ERC20 `function _maxTotalSupply() public view returns (uint256)`
   *
   * @dev Field is declared public: getter _maxTotalSupply() is created when compiled,
   *      it returns the maximum amount of tokens that can be minted.
   */
  uint256 public constant _maxTotalSupply = 50_000_000e18; // is set to 50 million * 10^18
  ...
  function mint(address _to, uint256 _value) public {
    ...
    require(_totalSupply + _value <= _maxTotalSupply, "max total supply set to 50 million");
