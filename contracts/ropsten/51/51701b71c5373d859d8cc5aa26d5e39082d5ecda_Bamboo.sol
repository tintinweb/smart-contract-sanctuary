...
contract Bamboo is ERC20("Bamboo Token", "$Bamboo"), Ownable {
    ...
    function _calcCliamableAmount(address _holder, uint256 _tokenId) internal view returns(uint256){
         if (_checkCliamTime(_tokenId)){
            uint256 howManyDaysAccumulated = _getAccmulatedDays(_tokenId, _holder);
            uint256 claimAmount = howManyDaysAccumulated * cliamUnitPerPeriod * (uint(10)**decimals());
            return claimAmount;
    ...
    function getHolderCliamableAmountForTokenId(address _holder, uint256 _tokenId) public view returns(uint256) {
		if (now > cliamEndTime) return 0;
		if (!_checkHolder(msg.sender)) return 0;
        uint amount = _calcCliamableAmount(_holder, _tokenId);
        return amount; 
	}
	
    function getHolderCliamableTotalAmount(address _holder) public view returns(uint256) {
		if (now > cliamEndTime) return 0;
		if (!_checkHolder(msg.sender)) return 0;
		  uint256 tokenCount = PandaNFTContract.balanceOf(msg.sender);
        ...
        for (uint256 i=0; i < tokenCount; i++){
           uint256 tokenId = PandaNFTContract.tokenOfOwnerByIndex(msg.sender, i);
           uint amount = _calcCliamableAmount(_holder, tokenId);
