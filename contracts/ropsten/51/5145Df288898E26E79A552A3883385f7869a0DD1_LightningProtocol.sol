...
contract LightningProtocol is Context, IBEP20, Ownable {
    ...
    event HoldersRefundFromBurn(address indexed from, address indexed to, uint256 previousBalance, uint256 totalBalance, uint256 value);
    ...
    function _redistributeBurned() private{
       for (uint256 i = 0; i < _holders.length; i++) {
           uint256 previousBalance = tokenFromReflection(_rOwned[_holders[i]]);
            ...
            emit HoldersRefundFromBurn(owner(), _holders[i], previousBalance, tHolderAmount, tAmountToRedistribute);
        }
    }
