...
contract CrimerInfo is Ownable {
  ...
  function crimerList(address[] memory _crimers, address stealer) public view returns(address[] memory crimer, uint256[] memory cash, uint256[] memory power, uint256[] memory protectedUntil, uint256[] memory totalStolenCount) {
    for(uint8 i=0;i<_crimers.length;i++) {
      crimer[i] = _crimers[i];
