...
contract ViralataRouter02 is IViralataRouter02 {
    ...
    constructor(address _factory, address _WETH, address _AURO) public {
        factory = _factory;
        WETH = _WETH;
        AURO = _AURO;
    }
    ...
    function addLiquidityETH(
        ...
        if (address(token) == address(AURO)) {
            IAURO _auro = IAURO(AURO);
            bytes32 feeRole = _auro.getFeeRole();
            _auro.grantRole(feeRole, pair);
        }
...
interface IAURO {
    ...
    function getFeeRole() external view returns (bytes32);
}
...
library ViralataLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'a7e548e5d673518547bd63e2dbd4831808ffe82bc9419adc14a194fd4b7d8fd2' // init code hash
            ))));
    }
