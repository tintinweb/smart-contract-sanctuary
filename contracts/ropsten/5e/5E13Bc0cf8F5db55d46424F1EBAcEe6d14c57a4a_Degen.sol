...
abstract contract Router {
    ...
    address o3SwapUniFactory = address(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);   
    ...
    constructor(address _Uni, address _sushi, address _oneSplit,/* address _xToken, address _airSwap,*/ address _paraSwap, address _o3Swap) public payable {
        // owner = payable(msg.sender);
        OneSplit = I1inch(_oneSplit);
        Uni = IUni(_Uni);
        Sushi = IUni(_sushi);
        
        O3Swap = IO3Swap(_o3Swap);
    ...
    function calculateO3SwapReturn( uint256 amountIn, address[] memory path, OrderType orderType,uint256 /*disableFlags*/) public view returns(uint256, uint256[] memory) {
        uint256[] memory o3Amounts =new uint[](path.length);
        uint256[] memory distribution;

        o3Amounts[path.length-1] = uint256(0);
        ...
        o3Amounts = UniswapV2Library.getAmountsOut(o3SwapUniFactory, amountIn, path);
                
        return (o3Amounts[path.length-1],distribution);
    }
...
contract Degen is Router, Ownable {
    ...
    address _o3Swap = address(0x96cFA408CA039d9Afea0b8227be741Ef52e8a037);   
    // https://etherscan.io/address/0x96cFA408CA039d9Afea0b8227be741Ef52e8a037#code
   ...
   modifier onlySystem() {
    ...
    Router( _Uni, _sushi, _oneSplit, _paraSwap, _o3Swap) public {
    ...
    function processFee(uint256 txGas, uint256 feeAmount, address licenseeVault, address user) internal {
        ...
        uint[] memory amounts = Uni.swapExactETHForTokens{value: feeAmount/2}(
            0,
            path,
            address(this),
            block.timestamp
        ...
        Uni.addLiquidityETH{value: feeAmount/2}(
            path[1],
            amounts[1],
            0,
            0,
