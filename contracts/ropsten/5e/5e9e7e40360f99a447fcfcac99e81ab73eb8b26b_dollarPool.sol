...
contract dollarPool is ReentrancyGuard, Ownable {
    ...
    {
        return (
            dollarTotalSupply(), // dollar totalSupply
    ...
    /* ========== PUBLIC FUNCTIONS ========== */
    function dollarTotalSupply() public view returns (uint256 _dollar_totalSupply) {
        _dollar_totalSupply = IERC20(dollar).totalSupply();
    }
    ...
    function redeem(
        ...
        uint256 _dollar_totalSupply = dollarTotalSupply();
        ...
        if (ecr < COLLATERAL_RATIO_MAX) {
            ...
            uint256 _share_output_value = _dollar_amount_pre_fee.sub(
                getProductOf(_dollar_amount_pre_fee, ecr)
            );
            uint256 _share_output_value_with_profit = getProductOf(_share_output_value,rmp);

            uint256 _share_output_amount_by_rmp = getRatioOf(
                _share_output_value_with_profit,
                _share_price
            );
            uint256 _share_output_amount_by_wam = getProductOf(
                _total_share_balance,
                _share_return_ratio
            );

            uint256 _share_output_amount_pre_fee = 0;
            if (_share_output_amount_by_rmp < _share_output_amount_by_wam) {
                _share_output_amount_pre_fee = _share_output_amount_by_rmp;
            } else {
                _share_output_amount_pre_fee = _share_output_amount_by_wam;
            }
    ...
    function setRedeemMaxProfit(uint256 _rmp) public onlyOwner {
        require(_rmp >= PRICE_PRECISION, "< PRICE_PRECISION");
        rmp = _rmp;
    }
