...
library Minting {
    ...
    {
        string[] memory tokenBlueprintParts =
            String.split(string(blueprint), ",");
        require(
            tokenBlueprintParts.length == 2,
            "Invalid token blueprint count"
        );
        string memory name = tokenBlueprintParts[0];
        uint256 issueNumber = uint256(String.toUint(tokenBlueprintParts[1]));
...
contract VeveToken is ERC721, ERC721Pausable, ERC721Enumerable, AccessControl {
    ...
    function isVeveToken() public pure returns (bool) {
        return true;
    }
    ...
    function setImx(address _imx) public onlyRole(ADMIN_ROLE) {
        imx = _imx;
        emit UpdatedImx(imx);
    }

    function enableImx() public whenImxDisabled onlyRole(ADMIN_ROLE) {
        imxEnabled = true;
        emit EnabledImx(_msgSender());
    }

    function disableImx() public whenImxEnabled onlyRole(ADMIN_ROLE) {
        imxEnabled = false;
    ...
    function _mint(
        ...
        (string memory name, uint256 issueNumber) =
            Minting.deserializeBlueprint(blueprint);
        require(
    ...
    function burn(uint256 tokenId) public onlyRole(BURNER_ROLE) {
        ...
        bytes memory tokenBlueprint =
            bytes(abi.encodePacked(name, ",", Strings.toString(issueNumber)));
        require(
