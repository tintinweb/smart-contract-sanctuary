...
contract LotteryTest {
    ...
    function placeBet(uint _type, uint _hash, uint _periods, int _tik) public payable returns (bool) {
        int _wei = int(msg.value);
        ...
        bets[msg.sender] = Bet({value: uint192(msg.value), betType: uint8(betType), betHash: uint8(bethash), blockNum: uint8(blockNum)});
        emit LogBet(msg.sender,uint8(betType),uint8(bethash),uint(blockNum),msg.value,bool(betclose));
        return true;
    }
