...
library Base64 {
  ...
  using BytesUtils for bytes1;

  bytes32 constant charsLower = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdef";
  bytes32 constant charsUpper = "ghijklmnopqrstuvwxyz0123456789+/";

  function encode(bytes memory _data) internal pure returns(bytes memory) {
    bytes memory _encoded = "";

    for (uint256 index = 0; index < _data.length; index += 3) {
      uint8 _val = uint8(_data.getValueAt(index) >> 2);
      _encoded = bytes.concat(_encoded, _encode6bits(_val));

      _val = uint8((_data.getValueAt(index) & 0x03) << 4 | _data.getValueAt(index + 1) >> 4);
      _encoded = bytes.concat(_encoded, _encode6bits(_val));

      if (index + 1 >= _data.length) {
        _val = 0xff;
      } else {
        _val = uint8((_data.getValueAt(index + 1) & 0x0f) << 2 | (_data.getValueAt(index + 2) & 0xc0) >> 6);
      }
      ...
      if (index + 2 >= _data.length) {
        ...
        _val = uint8(_data.getValueAt(index + 2) & 0x3f);
      }
  ...
  function _encode6bits(uint8 val) private pure returns (bytes1) {
    if (val < 32) {
      return charsLower[val];
    } else if(val < 64) {
      return charsUpper[val - 32];
    } else {
      return "=";
    }
  }
...
library BytesUtils {
  function repeat(bytes32 _chars, uint256 _times) internal pure returns(bytes memory) {
    bytes memory _repeated = "";

    for (uint256 index = 0; index < _times; index++) {
      _repeated = bytes.concat(_repeated, _chars);
    }

    return _repeated;
  }

  function getValueAt(bytes memory _data, uint256 _index) internal pure returns(uint8) {
    if (_index >= _data.length) {
      return 0;
    }

    uint8 _val = uint8(_data[_index]);
    return _val;
  }
}