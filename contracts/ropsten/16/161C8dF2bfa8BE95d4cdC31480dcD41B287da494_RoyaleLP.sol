...
contract RoyaleLP is RoyaleLPstorage,ReentrancyGuard {
    ...
    function checkWithdraw(uint256 amount,uint256 burnAmt,uint _index) internal{
        ...
        poolBalance =poolBalance.sub(reserveAmount[_index]);
    ...
    function _takeBackQ(uint256 amount,uint256 _burnAmount,uint256 _index) internal {
        ...
        if(!isInQ[msg.sender]) {
            ...
            withdrawRecipients[recipientCount]=msg.sender;
            recipientCount += 1;
        }
    ...
    function claimTokens() external nonReentrant{
        ...
        for(uint8 i=0; i<3; i++) {
            if(amountWithdraw[msg.sender][i] > 0) {
                ...
                updateLockedRPT(msg.sender,amountBurnt[msg.sender][i],i);
    ...
    function deposit() onlyWallet() external  {
        ...
        for(uint8 i=0; i<3; i++) {
            ...
            if(amounts[i] > threshold.mul(10**decimal)) {
                amounts[i] =((amounts[i].add(YieldPoolBalance[i]).sub(reserveAmount[i])).mul(DENOMINATOR.sub(poolPart)).div(DENOMINATOR)).sub(YieldPoolBalance[i]);
    ...
    function withdraw() onlyWallet() external  {
        ...
        for(uint8 i=0;i<3;i++){
            uint poolAmount;
            amounts[i]=getBalances(i);
            poolAmount=((amounts[i].add(YieldPoolBalance[i]).sub(reserveAmount[i])).mul(poolPart).div(DENOMINATOR));
            if(poolAmount>=amounts[i]){
                amounts[i] =totalWithdraw[i].add(poolAmount.sub(amounts[i]));
            }
            else{
                poolAmount=amounts[i].sub(poolAmount);
                if(totalWithdraw[i]!=0 && totalWithdraw[i]>=poolAmount){
                    amounts[i] =totalWithdraw[i].sub(poolAmount);
                }
            }
    ...
    function rebalance() onlyWallet() external {
        ...
        for(uint8 i=0;i<3;i++) {
           ...
           currentAmount=currentAmount.sub(reserveAmount[i]);
           decimal=tokens[i].decimals();
