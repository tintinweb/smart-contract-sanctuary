...
contract MysticBets is ERC721 {
  ...
  // check if token description exists
  mapping(string => bool) public tokenDescriptionExists;
  ...
  function mintMysticBets(string memory _name, string memory _description, string memory _tokenURI, address payable _feeRecipient, uint256 _sellerFeeBasisPoints) external {
    ...
    // check if the token description already exists or not
    require(!tokenDescriptionExists[_name]);

    // mint the token
    _mint(msg.sender, mysticBetsCounter);
    // set token URI (bind token id with the passed in token URI)
    _setTokenURI(mysticBetsCounter, _tokenURI);

    // make passed token URI as exists
    tokenURIExists[_tokenURI] = true;
    // make token name passed as exists
    tokenNameExists[_name] = true;
    // make token description passed as exists
    tokenNameExists[_description] = true;
