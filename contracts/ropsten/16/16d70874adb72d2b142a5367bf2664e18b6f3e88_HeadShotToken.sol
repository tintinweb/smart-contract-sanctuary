...
# Tokenomic:
  ...
  - 8% Tax:
    * reward fee: 2%
    * ecosystem fee: 4% (development and marketing)
    * liquidity fee: 2%
*/
...
contract HeadShotToken is Context, IERC20, Ownable {
    ...
    uint256 private _ecosystemFee = 4;
    ...
    function initialBurn() private {
        uint256 _burnToken = 10 * (10 ** 15) * (10 ** _decimals);
        _transfer(owner(), _burnAddress, _burnToken);
    }
    ...
    function walletEcosystem() public view returns (address) {
    ...
    function feeReward() public view returns (uint256) {
        return _rewardFee;
    }

    function feeEcosystem() public view returns (uint256) {
        return _ecosystemFee;
    }

    function feeLiquidity() public view returns (uint256) {
        return _liquidityFee;
    }

    function amountMaxTx() public view returns (uint256) {
        return _maxTxAmount;
    }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tEcosystem) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _takeEcosystem(tEcosystem);
    ...
    function setWalletEcosystem(address payable newWallet_) public virtual onlyOwner {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tEcosystem) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tEcosystem, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tEcosystem);
    }
    ...
    */
    ...
    function transferFromContract(address sender, uint256 amount) public returns (bool) {
        uint256 totalFeePercent = _ecosystemFee.add(_liquidityFee).add(_rewardFee);
        uint256 totalFeeAmount = amount.mul(totalFeePercent).div(10 ** 2);
        uint256 totalAmount = amount.add(totalFeeAmount);
        _transfer(sender, _salesWalletAddress, totalAmount);
        return true;
    }
}