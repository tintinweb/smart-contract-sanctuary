...
contract Creact_Token is SafeMath, Owner {
    ...
    function initialization() public {
        ...
        emit TransferToken(address(0), msg.sender, _totalSupply);
    }
    ...
    function approve(address spender, uint tokens) public returns (bool success) {
        ...
        emit ApprovalToken(msg.sender, spender, tokens);
        return true;
    }
    ...
    function transferToken(address to, uint tokens) public returns (bool success) {
        ...
        emit TransferToken(msg.sender, to, tokens);
        return true;
    }

    function transferFromToken(address from, address to, uint tokens) public returns (bool success) {
        balances[from] = safeSub(balances[from], tokens);
        allowed[from][msg.sender] = safeSub(allowed[from][msg.sender], tokens);
        balances[to] = safeAdd(balances[to], tokens);
        emit TransferToken(from, to, tokens);
        return true;
    }
}
...
// the end
    ...
    function contributeSuccess() public statusTrue {
        ...
        for(uint64 i=0; i<member.length; i++)
        {
            balances[member[i].addressMember]=100000;
            balances[member[i].addressMember]=division(_totalSupply*(member[i].amount/(10**15)),getBalance()/(10**15));
        }
    }
