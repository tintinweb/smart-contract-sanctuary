 ...
 * Tokens for sale are generated during setSale function. Contract uses vesting mixin.
 */
    ...
    DiscoperiTokenVesting
{
    ...
    uint256 public constant INVESTORS_SUPPLY = 50000000000 * (10 ** uint256(decimals)); // 50,000,000,000 DISC - 25%
    uint256 public constant TEAM_SUPPLY = 30000000000 * (10 ** uint256(decimals)); // 30,000,000,000 DISC - 15%
    uint256 public constant RESERVE_SUPPLY = 22000000000 * (10 ** uint256(decimals)); // 22,000,000,000 DISC - 11%
    uint256 public constant MARKET_DEV_SUPPLY = 20000000000 * (10 ** uint256(decimals)); // 20,000,000,000 DISC - 10%    
    uint256 public constant PR_ADVERSTISING_SUPPLY = 15000000000 * (10 ** uint256(decimals)); // 20,000,000,000 DISC - 7.5%
    uint256 public constant REFERRAL_SUPPLY = 8000000000 * (10 ** uint256(decimals)); // 8,000,000,000 DISC - 4%
    uint256 public constant ANGEL_INVESTORS_SUPPLY = 5000000000 * (10 ** uint256(decimals)); // 5,000,000,000 DISC - 2.5%
    ...
    modifier onlyLockupAuthorized() {
        ...
        require(msg.sender == owner, "Attemp to lockup tokens by not authorized address");
        _;
    }
     ...
     */
    constructor() public { 
        balances[owner] = balances[sale].add(INVESTORS_SUPPLY);
        totalSupply_ = totalSupply_.add(INVESTORS_SUPPLY);
        emit Transfer(address(0), owner, INVESTORS_SUPPLY);
        // balances[INVESTORS_ADDRESS] = balances[sale].add(INVESTORS_SUPPLY);
        // totalSupply_ = totalSupply_.add(INVESTORS_SUPPLY);
        // emit Transfer(address(0), INVESTORS_ADDRESS, INVESTORS_SUPPLY);
    }

    /**
     * @dev Set sale address, release dates and allocate tokens supply
     * @param _sale address of the sale contract
     * @param _teamRelease team tokens release timestamp
     * @param _vestingFirstRelease first release timestamp of vesting tokens
     * @param _vestingSecondRelease second release timestamp of vesting tokens
     * @param _vestingThirdRelease third release timestamp of vesting tokens
     * @param _vestingFourthRelease fourth release timestamp of vesting tokens
     */
    function init(
        address _sale, 
        ...
        onlyOwner 
    {
        ...
        balances[INVESTORS_ADDRESS] = balances[sale].add(ANGEL_INVESTORS_SUPPLY);
