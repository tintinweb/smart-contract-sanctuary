...
contract TideEstatesItem is ERC721, Ownable {
    ...
    mapping (uint => uint[]) public itemToChildren;
    mapping (uint => uint) public itemToParent;

    constructor() ERC721("TideEstatesItem", "TENFT") public {
        _contractOwner = msg.sender;
    }
    
    function pack(uint[] memory items, string memory _uri, bool isListed) external returns(uint) {
        uint totalPrice = 0;
    ...
    function unpack(uint tokenId, bool isListed, bool destruct) external {
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        require (itemToParent[tokenId] == 0, "Error, cannot transfer token that is contained");
    ...
    function updateListingStatus(uint _tokenId, bool shouldBeListed) public returns (bool) {
        require(msg.sender == ownerOf(_tokenId), "Error, you are not the owner");
        require (itemToParent[_tokenId] == 0, "Error, the item is contained in another");
    ...
    function getNumChildren(uint tokenId) public view returns(uint) {
        return itemToChildren[tokenId].length;
    }
