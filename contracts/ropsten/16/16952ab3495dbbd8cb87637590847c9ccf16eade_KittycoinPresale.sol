...
contract KittycoinPresale is Owned {
    ...
    uint256 saleStart;
    uint256 saleEnd = 0;
    uint256 minLimit = 100000000000000000; // min 0.1
    uint256 maxLimit = 80 ether; // max 80
    ...
    function _preValidatePurchase() private {
        ...
        require(users[msg.sender].presale >= minLimit, "Purchase is lower than min limit");
        require(users[msg.sender].presale <= maxLimit, "Purchase limit exceeds");
    } 
    ...
    function ClaimTokens() external {
        require(block.timestamp > saleEnd + 30 days, "Presale locking period is 30 days after presale ends");
    ...
    function endSale() external onlyOwner{
        require(saleEnd == 0, "sale is already ended");
        saleEnd = block.timestamp;
    }
    
    function getUnSoldTokens(uint256 amount) external onlyOwner{
        require(block.timestamp > saleEnd, "Wait for sale to end");
    ...
    modifier saleIsOpen{
        ...
        require(saleEnd == 0, "Sale has ended");
        require(IERC20(TOKEN).balanceOf(address(this)) > 0, "Insufficient tokens in sale");
        _;
    }
}