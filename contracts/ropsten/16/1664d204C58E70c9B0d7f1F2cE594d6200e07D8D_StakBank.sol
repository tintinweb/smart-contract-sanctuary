...
contract StakBank is Ownable {
    ...
    uint public lastDis;
    uint public decimal;
    uint public minEthNeededToReward; // 1e12 = 1000000000000 = (10^9 / 0.1) * 100(year)
    uint public ethRewardedNotWithdraw;
    uint public totalStaked;

    uint private unitCoinToDivide; // 1e17 = 100000000000000000 = 0.1 JST | Number of staked (JST) coin to be rewarded = 0.1 * N
    ...
    constructor(address _tokenAddress, uint _periodTime, uint _feeUnit, uint _decimal) {
        ...
        feeUnit = _feeUnit;
        minAmountToStake = 100000000000000000; // 0.1 JST
        lastDis = block.timestamp;
        decimal = _decimal;
        minEthNeededToReward = 1000000000000;  // 1e12 >= 1e9(coin) / 0.1(coin) * 100 (year).Pool can hold up to 1 billion JST
        totalStaked = 0;

        unitCoinToDivide = 100000000000000000; // 0.1 JST
    ...
    function setMinAmountToStake(uint value) external onlyOwner {
        require(value >= unitCoinToDivide, "Lower than 0.1 JST");
    ...
    function rewardDistribution() public onlyOwner {
        ...
        require(_checkIfNotEnoughEthToReward(ethToReward), "Need more eth to Reward");
    ...
    function _checkIfNotEnoughEthToReward(uint ethToReward) private returns (bool) {
        if (ethToReward >= minEthNeededToReward) return true;
        ...
        for(uint i = 0; i < stakingTrans.length; i++) {
            ...
            _eStaker[transaction.staker][_posIdStake].ethFirstReward = 0;
