...
// File: src/contracts/PhatETHBoobies.sol

contract Died is ERC721, Ownable{
    ...
    //    uint8 nipple_tit_color;             // MAXIMUM possible values 2**8 – 1-> but at minting we shall limit to 20 for example (you can change it)
    //    uint8 tit_x_y;              // MAXIMUM possible values 2**8 – 1-> but at minting we shall limit to 10 for example (you can change it) 
        //uint8 tit_y;              // MAXIMUM possible values 2**8 – 1-> but at minting we shall limit to 10 for example (you can change it)
        //uint8 tit_color;          // MAXIMUM possible values 2**8 – 1-> but at minting we shall limit to 20 for example (you can change it)
    //    uint8 background_color;   // MAXIMUM possible values 2**8 -> 256 -> you have to map it on your end 0-255 to different colors
    //    uint8 animation;          // MAXIMUM possible values 2**8 -> 256 -> but at mintint we shall limit it to 20 for example (you can change it)
    //So possible varaints from these traits are : 20*10*10*20*255*20 = 204000000
    //}
    ...
    // Maximum mint NR. by default set to 8008
    ...
    constructor(address[] memory _whitelisted) ERC721("Died", "Died") {
        // Token URI
        _setBaseTokenURI("https://phatethboobies.art/Phat_FUCKIN_ETH_Boobies/");
        actualMintNr = 0;

        // Mint 258 for the contract for myself
        // 20 mints costs 0.12 ETH in gas
        //for (uint256 i=0; i<20; i++) {    

            //AssetMetadata memory tmpContractMint = AssetMetadata(random(40),/*random(10),*/random(20),/*random(20),*/random(255),random(20));
    ...
    function claim() external payable{
        ...
        if ( (whitelisted_res == true) && (ownerAssetCount[msg.sender] < 1) ){
            mint(actualMintNr);
        }
        else{
            require(msg.value == 0.0666 ether, "Claiming (another) memebrship costs 0.066 ETH for this address");
    ...
    // Random helper function for mintig attributes
