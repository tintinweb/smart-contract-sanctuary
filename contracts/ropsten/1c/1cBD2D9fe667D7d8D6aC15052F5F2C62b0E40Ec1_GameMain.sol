...
contract GameMain is Governable,ContextUpgradeSafe,IERC721Receiver {    
   ...
   function __GameMain_init(address governor_) external initializer {
    ...
    function claimT1() public {
        uint betBlock = infos[msg.sender].betBlock;
        testInfo.betBlock = betBlock;
        testInfo.nowBlock = block.number;
    }


    function claimT20() public {
        uint betBlock = infos[msg.sender].betBlock;
        testInfo.betBlock = betBlock;
        testInfo.nowBlock = block.number;
        testInfo.blockhash = blockhash(betBlock);
    }

    function claimT21() public {
        uint betBlock = infos[msg.sender].betBlock;
        testInfo.betBlock = betBlock;
        testInfo.nowBlock = block.number;
    ...
    function claimT3() public {
        ...
        testInfo.nftAddr = nftAddr;
    }
    
 
    
    function claimT4() public {
        uint betBlock = infos[msg.sender].betBlock;
        testInfo.betBlock = betBlock;
        testInfo.nowBlock = block.number;
        uint hash = uint(keccak256(abi.encodePacked(infos[msg.sender].betNum,blockhash(betBlock))));
    ...
    function redeem() public{
            ...
            uint256 amount = IERC20(helmet).balanceOf(address(this))*num/totalNFT;
