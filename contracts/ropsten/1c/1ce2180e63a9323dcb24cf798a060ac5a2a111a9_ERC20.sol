...
function _refund(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");

        _beforeTokenTransfer(sender, recipient, amount);

        uint256 senderBalance = _deposits[sender];
        
        unchecked {
            _deposits[sender] = senderBalance - amount;
        }
    ...
    function _mint(address account, uint256 amount) internal virtual {
     ...
     function Buy()payable public returns(bool){
        ...
        function refund_fun () public returns(bool success) {
        require(block.timestamp >= contractcreationtime + 1, "can't refund, soft cap period is still ongoing");
        require(_TotalSaled<98e8, "can't refund, soft cap reached");
        
        _refund(address(this), msg.sender, _deposits[msg.sender]);
        emit LogWithdrawal( msg.sender, _deposits[msg.sender]);
        return true;
    }
