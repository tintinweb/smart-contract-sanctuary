...
/**
 * @title Presale
 * @dev Presale contract allowing investors to purchase the dark token.
...
contract Presale is Context {
     ...
     * @param darkToken_ Address of the dark token being sold
     * @param weiPerToken_ tokens amount paid for purchase a dark token
     */
    constructor (address payable wallet_, IERC721 darkToken_, uint256 weiPerToken_)
        public
    {
        require(wallet_ != address(0), "Presale: wallet is the zero address");
        require(address(darkToken_) != address(0), "Presale: dark token is the zero address");
        require(weiPerToken_ > 0, "Presale: token price must be greater than zero");
        _wallet = wallet_;
        _darkToken = darkToken_;
        _weiPerToken = weiPerToken_;
    }

    /**
     * @dev Fallback function revert your fund.
     * Only buy dark token with the buyToken function.
     */
     ...
     */
    function darkToken() public view returns (IERC721) {
        return _darkToken;
    }

    /**
     * @return Amount of wei to be pay for a dark token
     */
    ...
    /**
     * @dev Returns x and y where represent the position of the dark.
     */
    function darkById(uint256 tokenId) public pure returns (uint256 x, uint256 y){
        x = tokenId / 90;
        y = tokenId - (x * 90);
    }
    ...
    function buyToken(address beneficiary, uint256 tokenId) public payable{
        ...
        require(weiPerToken() == msg.value, "Presale: Not enough KCS");
        require(!_darkToken.exists(tokenId), "Presale: token already minted");
        require(tokenId < 11520, "Presale: tokenId must be less than max token count");
        (uint256 x, uint256 y) = darkById(tokenId);
        require(x < 38 || x > 53 || y < 28 || y > 43, "Presale: tokenId should not be in the unsold range");
        _wallet.transfer(msg.value);
        _darkToken.mint(beneficiary, tokenId);
    ...
    function buyBatchTokens(address beneficiary, uint256[] memory tokenIds) public payable{
        ...
        require(weiAmount == msg.value, "Presale: Not enough KCS");
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            require(!_darkToken.exists(tokenIds[i]), "Presale: token already minted");
            require(tokenIds[i] < 11520, "Presale: tokenId must be less than max token count");
            (uint256 x, uint256 y) = darkById(tokenIds[i]);
            require(x < 38 || x > 53 || y < 28 || y > 43, "Presale: tokenId should not be in the unsold range");
        }
        _wallet.transfer(msg.value);
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            _darkToken.mint(beneficiary, tokenIds[i]);
