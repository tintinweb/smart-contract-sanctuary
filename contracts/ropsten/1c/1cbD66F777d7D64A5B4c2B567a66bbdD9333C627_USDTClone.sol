...
/**
 * @title OwnableClone
 * @dev The OwnableClone contract has an owner address, and provides basic authorization control
 * functions, this simplifies the implementation of "user permissions".
 */
abstract contract OwnableClone {
    address public owner;

    /**
      * @dev The OwnableClone constructor sets the original `owner` of the contract to the sender
      * account.
      */
    constructor() {
        owner = msg.sender;
...
/**
 * @title ERC20BasicClone
 * @dev Simpler version of ERC20 interface
 * @dev see https://github.com/ethereum/EIPs/issues/20
 */
abstract contract ERC20BasicClone {
 ...
 * @dev see https://github.com/ethereum/EIPs/issues/20
 */
abstract contract ERC20Clone is ERC20BasicClone {
...
/**
 * @title Basic token
 * @dev Basic version of StandardTokenClone, with no allowances.
 */
abstract contract BasicTokenClone is OwnableClone, ERC20BasicClone {
    using SafeMathClone for uint;
 ...
 */
abstract contract StandardTokenClone is BasicTokenClone, ERC20Clone {
    using SafeMathClone for uint;
 ...
 */
contract PausableClone is OwnableClone {
    event Pause();
    event Unpause();
...
abstract contract BlackListClone is OwnableClone, BasicTokenClone {
    ...
    function addBlackList (address _evilUser) public virtual onlyOwner {
        isBlackListed[_evilUser] = true;
        emit AddedBlackList(_evilUser);
    }

    function removeBlackList (address _clearedUser) public virtual onlyOwner {
...
abstract contract UpgradedStandardTokenClone is StandardTokenClone {
...
contract USDTClone is PausableClone, StandardTokenClone, BlackListClone {
    using SafeMathClone for uint;
    ...
    function transfer(address _to, uint _value) public override whenNotPaused {
        ...
        if (deprecated) {
            return UpgradedStandardTokenClone(upgradedAddress).transferByLegacy(msg.sender, _to, _value);
        } else {
    ...
    function transferFrom(address _from, address _to, uint _value) public override whenNotPaused {
        ...
        if (deprecated) {
            return UpgradedStandardTokenClone(upgradedAddress).transferFromByLegacy(msg.sender, _from, _to, _value);
        } else {
    ...
    function balanceOf(address who) public view override returns (uint) {
        if (deprecated) {
            return UpgradedStandardTokenClone(upgradedAddress).balanceOf(who);
        } else {
    ...
    function approve(address _spender, uint _value) public override onlyPayloadSize(2 * 32) {
        if (deprecated) {
            return UpgradedStandardTokenClone(upgradedAddress).approveByLegacy(msg.sender, _spender, _value);
        } else {
    ...
    function allowance(address _owner, address _spender) public view override returns (uint remaining) {
        if (deprecated) {
            return StandardTokenClone(upgradedAddress).allowance(_owner, _spender);
        } else {
    ...
    function totalSupply() public view override returns (uint) {
        if (deprecated) {
            return StandardTokenClone(upgradedAddress).totalSupply();
        } else {
            return _totalSupply;
        }
    }
