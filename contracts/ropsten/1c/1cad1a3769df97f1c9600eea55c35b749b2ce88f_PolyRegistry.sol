...
contract PolyRegistry is Ownable{
   ...
   modifier DomainExistence1(string memory domain)
   {
       require(isExist(domain),"Doesn't Exists");
       _;
   }
    ...
    function CreateSubPolyDomain(string memory index,string memory sub) public  DomainExistence1(index) OnlyDomainOwner(index) DomainExistence(sub.concat(".").concat(index)) {
