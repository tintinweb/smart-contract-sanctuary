...
contract CeroToken is ICeroStruct, ERC721("Cero", "Cr"), NameGenerator {
    ...
    function generateFirstToken() external onlyOwner {
        ...
        createToken(msg.sender, 255, 999, 999, 999, 999, 0, 0, false);
    }
...
contract NameGenerator is ComplexOwnable, Randomizer {
    ...
    /// @notice Add new name to name1 or override exist.
    /// @param _newName New name
    /// @param _index Index to override
    /// @param _overrideIndex If true, override some name
    function setName1(
        string memory _newName,
        uint256 _index,
        bool _overrideIndex
    ) external onlyOwner {
        if (_overrideIndex == true) require(_index < name1.length, "[EN-101] - Invalid index for name1.");
        _setName(_newName, _index, _overrideIndex, name1);
    }

    /// @notice Add new name to name2 or override exist.
    /// @param _newName New name
    /// @param _index Index to override
    /// @param _overrideIndex If true, override some name
    function setName2(
        ...
        if (_overrideIndex == true) require(_index < name2.length, "[EN-102] - Invalid index for name1.");
        _setName(_newName, _index, _overrideIndex, name2);
    }
    ...
    function _setName(
        ...
        uint256 _index,
        bool _overrideIndex,
        string[] storage _nameContainer
    ) private {
        if (_overrideIndex == true) {
            _nameContainer[_index] = _newName;
        } else {
            _nameContainer.push(_newName);
        }
    }
}
