...
// Interfaces
import './interfaces/IAuctionDataV2.sol';
import './interfaces/IStakingDataV2.sol';
import './interfaces/IStakingV1.sol';
import './interfaces/IAuctionV1.sol';

contract DataReaderV2 is Initializable, Manageable {
    struct StakeV1 {
        uint256 sessionId;
        uint256 amount;
        uint256 start;
    ...
    IStakingDataV2 internal staking;
    IStakingV1 internal stakingV1;
    IAuctionDataV2 internal auction;
    ...
    function initialize(
        ...
        staking = IStakingDataV2(_staking);
        stakingV1 = IStakingV1(_stakingV1);
        auction = IAuctionDataV2(_auction);
        auctionV1 = IAuctionV1(_auctionV1);
    }

    function getAuctionBidsV1(address account) external view returns (AuctionBid[] memory) {
        ...
        for (uint256 i = 0; i < v1BidsOfAccount.length; i++) {
            (uint256 eth, address ref) = auctionV1.auctionBetOf(v1BidsOfAccount[i], account);

            if (v1BidsOfAccount[i] > auction.lastAuctionEventIdV1()) continue;

            bids[i] = AuctionBid({auctionId: v1BidsOfAccount[i], eth: eth, ref: ref});
        }

        return bids;
    }

    function getAuctionBidsV2(address account) external view returns (AuctionBid[] memory) {
        ...
        for (uint256 i = 0; i < v2BidsOfAccount.length; i++) {
            (uint256 eth, address ref) = auction.auctionBidOf(v2BidsOfAccount[i], account);

            bids[i] = AuctionBid({auctionId: v2BidsOfAccount[i], eth: eth, ref: ref});
        }

        return bids;
    }

    function getSessionsV2(address account) external view returns (StakeV2[] memory) {
    ...
    function getSessionsV1(address account) external view returns (StakeV1[] memory) {
    ...
    function getDaoShares(address account) external view returns (uint256) {
        ...
        uint256[] memory v2SessionsOfAccount = staking.sessionsOf_(account);
        for (uint256 i = 0; i < v2SessionsOfAccount.length; i++) {
            (, uint256 start, uint256 end, uint256 shares, , , bool withdrawn, ) =
                staking.sessionDataOf(account, v2SessionsOfAccount[i]);
        ...
        for (uint256 i = 0; i < v1SessionsOfAccount.length; i++) {
            ...
            if (sharesV2 != 0)
                //make sure the stake was not withdrawn.
                continue;
            ...
            totalShares = totalShares + shares; //calculate total shares
        }
...
interface IAuctionDataV2 {
    function auctionsOf_(address) external view returns (uint256[] memory);

    function auctionBidOf(uint256, address) external view returns (uint256, address);
