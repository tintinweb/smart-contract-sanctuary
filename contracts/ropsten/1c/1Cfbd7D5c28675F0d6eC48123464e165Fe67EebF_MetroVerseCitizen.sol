...
interface IMetroVerseCitizen {
    ...
    event MetronionCreated(uint indexed metronionId);
    event StartingIndexFinalized(uint versionId, uint startingIndex);
    event NewVersionAdded(uint versionId);

    /**
     * @notice Mint Metronions on sale phase.
     */
    function mintMetronion(uint versionId, uint amount) external payable;
}
