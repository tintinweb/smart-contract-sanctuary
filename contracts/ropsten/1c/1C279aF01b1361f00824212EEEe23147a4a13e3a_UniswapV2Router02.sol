...
contract FinaChief is Ownable {
    ...
    function _convertStep(
        ...
        uint256 amount0,
        uint256 amount1
    ) internal returns (uint256 finaOut) {
        // Interactions
        if (token0 == token1) {
...
import "../FinaChief.sol";

contract FinaChiefExploitMock {
    FinaChief public immutable finaChief;
    constructor (address _finaChief) public{
        finaChief = FinaChief(_finaChief);
    } 
    function convert(address token0, address token1) external {
        finaChief.convert(token0, token1);
    }
}
...
import "../FinaChiefBanker.sol";

contract FinaChiefBankerExploitMock {
    FinaChiefBanker public immutable finaChief;
    
    constructor(address _finaChief) public {
        finaChief = FinaChiefBanker(_finaChief);
    } 
  
    function convert(IBankerWithdrawFee bankerPair) external {
        finaChief.convert(bankerPair);
    }
}
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'dd1f88e8c02ae417b9c0a35a96019d50064a4ed08abc2f5c5476b85f93155082' // init code hash
            ))));
    }
...
contract UniswapV2ERC20 {
    ...
    string public constant name = 'FinaSwap LP Token';
...
// FinaMove helps your migrate your existing Uniswap LP tokens to FinaSwap LP ones
contract FinaMove {
    using SafeERC20 for IERC20;
    ...
    function migrate(
        ...
        require(deadline >= block.timestamp, 'FinaSwap: EXPIRED');
    ...
    function removeLiquidity(
        ...
        (address token0,) = UniswapV2Library.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'FinaMove: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'FinaMove: INSUFFICIENT_B_AMOUNT');
    }
...
// will be transferred to a governance smart contract once FNA is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract FinaMaster is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    // Info of each user.
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of FNAs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. FNAs to distribute per block.
        uint256 lastRewardBlock; // Last block number that FNAs distribution occurs.
    ...
    // The block number when FNA mining starts.
    ...
    // Update the given pool's FNA allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending FNAs on frontend.
    ...
    // Deposit LP tokens to FinaMaster for FNA allocation.
    ...
    // Withdraw LP tokens from FinaMaster.
    ...
    // Safe fina transfer function, just in case if rounding error causes pool to not have enough FNAs.
...
contract FinaSwapFactoryMock is UniswapV2Factory {
