...
contract StrongInuNodePool is Context, Ownable {
    ...
    uint256 public feeAmount = 1000000000000000;
    uint256 public feeDuration = 15 days;
    ...
    IERC20 public SINU = IERC20(0x3A6b4f8fbF0d62ce2EC4be985796e956A7B6fA20);
        ...
        for (uint256 i = 0; i < count; i ++) {
            _nodesOfUser[account].push(
                NodeEntity({
                    ...
                    dueTime: block.timestamp + i + feeDuration
                })
            );
            nodeOwners[account]++;
            totalNodesCreated++;
    ...
    function payNodeFee(uint256 _creationTime) payable external {
        ...
        node.dueTime = block.timestamp + feeDuration;
    }
    ...
    function payAllNodesFee() payable external {
        ...
        for (uint256 i = 0; i < nodes.length; i++) {
            if (nodes[i].dueTime >= block.timestamp ) {
                nodes[i].dueTime = block.timestamp + feeDuration;
            }
        }
    }
