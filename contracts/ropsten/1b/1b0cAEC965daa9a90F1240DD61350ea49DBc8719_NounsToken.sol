...
contract NounsToken is INounsToken, Ownable, ERC721Checkpointable {
    ...
    /**
     * @notice Mint a Noun to the tx.origin, NOTE: modified so no longer has onlyMinter
     * @dev Call _mintTo with the to address(es).
     */
    function mint() public override returns (uint256) {
        return _mintTo(tx.origin, _currentNounId++); 
    }
    ...
    function _mintTo(address to, uint256 nounId) internal returns (uint256) {
       ...
       // _setTokenURI(nounId, 'https://gateway.pinata.cloud/ipfs/QmYueiuRNmL4MiA2GwtVMm6ZagknXnSpQnB3z2gWbz36hP');
