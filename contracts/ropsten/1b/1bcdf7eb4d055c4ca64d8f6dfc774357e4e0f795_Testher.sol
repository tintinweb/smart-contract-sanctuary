...
contract Testher is ERC20, ERC20Detailed,
    ...
    uint256 internal unlockDate = 1625504400;
    uint256 internal timelockStart;
    ...
    constructor() public ERC20Detailed("Testher","TESTHER", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public mintingStartTime; //Proof-of-Age minting start time
...
//--------------------------------------------------------------------------------------
    ...
    function blockMintingInfo(address account) external view returns (address, uint256, uint256) {
        uint256 blocksCount = block.number.sub(genesisMintingBlockNumber);
...
//--------------------------------------------------------------------------------------
    ...
    function getProofOfEpochMinting(address account) internal returns (uint256) {
        ...
        uint256 depositedToken = balances[account];
        
        if(totalSupply < mintingTier.mul(1)) {
            depositedToken = balances[account].mul(1);
            ...
            depositedToken = balances[account].div(2);
        ...
        return depositedToken.mul(epochCount).div(epochTimes);
    }
