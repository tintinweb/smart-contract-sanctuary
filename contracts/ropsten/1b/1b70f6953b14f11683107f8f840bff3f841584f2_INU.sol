...
contract INU is Context, IERC20, Ownable {
    ...
    uint256 public _WalletFeeOnBuy = 8;
    
    
    //Sell Fee
    uint256 public _redisFeeOnSell = 5; // 1st
    uint256 public _WalletFeeOnSell = 10; //1st
    
    //Original Fee
    uint256 private _redisFee = _redisFeeOnSell;
    uint256 private _WalletFee = _WalletFeeOnSell;
    
    //Maximum Fee allowed per variable after deployment
    uint256 public _MaxWalletFee = 10;
    uint256 public _MaxRedistributionFee = 6;
    ...
    uint256 public _maxTxAmount = _tTotal/100; //0.69
    uint256 public _maxWalletSize = _tTotal/50; //1.38*
    ...
    function removeAllFee() private {
        if (_redisFee == 0 && _WalletFee == 0) return;
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router)) {
                ...
                _WalletFee = _WalletFeeOnBuy;
            }
    
            //Set Fee for Sells
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                _redisFee = _redisFeeOnSell;
                _WalletFee = _WalletFeeOnSell;
            }
        ...
        )
    {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam) =
            _getTValues(tAmount, _redisFee, _WalletFee);
        uint256 currentRate = _getRate();
        ...
        )
    {
        ...
        uint256 tTeam = tAmount.mul(WalletFee).div(100);
    ...
    function setFee(uint256 redisFeeOnBuy, uint256 redisFeeOnSell, uint256 WalletFeeOnBuy, uint256 WalletFeeOnSell) public onlyOwner {
        require(redisFeeOnBuy <= _MaxRedistributionFee 
        && WalletFeeOnBuy <= _MaxWalletFee 
        && WalletFeeOnSell <= _MaxWalletFee 
        && redisFeeOnSell <= _MaxRedistributionFee);
        
        _redisFeeOnBuy = redisFeeOnBuy;
        _redisFeeOnSell = redisFeeOnSell;
        
        _WalletFeeOnBuy = WalletFeeOnBuy;
        _WalletFeeOnSell = WalletFeeOnSell;
    }
