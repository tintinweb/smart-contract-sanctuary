...
contract MineNetworkRewardDistributor is IMineNetworkRewardDistributor, ReentrancyGuardUpgradeable {
  ...
  function claimTest(
    ...
    addr = data.toEthSignedMessageHash().recover(_signature);
  }

  function claimTestFull(
    address _addr,
    uint256 _pid,
    address _rewardToken,
    uint256 _amount,
    ...
    bytes32 data = keccak256(
      abi.encodePacked(_addr, _pid, _rewardToken, _amount, _doubleRewardToken, _doubleRewardAmount, _claimIndex)
    );
    addr = data.toEthSignedMessageHash().recover(_signature);
  }

  function getMaintainer()  public view returns (address addr) {
    addr = IMineNetworkAdmin(addressManager).maintainer();
  }
