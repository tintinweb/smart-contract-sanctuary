...
contract SanPhamCuaAnhBac{
    ...
    function createProduct(string _name, string _serialNo, uint _price) public {
        ...
        emit ProductCreated(productCount, _name, _serialNo, _price);
    }

    //hàm tìm sản phẩm theo mã số
    function getProduct(string serlialNo) view public returns(uint, string, string, uint) {
        Product  p;
