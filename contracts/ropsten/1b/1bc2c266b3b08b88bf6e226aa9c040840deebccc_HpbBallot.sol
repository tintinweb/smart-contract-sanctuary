...
contract HpbBallot {
    ...
    //选择总共几轮的数据作为选举结果，默认为7，就是7轮数据的累计得票数为选举结果
    uint public round = 7;
   ...
   */
   ...
   /**
    ...
    function changeStageBlock(
        ...
        emit ChangeStageBlock(voteStages[_voteStageIndex-1].blockNumber,_voteStageBlock);
    ...
    function addCandidate(
        ...
        //必须首轮才可以添加候选人
        require(voteStageIndexMap[currentVoteStageBlock]==0);
     ...
     */
    function deleteCandidateByStage(
        address _candidateAddr,
        uint _voteStageBlock
    ) onlyOwner onlyVoteAfterStart public{
    ...
    function deleteCandidates(
        ...
        deleteCandidateByStage(_candidateAddr,currentVoteStageBlock);
    }
    ...
    function fechAllCandidatesByStage(
        ...
        for(uint i=1;i<=cl;i++){
            ...
            _names[i]=stringToBytes32(
                voteStages[voteStageIndex].candidateArray[i].name
            );
            _facilityIds[i]=stringToBytes32(
    ...
    function voteResult(
        ...
        if(candidateLastArray.length>0){
            return getVoteResult();
             ...
             require(voteStages[0].candidateArray.length>1);
             uint vcl=voteStages[0].candidateArray.length-1;
             Candidate[] memory _candidates=new Candidate[](vcl);
             for (uint i = 1;i<=vcl;i++){
                 _candidates[i-1]=Candidate(
                     voteStages[0].candidateArray[i].candidateAddr,
                     voteStages[0].candidateArray[i].name,
                     voteStages[0].candidateArray[i].facilityId,
                     voteStages[0].candidateArray[i].numberOfVotes,
                     new address[](0)
                 );
             }
             //计算竞选累计值
             for(uint r=1;r<=round;r++){
                 for(uint k = 1;k<=vcl;k++){
                     _candidates[k-1].numberOfVotes=safeAdd(
                         _candidates[k-1].numberOfVotes,
                         voteStages[r].candidateArray[k].numberOfVotes
                     );
                 }
             }
    ...
    function fechAllSumNumForCandidateByStage(
        ...
        for(uint i=1;i<=vcl;i++){
            ...
            _nums[i-1]=fechSumNumForCandidateByStage(_candidateAddr,_voteStageBlock);
            _addrs[i-1]=_candidateAddr;
        }
        return (_addrs,_nums);
    }
    /**
    ...
    function fechSumNumForCandidateByStage(
        ...
        for(uint i=0;i<=_voteStageIndex;i++){
   ...
   /**
     ...
     * Total number of votes obtained from candidates
     */
    function fechVoteSumNumForCandidate(
        address candidateAddr
    ) onlyVoteAfterStart public constant returns (
        uint num
    ){
