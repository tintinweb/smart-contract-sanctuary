...
contract BettingExchange{
    ...
    function placeBet(uint256 gameId, uint256 _userId, bool _bool, uint256 _category) public returns(bool) {
        require(game[gameId].startTime > block.timestamp, "game started");
        require(_gameId >= gameId, "Game ID is invalid");
        require(block.timestamp >= game[gameId.sub(1)].endTime || _gameId.sub(1) == 0, "Game not yet start");
        ...
        uint commission = betting[_userId].betAmount.mul(0.8e18).div(100e18);
        require(token.transferFrom(msg.sender,address(this), commission),"commission not send");
        if (_category == 0 || _category == 1){
        users[userList[_userId]]._userAmt = users[userList[_userId]]._userAmt.sub(betting[_userId].betAmount.add(commission));
        }
        else if (_category == 2){
    ...
    function withdraw(uint256 gameid_withdraw, uint256 userid_withdraw) public returns(bool) {
        require(users[msg.sender].userStatus[gameid_withdraw] == false);
        uint amount;
    ...
    function adminWithdraw(uint _amount) public onlyOwner {
        token.transfer(ownerAddress,_amount);
    }
