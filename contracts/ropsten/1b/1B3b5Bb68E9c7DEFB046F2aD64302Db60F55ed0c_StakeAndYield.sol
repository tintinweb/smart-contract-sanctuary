...
contract StakeAndYield is Ownable {
    ...
    function depositFor(address _user, uint256 amount, uint256 stakeType)
    ...
    function claim() public {
        sendReward(
            msg.sender,
            earned(msg.sender, STAKE),
            earned(msg.sender, YIELD)
        );
    }
    ...
    function userInfo(address account) public view returns(
        ...
        numbers[7] = rewardTillNowPerToken.mul(365 days).div(PERIOD);
        numbers[8] = yieldRewardTillNowPerToken.mul(365 days).div(PERIOD);
