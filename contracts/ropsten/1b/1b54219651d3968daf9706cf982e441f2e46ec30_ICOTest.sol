...
interface IMinerCommon {
	...
	function multiplierSwitch()  external returns(uint256);
...
contract ICOTest{
	...
	function getMinerData3( address adr, address contract_address) public  returns(uint256, uint256, address,uint256,uint256, uint256) {
				return ( 
				...
				IMinerCommon(contract_address).multiplierSwitch()
				);
	}
