...
contract ubot is Context, IERC20, IERC20Metadata, Ownable, Sales {
    event distributeReferralIncomeEvent(int256 _i, uint256 _amount, address _ref);
    ...
    //*****
    function Distribute(uint256 RewardperShare) public onlyOwner {
        require(TotalMint <= 210000000000 * (uint256(10) ** 18), "no mint");
        require(TotalStake > 0, "no stake");
        TotalShare += RewardperShare / TotalStake;
    }
    ...
    function distributeReferralIncome(address _user, uint256 _amount, uint _phaseNo) internal returns (uint256) {
        ...
        for (uint256 i = 0; i < 6; i++) {
            ...
            emit distributeReferralIncomeEvent(int256(i), income, ref);
    ...
    function withdraw(uint _phaseNo) public {
        ...
        //*****
        uint256 reward = UserInfos[_msgSender()][_phaseNo].amount * (TotalShare - SnapShare[_msgSender()][_phaseNo]);
    ...
    function StakeToken(uint _phaseNo, uint256 amountTokens) public returns (bool) {
        ...
        transferFrom(_msgSender(), address(this), amountTokens);
