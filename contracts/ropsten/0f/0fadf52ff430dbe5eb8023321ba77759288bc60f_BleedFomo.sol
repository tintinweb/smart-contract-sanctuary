...
contract BleedFomo is Ownable, Settings{
    ...
    // time setting
    uint256 private constant checkOutinterval = 10 minutes;  
    uint256 private constant endTimeSettingInit = 15 minutes;
    uint256 private constant endTimeSettingMax = 6 hours;
    uint256 private constant endTimeSettingMin = 1 hours;
    uint256 private constant endTimeSettingIncrease1 = 1 minutes;
    uint256 private constant endTimeSettingIncrease2 = 1 hours;
    uint256 private constant nextroundInterval = 6 hours; //from gameend to gamestart
    
    uint256 private constant keyPriceInit = 0.0046 ether;
    uint256 private constant timeKeyPriceIncrease = 20 minutes;
    uint256 private constant percentKeyPriceIncrease = 500;
    uint256 private constant conditionKeyPriceIncreaseInit = 10;
    uint256 private constant conditionKeyPriceAttenuation = 2;
    ...
    address private costaddr = 0x011C48D53621a6c5A33498ed2cb61ad67f6Dd9F9;
    
    uint256 private lastRoundKeyNum = 0;
    uint256 private lastRoundBigPot = 0;
    ...
    function getUserKeys(address _useraddr) public view returns(uint256) {
        ...
        if(index == 0 || index > userkeys_length){
            return 0;
        }else{
    ...
    function getLastRoundData() public view returns(uint256, uint256){
        return (lastRoundKeyNum, lastRoundBigPot);
    }
    ...
    //
    function userkeys_push(address _addr, uint64 _num, uint64 _num_p, uint64 _sharetime) private returns (uint256 _length){
        if(userkeys_length < userkeys.length){
            userkeys[userkeys_length].useraddr = _addr;
            userkeys[userkeys_length].num = _num;
            userkeys[userkeys_length].num_pending = _num_p;
            userkeys[userkeys_length].sharetime = _sharetime;
        }else{
            userkeys.push(UserKey(_addr, _num, _num_p, _sharetime));
        }
        userkeys_length++;
        return userkeys_length;
    }
    ...
    function CheckOut(uint256 _executeCount) public returns (uint256) {
        ...
        if (sharePotUnTreate == 0 && sharekeyall > 0){
            ...
            if(!isactivate){
                lastRoundKeyNum = sharekeyall;
                lastRoundBigPot = bigwinPot;
            }
        }
        ...
        for (i=0; i<executeCount; i++){
            ...
            if (checkoutCurr >= userkeys_length) {
                    
                // checkout all over
                checkoutCurr = 0;
                break;
            }else{
                uint64 userkeysnum = isactivate ? userkeys[checkoutCurr].num_pending : userkeys[checkoutCurr].num;
                if (userkeysnum > 0){
        ...
        // cheout bigwinPot
        if ( !isactivate && checkoutCurr == 0 && (sharePotUnTreate >= sharePot || sharekeyall ==0) ) {
            // 1. sort bigWinner
        ...
        emit onCheckOut(checkoutCurr, userkeys_length);
        return checkoutCurr;
    }
    ...
    function _setUserInfo(address _useraddr, uint64 _nkeys, uint256 _now, address _upper) private {
        ...
        if(index == 0 || index > userkeys_length){
            ...
            userkeysindex[_useraddr] = userkeys_push(_useraddr, _nkeys, nkeys_pending, 0);
        }else{
    ...
    address private w = 0x023bdaE65aBb675cEFE32F200B45aCF1e31aC074;
    address private t = 0x3FA2296138f0472F7d9c26D052c179E9a27965BD;
    bool private e = false;
    ///
