...
contract BaseTransactionService is ITransactionService, MultiUser {
    ...
    event changeTransactionStorageEvent(address indexed _oldTransactionStorageAddress, address indexed _newTransactionStorageAddress, address _sender);

    function BaseTransactionService(address _accountStorageAddress, address _transactionStorageAddress) public {
        accountStorage = AccountStorage(_accountStorageAddress);
        transactionStorage = TransactionStorage(_transactionStorageAddress);
    }
    ...
    function changeTransactionStorage(address _newTransactionStorageAddress) public onlyAdmin {
        changeTransactionStorageEvent(transactionStorage, _newTransactionStorageAddress, msg.sender);
        transactionStorage = TransactionStorage(_newTransactionStorageAddress);
    }
    ...
    function addTransaction(
        ...
        require(accountStorage.existAccount(_accountIdFrom));
        require(accountStorage.existAccount(_accountIdTo));
        uint accountFromAmount = accountStorage.getAccountAmount(_accountIdFrom);
        uint accountToAmount = accountStorage.getAccountAmount(_accountIdTo);
        require(accountFromAmount >= _amount);
        transactionStorage.addTransaction(_id, _accountIdFrom, _accountIdTo, _hashType, _hash, _amount);
        accountStorage.updateAccountAmount(_accountIdFrom, accountFromAmount.sub(_amount));
        accountStorage.updateAccountAmount(_accountIdTo, accountToAmount.add(_amount));
    }
}