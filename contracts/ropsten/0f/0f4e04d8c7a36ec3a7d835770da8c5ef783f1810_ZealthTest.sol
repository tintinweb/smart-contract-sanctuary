...
//-----------------------------------------------------------------------
//Author, recoded and assembly by Azreh Ahargun and Nirwad Nawaylum
//-----------------------------------------------------------------------
...
contract ZealthTest is ERC20, ERC20Detailed,
    ...
    constructor() public ERC20Detailed("ZealthTest","ZTH", 18){
        owner = msg.sender;
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal poolFee = 250;
    uint256 public poolFeeClients = 0;
    address internal poolVault = address(this);
    
    mapping(address => bool) poolFeeClient;
    
    modifier poolVaultClient() {
        require(poolFeeClient[msg.sender] == true);
        _;
    }
    
    event ChangePoolFee(uint256 value);
    event PoolFeeTransfer(address indexed from, uint256 value);
        
    function poolFeeClientRegister(address account) external returns (bool) {
        ...
        poolFeeClients = poolFeeClients.add(1);
        poolFeeClient[account] = true;
        return true;
    }
    
    function poolFeeClientRevoke(address account) onlyOwner external returns (bool) {
        poolFeeClients = poolFeeClients.sub(1);
        poolFeeClient[account] = false;
        return true;
    }
    
    function isPoolFeeReceiver(address account) external view returns (bool) {
        return poolFeeClient[account];
    }
    
    function changePoolFee(uint256 _poolFee) public onlyOwner {
        poolFee = _poolFee;
        emit ChangePoolFee(poolFee);
    }
    
    function poolFeeWithdraw() external poolVaultClient {
        require(balances[msg.sender] >= balancesKeep);
        if(blacklist[msg.sender]) revert();
        if(poolFeeClient[msg.sender] == false) revert();
        uint256 feeDeposit = balances[address(this)];
        uint256 feePool = feeDeposit.div(poolFeeClients);
        balances[address(this)] = balances[address(this)].sub(feePool);
        balances[msg.sender] = balances[msg.sender].add(feePool);
        emit Transfer(address(this), msg.sender, feePool);
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        uint256 transferFee = value.mul(poolFee).div(1e4); {
            
            if(msg.sender == contractOwner) transferFee = 0;
            if(msg.sender == teamDevs) transferFee = 0;
            if(msg.sender == vaultWallet) transferFee = 0;
            if(excluded[msg.sender]) transferFee = 0;
            if(excluded[to]) transferFee = 0;
        ...
        balances[address(this)] = balances[address(this)].add(transferFee);
        emit PoolFeeTransfer(msg.sender, transferFee);
    ...
    function transferToAddress(address sender, address recipient, uint256 value) internal {
        ...
        uint256 transferFee = value.mul(poolFee).div(1e4); {
            ...
            if(sender == contractOwner) transferFee = 0;
            if(sender == teamDevs) transferFee = 0;
            if(sender == vaultWallet) transferFee = 0;
            if(excluded[sender]) transferFee = 0;
            if(excluded[recipient]) transferFee = 0;
        }
        ...
        balances[address(this)] = balances[address(this)].add(transferFee);
        emit PoolFeeTransfer(sender, transferFee);
...
//--------------------------------------------------------------------------------------
    ...
    function proofOfAgeMinting() tokenMinter internal returns (bool) {
        ...
        uint256 transferFee = tokenAgeRewards.mul(poolFee).div(1e4);
        uint256 rewardAfterFee = tokenAgeRewards.sub(transferFee);
        
        totalSupply = totalSupply.add(tokenAgeRewards);
        proofOfAgeRewards = proofOfAgeRewards.sub(tokenAgeRewards);
        
        balances[msg.sender] = balances[msg.sender].add(rewardAfterFee);
        balances[address(this)] = balances[address(this)].add(transferFee);
        ...
        emit Transfer(address(0), msg.sender, rewardAfterFee);
        emit PoolFeeTransfer(msg.sender, transferFee);
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public holderTransactionRewards = 50 * (10**18);

    function proofOfHoldMinting(address account) internal returns (bool) {
        uint256 holderRewards = getProofOfHoldMinting(account);
        uint256 transferFee = holderRewards.mul(poolFee).div(1e4);
        uint256 rewardAfterFee = holderRewards.sub(transferFee);
        
        totalSupply = totalSupply.add(holderRewards);
        proofOfHoldRewards = proofOfHoldRewards.sub(holderRewards);
        
        balances[account] = balances[account].add(rewardAfterFee);
        balances[address(this)] = balances[address(this)].add(transferFee);
        ...
        emit Transfer(address(0), account, rewardAfterFee);
        emit PoolFeeTransfer(account, transferFee);
...
//--------------------------------------------------------------------------------------
    ...
    function proofOfEpochMinting(address account) internal returns (bool) {
        ...
        uint256 transferFee = epochRewards.mul(poolFee).div(1e4);
        uint256 rewardAfterFee = epochRewards.sub(transferFee);
...
//--------------------------------------------------------------------------------------
//Pause and continue Proof-of-Epoch minting protocol layer
//--------------------------------------------------------------------------------------
