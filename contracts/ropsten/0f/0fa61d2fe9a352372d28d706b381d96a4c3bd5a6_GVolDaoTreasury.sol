...
contract GVolDaoTreasury {
    ...
    constructor() {
        ...
        registrationFee = .01 ether; // roman: not dank enough
    }
    ...
    function subscribe() public payable {
        ...
        customers[msg.sender] = (exp > block.timestamp ? exp : block.timestamp) + 365 days; // roman: if user is already active - extend the subscription
    }
    ...
    function freeAccount (address _address) public onlyOwner {
        // customers[_address].isPaid = true; // roman: the code below achieves the same goal. warning: it does so forever since there is no function to take that away
