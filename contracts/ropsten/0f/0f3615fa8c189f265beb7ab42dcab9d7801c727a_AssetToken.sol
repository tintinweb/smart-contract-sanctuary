...
contract AssetToken is ERC721BasicToken, Ownable {
    ...
    function createAsset(string _data, uint _price) public {
        uint id = assets.length;
        assets.push(Asset(_data, _price, false));
        _mint(msg.sender, id);
    }

    function updateAsset(uint _tokenId, string _data, uint _price) public {
        require(msg.sender == ownerOf(_tokenId) || msg.sender == owner);
        assets[_tokenId].data = _data;
