...
contract LaunchX is Ownable {
  ...
  event EIsFinalized(bool _isFinalized);
  ...
  event EBuyTokens(address _sender, uint256 _value, uint256 _totalToken, uint256 _rewardedAmount, uint256 _senderTotalAmount, uint256 _senderTotalRewardedAmount, uint256 _senderSoldAmount, uint256 _senderTotalRise, uint256 _totalParticipant);

  constructor() {
    _startTime = block.timestamp;
    maxPayableAmount = 1e18;
    // 1 ETH = x10 times on Token
    exchangeRate = 10;
  }
  ...
  function isStart() public view returns (bool) {
    ...
    return timestamp > _startTime;
  }

  // Read: Calculate Token
  function calculateAmount(uint256 ethAmountInWei) public view returns (uint256) {
    uint256 rate = ethAmountInWei.div(1 ether).mul(exchangeRate);
    return rate * 10 ** uint256(_token.decimals());
  }
  ...
  // Fallback: Revert receive ether without buyTokens
  fallback() external {
    revert();
  }

  // Write: Crowdsale Token
  ...
  // Write: Finalize to sale
  function finalize() external onlyOwner returns (bool) {
    isFinalized = true;
    // Emit event
    emit EIsFinalized(isFinalized);
    return isFinalized;
  }
  ...
  function redeemTokens() external {
    ...
    require(isFinalized, "Sale is not finalized yet");
  ...
  // Write: Owner withdraw all ETH balance
  function withdrawBalance() external onlyOwner {
    ...
    // Emit event
    emit EWithdrawBalance(sender, balance);
  }
  ...
  function withdrawRemainingTokens() external onlyOwner {
    ...
    address payable sender = _msgSender();
    if (getTotalToken() >= soldAmount) {
      remainingAmount = getTotalToken().sub(soldAmount);
    } else {
      remainingAmount = soldAmount.sub(getTotalToken());
    }
  ...
  function buyTokens() external payable {
    ...
    require(value > 0, "You must pay some ETH to get Tokens");
    require(
      maxPayableAmount >= whitelistSnapshot.amount.add(value),
    ...
    if (whitelistSnapshot.amount == 0 && value > 0) {
      totalParticipant = totalParticipant.add(1);
    }
    // Update whitelist detail info
    whitelist[senderAddress].amount = whitelistSnapshot.amount.add(value);
    ...
    // Emit buy event
    emit EBuyTokens(senderAddress, value, totalToken, rewardedAmount, whitelist[senderAddress].amount, whitelist[senderAddress].rewardedAmount, soldAmount, totalRaise, totalParticipant);
  }
}