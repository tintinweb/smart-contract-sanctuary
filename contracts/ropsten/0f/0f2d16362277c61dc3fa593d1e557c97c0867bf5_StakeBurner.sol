...
contract StakeBurner is IStakeBurner, Manageable, Migrateable, Pausable, ExternallyCallable {
    ...
    function burnStakeInternal(uint256 sessionId, address staker) internal returns (uint256) {
        /** TODO: Add check for sessionV3? */
        require(
            contracts.stakeToken.ownerOf(sessionId) == staker,
            'STAKE BURNER: Not owner of stake.'
        );

        (uint256 payout, uint256 penalty) = contracts.stakeManager.unsetStake(staker, sessionId);

        return handlePayoutAndPenalty(staker, payout, penalty);
    }
    ...
    */
    function unstakeLegacyStakeInternal(
        ...
        require(contracts.stakeToken.exists(sessionId) == false, 'STAKE BURNER: stake is v3.');
        require(
            contracts.stakeManager.getStakeWithdrawn(sessionId) == false,
            'STAKE BURNER: stake is withdrawn.'
        );
        ...
        if (shares != 0) {
            ...
            (payout, penalty) = contracts.stakeManager.unsetLegacyStake(
                staker,
                sessionId,
                shares,
                amount,
