...
contract FootballBetting {
  ...
  event BetCreated(address creator, uint id, uint amount, uint oddsNumerator, uint oddsDenominator);
  event BetAccepted(address creator, uint id, address acceptor,uint ratio);
  ...
  function createBet(uint betAmount, uint oddsNumerator, uint oddsDenominator) public payable {
    ...
    emit BetCreated(betCreator, betId, betAmount, oddsNumerator, oddsDenominator);
  }
  ...
  function acceptBet(address betCreator, uint betId, uint ratio) public payable {
    ...
    emit BetAccepted(betCreator, betId, betAcceptor, ratio);
  }
