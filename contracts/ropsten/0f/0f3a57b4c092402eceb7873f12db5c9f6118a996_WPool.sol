...
contract WPool is Ownable {
    ...
    function setTokens(address _rewardToken, address _nativeToken) external onlyOwner {
        ...
        IERC20 _wex = IERC20(_nativeToken);
        pool =
            PoolInfo({
                wex: _wex,
                lastRewardBlock: 0,
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && wexSupply != 0) {
            ...
            accTokenPerShare = pool.accTokenPerShare.add(tokenReward.mul(1e24).div(wexSupply));
        }
    ...
    function updatePool() internal {
        ...
        uint256 multiplier = block.number.sub(pool.lastRewardBlock);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        pool.accTokenPerShare = pool.accTokenPerShare.add(tokenReward.mul(1e24).div(wexSupply));
        pool.lastRewardBlock = block.number;
    }
