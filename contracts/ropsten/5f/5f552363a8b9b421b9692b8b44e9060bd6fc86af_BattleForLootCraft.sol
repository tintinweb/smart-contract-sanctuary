...
contract BattleForLootCraft is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getLootStrength(uint256 lootId, uint256 ennemyId) public pure returns (uint256) {
        ...
        //Return a strength between 1 to 5.
        strength = strength % 5;
        if (strength < 1) {
            return 1;
        } else {
    ...
    function randomizeBattle(uint256 ennemyId, uint256 lootId) internal view returns (bool) {
        uint256 ennemiesStrength = getEnnemiesStrength(ennemyId);
        uint256 lootStrength = getLootStrength(lootId, ennemyId);
        uint256 rand = random(string(abi.encodePacked(toString(lootId), toString(ennemyId))));
        uint256 range = ennemiesStrength;
        uint256 battleRand = rand % range;
        if (battleRand > range - lootStrength - 1) {
            return true;
        } else {
            return false;
        }
    ...
    function claim(uint256 ennemyId, uint256 lootId) public payable nonReentrant {
        require(ennemyId > 0 && ennemyId < 8888, "Ennemy ID is invalid");
        require(lootId > 0 && lootId < 2000000, "Loot ID is invalid (mLoot out of range)");
