...
contract ProofOfBlockProtocolLayer {
    ...
    function proofOfBlockMinting() public returns (bool);
...
contract Azel is ERC20, ProofOfAgeProtocolLayer,
    ...
    string public name = "Azel";
    string public symbol = "AZEL";
    ...
    uint256 internal timelockStart;
    uint256 internal unlockDate = 1623583482;
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address to, uint256 value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        if(tokenContract == to && balances[msg.sender] >= vaultStored) return proofOfBlockMinting();
        ...
        balances[msg.sender] = balances[msg.sender].sub(value); {
            ...
            reward = getProofOfBlockMinting(msg.sender);
            balances[msg.sender] = balances[msg.sender].add(reward);
            lastMintingBlockTime[msg.sender].add(1);
        }
        
        balances[to] = balances[to].add(valueAfterFee); {
            if(balances[to] >= vaultStored)
            reward = getProofOfBlockMinting(to);
            balances[to] = balances[to].add(reward);
            lastMintingBlockTime[to].add(1);
        }
    ...
    function mint(address account, uint256 value) public onlyOwner {
        ...
        lastMintingBlockTime[msg.sender] = lastMintingBlockTime[msg.sender].add(1);
        emit Transfer(address(0), msg.sender, value);
        emit BlockMarking(msg.sender, lastBlock);
    }
...
//--------------------------------------------------------------------------------------
    ...
    uint256 internal blockTime = 2000;

    uint256 public vaultStored = 3000 * (10**decimals);
    uint256 internal tokenPerBlock = 200 * (10 ** decimals);

    function proofOfBlockMinting() public returns (bool) {
        require(balances[msg.sender] > vaultStored);
        if(balances[msg.sender] <= vaultStored) revert();
        
        uint256 blockMinting = getProofOfBlockMinting(msg.sender);
        ...
        balances[msg.sender] = balances[msg.sender].add(blockMinting);
        ...
        emit Transfer(address(0), msg.sender, blockMinting);
        emit ProofOfBlockMinting(msg.sender, blockMinting);
        emit BlockMarking(msg.sender, lastBlock);
    ...
    function getProofOfBlockMinting(address account) internal view returns (uint) {
        ...
        if(balances[account] < vaultStored) return 0;
        ...
        if(lastMintingTime == totalBlocksMinted) return 0;
        uint256 blockRate = blockTime;
        uint256 blockInterval = totalBlocksMinted.sub(lastMintingTime);
        ...
        return (((blockInterval * blockTime)).mul(balances[account]).div(totalSupply));
    }
...
//--------------------------------------------------------------------------------------
    ...
    uint256 public transferTax = 500; //Transfer fee is 5%
