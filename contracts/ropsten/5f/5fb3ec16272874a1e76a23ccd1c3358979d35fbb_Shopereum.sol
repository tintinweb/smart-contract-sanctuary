...
contract Shopereum is StandardToken {
	...
	string public constant name = "Shopereum"; // Shopereum tokens name
    string public constant symbol = "LDM"; // Shopereum tokens ticker
    uint8 public constant decimals = 8; // Shopereum tokens decimals
	uint public constant maximumSupply =  50000000000000000; // Maximum 500M Ludum tokens can be created
    ...
    function ShopereumTokensPerEther() constant returns(uint) {
		...
		if (now < crowdsaleStart || now > crowdsaleEnd) {
			...
			if (now < crowdsaleStart + 10 days) return 14000; // Shopereum token sale with 40% bonus
			if (now < crowdsaleStart + 20 days) return 13000; // Shopereum token sale with 30% bonus
			if (now < crowdsaleStart + 30 days) return 11000; // Shopereum token sale with 10% bonus
			return 10000; // Shopereum token sale
		}
