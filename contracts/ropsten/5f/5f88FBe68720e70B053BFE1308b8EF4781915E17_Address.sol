...
*/
 ...
 contract swapverse is Context, IERC20, Ownable {
     using SafeMath for uint256;
     using Address for address;

     address public marketingFeeReceiver = 0x088B1fb7919Fe8E25377590AC759d4cD73378400;
     ...
     uint256 public _tFee = 2;
     uint256 private _previoustFee = _tFee;
     
     uint256 public _marketingFee =1;
     uint256 private _previousmaketingFee = _marketingFee;
    ...
    function excludeFromFee(address account) public onlyOwner() {
     ...
     function _transfer(
         ...
         //transfer amount, it will take tax, burn, maketing fee
     ...
     function _takeMarketingFee(uint256 marketingFee) private {
        ...
        if(_isExcluded[marketingFeeReceiver])
            _tOwned[marketingFeeReceiver] = _tOwned[marketingFeeReceiver].add(marketingFee);
    }

    function _takeBurnFee(uint256 burnFee) private {
        uint256 currentRate =  _getRate();
        uint256 rburnFee = burnFee.mul(currentRate);
        _rOwned[deadAddress] = _rOwned[deadAddress].add(rburnFee);
        if(_isExcluded[deadAddress])
            _tOwned[deadAddress] = _tOwned[deadAddress].add(burnFee);
    }
