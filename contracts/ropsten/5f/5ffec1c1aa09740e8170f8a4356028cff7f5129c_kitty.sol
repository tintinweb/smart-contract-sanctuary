...
contract kitty is Context, IERC20 {
    ...
    uint256 constant private _AntiDumpFee24hr = 2500;
    uint256 constant private _PaperHandFee = 4650;
    ...
    uint256 public _TeamRatio = 2;
    uint256 public _CitadelRatio = 2;
    ...
    address payable private _CitadelWallet;
...
//===============================================================================================================
    ...
    function setRatios(uint256 liquidity, uint256 marketing, uint256 team, uint256 citadel) external onlyOwner {
        require (liquidity + marketing + team + citadel == 1000, "Must add up to 100%");
        _liquidityRatio = liquidity;
        _marketingRatio = marketing;
        _TeamRatio = team;
        _CitadelRatio = citadel;
    }
    ...
    function setWallets(address payable marketingWallet, address payable TeamWallet, address payable CitadelWallet) external onlyOwner {
        _marketingWallet = payable(marketingWallet);
        _TeamWallet = payable(TeamWallet);
        _CitadelWallet = payable(CitadelWallet);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_liquidityRatio + _marketingRatio + _TeamRatio + _CitadelRatio == 0)
            return;
        uint256 toLiquify = ((contractTokenBalance * _liquidityRatio) / (_liquidityRatio + _marketingRatio + _TeamRatio + _CitadelRatio)) / 3;

        uint256 toSwapForEth = contractTokenBalance - toLiquify;
        swapTokensForEth(toSwapForEth);

        uint256 currentBalance = address(this).balance;
        uint256 liquidityBalance = ((currentBalance * _liquidityRatio) / (_liquidityRatio + _marketingRatio + _TeamRatio + _CitadelRatio)) / 3;
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _marketingWallet.transfer(((currentBalance - liquidityBalance) * _marketingRatio) / (_marketingRatio + _TeamRatio + _CitadelRatio));
            _TeamWallet.transfer(((currentBalance) * _TeamRatio) / (_CitadelRatio + _TeamRatio));
            _CitadelWallet.transfer(address(this).balance);
        }
    }
