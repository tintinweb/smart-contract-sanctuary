...
contract Apyfier is Ownable {
    ...
    */
    ...
    function usersAA(address _address,uint256 _id) public returns(uint256){
       uint256 amount = users[_address].strategyShares[_id];
       return amount;
    }
    
    function strategiesAAA(uint256 _id) public returns(uint256 _lastBalance, uint256 _shareValue, uint256 _shareTotal, uint256 _pendingBalance, uint256 _lockedBalance){
       Strategy memory strateg = strategies[_id];
       uint256 _lastBalance =  strateg.lastBalance;
       uint256 _shareValue = strateg.shareValue;
       uint256 _shareTotal = strateg.shareTotal;
       uint256 _pendingBalance = strateg.pendingBalance;
       uint256 _lockedBalance = strateg.lockedBalance;
    }
