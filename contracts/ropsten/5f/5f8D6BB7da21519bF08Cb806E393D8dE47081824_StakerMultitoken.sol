...
contract StakerMultitoken is Ownable {
    ...
    function stake(IERC20 token, uint256 value) external notHalted {
        require(value > 0, "Staking Contract: stake value should be greater than 0");
        require(address(token) == address(f9token) || address(token) == address(shibatoken), "Invalid stake token");
    ...
    function unstake(IERC20 token, uint256 value) external lockable {
        require(value > 0, "Staking Contract: unstake value should be greater than 0");
        require(_tokenBalances[address(token)][_msgSender()] >= value, 'Staker: insufficient staked balance');
