...
contract SNAS {
  ...
  mapping(bytes32 => address) public topBidder;
  mapping(bytes32 => uint256) public auctionDeadlines;
  ...
  function changeNameAddress(bytes32 name, address resolveTo)
    public
    returns (bool)
  {
    require(locked == false, "Locked");
  ...
  function handOver(bytes32 name, address resolveTo) public returns (bool) {
    require(locked == false, "Locked");
    require(nameOwners[name] == msg.sender, "Not owner");
    require(
  ...
  function bid(bytes32 name, uint256 bidPrice) public payable returns (bool) {
    require(locked == false, "Locked");
    require(
      bidPrice > bids[name],
  ...
  function endAuction(bytes32 name) public returns (bool) {
    require(locked == false, "Locked");
    require(
  ...
  function getCurrentBid(bytes32 name) public view returns (uint256) {
    return bids[name];
  }
