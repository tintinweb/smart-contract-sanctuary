...
contract CharityLiquidityGeneratorToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    address dead = 0x000000000000000000000000000000000000dEaD;
    uint256 public maxLiqFee = 0;
    uint256 public maxTaxFee = 0;
    
    uint256 public maxCharityFee = 0;
    
    uint256 public minMxTxPercentage = 50;
    uint256 public prevLiqFee;
    uint256 public prevTaxFee;
    
    uint256 public prevCharityFee;
    ...
    address public teamWalletAddress;     // charity/team wallet here
    ...
    constructor (address tokenOwner,string memory name, string memory symbol,uint8 decimal, uint256 amountOfTokenWei,uint8 setTaxFee, uint8 setLiqFee, uint256 _minMxTxPer, uint8 setCharityFee, address _teamWalletAddress) public {
        _name = name;
        _symbol = symbol;
        _decimals = decimal;
        ...
        _isCharity[_teamWalletAddress] = true;
    ...
    function setCharityFeePercent(uint256 charityPercent) external onlyOwner() {
        require(charityPercent >= 0 && charityPercent <=maxCharityFee,"teamFee out of range");
        _charityFee = charityPercent;
    }      
    ...
    function setTeamAddress(address _addr) public onlyOwner {
        require(!_isCharity[_addr], "Wallet address already set");
        _isCharity[_addr] = true;
    ...
    function _sendToCharity(uint256 tCharity) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[teamWalletAddress] = _rOwned[teamWalletAddress].add(rCharity);
        if(_isExcluded[teamWalletAddress])
            _tOwned[teamWalletAddress] = _tOwned[teamWalletAddress].add(rCharity);
    }    
