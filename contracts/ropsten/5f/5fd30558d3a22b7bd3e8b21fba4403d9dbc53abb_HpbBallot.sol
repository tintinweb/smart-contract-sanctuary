...
contract HpbBallot {
    ...
    // 更新候选者
    // update candidate
    event CandidateUpdated(address indexed candidateAddr,uint indexed index,string facilityId,string name);
    
    event CandidateUpdatedEntirety(address indexed candidateAddr,
        address newCandidateAddr,uint indexed index,string newFacilityId,string newName
    );
   ...
   */
   ...
   /**
    ...
    function updateCandidate(
        ...
        uint _voteStageIndex=voteStageIndexMap[_snapshotBlock];
        if(_voteStageIndex==0){
            require(_snapshotBlock==voteStages[0].snapshotBlock);
        }
        uint index=voteStages[_voteStageIndex].candidateIndexMap[_candidateAddr];
        // 判断候选人是否已经存在 Judge whether candidates exist.
        require(index!= 0);
        
        voteStages[_voteStageIndex].candidateArray[index].facilityId=_facilityId;
        voteStages[_voteStageIndex].candidateArray[index].name=_name;
        emit CandidateUpdated(_candidateAddr,_voteStageIndex,_facilityId,_name);
    }
    /**
     * 根据阶段删除候选者
