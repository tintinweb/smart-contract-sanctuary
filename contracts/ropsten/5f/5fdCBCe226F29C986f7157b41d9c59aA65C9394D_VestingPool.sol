...
import "./SafeMath.sol"; //todo: from old version of the project. Refactor in next iteration
import "./GTH.sol"; //todo: from old version of the project. Refactor in next iteration
import "./Ownable.sol"; //todo: from old version of the project. Refactor in next iteration
...
contract VestingPool is Ownable {
    ...
    function getAvailableAmountFor(bytes32 _category)
        ...
        returns (uint256)
    {
        uint256 currentMonth = block.timestamp.sub(vestingStartDate).div(
            SECONDS_PER_MONTH
        );
        ...
        onlyOwner
    {
        ...
        require(vesting[_category].adminEmergencyFirstApprove[_admin]);
        require(
            block.timestamp.sub(vesting[_category].initEmergencyDate) >
                SECONDS_PER_DAY
        );
    ...
    function _amountWithPrecision(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(10**uint256(token.decimals()));
    }
