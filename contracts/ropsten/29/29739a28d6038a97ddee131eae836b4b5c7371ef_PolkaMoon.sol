...
contract PolkaMoon is Context, IERC20, Ownable {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee,uint256 rBurnValue,uint256 tBurnValue,uint256 tTax,uint256 rTax) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);       
        _reflectFee(rFee, tFee, rBurnValue, tBurnValue,tTax,rTax);
    ...
    function _reflectFee(uint256 rFee, uint256 tFee,uint256 rBurnValue, uint256 tBurnValue,uint256 tTax,uint256 rTax) private {
        _rTotal = _rTotal.sub(rFee).sub(rBurnValue).sub(rTax);
        _tFeeTotal = _tFeeTotal.add(tFee).add(tBurnValue).add(tTax);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256,uint256,uint256,uint256,uint256) {
        ...
        return (_localVal[4], _localVal[5], _localVal[6], _localVal[0], _localVal[1], _localVal[7], _localVal[2],_localVal[8],_localVal[9]);
    }

    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256, uint256,uint256) {
        uint256 tTax = tAmount.div(10);
        uint256 tBurnValue = tAmount.div(100).mul(5);
        uint256 tFee = tAmount.div(100).mul(2);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurnValue).sub(tTax);
        return (tTransferAmount, tFee, tBurnValue,tTax);
    }
