...
contract Oasis is ERC20Interface, Owned {
	...
	function getper() public view returns(uint onceOuttimes, uint perss,uint runpers, uint pubpers, uint subpers, uint luckypers, uint lastpers, uint sellkeypers, uint sellpers) {
	...
	*/
	 ...
	 * 购买
	 */
	function buy(uint keyid) public payable returns(bool) {
		require(actived == true);
	    require(isend == false);
		address user = msg.sender;
		...
		ethnum = ethnum.add(money);
		sysethnum = sysethnum.add(amount);
		userethnum = userethnum.add(amount);
	...
	function sell(uint256 amount) public returns(bool success) {
		...
		uint useper = amount*sellper*keyprice/100;
