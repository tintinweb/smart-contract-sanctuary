...
/*
 * https://www.celestialproject.xyz
 */
 
contract CelestialProject is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Celestial Shard";
    ...
    uint256 public _rebalanceCallerFee = 400;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "SHARD: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'SHARD: The contract it self cannot be excluded');
        require(!_isExcluded[account], "SHARD: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "SHARD: Account is already included");
