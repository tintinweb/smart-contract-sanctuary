...
contract Marx is IERC20, IERC20Metadata, Context, Ownable{
    ...
    uint256 private _timeForDividends = 1 days;
    address public _dividendsAccount = 0x10241dde03634B6B8D14f15227F9FDD7554D54fd;
    ...
    //Esse mapeamento permite que seja armazenado em um address (o enedereço do dono do zumbi) e o valor em uma uint (que é quantos zumbis esse endereço possui)
    mapping (address => uint) ownerDividendsCount;
    
    
    modifier isCommunist(address _account) {
        //Pensar como proteger de contratos malignos
        //Precisa desse primeiro require?
        require (_account == msg.sender);
        require(_account != address(0), "Marx: Communist non detected");
    ...
    function withdrawDividends() public isCommunist(msg.sender) {
        uint256 _dividends = _balances[_dividendsAccount];
        uint256 myDividends = _dividends.div(accounts.length);
    
        emit Transfer(_dividendsAccount, msg.sender, myDividends); 
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _mintDividends(_dividendsAccount, amount/20);
        return true;
    }
