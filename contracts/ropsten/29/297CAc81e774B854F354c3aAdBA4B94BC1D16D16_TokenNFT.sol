...
contract SquareFactory is Ownable {
    ...
    mapping(uint256 => SPropAd) public _sProps;
    mapping(uint256 => bool) public _sForRent;
...
contract TokenNFT is ERC721, ERC721Metadata, SquareFactory {
    address proxyRegistryAddress;

    mapping(uint256 => uint256) private _sTokenPositionsSquare;
    mapping(uint256 => bool) private _existPositionsSquare;
    ...
    function _mint(
        address to,
        uint256 tokenId,
        uint256 positionSquare
    ) internal {
        require(_sTokenPositionsSquare[tokenId] == 0, "Already minted");
        require(!isExistSquare(positionSquare), "Already minted");
        require(
            positionSquare >= 1 && positionSquare <= 100**2,
            "Invalid position"
        );

        _sTokenPositionsSquare[tokenId] == positionSquare;
    ...
    function mint(address to, uint256 positionSquare) public onlyManager {
        uint256 nextTokenId = _getNextTokenId();
        _mint(to, nextTokenId, positionSquare);
    }
        ...
        onlyManager
    {
        ...
        for (uint256 index = 0; index < positions.length; index++) {
            uint256 nextTokenId = _getNextTokenId();
            _mint(to, nextTokenId, positions[index]);
        }
    }
