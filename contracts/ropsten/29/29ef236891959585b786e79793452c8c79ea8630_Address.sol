...
contract RESERVEToken is ERC20("RESERVE", "RSVE"), Ownable {
    uint256 private _cap = 21000e18;
    uint256 private _initial_supply = 7000e18;
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "RSVE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "RSVE::delegateBySig: invalid nonce");
        require(now <= expiry, "RSVE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "RSVE::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "RSVE::_writeCheckpoint: block number exceeds 32 bits");
    ...
    // In some circumstance, we should not burn RSVE on transfer, eg: Transfer from owner to distribute bounty, from depositing to swap for liquidity
...
interface IMigratorToRSVESwap {
    // Perform LP token migration from legacy UniswapV2 to RSVESwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // RSVESwap must mint EXACTLY the same amount of RSVESwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// RSVEMasterFarmer is the master of RSVE. He can make RSVE and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once RSVE is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract RSVEMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of RSVEs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRSVEPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accRSVEPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. RSVEs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that RSVEs distribution occurs.
        uint256 accRSVEPerShare; // Accumulated RSVEs per share, times 1e12. See below.
    }
    ...
    // The RSVE TOKEN!
    RESERVEToken public RSVE;
    // Dev address.
    address public devaddr;
    // RSVE tokens created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early RSVE makers.
    ...
    // The block number when RSVE mining starts.
    ...
    uint256 public burnPercent = 0; // init 0% burn RSVE

    uint256 public lockFromBlock;
    uint256 public lockToBlock;

    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorToRSVESwap public migrator;
    ...
    event SendRSVEReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    event Lock(address indexed to, uint256 value);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "RSVEMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's RSVE allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToRSVESwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 RSVEForBurn;
        uint256 RSVEForDev;
        uint256 RSVEForFarmer;
        (RSVEForBurn, RSVEForDev, RSVEForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);

        if (RSVEForBurn > 0) {
            RSVE.mint(address(this), RSVEForBurn);
            RSVE.burn(RSVEForBurn);
        }

        if (RSVEForDev > 0) {
            // Mint unlocked amount for Dev
            RSVE.mint(devaddr, RSVEForDev.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
            // Mint locked amount for Dev
            //For more simple, I lock reward for dev if mint reward in bonus time
            RSVE.mint(address(this), RSVEForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            farmLock(devaddr, RSVEForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
        }
        RSVE.mint(address(this), RSVEForFarmer);
        pool.accRSVEPerShare = pool.accRSVEPerShare.add(RSVEForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forBurn, uint256 forDev, uint256 forFarmer) {
        ...
        uint256 RSVECanMint = RSVE.cap().sub(RSVE.totalSupply());

        if (RSVECanMint < amount) {
            forBurn = 0;
            forDev = 0;
            forFarmer = RSVECanMint;
        }
        else {
    ...
    // View function to see pending RSVEs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accRSVEPerShare = pool.accRSVEPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 RSVEForFarmer;
            (, , RSVEForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accRSVEPerShare = accRSVEPerShare.add(RSVEForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accRSVEPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRSVEPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = RSVE.balanceOf(address(this));
            ...
            if(pending > 0) {
                RSVE.transfer(msg.sender, pending.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
                ...
                emit SendRSVEReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accRSVEPerShare).div(1e12);
        }
    }

    // Deposit LP tokens to RSVEMasterFarmer for RSVE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "RSVEMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from RSVEMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "RSVEMasterFarmer::withdraw: not good");
    ...
    // Safe RSVE transfer function, just in case if rounding error causes pool to not have enough RSVEs.
    function safeRSVETransfer(address _to, uint256 _amount) internal {
        uint256 RSVEBal = RSVE.balanceOf(address(this));
        if (_amount > RSVEBal) {
            RSVE.transfer(_to, RSVEBal);
        } else {
            RSVE.transfer(_to, _amount);
        }
    }
    ...
    function farmLock(address _holder, uint256 _amount) internal {
        require(_holder != address(0), "ERC20: lock to the zero address");
        require(_amount <= RSVE.balanceOf(address(this)), "ERC20: lock amount over blance");
        _locks[_holder] = _locks[_holder].add(_amount);
        _totalLock = _totalLock.add(_amount);
        if (_lastUnlockBlock[_holder] < lockFromBlock) {
            _lastUnlockBlock[_holder] = lockFromBlock;
    ...
    function unlock() public {
        ...
        uint256 amount = canUnlockAmount(msg.sender);
        // just for sure
        if (amount > RSVE.balanceOf(address(this))) {
            amount = RSVE.balanceOf(address(this));
        }
        RSVE.transfer(msg.sender, amount);
        _locks[msg.sender] = _locks[msg.sender].sub(amount);
        _lastUnlockBlock[msg.sender] = block.number;
        _totalLock = _totalLock.sub(amount);
    }
    ...
    function setTransferBurnRate(uint256 _tranferBurnRate) public onlyOwner {
        RSVE.setTransferBurnRate(_tranferBurnRate);
    }

    // In some circumstance, we should not burn RSVE on transfer, eg: Transfer from owner to distribute bounty, from depositing to swap for liquidity
    function addTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        RSVE.addTransferBurnExceptAddress(_transferBurnExceptAddress);
    }

    function removeTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        RSVE.removeTransferBurnExceptAddress(_transferBurnExceptAddress);
    }
}