   ...
   */
   ...
   /**
    ...
    function doVote(
        ...
        emit DoVoted(voterArray[index].voterAddr,candidateAddr,num,1);
    }   
    ...
    function fechAllCandidates(
        ...
        bytes32[] memory _names=new bytes32[](candidateArray.length);
        bytes32[] memory _facilityIds=new bytes32[](candidateArray.length);
        for(uint i=0;i<candidateArray.length;i++){
            _addrs[i]=candidateArray[i].candidateAddr;
            _names[i-1]=stringToBytes32(candidateArray[i].name);
