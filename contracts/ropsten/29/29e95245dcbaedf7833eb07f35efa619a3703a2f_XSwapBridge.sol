...
interface ITaalBridge {
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, uint amountOutMinX, address[] calldata pathx, address to, uint deadline)
    external
    payable
    returns (uint[] memory amounts);
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, uint amountInMaxX, address[] calldata pathx, address to, uint deadline)
    external
    returns (uint[] memory amounts);
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, uint amountOutMinX, address[] calldata pathx, address to, uint deadline)
    external
    returns (uint[] memory amounts);
    function swapETHForExactTokens(uint amountOut, address[] calldata path, uint amountOutX, address[] calldata pathx, address to, uint deadline)
    external
    payable
    returns (uint[] memory amounts);
...
contract XSwapBridge is ITaalBridge {
    ...
    ) external virtual override ensure(deadline) returns (uint[] memory amounts)
    {
        ...
        amounts = TaalLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'XSwapBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], WTAL, address(this), amounts[0]);
        require(IERC20(path[0]).balanceOf(address(this)) >= amounts[0]);
