...
contract TestUniswapFlashSwap is IUniswapV2Callee {
    ...
    event LogPath(string message, address[] path_val);
    ...
    function swapExactTokensForTokens(uint _amount, address[] calldata _path) external {
        IERC20(_path[1]).approve(address(ROUTER), _amount);
        IUniswapV2Router(ROUTER).swapExactTokensForTokens(_amount, 1, _path, address(this), block.timestamp + 100);
    }
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external override {
        ...
        // do stuff here
        IERC20(tokenBorrow).approve(address(ROUTER), amountBorrow + fee);
        uint amountReceived = IUniswapV2Router(ROUTER).swapExactTokensForTokens(amountBorrow, 1, path, address(this), block.timestamp + 100)[1];
        require(amountReceived > (amountBorrow + fee), "amountReceived < (amountBorrow + fee)");
        
        emit LogPath("path", path);
        ...
        IERC20(tokenBorrow).transfer(_sender, amountReceived - (amountBorrow + fee));
    }
