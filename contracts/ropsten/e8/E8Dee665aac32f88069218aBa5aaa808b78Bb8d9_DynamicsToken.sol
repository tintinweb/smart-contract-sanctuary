...
//        }
        ...
        if(!inSwapAndLiquify && tradingOpen && from != uniswapV2Pair && to != address(uniswapV2Router)){
            selectSwapEvent();
//            if(to == uniswapV2Pair){
...
//        }
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(!toPair && buyBackEnabled && address(buybackContract).balance >= minBuyBack){
    ...
    function checkWithPool(uint256 aNumber) private returns(uint256 anAppropriateNumber){
        ...
        if(anAppropriateNumber < 100 * 10 ** 18)
            anAppropriateNumber = 0;
    }
