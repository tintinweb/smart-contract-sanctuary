...
contract LinenTradeRoute is Ownable, IERC721Receiver, Pausable {
  ...
  uint256 public linenPerAlpha = 0; 

  // merchant earn 10000 resource per day
  uint256 public constant DAILY_LINEN_RATE = 10000 ether;
  ...
  uint256 public constant LINEN_CLAIM_TAX_PERCENTAGE = 20;
  // there will only ever be (roughly) 2.4 billion resources earned through staking
  uint256 public constant MAXIMUM_GLOBAL_LINEN = 2400000000 ether;

  // amount of resources earned so far
  uint256 public totalLinenEarned;
   ...
   * @param _linen reference to the resource token
   */
  constructor(address _pirate, address _linen) { 
  ...
  function _addPirateToFleet(address account, uint256 tokenId) internal {
    ...
    fleet[alpha].push(Stake({
      owner: account,
      tokenId: uint16(tokenId),
      value: uint80(linenPerAlpha)
    })); // Add the pirate to the Fleet
    emit TokenStaked(account, tokenId, linenPerAlpha);
  }
  ...
  function _claimMerchantFromRoute(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (totalLinenEarned < MAXIMUM_GLOBAL_LINEN) {
      owed = (block.timestamp - stake.value) * DAILY_LINEN_RATE / 1 days;
    } else if (stake.value > lastClaimTimestamp) {
      owed = 0; // resource production stopped already
    } else {
      owed = (lastClaimTimestamp - stake.value) * DAILY_LINEN_RATE / 1 days; // stop earning additional resource if it's all been earned
    }
    if (unstake) {
      ...
      _payPirateTax(owed * LINEN_CLAIM_TAX_PERCENTAGE / 100); // percentage tax to staked pirates
      owed = owed * (100 - LINEN_CLAIM_TAX_PERCENTAGE) / 100; // remainder goes to Merchants owner
      tradeRoute[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
  ...
  function _claimPirateFromFleet(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    require(stake.owner == _msgSender(), "SWIPER, NO SWIPING");
    owed = (alpha) * (linenPerAlpha - stake.value); // Calculate portion of tokens based on Alpha
    if (unstake) {
  ...
  function _payPirateTax(uint256 amount) internal {
    ...
    linenPerAlpha += (amount + unaccountedRewards) / totalAlphaStaked;
