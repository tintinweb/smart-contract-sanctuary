...
//--------------------------------------------------------VIEW FUNCTIONS --------------------------------------------------------
	...
	uint256 PendingRWT = (userWeightedAmount * accRwtPerShare / 1e12) - user.rewardDebt + user.claimableRWT;
    uint256 PendingETH = (userWeightedAmount * accETHPerShare / 1e12) - user.ETHrewardDebt + user.claimableETH;
...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
    ...
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn, uint256 amountETHMin, uint256 amountTokenMin, uint256 lockTime) public payable nonReentrant {
        ...
        require(msg.value > 0);
        require(lockTime >= 600 || user.lockedUntil >= block.timestamp, "addLiquidityNoFeeAndStake : Can't lock tokens for less than 1 week");
        updatePool(0);
        if (user.weightedBalance > 0) {
            addToClaimable(0, msg.sender);
        }
    ...
    function reinvestETHRewards(uint256 amountOutMin) public nonReentrant {
            ...
            if(tokenReleased) {
                ...
                if (pending > 0) {
                    ...
                    user.rewardDebt = (user.weightedBalance * poolInfo[1].accRwtPerShare / 1e12);
                }
            }
    ...
    function buyAndStakeETH(uint256 amountOutMin, uint256 lockTime) public payable {
        UserInfo storage user = userInfo[1][msg.sender];
        require(msg.value > 0);
        require(lockTime >= 600 || user.lockedUntil >= block.timestamp, "buyAndStakeETH : Can't lock tokens for less than 1 week");
        updatePool(1);
        if (user.weightedBalance > 0) {
            addToClaimable(1, msg.sender);
        }
        uint256 balanceBefore = Myobu.balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = WETH;
	...
	function withdrawNFT(uint256 _pid, address NFTContract, uint tokenID) public nonReentrant {
        ...
        addToClaimable(_pid, msg.sender);
...
//--------------------------------------------------------INTERNAL FUNCTIONS --------------------------------------------------------
    function _deposit(address sender, uint256 _pid, uint256 _amount, uint256 lockTime) internal {
        ...
        updatePool(_pid);
        if (user.weightedBalance > 0) {
            addToClaimable(_pid, sender);
            if(_amount == 0 && lockTime == 0){
                safeRWTTransfer(sender, user.claimableRWT);
                safeETHTransfer(sender, user.claimableETH);
                user.claimableRWT = 0;
                user.claimableETH = 0;
            }
        }
        if (_amount > 0) {
            require(lockTime >= 600 || user.lockedUntil >= block.timestamp, "deposit : Can't lock tokens for less than 1 week");
    ...
    function lockTokens(uint256 _pid, uint256 lockTime) internal {
        ...
        userInfo[_pid][msg.sender].lockTimeBoost = 2 * 1000 * (lockTime-600) / 30600; // 0 - 2000 
        updateUserWeightedBalance(_pid, msg.sender);
	...
	function updateUserWeightedBalance(uint256 _pid, address user) public {
		...
		userInfo[_pid][user].weightedBalance = amount * (1000 + userInfo[_pid][user].lockTimeBoost) * (1000 + userInfo[_pid][user].boostPointsBP) / 1000000;
		poolInfo[_pid].weightedBalance = poolBalance + userInfo[_pid][user].weightedBalance;
		userInfo[_pid][user].rewardDebt = userInfo[_pid][user].weightedBalance * poolInfo[_pid].accRwtPerShare / 1e12;
		userInfo[_pid][user].ETHrewardDebt = userInfo[_pid][user].weightedBalance * poolInfo[_pid].accETHPerShare / 1e12;
    }
    ...
    function updateETHRewards() internal {
        ...
        ETHPerBlock = ETHLeftUnshared / (5760 * 1);
        massUpdatePools();
    }
    
    function addToClaimable(uint256 _pid, address sender) internal {
        UserInfo storage user = userInfo[_pid][sender];
        PoolInfo storage pool = poolInfo[_pid];
        if(tokenReleased) {
