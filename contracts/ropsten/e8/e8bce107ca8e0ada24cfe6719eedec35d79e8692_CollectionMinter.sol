...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    constructor(string memory name_, string memory symbol_, uint256 _royalty, uint startTokenID, address _owner) {
        _name = name_;
        _symbol = symbol_;
        royalty = _royalty;
    ...
    modifier onlyMinter{
        require(isMinter[msg.sender],"Only Approved minter can mint");
        _;
    }
    
    modifier onlyOwner{
        require(collectionOwner == msg.sender,"Only Owner can call");
        _;
    }
    ...
    function bulkTransfer(address[] memory to, uint[] memory tokenIds) public virtual{
        ...
        require( to.length <= 25, "You can transfer max 25 tokens");
    ...
    function multiSendTokens(address to, uint[] memory tokenIds) public virtual{
        require( tokenIds.length <= 25, "You can transfer max 25 tokens");
...
contract CollectionMinter{
    event NewCollection(IERC721 collectionAddress);
    function createNewCollection(string memory name, string memory symbol, uint256 startId, uint256 royaltypercent) public{
        ERC721 collection = new ERC721(name,symbol,royaltypercent,startId,msg.sender);
