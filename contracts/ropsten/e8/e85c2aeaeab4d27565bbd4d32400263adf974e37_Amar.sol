...
contract Amar is ERC20, CoinAgeMintableStandard, CoinRateMintableStandard, Ownable {
...
//--------------------------------------------------------------------------------------
    ...
    function transfer(address _to, uint256 _value) onlyPayloadSize(2 * 32) external returns (bool) {
        ...
        //user will trigger both CoinAge and CoinRate minting simultaneously.
...
//--------------------------------------------------------------------------------------
    ...
    uint public maxInterval = 180 days; //Interval time of full weight : 180 days
    ...
    function getCoinRateMinting(address _address) internal view returns (uint) {
        ...
        uint coinRateMinting = coinRate.mul(mintingEra);
    ...
    function getPeriodRate(address _address, uint _now) internal view returns (uint _periodInterval) {
        ...
        for (uint i = 0; i < transferIns[_address].length; i++){
            ...
            _periodInterval = _periodInterval.add(uint(transferIns[_address][i].amount) * periodTime.div(1 days));
        }
    }
