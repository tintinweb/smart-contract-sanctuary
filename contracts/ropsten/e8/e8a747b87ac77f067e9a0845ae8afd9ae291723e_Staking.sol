...
contract Staking {
  ...
  uint256 constant public PERIOD_SERENITY             = 90;            // variable constant for time period managemnt
  uint256 constant public PERIOD_EQUILIBRIUM          = 180;           // variable constant for time period managemnt
  uint256 constant public PERIOD_TRANQUILLITY         = 270;           // variable constant for time period managemnt
  uint256 constant public WITHDRAW_TIME_SERENITY      = 45 ;            // variable constant to manage withdraw time lock up for silver
  uint256 constant public WITHDRAW_TIME_EQUILIBRIUM   = 90 * 1 days;            // variable constant to manage withdraw time lock up for gold
  uint256 constant public WITHDRAW_TIME_TRANQUILLITY  = 135 * 1 days;           // variable constant to manage withdraw time lock up for platinum
  uint256 public TOKEN_REWARD_PERCENT_SERENITY        = 3555807;       // variable constant to manage token reward percentage for silver
  uint256 public TOKEN_REWARD_PERCENT_EQUILIBRIUM     = 10905365;      // variable constant to manage token reward percentage for gold
  uint256 public TOKEN_REWARD_PERCENT_TRANQUILLITY    = 26010053;      // variable constant to manage token reward percentage for platinum
  uint256 public TOKEN_PENALTY_PERCENT_SERENITY       = 2411368;        // variable constant to manage token penalty percentage for silver
  uint256 public TOKEN_PENALTY_PERCENT_EQUILIBRIUM    = 3555807;       // variable constant to manage token penalty percentage for gold
  uint256 public TOKEN_PENALTY_PERCENT_TRANQUILLITY   = 14692434;       // variable constant to manage token penalty percentage for platinum
  uint256 constant public TOKEN_POOL_CAP              = 10*(10**18);
  ...
  */
  ...
  function setRewardPercent(uint256 silver, uint256 gold, uint256 platinum) external onlyOwner returns(bool){
    require(silver != 0 && gold != 0 && platinum !=0,"Invalid Reward Value or Zero value, Please Try Again!!!");
     TOKEN_REWARD_PERCENT_SERENITY = silver;
     TOKEN_REWARD_PERCENT_EQUILIBRIUM = gold;
     TOKEN_REWARD_PERCENT_TRANQUILLITY = platinum;
     return true;
  }
  ...
  function setPenaltyPercent(uint256 silver, uint256 gold, uint256 platinum) external onlyOwner returns(bool){
    require(silver != 0 && gold != 0 && platinum !=0,"Invalid Penalty Value or Zero value, Please Try Again!!!");
     TOKEN_PENALTY_PERCENT_SERENITY = silver;
     TOKEN_PENALTY_PERCENT_EQUILIBRIUM = gold;
     TOKEN_PENALTY_PERCENT_TRANQUILLITY = platinum;
     return true;
  }
  ...
  */
  ...
  function stakeToken(uint256 tokens, uint256 time) public tokenStakeCheck(tokens, time) returns(bool){
    ...
    if(time == PERIOD_SERENITY){
        require(totalStakedTokenInSerenityPool.add(tokens) <= TOKEN_POOL_CAP, "Pool Limit Reached");
        _tokentime = now + (time);
        ...
        require(totalStakedTokenInEquilibriumPool.add(tokens) <= TOKEN_POOL_CAP, "Pool Limit Reached");
        ...
        require(totalStakedTokenInTranquillityPool.add(tokens) <= TOKEN_POOL_CAP, "Pool Limit Reached");
        _tokentime = now + (time * 1 days);
        _tokenStakingCount = _tokenStakingCount +1;
        _tokenTotalDays[_tokenStakingCount] = time;
        _tokenStakingAddress[_tokenStakingCount] = msg.sender;
