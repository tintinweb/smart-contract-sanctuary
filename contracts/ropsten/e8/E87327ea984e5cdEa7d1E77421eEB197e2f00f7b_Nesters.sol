...
contract Nesters {
    ...
    function checkHouse(uint256 _id, uint256 _now) private {
        if (!houses[_id].offer) {
            if (_now - houses[_id].startDate > houses[_id].rentingPeriod) {
                ...
                users[houses[_id].tennat].properties = remove(
                    houses[_id].ID,
                    users[houses[_id].tennat].properties
                );
    ...
    /**
     * Helper Function: Remove a value from an array
     */

    function remove(uint256 _houseID, uint256[] memory _arr)
        private
        pure
        returns (uint256[] memory)
    {
        uint256[] memory auxArray;
        uint256 j = 0;
