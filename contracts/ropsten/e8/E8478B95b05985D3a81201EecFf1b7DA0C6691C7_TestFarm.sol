...
contract TestFarm {
    ...
    function claimReward(uint256 _pid) public {
        require(status == true, "TestFarmer::withdraw: can not claim reward");
        updatePool(_pid);
        _harvest(_pid);
    }
    ...
    // Deposit LP tokens to TestFarmer.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _referrer
    ) public {
        require(_amount > 0, "TestFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from TestFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(status == true, "TestFarmer::withdraw: can not withdraw");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "TestFarmer::withdraw: not good");
