...
contract NftBattleArena is Ownable, ERC721
{
	...
	// address voter => array of voter positions ids.
	...
	address public gasFee;    // Gas fee pool
	...
	// /// @notice Function for getting amount of nft in epoch.
	// /// @param epoch - number of epoch.
	// function getNftsInEpochLenght(uint256 epoch) public view returns(uint256 length) {
	// 	return nftsInEpoch[epoch].length;
	// }

	// /// @notice Function to get nfts in epoch by index.
	// /// @param epoch - epoch number.
	...
	function claimRewardFromStaking(uint256 positionId) public ///, address beneficiary) public
	{
		require(positions[positionId] == PositionType.StakerPostion, "this is not staker position");
		require(getCurrentStage() == Stage.FirstStage, "Must be at 1st stage!");  // Requires to be at first stage in battle epoch.
	...
	function pairNft(uint256 stakingPositionId) external
	{
		...
		require(nfts.length / 2 < nftsInGame / 2, "pairing error");
		uint256 index1;
	...
	function liquidateVotingPosition(uint256 positionId, address beneficiary) public
	{
		...
		for(uint i = 0; i < positionsOfVoter[msg.sender].length; i++)
		{
	...
	// / @param beneficiary - address of recipient.
	function claimRewardFromVoting(uint256 positionId) public ///, address beneficiary) public
	{
		require(getCurrentStage() == Stage.FirstStage, "Must be at first stage!");// Requires to be at first stage.
		require(ownerOf(positionId) == msg.sender, "You're not an owner"); // Requires to be owner of position.
		...
		uint256 reward = vault.withdraw(sharesToTokens(uint256(yTokens)));// beneficiary);
	...
	function getPendingVoterReward(uint256 positionId) public view returns (uint256 rewardAmount)
	{
		...
		uint256 reward = sharesToTokens(uint256(yTokens));
		return reward;
	}
