...
contract BoringHelperV1 is Ownable {
    ...
    IERC20 public reward; // ISushiToken(0x6B3595068778DD592e39A122f4f5a5cF09C90fE2);
    ...
    IFactory public pancakeFactory; // IFactory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);
    IERC20 public staked; // 0x8798249c2E607446EfB7Ad49eC89dD1865Ff4272;
    ...
    function getETHRate(IERC20 token) public view returns (uint256) {
        ...
        IPair pairUniV2;
        IPair pairSushi;
        if (pancakeFactory != IFactory(0)) {
            pairUniV2 = IPair(pancakeFactory.getPair(token, WETH));
        }
        if (factory != IFactory(0)) {
            pairSushi = IPair(factory.getPair(token, WETH));
        }
    ...
    struct UIInfo {
        ...
        uint256 rewardBarBalance;
        uint256 stakedBalance;
        uint256 stakedTotalSupply;
        uint256 stakedBarAllowance;
        Factory[] factories;
        uint256 ethRate;
        uint256 rewardRate;
        uint256 btcRate;
    ...
    function getUIInfo(
        ...
        if (reward != IERC20(0)) {
            info.rewardRate = getETHRate(reward);
            info.rewardBalance = reward.balanceOf(who);
            info.rewardBarBalance = reward.balanceOf(address(staked));
            info.stakedBarAllowance = reward.allowance(who, address(staked));
        }

        if (staked != IERC20(0)) {
            info.stakedBalance = staked.balanceOf(who);
            info.stakedTotalSupply = staked.totalSupply();
        }
        ...
        if (chef != IMasterChef(0)) {
            ...
            for (uint256 i = 0; i < poolLength; i++) {
                pendingReward += chef.pendingReward(i, who);
            }
    ...
    function pollPools(address who, uint256[] calldata pids) public view returns (UserPoolInfo[] memory) {
        ...
        for (uint256 i = 0; i < pids.length; i++) {
            ...
            pools[i].pending = chef.pendingReward(pids[i], who);
