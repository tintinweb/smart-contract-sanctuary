...
// File contracts/amm/StellaSwapV2Factory.sol
...
contract StellaSwapV2Factory is IStellaSwapV2Factory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(StellaSwapV2Pair).creationCode));
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(StellaSwapV2Pair).creationCode);
    }

    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, 'StellaSwapV2: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'StellaSwapV2: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'StellaSwapV2: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(StellaSwapV2Pair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        StellaSwapV2Pair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external override {
        require(msg.sender == feeToSetter, 'StellaSwapV2: FORBIDDEN');
        feeTo = _feeTo;
    }
    ...
    function setDevFee(address _pair, uint8 _devFee) external {
        require(msg.sender == feeToSetter, 'StellaSwapV2: FORBIDDEN');
        require(_devFee > 0, 'StellaSwapV2: FORBIDDEN_FEE');
        StellaSwapV2Pair(_pair).setDevFee(_devFee);
    }
    
    function setSwapFee(address _pair, uint32 _swapFee) external {
        require(msg.sender == feeToSetter, 'StellaSwapV2: FORBIDDEN');
        StellaSwapV2Pair(_pair).setSwapFee(_swapFee);
    }
