/**
 *Submitted for verification at moonbeam.moonscan.io on 2022-03-14
*/
...
contract StakingPools is OwnableUpgradeable {
    ...
    function getReward(
        ...
        if (currentUserData.entryAccuRewardPerShare.length == 0){
            return 0;
        }
    ...
    function extendEndBlockUnsafe(uint256 poolId, uint256 newEndBlock)
    external
    onlyOwner
    onlyPoolExists(poolId) // do not check for onlyPoolNotEnded
    {
        uint256 currentEndBlock = poolInfos[poolId].endBlock;
        require(newEndBlock > currentEndBlock, "StakingPools: end block not extended");
    ...
    function addRewardInfo(uint256 poolId, uint256 rewardRate, address rewarder) external onlyOwner onlyPoolExists(poolId) onlyPoolNotEnded(poolId){
        ...
        require(poolData[poolId].totalStakeAmount == 0, 
            "StakingPools: cannot add rewarder after a user has staked");

        RewardInfo memory rewardInfo = RewardInfo({
            rewardPerBlock: rewardRate,
            rewarder: rewarder,
            accuRewardPerShare: 0,
            accuRewardLastUpdateBlock: MathUpgradeable.max(poolInfos[poolId].startBlock, block.number)
        });
    ...
    function _updateStakerReward(uint256 poolId, address staker) private {
        ...
        for (uint256 i = currentUserData.entryAccuRewardPerShare.length; i < poolData[poolId].rewardInfos.length; i++){
