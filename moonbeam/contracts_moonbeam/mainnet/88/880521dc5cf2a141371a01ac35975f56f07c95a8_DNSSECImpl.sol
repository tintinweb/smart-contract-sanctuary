...
interface DNSSEC {
    ...
    event RRSetUpdated(bytes name, bytes rrset, bytes20 hash);
...
contract DNSSECImpl is DNSSEC, Owned {
    ...
    constructor(bytes memory _anchors) public {
        ...
        emit RRSetUpdated(hex"00", anchors, bytes20(keccak256(anchors)));
    }
    ...
    function _submitRRSet(bytes memory input, bytes memory sig, bytes memory proof) internal returns (bytes memory) {
        ...
        emit RRSetUpdated(name, rrs, bytes20(keccak256(rrs)));
    ...
    function validateSignedSet(bytes memory input, bytes memory sig, bytes memory proof) internal view returns(bytes memory name, bytes memory rrs) {
        ...
        require(validProof(input.readName(RRSIG_SIGNER_NAME), proof),"validProof failed!!!!");
        ...
        require(name.labelCount(0) == labels,"labelCount");

        // TODO: Check inception and expiration using mod2^32 math

        // o  The validator's notion of the current time MUST be less than or
        //    equal to the time listed in the RRSIG RR's Expiration field.
        require(expiration > now,"expiration > now");

        // o  The validator's notion of the current time MUST be greater than or
        //    equal to the time listed in the RRSIG RR's Inception field.
        require(inception < now,"inception < now");
    ...
    function validProof(bytes memory name, bytes memory proof) internal view returns(bool) {
        ...
        bool isValid = rrsets[keccak256(name)][dnstype].hash == bytes20(keccak256(proof));
        return isValid;
    }
    ...
    function validateRRs(bytes memory data, uint16 typecovered) internal pure returns (bytes memory name) {
        ...
        for (RRUtils.RRIterator memory iter = data.iterateRRs(0); !iter.done(); iter.next()) {
            ...
            require(iter.class == DNSCLASS_IN,"bad DNSCLASS");

            if(name.length == 0) {
                name = iter.name();
            } else {
                // Name must be the same on all RRs
                require(name.length == data.nameLength(iter.offset),"bad name length");
                require(name.equals(0, data, iter.offset, name.length),"name NOT equal");
            }

            // o  The RRSIG RR's Type Covered field MUST equal the RRset's type.
            require(iter.dnstype == typecovered,"NOT typecovered");
        }
    }
    ...
    function verifySignature(bytes memory name, bytes memory data, bytes memory sig, bytes memory proof) internal view {
        ...
        require(signerNameLength <= name.length,"signerNameLength");
        require(data.equals(RRSIG_SIGNER_NAME, name, name.length - signerNameLength, signerNameLength),"RRSIG_SIGNER_NAME");
        ...
        if (dnstype == DNSTYPE_DS) {
            require(verifyWithDS(data, sig, offset, proof),"verifyWithDS failed");
        } else if (dnstype == DNSTYPE_DNSKEY) {
            require(verifyWithKnownKey(data, sig, proof),"verifyWithKnownKey failed");
        } else {
        ...
        returns (bool)
    {
        ...
        require (keytag-keytag==1,'avoid compiler warnings');
    ...
    function verifyDSHash(uint8 digesttype, bytes memory data, bytes memory digest) internal view returns (bool) {
        ...
        bool verified = (digests[digesttype].verify(data, digest.substring(4, digest.length - 4))==true);
    ...
    //require(verified==true, bytes32ToAsciiString(bytes32( uint256(digesttype) ),32));                                      // digesttype 01
    //require(verified==true, bytes32ToAsciiString(bytesMemoryTobytes32( data ),32));                                        // dump 32 bytes
    //require(verified==true, bytes32ToAsciiString( SHA1.sha1(data),32) );                                                   // SHA1.sha1(data) == 0xd023063f22215d99fb56c228dd9df2d55c51be15  000000000000000000000000 data hash expecting  
