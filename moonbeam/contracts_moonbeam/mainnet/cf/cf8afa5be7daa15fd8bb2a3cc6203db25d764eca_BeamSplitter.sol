...
import "./interfaces/IBeamERC20.sol";
import "./interfaces/IBeamPair.sol";
import "./interfaces/IBeamFactory.sol";

contract BeamSplitter is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IBeamFactory public immutable factory;

    address public sendToAddress; // where to send tokenTo
    address internal immutable wglmr;
    address public tokenTo; //any erc20
    uint256 public devCut; // in basis points aka parts per 10,000 so 5000 is 50%, cap of 50%, default is 0  7693
    ...
    modifier onlyAuth() {
        require(isAuth[msg.sender] || anyAuth, "Beamsplitter: FORBIDDEN");
        _;
    }
    ...
    constructor(
        ...
        factory = IBeamFactory(_factory);
        sendToAddress = _share;
        tokenTo = _tokenTo;
        wglmr = _wglmr;
        devAddr = msg.sender;
    ...
    function setsendToAddress(address _sendToAddress) external onlyOwner {
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != tokenTo && token != wglmr && token != bridge,
            "Beamsplitter: Invalid bridge"
        );
    ...
    function setDevCut(uint256 _amount) external onlyOwner {
        require(_amount <= 8000, "setDevCut: cut too high");
        devCut = _amount;
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "Beamsplitter: must use EOA");
        _;
    }

    // F1 - F10: OK
    // F3: _convert is separate to save gas by only checking the 'onlyEOA' modifier once in case of convertMultiple
    // F6: There is an exploit to add lots of TOKEN to the sendToAddress, run convert, then remove the TOKEN again.
    //     As the size of the BEAMSHARE has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    function _convert(address token0, address token1) internal {
        ...
        if (token0 == token1) {
            ...
            IBeamPair pair = IBeamPair(factory.getPair(token0, token1));
            require(address(pair) != address(0), "Beamsplitter: Invalid pair");
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == tokenTo) {
                IERC20(tokenTo).safeTransfer(sendToAddress, amount);
                tokenOut = amount;
            } else if (token0 == wglmr) {
                tokenOut = _toToken(wglmr, amount);
            } else {
            ...
            // eg. TOKEN - GLMR
            IERC20(tokenTo).safeTransfer(sendToAddress, amount0);
            tokenOut = _toToken(token1, amount1).add(amount0);
        } else if (token1 == tokenTo) {
            // eg. USDT - TOKEN
            IERC20(tokenTo).safeTransfer(sendToAddress, amount1);
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    function _toToken(address token, uint256 amountIn)
        internal
        returns (uint256 amountOut)
    {
        ...
        amountOut = _swap(token, tokenTo, amount, sendToAddress);
    }
