...
// File contracts/lpsplitter/interfaces/IBeamERC20.sol
...
// File contracts/lpsplitter/interfaces/IBeamPair.sol
...
// File contracts/lpsplitter/interfaces/IBeamFactory.sol
...
// File contracts/lpsplitter/BeamSplitter.sol
...
contract BeamSplitter is Ownable {
    ...
    uint256 public devCut = 7693; // in basis points aka parts per 10,000 so 5000 is 50%, cap of 50%, default is 0  7693
    ...
    // F6: There is an exploit to add lots of TOKEN to the share, run convert, then remove the TOKEN again.
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == tokenTo) {
                IERC20(tokenTo).safeTransfer(share, amount);
                tokenOut = amount;
            ...
            // eg. TOKEN - GLMR
            IERC20(tokenTo).safeTransfer(share, amount0);
            tokenOut = _toToken(token1, amount1).add(amount0);
        } else if (token1 == tokenTo) {
            // eg. USDT - TOKEN
            IERC20(tokenTo).safeTransfer(share, amount1);
        ...
        returns (uint256 amountOut)
    {
        ...
        amountOut = _swap(token, tokenTo, amount, share);
    }
