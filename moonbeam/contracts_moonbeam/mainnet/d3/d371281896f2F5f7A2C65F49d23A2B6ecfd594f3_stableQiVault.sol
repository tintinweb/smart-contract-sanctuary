...
contract fixedVault is ReentrancyGuard, VaultNFTv4 {
    ...
    function _vaultDebt(uint256 vaultID)
        internal
        view
    ...
    function vaultDebt(uint256 vaultID) public view returns (uint256) {
        (, uint256 debt) = _vaultDebt(vaultID);
        return debt;
    }

    function updateVaultDebt(uint256 vaultID) public returns (uint256) {
        (uint256 fee, uint256 debt) = _vaultDebt(vaultID);
...
contract stableQiVault is fixedVault, Ownable {
    ...
    /*
        Oracle price feed is used in here. Interface's available in the at /interfaces/IPriceSourceAll.sol
    */
    ...
    function setOpeningFee(uint256 _openingFee) external onlyOwner {
        require(_openingFee > FEE_MIN && _openingFee <= FEE_MAX, "setOpeningFee: cannot be more than 5%");
    ...
    function setClosingFee(uint256 _closingFee) external onlyOwner {
        require(_closingFee > FEE_MIN && _closingFee <= FEE_MAX, "setClosingFee: cannot be more than 5%");
