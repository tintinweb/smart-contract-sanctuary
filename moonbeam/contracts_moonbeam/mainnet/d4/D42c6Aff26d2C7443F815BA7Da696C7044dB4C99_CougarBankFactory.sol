...
import "./CougarBank.sol";

contract CougarBankFactory is Ownable {
    event NewCougarBankContract(address indexed cougarBank);
     ...
     * @return address of new cougar bank contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(CougarBank).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address cougarBankAddress;

        assembly {
            cougarBankAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        CougarBank(cougarBankAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewCougarBankContract(cougarBankAddress);
    }
}
...
contract CougarBank is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function emittedReward() external view returns (uint256) {
        uint256 emitted = 0;
        if (block.number > startBlock) {
            emitted = (block.number-startBlock)*rewardPerBlock;
        }
        return emitted;
    }
