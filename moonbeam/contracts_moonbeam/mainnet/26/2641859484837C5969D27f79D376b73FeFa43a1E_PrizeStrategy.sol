    ...
    UUPSUpgradeable
{
    ...
    ) external override whenNotPaused onlyRole(VAULT_MANAGER_ROLE) {
    ...
    /**
     * @notice used to change the address of the Binary Search smart contract
     * @param _binarySearchAddress new address of the Binary Search smart contract
     */
    function setBinarySearch(address _binarySearchAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        binarySearch = IBinarySearch(_binarySearchAddress);
     ...
     */
    function computeUserReward(uint256 _vaultId, address _user)
        external
        override
        whenNotPaused
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        uint256 rewardAmount = multiToken
            ...
            .div(10000);
        multiToken.increaseReward(_user, _vaultId, rewardAmount);
    }

    function getRewardCoefficient(uint256 _vaultId)
        external
        view
        override
        returns (uint256)
    {
        return rewardCoefficients[_vaultId];
    }

    function _getTotalNumberOfPrizes(uint256 _vaultId)
        internal
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < tiers[_vaultId].length; i++) {
            numberOfPrizes += tiers[_vaultId][i];
        }
    ...
    function getPrizesAndWinners(
        ...
        returns (uint256[] memory _prizes, address[] memory _winners)
    {
        uint256 numberOfPrizes = _getTotalNumberOfPrizes(_vaultId);

        // randomNumbersArray.length == numberOfPrizes
        uint256[] memory randomNumbersArray = _getRandomNumbers(
            _randomNumber,
            numberOfPrizes,
            _totalMax
        );

        // _prizes.length == numberOfPrizes
        _prizes = _getPrizesArr(numberOfPrizes, _vaultId);

        // _winners.length == numberOfPrizes
        _winners = _getWinnerBinarySearch(
            accumulatedTwaArr,
            randomNumbersArray,
            _participants
        );

        emit Winners(_vaultId, _winners, _prizes);
    }

    function _getWinnerBinarySearch(
        uint256[] memory accumlatedTwaArr,
        uint256[] memory randomNumbersArray,
        address[] memory _participants
    ) internal returns (address[] memory winnersBinarySearch) {
        winnersBinarySearch = new address[](randomNumbersArray.length);
        for (uint256 i = 0; i < randomNumbersArray.length; i++) {
            uint256 w = binarySearch.binarySearch(
                accumlatedTwaArr,
                randomNumbersArray[i]
            );
            winnersBinarySearch[i] = _participants[w];
        }
    }
    ...
    {
        uint256[] memory _tiers = tiers[_vaultId];
        _prizesArr = new uint256[](_numberOfPrizes);
        uint256 ix = 0;
        for (uint256 i = 0; i < _tiers.length; i++) {
            for (uint256 j = 0; j < _tiers[i]; j++) {
                _prizesArr[ix] = percentages[_vaultId][i]
                    .mul(prizes[_vaultId])
                    .div(10000);
                ix += 1;
            }
     ...
     * @param _amountOfRandomNumbers quantity of random numbers
     * @param _totalMax cap for calculating random number
     * @return randomNumbersArray an array of pseudo-numbers and the coresponding
     * tier for each of them
     */
    function _getRandomNumbers(
        uint256 _randomNumber,
        uint256 _amountOfRandomNumbers,
        uint256 _totalMax
    ) internal pure returns (uint256[] memory randomNumbersArray) {
        randomNumbersArray = new uint256[](_amountOfRandomNumbers);
        for (uint256 i = 0; i < _amountOfRandomNumbers; i++) {
            randomNumbersArray[i] = UniformRandomNumber.uniform(
