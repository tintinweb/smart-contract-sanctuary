...
contract BestRateQuery {
    ...
    constructor(INarwhal _nar) public {
        narwhal = _nar;
    }
    ...
    returns (
        ...
        // uint256 _amountOut = narwhal.getDestinationReturnAmount(routes[i], src, dest, srcAmount, partnerIndex);
        bytes memory payload = abi.encodeWithSelector(narwhal.getDestinationReturnAmount.selector, route, src, dest, srcAmount, partnerIndex);
        (bool success, bytes memory data) = address(narwhal).staticcall(payload);
        if (success) {
