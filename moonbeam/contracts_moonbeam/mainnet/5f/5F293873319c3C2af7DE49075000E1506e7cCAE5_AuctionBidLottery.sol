...
contract AuctionBidLottery is RrpRequesterV0 {
    ...
    event WinnerSelected(address lotteryWinner);
    ...
    // Adds addresses to the list of lottery candidates
    function addBidders(address[] calldata addresses) external onlyOwner {
        for (uint256 i; i < addresses.length; i++) {
            auctionBidders.push(addresses[i]);
        }
    }

    // Seed the randomness array which will be used to select winners
    function plantSeeds(uint256 numSeeds) external onlyOwner {
        _makeRequestUint256Array(numSeeds);
    }

    // select lotteryWinner and remove them from the candidates array. This is not
    // very efficient, but we're not concerned as it only needs to be run once until
    // the winners array is full
    function selectWinner() external onlyOwner {
        require(randomSeeds.length > 0, "No random seeds");
        require(!winnerSelected, "Winner has already been selected");

        // pick a seed from the array
        uint256 seed = randomSeeds[0];

        uint256 randomIndex = uint256(
            keccak256(abi.encodePacked(block.number - 1, seed))
        ) % auctionBidders.length;

        winnerSelected = true;
        lotteryWinner = auctionBidders[randomIndex];

        // remove the seed so it cannot be resused
        randomSeeds.pop();
        emit WinnerSelected(lotteryWinner);
    }
