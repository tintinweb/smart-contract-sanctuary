/**
 *Submitted for verification at moonbeam.moonscan.io on 2022-03-15
*/
...
interface IStrategy {
    ...
    function noTimeLockFunc(uint256 number) external;
...
contract TimelockController_RewardsDistributor3 is
    AccessControl,
    ReentrancyGuard
{
    using SafeMath for uint256;
    ...
    // // In case new vaults require functions without a timelock as well, hoping to avoid having multiple timelock contracts
    function noTimeLockFunc(uint256 number, address _stratAddress)
        public
        onlyRole(EXECUTOR_ROLE)
    {
        IStrategy(_stratAddress).noTimeLockFunc(number);
    }
