    ...
    UUPSUpgradeable
{
    ...
    IERC20Upgradeable internal stableToken;
    ...
    function participantWithdraw(
        ...
        uint256 lockDuration = vaultsList.get(_vaultId).lockDuration;
        if (state == VaultsStorageLibrary.VaultState.Yielding) {
            require(
                blockNumberUpdated.add(lockDuration) <= block.number,
     ...
     */
    function transferAndBurn(uint256 _vaultId, address _participant) internal {
        IRandStableToken randStableToken = IRandStableToken(
            address(stableToken)
        );
