...
import "../../interfaces/stellaswap/IStellaRouter.sol";
...
contract StrategyStellaswapStable is StratManager, FeeManager {
    ...
    function chargeFees(address callFeeRecipient) internal {
        if (rewardToOutputRoute.length != 0) {
            for (uint i; i < rewardToOutputRoute.length; i++) {
                ...
                if (rewardBal > 0) {
                    IStellaRouter(unirouter).swapExactTokensForTokens(rewardBal, 0, rewardToOutputRoute[i], address(this), now);
                }
            }
        }

        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(45).div(1000);
        IStellaRouter(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);
    ...
    function addLiquidity() internal {
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        IStellaRouter(unirouter).swapExactTokensForTokens(outputBal, 0, outputToInputRoute, address(this), now);
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeBal;
        try IStellaRouter(unirouter).getAmountsOut(rewardBal[0], outputToNativeRoute)
            ...
            for (uint i; i < rewardToOutputRoute.length; i++) {
                for (uint j = 1; j < rewardAdd.length; j++) {
                    if (rewardAdd[j] == rewardToOutputRoute[i][0]) {
                        try IStellaRouter(unirouter).getAmountsOut(rewardBal[j], rewardToOutputRoute[i])
                        returns (uint256[] memory initialAmountOut) {
                            uint256 outputBal = initialAmountOut[initialAmountOut.length - 1];
                            try IStellaRouter(unirouter).getAmountsOut(outputBal, outputToNativeRoute)
