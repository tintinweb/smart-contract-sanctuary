...
contract SwapCalculator is BaseBoringBatchable {
    ...
    function calculateRemoveLiquidityOneToken(
        ...
        uint256 totalSupply = ISwap(pool).getLpToken().totalSupply();
        (, , , , uint256 swapFee, , , , , ) = ISwap(pool).swapStorage();
        ...
        //Current D, the StableSwap invariant
        if (v.totalSupply != 0) {
            v.d0 = getD(_xp(v.balances, v.multipliers), v.preciseA);
        }

        //Invariant after changes
        uint256[] memory newBalances = new uint256[](decimalsArr.length);
        for (uint256 i = 0; i < decimalsArr.length; i++) {
            if (i == tokenIndex) {
                newBalances[i] = v.balances[i].sub(tokenAmount);
            } else {
                newBalances[i] = v.balances[i];
            }
        }
        ...
        if (v.totalSupply != 0) {
            uint256 feePerToken = swapFee.mul(decimalsArr.length).div(
                decimalsArr.length.sub(1).mul(4)
            );
            for (uint256 i = 0; i < decimalsArr.length; i++) {
