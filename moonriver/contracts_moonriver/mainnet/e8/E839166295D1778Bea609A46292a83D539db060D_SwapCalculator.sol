...
contract SwapCalculator is BaseBoringBatchable {
    ...
    function calculateLiquidityOutput(
        ...
        ManageLiquidityInfo memory v = ManageLiquidityInfo(
            ...
            multipliers,
            0,
            new uint256[](decimalsArr.length),
            new uint256[](decimalsArr.length),
            new uint256[](decimalsArr.length)
        );
        ...
        )
    {
...

        {
            ...
            for (uint256 i = 0; i < decimalsArr.length; i++) {
                if (tokenIndex == i) {
                    v.balances1[i] = v.balances[i].sub(
                        tokenAmount,
                        "Cannot withdraw more than available"
                    );
                } else {
                    v.balances1[i] = v.balances[i];
                }
            }
            ...
            v.d2 = getD(v.xpReduced, v.preciseA);

            v.d3 = getD(_xp(v.balances1, v.multipliers), v.preciseA);
        }
        ...
        return (
            ...
            v.d1,
            v.d2,
            v.d0.sub(v.d2).mul(v.totalSupply).div(v.d0).add(1),
            v.d0.sub(v.d3).mul(v.totalSupply).div(v.d0).add(1)
        );
    }
}
