...
contract SwapCalculator is BaseBoringBatchable {
    ...
    function getXP(address pool) external view returns (uint256[] memory) {
        uint256[] memory decimalsArr = storedDecimals[pool];
        uint256[] memory balances = new uint256[](decimalsArr.length);
        ...
        uint256[] memory xp = new uint256[](decimalsArr.length);
        for (uint256 i = 0; i < decimalsArr.length; i++) {
            xp[i] = balances[i].mul(multipliers[i]);
        }
        return xp;
    }

    function getD(uint256[] memory xp, address pool)
        external
        view
        returns (uint256)
    {
        return getD(xp, ISwap(pool).getAPrecise());
    }
    ...
    function getYD(
        ...
        return getYD(ISwap(pool).getAPrecise(), tokenIndex, xp, d);
    }
    ...
    function calculateLiquidityInput(
        ...
        uint256[] memory xpReduced = new uint256[](xp.length);
        {
            ...
            for (uint256 i = 0; i < xp.length; i++) {
                ...
                uint256 difference = (i == tokenIndex)
                    ? newY.mul(v.d1).div(v.d0).sub(xpi)
                    : xpi.mul(v.d1).div(v.d0).sub(xpi);

                xpReduced[i] = xpi.add(
                    difference.mul(feePerToken).div(FEE_DENOMINATOR)
                );
            }
        }

        //new token amount considering fees
        uint256 dy = getYD(v.preciseA, tokenIndex, xpReduced, v.d1).sub(
            xpReduced[tokenIndex]
        );
        dy = dy.add(1).div(v.multipliers[tokenIndex]);
