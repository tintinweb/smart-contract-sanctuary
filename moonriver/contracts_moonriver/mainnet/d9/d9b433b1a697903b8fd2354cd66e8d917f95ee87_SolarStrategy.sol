...
contract SolarStrategy is BayStrategy {
    ...
    constructor(
        uint256 _poolId,
        BayVault _vault,
        BayRegistry _registry,
        ISolarRouter02 _router,
        ISolarDistributorV2 _chef,
        address[] memory _outputToNativeRoute,
        address[] memory _outputToLp0Route,
        address[] memory _outputToLp1Route
    ) BayStrategy(_vault, _registry) {
        poolId = _poolId;
        vault = _vault;
        router = _router;
        chef = _chef;
        depositToken = address(_vault.asset());
        ...
        // setup lp routing
        lpToken0 = ISolarPair(depositToken).token0();

        if (_outputToLp0Route[0] != address(output))
            revert InvalidRoute("outputToLp0Route[0] != output");
        if (_outputToLp0Route[_outputToLp0Route.length - 1] != lpToken0)
            revert InvalidRoute("outputToLp0Route[last] != lpToken0");

        outputToLp0Route = _outputToLp0Route;

        lpToken1 = ISolarPair(depositToken).token1();

        if (_outputToLp1Route[0] != address(output))
            revert InvalidRoute("outputToLp1Route[0] != output");
        if (_outputToLp1Route[_outputToLp1Route.length - 1] != lpToken1)
            revert InvalidRoute("outputToLp1Route[last] != lpToken0");
    ...
    function retireStrat() external onlyVault {
        ...
        IERC20(depositToken).safeTransfer(address(vault), depositTokenBal);
    }
    ...
    function unpause() external onlyOwner {
        ...
        registry.resumeStrategy(this);

        depositAll();
    }

    function _giveAllowances() internal {
        IERC20(depositToken).safeApprove(address(chef), MAX_UINT);
        IERC20(output).safeApprove(address(router), MAX_UINT);

        IERC20(lpToken0).safeApprove(address(router), 0);
        IERC20(lpToken0).safeApprove(address(router), MAX_UINT);

        IERC20(lpToken1).safeApprove(address(router), 0);
        IERC20(lpToken1).safeApprove(address(router), MAX_UINT);
    }

    function _removeAllowances() internal {
        IERC20(depositToken).safeApprove(address(chef), 0);
        IERC20(output).safeApprove(address(router), 0);
    ...
    function deposit(uint256 amount) external virtual override whenNotPaused {
        uint256 depositTokenBal = ISolarPair(depositToken).balanceOf(
            address(this)
        );

        if (depositTokenBal >= amount) _deposit(amount);
        else revert InsufficientBalance();
    }

    function depositAll() public virtual override whenNotPaused {
        uint256 depositTokenBal = ISolarPair(depositToken).balanceOf(
            address(this)
        );

        if (depositTokenBal > 0) _deposit(depositTokenBal);
    }

    function _deposit(uint256 amount) internal {
        ISolarDistributorV2(chef).deposit(poolId, amount);
    }

    /// @notice Called by the vault before depositing funds.
    ...
    function _harvest(address harvestRewardRecipient) internal {
        ISolarDistributorV2(chef).deposit(poolId, 0);
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        if (outputBal > 0) {
            chargeFees(harvestRewardRecipient);
            addLiquidity();
            uint256 depositTokenHarvested = balanceOfDeposit();
            _deposit(outputBal);
            lastHarvest = block.timestamp;
            emit StratHarvest(msg.sender, depositTokenHarvested, balance());
        }
    }
    ...
    function addLiquidity() internal {
        ...
        ISolarRouter02(router).addLiquidity(
            lpToken0,
            lpToken1,
            lp0Bal,
            lp1Bal,
    ...
    function chargeFees(address harvestRewardRecipient) internal {
        ...
        uint256 callFeeAmount = nativeBal.mul(harvestRewardBps).div(
            totalFeeBps
        );
        IERC20(native).safeTransfer(harvestRewardRecipient, callFeeAmount);

        uint256 vaultFeeAmount = nativeBal.mul(vaultFeeBps).div(totalFeeBps);
        IERC20(native).safeTransfer(vault.bayTreasury(), vaultFeeAmount);

        uint256 strategistFee = nativeBal.mul(strategistFeeBps).div(
            totalFeeBps
        );
    ...
    function withdraw(uint256 amount) external virtual override onlyVault {
        ...
        if (depositTokenBal < amount) {
            ISolarDistributorV2(chef).withdraw(
                poolId,
                amount.sub(depositTokenBal)
            );
            depositTokenBal = ISolarPair(depositToken).balanceOf(address(this));
        }
    ...
    function callReward() public view returns (uint256) {
        ...
        if (outputBal > 0) {
            try
                ISolarRouter02(router).getAmountsOut(
                    outputBal,
                    outputToNativeRoute,
                    25
                )
        ...
        return
            nativeBal.mul(45).div(1000).mul(harvestRewardBps).div(BPS_DIVISOR);
    }
