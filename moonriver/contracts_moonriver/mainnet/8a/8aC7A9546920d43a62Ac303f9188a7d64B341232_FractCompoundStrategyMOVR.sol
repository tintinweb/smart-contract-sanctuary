...
contract FractCompoundStrategyMOVR is FractCompoundStrategy {
    ...
    address public constant MOVR_TOKEN = 0x98878B06940aE243284CA214f92Bb71a2b032B8A;
     ...
     * @param rewardType The reward to claim. 0 for MFAM, 1 for MOVR.
    ...
    */ 
     ...
     * @param rewardType The reward to claim. 0 for MFAM, 1 for MOVR.
    */ 
    function harvestAll(uint8 rewardType, address borrowAddress) external onlyOwner {
        uint256 rewardBalance;
        address underlyingAddress;
        claimAllRewards(rewardType);
        rewardBalance = IERC20(rewardToken).balanceOf(address(this));
        underlyingAddress = CErc20Interface(borrowAddress).underlying();
        if (rewardType == 0) {
            address rewardToken = MOONWELL_TOKEN;
     ...
     */ 
    function claimRewardsByMarket(uint8 rewardType, address mintAddress, address borrowAddress) public onlyOwner {
        address[] memory claimAddresses = new address[](2);
        claimAddresses[0] = mintAddress;
        claimAddresses[1] = borrowAddress;
        //claim tokens
        comptroller.claimReward(rewardType, address(this), claimAddresses);
    }
     ...
     * @param rewardType The reward to claim. 0 for MFAM, 1 for MOVR.
     */ 
    function claimAllRewards(uint8 rewardType) public onlyOwner {
        comptroller.claimReward(rewardType, address(this));
    }
}
...
abstract contract FractCompoundStrategy is FractStrategyV1 {
    ...
    function repayAndRedeem(
        ...
        redeem(mintAddress, borrowAddress, underlyingRedeemAmount);
    }
     ...
     */
    function redeem(address mintAddress, address borrowAddress, uint256 underlyingRedeemAmount) public onlyOwner {
        CErc20Interface cErcTokenMint = CErc20Interface(mintAddress);
        uint256 redeemError = cErcTokenMint.redeemUnderlying(underlyingRedeemAmount);
        ...
        if (totalCollateral == 0) {
            CErc20Interface cErcTokenBorrow = CErc20Interface(borrowAddress);
            uint256 outstandingDebt = cErcTokenBorrow.borrowBalanceCurrent(address(this));
            require(outstandingDebt == 0, "Redeem failed");
            totalDebt = 0;
        }
    }
     ...
     */
    function repayBorrow(address borrowAddress) public onlyOwner {
        CErc20Interface cErcToken = CErc20Interface(borrowAddress);
        address underlyingAddress = cErcToken.underlying();
        uint256 underlyingBalance = IERC20(underlyingAddress).balanceOf(address(this));
        uint256 normalizedDebt = normalizeValue(underlyingBalance, borrowAddress);
        IERC20(underlyingAddress).approve(borrowAddress, underlyingBalance);
        uint256 repayError = cErcToken.repayBorrow(underlyingBalance);
    ...
    function borrow(address borrowAddress, uint256 collateralFactorBips) internal onlyOwner returns(uint256) {
        ...
        uint256 collateralPrice = getUnderlyingPrice(borrowAddress);
     ...
     * @param cTokenAddress The cToken address to use. 
     */
    function getUnderlyingPrice(address cTokenAddress) public view virtual onlyOwner returns(uint256){
