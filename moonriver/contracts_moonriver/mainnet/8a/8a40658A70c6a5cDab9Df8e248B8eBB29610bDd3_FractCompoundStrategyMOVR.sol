...
abstract contract FractCompoundStrategy is FractStrategyV1 {
    ...
    function repayAndRedeem(
        address mintAddress,
        address borrowAddress,
        uint256 underlyingRedeemAmount
    ) external virtual onlyOwner 
    {
        require(mintAddress != address(0), "0 Address");
        require(borrowAddress != address(0), "0 Address");
        require(underlyingRedeemAmount > 0, "Redeem failed");

        repayBorrow(borrowAddress);
        redeemUnderlying(mintAddress, borrowAddress, underlyingRedeemAmount);
    }

    /**
     * @notice Redeem cTokens for underlying tokens. Only call when borrowBalance = 0. 
    ...
    function redeemTokens(
        ...
        uint256 cTokenBalance = cErcToken.balanceOf(address(this));
        uint256 redeemError = cErcToken.redeem(cTokenBalance);

        require(redeemError == 0, "Redeem failed");

        CErc20Interface cErcTokenBorrow = CErc20Interface(borrowAddress);
        uint256 outstandingDebt = cErcTokenBorrow.borrowBalanceStored(
            address(this)
        );
    ...
    ) public onlyOwner 
    {
        ...
        uint256 redeemError = cErcTokenMint.redeemUnderlying(
    ...
    function repayBorrow(address borrowAddress) public onlyOwner {
        ...
        uint256 underlyingBalance = IERC20(underlyingAddress).balanceOf(
            address(this)
        );

        IERC20(underlyingAddress).approve(borrowAddress, underlyingBalance);

        uint256 borrowBalance = cErcToken.borrowBalanceStored(address(this));
        if (underlyingBalance > borrowBalance) {
            uint256 repayError = cErcToken.repayBorrow(borrowBalance);
            require(repayError == 0, "Repay Failed");
            //check borrow balance again
            uint256 remainingBalance = cErcToken.borrowBalanceStored(address(this));
            if (remainingBalance != 0) {
                uint256 repayError = cErcToken.repayBorrow(remainingBalance);
                require(repayError == 0, "Repay Failed");
                //final check
                uint256 finalBalance = cErcToken.borrowBalanceStored(address(this));
                require(finalBalance == 0, 'Repay Failed');
            }
        }
        else {
    ...
    /**
     * @notice Swap tokens through the specified swapRouter.
    ...
    {
        require(tokenIn != address(0), "0 Address");
        require(tokenOut != address(0), "0 Address");
    ...
    /**
     * @notice Swap eth(or native evm token) through the specified swapRouter.
     * @param tokenOut The token to receive.
     * @param ethAmount The amount of eth we want to swap.
     */
    function swapETH(address tokenOut, uint256 ethAmount) internal onlyOwner {
        require(tokenOut != address(0), "0 Address");
        require(ethAmount > 0, "0 amount");

        address[] memory path;
        path = new address[](2);
        path[0] = chainToken;
        path[1] = tokenOut;
        swapRouter.swapExactETHForTokens{value: ethAmount}(
            0, //min amount out
            path,
            address(this),
            block.timestamp + 30
    ...
    function mint(address mintAddress, uint256 mintAmount) internal onlyOwner {
        ...
        ComptrollerInterface comptroller = CTokenStorage(mintAddress)
            .comptroller();
     ...
     */
    function borrow(address borrowAddress, uint256 collateralFactorBips)
        internal
        onlyOwner
    {
        ...
        //set comptroller
        ComptrollerInterface comptroller = CTokenStorage(borrowAddress)
            .comptroller();
        CErc20Interface cErcToken = CErc20Interface(borrowAddress);
        ...
        require(liquidity > 0, "No excess collateral");
        uint256 collateralPrice = getUnderlyingPrice(borrowAddress);
        uint256 maxBorrow = liquidity * ONE_ETHER / collateralPrice;
        uint256 borrowAmount = maxBorrow * collateralFactorBips /
            BIPS_DIVISOR;

        uint256 borrowError = cErcToken.borrow(borrowAmount);
     ...
     */
    function normalizeValue(uint256 amount, address cTokenAddress)
        internal
        view
        returns (uint256)
    {
        address underlyingAddress = CErc20Interface(cTokenAddress).underlying();
        uint256 underlyingDecimals = CTokenStorage(underlyingAddress)
            .decimals();
    ...
    {
        address underlyingAddress = CErc20Interface(cTokenAddress).underlying();
        uint256 underlyingBalance = CErc20Interface(underlyingAddress)
            .balanceOf(address(this));
        return underlyingBalance;
    }

    /**
     * @notice Get the collateral balance based on mint address.
     * @param mintAddress The cToken mint address to use.
     */

    function getCollateralBalance(address mintAddress)
        public
        view
        virtual
        returns (uint256)
    {
        CErc20Interface cErcToken = CErc20Interface(mintAddress);
        uint256 exchangeRate = cErcToken.exchangeRateStored();
        uint256 cTokenBalance = cErcToken.balanceOf(address(this));
        uint256 collateralBalance = exchangeRate * cTokenBalance / ONE_ETHER;
        return collateralBalance;
    }

    /**
     * @notice Get the borrow balance based on borrow address.
     * @param borrowAddress The cToken borrow address to use.
     */
    function getDebtBalance(address borrowAddress)
        public
        view
        virtual
        returns (uint256)
    {
        CErc20Interface cErcToken = CErc20Interface(borrowAddress);
        uint256 debtBalance = cErcToken.borrowBalanceStored(address(this));
        return debtBalance;
    }
