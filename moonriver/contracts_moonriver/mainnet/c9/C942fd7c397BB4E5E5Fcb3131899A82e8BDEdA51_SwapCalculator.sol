...
contract SwapCalculator is BaseBoringBatchable {
    ...
    function calculateLiquidityOutput(
        ...
        uint256 feePerToken = _feePerToken(swapFee, decimalsArr.length);

        uint256[] memory fees = new uint256[](decimalsArr.length);
        {
            v.d0 = getD(_xp(v.balances, v.multipliers), v.preciseA);
            v.balances1[tokenIndex] = v.balances[tokenIndex].sub(
                tokenAmount,
                "Cannot withdraw more than available"
            );
            v.d1 = getD(_xp(v.balances1, v.multipliers), v.preciseA);

            for (uint256 i = 0; i < decimalsArr.length; i++) {
                uint256 idealBalance = v.d1.mul(v.balances[i]).div(v.d0);
                uint256 difference = idealBalance.difference(v.balances1[i]);
                fees[i] = feePerToken.mul(difference).div(FEE_DENOMINATOR);
                v.balances1[i] = v.balances1[i].sub(fees[i]);
            }

            v.d2 = getD(_xp(v.balances1, v.multipliers), v.preciseA);
        }
    ...
    function calculateRemoveLiquidityOutput(
        address pool,
        uint256 tokenAmount,
        uint8 tokenIndex
    )
        ...
        )
    {
        ...
        uint256 feePerToken = _feePerToken(swapFee, decimalsArr.length);
        {
            v.xp = _xp(v.balances, v.multipliers);

            v.d0 = getD(v.xp, v.preciseA);
            ...
            v.newY = getYD(v.preciseA, tokenIndex, v.xp, v.d1);

            uint256[] memory xpReduced = new uint256[](decimalsArr.length);
            ...
            for (uint256 i = 0; i < decimalsArr.length; i++) {
                ...
                // uint256 idealBalance = v.d1.mul(v.balances[i]).div(v.d0);
                // uint256 difference = idealBalance.difference(v.balances1[i]);
                // v.balances1[i] = v.balances1[i].sub(
                //     feePerToken.mul(difference).div(FEE_DENOMINATOR)
                // );

                // uint256 idealBalance = v.d1.mul(v.balances[i]).div(v.d0).sub(
                //     i == tokenIndex ? v.newY : 0
                // );
                ...
                uint256 dxExpected;
                if (i == tokenIndex) {
                    dxExpected = xpi.mul(v.d1).div(v.d0).sub(v.newY);
                } else {
                    dxExpected = xpi.sub(xpi.mul(v.d1).div(v.d0));
                }
                xpReduced[i] = xpi.sub(
                    dxExpected.mul(feePerToken).div(FEE_DENOMINATOR)
                );
            }

            v.d2 = getD(xpReduced, v.preciseA);
        }

        uint256 lpAmount = v.d0.sub(v.d2).mul(v.totalSupply).div(v.d0);
        lpAmount = lpAmount.add(1);
        return (v.newY, v.balances1[tokenIndex], v.d1, v.d0, lpAmount);
    }
}
