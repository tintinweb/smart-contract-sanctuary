...
contract SwapCalculator is BaseBoringBatchable {
    ...
    function calculateSwapInput(
        ...
        outputAmount = outputAmount.mul(multipliers[outputIndex]);
        ...
        inputAmount = calculateSwapInputCustom(
            ...
            outputIndex,
            outputAmount
        ).div(multipliers[inputIndex]).add(1);
    }
    ...
    /**
     * @notice Calculate the liquidity amount (LP Token) an user would receive by withdrawing one token
     * This function is used as a helper function to calculate how much LP token
     * the user should receive on removing liquidity imbalance.
     *
     * @param pool pool address
     * @param inputIndex index of withdrawing token
     * @param inputAmount the total amount of token user is withdrawing
     * @return the amount of Liquidity Tokens that the user should receive
     */
    function calculateLiquidityAmountWithdrawOneToken(
        address pool,
        uint8 inputIndex,
        uint256 inputAmount
        ...
        for (uint256 i = 0; i < decimalsArr.length; i++) {
            v.multipliers[i] = 10**BALANCE_DECIMALS.sub(decimalsArr[i]);
            v.balances[i] = ISwap(pool).getTokenBalance(uint8(i));
        }

        v.totalSupply = v.lpToken.totalSupply();

        v.feePerToken = swapFee.mul(uint256(decimalsArr.length)).div(
            uint256(decimalsArr.length).sub(1).mul(4)
        );

        //D invariant before balance change
        v.d0 = getD(_xp(v.balances, v.multipliers), v.preciseA);

        //Change balances
        for (uint256 i = 0; i < decimalsArr.length; i++) {
            if (inputIndex == i) {
                v.balancesAux[i] = v.balances[i].sub(
                    inputAmount,
                    "Cannot withdraw more than available"
                );
            } else {
                v.balancesAux[i] = v.balances[i];
            }
        }

        // D invariant after balance change
        v.d1 = getD(_xp(v.balancesAux, v.multipliers), v.preciseA);
        ...
        for (uint256 i = 0; i < decimalsArr.length; i++) {
            ...
            v.balancesAux[i] = v.balancesAux[i].sub(
                v.feePerToken.mul(diff).div(FEE_DENOMINATOR)
            );
        }

        v.d2 = getD(_xp(v.balancesAux, v.multipliers), v.preciseA);

        uint256 lpAmount = v.d0.sub(v.d2).mul(v.totalSupply).div(v.d0).add(1);
        return lpAmount;
    }

    /**
     * @notice Calculate the TOKEN amount an user would receive by an AMOUNT OF LP into liquidity
     * This function is used as a helper function to calculate how much OUTPUT INDEX token amount
     * the user should receive on adding liquidity into the pool
     *
     * @param pool pool address
     * @param outputIndex index of token user wants to receive
     * @param lpAmount the total amount of lp user is withdrawing
     * @return the amount of tokens that the user should receive
     */
    function calculateOneTokenAmountAddLiquidity(
        address pool,
        uint8 outputIndex,
        uint256 lpAmount
    ) external view returns (uint256) {
        ...
        //D invariant after adding LP Amount
        v.d1 = v.d0.add(lpAmount.mul(v.d0).div(v.totalSupply));

        //Expected TOKEN_INDEX amount considering initial balances at the price after adding LP Amount
        uint256 newY = getYD(v.preciseA, outputIndex, xp, v.d1);

        //Change balances
        for (uint256 i = 0; i < decimalsArr.length; i++) {
            uint256 idealBalance = v.d1.mul(xp[i]).div(v.d0);
            uint256 diff = (outputIndex == i)
                ? idealBalance.difference(newY)
                : idealBalance.difference(xp[i]);
            uint256 fee = diff.mul(v.feePerToken).div(FEE_DENOMINATOR);
            uint256 nA = v.preciseA.mul(xp.length);
            v.balancesAux[i] = xp[i].sub(
                fee.add(fee.mul(A_PRECISION).div(nA.mul(xp.length)))
            );
        }

        uint256 newYD = getYD(v.preciseA, outputIndex, v.balancesAux, v.d1).div(
            v.multipliers[outputIndex]
        );

        //new token amount considering fees
        uint256 dy = newYD
            .sub(v.balancesAux[outputIndex].div(v.multipliers[outputIndex]))
            .add(1);
