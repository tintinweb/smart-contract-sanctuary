...
contract MovrPlace is ReentrancyGuard {
  ...
  address private constant owner2 = 0x9187CA4C4426EEA21A97FA1c415836fF75b06a69;
  ...
  function addCommunity(string calldata _communityId) external onlyOwner {
    communities.push(_communityId);
  }

  function addCharity(string calldata _charityId, address _disbursementAddr) external onlyOwner {
    charities.push(_charityId);
    disbursementAddr[_charityId] = _disbursementAddr;
  }

  function disburseRaised(string calldata _charityId) external onlyOwner nonReentrant {
    address _disbursementAddr = disbursementAddr[_charityId];

    require(_disbursementAddr != address(0), "Not a valid charity");

    uint256 _disbursementAmt = totalByCharity[_charityId] - amtDisbursedToCharity[_charityId];

    amtDisbursedToCharity[_charityId] += _disbursementAmt;
  ...
  function getCommunityTotal(string calldata _communityId) public view returns(uint) {
    return totalByCommunity[_communityId];
  }

  function getCharityTotal(string calldata _charityId) public view returns(uint) {
  ...
  function emergencyWithdraw() external onlyOwner {
      _withdraw(owner1, address(this).balance);
  }
