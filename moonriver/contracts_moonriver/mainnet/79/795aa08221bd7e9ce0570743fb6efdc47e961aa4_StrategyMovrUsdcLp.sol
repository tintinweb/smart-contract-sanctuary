/**
 *Submitted for verification at moonriver.moonscan.io on 2022-03-10
*/
...
abstract contract StrategySolarFarmBase is StrategyBase {
    ...
    function harvest() public override onlyBenevolent {
        ...
        if (_solar > 0) {
            if (uniswapRoutes[movr].length > 1) {
                _swapSushiswapWithPath(uniswapRoutes[movr], _solar);
            }
        }
        uint256 _movr = address(this).balance;
        WETH(movr).deposit{value: _movr}();
        ...
        if(_movr > 0){
            ...
            if (uniswapRoutes[token0].length > 1 && token0 != movr) {
                _swapSushiswapWithPath(uniswapRoutes[token0], toToken0);
            }
            if (uniswapRoutes[token1].length > 1 && token1 != movr) {
...
contract StrategyMovrUsdcLp is StrategySolarFarmBase {
    uint256 public movr_usdc_poolId = 10;
