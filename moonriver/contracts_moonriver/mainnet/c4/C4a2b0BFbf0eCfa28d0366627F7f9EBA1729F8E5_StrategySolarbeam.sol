...
// File contracts/BIFI/strategies/Solarbeam/StrategySolarbeam.sol
...
contract StrategySolarbeam is StratManager, FeeManager {
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeBal;
        uint256[] memory amountOut = ISolarRouter(unirouter).getAmountsOut(rewardBal[0], outputToNativeRoute, 25);
            ...
            for (uint i; i < rewardToOutputRoute.length; i++) {
                for (uint j = 1; j < rewardAdd.length; j++) {
                    if (rewardAdd[j] == rewardToOutputRoute[i][0]) {
                        uint256[] memory initialAmountOut = ISolarRouter(unirouter).getAmountsOut(rewardBal[j], rewardToOutputRoute[i], 25);
                        uint256 outputBal = initialAmountOut[initialAmountOut.length - 1];
                        uint256[] memory finalAmountOut = ISolarRouter(unirouter).getAmountsOut(outputBal, outputToNativeRoute, 25);
