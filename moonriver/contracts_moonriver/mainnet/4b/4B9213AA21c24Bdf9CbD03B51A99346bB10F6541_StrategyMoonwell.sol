...
contract StrategyMoonwell is StratManager, FeeManager {
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        if (IComptroller(comptroller).pendingComptrollerImplementation() == address(0)) {
            ...
            uint256 amountOutFromOutput = IUniswapRouter(unirouter).getAmountsOut(outputBal, outputToWantRoute)[outputToWantRoute.length - 1];
            uint256 amountOutFromNative = IUniswapRouter(unirouter).getAmountsOut(nativeBal, nativeToWantRoute)[nativeToWantRoute.length - 1];
            if (amountOutFromOutput > 0 && amountOutFromNative > 0) {
