...
contract SwapCalculator is BaseBoringBatchable {
    ...
    function calculateLiquidityOutput(
        address pool,
        uint256 tokenAmount,
        uint8 tokenIndex
        ...
        for (uint256 i = 0; i < decimalsArr.length; i++) {
            ...
            balances[i] = ISwap(pool).getTokenBalance(uint8(i));
        }
        ...
        uint256 feePerToken = swapFee.mul(uint256(decimalsArr.length)).div(
...

        {
            ...
            for (uint256 i = 0; i < decimalsArr.length; i++) {
                uint256 diff = v.d1.mul(v.balances[i]).div(v.d0).difference(
                    balances1[i]
                );
                balances1[i] = balances1[i].sub(
                    feePerToken.mul(diff).div(FEE_DENOMINATOR)
                );
            }
        ...
        lpAmount = lpAmount.add(1);
        return lpAmount;
    }

    function calculateLiquidityInput(
        address pool,
        uint256 lpAmount,
        uint8 tokenIndex
        ...
        v.totalSupply = v.lpToken.totalSupply();
        uint256[] memory xp = _xp(v.balances, v.multipliers);
        uint256[] memory xpReduced = new uint256[](xp.length);
        {
            v.d0 = getD(xp, v.preciseA);
            v.d1 = v.d0.add(lpAmount.mul(v.d0).div(v.totalSupply));

            //token amount without fee
            uint256 newY = getYD(v.preciseA, tokenIndex, xp, v.d1);
            ...
            for (uint256 i = 0; i < xp.length; i++) {
                ...
                uint256 dxExpected = (i == tokenIndex)
                    ? newY.mul(v.d1).div(v.d0) //new token amount without fee + proportional - old token amount = difference
                    : xpi.mul(v.d1).div(v.d0); //new token amount proportional - old token amount = difference

                uint256 difference = idealBalance.difference(dxExpected);

                xpReduced[i] = dxExpected.sub(
                    feePerToken.mul(difference).div(FEE_DENOMINATOR)
                );
            }
        }

        //new token amount considering fees
        uint256 tokenAmount = getYD(v.preciseA, tokenIndex, xpReduced, v.d1)
            .sub(xpReduced[tokenIndex]);

        tokenAmount = tokenAmount.add(1).div(v.multipliers[tokenIndex]);

        return (getYD(v.preciseA, tokenIndex, xpReduced, v.d1), tokenAmount);
    }
}
