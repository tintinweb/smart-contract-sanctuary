...
import "./IEquipBlocksMinter.sol";

/**
 * Must be set as MINTER for EquipBlocksMinter
 */
contract HolidayEquipMinter is Initializable, AccessControlEnumerableUpgradeable, PausableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    uint256 public price;
    uint256 public maxSupply;
    uint256 public supply;
    uint256 private _equipDbId;
    address private _equipBlocksMinter;
    address private _heroBlocks;
    address private _monsterBlocks;
    ...
    function initialize() initializer public {
        ...
        __HolidayEquipMinter_init_unchained();
    }

    function __HolidayEquipMinter_init_unchained() internal initializer {
    ...
    function setPause(bool pause) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        if(pause) {
            _pause();
        } else {
            _unpause();
    ...
    /**
     * @dev Buy specific equip with associated price
     */
    function mint(uint256 amount_) external payable whenNotPaused nonReentrant {
        require(msg.value >= price * amount_, "Insufficient fee paid");
        require(IERC721Upgradeable(_heroBlocks).balanceOf(_msgSender()) > 0, "No balance");
        require(supply + amount_ <= maxSupply, "Exceeds Max Supply");
        IEquipBlocksMinter equipBlocksMinter = IEquipBlocksMinter(_equipBlocksMinter);
        for(uint256 i; i < amount_; i++) {
            equipBlocksMinter.operatorMint(_msgSender(), _equipDbId);
        }
        supply += amount_;
    }
    ...
    function setSupply(uint256 supply_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        supply = supply_;
    }

    function setEquipDbId(uint256 equipDbId_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _equipDbId = equipDbId_;
    }

    function setEquipBlocksMinter(address equipBlocksMinter_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _equipBlocksMinter = equipBlocksMinter_;
    }

    function getEquipBlocksMinter() external view returns(address) {
        return _equipBlocksMinter;
    }
...
interface IEquipBlocksMinter {
    function freeMint(address,uint256,uint256) external returns(uint256[] memory);
    function operatorMint(address,uint256) external returns(uint256);
    function mintSpecificWithStats(address, uint256, uint256, uint256[] memory, uint256[] memory, uint256[] memory) external returns(uint256);
}
