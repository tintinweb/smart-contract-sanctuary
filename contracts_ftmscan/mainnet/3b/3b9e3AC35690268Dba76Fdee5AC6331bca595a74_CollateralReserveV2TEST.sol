...
contract CollateralReserveV2TEST is Ownable{
    ...
    // add a collatereal
    function addCollateral(address _token, ICollateralReserveV2Calc _token_oracle) public onlyOwnerOrManager{
    ...
    function withdraw( uint _token_amount, address _token ) external {
        require( isCollateral[ _token ], "Not accepted.." );
    ...
    function getActualCollateralRatio() public view  returns(uint256 acr) {
        uint _collateral_value = getTotalCollateralValue();
    ...
    function getTotalCollateralRatio() public view  returns(uint256 tcr) {
        uint tcv = getTotalCollateralValue();
        uint total_supply_dollar = dollarSupply();
        if (total_supply_dollar == 0) {
            return COLLATERAL_RATIO_MAX;
        }
