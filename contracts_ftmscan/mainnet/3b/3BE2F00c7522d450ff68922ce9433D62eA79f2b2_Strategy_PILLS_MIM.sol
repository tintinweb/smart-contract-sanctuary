...
interface IFarm{
    function morph() external view returns (address);
...
contract Strategy_PILLS_MIM {
    // This strategy is for FTM network. PILLS - MIM pair. RewardToken = "morph"
    ...
    address public token0; // MIM
    address public token1; // PILLS
    address public vault;

    // ------------> Important constants <--------------
    uint256 public constant pid = 53;
    bool public constant hasPid = true;
    // uses for swapping MIM to PILLS
    address public router = 0x8aC868293D97761A1fED6d4A01E9FF17C5594Aa3;
    // uses for swapping FTM to MIM and vice versa
    address public spiritRouter = 0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52;
    address public constant lpToken = 0x93D93F3D1aa417B212BAe56D8DF9C88dCCf73A31;
    address public constant farm = 0xc7dad2e953Dc7b11474151134737A007049f576E;
    // ------------>        END          <--------------
    ...
    constructor(address _vault) {
        ...
        _approveToken(WETH, spiritRouter);
        _approveToken(token0, router);
        _approveToken(token0, spiritRouter);
    ...
    function requestWithdraw(address _receiver, uint256 _percent) public onlyVault {
        ...
        if (_totalLP > 0) {
            uint256 yelAmount = _swapLPToYEL(_percent);
            if(yelAmount > 0) {
    ...
    function migrate(uint256 _percent) public onlyVault {
        _swapLPtoWrapped(_percent * 10 ** 12);
    ...
    function _getAmountsOutSpirit(uint256 _amount, address[] memory path) internal view returns (uint256){
        uint256[] memory amounts = IRouter(spiritRouter).getAmountsOut(_amount, path);
        return amounts[amounts.length-1];
    }

    function _getTokenValues(uint256 _amountLP) public view returns (uint256 t0V, uint256 t1V) {
        (uint256 _reserve0, uint256 _reserve1,) = IPair(lpToken).getReserves();
        uint256 LPRatio = _amountLP * 1e12 / IPair(lpToken).totalSupply();
        t0V = LPRatio * _reserve0 / 1e12;
        t1V = LPRatio * _reserve1 / 1e12;
    }
    ...
    function _addLiquidity() internal {
        ...
        if(_balance0 > 100 && _balance1 > 100) {
            ...
            if(amount0 != 0 && amount1 !=0 && _canAddLiquidity(amount0, amount1)) {
                _transfer(token0, lpToken, amount0);
                _transfer(token1, lpToken, amount1);
    ...
    function _addLiquidityFromETH(uint256 _amountETH) internal {
        _check0Price();
        _check1Price();
        // swap all native tokens to MIM
        _swapExactETHForMIM(_amountETH);
        // a half of MIM swap to PILLS
        _swapMIMToPILLS();
        // add Liquidity for tokens
        _addLiquidity();
    }
    ...
    // swap wftm to mim and pills tokens
    function _swapWrappedToTokens() internal {
        // swap all wFTM to MIM
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;
        uint256 _balance = getBalanceOfToken(WETH);
        if (_balance > 0) {
            uint256 amount = _getAmountsOutSpirit(_balance, path);
            if(amount > 100) {
                _swapExactTokensForTokens(spiritRouter, _balance, amount, path);
            }
        }
        // swap half MIM to PILLS
        _swapMIMToPILLS();
    }

    function _swapWrappedToYEL() internal {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = YELtoken;
        uint256 _balance = getBalanceOfToken(WETH);
        if (_balance > 0) {
            uint256[] memory amounts = IRouter(yelLiquidityRouter).getAmountsOut(_balance, path);
            if(amounts[1] > 100) {
                _swapExactTokensForTokens(yelLiquidityRouter, _balance, amounts[1], path);
            }
        }
    }

    // swap all ftm to MIM tokens
    function _swapExactETHForMIM(uint256 _amountETH) internal {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;
        uint256[] memory amounts = IRouter(spiritRouter).getAmountsOut(_amountETH, path);
        if(amounts[1] > 100){
            IRouter(spiritRouter).swapExactETHForTokens{value:_amountETH}(
                amounts[1] - (amounts[1]*toleranceLevelPercent/100), // amountOutMin
                path,
                address(this),
    ...
    function _swapLPToYEL(uint256 _percent) internal returns (uint256 newYelBalance){ 
        // All PILLS to MIM, MIM to FTM
        _swapLPtoWrapped(_percent);
    ...
    function _swapLPtoWrapped(uint256 _percent) internal {
        uint256 _totalLP = getAmountLPFromFarm();
        if (_totalLP > 0) {
            ...
            _swapTokenToWrapped(rewardToken);

            // swap LPs to token0 and token1
            _removeLiquidity();

            // swap token0 and token1 to WETH
            _swapTokensToWrapped();
        }
    }

    function _swapTokensToWrapped() internal {
        // All PILLS to MIM, MIM to FTM
        _swapPILLSToMIM();
        // swap all MIM to wFTM
        _swapMIMToWrapped();
    }

    function _swapTokenToWrapped(address _token) internal {
        address[] memory path = new address[](2);
    ...
    function _swapMIMToPILLS() internal {
        // a half of MIM swap to PILLS
        address[] memory path = new address[](2);
        path[0] = token0;
        path[1] = token1;
        uint256 _balance = getBalanceOfToken(token0) / 2;
        if(_balance > 0){
            uint256 amount = _getAmountsOut(_balance, path);
            if(amount > 100){
                _swapExactTokensForTokens(router, _balance, amount, path);
    ...
    function _swapPILLSToMIM() internal {
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token0;
    ...
    function _swapMIMToWrapped() internal {
        address[] memory path = new address[](2);
        path[0] = token0;
        path[1] = WETH;
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            // amountWrapped = _getMimFromPILLS(t1V) * _getPriceWFTMToMIM() / d;
            uint256 amountMIMs = _getMimFromPILLS(t1V) + t0V;
            tCI = _getPriceWFTMToMIM() * amountMIMs / d;
        }
        // calculates total Capital from tokens that exist on the contract
        uint256 _balance = getBalanceOfToken(token1);
        if(_balance != 0) {
            tCI += _getMimFromPILLS(_balance) * _getPriceWFTMToMIM() / d;
        }

        _balance = getBalanceOfToken(token0);
        if(_balance != 0) {
            tCI += _getPriceWFTMToMIM() * _balance / d;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t0V != 0) {
                path[0] = token0;
                path[1] = WETH;
                TC = _getAmountsOutSpirit(t0V, path);
            }
            if(t1V != 0) {
                path[0] = token0;
                path[1] = token1;
                mimAmount = _getAmountsOut(t1V, path);
                path[0] = token1;
                path[1] = WETH;
                TC += _getAmountsOutSpirit(mimAmount, path);
            }
        }
        ...
        if(_balance != 0) {
            path[0] = token0;
            path[1] = token1;
            mimAmount = _getAmountsOut(_balance, path);
            path[0] = token1;
            path[1] = WETH;
            TC += _getAmountsOutSpirit(mimAmount, path);
        }
        ...
        if(_balance != 0) {
            path[0] = token0;
            path[1] = WETH;
            TC += _getAmountsOutSpirit(_balance, path);
        }
    ...
    function _getPriceWFTMToMIM() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;
    ...
    function _getPricePILLSToMIM() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = token0;
        path[1] = token1;
        return 1e36 / (_getAmountsOut(100 * 1e18, path) / 100);
    ...
    function _getMimFromPILLS(uint256 _pillsAmount) public view returns (uint256 mimAmount) {
        mimAmount = _getPricePILLSToMIM() * _pillsAmount / 1e18;
    }
    ...
    function _check0Price() internal {
        ...
        path[1] = token1;
        // price per one token
        token0Price = _getToken0Price(path);
    ...
    function _check1Price() internal {
        ...
        path[1] = token0;
        // price per one token
        token1Price = _getToken1Price(path);
