...
contract RocketRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ROCKETs to distribute in the pool.
        uint256 lastRewardTime; // Last time that ROCKETs distribution occurred.
        uint256 accTombPerShare; // Accumulated ROCKETs per share, times 1e18. See below.
    ...
    constructor(address _tomb, uint256 _poolStartTime) public {
        ...
        epochEndTimes[0] = poolStartTime + 1 days; // Day 2-5
        epochEndTimes[1] = epochEndTimes[0] + 2 days; // Day 6-10
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "RocketRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "RocketRewardPool: existing pool?");
        }
    }
    ...
    // Safe tomb transfer function, just in case if rounding error causes pool to not have enough ROCKETs.
