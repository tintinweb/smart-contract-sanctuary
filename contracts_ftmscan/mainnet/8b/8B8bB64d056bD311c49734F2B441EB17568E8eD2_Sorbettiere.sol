...
contract Sorbettiere is Ownable {
    ...
    bool private securityKey = false; //prevents Depositor address from changing
    ...
    function setDepositor(address _depositor) external onlyOwner {
        require (securityKey==false);
        Depositor=_depositor;
        //securityKey=true; //prevents Depositor address from changing
    }
...
// How many pools are in the contract
    ...
    function deposit(uint256 _pid, uint256 _amount,address _address) public {
        ...
        if (user.amount > 0) {
            ...
            user.remainingIceTokenReward = safeRewardTransfer(_address, pending);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount,address _address) public {
        require(msg.sender==Depositor);
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_address];
