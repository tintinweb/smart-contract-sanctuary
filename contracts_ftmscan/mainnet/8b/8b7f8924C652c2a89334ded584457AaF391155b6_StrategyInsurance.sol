...
contract StrategyInsurance {
    ...
    uint256 public maximumCompenstionRate = 2; // 2 bips per harvest default
    ...
    function setTargetFundSize(uint256 _targetFundSize) external {
        _onlyAuthorized();
        require(_targetFundSize < 200); // Must be less than 2%
        ...
        returns (uint256 _compensation)
    {
        ...
        // Only process compensation once a loss is experience for more than 1 harvest
        if (lossSum > 0) {
        ...
        returns (uint256 _compensation)
    {
        ...
        // Cofters are empties, we cannot compensate
        if (balance == 0) {
            lossSum = 0;
            return 0;
        }

        // security mechanic, only payout if loss has been experienced for 2 harvests
