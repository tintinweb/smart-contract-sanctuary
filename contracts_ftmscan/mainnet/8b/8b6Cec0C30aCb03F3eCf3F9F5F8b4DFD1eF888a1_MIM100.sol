 ...
 */
contract MIM100 is TokenHandler {
    ...
    address constant MINTER = 0x42B458056f887Fd665ed6f160A59Afe932e1F559;
    address constant POKE_ME = 0x6EDe1597c05A0ca77031cBA43Ab887ccf24cd7e8;
    ...
    function removeSourceToken(address token) external {
        ...
        for (uint i = 0; i < numSourceTokens; i++) {
            if (sourceTokens[i] == token) {
                sourceTokens[i] = sourceTokens[sourceTokens.length - 1];
                sourceTokens.pop();
                break;
            }
        }
    ...
    function supply(address wallet) external onlyManager(wallet) {
        _swapAll(wallet);
    ...
    function _supply(address wallet, uint amount, bool compound) private {
        _approveMaxIfNecessary(MIM, address(hMIM), amount);
    ...
    function _compound(address wallet) private {
        TokenMinter(MINTER).mint_for(hMIMg, wallet);
    ...
    modifier onlyManager(address wallet) {
        require(msg.sender == POKE_ME || msg.sender == wallet);
        _;
    }
