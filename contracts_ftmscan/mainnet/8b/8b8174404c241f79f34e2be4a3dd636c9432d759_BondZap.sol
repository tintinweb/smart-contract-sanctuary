...
contract BondZap is Ownable {
    ...
    function zapInToken(uint _amount, uint _maxPrice, address[] calldata _route, address _bondDepository, address _routerAddr, address _recipient) external {
        ...
        uint amtOut = _swap(_amount, _route, _routerAddr, address(this));
    ...
    function _approveTokenIfNeeded(address token, address router) private {
        if (IERC20(token).allowance(address(this), router) == 0) {
            IERC20(token).safeApprove(router, 1000000000000000000000000000000000000);
        }
    }

    function _swap(uint amount, address[] memory route, address routerAddr, address recipient) private returns (uint) {
        IUniswapRouterETH router = IUniswapRouterETH(routerAddr);

        uint[] memory amounts = router.swapExactTokensForTokens(amount, 0, route, recipient, block.timestamp);
