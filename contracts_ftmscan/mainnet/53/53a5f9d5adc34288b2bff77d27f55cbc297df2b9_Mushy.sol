...
contract Mushy is BEP20 {
    ...
    // Min amount to liquify. (default 10 MUSHYs)
    ...
    // The operator can call all functions except of minting MUSHY
    address private _operator;
    
    // The lpEarner address will be set to an Earning Contract which
    // will distribute Mushy-WFTM LP token to Mushy Stakers!
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "MUSHY::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the Mushy Token contract.
     */
    constructor() public BEP20("Mushy Smurf.Money", "MUSHY") {
    ...
    // transfer function to meet tokenomics of MUSHY
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (_isNonTaxedSender || _isNonTaxedReceipient || transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "MUSHY::transfer: Burn value invalid");

            // default 95.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "MUSHY::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the mushy -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "MUSHY::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "MUSHY::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 1000, "MUSHY::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 100, "MUSHY::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "MUSHY::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "MUSHY::transferOperator: new operator is the zero address");
...
// MasterChef is the master of Reward Token. He can make Reward Token and he is a fair guy.
//
...
contract MushyMaster is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime;  // Last timestamp that MUSHYs distribution occurs.
        uint256 accTokenPerShare;   // Accumulated MUSHYs per share, times 1e18. See below.
    ...
    // MushyBooster
    address public sboost = 0xdb48eB56AD1C264cAbfabdf1F02063285c994294;
    
    // Tokens created per timestamp.
    uint256 public tokenPerSecond;
    
    // DEV address.
    address public constant devaddr = 0x2096aFDaA68EEaE1EbF95DFdf565eE6d9B1fbA37;
    
    // Fee address.
    address public constant feeaddr = 0x45472B519de9Ac90A09BF51d9E161B8C6476361D;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) external onlyOwner nonDuplicated(_lpToken) {        
        _lpToken.balanceOf(address(this)); // valid BEP20 token
    ...
    function updatePool(uint256 _pid) public {
        ...
        try token.mint(devaddr, tokenReward / 10) {
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                uint256 boostBalance = IBEP20(sboost).balanceOf(msg.sender);
                uint256 rewardToUser = 0;
                
                if(boostBalance <= 285*10**18) {
                    rewardToUser = pending - (pending*(70)/(100));
                }
                else if(boostBalance <= 571*10**18) {
                    rewardToUser = pending - (pending*(60)/(100));
                }
                else if(boostBalance <= 857*10**18) {
                    rewardToUser = pending - (pending*(50)/(100));
                }
                else if(boostBalance <= 1142*10**18) {
                    rewardToUser = pending - (pending*(40)/(100));
                }
                else if(boostBalance <= 1428*10**18) {
                    rewardToUser = pending - (pending*(30)/(100));
                }
                else if(boostBalance <= 1714*10**18) {
                    rewardToUser = pending - (pending*(20)/(100));
                }
                else if(boostBalance <= 2000*10**18) {
                    rewardToUser = pending - (pending*(10)/(100));
                }
                else  { rewardToUser = pending; }
                
                safeTokenTransfer(msg.sender, rewardToUser);
                safeTokenTransfer(burnaddr, pending - rewardToUser);
            }
        }
