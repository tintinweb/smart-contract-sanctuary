...
contract RavenILPStrategy is StratManager, FeeManager {
    ...
    constructor(
        ...
        address _unirouter,
        address _ravenFeeRecipient, 
        address _elkFeeRecipient
    ) StratManager(_ravenFeeRecipient, _ravenFeeRecipient, _unirouter, _vault, _ravenFeeRecipient) public {
        want = _want;
        lpToken0 = IUniswapV2Pair(want).token0();
        lpToken1 = IUniswapV2Pair(want).token1();
        rewardPool = _rewardPool;
        elkFeeRecipient = _elkFeeRecipient;
    ...
    function elkLiq() internal {
        uint256 wantBal = IERC20(want).balanceOf(address(this)).mul(52).div(10000); //0.52% of 97% ~= 0.5% of 100%
        IERC20(want).safeTransfer(elkFeeRecipient, wantBal);
    }
    ...
    function harvestBoostReward() external whenNotPaused {
    ...
    function chargeFees() internal {
        uint256 toMatic = IERC20(output).balanceOf(address(this)).mul(30).div(1000); //3.5% fees on txn
        ...
        uint256 strategistFee = maticBal.mul(STRATEGIST_FEE).div(MAX_FEE);  //This is the shitpoast address
