...
abstract contract StrategyBase {
    ...
    // Dex
    address public univ2Router2 = 0xF491e7B69E4244ad4002BC14e878a34207E38c29;
...
contract StrategyScreamTUSDV1 is StrategyBase, Exponential {
    //want token
    address public constant USDC = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75;
    address public constant TUSD = 0x9879aBDea01a879644185341F7aF7d8343556B7a;
    address public constant comptroller = 0x260E596DAbE3AFc463e75B6CC05d8c46aCAcFB09; //done
    address public constant lens = 0x59B46Fbb487aa974DE815F31601cbE6ba7368A01; //done
    address public constant scTUSD = 0x789B5DBd47d7Ca3799f8E9FdcE01bC5E356fcDF1;
    ...
    public StrategyBase(TUSD, _governance, _strategist, _controller, _timelock)
    {
        ...
        IERC20(TUSD).safeApprove(scTUSD, uint256(-1));
    }
    ...
    function getLeveragedSupplyTarget(uint256 supplyBalance, uint256 _leverage) public view returns (uint256){
    ...
    function getMarketColFactor() public view returns (uint256) {
        (, uint256 colFactor) = IComptroller(comptroller).markets(scTUSD);
        return colFactor;
    }
    ...
    function getCompAccrued() public returns (uint256) {
        (, , , uint256 accrued) = ICompoundLens(lens).getCompBalanceMetadataExt(scream, comptroller, address(this));
        return accrued;
    }
    ...
    function getSupplied() public returns (uint256) {
        return ICToken(scTUSD).balanceOfUnderlying(address(this));
    }

    function getBorrowed() public returns (uint256) {
        return ICToken(scTUSD).borrowBalanceCurrent(address(this));
    }
    ...
    // 1. Borrow out <X> LINK
    // 2. Supply <X> LINK
    function leverageUntil(uint256 _supplyAmount, uint256 _maxLeverage, uint256 _unleveragedSupply) public onlyKeepers {
        ...
        while (supplied < _supplyAmount) {
            ...
            ICToken(scTUSD).borrow(_borrowAndSupply);
            deposit();
    ...
    function deleverageUntil(uint256 _supplyAmount) public onlyKeepers {
        ...
        do {
            ...
            require(ICToken(scTUSD).redeemUnderlying(_redeemAndRepay) == 0, "!redeemInDeleverageUntil");
            require(ICToken(scTUSD).repayBorrow(_redeemAndRepay) == 0, "!repay");
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal override returns (address, uint256){
        ...
        uint256 _bpBal = IERC20(WFTM).balanceOf(address(this));
        _swapUniswapWithDetailConfig(want, WFTM, _lpAmount, 1, _path, univ2Router2);
        uint256 _bpBalAfter = IERC20(WFTM).balanceOf(address(this));
        
        return (WFTM, _bpBalAfter > _bpBal? _bpBalAfter.sub(_bpBal) : 0);
    }
    ...
    function harvest() public override onlyBenignCallers {
        ...
        if (_scream > 0) {
            ...
            _path[2] = USDC;
            _path[3] = want;
            _swapUniswapWithDetailConfig(scream, want, _scream, 1, _path, univ2Router2);
        }
        ...
        if (buybackEnabled == true && _buybackAmount > 0) {
            (address _bp, uint256 _bpAmt) = _convertWantToBuyback(_buybackAmount);
            buybackAndNotify(_bp, _bpAmt);
        }
    ...
    function deposit() public override{
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            require(ICToken(scTUSD).mint(_want) == 0, "!deposit");
        }
    }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256){
        ...
        require(ICToken(scTUSD).getCash() >= _redeem, "!cash-liquidity");
        ...
        // Redeems underlying
        require(ICToken(scTUSD).redeemUnderlying(_redeem) == 0, "!redeemInWithdrawSome");
    ...
    function withdraw(IERC20 _asset) external override returns (uint256 balance) {
        require(scTUSD != address(_asset), "!cToken");
