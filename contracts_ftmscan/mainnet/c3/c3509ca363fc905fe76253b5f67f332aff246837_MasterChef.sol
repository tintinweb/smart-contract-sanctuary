...
// File: contracts/OrcanautsToken.sol

pragma solidity 0.6.12;

contract OrcanautsToken is ERC20("Orcanauts", "ORCANAUTS"), Ownable {
...
// MasterChef is the master of Orcanauts. He can make Orcanauts and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Orcanauts is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accOrcanautsPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accOrcanautsPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accOrcanautsPerShare; // Accumulated FISHes per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The ORCANAUTS TOKEN!
    OrcanautsToken public orcanautsToken;
    address public devAddress;
    address public feeAddress;

    // ORCANAUTS tokens created per block.
    uint256 public orcanautsPerBlock = 0.002 ether;
    ...
    // The block number when ORCANAUTS mining starts.
    uint256 public startBlock;

    uint256 constant MAX_ORCANAUTS_SUPPLY = 88888 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 orcanautsPerBlock);


    constructor(
        OrcanautsToken _orcanautsToken,
        uint256 _startBlock,
        address _devAddress,
        address _feeAddress
    ) public {
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accOrcanautsPerShare: 0,
                depositFeeBP: _depositFeeBP
            })
        );
    }

    // Update the given pool's ORCANAUTS allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP
    ...
    function pendingOrcanauts(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accOrcanautsPerShare = pool.accOrcanautsPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 orcanautsReward = multiplier
                .mul(orcanautsPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accOrcanautsPerShare = accOrcanautsPerShare.add(
                orcanautsReward.mul(1e18).div(lpSupply)
            );
        }
        return user.amount.mul(accOrcanautsPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(orcanautsToken.owner() != address(this)) {
        ...
        uint256 currentSupply = orcanautsToken.totalSupply();

        if (currentSupply >= MAX_ORCANAUTS_SUPPLY) {
            pool.lastRewardBlock = block.number;
            return;
        }
        ...
        uint256 devReward = orcanautsReward.div(10);
        orcanautsToken.mint(devAddress, devReward);
        orcanautsToken.mint(address(this), orcanautsReward);
        pool.accOrcanautsPerShare = pool.accOrcanautsPerShare.add(
            orcanautsReward.mul(1e18).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for ORCANAUTS allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount
    ) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accOrcanautsPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                uint256 devReward = pending.div(10);
                safeOrcanautsTransfer(msg.sender, pending.sub(devReward));
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accOrcanautsPerShare).div(1e18);
    ...
    function _withdraw(
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accOrcanautsPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            uint256 devReward = pending.div(10);
            safeOrcanautsTransfer(_user, pending);
        }
        if (_amount > 0) {
    ...
    // Safe orcanautsToken transfer function, just in case if rounding error causes pool to not have enough ORCANAUTS.
    function safeOrcanautsTransfer(address _to, uint256 _amount) internal {
        uint256 orcanautsBal = orcanautsToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > orcanautsBal) {
            transferSuccess = orcanautsToken.transfer(_to, orcanautsBal);
        } else {
            transferSuccess = orcanautsToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeOrcanautsTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _orcanautsTokenPerBlock)
        external
        onlyOwner
    {
        massUpdatePools();
        orcanautsPerBlock = _orcanautsTokenPerBlock;
        emit UpdateEmissionRate(msg.sender, _orcanautsTokenPerBlock);
    }
    ...
    function transferOrcanautsTokenOwnership(address _newOwner)
        external
        onlyOwner
    {
        orcanautsToken.transferOwnership(_newOwner);
    }
}