...
// File contracts/mocks/MasterChefMock.sol
...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to SpiritSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // SpiritSwap must mint EXACTLY the same amount of SpiritSwap LP tokens or
...
contract MasterChefMock is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accSpiritPerShare; // Accumulated SPIRITs per share, times 1e36. See below.
    }
    // The SPIRIT TOKEN!
    SpiritTokenMock public spirit;
    // Dev address.
    address public devaddr;
    // Block number when bonus SPIRIT period ends.
    ...
    // Update the given pool's SPIRIT allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 spiritReward =
                multiplier.mul(spiritPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accSpiritPerShare = accSpiritPerShare.add(
                spiritReward.mul(1e36).div(lpSupply)
            );
        }
        return user.amount.mul(accSpiritPerShare).div(1e36).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accSpiritPerShare).div(1e36).sub(
                    user.rewardDebt
                );
        ...
        user.rewardDebt = user.amount.mul(pool.accSpiritPerShare).div(1e36);
