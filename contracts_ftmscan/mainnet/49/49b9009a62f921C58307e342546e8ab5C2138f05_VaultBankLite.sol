...
contract VaultBankLite is ContextUpgradeSafe, ReentrancyGuard {
    ...
    function withdraw(
        ...
        uint256 _wdAmt = _withdraw(_vault, _shares);
        IVault(_vault).withdrawFor(msg.sender, _wdAmt, _min_output_amount);
    }

    function _withdraw(address _vault, uint256 _shares) internal returns (uint256) {
        uint256 _userBal = IERC20(address(_vault)).balanceOf(msg.sender);
        require(_userBal >= _shares, "Bank: _userBal < _shares");

        uint256 _before = IERC20(address(_vault)).balanceOf(address(this));
        IERC20(address(_vault)).safeTransferFrom(msg.sender, address(this), _shares);
        uint256 _after = IERC20(address(_vault)).balanceOf(address(this));
