...
contract PoolInitializer is IPoolInitializer {
  ...
  function getDesiredFactories() external view override returns (address[] memory) {
    return _factories;
  }
  ...
  function initialize(
    ...
    require(factories.length == len && amounts.length == len, "BiShares: Invalid arrays length");
  ...
  function finish() external override _lock_ _not_finished_ returns (bool) {
    ...
    address[] memory factories = new address[](len);
  ...
  function contributeTokens(
    ...
    address caller = msg.sender;
    uint256 len = tokens.length;
    require(factories.length == len && amountsIn.length == len, "BiShares: Invalid arrays length");
    credit = 0;
    for (uint256 i = 0; i < len; i++) {
...
interface IMarketCapSqrtController {
  ...
  function addTokens(uint256 categoryID, address[] calldata tokens) external returns (bool);
