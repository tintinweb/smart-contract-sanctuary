...
contract TestVAuto is Ownable, Pausable {
    ...
    uint256 public withdrawFeePeriod = 10 minutes; // 3 days
     ...
     */
    function calculateHarvestTestVRewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingTESTV(0, address(this));
     ...
     */
    function calculateTotalPendingTestVRewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingTESTV(0, address(this));
        amount = amount.add(available());
