 ...
 */
contract ReaperAutoCompoundxDAO_SingleAsset is Ownable, Pausable {
     ...
     */
    address[] public rewardTokenToWftmRoute = [rewardToken, usdc, wftm];
    ...
    constructor (
      ...
      address _treasury
        ...
        if (token == usdc) {
            rewardTokenToTokenRoute = [rewardToken, usdc];
        } else if (token == wftm) {
            rewardTokenToTokenRoute = [rewardToken, usdc, wftm];
        }else{
            rewardTokenToTokenRoute = [rewardToken, usdc, wftm, token];
        }
    ...
    function withdraw(uint256 _amount) external {
        ...
        IERC20(token).safeTransfer(vault, pairBal.sub(withdrawFee));
    }
    ...
    function giveAllowances() internal {
        IERC20(token).safeApprove(masterChef, uint256(-1));
