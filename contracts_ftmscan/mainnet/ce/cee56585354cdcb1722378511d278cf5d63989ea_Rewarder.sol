...
contract Rewarder is IRewarder,  BoringOwnable{
    ...
    function onReward (uint256 pid, address _user, uint256 averageBlockDeposit, uint256 shares) onlyMM override external {
        ...
        user.amount = shares;
        user.rewardDebt = shares.mul(pool.accTokenPerShare) / ACC_TOKEN_PRECISION;
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && totalShares != 0) {
            ...
            accTokenPerShare = accTokenPerShare.add(Reward.mul(ACC_TOKEN_PRECISION) / totalShares);
        }
        pending = rewardAcc[_pid][_user].add(((user.amount.mul(accTokenPerShare) / ACC_TOKEN_PRECISION).sub(user.rewardDebt)).mul((boostC.add(this.Boost(averageBlockDeposit)))/boostC));
        }
    ...
    function Boost(uint256 averageBlock) external view returns (uint256) {
        ...
        else if (elapsed >= 7*BLOCKS_IN_DAY){
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (totalShares > 0) {
                uint256 blocks = block.number.sub(pool.lastRewardBlock);
                uint256 Reward = blocks.mul(tokenPerBlock).mul(pool.allocPoint) / totalAllocPoint;
                pool.accTokenPerShare = pool.accTokenPerShare.add((Reward.mul(ACC_TOKEN_PRECISION) / totalShares).to128());
            }
            pool.lastRewardBlock = block.number.to64();
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardBlock, totalShares, pool.accTokenPerShare);
        }
        updateRewardPerBlock();
    }
