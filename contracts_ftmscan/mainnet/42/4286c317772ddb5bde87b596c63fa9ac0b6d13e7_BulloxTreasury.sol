...
contract BulloxTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SBULLOX }
    ...
    address public sBULLOX;
    uint public sBULLOXQueue; // Delays change to sBULLOX address
    ...
    /**
        @notice allow approved address to deposit an asset for BULLOX
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BULLOX needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BULLOX ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BULLOX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IBULLOXERC20( BULLOX ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBULLOX ).balanceOf( msg.sender ); // Can only borrow against sBULLOX held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BULLOX
        @param _amount uint
     */
    function repayDebtWithBULLOX( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IBULLOXERC20( BULLOX ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BULLOX, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BULLOX ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BULLOX ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BULLOX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BULLOX decimals
            value_ = _amount.mul( 10 ** IERC20( BULLOX ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SBULLOX ) { // 9
            sBULLOXQueue = block.number.add( blocksNeededForQueue );
        } else return false;
