...
import "./VampirePoint.sol";

contract VampireSwapProfile is Ownable, IVampireSwapProfile {
    string public name = "VampireSwap Profile";
    ...
    VampirePoint public vpoint;
    address public treasuryAddress;
    bool public isStartRefCompetition = false;
    bool public isStartPointMinting = false;
    uint public volumeRatio; // 1 swap volume = <volumeRatio> vPOINT
    uint public refVolumeRatio; // 1 ref's swap volume = <refVolumeRatio> vPOINT
    ...
    function setVampirePoint(VampirePoint _vpoint) external onlyOwner {
        vpoint = _vpoint;
    }
    ...
    function setIsStartPointMinting(bool _isStartPointMinting) external onlyOwner {
        isStartPointMinting = _isStartPointMinting;
    }

    function setRatios(uint _volumeRatio, uint _refVolumeRatio) public onlyOwner {
        volumeRatio = _volumeRatio;
        refVolumeRatio = _refVolumeRatio;
    }
    ...
    function changeTeam(uint _tid) external override {
        ...
        if (currentTid != _tid) {
            ...
            IVampireSwapTreasury treasury = IVampireSwapTreasury(treasuryAddress);
    ...
    function swapVolumeToPoint(uint _volume) external override {
        require(isStartPointMinting, "swapVolumeToPoint: NOT NOW");
        require(_volume.add(userInfo[msg.sender].usedVolume) <= userInfo[msg.sender].totalVolume, "swapVolumeToPoint: EXCEEDS TOTALVOLUME");
        userInfo[msg.sender].usedVolume = userInfo[msg.sender].usedVolume.add(_volume);
        // mint point
        vpoint.mint(msg.sender, _volume.mul(volumeRatio).div(1e18));

        emit SwapVolumeToPoint(msg.sender, _volume);
    }

    function swapRefVolumeToPoint(uint _refVolume) external override {
        require(isStartPointMinting, "swapRefVolumeToPoint: NOT NOW");
        require(_refVolume.add(userInfo[msg.sender].usedRefVolume) <= userInfo[msg.sender].totalRefVolume, "swapRefVolumeToPoint: EXCEEDS TOTALREFVOLUME");
        userInfo[msg.sender].usedRefVolume = userInfo[msg.sender].usedRefVolume.add(_refVolume);
        // mint point
        vpoint.mint(msg.sender, _refVolume.mul(refVolumeRatio).div(1e18));

        emit SwapRefVolumeToPoint(msg.sender, _refVolume);
    }
...
contract VampirePoint is ERC20("VampirePoint", "vPOINT"), VampireSwapMintable {
