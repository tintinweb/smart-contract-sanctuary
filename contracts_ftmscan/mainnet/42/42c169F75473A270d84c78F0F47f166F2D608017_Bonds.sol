...
contract Bonds is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        IBondStrategy strategy; // address of the strategy for the pool
    }
    ...
    event Add(IERC20 stakeToken, IBondStrategy strategy);
    ...
    event SetStrategyExtraEarnTokens(IBondStrategy _strategy, address[] _extraEarnTokens);
    ...
    function stakedTokens(uint256 _pid, address _user) public view returns (uint256) {
        ...
        IBondStrategy strategy = pool.strategy;

        uint256 sharesTotal = strategy.sharesTotal();
        return sharesTotal > 0 ? (user.shares * strategy.totalEarnTokens()) / sharesTotal : 0;
    }
     ...
     */
    function add(IBondStrategy _strategy) public onlyOperator {
     ...
     */
    function addMany(IBondStrategy[] calldata _strategies) external onlyOperator {
     ...
     * @param _maxPrice max bond price
     */
    function deposit(
        uint256 _pid,
        ...
        _deposit(_pid, _depositAmount, _maxPrice, msg.sender);
    }
    ...
    function depositTo(
        ...
        _deposit(_pid, _depositAmount, _maxPrice, _to);
    }

    /**
     * @notice starts a withdraw request
     * @param _pid the pool id
     * @param _withdrawAmount amount of tokens to withdraw
     */
    function requestWithdraw(uint256 _pid, uint256 _withdrawAmount) external nonReentrant {
        require(_withdrawAmount > 0, 'requestWithdraw: MUST_BE_GREATER_THAN_ZERO');
        UserInfo storage user = userInfo[_pid][msg.sender];
        IBondStrategy strategy = poolInfo[_pid].strategy;

        _tryEarn(strategy);

        uint256 sharesTotal = strategy.sharesTotal();

        require(user.shares > 0 && sharesTotal > 0, 'requestWithdraw: NO_SHARES');

        uint256 maxAmount = (user.shares * strategy.totalEarnTokens()) / sharesTotal;
        if (_withdrawAmount > maxAmount) {
            _withdrawAmount = maxAmount;
        }
        uint256 sharesRemoved = strategy.requestWithdraw(_withdrawAmount, msg.sender);
        user.shares = user.shares > sharesRemoved ? user.shares - sharesRemoved : 0;
        if (user.shares == 0) {
            userStakedPools[msg.sender].remove(_pid);
        }

        uint256 pending = (user.pendingWithdraw * strategy.accWithdrawPerShare()) / 1e12 - user.rewardDebt;
        if (pending > 0) {
            uint256 amountWithdrawn = strategy.withdraw(pending, msg.sender);
            user.pendingWithdraw -= amountWithdrawn;
        }

        user.pendingWithdraw += _withdrawAmount;
        user.rewardDebt = (user.pendingWithdraw * strategy.accWithdrawPerShare()) / 1e12;

        emit RequestWithdraw(msg.sender, _pid, _withdrawAmount);
    }

    /**
     * @notice transfers pending tokens to the user
    ...
    function claim(uint256 _pid) external nonReentrant {
        ...
        if (strategy.instantWithdraw()) {
            uint256 amountWithdrawn = strategy.withdraw(user.pendingWithdraw, msg.sender);
            user.pendingWithdraw -= amountWithdrawn;
            emit Claim(msg.sender, _pid, amountWithdrawn);
        } else {
            _tryEarn(strategy);
     ...
     */
    function setStrategyExtraEarnTokens(IBondStrategy _strategy, address[] calldata _extraEarnTokens)
        external
        onlyOwner
    {
    ...
    function _deposit(
        ...
        uint256 sharesAdded = pool.strategy.deposit(_depositAmount, _maxPrice);
    ...
    function _earn(uint256 _pid) internal returns (uint256 bountyRewarded) {
        IBondStrategy strategy = poolInfo[_pid].strategy;
        bountyRewarded = strategy.redeem(msg.sender);
        strategy.earn(10000);
        emit Earn(msg.sender, _pid, bountyRewarded);
    }

    function _tryEarn(IBondStrategy _strategy) internal {
        try _strategy.redeem(address(0)) {
            try _strategy.earn(10000) {} catch {}
        } catch {}
    }
}
...
interface IBondStrategy {
    function stakeToken() external view returns (address);

    function sharesTotal() external view returns (uint256);

    function redeem(address _bountyHunter) external returns (uint256);

    function earn(uint256 _earnPct) external;

    function deposit(uint256 _depositAmount, uint256 _maxPrice) external returns (uint256);

    function requestWithdraw(uint256 _withdrawAmount, address _withdrawTo) external returns (uint256);

    function withdraw(uint256 _withdrawAmount, address _withdrawTo) external returns (uint256);

    function instantWithdraw() external returns (bool);
    ...
    function totalEarnTokens() external view returns (uint256);

    function accWithdrawPerShare() external view returns (uint256);
