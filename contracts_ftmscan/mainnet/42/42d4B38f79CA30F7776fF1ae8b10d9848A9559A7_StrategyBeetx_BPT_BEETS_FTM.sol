...
import "../BeetxWithdraw.sol";

contract StrategyBeetx_BPT_BEETS_FTM is BeetXStrategy {

    address bpt_beets_ftm_diff;

    function initializeStrategy(
        address _store,
        address _vault
    ) public initializer {
        address masterchef = address(0x8166994d9ebBe5829EC86Bd81258149B87faCfd3);
        address beets_ftm_lp = address(0xcdE5a11a4ACB4eE4c805352Cec57E236bdBC3837);
        address beets = address(0xF24Bcf4d1e507740041C9cFd2DddB29585aDCe1e);
        address beethoven_vault = address(0x20dd72Ed959b6147912C2e529F0a0C651c33c9ce);
        bytes32 bpt_pool_id = 0xcde5a11a4acb4ee4c805352cec57e236bdbc3837000200000000000000000019;
        __Strategy_init(
            _store,
            beets_ftm_lp,
            _vault,
...
/// @title BeetX LP Strategy 
/// @author Chainvisions & jlontele 
/// @notice A strategy for BeethovenX LP farm pools.

contract BeetXStrategy is BaseUpgradeableStrategy {
    ...
    public initializer {
        ...
        (address poolToken) = IMasterChef(rewardPool()).lpTokens(_pid);
        require(poolToken == underlying(), "Strategy: Pool token does not match with underlying");

        _setPoolId(_pid);
        _setDepositToken(_depositToken);
        _setBVault(_bVault);
        _setRewardToDepositPoolID(_rewardToDepositPoolID);
        _setNTokens(_nTokens);
        _setDepositArrayIndex(_depositArrayIndex);

        (IERC20[] memory tokens,, ) = IBVault(_bVault).getPoolTokens(_rewardToDepositPoolID);
    ...
    */
    function doHardWork() external onlyNotPausedInvesting restricted {
        IMasterChef(rewardPool()).harvest(poolId(), address(this));
        _liquidateReward();
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            IMasterChef(rewardPool()).withdrawAndHarvest(poolId(), Math.min(_rewardPoolStake(), needToWithdraw), address(this));
        }
    ...
    * pauses the strategy.
    */
    function emergencyExit() public onlyGovernanceOrMultisig {
    ...
    */
    ...
    function unsalvagableTokens(address token) public view returns (bool) {
        return (token == underlying() || token == rewardToken() || token == depositToken());
    }
    ...
    function _investAllUnderlying() internal onlyNotPausedInvesting {
        ...
        if(underlyingBalance > 0) {
            ...
            IMasterChef(rewardPool()).deposit(poolId(), underlyingBalance, address(this));
        }
    }


    /* 
    * swap rewardToken to depositToken using BeethovenX 
    */ 

    function _swapRewardToDeposit() internal {
        ...
        singleSwap.assetIn = IAsset(rewardToken());
        ...
        IERC20(rewardToken()).safeApprove(bVault(), 0);
        IERC20(rewardToken()).safeApprove(bVault(), rewardBalance);

        IBVault(bVault()).swap(singleSwap, funds, 1, (block.timestamp + 600));
    }

    /* 
    * Joining the LP token using single side using depositToken
    */ 
    ...
    */
    function _liquidateReward() internal {
        ...
        if (rewardToken() == depositToken()) {
            // no swap needed, enter LP single side 
            _depositLP();
        } else {
            _swapRewardToDeposit();
            _depositLP(); 
        }
    }

    function _setDepositToken(address _depositToken) internal {
        _setAddress("depositToken", _depositToken);
    }
    ...
    function _setBVault(address _address) internal {
        _setAddress("bVault", _address);
    }

    function _setRewardToDepositPoolID(bytes32 _poolid) internal {
        _setBytes32("rewardToDepositPoolID", _poolid);
    }
...
interface IMasterChef {
    ...
    function lpTokens(uint256 _pid) external view returns (address);   // poolInfo
    function userInfo(uint256 _pid, address) external view returns (uint256, uint256);
}
