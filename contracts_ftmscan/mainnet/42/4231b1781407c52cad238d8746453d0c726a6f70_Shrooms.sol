...
contract Shrooms is ERC721Enumerable, Ownable {
  ...
  uint8 private currentPriceId;

  uint8 public maxPurchaseCount = 10;
  mapping (address=>uint256) public preMintedCnt;
  ...
  IERC721 private fantomBunch = IERC721(0x8e511Aefa68af20634A19577d40Ccf3C6504eaa3);
  IERC721 private babyPunk = IERC721(0xd7179025B82cb188A6Ce5A58E69e00F16c4E1843);
  IERC721 private degenerabBits = IERC721(0x8E7c434B248d49D873D0F8448E0FcEc895b1b92D);
  uint256[MAX_ENTRIES] internal availableIds;
  uint public maxCountForTeam = 250;
  uint public reservedCount;
  ...
  function reservedForTeam(uint amount) external {
    require(reservedCount + amount <= maxCountForTeam, "AMOUNT EXCEED FOR TEAM");
    for(uint i = 0; i < amount; i++)
    {
      _safeMint(admin, _getNewId(totalMinted+i));
    }
    totalMinted += amount;
  }
  ...
  function mint(uint256 _amount) external payable {
    require(paused == false, 'Unable to mint');
    require(_amount + totalMinted <= MAX_ENTRIES, 'Amount exceed');
    if (stage == STAGES.PRESALE && (block.timestamp - startTime) >= 86400) {
      stage = STAGES.PUBLICSALE;
    }
    if (stage == STAGES.PRESALE) {
      require(getWhitelistState(msg.sender), 'Only whitelisted address can mint first 250 NFTs');
      require(preMintedCnt[msg.sender]+_amount <= maxPurchaseCount, 'BUYABLE LIMIT EXCEED');
      preMintedCnt[msg.sender] += _amount;
    }
      ...
      require(freeminted[msg.sender] == 0, 'FREE MINT LIMIT EXCEED');
    ...
    if (degenerabBits.balanceOf(msg.sender) > 0) {
      estimatedPrice = estimatedPrice * 90 / 100;
    }
  ...
  function getCurrentStage() external view returns (STAGES) {
    if (stage == STAGES.PRESALE && (block.timestamp - startTime) >= 86400) {
      return STAGES.PUBLICSALE;
    }
    return stage;
  }

  function getWhitelistState(address user) public view returns(bool) {
    if(whitelisted[user] || fantomBunch.balanceOf(user) > 0 || babyPunk.balanceOf(user) > 0) {
      return true;
    } else return false;
  }
  ...
  function isDiscountAddress(address user) external view returns (bool) {
    if (degenerabBits.balanceOf(user) > 0)
      return true;
    return false;
  }
