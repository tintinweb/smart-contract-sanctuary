...
contract SupplyVaultStrategyV1 is ISupplyVaultStrategy, Ownable {
    ...
    function _authorize(ISupplyVault supplyVault) private {
        require(!isAuthorized[supplyVault], "SupplyVaultStrategyV1: ALREADY_AUTHORIZED");
    ...
    modifier onlyAuthorized() {
        require(isAuthorized[ISupplyVault(msg.sender)], "SupplyVaultStrategyV1: NOT_AUTHORIZED");
    ...
    ) private returns (uint256 gain_, uint256 loss_) {
        require(depositAmount > 0 != withdrawAmount > 0, "SupplyVaultStrategyV1: DEPOSIT_XOR_WITHDRAW");
        (uint256 currentSupplyRate, , ) = borrowable.getCurrentSupplyRate();
        if (currentSupplyRate == 0) {
            return (gain_ = 0, loss_ = 0);
        }
        (uint256 nextSupplyRate, , ) = borrowable.getNextSupplyRate(depositAmount, withdrawAmount);

        uint256 currentBalance = borrowable.underlyingBalanceOf(address(supplyVault));
        uint256 currentInterest = currentBalance.mul(currentSupplyRate);
        uint256 nextInterest;
        if (depositAmount > 0) {
            nextInterest = currentBalance.add(depositAmount).mul(nextSupplyRate);
        } else if (withdrawAmount > 0) {
            nextInterest = currentBalance.sub(withdrawAmount).mul(nextSupplyRate);
        } else {
            assert(false);
        }
    ...
    function getBorrowable(address _address) external view override onlyAuthorized returns (IBorrowable) {
        ...
        require(false, "SupplyVaultStrategyV1: INVALID_BORROWABLE");
    }
    ...
    function allocate() public override onlyAuthorized {
        ...
        uint256 numBorrowables = supplyVault.getBorrowablesLength();
        require(numBorrowables > 0, "SupplyVaultStrategyV1: NO_BORROWABLES");
        ...
        for (uint256 i = 0; i < numBorrowables; i++) {
            ...
            (uint256 gain, ) = getNetChange(supplyVault, borrowable, amount, 0);
    ...
    function _deallocateFromLowestSupplyRate(
        ...
        for (uint256 i = 0; i < numBorrowables; i++) {
            ...
            (, uint256 loss) = getNetChange(supplyVault, borrowable, 0, withdrawBorrowableAmountAsUnderlying);
            uint256 lossPerUnderlying = loss.mul(1e18).div(withdrawBorrowableAmountAsUnderlying);
            if (lossPerUnderlying < best.minLoss) {
                best.borrowable = borrowable;
        ...
        require(best.minLoss < type(uint256).max, "SupplyVaultStrategyV1: INSUFFICIENT_CASH");
    ...
    function deallocate(uint256 needAmount) public override onlyAuthorized {
        require(needAmount > 0, "SupplyVaultStrategyV1: ZERO_AMOUNT");
    ...
    function reallocate(uint256 _underlyingAmount, bytes calldata _data) external override onlyAuthorized {
        _data; // silence compiler

        deallocate(_underlyingAmount);
        allocate();
    }
}
