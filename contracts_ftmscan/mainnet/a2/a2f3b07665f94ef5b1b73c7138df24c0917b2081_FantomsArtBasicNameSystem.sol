...
contract FantomsArtBasicNameSystem is ERC721Enumerable, ERC165Storage, Ownable {
  ...
  string private base = ""; //just in case
  ...
  function isOwned(string memory _name) public view returns (bool) {
    _name = Strings2.upper(_name);
    if(_tokenIdToStruct[_reverseSearch[_name]]._exists) {
      return true;
    } else {
      return false;
    }
  ...
  function isOwnedR(string memory _name) public view returns (bool) {
    _name = Strings2.upper(_name);
    if(_tokenIdToStruct[_reverseSearch[_name]]._exists) {
      return false;
    } else {
  ...
  function getOwnerOfNameRaw(string memory _name) public view returns(address) {
    _name = Strings2.upper(_name);
    uint256 _searchFor = _reverseSearch[_name];
    return ownerOf(_searchFor);
  }

  function registerName(string memory _name) public payable {
    _name = Strings2.upper(_name);
    require(mintOpen,"FNS: Minting closed.");
    require(msg.value == FEE_AMT, "FNS: You need to pay the right amount >:(");//                   VvV
    require(isOwnedR(_name), "FNS: You cant take other peoples name");
    require(keccak256(abi.encodePacked(_search[msg.sender])) != keccak256(abi.encodePacked(uint(0))), "FNS: You cant own more than one name"); // the answer is --> NO <--
  ...
  function transferName(address _from, address _to, string memory _name) public {
    require(keccak256(abi.encodePacked(_search[_to])) != keccak256(abi.encodePacked(uint(0))),"FNS: You cant overwrite someones name, idiot");
    ...
    _reverseSearch[_search[_to]] = _reverseSearch[_searchFromOld];
    _reverseSearch[_searchFromOld] = uint256(0);

    _tokenIdToStruct[_reverseSearch[_search[_to]]] = _tokenIdToStruct[_reverseFromOld];
    _tokenIdToStruct[_reverseFromOld] = FantomsArtName(string(abi.encodePacked(uint(0))),0,address(0));

    safeTransferFrom(_from,_to,_reverseFromOld);
  }
  */
