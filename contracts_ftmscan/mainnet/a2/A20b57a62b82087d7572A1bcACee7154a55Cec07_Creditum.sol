...
import "./Core.sol";
import "./CreditumAbstract.sol";

/**
 * @title A decentralized, fixed-rate lending/borrowing protocol
 * @author Creditum
 */
contract Creditum is Core, CreditumAbstract {
    ...
    /// @param repayAmount The amount of fToken to repay (use type(uint).max to repay all)
    ...
    /// @param borrower The borrower to be liquidated
    /// @param collateral The borrower's collateral to liquidate
    /// @return (error code (See ErrorReporter.sol), amount of fToken burned, amount of collateral sender received)
    ...
    /// @return (error code (See ErrorReporter.sol), amount of fToken minted)
    ...
    /// @param burnAmount The amount of fToken to burn
...
abstract contract CreditumAbstract {    
    function enter(address collateral, uint depositAmount, uint borrowAmount) external virtual returns (uint, uint, uint);
    function exit(address collateral, uint withdrawAmount, uint repayAmount) external virtual returns (uint, uint, uint);
    function triggerLiquidation(address borrower, address collateral) external virtual returns (uint);
    function liquidateBorrow(address borrower, address collateral) external virtual returns (uint, uint, uint);
    
    function stabilizerMint(address underlying, uint amount) external virtual returns (uint, uint);
    function stabilizerRedeem(address underlying, uint amount) external virtual returns (uint, uint);
}
