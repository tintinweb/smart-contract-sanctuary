...
contract FantomsArtBasicNameSystem is ERC721Enumerable, ERC165Storage, Ownable {
  ...
  function isOwned(string memory _name) public view returns (bool) {
    ...
    if(_tokenIdToStruct[_reverseSearch[_name]]._exists && (_rSearch2[_name] != deployer)) {
      return true;
    } else {
      return false;
    }
  ...
  function getOwnerOfTokenIdUsingERC721(uint256 _tokenID) public view returns(address) {
    return ownerOf(_tokenID);
  }

  function getOwnerOfTokenId(uint256 _tokenID) public view returns(address) {
    return _tokenIdToStruct[_tokenID]._owner;
  }
  ...
  function invertBool(bool _bool) private returns(bool) {
    if (_bool) {
      return false;
    } else {
      return true;
  ...
  */
  ...
  function transferName(address _from, address _to, string memory _name) public {
    ...
    _tokenIdToStruct[_reverseFromOld] = FantomsArtName("",0,address(0),true,"");
