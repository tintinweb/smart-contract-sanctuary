...
contract TestToken is ERC20Permit, Ownable {
    ...
    uint256 constant private _maxTotalSupply = 12000000e18; // 12,000,000 max supply
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s ) external {
        ...
        require(nonce == _useNonce(signatory), "TEST::delegateBySig: invalid nonce");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime;  // Last block number that MAGMAs distribution occurs.
        uint256 accTokenPerShare; // Accumulated MAGMAs per share, times 1e12. See below.
    ...
    // Yield TOKEN MAGMA!
    TestToken public magma;
    // Dev address.
    address public devAddr;
    // Fee address.
    address public feeAddr;
    // MAGMA tokens created per second.
    ...
    // The block time when MAGMA mining starts.
    uint256 public startTime;
    // The block time when MAGMA mining ends because it has total supply.
    ...
    constructor(
        ...
        address _devAddr,
        address _feeAddr,
        uint256 _magmaPerSecond,
        uint256 _startTime
    ) {
        magma = _magma;
        devAddr = _devAddr;
        feeAddr = _feeAddr;
        tokenPerSecond = _magmaPerSecond;
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 magmaReward = multiplier.mul(tokenPerSecond).mul(pool.allocPoint).div(totalAllocPoint);            
            accTokenPerShare = accTokenPerShare.add(magmaReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        magma.mint(devAddr, magmaReward.div(10)); // 10% is dev reward
        magma.mint(address(this), magmaReward);

        pool.accTokenPerShare = pool.accTokenPerShare.add(magmaReward.mul(1e12).div(lpSupply));
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > magmaBal) {
            transferSuccess = magma.transfer(_to, magmaBal);
        } else {
            transferSuccess = magma.transfer(_to, _amount);
        }
