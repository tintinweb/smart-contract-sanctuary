...
contract test is Context, IERC20, Ownable {
    ...
    address public marketingAddress = 0xCDF149A5056Fe93d927A28CC816C03260C5cE89b;
    address public buybackpoolAddress = 0xc202281a2Fc4c508821CD314cF802bCC349c5F4B;
    ...
    struct valuesFromGetValues{
      ...
      uint256 rRfi;
      uint256 rBuybackpool;
      uint256 rMarketing;
      uint256 rLiquidity;
      uint256 rBuyback;
      uint256 tTransferAmount;
      uint256 tRfi;
      uint256 tBuybackpool;
      uint256 tMarketing;
      uint256 tLiquidity;
      uint256 tBuyback;
    }
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[buybackpoolAddress] = true;
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _buybackpool, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.buybackpool = _buybackpool;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _buybackpool, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.buybackpool = _buybackpool;
    ...
    function _takeBuybackpool(uint256 rBuybackpool, uint256 tBuybackpool) private {
        totFeesPaid.buybackpool +=tBuybackpool;
        if(_isExcluded[address(this)]){
             _tOwned[address(this)]+=tBuybackpool;
        }
        _rOwned[address(this)] +=rBuybackpool;
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rBuybackpool, to_return.rMarketing, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tBuybackpool = tAmount*sellFeeRates.buybackpool/1000;
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tBuybackpool-s.tMarketing-s.tLiquidity-s.tBuyback;
        }
        else{
            s.tRfi = tAmount*feeRates.rfi/1000;
            s.tBuybackpool = tAmount*feeRates.buybackpool/1000;
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rBuybackpool,uint256 rMarketing, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rBuybackpool = s.tBuybackpool*currentRate;
        rMarketing = s.tMarketing*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        rBuyback = s.tBuyback*currentRate;
        rTransferAmount =  rAmount-rRfi-rBuybackpool-rMarketing-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rBuybackpool,rMarketing,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        _takeBuybackpool(s.rBuybackpool,s.tBuybackpool);
        _takeLiquidity(s.rLiquidity,s.tLiquidity);
        _takeMarketing(s.rMarketing, s.tMarketing);
        _takeBuyback(s.rBuyback, s.tBuyback);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tBuybackpool + s.tBuyback);
    ...
    function swapETHForTokens(uint256 amount) private {
        // generate the spookyswap pair path of token -> weth
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.buybackpool) * 2;
        ...
        uint256 ftmToAddLiquidityWith = unitBalance * feeRates.liquidity;
        ...
        // Send FTM to buyback pool
        uint256 buybackpoolAmt = unitBalance * 2 * feeRates.buybackpool;
        if(buybackpoolAmt > 0){
          payable(buybackpoolAddress).transfer(buybackpoolAmt);
        }
    }
    ...
    function updateBuybackpoolWallet(address newWallet) external onlyOwner{
        require(buybackpoolAddress != newWallet ,'Wallet already set');
        buybackpoolAddress = newWallet;
        _isExcludedFromFee[buybackpoolAddress];
    }
    ...
    function transferBEP20(address tokenAddress) external onlyOwner{
