...
contract DUMBHovse {
    ...
    function purchase(uint256 pid) external {
        PriceInfo storage pInfo = priceInfo[pid];

        require(pInfo.tokenAmount > 0, 'Token amount should be greater than 0.');
        require(
            DUMB.allowance(msg.sender, address(this)) >= pInfo.tokenAmount,
            "Insufficient allowance."
        );
        require(
            DUMB.balanceOf(msg.sender) >= pInfo.tokenAmount,
            "Token amount should be less than the balance."
        );

        DUMB.transferFrom(msg.sender, address(this), pInfo.tokenAmount);

        emit Purchase(msg.sender, pInfo.tokenAmount, pInfo.gameCoinAmount);
    }
