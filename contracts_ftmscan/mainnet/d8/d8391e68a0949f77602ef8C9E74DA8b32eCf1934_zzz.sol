...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 TREEB = IBEP20(0xc60D7067dfBc6f2caf30523a064f416A5Af52963); // TREEB
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = TREEB.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WFTM;
        path[1] = address(TREEB);

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: msg.value
        }(0, path, address(this), block.timestamp);

        uint256 amount = TREEB.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            TREEB.transfer(shareholder, amount);
...
contract zzz is IBEP20, Auth {
    using SafeMath for uint256;

    uint256 public constant MASK = type(uint128).max;
    address TREEB = 0xc60D7067dfBc6f2caf30523a064f416A5Af52963; // TREEB
    address public WFTM = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEAD_NON_CHECKSUM = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "zzz";
    string constant _symbol = "zzz";
    ...
    address public autoLiquidityReceiver =
        0x7B2340D29bb824778D73Ae17f1042f10d7bC6d2e; //liq address
    address public marketingFeeReceiver =
        0x7B2340D29bb824778D73Ae17f1042f10d7bC6d2e; // marketing address
