...
/**
 *Submitted for verification at FtmScan.com on 2021-10-31
*/
...
contract VNOSale {
    ...
    constructor(address _baseToken, address _farmingToken, address _dev, uint256 _total) {
    ...
    function deposite() public onlyEOA {
        ...
        uint256 b = IERC20(baseToken).balanceOf(address(this));
        IERC20(baseToken).transfer(dev,b);
    ...
    function endStart() public {
        ...
        uint256 b = IERC20(baseToken).balanceOf(address(this));
        IERC20(baseToken).transfer(dev,b);
        b = IERC20(farmingToken).balanceOf(address(this));
        IERC20(farmingToken).transfer(dev,b);
    }
    ...
    function getAddressStatus() view external returns(uint8) {
        if(investor[msg.sender] && claimed[msg.sender]) return 2;
        else if(investor[msg.sender]) return 1;
        else return 0;
    }
}