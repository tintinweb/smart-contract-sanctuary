...
// File: ..\Contracts\adapters\FTM\SpiritSwap\SpiritSwapAdapter.sol
...
contract SpiritSwapAdapter is IAdapter {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ISpiritFarm constant SpiritFarm = ISpiritFarm(0x9083EA3756BDE6Ee6f27a6e996806FBD37F6F093);
    ...
    function _zapAny(address sellToken, uint256 poolId, uint256 amount, uint256 min_out, address to) external override returns(uint256) {
        ...
        if (poolId == 0) {
            if (sellToken != address(spirit)) {
                if (address(weth) == sellToken) {
                    address[] memory path = new address[](2);
                    path[0] = address(weth);
                    path[1] = address(spirit);
    ...
    function earnedReward(address, uint256 poolId, address user, uint256) external view override returns (uint256) {
        return SpiritFarm.pendingSpirit(poolId, user);
    }
