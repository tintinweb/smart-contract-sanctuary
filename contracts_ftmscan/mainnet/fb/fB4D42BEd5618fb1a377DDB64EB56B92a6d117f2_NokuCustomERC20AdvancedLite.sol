 ...
 */
contract SupportsInterfaceWithLookup is AccessControl {
  ...
  function supportsInterface(bytes4 _interfaceId) public view virtual override returns (bool) {
    return super.supportsInterface(_interfaceId) || supportedInterfaces[_interfaceId];
  }
...
abstract contract NokuCustomTokenLite {
    using SafeMath for uint256;

    /**
    * @dev Presence of this function indicates the contract is a Custom Token Lite.
    */
    function isCustomTokenLite() public pure returns(bool) {
        return true;
    }
...
abstract contract ERC20BurnableWithFinish is Ownable, ERC20Burnable {
    ...
    */
    function finishBurning() public onlyOwner canBurn returns(bool) {
        burningFinished = true;
        emit LogBurnFinished();
        return true;
    }

    function burn(uint256 amount) public override virtual canBurn {
        super.burn(amount);
    }

    function burnFrom(address account, uint256 amount) public override virtual canBurn {
...
abstract contract ERC20Mintable is Context, Ownable, ERC20 {
   ...
   */
  function mint(address _to, uint256 _amount) virtual hasMintPermission canMint public {
    _mint(_to, _amount);
  }
...
contract Frozenlist is Issuable {
  ...
  bytes32 public constant FROZENLISTED_ROLE = keccak256("FROZENLISTED_ROLE");
  ...
  modifier onlyIfNotFrozen(address _operator) {
    require(!hasRole(FROZENLISTED_ROLE, _operator), "Account frozen");
    _;
  }
  ...
  function addAddressToFrozenlist(address _operator) public onlyIssuer {
    grantRole(FROZENLISTED_ROLE, _operator);
  ...
  function frozenlist(address _operator) public view returns (bool) {
    return hasRole(FROZENLISTED_ROLE, _operator);
  }
   ...
   */
  function addAddressesToFrozenlist(address[] memory _operators) public onlyIssuer {
  ...
  function removeAddressFromFrozenlist(address _operator) public onlyIssuer {
    revokeRole(FROZENLISTED_ROLE, _operator);
  }
   ...
   * @param _operators addresses
   */
  function removeAddressesFromFrozenlist(address[] memory _operators) public onlyIssuer {
    for (uint256 i = 0; i < _operators.length; i++) {
...
contract Whitelist is Issuable {
  ...
  modifier onlyIfWhitelisted(address _operator) {
    if(whitelistEnabled) {
      require(hasRole(WHITELISTED_ROLE, _operator), "Account not whitelisted");
    }
    _;
  }
  ...
  function addAddressToWhitelist(address _operator) public onlyIssuer {
    grantRole(WHITELISTED_ROLE, _operator);
  }
  ...
  function whitelist(address _operator) public view returns (bool) {
    return hasRole(WHITELISTED_ROLE, _operator);
  }
   ...
   * @param _operators addresses
   */
  function addAddressesToWhitelist(address[] memory _operators) public onlyIssuer {
    for (uint256 i = 0; i < _operators.length; i++) {
  ...
  function removeAddressFromWhitelist(address _operator) public onlyIssuer {
    revokeRole(WHITELISTED_ROLE, _operator);
  }
   ...
   * @param _operators addresses
   */
  function removeAddressesFromWhitelist(address[] memory _operators) public onlyIssuer {
    for (uint256 i = 0; i < _operators.length; i++) {
...
contract BasicSecurityToken is ERC20BurnableWithFinish, ERC20Mintable, Frozenlist, Whitelist {
    ...
    */
    ...
    function mint(address account, uint256 amount) override public onlyOwner onlyIfNotFrozen(account) onlyIfWhitelisted(account) {
        super.mint(account, amount);
    }

    function transfer(address recipient, uint256 amount) override public onlyIfNotFrozen(_msgSender()) onlyIfNotFrozen(recipient) onlyIfWhitelisted(recipient) returns (bool) {
        require(super.transfer(recipient, amount), "Transfer failed");

        return true;
    }

    function transferFrom(address sender, address recipient, uint256 amount) override public onlyIfNotFrozen(sender) onlyIfNotFrozen(recipient) onlyIfWhitelisted(recipient) returns (bool) {
    ...
    function reassign(address from, address to, uint256 amount) public onlyIssuer {
        _transfer(from, to, amount);

        emit FundsReassigned(from, to, amount);
    }

    function revoke(address from) public onlyIssuer {
        uint256 fundsRevoked = balanceOf(from);
        _transfer(from, _msgSender(), fundsRevoked);

        emit FundsRevoked(from, fundsRevoked);
    }

    function revoke(address from, uint256 amount) public onlyIssuer {
        _transfer(from, _msgSender(), amount);

        emit FundsRevoked(from, amount);
    }
...
/**
* @dev The NokuCustomERC20AdvancedToken contract is a custom ERC20AdvancedLite, a security token ERC20-compliant, token available in the Noku Service Platform (NSP).
