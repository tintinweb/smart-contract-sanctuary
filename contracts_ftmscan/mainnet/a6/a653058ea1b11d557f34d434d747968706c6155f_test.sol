...
contract test is Context, IERC20, Ownable {
    ...
    // Liquidity Settings
    IUniswapV2Router02 private _spookyswapV2Router; // Spookyswap V2 Router
    address private _spookyswapV2LiquidityPair;     // Spookyswap V2 liquidity pairing for FBSI/WFTM
    ...
    // Addresses
    address payable public _marketingAddress = payable(0x476855Fc0Ae7B81b1950BC09b50A500bEdEAbB76); // Marketing address used to pay for marketing
    ...
    uint256 private _tTotal = 1* 10**9 * 10**9; // 1 quadrillion
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _totalReflections; // Total reflections
    
    // Token Tax Settings
    uint256 public  _taxFee           = 3;  // 6% tax 
    uint256 public  _sellTaxFee       = 7; // 12% tax 
    uint256 private _whaleSellTaxFee  = 30; // 18% tax 
    uint256 private _previousTaxFee;
    
    // Min / Max for Token Tax settings and limits. These values can not be changed anymore.
    uint256 private constant _maxTaxFee = 10; // Tax fee cannot be set beyond 10%
    uint256 private constant _maxSellTaxFee = 20; // Sell tax fee cannot be set beyond 20%
    uint256 private constant _maxWhaleSellTaxFee = 30; // Whale sell tax fee cannot be set beyond 30%
    uint256 private constant _minMaxTxAmount = 500 * 10**9 * 10**9; // Max Tx amount must be set to 500 billion or more
    
    // Token Limits
    uint256 private _maxTxAmount        = 1 * 10**9 * 10**9;  // 1 quadrillion
    uint256 private _tokenSwapThreshold = 1 * 10**6 * 10**9; // 100 billion
    
    // Anti-Whale Settings 
    uint256 private _whaleSellThreshold = 180 * 10**12 * 10**9; // 180 trillion (30% of initial supply)
    uint    private _whaleSellTimer     = 1 days;            
    mapping (address => uint256) private _amountSold;
    mapping (address => uint) private _timeSinceFirstSell;
    
    // BUYBACK 
    bool    private _enableBuyback        = false;             // Disabled on deployment
    uint256 private _buybackFTMThreshold  = 50 * 10**18;       // 50 FTM will be required to be inside the contract before a buyback will be performed
    uint256 private _buybackFTMPercentage = 100;               // 100% of reserved FTMs is used to buyback tokens
     ...
     */
    function withdrawFTM(uint256 amount) private onlyOwner() {
        if(amount == 0) {
     ...
     */
    function setRouterAddress(address router) private onlyOwner() {
    ...
    function setBuybackCROThreshold(uint256 ftmAmount) external onlyOwner() {
        require(_buybackFTMThreshold != ftmAmount, "Set a different amount");
        _buybackFTMThreshold = ftmAmount;
    }
    
    function setBuybackCROPercentage(uint256 percentage) external onlyOwner() {
