...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STOKEN }
    ...
    uint public sTOKENQueue; // Delays change to sTOKEN address
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20Metadata( sTOKEN ).balanceOf( msg.sender ); // Can only borrow against sTOKEN held
    ...
    function repayDebtWithTOKEN( uint _amount ) external {
        ...
        IERC20Burnable( TOKEN ).burnFrom( msg.sender, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20Metadata( TOKEN ).totalSupply().sub( totalDebt ) );
    }
    ...
    function auditReserves() external onlyOwner() {
        ...
        for( uint i = 0; i < reserveTokens.length; i++ ) {
            reserves = reserves.add (
                valueOf( reserveTokens[ i ], IERC20Metadata( reserveTokens[ i ] ).balanceOf( address(this) ) )
            );
        }
        for( uint i = 0; i < liquidityTokens.length; i++ ) {
            reserves = reserves.add (
                valueOf( liquidityTokens[ i ], IERC20Metadata( liquidityTokens[ i ] ).balanceOf( address(this) ) )
            );
        }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20Metadata( TOKEN ).decimals() ).div( 10 ** IERC20Metadata( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STOKEN ) { // 9
