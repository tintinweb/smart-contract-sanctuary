...
// File: contracts/staking/TokenVestingTeam.sol
...
contract TokenVestingTeam {
    ...
    function claim(address _receiver) external {
        require(startTime != 0);
        uint256 newLastTimeMint = lastTimeRewardMint + unlockDuration;
        require(block.timestamp >= newLastTimeMint, "Unlock every 2 weeks.");
        ...
        if (claimable > v.claimed) {
            ...
            v.claimed = claimable;
            if(amount > 0){
              lastTimeRewardMint = newLastTimeMint;
            }
