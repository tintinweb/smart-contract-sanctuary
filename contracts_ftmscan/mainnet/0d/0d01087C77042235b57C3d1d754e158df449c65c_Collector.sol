...
contract Collector is Ownable {
    ...
    function _allowance(address _token, address _wallet) internal view returns (uint256) {
    ...
    function balanceOf(address _token) public view returns (uint256) {
        ...
        for (uint8 i = 0; i < _wallets.length(); ++i) {
            total += _balanceOf(_token, _wallets.at(i));
        }

        return total;
    }

    function _balanceOf(address _token, address wallet)
        internal
        view
        returns (uint256)
    {
        return IERC20(_token).balanceOf(wallet);
    }
    ...
    function transfer(address _token) public returns (uint256) {
        ...
        for (uint8 i = 0; i < _wallets.length(); ++i) {
            ...
            uint256 allowance = _allowance(_token, _wallets.at(i));
