...
contract Market is IMarket, Ownable, ReentrancyGuard {
    ...
    modifier ownerOrMediaCall(address caller) {
        require(
            caller == msg.sender || msg.sender == mediaContract,
    ...
    function bidForTokenBidder(
        ...
        return _tokenBidders[tokenAddress][tokenId][bidder];
    }
    ...
    function setAsk(
        ...
        override
        nonReentrant
        onlyApprovedOrOwner(tokenOwner, tokenAddress, tokenId)
        ownerOrMediaCall(tokenOwner)
    {
        ...
        onlyExistingToken(tokenAddress, tokenId)
    {
        ...
        Bid storage existingBid = _tokenBidders[tokenAddress][tokenId][
            bid.bidder
        ];
        ...
        _tokenBidders[tokenAddress][tokenId][bid.bidder] = Bid(
            msg.value,
            bid.bidder,
            bid.recipient
        );
        ...
        if (
            ...
            // Finalize exchange
            _finalizeNFTTransfer(tokenAddress, tokenId, bid.bidder);
        }
    }
    ...
    function removeBid(
        ...
        Bid storage bid = _tokenBidders[tokenAddress][tokenId][bidder];
        uint256 bidAmount = bid.amount;

        require(bid.amount > 0, "Market: cannot remove bid amount of 0");

        emit BidRemoved(tokenAddress, tokenId, bid);
        delete _tokenBidders[tokenAddress][tokenId][bidder];
    ...
    function acceptBid(
        ...
        ownerOrMediaCall(tokenOwner)
    {
        Bid memory bid = _tokenBidders[tokenAddress][tokenId][
            expectedBid.bidder
        ];
    ...
    function _finalizeNFTTransfer(
        ...
        Bid memory bid = _tokenBidders[tokenAddress][tokenId][bidder];
    ...
    function transfer(
        ...
        removeAsk(from, tokenAddress, tokenId);
    ...
    IMedia
{
    ...
    function _transfer(
        ...
        IMarket(marketContract).removeAsk(from, address(this), tokenId);
...
interface IMedia {
    ...
    //  */
    // function setBid(uint256 tokenId, IMarket.Bid calldata bid) external payable;
