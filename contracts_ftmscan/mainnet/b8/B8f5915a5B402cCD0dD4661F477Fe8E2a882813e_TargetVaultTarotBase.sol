...
import "./TargetVaultTarot.sol";

contract TargetVaultTarotBase is TargetVaultTarot {
    ...
    constructor(
        ...
        tarotPoolToken = ITarotBorrowable(_tarotPoolToken);
...
import "@openzeppelin/contracts/math/SafeMath.sol";
import "@openzeppelin/contracts/token/ERC20/SafeERC20.sol";
import "../../../interfaces/targetVaults/protocols/ITarotBorrowable.sol";
import "../../../interfaces/targetVaults/protocols/ITarotRouter.sol";
import "../TargetVault.sol";

contract TargetVaultTarot is TargetVault {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    /// @dev Tarot router address
    address public constant tarotRouter = 0x283e62CFe14b352dB8e30A9575481DCbf589Ad98;

    /// @dev Tarot borrowable pool token address
    ITarotBorrowable public tarotPoolToken;
    ...
    function deposit() external virtual onlyVault {
        ...
        if (_balance > 0) {
            IERC20(token).safeIncreaseAllowance(address(tarotRouter), _balance);
            ITarotRouter(tarotRouter).mint(address(tarotPoolToken), _balance, address(this), block.timestamp + 120);
    ...
    function withdraw(uint256 _amount) external virtual onlyVault {
        if (_amount > 0) {
            tarotPoolToken.approve(address(tarotRouter), _amount);
            ITarotRouter(tarotRouter).redeem(address(tarotPoolToken), _amount, address(this), block.timestamp + 120, "");
            uint256 _balance = IERC20(token).balanceOf(address(this));
    ...
    function _collectFees() internal virtual {
        uint256 _newPricePerShare = targetPricePerShare();
        if (cachedPricePerShare > 0 && cachedPricePerShare < _newPricePerShare) {
            uint256 _balance = tarotPoolToken.balanceOf(address(this));
            ...
            if (_amount > 10**minimumFeesDecimal) {
                tarotPoolToken.approve(address(tarotRouter), _amount);
                ITarotRouter(tarotRouter).redeem(address(tarotPoolToken), _amount, address(this), block.timestamp + 120, "");
            }
            ...
            if (_fees > 0) {
                if (autoBuyBack) {
                    address _buyBackToken = tokenToBuyBackPath[tokenToBuyBackPath.length - 1];
                    uint256 buyBackBefore = IERC20(_buyBackToken).balanceOf(address(this));
                    ...
                    uint256 buyBackAfter = IERC20(_buyBackToken).balanceOf(address(this));
                    uint256 buyBackAmount = buyBackAfter.sub(buyBackBefore);
                    IERC20(_buyBackToken).safeTransfer(feesCollector, buyBackAmount);
    ...
    function emergencyWithdrawAll() external virtual onlyOwner {
        tarotPoolToken.approve(address(tarotRouter), tarotPoolToken.balanceOf(address(this)));
        ITarotRouter(tarotRouter).redeem(
            address(tarotPoolToken),
            tarotPoolToken.balanceOf(address(this)),
            address(this),
            block.timestamp + 120,
            ""
        );
    ...
    /**
     * @dev Balance of address in target vault token
     */
    function balanceOf() public view virtual returns (uint256) {
        return tarotPoolToken.balanceOf(address(this));
    }
    ...
    function targetPricePerShare() public view virtual returns (uint256) {
        return tarotPoolToken.exchangeRateLast();
    }
    ...
    function vaultToken() public view virtual returns (address) {
        return address(tarotPoolToken.underlying());
    }
    ...
    function setBuyBackToken(address _buyBackToken) public override onlyOwner {
        require(_buyBackToken != address(0), "TargetVaultTarot(setBuyBackToken): buy back token cannot be zero address");
...
interface ITarotRouter {
    function mint(
        address _poolToken,
        uint256 _amount,
        address _to,
        uint256 _deadline
    ) external returns (uint256 _tokens);
