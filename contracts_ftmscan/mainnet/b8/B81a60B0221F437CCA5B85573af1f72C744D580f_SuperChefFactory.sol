...
contract SuperChefInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // The address of the smart chef factory
    address public SUPER_CHEF_FACTORY;
    ...
    // The block.timestampwhen DMD mining ends.
    uint256 public bonusEndTime;

    // The block.timestampwhen DMD mining starts.
    uint256 public startTime;

    // The block.timestampof the last pool update
     ...
     * @param _startTime: start block
     * @param _bonusEndTime: end block
    ...
    function initialize(
        ...
        require(msg.sender == SUPER_CHEF_FACTORY, "Not factory");
     ...
     */
    function updateStartAndEndBlocks(uint256 _startTime, uint256 _bonusEndTime)
        external
        onlyOwner
    {
        require(block.timestamp < startTime, "Pool has started");
        require(
            _startTime < _bonusEndTime,
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTime && stakedTokenSupply != 0) {
            ...
            uint256 dmdReward = multiplier.mul(rewardPerSecond);
            uint256 adjustedTokenPerShare = accTokenPerShare.add(
                dmdReward.mul(PRECISION_FACTOR).div(stakedTokenSupply)
            );
            return
                user
                    .amount
...
contract SuperChefFactory is Ownable {
    event NewSuperChefContract(address indexed superChef);
     ...
     * @param _stakedToken: staked token address
     * @param _rewardToken: reward token address
     * @param _rewardPerSecond: reward per Time (in rewardToken)
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(SuperChefInitializable).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(_stakedToken, _rewardToken, _startTime)
        );
        address superChefAddress;

        assembly {
            superChefAddress := create2(
                0,
                add(bytecode, 32),
                mload(bytecode),
                salt
        ...
        SuperChefInitializable(superChefAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerSecond,
            _startTime,
        ...
        emit NewSuperChefContract(superChefAddress);
    }
}