...
import './interfaces/IPortalvault.sol';
...
/* ______          ______            ______                           ______   
 ...
 _\ \__/_/_       \ \__/_/_         \ \__/_/         /\___\         _\ \__/_/_ 
/\_\ \_____\      /\ \_____\        /\ \___\        /\ \___\       /\_\ \_____\
\ \ \/ / / /      \ \/ / / /        \ \/ / /        \ \/ / /       \ \ \/ / / /
 \ \/ /\/ /        \/_/\/ /          \/_/_/          \/_/_/         \ \/ /\/ / 
  \/_/\/_/            \/_/                                           \/_/\/_/  
                _____           _        _                  _ _                
               |  __ \         | |      | |                | | |               
               | |__) |__  _ __| |_ __ _| |_   ____ _ _   _| | |_              
               |  ___/ _ \| '__| __/ _` | \ \ / / _` | | | | | __|             
               | |  | (_) | |  | || (_| | |\ V / (_| | |_| | | |_              
               |_|   \___/|_|   \__\__,_|_| \_/ \__,_|\__,_|_|\__|             
...
{
    // royalties
    mapping(uint256 => uint256) public tokenIdToRoyaltyAmount;
    ...
    // addresses that can unlock tokens
    modifier onlyZuul() {
    ...
    constructor(
        ...
        ERC721PresetMinterPauserAutoIdRoyalty('Portalvault', 'PV', '', 0)
        ERC721SplitWithdrawals(_recipients, _splits)
    {
        // check for withdraw() in recieve() function of royalty contracts
        royaltyERC20Tokens = _erc20Tokens;
    }

    /// @notice mint a portalvault token
    /// @dev Only a minter can call this method
    /// @param _to the address to gift the portalvault to
    /// @param _tokenId the token ID of the portalvault to gift
    /// @param _tokenURI the token ID of the portalvault to gift
    /// @param _royaltyRecipients array of royalty recipient addresses
    /// @param _royaltySplits array of royalty splits, base 10000.
    function mintPortalvault(
        address _to,
        uint256 _tokenId,
        ...
        // create a royalty contract for this token ID if needed
        _createRoyaltyContractForToken(
            _tokenId,
            _royalty,
            _royaltyRecipients,
            _royaltySplits
    ...
    {
        return
            bytes(__tokenURIs[_tokenId]).length > 0
                ? __tokenURIs[_tokenId]
                : _baseURI();
    }
    ...
    {
        return (
            tokenIdToRoyaltyContract[_tokenId],
            (_salePrice * tokenIdToRoyaltyAmount[_tokenId]) / BASE
        );
    }
    ...
    /// @dev Set the base URI for the token, only by the minter role
    ...
    /// @dev Only the minter role can call this method
    /// @param _marketplace the marketplace contract address to whitelist
    /// @param _allowed the whitelist status
    function setMarketplaceApproval(address _marketplace, bool _allowed)
        external
        onlyZuul
    {
        emit MarketplaceApprovalUpdated(
            _marketplace,
            _allowed,
            __marketplaces[_marketplace]
        );
    ...
    function setErc20RoyaltyTokens(address[] memory _royaltyERC20Tokens)
        external
        onlyZuul
    {
    ...
    /// @dev Creates a new royalty contract for the token ID, or returns an existing one
    function _createRoyaltyContractForToken(
        uint256 _tokenId,
        uint256 _royalty,
        address[] memory _royaltyRecipients,
        uint16[] memory _royaltySplits
    ) internal returns (address) {
        // set the royalty for this token ID
        tokenIdToRoyaltyAmount[_tokenId] = _royalty;

        if (_royalty > 0) {
            // deploy the contract
            address _royaltyContract = address(
                new PortalheadsRoyaltyForwarder(
                    address(this),
                    _royaltyRecipients,
                    _royaltySplits
                )
            ...
            tokenIdToRoyaltyContract[_tokenId] = _royaltyContract;
        } else if (tokenIdToRoyaltyContract[_tokenId] != address(0)) {
            // no royalty, so set the contract to 0 address
            tokenIdToRoyaltyContract[_tokenId] = address(0);
        }

        return tokenIdToRoyaltyContract[_tokenId];
    }
...
interface IPortalvault {
    event PortalvaultsOpened(address indexed minter, uint8 amount);
