...
contract MIMCompounder is Ownable {
	...
	function mimBalance() public view returns (uint256) {
		return _tokenBalance(mim);
	}

	function weveBalance() public view returns(uint256) {
		return _tokenBalance(weve);
	}
	...
	function compound(uint256 _swapSlippage) public onlyOwner {
		_withdrawFromMasterchef(0);

		uint256 amountToSell = weveBalance();
		...
		uint256[] memory amountOut = IMinimalRouter(exchange).getAmountsOut(amountToSell, path);

		uint256 minAmount = amountOut[1] - ((amountOut[1] * _swapSlippage) / SLIPPAGE_DENOMINATOR);

		require(minAmount > 0, "Invalid output amount");

		IMinimalRouter(exchange).swapExactTokensForTokens(amountToSell, minAmount, path, address(this), block.timestamp + 600);

		_depositToMasterchef(mimBalance());
	}
	...
	function drain() public onlyOwner {
		if (mimBalance() > 0) {
			bool mimTransfer = IERC20(mim).transfer(msg.sender, mimBalance());
			require(mimTransfer, "Failed to transfer MIM");
		}

		if (weveBalance() > 0) {
			bool weveTransfer = IERC20(weve).transfer(msg.sender, weveBalance());
			require(weveTransfer, "Failed to transfer WeVe");
		}
	}
