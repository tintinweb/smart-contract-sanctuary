...
contract MasterChefV3 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accBanksyPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BANKSYs to distribute per second.
        uint256 lastRewardTime;  // Last time that BANKSYs distribution occurs.
        uint256 accBanksyPerShare;   // Accumulated BANKSYs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    uint256 public constant banksyMaximumSupply = 1 * (10 ** 6) * (10 ** 18); // 1,000,000 banksy
    uint256 public constant MAX_EMISSION_RATE = 1 * (10 ** 18); // 1
    // The BANKSY TOKEN!
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        // deposits aren't timed after farming ends.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (banksy.totalSupply() >= banksyMaximumSupply && emmissionEndTime == type(uint256).max)
    ...
    function setTreasuryAddress(address _treasuryAddress) external onlyOwner {
        require(_treasuryAddress != address(0), "!nonzero");
    ...
    function setEmissionRate(uint256 _banksyPerSecond) external onlyOwner {
        require(_banksyPerSecond > 0);
        require(_banksyPerSecond < MAX_EMISSION_RATE);

        massUpdatePools();
        banksyPerSecond = _banksyPerSecond;
        
        emit SetEmissionRate(msg.sender, banksyPerSecond, _banksyPerSecond);
    }
}
