...
contract CheckDex {
    ...
    function executeArb(address router1Address, address router2Address, address[] calldata inPath, address[] calldata outPath, uint256 max_can_sell_tokenA,uint256 max_buy_tokenB) external {
        ...
        router1.swapTokensForExactTokens(20000, max_can_sell_tokenA, inPath, address(this), block.timestamp + 1000);
        IERC20(outPath[0]).approve(address(router2), 10**30);
        require (IERC20(outPath[0]).balanceOf(address(this)) == 20000, "MY ERROR");
        router2.swapExactTokensForTokensSupportingFeeOnTransferTokens(max_can_sell_tokenA, 20000, outPath, msg.sender, block.timestamp + 1000);
