...
contract Claimable {
    ...
    constructor (address barnAddr, address woolfAddr){
        _barn = Barn(barnAddr);
        _woolf = Woolf(woolfAddr);
    }

    function getClaimable(uint256[] memory woolfs)
        public
        view
        returns (uint256[] memory claimable)
    {
        claimable = new uint256[](200);
        for (uint256 i = 0; i < woolfs.length; i++) {
            bool isSheep = _barn.isSheep(woolfs[i]);
            if (isSheep) {
                uint256 v;
                (, v, ) = _barn.barn(woolfs[i]);
                uint256 owed = ((block.timestamp - v) *
                    _barn.DAILY_WOOL_RATE()) / 1 days;

                claimable[i] = owed;
            } else {
                uint256 alphaIndex;
                (, , , , , , , , , alphaIndex) = _woolf.tokenTraits(woolfs[i]);
                uint256 alpha = 8 - alphaIndex;
                uint256 woolPerAlpha = _barn.woolPerAlpha();
                uint256 packIndices = _barn.packIndices(woolfs[i]);
                uint256 v;
                (, v, ) = _barn.pack(alpha, packIndices);
                uint256 owed = alpha * (woolPerAlpha - v);
                claimable[i] = owed;
            }
        }
    }
