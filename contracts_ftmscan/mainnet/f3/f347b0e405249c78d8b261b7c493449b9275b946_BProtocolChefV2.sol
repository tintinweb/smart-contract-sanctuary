...
contract BProtocolChefV2 is Ownable {
    ...
    mapping(uint => IERC20[]) public collateralTokens;
    ...
    /// @param _collateralTokens Addresses of the collateral types liquidated by this BAMM
    function add(
        uint allocPoint, 
        IBAMMv2 _lpToken, 
        ...
        for (uint i = 0; i < _collateralTokens.length; i++) {
            collateralTokens[lpTokens.length - 1].push(_collateralTokens[i]);
        }
    ...
    /// @param _collateralTokens Addresses of the collateral types liquidated by this BAMM
    /// @param overwriteCollateralTokens True if _collateralTokens should be `set`. Otherwise `_collateralTokens` is ignored.
    function set(
        uint _pid, 
        uint _allocPoint, 
        IRewarder _rewarder, 
        ...
        IERC20[] memory _collateralTokens,
        bool overwriteCollateralTokens
    ) public onlyOwner {
        totalAllocPoint = totalAllocPoint - poolInfo[_pid].allocPoint + _allocPoint;
        poolInfo[_pid].allocPoint = uint64(_allocPoint);
        if (overwriteRewarder) { 
            rewarders[_pid] = _rewarder; 
        }
        if (overwriteCollateralTokens) { 
            delete collateralTokens[_pid];
            for (uint i = 0; i < _collateralTokens.length; i++) {
                collateralTokens[_pid].push(_collateralTokens[i]);
            }
        }
        emit LogSetPool(
            _pid, 
            _allocPoint, 
            overwriteRewarder ? _rewarder : rewarders[_pid], 
            overwriteRewarder
        );
    }
    ...
    function _withdrawFromBAMM(uint pid, uint shares, address to) internal {
        ...
        for (uint i = 0; i < collateralTokens[pid].length; i++) {
            IERC20 collat = collateralTokens[pid][i];
            collat.transfer(msg.sender, collat.balanceOf(address(this)));
        }
