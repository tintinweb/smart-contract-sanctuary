...
contract OlaLeNLens {
    ...
    function viewLendingNetwork(ComptrollerOlaLeNLensInterface unitroller) external view returns (LendingNetworkView memory) {
    ...
    function viewMarketInternal(CToken market, MinistryOlaLenLensInterface ministry) internal view returns (MarketView memory) {
        MarketView memory marketView;

        address underlying = market.underlying();
        bool isNativeAsset = underlying == market.nativeCoinUnderlying();
        uint decimals = isNativeAsset ? 18 : Erc20ForOlaLeNLens(underlying).decimals();
        ...
        marketView.supplyUnits = underlyingSupplyRaw.mul(10**18).div(10**decimals);
        marketView.supplyUsd = marketView.supplyUnits * assetPrice;

        uint underlyingBorrowsRaw = market.totalBorrows();
        marketView.borrowsUnits = underlyingBorrowsRaw.mul(10 ** 18).div(10**decimals);
        marketView.borrowsUsd = marketView.borrowsUnits * assetPrice;
