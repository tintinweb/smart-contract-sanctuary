...
contract Granary
{
	...
	constructor (address _w, address _m, address _e, uint8 _p, address _R, address[] memory _r, string memory _id, address[3] memory _v)
	{
		want=IERC20(_w);
		mc=IMasterchef(_m);
		earn=IERC20(_e);
	...
	*/
	...
	function work(address ben) internal
	{
		...
		R.addLiquidity(
			...
			(earn.balanceOf(address(this)).mul(90).div(100)),
	...
	function tvl() public view returns(uint256)
	{
		ITVL tc = ITVL(utvl);
		uint256 aum = mc.userInfo(allnums[0],address(this)).amount + IERC20(want).balanceOf(address(this));
		return ((tc.p_lpt_usd(tvlu, tvlp)).mul(aum)).div(10**IERC20(want).decimals());
	}
