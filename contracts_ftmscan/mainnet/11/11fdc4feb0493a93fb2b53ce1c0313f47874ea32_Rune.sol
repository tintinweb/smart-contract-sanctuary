...
contract Rune is ERC20, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 RUNE-WFTM LP -> RUNE pool
    ...
    /**
     * @notice Constructs the RUNE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("RUNE", "RUNE") {
        // Mints 1 RUNE to contract creator for initial pool setup
    ...
    function _getRunePrice() internal view returns (uint256 _tombPrice) {
        try IOracle(tombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Rune: failed to fetch RUNE price from Oracle");
        }
    }
    ...
    function setRuneOracle(address _tombOracle) public onlyOperatorOrTaxOffice {
    ...
    /**
     * @notice Operator mints RUNE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of RUNE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRunePrice = _getRunePrice();
            currentTaxRate = _updateTaxRate(currentRunePrice);
            if (currentRunePrice < burnThreshold) {
                burnTax = true;
            }
        }
