...
contract JonesRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. JONES to distribute in the pool.
        uint256 lastRewardTime; // Last time that JONES distribution occurred.
    ...
    // The time when JONES mining starts.
    ...
    constructor(address _tomb, uint256 _poolStartTime) public {
        ...
        epochEndTimes[0] = poolStartTime + 24 hours; // Day 2-3
        epochEndTimes[1] = epochEndTimes[0] + 24 hours; // Day 3-4

        epochTombPerSecond[0] = epochTotalRewards[0].div(24 hours);
        epochTombPerSecond[1] = epochTotalRewards[1].div(24 hours);
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "JonesRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "JonesRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's JONES allocation point. Can only be called by the owner.
