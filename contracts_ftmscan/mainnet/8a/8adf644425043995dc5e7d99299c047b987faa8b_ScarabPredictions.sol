...
contract ScarabPredictions {
  ...
  */
  ...
  /**
  * @dev Modifier that only allows a prediction market in the Closed state to execute the function.
  * @param _marketId The id of the prediction market instance on which this modifier is called.
  */
  modifier onlyClosed(uint _marketId) {
    require(getMarketStatus(_marketId) == MarketStatus.Closed, "=== The prediction market must be closed to call this function ===");
    require(block.timestamp > getMarketExpireTime(_marketId), "=== The prediction market cannot be claimable yet :( ===");
    _;
  }
  ...
  */
  function createNewMarket() public onlyOperator onlyClosed(marketCount) returns(bool success) {
  ...
  * @param _marketId The id of the prediction market instance on which this function is called.
  */
  function withdrawWinnings(uint _marketId) external onlyClosed(_marketId) {
    Market storage m = markets[_marketId];
