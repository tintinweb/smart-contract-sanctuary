...
contract AlphaStrategyFantom is OwnableUpgradeable {
    ...
    */
    ...
    function pendingReward() public view returns (uint256) {
        return IMasterChef(xBooStakingMasterchef).pendingReward(xBooStakingPoolId, address(this));
    }
    ...
    function withdrawReward(address user) public onlyVault {
        ...
        uint256 _balance = IERC20(xBooRewardsToken).balanceOf(address(this));
        ...
        if(_pending > 0){
            // send reward to user
            IERC20(xBooRewardsToken).safeTransfer(user, _pending);
    ...
    */
    ...
    function setXBooStakingPoolId(uint256 _poolId, address _rewardsToken) public onlyMultisig {
        require(address(_rewardsToken) != address(0), "RewardToken address should be bigger than zero");

        exitRewardsForXBoo();

        xBooStakingPoolId = _poolId;
        xBooRewardsToken = _rewardsToken;
