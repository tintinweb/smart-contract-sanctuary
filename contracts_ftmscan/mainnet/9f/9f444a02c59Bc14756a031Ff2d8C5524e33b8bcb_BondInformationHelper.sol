...
contract BondInformationHelper {
    function name(address _contractAddress) public view returns (Result memory) {
        ...
        return Result({
            value0: ERC20( ILiquidityToken( principle ).token0() ).name(),
            value1: ERC20( ILiquidityToken( principle ).token1() ).name()
        });
    }
    ...
    function symbol(address _contractAddress) public view returns (Result memory) {
        ...
        return Result({
            value0: ERC20( ILiquidityToken( principle ).token0() ).symbol(),
            value1: ERC20( ILiquidityToken( principle ).token1() ).symbol()
        });
    }
}