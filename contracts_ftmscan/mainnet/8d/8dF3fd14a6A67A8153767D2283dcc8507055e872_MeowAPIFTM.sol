...
contract MeowAPIFTM {
  ...
  function ibTokenApy(address _vault) public view returns (uint256) {
    uint256 decimals = uint256(IERC20(_vault).decimals());
    uint256 reserveBps = IVaultConfig(IVault(_vault).config()).getReservePoolBps();
    uint256 apr = getInterestRate(_vault).mul(utilization(_vault)).div(10**decimals);
  ...
  function getSpiritMasterChef(address _worker) public view returns (address) {
  ...
  function getYieldFarmAPY(address[] memory _workers) public view returns (bytes memory) {
    if (_workers.length > 0) {
      if (IWorker(_workers[0]).router() == spookyRouter) {
        ...
        return getSpiritYieldFarmAPY(_workers);
      }
    }
  }
  ...
  // Return Spiritswap yield farm APY.
  function getSpiritYieldFarmAPY(address[] memory _workers) public view returns (bytes memory) {
    uint256 len = _workers.length;
    uint256[] memory yieldFarmAPY = new uint256[](len);
    address[] memory workersList = new address[](len);
    for (uint256 i = 0; i < _workers.length; i++) {
      address worker = _workers[i];
      uint256 pid = getPoolIdSpirit(worker);
      address lp = getLpToken(worker);
      uint256 numerator = ((spiritPerYear(pid).mul(getSPIRITPrice()).div(1e18))).mul(uint256(100)).mul(1e18);
      uint256 denominator = (
        (IERC20(lp).balanceOf(address(spiritMasterChef)).mul(1e18).div(IERC20(lp).totalSupply())).mul(
          getLPValue(worker)
        )
      ).div(1e18);
      yieldFarmAPY[i] = denominator == 0 ? 0 : numerator.div(denominator);
