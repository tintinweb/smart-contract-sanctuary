...
contract Strategy_USDC_FTM {
    // This strategy is for FTM network. USDC - FTM pair. RewardToken = "Boo"
    ...
    address public constant lpToken = 0x2b4C76d0dc16BE1C31D4C1DC53bF9B45987Fc75c;
    ...
    function __addLiquidityETH(address _token, uint256 desiredAmountETH) internal {
        ...
        if(amount0 != 0 && amount1 !=0 && _canAddLiquidity(amount0, amount1)) {
            ...
            IWETH(WETH).transfer(lpToken, amount1);
            IPair(lpToken).mint(address(this));
        }
    }
    ...
    function _getTokenPrice1() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return 1e24 / (_getAmountsOut(100 * 1e6, path) / 100);
    }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            // calculates how many nativeToken for tokens
            if(token0 == WETH && t1V != 0) {
                path[0] = token1;
                TC = _getAmountsOut(t1V, path) + t0V;
            } else if (token1 == WETH && t0V != 0) {
                path[0] = token0;
                TC = _getAmountsOut(t0V, path) + t1V;
            } else {
                if(t0V != 0) {
                    path[0] = token0;
                    TC = _getAmountsOut(t0V, path);
                }
                
                if(t1V != 0) {
                    path[0] = token1;
                    TC += _getAmountsOut(t1V, path);
                }
            }
        }
