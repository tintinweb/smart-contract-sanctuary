...
contract TimeLock {
    ...
    constructor(address admin_, uint delay_) public {
        require(delay_ >= MINIMUM_DELAY, "constructor: Delay must exceed minimum delay.");
        require(delay_ <= MAXIMUM_DELAY, "constructor: Delay must not exceed maximum delay.");
    ...
    function queueTransaction(address target, uint value, string memory signature, bytes memory data, uint eta) public returns (bytes32) {
        ...
        require(eta >= getBlockTimestamp().add(delay), "queueTransaction: Estimated execution block must satisfy delay.");
