...
struct UserInfo {
    ...
    // entitled to a user but is pending to be distributed is: (i is the reward token. 
...
abstract contract StrategyCurveBase is BaseStrategy {
    ...
    // Steak stuff
    ISteakHouseV2 public constant farm = ISteakHouseV2(0xe0c43105235C1f18EA15fdb60Bb6d54814299938); // Creditium Farm
    uint256 public constant pid = 0;

    // keepCRV stuff
    // uint256 public keepCRV = 0; // the percentage of CRV we re-lock for boost (in basis points)
    uint256 internal constant FEE_DENOMINATOR = 10000; // this means all of our fee values are in basis points

    IERC20 public constant reward =
        IERC20(0x77128DFdD0ac859B33F44050c6fa272F34872B5E);
    ...
    function stakedBalance() public view returns (uint256) {
        return farm.getUserInfo(pid, address(this)).amount;
    }
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        // Send all of our LP tokens to deposit to the farm if we have any
        uint256 _toInvest = balanceOfWant();
        if (_toInvest > 0) {
            farm.deposit(pid, _toInvest);
        }
    }
...
// File: StrategyCurvecUSD2Pool.sol

contract StrategyCurvecUSD2Pool is StrategyCurveBase {
    /* ========== STATE VARIABLES ========== */
    // these will likely change across different wants.

    // Curve stuff
    ICurveFiFactory public constant curve = ICurveFiFactory(0x78D51EB71a62c081550EfcC0a9F9Ea94B2Ef081c);

    // we use these to deposit to our curve pool
    // want = 0x96059756980fF6ced0473898d26F0EF828a59820
    address public targetToken; // this is the token we sell into, WETH, WBTC, or fUSDT
    IERC20 public constant cusd = IERC20(0xE3a486C1903Ea794eED5d5Fa0C9473c7D7708f40);
    IERC20 public constant dai = IERC20(0x96059756980fF6ced0473898d26F0EF828a59820);
    IERC20 public constant usdc = IERC20(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
    IUniswapV2Router02 public router = IUniswapV2Router02(0xF491e7B69E4244ad4002BC14e878a34207E38c29); // this is the router we swap with, start with spookyswap
        ...
        StrategyCurveBase(_vault)
    {
        ...
        want.approve(address(farm), type(uint256).max);
        reward.approve(spooky, type(uint256).max);
        reward.approve(spirit, type(uint256).max);

        // set our strategy's name
        stratName = _name;

        // these are our approvals and path specific to this contract
        cusd.approve(address(curve), type(uint256).max);
    ...
    {
        // harvest our rewards from the farm
        farm.deposit(pid, 0);
        uint256 rewardBalance = reward.balanceOf(address(this));

        // if we claimed any rewards, then sell it
        if (rewardBalance > 0) {
            _sellToken(address(reward), rewardBalance);
        }

        uint256 usdcBalance = usdc.balanceOf(address(this));

        if (usdcBalance > 0) {
            curve.add_liquidity(address(want), [0, 0, usdcBalance], 0);
        }
        ...
        if (_debtOutstanding > 0) {
            if (stakedBal > 0) {
                ...
                farm.withdraw(pid, Math.min(stakedBal, _debtOutstanding));
            }
            uint256 _withdrawnBal = balanceOfWant();
