...
contract Strategy_SPELL_fUSDT {
    ...
    function _check0Price() internal {
        ...
        if(price0Average >= price0) {
            require(100 - (price0 * 100 / price0Average) <= percentCheckDifference, msgError);
        } else {
            require(100 - (price0Average * 100 / price0) <= percentCheckDifference, msgError);
        }
    }
    ...
    function _check1Price() internal {
        ...
        if(price1Average >= price1) {
            require(100 - (price1 * 100 / price1Average) <= percentCheckDifference, msgError);
        } else {
            require(100 - (price1Average * 100 / price1) <= percentCheckDifference, msgError);
        }
    }
    ...
    function _getToken0Price(address[] memory path) internal view returns (uint256) {
        return _getAmountsOut(100 * 1e6, path) / 100;
    }

    function _getToken1Price(address[] memory path) internal view returns (uint256) {
        return _getAmountsOut(100 * 1e18, path) / 100;
    }

    function _getPriceWFTMToFUSDT() public view returns (uint256) {
        // wFTM to fUSDT
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;
    ...
    function _getPriceWFTMToSPELL() public view returns (uint256) {
        // wFTM to SPELL through fUSDT
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        if(_amount > 0) {
            ...
            tCI = _getPriceWFTMToSPELL() * t1V / 1e18 + _getPriceWFTMToFUSDT() * t0V / 1e6;
        }
        ...
        if(_balance != 0) {
            tCI += _getPriceWFTMToSPELL() * _balance / 1e18;
        }

        // ----> calculates fUSDC to wFTM <-----
        _balance = getBalanceOfToken(token0);
        if(_balance != 0) {
            tCI += _getPriceWFTMToFUSDT() * _balance / 1e6;
        }
