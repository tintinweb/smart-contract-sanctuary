 ...
 */
contract BLOODToken is ReentrancyGuard, ERC20PresetMinterPauser("Bubonic Bastards", "BLOOD") {
    /// @notice BLOOD tokens calaimable per day for each BUBO NFT holder
    uint256 public EMISSION_PER_DAY;

    /// @notice Start timestamp from contract deployment
    uint256 public emissionStart;

    /// @notice End date for BLOOD emissions to BUBO NFT holders
    uint256 public emissionEnd;

    /// @dev A record of last claimed timestamp for BUBO NFTs
    mapping(uint256 => uint256) private _lastClaim;

    /// @dev Contract address for BUBO NFT
    address private _nftAddress;

    /**
     * @notice Construct the BLOOD token
    ...
    constructor(uint256 emissionStartTimestamp) {
        ...
        emissionEnd = emissionStartTimestamp + (1 days * 90);
        EMISSION_PER_DAY = 166.666666667e18; // ~166.67 BLOOD
    }

    // External functions

    /**
     * @notice Sets the contract address to Bubonic Bastards BUBO NFTs upon deployment
     * @param nftAddress Address of verified BUBO NFT contract
    ...
    /**
     * @notice Check last claim timestamp of accumulated BLOOD for given BUBO NFT
     * @param tokenIndex Index of BUBO NFT to check
    ...
    /**
     * @notice Check accumulated BLOOD tokens for a BUBO NFT
     * @param tokenIndex Index of BUBO NFT to check balance
     * @return Total BLOOD accumulated and ready to claim
     */
    ...
    /**
     * @notice Check total accumulated BLOOD tokens for all BUBO NFTs
    ...
    /**
     * @notice Mint and claim available BLOOD for each BUBO NFT
     * @param tokenIndices Indexes of NFTs to claim for
     * @return Total BLOOD claimed
     */
    function claim(uint256[] memory tokenIndices) public nonReentrant returns (uint256) {
        require(block.timestamp > emissionStart, "Emission has not started yet");
        ...
        require(totalClaimQty != 0, "No accumulated BLOOD");
        _mint(_msgSender(), totalClaimQty);
        return totalClaimQty;
    }

    function resetEmissions(uint256 start, uint256 end, uint256 daily) private onlyRole(DEFAULT_ADMIN_ROLE) {
            emissionStart = start;
            emissionEnd = end;
            EMISSION_PER_DAY = daily;
    }
}