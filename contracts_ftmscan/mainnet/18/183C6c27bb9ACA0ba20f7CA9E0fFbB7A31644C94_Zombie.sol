...
===============================================
    ZOMBIES GAME
    https://zombies.game
    https://twitter.com/zombiesgamenft
    https://discord.com/invite/akKDsndCBr
===============================================
*/
...
contract Brain is ERC20("Brain", "BRAIN"), Ownable {
    address graveyardAddress;
    address zombieAddress;
    address brainJarAddress;

    constructor(address _brainJarAddress) {
        brainJarAddress = _brainJarAddress;
    }

    function setGraveyardAddress(address _graveyardAddress) external onlyOwner {
        require(address(graveyardAddress) == address(0), "Graveyard address already set");
        graveyardAddress = _graveyardAddress;
    }

    function setZombieAddress(address _zombieAddress) external onlyOwner {
        require(address(zombieAddress) == address(0), "Zombie address already set");
        zombieAddress = _zombieAddress;
    }

    function mint(address _to, uint256 _amount) external {
        require(_msgSender() == graveyardAddress, "Only the Graveyard contract can mint");
        _mint(_to, _amount);
    }

    function burn(address _from, uint256 _amount) external {
        require(_msgSender() == zombieAddress, "Only the Zombie contract can burn");
        _burn(_from, _amount);
    }

    function transferToBrainJar(address _from, uint256 _amount) external {
        require(_msgSender() == brainJarAddress, "Only the Brain Jar contract can call transferToBrainJar");
        _transfer(_from, brainJarAddress, _amount);
    }
}
...
contract Zombie is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant BASE_MINT_PRICE = 45 ether; // 45 FTM
    uint256 public constant PRESALE_MINT_PRICE = 35 ether; // 35 AVAX
    uint256 public constant BASE_ALPHA_PERCENTAGE = 5;
    uint256 public constant UPGRADE_SALES_OFFSET = 1 days;
    ...
    uint256 public lastBaseMintTimestamp;

    mapping(uint256 => uint256) private tokenLevel;
    mapping(uint256 => uint256) public baseTokenMintBlock;
    Level[] public levels;

    string public constant BASE_URI = "https://zombies.game/tokens/";

    constructor() ERC721("Zombie", "ZOMBIE") {
        // supply and price are ignored for the base levels of zombie and robot
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 1 }));
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 25 }));
    }

    /* Minting of base zombies */
    ...
    function _mintBaseTokens(uint16 _numTokens, address _for) internal {
        ...
        for (uint i = 0; i < _numTokens; i++) {
            ...
            if (_rand(totalSupply()) % 100 < BASE_ALPHA_PERCENTAGE) {
                tokenLevel[tokenId] = 1; // alpha
            } else {
                tokenLevel[tokenId] = 0; // grunt
            }
        }

        lastBaseMintTimestamp = block.timestamp;
    }
    ...
    function mintUpgrade(uint256 _level, uint16 _numTokens) external {
        ...
        require(brain.balanceOf(msg.sender) >= totalCost, "Insufficient brain balance");
        brain.burn(msg.sender, totalCost);
    ...
    function gameStartTimestamp() public view returns (uint256) {
        if (!gameStarted()) {
            return 0;
        } else if (baseSupply == MAX_BASE_SUPPLY) {
            return lastBaseMintTimestamp;
        } else {
            return salesStartTime + UPGRADE_SALES_OFFSET;
        }
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (isUpgrade(_tokenId)) {
            ...
            return string(abi.encodePacked(BASE_URI, "base/", _tokenId.toString(), "-alpha.json"));
        } else {
            return string(abi.encodePacked(BASE_URI, "base/", _tokenId.toString(), "-grunt.json"));
        }
    }
    ...
    function batchedZombiesOfOwner(
        address _owner,
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (ZombieInfo[] memory) {
        if (_offset >= balanceOf(_owner)) {
            return new ZombieInfo[](0);
        }
        ...
        ZombieInfo[] memory zombies = new ZombieInfo[](outputSize);
    ...
    function setGraveyardAddress(address _graveyardAddress) external onlyOwner {
        require(graveyardAddress == address(0), "Graveyard address already set");
        graveyardAddress = _graveyardAddress;
    }

    function setBrain(Brain _brain) external onlyOwner {
        brain = _brain;
    }
