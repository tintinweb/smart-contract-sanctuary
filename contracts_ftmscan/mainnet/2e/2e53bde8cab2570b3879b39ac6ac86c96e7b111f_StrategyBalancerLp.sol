...
contract StrategyBalancerLp is StrategyBase {
    ...
    uint256 public tokenCompoundIndex; //index to add lp
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        (uint256 _stakedAmount, ) = IBeethovenxMasterChef(farmPool).userInfo(poolId, address(this));
    ...
    function _swapTokens(
        ...
        if (path.length > 0) {
            ...
            uint8[] memory dexIds = new uint8[](path.length);
            firebirdRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(_input, _output, _amount, 1, path, dexIds, _receiver, block.timestamp);
        } else if (balancerPoolId.length > 0) {
            //use balancer
            IBalancerV2Vault.SingleSwap memory singleSwap = IBalancerV2Vault.SingleSwap(balancerPoolId, IBalancerV2Vault.SwapKind.GIVEN_IN, _input, _output, _amount, new bytes(0));
            IBalancerV2Vault.FundManagement memory funds = IBalancerV2Vault.FundManagement(address(this), false, _receiver, false);

            IBalancerV2Vault(balancerV2Vault).swap(singleSwap, funds, 1, block.timestamp);
        } else {
            // use Uniswap
            path = uniswapPaths[_input][_output];
            if (path.length == 0) {
    ...
    function _addLiquidity() internal {
        uint256 depositTokenBalance = IERC20(targetCompoundToken).balanceOf(address(this));
        ...
        IBalancerV2Vault(balancerV2Vault).joinPool(poolID_bytes, address(this), address(this), request);
    }

    function balanceOfPool() public view override returns (uint256) {
        (uint256 amount, ) = IBeethovenxMasterChef(farmPool).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function setApproveBalancerV2VaultForToken(address _token, uint256 _amount) public onlyStrategist {
    ...
    function setCompoundConfig(uint256 _totalUnderlyingTokens, uint256 _tokenCompoundIndex) external onlyStrategist {
