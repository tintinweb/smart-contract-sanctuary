...
// WildMaker is MasterChef's left hand and kinda a wizard. He can make WILD from pretty much anything!
// This contract handles "serving up" rewards for xwild holders by trading tokens collected from fees for WILD.

// T1 - T4: OK
contract WildMaker is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IUniswapV2Factory public immutable factory;

    address public immutable xwild;
    address private immutable wild;
    ...
    modifier onlyAuth() {
        require(isAuth[msg.sender] || anyAuth, "WildMaker: FORBIDDEN");
        _;
    }
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != wild && token != ftm && token != bridge,
            "WildMaker: Invalid bridge"
        );
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "WildMaker: must use EOA");
        _;
    }

    // F1 - F10: OK
    // F3: _convert is separate to save gas by only checking the 'onlyEOA' modifier once in case of convertMultiple
    // F6: There is an exploit to add lots of wild to the xwild, run convert, then remove the wild again.
    //     As the size of the wildxwild has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    function _convert(address token0, address token1) internal {
        ...
        if(token0 == token1) {
            ...
            require(address(pair) != address(0), "WildMaker: Invalid pair");
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    // All safeTransfer, _swap, _towild, _convertStep: X1 - X5: OK
    function _convertStep(
        address token0,
        address token1,
        uint256 amount0,
        uint256 amount1
    ) internal returns (uint256 wildOut) {
        // Interactions
        if (token0 == token1) {
            uint256 amount = amount0.add(amount1);
            if (token0 == wild) {
                IERC20(wild).safeTransfer(xwild, amount);
                wildOut = amount;
            } else if (token0 == ftm) {
                wildOut = _towild(ftm, amount);
            } else {
                address bridge = bridgeFor(token0);
                amount = _swap(token0, bridge, amount, address(this));
                wildOut = _convertStep(bridge, bridge, amount, 0);
            }
        } else if (token0 == wild) {
            // eg. wild - ETH
            IERC20(wild).safeTransfer(xwild, amount0);
            wildOut = _towild(token1, amount1).add(amount0);
        } else if (token1 == wild) {
            // eg. USDT - wild
            IERC20(wild).safeTransfer(xwild, amount1);
            wildOut = _towild(token0, amount0).add(amount1);
        } else if (token0 == ftm) {
            // eg. ETH - USDC
            wildOut = _towild(
                ftm,
                _swap(token1, ftm, amount1, address(this)).add(amount0)
            );
        } else if (token1 == ftm) {
            // eg. USDT - ETH
            wildOut = _towild(
                ftm,
                _swap(token0, ftm, amount0, address(this)).add(amount1)
            );
        } else {
            // eg. MIC - USDT
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "WildMaker: Cannot convert");
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    function _towild(address token, uint256 amountIn)
        internal
    ...
    {   
        ...
        amountOut = _swap(token, wild, amount, xwild);
    }

    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'WildMaker: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'WildMaker: INSUFFICIENT_LIQUIDITY');
