...
// Note that this pool has no minter key of BULLET (rewards).
// Instead, the governance will call BULLET distributeReward method and send reward to this pool at the beginning.
contract BulletRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BULLETs to distribute in the pool.
        uint256 lastRewardTime; // Last time that BULLETs distribution occurred.
        uint256 accTombPerShare; // Accumulated BULLETs per share, times 1e18. See below.
    ...
    // The time when BULLET mining starts.
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "BulletRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "BulletRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's BULLET allocation point. Can only be called by the owner.
    ...
    // View function to see pending BULLETs on frontend.
    ...
    // Safe tomb transfer function, just in case if rounding error causes pool to not have enough BULLETs.
