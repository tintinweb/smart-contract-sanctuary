...
interface ICollateralReserveV2OCalc{
...
contract CollateralReserveV2 is Ownable{
    ...
    address[] public depositors; // Push only, beware false-positives. Only for viewing.
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint CoUSD needed and store amount of rewards for distribution
        send_ = value.sub( _profit );

        IDollar( cousd ).pool_mint( msg.sender, send_ );
    ...
    function addDepositor(address addr) external onlyOwner{
        ...
        if( !listContains( depositors, addr ) ) {
            depositors.push( addr );
        }
    }

    function delDepositor(address addr) external onlyOwner{
        delete isDepositor[addr];
        for (uint i = 0; i < depositors.length; i++){ 
            if (depositors[i] == addr) {
    ...
    // add a collatereal 
    function addCollateral(address _token, ICollateralReserveV2OCalc _token_oracle) public onlyOwner{
    ...
    function buyBackCoffin(uint _amount, uint _min) public onlyBuybackManagerOrOwner {
        ...
        _rebalance(wftm, coffin, _amount, _min, true);
    }
    // buyback cousd
    ...
    function stakeBOO(uint _amount) public onlyBuybackManagerOrOwner {
    ...
    function unstakeBOO(uint _amount) public onlyBuybackManagerOrOwner {
        require(boomirrorworld!=address(0), "boomirrorworld address error");
        require(_amount!=0, "amount error");
    ...
    function burnXCoffinByCoffin(uint _coffin_ammount) public onlyBuybackManagerOrOwner {
        require(coffin!=address(0), "coffin address error");
        require(coffintheotherworld!=address(0), "coffintheotherworld address error");
    ...
    function rebalance(
        ...
        _rebalance(token0, token1, _amount, _min_output_amount, false);
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        value_ = ICollateralReserveV2OCalc( collateralCalc[ _token ] ).valuation(  _amount );
    }
    /**
    ...
    function getCollateralPrice(address _token) public view  returns (uint256) {
        require(address(collateralCalc[_token])!=address(0), "err0");
        ( uint256 price, uint8 d ) = ICollateralReserveV2OCalc(collateralCalc[_token]).getPrice();
    ...
    function getCollateralBalance(address _token) public view  returns (uint256) {
        ...
        uint256 missing_decimals = 18 -ERC20(_token).decimals();
