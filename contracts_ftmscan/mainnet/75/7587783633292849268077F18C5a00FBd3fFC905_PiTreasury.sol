...
contract PiTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SPID }
    ...
    address public sPID;
    uint public sPIDQueue; // Delays change to sPID address
    ...
    /**
        @notice allow approved address to deposit an asset for PID
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PID needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( PID ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PID for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPIDERC20( PID ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sPID ).balanceOf( msg.sender ); // Can only borrow against sPID held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PID
        @param _amount uint
     */
    function repayDebtWithPID( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPIDERC20( PID ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, PID, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( PID ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( PID ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PID valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PID decimals
            value_ = _amount.mul( 10 ** IERC20( PID ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPID ) { // 9
            sPIDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
