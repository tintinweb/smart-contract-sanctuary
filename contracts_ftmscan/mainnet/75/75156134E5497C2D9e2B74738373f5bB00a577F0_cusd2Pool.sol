...
interface IMasterChef {
    ...
    function userInfo(uint256 _pid, address _user) external view returns (uint256, uint256[] memory, uint256[] memory);
    function emergencyWithdraw(uint256 _pid) external;
}

interface ICurveSwap {
    function exchange_underlying(int128 inToken, int128 outToken, uint256 amountIn, uint256 minAmountOut) external;
}

interface ICurveLiq {
    function add_liquidity(address pool, uint256[] memory amounts, uint256 minOut) external;
}

pragma solidity ^0.6.0;

contract cusd2Pool is Ownable, Pausable {
    ...
    address public cusd = address(0xE3a486C1903Ea794eED5d5Fa0C9473c7D7708f40);
    ...
    address[] public wrappedToDaiRoute;
    address public curve = address(0x96059756980fF6ced0473898d26F0EF828a59820);
    address public curveLiq = address(0x78D51EB71a62c081550EfcC0a9F9Ea94B2Ef081c);
    ...
    function addLiquidity() internal {
        ...
        approveTxnIfNeeded(wrapped, wrappedToLp0Router, wrappedBal);
        
        IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedBal, 0, wrappedToDaiRoute, address(this), now);
        
        uint256 daiBal = IERC20(dai).balanceOf(address(this));
        approveTxnIfNeeded(lpToken0, curve, daiBal);
        ICurveSwap(curve).exchange_underlying(1, 0, daiBal, 1);

        uint256 cusdBal = IERC20(cusd).balanceOf(address(this));
        amounts = [cusdBal,0,0];   

        approveTxnIfNeeded(cusd, curveLiq, cusdBal);
        ICurveLiq(curveLiq).add_liquidity(curve, amounts, 1);
    }
