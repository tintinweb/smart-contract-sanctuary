...
contract PrisonDungeon is Ownable {
    ...
    mapping(address => mapping(uint => uint[])) user_last_adventure;
    ...
    function adventure(uint character_tokenid, uint bitdna_tokenid) public returns(bool) {
        ...
        user_last_adventure[msg.sender][character_tokenid] = new uint[](prizes.length);
        for(uint i = 0; i < prizes.length; i++){
            ...
            if((_random(10000, i) + 1) <= probability){
                ...
                user_last_adventure[msg.sender][character_tokenid][i] = token_num;
    ...
    function query_core_attrs(address addr, uint bitdna_tokenid) public view returns(uint[] memory, uint[] memory){
    ...
    function query_must_tokens(address addr) public view returns(uint[] memory, uint[] memory){
        ...
        for(uint i = 0; i < mustTokens.length; i++){
            ...
            user_must_tokens[i] = token_balance / (10 ** 18);
        }
        return (min_must_tokens, user_must_tokens);
    }

    function query_last_adventure(address addr, uint character_tokenid) public view returns(uint[] memory){
        return (user_last_adventure[addr][character_tokenid]);
    }
