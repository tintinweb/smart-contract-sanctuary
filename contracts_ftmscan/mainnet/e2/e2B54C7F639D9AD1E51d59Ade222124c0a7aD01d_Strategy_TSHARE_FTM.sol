...
contract Strategy_TSHARE_FTM is Ownable {
    // This strategy is for FTM network. TSHARE - FTM pair. RewardToken = "tshare"
    ...
    address public constant lpToken = 0x4733bc45eF91cF7CcEcaeeDb794727075fB209F2;
    address public constant farm = 0xcc0a87F7e7c693042a9Cc703661F5060c80ACb43;
    // ------------>        END          <--------------
    ...
    function autoCompound() external {
        ...
        if (amount > 0) {
            ...
            if(_balance > 0){
                path[0] = rewardToken;
                path[1] = WETH; // WFTM 
                amount = _getAmountsOut(router, _balance+pendingRewardsFee, path);
                if(amount > 100) {
    ...
    function _swapWETHToToken(address _router, address _token) internal {
        ...
        uint256 amount = _getAmountsOut(router, _balanceWETH, path);
        if(amount > 100) {
    ...
    function _getTokenPrice0() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
