 ...
 */
contract ReaperAutoCompoundxDAO_XAssetVA is Ownable, Pausable {
     ...
     * {token} - LP Token that the strategy maximizes.
    ...
    address public rewardToken = address(0xc165d941481e68696f43EE6E99BFB2B23E0E3114);
    ...
    address public masterChef = address(0xa7821C3e9fC1bF961e280510c471031120716c3d);
    uint8 public poolId;
    ...
    address[] public rewardTokenToTokenRoute;
    ...
    constructor (
      ...
      address _treasury
        ...
        if (token == wftm) {
            rewardTokenToTokenRoute = [rewardToken, wftm];
        } else if (token != rewardToken) {
            rewardTokenToTokenRoute = [rewardToken, wftm, token];
        }
     ...
     * It deposits {token} in the masterChef to farm {rewardToken}
     */
    function deposit() public whenNotPaused {
        uint256 pairBal = IERC20(xToken).balanceOf(address(this));
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {
            ...
            pairBal = IERC20(xToken).balanceOf(address(this));
        }
        ...
        IERC20(xToken).safeTransfer(vault, pairBal.sub(withdrawFee));
    }
    ...
    function addLiquidity() internal {
        ...
        IUniswapRouterETH(uniRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(rewardTokenBal, 0, rewardTokenToTokenRoute, address(this), now.add(600));

        //enter into xToken here
        uint256 tokenBal = IERC20(token).balanceOf(address(this));
        IXtoken(xToken).enter(tokenBal);
    ...
    function retireStrat() external {
        ...
        uint256 pairBal = IERC20(xToken).balanceOf(address(this));
        IERC20(xToken).transfer(vault, pairBal);
    }
    ...
    function giveAllowances() internal {
        IERC20(xToken).safeApprove(masterChef, uint256(-1));
        IERC20(token).safeApprove(xToken, uint256(-1));
        IERC20(rewardToken).safeApprove(uniRouter, uint256(-1));
    }

    function removeAllowances() internal {
        IERC20(xToken).safeApprove(masterChef, 0);
        IERC20(token).safeApprove(xToken, 0);
    ...
    function updateRoutes(address[] calldata _route) external onlyOwner returns (bool){
        rewardTokenToTokenRoute = _route;
    }
}