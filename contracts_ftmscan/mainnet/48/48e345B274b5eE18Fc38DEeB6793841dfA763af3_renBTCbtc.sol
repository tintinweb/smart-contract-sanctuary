...
interface ICurveBTCRouter {
    function exchange(int128 inToken, int128 outToken, uint256 amountIn, uint256 minAmountOut) external;
}

pragma solidity ^0.6.0;

contract renBTCbtc is Ownable, Pausable {
    ...
    address public curve = address(0x3eF6A01A0f81D6046290f3e2A8c5b843e738E604);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != wrapped) {
            IUniSwapRouter(wrappedToLp0Router).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedBal, 0, wrappedToLp0Route, address(this), now);
        }
        
        uint256 btcSwap = IERC20(lpToken0).balanceOf(address(this)).div(2);
        approveTxnIfNeeded(lpToken0, curve, btcSwap);
        ICurveBTCRouter(curve).exchange(0, 1, btcSwap, 1);
