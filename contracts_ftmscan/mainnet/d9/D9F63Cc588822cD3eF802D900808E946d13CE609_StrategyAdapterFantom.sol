...
contract StrategyAdapterFantom is AlphaStrategyFantom {
    ...
    function initialize(
        ...
        AlphaStrategyFantom.initializeAlphaStrategy(
            ...
            _xBooStakingPoolId, // 9
            _xBooRewardsToken // 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83 wFTM
        );
    }
}
...
contract AlphaStrategyFantom is OwnableUpgradeable {
    ...
    address public xBooRewardsToken;
    ...
    function initializeAlphaStrategy(
        ...
        xBooRewardsToken = _xBooRewardsToken;
    ...
    */
    ...
    function pendingXBoo() public view returns (uint256) {
        return IERC20(xBoo).balanceOf(address(this));
    }
    ...
    function withdrawReward(address user) public onlyVault {
        ...
        uint256 _pendingXBoo = IERC20(vault).balanceOf(user)
            .mul(accXBooPerShare)
            .div(1e36)
            .sub(userXBooDebt[user]);
    ...
    */
    ...
    function xBooMasterChefBalance() internal view returns (uint256 bal) {
        (bal,) = IMasterChef(xBooStakingMasterchef).userInfo(poolId, address(this));
    }

    function claimRewardsForXBoo() internal {
    ...
    function moveRewardsToTreasury() internal {
        claimRewardsForXBoo();

        uint256 balance = IERC20(xBooRewardsToken).balanceOf(address(this));

        if(balance > 0){
            IERC20(xBooRewardsToken).safeTransfer(treasury, balance);
        }
    }

    function setXBooStakingPoolId(address _xBooStakingMasterchef) public onlyMultisigOrOwner {
        moveRewardsToTreasury();

        xBooStakingMasterchef = _xBooStakingMasterchef;
    }

    function setXBooStakingPoolId(uint256 poolId, address rewardsToken) public onlyMultisig {
        require(poolId >= 0, "PoolId should be bigger than zero");
        require(address(rewardsToken) != address(0), "RewardToken address should be bigger than zero");

        moveRewardsToTreasury();

        xBooStakingPoolId = poolId;
        xBooRewardsToken = rewardsToken;
