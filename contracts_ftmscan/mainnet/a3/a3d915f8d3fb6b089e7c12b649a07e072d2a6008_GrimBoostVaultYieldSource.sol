    ...
    OwnableUpgradeable
{
    ...
    /// @dev (see _tokenToGShares & _gSharesToToken)
    /// @return Price per vault's share
    function _pricePerGShare() internal view returns (uint256) {
    ...
    function _totalAssetsInToken() public view returns (uint256) {
        return _balanceOfToken().add(_gSharesToToken(_balanceOfGShares()));
    }
    ...
    function _tokenToGShares(uint256 tokens) public view returns (uint256) {
        return tokens.mul(10**_vaultDecimals()).div(_pricePerGShare());
    }
    ...
    function _gSharesToToken(uint256 gShares) internal view returns (uint256) {
        return gShares.mul(_pricePerGShare()).div(10**_vaultDecimals());
    }
