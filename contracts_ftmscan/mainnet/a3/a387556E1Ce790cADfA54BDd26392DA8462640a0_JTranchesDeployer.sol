			...
			for (uint256 i = 1; i<= stkDetNum; i++){
				...
				if (tempAmount > 0) {
					if (tempAmount <= tempValue) {
						...
						IJYearn(jYearnAddress).setTrBStakingDetails(protTrancheNum, to, i, tempAmount, block.timestamp);
						tempValue = tempValue.sub(tempAmount);
					} else {
						uint256 remainingAmount = tempAmount.sub(tempValue);
						IJYearn(jYearnAddress).setTrBStakingDetails(protTrancheNum, from, i, remainingAmount, tempTime);
						IJYearn(jYearnAddress).setTrBStakingDetails(protTrancheNum, to, i, tempValue, block.timestamp);
						tempValue = 0;
					}
				}
				if (tempValue == 0)
