...
interface IMarsNFT {
    function mint(address to, uint256 nftId) external returns (uint256);
}
...
contract NFTClaim is Ownable, ReentrancyGuard {
    ...
    constructor(address marsNFT_) {
        marsNFT = IMarsNFT(marsNFT_);
    }
    ...
    function claimNFT() public nonReentrant {
        ...
        marsNFT.mint(sender, nftId);
    }

    function setMarsNFT(address marsNFT_) external onlyOwner {
        marsNFT = IMarsNFT(marsNFT_);
    }

    function setClaimNFTId(uint256 nftId_) external onlyOwner {
        nftId = nftId_;
    }
