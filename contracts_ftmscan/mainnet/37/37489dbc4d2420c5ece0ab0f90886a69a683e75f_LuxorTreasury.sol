...
contract LuxorTreasury is Ownable {
    ...
    address public Lumens;
    uint public LUMQueue; // Delays change to LUM address
    ...
    //    isLiquidityToken[ _LUXDAI ] = true;
    //    liquidityTokens.push( _LUXDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for LUX
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint LUX needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Luxor ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn LUX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ILUXERC20( Luxor ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( Lumens ).balanceOf( msg.sender ); // Can only borrow against LUM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with LUX
        @param _amount uint
     */
    function repayDebtWithLUX( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ILUXERC20( Luxor ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Luxor, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Luxor ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Luxor ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns LUX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match LUX decimals
            value_ = _amount.mul( 10 ** IERC20( Luxor ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.LUM ) { // 9
            LUMQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
