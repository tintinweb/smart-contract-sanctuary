...
// File: tests/MyVaultV4.sol


pragma solidity >=0.8.0;


contract VaultNFTv4 is ERC721Enumerable {
...
contract MapleVault is ReentrancyGuard, VaultNFTv4 {
    ...
    IERC20Metadata private collateralInfo;

    IMapleDollar public mai;
    IERC20Metadata private maiInfo;
    ...
    constructor(
        ...
        assert(minimumCollateralPercentage != 0);
                        //  | decimals start here
        debtCeiling=10000000000000000000;// 10 dollars
        closingFee=50; // 0.5%
        openingFee=0; // 0.0%
        ...
        collateralInfo = IERC20Metadata(_collateral);

        mai = IMapleDollar(_mai);
        maiInfo = IERC20Metadata(_mai);
    ...
    function calculateCollateralProperties(uint256 _collateral, uint256 _debt) private view returns (uint256, uint256) {
        ...
        uint256 collateralValue = _collateral.mul(getEthPriceSource()).mul(10**(uint256(maiInfo.decimals()).sub(uint256(collateralInfo.decimals()))));
    ...
    function borrowToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) {
        ...
        require(mai.totalSupply().add(amount) <= debtCeiling, "borrowToken: Cannot mint over available supply.");
        ...
        totalBorrowed = totalBorrowed.add(newDebt);
...
contract MapleBank is MapleVault, Ownable {
    ...
    function setDebtCeiling(uint256 amount) external onlyOwner() {
        require(mai.totalSupply()<=amount, "setCeiling: Must be over the amount of outstanding debt.");
        debtCeiling = amount;
    }
