...
contract HauntedHouse is Ownable {
    ...
    function contribute(uint256 amountOfTokens) external {
        ...
        require(presaleActive || publicActive, "CONTRIBUTE: SALE NOT ACTIVE");
        require(amountOfTokens >= minAlloc, "CONTRIBUTE: MIN ALLOCATION");
        require(
            amountOfBase.add(contributed[msg.sender]) <= maxAlloc,
            "CONTRIBUTE: MAX ALLOCATION"
        );
        require(
            amountOfBase.add(totalSold) <= maxCap,
            "CONTRIBUTE: MAX CAP OVERFLOW"
        );
        require(
            IERC20(BASE).balanceOf(msg.sender) >= amountOfBase,
            "CONTRIBUTE: BASE BALANCE TOO LOW"
        );
    ...
    function claim() external {
        require(claimActive, "CLAIM: SALE NOT ACTIVE");
    ...
    function refund() external {
        require(refundActive, "REFUND: SALE NOT ACTIVE");
