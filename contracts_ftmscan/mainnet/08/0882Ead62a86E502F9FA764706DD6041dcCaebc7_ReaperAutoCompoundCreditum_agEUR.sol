 ...
 */
contract ReaperAutoCompoundCreditum_agEUR is ReaperBaseStrategy {
    ...
    address public constant cUSD =
        address(0xE3a486C1903Ea794eED5d5Fa0C9473c7D7708f40);
    address public constant agEUR = 
    ...
    address public constant curvePool = 
        address(0x96059756980fF6ced0473898d26F0EF828a59820);
...
/**
    ...
    address[] public cUSDToagEURRoute = [cUSD, agEUR];
    ...
    {
        uint256[] memory pending = IMasterChef(masterChef).pendingRewards(poolId, address(this));
        for (uint8 i = 0; i < 5; i++) {
            ...
            // for all other reward Tokens, add wFTM equivalent to profit
            address token = rewardTokens[i];

            if (token == wftm) {
                profit = profit.add(pending[i]);
            } else {
                uint256[] memory amountOutMins = IUniswapV2Router02(uniRouter)
                    .getAmountsOut(pending[i], rewardTokenToWftmRoute[token]);
    ...
    function addLiquidity() internal {
        ...
        //trade usdc for cUSD on curve
        uint minAmountOut = USDCBal.mul(1e10).mul(95); //5% slippage max, dont trade if peg is below 5%
        ICurveFactory(curvePool).exchange_underlying(2,0,USDCBal, minAmountOut);
        //trade 50% cUSD for agEUR on spooky
    ...
    function getcUSDPriceinUSDC() public view returns(uint priceUSDC){
        priceUSDC = ICurveFactory(curvePool).get_dy_underlying(0, 2, 1e18);

    }
    function getagEURPriceinUSDC() public view returns(uint priceUSDC){
        uint lpBalEUR = IERC20(agEUR).balanceOf(lpPair);
        uint lpBalcUSD = IERC20(cUSD).balanceOf(lpPair);
        priceUSDC = lpBalEUR.mul(getcUSDPriceinUSDC()).div(lpBalcUSD);
    }
}
