...
Special thanks to: @BoringCrypto for his great libraries @ https://github.com/boringcrypto/BoringSolidity
https://docs.spartacadabra.money/
...
contract CharmToken is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "Charm Token";
    ...
    address payable public _charmAddress = payable(0x72b36b7108B4FB74FaeBEEa0CC7d24d2eA7fAE0e); //charm Team wallet
    address payable public _mktgAddress = payable(0xAB694510834aC0071D7F6eEC959aAA6e906bA873); //token marketing/development wallet
    ...
    event UpdatedCharmAddress(address charm); //investment wallet
    ...
    function sendETHToFee(uint256 amount) private {
        _charmAddress.transfer(amount.div(2));
        _mktgAddress.transfer(amount.div(2));
    }

    function manualSwap() external {
        require(_msgSender() == _charmAddress);
        uint256 contractBalance = balanceOf(address(this));
        swapTokensForEth(contractBalance);
    }

    function manualSend() external {
        require(_msgSender() == _charmAddress || _msgSender() == _mktgAddress);
        uint256 contractETHBalance = address(this).balance;
    ...
    function setMaxWalletSize(uint256 maxWalletSize) public onlyOwner {
     ...
     //set wallet address
    function _setCHARMAddress(address charmAddress) external onlyOwner {
        require(_charmAddress != address(0), "_charmAddress cannot be 0");
        _isExcludedFromFee[charmAddress] = false;
        charmAddress = payable(_charmAddress);
        _isExcludedFromFee[charmAddress] = true;
        emit UpdatedCharmAddress(_charmAddress);
    }
