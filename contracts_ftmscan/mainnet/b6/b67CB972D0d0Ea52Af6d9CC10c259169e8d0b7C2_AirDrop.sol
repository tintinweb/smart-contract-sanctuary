...
contract AirDrop is IMerkleDistributor {
    ...
    function extension(uint256 endOn_) isOwner public override{
        emit EndSet(endOn,endOn_);
        endOn = endOn_;
    }
    
    function end() public outTime isOwner {
    ...
    function changeWallet(address payable newWallet) external isOwner {
        emit WalletSet(wallet, newWallet);
        wallet = newWallet;
    }
    function changeOwner(address newOwner) isOwner  external override {
        emit OwnerSet(owner, newOwner);
        owner = newOwner;
    }
    
    function setRoot(bytes32 merkleRoot_) isOwner  external override {
