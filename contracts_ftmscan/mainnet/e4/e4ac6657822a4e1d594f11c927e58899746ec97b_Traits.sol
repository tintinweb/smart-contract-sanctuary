...
contract Traits is ITraits, Ownable {
    ...
    */
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IMarsWasteland.RobotSlime memory s = marsWasteland.getTokenTraits(tokenId);
    ...
    */
    function compileAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        traits = string(abi.encodePacked(
            attributeForTypeAndValue(_traitTypes[0], traitData[0 + shift][s.head].name),',',
            attributeForTypeAndValue(_traitTypes[1], traitData[1 + shift][s.eyes].name),',',
            attributeForTypeAndValue(_traitTypes[2], traitData[2 + shift][s.mouth].name),',',
            attributeForTypeAndValue(_traitTypes[3], traitData[3 + shift][s.body].name),',',
            attributeForTypeAndValue(_traitTypes[4], traitData[4 + shift][s.equipment].name),',',
            attributeForTypeAndValue("Alpha Score", s.isRobot 
                                                    ? _alphas_robot[s.alphaIndex]
                                                    : _alphas_slime[s.alphaIndex]),','
        ));
        return string(abi.encodePacked(
            '[',
            traits,
            '{"trait_type":"Generation","value":',
            tokenId <= marsWasteland.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
