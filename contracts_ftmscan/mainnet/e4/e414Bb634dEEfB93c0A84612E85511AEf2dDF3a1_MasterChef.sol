...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accSandManPerShare;   // Accumulated SANDMANs per share, times 1e24. See below.
    ...
    uint256 public totalSupplyMC = 0;
    ...
    function pendingSandMan(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accSandManPerShare = accSandManPerShare + ((sandManReward * 1e24) / pool.lpSupply);
        }

        return ((user.amount * accSandManPerShare) /  1e24) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSandManPerShare = pool.accSandManPerShare + ((sandManReward * 1e24) / pool.lpSupply);
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accSandManPerShare) / 1e24) - user.rewardDebt;
            if (pending > 0) {
                safeSandManTransfer(msg.sender, pending);
                payPendingUSDCReward(msg.sender, _pid);
            }
        ...
        if (_amount > 0) {
            ...
            totalSupplyMC = totalSupplyMC + _amount;
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if (usdcReceived > 0 && totalSupplyMC != 0){
                    accDepositUSDRewardPerShare = accDepositUSDRewardPerShare + ((usdcReceived * 1e24) / totalSupplyMC); // div en el total supply del mc
        ...
        user.rewardDebt = (user.amount * pool.accSandManPerShare) / 1e24;
    ...
    function pendingUSDC(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return ((user.amount * accDepositUSDRewardPerShare) / (1e24)) - user.usdRewardDebt;
    }

    function payPendingUSDCReward(address userAddress, uint256 _pid) public {

        UserInfo storage user = userInfo[_pid][userAddress];

        uint256 usdcPending = ((user.amount * accDepositUSDRewardPerShare) / 1e24) - user.usdRewardDebt;
