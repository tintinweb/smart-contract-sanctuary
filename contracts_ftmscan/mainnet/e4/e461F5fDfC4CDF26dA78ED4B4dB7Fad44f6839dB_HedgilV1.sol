...
contract HedgilV1 is IHedgilV1, HedgilPool {
    ...
    function name() public view override returns (string memory) {
        return string(abi.encodePacked("hedgil-", IERC20Extended(LPTOKEN.token0()).symbol(), "-", IERC20Extended(LPTOKEN.token1()).symbol()));
    }

    function symbol() public view override returns (string memory) {
        return string(abi.encodePacked("h-", IERC20Extended(LPTOKEN.token0()).symbol(), "-", IERC20Extended(LPTOKEN.token1()).symbol()));
    }
