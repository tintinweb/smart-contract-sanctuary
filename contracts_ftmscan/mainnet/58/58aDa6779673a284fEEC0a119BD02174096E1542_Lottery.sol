...
contract Lottery is LotteryOwnable, Initializable {
    ...
    // State variables 
    // Instance of Lqdr token (collateral currency for lotto)
    IERC20 internal lqdr_;
    // Storing of the NFT
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInLqdr;    // The amount of lqdr for prize money
        uint256 costPerTicket;      // Cost per ticket in $lqdr
    ...
    event RequestNumbers(uint256 lotteryId);
    ...
    function initialize(
        ...
        ticketIndicator_ = 1;
        burnPool_ = 0x05B7109b2dAe299D882c410c1a281fC194658E89;
    ...
    {
        prize = lqdr_.balanceOf(address(this)).sub(totalClaimable);
    }
        ...
        onlyOwner() 
    {
        ...
        emit RequestNumbers(_lotteryId);

        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, nft_.getTotalSupply())));
        numbersDrawn(_lotteryId, randomNumber);
    }
        ...
        internal
    {
        ...
        allLotteries_[_lotteryId].prizePoolInLqdr = getPrizeForCurrentLottery();
        ...
        onlyOwner() 
    {
        ...
        if(lastTicketId == allLotteries_[_lotteryId].endTicketID.add(1)) {
            ...
            uint256 claimablePrize = allLotteries_[_lotteryId].prizePoolInLqdr.mul(winPercent).div(100);
        ...
        returns(uint256 lotteryId)
    {
        ...
        if(lotteryId > 1) {
            uint256 burnAmount = allLotteries_[lotteryId-1].prizePoolInLqdr.mul(allLotteries_[lotteryId-1].prizeDistribution[0]).div(100);
            lqdr_.safeTransfer(burnPool_, burnAmount);
        }
        ...
        notContract()
    {
        ...
        // Transfers the required lqdr to this contract
        lqdr_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        uint256 lqdrBalance = lqdr_.balanceOf(address(this));
        if(lqdrBalance < prizeAmount)
            lqdr_.safeTransfer(address(msg.sender), lqdrBalance);
        else
            lqdr_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        uint256 lqdrBalance = lqdr_.balanceOf(address(this));
        if(lqdrBalance < totalPrize)
            lqdr_.safeTransfer(address(msg.sender), lqdrBalance);
        else
            lqdr_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in lqdr the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        prize = allLotteries_[_lotteryId].prizePoolInLqdr.mul(perOfPool).div(100).div(winnerCountsOfPool);
    ...
    function emergencyWithdraw() external onlyOwner {
        lqdr_.safeTransfer(msg.sender, lqdr_.balanceOf(address(this)));
    }
}
