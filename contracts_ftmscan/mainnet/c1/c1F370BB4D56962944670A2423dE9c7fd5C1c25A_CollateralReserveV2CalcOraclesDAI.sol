...
contract CollateralReserveV2CalcOraclesFTM is CoffinBandOracle, ICollateralReserveV2OCalc {
    ...
    function valuation( uint amount_ ) external view override returns ( uint _value ) {
        ...
        uint256 price2 = price.mul(PRICE_PRECISION).div(10**d); 
        _value = amount_.mul(price2).div(1e18);
    }
}

contract CollateralReserveV2CalcOraclesDAI is CoffinBandOracle, ICollateralReserveV2OCalc {
    using SafeMath for uint256;
    function getPrice() public view override returns (uint256, uint8) {
...
contract CollateralReserveV2CalcOraclesUSDC is CoffinBandOracle, ICollateralReserveV2OCalc {
    using SafeMath for uint256;
...
contract CollateralReserveV2CalcOraclesETH is CoffinBandOracle, ICollateralReserveV2OCalc {
    using SafeMath for uint256;

    function getPrice() public view override returns (uint256, uint8) {
        return (super.getBandRate("ETH","USD"), 18);
    }
        function valuation( uint amount_ ) external view override returns ( uint _value ) {
        ( uint256 price, uint8 d ) = getPrice();
        uint256 price2 = price.mul(PRICE_PRECISION).div(10**d); 
