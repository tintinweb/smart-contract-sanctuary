...
abstract contract FoldingBase is StrategyBase, IFoldStrategy {
  ...
  function doHardWork() external onlyNotPausedInvesting override restricted {
    uint gas = gasleft();
    console.log("DO HARD WORK", gas);
    investAllUnderlying();
    console.log("investAllUnderlying", gasleft());
    _claimReward();
    console.log("_claimReward", gasleft());
    _compound();
    console.log("_compound", gasleft());
    // supply underlying for avoiding liquidation in case of reward is the same as underlying
    if (underlyingBalance() > 0) {
      _supply(underlyingBalance());
    }
    console.log("_supply", gasleft());
    liquidateReward();
    console.log("liquidateReward", gasleft());
    if (foldState == 0) {
      if (!isFoldingProfitable() && fold) {
        _stopFolding();
      } else if (isFoldingProfitable() && !fold) {
    ...
    console.log("DO HARD WORK end", gas - gasleft(), gasleft());
  }
  ...
  function depositToPool(uint256 amount) internal override updateSupplyInTheEnd {
    ...
    uint256 borrowTarget = _borrowTarget();
    uint256 i = 0;
    console.log("depositToPool before cycle", gasleft());
    ...
    console.log("depositToPool adter cycle", gasleft());
  }
  ...
  function _redeemPartialWithLoan(uint256 amount) internal updateSupplyInTheEnd {
    console.log("_redeemPartialWithLoan", gasleft());
    (uint256 supplied, uint256 borrowed) = _getInvestmentData();
    ...
    uint256 i = 0;
    console.log("---gas used beofor cycle", gasleft());
    ...
    console.log("---gas used after cycle", gasleft());
    underlyingBalance = IERC20(_underlyingToken).balanceOf(address(this));
