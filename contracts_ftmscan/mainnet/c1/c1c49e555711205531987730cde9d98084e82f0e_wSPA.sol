...
contract wSPA is ERC20 {
    using SafeERC20 for ERC20;
    using Address for address;
    using SafeMath for uint;

    uint baseIndex = 7675210820;

    address public immutable sOHM;

    constructor( address _sOHM ) ERC20( 'Wrapped sSPA', 'wsSPA' ) {
    ...
    function wOHMTosOHM( uint _amount ) public view returns ( uint ) {
        return _amount.mul( (IsOHM( sOHM ).index()).mul(10 ** 9).div(baseIndex) ).div( 10 ** decimals() );
    }
    ...
    function sOHMTowOHM( uint _amount ) public view returns ( uint ) {
        return _amount.mul( 10 ** decimals() ).div( (IsOHM( sOHM ).index()).mul(10 ** 9).div(baseIndex) );
    }
