...
interface ICurveGeistPool {
    // add liquidity (dai or usdc) to receive back DAI+USDC
    function add_liquidity(uint256[3] memory _deposit_amounts, uint256 _min_mint_amount) external returns(uint256);
...
contract CurveGeistGaugeAllocator is Ownable {
    ...
    ICurveGeistPool public immutable curveGeistPool; // Curve GeistPool
    ...
    constructor ( 
        ...
        require( _curveGeistPool != address(0) );
        curveGeistPool = ICurveGeistPool( _curveGeistPool );
    ...
    function deposit( address token, uint amount, uint minAmount ) public onlyPolicy() {
        ...
        IERC20(token).approve(address(curveGeistPool), amount); // approve curve pool to spend tokens
        uint curveAmount = curveGeistPool.add_liquidity(amounts, minAmount); // deposit into curve
    ...
    function withdraw( address token, uint amount, uint minAmount ) public onlyPolicy() {
        ...
        IERC20(curveToken).approve(address(curveGeistPool), amount); // approve curveGeistPool to spend curveToken
        curveGeistPool.remove_liquidity_one_coin(amount, tokenInfo[ token ].index, minAmount); // withdraw from curve
    ...
    function addToken( int128 index, address principleToken, uint max ) external onlyPolicy() {
        ...
        address token=curveGeistPool.coins(uint(index));
        require( token != address(0) && principleToken==token,"principle token and index not matched");
        address curveToken=address(curveGeistPool);
    ...
    function showPrinciplePnL( address token ) public view returns (bool _profit, uint _amount) {
        ...
        uint underlyingAmount = curveGeistPool.calc_withdraw_one_coin(curveAmount,tokenInfo[ token ].index);
