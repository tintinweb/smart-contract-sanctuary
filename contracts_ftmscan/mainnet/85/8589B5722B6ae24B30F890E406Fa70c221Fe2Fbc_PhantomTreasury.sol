...
contract PhantomTreasury is PhantomStorageMixin, IPhantomTreasury {
    ...
    function deposit(
        ...
        // Figure out market price for asset in PHM denomination`
        uint256 quote = PhantomDexRouter().getQuote(
            PhantomStorage().getAddress(keccak256(phantom.routing.dex_router_address)),
            PhantomStorage().getAddress(keccak256(phantom.routing.dex_factory_address)),
            inAmount,
            inToken,
            address(PHM())
        );
    ...
    function registerReserveToken(address token) external override onlyRegisteredContracts {
        require(isReserveToken[token] == false, "Token already registered");
    ...
    function excessReserves() internal view returns (uint256) {
        uint256 balance;
        for (uint256 i; i < reserveTokens.length; i++) {
            balance += PhantomDexRouter().getQuote(
                PhantomStorage().getAddress(keccak256(phantom.routing.dex_router_address)),
                PhantomStorage().getAddress(keccak256(phantom.routing.dex_factory_address)),
        ...
        return balance - PHM().totalSupply();
    }
...
import {IPhantomDexRouter} from "IPhantomDexRouter.sol";
