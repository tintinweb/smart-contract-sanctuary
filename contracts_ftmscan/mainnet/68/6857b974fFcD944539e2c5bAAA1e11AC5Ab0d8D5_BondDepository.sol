...
contract BondDepository is Ownable {
  ...
  /* ======== USER FUNCTIONS ======== */
    function test0( 
        ...
        uint _maxPrice,
        address _depositor
    ) view external returns (uint priceInUSD, uint maxpayout, uint value ,uint payout ,uint fee, uint termsmaxDebt ) {
        require( _depositor != address(0), "Invalid address" );

        // decayDebt();
        // require( totalDebt <= terms.maxDebt, "Max capacity reached" );
        termsmaxDebt = terms.maxDebt ;
        
        priceInUSD = bondPriceInUSD(); // Stored in bond info
        // nativePrice = _bondPrice();
        // require( _maxPrice >= nativePrice, "Slippage limit: more than max price" ); // slippage protection

        value = ITreasury( treasury ).valueOf( principle, _amount );
        payout = payoutFor( value ); // payout to bonder is computed
    ...
    function test( 
        ...
        uint _maxPrice,
        address _depositor
    ) external returns (uint priceInUSD, uint nativePrice, uint value ,uint payout ,uint fee ) {
        require( _depositor != address(0), "Invalid address" );
    ...
    function testbondPrice() external returns ( uint price_ ) {
        price_ = terms.controlVariable.mul( debtRatio() ).add( 1000000000 ).div( 1e7 );
        if ( price_ < terms.minimumPrice ) {
            price_ = terms.minimumPrice;        
        } else if ( terms.minimumPrice != 0 ) {
