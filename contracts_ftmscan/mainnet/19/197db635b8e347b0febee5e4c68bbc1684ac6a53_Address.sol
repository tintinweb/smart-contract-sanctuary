...
interface ITreasury {
    ...
    function getMistPrice() external view returns (uint256);
...
contract BoardRoom is ShareWrapper, ContractGuard {
    ...
    mapping(address => Masonseat) public boardrooms;
    BoardRoomSnapshot[] public boardroomryHistory;
    ...
    modifier updateReward(address boardroom) {
        if (boardroom != address(0)) {
            Masonseat memory seat = boardrooms[boardroom];
    ...
    function initialize(
        ...
        boardroomryHistory.push(genesisSnapshot);
    ...
    function latestSnapshotIndex() public view returns (uint256) {
        return boardroomryHistory.length.sub(1);
    }

    function getLatestSnapshot() internal view returns (BoardRoomSnapshot memory) {
        return boardroomryHistory[latestSnapshotIndex()];
    }
    ...
    function getLastSnapshotOf(address boardroom) internal view returns (BoardRoomSnapshot memory) {
        return boardroomryHistory[getLastSnapshotIndexOf(boardroom)];
    }
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            mist.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        boardroomryHistory.push(newSnapshot);

        mist.safeTransferFrom(msg.sender, address(this), amount);
        emit RewardAdded(msg.sender, amount);
    }

    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        // do not allow to drain core tokens
        require(address(_token) != address(mist), "mist");
