...
contract FeedVaultBaseV3LP is FeedVault {
    ...
    function _collectEntryFees(uint256 _amount) internal override {
        // Collect Entry Fees
        uint256 _entryFees = _discountFees(_amount.mul(entryFeesBP).div(10000));
        if (_entryFees > 0) {
            if (autoSwapEntryFees) {
                _removeLiquidity(address(token), address(token0ToBuybackPath[0]), address(token1ToBuybackPath[0]), _entryFees);
                _buyBackFees(token0ToBuybackPath, address(token0ToBuybackPath[0]), referralEntryFeeBP, true, entryFeesCollector);
                _buyBackFees(token1ToBuybackPath, address(token1ToBuybackPath[0]), referralEntryFeeBP, true, entryFeesCollector);
            } else {
    ...
    function _collectExitFees(uint256 _amount) internal override returns (uint256) {
        uint256 _exitFees = _discountFees(_amount.mul(exitFeesBP).div(10000));
        if (_exitFees > 0) {
            if (autoSwapExitFees) {
                _removeLiquidity(address(token), address(token0ToBuybackPath[0]), address(token1ToBuybackPath[0]), _exitFees);
                _buyBackFees(token0ToBuybackPath, address(token0ToBuybackPath[0]), referralExitFeeBP, false, exitFeesCollector);
                _buyBackFees(token1ToBuybackPath, address(token1ToBuybackPath[0]), referralExitFeeBP, false, exitFeesCollector);
            } else {
    ...
    function _buyBackFees(
        ...
        if (_buyBackPath.length > 1) {
            uint256 _before = _buyBackToken.balanceOf(address(this));
            uint256 _amount = IERC20(_token).balanceOf(address(this));
            if (_amount > 0) {
                _safeSwap(swapRouterAddress, _amount, _buyBackPath, address(this), block.timestamp.add(600));
            }
            _buyBackAmount = _buyBackToken.balanceOf(address(this)).sub(_before);
        } else {
            _buyBackAmount = _buyBackToken.balanceOf(address(this));
        }

        uint256 _commissionPaid = _payReferralCommission(address(msg.sender), address(_buyBackToken), _buyBackAmount, _referralFeeBP);

        _buyBackToken.safeTransfer(_feesCollector, _buyBackAmount.sub(_commissionPaid));

        emit FeesCollected(_isEntry, _feesCollector, address(_buyBackToken), _buyBackAmount.sub(_commissionPaid));
    }
