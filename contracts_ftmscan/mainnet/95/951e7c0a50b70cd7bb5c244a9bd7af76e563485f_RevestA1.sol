...
contract RevestA1 is IRevest, AccessControlEnumerable, RevestAccessControl, RevestReentrancyGuard {
    ...
    function withdrawFNFT(uint fnftId, uint quantity, bytes memory args) external revestNonReentrant(fnftId) {
        ...
        address output = config.pipeToContract;
        vault.withdrawToken(fnftId, quantity, _msgSender());

        if(output.supportsInterface(OUTPUT_RECEIVER_INTERFACE_V2_ID)) {
            // Callback to OutputReceiverV2
            IOutputReceiverV2(output).receiveSecondaryCallback(fnftId, payable(_msgSender()), quantity, config, args);
        }
    ...
    function withdrawAndRemap(uint fnftId, IRevest.FNFTConfig memory fnftConfig) external {
        ...
        uint quantity = IFNFTHandler(fnftHandler).getSupply(fnftId);
        require(quantity > 0, 'E044');
        ...
        ITokenVault(vault).createFNFT(fnftId, fnftConfig, quantity, _msgSender());
    ...
    function doMint(
        ...
        // Code here is modified to send to admin wallet for multi-chain deployments
        // From there, it can be bridged back to ETH and distributed to the Revest stakers
        address adminWallet = addressesProvider.getAdmin();

        if(flatWeiFee > 0) {
            require(weiValue >= flatWeiFee, "E005");
            IERC20(WETH).safeTransfer(adminWallet, flatWeiFee);
        }

        {
            uint totalERC20Fee = erc20Fee * totalQuantity * fnftConfig.depositAmount / erc20multiplierPrecision;
            if(totalERC20Fee > 0) {
                IERC20(fnftConfig.asset).safeTransferFrom(_msgSender(), adminWallet, totalERC20Fee);
            }
        }
