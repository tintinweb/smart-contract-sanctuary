...
// Note that this pool has no minter key of VEXA (rewards).
// Instead, the governance will call VEXA distributeReward method and send reward to this pool at the beginning.
contract VexaRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. VEXAs to distribute in the pool.
        uint256 lastRewardTime; // Last time that VEXAs distribution occurred.
        uint256 accTombPerShare; // Accumulated VEXAs per share, times 1e18. See below.
    ...
    // The time when VEXA mining starts.
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "VexaRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "VexaRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's VEXA allocation point. Can only be called by the owner.
    ...
    // View function to see pending VEXAs on frontend.
    ...
    // Safe tomb transfer function, just in case if rounding error causes pool to not have enough VEXAs.
