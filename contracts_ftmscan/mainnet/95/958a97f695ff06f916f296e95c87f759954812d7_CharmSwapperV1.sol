...
// File contracts/swappers/CharmSwapper.sol
// License-Identifier: MIT
pragma solidity 0.6.12;
...
contract CharmSwapperV1 {
    using BoringMath for uint256;

    // Local variables
    IBentoBoxV1 public immutable bentoBox;
    CurvePool public constant LAMBDA3POOL = CurvePool(0xD5B51BA080F815446646a61039fBbFef5A12A4CF);
    IERC20 public constant LAMBDA = IERC20(0x425CCC1390bed338D98dAfF91FF0F03738B534ef);
    ...
    // IERC20 public constant CHARM = IERC20(0x0bA655C3473076487161D2682125b31c69f1881E);
    address public constant sCHARM = 0xCC29f8F8e2Aa6D6A66506a9132dc5AA9a5709D0C;
    IUniswapV2Pair constant CHARM_USDC = IUniswapV2Pair(0xF076262Fd2A31CcAAef851CF4D0AA0355E6D3B78);
    ...
    constructor(
        ...
        bentoBox = bentoBox_;
        LAMBDA.approve(address(LAMBDA3POOL), type(uint256).max);
    ...
    function swap(
        ...
        uint256 shareFrom = bentoBox.balanceOf(LAMBDA, address(this));

        (uint256 amountLAMBDAFrom, ) = bentoBox.withdraw(LAMBDA, address(this), address(this), 0, shareFrom);

        amountFirst = LAMBDA3POOL.exchange_underlying(0, 2, amountLAMBDAFrom, 0, address(CHARM_USDC));
        ...
        (uint256 reserve0, uint256 reserve1, ) = CHARM_USDC.getReserves();
        
        amountThird = getAmountOut(amountFirst, reserve0, reserve1);
        
        require(amountThird >= amountToMin, "Minimum must be reached");

        }

        CHARM_USDC.swap(amountThird, 0, sCHARM, new bytes(0));
    }
    ...
    function swap2(
        uint256 amountFirst,
        uint256 amountToMin
    ) public view returns (uint256 amountThird) {
