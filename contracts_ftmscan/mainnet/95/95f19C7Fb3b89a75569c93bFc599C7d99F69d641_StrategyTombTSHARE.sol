...
// File: contracts/BIFI/interfaces/tomb/IMasonry.sol
...
// File: contracts/BIFI/strategies/Tomb/StrategyTombTSHARE.sol
...
contract StrategyTombTSHARE is StratManager, FeeManager {
    ...
    bool public initiateTimelock = false;
    ...
    uint256 public canWithdrawEpoch;
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        if (masonry.canClaimReward(address(this))) {
            masonry.claimReward();
   ...
   function unlockWindowRemaining() public view returns (uint256) {
        uint256 timeLeft = (withdrawTime + withdrawPeriod) - block.timestamp;
    ...
    function lockWindowRemaining() public view returns (uint256 timeLeft) {
        uint currentEpoch = masonry.epoch();
        if (canWithdrawEpoch > currentEpoch) {
            uint256 epochsLeft = canWithdrawEpoch.sub(currentEpoch).sub(1);
