...
contract Consumable is ERC1155SupplyUpgradeable, OwnableUpgradeable {
    using SafeMath for uint256;

    string internal baseURI;
    uint256 public constant Rename_Card = 0;

    uint256 public price;

    event Renamed(address from, uint256 id, string newName);

    function initialize() external initializer {
        __Ownable_init();
    }

    function rename(uint256 _id, string memory _name) public {
        emit Renamed(msg.sender, _id, _name);
