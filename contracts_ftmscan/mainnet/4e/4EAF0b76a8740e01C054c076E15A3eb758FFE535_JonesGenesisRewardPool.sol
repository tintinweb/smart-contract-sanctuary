...
// Note that this pool has no minter key of JONES (rewards).
// Instead, the governance will call JONES distributeReward method and send reward to this pool at the beginning.
contract JonesGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. JONES to distribute.
        uint256 lastRewardTime; // Last time that JONES distribution occurs.
        uint256 accTombPerShare; // Accumulated JONES per share, times 1e18. See below.
    ...
    uint256 public tombPerSecond = 0.08680555555 ether; // 15000 TOMB / (1h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "JonesGenesisPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "JonesGenesisPool: existing pool?");
        }
    }
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (JONES or lps) if less than 90 days after pool ends
            require(_token != tomb, "jones");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
    ...
    function pushStartForward(uint256 _newStartTime) external onlyOperator {
        require(block.timestamp > poolStartTime, "already started");
        poolStartTime = _newStartTime;
    }
}
