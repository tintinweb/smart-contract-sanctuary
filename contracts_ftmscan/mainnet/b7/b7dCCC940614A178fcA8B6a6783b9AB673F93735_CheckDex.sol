...
contract CheckDex {
    ...
    function executeArb(address router1Address, address router2Address, address[] calldata inPath, address[] calldata outPath, uint256 amountIn) external {
        IERC20(inPath[0]).transferFrom(msg.sender, address(this), amountIn);
        assert (inPath[0] == outPath[outPath.length-1]);
        assert (outPath[0] == inPath[inPath.length-1]);
        IUniswapV2Router02 router1 = IUniswapV2Router02(router1Address);
        IUniswapV2Router02 router2 = IUniswapV2Router02(router2Address);
        IERC20(inPath[0]).approve(address(router1), amountIn);
        router1.swapExactTokensForTokens(amountIn, 0, inPath, address(this), block.timestamp + 1000);
        uint256 next = IERC20(outPath[0]).balanceOf(inPath[inPath.length-1]);
        uint256 minOut = amountIn + 1; // Reverts if the trade is going to lose us money
        IERC20(outPath[0]).approve(address(router2), next);
        router2.swapExactTokensForTokens(next, minOut, outPath, msg.sender, block.timestamp + 1000);
