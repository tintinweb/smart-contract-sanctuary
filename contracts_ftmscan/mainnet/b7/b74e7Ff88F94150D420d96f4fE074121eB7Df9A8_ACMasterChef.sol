...
abstract contract AutoCompoundStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    function _safeSwap(
        ...
        if (_path.length == 2 && _path[0] == address(earnToken) && _path[1] == address(stakeToken)) {
            // first swap
            ...
            if (_ignoreErrors) {
                try
                    liquidityRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        _amountIn,
                        0,
                        swapPath[address(earnToken)][WNATIVE],
                        _to,
                        block.timestamp
                    )
                {} catch {}
            ...
            // second swap
            uint256 amountSwap = IERC20(WNATIVE).balanceOf(address(this));
            IERC20(WNATIVE).safeIncreaseAllowance(address(swapRouter), amountSwap);
            if (_ignoreErrors) {
                try
                    swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        amountSwap,
                        0,
                        swapPath[WNATIVE][address(stakeToken)],
                        _to,
                        block.timestamp
                    )
                {} catch {}
