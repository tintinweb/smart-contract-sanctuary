...
contract Liquidity is Ownable {
    ...
    address[] usdcToWftmPath = [address(USDC), address(WFtm)];
 ...
 function withdrawAll()  external onlyOwner {
  ...
  function AutoLiquidity() external onlyOwner {
    ...
    uint256 wftmBalance = address(this).balance;
    uint256 banksyBalance = BANKSY.balanceOf(address(this));
    uint256 halfTokenAmount = banksyBalance / 2;

    if (usdcBalanceAfter > 0 && wftmBalance > 0 && banksyBalance > 0) {
        // add the liquidity banksy-ftm
        ROUTER.addLiquidityETH{value: wftmBalance}(
            address(BANKSY),
            halfTokenAmount,
            0,
            0,
