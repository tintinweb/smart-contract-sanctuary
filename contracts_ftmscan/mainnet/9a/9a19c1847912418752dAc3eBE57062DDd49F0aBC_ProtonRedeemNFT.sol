...
interface INFTProxy {
    function mintTo(address _user, uint8 tier) external;

    function balanceOf(address owner) external view returns (uint256 balance);

    function ownerOf(uint256 tokenId) external view returns (address owner);

    function tokenUsed(uint256 _id) external view returns (bool);

    function tokenTier(uint256 _id) external view returns (uint8);

    function useToken(uint256 _id, address _nftOwner) external;
}
