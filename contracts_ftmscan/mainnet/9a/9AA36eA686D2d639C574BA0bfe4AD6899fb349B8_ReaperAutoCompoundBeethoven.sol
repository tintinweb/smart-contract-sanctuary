 ...
 */
contract ReaperAutoCompoundBeethoven is Ownable, Pausable {
    ...
    bytes32 public constant usdcRoute_ID = 0x03c6b3f09d2504606936b1a4decefad204687890000200000000000000000015;
    bytes32 public constant usdcFtmRoute_ID = 0xcdf68a4d525ba2e90fe959c74330430a5a6b8226000200000000000000000008;
    ...
    constructor (
      ...
      address _treasury
        ...
        lpToken = _lpToken;
        poolId = _poolId;
        poolID_bytes = IBasePool(_lpToken).getPoolId();
        vault = _vault;
        treasury = _treasury;
        ...
        if(usdcUnderlying && !beetsUnderlying && !wftmUnderlying){
            route_ID = usdcRoute_ID;
        }else{
            if(usdcUnderlying && wftmUnderlying){
                usdcUnderlying = false;
            }
            route_ID = wftmRoute_ID;
        }
    ...
    function harvest() public whenNotPaused {
        ...
        if(beetsUnderlying == true){_addLiquidity(rewardToken, beetsPosition);}
        else if(wftmUnderlying == true){_addLiquidity(wftm, wftmPosition);}
        else if(usdcUnderlying == true){_addLiquidity(usdc, usdcPosition);}
    ...
    function _chargeFees() internal {
        ...
        if(beetsUnderlying == true || usdcUnderlying == true){
            ...
            fees = rewardBal;
            _swap(rewardToken, wftm, route_ID, fees);
            wftmFees = IERC20(wftm).balanceOf(address(this)).sub(wftmBal).mul(totalFee).div(PERCENT_DIVISOR);
        }
    ...
    function _addLiquidity(address token, uint8 position) internal {
        if(usdcUnderlying == true){
            if(!beetsUnderlying){
                uint rewardBal = IERC20(rewardToken).balanceOf(address(this));
                _swap(rewardToken, usdc, usdcRoute_ID, rewardBal);
            }
        }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(lpToken).safeApprove(masterChef, type(uint256).max);
    ...
    function _swapSecondToken(address token, bytes32 route) internal {
        ...
        if(wftmUnderlying == true || beetsUnderlying == true )
        {
            uint256 wftmBal = IERC20(wftm).balanceOf(address(this));
        ...
        else if(usdcUnderlying == true)
        {
            ...
            wftmBal = IERC20(wftm).balanceOf(address(this));
            _swap(wftm, usdc, usdcFtmRoute_ID, wftmBal);
        }
