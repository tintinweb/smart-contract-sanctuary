...
import './AceLab.sol';
import './interfaces/IXBOO.sol';

contract AceLabHelper is Ownable {
    ...
    function setAceLab(AceLab _acelab) external onlyOwner  {
        acelab = _acelab;
    }
    ...
    function getUserStakedXBOO(address _user) public view returns(uint256 amount) {
        uint256 _amt;
        for (uint256 i = 0; i < pids.length; i++) {
            (_amt,)  = acelab.userInfo(pids[i], _user);
            amount += _amt;
        }
    }
    
    function getVotes(address _user) public view returns(uint256 amount) {
        uint conversionRate = XBOO.xBOOForBOO(1e18) * 3;
        amount = (getUserStakedXBOO(_user) + XBOO.balanceOf(_user)) * conversionRate / 1e18;
    }
}
...
contract AceLab is Ownable {
    ...
    function checkForToken(IERC20 _Token) private view {
        require(address(_Token) != address(xboo), "recoverWrongTokens: Cannot be xboo");
        uint256 length = poolInfo.length;
        for (uint256 _pid = 0; _pid < length; _pid++) {
            require(address(poolInfo[_pid].RewardToken) != address(_Token), "checkForToken: reward token provided");
        }
    }
