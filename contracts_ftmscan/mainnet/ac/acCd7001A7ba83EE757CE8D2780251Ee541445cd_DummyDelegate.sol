...
// File: adapters\FTM\Dummy\DummyDelegate.sol
...
contract DummyDelegate {
    ...
    using SafeMath for uint256;
    IDummy constant SpiritFarm = IDummy(0xcB0D8e56F5c83394Bca540FA43039dC860D4C8Ab);
    IERC20 constant weth = IERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    IERC20 constant spirit = IERC20(0x5Cc61A78F164885776AA610fb0FE1257df78E59B);
    ...
    function deposit(address _adapter, uint256 poolId, uint256 amount) external  {
        ...
        lpToken.approve(address(SpiritFarm), uint256(amount));
        SpiritFarm.deposit(poolId, amount);
    }

    function withdraw(address, uint256 poolId, uint256 amount) external  {
        SpiritFarm.withdraw(poolId, amount);
    }

    function claimReward(address, uint256 poolId) external  {
        SpiritFarm.deposit(poolId, 0);
    }
