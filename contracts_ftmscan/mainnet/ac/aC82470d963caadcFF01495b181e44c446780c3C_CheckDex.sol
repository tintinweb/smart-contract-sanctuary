...
contract CheckDex {
    ...
    function executeArb(address router1Address, address router2Address, address[] calldata inPath, address[] calldata outPath, uint256 max_can_sell_tokenA,uint256 max_buy_tokenB) external {
        ...
        (uint112 inReserves2, uint112 outReserves2) = UniswapLibrary.getReserves(router2.factory(), outPath[0], outPath[1]);
        router2.swapTokensForExactTokens(UniswapLibrary.quote(max_buy_tokenB, inReserves2, outReserves2), max_buy_tokenB, outPath, msg.sender, block.timestamp + 1638007916);
    ...
    //         spookyTrade(amountIn, amountOutMin, path, deadline);(max_can_sell_tokenA, max_buy_tokenB, _getInPath(), block.timestamp+10000);
    //     } else {
