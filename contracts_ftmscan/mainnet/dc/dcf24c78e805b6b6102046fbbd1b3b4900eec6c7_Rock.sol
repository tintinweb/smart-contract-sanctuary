...
/**
 * @title SafeBEP20 ** NFT Gov Token - Project name redacted.
...
contract Rock is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0xF81D1eb206389F16E168938F2fb95C6be17c59Ac;
    ...
    // Min amount to liquify. (default 10 Rocks)
    ...
    // The operator can call all functions except of minting Rock
    address private _operator;
    
    // The lpEarner address set to burn address to lock liquidity forever
    address constant public lpEarner = 0xF81D1eb206389F16E168938F2fb95C6be17c59Ac;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "Rock::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the RockSwapToken contract.
     */
    constructor() public BEP20("Rock", "ROCK") {
    ...
    /// @dev overrides transfer function to meet tokenomics of Rock
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "Rock::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "Rock::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the Rock -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "Rock::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "ROCK::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "ROCK::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "ROCK::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "ROCK::updateSwapRouter: Invalid pair address.");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ROCKs to distribute per second.
        uint256 lastRewardSecond; // Last second that ROCKs distribution occurs.
    ...
    // ROCK tokens created per second.
    uint256 public RockPerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when Rock mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 RockPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _RockPerSecond,
        uint256 _startTime
    ) public {
        rock = _rock;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        RockPerSecond = _RockPerSecond;
    ...
    // View function to see pending rock on frontend.
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 rockReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
                uint256 devReward = rockReward.div(10);
                uint256 totalRewards = rock.totalSupply().add(devReward).add(
                rockReward
            );
                if (totalRewards > rock.maxSupply()) {
                rockReward = rock.maxSupply().sub(rock.totalSupply());
            }
            accRockPerShare = accRockPerShare.add(
                rockReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= rock.maxSupply()) {
            ...
            // mint the difference only to MC, update rockReward
            rockReward = rock.maxSupply().sub(rock.totalSupply());
            rock.mint(address(this), rockReward);
        }

        if (rockReward != 0) {
            // only calculate and update if rockReward is non 0
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accRockPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeRockTransfer(msg.sender, pending);
    ...
    function safeRockTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > rockBal) {
            transferSuccess = rock.transfer(_to, rockBal);
        } else {
            transferSuccess = rock.transfer(_to, _amount);
        }
        require(transferSuccess, "safeRockTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _RockPerSecond) external onlyOwner {
        require(_RockPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        RockPerSecond = _RockPerSecond;
        emit UpdateEmissionRate(msg.sender, _RockPerSecond);
    }
