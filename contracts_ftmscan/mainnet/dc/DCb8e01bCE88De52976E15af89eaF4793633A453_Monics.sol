...
contract Monics is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 MONICS-WETH LP -> MONICS pool
    uint256 public constant INITIAL_MONICS_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the MONICS ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("monics.money", "MONICS") {
        // Mints 1 MONICS to contract creator for initial pool setup
    ...
    function _getMonicsPrice() internal view returns (uint256 _monicsPrice) {
        try IOracle(monicsOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Monics: failed to fetch MONICS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _monicsPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_monicsPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMonicsOracle(address _monicsOracle) public onlyOperatorOrTaxOffice {
        require(_monicsOracle != address(0), "oracle address cannot be 0 address");
        monicsOracle = _monicsOracle;
    }
    ...
    /**
     * @notice Operator mints MONICS to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MONICS to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMonicsPrice = _getMonicsPrice();
            currentTaxRate = _updateTaxRate(currentMonicsPrice);
            if (currentMonicsPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_monicsPool != address(0), "!_monicsPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_monicsPool, INITIAL_MONICS_POOL_DISTRIBUTION);
