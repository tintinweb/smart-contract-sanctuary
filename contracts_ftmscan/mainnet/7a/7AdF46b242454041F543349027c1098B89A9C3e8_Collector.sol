...
// File contracts/Collector/Collector.sol

contract Collector is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;
    using SafeERC20 for IwOHM;

    IERC20 public immutable EXOD = IERC20(address(0x3b57f3FeAaF1e8254ec680275Ee6E7727C7413c7));
    IwOHM public immutable wsEXOD = IwOHM(address(0xe992C5Abddb05d86095B18a158251834D616f0D1));
    ...
    function _allowance(IERC20 _token, address _wallet) internal view returns (uint256) {
    ...
    function balanceOf(address _token) public view returns (uint256) {
        ...
        for (uint8 i = 0; i < _wallets.length(); ++i) {
            total += _balanceOf(IERC20(_token), _wallets.at(i));
        }

        return total;
    }

    function balanceOfEXOD() public view returns (uint256) {
        uint256 total = 0;

        for (uint8 i = 0; i < _wallets.length(); ++i) {
            total += _balanceOf(EXOD, _wallets.at(i));
        }

        return total;
    }

    function _balanceOf(IERC20 _token, address wallet)
        internal
        view
        returns (uint256)
    {
    ...
    function transfer(address _token) public returns (uint256) {
        ...
        for (uint8 i = 0; i < _wallets.length(); ++i) {
            uint256 balance = _balanceOf(token, _wallets.at(i));

            uint256 allowance = _allowance(token, _wallets.at(i));
    ...
    function transferEXOD() public returns (uint256) {
        uint256 total = 0;

        for (uint8 i = 0; i < _wallets.length(); ++i) {
            uint256 balance = _balanceOf(EXOD, _wallets.at(i));

            uint256 allowance = _allowance(EXOD, _wallets.at(i));

            if (allowance >= balance && balance != 0) {
                EXOD.safeTransferFrom(
                    _wallets.at(i),
                    address(this),
                    balance
                );
        ...
        EXOD.approve(address(wsEXOD), total);
        total = wsEXOD.wrapFromOHM(total);
        wsEXOD.transfer(owner(), total);

        return total;
    }

    function withdrawEXOD() public returns (uint256) {
        uint256 balance = EXOD.balanceOf(address(this));
        require(balance > 0, "no stuck balance");
        EXOD.safeTransfer(owner(), balance);
        return balance;
    }

    function withdrawWrappedStakedEXOD() public returns (uint256) {
        uint256 balance = wsEXOD.balanceOf(address(this));
        require(balance > 0, "no stuck balance");
        wsEXOD.safeTransfer(owner(), balance);
        return balance;
    }
}