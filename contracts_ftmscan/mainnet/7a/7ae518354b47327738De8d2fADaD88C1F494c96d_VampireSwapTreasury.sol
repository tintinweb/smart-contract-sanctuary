...
contract VampireSwapTreasury is Ownable, IVampireSwapTreasury {
    string public name = "VampireSwap Treasury";
    ...
    function setProfile(address _profileAddress) external override onlyOwner {
        require(profileAddress == address(0), "ONLY CHANGE ONCE");
        profileAddress = _profileAddress;
    }

    function setRouter(address _routerAddress) external override onlyOwner {
        require(routerAddress == address(0), "ONLY CHANGE ONCE");
    ...
    function updateTeam(uint _rid, uint _tid, uint _totalReward) internal {
        ...
        if (team.totalVolume > 0) {
            team.vamPerShare = team.totalReward.div(team.totalVolume);
        }
    }
    ...
    function closeRound(uint _rid) internal {
        ...
        vam.mint(address(this), totalReward);
    ...
    function addVolume(address _user, uint _vol) external override onlyRouter {
        if (roundInfo.length > 0) {
            ...
            (, int ftmPrice, , ,) = oracle.latestRoundData();
            uint volume = _vol.mul(uint(ftmPrice)).div(1e8);
            UserInfo storage user = userInfo[rid][_user];
            user.volume = user.volume.add(volume);
            IVampireSwapProfile profile = IVampireSwapProfile(profileAddress);
            uint tid = profile.teamid(_user);
            if (tid > 0) {
                user.tid = tid;
                TeamInfo storage team = teamInfo[rid][tid];
                team.totalVolume = team.totalVolume.add(volume);
                team.lastSwap = block.timestamp;
            }
            
            profile.setLastSwap(_user, volume);

            emit AddVolume(_user, volume);
        }
    }

    function pendingVam(address _user) public view returns (uint) {
        uint result = 0;
    ...
    function claimReward() external override onlyEOA {
            ...
            if (round.isClose) {
                ...
                if (user.volume > 0) {
                    ...
                    if (reward >= user.claimedReward) {
                        ...
                        uint balance = vam.balanceOf(address(msg.sender));
                        if (reward > balance) {
                            reward = balance;
                        }
                        user.claimedReward = user.claimedReward.add(reward);
                        vam.safeTransfer(address(msg.sender), reward);
