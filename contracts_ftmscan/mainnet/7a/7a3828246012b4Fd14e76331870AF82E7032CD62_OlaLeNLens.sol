...
contract OlaLeNLens {
    ...
    function viewLendingNetwork(ComptrollerOlaLeNLensInterface unitroller) external view returns (LendingNetworkView memory lendingNetworkView) {
        ...
        for (uint8 i = 0; i < allMarkets.length; i ++) {
            ...
            uint8 decimals = Erc20ForOlaLeNLens(underlying).decimals();
            PriceOracle priceOracle = ministry.getOracleForAsset(underlying);
            // The price scaled by mantissa
            uint assetPrice = priceOracle.getAssetPrice(underlying);

            uint underlyingSupplyRaw = market.totalSupply();
            uint supplyInUnits = underlyingSupplyRaw.mul(10**18).div(10**decimals);

            uint underlyingBorrowsRaw = market.totalBorrows();
            uint borrowsInUnits = underlyingBorrowsRaw.mul(10 ** 18).div(10**decimals);

            lendingNetworkView.totalSupply = lendingNetworkView.totalSupply.add(supplyInUnits.mul(assetPrice));
            lendingNetworkView.totalBorrows = lendingNetworkView.totalBorrows.add(supplyInUnits.mul(assetPrice));
        }
    }
