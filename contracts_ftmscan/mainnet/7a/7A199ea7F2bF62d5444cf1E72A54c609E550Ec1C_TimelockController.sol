...
/**
 * @dev CGSVFarm functions that do not require less than the min timelock
 */
interface ICGSVFarm {
    function add(
        uint256 _allocPoint,
...
contract TimelockController is AccessControl, ReentrancyGuard {
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        ICGSVFarm(_CGSVfarmAddress).set(_pid, _allocPoint, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        ICGSVFarm(_CGSVfarmAddress).add(0, _want, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
