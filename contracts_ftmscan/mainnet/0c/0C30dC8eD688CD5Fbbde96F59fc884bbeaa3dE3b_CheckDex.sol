...
contract CheckDex {
    ...
    function executeArb(address router1Address, address router2Address, address[] calldata inPath, address[] calldata outPath, uint256 max_can_sell_tokenA,uint256 max_buy_tokenB) external {
        ...
        uint256 bx = IERC20(inPath[1]).balanceOf(address(this));
        // router2.swapExactTokensForTokens(amountIn, amountOutMin, path, to, deadline);
        router2.swapExactTokensForTokensSupportingFeeOnTransferTokens(bx, 0, outPath, msg.sender, block.timestamp + 1638007916);
        uint256 b = IERC20(inPath[0]).balanceOf(address(this));
        // require(b > spent, "need to make money ser");
