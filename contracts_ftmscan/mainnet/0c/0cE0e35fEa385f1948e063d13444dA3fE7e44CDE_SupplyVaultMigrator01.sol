...
import "./interfaces/ISupplyVaultMigrator01.sol";

contract SupplyVaultMigrator01 is ISupplyVaultMigrator01 {
    using SafeERC20 for IERC20;
    using SafeERC20 for ISupplyVault;
    using SafeMath for uint256;

    receive() external payable {
        assert(false); // Do not accept ETH
    }

    function migrate(ISupplyVault fromVault, ISupplyVault toVault) external override returns (uint256 toShare) {
        require(fromVault != toVault, "SVR: SAME_VAULT");
        IERC20 underlying = fromVault.underlying();
        require(underlying == toVault.underlying(), "SVR: UNDERLYING");

        uint256 fromShare = IERC20(address(fromVault)).balanceOf(msg.sender);
        require(fromShare > 0, "SVR: ZERO_FROM_SHARE");
        IERC20(address(fromVault)).safeTransferFrom(msg.sender, address(this), fromShare);

        fromVault.leaveInKind(fromShare);

        uint256 numBorrowables = fromVault.getBorrowablesLength();
        for (uint256 i = 0; i < numBorrowables; i++) {
            IBorrowable borrowable = fromVault.borrowables(i);
            uint256 borrowableAmount = borrowable.balanceOf(address(this));
            if (borrowableAmount > 0) {
                require(toVault.getBorrowableEnabled(borrowable), "SVR: BORROWABLE");
                IERC20(address(borrowable)).safeApprove(address(toVault), borrowableAmount);
                uint256 addToShare = toVault.enterWithToken(address(borrowable), borrowableAmount);
                require(addToShare > 0, "SVR: ZERO_ADD_SHARE");
                toShare = toShare.add(addToShare);
            }
        }
        uint256 underlyingAmount = underlying.balanceOf(address(this));
        if (underlyingAmount > 0) {
            IERC20(underlying).safeApprove(address(toVault), underlyingAmount);
            uint256 addToShare = toVault.enterWithToken(address(underlying), underlyingAmount);
            require(addToShare > 0, "SVR: ZERO_ADD_SHARE");
            toShare = toShare.add(addToShare);
        }

        require(toShare > 0, "SVR: ZERO_TO_SHARE");
        IERC20(address(toVault)).safeTransfer(msg.sender, toShare);
    }
}
...
interface ISupplyVaultMigrator01 {
    function migrate(ISupplyVault fromVault, ISupplyVault toVault) external returns (uint256 toShare);
}
