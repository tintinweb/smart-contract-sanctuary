...
contract Voting1 {
    ...
    function _010_UseInfo_010_ (address account) external view returns(uint256 YouAreStar,uint256 starActiveHours,uint256 NewGade4Invite,uint256 UpdateHours,uint256 StarBonusNow,address Father){
    Father=Voters.GetRelationShip(account);
    UpdateHours=Voters.GetBlockNumber(account)/3600;
    StarBonusNow=Voters.GetStarPower(account);
    ( YouAreStar, starActiveHours, NewGade4Invite)=starInfo(account);
    return (YouAreStar,starActiveHours,NewGade4Invite,UpdateHours,StarBonusNow,Father);}
   
    function starInfo (address account) internal view returns(uint256 YouAreStar,uint256 starActiveHours,uint256 NewGade4Invite){
    ...
    if(YouAreStar1==true){YouAreStar=1;starActiveHours=(uint256(block.timestamp)-star1update)/3600;}
    if(YouAreStar2==true){YouAreStar=2;starActiveHours=(uint256(block.timestamp)-star2update)/3600;}
    if(YouAreStar3==true){YouAreStar=3;starActiveHours=(uint256(block.timestamp)-star3update)/3600;}
    if(YouAreStar4==true){YouAreStar=4;starActiveHours=(uint256(block.timestamp)-star4update)/3600;}
    return (YouAreStar,starActiveHours,NewGade4Invite);}
