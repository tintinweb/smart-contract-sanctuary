...
interface ICurveGeistPool {
    function add_liquidity(uint256[3] memory _deposit_amounts, uint256 _min_mint_amount, bool _use_underlying) external returns(uint256);
    function remove_liquidity_one_coin(uint256 _burn_amount, int128 i, uint256 _min_amount, bool _use_underlying) external returns(uint256);
...
contract CurveGeistGaugeAllocator is Ownable {
    ...
    function deposit( address token, uint amount, uint minAmount ) public onlyPolicy() {
        ...
        uint curveAmount = curveGeistPool.add_liquidity(amounts, minAmount, true); // deposit into curve

        address curveToken = tokenInfo[ token ].curveToken;
        require(curveAmount == IERC20( curveToken ).balanceOf(address(this)), "!curveAmount");
    ...
    function withdraw( address token, uint amount, uint minAmount ) public onlyPolicy() {
        ...
        curveGeistPool.remove_liquidity_one_coin(amount, tokenInfo[ token ].index, minAmount, true); // withdraw from curve
    ...
    function addToken( int128 index, address principleToken, uint max ) external onlyPolicy() {
        ...
        address token = curveGeistPool.underlying_coins(uint(index));
        ...
        address curveToken = curveGauge.lp_token();
