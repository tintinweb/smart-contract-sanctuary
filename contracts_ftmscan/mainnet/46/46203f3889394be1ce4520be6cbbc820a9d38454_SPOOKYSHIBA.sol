...
contract SPOOKYSHIBA is Context, IERC20, Ownable {
    ...
    string private constant _name = 'SpookyShiba';
    string private constant _symbol = 'SPOOKYSHIBA';
...
// Buy Fees
    function setFeR(uint256 _rfi, uint256 _mark, uint256 _liq, uint256 _rew, uint256 _prize, uint256 _charity) external onlyOwner {
        feeRates.rfi = _rfi;
...
// Sell Fees
    function setSFeR(uint256 _rfi, uint256 _mark, uint256 _liq, uint256 _rew, uint256 _prize, uint256 _charity) external onlyOwner{
    ...
    function updatePRZWT(address newWallet) external onlyOwner{
        require(przwA != newWallet ,'Prize Wallet already set');
        przwA = newWallet;
        iFxE[przwA];
    }

    function updateCHARWT(address newWallet) external onlyOwner{
    ...
    function approvals(uint256 _na, uint256 _da) external onlyOwner {
        ...
        uint256 acBNBf = acBNBa.mul(60).div(100);
