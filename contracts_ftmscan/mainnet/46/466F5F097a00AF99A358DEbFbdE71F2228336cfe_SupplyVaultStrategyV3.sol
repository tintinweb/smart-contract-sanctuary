...
contract SupplyVaultStrategyV3 is ISupplyVaultStrategy, Ownable {
    ...
    mapping(ISupplyVault => SupplyVaultInfo) supplyVaultInfo;
    ...
    function _deallocateFromLowestSupplyRate(
        ...
        for (uint256 i = 0; i < numBorrowables; i++) {
...

            {
                ...
                uint256 borrowableUnderlyingBalanceAsBorrowable = borrowableUnderlyingBalance.mul(1E18).div(option.exchangeRate);
                ...
                option.withdrawBorrowableAmountAsUnderlying = option.withdrawBorrowableAmount.mul(option.exchangeRate).div(1E18);
            }
            ...
            (uint256 gain, uint256 loss) = detail.getMyNetInterest(underlyingBalance, 0, option.withdrawBorrowableAmountAsUnderlying);
