...
contract Masterchef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accGoatPerShare;    
        uint16 depositFeeBP;
        uint256 lpSupply;
    }
    
    GoatToken public immutable goat;
    ...
    uint256 public goatPerSec = 0.009 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 goatPerSec);

    constructor(GoatToken _goat, uint256 _startTime, address _feeAdd, address _treasuryAdd) {
        goat = _goat;
        startTime = _startTime;
        feeAdd = _feeAdd;
        treasuryAdd = _treasuryAdd;
        MAXSUPPLY = _goat.maxSupply();
    }
    ...
    function safeGoatTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > goatBalance) {
            transferSuccess = goat.transfer(_to, goatBalance);
        } else {
    ...
    function updateEmissionRate(uint256 _goatPerSec) external onlyOwner {
        require(_goatPerSec >= MIN_PERSEC && _goatPerSec <= MAX_PERSEC, "Bad Emissions per second");
        massUpdatePools();
        goatPerSec = _goatPerSec;
        emit UpdateEmissionRate(msg.sender, _goatPerSec);
    }
    ...
    function pendingGoat(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardperSec && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 goatReward = (multiplier * goatPerSec * pool.allocPoint) / totalAllocPoint;
            uint256 distribution = goatReward / 110;
            if (goatReward + distribution + TOTALSUPPLY() > MAXSUPPLY) {
                goatReward = MAXSUPPLY - TOTALSUPPLY();
            }
            accGoatPerShare += goatReward * 1e18 / pool.lpSupply;
        }
        return ((user.amount * accGoatPerShare) / 1e18) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 goatReward = (multiplier * goatPerSec * pool.allocPoint) / totalAllocPoint;
        uint256 toTreasury = goatReward / 100;
        uint256 toBurn = goatReward / 10;
        
        if (TOTALSUPPLY() + goatReward + toTreasury + toBurn <= MAXSUPPLY) {
            goat.mint(treasuryAdd, toTreasury);
            goat.mint(destroyerAdd(), toBurn);
        ...
        pool.accGoatPerShare += goatReward * 1e18 / pool.lpSupply;
        pool.lastRewardperSec = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accGoatPerShare) / 1e18) - user.rewardDebt;
        ...
        user.rewardDebt = (user.amount * pool.accGoatPerShare) / 1e18;
