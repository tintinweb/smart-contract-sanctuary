...
contract SaleNFT is ReentrancyGuard, Ownable, ERC721Holder {
    ...
    IERC721Enumerable public banksyNFT = IERC721Enumerable(0x9C26e24ac6f0EA783fF9CA2bf81543c67cf446d2); 

    uint256 public nftCardPerAccountMaxTotal = 9; // 9 cards
    
    uint256 public salePrice = 20 * (10 ** 6); // 20 usdc
    ...
    function buyNftCard(uint256 _amountNFT) external nonReentrant {
        ...
        require(userNftCardTotally[_msgSender()] < nftCardPerAccountMaxTotal, "E3");        

        uint256 _balanceOfNFT = banksyNFT.balanceOf(address(this));

        require(_balanceOfNFT > 0, "E4");
        
        if ((userNftCardTotally[_msgSender()] + _amountNFT) > nftCardPerAccountMaxTotal)
            _amountNFT = nftCardPerAccountMaxTotal - userNftCardTotally[_msgSender()];
        ...
        require(_usdcSpend <= USDC.balanceOf(_msgSender()), "E6");
        ...
        if (_balanceOfNFT > 0) {
              ...
              userNftCardTotally[_msgSender()] = userNftCardTotally[_msgSender()] + _amountNFT;
              // send usdc to sale address
              USDC.safeTransferFrom(_msgSender(), SALE_ADDRESS, _usdcSpend);
        }
        
        emit BuyNftCard(_msgSender(), _amountNFT, _usdcSpend);
