...
contract VaultTesla is ERC20Simplified, Ownable {
    ...
    constructor() ERC20Simplified("EQ Shares Tesla", "EQS") {
    ...
    function strategyInfo(uint256 _nameID) public view returns (
        ...
        _router = IStrategy(_strategy).router();
        _lpToken = IStrategy(_strategy).lpToken();
        _token1 = IStrategy(_strategy).token1();
        _token0 = IStrategy(_strategy).token0();
        _farm = IStrategy(_strategy).farm();
        if(IStrategy(_strategy).hasPid()) {
            _pid = Strings.toString(IStrategy(_strategy).pid());
        } else {
            _pid = "No pid";
        }
        _totalCapital = IStrategy(_strategy).getTotalCapital();
        _totalCapitalInternal = IStrategy(_strategy).getTotalCapitalInternal();
        _totalLP = IStrategy(_strategy).totalLP();
    }
