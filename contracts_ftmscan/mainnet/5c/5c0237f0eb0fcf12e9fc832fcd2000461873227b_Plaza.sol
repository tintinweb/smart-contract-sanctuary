...
contract Plaza is Context, IERC20, Ownable {
    ...
    string private _name = "Plaza Finance (Official)";
        ...
        )
    {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tDev).sub(tBurning);
        return (tTransferAmount, tFee, tDev, tBurning);
    }
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rDev).sub(rBurning);
    ...
    function _burnTokens(uint256 tBurning) private {
        uint256 currentRate = _getRate();
        uint256 rBurning = tBurning.mul(currentRate);
    ...
    function calculateBurningFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_burningFee).div(10**2);
    }
