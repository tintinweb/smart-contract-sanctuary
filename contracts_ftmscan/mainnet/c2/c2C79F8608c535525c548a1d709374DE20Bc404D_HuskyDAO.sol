...
contract HuskyDAO is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 HUSKY-WETH LP -> HUSKY pool
    uint256 public constant INITIAL_HUSKY_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the HUSKY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("HUSKY DAO", "HUSKY") {
        // Mints 1 HUSKY to contract creator for initial pool setup
    ...
    function _getHuskyDAOPrice() internal view returns (uint256 _huskyPrice) {
        try IOracle(huskyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("HuskyDAO: failed to fetch HUSKY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _huskyPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_huskyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHuskyDAOOracle(address _huskyOracle) public onlyOperatorOrTaxOffice {
        require(_huskyOracle != address(0), "oracle address cannot be 0 address");
        huskyOracle = _huskyOracle;
    }
    ...
    /**
     * @notice Operator mints HUSKY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HUSKY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHuskyDAOPrice = _getHuskyDAOPrice();
            currentTaxRate = _updateTaxRate(currentHuskyDAOPrice);
            if (currentHuskyDAOPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_huskyPool != address(0), "!_huskyPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_huskyPool, INITIAL_HUSKY_POOL_DISTRIBUTION);
