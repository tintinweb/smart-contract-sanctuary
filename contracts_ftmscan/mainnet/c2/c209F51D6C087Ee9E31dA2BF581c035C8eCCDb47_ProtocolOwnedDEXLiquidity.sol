...
// File: contracts/interfaces/IBrujeria.sol
...
// File: contracts/PODL.sol

pragma solidity ^0.7.6;

interface ISoulSwapLPToken {
...
contract ProtocolOwnedDEXLiquidity is Ownable {
    ...
    ISoulSwapLPToken constant public lpToken = ISoulSwapLPToken(0xa2527Af9DABf3E3B4979d7E0493b5e2C6e63dC57); // SOUL-FTM
    IERC20 constant public WFTM = IERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83); 
    IMultiFeeDistribution constant public treasury = IMultiFeeDistribution(0x3C4D3D1A778b595fA2fC541ee8647b0CdBc2c4aB);
    address constant public DAO = 0xcB5ba2079C7E9eA6571bb971E383Fe5D59291a95;
    ...
    function initialize(
        ...
        require(!isInitialized, "has already begun");
        IBrujeria bruja = IBrujeria(0x822800989e69b2B33B70Dac596B9Dc60083848C9);
        bruja.setClaimReceiver(address(this), address(treasury));
    ...
    function protocolOwnedReserves() public view returns (uint256 wftm, uint256 soul) {
        (uint reserve0, uint reserve1,) = lpToken.getReserves();
        uint balance = lpToken.balanceOf(DAO);
    ...
    function availableFTM() public view returns (uint256) {
        return WFTM.balanceOf(address(this)) / 2;
    }
    ...
    function _buy(uint _amount, uint _cooldownTime) internal {
        ...
        require(_amount >= minBuyAmount, "Below minimum buy amount");
        require(block.timestamp >= u.nextClaimTime, "Claimed too recently");

        uint lpAmount = _amount.mul(lpTokensPerOneFTM()).div(1e18);
        lpToken.transferFrom(msg.sender, DAO, lpAmount);
        WFTM.transfer(msg.sender, _amount);
        WFTM.transfer(address(treasury), _amount);
    ...
    function superPODL(uint256 _amount) public {
        require(treasury.lockedBalances(msg.sender) >= minSuperPODLLock, "must lock SOUL");
        _buy(_amount, superPODLCooldown);
        emit SupremeActivated(msg.sender, _amount);
    }
}