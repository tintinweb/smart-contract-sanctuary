...
contract Treasury is ContractGuard {
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of PEPPER price
    uint256 public bootstrapEpochs;
    uint256 public bootstrapSupplyExpansionPercent;

    /* =================== Added variables =================== */
    uint256 public previousEpochPepperPrice;
    uint256 public maxDiscountRate; // when purchasing bond
    uint256 public maxPremiumRate; // when redeeming bond
    uint256 public discountPercent;
    uint256 public premiumThreshold;
    uint256 public premiumPercent;
    uint256 public mintingFactorForPayingDebt; // print extra PEPPER during debt phase
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getPepperPrice() > kittyPriceCeiling) ? 0 : getPepperCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    // oracle
    function getPepperPrice() public view returns (uint256 kittyPrice) {
        try IOracle(kittyOracle).consult(kitty, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult PEPPER price from the oracle");
        }
    }

    function getPepperUpdatedPrice() public view returns (uint256 _kittyPrice) {
    ...
    function getBurnablePepperLeft() public view returns (uint256 _burnablePepperLeft) {
        uint256 _kittyPrice = getPepperPrice();
        if (_kittyPrice <= kittyPriceOne) {
            uint256 _kittySupply = getPepperCirculatingSupply();
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnablePepper = _maxMintableBond.mul(_kittyPrice).div(1e18);
                _burnablePepperLeft = Math.min(epochSupplyContractionLeft, _maxBurnablePepper);
            }
        }
    }
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_kittyPrice > kittyPriceCeiling) {
            uint256 _totalPepper = IERC20(kitty).balanceOf(address(this));
            uint256 _rate = getBondPremiumRate();
            if (_rate > 0) {
                _redeemableBonds = _totalPepper.mul(1e18).div(_rate);
            }
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_kittyPrice <= kittyPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = kittyPriceOne.mul(1e18).div(_kittyPrice); // to burn 1 PEPPER
    ...
    function initialize(
        ...
        kittyPriceOne = 10**18; // This is to allow a PEG of 1 PEPPER per AVAX
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn PEPPER and mint tBOND)
    ...
    function setPepperOracle(address _kittyOracle) external onlyOperator {
        kittyOracle = _kittyOracle;
    }

    function setPepperPriceCeiling(uint256 _kittyPriceCeiling) external onlyOperator {
    ...
    function _updatePepperPrice() internal {
        try IOracle(kittyOracle).update() {} catch {}
    }

    function getPepperCirculatingSupply() public view returns (uint256) {
    ...
    function buyBonds(uint256 _kittyAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 kittyPrice = getPepperPrice();
        require(kittyPrice == targetPrice, "Treasury: PEPPER price moved");
        require(
        ...
        uint256 kittySupply = getPepperCirculatingSupply();
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        _updatePepperPrice();
        previousEpochPepperPrice = getPepperPrice();
        uint256 kittySupply = getPepperCirculatingSupply().sub(seigniorageSaved);
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _sendToBoardroom(kittySupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochPepperPrice > kittyPriceCeiling) {
                // Expansion ($PEPPER Price > 1 $ETH): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bbond).totalSupply();
                uint256 _percentage = previousEpochPepperPrice.sub(kittyPriceOne);
