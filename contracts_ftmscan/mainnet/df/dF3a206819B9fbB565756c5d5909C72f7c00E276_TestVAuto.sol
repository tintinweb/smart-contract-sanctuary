...
contract TestVAuto is Ownable, Pausable {
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingCake(0, address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IPancakeswapFarm(masterchef).leaveStaking(balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IPancakeswapFarm(masterchef).userInfo(0, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IPancakeswapFarm(masterchef).enterStaking(bal);
        }
    }
