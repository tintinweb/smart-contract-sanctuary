...
contract Treasury is Ownable {
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20Metadata(sTOKEN).balanceOf(msg.sender); // Can only borrow against sTOKEN held
  ...
  function excessReserves() public view returns (uint256) {
    return
      totalReserves.sub(IERC20Metadata(TOKEN).totalSupply().sub(totalDebt));
  }
  ...
  function auditReserves() external onlyOwner {
    ...
    for (uint256 i = 0; i < reserveTokens.length; i++) {
      reserves = reserves.add(
        valueOf(
          reserveTokens[i],
          IERC20Metadata(reserveTokens[i]).balanceOf(address(this))
        )
      );
    }
    for (uint256 i = 0; i < liquidityTokens.length; i++) {
      reserves = reserves.add(
        valueOf(
          liquidityTokens[i],
          IERC20Metadata(liquidityTokens[i]).balanceOf(address(this))
        )
      );
    }
