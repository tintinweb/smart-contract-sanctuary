...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Meat
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that Meat distribution occurs.
        uint256 accMeatPerShare;   // Accumulated Meat per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The Meat TOKEN!
    Meat public immutable token;
    // Dev address.
    address public devaddr;
    // Meat tokens created per block.
    uint256 public MeatPerBlock;
    // Deposit Fee address
    ...
    event UpdateEmissionRate(address indexed user, uint256 MeatPerBlock);
    ...
    event MeatMintError(bytes reason);
    ...
    // Update the given pool's Meat allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingMeat(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accMeatPerShare = pool.accMeatPerShare;
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 MeatReward = multiplier.mul(MeatPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accMeatPerShare = accMeatPerShare.add(MeatReward.mul(1e18).div(pool.lpSupply));
        }
        return user.amount.mul(accMeatPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try token.mint(devaddr, MeatReward.div(10)) {
        } catch (bytes memory reason) {
            MeatReward = 0;
            emit MeatMintError(reason);
        }
        
        try token.mint(address(this), MeatReward) {
        } catch (bytes memory reason) {
            MeatReward = 0;
            emit MeatMintError(reason);
        }
        
        pool.accMeatPerShare = pool.accMeatPerShare.add(MeatReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMeatPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accMeatPerShare).div(1e18);
    ...
    // Safe Meat transfer function, just in case if rounding error causes pool to not have enough ALPHAs.
    function safeMeatTransfer(address _to, uint256 _amount) internal {
        uint256 MeatBal = token.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > MeatBal) {
            transferSuccess = token.transfer(_to, MeatBal);
        } else {
            transferSuccess = token.transfer(_to, _amount);
        }
        require(transferSuccess, "safeMeatTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _MeatPerBlock) external onlyOwner {
        require(_MeatPerBlock <= MAX_EMISSION_RATE, "INCORRECT INPUT");
        massUpdatePools();
        MeatPerBlock = _MeatPerBlock;
        emit UpdateEmissionRate(msg.sender, _MeatPerBlock);
    }
