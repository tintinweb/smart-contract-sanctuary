...
// MasterChef is the master of SCUBs. He can make SCUBs and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once SCUB is sufficiently
// distributed and the community can show to govern itself.

contract ScubMasterchef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SCUB
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accScubPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accScubPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. SCUBs to distribute per second.
        uint256 lastRewardTime;  // Last timestamp that SCUBs distribution occurs.
        uint256 accScubPerShare;   // Accumulated SCUBs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The SCUB TOKEN!
    ScubToken public immutable scub;
    // Dev address.
    address public devaddr;
    // SCUB tokens created per timestamp.
    uint256 public scubPerSecond;
    // Deposit Fee address
    ...
    // The timestamp when SCUB distribution starts.
    uint256 public startTime;
    // Maximum SCUB Per Second
    ...
    event UpdateEmissionRate(address indexed user, uint256 scubPerSecond);
    ...
    event scubMintError(bytes reason);

    constructor(
        ScubToken _tokenAddress,
        address _devaddr,
        address _feeAddress,
        uint256 _scubPerSecond,
        uint256 _startTime
    ) {
        ...
        require(_scubPerSecond <= MAX_EMISSION_RATE, "Emission rate cannot be more than the maximum.");

        scub = _tokenAddress;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        scubPerSecond = _scubPerSecond;
    ...
    // Update the given pool's SCUB allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending SCUBs on frontend.
    function pendingScub(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        
        uint256 accScubPerShare = pool.accScubPerShare;
        
        if (block.timestamp > pool.lastRewardTime && pool.lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 scubReward = multiplier * scubPerSecond * pool.allocPoint / totalAllocPoint;
            accScubPerShare = accScubPerShare + scubReward * 1e18 / pool.lpSupply;
        }
        return user.amount * accScubPerShare / 1e18 - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try scub.mint(devaddr, scubReward / 10) {
        } catch (bytes memory reason) {
            scubReward = 0;
            emit scubMintError(reason);
        }
        
        try scub.mint(address(this), scubReward) {
        } catch (bytes memory reason) {
            scubReward = 0;
            emit scubMintError(reason);
        }
        
        pool.accScubPerShare = pool.accScubPerShare + scubReward * 1e18 / pool.lpSupply;
        pool.lastRewardTime = block.timestamp;
    }
    
    // Deposit LP tokens to MasterChef for SCUB allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount * pool.accScubPerShare / 1e18 - user.rewardDebt;
            if (pending > 0) {
                safeScubTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount * pool.accScubPerShare / 1e18;
    ...
    // Safe Treat transfer function, just in case if rounding error causes pool to not have enough SCUBs.
    function safeScubTransfer(address _to, uint256 _amount) internal {
        uint256 scubBal = scub.balanceOf(address(this));
        bool transferSuccess = false;
        
        if (_amount > scubBal) {
            transferSuccess = scub.transfer(_to, scubBal);
        } else {
            transferSuccess = scub.transfer(_to, _amount);
        }
        
        require(transferSuccess, "safeScubTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _scubPerSecond) external onlyOwner {
        require(_scubPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: INCORRECT INPUT");
        massUpdatePools();
        
        scubPerSecond = _scubPerSecond;
       
        emit UpdateEmissionRate(msg.sender, _scubPerSecond);
    }
