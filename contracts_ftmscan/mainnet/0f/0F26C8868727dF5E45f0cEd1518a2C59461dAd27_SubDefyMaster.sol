...
contract SubDefyMaster is Ownable {
    using SafeMath for uint256;
    ...
    // Reward Token Tax Ratio
    uint256 public taxRatio = 1;
    // Dev address.
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 blockTimestamp = block.timestamp < pool.rewardEndTimestamp ? block.timestamp : pool.rewardEndTimestamp;
            uint256 multiplier =
        ...
        return (user.amount.mul(accDefyPerShare).div(1e18).sub(user.rewardDebt)).mul(taxRatio).div(10000);
    }
    ...
    function withdrawRemainder() public onlyDev {
        ...
        rewardToken.transfer(feeAddress, rewardToken.balanceOf(address(this)));
    }
