...
contract Devilmons is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 DEVILMONS-WFTM LP -> DEVILMONS pool
    uint256 public constant INITIAL_DEVILMONS_POOL_DISTRIBUTION = 140000 ether;
    ...
    address public devilmonsOracle;
    ...
    //modify for check caller is tax office
    modifier onlyTaxOffice() {
        require(taxOffice == msg.sender, "Caller is not the tax office");
        _;
    }
    //modify for check caller is Operator or tax office
    ...
    /**
     * Constructs the DEVILMONS ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("DEVILMONS", "DVM") {
        // Mints 1 DEVILMONS to contract creator for initial pool setup
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        //remove instan of contract 
    ...
    function _getDevilmonsPrice() internal view returns (uint256 _devilmonsPrice) {
        try IOracle(devilmonsOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Devilmons: failed to fetch DEVILMONS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _devilmonsPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_devilmonsPrice >= taxTiersTwaps[tierId]) {
    ...
    function setDevilmonsOracle(address _devilmonsOracle) public onlyOperatorOrTaxOffice {
        require(_devilmonsOracle != address(0), "oracle address cannot be 0 address");
        devilmonsOracle = _devilmonsOracle;
    }
    ...
    /**
     * @notice Operator mints DEVILMONS to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DEVILMONS to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDevilmonsPrice = _getDevilmonsPrice();
            currentTaxRate = _updateTaxRate(currentDevilmonsPrice);
            if (currentDevilmonsPrice < burnThreshold) {
                burnTax = true;
            }
        }
     ...
     */
    function distributeReward(address _genesisPool, address _devilmonsPool, address _airdropWallet) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_devilmonsPool != address(0), "!_devilmonsPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_devilmonsPool, INITIAL_DEVILMONS_POOL_DISTRIBUTION);
