...
contract Vester is ReentrancyGuard {
    ...
    uint totalCycle;
    uint startTime;
    mapping(address => uint) public claimedFTML;
    ...
    )
    {
        ...
        duration = _duration;
        FTMLToken = IERC20(_FTMLToken);
        vestingWallet = _vestingWallet;
        ico = ICOSale(_contract);
    }

    function claim(address _user, uint _amount) external nonReentrant {
        require(block.timestamp > startTime,
        "Vester: Vesting has not started"
        );
        require(claimableFTMLCurrentCycle(_user) >= _amount,
        "Vester: you cannot claim more than you should"
        );

        claimedFTML[_user] +=  _amount;
        
        FTMLToken.safeTransferFrom(vestingWallet, _user, _amount);
    }
    
    function currentCycle() public view returns (uint) {
        uint current = (block.timestamp - startTime) / duration;
        current += 1;

        return current;
    }

    function getFTMLTotalToReceive(address _user) public view returns (uint) {
        uint ETHdeposited = getBalanceDeposited(_user);
        uint FTMLreceived = ETHdeposited * ico.getRate();
        uint FTMLTotalToReceive = FTMLreceived * 100 / 15;
        
        return FTMLTotalToReceive;
    }

    function getFTMLTotalToBeVested(address _user) public view returns (uint) {
        uint FTMLToBeVested = getFTMLTotalToReceive(_user) * 85 / 100;

        return FTMLToBeVested;
    }

    function claimableFTMLCurrentCycle(address _user) public view returns (uint) {
        uint FTMLToBeVested = getFTMLTotalToBeVested(_user);
        uint alreadyClaimed = claimedFTML[_user];
        uint amountThisCycle = (FTMLToBeVested * currentCycle() / totalCycle)- alreadyClaimed;

        return amountThisCycle;
    }

    function getBalanceDeposited(address _user) internal view returns (uint) {
        uint amount = ico.balanceDeposited(_user);
