...
contract mcDividendDistributor {
...
contract mc is IBEP20, Auth {
    ...
    uint256 liquidityFee = 200; // 2% to liquidity pool
    uint256 buybackFee = 0;
    uint256 reflectionFee = 800; // 8% to holders
    uint256 marketingFee = 800; // 8% to marketing
    ...
    mcDividendDistributor distributor;
    ...
    constructor (address distributorAddress) Auth(msg.sender) {
        ...
        distributor = mcDividendDistributor(distributorAddress);
        router = IUniswapV2Router(_router); // connect to router contract
