...
contract Aod is BEP20 {
    ...
    uint16 public constant maxTaxRate = 1500;
    
    // Fee address
    address payable public constant feeAddr = 0x9E167a4E9c66a57c8cc4298c99f409b0ccE4c768;
    ...
    // Min amount to liquify. (default 10 Aods)
    ...
    // The operator can call all functions except of minting Aod
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "AOD::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the AodSwapToken contract.
     */
    constructor() public BEP20("Aod", "AOD") {
    ...
    /// @dev overrides transfer function to meet tokenomics of AOD
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "AOD::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "AOD::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the AOD -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "AOD::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "AOD::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "AOD::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "AOD::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "AOD::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "AOD::transferOperator: new operator is the zero address");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. AODs to distribute per second.
        uint256 lastRewardSecond; // Last second that AODs distribution occurs.
        uint256 accAodPerShare; // Accumulated AODs per share, times 1e18. See below.
    ...
    // The block timestamp when Aod mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 AodPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 aodReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
                uint256 devReward = aodReward.div(10);
                uint256 totalRewards = aod.totalSupply().add(devReward).add(
                aodReward
            );
                if (totalRewards > aod.maxSupply()) {
                aodReward = aod.maxSupply().sub(aod.totalSupply());
            }
            accAodPerShare = accAodPerShare.add(
                aodReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accAodPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= aod.maxSupply()) {
            ...
            // mint the difference only to MC, update aodReward
            aodReward = aod.maxSupply().sub(aod.totalSupply());
            aod.mint(address(this), aodReward);
        }

        if (aodReward != 0) {
            // only calculate and update if aodReward is non 0
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accAodPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accAodPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeAodTransfer(msg.sender, pending);
    ...
    function safeAodTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > aodBal) {
            transferSuccess = aod.transfer(_to, aodBal);
        } else {
            transferSuccess = aod.transfer(_to, _amount);
        }
        require(transferSuccess, "safeAodTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _AodPerSecond) external onlyOwner {
        require(_AodPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        AodPerSecond = _AodPerSecond;
        emit UpdateEmissionRate(msg.sender, _AodPerSecond);
    }
