...
// File: contracts/YetiToken.sol

pragma solidity 0.6.12;

contract YetiToken is ERC20("Yeti Pirate", "YETI"), Ownable {
    uint256 constant MAX_CAP_SUPPLY = 8000 ether;
...
// MasterChef is the master of YetiPirate. He can make YetiPirate and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once YetiPirate is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accYetiPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accYetiPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accYetiPerShare; // Accumulated FISHes per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The YETI TOKEN!
    YetiToken public yetiToken;
    address public devAddress;
    address public feeAddress;

    // YETI tokens created per block.
    uint256 public yetiPerBlock = 0.05 ether;
    ...
    // The block number when YETI mining starts.
    uint256 public startBlock;

    uint256 constant MAX_YETI_SUPPLY = 88888 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 yetiPerBlock);


    constructor(
        YetiToken _yetiToken,
        uint256 _startBlock,
        address _devAddress,
        address _feeAddress
    ) public {
    ...
    // Update the given pool's YETI allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP
    ...
    function pendingYeti(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accYetiPerShare = pool.accYetiPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 yetiReward = multiplier
                .mul(yetiPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accYetiPerShare = accYetiPerShare.add(
                yetiReward.mul(1e18).div(lpSupply)
            );
        }
        return user.amount.mul(accYetiPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(yetiToken.owner() != address(this)) {
        ...
        uint256 currentSupply = yetiToken.totalSupply();

        if (currentSupply >= MAX_YETI_SUPPLY) {
            pool.lastRewardBlock = block.number;
            return;
        }
        ...
        uint256 devReward = yetiReward.div(10);
        yetiToken.mint(devAddress, devReward);
        yetiToken.mint(address(this), yetiReward);
        pool.accYetiPerShare = pool.accYetiPerShare.add(
            yetiReward.mul(1e18).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for YETI allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount
    ) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accYetiPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                uint256 devReward = pending.div(10);
                safeYetiTransfer(msg.sender, pending.sub(devReward));
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accYetiPerShare).div(1e18);
    ...
    function _withdraw(
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accYetiPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            uint256 devReward = pending.div(10);
            safeYetiTransfer(_user, pending);
        }
        if (_amount > 0) {
    ...
    // Safe yetiToken transfer function, just in case if rounding error causes pool to not have enough YETI.
    function safeYetiTransfer(address _to, uint256 _amount) internal {
        uint256 yetiBal = yetiToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > yetiBal) {
            transferSuccess = yetiToken.transfer(_to, yetiBal);
        } else {
            transferSuccess = yetiToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeYetiTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _yetiTokenPerBlock)
        external
        onlyOwner
    {
        massUpdatePools();
        yetiPerBlock = _yetiTokenPerBlock;
        emit UpdateEmissionRate(msg.sender, _yetiTokenPerBlock);
    }
    ...
    function transferYetiTokenOwnership(address _newOwner)
        external
        onlyOwner
    {
        yetiToken.transferOwnership(_newOwner);
    }
}