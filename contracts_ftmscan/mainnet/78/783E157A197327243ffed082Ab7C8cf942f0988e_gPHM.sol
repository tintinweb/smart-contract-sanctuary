...
contract gPHM is IgPHM, ERC20, PhantomStorageMixin {
    ...
    function _afterTokenTransfer(
        ...
        require(transfersFrozen == true && (from != address(0) || to != address(0)), "gPHM is not transferable");
        if (transfersFrozen == false && (to != address(PhantomVault()) || to != address(0))) {
            require(
        ...
        unchecked {
            ...
            if (nCheckpoints > 0 && checkpoints[delegatee][nCheckpoints - 1].fromTimestamp == block.timestamp) {
                ...
                checkpoints[delegatee][nCheckpoints] = Checkpoint(
                    safeCastTo32(block.timestamp),
                    safeCastTo96(newVotes)
                );
