...
contract MapleVault is ReentrancyGuard, VaultNFTv4 {
    ...
    IERC20 public musd;
    IERC20Metadata private musdInfo;
    ...
    function calculateCollateralProperties(uint256 _collateral, uint256 _debt) private view returns (uint256, uint256) {
        ...
        uint256 collateralValue = _collateral.mul(getEthPriceSource()).mul(10**(uint256(musdInfo.decimals()).sub(uint256(collateralInfo.decimals()))));
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external {
        require(musd.balanceOf(msg.sender) >= amount, "Token balance too low");
    ...
    function liquidateVault(uint256 vaultID) external {
        ...
        require(musd.balanceOf(msg.sender) >= halfDebt, "Token balance too low to pay off outstanding debt");

        //musd
        musd.safeTransferFrom(msg.sender, address(this), halfDebt);
...
contract MapleBank is MapleVault, Ownable {
    ...
    //Decreases the MUSD balance by the specified amount in the bank to control MUSD supply. 
    function decreaseDebtCeiling(uint256 _amount) public onlyOwner() {
        musd.safeTransferFrom(address(this), address(musd), _amount);
    }
