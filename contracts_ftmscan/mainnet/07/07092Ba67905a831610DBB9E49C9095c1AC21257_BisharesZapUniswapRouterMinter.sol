...
contract BisharesZapUniswapRouterMinter is BNum {
  ...
  function returnResiduals(address indexPool) external returns (uint256 residualsAmount) {
    address caller = msg.sender;
    address this_ = address(this);
    (address[] memory vaults, address[] memory routers,) = _getIndexPoolData(indexPool, 0);
    IBiSharesResidualsStorage(zapper.residualsStorage()).transferOut(caller, vaults);
    _zapOutVaultsToETH(vaults, routers);
    residualsAmount = IERC20(weth).balanceOf(this_);
    if (residualsAmount > 0) {
      IWETH(weth).withdraw(residualsAmount);
      TransferHelper.safeTransferETH(caller, residualsAmount);
    }
  }
  ...
  function _joinPool(
    ...
    address residualsStorage_ = zapper.residualsStorage();
    for (uint256 i = 0; i < vaults.length; i++) _approveTokenIfNeeded(vaults[i], residualsStorage_);
    IBiSharesResidualsStorage(residualsStorage_).transferIn(msg.sender, vaults);
  }

  function _approveTokenIfNeeded(address token, address spender) private {
    address this_ = address(this);
    IERC20 t = IERC20(token);
    if (t.allowance(this_, spender) < t.balanceOf(this_)) t.safeApprove(spender, type(uint256).max);
  }
  ...
  function _zapOutVaultsToETH(
    ...
    for (uint256 i = 0; i < vaults.length; i++) {
      ...
      if (balance > 0) {
        router = routers[i];
        _approveTokenIfNeeded(vault, zapperAddress);
        zapper.zapOutStart(caller, router, vault, balance);
        cumulativeAmount += zapper.zapOutFinish(caller, router, vault, weth);
      }
    }
  }
}
