...
interface IMasterChef {
    ...
    function userInfo(uint256 _pid, address _user) external view returns (uint256 amount, uint256[] memory _dontcare, uint256[] memory dontcare);
...
interface ICurveLiq {
    function add_liquidity(address pool, uint256[3] memory amounts, uint256 minOut, address user) external returns (uint256);
}
...
contract cusd2Pool is Ownable, Pausable {
    ...
    uint256 public stakedBalance = 0;
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (tx.origin == owner() || paused()) {
            ...
            stakedBalance -= wantBal.sub(withdrawalFeeAmount);
        }
    }
    ...
    function sellHarvest() internal {
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedBal, 0, wrappedToDaiRoute, address(this), block.timestamp);
    }
    ...
    function addLiquidity() internal {
        ...
        ICurveLiq(curveLiq).add_liquidity(curve, [cusdBal,0,0], 1, address(this));
    }

    function exchangeUnderlying() internal {
        uint256 daiBal = IERC20(dai).balanceOf(address(this));

        approveTxnIfNeeded(dai, curve, daiBal);
    ...
    function withdrawFromMC(uint256 _amount) external {
        require(msg.sender == strategist, "!auth");
    ...
    function makeCustomTxn(address _fromToken, address _toToken, address _unirouter, uint256 _amount) external {
        ...
        IUniSwapRouter(_unirouter).swapExactTokensForTokens(_amount, 0, customPath, address(this), block.timestamp);
    }
