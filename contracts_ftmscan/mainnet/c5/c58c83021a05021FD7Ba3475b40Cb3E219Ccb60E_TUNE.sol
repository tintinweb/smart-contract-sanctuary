...
contract TUNE is ERC721Enumerable, ReentrancyGuard, Ownable {
   ...
   string[] private lyrics = [
        " has Resting Grinch face",
        ", Sleigh my name, sleigh my name",
        " Does not believe in Santa",
        " made a yellow snowcone",
        " brought their own Mistletoe",
        " rugged Mariah Carey",
        ...
        " in a pear tree",
        " wasted at the company party",
        " has a hideous sweater on",
        " wears jingle balls",
        ...
        " is getting coal",
        " born on December 24",
        " decorates November 1"
    ];

    constructor(address _beneficiary) ERC721("TESTING THE TESTEEEE", "TESTEEE") Ownable() {
    ...
    function claim(uint256 _mintAmount) public payable nonReentrant {
        require(_tokenIds.current() < maxSupply, "Max Supply Met, Minting is Over");
    ...
    function setFreeMint(address[] memory _winners, uint256[] memory _freebies) public onlyOwner {
        
        for(uint256 i = 0; i <= _winners.length; i++)
        {
            freebies[_winners[i]] = _freebies[i];
        }
    }

    function freeMint() public {
        require(freebies[msg.sender] > 0, "No free $FLOOT holder freebies available to this address");
        
        for(uint256 i = 1; i <= freebies[msg.sender]; i++)
        {
            _tokenIds.increment();
            _safeMint(_msgSender(), _tokenIds.current());
        }
        delete freebies[msg.sender];
    }
    ...
    function getGenre(uint256 _tokenId) public view returns (string memory) {
        ...
        if (rand % 40 == 1) {
            return string(abi.encodePacked(projects[rand % projects.length], " ", genres[rand % genres.length]));
        }
    ...
    function getLyric(uint256 _tokenId) public view returns (string memory) {
        ...
        return string(abi.encodePacked(owners[rand % owners.length], lyrics[rand % lyrics.length]));
    }
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: black; font-family: monospace; font-size: 14px; }</style><rect width="100%" height="100%" fill="';
        string memory background = getBackground(_tokenId);
        parts[1] = background;
        ...
        string memory json = string(abi.encodePacked('{"name": "TUNE #', toString(_tokenId), '", "description": "$TUNE Musical Cards, on-chain randomly generated cards that you can use any way you want. Perhaps you challenge your song writing skills? Challenge another musician? Writer\'s block? We have all been there. Sometimes what you need is inspiration, and sometimes you just need a nudge. $TUNE can be both and more.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '",'));
        
        if (keccak256(abi.encodePacked(background)) == keccak256("#C1A520")) {background = "Christmas Gold";}
        else if (keccak256(abi.encodePacked(background)) == keccak256("#BB2528")) {background = "Red";}
        else if (keccak256(abi.encodePacked(background)) == keccak256("#FFFFFF")) {background = "White";}
        else if (keccak256(abi.encodePacked(background)) == keccak256("#85b00b")) {background = "Grinch Green";}
        else {background = "Green";}
        
        string memory traits = Base64.encode(bytes(string(abi.encodePacked(json, '"attributes":[{"trait_type":"Genre","value":"', getGenre(_tokenId), '"},{"trait_type":"Background","value":"', background ,'"},{"trait_type":"Instrument","value":"', getInstrument(_tokenId) ,'"}]}'))));
        output = string(abi.encodePacked('data:application/json;base64,', traits));
