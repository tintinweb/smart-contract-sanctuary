...
contract Candydrop is Ownable, ReentrancyGuard {
    ...
    event NewRewardPerTime(uint256 rewardPerTime);
     ...
     * @param _rewardPerTime: reward per block (in rewardToken)
     * @param _startTime: start block
     * @param _bonusEndTime: end block
     */
    function initialize(
        IERC20 _stakedToken,
        IERC20 _rewardToken,//Decimal should be at least 12 to act normally
        uint256 _amount,
        uint256 _startTime,
        ...
        rewardPerTime = _amount.div(_bonusEndTime-_startTime);
     ...
     * @dev Only callable by owner.
     * @param _rewardPerTime: the reward per block
     */
    function updateRewardPerTime(uint256 _rewardPerTime) external onlyOwner {
        rewardPerTime = _rewardPerTime;
        emit NewRewardPerTime(_rewardPerTime);
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTime && stakedTokenSupply != 0) {
            ...
            uint256 cakeReward = multiplier.mul(rewardPerTime);
