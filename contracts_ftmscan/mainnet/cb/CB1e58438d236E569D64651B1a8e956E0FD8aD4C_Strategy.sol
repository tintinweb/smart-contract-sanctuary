...
contract Strategy is BaseStrategy {
	...
	IERC20 public rewardToken = IERC20(0x4cdF39285D7Ca8eB3f090fDA0C069ba5F4145B37); // TShares
	IUniPair public lpToken = IUniPair(0x2A651563C9d3Af67aE0388a5c8F89b867038089e); // SpookySwap: TOMB-WFTM LP
	address public lpToken0 = wftm;
	address public lpToken1 = address(0x6c021Ae822BEa943b2E66552bDe1D2696a53fbB7);
	...
	function balanceOfLpToken1() public view returns (uint256 _amount) {
		return IERC20(lpToken1).balanceOf(address(this));
	}

	function balanceOfLpTokens() public view returns (uint256 _amount) {
	...
	function balanceOfPooled() public view returns (uint256 _amount) {
		...
		uint256 ftmPerTOMB = IUni(router).getAmountOut(1000, reserveB, reserveA);
		uint256 totalReservesInFTM = ftmPerTOMB.mul(reserveB).div(1000) + reserveA;

		uint256 strategyLpBalance = balanceOfLPInMasterChef().add(balanceOfLpTokens());

		uint256 lpBalancePortion = strategyLpBalance.mul(1e18).div(totalSupply);
		uint256 totalWantPooled = lpBalancePortion.mul(totalReservesInFTM).div(1e18);
	...
	function liquidateAllPositions() internal override returns (uint256 liquidated) {
		...
		enforceSlippageOut(eta, liquidated);
	 ...
	 */
	function _swap(uint256 _fullInvestment) internal returns (uint256[] memory swapedAmounts) {
		(uint256 reserveA, uint256 reserveB, ) = lpToken.getReserves();
		uint256 swapAmountIn = getSwapAmount(_fullInvestment, reserveA, reserveB);
		uint256 swapAmountOut = IUni(router).getAmountOut(swapAmountIn, reserveA, reserveB);
		swapedAmounts = IUni(router).swapExactTokensForTokens(
			swapAmountIn,
			swapAmountOut,
			lpPairPath,
			address(this),
	...
	function sellTombTokens() internal {
		uint256 lpToken1Balance = IERC20(lpPairPath[1]).balanceOf(address(this));
		(uint256 reserveA, uint256 reserveB, ) = lpToken.getReserves();
		uint256 swapAmountOut = IUni(router).getAmountOut(lpToken1Balance, reserveB, reserveA);
	...
	function removeLiquidity(uint256 toExitAmount) internal {
		...
		uint256 TombPerFTM = IUni(router).getAmountOut(1000, reserveB, reserveA);
		uint256 totalReservesInFTM = TombPerFTM.mul(reserveB).div(1000).add(reserveA);
	...
	function getSwapAmount(
		...
		uint256 nominator = IUni(router).getAmountOut(halfInvestment, reserveA, reserveB);
		uint256 denominator =
			IUni(router).quote(halfInvestment, reserveA.add(halfInvestment), reserveB.sub(nominator));
