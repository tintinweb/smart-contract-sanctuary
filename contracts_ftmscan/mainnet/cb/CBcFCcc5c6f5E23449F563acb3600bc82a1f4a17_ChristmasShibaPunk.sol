...
contract ChristmasShibaPunk is ERC721, ERC721URIStorage, ERC2981Collection, IMAX721, ERC165Storage, Developer, Ownable {
  using Counters for Counters.Counter;
  Counters.Counter private tokenIdCounter;
  Counters.Counter private badMintCounter;
  mapping(uint256 => bool) claimedFreebie;
  ...
  event UpdatedOriginalContract(address _old, address _new);
  ...
  constructor() ERC721("Christmas ShibaPunk", "XMASSHIBA") {
 ...
 */
  ...
  function publicMint(uint256[] memory ids) public {
    require(enableMinter, "Minter not active");
    require(tokenIdCounter.current() + ids.length <= mintSize, "Can not mint that many");
    for (uint i = 0; i < ids.length; i++) {
      bool addressCheck = IERC721(originalContract).ownerOf(ids[i]) == msg.sender;
      if(!claimedFreebie[ids[i]] && addressCheck) {
        uint value = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, ids[i], tokenIdCounter.current(), block.difficulty))) % 5;
        _safeMint(msg.sender, tokenIdCounter.current());
        _setTokenURI(tokenIdCounter.current(), _uint2str(value));
        tokenIdCounter.increment();
        claimedFreebie[ids[i]] = true;
      } else {
        badMintCounter.increment();
      }
    }
  }
 ...
 */
  ...
  // @notice will set old contract address for free mint by onlyDev
  function setOriginalContractAddress(address _contract) public onlyDev {
    address old = originalContract;
    originalContract = _contract;
    emit UpdatedOriginalContract(old, originalContract);
  }
  ...
  // @notice will return the "mis-mints" count
  function minterCurrentAttemptedDoubleMints() external view returns (uint256) {
    return badMintCounter.current();
  }

  // @notice will return Orginal Contract address
  function minterOriginalContractAddress() external view returns (address) {
    return originalContract;
  }

  // @notice will return if claimed or not
  function beenClaimed(uint256 _id) external view returns (bool) {
    return claimedFreebie[_id];
  }

  // @notice needed but not used
