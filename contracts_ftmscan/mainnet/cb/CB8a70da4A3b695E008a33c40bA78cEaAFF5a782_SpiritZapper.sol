...
contract SpiritZapper is ZapInBaseV3 {
    ...
    mapping(address => mapping(address => address)) private tokenBridgeForRouter; // Sipirt Zapper Code 
    ...
    function estimateZapInToken(address _from, address _to, address _router, uint _amt) public view returns (uint256, uint256) {
        ...
        if (_from == IUniswapV2Pair(_to).token0() || _from == IUniswapV2Pair(_to).token1()) { // check if we already have one of the assets
            ...
            uint nativeAmount = _from == wethTokenAddress ? _amt : _estimateSwap(_from, _amt, wethTokenAddress, _router);
            if (wethTokenAddress == IUniswapV2Pair(_to).token0()) {
                return (nativeAmount.div(2), _estimateSwap(wethTokenAddress, nativeAmount.div(2), IUniswapV2Pair(_to).token1(), _router ));
            }
            if (wethTokenAddress == IUniswapV2Pair(_to).token1()) {
                return (_estimateSwap(wethTokenAddress, nativeAmount.div(2), IUniswapV2Pair(_to).token0(), _router ), nativeAmount.div(2));
            }
                return (_estimateSwap(wethTokenAddress, nativeAmount.div(2), IUniswapV2Pair(_to).token0(), _router ), _estimateSwap(wethTokenAddress, nativeAmount.div(2), IUniswapV2Pair(_to).token1(), _router));
        }
    }

    function _estimateSwap(address _from, uint amount, address _to, address routerAddr) private view returns (uint) {
        // IUniswapV2Router01 router = IUniswapV2Router01(routerAddr); // Removed this becuase router is set in advance 
        ...
        if (fromBridge != address(0) && toBridge != address(0)) {
            ...
            if (_from == toBridge) {
                ...
                path[0] = _from;
                path[1] = _to;
            } else if (_from == wethTokenAddress) {
                path = new address[](3);
        ...
        } else if (_from == wethTokenAddress || _to == wethTokenAddress) {
            path = new address[](2);
            path[0] = _from;
            path[1] = _to;
        } else {
            // Go through wethTokenAddress
            path = new address[](3);
            path[0] = _from;
            path[1] = wethTokenAddress;
            path[2] = _to;
