...
contract AdvancedEarningPool is Ownable {
    using SafeMath for uint256;
    ...
    // Info of each pool.
    struct PoolInfo {
        IBEP20 stakeToken;          // Address of staked token contract.
    ...
    address public stakeToken = 0x53a5F9d5adC34288B2BFF77d27F55CbC297dF2B9;
    address public rewardToken = 0xb65cdd027ac9799892E0FC4912a3447e211eb167;
    
    // RewardTokenBalance
    uint256 public rewardTokenBalance;
    
    // Rewarded days
    uint256 rewardedDays = 15;
    
     // Blocks per day
    uint256 blocksPerDay = 96000;
    
    // rewardToken created per block.
    uint256 public rewardPerBlock = 0;
    
    // rewardToken created per Day.
    ...
    uint16 public depositFeetoFee = 0;
    ...
    uint256 public minStake = 50 *10**18;

    event Deposit(address indexed user, uint256 amount);
    event Withdraw(address indexed user, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 amount);
    event SetstakeTokenAddress(address indexed user, address indexed stakeToken);
    event SetrewardTokenAddress(address indexed user, address indexed rewardToken);
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && stakeTokenSupply != 0) {
            ...
            accStakeTokenPerShare = accStakeTokenPerShare.add(stakeTokenReward.mul(1e12).div(stakeTokenSupply));
        }
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 stakeTokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accStakeTokenPerShare = pool.accStakeTokenPerShare.add(stakeTokenReward.mul(1e12).div(stakeTokenSupply));
        pool.lastRewardBlock = block.number;
        pool.stakeToken = IBEP20(stakeToken);
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if(user.amount + _amount > minStake) {

            uint256 before = pool.stakeToken.balanceOf(address(this));
            pool.stakeToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            uint256 _after = pool.stakeToken.balanceOf(address(this));
            ...
            if(pool.depositFee > 0){
                ...
                pool.stakeToken.safeTransfer(feeAddress, depositFeeAmount);
    ...
    // Update reward per block to be distributed within rewardedDays
    function updateLogicRewardPerBlock() public {
        uint256 _rewardTokenBalance = IBEP20(rewardToken).balanceOf(address(this));
        rewardTokenBalance = _rewardTokenBalance;
        uint256 _rewardPerBlock = _rewardTokenBalance.div(rewardedDays).div(blocksPerDay);
        rewardPerBlock = _rewardPerBlock;
        rewardPerDay = _rewardPerBlock.mul(blocksPerDay);
        updatePool();        
    } 
    ...
    // update rewarded Days and blocks per Day (depending on average Blocktime specific on each chain)
    function updateDaysblocksperDay (uint256 _rewardedDays, uint256 _blocksPerDay) public onlyOwner {
    ...
    function updateBonusEndBlock(uint256 _bonusEndBlock) public onlyOwner {
      ...
      //  function to update the minimum stakung amount.
    ...
    function updateDepositFee(uint16 _depositFee) public onlyOwner {
        require(_depositFee <= 500, "updateDepositFee: invalid deposit fee");
        poolInfo[0].depositFee = _depositFee;
    ...
    poolInfo[0].stakeToken = IBEP20(stakeToken);
    updatePool();
    emit SetstakeTokenAddress(msg.sender, _stakeToken);
    }
    ...
    emit SetrewardTokenAddress(msg.sender, _rewardToken);
    }
    
    // Pay out any token from Token Contract
    function payoutToken(address token, uint256 amount) public onlyOwner {
