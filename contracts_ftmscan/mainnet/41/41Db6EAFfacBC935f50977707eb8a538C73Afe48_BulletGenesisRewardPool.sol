...
contract BulletGenesisRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BULLET to distribute.
        uint256 lastRewardTime; // Last time that BULLET distribution occurs.
        uint256 accTombPerShare; // Accumulated BULLET per share, times 1e18. See below.
    ...
    // The time when BULLET mining ends.
    uint256 public poolEndTime;

    // TESTNET
    uint256 public tombPerSecond = 3.0555555 ether; // 11000 BULLET / (1h * 60min * 60s)
    uint256 public runningTime = 48 hours; // 1 hours
    uint256 public constant TOTAL_REWARDS = 11000 ether;
    // END TESTNET

    // MAINNET
    // uint256 public bulletpersecond = 0.11574 ether; // 10000 BULLET / (24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "BulletGenesisPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "BulletGenesisPool: existing pool?");
        }
    }
    ...
    // View function to see pending BULLET on frontend.
    ...
    // Safe BULLET transfer function, just in case if rounding error causes pool to not have enough BULLETs.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (BULLET or lps) if less than 90 days after pool ends
