...
/**
 *Submitted by Cheynespc of DeGen Haus. Lets make this Degen Haus, the best Haus on the Block https://degen.haus
...
interface IDegenswapRouter02 is IDegenswapRouter01 {
...
library DegenswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'db8c51e7912212fc3e9160b61f1b0ce18d5a5865b220d792f6d0bdcb330cfe09' // init code hash
            ))));
    }
//3125d0a15fa7af49ce234ba1cf5f931bad0504242e0e1ee9fcd7d1d7aa88c651
//db8c51e7912212fc3e9160b61f1b0ce18d5a5865b220d792f6d0bdcb330cfe09
...
contract DegenswapRouter is IDegenswapRouter02 {
    using SafeMath for uint;
    ...
    function _addLiquidity(
        ...
        if (IDegenswapFactory(factory).getPair(tokenA, tokenB) == address(0)) {
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DegenswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = DegenswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DegenswapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(DegenswapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'DegenswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    {
        return DegenswapLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    {
        return DegenswapLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
