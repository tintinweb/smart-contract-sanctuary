...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 constant MAXSUPPLY = 8250 ether;
...
contract DrcToken is BEP20("Drc Cave", "CAVE") {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 drcReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        drc.mint(devaddr, drcReward.div(10));
    ...
    function safeDrcTransfer(address _to, uint256 _amount) internal {
        uint256 drcBal = drc.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > drcBal) {
            transferSuccess = drc.transfer(_to, drcBal);
        } else {
            transferSuccess = drc.transfer(_to, _amount);
        }
    ...
    function updateEmissionRate(uint256 _DrcPerBlock) external onlyOwner {
        require(_DrcPerBlock <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        DrcPerBlock = _DrcPerBlock;
        emit UpdateEmissionRate(msg.sender, _DrcPerBlock);
    }
