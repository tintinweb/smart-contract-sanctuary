    ...
    OwnableUpgradeable
{
    ...
    address public sponsorshipToken;
    ...
    function setSponsorshipToken(address token) external onlyOwner {
        sponsorshipToken = token;
    }

    function depositSponsor(
        address user,
        uint256 amount,
        address referrer
    ) public nonReentrant {
        IERC20Upgradeable(sponsorshipToken).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );
        // deposit to PodTogether prize pool
        IERC20Upgradeable(underlying).safeApprove(address(prizePool), amount);
        prizePool.depositTo(user, amount, sponsorshipToken, referrer);
    }
