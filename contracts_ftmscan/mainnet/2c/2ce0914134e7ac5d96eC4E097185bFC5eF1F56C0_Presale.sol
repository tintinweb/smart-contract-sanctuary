...
contract Presale is ReentrancyGuard {
    ...
    // Price of presale SKULL, 5 USDC
    uint256 public USDCPerToken = 5;
    // Amount of USDC received in presale
    uint256 public USDCReceived = 0;
    // Amount of USDC received in presale
    uint256 public CAP = 50000;
    // USDC token
    IBEP20 USDC;
    ...
    function buy(uint256 _amount, address beneficiary) public nonReentrant {
        ...
        uint256 amount = _amount.mul(10**12);
        uint256 tokens = amount.div(USDCPerToken);

        require(
            USDCReceived + amount <= 50000 ether,
    ...
    function getSKULLTokensLeft() external view returns (uint256) {
        return SKULL.balanceOf(address(this));
    }
    ...
    function claimTokens(address claimer) external {
        ...
        require(
            ...
            "User should own some SKULL tokens"
        );
        require(
            tokensUnclaimed[msg.sender] > 0,
            "User should have unclaimed SKULL tokens"
        );
        require(
            SKULL.balanceOf(address(this)) >= tokensUnclaimed[msg.sender],
            "There are not enough SKULL tokens to transfer."
        );

        SKULL.safeTransfer(msg.sender, tokensUnclaimed[msg.sender]);
    ...
    function withdrawUnsoldSKULL() external onlyOwner {
        uint256 amount = SKULL.balanceOf(address(this)) - totalTokensSold;
        SKULL.safeTransfer(msg.sender, amount);
    }

    function withdrawAllSKULL() external onlyOwner {
        SKULL.safeTransfer(msg.sender, SKULL.balanceOf(address(this)));
    }
}