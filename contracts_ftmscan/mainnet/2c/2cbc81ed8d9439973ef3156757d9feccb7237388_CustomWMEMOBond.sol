...
interface ICustomS {
    function scv() external view returns(uint256);
    function addScv(uint256 _scv) external;
    function rebase() external returns(bool);
}

contract CustomWMEMOBond is Ownable {
    ...
    constructor(
        ...
        require(_customS != address(0) );
        ics = ICustomS(_customS);
    ...
    enum PARAMETER { VESTING, PAYOUT, DEBT, MINPRICE, IDEBT }
    /**
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        ...
        } else if ( _parameter == PARAMETER.IDEBT ) { // 4
            totalDebt = _input;
        }
    }
    ...
    /**
     *  @notice change address of CustomS
     *  @param _customS address
     */
    function changeCustomS(address _customS) external onlyPolicy() {
        require(_customS != address(0) );
        ics = ICustomS(_customS);
    }
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        ...
        require( payout >= 10 ** payoutToken.decimals() / 10000, "Bond too small" ); 
    ...
    function maxPayout() public view returns ( uint ) {
        return ics.scv().mul( terms.maxPayout ).div( 100000 );
    }
     ...
     *  @notice protocols using Wonderland should be careful when quickly adding large %s to total supply
