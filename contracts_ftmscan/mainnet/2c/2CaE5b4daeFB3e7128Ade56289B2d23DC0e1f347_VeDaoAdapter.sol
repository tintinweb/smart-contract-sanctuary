...
contract VeDaoAdapter is IAdapter {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ISpiritFarm constant SpiritFarm = ISpiritFarm(0xE04C26444d37fE103B9cc8033c99b09D47056f51);
    IERC20 constant weth = IERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    IERC20 constant MIM = IERC20(0x82f0B8B456c1A451378467398982d4834b6829c1);
    IERC20 constant spirit = IERC20(0x911da02C1232A3c3E1418B834A311921143B04d7);
    IUniswapV2Router02 constant spiritRouter = IUniswapV2Router02(0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52);
    IUniswapV2Factory constant spiritFactory = IUniswapV2Factory(0xEF45d134b73241eDa7703fa787148D9C9F4950b0);
    ...
    function _zapAny(address sellToken, uint256 poolId, uint256 amount, uint256 min_out, address to) external override returns(uint256) {
        (address lpAddress,,,) = SpiritFarm.poolInfo(poolId);
        IERC20(sellToken).safeIncreaseAllowance(address(spiritRouter), amount);
        if (sellToken == address(spirit) && poolId!=6) {
            address[] memory path = new address[](2);
                    path[0] = address(sellToken);
                    path[1] = address(MIM);
                    spiritRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, address(this), block.timestamp + 60);
            sellToken = address(MIM);
            amount = MIM.balanceOf(address(this));
            IERC20(sellToken).safeIncreaseAllowance(address(spiritRouter), amount);
        }
        IERC20 buffer = weth;
        if (poolId != 6) {
            if (address(buffer)!=lpAddress){
                if (address(buffer) == sellToken) {
                    address[] memory path = new address[](2);
                    path[0] = address(buffer);
                    path[1] = address(lpAddress);
                    spiritRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, address(this), block.timestamp + 60);
                }
                else {
                    address[] memory path = new address[](3);
                    path[0] = sellToken;
                    path[1] = address(buffer);
                    path[2] = address(lpAddress);
                    spiritRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, address(this), block.timestamp + 60);
                }
            }
            else{if (address(buffer) != sellToken) {
                    address[] memory path = new address[](2);
                    path[0] = address(sellToken);
                    path[1] = address(buffer);
                    spiritRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, address(this), block.timestamp + 60);
            ...
            uint256 amountOut = IERC20(lpAddress).balanceOf(address(this));
            IERC20(lpAddress).safeTransfer(to, amountOut);
        ...
        IERC20(token0).safeIncreaseAllowance(address(spiritRouter),token0Amt);
        IERC20(token1).safeIncreaseAllowance(address(spiritRouter),token1Amt);

        spiritRouter.addLiquidity(
            token0,
            token1,
            token0Amt,
            token1Amt,
