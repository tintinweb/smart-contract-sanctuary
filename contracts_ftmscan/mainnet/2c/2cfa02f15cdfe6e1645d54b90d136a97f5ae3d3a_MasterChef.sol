...
// Looking for us?Our Telegram Group: https://t.me/candyfin
...
contract CandyToken is ERC20("Candy Token", "CANDY") {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CANDY (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Candy
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. CANDY to distribute per block.
        uint256 lastRewardBlock;  // Last block number that CANDY distribution occurs.
        uint256 accCandyPerShare;   // Accumulated CANDY per share, times 1e18. See below.
    ...
    uint256 public candyPerBlock = 0.002 ether;
    ...
    // View function to see pending CANDY on frontend.
    function pendingCandy(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accCandyPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 candyReward = multiplier.mul(candyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        candy.mint(devAddress, candyReward.div(10));
    ...
    // Safe CANDY transfer function, just in case if rounding error causes pool to not have enough.
    ...
    function updateEmissionRate(uint256 _candyPerBlock) external onlyOwner {
        massUpdatePools();
