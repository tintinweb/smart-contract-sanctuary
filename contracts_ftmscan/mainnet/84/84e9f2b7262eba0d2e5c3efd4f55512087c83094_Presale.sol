...
contract Presale is IPreIDOBase, Ownable {
    ...
    uint256 private constant DEF_LOCK = 5 days; // 1 hour;
    uint256 private constant MIN_LOCK = 1 hours; // 5 days;
    ...
    constructor(
        ...
        discountsLock[10] = DEF_LOCK;
        discountsLock[20] = 2 * DEF_LOCK;
        discountsLock[30] = 4 * DEF_LOCK;
    }

    receive() external payable inPresalePeriod {
        int256 price = getPrice();
        _order(msg.value, 18, price, priceFeed.decimals(), 2); // default to 2% discount rate
    }
    ...
    function setNotBeforeBlock(uint256 _notBeforeBlock) public onlyOwner {
        notBeforeBlock = _notBeforeBlock;
    }

    function setNotAfterBlock(uint256 _notAfterBlock) public onlyOwner {
        notAfterBlock = _notAfterBlock;
    }
