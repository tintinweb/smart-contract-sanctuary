...
contract LpAssetStrategyV2 is Ownable, Pausable {
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        if (!migrated) {
            if (block.timestamp > deploymentTime.add(_48hours)) {
                ...
                emit StrategistMigration(migrated, multisig);
            }
        }
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external {
        require (msg.sender == strategist && _path[0] != output && _path[_path.length - 1] == output, "Bad path || !auth");
    ...
    function setOutputToWrappedRoute(address[] calldata _route, address _router) external {
        require(msg.sender == strategist && _route[0] == output && _route[_route.length - 1] == wrapped, "Bad path || !auth");
    ...
    function setWrappedToLp0Route(address[] calldata _route, address _router) external {
        require (msg.sender == strategist && _route[0] == wrapped && _route[_route.length - 1] == lpToken0, "Bad path || !auth");
    ...
    function setWrappedToLp1Route(address[] calldata _route, address _router) external {
        require (msg.sender == strategist && _route[0] == wrapped && _route[_route.length - 1] == lpToken1, "Bad path || !auth");
    ...
    function setFees(uint256 newCallFee, uint256 newStratFee, uint256 newWithdrawFee, uint256 newBuyBackFee, uint256 newInsuranceFee, uint256 newTreasuryFee) external {
        require(msg.sender == strategist && newWithdrawFee <= 10, "Exceeds max || !auth");
