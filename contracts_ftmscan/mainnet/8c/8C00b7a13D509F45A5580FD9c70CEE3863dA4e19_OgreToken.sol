...
abstract contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * e.g. implement autoFantom token fees, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
     ...
     * e.g. set autoFantom allowances for certain subsystems, etc.
     *
     * Emits an {Approval} event.
     *
     * Requirements:
...
// OgreToken
contract OgreToken is ERC20('OGRE', 'OGR'), ReentrancyGuard, Ownable {

    address public constant feeAddress = 0x2554fe74a448750efa63b5bf249C5b20Af18545C;

    uint256 public salePriceE35 = (55) * (10 ** 34);  
 

    uint256 public constant OgreMaximumSupply = 6 * (10 ** 5) * (10 ** 18); 
    ...
    uint256 sevenDaysFantom = oneDayFantom * 7;
    ...
    constructor(uint256 _startTime) {
        ...
        endTime   = _startTime + sevenDaysFantom; 
    ...
    function setstartTime(uint256 _newstartTime) external onlyOwner {
        require(block.timestamp < startTime, "cannot change start time if sale has already started");
        require(block.timestamp < _newstartTime, "cannot set start time in the past");
        startTime = _newstartTime;
        endTime   = _newstartTime + sevenDaysFantom;
    ...
    function setSalePriceE35(uint256 _newSalePriceE35) external onlyOwner {
        require(block.timestamp < startTime , "cannot change price 4 hours before start time");
        require(_newSalePriceE35 >= 1 * (10 ** 34), "new price cannot be below 0.1 Fantom");
