...
contract CollateralReserveV2TEST is Ownable {
  ...
  // // buyback coffin
  function buyBackCoffin(uint256 _amount, uint256 _min) public onlyBuybackManagerOrOwner {
    require(coffin != address(0), 'coffin address error');
    rebalance(wftm, coffin, _amount, _min, true, spookyRouterAddress);
  }

  // buyback cousd
  function buyBackCoUSDwithUSDC(uint256 _amount, uint256 _min)
    public
    onlyBuybackManagerOrOwner
  {
  ...
  function withdraw(uint256 _token_amount, address _token) external {
    ...
    require(tokenSpender[_token] == msg.sender, 'Not approved....');
  ...
  //   function v1Debt() public view returns (uint256) {
  //     uint256 v1debut_amount = IGateV1(gatev1).unclaimed_pool_collateral();
  //     try ICollateralReserveV2Calc(collateralCalc[wftm]).valuation(v1debut_amount) returns (
  //       uint256 v
  //     ) {
  //       return v;
  //     } catch Error(
