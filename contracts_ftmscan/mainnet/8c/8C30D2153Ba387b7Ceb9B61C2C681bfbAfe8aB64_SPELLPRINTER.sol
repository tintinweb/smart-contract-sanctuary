...
// No frills, no bs 2OMB printer token.
...
// Dependency file: contracts/baby/SPELLPRINTERDividendTracker.sol
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function distributeSPELLDividends(uint256 amount) public onlyOwner {
...
contract SPELLPRINTERDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SPELLPRINTER contract."
        );
    }
...
// Root file: contracts/baby/SPELLPRINTER.sol
...
// import "contracts/interfaces/IUniswapV2Factory.sol";
// import "contracts/interfaces/IUniswapV2Router02.sol";
// import "contracts/baby/SPELLPRINTERDividendTracker.sol";
// import "contracts/BaseToken.sol";

contract SPELLPRINTER is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    SPELLPRINTERDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = SPELLPRINTERDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SPELLPRINTER: The dividend tracker already has that address"
        );

        SPELLPRINTERDividendTracker newDividendTracker = SPELLPRINTERDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "SPELLPRINTER: The new dividend tracker must be owned by the SPELLPRINTER token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "SPELLPRINTER: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SPELLPRINTER: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != UniswapV2Pair,
            "SPELLPRINTER: The Pancakeswap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SPELLPRINTER: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "SPELLPRINTER: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "SPELLPRINTER: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialSPELLBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapTokensForSPELL(tokens);
    ...
    function swapTokensForSPELL(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributeSPELLDividends(dividends);
