...
contract ERC20 is Context, IERC20, Ownable {
     ...
     */
    function mint(uint256 amount) internal returns (bool) {
...
// DEFYToken with Governance.
contract DefyToken is ERC20('DefyCoin', 'DEFY') {
    ...
    uint256 public _trFee = 50;
    uint256 public _devFee = 10;
    
    bool public isTaxOff;
    
    uint256 public _maxTxAmount = 225 * 10**3 * 1e18;
    uint256 public constant _maxSupply = 225 * 10**3 * 1e18;
    
    address public TR_VAULT; // Treasury fund vault for ILP and Farm
    address public dev;
    ...
    constructor(address _trVault) public {
     	require(_trVault != address(0), 'DEFY: dev cannot be the zero address');
     	dev = msg.sender;
        TR_VAULT = _trVault;
     	_mint(_msgSender(), _maxSupply);
        _moveDelegates(address(0), _delegates[_msgSender()], _maxSupply);
        _isRExcludedFromFee[msg.sender] = true;
        _isRExcludedFromFee[_trVault] = true;
        _isSExcludedFromFee[msg.sender] = true;
        _isSExcludedFromFee[_trVault] = true;
    }
    ...
    function setTrFee(uint256 trFee) external onlyDev {	
        require(trFee <= 80 , "Error : MaxTrFee is 8%");
        uint256 _previousTrFee = _trFee;	
        _trFee = trFee;	
        	
        emit TrFeeUpdated(_previousTrFee,_trFee);	
    }
    ...
    function setTaxOff(bool state)external onlyDev {
        isTaxOff = state;
        emit TaxOff(state);
    }
    ...
    function setTrVault(address _trVault) external onlyDev {
        _isRExcludedFromFee[TR_VAULT] = false;
        _isSExcludedFromFee[TR_VAULT] = false;
        TR_VAULT = _trVault;
        _isRExcludedFromFee[_trVault] = true;
        _isSExcludedFromFee[_trVault] = true;
        emit SetTrVault(_trVault);
    }
   ...
   //  @notice Destroys `amount` tokens from `account`, reducing the total supply.
    ...
    function burnFrom(address from, uint256 amount) private {
        _burn(from, amount);
        _moveDelegates(address(0), _delegates[from], amount);
        emit Burn(amount);
    }
    
    function transferTaxFree(address recipient, uint256 amount) public returns (bool) {
        require(_isPair[_msgSender()] || _msgSender() == router , "DEFY: Only DefySwap Router or Defy pair");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (_isSExcludedFromFee[sender] || _isRExcludedFromFee[recipient] || isTaxOff) {
            super._transfer(sender, recipient, amount);
        }
        else {
            // A percentage of every transfer goes to Burn ,TR Vault & Dev
            uint256 burnAmount = amount.mul(_burnFee).div(1000);
            uint256 trAmount = amount.mul(_trFee).div(1000);
            uint256 devAmount = amount.mul(_devFee).div(1000);
            
            // Remainder of transfer sent to recipient
            uint256 sendAmount = amount.sub(burnAmount).sub(trAmount).sub(devAmount);
            require(amount == sendAmount + burnAmount + trAmount + devAmount , "DEFY Transfer: Fee value invalid");

            burnFrom(sender, burnAmount);
            super._transfer(sender, TR_VAULT, trAmount);
