...
contract FeedVaultBaseV3LP is FeedVault {
    ...
    /// @dev Token 0 to Buyback Path
    address[] public token0ToBuybackPath;

    /// @dev Token 1 to Buyback Path
    address[] public token1ToBuybackPath;

    /// @dev Referral Bonus in basis points. Initially set to 50% of 0.1%
    ...
    constructor(
        ...
        address[] memory _token0ToBuybackPath,
        address[] memory _token1ToBuybackPath,
        address _feesRouter
    ) {
        token = IERC20(_token);
        ...
        token0ToBuybackPath = _token0ToBuybackPath;
        token1ToBuybackPath = _token1ToBuybackPath;
    }
    ...
    function _collectEntryFees(uint256 _amount) internal override {
        ...
        if (_entryFees > 0) {
            if (autoSwapEntryFees) {
                address _token0 = token0ToBuybackPath[0];
                address _token1 = token1ToBuybackPath[0];

                _removeLiquidity(address(token), address(_token0), address(_token1), _entryFees);

                _buyBackFees(token0ToBuybackPath, address(_token0), referralEntryFeeBP, true, entryFeesCollector);
                _buyBackFees(token1ToBuybackPath, address(_token1), referralEntryFeeBP, true, entryFeesCollector);
            } else {
                ...
                emit FeesCollected(true, entryFeesCollector, address(token), _remainingFees);
            }
        }
    }
    ...
    function _collectExitFees(uint256 _amount) internal override returns (uint256) {
        ...
        if (_exitFees > 0) {
            if (autoSwapExitFees) {
                ...
                _removeLiquidity(address(token), address(_token0), address(_token1), _exitFees);

                _buyBackFees(token0ToBuybackPath, address(_token0), referralExitFeeBP, false, exitFeesCollector);
                _buyBackFees(token1ToBuybackPath, address(_token1), referralExitFeeBP, false, exitFeesCollector);
            } else {
                ...
                emit FeesCollected(false, exitFeesCollector, address(token), _remainingFees);
            }
    ...
    function _buyBackFees(
        ...
        address _token,
        uint256 _referralFeeBP,
        bool _isEntry,
        address _feesCollector
    ) internal virtual returns (uint256) {
        address _buyBackToken = _buyBackPath[_buyBackPath.length - 1];
        uint256 _before = IERC20(_buyBackToken).balanceOf(address(this));
        uint256 _amount = IERC20(_token).balanceOf(address(this));
        if (_amount > 0) {
            _safeSwap(swapRouterAddress, _amount, _buyBackPath, address(this), block.timestamp.add(600));
        }
        uint256 _buyBackAmount = IERC20(_buyBackToken).balanceOf(address(this)).sub(_before);
        uint256 _commissionPaid = _payReferralCommission(address(msg.sender), address(_buyBackToken), _buyBackAmount, _referralFeeBP);

        IERC20(_buyBackToken).safeTransfer(_feesCollector, _buyBackAmount.sub(_commissionPaid));

        emit FeesCollected(_isEntry, _feesCollector, _buyBackToken, _buyBackAmount.sub(_commissionPaid));
    }
    ...
    function _removeLiquidity(
        ...
        IERC20(_lp).safeIncreaseAllowance(swapRouterAddress, _amount);
        IPancakeRouter02(swapRouterAddress).removeLiquidity(
            address(_token0),
            address(_token1),
            _amount,
            0,
    ...
    /**
     * @dev Set Token 0 to Buyback Path
     */
    function setToken0ToBuybackPath(address[] memory _token0ToBuybackPath) public virtual onlyRole(ADMIN_ROLE) nonReentrant {
        token0ToBuybackPath = _token0ToBuybackPath;
    }

    /**
     * @dev Set Token 1 to Buyback Path
     */
    function setToken1ToBuybackPath(address[] memory _token1ToBuybackPath) public virtual onlyRole(ADMIN_ROLE) nonReentrant {
        token1ToBuybackPath = _token1ToBuybackPath;
    }
    ...
    /**
     * @dev Set Referral Entry Fee Basis Point
     */
    function setReferralEntryFeeBp(uint256 _entryFeeBP) public onlyRole(ADMIN_ROLE) {
        require(_entryFeeBP <= MAXIMUM_REFERRAL_FEE_BP, "LIMIT");
        referralEntryFeeBP = _entryFeeBP;
    }

    /**
     * @dev Set Referral Exit Fee Basis Point
     */
    function setReferralExitFeeBp(uint256 _exitFeeBP) public onlyRole(ADMIN_ROLE) {
        require(_exitFeeBP <= MAXIMUM_REFERRAL_FEE_BP, "LIMIT");
        referralExitFeeBP = _exitFeeBP;
    }
}
