...
contract Strategy_Spirit is Strategy {
    constructor(
        address[] memory _addresses,
        uint256 _pid,
        // bool _isSameAssetDeposit,
        bool _isCGSVComp,
        address[] memory _earnedToCGSVPath,
        ...
        bool _isFarmReferral
    ) public {
        wbnbAddress = _addresses[0];
        govAddress = _addresses[1]; // TimelockController
        CGSVFarmAddress = _addresses[2]; // Vault Farming
        CGSVAddress = _addresses[3]; // Token

        wantAddress = _addresses[4]; // Staked LPs
        token0Address = _addresses[5]; // Staked LPs token - 0
        token1Address = _addresses[6]; // Staked LPs token - 1
        earnedAddress = _addresses[7]; // Reward Token of (PCS farming)

        farmContractAddress = _addresses[8]; // AMM vault farming (Ex: PCS)
        pid = _pid; 
        // isSameAssetDeposit = _isSameAssetDeposit;
        isCGSVComp = _isCGSVComp;

        uniRouterAddress = _addresses[9]; // AMM Router (Ex: PCS)
        earnedToCGSVPath = _earnedToCGSVPath; 
        ...
        rewardsAddress = _addresses[10]; // TimelockController_RewardDistributor
        buyBackRate = _buyBackRate;
        buyBackAddress = _addresses[11]; // TimelockController_RewardDistributor
        entranceFeeFactor = _entranceFeeFactor;
        withdrawFeeFactor = _withdrawFeeFactor;
        isFarmReferral = _isFarmReferral;

        transferOwnership(CGSVFarmAddress);
    }
}
...
interface IPancakeswapFarm {
    ...
    // View function to see pending CGSVs on frontend.
    function pendingCGSV(uint256 _pid, address _user)
        external
        view
        returns (uint256);
    ...
    // Deposit LP tokens to MasterChef for CAKE allocation (with referral).
...
abstract contract Strategy is Ownable, ReentrancyGuard, Pausable {
    ...
    bool public isCGSVComp; // this vault is purely for staking. eg. WBNB-CGSV staking vault.
    ...
    address public CGSVFarmAddress;
    address public CGSVAddress;
    ...
    uint256 public controllerFee = 0; // 150;
    uint256 public constant controllerFeeMax = 10000; // 100 = 1%
    uint256 public constant controllerFeeUL = 350;

    uint256 public buyBackRate = 0; // 200;
    ...
    bool public isFarmReferral = false;

    address[] public earnedToCGSVPath;
        ...
        returns (uint256)
    {
        ...
        uint256 _beforeStakedAmt = _totalStaked();
        ...
        uint256 _afterStakedAmt = _totalStaked();
        _wantAmt = _afterStakedAmt.sub(_beforeStakedAmt); // amount after depositing

        uint256 sharesAdded = _wantAmt;
        if (wantLockedTotalOld > 0 && sharesTotal > 0) {
            sharesAdded = _wantAmt
                .mul(sharesTotal)
                .mul(entranceFeeFactor)
                .div(wantLockedTotalOld)
    ...
    function _farm() internal virtual {
        require(isCGSVComp, "!isTestVComp");
        ...
        if (isFarmReferral) {
            IPancakeswapFarm(farmContractAddress).deposit(pid, wantAmt, rewardsAddress);
        }
        else {
        ...
        uint256 _afterStakedAmt = _totalStaked();
        wantLockedTotal = wantLockedTotal.add(_afterStakedAmt).sub(_beforeStakedAmt);
    }

    function _totalStaked() internal view returns (uint256) {
        (uint256 amount, ) = IPancakeswapFarm(farmContractAddress).userInfo(pid, address(this));
        return amount;
    }
        ...
        returns (uint256)
    {
        ...
        IERC20(wantAddress).safeTransfer(CGSVFarmAddress, _wantAmt);
