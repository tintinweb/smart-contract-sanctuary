...
contract SmurfBondFactory {
    ...
    // Seconds per Month
    uint256 public secondsPerMonth = 180; // (180s for testing only) 1 month (30 days, 2592000 seconds)
    ...
    // Withdraw all LPs by depositing all Bonds minted by the User
    function withdrawAllLps() public {
        UserInfo storage user = userInfo[msg.sender];
        ...
        uint256 currentBondHolding = user.userAmountBonds;

        // Transfer Bonds to Burn Address
        IERC20(sbond).safeTransferFrom(address(msg.sender), Burn, currentBondHolding);
        
        totalBondsBurned = totalBondsBurned + currentBondHolding;
        user.userAmountBonds = 0;
        user.userUnlockTime = 0;
        

        // Transfer all LPs deposited by user back to user

            IERC20(CleverFtmLp).safeTransferFrom(address(this), address(msg.sender), user.amountCleverFtm);
            user.amountCleverFtm = 0;

            IERC20(CleverUsdcLp).safeTransferFrom(address(this), address(msg.sender), user.amountCleverUsdc);
            user.amountCleverUsdc = 0;

            IERC20(CleverEthLp).safeTransferFrom(address(this), address(msg.sender), user.amountCleverEth);
            user.amountCleverEth = 0;

            IERC20(MushyFtmLp).safeTransferFrom(address(this), address(msg.sender), user.amountMushyFtm);
            user.amountMushyFtm = 0;

            IERC20(MushyUsdcLp).safeTransferFrom(address(this), address(msg.sender), user.amountMushyUsdc);
            user.amountMushyUsdc = 0;

            IERC20(Clever).safeTransferFrom(address(this), address(msg.sender), user.amountClever);
            user.amountClever = 0;

            IERC20(Mushy).safeTransferFrom(address(this), address(msg.sender), user.amountMushy);
            user.amountMushy = 0;

        emit WithdrawLps (msg.sender, currentBondHolding);
    }
