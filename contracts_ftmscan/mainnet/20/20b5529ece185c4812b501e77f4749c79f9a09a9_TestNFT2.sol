...
contract TestNFT2 is ERC721, Ownable {
    ...
    bool public checkApeOwnable = false;
    
    Fape private fape;

    constructor(string memory name, string memory symbol, string memory baseURI,address payable _developer , address payable _marketer, address _fape) ERC721(name, symbol) {
    ...
    function mintNFT(address[] memory _user, uint256[] memory _index) public onlyOwner {
        
        for(uint256 i=0; i<_user.length; i++){
            _safeMint(_user[i], _index[i]);    
        }
    ...
    function buyPunk(uint256 tokenId) payable public {
        ...
        require(tokenId < 4, "Out of tokenId");
        ...
        uint FeeAmount = (msg.value * 2 / 100);
    ...
    function offerPunkForSale(uint256 tokenId, uint256 minSalePriceInWei) public onlyTradablePunk(_msgSender(), tokenId) {
        ...
        if(checkApeOwnable){
            require(fape.balanceOf(_msgSender()) > 0, "you must have fape");
        }
    ...
    function acceptBidForPunk(uint256 tokenId, uint256 minPrice) public onlyTradablePunk(_msgSender(), tokenId) {
        ...
        uint FeeAmount = (bid.value * 2 / 100);
    ...
    function apeOwnable(bool _checkApeOwnable) external onlyOwner {
        checkApeOwnable = _checkApeOwnable;
    }
}