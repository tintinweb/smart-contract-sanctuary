...
contract VirtualAsset is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "VirtualAsset/not-authorized");
        _;
    }

    // --- ERC20 Data ---
    string  public constant name     = "VirtualAsset";
    string  public constant symbol   = "VAT";
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
      ...
      require(balanceOf[src] >= wad, "VirtualAsset/insufficient-balance");
        if (src != msg.sender && _allowance( src, msg.sender ) != uint(-1)) {
            require(_allowance( src, msg.sender ) >= wad, "VirtualAsset/insufficient-allowance");
    ...
    function adjustVirtualAssetlyDAILimit(uint _limit) external auth {
        dailyDAILimit = _limit;
    }

    function mint(address usr, uint wad) external {
      require(wards[msg.sender] == 1, "mint not allow");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "VirtualAsset/insufficient-balance");
        if (usr != msg.sender && _allowance( usr, msg.sender ) != uint(-1)) {
            require(_allowance( usr, msg.sender ) >= wad, "VirtualAsset/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "VirtualAsset/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "VirtualAsset/invalid-permit");
        require(expiry == 0 || block.timestamp <= expiry, "VirtualAsset/permit-expired");
        require(nonce == nonces[holder]++, "VirtualAsset/invalid-nonce");
