...
contract MarsWasteland is IRobotSlime, ERC721Enumerable {
    // number of tokens have been minted so far
    uint16 public minted;

    // mapping from tokenId to a struct containing the token's traits
    mapping(uint256 => RobotSlime) public tokenTraits;
    // mapping from hashed(tokenTrait) to the tokenId it's associated with
    // used to ensure there are no duplicates
    mapping(uint256 => uint256) public existingCombinations;

    // list of probabilities for each trait type
    // 0 - 5 are associated with Robots, 6 - 11 are associated with Slimes
    ...
    constructor(
        address traits_
    ) ERC721("Mars Wasteland", "MarsWasteland") {
        traits = ITraits(traits_);
        ...
        // head
        rarities[0] = 
            [ 190, 215, 240, 100, 110, 135, 160, 185, 80 , 210,
              235, 240, 80,  80,  100, 100, 100, 245, 250, 255
            ];
        aliases[0] =
            [ 1 , 2 , 4 , 0 , 5 , 6 , 7 , 9 , 0 , 10,
              11, 17, 0 , 0 , 0 , 0 , 4 , 18, 19, 19
            ];
    ...
    function selectTraits(uint256 seed)
        ...
        returns (RobotSlime memory t)
    {
        t.isRobot = (seed & 0xFFFF) % 10 != 0;
        uint8 shift = t.isRobot ? 0 : 6;
        seed >>= 16;
        ...
        seed >>= 16;
        t.body = selectTrait(uint16(seed & 0xFFFF), 3 + shift);
        seed >>= 16;
        t.equipment = selectTrait(uint16(seed & 0xFFFF), 4 + shift);
        seed >>= 16;
     ...
     */
    function structToHash(RobotSlime memory s) internal pure returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function setTraits(address traits_) public {
        traits = ITraits(traits_);
    }
}