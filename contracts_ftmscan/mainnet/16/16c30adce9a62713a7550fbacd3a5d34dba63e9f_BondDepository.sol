...
contract BondDepository is Ownable {
    ...
    address public immutable XRX; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints XRX when receives principle
    ...
    struct Bond {
        uint256 payout; // XRX remaining to be paid
        uint256 vesting; // Blocks left to vest
        uint256 lastBlock; // Last interaction
        uint256 pricePaid; // In BUSD, for front end viewing
    }
    ...
    function deposit(
        ...
        uint256 payout = payoutFor(value); // payout to bonder is computed 利息

        require(payout >= 10000000, "Bond too small"); // must be > 0.01 XRX ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) XRX
         */
        ...
        if (fee != 0) {
            ...
            IERC20(XRX).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(XRX).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(XRX).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(XRX).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(XRX).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to XRX supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(XRX).totalSupply();
    ...
    /**
     *  @notice calculate amount of XRX available for claim by depositor
     *  @param _depositor address
    ...
    function setDao(address addr) external onlyPolicy{
        DAO = addr;
    }
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or XRX) to the DAO
     *  @return bool
     */
