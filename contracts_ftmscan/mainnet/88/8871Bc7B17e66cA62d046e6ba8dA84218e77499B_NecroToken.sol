...
contract NecroToken is ERC20Permit, Ownable {
    using SafeMath for uint256;
    ...
    function getNewlyMintedAmount() external view returns (uint256) {
        uint256 newlyMinted = totalSupply().sub(25e18);
        return newlyMinted;
    }
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s ) external {
        ...
        require(signatory != address(0), "NECRO::delegateBySig: invalid signature");
        require(nonce == _useNonce(signatory), "NECRO::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "NECRO::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "NECRO::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying NECROs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "NECRO::_writeCheckpoint: block number exceeds 32 bits");
