...
*/
contract MUMMY is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for liquidity + presale
    uint256 public constant INITIAL_DISTRIBUTION_POOL = 20000 ether;
    // Initial distribution for the day 2-5 MUMMY-WFTM LP -> MUMMY pool
    uint256 public constant INITIAL_MUMMY_POOL_DISTRIBUTION = 140000 ether;

    // Have the rewards been distributed to the pools
    bool public rewardPoolDistributed = false; //== false on deployment
    ...
    uint256 public burnThreshold = 5e17;
    ...
    /**
     * @notice Constructs the MUMMY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("DADDY", "DADDY") {
        // Mints 1 MUMMY to contract creator for initial pool setup
    ...
    function _getMUMMYPrice() internal view returns (uint256 _MUMMYPrice) {
        try IOracle(MUMMYOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("MUMMY: failed to fetch MUMMY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _MUMMYPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_MUMMYPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMUMMYOracle(address _MUMMYOracle) public onlyOperatorOrTaxOffice {
        require(_MUMMYOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MUMMY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MUMMY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMUMMYPrice = _getMUMMYPrice();
            currentTaxRate = _updateTaxRate(currentMUMMYPrice);
            if (currentMUMMYPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_distributionWallet != address(0), "!_distributionWallet");
        require(_MUMMYRPool != address(0), "!_MUMMYRPool");
        rewardPoolDistributed = true;
        _mint(_distributionWallet, INITIAL_DISTRIBUTION_POOL);
        _mint(_MUMMYRPool, INITIAL_MUMMY_POOL_DISTRIBUTION);
    }
