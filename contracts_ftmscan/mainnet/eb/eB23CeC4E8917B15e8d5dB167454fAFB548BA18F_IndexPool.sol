...
contract IndexPool is BToken, IIndexPool {
  bool internal _mutex;
  address internal _controller;
  bool internal _publicSwap;
  address[] internal _tokens;
  address[] internal _routers;
  ...
  function routers() external view override returns (address[] memory) {
    return _routers;
  }

  function factories() external view override returns (address[] memory) {
    return _factories;
  }
  ...
  function configure(
    ...
    uint256 len = uniswapV2Factories.length;
    require(_controller == zero, "BiShares: Already configured");
    require(controller != zero, "BiShares: Controller is zero address");
    require(uniswapV2Routers.length == len, "BiShares: Invalid arrays length");
    require(uniswapV2Oracle != zero, "BiShares: Oracle is zero address");
    for (uint256 i = 0; i < len; i++) require(
      uniswapV2Factories[i] != zero && uniswapV2Routers[i] != zero,
      "BiShares: Array element is zero"
    );
    _controller = controller;
    _initializeToken(name, symbol);
    _oracle = uniswapV2Oracle; 
    _factories = uniswapV2Factories;
    _routers = uniswapV2Routers;
    return true;
  }
  ...
  function initialize(
    ...
    require(
      len == _factories.length && balances.length == len && denorms.length == len,
  ...
  function delegateCompLikeToken(
    address token,
    address delegatee
  ) external override _control_ returns (bool) {
