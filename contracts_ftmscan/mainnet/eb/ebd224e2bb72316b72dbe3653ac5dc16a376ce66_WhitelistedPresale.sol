...
contract WhitelistedPresale is Ownable {
    ...
    uint constant DAIdecimals = 10 ** 18;
    uint constant F0Xdecimals = 10 ** 9;
    uint public constant MAX_SOLD = 70000 * F0Xdecimals;
    uint public constant PRICE = 5 * DAIdecimals / F0Xdecimals ;
    uint public constant MAX_SALE_PER_ACCOUNT = 200 * F0Xdecimals;
    uint public constant MAX_PRESALE_PER_ACCOUNT = 400 * F0Xdecimals;
    ...
    constructor( address _dev, uint _presaleTimestamp, address dai_address) {
        dev = _dev;
    ...
    function claim() public onlyEOA {
      ERC20(F0X).transfer(msg.sender, invested[msg.sender]);
      owed -= invested[msg.sender];
      invested[msg.sender] = 0;

    }
    function setClaimingActive(address test) public {
