...
contract DrakoRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DRAKOs to distribute in the pool.
        uint256 lastRewardTime; // Last time that DRAKOs distribution occurred.
        uint256 accTombPerShare; // Accumulated DRAKOs per share, times 1e18. See below.
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "DrakoRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "DrakoRewardPool: existing pool?");
        }
    }
    ...
    // View function to see pending DRAKOs on frontend.
    ...
    // Safe tomb transfer function, just in case if rounding error causes pool to not have enough DRAKOs.
