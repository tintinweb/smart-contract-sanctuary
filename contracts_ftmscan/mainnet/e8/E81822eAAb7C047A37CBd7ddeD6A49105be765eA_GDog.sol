...
contract GDog is BEP20 {
    ...
    // Min amount to liquify. (default 10 GDogs)
    ...
    // The operator can call all functions except of minting GDog
    address private _operator;
    
    // The lpEarner address will be set to an Earning Contract which
    // will distribute GDog-WFTM LP token to GDog Stakers!
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "GDog::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the GDogSwapToken contract.
     */
    constructor() public BEP20("GDog Sex Cat", "GDog") {
    ...
    /// @dev overrides transfer function to meet tokenomics of GDog
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == brunAddr || sender == _operator || sender == noTaxSenderAddr || recipient == noTaxRecipientAddr || transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "GDog::transfer: Burn value invalid");

            // default 96% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "GDog::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the GDog -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "GDog::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "GDog::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "GDog::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 100, "GDog::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "GDog::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "GDog::transferOperator: new operator is the zero address");
