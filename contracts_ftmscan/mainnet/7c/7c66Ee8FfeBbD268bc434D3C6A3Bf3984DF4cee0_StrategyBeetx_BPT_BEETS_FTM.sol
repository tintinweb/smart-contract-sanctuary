...
contract StrategyBeetx_BPT_BEETS_FTM is BeetXStrategy {
    ...
    function initializeStrategy(
        ...
        address fbeetsToken = address(0xfcef8a994209d6916EB2C86cDD2AFD60Aa6F54b1);
        __Strategy_init(
            _store,
            beets_ftm_lp,
            _vault,
...
import "./interfaces/IBeetsBar.sol";
...
contract BeetXStrategy is BaseUpgradeableStrategy {
    ...
    */
    function investedUnderlyingBalance() external view returns (uint256) {
        ...
        uint256 stakeInUnderlying = (_rewardPoolStake() * _getRatio()) / 1e18;
        return (stakeInUnderlying + IERC20(underlying()).balanceOf(address(this)));
    }
    ...
    */
    function withdrawAllToVault() public restricted {
        ...
        // Withdraw from the BeetsBar.
        uint256 fbeetsBalance = IERC20(fbeets()).balanceOf(address(this));
        IBeetsBar(fbeets()).leave(fbeetsBalance);
    ...
    */
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            uint256 amountToBurn = (needToWithdraw * 1e18) / _getRatio();
            IMasterChef(rewardPool()).withdrawAndHarvest(poolId(), Math.min(_rewardPoolStake(), amountToBurn), address(this));
            IBeetsBar(fbeets()).leave(amountToBurn);
        }
    ...
    */
    ...
    function unsalvagableTokens(address token) public view returns (bool) {
        return (token == underlying() || token == rewardToken() || token == depositToken() || token == fbeets());
    }
    ...
    function _investAllUnderlying() internal onlyNotPausedInvesting {
        ...
        if(underlyingBalance > 0) {
            // First we need to stake into the BeetsBar contract.
            IERC20(underlying()).safeApprove(fbeets(), 0);
            IERC20(underlying()).safeApprove(fbeets(), underlyingBalance);
            IBeetsBar(fbeets()).enter(underlyingBalance);

            // Now we need to stake our fBEETS token into the BeethovenX MasterChef.
            uint256 fbeetsBalance = IERC20(fbeets()).balanceOf(address(this));
            IERC20(fbeets()).safeApprove(rewardPool(), 0);
            IERC20(fbeets()).safeApprove(rewardPool(), fbeetsBalance);
            IMasterChef(rewardPool()).deposit(poolId(), fbeetsBalance, address(this));
        }
    }
    ...
    */
    ...
    function _getRatio() internal view returns (uint256 ratio) {
        ratio = (IBeetsBar(fbeets()).totalSupply() * 1e18) / IERC20(underlying()).balanceOf(fbeets());
    }
