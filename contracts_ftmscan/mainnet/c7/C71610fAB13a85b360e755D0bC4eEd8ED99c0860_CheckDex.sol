...
contract CheckDex {
    ...
    function executeArb(address router1Address, address router2Address, address[] calldata inPath, address[] calldata outPath, uint256 max_can_sell_tokenA,uint256 max_buy_tokenB) external {
        ...
        IERC20(inPath[1]).approve(address(router2), 10**30);
        router1.swapTokensForExactTokens(max_buy_tokenB, max_can_sell_tokenA, inPath, address(this), block.timestamp + 1000);
        router2.swapExactTokensForTokensSupportingFeeOnTransferTokens(max_can_sell_tokenA, max_buy_tokenB, outPath, address(this), block.timestamp + 1000);
        IERC20(inPath[0]).transfer(msg.sender, IERC20(inPath[0]).balanceOf(address(this))); // no token left behind
        IERC20(inPath[1]).transfer(msg.sender, IERC20(inPath[0]).balanceOf(address(this))); 
    }
