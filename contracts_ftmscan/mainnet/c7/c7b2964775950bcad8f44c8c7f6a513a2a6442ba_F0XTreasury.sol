...
contract F0XTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SF0X }

    address public immutable F0X;
    ...
    address public sF0X;
    uint public sF0XQueue; // Delays change to sF0X address
    ...
    constructor (
       ...
       /* isLiquidityToken[ _F0XDAI ] = true;
       liquidityTokens.push( _F0XDAI ); */

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for F0X
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint F0X needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( F0X ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn F0X for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IF0XERC20( F0X ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sF0X ).balanceOf( msg.sender ); // Can only borrow against sF0X held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with F0X
        @param _amount uint
     */
    function repayDebtWithF0X( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IF0XERC20( F0X ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, F0X, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( F0X ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( F0X ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns F0X valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match F0X decimals
            value_ = _amount.mul( 10 ** IERC20( F0X ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SF0X ) { // 9
            sF0XQueue = block.number.add( blocksNeededForQueue );
        } else return false;
