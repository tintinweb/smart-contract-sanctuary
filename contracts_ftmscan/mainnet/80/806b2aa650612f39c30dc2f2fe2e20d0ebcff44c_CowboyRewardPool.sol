...
// Note that this pool has no minter key of COWBOY (rewards).
// Instead, the governance will call COWBOY distributeReward method and send reward to this pool at the beginning.
contract CowboyRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. COWBOYs to distribute per block.
        uint256 lastRewardTime; // Last time that COWBOYs distribution occurs.
        uint256 accTSharePerShare; // Accumulated COWBOYs per share, times 1e18. See below.
    ...
    // The time when COWBOY mining starts.
    uint256 public poolStartTime;

    // The time when COWBOY mining ends.
    uint256 public poolEndTime;

    uint256 public tSharePerSecond = 0.00186122 ether; // 59500 COWBOY / (370 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "CowboyRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "CowboyRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's COWBOY allocation point. Can only be called by the owner.
    ...
    // View function to see pending COWBOYs on frontend.
    ...
    // Safe tshare transfer function, just in case if rounding error causes pool to not have enough COWBOYs.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (Cowboy or lps) if less than 90 days after pool ends
            require(_token != tshare, "cowboy");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
