...
interface IERC20 {
     ...
     * that someone may use both the old and the new allowance by unfeverunate
...
contract FEVERSaleV2 is Ownable {
    using SafeERC20 for ERC20;
    using Address for address;

    uint constant MIMdecimals = 10 ** 18;
    uint constant FEVERdecimals = 10 ** 9;
    uint public constant MAX_SOLD = 300000 * FEVERdecimals;
    uint public PRICE = 1 * MIMdecimals / FEVERdecimals ;
    uint public MIN_PRESALE_PER_ACCOUNT = 50 * FEVERdecimals;
    uint public MAX_PRESALE_PER_ACCOUNT = 900 * FEVERdecimals;
    ...
    constructor() {
        MIM = ERC20(0x82f0B8B456c1A451378467398982d4834b6829c1);
        dev = 0x0a1b544c33C1B3e7ea723B51b37da291F497e304;
        sold = 0;
    }
    ...
    /* allow non-blacklisted users to buy FEVER */
    ...
    // deactivate claiming
    function setClaimingInactive() public {
        require(msg.sender == dev, "!dev");
        canClaim = false;
    }

    // activate claiming
    function setClaimingON() public {
        require(msg.sender == dev, "!dev");
        canClaim = true;
    }

    // claim FEVER allocation based on old + new invested amounts
  /**  function claimFEVER() public onlyEOA {
        require(canClaim, "cannot claim yet");
        require(!claimed[msg.sender], "already claimed");
        require(!blacklisted[msg.sender], "blacklisted");
        if ( invested[msg.sender] > 0 ) {
            ERC20(FEVER).transfer(msg.sender, invested[msg.sender]);
    ...
    function claimFever() public onlyEOA {
        ...
        if (isDev(msg.sender)) {
            ...
            dailyClaimed[msg.sender] += 100000;
        } else {
            ERC20(FEVER).transfer(msg.sender, amountPerClaim[msg.sender]);
            invested[msg.sender] -= amountPerClaim[msg.sender];
            dailyClaimed[msg.sender] += 100;
        }
    }
