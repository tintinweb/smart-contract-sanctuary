...
// File contracts/QiPowah-ftm.sol
...
contract QIPOWAHFANTOM {
    using SafeMath for uint256;

    IERC20 qi = IERC20(0x68Aa691a8819B07988B18923F712F3f4C8d36346);
    ...
    function totalSupply() public view returns (uint256) {
        ...
        uint256 qi_totalQi = qi.totalSupply(); /// x 4 because boost in eQi
    ...
    function calculateBalancerPoolBalance(
        ...
        if(IERC20(pool_address).balanceOf(user) == 0 || poolQiBalance == 0 || IERC20(pool_address).totalSupply() == 0){
            return 0;
        }
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        IMasterChef chef = IMasterChef(
            0x230917f8a262bF9f2C3959eC495b11D1B7E1aFfC
        ); // rewards contract
        ...
        lp_powah = lp_powah.add(calculateBalancerPoolBalance(
        ...
        uint256 totallpinmasterchef = IERC20(0x2ed55f12e700CdE978E1F0491D681919814ca44B).balanceOf(address(chef));

        uint256 qiheldbymasterchef = calculateBalancerPoolBalance(
            0x7ae6a223cde3a17e0b95626ef71a2db5f03f540a00020000000000000000008a,
            qi,
            address(chef)
        );

        if(lp_stakedBalance > 0) {
            qi_powah = qi_powah.add(lp_stakedBalance.mul(qiheldbymasterchef).div(totallpinmasterchef));
        }
        // get balance owned by user that is staking in the farm

        // add any unharvested Qi from the staked pools
        qi_powah = qi_powah.add(chef.pending(0, owner));
        qi_powah = qi_powah.add(chef.pending(1, owner));

        qi_powah = qi_powah.add(qi.balanceOf(owner));
