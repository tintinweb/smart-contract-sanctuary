...
contract Granary
{
	...
	constructor (address _w, address _a, address _m, address _e, uint8 _p, address _R, address[] memory _r, string memory _id, address[2] memory _v)
	{
		want=IERC20(_w);
		mc=IMasterchef(_m);
		earn=IERC20(_e);
        ...
        tvlp = _v[1];
		//Approvals
		//mc to take the asset
		IERC20(address(asset)).approve(address(mc),uint256(-1));
		//router to sell what we earn
		IERC20(address(earn)).approve(address(router),uint256(-1));
        //asset to take what it may want
		IERC20(address(want)).approve(address(asset),uint256(-1));
	...
	*/
	...
	function deposit(uint256 _amt) public rg
	{
		...
		uint256 abb = IERC20(asset).balanceOf(address(this));
		...
		uint256 win = vba.sub(vbb,"Dirty intake");
		IXT(asset).enter(win);
		uint256 aba = IERC20(asset).balanceOf(address(this));
		uint256 D = aba.sub(abb,"Dirty deposit");
		...
		(totalSupply > 0)
			...
			:	_mint = //M;
                    _amt;//This lets amount of "want" to be used
                        //instead of "asset" as the initial peg. helps info()
		totalSupply += _mint;
		uint256 _fee;
	...
	function withdraw(uint256 _amt) public rg
	{
		...
		//fancy math takes care of witthdraw fees
		uint256 aba = IERC20(asset).balanceOf(address(this));
		uint256 aout = aba.sub(abb,"Dirty unstake");
		IXT(asset).leave(aout);
		uint256 vba = want.balanceOf(address(this));
	...
	function work(address ben) internal
	{
		...
		uint256 ebb = (earn.balanceOf(address(this)));
		R.swapExactTokensForTokensSupportingFeeOnTransferTokens(ebb,1,route,address(this),block.timestamp);
		uint256 vbb = want.balanceOf(address(this));
		uint256 abb = IERC20(asset).balanceOf(address(this));
		uint256 mcbb = mc.userInfo(allnums[0],address(this)).amount;
		IXT(asset).enter(vbb);
		uint256 aba = IERC20(asset).balanceOf(address(this));
		aba.sub(abb,"Dirty deposit");
		uint256 abf = IERC20(asset).balanceOf(address(this));
		mc.deposit(allnums[0],abf);
		uint256 mcba = mc.userInfo(allnums[0],address(this)).amount;
		uint256 M = mcba.sub(mcbb,"Dirty stake");
	...
	function revokeEmergency() public DAO
	{
		...
		uint256 D = IERC20(asset).balanceOf(address(this));
		mc.deposit(allnums[0],D);
		emergency=false;
	}
	...
	function emergencyWithdraw(uint256 _amt) public rg
	{
		...
		uint256 vbb = want.balanceOf(address(this));
		uint256 abb = IERC20(asset).balanceOf(address(this));
		uint256 aout = ( _amt.mul(abb) ).div(ts);
		IXT(asset).leave(aout);
		uint256 vba = want.balanceOf(address(this));
		uint256 W = vba.sub(vbb,"Dirty withdrawal");
	...
	function info() public view returns (uint256, uint256, uint256, uint256, uint256, IMasterchef.PoolInfo memory)
	{
		uint256 asum = mc.userInfo(allnums[0],address(this)).amount + IERC20(asset).balanceOf(address(this));
        uint256 aum = IXT(asset).xBOOForBOO(asum) + IERC20(want).balanceOf(address(this));
