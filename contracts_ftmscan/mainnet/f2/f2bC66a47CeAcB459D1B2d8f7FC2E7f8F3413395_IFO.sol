...
contract IFO is ReentrancyGuard {
    ...
    // The timestamp in seconds when IFO starts
    uint256 public startTimestamp;
    // The timestamp in seconds when IFO ends
    ...
    function deposit(uint256 _amount) external onlyActiveIFO {
        ...
        require(userInfo[msg.sender].amount < limitPerUser || limitPerUser == 0, "user deposit limit is reached");
        require(totalAmount < raisingAmount || overflow == true, "100% raised");
        require(IBEP20(nativeToken).getTier(msg.sender) >= minTier, "need a higher tier");
        
        // trim _amount if higher than limit per user
        if (limitPerUser > 0) {
            // if deposited amount plus new amount is higher than the available limit
            if (userInfo[msg.sender].amount + _amount > limitPerUser) {
                _amount = limitPerUser - userInfo[msg.sender].amount;
            }
        }

        // trim _amount if higher than amount left to raise
        if (overflow == false) {
            // If deposited amount is bigger than the amount left to raise
            if (_amount > (raisingAmount-totalAmount)) {
                _amount = raisingAmount-totalAmount;
            }
        }

        // Init final deposit amount
        uint256 finalDepositAmount = _amount;
        // get final deposited amount
        uint256 pre = getTotalStakeTokenBalance();
        stakeToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        finalDepositAmount = getTotalStakeTokenBalance().sub(pre);
        // deposit internal
    ...
    function harvest() external nonReentrant {
        require(block.timestamp > endTimestamp, "not harvest time");
    ...
    function safeTransferStakeInternal(address _to, uint256 _amount) internal {
        ...
        require(_amount <= stakeBalance, "not enough stakeToken");
    ...
    function setLimitPerUser(uint256 _limitPerUser) external onlyAdmin {
        limitPerUser = _limitPerUser;
    }
    
    function setOverflow(bool _overflow) external onlyAdmin {
        overflow = _overflow;
    }
    
    function setMinTier(uint256 _minTier) external onlyAdmin {
        minTier = _minTier;
    }
