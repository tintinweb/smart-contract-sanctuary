...
contract LithiumDAO is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 LITHIUM-WETH LP -> LITHIUM pool
    uint256 public constant INITIAL_LITHIUM_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the LITHIUM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("LITHIUM DAO", "LITHIUM") {
        // Mints 1 LITHIUM to contract creator for initial pool setup
    ...
    function _getLithiumDAOPrice() internal view returns (uint256 _lithiumPrice) {
        try IOracle(lithiumOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("LithiumDAO: failed to fetch LITHIUM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _lithiumPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_lithiumPrice >= taxTiersTwaps[tierId]) {
    ...
    function setLithiumDAOOracle(address _lithiumOracle) public onlyOperatorOrTaxOffice {
        require(_lithiumOracle != address(0), "oracle address cannot be 0 address");
        lithiumOracle = _lithiumOracle;
    }
    ...
    /**
     * @notice Operator mints LITHIUM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of LITHIUM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentLithiumDAOPrice = _getLithiumDAOPrice();
            currentTaxRate = _updateTaxRate(currentLithiumDAOPrice);
            if (currentLithiumDAOPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _lithiumPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_lithiumPool != address(0), "!_lithiumPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_lithiumPool, INITIAL_LITHIUM_POOL_DISTRIBUTION);
