...
contract RShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accRSharePerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.00186122 ether; // 59500 rshare / (370 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "RShareRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "RShareRewardPool: existing pool?");
        }
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _rshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accRSharePerShare = accRSharePerShare.add(_rshareReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _rshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accRSharePerShare = pool.accRSharePerShare.add(_rshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    // Safe rshare transfer function, just in case if rounding error causes pool to not have enough tSHAREs.
