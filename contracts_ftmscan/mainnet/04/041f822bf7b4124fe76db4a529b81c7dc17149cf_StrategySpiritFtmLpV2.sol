...
contract StrategySpiritFtmLpV2 is StrategyBase {
    // LP Token addresses
    address public constant spiritFtmLP = 0x30748322B6E34545DBe0788C421886AEB5297789;
    address public constant spirit = 0x5Cc61A78F164885776AA610fb0FE1257df78E59B;
    address public constant spiritFarm = 0xEFe02cB895b6E061FA227de683C04F3Ce19f3A62;//ISpiritGauge
    ...
    public
    StrategyBase(spiritFtmLP, _governance, _strategist, _controller, _timelock){
        IERC20(spiritFtmLP).safeApprove(spiritFarm, uint256(-1));
    ...
    function balanceOfPool() public override view returns (uint256) {
        return ISpiritGauge(spiritFarm).balanceOf(address(this));
    }

    function getHarvestable() external view returns (uint256) {
        return ISpiritGauge(spiritFarm).earned(address(this));
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ISpiritGauge(spiritFarm).deposit(_want);
        }
    }
    ...
    returns (uint256)
    {
        ISpiritGauge(spiritFarm).withdraw(_amount);
        return _amount;
    }
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal override returns (address, uint256){
        ...
        (uint112 _wftmReserve, uint112 _spiritReserve,) = IUniswapV2Pair(want).getReserves();
        uint256 lpTotalSupply = IERC20(want).totalSupply();

        uint256 _token0Min = _lpAmount.mul(_wftmReserve).div(lpTotalSupply).mul(swapOutMinBPS).div(swapOutBPS);
        uint256 _token1Min = _lpAmount.mul(_spiritReserve).div(lpTotalSupply).mul(swapOutMinBPS).div(swapOutBPS);
        ...
        // Swap SPIRIT for WFTM
        uint256 _spirit = IERC20(spirit).balanceOf(address(this));
        if (_spirit > 0) {
            _swapUniswap(spirit, WFTM, _spirit);
        }
    ...
    function harvest() public override onlyBenignCallers{
        ...
        // Collects reward tokens (SPIRIT)
        ISpiritGauge(spiritFarm).getReward();
        uint256 _reward = IERC20(spirit).balanceOf(address(this));

        //swap half SPIRIT for WFTM
        if(_reward > 0){
        ...
        // We want to get back SPIRIT-FTM lp tokens
        uint256 _want = IERC20(want).balanceOf(address(this));
