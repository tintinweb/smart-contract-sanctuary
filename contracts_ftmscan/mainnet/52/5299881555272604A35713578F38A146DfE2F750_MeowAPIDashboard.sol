...
contract MeowAPIDashboard {
  ...
  function PositionInfo(address vaultAddress, uint256 id) public view returns (bytes memory info) {
    ...
    (address workerAddress, address owner, , , , , , , ) = vault.positions(id);
    ...
    uint256 lpAmount;
    {
      ...
      info1 = abi.encode(
        ...
        token1,
        lpAmount,
        positionValue.mul(priceBasetoken).div(10**6),
        positionDebt.mul(priceBasetoken).div(10**6)
      );
    }
    bytes memory info2;
    {
  ...
  function getStaking(address user) public view returns (bytes memory) {
    ...
    uint256[] memory lockedAmount = new uint256[](length);

    for (uint256 i = 0; i < length; i++) {
      (stakeToken[i], , , ) = MEOWMINING.poolInfo(i);
      (balance[i], , , lockedAmount[i], , ) = MEOWMINING.userInfo(i, user);
      stakeTokeValue[i] = getStakeTokenValue(stakeToken[i], balance[i], i);
      reward[i] = MEOWMINING.pendingMeow(i, user).add(lockedAmount[i]);
      rewardValue[i] = getRewardValue(reward[i]);
    }
  ...
  function mMeowPrice() internal view returns (uint256) {
  ...
  function getStakeTokenValue(
    ...
    address meowLp = IUniswapV2Factory(APIPrice.factory()).getPair(MEOW, APIPrice.wNative());
    uint256 length = MEOWMINING.poolLength();
    if (_stakeToken == address(MMEOW)) {
      //mMeow
      return getMMeowValue(_amount);
    } else if (_stakeToken == meowLp) {
      //LP
      return _amount.mul(APIPrice.lpPrice(meowLp)).div(1e18);
    } else if (_pid % 2 != 0 && _pid < length - 2) {
      //ib
      return getIbValue(_stakeToken, _amount);
    } else {
      //debtib
      return getDebtValue(_pid, _amount);
    }
  }

  function getDebtValue(uint256 _pid, uint256 _amount) internal view returns (uint256) {
    (address vault, , , ) = MEOWMINING.poolInfo(_pid + 1);
    uint256 debt = IVaultAPI(vault).debtShareToVal(_amount);
    address token = IVault(vault).token();
    uint256 decimals = uint256(IERC20(token).decimals());
    return debt.mul(APIPrice.tokenPrice(token)).div(APIPrice.powerDecimals(decimals));
  }

  function getRewardValue(uint256 _amount) internal view returns (uint256) {
    return _amount.mul(API1.meowPrice()).div(1e18);
  }
}
