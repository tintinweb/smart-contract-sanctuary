...
interface IMasterChef {
    ...
    function claim(address _user, address[] memory tokens) external;
...
contract GeistFtmStrat is Ownable, Pausable {
    ...
    address geistStake = address(0x49c93a95dbcc9A6A4D8f77E59c038ce5020e82f8);
    ...
    constructor(
        ...
        tokens = [want];
        gTokens = [address(0x39B3bd37208CBaDE74D0fcBDBb12D606295b430a), address(0x940F41F0ec9ba1A34CF001cc03347ac092F5F6B5), address(0x07E6332dD090D287d3489245038daF987955DCFB), address(0xe578C856933D8e1082740bf7661e379Aa2A30b26),
        address(0x25c130B2624CF12A4Ea30143eF50c5D68cEFA22f), address(0x38aCa5484B8603373Acc6961Ecd57a6a594510A3), address(0x690754A168B022331cAA2467207c61919b3F8A98), address(0xc664Fc7b8487a3E10824Cda768c1d239F2403bBe)];
    ...
    function claim() internal{
        IMasterChef(masterchef).claim(address(this), tokens);
    }
    ...
    function _deposit() internal whenNotPaused {
        ...
        IMasterChef(masterchef).deposit(tokens[0], wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        IMasterChef(masterchef).withdraw(tokens[0], _amount.sub(wantBal));
    ...
    function sweepGtokens() internal {
        for (uint i = 0; i < gTokens.length; i++){
            uint256 bal = IERC20(gTokens[i]).balanceOf(address(this));

            if (bal > 0){
                IERC20(gTokens[i]).safeTransfer(treasury, bal);
            }
        }
    }
    ...
    function callReward() public view returns (uint256) {
        uint256[] memory outputBal = IMasterChef(masterchef).claimableReward(address(this), tokens);
        uint256 nativeOut;
        if (outputBal[0] > 0) {
            try IUniSwapRouter(unirouter).getAmountsOut(outputBal[0], outputToWrappedRoute)
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IMasterChef(masterchef).userInfo(want, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IMasterChef(masterchef).emergencyWithdraw(tokens[0]);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
