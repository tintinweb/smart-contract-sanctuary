...
interface IERC721 is IERC165 {
     ...
     * - If the caller is not `from`, it must be have been allowed to move this token by either {nftApprove} or {setApprovalForAll}.
     ...
     * - If the caller is not `from`, it must be approved to move this token by either {nftApprove} or {setApprovalForAll}.
     *
     * Emits a {Transfer} event.
     */
     ...
     * Operators can call {nftTransferFrom} or {safeTransferFrom} for any token owned by the caller.
     *
     * Requirements:
     *
...
// File: tests/ERC721/IERC721Enumerable.sol
...
interface IERC721Enumerable is IERC721 {
     ...
     * Use along with {nftTotalSupply} to enumerate all tokens.
     */
    function tokenByIndex(uint256 index) external view returns (uint256);
}

// File: tests/ERC721/IERC721Metadata.sol
...
interface IERC721Metadata is IERC721 {
    /**
     * @dev Returns the token collection nftName.
     */
    function nftName() external view returns (string memory);

    /**
     * @dev Returns the token collection nftSymbol.
     */
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    /**
     * @dev Initializes the contract by setting a `nftName` and a `nftSymbol` to the token collection.
     */
    ...
    /**
     * @dev See {IERC721Metadata-nftName}.
     */
    ...
    /**
     * @dev See {IERC721Metadata-nftSymbol}.
     */
    ...
    /**
     * @dev See {IERC721-nftApprove}.
     */
    function nftApprove(address to, uint256 tokenId) public virtual override {
        address owner = ERC721.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");

        require(
            _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
            "ERC721: nftApprove caller is not owner nor approved for all"
        );
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(operator != _msgSender(), "ERC721: nftApprove to caller");
    ...
    /**
     * @dev See {IERC721-nftTransferFrom}.
     */
    function nftTransferFrom(
        address from,
        address to,
     ...
     *
     * Tokens can be managed by their owner or approved accounts via {nftApprove} or {setApprovalForAll}.
     *
     * Tokens start existing when they are minted (`_nftMint`),
     * and stop existing when they are burned (`_nftBurn`).
     */
     ...
     * Emits a {nftTransfer} event.
     */
    function _nftMint(address to, uint256 tokenId) internal virtual {
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(tokenId), "ERC721: token already minted");

        _beforeNftTransfer(address(0), to, tokenId);
     ...
     * Emits a {nftTransfer} event.
     */
    function _nftBurn(uint256 tokenId) internal virtual {
        address owner = ERC721.ownerOf(tokenId);

        _beforeNftTransfer(owner, address(0), tokenId);
     ...
     *  As opposed to {nftTransferFrom}, this imposes no restrictions on msg.sender.
     *
     * Requirements:
     *
     * - `to` cannot be the zero address.
    ...
    function _nftTransfer(
        ...
        _beforeNftTransfer(from, to, tokenId);
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit nftApproval(ERC721.ownerOf(tokenId), to, tokenId);
    }
...
// File: tests/ERC721/ERC721Enumerable.sol
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        require(index < ERC721.nftBalanceOf(owner), "ERC721Enumerable: owner index out of bounds");
        return _ownedTokens[owner][index];
    }

    /**
     * @dev See {IERC721Enumerable-nftTotalSupply}.
     */
    function nftTotalSupply() public view virtual override returns (uint256) {
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < ERC721Enumerable.nftTotalSupply(), "ERC721Enumerable: global index out of bounds");
    ...
    function _beforeNftTransfer(
        ...
        super._beforeNftTransfer(from, to, tokenId);
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = ERC721.nftBalanceOf(to);
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = ERC721.nftBalanceOf(from) - 1;
...
contract Stablecoin is ERC20, ReentrancyGuard, VaultNFTv4 {
    ...
    IERC20Metadata public collateralInfo;
    ...
    VaultNFTv4(vaultName, vaultSymbol, baseURI)
    ...
    function calculateCollateralProperties(uint256 _collateral, uint256 _debt) private view returns (uint256, uint256) {
        ...
        uint256 collateralValue = _collateral.mul(getEthPriceSource()).mul(10**(uint256(decimals()).sub(uint256(collateralInfo.decimals()))));
    ...
    function borrowToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) {
        ...
        require(totalSupply().add(amount) <= debtCeiling, "borrowToken: Cannot mint over available supply.");
    ...
    function liquidateVault(uint256 vaultID) external {
        ...
        require(balanceOf(msg.sender) >= halfDebt, "Token balance too low to pay off outstanding debt");

        //mai
        safeTransferFrom(msg.sender, address(this), halfDebt);
...
contract MesoStablecoin is Stablecoin, Ownable {
