...
/**
 **Submitted by Cheynespc from Degen Haus on FANTOM https://www.degen.haus find support on Telegram: t.me/degenhouse Lets make a community into a family that has no Rival
*/
...
contract DegenhausERC20 is IDegenhausERC20 {
    using SafeMath for uint;

    string public constant name = 'Degenhaus LPs';
...
interface IDegenhausCallee {
    function degenhausCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}

contract DegenhausPair is IDegenhausPair, DegenhausERC20 {
    using SafeMath  for uint;
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IDegenhausFactory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (data.length > 0) IDegenhausCallee(to).degenhausCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
...
contract DegenhausFactory is IDegenhausFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(DegenhausPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        bytes memory bytecode = type(DegenhausPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IDegenhausPair(pair).initialize(token0, token1);
