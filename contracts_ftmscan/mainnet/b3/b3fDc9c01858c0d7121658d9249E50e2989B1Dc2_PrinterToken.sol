...
contract PrinterToken is Ownable {
  ...
  /** ======= GLOBAL PARAMS ======= */
    ...
    // Global param to enable swap back; 
    bool internal swapEnabled = true;

    // Minimum amount of tokens contract should hold before we should do swapback; 
    uint256 internal swapThreshold;

    // Maximum amount of total supply that can be transfered in a transaction; 
    uint256 internal maxTxThreshold;
    
    // Maximum amount of total supply 1 address can have;
    ...
    // Used for fee calculation accuracy; 
    uint256 internal feeDenominator = 10000;
    
    // Percentage of the fee used to add liquidity to pair; 
    uint256 internal liquidityFee = 400;

    // Percentage of the fee distributed to holders; 
    uint256 internal reflectionFee = 800;

    // Percentage of the fee for marketing team; 
    uint256 internal marketingFee = 150;

    // Percentage of the fee for dev team; 
  ...
  /** ======= CONSTRUCTOR ======= */
    ...
    constructor(
      ...
      // Make a pair for WFTM/PrinterToken; 
      pair = IUniswapV2Factory(router.factory()).createPair(WFTM, address(this));

      // Ensure pair does not get dividends; 
      ...
      // Dev should be free from fees and txLimit so he can provide liquidity; 
      isFeeExempt[msg.sender] = true; 
      isTxLimitExempt[msg.sender] = true;

      // Make shure contract - pair and burn address get no dividends; 
  ...
  /** ======= INITIALIZER ======= */
    ...
    function initialize(address _dividendDistributor) external payable Swapping onlyOwner  {
      ...
      // Initialize the dividendDistributor;
      ...
      // Approve distributor for easy deposit; 
  ...
  /** ======= ERC20 INTERNAL FUNCTIONS ======= */
    ...
    function _transferFrom(address _sender, address _recipient, uint256 _amount) internal returns (bool) {
      // 1. Check if we are doing internal swaps, as we dont want to take fee's when swapping tokens back to ftm ect.; 
      if(inSwap) { 
          return _basicTransfer(_sender, _recipient, _amount); 
      } 

      // 2.0 Enforce max Tx limit; 
      require(isWithinTxLimits(_sender, _amount), 'Err - Max Tx limit triggerd');

      bool isSell = _recipient == pair || _recipient == address(router);

      // If transfer is not a sell make shure we enfore maxTxTreshHold;
      if (!isSell){
      ...
      // 5. Apply new balance for sender; 
      _balances[_sender] = _balances[_sender].sub(_amount, "ERC20:  Insufficient Balance");

      // 6. Check if we should take a fee based on sender address; 
...
contract Strategy is Ownable {
  ...
  /** ======= EXTERNAL ONLY DISTRIBUTOR ======= */
    ...
    function process() external payable onlyDistributor  {
        ...
        if(isAbovePeg()) {
          ...
          // Deposit FTM to farm; And EARN GSCARAB;
          ...
          // Exit farm if we have any tokens in there; 
          handleExitRewardPool();

          // Swap FTM for SCARAB so distributor can take it back or we can use it to buy bonds; 
          address[] memory path = new address[](2);
            path[0] = address(wftm);
          ...
          // Handle bonds buying with the amountOutScarab we leave the farm exit for token holders;
          if(shouldBuyBonds()) {
  ...
  /** ======= PUBLIC VIEW FUNCTIONS ======= */
    ...
    function handleRedeemBonds(uint256 _amount) internal {
        ...
        // Approve amountToRedeem; 
        sbond.approve(address(treasury), _amount);

        // Redeem BONDS with amountOut; 
    ...
    function handleDepositRewardPool(uint256 _amount) internal returns(uint256) {
        ...
        // Stake LP token in farm; - deposit will handle our last earnings;   
