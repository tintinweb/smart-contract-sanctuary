 ...
 */
 contract Presale is Owned{
    ...
    uint256 public minDaiLimit = 300 * (10 ** 18);
    uint256 public maxDaiLimit = 600 * (10 ** 18);
    uint256 public maxDaiLimitVip = 1200 * (10 ** 18);
    ...
    function setPreBuys(address[] memory addresses, uint amounts) public onlyOwner{
        for (uint i = 0; i < addresses.length; i++) {
            preBuys[addresses[i]].daiAmount = amounts;
            daiRaised = daiRaised.add(amounts);
    ...
    function setMaxVipDaiLimit(uint256 vipAmount) external onlyOwner {
        maxDaiLimitVip = vipAmount;    
    }
    ...
    function vipBuy(uint _amount) public {
        ...
        require(_amount >= minDaiLimit, "You need to sell at least some min amount");
        require(paidTotal[msg.sender] + _amount <= maxDaiLimitVip);
        // safe transferFrom of the payout amount
        dai.safeTransferFrom(msg.sender, address(this), _amount);
