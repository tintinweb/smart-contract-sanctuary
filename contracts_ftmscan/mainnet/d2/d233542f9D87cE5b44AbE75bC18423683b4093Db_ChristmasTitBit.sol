...
contract ChristmasTitBit is ERC721, ERC721URIStorage, ERC2981Collection, IMAX721, ERC165Storage, Developer, Ownable {
  ...
  constructor() ERC721("Christmas TitBit", "XXXMAS-TitBit") {
  ...
  function publicMint(uint256[] memory ids) public {
    ...
    for (uint i = 0; i < ids.length; i++) {
      ...
      if(!claimedFreebie[ids[i]] && addressCheck) {
        uint value = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, ids[i], tokenIdCounter.current(), block.difficulty))) % 6;
