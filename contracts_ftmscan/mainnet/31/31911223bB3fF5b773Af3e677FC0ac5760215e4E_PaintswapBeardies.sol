...
import './interfaces/IPaintswapBeardies.sol';
...
,,,,,,,,,,,,,,,,,,,,,,,,,,,,,(%&@@@%%%%%%%%%@@@@        @&@/,                   
               ,,,,,,,,,     #&&&&&%%&&&&&%%&&&%/       @@&                     
                          ...
                          @##@@@@&        ,@@@@%@&                              
                      @##(((((###%%       @#(///////.((@                        
                 ____  ____    __    ____  ____  ____  ____  ___                
                (  _ \( ___)  /__\  (  _ \(  _ \(_  _)( ___)/ __)               
                 ) _ < )__)  /(__)\  )   / )(_) )_)(_  )__) \__ \               
                (____/(____)(__)(__)(_)\_)(____/(____)(____)(___/               
*/

contract PaintswapBeardies is
    IPaintswapBeardies,
    ERC721SplitWithdrawals,
    ERC721PresetMinterPauserAutoIdRoyalty,
    Ownable
{
    ...
    // max per transactions
    uint8 public immutable MAX_PER_TX;
    // price per token mint
    uint256 public immutable MINT_PRICE;

    // prove the metadata provenance
    bytes32 public PROVENANACE;

    // randomized minting via VDF
    SlothParams private slothParams;
    // mint
    ...
    // royalties
    ERC2981 private royalties;
    // available tokens
    uint16[] private tokenIds;
    // offset after proving
    uint256 private proofOffset;
    // Whether the proof has been validated
    ...
    constructor(
        ...
        ERC721PresetMinterPauserAutoIdRoyalty('Paintswap Beardies', 'BEARDIES', _baseTokenURI, _royalty)
        ERC721SplitWithdrawals(_recipients, _splits)
    {
        // immutable
        MAX_SUPPLY = _maxSupply;
        MAX_PER_TX = _maxPerMint;
        ...
        royalties.tokens = _erc20Tokens;

        royalties.forwarder = new RoyaltiesForwarder(address(this), _royaltyRecipients, _royaltySplits);
    }

    /// @dev initialize the provenance to create a seed used to provably randomize the minting
    /// @param _provenance the provanance hash
    /// @param _iterations the number of iterations for the VDF
    function initialize(bytes32 _provenance, uint256 _iterations) public onlyOwner {
        require(PROVENANACE == 0x0, 'Provenance must be empty.');
        // provenance for metadata
        PROVENANACE = _provenance;
        slothParams.offset = 1;
        slothParams.prime = 432211379112113246928842014508850435796007;
        // iterations for the VDF
        slothParams.iterations = _iterations;
        // this is used to generate the VDF proof
        slothParams.seed = uint256(
            keccak256(abi.encodePacked(block.timestamp, _provenance, address(this), blockhash(block.number - 1)))
        );
    }

    /// @dev the VDF seed
    function seed() public view returns (uint256) {
        return slothParams.seed;
    }

    /// @dev the VDF prime
    function prime() public view returns (uint256) {
        return slothParams.prime;
    }

    /// @dev the VDF iterations
    function iterations() public view returns (uint256) {
        return slothParams.iterations;
    }

    /// @dev the vdf proof that was generated from the initialization seed
    /// @param _proof the vdf proof
    function proveIt(uint256 _proof) public {
        // we only want to prove it once
        require(!proofValid);

        // verify the proof using the request data
        bool _proven = SlothVerifiableDelay.verify(_proof, slothParams.seed, slothParams.prime, slothParams.iterations);
        require(_proven, 'Sloth proof failed.');

        proofOffset = _proof % MAX_SUPPLY;
        proofValid = true;
    }

    /// @dev The vdf proof that was generated from the initialization seed
    function createTokenIds(uint16 _from, uint16 _to) public {
        // we only want to initialize the tokenIds once
        require(proofValid);

        // create a list of token IDs to mint
        for (uint16 i = _from; i < _to; i++) {
            tokenIds.push(uint16((i + proofOffset) % MAX_SUPPLY) + slothParams.offset);
        }
        require(tokenIds.length <= MAX_SUPPLY && totalSupply() <= MAX_SUPPLY);
    }

    /// @dev mint some beardies
    /// @param _quantity the quantity to mint
    function mintBeardies(uint256 _quantity) public payable override nonReentrant {
        require(mintStart > 0 && mintStart <= block.timestamp, 'Minting not started.');
        require(MAX_PER_TX >= _quantity, 'Too many mints.');
        require(MINT_PRICE * _quantity == msg.value, 'Wrong value sent.');
        require(tokenIds.length > 0, 'No more tokens.');
        // mint the tokens
        _mintBeardies(_quantity);
    }

    /// @dev the provenance hash for all metadata
    function provenanceURI() public view returns (string memory) {
        return _getURI('provenance.json');
    }
    ...
    /// @dev calculate the hash for metadata
    /// @param _metadata The value for the hash
    function tokenMetadataHash(string memory _metadata) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_metadata));
    }

    /// @dev get the merkle proof for this token id
    /// @param _tokenId the token id
    function tokenProofUri(uint256 _tokenId) public view returns (string memory) {
        return _getURI(string(abi.encodePacked('proof.', _tokenId.toString(), '.json')));
    }

    /// @dev verify the metadata is valid per the provenance
    /// @param _tokenId The key for the hash
    /// @param _metadata The value for the hash
    /// @param _merkleProof The merkle proof
    /// @return True if the proof is valid
    function proveTokenMetadata(
        uint256 _tokenId,
        string memory _metadata,
        bytes32[] memory _merkleProof
    ) public view returns (bool) {
        return _verifyMerkleProof(_tokenId - slothParams.offset, _tokenId, _metadata, _merkleProof);
    }
    ...
    {
        return (address(royalties.forwarder), (_salePrice * royalty) / BASE);
    }
        ...
        returns (bool isOperator)
    {
        ...
        if (royalties.marketplaces[_operator]) {
            return true;
        }
        // user approvals
    ...
    function setMintStart(uint256 _mintStart) external override onlyOwner {
        require(mintStart < _mintStart, 'Invalid mint start.');
        mintStart = _mintStart;
    }
    ...
    function setRoyalty(uint16 _royalty) external override onlyOwner {
        require(_royalty >= 0 && _royalty <= 750, 'Royalty must be 0-7.5%.');
        royalty = _royalty;
    }
    ...
    function setMarketplaceApproval(address _marketplace, bool _allowed) external override onlyOwner {
        emit MarketplaceApprovalUpdated(_marketplace, _allowed, royalties.marketplaces[_marketplace]);
        royalties.marketplaces[_marketplace] = _allowed;
    }
    ...
    function setErc20RoyaltyTokens(address[] memory _royaltyERC20Tokens) external override onlyOwner {
        royalties.tokens = _royaltyERC20Tokens;
    }

    /// @dev Set the token id hasher address
    function setTokenIdHasherAddress(address _tokenIdHasherAddress) external override onlyOwner {
        __tokenIdHasher = IERC721TokenIdHasher(_tokenIdHasherAddress);
    }

    /// @dev Set the metadata provenance
    function setProvenanceHash(bytes32 _provenanceHash) external override onlyOwner {
        require(PROVENANACE != 0x0 && _provenanceHash != 0x0, 'Provenance must not be empty');
        PROVENANACE = _provenanceHash;
    }

    // INTERNAL

    // @dev get a PRNG
    /// @param _max The exclusive upper bound of the random number
    function _prng(uint256 _max) internal returns (uint256) {
        return
            uint256(keccak256(abi.encodePacked(block.timestamp, ++nonce, msg.sender, blockhash(block.number - 1)))) %
            _max;
    }

    /// @dev mint the tokens including bonus, randomly ordered
    /// @param _quantity the quantity to mint
    function _mintBeardies(uint256 _quantity) internal {
        uint256 _refund = 0;
        bool _bonus = false;
        ...
        if (tokenIds.length < _quantity) {
            ...
            uint256 _newQuantity = tokenIds.length;
            // return the extra
            _refund = (_quantity - _newQuantity) * MINT_PRICE;
            // update
            _quantity = _newQuantity;
        }
        // if the quantity is the max, add one as a bonus, unless it would exceed the max
        else if (_quantity == MAX_PER_TX && tokenIds.length > _quantity) {
            _quantity++;
            _bonus = true;
        }
        ...
        for (uint8 i = 0; i < _quantity; i++) {
            // get one of the token IDs
            uint256 index = _prng(tokenIds.length);
            uint16 _tokenId = tokenIds[index];
            // mint this token ID
            _safeMint(msg.sender, _tokenId);
            // overwrite this index with the last token ID
            tokenIds[index] = tokenIds[tokenIds.length - 1];
            // remove the last token ID
            tokenIds.pop();
            // the last one is the bonus
            if (_bonus && i == _quantity - 1) {
                emit BonusTokenCreated(msg.sender, _tokenId);
            }
        }

        if (_refund > 0) {
            (bool success, ) = payable(msg.sender).call{value: _refund}('');
    ...
    /// @dev get a URI using the base URI
    function _getURI(string memory _item) internal view returns (string memory) {
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(_baseURI(), _item)) : _item;
    }
    ...
    function _verifyMerkleProof(
        ...
        bytes32 _node = keccak256(abi.encodePacked(_tokenId, _metadata));
    ...
    receive() external payable virtual {
        ...
        for (uint256 i = 0; i < royalties.tokens.length; i++) {
            // send the tokens to the recipients
            IERC20 _token = IERC20(royalties.tokens[i]);
            if (_token.balanceOf(address(this)) > 0) {
                this.withdrawTokens(royalties.tokens[i]);
            }
        }
...
import './IERC721SplitWithdrawals.sol';

interface IPaintswapBeardies is IERC721Enumerable {
    struct SlothParams {
        // randomized minting
        uint16 offset;
        // randomized minting
    ...
    struct ERC2981 {
        IERC721SplitWithdrawals forwarder;
        address[] tokens;
    ...
    function mintBeardies(uint256 _quantity) external payable;
    ...
    function setTokenIdHasherAddress(address _tokenIdHasherAddress) external;

    function setProvenanceHash(bytes32 _provenanceHash) external;
}
...
abstract contract ERC721PresetMinterPauserAutoIdRoyalty is ERC2981Royalties, ERC721PresetMinterPauserAutoId {
    ...
    /// @notice Minting is not allowed the default mint() function
...
contract RoyaltiesForwarder is ERC721SplitWithdrawals {
    // The main ERC721 contract
    IPaintswapBeardies internal immutable token;
    ...
    constructor(
        ...
        token = IPaintswapBeardies(_token);
    }
    ...
    receive() external payable virtual {
        // check ERC20 token balances while we have the chance, if we send FTM it will forward them
        address[] memory _tokens = token.getRoyaltyERC20Tokens();
