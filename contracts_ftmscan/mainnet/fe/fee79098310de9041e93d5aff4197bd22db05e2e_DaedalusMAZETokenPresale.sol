...
contract DaedalusMAZETokenPresale is ERC20 {
    ...
    function buyPreMAZE(uint256 _amount) external {
        require((_amount > 0) && (_amount <= 2500000000000000000000) && (whitelist[msg.sender]) && (amountPurchased[msg.sender] <= 2500000000000000000000), "PreMaze: Invalid MIM input or already participated");
        require(active == true, "PreMaze: Presale is not active");
        MIM.transferFrom(msg.sender, address(this), _amount);

        require(amountPurchased[msg.sender] + _amount <= 2500000000000000000000, "PreMaze: Exceeds limit of 2500 MIM tokens");
