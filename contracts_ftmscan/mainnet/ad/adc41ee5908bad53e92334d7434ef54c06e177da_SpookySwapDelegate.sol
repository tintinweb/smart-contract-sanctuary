...
// File: ..\Contracts\adapters\FTM\SpookySwap\SpookySwapDelegate.sol
...
contract SpookySwapDelegate{
    ...
    function deposit(address _adapter, uint256 poolId, uint256 amount) external {
        ...
        lpToken.approve(address(spookyFarm), uint256(amount));
        spookyFarm.deposit(poolId, amount);
    }

    function withdraw(address, uint256 poolId, uint256 amount) external {
        spookyFarm.withdraw(poolId, amount);
    }

    function claimReward(address, uint256 poolId) external {
        spookyFarm.deposit(poolId, 0);
    }
