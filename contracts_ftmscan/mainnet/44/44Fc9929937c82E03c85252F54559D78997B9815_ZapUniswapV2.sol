...
contract ZapUniswapV2 {
    ...
    constructor(address _WETH) {
        require(_WETH != address(0), "BiShares: WETH is zero address");
    ...
    function zapIn(
        ...
        IERC20(WETH).safeTransferFrom(caller, this_, valueAmount);
        ...
        vaultAmount = vault_.balanceOf(this_);
        require(vaultAmount >= vaultAmountOutMin, "BiShares: Vault amount too low");
        vault_.safeTransfer(caller, vaultAmount);
        _returnAssets(path);
    }
    ...
    function zapOut(
        ...
        address token1 = pair.token1();
        address[] memory path = new address[](2);
        path[1] = _checkSwapTarget(router_, desiredToken, token0, token1);
        path[0] = path[1] == token1 ? token0 : token1;
    ...
    function _checkSwapTarget(
        ...
        uint256 tokenIn0Reserve = 0;
        uint256 tokenIn1Reserve = 0;
    ...
    function _getSwapAmount(
        ...
        uint256 nominator;
        try router.getAmountOut(halfInvestment, reserveA, reserveB) returns (uint256 nominator_) {
            nominator = nominator_;
        }
        catch {
            try router.getAmountOut(
                halfInvestment,
                reserveA,
                reserveB,
                path[0],
                path[1]
            ) returns (uint256 nominator_) {
                nominator = nominator_;
            }
            catch {
                uint256 swapFee = IUniswapV2Pair(
                ...
                nominator = router.getAmountOut(halfInvestment, reserveA, reserveB, swapFee);
            }
        }
    ...
    function _getVaultPair(
        IUniswapV2Router02 router,
        address vault
    ) private view returns (IVault vault_, IUniswapV2Pair pair) {
        vault_ = IVault(vault);
        pair = IUniswapV2Pair(address(vault_.token()));
    ...
    function _returnAssets(address[] memory tokens) private {
        ...
        for (uint256 i; i < tokens.length; i++) {
            balance = IERC20(tokens[i]).balanceOf(this_);
            if (balance > 0) {
