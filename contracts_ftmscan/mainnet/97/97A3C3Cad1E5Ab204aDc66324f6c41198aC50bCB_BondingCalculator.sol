...
// File contracts/libraries/SafeSquareRoot.sol


library SafeSquareRoot {
  function SqRt(uint256 a) internal pure returns (uint256 c) {
    if (a > 3) {
      c = a;
      uint256 b = SafeMath.add(SafeMath.div(a, 2), 1);
      while (b < c) {
        c = b;
        b = SafeMath.div(SafeMath.add(SafeMath.div(a, b), b), 2);
      }
    } else if (a != 0) {
      c = 1;
    }
...
// File contracts/BondingCalculator/BondingCalculator.sol
...
contract BondingCalculator is IBondingCalculator {
  ...
  using SafeSquareRoot for uint256;
  using SafeSquareRoot for uint112;
