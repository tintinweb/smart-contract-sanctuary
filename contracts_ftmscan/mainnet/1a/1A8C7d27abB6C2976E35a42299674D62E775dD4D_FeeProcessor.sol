...
contract FeeProcessor is Ownable, ReentrancyGuard, IFeeProcessor {
    ...
    IERC20 immutable TAROT;
    IERC20 immutable WETH;

    IClaimable m_claimable;
    ...
    function addBridgeToken(IERC20 token, IERC20 bridgeToken) external override onlyOwner {
        require(token != TAROT, "FeeProcessor: INVALID_TOKEN_TAROT");
    ...
    function setClaimable(IClaimable claimable) public override onlyOwner {
    ...
    function processBorrowable(IBorrowable borrowable, address to) public override nonReentrant {
        ...
        // TAROT balance before any redemption
        uint256 tarotBalanceBefore = TAROT.balanceOf(address(this));
...

        {
            ...
            // Convert whatever we received to TAROT
            _toTAROT(token, tokenAmount);
        }

        uint256 tarotBalanceAfter = TAROT.balanceOf(address(this));
        // Net amount to use for reward
        uint256 tarotBalanceChange = tarotBalanceAfter.sub(tarotBalanceBefore);
        // Now have TAROT so send reward to caller and rest to fee distributor

        uint256 reward = _getReward(info); // Reward rate for this borrowable
        uint256 rewardAmount = tarotBalanceChange.mul(reward).div(REWARD_SCALE);
        if (rewardAmount > 0) {
            TAROT.safeTransfer(to, rewardAmount);
        }
        uint256 tarotAmount = tarotBalanceAfter.sub(rewardAmount);
        TAROT.safeTransfer(address(m_feeDistributor), tarotAmount);

        emit ProcessBorrowable(msg.sender, address(borrowable), tarotAmount, rewardAmount);
    }

    function _toTAROT(IERC20 token, uint256 amount) private returns (uint256 tarotOut) {
        if (token == TAROT) {
            // We have TAROT so no-op
            tarotOut = amount;
        } else {
            IERC20 bridgeToken = getBridgeToken(token);
            if (bridgeToken == TAROT) {
                // Swap for TAROT and we are done
                tarotOut = _doOptimalSwap(token, TAROT, amount, address(this));
            } else {
                ...
                tarotOut = _toTAROT(bridgeToken, bridgeAmount);
            }
        }
    }
...
interface IFeeDistributor {
    function tarot() external view returns (address);

    function xTAROT() external view returns (address);
