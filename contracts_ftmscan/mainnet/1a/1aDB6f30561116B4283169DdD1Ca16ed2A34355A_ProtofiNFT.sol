...
contract Whitelistable is IWhitelistable, Ownable {
    ...
    event Blacklisted(address indexed _addr);
    ...
    modifier onlyWhitelist() {
        require(whitelisted(msg.sender), "Whitelist: address not whitelisted" );
        _;
    }
    function whitelisted(address _addr) public view override returns (bool) {
        return !!whitelist[_addr];
    }
    function addWhitelist( address _addr ) public override onlyOwner() {
        require( _addr != address(0), "Whitelist: address invalid" );
        whitelist[_addr] = true;
        emit Whitelisted(_addr);
    }
    function addBlacklist( address _addr ) public override onlyOwner() {
        require( _addr != address(0), "Whitelist: address invalid" );
