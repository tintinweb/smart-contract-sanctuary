...
contract BondDepository is Ownable {
    ...
    function initializeBondTerms( 
        ...
        lastDecay = uint256(block.timestamp);
    }
  ...
  /* ======== USER FUNCTIONS ======== */
    function test0( 
        ...
        termsmaxDebt = terms.maxDebt ; // 100000 
        ...
        // 1000000000000000001 - 2000000000000000242 - 20000000000000002
    }
    function test( 
        uint _amount, 
        uint _maxPrice,
    ...
    function redeem( address _recipient, bool _stake ) external returns ( uint ) {        
        ...
        if ( percentVested >= 10000 ) { // if fully vested
            ...
            bondInfo[ _recipient ] = Bond({
                ...
                vesting: info.vesting.sub( uint256( block.timestamp ).sub( info.lastTime ) ),
                lastTime: uint256(block.timestamp),
    ...
    function adjust() internal {
        ...
        if( adjustment.rate != 0 && block.timestamp >= timeCanAdjust ) {
            ...
            adjustment.lastTime = uint256(block.timestamp);
    ...
    function debtDecay() public view returns ( uint decay_ ) {
        uint256 timeSinceLast = uint256(block.timestamp).sub( lastDecay );
    ...
    function percentVestedFor( address _depositor ) public view returns ( uint percentVested_ ) {
        ...
        uint secondsSinceLast = uint256(block.timestamp).sub( bond.lastTime );
