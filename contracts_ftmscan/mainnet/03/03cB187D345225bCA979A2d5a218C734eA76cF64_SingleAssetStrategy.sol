...
contract SingleAssetStrategy is Ownable, Pausable {
    ...
    function setRouters(address _unirouter, address _sellRouter) external onlyOwner {
        unirouter = _unirouter;
        sellRouter = _sellRouter;
    }
    ...
    function setOutputToWrappedRoute(address[] memory _route, address router) external {
        require(msg.sender == strategist, "!auth");
