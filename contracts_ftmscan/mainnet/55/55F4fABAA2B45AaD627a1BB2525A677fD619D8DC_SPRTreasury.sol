...
contract SPRTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, xSPR }
    ...
    address public xSPR;
    uint public xSPRQueue; // Delays change to xSPR address
    ...
    constructor (
        ...
        nativeFloorPrice[ _MIM ] = 1000000000;
        isReserveToken[ _USDC ] = true;
        reserveTokens.push( _USDC );
        nativeFloorPrice[ _USDC ] = 1000000000;
        isLiquidityToken[ _SPRUSDC] = true;
        liquidityTokens.push( _SPRUSDC );
    ...
    /**
        @notice allow approved address to deposit an asset for SPR
        @param _amount uint
    ...
    */
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SPR needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SPR ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SPR for reserves
        @param _amount uint
        @param _token address
    */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISPRERC20( SPR ).burnFrom( msg.sender, value );
    ...
    */
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( xSPR ).balanceOf( msg.sender ); // Can only borrow against xSPR held
    ...
    */
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SPR
        @param _amount uint
    */
    function repayDebtWithSPR( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISPRERC20( SPR ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SPR, _amount, _amount );
    }
    ...
    */
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SPR ).mint( _recipient, _amount );
    ...
    */
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SPR ).totalSupply().sub( totalDebt ) );
    }
    ...
    */
    ...
    /**
        @notice returns SPR valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
    */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SPR decimals and divide by number of tokens for 1 SPR
            value_ = _amount.mul( 10 ** IERC20( SPR ).decimals() ).div( 10 ** IERC20( _token ).decimals() ).mul(10 ** 9).div(nativeFloorPrice[_token]);
    ...
    /**
        @notice sets SPR valuation of asset
        @param _token address
        @param _value uint
    */
    ...
    */
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.xSPR ) { // 9
            xSPRQueue = block.number.add( blocksNeededForQueue );
        } else return false;
