...
/**
 *Submitted for verification at FtmScan.com on 2021-11-05
*/
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 constant MAXSUPPLY = 9250 ether;
...
// DrcToken with Governance.
contract DrcToken is BEP20("Draco Cave", "CAVE") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "DRC::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "DRC::delegateBySig: invalid nonce"
        );
...
// MasterChef is the master of DRC. He can make Drc and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once DRC is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DRC
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDrcPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDrcPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DRCs to distribute per block.
        uint256 lastRewardBlock; // Last block number that DRCs distribution occurs.
        uint256 accDrcPerShare; // Accumulated DRCs per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The DRC TOKEN!
    DrcToken public cave;
    // Dev address.
    address public devaddr;
    // DRC tokens created per block.
    uint256 public cavePerBlock;
    // Deposit Fee address
    ...
    // The block number when Drc mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 DrcPerBlock);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _DrcPerBlock,
        uint256 _startBlock
    ) public {
        cave = _drc;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        cavePerBlock = _DrcPerBlock;
    ...
    // Update the given pool's Drc allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending Drcs on frontend.
    function pendingDrc(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 drcReward = multiplier
                .mul(cavePerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accDrcPerShare = accDrcPerShare.add(
                drcReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return
            user.amount.mul(accDrcPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 drcReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        cave.mint(devaddr, drcReward.div(10));
        cave.mint(address(this), drcReward);
    ...
    // Deposit LP tokens to MasterChef for Drc allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accDrcPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                safeDrcTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accDrcPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accDrcPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeDrcTransfer(msg.sender, pending);
    ...
    // Safe drc transfer function, just in case if rounding error causes pool to not have enough DRCs.
    function safeDrcTransfer(address _to, uint256 _amount) internal {
        uint256 caveBal = cave.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > caveBal) {
            transferSuccess = cave.transfer(_to, caveBal);
        } else {
            transferSuccess = cave.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDrcTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _cavePerBlock) external onlyOwner {
        require(_cavePerBlock <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        cavePerBlock = _cavePerBlock;
        emit UpdateEmissionRate(msg.sender,cavePerBlock);
    }
