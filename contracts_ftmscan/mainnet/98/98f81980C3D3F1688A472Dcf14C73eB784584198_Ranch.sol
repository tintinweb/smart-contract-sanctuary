...
/*
    http://ranch.money
*/
contract Ranch is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 RANCH-WFTM LP -> RANCH pool
    uint256 public constant INITIAL_RANCH_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the RANCH ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("RANCH", "RANCH") {
        // Mints 1 RANCH to contract creator for initial pool setup
    ...
    function _getRanchPrice() internal view returns (uint256 _ranchPrice) {
        try IOracle(ranchOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Ranch: failed to fetch RANCH price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ranchPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ranchPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRanchOracle(address _ranchOracle) public onlyOperatorOrTaxOffice {
        require(_ranchOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints RANCH to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of RANCH to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRanchPrice = _getRanchPrice();
            currentTaxRate = _updateTaxRate(currentRanchPrice);
            if (currentRanchPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_ranchPool != address(0), "!_ranchPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_ranchPool, INITIAL_RANCH_POOL_DISTRIBUTION);
    }
