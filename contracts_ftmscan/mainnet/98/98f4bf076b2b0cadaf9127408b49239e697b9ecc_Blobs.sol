...
contract Blobs is ERC721Enumerable, Ownable, RandomlyAssigned {
  using Strings for uint256;
  
  string public baseExtension = ".json";
  uint256 public cost = 0.015 ether; 
  uint256 public maxBOB = 45; 
  uint256 public maxBOBMintAmount = 10; 
  bool public paused = false;
  
  string public baseURI = "https://ipfs.io/ipfs/QmeTSEhL5Ai6kq11sF5zHjyv2ke5G6eZmsdomqKR9EeGd9";
  
  constructor(
  ) ERC721("Blobs", "BOB")
  RandomlyAssigned(45, 1) {}
  ...
  // public
    function mintBOBS(uint256 _BOBMintAmount) public payable {
    require(!paused);
    require(_BOBMintAmount > 0);
    require(_BOBMintAmount <= maxBOBMintAmount);
    require(totalSupply() + _BOBMintAmount <= maxBOB);
    require(msg.value >= cost * _BOBMintAmount);

    for (uint256 i = 1; i <= _BOBMintAmount; i++) {
        uint256 mintIndex = nextToken();
     if (totalSupply() < maxBOB) {
