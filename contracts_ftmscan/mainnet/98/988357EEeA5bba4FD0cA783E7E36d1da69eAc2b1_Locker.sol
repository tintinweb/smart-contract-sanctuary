...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        require(masonShare >= amount, "Locker: withdraw request greater than staked amount");
...
contract Locker is ShareWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "Locker: caller is not the operator");
        _;
    }

    modifier masonExists {
        require(balanceOf(msg.sender) > 0, "Locker: The mason does not exist");
        _;
    }
    ...
    modifier notInitialized {
        require(!initialized, "Locker: already initialized");
        _;
    }
    ...
    function stake(uint256 amount) public override onlyOneBlock updateReward(msg.sender) {
        require(amount > 0, "Locker: Cannot stake 0");
        super.stake(amount);
    ...
    function withdraw(uint256 amount) public override onlyOneBlock masonExists updateReward(msg.sender) {
        require(amount > 0, "Locker: Cannot withdraw 0");
        require(masons[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <= treasury.epoch(), "Locker: still in withdraw lockup");
        claimReward();
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            require(masons[msg.sender].epochTimerStart.add(rewardLockupEpochs) <= treasury.epoch(), "Locker: still in reward lockup");
            masons[msg.sender].epochTimerStart = treasury.epoch(); // reset timer
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        require(amount > 0, "Locker: Cannot allocate 0");
        require(totalSupply() > 0, "Locker: Cannot allocate when totalSupply is 0");
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        ...
        require(address(_token) != address(share), "dJones");
