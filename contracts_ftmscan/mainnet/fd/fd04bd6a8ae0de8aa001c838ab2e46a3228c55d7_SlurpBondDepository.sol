...
contract SlurpBondDepository is Ownable {
    ...
    address public immutable SLURP; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints SLURP when receives principle
    ...
    struct Bond {
        uint payout; // SLURP remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 SLURP ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) SLURP
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( SLURP ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( SLURP ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( SLURP ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( SLURP ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( SLURP ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to SLURP supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( SLURP ).totalSupply();
    ...
    /**
     *  @notice calculate amount of SLURP available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or SLURP) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != SLURP );
