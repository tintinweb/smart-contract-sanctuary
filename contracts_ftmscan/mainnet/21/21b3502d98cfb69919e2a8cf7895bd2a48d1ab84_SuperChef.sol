...
contract SuperChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime;  // Last block timestamp that dmds distribution occurs.
    ...
    // The block timestamp when dmd mining starts.
    uint256 public startTime;
    // The block timestamp when dmd mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 dmdReward = multiplier.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    /**
     * @dev Rescues random FTM funds stuck.
     */
    function inCaseFTMGetStuck() external onlyOwner {
        require(address(this).balance > 0, "non FTM exists");
