...
import "../base/MasterChefBaseStrategy.sol";

contract SpiritFantomDaiUsdcStrategy is MasterChefBaseStrategy {
    constructor() public {}

    function initialize(address _vault, address _parentStrategy)
        public
        initializer
    {
        ...
        uint256 poolId = 62; // DaiUsdc Pool id
        address spirit = address(0x5Cc61A78F164885776AA610fb0FE1257df78E59B);

        MasterChefBaseStrategy.initializeMasterChefBaseStrategy(
            _vault,
            _parentStrategy,
            underlying,
            address(0x9083EA3756BDE6Ee6f27a6e996806FBD37F6F093), // SpiritSwap MasterChef Staking Pool
            address(0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52), // SpiritSwap Router
            spirit,
            poolId, // Pool id
            1e17 // Sell Floor
        );

        address wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);

        address dai = address(0x8D11eC38a3EB5E956B052f67Da8Bdc9bef8Abf3E);
        address usdc = address(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
        address usdt = address(0x049d68029688eAbF473097a2fC38ef61633A3C7A);

        rewardRoutes[dai] = [spirit, wftm, usdc, dai];
        rewardRoutes[usdc] = [spirit, wftm, usdc];

        fromVaultAssetRoutes[dai][usdc] = [dai, usdc];
        fromVaultAssetRoutes[usdt][dai] = [usdt, wftm, usdc, dai];
        fromVaultAssetRoutes[usdt][usdc] = [usdt, wftm, usdc];
        fromVaultAssetRoutes[usdc][dai] = [usdc, dai];

        toVaultAssetRoutes[dai][usdc] = [usdc, dai];
        toVaultAssetRoutes[usdt][dai] = [dai, usdc, wftm, usdt];
        toVaultAssetRoutes[usdt][usdc] = [usdc, wftm, usdt];
        toVaultAssetRoutes[usdc][dai] = [dai, usdc];

        underlyingSellFloors[dai] = 1e18;
        underlyingSellFloors[usdc] = 1e6;
        underlyingSellFloors[usdt] = 1e6;
    }
}
...
contract MasterChefBaseStrategy is OwnableUpgradeable {
    ...
    mapping(address => uint256) public underlyingSellFloors;
    ...
    modifier restricted() {
        require(
            msg.sender == vault ||
                msg.sender == owner() ||
                msg.sender == parentStrategy,
            "The sender must be the owner or vault or parent strategy"
        );
        _;
    }

    function _enterRewardPool() internal {
        uint256 _balanceNotEntered = IERC20(underlying).balanceOf(
            address(this)
        );
        IMasterChef(masterChef).deposit(poolId, _balanceNotEntered);
    }

    function _exitRewardPool() internal {
        uint256 _bal = rewardPoolUnderlyingBalance();
        if (_bal != 0) {
            IMasterChef(masterChef).withdraw(poolId, _bal);
        }
    }

    function emergencyExitRewardPool() external onlyOwner {
        uint256 _bal = rewardPoolUnderlyingBalance();
        if (_bal != 0) {
    ...
    function _assetToUnderlying(
        ...
        if (_bal > underlyingSellFloors[_asset]) {
            address _token0 = IUniswapV2Pair(underlying).token0();
            address _token1 = IUniswapV2Pair(underlying).token1();
            ...
            (, , _liquidity) = IUniswapV2Router02(router).addLiquidity(
                _token0,
                _token1,
                _token0Amount,
                _token1Amount,
    ...
    function withdrawAllToVault(address _asset) public restricted {
        ...
        uint256 _assetBal = IERC20(_asset).balanceOf(address(this));

        if (_assetBal > 0) {
            IERC20(_asset).safeTransfer(vault, _assetBal);
        }
    }
    ...
    function rewardPoolUnderlyingBalance() public view returns (uint256 _bal) {
        (_bal, ) = IMasterChef(masterChef).userInfo(poolId, address(this));
    }
    ...
    function underlyingBalance() public view returns (uint256) {
        ...
        return
            rewardPoolUnderlyingBalance().add(
    ...
    function investedBalanceInUSD() public view returns (uint256) {
        uint256 _underlyingBal = underlyingBalance();
        return getUSDBalanceFromUnderlyingBalance(_underlyingBal);
    }
    ...
    function setSellFloor(uint256 _sellFloor) external onlyOwner {
        sellFloor = _sellFloor;
    }

    function setParentStrategy(address _parentStrategy) external onlyOwner {
        parentStrategy = _parentStrategy;
    }
