...
contract PUMPBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable PUMP; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints PUMP when receives principle
    ...
    struct Bond {
        uint payout; // PUMP remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 PUMP ( underflow protection )
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( PUMP ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( PUMP ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( PUMP ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( PUMP ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to PUMP supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( PUMP ).totalSupply();
    ...
    /**
     *  @notice calculate amount of PUMP available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or PUMP) to the DAO
     *  @return bool
     */
