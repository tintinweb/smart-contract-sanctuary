...
interface IBoardroom {
    ...
    function getChronusPrice() external view returns (uint256);
...
contract Treasury is ContractGuard {
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of CHRONUS price
    uint256 public bootstrapEpochs;
    uint256 public bootstrapSupplyExpansionPercent;

    /* =================== Added variables =================== */
    uint256 public previousEpochChronusPrice;
    uint256 public maxDiscountRate; // when purchasing bond
    uint256 public maxPremiumRate; // when redeeming bond
    uint256 public discountPercent;
    uint256 public premiumThreshold;
    uint256 public premiumPercent;
    uint256 public mintingFactorForPayingDebt; // print extra CHRONUS during debt phase
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getChronusPrice() > kittyPriceCeiling) ? 0 : getChronusCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    // oracle
    function getChronusPrice() public view returns (uint256 kittyPrice) {
        try IOracle(kittyOracle).consult(kitty, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult CHRONUS price from the oracle");
        }
    }

    function getChronusUpdatedPrice() public view returns (uint256 _kittyPrice) {
    ...
    function getBurnableChronusLeft() public view returns (uint256 _burnableChronusLeft) {
        uint256 _kittyPrice = getChronusPrice();
        if (_kittyPrice <= kittyPriceOne) {
            uint256 _kittySupply = getChronusCirculatingSupply();
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableChronus = _maxMintableBond.mul(_kittyPrice).div(1e18);
                _burnableChronusLeft = Math.min(epochSupplyContractionLeft, _maxBurnableChronus);
            }
        }
    }
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_kittyPrice > kittyPriceCeiling) {
            uint256 _totalChronus = IERC20(kitty).balanceOf(address(this));
            uint256 _rate = getBondPremiumRate();
            if (_rate > 0) {
                _redeemableBonds = _totalChronus.mul(1e18).div(_rate);
            }
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_kittyPrice <= kittyPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = kittyPriceOne.mul(1e18).div(_kittyPrice); // to burn 1 CHRONUS
    ...
    function initialize(
        ...
        kittyPriceOne = 10**18; // This is to allow a PEG of 1 CHRONUS per AVAX
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn CHRONUS and mint tBOND)
    ...
    function setChronusOracle(address _kittyOracle) external onlyOperator {
        kittyOracle = _kittyOracle;
    }

    function setChronusPriceCeiling(uint256 _kittyPriceCeiling) external onlyOperator {
    ...
    function _updateChronusPrice() internal {
        try IOracle(kittyOracle).update() {} catch {}
    }

    function getChronusCirculatingSupply() public view returns (uint256) {
    ...
    function buyBonds(uint256 _kittyAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 kittyPrice = getChronusPrice();
        require(kittyPrice == targetPrice, "Treasury: CHRONUS price moved");
        require(
        ...
        uint256 kittySupply = getChronusCirculatingSupply();
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        _updateChronusPrice();
        previousEpochChronusPrice = getChronusPrice();
        uint256 kittySupply = getChronusCirculatingSupply().sub(seigniorageSaved);
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _sendToBoardroom(kittySupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochChronusPrice > kittyPriceCeiling) {
                // Expansion ($CHRONUS Price > 1 $ETH): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bbond).totalSupply();
                uint256 _percentage = previousEpochChronusPrice.sub(kittyPriceOne);
