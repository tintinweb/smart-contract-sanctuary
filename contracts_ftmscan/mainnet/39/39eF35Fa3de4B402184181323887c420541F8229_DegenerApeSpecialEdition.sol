...
contract DegenerApeSpecialEdition is ERC721, ERC2981Collection, IMAX721, ERC165Storage, Developer, Ownable {
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIdCounter;
  uint256 private ownerMintSize;
  string private base;
  ...
  constructor() ERC721("DegenerApe SE", "DA-SE") {
 ...
 */
  ...
  function ownerMint(address _address) public onlyOwner {
    require(ownerMintSize != 0, "Owner minting not enabled");
 ...
 */
  ...
  function setRoyaltyInfo(address _royaltyAddress, uint256 _percentage) public onlyDev {
  ...
  function setMintSize(uint256 _amount) public onlyDev {
    uint256 old = ownerMintSize;
    ownerMintSize = _amount;
    emit UpdatedMintSize(old, ownerMintSize);
  }
  ...
  function minterStatus() external pure override(IMAX721) returns (bool) {
    return true;
  }

  // @notice will return minting fees
  function minterFees() external pure override(IMAX721) returns (uint256) {
    return 0;
  }

  // @notice will return maximum mint capacity
  function minterMaximumCapacity() external view override(IMAX721) returns (uint256) {
    return ownerMintSize;
  }

  // @notice will return maximum "team minting" capacity
  function minterMaximumTeamMints() external pure override(IMAX721) returns (uint256) {
    return 0;
  }
  // @notice will return "team mints" left
  function minterTeamMintsRemaining() external pure override(IMAX721) returns (uint256) {
    return 0;
  }

  // @notice will return "team mints" count
  function minterTeamMintsCount() external pure override(IMAX721) returns (uint256) {
    return 0;
  }
