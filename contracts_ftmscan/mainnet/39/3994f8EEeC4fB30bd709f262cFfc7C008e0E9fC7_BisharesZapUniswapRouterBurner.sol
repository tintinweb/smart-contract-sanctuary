...
contract BisharesZapUniswapRouterBurner {
  ...
  function burnForAllTokens(
    ...
    return _swapTokens(vaults, routers);
  }
  ...
  function swapForTokens(
    ...
    for (uint256 i = 0; i < vaults.length; i++) {
      amountOutTotal += IZapUniswapV2(zapper).zapOutFinish(
        caller,
        routers[i],
        vaults[i],
        tokenOut
    ...
    TransferHelper.safeTransfer(tokenOut, caller, amountOutTotal);
  }

  function swapForETH(address indexPool) external returns (uint256 amountOutTotal) {
    address caller = msg.sender;
    address[] memory vaults = IIndexPool(indexPool).getCurrentTokens();
    address[] memory routers = IIndexPool(indexPool).routers();
    for (uint256 i = 0; i < vaults.length; i++) {
    ...
    TransferHelper.safeTransferETH(caller, amountOutTotal);
  }
  ...
  function _swapTokens(
    ...
    for (uint256 i = 0; i < vaults.length; i++) {
      ...
      IZapUniswapV2(zapper).zapOutStart(
        caller,
        routers[i],
        vaults[i],
        amount
...
interface IZapUniswapV2 {
    function residualsStorage() external view returns (address);
