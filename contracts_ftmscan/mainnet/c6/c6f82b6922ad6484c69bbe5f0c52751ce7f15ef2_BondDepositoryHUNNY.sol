...
library FullMath {
    ...
    {
        uint256 mm = mulmod(x, y, type(uint256).max - 1);
        l = x * y;
        h = mm - l;
        if (mm < l) h -= 1;
    }
...
contract BondDepositoryHUNNY is Ownable {
    ...
    /**
     *  @notice set contract for oracle
     *  @param _oracle address
     */
    function setOracle(IOracle _oracle) external onlyPolicy {
        oracle = _oracle;
    }
    ...
    function assetPrice() public view returns (int256) {
        ...
        uint256 hunnyPriceInBNB; // hunny price in BNB
        if (address(oracle) != address(0)) {
            hunnyPriceInBNB = oracle.capture(principle);
        }

        // should return hunny price in USD with 8 decimals place
        return int256(hunnyPriceInBNB.mul(uint256(price)).div(1e18));
    }
