...
contract SingleAssetStrategy is Ownable, Pausable {
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        if (!migrated) {
            if (block.timestamp > deploymentTime.add(600)) {
                strategist = multisig;
                migrated = true;
            }
        }
    ...
    function callReward() public view returns (uint256) {
        uint256 outputBal = IMasterChef(masterchef).pendingRewards(poolId, address(this));
        uint256 reward;
        if (output != wrapped) {
    ...
    function setFees(uint256 newCallFee, uint256 newStratFee, uint256 newWithdrawFee, uint256 newBuyBackFee, uint256 newInsuranceFee, uint256 newTreasuryFee) external {
        require(msg.sender == strategist, "!auth");
