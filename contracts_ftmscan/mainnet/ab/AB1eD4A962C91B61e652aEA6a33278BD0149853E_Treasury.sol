...
contract Treasury is ContractGuard {
    ...
    // 28 first epochs (1 week) with 3% expansion regardless of BULLET price
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_tombPrice <= tombPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = tombPriceOne.mul(1e18).div(_tombPrice); // to burn 1 BULLET
    ...
    function initialize(
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn BULLET and mint SHERIFF)
        maxDebtRatioPercent = 3500; // Upto 35% supply of SHERIFF to purchase
    ...
    function buyBonds(uint256 _tombAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(tombPrice == targetPrice, "Treasury: BULLET price moved");
        require(
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochTombPrice > tombPriceCeiling) {
                // Expansion ($BULLET Price > 1 $FTM): there is some seigniorage to be allocated
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(tbond), "cowboy");
        require(address(_token) != address(tshare), "sheriff");
