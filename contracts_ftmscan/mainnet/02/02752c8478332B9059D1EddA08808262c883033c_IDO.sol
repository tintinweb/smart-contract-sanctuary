...
contract IDO is ReentrancyGuard {
  ...
  // statut type
  enum Status {inactive, active, completed}      // 0, 1, 2
  ...
  function getStatus() public view returns(Status)  {
    ...
    else if (block.timestamp <= startTime || startTime == 0)
    {
  ...
  function updateStartTime(uint256 _startTime) public onlyAdmin {
    require (_startTime < startTime, 'no');
    startTime = _startTime;
  }
  
  function updateEndTime(uint256 _endTime) public onlyAdmin {
    endTime = _endTime;
  }
