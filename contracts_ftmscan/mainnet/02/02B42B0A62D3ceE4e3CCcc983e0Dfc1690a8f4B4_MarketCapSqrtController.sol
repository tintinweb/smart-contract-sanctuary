...
contract MarketCapSqrtController is MarketCapSortedTokenCategories {
  ...
  function finishPreparedIndexPool(
    ...
    require(factories.length == len && balances.length == len, "BiShares: Invalid arrays length");
  ...
  function reindexPool(address poolAddress) external returns (bool) {
    ...
    require(block.timestamp - meta.lastReweigh >= POOL_REWEIGH_DELAY, "BiShares: Pool reweigh delay overflow");
...
contract MarketCapSortedTokenCategories is OwnableProxy {
  ...
  function computeAverageMarketCap(
    ...
    uint256 vaultPrice = vault.getPricePerFullShare();
    uint256 vaultSupply = lpToken.totalSupply() * PRECISION / vaultPrice;
  ...
  function computeAverageMarketCaps(
    ...
    uint256 len = tokens.length;
    require(factories.length == len, "BiShares: Invalid arrays length");
    uint256[] memory totalSupplies = new uint256[](len);
    for (uint256 i = 0; i < len; i++) {
      IVault vault = IVault(tokens[i]);
      IERC20 lpToken = vault.token();
      uint256 vaultPrice = vault.getPricePerFullShare();
      totalSupplies[i] = lpToken.totalSupply() * PRECISION / vaultPrice;
    }
  ...
  function createCategory(bytes32 metadataHash, IBisharesUniswapV2Oracle oracle) external onlyOwner returns (bool) {
    require(address(oracle) != address(0), "BiShares: Oracle is zero address");
