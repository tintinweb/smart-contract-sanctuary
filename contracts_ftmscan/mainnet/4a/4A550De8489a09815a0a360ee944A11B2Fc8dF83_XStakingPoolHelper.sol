...
import './XStakingPoolController.sol';

contract XStakingPoolHelper is Ownable {
    ...
    XStakingPoolController public xStakingPoolController;
    IERC20 public immutable xTAROT;

    constructor(
      XStakingPoolController _xStakingPoolController,
      IERC20 _xTAROT) {
        xStakingPoolController = _xStakingPoolController;
        xTAROT = _xTAROT;
    }
    ...
    function pidsLength() external view returns (uint256) {
        return pids.length;
    }

    function setXStakingPoolController(XStakingPoolController _xStakingPoolController) external onlyOwner  {
        xStakingPoolController = _xStakingPoolController;
    }

    function setPids(uint[] memory newPids) public onlyOwner {
        delete pids;
        for (uint i = 0; i < newPids.length; i++) {
            pids.push(newPids[i]);
        }
    }

    function getUserStakedXTAROT(address _user) public view returns(uint256 amount) {
        uint256 _amt;
        for (uint256 i = 0; i < pids.length; i++) {
            (_amt,)  = xStakingPoolController.userInfo(pids[i], _user);
            amount += _amt;
        }
    }
    
    function getVotes(address _user) public view returns(uint256 amount) {
        amount = getUserStakedXTAROT(_user) + xTAROT.balanceOf(_user);
    }
}
