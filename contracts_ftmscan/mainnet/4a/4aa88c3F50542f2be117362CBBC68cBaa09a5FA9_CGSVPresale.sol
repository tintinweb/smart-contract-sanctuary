...
contract CGSVPresale is ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // The number of unclaimed CGSV tokens the user has
    mapping(address => uint256) public CGSVUnclaimed;
    // Last time user claimed CGSV
    mapping(address => uint256) public lastCGSVClaimed;
    ...
    // Total CGSV sold
    uint256 public totalCGSVSold = 0;

    // Price of presale CGSV: 1 BuyToken
    ...
    // Max Buy Per User
    uint256 public maxBuyPerUser = 50000*(1e6); // decimal 6

    uint256 public firstHarvestTimestamp;

    address payable owner;

    uint256 public constant CGSV_HARDCAP = 100000*(1e6);
    ...
    function buy(uint256 _amount, address _buyer) public nonReentrant {
        ...
        require(
            totalCGSVSold + tokens <= CGSV_HARDCAP,
            "CGSV presale hardcap reached"
        );

        require(
            CGSVUnclaimed[buyer] + tokens <= maxBuyPerUser,
            "Your amount exceeds the max buy number"
        );

        BuyToken.safeTransferFrom(buyer, address(this), _amount);

        CGSVUnclaimed[buyer] = CGSVUnclaimed[buyer].add(tokens);
        totalCGSVSold = totalCGSVSold.add(tokens);
    ...
    function claim() external {
        ...
        require(
            CGSVUnclaimed[msg.sender] > 0,
            "User should have unclaimed CGSV tokens"
        );
        require(
            CGSV.balanceOf(address(this)) >= CGSVUnclaimed[msg.sender],
            "There are not enough CGSV tokens to transfer."
        );

        if (lastCGSVClaimed[msg.sender] == 0) {
            lastCGSVClaimed[msg.sender] = firstHarvestTimestamp;
        }

        uint256 allowedPercentToClaim = block
        .timestamp
        .sub(lastCGSVClaimed[msg.sender])
        .div(timePerPercent);

        require(
            allowedPercentToClaim > 0,
            "User cannot claim CGSV tokens when Percent is 0%"
        );
        ...
        lastCGSVClaimed[msg.sender] = block.timestamp;

        uint256 CGSVToClaim = CGSVUnclaimed[msg.sender]
        .mul(allowedPercentToClaim)
        .div(100);
        CGSVUnclaimed[msg.sender] = CGSVUnclaimed[msg.sender].sub(CGSVToClaim);

        CGSVToClaim = CGSVToClaim.mul(1e12);
        CGSV.safeTransfer(msg.sender, CGSVToClaim);
        emit TokenClaim(msg.sender, CGSVToClaim);
    }
    ...
    function withdrawUnsoldCGSV() external onlyOwner {
        uint256 amount = CGSV.balanceOf(address(this)) - totalCGSVSold.mul(1e12);
        CGSV.safeTransfer(msg.sender, amount);
    }

    function emergencyWithdraw() external onlyOwner {
        CGSV.safeTransfer(msg.sender, CGSV.balanceOf(address(this)));
    }

    function updateMaxBuyPerUser(uint256 _maxBuyPerUser) external onlyOwner {
        require(_maxBuyPerUser <= CGSV_HARDCAP, "CGSV::updateMaxBuyPerUser: maxBuyPerUser must not exceed the hardcap.");
