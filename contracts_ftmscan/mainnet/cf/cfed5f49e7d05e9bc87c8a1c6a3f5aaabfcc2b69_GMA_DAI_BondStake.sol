...
contract GMA_DAI_BondStake is Ownable {
    ...
    address public immutable GMA; // intermediate reward token from treasury
    address public immutable sGMA; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sGMA gons remaining to be paid
    ...
    function deposit( 
        ...
        IERC20( GMA ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        
        uint stakeGons=ISGMA(sGMA).gonsForBalance(payout);
    ...
    function redeem( address _recipient) external returns ( uint ) {        
        ...
        uint _amount = ISGMA(sGMA).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sGMA ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sGMA balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastTime,uint pricePaid ) {
        ...
        payout=ISGMA(sGMA).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISGMA(sGMA).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
