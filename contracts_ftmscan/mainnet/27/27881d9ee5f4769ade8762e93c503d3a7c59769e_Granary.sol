...
NOTE: ftm.guru merely provides secure auto-compounding vaults for masterchefs. The 0xDAO $OXD is not the real Solidly project, neither associated with it.
...
interface IMasterchef {
	...
	struct PoolInfo {
        ...
        uint256 lastRewardBlock; // Last block number that OXDs distribution occurs.
...
interface IERC20 {
	/**
	 * @dev Returns the decimal depth of tokens.
	 */
...
contract Granary
{
	...
	constructor (address _w, address _m, address _e, uint8 _p, address _R, address[] memory _r, string memory _id, address[2] memory _v)
	{
		want=IERC20(_w);
		mc=IMasterchef(_m);
		earn=IERC20(_e);
	...
	*/
	...
	function tvl() public view returns(uint256)
	{
		ITVL tc = ITVL(utvl);
		uint256 aum = mc.userInfo(allnums[0],address(this)).amount + IERC20(want).balanceOf(address(this));
		return ((tc.p_t_coin_usd(address(tvlp))).mul(aum)).div(10**IERC20(want).decimals());
	}
