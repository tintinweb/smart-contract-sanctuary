...
// MasterChef is the master of TRICKs. He can make TRICKs and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once TRICK is sufficiently
// distributed and the community can show to govern itself.

contract TrickOrTreatBasket is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of TRICK
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTrickPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accTrickPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. TRICKs to distribute per second.
        uint256 lastRewardTime;  // Last timestamp that TRICKs distribution occurs.
        uint256 accTrickPerShare;   // Accumulated TRICKs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The TRICK TOKEN!
    TrickToken public immutable trick;
    // Dev address.
    address public devaddr;
    // TRICK tokens created per timestamp.
    uint256 public trickPerSecond;
    // Deposit Fee address
    ...
    // The timestamp when TRICK distribution starts.
    uint256 public startTime;
    // Maximum TRICK Per Second
    ...
    event UpdateEmissionRate(address indexed user, uint256 trickPerSecond);
    ...
    event trickMintError(bytes reason);

    constructor(
        TrickToken _tokenAddress,
        address _devaddr,
        address _feeAddress,
        uint256 _trickPerSecond,
        uint256 _startTime
    ) {
        ...
        require(_trickPerSecond <= MAX_EMISSION_RATE, "Emission rate cannot be more than the maximum.");

        trick = _tokenAddress;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        trickPerSecond = _trickPerSecond;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) external onlyOwner nonDuplicated(_lpToken) {
        ...
        require(_depositFeeBP <= 420, "add: invalid deposit fee basis points");
    ...
    // Update the given pool's TRICK allocation point and deposit fee. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) external onlyOwner {
        require(_depositFeeBP <= 420, "set: invalid deposit fee basis points");
    ...
    // View function to see pending TRICKs on frontend.
    function pendingTrick(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        
        uint256 accTrickPerShare = pool.accTrickPerShare;
        
        if (block.timestamp > pool.lastRewardTime && pool.lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 trickReward = multiplier * trickPerSecond * pool.allocPoint / totalAllocPoint;
            accTrickPerShare = accTrickPerShare + trickReward * 1e18 / pool.lpSupply;
        }
        return user.amount * accTrickPerShare / 1e18 - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try trick.mint(devaddr, trickReward / 10) {
        } catch (bytes memory reason) {
            trickReward = 0;
            emit trickMintError(reason);
        }
        
        try trick.mint(address(this), trickReward) {
        } catch (bytes memory reason) {
            trickReward = 0;
            emit trickMintError(reason);
        }
        
        pool.accTrickPerShare = pool.accTrickPerShare + trickReward * 1e18 / pool.lpSupply;
        pool.lastRewardTime = block.timestamp;
    }
    
    // Deposit LP tokens to MasterChef for TRICK allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount * pool.accTrickPerShare / 1e18 - user.rewardDebt;
            if (pending > 0) {
                safeTrickTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount * pool.accTrickPerShare / 1e18;
    ...
    // Safe Treat transfer function, just in case if rounding error causes pool to not have enough TRICKs.
    function safeTrickTransfer(address _to, uint256 _amount) internal {
        uint256 trickBal = trick.balanceOf(address(this));
        bool transferSuccess = false;
        
        if (_amount > trickBal) {
            transferSuccess = trick.transfer(_to, trickBal);
        } else {
            transferSuccess = trick.transfer(_to, _amount);
        }
        
        require(transferSuccess, "safeTrickTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _trickPerSecond) external onlyOwner {
        require(_trickPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: INCORRECT INPUT");
        massUpdatePools();
        
        trickPerSecond = _trickPerSecond;
       
        emit UpdateEmissionRate(msg.sender, _trickPerSecond);
    }
