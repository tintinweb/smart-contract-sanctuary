...
contract Oracle is Epoch {
    ...
    constructor(IUniswapV2Pair _pair, uint256 _period, uint256 _startTime ) public Epoch(_period, _startTime, 0) {
        pair = _pair;
        token0 = pair.token0();
        token1 = pair.token1();
...
contract Epoch is Operator {
    ...
    uint256 private period = 5 minutes;
    ...
    function setPeriod(uint256 _period) external onlyOperator {
        require(_period >= 1 minutes && _period <= 48 hours, '_period: out of range');
        period = _period;
    }
