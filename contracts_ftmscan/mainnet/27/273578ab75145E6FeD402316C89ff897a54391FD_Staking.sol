...
interface IStakedToken {
  function rebase(uint256 TOKENProfit_, uint256 epoch_)
    external
    returns (uint256);
...
contract Staking is Ownable {
  ...
  /**
        @notice stake TOKEN to enter warmup
        @param _amount uint
        @return bool
     */
  function stake(uint256 _amount, address _recipient) external returns (bool) {
    ...
    warmupInfo[_recipient] = Claim({
      ...
      gons: info.gons.add(IStakedToken(sTOKEN).gonsForBalance(_amount)),
      expiry: epoch.number.add(warmupPeriod),
      lock: false
    });

    IERC20(sTOKEN).safeTransfer(warmupContract, _amount);
    return true;
  }

  /**
        @notice retrieve sTOKEN from warmup
  ...
  function claim(address _recipient) public {
    ...
    if (epoch.number >= info.expiry && info.expiry != 0) {
      ...
      IWarmup(warmupContract).retrieve(
        _recipient,
        IStakedToken(sTOKEN).balanceForGons(info.gons)
      );
    }
  }

  /**
        @notice forfeit sTOKEN in warmup and retrieve TOKEN
     */
  function forfeit() external {
    ...
    IERC20(TOKEN).safeTransfer(msg.sender, info.deposit);
  }
  ...
  function index() public view returns (uint256) {
    return IStakedToken(sTOKEN).index();
  }
  ...
  function rebase() public {
    if (epoch.endBlock <= block.number) {
      IStakedToken(sTOKEN).rebase(epoch.distribute, epoch.number);
      ...
      uint256 staked = IStakedToken(sTOKEN).circulatingSupply();
  ...
  /**
        @notice returns contract TOKEN holdings, including bonuses provided
        @return uint
     */
  function contractBalance() public view returns (uint256) {
    return IERC20(TOKEN).balanceOf(address(this)).add(totalBonus);
  }
  ...
  function giveLockBonus(uint256 _amount) external {
    ...
    IERC20(sTOKEN).safeTransfer(locker, _amount);
  }
  ...
  function returnLockBonus(uint256 _amount) external {
    ...
    IERC20(sTOKEN).safeTransferFrom(locker, address(this), _amount);
  }
