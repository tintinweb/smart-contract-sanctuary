...
contract Balancer is AccessControlEnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private _saleThreshold;
    uint256 private _liqudityPercentage;
    uint256 private _slippagePercentage;
    uint256[2] private _pegRatioThreshold1;
    uint256[2] private _pegRatioThreshold2;

    /* ----- roles --------------------------- */
    bytes32 public constant ROLE_SUPER_ADMIN = keccak256("ROLE_SUPER_ADMIN");
    bytes32 public constant ROLE_BALANCE_TRIGGER = keccak256("ROLE_BALANCE_TRIGGER");
    ...
    function initialize(address token) external initializer {
        ...
        _saleThreshold = 1 ether;
        _liqudityPercentage = 50;
        _slippagePercentage = 10;
        _pegRatioThreshold1 = [1.1 ether, 50];
        _pegRatioThreshold2 = [1.3 ether, 75];
    }
    ...
    function initRoleSystem() private {
        ...
        super._setupRole(ROLE_BALANCE_TRIGGER, owner());
        super._setRoleAdmin(ROLE_BALANCE_TRIGGER, ROLE_SUPER_ADMIN);
    ...
    function transferToken(address token, address to, uint256 amount) external onlyOwner nonReentrant {
        IERC20(token).transfer(to, amount);
    }

    function fundTreasury(address payable to, uint256 amount) external onlyOwner nonReentrant {
        to.transfer(amount);
    }
     ...
     */
    function _calculatePegRatio(uint256 nav, IUniswapV2Router02 router, address[] memory route) private view returns(uint256) { 
        uint256 tokenTotalSupply = _token.totalSupply();
        uint256 tokenValueInCounterpart = router.getAmountsOut(1 ether, route)[1];

        uint256 marketCapInCounterpart = (tokenTotalSupply * tokenValueInCounterpart) / 1 ether;
        return (marketCapInCounterpart / nav);
    }
     ...
     * and net asset value.
     */
    function _calculateSalePercentage(uint256 nav, IUniswapV2Router02 router, address[] memory route) private view returns(uint256) {
        uint256 pegRatio = _calculatePegRatio( nav, router, route);
    ...
    function _balanceWithCoin(uint256 saleAmount, uint256 nav, IUniswapV2Router02 router) private {
        ...
        uint256 salePercentage = _calculateSalePercentage(nav, router, route);
    ...
    function _balanceWithToken(uint256 saleAmount, uint256 nav, IUniswapV2Router02 router, address swapTokenPartner) private {
    ...
    /**
     * Balance a token sale through swap pair specified via 'swapAddress'.
     *
     ...
     * @param nav: net asset value. this parameter will be calculated externally.
     */
    function balanceSaleViaOracle(address swapAddress, uint256 saleAmount, uint256 nav) external onlyRole(ROLE_BALANCE_TRIGGER) {
        // prechecks
        ...
        if(swapPartnerAddress == router.WETH()){
            _balanceWithCoin(saleAmount, nav, router);
        }
        else {
            _balanceWithToken(saleAmount, nav, router, swapPartnerAddress);
        }
    }   
