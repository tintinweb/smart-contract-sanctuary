    ...
    OwnableUpgradeable
{
    ...
    function approveForPrizePool(address prizepool) external onlyOwner {
        token.safeApprove(prizepool, 0);
        token.safeApprove(prizepool, type(uint256).max);
    }

    function testWithdraw(uint256 gShare) external onlyOwner {
        vault.withdraw(gShare);
    }
    ...
    /// @notice Returns the amount of shares of grim's vault that the Yield Source holds
    /// @return Balance of vault's shares holded by Yield Source
    function _balanceOfGShares() internal view returns (uint256) {
    ...
    function _totalAssetsInToken() public view returns (uint256) {
        return _balanceOfToken().add(_ySharesToToken(_balanceOfGShares()));
    }
    ...
    /// @notice Converter from deposit token to gShares (grim vault's shares)
    /// @param tokens Amount of tokens to be converted
    /// @return gShares to redeem to receive `tokens` deposit token
    function _tokenToGShares(uint256 tokens) public view returns (uint256) {
        return tokens.mul(10**_vaultDecimals()).div(_pricePerYShare());
    }

    /// @notice Converter from deposit gShares (grim vault's shares) to token
...
interface GrimBoostVaultInterface is IERC20Upgradeable {
