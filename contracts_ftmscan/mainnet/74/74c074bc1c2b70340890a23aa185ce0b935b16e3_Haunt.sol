...
contract Haunt is ERC20Permit, VaultOwned {

    using SafeMath for uint256;

	address private creator;
	uint private maxTxAmount;
	uint private maxWalletAmount;

    constructor() ERC20("Haunt", "HAUNT", 9) {
		creator = msg.sender;
	}
	...
	function setTxLimits(uint256 _maxTxAmount, uint256 _maxWalletAmount)
		external onlyVault()
	{
		require(_maxTxAmount > 0 && maxWalletAmount > 0, "Can't disable trading");
		...
		internal virtual override
	{
		...
		// protection only if != owner
		if (_from != creator && _to != creator) {
			require(_amount <= maxTxAmount, "Amount exceeds maxTx.");
			require(balanceOf(msg.sender) + _amount <= maxWalletAmount);
		}
	}
}