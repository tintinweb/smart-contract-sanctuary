...
contract ERC20 is Context, IERC20 {
     ...
     *
     * This is an alter wrapped to {approve} that can be used as a mitigation for
...
contract StratManager is Ownable, Pausable {
     ...
     * @param _grimFeeRecipient address where to send Grim's fees.
     */
    constructor(
        address _unirouter,
    ...
    // checks that caller is the owner.
    ...
    /**
     * @dev Updates Grim fee recipient.
     * @param _grimFeeRecipient new Grim fee recipient address.
     */
...
abstract contract FeeManager is StratManager {
    ...
    //Fees
    uint256 public WITHDRAWAL_MAX = 100000; // withdrawal divisor
    uint256 public WITHDRAW_FEE = 100;  //0.1% 
    uint256 public MAX_FEE = 1000;  // divisor
...
contract ScreamStratWftm is StratManager, FeeManager {
    ...
    ) StratManager(_unirouter, _grimFeeRecipient) public {
    ...
    function _harvest() public whenNotPaused {
    ...
    function chargeFees() internal {
        ...
        IUniswapRouter(unirouter).swapExactTokensForTokens(toWrapped, 0, outputToWrappedRoute, address(this), now);
        uint balAfter = balanceOfWant();
        uint wrappedBal = balAfter.sub(balBefore);
    ...
    function callReward() public returns (uint256) {
        ...
        if (outputBal > 0) {
            ...
            {
                wrappedOut = amountOut[amountOut.length -1];
            }
            catch {}
        }

        return wrappedOut.mul(PLATFORM_FEE).div(MAX_FEE).mul(CALL_FEE).div(MAX_FEE);
    }
