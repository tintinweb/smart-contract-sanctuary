...
// MasterChef is the master of FROGs. He can make FROGs and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once FROG is sufficiently
// distributed and the community can show to govern itself.

contract frogMasterchef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of FROG
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accFrogPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accFrogPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. FROGs to distribute per second.
        uint256 lastRewardTime;  // Last timestamp that FROGs distribution occurs.
        uint256 accFrogPerShare;   // Accumulated FROGs per share, times 1e18. See below.
    ...
    // The FROG TOKEN!
    FrogToken public immutable frog;
    // Dev address.
    address public devaddr;
    // FROG tokens created per timestamp.
    uint256 public frogPerSecond;
    // Deposit Fee address
    ...
    // The timestamp when FROG distribution starts.
    uint256 public startTime;
    // Maximum FROG Per Second
    ...
    event UpdateEmissionRate(address indexed user, uint256 frogPerSecond);
    ...
    event frogMintError(bytes reason);

    constructor(
        FrogToken _tokenAddress,
        address _devaddr,
        address _feeAddress,
        uint256 _frogPerSecond,
        uint256 _startTime
    ) {
        ...
        require(_frogPerSecond < MAX_EMISSION_RATE, "Emission rate cannot be more than the maximum.");

        frog = _tokenAddress;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        frogPerSecond = _frogPerSecond;
    ...
    function initializePools(IERC20 _FrogWFTMPair) external onlyOwner {
        require(poolInfo.length == 0, "Pools are already existing");

        add(200, IERC20(_FrogWFTMPair), 0, true, false); // FROG-FTM pool
        add(150, IERC20(0xFA5c941BC491Ee6Dc1E933f38d01d8B5D5637205), 0, true, false);  // FROG pool
        add(5, IERC20(0x484f2ff94a7790759D56Fb1eFbAce8075aBA5e06), 0, true, false);   // TREAT pool
        add(5, IERC20(0xA5aFce54270D9afA6a80464bBD383BE506888e6A), 0, true, false);   // TRICK pool
        add(70, IERC20(0xDDc0385169797937066bBd8EF409b5B3c0dFEB52), 400, false, false);   // WMEMO pool
        add(40, IERC20(0x82f0B8B456c1A451378467398982d4834b6829c1), 400, false, false);   // MIM pool
        add(15, IERC20(0xf16e81dce15B08F326220742020379B855B87DF9), 400, false, false);   // ICE pool
        add(15, IERC20(0x468003B688943977e6130F4F68F23aad939a1040), 400, false, false);   // SPELL pool
    }
    ...
    // Update the given pool's FROG allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingFrog(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 frogReward = multiplier * frogPerSecond * pool.allocPoint / totalAllocPoint;
            accFrogPerShare = accFrogPerShare + frogReward * 1e18 / pool.lpSupply;
        }
        return user.amount * accFrogPerShare / 1e18 - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try frog.mint(devaddr, frogReward / 10) {
        } catch (bytes memory reason) {
            frogReward = 0;
            emit frogMintError(reason);
        }
        
        try frog.mint(address(this), frogReward) {
        } catch (bytes memory reason) {
            frogReward = 0;
            emit frogMintError(reason);
        }
        
        pool.accFrogPerShare = pool.accFrogPerShare + frogReward * 1e18 / pool.lpSupply;
        pool.lastRewardTime = block.timestamp;
    }
    
    // Deposit LP tokens to MasterChef for FROG allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount * pool.accFrogPerShare / 1e18 - user.rewardDebt;
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount * pool.accFrogPerShare / 1e18;
    ...
    // Safe Treat transfer function, just in case if rounding error causes pool to not have enough FROGs.
    function safeFrogTransfer(address _to, uint256 _amount) internal {
        uint256 frogBal = frog.balanceOf(address(this));
        bool transferSuccess = false;
        
        if (_amount > frogBal) {
            transferSuccess = frog.transfer(_to, frogBal);
        } else {
            transferSuccess = frog.transfer(_to, _amount);
        }
        
        require(transferSuccess, "safeFrogTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _frogPerSecond) external onlyOwner {
        require(_frogPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: INCORRECT INPUT");
        massUpdatePools();
        
        frogPerSecond = _frogPerSecond;
       
        emit UpdateEmissionRate(msg.sender, _frogPerSecond);
    }
