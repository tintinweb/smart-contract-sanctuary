...
contract NativeFarm is Ownable, ReentrancyGuard {
    ...
    // Native total supply: 3826640 = 3826640e18
    uint256 public constant NATIVEMaxSupply = 3826640e18;
    // Natives per second: 0.1
    uint256 public constant NATIVEPerSecond = 100000000000000000; // NATIVE tokens created per second
    // Approx 20th Jan 2022
    uint256 public constant startTime = 1642698000;

    // withdrawalFee 0.1% = 9990, 1% = 9900
    uint256 public withdrawalFeeFactor = 10000;
    uint256 public constant withdrawalFeeFactorMax = 10000; // 100 = 1%
    uint256 public constant withdrawalFeeFactorLL = 9500;

    uint256 public constant devNATIVEReward = 1000;
    uint256 public constant xLICHNATIVEReward = 200;
    uint256 public constant burnNATIVEReward = 300;
       
    // burn address
    address public constant burnaddr = 0x000000000000000000000000000000000000dEaD;
    // xLICH pool address
    address public pooladdr = 0x3C4821d86016d72Ce152AC9115CE2DA6b7E0412b; // This is updated into xLICH pool contract
     // treasury address
    address public devaddr = 0x3C4821d86016d72Ce152AC9115CE2DA6b7E0412b; // treasury address
    ...
    modifier nonDuplicatedLP(IERC20 _want) {
        require(poolExistence[_want] == false, "nonDuplicated: Duplicated LPToken");
        _;
    }
    ...
    function add(
        ...
        IERC20 _want,
        address _strat
    ) external onlyOwner nonDuplicatedLP(_want){
    ...
    function updatePool(uint256 _pid) public {
        ...
        NATIVEToken(NATIVE).mint(
            devaddr,
            NATIVEReward.mul(devNATIVEReward).div(10000)
        );
        NATIVEToken(NATIVE).mint(
            pooladdr,
            NATIVEReward.mul(xLICHNATIVEReward).div(10000)
        );
        NATIVEToken(NATIVE).mint(
            burnaddr,
            NATIVEReward.mul(burnNATIVEReward).div(10000)
        );        
    ...
    // Update pooladdr by owner.
    function setPooladdr(address _addr)
        external
        onlyOwner
    {
        pooladdr = _addr;
