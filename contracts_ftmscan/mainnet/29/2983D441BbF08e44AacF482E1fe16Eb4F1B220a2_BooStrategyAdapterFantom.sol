...
contract BooStrategyFantom is OwnableUpgradeable {
    ...
    */
    ...
    // (,4062028926327777494033646596697, 81240578526)
    function getPendingShare(address user, uint256 perShare, uint256 debt) internal returns (uint256 share) {
        ...
        // 20000000000000001 * 4062028926327777494033646596697 / 1e36

        // 81240578526.55555394270185826172 < 81240578526
        if(current < debt){
            return 0;
        }
    ...
    function withdrawReward(address user) public onlyVault {
        ...
        uint256 _balance = pendingReward(); // 41473380787
        ...
        if(_pending > 0 && curPendingReward > _pending){
            // send reward to user
            IERC20(secondRewardsToken).safeTransfer(user, _pending);
    ...
    */
    function withdrawToVault(uint256 amount) public onlyVault {
        ...
        IERC20(boo).safeTransfer(vault, MathUpgradeable.min(amount, booBalance()));
    }
    /*
    ...
    */
    function investedUnderlyingBalance() external view returns (uint256) {
        ...
        uint256 totalXBoo = xBooBalance.add(xBooInPool);

        uint256 booFromXBoo = IXBoo(xBoo).xBOOForBOO(totalXBoo);
    ...
    function enterXBooRewardPool() internal {
        uint256 entireBalance = xBooBalance();
