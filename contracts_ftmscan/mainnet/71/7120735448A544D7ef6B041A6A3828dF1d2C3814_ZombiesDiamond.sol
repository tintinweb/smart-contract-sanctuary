...
/**
 Welcome to Zombies Diamond

 $ZDIAMOND has 6% tax on buys and 12% on sells. 
 We use fee tokens for reflection rewards (1%), auto-liquidity, buyback, and marketing. 
 A portion (1%) is burnt which makes this token a deflationary crypto asset.
*/
...
contract ZombiesDiamond is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name     = "Zombies Diamond";
    string private _symbol   = "ZDIAMOND";
    uint8  private _decimals = 9;
    
    // Liquidity Settings
    IUniswapV2Router02 public _spookyswapV2Router; // Spookyswap V2 Router
    address public _spookyswapV2LiquidityPair;     // Spookyswap V2 liquidity pairing for ZDIAMOND/WFTM
    ...
    // Addresses
    address payable public _marketingAddress = payable(0x3F4681936F3d4d106389eCe252821C0506DbfFF0); // Marketing address used to pay for marketing
    ...
    uint256 public  _sellTaxFee       = 12; // 12% tax 
    uint256 public  _whaleSellTaxFee  = 18; // 18% tax 
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // We now have 6/7ths left of FTM (converted from FBSI)
        // We want to send 4/7ths to the marketing wallet and keep 2/7ths within the contract for buyback
        uint256 marketingFTM = receivedFTM.div(7).mul(4);
        
        // Send the remaining FTM to the marketing wallet
        transferFTMToAddress(_marketingAddress, marketingFTM);
