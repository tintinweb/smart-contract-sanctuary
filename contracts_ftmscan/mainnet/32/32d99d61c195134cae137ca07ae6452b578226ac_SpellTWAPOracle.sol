...
// File contracts/oracles/wsSPATWAPOracle.sol
...
contract SpellTWAPOracle is IOracle {
    ...
    IAggregator public constant DAI_USD = IAggregator(0x91d5DEFAFfE2854C7D02F50c80FA1fdc8A721e52);
    IUniswapV2Pair public constant pair = IUniswapV2Pair(0xFa5a5F0bC990Be1D095C5385Fff6516F6e03c0a7);

    IsOHM public constant sSPA = IsOHM(0x8e2549225E21B1Da105563D419d5689b80343E01);
    ...
    // function toWSSPA(uint256 amount) public view returns (uint256) {
    //     return amount.mul(sSPA.index()) / baseIndex;
    // }
    // function toWSPA( uint amount ) public view returns ( uint ) {
    //     return amount.mul(1e18) / ( (sSPA.index()).mul(1e9) / baseIndex );
    // }
    ...
    function get(bytes calldata data) external override returns (bool, uint256) {
        ...
        pairInfo.priceAverage = uint144(1e44 / (uint256(FixedPoint
            ...
            .mul(1e9)
            .decode144()).mul(uint256(DAI_USD.latestAnswer())).mul(1e18) / sSPA.sOHMTowOHM(1e9)));
        pairInfo.blockTimestampLast = blockTimestamp;
    ...
    function peek(bytes calldata data) public view override returns (bool, uint256) {
        ...
        uint256 priceCumulative = _get(blockTimestamp);
        uint144 priceAverage = uint144(1e35 / (uint256(FixedPoint
            .uq112x112(uint224((priceCumulative - pairInfo.priceCumulativeLast) / timeElapsed))
            .mul(1e9)
            .decode144()).mul(uint256(DAI_USD.latestAnswer())).mul(1e18) / sSPA.sOHMTowOHM(1e9)));
    ...
    function peekSpot(bytes calldata data) external view override returns (uint256 rate) {
        ...
        // rate = 1e35 / toWSSPA(reserve1.mul(1e9) / reserve0).mul(uint256(DAI_USD.latestAnswer()));
        rate = 1e44 / ( uint256(1e18).mul( (reserve1.mul(1e9) / reserve0).mul(uint256(DAI_USD.latestAnswer()))) / sSPA.sOHMTowOHM(1e9) );
    ...
    function test1() public returns (uint256){
        return sSPA.sOHMTowOHM(1e9);
    }

    function test2() public returns (uint256){
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        return reserve1.mul(1e9) / reserve0;
    }

    function test3() public returns (uint256){
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        return (reserve1.mul(1e9) / reserve0).mul(uint256(DAI_USD.latestAnswer()));
    }

    function test4() public returns (uint256){
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        return uint256(1e18).mul( (reserve1.mul(1e9) / reserve0).mul(uint256(DAI_USD.latestAnswer())));
    }

    function test5() public returns (uint256){
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        return uint256(1e18).mul( (reserve1.mul(1e9) / reserve0).mul(uint256(DAI_USD.latestAnswer()))) / sSPA.sOHMTowOHM(1e9) ;
    }


    /// @inheritdoc IOracle
    function name(bytes calldata) public view override returns (string memory) {
        return "wsSAP TWAP CHAINLINK";
    }
