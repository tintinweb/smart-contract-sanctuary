    ...
    ReentrancyGuard
{
    ...
    address payable public TESTVStakingAddress;
    address public viaAddress = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83; // FTM

    uint256 public TESTVStakingRewardsFactor; // X/10,000 of rewards go to TESTV staking vault. The rest goes to treasury.
    ...
    event TESTVStakingRewardsFactorChange(
        uint256 oldTESTVStakingRewardsFactor,
        uint256 newTESTVStakingRewardsFactor
    );
    ...
    constructor(
        ...
        address payable _TESTVStakingAddress,
        uint256 _TESTVStakingRewardsFactor,
        uint256 _minDelay,
        uint256 _minDelayReduced // address[] memory proposers, address[] memory executors
    ) public {
        devWalletAddress = _devWalletAddress;
        treasuryAddress = _treasuryAddress;
        TESTVStakingAddress = _TESTVStakingAddress;
        TESTVStakingRewardsFactor = _TESTVStakingRewardsFactor;
        minDelay = _minDelay;
    ...
    function setTESTVStakingRewardsFactor(uint256 newTESTVStakingRewardsFactor)
        external
        virtual
    {
        require(
        ...
        require(
            newTESTVStakingRewardsFactor <= 1000,
            "newTESTVStakingRewardsFactor > 1000"
        );
        emit TESTVStakingRewardsFactorChange(
            TESTVStakingRewardsFactor,
            newTESTVStakingRewardsFactor
        );
        TESTVStakingRewardsFactor = newTESTVStakingRewardsFactor;
    }
    ...
    function convertTokenToWBNB(
        ...
        require(tokenStartAddress != viaAddress, "Cannot sell WBNB");
        require(tokenEndAddress == viaAddress, "Must end with WBNB");
        ...
        returns (uint256)
    {
        ...
        uint256 rewardsAmt = IERC20(viaAddress).balanceOf(address(this));

        if (TESTVStakingRewardsFactor > 0) {
            uint256 TESTVStakingRewards =
                rewardsAmt.mul(TESTVStakingRewardsFactor).div(1000);
            IERC20(viaAddress).safeTransfer(
                TESTVStakingAddress,
                TESTVStakingRewards
            );
            rewardsAmt = rewardsAmt.sub(TESTVStakingRewards);
        }

        IERC20(viaAddress).safeTransfer(treasuryAddress, rewardsAmt);
    }
    ...
    function _wrapBNB() internal virtual {
        ...
        if (bnbBal > 0) {
            IWBNB(viaAddress).deposit{value: bnbBal}(); // BNB -> WBNB
        }
    }
