...
// File: contracts/Market-Place.sol
...
contract MarketPlace is Context, Ownable, Pausable, IERC721Receiver{
    uint256 public MAX_PRICE = 200000000000000;/// 2m MCL
    Item[] public items;
    IERC20 nativeToken;
    ...
    constructor() public{
        nftContract = IERC721Meta(0x8F2Ca2640Aa51fFd00b8fD49F6C05578Bcec37F3);
        nativeToken = IERC20(0x675d3130C01cE1C402C903053033b8D9604CA712);
    }
    
    event ItemUpdated(uint _index, Item _item);
    event ItemAttached(uint _index, Item _item);
    event ItemDetached(uint _index, Item _item);
    event ItemSold(uint _index, Item _item);
    
    modifier ItemAccess(uint256 id){
        uint _index = indexOfItem(id);
    ...
    function indexOfItem(uint256 id) public view returns(uint){
    ...
    function attach(uint256 id, uint256 price) external whenNotPaused returns(uint _index, Item memory _item){
        ...
        try this.indexOfItem(id) returns(uint _i){
            _index = _i;
            items[_index] = _item;
        } catch {
            items.push(_item);
    ...
    function updateItemPrice(uint256 id, uint256 price) external ItemAccess(id) whenNotPaused{
        require( price <= MAX_PRICE, "Price is very high" );
        uint _index = indexOfItem(id);
    ...
    function getAvailableItems(address _account) public view returns(uint256[] memory ids, uint256[] memory prices, CarProps[] memory props){
        ...
        for(uint64 i = 0; i < items.length; i++){
            ...
            if(_item.available && (_account == _addrZero || _account == _item.owner) ){
    ...
    function buyItem(uint256 id, uint256 price) external whenNotPaused {
        ...
        require( nativeToken.transferFrom( buyer, item.owner, item.price ) );
