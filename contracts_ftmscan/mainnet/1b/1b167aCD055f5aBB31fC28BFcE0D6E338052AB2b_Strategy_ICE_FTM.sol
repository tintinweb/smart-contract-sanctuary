...
contract Strategy_ICE_FTM is Ownable {
    // This strategy is for FTM network. ICE - FTM pair. RewardToken = "boo"
    ...
    address public token0; // wftm
    address public token1; // ice
    address public vault;

    // ------------> Important constants <--------------
    uint256 public constant pid = 17;
    bool public constant hasPid = true;
    address public routerLP = 0xF491e7B69E4244ad4002BC14e878a34207E38c29;
    address public routerMain = 0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52;
    address public routerReward = 0xF491e7B69E4244ad4002BC14e878a34207E38c29;
    address public yelLiquidityRouter = 0xF491e7B69E4244ad4002BC14e878a34207E38c29;
    address public constant lpToken = 0x623EE4a7F290d11C11315994dB70FB148b13021d;
    address public constant farm = 0x2b2929E785374c651a81A63878Ab22742656DcDd;
    // ------------>        END          <--------------
    ...
    function autoCompound() external {
        ...
        _approveToken(WETH, routerLP);
        _approveToken(WETH, routerMain);
    ...
    function _getAmountsOut(
        address _router, uint256 _amount, address[] memory path) internal view returns (uint256){
    ...
    function _takeFee() internal {
        ...
        if(rewardsFeeInWETH > 0) {
            uint256 amount = _getAmountsOut(routerMain, rewardsFeeInWETH + pendingFee, path);
            if(amount > 100) {
                _swapExactTokensForTokens(routerMain, rewardsFeeInWETH + pendingFee, amount, path);
                pendingFee = 0;
    ...
    function _swapExactETHForTokens(address _token, uint256 _amountETH) internal {
        ...
        if(_token != WETH) {
            path[1] = _token;
            uint256 desiredAmountToken = _getAmountsOut(routerLP, _amountETH/2, path);
    ...
    function __addLiquidityETH(address _token, uint256 desiredAmountETH) internal {
        ...
        if(amount0 != 0 && amount1 !=0 && _canAddLiquidity(amount0, amount1)) {
            ...
            IWETH(WETH).transfer(lpToken, amount0);
            IPair(lpToken).mint(address(this));
        } else {
            revert("Not enough funds to add liquidity");
        }
    ...
    function _swapLPtoWETH(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            _swapTokenToWETH(routerReward, rewardToken);

            // swap LPs to token0 and token1
            _removeLiquidity();

            // swap token1 to WETH
            _swapTokenToWETH(routerLP, token1);
        }
    }
    ...
    function _swapTokenToWETH(address _router, address _token) internal {
        address[] memory path = new address[](2);
    ...
    function _swapWETHToToken(address _router, address _token) internal {
        ...
        uint256 amount = _getAmountsOut(_router, _balanceWETH, path);
        if(amount > 100) {
            _swapExactTokensForTokens(_router, _balanceWETH, amount, path);  
        }
    }
    ...
    function _swapExactTokensForTokens(
        ...
        IRouter(_router).swapExactTokensForTokens(
            ...
            _amount2 - (_amount2*toleranceLevelPercent)/100, // min desired WFTM
            _path,
            address(this),
    ...
    function _swapWETHToTokens() internal {
        ...
        uint256 _balance = getBalanceOfToken(WETH) / 2;
        if (_balance > 0) {
            path[1] = token1;
    ...
    function _getToken1Price() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return 1e36 / (_getAmountsOut(routerLP, 100 * 1e18, path) / 100);
    }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t1V != 0) {
                path[0] = token1;
                TC = _getAmountsOut(routerLP, t1V, path) + t0V;
            }
        }
        ...
        if (_balance > 0) {
            path[0] = token1;
            TC += _getAmountsOut(routerLP, _balance, path);
        }
