...
contract ZhuanZhangZhuShou is Ownable {
    ...
    function normal_ZhuanZhang(address _tokenIn, address[] calldata AimedWallet, uint256[] calldata amounts) external {   
    ...
    function ETH_ZhuanZhang(address payable[] calldata AimedWallet, uint256[] calldata amounts) external payable {
        require(AimedWallet.length == amounts.length, "wallet and amount total number not match, dude");
        for(uint256 indx = 0; indx < AimedWallet.length; indx++) {
