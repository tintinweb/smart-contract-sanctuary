...
contract FTMPAnyPrint is Auth {
    ...
    function PrintToken(address tokenToPrint) external {
        ...
        FTMPay(payable(FantomPay)).transfer(msg.sender, 1); //Workaround to claim dividend contractually.
        ...
        address[] memory path = new address[](3); //Create the path for the swap. USDC -> WFTM -> Wanted Token
        path[0] = USDC;
        path[1] = WFTM;
        path[2] = tokenToPrint;
