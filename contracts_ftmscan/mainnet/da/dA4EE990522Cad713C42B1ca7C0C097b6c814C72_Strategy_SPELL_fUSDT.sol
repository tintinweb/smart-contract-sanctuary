...
contract Strategy_SPELL_fUSDT is Ownable {
    ...
    function _swapFUSDTToSPELL() internal {
        ...
        if(balanceSpell > 0){
            ...
            path[0] = USDT;
            path[1] = token1;
            uint256 amount = _getAmountsOut(router, balanceSpell, path);
            if(amount > 100){
    ...
    function _getToken0Price() internal view returns (uint256) {
        ...
        path[0] = token1;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100) / 1e12;
    }
    ...
    function _getToken1Price() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return 1e24 / (_getAmountsOut(router, 100 * 1e6, path) / 100);
    }
    ...
    function _getPriceWFTMToSPELL() internal view returns (uint256 result) {
        ...
        if (amountFUSDT > 0) {
            path[0] = token0;
            path[1] = token1;
            result = 1e36 / (_getAmountsOut(router, amountFUSDT, path) / 100);
        }
    }
