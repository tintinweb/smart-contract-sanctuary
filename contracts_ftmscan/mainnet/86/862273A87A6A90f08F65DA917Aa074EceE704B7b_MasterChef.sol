...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 depositFee; // Deposit fee in base points (/10.000)
    ...
    // Natives per block: 0.25
    uint256 public NATIVEPerBlock = 20000000000000000; // NATIVE tokens created per block
    // Approx 8/3/2021
    uint256 public startBlock; // https://bscscan.com/block/countdown/8049678
    // Fee address
    address public feeAddress = 0x45BCAd5e65f7BCEB5C01C087a093db03A8279786;
    ...
    // Constructor
    constructor(address _native, uint256 _startBlock) public {
        NATIVE = _native;
    ...
    function add(uint256 _allocPoint, IERC20 _want, uint256 _depositFee, bool _withUpdate, address _strat) public onlyOwner {
        require(_depositFee <= 600, "add: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update reward variables of the given pool to be up-to-date. (STRATEGY)
    function updatePoolStrategy(uint256 _pid, uint256 NATIVEReward) public {
        // Get pool info
        PoolInfo storage pool = poolInfo[_pid];
        // Require call by the pool strategy
        require(address(msg.sender) == address(pool.strat), 'no strat calling');
        // Transfer tokens from strategy to masterchef
        IERC20(NATIVE).safeTransferFrom(address(msg.sender), address(this), NATIVEReward);
        // Get strat shares total
        uint256 sharesTotal = IStrategy(pool.strat).sharesTotal();
        // If no shares, return
        if (sharesTotal == 0) {
            return;
        }
        // Add accNATIVEPerShare
        pool.accNATIVEPerShare = pool.accNATIVEPerShare.add(NATIVEReward.mul(1e12).div(sharesTotal));
    }
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            ...
            uint256 sharesAdded = IStrategy(poolInfo[_pid].strat).deposit(msg.sender, _wantAmt, _pid);
    ...
    // Get stucked tokens from contract
    function inCaseTokensGetStuck(address _token, uint256 _amount) public onlyOwner {
        require(_token != NATIVE, "!safe");
        IERC20(_token).safeTransfer(msg.sender, _amount);
    }
    
    // set native per block, capped to 10 tokens per block for security
    function setNATIVEPerBlock(uint256 _NATIVEPerBlock) public onlyOwner {
        require(_NATIVEPerBlock <= 10000000000000000000, 'invalid native per block');
        NATIVEPerBlock = _NATIVEPerBlock;
    }

    // set start block
    function setStartBlock(uint256 _startBlock) public onlyOwner {
        require(block.number < startBlock, "masterchef has already started");
        startBlock = _startBlock;
    }
}