...
contract CrimeGoldV2 is Initializable, ERC20Upgradeable, GovernableUpgradable {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal override {
    ...
    if(pancakePair == recipient && !_isExcludedFromBurn[sender]) { 
  ...
  function setBurnRatePercent(uint256 burnRatePercent) external {
    onlyOwner();
    _burnRatePercent = burnRatePercent;
  }

  function setBurnRecipients(address _address, bool isExsist) external {
    onlyOwner();
    _isExcludedFromBurn[_address] = isExsist;
  }

  function setPancakePair(address pairAddress) external{
    onlyOwner();
