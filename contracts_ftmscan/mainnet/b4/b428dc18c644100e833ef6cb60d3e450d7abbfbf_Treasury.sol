...
// CLAIM
    ...
    function airdropAvailable(address a) public view returns(uint) {
    	if(airdrops[a].amount>=1e20){
    		uint rate = 31e14/totalAirdrops;
			if(rate>31e13){rate=31e13;}
	...
	function posterRewardsAvailable(address a) public view returns(uint) {
		if(posters[a].amount>0){
			uint rate =31e14*2/totalPosters;
...
//    function rpcTest(address a) external view returns(uint, uint,uint,uint) {
//    	uint p = posterRewardsAvailable(a);
//    	uint airdrop = airdropAvailable(a);
//    	uint postersAmount = posters[a].amount;
//    	uint airdropsAmount = airdrops[a].amount;
//    	return (p,postersAmount,airdrop,airdropsAmount);
//    }
