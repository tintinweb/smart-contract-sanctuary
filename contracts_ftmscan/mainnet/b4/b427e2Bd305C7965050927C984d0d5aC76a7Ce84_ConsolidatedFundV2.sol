...
contract ConsolidatedFundV2 is Ownable {
    ...
    // Add new Pool
    function addPool(address pool_address, address _token) public onlyOwner {
        require(pools[pool_address] == address(0), "poolExisted");
        require(_token != address(0), "invalid token");
        pools[pool_address] = _token;
        emit PoolAdded(pool_address, _token);
    }
    ...
    function rebalance(
        ...
        public onlyOwner
    {
        ERC20(_from_address).approve(address(router), 0);
        ERC20(_from_address).approve(address(router), _amount);

        address[] memory router_path = new address[](2);
        router_path[0] = _from_address;
        router_path[1] = _to_address;
        ...
        emit Rebalance(_from_address,_to_address,_amount,
    ...
    event PoolAdded(address pool, address token);
