...
contract Strategy_PILLS_MIM {
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        uint256 _balance1 = getBalanceOfToken(token1);
        if(_balance1 != 0) {
            tCI += (_getMimFromPILLS(_balance1) + _balance0) * _getPriceWFTMToMIM() / d;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t1V != 0) {
                ...
                path[0] = token0;
                path[1] = WETH;
                TC = _getAmountsOutSpirit(mimAmount + t0V, path);
            }
        }
        ...
        if(_balance1 != 0) {
            path[0] = token1;
            path[1] = token0;
            mimAmount = _getAmountsOut(_balance1, path);
            path[0] = token0;
            path[1] = WETH;
            TC += _getAmountsOutSpirit(mimAmount + _balance0, path);
        }
    ...
    function _getPriceWFTMToMIM() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(100 * 1e18, path) / 100);
    ...
    function _getPricePILLSToMIM() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = token0;
        path[1] = token1;
        return 1e36 / (_getAmountsOut(100 * 1e18, path) / 100);
    ...
    function _getMimFromPILLS(uint256 _pillsAmount) internal view returns (uint256 mimAmount) {
