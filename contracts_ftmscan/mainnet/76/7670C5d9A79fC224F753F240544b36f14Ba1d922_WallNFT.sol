...
// File contracts/libs/IWallFactory.sol
...
// File contracts/StrategyNFT.sol
...
contract WallNFT is Ownable, ReentrancyGuard, Pausable, AccessControl {
    ...
    IWallFactory public iWallFactory;

    // Whether a limit is set for users
    bool public hasUserLimit;

    // The time number when dream mining ends.
    uint256 public endTime;

    // The time number when dream mining starts.
    uint256 public startTime;

    // The pool limit (0 if none)
    uint256 public poolLimitPerUser;

    // tokens created per time.
    uint256 public rewardPerSecond;

    // experience created per time.
    ...
    function deposit(uint256 _amount, uint256 _nftID) external nonReentrant {
        ...
        if (iWallFactory.balanceOf(msg.sender) > 0) {
            if(!user.hasNFT)
    ...
    function withdraw() external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (user.hasNFT) {
                ...
                iWallFactory.setExperience(user.nftID, _newExperience);
    ...
    function pendingReward(address _userAddress) external view returns (uint256) {
        ...
        if (block.timestamp > poolInfo.lastRewardTime && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(poolInfo.lastRewardTime, block.timestamp);
    ...
    function pendingExperience(address _userAddress) external view returns (uint256) {
        ...
        if (block.timestamp > poolInfo.lastExperienceTime && supplyWithNFT != 0) {
            uint256 multiplier = _getMultiplier(poolInfo.lastExperienceTime, block.timestamp);
            uint256 experienceReward = multiplier.mul(experiencePerSecond);
    ...
    function _updateExperiencePool() internal {
        if (block.timestamp <= poolInfo.lastExperienceTime) {
            return;
        }
        ...
        if (supplyWithNFT == 0) {
            poolInfo.lastExperienceTime = block.timestamp;
            return;
        }
    ...
    function _initializeNFT(uint256 _nftID, address _userAddress) internal  {
        ...
        if (!nftIDs[_nftID]) {
            if (iWallFactory.ownerOf(_nftID) == _userAddress) {
                user.hasNFT = true;
                user.nftID = _nftID;
    ...
    function _getNFTPowerStaking(uint256 _nftID) internal returns (uint256) {
        (uint256 power,,) = iWallFactory.getArtWorkOverView(_nftID);

        return power;
    }

    function _getNFTExperience(uint256 _nftID) internal returns (uint256) {
        (,uint256 experience,) = iWallFactory.getArtWorkOverView(_nftID);

        return experience;
    }

    function updateSeconds(uint256 _startTime, uint256 _endTime) external onlyOwner {
        require(block.timestamp < startTime);

        startTime  = _startTime;
        endTime    = _endTime;
        poolInfo.lastRewardTime = startTime;
        poolInfo.lastExperienceTime = startTime;
    }
