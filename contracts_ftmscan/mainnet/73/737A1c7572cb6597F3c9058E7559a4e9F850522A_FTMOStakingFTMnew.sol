...
/**
 * Staking contract for FTM-FTMO
 */

import "./FTMOToken.sol";

contract FTMOStakingFTMnew {
    struct _rewardInfo {
    ...
    function depositStake()
        ...
        payable
    {
        require(poolActive, "FTMOStakingFTM: pool not active");
    ...
    function withdrawStake(uint256 amount)
        ...
        onlyStakers
    {
        require(totalTokensStaked > 0, "FTMOStakingFTM: no tokens staked in pool");
        require(amount > 0, "FTMOStakingFTM: invalid withdraw amount");
        uint256 balance = _stakerInfo[msg.sender].tokensStaked;
        require(amount <= balance, "FTMOStakingFTM: over withdraw limit");
        ...
        onlyStakers
    {
        ...
        require(
            amount <= max,
            "FTMOStakingFTM: amount exceeds withdrawable FTMO"
        );
    ...
    function pausePool()
        ...
        onlyAdmin
    {
        require(poolActive, "FTMOStakingFTM: pool is paused");
        updateRunningSum();
        poolActive = false;
    }
    ...
    function resumePool()
        ...
        onlyAdmin
    {
        require(!poolActive, "FTMOStakingFTM: pool is active");
        lastTime = block.timestamp;
        poolActive = true;
    }
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == _admin,
            "FTMOStakingFTM: only admin can do this"
        );
        _;
    }
    ...
    modifier onlyStakers() {
        require(
            isStaker[msg.sender],
            "FTMOStakingFTM: only stakers allowed"
        );
        _;
    }
    ...
    modifier poolNotStarted() {
        require(!poolStarted, "FTMOStakingFTM: pool already started");
        _;
    }
}