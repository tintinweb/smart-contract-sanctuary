...
contract Strategy_SPELL_fUSDT {
    ...
    function _getPriceWFTMToFUSDC() public view returns (uint256) {
        // wFTM to fUSDT
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        if(_amount > 0) {
            ...
            tCI = _getPriceWFTMToSPELL() * t1V / 1e18 + _getPriceWFTMToFUSDC() * t0V / 1e6;
        }
        ...
        if(_balance != 0) {
            tCI += _getPriceWFTMToFUSDC() * _balance / 1e6;
        }
