...
contract CGSVAuto is Ownable, Pausable {
     ...
     */
    function calculateHarvestCGSVRewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingCGSV(0, address(this));
     ...
     */
    function calculateTotalPendingCGSVRewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingCGSV(0, address(this));
        amount = amount.add(available());
