...
/**
 * @dev Stakes MIM in 0xDAO.
 */
contract OxMIM is GelatoTask, TokenHandler {

    using SafeERC20 for IERC20;

    uint constant MIM_POOL = 6;
    address constant MIM = 0x82f0B8B456c1A451378467398982d4834b6829c1;
    address constant OXD = 0xc165d941481e68696f43EE6E99BFB2B23E0E3114;
    address constant DAO = 0xa7821C3e9fC1bF961e280510c471031120716c3d;
    ...
    function setOXDSwapPath(address[] memory path) external onlyOwner {
        require(path[0] == OXD, "path not starting with OXD");
    ...
    function release() external {
        ...
        uint amount = _staked[depositor];
        require(amount > 0, "nothing staked");
        MasterChef(DAO).withdraw(MIM_POOL, amount);
        delete _staked[depositor];
        amount += _harvestMIM();
        IERC20(MIM).transfer(depositor, amount);
    }
    ...
    function supply(address wallet) external onlyManager(wallet) validGasPrice {
        uint amount = _swapAll(wallet);
        uint transferableAmount = _transferableAmount(wallet, MIM);
    ...
    function _harvestMIM() private returns (uint amount) {
        MasterChef(DAO).harvestAll();
        amount = IERC20(OXD).balanceOf(address(this));
        if (amount > 0) {
            _approveMaxIfNecessary(OXD, SPOOKY, amount);
            uint[] memory amounts = IRouter(SPOOKY).swapExactTokensForTokens(amount, 0, _OXDSwapPath, address(this), block.timestamp);
            amount = amounts[amounts.length - 1];
        }
    }

    function _swapAll(address wallet) private returns (uint amount_) {
        address[] memory sourceTokens = _sourceTokens[wallet];
        uint numSourceTokens = sourceTokens.length;
        for (uint i = 0; i < numSourceTokens; i++) {
            ...
            if (amount > 0) {
                ...
                uint[] memory amountsOut = IRouter(SPOOKY).swapExactTokensForTokens(amount, 0, _swapPaths[wallet][token], address(this), block.timestamp);
                amount_ += amountsOut[amountsOut.length - 1];
            }
        }
    }
    ...
    function _supply(address wallet, uint amount, bool compound) private {
        ...
        _approveMaxIfNecessary(MIM, DAO, amount);
        MasterChef(DAO).deposit(MIM_POOL, amount);
        _staked[wallet] += amount;
    }
}
