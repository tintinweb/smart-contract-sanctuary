...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. crows to distribute per second.
        uint256 lastRewardSecond; // Last second that crows distribution occurs.
        uint256 acccrowPerShare; // Accumulated crows per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The crow TOKEN!
    SpookyCrowToken public crow;
    // Dev address.
    address public devaddr;
    // crow tokens created per second.
    uint256 public crowPerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when crow mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 crowPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        SpookyCrowToken _crow,
        address _devaddr,
        address _feeAddress,
        uint256 _crowPerSecond,
        uint256 _startTime
    ) public {
        crow = _crow;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        crowPerSecond = _crowPerSecond;
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 crowReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            acccrowPerShare = acccrowPerShare.add(
                crowReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(acccrowPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 totalRewards = crow.totalSupply().add(devReward).add(
            crowReward
        );
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.acccrowPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.acccrowPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safecrowTransfer(msg.sender, pending);
    ...
    function safecrowTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > crowBal) {
            transferSuccess = crow.transfer(_to, crowBal);
        } else {
            transferSuccess = crow.transfer(_to, _amount);
        }
        require(transferSuccess, "safecrowTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _crowPerSecond) external onlyOwner {
        require(_crowPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        crowPerSecond = _crowPerSecond;
        emit UpdateEmissionRate(msg.sender, _crowPerSecond);
    }
