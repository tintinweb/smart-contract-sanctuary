...
contract Battle is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the days 2-3 BATTLE-WFTM LP -> BATTLE pool
    uint256 public constant INITIAL_BATTLE_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the BATTLE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("BATTLE Finance", "BATTLE") {
        // Mints 1 BATTLE to contract creator for initial pool setup
    ...
    function _getBATTLEPrice() internal view returns (uint256 _battlePrice) {
        try IOracle(battleOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("BATTLE: failed to fetch BATTLE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _battlePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_battlePrice >= taxTiersTwaps[tierId]) {
    ...
    function setBATTLEOracle(address _battleOracle) public onlyOperatorOrTaxOffice {
        require(_battleOracle != address(0), "oracle address cannot be 0 address");
        battleOracle = _battleOracle;
    }
    ...
    /**
     * @notice Operator mints BATTLE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BATTLE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBATTLEPrice = _getBATTLEPrice();
            currentTaxRate = _updateTaxRate(currentBATTLEPrice);
            if (currentBATTLEPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_battlePool != address(0), "!_battlePool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_battlePool, INITIAL_BATTLE_POOL_DISTRIBUTION);
