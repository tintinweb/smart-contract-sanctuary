...
/**
  * @ LoFiTown is metaverse land plots allow building
  * @ based on ERC-721.
  */
/**
...
contract LoFiTownLand is ERC721Full, Governance {
    ...
    constructor() public ERC721Full("LFLand", "LFL") {
    }
    ...
    function mint(address to,
        ...
        int128 _x,
        int128 _y,
        int128 _t) external returns (bool) {

        require(_minters[msg.sender], "minter invalid");
        require(plot_id_at[_x][_y] == 0, "unclaimable plot");
        require(!_exists(tokenId), "mint invalid tokenid");
        _mint(to, tokenId);

        plots[tokenId] = Plot(_x, _y, _t, block.timestamp, true);
        plot_id_at[_x][_y] = tokenId;
        
        emit PlotMinted(to, tokenId, _x, _y, _t);
