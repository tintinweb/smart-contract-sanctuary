...
contract BoxingIGame is Ownable {
    ...
    function dogfall_gold(uint256 _creater, uint256 _parter) external onlyOwner {
        require((goldBalances[_creater] + goldBalances[_parter]) > 0, "not enough balance!");
        
        uint256 _ca = goldBalances[_creater];
        uint256 _pa = goldBalances[_parter];
        goldBalances[_creater] = 0;
        goldBalances[_parter] = 0;
        require(gold.transfer(BOX_SUMMONER_ID, _creater, _ca),"dogfall gold failure!");
        require(gold.transfer(BOX_SUMMONER_ID, _parter, _pa),"dogfall gold failure!");
    ...
    function dogfall_ftm(address _creater, address _parter) external onlyOwner {
        require((ftmBalances[_creater] + ftmBalances[_parter]) > 0, "no balance!");
        require(ftmDepositAmount >= (ftmBalances[_creater] + ftmBalances[_parter]), "beyond total deposit!");
        
        uint256 _ca = ftmBalances[_creater];
        uint256 _pa = ftmBalances[_parter];
        ftmBalances[_creater] = 0;
        ftmBalances[_parter] = 0;
        ftmDepositAmount -= _ca;
        ftmDepositAmount -= _pa;
        
        payable(_creater).transfer(_ca);
        payable(_parter).transfer(_pa);
