...
interface SpookySwapRouter02 is SpookySwapRouter01 {
...
contract NODERewardManagement {
    ...
    uint256[] public multipliersLevels;
    ...
    constructor(
        ...
        uint256[] memory _multipliersLevels
    ) {
        _managers[msg.sender] = true;
        nodePrice = _nodePrice;
        rewardsPerMinute = _rewardsPerMinute;
        multipliers = _multipliers;
        multipliersLevels = _multipliersLevels;
    }
    ...
    function createNode(address account, string memory name)
        external
        onlyManager
    {
        ...
        emit NodeCreated(
            account,
            name,
            _nodesOfUser[account].length,
            totalNodesCreated
        );
    }

    function dividendsOwing(NodeEntity memory node, address account)
        private
        view
        returns (uint256 availableRewards)
    {
        uint256 currentTime = block.timestamp;
        uint256 minutesPassed = (currentTime).sub(node.creationTime).div(
            claimInterval
        );
        ...
        for (uint256 i = 0; i < multipliers.length; i++) {
            if (nodesCount >= multipliersLevels[i]) multiplier = multipliers[i];
        }
        uint256 rewardsAfterMultiplier = rewardsPerMinute.mul(multiplier).div(
            1000
        );
        return
            minutesPassed.mul(rewardsAfterMultiplier).add(nodeStartAmount).sub(
                node.dividendsPaid
            );
    }

    function _getNodeByIndex(NodeEntity[] storage nodes, uint256 index)
        private
        view
        ...
        returns (uint256)
    {
        ...
        uint256 rewardNode = dividendsOwing(node, account);
        ...
        returns (uint256)
    {
        NodeEntity[] storage nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
        require(nodesCount > 0, "NODE: You don't own a node.");
        NodeEntity storage _node;
        uint256 rewardsTotal = 0;
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            uint256 rewardNode = dividendsOwing(_node, account);
    ...
    function _getRewardAmountOf(address account)
        ...
        returns (uint256)
    {
        require(isNodeOwner(account), "GET REWARD OF: You don't own a node.");
        uint256 nodesCount;
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            rewardCount += dividendsOwing(_node, account);
        }
        ...
        returns (uint256)
    {
        ...
        return dividendsOwing(node, account);
    }
    ...
    {
        require(isNodeOwner(account), "GET CREATIME: You don't own a node.");
    ...
    function _getNodesRewardAvailable(address account)
        ...
        returns (string memory)
    {
        require(isNodeOwner(account), "GET REWARD: You don't own a node.");
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
        NodeEntity memory _node;
        string memory _rewardsAvailable = uint2str(
            dividendsOwing(nodes[0], account)
        );
        string memory separator = "#";
        ...
        for (uint256 i = 1; i < nodesCount; i++) {
            ...
            _rewardsAvailable = string(
                abi.encodePacked(
                    _rewardsAvailable,
                    separator,
                    uint2str(dividendsOwing(_node, account))
                )
            );
        }
        ...
        returns (string memory)
    {
        require(
            isNodeOwner(account),
            "LAST CLAIME TIME: You don't own a node."
        );
        NodeEntity[] memory nodes = _nodesOfUser[account];
        uint256 nodesCount = nodes.length;
        NodeEntity memory _node;
    ...
    function _getNodes(address user)
        external
        view
    ...
    function _changeNodeStartAmount(uint256 newStartAmount)
        external
        onlyManager
    {
    ...
    function _changeMultipliers(uint256[] memory newMultipliers)
        external
        onlyManager
    {
    ...
    function _changeMultipliersLevels(uint256[] memory newMultipliersLevels)
        external
        onlyManager
    {
        multipliersLevels = newMultipliersLevels;
    }
...
contract Paranodes is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;

    NODERewardManagement public nodeRewardManagement;

    SpookySwapRouter02 public uniswapV2Router;
    ...
    uint256 public maxTxAmount = 300 * 10**18;
    uint256 public maxBuyAmount = 700 * 10**18;
    uint256 public maxNodesAmount = 100;
    ...
    ) ERC20("PARANODES", "PARA") PaymentSplitter(payees, shares) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        ...
        uniswapV2Router = SpookySwapRouter02(newAddress);
        address _uniswapV2Pair = SpookySwapFactory(uniswapV2Router.factory())
    ...
    function updateCashoutFee(uint256 value) external onlyOwner {
        require(value <= 25, "RWFEE: Must be under 25%.");
        cashoutFee = value;
    }
    ...
    function changeMaxTxAmount(uint256 value) external onlyOwner {
        require(
            value >= 100 * 10**18,
            "MAXTX: Must be over or equal to 100 $PARA."
        );
        maxTxAmount = value;
    }

    function changeMaxBuyAmount(uint256 value) external onlyOwner {
        require(
            value >= 300 * 10**18,
            "MAXTX: Must be over or equal to 300 $PARA."
        );
        maxBuyAmount = value;
    }
    ...
    function changeIsLaunched(bool value) external onlyOwner {
        isLaunched = value;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (from == uniswapV2Pair) {
                require(
                    amount <= maxBuyAmount,
                    "TRSFR: Please buy under the max buy amount."
                );
            } else {
                require(
                    amount <= maxTxAmount,
                    "TRSFR: Please transfer under the max transaction amount"
                );
            }
            if (protectSale) {
                require(to != uniswapV2Pair, "don't add liquidty bro...");
                if (to == uniswapV2Pair) {
            ...
            if (protectSale && from == uniswapV2Pair) {
                require(from != uniswapV2Pair, "how are you even buying?");
            }
        }
    ...
    function createNodeWithTokens(string memory name) public {
        require(isLaunched, "NODE CREATION: Please wait after launch.");
        require(
            nodeRewardManagement._getNodeNumberOf(msg.sender) < maxNodesAmount,
            "NODE CREATION: You've reached the maximum amount of nodes"
        );
        require(
            bytes(name).length > 3 && bytes(name).length < 20,
            "NODE CREATION: Name must between 3 and 20 characters."
        );
    ...
    function createManyNodeWithTokens(string memory prefix, uint256 quantity)
        public
    {
        require(isLaunched, "NODE CREATION: Please wait after launch.");
        require(
            bytes(prefix).length > 3 && bytes(prefix).length < 16,
            "NODE CREATION: Prefix must between 3 and 16 characters."
        );
        address sender = msg.sender;
        uint256 nodePrice = nodeRewardManagement.nodePrice();
        require(
            balanceOf(sender) >= nodePrice.mul(quantity),
            "NODE CREATION: Balance too low for creation."
        );
        for (uint256 i = 0; i < quantity; i++) {
            createNodeWithTokens(
                string(abi.encodePacked("[", uint2str(i), "] ", prefix))
            );
        }
    }
    ...
    function changeMaxNodes(uint256 newMaxNodesAmount) public onlyOwner {
        maxNodesAmount = newMaxNodesAmount;
    }

    function getMaxNodes() public view returns (uint256) {
        return maxNodesAmount;
    }
    ...
    function changeMultipliers(uint256[] memory newMultipliers)
        public
        onlyOwner
    {
        nodeRewardManagement._changeMultipliers(newMultipliers);
    }

    function getMultipliers(uint256 index) public view returns (uint256) {
        return nodeRewardManagement.multipliers(index);
    }

    function changeMultipliersLevels(uint256[] memory newMultipliersLevels)
        public
        onlyOwner
    {
        nodeRewardManagement._changeMultipliersLevels(newMultipliersLevels);
    }

    function getMultipliersLevels(uint256 index) public view returns (uint256) {
        return nodeRewardManagement.multipliersLevels(index);
    }
    ...
    function getNodesCreationTime(address account)
        ...
        returns (string memory)
    {
        return nodeRewardManagement._getNodesCreationTime(account);
    }

    function getNodesRewards(address account)
        public
        view
        returns (string memory)
    {
        return nodeRewardManagement._getNodesRewardAvailable(account);
    }

    function getNodes(address account)
        public
        view
        returns (NODERewardManagement.NodeEntity[] memory nodes)
    {
        return nodeRewardManagement._getNodes(account);
    }
