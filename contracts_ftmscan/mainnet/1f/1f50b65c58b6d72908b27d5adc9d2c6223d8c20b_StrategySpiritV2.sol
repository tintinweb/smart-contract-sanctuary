...
abstract contract StrategyBase {
    ...
    address public mmToken = 0xbFaf328Fe059c53D936876141f38089df0D1503D;
    address public masterChef = 0x6539C525f74c3b8ce16fd4e89208A518e3BEC7Ff;
    ...
    // buyback coins
    address public constant spiritBuyback = 0x5Cc61A78F164885776AA610fb0FE1257df78E59B;
    ...
    // Dex
    address public univ2Router2 = 0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52;
    ...
    function setSpookyRoute(address _route) external {
        require(msg.sender == governance, "!governance");
        spookyRouter = _route;
    }

    function setSpiritRoute(address _route) external {
        require(msg.sender == governance, "!governance");
        spiritRouter = _route;
    }
    ...
    function figureOutPath(address _from, address _to, uint256 _amount) public view returns (bool useSushi, address[] memory swapPath){
        ...
        if (_to == mmToken && buybackEnabled == true) {
            path = new address[](3);
            path[0] = _from;
            path[1] = WFTM; // use WFTM on Fantom
            path[2] = _to;
        } else{
            path = new address[](2);
            path[0] = _from;
...
// 
contract StrategySpiritV2 is StrategyBase {
    // Token addresses
    address public spirit = 0x5Cc61A78F164885776AA610fb0FE1257df78E59B;
    address public tarotRoute = 0x283e62CFe14b352dB8e30A9575481DCbf589Ad98;
    address public bTAROT = 0xc5EB7dB755A870B91d18AdCbBf60549a294B4130;

    address public constant spiritFtmLP = 0x30748322B6E34545DBe0788C421886AEB5297789;
    address public mSpiritFtmVault = 0xe74358fEc47B55F94F5aCb4f32a30a36623Fc373;
    address public mSpiritVault = 0xd21E83422335b21E2e11567c13780a3f3972c0E8;
    address public tarotCollateral = 0xAbCcF1bF4B58d8dc55A6104971694C30ad6B0204;

    uint256 public principal;
    uint256 public redeemLimit = 1e16;

    uint256 public constant swapOutBPS = 10000;
    uint256 public swapOutMinBPS = 9000;
    uint256 public lpConvertBPS = 3000;

    uint256 public spiritReserve;
    uint256 public ftmReserve;
    ...
    public
    StrategyBase(spirit, _governance, _strategist, _controller, _timelock){
        IERC20(spirit).safeApprove(tarotRoute, uint256(-1));
        IERC20(bTAROT).safeApprove(tarotRoute, uint256(-1));
        IERC20(spiritFtmLP).safeApprove(mSpiritFtmVault, uint256(-1));
        IERC20(spiritFtmLP).safeApprove(univ2Router2, uint256(-1));
    }

    // **** setter ****
    function setRedeemLimit(uint256 _limit) external onlyBenevolent{
        redeemLimit = _limit;
    }

    function setSwapBPS(uint256 _bps) external onlyBenevolent {
        swapOutMinBPS = _bps;
    }

    function syncPrincipal(uint256 _principal) external onlyBenevolent {
        principal = _principal;
    }

    function setLpConvertBPS(uint256 _bps) external onlyBenevolent {
        lpConvertBPS = _bps;
    }
    ...
    function harvest() public override onlyBenignCallers {
        ...
        updateTarotCollateral();

        uint256 _rewards = getHarvestable();
        if(_rewards < redeemLimit){
            return;
        }
        // Collects SPIRIT tokens
        withdrawFromPool(_rewards.mul(lpConvertBPS).div(swapOutBPS));

        // We want to get back SPIRIT tokens
        ...
        if (_want > 0) {
            ...
            lastHarvestInWant = _want.sub(_buybackLpAmount);
    ...
    function depositInMMVault() internal {
        uint256 _reward = balanceOfWant();
        //swap half SPIRIT to WFTM
        if(_reward > 0){
            (,uint112 _spiritReserve,) = IUniswapV2Pair(spiritFtmLP).getReserves();
            uint256 _rewardForSwap = UniswapV2SingleSided.calculateSingleSidedAmt(_reward, uint256(_spiritReserve));

            _swapUniswap(spirit, WFTM, _rewardForSwap);
        }

        // Adds in liquidity for SPIRIT/WFTM
        uint256 _token0 = IERC20(WFTM).balanceOf(address(this));
        uint256 _token1 = IERC20(spirit).balanceOf(address(this));
        if (_token0 > 0 && _token1 > 0) {
            IERC20(WFTM).safeApprove(univ2Router2, 0);
            IERC20(WFTM).safeApprove(univ2Router2, _token0);
            IERC20(spirit).safeApprove(univ2Router2, 0);
            IERC20(spirit).safeApprove(univ2Router2, _token1);
            ...
            UniswapRouterV2(univ2Router2).addLiquidity(
                ...
                _token0,
                _token1,
                _token0.mul(swapOutMinBPS).div(swapOutBPS),
                _token1.mul(swapOutMinBPS).div(swapOutBPS),
                address(this),
                now
            );
        }

        uint256 _lpBalance = IERC20(spiritFtmLP).balanceOf(address(this));
        if(_lpBalance > 0){
            //deposit into mmVault
            IVault(mSpiritFtmVault).deposit(_lpBalance);
        }
    ...
    function withdraw(IERC20 _asset) external override returns (uint256 balance) {
        require(address(_asset) != bTAROT, "bTAROT");
        require(address(_asset) != mSpiritFtmVault, "mToken");
    ...
    function getName() external override pure returns (string memory) {
        return "StrategySpiritV2";
    }

    function updateTarotCollateral() public {
        (uint256 _reserveFtm, uint256 _reserveSpirit) = TarotCollateral(tarotCollateral).getPrices();
        spiritReserve = _reserveSpirit;
        ftmReserve = _reserveFtm;
    }

    function balanceOfPool() public override view returns (uint256) {
        uint256 _spiritInPool = balanceOfTarotPool();

        // spirit-ftm lp amount in mmVault
        uint256 _mTokenAmt = IVault(mSpiritFtmVault).balanceOf(address(this));
        if (_mTokenAmt == 0){
            return _spiritInPool;
        }
        uint256 _mRatio = IVault(mSpiritFtmVault).getRatio();
        uint256 _lpAmtInMVault = _mTokenAmt.mul(_mRatio).div(1e18);

        //cover lp amount to spirit amount
        uint256 _spiritEqAmt = _lpAmtInMVault.mul(1e18).div(spiritReserve);

        return _spiritInPool.add(_spiritEqAmt);
    }

    function getHarvestable() public view returns (uint256) {
        uint256 _balanceInPool = balanceOfTarotPool();
        if (_balanceInPool > principal){
            return _balanceInPool.sub(principal);
        }
        return 0;
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            TarotRouter(tarotRoute).mint(bTAROT, _want, address(this), now + 60);
            principal = principal.add(_want);
        }
    }
    ...
    returns (uint256)
    {
        ...
        if (_amount >= balanceOfPool()){
            _withdrawFromTarot(IERC20(bTAROT).balanceOf(address(this)));

            IVault(mSpiritFtmVault).withdraw(IVault(mSpiritFtmVault).balanceOf(address(this)));
            uint256 _lpBalance = IERC20(spiritFtmLP).balanceOf(address(this));

            uint256 _halfLp = _lpBalance.mul(1e18).div(2);
            uint256 _needFtmAmt = _halfLp.div(ftmReserve);
            uint256 _needSpiritAmt = _halfLp.div(spiritReserve);

            coverLpToSpirit(_lpBalance,_needFtmAmt, _needSpiritAmt);

            uint256 _spiritFinalBal = balanceOfWant();
            return _spiritFinalBal > _amount ? _amount : _spiritFinalBal;
        }

        uint256 _balanceInPool = balanceOfTarotPool();
        if (_amount > _balanceInPool){
            withdrawFromPool(_balanceInPool);

            uint256 _spiritBal = balanceOfWant();
            if (_spiritBal >= _amount){
                return _amount;
            }
            uint256 _needSpiritFromVault = _amount.sub(_spiritBal);
            uint256 _needSpiritAmt = _needSpiritFromVault.div(2);

            uint256 _needFtmAmt = _needSpiritAmt.mul(spiritReserve).div(ftmReserve);
            uint256 _needLpAmount = _needSpiritFromVault.mul(spiritReserve).div(1e18);

            uint256 _needWithdrawShareFromVault = _needLpAmount.mul(1e18).div(IVault(mSpiritFtmVault).getRatio());

            uint256 _balanceShareInVault = IVault(mSpiritFtmVault).balanceOf(address(this));
            if (_needWithdrawShareFromVault > _balanceShareInVault){
                _needWithdrawShareFromVault = _balanceShareInVault;
            }
            IVault(mSpiritFtmVault).withdraw(_needWithdrawShareFromVault);
        ...
        return withdrawFromPool(_amount);
    }

    function coverLpToSpirit(uint256 _lpAmt, uint256 _needFtmAmt, uint256 _needSpiritAmt) internal {
        if (_lpAmt <= 0){
            return;
        }
		
        uint256 fmtMin = _needFtmAmt.mul(swapOutMinBPS).div(swapOutBPS);
        uint256 spiritMin = _needSpiritAmt.mul(swapOutMinBPS).div(swapOutBPS);
        ...
        if (_wftm > 0) {
            _swapUniswap(WFTM, spirit, _wftm);
        }
    }

    function withdrawFromPool(uint256 _amount) internal returns(uint256){        
        uint256 _shareAmt = _amount.mul(1e18).div(poolExchangeRate());
        uint256 _bTarotBal = IERC20(bTAROT).balanceOf(address(this));
        _shareAmt = _shareAmt <= _bTarotBal ? _shareAmt : _bTarotBal;
		
        return _withdrawFromTarot(_shareAmt);
    }

    function _withdrawFromTarot(uint256 _shareAmt) internal returns(uint256){
        bool redeemAll = _shareAmt < IERC20(bTAROT).balanceOf(address(this))? false : true;
		
        uint256 _beforeBal = balanceOfWant();
        TarotRouter(tarotRoute).redeem(bTAROT, _shareAmt, address(this), now + 60, "");
        uint256 _afterBal = balanceOfWant();
        uint256 _withdrawn = _afterBal.sub(_beforeBal);
        
        uint256 bTarotRate = poolExchangeRate();
        uint256 mVaultRate = IVault(mSpiritVault).getRatio();
        if (!redeemAll){
            uint256 principalAdjusted = _withdrawn;
            if (principal < balanceOfTarotPool()){
                // adjust withdrawn principal delta according to Tarot exchangeRate & mVault ratio
                if (bTarotRate > 1e18){
                    principalAdjusted = principalAdjusted.mul(1e18).div(bTarotRate);
                }
                if (mVaultRate > 1e18){
                    principalAdjusted = principalAdjusted.mul(1e18).div(mVaultRate);
                }
            }
            principal = principal > principalAdjusted? principal.sub(principalAdjusted) : 0;
        }else{
            principal = 0;
        }
    ...
    function balanceOfTarotPool() public view returns (uint256){
        uint256 _bToken = IERC20(bTAROT).balanceOf(address(this));
        return _bToken.mul(poolExchangeRate()).div(1e18);
    }


    function poolExchangeRate() public view returns(uint256){
        return PoolToken(bTAROT).exchangeRateLast();
    }
}