...
contract MeowAPIFTM {
  ...
  function getNewSpookyYieldFarmAPY(address _worker, uint256 _amount) public view returns (uint256) {
    uint256 pid = getPoolIdSpooky(_worker);
    address lp = getLpToken(_worker);
    uint256 numerator = ((booPerYear(pid).mul(getBOOPrice()).div(1e18))).mul(uint256(100)).mul(1e18);
    uint256 denominator = (
      ((IERC20(lp).balanceOf(address(spookyMasterChef)).add(_amount)).mul(1e18).div((IERC20(lp).totalSupply()).add(_amount))).mul(getLPValue(_worker))
    ).div(1e18);
  }
