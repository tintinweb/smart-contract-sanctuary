...
import "./ICurrencyBlocksMinter.sol";

contract CurrencyBlocksMinter is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, CryptoBlocksMinterUpgradeable, ICurrencyBlocksMinter {
    address private _currencyBlocks;                    // CurrencyBlocks Contract
    ...
    function initialize() initializer public {
        ...
        __CurrencyBlocksMinter_init_unchained();
    }

    function __CurrencyBlocksMinter_init_unchained() internal initializer {
        _pause();
    }

    function _authorizeUpgrade(address) internal override onlyRole(AccessControlConstants.ADMIN_ROLE) {}

    function buy(uint256 currencyId_, uint256 amount_, address referral_) external payable whenNotPaused nonReentrant {
        uint256 price_ = ICryptoBlocks(_currencyBlocks).getBlockAttribute(currencyId_, CurrencyBlocksConstants.UINT_PRICE);
        uint256 buyable_ = ICryptoBlocks(_currencyBlocks).getBlockAttribute(currencyId_, CurrencyBlocksConstants.UINT_BUYABLE);

        require(buyable_ == 1, "Not buyable");
        require(msg.value >= price_ * amount_, "Insufficient fee paid");

        safeMint(_msgSender(), currencyId_, amount_);
    ...
    function safeMint(address to_, uint256 currencyId_, uint256 amount_) internal {
        uint256 totalSupply_ = ICryptoBlocksMintableERC1155(_currencyBlocks).getTotalSupply(currencyId_);
        uint256 maxSupply_ = ICryptoBlocks(_currencyBlocks).getBlockAttribute(currencyId_, CurrencyBlocksConstants.UINT_MAX_SUPPLY);
        require(totalSupply_ + amount_ <= maxSupply_, "Not enough supply");
        ICryptoBlocksMintableERC1155(_currencyBlocks).mint(to_, currencyId_, amount_);
    }

    /**
     * @dev Mint without paying fee
     */
    function freeMint(address to_, uint256 currencyId_, uint256 amount_) external override onlyRole(AccessControlConstants.MINTER_ROLE) {
        safeMint(to_, currencyId_, amount_);
    }

    /**
     * @dev Mass mint
     */
    function massMint(address[] calldata to_, uint256 currencyId_, uint256 amount_) external onlyRole(AccessControlConstants.MINTER_ROLE) {
        for(uint256 i = 0; i < to_.length; i++) {
            safeMint(to_[i], currencyId_, amount_);
        }
    }

    /**
     * @dev Mint while increasing maxSupply
     * Should never be called if maxSupply is max
     */
    function extraMint(address to_, uint256 currencyId_, uint256 amount_) external onlyRole(AccessControlConstants.MINTER_ROLE) {
        uint256 maxSupply_ = ICryptoBlocks(_currencyBlocks).getBlockAttribute(currencyId_, CurrencyBlocksConstants.UINT_MAX_SUPPLY);
        require((maxSupply_ + amount_) < type(uint256).max, "Max supply at limit");
        ICryptoBlocksMintableERC1155(_currencyBlocks).mint(to_, currencyId_, amount_);
        ICryptoBlocksAttributes(_currencyBlocks).setBlockAttribute(currencyId_, CurrencyBlocksConstants.UINT_MAX_SUPPLY, maxSupply_ + amount_);
    }
