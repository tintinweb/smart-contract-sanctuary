...
contract MasterChef is Ownable {
    ...
    function add(uint256 _allocPoint, address _lpToken, bool _withUpdate, bool _isNFT,
    ...
    function setPerBlock(uint256 _sushiPerBlock) public {
        require(msg.sender == owner(), "!dev addr");
        sushiPerBlock = _sushiPerBlock;
    }
    ...
    function getTimeMultiplex(uint256 sec_) public pure returns (uint256) {
        ...
        else if (sec_ <= 7 days) return 120;
        else if (sec_ <= 14 days) return 140;
        else if (sec_ <= 30 days) return 160;
        else return 200;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _durationSec) public payable {
        ...
        require(!pool.isNFT, "!is NFT: not good");
        updatePool(_pid);
        _harvest(pool, user);
        if (msg.value > 0) {
    ...
    function depositNFT(uint256 _pid, uint256 _amount, uint256 _durationSec) public payable {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        UserInfoSet storage userset = userInfoSet[_pid][msg.sender];
        require(pool.isNFT, "!is not NFT: not good");
        updatePool(_pid);
        _harvest(pool, user);

        for(uint256 i = 0; i < _amount; i++){
            uint256 tokenId = IERC721Enumerable(address(pool.lpToken)).tokenOfOwnerByIndex(msg.sender, 0);
            userset.holderTokens.add(tokenId);
            IERC721Enumerable(address(pool.lpToken)).safeTransferFrom(address(msg.sender), address(this), tokenId);
        }
        ...
        user.rewardDebt = user.amount.mul(pool.accSushiPerShare).div(1e12);
        emit DepositNFT(msg.sender, _pid, _amount);
    }
    function withdrawNFT(uint256 _pid, uint256 _amount) public payable {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        UserInfoSet storage userset = userInfoSet[_pid][msg.sender];
        require(pool.isNFT, "!is not NFT: not good");
        ...
        for(uint256 i = 0; i < _amount; i++) {
            uint256 tokenId = userset.holderTokens.at(0);
            IERC721Enumerable(address(pool.lpToken)).safeTransferFrom(address(this), address(msg.sender), tokenId);
            userset.holderTokens.remove(tokenId);
        }
        user.amount = user.amount.sub(_amount);
        pool.amount = pool.amount.sub(_amount);

        user.rewardDebt = user.amount.mul(pool.accSushiPerShare).div(1e12);
        emit WithdrawNFT(msg.sender, _pid, _amount);
    }
    ...
    function mint(address to, uint256 rewardAmount) internal {
        ...
        IMintToken(address(sushi)).mint(to, rewardAmount);
