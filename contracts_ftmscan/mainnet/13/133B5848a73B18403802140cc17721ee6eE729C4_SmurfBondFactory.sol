...
contract SmurfBondFactory {
 ...
 //   }
    ...
    function withdrawLps(uint256 _depositAmt) public {
        ...
        IERC20(sbond).transferFrom(address(msg.sender), Burn, _depositAmt);
        ...
        if (user.amountCleverFtm > 0) {
            ...
            IERC20(CleverFtmLp).transfer(msg.sender, sendAmount1);
        ...
        if (user.amountCleverUsdc > 0) {
            ...
            IERC20(CleverUsdcLp).transfer(msg.sender, sendAmount2);
            user.amountCleverUsdc = user.amountCleverUsdc.sub(sendAmount2);
        }
        if (user.amountCleverEth > 0) {
            uint256 sendAmount3 = (sharePaidBack * user.amountCleverEth) / 1e18;
            IERC20(CleverEthLp).transfer(msg.sender, sendAmount3);
            user.amountCleverEth = user.amountCleverEth.sub(sendAmount3);
        }
        if (user.amountMushyFtm > 0) {
            uint256 sendAmount4 = (sharePaidBack * user.amountMushyFtm) / 1e18;
            IERC20(MushyFtmLp).transfer(msg.sender, sendAmount4);
            user.amountMushyFtm = user.amountMushyFtm.sub(sendAmount4);
        }
        if (user.amountMushyUsdc > 0) {
            uint256 sendAmount5 = (sharePaidBack * user.amountMushyUsdc) / 1e18;
            IERC20(MushyUsdcLp).transfer(msg.sender, sendAmount5);
            user.amountMushyUsdc = user.amountMushyUsdc.sub(sendAmount5);
        }
        if (user.amountClever > 0) {
            uint256 sendAmount6 = (sharePaidBack * user.amountClever) / 1e18;
            IERC20(Clever).transfer(msg.sender, sendAmount6);
            user.amountClever = user.amountClever.sub(sendAmount6);
        }
        if (user.amountMushy > 0) {
            uint256 sendAmount7 = (sharePaidBack * user.amountMushy) / 1e18;
            IERC20(Mushy).transfer(msg.sender, sendAmount7);
            user.amountMushy = user.amountMushy.sub(sendAmount7);
        }
