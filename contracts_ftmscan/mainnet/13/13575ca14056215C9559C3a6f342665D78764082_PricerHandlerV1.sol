...
library SimplePairPricer {
    function getReserveInOutTokenAndAmount(IUniswapV2Pair pair, address tokenIn, uint256 amountIn) internal view returns (uint256 reserveIn, address tokenOther, uint256 amountOut) {
        ...
        return (reserve0, token1, calculateAmountOut(amountIn, reserve0, reserve1));
    }

    function getOutTokenAndAmount(IUniswapV2Pair pair, address tokenIn, uint256 amountIn) internal view returns (address tokenOther, uint256 amountOut) {
        // force token0 to be token and token1 to be the pair token
        address token1 = pair.token1();
        (uint256 reserve0, uint256 reserve1,) = pair.getReserves();
        if(token1 == tokenIn) {
        ...
        return (token1, calculateAmountOut(amountIn, reserve0, reserve1));
    }
...
contract PricerHandlerV1 is IPricer, AccessControlEnumerableUpgradeable {
    ...
    function getValue(
        ...
        try pairChecker.isPair(asset) {
            ...
            address token1 = IUniswapV2Pair(asset).token0();
    ...
    function getTokenValue(address token, uint256 amount) internal view returns (uint256) {
        ...
        if (address(overridenPair) != address(0)) {
            (address outToken, uint256 outAmount) = overridenPair
                .getOutTokenAndAmount(token, amount);
            return getTokenValue(outToken, outAmount);
        }
