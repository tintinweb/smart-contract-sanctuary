...
contract _MUMMY_Token is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 MUMMY-WETH LP -> MUMMY pool
    ...
    uint256 public burnThreshold = 0.05e18;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MUMMY Token", "MUMMY") {
    ...
    modifier whitelist(address who) {
        require(blacklistUsers[who] == false, "You are blacklisted.");
        _;
    }

    function addBlacklist(address _blacklist) external onlyOperator {
    ...
    function removeBlacklist(address _blacklist) external onlyOperator {
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) public whitelist(_msgSender()) override returns (bool) {
    ...
    function _transferWithTax(
        ...
        uint256 amount,
        bool burnTax
    ) internal whitelist(_msgSender()) returns (bool) {
    ...
    function distributeReward(
        address _genesisPool, //used for liquidity
        address _MUMMYPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_MUMMYPool != address(0), "!_MUMMYPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_MUMMYPool, INITIAL_MUMMY_POOL_DISTRIBUTION);
    }
