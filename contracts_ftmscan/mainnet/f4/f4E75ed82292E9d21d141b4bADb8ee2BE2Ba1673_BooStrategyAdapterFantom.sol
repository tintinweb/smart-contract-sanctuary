...
contract BooStrategyFantom is OwnableUpgradeable {
    ...
    */
    function investedUnderlyingBalance() external view returns (uint256) {
        uint256 xBooBalance = xBooBalance();
        uint256 xBooInPool = xBooMasterChefBalance();

        uint256 booFromXBoo = IXBoo(xBoo).BOOForxBOO(xBooBalance.add(xBooInPool));

        return booBalance().add(booFromXBoo);
    }

    function xBooBalance() public view returns (uint256 bal) {
        return IXBoo(xBoo).balanceOf(address(this));
    }

    function booBalanceInXboo() public view returns (uint256 bal) {
    ...
    function stakeXBoo() external {
        uint256 booRewardBalance = booBalance();
