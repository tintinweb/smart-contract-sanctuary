...
// File: contracts/fantomized/fantomgates.sol
 ...
 */
abstract contract ERC721Tradable is ERC721Enumerable, ReentrancyGuard, Ownable {
    using SafeMath for uint256;

    uint256 private _currentTokenId;

    uint private _maxClaim; //  = 100;
    // uint private _amountClaim = 55000000000000000;  // 0.055 - 550000000000000 0.00055
    uint private _amountClaim;
    uint private _amountClaimDiscount;

    IERC721 private constant SPECTACULARS = IERC721(0x0c8763aC45fB87cAF89c736fd7EFe11627D33622);

    uint public constant MAX_SUPPLY = 1900;
    ...
    function setClaim(uint _max, uint _amount, uint _amountDiscount) public onlyOwner {
        require(_max <= MAX_SUPPLY, "Over supply");
        _maxClaim = _max;
        _amountClaim = _amount;
        _amountClaimDiscount = _amountDiscount;
    }
    ...
    function getClaimAmountDiscount() public view returns (uint) {
        return _amountClaimDiscount;
    }
    ...
    function mintNTo(address _to, uint256 number) public onlyOwner {
        require(_currentTokenId <= 1000, "Not allowed after 1000");  // Stop before max supply
    ...
    function claim(
        ...
        require(_currentTokenId < _maxClaim, "Minting is over for this batch");
        require(msg.value >= _amountClaim, "Too few eth, check batch price");
        uint256 newTokenId = _currentTokenId.add(1);
        _mint(_to, newTokenId);
        _currentTokenId++;
    }
    ...
    /**
     * @dev Function to claim a token at discounted price for spectaculars holders
     * @param _to The address that will receive the minted tokens.
     *
     */
    function claimDiscount(
        ...
        require(_currentTokenId < _maxClaim, "Minting is over for this batch");
        require(msg.value >= _amountClaimDiscount, "Too few eth, check batch price");
        // check balance
        require(SPECTACULARS.balanceOf(msg.sender) > 0, "No Spectacular Held");
        uint256 newTokenId = _currentTokenId.add(1);
        _mint(_to, newTokenId);
        _currentTokenId++;
    }
 ...
 * FantomGates
 */
contract FantomGates is ERC721Tradable {


    string _baseUrl="ipfs://QmZQBrLtVDiwMmoddDSSWWis5Cc2VdxgPSoytsAcE6R3ua/";

    constructor()
        ERC721Tradable("FantomGates", "FGATE")
    {}
    ...
    function updateBase(string memory newBase) public onlyOwner {
        // Needed to add extra properties and future use in metaverse
        _baseUrl = newBase;
    }
