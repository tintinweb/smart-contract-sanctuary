...
contract MasterChef is Ownable {
    ...
    using SafeERC20 for VampireSwapToken;
    ...
    function setProfile(
        address _profile) public onlyOwner {
        require(_profile == address(0), "setProfile: ONLY CHANGE ONCE");
        profile = _profile;
    }
    ...
    function pendingVam(uint _pid, address _user) public view returns (uint) {
        ...
        if (block.timestamp > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint farmVam = calculate(vamReward);

            accVamPerShare = accVamPerShare.add(farmVam.mul(1e18).div(lpSupply));
        }
    ...
    function calculate(uint _reward) public view returns (uint farmVam) {
        if (totalVam <= mintedVam.add(_reward)) {
            _reward = totalVam.sub(mintedVam);
        }
    ...
    function updatePool(uint _pid) public {
        ...
        vam.mint(address(this), farmVam);
        mintedVam = mintedVam.add(farmVam);

        pool.accVamPerShare = pool.accVamPerShare.add(farmVam.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.timestamp;
    }
