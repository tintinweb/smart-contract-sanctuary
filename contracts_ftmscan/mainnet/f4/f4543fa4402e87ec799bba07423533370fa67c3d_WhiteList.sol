...
contract WhiteList is Ownable, IWhiteList {
    ...
    function deleteFromWhiteList(address[] memory accounts) public onlyOwner {
        uint256 length = accounts.length;
        for (uint256 i = 0; i < length; ++i) {
    ...
    function getAccountByIndex(uint256 index) public view returns (address) {
        return whiteList.at(index);
    }

    function isInWhiteList(address account) public view virtual override returns (bool) {
