...
contract FlashBot is ContractOwnable, Initializable {
        ...
        )
    {
        ...
        require(baseTokens.contains(pool0Token0) || baseTokens.contains(pool0Token1), 'No base token in pair');
    ...
    function swap(address pool0, address pool1) external returns (bool){
        ...
        // avoid stack too deep error
        {
            ...
            require(baseOutAmount > startAmount, 'Arbitrage fail, no profit');
            console.log('-estimated profit:', (baseOutAmount - startAmount) /* / 1 ether*/);

            require(startAmount<=balanceBefore, 'Not enough base token balance');
...
//        console.log('-Sell  to pool:', higherPool);
    ...
    {
        require(pool0 != pool1, 'F: Same pair address');
        (address pool0Token0, address pool0Token1) = (IUniswapV2Pair(pool0).token0(), IUniswapV2Pair(pool0).token1());
        (address pool1Token0, address pool1Token1) = (IUniswapV2Pair(pool1).token0(), IUniswapV2Pair(pool1).token1());
        require(pool0Token0 < pool0Token1 && pool1Token0 < pool1Token1, 'F: Non standard uniswap AMM pair');
        require(pool0Token0 == pool1Token0 && pool0Token1 == pool1Token1, 'F: Require same token pair');
//        require(baseTokens.contains(pool0Token0) || baseTokens.contains(pool0Token1), 'F: No base token in pair');
