...
contract CheckDex {
    ...
    function executeArb(address router1Address, address router2Address, address[] calldata inPath, address[] calldata outPath, uint256 max_can_sell_tokenA,uint256 max_buy_tokenB) external {
        ...
        IUniswapV2Router02 router1 = IUniswapV2Router02(router1Address);
        IUniswapV2Router02 router2 = IUniswapV2Router02(router2Address);
        IERC20(inPath[0]).approve(address(router1), max_can_sell_tokenA);
        router1.swapTokensForExactTokens(max_buy_tokenB, max_can_sell_tokenA, inPath, address(this), block.timestamp + 1000);
        IERC20(outPath[0]).approve(address(router2), max_buy_tokenB);
        router2.swapExactTokensForTokens(max_buy_tokenB, max_can_sell_tokenA, outPath, msg.sender, block.timestamp + 1000);
