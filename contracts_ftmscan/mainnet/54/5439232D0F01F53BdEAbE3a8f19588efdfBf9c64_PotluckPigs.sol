...
abstract contract ERC721URIStorage is ERC721 {
        ...
        returns (string memory)
    {
       ...
       return string(abi.encodePacked(base, "potluckpig",(tokenId + 1).toString(), ext));
...
{
    modifier onlyDevs {
         require(msg.sender == primaryDevAddress || msg.sender == secondaryDevAddress || msg.sender == tertiaryDevAddress || msg.sender == quaternaryDevAddress, "Dev Only: caller is not the developer");
        _;
    }
    ...
    // FEES
    uint256 public primaryDevFee = 2000;
    uint256 public secondaryDevFee = 4000;
    uint256 public tertiaryDevFee = 3000;
    uint256 public quaternaryDevFee = 1000;
    uint256 public royalty = 600;
    // COLLECTED FESS
    mapping(address => uint256) public devFees;
  
    constructor() ERC721("Potluck Pigs", "PLPG") {
        _pause();
    }
    ...
    function mint(uint256 count) public payable {
        ...
        require(count > 0 && count <= maxBuy,"Number of NFTs must be more than 0 and less than 10");
        require(msg.value == price * count, "Must send correct price");
        require(supply + count - 1 < maxSupply, "Cannot mint more than total supply");
    ...
    function pigsOwned(address _owner)
        external
        view
