...
contract ReaperVaultv1_3 is ERC20, Ownable, ReentrancyGuard {
     ...
     */
    function withdrawAll(bool _useFL) external {
        withdraw(balanceOf(msg.sender), _useFL);
    }
     ...
     */
    function withdraw(uint256 _shares, bool _useFL) public nonReentrant {
        ...
        if (b < r) {
            ...
            IStrategy(strategy).withdraw(_withdraw, _useFL);
            uint256 _after = token.balanceOf(address(this));
    ...
    function upgradeStrat(bool _useFL) public onlyOwner {
        require(
        ...
        IStrategy(strategy).retireStrat(_useFL);
