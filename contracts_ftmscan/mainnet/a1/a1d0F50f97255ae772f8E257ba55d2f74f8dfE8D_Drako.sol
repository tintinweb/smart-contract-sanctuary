...
contract Drako is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 DRAKO-WFTM LP -> DRAKO pool
    ...
    /**
     * @notice Constructs the DRAKO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("DRAKO", "DRAKO") {
        // Mints 1 DRAKO to contract creator for initial pool setup
    ...
    function _getTombPrice() internal view returns (uint256 _tombPrice) {
        try IOracle(tombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            ...
            revert("Drako: failed to fetch DRAKO price from Oracle");
        }
    }
    ...
    /**
     * @notice Operator mints DRAKO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DRAKO to mint to
