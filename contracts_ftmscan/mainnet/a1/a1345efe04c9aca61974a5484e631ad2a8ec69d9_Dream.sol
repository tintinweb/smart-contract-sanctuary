...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 constant MAXSUPPLY = 400 ether;
...
contract Dream is BEP20 {
    ...
    // Min amount to liquify. (default 10 DREAMs)
    ...
    // The operator can call all functions except of minting DREAM
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "DREAM::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the DreamSwapToken contract.
     */
    constructor() public BEP20("Dream Token", "DREAM") {
    ...
    /// @dev overrides transfer function to meet tokenomics of DREAM
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "DREAM::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "DREAM::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the dream -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "DREAM::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "DREAM::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "DREAM::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "DREAM::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "DREAM::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "DREAM::transferOperator: new operator is the zero address");
...
// MasterChef is the master of DREAM. He can make Dream and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DREAM
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DREAMs to distribute per second.
        uint256 lastRewardSecond; // Last second that DREAMs distribution occurs.
    ...
    // DREAM tokens created per second.
    uint256 public DreamPerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when Dream mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 DreamPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _DreamPerSecond,
        uint256 _startTime
    ) public {
        dream = _dream;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        DreamPerSecond = _DreamPerSecond;
    ...
    // View function to see pending dream on frontend.
    function pendingDream(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 dreamReward = multiplier
                .mul(DreamPerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
                uint256 devReward = dreamReward.div(10);
                uint256 totalRewards = dream.totalSupply().add(devReward).add(
                dreamReward
            );
                if (totalRewards > dream.maxSupply()) {
                dreamReward = dream.maxSupply().sub(dream.totalSupply());
            }
            accDreamPerShare = accDreamPerShare.add(
                dreamReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= dream.maxSupply()) {
            ...
            // mint the difference only to MC, update dreamReward
            dreamReward = dream.maxSupply().sub(dream.totalSupply());
            dream.mint(address(this), dreamReward);
        }

        if (dreamReward != 0) {
            // only calculate and update if dreamReward is non 0
            pool.accDreamPerShare = pool.accDreamPerShare.add(
                dreamReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accDreamPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeDreamTransfer(msg.sender, pending);
    ...
    function updateEmissionRate(uint256 _DreamPerSecond) external onlyOwner {
        require(_DreamPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        DreamPerSecond = _DreamPerSecond;
        emit UpdateEmissionRate(msg.sender, _DreamPerSecond);
    }
