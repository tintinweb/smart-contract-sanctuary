...
// Rewards gauge interface - some function names may be different depending on the underlying farm
...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external view override returns (uint256) {
        return IRewardsGauge(rewards).claimable_reward(address(this), harvestedToken);
    }
    ...
    function deposit(uint256 _amount) internal override {
        ...
        if (_amount > 0) {
            ...
            IRewardsGauge(rewards).deposit(_amount);
        }
    }
        ...
        returns (uint256)
    {
        ...
        IRewardsGauge(rewards).withdraw(_amount);
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    address public constant crvAddr = 0x1E4F97b9f9F913c46F1632781732927B9019C68b;
    address public constant btcAddr = 0x321162Cd933E2Be498Cd2267a90534A804051b11;
    // Fee token
    ...
    address public constant feeTokenRouterAddr =
        0xF491e7B69E4244ad4002BC14e878a34207E38c29; // SpookySwap router
    address public constant pool = 0x3eF6A01A0f81D6046290f3e2A8c5b843e738E604; // Curve pool
        ...
        )
    {
        ...
        crvToFtmPath = new address[](2);
        crvToFtmPath[0] = crvAddr;
        crvToFtmPath[1] = rewardTokenAddr;

        rewardToFarmTokenPath1 = new address[](2);
        rewardToFarmTokenPath1[0] = rewardTokenAddr;
        rewardToFarmTokenPath1[1] = btcAddr;
    }
    ...
    function set_ftmCutoff(uint256 newCutoff) external onlyOwner {
        ftmCutoffBps = newCutoff;
        emit SetHarvestCutoff(ftmCutoffBps);
    }
    ...
    function harvest() public override nonReentrant {
        ...
        // Swap Crv to wFTM
        swapAmount = calculateSwapAmount(crvAddr, 10000);
        if (swapAmount > ftmCutoffBps) {
          _swapUniswapWithPathForFeeOnTransferTokens(
              crvToFtmPath,
              swapAmount,
              farmTokenRouterAddr
        ...
        if (_rewardBalance > harvestCutoffBps) {
            ...
            // Add liquidity
            uint256 _btc = IERC20(btcAddr).balanceOf(address(this));
            IERC20(btcAddr).safeApprove(pool, _btc);
            uint256[2] memory amounts;
            amounts[0] = _btc;
            ICurveSwap2(pool).add_liquidity(amounts, 0, true);
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public FARM_MASTER_CHEF = 0xBdFF0C27dd073C119ebcb1299a68A6A92aE607F0;
    address public LP_TOKEN = 0x5B5CFE992AdAC0C9D48E05854B2d91C73a003858;
