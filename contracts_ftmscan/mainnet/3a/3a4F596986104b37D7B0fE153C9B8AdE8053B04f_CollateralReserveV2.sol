...
contract CollateralReserveV2 is Ownable{
    ...
    address public gatev1 = 0x98e119990E3653486d84Ba46b66BbC4d82f7f604;
    ...
    modifier onlyOwnerOrGate() {
        require(owner() == msg.sender || gatev2 == msg.sender, "Only gate or owner can trigger this function");
        _;
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        value_ = 0;
        try ICollateralReserveV2OCalc( collateralCalc[ _token ] ).valuation(  _amount ) returns (uint v) {
            value_ = v;
    ...
    function getCollateralPrice(address _token) public view  returns (uint256) {
        require(address(collateralCalc[_token])!=address(0), "err0");
        try ICollateralReserveV2OCalc(collateralCalc[_token]).getPrice() returns ( uint256 price, uint8 d ) {
    ...
    function getCollateralBalance(address _token) public view  returns (uint256) {
        ...
        try ERC20(_token).decimals() returns (uint8 d) {
            ...
            try IERC20(_token).balanceOf(address(this)) returns ( uint256 balance ) {
                return balance.mul(10**missing_decimals);
            } catch Error(string memory /*reason*/) {
                return 0;
            }
        } catch Error(string memory /*reason*/) {
