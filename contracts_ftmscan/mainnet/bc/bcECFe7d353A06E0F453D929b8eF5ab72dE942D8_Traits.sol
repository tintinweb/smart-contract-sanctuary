...
import "./interfaces/INinjaToken.sol";
...
contract Traits is Ownable, ITraits {
    ...
    function setGame(address _YinVsYang) external onlyOwner {
        YinVsYang = INinjaToken(_YinVsYang);
    }
   ...
   */
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        INinjaToken.NinjaTokenTraits memory s = YinVsYang.getTokenTraits(tokenId);
        uint8 shift = s.isYin ? 0 : 10;

        string memory svgString = string(abi.encodePacked(
                s.isYin ? draw(thiefBody) : draw(policeBody),
                ...
                s.isYin ? drawTrait(traitData[5 + shift][s.headgear % traitCountForType[5 + shift]]) : drawTrait(traitData[5 + shift][s.alphaIndex]),
                !s.isYin ? drawTrait(traitData[6 + shift][s.neckGear % traitCountForType[6 + shift]]) : ''
            ));

        return string(abi.encodePacked(
                '<svg id="YinVsYang" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                svgString,
                "</svg>"
            ));
    }
   ...
   */
    function compileAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        return string(abi.encodePacked(
                ...
                tokenId <= YinVsYang.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
   ...
   */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked(
                ...
                s.isYin ? 'Yin #' : 'Yang #',
                tokenId.toString(),
                '", "description": "Yin & Yang Game is a new generation play-to-earn NFT game on FTM that incorporates probability-based derivatives alongside NFTs. Through a vast array of choices and decision-making possibilities, Yin & Yang Game promises to instil excitement and curiosity amongst the community as every individual adopts different strategies to do better than one another and to come out on top. The real question is, are you #TeamYin or #TeamYang? Choose wisely or watch the other get rich!", "image": "data:image/svg+xml;base64,',
...
interface INinjaToken {
  // struct to store each token's traits
  struct NinjaTokenTraits {
    bool isYin;
    uint8 uniform;
    uint8 hair;
    uint8 eyes;
  ...
  function getTokenTraits(uint256 tokenId)
    external
    view
    returns (NinjaTokenTraits memory);
}
...
interface IBank {
  ...
  function randomYangOwner(uint256 seed) external view returns (address);
...
contract YGOLD is ERC20, Ownable {
  // a mapping from an address to whether or not it can mint / burn
  mapping(address => bool) controllers;

  constructor() ERC20("YGOLD", "YGOLD") {
  ...
  /**
   * burns $YGOLD from a holder
   * @param from the holder of the $YGOLD
   * @param amount the amount of $YGOLD to burn
   */
  function burn(address from, uint256 amount) external {
...
import "./interfaces/ITraits.sol";
import "./interfaces/IYGOLD.sol";
import "./interfaces/ISeed.sol";

contract NinjaToken is
  INinjaToken,
  ERC721Enumerable,
  Ownable,
  Pausable
{
  ...
  uint256 public MINT_PRICE = 20 ether;
  uint256 public immutable MAX_TOKENS = 30000;
  uint256 public PAID_TOKENS = 5000;
  uint16 public minted;
  mapping(uint256 => NinjaTokenTraits) public tokenTraits;
    ...
    whenNotPaused
  {
    ...
    for (uint256 i = 0; i < amount; i++) {
      ...
      totalygoldCost += mintCost(minted);
    ...
    if (totalygoldCost > 0) ygold.burn(_msgSender(), totalygoldCost);
  ...
  function generate(uint256 tokenId, uint256 seed)
    internal
    returns (NinjaTokenTraits memory t)
  {
    t = selectTraits(seed);
  ...
  function selectRecipient(uint256 seed) internal view returns (address) {
    ...
    address Yin = bank.randomYangOwner(seed >> 144);
    // 144 bits reserved for trait selection
    if (Yin == address(0x0)) return _msgSender();
    return Yin;
  }
  ...
  function selectTraits(uint256 seed)
    ...
    returns (NinjaTokenTraits memory t)
  {
    t.isYin = (seed & 0xFFFF) % 10 != 0;
    uint8 shift = t.isYin ? 0 : 10;
  ...
  function structToHash(NinjaTokenTraits memory s) internal pure returns (uint256) {
    return
      uint256(
        keccak256(
          abi.encodePacked(
...
contract Bank is Ownable, IERC721Receiver, Pausable {
  ...
  // reference to the YinVsYang NFT contract
  NinjaToken game;
  // reference to the $YGOLD contract for minting $YGOLD earnings
  YGOLD ygold;
  ...
  // amount of $YGOLD due for each alpha point staked
  uint256 public ygoldPerAlpha = 0;

  // thief earn 38 $YGOLD per day
  uint256 public DAILY_ygold_RATE = 38 ether;
  // thief must have 2 days worth of $YGOLD to unstake or else it's too cold
  // uint256 public MINIMUM_TO_EXIT = 2 days;
  uint256 public MINIMUM_TO_EXIT = 2 days;
  // wolves take a 20% tax on all $YGOLD claimed
  uint256 public constant ygold_CLAIM_TAX_PERCENTAGE = 20;
  // there will only ever be (roughly) 1 billion $YGOLD earned through staking
  uint256 public constant MAXIMUM_GLOBAL_ygold = 1000000000 ether;

  // amount of $YGOLD earned so far
  uint256 public totalygoldEarned;
  // number of Thief staked in the Bank
  uint256 public totalThiefStaked;
  // the last time $YGOLD was claimed
  uint256 public lastClaimTimestamp;

  // emergency rescue to allow unstaking without any checks but without $YGOLD
  ...
  /**
   * @param _game reference to the YinVsYang NFT contract
   * @param _ygold reference to the $YGOLD token
   */
  constructor(
    address _game,
    address _ygold
  ) {
    game = NinjaToken(_game);
    ygold = YGOLD(_ygold);
    _pause();
  }
  ...
  function _addThiefToBankWithTime(
    ...
    totalygoldEarned +=
      ((time - lastClaimTimestamp) * totalThiefStaked * DAILY_ygold_RATE) /
      1 days;
  ...
  function _addPoliceToPack(address account, uint256 tokenId) internal {
    ...
    pack[alpha].push(
      Stake({
        owner: account,
        tokenId: uint16(tokenId),
        value: uint80(ygoldPerAlpha)
      })
    );
    // Add the police to the Pack
    emit TokenStaked(account, tokenId, ygoldPerAlpha);
  }

  /***CLAIMING / UNSTAKING */

  /**
   * realize $YGOLD earnings and optionally unstake tokens from the Bank / Pack
   * to unstake a Thief it will require it has 2 days worth of $YGOLD unclaimed
  ...
  function estimatedRevenuesOf(uint16[] calldata tokenIds) view external returns(uint) {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      if (isThief(tokenIds[i])) {
        ...
        uint newOwed = 0;
        if (totalygoldEarned < MAXIMUM_GLOBAL_ygold) {
          newOwed = ((block.timestamp - stake.value) * DAILY_ygold_RATE) / 1 days;
        } else if (stake.value > lastClaimTimestamp) {
          newOwed = 0;
        } else {
          newOwed = ((lastClaimTimestamp - stake.value) * DAILY_ygold_RATE) / 1 days;
        }
        owed += (newOwed * (100 - ygold_CLAIM_TAX_PERCENTAGE)) / 100;
      } else {
        uint256 alpha = _alphaForPolice(tokenIds[i]);
        Stake memory stake = pack[alpha][packIndices[tokenIds[i]]];
        owed += (alpha) * (ygoldPerAlpha - stake.value);
      }
    }
    return owed;
  }
  ...
  /**
   * realize $YGOLD earnings for a single Thief and optionally unstake it
   * if not unstaking, pay a 20% tax to the staked Polices
   * if unstaking, there is a 50% chance all $YGOLD is stolen
   * @param tokenId the ID of the Thief to claim earnings from
   * @param unstake whether or not to unstake the Thief
   * @return owed - the amount of $YGOLD earned
   */
    ...
    returns (uint256 owed)
  {
    ...
    require(
      ...
      "GONNA BE COLD WITHOUT TWO DAY'S YGOLD"
    );
    if (totalygoldEarned < MAXIMUM_GLOBAL_ygold) {
      owed = ((block.timestamp - stake.value) * DAILY_ygold_RATE) / 1 days;
    } else if (stake.value > lastClaimTimestamp) {
      owed = 0;
      // $YGOLD production stopped already
    } else {
      owed = ((lastClaimTimestamp - stake.value) * DAILY_ygold_RATE) / 1 days;
      // stop earning additional $YGOLD if it's all been earned
    }
    if (unstake) {
      _remove(_msgSender(), tokenId);
      if (random(tokenId) & 1 == 1) {
        // 50% chance of all $YGOLD stolen
        _payPoliceTax(owed);
        owed = 0;
      }
      ...
      _payPoliceTax((owed * ygold_CLAIM_TAX_PERCENTAGE) / 100);
      // percentage tax to staked wolves
      owed = (owed * (100 - ygold_CLAIM_TAX_PERCENTAGE)) / 100;
  ...
  /**
   * realize $YGOLD earnings for a single Police and optionally unstake it
   * Polices earn $YGOLD proportional to their Alpha rank
  ...
  /**
   * add $YGOLD to claimable pot for the Pack
   * @param amount $YGOLD to add to the pot
   */
  function _payPoliceTax(uint256 amount) internal {
    if (totalAlphaStaked == 0) {
      // if there's no staked wolves
      unaccountedRewards += amount;
      // keep track of $YGOLD due to wolves
      return;
    }
    // makes sure to include any unaccounted $YGOLD
    ygoldPerAlpha += (amount + unaccountedRewards) / totalAlphaStaked;
    unaccountedRewards = 0;
  }

  /**
   * tracks $YGOLD earnings to ensure it stops once 2.4 billion is eclipsed
   */
