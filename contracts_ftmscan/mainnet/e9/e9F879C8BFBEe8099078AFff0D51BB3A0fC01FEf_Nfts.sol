...
contract Nfts is NFTokenEnumerableMetadata, Ownable {
    ...
    function canSwap(address user) public view returns (bool) {
        return IErc20(swapTokenAddress).balanceOf(user) >= swapPrice;
    }
	...
	function mint(address _referrer) external payable {
        ...
        if (canSwap(msg.sender)) {
            ...
            IErc20(swapTokenAddress).transferFrom(msg.sender, address(0), swapPrice);
            amount = 1;
        } else {
    ...
    function setSwapTokenAddress(address _address) public onlyOwner {
        swapTokenAddress = _address;
    }
