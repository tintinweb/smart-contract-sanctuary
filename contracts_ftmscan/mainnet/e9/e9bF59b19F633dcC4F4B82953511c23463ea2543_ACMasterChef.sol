...
contract ACMasterChef is AutoCompoundStrategy {
    ...
    function _safeSwap(
        ...
        if (_path[0] == address(earnToken) && _path[2] == address(stakeToken) && _path.length == 3) {
            // first swap
            IERC20(_path[0]).safeIncreaseAllowance(address(liquidityRouter), _amountIn);
            ...
            // second swap
            uint256 amountSwap = IERC20(_path[1]).balanceOf(address(this));
            IERC20(_path[1]).safeIncreaseAllowance(address(swapRouter), amountSwap);
            swapPath[0] = _path[1];
            swapPath[1] = _path[2];
            if (_ignoreErrors) {
                try
