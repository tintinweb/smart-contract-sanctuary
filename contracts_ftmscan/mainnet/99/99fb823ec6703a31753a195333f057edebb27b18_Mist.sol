...
contract Mist is ERC20, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 MIST-WFTM LP -> MIST pool
    uint256 public constant INITIAL_MIST_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the MIST ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MIST", "MIST") {
        // Mints 1 MIST to contract creator for initial pool setup
    ...
    function _getMistPrice() internal view returns (uint256 _mistPrice) {
        try IOracle(mistOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Mist: failed to fetch MIST price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _mistPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_mistPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMistOracle(address _mistOracle) public onlyOperatorOrTaxOffice {
        require(_mistOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MIST to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MIST to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMistPrice = _getMistPrice();
            currentTaxRate = _updateTaxRate(currentMistPrice);
            if (currentMistPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_mistPool != address(0), "!_mistPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_mistPool, INITIAL_MIST_POOL_DISTRIBUTION);
