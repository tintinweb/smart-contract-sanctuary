...
contract wSPALiquidateSwapper is ISwapper {
    using BoringMath for uint256;
    ...
    IUniswapV2Pair constant SPA_DAI = IUniswapV2Pair(0xFa5a5F0bC990Be1D095C5385Fff6516F6e03c0a7);
    ...
    IERC20 public constant sSPA = IERC20(0x8e2549225E21B1Da105563D419d5689b80343E01);
    
    IWSPA public constant wSPA = IWSPA(0x89346B51A54263cF2e92dA79B1863759eFa68692);
    IStakingManager public constant STAKING_MANAGER = IStakingManager(0x9863056B4Bdb32160A70107a6797dD06B56E8137);
    
    IERC20 public constant SPA = IERC20(0x5602df4A94eB6C680190ACCFA2A475621E0ddBdc);

    constructor() public {
        // TETHER.approve(address(MIM3POOL), type(uint256).max);
        // MIM.approve(address(MIM3POOL), type(uint256).max);
        // SPA.approve(address(STAKING_MANAGER), type(uint256).max);
        // sSPA.approve(address(wSPA), type(uint256).max);
        DAI.approve(address(MIM3POOL), type(uint256).max);
        sSPA.approve(address(STAKING_MANAGER), type(uint256).max);
    }
    ...
    function swap(
        ...
        uint256 amountFirst = wSPA.unwrap(amountFrom);

        STAKING_MANAGER.unstake(amountFirst, true);

        SPA.transfer(address(SPA_DAI), amountFirst);


        (uint256 reserve0, uint256 reserve1, ) = SPA_DAI.getReserves();
        uint256 amountIntermediate = getAmountOut(amountFirst, reserve0, reserve1);
        SPA_DAI.swap(0, amountIntermediate, address(this), new bytes(0));


        uint256 amountTo = MIM3POOL.exchange_underlying(1, 0, amountIntermediate, 0, address(bentoBox));
