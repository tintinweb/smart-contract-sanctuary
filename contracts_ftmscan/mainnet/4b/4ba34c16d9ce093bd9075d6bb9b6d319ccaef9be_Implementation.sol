...
/*
    ...
    Copyright 2020 Zero Collateral Devs, standing on the shoulders of the Empty Set Squad <[email protected]>
...
contract Govern is Setters, Permission, Upgradeable {
    ...
    function commit(address candidate) external {
        ...
        uint256 endsAfter = startFor(candidate).add(periodFor(candidate)).sub(
            1
        );
        ...
        Require.that(
            epoch() <=
                endsAfter.add(1).add(Constants.getGovernanceExpiration()),
            FILE,
            "Expired"
        );
