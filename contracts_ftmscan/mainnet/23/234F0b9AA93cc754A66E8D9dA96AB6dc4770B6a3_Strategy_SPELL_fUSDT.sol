...
contract Strategy_SPELL_fUSDT is Ownable {
    ...
    address public routerLP = 0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52;
    address public routerReward = 0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52;
    address public routerMain = 0xF491e7B69E4244ad4002BC14e878a34207E38c29;
    ...
    function autoCompound() external {
        ...
        _approveToken(WETH, routerReward);
    ...
    function _swapWETHToTokens() internal {
        ...
        if (_balance > 0) {
            amount = _getAmountsOut(routerMain, _balance, path);
            if(amount > 100) {
                _swapExactTokensForTokens(routerMain, _balance, amount, path);
            }
        }
        ...
        path[1] = token1;
        if (_balance > 0) {
            amount = _getAmountsOut(routerLP, _balance, path);
            if(amount > 100) {
    ...
    function _swapFUSDTToSPELL() internal {
        ...
        if(balanceSpell > 0){
            ...
            path[0] = USDT;
            path[1] = token1;
            uint256 amount = _getAmountsOut(routerLP, balanceSpell, path);
            if(amount > 100){
                _swapExactTokensForTokens(routerLP, balanceSpell, amount, path);
            }
        }
    ...
    function _swapTokensToWETH() internal {
        // swap SPELL to fUSDT
        _swapSPELLToFUSDT();
        // swap all fUSDT to wFTM
        _swapTokenToWETH(routerMain, token0);
    }
    ...
    function _getToken0Price() internal view returns (uint256) {
        ...
        path[0] = token1;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(routerLP, 100 * 1e18, path) / 100) / 1e12;
    }
    ...
    function _getToken1Price() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return 1e24 / (_getAmountsOut(routerLP, 100 * 1e6, path) / 100);
    }

    function _getPriceWFTMToFUSDT() internal view returns (uint256) {
        // wFTM to fUSDT
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(routerMain, 100 * 1e18, path) / 100) / 1e12;
    }

    function _getPriceWFTMToSPELL() internal view returns (uint256 result) {
        // wFTM to SPELL through fUSDT
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = token0;

        uint256 amountFUSDT = _getAmountsOut(routerMain, 100 * 1e18, path);
        if (amountFUSDT > 0) {
            path[0] = token0;
            path[1] = token1;
            result = 1e36 / (_getAmountsOut(routerLP, amountFUSDT, path) / 100);
        }
    }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            // calculate SPELL as fUSDT
            if(t1V != 0) {
                path[0] = token1;
                path[1] = token0;
                amount = _getAmountsOut(routerLP, t1V, path);
            }
            // calculate fUSDT to wFTM 
            if(t0V + amount != 0) {
                path[0] = token0;
                path[1] = WETH;
                TC = _getAmountsOut(routerMain, t0V + amount, path);
            }
        }

        // calculates total Capital from tokens that exist on the contract
        
        // ----> calculates SPELL as fUSDT <----
        uint256 _balance = getBalanceOfToken(token1);
        amount = 0;
        if(_balance != 0) {
            path[0] = token1;
        ...
        if(_balance + amount != 0) {
            path[0] = token0;
            path[1] = WETH;
            TC += _getAmountsOut(routerMain, _balance + amount, path);
        }
