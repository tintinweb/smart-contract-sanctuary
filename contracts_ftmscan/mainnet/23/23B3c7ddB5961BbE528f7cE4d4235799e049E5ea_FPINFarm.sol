 ...
 * 
 *  - Users stake Pancakeswap LP Tokens to receive WBNB and SPIN Tokens as Rewards
 * 
 *  - Reward Tokens (SPIN) are added to contract balance upon deployment by deployer
 * 
 *  - After Adding the SPIN rewards, admin is supposed to transfer ownership to Governance contract
 * 
 *  - Users deposit Set (Predecided) pancakeswap LP Tokens and get a share of the farm.
 * 
 *  - The smart contract disburses `disburseAmount` SPIN as rewards over `disburseDuration`
 * 
 ...
 *      - SPIN-WBNB
 *      - SPIN-WBTC-WBNB (assumes appropriate liquidity is available in WBTC-WBNB pair)
 *      - SPIN-USDT-WBNB (assumes appropriate liquidity is available in USDT-WBNB pair)
 *      - SPIN-USDC-WBNB (assumes appropriate liquidity is available in USDC-WBNB pair)
 * 
 *  - Any swap may not have a price impact on SPIN price of more than approx ~2.49% for the related SPIN pair
 *      SPIN-WBNB swap may not have a price impact of more than ~2.49% on SPIN price in SPIN-WBNB pair
 *      SPIN-WBTC-WBNB swap may not have a price impact of more than ~2.49% on SPIN price in SPIN-WBTC pair
 *      SPIN-USDT-WBNB swap may not have a price impact of more than ~2.49% on SPIN price in SPIN-USDT pair
 *      SPIN-USDC-WBNB swap may not have a price impact of more than ~2.49% on SPIN price in SPIN-USDC pair
 * 
 *  - After the swap,converted WBNB is distributed to stakers at pro-rata basis, according to their share of the staking pool
 *    on the moment when the WBNB distribution is done. And remaining SPIN is added to the amount to be distributed or burnt.
 *    The remaining SPIN are also attempted to be swapped to WBNB in the next swap if the price impact is ~2.49% or less
 * 
 *  - At a set delay from last execution, Governance contract (owner) may execute disburse or burn features
 * 
 *  - Burn feature should send the SPIN tokens to set BURN_ADDRESS
 * 
 *  - Disburse feature should disburse the SPIN 
 *    (which would have a max price impact ~2.49% if it were to be swapped, at disburse time 
 *    - remaining SPIN are sent to BURN_ADDRESS) 
 *    to stakers at pro-rata basis according to their share of
 *    the staking pool at the moment the disburse is done
 * 
 *  - Users may claim their pending WBNB and SPIN anytime
 * 
 ...
 * 
 *  - Owner may transfer out WBNB and SPIN Tokens from this contract once `adminClaimableTime` is reached
 * 
 *  - CONTRACT VARIABLES must be changed to appropriate values before live deployment
 */

contract FPINFarm is Ownable {
    ...
    address public  trustedDepositTokenAddress; //= 0xC9811A8fE502fA23dfd6192a8Bf88d6d60a18415; //PancakePair contract address (testnet address)
    address public  trustedRewardTokenAddress; //= 0xcbEAF3BDe82155F56486Fb5a1072cb8baAf547cc; // SPIN token address
    
    // Make sure to double-check BURN_ADDRESS
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    
    // cliffTime - withdraw is not possible within cliffTime of deposit
    uint public constant cliffTime = 1 days;
    ...
    uint public constant adminCanClaimAfter = 60 days;
    ...
    constructor(address[] memory swapPath, address _trustedDepositTokenAddress, address _trustedRewardTokenAddress) public {
        ...
        pancakeswapRouterV2 = IpancakeswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506); //PancakeRouter Address (Testnet address)
    ...
    // To be executed by admin after deployment to add SPIN to contract
    ...
    // view function to check last updated SPIN pending rewards
    ...
    // private function to distribute SPIN rewards
    ...
    // private function to allocate SPIN to be disbursed calculated according to time passed
    ...
    // view function to calculate amount of SPIN pending to be allocated since `lastDisburseTime` 
