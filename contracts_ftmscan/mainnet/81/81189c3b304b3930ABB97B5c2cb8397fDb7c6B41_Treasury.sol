...
contract Treasury is ContractGuard {
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of ROCKET price
    ...
    uint256 public mintingFactorForPayingDebt; // print extra ROCKET during debt phase
    ...
    function getTombPrice() public view returns (uint256 tombPrice) {
        try IOracle(tombOracle).consult(tomb, 1e18) returns (uint144 price) {
            ...
            revert("Treasury: failed to consult ROCKET price from the oracle");
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_tombPrice <= tombPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = tombPriceOne.mul(1e18).div(_tombPrice); // to burn 1 ROCKET
    ...
    function initialize(
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn ROCKET and mint rBOND)
    ...
    function buyBonds(uint256 _tombAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(tombPrice == targetPrice, "Treasury: ROCKET price moved");
        require(
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochTombPrice > tombPriceCeiling) {
                // Expansion ($ROCKET Price > 1 $FTM): there is some seigniorage to be allocated
