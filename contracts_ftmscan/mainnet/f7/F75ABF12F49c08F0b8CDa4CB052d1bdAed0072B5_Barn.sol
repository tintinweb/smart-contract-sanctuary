...
contract Barn is Ownable, IERC721Receiver, Pausable {
        ...
        external
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            tokenIndex[tokenIds[i]] = userInfo[account].length;
            userInfo[account].push(tokenIds[i]);
        ...
        _updateEarnings
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(unstake){
                uint256 tokenIdx = tokenIndex[tokenIds[i]];
                uint256 lastElementTokenId = userInfo[_msgSender()][userInfo[_msgSender()].length-1];
                userInfo[_msgSender()][tokenIdx] = lastElementTokenId;
                tokenIndex[lastElementTokenId] = tokenIdx;
                userInfo[_msgSender()].pop();
            }
        }
        if (owed == 0) return;
    ...
    function getUserInfo(address wallet) public view returns(uint256[] memory){
        return userInfo[wallet];
    }

    function getTokenIndex(address wallet) public view returns(uint256[] memory){
        return userInfo[wallet];
    }
...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public wool_price = 500000 ether;
    uint256 public weed_price = 5000 ether;
    ...
    */
    function mint(
        ...
        if (tokenType == 0 && minted < PAID_TOKENS) {
            if (minted + amount <= PAID_TOKENS) {
                require(msg.value >= amount * MINT_PRICE, "not enought FTM");
            } else {
                require(
                    msg.value >= (amount + minted - PAID_TOKENS) * MINT_PRICE,
                    "not enought FTM"
                );
            }
        }
        ...
        uint256 totalWeedCost = 0;
        uint256 totalMilkCost = 0;
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            if (tokenType == 1 && minted < PAID_TOKENS) {
                totalWoolCost += wool_price;
            } else if (tokenType == 2 && minted < PAID_TOKENS) {
                totalWeedCost += weed_price;
            }
            totalMilkCost += mintCost(minted);
        }

        if (totalWoolCost > 0) wool.burn(_msgSender(), totalWoolCost);
        if (totalMilkCost > 0) milk.burn(_msgSender(), totalMilkCost);
        if (totalWeedCost > 0)
            weed.transferFrom(_msgSender(), address(this), totalWeedCost);
        ...
        returns (SheepWolf memory t)
    {
        ...
        return generate(tokenId, randomizer.random(seed));
    }
    ...
    function setMintPrice(uint256 _price, uint256 tokenType)
        external
        onlyOwner
    {
        if (tokenType == 0) {
    ...
    function setWeed(address _weed) external onlyOwner {
        weed = IERC20(_weed);
    }

    function setWool(address _wool) external onlyOwner {
        wool = WOOL(_wool);
    }

    function setMilk(address _milk) external onlyOwner {
        milk = MILK(_milk);
    }
