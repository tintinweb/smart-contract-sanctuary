...
// File: contracts/GRINCHDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable BUSDToken = address(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE);
...
contract GRINCH is ERC20, Ownable {
    ...
    address public immutable BUSDToken = address(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE);
    address private _marketingWallet = address(0x5C190D82B891d695b727dd628E90e68d3D9b8096);
    ...
    uint256 public _maxWalletToken = 20000000000000 * (10**18); // 2.0% of total supply
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GRINCH: The dividend tracker already has that address");

        GRINCHDividendTracker newDividendTracker = GRINCHDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GRINCH: The new dividend tracker must be owned by the GRINCH token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GRINCH: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GRINCH: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GRINCH: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GRINCH: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "GRINCH: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GRINCH: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GRINCH: Cannot update gasForProcessing to same value");
...
contract GRINCHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("GRINCH_Dividend_Tracker", "GRINCH_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GRINCH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GRINCH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GRINCH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GRINCH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GRINCH_Dividend_Tracker: Cannot update claimWait to same value");
