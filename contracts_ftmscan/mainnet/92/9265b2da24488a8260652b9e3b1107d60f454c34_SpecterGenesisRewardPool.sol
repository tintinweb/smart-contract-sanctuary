...
// File: contracts/distribution/SpecterGenesisRewardPool.sol
...
contract SpecterGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SPECTER to distribute.
        uint256 lastRewardTime; // Last time that SPECTER distribution occurs.
        uint256 accSpecterPerShare; // Accumulated SPECTER per share, times 1e18. See below.
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SpecterGenesisPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "SpecterGenesisPool: existing pool?");
        }
    }
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(specterPerSecond);
            return poolEndTime.sub(_fromTime).mul(specterPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(specterPerSecond);
            return _toTime.sub(_fromTime).mul(specterPerSecond);
        }
    }

    // View function to see pending SPECTER on frontend.
    ...
    function safeSpecterTransfer(address _to, uint256 _amount) internal {
        uint256 _specterBalance = specter.balanceOf(address(this));
        if (_specterBalance > 0) {
            if (_amount > _specterBalance) {
                specter.safeTransfer(_to, _specterBalance);
            } else {
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (SPECTER or lps) if less than 90 days after pool ends
