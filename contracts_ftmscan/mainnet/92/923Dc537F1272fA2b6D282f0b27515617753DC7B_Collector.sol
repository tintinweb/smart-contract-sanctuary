...
contract Collector is Ownable {
    ...
    IERC20 public immutable token = IERC20(address(0x3b57f3FeAaF1e8254ec680275Ee6E7727C7413c7));
    IwOHM public immutable wrappedToken = IwOHM(address(0xe992C5Abddb05d86095B18a158251834D616f0D1));
    ...
    function _allowance(address _wallet) internal view returns (uint256) {
        return token.allowance(_wallet, address(this));
    }
    ...
    function balanceOf() public view returns (uint256) {
        ...
        for (uint8 i = 0; i < _wallets.length(); ++i) {
            total += _balanceOf(_wallets.at(i));
        }
    ...
    function transfer() public returns (uint256) {
        ...
        for (uint8 i = 0; i < _wallets.length(); ++i) {
            uint256 balance = _balanceOf(_wallets.at(i));

            uint256 allowance = _allowance(_wallets.at(i));
        ...
        token.approve(address(wrappedToken), total);
        total = wrappedToken.wrapFromOHM(total);
        wrappedToken.transfer(owner(), total);
    ...
    function withdrawWrapped() public returns (uint256) {
        uint256 balance = wrappedToken.balanceOf(address(this));
        require(balance > 0, "no stuck balance");
        wrappedToken.safeTransfer(owner(), balance);
        return balance;
    }
}