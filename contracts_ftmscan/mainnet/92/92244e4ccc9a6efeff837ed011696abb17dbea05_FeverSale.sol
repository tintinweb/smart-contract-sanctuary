...
// File: contracts/eversale.sol

pragma solidity 0.8.0;

contract FeverSale is Ownable {
    using SafeERC20 for ERC20;
    using Address for address;
//Change values
    uint constant FTMdecimals = 10 ** 18;
    uint pricedecimals = 10 ** 17;
    uint constant Feverdecimals = 10 ** 18;
    uint public MAX_SOLD = 300000 * Feverdecimals;
    uint public PRICE = 1 * pricedecimals;
    uint public MIN_PRESALE_PER_ACCOUNT = 25 * Feverdecimals;
    uint public MAX_PRESALE_PER_ACCOUNT = 450 * Feverdecimals;
    address public owners;
    ERC20 FTM;

    uint public sold;
    address public Fever;
    bool canClaim;
    bool publicSale;
    bool privateSale;
    address[] devAddr = [0x0a1b544c33C1B3e7ea723B51b37da291F497e304, 0xd3318ADa5bb581608aA3dc1D0b7C86ecaaAfb60E,
    ...
    constructor() {
        ...
        FTM = ERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
        owners = msg.sender;
        sold = 0;
        //DEV REWARDS
        for( uint256 iteration_ = 0; devAddr.length > iteration_; iteration_++ ) {
            invested[ devAddr[ iteration_ ] ] = 100 * Feverdecimals;
        } 
    }
    ...
    function buyFever(uint256 amount) public onlyEOA {
        ...
        FTM.safeTransferFrom( msg.sender, address(this), amount * PRICE );
        invested[msg.sender] += amount;
        sold += amount;
    }

    // set Fever token address and activate claiming
    function setClaimingActive(address fever) public {
    ...
    // claim Fever allocation based on invested amounts
    function claimFever() public onlyEOA {
        ...
        if (isDev(msg.sender)) {
            ERC20(Fever).transfer(msg.sender, amountPerClaim[msg.sender]);
    ...
    function maxSold(uint _MAX_SOLD) public {
        require(msg.sender == owners, "not owners");
        MAX_SOLD = _MAX_SOLD;
    }
    ...
    function setPriceDecimals(uint _pricedecimals) public {
        require(msg.sender == owners, "not owners");
    ...
    // emergency function
    function stuckBalance(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "insufficient ETH balance");
        payable(msg.sender).transfer(weiAmount);
    }

    function transferTKNs(address tokenAddress) external onlyOwner{
