...
// JuixToken with Governance.
contract JuixToken is BEP20 {
    ...
    event JuixSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "JUIX::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the JuixToken contract.
     */
    constructor() public BEP20("Juix Token", "JUIX") {
    ...
    /// @dev overrides transfer BEP20 function to meet tokenomics of JUIX
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        
        if (recipient == BURN_ADDRESS || _excludedFromTrsfTax[sender] == true || _excludedFromTrsfTax[recipient] == true || burnRateTax == 0) {            
            ...
            require(amount == sendAmount + burnAmount, "JUIX::transfer: Burn value invalid");
    ...
    function maxTransferAmount() public view returns (uint256) {
        ...
        // if antiwhale maxTransferAmountRate is set too low, a minimum of 5000 JUIX is applied
    ...
    function updateBurnRate(uint16 _burnRateTax) public onlyOperator {
        require(_burnRateTax <= 200, "JUIX::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "JUIX::updateMaxTransferAmountRate: Max transfer amount rate must not exceed 100 percents.");
        require(_maxTransferAmountRate >= 1, "JUIX::updateMaxTransferAmountRate: Max transfer amount rate must not be less than 0.01 percents.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "JUIX::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "JUIX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "JUIX::delegateBySig: invalid nonce");
        require(now <= expiry, "JUIX::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "JUIX::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying JUIXs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "JUIX::_writeCheckpoint: block number exceeds 32 bits");
    ...
    //a way to get back other BEP20 tokens sended by error into the JUIX token contract
