...
/**
 * Staking contract for FTMO-LP
 */

import "./FTMOToken.sol";
import "./IERC20.sol";

contract FTMOStakingLPnew {
    struct _rewardInfo {
    ...
    IERC20 public LP; // LP token contract
    ...
    constructor(address admin)
    {
        ...
        LP = IERC20(0x9A3C6C8E58BecF7Fed10606dD368dA79Ec6E0b0E); // USDCLP
        lastTime = 0;
        startTime = 0;
        RATE = 0;
        totalTokensStaked = 0;
    ...
    function depositStake(uint256 amount)
        external
    {
        require(poolActive, "FTMOStakingLP: pool not active");
        require(amount > 0, "FTMOStakingLP: invalid deposit amount");
        require(
            LP.allowance(msg.sender, address(this)) >= amount,
            "FTMOStakingLP: insufficient allowance"
        );
        require(
            LP.balanceOf(msg.sender) >= amount,
            "FTMOStakingLP: not enough tokens"
        );
    ...
    function withdrawStake(uint256 amount)
        ...
        onlyStakers
    {
        require(totalTokensStaked > 0, "FTMOStakingLP: no tokens staked in pool");
        require(amount > 0, "FTMOStakingLP: invalid withdraw amount");
        uint256 balance = _stakerInfo[msg.sender].tokensStaked;
        require(amount <= balance, "FTMOStakingLP: over withdraw limit");
        ...
        onlyStakers
    {
        ...
        require(
            amount <= max,
            "FTMOStakingLP: amount exceeds withdrawable FTMO"
        );
    ...
    function pausePool()
        ...
        onlyAdmin
    {
        require(poolActive, "FTMOStakingLP: pool is paused");
        updateRunningSum();
        poolActive = false;
    }
    ...
    function resumePool()
        ...
        onlyAdmin
    {
        require(!poolActive, "FTMOStakingLP: pool is active");
        lastTime = block.timestamp;
        poolActive = true;
    }
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == _admin,
            "FTMOStakingLP: only admin can do this"
        );
        _;
    }
    ...
    modifier onlyStakers() {
        require(
            isStaker[msg.sender],
            "FTMOStakingLP: only stakers allowed"
        );
        _;
    }
    ...
    modifier poolNotStarted() {
        require(!poolStarted, "FTMOStakingLP: pool already started");
        _;
    }
}