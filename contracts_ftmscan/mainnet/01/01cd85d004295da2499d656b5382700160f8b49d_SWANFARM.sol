...
contract ERC20 is Context, IERC20 {
    ...
    bool public LimitMode = true;   // LimitMode is activated and limit to 1000 SWAN per buy. It will be shut down after few minutes. It can only be deactivated.

    address constant public devWallet = 0x6035c22FA95597b9862452cC3426bF82aCf13A11; 
    ...
    // Make sure bots dont take over. ListingMode is disable after listing and cant be enabled again
    // Only dev can buy / transfer
        if (ListingMode) {
            require(sender == devWallet, "Listing Mode on : no transfer or buy allowed");
        }

    // Botbuster, we plan to limit the amount of token that one can buy to 2800 SWAN. 
        if (LimitMode == true && sender != devWallet) {
            require(amount <= uint(28e20), "Limit Mode on : max buy authorized is 2800 SWAN");
        }
...
// SWANToken
contract SWANToken is ERC20("SWAN", "SWAN"), Ownable {
    using SafeMath for uint256;
    ...
    // Disable any buy while creating the LP and distributing to the WL
    function disableListingMode() public onlyOwner {
            ListingMode = false;
        }
  
    // Disable only function. Botbuster to let everyone buy SWAN at fair price at listing
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "SWAN::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SWAN::delegateBySig: invalid nonce");
        require(now <= expiry, "SWAN::delegateBySig: signature expired");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SWANs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "SWAN::_writeCheckpoint: block number exceeds 32 bits");
...
contract SWANFARM is Ownable {
    ...
    SWANToken public SWAN;    // The SWAN TOKEN!
    ...
    uint256 public constant startBlock = 64287;    // The block number when SWAN mining starts.
    uint256 public bonusEndBlock = 64287;
    
    uint256 public constant DEV_TAX = 5;
    uint256 public constant BONUS_MULTIPLIER = 1;

	uint256 public swanPerBlock = 1e18;
	uint256 public berhaneValue = 3e12;
    ...
    function pendingSWAN(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 swanReward = blocksToReward.mul(swanPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        // Get number of blocks since last update of swanPerBlock
        ...
        // Set the new number of swanPerBlock with Berhane
        swanPerBlock = swanPerBlock.sub(Berhane);
        ...
        uint256 swanReward = blocksToReward.mul(swanPerBlock.add(CompensationSinceLastRewardUpdate)).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Deposit LP tokens to swanfarm for SWAN allocation.
    ...
    // Withdraw LP tokens from swanfarm.
    ...
    function safeSwanTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > SwanBal) {
            SWAN.transfer(_to, SwanBal);
        } else {
            SWAN.transfer(_to, _amount);
        }
    }
