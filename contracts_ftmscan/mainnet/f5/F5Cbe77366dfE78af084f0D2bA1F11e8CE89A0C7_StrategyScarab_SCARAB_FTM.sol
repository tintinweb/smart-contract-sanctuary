...
import {BaseStrategyMCMaximizer} from "../../base/BaseStrategyMCMaximizer.sol";

contract StrategyScarab_SCARAB_FTM is BaseStrategyMCMaximizer {

    address scarab_ftm_diff;

    function initializeStrategy(
        address _store,
        address _vault
    ) public initializer {
        address masterchef = address(0xc88690163b10521d5fB86c2ECB293261F7771525);
        address scarab_ftm_lp = address(0x78e70eF4eE5cc72FC25A8bDA4519c45594CcD8d4);
        address gscarab = address(0x6ab5660f0B1f174CFA84e9977c15645e4848F5D6);
        address wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
        address spiritRouter = address(0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52);
        address bgscarab_ftm_lp = address(0x92c04044662ca04AadD616A1ac360E0E978B71ec);
        __Strategy_init(
            _store,
            scarab_ftm_lp,
            _vault,
        ...
        routes[gscarab] = [gscarab];
        routes[wftm] = [gscarab, wftm];

        // Set the sell floor to a low amount to ensure liquidations go through.
        _setSellFloor(1e10);
    }
}
    ...
    */
    function _liquidateReward() internal {
        ...
        if(routes[token0].length > 1) {
            IERC20(rewardToken()).safeApprove(swapRouter(), 0);
            IERC20(rewardToken()).safeApprove(swapRouter(), toToken0);
            uint256[] memory amounts = IUniswapV2Router02(swapRouter()).swapExactTokensForTokens(toToken0, 0, routes[token0], address(this), (block.timestamp + 600));
        ...
        if(routes[token1].length > 1) {
            IERC20(rewardToken()).safeApprove(swapRouter(), 0);
            IERC20(rewardToken()).safeApprove(swapRouter(), toToken1);
            uint256[] memory amounts = IUniswapV2Router02(swapRouter()).swapExactTokensForTokens(toToken1, 0, routes[token1], address(this), (block.timestamp + 600));
        ...
        IERC20(token0).safeApprove(swapRouter(), token0Amount);

        IERC20(token1).safeApprove(swapRouter(), 0);
        IERC20(token1).safeApprove(swapRouter(), token1Amount);

        IUniswapV2Router02(swapRouter()).addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), (block.timestamp + 600));
    }
