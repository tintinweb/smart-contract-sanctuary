...
contract SmurfBondFactory {
    ...
    function withdrawLps(uint256 _depositAmt) public {
        UserInfo storage user = userInfo[msg.sender];
        ...
        require (_depositAmt <= user.userAmountBonds, "withdrawLps: output amount to high");
        require (_depositAmt > 0, "withdrawLps: invalid output amount");

        // Calculate share of paid back Bonds
        uint256 sharePaidBack = (_depositAmt * 1e18) / user.userAmountBonds;

        // Transfer Bonds to Burn Address    
        IERC20(sbond).transferFrom(address(msg.sender), address(this), _depositAmt);
        user.userAmountBonds = user.userAmountBonds.sub(_depositAmt);
        totalBondsBurned = totalBondsBurned.add(_depositAmt);

        if (user.amountCleverFtm > 0) {
            uint256 sendAmount1 = (sharePaidBack * user.amountCleverFtm) / 1e18;
            IERC20(CleverFtmLp).transferFrom(address(this), address(msg.sender), sendAmount1);
            user.amountCleverFtm = user.amountCleverFtm.sub(sendAmount1);
        }
