...
contract Graveyard is Ownable {
    using SafeMath for uint256;

    Zombie public zombie;
    Brain public brain;
    address public brainJarAddress;

    uint256 public constant YIELD_BPS = 16666666666666667; // brains collected per second per unit of yield
    uint256 public constant CLAIM_TAX_PERCENTAGE = 30;
    ...
    mapping(address => uint256) public stakedUpgradesCount;
    mapping(address => uint256) public numUpgradeSlots;
    ...
    constructor(Zombie _zombie, Brain _brain, address _brainJarAddress) {
        zombie = _zombie;
        brain = _brain;
        brainJarAddress = _brainJarAddress;
    }
    ...
    function getBrainsAccruedForMany(uint256[] calldata _tokenIds) external view returns (uint256[] memory) {
        uint256[] memory brainAmounts = new uint256[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            brainAmounts[i] = _getBrainsAccruedFor(_tokenIds[i], false);
        }
        return brainAmounts;
    }

    function _getBrainsAccruedFor(uint256 _tokenId, bool checkOwnership) internal view returns (uint256 accrual) {
        ...
        uint256 stakeStartPeriod = (stake.startTimestamp - zombie.gameStartTimestamp()) / HALVING_PERIOD;
        uint256 currentPeriod = (block.timestamp - zombie.gameStartTimestamp()) / HALVING_PERIOD;

        if (stakeStartPeriod == currentPeriod) {
            accrual = (block.timestamp - stake.startTimestamp) * (zombie.tokenYield(_tokenId) * YIELD_BPS >> currentPeriod);
        } else {
            uint256 elapsedInStartPeriod = 1 days - (stake.startTimestamp - zombie.gameStartTimestamp()) % HALVING_PERIOD;
            uint256 elapsedInCurrentPeriod = (block.timestamp - zombie.gameStartTimestamp()) % HALVING_PERIOD;

            accrual += elapsedInStartPeriod * (zombie.tokenYield(_tokenId) * YIELD_BPS >> stakeStartPeriod);
            for (uint256 t = stakeStartPeriod + 1; t < currentPeriod; t++) {
                accrual += 1 days * (zombie.tokenYield(_tokenId) * YIELD_BPS >> t);
            }
            accrual += elapsedInCurrentPeriod * (zombie.tokenYield(_tokenId) * YIELD_BPS >> currentPeriod);
        }
    }
    ...
    function stakeMany(uint256[] calldata _tokenIds) external {
        require(zombie.gameStarted(), "The game has not started");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(zombie.ownerOf(tokenId) == _msgSender(), "You don't own this token");
            require(zombie.tokenRevealed(tokenId), "Token not yet revealed");

            zombie.transferFrom(_msgSender(), address(this), tokenId);
            _addZombieToGraveyard(tokenId, _msgSender());

            if (zombie.isUpgrade(tokenId)) {
                stakedUpgradesCount[_msgSender()]++;
            } else {
                if (zombie.revealedTokenLevel(tokenId) == 0) {
                    numUpgradeSlots[_msgSender()]++;
                } else {
                    numUpgradeSlots[_msgSender()] += 10;
                }
            }
        }

        require(stakedUpgradesCount[_msgSender()] <= numUpgradeSlots[_msgSender()], "Insufficient upgrade slots");
    }

    function claimBrainsAndMaybeUnstake(uint256[] calldata _tokenIds, bool unstake) external {
        uint256 totalClaimed = 0;
        uint256 totalTaxed = 0;

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            uint256 brains = _getBrainsAccruedFor(tokenId, true); // also checks that msg.sender owns this token
            uint256 taxAmount = (brains * CLAIM_TAX_PERCENTAGE + 99) / 100; // +99 to round the division up

            totalClaimed += brains - taxAmount;
            totalTaxed += taxAmount;
            stakes[tokenId].startTimestamp = block.timestamp;

            if (unstake) {
                if (zombie.isUpgrade(tokenId)) {
                    _removeUpgradeFromGraveyard(tokenId);
                    stakedUpgradesCount[_msgSender()]--;
                    zombie.transferFrom(address(this), _msgSender(), tokenId);

                } else {
                    _moveZombieToCooldown(tokenId);
        ...
        brain.mint(_msgSender(), totalClaimed);
        brain.mint(brainJarAddress, totalTaxed);
    }

    function withdrawZombies(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            ...
            zombie.transferFrom(address(this), _msgSender(), tokenId);
            _removeZombieFromCooldown(tokenId);
        }
    }

    function _addZombieToGraveyard(uint256 _tokenId, address _owner) internal {
    ...
    function _moveZombieToCooldown(uint256 _tokenId) internal {
    ...
    function _removeZombieFromCooldown(uint256 _tokenId) internal {
    ...
    function _removeUpgradeFromGraveyard(uint256 _tokenId) internal {
        address owner = stakes[_tokenId].owner;
        delete stakes[_tokenId];
        _removeStakeFromOwnerEnumeration(owner, _tokenId);
    }
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = OwnedStakeInfo({
                tokenId: tokenId,
                level: zombie.revealedTokenLevel(tokenId),
                startTimestamp: stakes[tokenId].startTimestamp
            });
        }
