...
contract Cloud is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the CLOUD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("CLOUD", "CLOUD") {
        // Mints 1 CLOUD to contract creator for initial pool setup
    ...
    function _getCloudPrice() internal view returns (uint256 _cloudPrice) {
        try IOracle(cloudOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Cloud: failed to fetch CLOUD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cloudPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cloudPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCloudOracle(address _cloudOracle) public onlyOperatorOrTaxOffice {
        require(_cloudOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CLOUD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CLOUD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCloudPrice = _getCloudPrice();
            currentTaxRate = _updateTaxRate(currentCloudPrice);
            if (currentCloudPrice < burnThreshold) {
                burnTax = true;
            }
        }
