...
contract ExtractorCalculator is IExtractorCalculator {
    ...
    constructor (address _Cyber) {
        require(_Cyber != address(0));
        Cyber = IERC20(_Cyber);
    }

    function getKValue (address _pair) public view returns (uint k_) {
        uint token0 = IERC20(IPair(_pair).token0()).decimals();
        uint token1 = IERC20(IPair(_pair).token1()).decimals();
    ...
    function valuation (address _pair, uint amount_) external view override returns (uint _value) {
        ...
        uint totalSupply = IPair(_pair).totalSupply();
    ...
    function markdown (address _pair) external view returns (uint) {
        ...
        uint reserve;
        if (IPair(_pair).token0() == address(Cyber)) {
            reserve = reserve1;
        } else {
            require(IPair(_pair).token1() == address(Cyber), "Not a Cyber LP pair");
            reserve = reserve0;
        }

        return reserve.mul(2 * (10 ** Cyber.decimals())).div(getTotalValue(_pair));
    }
}