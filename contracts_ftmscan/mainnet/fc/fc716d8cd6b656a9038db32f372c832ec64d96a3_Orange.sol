...
contract Orange is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the days 2-3 ORANGE-WFTM LP -> ORANGE pool
    uint256 public constant INITIAL_ORANGE_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the ORANGE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ORANGE Finance", "ORANGE") {
        // Mints 1 ORANGE to contract creator for initial pool setup
    ...
    function _getORANGEPrice() internal view returns (uint256 _orangePrice) {
        try IOracle(orangeOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("ORANGE: failed to fetch ORANGE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _orangePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_orangePrice >= taxTiersTwaps[tierId]) {
    ...
    function setORANGEOracle(address _orangeOracle) public onlyOperatorOrTaxOffice {
        require(_orangeOracle != address(0), "oracle address cannot be 0 address");
        orangeOracle = _orangeOracle;
    }
    ...
    /**
     * @notice Operator mints ORANGE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ORANGE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentORANGEPrice = _getORANGEPrice();
            currentTaxRate = _updateTaxRate(currentORANGEPrice);
            if (currentORANGEPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_orangePool != address(0), "!_orangePool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_orangePool, INITIAL_ORANGE_POOL_DISTRIBUTION);
