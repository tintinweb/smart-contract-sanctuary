...
/**
 *Join our Telegram Group: https://t.me/fantomspooky
*/
...
contract FantomSpookyToken is ERC20('Fantom Spooky Token', 'FST') {

    constructor() {
        _mint(_msgSender(), uint256(50000000000000000000));
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accSpookyPerShare;    
        uint16 depositFeeBP;
        uint256 lpSupply;
    }
    
    FantomSpookyToken public immutable spooky;
    
    uint256 public startTime;
    address public feeAdd;
    address public treasuryAdd;
    
    uint256 private immutable SPOOKYMAXSUPPLY;
    uint256 public spookyPerSec = 0.001 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 spookyPerSec);

    constructor(FantomSpookyToken _spooky, uint256 _startTime, address _feeAdd, address _treasuryAdd) {
        spooky = _spooky;
        startTime = _startTime;
        feeAdd = _feeAdd;
        treasuryAdd = _treasuryAdd;
        SPOOKYMAXSUPPLY  = _spooky.maxSupply();
    }
    ...
    function safeSpookyTransfer(address _to, uint256 _amount) internal {
        uint256 spookyBalance = spooky.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > spookyBalance) {
            transferSuccess = spooky.transfer(_to, spookyBalance);
        } else {
            transferSuccess = spooky.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTerraTransfer: Transfer failed");
    }
    
    function updateEmissionRate(uint256 _spookyPerSec) external onlyOwner {
        require(_spookyPerSec >= MIN_PERSEC && _spookyPerSec <= MAX_PERSEC, "Bad Emissions per sec");
        massUpdatePools();
        spookyPerSec = _spookyPerSec;
        emit UpdateEmissionRate(msg.sender, _spookyPerSec);
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (spooky.totalSupply() >= SPOOKYMAXSUPPLY) { 
            return 0; 
        }
        return _to - _from;
    }

    function pendingSpooky(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accSpookyPerShare = pool.accSpookyPerShare;
        ...
        if (block.timestamp > pool.lastRewardperSec && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 spookyReward = (multiplier * spookyPerSec * pool.allocPoint) / totalAllocPoint;
            accSpookyPerShare += spookyReward * 1e18 / pool.lpSupply;
        }
        return ((user.amount * accSpookyPerShare) / 1e18) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 spookyReward = (multiplier * spookyPerSec * pool.allocPoint) / totalAllocPoint;
        uint256 toTreasury = spookyReward / 10;
        uint256 SpookyCalc = spooky.totalSupply() + spookyReward + toTreasury;
        
      if (SpookyCalc <= SPOOKYMAXSUPPLY) {
            spooky.mint(treasuryAdd, toTreasury);
            spooky.mint(address(this), spookyReward);
        } else {
            spookyReward = SPOOKYMAXSUPPLY - spooky.totalSupply();
            spooky.mint(address(this), spookyReward);
        }
        
        pool.accSpookyPerShare += spookyReward * 1e18 / pool.lpSupply;
        pool.lastRewardperSec = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accSpookyPerShare) / 1e18) - user.rewardDebt;
            
            if (pending > 0) {
                safeSpookyTransfer(msg.sender, pending);
            }
        }
        ...
        user.rewardDebt = (user.amount * pool.accSpookyPerShare) / 1e18;
