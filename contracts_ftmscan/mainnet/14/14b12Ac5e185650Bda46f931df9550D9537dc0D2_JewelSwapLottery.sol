...
// File: contracts/interfaces/IJewelSwapLottery.sol

pragma solidity ^0.8.4;

interface IJewelSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in JWL per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in JWL token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInJwl: price of a ticket in JWL
...
// File: contracts/JewelSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title JewelSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract JewelSwapLottery is ReentrancyGuard, IJewelSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInJwl = 50 ether;
    uint256 public minPriceTicketInJwl = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInJwl;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] jwlPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInJwl;
        uint32 finalNumber;
    }
     ...
     * @param _jwlTokenAddress: address of the JWL token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _jwlTokenAddress, address _randomGeneratorAddress) {
        jwlToken = IERC20(_jwlTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of JWL to this contract
        uint256 amountJwlToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInJwl,
            _ticketNumbers.length
        );

        // Transfer jwl tokens to this contract
        jwlToken.safeTransferFrom(address(msg.sender), address(this), amountJwlToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInJwl += amountJwlToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInJwlToTransfer
        uint256 rewardInJwlToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInJwlToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        jwlToken.safeTransfer(msg.sender, rewardInJwlToTransfer);

        emit TicketsClaim(msg.sender, rewardInJwlToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInJwl) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in JWL for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].jwlPerBracket[j] =
                ...
                // A. No JWL to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].jwlPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInJwl - amountToShareToWinners);

        // Transfer JWL to treasury address
        jwlToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        jwlToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInJwl += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInJwl >= minPriceTicketInJwl) && (_priceTicketInJwl <= maxPriceTicketInJwl),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInJwl: _priceTicketInJwl,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            jwlPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInJwl: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(jwlToken), "Cannot be JWL token");
    ...
    /**
     * @notice Set JWL price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInJwl: minimum price of a ticket in JWL
     * @param _maxPriceTicketInJwl: maximum price of a ticket in JWL
     */
    function setMinAndMaxTicketPriceInJwl(uint256 _minPriceTicketInJwl, uint256 _maxPriceTicketInJwl)
        external
        onlyOwner
    {
        require(_minPriceTicketInJwl <= _maxPriceTicketInJwl, "minPrice must be < maxPrice");

        minPriceTicketInJwl = _minPriceTicketInJwl;
        maxPriceTicketInJwl = _maxPriceTicketInJwl;
    }
     ...
     * @param _priceTicket price of a ticket (in JWL)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].jwlPerBracket[_bracket];
        } else {
            return 0;
        }
    }
