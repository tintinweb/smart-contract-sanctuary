...
contract CHRONUS_Token is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 CHRONUS-WETH LP -> CHRONUS pool
    uint256 public constant INITIAL_CHRONUS_POOL_DISTRIBUTION = 140000 ether;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Chronus Token", "CHRONUS") {
        // Mints 1 CHRONUS to contract creator for initial pool setup
    ...
    function _getChronusPrice() internal view returns (uint256 _ChronusPrice) {
        try IOracle(ChronusOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Chronus: failed to fetch CHRONUS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ChronusPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ChronusPrice >= taxTiersTwaps[tierId]) {
    ...
    function setChronusOracle(address _ChronusOracle) public onlyOperatorOrTaxOffice {
        require(_ChronusOracle != address(0), "oracle address cannot be 0 address");
        ChronusOracle = _ChronusOracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentChronusPrice = _getChronusPrice();
            currentTaxRate = _updateTaxRate(currentChronusPrice);
            if (currentChronusPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _ChronusPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_ChronusPool != address(0), "!_ChronusPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_ChronusPool, INITIAL_CHRONUS_POOL_DISTRIBUTION);
    }
