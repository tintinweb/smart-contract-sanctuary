...
contract FeedVaultBaseV2 is FeedVault {
    ...
    function _collectEntryFees(uint256 _amount) internal override {
        ...
        if (_entryFees > 0) {
            if (autoSwapEntryFees) {
                ...
                IERC20(_entrySwapToken).safeTransfer(entryFeesCollector, buyBackAmount);

                emit FeesCollected(true, address(entryFeesCollector), address(_entrySwapToken), _entryFees);
            } else {
    ...
    function _collectExitFees(uint256 _amount) internal override returns (uint256) {
        ...
        if (_exitFees > 0) {
            if (autoSwapExitFees) {
                ...
                uint256 buyBackAmount = buyBackAfter.sub(buyBackBefore);
                IERC20(_exitSwapToken).safeTransfer(exitFeesCollector, buyBackAmount);

                emit FeesCollected(false, address(exitFeesCollector), address(_exitSwapToken), _exitFees);
            } else {
     ...
     */
    function setEntryFeesToTokenPath(address[] memory _entryFeesToTokenPath) public virtual onlyRole(ADMIN_ROLE) nonReentrant {
        entryFeesToTokenPath = _entryFeesToTokenPath;
    }
     ...
     */
    function setExitFeesToTokenPath(address[] memory _exitFeesToTokenPath) public virtual onlyRole(ADMIN_ROLE) nonReentrant {
        exitFeesToTokenPath = _exitFeesToTokenPath;
    }
}
