...
contract SaleFactory is ISaleFactory, Ownable {
     ...
     */
    function initFactory(ISaleModel[] memory _models) external onlyOwner {
    ...
    /**
        ...
        @param model    Address of the new sale model contract.
     */
    function deployModel(ISaleModel model) external override init onlyOwner {
        models.push(model);
        emit NewSaleModel();
    }
     ...
     */
    function removeModel(uint256 index) external init onlyOwner {
    ...
    function createSale(
        ...
        (address cloneContract, ) = _createSale(collection, model);
        ...
        require(success, "payload initialisation error");
    ...
    function createSale(
        ...
        (address cloneContract, bool success) = _createSale(collection, model);
    ...
    /**
        @notice Handles the cloning of a sale model.
        @param collection       NFT collection being sold.
        @param model            Sale model being used to conduct sale.
     */
    function _createSale(
        INftCollectionSale collection, 
        uint16 model
    ) internal returns (address result, bool successfulInit) {
