...
contract Vexa is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 VEXA-WFTM LP -> VEXA pool
    ...
    /**
     * @notice Constructs the VEXA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("VEXA", "VEXA") {
    ...
    function _getTombPrice() internal view returns (uint256 _tombPrice) {
        try IOracle(tombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            ...
            revert("Vexa: failed to fetch VEXA price from Oracle");
        }
    }
    ...
    /**
     * @notice Operator mints VEXA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of VEXA to mint to
