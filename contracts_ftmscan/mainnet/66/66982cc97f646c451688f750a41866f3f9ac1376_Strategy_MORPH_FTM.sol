...
contract Strategy_MORPH_FTM {
    // This strategy is for FTM network. MORPH - FTM pair. RewardToken = "MORPH"
    ...
    address public token0; // morph
    address public token1; // wftm
    address public vault;

    // ------------> Important constants <--------------
    uint256 public constant pid = 0;
    bool public constant hasPid = true;
    address public router = 0xF491e7B69E4244ad4002BC14e878a34207E38c29; // can be changed by owner
    address public constant lpToken = 0x7D42442f764985F208E6fa6A7CD0e253CB447D58;
    ...
    function _getLiquidityAmounts(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = IPair(lpToken).quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                ...
                uint amountAOptimal = IPair(lpToken).quote(amountBDesired, reserveB, reserveA);
    ...
    function _checkPrice(address[] memory path) internal {
        // check that price difference no more than 5 percent
        token1Price = _getTokenPrice(path);
        ...
        address[] memory pathTemp = new address[](2);
        pathTemp[0] = path[1];
        pathTemp[1] = path[0];
        token0Price = _getTokenPrice(pathTemp);
    }
    ...
    function __addLiquidityETH(address _token, uint256 desiredAmountETH) internal {
        ...
        _transfer(_token, lpToken, desiredAmountToken);
        IWETH(WETH).deposit{value: desiredAmountETH}();
        IWETH(WETH).transfer(lpToken, desiredAmountETH);
        IPair(lpToken).mint(address(this));
    }
    ...
    function _getTokenPrice(address[] memory path) internal view returns (uint256) {
        // how many tokens in path[1] consts for tokens in path[0]
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            // calculates how many wrapped tokens for morph tokens
            path[1] = token0; // morph
            tCI = _getTokenPrice(path) * t0V / d + t1V;
        }
        ...
        if (_balance > 0) {
            path[1] = token0;
            tCI += _getTokenPrice(path) * _balance / d;
        }
