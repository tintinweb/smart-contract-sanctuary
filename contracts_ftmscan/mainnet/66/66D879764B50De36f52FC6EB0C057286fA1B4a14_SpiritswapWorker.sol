...
contract SpiritswapWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, ISpiritWorker {
  /// @notice Libraries
  ...
  event SetSpiritFee(address indexed caller, uint256 indexed fee);

  /// @notice Configuration variables
  ISpiritMasterChef public override masterChef;
  ...
  address public override spirit;
  ...
  function initialize(
    ...
    require(
      ...
      "SpiritswapWorker::initialize:: reinvestBountyBps exceeded maxReinvestBountyBps"
    );
    require(
      (farmingToken == lpToken.token0() || farmingToken == lpToken.token1()) &&
        (baseToken == lpToken.token0() || baseToken == lpToken.token1()),
      "SpiritswapWorker::initialize:: LP underlying not match with farm & base token"
    );
  }

  /// @dev Require that the caller must be an EOA account to avoid flash loans.
  modifier onlyEOA() {
    require(msg.sender == tx.origin, "SpiritswapWorker::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(msg.sender == operator, "SpiritswapWorker::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[msg.sender], "SpiritswapWorker::onlyReinvestor:: not reinvestor");
    _;
  }
  ...
  function getAmountOutMin(uint256 _amountIn, address[] memory _path) public view returns (uint256) {
    ...
    uint256 amountOut;
    require(length >= 2, "SpiritswapWorker::getAmountOutMin:: bad length.");
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    ...
    // 7. Reset approve
    spirit.safeApprove(address(router), 0);
  ...
  function work(
    ...
    require(okStrats[strat], "SpiritswapWorker::work:: unapproved work strategy");
    require(
      lpToken.transfer(strat, lpToken.balanceOf(address(this))),
      "SpiritswapWorker::work:: unable to transfer lp to strat"
    );
  ...
  function getMktSellAmount(
    ...
    if (aIn == 0) return 0;
    require(rIn > 0 && rOut > 0, "SpiritswapWorker::getMktSellAmount:: bad reserve values");
  ...
  function setReinvestBountyBps(uint256 _reinvestBountyBps) external onlyOwner {
    require(
      ...
      "SpiritswapWorker::setReinvestBountyBps:: _reinvestBountyBps exceeded maxReinvestBountyBps"
    );
    reinvestBountyBps = _reinvestBountyBps;
  ...
  function setMaxReinvestBountyBps(uint256 _maxReinvestBountyBps) external onlyOwner {
    require(
      ...
      "SpiritswapWorker::setMaxReinvestBountyBps:: _maxReinvestBountyBps lower than reinvestBountyBps"
    );
    require(
      _maxReinvestBountyBps <= 500,
      "SpiritswapWorker::setMaxReinvestBountyBps:: _maxReinvestBountyBps exceeded 5%"
    );
  ...
  function setSlippage(uint256 _slippage) external onlyOwner {
    require(_slippage <= 2000, "SpiritswapWorker::setSlippage:: slippage exceeded 20%");
    slippage = _slippage;
  ...
  function setOracle(IOracle _oracle) external onlyOwner {
    require(address(_oracle) != address(0), "SpiritswapWorker::setOracle:: not ZERO address.");
    oracle = _oracle;
    emit SetOracle(msg.sender, oracle);
  }

  function setSpiritFee(uint256 _fee) external onlyOwner {
    require(_fee <= 10000, "SpiritswapWorker::setSpiritFee:: amount exceeded 100%");
    fee = _fee;
    emit SetSpiritFee(msg.sender, fee);
  }
}
