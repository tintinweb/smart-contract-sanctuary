# Smart-Contract-Sanctuary - STATS 📊

* [Ethereum](#ethereum)
  * Mainnet
  * Ropsten
  * Goerli
  * Kovan
  * Rinkeby
* [Binance Chain](#binance-chain)
  * Mainnet
  * Testnet
* [Polygon/Matic](#polygon-matic)
  * Mainnet
  * Mumbai

_______________

<sup>
last updated: Fri Jul 09 2021 21:25:51 GMT+0000 (Coordinated Universal Time)
</sup>

## Ethereum


### Mainnet

**SourceUnits:** `95359`   
**Unique Submissions (Name):** `44496`   

**First Submission:** `3/24/2016`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `v0.4.24` (89835)
   * `0.6.12` (46264)
   * `v0.4.19` (44720)
   * `v0.4.21` (44668)
   * `v0.4.18` (37833)
   * `v0.4.23` (20723)
   * `v0.4.11` (18453)
   * `v0.4.20` (18043)
   * `0.5.17` (16298)
   * `0.7.6` (14662)   

**Names:**
   * `lockEtherPay` (20978)
   * `ADZbuzzCommunityToken` (5948)
   * `ERC20Token` (5818)
   * `TokenERC20` (5386)
   * `Crowdsale` (4872)
   * `Token` (4869)
   * `SimpleERC20` (4468)
   * `ERC20` (2399)
   * `MintedTokenCappedCrowdsaleExt` (2334)
   * `MultiSigWallet` (2088)   

**Submission Dates:** 
   * `6/12/2018` (2860)
   * `5/7/2021` (2672)
   * `2/2/2021` (2613)
   * `7/6/2021` (2584)
   * `6/14/2018` (2554)
   * `6/21/2018` (2459)
   * `6/13/2018` (2411)
   * `6/19/2018` (2347)
   * `6/11/2018` (2343)
   * `6/20/2018` (2278)   
        

### Ropsten

**SourceUnits:** `62090`   
**Unique Submissions (Name):** `38509`   

**First Submission:** `11/21/2016`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `v0.4.24` (97004)
   * `v0.4.19` (39788)
   * `0.6.12` (35559)
   * `v0.4.21` (31648)
   * `v0.4.18` (29082)
   * `v0.4.23` (25906)
   * `0.5.0` (20229)
   * `v0.4.25` (18479)
   * `v0.4.20` (17729)
   * `0.8.4` (16010)   

**Names:**
   * `ERC20Token` (7449)
   * `Token` (5989)
   * `Address` (5263)
   * `Crowdsale` (4862)
   * `ERC20` (3645)
   * `HumanStandardToken` (3387)
   * `TokenERC20` (2963)
   * `MyToken` (1988)
   * `MyAdvancedToken` (1565)
   * `SafeMath` (1506)   

**Submission Dates:** 
   * `7/6/2021` (3224)
   * `4/30/2021` (2520)
   * `7/5/2021` (2344)
   * `7/2/2021` (2333)
   * `5/6/2021` (2282)
   * `6/17/2021` (2192)
   * `5/28/2021` (2151)
   * `7/7/2021` (2149)
   * `5/22/2021` (2091)
   * `6/18/2021` (2063)   
        

### Goerli

**SourceUnits:** `5119`   
**Unique Submissions (Name):** `1957`   

**First Submission:** `2/24/2021`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `0.6.12` (28125)
   * `0.7.6` (16335)
   * `0.8.4` (9389)
   * `0.8.0` (5503)
   * `0.6.6` (4530)
   * `0.7.1` (3852)
   * `0.5.16` (3661)
   * `0.7.4` (3439)
   * `0.6.11` (2943)
   * `0.7.3` (2443)   

**Names:**
   * `Treasury` (1162)
   * `Storage` (831)
   * `Calc` (739)
   * `Oracle` (714)
   * `Boardroom` (689)
   * `WolfyRiskPlatform` (639)
   * `VErc20Proxy` (627)
   * `GroupWalletFactory` (611)
   * `SimpleERCFund` (603)
   * `MockDai` (590)   

**Submission Dates:** 
   * `3/17/2021` (2585)
   * `5/24/2021` (2329)
   * `6/25/2021` (2096)
   * `5/27/2021` (2064)
   * `5/7/2021` (2000)
   * `5/26/2021` (1957)
   * `5/25/2021` (1879)
   * `3/18/2021` (1735)
   * `4/5/2021` (1678)
   * `5/28/2021` (1657)   
        

### Kovan

**SourceUnits:** `23170`   
**Unique Submissions (Name):** `5506`   

**First Submission:** `3/8/2021`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `0.6.12` (39257)
   * `0.7.6` (21330)
   * `0.8.0` (7976)
   * `0.8.4` (7114)
   * `0.5.16` (6894)
   * `0.5.17` (6223)
   * `0.8.3` (5391)
   * `0.6.6` (5070)
   * `0.8.1` (4357)
   * `0.7.3` (3928)   

**Names:**
   * `InitializableImmutableAdminUpgradeabilityProxy` (2664)
   * `Token` (1042)
   * `Cash` (958)
   * `DefaultReserveInterestRateStrategy` (944)
   * `DsecDistribution` (707)
   * `TreasuryPool` (706)
   * `OwnedUpgradeabilityProxy` (694)
   * `ERC20` (600)
   * `BFactory` (582)
   * `Oracle` (560)   

**Submission Dates:** 
   * `5/7/2021` (2963)
   * `4/29/2021` (2912)
   * `5/21/2021` (2609)
   * `5/14/2021` (2485)
   * `7/1/2021` (2418)
   * `6/18/2021` (2340)
   * `7/7/2021` (2254)
   * `5/13/2021` (2201)
   * `6/25/2021` (2179)
   * `6/11/2021` (1993)   
        

### Rinkeby

**SourceUnits:** `35144`   
**Unique Submissions (Name):** `8713`   

**First Submission:** `3/8/2021`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `0.6.12` (17870)
   * `0.8.0` (12454)
   * `0.7.6` (9343)
   * `0.8.4` (9080)
   * `0.8.3` (5822)
   * `0.6.6` (5146)
   * `0.7.4` (4213)
   * `0.5.17` (3819)
   * `0.7.0` (3587)
   * `0.6.11` (3375)   

**Names:**
   * `Token` (771)
   * `Address` (544)
   * `SimpleCollectible` (504)
   * `ERC20` (495)
   * `MasterChef` (486)
   * `NFTSimple` (443)
   * `TokenVesting` (434)
   * `TokenMock` (410)
   * `Vault` (397)
   * `NFT` (397)   

**Submission Dates:** 
   * `7/8/2021` (1771)
   * `4/30/2021` (1620)
   * `6/25/2021` (1592)
   * `7/9/2021` (1440)
   * `3/26/2021` (1437)
   * `7/7/2021` (1419)
   * `5/28/2021` (1407)
   * `5/7/2021` (1359)
   * `3/19/2021` (1338)
   * `4/2/2021` (1333)   
        

### ___totals___

**SourceUnits:** `220882`   
**Unique Contract Names (submissions):** `99181`   
        

--------

## Binance Chain


### Mainnet

**SourceUnits:** `3079`   
**Unique Submissions (Name):** `2171`   

**First Submission:** `7/6/2021`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `0.6.12` (1754)
   * `0.8.4` (490)
   * `0.5.16` (232)
   * `0.8.6` (186)
   * `0.5.17` (122)
   * `0.8.2` (100)
   * `0.7.6` (90)
   * `0.8.0` (89)
   * `0.4.26` (88)
   * `0.8.3` (69)   

**Names:**
   * `BEP20Token` (111)
   * `Token` (99)
   * `Address` (65)
   * `ERC20` (57)
   * `MasterChef` (28)
   * `Excavator` (24)
   * `CoinToken` (22)
   * `YouCoin` (21)
   * `miniBABYDOGE` (18)
   * `AuntyDoge` (13)   

**Submission Dates:** 
   * `7/6/2021` (1000)
   * `7/7/2021` (1000)
   * `7/8/2021` (1000)
   * `7/9/2021` (500)   
        

### Testnet

**SourceUnits:** `2545`   
**Unique Submissions (Name):** `1122`   

**First Submission:** `7/6/2021`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `0.6.12` (1068)
   * `0.8.4` (438)
   * `0.8.6` (324)
   * `0.8.0` (208)
   * `0.5.16` (147)
   * `0.7.6` (132)
   * `0.6.6` (55)
   * `0.5.17` (53)
   * `0.8.3` (43)
   * `0.6.8` (42)   

**Names:**
   * `Address` (72)
   * `MasterChef` (63)
   * `BEP20Token` (49)
   * `DogUFarm` (44)
   * `Swap` (38)
   * `Token` (36)
   * `IterableMapping` (32)
   * `Timelock` (26)
   * `LUCKINU` (23)
   * `PawnContract` (21)   

**Submission Dates:** 
   * `7/8/2021` (981)
   * `7/7/2021` (972)
   * `7/9/2021` (747)
   * `7/6/2021` (200)   
        

### ___totals___

**SourceUnits:** `5624`   
**Unique Contract Names (submissions):** `3293`   
        

-------

## Polygon Matic



### Mainnet

**SourceUnits:** `1300`   
**Unique Submissions (Name):** `771`   

**First Submission:** `7/5/2021`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `0.6.12` (2084)
   * `0.8.4` (458)
   * `0.5.16` (326)
   * `0.6.6` (287)
   * `0.8.0` (239)
   * `0.8.6` (91)
   * `0.5.17` (69)
   * `0.7.6` (64)
   * `0.8.1` (51)
   * `0.8.3` (47)   

**Names:**
   * `MasterChef` (202)
   * `Timelock` (90)
   * `token` (85)
   * `Presale` (74)
   * `Token` (51)
   * `IterableMapping` (42)
   * `TestWorker` (40)
   * `IDO` (39)
   * `Address` (38)
   * `GVault` (35)   

**Submission Dates:** 
   * `7/6/2021` (1418)
   * `7/7/2021` (985)
   * `7/8/2021` (727)
   * `7/5/2021` (463)
   * `7/9/2021` (407)   
        

### Mumbai

**SourceUnits:** `842`   
**Unique Submissions (Name):** `489`   

**First Submission:** `6/25/2021`    
**Most Recent Submission:** `7/9/2021`   

#### Top 10

**Compiler:** 
   * `0.8.4` (884)
   * `0.6.12` (874)
   * `0.5.16` (338)
   * `0.7.6` (200)
   * `0.8.0` (191)
   * `0.8.3` (184)
   * `0.4.24` (135)
   * `0.6.6` (111)
   * `0.5.12` (74)
   * `0.5.17` (66)   

**Names:**
   * `BellyFood` (125)
   * `MasterChef` (89)
   * `BDay` (60)
   * `GuildNFTStaking` (38)
   * `LiquidityLocker` (35)
   * `SimpleGame` (33)
   * `YieldManagerAave` (33)
   * `EternalStorageProxy` (32)
   * `Token` (30)
   * `Polypunks` (30)   

**Submission Dates:** 
   * `7/5/2021` (697)
   * `7/7/2021` (528)
   * `7/6/2021` (523)
   * `7/8/2021` (476)
   * `7/2/2021` (335)
   * `7/1/2021` (258)
   * `7/3/2021` (151)
   * `7/9/2021` (124)
   * `7/4/2021` (108)
   * `6/30/2021` (108)   
        

### ___totals___

**SourceUnits:** `2142`   
**Unique Contract Names (submissions):** `1260`   
        
