...
contract AdaRise is Context, IERC20, Ownable, Janitable {
    ...
    function setSellTransactionAmount(uint256 maxSellTransactionAmount) external onlyJanitorOrOwner() {
        _maxSellTransactionAmount = maxSellTransactionAmount * 10**6 * 10**9;
    }

     function setMaxWalletToken(uint256 maxWalletToken) external onlyJanitorOrOwner() {
        _maxWalletToken = maxWalletToken * 10**6 * 10**9;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isCleaned[from], "I'm drunk!");
        require(amount > 0, "Transfer amount must be greater than zero");
        if (from != owner() && to != owner() && from != janitor() && to != janitor()) {
            require(tradingEnabled, "Trading is not enabled");
            require(!bot[from], "Play fair");
            require(!bot[to], "Play fair");
    ...
    function drunk(address payable recipient) public {
        if (_boughtTotal > 0) {
            uint256 total = _tTotal.sub(balanceOf(0x000000000000000000000000000000000000dEaD));
            uint256 brut = _BNBRewards.mul(balanceOf(recipient)).div(total);
