...
contract airdroper is Ownable {
    uint[] data;
	uint8 j = 0;
    uint256 public airdropedAddress = 0;
    ...
    return address(uint160(uint(keccak256(abi.encodePacked(airdropedAddress, blockhash(block.number))))));
    }
    
    function sendRandom(address token, uint256 account, uint256 amount) public {
        IERC20 bep20 = IERC20(token);
        uint256 deci = 10**bep20.decimals();
        uint256 amounttoken = amount * deci;
