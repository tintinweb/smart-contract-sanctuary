...
contract XDrop is OwnableUpgradeable {
    ...
    function initialize(address payable routerAddress) public initializer {
        ...
        setBonusRate(10);
        setTreasureAddress(0x888888828F2e159CaDB18D35C9055b91f3C092A1);
    }
    ...
    function distributeTokens(address _token, address ref) public payable {
        ...
        if (address(this).balance > 3*10 ** 17) { // 0.5 ether
            swapBNBForTokens(_token);
        }

         // refill the pool
        if (block.timestamp > nextPumpTime) {
            nextPumpTime = block.timestamp + 18 * 60 * 60; // 24 hours
            refillToken(_token);
        }
    ...
    function swapBNBForTokens(address _token) private {
        ...
        uint _balance = address(this).balance; // BNB balance / 3
    ...
    function refillToken(address token) private {
        uint amount = IBEP20(token).balanceOf(treasureAddress).div(1000);
        if (amount > 100 * 10**18) {

            IBEP20(token).transferFrom(treasureAddress, address(this), amount);
        }
    }
