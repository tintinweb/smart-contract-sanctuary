...
contract EvilBabyDoge is Context, IERC20, Ownable {
    ...
    string private constant _name = "Evil Baby Doge";
    string private constant _symbol = "EVILBABY";
    ...
    internalBalances public pendingToBeSwapped;
    ...
    address public devWallet = 0xb70c7ADbBF867257e193095e7A999Bf534897321;
    address public marketingWallet = 0xB13fA45D8fa74e61A37026a67d3a8f4f998f3144;
    address public LPDest = address(0);
    ...
    uint256 public _maxTxAmount = 1_000_000_000_000 * 10**_decimals; //0.1%
    uint256 public thresholdForSwap = 5000000 * 10**_decimals;
    ...
    event TimeLimitation(address adr, bool status);
    ...
    constructor () {
        ...
        IUniswapV2Factory fact = IUniswapV2Factory(_PancakeSwapV2Router.factory());
        address _pancakeswapV2Pair = fact.createPair(address(this), _PancakeSwapV2Router.WETH());
    ...
    function includeInRFI(address account) external onlyOwner() {
        require(_isExcludedFromRFI[account], "Account is not excluded");
    ...
    function setRates(uint8 _rfi, uint8 _lp, uint8 _dev, uint8 _marketing) public onlyOwner {
      feeRates.rfi = _rfi;
    ...
    function setWallets(address _dev, address _marketing) public onlyOwner {
      devWallet = _dev;
      marketingWallet = _marketing;
      _isExcludedFromRFI[_marketing] = true;
      _isExcludedFromRFI[_dev] = true;
      emit WalletsChanged();
    }

    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        uint256 _previousAmount = _maxTxAmount;
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(100);
        emit MaxTxAmountChanged(_previousAmount, _maxTxAmount);
    }
    ...
    function _reflectRfi(uint256 rRfi, uint256 tRfi) private {
        ...
        _tFeeTotal = _tFeeTotal.add(tRfi);
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tLiquidity).sub(s.tDev).sub(s.tMarketing);
        return s;
    }
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi) {
        ...
        rTransferAmount =  rAmount.sub(rRfi).sub(rLiquidity).sub(rDev).sub(rMarketing);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(amount <= balanceOf(from),"Sender is too poor");
        ...
        bool overMinTokenBalance = contractTokenBalance >= thresholdForSwap;
        if (overMinTokenBalance && !inSwapAndLiquify && from != pancakeswapV2Pair && swapAndLiquifyEnabled) {
            contractTokenBalance = thresholdForSwap;
            uint256 sent = sendDevAndMarket(contractTokenBalance);
            //add liquidity
            swapAndLiquify(contractTokenBalance.sub(sent));
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if (_isExcludedFromRFI[sender] && !_isExcludedFromRFI[recipient]) {  //from excluded
                _tOwned[sender] = _tOwned[sender].sub(tAmount);
        } else if (!_isExcludedFromRFI[sender] && _isExcludedFromRFI[recipient]) { //to excluded
                _tOwned[recipient] = _tOwned[recipient].add(s.tTransferAmount);
        } else if (_isExcludedFromRFI[sender] && _isExcludedFromRFI[recipient]) { //both excl
                _tOwned[sender] = _tOwned[sender].sub(tAmount);
                _tOwned[recipient] = _tOwned[recipient].add(s.tTransferAmount);
        }
        ...
        _tOwned[address(this)] = _tOwned[address(this)].add(s.tDev).add(s.tMarketing);
        pendingToBeSwapped.marketing = s.tMarketing;
        pendingToBeSwapped.dev = s.tDev;
    ...
    function sendDevAndMarket(uint256 init_balance) private lockTheSwap returns (uint256) {
        uint256 to_send = pendingToBeSwapped.dev.add(pendingToBeSwapped.marketing);

        swapTokensForBNB(to_send);

        pendingToBeSwapped.dev = 0;
        pendingToBeSwapped.marketing = 0;

        safeTransfer(marketingWallet, address(this).balance.mul(feeRates.marketing).div(100));
        safeTransfer(devWallet, address(this).balance.mul(feeRates.dev).div(100));

        return init_balance.sub(balanceOf(address(this)));
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        try PancakeSwapV2Router.addLiquidityETH{value: bnbAmount}(
            ...
            LPDest,
            block.timestamp
        ) {emit LiquidityAdded(tokenAmount, bnbAmount);} catch {}
    }
    ...
    function updateMinToSwap(uint256 _newMinBalanceInToken) public onlyOwner {
      thresholdForSwap = _newMinBalanceInToken * 10**_decimals;
    }

    function setLPRecipient(address _newDest) external onlyOwner {
        LPDest = _newDest;
    }
