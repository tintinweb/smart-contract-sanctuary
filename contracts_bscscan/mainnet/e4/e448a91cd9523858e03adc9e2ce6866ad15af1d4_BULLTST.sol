...
contract BULLTST is Context, IERC20, Ownable {
    ...
    uint public Winners=5;
    uint256 public _BuyForJackpot = 10000000 * 10**_decimals;
    ...
    uint256 public maxSellAmount = _tTotal.mul(1).div(200);
    ...
    address public marketingAddress = 0xb1AFa8EC8C0f0851e4cC63dcAB2ACCc499Ba7D5F;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;
    address public operationsAddress = 0xF1C406A88F8CbC67967DbFF3a2D7d61832CB1AaE;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "BULL TST";
    string private constant _symbol = "BULLTST";
    ...
    function setFeeRates(uint256 _rfi, uint256 _operations, uint256 _marketing, uint256 _liquidity, uint256 _buyback, uint256 _burn) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _operations, uint256 _marketing, uint256 _liquidity, uint256 _buyback, uint256 _burn) external onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rOperations, to_return.rMarketing, to_return.rLiquidity, to_return.rBuyback,  to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tMarketing-s.tLiquidity-s.tBuyback-s.tBurn;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rOperations,uint256 rMarketing, uint256 rLiquidity, uint256 rBuyback, uint256 rBurn) {
        ...
        rTransferAmount =  rAmount-rRfi-rOperations-rMarketing-rLiquidity-rBuyback-rBurn;
        return (rAmount, rTransferAmount, rRfi,rOperations,rMarketing,rLiquidity,rBuyback,rBurn);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if( from != owner() &&
            ...
            if ((balanceOf(from) - (amount)) <= _BuyForJackpot && JackpotAdded [from] == true) {
				//Sold tokens, end balance below operations minimum
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.operations + feeRates.marketing) * 2;
