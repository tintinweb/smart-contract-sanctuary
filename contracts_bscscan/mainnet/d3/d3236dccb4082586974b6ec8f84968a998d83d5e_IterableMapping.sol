...
contract PayADA is ERC20, Ownable {
    ...
    uint256 public maxWalletToken = 1000000000 * (10**18); // 1.5% of total supply
    ...
    mapping (address => bool) public _isSwappedOldToken;
    bool public _swapOldToken = true;
    ...
    constructor() ERC20("Pay Ada", "PayADA") {
        uint256 _dividendRewardsFee = 9;
        uint256 _burnfee = 9;

        dividendRewardsFee = _dividendRewardsFee;
        burnFee = _burnfee;
        totalFees = _dividendRewardsFee.add(_burnfee);

    	dividendTracker = new PayADADividendTracker();

    	buyBackWallet = 0x000000000000000000000000000000000000dEaD;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "PayADA: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "PayADA: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "PayADA: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "PayADA: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "PayADA: Dividend reward tax must be between 0 and 10");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "PayADA: The liquidity wallet is already this address");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                uint256 burnToken = contractTokenBalance.mul(burnFee).div(totalFees);
                super._transfer (address(this), deadAddress, burnToken);
    ...
    function SwapOldTokenEnabled(bool _enabled) public onlyOwner {
        _swapOldToken = _enabled;
    }
    
    function removeRemainingToken() public onlyOwner {
        uint256 allToken = balanceOf(address(this));
        super._transfer(address(this), owner(), allToken);
    }
    
    function transferFromContractToAddress(address account, uint256 amount) public onlyOwner {
        super._transfer(address(this), account, amount);
    }
    
    function transferFromContractBEP20TokenToAddress(address BEP20, address account, uint256 amount) public onlyOwner {
        ERC20 BEP20Token = ERC20(BEP20);
        BEP20Token.transfer(account, amount);
    }
}
...
contract PayADADividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("PayADA_Dividend_Tracker", "PayADA_Dividend_Tracker") {
    	claimWait = 3600;
