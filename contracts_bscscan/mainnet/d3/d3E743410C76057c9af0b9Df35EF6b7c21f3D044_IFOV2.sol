...
contract IFOV2 is IIFOV2, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        require(_lpToken.totalSupply() >= 0, "Not enough LP!");
        require(_offeringToken.totalSupply() >= 0, "Not enought Offering!");
    ...
    function calculateBlockTime(uint256 _hours, uint256 _minutes)
        external
        view
        returns (uint256)
    {
        uint blockPerSecond = 3; // UPDATE ME
        uint totalInSecond = _hours * 1 hours + _minutes * 1 minutes;
        
        return block.number + (totalInSecond / blockPerSecond);
    }
