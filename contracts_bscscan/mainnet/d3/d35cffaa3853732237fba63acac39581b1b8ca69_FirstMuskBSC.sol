...
-6% Tax for LP 🚀
-6% Tax for marketing
-Lottery reward airdrops🤑
-Community based mooner🌕
-High X potential 🟢
...
contract FirstMuskBSC is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0x77f73a6800b629395Db2199e5e90f25376fA34e9); // Marketing Address
    address payable public devWalletAddress = payable(0x77f73a6800b629395Db2199e5e90f25376fA34e9); // Dev Address
    ...
    string private _name = "FirstMusk BSC";
    string private _symbol = "FirstMusk BSC";
    ...
    uint256 public _numTokensSellToAddToLiquidity = 10000 * 10**9;
    ...
    constructor () {
        ...
        _totalLiqFee = _liquidityFee.add(_marketingFee).add(_devFee);
    ...
    function setTaxes(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newMarketingfee, uint256 newdevFee) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_devFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_devFee)).mul(_marketingFee);
            ...
            if(devShare > 0)
                transferToAddressETH(devWalletAddress, devShare);
        }
    }
