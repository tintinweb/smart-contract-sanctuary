...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    */
    function ExtendPool(uint256 _amountExtend) external {
        // setup for calculations
        uint256 preExtendedAmount;
        uint256 tokenDifference;

        // check current reward balance
        preExtendedAmount = rewardToken.balanceOf(address(this));

        // send new reward tokens
        rewardToken.safeTransferFrom(address(msg.sender), address(this), _amountExtend);

        // calculate actual new extended rewards
        tokenDifference = (rewardToken.balanceOf(address(this)) - preExtendedAmount);

        // if pool has ended increase end block based on current block with a start block approx 24 hrs from now
        if (bonusEndBlock < block.number) {
            startBlock = (block.number + 28800);
            bonusEndBlock = startBlock + (tokenDifference / rewardPerBlock);
        }
        // if pool is still active increase end block by required blocks
        bonusEndBlock = bonusEndBlock + (tokenDifference / rewardPerBlock);
    }

    function RenewPool(uint256 _amountExtend, uint256 _totalBlocks) external {
        // make sure pool has ended
        require(bonusEndBlock < block.number, "Pool hasnt ended, use ExtendPool Call");
        ...
        // if pool has ended increase end block based on current block with a start block approx 24 hrs from now
        startBlock = (block.number + 28800);
        bonusEndBlock = startBlock + _totalBlocks;
        rewardPerBlock = tokenDifference / _totalBlocks;        
    }
