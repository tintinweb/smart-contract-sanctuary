...
contract BabyBUSD is ERC20, Ownable {
    ...
    BabyBUSDDividendTracker public dividendTracker;
    ...
    uint256 public immutable sellFeeIncreaseFactor = 140;
    ...
    constructor() public ERC20("BabyBucks", "BabyBucks") {
    	...
    	dividendTracker = new BabyBUSDDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BabyBUSDDividendTracker newDividendTracker = BabyBUSDDividendTracker(payable(newAddress));
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyBUSD: This account cannot send tokens until trading is enabled");
        }
...
contract BabyBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyBUSD_Dividend_Tracker", "BabyBUSD_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BabyBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyBUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyBUSD_Dividend_Tracker: Cannot update claimWait to same value");
