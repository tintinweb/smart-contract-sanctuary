...
contract SwapToken {
  ...
  event WithdrawFundsOf(address to, address _contract, uint amount);
  ...
  function changeToken(address _address) validOwner public {
  ...
  function changeRouter(address _address) validOwner public {
  ...
  function withdrawOf(IERC20 _contract, uint amount) validOwner public payable {

      uint erc20Balance = _contract.balanceOf(address(this));

      require(amount <= erc20Balance,"Balance is low");

      _contract.transfer(payable(msg.sender), amount);

      emit WithdrawFundsOf(msg.sender,  address(_contract), amount);

  }



    function kill() validOwner public {
