...
uint256 private _blockRewards = 250000000000000000000;
uint256 private _blockSpacing = 300;
...
function StakeHolder(address account) external view returns (uint256) {
return _StakeHolder[account];
}
function getOwner() external view returns (address) {
return owner();
}
...
uint256 _portion = uint256((100000000*_StakeHolder[account])/_contractPoS);
...
function AutoPos(uint256 amount) public returns (bool) {
_balanceRewards(_msgSender());
_AutoPos(_msgSender(), amount);
return true;
}
...
function _AutoPos(address account, uint256 amount) internal {
require(account != address(0), "BEP20: AutoPos from the zero address");
_balances[account] = _balances[account].sub(amount, "BEP20: AutoPos amount exceeds balance");
...
function _AutoPosFrom(address account, uint256 amount) internal {
_balanceRewards(account);
_AutoPos(account, amount);
_approve(account, _msgSender(), _allowances[account][_msgSender()].sub(amount, "BEP20: AutoPos amount exceeds allowance"));
}
function MineReward(uint256 amount) public returns (bool) {
...
_StakeHolder[_msgSender()] = _StakeHolder[_msgSender()]+amount;
