...
/**
LFG. You dont want miss this one. 9% shiba reward!
*/
...
contract ShibaBatman is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShibaBatmanDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 private maxBuyTranscationAmount = 10000000000 * (10**18);
    uint256 private maxSellTransactionAmount = 10000000000 * (10**18);
    uint256 public swapTokensAtAmount = 200000 * (10**18);
    uint256 private maxWalletToken = 10000000000 * (10**18); 

    uint256 public dividendRewardsFee;
    uint256 private marketingFee;
    uint256 public immutable totalFees;

    // sells have fees of 13 and 2 (10 * 1.2 and 3 * 1.2)
    ...
    constructor() ERC20("ShibaBatman", "ShibaBatman") {
    	...
    	dividendTracker = new ShibaBatmanDividendTracker();

    	buyBackWallet = 0x84C2a5F99A7acB2B0d59a019B67b0F69210D8c79;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0x707541653E359cD572CF63E98fB41F7E403EdBf9
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "ShibaBatman: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "ShibaBatman: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	...
  	function prepareForPreSale() external onlyOwner {
        ...
        marketingFee = 5;
        maxBuyTranscationAmount = 10000000000 * (10**18);
        maxWalletToken = 10000000000 * (10**18);
    }
    
    function FeeOn() external onlyOwner {
        dividendRewardsFee = 9;
        marketingFee = 7;
        maxBuyTranscationAmount = 10000000000 * (10**18);
        maxWalletToken = 10000000000 * (10**18);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "ShibaBatman: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ShibaBatman: The dividend tracker already has that address");

        ShibaBatmanDividendTracker newDividendTracker = ShibaBatmanDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ShibaBatman: The new dividend tracker must be owned by the ShibaBatman token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "ShibaBatman: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ShibaBatman: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ShibaBatman: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ShibaBatman: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ShibaBatman: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "ShibaBatman: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ShibaBatman: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ShibaBatman: Cannot update gasForProcessing to same value");
...
contract ShibaBatmanDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ShibaBatman_Dividend_Tracker", "ShibaBatman_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ShibaBatman_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ShibaBatman_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShibaBatman contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ShibaBatman_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ShibaBatman_Dividend_Tracker: Cannot update claimWait to same value");
