...
contract BUTTHURT is IBEP20, Auth {
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        if(sender != pair){ require (amount <= _maxsellTxAmount || isTxLE[sender], "TX Limit Exceeded"); }
        if(sender == pair){ require (amount <= _maxbuyTxAmount || isTxLE[sender], "TX Limit Exceeded"); }
    }
    ...
    function checksellTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxsellTxAmount || isTxLE[sender], "TX Limit Exceeded");
    }
    ...
    function variableST(uint256 amount) internal view returns (uint256) {
    ...
    function vSBF(uint256 amount) internal swapping {
        uint256 dynamicLiq = isOverLiquified(targetL, targetLD) ? 0 : yr;
        uint256 amountL = variableST(amount).mul(dynamicLiq).div(dr).div(2);
        uint256 totalSw = variableST(amount).sub(amountL);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
        uint256 bB = address(this).balance;
