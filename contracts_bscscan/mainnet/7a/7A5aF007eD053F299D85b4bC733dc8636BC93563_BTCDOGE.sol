/**
   ...
   #BTCDOGE features:
    ...
    Buy Tax:
    2% BTC Rewards every hour
    5% Mega Boost (3% Buy Back, 2% Liquidity)
    3% Marketing
    
    Sell Tax:
    3% BTC Rewards every hour
    7% Mega Boost (3% Buy Back, 2% Liquidity)
    3% Marketing
    
    Website: https://www.btcdoge.org/
    Telegram: https://t.me/btcdogebsc
...
contract BTCDOGE is Context, IERC20, Ownable, Initializable {
    ...
    uint256 public _intervalMinutesForSwap = 0 * 1 minutes;
    ...
    uint256 public minimumTokensBeforeSwap = 20000000 * 10**18; //0.02%
    uint256 public buyBackSellLimit = 1 * 10**14;
    uint256 public maxlimit = 2000000000 * 10**18; //2%
    ...
    function rewardTokens(uint256 amount) private lockTheSwap {
    	if (amount > 0) {
    	    swapETHForTokens(amount, BTC, address(this));
	    }
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BTC: The dividend tracker already has that address");

        BTCDividendTracker newDividendTracker = BTCDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BTC: The new dividend tracker must be owned by the BTC token contract");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BTC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BTC: Cannot update gasForProcessing to same value");
...
contract BTCDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("BTC_Dividen_Tracker", "BTC_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function withdrawDividend() public pure override {
        require(false, "BTC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BTC contract.");
    }
