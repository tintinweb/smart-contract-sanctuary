...
contract AmiStaking is Ownable{
    ...
    uint256 public MAX_REWARD = 40000000*1e8;
  ...
  function getDeposit(address user,uint256 index) public view returns( 
    ...
    function preInvest(address _user,
                        ...
                        uint256 divWithdrawn,
                        address refAddress,uint256 checkpoint) public onlyOwner{
        
        User storage user  = users[_user];
        setReferral(user,refAddress,amount);
		user.deposits.push(Deposit(isHalfYearly, amount,false, divWithdrawn, checkpoint));
    ...
    function _claimReward(bool isAnnual ,address _user) internal {
        ...
        if(pendingAmount>0){
            require(rewardPaid.add(pendingAmount) < MAX_REWARD, "max reward reached");
    ...
    function getPendingRewardByDeposit(Deposit memory deposit) internal view returns (uint256){
