...
contract SixtyNine is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "69 Token";
    ...
    uint256 public contestFee          = 3;
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + contestFee;
    ...
    address public contestFeeReceiver;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != contestFeeReceiver  && recipient != autoLiquidityReceiver){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDev = amountBNB.mul(contestFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(contestFeeReceiver).call{value: amountBNBDev, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _contestFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        contestFee = _contestFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_contestFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/1, "Fees cannot be more than 33%");
    }

    
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _contestFeeReceiver ) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        contestFeeReceiver = _contestFeeReceiver;
    }
