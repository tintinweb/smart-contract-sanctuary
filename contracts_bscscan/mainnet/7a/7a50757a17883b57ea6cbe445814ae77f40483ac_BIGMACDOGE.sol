...
contract BIGMACDOGE is IBEP20, Auth {
   using SafeMath for uint256;
   ...
   string constant _name = "BigMacDoge";
   string constant _symbol = "BIGMACDOGE";
   uint8 constant _decimals = 9;
 
   uint256 _totalSupply = 2000000000 * (10 ** _decimals);
   uint256 public _maxBuyTxAmount = ( _totalSupply * 3 ) / 100; // 3%
    uint256 public _maxSellTxAmount = ( _totalSupply * 1 ) / 100; // 1%
   
   //max wallet holding of 6% 
    uint256 public _maxWalletToken = ( _totalSupply * 6 ) / 100;
   ...
   // Buy Fees
    ...
    uint256 buybackFeeBuy = 300;
    uint256 reflectionFeeBuy = 0;
    uint256 marketingFeeBuy = 800;
    uint256 totalFeeBuy = 1500;

    // Sell fees
    uint256 liquidityFeeSell = 800;
    uint256 buybackFeeSell = 600;
    uint256 reflectionFeeSell = 0;
    uint256 marketingFeeSell = 1600;
    uint256 totalFeeSell = 3000;
...
function shouldAutoBuyback() internal view returns (bool) {
   ...
   // Set max buy TX 
    function setBuyTxLimitInPercent(uint256 maxBuyTxPercent) external authorized {
        _maxBuyTxAmount = _totalSupply.mul(maxBuyTxPercent).div(100);
    }

    // Set max sell TX 
    function setSellTxLimitInPercent(uint256 maxSellTxPercent) external authorized {
        _maxSellTxAmount = _totalSupply.mul(maxSellTxPercent).div(100);
    }
