...
// File: contracts/HOHOHODividendPayingToken.sol
pragma solidity ^0.7.6;
...
contract HOHOHO is ERC20, Ownable {
    ...
    HOHOHODividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTransactionAmount = 100000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 2000 * (10**18);
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "HOHOHO: The dividend tracker already has that address");

        HOHOHODividendTracker newDividendTracker = HOHOHODividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "HOHOHO: The new dividend tracker must be owned by the HOHOHO token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "HOHOHO: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "HOHOHO: Account is already the value of 'excluded'");
   ...
   function blacklistAddress(address account, bool excluded) public onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "HOHOHO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "HOHOHO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HOHOHO: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "HOHOHO: This account cannot send tokens until trading is enabled");
        }
...
contract HOHOHODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("HOHOHO_Dividend_Tracker", "HOHOHO_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "HOHOHO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "HOHOHO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HOHOHO contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HOHOHO_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "HOHOHO_Dividend_Tracker: Cannot update claimWait to same value");
