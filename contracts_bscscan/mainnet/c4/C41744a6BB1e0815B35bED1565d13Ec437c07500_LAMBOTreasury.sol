...
contract LAMBOTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SLAMBO }

    address public immutable LAMBO;
    ...
    address public sLAMBO;
    uint public sLAMBOQueue; // Delays change to sLAMBO address
    ...
    /**
        @notice allow approved address to deposit an asset for LAMBO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint LAMBO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( LAMBO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn LAMBO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ILAMBOERC20( LAMBO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sLAMBO ).balanceOf( msg.sender ); // Can only borrow against sLAMBO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with LAMBO
        @param _amount uint
     */
    function repayDebtWithLAMBO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ILAMBOERC20( LAMBO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, LAMBO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( LAMBO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( LAMBO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns LAMBO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match LAMBO decimals
            value_ = _amount.mul( 10 ** IERC20( LAMBO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SLAMBO ) { // 9
            sLAMBOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
