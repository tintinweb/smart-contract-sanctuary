...
contract SniperRouter {
    ...
    function safebuy(address token, uint256 _amountIn, uint256 _amountsell) external {
      ...
      _path[0] = token;
      _path[1] = WBNB;
        uint256 _amountIn2 = IERC20(token).balanceOf(address(this));
      IERC20(token).approve(address(swap), _amountIn2);
      swap.swapExactTokensForTokens(_amountsell, minOut, _path, owner, block.timestamp + 1);
    ...
    function sell(address token, uint256 _minOut) external {
      ...
      IERC20(token).approve(address(swap), _amountIn);
