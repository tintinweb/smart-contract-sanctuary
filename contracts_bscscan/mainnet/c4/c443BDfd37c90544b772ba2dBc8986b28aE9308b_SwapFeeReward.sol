...
contract SwapFeeReward is OwnableUpgradeable {
    ...
    function getSwapRebateAmount(address input, address output, uint256 amount) public view onlyRouter returns (uint) {
    ...
    function getSwapFee(address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = uint(10000).sub(IFinsPair(pairFor(tokenA, tokenB)).swapFee());
    }
