...
//      '.\ /.'
//        '.'
contract DiamondPaws {
    KittyCoin _tokenContract;
    ...
    uint[] private _timeSpans = [31, 183, 365, 1095, 1825]; //In days
    ...
    constructor(address tokenContract) {
        _tokenContract = KittyCoin(tokenContract);
        _admin = msg.sender;
    ...
    */
    ...
    function createStake(StakeType stakeType, uint amount) public returns (bool) {
        require(amount >= 1000 * 10**8, "Minimum stake of 1000 Kittycoin.");
        _tokenContract.transferFrom(msg.sender, address(this), amount);
        Stake memory stake = Stake(stakeType, false, block.timestamp, amount);
    ...
    function claimStake(uint stakeID) public returns (uint) {
        ...
        uint endTime = stake.beginTime + numOfStakeDays * (1 days);
