...
contract QuamPool {
	...
	modifier onlyMargin() {
		require(IQuamFactory(factory).allowedMargins(msg.sender), "ONLY_MARGIN_CONTRACT");
		_;
	}
	...
	function withdraw(uint256 _amount) external returns (bool) {
		...
		if (outstandingLoans > 0) {
			...
			require(
				(currentUtilization <=
					IQuamFactory(factory).utilizationScaled(address(token))),
				"NO_LIQUIDITY"
			);
		}
...
// File contracts/SwapPathCreator.sol
