...
address constant ADDR_WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
...
address constant ADDR_WBNB_SS = 0x270449618BCb161798cBF071CdFbF00B902b05CD;
...
IWBNB constant WBNB = IWBNB(ADDR_WBNB);
...
contract Donations {
	...
	function depositBNB(uint amountBUSD, uint minLP) public payable {
		(bool success, ) = ADDR_WBNB.call{value:msg.value}("");
		require(success, "conversion of BNB to WBNB failed");
		WBNB.transfer(ADDR_WBNB_BUSD, msg.value);
		BUSD.transferFrom(msg.sender, ADDR_WBNB_BUSD, amountBUSD);
		uint liquidity = PAIR_WBNB_BUSD.mint(address(this));
		require(liquidity >= minLP, "not enough LP tokens - reserves may have changed since last quote");
		PAIR_WBNB_BUSD.approve(CHEF_ADDR, liquidity);
		CHEF.deposit(WBNB_BUSD_PID, liquidity);
		User storage user = users[msg.sender];
		user.wbnb.deposited += msg.value;
		user.busd.deposited += amountBUSD;
		user.lp += liquidity;
		totalLPStaked += liquidity;
		_processCake();
		emit Deposit(msg.sender, msg.value, amountBUSD, liquidity);
	}
	...
	function withdrawBNB(uint amount) public {
		...
		CHEF.withdraw(WBNB_BUSD_PID, amount);
		PAIR_WBNB_BUSD.transfer(ADDR_WBNB_BUSD, amount);
		(uint amountWBNB, uint amountBUSD) = PAIR_WBNB_BUSD.burn(address(this));
		user.wbnb.withdrawn += amountWBNB;
		user.busd.withdrawn += amountBUSD;
		user.lp -= amount;
		totalLPStaked -= amount;
		BUSD.transfer(msg.sender, amountBUSD);
		WBNB.withdraw(amountWBNB);
		payable(msg.sender).transfer(amountWBNB);
		emit Withdraw(msg.sender, amountWBNB, amountBUSD, amount);
		_processCake();
	}
