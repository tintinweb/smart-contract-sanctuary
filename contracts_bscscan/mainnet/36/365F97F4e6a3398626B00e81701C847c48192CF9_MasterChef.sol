...
contract MasterChef is SafeOwnable, ReentrancyGuard {
    ...
    function updateGIBXPerBlock(uint256 _rewardPerBlock) external onlyOwner {
    ...
    function pendingGIBX(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accGIBXPerShare = accGIBXPerShare.add(rewardReward.mul(1e12).div(lpSupply));
        }
