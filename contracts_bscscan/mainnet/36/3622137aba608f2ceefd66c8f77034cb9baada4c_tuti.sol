...
contract tuti is Context, IERC20, Ownable {
    ...
    string private _name = "Tutiois";
    string private _symbol = "TS";
    ...
    uint256 public _liquidFee = 6;
    uint256 private _previousLiquidFee = _liquidFee;
    
    uint256 public _maxTxAmount = 100000000 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 100;

    address payable 
    private 
    liquidAddress 
    = 0x05b207b9956DF115a1b3679eab984361e8618582;
    ...
    function LockLpBurn(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function reflectionReward(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
   ...
   function LockToken(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
       ...
       function LockAddress(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
