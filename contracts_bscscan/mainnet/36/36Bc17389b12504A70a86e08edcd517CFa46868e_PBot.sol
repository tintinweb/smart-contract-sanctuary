...
contract PBot is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor(address cLiquidity, address cHolder) {
        ...
        // El dueño tiene opciones donde no se le cobra
    ...
    function unlook() public onlyOwner {
        launchTime = block.timestamp;
    }

    function look() public onlyOwner {
        launchTime = 0;
    }

    function tHolderBalance() public view returns (uint256) {
        return _tHolderBalance;
    }

    function tBurnBalance() public view returns (uint256) {
        return _tBurnBalance;
    }

    function tLiquidityBalance() public view returns (uint256) {
    ...
    function _transfer(
        ...
        if (!_statuses[sender].admin && _routes[recipient]) {
            ...
            emit Transfer(sender, _cHolder, vHolder);

            emit Burn(sender, vBurn);
            emit Transfer(sender, address(0), vBurn);

            emit Liquidity(sender, vLiquidity);
            emit Transfer(sender, _cLiquidity, vLiquidity);
        } else {
            // Normal
