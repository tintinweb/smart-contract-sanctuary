...
contract BABYBLADES is Ownable, ERC721Enumerable {
    ...
    uint256 public SPECIAL_MINTS;
    address public MARKETING_WALLET;

    string private _baseTokenURI = "";
    mapping(uint256 => string) private _specialMintUri;

    constructor(uint256 _mints, address _marketingWallet) ERC721("BABYBLADES", "NFBBL") {
        MAX_MINTS = _mints;
    ...
    modifier isMintable(uint256 count) {
        require(ALLOW_MINTING, "Error: Minting is disabled");
        require(
            totalSupply().add(count) <= MAX_MINTS + SPECIAL_MINTS,
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (tokenId > MAX_MINTS) {
            return _specialMintUri[tokenId];
        }
    ...
    /**
     * These are special ones reserved for giveaways only
     */
    function reserveMints(string[] calldata tokenUris) external onlyOwner {
        require(SPECIAL_MINTS.add(tokenUris.length) <= 100, "Error: Maximum number of special mints have been minted");
        for (uint256 index = 0; index < tokenUris.length; index++) {
            uint256 id = MAX_MINTS + ++SPECIAL_MINTS;
            _safeMint(msg.sender, id);
            _specialMintUri[id] = tokenUris[index];
        }
    }
    ...
    function airdropRandom(address to, uint256 count) external onlyOwner isMintable(count) {
        for (uint256 index = 0; index < count; index++) {
            _safeMint(to, totalSupply().add(1).sub(SPECIAL_MINTS));
        }
    }
    ...
    function mint(uint256 count) public payable isMintable(count) {
        require(msg.value >= MINT_PRICE.mul(count), "Error: Ether value sent is lower than expected");
        for (uint256 index = 0; index < count; index++) {
            _safeMint(msg.sender, totalSupply().add(1).sub(SPECIAL_MINTS));
        }
