...
The liquidity is initially locked for 6 months in a timelocked smart contract, renewable.
*/
...
contract Master is Ownable {
    ...
    IEarthie public earthie;

    // Market oracle provides the EARTHIE/USD exchange rate as an 18 decimal fixed point number.
    ...
    constructor(address _earthie) public {
    ...
    function rebase() external {
        ...
        // Ignore positive rebases
        if (supplyDelta > 0) {
            supplyDelta = 0;
        }
        
        uint256 supplyAfterRebase = earthie.rebase(epoch, supplyDelta);
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && earthie.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(earthie.totalSupply())).toInt256Safe();
        }
        ...
        returns (int256)
    {
        ...
        return earthie.totalSupply().toInt256Safe()
