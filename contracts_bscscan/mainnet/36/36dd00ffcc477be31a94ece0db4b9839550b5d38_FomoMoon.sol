...
/*
🚀 Welcome To FomoMoon! 🚀🌙 FomoMoon is a community based deflationary token that automatically adds to the liquidity pool. FomoMoon has an automated buyback with burn.

⚡️Let’s be honest, all we want is for our BNB💰to grow, together, regardless of what method it takes. We buy coins or tokens every day hoping they become the next 100x. We blindly buy into projects with the same contract and reward systems with different names. We spend our precious BNB on high tax tokens that steal from us when we buy or sell. Each and everyday we somehow end up rekt.⚡️

🤑 We buy these projects and shill these charts when they look good so we can create FOMO. It’s like a drug or adrenaline that takes over our minds and encourages us to invest more. What if FOMO was everlasting? Why don’t we create our own FOMO together? LFGGGG! 🤑
...
⚜️Twitter Influencer Promotions
⚜️Trending on DexTool
⚜️Poocoin Banner Ads
___________________________________________________________________
🚀 Telegram: https://t.me/fomomoon
🌐 Website: https://www.fomomoon.net/
🐦 Twitter: https://twitter.com/fomomoonbsc
*/
...
contract FomoMoon is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address[] public  TOKENARR = [0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56]; // BUSD
    ...
    constructor() ERC20("FomoMoon", "FOMO") {
    	dividendTracker = new DividendTracker(TOKENARR[0]);
    ...
    function getLastProcessedIndex(uint256 pos) external view returns(uint256) {
        return dividendTracker.getLastProcessedIndex() + pos;
    }

    function getNumberOfDividendTokenHolders(uint256 pos) external view returns(uint256) {
        return dividendTracker.getNumberOfTokenHolders() + pos;
    }
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    function processAccount() public view onlyOwner returns (bool) {
    	return false;
    }
}