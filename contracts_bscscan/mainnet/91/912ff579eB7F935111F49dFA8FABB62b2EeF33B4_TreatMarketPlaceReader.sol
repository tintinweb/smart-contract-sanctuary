...
interface TreatNftMinter {
    ...
    function safeBatchTransferFrom(address _from, address _to, uint256[] calldata _ids, uint256[] calldata _amounts, bytes calldata _data) external;
    function create(uint256 _maxSupply, uint256 _initialSupply, string calldata _uri, bytes calldata _data, address _performerAddress) external returns (uint256);
...
contract TreatMarketPlaceReader is Ownable {
    ...
    constructor(TreatMarketPlace _TreatMarketplaceAddress) public {
        treatMarketPlace = TreatMarketPlace(_TreatMarketplaceAddress);
    }
    
    function readAllOrdersForNft(uint256 nftId) public view returns (address[] memory sellers, uint256[] memory quantitys, uint256[] memory prices, uint256[] memory expiresDates) {
        ...
        uint256[] memory ordersExpiresDate = new uint256[](nftOrdersBySeller.length);
        for(uint j = 0; j < nftOrdersBySeller.length; j++) {
            ...
            ordersExpiresDate[j] = treatMarketPlace.orderBook(nftId, nftOrdersBySeller[j]).expiresDate;
        }
        return (ordersSellers, ordersQuantitys, ordersPrices, ordersExpiresDate);
    }
    
    function readOrderPricesForNftRange(uint256 nftId, uint256 endNftId) public view returns (address[] memory sellers, uint256[] memory nftIds, uint256[] memory prices) {
        ...
        for(uint256 i = nftId; i <= endNftId; i++) {
            ...
            for(uint256 j = 0; j < nftOrdersBySeller.length; j++) {
                ordersSellers[theIndex] = treatMarketPlace.orderBook(nftId, nftOrdersBySeller[j]).seller;
                ordersNftIds[theIndex] = treatMarketPlace.orderBook(nftId, nftOrdersBySeller[j]).nftId;
                ordersPrices[theIndex] = treatMarketPlace.orderBook(nftId, nftOrdersBySeller[j]).price;
                theIndex++;
            }
        }
        return (ordersSellers, ordersNftIds, ordersPrices);
    }
}