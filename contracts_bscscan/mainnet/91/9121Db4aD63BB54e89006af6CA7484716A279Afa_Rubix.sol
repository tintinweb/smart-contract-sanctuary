...
// File:  (3)/remixbackup/contracts/RBX(DEPLOYED).sol
...
contract Rubix is Context, IBEP20, Ownable {
  ...
  function swapThirdLane(address _user) public {
    ...
    function SwapableBalance(address _user) public view returns(uint256 balance) {
        uint256 eighteenToNineDecimals = _oldRBX.balanceOf(_user).div(1e18).mul(1e9);
  ...
  function Deposit(uint256 _amount) external {
        ...
        if(_PoolData[currentPool].endTime < now) {
            ...
            _balances[address(1)] = _balances[address(1)].add(3500000 * 10**9);
        ...
        _balances[address(1)] = _balances[address(1)].add(_amount.add(_amount.mul(9500).div(100000)));
        ...
        //Adds Pool Rewards
        _PoolData[currentPool].PoolRewards = _PoolData[currentPool].PoolRewards.add(_amount.mul(9500).div(100000));
...
function getStakingBalance(address _stakeholder) public view returns(uint256) {
 ...
 function _poolShare(address _stakeholder) public view returns(uint256) {
  ...
  function estimatedApr(uint256 _amount) public view returns(uint) {
      ...
      uint _aprFromDeposit = _amount.mul(9500).div(100000);
      ...
      return _MulRewards.div(_amount).mul(7);
  }
