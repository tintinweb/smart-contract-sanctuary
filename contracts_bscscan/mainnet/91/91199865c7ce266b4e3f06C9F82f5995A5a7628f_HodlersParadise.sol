...
contract HodlersParadise is  IERC20, Ownable {
    ...
    string private _name = "Hodler's Paradise";
    string private _symbol = "HODLERS";
    ...
    address MarketingWallet=msg.sender;
    
    event ExcludeFromFees(address indexed account, bool isExcluded);
    function ClaimMarketings() external onlyOwner {
        uint8 totalTax=_DevTax+_marketingTax;
        uint256 ContractBalance=address(this).balance;
        bool sent;
        (sent,)=DevWallet.call{value:ContractBalance*_DevTax/totalTax}("");
        require(sent);
        ContractBalance=address(this).balance;
        (sent,)=MarketingWallet.call{value:ContractBalance}("");
        require(sent);
    }

    function setFeeReceivers (address _Dev, address _Marketing) external onlyOwner {
    DevWallet = _Dev;
    ...
    constructor ()  {
        ...
        excludeFromMaxTransaction(address(_pancakePairAddress), true);

        maxTransactionAmount = _tTotal * 5 / 1000;
        maxWallet = _tTotal * 5 / 1000;
    ...
    function calculateContractFee(uint256 _amount, uint8 taxes) private view returns (uint256) {
        return (_amount*(_liquidityTax+_marketingTax+_DevTax)*taxes)/10000;
    }
...
////////////////////////////////////////////////////////////////////////////////
    ...
    uint8 _liquidityTax=17;
    uint8 _marketingTax=33;
    uint8 _DevTax=33;
    uint8 _reflectionTax=17;
    uint8 _buyTax=0;
    uint8 _sellTax=30;
    uint8 _botTax=99;
    ...
    uint256 BotProtectionTime=15;
    
    
    function SetTaxes(uint8 LP_, uint8 marketing_, uint8 Dev_, uint8 reflection_, uint8 buy_, uint8 sell_, uint8 bot_, uint8 transfer_) public onlyOwner{
        require((LP_+marketing_+Dev_+reflection_)==100);
        _liquidityTax=LP_;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(takeFee){
            ...
            if(sender==_pancakePairAddress && !_isExcludedMaxTransactionAmount[recipient]){//BUY
                ...
                if(block.timestamp<LaunchTimestamp+BotProtectionTime){
                taxes=_botTax;
                }
            }

            else if(recipient==_pancakePairAddress && !_isExcludedMaxTransactionAmount[sender]) {//SELL
            //Can only sell if Bot protection timestamp is set, or Bot Protection time is over
                require(block.timestamp>LaunchTimestamp+BotProtectionTime);
                require(amount <= maxTransactionAmount, "Sell transfer amount exceeds the maxTransactionAmount.");
                taxes=_sellTax;
            }
            
            else if(!_isExcludedMaxTransactionAmount[sender] && !_isExcludedMaxTransactionAmount[recipient]){//TRANSFER
            //Can only sell if Bot protection timestamp is set, or Bot Protection time is over
                require(block.timestamp>LaunchTimestamp+BotProtectionTime);
                require(amount + balanceOf(recipient) <= maxWallet, "Max wallet exceeded");
                taxes=_transferTax;
            }
            else {//TRANSFER from/to excluded from max transaction amount
                taxes=_transferTax;
            }
