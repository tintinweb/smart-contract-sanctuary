...
contract VDTEST is Context, IBEP20, Ownable {
    ...
    address payable public charityAddress = 0x45f05b7D8c8b409815b75C8d94723FE8eE216b89; // Charity Address
    ...
    string private _name = 'VDTest';
    string private _symbol = 'VDTest';
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity,uint256 rCharity, uint256 tCharity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        // (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rCharity) = _getRValues(tAmount, tFee, tLiquidity, tCharity);
        return (feeT.tTransferAmount, feeT.tFee, feeT.tLiquidity, feeT.tCharity, feeR.rAmount, feeR.rTransferAmount, feeR.rFee, feeR.rCharity);
        
    }

    function _TransferAmount(uint256 amount, uint256 fee,uint256 liquify,uint256 charity) private view returns(uint256){
            return amount.sub(fee).sub(liquify).sub(charity);
    }
    ...
    function _getTValues(uint256 tAmount) private  returns (uint256, uint256, uint256, uint256) {
        ...
        setfeeT(tTransferAmount, tFee, tLiquidity, tCharity);
        return (tTransferAmount, tFee, tLiquidity, tCharity);
    }
    
    function setfeeT(uint256 tTransferAmount, uint256 tFee,  uint256 tLiquidity, uint256 tCharity) private {
        feeT = feeStructT(tTransferAmount, tFee, tLiquidity , tCharity);
    }
    
    function setfeeR(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rCharity) private {
        feeR = feeStructR(rAmount, rTransferAmount, rFee, rCharity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity) private  returns (uint256, uint256, uint256, uint256) {
        uint256 currentRate = _getRate();
        ...
        setfeeR(rAmount, rTransferAmount, rFee, rCharity);
