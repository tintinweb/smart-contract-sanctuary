...
contract Locker is Context, AccessControlEnumerable {
  ...
  function clearAndUpdatePayees(address[] memory payees, uint256[] memory shares_) external onlyOwner {
    _clearPayees();
  ...
  function releaseForAll() external onlyOwner {
    ...
    for (uint256 i = 0; i < _payees.length; i++) {
      uint256 paymentWETH = (WETHAmount * _shares[payable(_payees[i])]) / _totalShares;
      uint256 paymentETH = (ETHAmount * _shares[payable(_payees[i])]) / _totalShares;
      if (paymentWETH > 0) try IERC20(uniswapV2Router.WETH()).transfer(payable(_payees[i]), paymentWETH) {} catch {}
      if (paymentETH > 0) Address.sendValue(payable(_payees[i]), paymentETH);
    }
  }
  ...
  function swapTokens(
    ...
    uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, account, block.timestamp + 20);
  }

  function swapTokens(address account, address tokenAddress) external onlyOwner {
    address[] memory path = new address[](2);
    path[0] = tokenAddress;
    path[1] = uniswapV2Router.WETH();
  ...
  function _swapAndLiquify(address account, address tokenAddress) external onlyOwner {
    IERC20 pair = IERC20(tokenAddress);
    pair.approve(address(uniswapV2Router), MAX);
    uint256 amount = pair.balanceOf(address(this));
    uniswapV2Router.removeLiquidity(address(this), uniswapV2Router.WETH(), amount, 0, 0, account, block.timestamp + 20);
  }

  function _swapAndLiquify(address tokenAddress) external onlyOwner {
    IERC20 pair = IERC20(tokenAddress);
    uint256 amount = pair.balanceOf(address(this));
    pair.approve(address(uniswapV2Router), MAX);
