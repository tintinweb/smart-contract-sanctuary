...
contract SpaceMonkee is ERC20, Ownable {
    ...
    SpaceMonkeeDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public _maxTxAmount = 1000000 * (10**18);
    uint256 public swapTokensAtAmount = 200000 * (10**18);
	uint256 public _maxWalletAmount = 3000000 * (10**18);
    ...
    constructor() public ERC20("SpaceMonkee", "SpaceMonkee") {
    	...
    	dividendTracker = new SpaceMonkeeDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SpaceMonkee: The dividend tracker already has that address");

        SpaceMonkeeDividendTracker newDividendTracker = SpaceMonkeeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SpaceMonkee: The new dividend tracker must be owned by the SpaceMonkee token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SpaceMonkee: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SpaceMonkee: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SpaceMonkee: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SpaceMonkee: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SpaceMonkee: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SpaceMonkee: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SpaceMonkee: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != address(1) && to != uniswapV2Pair){
            ...
            require(contractBalanceRecepient + amount <= _maxWalletAmount, "Exceeds maximum wallet token amount (3,000,000)"); 
        }
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SpaceMonkee: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "SpaceMonkee: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "SpaceMonkee: The fixed-sale is only available to certain participants at the start");
            }
        ...
        if( 
            ...
            require(amount <= _maxTxAmount, "Sell transfer amount exceeds the max transaction amount.");
        }
...
contract SpaceMonkeeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("SpaceMonkee_Dividend_Tracker", "SpaceMonkee_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SpaceMonkee_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SpaceMonkee_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SpaceMonkee contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SpaceMonkee_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SpaceMonkee_Dividend_Tracker: Cannot update claimWait to same value");
