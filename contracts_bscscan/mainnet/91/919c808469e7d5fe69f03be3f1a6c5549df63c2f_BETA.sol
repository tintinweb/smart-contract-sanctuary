...
// BETA with Governance.
contract BETA is BEP20 {
    ...
    // Min amount to liquify. (default 500 BETA)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to BETA's router when our own AMM release
    IUniswapV2Router02 public BETASwapRouter;
    // The trading pair
    address public BETASwapPair;
    //busdAddress
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "BETA::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    /**
     * @notice Constructs the BETA contract.
     */
    constructor() public BEP20("BETA FINANCE", "BETA") {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (BETAMaster).
    function mint(address _to, uint256 _amount) public onlyMinter {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true &&
            _inSwapAndLiquify == false &&
            address(BETASwapRouter) != address(0) &&
            BETASwapPair != address(0)
        ) {
            swapAndLiquify();
        }
        _mint(_to, _amount);
    ...
    function _transfer(
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(
                ...
                "BETA::transfer: Burn value invalid"
            );
            ...
            require(
                ...
                "BETA::transfer: Tax value invalid"
            );
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        if(currentPrice.mul(10000) > expectedPrice.mul(alowanceDiff.add(10000))) {
           uint256 amountToSwap =  ((currentPrice.mul(1e8).div(expectedPrice))).sqrt().sub(1e4).mul(balanceOf(BETASwapPair)).div(1e4); // 160 000 000  => 12 649 .sub  2649 * price
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the BETASWAP pair path of token -> WBNB
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = busdAddress;

        _approve(address(this), address(BETASwapRouter), tokenAmount);
        // make the swap
        BETASwapRouter.swapExactTokensForTokens(
            tokenAmount,
    ...
    function getWexPrice() public view returns (uint256) {
        // wexLPAddress
        return IBEP20(busdAddress).balanceOf(BETASwapPair).mul(1e2).div(balanceOf(BETASwapPair));
    }
    // /// @dev Add liquidity
    ...
    //     // add the liquidity
    //     BETASwapRouter.addLiquidityETH{value: ethAmount}(
    //         address(this),
    //         tokenAmount,
    ...
    // To receive BNB from BETASwapRouter when swapping
        ...
        onlyOperator
    {
        require(
            ...
            "BETA::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(
            _burnRate <= 100,
            "BETA::updateBurnRate: Burn rate must not exceed the maximum rate."
        );
        ...
        onlyOperator
    {
        require(
            ...
            "BETA::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
     ...
     * Can only be called by the current operator.
     */
    function updateBETASwapRouter(address _router) public onlyOperator {
        BETASwapRouter = IUniswapV2Router02(_router);
        BETASwapPair = IUniswapV2Factory(BETASwapRouter.factory()).getPair(
            address(this),
            busdAddress
        );
        require(
            BETASwapPair != address(0),
            "BETA::updateBETASwapRouter: Invalid pair address."
        );
        emit BETASwapRouterUpdated(
            msg.sender,
            address(BETASwapRouter),
            BETASwapPair
        );
    }
    ...
    function recoverLostTokensExceptOurTokens(address _token, uint256 amount)
        ...
        onlyOperator
    {
        require(_token != address(this), "Cannot recover BETA tokens");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(
            ...
            "BETA::transferOperator: new operator is the zero address"
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "BETA::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "BETA::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "BETA::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BETA (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "BETA::_writeCheckpoint: block number exceeds 32 bits"
        );
