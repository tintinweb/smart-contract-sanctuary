...
// File: contracts/NSBCLottery.sol
 ...
 */
contract NSBCLottery is ReentrancyGuard, INSBCLottery, Ownable {
    ...
    uint256 public maxPriceTicketInNsbc = 50 ether;
    uint256 public minPriceTicketInNsbc = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInNsbc;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] nsbcPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInNsbc;
        uint32 finalNumber;
    }
     ...
     * @param _nsbcTokenAddress: address of the NSBC token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _nsbcTokenAddress, address _randomGeneratorAddress) {
        nsbcToken = IERC20(_nsbcTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of NSBC to this contract
        uint256 amountNsbcToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInNsbc,
            _ticketNumbers.length
        );

        // Transfer nsbc tokens to this contract
        nsbcToken.safeTransferFrom(address(msg.sender), address(this), amountNsbcToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInNsbc += amountNsbcToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInNsbcToTransfer
        uint256 rewardInNsbcToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInNsbcToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        nsbcToken.safeTransfer(msg.sender, rewardInNsbcToTransfer);

        emit TicketsClaim(msg.sender, rewardInNsbcToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInNsbc) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in NSBC for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].nsbcPerBracket[j] =
                ...
                // A. No nsbc to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].nsbcPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInNsbc - amountToShareToWinners);

        // Transfer Nsbc to treasury address
        nsbcToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        nsbcToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInNsbc += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInNsbc >= minPriceTicketInNsbc) && (_priceTicketInNsbc <= maxPriceTicketInNsbc),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInNsbc: _priceTicketInNsbc,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            nsbcPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInNsbc: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(nsbcToken), "Cannot be NSBC token");
    ...
    function setMinAndMaxTicketPriceInNsbc(uint256 _minPriceTicketInNsbc, uint256 _maxPriceTicketInNsbc)
        external
        onlyOwner
    {
        require(_minPriceTicketInNsbc <= _maxPriceTicketInNsbc, "minPrice must be < maxPrice");

        minPriceTicketInNsbc = _minPriceTicketInNsbc;
        maxPriceTicketInNsbc = _maxPriceTicketInNsbc;
    }
     ...
     * @param _priceTicket price of a ticket (in NSBC)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].nsbcPerBracket[_bracket];
        } else {
            return 0;
        }
    }
