...
contract BananaLandTokenWhitelisted is Context, IBEP20, Ownable, LGEWhitelisted {
    ...
    constructor() {
        ...
        _LatestLockTime = block.timestamp;
        _lockTime = 5 * 60; // 5 mins initially
    ...
    function removeWhiteList(address user) public onlyOwner{
        ...
        if(i >= _whiteList.length) return;
        for(uint256 j = i; j < _whiteList.length - 1; j ++){
            _whiteList[j] = _whiteList[j + 1];
        }
        _whiteList.pop();
    }

    function addMultiWhiteList(address[] memory users) public onlyOwner{
        for(uint i = 0; i < users.length; i ++){
            addWhiteList(users[i]);
        }
    }

    function removeMultiWhiteList(address[] memory users) public onlyOwner{
        for(uint i = 0; i < users.length; i ++){
    ...
    */
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        require(block.timestamp > _LatestLockTime + _lockTime || _isWhiteList(msg.sender), "not availble in lock time");
