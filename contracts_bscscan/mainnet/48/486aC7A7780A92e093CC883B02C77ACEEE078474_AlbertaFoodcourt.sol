...
contract AlbertaFoodcourt is IAlberta, Ownable, ReentrancyGuard {
    ...
    function add(
        ...
        if (_paidToken == address(0)) {
            ...
            (path[0], path[1]) = (WBNB, _wantToken);

            uint256 bnbBal = address(this).balance;
            _safePriceImpactSwap(bnbBal, path, _maxPriceImpact);
            ROUTER.swapExactETHForTokens{value: bnbBal}(0, path, _to, block.timestamp);
        } else {
            IERC20(_paidToken).safeTransferFrom(msg.sender, address(this), _amountIn);

            address[] memory path = new address[](3);
            (path[0], path[1], path[2]) = (_paidToken, WBNB, _wantToken);
            ...
            _safePriceImpactSwap(paidBal, path, _maxPriceImpact);
            ROUTER.swapExactTokensForTokens(paidBal, 0, path, _to, block.timestamp);
        }
    }
    ...
    function remove(
        ...
        IERC20(wantToken).safeTransferFrom(msg.sender, address(this), _amountIn);

        if (paidToken == address(0)) {
            address[] memory path = new address[](2);
            (path[0], path[1]) = (wantToken, WBNB);

            IERC20(wantToken).safeApprove(address(ROUTER), 0);
            IERC20(wantToken).safeApprove(address(ROUTER), type(uint256).max);

            uint256 wantBal = IERC20(wantToken).balanceOf(address(this));
            _safePriceImpactSwap(wantBal, path, _maxPriceImpact);
            ROUTER.swapExactTokensForETH(wantBal, 0, path, _to, block.timestamp);
        } else {
            address[] memory path = new address[](3);
            (path[0], path[1], path[2]) = (wantToken, WBNB, paidToken);
            ...
            uint256 wantBal = IERC20(wantToken).balanceOf(address(this));
            _safePriceImpactSwap(wantBal, path, _maxPriceImpact);
            ROUTER.swapExactTokensForTokens(wantBal, 0, path, _to, block.timestamp);
        }
    }
