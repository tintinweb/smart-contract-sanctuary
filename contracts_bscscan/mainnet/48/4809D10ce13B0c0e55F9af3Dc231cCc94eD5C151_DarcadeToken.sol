...
contract DarcadeToken is Ownable, ERC20 {

    //_isBlacklisted = Can not buy or sell or transfer tokens at all <-----for bots!
    mapping (address => bool) public _isBlacklisted;

    //_isAuthorizedWallet = Can hold more then 2% of supply ( possibly needed for internal wallets )
    mapping (address => bool) public _isAuthorizedWallet;

    //max wallet holding 2% of total supply
    uint256 public _maxWalletToken = 2000000 * 10**18;
    ...
    constructor(uint256 initialSupplyInTokens) ERC20("DARCADE", "DARCADE") {
        _mint(msg.sender, initialSupplyInTokens * 10**18);
    }

    //adds to authorized wallets list
    function addToAuthorizedWalletList(address[] calldata addresses) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _isAuthorizedWallet[addresses[i]] = true;
        }
    }

    //set the Max transaction amount (in tokens)
    function setMaxTxTokens(uint256 maxTxTokens) external onlyOwner() {
        _maxTxAmount = maxTxTokens * 10**18;
    }
    ...
    function setMaxWalletTokens(uint256 maxWallTokens) external onlyOwner() {
        _maxWalletToken = maxWallTokens * 10**18;
    }
    ...
    function transfer(address to, uint256 amount) override public returns (bool) {
        ...
        if (to != owner() && to != address(this) && _isAuthorizedWallet[to] != true){
        ...
        require(!_isBlacklisted[_msgSender()] && !_isBlacklisted[to], "This address is blacklisted");

        //limit the maximum number of tokens that can be bought or sold in one transaction
        if(_msgSender() != owner() && to != owner() && _isAuthorizedWallet[_msgSender()] != true)
