...
contract lizMinerV2 is ReentrancyGuard {
    ...
    constructor(address oldminer) {
        _owner = msg.sender;
        _oldminer= IlizMiner(oldminer);
    ...
    function addTradingPool(
        ...
        LpWallet wallet =new LpWallet(tokenAddress, _lizaddr, _feeowner, _owner);
    ...
    {
        return  _oldminer.getMyLpInfo(user, tokenaddress);
    }
    ...
    function getUserDhash(address user,address child) external view returns (uint256)
    {
     ...
     function MappingUserFromOld(address user) public onlyOwner {
         ...
         address parentk=_oldminer.getParent(user);
         _parents[user] = parentk;
        _mychilders[parentk].push(user);
        _userInfo[user][USERLEVELA] = _oldminer.getUserLevel(user);
        _userInfo[user][PENDINGCOIN] = _oldminer.getPendingCoin(user);

        uint256 jthash =  _oldminer.getUserSelfHash(user);
        if(jthash > 0)
        {
            _userInfo[user][SELFHASHA]=jthash;
            address parent = user;
            uint256 total = jthash;
    
            for (uint256 i = 0; i < 20; i++) {
                parent = _oldminer.getParent(parent);
                if (parent == address(0)) {break;}

                    uint256 parentself= _oldminer.getUserSelfHash(parent).mul(3);
                    if(parentself==0)
                    {
                        continue;
                    }
                    uint256 totalhash=jthash;
                    ...
                    uint256 levelconfig = _levelconfig[_oldminer.getUserLevel(parent)][i];
                    if (levelconfig > 0) 
                    {
    ...
    function TakeBackV1(address user,address tokenAddress,uint256 pct) public onlydataManager
    {
        uint256[3] memory  p=_oldminer.getMyLpInfo(user, tokenAddress);
        uint256 totalhash = p[2];
