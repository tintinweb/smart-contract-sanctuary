...
contract Vesting is Ownable {
    ...
    mapping(address => mapping(string => uint256)) private _allocations;
    ...
    function claim() external isStarted {
        uint256 unclaimed_ = unclaimed(msg.sender);
    ...
    function unclaimed(address user) public view isStarted returns (uint256) {
        uint256 total = 0;
        uint256 length = _setups.length;
        for (uint256 id = 1; id < length; id++) total += unclaimedInRound(user, id);
    ...
    function unclaimedInRound(address user, uint256 roundId)
        public
        view
        isStarted
        returns (uint256)
    {
        ...
        uint256 allocation = _allocations[user][_idToRound[roundId]];
    ...
    function setupWallets(
        ...
        require(
            ...
            'Different amount of elements in users and allocations array'
        );
        uint256 length = allocations.length;
        for (uint256 i = 0; i < length; i++) {
            _allocations[users[i]][round] = allocations[i];
        }
    }
    ...
    function userInfo(address user) external view returns (Info memory) {
        Info memory info;
        info.user = user;

        uint256 length = _setups.length;
        for (uint256 id = 1; id < length; id++) {
            if (_setups[id].amountOnUnlock.denominator == 0) continue;
            info.allocation += _allocations[user][_idToRound[id]];
        }

        info.unclaimed = unclaimed(user);
