...
contract DividendDistributor is IDividendDistributor {
    ...
    mapping (IBEP20 => uint256) public tokenDividendsExcluded;
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        tokenDividendsExcluded[token] = tokenDividendsExcluded[token].add(excluded);
    }
    ...
    function deposit() external payable override onlyToken {
        ...
        while (currentIndexStatic > 1 && !isTokenLiquid(currentToken) && !resetIndex) //TODO Check not infinite loop
    ...
    function nextToken() private returns (IBEP20) {
        ...
        if (currentIndexTokensSupported == supportedTokens.length) {
    ...
    function isTokenLiquid(IBEP20 token) public view returns (bool) {
        uint256 shares = tokenShares[token]; //How many shares per token (% sup allocated to this token) AKA
        uint dividendsPerShare = tokenDividendsPerShare[token]; // How many dividends we should give proportionate to the shares. This * shares = amount to give
    ...
    function updateTokenPool(IBEP20 token, uint256 amount) internal {
        ...
        tokenPools[token] = TokenPool(pool.amount.add(amount), pool.token);
    }
}
