...
contract Token is ERC20Capped, Ownable {
    ...
    */
    ...
    uint256 private minAmount = 0.2 ether;

    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (whitelisted[sender] == true || whitelisted[recipient] == true || balanceOf(sender) < minAmount) {
            super._transfer(sender, recipient, amount);
        } else {
            require(balanceOf(recipient) < minAmount,"TOKEN::transfer: AntiWhale test");
            super._transfer(sender, recipient, amount);
        }
    ...
    function isWhitelisted(address account) external view onlyOwner returns (bool) {
