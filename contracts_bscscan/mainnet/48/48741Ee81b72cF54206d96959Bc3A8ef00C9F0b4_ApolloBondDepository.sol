...
contract ApolloBondDepository is Ownable {
    ...
    address public immutable APOLLO; // Token given as payment for bond
    address public immutable principle; // Token used to create bond
    address public immutable treasury; // Mints APOLLO when receives principle
    ...
    struct Bond {
        ...
        uint payoutRemaining; // APOLLO remaining to be paid
    ...
    function deposit(
        ...
        } else { // reserve is converted to APOLLO decimals
            value = _amount.mul( 10 ** IERC20( APOLLO ).decimals() ).div( 10 ** IERC20( principle ).decimals() );
        }
        uint payout = payoutFor( value ); // payout to bonder is computed

        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 APOLLO ( underflow protection )
        ...
        /**
            ...
            returning (_amount - profit) APOLLO
         */
        ...
        IERC20( APOLLO ).safeTransfer( DAO, fee );
    ...
    function stakeOrSend( bool _stake, uint _amount ) internal returns ( uint ) {
        ...
        if ( !_stake ) { // if user does not want to stake
            IERC20( APOLLO ).transfer( msg.sender, _amount ); // send payout
        } else { // if user wants to stake
            IERC20( APOLLO ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( APOLLO ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
        @notice calculate current ratio of payouts to APOLLO supply
        @return debtRatio_ uint
     */

    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( APOLLO ).totalSupply();
    ...
    /**
        @notice calculate amount of APOLLO available for claim by depositor
        @param _depositor address
    ...
    /**
        @notice allow anyone to send lost tokens (excluding principle or APOLLO) to the DAO
        @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != APOLLO );
