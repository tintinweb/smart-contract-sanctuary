...
contract Barracks is Ownable {
    ...
    struct Barrack {
        ...
        uint minimum;                           // maximum amount to deposit.
        ...
        return BarrackInfo(
            {
                ...
                minimum : barracks[_barrackId].minimum,
    ...
    modifier checkBarrack(uint _barrackID) {
    ...
    function depositBNB(uint _barrackID) public checkBarrack(_barrackID) payable {
        // checks for min deposit value
        require(msg.value > 0 && msg.value >= barracks[_barrackID].minimum, "Not enough balance.");
        uint fee = (msg.value / 100) * barracks[_barrackID].feePercentage;
        treasury.transfer(fee);
        uint depositedAmount = msg.value - fee;
        // if new deposit then adding the address to addresses list
    ...
    // function to deposit tokens in barrack
    function depositToken(address _token, uint _barrackID, uint _amount) public checkBarrack(_barrackID) payable {
        require(tokenInfo[_token].isEnabled, "Token is disabled.");
        ...
        require(_amount > 0 && _amount >= barracks[_barrackID].minimum, "Amount less than minimum.");
    ...
    function refundEarlier(uint _barrackID, uint _amount) public {
        ...
        require((barracks[_barrackID].deposits[msg.sender].amount - _amount) > barracks[_barrackID].minimum, "Must be more than minimum deposited.");
        if (barracks[_barrackID].bnb) {
