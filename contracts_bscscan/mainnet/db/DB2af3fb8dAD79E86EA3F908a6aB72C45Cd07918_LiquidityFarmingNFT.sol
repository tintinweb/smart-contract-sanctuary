...
contract LiquidityFarmingNFT is IERC900, IERC2917, AccessControl, ReentrancyGuard, Pausable {
    ...
    constructor(SporeToken _SPORE, IERC20 _HOST, IERC1155 _NFT, uint256 _key, uint256 _reward) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    ...
    function stake(uint256 amount, bytes memory data) public override nonReentrant whenNotPaused update {
        require(NFT.balanceOf(_msgSender(), _poolKEY) > 0, "You need the NFT key to access this pool");
        ...
        require(HOST.transferFrom(_msgSender(), address(this), amount), "Tokens cannot be staked");
    ...
    function unstake(uint256 amount, bytes memory data) public override nonReentrant whenNotPaused update {
        ...
        require(HOST.transfer(_msgSender(), amount), "Tokens cannot be unstaked");
