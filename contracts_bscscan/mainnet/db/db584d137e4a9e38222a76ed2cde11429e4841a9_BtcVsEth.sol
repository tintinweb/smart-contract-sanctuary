...
// File: contracts/BtcVsEth.sol

pragma solidity 0.6.12;

contract BtcVsEth is Ownable, Pausable {
    ...
    struct Round {
        ...
        uint256 lockBlock;
        uint256 endBlock;
        int256 lockFirstPrice;
        int256 closeFirstPrice;
        int256 lockSecondPrice;
        int256 closeSecondPrice;
        uint256 totalAmount;
        uint256 bullAmount;
        uint256 bearAmount;
    ...
    AggregatorV3Interface internal oracleFirst;
    uint256 public oracleFirstLatestRoundId;
    
    AggregatorV3Interface internal oracleSecond;
    uint256 public oracleSecondLatestRoundId;
    ...
    event LockRound(uint256 indexed epoch, uint256 blockNumber, int256 firstPrice, int256 secondPrice);
    event EndRound(uint256 indexed epoch, uint256 blockNumber, int256 firstPrice, int256 secondPrice);
    ...
    constructor(
        AggregatorV3Interface _oracleFirst,
        AggregatorV3Interface _oracleSecond,
        ...
        oracleFirst = _oracleFirst;
        oracleSecond = _oracleSecond;
    ...
    /**
     * @dev set First Oracle address
     * callable by admin
     */
    function setFirstOracle(address _oracle) external onlyAdmin {
        require(_oracle != address(0), "Cannot be zero address");
        oracleFirst = AggregatorV3Interface(_oracle);
    }
    
    /**
     * @dev set Second Oracle address
     * callable by admin
     */
    function setSecondOracle(address _oracle) external onlyAdmin {
        require(_oracle != address(0), "Cannot be zero address");
        oracleSecond = AggregatorV3Interface(_oracle);
    }
    ...
    function genesisLockRound() external onlyOperator whenNotPaused {
        ...
        int256[2] memory currentPrices = _getPriceFromOracle();
        _safeLockRound(currentEpoch, currentPrices[0], currentPrices[1]);
    ...
    function executeRound() external onlyOperator whenNotPaused {
        ...
        _safeLockRound(currentEpoch, currentPrices[0], currentPrices[1]);
        _safeEndRound(currentEpoch - 1, currentPrices[0], currentPrices[1]);
     ...
     */
    function claimable(uint256 epoch, address user) public returns (bool) {
        BetInfo memory betInfo = ledger[epoch][user];
        Round memory round = rounds[epoch];
        int256 firstPercent = round.closeFirstPrice.sub(round.lockFirstPrice).div(round.lockFirstPrice).mul(100);
        int256 secondPercent = round.closeSecondPrice.sub(round.lockSecondPrice).div(round.lockSecondPrice).mul(100);
        emit TestPercent(
           firstPercent,
           secondPercent,
           round.oracleCalled,
           epoch,
           user,
           firstPercent > secondPercent && betInfo.position == Position.Bull,
           firstPercent < secondPercent && betInfo.position == Position.Bear
        );
        if ((round.lockFirstPrice == round.closeFirstPrice) && (round.lockSecondPrice == round.closeSecondPrice)) {
            return false;
        }
     ...
     * @dev Lock round
     */
    function _safeLockRound(uint256 epoch, int256 firstPrice, int256 secondPrice) internal {
        ...
        _lockRound(epoch, firstPrice, secondPrice);
    }

    function _lockRound(uint256 epoch, int256 firstPrice, int256 secondPrice) internal {
        Round storage round = rounds[epoch];
        round.lockFirstPrice = firstPrice;
        round.lockSecondPrice = secondPrice;
        emit LockRound(epoch, block.number, round.lockFirstPrice, round.lockSecondPrice);
    }

    /**
     * @dev End round
     */
    function _safeEndRound(uint256 epoch, int256 firstPrice, int256 secondPrice) internal {
        ...
        _endRound(epoch, firstPrice, secondPrice);
    }

    function _endRound(uint256 epoch, int256 firstPrice, int256 secondPrice) internal {
        Round storage round = rounds[epoch];
        round.closeFirstPrice = firstPrice;
        round.closeSecondPrice = secondPrice;
        round.oracleCalled = true;

        emit EndRound(epoch, block.number, round.closeFirstPrice,  round.closeSecondPrice);
    }
     ...
     */
    function _getPriceFromOracle() internal returns (int256[2] memory) {
        uint256 leastAllowedTimestamp = block.timestamp.add(oracleUpdateAllowance);
        
        (uint80 roundFirstId, int256 firstPrice, , uint256 firstTimestamp, ) = oracleFirst.latestRoundData();
        (uint80 roundSecondId, int256 secondPrice, , uint256 secondTimestamp, ) = oracleSecond.latestRoundData();
        require(firstTimestamp <= leastAllowedTimestamp, "First Oracle update exceeded max timestamp allowance");
        require(secondTimestamp <= leastAllowedTimestamp, "Second Oracle update exceeded max timestamp allowance");
        require(roundFirstId > oracleFirstLatestRoundId, "First Oracle update roundId must be larger than oracleLatestRoundId");
        require(roundSecondId > oracleSecondLatestRoundId, "Second Oracle update roundId must be larger than oracleLatestRoundId");
        oracleFirstLatestRoundId = uint256(roundFirstId);
        oracleSecondLatestRoundId = uint256(roundSecondId);
