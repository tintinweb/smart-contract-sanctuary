...
contract BisharesUniswapRouterMinter is BMath {
  ...
  constructor(address factory_, address weth_, address multisig_) {
    address zero = address(0);
    require(factory_ != zero, "BiShares: Factory is zero address");
    require(weth_ != zero, "BiShares: WETH is zero address");
    require(multisig_ != zero, "BiShares: Multisig is zero address");
    factory = factory_;
    weth = weth_;
    multisig = multisig_;
  }
  ...
  function swapTokensForAllTokensAndMintExact(
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      amountInMax = SafeMath.sub(amountInMax, amounts[0], "BiShares: Excessive input amount");
      if (swap) {
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      if (swap) {
        require(
          ...
          "BiShares: Wrapped transfer fail"
        );
        _swap(amounts, path);
      }
      uint256 amountToPool = amounts[amounts.length - 1];
