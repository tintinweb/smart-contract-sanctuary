...
/**
 * @dev CornDeFi functions that do not require less than the min timelock
 */
interface ICornDeFi {
    function add(
        uint256 _allocPoint,
 ...
 * to position this {CornTimelock} as the owner of a smart contract, with
 * a multisig or a DAO as the sole proposer.
 *
 * _Available since v3.3._
 */
contract CornTimelock is AccessControl, ReentrancyGuard {
    ...
    modifier onlyRole(bytes32 role) {
        require(
            ...
            "CornTimelock: sender requires permission"
        );
        _;
    }
    ...
    function _schedule(bytes32 id, uint256 delay) private {
        require(
            _timestamps[id] == 0,
            "CornTimelock: operation already scheduled"
        );
        require(delay >= minDelay, "CornTimelock: insufficient delay");
        // solhint-disable-next-line not-rely-on-time
    ...
    function cancel(bytes32 id) public virtual onlyRole(PROPOSER_ROLE) {
        require(
            ...
            "CornTimelock: operation cannot be cancelled"
        );
    ...
    function _beforeCall(bytes32 predecessor) private view {
        require(
            ...
            "CornTimelock: missing dependency"
        );
    }
    ...
    function _afterCall(bytes32 id) private {
        require(
            isOperationReady(id),
            "CornTimelock: operation is not ready"
        );
        _timestamps[id] = _DONE_TIMESTAMP;
    }
    ...
    function _call(
        ...
        require(success, "CornTimelock: underlying transaction reverted");
    ...
    function updateMinDelay(uint256 newDelay) external virtual {
        require(
            ...
            "CornTimelock: caller must be timelock"
        );
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        ICornDeFi(_cornDefiAddress).set(_pid, _allocPoint, _depositFeeBP, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        ICornDeFi(_cornDefiAddress).add(0, _lpToken, _depositFeeBP, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
