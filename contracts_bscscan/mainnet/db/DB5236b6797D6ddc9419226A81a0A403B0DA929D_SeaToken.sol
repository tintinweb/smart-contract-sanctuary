...
contract SeaToken is BEP20('Sea Token', 'SEA'), MinterRole, Pausable {
    ...
    uint16 public burnRate = 20;            // Burn rate % of transfer tax (default 50% * 4% = 2% of total amount)
    ...
    modifier onlyOperator() {
        require(_operator == msg.sender, "SEA: caller is not the operator");
        _;
    }
    ...
    function _transfer(address _sender, address _recepient, uint256 _amount) internal override antiWhale(_sender, _recepient, _amount) {
        ...
        require (_blocklistFromTransfer[_sender] == false && _blocklistFromTransfer[msg.sender] == false, 'SEA::transfer blocked');
        ...
        if (_recepient == BURN_ADDRESS || transferTaxRate == 0 || _excludedFromTransactionFee[msg.sender] == true) {
            ...
            require(taxAmount == burnAmount + marketingAmount, "SEA::transfer: Burn value invalid");
            uint256 sendAmount = _amount.sub(taxAmount);
            require(_amount == sendAmount + taxAmount, "SEA::transfer: Tax value invalid");
    ...
    function updateExcludedAddressFromAntiWhale(address _user, bool _isExcluded) external onlyOperator {
        _excludedFromAntiWhale[_user] = _isExcluded;
    }

    // update exclued address from tax fee. can be call by on the current operator
    function updateExcludedAddressFromTransactionFee(address _user, bool _isExcluded) external onlyOperator {
        _excludedFromTransactionFee[_user] = _isExcluded;
    }

    function updateMarketingWalletAddress(address _marketingWalletAddress) external onlyOperator {
      marketingWalletAddress = _marketingWalletAddress;
    }

    function updateBlocklistFromTransfer(address _user, bool _isBlocked) external onlyOperator {
      _blocklistFromTransfer[_user] = _isBlocked;
    }

    // pause tax fee strategy
    function pauseTaxStrategy() external onlyOperator {
        transferTaxRate = 0;
    }

    // unpause tax fee strategy
    function unpauseTaxStrategy() external onlyOperator {
        transferTaxRate = 400;
    }
