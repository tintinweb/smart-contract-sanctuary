...
// File: src/contracts/Dragoland_NFT.sol


pragma solidity ^0.8.0;
contract Dragoland_NFT is ERC721, ERC721Enumerable, ERC721URIStorage {
  using SafeMath for uint256;

  // Initiatialize
  constructor() ERC721("Dragoland_NFT", "Drago_NFT") {
    nextGrow = 0;
    mintPrice = 75000; //Drago
    growPrice = 1; //Dragoland_DragonBreath
    evolvePrice = 1; //Dragoland_LifeElixir
    deployerWallet = msg.sender;
  }

  // Enums
  enum Level { MysteryEgg, FireEgg, WaterEgg, EarthEgg, FireDrago, WaterDrago, EarthDrago, EnergyEgg, DarknessEgg, LightEgg, EnergyDrago, DarknessDrago, LightDrago }
  // Structs
  struct DragoNFT {
    uint256 idx;
    uint256 id;
  ...
  function setDragoAddr(address a) public {
    require(msg.sender == deployerWallet);
    DragoAddr = a;
  }
  function setDragonBreathAddr(address a) public {
    require(msg.sender == deployerWallet);
    DragonBreathAddr = a;
  }
  function setLifeElixirAddr(address a) public {
    require(msg.sender == deployerWallet);
    LifeElixirAddr = a;
  }
  ...
  function setrewardsEcosystemContract(address a) public {
    require(msg.sender == deployerWallet);
    rewardsEcosystemContract = a;
  }
  ...
  function getLevel(uint256 seed) private pure returns(Level) {
    uint256 spinResult = SafeMath.mod(seed, 100);
  ...
  function manualMint(
    ...
    DragoNFT memory _toCreate = DragoNFT(idx, id, level, _age, _edition, block.timestamp, false, 0, _owner);
  ...
  function mintWithFee(string memory _name, string memory _tokenURI) public {
    ...
    uint256 allowance = Drago.allowance(senderAddr, dragoNFTAddr);
    ...
    Drago.transferFrom(senderAddr, deadWallet, onePercent); // <= Hasta aca OK!
  ...
  function grow(uint256 _tokenIdx) public {
    ...
    require(current.level == Level.MysteryEgg, "no mystery egg");
  ...
  function buy(uint256 _tokenId) public {
    ...
    Drago.transferFrom(buyerAddr, initialOwner, price);
  }
