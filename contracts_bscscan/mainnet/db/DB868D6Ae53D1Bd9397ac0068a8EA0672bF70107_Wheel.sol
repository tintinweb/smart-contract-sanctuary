...
contract Wheel is Context, Ownable {
    ...
    function addPool(bytes32 name, string memory description, uint256 price, uint max) external onlyOwner(){
    ...
    function updatePool(bytes32 name, uint256 price, uint max, uint pool_id, string memory description) external onlyOwner(){
        Pool storage pool = pools[pool_id];
        pool.name = name;
