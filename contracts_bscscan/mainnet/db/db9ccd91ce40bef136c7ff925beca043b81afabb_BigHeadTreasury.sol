...
contract BigHeadTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SBHD }

    address public immutable BHD;
    ...
    address public sBHD;
    uint public sBHDQueue; // Delays change to sBHD address
    ...
    /**
        @notice allow approved address to deposit an asset for BHD
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BHD needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BHD ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BHD for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IBHDERC20( BHD ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBHD ).balanceOf( msg.sender ); // Can only borrow against sBHD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BHD
        @param _amount uint
     */
    function repayDebtWithBHD( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IBHDERC20( BHD ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BHD, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BHD ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BHD ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BHD valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BHD decimals
            value_ = _amount.mul( 10 ** IERC20( BHD ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SBHD ) { // 9
            sBHDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
