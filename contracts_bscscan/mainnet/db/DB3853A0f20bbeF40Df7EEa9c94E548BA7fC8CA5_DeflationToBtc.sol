...
contract DeflationToBtc is ERC20, Owned {
    ...
    constructor() ERC20("Deflation To BTC", "DTB") {
        minSupply = 21000000 * (10 ** 18);
    ...
    function _transfer(
        ...
        uint256 burnedFee = calculateFee(amount, 1000);
        if (totalSupply() - burnedFee >= minSupply) {
            _burn(sender, burnedFee);
        } else {
            burnedFee = 0;
        }
        uint256 transferFee = calculateFee(amount, 500);
