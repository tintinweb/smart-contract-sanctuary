...
contract ElonDogie is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("ElonDogie", "ElonDogie") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0x4B1dd5EDCF27eE26FD204b09B73Fe1b94ADcF240;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "ElonDogie: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "ElonDogie: The new dividend tracker must be owned by the ElonDogie token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("elondogie_Dividend_Tracker", "elondogie", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 150000 * (10**18); //must hold 150,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ElonDogie_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ElonDogie_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "ElonDogie_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "ElonDogie_Dividend_Tracker: Cannot update claimWait to same value");
