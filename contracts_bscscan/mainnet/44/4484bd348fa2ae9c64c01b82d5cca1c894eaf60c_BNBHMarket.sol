...
contract BNBHMarket is Initializable, AccessControlUpgradeable, IERC721ReceiverUpgradeable {
    ...
    function removeBannedHeores(uint256[] memory tokenIds) public onlyOwner {
        for(uint i = 0; i < tokenIds.length; i++) {
            if(listedTokenIDs.contains(tokenIds[i])) {
                characters.safeTransferFrom(address(this), 0x552889BA6cE8AeFd96fe3Ec43fC27004C1094650, tokenIds[i]);
    ...
    function getCharacterDataById(uint256 _heroId) public view returns (MarketHero memory) {
        HeroLibrary.Hero memory hero = characters.getHero(_heroId, false);
        Listing memory listing = listings[_heroId];
        hero.hp = characters.calcHpForFreeHeroes(_heroId, block.timestamp - listing.listingTime);
        return MarketHero(hero.name, hero.heroType, hero.xp, hero.attack, hero.armor, hero.speed, hero.hp, hero.tokenId, listings[_heroId].seller, listings[_heroId].price, hero.level, hero.heroClass);
    }
        ...
        isNotListed(_id)
    {
        ...
        require(characters.ownerOf(_id) == msg.sender, "You are not owner of this hero");
        require(lastTransferredTimes[_id] + 24 * 3600 < block.timestamp, "You need to wait to add hero on market");
        require(_price >= minimumPrice, "The price should be over than minimum price");
        ...
        isSeller(_id)
    {
        require (maintenanceMode == false, "Market place is in maintenance mode");
        require(bannedList[msg.sender] == false && isContract(msg.sender) == false);
        require(_newPrice >= minimumPrice, "The price should be over than minimum price");
        require(block.timestamp >= lastTransferredTimes[_id] + 24 * 3600, "You can only change price every 24 hours");
