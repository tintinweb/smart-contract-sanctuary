...
contract BashToken is BEP20, MinterRole {
    ...
    uint256 public minSwapAmount = 100000000000;   //10 PAL tokens
    // Trading bool
    bool public tradingOpen; 

    // In swap and liquify
    bool private _inSwapForBUSD;

    // Automatic swap and liquify enabled
    bool public swapForBUSdEnabled = true; // INMUTABLE
     ...
     */
    constructor(address _feeWallet) public BEP20("BASH PROTOCOL", "BPT") {
        operator = _msgSender();
        ...
        require(_feeWallet != address(0), "wallet address is zero");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "PAL::transferOperator: new operator is the zero address");
     ...
     */
    function updateFeeWallet(address _feeWallet) public onlyOperator {
        require( _feeWallet != address(0), "PAL:: tax fee address is zero" );
        emit UpdateFeeWallet(_feeWallet);
        feeWallet = _feeWallet;
    }

    function updateBuyFee(uint16 _buyFee) public onlyOperator {
        require( _buyFee <= 100, "PAL:: Biu Fee can't exceed 100%" );
        emit UpdateBuyFee(_buyFee);
        buyFee = _buyFee;
    }

    function updateSellFee(uint16 _sellFee) public onlyOperator {
        require( _sellFee <= 100, "PAL:: Sell Fee can't exceed 100%");
    ...
    function enableSwapTokenForBusd(bool _enabled) public onlyOperator {
        swapForBUSdEnabled = _enabled;
    }
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BASHs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBashPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBashPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BASH to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BASH distribution occurs.
        uint256 accBashPerShare;   // Accumulated BASH per share, times 1e12. See below.
        uint16 withdrawFeeBP;      // Withdraw fee in basis points
    }

    // The BASH TOKEN!
    BashToken public bash;
    // BASH tokens created per block.
    uint256 public bashPerBlock;
    // Bonus muliplier for early BASH makers.
    ...
    // The block number when BASH mining starts.
    ...
    event EmissionRateUpdated(address indexed user, uint256 bashPerBlock);

    constructor(
        BashToken _bash,
        address _feeAddDev,
        uint256 _bashPerBlock,
        uint256 _startBlock
    ) public {
        bash = _bash;
        feeAddDev = _feeAddDev;
        bashPerBlock = _bashPerBlock;
    ...
    // Update the given pool's BASH allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BASHs on frontend.
    function pendingBash(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBashPerShare = pool.accBashPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 bashReward = multiplier.mul(bashPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBashPerShare = accBashPerShare.add(bashReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accBashPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 bashReward = multiplier.mul(bashPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bash.mint(address(this), bashReward);
        pool.accBashPerShare = pool.accBashPerShare.add(bashReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for BASH allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant poolExists(_pid) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBashPerShare).div(1e12);
    ...
    // Safe BASH transfer function, just in case if rounding error causes pool to not have enough BASHs.
    function safeBashTransfer(address _to, uint256 _amount) internal {
        uint256 bashBal = bash.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > bashBal) {
            transferSuccess = bash.transfer(_to, bashBal);
        } else {
            transferSuccess = bash.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBashTransfer: transfer failed");
    }
    ...
    function harvest(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accBashPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            // send rewards
            safeBashTransfer(msg.sender, pending);
        }
    }

    // Pancake has to add hidden dummy pools in order to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _bashPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, _bashPerBlock);
        bashPerBlock = _bashPerBlock;
    }
