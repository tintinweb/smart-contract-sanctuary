...
// import '/Users/alexandermazaletskiy/Documents/Upwork/Test3four/node_modules/@uniswap/v2-periphery/contracts/interfaces/IUniswapV2Router01.sol';
...
// Root file: contracts/Test3FOUR.sol
...
/*
1. Fees from selling / transferring
2.buying Test3four unlimited, selling Test3four as equivalent 5 BNB
3. Reward Fee - 2% (stored as BNB)
4. Tresuary Fee - 5% (stored as BNB)
5. Buyback and Burn Fee - 1%
6. Reflections Fee for all users (who have balance more than 1 Test3four) - 3%


Total Fees: 11%

Rewards accumulates and available for claim after 24 hours.
*/
contract Test3FOUR is Context, IERC20, Ownable {
    ...
    string private constant _name = "Test3FOUR";
    string private constant _symbol = "Test3FOUR";
    ...
    //total supply
    uint256 private constant _tTotal = 500000000000 ether; //500 billions
    ...
    uint256 private _previousbnbRewardsFee = bnbRewardsFee;
    uint256 public bnbRewardsFeeBalance;
    ...
    bool private _isSwapFeesEnabled;

    uint256 maxSellAmount = 5 ether;

    event SendBNBRewardsFee(address to, uint256 amountBNB);
    ...
    event SwapAndLiquify(
        uint256 tokensSwappedForBNB,
        uint256 BNBAddedForLp,
    ...
    function initContract(address routerAddress, address treasuryAddress)
        external
        onlyOwner
    {
        ...
        IUniswapV2Router02 _pancakeSwapRouter = IUniswapV2Router02(
    ...
    /// @notice start trading and initialize fees
    ...
    /// @notice get last time for claim for wallet
    function getLastbnbRewardsFeeClaim(address wallet)
        external
        view
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        // buy
        if (
            (from == pancakeSwapPair || _isPancakeSwapPair[from]) &&
        ...
        // sell or transfer
        if (!_inSwapAndLiquify && _tradingOpen && _isSwapFeesEnabled) {

            //if sell check max amount
            if (to == pancakeSwapPair || _isPancakeSwapPair[to]) {
                (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(
                    pancakeSwapPair
                ).getReserves();

                (reserve0, reserve1) = IUniswapV2Pair(pancakeSwapPair)
                    .token0() == address(this)
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);

                uint256 estimatedEquivalentBNB = pancakeSwapRouter.getAmountOut(
                    amount,
                    reserve0,
                    reserve1
                );
            ...
            if (_contractTokenBalance > 0) {
                ...
                uint256 maxSwapAmount = balanceOf(pancakeSwapPair)
                        .mul(feeRate)
                        .mul(PRECISION)
                        .div(100)
                        .div(PRECISION);
    ...
    function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
        ...
        bnbRewardsFeeBalance += bnbBalanceUpdate.mul(bnbRewardsFee).div(
            _liquidityFeeTotal
        );

        // send BNB to treasury address
        uint256 treasuryBNBBalance = bnbBalanceUpdate.mul(treasuryFee).div(
            _liquidityFeeTotal
        );
        if (treasuryBNBBalance > 0) {
            _sendBNBToTreasury(treasuryBNBBalance);
        }

        // buy back
        uint256 buybackBNBBalance = bnbBalanceUpdate.mul(buybackFee).div(
            _liquidityFeeTotal
        );
        if (buybackBNBBalance > 0) {
            _buyBackTokens(buybackBNBBalance);
        }
    }

    function _sendBNBToTreasury(uint256 amount) private {
        _deliverFunds(treasuryWallet, amount, "Cant send BNB to address");
    }
    ...
    function _buyBackTokens(uint256 amount) private {
        ...
        emit SwapBNBForTokens(buybackReceiver, amount, path);
    }
    ...
    function _liquidityFeeAggregate() private view returns (uint256) {
        return treasuryFee.add(bnbRewardsFee).add(buybackFee);
    }
    ...
    function getSellSlippage(address seller) external view returns (uint256) {
        uint256 feeAgg = treasuryFee.add(bnbRewardsFee).add(buybackFee);
    ...
    function setIsSwapFeesEnabled(bool _enabled) external onlyOwner {
        _isSwapFeesEnabled = _enabled;
    }

    function setMaxSellAmount(uint256 _maxSellAmount) external onlyOwner {
        maxSellAmount = _maxSellAmount;
    }
    function setPancakeSwapRouter(address _pancakeSwapRouterAddress)
        external
        onlyOwner
    {
    ...
    function calculateBNBRewards(address wallet)
        public
        view
        returns (uint256)
    {
        uint256 baseRewards = bnbRewardsFeeBalance.mul(balanceOf(wallet)).div(
            _tTotal.sub(balanceOf(DEAD_ADDRESS)) // circulating supply
        );
    ...
    function canClaimRewards(address user) public view returns (bool) {
        return
            block.timestamp >
            _rewardsLastClaim[user].add(rewardsClaimTimeSeconds);
    }

    function claimBNBRewards() external {
        require(
            balanceOf(_msgSender()) > 0,
            "You must have a balance to claim BNB rewards"
        );
        require(
        ...
        uint256 rewardsSent = calculateBNBRewards(_msgSender());
        bnbRewardsFeeBalance -= rewardsSent;

        _deliverFunds(_msgSender(), rewardsSent, "Cant send BNB to address");

        emit SendBNBRewardsFee(_msgSender(), rewardsSent);
    }
