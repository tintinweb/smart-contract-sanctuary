...
contract DogeFire is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Doge Fire";
    string constant _symbol = "DOGEFIRE";
    ...
    function getMultipliedFee() public view returns (uint256) {
        // buyback time + 30mins - now()
        uint256 remainingTime = buybackMultiplierTriggeredAt.add(buybackMultiplierLength).sub(block.timestamp);
        uint256 feeIncrease = totalFee.mul(buybackMultiplierNumerator).div(buybackMultiplierDenominator).sub(totalFee);
        // gradually decrease the increased fee
    ...
    function swapBack() internal swapping {
        // if over LP is > 25% of MC, then 0 otherwise the LP fees
    ...
    function setTargetLiquidity(uint256 _target, uint256 _denominator) external authorized {
        //Function to set max LP ratio to MC, default values are 25, 100 i.e. 25%
