...
// File: contracts/fancyDOGEDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable BUSD = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
...
contract fancyDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
    address private _liqAdd = address(0xc685aD263EA1B07C3b36dCF9bD9092b69EeA9e02);
    address private _LPadd = address(0x4B872e3F0d7F6c27E772b25e51f8342067286C72);

    bool private swapping;
    bool public airdrop;

    fancyDOGEDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public liqAdd;
    address public LPadd = _LPadd;

    uint256 public maxBuyTranscationAmount = 100000000000 * (10**18); // 2%
    ...
    constructor() public ERC20("fancyDOGE", "fDOGE") {
    	...
    	dividendTracker = new fancyDOGEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "fancyDOGE: The dividend tracker already has that address");

        fancyDOGEDividendTracker newDividendTracker = fancyDOGEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "fancyDOGE: The new dividend tracker must be owned by the fancyDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "fancyDOGE: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "fancyDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "fancyDOGE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "fancyDOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "fancyDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "fancyDOGE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "fancyDOGE: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            sellFeeIncreaseFactor = sellFeeIncreaseFactor + 1;
        ...
        if(
            ...
            uint256 bnbrewardstoken = contractTokenBalance.mul(9).div(totalFees);
            swapTokensForBNB(bnbrewardstoken);
            
            uint256 liqtokens = contractTokenBalance.mul(6).div(totalFees);
...
contract fancyDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("fancyDOGE_Dividend_Tracker", "fancyDOGE_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "fancyDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "fancyDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main fancyDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "fancyDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "fancyDOGE_Dividend_Tracker: Cannot update claimWait to same value");
