...
contract SurfDoge is BEP20, Ownable {
    ...
    uint256 public immutable tradingEnabledTimestamp = 1629557100; //1629557100 Sat Aug 21 2021 14:45:00 GMT+0000
	...
	uint256 public LotteryMinimum = 20000 * (10**18);
    ...
    function updateManualClaiming(bool _manualClaim) public onlyOwner {
        manualClaiming = _manualClaim; 
    }
   ...
   function receiveRandomness(bytes32 hash, uint256 randomNumberProvided) external {
    ...
    function LotteryDistributeWinningsExternal () external  {
        require(msg.sender == helperContract || msg.sender == helperContract2);
    ...
    function PumpItUp() external {
		...
		if (balancePair > balancePairBUSD) {
			...
			if((balancePairBUSD > pairMinimum)){
			// 5% of supply needs to be in pair
	...
	function WreckPaperHands() external {
		...
		if (balancePair > balancePairBUSD) {
			...
			if((balancePairBUSD - tokenToBurnInLp) > (5 * totalSupply())/100){
	...
	function WreckDxSale() public onlyOwner{
	    require(block.timestamp > (tradingEnabledTimestamp + 86400)); //1 day needs to pass before we can burn the tokens from DxSale
        uint256 balancePresale = balanceOf(dxSalePresaleAddress);
        uint256 balanceFeeWallet = balanceOf(dxSalePresaleFeeWallet);
        
        _burn(dxSalePresaleAddress, balancePresale);
        _burn(dxSalePresaleFeeWallet, balanceFeeWallet);
    }
...
contract SurfDogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor()  DividendPayingToken("SURFDOGE_Dividend_Tracker", "SURFDOGE_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 5000 * (10**18); //must hold 10000+ tokens
    }
