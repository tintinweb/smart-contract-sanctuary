...
import "./IGateManagerMultiRewards.sol";
...
contract PotsConverter is Ownable {
    ...
    uint128 public totalAlloc = 1000;
    ...
    event NewReserve(address newReserve, address oldReserve);
    ...
    function singleHarvest(uint256 pid) public onlyHarvester {
            SinglePrizes storage convert = singlePrize[pid];
            ReserveInterface(reserve).withdrawReserve(convert.prizePool, address(this));
    ...
    // Main function. Divides Beefy's profits.
    function beltHarvest(uint256 pid) public onlyHarvester {
    ...
    // Main function. Divides Beefy's profits.
    function lpHarvest(uint256 pid) public onlyHarvester {
    ...
    function notifyPots() external onlyOwner {
        ...
        IERC20Upgradeable(pots).safeTransfer(ziggyManager, potsBal);
        IGateManagerMultiRewards(ziggyManager).notifyRewardAmount(0, potsBal);
        potsBuyBack = 0;
    }
    ...
    function setReserve(address _reserve) external onlyOwner {
        emit NewReserve(reserve, _reserve);
        reserve = _reserve;
    }
    ...
    function setLpUnirouter(uint256 prizeId, address _router) external onlyOwner {
        ...
        IERC20Upgradeable(token0).safeApprove(lpPrize[prizeId].router, 0);
        IERC20Upgradeable(token1).safeApprove(lpPrize[prizeId].router, 0);
    ...
    function transferReserveOwner(address owner) external onlyOwner {
        ReserveInterface(reserve).transferOwnership(owner);
    }

    function setReserveRateMantissa(uint256 _rate) external onlyOwner {
        ReserveInterface(reserve).setRateMantissa(_rate);
    }
