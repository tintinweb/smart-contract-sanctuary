...
// File: contracts/ATHENDividendPayingToken.sol
...
contract ATHEN is ERC20, Ownable {
    ...
    ATHENDividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTransactionAmount = 2800000 * (10**18);
    uint256 public maxSellTransactionAmount = 280000 * (10**18);
    uint256 public swapTokensAtAmount = 150000 * (10**18);
    uint256 public _maxWalletToken = 5600000 * (10**18); // 1.5% of total supply
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "ATHEN: The dividend tracker already has that address");

        ATHENDividendTracker newDividendTracker = ATHENDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ATHEN: The new dividend tracker must be owned by the ATHEN token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ATHEN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "ATHEN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "ATHEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ATHEN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ATHEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ATHEN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            revert("ATHEN: This account cannot send tokens until trading is enabled");
        }
        if(amount == 0) {
...
contract ATHENDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("ATHEN_Dividend_Tracker", "ATHEN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 7000 * (10**18); //must hold 7000+ tokens
    }
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "ATHEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "ATHEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ATHEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ATHEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ATHEN_Dividend_Tracker: Cannot update claimWait to same value");
