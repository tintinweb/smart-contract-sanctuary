...
interface IERC20 {
     ...
     * https://github.com/ethreum/EIPs/issues/20#issuecomment-263524729
     *
...
@author Roger Wu (https://github.com/roger-wu)
 An interface for a dividend-paying token contract.
*/
interface IDividendPayingToken {
  ...
  //  SHOULD transfer `dividendOf(msg.sender)` wei to `msg.sender`, and `dividendOf(msg.sender)` SHOULD be 0 after the transfer.
  //  MUST emit a `DividendWithdrawn` event if the amount of ether transferred is greater than 0.
  function withdrawDividend() external;

  //  This event MUST emit when ether is distributed to token holders.
...
interface IDividendPayingTokenOptional {
  ...
  //  accumulativeDividendOf(_owner) = withdrawableDividendOf(_owner) + withdrawnDividendOf(_owner)
  // @param _owner The address of a token holder.
...
@author Roger Wu (https://github.com/roger-wu)
 A creatable ERC20 token that allows anyone to pay and distribute ether
...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    //  see https://github.com/ethreum/EIPs/issues/1726#issuecomment-472352728
    ...
    // Distributes ether to token holders as dividends.
    ...
    //  Withdraws the ether distributed to the sender.
    //  It emits a `DividendWithdrawn` event if the amount of withdrawn ether is greater than 0.
...
contract DogeMetaverse is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Doge Metaverse", "DGMT") {
    	dogeDividendTracker = new DogeDividendTracker();

    	marketingWallet = 0xB9Cb973159fC76Ce78e91B60B0A2Ed18E1434217;
    	divident = 0xD981D6eb5522c8c477E93b8cF955C46478Fe0A1D;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "DogeMetaverse: The marketing wallet is already this address");
    ...
    function setLiquifyEnabledEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = LiquidityFee.add(marketingFee).add(dogeDividendRewardsFee);
            LiquifyEnabled = _enabled;
        }
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeDividendTracker), "DogeMetaverse: The dividend tracker already has that address");

        DogeDividendTracker newDogeDividendTracker = DogeDividendTracker(payable(newAddress));

        require(newDogeDividendTracker.owner() == address(this), "DogeMetaverse: The new dividend tracker must be owned by the DogeMetaverse token contract");
    ...
    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        LiquidityFee = newFee;
        totalFees = LiquidityFee.add(dogeDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DogeMetaverse: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "DogeMetaverse: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DogeMetaverse: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "DogeMetaverse: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "DogeMetaverse: Cannot update gasForProcessing to same value");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 dividentPortion = address(this).balance.div(10**2).mul(25);
...
contract DogeDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DogeMetaverse_Dogereum_Dividend_Tracker", "DogeMetaverse_Dogereum_Dividend_Tracker", 0xbA2aE424d960c26247Dd6c32edC70B295c744C43) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DogeMetaverse_Dogereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DogeMetaverse_Dogereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogeMetaverse contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "DogeMetaverse_Dogereum_Dividend_Tracker: Cannot update claimWait to same value");
