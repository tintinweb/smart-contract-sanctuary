...
contract StarlinkX is Context, IERC20, IERC20Extented, Ownable {
   ...
   function calculatePercentage(uint _currentPrice, uint _previousPrice) public pure returns (uint) {
       uint percentChange;
   ...
   function checkAndSetTaxOnTransfer(uint currentPrice) internal returns (DynamicTax memory currentTax) {
       ...
       bool isSTART21 = block.timestamp % blockWindow >= STARTblock;
       bool isEND23 = block.timestamp % blockWindow <= ENDblock;
       bool isSTART23 =  block.timestamp % blockWindow > ENDblock;
 
       if (isSTART21 && isEND23) { //if between 21-23 EST
           windowStarted = true;
           startGoldenHour();
       ...
       if (isSTART23 && windowStarted) { // update previousClose with the new price after 30% allowed dip window ends
           windowStarted = false;
           previousClose = currentPrice;
       }
      
       if (isSTART23 && windowEnded) {
           windowEnded = true;
           endGoldenHour();
       }
   ...
   function setEndOfDay() external {
       require(block.timestamp % blockWindow >= 0 && block.timestamp % blockWindow <= 1200, "Time not 12AM UTC yet");
       previousDayPrice["get"].time = block.timestamp;
       previousDayPrice["get"].price = this.getTokenPrice();
   }
 
   function getPreviousDayData() view public onlyOwner returns(uint) {
       return previousDayPrice["get"].time;
   }
   ...
   function endGoldenHour() internal onlyOwner {
       _buyMarketingFee = 5;
       _buyBuybackFee = 3;
       _buyReflectionFee = 1;
       _buyLiquidityFee = 2;
...
function NormalEngineHours() internal onlyOwner {
       _buyMarketingFee = 5;
       _buyBuybackFee = 3;
       _buyReflectionFee = 1;
       _buyLiquidityFee = 2;
   ...
   function startGoldenHour() internal onlyOwner {
       _buyMarketingFee = 3;
       _buyBuybackFee = 2;
       _buyReflectionFee = 1;
       _buyLiquidityFee = 2;
   ...
   function disableRandomFloor() external onlyOwner() {
       require(randomFloor ==  true, "randomFloor already disabled");
       randomFloor = false;
   }
 
   function enableRandomFloor() external onlyOwner() {
       require(randomFloor == false, "randomloor already enabled");
       randomFloor = true;
   }
 
   function setFloor() external onlyOwner() {
       require(randomFloor == true, "must enable randomFloor");
