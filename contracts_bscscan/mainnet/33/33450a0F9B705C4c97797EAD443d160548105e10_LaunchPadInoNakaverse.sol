...
contract LaunchPadInoNakaverse is Ownable {
    ...
    function round2Allowance(address _address) public view returns (uint256) {
        ...
        if (_hasParticipated[_address]) {
            ...
            result = round1Allowance[_address].mul(3);
        }
        return result;
    }

    uint256 public NFTTarget = 50;
    uint256 public NFTPrice = 5000 * 1E18;
    ...
    function initSale(uint256 _NFTTarget, uint256 _NFTPrice)
        external
        onlyOwner
    {
        require(_NFTTarget > 0, "Target can't be Zero");
        require(_NFTPrice > 0, "Price can't be Zero");
        NFTTarget = _NFTTarget;
        NFTPrice = _NFTPrice;
    }
    ...
    function buyRound1Stable(uint256 _amount) external {
        ...
        require(NFTSold.add(_amount) <= NFTTarget, "Target already hit");
        require(
        ...
        if (!_hasParticipated[msg.sender]) {
            ...
            _round2Allowance[msg.sender] = round1Allowance[msg.sender].mul(3);
        }
        ...
        claimableAmount[msg.sender] = claimableAmount[msg.sender].add(_amount);
        NFTSold = NFTSold.add(_amount);

        if (NFTSold == NFTTarget) {
            emit RoundChange(3);
            endUnlocked = true;
            emit EndUnlockedEvent(block.timestamp);
        }
    ...
    function buyRound2Stable(uint256 _amount) external {
        require(_roundNumber() == 2, "presale isn't on good round");
        require(NFTSold.add(_amount) <= NFTTarget, "Target already hit");
        ...
        require(NFTSold.add(_amount) <= NFTTarget, "sold out");
