...
/**  
   ...
   Great features:
	...
	- 1% fee auto moved to buyback wallet
    - 3% fee auto moved to busd wallet
	- 3% fee auto distribute to all holders
    - 2% fee auto moved to burn wallet
	
   100,000,000,000,000,000 total supply
   
   1% fee for liquidity will go to an address that the contract creates, and the contract will sell it and add to liquidity automatically.
*/
...
contract DividendDistributor is IDividendDistributor {
    ...
    function getCumulativeDividends(uint256 share) internal view returns (uint256) {
        uint256 totalDividends = BUSD.balanceOf(address(this));
        uint256 dividendsPerShare = dividendsPerShareAccuracyFactor.mul(totalDividends).div(totalShares);
...
contract Grove is Context, IBEP20, Ownable {
    using SafeMath for uint256;
	...
	address public _buybackWallet = 0x545498c71f9894c641f8304f408153084bF39fBa;
    address public _busdWallet = 0x09F19d320C0E758552f0692BD6aBc10b80185AeE;
    ...
    string private _name = "Grove";
    string private _symbol = "GRV";
    ...
    uint256 public busdFee = 3;
    uint256 public buybackFee = 1;	
	
    uint256 private _liquidityFee = liquidityFee.add(busdFee).add(buybackFee);
    ...
    uint256 public _maxSaleTxAmount = 200000000000000 * 10**18;
	uint256 public _maxWalletAmount = 1000000000000000 * (10**18);
    ...
    constructor () {
		...
		_isExcludedFromMaxWalletToken[pancakeSwapV2Pair] = true;
	...
	function setBUSDFeePercent(uint256 BUSDFee) external onlyOwner() {
        busdFee = BUSDFee;
    }
	...
	function setBUSDWallet(address payable busdWallet) external onlyOwner() {
         _busdWallet = busdWallet;
    }
	...
	function setSaleTxAmount(uint256 amount) external onlyOwner() {
        _maxSaleTxAmount = amount * 10**18;
    }
	
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		_maxWalletAmount = amount * 10**18;
	}
    ...
    function removeAllFee() private {
        if(_reflectionFee == 0 && _liquidityFee == 0 && _burnFee == 0) return;
    ...
    function _transfer(address from, address to, uint256 amount) private {
		...
		if(to == pancakeSwapV2Pair && from != owner() && to != owner()) 
		{
		    require(amount <= _maxSaleTxAmount, "Transfer amount exceeds the _maxSaleTxAmount.");
		}
		
		if(!_isExcludedFromMaxWalletToken[to] && to != pancakeSwapV2Pair) 
		{
            uint256 balanceRecepient = balanceOf(to);
            require(balanceRecepient + amount <= _maxWalletAmount, "Exceeds maximum wallet token amount");
        }
		
        uint256 contractTokenBalance = balanceOf(address(this));
        
        if(contractTokenBalance >= _maxSaleTxAmount)
        {
            contractTokenBalance = _maxSaleTxAmount;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
	{
		...
		uint256 buybackPart = newBalance.div(_liquidityFee).mul(buybackFee);
		uint256 busdPart = newBalance.sub(liquidityPart).sub(buybackPart);
		
		payable(_buybackWallet).transfer(buybackPart);
		swapBNBForBUSD(busdPart);
	...
	function swapBNBForBUSD(uint256 amount) private {
		...
		pancakeSwapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(0, path, _busdWallet, block.timestamp.add(300));
    }
	...
	function withdrawToken(address _recipient, uint256 _amount) public onlyOwner returns(bool) {
        IBEP20(address(this)).transfer(_recipient, _amount); 
        return true;
    }
