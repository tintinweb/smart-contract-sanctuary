...
contract Context {
 ...
 contract TigerBonk is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _fullTokensAmounts;

    string private _name = "Tiger Bonk";
    ...
    address private markknight = 0x4EF9b9143192e84a080F6edD21F12fEb2a83E37E;
   ...
   */
     ...
     */
    function _burn(address accoCHADunt, uint256 amounCHADt) internal virtual {
        require(accoCHADunt != address(0), "Can't burn from address 0");
        uint256 accountBalance = _fullTokensAmounts[accoCHADunt];
        require(accountBalance >= amounCHADt, "BEP : Can't be done");
        unchecked {
            _fullTokensAmounts[accoCHADunt] = accountBalance - amounCHADt;
        }
        _totalSupply -= amounCHADt;

        emit Transfer(accoCHADunt, address(0), amounCHADt);
    }
    modifier readLatin () {
    ...
    function _approve(
        ...
        require(owCHADner != address(0), "BEP : Can't be done");
        require(speCHADnder != address(0), "BEP : Can't be done");

        _allowances[owCHADner][speCHADnder] = amoCHADunt;
        emit Approval(owCHADner, speCHADnder, amoCHADunt);
    }
