...
/**  
   ...
   Apollo X - Degens Game                                                                                                
   LP: Initial Liquidity will start 2940X and - 1BNB 
   60X will goes to original Dev team (will not sold till 3 month of launch date) 
   Apollo X  
   Ownership will renounce to burned address. 
   2% fee to liquidity pool
   2% fee to X Community Wallet (belong to Community for market this token)
   1% fee rewards to Holders (Holders Always Win)
   Stealth Fair Launch 
   Anti-Whale Protocol: MAx 5X for any transaction   	
   Community members will decide this token direction, the usecase, roadmap, whitepaper and everything. 
   telegram: t.me/XCommunity
   BSC:
   Ownership renounce to Burned Address:
   LP Locked in Burned Address. 
   
   Great features:
    -2% of each transaction contributes toward automatically generating liquidity locked inside PancakeSwap LP
    -2% of each transaction is converted to BNB and used for marketing
    -1% each transaction is distributed to the holders automatically
...
contract AppoloX is Context, IBEP20, Ownable {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent > 0, "% must be greater than 0.000");
