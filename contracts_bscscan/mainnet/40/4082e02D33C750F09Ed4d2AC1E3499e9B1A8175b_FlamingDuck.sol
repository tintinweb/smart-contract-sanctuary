...
library SafeMath {
    ...
    function trySub(uint df1, uint df2) internal pure returns (bool, uint) {
        unchecked {
    ...
    function tryDiv(uint df1, uint df2) internal pure returns (bool, uint) {
        unchecked {
    ...
    function tryMod(uint df1, uint df2) internal pure returns (bool, uint) {
        unchecked {
            if (df2 == 0) return (false, 0);
            return (true, df1 % df2);
        }
    ...
    function mod(uint df1, uint df2) internal pure returns (uint) {
        return df1 % df2;
    }
...
contract FlamingDuck is Contexts, ICOIN, ICOINMetadata, Ownable {
