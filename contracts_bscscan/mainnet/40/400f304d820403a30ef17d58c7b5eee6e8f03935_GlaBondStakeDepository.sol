...
contract GlaBondStakeDepository is Ownable {
    ...
    address public immutable GLA; // intermediate reward token from treasury
    address public immutable sGLA; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sGLA gons remaining to be paid
        uint glaPayout; //gla amount at the moment of bond
    ...
    function redeem( address _recipient ) external returns ( uint ) {        
        ...
        uint _amount = ISGLA(sGLA).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sGLA ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sGLA balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISGLA(sGLA).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISGLA(sGLA).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
