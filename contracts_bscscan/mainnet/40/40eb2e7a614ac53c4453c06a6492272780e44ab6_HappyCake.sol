...
contract HappyCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    HappyCakeDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x2782Cd67B57f5A454647Ef1b7002d1085cFe8e49); 
    address payable public operations2Address =
        payable(0x0Cd6E88378A8eE430EF0308D1B800923FC9238cb);
    ...
    uint256 public devFees = 5; // Dev 1.5% from here, Marketing is 3% 
    uint256 public liquidityFee = 0;
    uint256 public cakeRewardsBuyFee = 0;
    uint256 public cakeRewardsSellFee = 10;
    
    
    uint256 public snipecakeRewardsBuyFee = 0;
    uint256 public snipecakeRewardsSellFee = 0;
    ...
    event UpdateFees(
        uint256 dev,
        uint256 liquidity,
        uint256 cakeRewardsBuy,
        uint256 cakeRewardsSell
    );
    ...
    constructor() ERC20("HappyCake", "HappyCake") {
        dividendTracker = new HappyCakeDividendTracker(payable(this));
        ...
        totalSellFees = cakeRewardsSellFee.add(liquidityFee).add(devFees);
        // totalBuyFees = cakeRewardsBuyFee.add(liquidityFee).add(devFees);
        totalBuyFees = cakeRewardsBuyFee.add(liquidityFee);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        HappyCakeDividendTracker newDividendTracker = HappyCakeDividendTracker(
            payable(newAddress)
        );
    ...
    function updatecakeRewardsSell(uint256 newValue) public onlyOwner {
        cakeRewardsSellFee = newValue;
    }
    function updatesnipecakeRewardsSell(uint256 newValue) public onlyOwner {
        snipecakeRewardsSellFee = newValue;
    }
    function updatesnipecakeRewardsBuy(uint256 newValue) public onlyOwner {
        snipecakeRewardsBuyFee = newValue;
    }
    ...
    function updateFees(
        ...
        // require(
        //     cakeRewardsBuy >= 0 && cakeRewardsBuy <= 40,
        //     "cake reward fees must be between 0 and 40"
        // );
        // require(
        //     cakeRewardsSell >= 5 && cakeRewardsSell <= 40,
        //     "cake reward fees must be between 5 and 40"
        // );

        devFees = dev;
        liquidityFee = liquidity;
        cakeRewardsBuyFee = cakeRewardsBuy;
        cakeRewardsSellFee = cakeRewardsSell;

        totalSellFees = cakeRewardsSellFee.add(liquidityFee).add(devFees);
        totalBuyFees = cakeRewardsBuyFee.add(liquidityFee).add(devFees);

        emit UpdateFees(dev, liquidity, cakeRewardsBuy, cakeRewardsSell);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if(killSnipeEnabled && !_isExcludedFromSnipe[from] && !_isExcludedFromSnipe[to]) {
                if (isSelling) {
                    cakeRewardsFee = snipecakeRewardsSellFee;
                } else {
                    _isConsiderAsSnipe[to] = true;
                    cakeRewardsFee = snipecakeRewardsBuyFee;
                }
            } else {
                if (isSelling) {
                    if(!_isConsiderAsSnipe[from]) {
                        ...
                        cakeRewardsFee = cakeRewardsSellFee;
                    } else {
                        cakeRewardsFee = snipecakeRewardsSellFee;
                    }
                } else {
                    cakeRewardsFee = cakeRewardsBuyFee;
                }
            }
            ...
            uint256 totalFees = cakeRewardsFee.add(liquidityFee).add(devFees);
            if(!isSelling) {
                totalFees = cakeRewardsFee.add(liquidityFee);
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .mul(buyAmount)
            .div(totalAmount)
            .mul(cakeRewardsBuyFee)
            .div(cakeRewardsBuyFee.add(devFees));

        uint256 dividendsFromSell = address(this)
            .balance
            .mul(sellAmount)
            .div(totalAmount)
            .mul(cakeRewardsSellFee)
            .div(cakeRewardsSellFee.add(devFees));
...
contract HappyCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "HappyCake_Dividend_Tracker",
            "HappyCake_Dividend_Tracker"
        )
    {
        Happycake = HappyCake(mainContract);
    ...
    function _transfer(
        ...
        require(false, "HappyCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "HappyCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HappyCake contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Happycake);

            uint256 prevBalance = Happycake.balanceOf(address(this));
            ...
            {
                uint256 received = Happycake.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    Happycake.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            // if no tokenAddress assume cake payout
            if (
                !allowCustomTokens ||
