...
contract SakePerpVault is ISakePerpVault, OwnableUpgradeable {
    ...
    event HedgeProfitSend(address indexed exchange, uint256 highAmount, uint256 lowAmount);
    ...
    function sendHedgeProfit(address _exchange, uint256 _profit) public {
        IExchange(_exchange).quoteAsset().safeTransferFrom(_msgSender(), address(this), _profit);
        (Decimal.decimal memory highFactor, Decimal.decimal memory lowFactor) = _getMMFactor(_exchange);
        ExchangeInfo storage _exchangeInfo = exchangeInfo[_exchange];
        PoolInfo storage highPool = _exchangeInfo.poolInfo[uint256(Risk.HIGH)];
        PoolInfo storage lowPool = _exchangeInfo.poolInfo[uint256(Risk.LOW)];
        Decimal.decimal memory profit = Decimal.decimal(_profit);
        Decimal.decimal memory profitForHigh = profit.mulD(highFactor).divD(highFactor.addD(lowFactor));
        Decimal.decimal memory profitForLow = profit.subD(profitForHigh);
        highPool.totalLiquidity = highPool.totalLiquidity.addD(profitForHigh);
        lowPool.totalLiquidity = lowPool.totalLiquidity.addD(profitForLow);
        emit HedgeProfitSend(_exchange, profitForHigh.toUint(), profitForLow.toUint());
    }
