...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public dividendsPerShareAccuracyFactor = 10 ** 22;
    ...
    event GasUsedDeposit(uint256 gasUsed);
    event GasUsedSwap(uint256 gasUsed);
    event GasUsedPrepareSwap(uint256 gasUsed);
    function deposit(uint amount) external override onlyToken {
        totalDividends = totalDividends.add(amount);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(totalShares));
    }

    event GasUsedProcess(uint256 gas);
    ...
    function setDividendToken(IBEP20 _dividendToken) external onlyToken {
        nativeToken = _dividendToken;
    }
...
contract SafeEarn is IBEP20, Auth {
    ...
    uint256 _totalSupply = 29000000000 * (10 ** _decimals);
    ...
    constructor (
        ...
        WBNB = router.WETH();
        distributor = new DividendDistributor(_dexRouter, msg.sender, _nativeToken);
        nativeToken = _nativeToken;
    ...
    function swapBack() internal swapping {
        ...
        uint256 balanceBeforeNT = nativeToken.balanceOf(distributorAddress);
        ...
        uint256 amount = nativeToken.balanceOf(address(this)).sub(balanceBeforeNT);

        try distributor.deposit(amount) {} catch {}
