...
contract ADADOGE is IERC20, Ownable, Pausable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string constant public name = "ADADOGE";
    ...
    function setFeePercentage(uint8 feePercentage_) external onlyOwner {
        require(feePercentage_ <= 30, "ADADOGE: transaction fee percentage exceeds 30");
        require(feePercentage_ >= 0, "ADADOGE: transaction fee percentage equals 0");
    ...
    function blacklist(address[] calldata addresses, bool isBlacklist) external {
        require(hasBlacklistPermission[msg.sender], "ADADOGE:insufficient permission");
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "ADADOGE: transfer from blacklisted address");
        require(!isBlacklisted[recipient], "ADADOGE: transfer to blacklisted address");
        require(!isBlacklisted[tx.origin], "ADADOGE: transfer called from blacklisted address");
