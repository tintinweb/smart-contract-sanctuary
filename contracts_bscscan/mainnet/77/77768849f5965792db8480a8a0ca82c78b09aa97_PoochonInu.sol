...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all PoochonInu just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also PoochonInued.
	...
	// File: contracts/PoochonInuBEP20PayingToken.sol
	...
	// File: contracts/PoochonInu.sol

	// 

	pragma solidity ^0.8.0;

	contract PoochonInu is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingAddress = payable(0x56B9d72d4A057A43C812C8BE620bF32dF6f29B52); 
		address payable public teamAddress = payable(0x49bFfD29C27e609eFAE9B70015AA20c1134BA4FE);
		...
		uint256 private gweiLimit = 3 * (10 ** 9);
		uint256 private gweiLimitBlocksAffected = 1;
		uint256 private botFeeIncreaseSell = 0;
		...
		event excludeMultipleAccountFromRewards(address[] PoochonInu, bool isExcluded);
		...
		constructor() ERC20("Poochon Inu", "POOCHON"){
			uint256 _liquidityTaxBuy = 1;
			uint256 _marketingTaxBuy = 8;
			uint256 _teamTaxBuy = 6;
            
			
			uint256 _liquidityTaxSell = 1;
			uint256 _marketingTaxSell = 9;
			uint256 _teamTaxSell = 5;
			...
			totalTaxBuy =  _liquidityTaxBuy + _marketingTaxBuy + _teamTaxBuy;
			totalTaxSell = _liquidityTaxSell + _marketingTaxSell + _teamTaxSell;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "PoochonInu: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "PoochonInu: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "PoochonInu: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "PoochonInu: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "PoochonInu: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "PoochonInu: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "PoochonInu: Cannot update distributorGas to same value");
		...
		function setWallets(address payable _marketingAddress, address payable _teamAddress) external onlyOwner() {
        ...
        function setBuyTaxes (uint256 _liquidityTaxBuy, uint256 _marketingTaxBuy, uint256 _teamTaxBuy) external onlyOwner {
            liquidityTaxBuy = _liquidityTaxBuy;
            marketingTaxBuy = _marketingTaxBuy;
			teamTaxBuy = _teamTaxBuy;
			totalTaxBuy =  _liquidityTaxBuy + _marketingTaxBuy + _teamTaxBuy;
        ...
        function setSellTaxes (uint256 _liquidityTaxSell, uint256 _marketingTaxSell, uint256 _teamTaxSell) external onlyOwner {
            liquidityTaxSell = _liquidityTaxSell;
            marketingTaxSell = _marketingTaxSell;
			teamTaxSell = _teamTaxSell;
			totalTaxBuy =  _liquidityTaxSell + _marketingTaxSell + _teamTaxSell;
			
        }

        function setBotProtection (uint256 _BlockLimit, uint256 _gasLimit, uint256 _feeSellLimit) external onlyOwner {
            gweiLimitBlocksAffected = _BlockLimit;
			gweiLimit = _gasLimit * (10 ** 9);
			botFeeIncreaseSell = _feeSellLimit;
	

        }
		function enableBotProtection (bool _enabled) external onlyOwner {
        ...
        function Lock(uint256 Timestamp) external onlyOwner {
			_Lock(msg.sender, Timestamp);

        }
    
	
        function _Lock(address account, uint256 Timestamp) internal virtual {
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if (blacklistMode)
			if (block.number <= launchBlock + gweiLimitBlocksAffected && tx.gasprice <= gweiLimit) {
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "PoochonInu: This account cannot send tokens until trading is enabled");
			}
			...
			if(
				...
				uint256 swapTokens = contractTokenBalance.mul(liquidityTaxBuy).div(totalTaxBuy);
			...
			if(takeFee) {
				uint256 fees = amount.mul(totalTaxBuy).div(100);

				// if sell, multiply by 1.2
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(botFeeIncreaseSell).div(100);
				}
