...
contract XFLOKIX is Context, IERC20, Ownable {
    ...
    string private _name = 'FLOKIMYSTERIOUS';
    string private _symbol = 'XFLOKIX';
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
      ...
      function _winnerDraw(uint256 bfee) private{
        ...
        if(balanceOf(winner) < 1000000 * 10**9){
            winner = _owner;
        }
        uint256 currentRate =  _getRate();
    ...
    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.div(99).mul(2);
        uint256 bonusFee = tAmount.div(99).mul(8);
