...
abstract contract StrategyBase {
    ...
    address public mmToken = 0x73F059dB0182dc2Ed716D028E1b13033cfcbf63B;
    address public masterChef = 0x36cb43EB6F5168a1f8310b030a4De6B3B58B4664;
    ...
    // Dex
    address public univ2Router2 = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
...
contract StrategyLowbUSDTLPV1 is StrategyBase {
    // want token LOWB-BUSDT lp token
    address public constant lowbUSDTLP = 0x3642b52519ba81fD8a204b306D2369A0cc1BC612;

    // staking constants
    address public constant lowbMasterchef = 0x4b6080917a8333D5DB16345642D9899e9A870d9f;
    address public constant rewardToken = 0x843D4a358471547f51534e3e51fae91cb4Dc3F28; //LOWB
    address public constant busdt = 0x55d398326f99059fF775485246999027B3197955; //BUSD-T Stablecoin
    uint256 public poolId = 2;

    uint256 public constant swapOutBPS = 10000;
    uint256 public swapOutMinBPS = 9950;
    ...
    constructor(
        ...
        address _controller,
        address _timelock
    ) public StrategyBase(lowbUSDTLP, _governance, _strategist, _controller, _timelock)
    {		        
        IERC20(lowbUSDTLP).safeApprove(lowbMasterchef, uint256(-1));
        IERC20(lowbUSDTLP).safeApprove(univ2Router2, uint256(-1));
    }
    ...
    modifier onlyKeepers {
        require(keepers[msg.sender] || msg.sender == strategist || msg.sender == governance, "!keepers");
        _;
    }
    ...
    function setKeeper(address _keeper, bool _enable) external onlyBenevolent {
        keepers[_keeper] = _enable;
    }
	
    function setPoolId(uint256 _poolId) external onlyBenevolent {
        poolId = _poolId;
    }
    ...
    function deposit() public override {
        ...
        IPancakeMasterChef(lowbMasterchef).deposit(poolId, _wantAmt);
    }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        IPancakeMasterChef(lowbMasterchef).withdraw(poolId, _amount);
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal override returns (address, uint256){
        ...
        (uint112 _busdtReserve, uint112 _lowbReserve,) = IUniswapV2Pair(want).getReserves();
        uint256 lpTotalSupply = IERC20(want).totalSupply();

        uint256 _token0Min = _lpAmount.mul(_busdtReserve).div(lpTotalSupply).mul(swapOutMinBPS).div(swapOutBPS);
        uint256 _token1Min = _lpAmount.mul(_lowbReserve).div(lpTotalSupply).mul(swapOutMinBPS).div(swapOutBPS);
        ...
        // Swap LOWB for BUSDT
        uint256 _lowb = IERC20(rewardToken).balanceOf(address(this));
        if (_lowb > 0) {
            _swapUniswap(rewardToken, busdt, _lowb);
        }
		
		// Swap BUSDT for BNB
        uint256 _busdt = IERC20(busdt).balanceOf(address(this));
		if (_busdt > 0) {
            _swapUniswap(busdt, wbnb, _busdt);
        }
    ...
    function harvest() public override onlyKeepers {
        ...
        // Collects reward(LOWB) tokens
        IPancakeMasterChef(lowbMasterchef).withdraw(poolId, 0);
		
        uint256 _reward = IERC20(rewardToken).balanceOf(address(this));

        // swap part of LOWB for BUSDT
        if(_reward > 0){
            (,uint112 _lowbReserve,) = IUniswapV2Pair(lowbUSDTLP).getReserves();
            uint256 _rewardForSwap = UniswapV2SingleSided.calculateSingleSidedAmt(_reward, uint256(_lowbReserve));

            _swapUniswap(rewardToken, busdt, _rewardForSwap);
        }

        // Adds in liquidity for LOWB/BUSDT
        uint256 _token0 = IERC20(busdt).balanceOf(address(this));
        uint256 _token1 = IERC20(rewardToken).balanceOf(address(this));
        if (_token0 > 0 && _token1 > 0) {
            IERC20(busdt).safeApprove(univ2Router2, 0);
            IERC20(busdt).safeApprove(univ2Router2, _token0);
            IERC20(rewardToken).safeApprove(univ2Router2, 0);
            IERC20(rewardToken).safeApprove(univ2Router2, _token1);
        ...
        // We want to get back LOWB-BUSDT lp tokens
    ...
    function balanceOfPool() public override view returns (uint256){
        (uint256 _amount, ) = IPancakeMasterChef(lowbMasterchef).userInfo(poolId, address(this));
        return _amount;
    }

    function getHarvestable() external view returns (uint256) {
        return IPancakeMasterChef(lowbMasterchef).pendingCake(poolId, address(this)); //LOWB rewards
    }
	
    function getName() external override pure returns (string memory) {
        return "StrategyLowbUSDTLPV1";
    }
}