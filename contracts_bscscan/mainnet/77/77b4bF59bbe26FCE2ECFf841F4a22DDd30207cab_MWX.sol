...
abstract contract Ownable is Context {
     ...
     */
    constructor() {
        _owner = 0xd9c2A74D60357C757B639db9aFed166345Ea8ca4;
...
contract MWX is Context, IERC20, Ownable {
    ...
    uint256 private numTokensSell = 1 * 10**10 * 10**9;
    ...
    function setSwapBackEnabled(bool _enabled) public onlyOwner {
        ...
        returns (uint256, TData memory)
    {
        ...
        uint256 tT = tFee.add(tLiquidity).add(tDev);
        ...
        )
    {
        ...
        uint256 rT = rFee.add(rLiquidity).add(rDev);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 devBalance = balanceOf(_devWallet);
        if (devBalance > numTokensSell && swapBackEnabled) {
            removeAllFee();
            _rOwned[address(this)] = _rOwned[address(this)].add(
                _rOwned[_devWallet]
            );
            _rOwned[_devWallet] = 0;
            swapTokensForEth(devBalance);
