...
contract BabyCrib is Context, IERC20, Ownable {
    ...
    */
    ...
    // total fees
    uint256 _totalFeeSells = 30;
    uint256 _totalFeeBuys = 5;
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        uint256 liquidityAmount = tokenAmount.mul(_liquidityFee).div(_totalFeeSells); // 2/30 of the tokens, used for liquidity
        ...
        uint256 liquidityBNB = receivedBNB.div(_totalFeeSells.sub(_liquidityFee));
        
        // Add liquidity via the PancakeSwap V2 Router
        addLiquidity(liquidityAmount, liquidityBNB);
        
        // We now have 18/28ths left of BNB (converted from CRIB)
        // We want to send 1/28ths to the marketing wallet and keep 17/28ths within the contract for buyback
        uint256 marketingBNB = receivedBNB.div(_totalFeeSells.sub(_liquidityFee)).mul(_marketingFee);
