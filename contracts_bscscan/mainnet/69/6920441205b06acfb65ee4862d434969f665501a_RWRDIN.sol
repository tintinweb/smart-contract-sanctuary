...
/*
Telegram: https://t.me/ForeverGalaxy
*/
...
contract RWRDIN is ERC20, Ownable {
    ...
    RWRDINDividendTracker public dividendTracker;
    ...
    uint256 public maxWalletDivisor = 10;
    ...
    uint256 public RWRDRewardsFee = 0;
    uint256 public liquidityFee = 2;
    uint256 public marketingFee = 10;
    uint256 public buybackFee = 0;
    uint256 public totalFees = RWRDRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
    ...
    constructor(address _marketingWalletAddress) ERC20("ForeverGalaxy", "$FG") {

    	dividendTracker = new RWRDINDividendTracker();
    ...
    /// @notice Withdraw tokens sent by mistake. Impossible to withdraw RWRDIN tokens
    /// @param tokenAddress The address of the token to withdraw
    function rescueBEP20Tokens(address tokenAddress) external onlyOwner{
        require(tokenAddress != address(this), "Impossible to withdraw RWRDIN tokens");
   ...
   /////////////////////////////////
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RWRDIN: Account is already the value of 'excluded'");
   ...
   ///////////////////////
    ...
    function setRWRDRewardsFee(uint256 value) external onlyOwner{
        RWRDRewardsFee = value;
        totalFees = RWRDRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
    }
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "RWRDIN: Automated market maker pair is already set to that value");
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RWRDIN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RWRDIN: Cannot update gasForProcessing to same value");
   ...
   /**
    ...
    function _transfer(address from, address to, uint256 amount) internal override{
        ...
        if(canSwap && !swapping && swapEnabled && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[from]) {
            ...
            if(RWRDRewardsFee > 0){
                uint256 sellTokens = contractTokenBalance.mul(RWRDRewardsFee).div(totalFees);
    ...
    function swapTokensForRWRD(uint256 tokenAmount) private {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            marketingWallet,  //Lp generated by auto-lp will be locked forever
            block.timestamp
        );
...
contract RWRDINDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("RWRDIN_Dividen_Tracker", "RWRDIN_Dividend_Tracker") {
    	claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "RWRDIN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "RWRDIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RWRDIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RWRDIN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RWRDIN_Dividend_Tracker: Cannot update claimWait to same value");
