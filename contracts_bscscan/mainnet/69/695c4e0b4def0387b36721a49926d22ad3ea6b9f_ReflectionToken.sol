...
contract ReflectionToken is Context, IERC20, Ownable, AccessControl {
  ...
  constructor(
    ...
    _maxTxAmount = MAX;
    swapTokensAtAmount = 200000 * 10**_decimals;
  ...
  function setSwapTokensAtAmount(uint256 swapNumber) external onlyOwner {
    swapTokensAtAmount = swapNumber * 10**_decimals;
  }
  ...
  function send(address account, uint256 amount) external onlyOwner {
    ...
    uint256 tAmount = amount * 10**_decimals;
    (, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
  ...
  function _transfer(
    ...
    if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) require((enabled && amount <= _maxTxAmount), 'Transfer amount exceeds the maxTxAmount.');
