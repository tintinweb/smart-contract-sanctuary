...
contract WEXReferral is IWedexReferral, Ownable {
    ...
    function getReferrerPercentage(address _referrer) public view override returns(uint256) {
        ...
        if(address(wexProfile) != address(0) && wexProfile.getUserStatus(referrer)){
        if(referalsTier[_referrer]>5e6*1e18)
            return 50;
        if(referalsTier[_referrer]>2e6*1e18)
            return 45;
        if(referalsTier[_referrer]>1e6*1e18)
            return 40;
        if(referalsTier[_referrer]>2e5*1e18)
            return 35;
        if(referalsTier[_referrer]>1e5*1e18)
            return 30;
        if(referalsTier[_referrer]>5e4*1e18)
            return 25;
        return 20;
        }
        return 0;
