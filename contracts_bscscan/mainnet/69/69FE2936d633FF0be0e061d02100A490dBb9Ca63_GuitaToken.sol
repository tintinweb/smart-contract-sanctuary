...
// SPDX-License-Identifier: GPL-3.0-or-later Or MIT Or MIT
// File: contracts\GUITAToken.sol
 ...
 */
abstract contract IERC1363 is IERC20, ERC165 {
    /*
 ...
 *  https://github.com/ethereum/EIPs/issues/1363
 */
abstract contract IERC1363Receiver {
    /*
     ...
     *  unless throwing
     */
    function onTransferReceived(address operator, address from, uint256 value, bytes memory data) public virtual returns (bytes4); // solhint-disable-line  max-line-length
}
 ...
 *  https://github.com/ethereum/EIPs/issues/1363
 */
abstract contract IERC1363Spender {
    /*
     ...
     *  unless throwing
     */
    function onApprovalReceived(address owner, uint256 value, bytes memory data) public virtual returns (bytes4);
}
...
contract BaseERC20Token is ERC20Detailed, ERC20Capped, ERC20Burnable, OperatorRole, TokenRecover {
     ...
     */
    function mint(address to, uint256 value) public virtual override canMint returns (bool) {
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    function transfer(address to, uint256 value) public virtual override(ERC20, IERC20) canTransfer(msg.sender) returns (bool) {
...
contract BaseERC1363Token is BaseERC20Token, ERC1363 {
     ...
     * - `value` must not cause the total supply to go over the cap.
     */
    function _mint(address account, uint256 value) internal override(ERC20, BaseERC20Token){
        super._mint(account, value);
    }
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    function transfer(address to, uint256 value) public virtual override(ERC20, BaseERC20Token) canTransfer(msg.sender) returns (bool) {
        return super.transfer(to, value);
    }
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    function transferFrom(address from, address to, uint256 value) public override(ERC20, BaseERC20Token) canTransfer(from) returns (bool) {
        return super.transferFrom(from, to, value);
    }
}
...
// GuitaToken with Governance.
contract GuitaToken is ERC20Token {
    ...
    // Min amount to liquify. (default 500 GUITAs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to Guita's router when our own AMM release
    IUniswapV2Router02 public guitaRouter;
    // The trading pair
    address public guitaPair;
    // In swap and liquify
    ...
    event guitaRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "GUITA::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the GUITA Token contract.
     */
    constructor(
        string memory name,
        string memory symbol,
        ...
        uint256 initialSupply,
        bool transferEnabled
    ) public ERC20Token(name, symbol, decimals, cap, initialSupply, transferEnabled) {
    ...
    /// @dev overrides transfer function to meet tokenomics of GUITA
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        require(sender != address(0), "GUITA::sender is not valid");
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "GUITA::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "GUITA::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the guita pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = guitaRouter.WETH();

        _approve(address(this), address(guitaRouter), tokenAmount);

        // make the swap
        guitaRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        guitaRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from guitaRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "GUITA::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "GUITA::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "GUITA::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateguitaRouter(address _router) public onlyOperator {
        guitaRouter = IUniswapV2Router02(_router);
        guitaPair = IUniswapV2Factory(guitaRouter.factory()).getPair(address(this), guitaRouter.WETH());
        require(guitaPair != address(0), "GUITA::updateguitaRouter: Invalid pair address.");
        emit guitaRouterUpdated(msg.sender, address(guitaRouter), guitaPair);
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "GUITA::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "GUITA::delegateBySig: invalid nonce");
        require(now <= expiry, "GUITA::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "GUITA::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying GUITAs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "GUITA::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function addBlockAddr(address _addBlockAddr) public onlyOwner returns (bool) {
        require(_addBlockAddr != address(0), "GUITA: _addBlockAddr is the zero address");
    ...
    function delBlockAddr(address _delblockAddr) public onlyOwner returns (bool) {
        require(_delblockAddr != address(0), "GUITA: _delblockAddr is the zero address");
    ...
    function getBlockAddr(uint256 _index) public view onlyOwner returns (address){
        require(_index <= getBlockAddrLength() - 1, "GUITA: index out of bounds");
