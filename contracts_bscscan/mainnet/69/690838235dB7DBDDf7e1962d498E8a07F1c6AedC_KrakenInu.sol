...
/**
    ...
    3% Static Reflections
    4% Buy Back Fund
    3% Charity
    
    Telegram: https://t.me/krakeninu
    Website: https://www.krakeninu.com/
...
contract KrakenInu is Context, IERC20, Ownable, Initializable {
    ...
    uint256 private feeDivisor = _liquidityFee.add(_marketingFee);
    ...
    uint256 public numTokensMax = 55000 * 10 ** 9; //0.055%
    uint256 public numTokensSellToAddToLiquidity = 50000 * 10 ** 9; //0.05% 
    uint256 public maxlimit = 1000000 * 10 ** 9; //1%
    ...
    function _takeLiquidity(uint256 tLiquidity, uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 divisor = _charityFee.add(_burnFee);
        if (divisor < 1) {
            divisor = 1;
        }
        uint256 tDevelopment = tDev.div(divisor).mul(_charityFee);
    ...
    function calculateDevelopmentBurnFee(uint256 _amount) private view returns (uint256) {
        uint256 fee = _charityFee.add(_burnFee);
    ...
    function calculateLiquidityPooFee(uint256 _amount) private view returns (uint256) {
        uint256 fee = _liquidityFee.add(_marketingFee);
        return _amount.mul(fee).div(
            10**2
        );
    }
    ...
    function sendBNBToMarketing() private { 
