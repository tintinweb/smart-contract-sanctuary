...
contract TimeLock is Ownable {
    ...
    address public MTKOToken = 0x0c4fb728e7D679043e552D994E206bE1BEFe1BFB;
    address public lpToken = 0x7B30F38Ae75C1dBE2538b6ACe8Ac3E4e8c3bc2E1;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseMTKOToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(MTKOToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(MTKOToken).safeTransfer(owner(), amount);
    }
    ...
    function getMTKOTokenBalance() external view returns(uint256) {
        return IERC20(MTKOToken).balanceOf(address(this));
    }
    ...
    function setMTKOToken(address _MTKOToken) external onlyOwner {
        MTKOToken = _MTKOToken;
    }
