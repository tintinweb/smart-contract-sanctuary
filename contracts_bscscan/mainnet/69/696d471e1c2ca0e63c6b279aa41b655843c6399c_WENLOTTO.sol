...
:::::::::::::(___)::::::::(___)::::::::smd  
 ...
 TELEGRAM: WenLottoBSC
 

 1T Supply | 30% Initial Burn | 0.5% Max Tx | 2.5% Max Wallet
 
 Fees: 5% Liquidity, 4% Burn, 2% Marketing, 4% Lottery
 
 Locking Liquidity on Launch
 
 Try your luck with WENLOTTO ($LOTTO)!
 
 Automatic Lottery Draws Every 50 Transactions
...
contract WENLOTTO is Context, IBEP20, Ownable {
    ...
    address private constant _marketingWallet = 0x87A430B00ED9796D574FBC8155632cA601c5f679;

    address private constant _potAddress = 0x59bD418b10B14F029e3753d5e63F7232ca7ed746;
    ...
    string private _name     = "WenLOTTO";
    string private _symbol   = "LOTTO";
    uint8 private  _decimals = 9;

    uint256 public  _taxFee       = 0; // redistributed to holders
    uint256 public  _liquidityFee = 5; // goes into liquidity
    uint256 public  _marketingFee = 2; // marketing wallet
    uint256 public  _burnFee      = 4; // burned
    uint256 public  _potFee       = 4; // pot fees
    ...
    uint256 private _maxWalletToken = 25 * 10**9 * 10**9; // 25 Billion (2.5%)
    ...
    uint256 private transactionsPerLottery = 50;
    uint256 private _potDivider = 50;
    ...
    event LotterySkipped(address skippedAddress, uint256 prevPot, uint256 newPot, uint256 minBalanceWas, uint256 skippedBalance);
    ...
    function setTransactionsPerLottery(uint256 txCount) external onlyOwner {
        transactionsPerLottery = txCount;
    }
    
    function setPotDivider(uint256 newPotDivider) external onlyOwner {
        _potDivider = newPotDivider;
    }
    ...
    function _transfer(
        ...
        require(!_isSniper[to], "Go snipe some other token");
        require(!_isSniper[msg.sender], "Go snipe some other token");
    ...
    function random(uint256 _totalPlayers) private view returns (uint256) {
        ...
        uint256 salty = block.number.add(_txCounter).add(_totalPlayers);
    ...
    function _handleLottery(address recipient, uint256 potContribution, uint256 currentRate) private returns (bool) {
        ...
        if (transactionsSinceLastLottery > 0 && transactionsSinceLastLottery.mod(transactionsPerLottery) == 0) {
            ...
            uint256 _minBalance = _pot
                .div(_potDivider) // You stand to win 50x.
                .div(transactionsSinceLastLottery.div(transactionsPerLottery));

            if (!_isSniper[_winnerAddress] && _balanceWinner >= 0 && _balanceWinner >= _minBalance) {
                ...
                emit LotterySkipped(_winnerAddress, _pot.sub(potContribution), _pot, _minBalance, _balanceWinner);
            }
        }
