...
// File: contracts/interfaces/IAlitaSwapFactory.sol

pragma solidity >=0.5.0;

interface IAlitaSwapFactory {
...
// File: contracts/interfaces/IAlitaSwapPair.sol

pragma solidity >=0.5.0;

interface IAlitaSwapPair {
...
// File: contracts/interfaces/IAlitaSwapERC20.sol

pragma solidity >=0.5.0;

interface IAlitaSwapERC20 {
    event Approval(address indexed owner, address indexed spender, uint value);
    event Transfer(address indexed from, address indexed to, uint value);
...
// File: contracts/AlitaSwapERC20.sol
...
// File: contracts/interfaces/IAlitaSwapCallee.sol
...
// File: contracts/AlitaSwapPair.sol
...
contract AlitaSwapPair is IAlitaSwapPair, AlitaSwapERC20 {
    using SafeMath  for uint;
    ...
    modifier lock() {
        require(unlocked == 1, 'AlitaSwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    modifier allowSwap(){
        require(now >= startingSwapTime, 'AlitaSwap: not allow swap');
        _;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'AlitaSwap: TRANSFER_FAILED');
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'AlitaSwap: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IAlitaSwapFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'AlitaSwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'AlitaSwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock allowSwap {
        require(amount0Out > 0 || amount1Out > 0, 'AlitaSwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'AlitaSwap: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'AlitaSwap: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IAlitaSwapCallee(to).AlitaSwapCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'AlitaSwap: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(2));
        uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(2));
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'AlitaSwap: K');
        }
...
// File: contracts/AlitaSwapFactory.sol
