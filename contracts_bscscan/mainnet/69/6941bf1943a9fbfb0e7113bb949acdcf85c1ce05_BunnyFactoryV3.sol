...
contract BunnyFactoryV3 is Ownable {
    ...
    uint8 private  numberBunnyIds = 10;

    // number of previous series (i.e. different visuals)
    uint8 private  previousNumberBunnyIds = 5;
    
      
    // Map the token max number to id
    mapping(uint8 => uint8) private bunnyNumber;
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        require(_bunnyId < numberBunnyIds, "bunnyId too high");
         // Check that the _bunnyId is within boundary:
         require(bunnyNumber[_bunnyId] < 200, "over bunnyNumber");
         
         bunnyNumber[_bunnyId]=bunnyNumber[_bunnyId]+1;
    ...
    function claimFee(uint256 _amount) external onlyOwner {
      ...
      function updateipfsHash(string memory _ipfsHash) external onlyOwner {
       ...
       function updateNumberBunnyIds(uint8   _numberBunnyIds) external onlyOwner {
        numberBunnyIds=_numberBunnyIds;
    }
    
        function updatePreviousNumberBunnyIds(uint8   _previousNumberBunnyIds) external onlyOwner {
          previousNumberBunnyIds=_previousNumberBunnyIds;
    }
    
    
       function updatebunnyNumber(uint8   _bunnyId,uint8 _numberBunnyId) external onlyOwner {
          bunnyNumber[_bunnyId]=_numberBunnyId;
    }
    
    function queryBunnyNumber(uint8 _bunnyId) external view returns (uint8 _numberBunnyId) {
      
            return bunnyNumber[_bunnyId];
         
    }
    
      function querybunnyUri(uint8 _bunnyId) external view returns (string memory _bunnyIdJson) {
      
            return bunnyIdURIs[_bunnyId];
         
    }
    
      
    
     function updatebunnyUri1(uint8   _bunnyId,string calldata _bunnyIdJson) external onlyOwner {
          bunnyIdURIs[_bunnyId]=_bunnyIdJson;
    }
     
       function updatebunnyUri2(uint8 _bunnyId,string calldata _bunnyIdJson) external onlyOwner {
