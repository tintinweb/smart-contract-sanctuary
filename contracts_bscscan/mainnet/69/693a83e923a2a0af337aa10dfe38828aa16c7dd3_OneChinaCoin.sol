...
contract Ownable is Context {
    ...
    constructor() {
        ...
        _owner = 0x3d23e0B07c23632387e2A4f41bD675F53d6E15D6;
...
contract OneChinaCoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "OneChinaCoin";
    ...
    uint256 internal _tokenTotal = 1e18;
    ...
    uint256 public _taxFee = 500; // means 5% which distribute to all holders reflection fee
    uint256 public _treasuryFee = 100;// means 1% bnb to treasury Wallet
    ...
    address treasuryAddress=0x624444aa386d8120B68ed04BE9A290ceAF153d3F;//dummy addresss Bnb 
    ...
    uint256 public minTokensBeforeSwap = 100_000e15;
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_liquidityFee != 0){
         ...
         if(_treasuryFee != 0){
            uint256 buyBackFee = amount.mul(_treasuryFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(buyBackFee);
            _reflectionBalance[treasuryAddress] = _reflectionBalance[treasuryAddress].add(buyBackFee.mul(rate));
            if (_isExcluded[treasuryAddress]) {
                _tokenBalance[treasuryAddress] = _tokenBalance[treasuryAddress].add(buyBackFee);
            }
            emit Transfer(account,treasuryAddress,buyBackFee);
        }
    ...
    function setTresuryFee(uint256 fee) external onlyOwner {
        _treasuryFee = fee;
    }
    ...
    function settreasuryAddress(address add) external onlyOwner{
        treasuryAddress=add;
    }
