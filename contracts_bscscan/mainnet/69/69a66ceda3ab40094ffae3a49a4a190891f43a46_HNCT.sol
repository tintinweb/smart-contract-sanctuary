...
// HNCT - A BSC Token, https://helpncare.co
...
struct TaxCalculationParameters {
    ...
    uint256 communityReflectionValue;
    uint256 operatingCostValue;
    uint256 operatingCostReflectionValue;
...
contract HNCT is IERC20, Ownable {
    ...
    string public constant name = "HNC Token v2";
    string public constant symbol = "HNCT";
    uint8 public constant decimals = 9;
    uint256 public constant TOTAL_SUPPLY = 10e17;
    uint256 private constant _MAX_UINT = ~uint256(0);
    uint8 private constant _HUNDRED_PERCENT = 100;
    uint256 private _TOTAL_REFLECTION = (_MAX_UINT -
        _MAX_UINT.mod(TOTAL_SUPPLY));
    ...
    uint256 internal tokenLiquidityThreshold = 50e13;
    ...
    event VaultsUpdated(address communityVault, address operatingCostVault);
    ...
    constructor() {
        ...
        uint256 tokensToBurn = _stats.totalTokens.mul(0).div(100);
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "HNCT: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        require(
            ...
            "HNCT: approve from non-zero to non-zero allowance"
        );
        ...
        returns (bool)
    {
        ...
        require(
            oldValue >= value,
            "HNCT: cannot decrease allowance below zero"
        );
    ...
    function setVaultsAddresses(address community, address operatingCost)
        external
        onlyOwner
    {
        Vaults storage vaults = _vaults;

        vaults.communityVault = community;
        vaults.operatingCostVault = operatingCost;

        _exemptions[vaults.communityVault].isExcluded = true;
        _exemptions[vaults.operatingCostVault].isExcluded = true;

        emit VaultsUpdated(community, operatingCost);
    }
    ...
    function updateAccountExclusionState(
        ...
        emit AccountExclusionStateChanged(
            account,
            excludeFromReward,
            excludeFromFees
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "HNCT: amount must be less than total supply"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "HNCT: amount has to be less or equal to total reflection"
        );
        ...
        returns (CalculationParameters memory, TaxCalculationParameters memory)
    {
        ...
        taxParams = isTakingFees
            ? calculateTaxes(_taxRates, tokenAmount, rate)
            : taxParams;
    ...
    function calculateTaxes(
        ...
        params.instantBoostValue = tokenAmount.mul(taxes.instantBoost).div(
            _HUNDRED_PERCENT
        );
        params.instantBoostReflectionValue = params.instantBoostValue.mul(rate);

        params.communityValue = tokenAmount.mul(taxes.community).div(
            _HUNDRED_PERCENT
        );
        params.communityReflectionValue = params.communityValue.mul(rate);

        params.operatingCostValue = tokenAmount.mul(taxes.operatingCost).div(
            _HUNDRED_PERCENT
        );
        params.operatingCostReflectionValue = params.operatingCostValue.mul(
            rate
        );

        params.liquidityValue = tokenAmount.mul(taxes.liquidity).div(
            _HUNDRED_PERCENT
        );
        ...
        params.tokenTaxSum = tokenAmount.mul(taxes.totalTaxRate).div(
            _HUNDRED_PERCENT
        );
    ...
    function extendedTransferStandard(
        ...
        ) = calculateValues(amount, takeFees);
    ...
    function collectVaultTaxes(
        ...
        _balances[_vaults.communityVault].tokens = _balances[
            _vaults.communityVault
        ]
        .tokens
        .add(params.communityValue);
        _balances[_vaults.communityVault].reflection = _balances[
            _vaults.communityVault
        ]
        .reflection
        .add(params.communityReflectionValue);
        stats.totalExcludedReflection = _stats.totalExcludedReflection.add(
            params.communityReflectionValue
        );
        stats.totalExcludedTokens = _stats.totalExcludedTokens.add(
            params.communityValue
        );

        emit Transfer(sender, _vaults.communityVault, params.communityValue);
        emit VaultDistribution(_vaults.communityVault);

        _balances[_vaults.operatingCostVault].tokens = _balances[
            _vaults.operatingCostVault
        ]
        .tokens
        .add(params.operatingCostValue);
        _balances[_vaults.operatingCostVault].reflection = _balances[
            _vaults.operatingCostVault
        ]
        .reflection
        .add(params.operatingCostReflectionValue);
        stats.totalExcludedReflection = _stats.totalExcludedReflection.add(
            params.operatingCostReflectionValue
        );
        stats.totalExcludedTokens = _stats.totalExcludedTokens.add(
            params.operatingCostValue
        );
        emit Transfer(
            sender,
            _vaults.operatingCostVault,
        ...
        emit VaultDistribution(_vaults.operatingCostVault);
    }
