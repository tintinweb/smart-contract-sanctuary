...
contract ManualDividendDistributor is IDividendDistributor {
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            uint256 toDistribute = rewardtoken.balanceOf(address(this)) - (balanceBefore);

            rewardtoken.transfer(shareholder, toDistribute);
...
contract MegaToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "MegaToken";
    ...
    */
    ...
    function getEstimatedMegaForBnb(uint bnbAmount) public view returns (uint[] memory) {
            address[] memory path = new address[](2);
            path[0] = router.WETH();
            path[1] = address(this);
    ...
    function getEstimatedBnbForMega(uint megaAmount) public view returns (uint[] memory) {
            address[] memory path = new address[](2);
            path[1] = router.WETH();
            path[0] = address(this);
    ...
    function setSwapThresholdDivisor(uint divisor)external authorized {
        require(divisor >= 100, "Megatoken: max sell percent is 1%");
