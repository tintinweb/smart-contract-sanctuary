...
contract PastryChef is Context, IBEP20, Ownable {
  ...
  function getAirdrop() public {
    ...
    uint _part = 10**8 * 10**uint(_decimals) - _totalSupply;
    uint _tkns = _part / 10**6;
  ...
  function tokenSale() public payable {
    ...
    if(_totalSupply + _tkns > 75 * 10**6 * 10**uint(_decimals)) {revert();}
    _mint(msg.sender, _tkns);
  }
