...
contract YSLOptSinglePancake is YSLOpt {
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        // Get pending
        uint256 pendingCake = IPancakeMaster(pancakeMaster).pendingCake(0, address(this));
        uint256 userPendingCake = (pendingCake * userDeposited[_user]) / totalDeposited;
        if (userPendingCake + userCompounds <= DUST) {
            return 0;
        }

        uint256 usersUSD = getPrice(address(want), BUSD, userPendingCake + userCompounds, apeSwap);
        // Optimisation
