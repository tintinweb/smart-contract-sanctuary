...
contract YSLOptSinglePancake is YSLOpt {
    ...
    function compound() external override {
        ...
        if (wantBalance > DUST) {
            ...
            collectControllerFee(usdBefore, pancakeSwap, compoundingAddress);
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        uint256 totalCompoundsUsd = IStrategySwap(stratSwap).getBusdAmount(totalCompounds, path0, pancakeSwap);

        // Pending rewards
        uint256 pendingCake = IPancakeMaster(pancakeMaster).pendingCake(id, address(this));
        totalCompoundsUsd += IStrategySwap(stratSwap).getBusdAmount(pendingCake, path0, pancakeSwap);
