...
/**
 * TG = https://t.me/PuffDoge
 * https://www.puffdoge.com/
*/
...
contract PuffDoge is Context, IERC20, Ownable {
    ...
    string private _name = "PuffDoge";
    string private _symbol = "PuffDoge";
    ...
    event SwapAndRewards(
        uint256 tokensSwapped,
        uint256 ethReceived,
        uint256 tokensIntoRewards
    );
    ...
    function showRewardsaddress() public view returns(address payable) {
        return _RewardsAddress;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusRewardsFee(tAmount);
    ...
    function calculateLiquidityPlusRewardsFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _RewardsFee + _marketingAndDevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingAndDevBudget == 0 && _RewardsFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingAndDevBudget + _liquidityFee + _RewardsFee;
        ...
        // calculate the portions of the liquidity to add to Rewards
        uint256 RewardsBalance = newBalance.div(totalLiqFee).mul(_RewardsFee);
        uint256 RewardsPortion = otherHalf.div(totalLiqFee).mul(_RewardsFee);
        ...
        uint256 finalBalance = newBalance.sub(RewardsBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(RewardsPortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _RewardsAddress.call{value: RewardsBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, RewardsPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, RewardsPortion);
        } else {
            addLiquidity(RewardsPortion, RewardsBalance, _RewardsAddress);
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address Rewards) private {
