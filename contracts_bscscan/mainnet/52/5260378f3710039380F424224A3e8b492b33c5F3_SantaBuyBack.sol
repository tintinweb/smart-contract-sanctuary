...
contract SantaBuyBack is Context, Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	using Address for address;

	IPancakeRouter02 public immutable pancakeRouter;
	address public immutable santaDogeContract;
	uint256 private burnedAmount;
	
	event BuyBackAndBurnEvent(
		uint256 bnbValue,
		uint256 santaDogeValue
	);
	
	constructor (
		address payable routerAddress,
		address _santaDogeContract
	) public {
		IPancakeRouter02 _pancakeRouter = IPancakeRouter02(routerAddress);
		pancakeRouter = _pancakeRouter;
		santaDogeContract = _santaDogeContract;
	}
	...
	function getSantaDogeBalance() public view returns (uint256) {
		return IBEP20(santaDogeContract).balanceOf(address(this));
	}
	...
	function buyback(uint256 amount) public onlyAuthorizedCallers {
		if (amount == 0) return;
		uint256 maxToBuyBack = address(this).balance;
		if (amount > maxToBuyBack) {
			amount = maxToBuyBack;
		}
		uint256 initialBalance = IBEP20(santaDogeContract).balanceOf(address(this));
		_swapETHForTokens(santaDogeContract,address(this),amount);
		uint256 deltaBalance = IBEP20(santaDogeContract).balanceOf(address(this)).sub(initialBalance);
		// BURN
		bool sent = IBEP20(santaDogeContract).transfer(address(0x000000000000000000000000000000000000dEaD),deltaBalance);
		require(sent, "Error: Cannot BURN TOKEN");
		burnedAmount = burnedAmount+deltaBalance;
		emit BuyBackAndBurnEvent(amount,deltaBalance);
	}
