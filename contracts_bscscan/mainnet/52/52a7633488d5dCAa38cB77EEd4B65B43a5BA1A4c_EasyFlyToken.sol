...
Lançamento dia 16.08.2021 as 21:00 Horas de Brasilia (-3 UTC)
...
- >> Stake para Holders << - 
	   5% de todas as transações são redistribuídas para todos os holdes EasyFly Isso significa que 
    você pode ganhar uma renda passiva simplesmente segurando EasyFly em sua carteira.
  
- >> Stake para Markenting << - 
	   5% de todas as transações são enviadas para carteira de Markenting EasyFly Isso significa que 
    poderemos investir um valor bom em Markenting.

- >> LP << - 
	   5% de cada negociação contribui para a geração automática de mais liquidez para a comunidade EasyFly

- >> Anti=Baleia << - 
	   A quantidade máxima de tokens em qualquer carteira (endereço) é de 0,8% ( 8.000.000 ) do fornecimento total.

- >> Anti-Pump e Anti-Dump<< - 
	   A quantidade máxima de compra e venda de token é de 0,2% ( 2.000.000 ) do fornecimento total por transação.	   
*/
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x5C3c516F91c5DA990EC3AEf23e1f8b04B098ADDf;
    ...
    modifier onlyOwner() {
        require(owner() == _msgSender(), "Ownable: o chamador nao e o dono");
        _;
    }
...
contract EasyFlyToken is Context, IERC20, Ownable {
    ...
    address private _marketingWalletAddress = 0x87CBB0dD55eF6D40FEd30B44Aff35844300C46dB;
    ...
    string private _name = "EasyFlyToken";
    ...
    uint256 public _maxWalletToken = 8000000 * 10**18;
...
//      IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    function includeInReward(address account) external onlyOwner() {
        require(_isExcluded[account], "A conta ja esta incluida");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && to != address(1)) {
            require(amount <= _maxTxAmount, "O valor da transferencia excede o quantidade maxima de Tx.");
        }
        
        if(from != owner() && to != owner() && to != address(1) && to != uniswapV2Pair){
            uint256 contractBalanceRecepient = balanceOf(to);
            require(contractBalanceRecepient + amount <= _maxWalletToken, "Excede o valor maximo do token da carteira (8.000.000)"); 
        }
