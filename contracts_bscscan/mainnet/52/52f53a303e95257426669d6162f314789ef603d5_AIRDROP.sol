...
Whitelist the contract address for your token! 

Make contract address limit exempt and exclude from fees
...
contract AIRDROP is Ownable{
    ...
    */
    ...
    function UpdateReflections(address random_Token_Address, uint8 Decimals, address[] calldata Wallets)  external onlyOwner(){

        // SEND ENOUGH TOKEN TO THE CONTRACT FIRST!!!

        require(Wallets.length <= 500, "Limit sending to 500 to reduce errors"); 

        for (uint i=0; i < Wallets.length; i++) {
            IERC20(random_Token_Address).transfer(Wallets[i], 1*10**Decimals);
        }
    }
