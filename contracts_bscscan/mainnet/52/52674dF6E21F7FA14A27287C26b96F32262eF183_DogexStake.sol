...
contract DogexStake is Ownable, Pausable {
    ...
    function deposit(uint256 pid, uint256 amount) external whenNotPaused updateReward(pid) {
    ...
    function withdraw(uint256 pid, uint256 amount) public updateReward(pid) {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
    ...
    function claim(uint256 pid) public updateReward(pid) {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
    ...
    function claimable(uint256 pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingRewards);
    }
