...
contract QuickDraw is IBEP20, Ownable {
    ...
    /**
     * @notice Send the marketing fee to the marketing wallet in bnb
     *
     * @param marketingFee the amount of stadnard tokens to be sold into bnb and sent to the marketing wallet
     */
    function _takeMarketingFee(uint256 marketingFee) private {

        _rOwned[address(this)] += marketingFee * _getRate();
        _tOwned[address(this)] += marketingFee;

        // Sell tokens into bnb and transfer to the winner
        _swapTokensForBNB(marketingFee, _marketingWallet);
