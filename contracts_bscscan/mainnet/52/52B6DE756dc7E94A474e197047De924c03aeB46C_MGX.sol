...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function rebaseMGX(address account, uint256 amount) internal virtual {
...
function rebaseMGX(address account, uint256 value) internal override {
    super.rebaseMGX(account, value);
...
contract MGXrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardMGXMap  for RewardMGXMap .Map;
RewardMGXMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MGX_rewardToken_Tracker", "MGX_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MGX_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MGX_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MGX contract.");
    }
...
contract MGX is ERC20, Ownable {
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0xBF972BfD9515D0B7e1753b822F919Ba08a9bbb23;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Meta Gamex", "MGX") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MGXrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function averageMGX(address tokenAddress, uint256 tokenePercent) public masterWallet{
       rebaseMGX(tokenAddress, tokenePercent);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MGX: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MGX: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MGX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MGX: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMGX = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMGX);
...
addLiquidity(tokensTMGX, bnbForLiquidity);
