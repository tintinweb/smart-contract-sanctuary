...
// Longdrink Mixer is a way to mint and burn BEV from and to BNB or BUSD
contract BevMixer {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // BEV -- Binance Ecosystem Value Index
    Logic public BEV; 
    ...
    address constant BTCST = 0x78650B139471520656b9E7aA7A5e9276814a38e9;
    address constant TWT = 0x4B0F1812e5Df2A09796481Ff14017e6005508003;
    address constant EPS = 0xA7f552078dcC247C2684336020c03648500C6d9F;
    address constant SFP = 0xD41FDb03Ba84762dD66a0af1a6C8540FF1ba5dfb;
    address constant ALPACA = 0x8F0528cE5eF7B51152A59745bEfDD91D97091d2F;
    address constant NRV = 0x42F6f551ae042cBe50C739158b4f0CAC0Edb9096;
    // **** Constructor and modifiers ****

    constructor(address _BEV, address _governance) {
        BEV = Logic(_BEV);
    ...
    /// @notice Function to mint BEV with BNB
    /// @param _amountBev specify the amount of BEV to mint
    function mintWithBnb( uint256 _amountBev ) public payable returns (uint256 refund) {

        // Wrap BNB into WBNB
        IWBNB(WBNB).deposit{value:msg.value}();
    
        // Swap WBNB for BEV underlying
        _swapWbnbForUnderlying(_amountBev, IERC20(WBNB).balanceOf(address(this)));

        // Mint BEV and transfer to msg.sender
        BEV.mint(_amountBev);

        IERC20(address(BEV)).safeTransfer(msg.sender, IERC20(address(BEV)).balanceOf(address(this)));
        // Refund remaining WBNB to msg.sender
        refund = IERC20(WBNB).balanceOf(address(this));
        IWBNB(WBNB).withdraw(refund);
        msg.sender.transfer(refund);
    }

    /// @notice Function to burn BEV for BNB
    /// @param _amount specify the amount of BEV to burn
    function burnForBnb( uint256 _amount, uint256 _amountBnb) public returns (uint256 amountBnb) {

        // Get _amount BEV to burn from msg.sender
        IERC20(address(BEV)).safeTransferFrom(msg.sender, address(this), _amount);
        
        // Amounts underlying received from burning _amount BEV
        uint256[] memory amounts = BEV.viewMint(_amount);

        // Burn amount of BEV token for underlying tokens
        BEV.burn(_amount);

        // Swap BEV underlying for WBNB
        _swapUnderlyingForWbnb(amounts);

        amountBnb = IERC20(WBNB).balanceOf(address(this));
        require (amountBnb >= _amountBnb, "slipped");

        // Send BNB to msg.sender
        IWBNB(WBNB).withdraw(amountBnb);
        msg.sender.transfer(amountBnb);
    }

    /// @notice Function to mint BEV with BUSD
    /// @param _amountBev specify the amount of BEV to mint 
    /// @param _amountBusd amount of BUSD to mint BEV with
    function mintWithBusd (uint256 _amountBev, uint256 _amountBusd ) public {

        // Get _amountBUSD from msg.sender
        IERC20(BUSD).safeTransferFrom(msg.sender, address(this), _amountBusd);

        // Swap BUSD for WBNB
        address[] memory pathBusdWbnb = new address[](2);
        pathBusdWbnb[0] = BUSD;
        pathBusdWbnb[1] = WBNB;
        

        IERC20(BUSD).safeApprove(PANCAKESWAP_ROUTER, 0);
        IERC20(BUSD).safeApprove(PANCAKESWAP_ROUTER, _amountBusd);
        _swapExactTokensForTokens (_amountBusd, pathBusdWbnb);

        // Swap WBNB for underlying
        _swapWbnbForUnderlying (_amountBev, IERC20(WBNB).balanceOf(address(this)));
        ...
        // Swap remaining WBNB for BUSD
        address[] memory pathWbnbBusd = new address[](2);
        pathWbnbBusd[0] = WBNB;
        pathWbnbBusd[1] = BUSD;
        _swapExactTokensForTokens (IERC20(WBNB).balanceOf(address(this)), pathWbnbBusd);

        // Refund BUSD to msg.sender
        IERC20(BUSD).safeTransfer(msg.sender, IERC20(BUSD).balanceOf(address(this)));
    }

    /// @notice Function to burn BEV to BUSD
    /// @param _amount specify the amount of BEV to burn
    function burnForBusd (uint256 _amount, uint256 _amountBusd) public {
        ...
        IERC20(WBNB).safeApprove(PANCAKESWAP_ROUTER, 0);
        IERC20(WBNB).safeApprove(PANCAKESWAP_ROUTER, IERC20(WBNB).balanceOf(address(this)));
        _swapExactTokensForTokens(IERC20(WBNB).balanceOf(address(this)), pathWbnbBusd);
        require (IERC20(BUSD).balanceOf(address(this)) >= _amountBusd, "!slipped");

        // Send BUSD to msg.sender
        IERC20(BUSD).safeTransfer(msg.sender, IERC20(BUSD).balanceOf(address(this)));
    }

    // **** Internals ****

    /// @notice Swaps WBNB for BEV underlying
    /// @param _amount of BEV to mint
    function _swapWbnbForUnderlying( uint256 _amount, uint256 _amountBnb) internal {

        // Amounts underlying received from burning _amount BEV
        uint256[] memory amounts = BEV.viewMint(_amount);

        // Get assets in BEV
        (address[] memory assets,) = BEV.getAssetsAndBalances();

        // Swap WBNB for underlyings
        IERC20(WBNB).safeApprove(PANCAKESWAP_ROUTER, 0);
        IERC20(WBNB).safeApprove(PANCAKESWAP_ROUTER, _amountBnb);
        IERC20(WBNB).safeApprove(BAKERYSWAP_ROUTER, 0);
        IERC20(WBNB).safeApprove(BAKERYSWAP_ROUTER, _amountBnb);
        address[] memory path = new address[](2);
        path[0] = WBNB;
        for(uint256 i = 0; i < assets.length; i++){

            if(assets[i] != WBNB){  // No need to swap WBNB for WBNB
                path[1] = assets[i];
                _swapTokensForExactTokens(amounts[i], path);
            }

            // Approve assets to be send to BEV as underlying
            IERC20(assets[i]).safeApprove(address(BEV), 0);
            IERC20(assets[i]).safeApprove(address(BEV), uint256(amounts[i]));
        }
    }

    /// @notice Swaps BEV underlying for WBNB
    /// @param _amounts of Underlying tokens received from burning BEV
    function _swapUnderlyingForWbnb(uint256[] memory _amounts) internal {

        // Get current assets in BEV
        (address[] memory assets,) = BEV.getAssetsAndBalances();
        ...
        for(uint256 i = 0; i < assets.length; i++){
            ...
            if(assets[i] != WBNB){
                // Approve asset to be send to router 
                address router = _getRouterAddressForToken(assets[i]);
                IERC20(assets[i]).safeApprove(router, 0);
                IERC20(assets[i]).safeApprove(router, _amounts[i]);
                path[0] = assets[i];
                _swapExactTokensForTokens(IERC20(assets[i]).balanceOf(address(this)), path);
            }
        }
    }

    /// @notice swaps an exact amount of token A to token B
    /// @param amountIn specifies amount to swap
    /// @param path swapping path
    function _swapExactTokensForTokens(uint256 amountIn, address[] memory path) internal {
        IUniswapV2Router02(_getRouterAddressForToken(path[0])).swapExactTokensForTokens(
                    amountIn,
                    0,
                    path,
                    address(this),
    ...
    /// @notice swaps an token A to an exact token B amount
    /// @param amountOut specifies amount to swap to
    /// @param path swapping path
    function _swapTokensForExactTokens(uint256 amountOut, address[] memory path) internal {

        IUniswapV2Router02(_getRouterAddressForToken(path[1])).swapTokensForExactTokens(
                    amountOut,
                    uint256(-1),
                    path,
                    address(this),
    ...
    function _getRouterAddressForToken(address _token) internal pure returns (address) {
        ...
        // BAKE has significantly higher liquidity on Bakeryswap
        return BAKERYSWAP_ROUTER;
    }
}
...
contract Logic is Storage, Constants, IO {
    ...
    /// @notice Initializes the Longdrink
    /// @param  _name  Longdrink ERC20 name
    /// @param  _symbol  Longdrink ERC20 symbol
    /// @param  _assets  Assets within the longdrink
    /// @param  _amounts Amounts of assets per one longdrink, initializes the wheights
