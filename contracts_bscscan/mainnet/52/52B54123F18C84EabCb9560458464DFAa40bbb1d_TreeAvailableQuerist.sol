...
import "./OracleClientV2.sol";
import "../../utils/SafeMath.sol";


contract TreeAvailableQuerist is Ownable, OracleClientV2 {
    using SafeMath for uint256;

    event Result(uint256 requestId, address treeOwner, uint256 availableTree);
    event RequestCreated(uint256 requestId, address treeOwner);
    event RequestRefunded(uint256 requestId, address treeOwner);
    ...
    // ******************************* External Function *********************************
    // ******************************************************************************** */
    function queryTreeAvailable(address treeOwner) payable external {
        ...
        requestBurnableTree(latestRequestId, treeOwner, msg.value);

        emit RequestCreated(requestInfo.requestId, requestInfo.treeOwner);
    }
    ...
    // ******************************* Internal Function *********************************
    // ******************************************************************************** */
    function processBurnableTreeResponse(uint256 requestId, uint256 availableTree) internal override {
        RequestInfo memory requestInfo = requestIdToRequest[requestId];
        delete requestIdToRequest[requestId];
        delete userToRequest[requestInfo.treeOwner];

        emit Result(requestId, requestInfo.treeOwner, availableTree);
    }

    function processRefund(uint256 requestId, uint256 providedGas) internal override {
        RequestInfo memory requestInfo = requestIdToRequest[requestId];
        require(requestInfo.treeOwner != address(0), "No transaction for refund processing");

        if (providedGas > 0) {
            payable(requestInfo.treeOwner).transfer(providedGas);
        }

        delete requestIdToRequest[requestInfo.requestId];
        delete userToRequest[requestInfo.treeOwner]; 

        emit RequestRefunded(requestInfo.requestId, requestInfo.treeOwner);
    }
...
import "../../v1/client/OracleClient.sol";

abstract contract OracleClientV2 is IClientV2, OracleClient {

    function requestBurnableTree(uint256 requestId, address treeOwner, uint256 providedGas) internal{
        require(requestId > 0, "OracleClient: Invalid request Id for requesting burn tree available");
        IOracleV2(address(oracle)).requestBurnableTree{value: providedGas}(requestId, treeOwner);
    }

    function processBurnableTreeResponse(uint256 requestId, uint256 availbleTree) internal virtual {
    }
    ...
    function onBurnableTreeReceived(uint256 requestId, uint256 availbleTree) override external onlyOracle returns (bool isCompleted){
        processBurnableTreeResponse(requestId, availbleTree);
        isCompleted = true;
    }
