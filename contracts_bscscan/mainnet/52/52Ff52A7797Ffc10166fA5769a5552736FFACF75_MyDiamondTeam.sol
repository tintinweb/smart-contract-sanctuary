...
contract MyDiamondTeam {
    ...
    mapping(uint32 => address) public id2Address;
    ...
    mapping(uint32 => uint32) public team_members_count;
    ...
    */
    ...
    function _setUpline(address _addr, address _upline) private {
        if(this.checkUplineValid(_addr, _upline)) {
            ...
            if(user_referral_team[_upline].exists == false){
                uint32 teamId = _createTeam(_upline, true); // create first team on upline-user. this contains the direct referrals
                user_referral_team[_upline].id = teamId;
                user_referral_team[_upline].exists = true;

                _addTeamMember(teamId, _addr); // add referral user to upline users referral-team
            }
    ...
    function teamAirdrop(uint32 teamId) payable external {
    ...
    function _createTeam(address userAddress, bool is_referral_team) private returns(uint32 teamId){
    ...
    function createTeam() external returns(uint32){
        return _createTeam(msg.sender, false);
    }

    function _addTeamMember(uint32 teamId, address member) private {
    ...
    function addTeamMember(uint32 teamId, address member) external {
   ...
   function changeTeamName(uint32 teamId, string memory name) external {
        Team storage team = teams[teamId];
    ...
    function checkNickname(string calldata name) external view returns (bool result){
        for( uint32 i = 0; i < total_users; i++){
    ...
    */
    ...
    function userInfo2(address _addr) view external returns(uint256 last_airdrop, uint8 teams_counter, TeamInfo[] memory member_of_teams, uint256 referral_team) {
        return (airdrops[_addr].last_airdrop, user_teams_counter[_addr], user_teams[_addr], user_referral_team[_addr].id);
    }

    function teamInfo(uint32 teamId) view external returns(Team memory _team) {
        return (teams[teamId]);
    }
