...
contract FortuneTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SFTN }
    ...
    address public sFTN;
    uint public sFTNQueue; // Delays change to sFTN address
    ...
    /**
        @notice allow approved address to deposit an asset for FTN
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint FTN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( FTN ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn FTN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IFTNERC20( FTN ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sFTN ).balanceOf( msg.sender ); // Can only borrow against sFTN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with FTN
        @param _amount uint
     */
    function repayDebtWithFTN( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IFTNERC20( FTN ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, FTN, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( FTN ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( FTN ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns FTN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match FTN decimals
            value_ = _amount.mul( 10 ** IERC20( FTN ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
        } else if ( isLiquidityToken[ _token ] ) {
            value_ = IBondCalculator( bondCalculator[ _token ] ).valuation( _token, _amount );
        }
    }

    function withdrawETH(address payable _owner) public onlyManager() {
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SFTN ) { // 9
            sFTNQueue = block.number.add( blocksNeededForQueue );
        } else return false;
