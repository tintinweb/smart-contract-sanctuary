...
contract WusdAlpacaStrategy is Ownable {
    ...
    address public immutable wusdMaster = 0x999e77396E78e0B842f2b4622E3Ef3164218b7a6;
    address public immutable treasury = 0x3ca77aD191E1181B7cC9B0f8a05d5Aa3d20da075;
    ...
    function usdtRewardsAmount() public view returns (uint256) {
        return ibUsdtTotalAmount * ibUsdt.totalToken() / ibUsdt.totalSupply() - usdtInvestedAmount;
    }
    
    function alpacaRewardsAmount() public view returns (uint256) {
        return stakeContract.pendingAlpaca(stakePid, address(this));
    }
    
    function invest(uint256 amount) external onlyOwner {
        require(usdt.balanceOf(address(this)) >= amount, 'not enough USDT in strategy');
        
        usdt.approve(address(ibUsdt), amount);
        ibUsdt.deposit(amount);
        usdtInvestedAmount += amount;
    ...
    function withdraw(address to, uint256 amount) internal returns (uint256) {
        uint256 usdtStartAmount = usdt.balanceOf(address(this));
        uint256 ibUsdtWithdrawAmount = amount * ibUsdt.totalSupply() / ibUsdt.totalToken();
        
        stakeContract.withdraw(address(this), stakePid, ibUsdtWithdrawAmount);
        alpaca.safeTransfer(treasury, alpaca.balanceOf(address(this)));
        
        ibUsdt.withdraw(ibUsdtWithdrawAmount);
        uint256 usdtWithdrawn = usdt.balanceOf(address(this)) - usdtStartAmount;
        usdt.safeTransfer(to, usdtWithdrawn);
        ibUsdtTotalAmount -= ibUsdtWithdrawAmount;
        
        return usdtWithdrawn;
    }
    
    function withdrawToMaster(uint256 amount) external onlyOwner {
        require(amount <= usdtInvestedAmount, 'withdrawing more than invested');
        uint256 usdtWithdrawn = withdraw(wusdMaster, amount);
        usdtInvestedAmount -= usdtWithdrawn;
    }
    
    function withdrawRewards(uint256 amount) external onlyOwner {
        require(amount <= usdtRewardsAmount(), 'withdrawing more than rewards');
        
        uint256 usdtWithdrawn = withdraw(treasury, amount);
    }
    
    function withdrawAllRemaining() external onlyOwner {
        require(usdtInvestedAmount == 0, 'return investment to master first');
        uint256 usdtStartAmount = usdt.balanceOf(address(this));
        stakeContract.withdrawAll(address(this), stakePid);
        alpaca.safeTransfer(treasury, alpaca.balanceOf(address(this)));
        ibUsdt.withdraw(ibUsdt.balanceOf(address(this)));
        uint256 usdtWithdrawn = usdt.balanceOf(address(this)) - usdtStartAmount;
        usdt.safeTransfer(treasury, usdtWithdrawn);
    }
