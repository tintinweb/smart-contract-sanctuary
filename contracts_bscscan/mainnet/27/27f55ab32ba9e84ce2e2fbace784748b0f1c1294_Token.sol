...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "SccBSCP1";
    string private _symbol = "SccBSCP1";
    uint8 private _decimals = 18;
    
    uint256 private _blackholeFee = 1;
    uint256 private _luckyBonusFee = 2;
    uint256 private _holderBonusFee = 6; // 4: holder bonus
    uint256 private _airdropFee = 1;
    uint256 private _liquidityFee = 10;
    uint256 private _allTaxFee = 20; // all up fee.
    ...
    uint256 public _maxTxAmount = 1 * 10**13 *  10**18;  //100 * 10**8 * 10**18; 
    uint256 public _maxTxAmount2 = 1  *  10**15;  //100 * 10**8 * 10**18;     
    ...
    function setMaxTxAmount2(uint256 maxTxAmount2) external onlyOwner() {
        _maxTxAmount2 = maxTxAmount2;
    }
    ...
    function _transfer(
        ...
        if(fromSwap && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]
            ...
            if (_isExcludedFromFee2[from]) {
                ...
                require(amount <= _maxTxAmount2, "Transfer amount exceeds the maxTxAmount.");
            }
            
            if (_Presale) {
                require(amount <= _maxTxAmount2, "Presale is on, restrict on buying or selling.");
            }
    ...
    function dealBonusAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 half = contractTokenBalance.mul(3).div(16); //scc original
        uint256 otherHalf = contractTokenBalance.sub(half); //scc original
        ...
        // uint256 liquidBalance = newBalance.mul(3).div(13); //scc original
        uint256 rBonus = newBalance; //scc redeem
        // holdBonus save in this contract.
        // uint256 holderBonus = newBalance.sub(liquidBalance).sub(holderBonus);
        // add liquidity to uniswap
        // addLiquidity(half, liquidBalance);  //scc original
        payable(redeemAddress).transfer(rBonus);
        // emit SwapAndLiquify(half, newBalance, otherHalf); //scc original
    }
