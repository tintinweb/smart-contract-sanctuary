...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    address payable constant public BURN_WALLET = payable(0x000000000000000000000000000000000000dEaD);
    uint public constant PERCENT_DIVIDER = 10000;//100%
    uint public constant MAX_FEE = 2000;//20%
    uint public BURNFEE = 400;//4%
    uint public COMUNITY_FEE = 400;//4%
    uint public DEVFEE = 100;//1%
    uint public constant MAX_WITHDRAWFEE = 1000;//10%
    ...
    function withdraw(uint256 _pid, uint256 _amount) internal {
        ...
        if (_amount > 0) {
            ...
            withdrawDeposit(pool.lpToken, msg.sender, _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if(blockPool[_pid] == true || tokenMaster.balanceOf(address(this)) == 0) {
            ...
            withdrawDeposit(pool.lpToken, msg.sender, amount);
        }
    ...
    function setFees(uint _devfee, uint _comunityFee, uint _burnFee) external onlyOwner {
        require(_devfee.add(_comunityFee).add(_burnFee) <= MAX_WITHDRAWFEE, "fees must add up to MAX_WITHDRAWFEE");
        BURNFEE = _burnFee;
        DEVFEE = _devfee;
        COMUNITY_FEE = _comunityFee;
    }

    function withdrawDeposit(IBEP20 from, address to, uint amount) internal {
        uint devFee = amount.mul(DEVFEE).div(PERCENT_DIVIDER);
        uint extraFee = devFee;
        payFees(from, devFee);
        if(address(from) == address(tokenMaster)) {
            uint comunityFee = amount.mul(COMUNITY_FEE).div(PERCENT_DIVIDER);
            uint burnFee = amount.mul(BURNFEE).div(PERCENT_DIVIDER);
            vault.safeTransfer(from, address(this), comunityFee);
            vault.safeTransfer(from, BURN_WALLET, burnFee);
            extraFee = extraFee.add(burnFee).add(comunityFee);
        }
        vault.safeTransfer(from, to, amount.sub(extraFee));
    }
