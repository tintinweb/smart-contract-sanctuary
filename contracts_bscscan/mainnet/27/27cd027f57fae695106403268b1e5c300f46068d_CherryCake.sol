/**
// CherryCake with dividends feature in the contract.
// Dividends with $CAKE
// With the auto-claim feature,
// Simply hold CherryCake and you'll receive CAKE automatically in your wallet.
// � Telegram: https://t.me/CherryCakeToken
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable Dividendstoken = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // CAKE huh?!
...
contract CherryCake is ERC20, Ownable {
    ...
    address public immutable Dividendstoken = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); //CAKE YEAH!

    bool private swapping;
    bool public buyTransaction;

    CherryDividendTracker public dividendTracker;
    ...
    address public _marketingWalletAddress = 0x86F7b56ADd48c96fe2283C4c5A9527c8304d8a69;
    ...
    constructor() public ERC20("CherryCake", "Cherry") {
    	...
    	dividendTracker = new CherryDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Cherry: The dividend tracker already has that address");

        CherryDividendTracker newDividendTracker = CherryDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Cherry: The new dividend tracker must be owned by the token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Cherry: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Cherry: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Cherry: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Cherry: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Cherry: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Cherry: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cherry: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "Cherry: This account cannot send tokens until trading is enabled");
        }
...
contract CherryDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("Cherry_Dividend_Tracker", "Cherry_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "Cherry_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Cherry_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main  contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Cherry_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cherry_Dividend_Tracker: Cannot update claimWait to same value");
