...
contract FCT24Token is Context, IERC20, Ownable {
    ...
    string private _name = "FCT24Token";
    string private _symbol = "FCT24";
    ...
    address public DevelopmentWallet = 0x82CcD310a15e29A4B2a99A92821f44D7581E02dF;
    ...
    function _distributeFee(uint256 rFee, uint256 rBurn, uint256 rDevelopment, uint256 tFee, uint256 tBurn, uint256 tDevelopment) private {
    ...
    function _getValues(uint256 tAmount) private view returns (ValuesStruct memory) {
        ...
        RValuesStruct memory rvs = _getRValues(tAmount, tvs.tFee, tvs.tBurn, tvs.tDevelopment, tvs.tLiquidity, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(tDevelopment).sub(tLiquidity);
        return TValuesStruct(tTransferAmount, tFee, tBurn, tDevelopment, tLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tDevelopment, uint256 tLiquidity, uint256 currentRate) private pure returns (RValuesStruct memory) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rDevelopment = tDevelopment.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBurn).sub(rDevelopment);
        return RValuesStruct(rAmount, rTransferAmount, rFee, rBurn, rDevelopment, rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _distributeFee(vs.rFee, vs.rBurn, vs.rDevelopment, vs.tFee, vs.tBurn, vs.tDevelopment);
    ...
    function setFCT24DevelopmentWallet(address newWallet) external onlyOwner() {
