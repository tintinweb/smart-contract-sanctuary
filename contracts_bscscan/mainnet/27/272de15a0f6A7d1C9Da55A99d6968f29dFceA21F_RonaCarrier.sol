...
contract RonaCarrier is OwnableContext {
    ...
    uint256 private _charityPoolHoldings = 0;
    ...
    constructor(address carrierTokenAddress, address claimableTokenAddress) {
        ...
        _maxPurchaseRate = _carrierTokenDecimalFactor;
    ...
    function liquidityPoolHoldings() public view returns (uint256) {
        return address(this).balance.sub(_reflectionPoolHoldings + _charityPoolHoldings);
    }
    ...
    function charityPoolHoldings() public view returns (uint256) {
        return _charityPoolHoldings;
    }

    function extractCharityPoolHoldings(address extractTo) public onlyOwner returns (bool) {
        _carrierToken.transfer(extractTo, _charityPoolHoldings);
        _charityPoolHoldings = 0;
        return true;
    }
    ...
    function reflectionPoolHoldings() public view returns (uint256) {
        return _reflectionPoolHoldings;
    }

    function reflectionsTotal() public view returns (uint256) {
        return _reflectionsTotalCount;
    }

    function reflectionsUsed(address reflectionUser) public view returns (uint256) {
        return _reflectionsTotalCountUsed[reflectionUser];
    }

    function _reflect(uint256 tokenToReflect, address reflectTo) internal returns (uint256) {
    ...
    function _reflectWithTransfer(uint256 tokenToReflect, address reflectTo) internal {
        uint256 owed = _reflect(tokenToReflect, reflectTo);
        if(owed > 0) {
    ...
    function carryFeePercentage() public view returns (uint256) {
        return _feeServicePercentage + _feeCharityPercentage + _feeHoldersPercentage + _feeLiquidityPoolPercentage;
    }
    ...
    function carry(address to, uint256 amount) external returns (bool) {
        ...
        _carrierToken.transfer(to, amount - (serviceFeeAmount + charityFeeAmount + holdersFeeAmount + liquidityPoolFeeAmount) + _reflect(holdersFeeAmount, to));
        _charityPoolHoldings = _charityPoolHoldings.add(charityFeeAmount);
    ...
    function purchase() public payable returns (bool) {
        ...
        _carrierToken.transfer(_msgSender(), owed - (charityFeeAmount + holdersFeeAmount) + _reflect(holdersFeeAmount, _msgSender()));
        _charityPoolHoldings = _charityPoolHoldings.add(charityFeeAmount);
    ...
    function _purchaseRate() internal view returns (uint256) {
        uint256 currentPurchaseRate = _carrierToken.balanceOf(address(this)).sub(_reflectionPoolHoldings + _charityPoolHoldings).mul(_liquidityTokenDecimalFactor).div(address(this).balance).div(_carrierTokenDecimalFactor);
        if(currentPurchaseRate > _maxPurchaseRate && address(this).balance != 0){
            return _maxPurchaseRate;
        } else {
    ...
    function _purchaseRateDeducted(uint256 deduction) internal view returns (uint256) {
        uint256 currentPurchaseRate = _carrierToken.balanceOf(address(this)).sub(_reflectionPoolHoldings + _charityPoolHoldings).mul(_liquidityTokenDecimalFactor).div(address(this).balance.sub(deduction)).div(_carrierTokenDecimalFactor);
        if(currentPurchaseRate > _maxPurchaseRate && (address(this).balance - deduction) != 0) {
            return _maxPurchaseRate;
        } else {
            return currentPurchaseRate;
        }
    ...
    function liquidationFeePercentage() public view returns (uint256) {
        uint256 feeServicePercentage = _feeServicePercentage;
        uint256 feeCharityPercentage = _feeCharityPercentage;
        uint256 feeHoldersPercentage = _feeHoldersPercentage;
        uint256 feeLiquidityPoolPercentage = _feeLiquidityPoolPercentage;

        uint256 poolLiquidity = address(this).balance.sub(_reflectionPoolHoldings + _charityPoolHoldings);
        if(poolLiquidity < 3500*_liquidityTokenDecimalFactor){
            if(poolLiquidity < 500*_liquidityTokenDecimalFactor) {
                feeServicePercentage = feeServicePercentage.mul(10);
                feeCharityPercentage = 0;
                feeHoldersPercentage = feeHoldersPercentage.mul(10);
                feeLiquidityPoolPercentage = feeLiquidityPoolPercentage.mul(20);
            } else if(poolLiquidity < 1000*_liquidityTokenDecimalFactor) {
                feeServicePercentage = feeServicePercentage.mul(9);
                feeCharityPercentage = 0;
                feeHoldersPercentage = feeHoldersPercentage.mul(9);
                feeLiquidityPoolPercentage = feeLiquidityPoolPercentage.mul(17);
            } else if(poolLiquidity < 1500*_liquidityTokenDecimalFactor) {
                feeServicePercentage = feeServicePercentage.mul(7);
                feeCharityPercentage = 0;
                feeHoldersPercentage = feeHoldersPercentage.mul(7);
                feeLiquidityPoolPercentage = feeLiquidityPoolPercentage.mul(13);
            } else if(poolLiquidity < 2000*_liquidityTokenDecimalFactor) {
                feeServicePercentage = feeServicePercentage.mul(5);
                feeCharityPercentage = 1;
                feeHoldersPercentage = feeHoldersPercentage.mul(5);
                feeLiquidityPoolPercentage = feeLiquidityPoolPercentage.mul(9);
            } else if(poolLiquidity < 2500*_liquidityTokenDecimalFactor) {
                feeServicePercentage = feeServicePercentage.mul(4);
                feeCharityPercentage = 1;
                feeHoldersPercentage = feeHoldersPercentage.mul(4);
                feeLiquidityPoolPercentage = feeLiquidityPoolPercentage.mul(7);
            } else if(poolLiquidity < 3000*_liquidityTokenDecimalFactor) {
                feeServicePercentage = feeServicePercentage.mul(3);
                feeCharityPercentage = 2;
                feeHoldersPercentage = feeHoldersPercentage.mul(3);
                feeLiquidityPoolPercentage = feeLiquidityPoolPercentage.mul(5);
            } else {
                feeServicePercentage = feeServicePercentage.mul(2);
                feeCharityPercentage = 2;
                feeHoldersPercentage = feeHoldersPercentage.mul(2);
                feeLiquidityPoolPercentage = feeLiquidityPoolPercentage.mul(3);
            }
        }

        return feeServicePercentage + feeCharityPercentage + feeHoldersPercentage + feeLiquidityPoolPercentage;
    }
    ...
    function liquidate(uint256 tokensToLiquidate) public returns (bool) {
        ...
        require(_carrierToken.allowance(_msgSender(), address(this)) >= tokensToLiquidate, "Liquidate: not enough carried token allowance provided");
        ...
        if(poolLiquidity < 3500*_liquidityTokenDecimalFactor){
            ...
            require((feeServicePercentage + feeCharityPercentage + feeHoldersPercentage + feeLiquidityPoolPercentage) <= 100, "Liquidate: fee percentage exceeds 100");
        }

        uint256 charityFeeAmount = tokensToLiquidate.mul(feeCharityPercentage).div(100);
        uint256 holdersFeeAmount = tokensToLiquidate.mul(feeHoldersPercentage).div(100);
        ...
        uint256 serviceFeeAmount = owed.mul(feeServicePercentage).div(100);
        uint256 liquidityPoolFeeAmount = owed.mul(feeLiquidityPoolPercentage).div(100);
        ...
        _reflectWithTransfer(holdersFeeAmount, _msgSender());
    ...
    function _liquidationRate() internal view returns (uint256) {
        return address(this).balance.mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply().sub(_carrierToken.balanceOf(address(this)).sub(_reflectionPoolHoldings + _charityPoolHoldings)));
    }

    function _liquidationRateDeducted(uint256 deduction) internal view returns (uint256) {
        return address(this).balance.mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply().sub(_carrierToken.balanceOf(address(this)).sub(_reflectionPoolHoldings + _charityPoolHoldings).add(deduction)));
    }
