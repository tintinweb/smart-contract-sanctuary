...
import "./TargetVaultBunny.sol";

contract TargetVaultBunnyBNB is TargetVaultBunny {
    ...
    constructor(
        ...
        bunnyVault = IBunnyVault(_bunnyVault);
        ...
        rewardTokenToTokenPath = [address(rewardToken), wBNB];
    }
    ...
    function _collectFees() internal override {
        ...
        cachedPricePerShare = bunnyVault.priceShare();
    }

    function _harvest() internal override {
        bunnyVault.getReward();
        uint256 swapAmt = rewardToken.balanceOf(address(this));
        if (swapAmt > 0) {
            rewardToken.safeApprove(swapRouterAddress, 0);
            rewardToken.safeApprove(swapRouterAddress, swapAmt);
    ...
    function _deposit() internal override {
        ...
        if (_balance > 0) {
            bunnyVault.depositBNB{value: _balance}();
    ...
    function withdraw(uint256 _amount) external override onlyVault {
        if (_amount > 0) {
            bunnyVault.withdrawUnderlying(_amount);
            uint256 _balance = address(this).balance;
    ...
    function setBuyBackToken(address _buyBackToken) public override onlyOwner {
        require(_buyBackToken != address(0), "TargetVaultBunnyBNB(setBuyBackToken): buy back token cannot be zero address");
    ...
    function setRewardToken(address _rewardToken) external override onlyOwner {
        require(_rewardToken != address(0), "TargetVaultBunnyBNB(setRewardToken): reward token cannot be zero address");

        rewardToken = IERC20(_rewardToken);

        rewardTokenToTokenPath = [address(rewardToken), wBNB, address(token)];
...
import "@openzeppelin/contracts/math/SafeMath.sol";
import "@openzeppelin/contracts/token/ERC20/SafeERC20.sol";
import "../../../interfaces/targetVaults/protocols/IBunnyVault.sol";
import "../TargetVault.sol";

contract TargetVaultBunny is TargetVault {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    /**
     * @dev bunnyVault address
     */
    IBunnyVault public bunnyVault;

    /**
     * @dev RewardToken Token address
     */
    ...
    /**
     * @dev Emitted when reward token changed
     */
    ...
    function _deposit() internal virtual {
        ...
        if (_balance > 0) {
            token.safeIncreaseAllowance(address(bunnyVault), _balance);
            bunnyVault.deposit(_balance);
    ...
    function harvest() external virtual {
        require(address(msg.sender) == harvester, "TargetVaultBunny(harvest): sender is not harvester");
        _harvest();
    }
    ...
    function vaultBalance() public view virtual returns (uint256) {
        return bunnyVault.withdrawableBalanceOf(address(this));
    }
    ...
    function vaultToken() public view virtual returns (address) {
        return address(bunnyVault.stakingToken());
    }
    ...
    function setHarvester(address _harvester) external virtual onlyOwner {
        require(_harvester != address(0), "TargetVaultBunny(setHarvester): harvester cannot be zero address");
     ...
     * @dev Set reward token
     */
    function setRewardToken(address _rewardToken) external virtual onlyOwner {
        require(_rewardToken != address(0), "TargetVaultBunny(setRewardToken): reward token cannot be zero address");
...
interface IBunnyVault is IERC20 {
