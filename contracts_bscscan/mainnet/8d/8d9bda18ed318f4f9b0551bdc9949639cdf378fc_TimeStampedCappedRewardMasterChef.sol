...
contract TimeStampedCappedRewardMasterChef is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    uint256 constant public DIVISOR = 10 ** 10;
    uint256 constant public MAX_STAKE_DURATION = 2628000;
    ...
    address public adminAddress = address(0x67E6aA19cbDEAccB633c01e0203085eD45aE95Ae);
    uint256 tax = 25; // 2.5%;
    ...
    function add(uint256 _rewardRate, IBEP20 _rewardToken, IBEP20 _lpToken, IBEP20 _lpQuoteToken, PriceFeedRouter _router, uint256 _min) public onlyOwner {
    ...
    function setMinStake(uint256 _pid, uint256 _min) public onlyOwner {
        poolInfo[_pid].min = _min;
    }
    ...
    function _roi(uint256 _amount, uint256 _createdAt, uint256 _lastWithdrawal) internal view returns(uint256) {
        uint256 period = Math.min(block.timestamp, _createdAt.add(MAX_STAKE_DURATION)); // max reward;
        uint256 thisRewardPeriod = period.sub(_lastWithdrawal);
        return _amount.mul(thisRewardPeriod).div(MAX_STAKE_DURATION);
    }
    
    function estimateReward(uint256 _pid, uint256 _amount) public view returns(uint256 estimate) {
        PoolInfo storage pool = poolInfo[_pid];
        
        if (pool.lpToken == pool.rewardToken) {
            // staking type
            estimate = _amount.mul(MAX_STAKE_DURATION).mul(pool.rewardRate).div(DIVISOR);
            ...
            uint256 quoteEstimate = _amount.mul(MAX_STAKE_DURATION).mul(pool.rewardRate).div(DIVISOR);

            if (quoteEstimate > 0) {
                estimate = quoteTokenToRewardToken(quoteEstimate, address(pool.lpToken), address(pool.rewardToken), pool.router);
            }

        } else {
            // it's an lp token
            uint256 quoteValue = lpTokenToQuoteToken(_amount, pool.lpToken, pool.lpQuoteToken);
            uint256 quoteEstimate = quoteValue.mul(2).mul(MAX_STAKE_DURATION).mul(pool.rewardRate).div(DIVISOR); // 2x of quoteValue is the user lpAmount;

            if (quoteEstimate > 0) {
                estimate = quoteTokenToRewardToken(quoteEstimate, address(pool.lpQuoteToken), address(pool.rewardToken), pool.router);
            }
        }
    }
    ...
    function pendingReward(uint256 _pid, address _user) public view returns(uint256 pending) {
        ...
        if (user.withdrawn >= user.maxRewardToken) {
            return pending;
        }
        
        uint256 vpending = _roi(user.maxRewardToken, user.createdAt, user.lastWithdrawal);
        
        if (user.withdrawn.add(vpending) > user.maxRewardToken) {
            pending = user.maxRewardToken.sub(user.withdrawn);
        } else {
            pending = vpending;
        }
    }

    // Deposit tokens to MasterChef for reward token.
    function deposit(uint256 _pid, uint256 _amount) public {

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        
        require(_amount >= pool.min, "less than minimum deposit amount");
        ...
        user.maxRewardToken = estimateReward(_pid, amount);

        emit Deposit(_msgSender(), _pid, amount);
    }

    // Withdraw reward from MasterChef.
    function withdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        require(user.amount >= 0, "!staking");
        require(user.withdrawn < user.maxRewardToken, "reward fully paid");
        ...
        if (pending > 0) {
            ...
            uint256 fee = pending.mul(tax).div(1000); // fee for withdrawal
            pool.rewardToken.safeTransfer(_msgSender(), pending.sub(fee));
            pool.rewardToken.safeTransfer(adminAddress, fee);
        }

        emit Withdraw(_msgSender(), _pid, pending);
    }
    
    function resetAccount(uint256 _pid) public {
        // reset account to enable new Deposit
        // Only after current staking reward has been fully paid
        
        UserInfo storage user = userInfo[_pid][_msgSender()];
        require(user.amount > 0, "Nothing to reset");
        require(user.withdrawn >= user.maxRewardToken, "Cannot reset earning account");
        
        user.amount = 0;
        user.withdrawn = 0;
        user.lastWithdrawal = 0;
        user.createdAt = 0;
        user.maxRewardToken = 0;
    ...
    // Withdraw staked token from MasterChef
    function withdrawTokenByAddress(IBEP20 _token, uint256 _amount) public onlyOwner {
