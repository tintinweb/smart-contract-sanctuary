...
contract DropPresale is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor (address _DROP, address _BUSD, uint256 _startingTimestamp) public {
        DROP = IBEP20(_DROP);
        BUSD = IBEP20(_BUSD);
        isSaleActive = true;
        owner = msg.sender;
    ...
    function buy (uint256 _amount, address beneficiary) public nonReentrant {
        ...
        require (busdReceived +  _amount <= 25000 ether, "Presale hardcap reached");
    ...
    function getDROPTokensLeft() external view returns (uint256) {
        return DROP.balanceOf(address(this));
    }

    function claimTokens (address claimer) external {
        require (isClaimActive, "Claim is not allowed yet");
        require (tokensOwned[msg.sender] > 0, "User should own some DROP tokens");
        require (tokensUnclaimed[msg.sender] > 0, "User should have unclaimed DROP tokens");
        require (DROP.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough DROP tokens to transfer.");

        DROP.safeTransfer(msg.sender, tokensUnclaimed[msg.sender]);
    ...
    function withdrawUnsoldDROP() external onlyOwner {
        uint256 amount = DROP.balanceOf(address(this)) - totalTokensSold;
        DROP.safeTransfer(msg.sender, amount);
    }

    function withdrawAllDROP() external onlyOwner {
        DROP.safeTransfer(msg.sender, DROP.balanceOf(address(this)));
    }
}