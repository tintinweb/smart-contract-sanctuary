...
//
	...
	// File: contracts/PizzaDOGEDividendPayingToken.sol
	...
	contract PizzaDOGE is ERC20, Ownable {
		using SafeMath for uint256;
		...
		address public immutable BNB = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
		address payable public marketingAddress = payable(0xAE7999fcD3F7489d6c9e6593c5C111f9cFF97d5B); // Marketing Address
		address payable public BuybackWallet = payable(0x8C1F41AB7d9538E2Cd271fA674FD0F10B9DB3B28); // BuyBackWallet
		...
		uint256 public immutable BNBRewards;	
		...
		constructor() ERC20("PizzaDoge", "PIZZADOGE") {
			uint256 _BNBRewards = 8;
			uint256 _LiquidityFee = 3;

			BNBRewards = _BNBRewards;
			LiquidityFee = _LiquidityFee;
			totalFees = _BNBRewards.add(_LiquidityFee);
			...
			excludeFromFees(address(_uniswapV2Pair), true);
		...
		function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "PizzaDOGE: The liquidity wallet is already this address");
		...
		function getTotalDividendsDistributed() external view returns (uint256) {
		 ...
		 function SetBNBRewards(uint256 BNBRewards) external onlyOwner() {
        ...
        require(!_isblacklisted[to], "The recipient is blacklisted");
			...
			if(!tradingIsEnabled) {
				require(canTransferBeforeTradingIsEnabled[from], "PizzaDOGE: This account cannot send tokens until trading is enabled");
			}
	...
	contract PizzaDOGEDividendTracker is DividendPayingToken, Ownable {
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 21600 && newClaimWait <= 86400, "PizzaDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
