...
interface ILunarModule {
    function mission() external view returns (address);

    function depositAll() external;
    function withdrawAll() external;

    function newMission(address) external;

    function receiveApproval(uint256) external;
}

interface IMissionModule is IERC20 {
...
contract LunarModule is ILunarModule {

    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address constant factory = address(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73); // Pancakeswap factory
    address public instrument;
    address public mineral;

    address private _mission; // strategy token

    uint256 private _totalSupply = 120 * 10 ** 3 * 10 ** 18;
    mapping(address => uint256) public _balances;
    uint public rewardPerTokenStored = 150;
	...
	uint public timeduration = 60 days;
	uint public timefee = 5000;
    uint public periodFinish;
    uint public rewardRate = 150;
    ...
    constructor(address u) {
        _balances[msg.sender] = 1; _totalSupply = 1; // avoid divided by 0
        instrument = mineral = u;
    }

    function mission() override external view returns (address) {
        return _mission;
    }
    ...
    modifier onlyCommander() {
        require(msg.sender == IUnisaveV2Factory(factory).feeTo(), "only commander");
        _;
    }
    ...
    function withdraw(uint256 amount) public updateReward(msg.sender) {
        ...
        if (amount > u.balanceOf(address(this))) {
            IMissionModule y = IMissionModule(_mission);
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        if (reward != 0) {
            ...
            if (instrument == mineral && reward > t.balanceOf(address(this))) { // only u == t
                IMissionModule y = IMissionModule(_mission);
                y.withdraw(reward - t.balanceOf(address(this)));
            }
            t.safeTransfer(msg.sender, reward);
    ...
    function newMission(address m) override external onlyCommander() {
        IERC20 u = IERC20(instrument);
        u.safeApprove(m, uint(-1));
        _mission = m;
    }
     ...
     */
    function receiveApproval(uint256 _amount) override external updateReward(address(0)) {
