...
// File: contracts/interfaces/IFreeFinanceLottery.sol

pragma solidity ^0.8.4;

interface IFreeFinanceLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in FREE per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in FREE token
     * @dev Callable by operator
     */
     ...
     * @dev Callable by operator
     * @param _endTimeInDays: endTime of the lottery in days
     * @param _priceTicketInFree: price of a ticket in FREE
     */
    function startLottery(
        uint256 _endTimeInDays,
        uint256 _priceTicketInFree
    ) external;
...
// File: contracts/FreeFinanceLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title FreeFinance Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract FreeFinanceLottery is ReentrancyGuard, IFreeFinanceLottery, Ownable {
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInFree;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] freePerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInFree;
        uint32 finalNumber;
    }
     ...
     * @param _freeTokenAddress: address of the FREE token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _freeTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of FREE to this contract
        uint256 amountFreeToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInFree,
            _ticketNumbers.length
        );

        // Transfer free tokens to this contract
        freeToken.safeTransferFrom(address(msg.sender), address(this), amountFreeToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInFree += amountFreeToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInFreeToTransfer
        uint256 rewardInFreeToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInFreeToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        freeToken.safeTransfer(msg.sender, rewardInFreeToTransfer);

        emit TicketsClaim(msg.sender, rewardInFreeToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInFree) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in FREE for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].freePerBracket[j] =
                ...
                // A. No FREE to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].freePerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInFree - amountToShareToWinners);

        // Transfer FREE to treasury address
        freeToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        freeToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInFree += _amount;

        emit LotteryInjection(_lotteryId, _amount);
    }
    
    
    uint256 private discountDivisor = 2000;
    uint256 private treasuryFee = 2000;
    
    
    uint256[6] private rewardsBreakdown = [250,375,625,1250,2500,5000];
    
    // EXTRA SETTINGS
    /**
     * @notice Chenge rewards
     * @dev Callable by owner
     * @param _rewardsBreakdown: breakdown of rewards per bracket (must sum to 10,000)
     */
    function changeReword(uint256[6] calldata _rewardsBreakdown) external onlyOwner {
        require(
            (_rewardsBreakdown[0] +
                _rewardsBreakdown[1] +
                _rewardsBreakdown[2] +
     ...
     */
    function changeDivisor(uint256 _discountDivisor) external onlyOwner {
    ...
    uint256 public aDay = 60; // one day is 86400
    function changeaDay(uint256 _inSeconds) external onlyOwner {
        aDay = _inSeconds;
    }
    ...
    function startLottery(
        ...
        uint256 _endTime = _endTimeInDays * aDay + block.timestamp;
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInFree: _priceTicketInFree,
            discountDivisor: discountDivisor,
            rewardsBreakdown: rewardsBreakdown,
            treasuryFee: treasuryFee,
            freePerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInFree: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(freeToken), "Cannot be FREE token");
     ...
     * @param _priceTicket price of a ticket (in FREE)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].freePerBracket[_bracket];
        } else {
            return 0;
        }
    }
