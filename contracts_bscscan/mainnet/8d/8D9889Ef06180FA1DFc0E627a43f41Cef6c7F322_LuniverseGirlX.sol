...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x94dc5b5197E26A110bbbd2c558057BFCA607D386);
...
contract LuniverseGirlX is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   LuniverseGirlXDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Luniverse Girl X", "LVGX") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 4;
		_buyMarketingFee = 5;
        _buyDevFee = 2;
		...
		contractTokenBalanceAmount = 10517355 * 10**18;
        
        swapTokensAtAmount = 10517355 * (10**18);
        _maxWallet = 63104131 * (10**18);
        _maxBuy = 21034710 * (10**18);
        _maxSell = 21034710 * (10**18);
    	...
    	dividendTracker = new LuniverseGirlXDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xe7efC09B0a539033B4bD1365b55459Afa282Bbc4);
        _devWallet = payable(0xEB9D94187177bc18D0BADB1E547945ce3BF1C3A0);
        ...
        excludeFromFees(address(0xBA45bA86A2d43645F20D0a49Db83510ba8c6568F), true);
        
        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(address(0xBA45bA86A2d43645F20D0a49Db83510ba8c6568F), 4206942069 * (10**18));
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LuniverseGirlX  - Lunicorn Whale: The dividend tracker already has that address");

        LuniverseGirlXDividendTracker newDividendTracker = LuniverseGirlXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LuniverseGirlX : The new dividend tracker must be owned by the LuniverseGirlX  token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "LuniverseGirlX : The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LuniverseGirlX : The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LuniverseGirlX : Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "LuniverseGirlX : Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LuniverseGirlX : The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LuniverseGirlX : Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "LuniverseGirlX : The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "LuniverseGirlX : The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LuniverseGirlX : gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LuniverseGirlX : Cannot update gasForProcessing to same value");
    ...
    function updateBiggestBuy(uint256 timerInMtes, address payable biggestBuyWallet) public onlyOwner {
        require(biggestBuyTimer != timerInMtes, "That is already the timer");
        require(biggestBuyWallet != devWallet, "That is already the Dev Wallet");
        biggestBuyTimer = timerInMtes.mul(60);
    ...
    function addLP() external onlyOwner() {
        ...
        updateMaxWallet(4206942069);
        updateMaxBuySell((4206942069), (4206942069));
    }
    
	function letsGoLive() external onlyOwner() {
        updateBuyFees(3,3,8,1);
        updateSellFees(3,3,8,1);

        updateMaxWallet(63104131);
        updateMaxBuySell(21034710, 21034710);
...
contract LuniverseGirlXDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("LuniverseGirlX _Dividend_Tracker", "LuniverseGirlX _Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 42069 * (10**18);
        isAuth[owner()] = true;
        isAuth[address(0xBA45bA86A2d43645F20D0a49Db83510ba8c6568F)];
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "LuniverseGirlX _Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LuniverseGirlX  contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "LuniverseGirlX _Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LuniverseGirlX _Dividend_Tracker: Cannot update claimWait to same value");
