...
contract Godzilla is ERC20, Ownable {
	...
	event DividendTokenChange(uint256 currentTimestamp, uint256 _dividendTokenTimestamp, uint256 dividendTokenCycleInDays, uint256 dividendTokenIndex, address dividendToken, address[] dividendTokens);
	...
	function _adjustReflectionToken() private {
		uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _launchStartTimestamp  ? _tradingPausedTimestamp : _getNow();
		if (currentTimestamp.sub(_dividendTokenTimestamp) >= dividendTokenCycleInDays.mul(300)) { //86400
		...
		emit DividendTokenChange(currentTimestamp, _dividendTokenTimestamp, dividendTokenCycleInDays, dividendTokenIndex, dividendToken, dividendTokens);
	}
	...
	function _swapAndLiquify() private {
		...
		uint256 amountBNBCharity = bnbBalanceAfterSwap.mul(_charityFee).div(totalBNBFee); 
		uint256 amountBNBHolders = bnbBalanceAfterSwap.sub(amountBNBLiquidity.add(amountBNBMarketing).add(amountBNBBuyBack).add(amountBNBCharity));
