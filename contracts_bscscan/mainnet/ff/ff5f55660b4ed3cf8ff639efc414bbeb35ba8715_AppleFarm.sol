...
contract AppleFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function pendingApple(uint256 _pid, address _user) external returns (uint256) {
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(appleReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                apple.transfer(referrer, commissionAmount);
