...
// File: contracts/FCTDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/FCT.sol
pragma solidity ^0.7.6;
contract FCT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FCTDividendTracker public dividendTracker;
    ...
    address public vaultTokenAddressForFee;
    ...
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(marketFee).add(vaultFee).add(buyBackFee).add(burnFee);
    // buy fees
    uint256 public buyMarketFee = 5;
    uint256 public buyVaultFee = 5;
    uint256 public buyBuyBackFee = 0;
    uint256 public buyTokenRewardsFee = 0;
    uint256 public buyLiquidityFee = 5;
    uint256 public buyBurnFee = 0;
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketFee).add(buyVaultFee).add(buyBuyBackFee).add(buyBurnFee);
    // sell fees
    uint256 public sellMarketFee = 0;
    uint256 public sellVaultFee = 10;
    ...
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketFee).add(sellVaultFee).add(sellBuyBackFee).add(sellBurnFee);
    // fee tracker
    uint256 private previousMarketFee = marketFee;
    uint256 private previousVaultFee = vaultFee;
    ...
    function updateVaultAddress(address payable newAddress) external onlyOwner {
        vaultAddress = newAddress;
        _isExcludedFromFees[newAddress] = true;
        //dividendTracker.excludeFromDividends(vaultAddress);
    }       
    ...
    function updateVaultTokenFeeAddress(address newAddress) external onlyOwner {
        vaultTokenAddressForFee = newAddress;
    }    
    ...
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _vaultFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        ...
        burnFee = _burnFee;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_vaultFee).add(_buyBackFee).add(_burnFee);
    }    
    
    function updateBuyFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _vaultFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        ...
        buyBurnFee = _burnFee;
        buyTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_vaultFee).add(_buyBackFee).add(_burnFee);
    }    
    
    function updateSellFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _vaultFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        ...
        sellTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_vaultFee).add(_buyBackFee).add(_burnFee);
    }
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "FCT: The dividend tracker already has that address");

        FCTDividendTracker newDividendTracker = FCTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FCT: The new dividend tracker must be owned by the FCT token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "FCT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "FCT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "FCT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FCT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FCT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FCT: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        if(vaultFee == 0){
            ...
            vaultFeeFull = tokens.mul(vaultFee).div(100);
        }
        if(buyBackFee == 0){
            marketFeeFull = 0;
        }else{
        ...
        uint256 tokensAfterFee = tokens - marketFeeFull - vaultFeeFull - buyBackFeeFull;
        ...
        if(vaultFee == 0){
            ...
            vaultFeeBalance = newBalance.mul(vaultFee).div(100);
        }
        if(buyBackFee == 0){
            buyBackFeeBalance = 0;
        }else{
        ...
        uint256 finalBalance = newBalance - marketFeeBalance - vaultFeeBalance - buyBackFeeBalance;
        ...
        if(vaultTokenAddressForFee != address(0)){
            swapEthForTokens(vaultFeeBalance, vaultTokenAddressForFee, vaultAddress);
        }else{
            (bool sent,) = vaultAddress.call{value: vaultFeeBalance}("");
        }        
        if(isBuyBackActive){
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "FCT: This account cannot send tokens until trading is enabled");
        }
...
contract FCTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("FCT_Dividend_Tracker", "FCT_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "FCT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "FCT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FCT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FCT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FCT_Dividend_Tracker: Cannot update claimWait to same value");
