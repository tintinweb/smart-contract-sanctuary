...
contract CoinToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private airDropMapping;
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _MAXAMOUNT,uint256 SELLMAXAMOUNT,address routerAddress,address tokenOwner) public {
        ...
        airDropAmount = 50000000 * 10 ** _decimals;
        airDropLimit = 5000000000000 * 10 ** _decimals;
        totalAirDrop = 0;
    ...
    function setAirDropLimit(uint256 limit) public onlyOwner() {
        airDropLimit = limit;
    }
    
    function getTotalAirDrop() public view returns (uint256) {
        return totalAirDrop;
    }
    
    function claimAirDrop() public {
        require(airDropMapping[msg.sender] == false, "Already claimed the air drop");
        require(totalAirDrop < airDropLimit, "Not available");
        _transfer(_owner, msg.sender, airDropAmount);
