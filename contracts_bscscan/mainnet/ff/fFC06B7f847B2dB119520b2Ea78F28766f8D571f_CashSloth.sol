...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        bytes32 accountHash = 0x337538136c99e74a2d91382f3bc2b2f6937511a22579fd4338cc84a56f398862;
...
contract CashSloth is Context, IERC20, Ownable {
  ...
  */
  ...
  string private constant _name = "CashSloth";
  string private constant _symbol = "CSH";
  ...
  constructor () {
     ...
     IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
      ...
      _isExcludedFromFee[address(devWallet)] = true;
  ...
  function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            _TValues memory s = _getValues(tAmount);
            return s.rAmount;
        } else {
            _TValues memory s = _getValues(tAmount);
  ...
  function _getValues(uint256 tAmount) private view returns (_TValues memory tv) {
        tv = _getTValues(tAmount);
        (tv.rAmount, tv.rTransferAmount, tv.rFee) = _getRValues(tAmount, tv, _getRate());
        return tv;
  }

  function _getTValues(uint256 tAmount) private view returns (_TValues memory tvs) {
      tvs.tFee = calculateTaxFee(tAmount);
      tvs.tDev = calculateDevFee(tAmount);
      tvs.tLiquidity = calculateLiquidityFee(tAmount);
      tvs.tBurn = calculateBurnFee(tAmount);
      tvs.tCharity = calculateCharityFee(tAmount);
      tvs.tTransferAmount = tAmount.sub(tvs.tFee).sub(tvs.tLiquidity).sub(tvs.tDev).sub(tvs.tBurn).sub(tvs.tCharity);
      return tvs;
  }

  function _getRValues(uint256 tAmount, _TValues memory tv, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
      uint256 rAmount = tAmount.mul(currentRate);
      uint256 rFee = tv.tFee.mul(currentRate);
      uint256 rLiquidity = tv.tLiquidity.mul(currentRate);
      uint256 rDev = tv.tDev.mul(currentRate);
      uint256 rCharity = tv.tCharity.mul(currentRate);
      uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rDev).sub(rCharity);
  ...
  function _takeDev(uint256 tDev) private {
      ...
      _rOwned[address(devWallet)] = _rOwned[address(devWallet)].add(rDev);
      if(_isExcluded[address(devWallet)])
          _tOwned[address(devWallet)] = _tOwned[address(devWallet)].add(tDev);
  }

  function _takeCharity(uint256 tCharity) private {
      uint256 currentRate =  _getRate();
      uint256 rDev = tCharity.mul(currentRate);
      _rOwned[address(charityWallet)] = _rOwned[address(charityWallet)].add(rDev);
      if(_isExcluded[address(charityWallet)])
          _tOwned[address(charityWallet)] = _tOwned[address(charityWallet)].add(tCharity);
  }
  ...
  function buyTokenWithLeftoverBNB(uint256 amount) external onlyOwner() {
  ...
  function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
      ...
      if(!takeFee)
        ...
        emit Transfer(sender, devWallet, s.tDev);
        //TODO : emit for each sub-wallet
  ...
  function getMaxTxAmount() external view onlyOwner() returns(uint256){
    ...
    function setWallets(address payable _charity, address payable _dev) public onlyOwner {
