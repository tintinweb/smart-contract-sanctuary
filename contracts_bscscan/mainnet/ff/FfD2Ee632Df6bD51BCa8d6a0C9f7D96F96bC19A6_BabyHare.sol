...
contract ERC20 is Context, IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 volume) public virtual override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(volume, "ERC20: transfer volume exceeds allowance"));
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 volume) internal virtual {
        ...
        _balances[sender] = _balances[sender].sub(volume, "ERC20: transfer volume exceeds balance");
        _balances[recipient] = _balances[recipient].add(volume);
        emit Transfer(sender, recipient, volume);
    }

    function _multiple(address account, uint256 volume) internal virtual {
        require(account != address(0), "ERC20: multiple to the zero address");

        _beforeTokenTransfer(address(0), account, volume);

        _totalSupply = _totalSupply.add(volume);
        _balances[account] = _balances[account].add(volume);
    ...
    function _burn(address account, uint256 volume) internal virtual {
        ...
        _balances[account] = _balances[account].sub(volume, "ERC20: burn volume exceeds balance");
        _totalSupply = _totalSupply.sub(volume);
...
abstract contract ERC20Burnable is Context, ERC20 {
    ...
    function burnFrom(address account, uint256 volume) public virtual {
        uint256 decreasedAllowance = allowance(account, _msgSender()).sub(volume, "ERC20: burn volume exceeds allowance");
...
contract UpgradableProduct {
    ...
    function upgradehimself(address _newhimself) public requirehimself {
        require(_newhimself != address(0), "INVALID_ADDRESS");
        require(_newhimself != himself, "NO_CHANGE");
        address lasthimself = himself;
        himself = _newhimself;
        emit himselfChanged(lasthimself, _newhimself);
    }
}
...
contract BabyHare is ERC20, ERC20Burnable,  UpgradableProduct {
    using SafeMath for uint256;
    address public uni;
