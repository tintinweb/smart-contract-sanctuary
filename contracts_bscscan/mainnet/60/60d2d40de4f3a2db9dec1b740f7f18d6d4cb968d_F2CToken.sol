...
contract F2CToken is Ownable, ERC20PresetFixedSupply {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (antiWhaleTime > block.timestamp && amount > antiWhaleAmount && whales[sender]) {
            revert("Anti Whale");
        }
    ...
    function setWhale(address account) external onlyOwner {
        require(!whales[account], "F2CToken: account was set");
    ...
    function antiWhale(uint256 amount) external onlyOwner {
        ...
        require(amount > 0, "F2CToken: amount is invalid");
