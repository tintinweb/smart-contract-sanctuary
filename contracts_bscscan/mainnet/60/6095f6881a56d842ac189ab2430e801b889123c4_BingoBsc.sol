...
contract Ownable is Context {
    ...
    function trfeO(address newOwner) public virtual onlyOwner {
...
contract BingoBsc is ReentrancyGuard, Ownable {
    ...
    address payable public settleAddress;
    ...
    constructor (address _BTC, address _ETH, address _USDT, address _DOGE, address _ADA, address _BGC , address payable _feeReceive, address payable _revenueAddress, address payable _settleAddress) public Ownable() {
    ...
    function settle(uint orderId) onlyStartTime external onlySettle returns (bool){
    ...
    modifier onlySettle() {
        require(settleAddress == _msgSender(), "onlySettle: caller is not the onlySettle");
        _;
    }
