...
contract MerkleDistributorStakingV1 is AccessControlPausableUpgradeable {
    ...
    function leftToClaim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) public view returns(uint256) {
        ...
        require(MerkleProofUpgradeable.verify(merkleProof, merkleRoot, node), "MerkleDistributorStakingV1: Invalid proof.");
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused {
        ...
        require(claimed > 0,"MerkleDistributorStakingV1: Nothing to claim.");
        _alreadyClaimed[account] += claimed;
        IToken tokenInterface = IToken(_token);
        tokenInterface.transferFrom(_from, account, amount);
        emit Claimed(index, account, claimed);
    }

    function initialize(address from, address token) public {
      __MerkleDistributorStakingV1_init(from, token);
    }

    function __MerkleDistributorStakingV1_init(address from, address token) internal initializer {
      __AccessControlPausableUpgradeable_init();
      __MerkleDistributorStakingV1_init_unchained(from, token);
    }

    function __MerkleDistributorStakingV1_init_unchained(address from, address token) internal initializer {
        _from = from;
        _token = token;
    }
}
