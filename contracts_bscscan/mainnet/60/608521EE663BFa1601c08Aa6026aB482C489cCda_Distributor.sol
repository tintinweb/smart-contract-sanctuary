...
contract Distributor is Ownable {
    ...
    constructor(address[] memory _addresses, uint256[] memory _percents, address router) public {
    ...
    /**
    Distributes all tokens that are held on the contract based on the distribution config
    @param token token address to distribute
     */
    function distributeToken(IERC20 token) external {
        distributeTokenWithAmount(token, token.balanceOf(address(this)));
    }

    /**
    Convert tokens on the contract to native currrency and distribute
    @param token token to convert
    @param amount amount to convert
    @param minSwapAmount min amount of native currency to receive (slipage check)
     */
    function convertToBnbAndDistribute(IERC20 token, uint256 amount, uint256 minSwapAmount) external {
    ...
    /**
    Disribute tokens base on the distribution config
    Callable only by owner
    @param token to distribute
    @param amount amount of the tokens to distribute
     */
    function distributeTokenWithAmount(IERC20 token, uint256 amount) public onlyOwner {
    ...
    /**
    Distributes all amount of the native currency based on the disribution config
     */
    function distributeWei() public {
    ...
    /**
    Sets distribution config: which address which percent of the distributor contract balance will receive
    Callable only by the owner of the contract.
    Percents are scaled by 100, e.g. for 10% should pass 1000
    @param _addresses arrays of addresses for distribution
    @param _percents array of scaled percents. Must sum up to 10000
    */
