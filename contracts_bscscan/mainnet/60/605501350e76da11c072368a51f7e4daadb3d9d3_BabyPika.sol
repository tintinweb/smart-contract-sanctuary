...
abstract contract BabyPikaBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "BabyPika";
	string private constant SYMBOL = "BABYPIKA";
    ...
    // marketing address
    address private _marketingWallet = 0x0cd18525D2e0cc74876a47576E348AF5d0c7f494;
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and BABYPIKA<-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		setPancakeSwapRouter(routerAddress);

		// 1% liquidity fee, 8% reward fee, 2% buyback fee, 3% marketing fee, 7% additional sell fee
		setFees(1, 8, 2, 3, 7);
	...
	function activate() public onlyOwner {
		...
		setTransactionLimit(5000); // only 0.5% of the total supply can be sold at once
		activateBuying();
		onActivated();
	}
	...
	function executeSwap(uint256 amount) private {
		...
		uint256 tokensReservedForReward = amount - tokensReservedForLiquidity - tokensReservedForBuyback - tokensReservedForMarketing;
		...
		uint256 tokensToSwap = tokensReservedForReward + tokensToSwapForLiquidity + tokensReservedForBuyback + tokensReservedForMarketing;
	...
	// This function swaps a {tokenAmount} of BABYPIKAtokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for BABYPIKA-> WBNB
	...
	function swapBNBForTokens(uint256 bnbAmount, address to) internal returns(bool) { 
		// Generate pair for WBNB -> BABYPIKA
		address[] memory path = new address[](2);
	...
	// Returns how many more $BABYPIKAtokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 additionalSellFee) public onlyOwner {
		...
		// Enforce invariant
		_poolFee = _rewardFee + _liquidityFee + _buybackFee + _marketingFee;
	}
...
contract BabyPika is BabyPikaBase {

	// REWARD CYCLE
	uint256 private _rewardCyclePeriod = 3600; // The duration of the reward cycle (e.g. can claim rewards once 12 hours)
	...
	uint256 private _totalBNBAsBABYPIKAClaimed; // The total number of BNB that was converted to BABYPIKAand claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsBABYPIKAClaimed; // The amount of BNB converted to BABYPIKAand claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy BABYPIKAautomatically
	...
	bool private _reimburseAfterBABYPIKAClaimFailure; // If true, and BABYPIKAreward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress) BabyPikaBase(routerAddress) {
		...
		// If someone sends or receives more than 100% of their balance in a transaction, their reward cycle date will increase accordingly
		setRewardCycleExtensionThreshold(100);
	}
	...
	function onActivated() internal override {
		...
		setReimburseAfterBABYPIKAClaimFailure(true);
		setMinRewardBalance(5000000 * 10**decimals());  //At least 50 million tokens are required to be eligible for rewards
		setGradualBurnMagnitude(10); //Buy tokens using 0.1% of reward pool and burn them
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough BABYPIKAbalance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim BABYPIKAtokens
		if (!claimBABYPIKA(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterBABYPIKAClaimFailure) {
	...
	function claimBABYPIKA(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsBABYPIKAClaimed[user] += bnbAmount;
		_totalBNBAsBABYPIKAClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $BABYPIKAon a zero balance address:
		...
		// Depending on the % of $BABYPIKAtokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsBABYPIKA(address byAddress) public view returns (uint256) {
		return _bnbAsBABYPIKAClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsBABYPIKA() public view returns (uint256) {
		return _totalBNBAsBABYPIKAClaimed;
	}
	...
	function reimburseAfterBABYPIKAClaimFailure() public view returns(bool) {
		return _reimburseAfterBABYPIKAClaimFailure;
	}


	function setReimburseAfterBABYPIKAClaimFailure(bool value) public onlyOwner {
		_reimburseAfterBABYPIKAClaimFailure = value;
	}
