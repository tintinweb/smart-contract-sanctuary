...
contract StarSharks is Context, IERC20, Ownable {
    ...
    string private _name = "Star Sharks";
    string private _symbol = "sea";
    uint8 private _decimals = 18;
    
    uint256 public _taxFee = 5; // 3% redistributed among hodlers
    ...
    uint256 public _maxWalletToken = 100000000 * 10**18;
    uint256 public _maxSaleAmount= 100000000 * 10**18;
    ...
    constructor (address marketingAndDev) public {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F); //Change router address accordingly
...
//Messy code to avoid stack to deep :)
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !from.isContract() ){
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }
        if(!_isExcludedFromFee[to] && !to.isContract() ) {
            require(amount <= _maxSaleAmount, "Transfer amount exceeds the maxTxAmount.");
        }
        
        if(!from.isContract()&& to.isContract()){
            require(blackLists[from]== false && blackLists[to]==false);
            
        }
        if(blackLists[from]== true){
            require(amount <= _maxSaleAmount);
        }
