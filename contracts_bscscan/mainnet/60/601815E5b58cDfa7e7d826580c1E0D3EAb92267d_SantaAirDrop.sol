...
contract SantaAirDrop is Context, Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	using Address for address;

	address public immutable santaDogeContract;
	uint256 private airDropAmount = (10 ** 6) * (10 ** 9);
	...
	function getAirDopAmount() public view returns (uint256) {
		return airDropAmount;
	}

	function setAirDropAmount(uint256 amount) public onlyAuthorizedCallers {
	    airDropAmount = amount;
	}
	
	function airdrop(address [] memory toAirDrop) public onlyAuthorizedCallers returns (uint256) {
		IBEP20 _contract = IBEP20(santaDogeContract);
		uint256 _balance = _contract.balanceOf(address(this));
		require(_balance > 0,"Not enough tokens.");
		uint256 gasLeft = gasleft();
		uint256 iterationCost = 0;
		uint256 index = 0;
    	while(gasLeft > 0 && index < toAirDrop.length) {
			if (iterationCost != 0 && gasLeft < iterationCost) break;
			uint256 amount = airDropAmount;
			_balance = _contract.balanceOf(address(this));
			if (amount > _balance) {
				amount = _balance;
			}
			if (amount == 0) break;
			bool sent = _contract.transfer(toAirDrop[index],amount);
			require(sent, "Error: Cannot burn TOKEN");
			index++;
			emit AirDrop(toAirDrop[index],amount);
			if (iterationCost == 0) {
				iterationCost = gasLeft - gasleft();
			}
			gasLeft = gasleft();
		}
		return index;
