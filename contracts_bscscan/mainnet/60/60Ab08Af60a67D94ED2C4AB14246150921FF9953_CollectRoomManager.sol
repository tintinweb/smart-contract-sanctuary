...
contract CollectRoomManager is SafeOwnable, Random {
    ...
    event NFTCreated(IPandoraERC1155 nftToken, uint rid, uint[] ids, uint[] types, uint[] values);
    ...
    constructor(address _WETH, IPandoraERC1155 _nftToken, address _receiver, address _feeReceiver, address _rewardReceiver, address _linkAccessor) Random(_linkAccessor) {
    ...
    function add(
        ...
        uint[] memory nftIDs = nftToken.createBatchDefault(_nftTypes, _nftValues, true);
        ...
        for (uint i = 0; i < _nftTypes.length; i ++) {
            ...
            emit RoomRange(rid, _nftTypes[i], nftIDs[i], lastEndIndex, lastEndIndex.add(_nftPercents[i]));
    ...
    function claim(uint256 rid, address to) external {
        ...
        tokenTransfer(room.rewardToken, to, reward);
