...
contract BattleNFT is IBattleNFT, ERC721, Manageable {
    ...
    function mint(address _to, uint256[] memory _basicAttrs, uint256[] memory _skillIds, uint256[] memory _battleAttrs) external override onlyManager {
        ...
        string memory temp = "/";
        setTokenURI(tokenID, string(abi.encodePacked(cardid, temp, tokenidstring)));
...
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function getApproved(uint256 tokenId) public view virtual override validToken(tokenId) returns (address) {
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual validToken(tokenId) returns (bool) {
        address owner = ERC721.ownerOf(tokenId);
    ...
    function _setTokenURI(uint256 tokenId, string memory _tokenURI) internal virtual validToken(tokenId) {
