...
contract LinearRelease is Ownable, ReentrancyGuard {
  ...
  mapping(address => uint256) private _totalHasBeenLocked;
  ...
  function totalLockUntilToday(address _user) public view returns (uint256) {
    return _totalHasBeenLocked[_user];
  }
  ...
  function lock(address _user, uint256 _amount) public nonReentrant onlyOwner {
    require(_user != address(0), "lock: no address(0)");

    _locks[_user] = _locks[_user].add(_amount);
    _totalHasBeenLocked[_user] = _totalHasBeenLocked[_user].add(_amount);
  ...
  function claim() public nonReentrant {
    require(_locks[msg.sender] > 0, "linear release: no locked");
  ...
  function rescuseFund( uint256 _amount) public nonReentrant onlyOwner {
