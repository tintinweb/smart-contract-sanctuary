...
contract DividendDistributor is IDividendDistributor {
    ...
    mapping (uint256 => TokenPool) public indexedPools;
    ...
    uint256 public currentIndexTokensSupported = 0;
    ...
    function setIndex(uint256 i) public {
        currentIndexTokensSupported = i;
    }
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        pool.shares = pool.shares.sub(shares[shareholder][token].amount).add(amount);
        shares[shareholder][token].amount = amount;
        uint256 excluded = getCumulativeDividends(shares[shareholder][token].amount, token);
        shares[shareholder][token].totalExcluded = excluded;
        pool.realisedDividends = pool.realisedDividends.add(excluded);
        tokenPools[token] = pool;
    }

    bool private resetIndex = false;

    function deposit() external payable override onlyToken {
        IBEP20 currentToken = nextToken();
        while (!isTokenLiquid(currentToken) && !resetIndex) //TODO Check not infinite loop
            currentToken = nextToken();

        TokenPool memory pool = tokenPools[currentToken];
        ...
        pool.dividends = pool.dividends.add(amount);
        pool.dividendsPerShare = pool.dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(pool.shares));

        tokenPools[currentToken] = pool;
    }

    event UsingToken(address token);
    event UsingIndex(uint256 index);
    function nextToken() public returns (IBEP20) {
        resetIndex = false;
        currentIndexTokensSupported = currentIndexTokensSupported.add(1);
        if (currentIndexTokensSupported == currentPoolIndex) {
            currentIndexTokensSupported = 0;
            resetIndex = true;
        }
        emit UsingIndex(currentIndexTokensSupported);
        IBEP20 token = indexedPools[currentIndexTokensSupported].token;
        emit UsingToken(address(token));
        return token;
    }
    ...
    function isTokenLiquid(IBEP20 token) public view returns (bool) {
        uint256 shares = tokenPools[token].shares; //How many shares per token (% sup allocated to this token) AKA
        uint dividendsPerShare = tokenPools[token].dividendsPerShare; // How many dividends we should give proportionate to the shares. This * shares = amount to give
        if (dividendsPerShare == 0) {
            return true; // Never calculated or used yet
        }
        return shares.mul(dividendsPerShare).div(dividendsPerShareAccuracyFactor) < 10000 wei;
    }

    function getCumulativeDividends(uint256 share, IBEP20 token) internal view returns (uint256) {
        return share.mul(tokenPools[token].dividendsPerShare).div(dividendsPerShareAccuracyFactor);
    }
    ...
    function addTokenPool(IBEP20 token) external onlyToken {
        ...
        TokenPool memory pool = TokenPool(0, token, 0, 0, 0, 0);
        indexedPools[currentPoolIndex] = pool;
        currentPoolIndex = currentPoolIndex.add(1);
        tokenPools[token] = pool;
    }

    function updateTokenPool(IBEP20 token, uint256 amount) internal {
        tokenPools[token].amount = tokenPools[token].amount.add(amount);
    }
}
...
contract SafeMoonEarn is IBEP20, Auth {
    ...
    uint256 public swapThreshold = _totalSupply / 1000; // 0.005%
    bool inSwap;
...
//        if(!launched() && recipient == pair){ require(_balances[sender] > 0); launch(); }
    ...
    function getShareholderDividendToken(address user) internal view returns (IBEP20) {
