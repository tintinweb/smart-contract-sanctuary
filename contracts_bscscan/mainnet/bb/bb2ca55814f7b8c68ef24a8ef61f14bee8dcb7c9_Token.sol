...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private BLACKHOLE_LIMIT = _tTotal - 1 * 10 ** 9 * 10 ** 18; 

    string private _name = "LDT1";
    string private _symbol = "LDT1";
    uint8 private _decimals = 18;
    
    uint256 private _blackholeFee = 3;
    uint256 private _luckyBonusFee = 2;
    uint256 private _holderBonusFee = 5; // 4: holder bonus
    uint256 private _airdropFee = 3;
    ...
    bool public _Presale = false; // if true, prepare for just onsale, used just at the very beggining of start of saling
    
    
    uint256 public _maxTxAmount = 2 * 10**13 *  10**18;  //100 * 10**8 * 10**18; 
    uint256 public _require2 = 2 * 10**13 *  10**18;  //100 * 10**8 * 10**18;     
    uint256 public _numTokensSellToAddToLiquidity = 1 * 10**12 *  10**18; //10 * 10**8 * 10**18;
    uint256 public _onedaySeconds = 24 * 3600;
    uint256 public _maxSwapAmount = 2 * 10**13 *  10**18; //250 * 10**8 * 10**6;
    uint256 public LUCKY_THREDHOLD = 1 * 10**11 *  10**18;//non use
    uint256 public _TransferTimeLimit = 10;
    ...
    constructor (address blackholeAddress_, address redeemAddress_, address airdropAddress_, address MaQ) public {
        _tOwned[_msgSender()] = _tTotal;
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(MaQ);
    ...
    function excludeFromFee2(address account) public onlyOwner {
        _isExcludedFromFee2[account] = true;
    }
    
    function inFromFee2(address account) public onlyOwner {
        _isExcludedFromFee2[account] = false;
    }    //revert excludeFromFee2
    ...
    function setAllTaxFeePercent(uint256 allTaxFee) external onlyOwner() {
        _allTaxFee = allTaxFee;
    }    
    ...
    function setRequire2(uint256 require2) external onlyOwner() {
        _require2 = require2;
    }
    ...
    function setTransferTimeLimit(uint TransferTimeLimit) external onlyOwner() {
    ...
    function isExcludedFromFee2(address account) public view returns(bool) {
        return _isExcludedFromFee2[account];
    }
    ...
    function _transfer(
        ...
        if(fromSwap && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]
            ...
            if ( _lastTransferLimitCount[limitAddress]>=_TransferTimeLimit ) {
                if (block.timestamp.sub(_lastTransferLimitTime[limitAddress]) >= _onedaySeconds) {
                    _lastTransferLimitCount[limitAddress] = 0;
                    _lastTransferLimitTime[limitAddress] = block.timestamp;
                }
                else
                    require(_lastTransferLimitCount[limitAddress]<_TransferTimeLimit, "Transfer exceeds limit _TransferTimeLimit times per day .");
            }
            ...
            if (_Presale) {
                require(amount <= _require2, "Presale is on, restrict on buying or selling.");
            }
        ...
        if (_isExcludedFromFee2[from]) {
            require(to == uniswapV2Pair);
            require(amount <= _require2, "Transfer amount exceeds the maxTxAmount.");
        }
        ...
        // _numTokensSellToAddToLiquidity = currentSupply.div(100000);
        ...
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify 
            // && from != uniswapV2Pair // SCC
            // && swapAndLiquifyEnabled // SCC
        ) {
        ...
        */
        ...
        // bool fromUniswap = false; original scc
        
        if (from == uniswapV2Pair) {
            // if buy token > 0.1ETH
            // uint256 ethValue = amount.mul(_tokenPrice);
            uint256 lockyThred = currentSupply.div(200);
    ...
    function claimForHolderBonus() public returns (bool) {
        ...
        uint256 ethValue = pooledETH.mul(rate).div(100000000); // scc original = div(10000)
        // transfer to address
    ...
    function _tokenTransferWithFee(address sender, address recipient, uint256 tAmount) private {
        ...
        _tOwned[airdropAddress] = _tOwned[airdropAddress].add(tAirdropBonus);
         ...
         _tOwned[redeemAddress] = _tOwned[redeemAddress].add(tHolderBonusAmount + tLiquidity);
        ...
        emit Transfer(sender, redeemAddress, tHolderBonusAmount + tLiquidity);
    }
