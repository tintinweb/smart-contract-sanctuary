...
contract LifeTimeCoinICO is Ownable {
  using SafeMath for uint256;
  TeamToken token;
  uint256 public constant RATE = 13698; // Number of tokens per Ether
  ...
  function isActive() public view returns (bool) {
    return (
        block.timestamp > 1642194840 && block.timestamp < 1644796800 && initialized == true// Goal must not already be reached
    );
  }
