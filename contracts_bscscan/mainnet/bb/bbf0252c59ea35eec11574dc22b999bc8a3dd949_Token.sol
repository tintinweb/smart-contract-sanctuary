...
contract Token is IBEP20, Auth {
    ...
    uint256 buyreflectionFee   = 12;
    uint256 buymarketingFee    = 0;
    uint256 public totalFeeBuy = 0;
    uint256 buyfeeDenominator  = 100;
    
    uint256 sellliquidityFee    = 3;
    uint256 sellreflectionFee   = 12;
    ...
    uint8 public buycooldownInterval = 10;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x4016256b7279E8A7fD47A53d64B778C02653a3d9;
    ...
    function setIsFeeExempt(address holder, bool _status) external authorized {
        isFeeExempt[holder] = _status;
    }

    function setIsTxLimitExempt(address holder, bool _status) external authorized {
        isTxLimitExempt[holder] = _status;
    }

    function setFeesBuy(uint256 _buyliquidityFee, uint256 _buyreflectionFee, uint256 _buymarketingFee) external authorized {
        buyliquidityFee = _buyliquidityFee;
        buyreflectionFee = _buyreflectionFee;
        buymarketingFee = _buymarketingFee;
        totalFeeBuy = _buyliquidityFee.add(_buyreflectionFee).add(_buymarketingFee);
    }

    function setFeesSell(uint256 _sellliquidityFee, uint256 _sellreflectionFee, uint256 _sellmarketingFee) external authorized {
        sellliquidityFee = _sellliquidityFee;
        sellreflectionFee = _sellreflectionFee;
        sellmarketingFee = _sellmarketingFee;
        totalFeeSell = _sellliquidityFee.add(_sellreflectionFee).add(_sellmarketingFee);
    }
    ...
    function cooldownEnabled(bool _buystatus, bool _sellstatus, uint8 _buyinterval) public onlyOwner {
        buyCooldownEnabled = _buystatus;
        sellCooldownEnabled = _sellstatus;
        buycooldownInterval = _buyinterval;
    }
    
    function setIsTimelockExempt(address holder, bool _status) external authorized {
        isTimelockExempt[holder] = _status;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // cooldown buy
        if(sender == pair && buyCooldownEnabled && !authorizations[recipient] && !isTimelockExempt[recipient]){
                require(buycooldown[recipient] < block.timestamp, "Please wait for cooldown between buys");
                buycooldown[recipient] = block.timestamp + buycooldownInterval;
        }
        
        // cooldown sell
        if(recipient == pair && sellCooldownEnabled && !authorizations[sender] && !isTimelockExempt[sender]){
                ...
                if (sellnumber[sender] == 0) {
                    ...
                    sellnumber[sender]++;
                    sellcooldown[sender] = block.timestamp + (4 hours);
                }
                else if (sellnumber[sender] == 2) {
                    sellnumber[sender]++;
                    sellcooldown[sender] = block.timestamp + (8 hours);
                }
        ...
        // cooldown transfer
        if(recipient != pair && sender != pair && sellCooldownEnabled && !authorizations[sender] && !isTimelockExempt[sender] && !authorizations[recipient] && !isTimelockExempt[recipient]){
                require(sellcooldown[sender] < block.timestamp,"Please wait for cooldown between sells");
                if(firstsell[sender] + (1 days) < block.timestamp){
                    sellnumber[sender] = 0;
                }
        ...
        // takefee buy
        if(sender == pair) {
            uint256 buyamountReceived = shouldTakeFee(sender) ? takeBuyFee(sender, amount) : amount;
            _balances[recipient] = _balances[recipient].add(buyamountReceived);
            
            emit Transfer(sender, recipient, buyamountReceived);
        }
        
        // takefee sell
        if(recipient == pair) {
            uint256 sellamountReceived = shouldTakeFee(sender) ? takeSellFee(sender, amount) : amount;
            _balances[recipient] = _balances[recipient].add(sellamountReceived);
            
            emit Transfer(sender, recipient, sellamountReceived);
        }
    ...
    function takeBuyFee(address sender, uint256 amount) internal returns (uint256) {
        uint256 buyfeeAmount = amount.mul(totalFeeBuy).div(buyfeeDenominator);
    ...
    function takeSellFee(address sender, uint256 amount) internal returns (uint256) {
        uint256 sellfeeAmount = amount.mul(totalFeeSell).div(sellfeeDenominator);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(dynamicLiquidityFee).div(totalFeeBuy).div(2);
