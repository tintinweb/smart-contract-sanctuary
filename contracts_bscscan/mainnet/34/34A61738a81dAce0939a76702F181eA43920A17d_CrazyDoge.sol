...
CrazyDoge is built upon the fundamentals of Buyback and increasing the investor's value
    
Main features are
    
1) 2% tax is collected and distributed to holders for HODLing
2) 9% buyback and donation tax is collected and 3% of it is sent for donation fund and othe 6% is used to buyback the tokens
...
contract Ownable is Context {
	...
	modifier onlyStakingContract() {
		require(_stakingContract == _msgSender(), "Ownable: mintTo is only for staking");
		_;
	}
    
	function setStakingContract(address staking) public virtual onlyOwner {
...
contract CrazyDoge is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x56b1A053CBA69C197B3BD6530f8eae4c0598f55C); // Marketing Address
    ...
    function isExcludedFromFee(address account) public view returns(bool) {
        return ((_isExcludedFromFee[account]) || (account == stakingContract()));
    }
	...
	function mintTo(address to, uint256 tokens) public onlyStakingContract {
        (uint256 rAmount,,,,,) = _getValues(tokens);
		_tTotal = _tTotal.add(tokens);
		_rTotal = _rTotal.add(rAmount);
		_rOwned[to] = _rOwned[to].add(rAmount);
		_tOwned[to] = _tOwned[to].add(tokens);
