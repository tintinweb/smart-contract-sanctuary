...
contract SafemoonSwapper is Context, Ownable {
  ...
  function swapForSafemoon(address sender, uint256 value) internal {
    ...
    // Uniswap Pair Path for BNB -> Safemoon V1
    address[] memory pathv1 = new address[](2);
    pathv1[0] = uniswapV1Router.WETH();
    pathv1[1] = _safemoonAddr;
    
    // Uniswap Pair Path for BNB -> Safemoon V2
    address[] memory pathv2 = new address[](2);
    pathv2[0] = uniswapV2Router.WETH();
    pathv2[1] = _safemoonAddr;
    
    uint256 v1Amt = uniswapV1Router.getAmountsOut(value, pathv1)[0];
    uint256 v2Amt = uniswapV2Router.getAmountsOut(value, pathv2)[0];
    
    if (v1Amt > v2Amt) {
        buySafemoonV1(sender, value);
    } else {
        buySafemoonV2(sender, value);
    }
     
  }
  
  function buySafemoonV2(address sender, uint256 value) internal {
  ...
  function buySafemoonV1(address sender, uint256 value) internal {
     // Uniswap Pair Path for BNB -> Token
    address[] memory path = new address[](2);
    ...
    // Swap BNB for Token
    try uniswapV1Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: value}(
        0, // accept as many tokens as we can
        path,
        sender, // Send To Recipient
        block.timestamp.add(30)
