...
contract CR7UNITED is Context, IERC20, Ownable {
    ...
    address payable marketingAddress = payable(0xFa722f9bD6515A7E4C77f2AbCA7C006C4af4c468); 

    string private _name = "CR7UNITED";
    ...
    uint256 public _maxTxAmount = _tTotal / 10000; // 0.1%
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount, uint256 BS) private {
        ...
        uint256 tFeeSave = _liquidityFee;
        if (BS==1) {
            _liquidityFee = _liquidityFee + _sellFee.div(10**2);
        } else {
    ...
    function _transfer(
        ...
        // max wallet code
        if (!authorizations[from] && to != address(this)  && to != address(deadAddress) && to != uniswapV2Pair){
