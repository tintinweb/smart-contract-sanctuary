...
contract TokenVesting is Ownable {
    ...
    constructor(
        ...
        require(_cliffDuration <= duration);
    ...
    function revoke(IERC20 token) public onlyOwner {
        require(revocable, "TokenVesting::revoke: cannot revoke");
        require(
    ...
    function setBeneficiary(address newBeneficiary) public {
        ...
        require(
            ...
            "TokenVesting::setBeneficiary: Same beneficiary address as old"
        );
        beneficiary = newBeneficiary;
...
contract TokenVestingFactory is Ownable {
    ...
    /// @notice ORK token address
    IERC20 public immutable vestedToken;

    /// @notice vesting mapping per beneficiary
    mapping(address => TokenVesting) public vestings;

    /// @notice An event thats emitted when fundee contract is deployed
    event VestingCreated(
        address indexed fundeeAddress,
        address indexed beneficiaryAddress,
        uint256 amount
    );

    /// @notice An event thats emitted when a fundee is revoked
    event RevokeVesting(address indexed fundeeAddress);

    /**
     * @notice Construct TokenVestingFactory Factory
     * @param _vestedToken The vested token address
     */
    constructor(IERC20 _vestedToken) {
        require(
            address(_vestedToken) != address(0),
            "TokenVestingFactory::constructor: vestedToken is the zero address"
        );
    ...
    function deployVesting(
        ...
        vestings[beneficiary] = vestingContract;

        if (amount > 0) {
            vestedToken.safeTransfer(vestingContractAddress, amount);
        }

        emit VestingCreated(vestingContractAddress, beneficiary, amount);
    }

    /**
     * @dev Revokes the tokens from someone and sends back to this contract
     * @param _tokenVesting address of the beneficiary vesting contract
     */
    function revokeFundeeTokens(TokenVesting _tokenVesting) external onlyOwner {
        _tokenVesting.revoke(vestedToken);
        emit RevokeVesting(address(_tokenVesting));
    }
