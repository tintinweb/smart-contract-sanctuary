...
contract Ology is Context, IERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "Ology";
    string private _symbol = "OLOGY";
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityFee(tAmount); // 3% Liquidity + 5%  reward
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);  // 90% token
    ...
    // Innovation for protocol by Ology Team
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 liquidityOLOGYRate = 3;

            uint256 tokenPiece = contractTokenBalance.mul(liquidityOLOGYRate).div(totalFee);
            ...
            uint256 bnbToBeAddedToLiquidity = deltaBalance.mul(liquidityOLOGYRate).div(swapBNBRate);
    ...
    function activateContract() public onlyOwner {
        ...
        rewardCycleBlock = 1 hours;
        easyRewardCycleBlock = 1 hours;
