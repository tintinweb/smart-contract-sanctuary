...
/**
//     ALPHA BUSD  https://alphabusd.one/
//        
//     Telegram : https://t.me/alphabusdbsc
//
//     Join the Developers, Buy the token, and get paid in BUSD every 15 minute as long as you are holding
//
//     Tokenomics:
//     10% BUSD reflection
...
// File: contracts/AlphaBUSDDividendPayingToken.sol
...
contract AlphaBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    AlphaBUSDDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("Alpha BUSD", "ALD") {
    	...
    	dividendTracker = new AlphaBUSDDividendTracker();

    	liquidityWallet = owner();


    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AlphaBUSD: The dividend tracker already has that address");

        AlphaBUSDDividendTracker newDividendTracker = AlphaBUSDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AlphaBUSD: The new dividend tracker must be owned by the AlphaBUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AlphaBUSD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AlphaBUSD: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AlphaBUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AlphaBUSD: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "AlphaBUSD: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AlphaBUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AlphaBUSD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "AlphaBUSD: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of AlphaBUSD
            path,
            recipient,
            block.timestamp
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
       uniswapV2Router.addLiquidityBNB{value: ethAmount}(
            address(this),
            tokenAmount,
...
contract AlphaBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("AlphaBUSD_Dividend_Tracker", "AlphaBUSD_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "AlphaBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "AlphaBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AlphaBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "AlphaBUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AlphaBUSD_Dividend_Tracker: Cannot update claimWait to same value");
