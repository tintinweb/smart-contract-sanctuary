...
contract Presale is Ownable,ReentrancyGuard {
    ...
    address[] public receivingPresaleWalletAddress;
    ...
    function buyFromPresale(address receivingTokenWallet_) external payable nonReentrant{
        require(saleStarted, "Presale not started");
        ...
        require(!isHardCapEnabled || totalPayments + msg.value <= hardcap,"Error: hardcap exceeded");
        require(!isWhiteListEnabled || isWhiteListed[_msgSender()],"Error: you are not whitelisted");
        // 
        if (Receiving[receivingTokenWallet_] == 0) {
            receivingPresaleWalletAddress.push(receivingTokenWallet_);
        }
        totalPayments += msg.value;
        Deposits[_msgSender()] += msg.value;
        Receiving[receivingTokenWallet_] += msg.value;
    }

    function ejectBnb() public onlyOwner nonReentrant {
    ...
    function setMaxAmountPerAddress(uint256 _amount) external onlyOwner{
        require(_amount > 0, "amount must be greater than 0");
        maxAmountPerAddress = _amount;
    }
    ...
    function setIsHardCapEnabled(bool _status) public onlyOwner{
        require(!_status || hardcap > 0, "set hardcap > 0 before enabling");
        isHardCapEnabled = _status;
    }

    function setHardCap(uint256 _hardcap) external onlyOwner{  
        require(!isHardCapEnabled || _hardcap > 0, "disable hardcap or set hardcap > 0");
        hardcap = _hardcap;
    }
    ...
    function getAllReceivingAddress() external view returns (address[] memory){
        return receivingPresaleWalletAddress;
    }
}
