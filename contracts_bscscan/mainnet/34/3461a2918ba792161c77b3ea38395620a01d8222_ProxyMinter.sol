...
// File contracts/bsc/ProxyMinter.sol
...
contract ProxyMinter is Ownable {
    ...
    constructor(address token, address bridge_) Ownable() {
        bridge = bridge_;
    ...
    function setBridgeAddress(address bridge_) public onlyOwner {
        bridge = bridge_;
    }

    function withdrawERC20(address token, address recipient, uint256 amount) public onlyOwner {
        require(IERC20(token).transfer(recipient, amount), "ProxyMinter: Something went wrong with ERC20 withdrawal");
    }
}