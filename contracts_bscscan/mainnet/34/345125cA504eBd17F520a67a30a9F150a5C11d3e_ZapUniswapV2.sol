...
contract ZapUniswapV2 {
    ...
    function _checkSwapTarget(
        ...
        if (UniswapV2Library.pairFor(factory, token0, tokenIn) != address(0)) {
            (token0Reserve, tokenIn0Reserve) = UniswapV2Library.getReserves(factory, token0, tokenIn);
        }
        if (UniswapV2Library.pairFor(factory, token1, tokenIn) != address(0)) {
            (token1Reserve, tokenIn1Reserve) = UniswapV2Library.getReserves(factory, token1, tokenIn);
        }
