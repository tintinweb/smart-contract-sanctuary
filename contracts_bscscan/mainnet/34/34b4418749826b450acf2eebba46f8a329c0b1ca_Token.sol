...
contract Token is Context, IERC20, Ownable {
    ...
    constructor () payable {
        ...
        _marketingWallet = payable(0xb087eef437B88efc4ee8B8D099AD2fF957f635a2);
    ...
    function setWallet(address payable _new_marketingWallet, address _new_liquidityWallet) external onlyOwner {
        _marketingWallet = payable(_new_marketingWallet);
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        uint256 _take_amount = _amount.mul(_liquidityFee.add(_marketingFee)).div(_feeDivisor);
        
        if (_isseller) {
            _take_amount = _amount.mul(_liquidityFee.add(_marketingFee).add(_extrasellFee)).div(_feeDivisor);
        }
        
        if (botProtection) {
            if (block.timestamp <= botlaunchTimestamp + botFeeTime) {
                _take_amount = _amount.mul(_liquidityFee.add(_marketingFee).add(_extrasellFee).add(calculatetotalFeeBot())).div(_feeDivisor);
            }
        }
