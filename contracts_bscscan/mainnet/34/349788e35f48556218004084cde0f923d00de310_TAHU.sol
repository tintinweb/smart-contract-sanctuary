...
contract TAHU  is Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private _rOwned; 
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
	address payable public TAHU3DNFT = payable(0x9a283aecD008ae7983398D7069a0d3D1fb78657a);
    ...
    string private _name = "TAHU";
    string private _symbol = "TAHU";	
    uint8 private _decimals = 18;
    
    uint256 private _RestributionFee = 0;
    uint256 private _previousReflection = _RestributionFee;

    uint256 private LiquidityFee = 5;
    uint256 private _previousLiquidityFee = LiquidityFee;
	
	uint256 public BuyCooldownTime = 3;
	
    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;
    
    bool inSwapAndLiquify;
    bool public enableNFT = false;
    bool public antiBot = true;
    bool public buyCooldown = true;
    
    
    uint256 private _maxBuyAmount = 10000000 * 10**18;
    uint256 private minimumTokensBeforeSwap = 200000 * 10**18;
    
    event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
    event enableNFTUpdated(bool enabled);
    ...
    function setenableNFT(bool _enabled) public onlyOwner {
        enableNFT = _enabled;
        emit enableNFTUpdated(_enabled);
    }
   ...
   //this method is responsible for taking all fee, if takeFee is true
    ...
    function calculateReflection(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_RestributionFee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_RestributionFee == 0 && LiquidityFee == 0) return;
        
        _previousReflection = _RestributionFee;
    ...
    function restoreAllFee() private {
        _RestributionFee = _previousReflection;
