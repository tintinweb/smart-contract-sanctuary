...
contract BSCWinBulls is Context, ERC20, Ownable, Proxiable, DataLayout {
    ...
    function isTimeForLottery() public view returns(bool) {
        ...
        for (uint i; i < lotteryTimes.length; i++) {
            require(lotteryTimes[i] != 0);
        }
    ...
    function triggerLottery() external {
        ...
        require(lastCheckBlock[msg.sender] < block.number, "Cannot trigger twice in the same block");
        lastCheckBlock[msg.sender] = block.number;
    ...
    function getRandomNumber(uint256 upperNumber, bool shuffle) internal view returns(uint256) {
        ...
        uint256 callerBNBBalance = msg.sender.balance;
    ...
    function _rebalanceTickets(address user) internal {
        ...
        if (_isExcludedFromLottery[user] || user == cakePair || user == address(this) || 
    ...
    function excludeFromLottery(address user, bool allowed) public onlyOwner {
        _isExcludedFromLottery[user] = allowed;
        //set the amount of tickets the user has based on balance
        if (balanceOf(user) > 0) {
            userTickets[user] = balanceOf(user).div(ticketDivider);
        }
        //add user to list if they do not have an ID and have minimum tickets
        if (allowed && userId[user] == 0 && balanceOf(user) >= minimumLotteryBalance) {
            userId[user] = topUserId.add(1);
            topUserId = topUserId.add(1);
            idAddress[uint32(topUserId)] = user;
        }
        //remove user from list if minimum tickets not met
        if (!allowed && balanceOf(user) < minimumLotteryBalance && userId[user] != 0) {
            uint32 availableId = uint32(userId[user]);
            userId[user] = 0;
            userId[idAddress[uint32(topUserId)]] = uint32(availableId);
            idAddress[availableId] = idAddress[uint32(topUserId)];
    ...
    function _transfer(
        ...
        // also, don't swap & liquify if sender is cake pair.
        ...
        if (!inSwapAndLiquify && to == cakePair) {
            if(contractTokenBalance > 0) {
                if(contractTokenBalance > balanceOf(cakePair).mul(_liquidityFee).div(100)) {
                    contractTokenBalance = balanceOf(cakePair).mul(_liquidityFee).div(100);
                }
        ...
        if((from==cakePair || to==cakePair) && !(_isExcludedFromFee[from] || _isExcludedFromFee[to])){
            takeFee = true;
        }
        ...
        //lastBuyBlock[msg.sender] = block.timestamp;
        // if (contractTokenBalance > 0) {
        //     _tokenTransfer(address(this),DAOWallet,contractTokenBalance,false);
        // }
    }

    function swapTokensForBUSD(uint256 tokenAmount) internal lockTheSwap {
        ...
        // make the swap
        cakeRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BUSD
            path,
            marketingWallet, // The contract
            block.timestamp
        );
