...
contract Strategy_ZOO_BUSD {
    ...
    function _swapExactETHForToken1(uint256 _amountETH) internal {
        if(_amountETH > 0){
            address[] memory path = new address[](2);
            path[0] = WBNB;
            path[1] = token1;
    ...
    function _addLiquidityFromETH(uint256 _amountETH) internal {
        ...
        _swapExactETHForToken1(_amountETH);
        _swapToken1ToToken0();
    ...
    function _swapWBNBToToken(address _router, address _token) internal {
        ...
        path[0] = WBNB;
        path[1] = _token;
        uint256 _balance = getBalanceOfToken(WBNB);
        if(_balance > 0) {
