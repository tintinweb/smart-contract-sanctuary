...
contract UniswapAnchoredView is UniswapConfig {
    ...
    constructor(OpenOraclePriceData priceData_,
        ...
        for (uint i = 0; i < configs.length; i++) {
            ...
            if (config.symbolHash == referenceAssetHash) {
                isCoreAnchored = config.priceSource == PriceSource.REPORTER || config.priceSource == PriceSource.UNISWAP;
            }
        }
    }
    ...
    function priceInternal(TokenConfig memory config) internal view returns (uint) {
        if (config.priceSource == PriceSource.REPORTER || config.priceSource == PriceSource.UNISWAP || config.priceSource == PriceSource.SIGNED_ONLY) return prices[config.symbolHash];
    ...
    function postPriceInternal(string memory symbol, uint referenceAssetPrice) internal {
        ...
        require(config.priceSource == PriceSource.REPORTER ||
                ...
                config.priceSource == PriceSource.SIGNED_ONLY, "only reporter, uniswap or signed-only prices get posted");
...
//    }
    ...
    function fetchreferenceAssetPrice() internal returns (uint) {
        if (isCoreAnchored) {
            return fetchAnchorPrice(referenceAssetSymbol, getTokenConfigBySymbolHash(referenceAssetHash), ethBaseUnit);
        } else {
            return priceData.getPrice(reporter, referenceAssetSymbol);
        }
    }
