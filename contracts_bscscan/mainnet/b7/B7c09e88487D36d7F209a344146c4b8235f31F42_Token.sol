...
contract Token is Context, IERC20, Ownable {
    ...
    event IncludeFromReward(address account);
    event IncludeInFee(address account);
    event SetBuybackUpperLimit(uint256 buyBackLimit);
    ...
    function setBuybackUpperLimit(uint256 buyBackLimit) external onlyOwner() {
        ...
        emit SetBuybackUpperLimit(buyBackLimit);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(_burnFee != 0){
            spentAmount  = contractTokenBalance.mul(_burnFee).div(totFee);
            _tokenTransferNoFee(address(this), dead, spentAmount);
            totSpentAmount = spentAmount;
        }

        if(_walletFee != 0){
            spentAmount = contractTokenBalance.mul(_walletFee).div(totFee);
            _tokenTransferNoFee(address(this), feeWallet, spentAmount);
            totSpentAmount = totSpentAmount + spentAmount;
        }

        if(_buybackFee != 0){
            spentAmount = contractTokenBalance.mul(_buybackFee).div(totFee);
