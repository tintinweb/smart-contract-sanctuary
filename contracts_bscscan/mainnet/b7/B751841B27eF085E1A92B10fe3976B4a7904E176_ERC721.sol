...
contract ERC721 is Context ,ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    address public systemAddress = 0xE0BD740De91581240D70674a03238893aE95C991;
    ...
    /**
     * @dev Method for migrating tokens from an old contract to a new one.
     */
    function migrateTokenToNewConctract(uint fromIndex, uint toIndex) public onlyOwner() {

        address NAFTY_CONTRACT_VERSION_1 = 0xc753316ee782030C2E898e0bbacFB1DbAC31eaA7;

        for (uint i = fromIndex; i < toIndex; i++) {
            address owner = IERC721(NAFTY_CONTRACT_VERSION_1).ownerOf(i);
            if(owner == address(0)) revert();

            uint256 tokenId = totalSupply();
            _safeMint(owner, tokenId);
            tokenOfCreator[tokenId] = owner;
    ...
    // Token name
    string private _name='Nafty';

    // Token symbol
    string private _symbol='NAFTY';
