...
/* contacts let = "name crystal" id =1000000000000000 
getAmountOut(crystal)
...
// File: contracts\crystal.sol

contract CRYSTAL is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	uint256 private constant MAX = ~uint256(0);
	
	// MAIN TOKEN PROPERTIES
	string private constant _name = "CRYSTAL";
	...
	uint256 private constant _totalTokens = 1000000000 * 10**_decimals;	//1 million total supply
	...
	// CHARITY
	address payable private constant _charityAddress = payable(0x72b1A74b1b617F2E2CaB9b3737daD09356dcEa45); // A percentage of the BNB pool will go to the charity address
	...
	// BUYBACK
	address payable private constant _buybackAddress = payable(0x9F05bB074C3599E76Be4f2F233339e53F91CF5FB); // A percentage of the BNB pool will go to the charity address
	...
	// TEAM
	address payable private constant _teamAddress = payable(0x9F05bB074C3599E76Be4f2F233339e53F91CF5FB);
	...
	constructor (address routerAddress) {
		...
		uint256 _tokenForLiquidity = _totalTokens * 27 / 100;
		_balances[_msgSender()] = _tokenForLiquidity * currentRate;
		emit Transfer(address(0), _msgSender(), _tokenForLiquidity);
		
		uint256 _tokenForTeam = _totalTokens * 3 / 100;
		_balances[_teamAddress] = _tokenForTeam * currentRate;
		emit Transfer(address(0), _teamAddress, _tokenForTeam);
		
		uint256 _tokenForBurn = _totalTokens * 70 / 100;
		_balances[_burnWallet] = _tokenForBurn * currentRate;
		emit Transfer(address(0), _burnWallet, _tokenForBurn);
	...
	function activate() public onlyOwner {
		_isSwapEnabled = true;
		_isFeeEnabled = true;
		setTransactionLimit(20); // only 1% of the total supply can be exchanged at once
