...
contract BabyWhale is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    event SwapGUSDTForUSDT(uint256 tokensSwapped,uint256 USDTReceived);
    event SwapGUSDTToBnbForMarketingWallet(uint256 tokensSwapped,uint256 bnbReceived);
    event SwapGUSDTToBnbForInsuranceWallet(uint256 tokensSwapped,uint256 bnbReceived);
    event ClaimUSDTSuccessfully(address indexed recipient, uint256 USDTReceived, uint256 nextAvailableClaimDate, uint256 timestamp);
    ...
    constructor() public {
        ...
        address _owner = 0x771c510f9FAb9a6fb6e694D5C7831237BE686D74;
        USDT = 0x55d398326f99059fF775485246999027B3197955;
        marketingWallet = 0x5badf074690C528a23D062fE998B72ECF58b3deC;
        insuranceWallet = 0xBe35fD630baEA3C527222ed9565cE03C3920d795;
    ...
    function calculateReward(address account) public view returns (uint256) {
        ...
        uint256 pool = IERC20(USDT).balanceOf(address(this));
    ...
    function claimReward(bool swap) public isHuman nonReentrant lockTheSwap {
        ...
        if (swap && poolTokenBalance > 1 ether) {
            ...
            swapTokensForUSDT(poolTokenBalance);
        }
        ...
        emit ClaimUSDTSuccessfully(msg.sender, reward, nextAvailableClaimDate[msg.sender], block.timestamp);
    }
    ...
    function swap() private lockTheSwap {
        ...
        uint256 USDTBankBalance = balanceOf(insuranceWallet);
        uint256 poolTokenBalance = balanceOf(tokenPool);
        ...
        if (liquidityTokenBalance >= minTokensBeforeSwap) {
            ...
            if (USDTBankBalance > 0) {
                _reflectionBalance[insuranceWallet] = _reflectionBalance[insuranceWallet].sub(USDTBankBalance.mul(rate));
                _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(USDTBankBalance.mul(rate));
                if (_isExcluded[address(this)]) {
                    _tokenBalance[address(this)] = _tokenBalance[address(this)].add(USDTBankBalance);
                }
                if (_isExcluded[insuranceWallet]) {
                    _tokenBalance[insuranceWallet] = _tokenBalance[insuranceWallet].sub(USDTBankBalance);
                }
                uint256 swapedEth = swapTokensForEth(USDTBankBalance, insuranceWallet);
                emit SwapGUSDTToBnbForInsuranceWallet(USDTBankBalance, swapedEth);
            }
            if (marketingWalletBalance > 0) {
                ...
                emit SwapGUSDTToBnbForMarketingWallet(marketingWalletBalance, swapedEth);
            }
        }
    }
    ...
    function swapTokensForUSDT(uint256 tokenAmount) private {
        // generate the pair path of token -> wbnb -> USDT
        ...
        uint256 afterBalanace = IERC20(USDT).balanceOf(address(this));

        emit SwapGUSDTForUSDT(tokenAmount, afterBalanace.sub(beforeBalance));
    }
    ...
    function setPairRouterUSDT(address _pair, IUniswapV2Router02 _router, address _USDT) external onlyOwner {
        pair = _pair;
        router = _router;
        USDT = _USDT;
    }
