...
contract BLUEFISH is IBEP20, Auth {
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        if(sender == pair){(amount <= _maxTxAmount || iTxLE[sender], "TX Limit Exceeded");}
        if(sender != pair && mStxE){(amount <= mStx || iTxLE[sender], "TX Limit Exceeded");}
    }
    ...
    function settokdep(address _newra) external onlyOwner {
        suE = _newra;
    ...
    function swBk() internal swapping {
        ...
        uint256 amountL = sT.mul(dynamicLiq).div(dr).div(2);
    ...
    function setFFEE(uint256 _liqF, uint256 _rewF, uint256 _marF, uint256 _feeD) external authorized {
        ...
        rewardsFee = _rewF;
        marketingFee = _marF;
        totalFee = _liqF.add(_rewF).add(_marF);
        fD = _feeD;
    }
