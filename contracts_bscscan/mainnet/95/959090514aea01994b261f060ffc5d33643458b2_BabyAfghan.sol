...
Telegram: https://t.me/babyafghan
Website: https://babyafghan.xyz/

Baby Afghanistan, Together we can make a difference.
...
contract BabyAfghan is Context, IERC20, Ownable {
    ...
    address payable public MarketingCharityAddress = payable(0x73Ab97FB64c0a78f794de6478Fb68cDFA77E0152);
    ...
    string private _name = "BabyAfghan";
    string private _symbol = "BAFG";
    uint8 private _decimals = 9;

    uint256 public _MarketingCharityFee = 40;
    uint256 private _previousMarketingCharityFee = _MarketingCharityFee;
    ...
    uint256 private _liquidityFee = _MarketingCharityFee.add(_crctLiquidityFee);
    ...
    function toggleMaxTx(address account, bool status) external onlyOwner {
        _isExcludedFromMaxTX[account] = status;
    }
    ...
    function setMarketingCharityFeePercent(uint256 Fee) external onlyOwner {
        _MarketingCharityFee = Fee;
        _liquidityFee = _crctLiquidityFee.add(_MarketingCharityFee);
    }
    ...
    function setMaxTxTokens(uint256 maxTxTokens) external onlyOwner {
        _maxTxAmount = maxTxTokens.mul(10**9);
    }

    function setMarketingCharityAddress(address payable _MarketingCharityAddress)
        external
        onlyOwner
    {
        MarketingCharityAddress = _MarketingCharityAddress;
    }
    ...
    function removeAllFee() private {
        ...
        _previousMarketingCharityFee = _MarketingCharityFee; // MarketingCharity fee
    ...
    function restoreAllFee() private {
        ...
        _MarketingCharityFee = _previousMarketingCharityFee; // MarketingCharity fee
    ...
    function setMinTokensSendToMarketingCharity(uint256 minMarketingCharityValue)
        public
        onlyOwner
    {
        numTokensSellToAddToLiquidity = minMarketingCharityValue;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // first split contract into MarketingCharity fee and liquidity fee
        uint256 liquidityAmount = contractTokenBalance;
        uint256 initialBalance = address(this).balance;

        if (_MarketingCharityFee > 0) {
            uint256 MarketingCharityAmount = contractTokenBalance.mul(_MarketingCharityFee).div(
                _liquidityFee
            );
            liquidityAmount = contractTokenBalance.sub(MarketingCharityAmount);
            swapTokensForEth(MarketingCharityAmount);

            transferForMarketingCharityETH(
                MarketingCharityAddress,
    ...
    function transferForMarketingCharityETH(address payable recipient, uint256 amount)
        private
    {
        require(recipient != address(0), "BEP20: MarketingCharity to the zero address");
