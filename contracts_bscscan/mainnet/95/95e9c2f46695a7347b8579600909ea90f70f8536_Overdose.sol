 ...
 *
 *
 *      Overdose (mETH) made by https://t.me/ThePillarsOfCreation
 *      ---------------------------------------------------------
 ...
 *      2% Liquidity
 *      ---------------------------------------------------------
 *      https://pillars.community/overdose
 *      https://t.me/NoNameToken
 *
 */
...
/* Token contract */
contract Overdose is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    // Name and symbol
    string constant _name = "Overdose";
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 50 / 10000; // 0.5 on launch or 500k tokens
    ...
    // Overdose
    uint256 overdoseTriggeredAt;
    uint256 overdoseDuration = 3600;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in overdoseTime
        bool overdoseMode = inOverdoseTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, overdoseMode) : amount;
    ...
    // Check if we are in overdose time
    function inOverdoseTime() public view returns (bool){
        if(overdoseTriggeredAt.add(overdoseDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the overdose Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool overdoseMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are overdosed
        if (overdoseMode){
            if(isSell){
    ...
    // Enable overdose
    function enableOverdose(uint256 _seconds) public authorized {
        overdoseTriggeredAt = block.timestamp;
        overdoseDuration = _seconds;
    }

    // Disable the overdose mode
    function disableOverdose() external authorized {
        overdoseTriggeredAt = 0;
    }
