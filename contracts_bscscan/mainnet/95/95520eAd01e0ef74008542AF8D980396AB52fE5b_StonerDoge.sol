...
contract StonerDoge is Context, IERC20, Ownable {
    ...
    string private _name = "Stoner Doge";
    ...
    address public marketingWallet = 0xB80243E26D87fD8b5d76a4337bc612224E4D1232;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(markketingAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(markketingAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(markketingAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(markketingAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(markketingAmt)));
        }
