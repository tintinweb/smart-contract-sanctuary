...
contract PoolAllocator is Auth {
    ...
    uint allocAccuracy = 1 ether * 1 ether;
    ...
    uint marketingFee = 100; // Due to technical hurdles, this contract now handles the marketing fee, which is 1% of the allocated to the RBLOs. (So now MKT fee is lowered)
    uint allocatorIncentive = 100;
    uint allocatorAcc = 100000;
    uint allocatorMultiplier = 1 ether;


    uint balanceBeforeDistribution;
    //Gives incentives for distributing the bnb amongst rblos.
    function distribute() public {
        require(address(this).balance > 0, "Too late, someone else distributed :(");

        uint incentive = (allocatorIncentive * allocatorMultiplier) * address(this).balance / allocatorAcc / allocatorMultiplier;
        uint mkt = (marketingFee * allocatorMultiplier) * address(this).balance / allocatorAcc / allocatorMultiplier;

        balanceBeforeDistribution = address(this).balance - incentive - mkt;
        for (uint i = 0; i < activePools.length; i++) {
            if (activePools[i].sc != address(0))
                _liquifyPool(activePools[i]);
        }
        payable(msg.sender).call{value:incentive}("");
        payable(marketingAddress).call{value:mkt}("");
    }

    function _liquifyPool(Pool memory _pool) internal {
        //Pools handle the logic of liquifyinxg themselves on receive
        uint amt = _pool.alloc * allocAccuracy / totalAllocs * balanceBeforeDistribution / allocAccuracy;
        address (_pool.sc).call{value:amt}("");
    }


    function addActivePool(address _poolHash, uint alloc) external authorized {
        //TODO Automatize reflection fees
    ...
    function setAllocatorIncentive(uint incentive) external authorized {
        require(incentive < 1000);
...
import "./ReflectionLocker.sol";
import "./SafeEarn.sol";

contract StakingContract is IReflectionPool, Ownable {
    using SafeMath for uint256;

    IBEP20 public rewardsToken;

    mapping (address => bool) excludeSwapperRole;
    mapping (address => ReflectionLocker) lockers;
    ...
    SafeEarn safeEarn = SafeEarn(payable(0x099f551eA3cb85707cAc6ac507cBc36C96eC64Ff));
    ...
    constructor (address _router, IBEP20 _rewardsToken, IBEP20 _stakingToken) {
    ...
    function enterStaking(uint256 amount) external {
        require(amount < tokenPool.stakingToken.balanceOf(msg.sender), "Insufficient balance");
        ...
        tokenPool.stakingToken.transferFrom(msg.sender, address(this), amount);

        // Create a reflection locker for type A pool
        if (address(tokenPool.stakingToken) == address(safeEarn)) {
            ReflectionLocker locker = address(lockers[msg.sender]) == address (0) ? new ReflectionLocker(msg.sender) : lockers[msg.sender];
            lockers[msg.sender] = locker;
            safeEarn.setIsFeeExempt(address(lockers[msg.sender]), true);
            safeEarn.transfer(address(lockers[msg.sender]), amount);
        }
    ...
    function leaveStaking() external {
        require(shares[msg.sender].amount > 0, "You are not currently staking.......... geez");
        giveStakingReward(msg.sender);
        uint leaveAmt = shares[msg.sender].amount;
        removeShareHolder();

        // Get rewards from locker
        if (address(tokenPool.stakingToken) == address(safeEarn)) {
           lockers[msg.sender].claimAll();
        } else {
            tokenPool.stakingToken.transfer(msg.sender, leaveAmt);
        }
    }

    function harvest() external {
        require(getUnpaidEarnings(msg.sender) > 0, "No earnings ser, village need SafeEarn staked");
    ...
    // Update pool shares and user data on the vault
...
contract ReflectionLocker is IReflectionLocker {
    ...
    constructor (address _lockOwner) public {
        lockOwner = _lockOwner;

    }

    modifier onlyLockOwner {
        require(tx.origin == lockOwner || msg.sender == address(rblo), "Fuck off.");
        _;
    }

    // Amt 0 is claim all
    function claimTokens(uint amt) external override onlyLockOwner returns (uint) {
        if (amt == 0) {
            amt = safeEarn.balanceOf(address(this));
            safeEarn.transfer(msg.sender, amt);
        }
        else {
            safeEarn.transfer(msg.sender, amt);
        }
    ...
    function claimReflections() external override onlyLockOwner returns (uint) {
        _getFromDistributor();
    ...
    function claimAll() external override onlyLockOwner returns (uint, uint) {
        ...
        safeMoon.transfer(msg.sender, amtMoon);
