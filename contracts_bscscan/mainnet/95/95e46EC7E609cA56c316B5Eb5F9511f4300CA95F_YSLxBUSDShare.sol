...
contract YSLxBUSDShare is YSLOpt {
    using SafeERC20 for IERC20;
    ...
    function transferOut(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        // TODO add withdrawal
        revert("Transfer out is disabled yet");
    ...
    function transferIn(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        // Add liquidity
        uint256 liqAmount = _amount / 4; //25%
        // Calculate YSL emissions and add it together with BUSD to locked liquidity
        uint256 emission = getPrice(BUSD, _YSL, liqAmount, apeSwap);
        IsYSL(_YSL).mintFor(address(this), emission);
        // Get 25% of YSL
        IERC20(BUSD).approve(stratSwap, _amount / 4);
        IStrategySwap(stratSwap).swapBusdToLP(_amount / 4, apeSwap, path);


        uint256 lpBalanceBefore = IERC20(lpToken).balanceOf(address(this));
        addLiquidity(BUSD, _YSL, _amount / 2, IERC20(_YSL).balanceOf(address(this)), apeSwap, address(this));

        // Locked liquidity
        emission = getPrice(BUSD, _YSL, _amount / 2, apeSwap);
        IsYSL(_YSL).mintFor(address(this), emission);
        addLiquidity(BUSD, _YSL, _amount / 2, IERC20(_YSL).balanceOf(address(this)), apeSwap, address(this));

        uint256 lpBalanceAfter = IERC20(lpToken).balanceOf(address(this));
        IERC20(lpToken).safeTransfer(adapter, (lpBalanceAfter - lpBalanceBefore) / 2);
        return (lpBalanceAfter - lpBalanceBefore) / 2;
    }
