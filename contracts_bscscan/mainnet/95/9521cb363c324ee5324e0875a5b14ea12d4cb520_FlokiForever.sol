...
/**
    ...
    Missed FegX?? Don't miss FLOKI!! Get rewarded in $FLOKI for HODLing!!
	
	Telegram: https://t.me/FlokiForever
	Twitter: https://twitter.com/FLOKI
	Website: https://FLOKI.webflow.io/

    TOTAL FEES: 18%
    
    FLOKI Rewards: 7% fee
    Liquidity: 5% fee
    Marketing/Team: 6% fee (~2% to team)
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable FLOKI = address(0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4); //FLOKI
  ...
  function distributeFLOKIDividends(uint256 amount) public onlyOwner{
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IERC20(FLOKI).transfer(user, _withdrawableDividend);
...
contract FlokiForever is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FLOKIDividendTracker public dividendTracker;
    ...
    uint256 public FLOKIRewardsSplitValue = 7;
    uint256 public liquiditySplitValue = 5;
    uint256 public marketingSplitValue = 6;
    uint256 public bananaRewardSplitValue = 0;
    uint256 public feeDecimalFactor = 10;
    uint256 public buyingFeesWithDecimals = (FLOKIRewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    uint256 public sellingFeesWithDecimals = (FLOKIRewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    address public _marketingWalletAddress = 0xE796C388C70475CF1FE957cb0843dc225504688B;
    bool public hasLaunched;
    address public liquidityFeeReceiver = 0x5804307b687eB7502D8966f611a4a1f0cfd4f032;

    // use by default 300,000 gas to process auto-claiming dividends
    uint256 public gasForProcessing = 300000;
    uint256 public maxBuyTransactionAmount =  100000000 * (10**18);
    ...
    constructor() public ERC20("FlokiForever", "FlokiForever") {

    	dividendTracker = new FLOKIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FLOKI: The dividend tracker already has that address");

        FLOKIDividendTracker newDividendTracker = FLOKIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FLOKI: The new dividend tracker must be owned by the FLOKI token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FLOKI: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FLOKI: Account is already the value of 'excluded'");
    ...
    function setFLOKIRewardsSplitValue(uint256 value) external onlyOwner{
        require(value >= 1 && value <= 1000);
        FLOKIRewardsSplitValue = value;
    }

    function setLiquiditySplitValue(uint256 value) external onlyOwner{
        require(value >= 1 && value <= 1000);
        liquiditySplitValue = value;
    }
    
    function setMarketingSplitValue(uint256 value) external onlyOwner{
        require(value >= 1 && value <= 1000);
        marketingSplitValue = value;
    }

    function setBananaRewardSplitValue(uint256 banana) public onlyOwner{
        require(banana >= 1 && banana <= 1000);
        bananaRewardSplitValue = banana;
    }
    
    function setLiquidityFeeReceiverAddress(address liqFeeReceiver) public onlyOwner{
        liquidityFeeReceiver = liqFeeReceiver;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FLOKI: The PanFLOKISwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FLOKI: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FLOKI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FLOKI: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(totalBananas > contractTokenBalance){
		    totalBananas = 0;
		}else{
		    contractTokenBalance = contractTokenBalance - totalBananas;
		}
        ...
        if( canSwap &&
            ...
            uint256 marketingTokens = contractTokenBalance.mul(marketingSplitValue).div(FLOKIRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
            swapAndSendToFee(marketingTokens);

            uint256 swapTokens = contractTokenBalance.mul(liquiditySplitValue).div(FLOKIRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
            swapAndLiquify(swapTokens);

            totalBananas += contractTokenBalance.mul(bananaRewardSplitValue).div(FLOKIRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);

            uint256 sellTokens = contractTokenBalance.mul(FLOKIRewardsSplitValue).div(FLOKIRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
        ...
        if(takeFee) {
            ...
            if(autoSetMaxTX == true){
                if(launchTime + 15 seconds < block.timestamp){
	                maxBuyTransactionAmount = 1500000000 * (10**18);
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        uint256 initialFLOKIBalance = IERC20(FLOKI).balanceOf(address(this));

        swapTokensForFLOKI(tokens);
        uint256 newBalance = (IERC20(FLOKI).balanceOf(address(this))).sub(initialFLOKIBalance);
        IERC20(FLOKI).transfer(_marketingWalletAddress, newBalance);
    }
    ...
    function _APETIME(uint cPrice) private{
        if(isAPETIME == false){
	        ...
	        sellingFeesWithDecimals = 30 * feeDecimalFactor;
        }
    }
    ...
    function apeTimeMonitoring(uint256 _currPrice) private{
        if(ATH > 2 && _currPrice <= ATH / 2 && nextAPETIMEStartingAt == 0){
    ...
    function swapAndSendDividends(uint256 tokens) private{
        uint256 initialPanFLOKIBalance = IERC20(FLOKI).balanceOf(address(this));
        swapTokensForFLOKI(tokens);
        uint256 dividends = (IERC20(FLOKI).balanceOf(address(this))).sub(initialPanFLOKIBalance);
        bool success = IERC20(FLOKI).transfer(address(dividendTracker), dividends);

        if (success) {
            dividendTracker.distributeFLOKIDividends(dividends);
...
contract FLOKIDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("FLOKI_Dividend_Tracker", "FLOKI_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "FLOKI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "FLOKI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FLOKI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FLOKI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FLOKI_Dividend_Tracker: Cannot update claimWait to same value");
