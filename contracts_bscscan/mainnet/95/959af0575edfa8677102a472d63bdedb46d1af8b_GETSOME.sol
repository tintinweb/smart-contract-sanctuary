...
contract GETSOME is ERC20, Ownable {
	using SafeMath for uint256;
	...
	GETSOMEDividendTracker public dividendTracker;
	GETSOMETransferHelper private transferHelper;
	...
	constructor(address routerAddress, address payable marketingWalletAddress, address payable devWalletAddress) public ERC20("GETSOME", "GETSOME1") {
		dividendTracker = new GETSOMEDividendTracker();
		transferHelper = new GETSOMETransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "GETSOME: The dividend tracker already has that address");

		GETSOMEDividendTracker newDividendTracker = GETSOMEDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "GETSOME: The new dividend tracker must be owned by the GETSOME token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "GETSOME: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "GETSOME: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "GETSOME: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "GETSOME: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "GETSOME: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "GETSOME: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "GETSOME: Cannot update gasForProcessing to same value");
	...
	function _transfer(address _from, address _to, uint256 _amount) internal override {
		...
		if (takeFee) {
			...
			if (feePercent > 0) {
				if (block.number <= (launchedAt + 5) && automatedMarketMakerPairs[_from] && _to != address(uniswapV2Router) && _to != address(this) && _to != owner()) {
					feePercent = 90;
				}
...
contract GETSOMEDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("GETSOME_Dividend_Tracker", "GETSOME_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "GETSOME_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "GETSOME_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GETSOME contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "GETSOME_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "GETSOME_Dividend_Tracker: Cannot update claimWait to same value");
...
contract GETSOMETransferHelper is Ownable {
