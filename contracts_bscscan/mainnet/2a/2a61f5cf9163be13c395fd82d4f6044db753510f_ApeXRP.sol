...
contract ApeXRP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("ApeXRP", "ApeXRP") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xD0e78861d3c078F8F43628a85C254AC6c700C2e9;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "ApeXRP: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "ApeXRP: The new dividend tracker must be owned by the ApeXRP token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ApeXRP_Dividend_Tracker", "ApeXRP_Dividend_Tracker", 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ApeXRP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ApeXRP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "ApeXRP_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "ApeXRP_Dividend_Tracker: Cannot update claimWait to same value");
