...
contract OracleSimpleBNBUsdtBLP {
    using FixedPoint for *;

    uint256 public constant PERIOD = 12 hours;
    address public constant pair = address(0xDA8ceb724A06819c0A5cDb4304ea0cB27F8304cF);
    ...
    UniswapV2 biswap = UniswapV2(0x3a6d8cA21D1CF76F653A67577FA0D27453350dD8);
    uint256 public usdtAmount;
    uint256 public busdAmount;
    uint256 public lpAmount;
    
    constructor() public {
        usdtAmount = ERC20(usdt).balanceOf(pair);
        busdAmount = ERC20(busd).balanceOf(pair);
    ...
    function update() external {
       ...
       if (timeElapsed >= PERIOD) {
            ...
            path[0] = address(usdt);
            path[1] = address(busd);
            amounts = biswap.getAmountsOut((10 ** 18), path);
    ...
    function consultLP(address token) external view returns (uint, uint) {
        if (token == usdt) {
            return(usdtAmount, lpAmount);
        } else {
            require(token == busd);
            return(busdAmount, lpAmount);
        }
    }
}
