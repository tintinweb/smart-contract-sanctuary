 ...
 */
contract TimeLocklps is Ownable {
    using SafeERC20 for IERC20;

    // 2000 day timestamp when token release is enabled
    uint256 public releaseTime = 1628346191;
    
    address public BLRToken = 0xEc65A8d99438545C6E87Bf864ff2cEA7F6330bfB;
    address public lpToken = 0x1bEa3a263779Fb6D0a1f06449CD7cADEE1d8da47;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseBLRToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(BLRToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(BLRToken).safeTransfer(owner(), amount);
    }
    ...
    function getBLRTokenBalance() external view returns(uint256) {
        return IERC20(BLRToken).balanceOf(address(this));
    }
    ...
    function setBLRToken(address _BLRToken) external onlyOwner {
        BLRToken = _BLRToken;
    }
