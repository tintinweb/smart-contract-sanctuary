...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also NewYorkCakeed.
...
// File: contracts/NewYorkCakeDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable CAKEToken = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82);
  ...
  function distributeCAKETokenDividends(uint256 amount) public {
    require(totalSupply() > 0);
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IERC20(CAKEToken).transfer(user, _withdrawableDividend);
...
// File: contracts/NewYorkCake.sol
...
contract NewYorkCake is ERC20, Ownable {
    ...
    address public immutable CAKEToken = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // CAKEToken
	address payable public MarketingWallet = payable(0x3406bC28A0322ec42230A0120c7753Cd6a4E8c86); // Marketing Address
    ...
    uint256 private _FeeFactor = 100000000000 * (10**18); 
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "NewYorkCake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "NewYorkCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "NewYorkCake: The liquidity wallet is already this address");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "NewYorkCake: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForCAKEToken(uint256 tokenAmount, address recipient) private {
       
        // generate the PancakeSwap pair path of weth -> CAKEToken
        ...
        PancakeSwapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of CAKEToken
            path,
            recipient,
            block.timestamp
        );
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForCAKEToken(tokens, address(this));
        uint256 dividends = IERC20(CAKEToken).balanceOf(address(this));
        bool success = IERC20(CAKEToken).transfer(address(dividendTracker), dividends);
        
        if (success) {
            dividendTracker.distributeCAKETokenDividends(dividends);
...
contract NewYorkCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("NewYorkCake_Dividend_Tracker", "NewYorkCake_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "NewYorkCake_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "NewYorkCake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
