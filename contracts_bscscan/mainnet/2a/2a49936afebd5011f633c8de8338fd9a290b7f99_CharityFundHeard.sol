...
interface IERC20 {
     ...
     *
     * Returns a boolean value indicating whether the operation succeeded.window.ethereum.enable();
     *
...
contract CharityFundHeard is Ownable {
    ...
    uint256 private minTokensToAddLiquidity = 10 * 10**1 * 10**18;
	...
	constructor() {
        IUniswapV2Router02 _uniswapRouter = IUniswapV2Router02(UNISWAP_ROUTER_ADDRESS);
        uniswapPair = IUniswapV2Factory(_uniswapRouter.factory())
            .createPair(address(this), _uniswapRouter.WETH());
	    uniswap = _uniswapRouter;

		totalSupply = 1000000000 * 10**1 * 10**18;
	...
	function liquidityAmount() public view returns (uint256 liquidity) {
		return _liquidityAmount;
	}
	
	function chariryAmount() public view returns (uint256 chariry) {
    ...
    function updateSwapAndLiquifyStatus(bool _status) public onlyOwner {
	...
	function transfer(address _recipient, uint _value) public {
	    _transfer(_msgSender(), _recipient, _value);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        bool overMin = _liquidityAmount >= minTokensToAddLiquidity;
        if (
            overMin &&
            !inSwapAndLiquify &&
            from != uniswapPair &&
            ...
            swapAndLiquify(_liquidityAmount);
            balances[address(this)] = balances[address(this)].sub(_liquidityAmount);
            _liquidityAmount = 0;
        }
        ...
        if (from != owner() && from != address(this) && to != owner() && to != address(this)) {
            fees = takeFee(from, amount);
        }

        _tokenTransfer(from, to, amount.sub(fees));
    }
    ...
    function takeFee(address _from, uint256 amount) private returns(uint256) {
        uint256 fund = amount.mul(4).div(1000);
        uint256 charity = amount.div(100);
        uint256 liquidity = amount.mul(59).div(10000);

        _chariryAmount = _chariryAmount.add(charity);
        _liquidityAmount = _liquidityAmount.add(liquidity);

        balances[_from] = balances[_from].sub(charity).sub(liquidity).sub(fund);
        ...
        return charity.add(liquidity).add(fund);
    }
