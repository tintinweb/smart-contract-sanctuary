...
abstract contract YSLOpt is AccessControlUpgradeable, IStrategy {
    ...
    function collectControllerFee(uint256 _amount, address _router, address _feeAddress) internal {
        ...
        IPancakeRouter02(_router).swapExactTokensForETH(_amount, 1, _path, _feeAddress, block.timestamp + 10000);
    }
...
contract YSLOptApe is YSLOpt {
    ...
    function compound() external override {
        ...
        if (wantBalance > DUST) {
            ...
            collectControllerFee(usdBefore, apeSwap, 0xa03C61784D51565411DE238de0bfF5901E4d1477);
    ...
    function currentRewardUSD(address _user) public view returns (uint256) {
        ...
        uint256 totalCompoundsUsd = IStrategySwap(stratSwap).getBusdAmount(totalCompounds, lpToken, apeSwap, path0, path1);

        // Pending rewards
        uint256 pendingBanana = IPancakeMaster(apeMaster).pendingCake(id, address(this));
        address[] memory bananaPath = new address[](2);
        bananaPath[0] = address(want);
        bananaPath[1] = BUSD;
        totalCompoundsUsd += IStrategySwap(stratSwap).getBusdAmount(pendingBanana, bananaPath, apeSwap);
