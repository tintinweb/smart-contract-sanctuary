...
contract NEOMCoin is Context, IERC20, Ownable {
    ...
    string private _name = "NEOM Coin";
    string private _symbol = "NEOM";
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tDistribution, uint256 tMarketingAndBurn, uint256 tLiquidity) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rDistribution) = _getRValues(tAmount, tDistribution, tMarketingAndBurn, tLiquidity, _getRate());
        return (rAmount, rTransferAmount, rDistribution, tTransferAmount, tDistribution, tMarketingAndBurn, tLiquidity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tDistribution).sub(tMarketingAndBurn).sub(tLiquidity);
        return (tTransferAmount, tDistribution, tMarketingAndBurn, tLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 tDistribution, uint256 tMarketingAndBurn, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rDistribution).sub(rMarketingAndBurn).sub(rLiquidity);
    ...
    function removeAllFee() private {
        if(_distributionFee == 0 && _burnFee == 0 && _marketingFee == 0 && _liquidityFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rDistribution, uint256 tTransferAmount, uint256 tDistribution, uint256 tMarketingAndBurn, uint256 tLiquidity) = _getValues(tAmount);
