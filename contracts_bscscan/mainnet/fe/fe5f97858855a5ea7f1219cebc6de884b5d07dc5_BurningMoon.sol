...
contract BurningMoon is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    event TokenBurn(address indexed from, uint256 value);
    event SetLiquidityFee(uint256 amount);
    event SetInfinityFee(uint256 amount);
    event SetBurnFee(uint256 amount);
    
    string private _name = "Burning";
    string private _symbol = "MOON";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 8000000000 * 10**_decimals;
    
    address payable public infinityAddress = payable(0x32FD0aFc31d83cb83760251218566A4bB8182a10);
    address public infinityWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    uint256 private constant _maxFees = 20;
    uint256 private _totalFees;
    uint256 private _totalFeesToContract;
    uint256 private _liquidityFee;
    uint256 private _burnFee;
    uint256 private _infinityFee;
    ...
    constructor () {
        ...
        _infinityFee = 6;
        _burnFee = 9;
        _totalFees = _liquidityFee.add(_infinityFee).add(_burnFee);
        _totalFeesToContract = _liquidityFee.add(_infinityFee);

        _liquifyThreshhold = 8000000 * 10**_decimals;
        _maxBalance = 160000000 * 10**_decimals;
    ...
    function setInfinityAddr(address payable newInfinityAddress) external onlyOwner() {
        infinityAddress = newInfinityAddress;
    }

    function clearStuckBalance(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(infinityAddress).transfer(amountBNB * amountPercentage / 100);
    }  

    function setLiquidityFeePerc(uint256 newLiquidityFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(newLiquidityFee.add(_burnFee).add(_infinityFee) <= _maxFees, "Fees are too high.");
    ...
    function setInfinityFeePerc(uint256 newInfinityFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFee.add(_burnFee).add(newInfinityFee) <= _maxFees, "Fees are too high.");
        _infinityFee = newInfinityFee;
        _totalFees = _liquidityFee.add(_infinityFee).add(_burnFee);
        _totalFeesToContract = _liquidityFee.add(_infinityFee);
        emit SetInfinityFee(_infinityFee);
    }

    function setBurnFeePerc(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFee.add(newBurnFee).add(_infinityFee) <= _maxFees, "Fees are too high.");
        _burnFee = newBurnFee;
        _totalFees = _liquidityFee.add(_infinityFee).add(_burnFee);
    ...
    function setInfinityWalletToken(address _infinityWalletToken) external onlyOwner(){
        infinityWalletToken = _infinityWalletToken;
    }
    ...
    function infinityFee() public view returns (uint256) {
        return _infinityFee;
    }
    ...
    function _transfer(
        ...
        if(
            ...
            _totalFeesToContract > 0 &&                         // LiquidityFee + infinityFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 infinityTokensToSell = balanceOf(address(this)).mul(_infinityFee).div(_totalFeesToContract);
        
        // Get collected Liquidity Fees 
        swapAndLiquify(liquidityTokensToSell);  

        // Get collected Infinity Fees 
        swapAndSendToFee(infinityTokensToSell); 
    }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        swapTokensForInfinityToken(tokens);

        // Transfer sold Token to infinityWallet
        IERC20(infinityWalletToken).transfer(infinityAddress, IERC20(infinityWalletToken).balanceOf(address(this)));
    }
    ...
    function swapTokensForInfinityToken(uint256 tokenAmount) private {

        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = infinityWalletToken;
