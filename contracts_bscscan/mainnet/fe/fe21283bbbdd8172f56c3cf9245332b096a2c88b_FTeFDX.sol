...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 COIN = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
...
contract FTeFDX is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "FTeFDX";
    string constant _symbol = "$FTFDX";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10**12 * (10 ** _decimals); //
    uint256 public _maxbuyTxAmount = _totalSupply * 3 / 100; //
    ...
    uint8 public sellcooldownTimerInterval = 0;
    uint8 public buycooldownTimerInterval = 0;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x6b0A470035Bce1C369F7e13749710d4a71FED4AE;
    ...
    function checkTxLimit(address sender, address receiver, uint256 amount) internal view {
       ...
       if(sender == pair) {
         require(amount <= _maxbuyTxAmount || isTxLimitExempt[receiver], "TX Limit Exceeded");  
       }
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        // if sell, multiply by *
            if(receiver == pair) {
                feeAmount = feeAmount.mul(sellFeeIncreaseFactor).div(100);
            }
    ...
    function setTxLimit(uint256 _buyTx, uint256 _sellTx) external authorized {
        _maxbuyTxAmount = _buyTx;
        _maxsellTxAmount = _sellTx;
    }
    ...
    function setSMFee(uint256 feeAmount) external authorized {
