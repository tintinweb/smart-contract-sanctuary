...
contract LILUNIVERSE is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   LILUNIVERSEDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("LIL UNI VERSE", "LuniV") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 2;
        _buyMarketingFee = 6;
        _buyDevFee = 2;
        ...
        dividendTracker = new LILUNIVERSEDividendTracker();

        liquidityWallet = owner();
        
        marketingWallet = payable(0x3652366A4f206B9fC7210d6A1B22e0C0bB5F6851);
        _devWallet = payable(0xE80d80664a8F713EE05097b8d867B803342e15c3);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LIL UNI VERSE: The dividend tracker already has that address");

        LILUNIVERSEDividendTracker newDividendTracker = LILUNIVERSEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LIL UNI VERSE: The new dividend tracker must be owned by the LIL UNI VERSE token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "LIL UNI VERSE: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LIL UNI VERSE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LIL UNI VERSE: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "LIL UNI VERSE: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LIL UNI VERSE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LIL UNI VERSE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "LIL UNI VERSE: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "LIL UNI VERSE: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LIL UNI VERSE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LIL UNI VERSE: Cannot update gasForProcessing to same value");
...
contract LILUNIVERSEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(false, "CTG_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LIL UNI VERSE contract.");
    }
