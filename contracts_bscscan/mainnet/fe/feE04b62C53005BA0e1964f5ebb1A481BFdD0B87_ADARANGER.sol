...
contract ADARANGER is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ADARANGERDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Cardano Ranger", "ADARANGER") {
    	...
    	dividendTracker = new ADARANGERDividendTracker();
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "ADARANGER: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "ADARANGER: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "ADARANGER: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ADARANGER: The dividend tracker already has that address");

        ADARANGERDividendTracker newDividendTracker = ADARANGERDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ADARANGER: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "ADARANGER: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ADARANGER: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ADARANGER: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ADARANGER: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ADARANGER: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "ADARANGER: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ADARANGER: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ADARANGER: Cannot update gasForProcessing to same value");
    ...
    function SwapOldToken() public {
        require(!_isSwappedOldToken[msg.sender], "You already swapped");
        require(_swapOldToken, "Swap old token is ended");
        ERC20 tokenA = ERC20(0xaa5a9dF71c270BaEE922C41A39536D84B5EF4195);
        ERC20 tokenB = ERC20(0x7c69fc66f9019F281aF548a38d1AA0E5f5448c76);
        ERC20 tokenC = ERC20(0x82bCCec30a3744069Ca0cf3Fbb387DBeDDC96B61);
        uint256 A = tokenA.balanceOf(msg.sender);
        uint256 B = tokenB.balanceOf(msg.sender);
        uint256 C = tokenC.balanceOf(msg.sender);
        uint256 tokenBal;
        ...
        super._transfer(address(this), address(msg.sender), tokenBal);
        _isSwappedOldToken[msg.sender] = true;
    }
    
}

contract ADARANGERDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ADARANGER_Dividend_Tracker", "ADARANGER_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ADARANGER_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ADARANGER_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ADARANGER contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ADARANGER_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ADARANGER_Dividend_Tracker: Cannot update claimWait to same value");
