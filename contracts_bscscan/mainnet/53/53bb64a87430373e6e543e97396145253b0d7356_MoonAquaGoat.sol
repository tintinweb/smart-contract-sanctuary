...
/*
    ...
    MoonAquaGoat - the next big meme token (exclusively run by the community)
   
    This s a stealth launch project which will be run by the community.
    ...
    - Tax Buy: 15%
    - Tax Sell: 15%
    - 40% Burnt before launch
    - 10% BTC reflection
    
    MoonAquaGoat will start with 1 BNB in liquidity so no whales will be present.
    As more holders buy, you can then increase your buy orders.
    
    
    
    Welcome to MoonAquaGoat.
...
contract DividendDistributor is IDividendDistributor {
    ...
    address WBNB = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;
    IDEXRouter router;
    ...
    uint256 public minimumTokenBalanceForDividends = 1000000 * (10**9); // user must hold 1,000,000 token to receive Bitcoin
...
contract SafeToken is Ownable {
    ...
    function withdrawBitcoin(uint256 _amount) external {
        require(msg.sender == safeManager);
...
contract MoonAquaGoat is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "MoonAquaGoat";
    string constant _symbol = "MoonAQUAGOAT";
    uint8 constant _decimals = 9;
    uint256 public _totalSupply = 1000000000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply.mul(10).div(100); //Max tx one time is 1%
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountBitcoin = address(this).balance.sub(balanceBefore);
            uint256 amountBitcoinReflection = amountBitcoin.mul(reflectionFee).div(totalFee);
            uint256 amountBitcoinMarketing = amountBitcoin.mul(marketingFee).div(totalFee);

            try distributor.deposit{value: amountBitcoinReflection}() {} catch {}
            payable(marketing).call{value: amountBitcoinMarketing, gas: 30000}("");
    ...
    function buyBackTokens() private swapping {
        ...
        if (amount > 0) {
            swapBitcoinForTokens(amount);
        }
    }

    function swapBitcoinForTokens(uint256 amount) private {
        ...
        emit SwapBitcoinForTokens(amount, path);
    }
    ...
    event SwapBitcoinForTokens(uint256 amount, address[] path);
}