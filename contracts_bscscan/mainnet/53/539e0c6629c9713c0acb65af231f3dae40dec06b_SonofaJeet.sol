...
/**
 This token has been launched by Jeeters Launchpad. Find us at https://t.me/jeeterslaunchpad
 There are high taxes at launch to combat bots. Do not buy until given the word.
*/
...
contract SonofaJeet is Context, IERC20, Ownable {
    ...
    address payable private marketingWallet = payable(0x146e60CCFfa7d40A7004d0863A4858A1589b6A7a); // Marketing Wallet
    address payable private ecosystemWallet = payable(0x53b73157518959345192EA6f4Ba881b72051Ca90); // Ecosystem Wallet
    address payable private lotteryWallet = payable(0x5282AAc6F29589E73F61E063099C0270024aC29A); // Lottery Wallet
    ...
    string private _name = "Son of a Jeet";
    string private _symbol = "SOAJ";

    uint256 public _maxWalletToken = _tTotal.div(1000).mul(5); //0.5% for first few mins

// High taxes at launch
    uint256 public _buyLiquidityFee = 50;   //5%
    uint256 public _buyLotteryFee = 130;     //13% 
    uint256 public _buyMarketingFee = 130;   //13%
    uint256 public _buyReflectionFee = 0;   //0%

    uint256 public _sellLiquidityFee = 50; //50%
    uint256 public _sellMarketingFee = 130;  //13%
    uint256 public _sellLotteryFee = 130;   //13%
    uint256 public _sellReflectionFee = 0; //0%
    
    uint256 private ecosystemFee = 40;   //4% same for buys and sells
    uint256 private liquidityFee = _buyLiquidityFee;
    uint256 private marketingFee = _buyMarketingFee;
    uint256 private lotteryFee = _buyLotteryFee;
    uint256 private reflectionFee=_buyReflectionFee;


    uint256 private totalFee =
        liquidityFee.add(marketingFee).add(lotteryFee).add(ecosystemFee);
    ...
    function _transfer(
        ...
        if(tradingOpen && to == uniswapV2Pair) { //sell
            currenttotalFee= _sellLiquidityFee.add(_sellMarketingFee).add(_sellLotteryFee);
    ...
    function setWallets(address _marketingWallet,address _lotteryWallet) external onlyOwner() {
        marketingWallet = payable(_marketingWallet);
        lotteryWallet = payable(_lotteryWallet);
    }
    ...
    function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _marketingFee,uint256 _lotteryFee,uint256 _ecosystemFee) external onlyOwner {
       
        _buyLiquidityFee = _liquidityFee;
        _buyMarketingFee = _marketingFee;
        _buyLotteryFee = _lotteryFee;
        ...
        totalFee = liquidityFee.add(marketingFee).add(lotteryFee).add(ecosystemFee);
    ...
    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _marketingFee,uint256 _lotteryFee) external onlyOwner {
        _sellLiquidityFee = _liquidityFee;
        _sellMarketingFee = _marketingFee;
        _sellLotteryFee = _lotteryFee;
        _sellReflectionFee= _reflectionFee;
        require(_sellLiquidityFee.add(_sellMarketingFee).add(_sellLotteryFee).add(_sellReflectionFee).add(ecosystemFee) <= 500, "Must keep taxes below 50%");
    }
