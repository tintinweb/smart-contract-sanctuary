...
contract BUSDTest is ERC20, Ownable {
   ...
   BUSDTestDividendTracker public dividendTracker;
    ...
    uint256 public _maxWalletToken = 30000000 * (10**18); // 3% of total supply
    ...
    constructor() public ERC20("BUSDTest", "BUSDTest") {

    	dividendTracker = new BUSDTestDividendTracker();

    	liquidityWallet = owner();
    	marketingWallet = address (0xe57C1e8e932e67Bafb1d419AFE077bcD4646B4d6);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BUSDTest: The dividend tracker already has that address");

        BUSDTestDividendTracker newDividendTracker = BUSDTestDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BUSDTest: The new dividend tracker must be owned by the BUSDTest token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BUSDTest: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BUSDTest: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BUSDTest: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BUSDTest: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BUSDTest: The liquidity wallet is already this address");
    ...
    function setMaxWalletToken(uint256 value) external onlyOwner{
        require(value >= 20000000 * (10**18), "BUSDTest: Minimum max wallet limit is 2 percent");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BUSDTest: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BUSDTest: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BUSDTest: This account cannot send tokens until trading is enabled");
        }
...
contract BUSDTestDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BUSDTest_Dividend_Tracker", "BUSDTest_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BUSDTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BUSDTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BUSDTest contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BUSDTest_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BUSDTest_Dividend_Tracker: Cannot update claimWait to same value");
