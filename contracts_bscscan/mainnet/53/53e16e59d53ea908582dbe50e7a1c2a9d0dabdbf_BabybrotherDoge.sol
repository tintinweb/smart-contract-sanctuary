...
contract BabybrotherDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BabybrotherDoge_Dividend_Tracker", "BabybrotherDoge_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BabybrotherDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BabybrotherDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabybrotherDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabybrotherDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BabybrotherDoge_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BabybrotherDoge is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BabybrotherDogeDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 10000000000000 * (10**9);
    uint256 public maxBuyTransactionAmount = 10000000000000 * (10**9);
    ...
    function SetSwapTokensAtAmount(uint256 _newLimit) public onlyOwner {
        swapTokensAtAmount = _newLimit * (10**9);
    }
    ...
    constructor() ERC20("BabybrotherDoge", "BBD") {
        BNBRewardsFee = 7;
        liquidityFee = 3;
        marketingFee = 2;
        marketingWallet = payable(0x7EbacBcA2a8D61916a7ac67883d3B55D33E071EB);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new BabybrotherDogeDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabybrotherDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabybrotherDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabybrotherDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabybrotherDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabybrotherDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabybrotherDoge: Cannot update gasForProcessing to same value");
