...
contract BABYAAVE is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0xF2cB3D6931f2D93dA86876d6668996fa1A5D7803); // Marketing Address
    address payable public AAVECharityWalletAddress = payable(0x4e1875fccda47DBf6ab36d110964DD016181D497); // AAVE Address
    ...
    string private _symbol = "BBYAAVE";
    uint8 private _decimals = 9;

    uint256 public _burnFee = 2;
    uint256 public _liquidityFee = 5;
    uint256 public _marketingFee = 5;
    uint256 public _AAVEcharity = 5;
    ...
    constructor () {
        ...
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_AAVEcharity);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_AAVEcharity) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_AAVEcharity
     )).mul(_marketingFee);
            uint256 aaveWalletShare = newBalance.sub(marketingShare);
            
            if(marketingShare > 0)
                transferToAddressETH(marketingWalletAddress, marketingShare);
            
            if(aaveWalletShare > 0)
                transferToAddressETH(AAVECharityWalletAddress, aaveWalletShare);
        }
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newAAVECharityTax) external onlyOwner() {
        _burnFee = newBurnFee;
        _liquidityFee = newLiquidityTax;
        _marketingFee = newMarketingTax;
        _AAVEcharity = newAAVECharityTax;
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_AAVEcharity
 );
        _prevTotalTaxPercent = _totalTaxPercent;
    }
    ...
    function setAAVECharityWalletAddress(address newAddress) external onlyOwner() {
        AAVECharityWalletAddress = payable(newAddress);
    }
