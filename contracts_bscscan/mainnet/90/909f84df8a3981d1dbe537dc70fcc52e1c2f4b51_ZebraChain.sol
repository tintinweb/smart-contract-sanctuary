...
contract ZebraChain is Context, IERC20, Ownable {   // nice
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x06a23D87bED931619cb1ec3404a563A32326206E); // Marketing Address
    address payable public charityAddress = payable(0x06a23D87bED931619cb1ec3404a563A32326206E); // Charity/Dev Address
    ...
    uint256 private _tTotal = 9800000 * 10**18; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Zebra Chain "; // nice
    ...
    uint256 public _maxTxAmount = 800 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 80 * 10**18;  //allow maximum sell amount(anti-dump setup)
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    uint256 private _airdropToken = 8000000000000000000; // nice Claim 8 (0.01BNB = 80) 0.0625-0.625 
    ...
    uint256 private salePrice = 8000;// 0.01*80 token for 0.01 bnb (set as per requirements)
