...
contract OK is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xfDFD77CDc044E9b9236a0fE23cf7E94F6e99A4C7);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromMaxBalanceOK;
    mapping (address => bool) private _isExcludedFromFeeOK;

    uint256 private constant _maxFeeOK = 9;
    uint256 private _totalFeeOK;
    uint256 private _totalFeeOKContract;
    uint256 private _liquidityFeeOK;
    uint256 private _marketingFeeOK;
    uint256 private _burnFeeOK;
    
    uint256 private _maxBalanceWalletOK;
    ...
    constructor () {
        ...
        _isExcludedFromFeeOK[owner()] = true;
        _isExcludedFromFeeOK[address(this)] = true;
        
        _isExcludedFromMaxBalanceOK[owner()] = true;
        _isExcludedFromMaxBalanceOK[address(this)] = true;
        _isExcludedFromMaxBalanceOK[uniswapV2Pair] = true;

        _liquidityFeeOK = 1;
        _marketingFeeOK = 8;
        _burnFeeOK = 0;
        _totalFeeOK = _liquidityFeeOK.add(_marketingFeeOK).add(_burnFeeOK);
        _totalFeeOKContract = _liquidityFeeOK.add(_marketingFeeOK);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _maxBalanceWalletOK = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeeOK).add(_marketingFeeOK) <= _maxFeeOK, "Fees are too high.");
        _liquidityFeeOK = newLiquidityFee;
        _totalFeeOK = _liquidityFeeOK.add(_marketingFeeOK).add(_burnFeeOK);
        _totalFeeOKContract = _liquidityFeeOK.add(_marketingFeeOK);
        emit SetLiquidityFee(_liquidityFeeOK);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeeOK.add(_burnFeeOK).add(newMarketingFee) <= _maxFeeOK, "Fees are too high.");
        _marketingFeeOK = newMarketingFee;
        _totalFeeOK = _liquidityFeeOK.add(_marketingFeeOK).add(_burnFeeOK);
        _totalFeeOKContract = _liquidityFeeOK.add(_marketingFeeOK);
        emit SetMarketingFee(_marketingFeeOK);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeeOK.add(newBurnFee).add(_marketingFeeOK) <= _maxFeeOK, "Fees are too high.");
        _burnFeeOK = newBurnFee;
        _totalFeeOK = _liquidityFeeOK.add(_marketingFeeOK).add(_burnFeeOK);
        emit SetBurnFee(_burnFeeOK);
    }
    ...
    function setMaxBalance(uint256 newMaxBalance) external onlyOwner(){
        // Minimum _maxBalanceWalletOK is 0.5% of _totalSupply 
        require(newMaxBalance >= _totalSupply.mul(5).div(1000));
        _maxBalanceWalletOK = newMaxBalance;
    }

    function isExcludedFromFees(address account) public view returns(bool) {
        return _isExcludedFromFeeOK[account];
    }
    
    function excludeFromFees(address account) public onlyOwner {
        _isExcludedFromFeeOK[account] = true;
    }
    
    function includeInFees(address account) public onlyOwner {
        _isExcludedFromFeeOK[account] = false;
    }

    function isExcludedFromMaxBalance(address account) public view returns(bool) {
        return _isExcludedFromMaxBalanceOK[account];
    }
    
    function excludeFromMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalanceOK[account] = true;
    }
    
    function includeInMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalanceOK[account] = false;
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletOK
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceOK[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletOK,
        ...
        if(
            ...
            _totalFeeOKContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeeOK[from] || _isExcludedFromFeeOK[to])
            && _totalFeeOK > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeeOKContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeeOK).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeeOK).div(_totalFeeOKContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeOK).div(_totalFeeOKContract);
