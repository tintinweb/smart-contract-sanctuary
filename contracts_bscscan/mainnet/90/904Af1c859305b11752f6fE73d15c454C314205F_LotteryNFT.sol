...
contract Lottery is LotteryOwnable, Initializable {
    ...
    mapping (uint256 => uint8[6]) private historyCounts;
    ...
    uint8[6] private matchingCounts;

    // default false
    bool public drawingPhase = true;
    ...
    function reset(uint256 _externalRandomNumber) external onlyAdmin {
        require(drawingPhase, "drawed?");
        ...
        historyCounts[lotteryId] = matchingCounts;
        
        for (uint i = 0; i < 6; i ++) {
            winningNumbers[i] = 0;
            historyCounts[lotteryId][i] = 0;
            matchingCounts[i] = 0;
        }
    ...
    function calculateTotalAmount() internal {
        uint256 burnAmount = totalAmount.mul(burn).div(100);
        uint256 marketingReward = totalAmount.mul(marketingFee).div(100);
        mltpx.safeTransfer(burnAddress, burnAmount);
        mltpx.safeTransfer(marketingAddress, marketingReward);
        
        uint256 totalReward = 0;
        
        for (uint i = 0; i < 6; i ++) {
            if (historyCounts[lotteryId][i] > 0)
                totalReward = totalReward.add(totalAmount.mul(allocation[5 - i]).div(100));
        }
        
        totalAmount = totalAmount.sub(burnAmount).sub(marketingReward).sub(totalReward);
    }
    ...
    function drawing(uint256 _externalRandomNumber) internal {
        ...
        assembly {_randomNumber := mod(_randomNumber, _maxNumber)}
    ...
    function calculateMatchingCounts(uint8[6] memory _numbers) internal {
        uint8 count1 = 0;
        uint8 count2 = 0;
        uint8 count3 = 0;
        uint8 count4 = 0;
        uint8 count5 = 0;
        uint8 count6 = 0;
        uint8[6] memory _winningNumbers = winningNumbers;

        if (_winningNumbers[0] == _numbers[0]) {
            count1 ++;
            if (_winningNumbers[1] == _numbers[1]) {
                count1 --;
                count2 ++;
                if (_winningNumbers[2] == _numbers[2]) {
                    count2 --;
                    count3 ++;
                    if (_winningNumbers[3] == _numbers[3]) {
                        count3 --;
                        count4 ++;
                        if (_winningNumbers[4] == _numbers[4]) {
                            count4 --;
                            count5 ++;
                            if (_winningNumbers[5] == _numbers[5]) {
                                count5 --;
                                count6 ++;
                            }
                        }
        ...
        matchingCounts[0] += count1;
        matchingCounts[1] += count2;
        matchingCounts[2] += count3;
        matchingCounts[3] += count4;
        matchingCounts[4] += count5;
        matchingCounts[5] += count6;
    }
    ...
    function buy(uint256 _price, uint8[6] memory _numbers) external {
        ...
        calculateMatchingCounts(_numbers);
    ...
    function  multiBuy(uint256 _price, uint8[6][] memory _numbers) external {
        ...
        for (uint i = 0; i < _numbers.length; i++) {
            ...
            calculateMatchingCounts(_numbers[i]);
            lotteryInfo[lotteryId].push(tokenId);
            if (userInfo[msg.sender].length == 0) {
                totalAddresses = totalAddresses + 1;
            }
    ...
    function getTotalRewards(uint256 _lotteryId) public view returns(uint256) {
        ...
        if(_lotteryId == lotteryId) {
            return totalAmount;
        }
        return historyAmount[_lotteryId];
    }
    
    function getTicketCount(uint256 _lotteryId) public view returns(uint256) {
        require (_lotteryId <= lotteryId, '_lotteryId <= lotteryId');

        return lotteryInfo[_lotteryId].length;
    }
    ...
    function getRewardView(uint256 _tokenId) public view returns (uint256) {
        ...
        reward = totalAmount.mul(allocation[matchingNumber - 1]).div(100).div(historyCounts[_lotteryId][6 - matchingNumber]);
        return reward;        
    }
