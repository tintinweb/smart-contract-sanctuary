...
// File: contracts/INSTANKDividendPayingToken.sol
...
// File: contracts/INSTANK.sol
pragma solidity ^0.7.6;

contract INSTANK is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    INSTANKDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(marketFee).add(buyBackFee).add(charityFee).add(burnFee);
    // buy fees
    ...
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketFee).add(buyBuyBackFee).add(buyCharityFee).add(buyBurnFee);
    // sell fees
    ...
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketFee).add(sellBuyBackFee).add(sellCharityFee).add(sellBurnFee);
    // fee tracker
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "INSTANK: The dividend tracker already has that address");

        INSTANKDividendTracker newDividendTracker = INSTANKDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "INSTANK: The new dividend tracker must be owned by the INSTANK token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "INSTANK: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "INSTANK: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "INSTANK: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "INSTANK: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "INSTANK: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "INSTANK: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "INSTANK: This account cannot send tokens until trading is enabled");
        }
...
contract INSTANKDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("INSTANK_Dividend_Tracker", "INSTANK_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "INSTANK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "INSTANK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main INSTANK contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "INSTANK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "INSTANK_Dividend_Tracker: Cannot update claimWait to same value");
