...
contract Catch22 is Context, IERC20, Ownable {
    ...
    string private _name = "CATCH-22";
    string private _symbol = "C22";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0x1a81b551786D3C4d1C15D01D840739e21d8bd817);
    ...
    uint256 private constant _maxFee = 14;
    ...
    constructor () {
        ...
        _marketingFee = 13;
        _burnFees = 0;
        _totalFee = _liquidityFee.add(_marketingFee).add(_burnFees);
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFees).add(_marketingFee) <= _maxFee, "Fees are too high.");
    ...
    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFee.add(_burnFees).add(newMarketingFee) <= _maxFee, "Fees are too high.");
