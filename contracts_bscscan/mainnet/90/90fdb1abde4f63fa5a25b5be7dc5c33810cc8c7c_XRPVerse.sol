...
contract XRPVerse is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    XRPVerseDividendTracker public dividendTracker;
    ...
    constructor() ERC20("XRPVerse", "XRPV") {
        marketingWallet = payable(0x16BA5e2D1a735EcB8c6bfA5D87C420568Cf4cb95);
        ...
        dividendTracker = new XRPVerseDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (21_000_000_000) * (10**18);
    ...
    function updateFees(
        ...
        require(
            marketingBuy <= 20 && marketingSell <= 20,
            "Marketing fees can't exceed 20%"
        );
        buyDeadFees = deadBuy;
...
contract XRPVerseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    XRPVerse public XRPVerseContract;
    ...
    constructor(
        ...
        defaultToken = token;
        XRPVerseContract = XRPVerse(mainContract);
    ...
    function _transfer(
        ...
        require(false, "XRPVerse_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "XRPVerse_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XRPVerse contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(XRPVerseContract);

            uint256 prevBalance = XRPVerseContract.balanceOf(address(this));
            ...
            {
                uint256 received = XRPVerseContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    XRPVerseContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
