...
// File: contracts\librarys\Address.sol
...
// File: contracts\librarys\Utils.sol
...
// File: contracts\token\ReentrancyGuard.sol
...
contract NF3 is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x18242ed9566463Ab7fB9DeF06D639d806c409DbF); // Marketing Address
    ...
    uint256 private _tTotal =  913 * 10**4 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "NF3";
    string private _symbol = "NF3";
    ...
    function changeRouterVersion(address _router) public onlyOwner returns(address _pair) {
        ...
        _pair = IPancakeFactory(_uniswapV2Router.factory()).getPair(address(this), _uniswapV2Router.WETH());
        if(_pair == address(0)){
            // Pair doesn't exist
            _pair = IPancakeFactory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH());
        }
        uniswapV2Pair = _pair;
