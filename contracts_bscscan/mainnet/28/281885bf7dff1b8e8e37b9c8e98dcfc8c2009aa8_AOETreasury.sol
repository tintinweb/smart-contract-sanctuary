...
contract AOETreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SAOE }
    ...
    address public sAOE;
    uint public sAOEQueue; // Delays change to sAOE address
    ...
    /**
        @notice allow approved address to deposit an asset for AOE
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint AOE needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( AOE ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn AOE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAOEERC20( AOE ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAOE ).balanceOf( msg.sender ); // Can only borrow against sAOE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AOE
        @param _amount uint
     */
    function repayDebtWithAOE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IAOEERC20( AOE ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, AOE, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( AOE ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( AOE ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns AOE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AOE decimals
            value_ = _amount.mul( 10 ** IERC20( AOE ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAOE ) { // 9
            sAOEQueue = block.number.add( blocksNeededForQueue );
        } else return false;
