...
interface IMasterBelt {
    ...
    function stakedWantTokens(uint256 poolId, address account) external view returns (uint256);

    // solhint-disable-next-line func-name-mixedcase
    function pendingBELT(uint256 poolId, address account) external view returns (uint256);

    // solhint-disable-next-line func-name-mixedcase
    function BELT() external pure returns (IBEP20);
}

interface IDepositB {
    // solhint-disable-next-line func-name-mixedcase
    function add_liquidity(uint256[4] memory amounts, uint256 minimumMintAmount) external;
}

contract BeltStrategy is IStrategy, Ownable {
    using SafeBEP20 for IBEP20;
    using PancakeSwap for IBEP20;

    IBEP20 public constant USDT = IBEP20(0x55d398326f99059fF775485246999027B3197955);
    IMasterBelt public constant MASTER_BELT = IMasterBelt(0xD4BbC80b9B102b77B21A06cb77E954049605E6c1);
    IDepositB public constant DEPOSIT_B = IDepositB(0xF6e65B33370Ee6A49eB0dbCaA9f43839C1AC04d5);
    ...
    function invest(IBEP20 token, uint256 amount) public override {
        token.approve(address(MASTER_BELT), amount);
        MASTER_BELT.deposit(getPoolIdByUnderlyingToken(token), amount);
    }

    function divest(IBEP20 token, uint256 amount) public override {
        MASTER_BELT.withdraw(getPoolIdByUnderlyingToken(token), amount);
    }

    function harvest(IBEP20 token) public override {
        uint256 poolId = getPoolIdByUnderlyingToken(token);
        MASTER_BELT.withdraw(poolId, 0);

        IBEP20 belt = MASTER_BELT.BELT();
        ...
        if (balance > 0) {
            ...
            USDT.approve(address(DEPOSIT_B), balance);
            DEPOSIT_B.add_liquidity([0, 0, balance, 0], 0);
        }
    }

    function getInvestedAmount(IBEP20 token) public view override returns (uint256 investedAmount) {
        uint256 poolId = getPoolIdByUnderlyingToken(token);
        investedAmount = MASTER_BELT.stakedWantTokens(poolId, address(this));
    }

    function estimateTotalValue(IBEP20 token) public view override returns (uint256 totalValue) {
        uint256 poolId = getPoolIdByUnderlyingToken(token);
        IBEP20 belt = MASTER_BELT.BELT();

        (, uint256 estimatedSwappedAmount) = belt.estimateSwap(token, MASTER_BELT.pendingBELT(poolId, address(this)));

        uint256 investedAmount = MASTER_BELT.stakedWantTokens(poolId, address(this));
        totalValue = investedAmount + estimatedSwappedAmount;
    }

    function getPoolIdByUnderlyingToken(IBEP20 underlyingToken) internal pure returns (uint256 poolId) {
        // 4Belt
        if (underlyingToken == IBEP20(0x9cb73F20164e399958261c289Eb5F9846f4D1404)) {
            return 3;
        }
