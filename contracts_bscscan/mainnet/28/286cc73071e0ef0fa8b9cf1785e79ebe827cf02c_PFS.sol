...
contract Ownable is Context {
    ...
    constructor()  {
        ...
        _owner = 0xF5E1Fd6da44b7c99890232694E0EB65A49283c9C;
...
contract PFS is Context, IERC20, Ownable {
    ...
    string private _name = "Pogi Farm Savings";
    string private _symbol = "PFS";
    uint256 private _decimals = 18;


    address liquidityAddress = 0x5deD5642453F1E484a95697F73B6c1ee2B63dAB4; //Liquidity address
    address marketingAddress = 0x32Da769F44b5d337BE220EFed8256bBA17e8FE23; // Marketing Address
    address treasuryFundAddress = owner(); //This address is same as owner address
    address airDropAddress = 0xA030C568592317798111fBdAa1AE1980CB41de9D; //Airdrop address
    address devTeamAddress = 0x7bDe5d3BBFA0cA8E9ae19D16Cb9E5c5dB66EAd2D; // devTeamAddress Address
    address burnAddress = address(0); // This is burn address
    ...
    uint256 public _liquidityFee = 100; // means 1% add liquidity on each buy and sell
    uint256 public _marketingFee = 200; // 2 % for marketing
    ...
    constructor()  {
        ...
        _reflectionBalance[owner()] = reflectionFromToken(550_000_000 *(10**decimals()),false);  // 55% of supply token Sale 20% and treasury 30%
        _reflectionBalance[airDropAddress] = reflectionFromToken(50_000_000 *(10**decimals()),false);// 5% for AirDrop supply
        _reflectionBalance[liquidityAddress] = reflectionFromToken(100_000_000 *(10**decimals()),false); //10% for liquidity supply
        _reflectionBalance[burnAddress] =  reflectionFromToken(200_000_000 *(10**decimals()),false); //20% for burn supply
        _reflectionBalance[marketingAddress] = reflectionFromToken(50_000_000 *(10**decimals()),false);// 5% for Marketing supply
        _reflectionBalance[devTeamAddress] = reflectionFromToken(50_000_000 *(10**decimals()),false);// 5% for devTeamAddress supply
        
        
        emit Transfer(address(0),owner(),550_000_000 *(10**decimals()));
        emit Transfer(address(0),airDropAddress, 50_000_000 *(10**decimals()));
        emit Transfer(address(0),liquidityAddress,100_000_000 *(10**decimals()));
        emit Transfer(address(0),burnAddress,200_000_000 *(10**decimals()));
        emit Transfer(address(0),marketingAddress,50_000_000 *(10**decimals()));
        emit Transfer(address(0),devTeamAddress,50_000_000 *(10**decimals()));
