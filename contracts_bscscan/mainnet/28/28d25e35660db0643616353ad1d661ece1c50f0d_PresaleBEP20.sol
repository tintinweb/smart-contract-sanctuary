...
contract PresaleBEP20 is Ownable {
    ...
    address payable public BNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    uint256 public hardCapEthAmount = 560 ether;
    ...
    uint256 public tokenPrice = 0.4 ether;   // 10 BNB = 1 token
    ...
    constructor(
        ...
        presaleEndTimestamp = now.add(10 days + 1 hours + 30 minutes);
    }
    function setPresaleTime( uint256 start, uint256 end)  external onlyOwner{
        require(start < end, "start < end!!");
        presaleStartTimestamp = start;
        presaleEndTimestamp = end;
    }
    function setMinMax(uint256 min, uint256 max) external{
        minimumDepositEthAmount = min * 1e18;
        maximumDepositEthAmount = max * 1e18;
    }
    function setTokenAddress( address addr) external onlyOwner {
        require ( addr != address(0), "zero address");
        token = addr;
    }
    function setClaimable(bool f)  external onlyOwner {
        isClaimable = f;
    }
    ...
    function claim() public {
        require(isClaimable, "Not claimable now");
        require(deposits[msg.sender] != 0, "Deposited 0");
        uint256 tokenAmount =  deposits[msg.sender].mul(tokenDecimal).div(tokenPrice);
        require(tokenAmount <= IERC20(token).balanceOf(address(this)), "Lack of token on presale contract");
