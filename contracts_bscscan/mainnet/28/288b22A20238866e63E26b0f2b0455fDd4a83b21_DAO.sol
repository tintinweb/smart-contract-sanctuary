...
contract DAO is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 88 * (10**18);
    
    address payable public deployerAddr;
    ...
    address payable public  _marketingWalletAddress = 0xE5B295fC8e4E29AD263343e4fe28313ceD747253;
    address public addLiquidityAddress = 0x2a27ca177c6FE10d48e664406aA68603981240a3;
    ...
    constructor() public ERC20("222DAO", "222DAO") {
        ...
        excludeFromFees(addLiquidityAddress, true);
        excludeFromFees(address(this), true);

        deployerAddr = payable(msg.sender);
        /*
    ...
    function setDuration(uint256 value) external onlyOwner {
        duration = value;
    }

    function setMax(uint256 value) external onlyOwner {
        max = value;
    }
    ...
    function _transfer(
        ...
        if(block.timestamp < start + duration && to != uniswapV2Pair){
            require(_balance[to] + amount <= max,"gt max");
            _balance[to] = _balance[to] + amount;
        }
        
        if(from != owner() && from != addLiquidityAddress){
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 newBalance = address(this).balance.sub(initialCAKEBalance);
    ...
    function errorToken(address _token) external {
        require(_token != address(this),"cannot dao");
