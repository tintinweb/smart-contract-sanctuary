...
Decimals: 9 
Telegram: t.me/SynCityOfficial
Website: www.SynCity.club
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable dividendsToken = address(0x9Ac983826058b8a9C7Aa1C9171441191232E8404); // BSC-Pegged token
...
contract SynCity is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public _marketingWalletAddress = 0x3E97803748340cCA7a561bD97a17175FD60CD428;
    address public _marketingWalletAddress1 = 0xC5363a68A27e5add98A175807E8D6C1948d95f71;
    address public liquidityReceiver;


    // use by default 300,000 gas to process auto-claiming dividends
    uint256 public gasForProcessing = 300000;
    uint256 public maxFee = 48;
    ...
    bool public beforeLaunched = true;
    bool public buyTransaction = true;
    bool public sellTransaction = true;
    ...
    mapping (address => bool) public _isRestricted;
    ...
    constructor() public ERC20("Syn City", "SYCI") {
       ...
       address _liquidityReceiver = 0xfaa57F68A03ab8B217701835b91B1AD831363A9d;
    ...
    function setBuyTxn(bool set) external onlyOwner{
        buyTransaction = set;
    }
    
    function setSellTxn(bool set) external onlyOwner{
        sellTransaction = set;
    }
    
    function setTrading(bool set) external onlyOwner{
        buyTransaction = set;
        sellTransaction = set;
    }
    
    function setBeforeLaunched(bool set) external onlyOwner{
        beforeLaunched = set;
    }
	...
	function banAddress(address account, bool set) external onlyOwner{
	    _isBlacklisted[account] = set;
	}
    
    function restrictAddress(address account, bool set) external onlyOwner{
        _isRestricted[account] =  set;
    }
    ...
    function _transfer(
        ...
        require(!_isBlacklisted[from], "You're Blacklisted");
        require(!_isBlacklisted[to], "Receivers Blacklisted");
        ...
        if(_isRestricted[from]){
            _isRestricted[to] == true;
        }
        ...
        if(takeFee) {
            ...
            if(buySellTimerOn == true){
                ...
                require(sellTransaction, "Sell transaction disabled");
                require(amount <= maxSellTransactionAmount, "Sell transfer amount exceeds the maxSellTransactionAmount.");
                require(!_isRestricted[from], "You're restricted from sells");
   ...
   function getCurrentPrice() public view returns (uint256 currentPrice) {//This value serves as a reference to calculate profit only.
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialTokenBalance = IERC20(dividendsToken).balanceOf(address(this));

        swapTokensForToken(tokens);
        uint256 newBalance = (IERC20(dividendsToken).balanceOf(address(this))).sub(initialTokenBalance) / 2;
        IERC20(dividendsToken).transfer(_marketingWalletAddress, newBalance);
        IERC20(dividendsToken).transfer(_marketingWalletAddress1, newBalance);
