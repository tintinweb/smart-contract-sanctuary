...
// File: contracts/ERC721DataStorage.sol
...
contract ERC721DataStorage is ERC721, Whitelistable {
    struct Data {
        string name_;
        address artist;
        uint256 grade;
    ...
    function mint(address to) public onlyWhitelist returns (uint256 tokenId) {
    ...
    function setTokenURI(uint256 tokenId, string memory tokenURI_) public onlyWhitelist {
        require(_exists(tokenId), 'ERC721: operator query for nonexistent token');
        _tokenURIs[tokenId] = tokenURI_;
        emit UpdatedTokenURI(tokenId, tokenURI_);
    }

    function setTokenData(uint256 tokenId, Data memory data_) public onlyWhitelist {
        require(_exists(tokenId), 'ERC721: operator query for nonexistent token');
        _datas[tokenId] = data_;
    }
    ...
    function infoBundleForToken(uint256 tokenId) external view returns (string memory tokenURI_, Data memory data_) {
        return (tokenURI(tokenId), _datas[tokenId]);
    }
        ...
        )
    {
        ...
        Data[] memory datas_ = new Data[](balance_);

        for (uint256 i = 0; i < tokenCounter; i++) {
            if (ownerOf(i) == user) {
                tokenIds_[counter] = i;
                tokenURIs_[counter] = _tokenURIs[i];
                datas_[counter] = _datas[i];
                counter++;
            }
        }

        return (balance_, tokenIds_, tokenURIs_, datas_);
    }
}
...
// File: contracts/StakingAddble.sol
...
contract StakingAddble is Whitelistable {
    ...
    uint constant public stakePeriod = 0;
...
contract Lottery is Whitelistable, ERC721Holder {
    struct Drop {
        uint256[] tokenIds;
        address[] stakings;
    ...
    ///@dev temporary var for closing Lottery
    mapping(uint256 => address) public ticketsToUser;
    ...
    event UserClaimed(address indexed sender, uint256 dropId, uint256 tokenId);
    ...
    function createNFT(
        ...
        uint256 amount,
        ERC721DataStorage.Data memory data_,
        uint256 price,
        uint256 amountOfTickets,
        uint256 period
    ) external onlyWhitelist {
        uint256[] memory tokenIds = nft.fullMint(address(this), tokenURI_, amount, data_);
        createDrop(tokenIds, stakings, price, amountOfTickets, period);
    }
    ...
    function createDrop(
        ...
        require(price != 0 && amountOfTickets != 0 && period != 0, 'Cannot input zero values');
    ...
    function openLottery(uint256 dropId) public onlyWhitelist {
        Drop storage drop = drops[dropId];
        require(drop.status == DropStatus.PENDING, 'Lottery does not exist or already lauched');
    ...
    function closeLottery(uint256 dropId) external onlyWhitelist {
        ...
        require(
            drop.endTimestamp <= block.timestamp || drop.amountOfTickets == drop.tookTickets,
            'Lottery has not ended'
        );

        if (drop.users.length == 0) {
            uint256 length = drop.tokenIds.length;
            for (uint256 i = 0; i < length; i++) {
                nft.transferFrom(address(this), msg.sender, drop.tokenIds[i]);
            }
            drop.status = DropStatus.STOPPED;
            return;
        }

        uint256 ticketsCounter = 1;
        uint256 amountOfUsers = drop.users.length;
        for (uint256 i = 0; i < amountOfUsers; i++) {
            uint256 ticketsAmount = drop.ticketsAmount[drop.users[i]];
            deposited[wrapAddresses(drop.stakings)][drop.users[i]] -= ticketsAmount * drop.price;
            for (uint256 j = 0; j < ticketsAmount; j++) {
                ticketsToUser[ticketsCounter] = drop.users[i];
                ticketsCounter++;
            }
        }

        uint256 amountOfTokens = drop.tokenIds.length;

        for (uint256 i = 0; i < amountOfTokens; i++) {
            uint256 tokenId = drop.tokenIds[i];
            address winner = ticketsToUser[randomInt(1, ticketsCounter, i)];
            drop.winnedTokens[winner].push(tokenId);
        }
    ...
    function playLottery(uint256 dropId, uint256 ticketAmount) external {
        ...
        deposited[wrapAddresses(drops[dropId].stakings)][msg.sender] += slots * drops[dropId].price;
    ...
    function claimWin(uint256 dropId) external {
        Drop storage drop = drops[dropId];

        require(drop.played[msg.sender], 'Only lottery participants');

        require(token.balanceOf(msg.sender) >= drop.price, 'Not enough');

        uint256[] memory winnedTokens = drop.winnedTokens[msg.sender];
        require(winnedTokens.length > 0, 'You didnt win the token');
        uint256 length = winnedTokens.length;

        for (uint256 i = 0; i < length; i++) {
            token.transferFrom(msg.sender, platformWallet, nft.data(winnedTokens[i]).nftPrice);
            nft.transferFrom(address(this), msg.sender, winnedTokens[i]);
            emit UserClaimed(msg.sender, dropId, winnedTokens[i]);
        }
        drop.claimed[msg.sender] = true;
    }

    // view functions
    function getTotalDeposit(address user, uint256 dropId) public view returns (uint256 ticketsAmount) {
        uint256 totalSum = sumStaking(user, drops[dropId].stakings);
        uint256 deposit = totalSum - deposited[wrapAddresses(drops[dropId].stakings)][user];
        if (deposit > 0 && !drops[dropId].played[user] && drops[dropId].amountOfTickets != 0) return deposit;
        return 0;
    }

    function sumStaking(address user, address[] memory stakings) public view returns (uint256) {
        uint256 length = stakings.length;
        uint256 sum = 0;
        for (uint256 i = 0; i < length; i++) {
            StakingAddble staking = StakingAddble(stakings[i]);
            if ((block.timestamp - staking.getStake(user).startStaking) < stakedPeriod) continue;
    ...
    function getStatus(address user, uint256 dropId) public view returns (UserStatus status) {
        Drop storage drop = drops[dropId];
        if (drop.status == DropStatus.STARTED) return UserStatus.PENDING;
        else if (drop.winnedTokens[user].length > 0) return UserStatus.WON;
    ...
    function maxTickets(uint256 dropId, address user) public view returns (uint256) {
        ...
        uint256 slots = deposit / drops[dropId].price;
        slots = slots < drop.amountOfTickets - drop.tookTickets ? slots : drop.amountOfTickets - drop.tookTickets;
        return slots;
    }
    ...
    function randomInt(
        ...
        uint256 randomNumber = uint256(
            keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, salt))
        );
    ...
    function infoBundlerUserDrop(address user, uint256 dropId) public view returns (InfoUserDrop memory infoUserDrop) {
        Drop storage drop = drops[dropId];
        ...
        infoUserDrop.data = nft.data(drop.tokenIds[0]);
        infoUserDrop.tokenURI = nft.tokenURI(drop.tokenIds[0]);
    }

    function infoBundlerUser(address user) public view returns (InfoUserDrop[] memory infoUserDrops) {
        uint256 amount;
    ...
    function infoBundlerDrop(uint256 dropId) public view returns (InfoDrop memory infoDrop) {
        Drop storage drop = drops[dropId];

        infoDrop.dropId = dropId;
        infoDrop.status = drop.status;
        infoDrop.tokenIds = drop.tokenIds;
        infoDrop.stakings = drop.stakings;
        infoDrop.price = drop.price;
        ...
        infoDrop.data = nft.data(drop.tokenIds[0]);
        infoDrop.tokenURI = nft.tokenURI(drop.tokenIds[0]);
    }
