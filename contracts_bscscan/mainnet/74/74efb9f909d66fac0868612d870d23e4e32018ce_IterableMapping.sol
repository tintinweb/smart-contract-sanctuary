 ...
 * 4% lp
 ...
 Community Token !! Cms After Launch
...
contract ADAMoonPump is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ADAMoonPumpDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("ADAMoonPump", "ADAMP") {
        _ETHRewardsFee = 8;
        _liquidityFee = 4;
		_marketingFee = 3;
    	...
    	dividendTracker = new ADAMoonPumpDividendTracker();
    	antiBotEnabled = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ADAMoonPump: The dividend tracker already has that address");

        ADAMoonPumpDividendTracker newDividendTracker = ADAMoonPumpDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ADAMoonPump: The new dividend tracker must be owned by the ADAMoonPump token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ADAMoonPump: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ADAMoonPump: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ADAMoonPump: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ADAMoonPump: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ADAMoonPump: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ADAMoonPump: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ADAMoonPump: Cannot update gasForProcessing to same value");
...
contract ADAMoonPumpDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ADAMoonPumpDividend_Tracker", "ADAMoonPump_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "ADAMoonPump_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ADAMoonPumpDividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ADAMoonPump contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ADAMoonPump_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ADAMoonPump_Dividend_Tracker: Cannot update claimWait to same value");
