 ...
 * Get Polkadot rewards automaticaly just by buying and holding our token.

 * Anti whale - Anti Dump - Automatic reward claiming
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402);
...
contract PolkadotBaby is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   PolkadotBabyDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("PolkadotBaby", "PolkadotBaby") {
        _ETHRewardsFee = 4;
        _liquidityFee = 4;
		_marketingFee = 10;
    	...
    	dividendTracker = new PolkadotBabyDividendTracker();
    	antiBotEnabled = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PolkadotBaby: The dividend tracker already has that address");

        PolkadotBabyDividendTracker newDividendTracker = PolkadotBabyDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PolkadotBaby: The new dividend tracker must be owned by the PolkadotBaby token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PolkadotBaby: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PolkadotBaby: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PolkadotBaby: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PolkadotBaby: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PolkadotBaby: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PolkadotBaby: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PolkadotBaby: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForETH(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of PolkadotBaby
            path,
            recipient,
            block.timestamp
        );
...
contract PolkadotBabyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PolkadotBaby_Dividend_Tracker", "PolkadotBaby_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "PolkadotBaby_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PolkadotBaby_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PolkadotBaby contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "PolkadotBaby_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PolkadotBaby_Tracker: Cannot update claimWait to same value");
