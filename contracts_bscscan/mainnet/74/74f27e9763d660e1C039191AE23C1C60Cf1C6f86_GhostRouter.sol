...
import './interfaces/IGhostPair.sol';
import '../node_modules/@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol';
import './interfaces/IGhostRouter.sol';
import './interfaces/IWETH.sol';
import './libraries/SafeMath.sol';
import '../node_modules/@openzeppelin/contracts/access/Ownable.sol';
import './libraries/SafeBEP20.sol';

contract GhostRouter is IGhostRouter, Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    address public override factory;
    address public override WETH;
    uint256 public override factory_init_hash;
    uint256 public constant MASK = type(uint256).max;

    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, 'Error : Router expired');
        _;
    }
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex'd508507167d80833c74a7303e14d764844bd6e183e324caf32127f9e2ae17b86'
                        )
                    )
                )
            )
    ...
    function getReserves(address tokenA, address tokenB) internal view returns (uint256 reserveA, uint256 reserveB) {
        ...
        address pair = IGhostFactory(factory).getPair(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1, ) = IGhostPair(pair).getReserves();
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, 'Error : INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'Error : INSUFFICIENT_LIQUIDITY');
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'Error : INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'Error : INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        IGhostPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
    ...
    function removeLiquidityWithPermit(
        ...
        IGhostPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IGhostPair(pairFor(input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'Error : INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'Error : EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, pairFor(path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }
    ...
    function swapExactETHForTokens(
        ...
        require(path[0] == WETH, 'Error : INVALID_PATH');
    ...
    function swapTokensForExactETH(
        ...
        require(path[path.length - 1] == WETH, 'Error : INVALID_PATH');
        amounts = getAmountsIn(amountOut, path);
        require(amounts[0] <= amountInMax, 'Error : EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, pairFor(path[0], path[1]), amounts[0]);
    ...
    function swapETHForExactTokens(
        ...
        require(path[0] == WETH, 'Error : INVALID_PATH');
        amounts = getAmountsIn(amountOut, path);
        require(amounts[0] <= msg.value, 'Error : EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(pairFor(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = sortTokens(input, output);
            IGhostPair pair = IGhostPair(pairFor(input, output));
            uint256 amountInput;
...
interface IGhostRouter {
    ...
    function factory_init_hash() external view returns (uint256);
