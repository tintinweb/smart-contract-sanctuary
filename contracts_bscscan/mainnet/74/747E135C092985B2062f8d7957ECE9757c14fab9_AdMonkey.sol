...
contract AdMonkey is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public _taxFee = 2; // 2% RFI Static Rewards 
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 2; // 3% Buyback, 3% BNB Reward, 2% Marketing
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public bnbRewardFees = 10;
    uint256 public totalFee = bnbRewardFees.add(_liquidityFee);
    ...
    event SwapETHForTokensMoonshot(
        uint256 amountIn,
        address[] path
    );
    ...
    function buyBackTokens(uint256 amount) private lockTheSwap {
    	if (amount > 0) {
    	    Utils.swapETHForTokens(address(pancakeRouter),address(0x000000000000000000000000000000000000dEaD),amount);
	    }
    }
    ...
    /**
     * @dev Call "The MoonShot". 1 equals 0.001 (15 decimal places)
     * 
     * ONLY TO BE USED IN TIME OF NEED 
     * 
     */
