...
contract CoShiInu  is Context, IERC20, Ownable {
    ...
    uint256 private _ReflectiontoHolder = 1;
    uint256 private _previousReflection = _ReflectiontoHolder;
	...
	function getAccumulatedFees() external view returns (uint256, uint256) {
            return (_ReflectiontoHolder, LiquidityFee);
	}
   ...
   //this method is responsible for taking all fee, if takeFee is true
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        _ReflectiontoHolder = Reflection;
    }
    ...
    function calculateReflection(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_ReflectiontoHolder).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_ReflectiontoHolder == 0 && LiquidityFee == 0) return;
        
        _previousReflection = _ReflectiontoHolder;
        _previousLiquidityFee = LiquidityFee;
        
        _ReflectiontoHolder = 0;
        LiquidityFee = 0;
    }
    
    function restoreAllFee() private {
        _ReflectiontoHolder = _previousReflection;
