...
contract SantaShiba is ERC20, Ownable {
    ...
    SantaShibaDividendTracker public dividendTracker;
    ...
    address public marketingWallet = 0xD4f14c62dCbD8B545b15081930B7DfA4d07b8ec7;
    address public liquidityWallet = 0xD4f14c62dCbD8B545b15081930B7DfA4d07b8ec7;
    ...
    constructor()  ERC20("Santa Shiba", "SANTASHIB") {
        
    	dividendTracker = new SantaShibaDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SantaShiba: The dividend tracker already has that address");

        SantaShibaDividendTracker newDividendTracker = SantaShibaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SantaShiba: The new dividend tracker must be owned by the SantaShiba token contract");
    ...
    function updateRouter(address newAddress) public onlyOwner {
        require(newAddress != address(router), "SantaShiba: The router already has that address");
        emit Updaterouter(newAddress, address(router));
        router = IRouter(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SantaShiba: Account is already the value of 'excluded'");
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "SantaShiba: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SantaShiba: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SantaShiba: Cannot update gasForProcessing to same value");
...
contract SantaShibaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor()  DividendPayingToken("SantaShiba_Dividend_Tracker", "SantaShiba_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 150e9 * (10**9); //must hold 150_000_000_000 tokens
    }

    function _transfer(address, address, uint256) internal pure override{
        require(false, "SantaShiba_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SantaShiba_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SantaShiba contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SantaShiba_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SantaShiba_Dividend_Tracker: Cannot update claimWait to same value");
