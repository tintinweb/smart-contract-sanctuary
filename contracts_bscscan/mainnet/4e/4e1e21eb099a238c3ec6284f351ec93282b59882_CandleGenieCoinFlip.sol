...
contract CandleGenieCoinFlip is Ownable, ReentrancyGuard 
{
    ...
    function Flip(int position) external payable nonReentrant notContract 
    {
        ...
        require(Bets[currentBetId].betAmount == 0, "Can only bet once for each block");
        ...
        if (currentBetId > 0)
        {
            require(block.timestamp > Bets[currentBetId - 1].timestamp, "Too early! Wait current block mined to make a new flip");
        }
