...
contract PancakeHunter is Ownable {
    ...
    function swapWBNBForToken(IBEP20 token, uint256 amount)  public onlyOwner returns(uint256) {
        ...
        //burada wbnb harcanıyor. Eğer tam bakiye kullanılırsa hata alınabilir.
    ...
    function getTokenBuyAmount(IBEP20 token)  public view returns(uint256) {
        ...
        require(tokenSwapTestGap < minBuy, "Token test gap is higher than min buy.");
        require(contractBalance > tokenSwapTestGap, "Contract token balance lower than test gap");
        require(tokenSwapTestGap < maxBuy, "Token test gap is higher than max buy.");
        ...
        if(contractBalance >= maxBuy) {
            return maxBuy.sub(tokenSwapTestGap);
        } else {
            uint256 buyAmount = minBuy.sub(tokenSwapTestGap);
