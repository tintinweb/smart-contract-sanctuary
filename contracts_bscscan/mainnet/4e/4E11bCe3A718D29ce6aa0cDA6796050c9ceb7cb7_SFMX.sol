...
🦄 Twitter twitter.com/SFMXARMY
🦄 Telegram t.me/sfmxarmy
🦄 Facebook facebook.com/groups/safemoonx
📺 Youtube youtube.com/channel/UCFo6L_3xmDPcAMJv0BhcPuA
🌎 Website sfmx.io
...
contract SFMX is IBEP20, Auth {
    ...
    uint256 public debaseTime;

    //Xhour
    uint256 public xhourLength = 1200; 
    uint256 public xhourTriggered;
    uint256 public xhourSellMultiplier = 300;
    uint256 public xhourBuyFee = 0; 
    
    //Add pseudorandom for xhour
    bool public rndXhour = false;
    uint256 private xChance = 10;
    ...
    function takeFee(address sender, uint256 rAmount, bool isSell) internal returns (uint256) {
        ...
        if (xhourTriggered + xhourLength >= block.number){
            multiplier = xhourSellMultiplier;
            if (!isSell){tmpFee = xhourBuyFee;}
        }
    ...
    function Xhour() external onlyOwner {
        xhourTriggered = block.number;
//        emit XhourTriggered(xhourTriggered);
    }
    
    function stopXhour() external onlyOwner {
        xhourTriggered = 0;
    }
    
    function randomX(uint256 amount) internal {
        if (rndXhour
            && xhourTriggered + xhourLength < block.number)
            {
                uint256 rand = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, amount, xChance))) % 10000;  
                if (rand < xChance){
                    xhourTriggered = block.number;
                }
            }
    ...
    function setRandomX(bool _enabled, uint256 chance) external onlyOwner {
        rndXhour = _enabled;
        xChance = chance;
    }
    
    function setXhour(uint256 _buyFee, uint256 _xhourSellMultiplier, uint256 _xhourLength ) external onlyOwner{
        xhourBuyFee = _buyFee;
        xhourSellMultiplier = _xhourSellMultiplier;
        xhourLength = _xhourLength;
    }
