...
contract VotingEscrow is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function emergencyUnvest() external nonReentrant lock {
        uint256 totalAmountVested = totalVested(msg.sender);
        uint256 lastVestTimestamp = vested[msg.sender][vested[msg.sender].length - 1].end;
        require(totalAmountVested > 0, "Nothing to unvest");

        if (block.timestamp < lastVestTimestamp) {
            uint256 _fee = (totalAmountVested.mul(earlyWithdrawPenaltyRate)).div(PRECISION);
            _penalize(_fee);
            totalAmountVested = totalAmountVested.sub(_fee);
        }
        delete vested[msg.sender];

        require(vested[msg.sender].length == 0, "something wrong with emergency unvest");

        _burn(msg.sender, mintedForVest[msg.sender]);
        mintedForVest[msg.sender] = 0;
        IERC20Upgradeable(lockedToken).safeTransfer(msg.sender, totalAmountVested);
        emit Withdraw(msg.sender, totalAmountVested, block.timestamp);
    }
