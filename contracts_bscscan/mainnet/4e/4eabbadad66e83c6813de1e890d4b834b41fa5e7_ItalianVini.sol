...
/*
    Telegram : https://t.me/italianvini
    Website : https://italianvini.com/
...
contract ItalianVini is Context, IERC20, Ownable {
    ...
    string private _name = "Italian Vini";
    string private _symbol = "$VINI";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 10000 * 10**2 * 10**_decimals;
    
    address payable public marketingAddress = payable(0x732B57974bb901d8f143A2b297a6b8a27fC4F526);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant developmentAddress = 0x732B57974bb901d8f143A2b297a6b8a27fC4F526;
    ...
    uint256 private constant _isMaxFee = 15;
    uint256 private _isTotalFee;
    uint256 private _isTotalFeeContract;
    uint256 private _isLiquidityFee;
    uint256 private _isMarketingFee;
    uint256 private _isDevelopmentFee;
    
    uint256 private _isMaxBalanceWallet;
    ...
    constructor () {
        ...
        _excludedFromMaxBalance[owner()] = true;
        _excludedFromMaxBalance[address(this)] = true;
        _excludedFromMaxBalance[uniswapV2Pair] = true;

        _isLiquidityFee = 1;
        _isMarketingFee = 13;
        _isDevelopmentFee = 1;
        _isTotalFee = _isLiquidityFee.add(_isMarketingFee).add(_isDevelopmentFee);
        _isTotalFeeContract = _isLiquidityFee.add(_isMarketingFee);

        _liquidityThreshhold = 1 * 10**2 * 10**_decimals;
        _isMaxBalanceWallet = 10000 * 10**2 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_isDevelopmentFee).add(_isMarketingFee) <= _isMaxFee, "Fees are too high.");
        _isLiquidityFee = newLiquidityFee;
        _isTotalFee = _isLiquidityFee.add(_isMarketingFee).add(_isDevelopmentFee);
        _isTotalFeeContract = _isLiquidityFee.add(_isMarketingFee);
        emit SetLiquidityFee(_isLiquidityFee);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_isLiquidityFee.add(_isDevelopmentFee).add(newMarketingFee) <= _isMaxFee, "Fees are too high.");
        _isMarketingFee = newMarketingFee;
        _isTotalFee = _isLiquidityFee.add(_isMarketingFee).add(_isDevelopmentFee);
        _isTotalFeeContract = _isLiquidityFee.add(_isMarketingFee);
        emit SetMarketingFee(_isMarketingFee);
    }

    function setDevelopmentFeePercent(uint256 newDevelopmentFee) external onlyOwner() {
        require(_isLiquidityFee.add(newDevelopmentFee).add(_isMarketingFee) <= _isMaxFee, "Fees are too high.");
        _isDevelopmentFee = newDevelopmentFee;
        _isTotalFee = _isLiquidityFee.add(_isMarketingFee).add(_isDevelopmentFee);
        emit SetDevelopmentFee(_isDevelopmentFee);
    }
    ...
    function setMaxBalance(uint256 newMaxBalance) external onlyOwner(){
        // Minimum _isMaxBalanceWallet is 0.5% of _totalSupply 
        require(newMaxBalance >= _totalSupply.mul(5).div(1000));
        _isMaxBalanceWallet = newMaxBalance;
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _isMaxBalanceWallet
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_excludedFromMaxBalance[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _isMaxBalanceWallet,
        ...
        if(
            ...
            _isTotalFeeContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        if(
        	...
        	uint256 feesToContract = amount.mul(_isTotalFeeContract).div(100);
            uint256 toBurnAmount = amount.mul(_isDevelopmentFee).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_isLiquidityFee).div(_isTotalFeeContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_isMarketingFee).div(_isTotalFeeContract);
