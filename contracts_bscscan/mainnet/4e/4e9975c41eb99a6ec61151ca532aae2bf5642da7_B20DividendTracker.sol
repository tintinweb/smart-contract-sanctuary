...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    address public adminAddress = 0xF68ECA313CFc9EAad74fbDCf8e70721d44b0fd0b;
...
contract ThunderBnb is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public toBurnThunderCakeAddress = 0xce4A4A15FcCD532eAd67BE3ECf7e6122c61D06bb;
    uint256 public toBurnThunderCakeFee = 2;
    ...
    event UpdateA80DividendTracker(address indexed newAddress, address indexed oldAddress);
    event UpdateB20DividendTracker(address indexed newAddress, address indexed oldAddress);

    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);

    event BuyBackAndLiquifyEnabledUpdated(bool enabled);
    event MarketingEnabledUpdated(bool enabled);
    event A80DividendEnabledUpdated(bool enabled);
    event B20DividendEnabledUpdated(bool enabled);
    ...
    constructor() ERC20("Test1", "TEST1") {
        ...
        marketingWallet = 0xB1E798E8EA32a835dDB69bB1595527d0417944cf;
        teamWallet = 0xB34354B675C97fC35d257f78c57d25E7F8C4749F;
        a80DividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    function updateBurnThunderCakeAddress(address _newAddress) external onlyOwner {
        toBurnThunderCakeAddress = _newAddress;
    }
    ...
    function setToBurnThunderCakeFee(uint256 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        toBurnThunderCakeFee = newFee;
        _updateTotalFee();
    }
    ...
    function _updateTotalFee() internal {
        totalFees = buyBackAndLiquidityFee.add(marketingFee).add(a80DividendRewardsFee).add(b20DividendRewardsFee).add(toBurnThunderCakeFee);
    }
    ...
    function setA80DividendEnabled(bool _enabled) external onlyOwner {
        ...
        emit A80DividendEnabledUpdated(_enabled);
    }
    ...
    function setB20DividendEnabled(bool _enabled) external onlyOwner {
        ...
        emit B20DividendEnabledUpdated(_enabled);
    }
    ...
    function updateA80DividendTracker(address newAddress) external onlyOwner {
        ...
        require(newA80DividendTracker.owner() == address(this), "must be owned by ThunderBnb");
        ...
        emit UpdateA80DividendTracker(newAddress, address(a80DividendTracker));
    ...
    function updateB20DividendTracker(address newAddress) external onlyOwner {
        ...
        require(newB20DividendTracker.owner() == address(this), "must be owned by ThunderBnb");
        ...
        emit UpdateB20DividendTracker(newAddress, address(b20DividendTracker));
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if(toBurnThunderCakeFee > 0){
                    uint256 swapTokensToBurnThunderCake = contractTokenBalance.mul(toBurnThunderCakeFee).div(totalFees);
                    buyBackThunderCakeAndBurn(swapTokensToBurnThunderCake);
                }
    ...
    function buyBackThunderCakeAndBurn(uint256 amount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = toBurnThunderCakeAddress;
    ...
    function swapAndSendA80Dividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), a80DividendToken);
        uint256 a80Dividends = IERC20(a80DividendToken).balanceOf(address(this));
        transferDividends(a80DividendToken, address(a80DividendTracker), a80DividendTracker, a80Dividends);
    }

    function swapAndSendB20Dividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), b20DividendToken);
        uint256 b20Dividends = IERC20(b20DividendToken).balanceOf(address(this));
        transferDividends(b20DividendToken, address(b20DividendTracker), b20DividendTracker, b20Dividends);
    }
...
contract A80DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("A_Dividend_Tracker", "A_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
        claimWait = 3600;
...
contract B20DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("B_Dividend_Tracker", "B_Dividend_Tracker", 0x580dE58c1BD593A43DaDcF0A739d504621817c05) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
