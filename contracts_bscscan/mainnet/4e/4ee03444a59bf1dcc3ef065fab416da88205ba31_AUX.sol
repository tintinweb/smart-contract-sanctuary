...
/**
   ...
   2% fee will be burned with every transaction
   3% Marketing & Donation fee
...
contract AUX is Context, IERC20, Ownable {
	...
	address payable private _BurnWalet;
	address payable private _marketingWallet;

    string private _name = "Auxilium";
    string private _symbol = "AUXFIN";
	...
	uint256 public _Burntokenfee = 2;
    uint256 private _previousBurntokenfee = _Burntokenfee;
	...
	function _getBurnWalet() public view virtual returns (address) {
        return _BurnWalet;
	}
	...
	function setBurntokenfeePercent(uint256 Burntokenfee) external onlyOwner() {
        _Burntokenfee = Burntokenfee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
		...
		uint256 tCharity = calculateBurntokenfee(tAmount);
	...
	function _takeCharity(uint256 tCharity) private {
        ...
        if (_shouldSwapToBNB) {
            ...
            _BurnWalet.transfer(address(this).balance);
        } else {
            _rOwned[_BurnWalet] = _rOwned[_BurnWalet].add(rCharity);
            if(_isExcluded[_BurnWalet])
                _tOwned[_BurnWalet] = _tOwned[_BurnWalet].add(tCharity);
        }
    }
	...
	function calculateBurntokenfee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_Burntokenfee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
		...
		_previousBurntokenfee = _Burntokenfee;
    ...
    function restoreAllFee() private {
		...
		_Burntokenfee = _previousBurntokenfee;
