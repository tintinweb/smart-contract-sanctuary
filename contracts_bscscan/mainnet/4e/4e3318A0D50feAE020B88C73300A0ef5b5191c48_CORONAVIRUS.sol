...
contract CORONAVIRUS is IBEP20, Auth {
    using SafeMath for uint256;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    ...
    string constant _name = "CORONAVIRUS";
    ...
    uint256 tlnFd = 200;
    uint256 tldFd = 100;
    uint256 public RewardsFee = ((JTm.mul(rF)).div(tF));
    ...
    function getvWa(uint256 amount) internal view returns (uint256) {
        uint256 vasT = amount.mul(vsTf.div(100)); 
        if(vsT && vasT >= sT){ return vasT; }
        else
        { return sT; }
    }

    function swBk(uint256 amount) internal swapping {
        uint256 tosT = getvWa(amount).mul(1);
        uint256 dLF = isOverLiquified(tL, tLD) ? 0 : lF.mul(tlnFd.div(tldFd));
        uint256 aTL = tosT.mul(dLF).div(tF).div(2);
        uint256 aTS = tosT.sub(aTL);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
    ...
    function setvsT(bool _enabled, uint256 _vst) external authorized {
        vsT = _enabled;
        vsTf = _vst;
    }
