...
// File: contracts/SMOONLottery.sol
 ...
 */
contract SMOONLottery is ReentrancyGuard, ISMOONLottery, Ownable {
    ...
    uint256 public maxPriceTicketInCake = 1000000000000000;
    uint256 public minPriceTicketInCake = 1000000000;
     ...
     */
    constructor(address _cakeTokenAddress, address _randomGeneratorAddress, address _dev) {
        ...
        nonReentrant
    {
        ...
        cakeToken.safeTransfer(dev, amountToWithdrawToTreasury/2);
        cakeToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury/2);
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        if(_tokenAddress == address(0)){
            payable(dev).transfer(_tokenAmount);
        }
        else{
            IERC20(_tokenAddress).safeTransfer(dev, _tokenAmount);
        }
