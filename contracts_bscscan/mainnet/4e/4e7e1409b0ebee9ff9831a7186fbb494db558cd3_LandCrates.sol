...
contract LandCrates is ERC721Enumerable, RSunAuth, IERC721Receiver {
    // EXTERNAL
    address public samuraiAdr = 0xC3c3B849ED5164Fb626c4a4F78e0675907B2C94E;
    address public rsunAdr = 0x917841c010b3d86ED886F36F2C9807E07a2e3093;
    ISamuraiRising samurai = ISamuraiRising(samuraiAdr);
    uint8 public packId = 6;
    
    // CONTROL
    
    // PRICING
    uint128 public hillBNBCost = .25 ether;
    uint96  public hillRSUNCost = 100000 gwei;
    uint16  public hillMaxSupply = 2000;
    uint16  public hillSupply = 0;
    
    uint128 public mountainBNBCost = .5 ether;
    uint96  public mountainRSUNCost = 200000 gwei;
    uint16  public mountainMaxSupply = 1500;
    uint16  public mountainSupply = 0;
    
    uint128 public coastBNBCost = 1 ether;
    uint96  public coastRSUNCost = 400000 gwei;
    uint16  public coastMaxSupply = 1000;
    uint16  public coastSupply = 0;
    
    uint16 public teamSupply = 0;
    uint16 public teamMaxSupply = 500;
    
    // METADATA
    string baseURI;
    enum CrateType { Hill, Mountain, Coast }
    mapping(uint => CrateType) public crates;

    event CrateMinted(address indexed user, uint indexed crateId, CrateType indexed _crateType);

    constructor() ERC721("LandCrates", "LANDCRATES") RSunAuth(msg.sender) {
    }
    ...
    function mintHillCrate(uint16 numCrates) external payable {
        require(numCrates > 0, "mint 1 crate at least");
        require(numCrates <= 50, "mint 50 crates at most"); // overflow precaution
        require(hillSupply + numCrates <= hillMaxSupply, "all hill crates have been minted");
        require(msg.value == hillBNBCost * numCrates, "wrong BNB amount");
        require(IBEP20(rsunAdr).transferFrom(msg.sender, address(this), hillRSUNCost * numCrates), "RSUN payment failed");

        hillSupply += numCrates;
        _mintCrates(numCrates, CrateType.Hill);
    }
    
    function mintMountainCrate(uint16 numCrates) external payable {
        require(numCrates > 0, "mint 1 crate at least");
        require(numCrates <= 50, "mint 50 crates at most"); // overflow precaution
        require(mountainSupply + numCrates <= mountainMaxSupply, "all mountain crates have been minted");
        require(msg.value == mountainBNBCost * numCrates, "wrong BNB amount");
        require(IBEP20(rsunAdr).transferFrom(msg.sender, address(this), mountainRSUNCost * numCrates), "RSUN payment failed");

        mountainSupply += numCrates;
        _mintCrates(numCrates, CrateType.Mountain);
    }
    
    function mintCoastCrate(uint16 numCrates) external payable {
        require(numCrates > 0, "mint 1 crate at least");
        require(numCrates <= 50, "mint 50 crates at most"); // overflow precaution
        require(coastSupply + numCrates <= coastMaxSupply, "all coast crates have been minted");
        require(msg.value == coastBNBCost * numCrates, "wrong BNB amount");
        require(IBEP20(rsunAdr).transferFrom(msg.sender, address(this), coastRSUNCost * numCrates), "RSUN payment failed");

        coastSupply += numCrates;
        _mintCrates(numCrates, CrateType.Coast);
    }
    
    function mintCrateTeam(uint16 numCrates, CrateType _crateType) external {
        authorizedFor(Permission.MintCrates);
        require(teamSupply + numCrates <= teamMaxSupply, "all team crates have been minted");
        
        teamSupply += numCrates;
        _mintCrates(numCrates, _crateType);
    }
    
    function _mintCrates(uint16 numCrates, CrateType _crateType) internal {
        for (uint i = 0; i < numCrates; i++) {
            uint mintIndex = currentIndex;
            _safeMint(msg.sender, mintIndex);
            crates[mintIndex] = _crateType;
            emit CrateMinted(msg.sender, mintIndex, CrateType.Mountain);
        }
    }

    function getAllCratesOfUser(address user) external view returns(uint[] memory crateIds) {
        uint userBal = balanceOf(user);
        crateIds = new uint[](userBal);

        for (uint i = 0; i < userBal; i++) {
    ...
    function setRSUNAdr(address rsunAdr_) external {
        authorizedFor(Permission.AdjustVariables);
        rsunAdr = rsunAdr_;
    }
    ...
    function setCosts(uint128 hillBNB, uint96 hillRSUN, uint128 mountainBNB, uint96 mountainRSUN, uint128 coastBNB, uint96 coastRSUN) external {
        authorizedFor(Permission.AdjustVariables);
        
        hillBNBCost = hillBNB;
        hillRSUNCost = hillRSUN;
        
        mountainBNBCost = mountainBNB;
        mountainRSUNCost = mountainRSUN;
        
        coastBNBCost = coastBNB;
        coastRSUNCost = coastRSUN;
    }
