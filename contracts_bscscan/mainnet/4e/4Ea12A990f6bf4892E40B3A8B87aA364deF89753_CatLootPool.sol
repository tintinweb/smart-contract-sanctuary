...
contract CatLootPool is Ownable, ReentrancyGuard, IERC721Receiver, ERC721Holder {

    address public immutable CatlootContract;
    address public immutable CatnipToken;
    address public immutable NFTContract;
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 lastClaimBlock;
    uint256 claimPrice;

    constructor(address _CatlootContract, address _CatnipToken, address _MyRevengeNFT) Ownable() {
        CatlootContract = _CatlootContract;
        CatnipToken = _CatnipToken;
        NFTContract = _MyRevengeNFT;
        lastClaimBlock = block.number;
        claimPrice = 2000;
        }
    
    function safeTransferBNB(address to, uint256 value) public onlyOwner {
    ...
    function availableTokenID() public view returns (uint256) {
        return CatLoot(CatlootContract).tokenOfOwnerByIndex(address(this), 0);
    }

    function currentClaimPrice() public view returns (uint256){
        uint256 decay = (block.number - lastClaimBlock) * 50;
        if (decay >= 2000) {
            return 100e18;
        }
        return (claimPrice - decay) * 1e18;
    }
        
    function claim() public nonReentrant {
        uint256 currentPrice = currentClaimPrice();
        uint256 currentCatloot = availableTokenID();
        require(MyRevengeNFT(NFTContract).balanceOf(msg.sender) > 0, "Must own the Carl NFT"); //Need to own the Carl NFT to call this function.
        Catnip(CatnipToken).transferFrom(msg.sender, deadAddress, currentPrice); //Burns CATNIP tokens. Oh no!!
        CatLoot(CatlootContract).transferFrom(address(this), msg.sender, currentCatloot); //take the CATLOOT!
        lastClaimBlock = block.number;
        CatLoot(CatlootContract).claim{value: 0.25 ether}(1);
        }

    function withdrawCatLoot(uint256 tokenID) public onlyOwner {
        CatLoot(CatlootContract).transferFrom(address(this), msg.sender, tokenID); //take the CATLOOT!
    }
}
...
contract Catnip is ERC20("Catnip", "CATNIP"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner.
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
    }
