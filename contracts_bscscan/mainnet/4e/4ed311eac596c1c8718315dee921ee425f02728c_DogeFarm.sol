...
//
// $DOGEFARM proposes an innovative feature in its contract.
//
// DIVIDEND YIELD PAID IN DOGECOIN! With the auto-claim feature,
// simply hold DOGEFARM and you'll receive $DOGECOIN automatically in your wallet.
// 
// Buy DOGEFARM and get rewarded in $DOGECOIN on every transaction!
//
//
// 📱 Telegram: https://t.me/DogeFarmOfficial
// 🌎 Website: https://www.DogeFarm.xyz
//
...
contract DogeFarm is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DogeFarmDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DogeFarm", "DOGEFARM") {
    	...
    	dividendTracker = new DogeFarmDividendTracker();

    	buyBackWallet = 0xeF982Ebec8C6CaB7F5836674864F9b79204d88b3;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "DogeFarm: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "DogeFarm: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	...
  	function prepareForPreSale() external onlyOwner {
        ...
        marketingFee = 0;
        maxBuyTranscationAmount = 100000000000000000000000 * (10**18);
        maxWalletToken = 10000000000000000000000000 * (10**18);
    }
    
    function afterPreSale() external onlyOwner {
        dividendRewardsFee = 10;
        marketingFee = 5;
        maxBuyTranscationAmount = 30000000000000000000000000000 * (10**18);
        maxWalletToken = 15000000000000000000000000 * (10**18);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "DogeFarm: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DogeFarm: The dividend tracker already has that address");

        DogeFarmDividendTracker newDividendTracker = DogeFarmDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DogeFarm: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DogeFarm: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DogeFarm: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DogeFarm: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DogeFarm: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DogeFarm: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "DogeFarm: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DogeFarm: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DogeFarm: Cannot update gasForProcessing to same value");
...
contract DogeFarmDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DogeFarm_Dividend_Tracker", "DogeFarm_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DogeFarm_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DogeFarm_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogeFarm contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DogeFarm_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DogeFarm_Dividend_Tracker: Cannot update claimWait to same value");
