...
/**
 * OFFICIAL Astro Finance MasterChef
 * 
 * 
 * Telegram https://t.me/astroprotocol
 * Website  https://twitter.com/LaikaProtocol
 * 
 * A second generation yield farm on Binance Smart Chain.
 * 
 * 
*/
...
contract AstroToken is BEP20('AstroProtocol', 'ASTRO') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "ASTRO::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "ASTRO::delegateBySig: invalid nonce");
        require(now <= expiry, "ASTRO::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "ASTRO::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ASTRO (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "ASTRO::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of ASTRO. He can make ASTRO and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once ASTRO is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ASTRO
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accASTROPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accASTROPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. ASTRO to distribute per block.
        uint256 lastRewardBlock;  // Last block number that ASTRO distribution occurs.
        uint256 accEggPerShare;   // Accumulated ASTRO per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The Astro TOKEN !
    AstroToken public egg;
    // Dev address.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _astroPerBlock,
        uint256 _startBlock
    ) public {
        egg = _astro;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        eggPerBlock = _astroPerBlock;
    ...
    // Update the given pool's AstroToken allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending AstroToken on frontend.
    ...
    // Safe egg transfer function, just in case if rounding error causes pool to not have enough ASTRO.
    ...
    function updateEmissionRate(uint256 _eggPerBlock) public onlyOwner {
        require(_eggPerBlock <= 25000000000000000000);
        massUpdatePools();
