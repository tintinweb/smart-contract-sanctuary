...
contract AlpacaSoloBank is Ownable, ReentrancyGuard {
    ...
    function add(address _depositToken, bool _withUpdate) public onlyOwner nonDuplicated(_depositToken) {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 lastRewardBlock = block.number > startBlock ? block.number : startBlock;
        uint256 soloPid = ISolo(soloContract).pidOfToken(_depositToken);
    ...
    function userDepositInfo(uint256 _pid, address _user) public view returns (uint256 deposit, uint256 earned) {
        ...
        deposit = user.amount;
        earned = user.rewardDebt.add(unclaimedReward(_pid));
    }
