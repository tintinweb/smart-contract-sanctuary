...
//
// $BabyApeBanana proposes an innovative feature in its contract.
//
// DIVIDEND YIELD PAID IN $Banana! With the auto-claim feature,
// simply hold $BabyApeBanana and you'll receive $Banana automatically in your wallet.
// 
//
// 📱 Telegram: t.me/babyapebanana
// 🌎 Website: www.babyapebanana.com
// 🌐 Twitter: twitter.com/babyapebanana_
...
import "./BabyApeBananaTransferHelper.sol";

contract BabyApeBanana is ERC20, Ownable {
	...
	BabyApeBananaDividendTracker public dividendTracker;
	BabyApeBananaTransferHelper private transferHelper;

	address public deadWallet = 0x000000000000000000000000000000000000dEaD;

	address public immutable BANANA = 0x603c7f932ED1fc6575303D8Fb018fDCBb0f39a95;
	...
	constructor(address routerAddress, address payable marketingWalletAddress) public ERC20("BabyApeBanana", "BABYAPEBANANA") {
		dividendTracker = new BabyApeBananaDividendTracker();
		transferHelper = new BabyApeBananaTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "BABYAPEBANANA: The dividend tracker already has that address");

		BabyApeBananaDividendTracker newDividendTracker = BabyApeBananaDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "BABYAPEBANANA: The new dividend tracker must be owned by the BABYAPEBANANA token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "BABYAPEBANANA: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "BABYAPEBANANA: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "BABYAPEBANANA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "BABYAPEBANANA: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "BABYAPEBANANA: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "BABYAPEBANANA: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "BABYAPEBANANA: Cannot update gasForProcessing to same value");
	...
	function swapAndSendDividends(uint256 amountIn) private {
		uint256 dividends = swapExactETHForTokens(amountIn, BANANA);
...
contract BabyApeBananaDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	...
	constructor() public DividendPayingToken("BABYAPEBANANA_Dividend_Tracker", "BABYAPEBANANA_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "BABYAPEBANANA_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "BABYAPEBANANA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BABYAPEBANANA contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "BABYAPEBANANA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "BABYAPEBANANA_Dividend_Tracker: Cannot update claimWait to same value");
