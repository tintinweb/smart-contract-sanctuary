  ...
  UUPSUpgradeable
{
  ...
  constructor() initializer {
    ...
    GnosisSafeVault = payable(address(0x3D87f8923c3a16c5AB5D460ffA548418b58d9Fd8)); //
    specialBlock = 0; // APPROX a month after launch. 
    DragoAddress = address(0x3D87f8923c3a16c5AB5D460ffA548418b58d9Fd8);
    ...
    specialStartBlockTime = 0;
    specialEndBlockTime = 0;
    rewardFactor = 3;
  }
  ...
  function stakeKingdom(uint256 kingdomNftID) public onlyOwner {
    ...
    // if there are Kingdoms with dragosnft of this kind at stack, make a claim without un-stacking
  ...
  function unStakeKingdom(uint256 genericType) public {
    ...
    uint256 maxSlots = addressToKingdomsType[sender][genericType].length * 3;
    uint256 freeSlots = maxSlots - dragosByType;
    require(freeSlots >= 3, "still dragosnft on Kingdom");
  ...
  function unStakeDragos(uint256 genericType) public {
    ...
    IDragos dragosInstance = IDragos(dragoNFTContract);
    require(addressToDragosType[sender][genericType].length > 0, "not dragosnft of this type");
  ...
  */
  function claimRewards(uint256 genericType) public {
    address sender = address(msg.sender);
    require(addressToDragosType[sender][genericType].length > 0, "no dragosnft of this type");
  ...
  function getDragoAge(uint256 dragoNftId) internal view returns (uint256){
    ...
    (
	    ...
	    bool onSale,
	    uint256 price
    ) = dragoInstance.dragosnft(dragoNftId - 1);
    return bornAt;
  }
  ...
  function fightAgainst(uint256 dragoNftID, uint256 monster) public returns (bool) {
    ...
    bool isWarrior = dragoInstance.getTypeIdByNftId(dragoNftID) >= 7;
    require(isOwner, "This DRAGO doesn't belong to you");
    require(isWarrior, "Only Light, Energy, and Darkness Dragos can battle");
    IDrago DragoInstance = IDrago(DragoAddress);
    require(DragoInstance.balanceOf(sender) >= monsters_[monster].bait, "Insufficient Drago to fight");
    require(DragoInstance.allowance(sender, address(this)) >= monsters_[monster].reward, "Insufficient allowance");

    bool won = fight(monsters_[monster].winningProb);
    if(won){
      DragoInstance.transfer(sender, (monsters_[monster].reward));
    ...
    {
      DragoInstance.transferFrom(sender, address(this), monsters_[monster].bait);
      totalBattlesByNftId[dragoNftID] += 1;
