...
contract SousChef {
    ...
    // Info of each user that stakes xBGSP tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = xBGSP.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 xBGSPSupply = xBGSP.balanceOf(address(this));
        if (xBGSPSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(xBGSPSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit xBGSP tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];

        updatePool();
        xBGSP.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw xBGSP tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        xBGSP.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        xBGSP.safeTransfer(address(msg.sender), user.amount);
