...
contract FixedBEP20 is Context, IBEP20, Ownable {
    ...
    */
    ...
    function _mint(address account, uint256 amount) internal {
        require(account != address(0), 'WMEL: mint to the zero address');
        
        uint256 remainingAmount = _totalSupply.sub(_totalMinted);
        require(remainingAmount != 0, 'WMEL: mint zero amount');
...
// File: contracts\WMELToken.sol

pragma solidity 0.6.12;


// WMELToken with Governance.
contract WMELToken is FixedBEP20('WMEL Token', 'WMEL', 47000000e18) {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The block number when WMEL mining ends.
    ...
    function pendingWMEL(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 WMELReward = poolReward.mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 poolReward = getBlockReward(pool.lastRewardBlock, block.number);
        uint256 WMELReward =  poolReward.mul(pool.allocPoint).div(totalAllocPoint);
        uint256 treasuryReward = WMELReward.mul(8).div(100);
        uint256 contractReward = WMELReward.mul(92).div(100);
        WMEL.mint(devaddr, treasuryReward);
        WMEL.mint(address(this), contractReward);
    ...
    function WMELPerBlock() public view returns (uint256) {
        uint256 reward = 0;
        for (uint256 i = 0; i < rewardInfo.length; i++) {
            BlockRewardInfo memory info = rewardInfo[i];
            if (block.number >= info.firstBlock && block.number < info.lastBlock) {
                reward = info.reward;
                break;
            }
        }
