...
contract DividendDistributor is IDividendDistributor {
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        IBEP20 token = getShareholderDividendToken(shareholder);
    ...
    function setInterestToken(IBEP20 token, address benefactor) external onlyToken {
        ...
        IBEP20 oldToken = getShareholderDividendToken(benefactor);
        currentDividendToken[benefactor] = token;
        distributeDividend(benefactor);
    }

    function getShareholderDividendToken(address _benefactor) public view returns (IBEP20) {
...
//        if(!launched() && recipient == pair){ require(_balances[sender] > 0); launch(); }
    ...
    function getShareholderDividendToken(address user) internal returns (IBEP20) {
        return distributor.getShareholderDividendToken(user);
    }
