 ...
 */
contract TukStrategy is Ownable, Pausable {
     ...
     * {tuk} - Token generated by staking our funds. 
     ...
     */
    address constant public tuk = address(0xe363096E780FB8675966b3FFCa88BEDD62b838fA);
    address constant public ttusd = address(0x13A5bCFf72b024619Df4D62093941c382b6D6348);
     ...
     */
    address constant public unirouter  = address(0x8CA43fC055Ae47E11959e079F37051831F850cA7);
    address constant public masterchef = address(0x30D010a7815443E01ba641Ba4Ddab36ac50847c9);
    uint public poolId;
    
    // performance fee
    uint256 constant public MAX_FEE = 10000;
    uint256 public performanceFee = 250; // 2.5%
    uint256 public callerFee = 3000; // 0.75% 
    address public fundAddress; // 1.75%
     ...
     * {tukToLp0Route} - Route we take to get from {tuk} into {lpToken0}.
     * {tukToLp1Route} - Route we take to get from {tuk} into {lpToken1}.
     */
    address[] public tukToLp0Route;
    address[] public tukToLp1Route;
     ...
     */
    constructor(address _lpPair, uint _pid, address _vault, address _fundAddress, address _feeAddress) {
        lpPair = _lpPair;
        ...
        if (lpToken0 == ttusd) {
            tukToLp0Route = [tuk, ttusd];
        } else if (lpToken0 != tuk) {
            tukToLp0Route = [tuk, ttusd, lpToken0];
        }

        if (lpToken1 == ttusd) {
            tukToLp1Route = [tuk, ttusd];
        } else if (lpToken1 != tuk) {
            tukToLp1Route = [tuk, ttusd, lpToken1];
        }

        IERC20(lpPair).safeApprove(masterchef, MAX_UINT256);
        IERC20(tuk).safeApprove(unirouter, MAX_UINT256);
     ...
     * It deposits {lpPair} in the MasterChef to farm {tuk}
     */
     ...
     */
    function withdraw(uint256 _amount, bool _isNoFee) external onlyVault {
        uint256 pairBal = IERC20(lpPair).balanceOf(address(this));
        ...
        if (!_isNoFee) {
            withdrawalFee = pairBal.mul(WITHDRAWAL_FEE).div(WITHDRAWAL_MAX);
            IERC20(lpPair).safeTransfer(feeAddress, withdrawalFee);
        }
     ...
     * 3. It swaps the {tuk} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        // performance fee 2.5%
        uint256 feeBalance = IERC20(tuk).balanceOf(address(this)).mul(performanceFee).div(MAX_FEE);
        
        // caller 0.75% (cost for compound)
        uint256 callerBalance = feeBalance.mul(callerFee).div(MAX_FEE);
        
        // remaining 1.75%
        // 80% = buy back fund
        // 20% = dev fund
        uint256 fundBalance = feeBalance.sub(callerBalance);
        
        IERC20(tuk).transfer(msg.sender, callerBalance);
        IERC20(tuk).transfer(fundAddress, fundBalance);
    }

    /**
     * @dev Swaps {tuk} for {lpToken0}, {lpToken1} & {wbnb}
     */
    function addLiquidity() internal {   
        uint256 half = IERC20(tuk).balanceOf(address(this)).div(2);
        
        if (lpToken0 != tuk) {
            IUniRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(half, 0, tukToLp0Route, address(this), block.timestamp.add(600));
        }

        if (lpToken1 != tuk) {
            IUniRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(half, 0, tukToLp1Route, address(this), block.timestamp.add(600));
        }

        uint256 lp0Bal = IERC20(lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lpToken1).balanceOf(address(this));
        IUniRouter(unirouter).addLiquidity(lpToken0, lpToken1, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp.add(600));
    }
    ...
    function pause() public onlyOwner {
        ...
        IERC20(tuk).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
    }
    ...
    function setPerformanceFee(uint256 _performanceFee) external onlyOwner {
        require(_performanceFee < 350, "too much fee");
