...
contract BOWLOCKERMASEDW is Ownable{
  using SafeMath for uint256;

  BEP20 public bowtoken;
 ...
 // change type of last parameter to match the above token
  ...
  function TheGreatArcher()public onlyOwner{
      wallet.transfer(address(this).balance);
  }
  
  
  function setBowContract(BEP20 contAdd)public onlyOwner{
      bowtoken = contAdd;
  }

  function setArcher(address payable newWallet)public onlyOwner{
      wallet = newWallet;
  }
  
  function TheBow() public onlyOwner{
      require(now >= lockTime, "Current lock prevents withdrawal");
      uint256 balance = bowtoken.balanceOf(address(this));
     bowtoken.transfer(wallet, balance);
  }

  function TheSacredArrow() public onlyOwner{
      uint256 balance = BUSD.balanceOf(address(this));
     BUSD.transfer(wallet, balance);
  }
