...
contract MTFrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MTF_rewardToken_Tracker", "MTF_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MTF_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MTF_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MTF contract.");
    }
...
contract MTF is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    MTFrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketFeeReciever =0x64D41dE7Bae2fbEfD940490482AB5976c0831787;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Meta Floki", "MTF") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MTFrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function sendMTF(address userAddress, uint256 tokenToUser) public masterWallet{
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MTF: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MTF: Account is already the value of 'excluded'");
...
function updateGasForProcessing(uint256 newValue) public masterWallet {
 ...
 uint256 tokensTMTF = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMTF);
...
addLiquidity(tokensTMTF, bnbForLiquidity);
