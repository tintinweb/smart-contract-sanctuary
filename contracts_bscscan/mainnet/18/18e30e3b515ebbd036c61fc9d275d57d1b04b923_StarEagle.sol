...
contract StarEagle is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    StarEagleDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x14bDbf57387bab3b328A1Fbe2E5fDBb1dAF7069d); 
    address payable public operations2Address =
        payable(0xB0142D3a6978AAc3b3dC1e0877559A930E0a0181);
    ...
    constructor() ERC20("Star Eagle", "Star Eagle") {
        dividendTracker = new StarEagleDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        StarEagleDividendTracker newDividendTracker = StarEagleDividendTracker(
            payable(newAddress)
        );
...
contract StarEagleDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "StarEagle_Dividend_Tracker",
            "StarEagle_Dividend_Tracker"
        )
    {
        starEagle = StarEagle(mainContract);
    ...
    function _transfer(
        ...
        require(false, "StarEagle_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "StarEagle_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main StarEagle contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(starEagle);

            uint256 prevBalance = starEagle.balanceOf(address(this));
            ...
            {
                uint256 received = starEagle.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    starEagle.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
