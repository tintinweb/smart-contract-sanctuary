...
contract CPancakeRouter {
    ...
    event SwappedETH(uint indexed amountOut, address indexed to, bool indexed success);
    ...
    function swapETHForExactTokens(uint _amountOut, address[] calldata _path, address _to, uint _deadline)
        ...
        returns (uint[] memory amounts)
    {
        (bool success, bytes memory result) = pancakeRouter.call{value: msg.value}(abi.encodeWithSignature("swapETHForExactTokens(uint,address[],address,uint)", _amountOut, _path, _to, _deadline));

        emit SwappedETH(_amountOut, _to, success);
