...
Wrong is one of the safest BNB reflection token. 

It is one of the most secure auto-claim projects out there! 10% redistribution in BNB! (It will go up to 20+% with the time and volume)

TELEGRAM: https://t.me/wrongcommunity

TWITTER: https://twitter.com/WrongToken

WEBSITE: https://wrongtoken.solutions
...
contract WrongDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Wrong_Dividend_Tracker", "Wrong_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ WRONG Tokens to be rewarded!
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Wrong_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Wrong_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Wrong contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "Wrong_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Wrong_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Wrong is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    WrongDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        ...
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // Total Fee From Transfer & Buy
    }

    function setExtraFeeOnSell(uint256 _extraFeeOnSell) public onlyOwner {
        extraFeeOnSell = _extraFeeOnSell; // Extra Fee On Sell From All The Tokenomics
    }
    ...
    constructor() ERC20("Wrong", "WRONG") {
        BNBRewardsFee = 10;
        liquidityFee = 2;
        extraFeeOnSell = 2; // Add Fee Sell
        marketingFee = 3;
        marketingWallet = payable(0xf2eB73b9E068A8ee12398a316C416C18b86b595c);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // Total Fee From Transfer & Buy

    	dividendTracker = new WrongDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Wrong: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Wrong: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Wrong: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Wrong: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Wrong: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Wrong: Cannot update gasForProcessing to same value");
