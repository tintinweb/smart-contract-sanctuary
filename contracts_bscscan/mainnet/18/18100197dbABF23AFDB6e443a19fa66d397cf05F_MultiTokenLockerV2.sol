...
contract MultiTokenLockerV2 is SafeOwnable {
    ...
    function defaultListTokesLength() external view returns (uint) {
    ...
    function inviterPending(address _user) external view returns (ITokenLockerV2.RewardInfo[] memory rewards) {
        ...
        for (uint i = 0; i < tokenLockers.length; i ++) {
            ...
            for (uint j = 0; j < currentRewards.length; j ++) {
                ...
                rewards[index - 1].totalAmount = rewards[index - 1].totalAmount.add(currentRewards[j].totalAmount);
                rewards[index - 1].pendingAmount = rewards[index - 1].pendingAmount.add(currentRewards[j].pendingAmount);
            }
        }
    }
