...
// File: contracts/SAFEMIXMATICDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DividendToken = address(0xCC42724C6683B7E57334c4E856f4c9965ED682bD); //Change dividend
...
// File: contracts/SAFEMIXMATIC.sol
...
contract SAFEMIXMATIC is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public DividendToken = address(0xCC42724C6683B7E57334c4E856f4c9965ED682bD); //change dividend token

    bool private swapping;

    SAFEMIXMATICDividendTracker public dividendTracker;
    ...
    address _marketingWalletAddress = 0xE14E79965E0C40470f65a36455fC91611CFF90D7; //change this
    ...
    constructor() ERC20("SAFEMIXMATIC", "SMMATIC") {
    	...
    	dividendTracker = new SAFEMIXMATICDividendTracker();
    ...
    function changeSellFee(uint256 amount) external onlyOwner {
	    require(amount >= 100 && amount <= 150, "SAFEMIXMATIC: SellFee must be updated to between 100 and 150.");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SAFEMIXMATIC: The dividend tracker already has that address");

        SAFEMIXMATICDividendTracker newDividendTracker = SAFEMIXMATICDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SAFEMIXMATIC: The new dividend tracker must be owned by the SAFEMIXMATIC token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SAFEMIXMATIC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SAFEMIXMATIC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SAFEMIXMATIC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SAFEMIXMATIC: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SAFEMIXMATIC: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SAFEMIXMATIC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SAFEMIXMATIC: Cannot update gasForProcessing to same value");
    ...
    function updateRewardsFee(uint256 RewardFeePercent) external onlyOwner{
        require(RewardFeePercent <= 20, "SAFEMIXMATIC: updateRewardsFee must be between 0 and 20.");
        TokenRewardsFee = RewardFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateLiquidityFee(uint256 liquidityFeePercent) external onlyOwner{
        require(liquidityFeePercent <= 5, "SAFEMIXMATIC: updateRewardsFee must be between 0 and 5.");
        liquidityFee = liquidityFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee);
    }
    
    function updateMarketingFee(uint256 marketingFeePercent) external onlyOwner{
        require(marketingFeePercent <= 10, "SAFEMIXMATIC: updateRewardsFee must be between 0 and 10.");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SAFEMIXMATIC: This account cannot send tokens until trading is enabled");
        }
...
contract SAFEMIXMATICDividendTracker is Ownable, DividendPayingToken{
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SAFEMIXMATIC_Dividend_Tracker", "SAFEMIXMATIC_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SAFEMIXMATIC: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SAFEMIXMATIC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SAFEMIXMATIC_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }

    function updateMinimunForRewards(uint256 newValueMinimunForRewards) external onlyOwner {
        require(newValueMinimunForRewards >= 10000 && newValueMinimunForRewards <= 500000, "SAFEMIXMATIC_Dividend_Tracker: newValueMinimunForRewards must be updated to between 10000 and 500000");
        require(newValueMinimunForRewards != minimunForRewards, "SAFEMIXMATIC_Dividend_Tracker: Cannot update claimWait to same value");
