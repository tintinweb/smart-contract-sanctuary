...
/**
RichADA | CARDANO Reflection

Tokenomics : 4% AutoLP Tax to make the chart Less Volatile.
             11% ADA Rewards ,Investors can earn back their money without selling.
             2% Marketing Fee ,Everything will be utilized to pump RICA Through the roof 
             
WhitePaper with Marketing Roadmap revealed on our Social Mediums :
Website : www.RichADA.co
Telegram : RichADAbsc
Twitter : https://twitter.com/RichADAbsc

Cardano Token Address : 0x3ee2200efb3400fabb9aacf31297cbdd1d435d47
...
// File: contracts/RichADADividendPayingToken.sol
...
contract RichADA is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   RichADADividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet = 0x0e2be7D90C23A398A2AC1e38dcFB2a37e831894b;
    ...
    constructor() public ERC20("RichADA", "RICA") {
    	...
    	dividendTracker = new RichADADividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "RichADA: The dividend tracker already has that address");

        RichADADividendTracker newDividendTracker = RichADADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "RichADA: The new dividend tracker must be owned by the RichADA token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "RichADA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RichADA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "RichADA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RichADA: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "RichADA: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RichADA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RichADA: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "RichADA: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of RichADA
            path,
            recipient,
            block.timestamp
        );
...
contract RichADADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("RichADA_Dividend_Tracker", "RichADA_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "RichADA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "RichADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RichADA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RichADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RichADA_Dividend_Tracker: Cannot update claimWait to same value");
