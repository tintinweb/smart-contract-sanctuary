...
contract StrategyPancakeSwap is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant feeAddress = 0x8E73A59B63b241811D95611D5Ee04990D5894aa3;
    address public constant withdrawFeeAddress = 0x8E73A59B63b241811D95611D5Ee04990D5894aa3;
    ...
    function _farm() internal returns (uint256) {
        ...
        IMasterchef(pancakeSwapAddress).deposit(poolId, wantAmt);
    ...
    function withdraw(address _userAddress, uint256 _wantAmt) external onlyOwner nonReentrant returns (uint256) {
        ...
        if (_wantAmt > wantAmt) {
            IMasterchef(pancakeSwapAddress).withdraw(poolId, _wantAmt.sub(wantAmt));
    ...
    function earn() public nonReentrant whenNotPaused onlyGov {
        // Harvest farm tokens
        IMasterchef(pancakeSwapAddress).deposit(poolId, 0);

        // Converts farm tokens into want tokens
        uint256 earnedAmt = IERC20(earnedAddress).balanceOf(address(this));

        if (earnedAmt > 100000000) {
    ...
    function vaultSharesTotal() public view returns (uint256) {
        (uint amount, ) = IMasterchef(pancakeSwapAddress).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function _resetAllowances() internal {
        IERC20(wantAddress).safeApprove(pancakeSwapAddress, uint256(0));
    ...
    function panic() external onlyGov {
        _pause();
        IMasterchef(pancakeSwapAddress).withdraw(poolId, vaultSharesTotal());
    }
