...
contract $SIEPresale is Ownable {
     ...
     */
    modifier isActive() {
        require(block.timestamp > startDate, "$SIEPresale: You are too early!");
        require(block.timestamp < endDate, "$SIEPresale: You are too late!");
        _;
    }
    ...
    modifier tokensUnlocked() {
        require(block.timestamp > unlockDate, "$SIEPresale: tokens not unlocked yet");
        _;
    }

    /**
     * @dev Initializes the Fair-Launch by giving the dates, commitments and prices
     * Emits the event "PresaleInitialized" upon completion.
     *
     */
    ...
    onlyOwner
    {
        require(isInitialized == false, "$SIEPresale: Presale has already been initialized.");
        require(_softCap < _hardCap, "$SIEPresale: softCap cannot be higher then hardCap");
        require(_startDate < _endDate, "$SIEPresale: startDate cannot be after endDate");
        require(_endDate > block.timestamp, "$SIEPresale: endDate must be in the future");
        require(_minCommitment < _maxCommitment, "$SIEPresale: minCommitment cannot be higher then maxCommitment");
        require(_pricePancakeswap < _pricePresale, "$SIEPresale: Pancakeswap price cannot be lower then Fair-Launch");
        require(_unlockDate > _endDate || _unlockDate == 0, "$SIEPresale: invalid unlockDate");
    ...
    */
    ...
    function purchaseTokens() external payable isActive  {
        ...
        require(!isClosed, "$SIEPresale: sale closed");
        require(amount >= minCommitment, "$SIEPresale: amount to low");
        require(tokensPurchased[msg.sender].add(amount) <= maxCommitment, "$SIEPresale: maxCommitment reached");
        require(tokensSold.add(amount) <= hardCap, "$SIEPresale: hardcap reached");
    ...
    function closeSale() external tokensUnlocked {
        require(!isClosed, "$SIEPresale: already closed");
        require(block.timestamp > endDate || tokensSold == hardCap, "$SIEPresale: endDate not passed or hardcap not reached");
        require(tokensSold >= softCap, "$SIEPresale: softCap not reached");
        isClosed = true;
    ...
    function claimTokens() external tokensUnlocked {
        require(isClosed, "$SIEPresale: sale not closed");
        require(canClaimTokens, "$SIEPresale: Claiming is not allowed yet!");
        require(tokensPurchased[msg.sender] > 0, "$SIEPresale: no tokens to claim");
    ...
    function releaseTokens() external tokensUnlocked {
        require(!isClosed, "$SIEPresale: cannot release tokens for closed sale");
        require(softCap > 0, "$SIEPresale: no softCap");
        require(block.timestamp > endDate, "$SIEPresale: endDate not passed");
        require(tokensPurchased[msg.sender] > 0, "$SIEPresale: no tokens to release");
        require(tokensSold < softCap, "$SIEPresale: softCap reached");
    ...
    function getReservedTokens() external view returns (uint256) {
        return tokensPurchased[msg.sender] > 0 ? tokensPurchased[msg.sender].mul(pricePresale).div(10**9) : 0;
    }
