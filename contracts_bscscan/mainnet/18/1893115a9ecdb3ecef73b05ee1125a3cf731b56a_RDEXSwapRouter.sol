...
library RDEXSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'f5727f6af82de0d6d1b4dbe64bec47cb353f96a18c8ebae674ba65c44911e33d' // init code hash
            ))));
    }
...
contract RDEXSwapRouter is IRDEXSwapRouter02 {
    ...
    function blacklistToken(address token, bool value) external onlyOwner{
    ...
    function setIDOStartTime(uint256 _time) public onlyOwner {
        idoStartTime = _time;
    }

    function setIDOEndTime(uint256 _time) public onlyOwner {
        idoEndTime = _time;
    }

    function setIDOReached(bool _reached) public onlyOwner {
        idoReached = _reached;
    }
    ...
    function removeIDOList(address[] memory _users) public onlyOwner(){
        for (uint i = 0; i < _users.length; i ++)
            _idoList[_users[i]] = 0;     
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        ...
        if (path[0] == tokenAddress) {
            require(block.timestamp > _idoList[msg.sender].add(SELL_DELAY) && idoReached && (block.timestamp < idoStartTime && block.timestamp > idoEndTime), "IDO DELAY ERROR!");
        ...
        for (uint i; i < path.length - 1; i++) {
            require(!_isBlacklistedToken[path[i + 1]], 'Blacklisted Token');
