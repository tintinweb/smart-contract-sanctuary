...
contract LZToken is BEP20('LaunchZone', 'LZ'), Pausable, ILZ, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedMintCaller() {
        require(_msgSender() == owner() || _authorizedMintCaller[_msgSender()],"LZ: MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    
    uint256 public _liquidityFee = 2;
    uint256 public _stakingAndBurnAndTeamAlloc = 4;
    ...
    constructor() public {
        ...
        _liquidityReservoir = 50000 * (10 ** uint256(decimals()));
        _maxCap = 50000000 * (10 ** uint256(decimals()));
    }
    ...
    function mintUnlockedToken(address to, uint256 amount) onlyAuthorizedMintCaller external override {
        require(amount.add(totalSupply()) <= _maxCap, "Max Cap is reached");
        _mint(to, amount);
    ...
    function setStakingAndBurnAndTeamPercent(uint256 stakingAndBurnAndTeamAlloc) external onlyOwner() {
        _stakingAndBurnAndTeamAlloc  = stakingAndBurnAndTeamAlloc;
    }
    
    // In some circumstance, we should not set fee on transfer, eg: Transfer from owner to distribute bounty, from depositing to swap for liquidity
    ...
    function setFoundation(address foundation) external {
        ...
        delete _isExcludedFromFee[_foundation];
        _isExcludedFromFee[foundation] = true;
