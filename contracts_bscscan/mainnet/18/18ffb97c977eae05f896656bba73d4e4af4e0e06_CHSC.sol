...
/* 
CHRISTMAS CASINO BY SIGMACORPS
...
contract CHSC is Context, IERC20, Ownable {
    ...
    mapping(address=>uint) public BuyForJackpot;
    mapping(address=>bool) public JackpotAdded;
    address [] public eligibleForJackpot;
    uint public Winners=3;
    uint256 public _BuyForJackpot = 1000 * 10**_decimals;
    ...
    uint256 public maxBuyAmount =  5000 * 10**_decimals;
    uint256 public maxSellAmount = 2000 * 10**_decimals;
    ...
    address public marketingAddress = 0xfCB5ac90CFDB6F5A1b631Fb7A70F69C9076F57c9;
    address public jackpotAddress = 0xFD2a678316B3f69DF49b5022d8D5816D89F27Db0;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "CHRISTMAS CASINO";
    string private constant _symbol = "CHSC";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[jackpotAddress] = true;
    ...
    function setMinBuyForJackpot(uint256 value) external onlyOwner{
        _BuyForJackpot = value* 10**_decimals;
    }

    function setTotalWinners(uint value) external onlyOwner{
        Winners = value;
    }
    ...
    function setFeeRates(uint256 _rfi, uint256 _jackpot, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.jackpot = _jackpot;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _jackpot, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.jackpot = _jackpot;
    ...
    function _takejackpot(uint256 rjackpot, uint256 tjackpot) private {
        totFeesPaid.jackpot +=tjackpot;
        if(_isExcluded[address(this)]){
             _tOwned[address(this)]+=tjackpot;
        }
        _rOwned[address(this)] +=rjackpot;
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rjackpot, to_return.rMarketing, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tjackpot = tAmount*sellFeeRates.jackpot/1000;
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tjackpot-s.tMarketing-s.tLiquidity-s.tBuyback;
        }
        else{
            s.tRfi = tAmount*feeRates.rfi/1000;
            s.tjackpot = tAmount*feeRates.jackpot/1000;
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rjackpot,uint256 rMarketing, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rjackpot = s.tjackpot*currentRate;
        rMarketing = s.tMarketing*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        rBuyback = s.tBuyback*currentRate;
        rTransferAmount =  rAmount-rRfi-rjackpot-rMarketing-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rjackpot,rMarketing,rLiquidity, rBuyback);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if( from != owner() &&
           ...
           uint256 walletCurrentBalance = balanceOf(to);
            ...
            if (amount >= _BuyForJackpot && JackpotAdded[to] == false) {
			     JackpotAdded[to] = true;
        ...
        if( from != owner() &&
            ...
            if ((balanceOf(from) - (amount)) <= _BuyForJackpot && JackpotAdded [from] == true) {
				//Sold tokens, end balance below jackpot minimum
                JackpotAdded[from] = false; 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        _takejackpot(s.rjackpot,s.tjackpot);
        _takeLiquidity(s.rLiquidity,s.tLiquidity);
        _takeMarketing(s.rMarketing, s.tMarketing);
        _takeBuyback(s.rBuyback, s.tBuyback);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tjackpot + s.tBuyback + s.tMarketing);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.jackpot + feeRates.marketing) * 2;
        ...
        // Send BNB to jackpotWallet
        uint256 jackpotAmt = unitBalance * 2 * feeRates.jackpot;
        if(jackpotAmt > 0){
          payable(jackpotAddress).transfer(jackpotAmt);
        }
    ...
    function updatejackpotWallet(address newWallet) external onlyOwner{
        require(jackpotAddress != newWallet ,'Wallet already set');
        jackpotAddress = newWallet;
        _isExcludedFromFee[jackpotAddress];
    }
