...
import "../../interfaces/foodcourt/ICafeteriaV2.sol";
import "../../interfaces/IByalan.sol";

import "../ByalanIsland.sol";
import "../Sailor.sol";

contract FoodcourtByalanSingle is ByalanIsland, Sailor, IByalan, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Address for address payable;

    // Tokens used
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant COUPON = 0x084bb94e93891D74579B54Ab63ED24C4ef9cd5Ef;
    address public immutable override want;

    // Third party contracts
    address public constant MASTERCHEF = 0x07b48c393CabABf6458E72d57b7CcD8f73E94c03;
    uint256 public immutable pid;

    // Routes
    address[] public couponToWbnbRoute;
    ...
    constructor(
        ...
        ByalanIsland(
            _hydra,
            0x0F4610aB02920a99f639F675085A5d3e24b0D7ae,
            _izlude,
            _kswFeeRecipient,
        ...
        )
    {
        ...
        want = ICafeteria(MASTERCHEF).poolInfo(_pid).lpToken;

        couponToWbnbRoute = [COUPON, WBNB];
    ...
    function deposit() public override whenNotPaused {
        ...
        if (wantBal > 0) {
            ICafeteria(MASTERCHEF).deposit(pid, wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external override onlyIzlude {
        ...
        if (wantBal < _amount) {
            ICafeteria(MASTERCHEF).withdraw(pid, _amount - wantBal);
    ...
    function harvest() external override whenNotPaused onlyEOA onlyHarvester gasThrottle {
        ICafeteria(MASTERCHEF).deposit(pid, 0);
        chargeFees();
        deposit();
    ...
    function chargeFees() internal nonReentrant {
        uint256 toBnb = (IERC20(COUPON).balanceOf(address(this)) * totalFee) / MAX_FEE;
    ...
    function balanceOfPool() public view override returns (uint256) {
        return ICafeteria(MASTERCHEF).userInfo(pid, address(this)).amount;
    }
        ...
        returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts)
    {
        ...
        rewardTokens[0] = IERC20(COUPON);
        rewardAmounts[0] =
            ICafeteria(MASTERCHEF).pendingCoupon(pid, address(this)) +
            IERC20(COUPON).balanceOf(address(this));
    }
    ...
    function retireStrategy() external override onlyIzlude {
        ICafeteria(MASTERCHEF).emergencyWithdraw(pid);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(COUPON).safeApprove(unirouter, type(uint256).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(MASTERCHEF, 0);
        IERC20(COUPON).safeApprove(unirouter, 0);
    }
