...
$MAR Token from MAR Team

$MAR is a Secure Moonrat Token fork with bug fixes and additional anti-bot / anti-whale features added

Earn claimable BNB by holding!  

Hold for Claimable BNB and Reflect
...
BNB is claimable daily for the first two weeks, then every 3 days afterwards.

Anti-Bot / Anti-Whale / Safety Measures

* Max Transaction at launch: 1 Billion Tokens (0.05% of total supply, .1% of circulating supply) (Will be increased to 5 Billion (0.5%) 30 minutes after launch)
* .05% of circulating supply available for purchase per txn at launch (limit will be increased to .5% of circulating supply after 30 minutes)
* 30 second delay between buys for the first 30 minutes of launch (limit will be completely lifted and cannot be reintroduced shortly after launch)
...
* Launch Limits will be lifted and contract will be timelocked for 5 days immediately following launch, all fees are unchangeable once the contract is activated to increase overall contract safety.
* LP Tokens generated via Locking Liquidity will be sent to address(0) while in timelock.  I will only remove the timelock to make changes very rarely, and only when adding new features.  

Forked and enhanced by Sir Tristan of MAR Team
...
contract MAR is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) private _isExcludedFromClaimTopUp;
    ...
    bool public launchLimitsEnabled = false; // this will set to true for launch, and false upon removal of Launch Limits
    bool public contractActivated = false; // this tracks whether the contract has been activated or not yet

    address[] private _excluded;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 2 * 10 ** 12 * 10 ** 18; // 2 Trillion tokens (12 0's, 18 decimal places)
    ...
    uint256 public _liquidityFee = 8; // 4% will be added pool, 4% will be converted to BNB for claim
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    // BNB Claim variables and mappings
    ...
    uint256 private minTokenNumberToSell = _tTotal.mul(5).div(10000).div(10); // 0.005% min liquidity to sell to trigger liquidity locking

    address public battlefieldAddress;
    address private marketingAddress;

    string private _name = "MAR Token";
    string private _symbol = "MAR";
    ...
    constructor (
        ...
        _rOwned[address(0x000000000000000000000000000000000000dEaD)] = _rTotal.div(2);
        ...
        battlefieldAddress = address(0x8c0A7C89C46d1793b70BC197B69324408c78eBA8);
        marketingAddress = address(0x97e09Ed54d038295a8D6e6e77524328d7d9D3fca);

        // exclude features from triggering TopUp for holders.
        _isExcludedFromClaimTopUp[owner()] = true;
        _isExcludedFromClaimTopUp[address(this)] = true;
        _isExcludedFromClaimTopUp[address(battlefieldAddress)] = true;
        _isExcludedFromClaimTopUp[address(marketingAddress)] = true;

        // exclude owner and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[address(battlefieldAddress)] = true;
        _isExcludedFromFee[address(marketingAddress)] = true;

        // exclude from max tx
        _isExcludedFromMaxTx[owner()] = true;
        _isExcludedFromMaxTx[address(battlefieldAddress)] = true;
    ...
    // This function sets the duration between transactions for launch only
    ...
    function excludeFromClaimTopUp(address account) public onlyOwner {
        _isExcludedFromClaimTopUp[account] = true;
    }

    function includeInClaimTopUp(address account) public onlyOwner {
        _isExcludedFromClaimTopUp[account] = false;
    }
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(contractActivated == false, "setTaxFeePercent:: May not change contract once it is activated to protect users.");
        _taxFee = taxFee;
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(contractActivated == false, "setLiquidityFeePercent:: May not change contract once it is activated to protect users.");
        _liquidityFee = liquidityFee;
    }

    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
        require(contractActivated == false, "setSwapAndLiquifyEnabled:: May not change contract once it is activated to protect users.");
    ...
    function _transfer(
        ...
        // at launch if the transfer delay is enabled, ensure the block timestamps for purchasers is set -- during launch, restriction does not apply to owner, pancakeRouter, or pancakePair
        if (transferDelayEnabled){
            if (to != owner() && to != address(pancakeRouter) && to != address(pancakePair)){
                require(_holderLastTransferTimestamp[to].add(transferDelay) <= block.timestamp, "_transfer:: Transfer Delay enabled.  Please try again in 30 seconds.");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        // top up claim cycle 
        // Exclude specific features from triggering claim time top up (such as Battlefield or games)

        if(!_isExcludedFromClaimTopUp[sender] && !_isExcludedFromClaimTopUp[recipient]){
    ...
    function claimBNBReward() isHuman nonReentrant public {
        require(nextAvailableClaimDate[msg.sender] <= block.timestamp, 'claimBNBReward:: Error: next available claim time not reached');
        require(balanceOf(msg.sender) >= 0, 'claimBNBReward:: Error: must own MAR to claim reward');

        uint256 reward = calculateBNBReward(msg.sender);

        // reward threshold - if over threshold, 20% of the reward is used to buyback and burn tokens.
        ...
        emit ClaimedBNB(msg.sender, reward, nextAvailableClaimDate[msg.sender]);
    ...
    // contract Activation.  Can only call once ever.
    function activateContract() external onlyOwner {
        require(contractActivated == false, "activateContract:: Cannot reactivate after activating");
        // reward claim
        disableEasyRewardFrom = block.timestamp + 2 weeks;
        ...
        setTransferDelayTime(30 seconds);
        setMaxTxPercent(5); // .05% of total, .1% of active supply
        setSwapAndLiquifyEnabled(true);

        // approve contract
        _approve(address(this), address(pancakeRouter), 2 ** 256 - 1);

        // lock contract (no changes until removeLaunchLimits is called)
        lock(1);
        contractActivated = true; // disable double activation (can't mess with contract settings after activation)
        launchLimitsEnabled = true; // one-time set launch limits.
    }

    // contract launch limit removal.  Can only call once ever.
    function removeLaunchLimits() external {
        require(launchLimitsEnabled == true, "removeLaunchLimits:: Cannot remove launch limits as they are already removed!");
        unlock(); // unlock to change ownership back to prev owner temporarily to make changes only (this will fail if someone other than previous owner calls it)
        transferDelayEnabled = false;
        setMaxTxPercent(25); // .25% of total, .5% of active supply
        lock(432000); // relock for 5 days (ownership is required from time to time to make small tweaks to the contract, such as excluding contracts from the TopUp or max Txn functions)
        launchLimitsEnabled = false;
    }
