...
contract SushiFloki is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0x51a709b13AFD9Ee04221962BaB5971cb63205A32);
        
    address payable public liquidityAddress =
        payable(0x3ede6d3ff297db595cb7df7eE5C4DbE2a8496f93);
    ...
    uint256 private constant _tTotal = 5 * 1e11 * 1e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "SushiFloki";
    string private constant _symbol = "SFL";
    ...
    uint256 public _TaxFees = 2;
    uint256 public _LiquidityFees = 4;
    uint256 private _MarketingFees = 5;
    uint256 public _BurnFees = 0;
    ...
    uint256 private maximumTokensSwap = _tTotal * 5 / 100;
    ...
    event SetFees(uint256 marketingFee, uint256 liquidityFee, uint256 reflectFee, uint256 burnFee);
    ...
    constructor() {
        address newOwner = address(0x51a709b13AFD9Ee04221962BaB5971cb63205A32);
        ...
        maximumTokensSwap = _tTotal * 5 / 100; 
        maxWallet = _tTotal * 5 / 100; 
    ...
    function maximumTokensSwapAmount() external view returns (uint256) {
        return maximumTokensSwap;
    }
    ...
    function _transfer(
        ...
        bool overMinimumTokenBalance = contractTokenBalance >= maximumTokensSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            // Sell
                ...
                removeAllFee();
                _taxFee = _TaxFees;
                _liquidityFee = _LiquidityFees + _MarketingFees;
                _burnFee = _BurnFees;
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY && _liquidityFee > 0){
            ...
            _liquidityTokensToSwap += tLiquidity * _LiquidityFees / _liquidityFee;
            _marketingTokensToSwap += tLiquidity * _MarketingFees / _liquidityFee;
        }
        uint256 currentRate = _getRate();
    ...
    function setFees(uint256 TaxFees, uint256 LiquidityFees, uint256 MarketingFees, uint256 BurnFees)
        external
        onlyOwner
    {
        _TaxFees = TaxFees;
        _LiquidityFees = LiquidityFees;
        _MarketingFees = MarketingFees;
        _BurnFees = BurnFees;
        require(_TaxFees + _LiquidityFees + _BurnFees <= 20, "Must keep taxes below 20%");
        emit SetFees(MarketingFees, LiquidityFees, TaxFees, BurnFees);
    }
