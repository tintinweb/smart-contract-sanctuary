...
contract PeanutRouter is IPeanutRouter, Ownable {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'PeanutRouter: EXPIRED');
        _;
    }
    ...
    function pairFor(address tokenA, address tokenB) public view returns (address pair){
        pair = IPeanutFactory(factory).pairFor(tokenA, tokenB);
    }
    ...
    function _addLiquidity(
        ...
        if (IPeanutFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IPeanutFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = IPeanutFactory(factory).getReserves(tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = IPeanutFactory(factory).quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'PeanutRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = IPeanutFactory(factory).quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'PeanutRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        liquidity = IPeanutPair(pair).mint(to);
    }
    ...
    function removeLiquidity(
        ...
        address pair = pairFor(tokenA, tokenB);
        IPeanutPair(pair).transferFrom(msg.sender, pair, liquidity);
        // send liquidity to pair
        (uint amount0, uint amount1) = IPeanutPair(pair).burn(to);
        (address token0,) = IPeanutFactory(factory).sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'PeanutRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'PeanutRouter: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function removeLiquidityWithPermit(
        ...
        IPeanutPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = IPeanutFactory(factory).sortTokens(input, output);
            ...
            IPeanutPair(pairFor(input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = IPeanutFactory(factory).getAmountsOut(amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PeanutRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        amounts = IPeanutFactory(factory).getAmountsIn(amountOut, path);
        require(amounts[0] <= amountInMax, 'PeanutRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, pairFor(path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    returns (uint[] memory amounts)
    {
        require(path[0] == WBNB, 'PeanutRouter: INVALID_PATH');
        amounts = IPeanutFactory(factory).getAmountsOut(msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PeanutRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WBNB, 'PeanutRouter: INVALID_PATH');
        amounts = IPeanutFactory(factory).getAmountsIn(amountOut, path);
        require(amounts[0] <= amountInMax, 'PeanutRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, pairFor(path[0], path[1]), amounts[0]
    ...
    returns (uint[] memory amounts)
    {
        require(path[0] == WBNB, 'PeanutRouter: INVALID_PATH');
        amounts = IPeanutFactory(factory).getAmountsIn(amountOut, path);
        require(amounts[0] <= msg.value, 'PeanutRouter: EXCESSIVE_INPUT_AMOUNT');
        IWBNB(WBNB).deposit{value : amounts[0]}();
        assert(IWBNB(WBNB).transfer(pairFor(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = IPeanutFactory(factory).sortTokens(input, output);
            IPeanutPair pair = IPeanutPair(pairFor(input, output));
            uint amountInput;
            uint amountOutput;
            {// scope to avoid stack too deep errors
                ...
                amountOutput = IPeanutFactory(factory).getAmountOut(amountInput, reserveInput, reserveOutput, input, output);
            }
            if (swapMining != address(0)) {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'PeanutRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'PeanutRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function quote(uint256 amountA, uint256 reserveA, uint256 reserveB) public view override returns (uint256 amountB) {
        return IPeanutFactory(factory).quote(amountA, reserveA, reserveB);
    }
    ...
    function getAmountOut(uint256 amountIn, uint256 reserveIn, uint256 reserveOut, address token0, address token1) public view override returns (uint256 amountOut){
        return IPeanutFactory(factory).getAmountOut(amountIn, reserveIn, reserveOut, token0, token1);
    }
