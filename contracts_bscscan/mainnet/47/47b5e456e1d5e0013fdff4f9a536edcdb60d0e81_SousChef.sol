...
contract SousChef {
    ...
    // The ZINACT TOKEN!
    IBEP20 public zinact;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes Zinact tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = zinact.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 zinactSupply = zinact.balanceOf(address(this));
        if (zinactSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(zinactSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit Zinact tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];

        updatePool();
        zinact.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Zinact tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        zinact.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        zinact.safeTransfer(address(msg.sender), user.amount);
