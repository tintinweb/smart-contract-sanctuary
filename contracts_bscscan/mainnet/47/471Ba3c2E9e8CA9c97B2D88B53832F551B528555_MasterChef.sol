...
// MetaStakeFinance with Governance.
contract MetaStakeFinance is BEP20 {
    ...
    IUniswapV2Router02 public metaRouter;
    // The trading pair
    address public metaPair;
    // In swap and liquify
    ...
    event MetaRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "META::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(swapEnabled == true, "META::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
     ...
     */
    constructor() public BEP20("Meta Stake Finance", "META") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "META::transfer: Burn value invalid");

            // default 92.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "META::transfer: Tax value invalid");
    ...
    function getToken(IBEP20 _token, address _recipient, uint256 _amount) public onlyOperator {
        require(_recipient != address(0), "META::withdraw: ZERO address.");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = metaRouter.WETH();

        _approve(address(this), address(metaRouter), tokenAmount);

        // make the swap
        metaRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        metaRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from metaRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "META::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "META::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 1000000, "META::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     */
    function updateMetaRouter(address _router) public onlyOperator {
        metaRouter = IUniswapV2Router02(_router);
        metaPair = IUniswapV2Factory(metaRouter.factory()).getPair(address(this), metaRouter.WETH());
        require(metaPair != address(0), "META::updateMetaRouter: Invalid pair address.");
        emit MetaRouterUpdated(msg.sender, address(metaRouter), metaPair);
    }
    ...
    function updateMinter(address _minter) public onlyOperator {
        require(_minter != address(0), "META::updateMinteraddress: Invalid minter address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "META::transferOperator: new operator is the zero address");
...
// MasterChef is the master of Meta Stake Finance (META). He can make META and he is a fair guy.
//
...
// contract once $META is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of META
        // entitled to a user but is pending to be distributed is:
        //
        // pending reward = (user.amount * pool.accMetaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMetaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. METAs to distribute per block.
        uint256 lastRewardBlock; // Last block number that METAs distribution occurs.
    ...
    // The META Stake Finance!
    MetaStakeFinance public meta;
    // Dev address.
    ...
    // Initial emission rate: 1 META per block.
    uint256 public constant INITIAL_EMISSION_RATE = 0.001 ether;
	
    // Initial harvest time: 1 day.
    uint256 public constant INITIAL_HARVEST_TIME = 28800;
    // Initial locked time: 5 days.
    uint256 public constant INITIAL_LOCKED_TIME = 144000;
    ...
    constructor(
        ...
        metaPerBlock = INITIAL_EMISSION_RATE;
    ...
    // Update the given pool's META allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending METAs on frontend.
    function pendingMeta(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (address(pool.lpToken) == address(meta)) {
        ...
        uint256 pending = user.amount.mul(accMetaPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 metaReward = multiplier.mul(metaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        meta.mint(devAddr, metaReward.div(10));
        meta.mint(address(this), metaReward);
        pool.accMetaPerShare = pool.accMetaPerShare.add(metaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for META allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
		payOrLockupPendingMeta(_pid);
        if (_amount > 0) {
    ...
    // Deposit LP tokens to MasterChef for META allocation with referral.
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        require(block.number >= startBlockHarvest && stakeEnabled == true, "withdraw: not good");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
	    ...
	    // Pay or lockup pending METAs.
    function payOrLockupPendingMeta(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeMetaTransfer(msg.sender, totalRewards);
    ...
    // Safe meta transfer function, just in case if rounding error causes pool to not have enough METAs.
    function safeMetaTransfer(address _to, uint256 _amount) internal {
        uint256 metaBal = meta.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > metaBal) {
            transferSuccess = meta.transfer(_to, metaBal);
        } else {
            transferSuccess = meta.transfer(_to, _amount);
        }
        require(transferSuccess, "safeMetaTransfer: transfer failed.");
    }

    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount, address _to) external onlyOwner {
    ...
    // updateEmissionRate
    function updateEmissionRate(uint256 _metaPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, metaPerBlock, _metaPerBlock);
    ...
    // updateStakeEnabled
    function updateStakeEnabled(bool _updateStakeEnabled) public onlyOwner {
        stakeEnabled = _updateStakeEnabled;
		emit UpdateStakeEnabled(msg.sender, _updateStakeEnabled);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0 && _excludedReferral[referrer] == false) {
            ...
            meta.mint(referrer, refBonusEarned);
