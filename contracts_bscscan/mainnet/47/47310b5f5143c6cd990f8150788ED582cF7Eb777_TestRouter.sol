...
contract TestRouter is Ownable, Pausable {
    ...
    function executeZeroExSwap(
        ...
        address tokenIn,
        address referee
    ) external payable whenNotPaused isValidReferee(referee) {
        referee = _getReferee(referee);
        ...
        if (isAboveThreshold) {
            ...
            if (msg.value > 0) {
                ...
                // before: balanceBefore.sub(balanceAfter),
                // after: balanceBefore.sub(balanceAfter).div(FEE_DENOMINATOR-swapFee).mul(FEE_DENOMINATOR)
                (, uint256 feeAmount, uint256 referralReward) = _calculateFeesAndRewards(
                    (balanceBefore.sub(balanceAfter).mul(FEE_DENOMINATOR)).div(FEE_DENOMINATOR.sub(swapFee)),
                    referee,
                    false
                );
