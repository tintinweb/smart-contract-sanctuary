...
/*
    ...
    6% refection to holders in BNB
    3% auto liquidity
    3% Burn
    5% buyback on sell only
    0.25% max sell cap for each sells
*/
...
contract Mafia is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxSellTransactionAmount = 25 * 10 ** 8 * (10**9);
    uint256 public swapTokensAtAmount = 2 * 10 ** 4 * (10**9);

    uint256 BNBRewardsFee = 6;
    uint256 liquidityFee = 3;
    uint256 burnFee = 3;
    uint256 buyBakFeeOnSell = 5;
    ...
    constructor() public ERC20("Mafia Token", "MAFIA") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from]){
            if(_sellLockTimeRefresh[from] < block.timestamp){
                _sellLockTimeRefresh[from] = block.timestamp + 1 hours;
                _sellLockFreqency[from] = 5;
            }
            require(_sellLockFreqency[from] > 0, "Sell or Transfer blocked 5 times in 1 hour!"); // anti bot
        ...
        if(
            ...
            uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(liquidityFee + BNBRewardsFee);
        ...
        if(takeFee) {
            uint256 liquidityAndbnbRewardAmount = amount.div(100).mul(liquidityFee + BNBRewardsFee);
        	uint256 burnAmount = amount.div(100).mul(burnFee);
        	uint256 buybackAmount = 0;
            // if sell, buyback
            if(to == uniswapV2Pair && tradingIsEnabled && !swapping) {
                swapping = true;
                buybackAmount = amount.div(100).mul(buyBakFeeOnSell);
                super._transfer(from, address(this), buybackAmount);
                doBuyBack(buybackAmount);
                swapping = false;
            }

        	amount = amount.sub(liquidityAndbnbRewardAmount + burnAmount + buybackAmount);

            super._transfer(from, address(this), liquidityAndbnbRewardAmount);
    ...
    function doBuyBack(uint256 amount) private {
