...
/*
    ApeUSDT
    Automatically receive USDT for aping in and HODLing!
...
contract ApeUSDT is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "ApeUSDT"; 
    string constant _symbol = "ApeUSDT";
    ...
    uint256 public swapThreshold = _totalSupply * 10 / 4000; // will start swapping once 0.025% of supply is in the swap wallet
    bool inSwap;
    ...
    function swapBack() internal swapping {
        ...
        InReward += amountBNB.sub(amountBNBReflection).sub(amountBNBMarketing).sub(amountBNBLiquidity);
...
//LP lock
    ...
    function LPLockTime() public view returns (uint256){
    ...
    function LockLiquidity(uint256 lockTimeInSeconds) public access{
        _LockLiquidity(lockTimeInSeconds+block.timestamp);
    }
    function _LockLiquidity(uint256 newUnlockTime) private{
    ...
    function setTxLimit(uint256 amount) external access {
        //to prevent honeypotting - TxLimit cannot be set below 0.5% of total supply
        require(((amount>=(_totalSupply * 1 / 200))), "NewTxLimit must be greater than 0.5% of total supply");
        _maxTxAmount = amount;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external access {
        ...
        //totalFee cannot be greater than 25%
