...
contract Deposite is ReentrancyGuard, Context, Ownable {
    ...
    constructor() {
        _token = IERC20(0x95f1d9e4Fc0576b0fF99b8811c2874e40aF43afA);
    }

    //Pre-Sale
    function deposit(uint256 amount) public payable nonReentrant {
        uint256 depositAmt = amount * 10**9;
        _token.transferFrom(msg.sender, address(this), depositAmt);
    }

    function cashOut(address beneficiary, uint256 amount)
        public
        payable
        nonReentrant
        onlyOwner
    {
        require(msg.sender == owner(), "Only owner is authorised to use this.");
    ...
    function _processPurchase(address beneficiary, uint256 tokenAmount)
        internal
    {
        uint256 amt = tokenAmount * 10**9;
        _token.transfer(beneficiary, amt);
    }
