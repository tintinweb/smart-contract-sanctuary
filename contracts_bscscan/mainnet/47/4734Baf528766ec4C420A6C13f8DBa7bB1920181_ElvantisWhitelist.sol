...
contract ElvantisWhitelist is Ownable, AccessControl {
    ...
    constructor(
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("ELVANTIS_WHITELIST")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
    ...
    function contribute(
        ...
        require(
            contributionEnabled,
            "ElvantisWhitelist: Contribution is disabled"
        );
        require(roundLimits[currentRound] >= totalContributions[currentRound] + msg.value, "ElvantisWhitelist: Hardcap for current round is filled");
        require(deadline >= block.timestamp, "ElvantisWhitelist: Expired");
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(
                    abi.encode(CONTRIBUTION_TYPEHASH, msg.sender, deadline)
                )
            )
        );
        address signer = ecrecover(digest, v, r, s);
        require(
            hasRole(WHITELIST_SIGNER_ROLE, signer),
            "ElvantisWhitelist: Invalid Signature"
        );
        userContribution[msg.sender] += msg.value;
        totalContributions[currentRound] += msg.value;

        require(
            userContribution[msg.sender] >= minContribution,
            "ElvantisWhitelist: Minimun Contribution not met"
        );
        require(
            userContribution[msg.sender] <= maxContribution,
            "ElvantisWhitelist: Maximum Contribution exceeded"
        );
    ...
    function claim() external {
        require(claimEnabled, "ElvantisWhitelist: Claim disabled");
        uint256 _contributions = userContribution[msg.sender] - userClaim[msg.sender];
        require(_contributions > 0,"ElvantisWhitelist: Only contributors are allowed to claim");

        uint256 _tokenAmount = _contributions * tokenRate;
        elvantis.transfer(msg.sender, _tokenAmount);
    ...
    function setSignerRoleAdmin(address admin) external onlyOwner {
        require(admin != address(0), "ElvantisWhitelist: Invalid admin");
        _setupRole(DEFAULT_ADMIN_ROLE, admin);
    }

    function grandRewardSignerRole(address account) external onlyOwner {
        require(account != address(0), "ElvantisWhitelist: Invalid account");
    ...
    function setContributionEnabled(bool _contributionEnabled)
        external
        onlyOwner
    {
    ...
    function moveToNextRound() external onlyOwner {
        require(totalRounds > currentRound, "ElvantisWhitelist: Rounds are finished");
        currentRound++;
