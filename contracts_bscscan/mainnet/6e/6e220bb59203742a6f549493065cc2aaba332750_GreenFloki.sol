...
contract GreenFloki is IERC20, Ownable {
    ...
    string _name = "Green Floki";
    string _symbol = "GREENFLOKI";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);    // 100,000,000,000
    uint256 public _tsCap = (_totalSupply * 10) / 1000;        // 1% of total supply
    ...
    uint256 swapThreshold = _totalSupply.mul(496847210).div(100000000000);
    ...
    constructor () {
        ...
        isTxLimitExempt[pair] = true; // template
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _tsCap || isTxLimitExempt[sender], "TX Limit Exceeded");
    }
    ...
    function changeTsnCap(uint256 percent, uint256 accuracy) external onlyOwner { 
        require(percent >= 1 && accuracy >= 100, "Max transaction must be greater than 1%");
        _tsCap = _totalSupply.mul(percent).div(accuracy);
    }
    
    function changeMaxWallet(uint256 percent, uint256 accuracy) external onlyOwner {
        require(percent >= 1 && accuracy >= 100, "Max wallet must be greater than 1%");
        _maxWalletSize = _totalSupply.mul(percent).div(accuracy);
    }
