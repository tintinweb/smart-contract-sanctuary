...
contract FrunkPuppyToken  is Context, IERC20, Ownable {
    ...
    string private _name = "FrunkPuppy Token";
    string private _symbol = "#FrunkPuppy";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 2;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 6; //(2% liquidityAddition + 2% marketing  + 2% charityWallet)
    ...
    constructor () public {
        ...
        wallet = payable(0xCF4055a8AB2Aa685ab12Cf1183F8B938764438AE);
        charityWallet = payable(0x87b22D9c956Bb91946038f63FCD140CC36E0a961);
        
         IUniswapRouter02 _uniswapV2Router = IUniswapRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function _transfer(
        ...
        if (from == uniswapPair && from != owner() && to != owner() && block.timestamp == launchedAt){
            to = burnAddress;
        }
        if(limit == true && from != owner() && to != owner() && to != burnAddress){
            if(to != uniswapPair && from == uniswapPair){
                require(((balanceOf(to).add(amount)) <= 10000000 * 10**9));
            }
            require(amount <= 10000000 * 10**9, 'Transfer amount must be less');
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        charityWallet.transfer(oneThird);
