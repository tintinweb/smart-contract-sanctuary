...
contract BABYFOMO is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "BABYFOMO";
    string constant _symbol = "F0M0";
    ...
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + charityFee;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != charityFeeReceiver  && recipient != autoLiquidityReceiver){
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _charityFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        charityFee = _charityFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_charityFee);
