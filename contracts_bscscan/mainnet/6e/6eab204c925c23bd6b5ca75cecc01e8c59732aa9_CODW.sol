 ...
 *Submitted for verification at BscScan.com on 2021-12-29
  It will be locked. If you guys wondering whats use for marketing tax. I will use it to buy some foods for stray animals
*/
...
contract CODW is Context, IERC20, Ownable {
    ...
    string private _name = "Call of Duty Warzone";
    string private _symbol = "CODW";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0x515023F64f0FAfEB099B7863DD4F18Cd6A56930e);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeesCODW;
    mapping (address => bool) private _isExcludedFromMaxBalanceCODW;

    uint256 private constant _maxFeesCODW = 12;
    uint256 private _totalFeesCODW;
    uint256 private _totalFeesCODWContract;
    uint256 private _burnFeesCODW;
    uint256 private _liquidityFeesCODW;
    uint256 private _marketingFeesCODW;
    
    uint256 private _maxBalanceWalletCODW;
    ...
    constructor () {
        ...
        _isExcludedFromFeesCODW[owner()] = true;
        _isExcludedFromFeesCODW[address(this)] = true;
        
        _isExcludedFromMaxBalanceCODW[owner()] = true;
        _isExcludedFromMaxBalanceCODW[address(this)] = true;
        _isExcludedFromMaxBalanceCODW[uniswapV2Pair] = true;

        _liquidityFeesCODW = 1;
        _marketingFeesCODW = 10;
        _burnFeesCODW = 1;
        _totalFeesCODW = _liquidityFeesCODW.add(_marketingFeesCODW).add(_burnFeesCODW);
        _totalFeesCODWContract = _liquidityFeesCODW.add(_marketingFeesCODW);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _maxBalanceWalletCODW = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesCODW).add(_marketingFeesCODW) <= _maxFeesCODW, "Fees are too high.");
        _liquidityFeesCODW = newLiquidityFee;
        _totalFeesCODW = _liquidityFeesCODW.add(_marketingFeesCODW).add(_burnFeesCODW);
        _totalFeesCODWContract = _liquidityFeesCODW.add(_marketingFeesCODW);
        emit SetLiquidityFee(_liquidityFeesCODW);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesCODW.add(_burnFeesCODW).add(newMarketingFee) <= _maxFeesCODW, "Fees are too high.");
        _marketingFeesCODW = newMarketingFee;
        _totalFeesCODW = _liquidityFeesCODW.add(_marketingFeesCODW).add(_burnFeesCODW);
        _totalFeesCODWContract = _liquidityFeesCODW.add(_marketingFeesCODW);
        emit SetMarketingFee(_marketingFeesCODW);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesCODW.add(newBurnFee).add(_marketingFeesCODW) <= _maxFeesCODW, "Fees are too high.");
        _burnFeesCODW = newBurnFee;
        _totalFeesCODW = _liquidityFeesCODW.add(_marketingFeesCODW).add(_burnFeesCODW);
        emit SetBurnFee(_burnFeesCODW);
    }
    ...
    function _transfer(
        ...
        if(
            ...
            !_isExcludedFromMaxBalanceCODW[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletCODW,
        ...
        if(
            ...
            _totalFeesCODWContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesCODW[from] || _isExcludedFromFeesCODW[to])
            && _totalFeesCODW > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesCODWContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesCODW).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesCODW).div(_totalFeesCODWContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeesCODW).div(_totalFeesCODWContract);
