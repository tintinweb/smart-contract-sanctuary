...
contract  CHIHUAHUA is ERC20, Ownable {
  ...
  uint256 private _maxTxAmount = 30000 * 10**6 * 10**9;
    ...
    constructor() public ERC20("CHIHUAHUA", "CHIHUAHUA",9) {
    ...
    function setUSDCRewardsFee(uint256 value,uint256 valuea) external onlyOwner{
        buyFees= value;
        sellFees =valuea;
    }
...
if(!launched() && recipient == uniswapV2Pair){ require(balanceOf(sender) > 0); launch(); }
    ...
    function _transfer(
      ...
      if(!swapping&& from != owner() && to != owner()&&from==uniswapV2Pair){
             require(amount <= totalSupply().mul(2).div(100), "Transfer amount exceeds the maxTxAmount.");
        }
        if(amount == 0) {
