...
/* 
    $SweetyCupcakes
    earn $CAKE with 10% dividend for every transaction!
    
    https://t.me/sweetycupcakesBSC
...
contract SweetyCupcakes is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SweetyCupcakesDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("SweetyCupcakes", "SCAKES") {
        //Reflection Rewards %
        _ETHRewardsFee = 10;
        //Liqudity Fee %
        _liquidityFee = 3;
    	...
    	dividendTracker = new SweetyCupcakesDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
        //Marketing Wallet Address
		marketingWallet = payable(0x0d4b515e7497c25A92583D35e0bADae2f7c61FAe);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SweetyCupcakes: The dividend tracker already has that address");

        SweetyCupcakesDividendTracker newDividendTracker = SweetyCupcakesDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SweetyCupcakes: The new dividend tracker must be owned by the SweetyCupcakes token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SweetyCupcakes: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SweetyCupcakes: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SweetyCupcakes: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SweetyCupcakes: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SweetyCupcakes: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SweetyCupcakes: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SweetyCupcakes: Cannot update gasForProcessing to same value");
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "SweetyCupcakes: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "SweetyCupcakes: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "SweetyCupcakes: ETH Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract SweetyCupcakesDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SweetyCupcakes_Dividend_Tracker", "SweetyCupcakes_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SweetyCupcakes_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SweetyCupcakes_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SweetyCupcakes contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "SweetyCupcakes_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SweetyCupcakes_Dividend_Tracker: Cannot update claimWait to same value");
