...
contract KEBABtoken is IERC20, IERC20Metadata, Ownable {
    ...
    mapping(address => bool) public _isServiceWallet; //mapping of wallets that belong to exchanges

    mapping(address => bool) public _isExcluded; // mapping of wallets that are not subjected to limitations and fees

    mapping(address => uint256[]) public _dailySales; // mapping of daily sales per wallet

    uint256 public _startBlock; //number of block when presale was started
    ...
    uint256 private MAX_SALE_AMOUNT = 10000000 * 10 ** _decimals;

    uint256 private MAX_START_BUY = 100000000000000 * 10 ** _decimals;

    uint256[4] public _saleFees = [10,20,25,30]; // fee percentages for each sale within day

    uint256 public _userFee = 99; // wallet to wallet anti-whale
    ...
    constructor(address cOwner) Ownable(cOwner) {
        ...
        //testnet
        _isServiceWallet[0xD99D1c33F9fC3444f8101754aBC46c52416550D1] = true;

        //mainnet
        //_isServiceWallet[0x10ED43C718714eb63d5aA57B78B54704E256024E] = true;
        emit Transfer(address(0), cOwner, _totalSupply);

    }

    /**
     * Starts pesale, allowing users to buy/sell tokens. Can be executed only once
     *
     */
    ...
    /**
     * @dev Adds wallet address to list of exchange wallets

     * Requirements:
     *
     * account must not be set as an exchange wallet before
     */
    ...
    /**
     * @dev Removes wallet address from list of exchange wallets

     * Requirements:
     *
     * account must be set as an exchange wallet before
     */
    ...
    /**
     * @dev Enables limitations and fees for wallet

     * Requirements:
     *
     * limitations for account must be disabled before
     */
    ...
    /**
     * @dev Disables limitations and fees for wallet

     * Requirements:
     *
     * limitations for account must be enabled before
     */
    ...
    /**
     * @dev Checks if wallet belongs to exchange wallets or has limitations and fees disabled
     */
    ...
    function _transfer(
        ...
        if (isUser(sender) && isUser(recipient)) {
            require(_startBlock > 0, "Presale not started.");
            ...
            if (_dailySales[sender].length == 4) {
                require(_dailySales[sender][0] < block.timestamp-86400, "Circle");
                delete _dailySales[sender];
                _dailySales[sender][0] = block.timestamp;
            } else if (_dailySales[sender].length > 0) {
                require(_dailySales[sender][_dailySales[sender].length-1] < block.timestamp-120, "2 minutes delay between sales required");
            ...
            require(_startBlock > 0, "Presale not started.");
            if (_startBlock > 0 && block.number - _startBlock <= 10 ) {
