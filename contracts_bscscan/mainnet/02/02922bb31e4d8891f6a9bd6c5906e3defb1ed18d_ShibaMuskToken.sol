    ...
    ITransferLockToken
{
    ...
    function initialize(uint256 _unlockTime, uint256 _unlockPerSecond)
        public
        initializer
    {
        require(_unlockTime > 0, "invalid unlock time value");
        require(_unlockPerSecond > 0, "invalid unlockPerSecond value");

        unlockTime = _unlockTime;
        unlockPerSecond = _unlockPerSecond;

        __ERC20PresetMinterPauser_init("ShibaMusk", "ShibaMusk");
    }
    ...
    function unlockToken() external {
        ...
        if (unlockAmount > 0) {
            users[sender].amountLocked = users[sender].amountLocked.sub(
                unlockAmount
            );
    ...
    function getAvailableBalance(address account)
        ...
        returns (uint256)
    {
        uint256 availableAmount = this.balanceOf(account).sub(
            users[account].amountLocked
        );
    ...
    function setUnlockPerSecond(uint256 _unlockPerSecond) external {
        require(
    ...
    function transferLockToken(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        users[recipient].amountLocked = users[recipient].amountLocked.add(
            amount
        );
    ...
    function _getUnlockAmount(address account, uint256 currentTime)
        ...
        returns (uint256)
    {
        if (unlockTime > currentTime) return 0;
        if (users[account].amountLocked == 0) return 0;
        uint256 claimAt = users[account].claimAt;
        if (claimAt < unlockTime) claimAt = unlockTime;

        uint256 diff = currentTime.sub(claimAt);
        uint256 claimAmount = users[account].amountLocked.div(1e18).mul(
            diff.mul(unlockPerSecond)
        );

        if (claimAmount > users[account].amountLocked)
            claimAmount = users[account].amountLocked;
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        _checkAvailableAmount(_msgSender(), amount);
    ...
    function transferFrom(
        ...
        _checkAvailableAmount(sender, amount);
        return super.transferFrom(sender, recipient, amount);
    }

    function _checkAvailableAmount(address sender, uint256 amount) internal virtual {
        uint256 amountLocked = users[sender].amountLocked;
        uint256 availableAmount = this.balanceOf(sender).sub(amountLocked);
        require(availableAmount >= amount, "Balance is locked");
    }
}
