...
/**
 * OneShot - How long will you hold before selling for profit?
 *
 * Once you sell, you can't buy back in again - EVER
 *
 * OneShot is a unique BSC project which aims to remove the paperhands and reward the diamond hands.
 *
 * 10% BNB Rewards - Automatically sent to your wallet
 * 2% Marketing
 * 2% LP
 * 
 * 14% Buy Tax 
 * 16% Sell Tax 
 * 
 * Website: oneshottoken.com 
 * Telegram: t.me/oneshottoken 
 * Twitter: twitter.com/OneShotToken 
 *
 */
...
contract OneShotBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {

	// MAIN TOKEN PROPERTIES
	string private constant NAME = "OneShot";
	string private constant SYMBOL = "OST";
    ...
    uint8 private _marketingFee; // % of each transaction that will be used for marketing pool
	...
	uint256 private constant _totalTokens = 1000000 * 10**DECIMALS;	//1 trillion total supply
    ...
    mapping (address => bool) private _isBlacklisted; // The list of wallets which are blacklisted after selling.
    mapping (address => bool) private _isAirdropped; // List of wallets which received an airdrop 
	...
	address public marketingWallet = 0x134Ff8A3Fe88B084D9849aD44FDf737cb737B449;
    ...
    // 15 MINUTE SELL RESTRICTION AFTER LAUNCH
    uint256 private _canSellAfter;
	...
	constructor () {
		...
		// Initialize PancakeSwap V2 router and OST <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		setPancakeSwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);

		// 2% liquidity fee, 10% reward fee, 2% marketing fee, 2% additional sell fee 
		setFees(2, 10, 2, 2);
	...
	function activate() public onlyOwner {
		...
		activateBuying();
		onActivated();
		_canSellAfter = block.timestamp + 900; // 15 minutes after contract activation
	}
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "Dumbo: airdropToWallets:: Arrays must be the same length");
        for(uint256 i = 0; i < airdropWallets.length; i++){
            address wallet = airdropWallets[i];
            uint256 airdropAmount = amount[i];
            if(_isAirdropped[wallet] == false && !isNonWallet(wallet)){ // prevent double sending and don't airdrop contracts, only wallets.
                _isAirdropped[wallet] = true;
                doTransfer(msg.sender, wallet, airdropAmount);
            }
        }
    }
	...
	function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        // Let's check if the user is blacklisted, if they are reject their transaction.
        if(!isPancakeSwapPair(sender) && _isBlacklisted[sender]){
            require(!_isBlacklisted[sender], "User is blacklisted from buying");
        }
		
		if(isPancakeSwapPair(recipient)){
		    require(block.timestamp > _canSellAfter, "Selling can't occur until after the 15 minute restriction");
		}

        // Can only sell 100% of their holdings, so let's make sure that's what they are doing. If not, we make the amount 100% of their holding.
        if(amount < _balances[sender] && !_addressesExcludedFromFees[sender]){
            amount = _balances[sender]; // If amount is less than the user balance, change the amount to their total balance.
	...
	// This function swaps a {tokenAmount} of OST tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for OST -> WBNB
		...
		uint256 marketingBalance = (address(this).balance - initialBalance) * 6;
		
		bool success;
		
		(success,) = payable(marketingWallet).call{value: marketingBalance}("");
        require(success, "OST: Unable to send BNB to MarketingWallet");
	...
	// Returns how many more $OST tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 marketingFee, uint8 additionalSellFee) public onlyOwner {
		...
		require(marketingFee <= 5, "Additional sell fee cannot exceed 5%");
		require(liquidityFee + rewardFee + marketingFee <= 25, "Total fees cannot exceed 25%");
		...
		// Enforce invariant
		_poolFee = _rewardFee + _liquidityFee + marketingFee; 
	}
	
	function isNonWallet(address account) internal view returns (bool) {
	...
	// Airdrop supplied wallet addresses and amounts - Mark as an airdropped wallet
	
	
    // Remove sender from blacklist 
    function removeFromBlacklist(address sender) public onlyOwner {
        
        require(_isBlacklisted[sender], "User is not on the blacklist");
        
        _isBlacklisted[sender] = false;   
    }
    
    // Add sender to blacklist 
    function addToBlacklist(address sender) public onlyOwner {
        
        require(!_isBlacklisted[sender], "User is already on the blacklist");
	...
	function setWmarketingWallet(address newMarketingWallet) public onlyOwner {
...
contract OneShot is OneShotBase {
	...
	uint256 private _totalBNBAsOSTClaimed; // The total number of BNB that was converted to OST and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsOSTClaimed; // The amount of BNB converted to OST and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy OST automatically
	...
	bool private _reimburseAfterOSTClaimFailure; // If true, and OST reward claim portion fails, the portion will be given as BNB instead
	...
	function onActivated() internal override {
		...
		setReimburseAfterOSTClaimFailure(true);
		setMinRewardBalance(150 * 10**decimals());  //At least 150 tokens are required to be eligible for rewards
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough OST balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim OST tokens
		if (!claimOST(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterOSTClaimFailure) {
	...
	function claimOST(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsOSTClaimed[user] += bnbAmount;
		_totalBNBAsOSTClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $OST on a zero balance address:
		...
		// Depending on the % of $OST tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsOST(address byAddress) public view returns (uint256) {
		return _bnbAsOSTClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsOST() public view returns (uint256) {
		return _totalBNBAsOSTClaimed;
	}
	...
	function reimburseAfterOSTClaimFailure() public view returns(bool) {
		return _reimburseAfterOSTClaimFailure;
	}


	function setReimburseAfterOSTClaimFailure(bool value) public onlyOwner {
		_reimburseAfterOSTClaimFailure = value;
	}
