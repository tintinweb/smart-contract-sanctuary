...
contract GhostDogeDividendDistributor is IDividendDistributor {
...
contract GhostDoge is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "GhostDoge";
    string constant _symbol = "DG";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10**9 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply / 1; 
    uint256 public _maxSellTxAMount = _totalSupply / 2;
    uint256 public _maxHoldAmount = _totalSupply / 1;
    ...
    */
    ...
    GhostDogeDividendDistributor public distributor;
    ...
    constructor (
        ...
        distributor = new GhostDogeDividendDistributor(address(router), Shiba);
	...
	if(inSwap){ return _basicTransfer(sender, recipient, amount); }
        ...
        if (recipient == pair && !isFeeExempt[sender]) 
        {
            ...
            if(compactLimit>3){
			    ...
			    revert("Transaction cannot compute");
            }
		}
...
function switchRewardToken(address rewardToken) public onlyOwner {
        Shiba = rewardToken;
        distributor = new GhostDogeDividendDistributor(address(router), rewardToken);
    }	
