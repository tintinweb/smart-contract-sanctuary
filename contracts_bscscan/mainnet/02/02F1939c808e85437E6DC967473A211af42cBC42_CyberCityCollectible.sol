...
/**
 * @title CyberCityCollectible
 */
contract CyberCityCollectible is ERC721Enumerable, OperatorAccess {
    // Max token supply
    ...
    constructor(string memory _uri)
        ERC721("Cyber City NFTb Clan", "CYBR")
    {
        _veryBaseURI = _uri;
        emit BaseUriChanged(_uri);
    }

    // @dev Batch token mint.
    // Permission: only operator
    function mintTokens(uint _amount) external onlyOperator {
        require(
...
/**
 * @title OperatorAccess - access with operator rights
 */
