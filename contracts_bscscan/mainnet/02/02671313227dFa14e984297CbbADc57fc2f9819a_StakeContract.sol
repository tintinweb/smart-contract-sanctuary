...
contract StakeContract {
    ...
    constructor (IBEP20 _token){
        ...
        endPool = 1643144400;       // 23:00 Jan 26 utc time.
    ...
    function calcCurrentReward(address _addr) public view returns(uint256) {
        ...
        uint256 curBlock = block.timestamp;
        uint256 interval;
        if (curBlock > (endPool + lockTokenPeriod)) {
            interval = lockTokenPeriod;
        } else {
            interval = curBlock - endPool;
        }

        uint256 reward = user.amount.mul(APY-100).mul(interval).div(100).div(lockTokenPeriod);
        return reward;
    }
