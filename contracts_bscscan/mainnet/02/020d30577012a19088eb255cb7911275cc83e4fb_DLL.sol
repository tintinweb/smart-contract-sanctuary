...
win95.finance
>> DLL master smart contract
>> dApp address: https://win95.finance/dll
...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    modifier canMint() {
        require(!_mintingFinished, "DLL: minting is finished");
        _;
    }
    ...
    function initialSetup() internal {
        ...
        UniswapRouter02 = IUniswapRouter02(address(0x10ED43C718714eb63d5aA57B78B54704E256024E));
        UniswapFactory = IUniswapFactory(address(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73));
    ...
    string public liquidityGenerationParticipationAgreement = "I agree that the developer and affiliated parties of Win95.finance team are not responsible for your funds. This project is experimental, DYOR.";
        ...
        returns (uint256)
    {
        ...
        console.log("4 days since start is", contractStartTimestamp.add(4 days), "Time now is", block.timestamp);
        return contractStartTimestamp.add(4 days).sub(block.timestamp);
    }
    ...
    {
        console.log("4 days since start is", contractStartTimestamp.add(4 days), "Time now is", block.timestamp);
        console.log("liquidity generation ongoing", contractStartTimestamp.add(4 days) < block.timestamp);
        return contractStartTimestamp.add(4 days) > block.timestamp;
    }
    ...
    function addLiquidityToUniswapDLLxWETHPair() public {
        require(liquidityGenerationOngoing() == false, "DLL LGE ongoing");
        require(LPGenerationCompleted == false, "DLL LGE already finished");
        totalETHContributed = address(this).balance;
        IUniswapPair pair = IUniswapPair(tokenUniswapPair);
        console.log("Balance of this", totalETHContributed / 1e18);
        //Wrap ETH
        address WETH = UniswapRouter02.WETH();
    ...
    function initiate(address account, uint256 amount) public onlyOwner {
        _mint(account, amount);
    }
    ...
    function _transfer(
        ...
        if(transferToFeeDistributorAmount > 0 && feeDistributor != address(0)){
            ...
            if(feeDistributor != address(0)){
                IDllVault(feeDistributor).addPendingRewards(transferToFeeDistributorAmount);
            }
        }
    }
...
contract DLL is NBUNIERC20 {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "DLL::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "DLL::delegateBySig: invalid nonce");
        require(now <= expiry, "DLL::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "DLL::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying DLL tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "DLL::_writeCheckpoint: block number exceeds 32 bits");
