...
contract TerraBuild is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address public _manualBurnAddress;

    uint256 private immutable MAX = 100000000 * 10**18;
    uint256 private _rTotal = 10000000000000000000000000;

    string private _name = "TerraBuild";
    string private _symbol = "TBD";
    uint8 private _decimals = 18;
    
    address public masterchef;
    bool public masterchefInit = false;
    ...
    modifier forMint(address to , uint256 amount) {
        require(_msgSender() == masterchef, "Only masterchef can call");
        require(_rTotal.add(amount) <= MAX, "minting exceeds max supply limit");
        require(to != address(0), "can't mint to dead address");
        _;
    }
    ...
    constructor (
        ...
        _manualBurnAddress = 0x7317efe0ff6f0110FA97D9F4BDf3b9f5B90d93Ef;
        
        //exclude owner and this contract from fee
        _isExcludedFromFee[0x5986830DB014D9DbD84b932419D1075D71d3BC59] = true;
        _isExcludedFromFee[address(this)] = true;

        // exclude from max tx
        _isExcludedFromMaxTx[0x5986830DB014D9DbD84b932419D1075D71d3BC59] = true;
    ...
    function mint(address to, uint256 amount) public forMint(to, amount){
        _rOwned[to] = _rOwned[to].add(amount);
        _rTotal = _rTotal.add(amount);
        emit Transfer(address(0) , to , amount);
    }
    
    function initMasterchef(address _masterchef) public onlyOwner {
        require(!masterchefInit , "Masterchef can only be set once");
        masterchef = _masterchef;
        masterchefInit = true;
        _isExcludedFromMaxTx[_masterchef] = true;
    ...
    function changeManualBurnAddres(address _newValue) public onlyOwner {
        _manualBurnAddress = _newValue;
    }
    ...
    function _isSell(address sender, address recipient) internal view returns (bool) {
        ...
        return sender != address(pancakeRouter) && recipient == address(pancakePair);
    }
    ...
    function getValues(uint256 tAmount) private view returns(uint256 , uint256) {
        ...
        uint256 transferAmount = tAmount.sub(tLiquidity);
        return (tLiquidity , transferAmount);
    }

    function _transferStandard(address sender, address recipient, uint256 amount) private {
        _rOwned[sender] = _rOwned[sender].sub(amount);
        (uint256 tLiquidity , uint256 transferAmount) = getValues(amount);
    ...
    uint256 public _maxTxAmount = _rTotal; // should be 0.05% percent per transaction, will be set again at activateContract() function
    ...
    uint256 minTokenNumberToSell = _rTotal.mul(1).div(10000).div(10); // 0.001% max tx amount will trigger swap and add liquidity

    function setMaxTxPercent(uint256 maxTxPercent) public onlyOwner() {
        _maxTxAmount = _rTotal.mul(maxTxPercent).div(10000);
    }
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {
        uint256 _totalSupply = uint256(_rTotal)
