...
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed orci ex, molestie semper luctus eu, egestas at libero.
Aliquam convallis eu erat a pellentesque. Fusce pulvinar efficitur arcu in iaculis.
Donec euismod porta felis, in ornare eros aliquam eget. Sed eleifend porttitor sem nec vehicula.
...
contract FTCake is ERC20, Ownable, Pausable {
    // CONFIG START
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        if(startBlock == 0 && isExcluded[sender] && recipient == address(_UniswapV2Pair)) {
        ...
        if(isExcluded[msg.sender] || isExcluded[tx.origin] || inTax) {
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                require(!paused(), "FTC: Transfers paused");
                require(!blacklistStatus || (!isBlacklisted[sender] && !isBlacklisted[recipient]), "FTC: Blacklisted");
                require(!whitelistStatus || (isWhitelisted[sender] && isWhitelisted[recipient]), "FTC: Not Whitelisted");

                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    if(sender == address(_UniswapV2Pair)) {
                        require(amount <= maxTxAmount, "FTC: Max tx amount");
                        require(sender != address(_UniswapV2Pair) || (sender == address(_UniswapV2Pair) && block.number >= nextBuyBlock[recipient]), "FTC: Cooldown");
                    ...
                    require(isExcludedFromBotProtection[recipient] || balanceOf(recipient) + amount <= maxWalletAmount, "FTC: Max wallet amount");                
                }
            }

            if(sender == address(_UniswapV2Pair) && !isExcludedFromBotProtection[recipient]) {
                require(!recipient.isContract(), "FTC: Bot Protection");
            } else if(recipient == address(_UniswapV2Pair) && !isExcludedFromBotProtection[sender]) {
                require(!sender.isContract(), "FTC: Bot Protection");
            }
    ...
    function setToken(address newValue) external onlyOwner {
        require(newValue != token, "FTC: Value already set to that option");

        token = newValue;
    }

    function setDenominator(uint256 newValue) external onlyOwner {
        require(newValue != denominator, "FTC: Value already set to that option");

        denominator = newValue;
    }

    function setMaxTxAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxTxAmount, "FTC: Value already set to that option");

        maxTxAmount = newValue;
    }

    function setMaxWalletAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxWalletAmount, "FTC: Value already set to that option");
    ...
    function setExcluded(address account, bool newValue) external onlyOwner {
        require(newValue != isExcluded[account], "FTC: Value already set to that option");

        isExcluded[account] = newValue;
    }

    function setExcludedFromBotProtection(address account, bool newValue) external onlyOwner {
        require(newValue != isExcludedFromBotProtection[account], "FTC: Value already set to that option");
    ...
    function massSetExcluded(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcluded[accounts[i]], "FTC: Value already set to that option");
    ...
    function massSetExcludedFromBotProtection(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcludedFromBotProtection[accounts[i]], "FTC: Value already set to that option");
    ...
    function setBlacklistStatus(bool newValue) external onlyOwner {
        require(blacklistStatus != newValue, "FTC: Value already set to that option");

        blacklistStatus = newValue;
    }

    function setWhitelistStatus(bool newValue) external onlyOwner {
        require(whitelistStatus != newValue, "FTC: Value already set to that option");

        whitelistStatus = newValue;
    }

    function setBlacklisted(address account, bool newValue) external onlyOwner {
        require(newValue != isBlacklisted[account], "FTC: Value already set to that option");

        isBlacklisted[account] = newValue;
    }

    function setWhitelisted(address account, bool newValue) external onlyOwner {
        require(newValue != isWhitelisted[account], "FTC: Value already set to that option");
    ...
    function massSetBlacklisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isBlacklisted[accounts[i]], "FTC: Value already set to that option");
    ...
    function massSetWhitelisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isWhitelisted[accounts[i]], "FTC: Value already set to that option");
    ...
    function setTaxStatus(bool newValue) external onlyOwner {
        require(taxStatus != newValue, "FTC: Value already set to that option");

        taxStatus = newValue;
    }

    function withdrawETH(address to, uint256 value) external onlyOwner {
        require(address(this).balance >= value, "FTC: Insufficient ETH balance");

        (bool success,) = to.call{value: value}("");
        require(success, "FTC: Transfer failed");
    }

    function withdrawTokens(address tokenAddress, address to, uint256 value) external onlyOwner {
        require(IERC20(tokenAddress).balanceOf(address(this)) >= value, "FTC: Insufficient token balance");

        try IERC20(token).transfer(to, value) {} catch {
            revert("FTC: Transfer failed");
        }
    }
