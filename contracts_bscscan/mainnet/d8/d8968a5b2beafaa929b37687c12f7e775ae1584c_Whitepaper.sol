...
contract Whitepaper is ERC20, Ownable {
    ...
    IERC20 public constant TUSD = IERC20(0x14016E85a25aeb13065688cAFB43044C2ef86784); // TUSD
    ...
    constructor() ERC20("Whitepaper", "WP") {
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), address(TUSD));
    ...
    function _transfer(
        ...
        if(takeFee){
            // 99% fee for 5 blocks.  All tokens go to marketing so if provably not a bot/sniper, can refund.
            if(automatedMarketMakerPairs[from] && block.number <= tradingBlock +5){
                fees = amount * 990 / 1000;
                tokensForMarketing = fees;
            }
            // on sell
            else if (automatedMarketMakerPairs[to] && sellTotalFeesTier2 > 0 && (isSellTier2(from) || block.timestamp + 30 minutes <= lastBuyBack)){
    ...
    function swapTokensForTUSD(uint256 tokenAmount) internal {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of TUSD
            path,
            address(this),
            block.timestamp
        );

        swapEthForTusd(address(this).balance);
    }

    function swapEthForTusd(uint256 ethAmountInWei) internal {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 tusdAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        TUSD.approve(address(uniswapV2Router), tusdAmount);
    ...
    function swapBack() private {
        ...
        uint256 amountToSwapForTUSD = contractBalance - liquidityTokens;
        
        uint256 initialTUSDBalance = TUSD.balanceOf(address(this));

        swapTokensForTUSD(amountToSwapForTUSD); 
        
        uint256 tusdBalance = TUSD.balanceOf(address(this)) - initialTUSDBalance;
        
        uint256 tusdForBuyBack = tusdBalance * tokensForBuyBack / totalTokensToSwap;
        
        uint256 tusdForLiquidity = tusdBalance - tusdForBuyBack;
        
        tokensForLiquidity = 0;
        tokensForBuyBack = 0;
                
        if(liquidityTokens > 0 && tusdForLiquidity > 0){
            addLiquidity(liquidityTokens, tusdForLiquidity);
            emit SwapAndLiquify(amountToSwapForTUSD, tusdForLiquidity, tokensForLiquidity);
        }
        
        TUSD.transfer(buyBackWallet, TUSD.balanceOf(address(this)));
    }
}