...
contract NUNLPPool is Ownable {
    ...
    function getReward(PoolInfo storage pool, UserInfo storage user) private {
        
        uint256 reward = user.amount.mul(pool.accTokenPerShare).div(1e18).sub(user.rewardDebt);
        ...
        if(reward > 0) {
            ...
            {
                // referrer
                address referrer1 = referralRelationships[msg.sender];
                
                uint256 referrerFee1 = reward.mul(10).div(100);
                
                referralRewards[referrer1] =  referralRewards[referrer1].add(referrerFee1);
                safeTransfer(referrer1, referrerFee1); 
