/**
 *  SuccessToken is a project where we will earn BNB per transaction together as an holder. With the BNB reflections the team will donate for Charitys.
 *
 *  With 9% BNB coin reflections and 2% SCS tokens reflections to our holders.
 *
 *  Website: successtoken.io
 *  Telegram: t.me/SuccessToken
*/
...
contract SuccessToken is Context, IERC20, Ownable{
    ...
    string private _name = "Success Token";
    string private _symbol = "SCS";
    ...
    address payable teamWallet = 0x3981842319394E2EB014C14f0E02f14eDA1028fd;
    address payable charityWallet = 0xC9Db3afec85117778A32737Bd0AB988F8dcACacd;
    address payable marketingWallet = 0x60E0466ef666D558AE98f543037F401bf9c84a7A;

    // fees are set by promille - 20 equals to 2% and 100 equals to 10%
    ...
    uint256 public _rewardFee = 90; 
    uint256 private _previousRewardFee = _rewardFee;

    uint256 public _maxSellTransactionAmount = 1500000000000 * 10**18; // can't sell more than this
    uint256 public _maxWalletToken = 10000000000000 * 10**18; // can't buy or accumulate more than this
    uint256 public _numTokensSellToAddToLiquidity = 2000000 * 10**18;
    ...
    event SwapAndLiquify(uint256 tokens,uint256 bnb,uint256 bnb2);
    ...
    function setTeamFeePromille(uint256 teamFee) external onlyOwner() {
        _teamFee = teamFee;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
            ...
            ) 
       {
            ...
            uint256 landrFees = _liquidityFee.add(_rewardFee);

            uint256 liquidTokens = contractTokenBalance.mul(_liquidityFee).div(landrFees);
            swapAndLiquify(liquidTokens);

            uint256 _teamFeeTokens = contractTokenBalance.mul(_teamFee).div(_rewardFee);
            _tokenTransfer(address(this),teamWallet,_teamFeeTokens, false);
            uint256 _charityFeeTokens = contractTokenBalance.mul(_charityFee).div(_rewardFee);
            _tokenTransfer(address(this),charityWallet,_charityFeeTokens, false);

            uint256  _marketingFeeTokens = contractTokenBalance.mul(_marketingFee).div(_rewardFee);
            swapAndSendMarketing(_marketingFeeTokens);
    ...
    function swapAndSendMarketing(uint256 tokens) private {
        ...
        (bool success,) = address(marketingWallet).call{value: dividends}("");
...
contract BNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("SuccessTokenDivsTracker", "SuccessTokenDivsTracker") {
    	claimWait = 900;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 7200, "claimWait must be updated to between 15 minutes and 2 hours");
