...
// File: contracts/ChoochPresale.sol
...
contract ChoochPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale Chooch, 10 BUSD
	uint256 BUSDPerToken = 3* 10 ** 9;
	// Amount of BUSD received in presale
	uint256 busdReceived = 0;

    uint256 maxHardCap = 10 ** 6 * 10 ** 18;

    uint256 public minimumDepositEthAmount = 10 * 10 ** 18;
    uint256 public maximumDepositEthAmount = 1000 * 10 ** 18;

    address public multisign_address = 0xcCA8C0c638B0979Cd4D884Ac1184ECd58756A3e3;
	...
	function buy(uint256 _amount) public nonReentrant {
		...
		BUSD.safeTransferFrom(msg.sender, multisign_address, _amount);
	...
	function getChoochTokensLeft() external view returns (uint256) {
		return Chooch.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some Chooch tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed Chooch tokens");
		require(Chooch.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough Chooch tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		Chooch.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldChooch() external onlyOwner {
		uint256 amount = Chooch.balanceOf(address(this)) - totalTokensSold;
		Chooch.safeTransfer(msg.sender, amount);
	}

	function withdrawAllChooch() external onlyOwner {
		Chooch.safeTransfer(msg.sender, Chooch.balanceOf(address(this)));
	}
