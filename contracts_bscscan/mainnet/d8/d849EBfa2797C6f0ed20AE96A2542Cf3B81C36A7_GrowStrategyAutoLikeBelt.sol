...
// File contracts/strategies/BaseGrowStrategyV2_2.sol
pragma solidity 0.8.6;
...
abstract contract BaseGrowStrategyV2_2 is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
...
// File contracts/strategies/GrowStrategyAutoLikeBelt.sol
pragma solidity 0.8.6;
...
contract GrowStrategyAutoLikeBelt is BaseGrowStrategyV2_2 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    modifier transferTokenToVault(uint256 value) {
        if (ORIGINAL_TOKEN == WRAPPED_NATIVE_TOKEN) {
            if (msg.value != 0) {
            ...
            require(msg.value == 0, "Not BNB strategy");
        }
        _;
    }
    ...
    function withdrawOriginToken(uint256 wantTokenAmount) external virtual nonEmergency nonReentrant {
        _getRewards(msg.sender);
        uint256 withdrawnWantTokenAmount = _withdraw(wantTokenAmount);
        _sendOriginToken(msg.sender, withdrawnWantTokenAmount);
    }

    function withdrawAllOriginToken() external virtual nonEmergency nonReentrant {
        _getRewards(msg.sender);
        uint256 withdrawnWantTokenAmount = _withdraw(balanceOf(msg.sender));
        _sendOriginToken(msg.sender, withdrawnWantTokenAmount);
    }

    function depositOriginToken(uint256 originTokenAmount) external payable virtual nonEmergency nonReentrant transferTokenToVault(originTokenAmount) {
        _getRewards(msg.sender);
        uint256 wantTokenAmount = _receiveOriginToken(msg.sender, originTokenAmount);
    ...
    function _receiveOriginToken(address sender, uint256 amount) internal returns (uint256) {
        if (ORIGINAL_TOKEN != WRAPPED_NATIVE_TOKEN) {
            IERC20(ORIGINAL_TOKEN).safeTransferFrom(sender, address(this), amount);
        }

        approveToken(ORIGINAL_TOKEN, STAKING_TOKEN, amount);
        uint256 wantTokenBefore = IERC20(STAKING_TOKEN).balanceOf(address(this));
        MultiStrategyToken(STAKING_TOKEN).deposit(amount, 0);

        return IERC20(STAKING_TOKEN).balanceOf(address(this)).sub(wantTokenBefore);
    }

    function _sendOriginToken(address receiver, uint256 amount) internal {
        uint256 originTokenBefore = IERC20(ORIGINAL_TOKEN).balanceOf(address(this));
        MultiStrategyToken(STAKING_TOKEN).withdraw(amount, 0);
        uint256 originTokenReceived = IERC20(ORIGINAL_TOKEN).balanceOf(address(this)).sub(originTokenBefore);

        if (ORIGINAL_TOKEN == WRAPPED_NATIVE_TOKEN) {
            // Trust WETH will received given value
            IWETH(WRAPPED_NATIVE_TOKEN).withdraw(originTokenReceived);
            SafeToken.safeTransferETH(receiver, originTokenReceived);
        } else {
            IERC20(ORIGINAL_TOKEN).safeTransfer(receiver, originTokenReceived);
        }
    }
