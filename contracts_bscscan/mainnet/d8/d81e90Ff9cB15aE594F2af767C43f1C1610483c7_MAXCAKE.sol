...
contract MAXCAKE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MAXCAKEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MAXCAKE", "MAXCAKE") {
        dividendTracker = new MAXCAKEDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MAXCAKEDividendTracker newDividendTracker = MAXCAKEDividendTracker(
            payable(newAddress)
        );
...
contract MAXCAKEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "MAXCAKE_Dividend_Tracker",
            "MAXCAKE_Dividend_Tracker"
        )
    {
        mAXCAKE = MAXCAKE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "MAXCAKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MAXCAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MAXCAKE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(mAXCAKE);

            uint256 prevBalance = mAXCAKE.balanceOf(address(this));
            ...
            {
                uint256 received = mAXCAKE.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    mAXCAKE.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
