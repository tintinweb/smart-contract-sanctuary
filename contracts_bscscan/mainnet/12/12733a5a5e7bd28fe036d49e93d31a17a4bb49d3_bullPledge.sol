...
contract bullPledge  is Context,Ownable,ReentrancyGuard {
    ...
    function GetMyInfo() public view returns(address inviter,uint256 rateProfit,uint256 bonusProfit0,uint256 bonusProfit1,uint256 sumPledgeNum,uint256 pledgingNum,uint256 pledgingAmount,uint256 pledgedAmount,uint256 pledgedRedeemableNum,uint256 pledgedRedeemableAmount){

        inviter=Users[msg.sender].inviter;
        pledgingNum=Users[msg.sender].pledgingNum;
        pledgingAmount=Users[msg.sender].pledgingAmount;
        pledgedAmount=Users[msg.sender].pledgedAmount;
        rateProfit=Users[msg.sender].rateProfit;
        bonusProfit0=Users[msg.sender].bonusProfit0;
        bonusProfit1=Users[msg.sender].bonusProfit1;
        
        sumPledgeNum=PledgeRecords[msg.sender].length;
        
        for(uint256 i=0;i<sumPledgeNum;i++){
            ...
            if(PledgeRecords[msg.sender][i].beginTime>0 &&PledgeRecords[msg.sender][i].endTime==0  && block.timestamp >=(PledgeRecords[msg.sender][i].beginTime + PledgeRecords[msg.sender][i].day*1 minutes) ){
                pledgedRedeemableNum++;
                pledgedRedeemableAmount+=PledgeRecords[msg.sender][i].amount;
            }
        }
        
    }

    function xGetUserInfo(address user) internal view returns(address inviter,uint256 rateProfit,uint256 bonusProfit0,uint256 bonusProfit1,uint256 sumPledgeNum,uint256 pledgingNum,uint256 pledgingAmount,uint256 pledgedAmount,uint256 pledgedRedeemableNum,uint256 pledgedRedeemableAmount){
        inviter=Users[user].inviter;
        pledgingNum=Users[user].pledgingNum;
        pledgingAmount=Users[user].pledgingAmount;
        pledgedAmount=Users[user].pledgedAmount;
        rateProfit=Users[user].rateProfit;
        bonusProfit0=Users[user].bonusProfit0;
        bonusProfit1=Users[user].bonusProfit1;
        
        sumPledgeNum=PledgeRecords[user].length;
        
        for(uint256 i=0;i<sumPledgeNum;i++){
            
            if(PledgeRecords[user][i].beginTime>0 &&PledgeRecords[user][i].endTime==0  && block.timestamp >=(PledgeRecords[user][i].beginTime + PledgeRecords[user][i].day*1 minutes) ){
                pledgedRedeemableNum++;
                pledgedRedeemableAmount+=PledgeRecords[user][i].amount;
            }
        }
    }
