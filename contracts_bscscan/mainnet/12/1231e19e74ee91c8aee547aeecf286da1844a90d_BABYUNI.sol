...
contract BABYUNI is ERC20, Ownable {
    ...
    address public _dividendToken = 0xBf5140A22578168FD562DCcF235E5D43A02ce9B1;
    ...
    uint256 public maxWalletToken = 1000000000000000 * (10**18); // 10% of total supply

    uint256 public dividendRewardsFee;
    uint256 public marketingFee;
    uint256 public immutable totalFees;

    // sells have fees of 11 and 6 (10 * 1.2 and 5 * 1.2)
    ...
    constructor() ERC20("Baby Uniswap", "BABYUNI") {
    	...
    	buyBackWallet = 0x220859D7a209Ef07f725404d502ae22af8640094;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "BABYUNI: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "BABYUNI: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "BABYUNI: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "BABYUNI: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BABYUNI: Dividend reward tax must be between 0 and 10");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BABYUNI: The liquidity wallet is already this address");
...
contract BABYUNIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DogeReward_Dividend_Tracker", "BABYUNI_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BABYUNI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
