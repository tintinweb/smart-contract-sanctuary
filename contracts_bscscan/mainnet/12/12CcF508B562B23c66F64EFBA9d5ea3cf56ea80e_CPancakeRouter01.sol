...
contract CPancakeRouter01 {
    ...
    function swapExactTokensForTokens(
        ...
        amounts = IPancakeRouter01(pancakeRouter).getAmountsOut(_amountIn.sub(feeAmount), _path);
        IBEP20(_path[0]).safeTransferFrom(msg.sender, address(this), _amountIn);
        IBEP20(_path[0]).approve(pancakeRouter, _amountIn);
        amounts = IPancakeRouter01(pancakeRouter).swapExactTokensForTokens(
        ...
        returns (uint[] memory amounts)
    {
        ...
        IBEP20(_path[0]).safeTransferFrom(msg.sender, address(this), _amountIn);
        IBEP20(_path[0]).approve(pancakeRouter, _amountIn);
        amounts = IPancakeRouter01(pancakeRouter).swapExactTokensForETH(
            _amountIn.sub(feeAmount),
            _amountOutMin.sub(_amountOutMin.mul(IClimbFeeStrategy(climbStrategy).slippageForNativeTokens()).div(10000)),
            _path,
            _to,
    ...
    function swapETHForExactTokens(uint _amountOut, address[] calldata _path, address _to, uint _deadline)
        ...
        returns (uint[] memory amounts)
    {
        amounts = IPancakeRouter01(pancakeRouter).swapETHForExactTokens{value: msg.value}(_amountOut, _path, _to, _deadline);
    ...
    function swapTokensForExactTokens(
        ...
        amounts = IPancakeRouter01(pancakeRouter).getAmountsIn(_amountOut, _path);
        ...
        uint[] memory amountsOut = IPancakeRouter01(pancakeRouter).getAmountsOut(amounts[0], _path);
        amounts = IPancakeRouter01(pancakeRouter).swapTokensForExactTokens(amountsOut[amountsOut.length - 1], _amountInMax, _path, _to, _deadline);
        if (feeAmount > 0) {
            IBEP20(_path[0]).safeTransfer(IClimbFeeStrategy(climbStrategy).feeAddress(), feeAmount);
        }
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts[0] = amounts[0].sub(feeAmount);
        uint[] memory amountsOut = IPancakeRouter01(pancakeRouter).getAmountsOut(amounts[0], _path);
        amounts = IPancakeRouter01(pancakeRouter).swapTokensForExactETH(amountsOut[amountsOut.length - 1], _amountInMax, _path, _to, _deadline);
        if (feeAmount > 0) {
            IBEP20(_path[0]).safeTransfer(IClimbFeeStrategy(climbStrategy).feeAddress(), feeAmount);
        }
    }
    ...
    function swapExactETHForTokens(uint _amountOutMin, address[] calldata _path, address _to, uint _deadline)
        ...
        returns (uint[] memory amounts)
    {
        amounts = IPancakeRouter01(pancakeRouter).swapExactETHForTokens{value: msg.value}(_amountOutMin, _path, _to, _deadline);
    }
    ...
    function setClimbStrategy(address _climbStrategy) public onlyOperator {
        climbStrategy = _climbStrategy;
    }
