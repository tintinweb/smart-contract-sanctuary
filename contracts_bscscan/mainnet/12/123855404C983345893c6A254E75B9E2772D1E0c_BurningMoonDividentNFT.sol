...
contract BurningMoonDividentNFT is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    ...
    // Token name
    string private _name="BurningMoon Spirit Animals";
    // Token symbol
    string private _symbol="BMSA";
    ...
    enum Rarity{ Common, Rare, UltraRare, Legendary, LegendaryShiny }
    struct NFTData{
        string Name;
        string URI;
        Rarity rarity;
    ...
    uint256 public currentValue=10**18/2; //0.5 BNB for first NFT
    uint8 Taxes=18;//1.8% increase per sell, equals last NFT sold for ~3 bnb
    ...
    constructor() {
        ...
        BM=BurningMoon(0x97c6825e6911578A515B11e25B552Ecd5fE58dbA);
        //BM=BurningMoon(0x1Fd93329706579516e18ef2B51890F7a146B5b14);
        PCS=IPancakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        //PCS=IPancakeRouter02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    }
    bool _isInFunction;
    ...
    function AirdropBM() external isInFunction{
        _distributeCompound();
        for(uint i=0;i<NFTHolders.length;i++)
            _payout(NFTHolders[i]);
    }
    function AccountClaimDividents() external isInFunction{
        _distributeCompound();
    ...
    function _distributeBNB() private{
        uint256 amount=address(this).balance-ContractBNB;
        if(amount<=0) return;
            uint256 BMAmount=amount*7/10;
            ContractBNB+=amount-BMAmount;
    ...
    function _buyAndSacrificeBM(uint256 amount)private{
        ...
        PCS.swapExactETHForTokensSupportingFeeOnTransferTokens{value:amount}(
            0,
            path,
            address(this),
            block.timestamp
        ...
        //Sacrifice new BM
        uint256 newBalance=BM.balanceOf(address(this))-initialBalance;
        BM.transfer(address(0xdead),newBalance);
    }
    function _distributeCompound() private{
    ...
    function OwnerAddNFTs(NFTData[] memory data) external onlyOwner{
        for(uint256 i=0;i<data.length;i++)
            NFTs.push(data[i]);
    }
    function OwnerClaimBNB() external onlyOwner{
        require(ContractBNB>0);
        uint256 bnb=address(this).balance;        
        if(ContractBNB>bnb)ContractBNB=bnb;
        uint256 amount=ContractBNB;
        ContractBNB=0;
    ...
    function BuyNFT(string memory NFTName) external payable{
        uint buyID;
        uint cheapestValue = type(uint).max;
        bytes32 nameHash = keccak256(abi.encodePacked(NFTName));
        for(uint i=0;i<TokenForSale.length();i++){
            uint NFTID=TokenForSale.at(i);    
            if(keccak256(abi.encodePacked(NFTs[NFTID].Name))==nameHash){         
                uint newValue=getValue(NFTID);
                if(newValue<cheapestValue){
                    cheapestValue=newValue;
                    buyID=NFTID;
    ...
    //NFTPresale//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////     
    function PresaleIsIn(address account) public view returns (bool){
        return presalePurchases.contains(account);
    }
    function PresalePurchaseAt(uint256 ID) public view returns(address){
        return presalePurchases.at(ID);
    }
    function PresalePurchase() public payable isInFunction{
        require(_presaleOpen,"Presale not yet open");
        require(!_onlyWhitelist||Whitelist[msg.sender],"not on whitelist");
        require(NFTHolders.length+presalePurchases.length()<NFTs.length,"No NFTs left");
        require(!presalePurchases.contains(msg.sender),"sender already in presale");
        uint256 Value=currentValue;
        currentValue= currentValue*(1000+Taxes)/1000;
        require(msg.value>=Value,"msg.value less than NFT value");
        presalePurchases.add(msg.sender);
        if(msg.value>Value){
    ...
    EnumerableSet.AddressSet presalePurchases;
    mapping (address=>bool) Whitelist;
    bool _presaleOpen;
    bool _onlyWhitelist;
    function OwnerSetWhitelist(address[] memory addresses, bool add) public onlyOwner{
        for(uint i=0;i<addresses.length;i++)
            Whitelist[addresses[i]]=add;
    } 
    function OwnerSetPresale(bool PresaleOpen, bool OnlyWhitelist) public onlyOwner{
        _presaleOpen=PresaleOpen;
        _onlyWhitelist=OnlyWhitelist;
    }
    function OwnerAddToPresale(address[] memory recipients, bool add) external onlyOwner{
        require(!add||((NFTHolders.length+presalePurchases.length()+recipients.length)<NFTs.length),"No NFTs left");
        for(uint256 i=0; i<recipients.length; i++){
            if(add) presalePurchases.add(recipients[i]);
            else presalePurchases.remove(recipients[i]);
        }
    }
  	function OwnerMintPresalePurchases(uint256 count) public onlyOwner{
  	    require(presalePurchases.length()>0);
  	    
  	    for(uint256 i=0;i<count;i++){
  	        uint256 index=_prng(presalePurchases.length(),i);
  	        address current=presalePurchases.at(index);
  	        presalePurchases.remove(current);
  	        _mint(current);
  	        if(presalePurchases.length()==0) {
  	            return;
  	    }}
  	}
    ...
    function getSharesOfNFT(uint256 ID) public view returns(uint256){
        require(_exists(ID));
        uint rarity=uint(NFTs[ID].rarity);
        if(rarity==0) return 1;
        if(rarity==1) return 2;
        if(rarity==2) return 4;
        if(rarity==3) return 8;
        if(rarity==4) return 16;
        else return 1;
    }
    ...
    function _mint(address to) private{
        ...
        require(ID<NFTs.length, "no Tickets left");
        ownerID.push(0);
        NFTHolders.push(to);
        _AddNFT(to,ID);
        NFTValue.push(0);
    ...
    function _AddNFT(address account, uint256 ID) private{
        ...
        uint256 NFTShares=getSharesOfNFT(ID);
  ...
  //  }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory _tokenURI = NFTs[tokenId].URI;
