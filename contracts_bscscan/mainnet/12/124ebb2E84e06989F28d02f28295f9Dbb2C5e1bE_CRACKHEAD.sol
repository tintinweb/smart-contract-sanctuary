...
interface IDividendDistributor {
    ...
    function setnewRewardsToken(address _nT) external;
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function setnewRewardsToken(address _nT) external override onlyToken {
        REWARDS = IBEP20(_nT);
    }
}

contract CRACKHEAD is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "CRACKHEAD";
    string constant _symbol = "CRACK";
    ...
    uint256 _mFm = tmnFd.div(tmdFd);
    uint256 _lFm = tlnFd.div(tldFd);
    uint256 _rFm = trnFd.div(trdFd);
    ...
    uint256 public _sMN = 125;
    uint256 public _sMD = 100;
    ...
    function setMTP(uint256 mTP) external onlyOwner() {
        _maxTxAmount = _totalSupply.mul((mTP).div(100));
    }

    function setMWP(uint256 mWP) external onlyOwner() {
        _maxWalletToken = _totalSupply.mul((mWP).div(100));
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == pair &&
            ...
            cdT[recipient] = block.timestamp + cTI;
        }
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling && sME){ return ((tF.mul(_sMN)).div(_sMD)); }
    ...
    function swapBack() internal swapping {
        uint256 dLF = isOverLiquified(tL, tLD) ? 0 : lF.mul(_lFm);
        ...
        uint256 aBNBR = aBNB.mul(rF.div(_rFm)).div(tBNBF);
        uint256 aBNBM = aBNB.mul(mF.mul(_mFm)).div(tBNBF);
    ...
    function setnRT(address _nT) external authorized {
        distributor.setnewRewardsToken(_nT);
    }
