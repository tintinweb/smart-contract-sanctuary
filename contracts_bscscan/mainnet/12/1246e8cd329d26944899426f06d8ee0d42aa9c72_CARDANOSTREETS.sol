...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
//   Social Media Handles Of CARDANO STREETS PEOJET   

//  CARDANO STREETS is a community-driven token that provides a use case,A PVP
...
contract CARDANOSTREETS is Context, IERC20, Ownable { 
    ...
    address payable private _devWalletAddress = payable(0x3EA35b8eE307487e917D6be95aCfcD485D69aaE0); //0x3EA35b8eE307487e917D6be95aCfcD485D69aaE0
    address payable private _marketingWalletAddress = payable(0x978fb679b3A133cFc3135b016497E635330c166B); // 0x978fb679b3A133cFc3135b016497E635330c166B
    address payable private _developmentWalletAddress = payable(0x100f3dD7467BfC05b17d773307F2e2A32770de38); //0x100f3dD7467BfC05b17d773307F2e2A32770de38
    ...
    string private _symbol = "CARSTREETS";
    ...
    uint256 public _marketingfee_Buy= 3;
    uint256 public _developmentfee_Buy = 3;
    uint256 public _reflectionFee_Buy = 2;
    uint256 private _previousReflectionFee_Buy = _reflectionFee_Buy;
    uint256 public _devFee_Buy = 3;
    uint256 public _liquidityFee_Buy = 2;
    uint256 private _totalBNBFees_Buy = _marketingfee_Buy + _developmentfee_Buy + _devFee_Buy + _liquidityFee_Buy;
    uint256 private _previousTotalFee_Buy = _totalBNBFees_Buy;

    uint256 public _marketingfee_Sell =6;
    uint256 public _developmentfee_Sell = 5;
    uint256 public _reflectionFee_Sell = 2;
    uint256 private _previousReflectionFee_Sell = _reflectionFee_Sell;
    uint256 public _devFee_Sell = 5;
    uint256 public _liquidityFee_Sell = 2;
    uint256 private _totalBNBFees_Sell = _marketingfee_Sell + _developmentfee_Sell + _devFee_Sell + _liquidityFee_Sell;
    uint256 private _previousTotalFee_Sell = _totalBNBFees_Sell;



    uint256 public deployTime;
    uint256 public firstblockNumber;
    bool public isFirstHour;
    bool public initialBlocks;
    

    uint256 public maxWalletBalance = (totalSupply() * 2) / 100;
    ...
    uint256 public numTokensSellToAddToLiquidity = 400000 * 10**9;
    uint256 public numTokensBuyToAddToLiquidity = 400000 * 10**9;
    ...
    function setTokensToSwapSell(uint256 _swap) external onlyOwner() {
        numTokensSellToAddToLiquidity = _swap * 10**9;
    }
    
    function setTokensToSwapBuy(uint256 _swap) external onlyOwner() {
        numTokensBuyToAddToLiquidity = _swap * 10**9;
    }
    
    function setReflectionFeePercent_Buy(uint256 taxFee) external onlyOwner() {
        _reflectionFee_Buy = taxFee;
    }
    
    function setDevFeePercent_Buy(uint256 giveawayFee) external onlyOwner() {
        _devFee_Buy = giveawayFee;
        _totalBNBFees_Buy = _marketingfee_Buy + _developmentfee_Buy + _devFee_Buy + _liquidityFee_Buy;
        _previousTotalFee_Buy = _totalBNBFees_Buy;
    }
    
     function setDevelopmentFeePercent_Buy(uint256 giveawayFee) external onlyOwner() {
        _developmentfee_Buy = giveawayFee;
        _totalBNBFees_Buy = _marketingfee_Buy + _developmentfee_Buy + _devFee_Buy + _liquidityFee_Buy;
        _previousTotalFee_Buy = _totalBNBFees_Buy;
    }
    
     function setMarketingFeePercent_Buy(uint256 giveawayFee) external onlyOwner() {
        _marketingfee_Buy = giveawayFee;
        _totalBNBFees_Buy = _marketingfee_Buy + _developmentfee_Buy + _devFee_Buy + _liquidityFee_Buy;
        _previousTotalFee_Buy = _totalBNBFees_Buy;
    }
    
    function setLiquidityFeePercent_Buy(uint256 liquidityFee) external onlyOwner() {
   ...
   function setReflectionFeePercent_Sell(uint256 taxFee) external onlyOwner() {
        _reflectionFee_Sell = taxFee;
    }
    
    function setDevFeePercent_Sell(uint256 giveawayFee) external onlyOwner() {
        _devFee_Sell = giveawayFee;
        _totalBNBFees_Sell = _marketingfee_Sell + _developmentfee_Sell + _devFee_Sell + _liquidityFee_Sell;
        _previousTotalFee_Sell = _totalBNBFees_Sell;
    }
    
     function setDevelopmentFeePercent_Sell(uint256 giveawayFee) external onlyOwner() {
        _developmentfee_Sell = giveawayFee;
        _totalBNBFees_Sell = _marketingfee_Sell + _developmentfee_Sell + _devFee_Sell + _liquidityFee_Sell;
        _previousTotalFee_Sell = _totalBNBFees_Sell;
    }
    
     function setMarketingFeePercent_Sell(uint256 giveawayFee) external onlyOwner() {
        _marketingfee_Sell = giveawayFee;
        _totalBNBFees_Sell = _marketingfee_Sell + _developmentfee_Sell + _devFee_Sell + _liquidityFee_Sell;
        _previousTotalFee_Sell = _totalBNBFees_Sell;
    }
    
    function setLiquidityFeePercent_Sell(uint256 liquidityFee) external onlyOwner() {
    ...
    function _transfer(
        ...
        if (
            ...
            if(isFirstHour && !initialBlocks){
                    _marketingfee_Buy= 3;
                     _developmentfee_Buy = 3;
                     _reflectionFee_Buy = 2;
                     _previousReflectionFee_Buy = _reflectionFee_Buy;
                     _devFee_Buy = 3;
                     _liquidityFee_Buy = 2;
                     _totalBNBFees_Buy = _marketingfee_Buy + _developmentfee_Buy + _devFee_Buy + _liquidityFee_Buy;
                     _previousTotalFee_Buy = _totalBNBFees_Buy;
                    _marketingfee_Sell =6;
                     _developmentfee_Sell = 5;
                     _reflectionFee_Sell = 2;
                     _previousReflectionFee_Sell = _reflectionFee_Sell;
                     _devFee_Sell = 5;
                     _liquidityFee_Sell = 2;
                     _totalBNBFees_Sell = _marketingfee_Sell + _developmentfee_Sell + _devFee_Sell + _liquidityFee_Sell;
                     _previousTotalFee_Sell = _totalBNBFees_Sell;
                    if((deployTime+3600)<=block.timestamp){
                        isFirstHour  = false;
                    }
            }else if(initialBlocks){
                    _marketingfee_Buy= 70;
                     _developmentfee_Buy = 0;
                     _reflectionFee_Buy = 0;
                     _previousReflectionFee_Buy = _reflectionFee_Buy;
                     _devFee_Buy = 0;
                     _liquidityFee_Buy = 20;
                     _totalBNBFees_Buy = _marketingfee_Buy + _developmentfee_Buy + _devFee_Buy + _liquidityFee_Buy;
                     _previousTotalFee_Buy = _totalBNBFees_Buy;
                    
                     _marketingfee_Sell =70;
                     _developmentfee_Sell = 0;
                     _reflectionFee_Sell = 0;
                     _previousReflectionFee_Sell = _reflectionFee_Sell;
                     _devFee_Sell = 0;
                     _liquidityFee_Sell = 20;
                     _totalBNBFees_Sell = _marketingfee_Sell + _developmentfee_Sell + _devFee_Sell + _liquidityFee_Sell;
                     _previousTotalFee_Sell = _totalBNBFees_Sell;
                    if((firstblockNumber+15)<=block.number){
                        initialBlocks = false;
                    }
            }else{
                if(from == uniswapV2Pair){
                        _marketingfee = _marketingfee_Buy;
                        _developmentfee = _developmentfee_Buy;
                        _reflectionFee = _reflectionFee_Buy;
                        _previousReflectionFee = _previousReflectionFee_Buy;
                        _devFee =_devFee_Buy;
                        _liquidityFee = _liquidityFee_Buy;
                        _totalBNBFees = _totalBNBFees_Buy;//_marketingfee + _developmentfee + _devFee + _liquidityFee;
                        _previousTotalFee = _previousTotalFee_Buy;
    
                contractTokenBalance = numTokensBuyToAddToLiquidity;
    
                }else{
                        _marketingfee = _marketingfee_Sell;
                        _developmentfee = _developmentfee_Sell;
                        _reflectionFee = _reflectionFee_Sell;
                        _previousReflectionFee = _previousReflectionFee_Sell;
                        _devFee =_devFee_Sell;
                        _liquidityFee = _liquidityFee_Sell;
                        _totalBNBFees = _totalBNBFees_Sell;//_marketingfee + _developmentfee + _devFee + _liquidityFee;
                        _previousTotalFee = _previousTotalFee_Sell;
