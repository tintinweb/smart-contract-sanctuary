...
contract BattleCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BattleCakeDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public operations1Address =
        payable(0x889d91731a7E5E08548F891483103a97a5e401Db);
    address payable public operations2Address =
        payable(0x39c06f91A05bf4dfE5DA177F766F5C7d1085c951);
    ...
    constructor() ERC20("BattleCake", "BCAKE") {
        dividendTracker = new BattleCakeDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BattleCakeDividendTracker newDividendTracker = BattleCakeDividendTracker(
            payable(newAddress)
        );
...
contract BattleCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "BattleCake_Dividend_Tracker",
            "BattleCake_Dividend_Tracker"
        )
    {
        battleCake = BattleCake(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BattleCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BattleCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BattleCake contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(battleCake);

            uint256 prevBalance = battleCake.balanceOf(address(this));
            ...
            {
                uint256 received = battleCake.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    battleCake.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
