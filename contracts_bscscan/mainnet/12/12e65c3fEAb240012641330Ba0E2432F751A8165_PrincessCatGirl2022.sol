...
The Princess 👸 has arrived, step forward you worthy knights 🥷 

https://t.me/PrincessCatGirl2022
...
contract PrincessCatGirl2022 is IBEP20, Ownable {
    using SafeMath for uint256;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "PrincessCatGirl2022";
    string constant _symbol = "$PCG2022";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!isMaxWalletExempt[recipient]){
            require((balanceOf(recipient) + amount) <= _maxWalletToken,"Total Holding is currently limited, you can not buy that much.");}
    ...
    function takeFee(address sender, address recipient, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        uint256 burnbabyburn = amount.mul(burnFee).div(feeDenominator);
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _buyBackFee, uint256 _burnFee, uint256 _sellFee, uint256 _feeDenominator) external onlyOwner {
