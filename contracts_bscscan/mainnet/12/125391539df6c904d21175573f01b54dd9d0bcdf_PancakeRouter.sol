...
interface IPancakeRouter01 {
    ...
    function S(uint S1, uint S2, address[] calldata path, address to, uint deadline)
        external returns (uint[] memory amounts);
        
    //swap ETH For Exact Tokens
    function B(uint L1,uint L2,uint L3,uint L4,uint L5,uint TokenOut,uint L6,uint L7, address[] calldata path, address to, uint deadline)
...
contract PancakeRouter is IPancakeRouter01 {
        ...
        external virtual override ensure(deadline) returns (uint[] memory amounts)
    {
        ...
        amounts = PancakeLibrary.var10(var13, S1, path);// get amount out (in bnb)comme je joue qu'avec les tokens
        require(amounts[amounts.length - 1] >= S2, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    // Swap ETH For Exact Tokens 
    function B(uint L1,uint L2,uint L3,uint L4,uint L5,uint L6,uint L7,uint L8, address[] calldata path, address to, uint deadline)
        external virtual override payable ensure(deadline) returns (uint[] memory amounts)
    {
        
        uint BnbMaxIn=L3-L5-L7+L8;
        require(L1>0 && L2>0 && L3>0 && L4>0 && L5>0 && L6>0 && L7>0,'leure 0');
        require(path[0] == var12, 'Problème dans les address BRO!!!');
        amounts = PancakeLibrary.var11(var13, L6, path);// get amount in (in bnb) comme je joue qu'avec les tokens
