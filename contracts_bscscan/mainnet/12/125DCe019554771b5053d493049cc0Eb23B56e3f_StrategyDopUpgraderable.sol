...
// File: contracts/interfaces/IDoppleXFarm.sol


pragma solidity 0.6.12;

interface IDoppleXFarm {
...
// File: contracts/StrategyDopUpgraderable.sol
...
contract StrategyDopUpgraderable is ReentrancyGuardUpgradeSafe, OwnableUpgradeable , Pausable {
    ...
    address public doppleSwapAddress;
    ...
    function initialize(
        ...
        if (isGrandComp) {
            ...
            doppleSwapAddress = _doppleSwapAddress;
            IERC20(busd).approve(doppleSwapAddress, uint256(-1)); // 100% trust in the doppleSwap
            IERC20(usdt).approve(doppleSwapAddress, uint256(-1)); // 100% trust in the doppleSwap
    ...
    function _farm() internal {
        ...
        IDoppleXFarm(farmContractAddress).deposit(pid, wantAmt);
        ...
        returns (uint256)
    {
        ...
        if (isGrandComp) {
            IDoppleXFarm(farmContractAddress).withdraw(pid, _wantAmt);
        }
    ...
    function earn() public whenNotPaused {
        ...
        // Harvest farm tokens
        IDoppleXFarm(farmContractAddress).deposit(pid,0);
        ...
        address fToken = IERC20(busd).balanceOf(doppleSwapAddress) < IERC20(usdt).balanceOf(doppleSwapAddress) ? busd : usdt;
        address[] memory earnedToFTokenPath = new address[](3);
        (earnedToFTokenPath[0], earnedToFTokenPath[1], earnedToFTokenPath[2]) = (earnedAddress, wbnb, fToken);
        ...
        uint256 fTokenAmt = IERC20(fToken).balanceOf(address(this));
        if (fTokenAmt > 0) {
            uint256[] memory amounts = new uint256[](3);
            (amounts[0], amounts[1], amounts[2]) = (0, IERC20(busd).balanceOf(address(this)), IERC20(usdt).balanceOf(address(this)));
            IDoppleSwap(doppleSwapAddress).addLiquidity(amounts, 0, now);
        }
