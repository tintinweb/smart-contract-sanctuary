    ...
    IDividendPayingTokenOptional
{
    ...
    address public dividendToken = 0x55d398326f99059fF775485246999027B3197955; // Binance-Peg USD
...
contract sbbn is ERC20, Ownable {
    ...
    address public _dividendToken = 0x55d398326f99059fF775485246999027B3197955; // Binance-Peg USD 
    ...
    shibabinanceDividendTracker public dividendTracker;

    address public shibabinanceWallet;
    ...
    uint256 public shibabinanceFee;
    ...
    event shibabinanceWalletUpdated(
    ...
    constructor() ERC20("sbbn", "sbbn") {
        ...
        uint256 _shibabinanceFee = 10;

        dividendRewardsFee = _dividendRewardsFee;
        shibabinanceFee = _shibabinanceFee;
        totalFees = _dividendRewardsFee.add(_shibabinanceFee);

        dividendTracker = new shibabinanceDividendTracker();

        shibabinanceWallet = 0x0543eDB2c291419dBf43b18c7A5329924D0892a7;
        ...
        excludeFromFees(shibabinanceWallet, true);
        ...
        onlyOwner
    {
        require(
            ...
            "shibabinance: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "shibabinance: Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "shibabinance: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "shibabinance: The dividend tracker already has that address"
        );

        shibabinanceDividendTracker newDividendTracker = shibabinanceDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "shibabinance: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "shibabinance: Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updateshibabinanceFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "shibabinance: Dividend reward tax must be between 0 and 10"
        );
        shibabinanceFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "shibabinance: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "shibabinance: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "shibabinance: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "shibabinance: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateshibabinanceWallet(address newshibabinanceWallet) public onlyOwner {
        require(
            newshibabinanceWallet != shibabinanceWallet,
            "shibabinance: The liquidity wallet is already this address"
        );
        excludeFromFees(newshibabinanceWallet, true);
        shibabinanceWallet = newshibabinanceWallet;
        emit shibabinanceWalletUpdated(newshibabinanceWallet, shibabinanceWallet);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue >= 200000 && newValue <= 500000,
            "shibabinance: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "shibabinance: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(shibabinanceFee).div(
                    totalFees
                );
                swapTokensForBNB(swapTokens);
                transferToshibabinanceWallet(
                    payable(shibabinanceWallet),
    ...
    function transferToshibabinanceWallet(address payable recipient, uint256 amount)
        private
    {
...
contract shibabinanceDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "shibabinance_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "shibabinance_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main shibabinance contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "shibabinance_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "shibabinance_Dividend_Tracker: Cannot update claimWait to same value"
        );
