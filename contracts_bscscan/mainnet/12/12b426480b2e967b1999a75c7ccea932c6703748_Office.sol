...
contract Code is ERC20("Code", "CODE"), Ownable {
    ...
    function transferToPantry(address _from, uint256 _amount) external {
        require(_msgSender() == repoAddress, "Only the Repo contract can call transferToPantry");
...
contract Office is Ownable {
    ...
    uint256 public constant YIELD_CPS = 16666666666666667; // codes writed per second per unit of yield
    uint256 public constant CLAIM_CODE_TAX_PERCENTAGE = 20;
    ...
    constructor(Developer _developer, Code _code, address _repoAddress) {
    ...
    function getCodesAccruedForMany(uint256[] calldata _tokenIds) external view returns (uint256[] memory) {
        uint256[] memory codeAmounts = new uint256[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            codeAmounts[i] = _getCodesAccruedFor(_tokenIds[i], false);
        }
        return codeAmounts;
    }

    function _getCodesAccruedFor(uint256 _tokenId, bool checkOwnership) internal view returns (uint256) {
        ...
        return (block.timestamp - stake.startTimestamp) * developer.tokenYield(_tokenId) * YIELD_CPS;
    }

    /* Mutators */

    function stakeMany(uint256[] calldata _tokenIds) external {
        require(developer.gameStarted(), "The game has not started");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(developer.ownerOf(tokenId) == _msgSender(), "You don't own this token");
            require(developer.tokenRevealed(tokenId), "Token not yet revealed");

            developer.transferFrom(_msgSender(), address(this), tokenId);
            _addDeveloperToOffice(tokenId, _msgSender());
        }
    }

    function claimCodesAndMaybeUnstake(uint256[] calldata _tokenIds, bool unstake) external {
        uint256 totalClaimed = 0;
        uint256 totalTaxed = 0;

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            uint256 codes = _getCodesAccruedFor(tokenId, true); // also checks that msg.sender owns this token
            uint256 taxAmount = (codes * CLAIM_CODE_TAX_PERCENTAGE + 99) / 100; // +99 to round the division up

            totalClaimed += codes - taxAmount;
            totalTaxed += taxAmount;
            stakes[tokenId].startTimestamp = block.timestamp;

            if (unstake) {
                _moveDeveloperToCooldown(tokenId);
            }
        }

        code.mint(_msgSender(), totalClaimed);
        code.mint(repoAddress, totalTaxed);
    }

    function withdrawDevelopers(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            ...
            developer.transferFrom(address(this), _msgSender(), tokenId);
            _removeDeveloperFromCooldown(tokenId);
        }
    }

    function _addDeveloperToOffice(uint256 _tokenId, address _owner) internal {
    ...
    function _moveDeveloperToCooldown(uint256 _tokenId) internal {
    ...
    function _removeDeveloperFromCooldown(uint256 _tokenId) internal {
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = OwnedStakeInfo({
                tokenId: tokenId,
                level: developer.revealedTokenLevel(tokenId),
                accrual: _getCodesAccruedFor(tokenId, false)
            });
        }
