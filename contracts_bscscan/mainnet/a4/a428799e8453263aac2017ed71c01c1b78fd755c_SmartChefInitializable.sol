...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (depositFee > 0) {
                ...
                stakedToken.safeTransfer(address(feeAddress), feeAmount);
                user.amount = user.amount.add(_amount).sub(feeAmount);
                totalStakedAmount = totalStakedAmount.add(_amount).sub(feeAmount);
            } else {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && totalStakedAmount != 0) {
            ...
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(tokenReward.mul(PRECISION_FACTOR).div(totalStakedAmount));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        rewardToken.safeTransfer(devAddr, tokenReward.div(10));
