...
// Join https://t.me/Fairlaunchesonly
...
interface IBEP20 {
    ...
    interface DynamicTaxHelper {
    function getTaxMultiplier(uint256 _tTotal, uint256 _poolTokens) external returns(uint256, uint256);
}
...
contract DynamicTaxes is Context, IBEP20, Ownable {
    ...
    address payable _marketingWallet = payable(0xA0ecb1456aC382215a739bFb23942dFEef7cA941);
    ...
    uint256 private _poolTokens = _tTotal;
    uint256 private _buyTax;
    uint256 private _sellTax ;
    

    string private _name     = "Dynamo";
    string private _symbol   = "Dynamo";
    uint8 private  _decimals = 9;

    uint256 private  _taxFee       = 5; // redistributed to holders
    uint256 private  _liquidityFee = 5; // kept for liquidity
    uint256 private  _marketingFee = 5; // marketing wallet
    uint256 private  _burnFee      = 5; // burned
    ...
    event TaxesChanged(uint256 _buyTax, uint256 _sellTax);
    ...
    function setMaxTx(uint256 maxTx) external onlyOwner {
        require(maxTx >= _tTotal.div(200),"Maximum Transaction can not set to less than 0.5% of the supply");
        _maxTxAmount = maxTx;
    }
    ...
    _poolTokens = balanceOf(uniswapV2Pair);
        
    }
    
    function getTaxesExternal() internal{
     (_buyTax,_sellTax) = DynamicTaxHelper(0x052b0A032e28b7B5bbB41b8de72F0079c54bb0aB).getTaxMultiplier(_tTotal,_poolTokens);
    }
    
    function calculateDynamicBuyTaxes() internal{
        _taxFee       = _taxFee.mul(_buyTax).div(1000);
        _liquidityFee = _liquidityFee.mul(_buyTax).div(1000);
        _marketingFee = _marketingFee.mul(_buyTax).div(1000);
        _burnFee      = _burnFee.mul(_buyTax).div(1000);
    }
    
    function calculateDynamicSellTaxes() internal{
        _taxFee       = _taxFee.mul(_sellTax).div(200);
        _liquidityFee = _liquidityFee.mul(_sellTax).div(200);
        _marketingFee = _marketingFee.mul(_sellTax).div(200);
        _burnFee      = _burnFee.mul(_sellTax).div(200);
    }
...
//            require(amount <= _maxTxAmount, "You are trying to transfer too many tokens at once. Please respect the maximum transaction amount.");
//        }
...
//            require(contractBalanceRecepient + amount <= _maxWalletToken, "You are trying to buy too many tokens, please buy less than the maxWallet");
        }
...
//            contractTokenBalance = numTokensSellToAddToLiquidity;
        ...
        if(
            to == uniswapV2Pair
                ...
                calculateDynamicSellTaxes();
            }
            
        if(
            from == uniswapV2Pair
            ){
                calculateDynamicBuyTaxes();
            }
            
            emit TaxesChanged(_buyTax, _sellTax);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        _marketingWallet.transfer(initialBalance.add(newBalance.div(2)));
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        takeTransactionFee(address(this), fd.tMarketing, fd.currentRate);
