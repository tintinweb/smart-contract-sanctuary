...
contract YSLOptSinglePancake is YSLOpt {
    using SafeERC20 for IERC20;
    ...
    address public constant pancakeMaster = 0x73feaa1eE314F8c655E354234017bE2193C9E24E;
    address public constant pancakeSwap = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    function compound() external override {
        ...
        if (wantBalance > DUST) {
            ...
            IStrategySwap(stratSwap).swapBusdToLP(IERC20(BUSD).balanceOf(address(this)), pancakeSwap, path0);
        }
        // Deposit (compound) underlyings
        uint256 lpBalance = lpToken.balanceOf(address(this));
        if (lpBalance > DUST) {
            lpToken.approve(pancakeMaster, lpToken.balanceOf(address(this)));
            IPancakeMaster(pancakeMaster).enterStaking(lpToken.balanceOf(address(this)));
        }
    }
    ...
    function earn(address _user, bool _isAmplified) external override onlyRole(STRAT_ROLE) {
        ...
        if (curBalance < userCompounds) {
            IPancakeMaster(pancakeMaster).leaveStaking(userCompounds - curBalance);
        }

        // Sell user's compounded amount into BUSD
        lpToken.approve(stratSwap, userCompounds);
        uint256 usersCompUSD = IStrategySwap(stratSwap).swapLPToBusd(userCompounds, pancakeSwap, path0, lpToken);
    ...
    function transferOut(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        IStrategySwap(stratSwap).swapLPToBusd(fee, pancakeSwap, path0, lpToken);
        ...
        IStrategySwap(stratSwap).swapLPToBusd(_amount, pancakeSwap, path0, lpToken);
    ...
    function transferIn(address _user, uint256 _amount) public override onlyRole(STRAT_ROLE) returns (uint256) {
        ...
        IStrategySwap(stratSwap).swapBusdToLP(_amount, pancakeSwap, path0);
    ...
    function getUserDepositedUSD(address _user) public view override returns (uint256 amount) {
        ...
        return IStrategySwap(stratSwap).getBusdAmount(userDeposited[_user], path0, pancakeSwap);
    }
    ...
    function getTotalDepositedUSD() public view returns (uint256 amount) {
        ...
        return IStrategySwap(stratSwap).getBusdAmount(totalDeposited, path0, pancakeSwap);
    }
    ...
    function withdrawAcc(uint256 _amount) internal override {
        IPancakeMaster(pancakeMaster).leaveStaking(_amount);
    }
