...
interface IStrategy {
    ...
    // Transfer want tokens ChupaFarm -> strategy
    function deposit(uint256 _wantAmt) external returns (uint256);

    // Transfer want tokens strategy -> ChupaFarm
...
// File: contracts/ChupaToken.sol

pragma solidity 0.6.12;

// Chupa with Governance.
contract ChupaToken is BEP20 {
    ...
    /**
     * @notice Constructs the Chupa contract.
     */
    constructor() public BEP20("Chupa Token", "Chupa") {
...
// File: contracts/ChupaMaster.sol

pragma solidity 0.6.12;

contract ChupaMaster is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // @TODO: Add ChupaToken address to here
    address public constant earningToken = 0x6fA9ac40b8419914A8ebB311ba0d35600e91aeBc;
    ...
    function updatePool(uint256 _pid) public {
        ...
        ChupaToken(earningToken).mint(
            devaddr,
            multiplier.mul(EarningsDevPerBlock).mul(pool.allocPoint).div(totalAllocPoint)            
        );

        ChupaToken(earningToken).mint(address(this), earningsReward);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                ChupaToken(earningToken).mint(referrer, commissionAmount);
