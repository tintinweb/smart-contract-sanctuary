...
contract DCBVault is Ownable, Pausable {
    ...
    notContract
  {
    ...
    (, , , , , , uint256 min, uint256 max, , ) = masterchef.poolInfo(_pid);
    ...
    require(
      _amount >= min && _amount.add(user.totalInvested) <= max,
      "Invalid amount!"
    );
  ...
  function withdraw(uint256 _pid, uint256 _shares) public notContract {
    ...
    uint256 currentAmount = (balanceOf(_pid).mul(_shares)).div(
      pool.totalShares
    );

    uint256 totalReward = currentAmount -
      user.totalInvested.mul(_shares).div(user.shares);

    uint256 multipliedAmount = masterchef.handleNFTMultiplier(
      _pid,
      msg.sender,
      totalReward
    );

    user.totalInvested = user.totalInvested.sub(
      user.totalInvested.mul(_shares).div(user.shares)
    );
    user.shares = user.shares.sub(_shares);
    pool.totalShares = pool.totalShares.sub(_shares);
    user.totalClaimed = user.totalClaimed.add(totalReward).add(
      multipliedAmount
    ); //With NFT Boost
   ...
   */
  function calculateTotalPendingRewards(uint256 _pid)
    external
    view
    returns (uint256)
  {
   ...
   */
  function calculateHarvestDcbRewards(uint256 _pid)
    external
    view
    returns (uint256)
  {
  ...
  function getPricePerFullShare(uint256 _pid) public view returns (uint256) {
    ...
    return
      pool.totalShares == 0
        ? 1e18
        : balanceOf(_pid).mul(1e18).div(pool.totalShares);
  }

  /**
   * @notice Calculate total reward
   */
  function getRewardOfUser(address _user, uint256 _pid)
    external
    view
    returns (uint256)
  {
  ...
  function canUnstake(address _user, uint256 _pid) public view returns (bool) {
    ...
    return (block.timestamp >=
      user.lastDepositedTime.add(lockPeriod.mul(1 days)));
  }
