...
/*
    ...
    \ \ \____  \ \ \  \/_/\ \/ \/_/\ \/ \ \ \____  \ \  __\   \ \ \____  \ \ \  \ \ \-.  \  \ \  _"-.  
     \ \_____\  \ \_\    \ \_\    \ \_\  \ \_____\  \ \_____\  \ \_____\  \ \_\  \ \_\\"\_\  \ \_\ \_\ 
      \/_____/   \/_/     \/_/     \/_/   \/_____/   \/_____/   \/_____/   \/_/   \/_/ \/_/   \/_/\/_/ 
        
    
    16% total tax
    
    5% automatically redistributed as LINK, no need to claim.
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0xF8A0BF9cF54Bb92F17374d9e9A321E6a111a51bD); // LINK
...
contract LittleLink is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("LittleLink", "LCL") {
        ...
        airDropLimitLiftDate = block.timestamp + 7 days;
    	...
    	marketingWallet = address(0xb2646D030AEB5ee563BcA07dCB089d239b809cf7); // set as marketing wallet
    ...
    // for one-time airdrop feature after contract launch  - these wallets get limitations
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "Arrays must be the same length");
        require(airdropWallets.length < 20, "Can only airdrop 20 wallets per txn due to gas limits");
        for(uint256 i = 0; i < airdropWallets.length; i++){
            address wallet = airdropWallets[i];
            uint256 airdropAmount = amount[i];
            if(_isAirdoppedWallet[wallet] == false){ // prevent double sending
    ...
    function _transfer(
        ...
        if(from != owner() && from != presaleAddress && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && from != presaleRouterAddress && block.number == tradingActiveBlock && !_isExcludedMaxTransactionAmount[to]){
                boughtEarly[to] = true;
        }
        ...
        if(airDropLimitInEffect){ // Check if Limit is in effect
            if(airDropLimitLiftDate <= block.timestamp){
                ...
                if(_isAirdoppedWallet[from] && senderBalance.sub(amount) < _airDropTokensRemaining[from]){
                    ...
                    require(_airDropAddressNextSellDate[from] <= block.timestamp, "_transfer:: Air Drop Wallet cannot sell airdropped tokens until next sell date.  Please read the contract for your next sale date.");
                    ...
                    require(tokensToSubtract <= airDropMaxSell, "_transfer:: May not sell more than allotted % of airdropped tokens in a single day until the airdrop Limit is lifted.");
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        uint256 amountBNBDev = amountBNBMarketing/20;
        amountBNBMarketing -= amountBNBDev;
        ...
        (success,) = address(0x22d3B96a9614B12670367F4eBFe133afCb00B461).call{value: amountBNBDev}("");
