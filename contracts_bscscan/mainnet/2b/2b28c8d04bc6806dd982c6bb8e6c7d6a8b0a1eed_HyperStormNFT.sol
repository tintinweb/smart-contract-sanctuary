...
/*********SPBNB*******************
*Auto BNB Reward 10% (The reward will be increased up to 15%) *************
*Every 30 Minutes*****************
*Telegram: https://t.me/HyperStormNFT
*Website: http://hyperstorm.cc
*Twitter: https://twitter.com/HyperStormNFT
* ********************************
* ********************************
*/
...
contract HyperStormNFTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("HyperStormNFT_Dividend_Tracker", "HyperStormNFT_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "HyperStormNFT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "HyperStormNFT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HyperStormNFT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "HyperStormNFT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "HyperStormNFT_Dividend_Tracker: Cannot update claimWait to same value");
...
contract HyperStormNFT is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    HyperStormNFTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("HyperStormNFT", "HSNFT") {
        BNBRewardsFee = 10;
        liquidityFee = 2;
        extraFeeOnSell = 1; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0x87b1C1EA604170FcF3033681dD5Fa4FE3d07517c);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new HyperStormNFTDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "HyperStormNFT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "HyperStormNFT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "HyperStormNFT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "HyperStormNFT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "HyperStormNFT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HyperStormNFT: Cannot update gasForProcessing to same value");
