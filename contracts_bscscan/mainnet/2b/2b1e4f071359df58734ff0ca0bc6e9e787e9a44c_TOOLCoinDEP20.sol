...
contract sunafeMath {
    function sunafeAdd(uint256 CC, uint256 DEP) public pure returns (uint256 c1) {
        c1 = CC + DEP;
        require(c1 >= CC);
    }
    function sunafeSub(uint256 CC, uint256 DEP) public pure returns (uint256 c1) {
        require(DEP <= CC);
        c1 = CC - DEP;
    }
    function sunafeMul(uint256 CC, uint256 DEP) public pure returns (uint256 c1) {
        c1 = CC * DEP;
        require(c1 == 0 || c1 / CC == DEP);
    }
    function sunafeDiv(uint256 CC, uint256 DEP) public pure returns (uint256 c1) {
        require(DEP > 0);
        c1 = CC / DEP;
    }
}

contract TOOLCoinDEP20 is  DEP20Coin, sunafeMath {
    string public name = "Candy Finance";
    string public symbol = "CFN";
    ...
    function _transfer(address from, address to, uint256 tokens) private returns (bool success) {
        uint256 amountToBurn = sunafeDiv(tokens, 20); // 5% of the transaction shall be burned
