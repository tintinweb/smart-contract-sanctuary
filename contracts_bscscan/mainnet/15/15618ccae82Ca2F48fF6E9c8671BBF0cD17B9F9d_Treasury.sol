...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SHAZEL }

    address public immutable HAZEL;
    ...
    address public sHAZEL;
    uint public sHAZELQueue; // Delays change to sHAZEL address
    ...
    /**
        @notice allow approved address to deposit an asset for HAZEL
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint HAZEL needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( HAZEL ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn HAZEL for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IHAZELERC20( HAZEL ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHAZEL ).balanceOf( msg.sender ); // Can only borrow against sHAZEL held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with HAZEL
        @param _amount uint
     */
    function repayDebtWithHAZEL( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHAZELERC20( HAZEL ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, HAZEL, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( HAZEL ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( HAZEL ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns HAZEL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match HAZEL decimals
            value_ = _amount.mul( 10 ** IERC20( HAZEL ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SHAZEL ) { // 9
            sHAZELQueue = block.number.add( blocksNeededForQueue );
        } else return false;
