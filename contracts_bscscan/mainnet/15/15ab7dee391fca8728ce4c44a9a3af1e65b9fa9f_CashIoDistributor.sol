...
contract CashIoDistributor is Ownable {
    ...
    uint256 public distributePercentForDevelopment = 40;
    ...
    function setPrizePoolShare(uint256 percentForCashIoDividend, uint256 percentForCashIoJackpot, uint256 percentForNftHolders, uint256 percentForDevelopment) public onlyOwner {
        require(percentForCashIoDividend + percentForCashIoJackpot + percentForNftHolders + percentForDevelopment == 100);
    ...
    function distribute() public {
        ...
        if(amountDividend > 0)
            try IDividendDistributor(addressCashIoDividend).deposit{value: amountDividend}() {} catch {}

        if(amountJackpot > 0) 
            payable(addressCashIoJackpot).transfer(amountJackpot);

        if(amountNftHolders > 0)
            payable(addressNftHolders).transfer(amountNftHolders);
        
        if(amountDevelopment > 0)
            payable(addressDevelopment).transfer(amountDevelopment);
    }
}