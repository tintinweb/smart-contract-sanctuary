...
/* 
    $CAKECHIPS -- BIG AND NEW rewards token that GIVES you $CAKE with 10% dividend for every transaction!
    
    https://t.me/CAKECHIPS
...
contract CAKECHIPS is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   CAKECHIPSDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("CAKECHIPS", "CAKECHIPS") {
        //Reflection Rewards %
        _ETHRewardsFee = 10;
        //Liqudity Fee %
        _liquidityFee = 3;
        ...
        maxBuyTranscationAmount = 5000000000 * (10**18); //Unlimted
        maxSellTransactionAmount = 100000000000 * (10**18); //Max Sell 1%
        swapTokensAtAmount = 400000000 * (10**18); // 0.4% of the supply in order to get rewards
        _maxWalletToken = 5000000000 * (10**18); // 3% of total supply


    	dividendTracker = new CAKECHIPSDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
        //Marketing Wallet Address
		marketingWallet = payable(0x6Ef1AaF44Bc5444Ce7D2f9ebC9E6E7b278bE4728);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CAKECHIPS: The dividend tracker already has that address");

        CAKECHIPSDividendTracker newDividendTracker = CAKECHIPSDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CAKECHIPS: The new dividend tracker must be owned by the CAKECHIPS token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CAKECHIPS: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CAKECHIPS: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CAKECHIPS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CAKECHIPS: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CAKECHIPS: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CAKECHIPS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CAKECHIPS: Cannot update gasForProcessing to same value");
    ...
    function enableTrade() external onlyOwner() {
        ...
        maxSellTransactionAmount = 100000000000;
    }
    ...
    function restoreAllFee() external onlyOwner() {
        ...
        _maxWalletToken = 4000000000 * 10**18;
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CAKECHIPS: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CAKECHIPS: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "CAKECHIPS: ETH Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract CAKECHIPSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CAKECHIPS_Dividend_Tracker", "CAKECHIPS_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "CAKECHIPS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CAKECHIPS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CAKECHIPS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CAKECHIPS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CAKECHIPS_Dividend_Tracker: Cannot update claimWait to same value");
