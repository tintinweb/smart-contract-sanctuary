...
/**
       ...
       Welcome to #BLADECAKE - BLAKE 
       
      Shiba,Rfi,Feg,CT,Safemoon,Pig combine together to create #BLAKE designed.
       ...
       I will renounce the ownership to burn addresses to transfer #BLAKE to the community, make sure it's 100% safe.
    
       I will add 1 BNB and all the left 100% total supply to the pool
       Can you make #BLAKE 10000X? 
       ...
       it's the best part of the #BLAKE idea, increasing the liquidity pool automatically, 
...
// File: contracts/BladeCAKEDividendPayingToken.sol
...
contract BladeCAKE is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BladeCAKEDividendTracker public dividendTracker;

    address public liquidityWallet;
    address private marketingWallet = 0xFc96DEe6774f677aB7ac63536600366Dc26E3EFA;
    ...
    constructor() public ERC20("BladeCAKE", "BLAKE") {
    	...
    	dividendTracker = new BladeCAKEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BladeCAKE: The dividend tracker already has that address");

        BladeCAKEDividendTracker newDividendTracker = BladeCAKEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BladeCAKE: The new dividend tracker must be owned by the BladeCAKE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BladeCAKE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BladeCAKE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BladeCAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BladeCAKE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BladeCAKE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BladeCAKE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BladeCAKE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BladeCAKE: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of BladeCAKE
            path,
            recipient,
            block.timestamp
        );
...
contract BladeCAKEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BladeCAKE_Dividend_Tracker", "BladeCAKE_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BladeCAKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BladeCAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BladeCAKE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BladeCAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BladeCAKE_Dividend_Tracker: Cannot update claimWait to same value");
