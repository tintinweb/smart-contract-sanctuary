...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also BABYLOKIed.
...
// File: contracts/BABYLOKIDividendPayingToken.sol
...
contract BABYLOKI is ERC20, Ownable {
    using SafeMath for uint256;
	...
	address payable public MarketingWallet = payable(0xe5336147Ec092cb045D3CDE367E04508999d5236); // Marketing Address

    bool private swapping;

    BABYLOKIDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 private _maxSellAmount = 10000000000 * (10**18); 
    ...
    constructor() ERC20("Baby LOKI", " Baby LOKI ") {
        ...
        dividendTracker = new BABYLOKIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BABYLOKI: The dividend tracker already has that address");

        BABYLOKIDividendTracker newDividendTracker = BABYLOKIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BABYLOKI: The new dividend tracker must be owned by the BABYLOKI token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BABYLOKI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "BABYLOKI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BABYLOKI: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BABYLOKI: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BABYLOKI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BABYLOKI: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "BABYLOKI: This account cannot send tokens until trading is enabled");
        }
...
contract BABYLOKIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BABYLOKI_Dividend_Tracker", "BABYLOKI_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "BABYLOKI_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "BABYLOKI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BABYLOKI_Dividend_Tracker: Cannot update claimWait to same value");
