...
contract DTFMarketForETH is Ownable, Pausable {
    ...
    {
        Order storage order = sellOrderMap[_orderId];
        etf.safeTransfer(msg.sender, order.amount.sub(order.filled));

        totalSupply = totalSupply.sub(order.amount.sub(order.filled), "exceeded totaly supply");
        delete sellOrderMap[_orderId];
        sellOrders.remove(_orderId);
    }
    ...
    {
        Order storage order = buyOrderMap[_orderId];
        uint256 amount = order.price.mul(order.amount.sub(order.filled)).div(1e18);
