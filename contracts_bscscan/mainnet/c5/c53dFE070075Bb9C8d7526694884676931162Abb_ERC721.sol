...
contract ERC721 is Context ,ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    //system address
    address public systemAddress = 0x5471793B921D534974E81E80552f352A9b9170F2;
    ...
    mapping (string =>  uint256[]) public presaleTokens;
    ...
    uint256 private rateCount = 1;
    uint256 private tokensCounter = 1;


    constructor(){
       addToAdminPool(_msgSender());
       addToAdminPool(systemAddress);
    }
    ...
    function buyInpresale(string memory category) public  payable  {
        
        uint256 tokenId = getLastTokenInPresaleByCategory(category);
        ...
        deleteLastTokenInPresaleByCategory(category); 

        emit CloseAuction(auction.tokenId,_msgSender(),msg.value,auction.id);
    }
    
    function buyInpresaleSeveralsTokens(string memory category,uint maxAmount) public  payable  {
        require(maxAmount <= 9 && maxAmount > 0,'Max buy tokens amount error');
        
        uint256[] memory tokens = presaleTokens[category];
        uint buyTokensCount = 0;
        uint256 balance = msg.value;
        
        for (uint256 i = 0; i < tokens.length; i++) {
            if(tokens[i] > 0 && buyTokensCount < maxAmount){
                uint256 tokenId = getLastTokenInPresaleByCategory(category);
                Auction memory auction = auctionOfToken[tokenId];
                if(i == 0) {
                    require(balance >= auction.minBidPrice,'Wrong balance');
                }
                if(balance >= auction.minBidPrice) {
                    address payable owner = payable(address(uint160(ERC721.ownerOf(auction.tokenId))));
         
                    //Transfer token to new owner
                    owner.transfer(auction.minBidPrice);
                    _safeTransfer(owner, _msgSender(), auction.tokenId, '');
                    ...
                    buyTokensCount += 1;
                    balance -= auction.minBidPrice; 
                    emit CloseAuction(auction.tokenId,_msgSender(),auction.minBidPrice,auction.id);
                } else {
                    return;
                }
            }
    ...
    function getPriceLastTokenInPresale(string memory category) public  view returns(uint256)  {
    ...
    function createTokenAndSimpleAuction(uint256 minBidPrice,string memory category) onlyAdmin() public  {
        uint256 tokenId = createToken(category);
        createSimpleAuction(tokenId,minBidPrice,0x0000000000000000000000000000000000000000);
        presaleTokens[category].push(tokenId);
    }
    ...
    function createRate(uint256 tokenId,uint256 tokensAmount) public  payable  {
        ...
        require(ERC721.ownerOf(tokenId) != systemAddress, "It system locked token");
        ...
        if(isBnbTransaction) tokensAmount = msg.value;
            ...
            uint256 ownerPercent = 100 - systemPercent;
    ...
    function getLastTokenInPresaleByCategory(string memory category) public view returns (uint256) {
        uint256[] memory tokens = presaleTokens[category];
        for (uint256 i = 0; i < tokens.length; i++) {
            if(tokens[i] > 0){
                return tokens[i];
        ...
        revert('Tokens not found');
    }
    
    function deleteLastTokenInPresaleByCategory(string memory category) private {
        uint256[] memory tokens = presaleTokens[category];
        for (uint256 i = 0; i < tokens.length; i++) {
            if(tokens[i] > 0){
                delete presaleTokens[category][i];
                return;
            }
        }
        revert('Tokens not found');
    ...
    // Create tokens
    function createToken(string memory category) onlyAdmin()  public returns(uint256)  {
        uint256 tokenId = tokensCounter;
        _safeMint(_msgSender(), tokenId);

        categoryOfToken[tokenId] = category;
    ...
    // Token name
    string private _name='CryptoSaints';

    // Token symbol
    string private _symbol='CSTNFT';
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return "https://nft.cryptosaints.com/api/token/get_token_info/";
    }
