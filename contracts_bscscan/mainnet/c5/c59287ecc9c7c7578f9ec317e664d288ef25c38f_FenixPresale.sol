...
contract FenixPresale is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    IRouter constant internal router = IRouter(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    ...
    uint256 public immutable liquidityPct;
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "FenixPresale: sale is not open");
        require(!hasEnded(), "FenixPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "FenixPresale: min contribution criteria not met"
        );
        require(
    ...
    function claimTokens() external {
        require(hasEnded(), "FenixPresale: presale is not over");
        require(softCapReached(), "FenixPresale: soft cap not reached, refund is available");
        require(finalized, "FenixPresale: not yet finalized, wait for liquidity");
        require(
            contributions[msg.sender] > 0,
            "FenixPresale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "FenixPresale: presale is not over");
        require(!softCapReached(), "FenixPresale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "FenixPresale: nothing to claim"
        );
    ...
    function endPresale() external onlyOwner {
        require(!finalized, "FenixPresale: presale is already over");
        finalized = true;
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "FenixPresale: presale is not over");
        // uint256 unsoldTokens = _getTokenAmount(hardCap.sub(weiRaised));
        uint256 tokens = token.balanceOf(address(this));
        token.transfer(owner(), tokens);
    }

    function endPresaleAddLiquidity() public onlyOwner {
        require(!finalized, "FenixPresale: presale is already over");
    ...
    function _addLiquidity() internal {
        uint256 totalWeiRaised = address(this).balance;    
        uint256 totalWeiLiquidity = totalWeiRaised.mul(liquidityPct).div(10000);
        uint256 tokensForPancake = _getTokenAmount(totalWeiLiquidity);

        token.approve(address(router), tokensForPancake);
        router.addLiquidityETH
        { value: totalWeiLiquidity }
        (
            address(token),
            tokensForPancake,
            tokensForPancake,
