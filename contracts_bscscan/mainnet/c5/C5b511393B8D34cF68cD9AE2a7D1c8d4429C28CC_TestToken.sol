...
contract TestToken is ERC20, Ownable {
    ...
    address public marketingWallet = 0x8B3a9bC8E990847c6050326a80b122D4b3803f32;
    address public gameWallet = 0x0d0e630553d65241E628C0935a1f6D9Fe60CE965;
    ...
    constructor() public ERC20("TEST TOKEN 9", "TST9") {
    ...
    function _transfer(
        ...
        if (((tradingEnabledTimestamp + 10 minutes >= block.timestamp) || tradingEnabledTimestamp == 0) && from == uniswapV2Pair && !_isExcludedFromFees[to]){
            require(false, "Buy blocked after 10 min launch on pcs!");  // anti snip
        }  


        if (!_isExcludedFromFees[from]){
            if(_sellLockTimeRefresh[from] < block.timestamp){
                _sellLockTimeRefresh[from] = block.timestamp + 10 minutes;
...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyCEO {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
