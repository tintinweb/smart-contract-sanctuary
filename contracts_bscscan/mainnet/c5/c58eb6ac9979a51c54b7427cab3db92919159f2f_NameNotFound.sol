...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all NameNotFound just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also NameNotFounded.
	...
	// File: contracts/NameNotFoundBEP20PayingToken.sol
	...
	// File: contracts/NameNotFound.sol

	// 

	pragma solidity ^0.8.0;

	contract NameNotFound is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingWallet = payable(0x48426dE4C36d624b2769f9C6b33f9c21048a62A1); 
		...
		uint256 public antiSniperPercentage = 0;
		uint256 public antiDumpPercentage = 0;
		...
		event excludeMultipleAccountFromRewards(address[] NameNotFound, bool isExcluded);
		...
		constructor() ERC20("Name Not Found", "404"){
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "NameNotFound: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "NameNotFound: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "NameNotFound: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "NameNotFound: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "NameNotFound: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "NameNotFound: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "NameNotFound: Cannot update distributorGas to same value");
        ...
        function changeAntiDumpPercentage (uint256 percentage) external onlyOwner {
            antiDumpPercentage = percentage;
	
        }
		function changeAntiSniperPercentage (uint256 percentage) external onlyOwner {
            antiSniperPercentage = percentage;
	

        }
		function setAntiSniper (bool _enabled) external onlyOwner {
           antiSniper = _enabled;
		
		}
		
		function setAntiDump (bool _enabled) external onlyOwner {
           antiDump = _enabled;
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if (antiSniper)
			if (block.number <= launchBlock + antiSniperPercentage && tx.gasprice <= gweiLimit) {
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "NameNotFound: This account cannot send tokens until trading is enabled");
			}
			...
			if(takeFee) {
				...
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(antiDumpPercentage).div(100);
				}
