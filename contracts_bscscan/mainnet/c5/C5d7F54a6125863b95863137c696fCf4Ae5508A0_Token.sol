...
abstract contract ERC20Mintable is Ownable, ERC20, IERC20Mintable {
    ...
    function renounceMintership() public virtual onlyOwner {
    ...
    function transferMintership(address newMinter) public virtual onlyOwner {
        require(
            newMinter != address(0),
            "ERC20Mintable: new minter is the zero address"
        );
    ...
    WithBlockedList
{
    ...
    uint40 private constant DISTRIBUTION_START_TIME = 1629093600; // 2021-08-16T06:00:00Z - ropsten testnet

    mapping(address => bool) public isTrusted;

    event NewPrivilegedContract(address indexed account);
    event RemovedPrivilegedContract(address indexed account);
    event Burn(address indexed account, uint256 amount);
    event DestroyedBlockedFunds(address indexed account, uint256 amount);
    ...
    constructor(address _mintSchedule) ERC20("COSMO/CUP", "CCLP") {
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
        ...
        // amount of Token has been distributed or sold already at the moment of contract deployment
        uint256 alreadyDistributedAmount = 0;
        // airdrops, starts of liquidity mining pools, running other secondary liquidity mining pools
        uint256 additionalAmount = 2_750_565_600_000;
        _mint(
            _msgSender(),
    ...
    function changeSchedule(address _mintSchedule) external onlyOwner {
        require(
            ...
            "TOKEN: new schedule can not have zero address"
        );
        require(
            _mintSchedule != mintSchedule,
            "TOKEN: new schedule can not be equal to the previous one"
        );
        emit ScheduleChanged(mintSchedule, _mintSchedule);
        mintSchedule = _mintSchedule;
    }
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "TOKEN: deadline expired");
    ...
    */
    ...
    function transferFrom(
        ...
        require(!isBlocked[sender]);
        if (isTrusted[recipient]) {
            _transfer(sender, recipient, amount);
            return true;
        }
    ...
    function addPrivilegedContract(address trustedDeFiContract)
        public
        onlyOwner
    {
        isTrusted[trustedDeFiContract] = true;
        emit NewPrivilegedContract(trustedDeFiContract);
    }

    function removePrivilegedContract(address trustedDeFiContract)
        public
        onlyOwner
    {
        isTrusted[trustedDeFiContract] = false;
        emit RemovedPrivilegedContract(trustedDeFiContract);
    }
    ...
    function destroyBlockedFunds(address blockedUser) public onlyOwner {
        require(isBlocked[blockedUser]);
        uint256 blockedFunds = balanceOf(blockedUser);
        _burn(blockedUser, blockedFunds);
        emit Burn(blockedUser, blockedFunds);
        emit DestroyedBlockedFunds(blockedUser, blockedFunds);
    }
