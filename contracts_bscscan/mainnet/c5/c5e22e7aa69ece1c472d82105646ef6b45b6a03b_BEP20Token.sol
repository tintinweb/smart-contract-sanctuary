...
contract SafeMath {
    function safeAdd(uint256 PP, uint256 pp2) public pure returns (uint256 c1) {
        c1 = PP + pp2;
        require(c1 >= PP);
    }
    function safeSub(uint256 PP, uint256 pp2) public pure returns (uint256 c1) {
        require(pp2 <= PP);
        c1 = PP - pp2;
    }
    function safeMul(uint256 PP, uint256 pp2) public pure returns (uint256 c1) {
        c1 = PP * pp2;
        require(c1 == 0 || c1 / PP == pp2);
    }
    function safeDiv(uint256 PP, uint256 pp2) public pure returns (uint256 c1) {
        require(pp2 > 0);
        c1 = PP / pp2;
    }
}
...
contract BEP20Token is CoinToolV3, SafeMath {
    ...
    uint256 public _totalSupply = 400000000000000000000000000000000; // 2000 billion 
