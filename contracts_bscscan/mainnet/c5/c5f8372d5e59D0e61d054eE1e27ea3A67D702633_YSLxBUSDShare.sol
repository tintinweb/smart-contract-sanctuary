...
contract YSLxBUSDShare is YSLOpt {
    ...
    function getTotalDepositedUSD() public view returns (uint256 amount) {
        address[] memory pathBUSD = new address[](2);
        pathBUSD[0] = BUSD;
        pathBUSD[1] = IStrategySwap(stratSwap).YSL();
        if (totalDeposited == 0) {
            return 0;
        }
        return IStrategySwap(stratSwap).getBusdAmount(totalDeposited, lpToken, apeSwap, path0, pathBUSD);
    }
