...
import "./RegistrationLibrary.sol";

contract LevelManager is Adminable, ILevelManager, WithLevels, WithPools {
    using RegistrationLibrary for RegistrationLibrary.Registration;
    ...
    mapping(address => RegistrationLibrary.Registration) private userState;
    ...
    // When user locks a staking pool
    event Locked(address indexed account, address pool, uint256 time);
        ...
        returns (bool)
    {
        ...
        uint256 unlocksAt = getUserUnlocksAt(pool, account);
        return unlocksAt > 0 && unlocksAt > block.timestamp;
    }

    function getLockPeriod(address pool)
        public
        view
        override
        returns (uint256)
    {
        return poolLockPeriod[pool];
    }
    ...
    {
        return
            userState[account].getUnlocksAt(
                pool,
                getLockPeriod(pool),
                lockEnabled
            );
    ...
    {
        return userState[account].getRegistrations();
    }
    ...
    function getUserLocks(address account)
        ...
        returns (address[] memory, uint256[] memory)
    {
        return userState[account].getLocks(pools);
    }
    ...
    {
        return userState[account].getLatestRegistration();
    }
    ...
    function getUserAmount(address account) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            if (poolEnabled[poolAddr]) {
                totalAmount +=
                    (pool.getLockedAmount(account) * multiplier) /
                    DEFAULT_MULTIPLIER;
            }
        }
    ...
    function internalLock(
        ...
        address[] memory lockedPools = userState[account].registerAndLock(
            saleAddress,
            registeredAt,
            pools,
            poolLockPeriod,
        ...
        for (uint256 i; i < lockedPools.length; i++) {
            if (lockedPools[i] != address(0)) {
                emit Locked(account, lockedPools[i], registeredAt);
            }
        }
    }

    function internalUnlock(address account, address saleAddress) internal {
        userState[account].unregisterAndUnlock(saleAddress, pools);
    ...
    function batchLock(
        ...
        uint256[] calldata lockedAt
    ) external onlyOwnerOrAdmin {
        for (uint256 i = 0; i < addresses.length; i++) {
            userState[addresses[i]].locks[pools[i]] = lockedAt[i];
        }
    }

    function batchUnlock(address[] calldata addresses, address[] calldata pools)
        external
        onlyOwnerOrAdmin
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            for (uint256 j = 0; j < pools.length; j++) {
                userState[addresses[i]].locks[pools[j]] = 0;
            }
        }
    }
}
...
library RegistrationLibrary {
    using EnumerableSet for EnumerableSet.AddressSet;

    struct Registration {
        mapping(address => uint256) registeredAt;
        EnumerableSet.AddressSet sales;
        // pool -> lock date. Once current time is past the lockedAt + lockPeriod, we refresh lockedAt
        mapping(address => uint256) locks;
    }

    function getUnlocksAt(
        Registration storage self,
        address pool,
        uint256 lockPeriod,
        bool lockEnabled
    ) public view returns (uint256) {
        uint256 lockedAt = self.locks[pool];
        if (!lockEnabled || lockedAt == 0) {
            return 0;
        }

        return lockedAt + lockPeriod;
    }

    function getRegistrations(Registration storage self)
        public
        view
        returns (address[] memory, uint256[] memory)
    {
        uint256 len = self.sales.length();
        address[] memory sales = new address[](len);
        uint256[] memory times = new uint256[](len);
        for (uint256 i; i < len; i++) {
            sales[i] = self.sales.at(i);
            times[i] = self.registeredAt[sales[i]];
        }

        return (sales, times);
    }

    function getLocks(Registration storage self, address[] storage pools)
        public
        view
        returns (address[] memory, uint256[] memory)
    {
        ...
        uint256[] memory times = new uint256[](len);
        for (uint256 i; i < len; i++) {
            addr[i] = address(pools[i]);
            times[i] = self.locks[addr[i]];
        }

        return (addr, times);
    }

    function getLatestRegistration(Registration storage self)
        public
        view
        returns (address, uint256)
    {
        ...
        uint256 time = 0;
        for (uint256 i; i < self.sales.length(); i++) {
            address s = self.sales.at(i);
            uint256 t = self.registeredAt[s];
            if (t > time) {
                time = t;
                sale = s;
            }
    ...
    function registerAndLock(
        Registration storage self,
        address saleAddress,
        uint256 registeredAt,
        address[] storage pools,
        mapping(address => uint256) storage poolLockPeriod,
        mapping(address => bool) storage poolEnabled
    ) public returns (address[] memory) {
        require(
            self.registeredAt[saleAddress] == 0,
            "LevelManager: User is already registered"
        );

        self.sales.add(saleAddress);
        self.registeredAt[saleAddress] = registeredAt;

        address[] memory lockedPools = new address[](pools.length);
        for (uint256 i; i < pools.length; i++) {
            address pool = address(pools[i]);
            if (!poolEnabled[pool] || poolLockPeriod[pool] == 0) {
                continue;
            }
            bool isLocked = self.locks[pool] > 0 &&
                self.locks[pool] + poolLockPeriod[pool] > block.timestamp;
            if (isLocked) {
                continue;
            }
            self.locks[pool] = registeredAt;
    ...
    function unregisterAndUnlock(
        ...
        for (uint256 i; i < pools.length; i++) {
            ...
            // If no registrations left, unlock user
            if (latestRegTime == 0) {
                self.locks[pool] = 0;
                continue;
            }
            if (latestRegTime < lockedAt) {
                // If latest registration happens before the lock time now,
                self.locks[pool] = latestRegTime;
            }
        }
    }
}
...
abstract contract WithPools is Adminable, ILevelManager {
    ...
    function addPool(
        ...
        setPoolLockPeriod(pool, lockPeriod);
    }
    ...
    function setPoolLockPeriod(address pool, uint256 lockPeriod)
        public
        onlyOwnerOrAdmin
    {
        require(poolExists(pool), "LevelManager: Pool does not exist");
        poolLockPeriod[pool] = lockPeriod;
        emit PoolLockPeriodSet(pool, lockPeriod);
    }
