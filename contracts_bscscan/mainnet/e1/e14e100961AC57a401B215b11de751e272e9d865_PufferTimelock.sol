...
/**
 * @dev IPufferFarm functions that do not require less than the min timelock
 */
interface IPufferFarm {
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
 ...
 * to position this {PufferTimelock} as the owner of a smart contract, with
 * a multisig or a DAO as the sole proposer.
 *
 * _Available since v3.3._
 */
contract PufferTimelock is AccessControl, ReentrancyGuard {
    ...
    modifier onlyRole(bytes32 role) {
        require(
            ...
            "PufferTimelock: sender requires permission"
        );
        _;
    }
    ...
    function scheduleBatch(
        ...
        require(
            ...
            "PufferTimelock: length mismatch"
        );
        require(
    ...
    function _schedule(bytes32 id, uint256 delay) private {
        require(
            _timestamps[id] == 0,
            "PufferTimelock: operation already scheduled"
        );
        require(delay >= minDelay, "PufferTimelock: insufficient delay");
        // solhint-disable-next-line not-rely-on-time
    ...
    function cancel(bytes32 id) public virtual onlyRole(PROPOSER_ROLE) {
        require(
            ...
            "PufferTimelock: operation cannot be cancelled"
        );
    ...
    function _beforeCall(bytes32 predecessor) private view {
        require(
            ...
            "PufferTimelock: missing dependency"
        );
    }
    ...
    function _afterCall(bytes32 id) private {
        require(
            isOperationReady(id),
            "PufferTimelock: operation is not ready"
        );
        _timestamps[id] = _DONE_TIMESTAMP;
    }
    ...
    function _call(
        ...
        require(success, "PufferTimelock: underlying transaction reverted");
    ...
    function updateMinDelay(uint256 newDelay) external virtual {
        require(
            ...
            "PufferTimelock: caller must be timelock"
        );
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        IPufferFarm(_pufferFarmAddress).set(_pid, _allocPoint, _depositFeeBP, _withUpdate);
        _afterCall(id);
    }
    ...
    function add(
        ...
        IPufferFarm(_pufferFarmAddress).add(0, _lpToken, _depositFeeBP, _withUpdate, _strat); // allocPoint = 0. Schedule set (timelocked) to increase allocPoint.
    }
