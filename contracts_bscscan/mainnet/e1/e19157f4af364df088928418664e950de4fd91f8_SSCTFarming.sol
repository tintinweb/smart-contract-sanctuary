...
contract SSCTFarming is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            if (canWithdraw(msg.sender)) {
                ...
                uint256 feeAmount = 0;
                if (withdrawLockMethod == 2 && withdrawFeeMethod != 0 && withdrawFee > 0 && block.timestamp < user.nextWithdrawUntil) {
                    feeAmount = _amount.mul(withdrawFee).div(10000);
                    if (feeAmount > 0) {
                        if (withdrawFeeMethod == 1) {
                            ...
                            if (half > 0 && otherHalf > 0) {
                                if (isBnbPairToken) {
                                    ...
                                    uint256 initialPairBalance = pairToken.balanceOf(address(this));
    ...
    function payOrLockupPendingReward() internal {
        ...
        if (canHarvest(msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                _safeRewardTransfer(address(msg.sender), totalRewards);
            }
        } else if (pending > 0) {
