...
Telegaram : https://t.me/flokifenix
...
contract FLOKIFENIX is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("FLOKIFENIX", "FLOKIFENIX") {
    ...
    function tradeStart() external onlyOwner {
        ...
        maxBuyTranscationAmount = 200000000000 * (10**18);
        maxSellTransactionAmount = 100000000000 * (10**18);
        maxWalletToken = 200000000000 * (10**18);
    }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "FLOKIFENIX: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "FLOKIFENIX: The new dividend tracker must be owned by the FLOKIFENIX token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("FLOKIFENIX_Dividend_Tracker", "FLOKIFENIX_Dividend_Tracker", 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "FLOKIFENIX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "FLOKIFENIX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "FLOKIFENIX_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "FLOKIFENIX_Dividend_Tracker: Cannot update claimWait to same value");
