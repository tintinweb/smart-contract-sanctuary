...
contract MasterChefPool is BasePool {
                ...
                priceProvider)
    {
        ...
        require(assetToken.approve(address(masterChef), MAX_INT));
    }
    ...
    function _stakeAsset(address celebrity, uint256 assetStakeAmount, StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo) internal virtual override{
        if(assetStakeAmount  > 0)
            require(_assetToken.transferFrom(msg.sender, address(this), assetStakeAmount), '_stakeAsset: fail transfer asset');
    ...
    function _celebrityClaimAsset(address celebrity, CelebrityInfo memory celebrityInfo)  internal virtual override {
        ...
        if (claimAmount.claimAmount > 0)
        {
            ...
            if(systemReward > 0)
                require(_assetToken.transfer(_platformFeeAddress, systemReward), '_celebrityClaimAsset: fail transfer platform fee');
            if(claimAmount.claimAmount > systemReward)
                require(_assetToken.transfer(celebrity, claimAmount.claimAmount - systemReward), '_celebrityClaimAsset: fail transfer celebrity reward');
    ...
    function _prepareBalance(uint256 amount) private {
        ...
        if(amount < pending)
            amount = 0; // If amount is less than pending reward then just take the reward
        else
    ...
    function _unstakeAsset(address celebrity, StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo, uint256 amount) internal virtual override {
        ...
        if(amount > 0)
            require(_assetToken.transfer(msg.sender, amount), '_unstakeAsset: fail transfer asset');
        //-------------cake unstake--------------
...
contract Vault {
    ...
    constructor(address token){
        require(IBEP20(token).approve(msg.sender, MAX_INT), 'Vault:constructor: fail approve');
    }
}
...
abstract contract BasePool is Ownable {
    ...
    modifier nonReentrant() {
        require(!rentrancy_lock, 'nonReentrant');
        rentrancy_lock = true;
        _;
    ...
    constructor(
        ...
        require(address(assetToken) != address(0), 'constructor:assetToken address cannot be zero');
        require(address(egoToken) != address(0), 'constructor:egoToken address cannot be zero');
        require(platformFeeAddress != address(0), 'constructor:platformFeeAddress address cannot be zero');
        require(initialVEGORate != 0, 'constructor:initialVEGORate cannot be zero');
        require(decimalAdjustment != 0, 'constructor:decimalAdjustment cannot be zero');
        require(address(assetPriceProvider) != address(0), 'constructor:assetPriceProvider address cannot be zero');
    ...
    function setAssetPriceProvider(IPriceProvider assetPriceProvider) external onlyOwner {
        require(address(assetPriceProvider) != address(0), 'setAssetPriceProvider: assetPriceProvider address cannot be zero');
        AssetPriceProvider = assetPriceProvider;
    }
    
    // Sets decimal adjustment variable, e.g. 18 - <asset token decimals>
    function setDecimalAdjustment(uint256 decimalAdjustment) public onlyOwner {
        require(decimalAdjustment != 0, 'setDecimalAdjustment:decimalAdjustment cannot be zero');
    ...
    function stake(
        ...
        require(!Lock, 'stake: Locked');
        require(assetStakeAmount!=0, 'stake: assetStakeAmount cannot be zero');
        require(celebrity != address(0), 'stake:celebrity address cannot be zero');
        ...
        uint256 vEgoAdded = egoAmount * DEFAULT_BASE / CurrentRate;
        //ego
    ...
    function stakerClaim(address owner, address celebrity) nonReentrant external {
        _stakerClaim(owner, celebrity);
    }

    // Staker claims EGO and Asset reward
    // Asset is split between the Celebrity and Platform fee
    function celebrityClaim(address celebrity) nonReentrant public {
        
        require(msg.sender == celebrity, 'celebrityClaim: celebrity not sender');
        ...
        if (egoClaim.claimAmount > 0) {
            ...
            if(egoClaim.claimAmount  > 0)
                if(_egoToken.balanceOf(address(RewardVault))>  egoClaim.claimAmount)
                    require(_egoToken.transferFrom(
                        address(RewardVault),
                        celebrity,
                        egoClaim.claimAmount
                    ), 'celebrityClaim: fail transfer ego');
    ...
    function unstake(address celebrity, uint256 assetStakeAmount) nonReentrant public {
        require(assetStakeAmount!=0, 'unstake: assetStakeAmount cannot be zero');
        ...
        // update rate inside
        _stakerClaim(msg.sender, celebrity);
    ...
    function _stakerClaim(address owner, address celebrity) internal{
        require(msg.sender == owner, 'stakerClaim: sender not owner');
    ...
    function setPlatformFee(uint256 fee) external onlyOwner{
        require(fee<=DEFAULT_BASE, 'setPlatformFee: fee shouold be less 100%');
        PLATFORM_FEE = fee;
    }
    ...
    function setPlatformFeeAddress(address platformFeeAddress) external onlyOwner{
        require(platformFeeAddress != address(0), 'platformFeeAddress should not be 0');
        _platformFeeAddress = platformFeeAddress;
    }

    // Withdraw funds from the Reward Vault
    function withdrawalRewards(uint256 amount) external onlyOwner {
        require(_egoToken.transferFrom(
                address(RewardVault),
                address(owner()),
                amount
            ), 'withdrawalRewards: fail transfer ego');
        emit WithdrawalRewards();
    }
}
