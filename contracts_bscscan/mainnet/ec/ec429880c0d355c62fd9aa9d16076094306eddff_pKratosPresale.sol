...
// File: contracts/pKratosPresale.sol

pragma solidity 0.6.12;



contract pKratosPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale pKratos, 5 DAI
	uint256 DAIPerToken = 5;
	// Amount of DAI received in presale
	uint256 daiReceived = 0;

    uint256 public minimumDepositEthAmount = 1 * 10 ** 18;
    ...
    function approveBuyers( address[] calldata newBuyers_ ) external onlyOwner() returns ( uint256 ) {
        require(newBuyers_.length <= 333,"too many addresses");
	...
	function buyPKratos(uint256 _amount) public nonReentrant {
        ...
        require(_amount + (tokensOwned[msg.sender] * DAIPerToken) <= maximumDepositEthAmount, "high amount than max");
	...
	function getPKratosTokensLeft() external view returns (uint256) {
		return pKratos.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some pKratos tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed pKratos tokens");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
        IpKratos( address(pKratos) ).mint( msg.sender, callerTokensUnclaimed );
