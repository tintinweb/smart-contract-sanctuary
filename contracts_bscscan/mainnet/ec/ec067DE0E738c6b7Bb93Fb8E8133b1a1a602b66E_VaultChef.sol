...
interface IStrategy {
    ...
    function tvlOnMC() external view returns (uint256);
...
contract VaultChef is Ownable, ReentrancyGuard {
    ...
    function lastEarnBlock(uint256 _pid) public view returns (uint256) {
    ...
    function tvlOnMC(uint256 _pid) public view returns (uint256) {
        return IStrategy(poolInfo[_pid].strat).tvlOnMC();
    }
    ...
    function originAPR(uint256 _pid) external view returns (uint256){
        uint256 blocks = lastEarnBlock(_pid);
    ...
    function getTVL(uint256 _pid) view external returns(uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        return pool.want.balanceOf(pool.strat);
    }
