...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function distributeDividendsBUSD(uint256 amount) public override onlyOwner {
        require(totalSupply() > 0);

        if (amount > 0) {
            magnifiedDividendPerShare.BUSD = magnifiedDividendPerShare.BUSD.add(
                (amount).mul(magnitude) / totalSupply()
            );
            emit DividendsDistributed(msg.sender, amount, "BUSD");

            _totalDividendsDistributed.BUSD = _totalDividendsDistributed.BUSD.add(amount);
        }
    }
    ...
    function distributeDividends() public payable override {
        ...
        if (msg.value > 0) {
            ...
            emit DividendsDistributed(msg.sender, msg.value, "BNB");

            _totalDividendsDistributed.BNB = _totalDividendsDistributed.BNB.add(msg.value);
        }
    }
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (Tokens memory withdrawable) {
        ...
        if (withdrawable.BNB > 0 ) {
            ...
            (bool success,) = user.call{value: withdrawable.BNB}("");
            if(!success) {
        ...
        if (withdrawable.BUSD > 0 ) {
            withdrawnDividends[user].BUSD = withdrawnDividends[user].BUSD.add(withdrawable.BUSD);
            emit DividendWithdrawn(user, withdrawable.BUSD, "BUSD");

            bool success = IERC20(BUSD).transfer(user, withdrawable.BUSD);
            if(!success) {
                withdrawnDividends[user].BUSD = withdrawnDividends[user].BUSD.sub(withdrawable.BUSD);
                withdrawable.BUSD = 0;
            }
        }
    }
    ...
    function withdrawableDividendOf(address _owner) public view override returns(Tokens memory acc) {
        ...
        acc.BUSD -= withdrawnDividends[_owner].BUSD;
    }
    ...
    function accumulativeDividendOf(address _owner) public view override returns(Tokens memory) {
        ...
        acc.BUSD = magnifiedDividendPerShare.BUSD.mul(balanceOf(_owner)).toInt256Safe()
        .add(magnifiedDividendCorrections[_owner].BUSD).toUint256Safe() / magnitude;
        return acc;
    }
    ...
    function _mint(address account, uint256 value) internal override {
        ...
        magnifiedDividendCorrections[account].BUSD = magnifiedDividendCorrections[account].BUSD
        .sub((magnifiedDividendPerShare.BUSD.mul(value)).toInt256Safe() );
    }
    ...
    function _burn(address account, uint256 value) internal override {
        ...
        magnifiedDividendCorrections[account].BUSD = magnifiedDividendCorrections[account].BUSD
        .add((magnifiedDividendPerShare.BUSD.mul(value)).toInt256Safe() );
    }
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor(address token) DividendPayingToken("Reward_Dividend_Tracker", "Reward_Dividend_Tracker") {
        claimWait = 1800;
    ...
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Reward_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "Reward_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Reward contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOperator {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "Reward_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Reward_Dividend_Tracker: Cannot update claimWait to same value");
    ...
    function withdrawStuckTokens(address _token, uint256 _amount) public onlyOperator {
    ...
    function processAccount(address payable account, bool automatic) public onlyOwner returns (bool) {
        ...
        if(w.BUSD > 0) {
            emit Claim(account, w.BUSD, automatic);
        } else {
            res = false;
        }
...
interface IDividendPayingToken is AdditionalTypes {
    ...
    function distributeDividendsBUSD(uint256 amount) external;
...
interface IRewardDividendTracker is AdditionalTypes, IERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    function owner() external view returns (address);
    function excludeFromDividends(address account) external;
    function updateClaimWait(uint256 newClaimWait) external;
    function claimWait() external view returns (uint256);
    function totalDividendsDistributed() external view returns(Tokens memory);
    function getAccountAtIndex(uint256 index) external view returns (address, int256, int256, Tokens memory, Tokens memory, uint256, uint256, uint256);
    ...
    function getLastProcessedIndex() external view returns(uint256);
    function getNumberOfTokenHolders() external view returns(uint256);
    function getAccount(address _account)external view returns (address account, int256 index, int256 iterationsUntilProcessed, Tokens memory withdrawableDividends, Tokens memory totalDividends, uint256 lastClaimTime, uint256 nextClaimTime, uint256 secondsUntilAutoClaimAvailable);
...

{
  ...
  "libraries": {
    "contracts/libs/IterableMapping.sol": {
      "IterableMapping": "0xea05a5c9808f477c687fed1df3c92db182a0b939"
    }
  }
}