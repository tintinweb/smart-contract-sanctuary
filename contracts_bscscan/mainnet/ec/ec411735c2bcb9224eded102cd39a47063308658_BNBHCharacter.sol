...
contract BNBHCharacter is Initializable, AccessControlUpgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable {
    ...
    function fight(address player, uint256 _attackingHero, uint enemyType, uint256 seed) external restricted onlyOwnerOf(player, _attackingHero) returns(uint256) {
        ...
        uint16 hpLoss = 0;
        // win
        if (seed.mod(1000) < successChance) {
            hero.xp += baseEnemyXps[enemyType];
            
            bnbRewards = baseBNBRewards[enemyType] + baseBNBRewards[enemyType] * attacker.speed / 1000;
            
            uint8 level = _getTownLevel(player, 0);
            bnbRewards = bnbRewards.add(bnbRewards.mul(baseTownRatio[level]).div(100));
            
            level = _getTownLevel(player, 2);
            
            hero.xp += baseTownRatio[8 + level];
            
            if (hero.xp >= (hero.level + 1) * 1000) {
                hero.xp = (hero.level + 1) * 1000 - 1;
            }

            if (requiredHps[enemyType] > attacker.armor / 10) {
                hpLoss = uint16(requiredHps[enemyType] - attacker.armor / 10);
                hero.hp += hpLoss * _secondsPerHp; 
            }            
        }
        // lose
        else {
            hpLoss = uint16(requiredHps[enemyType]);
            hero.hp += hpLoss * _secondsPerHp; 
        }
