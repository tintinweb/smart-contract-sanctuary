...
contract BondDepository is Ownable {
    ...
    address public immutable POWER; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints POWER when receives principle
    ...
    struct Bond {
        uint256 payout; // POWER remaining to be paid
    ...
    function deposit(
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) POWER
         */
        ...
        if (fee != 0) {
            ...
            IERC20(POWER).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(POWER).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(POWER).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(POWER).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(POWER).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function bondPremium() public view returns (uint256 premium_) {
        premium_ = terms.controlVariable.mul(debtRatio()).add(1000000000).div(1e7);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to POWER supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(POWER).totalSupply();
    ...
    /**
     *  @notice calculate amount of POWER available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or POWER) to the DAO
     *  @return bool
     */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != POWER);
