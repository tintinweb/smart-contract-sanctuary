...
/**
 *Submitted for verification at BscScan.com on 2021-12-5
*/
...
contract holygonfinancecoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xC563Cd454dBf38B6e665BD0b1BFD10979533f33E); // Marketing Address
    address payable public charityAddress = payable(0xC563Cd454dBf38B6e665BD0b1BFD10979533f33E); // Charity/Dev Address
    ...
    uint256 private _tTotal = 1000000000000000 * 10**9; // 1 Qud
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "holygon finance coin";
    ...
    uint256 public _maxTxAmount = 100000000000000 * 10**9;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 10000000 * 10**9;  //allow maximum sell amount(anti-dump setup)
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    uint256 private _airdropEth =   6000000000000000; //0.01 BNB Airdrop fee
    uint256 private _airdropToken = 100000000000000000; // 100000000; token will be given on airdrop
    ...
    uint256 private saleMiniBlock;
    uint256 private salePrice = 10000000000000000000;// 0.1*10000000000=10,000,000,000 token for 0.1 bnb (set as per requirements)
    uint256 private saleMaxBlock;
    uint256 private buyPrice = 1000000000000000000000;// 20*1000000000000=1000000000000 token for 20 bnb (set as per requirements)
