...
contract e1TronicToken is BEP20('1TronicSwap Token', 'e1TRC') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "e1TRC::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "e1TRC::delegateBySig: invalid nonce");
        require(now <= expiry, "e1TRC::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "e1TRC::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying e1TRCs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "e1TRC::_writeCheckpoint: block number exceeds 32 bits");
...
// RoboVOLT with Governance.
contract RoboVOLT is BEP20('RoboVOLT Token', 'VOLTRON') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (Mike26).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    // The e1TRC TOKEN!
    e1TronicToken public e1trc;


    constructor(
        e1TronicToken _e1trc
    ) public {
        e1trc = _e1trc;
    }

    // Safe e1trc transfer function, just in case if rounding error causes pool to not have enough e1TRCs.
    function safee1trcTransfer(address _to, uint256 _amount) public onlyOwner {
