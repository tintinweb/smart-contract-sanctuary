...
contract Cosmo is Ownable {
    ...
    // Market oracle provides the GAL/USD exchange rate as an 18 decimal fixed point number.
    ...
    function setGalileoAddress(address newGalileoAddress) external onlyOwner{
        gal = IGal(newGalileoAddress);
    }
    ...
    function rebase() public {
        ...
        uint256 supplyAfterRebase = gal.rebase(epoch, supplyDelta);
    ...
    function adjustTargetRate() external
    {
      ...
      if (exchangeRate > targetRate){
        targetRate = exchangeRate * 981 / 800;
      }

    }

    // increment by 1.22625 to achieve 9.81% every 8hrs
    function incrementTargetRate() internal {

      uint256 newRate = targetRate * 981 / 800;
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && gal.totalSupply() + (uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY - (gal.totalSupply())).toInt256Safe();
        }
    ...
    function computeSupplyDelta(uint256 rate) internal view returns (int256){
        ...
        return gal.totalSupply().toInt256Safe() * (rate.toInt256Safe() - (targetRateSigned)) / (targetRateSigned);
    }
