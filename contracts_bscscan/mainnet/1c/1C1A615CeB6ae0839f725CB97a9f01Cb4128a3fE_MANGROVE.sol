...
contract MANGROVE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MANGROVEDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x503C88c5b1d16e40EC7775f533A593753B6aFDa0); 
    address payable public operations2Address =
        payable(0x1e46116DA4D72c868657FC787f5a2fF14591A063);
    ...
    uint256 public snipeBNBRewardsSellFee = 23;
    ...
    constructor() ERC20("MANGROVE", "MANGROVE") {
        dividendTracker = new MANGROVEDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MANGROVEDividendTracker newDividendTracker = MANGROVEDividendTracker(
            payable(newAddress)
        );
...
contract MANGROVEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "MANGROVE_Dividend_Tracker",
            "MANGROVE_Dividend_Tracker"
        )
    {
        mANGROVE = MANGROVE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "MANGROVE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MANGROVE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MANGROVE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(mANGROVE);

            uint256 prevBalance = mANGROVE.balanceOf(address(this));
            ...
            {
                uint256 received = mANGROVE.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    mANGROVE.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
