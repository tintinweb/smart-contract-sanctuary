...
contract ERC20 is Context, IERC20 {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        /*
            NOTE: Mint function in this contract or any BPHANTOM rewards contract is necessary for the tokenomics
            to work. It is not intended to mint any tokens except for the rewards tracker and BPHANTOM-lps for marketing.
        */
    }
...
contract BabyPhantom is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BabyPhantom", "BPHANTOM") {
    	firstDividendTracker = new FIRSTDividendTracker();
 
    	marketingWallet = 0x4ba45d4FbcEDF899b9065C9a15b994a612f8B0Bd;
    	firstDividendToken = 0x8b3b45E48bE6C31366ffd9dD4F29C1edFFcbA97D;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "BabyPhantom: The marketing wallet is already this address");
    ...
    function updatefirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "BabyPhantom: The dividend tracker already has that address");
 
        FIRSTDividendTracker newfirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newfirstDividendTracker.owner() == address(this), "BabyPhantom: The new dividend tracker must be owned by the BabyPhantom token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyPhantom: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BabyPhantom: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyPhantom: The PanBPHANTOMSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "BabyPhantom: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "BabyPhantom: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "BabyPhantom: Trading has not started yet");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BabyPhantom_Dividend_Tracker", "BabyPhantom_Dividend_Tracker", 0x8b3b45E48bE6C31366ffd9dD4F29C1edFFcbA97D) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyPhantom_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "BabyPhantom_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyPhantom contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyPhantom_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyPhantom_Dividend_Tracker: Cannot update claimWait to same value");
