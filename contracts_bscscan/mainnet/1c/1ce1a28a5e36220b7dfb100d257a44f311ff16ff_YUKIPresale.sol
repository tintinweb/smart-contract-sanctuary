...
contract YUKIPresale is Ownable ,Whitelisted {
    ...
    YUKITOKEN public token;
    
    constructor(address payable _walletMajorOwner) 
    {
        token = YUKITOKEN(0xBcDa505B91b47360f16Ae1d6005A6370650cC23F); //Token Contract
        startTime = 1625058000;   
        endTime = startTime + 192 hours;
    ...
    function buy(address beneficiary, address payable referral) public payable
    {
        ...
        forwardFunds(partnerCoins, msg.value);
    }
    ...
    // Ensure requested tokens aren't users YUKI tokens
    function recoverLostTokensExceptOurTokens(address _token, uint256 amount) public onlyOwner {
         ...
         YUKITOKEN(_token).transfer(msg.sender, amount);
    }
