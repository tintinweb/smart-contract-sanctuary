...
import "./IUniswapV2Router.sol";
    ...
    contract BabySYC is ERC20, Ownable {
    ...
    constructor() ERC20("BabySYC", "BabySYC") {
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "BabySYC: The marketing wallet is already this address");
    ...
    function updatebttDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(bttDividendTracker), "BabySYC: The dividend tracker already has that address");
 
       BTTDividendTracker newbttDividendTracker =BTTDividendTracker(payable(newAddress));
 
        require(newbttDividendTracker.owner() == address(this), "BabySYC: The new dividend tracker must be owned by the BabySYC token contract");
    ...
    function updatetrxDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(trxDividendTracker), "BabySYC: The dividend tracker already has that address");
 
        TRXDividendTracker newtrxDividendTracker = TRXDividendTracker(payable(newAddress));
 
        require(newtrxDividendTracker.owner() == address(this), "BabySYC: The new dividend tracker must be owned by the BabySYC token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabySYC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BabySYC: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabySYC: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "BabySYC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "BabySYC: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "BabySYC: Trading has not started yet");
    ...
    constructor() DividendPayingToken("BabySYC_Btt_Dividend_Tracker", "BabySYC_Btt_Dividend_Tracker",  0x8595F9dA7b868b1822194fAEd312235E43007b49) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); 
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabySYC_Btt_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "BabySYC_Btt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabySYC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BabySYC_Btt_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabySYC_Btt_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TRXDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BabySYC_Trx_Dividend_Tracker", "BabySYC_Trx_Dividend_Tracker", 0x85EAC5Ac2F758618dFa09bDbe0cf174e7d574D5B) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabySYC_Trx_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "BabySYC_Trx_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabySYC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BabySYC_Trx_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabySYC_Trx_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
