...
https://t.me/Cloud9Official

AntiBot Feature!
    On launch tax will be set to 99%
    After 2 minutes buying tax will drop to 10%
    Selling tax is always 20%

DiamondHand Feature!
    When claiming, there is a 50% tax on rewards if you have sold even once. HODL for more rewards!
    Claiming is manual!! You must claim on the contract before you sell!

Tokenomics:
    Supply: 100B tokens
    Max Wallet: 1%
    Max Tx: 0.3%
    Sell Cooldown 3 minutes
...
//Cloud9 Contract ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////
contract Cloud9 is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'Cloud9';
    string private constant _symbol = 'C9';
    uint8 private constant _decimals = 9;
    uint256 public constant _totalSupply = 1000 * 10**8 * 10**9;//equals 100,000,000,000

    //BotProtection values
    bool private _botProtection;
    uint8 constant BotMaxTax=99;
    uint256 constant BotTaxTime=2 minutes;
    uint256 public launchTimestamp;
    uint8 private constant _whiteListBonus=80;

    //Divider for the Minimal MaxBalance based on circulating Supply (2%)
    uint16 public constant BalanceLimitDivider=100;
    //Divider for Minimal sellLimit based on circulating Supply (0.5%)
    uint16 public constant SellLimitDivider=300;
    //Sellers get locked for MaxSellLockTime so they can't dump repeatedly
    uint16 public constant MaxSellLockTime=3 minutes;
    ...
    address public constant TeamWallet=0xbE83EB8d2abAC7F8cD64a6678eF6E088cC380F83;
    //The dev wallet
    address public constant devWallet=0x8303530E95C9Ee00371842dbf17298F16ceae972;
    ...
    constructor () {
        ...
        //Limits start enabled
        sellLockDisabled=false;
       ...
       //Sets sellLockTime to be max by default
        ...
        //80% of the tax goes to Liquidity at start to generate a lot of liquidity during
        //bot Protection
        _stakingTax=10;
        _liquidityTax=90;
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            if(!sellLockDisabled&&!_excludedFromSellLock.contains(sender)){
                ...
                //Sets the time sellers get locked(3 minutes by default)
                _sellLock[sender]=block.timestamp+sellLockTime;
                //Sells can't exceed the sell limit(5,000,000 Tokens at start, can be updated to circulating supply)
                require(amount<=sellLimit,"Dump protection");
            }   
            //As soon someone sells, DiamondHands are gone
            if(!_whiteList.contains(sender)){
    ...
    //marketing share starts at 20% to push initial marketing, after start
    //its capped to 50% max, the percentage of the staking that gets used for
    //marketing/paying the team
    uint8 public marketingShare=20;
    ...
    function TeamSetDiamondHands(uint8 fee, bool feeOn) public onlyTeam{
        require(fee<=50,"diamond Hands Fee is capped to 50%");
    ...
    function getTaxes() public view returns(uint256 liquidityTax,uint256 StakingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
    ...
    uint16 public AutoLPTreshold=5;
    ...
    function TeamWithdrawMarketingBNB() public onlyTeam{
        ...
        uint256 devamount = marketingBalance / 10;
        uint256 marketingamount=amount - devamount;  
        marketingBalance=0;
        (bool sent,) =TeamWallet.call{value: (marketingamount)}("");
        (sent,) =devWallet.call{value: (devamount)}("");       
    ...
    function SetupCreateLPandLock(uint8 TeamTokenPercent) public payable onlyTeam{
    ...
    uint256 public _liquidityUnlockTime;
