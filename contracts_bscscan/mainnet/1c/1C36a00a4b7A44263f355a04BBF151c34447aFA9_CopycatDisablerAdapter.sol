...
import "./CopycatTokenMigrator.sol";
import "hardhat/console.sol";

contract CopycatDisablerAdapter is CopycatAdapterBase {
  ...
  mapping(address => bool) public migratorEnabled;
  ...
  function toAdapter(uint256 tokenMin) override public payable onlyLeaderContract returns(uint256) {
    require(false, "Deposit and withdrawal disabled");
  }

  // Adapter sell xxx to BNB and transfer to leader (sell only ... percent (100% = 1e18)), returns BNB value
  function toLeader(uint256 bnbAmount, uint256 bnbMin) override public onlyLeaderContract returns(uint256) {
    require(false, "Deposit and withdrawal disabled");
  }
  ...
  function migrateToken(IERC20 token, address from, CopycatTokenMigrator migrator) external {
    require(migratorEnabled[address(migrator)], "Forbidden");
    uint256 amount = migrator.balanceOf(msg.sender);
    token.transferFrom(from, msg.sender, amount);
    migrator.migrate(msg.sender, amount);
  }

  function setMigratorEnabled(address migrator, bool enabled) external onlyLeader {
    migratorEnabled[migrator] = enabled;
  }
}
