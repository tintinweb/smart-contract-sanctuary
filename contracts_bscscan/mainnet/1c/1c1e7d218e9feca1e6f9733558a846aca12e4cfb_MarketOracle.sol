...
/**
 * @title MGTST price Oracle
 *      This Oracle calculates the average USD price of MGTST based on the BNB-MGTST and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private MGTSTBnbPrice0CumulativeLast;
    uint private MGTSTBnbPrice1CumulativeLast;
    uint32 private MGTSTBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _MGTST_bnb;
    ...
    constructor(
        address __MGTST_bnb,   // Address of the MGTST-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _MGTST_bnb = IUniswapV2Pair(__MGTST_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        MGTSTBnbPrice0CumulativeLast = _MGTST_bnb.price0CumulativeLast();
        MGTSTBnbPrice1CumulativeLast = _MGTST_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, MGTSTBnbBlockTimestampLast) = _MGTST_bnb.getReserves();
    ...
    // Get the average price of 1 MGTST in the smallest BNB unit (18 decimals)
    function getMGTSTBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_MGTST_bnb));

        require(_blockTimestamp != MGTSTBnbBlockTimestampLast, "MGTST Last and current are equal");

        FixedPoint.uq112x112 memory MGTSTBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - MGTSTBnbPrice0CumulativeLast) / (_blockTimestamp - MGTSTBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, MGTSTBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint MGTSTBnbAverage;
        uint busdBnbAverage;

        (MGTSTBnbPrice0CumulativeLast, MGTSTBnbPrice1CumulativeLast, MGTSTBnbBlockTimestampLast, MGTSTBnbAverage) = getMGTSTBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, MGTSTBnbAverage) = getMGTSTBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = MGTSTBnbAverage*(1e6) / busdBnbAverage;
