...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable Cardano = address(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47); //Cardano BSC-Pegged token
  ...
  function distributeCardanoDividends(uint256 amount) public onlyOwner{
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IERC20(Cardano).transfer(user, _withdrawableDividend);
...
contract PinaColada is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PinaColadaDividendTracker public dividendTracker;
    ...
    uint256 public CardanoRewardsFee = 8;
    uint256 public liquidityFee = 1;
    uint256 public marketingFee = 6;
    uint256 public totalFees = CardanoRewardsFee.add(liquidityFee).add(marketingFee);

    address public _marketingWalletAddress = 0x900cCA28697525D1a177F592Fea2e35a18C863cf;


    // use by default 300,000 gas to process auto-claiming dividends
    uint256 public gasForProcessing = 300000;
    uint256 public maxFee = 34;
    uint256 public buyTimer = 20 seconds;
    ...
    bool public buySellTimerOn = true;
    ...
    uint256 private percentEquivalent = 200000000 * (10**18);
    ...
    constructor() public ERC20("PinaColada", "PinADA") {

    	dividendTracker = new PinaColadaDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PiñaColada: The dividend tracker already has that address");

        PinaColadaDividendTracker newDividendTracker = PinaColadaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PiñaColada: The new dividend tracker must be owned by the PiñaColada token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PiñaColada: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PiñaColada: Account is already the value of 'excluded'");
    ...
    function setCardanoRewardsFee(uint256 value) external onlyOwner{
        CardanoRewardsFee = value;
        totalFees = CardanoRewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PiñaColada: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PiñaColada: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PiñaColada: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PiñaColada: Cannot update gasForProcessing to same value");
   ...
   function getCurrentPrice() public view returns (uint256 currentPrice) {//This value serves as a reference to calculate profit only.
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialCardanoBalance = IERC20(Cardano).balanceOf(address(this));

        swapTokensForCardano(tokens);
        uint256 newBalance = (IERC20(Cardano).balanceOf(address(this))).sub(initialCardanoBalance);
        IERC20(Cardano).transfer(_marketingWalletAddress, newBalance);
    }
    ...
    function swapTokensForCardano(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private{
        swapTokensForCardano(tokens);
        uint256 dividends = IERC20(Cardano).balanceOf(address(this));
        bool success = IERC20(Cardano).transfer(address(dividendTracker), dividends);

        if (success) {
            dividendTracker.distributeCardanoDividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }
}

contract PinaColadaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("PiñaColada_Dividend_Tracker", "PiñaColada_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "PiñaColada_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "PiñaColada_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PiñaColada contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PiñaColada_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PiñaColada_Dividend_Tracker: Cannot update claimWait to same value");
