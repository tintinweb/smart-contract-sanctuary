...
contract ElonCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ECakeDividendTracker public dividendTracker;

    address private buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 1500000000 * (10**18); // 1,5% of total supply
    uint256 public maxSellTransactionAmount = 300000000 * (10**18);// 0.3% of total supply
    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 public maxWalletToken = 2500000000 * (10**18);
    ...
    constructor() ERC20("CakeMiner", "CakeMiner") {
        ...
        dividendTracker = new ECakeDividendTracker();

        buyBackWallet = 0xFc96DEe6774f677aB7ac63536600366Dc26E3EFA;
    ...
    function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
        require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "ECake: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
        sellFeeIncreaseFactor = multiplier;
    }
    
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(marketingDivisor >= 0 && marketingDivisor <= 100, "ECake: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "ECake: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ECake: The dividend tracker already has that address");

        ECakeDividendTracker newDividendTracker = ECakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ECake: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "ECake: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ECake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ECake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ECake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ECake: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "ECake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ECake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ECake: Cannot update gasForProcessing to same value");
...
contract ECakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ECake_Dividend_Tracker", "ECake_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ECake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ECake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ECake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "ECake_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours");
        require(newClaimWait != claimWait, "ECake_Dividend_Tracker: Cannot update claimWait to same value");
