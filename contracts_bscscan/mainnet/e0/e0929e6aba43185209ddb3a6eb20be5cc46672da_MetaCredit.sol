...
interface CreditSafe {
  ...
  /**
   * @dev CreditOwn `amount` tokens from the caller's account to `recipient`.
   *
  ...
  /**
   * @dev CreditOwn `amount` tokens from `sender` to `recipient` using the
...
contract MetaCredit is CreditSafe, Context, CreditOwn  {
  
    // common addresses
    address private _owner;
    address private TCredit;
    address private ACredit;
    ...
    string public override name = "MetaCredit";
    string public override symbol = "MetaCredit";
    ...
    constructor(uint totalSupplyValue, address TCreditAddress, address ACreditAddress) {
        // set total supply
        totalSupply = totalSupplyValue;
        
        // designate addresses
        _owner = msg.sender;
        TCredit = TCreditAddress;
        ACredit = ACreditAddress;
        
        // split the tokens according to agreed upon percentages
        balances[TCredit] =  totalSupply * 50 / 100;
        balances[ACredit] = totalSupply * 49 / 100;
    ...
    // Get the address of the token's CreditOwn pot
