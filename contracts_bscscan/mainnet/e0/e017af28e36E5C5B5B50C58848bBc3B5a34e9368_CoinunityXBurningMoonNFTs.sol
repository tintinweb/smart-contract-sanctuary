...
contract CoinunityXBurningMoonNFTs is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    // Token name
    string private _name="Coinunity x BurningMoon";
    // Token symbol
    string private _symbol="CNTYXBM";
    ...
    address constant TokenAddress=0x5D7359B82d221F8d7C335e2d2b02543CB9450c72; //CNTY
    ...
    uint256 public currentValue=10**16*2; //0.02BNB per NFT
    uint[] public Minted;
    constructor() {
        ...
        NFTPrototypes.push(NFTData("Base Card", "QmfJKN6s891ATYZF343Q5dJrpp9dryJ8PYnwACBtJMVJ6M"));
        NFTPrototypes.push(NFTData("VotingPower +10", "Qmen3o25vU7qaYkTVfGzp3UL4xjeH76E57iHQUgw7X2LK3"));
        NFTPrototypes.push(NFTData("VotingPower +20", "QmaeDVLWrHP29Yj79JQep7spaAmbaPTqmG2NCuL6ZdUFi6"));
        NFTPrototypes.push(NFTData("VotingPower +40", "QmSht6Xx3dNQrf5o9dCnFoQKX445xbef55AshKiEzuqKkm"));
        NFTPrototypes.push(NFTData("VIP Level 2", "QmbwG7SHTbXNaQxjeCXTjq6PNfpD2Ht39MutAfgELHf9nV"));
        NFTPrototypes.push(NFTData("50k MetaBurn", "QmZTJPeWKPJ5HtKJwzd2z72SnU7J7nqaer62BonqJxi5m3"));
        
        Minted=new uint[](6);
        
        BM=BurningMoon(BMaddress);
        BMPCS=IPancakeRouter02(PCSaddress);
        TokenPCS=IPancakeRouter02(0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7);
        Token=IBEP20(TokenAddress);
    }
    ...
    function _distributeBNB(uint amount) private{
        ...
        if(amount<=0) return;
        uint256 BMAmount=amount*50/100;
        uint RewardsAmount=amount*40/10;
    ...
    //0x9fed68db3e56164fe7842f7a387b881867eee11c
...
//1* - Base card - 1500x - Dropchance: 37,5%
//2* -  Voting power +10 - 1000x - Dropchance: 25%
//3* - Voting power +20 - 750x - Dropchance: 18,75%
//4* - Voting power +40 - 500x - Dropchance: 12,5%
//5* - VIP level 2 - 242x - Dropchance: 6,05
//6* - 50 k MetaBurn - 8x - Dropchance: Milestones every 500th drop
    function _mint(address to, uint seed) private returns (uint){
        ...
        if((Number+1)%500==0)
            ...
            uint value=_prng(3992, seed);
        if(value<1500) ID=0;      
        else if(value<2500)ID=1;  
        else if(value<3250)ID=2;  
        else if(value<3750)ID=3;  
        else ID=4;   
        }
        Minted[ID]++;
