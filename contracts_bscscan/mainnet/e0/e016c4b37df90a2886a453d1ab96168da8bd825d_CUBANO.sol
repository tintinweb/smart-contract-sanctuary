...
contract Ownable is Context {
    ...
    constructor () {
        ...
        _owner = 0x5714EE5e129b4239DC8A484655b16d3BED05C36E;
...
contract CUBANO is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public utilityWallet = payable(0xd0B6F301f5ea93CcF8a4aB66dF9104B48dAC1Ce7); // utility wallet Address
    address payable public marketingWallet = payable(0x11Fc567A759F396823b1376E5bF16505e577c0ff); // marketing Address
    ...
    string private _name = "cubano";
    string private _symbol = "CUBANO";
   ...
   //Buy Fees
    ...
    uint256 public _buyUtilityFee = 2;
    ...
    uint256 public _sellUtilityFee = 2;
    ...
    uint256 private _utilityFee;
    uint256 private _burnFee;
        
    uint256 public _maxTxAmount = 10000000 * 10**18;
    uint256 private minimumTokensBeforeSwap = 10000000 * 10**18;
    ...
    {
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellUtilityFee);
        uint256 halfLiquidityTokens = contractTokenBalance.mul(_sellLiquidityFee).div(totalFee).div(2);
        uint256 swapableTokens = contractTokenBalance.sub(halfLiquidityTokens);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(swapableTokens); 
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 ethForLiquidity = newBalance.mul(_sellLiquidityFee).div(totalFee).div(2);
        ...
        uint256 ethForMarketing = newBalance.mul(_sellMarketingFee).div(totalFee);
        uint256 ethForCharity = newBalance.sub(ethForMarketing).sub(ethForLiquidity);
        transferToAddressETH(utilityWallet, ethForCharity);
    ...
    returns (uint256, uint256)
    {
        if(_utilityFee==0 && _marketingFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tCharity = tAmount.div(100).mul(_utilityFee.add(_marketingFee));
    ...
    function setBuyFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 utilityFee, uint256 burnFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        _buyMarketingFee = MarketingFee;
        _buyLiquidityFee = liquidityFee;
        _buyUtilityFee = utilityFee;
        _buyBurnFee = burnFee;
    }

    function setSellFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 utilityFee, uint256 burnFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        _sellMarketingFee = MarketingFee;
        _sellLiquidityFee = liquidityFee;
        _sellUtilityFee = utilityFee;
    ...
    function setutilityWallet(address _utilityAddress) external onlyOwner() 
    {
        utilityWallet = payable(_utilityAddress);
    }
