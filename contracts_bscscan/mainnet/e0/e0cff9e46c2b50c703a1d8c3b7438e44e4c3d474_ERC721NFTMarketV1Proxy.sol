...
// File: contracts/ERC721NFTMarketV1Proxy.sol
...
interface ERC721NFTMarketV1 {
    function buyTokenUsingBNB(address _collection, uint256 _tokenId) external payable; 
}

contract ERC721NFTMarketV1Proxy is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Constructor
     * @param _market: address of the market to proxy calls to
     */
   constructor(address _market) {
        require(_market != address(0), "Operations: Market address cannot be zero");
        market = _market;
    }
    ...
    function buyTokenUsingBNB(address _collection, uint256 _tokenId) external payable nonReentrant {
       ERC721NFTMarketV1(market).buyTokenUsingBNB{value: msg.value}(_collection, _tokenId);
       IERC721(_collection).safeTransferFrom(address(this), address(msg.sender), _tokenId);
       emit Trade(_collection, _tokenId, msg.sender, msg.value);
    }
