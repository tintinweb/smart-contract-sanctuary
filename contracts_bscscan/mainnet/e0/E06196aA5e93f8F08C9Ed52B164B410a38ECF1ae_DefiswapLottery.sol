...
contract DefiswapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInDefi = 50 ether;
    uint256 public minPriceTicketInDefi = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInDefi;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] defiPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInDefi;
        uint32 finalNumber;
    }
     ...
     * @param _defiTokenAddress: address of the DEFI token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _defiTokenAddress, address _randomGeneratorAddress) {
        require(_defiTokenAddress != address(0), "DefiswapLottery:: defiTokenAddress is zero address"); 
        require(_randomGeneratorAddress != address(0), "DefiswapLottery:: randomGeneratorAddress is zero address"); 
        //
        ...
        nonReentrant
    {
        ...
        // Calculate number of DEFI to this contract
        uint256 amountDefiToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInDefi,
            _ticketNumbers.length
        );

        // Transfer defi tokens to this contract
        defiToken.safeTransferFrom(address(msg.sender), address(this), amountDefiToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInDefi += amountDefiToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInDefiToTransfer
        uint256 rewardInDefiToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInDefiToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        defiToken.safeTransfer(msg.sender, rewardInDefiToTransfer);

        emit TicketsClaim(msg.sender, rewardInDefiToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in DEFI per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInDefi) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in DEFI for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].defiPerBracket[j] =
                ...
                // A. No DEFI to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].defiPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInDefi - amountToShareToWinners);

        // Transfer DEFI to treasury address
        defiToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in DEFI token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        defiToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInDefi += _amount;
     ...
     * @param _priceTicketInDefi: price of a ticket in DEFI
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInDefi >= minPriceTicketInDefi) && (_priceTicketInDefi <= maxPriceTicketInDefi),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInDefi: _priceTicketInDefi,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            defiPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInDefi: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(defiToken), "Cannot be DEFI token");
    ...
    /**
     * @notice Set DEFI price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInDefi: minimum price of a ticket in DEFI
     * @param _maxPriceTicketInDefi: maximum price of a ticket in DEFI
     */
    function setMinAndMaxTicketPriceInDefi(uint256 _minPriceTicketInDefi, uint256 _maxPriceTicketInDefi)
        external
        onlyOwner
    {
        require(_minPriceTicketInDefi <= _maxPriceTicketInDefi, "minPrice must be < maxPrice");

        minPriceTicketInDefi = _minPriceTicketInDefi;
        maxPriceTicketInDefi = _maxPriceTicketInDefi;
    }
     ...
     * @param _priceTicket price of a ticket (in DEFI)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].defiPerBracket[_bracket];
        } else {
            return 0;
        }
    }
