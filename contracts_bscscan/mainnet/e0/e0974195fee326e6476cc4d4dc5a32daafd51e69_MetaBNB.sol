...
contract MetaBNB is IBEP20, Auth {
    ...
    string constant _symbol = "mBNB";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 10000000000 * (10 ** _decimals); // 10.000.000.000
    uint256 public _maxTxAmount = _totalSupply / 200; //100 0.1% 
    uint256 public _maxWallet = _totalSupply / 63; // 3%
    ...
    function setMaxBuyTransaction(uint256 amount) external authorized {
        _maxTxAmount = amount;
	...
	function Function(address user, bool banned) external authorized {
		if (banned) {
    ...
    function setMaxSellTransaction(bool _enabled, uint256 _amount) external authorized {
    ...
    function setMaxBuyTransaction() external view returns (bool,uint256) {
        return (
            swapEnabled,
            swapThreshold
        );
