...
/**
  ...
  * Telegram =>  https://t.me/MoonshotEther
...
contract MoonshotEther is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MoonshotEtherDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MoonshotEther", "MoonshotEther") {
    	...
    	dividendTracker = new MoonshotEtherDividendTracker();
		marketingWallet = payable(0x571aeDb5B74a7Dc524e92160755Bf68d81c06907);

    	buyBackWallet = payable(0x4300Ec6957bbb3a4A2C45bE9dfEB47167fE16C4D);
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "MoonshotEther: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "MoonshotEther: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "MoonshotEther: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MoonshotEther: The dividend tracker already has that address");

        MoonshotEtherDividendTracker newDividendTracker = MoonshotEtherDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MoonshotEther: The new dividend tracker must be owned by the MoonshotEther token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "MoonshotEther: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MoonshotEther: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MoonshotEther: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MoonshotEther: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MoonshotEther: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "MoonshotEther: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BNBaller: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MoonshotEther: Cannot update gasForProcessing to same value");
...
contract MoonshotEtherDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MoonshotEther_Dividend_Tracker", "MoonshotEther_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 150000 * (10**18); //must hold 75,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(true, "MoonshotEther_Dividend_Tracker: Transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MoonshotEther_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MoonshotEther contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 3600, "MoonshotEther_Dividend_Tracker: claimWait must be updated to between 30 Minutes and 1 Hour");
        require(newClaimWait != claimWait, "MoonshotEther_Dividend_Tracker: Cannot update claimWait to same value");
