...
contract SHIBADABBSC is Context, IERC20, Ownable {
    ...
    uint256 private _tbabymuskdogeTokenTotal;

    string private _name = "ShibaDabBSC";
    string private _symbol = "SHIDAB";
    ...
    uint256 public _babymuskdogeTokenFee = 15;
    uint256 private _previousbabymuskdogeTokenFee = _babymuskdogeTokenFee;
    
    uint256 public _maxTxAmount = 1000 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 1 * 10**9 * 10**9; 
    
    address payable public FFI = 0x6Fac115364A482d88C4DA0f4F2a91a82762373ad; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tbabymuskdogeTokenTotal = _tbabymuskdogeTokenTotal.add(address(this).balance);
        TransferCharityETH(FFI, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_babymuskdogeTokenFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _babymuskdogeTokenFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousbabymuskdogeTokenFee = _babymuskdogeTokenFee;
        
        _taxFee = 0;
        _burnFee = 0;
        _babymuskdogeTokenFee = 0;
    }
    
    function restoreAllFee() private {
        _taxFee = _previousTaxFee;
        _burnFee = _previousBurnFee;
        _babymuskdogeTokenFee = _previousbabymuskdogeTokenFee;
    }
    ...
    function setbabymuskdogeTokenFeePercent(uint256 babymuskdogeTokenFee) external onlyOwner() {
        _babymuskdogeTokenFee = babymuskdogeTokenFee;
    }
