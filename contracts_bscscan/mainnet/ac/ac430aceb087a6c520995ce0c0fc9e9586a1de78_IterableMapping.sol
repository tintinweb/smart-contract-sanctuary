...
contract WAKAWAKADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("WAKAWAKA_Dividend_Tracker", "WAKAWAKA_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 200000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "WAKAWAKA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "WAKAWAKA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WAKAWAKA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "WAKAWAKA_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours");
        require(newClaimWait != claimWait, "WAKAWAKA: Cannot update claimWait to same value");
...
contract WAKAWAKA is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    WAKAWAKADividendTracker public dividendTracker;
    ...
    constructor() ERC20("WAKAWAKA", "WAKA") {
        BNBRewardsFee = 10;
        liquidityFee = 2;
        extraFeeOnSell = 1;
        marketingFee = 3;
        marketingWallet = payable(0x68b294E13C4E7BaCFa4B9717Ed1BeCB08c8B43d7);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new WAKAWAKADividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WAKAWAKA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WAKAWAKA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WAKAWAKA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WAKAWAKA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WAKAWAKA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WAKAWAKA: Cannot update gasForProcessing to same value");
