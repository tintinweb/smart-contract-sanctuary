...
/**
BlackBull Tokenomics:
+  Total Supply : 100.000.000.000.000 
+  Burn: 50.000.000.000.000 (50%) 
+  PCS : 40.000.000.000,000 (40%) 
+  Marketing, airdop:  5.000.000.000.000 (5%)
+  Shilling Contest + TikTok channenge:  3.000.000.000.000 (3%)
+  Devs Wallet:  2.000.000.000.000 (2%)
 
 15% Transaction Fees
 12% is auto redistributed to all holders In BTC Hold BlackBull tokens, earn BTC. 
 Auto Paid For the first time ever, you don’t need to claim your earned BNB. It’s automatically sent to your wallet.
 2% go to BuyBack Function so that you will never see 2 sell in a row or a big drop in price 
 1% Marketing Wallet: 2% of every transaction is allocated to Marketing for future funds of the project and longevity

=> Liquidity Secured By DxSale Liquidity will be locked with DXSALE for 1 month
<!-- There will be some refs to groups --->
...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 IWBNB = IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external override onlyToken {
        require(_minDistribution >= 1 * (10 ** 18), "setDistributionCriteria:: invalid minDistribution");
...
contract BabaBoi is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "BlackBull";
    string constant _symbol = "BBULL";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100 * 10**6 * 10**6 * (10 ** _decimals); 
    ...
    uint256 public swapThreshold = (_totalSupply * 50) / 100; 
    bool inSwap;
    ...
    function swapBack() internal swapping {
        ...
        autoLiquidityReceiver.transfer(amountBNBBuyback);
        emit AmountBNBBuyback(amountBNBBuyback);
    }

    function triggerManualBuyback(uint256 amount) external onlyOwner {
        buyBabaBoiToken(amount, DEAD);
    }

    function triggerAutoBuyback() internal {
        buyBabaBoiToken(autoBuybackAmount, DEAD);
        autoBuybackBlockLast = block.number;
        autoBuybackAccumulator = autoBuybackAccumulator.add(autoBuybackAmount);
        if(autoBuybackAccumulator > autoBuybackCap){ autoBuybackEnabled = false; }
    }

    function buyBabaBoiToken(uint256 amount, address to) internal swapping {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(this);
    ...
    function setAutoBuybackSettings(bool _enabled, uint256 _cap, uint256 _amount, uint256 _period) external onlyOwner {
        require(_amount <= _totalSupply, "AutoBuybackSettings:: invalid amount!");
    ...
    function setTxLimit(uint256 amount) external onlyOwner {
        require((amount >= _totalSupply/100) && (amount <= (_totalSupply*10)/100), "Txlimit is invalid!"); 
        _maxTxAmount = amount;
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount) external onlyOwner {
        require(_amount >= _totalSupply / 100, "SwapBackSetting:: invalid amount!");
