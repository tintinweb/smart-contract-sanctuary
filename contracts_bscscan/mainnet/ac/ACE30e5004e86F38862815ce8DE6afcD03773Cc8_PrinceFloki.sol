...
contract PrinceFloki is ERC20, Ownable {
    ...
    PrinceFlokiDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Prince Floki V2", "PrinceFloki") {

    	dividendTracker = new PrinceFlokiDividendTracker();
    ...
    /// @notice Withdraw tokens sent by mistake. Impossible to withdraw PrinceFloki tokens
    /// @param tokenAddress The address of the token to withdraw
    function rescueBEP20Tokens(address tokenAddress) external onlyOwner{
        require(tokenAddress != address(this), "Impossible to withdraw PrinceFloki tokens");
   ...
   /////////////////////////////////
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PrinceFloki: Account is already the value of 'excluded'");
   ...
   ///////////////////////
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "PrinceFloki: Automated market maker pair is already set to that value");
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PrinceFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PrinceFloki: Cannot update gasForProcessing to same value");
...
contract PrinceFlokiDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("PrinceFloki_Dividen_Tracker", "PrinceFloki_Dividend_Tracker") {
    	claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PrinceFloki_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PrinceFloki_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PrinceFloki contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PrinceFloki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PrinceFloki_Dividend_Tracker: Cannot update claimWait to same value");
