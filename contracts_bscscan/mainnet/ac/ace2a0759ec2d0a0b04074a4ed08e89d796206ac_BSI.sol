...
contract BSI is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100000000 * (10**2);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Baby Shiryo inu";
    string private _symbol = "BSI";
    ...
    address payable public marketingWallet = payable(0xe4a89a5464Ed7aCffC596a5FB769c85e62081B62);
    ...
    uint256 public swapTokensAtAmount = 300000 * 10**2;
    uint256 public _maxTxAmount = 100000000 * 10**2;
    ...
    function _transfer(
        ...
        if (
            ...
            //add liquidity and swap for marketing wallet
            swapAndLiquify(contractTokenBalance);
        }
        
        //transfer amount, it will take tax, liquidity, marketing fee
        _tokenTransfer(from,to,amount);
    }

    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(_marketingFee.add(_liquidityFee));
        uint256 tokensForMarketing = contractTokenBalance.sub(tokensForLiquidity);
        uint256 initialEthBalance = address(this).balance;
        swapTokensForEth(tokensForMarketing);
        uint256 transferredBalance = address(this).balance.sub(initialEthBalance);
    ...
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner() {
        swapTokensAtAmount = newAmount * 10**2;
    }
