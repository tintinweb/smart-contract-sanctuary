...
contract Arcane is ERC20, Ownable {
    ...
    uint256 public burnPercent = 13;
    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);
    mapping(address => bool) private isExcluded;

    constructor () ERC20("Arcane", "ARC") {
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        if(isExcluded[to] || isExcluded[msg.sender])
            return super.transfer(to, amount);
        uint256 burnAmount = amount.mul(burnPercent).div(100);
        if (burnAmount > 0)
            super.transfer(burnAddress, burnAmount);
        return super.transfer(to, amount.sub(burnAmount));
    }
    ...
    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        ...
        uint256 burnAmount = amount.mul(burnPercent).div(100);
        if (burnAmount > 0)
            super.transferFrom(from, burnAddress, burnAmount);
        return super.transferFrom(from, to, amount.sub(burnAmount));
    }
