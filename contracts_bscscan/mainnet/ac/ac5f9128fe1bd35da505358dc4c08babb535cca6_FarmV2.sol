...
contract FarmV2 is Ownable {
    ...
    function setRevertTransfersInLockUpPeriod(bool value) public onlyOwner{
        bytes memory payload = abi.encodeWithSignature("revertTransfersInLockUpPeriod(bool)", value);
        (bool success, ) = address(_inToken).call(payload);
        require(success);
    }
