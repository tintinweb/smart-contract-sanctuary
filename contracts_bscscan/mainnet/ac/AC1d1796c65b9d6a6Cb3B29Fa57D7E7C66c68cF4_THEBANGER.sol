...
contract THEBANGER is IBEP20, Auth {
    ...
    function getTF(bool selling, address sender) public view returns (uint256) {
        ...
        if(!selling && bbt && !itCDh[sender] && 
            (_balances[sender] > tAl) &&
            (tCDtb[sender] + tCDt1 > block.timestamp) && 
            (tCDtb[sender] + tCDt2 < block.timestamp)){ 
                return bbf1.mul(1); }
        if(!selling && bbt && !itCDh[sender] &&
            (_balances[sender] > tAl) &&
            (tCDtb[sender] + tCDt2 > block.timestamp) && 
            (tCDtb[sender] + tCDt3 < block.timestamp)){ 
                return bbf2.mul(1); }
        if(!selling && bbt && !itCDh[sender] &&
            (_balances[sender] > tAl) &&
            (tCDtb[sender] + tCDt3 > block.timestamp) && 
            (tCDtb[sender] + tCDt4 < block.timestamp)){ 
                return bbf3.mul(1); }
        if(!selling && bbt && !itCDh[sender] &&
            (_balances[sender] > tAl) &&
            (tCDtb[sender] + tCDt4 > block.timestamp)){ 
    ...
    function _MyBuyFee(bool selling, address recipient) external view returns (uint256) {
        selling = bool(false);
        recipient = address(msg.sender);
        return getTF(selling, recipient);
    }
