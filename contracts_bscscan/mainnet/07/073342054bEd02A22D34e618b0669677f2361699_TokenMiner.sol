...
contract TokenMiner{
  ...
  using SafeBEP20 for IBEP20;
    ...
    // using IBEP20 for mining token
    IBEP20 public miningToken;
    ...
    function sellEggs() public {
        ...
        miningToken.safeTransfer(ceoAddress, fee);
        miningToken.safeTransfer(address(msg.sender), (eggValue.sub(fee)));
    }

    function buyEggs(address ref, uint256 amountTOKEN) public {
        require(initialized);
        miningToken.safeTransferFrom(msg.sender,address(this),amountTOKEN);
        uint256 eggsBought=calculateEggBuy(amountTOKEN,miningToken.balanceOf(address(this)).sub(amountTOKEN));
        eggsBought=eggsBought.sub(devFee(eggsBought));
        uint256 fee=devFee(amountTOKEN);
        miningToken.safeTransferFrom(msg.sender,ceoAddress,fee);
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256){
        return calculateTrade(eggs,marketEggs,miningToken.balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 _miningToken,uint256 contractBalance) public view returns(uint256){
        return calculateTrade(_miningToken,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 _miningToken) public view returns(uint256){
        return calculateEggBuy(_miningToken,miningToken.balanceOf(address(this)));
    }
    ...
    function getEggsSinceLastHatch(address adr) public view returns(uint256){
        uint256 secondsPassed=min(EGGS_TO_HATCH_1MINERS,now.sub(lastHatch[adr]));
