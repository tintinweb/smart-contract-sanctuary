...
contract FlokiMetaverseReward is RewardHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() RewardHolders("FlokiMetaverse_Reward_Tracker", "FlokiMetaverse_Reward_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "FlokiMetaverse_Reward_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "FlokiMetaverse_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiMetaverse contract.");
    }
...
contract FlokiMetaverse is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
  ...
  uint256 public gasForProcessing = 300000;
    ...
    FlokiMetaverseReward public Reward;
   uint256 _totalSupply =10000000  * (10**9);
    address public teamMarketAdress =0x4f48D7B82C7445d530115BfFFD5CEa33909044A9;
...
uint256 public  marketFee = 1;
 ...
 DEXV2Router = _DEXV2Router;
        ...
        Reward = new FlokiMetaverseReward();
...
isTimelockLimited[owner()] = true;
 ...
 _mint(owner(), _totalSupply);
    ...
    function getTotalRewardHoldersDistributed() external view returns (uint256) {
        return Reward.totalRewardHoldersDistributed();
    }
...
emit RebaseUpdate(epoch, _totalSupply);
 ...
 function setDevWallet(address payable wallet) public masterWallet {
        teamMarketAdress = wallet;
    }
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "FlokiMetaverse: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "FlokiMetaverse: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "FlokiMetaverse: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FlokiMetaverse: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTFlokiMetaverse = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTFlokiMetaverse);
...
addLiquidity(tokensTFlokiMetaverse, bnbForLiquidity);
payable(teamMarketAdress).transfer(bnbForDev);
