...
GET BNB AS A HOLDING REWARD = EVERY 30 MINUTES

TELEGRAM: https://t.me/ColorBNB

WEBSITE: https://www.colorbnb.trade
...
contract COLORBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("COLORBNB_Dividend_Tracker", "COLORBNB_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ COLORBNB Tokens to be rewarded
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "COLORBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "COLORBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main COLORBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "COLORBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "COLORBNB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract COLORBNB is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    COLORBNBDividendTracker public dividendTracker;
    ...
    constructor() ERC20("COLORBNB", "C-BNB") {
        BNBRewardsFee = 10;
        liquidityFee = 2;
        extraFeeOnSell = 3; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0xca6E98c7a6759DE47cb7e5DEF42446Be6A999742);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new COLORBNBDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "COLORBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "COLORBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "COLORBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "COLORBNB: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "COLORBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "COLORBNB: Cannot update gasForProcessing to same value");
