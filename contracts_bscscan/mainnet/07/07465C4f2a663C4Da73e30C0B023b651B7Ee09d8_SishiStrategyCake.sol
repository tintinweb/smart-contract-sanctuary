...
import "../interfaces/CakeChef.sol";

contract SishiStrategyCake is Initializable {
    ...
    function deposit() public {
      ...
      if (_want > 0) {
        ...
        _want = IERC20Upgradeable(want).balanceOf(address(this));
        if (_want > 0) {
          _payFees(_want);
          _stakeCake();
        }
    ...
    function _stakeCake() internal {
      if(paused) return;
      uint256 _want = IERC20Upgradeable(want).balanceOf(address(this));
      IERC20Upgradeable(want).safeApprove(cakeChef, 0);
      IERC20Upgradeable(want).safeApprove(cakeChef, _want);
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
      ...
      _want = IERC20Upgradeable(want).balanceOf(address(this)).sub(_want).sub(_amount);
      if (_want > 0) {
        _payFees(_want);
      }
    ...
    function _buybackAndBurn(uint256 _rewardAmount) internal {
      if(_rewardAmount > 0 ) {
        IERC20Upgradeable(want).safeApprove(uniswapRouter, 0);
        IERC20Upgradeable(want).safeApprove(uniswapRouter, _rewardAmount);
    ...
    function harvest() public returns (uint harvesterRewarded) {
      ...
      uint _want = IERC20Upgradeable(want).balanceOf(address(this));
      if (_want > 0) {
        _payFees(_want);
        uint256 _harvesterReward = _want.mul(harvesterReward).div(FEE_DENOMINATOR);
        IERC20Upgradeable(want).safeTransfer(msg.sender, _harvesterReward);


        if(buyBackAndBurnFee > 0){
          uint256 _buybackfee = _want.mul(buyBackAndBurnFee).div(FEE_DENOMINATOR);
    ...
    function _payFees(uint256 _want) internal {
      ...
      IERC20Upgradeable(want).safeTransfer(IController(controller).rewards(), _fee);
      IERC20Upgradeable(want).safeTransfer(strategist, _reward);
    }
