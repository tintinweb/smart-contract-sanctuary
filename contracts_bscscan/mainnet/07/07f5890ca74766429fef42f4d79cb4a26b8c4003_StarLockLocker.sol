...
contract StarLockLocker is Ownable, ReentrancyGuard {
    ...
    function lockTokens(
        ...
        require(IBEP20(_tokenAddress).allowance(msg.sender, address(this)) >= _amount, "Target Token not approved");
        ...
        if (_feeInBnb) {
            ...
            uint256 fee = lockAmount.mul(lpFeeThousandth).div(1000).mul(depositper);
            lockAmount = lockAmount.sub(fee);

            if (tokensFees[_tokenAddress] == 0) {
                tokenAddressesWithFees.push(_tokenAddress);
                walletTokenBalance[_tokenAddress][msg.sender] = 0;
            }
