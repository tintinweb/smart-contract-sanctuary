...
contract Testing is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "TSG";	
    ...
    function excludeFromReward(address account) public onlyOwner() {
        // require(account != 0xDD31a0Ee1Dcb2a1b4BD08296492CDca5f937AE8b, 'We can not exclude Uniswap router.');
    ...
    //Setter for the FundFee which goes to the burnwallet in Tokens	
    ...
    //Setter for the FundFee which goes to a wallet in Tokens	
    function setSellFeePercent(uint256 _sellPercent) external onlyOwner() {	
        _previoussellFee = sellFee;	
        sellFee = _sellPercent;	
    }	
    ...
    function _transfer(	
        ...
        if(to == pancakeswapV2Pair && !_isExcludedFromFee[from]){
            uint256 sellFeeAmmount = amount.mul(sellFee).div(10**2);
            amount = amount.sub(sellFeeAmmount);
            _tokenTransfer(from,address(0),sellFeeAmmount,false);
