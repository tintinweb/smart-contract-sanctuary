...
contract MarsFlokiReward is RewardHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() RewardHolders("MarsFloki_Reward_Tracker", "MarsFloki_Reward_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MarsFloki_Reward_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MarsFloki_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MarsFloki contract.");
    }
...
contract MarsFloki is ERC20, Ownable {
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketFeeReciever=0x4bBE0a052BF769b7941E952934Abc63f498a387D;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Mars Floki", "MRF") {
...
emit RebaseUpdate(epoch, _totalSupply);
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MarsFloki: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MarsFloki: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MarsFloki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MarsFloki: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMarsFloki = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMarsFloki);
...
addLiquidity(tokensTMarsFloki, bnbForLiquidity);
