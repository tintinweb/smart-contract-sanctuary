...
library AdacashV1Library {
    using SafeMath for uint;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'AdacashV1Library: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'AdacashV1Library: ZERO_ADDRESS');
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IAdacashV1Pair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'AdacashV1Library: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'AdacashV1Library: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'AdacashV1Library: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'AdacashV1Library: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'AdacashV1Library: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'AdacashV1Library: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'AdacashV1Library: INVALID_PATH');
...
//   Adacash Factory Contract Interface
...
// Adacash Pair Contract Interface
...
// Adacash Router01 Contract Interface

interface IAdacashV1Router01 {
...
interface IAdacashV1Router02 is IAdacashV1Router01 {
...
// Adacash Router02 Contract

contract AdacashV1Router02 is IAdacashV1Router02 {
    using SafeMath for uint;
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'AdacashV1Router: EXPIRED');
        _;
    }
    ...
    function _addLiquidity( address tokenA, address tokenB, uint amountADesired, uint amountBDesired, uint amountAMin, uint amountBMin) internal virtual returns (uint amountA, uint amountB) {
        ...
        if (IAdacashV1Factory(factory).getPair(tokenA, tokenB) == address(0)) {
            IAdacashV1Factory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = AdacashV1Library.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = AdacashV1Library.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'AdacashV1Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = AdacashV1Library.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'AdacashV1Router: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity( address tokenA, address tokenB, uint amountADesired, uint amountBDesired, uint amountAMin, uint amountBMin, address to, uint deadline ) external virtual override ensure(deadline) returns (uint amountA, uint amountB, uint liquidity) {
        ...
        address pair = AdacashV1Library.pairFor(factory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = IAdacashV1Pair(pair).mint(to);
    }
    function addLiquidityETH( address token, uint amountTokenDesired, uint amountTokenMin, uint amountETHMin, address to, uint deadline ) external virtual override payable ensure(deadline) returns (uint amountToken, uint amountETH, uint liquidity) {
        ...
        address pair = AdacashV1Library.pairFor(factory, token, WETH);
    ...
    function removeLiquidity( address tokenA, address tokenB, uint liquidity, uint amountAMin, uint amountBMin, address to, uint deadline) public virtual override ensure(deadline) returns (uint amountA, uint amountB) {
        address pair = AdacashV1Library.pairFor(factory, tokenA, tokenB);
        IAdacashV1Pair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IAdacashV1Pair(pair).burn(to);
        (address token0,) = AdacashV1Library.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'AdacashV1Router: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'AdacashV1Router: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function removeLiquidityWithPermit( address tokenA, address tokenB, uint liquidity, uint amountAMin, uint amountBMin, address to, uint deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s ) external virtual override returns (uint amountA, uint amountB) {
        ...
        IAdacashV1Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = AdacashV1Library.sortTokens(input, output);
            uint amountOut = amounts[i + 1];
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            address to = i < path.length - 2 ? AdacashV1Library.pairFor(factory, output, path[i + 2]) : _to;
            IAdacashV1Pair(AdacashV1Library.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens( uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) returns (uint[] memory amounts) {
        amounts = AdacashV1Library.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'AdacashV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom( path[0], msg.sender, AdacashV1Library.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function swapTokensForExactTokens( uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) returns (uint[] memory amounts) {
        amounts = AdacashV1Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'AdacashV1Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, AdacashV1Library.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual override payable ensure(deadline) returns (uint[] memory amounts) {
        require(path[0] == WETH, 'AdacashV1Router: INVALID_PATH');
        amounts = AdacashV1Library.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'AdacashV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(AdacashV1Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline)returns (uint[] memory amounts){
        require(path[path.length - 1] == WETH, 'AdacashV1Router: INVALID_PATH');
        amounts = AdacashV1Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'AdacashV1Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, AdacashV1Library.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address to, uint deadline) external virtual override payable ensure(deadline) returns (uint[] memory amounts){
        require(path[0] == WETH, 'AdacashV1Router: INVALID_PATH');
        amounts = AdacashV1Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'AdacashV1Router: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(AdacashV1Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = AdacashV1Library.sortTokens(input, output);
            IAdacashV1Pair pair = IAdacashV1Pair(AdacashV1Library.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = AdacashV1Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens( uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, AdacashV1Library.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'AdacashV1Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    ) external virtual override payable ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(AdacashV1Library.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    ) external virtual override ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'AdacashV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
