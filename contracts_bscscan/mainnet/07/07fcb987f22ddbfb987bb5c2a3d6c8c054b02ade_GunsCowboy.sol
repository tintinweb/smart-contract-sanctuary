...
contract GunsCowboy is Ownable {
    ...
    event EmitShoot(address indexed player, uint256 number, uint256 damage, address indexed opponent, uint256 health);
   ...
   function hasOpponent(address player) public view returns (bool) {
    ...
    function setStats(uint256 health, uint256 damage, uint256 accuracy, uint256 defense) external {
        ...
        _power.accuracy = accuracy.add(500);
    ...
    function shoot() public {
        ...
        if (rndRndIdsReceived[game.rndAwaiting] == 0){
            // rnd numbers are set once if they are 0
            bytes32 requestId = rndRndIdsRequestIds[game.rndAwaiting];
            uint256 rnd = vrf.getRequest(requestId);
            // store received rnd for request id
            rndRndIdsReceived[game.rndAwaiting] = rnd;
        }
        uint256 number = rndRndIdsReceived[game.rndAwaiting];
        // compute random number in percent (1 - 100)
        uint256 _number = uint256(keccak256(abi.encode(number, game.gameId, game.health, gameOpponent.health, _stats.xp, block.timestamp))) % 1001;
        // compute damage
        uint256 damage; 
        ...
        // emit event
        emit EmitShoot(msg.sender, number,  damage, game.opponent, gameOpponent.health);
