...
contract MarsDoge is Context, IBEP20, Ownable {
    ...
    address private constant _marketingWallet = 0x94eD17A92A663D38906B5d6cD8c147f9A90e1682;
    address private constant _donationWallet = 0x7A00aE0fa3DBe138E7eD40c06251E68a5f830A16;

    address public immutable _burnAddress = 0x000000000000000000000000000000000000dEaD;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 10**5 * 10**16 * 10**9;

    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Mars Doge";
    string private _symbol = "Mars Doge";

    uint8 private  _decimals = 18;

    
    uint256 public  _taxFee       = 4; // redistributed to holders
    uint256 public  _liquidityFee = 6; // kept for liquidity
    uint256 public  _marketingFee = 1; // marketing wallet
    uint256 public  _burnFee      = 1; // burned
    uint256 public  _donationFee  = 1; // charity
    uint256 public _liquidityFeeSellExtra = 13; // sell fee
    ...
    uint256 private _previousLiquidityFeeSellExtra = _liquidityFeeSellExtra;

    uint256 public _maxTxAmount = 500 * 10**18 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 500 * 10**18 * 10**9;
    uint256 private _maxWalletToken = 500 * 10**18 * 10**9;
    ...
    function BanSniper(address account) external onlyOwner() {
    ...
    function setLiquidityExtraFeePercent(uint256 liquidityFeeEx) external onlyOwner {
        _liquidityFeeSellExtra = liquidityFeeEx;
        _previousLiquidityFeeSellExtra = liquidityFeeEx;
    }
  ...
  function setDonationFeePercent(uint256 donationFee) external onlyOwner {
    ...
    function removeAllFee() private {
        ...
        _previousLiquidityFeeSellExtra  = _liquidityFeeSellExtra;
    ...
    function restoreAllFee() private {
        ...
        _liquidityFeeSellExtra  = _previousLiquidityFeeSellExtra;
    }
    ...
    function _transfer(
        ...
        */
        ...
        takeMoreLpFee = false;
        if(takeFee && (to == address(uniswapV2Pair) || to == address(uniswapV2Router))) {
            require(!_isSniper[from], "Go snipe some other token");
            takeMoreLpFee = true;
        }
    ...
    function _getTValues(uint256 tAmount) private view returns (feeData memory) {
        ...
        fd.tLiquidity = calculateFee(tAmount, !takeMoreLpFee ? _liquidityFee : _liquidityFee.add(_liquidityFeeSellExtra));
        fd.tMarketing = calculateFee(tAmount, _marketingFee);
        fd.tDonation  = calculateFee(tAmount, _donationFee);
        ...
        fd.tTransferAmount = fd.tTransferAmount.sub(fd.tDonation);
    ...
    function _getRValues(feeData memory fd, uint256 currentRate) private pure returns (feeData memory) {
        ...
        fd.rDonation  = fd.tDonation.mul(fd.currentRate);
        ...
        fd.rTransferAmount = fd.rTransferAmount.sub(fd.rDonation);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        takeTransactionFee(address(_donationWallet), fd.tDonation, fd.currentRate);
