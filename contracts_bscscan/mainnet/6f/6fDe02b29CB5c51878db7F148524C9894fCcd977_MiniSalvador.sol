...
contract MiniSalvador is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public tradeDisableLock = false;
    BTCDividendTracker public btctrack;
    ...
    uint256 private antiBotBlockAmount;
    ...
    constructor(uint256 antiBlockBotAmountP) ERC20("miniSalvador", "$MSALA") {
        require(antiBlockBotAmountP >= 1 && antiBlockBotAmountP <= 10);
        // Anti bot block delay
        antiBotBlockAmount = antiBlockBotAmountP;
    	btctrack = new BTCDividendTracker();

    	marketingWallet = 0xa89F5A2B9f7DB752bFF3Cb2133f9e8Fa59C4Ad0D;
    	teamWallet = 0x393f459244bFa2E4f0aF1ccF93AA9DA713F52E2E;
        ethDividendToken = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;
        ...
        excludeFromDividend(address(btctrack));
    ...
    receive() external payable {
  	...
  	function setMaxBuyTransaction(uint256 _maxTxn) external onlyOwner {
          require(_maxTxn >= 1000000000 * (10**18), "MiniSalvador: Max Buy Must be greater than 1% of the supply (Anti HP 1.0.0A)");
  	    maxBuyTranscationAmount = _maxTxn * (10**18);
  	}
  	
  	function setMaxSellTransaction(uint256 _maxTxn) external onlyOwner {
        require(_maxTxn >= 600000000 * (10**18), "MiniSalvador: Max Sell Must be greater than .6% of the supply (Anti HP 1.0.0A)");
  	...
  	function updateBTCDividendToken(address _newContract) external onlyOwner {
  	    ...
  	    btctrack.setDividendTokenAddress(_newContract);
  	}
  	
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "MiniSalvador: Token is Already set to this address.");
    ...
    function initSC() external onlyOwner {
        ethDividendRewardsFee = 8;
        marketingFee = 4;
        ...
        // 1.2% of supply max buy.
        maxBuyTranscationAmount = 1200000000 * (10**18);
        // .3% of supply max sell.
        maxSellTransactionAmount = 800000000 * (10**18);
        // 4% of Supply Max Wallet Token.
        maxWalletToken = 4000000000 * (10**18);
    }
    
    function setTradingIsEnabled() external onlyOwner {
        require(tradeDisableLock != true, "MiniSalvador: Cannot recall a Suicidal Trade Enable.");
        tradeDisableLock = true;
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(ethDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setEthDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(buyBackAndLiquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBTCDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "MiniSalvador: Must be less than 6%");
    ...
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = buyBackAndLiquidityFee.add(ethDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MiniSalvador: Current AMM router is already the inputted router.");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "MiniSalvador: Account Already Excluded.");
    ...
    function excludeFromDividend(address account) public onlyOwner {
        btctrack.excludeFromDividends(address(account));
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MiniSalvador: Pair is equal to the set value");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "MiniSalvador: Pair is equal to the set value");
        automatedMarketMakerPairs[pair] = value;

        if(value) {
            btctrack.excludeFromDividends(pair);
        }
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "MiniSalvador: Processing Gas must be not equal to the current value.");
        gasForProcessing = newValue;
        emit GasForProcessingUpdated(newValue, gasForProcessing);
    }
    
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        btctrack.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        require(claimWait <= 720000, "MiniSalvador: ClaimWait cannot exceed a large number");
        btctrack.updateClaimWait(claimWait);
    }

    function getEthClaimWait() external view returns(uint256) {
        return btctrack.claimWait();
    }

    function getTotalEthDividendsDistributed() external view returns (uint256) {
        return btctrack.totalDividendsDistributed();
    }
    ...
    function getAccountethDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return btctrack.getAccount(account);
    }
	...
	function getAccountethDividendsInfoAtIndex(uint256 index)
        external view returns (
            ...
            uint256,
            uint256) {
    	return btctrack.getAccountAtIndex(index);
    }
    

	function processDividendTracker(uint256 gas) external onlyOwner {		
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = btctrack.process(gas);
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "MiniSalvador: Only Excluded Accounts can Trade while trade disabled.");
        ...
        if (
            ...
            require(
                ...
                "MiniSalvador: Max Buy Is greater than the current allowment"
            );
            
            uint256 contractBalanceRecepient = balanceOf(to);
            require(
                contractBalanceRecepient + amount <= maxWalletToken,
                "MiniSalvador: Max Wallet Exceedes the Current amount"
            );
        } else if (
        	tradingIsEnabled &&
            ...
            require(amount <= maxSellTransactionAmount, "MiniSalvador: Sell exceedes the current Max Sell");
            ...
            if (!swapping && canSwap) {
                ...
                if (ethDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee.add(ethDividendRewardsFee)).mul(ethDividendRewardsFee);
        ...
        if(takeFee) {
            uint256 fees;
            // Block based Antibot.
        if((launchedAt + antiBotBlockAmount >= block.number) && !excludedAccount){
            fees = amount.div(100).mul(99);
         }else{
            fees = amount.div(100).mul(totalFees);
        }

            // On Sell, Increase the tokenBal sent to the SC
            // NOTE: We don't have to worry on anti bot on sells using the max tax method :^)
        ...
        try btctrack.setBalance(payable(from), balanceOf(from)) {} catch {}
        try btctrack.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
	    	uint256 gas = gasForProcessing;
	    	
	    	try btctrack.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        // make the swap
        // TODO: Implement Slippage Tracking to Help prevent chart dumping.
    ...
    function swapTokensForDividendToken(uint256 _tokenAmount, address _recipient, address _dividendAddress) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            _tokenAmount,
            0, // TODO: Implement Slippage to help prevent price dumping.
            path,
            _recipient,
            block.timestamp
        );
    ...
    function swapAndSendBTCDividends(uint256 tokens) private {
        ...
        transferDividends(ethDividendToken, address(btctrack), btctrack, ethDividends);
    }
...
contract BTCDividendTracker is DividendPayingToken{
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SALVA_Dividend_Tracker", "SALVA_Dividend_Tracker", 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); // User Must Hold 200000 Tokens to qualify for dividends
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SALVA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SALVA_Dividend_Tracker: withdrawDividend disabled. Claim thru the main SC.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SALVA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SALVA_Dividend_Tracker: Cannot update claimWait to same value");
