...
contract Team is owned{
    ...
    */
     ...
     *lock tokens
    */
    function TeamLock(address _tokenAddress, uint256 _amount, uint256 _vestingTime) public returns (uint256 _id) {
        require(_amount > 0, 'token amount is Zero');
        require(_vestingTime < 1000000000000000, 'Enter an unix timestamp in seconds, not miliseconds');
        ...
        lockedToken[_id].vestingTime = _vestingTime-block.timestamp;
    ...
    */
    function withdrawTokens(uint256 _id) public {
        require(block.timestamp >= lockedToken[_id].vestingTime, 'Tokens are locked');
    ...
    function getDepositDetails(uint256 _id) view public returns (address, address, uint256, uint256, bool)
    {
        ...
        lockedToken[_id].vestingTime,lockedToken[_id].withdrawn);
    }
