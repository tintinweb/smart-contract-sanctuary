...
contract SapBondingCalculator is IBondingCalculator {

    using FixedPoint for *;
    using SafeMath for uint;
    using SafeMath for uint112;

    address public immutable SAP;

    constructor( address _SAP ) {
        require( _SAP != address(0) );
        SAP = _SAP;
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == SAP ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( SAP ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}