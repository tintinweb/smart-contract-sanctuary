...
contract Test3 is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x008e28225E7A1BeCA338D7DF21F7a61fbEA6F9bd); // Marketing Address
    address payable public houseWalletAddress = payable(0xB50B16b7B7fa3F4ec1Adf658FD5023D6c724EcF1); // house Address
    ...
    uint256 public _housewalletFee = 2;
    ...
    constructor () {
        ...
        _totalTaxPercent = _liquidityFee.add(_marketingFee).add(_housewalletFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_housewalletFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_housewalletFee)).mul(_marketingFee);
            uint256 HouseWalletShare = newBalance.sub(marketingShare);
            
            if(marketingShare > 0)
                transferToAddressETH(marketingWalletAddress, marketingShare);
            
            if(HouseWalletShare > 0)
                transferToAddressETH(houseWalletAddress, HouseWalletShare);
        }
    ...
    function setTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newHouseWalletTax) external onlyOwner() {
        _liquidityFee = newLiquidityTax;
        _marketingFee = newMarketingTax;
        _housewalletFee = newHouseWalletTax;
        _totalTaxPercent = _liquidityFee.add(_marketingFee).add(_housewalletFee);
        _prevTotalTaxPercent = _totalTaxPercent;
    }
    ...
    function sethouseWalletAddress(address newAddress) external onlyOwner() {
        houseWalletAddress = payable(newAddress);
    }
