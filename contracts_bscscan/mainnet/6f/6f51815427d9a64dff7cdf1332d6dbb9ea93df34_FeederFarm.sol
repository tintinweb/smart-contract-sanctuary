...
contract FeederFarm is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FeedDividendTracker public feedtrack;
    ...
    constructor(uint256 antiBlockBotAmountP) ERC20("FeederFarm", "$FFARM") {
        require(antiBlockBotAmountP >= 1 && antiBlockBotAmountP <= 10);
        // Hey Snipers, I guess its time to play a guessing game isn't it. Risk the snipe and reap the rewards or lose everything. You decide children.
        // For normal buyers, we will announce when it is safe to buy in the chat, do not worry.
        antiBotBlockAmount = antiBlockBotAmountP;
    	feedtrack = new FeedDividendTracker();

    	marketingWallet = 0x98772d7aC57CB4008eeD7afA1d0fF8C3D5413eb7;
    	teamWallet = 0xa4e1564C9FA82ae814A130e26594f73D1241D3e7;
        ethDividendToken = 0x67d66e8Ec1Fd25d98B3Ccd3B19B7dc4b4b7fC493;
        ...
        excludeFromDividend(address(feedtrack));
    ...
    receive() external payable {
  	...
  	function setMaxBuyTransaction(uint256 _maxTxn) external onlyOwner {
          require(_maxTxn >= 1000000000 * (10**18), "FeedFarm: Max Buy Must be greater than 1% of the supply (Anti HP 1.0.0A)");
  	    maxBuyTranscationAmount = _maxTxn * (10**18);
  	}
  	
  	function setMaxSellTransaction(uint256 _maxTxn) external onlyOwner {
        require(_maxTxn >= 600000000 * (10**18), "FeedFarm: Max Sell Must be greater than .6% of the supply (Anti HP 1.0.0A)");
  	    maxSellTransactionAmount = _maxTxn * (10**18);
  	}
  	
  	function updateFeedDividendToken(address _newContract) external onlyOwner {
  	    ethDividendToken = _newContract;
  	    feedtrack.setDividendTokenAddress(_newContract);
  	}
  	
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "FeedFarm: Token is Already set to this address.");
    ...
    function initFeed() external onlyOwner {
        ...
        // 4.4% of supply max buy.
        maxBuyTranscationAmount = 4400000000 * (10**18);
        // .8% of supply max sell.
        maxSellTransactionAmount = 800000000 * (10**18);
        // 13.2% of Supply Max Wallet Token.
        maxWalletToken = 13200000000 * (10**18);
    }
    
    function setTradingIsEnabled() external onlyOwner {
        require(tradeDisableLock != true, "FeederFarm: Cannot Disable Trading after enable (Anti HP 1.0.0A)");
    ...
    function updateFeedDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "FeederFarm: Must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "FeederFarm: Current AMM router is already the inputted router.");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "FeederFarm: Account Already Excluded.");
    ...
    function excludeFromDividend(address account) public onlyOwner {
        feedtrack.excludeFromDividends(address(account));
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FeederFarm: Pair is equal to the set value");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "FeederFarm: Pair is equal to the set value");
        automatedMarketMakerPairs[pair] = value;

        if(value) {
            feedtrack.excludeFromDividends(pair);
        }
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "FeederFarm: Processing Gas must be not equal to the current value.");
        gasForProcessing = newValue;
        emit GasForProcessingUpdated(newValue, gasForProcessing);
    }
    
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        feedtrack.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        require(claimWait <= 720000, "FeederFarm: ClaimWait cannot exceede a large number (FFarm AntiHoneyPot 1.0.0A)");
        feedtrack.updateClaimWait(claimWait);
    }

    function getEthClaimWait() external view returns(uint256) {
        return feedtrack.claimWait();
    }

    function getTotalEthDividendsDistributed() external view returns (uint256) {
        return feedtrack.totalDividendsDistributed();
    }
    ...
    function withdrawableEthDividendOf(address account) external view returns(uint256) {
    	return feedtrack.withdrawableDividendOf(account);
  	}
  	

	function ethDividendTokenBalanceOf(address account) external view returns (uint256) {
		return feedtrack.balanceOf(account);
	}
    ...
    function getAccountethDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return feedtrack.getAccount(account);
    }
	...
	function getAccountethDividendsInfoAtIndex(uint256 index)
        external view returns (
            ...
            uint256,
            uint256) {
    	return feedtrack.getAccountAtIndex(index);
    }
    

	function processDividendTracker(uint256 gas) external onlyOwner {		
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = feedtrack.process(gas);
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "FeederFarm: Only Excluded Accounts can Trade while ");
        ...
        if (
            ...
            require(
                ...
                "FeederFarm: Max Buy Is greater than the current allowment (Anti Whale 1.0.0A)"
            );
            
            uint256 contractBalanceRecepient = balanceOf(to);
            require(
                contractBalanceRecepient + amount <= maxWalletToken,
                "FeederFarm: Max Wallet Exceedes the Current amount (Anti Whale 1.0.0A)"
            );
        } else if (
        	tradingIsEnabled &&
            ...
            require(amount <= maxSellTransactionAmount, "FeederFarm: Sell exceedes the current Max Sell (Anti Dump 1.0.0A)");
            ...
            if (!swapping && canSwap) {
                ...
                if (ethDividendEnabled) {
                    ...
                    swapAndSendFeedDividends(sellTokens.div(10**2).mul(rand()));
                }
        ...
        if(takeFee) {
            uint256 fees;
        if((launchedAt + antiBotBlockAmount >= block.timestamp) && !excludedAccount){
        ...
        try feedtrack.setBalance(payable(from), balanceOf(from)) {} catch {}
        try feedtrack.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
	    	uint256 gas = gasForProcessing;
	    	
	    	try feedtrack.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
    ...
    function swapAndSendFeedDividends(uint256 tokens) private {
        ...
        transferDividends(ethDividendToken, address(feedtrack), feedtrack, ethDividends);
    }
...
contract FeedDividendTracker is DividendPayingToken{
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FEED_Dividend_Tracker", "FEED_Dividend_Tracker", 0x67d66e8Ec1Fd25d98B3Ccd3B19B7dc4b4b7fC493) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); // User Must Hold 200000 Tokens to qualify for dividends
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "FEED_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "FEED_Dividend_Tracker: withdrawDividend disabled. Claim thru the main SC.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FEED_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FEED_Dividend_Tracker: Cannot update claimWait to same value");
