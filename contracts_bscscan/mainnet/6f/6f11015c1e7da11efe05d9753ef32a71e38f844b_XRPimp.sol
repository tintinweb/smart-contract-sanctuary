...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also XRPimped.
...
// File: contracts/XRPimpDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable XRPToken = address(0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE);
  ...
  function distributeXRPTokenDividends(uint256 amount) public {
    require(totalSupply() > 0);
...
contract XRPimp is ERC20, Ownable {
    ...
    address public immutable XRPToken = address(0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE); // XRPToken
	address payable public MarketingWallet = payable(0x8A3A49022F6bf84e97649705ab8D23C6FA3E4768); // Marketing Address
    ...
    constructor() ERC20("XRPimp", "XRPimp")public{
        uint256 _XRPRewardsFee = 12;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeSwapV2Pair, "XRPimp: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "XRPimp: The liquidity wallet is already this address");
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingOpen) {
            require(canTransferBeforetradingOpen[from], "XRPimp: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForXRPToken(uint256 tokenAmount, address recipient) private {
       
        // generate the PancakeSwap pair path of weth -> XRPToken
        ...
        PancakeSwapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of XRPToken
            path,
            recipient,
            block.timestamp
        );
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForXRPToken(tokens, address(this));
        ...
        if (success) {
            dividendTracker.distributeXRPTokenDividends(dividends);
...
contract XRPimpDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("XRPimp_Dividend_Tracker", "XRPimp_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "XRPimp_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "XRPimp_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
