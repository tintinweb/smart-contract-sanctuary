...
contract PandoraERC1155 is ERC1155, SafeOwnable {
    using SafeMath for uint256;
    ...
    mapping(uint256 => bool) public enableTransfer;
    string public name;
    ...
    function createBatchDefault(uint256[] memory _types, uint256[] memory _values, bool _enableTransfer) external returns (uint256[] memory tokenIds) {
        return createBatch(uint(-1), 0, _types, _values, _enableTransfer, new bytes(0));
    }
    ...
    { 
        if (from != address(0) && to != address(0)) {
            for (uint i = 0; i < ids.length; i ++) {
                require(amounts[i] == 0 || enableTransfer[ids[i]], "Token Transfer Disabled");
            }
        }
    }
