...
contract FounderNFTMarketPlace is Initializable, PausableUpgradeable, OwnableUpgradeable, IAddFounderNFTForSale, IERC721ReceiverUpgradeable {
    ...
    uint256 public currentTokenIndexForSale;
    function initialize() public initializer {
        __Pausable_init();
        __Ownable_init();

        _founderNFTPrice = 0; // founder nft price
        currentTokenIndexForSale = 0;
    }
    ...
    function setCurrentTokenIndexForSale(uint256 index ) public onlyOwner {
        currentTokenIndexForSale = index;
    }
    ...
    function buyFounderNFT() external whenNotPaused {
        ...
        uint256 tokenId = _founderNFTTokenId[currentTokenIndexForSale]; // get the last token id from the list
        _founderNFTTokenId[currentTokenIndexForSale] = 0; // set to zero value it does not reduce the size of list

        require(tokenId>0,"Token ID is zero");
        
        currentTokenIndexForSale = currentTokenIndexForSale.add(1); // increment the token index for sale
    ...
    function getTokenIdByIndex(uint256 indx) external view returns (uint256) {
        uint256 tokenId = _founderNFTTokenId[indx];
        return tokenId;
    }
...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.2;
interface IAddFounderNFTForSale {
    function addFounderNFTToList(uint256 tokenId) external;
}
