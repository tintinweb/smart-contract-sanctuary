...
interface IDividendDistributor {
    ...
    function setMinBnbBeforeSwap(uint256 _min) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minBnbBeforeSwap = 1 ether;
...
//    }
    ...
    function shouldSwap() internal returns (bool) {
        return address(this).balance > minBnbBeforeSwap;
    }

    function setMinBnbBeforeSwap(uint256 _min) external override onlyToken {
        minBnbBeforeSwap = _min;
    }
    ...
    function shouldProcess() internal returns (bool) {
        return SM.balanceOf(address(this)) > 0;
    }
...
//
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal swapping returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator);
        _balances[address(this)] = _balances[address(this)].add(feeAmount);

        if (sender != pair) {
            uint liqAmount = feeAmount.mul(liquidityFee).div(totalFee).div(2);
            uint256 amountBNB = getBNB(feeAmount-liqAmount);
            uint256 amountBNBLiquidity = amountBNB.mul(liquidityFee).div(totalFee).div(2);
            ...
            emit AutoLiquify(amountBNBLiquidity, liqAmount);
        }
...
//        emit Transfer(sender, address(this), feeAmount);
    ...
    function getBNB(uint256 amountToSwap) internal returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNB =  getBNB(amountToSwap);
    ...
    function setMinBnbBeforeSwap(uint _min) external authorized {
        distributor.setMinBnbBeforeSwap(_min);
    }
