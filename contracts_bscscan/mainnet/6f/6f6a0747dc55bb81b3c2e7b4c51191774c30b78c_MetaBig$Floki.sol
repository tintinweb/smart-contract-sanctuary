...
/**
MetaBig$Floki is a deflationary BEP-20 Token Based on Binance Smartchain that uses both the reflection model and dividen model to benefit holders and guarantee the project longevity.
https://t.me/metabigfloki
*/
...
contract MetaBig$Floki is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MetaBig$FlokiDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 private maxBuyTranscationAmount = 1000000000000000 * (10**18);
    uint256 private maxSellTransactionAmount = 1000000000000000 * (10**18);
    ...
    constructor() ERC20("MetaBig$Floki", "MetaBig$Floki") {
    	...
    	dividendTracker = new MetaBig$FlokiDividendTracker();

    	buyBackWallet = 0x19F31a054c8C80FeA2d500A38d6320a3e49967D6;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0xfb380198e4BD2e5a504d2509F9Ac8196CF452Ac1
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "MetaBig$Floki: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "MetaBig$Floki: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	    sellFeeIncreaseFactor = divisor;
  	}
  	
  	function FeeOff() external onlyOwner {
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "MetaBig$Floki: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MetaBig$Floki: The dividend tracker already has that address");

        MetaBig$FlokiDividendTracker newDividendTracker = MetaBig$FlokiDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MetaBig$Floki: The new dividend tracker must be owned by the MetaBig$Floki token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "MetaBig$Floki: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaBig$Floki: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaBig$Floki: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MetaBig$Floki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaBig$Floki: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "MetaBig$Floki: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MetaBig$Floki: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaBig$Floki: Cannot update gasForProcessing to same value");
...
contract MetaBig$FlokiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MetaBig$Floki_Dividend_Tracker", "MetaBig$Floki_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 100000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MetaBig$Floki_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MetaBig$Floki_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaBig$Floki contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "MetaBig$Floki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaBig$Floki_Dividend_Tracker: Cannot update claimWait to same value");
