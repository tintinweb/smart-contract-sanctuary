...
// nanoBTC @ https://t.me/NanoBTC_BSC
...
// File: contracts/nanoBTCDividendPayingToken.sol
...
contract nanoBTC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
    address private _marketingWallet = address(0x5c655538909D867ec050336FBF804D2bb3911e7a);
    ...
    nanoBTCDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("nanoBTC", "nanoBTC") {
    	...
    	dividendTracker = new nanoBTCDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "nanoBTC: The dividend tracker already has that address");

        nanoBTCDividendTracker newDividendTracker = nanoBTCDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "nanoBTC: The new dividend tracker must be owned by the nanoBTC token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "nanoBTC: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "nanoBTC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "nanoBTC: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "nanoBTC: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "nanoBTC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "nanoBTC: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "nanoBTCBSC: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "nanoBTC: This account cannot send tokens until trading is enabled");
        }
...
contract nanoBTCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("nanoBTC_Dividend_Tracker", "nanoBTC_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "nanoBTC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "nanoBTC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main nanoBTC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "nanoBTC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "nanoBTC_Dividend_Tracker: Cannot update claimWait to same value");
