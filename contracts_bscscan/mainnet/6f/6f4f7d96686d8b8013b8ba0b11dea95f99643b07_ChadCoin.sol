...
/**
ChadCoin is for all the sweet dudes and dudettes who just love to kick it with their buds, crack open a brewski, and enjoy life for all its living.
...
contract ChadCoin is Context, IERC20, Ownable {
  ...
  address payable vaultWallet = 0x5647159F3E841072f76a8EdD934287bcDD82b814;
  ...
  uint256 public _maxTxAmount = 6666666 * 10**18;
  uint256 private numTokensSellToAddToLiquidity = 333333 * 10**18;
  ...
  constructor (address _dexAddress) public {
      ...
      IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_dexAddress);// v2 testnet 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
  ...
  function _takeDev(uint256 tDev) private {
      uint256 currentRate =  _getRate();
      uint256 rDev = tDev.mul(currentRate);
      _rOwned[vaultWallet] = _rOwned[vaultWallet].add(rDev);
      if(_isExcluded[vaultWallet])
          _tOwned[vaultWallet] = _tOwned[vaultWallet].add(tDev);
  }
  ...
  function _transfer(
    ...
    function testSendEth(uint256 _amount) public {
        sendEthToVault(_amount);
    }

    function sendEthToVault(uint256 _amount) internal {
        vaultAddress.transfer(_amount);
    }
    
    //switched to public for testing, remember to lock back to private, maybe same with xfer to vault
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        ...
        uint256 transferBalance = preTransferBalance.div(2);
        sendEthToVault(transferBalance);
  ...
  function setVaultWallet(address payable newWallet) external onlyOwner(){
