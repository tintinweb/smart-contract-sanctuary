...
// File: contracts/libs/IAppleReferral.sol

pragma solidity 0.6.12;

interface IAppleReferral {
    /**
     * @dev Record referral.
     */
...
contract AppleToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 APPLEs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to AppleSwap's router when our own AMM release
    IUniswapV2Router02 public appleSwapRouter;
    // The trading pair
    address public appleSwapPair;
    // In swap and liquify
    ...
    event AppleSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "APPLE::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the APPLE contract.
     */
    constructor() public BEP20("AppleSwap", "APPLE") {
    ...
    /// @dev overrides transfer function to meet tokenomics of APPLE
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(appleSwapRouter) != address(0)
            && appleSwapPair != address(0)
            && sender != appleSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "APPLE::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "APPLE::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the AppleSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = appleSwapRouter.WETH();

        _approve(address(this), address(appleSwapRouter), tokenAmount);

        // make the swap
        appleSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        appleSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from appleSwapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "APPLE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "APPLE::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "APPLE::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateAppleSwapRouter(address _router) public onlyOperator {
        appleSwapRouter = IUniswapV2Router02(_router);
        appleSwapPair = IUniswapV2Factory(appleSwapRouter.factory()).getPair(address(this), appleSwapRouter.WETH());
        require(appleSwapPair != address(0), "APPLE::updateAppleSwapRouter: Invalid pair address.");
        emit AppleSwapRouterUpdated(msg.sender, address(appleSwapRouter), appleSwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "APPLE::transferOperator: new operator is the zero address");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // APPLE referral contract address.
    IAppleReferral public appleReferral;
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(appleReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            appleReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingApple(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(apple)) {
                uint256 transferTax = _amount.mul(apple.transferTaxRate()).div(10000);
    ...
    // Update the APPLE referral contract address by the owner
    function setAppleReferral(IAppleReferral _appleReferral) public onlyOwner {
        appleReferral = _appleReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(appleReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = appleReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                apple.mint(referrer, commissionAmount);
                appleReferral.recordReferralCommission(referrer, commissionAmount);
