...
//BSC Core Guild Vault.Swap BNB to YGT claim token to earn Yields!
...
contract TapSwap is BEP20('Yield Guild Claim Token', 'YGT') {
...
contract ILO {
    ...
    TapSwap public token;
    address public owner;
    bool isActive=false;
    
    //price of per token in BNB
    uint pricePerToken=2100000000000000;

    uint public availableToken;
    uint MinPurchase=1000000000000000000; // 1 YGT
    ...
    function initialise(TapSwap _token,uint _availabletoken) public onlyOwner{
        token=_token;
    ...
    function WithdrawToken(uint _amount) external onlyOwner{
        ...
        TapSwap(token).transfer(msg.sender,_amount);
    }
