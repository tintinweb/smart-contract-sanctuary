...
This is a community token so u all need to creat the tg and twitter and everything the point of creating this 
is to teach ppl how to hold and help in each coin they r in, so u all need to work for u all bags 
and don't worry I'm not gonna rug or scam u money I'm just a person that want to build a safe space 
<<<<<< goodluck >>>>> see u all in the moon!

<<<<<<<<<<< I will Lock the Lp for 1 day if I see the community working and doing the job I will extend the lock time >>>>>>>>

💰💰💰💰>>>>>>>>>>>> TRX REWARD <<<<<<<<<<<💰💰💰💰


>>>>>>> sell tax will be 2x in the first hour
also will be randomly added if I see sales killing the chart <<<<<<<<<<<


there will be a buyback to keep the chart up
...
// CHANGE
contract TRONX is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "TRONX";
    string constant _symbol = "$TRONX";
    ...
    uint256 public _sellMultiplier = 2; // 2x for sells in the first hour... manually handled
