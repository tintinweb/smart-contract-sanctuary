...
// File: contracts/library/StringUtil.sol
...
// File: contracts/nft/ERC721Token.sol
...
contract PandaERC721Token is ERC721, Governance, IERC2981 {
    ...
    constructor() public ERC721("ThePandaTokenNFT", "PANDANFT") {
        _setBaseURI("https://r360.pandahausfutbol.com/coil-web/api/bsc/721/pandanft/");
    ...
    modifier creatorOnly(uint256 _tokenId) {
        require(
            ...
            "ERC721Token: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
...
//    e.g. _royalty = 1000000; // 10%
    function mint(address _to, uint256 _tokenId, uint256 _royalty) public {
        require(_minters[msg.sender], "!minter");
        require(_royalty > 0, "Royalty value should be a positive number");
        require(_royalty < 2500000, "Royalty value should less than 25%");


        _mint(_to, _tokenId);
        _setTokenURI(_tokenId, string(abi.encodePacked(StringUtil.uintToString(_tokenId), "")));
        _royaltyInfos[_tokenId].creator = _to;
        _royaltyInfos[_tokenId].amount = _royalty;
    }
