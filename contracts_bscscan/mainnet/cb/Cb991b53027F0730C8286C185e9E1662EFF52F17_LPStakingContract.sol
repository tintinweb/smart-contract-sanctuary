...
contract LPStakingContract is Ownable, ReentrancyGuard {
    ...
    constructor(address _protocolToken, address _lpToken, address _distributor, address _DAO, address _stakingContract, uint256 _initRewards, uint256 _claimTimelock, uint256 _prestakeTimestamp, uint256 _bonusRewardTimestamp) {
        ...
        require(_stakingContract != address(0), 'Should not be the zero address');
    ...
    function setProtocolStakingAllowance(uint256 amount) external onlyOwner {
        IERC20(protocolToken).approve(StakingContract, amount);
    }
    ...
    function deposit(address recipient, uint256 amount) public {
        ...
        if (operator != address(this) && operator != owner()) {
            IERC20(LPToken).safeTransferFrom(operator, address(this), amount); 
        }
    ...
    function claimInterest(uint256[] calldata depositIds, bool compound) external nonReentrant returns(uint256) {
        ...
        if (compound == true) {
            ...
            IStaking(StakingContract).deposit(depositor, amountClaimed);
    ...
    function autoCompoundInterest(address depositor, uint256[] calldata depositIds) external onlyOwner {
        ...
        IStaking(StakingContract).deposit(depositor, netAmount);
        //deposit(depositor, netAmount);

        IERC20(protocolToken).safeTransfer(DAO, compoundFee);
    ...
    function withdraw(uint256[] calldata depositIds) external nonReentrant {
        ...
        IERC20(LPToken).safeTransfer(depositor, amountPrincipal);
