...
contract TimerrWatch is ERC721Metadata {
    ...
    modifier periodStarted() {
        require(block.timestamp >= startTime && startTime != 0, "TimerrWatch: Period not started");
        _;
    }
    ...
    constructor(
        string memory baseURI_,
        address admin_
    ) ERC721Metadata("TimerrWatch", "TimerrWatch", baseURI_) {
        _admin = admin_;
    }

    function setStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime > 0, "TimerrWatch: invalid _startTime");
    ...
    function presalePurchase(uint256 numberOfNfts) external payable onlyWhitelisted{
        ...
        require(pendingCount > 0, "TimerrWatch: All minted");
        require(numberOfNfts > 0, "TimerrWatch: numberOfNfts cannot be 0");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"TimerrWatch: sale already ended");
        require(MINT_PRICE * (numberOfNfts) == msg.value, "TimerrWatch: invalid ether value");
    ...
    function purchase(uint256 numberOfNfts) external payable periodStarted {
        require(pendingCount > 0, "TimerrWatch: All minted");
        require(numberOfNfts > 0, "TimerrWatch: numberOfNfts cannot be 0");
        require(numberOfNfts <= 20, "TimerrWatch: You may not buy more than 20 NFTs at once");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"TimerrWatch: sale already ended");
        if(presaleEnabled) require(isWhitelisted[msg.sender], "You are not whitelisted");
        if(tax_freeUsers[msg.sender] == 0) require(MINT_PRICE * (numberOfNfts) == msg.value, "TimerrWatch: invalid ether value");
        else if (tax_freeUsers[msg.sender] < numberOfNfts){
            require(MINT_PRICE * (numberOfNfts -  tax_freeUsers[msg.sender]) == msg.value, "TimerrWatch: invalid ether value");
    ...
    function claimReward(uint256 tokenId) public {
        require(
            ...
            "TimerrWatch: Only owner or approved can claim rewards"
        );
    ...
    function _randomMint(address _to) internal returns (uint256) {
        require(totalSupply() < MAX_NFT_SUPPLY, "TimerrWatch: max supply reached");
