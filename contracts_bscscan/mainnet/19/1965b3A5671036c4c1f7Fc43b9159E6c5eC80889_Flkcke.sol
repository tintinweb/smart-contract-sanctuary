...
contract Flkcke is IBEP20, Auth {

    using SafeMath for uint256;

    string constant _name = "Flkck";
    string constant _symbol = "Flkck";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0x7d6194dcf3f1eef5886AaE86FE37657e9BD885fe;  // marketingwallet
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        if(_allowances[sender][msg.sender] != uint256(-1)){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "FlokiCake: Insufficient Allowance");
        }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(_isBlacklisted[sender] == false, "FlokiCake: You are banned");
        require(_isBlacklisted[recipient] == false, "FlokiCake: The recipient is banned");

        if(!authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen, "FlokiCake: Trading is not open yet");
        }

        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "FlokiCake: Transaction Limit Exceeded");
        ...
        //Exchange tokens
        _balances[sender] = _balances[sender].sub(amount, "FlokiCake: Insufficient Balance");
