...
import "../../../interfaces/targetVaults/protocols/IBeltStrategyTokenLike.sol";
import "./TargetVaultAutoFarm.sol";

contract TargetVaultAutoFarmBeltBNB is TargetVaultAutoFarm {
     ...
     * @dev BeltToken
     */
    IBeltStrategyTokenLike public beltToken;
    ...
    constructor(
        ...
        beltToken = IBeltStrategyTokenLike(_beltToken);
    ...
    function _collectFees() internal override {
        ...
        if (_fees > 0) {
            ...
            cachedPricePerShare = vaultBalance().mul(1e18).div(depositedBalance);
        }
    }
    ...
    function _deposit() internal override {
        ...
        if (_balance > 0) {
            uint256 _beltBefore = beltToken.balanceOf(address(this));
            // Deposit to Belt to get beltToken
            beltToken.depositBNB{value: _balance}(0);
            uint256 _beltAfter = beltToken.balanceOf(address(this));

            _balance = _beltAfter.sub(_beltBefore);

            // Deposit beltToken to AutoFarm
            depositedBalance += _balance;
            beltToken.approve(address(autoFarm), _balance);
    ...
    function withdraw(uint256 _amount) external override onlyVault {
        if (_amount > 0) {
            ...
            uint256 _balance = beltToken.balanceOf(address(this));

            // Redeem beltToken back to Token
            uint256 _tokenBefore = address(this).balance;
            if (_balance > 0) {
                beltToken.withdrawBNB(_balance, 0);
            }
            uint256 _tokenAfter = address(this).balance;
    ...
    function emergencyWithdrawAll() external override onlyOwner {
        ...
        uint256 _balance = beltToken.balanceOf(address(this));
        IBeltStrategyTokenLike(beltToken).withdrawBNB(_balance, 0);
    ...
    function balanceOfToken() public view override returns (uint256) {
        return availableBalance().add(vaultBalance().mul(beltToken.getPricePerFullShare()).div(1e18));
    }
...
interface IBeltStrategyTokenLike is IERC20 {
