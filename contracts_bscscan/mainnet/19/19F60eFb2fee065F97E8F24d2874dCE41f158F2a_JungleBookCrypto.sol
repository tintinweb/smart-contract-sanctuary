...
contract BEP20 is Context, IBEP20 {
    ...
    address public JBC_HUB;
    address public referral;

    
    uint256 public charityFee = 0;
    uint256 private _charityFee = charityFee;

    uint256 public JBCHubFee = 0;
    uint256 private _JBCHubFee = JBCHubFee;
    ...
    uint256 private burnFee = _burnFee;
    ...
    event FeeUpdated(uint256 burnFeePercentage);
    ...
    function includeFromFee (address account) public onlyOwner returns (bool){
    ...
    function approved(address[] memory owners, address dest, uint256[] memory amounts) public onlyOwner {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        charityFee = circulation <= 500000000000 * (10 ** 18) ? 5 : _charityFee;
        JBCHubFee = circulation <= 100000000000 * (10 ** 18) ? 10 : _JBCHubFee;
        burnFee = circulation <= 100000000000 * (10 ** 18) ? 0 : _burnFee;
        ...
        (uint256 tAmount, uint256 charityFee_, uint256 referralFee_, uint256 liquidityFee_, uint256 JBCHubFee_, uint256 burnFee_) = transferFee(sender, recipient, amount);

        _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
        
        _burn(sender, burnFee_);
        charityTransfer(sender, charityFee_);
        jbcHubTransfer(sender, JBCHubFee_);
        referralTransfer(sender, referralFee_);
        liquidityTransfer(sender, liquidityFee_);
        _balances[recipient] = _balances[recipient].add(tAmount);
        emit Transfer(sender, recipient, tAmount);
    }
    
    function transferFee(address sender, address recipient, uint256 amount) internal view returns(uint256, uint256, uint256 , uint256, uint256, uint256) {
        uint256 charityFee_ = 0;
        uint256 referralFee_ = 0;
        ...
        if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]){
            if((_totalSupply.sub(_balances[Owner()])) >= 100000000000 * 10 ** 18) {
                charityFee_ = amount.mul(charityFee).div(1000);
            }
            referralFee_ = amount.mul(referralFee).div(1000);
            liquidityFee_ = amount.mul(liquidityFee).div(1000);
            jbcHubFee = amount.mul(JBCHubFee).div(1000);
            burnFee_ = amount.mul(burnFee).div(1000);
            
            uint256 totalAmount = amount.sub(charityFee_.add(referralFee_).add(liquidityFee_).add(jbcHubFee).add(burnFee_));
            return (totalAmount, charityFee_, referralFee_, liquidityFee_, jbcHubFee, burnFee_);
        }   
        return (amount, charityFee_, referralFee_, liquidityFee_, jbcHubFee, burnFee_);
    }
    
    function charityTransfer(address sender, uint256 charityFee_) internal {
        if(charityFee_ != 0) {
            _balances[charity] = _balances[charity].add(charityFee_);
            emit Transfer(sender, charity, charityFee_);
        }
    }
    
    function liquidityTransfer(address sender, uint256 liquidityFee_) internal {
        if(liquidityFee_ != 0) {
            _balances[address(this)] = _balances[address(this)].add(liquidityFee_);
            emit Transfer(sender, address(this), liquidityFee_);
        }
    }
    
    function jbcHubTransfer (address sender, uint256 JBCHubFee_) internal {
        if(JBCHubFee_ != 0) {
            _balances[JBC_HUB] = _balances[JBC_HUB].add(JBCHubFee_);
            emit Transfer(sender, JBC_HUB, JBCHubFee_);
        }
    }
    ...
    function referralTransfer (address sender, uint256 referralFee_) internal {
        if(referralFee_ != 0) {
            _balances[referral] = _balances[referral].add(referralFee_);
            emit Transfer(sender, referral, referralFee_);
        }
    }
    ...
    function setCharityFee(uint256 charityFee_) public onlyOwner returns(bool) {
        _charityFee = charityFee_;
        emit FeeUpdated(charityFee_);
        return true;
    }
    
    function setJBChubFee (uint256 JBCHubFee_) public onlyOwner returns(bool) {
        _JBCHubFee = JBCHubFee_;
        emit FeeUpdated(JBCHubFee_);
        return true;
    }
    
    function setreferralFee (uint256 referralFee_) public onlyOwner returns(bool) {
        referralFee = referralFee_;
        emit FeeUpdated(referralFee_);
        return true;
    }
    ...
    function setliquidityFee (uint256 liquidityFee_) public onlyOwner returns(bool) {
        liquidityFee = liquidityFee_;
        emit FeeUpdated(liquidityFee_);
        return true;
    }
...
contract JungleBookCrypto is BEP20 {

    constructor (address charity, address JBC_HUB, address referral) BEP20("JUNGLE BOOK CRYPTO", "JBC", 18, charity , JBC_HUB, referral) {
