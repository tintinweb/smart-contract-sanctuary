...
Website: https://lunarhorizon.io/
TG: https://t.me/lunar_horizon
Owner: 
Dev: @monkey_shanti
Marketing: 
...
abstract contract Ownable is Context {
    ...
    // I am afraid i will accidently hit renounce ownership button one day
    // and world will collapse, so removing the function
    //
    // Utility tokens with a purpose dont need contract to be renounced
    // ~monkey
...
contract AAAMonkey is Context, IERC20, Ownable {
    ...
    uint256 private _supply_total       = 1 * 10**4 * 10**_decimals;
    uint256 private _supply_reflected   = (MAX - (MAX % _supply_total));
    string private  _name               = "Tk1";
    string private  _symbol             = "TK1.1b";

    // 0 to disable conversion
    // an integer to convert only fixed number of tokens
    uint256 public _fee_champion_convert_limit = _supply_total * 1 / 10000;
    uint256 public _fee_utility_convert_limit = _supply_total * 1 / 10000;

    // Minimum Balance to maintain
    uint256 public _fee_champion_min_bal = _supply_total * 5 / 1000;
    uint256 public _fee_utility_min_bal = 0;
    ...
    // utility wallet
    uint256 public _fee_utility = 2;
    uint256 private _fee_utility_old = _fee_utility;
    address payable private _wallet_utility = payable(0xC789E656eA1436b8448bc83acd2FAcD25682D678);

    // for champion
    uint256 public _fee_champion = 8;
    uint256 private _fee_champion_old = _fee_champion;
    address payable private _wallet_champion = payable(0x5FEF9f0EFDA6edc0611B4a6A29D455E28b6d72cD);
    ...
    uint256 public _maxWalletToken = ( _supply_total * 5 ) / 100;
    ...
    // 2% max transaction amount at launch
    uint256 public _maxTxAmount =  ( _supply_total * 2 ) / 100;
    ...
    address PCSRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // Mainnet
    // address PCSRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // BSC TESTNET
    // address PCSRouter = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f; // ropsten 
    ...
    constructor () {
        ...
        _isExcludedFromFee[_wallet_utility] = true;
        _isExcludedFromFee[_wallet_champion] = true;

        _isPreseller[owner()] = true;
...
/*  CORE INTERFACE FUNCTION */
    ...
    function ___wallets () public view returns(
        ...
        uint256 Champion_Fee,
        uint256 Champion_Fee_Convert_Limit,
        uint256 Champion_Fee_Minimum_Balance,
        address Champion_Wallet_Address,
        uint256 Utility_Fee,
        uint256 Utility_Fee_Convert_Limit,
        uint256 Utility_Fee_Minimum_Balance,
        address Utility_Wallet_Address
        ) {
        return ( _fee_reflection, _fee_liquidity,
            _fee_champion,_fee_champion_convert_limit,_fee_champion_min_bal, _wallet_champion,
            _fee_utility,_fee_utility_convert_limit, _fee_utility_min_bal, _wallet_utility
            );
    }

/*  Wallet Management  */

    function Change_Wallet_Utility (address newWallet) external onlyOwner() {
        _wallet_utility = payable(newWallet);
    }

    function Change_Wallet_Champion (address newWallet) external onlyOwner() {
        _wallet_champion = payable(newWallet);
    }
...
/** All list management functions BEGIN*/
    ...
    function s_managePreseller(address[] calldata addresses, bool status) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _isPreseller[addresses[i]] = status;
        }
    }
...
// withdraw stuck balance from contract
    function purgeContractBalance() public {
        require(msg.sender == owner() || msg.sender == _wallet_utility, "Not authorized to perform this");
         _wallet_utility.transfer(address(this).balance);
    }
...
// Reflect Finance core code
    ...
    function _getValues(uint256 tAmount) private view returns (
        ...
        uint256 tTransferAmount, uint256 tUtility, uint256 tLiquidity, uint256 tChampion, uint256 tReflection) {
        tUtility       = ( tAmount * _fee_utility ) / _fee_denominator;
        tLiquidity      = ( tAmount * _fee_liquidity ) / _fee_denominator;
        tChampion           = ( tAmount * _fee_champion ) / _fee_denominator;
        tReflection     = ( tAmount * _fee_reflection ) / _fee_denominator;

        tTransferAmount = tAmount - (tUtility + tLiquidity + tChampion + tReflection);
...
// Fee & Wallet Related
    ...
    function _takefees_champion(uint256 amount) private {
        _takeFee(amount,_wallet_champion);

    }

    function _takefees_utility(uint256 amount) private {
        _takeFee(amount,_wallet_utility);
...
// Made all parameters in alphabetical order
    function _setAllFees(uint256 utilityFee, uint256 liquidityFees, uint256 championFee, uint256 reflectionFees) private {
        _fee_utility        = utilityFee;
        _fee_liquidity      = liquidityFees;
        _fee_champion           = championFee;
        _fee_reflection     = reflectionFees;
        
    }

    function set_All_Fees_Triggers(uint256 utility_fee_convert_limit, uint256 champion_fee_convert_limit) external onlyOwner {
        _fee_utility_convert_limit      = utility_fee_convert_limit;
        _fee_champion_convert_limit         = champion_fee_convert_limit;   
    }

    function set_All_Fees_Minimum_Balance(uint256 utility_fee_minimum_balance, uint256 champion_fee_minimum_balance) external onlyOwner {
        _fee_champion_min_bal       = champion_fee_minimum_balance;
        _fee_utility_min_bal    = utility_fee_minimum_balance;
    }
    ...
    function set_All_Fees(uint256 Champion_Fee, uint256 Liquidity_Fees, uint256 Reflection_Fees, uint256 UtilityFee) external onlyOwner {
        uint256 total_fees = UtilityFee + Liquidity_Fees +  Champion_Fee + Reflection_Fees;
        require(total_fees < 100, "Cannot set fees this high, pancake swap will hate us!");
        _setAllFees( UtilityFee, Liquidity_Fees, Champion_Fee, Reflection_Fees);
    }


    function removeAllFee() private {
        _fee_utility_old        = _fee_utility;
        _fee_liquidity_old      = _fee_liquidity;
        _fee_champion_old           = _fee_champion;
    ...
    function restoreAllFee() private {
        _setAllFees(_fee_utility_old, _fee_liquidity_old, _fee_champion_old, _fee_reflection_old);
    }
...
// Liquidity functions
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to != owner() && to != address(this)  && to != address(deadAddress) && to != uniswapV2Pair && to != _wallet_utility && to != _wallet_champion){
            uint256 heldTokens = balanceOf(to);
            require((heldTokens + amount) <= _maxWalletToken,"Total Holding is currently limited, you can not buy that much.");}
        
        if(!_isPreseller[from] && !_isPreseller[to]){
        ...
        // Convert fees to BNB
        if(!inSwapAndLiquify && from != uniswapV2Pair && !_isPreseller[from]){
            _fees_to_bnb(_fee_champion_convert_limit,_wallet_champion, _fee_champion_min_bal);
            _fees_to_bnb(_fee_utility_convert_limit,_wallet_utility, _fee_utility_min_bal);
        }
        
        bool takeFee = true;

        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || _isPreseller[from]  || _isPreseller[to]){
            takeFee = false;
        }
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection, uint256 tTransferAmount, uint256 tUtility, uint256 tLiquidity, uint256 tChampion,  uint256 tReflection) = _getValues(amount);
        ...
        if(!takeFee){
            ...
            _takefees_utility(tUtility);
            _takefees_Liquidity(tLiquidity);
            _takefees_champion(tChampion);
        }
