...
contract Admin {
    ...
    uint256 public DEFAULT_TAX = 5;

    constructor () public {
        _gov = msg.sender;
        _eightLevelRebate = [40, 20, 10, 10, 6, 6, 4, 4];
    }

    function setDefaultTax(uint256 _defaultTax) public {
        require(_gov == msg.sender, "access denied");
        DEFAULT_TAX = _defaultTax;
    }
    ...
    function setInviter(address _inviter) public {
        require(_inviter != msg.sender, "can't invite yourself");
    ...
    function estimateWBNBRewards(address _msgsender) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _tokenList.length; i++) {
            ...
            if (token == _oracle.anchorToken()) {
                total = total.add(_rewards[_msgsender][token]);
                continue;
            }
            uint256 decimal = IERC20p(token).decimals();
            uint256 wBNBPrice = _oracle.getCurrentPrice(token);
            total = total.add(_rewards[_msgsender][_tokenList[i]].mul(wBNBPrice).div(10 ** decimal));
        }
        return total;
    }

    function claimRewardsSwap(address _token) public {
        uint256 tokenRewards = _rewards[msg.sender][_token];
        require(tokenRewards > 0, "rewards < 0");
        if (_token == _oracle.anchorToken()) {
            IERC20(_token).transfer(msg.sender, tokenRewards);
            _rewards[msg.sender][_token] = 0;
        } else {
            address[] memory path = new address[](2);
            path[0] = _token;
            path[1] = _oracle.anchorToken();
            require(IERC20(_token).approve(address(_router), tokenRewards), "approve failed");
            require(IPancakeRouter01(_router).swapExactTokensForETH(
                tokenRewards,
                0,
                path,
                msg.sender,
                now.add(600)
            ).length > 0, "swap failed");
    ...
    function claimRewards() public {
        ...
        for (uint256 i = 0; i < _tokenList.length; i++) {
            ...
            if (token == _oracle.anchorToken()) {
                IERC20(token).transfer(msg.sender, tokenRewards);
            ...
            path[0] = token;
            path[1] = _oracle.anchorToken();
            IERC20(token).approve(address(_router), tokenRewards);
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
            ...
            //hex'd0d4c4cd0848c93cb4fd1f498d7013ee6bfb25783ea21593d5834f5d250ece66' // init code hash
                hex'767867586dcb56e476d57c0afe43930002594f18ee5909d5d96fc6c19648adbc'   // Change to INIT_CODE_PAIR_HASH of Pancake Factory
            ))));
    }
