...
interface IERC20MetDotta is IERC20 {
    /**
...
contract ERC20 is Context, IERC20, IERC20MetDotta {
...
contract DiamondTree is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DiamondTreeDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0xd505f739Fd69FF3ee9687e34C725F0382f92B351); 
    address payable public operations2Address =
        payable(0xC4517b0d25AA8d72eba6429c7daA668Da6c875e1);

    uint256 public maxSellTransactionAmount = 10**9 * (10**9);
    uint256 public maxSellTransactionAmount2 = 10**9 * (10**9);
    uint256 public maxBuyTransactionAmount = 10**9 * (10**9);
    uint256 public swapTokensAtAmount = 10000000000 * (10**9);

    uint256 public devFees = 2; // Dev 2% from here, Marketing is 3% 
    uint256 public liquidityFee = 3;
    uint256 public DotRewardsBuyFee = 6;
    uint256 public DotRewardsSellFee = 10;
    
    
    uint256 public snipeDotRewardsBuyFee = 0;
    uint256 public snipeDotRewardsSellFee = 0;
    ...
    event UpdateFees(
        uint256 dev,
        uint256 liquidity,
        uint256 DotRewardsBuy,
        uint256 DotRewardsSell
    );
    ...
    constructor() ERC20("DiamondTree", "DiamondTree") {
        dividendTracker = new DiamondTreeDividendTracker(payable(this));
        ...
        totalSellFees = DotRewardsSellFee.add(liquidityFee).add(devFees);
        // totalBuyFees = DotRewardsBuyFee.add(liquidityFee).add(devFees);
        totalBuyFees = DotRewardsBuyFee.add(liquidityFee);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        DiamondTreeDividendTracker newDividendTracker = DiamondTreeDividendTracker(
            payable(newAddress)
        );
    ...
    function updateDotRewardsSell(uint256 newValue) public onlyOwner {
        DotRewardsSellFee = newValue;
    }
    function updatesnipeDotRewardsSell(uint256 newValue) public onlyOwner {
        snipeDotRewardsSellFee = newValue;
    }
    function updatesnipeDotRewardsBuy(uint256 newValue) public onlyOwner {
        snipeDotRewardsBuyFee = newValue;
    }
    ...
    function updateFees(
        ...
        // require(dev <= 20, "operation fees can't exceed 20%");
        // require(liquidity <= 5, "liquidity fees can't exceed 5%");
        // require(
        //     DotRewardsBuy >= 0 && DotRewardsBuy <= 40,
        //     "Dot reward fees must be between 0 and 40"
        // );
        // require(
        //     DotRewardsSell >= 5 && DotRewardsSell <= 40,
        //     "Dot reward fees must be between 5 and 40"
        // );

        devFees = dev;
        liquidityFee = liquidity;
        DotRewardsBuyFee = DotRewardsBuy;
        DotRewardsSellFee = DotRewardsSell;

        totalSellFees = DotRewardsSellFee.add(liquidityFee).add(devFees);
        totalBuyFees = DotRewardsBuyFee.add(liquidityFee).add(devFees);

        emit UpdateFees(dev, liquidity, DotRewardsBuy, DotRewardsSell);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if(killSnipeEnabled && !_isExcludedFromSnipe[from] && !_isExcludedFromSnipe[to]) {
                ...
                if (isSelling) {
                    if(!_isConsiderAsSnipe[from]) {
                        ...
                        DotRewardsFee = DotRewardsSellFee;
                    } else {
                        DotRewardsFee = snipeDotRewardsSellFee;
                    }
                } else {
                    DotRewardsFee = DotRewardsBuyFee;
                }
            }
            //Sniper Wallet cannot transfer
            ...
            uint256 totalFees = DotRewardsFee.add(liquidityFee).add(devFees);
            if(!isSelling) {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .mul(buyAmount)
            .div(totalAmount)
            .mul(DotRewardsBuyFee)
            .div(DotRewardsBuyFee.add(devFees));

        uint256 dividendsFromSell = address(this)
            .balance
            .mul(sellAmount)
            .div(totalAmount)
            .mul(DotRewardsSellFee)
            .div(DotRewardsSellFee.add(devFees));
...
contract DiamondTreeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DiamondTree public diamondTree;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "DiamondTree_Dividend_Tracker",
            "DiamondTree_Dividend_Tracker"
        )
    {
        diamondTree = DiamondTree(mainContract);
        minimumTokenBalanceForAutoDividends = 50000000000 * (10**9);
    ...
    function _transfer(
        ...
        require(false, "DiamondTree_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DiamondTree_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DiamondTree contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(diamondTree);

            uint256 prevBalance = diamondTree.balanceOf(address(this));
            ...
            {
                uint256 received = diamondTree.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    diamondTree.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            // if no tokenAddress assume Dot payout
            if (
                !allowCustomTokens ||
