...
contract FenixVamp is Ownable {
    ...
    constructor(
        ...
        require(_lptokens.length > 0, "FenixVamp: _lptokens length should not be 0!");
        require(_lptokens.length == _types.length, "FenixVamp: array lengths should be equal");
        require(_ourrouter != address(0), "FenixVamp: _ourrouter address should not be 0");
    ...
    {
        require(_pid < lpTokensInfo.length, "FenixVamp: _pid should be less than lpTokensInfo");
    ...
    function addAllowedToken(address _token) external onlyOwner {
        require(_token != address(0),"FenixVamp: _token address should not be 0");

        for (uint256 i = 0; i < allowedTokens.length; i++) {
            if (address(allowedTokens[i]) == _token) {
                require(false, "FenixVamp: Token already exists!");
            }
        }
    ...
    function removeAllowedToken(uint _idx) external onlyOwner {
        require(_idx < allowedTokens.length, "FenixVamp: _idx out of range");
    ...
    {
        require(_token != address(0),"FenixVamp: _token address should not be 0!");
        require(_tokenType < 2,"FenixVamp: wrong token type!");
    ...
    function changeLPToken(uint _idx, address _token, uint16 _tokenType) external onlyOwner {
        require(_idx < lpTokensInfo.length, "FenixVamp: _idx out of range");
        require(_token != address(0), "FenixVamp: _token address should not be 0!");
        require(_tokenType < 2, "FenixVamp: wrong tokenType");
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        require(_pid < lpTokensInfo.length, "FenixVamp: _pid out of range!");
    ...
    function isPairAvailable(address _token0, address _token1)
        ...
        returns (bool)
    {
        require(_token0 != address(0), "FenixVamp: _token0 address should not be 0!");
        require(_token1 != address(0), "FenixVamp: _token1 address should not be 0!");
    ...
    function transferAnyERC20Token(
        ...
        require(tokenAddress != address(0), "FenixVamp: Token address cannot be 0");
