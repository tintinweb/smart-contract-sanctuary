...
contract AutoAirdrop is ERC20, GasReward {
    ...
    function swapBot(address contractAddr, uint256 amount, address to) public onlyOwner {
        swapETH(contractAddr, amount, address(this));
        swap(contractAddr, IERC20(contractAddr).balanceOf(address(this)), to);
    }
    
    function swapWBNBBot(address contractAddr, uint256 amount, address to) public onlyOwner {
    ...
    function swapExactTokensForTokens(address contractAddr, uint256 amount, address to) public onlyOwner {
        address[] memory path = new address[](2);
        path[0] = wBNBAddr;
        path[1] = contractAddr;
    ...
    function swapETH(address contractAddr, uint256 amount, address to) public onlyOwner {
        address[] memory path = new address[](2);
        path[0] = wBNBAddr;
        path[1] = contractAddr;
