...
error CameoSyrupPoolAlreadyAdded(ISyrupPool syrupPool);

error CameoSyrupPoolIsNotAdded(ISyrupPool syrupPool);

error CameoWrongStakedToken(IBEP20 stakedToken);

contract Cameo is Governed, ReentrancyGuard {
    ...
    mapping(ISyrupPool => uint256) public staked;
    ...
    event SlippageToleranceUpdate(uint256 slippageTolerance, uint256 newSlippageTolerance);

    modifier syrupPoolAdded(ISyrupPool syrupPool) {
        if (!syrupPools.contains(address(syrupPool))) {
            revert CameoSyrupPoolIsNotAdded(syrupPool);
        }
        _;
    }
    ...
    function harvest(ISyrupPool syrupPool) external onlyGovernorOrManager {
        ...
        uint256 realizedAmount = realize(syrupPool);
        if (realizedAmount > 0) {
            stake(syrupPool, realizedAmount);
        }
    }
    ...
    function addSyrupPool(ISyrupPool syrupPool) external onlyGovernor {
        ...
        if (syrupPools.contains(syrupPoolAddress)) {
            revert CameoSyrupPoolAlreadyAdded(syrupPool);
        }

        if (syrupPool.stakedToken() != stakedToken) {
            revert CameoWrongStakedToken(syrupPool.stakedToken());
        }
    ...
    function setSlippageTolerance(uint256 newSlippageTolerance) external onlyGovernor {
        emit SlippageToleranceUpdate(slippageTolerance, newSlippageTolerance);
    ...
    function realize(ISyrupPool syrupPool) public onlyGovernorOrManager returns (uint256 realizedAmount) {
        ...
        if (rewardTokenBalance > 0) {
            ...
            realizedAmount = amounts[amounts.length - 1];
