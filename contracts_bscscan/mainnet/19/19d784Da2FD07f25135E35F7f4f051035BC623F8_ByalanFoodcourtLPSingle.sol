...
import "../../interfaces/foodcourt/ICafeteria.sol";
...
contract ByalanFoodcourtLPSingle is ByalanIsland, Sailor, GasThrottler, IByalan, ReentrancyGuard {
    ...
    address public constant MASTERCHEF = 0xe43B7C5c4c2DF51306CCEb7cBc4b2fcC038874F1;
    ...
    constructor(
        ...
        uint256 _pid
    ) ByalanIsland(_hydra, _unirouter, _izlude, _kswFeeRecipient, _treasuryFeeRecipient) {
        pid = _pid;
    ...
    function harvest() public override whenNotPaused onlyEOAandIzlude gasThrottle {
    ...
    function chargeFees() internal nonReentrant {
        uint256 toWbnb = (IERC20(want).balanceOf(address(this)) * totalFee) / MAX_FEE;
        IUniswapV2Router02(unirouter).swapExactTokensForETH(toWbnb, 0, wantToWbnbRoute, address(this), block.timestamp);
    ...
    function pendingReward() external view override returns (uint256) {
        return ICafeteria(MASTERCHEF).pendingCoupon(pid, address(this));
    }

    // called as part of strategy migration. Sends all the available funds back to the izlude.
    function retireStrategy() external override onlyIzlude {
        ICafeteria(MASTERCHEF).emergencyWithdraw(0);
        IERC20(want).transfer(izlude, IERC20(want).balanceOf(address(this)));
    }
...
abstract contract Sailor is ByalanIsland, ISailor {
    ...
    uint256 public override callFee = 3500; // 35% of total fee
    uint256 public treasuryFee = 2500; // 25% of total fee
    uint256 public override kswFee = 4000; // 40% of total fee
