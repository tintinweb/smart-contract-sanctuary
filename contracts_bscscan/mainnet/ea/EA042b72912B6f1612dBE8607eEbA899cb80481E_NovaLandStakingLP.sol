...
contract NovaLandStakingLP is Initializable , OwnableUpgradeable{
  ...
  function updateConfig(
        uint32 _interestRate, address _stakeToken, address _rewardToken
    ) public onlyOwner{
  ...
  function getReward(
        ...
        (uint256 reverse0 , , ) = IPancakePair(STAKE_TOKEN).getReserves();
        uint256 tokenPerLp = (reverse0 * 1e4) / IPancakePair(STAKE_TOKEN).totalSupply();
        uint256 yearReward = tokenPerLp * listStaking[_address].amount * interestRate / (1000 * 1e4);
  ...
  function staking(
        uint256 _amount
    ) public onlyNonContract returns (bool) {
        // Return reward if user is staking
        if (listStaking[msg.sender].amount > 0) {
            uint256 reward = getReward(msg.sender);
            IERC20Upgradeable(REWARD_TOKEN).transfer(msg.sender, reward);
        }
