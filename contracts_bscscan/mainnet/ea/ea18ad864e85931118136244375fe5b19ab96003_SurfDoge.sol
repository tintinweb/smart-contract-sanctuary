...
// File: contracts/AkuAkuDividendPayingToken.sol
...
contract SurfDoge is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
	
	ERC20 private BUSDtoken;
	
    address public immutable uniswapV2Pair;
	address public immutable uniswapV2PairBUSD;
	address public helperContract;
	address public feeDistributionContract;
	...
	uint256 public maxHolders = 0;
	uint256 LotteryMinimum = 50000 * (10**18);

	event UpdateWalletLotteryStatus(address indexed holder, bool indexed participating);
	event SpotInMapping(address indexed holder, uint256 indexed participating);
    ...
    constructor() ERC20("SurfTest2", "Test2") {
        ...
        totalFees = _BUSDRewardsFee.add(_liquidityFee).add(_marketingTeamFee).add(_lotteryFee);
    ...
    function updateCanTransferBeforeTradingIsEnabledTurnOff(address newAddress) public onlyOwner {
        canTransferBeforeTradingIsEnabled[newAddress] = false;
    }

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Surfdoge: The dividend tracker already has that address");

        SurfDogeDividendTracker newDividendTracker = SurfDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Surfdoge: The new dividend tracker must be owned by the AkuAku token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Surfdoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Surfdoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Surfdoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Surfdoge: Automated market maker pair is already set to that value");
   ...
   function getUnlockTimeHours() public view returns (uint256) {
        if (_owner == address(0)) {
            if (_lockTime > block.timestamp) {
                return (_lockTime.sub(block.timestamp)).div(3600);
            }
    ...
    function updateFeeDistributionContract(address newFeeDistributionContract) public onlyOwner {
        require(newFeeDistributionContract != feeDistributionContract, "The fee distribution contract is already this address");
        ...
        feeDistributionContract = newFeeDistributionContract;
    }
	
    function updateHelperContract(address newHelperContract) public onlyOwner {
        require(newHelperContract != helperContract, "The helper contract is already this address");
        excludeFromFees(newHelperContract, true);
        emit HelperContractUpdated(newHelperContract, helperContract);
        helperContract = newHelperContract;
    }

    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Surfdoge: The liquidity wallet is already this address");
    ...
    function updateMarketingTeamFee(uint256 newMarketingTeamFee) public onlyOwner {
        require(newMarketingTeamFee <= 6, "Marketing and Team fee can't be more than 6%");
        emit marketingTeamFeeUpdated(newMarketingTeamFee, marketingTeamFee);
        marketingTeamFee = newMarketingTeamFee;
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingTeamFee).add(lotteryFee);
    }
	
    function updateLiquidityFee(uint256 newLiquidityFee) public onlyOwner {
        require(newLiquidityFee <= 6, "Liquidity fee can't be more than 6%");
    ...
    function updateRewardsFee(uint256 newRewardsFee) public onlyOwner {
        require(newRewardsFee <= 6, "Rewards fee can't be more than 6%");
    ...
    function updateLotteryFee(uint256 newLotteryFee) public onlyOwner {
        require(newLotteryFee <= 4, "Lottery fee can't be more than 4%");
    ...
    function updateSellFeeIncreaseFactor(uint256 newSellFeeIncreaseFactor) public onlyOwner {
        require(newSellFeeIncreaseFactor <= 200, "Sell Increase factor can't be more than 200%");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Surfdoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Surfdoge: Cannot update gasForProcessing to same value");
   ...
   function viewLotteryWallets () public view returns (address[] memory) {
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "Surfdoge: This account cannot send tokens until trading is enabled");
        }
		...
		if (automatedMarketMakerPairs[from] || from == dxSalePresaleAddress) {
			...
			if (amount.add(balanceOf(to)) > LotteryMinimum && holderInLottery[to] == false) {
				//Bought for first time OR bought enough for lottery minimum OR got from presale
				lotteryArray.push(to);
				holderLotteryMapping[to] = (lotteryArray.length).sub(1); 
				holderInLottery[to] = true;
				
				emit UpdateWalletLotteryStatus(to, true);
		...
		if (automatedMarketMakerPairs[to]) {
			...
			if ((balanceOf(from).sub(amount)) < LotteryMinimum && holderInLottery[from] == true) {
				//Sold tokens, end balance below lottery minimum
				
				lotteryArray[holderLotteryMapping[from]] = lotteryArray[(lotteryArray.length).sub(1)]; 
        ...
        if(
            ...
            swapTokensForEth(tokensForSwapping); //Get all the tokens in BNB, converts only the setpoint amount of tokens - liqudiity tokens
            uint256 BNBBalance = address(this).balance;
            
            uint256 hypotheticalFullBNBBalance = (BNBBalance.mul(totalFees)).div(totalFees.sub(liquidityFee.div(2))); //BNB if we swapped all tokens
            
            uint256 BNBForLiquidity = hypotheticalFullBNBBalance.mul(liquidityFee).div(totalFees).div(2); 
            uint256 BNBForMarketingAndTeam = hypotheticalFullBNBBalance.mul(marketingTeamFee).div(totalFees); 
            uint256 BNBForRewards = hypotheticalFullBNBBalance.mul(BUSDRewardsFee).div(totalFees); 
            uint256 BNBForLottery = hypotheticalFullBNBBalance.mul(lotteryFee).div(totalFees); 
            
			uint256 BNBForMarketingAndTeamSent = (BNBForMarketingAndTeam.mul(9)).div(10);
			uint256 BNBForManagerContract = BNBForMarketingAndTeam.sub(BNBForMarketingAndTeamSent); // 10% of MarketinTeamFee
			
			lotteryBalance.add(BNBForLottery); //Stays inside contract, no trasfer
            ...
            (bool successMarket, ) =  feeDistributionContract.call {value: BNBForMarketingAndTeam}("");
		    (bool successManger, ) = helperContract.call {value: BNBForManagerContract}(""); //Called so it can call the main contract funcions
    ...
    function swapAndSendDividends(uint256 rewardsBNB) private {
        ...
        uint256 dividends = IERC20(addressBUSD).balanceOf(address(this));
        bool success = IERC20(addressBUSD).transfer(address(dividendTracker), dividends);
...
contract SurfDogeDividendTracker is DividendPayingToken, Ownable {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "AkuAku_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "AkuAku_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AkuAku contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "AkuAku_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AkuAku_Dividend_Tracker: Cannot update claimWait to same value");
