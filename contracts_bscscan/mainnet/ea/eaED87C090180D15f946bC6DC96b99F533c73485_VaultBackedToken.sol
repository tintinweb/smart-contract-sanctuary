...
contract VaultBackedToken is ERC20("CakeStake Test", "CSTKT"), Ownable {
    ...
    function _beforeTokenTransfer(
        ...
        else if (!paused && from != address(0) && _checkMintable(from, to) && swapAndLiquifyEnabled) {
...
contract YearnVaultLiqGen is Ownable, ERC20("StakedCAKE", "SCAKE") {
    ...
    function withdrawProfits() public {
        _redeemProfit(msg.sender, msg.sender, getProfitForUser(msg.sender));
    }

    /**
     * @notice Withdraws unrealized profits from caller to a specified address
     * @param to The address to transfer the profits to
     */
    function withdrawProfitsTo(address to) public {
        _redeemProfit(msg.sender, to, getProfitForUser(msg.sender));
    }
    ...
    function withdrawProfitsOnPair() public {
        _redeemProfit(address(StakeCpair), address(curC), getProfitForUser(StakeCpair));
    }
