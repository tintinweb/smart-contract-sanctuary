...
contract Vault is ERC20 {
    ...
    function allowTokenA(address paraswap, uint256 amount) public {

        IERC20(tokenA).safeApprove(paraswap, amount);
    }
    
    function allowTokenB(address paraswap, uint256 amount) public {

        IERC20(tokenB).safeApprove(paraswap, amount);
    }

    function poolSize() public view returns (uint256) {
        return
            IERC20(tokenA).balanceOf(address(this)).add(_getTokenAFromTokenB());
    }

    function deposit(uint256 amount) public {

        // 0. calculate pool amount
        uint256 _pool = poolSize();
        
        // 1. transfer amount from sender to this vault
        uint256 _before = IERC20(tokenA).balanceOf(address(this));
        IERC20(tokenA).safeTransferFrom(msg.sender, address(this), amount);
        uint256 _after = IERC20(tokenA).balanceOf(address(this));
        amount = _after.sub(_before); // Additional check for deflationary tokens

        // 2. calculate share and send back xUBXT
        uint256 shares = 0;
        if (totalSupply() == 0) {
            shares = amount;
        } else {
    ...
    function withdraw(uint256 shares) public  {
        ...
        // 1. calculate amount from shares for both tokenA and B
        uint256 amountA = (IERC20(tokenA).balanceOf(address(this)).mul(shares)).div(totalSupply());
        uint256 amountB = (IERC20(tokenB).balanceOf(address(this)).mul(shares)).div(totalSupply());

        // 2. send tokenA and B to the sender wallet
        if (amountA > 0) {
        ...
        // 3. burn these shares from the sender wallet
        _burn(msg.sender, shares);

        // 4. if the vault is in open position (status == 1), update the soldAmount (remove the amount related to the shares)
        if (position == 1) { // the trade is in open position
            soldAmount = soldAmount.sub(soldAmount.mul(shares).div(totalSupply()));
        }
    }
    ...
    function buy() public {
        ...
        // 2. get the amount of tokenA to trade
        uint256 amount = IERC20(tokenA).balanceOf(address(this));

        // 3. takeUpbotsFees
        amount = takeUpbotsFees(tokenA, amount);

        // 4. save the remaining to soldAmount
        soldAmount = amount;

        // 5. swap tokens to B
        _swapPancakeswap(tokenA, tokenB, amount);
    ...
    function sell() public {
        ...
        // 2. get the amount of tokenB to trade
        uint256 amount = IERC20(tokenB).balanceOf(address(this));

        // 3. takeUpbotsFee
        amount = takeUpbotsFees(tokenB, amount);

        // 3. swap tokens to A and get the newly create tokenA
        uint256 _before = IERC20(tokenA).balanceOf(address(this));
        _swapPancakeswap(tokenB, tokenA, amount);
        uint256 _after = IERC20(tokenA).balanceOf(address(this));
    ...
    function takePerformanceFees(uint256 amount) private {
        ...
        uint256 _before = IERC20(ubxt).balanceOf(address(this));
        _swapPancakeswap(tokenA, ubxt, _total);
        uint256 _after = IERC20(ubxt).balanceOf(address(this));
        uint256 ubxtAmt = _after.sub(_before);
    ...
    function _getTokenAFromTokenB() internal view returns(uint256) {
        ...
        uint256[] memory amounts = UniswapRouterV2(pancakeRouter).getAmountsOut(amountB, pathBackward);
        return amounts[2];
    }
