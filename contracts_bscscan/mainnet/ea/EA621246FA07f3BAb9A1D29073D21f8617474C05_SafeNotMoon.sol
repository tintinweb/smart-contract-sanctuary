...
interface AntiSnipeContract {
    function onPreTransferSnipeCheck() external returns(bool);

    function initiateAntiSnipe() external;
...
contract SafeNotMoon is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "SafeNotMoon";
    string private _symbol = "SAFENOTMOON";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress; // Marketing Address
    address payable public BuyBackWalletAddress;  // BuyBack Address
    ...
    constructor (address _marketing, address _buyback, address _antiSnipeContractInput) {
        marketingWalletAddress = payable(_marketing); // Marketing Address
        BuyBackWalletAddress = payable(_buyback); // BuyBack Address

        antiSnipeContract = AntiSnipeContract(_antiSnipeContractInput);
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); 

        uniswapPair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH());
        antiSnipeContract.initiateAntiSnipe();
        ...
        _includeInLpList[address(uniswapPair)] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !isMarketPair[sender] && swapAndLiquifyEnabled && !isIncludedInLpList(sender)) 
            {
            ...
            // if sender is a sniper address, reject the sell.
                ...
                if(!_hasLiqBeenAdded) {
                    ...
                    if(antiSnipeContract.onPreTransferSnipeCheck()) {
    ...
    function _checkLiquidityAdd(address from, address to) private {
        ...
        if(_liquidityHolders[from] && to == uniswapPair) {
          ...
          antiSnipeContract.launch();
        }
    }


    function addPinksalePresaleAddress(address _presale) public onlyOwner {
    ...
    function setIncludeInLpList(address _address, bool _isIncludedOrNot) public onlyOwner {
        ...
        require(_address != uniswapPair || _isIncludedOrNot == true, "Cannot exclude uniswapV2Pair");
