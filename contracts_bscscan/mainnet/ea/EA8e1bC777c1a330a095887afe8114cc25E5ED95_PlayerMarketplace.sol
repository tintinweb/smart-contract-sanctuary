...
contract PlayerMarketplace is Ownable {
    ...
    mapping(uint256 => uint256) public auctionsAndActiveIndices; 
    ...
    function createAuction(address paymentTokenAddress, uint256 _price, address nftAddress, uint256 _durationBlocks, uint256 tokenID, bool _bidOnly, uint256 _minimumBidAmount) external {
        ...
        auctionsAndActiveIndices[_id] = activeAuctions.length + 1;
        activeAuctions.push(_id);
        updateBottomAuctions();
    ...
    function bid(uint256 auctionID, uint256 amount) external {
        ...
        massUpdateAuctionsAfterAuctionID(auctionID);
    ...
    function buyOut(uint256 auctionID) external {
        ...
        buyOutCount++;
        if(auctionsAndActiveIndices[auctionID] > 0)
        {
            removeFromActiveAuctions(auctionsAndActiveIndices[auctionID]-1);
        }
        massUpdateAuctionsAfterAuctionID(auctionID);
    ...
    function removeFromActiveAuctions(uint256 index) private {
        inactiveAuctions.push(activeAuctions[index]);
        auctionsAndActiveIndices[activeAuctions[index]] = 0;
        auctionsAndActiveIndices[activeAuctions[activeAuctions.length - 1]] = index + 1;
        activeAuctions[index] = activeAuctions[activeAuctions.length - 1];
        activeAuctions.pop();
    }

    function massUpdateAuctionsAfterAuctionID(uint256 auctionID) private {
        uint256 startingPoint = auctionsAndActiveIndices[auctionID] - 1;
        uint256 endPoint = startingPoint.add(updateTopCount);
        if(endPoint > activeAuctions.length)
        {
            endPoint = activeAuctions.length;
        }
        for(uint256 i = startingPoint; i < endPoint; i++)
        {
            Auction storage auction = auctions[activeAuctions[i]];
            if(auction.status == "Active")
            {
                if(auction.endBlock <= block.timestamp)
                {
                    auctions[activeAuctions[i]].status = "Expired";
                    if(auctionsAndActiveIndices[auction.auctionID] > 0)
                    {
                        removeFromActiveAuctions(auctionsAndActiveIndices[auction.auctionID]-1);
                    }
                }
            }
        }
    ...
    function updateBottomAuctions() private {
        uint256 startingPoint = 0;
        uint256 endPoint = updateBottomCount;
        if(endPoint > activeAuctions.length)
        {
            endPoint = activeAuctions.length;
        }
    ...
    function publicMassUpdateAuctions() public {
    ...
    function publicUpdateAuction(uint256 auctionID) public {
        Auction storage auction = auctions[auctionID];
        if(auction.status == "Active")
        {
            if(auction.endBlock <= block.timestamp)
            {
                auctions[auctionID].status = "Expired";
                if(auctionsAndActiveIndices[auction.auctionID] > 0)
                {
                    removeFromActiveAuctions(auctionsAndActiveIndices[auction.auctionID]-1);
                }
    ...
    function adminEmergencyCancelAuction(uint256 auctionID) external onlyOwner {
        Auction storage auction = auctions[auctionID];
        require(auction.status == "Active" || auction.status == "Expired", "adminCancelAuction: not active or expired.");
        if(auction.highestBidder != address(0))
        {
    ...
    function adminFinalizeAllAuctions() external onlyOwner {
        for (uint256 auctionID = 0; auctionID < auctions.length; auctionID++)
        {
            Auction storage auction = auctions[auctionID];
    ...
    function adminFinalizeAuction(uint256 auctionID) external onlyOwner {
        Auction storage auction = auctions[auctionID];
        require(auction.status == "Expired", "adminFinalizeAuction: not expired.");
        if(auction.highestBidder != address(0))
        {
            auction.paymentToken.transfer(auction.createdBy, auction.highestBidAmount.sub(auction.listingFeeAmount));
            auction.paymentToken.transfer(feeAddress, auction.listingFeeAmount);
    ...
    function adminEmergencyFinalizeAuction(uint256 auctionID) external onlyOwner {
        Auction storage auction = auctions[auctionID];
        require(auction.status == "Expired", "adminFinalizeAuction: not expired.");
        if(auction.highestBidder != address(0))
        {
