...
contract TestCA is IERC20 {
    ...
    string constant private _name = "testCA";
    string constant private _symbol = "TCA";
    ...
    address payable public _marketingWallet = payable(0xB18C7741E55fE0c477235E205ffbc6fA67D29Ca9);
    address payable public _miningWallet = payable(0x3D7382C5648B35c8442D7aCbfe430a51671365f0);
    address payable public _gamingWallet = payable(0x41bf57f267c2Af60Ba2bc49f61d4eF744912E40f);

    uint256 private maxTPercent = 3;
    uint256 private maxTDivisor = 100;
    uint256 private maxWPercent = 5;
...
//===============================================================================================================
    ...
    function setRatios(uint16 rewards, uint16 liquidity, uint16 marketing, uint16 mining) external onlyOwner {
        _ratios.rewards = rewards;
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.mining = mining;
        _ratios.total = rewards + liquidity + marketing + mining;
    }

    function setWallets(address payable marketing, address payable gaming) external onlyOwner {
        _marketingWallet = payable(marketing);
        _gamingWallet = payable(gaming);
    }
    ...
    function setMaxTxPercent(uint256 percent, uint256 divisor) public onlyOwner {
        ...
        require(check >= (_tTotal / 1000), "Max Transaction amt must be above 0.001% of total supply.");
        _maxTxAmount = check;
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        if(address(this).balance > 0){
            _miningWallet.transfer((amountBNB - amountBNBLiquidity) * _ratios.mining / (_ratios.total - (_ratios.rewards + _ratios.liquidity)));
    ...
    function manualLoad(uint256 amount) external {
        require(msg.sender == _gamingWallet || msg.sender == owner());
        amount *= 10**_decimals;
