...
contract DeGeThalPresaleStageOne is Ownable {
    using SafeMath for uint256;
    ...
    bool public presaleStageTwoFinalized;
    ...
    function contribute(address referrer) public payable {
        require(
            referrer != msg.sender,
            "DeGeThalPresaleStageOne: self referral not allowed"
        );
    ...
    function _buyTokens(
        ...
        if (referrer != address(0)) {
            ...
            referrerContributions[referrer] = referrerContributions[referrer].add(referrerFeeAmount);
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "DeGeThalPresaleStageOne: sale is not open");
        require(!hasEnded(), "DeGeThalPresaleStageOne: sale is over");
        require(
            weiAmount >= minContribution,
            "DeGeThalPresaleStageOne: min contribution criteria not met"
        );
        require(
            tokenContributions[beneficiary].add(weiAmount) <= maxContribution,
            "DeGeThalPresaleStageOne: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "DeGeThalPresaleStageOne: sale is not over");
        require(
            softCapReached(),
            "DeGeThalPresaleStageOne: soft cap not reached, refund is available"
        );
        require(
            presaleStageTwoFinalized,
            "DeGeThalPresaleStageOne: tokens could only be clamed after DeGeThal presale stage two is over"
        );
        require(
            tokenContributions[msg.sender] > 0,
            "DeGeThalPresaleStageOne: nothing to claim"
        );
    ...
    function claimRefund() external {
        ...
        require(
            !softCapReached(),
            "DeGeThalPresaleStageOne: soft cap not reached"
        );
        require(
            tokenContributions[msg.sender] > 0,
            "DeGeThalPresaleStageOne: nothing to claim"
    ...
    function endPresale() external onlyOwner {
        require(!finalized, "DeGeThalPresaleStageOne: sale is already over");
        finalized = true;
    ...
    function setPresaleStageTwoFinalized() external onlyOwner {
        presaleStageTwoFinalized = true;
    }

    function withdrawTokens() public onlyOwner {
        require(finalized, "DeGeThalPresaleStageOne: sale is not over");
