...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all FuckInfluencers just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also FuckInfluencersed.
	...
	// File: contracts/FuckInfluencersBEP20PayingToken.sol
	...
	// File: contracts/FuckInfluencers.sol

	// 

	pragma solidity ^0.8.0;

	contract FuckInfluencers is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingWallet = payable(0x60aD93EC17aaC6AE18a554975444574911F24191); 
		...
		event excludeMultipleAccountFromRewards(address[] FuckInfluencers, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _val = 100000000 * (10**18); 
		...
		constructor() ERC20("Fuck Influencers", "FCKINFLUENCERS"){
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "FuckInfluencers: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "FuckInfluencers: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "FuckInfluencers: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "FuckInfluencers: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "FuckInfluencers: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "FuckInfluencers: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "FuckInfluencers: Cannot update distributorGas to same value");
		...
		function changeDeadBlocks (uint256 percentage) external onlyOwner {
            deadBlocks = percentage;
	

        }
		function setDeadBlocks (bool _enabled) external onlyOwner {
           deadBlocksEnabled = _enabled;
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if (deadBlocksEnabled)
			if (block.number <= launchBlock + deadBlocks && tx.gasprice <= gweiLimit) {
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "FuckInfluencers: This account cannot send tokens until trading is enabled");
			}
			...
			if(takeFee) {
				...
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(deadFees).div(100);
				}
