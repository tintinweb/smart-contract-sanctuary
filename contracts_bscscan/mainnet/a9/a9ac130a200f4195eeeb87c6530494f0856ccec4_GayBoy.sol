...
contract GayBoy is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x02717Fa5A598feD493d4983FAE722d0C3B432F15); // Marketing Address
    address payable public devAddress = payable(0x374381D4Ba06e19378cFfE55E58F8E1CfFbd9948); // DEV Address
    ...
    string private _name = "Gay Boy";
    string private _symbol = "GIBO";
    uint8 private _decimals = 9;

    bool private sniperProtection = true;
    bool public _hasLiqBeenAdded = false;
    uint256 private _liqAddBlock = 0;
    uint256 private snipeBlockAmt = 4;
    ...
    uint256 public _maxWalletAmount = 20000000 * 10**6 * 10**9; // 20 trillion, or 2% of total supply
    ...
    function checkBlackist(address account) public view returns (bool){
        bool blacklisted = _isBlacklisted[account];
        return blacklisted;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != address(0xdead) && to != uniswapV2Pair) {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            if (overMinimumTokenBalance) {
                ...
                swapTokens(contractTokenBalance, marketingDivisor, devDivisor);    
            }
    ...
    function swapTokens(uint256 contractTokenBalance, uint256 marketingFee, uint256 devFee) private lockTheSwap {
        ...
        transferToAddressETH(marketingAddress, transferredBalance.div(_liquidityFee).div(devFee).mul(marketingFee));
        //Send to Dev address
        transferToAddressETH(devAddress, transferredBalance.div(_liquidityFee).div(marketingFee).mul(devFee));
