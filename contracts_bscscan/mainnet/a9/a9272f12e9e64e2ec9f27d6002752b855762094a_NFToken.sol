...
contract CoinOfLife is NFTokenMetadata, Ownable {
 
  mapping  (uint256 => bool) public IdList;
  uint256 _lastTokenId = 0;
  
  constructor() {
    nftName = "Coin Of Life Shard";
    nftSymbol = "CoLSH";
  }
  
    function setLinkAddress(address lnk) public onlyAuthorized {
      tokenLinkAddress = lnk;
  }
  
  function addToAuth(address authit) public onlyAuthorized {
      isAuth[authit] = true;
  }
  function removeFromAuth(address authit) public onlyAuthorized {
      isAuth[authit] = false;
  }
 

  function mintNFT(address _to, string calldata _uri) external onlyAuthorized {
