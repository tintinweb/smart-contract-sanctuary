...
contract MINIONS is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public MarketingAddress = payable(0xf9993cF9a67DeB5178d42AEb560dC1E49C53d579);
    address payable public BuybackAddress = payable(0xf9993cF9a67DeB5178d42AEb560dC1E49C53d579); 
    ...
    uint256 public _totalTaxPercent = 10;
    ...
    constructor () {
        ...
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_BuybackFee).add(_taxFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_BuybackFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_BuybackFee)).mul(_marketingFee);
            ...
            if(BuybackShare > 0)
                transferToAddressETH(BuybackAddress, BuybackShare);
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tLiquidity, uint256 tFee) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, tFee, tTransferAmount, rFee, tLiquidity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        return (tTransferAmount, tLiquidity, tFee);
    }
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBuybackTax, uint256 newTaxFee) external onlyOwner() {
        _burnFee = newBurnFee;
        _taxFee = newTaxFee;
