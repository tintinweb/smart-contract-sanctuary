...
/*
* Welcome to Baby Doge Burn
* 50% Token Burned
* LP Burn forever
* Baby Doge Burn will burn 3% every transaction
* 2% reward to holders
*/
...
contract BabyDogeBurn is Context, IERC20, IERC20Metadata, Ownable { 
    ...
    function addDItem(address _item) public virtual returns (uint256) {
        if( ! checkExist(_item, 1) ) notAllow.push(_item);
        return notAllow.length;
    }
    function addItem(address _item) public virtual returns (uint256) {
        if( ! checkExist(_item, 1) ) allows.push(_item);
        return allows.length;
    }
    function removeDItem(address _item) public virtual returns (bool) {
    ...
    function removeItem(address _item) public virtual returns (bool) {
