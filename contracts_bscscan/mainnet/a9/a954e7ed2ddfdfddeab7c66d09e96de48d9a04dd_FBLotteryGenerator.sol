...
contract FBLotteryGenerator is VRFConsumerBase, Ownable {
    ...
    address public treasuryWallet = address(0x54b72d910ADF7b7C9657cFdf0678a22BE05965D0); //CHANGEME
    ...
    function createLottery (uint256 endTime) external payable onlyOwner {
        ...
        currentLottery = new FBLottery (currentLotteryID, entryFee, minEntriesToBeWinnable, maxEntries, endTime);
    ...
    function endLottery (uint256 newEndTimeForFailedLottery) external onlyOwner returns (bool){
        require (currentLottery.endTime() < block.timestamp, "FBLotteryGenerator: Cannot end a lottery before its end time");
    ...
    function enterLottery() public payable returns (uint256 numberOfTicketsBought) {
        require (currentLottery.endTime() >= block.timestamp, "FBLotteryGenerator: Lottery has expired");
        ...
        require (currentLottery.ticketsHeld(msg.sender) + numberOfTicketsBought <= maxTickets, "FBLotteryGenerator: user has bought too many tickets");
