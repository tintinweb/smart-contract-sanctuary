...
contract SOWL is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function liquidityMining(uint256 amount)
        public
        payable
        nonReentrant
        isHuman
    {
        require(balanceOf(_msgSender()) >= amount, "Insufficient user balance");
        ...
        uint256 bonus = amount.mul(bonusPercent).div(1e3);
        _transfer(owner(), _msgSender(), bonus);
    }
