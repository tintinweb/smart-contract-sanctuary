...
contract PACMOON is ERC20, Ownable {
    ...
    address payable public operations1Address =
        payable(0x8530DDF2745b6C741827099D2dc06078F78d6fe7); 
    address payable public operations2Address =
        payable(0x02D5A7A84C088432FfBb05b43cf8F1d9fD9aB062);
    ...
    constructor() ERC20("PACMOON", "PMOON") {
        dividendTracker = new PACMOONDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        PACMOONDividendTracker newDividendTracker = PACMOONDividendTracker(
            payable(newAddress)
        );
...
contract PACMOONDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "PACMOON_Dividend_Tracker",
            "PACMOON_Dividend_Tracker"
        )
    {
        pACMOON = PACMOON(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "PACMOON_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PACMOON contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(pACMOON);

            uint256 prevBalance = pACMOON.balanceOf(address(this));
            ...
            {
                uint256 received = pACMOON.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    pACMOON.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
