...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    */
    ...
    event ERC20AddressUpdated(address previous, address current);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_marketingRewardEnabled && _autoSwapAndLiquifyEnabled)
        ...
        if( // BUY
            ...
            require(amount <= maxBuyAmount, "Buy transfer amount exceeds max limit.");
            buying = true;
        }
        ...
        if (!_isExcludedFromFee[recipient]) {
            require(balanceOf(recipient)+values.tTransferAmount <= maxWalletAmount, "Anti-whale: Cannot store that amount of tokens.");
        }
    ...
    function _beforeTokenTransfer() internal virtual {
        ...
        {
            uint256 sendShare = (contractBalance * _taxMarketing) / (_taxMarketing + _taxLiquify);
            uint256 liqShare = contractBalance - sendShare;

            sendFeeInERC20ToAddress(marketingAddress, sendShare);
            swapAndLiquify(liqShare);
        }
    }
    ...
    function _afterTokenTransfer(ValuesFromAmount memory values, bool selling, bool buying) internal virtual {
        ...
        if (buying || selling) {
            ...
            if (_marketingRewardEnabled && _autoSwapAndLiquifyEnabled) {
                _tokenBalances[address(this)] += values.tMarketingFee + values.tLiquifyFee;
                _reflectionBalances[address(this)] += values.rMarketingFee + values.rLiquifyFee;
            }
        }
    }
    ...
    */
    ...
    /**
      ...
      * (because tax rate is in percentage)
      */
    function enableAutoSwapAndLiquify(uint32 taxLiquify_, uint32 taxLiquifyDecimals_, address routerAddress, uint256 minTokensBeforeSwap_) private {
    ...
    function enableMarketingTax(uint32 taxMarketing_, uint32 taxMarketingDecimals_, address marketingAddress_) private {
    ...
    function enableAutoSwapAndLiquifyAndMarketingTax(
        uint32 taxLiquify_, 
        uint32 taxLiquifyDecimals_, 
        address routerAddress,
        uint32 taxMarketing_,
        uint32 taxMarketingDecimals_,
        address marketingAddress_,
        uint256 minTokensBeforeSwap_ ) public onlyOwner
    {
        enableAutoSwapAndLiquify(taxLiquify_, taxLiquifyDecimals_, routerAddress, minTokensBeforeSwap_);
        enableMarketingTax(taxMarketing_, taxMarketingDecimals_, marketingAddress_);
    }
    ...
    /**
      ...
      */
    function disableAutoSwapAndLiquify() private {
    ...
    function disableMarketingTax() private {
    ...
    function disableAutoSwapAndLiquifyAndMarketingTax() public onlyOwner {
    ...
    function setErc20Address(address erc20Address_) public onlyOwner {
        require(erc20Address != erc20Address_, "New ERC20 address must be different than old one.");

        address previous = erc20Address;
        erc20Address = erc20Address_;

        emit ERC20AddressUpdated(previous, erc20Address_);
    }

    function sendFeeInERC20ToAddress(address addr, uint256 amount) private lockTheSwap {
        swapTokensForERC20(amount, addr, erc20Address);
    }

    function swapTokensForERC20(uint256 amount_, address recipient_, address erc20Address_) private {
        // generate the uniswap pair path of token -> weth -> erc20
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _uniswapV2Router.WETH();
        path[2] = erc20Address_;

        _approve(address(this), address(_uniswapV2Router), amount_);

        // make the swap
        _uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amount_,
            0, // accept any amount of erc20 token
            path,
            recipient_,
            block.timestamp + 60 * 1000
        );
...
contract SuperMetaInu is ExtendedReflections {

    uint256 private _tokenSupply = 1000000000000000; // 1Q

    uint32 private _taxBurn = 1;
    uint32 private _taxReward = 1;
    uint32 private _taxMarketing = 4;
    uint32 private _taxLiquify = 4;

    uint32 private _taxDecimals = 0;
    uint32 private _decimals = 9;

    uint256 private _minTokensBeforeSwap = 1000000 * (10 ** _decimals);
    uint256 private _maxBuyAmount = _tokenSupply;
    uint256 private _maxWalletAmount = 50000000000000; // 5% of tokenSupply
     ...
     */
    address private _erc20RewardAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;

    address private _marketingAddress = 0xa80DF0673130fa50cc7dA7fDcBB93bcE8ffc7188;

    constructor () ExtendedReflections("SuperMeta Inu", "SMINU", _decimals, _tokenSupply) {
        enableAutoBurn(_taxBurn, _taxDecimals);
        enableReward(_taxReward, _taxDecimals);
        enableAutoSwapAndLiquifyAndMarketingTax(_taxLiquify, _taxDecimals, _routerAddress, _taxMarketing, _taxDecimals, _marketingAddress, _minTokensBeforeSwap);
        changeMaxBuyAmount(_maxBuyAmount);
        changeMaxWalletAmount(_maxWalletAmount);
        setErc20Address(_erc20RewardAddress);
    }
}