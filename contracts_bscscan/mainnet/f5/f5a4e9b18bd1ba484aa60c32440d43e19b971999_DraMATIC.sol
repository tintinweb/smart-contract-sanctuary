...
contract DraMATIC is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "DraMATIC";
    string constant _symbol = "dM";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000 * (10 ** _decimals); // 100,000,000,000
    ...
    function TradingStatus(uint256 amount) external authorized {
        _maxTxAmount = amount;
	...
	function BabyRiseFRSniperBan(address user, bool banned) external authorized {
		if (banned) {
    ...
    function BabyRiseFRsetSwapBackSettings(bool _enabled, uint256 _amount) external authorized {
