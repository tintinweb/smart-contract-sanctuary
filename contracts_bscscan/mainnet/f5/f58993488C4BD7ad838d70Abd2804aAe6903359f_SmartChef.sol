...
contract SmartChef is Ownable {
    ...
    address public treasuryAddress = 0xbCc5EE633A89FFf50Edaed625A3A63Ed68569c0A;
    ...
    function safeTokenTransfer(IBEP20 token, address _to, uint256 _amount) internal {
        uint256 MashBal = token.balanceOf(address(this));
        if (_amount > MashBal) {
            token.safeTransfer(_to, MashBal);
        } else {
            token.safeTransfer(_to, _amount);
        }
    }

    // Stake Mash tokens to SmartChef
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) { 
            _amount = deflacionaryDeposit(stakeToken,_amount);
            if (depositFee > 0) {
                uint256 feeAmount = _amount.mul(depositFee).div(10000);
                stakeToken.transfer(treasuryAddress, feeAmount);
            }
    ...
    function updateTreasuryAddress(address _treasuryAddress) public {
        require(treasuryAddress == msg.sender, 'not treasury??');
