...
contract WedexChef is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (address(wedexReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            wedexReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingWedex(_pid);
        if (_amount > 0) {
            if(address(wedexReferral) != address(0)) {
                address referrer = wedexReferral.getReferrer(msg.sender);
                if(referrer!=address(0)) {
                    wedexReferral.addTotalFund(referrer,_amount,0);
                    }
                else {
                    wedexReferral.addTotalFund(referrer,_amount,0);
                }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if(address(wedexReferral) != address(0)&&wedexReferral.getReferrer(msg.sender)!=address(0)){
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if(address(wedexReferral) != address(0)&&wedexReferral.getReferrer(msg.sender)!=address(0)){
            wedexReferral.reduceTotalFund(wedexReferral.getReferrer(msg.sender),amount,0);
            }
