...
contract TheRelis is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "The Real Life Simulations";
    string private constant _symbol = "TheRelis";
    uint8 private constant _decimals = 18;
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromFee;
    uint256 private constant _tTotal = 1000000000000 * 10**18; // 1 Trillion
    uint256 public _priceImpact = 2;
    uint256 private _firstBlock;
    uint256 private _botBlocks;
    uint256 public _maxWalletAmount;
    uint256 private _maxSellAmountBNB = 5; // 5 BNB
    ...
    address payable private _marketingAddress = payable(0xFd58bd13DFdd1b9dC32F12BC28843D8d50159819);
    address payable private _teamAddress = payable(0xFd58bd13DFdd1b9dC32F12BC28843D8d50159819);
    ...
    event FeesUpdated(uint256 _marketingFee, uint256 _liquidityFee, uint256 _teamFee, uint256 _burnFee);
    ...
    function removeAllFee() private {
        if (_marketingFee == 0 && _liquidityFee == 0 && _teamFee == 0 && _burnFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        ...
        fees.tAmount = amount.sub(fees.tMarketing).sub(fees.tLiquidity).sub(fees.tTeam).sub(fees.tBurn);
    ...
    function setTaxes(uint256 marketingFee, uint256 liquidityFee, uint256 teamFee, uint256 burnFee) external onlyOwner() {
        uint256 totalFee = marketingFee.add(liquidityFee).add(teamFee).add(burnFee);
        ...
        emit FeesUpdated(_marketingFee, _liquidityFee, _teamFee, _burnFee);
    }
