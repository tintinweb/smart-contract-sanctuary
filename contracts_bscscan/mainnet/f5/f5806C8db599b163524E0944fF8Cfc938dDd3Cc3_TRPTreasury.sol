...
contract TRPTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STRP }
    ...
    address public sTRP;
    uint public sTRPQueue; // Delays change to sTRP address
    ...
    /**
        @notice allow approved address to deposit an asset for TRP
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint TRP needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TRP ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TRP for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITRPERC20( TRP ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTRP ).balanceOf( msg.sender ); // Can only borrow against sTRP held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TRP
        @param _amount uint
     */
    function repayDebtWithTRP( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITRPERC20( TRP ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TRP, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TRP ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TRP ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TRP valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TRP decimals
            value_ = _amount.mul( 10 ** IERC20( TRP ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STRP ) { // 9
            sTRPQueue = block.number.add( blocksNeededForQueue );
        } else return false;
