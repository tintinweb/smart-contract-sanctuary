...
contract gummygame {
    ...
    address public pancakeAddress = 0x6884938B9221138b7EADC57BBBD988675F872bf8;
    ...
    function play() public {
        ...
        if ( keccak256(bytes(ultimatebean)) == keccak256(bytes("orange"))) {
            tokenAmount = ((orangeReward * (tokenPrice/10))/100) * tokendecimal;
        }
        if ( keccak256(bytes(ultimatebean)) == keccak256(bytes("pink"))) {
            tokenAmount = ((pinkReward * (tokenPrice/10))/100) * tokendecimal;
        }
        if ( keccak256(bytes(ultimatebean)) == keccak256(bytes("red"))) {
            tokenAmount = ((redReward * (tokenPrice/10))/100) * tokendecimal;
        }
        if ( keccak256(bytes(ultimatebean)) == keccak256(bytes("green"))) {
            tokenAmount = ((greenReward * (tokenPrice/10))/100) * tokendecimal;
        }
        if ( keccak256(bytes(ultimatebean)) == keccak256(bytes("blue"))) {
            tokenAmount = ((blueReward * (tokenPrice/10))/100) * tokendecimal;
        }
