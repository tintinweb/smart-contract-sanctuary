...
contract BakeTheCake is ERC20, Ownable {
    ...
    function updateBakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(bakeDividendTracker), "EtherBack: The dividend tracker already has that address");

        BakeDividendTracker newBakeDividendTracker = BakeDividendTracker(payable(newAddress));

        require(newBakeDividendTracker.owner() == address(this), "EtherBack: The new dividend tracker must be owned by the EtherBack token contract");
