...
contract DividendDistributor is IDividendDistributor {
    ...
    function setDistributionCriteria(uint256 newMinPeriod, uint256 newMinDistribution) external override MainCoonToken {
        minPeriod = newMinPeriod;
        minDistribution = newMinDistribution;
    }

    function setShare(address shareholder, uint256 amount) external override MainCoonToken {
    ...
    function deposit() external payable override MainCoonToken {
    ...
    function process(uint256 gas) external override MainCoonToken {
    ...
    function claimDividend(address shareholder) external MainCoonToken{
        distributeDividend(shareholder);
    }
    
    function rescueDividends(address to) external MainCoonToken {
        RewardToken.transfer(to, RewardToken.balanceOf(address(this)));
    }
    
    function setRewardToken(address _rewardToken) external MainCoonToken{
...
contract MainCoonCoin is IBEP20, Auth {
    
    using SafeMath for uint256;

    string constant _name = "MainCoonCoin";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0xfa04262bE4308E8f9a1D53969710C40bADA29078; //Where the 1% marketing fee will be sent
