...
contract BananaPresale is Ownable, ReentrancyGuard {
  ...
  constructor (address _bananaToken, uint256 _minInvestFund, uint256 _maxInvestFund) {
  ...
  function getAvailableToken(address investor) public view returns(uint256) {
    require(stages.length > 0, "Banana Sales: Invalid Stage Count");
  ...
  function setStage(
    ...
    require(stageIndex < stages.length, "Banana Sales: Invalid Stage Index");
  ...
  function setInvestRange(
    ...
    require(_min < _max, "Banana Sales: Invalid Range");
    minInvestFund = _min;
    maxInvestFund = _max;
  }
  ...
  function setVestingStrategy(
    ...
    require(_tge <= 100, "Banana Sales: invalid tge percentage");
    require(_duration > 0, "Banana Sales: invalid duration");

    tge = _tge;
    duration = _duration;
  }

  function setBananaToken(address _bananaToken) external onlyOwner {
    require(_bananaToken != address(0), "Banana Sales: zero token address");
    bananaToken = IERC20(_bananaToken);    
  }
  ...
  function invest() external payable {
    ...
    require(_bnbAmount != 0, "Banana Sales: Zero Fund");
    require(_bnbAmount >= minInvestFund && _bnbAmount <= maxInvestFund, "Banana Sales: Invalid Investment Amount");

    (uint256 stageIndex, bool isLive) = getCurrentStage();

    require(isLive, "Banana: Stage is not live now");
  
    uint256 newAmountRaised = stages[stageIndex].amountRaised.add(_bnbAmount);
    require(newAmountRaised <= stages[stageIndex].fundingGoal, "Banana Sales: Too much investment");
  ...
  function claim() external  _afterDeadline nonReentrant {
    require(balanceOf[msg.sender] > 0, "Banana Sales: No tokens to claim");

    uint256 tokensAvailable = getAvailableToken(msg.sender);
    require(tokensAvailable > 0, "Banana Sales: No tokens to claim");

    bananaToken.transfer(msg.sender, tokensAvailable);
  ...
  function forwardStageFunds(uint256 stageIndex, uint256 percent, address beneficiary) external onlyOwner {
    require(stageIndex < stages.length, "Banana Sales: invalid stage index");
    require(block.timestamp > stages[stageIndex].closeTime, "Banana Sales: stage not ended yet");
    require(stages[stageIndex].forwarded == false, "Banana Sales: stage funds already forwarded");

    require(percent <= 19, "Banana Sales: too much percent");

    uint256 totalAmount = stages[stageIndex].amountRaised;
    uint256 forwardBalance = totalAmount.mul(percent).div(100);

    require(forwardBalance <= address(this).balance, "Banana Sales: insufficient balance");

    stages[stageIndex].forwarded = true;
    payable(beneficiary).transfer(forwardBalance);
  }

  function forwardBNB(address beneficiary) external onlyOwner {
    require(beneficiary != address(0), "Banana Sales: zero beneficiary address");
    uint256 withdrawBalance = address(this).balance;
    payable(beneficiary).transfer(withdrawBalance);
  }

  function withdrawToken() external onlyOwner {

    uint256 currentBalance = bananaToken.balanceOf(address(this));

    uint256 withdrawBalance = currentBalance.add(totalClaimed).sub(totalSold);

    require(withdrawBalance > 0, "Banana Sales: Zero Token");

    bananaToken.transfer(msg.sender, withdrawBalance);
  }
