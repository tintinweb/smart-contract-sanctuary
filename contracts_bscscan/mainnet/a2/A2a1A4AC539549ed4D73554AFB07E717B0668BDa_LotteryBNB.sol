...
contract LotteryBNB is LotteryOwnable, Initializable {
    using SafeMath for uint256;
    ...
    function buy(uint8[4] memory _numbers) external payable{
        require(!drawed(), 'drawed, can not buy now');
    ...
    function multiBuy( uint8[4][] memory _numbers) external payable{
        require (!drawed(), 'drawed, can not buy now');
        require(!drawingPhase, 'drawing, can not buy now');
        uint256 _price = msg.value;
        require (_price >= minPrice, 'price must above minPrice');
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            payable(address(msg.sender)).transfer(totalReward);
        }
