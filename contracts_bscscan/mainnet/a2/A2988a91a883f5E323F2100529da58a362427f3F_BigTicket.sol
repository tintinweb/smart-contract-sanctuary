...
contract BigTicketDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BigTicket_Dividend_Tracker", "BigTicket_Dividend_Tracker") {
        claimWait = 14400;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BigTicket_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "BigTicket_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BigTicket contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BigTicket_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BigTicket_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BigTicket is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BigTicketDividendTracker public dividendTracker;
    ...
    function setCharityWallet(address payable _newAdd) public onlyOwner {
        CharityWallet = _newAdd;
    }


    constructor() ERC20("BigTicket", "BIGT") {
        BNBRewardsFee = 8;
        liquidityFee = 2;
        ...
        CharityWallet = payable(0xc214Dd8F50D4A0AA3ced5cba4249105Dbcccf2A8); // Acc2

        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee).add(CharityFee); // total fee transfer and buy

    	dividendTracker = new BigTicketDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BigTicket: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BigTicket: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BigTicket: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BigTicket: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 600000, "BigTicket: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BigTicket: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if((!_whiteList[from])){
            require(balanceOf(from)<=2000000*10**18, "Balance of holder is more than max limit");
        }
        if(amount == 0) {
