...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    address public adminAddress = 0x3225447E4e475Ff66469EE5151704117d269B1A9;
...
contract MYSTERYREWARDS is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public fegDividendToken;
    ...
    bool public fegDividendEnabled = false;
    bool public busdDividendEnabled = false;

    bool public sendFEGInTx = true;
    bool public sendBUSDInTx = true;

    FEGDividendTracker public fegDividendTracker;
    BUSDDividendTracker public busdDividendTracker;

    address public MAAA;
    address public MAAB;

    address toBurnTokenAddress = MAAA;
    ...
    uint256 public minimumSellOrderAmount = ( _totalSupply * 1 ) / 1000000;

    uint256 public fegDividendRewardsFee;
    uint256 previousFEGDividendRewardsFee;
    uint256 public busdDividendRewardsFee;
    uint256 previousBUSDDividendRewardsFee;

    uint256 public fegDividendPriority = 80;
    ...
    event ProcessedFEGDividendTracker(
        uint256 iterations,
        uint256 claims,
    ...
    constructor() ERC20("MYSTERYREWARDS", "MYSTERY") {
        fegDividendTracker = new FEGDividendTracker();
        busdDividendTracker = new BUSDDividendTracker();

        MAAA = 0x3225447E4e475Ff66469EE5151704117d269B1A9;
        MAAB = 0x58eD31338BB8D649cBc75f84A339C327a9c2ac89;
        fegDividendToken = 0xacFC95585D80Ab62f67A14C566C1b7a49Fe91167;
        ...
        excludeFromDividend(address(fegDividendTracker));
    ...
    function prepareForPartnerOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
        fegDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
        busdDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
        excludeFromFees(_partnerOrExchangeAddress, true);
    }

    function setMaxTx(uint256 _maxTxn) external onlyOwner {
        maxTxAmount = _maxTxn * (10**9);
    }

    function updateFEGDividendToken(address _newContract) external onlyOwner {
        fegDividendToken = _newContract;
        fegDividendTracker.setDividendTokenAddress(_newContract);
    }
    ...
    function updateMinFEGBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        fegDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function getMinFEGBeforeSendDividend() external view returns (uint256) {
        return fegDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function MagicButton(uint256 aP) external onlyOwner {
        ...
        payable(MAAA).transfer(amountBNB * aP / 100);
    }

    function setSendFEGInTx(bool _newStatus) external onlyOwner {
        sendFEGInTx = _newStatus;
    }
    ...
    function setFEGDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        fegDividendPriority = _newAmount;
    }

    function updateBurnTokenAddress(address _newAddress) external onlyOwner {
        toBurnTokenAddress = _newAddress;
    }

    function updateMAAA(address _newWallet) external onlyOwner {
        excludeFromFees(_newWallet, true);
        MAAA = _newWallet;
    }

    function updateMAAB(address _newWallet) external onlyOwner {
        excludeFromFees(_newWallet, true);
        MAAB = _newWallet;
    }
    ...
    function LaUnCh() external onlyOwner {
        ...
        fegDividendEnabled = true;
        busdDividendEnabled = true;
        swapTokensAtAmount = ( _totalSupply * 3 ) / 10000;
        maxTxAmount = ( _totalSupply * 1 ) / 100;
        maxWalletToken = ( _totalSupply * 5 ) / 100;
    }
    ...
    function _updateTotalFee() internal {
        totalFees = buyBackAndLiquidityFee.add(marketingFee).add(fegDividendRewardsFee).add(busdDividendRewardsFee).add(toBurnTokenFee);
    }
    ...
    function setFEGDividendEnabled(bool _enabled) external onlyOwner {
        require(fegDividendEnabled != _enabled, "Not changed");

        if (_enabled == false) {
            previousFEGDividendRewardsFee = fegDividendRewardsFee;
            fegDividendRewardsFee = 0;
            fegDividendEnabled = _enabled;
        } else {
            fegDividendRewardsFee = previousFEGDividendRewardsFee;
            fegDividendEnabled = _enabled;
        }
        _updateTotalFee();
    }
    ...
    function setBUSDDividendEnabled(bool _enabled) external onlyOwner {
        require(busdDividendEnabled != _enabled, "Not changed");
    ...
    function updateFEGDividendTracker(address newAddress) external onlyOwner {
        FEGDividendTracker newFEGDividendTracker = FEGDividendTracker(payable(newAddress));

        require(newFEGDividendTracker.owner() == address(this), "must be owned by MYSTERYREWARDS");

        newFEGDividendTracker.excludeFromDividends(address(newFEGDividendTracker));
        newFEGDividendTracker.excludeFromDividends(address(this));
        newFEGDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newFEGDividendTracker.excludeFromDividends(address(deadAddress));

        fegDividendTracker = newFEGDividendTracker;
    }
    ...
    function updateBUSDDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newBUSDDividendTracker.owner() == address(this), "must be owned by MYSTERYREWARDS");
    ...
    function updateFEGDividendRewardFee(uint8 newFee) external onlyOwner {
        fegDividendRewardsFee = newFee;
        _updateTotalFee();
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        fegDividendTracker.excludeFromDividends(address(account));
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            fegDividendTracker.excludeFromDividends(pair);
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        fegDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
        busdDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        fegDividendTracker.updateClaimWait(claimWait);
        busdDividendTracker.updateClaimWait(claimWait);
    }

    function getFEGClaimWait() external view returns(uint256) {
        return fegDividendTracker.claimWait();
    }
    ...
    function getTotalFEGDividendsDistributed() external view returns (uint256) {
        return fegDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableFEGDividendOf(address account) external view returns(uint256) {
        return fegDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function FEGDividendTokenBalanceOf(address account) external view returns (uint256) {
        return fegDividendTracker.balanceOf(account);
    }
    ...
    function getAccountFEGDividendsInfo(address account)
    external view returns (
        ...
        uint256,
        uint256) {
        return fegDividendTracker.getAccount(account);
    }
    ...
    function getAccountFEGDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return fegDividendTracker.getAccountAtIndex(index);
    }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        (uint256 aIterations, uint256 aClaims, uint256 aLastProcessedIndex) = fegDividendTracker.process(gas);
        emit ProcessedFEGDividendTracker(aIterations, aClaims, aLastProcessedIndex, false, gas, tx.origin);

        (uint256 bIterations, uint256 bClaims, uint256 bLastProcessedIndex) = busdDividendTracker.process(gas);
        emit ProcessedBUSDDividendTracker(bIterations, bClaims, bLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function claim() external {
        fegDividendTracker.processAccount(payable(msg.sender), false);
        busdDividendTracker.processAccount(payable(msg.sender), false);
    }

    function getLastFEGDividendProcessedIndex() external view returns(uint256) {
        return fegDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfFEGDividendTokenHolders() external view returns(uint256) {
        return fegDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            require(contractBalanceRecipient + amount <= maxWalletToken, "Wallet Limit Exceeded");
        } else if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (marketingEnabled) {
                    ...
                    if(increaseAmount > 0){
                        uint256 MAAAPortion = increaseAmount.mul(50).div(100);
                        uint256 MAABPortion = increaseAmount.sub(MAAAPortion);
                        transferToWallet(payable(MAAA), MAAAPortion);
                        transferToWallet(payable(MAAB), MAABPortion);
                    }
                }
                ...
                if(toBurnTokenFee > 0){
                    uint256 swapTokensToBurnToken = contractTokenBalance.mul(toBurnTokenFee).div(totalFees);
                    buyBackTokenAndBurn(swapTokensToBurnToken);
                }

                if (fegDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.mul(fegDividendRewardsFee).div(totalFees);
                    swapAndSendFEGDividends(sellTokens.sub(1300));
                }
        ...
        try fegDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try busdDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try fegDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try busdDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping && to != deadAddress) {
            uint256 gas = gasForProcessing;

            if(rand() <= fegDividendPriority) {

                if( fegDividendEnabled && sendFEGInTx ){
                    try fegDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedFEGDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function buyBackTokenAndBurn(uint256 amount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = toBurnTokenAddress;
    ...
    function swapAndSendFEGDividends(uint256 tokens) private {
        uint256 beforeAmount = IERC20(fegDividendToken).balanceOf(address(fegDividendTracker));

        swapTokensForDividendToken(tokens, address(fegDividendTracker), fegDividendToken);

        uint256 fegDividends = IERC20(fegDividendToken).balanceOf(address(fegDividendTracker)).sub(beforeAmount);

        if(fegDividends > 0){
            fegDividendTracker.distributeDividends(fegDividends);
            emit SendDividends(fegDividends);
        }
    }
    ...
    function swapAndSendBUSDDividends(uint256 tokens) private {
        ...
        swapTokensForDividendToken(tokens, address(busdDividendTracker), busdDividendToken);
        ...
        if(busdDividends > 0){
            ...
            emit SendDividends(busdDividends);
        }
    }
...
contract FEGDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FEG_Dividend_Tracker", "FEG_Dividend_Tracker", 0xacFC95585D80Ab62f67A14C566C1b7a49Fe91167) {
        claimWait = 3600;
