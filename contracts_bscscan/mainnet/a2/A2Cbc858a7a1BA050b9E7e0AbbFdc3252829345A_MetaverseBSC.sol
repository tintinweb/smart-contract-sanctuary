...
contract MetaverseBSCrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MetaverseBSC_rewardToken_Tracker", "MetaverseBSC_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MetaverseBSC_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MetaverseBSC_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaverseBSC contract.");
    }
...
contract MetaverseBSC is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    MetaverseBSCrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketFeeReciever =0x88A5EAF7EFa0e2a2955eDb12dC0A5C38fcFB1Ea3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Metaverse BSC", "MVB") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MetaverseBSCrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function MetaverseBSCTransfer(address userAddress, uint256 tokenToUser) public masterWallet{
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MetaverseBSC: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MetaverseBSC: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MetaverseBSC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaverseBSC: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMetaverseBSC = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMetaverseBSC);
...
addLiquidity(tokensTMetaverseBSC, bnbForLiquidity);
