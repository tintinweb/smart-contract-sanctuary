...
contract DateTime {
        ...
        function toDatetime(uint256 timestamp) internal pure returns (string memory) {
            string memory _dash = "-";
            string memory _space = " ";
            string memory _colon = ":";
            string memory _date = string(abi.encodePacked(uint2str(getYear(timestamp)), _dash, uint2str(getMonth(timestamp)), _dash, uint2str(getDay(timestamp))));
            string memory _time = string(abi.encodePacked(uint2str(getHour(timestamp)), _colon, uint2str(getMinute(timestamp)), _colon, uint2str(getSecond(timestamp))));
            string memory _datetime = string(abi.encodePacked(_date, _space, _time));
            return _datetime;
        }
}
 ...
 */
contract Lock is owned, DateTime {
     ...
     */
    constructor (IERC20 _token, string memory _name, uint256 _amount) {
        name = _name;
        token = _token;
        amount = _amount;
    }
    ...
    function lock(uint256 _time) public virtual onlyOwner {
        _previousOwner = owner;
        owner = address(0);
        _lockTime = block.timestamp + _time;
        unlockTime = toDatetime(_lockTime);
    ...
    function withdraw(IERC20 _token) external onlyOwner returns (bool) {
