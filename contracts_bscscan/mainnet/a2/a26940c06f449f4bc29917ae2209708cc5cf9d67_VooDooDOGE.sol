...
/*
    ...
    Launch Limits:  500,000 Max Sell, 1,500,000 max wallet
    
    16% total tax
    
    10% automatically redistributed as BNB, no need to claim.
...
contract VooDooDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("VooDooDoge", "VOODOGE") {
        uint256 _rewardsFee = 10;
        uint256 _marketingFee = 3;
        uint256 _liquidityFee = 3;
        
        uint256 totalSupply = 100000000 * 10**18; // 100 Billion total supply
    	...
    	marketingWallet = address(0x4A1933bcab659E82C17E8dF722E226FA156653a5);
        ...
        if(limitsInEffect){
            if (
                ...
                if (automatedMarketMakerPairs[from]) {
                    if (!_isExcludedMaxSellTransactionAmount[to]) {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividends = (address(this).balance.sub(initialBalance)).mul(rewardsFee).div(totalFees);
        ...
        // dev fee takes 4% of marketing Amt (1/25)
        uint256 devAmt = (address(this).balance.sub(initialBalance)).mul(marketingFee).div(totalFees - rewardsFee) / 25;
        (success,) = address(devWallet).call{value: devAmt}("");
        
        uint256 marketingAmt = (address(this).balance.sub(initialBalance)).mul(marketingFee).div(totalFees - rewardsFee) - devAmt;
