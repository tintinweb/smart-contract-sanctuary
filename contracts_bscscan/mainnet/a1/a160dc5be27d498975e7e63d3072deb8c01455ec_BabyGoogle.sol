...
* BabyGoogle ($BabyG)
* 
* Tokenomics will be a tax of 10% Buy and 14% Sell
*
* 30% for liquidity 
* 10% to burn wallet 
* 60% to staking
*
* Total Supply 10000
* Max per Holder: 3% of Total Supply (300 tokens)
* Max sell: 1% of supply (100 tokens)
* 
*
* Hold and claim GOOGL
*
*
* TG: t.me/BabyGoogleBSC
* 
*/
...
abstract contract Ownable {
    ...
    modifier isTeam() {
        require(_isTeam(msg.sender), "Not Team"); _;
    }
...
contract BabyGoogle is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'BabyGoogle';
    string private constant _symbol = '$BabyG';
    ...
    uint8 public constant BalanceLimitDivider=33;
    ...
    function _checkLiquidityAdd(address sender, address recipient) private {
        address pancakeRouter=address(_pancakeRouter);
        require(!_hasLiqBeenAdded, "Liquidity already added and marked.");
        if (sender == liqHolder && (recipient == _pancakePairAddress || recipient == pancakeRouter)){
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            if(!_excludedFromSellLock.contains(sender)){
                ...
                if(sniperProtection){if(isSniper(sender)){revert("Sniper rejected");}}
            }
            ...
            if(block.number - _liqAddBlock < snipeBlockAmt){_isSniper[recipient] = true; snipersCaught ++; emit SniperCaught(recipient);}
            tax=_buyTax;
            ...
            if(amount<=10**(_decimals)) claimGOOGL(sender);
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            require(recipientBalance+amount<=balanceLimit,"whale protection");
        ...
        //Swapping AutoLP and MBNB is only possible if sender is not pancake pair, 
    ...
    //distributes bnb between M share and dividents 
    function _distributeStake(uint256 BNBamount) private {
       ...
       dBalance+=dSplit;
        ...
        if (amount > 0) {
            ...
            //when there are 0 shares, add everything to M budget
            if (totalShares == 0) {
                dBalance += amount;
            }else{
    ...
    event OnWithdrawGOOGL(uint256 amount, address recipient);
    
    //withdraws all dividents of address
    function claimGOOGL(address addr) private{
        ...
        path[1] = 0x62D71B23bF15218C7d2D7E48DBbD9e9c650B173f;  //mGOOGL
        ...
        emit OnWithdrawGOOGL(amount, addr);
        _isWithdrawing=false;
    }
    ...
    //swaps the token on the contract for M BNB and LP Token.
    ...
    function _swapContractToken() private lockTheSwap{
        ...
        //splits the token in TokenForLiquidity and tokenForM
        uint256 tokenForLiquidity=(tokenToSwap*_liquidityTax)/totalTax;
        uint256 tokenForM= tokenToSwap-tokenForLiquidity;
        ...
        uint256 swapToken=liqBNBToken+tokenForM;
        ...
        //distributes remaining BNB between stakers and M
    ...
    function getTaxes() public view returns(uint256 burnTax,uint256 liquidityTax,uint256 stakingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
    ...
    function GOOGLWithdraw() public{
        claimGOOGL(msg.sender);
    }
    ...
    function TeamWithdrawdBNB() public isTeam{
        uint256 amount=dBalance;
        dBalance=0;
        (bool sent,) =TeamWallet.call{value: (amount)}("");
        require(sent,"withdraw failed");
    } 
    function TeamWithdrawdBNB(uint256 amount) public isTeam{
        require(amount<=dBalance);
        dBalance-=amount;
        (bool sent,) =TeamWallet.call{value: (amount)}("");
        require(sent,"withdraw failed");
    } 

    //switches autoLiquidity and M BNB generation during transfers
    function TeamSwitchManualBNBConversion(bool manual) public isTeam{
        manualConversion=manual;
    }

    //Disables the timeLock after selling for everyone
    function TeamDisableSellLock(bool disabled) public isTeam{
        sellLockDisabled=disabled;
    }

    //Sets SellLockTime, needs to be lower than MaxSellLockTime
    function TeamSetSellLockTime(uint256 sellLockSeconds)public isTeam{
    ...
    function TeamSetTaxes(uint8 burnTaxes, uint8 liquidityTaxes, uint8 stakingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public isTeam{
        uint8 totalTax=burnTaxes+liquidityTaxes+stakingTaxes;
        require(totalTax==100, "burn+liq+staking needs to equal 100%");
    ...
    //How much of the staking tax should be allocated for M
    function TeamChangedShare(uint8 newShare) public isTeam{
        require(newShare<=50); 
        dShare=newShare;
    }
    //manually converts contract token to LP and staking BNB
    function TeamCreateLPandBNB() public isTeam{
    _swapContractToken();
    }
    
     //Limits need to be at least target, to avoid setting value to 0(avoid potential Honeypot)
    function TeamUpdateLimits(uint256 newBalanceLimit, uint256 newSellLimit) public isTeam{
    ...
    function SetFeeReceiver(address _TeamWallet) public isTeam{
        TeamWallet = _TeamWallet;
        _Team[_TeamWallet] = true;
    ...
    function TeamUnlockLiquidityInSeconds(uint256 secondsUntilUnlock) public isTeam{
    ...
    function TeamReleaseLiquidity() public isTeam {
    ...
    function TeamRemoveLiquidity(bool addToStaking) public isTeam{
        //Only callable if liquidity Unlock time is over
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
