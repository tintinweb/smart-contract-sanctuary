...
contract Metaverse is Context, IERC20, Ownable {
    ...
    uint256 public minimumTokenBalanceForDividends = 100 * 10**3 * (10 ** 9); //minimum supply to hold for dividend
    ...
    function setFee(uint256 _newtaxFee, uint256 _newliquidityFee, uint256 _newmarketingFee) public onlyOwner() {
        require(_newtaxFee.add(_newliquidityFee).add(_newmarketingFee) <= 30); //the team cannot set the fees to more than 30%
        _taxFee = _newtaxFee;
    ...
    function clearstucktoken(address _token_address, address _recipient, uint256 _amount_percentage_token, uint256 _amount_percentage_eth) external onlyOwner() {
        if (_amount_percentage_token > 0) {
            ...
            token.transfer(_recipient, (amounttoken * _amount_percentage_token / 100));
        }
        if (_amount_percentage_token > 0) {
            uint256 amounteth = address(this).balance;
            require(amounteth > 0, "Transfer amount must be greater than zero");
            payable(_recipient).transfer(amounteth * _amount_percentage_eth / 100);
        }
    }
