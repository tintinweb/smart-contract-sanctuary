...
contract ISI is Context, IERC20, Ownable {
    using SafeMath for uint256;
 
    string private constant _name = "iShib";
    string private constant _symbol = "ISI";
    ...
    uint256 public minimumContractBalanceToSwap = 60000000000 * 10**9;   // 6.00% of total supply for both LQ and ETH distribution
    ...
    constructor(address payable teamFunds, address payable marketingFunds, address payable charityFunds) {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && swapEnabled && from != uniswapV2Pair) {  // Selling
                ...
                if (sellNumber[from] == 0) {
                    ...
                    sellCooldown[from] = block.timestamp + (10 seconds);
                }
                    ...
                    _taxFee = 16;
                    sellNumber[from]++;
                    sellCooldown[from] = firstSell[from] + (60 seconds);
                }
            }
    ...
    function addInitialLiquidity() external onlyOwner() {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // CHANGEIT to nominal router.
