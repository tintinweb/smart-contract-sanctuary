...
contract AlaskaFomo {
    ...
    function transferNotify(address user) external {
        require(msg.sender == address(dam), "permission denied");
    ...
    function swap() public {
        ...
        inSwap = true;
        uint256 tokenAmount = dam.balanceOf(address(this));
        if (tokenAmount == 0) {
            inSwap = false;
            return;
        }
        ...
        dam.approve(address(uniswapV2Router), tokenAmount);
    ...
    function buyAndBurn() public {
        ...
        // burn token
        uint256 tokenBalance = dam.balanceOf(address(this));
        if (tokenBalance > 0) {
            dam.transfer(blackHole, tokenBalance);
        }
    }
