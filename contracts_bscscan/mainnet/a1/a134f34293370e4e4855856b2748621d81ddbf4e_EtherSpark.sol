...
Welcome To EtherSpark,

Hold EtherSpark To Earn 9% ETH 

Join Us At https://t.me/EtherSpark
...
contract EtherSpark is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EtherSparkDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("EtherSpark", "EtherSpark") {
        _ETHRewardsFee = 9;
        _liquidityFee = 3;
		_marketingFee = 4;
    	...
    	dividendTracker = new EtherSparkDividendTracker();
    	antiBotEnabled = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "EtherSpark: The dividend tracker already has that address");

        EtherSparkDividendTracker newDividendTracker = EtherSparkDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EtherSpark: The new dividend tracker must be owned by the EtherSpark token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "EtherSpark: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "EtherSpark: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EtherSpark: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EtherSpark: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EtherSpark: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EtherSpark: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EtherSpark: Cannot update gasForProcessing to same value");
...
contract EtherSparkDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("EtherSparkDividend_Tracker", "EtherSpark_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "EtherSpark_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "EtherSpark_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherSpark contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "EtherSpark_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EtherSpark_Dividend_Tracker: Cannot update claimWait to same value");
