...
contract TaiChiBondingCalculator is IBondingCalculator {
    ...
    constructor( address _TAC ) {
        require( _TAC != address(0) );
        TAC = _TAC;
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == TAC ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( TAC ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}