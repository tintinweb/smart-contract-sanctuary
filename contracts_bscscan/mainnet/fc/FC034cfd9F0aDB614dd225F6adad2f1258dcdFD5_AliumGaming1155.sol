...
abstract contract AbstractRoyalties {
    ...
    function _saveRoyalties(uint256 id, LibPart.Part[] memory _royalties_)
        internal
    {
        uint256 totalValue;
        for (uint256 i = 0; i < _royalties_.length; i++) {
            require(
                _royalties_[i].account != address(0x0),
                "Recipient should be present"
            );
            require(
                _royalties_[i].value != 0,
    ...
    function _updateAccount(
        ...
        uint256 length = _royalties[_id].length;
    ...
    function _onRoyaltiesSet(uint256 id, LibPart.Part[] memory _royalties_)
        internal
        virtual;
}
