...
contract SpaceXChef is Ownable, ReentrancyGuard {
    ...
    event Add(uint256 allocPoint, address lpToken, uint16 depositFeeBP, bool unimexIntegration);
    event Set(uint256 pid, uint256 allocPoint, uint16 depositFeeBP, bool unimexIntegration);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _unimexIntegration) external onlyOwner nonDuplicated(_lpToken) {
        ...
        emit Add(_allocPoint, address(_lpToken), _depositFeeBP, _unimexIntegration);
    }

    // Update the given pool's SpaceX allocation point and deposit fee. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _unimexIntegration) external onlyOwner {
        ...
        emit Set(_pid, _allocPoint, _depositFeeBP, _unimexIntegration);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 spaceXReward = multiplier.mul(spaceXPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        spaceXToken.mint(devAddress, spaceXReward.div(10));
        spaceXToken.mint(address(this), spaceXReward);
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                uint256 poolDepositAmount = depositFee.sub(feeAddressAmount);
                pool.lpToken.safeTransfer(vaultAddress, poolDepositAmount);
