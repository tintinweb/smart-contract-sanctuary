...
contract Fuzion is ERC20, Ownable {
    ...
    // Fees
    uint256 public _futurUseSellFee = 5; // 5
    uint256 public _burnSellFee = 10; // 10
    uint256 public _futurUseForgeFee = 10; // 15
    uint256 public _liquidityPoolFee = 5; // 5
    uint256 public _distributionSwapFee = 2; // 2 | 2% of the 85% from rewardsForgeFee

    uint256 public _referralChildAmount = 3;

    // Pools & Wallets
    address public _futurUsePool = 0x4D6eB06BD2a7Dc5F3815c17De546006C5741E271;
    address public _distributionPool = 0x1C191A0cc91d7d79BD681fa167D7Af81bCF07213;
    ...
    uint256 public swapTokensAtAmount = 200 * (10**8);
    ...
    function giveForge(address[] memory users, string memory name, uint256 forgeType) external onlyOwner {
      for (uint256 i = 0; i < users.length; i++)
        forgeManager._createForge(users[i], name, forgeType);
    }
...
contract FuzionForgeManager is Ownable {
    ...
    function _setReferralCode(address user, string memory code) external onlyOwner {
      ...
      require(getNumberOfSpecifiedMinForges(user, _minimumForgeSetRef) > 0, "FNM: You need to have a specific forge");
      ...
      referralDatas[user].referralCode = code;
      referralDatas[user].referralGodFather = user;
    }
    ...
    function _useReferralCode(address user, string memory code) external onlyOwner returns (address) {
      ...
      require(getNumberOfSpecifiedMinForges(user, _minimumForgeUseRef) > 0, "FNM: You need to have a specific forge");

      address codeOwner = referralCodes[code].codeOwner;
      require(codeOwner != user, "FNM: You cannot use your code");
      require(
        referralDatas[codeOwner].referralChilds.length < getNumberOfSpecifiedMinForges(codeOwner, _minimumForgeSetRef) * _maxAmountPerForge,
    ...
    function _upgradeAllForges(address user) external onlyOwner returns (uint256) {
      ...
      for (uint256 i = 0; i < forges.length; i++) {
        ...
        if(!forge.upgrading && forgeType.upgradable) {
          forge.upgrading = true;
          forge.fuzionUpgradeStartTimestamp = block.timestamp;
          count++;
        }
    ...
    function getNumberOfSpecifiedMinForges(address user, uint256 forgeTypeId) private view returns (uint256) {
      Forge[] storage forges = usersForges[user];
      Forge storage forge;
      uint256 count = 0;
      ...
      for (uint256 i = 0; i < forges.length; i++) {
        forge = forges[i];
        if(forge.forgeType >= forgeTypeId && !_forgeTypes[forge.forgeType].bonus) count++;
      }
