...
import "./interfaces/IMorroc.sol";

import "./Prontera.sol";

contract MorrocView {
    function balance(IMorroc _morroc) public view returns (uint256) {
        return _morroc.izlude().balance();
    }

    function balanceOf(IMorroc _morroc, address _user) public view returns (uint256) {
        return _morroc.izlude().balanceOf(_user);
    }

    function balanceOfMasterChef(IMorroc _morroc) external view returns (uint256) {
        return _morroc.izlude().byalan().balanceOfMasterChef();
    }

    function getPricePerFullShare(IMorroc _morroc) public view returns (uint256) {
        return _morroc.izlude().getPricePerFullShare();
    }

    function want(IMorroc _morroc) public view returns (IERC20) {
        return _morroc.izlude().want();
    }

    function pendingKSW(IMorroc _morroc, address _account) public view returns (uint256) {
        return _morroc.prontera().pendingKSW(address(_morroc.izlude()), _account);
    }

    function pendingReward(IMorroc _morroc) public view returns (uint256) {
        return _morroc.izlude().byalan().pendingReward();
    }

    function kafraMaxFee(IMorroc _morroc) public view returns (uint256) {
        if (address(_morroc.izlude().feeKafra()) == address(0)) {
            return 0;
        }
        return _morroc.izlude().feeKafra().MAX_FEE();
    }

    function kafraMaxAllocation(IMorroc _morroc) public view returns (uint16) {
        if (address(_morroc.izlude().allocKafra()) == address(0)) {
            return 0;
        }
        return _morroc.izlude().allocKafra().MAX_ALLOCATION();
    }

    function limitAllocation(IMorroc _morroc) public view returns (uint16) {
        if (address(_morroc.izlude().allocKafra()) == address(0)) {
            return 0;
        }
        return _morroc.izlude().allocKafra().limitAllocation();
    }
    ...
    function canAllocate(
        ...
        return
            _morroc.izlude().allocKafra().canAllocate(
                _amount,
                _morroc.izlude().byalan().balanceOf() + _amount,
                _morroc.izlude().byalan().balanceOfMasterChef(),
                _user
            );
    }

    function depositFee(IMorroc _morroc) public view returns (uint256) {
        if (address(_morroc.izlude().feeKafra()) == address(0)) {
            return 0;
        }
        return _morroc.izlude().feeKafra().depositFee();
    }

    function withdrawFee(IMorroc _morroc) public view returns (uint256) {
        if (address(_morroc.izlude().feeKafra()) == address(0)) {
            return 0;
        }
        return _morroc.izlude().feeKafra().withdrawFee();
    }
    ...
    function calculateDepositFee(
        ...
        return _morroc.izlude().feeKafra().calculateDepositFee(_wantAmount, _user);
    }
    ...
    function calculateWithdrawFee(
        ...
        return _morroc.izlude().feeKafra().calculateWithdrawFee(_wantAmount, _user);
    }

    function performanceFee(IMorroc _morroc) public view returns (uint256) {
        return _morroc.izlude().byalan().totalFee();
    }

    function performanceMaxFee(IMorroc _morroc) public view returns (uint256) {
        return _morroc.izlude().byalan().MAX_FEE();
    }

    function izludeToPoolId(IMorroc _morroc, address _izlude) public view returns (uint256) {
        return _morroc.prontera().izludeToPool(_izlude);
    }

    function paused(IMorroc _morroc) external view returns (bool) {
        return _morroc.izlude().byalan().paused();
    }

    function allocPoint(IMorroc _morroc) external view returns (uint256) {
        Izlude izlude = _morroc.izlude();
        uint256 pid = _morroc.prontera().izludeToPool(address(izlude));
        (Izlude iz, uint256 ap, , ) = _morroc.prontera().poolInfo(pid);
        if (izlude != iz) {
            return 0;
        }
        return ap;
...
interface IMorroc {
    function prontera() external view returns (Prontera);

    function izlude() external view returns (Izlude);

    function alberta() external view returns (IAlberta);
}
