...
contract ShibaSuperstar is Context, IERC20, Ownable {
    ...
    uint256 public _marketingMilipercent = 1000;
    uint256 public _buyBackMilipercent = 5000;
    uint256 public _dividendMilipercent = 0;
    ...
    constructor() {
        _name = 'ShibaSuperstar';
        _symbol = 'Superstar';
        _decimals = 9;
        _totalSupply = 9 * 10**4 * 10**9;
        ...
        _whiteListEnabled = true;
        setW(address(0), true);
        setW(address(this), true);
        setW(_msgSender(), true);
     ...
     * @return boolean
     */
    function setMarketingM(uint256 value) external onlyOwner returns (bool) {
        _marketingMilipercent = value;
         require(_marketingMilipercent + _buyBackMilipercent + _dividendMilipercent <= 15000, 'Fee must not increase');
     ...
     * @return boolean
     */
    function setBuybackM(uint256 value) external onlyOwner returns (bool) {
     ...
     * @return boolean
     */
    function setDividendM(uint256 value) external onlyOwner returns (bool) {
     ...
     * @return boolean
     */
    function setWEnabled(bool value) public onlyOwner returns (bool) {
     ...
     * @return boolean
     */
    function setW(address account, bool value) public onlyOwner returns (bool) {
    ...
    function emergencyC() external onlyOwner returns (bool) {
        IERC20 BusdToken = IERC20(BUSD);
        uint256 busdBalance = BusdToken.balanceOf(address(this));
        if (busdBalance > 0){
