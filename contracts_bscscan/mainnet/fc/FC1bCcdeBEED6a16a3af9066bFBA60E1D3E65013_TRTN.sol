...
==========T===R===I===T===O===N=====================<%{%{%{>>+===--->
	...
	Triton - First Moon of Neptune
	The first official spin-off stealth launch rewarding in NEPTUNE!

	Tokenomics:
	10% rewards fee for NEPTUNE rewards distribution
	2% liquidity fee
	3% burn fee
	1% max wallet balance
	
	Liquidity will be locked for a minimum of one month.
	
	Telegram: https://t.me/Triton_Official
 	Website:  https://www.neptune-bsc.com/#Roadmap
...
contract TRTN is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    event TokenBurn(address indexed from, uint256 value);
    event SetLiquidityFee(uint256 amount);
    event SetRewardsFee(uint256 amount);
    event SetBurnFee(uint256 amount);
    
    string private _name = "Triton";
    string private _symbol = "TRTN";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public rewardsDistributor = payable(0x2A01D7eD5B86fcad176C91B2B3Ce748Bc88fad44);
    address public rewardsToken = 0xbFc5D671Ccdeb25654EF83C61b1535B2CB0A29c7;
    ...
    constructor () {
        ...
        _rewardsFees = 10;
        _burnFees = 3;
        _totalFees = _liquidityFees.add(_rewardsFees).add(_burnFees);
        _totalFeesContract = _liquidityFees.add(_rewardsFees);
    ...
    function setRewardsDistributor(address payable newRewardsDistributor) external onlyOwner() {
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFees).add(_rewardsFees) <= _maxFees, "Fees are too high.");
    ...
    function setRewardsFeePercent(uint256 newRewardsFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFees.add(_burnFees).add(newRewardsFee) <= _maxFees, "Fees are too high.");
        _rewardsFees = newRewardsFee;
        _totalFees = _liquidityFees.add(_rewardsFees).add(_burnFees);
        _totalFeesContract = _liquidityFees.add(_rewardsFees);
        emit SetRewardsFee(_rewardsFees);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFees.add(newBurnFee).add(_rewardsFees) <= _maxFees, "Fees are too high.");
    ...
    function _transfer(
        ...
        if(
            ...
            _totalFeesContract > 0 &&                         // LiquidityFee + RewardsFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 rewardsTokensToSell = balanceOf(address(this)).mul(_rewardsFees).div(_totalFeesContract);
        
        // Get collected Liquidity Fees 
        swapAndLiquify(liquidityTokensToSell);  

        // Get collected Rewards Fees 
        swapAndSendToFee(rewardsTokensToSell); 
    }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        // Transfer sold Token to rewardsDistributor
        IERC20(rewardsToken).transfer(rewardsDistributor, IERC20(rewardsToken).balanceOf(address(this)));
    }
