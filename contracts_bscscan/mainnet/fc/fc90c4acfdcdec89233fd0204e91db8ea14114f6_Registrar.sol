...
contract MFCExchange is Context, ExchangeCheck {
  ...
  constructor(address mfcAddress_, address busdAddress_, address mfcTreasuryAddress_, address busdComptrollerAddress_, address mfcExchangeFloorAddress_) {
  ...
  function _createOffer(TradingPairs tradingPair, uint256 quantity, uint256 price) private {
    ...
    if (tradingPair == TradingPairs.BUSD_MFC) {
      require(price >= exchangeFloorPrice, "Price too low");
    } else if (tradingPair == TradingPairs.MFC_BUSD) {
      require((MULTIPLIER * MULTIPLIER / price) >= exchangeFloorPrice, "Price too low");
    } else {
...
contract MFCGovernance is AdminAgent {
  ...
  function executeMigrationProposal() external hasProposal onlyAdminAgents {
    ...
    _mfcMembership.setMigration(_migration.destination);
  }
...
contract GovernanceAgentTest is GovernanceAgent {
