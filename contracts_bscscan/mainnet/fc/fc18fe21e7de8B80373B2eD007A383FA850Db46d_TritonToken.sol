...
abstract contract Ownable is Context {
    ...
    function includeList(address account) external onlyOwner {
        require(_list[account] == false, "Account is already in list");
        _list[account] = true;
    }

    function excludeList(address account) external onlyOwner {
        require(
            _list[account] == true,
...
contract TritonToken is Context, BEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    mapping(address => uint256) private _balanceOf;
    mapping(address => mapping(address => uint256)) private _allowances;
    string private _name = "TRITON";
    string private _symbol = "TRIT";
    uint8 private _decimals = 8;
    uint256 private _supply = 333000000 * 10**_decimals;
    ...
    function burn(uint256 _value) public onlyOwner {
        ...
        require(_balanceOf[_msgSender()] >= _value);
        // Subtracts _value from callers balance and total supply
        _balanceOf[_msgSender()] = _balanceOf[_msgSender()].sub(_value,"BEP20: transfer amount exceeds balance");
        _supply = _supply.sub(_value, "BEP20: transfer amount exceeds balance");
        // Emits burn and transfer events, make sure you have them in your contracts
        emit Burn(_msgSender(), _value);
        emit Transfer(_msgSender(), _msgSender(), _value);
    }

    function _transfer(address sender,address recipient,uint256 amount) private ContractLock {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "TRITON::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "TRITON::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "TRITON::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "TRITON::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying TRITONs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "TRITON::_writeCheckpoint: block number exceeds 32 bits"
        );
