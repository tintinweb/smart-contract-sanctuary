...
// File: contracts/GetUniswapDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0xBf5140A22578168FD562DCcF235E5D43A02ce9B1);
...
// File: contracts/GetUniswap.sol

//

pragma solidity ^0.7.6;

contract GetUniswap is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GetUniswapDividendTracker public dividendTracker;

    address private liquidityWallet;

    uint256 public maxBuyTranscationAmount = 100000000 * (10**18);
    uint256 public maxSellTransactionAmount = 25000000 * (10**18);
    uint256 public swapTokensAtAmount = 5000000 * (10**18);
    uint256 public _maxWalletToken = 30000000 * (10**18); // 1.5% of total supply
    ...
    constructor() public ERC20("GetUniswap", "GUNI") {
    	...
    	dividendTracker = new GetUniswapDividendTracker();

    	liquidityWallet = 0xFc96DEe6774f677aB7ac63536600366Dc26E3EFA;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GetUniswap: The dividend tracker already has that address");

        GetUniswapDividendTracker newDividendTracker = GetUniswapDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GetUniswap: The new dividend tracker must be owned by the GetUniswap token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GetUniswap: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GetUniswap: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GetUniswap: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GetUniswap: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "GetUniswap: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GetUniswap: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GetUniswap: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "GetUniswap: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of GetUniswap
            path,
            recipient,
            block.timestamp
        );
...
contract GetUniswapDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("GetUniswap_Dividend_Tracker", "GetUniswap_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "GetUniswap_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "GetUniswap_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GetUniswap contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GetUniswap_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GetUniswap_Dividend_Tracker: Cannot update claimWait to same value");
