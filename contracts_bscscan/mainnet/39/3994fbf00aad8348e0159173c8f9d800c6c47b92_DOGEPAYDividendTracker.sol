...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also DOGEPAYed.
...
// File: contracts/DOGEPAYDividendPayingToken.sol
...
// File: contracts/DOGEPAY.sol

pragma solidity ^0.7.6;

contract DOGEPAY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DOGEPAYDividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTranscationAmount = 150000000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 150000000000000 * (10**18);
    uint256 public swapTokensAtAmount = 10000000 * (10**18);
    uint256 public _maxWalletToken = 150000000000000 * (10**18); 
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "DOGEPAY: The dividend tracker already has that address");

        DOGEPAYDividendTracker newDividendTracker = DOGEPAYDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DOGEPAY: The new dividend tracker must be owned by the DOGEPAY token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DOGEPAY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "DOGEPAY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "DOGEPAY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DOGEPAY: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DOGEPAY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DOGEPAY: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DOGEPAY: This account cannot send tokens until trading is enabled");
        }
...
contract DOGEPAYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DOGEPAY_Dividend_Tracker", "DOGEPAY_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 888888888+ tokens
    } 
      //Smart-contract developed by snipe.finance: [email protected] - t.me/SnipeFinance
    function updateMinimumBalanceForDividends(uint256 newAmountNoDecimials) external onlyOwner{
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DOGEPAY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DOGEPAY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DOGEPAY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DOGEPAY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DOGEPAY_Dividend_Tracker: Cannot update claimWait to same value");
