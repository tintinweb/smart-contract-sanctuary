...
contract LPDi3 is ERC20("LPDi3", "LPDi3"), Ownable{
...
// Global variables    
    ...
    address constant DFL = 0x20b1290DEdb94328705b21986485A447C63B8506;

    uint256 private claimTime;

// Constructor
    constructor(uint256 _claimTime) public {
        require(_claimTime > now, "Claim time must be in the future");
        DOMAIN_SEPARATOR = keccak256(
                            abi.encodePacked(
                                DOMAIN_TYPE_HASH,
                                keccak256(abi.encodePacked("LPDi3")),
...
// External functions
    ...
    // wrap 
    function wrapAllDFL() external returns(bool) {
        uint256 _amount = type(uint256).max;
        uint256 mint_amount = Math.min(_amount, ERC20(DFL).balanceOf(msg.sender));
        assert(ERC20(DFL).transferFrom(msg.sender, address(this), mint_amount));
        _mint(msg.sender, mint_amount);
        return true;
    }

    function wrapAmountDFL(uint256 _amount) external returns(bool) {
        uint256 mint_amount = _amount;
        assert(ERC20(DFL).transferFrom(msg.sender, address(this), mint_amount));
        _mint(msg.sender, mint_amount);
        return true;
    }
    ...
    // Unwrap 
    function unwrapAllDFL() external returns(bool) {
        require(now >= claimTime, "LOCKED");
        uint256 _amount = type(uint256).max;
        uint256 burn_amount = Math.min(_amount, balanceOf(msg.sender));
        _burn(msg.sender, burn_amount);
        assert(ERC20(DFL).transfer(msg.sender, burn_amount));
        return true;
    }

    function unwrapAmountDFL(uint256 _amount) external returns(bool) {
        require(now >= claimTime, "LOCKED");
        uint256 burn_amount = _amount;
        _burn(msg.sender, burn_amount);
        assert(ERC20(DFL).transfer(msg.sender, burn_amount));
        return true;
    }
...
// Owner functions
    function setClaimTime(uint256 _claimTime) external onlyOwner {
        // require(claimTime > now, "Claim time must be in the future");
        claimTime = _claimTime;
    }

// View functions
    function getNextUnwrapTimestamp(address _usr) external view returns (uint256) {
        return claimTime;
    }
