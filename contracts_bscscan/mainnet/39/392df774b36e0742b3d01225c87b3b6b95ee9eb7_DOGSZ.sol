...
import "./SafeMath.sol";
import "./DOGSZDividendTracker.sol";
...
contract DOGSZ is ERC20, Ownable {
    ...
    DOGSZDividendTracker public dividendTracker;
    ...
    uint256 public maxTxAmount = 25000000000000 * (10**18); //
    ...
    constructor() public ERC20("Dogespyy", "DOGSZ") {
        ...
        dividendTracker = new DOGSZDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DOGSZ: The dividend tracker already has that address");

        DOGSZDividendTracker newDividendTracker = DOGSZDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DOGSZ: The new dividend tracker must be owned by the DOGSZ token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DOGSZ: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DOGSZ: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DOGSZ: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DOGSZ: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DOGSZ: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "DOGSZ: Cannot update gasForProcessing to same value");
    ...
    function updatePayoutToken(address token) public {
        require(balanceOf(msg.sender) > 0, "You must own more than zero $ShibaForever tokens to switch your payout token!");
