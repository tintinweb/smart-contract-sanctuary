...
contract OmaxRewardSystem {
    ...
    address marketingWallet = 0x7962fd13E436eac95cc62e4e61d47E27CCac4Ef3;
    
    uint256 totalOmaxDeposited;
    uint256 totalOmaxWithdrawn;
    uint256 totalOmaxWithdrawnIgnoringEarlyWithdrawalFee; //this is needed so that we can calculate the amount of reflection tokens this contract owns
    ...
    function setLockFeePenalisation (uint _lockFeePenalisation) external {
        require(msg.sender == contractOwner, "Only the contract owner can set this.");
    ...
    function getEarlyWithdrawalFeeBalance() external view returns(uint256) {
        return earlyWithdrawalFeeBalance;
    }
    
    function getReflectionBalance() external view returns(uint256) {
        return (_token.balanceOf(address(this)) - (totalOmaxDeposited - totalOmaxWithdrawn));
    }
    
    function withdrawEarlyWithdrawalFeeBalance(address walletAddress) external returns(bool) {
        ...
        emit WithdrawnEarlyBalance(earlyWithdrawalFeeBalance);
        earlyWithdrawalFeeBalance = 0;
        
        return true;
    }
    
    function withdrawReflectionBalance(address walletAddress) external returns(bool) {
        
        require(msg.sender == contractOwner, "Only the contract owner use this.");
        
        _token.transfer(
                 walletAddress, 
                 (_token.balanceOf(address(this)) - (totalOmaxDeposited - totalOmaxWithdrawn))
        );
    ...
    function depositOmax(uint256 amountToLockInWei, uint256 timeLockSeconds) external {
        ...
        totalOmaxDeposited += amountToLockInWei;
    ...
    function withdrawDeposit(uint256 depositId) external {
        ...
        "You can't yet unlock this deposit. If you want to forcefully withdrawn the OMAX from the staking and lose 25% of them, please use forceWithdrawDeposit instead"
        );
        ...
        _token.transferFrom(
                 marketingWallet,
                 msg.sender, 
                 omaxDeposits[depositId].depositValue + (omaxDeposits[depositId].depositValue.mul(percentageRewardForTimelock[omaxDeposits[depositId].timeLockInSeconds]).div(100))
        );
        
        totalOmaxWithdrawnIgnoringEarlyWithdrawalFee += omaxDeposits[depositId].depositValue;
