...
contract YSpace is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    YSpaceDividendTracker public dividendTracker;
    ...
    constructor() ERC20("YSpace", "YSP") {
        marketingWallet = payable(0x4c5de50c2863fc96A481B3CbCa8D24EE459378FB);
        ...
        dividendTracker = new YSpaceDividendTracker(
            payable(this),
            router,
...
contract YSpaceDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        YSpaceContract = YSpace(mainContract);
    ...
    function _transfer(
        ...
        require(false, "YSpace_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "YSpace_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YSpace contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(YSpaceContract);

            uint256 prevBalance = YSpaceContract.balanceOf(address(this));
            ...
            {
                uint256 received = YSpaceContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    YSpaceContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
