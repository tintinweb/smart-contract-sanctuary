...
// GreedySmurf Token
contract GreedySmurf is BEP20{
    
    // Burn address
    address public constant burnAddress = 0x000000000000000000000000000000000000dEaD;
    // DEV address
    address public constant devAddress = 0xdc993bEABeCf9D5a31cEA84Aa8322F7Cc93877F1;
    // Fee address
    address public constant feeAddress = 0x7B0C60bbf5e35fC82098C0A2ABBBa0A83EC51C4c;
    
    // Max transfer rate: 2.5%.
    uint16 public constant maxTransferRate = 250;    
    // Max transfer rate: 0.5%.
    uint16 public constant minTransferRate = 50;

    // Anti Whale! Max transfer amount rate in basis points. 2.5%!
    // SHould be updated in small increments AFTER farming start.
    uint16 public maxTransferAmountRate = 250;
    
    // Transfer Burn Rate: 2%.
    uint16 public constant BURN_RATE = 20;    
    // Transfer FEE Rate: 2%.
    uint16 public constant FEE_RATE = 20;
    ...
    /// @notice Constructs the GreedySmurf contract.    
    constructor() public BEP20('GreedySmurf', 'GREEDY') {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        if (recipient == burnAddress || sender == devAddress || recipient == feeAddress ) {
            ...
            // 2% of every transfer ist sent to feeAddress
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= maxTransferRate, "updateMaxTransferAmountRate: Max transfer amount rate can not exceed the max rate.");
        require(_maxTransferAmountRate >= minTransferRate, "updateMaxTransferAmountRate: Max transfer amount rate can not be bleow min rate.");
