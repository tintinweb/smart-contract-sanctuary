...
contract ElonEarnBTC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ElonEarnBTCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ElonEarnBTC", "ELEARN") {
        uint256 _dividendRewardsFee = 7;
        uint256 _devFee = 10;

        dividendRewardsFee = _dividendRewardsFee;
        devFee = _devFee;
        totalFees = _dividendRewardsFee.add(_devFee);

        dividendTracker = new ElonEarnBTCDividendTracker();

        devWallet = 0x00bc2aa1281BBBd3B9Af8A39b69eB39d0F6AE501;
        ...
        onlyOwner
    {
        require(
            ...
            "ElonEarnBTC: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(
            ...
            "ElonEarnBTC: Marketing divisor must be between 0 (0%) and 100 (100%)"
        );
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(
            !swapping,
            "ElonEarnBTC: A swapping process is currently running, wait till that is complete"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "ElonEarnBTC: The dividend tracker already has that address"
        );

        ElonEarnBTCDividendTracker newDividendTracker = ElonEarnBTCDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "ElonEarnBTC: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "ElonEarnBTC: Dividend reward tax must be between 0 and 10"
        );
        dividendRewardsFee = newFee;
    }

    function updatedevFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "ElonEarnBTC: Dividend reward tax must be between 0 and 10"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "ElonEarnBTC: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "ElonEarnBTC: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "ElonEarnBTC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "ElonEarnBTC: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updatedevWallet(address newdevWallet) public onlyOwner {
        require(
            ...
            "ElonEarnBTC: The liquidity wallet is already this address"
        );
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "ElonEarnBTC: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "ElonEarnBTC: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = contractTokenBalance.mul(devFee).div(
                    totalFees
                );
    ...
    function transferTodevWallet(address payable recipient, uint256 amount)
        private
    {
...
contract ElonEarnBTCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "ElonEarnBTC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ElonEarnBTC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ElonEarnBTC contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "ElonEarnBTC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "ElonEarnBTC_Dividend_Tracker: Cannot update claimWait to same value"
        );
