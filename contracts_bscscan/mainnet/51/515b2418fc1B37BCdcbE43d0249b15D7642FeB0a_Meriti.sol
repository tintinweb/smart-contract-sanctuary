...
contract Meriti is Context, IBEP20 {
    ...
    modifier onlySender() {
        require(presaleAddress() == _msgSender(), "Caller is not presaleAccount");
        _;
    }
    ...
    event PresaleAddressUpdated(address oldAdd, address newAdd);
    ...
    constructor(address presaleAdd, address marketing, address futureProjects, address charity) {
        ...
        emit ExcludedFromFee(presaleAddress());
    ...
    function presaleTokenTransfer(address recipient, uint256 amount) public onlySender returns (bool) {
        ...
        _tokenTransfer(presaleAddress(), recipient, amount, false);
    ...
    function updatePresaleAddress(address newAdd) external onlyOwner returns(bool) {
        require(newAdd != address(0), "PreSale Address can not be null");
        includedInFee(_presaleAddress);
        _presaleAddress = newAdd;
