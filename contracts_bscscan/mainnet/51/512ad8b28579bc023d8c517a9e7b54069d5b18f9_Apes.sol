    ...
    UUPSUpgradeable
{
    ...
    function mintWithFee(string memory _name, string memory _tokenURI) public {
        ...
        uint256 allowance = aGold.allowance(senderAddr, address(this));
        require(senderBalance >= mintPrice, "not enough balance");
        require(allowance >= mintPrice, "not enough allowance");

        aGold.transferFrom(senderAddr, wAGoldAddr, (mintPrice * aGold.totalFees() / 100));
        mintPrice = mintPrice - (mintPrice * aGold.totalFees() / 100);
    ...
    function buy(uint256 _tokenId) public {
        ...
        aGold.transferFrom(buyerAddr, wAGoldAddr, (price * aGold.totalFees() / 100));
        aGold.transferFrom(buyerAddr, initialOwner, (price - (price * aGold.totalFees() / 100)));
    }
    ...
    function changeApeName(uint256 tokenId, string memory newName) public onlyOwner {
