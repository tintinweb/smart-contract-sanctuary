...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also WhenLamboed.
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function distributeCAKEDividends(uint256 amount) public {
    ...
    if (amount > 0) {
      magnifiedDividendPerShare = magnifiedDividendPerShare.add(
        (amount).mul(magnitude) / MaxSupply()
      );
...
contract WhenLambo is ERC20, Ownable {
    ...
    uint256 private  tradingEnabledTimestamp = 1628096370;
    ...
    function unBlacklistSniper(address addresses) external onlyOwner(){
         if(_isblacklisted[addresses] == false) return;
        _isblacklisted[addresses] = false;
    }
    
    function unBlacklistMultipleSniper(address[] calldata addresses) public onlyOwner(){
