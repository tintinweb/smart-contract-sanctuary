...
contract DodgeyDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DodgeyDogeDividendTracker public dividendTracker;
    ...
    uint256 private firstFewBlocksF = 30;
    ...
    uint256 public maxSellTransactionAmount = 750000000 * (10**9);
    ...
    constructor(address router_, address initial, address payable mk) ERC20("DodgeyDOGE", "DodgeyDOGE") {
        
        totalFees = BNBRewardsFee.add(liquidityFee);


    	dividendTracker = new DodgeyDogeDividendTracker();
        ...
        dividendTracker.excludeFromDividends(initial);
        ...
        excludeFromFees(initial, true);

        // enable owner and fixed-sale wallet to send tokens before presales are over
        canTransferBeforeTradingIsEnabled[initial] = true;

        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(initial, 1000000000000 * (10**9));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DodgeyDOGE: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DodgeyDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DodgeyDOGE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1000000, "DodgeyDOGE: gasForProcessing must be between 200,000 and 1,000,000");
        require(newValue != gasForProcessing, "DodgeyDOGE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DodgeyDOGE: This account cannot send tokens until trading is enabled");
        }
...
contract DodgeyDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DodgeyDoge_Dividend_Tracker", "DodgeyDoge_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 10000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "DodgeyDoge_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public override {
        require(false, "DodgeyDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DodgeyDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 86400, "DodgeyDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DodgeyDoge_Dividend_Tracker: Cannot update claimWait to same value");
