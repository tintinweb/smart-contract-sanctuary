...
contract Testland is ERC20, Ownable {
	using SafeMath for uint256;

	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private _name = "Testland";
	string private _symbol = "TEST";
	uint8 private _decimals = 9;

	TestlandDividendTracker public dividendTracker;
	...
	constructor() public ERC20("Testland", "TEST") {
		liquidityWallet = owner();
		marketingWallet = owner();

		dividendTracker = new TestlandDividendTracker();
	...
	function cancelLaunch() public onlyOwner {
		require(this.isInLaunch(), "Testland: Launch is not set");
	...
	function setSugarRushHour() public onlyOwner {
		require(!this.isInSugarRushHour(), "Testland: SugarRush Hour is already set");
		require(isTradingEnabled, "Testland: Trading must be enabled first");
		require(!this.isInLaunch(), "Testland: Must not be in launch period");
		emit SugarRushHourChange(true, false);
		_sugarRushHourStartTimestamp = _getNow();
	}
	function cancelSugarRushHour() public onlyOwner {
		require(this.isInSugarRushHour(), "Testland: SugarRush Hour is not set");
		emit SugarRushHourChange(false, true);
		_sugarRushHourStartTimestamp = 0;
	}
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "Testland: The dividend tracker already has that address");
		TestlandDividendTracker newDividendTracker = TestlandDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "Testland: The new dividend tracker must be owned by the Testland token contract");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "Testland: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "Testland: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "Testland: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "Testland: Account is already the value of 'excluded'");
		_isExcludedFromMaxWalletLimit[account] = excluded;
		emit ExcludeFromMaxWalletChange(account, excluded);
	}
	function blockAccount(address account) public onlyOwner {
		uint256 currentTimestamp = _getNow();
		require(!_isBlocked[account], "Testland: Account is already blocked");
		if (_isLanched) {
			require(currentTimestamp.sub(_launchStartTimestamp) < _blockedTimeLimit, "Testland: Time to block accounts has expired");
		}
		_isBlocked[account] = true;
		emit BlockedAccountChange(account, true);
	}
	function unblockAccount(address account) public onlyOwner {
		require(_isBlocked[account], "Testland: Account is not blcoked");
		_isBlocked[account] = false;
		emit BlockedAccountChange(account, false);
	}
	function setWallets(address newLiquidityWallet, address newMarketingWallet) public onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "Testland: The liquidityWallet cannot be 0");
			emit LiquidityWalletChange(newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "Testland: The marketingWallet cannot be 0");
	...
	function setUniswapRouter(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "Testland: The router already has that address");
		emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
		uniswapV2Router = IUniswapV2Router02(newAddress);
	}
	function setGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "Testland: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "Testland: Cannot update gasForProcessing to same value");
	...
	function setMaxTransactionAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxTxAmount, "Testland: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxWalletAmount, "Testland: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) public onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "Testland: Cannot update minimumTokensBeforeSwap to same value");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "Testland: Cannot send more than contract balance");
	...
	function _transfer(
		...
		if(from != owner() && to != owner()) {
			require(isTradingEnabled, "Testland: Trading is currently disabled.");
			require(!_isBlocked[to], "Testland: Account is blocked");
			require(!_isBlocked[from], "Testland: Account is blocked");
			if (_isInLaunch && currentTimestamp.sub(_launchStartTimestamp) <= 300 && isBuyFromLp) {
				require(currentTimestamp.sub(_buyTimesInLaunch[to]) > 60, "Testland: Cannot buy more than once per min in first 5min of launch");
			}
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "Testland: Buy amount exceeds the maxTxBuyAmount.");
			}
			if (!_isExcludedFromMaxWalletLimit[to]) {
				require(balanceOf(to).add(amount) <= maxWalletAmount, "Testland: Expected wallet amount exceeds the maxWalletAmount.");
			}
		}
...
contract TestlandDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("Testland_Dividend_Tracker", "Testland_Dividend_Tracker") {
		claimWait = 3600;
	...
	function _transfer(address, address, uint256) internal override {
		require(false, "Testland_Dividend_Tracker: No transfers allowed");
	}
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "Testland_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "Testland_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "Testland_Dividend_Tracker: Cannot update claimWait to same value");
