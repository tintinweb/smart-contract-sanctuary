...
contract RubyToken is ERC20("Ruby Geology Token", "RUBY"), Ownable {
    /// @dev Creates `_amount` token to `_to`. Must only be called by the owner (RubyMaster).
    ...
    receive() external payable {
        ...
        require(msg.value >= 3*1e16, "Minimum of value is 0.03BNB");
        _mint(msg.sender, 375*1e15);
...
// File: contracts\RubyMaster.sol

pragma solidity 0.6.12;

// RubyMaster is the hub that accumulates and distributes RUBY
//
...
contract RubyMaster is Ownable {
    ...
    function changeRubyPerBlock(uint256 _newRubyPerBlock) public onlyOwner {
        rubyPerBlock = _newRubyPerBlock;
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accRubyPerShare: 0}));
    }
    ...
    // Withdraw LP tokens from RubyMaster.
    ...
    function safeRubyTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > rubyBalance) {
            ruby.transfer(_to, rubyBalance);
        } else {
