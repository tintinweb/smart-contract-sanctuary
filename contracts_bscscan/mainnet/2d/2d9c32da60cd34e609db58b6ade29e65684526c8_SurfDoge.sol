...
contract SurfDoge is BEP20, Ownable {
	...
	address public immutable pancakePairBUSD;
    ...
    constructor() BEP20("SurfTest5", "Test5") {
        ...
        address _pancakePair = IPancakeFactory(_pancakeRouter.factory())
		 ...
		 address _pancakePairBUSD = IPancakeFactory(_pancakeRouter.factory())
            .createPair(address(this), addressBUSD);

        pancakeRouter = _pancakeRouter;
        pancakePair = _pancakePair;
		pancakePairBUSD = _pancakePairBUSD;

        _setAutomatedMarketMakerPair(_pancakePair, true);
        _setAutomatedMarketMakerPair(_pancakePairBUSD, true);
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "Surfdoge: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakePair, "Surfdoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
   ...
   function getUnlockTimeHours() public view returns (uint256) {
    ...
    function updateMaxSellTransactionAmount(uint256 _maxSellTransactionAmount) public onlyOwner {
        require(_maxSellTransactionAmount >= 50000 * (10**18), "Max sell has to be higher than 0.05 % of the supply");
...
contract SurfDogeDividendTracker is DividendPayingToken, Ownable {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SurfDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SurfDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AkuAku contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SurfDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SurfDoge_Dividend_Tracker: Cannot update claimWait to same value");
