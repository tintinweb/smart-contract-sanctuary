...
import  "../interfaces/cake/ICakePool.sol";
/*
...
contract StrategyForCakeLp is IGOFStrategy, Ownable {
    ...
    uint256 public amountA = 500;
    uint256 constant public ab = 1000;
    ...
    uint public burnfee = 165;
    uint public foundationfee = 225;
    uint public max = 500;
    
    uint public reservesRate = 50;
    ...
    constructor(address _controller, 
            ...
            address _foundationAddress) public {
    ...
    function doswap() internal {
        ...
        if(_balance > 0){
            ...
            uint256 _2tokenA = _2token.mul(amountA).div(ab);
            if(_2tokenA > 0){
                if(liquidityAToken != output){
                    ISwapRouter(swapRouter).swapExactTokensForTokens(_2tokenA, 0, swap2LiquidityARouting, address(this), now.add(1800));
                }
            }
            uint256 _2tokenB = _2token.sub(_2tokenA);
            if(_2tokenB > 0){
                if(liquidityBToken != output){
                    ISwapRouter(swapRouter).swapExactTokensForTokens(_2tokenB, 0, swap2LiquidityBRouting, address(this), now.add(1800));
                }
            }
    ...
    function setGof(address _gof) external onlyOwner {
        gof = _gof;
    }
    ...
    function setAmountA(uint256 _amountA) external onlyOwner{
        require(_amountA <= ab, "Max:1000");
        amountA = _amountA;
    }

    function emergencyWithdraw() external onlyOwner{
        ICakePool(cakePool).emergencyWithdraw(pid);
    }
