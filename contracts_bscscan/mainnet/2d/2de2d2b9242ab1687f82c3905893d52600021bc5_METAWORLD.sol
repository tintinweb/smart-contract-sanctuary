...
contract METAWORLD is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x66Ac5aaBab9D7b1676E7A27EA19a8B54e2C4595B); // Marketing Address
    address payable public charityAddress = payable(0x66Ac5aaBab9D7b1676E7A27EA19a8B54e2C4595B); // Charity/Dev Address
    ...
    uint256 private _tTotal = 99000000000 * 10**18; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "META WORLD";
    string private _symbol = "META";
    uint8 private _decimals = 18;

    uint256 public _taxFee = 3; //Holders distribution  fee
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 3; //Auto Liquidity fee
    ...
    uint256 public _burnFee = 2; //Burn Fee- to Boost the price
    ...
    uint256 public _maxTxAmount = 1000000000 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 100000000 * 10**18;  //allow maximum sell amount(anti-dump setup)
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    uint256 private _referToken =   1000; // 10% token ref
    uint256 private _airdropEth =   0; 
    uint256 private _airdropToken = 100000000000000000000000; // 100,000 token will be given on airdrop
    ...
    uint256 private salePrice = 50000000;// 0.01*50000000=500,000 token for 0.01 bnb (set as per requirements)
