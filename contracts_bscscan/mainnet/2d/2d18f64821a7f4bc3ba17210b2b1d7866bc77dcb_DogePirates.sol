...
/**
 Come join the telegram at: @DogePiratesBSC
 
 Earn DOGE just for holding DOGEPIRATES!
 Auto buyback!
 100% safu!
 Lets get to LOOTING doge together🏴‍☠️
 Website will be created at 100K market cap
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43;
...
contract DogePirates is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DogePiratesDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 3000000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 3000000000000 * (10**18);
    uint256 public swapTokensAtAmount = 20000000000 * (10**18);
    uint256 public maxWalletToken = 12000000000000 * (10**18); // 5% of total supply

    uint256 public dividendRewardsFee;
    uint256 public marketingFee;
    uint256 public immutable totalFees;

    // sells have fees of 8 and 2 (5 * 1.2 and 3 * 1.2)
    ...
    constructor() ERC20("DogePirates", "DogePirates") {
        uint256 _dividendRewardsFee = 5;
    	...
    	dividendTracker = new DogePiratesDividendTracker();

    	buyBackWallet = 0x725417a51da712Cf0038afD8469c35F18cFd3F2b;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "DogePirates: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "DogePirates: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	    sellFeeIncreaseFactor = divisor;
  	}
  	
  	function prepareForPreSale() external onlyOwner {
        setTradingIsEnabled(false);
        dividendRewardsFee = 5;
        marketingFee = 3;
    ...
    function afterPreSale() external onlyOwner {
        dividendRewardsFee = 5;
        marketingFee = 3;
        maxBuyTranscationAmount = 15000000000 * (10**18);
        maxWalletToken = 15000000000 * (10**18);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "DogePirates: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DogePirates: The dividend tracker already has that address");

        DogePiratesDividendTracker newDividendTracker = DogePiratesDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DogePirates: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DogePirates: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DogePirates: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DogePirates: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DogePirates: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DogePirates: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "DogePirates: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DogePirates: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DogePirates: Cannot update gasForProcessing to same value");
...
contract DogePiratesDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DogePirates_Dividend_Tracker", "DogePirates_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DogePirates_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DogePirates_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogePirates contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "DogePirates_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DogePirates_Dividend_Tracker: Cannot update claimWait to same value");
