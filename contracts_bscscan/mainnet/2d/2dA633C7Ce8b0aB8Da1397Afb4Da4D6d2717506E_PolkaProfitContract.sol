...
interface IERC721 {
    ...
    function isLeased(uint256 _assetId) external view returns (bool);
    function leasedAssets(uint256 _assetId) external view returns (address _lesee, uint256 _endTime);
}
...
contract PolkaProfitContract is Ownable {
    ...
    address public nftAddress = 0x364151EDBAC312C7a636CfA7996C3A2B6C2eC590;
    address public tokenAddress = 0x6Ae9701B9c423F40d54556C9a443409D79cE170a;
    address public walletAddress;
    uint256 public bankEarnings;
    uint256 public galleryEarnings;
    uint256 public galleryBNBEarnings;
    ...
    constructor() {
        ...
        fillPayments(47,  200000000000000000000, 0, 0);
        fillPayments(50,  250000000000000000000, 0, 0);

        walletAddress = 0xeA50CE6EBb1a5E4A8F90Bfb35A2fb3c3F0C673ec;
        marketContract = 0xE105D42EB25EB4359E6a1D59A1761ff1CCf20309;
    ...
    function profitsPayment(uint256 _assetId) public returns (bool success) {
        ...
        address assetOwner;
        bool onLease = nft.isLeased(_assetId);
        if (onLease == true) {
            (assetOwner,) = nft.leasedAssets(_assetId);
        } else {
            assetOwner = nft.ownerOf(_assetId);
        }
        ...
        Claim memory thisclaim = Claim(assetOwner, _assetId, totalPayment, block.timestamp);
        payments.push(thisclaim);
        emit Payment(assetOwner, totalPayment);
        return true;
    }
    ...
    function calcProfit(uint256 _assetId) public view returns (uint256 _profit, uint256 _lastPayment) {
        ...
        if ((cTime) < wUnit) { 
            return (0, lastTransfer);
             ...
             uint256 weekCount;  
            if (dTime == 0) {
                uint256 btxw = (lastTransfer - lastPayment)/(wUnit);
                weekCount = ((cTime)/(wUnit)) + btxw;
            } else {
    ...
    function addGalleryEarnings(uint256 _amount) public {
        require(msg.sender == marketContract, "Not Allowed");
        galleryEarnings += _amount;
    }
    
    function addGalleryBNBEarnings(uint256 _amount) public {
        require(msg.sender == marketContract, "Not Allowed");
        galleryBNBEarnings += _amount;
    }
    
    function migrateBankEarnings(uint256 _amount) public onlyOwner {
        bankEarnings = _amount;
    }
    
    function migrateGalleryEarnings(uint256 _amount) public onlyOwner {
        galleryEarnings = _amount;
    }
