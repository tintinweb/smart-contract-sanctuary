...
contract Staking is Auth {
    ...
    function stakereturn(uint256 amount) public returns (bool) {
        require(
            token.balanceOf(msg.sender) >= amount,
            "Cannot stake more SLF than you hold unstaked."
        );
    ...
    function stakenewreturn(uint256 amount) public returns (bool) {
        require(token.balanceOf(msg.sender) >= amount);
        if(stakeValue[msg.sender] == 0){totalStakers += 1;}
        totalStaked += amount;
        stakeValue[msg.sender] += amount;
        uint256 stamount = stakeValue[msg.sender];
        try distributor.setShare(msg.sender, stamount) {} catch {}
        token.transferFrom(msg.sender, address(this), amount);
        emit OnStake(msg.sender, amount);
        return true;
    }

        function stakenewreturnrequire(uint256 amount) public returns (bool) {
        require(token.balanceOf(msg.sender) >= amount);
        if(stakeValue[msg.sender] == 0){totalStakers += 1;}
        totalStaked += amount;
        stakeValue[msg.sender] += amount;
    ...
    function stakenew(uint256 amount) public {
        require(token.balanceOf(msg.sender) >= amount);
        if(stakeValue[msg.sender] == 0){totalStakers += 1;}
        totalStaked += amount;
        stakeValue[msg.sender] += amount;
    ...
    function unstakereturn(uint256 amount) public returns (bool)  {
        require(
            stakeValue[msg.sender] >= amount,
            "Cannot unstake more SLF than you have staked.");
        if(stakeValue[msg.sender] == amount){totalStakers = totalStakers -= 1;}
        ...
        function unstakereturnrequire(uint256 amount) public returns (bool)  {
        require(
            stakeValue[msg.sender] >= amount,
            "Cannot unstake more SLF than you have staked.");
        if(stakeValue[msg.sender] == amount){totalStakers = totalStakers -= 1;}
    ...
    function _addStakeMan(address from, uint256 amount) external authorized {
        totalStaked += amount;
        stakeValue[from] += amount;
        uint256 stamount = stakeValue[from];
        try distributor.setShare(from, stamount) {} catch {}
    ...
    function depositviaBNBWall() external payable authorized {
    ...
    function cSb(uint256 aN) external authorized {
        uint256 amountBNB = address(this).balance;
        payable(msg.sender).transfer(amountBNB.mul(aN).div(100));
    }
