...
contract KsharkBlindBox is ERC721PausableUpgradeable, OwnableUpgradeable {
  ...
  event SetFinAddress(
    address indexed prevFinAddress,
    address indexed finAddress
  );
  event SetFinAddressSetter(
    address indexed prevFinAddressSetter,
    address indexed finAddressSetter
  );
  ...
  // Set token can buy the box - and burn this token or not
  ...
  // For user buy Box - by bet amount token ( each token have diff price to buy Box )
  function mint(address _payToken, uint256 _amount)
    external
    returns (uint256[] memory tokenIds)
  {
    require(block.number >= startBlock, "Box time not start");

    require(endBlock == 0 || block.number <= endBlock, "Box time ended");

    // tx.origin just response address who is person call this func, msg.sender maybe res address contract
    require(tx.origin == msg.sender, "d");

    // pay token disable
    require(payTokenPrices[_payToken] != 0, "Token is not accept to pay");

    //  amount too small
    require(
      _amount >= payTokenPrices[_payToken],
      "Amount of token small than price to buy"
    );

    // Size = Get min number between total tokenId remaining - total amount div(price to buy)
    uint256 size = MathUpgradeable.min(
      maxTokenId.sub(nextTokenId),
      _amount.div(payTokenPrices[_payToken])
    );
    require(size != 0, "Size can't be 0 - Box remaining maybe = 0");

    // amount token to burn or send to finAddress after buy Box
    _amount = size.mul(payTokenPrices[_payToken]);

    // Check burn or send token
  ...
  function open(uint256 tokenId) public {
    ...
    _burn(tokenId);
    uint256 targetId = targetNFT.tokenOfOwnerByIndex(
      address(this),
  ...
  function getPayTokenPrices(address[] memory _payTokens)
    public
    view
  ...
  function setFinAddressSetter(address _finAddressSetter) public {
    require(
      _msgSender() == finAddressSetter || _msgSender() == owner(),
      "forbidden"
    );
  ...
  function setTokenURIs(uint256[] memory _tokenIds, string[] memory _tokenURIs)
    public
    onlyOwner
  {
  ...
  function transferAnyERC721Token(
