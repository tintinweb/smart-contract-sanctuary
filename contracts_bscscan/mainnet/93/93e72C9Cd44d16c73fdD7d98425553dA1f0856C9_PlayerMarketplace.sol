...
contract PlayerMarketplace is Ownable {
    ...
    mapping(address => mapping(uint256 => uint256[])) public previousBidHistory;
    ...
    constructor(address fee, uint256 _listingfeePercentage, uint256 _biddingFeePercentage, uint256 _listingDailyFeePercentage, uint256 _auctionCreatorBiddingFeePercentage, uint256 _buyOutFeePercentage) public {
    ...
    function createAuction(address paymentTokenAddress, uint256 _price, address nftAddress, uint256 _durationBlocks, uint256 tokenID, bool _buyOutOnly, uint256 _minimumBidAmount) external {
        ...
        uint256 _endblock =  block.timestamp + _durationBlocks;
        massUpdateAuctions();
    ...
    function setListingDailyFeePercentage(uint256 _feePercentage) external onlyOwner {
        require(_feePercentage <= 10000, "setListingDailyFeePercentage: Not good.");
    ...
    function removeFromActiveAuctions(uint256 index) internal {
        ...
        activeAuctions[index] = activeAuctions[auctions.length - 1];
        activeAuctions.pop();
    }
    ...
    function massUpdateAuctions() public {
        for(uint256 i = 0; i < activeAuctions.length; i++)
        {
            if(auctions[activeAuctions[i]].endBlock <= block.timestamp)
            {
                auctions[activeAuctions[i]].status = "Expired";
                removeFromActiveAuctions(i);
            }
        }
    }
