...
abstract contract Ownable is Context {
     ...
     */
    constructor () {
        _owner = 0x4253C514B535D04fB5B9cd14fBEb2516c99FC0d4;
...
contract INDOBISA is Context, IERC20, Ownable {
    ...
    // To change
    address private _charityWalletAddress = 0x61BDeeeBE124Eae429E33c619bB1Bafa189b75af; 
    address payable public marketingAddress = payable(0xcbC33229c5F4A4ebBe738E380E727ae84A827e63); 
    ...
    string private _name = "INDOBISA";
    string private _symbol = "INDOBISA";
    ...
    bool private swapAndLiquifyBNB = true;
    bool private limitTheWhales = false;
    uint256 private totalLiq = 100; 
    mapping (address => uint256) public previousSale;
    
    uint256 public _maxTxAmount = 1 * 10**15 * 10**9;
    uint256 public _maxBuyAmount = 1 * 10**14 * 10**9;
    uint256 public _maxSellAmount = 1 * 10**13 * 10**9;
    uint256 public _maxWhaleAmount = 1 * 10**13 * 10**9; // 1% of stock
    
    uint256 public numTokensSellToAddToLiquidity001 = 1 * 10**10 * 10**9; // 0.005%
    uint256 public numTokensSellToAddToLiquidity005 = 5 * 10**10 * 10**9; // 0.005%
    uint256 public numTokensSellToAddToLiquidity008 = 8 * 10**10 * 10**9; // 0.008%
    uint256 public numTokensSellToAddToLiquidity02 = 2 * 10**11 * 10**9; // 0.02%
    uint256 public numTokensSellToAddToLiquidity04 = 4 * 10**11 * 10**9; // 0.04%
    uint256 public numTokensSellToAddToLiquidity06 = 6 * 10**11 * 10**9; // 0.06%
    uint256 public numTokensSellToAddToLiquidity08 = 8 * 10**11 * 10**9; // 0.08%
    uint256 public numTokensSellToAddToLiquidity = 1 * 10**12 * 10**9; // 0.1%
    uint256 public numTokensSellToAddToLiquidity3 = 3 * 10**12 * 10**9; // 0.3%
    uint256 public numTokensSellToAddToLiquidity5 = 5 * 10**12 * 10**9; // 0.5%
    uint256 public numTokensSellToAddToLiquidity7 = 7 * 10**12 * 10**9; // 0.7%
    uint256 public numTokensSellToAddToLiquidity9 = 9 * 10**12 * 10**9; // 0.9%
    uint256 public numTokensSellToAddToLiquidity10 = 10 * 10**12 * 10**9; // 1.0%
    uint256 public divfactor = 9; 
    ...
    mapping(address => bool) public _isConsiderAsSnipe;
   	uint256 public _transactionLockTime = 1400 minutes;
    ...
    uint256 public _maxWalletToken = 41 * 10**12 * 10**9;
    ...
    constructor () {
        ...
        _isExcludedFromTransactionlock[_charityWalletAddress] = true;
    ...
    function getTotalLiq() public view returns (uint256) {
        return totalLiq;
    }
    function setTotalLiq(uint256 sTotalLiq) external onlyOwner() {
        totalLiq = sTotalLiq;
    }
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        require(_excluded.length <= 50, "Only 50 accounts may be excluded");
    ...
    function changeCharityWallet(address account) external onlyOwner() {
        _charityWalletAddress = account;
    }
    ...
    function setDivFactor(uint256 setamt) external onlyOwner {
        divfactor = setamt;
    }
    ...
    function _transfer(
            ...
            if (limitTheWhales) {
                ...
                if(to == address(uniswapV2Pair) || to == address(uniswapV2Router)) {
                    ...
                    if (fromBalance > threshold) {
                        ...
                        require(amount < fromBalance / (2), "Max sell is 50% if you hold more than 1% or more of total supply for 30 min.");
                        require( _now - (previousSale[from]) > 30 minutes, "Max sell is 50% if you hold more than 1% or more of total supply for 30 min.");
                    }
                }
            }
        }
        
        if (!_isExcludedFromTransactionlock[from] && limitByDay && (to == uniswapV2Pair) && (from != uniswapV2Pair) && from != owner() && to != owner()) {
            ...
            require(_isExcludedFromTransactionlock[from] || (_transactionCheckpointAmt[from] + amount <= _maxSellAmount)
        ...
        if (
            ...
            amount > (numTokensSellToAddToLiquidity10 * divfactor)  &&
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity9 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity7 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity5 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity3 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity08 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity06 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity04 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity02 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity008 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity005 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
            ...
            from != uniswapV2Pair &&
            swapAndLiquifyEnabled &&
            amount > (numTokensSellToAddToLiquidity001 * divfactor)  &&
            !_isExcludedFromFee[from]
        ) {
    ...
    function updateMarketingAddress(address payable newAddress)
        public
        onlyOwner
    {
