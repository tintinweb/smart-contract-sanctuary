...
`----'                    `----'            `--` '---'            `--`----'    '---'        `---'                      
 ...
 TG: https://t.me/BabyMudra
 
 Hodl $BABYMUDRA and get MUDRA automatically in your wallet every hour or so!
 $MUDRA: 0xd0C1d18f091B182B80f342C654280B9004c0b124
 
 # of Tokens: 100,000,000,000 (100 Billion)
 Max Wallet Size = 4,000,000,000 (4 Billion or 4%)
 Max Transaction Size = 2,000,000,000 (2 Billion or 2%)
 
 Reflection/Taxes: 6% in MUDRA, 3% in Liquidity, 1% to Developer
 
 Buy Slippage: 11-13% (12% should be OK)

 *** THE MAX A WALLET CAN HAVE IS 40,000,000! THAT IS 4%! YOU CAN SELL A MAX OF 4%! ***
 *** No Dev Tokens, No Friend Tokens, Fair Launch ***
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0xd0C1d18f091B182B80f342C654280B9004c0b124);
...
contract BabyMudra is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BabyMudraDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("BabyMudra", "BabyMudra") {
        //Reflection Rewards %
        _ETHRewardsFee = 7;
        //Liqudity Fee %
        _liquidityFee = 3;
        ...
        maxBuyTranscationAmount = 2000000000 * (10**18); //Unlimted
        maxSellTransactionAmount = 4000000000 * (10**18); //Max Sell 1%
        swapTokensAtAmount = 400000000 * (10**18); // 0.4% of the supply in order to get rewards
        _maxWalletToken = 4000000000 * (10**18); // 4% of total supply


    	dividendTracker = new BabyMudraDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
        //Marketing Wallet Address
		marketingWallet = payable(0xe98a78864c30740B64B1eCd6160279E05F10A15A);
    ...
    function updateTokensAmountForRewards(uint256 amountOfTokens) public onlyOwner {
        amountOfTokens = amountOfTokens * (10**18);
        swapTokensAtAmount = amountOfTokens;
    }

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyMudra: The dividend tracker already has that address");

        BabyMudraDividendTracker newDividendTracker = BabyMudraDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyMudra: The new dividend tracker must be owned by the BabyMudra token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyMudra: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyMudra: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyMudra: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyMudra: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyMudra: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyMudra: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyMudra: Cannot update gasForProcessing to same value");
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BabyMudra: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BabyMudra: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "BabyMudra: ETH Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract BabyMudraDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BabyMudra_Dividend_Tracker", "BabyMudra_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "BabyMudra_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BabyMudra_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MicroCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BabyMudra_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyMudra_Dividend_Tracker: Cannot update claimWait to same value");
