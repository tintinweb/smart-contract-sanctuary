...
// File contracts/boosting/CCV.sol

pragma solidity 0.6.12;



contract CCV is ERC20, Ownable {
     uint256 public immutable maxSupply;

    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private _minters;

    constructor(uint256 _maxSupply) public ERC20("CC Voucher", "CCV"){
    ...
    function batchApprove(address[] memory _ccvSpenders, uint256[] memory _ccvAmounts) public returns (bool){
        for (uint i = 0; i < _ccvSpenders.length; i++){
            approve(_ccvSpenders[i], _ccvAmounts[i]);
        }
        return true;
    }
