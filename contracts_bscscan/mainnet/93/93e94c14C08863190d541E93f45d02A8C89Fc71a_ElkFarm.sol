...
// File: contracts/ElkToken.sol

pragma solidity 0.6.12;

// ElkToken with Governance.
contract ElkToken is BEP20 {
    ...
    // Min amount to liquify. (default 10 ELKs)
    uint256 public minAmountToLiquify = 10 ether;
    // The swap router, modifiable. Will be changed to Elk's router when our own AMM release
    IPancakeRouter02 public elkRouter;
    // The trading pair
    address public elkPair;
    // In swap and liquify
    ...
    event ElkRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "ELK::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(swapEnabled == true, "ELK::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the ElkToken contract.
     */
    constructor() public BEP20("ElkDeFi Token", "ELK") {
    ...
    /// @dev overrides transfer function to meet tokenomics of ELK
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(elkRouter) != address(0)
            && elkPair != address(0)
            && sender != elkPair
            && sender != owner()
        ) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "ELK::transfer: Burn value invalid");

            // default 100% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "ELK::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the elk pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = elkRouter.WETH();

        _approve(address(this), address(elkRouter), tokenAmount);

        // make the swap
        elkRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        elkRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from elkRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "ELK::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "ELK::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "ELK::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateElkRouter(address _router) public onlyOperator {
        elkRouter = IPancakeRouter02(_router);
        elkPair = IPancakeV2Factory(elkRouter.factory()).getPair(address(this), elkRouter.WETH());
        require(elkPair != address(0), "ELK::updateElkRouter: Invalid pair address.");
        emit ElkRouterUpdated(msg.sender, address(elkRouter), elkPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "ELK::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "ELK::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "ELK::delegateBySig: invalid nonce");
        require(now <= expiry, "ELK::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "ELK::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ELKs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "ELK::_writeCheckpoint: block number exceeds 32 bits");
...
// ElkFarm is the master of ElkFarm Token (ELK). He can make ELK and he is a fair guy.
//
...
// contract once $ELK is sufficiently distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract ElkFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ELKs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accElkPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accElkPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ELKs to distribute per block.
        uint256 lastRewardBlock; // Last block number that ELKs distribution occurs.
        uint256 accElkPerShare; // Accumulated ELKs per share, times 1e12. See below.
    ...
    // ELK tokens created per block.
    uint256 public elkPerBlock;
    // Deposit Fee address
    ...
    // The block number when ELK mining starts.
    ...
    // Initial emission rate: 1 ELK per block.
    ...
    constructor(
        ...
        elkPerBlock = INITIAL_EMISSION_RATE;
    ...
    // Update the given pool's ELK allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending ELKs on frontend.
    function pendingElk(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accElkPerShare = pool.accElkPerShare;
        uint256 lpSupply = IStrategy(pool.strat).wantLockedTotal();
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 elkReward = multiplier.mul(elkPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accElkPerShare = accElkPerShare.add(
                elkReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accElkPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 elkReward =
            multiplier.mul(elkPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
        elk.mint(devAddr, elkReward.div(10));
        elk.mint(address(this), elkReward);
        pool.accElkPerShare = pool.accElkPerShare.add(
            elkReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to ElkFarm for ELK allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);		
		payOrLockupPendingElk(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(elk)) {
                uint256 transferTax = _amount.mul(elk.transferTaxRate()).div(10000);
        ...
        user.rewardDebt = user.amount.mul(pool.accElkPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }	

    // Deposit LP tokens to ElkFarm for ELK allocation with referral.
    ...
    // Withdraw LP tokens from ElkFarm.
	...
	// Pay or lockup pending ELKs.
    function payOrLockupPendingElk(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 pending = user.amount.mul(pool.accElkPerShare).div(1e12).sub(user.rewardDebt);
        ...
        if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeElkTransfer(msg.sender, totalRewards);
    ...
    // Safe elk transfer function, just in case if rounding error causes pool to not have enough ELKs.
    function safeElkTransfer(address _to, uint256 _amount) internal {
        uint256 elkBal = elk.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > elkBal) {
            transferSuccess = elk.transfer(_to, elkBal);
        } else {
            transferSuccess = elk.transfer(_to, _amount);
        }
        require(transferSuccess, "safeElkTransfer: transfer failed.");
    }
    ...
    // updateEmissionRate
    function updateEmissionRate(uint256 _elkPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, elkPerBlock, _elkPerBlock);
        elkPerBlock = _elkPerBlock;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            elk.mint(referrer, refBonusEarned);
