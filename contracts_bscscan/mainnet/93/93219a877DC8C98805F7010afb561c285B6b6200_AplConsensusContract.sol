...
contract AplConsensusContract is Pausable, Ownable {
    ...
    // uint256 public aplBalance; // 2 当前钱包地址APL余额
    ...
    uint256 public systemTotalAplAmount; // 5 所有地址，历史累计总锁仓量，不用减去已释放的
    uint256 public systemTotalWithdrawedAplAmount; // 6 所有地址，历史累计领取数额求和
    mapping(address => uint256) public ownerOfTotalUsdt;
    mapping(address => uint256) public ownerOfAplAmount;
    mapping(address => uint256) public ownerOfWithdrawedAplAmount; // 3 当前地址历史累计领取数额
    ...
    function buyAplWithUsdt(uint256 _usdtAmount) public {
        ...
        systemTotalAplAmount = systemTotalAplAmount.add(aplAmount);
    ...
    function withdrawApl() public {
        ...
        ownerOfWithdrawedAplAmount[msg.sender] = ownerOfWithdrawedAplAmount[msg.sender].add(withdrawAmount);
        apl.transfer(msg.sender, withdrawAmount);
        systemTotalWithdrawedAplAmount = systemTotalWithdrawedAplAmount.add(withdrawAmount);
    }

    function getAplBalance(address _address) public view returns(uint256) { // 1 当前地址,已释放、待领取的APL数额
    ...
    function getTotalAddresses() public view returns(uint256) { // 4 累计参与的总的地址数，只要有锁仓都算，已经释放完的也算
        return addresses.length;
    }

    function getTotalUnclaim() public view returns(uint256) { // 7 所有地址，已释放、待领取的APL数额求和 算不出来
        uint256 total = 0;
        for(uint256 i = 0; i < addresses.length; i++){
            total = total.add(getAplBalance(addresses[i]));
        }
        return total;
    }
