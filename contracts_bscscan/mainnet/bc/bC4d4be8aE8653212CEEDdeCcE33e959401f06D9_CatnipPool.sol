...
contract CatnipPool is Ownable, ReentrancyGuard {
    ...
    constructor(address _LoveToken, address _CatnipToken, address _MyRevengeNFT) Ownable() {
    ...
    function available() public view returns (uint256) {
        return Catnip(CatnipToken).balanceOf(address(this));
    }
        
    function claim() public nonReentrant {
        uint256 currentCatnip = available();
        require(MyRevengeNFT(NFTContract).balanceOf(msg.sender) > 0, "Must own the NFT"); //Need to own an NFT to call this function.
        require(currentCatnip > 0, "wut? You can't claim 0 CATNIP lmao"); //lmao
        Love(LoveToken).transferFrom(msg.sender, deadAddress, 1e18); //Burns 1 LOVE tokens. Oh no!!
        Catnip(CatnipToken).transferFrom(address(this), msg.sender, currentCatnip); //take the CATNIP! Yum
        }
}
