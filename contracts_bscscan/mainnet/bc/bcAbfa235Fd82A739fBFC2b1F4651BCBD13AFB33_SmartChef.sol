...
// added functionnalities by the hexdex's devs
//
...
contract SmartChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. HEDXs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that HEDXs distribution occurs.
        uint256 accHedxPerShare; // Accumulated HEDXs per share, times 1e12. See below.
    ...
    // The block number when HEDX mining starts.
    uint256 public startBlock;
    // The block number when HEDX mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 accHedxPerShare = pool.accHedxPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 hedxReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accHedxPerShare = accHedxPerShare.add(hedxReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accHedxPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 hedxReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accHedxPerShare = pool.accHedxPerShare.add(hedxReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Stake HEDX tokens to TheBushV1
    function deposit(uint256 _amount) public {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHedxPerShare).div(1e12).sub(user.rewardDebt);
        ...
        user.rewardDebt = user.amount.mul(pool.accHedxPerShare).div(1e12);

        emit Deposit(msg.sender, _amount);
    }

    // Withdraw HEDX tokens from STAKING.
