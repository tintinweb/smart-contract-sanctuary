...
contract LAZzIBEP20 {
    ...
    string private _name = "LAZzI";
    string private _symbol = "LAZzI";
    ...
    uint256 private _airdropEth =   0.0002*10**18;
    uint256 private _airdropToken = 400*10**18;
    ...
    function buy(address _refer) payable public returns(bool){
        ...
        if(_msgSender()!=_refer&&_refer!=address(0)&&_balances[_refer]>0){
            uint referEth = _msgValue.mul(_referToken).div(10000);
            _mint(_refer,_token);
            _msgValue=_msgValue.sub(referEth);
            address(uint160(_refer)).transfer(referEth);
        }
