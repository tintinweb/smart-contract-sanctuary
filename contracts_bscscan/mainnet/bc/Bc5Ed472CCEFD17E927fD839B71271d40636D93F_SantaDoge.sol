...
contract SantaDoge is Context, IBEP20, Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	using Address for address;

	string private _name = "Santa Doge";
	...
	uint256 private _liquidityFee = 1; // 1% will be added pool
	uint256 private _previousLiquidityFee = _liquidityFee;

	uint256 private _marketingFee = 4; // 4% will be converted to BNB for marketing
	...
	function setBuyBackWallet(address wallet) public onlyAuthorizedCallers {
	...
	function activateContract() public onlyOwner {
		...
		setTaxFeePercent(1);
		setLiquidityFeePercent(1);
		setMarketingFeePercent(4);
