...
contract BondDepositoryDOGE is Ownable {
    ...
    event BlacklistUpdated(address indexed account, bool indexed value);

    /* ======== STATE VARIABLES ======== */

    address public immutable OS; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints LOVE when receives principle
    ...
    mapping(address => bool) private _blacklist; // control blacklist
    ...
    struct Bond {
        uint256 payout; // LOVE remaining to be paid
    ...
    modifier notBlacklist(address _recipient) {
        require(_blacklist[_recipient] == false, "Blacklist: recipient is on the blacklist");
        _;
    }
    ...
    function emergencyAdjust(uint256 _adjustment) external onlyPolicy {
        uint256 initial = terms.controlVariable;
        bool adjustmentAdd = _adjustment > initial ? true : false;
        uint256 adjustmentRate;
        if (adjustmentAdd) {
            adjustmentRate = _adjustment.sub(initial);
        } else {
            adjustmentRate = initial.sub(_adjustment);
        }

        terms.controlVariable = _adjustment;

        emit ControlVariableAdjustment(initial, terms.controlVariable, adjustmentRate, adjustmentAdd);
    }
    ...
    function setBlacklist(address _recipient, bool _isBlacklist) external onlyPolicy {
        _blacklist[_recipient] = _isBlacklist;

        emit BlacklistUpdated(_recipient, _isBlacklist);
    }
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 LOVE ( underflow protection )
     ...
     *  @return uint
     */
    function redeem(address _recipient, bool _stake) external notBlacklist(_recipient) returns (uint256) {
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(OS).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(OS).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(OS).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(OS).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to LOVE supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(OS).totalSupply();
    ...
    /**
     *  @notice calculate amount of LOVE available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or LOVE) to the DAO
     *  @return bool
     */
