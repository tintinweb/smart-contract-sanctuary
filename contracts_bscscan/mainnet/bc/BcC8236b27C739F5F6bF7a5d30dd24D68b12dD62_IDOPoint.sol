...
import "../interfaces/staking/IWaultFarm.sol";
...
contract IDOPoint is Ownable, IIDOPoint {
    ...
    IStaking public playStaking;
    ILPMining public playBusdLpMining;

    // PLAY-wUSD FarmingPool Info
    IWaultFarm public waultFarm;
    uint256 public playWUsdPoolId;

    // weight
    uint256 public playWeight = 3; // 0.3
    uint256 public stakedPlayWeight = 12; // 1.2; multiplied by 10
    uint256 public lpTokenWeight = 30; // 3
    ...
    function getPoint(address user) external view override returns (uint256) {
        ...
        if (address(waultFarm) != address(0)) {
            // sum locked playWUsd amount * lpTokenLockedWeight
            uint256 waultAmount = waultFarm.userInfo(playWUsdPoolId, user).amount;
            total = total + (waultAmount * lpTokenLockedWeight) / weightMultiplier;
        }
    ...
    /**
     * set address of id of playWUsd farming pool on wault finance
     */
    function setWaultFarm(IWaultFarm _waultFarm, uint256 _playWUsdPoolId) external onlyOwner {
        require(address(_waultFarm) != address(0), "Invalid address");

        waultFarm = _waultFarm;
        playWUsdPoolId = _playWUsdPoolId;
    }
...
interface IWaultFarm {
    function userInfo(uint256, address) external view returns (SharedStructs.UserInfo memory);
}
