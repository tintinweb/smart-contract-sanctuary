...
contract SwapFeeReward is Ownable{
    ...
    function setSwapFee(uint32 _swapFee) external onlyOwner {
        require(_swapFee > 0, "SwapFeeReward: lower then 0");
        require(_swapFee <= 1000, 'SwapFeeReward: FORBIDDEN_FEE');
        swapFee = _swapFee;
    }
    ...
    function getSwapFee() internal view returns (uint _swapFee) {
        _swapFee = uint(1000).sub(swapFee);
    }
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) { 
        ...
        uint256 pairFee = getSwapFee();
        uint256 fee = amount.div(pairFee);
        uint256 quantity = getQuantity(output, fee, targetToken); 
        quantity = quantity.mul(pool.percentReward).mul(2).div(100); // 2?
    ...
    //Allow admin to withdraw token in emergency case
    function withdrawReward(uint256 _amt) public onlyOwner{
        IERC20(bscsToken).transfer(msg.sender, _amt);
    }
