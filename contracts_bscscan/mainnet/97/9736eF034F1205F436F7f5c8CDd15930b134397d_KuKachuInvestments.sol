...
contract KuKachuInvestments is AccessControl, Pausable, TokenVesting {
    uint256 private _cap;

    bytes32 public constant ADMIN = keccak256("CAPPER_ROLE");
    bytes32 public constant WHITELISTED_ROLE = keccak256("WHITELISTED_ROLE");
    
    constructor (IERC20 token_) TokenVesting(token_) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(ADMIN, _msgSender());
    }

    function addVestings(uint256[] calldata vestingIds, uint256[] calldata time, uint256[] calldata percentages) external onlyRole(ADMIN) {
        require(vestingIds.length == time.length, "KuKachuInvestments: delay.length not the same as vestingIds.length");
        require(vestingIds.length == percentages.length, "KuKachuInvestments: percentages.length not the same as vestingIds.length");
        uint256 totalPercentages_;
        for (uint256 i = 0; i < percentages.length; i++) {
            totalPercentages_ += percentages[i];
        }
        require(totalPercentages_ == 1000, "KuKachuInvestments: total percentages not 1000");
        for (uint256 i = 0; i < vestingIds.length; i++) {
            require(time[i] != 0, "KuKachuInvestments: delays[i] is 0");
            require(percentages[i] != 0, "KuKachuInvestments: percentages[i] is 0");
            super._addVesting(vestingIds[i], time[i], percentages[i]);
        }
    }

    /**
    * @dev set whitelisted addresses including tokenAmount.
    * @param whitelists array of addresses
    */
    function setWhitelist(address[] calldata whitelists, uint256[] calldata tokenAmount) external onlyRole(ADMIN) {
        require(whitelists.length == tokenAmount.length, "KuKachuInvestments: delay.length not the same as vestingIds.length");
        for (uint256 i = 0; i < whitelists.length; i++) {
            address whitelisted = whitelists[i];
            require(whitelisted != address(0), "KuKachuInvestments: whitelisted is the zero address");
            _setupRole(WHITELISTED_ROLE, whitelisted);
            super._tokenDeposit(getToken(), whitelisted, tokenAmount[i]);
        }
    }
    ...
    */
    function _prevalidateFallbackRedeem(IERC20 token_,  address to, uint256 amount) internal virtual onlyRole(ADMIN) override view {
    ...
    */
    function _preClaim(address beneficiary, uint256 vestingId_) internal virtual override view {
        require(beneficiary == msg.sender, "KuKachuInvestments: only allow transfer the token to the msg.sender");
        super._preClaim(beneficiary, vestingId_);
        require(hasRole(WHITELISTED_ROLE, _msgSender()), "KuKachuInvestments: caller is not whitelisted");
    }
}
