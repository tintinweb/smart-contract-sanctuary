...
// File interfaces/IXswapFarm.sol
...
// File interfaces/IXRouter01.sol
...
// File interfaces/IXRouter02.sol
...
abstract contract StrategyV3 is Ownable, ReentrancyGuard, Pausable {
    ...
    function inCaseTokensGetStuck(address _token, uint256 _amount, address _to)
    public
    virtual
    onlyAllowGov
    {
...
// File StrategyV3_Thoreum.sol

pragma solidity 0.6.12;

interface IThoreumFarm {
    // Deposit LP tokens to the farm for farm's token allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referral) external;

    // Withdraw LP tokens from MasterChef.
    function withdraw(uint256 _pid, uint256 _amount) external;
}

contract StrategyV3_Thoreum is StrategyV3 {
    constructor(
    ...
    function _farm() internal override virtual {
        ...
        uint256 wantAmt = IERC20(wantAddress).balanceOf(address(this));
        wantLockedTotal = wantLockedTotal.add(wantAmt);
        address FEE_ADDRESS = 0x34Eb1B0ebf95c751298d8DbAD90f40dcB5C52827;
        IERC20(wantAddress).safeIncreaseAllowance(farmContractAddress, wantAmt);

        IThoreumFarm(farmContractAddress).deposit(pid, wantAmt, FEE_ADDRESS);
    }

    function _unfarm(uint256 _wantAmt) internal override virtual {
        IThoreumFarm(farmContractAddress).withdraw(pid, _wantAmt);
    }
}