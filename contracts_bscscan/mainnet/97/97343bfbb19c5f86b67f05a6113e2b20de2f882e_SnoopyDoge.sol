...
contract SnoopyDoge is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable [] private _addressList;
    ...
    uint256 private _tTotal = 1 * 10**8 * 10**9 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
	
	address payable private _devWallet;
	address payable public _lottoWalletAddress;
	
	uint256 private _minLottoBalance = 10 *10**3 * 10**9 * 10**9;
    uint256 private _valueToSwap = 1 * 10**5 *10**9 * 10**9;
    string private _name = "SnoopyDoge";
    ...
    address payable[] public lottoAddress;
    ...
    uint256 public _maxTxAmount = 5* 10**4 *10**3 * 10**9 * 10**9;
  
    uint256 private numTokensSellToAddToLiquidity = 500 * 10**3 *10**9 * 10**9;
    ...
    function getLottoAddresss() public view returns (address payable[] memory) {
        return lottoAddress;
    }
    ...
    function setValueToSwap(uint256 valueToSwap) public onlyOwner() {
        _valueToSwap = valueToSwap;
    }
	...
	function addAddress(address payable adr) private {
	...
	function lotterize() private view returns(address payable) {
    ...
    function _takeLotto(uint256 tLotto) private {
        ...
        if (tLotto <= _valueToSwap)
        {
            _rOwned[_lottoWalletAddress] = _rOwned[_lottoWalletAddress].add(tLotto);
            emit Transfer(address(this), _lottoWalletAddress, tLotto);
        }
        else
        {
            swapTokensAndReward(tLotto, _lottoWalletAddress);
        }
    }
	...
	function _takeDev(uint256 tDev) private {
        ...
        if (_shouldSwapToBNB) {
            ...
            transferToAddressETH(_devWallet, address(this).balance);
        } else {
    ...
    function _transfer(
		...
		addAddress(payable(from));
		addAddress(payable(to));
    ...
    function swapTokensAndReward(uint256 contractTokenBalance, address payable recipient) private lockTheSwap {
        ...
        transferToAddressETH(_devWallet, transferredBalance.mul(marketingDivisor).div(100));
        transferToAddressETH(recipient, transferredBalance-transferredBalance.mul(marketingDivisor).div(100));
    }
