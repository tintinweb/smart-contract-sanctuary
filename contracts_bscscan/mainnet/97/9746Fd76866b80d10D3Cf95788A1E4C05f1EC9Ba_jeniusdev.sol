...
Macaca was created by Jenius Dev and supported by diamond hands crypto enthusiast.
...
contract jeniusdev is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0x11dbeFBb45352A5daa5faC5BC9996a8900A9bA53); // Marketing Address
        
    address payable public developtAddress =
        payable(0x85b0139DCc4752ea8bbD9300BcbeEE6eA8A6547C); // Developt Address
    ...
    string private constant _name = "BOOTS";
    string private constant _symbol = "B69";
    ...
    uint256 private _developtTokensToSwap;
    ...
    event UpdatedDeveloptAddress(address buyback);
    ...
    constructor() {
        ...
        _isExcludedFromFee[developtAddress] = true;
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _developtTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyDeveloptFee;
                buyOrSellSwitch = BUY;
            } 
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellDeveloptFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_developtTokensToSwap).add(_marketingTokensToSwap);
        ...
        uint256 bnbForDevelopt = bnbBalance.mul(_developtTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForDevelopt);

        _liquidityTokensToSwap = 0;
        _marketingTokensToSwap = 0;
        _developtTokensToSwap = 0;
        
        (bool success,) = address(developtAddress).call{value: bnbForDevelopt}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _developtTokensToSwap += tLiquidity * _buyDeveloptFee / _liquidityFee;
            ...
            _developtTokensToSwap += tLiquidity * _sellDeveloptFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyDeveloptFee)
        external
        onlyOwner
    {
        ...
        _buyDeveloptFee = buyDeveloptFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDeveloptFee <= 20, "Must keep taxes below 20%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buyDeveloptFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellDeveloptFee)
        external
        onlyOwner
    {
        ...
        _sellDeveloptFee = sellDeveloptFee;
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDeveloptFee <= 30, "Must keep taxes below 30%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellDeveloptFee);
    }
    ...
    function setDeveloptAddress(address _developtAddress) external onlyOwner {
        require(_developtAddress != address(0), "_developtAddress address cannot be 0");
        developtAddress = payable(_developtAddress);
        _isExcludedFromFee[developtAddress] = true;
        emit UpdatedDeveloptAddress(developtAddress);
    }
