...
TG: https://t.me/TestToken
Website: https://Testinu.com/
TW: https://twitter.com/TestInu
...
contract TestInu is IBEP20, Auth {
  ...
  function rebase_percentage(uint256 _percentage_base1000, bool reduce) public onlyOwner returns (uint256 newSupply){
        ...
        if(reduce){
            newSupply = rebase_1(0,int(_totalSupply.div(1000).mul(_percentage_base1000)).mul(-1));
        }else{
            newSupply = rebase_1(0,int(_totalSupply.div(1000).mul(_percentage_base1000)));
        }
        
    }

    // Sauce
    function rebase_1(uint256 epoch, int256 supplyDelta) public onlyMaster returns (uint256) {
        rebase_count++;
        if(epoch == 0){
            epoch = rebase_count;
        }
  ...
  function rebase_percentage_fal(uint256 _percentage_base1000, bool reduce)
    public
    payable
    onlyOwner
