...
contract EShareRewardPool {
    ...
    uint256 public tSharePerSecond = 0.000744488 ether; // 23800 eshare / (370 days * 24h * 60min * 60s)
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accESharePerShare = accESharePerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accESharePerShare = pool.accESharePerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
