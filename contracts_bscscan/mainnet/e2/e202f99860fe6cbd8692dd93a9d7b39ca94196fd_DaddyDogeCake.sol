...
library SafeMath {
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        // See: https://github.com/OpenZeppelin/d*o*ge_ca_kes/pull/522
        if (a == 0) {
            return 0;
        }
...
contract DaddyDogeCake is IERC20 {
    ...
    uint256 private constant startingSupply = 10_000_000_000; // 10 Billion, underscores aid readability

    uint8 private _decimals = 9;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "DaddyDogeCake";
    string constant _symbol = "DaddyDogeCake";

    uint256 public _reflectionFee = 900;
    ...
    uint256 public maxLiquidityFee = 800;
    ...
    address payable private _marketingWallet = payable(0x81bdE655F77975e5Ab660D666830402C5d9150cc);
    ...
    uint256 private sellMaxTxPercent = 1; // Less fields to edit
    uint256 private sellMaxTxDivisor = 1000000;
    ...
    bool private tradingPaused = true;
    ...
    constructor (address cInitializer) payable {
    ...
    function setTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee) external onlyOwner {
    ...
    function setMaxTxPercents(uint256 buyPercent, uint256 buyDivisor, uint256 sellPercent, uint256 sellDivisor) external onlyOwner() {
        require(buyDivisor <= 1000 && sellDivisor <= 1000); // Cannot set lower than 0.01%
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if(_hasLimits(from, to)) {
            require(tradingPaused == false, "Trading is currently paused.");
            if(to == lpPair) {
