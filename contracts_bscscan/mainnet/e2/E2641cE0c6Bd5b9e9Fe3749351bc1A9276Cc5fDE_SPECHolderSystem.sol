...
contract SPECHolderSystem is Ownable {
    using SafeMath for uint256;
    ...
    function deposit(uint256 _amount) public {
        ...
        require(user.depositTime + 2592000 < now, "deposit: can't deposit more for 30 days");
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0 && user.depositTime + 2592000 < now && rewardToken.balanceOf(address(this)) > pending) {
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(user.depositTime + 2592000 < now, "withdraw: can't withdraw for 30 days");
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0 && user.depositTime + 2592000 < now && rewardToken.balanceOf(address(this)) > pending) {
            rewardToken.safeTransfer(address(msg.sender), pending);
