...
Telegram: t.me/shibanomics
Twitter: twitter.com/shibanomicsbsc

------------------------------
Contract Dev: t.me/anoopppp
------------------------------
...
contract SHIBIN is ERC20, Ownable {
    ...
    SHIBINDividendTracker public dividendTracker;
    
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public constant SHIB = address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); //SHIB
    ...
    uint256 public totalFees = SHIBRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
    ...
    constructor(address _marketingWalletAddress) ERC20("SHIBANOMICS", "SHIBIN") {

    	dividendTracker = new SHIBINDividendTracker();
    ...
    /// @notice Withdraw tokens sent by mistake. Impossible to withdraw SHIBIN tokens
    /// @param tokenAddress The address of the token to withdraw
    function rescueBEP20Tokens(address tokenAddress) external onlyOwner{
        require(tokenAddress != address(this), "Impossible to withdraw SHIBIN tokens");
   ...
   /////////////////////////////////
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SHIBIN: Account is already the value of 'excluded'");
   ...
   ///////////////////////
    ...
    function setSHIBRewardsFee(uint256 value) external onlyOwner{
        ...
        totalFees = SHIBRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
    }
    ...
    function setCoolDown(uint256 interval, bool _enabled) external onlyOwner{
        coolDownEnabled = _enabled;
        coolDownInterval = interval * 1 seconds;
    }
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "SHIBIN: Automated market maker pair is already set to that value");
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SHIBIN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SHIBIN: Cannot update gasForProcessing to same value");
    ...
    function buyback(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "Insufficient BNB balance");
        swapBNBForTokens(weiAmount);
    }
   ...
   /**
    ...
    function _transfer(address from, address to, uint256 amount) internal override{
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && !swapping){
            ...
            if(coolDownEnabled){
                require(_lastTrade[from].add(coolDownInterval) <= block.timestamp, "Cooldown not passed yet");
            }
        ...
        if(!_isExcludedFromFees[to] && automatedMarketMakerPairs[from] && !swapping){
            ...
            if(coolDownEnabled){
                require(_lastTrade[to].add(coolDownInterval) <= block.timestamp, "Cooldown not passed yet");
            }
        ...
        if(canSwap && !swapping && swapEnabled && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[from]) {
            ...
            if(liquidityFee > 0 || marketingFee > 0 || buybackFee > 0){
                uint256 swapTokens = contractTokenBalance.mul(liquidityFee.add(marketingFee).add(buybackFee)).div(totalFees);
                swapAndLiquify(swapTokens);
            }
            if(SHIBRewardsFee > 0){
                uint256 sellTokens = contractTokenBalance.mul(SHIBRewardsFee).div(totalFees);
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 denominator= (liquidityFee + marketingFee + buybackFee) * 2;
    ...
    function swapBNBForTokens(uint256 weiAmount) private{
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(this);
        
        // make the swap
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: weiAmount}(
...
contract SHIBINDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("SHIBIN_Dividen_Tracker", "SHIBIN_Dividend_Tracker") {
    	claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SHIBIN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SHIBIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHIBIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SHIBIN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SHIBIN_Dividend_Tracker: Cannot update claimWait to same value");
