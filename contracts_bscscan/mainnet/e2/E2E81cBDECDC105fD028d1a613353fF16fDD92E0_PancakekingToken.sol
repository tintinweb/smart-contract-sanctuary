...
contract PancakekingToken is Context, IERC20, Ownable {
    ...
    function setFeeRates(uint256 _rfi, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _liquidity, uint256 _buyback) external onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tLiquidity-s.tBuyback;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rTransferAmount =  rAmount-rRfi-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rLiquidity, rBuyback);
    }
