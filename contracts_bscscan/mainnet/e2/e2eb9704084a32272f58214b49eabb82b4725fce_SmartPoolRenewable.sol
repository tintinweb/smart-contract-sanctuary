...
contract SmartPoolRenewable is Ownable, ReentrancyGuard {
    ...
    // Total Actual Staked and extra tokens
    uint256 extraTokens;
    ...
    */
    ...
    function ExtendPoolWithFundsAlreadyInContract() external onlyOwner {
        require(bonusEndBlock > block.number, "Pool has Ended, use RenewPool");
        ...
        bonusEndBlock = bonusEndBlock + (totalNewReward / rewardPerBlock);
    }

    function startNewPool( uint256 _startInDays, uint256 _poolLengthDays ) external onlyOwner {
        // make sure pool has ended
        require(bonusEndBlock < block.number, "Cannot start New pool");
        // setup for calculations
        uint256 totalNewReward;
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        if (block.number < bonusEndBlock + 2880000) {
    ...
    function RemoveStakedReflections() external onlyOwner {
        if (stakedToken == rewardToken) {
            uint256 StakedTokenBalanceOnly = stakedToken.balanceOf(address(this)) - prevAndCurrentRewardsBalance;
            extraTokens = totalStaked - StakedTokenBalanceOnly;
        } else {
            uint256 StakedTokenBalanceOnly = stakedToken.balanceOf(address(this));
            extraTokens = totalStaked - StakedTokenBalanceOnly;
        }
