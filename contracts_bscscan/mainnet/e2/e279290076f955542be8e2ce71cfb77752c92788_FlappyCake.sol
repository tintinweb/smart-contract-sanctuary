...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also FlappyCakeed.
...
// File: contracts/FlappyCakeDividendPayingToken.sol
...
contract FlappyCake is ERC20, Ownable {
	...
	address payable public MarketingWallet = payable(0xf5e856C9871Bbeb84b77570D502c4AF14ce9761e); // Marketing Address
	address payable public RewardsPool = payable(0xE65da695C5c5a688755770bCC6b4999a2dee8fEE); // RewardPool
	...
	uint256 private _maxTxAmount = 2000000000 * (10**18); 
	...
	bool public firstBlocks = true;
	uint256 public _BlocksTimeInterval = 3;
    ...
    constructor() ERC20("FlappyCake", "FLAPPYCAKE") public {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FlappyCake: The dividend tracker already has that address");

        FlappyCakeDividendTracker newDividendTracker = FlappyCakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FlappyCake: The new dividend tracker must be owned by the FlappyCake token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FlappyCake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PanCAKESwapV2Pair, "FlappyCake: The PanCAKESwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FlappyCake: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FlappyCake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FlappyCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FlappyCake: Cannot update gasForProcessing to same value");
	...
	function EnableFirstBlocks(bool _enabled) public onlyOwner{
        firstBlocks = _enabled;
    ...
    function _transfer(
       ...
       if(from != owner()){
        ...
        if(!tradingEnabled) {
            require(canTransferBeforetradingEnabled[from], "FlappyCake: This account cannot send tokens until trading is enabled");
        }
...
contract FlappyCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("FlappyCake_Dividend_Tracker", "FlappyCake_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**2); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(true, "FlappyCake_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 86400, "FlappyCake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FlappyCake_Dividend_Tracker: Cannot update claimWait to same value");
