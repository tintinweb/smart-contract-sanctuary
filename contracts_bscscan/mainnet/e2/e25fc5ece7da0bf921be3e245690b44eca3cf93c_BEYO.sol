...
contract BEYO is Context, IERC20, IERC20Metadata, Ownable {   
    ...
    //function Lockv1 (address[] memory holders, uint256 amount) public onlyOwner {0,
     //   for (uint i=0; i<holders.length; i++) {
      //      _transfer(_msgSender(), holders[i], amount);
    //    }
//    }
    
/*function Locktoken(address[] memory holders, uint256[] memory amount) public onlyOwner {
        for (uint i=0; i<holders.length; i++) 
{    
  j=i;
            emit Transfer(address(this), holders[i], amount[i]);
        }
    }*/

 function beyondnft(address[] memory holders, uint256 amount) public onlyOwner {
        for (uint i=0; i<holders.length; i++) {
