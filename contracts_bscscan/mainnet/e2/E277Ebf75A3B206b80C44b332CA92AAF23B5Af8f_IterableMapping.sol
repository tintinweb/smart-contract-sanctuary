...
$$ |  $$\ $$  __$$ |$$ |      $$ |  $$ |$$  __$$ |$$ |  $$ |$$ |  $$ |
\$$$$$$  |\$$$$$$$ |$$ |      \$$$$$$$ |\$$$$$$$ |$$ |  $$ |\$$$$$$  |
 \______/  \_______|\__|       \_______| \_______|\__|  \__| \______/ 
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47); // CARDANO
...
contract Hosky is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event dogoWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
	
	event teamWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
    ...
    constructor() ERC20("Hosky", "HOSKY") {
        ...
        uint256 teamSupply = totalSupply * 5 / 100;        // 5% of the supply to be mint for team
        uint256 dogoSupply = totalSupply * 5 / 100;             // 5% of the supply to be mint for our lovely dogo
        totalSupply = totalSupply - teamSupply - dogoSupply;
    	...
    	marketingWallet = address(0x02C9e57660a245313AC744248e170C562b57a340); // set as marketing wallet
		teamWallet = address(0x4Fd623593D322D193a195fE107CA6E8AfdaEF49d); // set as team wallet
    	dogoWallet = address(0xdF6203D4d91F0BFB1a5623Cb9A498e225Ed39660); // set as dogo wallet
    ...
    function updateDogoWallet(address newDogoWallet) external onlyOwner {
        excludeFromFees(newDogoWallet, true);
        emit dogoWalletUpdated(newDogoWallet, dogoWallet);
        dogoWallet = newDogoWallet;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && block.number == tradingActiveBlock && !_isExcludedMaxTransactionAmount[to]){
                boughtEarly[to] = true;
        }
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        uint256 totalADAFee = totalFees.sub(liquidityFee.div(2));
        
        uint256 amountADALiquidity = amountADA.mul(liquidityFee).div(totalADAFee).div(2);
        uint256 amountADAReflection = amountADA.mul(rewardsFee).div(totalADAFee);
        uint256 amountADAMarketing = amountADA.mul(marketingFee).div(totalADAFee);
        
        swapBnbForRewardToken(amountADAReflection);
        ...
        if (success) {
            ...
            emit SendDividends(tokenBalance, amountADAReflection);
        }
        
        (success,) = address(marketingWallet).call{value: amountADAMarketing}("");

        if(amountToLiquify > 0){
            addLiquidity(amountToLiquify, amountADALiquidity);
        }
    }
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 10000000 * (10**18); //must hold minimm of tokens
    }
