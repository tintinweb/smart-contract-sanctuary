...
contract CorgiQDoge is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xAbf5F07Ce6d33C2a48802e2d821dc6d3CF0C5eCC); // Marketing Address
    ...
    string private _name = "CorgiQDoge";
    string private _symbol = "CorgiQDoge";
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 3, "taxFee must not be higher than 3%");
        _taxFee = taxFee;
    }
        
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee) external onlyOwner {
        require(buyTaxFee <= 10, "buyTaxFee: must not be higher than 10%");
        require(buyLiquidityFee <= 10, "buyLiquidityFee: must not be higher than 10%");
        _buyTaxFee = buyTaxFee;
        _buyLiquidityFee = buyLiquidityFee;
    }
   
    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee) external onlyOwner {
        require(sellTaxFee <= 10, "sellTaxFee: must not be higher than 10%");
        require(sellLiquidityFee <= 10, "sellLiquidityFee: must not be higher than 10%");
        _sellTaxFee = sellTaxFee;
        _sellLiquidityFee = sellLiquidityFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require(liquidityFee <= 10, "liquidityFee: Over amount");
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
        require(maxTxAmount >= 1000 * 10**6 * 10**9, "maxTxAmount: min amount require");
        _maxTxAmount = maxTxAmount;
    }
    
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(divisor <= 5, "marketingDivisor: Over amount");
    ...
    function corgiQToMoon(uint256 amount) external onlyOwner(){
