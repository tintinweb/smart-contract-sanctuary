...
contract CakeHeaven is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   CakeHeavenDividendTracker public dividendTracker;
    ...
    uint256 private _previousCAKERewardsFee;
    uint256 private _previousMarketingFee;
    uint256 private _liquidityFee;
    uint256 private _CAKERewardsFee;
    ...
    constructor() ERC20 ("CakeHeaven", "CakeHeaven") {
        _CAKERewardsFee = 11;
        _liquidityFee = 4;
		_marketingFee = 1;
    	...
    	dividendTracker = new CakeHeavenDividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x467Ef8C3093b4BCFd9Dd8d8D28c140972082BAef);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CakeHeaven: The dividend tracker already has that address");

        CakeHeavenDividendTracker newDividendTracker = CakeHeavenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CakeHeaven: The new dividend tracker must be owned by the BuCAKEe token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CakeHeaven: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CakeHeaven: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxSell(address account, bool excluded) public onlyOwner {
        require(_isExcludedMaxSellTransactionAmount[account] != excluded, "CakeHeaven: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CakeHeaven: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CakeHeaven: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CakeHeaven: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CakeHeaven: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CakeHeaven: Cannot update gasForProcessing to same value");
    ...
    function removeAllFee() external onlyOwner() {
        ...
        _previousCAKERewardsFee  = _CAKERewardsFee;
    ...
    function restoreAllFee() external onlyOwner() {
        ...
        _CAKERewardsFee = _previousCAKERewardsFee;
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CakeHeaven: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CakeHeaven: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateCAKERewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "CakeHeaven: CAKE Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract CakeHeavenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CakeHeaven_Dividend_Tracker", "CakeHeaven_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 50000000 * (10**18);
    }



    function _transfer(address, address, uint256) internal pure override {
        require(false, "CakeHeaven_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CakeHeaven_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CakeHeaven contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CakeHeaven_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CakeHeaven_Dividend_Tracker: Cannot update claimWait to same value");
