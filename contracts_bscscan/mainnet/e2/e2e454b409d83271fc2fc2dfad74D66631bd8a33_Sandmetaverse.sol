 ...
 * that a supply mechanism has to be added in a derived contract using {initiateData}.
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     */
    function initiateData(address account, uint256 amount) internal virtual {
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  function initiateData(address account, uint256 value) internal override {
    super.initiateData(account, value);
  ...
  function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      initiateData(account, mintAmount);
...
contract SandmetaverseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Sandmetaverse_Dividend_Tracker", "Sandmetaverse_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 75000000000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Sandmetaverse_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Sandmetaverse_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Sandmetaverse contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Sandmetaverse_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Sandmetaverse_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Sandmetaverse is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SandmetaverseDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Sand Metaverse", "SDM") {
        ...
        marketingFee = 3;
        marketingWallet = payable(0xd1875058632073806D3d83F955ab1596F5Ac6c5F);

    
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new SandmetaverseDividendTracker();
...
initiateData(owner(), 1000000000 * (10**18));
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Sandmetaverse: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Sandmetaverse: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Sandmetaverse: Automated market maker pair is already set to that value");
