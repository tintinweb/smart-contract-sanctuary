...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Satoswap Lottery Ticket", "SATOLT") {}
...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        sato.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        sato.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safeSatoTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safeSatoTransfer(address(msg.sender), totalReward);
        }
    ...
    // Safe SATO transfer function, just in case if rounding error causes pool to not have enough SATOs.
    function safeSatoTransfer(address _to, uint256 _amount) internal {
        uint256 satoBal = sato.balanceOf(address(this));
        if (_amount > satoBal) {
            sato.transfer(_to, satoBal);
        } else {
            sato.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        sato.safeTransfer(address(msg.sender), _amount);
