 ...
 */ 
 ...
 // @ELONSELLTESLA, Join us on telegram now!
...
contract ELONSELLTESLA is IERC20Metadata, Ownable {
    ...
    uint256 public TESLASHORTPortionOfSwap = 30;
    ...
    uint256 public sellingTESLASHORTFee = 3;
    uint256 public buyingTESLASHORTFee = 3;
    
    uint256 public buyersTotalFees = buyingMarketingFee.add(buyingRfiFee).add(buyingLpFee).add(buyingTESLASHORTFee);
    uint256 public sellersTotalFees = sellingMarketingFee.add(sellingRfiFee).add(sellingLpFee).add(sellingTESLASHORTFee);
    ...
    event TESLASHORTWalletUpdated(address indexed newTESLASHORTWallet, address indexed oldTESLASHORTWallet);
    ...
    function calcBuyersTotalFees() private {
        buyersTotalFees = buyingMarketingFee.add(buyingRfiFee).add(buyingLpFee).add(buyingTESLASHORTFee);
    }
    
    function calcSellersTotalFees() private {
        sellersTotalFees = sellingMarketingFee.add(sellingRfiFee).add(sellingLpFee).add(sellingTESLASHORTFee);
    }
    ...
    function updateSellingTESLASHORTFee (uint256 newFee) external onlyOwner {
        sellingTESLASHORTFee = newFee;
        calcSellersTotalFees();
    }
    ...
    function updateBuyingTESLASHORTFee (uint256 newFee) external onlyOwner {
        buyingTESLASHORTFee = newFee;
        calcBuyersTotalFees();
    }
    ...
    function updateTESLASHORTWallet(address newTESLASHORTWallet) external onlyOwner {
        require(newTESLASHORTWallet != TESLASHORTWallet, "The TESLASHORT wallet is already this address");
        TESLASHORTWallet = newTESLASHORTWallet;
        
        emit TESLASHORTWalletUpdated(newTESLASHORTWallet, TESLASHORTWallet);
    }
    ...
    function updateTESLASHORTPortionOfSwap(uint256 newAmount) external onlyOwner {
        TESLASHORTPortionOfSwap = newAmount;
    }
    ...
    function _takeFees(uint256 amount, uint256 currentRate, uint256 sumOfFees, bool isBuying) private {
        if ( sumOfFees > 0 && !isInPresale ){
            if(isBuying) {
                ...
                _takeFee( amount, currentRate, buyingTESLASHORTFee, address(this)); 
            }
            else{
                ...
                _takeFee( amount, currentRate, sellingTESLASHORTFee, address(this)); 
            }
        }
    }
    ...
    function _beforeTokenTransfer(address recipient) private {
        ...
        if ( !isInPresale ){
            ...
            if (!swapping && canSwap && swapEnabled && recipient == pair) {
                ...
                uint256 TESLASHORTPercent = contractTokenBalance.mul(TESLASHORTPortionOfSwap).div(FEES_DIVISOR);
                uint256 TESLASHORTPAndMarkP = marketingPercent.add(TESLASHORTPercent);
                uint256 liquidityPercent = contractTokenBalance.sub(TESLASHORTPAndMarkP);
                
                //swap and send to marketing wallet
                sendToMarketingAddress(marketingPercent); 
                
                //swap and send to TESLASHORT wallet
                sendToTESLASHORTAddress(TESLASHORTPercent);
    ...
    function sendToTESLASHORTAddress(uint256 tokenAmount) private {
        uint256 initialBalance = address(this).balance;
        swapTokensForBNB(tokenAmount);
        
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        
         //Send to TESLASHORT address
       transferToAddressBNB(payable(TESLASHORTWallet), transferredBalance);
    }
