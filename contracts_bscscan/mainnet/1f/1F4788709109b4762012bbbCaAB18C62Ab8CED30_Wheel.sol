...
contract Wheel is Context, Ownable {
    ...
    uint256 private minToBurn = 10**9*10**9; //1B OROS
    uint256 private minToSpot = 10**9*10**9; //1B OROS
    uint256 private spotTracker = 0;
    uint256 private burnTracker = 0;
    ...
    mapping(bytes32 => uint) private pools_indexes;
    mapping(bytes32 => mapping(uint256 => Game)) private games;

    event Joined(bytes32 pool_name, uint256 indexed playedCount, address[] users, uint winnerNum);
    ...
    function addPool(bytes32 name, bytes32 description, uint256 price, uint max) external onlyOwner(){
    ...
    function setMinToBurn(uint256 min) external onlyOwner(){
        minToBurn = min;
    }

    function setMinToSpot(uint256 min) external onlyOwner(){
        minToSpot = min;
    }

    function getMinToBurn() external view returns(uint256){
        return minToBurn;
    }

    function getMinToSpot() external view returns(uint256){
        return minToSpot;
    }
    ...
    function terminateGame(uint pool_id, uint random) private{
        ...
        spotTracker.add(jackSpotWin);
        burnTracker.add(burnWin);

        transf2(userWin, game.winner);

        if(spotTracker >= minToSpot){
            transf2(spotTracker, jackSpotCharityContract);
            spotTracker = 0;
        }
        
        if(burnTracker >= minToBurn){
            transf2(burnTracker, deadAddress);
            burnTracker = 0;
        }
    }

    function join(bytes32 pool_name) external {
        uint pool_id = pools_indexes[pool_name];
        Pool storage pool = pools[pool_id];

        require(!_isJoined(pool), "Nothing to do here");
        require(pool.joined < pool.max && token.balanceOf(_msgSender()) >= pool.price, "Wait for next");

        if(pool.joined.add(1) == pool.max){
            _join(pool);
            uint random = _random(pool.max);
            terminateGame(pool_id, random);
            Game memory game = games[pool.name][pool.playedCount];
            uint256 _game_id = pool.playedCount;
            pool.playedCount = pool.playedCount.add(1);
            pool.bSelectSpin = block.number;
            allGames.push(game);
            emit Joined(pool.name, _game_id, game.joined_users, game.winnerNum);
            pool.joined = 0;
            pool.spot = 0;
        }else{
            _join(pool);
            Game memory game = games[pool.name][pool.playedCount];
            emit Joined(pool.name, pool.playedCount, game.joined_users, 10**9);
        }
    }
    ...
    function updatePool(bytes32 name, uint256 price, uint max, uint pool_id, bytes32 description) external onlyOwner(){
        Pool storage pool = pools[pool_id];
        pool.name = name;
    ...
    function getPoolByName(bytes32 pool_name) external view returns(Pool memory){
      return pools[pools_indexes[pool_name]];
    }

     function getGame(bytes32 pool_name) external view returns(Game memory){
    ...
    function _random(uint max) private view returns(uint)
    {
        uint256 seed = uint256(keccak256(abi.encodePacked(
            block.timestamp + block.difficulty + _generateNumber()
        )));
