...
import "./lib/Ownable.sol";
import "./GameWinMatchLibrary.sol";
import "./GameWinMatch.sol";

contract GameWinMatchGenerator is Ownable {
    using SafeMath for uint256;

    event MatchCreated(string title, uint256 matchId, address creator);
    event Received(address indexed from, uint256 amount);

    GameWinMatchLibrary public immutable gameLibrary;
    ERC20 public gameToken;

    constructor(address _libraryAddress, address _gameToken) public {
        gameLibrary = GameWinMatchLibrary(_libraryAddress);
    ...
    modifier onlyDev() {
        require(msg.sender == owner || gameLibrary.getDev(msg.sender));
        _;
    }
    ...
    modifier onlyCreator() {
        require(
            msg.sender == owner ||
                gameLibrary.getDev(msg.sender) ||
                gameLibrary.getCreator(msg.sender)
        );
        _;
    }
    ...
    function initializeMatch(
        GameWinMatch _match,
        uint256 _matchId,
        MatchMainInfo calldata _info,
        MatchStringInfo calldata _stringInfo
    ) internal {
        _match.setAddressInfo(
            msg.sender,
            _info.tokenAddress,
            _info.tokenDecimals,
            address(gameToken)
        ...
        _match.setMatchTitle(
            _stringInfo.matchTitle,
            _stringInfo.matchUrl,
            _stringInfo.liveUrl,
            _stringInfo.gameType
        );

        _match.setStringInfo(
            _stringInfo.linkWebsite,
            _stringInfo.description,
            _stringInfo.tournament
        );
    }
    ...
    function createMatch(
        ...
        GameWinMatch newMatch = new GameWinMatch(
            address(this),
            address(gameLibrary),
            gameLibrary.owner()
        );

        uint256 matchId = gameLibrary.addMatchAddress(address(newMatch));
        initializeMatch(newMatch, matchId, _info, _stringInfo);

        emit MatchCreated(_stringInfo.matchTitle, matchId, msg.sender);
    }
}
