...
| |  | | | (_) | ||  __/ (__| || (_) | |   
\_|  |_|  \___/ \__\___|\___|\__\___/|_|   

     Released By Rizen & DuhItzMark (Mark Thomas)                                      
*/
...
contract HuskyProtector is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Husky Protector";
    string private _symbol = "HuskyProt";
    ...
    uint256 internal _tokenTotal = 5 *10**17; // Total supply 500M
    ...
    uint256 public _charity2Fee = 0; //

    
    uint256 public _taxFeeTotal;  //reflection fee
    uint256 public _liquidityFeeTotal; //liquidity fee
    uint256 public _charityFeeTotal; //charity fee
    uint256 public _charity2FeeTotal; //charity2 fee
    
    address public charity2Address = 0x0000000000000000000000000000000000000000; // burn address
    address public charityAddress = 0x793BF433b6A8d473798009A0C544156AA1F47626; // marketing wallet
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        uint256 transferAmount = amount;
        uint256 charity2Fee = amount.mul(_charity2Fee).div(10000);
        ...
        //@dev 2nd charity fee
        if(charity2Fee > 0){
            transferAmount = transferAmount.sub(charity2Fee);
            _reflectionBalance[charity2Address] = _reflectionBalance[charity2Address].add(charity2Fee.mul(rate));
            _charity2FeeTotal = _charity2FeeTotal.add(charity2Fee);
            emit Transfer(account,charity2Address,charity2Fee);
        }
