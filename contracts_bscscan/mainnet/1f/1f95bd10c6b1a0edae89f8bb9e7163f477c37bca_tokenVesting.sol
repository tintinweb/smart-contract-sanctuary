...
contract tokenVesting is Ownable, Pausable {
    ...
    function claim (uint stage) external {
        ...
        if(stage == 0){
            ...
            amountToTranfer = (userAllocation[msg.sender] * 4000) / 10000;
        } else {
            require(startTime + 1 days + (stage * 7 days) <= block.timestamp, "Stage not ready to claim yet");
            amountToTranfer = (userAllocation[msg.sender] * 1500) / 10000; 
        }
