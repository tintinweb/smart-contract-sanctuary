...
contract BorrowProxy is IBorrowProxy, ConfigBase {
    ...
    function borrow(address owner, uint256 id, address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 leverage, uint256 deadLine) override external returns (uint256, uint256, uint256) {
        ...
        if (0 < leverage) {
            ...
            if (tokenA == borrowToken) {
                ...
                amountA = amountA.add(config.getExchange().swap(borrowToken, tokenA, leverage, deadLine));
            }
        }
    ...
    function trim(address tokenA, uint256 amountA, address tokenB, uint256 amountB, uint256 deadLine) external returns (uint256, uint256) {
        ...
        uint256 price = config.getExchange().swapEstimate(tokenA, tokenB, oneA);
        ...
        if (valueA < valueB) {
            ...
            if (0 < swapB) {
                ...
                price = config.getExchange().swap(tokenB, tokenA, swapB, deadLine);
            ...
            if (0 < swapA) {
                ...
                price = config.getExchange().swap(tokenA, tokenB, swapA, deadLine);
    ...
    function trimV2(address tokenA, uint256 amountA, address tokenB, uint256 amountB) external view returns (uint256, uint256) {
        ...
        if (valueA < valueB) {
            ...
            if (0 < swapB) {
                amountB = amountB.sub(swapB);
                price = config.getExchange().swapEstimate(tokenB, tokenA, swapB);
                amountA = amountA.add(price);
            }
        }else {
            uint256 swapA = valueA.sub(valueB).div(oneB).div(2);
            if (0 < swapA) {
                amountA = amountA.sub(swapA);
                price = config.getExchange().swapEstimate(tokenA, tokenB, swapA);
                amountB = amountB.add(price);
            }
        }
        return (amountA, amountB);
    ...
    function getBorrowAmount(address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 leverage) external view returns (uint256) {
        ...
        if (0 < leverage) {
            ...
            uint256 price = config.getExchange().swapEstimate(tokenA, borrowToken, 10 ** decimal);
            uint256 value = leverage.mul(amountA).mul(price);
            borrowAmount = value.div(10 ** decimal).div(10);
            decimal = IERC20(tokenB).decimals();
            price = config.getExchange().swapEstimate(tokenB, borrowToken, 10 ** decimal);
    ...
    function swap(address borrowToken, uint256 borrowAmount, address tokenA, uint256 deadLine) external {
        ...
        config.getExchange().swap(borrowToken, tokenA, borrowAmount, deadLine);
    }
