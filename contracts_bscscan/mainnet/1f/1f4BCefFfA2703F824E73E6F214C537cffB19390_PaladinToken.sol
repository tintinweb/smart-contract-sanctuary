...
contract PaladinToken is BEP20, MinterRole {
    ...
    address public taxWallet;// tax Fee Wallet address

    uint256 public buyTax = 0;
    uint256 public sellTax = 0;   // default 10%
    ...
    event UpdateTaxWallet(address indexed walletAddress);
    event UpdateBuyTax(uint16 buyTax);
    event UpdateSellTax(uint16 sellTax);
    
    /**
     * @notice Operator CAN do modifier
     */
     ...
     */
    constructor(address _taxWallet) public BEP20("Paladins", "PAL") {
        operator = _msgSender();
        ...
        require(_taxWallet != address(0), "wallet address is zero");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            require(tradingActive == true, "Trading is not Active yet.");

            uint256 sendAmount = amount;
            uint256 feeAmount;
            //Buy Token
            if(automatedMarketMakerPairs[sender] && buyTax > 0) {
                feeAmount = amount.mul(buyTax).div(100);
                sendAmount = amount.sub(feeAmount);
            }
            //Sell Token
            if(automatedMarketMakerPairs[recipient] && sellTax > 0) {
     ...
     */
    function updateTaxWallet(address _taxWallet) public onlyOperator {
        require( _taxWallet != address(0), "PAL:: tax fee address is zero" );
        emit UpdateTaxWallet(_taxWallet);
        taxWallet = _taxWallet;
    }

    function updateBuyTax(uint16 _buyTax) public onlyOperator {
        require( _buyTax <= 100, "PAL:: Biu Fee can't exceed 100%" );
        emit UpdateBuyTax(_buyTax);
        buyTax = _buyTax;
    }

    function updateSellTax(uint16 _sellTax) public onlyOperator {
        require( _sellTax <= 100, "PAL:: Sell Fee can't exceed 100%");
        emit UpdateSellTax(_sellTax);
        sellTax = _sellTax;
    }
    ...
    function openTrading(bool isActive) public onlyOperator {
