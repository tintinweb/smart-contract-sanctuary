...
contract OracleSimpleUstUsdcPLP {
    using FixedPoint for *;

    uint256 public constant PERIOD = 12 hours;
    address public constant pair = address(0x74d51fC48013546D716a7884cc69d0Bb75807f8A);
    address public constant ust = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ...
    UniswapV2 planet = UniswapV2(0x9F088377BcdC220CB0E1Ad15aE6Bc75074beE9F6);
    uint256 public usdcAmount;
    uint256 public ustAmount;
    uint256 public lpAmount;
    
    constructor() public {
        usdcAmount = ERC20(usdc).balanceOf(pair);
        ustAmount = ERC20(ust).balanceOf(pair);
        lpAmount = ERC20(pair).totalSupply();
    }
    ...
    function update() external {
       ...
       if (timeElapsed >= PERIOD) {
            ...
            // additional LP soft oracle (boundried for v1)
            address[] memory path = new address[](2);
            uint256[] memory amounts = new uint256[](2);
            path[0] = address(usdc);
            path[1] = address(ust);
            amounts = planet.getAmountsOut((10 ** 18), path);
            uint256 twap = price0Average.mul((10 ** 18)).decode144();
            if (amounts[1] * 3 > twap && amounts[1] < twap * 3) {
                usdcAmount = ERC20(usdc).balanceOf(pair);
                ustAmount = ERC20(ust).balanceOf(pair);
                lpAmount = ERC20(pair).totalSupply();
            }
