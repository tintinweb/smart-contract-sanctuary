...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender() || _prev == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract Presale is Ownable {
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "LITPresale: sale is not open");
        require(!hasEnded(), "LITPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "LITPresale: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "LITPresale: max contribution criteria not met"
        );
        this;
    }
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "LITPresale: presale is not over");
