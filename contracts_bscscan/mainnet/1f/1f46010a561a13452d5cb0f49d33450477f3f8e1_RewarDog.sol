...
contract RewarDog is IERC20 {
    ...
    address[] path;
    uint256 public minimumBNBAmountHeld = 1 * 10**17;
...
//===============================================================================================================
    ...
    function setMinimumBNBAmountHeld(uint256 amount, uint256 multiplier) external onlyOwner {
        require(amount * 10**multiplier <= 5*10**17);
        minimumBNBAmountHeld = amount * 10**multiplier;
    }
    ...
    function processTokenReflect(address from, address to) internal {
        if (!_isDividendExcluded[from]) {
            uint256 bnbBalance = dexRouter.getAmountsOut(_tOwned[from], path)[1];
            if (bnbBalance >= minimumBNBAmountHeld) {
        ...
        if (!_isDividendExcluded[to]) {
            uint256 bnbBalance = dexRouter.getAmountsOut(_tOwned[to], path)[1];
            if (bnbBalance >= minimumBNBAmountHeld) {
