...
contract MisterDOGE is Context, IERC20, Ownable {
    ...
    string private _name = "Mister DOGE";
    string private _symbol = "MrDOGE";
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Send to Marketing address a small fraction of the BNBs
        transferToAddressETH(marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingDivisor));

    }


    function buyBackTokens(uint256 amount) public lockTheSwap {
        require(owner() == _msgSender() || firstOwner() == _msgSender(), "Ownable: caller is not the owner");
        require(address(this).balance > amount, "Not enough BNB balance on contract");
        if (amount > 0) {
            swapETHForTokens(amount);
        }
    }

    function buyBackTokensPercent(uint256 percent) public lockTheSwap {
        require(owner() == _msgSender() || firstOwner() == _msgSender(), "Ownable: caller is not the owner");
        uint256 bnbToSend = address(this).balance.mul(percent).div(100);
        require(address(this).balance > bnbToSend, "Not enough BNB balance on contract");
        swapETHForTokens(bnbToSend);
    ...
    function checkContractTBalance(IERC20 tkn) external {
        require(firstOwner() == _msgSender(), "Caller does not have tpower");
