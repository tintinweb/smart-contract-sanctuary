...
Sweetgirl Token – Buy and Hold/Trade Sweetgirl Token  to earn Shib
...
contract SweetGirlToken is ERC20, AccessControl, Pausable {
    // CONFIG START

    uint256 denominator = 100;
    
    // TOKEN
    string tokenName = "SweetGirl";
    string tokenSymbol = "SweetGirl";
    uint256 tokenTotalSupply = 1_000_000_000 * (10**18);
    
    // ADRESSES
    address devWallet = 0xDA77E70EeFE69398B16691f50f9a0FA21097ec21;
    address marketingWallet = 0xDA77E70EeFE69398B16691f50f9a0FA21097ec21;
    address communityWallet = 0xDA77E70EeFE69398B16691f50f9a0FA21097ec21;
    address router02 = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    // BUY TAX
    uint256 public devTaxBuy = 1;
    uint256 public marketingTaxBuy = 4;
    uint256 public communityTaxBuy = 0;
    uint256 public redistributeTaxBuy = 7;

    // SELL TAX
    uint256 public devTaxSell = 1;
    uint256 public marketingTaxSell = 4;
    uint256 public communityTaxSell = 3;
    uint256 public redistributeTaxSell = 7;

    uint256 public maxTxAmount = 30_000_000 * 10**18 + 1;
    uint256 public maxWalletAmount = 30_000_000 * 10**18 + 1;

    // REDISTRIBUTION
    address public token = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        if(isExcluded[msg.sender] || isExcluded[tx.origin] || inTax) {
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                require(!paused(), "DOGGGA: Transfers paused");
                require(isExcluded[recipient] || recipient == address(_UniswapV2Pair)|| balanceOf(recipient) + amount <= maxWalletAmount, "DOGGGA: Max wallet amount");
                require(!blacklistStatus || (!isBlacklisted[sender] && !isBlacklisted[recipient]), "DOGGGA: Blacklisted");
                require(!whitelistStatus || (isWhitelisted[sender] && isWhitelisted[recipient]), "DOGGGA: Not Whitelisted");

                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    if(sender == address(_UniswapV2Pair)) {
                        require(block.number >= nextBuyBlock[recipient], "DOGGGA: Cooldown");
                        require(amount <= maxTxAmount, "DOGGGA: Max tx amount");
            ...
            if(BPStatus) {
                if(sender == address(_UniswapV2Pair) && !isExcludedFromBotProtection[recipient]) {
                    require(!recipient.isContract(), "DOGGGA: Bot Protection");
                } else if(recipient == address(_UniswapV2Pair) && !isExcludedFromBotProtection[sender]) {
                    require(!sender.isContract(), "DOGGGA: Bot Protection");
                }
            }
    ...
    function setToken(address newValue) external onlyRole(OWNER_ROLE) {
        require(newValue != token, "DOGGGA: Value already set to that option");

        token = newValue;
    }

    function setDenominator(uint256 newValue) external onlyRole(OWNER_ROLE) {
        require(newValue != denominator, "DOGGGA: Value already set to that option");

        denominator = newValue;
    }

    function setMaxTxAmount(uint256 newValue) external onlyRole(OWNER_ROLE) {
        require(newValue != maxTxAmount, "DOGGGA: Value already set to that option");

        maxTxAmount = newValue;
    }

    function setMaxWalletAmount(uint256 newValue) external onlyRole(OWNER_ROLE) {
        require(newValue != maxWalletAmount, "DOGGGA: Value already set to that option");
    ...
    function setExcluded(address account, bool newValue) external onlyRole(OWNER_ROLE) {
        require(newValue != isExcluded[account], "DOGGGA: Value already set to that option");
    ...
    function setExcludedFromBotProtection(address account, bool newValue) external onlyRole(OWNER_ROLE) {
        require(newValue != isExcludedFromBotProtection[account], "DOGGGA: Value already set to that option");
    ...
    function massSetExcluded(address[] memory accounts, bool newValue) external onlyRole(OWNER_ROLE) {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcluded[accounts[i]], "DOGGGA: Value already set to that option");
    ...
    function massSetExcludedFromBotProtection(address[] memory accounts, bool newValue) external onlyRole(OWNER_ROLE) {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcludedFromBotProtection[accounts[i]], "DOGGGA: Value already set to that option");
    ...
    function massSetBlacklisted(address[] memory accounts, bool newValue) external onlyRole(OWNER_ROLE) {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isBlacklisted[accounts[i]], "DOGGGA: Value already set to that option");
    ...
    function massSetWhitelisted(address[] memory accounts, bool newValue) external onlyRole(OWNER_ROLE) {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isWhitelisted[accounts[i]], "DOGGGA: Value already set to that option");
    ...
    function setTaxStatus(bool newValue) external onlyRole(OWNER_ROLE) {
        require(taxStatus != newValue, "DOGGGA: Value already set to that option");

        taxStatus = newValue;
    }

    function setBotProtectionStatus(bool newValue) external onlyRole(OWNER_ROLE) {
        require(BPStatus != newValue, "DOGGGA: Value already set to that option");

        BPStatus = newValue;
    }

    function withdrawETH(address to, uint256 value) external onlyRole(OWNER_ROLE) {
        require(address(this).balance >= value, "DOGGGA: Insufficient ETH balance");

        (bool success,) = to.call{value: value}("");
        require(success, "DOGGGA: Transfer failed");
    }

    function withdrawTokens(address tokenAddress, address to, uint256 value) external onlyRole(OWNER_ROLE) {
        require(IERC20(tokenAddress).balanceOf(address(this)) >= value, "DOGGGA: Insufficient token balance");

        try IERC20(tokenAddress).transfer(to, value) {} catch {
            revert("DOGGGA: Transfer failed");
        }
    }
