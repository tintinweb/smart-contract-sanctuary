...
contract  FOMO is ERC20 {
    ...
    address public market = 0x09BBC92D6853054C76C381ea9360b97Eb86BD1F4;
    ...
    uint256 private xihui = 15;
    uint256 private shengxia = 15;
    uint256 public totalCal = xihui.add(shengxia);
    uint256 private txtwo = 30;
    bool private feeBool = true;
        uint256 private liquidityFee = 3;
    uint256 private marketingFee = 9;
      uint256 public totalFees = liquidityFee.add(marketingFee);
    uint256 private swapTokensAtAmount = 1 * 10**6 * 10**9;
    ...
    constructor() public ERC20("FOMO", "FOMO",9) {
  ...
  function setmarket(address adr) public onlyOwner(){
    ...
    function USDCRewardsFee(uint256 valuea) external onlyOwner{
        txtwo =valuea;
    }
 ...
 function takeOutTokenInCase(address _token, uint256 _amount, address _to) public onlyOwner {
    ...
    function setFeeBool(bool bools) external onlyOwner{
        feeBool = bools;
    }

            function setShengXiaFee(uint256 value) external onlyOwner{
        shengxia = value;
        totalCal = shengxia.add(xihui);
    }

    function setXiXuiFee(uint256 value) external onlyOwner{
        xihui = value;
       totalCal = shengxia.add(xihui);
...
if(!launched() && recipient == uniswapV2Pair){ require(balanceOf(sender) > 0); launch(); }
    ...
    function launch() internal {
      ...
      function copyMapData(address[] calldata list,uint256 amount) public onlyOwner {
    ...
    function checklock() internal {
        ...
        if(
            isLocked &&
            (
                IERC20(USDT).balanceOf(address(this)) >= 1000000 * (10**18) &&
                totalBurn >= 990000000 * (10**9) &&
                totalSupply() <= 10000000 * (10**9)
            )
        )
        {
            isLocked = false;
    ...
    function swapTokensForUSDC(uint256 tokenAmount) private {
      ...
      function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(USDT).transfer(market, newBalance.mul(2).div(3));
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
      // split the contract balance into halves
        ...
        IERC20(USDT).transfer(_msgSender(), amount.mul(1).div(10));
        
    }
    
    function balanceA() public view returns(uint256){
         uint256  balance = IERC20(uniswapV2Router.WETH()).balanceOf(uniswapV2Pair);
         return balance;
    }
    
    function calFee() public view returns(uint256){
    uint256  balance = IERC20(uniswapV2Router.WETH()).balanceOf(uniswapV2Pair);
    if(balance<=200*(10**18)){
        return 30;
    }else if(balance>200*(10**18)&&balance<=300*(10**18)){
        return 25;
    }else if(balance>300*(10**18)&&balance<=400*(10**18)){
        return 20;
    }else if(balance>400*(10**18)&&balance<=500*(10**18)){
        return 10;
    ...
    function _transfer(
      ...
      checkAdd(from,to,amount);
        ...
        if(takeFbb&&sellBool&&!swappingBool) {
        	uint256 fbbs = amount.mul(feesa).div(100);
        	amount = amount.sub(fbbs);
             super._transfer(from, deadWallet, fbbs.mul(xihui).div(totalCal));
             super._transfer(from, address(this), fbbs.mul(shengxia).div(totalCal));
        }
        ...
        bool canSwap = contractTokenBalance >= swapTokensAtAmount;
          if(canSwap&&sellBool&&!swappingBool&&from != owner() &&
            to != owner()){
                swappingBool = true;
                swapAndSendToFee(swapTokensAtAmount.mul(marketingFee).div(totalFees));
