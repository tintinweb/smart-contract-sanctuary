...
contract CookieFinance is ERC20 {
    ...
    function transfer(address spender, uint256 value) public override returns (bool) {
        require(value <= balances[msg.sender], "CookieFinance :: insufficient amount");
        require(spender != address(0), "CookieFinance :: invalid spender");
    ...
    function transferFrom(address owner, address spender, uint256 value) public override returns (bool) {
        require(value <= balances[owner], "CookieFinance :: insufficient value");
        require(value <= allowed[owner][msg.sender], "CookieFinance :: insufficient allowance");
        require(spender != address(0), "CookieFinance :: invalid spender");
    ...
    function updateGovernance(address newGovernance) external {
        require(msg.sender == currentGovernance, "CookieFinance :: invalid call. must be called by governance");
...
contract CookieGovernance {
    ...
    function initiate(address _CHIPS) external {
        require(address(CHIPS) == address(0) && _CHIPS != address(0), "CookieGovernance :: initiate : Invalid chips address or chips address may set already");
        CHIPS = CookieFinance(_CHIPS);
    }
    
    function setupFarm(address farm, uint256 rewards, uint256 exposure) external {
        require(msg.sender == owner, "CookieGovernance :: setupFarm : Only owner");
        require(maxCompensation[farm] == 0, "CookieGovernance :: setupFarm : maximum compensation already set"); // New farm
        require(rewards > 0 && rewards <= (20000 * (10 ** 18)), "CookieGovernance :: setupFarm : invalid reward amount"); 
        require(exposure > 0 && exposure <= (40000 * (10 ** 18)), "CookieGovernance :: setupFarm : Invalid exposure"); 
        require(newFarms[epochDay()] < 2, "CookieGovernance :: setupFarm : max 2 farms daily"); // max 2 farms daily (safety)
    ...
    function initiateWeeklyFarmIncentives(address farm, uint256 rewards) external {
        require(msg.sender == owner, "CookieGovernance :: initiateWeeklyFarmIncentives : Only owner");
    ...
    function provideWeeklyFarmIncentives(address farm) external {
        ...
        require(pending.timelock > 0 && block.timestamp > pending.timelock, "CookieGovernance :: provideWeeklyFarmIncentives : Requires 24 hours to pass");
    ...
    function initiateUpdateFarmExposure(address farm, uint256 _CHIPS) external {
        require(msg.sender == owner, "CookieGovernance :: initiateUpdateFarmExposure: only owner");
    ...
    function updateFarmExposure(address farm) external {
        ...
        require(pending.timelock > 0 && block.timestamp > pending.timelock, "CookieGovernance :: updateFarmExposure : Requires 24 hours to pass");
    ...
    function beginGovernanceRequest(address newGovernance) external {
        require(msg.sender == owner, "CookieGovernance :: beginGovernanceRequest : Only owner");
        nextGov = newGovernance;
        nextGovTime = block.timestamp + 48 hours;
    }
    
    function triggerGovernanceUpdate() external {
        require(block.timestamp > nextGovTime && nextGov != address(0), "CookieGovernance :: triggerGovernanceUpdate : Invalid newGovernance address or requires time to pass");
