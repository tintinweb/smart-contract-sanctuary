...
contract GodOfWar is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 125000 * 10**12 * uint256(_decimals);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    address[] private _excluded;
    address public _marketingAndTeam = 0x7C9491b23705c7745aa4732cd4Ff2F62E1c36846;
    address public _gameFundAddress = 0x042c4291e8b1ad176016EbFEb3114d4259899A31;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        */
        ...
        if (takeFee){
            ...
            _tOwned[_gameFundAddress] = _tOwned[_gameFundAddress].add(calculateGameFundFee(amount));
            _rOwned[_gameFundAddress] = _rOwned[_gameFundAddress].add(calculateGameFundFee(amount).mul(_getRate()));

            emit Transfer(from, _burn, calculateBurnFee(amount));
            emit Transfer(from, _marketingAndTeam, calculateMarketingAndTeamFee(amount));
            emit Transfer(from, _gameFundAddress, calculateGameFundFee(amount));
        }
    }
