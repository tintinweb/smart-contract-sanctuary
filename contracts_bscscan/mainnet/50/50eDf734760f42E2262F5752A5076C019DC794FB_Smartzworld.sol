...
contract ExtendedERC20 is Context, IERC20, Ownable, Pausable {
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual whenNotPaused {
        ...
        ValuesFromAmount memory values = _getValues(amount, !takeFee, selling, buying);
    ...
    function _afterTokenTransfer(ValuesFromAmount memory values, bool selling, bool buying) internal virtual {
        ...
        if (selling) {
            if (_marketingRewardEnabled) {
                sendFeeToAddress(marketingAddress, values.tMarketingFee);
            }

            if (_ownerRewardEnabled) {
                sendFeeToAddress(owner(), values.tOwnerFee);
            }
...
contract Smartzworld is ExtendedERC20 {
    ...
    uint256 private _tokenSupply = 500000; // 0.5M
    uint256 private _maxSupply = 1000000000; // 1B

    uint32 private _taxSellMarketing = 3;
    ...
    uint32 private _taxTransferMarketing = 3;
    ...
    address private _marketingAddress = 0x578a0a00D5390b8E04cb4eD5F7C89821032828bF;
    address private _liquidityWallet = 0x578a0a00D5390b8E04cb4eD5F7C89821032828bF;
