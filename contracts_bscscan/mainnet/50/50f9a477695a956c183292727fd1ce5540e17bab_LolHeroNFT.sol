...
contract LolHeroNFT is ERC721Enumerable, LolAuth {
    struct LevelUpPrice {
        uint timo;
        uint lolHero;
    }
    ...
    address public timoAddress = 0x580dE58c1BD593A43DaDcF0A739d504621817c05;
    IBEP20 private TimoToken;
    address public lolHeroAddress = 0x47cC5334F65611EA6Be9e933C49485c88C17F5F0;
    IBEP20 private LolHeroToken;
    address public reflectAddress;

    // Base URI
    string private _baseUriExtended = "https://www.lolgamefi.com/api/hero/v1/";

    constructor() ERC721("Lol Hero 3D NFT", "LolHero3DNFT") LolAuth(msg.sender) {
        TimoToken = IBEP20(timoAddress);
        LolHeroToken = IBEP20(lolHeroAddress);
        ...
        //Pack Genesis Heroes
        addPack(10000 * 10 ** 18, 1, false, 0, timoAddress);
        addPack(28500 * 10 ** 18, 3, false, 0, timoAddress);
        addPack(45000 * 10 ** 18, 5, false, 0, timoAddress);

        //Pack Born Heroes
        addPack(1000000 * 10 ** 18, 1, false, 1, timoAddress);
    ...
    function levelUp(uint heroId) external {
        ...
        require(
            (
                levelData.timo == 0 ||
                (levelData.timo > 0 && TimoToken.balanceOf(msg.sender) >= levelData.timo)
            ) &&
            (
                levelData.lolHero == 0 ||
                (levelData.lolHero > 0 && LolHeroToken.balanceOf(msg.sender) >= levelData.lolHero)
            )
        ,"Not enough fee");

        if(levelData.timo > 0){
            require(TimoToken.transferFrom(msg.sender, address(this), levelData.timo), "Timo transfer failed");
        }

        if(levelData.lolHero > 0){
            require(LolHeroToken.transferFrom(msg.sender, address(this), levelData.lolHero), "LolHero transfer failed");
        }

        if(newLevel == 1){
            hero.level = newLevel;

            try ILolHeroReflect(reflectAddress).registerNewMint( heroId ) {} catch {}
    ...
    function setTimoAddress(address _newAdr) external {
        authorizedFor(Permission.AdjustVariables);

        timoAddress = _newAdr;
        TimoToken = IBEP20(_newAdr);
    }

    function setLolHeroAddress(address _newAdr) external {
        authorizedFor(Permission.AdjustVariables);

        lolHeroAddress = _newAdr;
        LolHeroToken = IBEP20(_newAdr);
    }
    ...
    function addLevelUpPrice(uint _timo, uint _lolHero) public{
        authorizedFor(Permission.ManageAttributes);

        levelUpPrices.push(LevelUpPrice(_timo, _lolHero));
    }

    function editLevelUpPrice(uint8 level, uint _timo, uint _lolHero) public {
        authorizedFor(Permission.ManageAttributes);

        require(level < levelUpPrices.length,"Invalid level");

        levelUpPrices[level].timo = _timo;
        levelUpPrices[level].lolHero = _lolHero;
    }
    ...
    function adminKillHero(uint heroId) external {
        ...
        try ILolHeroReflect(reflectAddress).updateBurnedToken( heroId ) {} catch {}
