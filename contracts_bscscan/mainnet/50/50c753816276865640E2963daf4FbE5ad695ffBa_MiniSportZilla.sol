...
contract MiniSportZilla is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    constructor(address _router ,address _owner,address _marketingWallet, address _developmentWallet, address _pinkAntiBot) public {
    ...
    function calculateFee(uint256 feeIndex, uint256 amount) internal returns(uint256, uint256) {
        ...
        return (taxFee, liqFee.add(marketingFee).add(developmentFee));
    }
    ...
    function swap() private lockTheSwap {
        uint256 totalFee = _liqFeeCollected.add(_developmentFeeCollected).add(_marketingFeeCollected);
