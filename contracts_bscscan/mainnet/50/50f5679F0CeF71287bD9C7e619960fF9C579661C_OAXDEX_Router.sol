...
// File contracts/interfaces/IOAXDEX_Router.sol


pragma solidity =0.6.11;

interface IOAXDEX_Router {
...
// File contracts/OAXDEX_Router.sol


pragma solidity =0.6.11;
contract OAXDEX_Router is IOAXDEX_Router {
    ...
    function _addLiquidity(
        ...
        if (IOAXDEX_Factory(factory).getPair(tokenA, tokenB) == address(0)) {
            IOAXDEX_Factory(factory).createPair(tokenA, tokenB);
        }
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'OAXDEX_Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'OAXDEX_Router: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        liquidity = IOAXDEX_Pair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
    ...
    function removeLiquidity(
        ...
        address pair = pairFor(tokenA, tokenB);
        IOAXDEX_ERC20(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IOAXDEX_Pair(pair).burn(to);
        (address token0,) = sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'OAXDEX_Router: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'OAXDEX_Router: INSUFFICIENT_B_AMOUNT');
    }
    function removeLiquidityETH(
        address token,
        uint liquidity,
    ...
    function removeLiquidityWithPermit(
        ...
        IOAXDEX_ERC20(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IOAXDEX_Pair(pairFor(input, output)).swap(
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        // uint feeDeducted = uint(FEE_BASE).sub(IOAXDEX_Factory(factory).tradeFee());
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = sortTokens(input, output);
            IOAXDEX_Pair pair = IOAXDEX_Pair(pairFor(input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'OAXDEX_Library: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'OAXDEX_Library: ZERO_ADDRESS');
    }
    ...
    function pairFor(address tokenA, address tokenB) internal view returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'03da9b8a4f5afd3c213fa0c66ce4ee297243ac54323b7abe5fe7245167cfde95' // init code hash
            ))));
    }
    ...
    function getReserves(address tokenA, address tokenB) public view override returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IOAXDEX_Pair(pairFor(tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) public pure override returns (uint amountB) {
        require(amountA > 0, 'OAXDEX_Library: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'OAXDEX_Library: INSUFFICIENT_LIQUIDITY');
    ...
    {
        return IOAXDEX_Pair(pairFor(tokenIn, tokenOut)).getAmountOut(tokenIn, amountIn);
    }
    ...
    {
        return IOAXDEX_Pair(pairFor(tokenIn, tokenOut)).getAmountIn(tokenOut, amountOut);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        require(path.length >= 2, 'OAXDEX_Library: INVALID_PATH');
