...
contract WarLords is BEP20, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    ...
    address public poolPlayToEarn;
    address public poolAirdropAddress;
    address public poolMarketingAddress;

    mapping(address => bool) private _excludeFromFee;
    bool isSwapping = false;
    constructor(string memory _name, string memory _symbol, address _poolPlayToEarn) BEP20(_name, _symbol) {
        _mint(_msgSender(), maxSupply);
        poolPlayToEarn = _poolPlayToEarn;
    ...
    function _transfer( address sender, address recipient, uint256 amount ) internal virtual override {
        ...
        if(
            ...
            && sender != poolPlayToEarn && recipient != poolPlayToEarn
            && isSwapping == false
            ...
            if(_rewardFee > 0 && _operationFee > 0){
                ...
                super._transfer(sender, poolPlayToEarn, _rewardFee);
    ...
    function removeExcludeFromFee( address[] memory users) external onlyOwner {
     ...
     function SetDecreaseFee(uint256 operationFee, uint256 rewardFee) external onlyOwner {
        DecreaseFee = operationFee;
        sellRewardFee = rewardFee;
    }
    function setPlayToEarnAddress(address _poolPlayToEarn) external onlyOwner{
        poolPlayToEarn = _poolPlayToEarn;
    }
    function setAirdropAddress(address _poolAirdropAddress) external onlyOwner{
        poolAirdropAddress = _poolAirdropAddress;
    }
    function setMarketingAddress(address _poolMarketingAddress) external onlyOwner{
