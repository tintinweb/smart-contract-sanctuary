...
contract IBeePlace is BEP20, MinterRole {
    ...
    // 50 => 50% of TransferTaxRate
    uint16 public devRate = 50;

    // dev Fee Wallet address
    address public devAddress;

    // marketing Fee Wallet address
    ...
    event UpdateMarkAddress(address indexed markAddress);
    event UpdateTaxRate(uint16 taxRate);
    event UpdateDevRate(uint16 devRate);
    ...
    /**z
     * @notice Constructs the BP token contract.
     */
    constructor(address _devAddress, address _markAddress) public BEP20("IBeePlace", "IBPT") {
        ...
        require(_markAddress != address(0), "marketing address is zero");
    ...
    /// @dev overrides transfer function to meet tokenomics of BP
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        // Pre-flight checks
        require(amount > 0, "Transfer amount must be greater than zero");

        if (sender == owner() || recipient == owner() || transferTaxRate == 0 || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            super._transfer(sender, recipient, amount);
        } else {
            require(tradingOpen == true, "Trading is not yet open.");

            uint256 sendAmount = amount;
            if((automatedMarketMakerPairs[sender] || automatedMarketMakerPairs[recipient]) && transferTaxRate > 0) {
            // default tax is 10% of trading
                uint256 taxAmount = amount.mul(transferTaxRate).div(10000);
                uint256 devFeeAmount = taxAmount.mul(devRate).div(100);
                uint256 markFeeAmount = taxAmount.sub(devFeeAmount);
                require(taxAmount == devFeeAmount.add(markFeeAmount), "BP::transfer: DevFeeAmount or MarkFeeAmount value invalid");

                sendAmount = amount.sub(taxAmount);
                require(amount == sendAmount.add(taxAmount), "BP::transfer: Tax value invalid");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "BP::transferOperator: new operator is the zero address");
    ...
    function updateDevAddress(address _devAddress) public onlyOperator {
        require(_devAddress != address(0), "BP:: dev address is zero");
     ...
     * Can only be called by the current operator.
     */
    function updateMarkAddress(address _markAddress) public onlyOperator {
        require(_markAddress != address(0), "BP:: marekting address is zero");
        emit UpdateMarkAddress(_markAddress);
        markAddress = _markAddress;
    }

    function updateTaxRate(uint16 _taxRate) public onlyOperator {
        require(_taxRate <= 10000, "BP:: tax rate cannot exceed 100%");
        emit UpdateTaxRate(_taxRate);
        transferTaxRate = _taxRate;
    }

    function updateDevRate(uint16 _devRate) public onlyOperator {
        require(_devRate <= 100, "BP:: dev rate cannot exceed 100%");
        emit UpdateDevRate(_devRate);
        devRate = _devRate;
    }
}