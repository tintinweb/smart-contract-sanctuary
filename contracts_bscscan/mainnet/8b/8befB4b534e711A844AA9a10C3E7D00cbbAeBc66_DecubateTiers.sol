...
contract DecubateTiers is Ownable {
  ...
  IDecubateMasterChef private legacyStakingContract; //Legacy staking contract instance
  ...
  constructor(
    IDecubateMasterChef _legacyStakingContract,
   ...
   *
   */
  function setLegacyStakingContract(IDecubateMasterChef _stakingContract)
    external
    onlyOwner
  {
  ...
  function getTotalDeposit(address addr) public view returns (uint256 amount) {
    uint256 legacy_len = legacyStakingContract.poolLength();
    uint256 tempAmt;

    for (uint256 i = 0; i < legacy_len; i++) {
