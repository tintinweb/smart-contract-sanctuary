...
contract MABNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MABNBDividendTracker public dividendTracker;
    ...
    address payable public marketingWallet = 0x44511cDf22DFbB531Fe115D9E014be6df66AE663;

    uint256 public swapTokensAtAmount = 2000000 * (10**18);
    uint256 public maxSellTransactionAmount = 1000000000 * (10**18);

    uint256 public BNBRewardsFee = 12;
    ...
    constructor() ERC20("BNB MASTERY", "MABNB") {
        
        totalFees = BNBRewardsFee.add(liquidityFee);


    	dividendTracker = new MABNBDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MABNB: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MABNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MABNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MABNB: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1000000, "MABNB: gasForProcessing must be between 200,000 and 1,000,000");
        require(newValue != gasForProcessing, "MABNB: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MABNB: This account cannot send tokens until trading is enabled");
        }
...
contract MABNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MABNB_Dividend_Tracker", "MABNB_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "MABNB_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public override {
        require(false, "MABNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MABNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 86400, "MABNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MABNB_Dividend_Tracker: Cannot update claimWait to same value");
