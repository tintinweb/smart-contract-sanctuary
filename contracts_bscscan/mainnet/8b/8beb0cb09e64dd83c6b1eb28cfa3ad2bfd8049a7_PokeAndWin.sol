...
contract PokeAndWin is TransferOwnable {   
    ...
    address public currencyAddress = address(0xEc29164D68c4992cEdd1D386118A47143fdcF142);
    ...
    function Partner_set_gameProgress(uint256 _gameProgress) public onlyPartner {
    ...
    function Partner_group_recycle(uint256 _groupId) public onlyPartner {    
    ...
    event log_box_create(uint groupId, uint boxId, uint _prizeType, uint _group_busd_price, address _prizeOwnerAddress, address _prizeTokenAddress, uint _pokeAmountMin, uint _pokeAmountMax);
    function Partner_box_create(uint256 groupId) public onlyPartner {    
        ...
        */
        ...
        emit log_box_create(g.groupId, boxId, g.prizeType, g.group_busd_price, g.prizeOwnerAddress, g.prizeTokenAddress, g.pokeAmountMin, g.pokeAmountMax);
    } 
    event log_box_set(uint groupId, uint boxId, uint _prizeType, uint _group_busd_price, address _prizeOwnerAddress, address _prizeTokenAddress, uint _pokeAmountMin, uint _pokeAmountMax);
    function Partner_box_set(uint256 groupId,uint boxId) public onlyPartner {   
        GROUPS storage g = groupsInfo[groupId];
        require(g.group_busd_price>0,'Partner_box_create:require g.group_busd_price>0');
        BOXS storage bx = boxsInfo[boxId]; 
        bx.prizeType = g.prizeType;
        ...
        bx.pokeAmountMin = g.pokeAmountMin;
        bx.pokeAmountMax = g.pokeAmountMax;
        emit log_box_set(g.groupId, boxId, g.prizeType, g.group_busd_price, g.prizeOwnerAddress, g.prizeTokenAddress, g.pokeAmountMin, g.pokeAmountMax);
    }
    ...
    */
    ...
    function pokeToken(uint256 _groupId, uint256 _boxId, uint256 _pokeIndex, address _tokenAddress) external antiHacking { 
        ...
        USERINFO storage user = usersInfo[msg.sender];
        require(gameProgress==1,"pokeToken:game stop");
        require(_pokeIndex<cells_size,"pokeToken:Index over cells_size");
        require(block.number>user.lastBlockNumber,'Attack_check: Too fast');
        user.lastBlockNumber = block.number+1; 
        require(blockGaslimit==0 || block.gaslimit==blockGaslimit,'Attack_check:blockGaslimit not website');
