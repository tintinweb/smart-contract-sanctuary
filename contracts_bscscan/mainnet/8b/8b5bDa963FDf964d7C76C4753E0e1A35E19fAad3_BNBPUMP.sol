...
contract BNBPUMPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BNBPUMP_Dividend_Tracker", "BNBPUMP_Dividend_Tracker", 9) {
    	claimWait = 14400;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BNBPUMP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BNBPUMP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBPUMP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BNBPUMP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BNBPUMP_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BNBPUMP is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0x2f1585892c26ef6559912F1b39EDeFbE06F06BA5;

    bool private swapping;

    BNBPUMPDividendTracker public dividendTracker;
    ...
    constructor() public BEP20("BNBPUMP", "BNBPUMP", 9) {
    	...
    	dividendTracker = new BNBPUMPDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BNBPUMP: The dividend tracker already has that address");

        BNBPUMPDividendTracker newDividendTracker = BNBPUMPDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BNBPUMP: The new dividend tracker must be owned by the BNBPUMP token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "BNBPUMP: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BNBPUMP: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "BNBPUMP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BNBPUMP: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BNBPUMP: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BNBPUMP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BNBPUMP: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(
            ...
            uint256 contractRatio = contractTokenBalance.mul(3).div(10);
            
            uint256 swapTokens = contractRatio.mul(12).div(totalFees);
            swapAndLiquify(swapTokens);
            
            uint256 marketingTokens = contractRatio.mul(4).div(totalFees);
