...
// File: contracts/LITcoinDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable BUSD = address(0x4338665CBB7B2485A8855A139b75D5e34AB0DB94);
...
contract LITcoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0x4338665CBB7B2485A8855A139b75D5e34AB0DB94);
    address private _marketingWallet = address(0x5B69DA8B17A231843A578FaCfF4f42Aa43C6021A);
    address private _devWallet = address(0x706222885CA21C528a923fCd8DAaD77b744632a2);

    bool private swapping;
    bool public airdrop;

    LITcoinDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet;
    address public devWallet = _devWallet;

    uint256 public maxBuyTranscationAmount = 1000000000000000 * (10**18); // No limit
    ...
    constructor() public ERC20("LITcoin", "LITcoin") {
    	...
    	dividendTracker = new LITcoinDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LITcoin: The dividend tracker already has that address");

        LITcoinDividendTracker newDividendTracker = LITcoinDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LITcoin: The new dividend tracker must be owned by the LITcoin token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LITcoin: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LITcoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LITcoin: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "LITcoin: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LITcoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LITcoin: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "LITcoin: This account cannot send tokens until trading is enabled");
        }
...
contract LITcoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("LITcoin_Dividend_Tracker", "LITcoin_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "LITcoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "LITcoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LITcoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LITcoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LITcoin_Dividend_Tracker: Cannot update claimWait to same value");
