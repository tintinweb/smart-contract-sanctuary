...
/**
DOGE PROF is a hyper-deflationary token, which rewards investors for holding tokens, 
while also incorporating the Buyback approach currently present in the stock market.

DOGE PROF mechanism will charge a handling fee for each transaction at different stages 
(respectively: 15%.11%.7%.3%.0%) and maintain it in accordance with the contract. Whenever
a purchase or sale occurs, a small part of the repurchase amount will be used to automatically
purchase from the liquidity pool and burn these tokens accordingly.

DOGE PROF  is a deflationary token designed to become more scarce over time. All holders of 
DOGE PROF will earn more DOGE PROF that is automatically sent to your wallet by simply holding 
DOGE PROF coins in your wallet. Watch the amount of DOGE PROF grow in your wallet as DOGE PROF 
holders automatically receive a (3%.2%.1%.0%) fee from every transaction that happens on the 
DOGE PROF network. The community receives more DOGE PROF  from the fees generated each transaction.
*/
...
contract DOGEPROF is Context, IERC20, Ownable {
    ...
    function getHealthLevel() private view returns(uint256) {
        ...
        if (bal <= 50000 * 10**18) {
            return 4;
        } else if (bal <= 60000 * 10**18) {
            return 3;
        } else if (bal <= 360000 * 10**18) {
            return 2;
        } else if (bal <= 2160000 * 10**18) {
            return 1;
        }
        return 0;
    }
