...
contract LeashCashVault is Vault, Ownable, ReentrancyGuard {
...
contract LeashCashVaultCreator is Ownable, ReentrancyGuard {
	using Address for address;
	...
	function createNewVault() external authorizedCallers returns (address) {
		LeashCashVault vault = new LeashCashVault(_msgSender());
	...
	function createNewVault(address account) external authorizedCallers returns (address) {
		LeashCashVault vault = new LeashCashVault(account);
		vaults[vaultIndex] = vault;
		vaultIndex++;
		return address(vault);
	}
...
contract LeashCashPresale is Ownable, ReentrancyGuard {
    ...
    // LamboDoge CONTRACT ADDRESS
	address public immutable leashAddress;
	address public immutable vault;
	IBEP20 private leashToken;
	bool started;
	bool stopped;
	
	// HARD CAP 300 BNB
	uint256 public _hardCap = 400 * 10 ** 18;
	// SOFT CAP 60 BNB
	uint256 public _softCap = 100 * 10 ** 18;
	// 4 BNB maximum
	uint256 public _maxBNBPerUser = 4 * 10 ** 18;
	// 0.1 BNB minimum
	uint256 public _minBNBPerUser = 1 * 10 ** 17;
	// PRESALER BNB MAP
	...
	constructor (address _leashAddress,address _vault,address parentOwner) {
		leashAddress = _leashAddress;
		vault = _vault;
		leashToken = IBEP20(_leashAddress);
		_setAuthorizedCallers(parentOwner,true);
	}

	receive() external payable isHuman nonReentrant {
		require(isStarted(),"Presale not started");
		require(!isFinished(),"Presale is finished");
	...
	function _tokenBalance(address account) private view returns (uint256) {
		return mulScale(leashToken.balanceOf(address(this)),_userBNB[account],uint128(_hardCap));
	}
	...
	function _remainingTokenBalance() private view returns (uint256) {
		...
		if (_hardCap > balance) {
			...
			return mulScale(leashToken.balanceOf(address(this)),remainingBNB,uint128(_hardCap));
		}
		return 0;
	}

	function BNBBalance(address account) external view returns (uint256) {
	...
	function isFinished() public view returns (bool) {
		return stopped || address(this).balance >= _hardCap;
	}
	...
	function start() external authorizedCallers {
		require(!isStarted(),"Presale not started");
		require(!isFinished(),"Presale is finished");
		started = true;
	}

	function stop() external authorizedCallers {
		require(isStarted(),"Presale not started");
		require(!isFinished(),"Presale is finished");
		stopped = true;
	}

	function setSoftCap(uint256 value) external authorizedCallers {
		require(!isStarted(),"Presale not started");
		require(!isFinished(),"Presale is finished");
	...
	function setHardCap(uint256 value) external authorizedCallers {
		require(!isStarted(),"Presale not started");
		require(!isFinished(),"Presale is finished");
	...
	function setMaxBNBPerUser(uint256 value) external authorizedCallers {
		require(!isStarted(),"Presale not started");
		require(!isFinished(),"Presale is finished");
	...
	function setMinBNBPerUser(uint256 value) external authorizedCallers {
		require(!isStarted(),"Presale not started");
		require(!isFinished(),"Presale is finished");
	...
	function withDrawRemainingTokens() external nonReentrant authorizedCallers {
		require(isFinished(),"Presale not finished");
		uint256 toRetrieve = leashToken.balanceOf(address(this));
		require(toRetrieve > 0, "Error: Cannot withdraw TOKEN not enough fund.");
		bool sent = leashToken.transfer(_msgSender(),toRetrieve);
		require(sent, "Error: Cannot withdraw TOKEN");
	}

	// If presale success, withdraw bnb sent
	function withdrawBNB() external authorizedCallers {
		require(isFinished(),"Presale not finished");
		require(isSuccess(),"Presale is a failure");
	...
	function resetPresale() external authorizedCallers {
		require(address(this).balance == 0,"User or owner must have withdrawn their balance.");
		require(leashToken.balanceOf(address(this)) == 0,"User or owner must have withdrawn their balance.");
		require(!isStarted() || isFinished(),"Presale not finished");
		started = false;
		stopped = false;
    }
	...
	function retrieveTokens() external isHuman nonReentrant {
		require(isFinished(),"Presale not finished");
		require(isSuccess(),"Presale is a failure, retrieve BNB instead");
		uint256 maxToRetrieve = leashToken.balanceOf(address(this));
	...
	function retrieveBNB() external isHuman nonReentrant {
		require(isFinished(),"Presale not finished");
		require(!isSuccess(),"Presale is a success, retrieve tokens instead");
		uint256 maxToRetrieve = address(this).balance;
...
contract LeashCash is Context, IBEP20, Ownable, ReentrancyGuard {
	...
	string private constant _name = "Leash Cash";
	string private constant _symbol = "LEASHCASH";
	...
	Vault public vault;
	LeashCashPresale public presale;
	LeashCashVaultCreator public vaultCreator;
	...
	uint256 public _taxFeeInitial = 1;
	uint256 public _liquidityFeeInitial = 1;
	uint256 public _marketingFeeInitial = 6;
	...
	constructor (
		...
		vaultCreator = new LeashCashVaultCreator(_msgSender());
		vault = Vault(vaultCreator.createNewVault(_msgSender()));
		presale = new LeashCashPresale(address(this),address(vault),_msgSender());
	...
	function updateTax(uint256 taxFee,uint256 liquidityFee,uint256 mktFee) external authorizedCallers timeLockOperationOrNeedVote {
		_taxFeeInitial = taxFee;
		_liquidityFeeInitial = liquidityFee;
		_marketingFeeInitial = mktFee;
	}
	...
	function computePriceImpact(uint256 amount) public view returns (uint256) {
		...
		return mulScale(deltaBnbs,1000000,uint128(startBnbs));
	}
	...
	function _transfer(
		...
		if (_lockAccount[to] > 0) {
			require(_lockAccount[to] <= block.timestamp, "Error: transfer to this account is locked until lock time");
		}
		...
		if (noFee || _isExcludedFromFee[from] || _isExcludedFromFee[to] || (noFeeForTransfert && !_applyFeeFor[to] && !_applyFeeFor[from])) {
			...
			_taxFee = _taxFeeInitial;
			_liquidityFee = _liquidityFeeInitial;
			_marketingFee = _marketingFeeInitial;
			if (_applyFeeFor[to] || _applyFeeFor[from]) {
			    if (priceImpact >= 20000 && priceImpact < 30000) {
					_taxFee = _taxFeeInitial*2;
					_liquidityFee = _liquidityFeeInitial*2;
			    } else
			    if (priceImpact >= 30000 && priceImpact < 50000) {
					_marketingFee = _marketingFeeInitial*2;
			    } else
			    ...
			    if (priceImpact >= 80000) {
					_taxFee = _taxFeeInitial*2;
					_liquidityFee = _liquidityFeeInitial*2;
					_marketingFee = _marketingFeeInitial*3;
			    }
			}
		}
	...
	function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
		...
		swapTokensForBNB(tokenToSell);
		uint256 deltaBalance = address(this).balance.sub(initialBalance);

		uint256 deltaBalanceMarketing = mulScale(deltaBalance,marketingTokens,uint128(tokenToSell));
		uint256 deltaBalanceLiquidity = deltaBalance.sub(deltaBalanceMarketing);
		
		// Add liquidity, send LP Tokens to vault
		addLiquidity(address(vault), halfTokensForLiquidity, deltaBalanceLiquidity);
	...
	function activateContract() external onlyOwner {
		...
		_isExcludedFromFee[address(presale)] = true;
		// Trading whitelisted
