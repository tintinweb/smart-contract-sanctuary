...
contract test is ERC20, Ownable {
    ...
    uint256 public ADARewardsSplitValue = 5;
    uint256 public liquiditySplitValue = 2;
    uint256 public marketingSplitValue = 5;
    uint256 public apeSplitValue = 3;
    uint256 public bananaRewardSplitValue = 1;
    uint256 public feeDecimalFactor = 10;
    uint256 public buyingFeesWithDecimals = (ADARewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(apeSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    uint256 public sellingFeesWithDecimals = (ADARewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(apeSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    address public _marketingWalletAddress = 0x4545A8832b205572952aa78c55677f442d1D4322;
    bool public hasLaunched;
    address public liquidityFeeReceiver = 0x4545A8832b205572952aa78c55677f442d1D4322;
    ...
    uint256 previousMaxBuyTransactionAmount;
    uint256 previousMaxWalletToken;
    uint256 previousBuyingFees;
    uint256 previousSellingFees;
    uint256 previousFeeDecimalFactor;
    uint256 APETIMEID;
    uint256 apeCount;
    ...
    function setApeSplitValue(uint256 value) external onlyOwner{
        require(value >= 1 && value <= 1000);
        apeSplitValue = value;
    }
    ...
    function redirectToMarketingWallet(uint256 percentage) public onlyOwner{
  	    require(percentage <= 100 && swapping == false);
  	    swapping = true;
        uint256 tokens = address(this).balance;
  	    tokens = tokens.mul(percentage).div(100);
  	    if(tokens > 0){
            payable(_marketingWalletAddress).transfer(tokens);
  	    }
  	    swapping = false;
  	}
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 marketingTokens = contractTokenBalance.mul(marketingSplitValue).div(ADARewardsSplitValue + liquiditySplitValue + marketingSplitValue + apeSplitValue + bananaRewardSplitValue);
            swapAndSendToFee(marketingTokens);

            uint256 swapTokens = contractTokenBalance.mul(liquiditySplitValue).div(ADARewardsSplitValue + liquiditySplitValue + marketingSplitValue + apeSplitValue + bananaRewardSplitValue);
            swapAndLiquify(swapTokens);
            
            uint256 apePoolTokens = contractTokenBalance.mul(apeSplitValue).div(ADARewardsSplitValue + liquiditySplitValue + marketingSplitValue + apeSplitValue + bananaRewardSplitValue);
            swapTokensForEth(apePoolTokens);
            
            totalBananas += contractTokenBalance.mul(bananaRewardSplitValue).div(ADARewardsSplitValue + liquiditySplitValue + marketingSplitValue + apeSplitValue + bananaRewardSplitValue);

            uint256 sellTokens = contractTokenBalance.mul(ADARewardsSplitValue).div(ADARewardsSplitValue + liquiditySplitValue + marketingSplitValue + apeSplitValue + bananaRewardSplitValue);
        ...
        if(takeFee) {
            ...
            if(address(this).balance >= 1 * (10**18) && isAPETIME == true){
                apeSup = true;
            }else if(isAPETIME == true && nextAPETIMEStartingAt + 5 minutes < block.timestamp){
                APETIMEEnded();
            }
            setATH(currPrice);
            apeTimeMonitoring(currPrice);
            if(autoSetMaxTX == true){
                if(launchTime + 10 seconds < block.timestamp){
            ...
            if(automatedMarketMakerPairs[from]){
                if(launchBlockNumber == block.number || launchTime > block.timestamp - 10 seconds){
                    if(botList[to] == false){
                        botList[to] = true;
                    }
                }
    ...
    function apeTimeMonitoring(uint256 _currPrice) private{
        if(ATH > 0 && _currPrice <= ATH / 2 && nextAPETIMEStartingAt == 0){
    ...
    function APESUPPORT(uint256 tokensBought) private {
       (uint256 tokens, uint256 bnb) = _getReserves();
       if(tokens > 20 && tokensBought < tokens / 20){
           uint256 bnbInPool = address(this).balance;
           uint256 bnbNeeded = bnb / (tokens / tokensBought);
           if(bnbInPool > bnbNeeded){
              buyBackTokens(tokensBought / 2);
           }
       }
    }
    
    function buyBackTokens(uint256 tokensToBuy) private{
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);

        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: tokensToBuy}(
            0,
            path,
    ...
    function swapAndSendDividends(uint256 tokens) private{
        uint256 initialCardanoBalance = IERC20(ADA).balanceOf(address(this));
        swapTokensForADA(tokens);
        uint256 dividends = (IERC20(ADA).balanceOf(address(this))).sub(initialCardanoBalance);
