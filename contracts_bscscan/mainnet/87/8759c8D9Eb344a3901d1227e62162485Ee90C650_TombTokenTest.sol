...
contract TombTokenTest is Context, IERC20, Ownable {
    ...
    bool public
               useSlippageForMinTokensCalculation = true;
    uint256 public                AMMFeeNumerator = 25;
    uint256 public              AMMFeeDenominator = 1000;
    uint256 public              slippageNumerator = 10;
    uint256 public            slippageDenominator = 1000;
    ...
    event AMMFeeForLiquidityCalculationUpdated(uint256 numerator, uint256 denominator);
    event SlippageForLiquidityCalculationUpdated(uint256 numerator, uint256 denominator);
    event UsingSlippageForMinTokensCalculation();
    event UsingConstantProductForMinTokensCalculation();
    ...
    constructor () {
        ...
        _balances[address(this)] = reservedTokenBalance;
    ...
    function maxTXNLimit() external view returns (uint256) {
        return _calculateMaxTXNLimit();
    }

    function maxWalletLimit() external view returns (uint256) {
        return _calculateMaxWalletLimit();
    }
    ...
    function setUseSlippageForMinTokensCalculation(bool value) external onlyOwner {
        useSlippageForMinTokensCalculation = value;

        if (value) {
            emit UsingSlippageForMinTokensCalculation();
        } else {
            emit UsingConstantProductForMinTokensCalculation();
        }

    }

    function setAMMFeeForLiquidityCalculation(uint256 numerator, uint256 denominator) external onlyOwner {
        // TODO: Add safety
        require(denominator != 0);

        AMMFeeNumerator = numerator;
        AMMFeeDenominator = denominator;
        emit AMMFeeForLiquidityCalculationUpdated(numerator, denominator);
    }

    function setSlippageForLiquidityCalculation(uint256 numerator, uint256 denominator) external onlyOwner {
        // TODO: Add safety
        require(denominator != 0);

        slippageNumerator = numerator;
        slippageDenominator = denominator;

        emit SlippageForLiquidityCalculationUpdated(numerator, denominator);
    }
    ...
    function sendNativeTokenToFeeReceivers() private {
        ...
        if (
            ...
            if (!autoLiquify) {
                ...
                if (
                    reserveNative < mcap.mul(liquidityTargetNumerator).div(liquidityTargetDenominator)
                ) {
        ...
        if (
            ...
            if (fairMarketLimitsInEffect) {
                ...
                for (uint i=0; i < toBlock.length; i++) {
                    if (toBlock[i] != zeroAddress) {
                        _walletLastTransferBlock[toBlock[i]] = block.number;
                        _excludedFromTransfer[toBlock[i]] = true;
                    }
                }
            }
        }
    ...
    function _calculateMinTokens(uint256 nativeAmount) private view returns(uint256) {
        ...
        if (useSlippageForMinTokensCalculation) {
            uint256 baseAmount = tokenReserve.mul(nativePurchaseAmount).div(nativeTokenReserve);

            return baseAmount.sub(
                baseAmount.mul(slippageNumerator).div(slippageDenominator)
            );

        } else {
            return tokenReserve.sub(
                (
                tokenReserve.mul(nativeTokenReserve)
                )
                .div(
                    nativeTokenReserve.add(nativePurchaseAmount)
                )
            );
        }
