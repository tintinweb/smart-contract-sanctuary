...
contract test is IBEP20, Auth {
    ...
    mapping(address => bool) isBasic;
    mapping(address => bool) isPro;
    // contracts traded without subscription
    mapping(address => bool) isCaWl;
    // subscription
    mapping(address => uint256) subscriptionExpiryHolder;
    mapping(address => bool) isLifeTime;
    // link main wallet to alt wallet to use priv key
    mapping(address => address) altWalletHolder;
    ...
    //buying fee which moves from 3 to 0
    uint256 public totalFeeDef = 3;
    uint256 public totalFee = totalFeeDef;
    uint256 public minTotalFee = 0;

    // selling moves from 3-8
    uint256 public totalSellFeeDef = totalFeeDef.add(2);
    uint256 public totalSellFee = totalSellFeeDef;
    uint256 public maxTotalSellFee = 6;
    uint256 public minTotalSellFee = 3;
    uint256 feeDenominator = 100;

    // only 0.05% sells/buys move the tax.
    uint256 public taxChangeEffectiveAmount = (_totalSupply * 5) / 10000;
    uint256 public taxChangeEffectiveTrades = 2;
    ...
    uint256 public rollingBuys = 0;
    uint256 public rollingSells = 0;

    //Migration starts from deployment to 7 days, IDO starts 1 day after migration ends.
    uint256 public migrationEndTime = block.timestamp + 3600;
    ...
    function takeFee(
        ...
        //buying
        if (sender == pair) {
            if (amount >= taxChangeEffectiveAmount) {
                rollingBuys.add(1);

                if (
                    rollingSells > taxChangeEffectiveTrades &&
                    totalFee != minTotalFee
                ) {
                    totalFee = totalFeeDef.sub(
                        rollingSells.sub(taxChangeEffectiveTrades)
                    );
                }

                if (rollingBuys > taxChangeEffectiveTrades) {
                    totalSellFee = totalSellFeeDef;
                    if (totalFee < totalFeeDef) totalFee = totalFeeDef;
                }
        ...
        //selling
            ...
            if (amount >= taxChangeEffectiveAmount) {
                ...
                if (
                    ...
                    // sell tax increases in increments of 1
                    totalSellFee = totalSellFeeDef.add(
                        rollingSells.sub(taxChangeEffectiveTrades)
                    );
                }

                if (
                    rollingBuys > (taxChangeEffectiveTrades * 2) &&
                    totalSellFee > minTotalSellFee
                ) {
                    totalSellFee = totalSellFeeDef.sub(
                        rollingBuys.sub(taxChangeEffectiveTrades)
                    );
                }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountDev = balanceOfSwapAndSubscription.mul(devAmount).div(
            totalAmountDivider
        );

        bUSDI.transfer(marketingAmountReceiver, amountMarketing);
        bUSDI.transfer(marketingAmountReceiver, amountDev);
    }
    ...
    function getIsBasic(address holder) external view returns (bool) {
        return isBasic[holder];
    }
    ...
    function getIsPro(address holder) external view returns (bool) {
        return isPro[holder];
    }
    ...
    function getIsCaWl(address holder) external view returns (bool) {
        return isCaWl[holder];
    }
    ...
    /**
     * Check if an address is lifetime subscribed
     */
    function getIsLifeTime(address holder) external view returns (bool) {
        return isLifeTime[holder];
    }

    /**
     * For monthly subscription users, the expiry date is useful to display in the dapp
     */
    function getExpiryDate(address holder) external view returns (uint256) {
        return subscriptionExpiryHolder[holder];
    }
    ...
    function migrateToNewToken(uint256 _inputAmt, uint256 mode) external {
        ...
        if (mode == 1) {
            ...
            oldToken.transferFrom(msg.sender, address(this), _inputAmt);
            emit Transfer(address(this), msg.sender, _inputAmt.div(10));
        } else {
            require(
                oldTokenAdaMini.balanceOf(msg.sender) >= _inputAmt,
                "You do not have enough balance to do the migration."
            );
            oldTokenAdaMini.transferFrom(msg.sender, address(this), _inputAmt);
            emit Transfer(address(this), msg.sender, _inputAmt.div(10));
        }
    }
     ...
     */
    function swapOldTokensForMarketing() external onlyOwner {
        require(
            block.timestamp > migrationEndTime,
