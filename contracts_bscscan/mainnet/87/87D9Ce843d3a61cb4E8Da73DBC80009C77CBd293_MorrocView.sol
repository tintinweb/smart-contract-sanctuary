...
contract MorrocView {
    ...
    function pendingReward(IMorroc _morroc) public view returns (uint256) {
        IByalan byalan = _morroc.izlude().byalan();

        if (address(_morroc.izlude()) == 0xd7e5E527df757cD75cEB1d26a34D90A171e600D2) {
            // COUPON-BNB
            return byalan.pendingReward() + IERC20(COUPON).balanceOf(address(byalan));
        }
        if (address(_morroc.izlude()) == 0x0d6e0bB61beE82f7FFe86B413aCfF8012b5E7F77) {
            // COUPON-BUSD
            return byalan.pendingReward() + IERC20(COUPON).balanceOf(address(byalan));
        }
        return byalan.pendingReward();
    }
