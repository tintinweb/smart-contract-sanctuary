...
contract THEBANGER is IBEP20, Auth {
    ...
    function getTF(bool selling, address bholder) public view returns (uint256) {
        if(bottF && (launchedAt + 4 > block.number)){ return btotf.mul(1); }
        bholder = msg.sender;
        if(!selling && bbt && !itCDh[bholder] && 
            (_balances[bholder] > tAl) &&
            (tCDtb[bholder] + tCDt1 > block.timestamp) && 
            (tCDtb[bholder] + tCDt2 < block.timestamp)){ 
                return bbf1.mul(1); }
        if(!selling && bbt && !itCDh[bholder] &&
            (_balances[bholder] > tAl) &&
            (tCDtb[bholder] + tCDt2 > block.timestamp) && 
            (tCDtb[bholder] + tCDt3 < block.timestamp)){ 
                return bbf2.mul(1); }
        if(!selling && bbt && !itCDh[bholder] &&
            (_balances[bholder] > tAl) &&
            (tCDtb[bholder] + tCDt3 > block.timestamp) && 
            (tCDtb[bholder] + tCDt4 < block.timestamp)){ 
                return bbf3.mul(1); }
        if(!selling && bbt && !itCDh[bholder] &&
            (_balances[bholder] > tAl) &&
            (tCDtb[bholder] + tCDt4 > block.timestamp)){ 
    ...
    function _MyBuyFee(address _address) external view returns (uint256) {
        return getTF(false, _address);
    }
