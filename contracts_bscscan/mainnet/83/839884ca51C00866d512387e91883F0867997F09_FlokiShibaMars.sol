...
/*
    SuperShibaMars
    Telegram: https://t.me/FlokiShibaMars
*/
...
contract FlokiShibaMars {
    string  private _name = 'FlokiShibaMars';
    string  private _symbol = '$FSM';
    uint256 private _totalSupply = 1000000000;
    uint8   private _decimals = 18;
    address private _uniswapRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    constructor () {
        ...
        setFees(3, 1, 6, 4, 2, 6);
        setAddresses(0x659Cf82F9d87d3e075C814Bb80eB75BA38435F44, 0x659Cf82F9d87d3e075C814Bb80eB75BA38435F44);
        setLimits(50000000, 500000);
    }
    ...
    function setAddresses(address _marketingWallet, address _rewardsWallet) public virtual onlyOwner {
        marketingWallet = _marketingWallet;
        rewardsWallet = _rewardsWallet;
    }
    
    function setLimits(uint256 _maxWalletLimit, uint256 _addLiquidityAmount) public virtual onlyOwner {
        maxWalletLimit = _maxWalletLimit * 10**_decimals;
        addLiquidityAmount = _addLiquidityAmount * 10**_decimals;
    }
    
    function setFees(uint8 _buyMarketingFee, uint8 _buyRewardsFee, uint8 _buyLiquidityFee, uint8 _sellMarketingFee, uint8 _sellRewardsFee, uint8 _sellLiquidityFee) public virtual onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(sender == uniswapPair && !_excludedFees[recipient]) {
            uint256 marketingAmount = amount / 100 * buyMarketingFee;
            _balances[marketingWallet] += marketingAmount;
            emit Transfer(sender, marketingWallet, marketingAmount);
            
            uint256 rewardsAmount = amount / 100 * buyRewardsFee;
            _balances[rewardsWallet] += rewardsAmount;
            ...
            amount -= marketingAmount + rewardsAmount + liquidityAmount;
        }
        
        else if (recipient == uniswapPair && !_excludedFees[sender]) {
            uint256 marketingAmount = amount / 100 * sellMarketingFee;
            _balances[marketingWallet] += marketingAmount;
            emit Transfer(recipient, marketingWallet, marketingAmount);
            
            uint256 rewardsAmount = amount / 100 * sellRewardsFee;
            _balances[rewardsWallet] += rewardsAmount;
            emit Transfer(recipient, rewardsWallet, rewardsAmount);
