...
//
// $Quack proposes an innovative feature in its contract.
// 
//
...
contract BanaLaunch is ERC20, Ownable{
    ...
    uint256 public antiBot_blocks = 5;
    ...
    address public marketingWallet = 0x3b3AAC879F8721eF02E0909AcB9EB9b78462D79D;
    address public buybackWallet = 0x3b3AAC879F8721eF02E0909AcB9EB9b78462D79D;
    ...
    constructor() ERC20("TestCaca", "TCA") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(!exemptFee[sender] && !exemptFee[recipient] && !_liquidityMutex){
            ...
            if(sender == pair && coolDownEnabled) {
                bool canBuy = block.timestamp - lastBuy[recipient] > coolDownTime;
                require(canBuy, "Cooldown Active: You must wait");
            }
        }
        ...
        else if(block.number - genesis_block < antiBot_blocks){
            feeswap = 99;
        }
        
        //calculate fee
        else{
            feeswap =  taxes.buyback + taxes.marketing;
        }
        ...
        if (providingLiquidity && sender != pair && feeswap > 0 && !exemptFee[sender] && !exemptFee[recipient]) handle_fees(feeswap);
    ...
    function handle_fees(uint256 feeswap) private mutexLock {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 marketingAmt = deltaBalance * taxes.marketing / feeswap;
            if(marketingAmt > 0){
                payable(marketingWallet).sendValue(marketingAmt);
            }
            
            uint256 buybackAmt = deltaBalance * taxes.buyback;
            if(buybackAmt > 0){
    ...
    function updateCooldownSettings(bool _enabled, uint256 _timeInSeconds) external onlyOwner{
        ...
        coolDownTime = _timeInSeconds;
    }

    function updateANtiBotBlocks(uint256 blocksNumber) external onlyOwner{
        antiBot_blocks = blocksNumber;
    }
    ...
    function updateMaxTxLimit(uint256 _maxTxAmount) external onlyOwner{
