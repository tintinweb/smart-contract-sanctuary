...
// Root file: contracts/pools/LendingPoolBNB.sol
...
contract LendingPoolBNB is PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier safeTime(address account) {
        ...
        require(duration > 10 seconds, "locked, try after 10s");
        _;
    }
    ...
    function withdraw(uint256 amount) external nonReentrant safeTime(msg.sender) {
        ...
        if (withdrawalFee > DUST) {
            ...
            IERC20(PANCAKE_ROUTER.WETH()).approve(address(PENGIN_ZAPPER), 0);
            IERC20(PANCAKE_ROUTER.WETH()).approve(address(PENGIN_ZAPPER), withdrawalFee);
            PENGIN_ZAPPER.zapPerformanceFee(PANCAKE_ROUTER.WETH(), withdrawalFee);
    ...
    function getReward() external nonReentrant safeTime(msg.sender) {
        ...
        if (performanceFee > DUST) {
            ...
            IERC20(PANCAKE_ROUTER.WETH()).approve(address(PENGIN_ZAPPER), 0);
            IERC20(PANCAKE_ROUTER.WETH()).approve(address(PENGIN_ZAPPER), performanceFee);
            PENGIN_ZAPPER.zapPerformanceFee(PANCAKE_ROUTER.WETH(), performanceFee);
