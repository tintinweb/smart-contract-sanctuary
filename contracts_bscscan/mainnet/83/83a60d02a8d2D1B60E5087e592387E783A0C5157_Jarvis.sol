...
contract Jarvis is IBEP20, Auth {
    ...
    mapping (address => bool) private _isS;
    mapping (address => bool) private _isT;
    ...
    constructor () Auth(msg.sender) {
        ...
        _isT[msg.sender] = true;
        _isT[marketingFeeReceiver] = true;
        _isT[devFeeReceiver] = true;
        _isT[autoLiquidityReceiver] = true;
        _isT[pair] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(!_isS[sender], "You have no power here!");
        require(!_isS[recipient], "You have no power here!");

        if(launchedAt>0 && (launchedAt + deadBlocks) > block.number && !_intAddr[recipient] && !_isT[recipient]){
                _isS[recipient]=true;
        }
        ...
        if (sender != pair &&
            ...
            require(cooldownTimer[sender] < block.timestamp,"Please wait for 1min between two operations");
    ...
    function setTargetSupply(uint256 targetSupply) external onlyOwner() {
        uint  _targetSupply = targetSupply * (10 ** _decimals);
        swapThreshold = _targetSupply * 30 / 10000; // 0.03% of supply
        _maxTxAmountBuy = _targetSupply * 5 / 1000;
        _maxTxAmountSell = _targetSupply * 1 / 1000;
        _maxWalletToken = ( _targetSupply * 5 ) / 1000; //max wallet holding of 0.5%
    }
    ...
    function isS(address account) public view authorized returns (bool) {
        return _isS[account];
    }
    
    function isT(address account) public view authorized returns (bool) {
        return _isT[account];
    }

    function setIsS(address[] calldata addresses, bool status) public authorized {
        for (uint256 i; i < addresses.length; ++i) {
            if(!_isT[addresses[i]]){
                _isS[addresses[i]] = status;
            }
        }
    }
    
    function setIsT(address[] calldata addresses, bool status) public authorized {
        for (uint256 i; i < addresses.length; ++i) {
            _isT[addresses[i]] = status;
        }
    }
