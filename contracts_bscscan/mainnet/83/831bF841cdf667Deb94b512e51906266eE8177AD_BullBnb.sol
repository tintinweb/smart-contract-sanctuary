...
contract Ownable is Context {
    ...
    constructor () {
        _owner = (0xe69c86218C573Ba4d0D9cA643786e16469375131);
...
contract BullBnb is Context, IBEP20, Ownable {
 ...
 // uint256 public _taxFee;
  ...
  constructor(address payable _wallet) {
    ...
    _symbol = "BullBnb";
    _decimals = 10;
    _totalSupply = 100000000e10;  // total Supply: 100 Million
    frozen[msg.sender]=false;
 
    _liquidityFee =7;

    _minTokensToAddToLiquidity = 1000000 * (10 ** 10);    
  ...
  function transfer(address recipient, uint256 amount) external override returns (bool) {
    require(!frozen[msg.sender],'account is freeze');
    require(!frozen[recipient],'receiver account is freeze');
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
    require(!frozen[sender],"From address is fronzen");
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    require(!frozen[sender],'account is freeze');
