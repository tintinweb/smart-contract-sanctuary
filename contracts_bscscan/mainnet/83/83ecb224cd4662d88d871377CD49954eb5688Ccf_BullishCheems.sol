...
contract BullishCheems is Context, IERC20, Ownable {
    ...
    uint256 private _tBCTotal;

    string private _name = "BullishCheems";
    ...
    uint256 public _BCFee = 12;
    uint256 private _previousBCFee = _BCFee;
    
    uint256 public _maxTxAmount = 1000 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 3 * 10**6 * 10**9; 
    
    address payable public BCWallet = 0x738C0A9eA78DF7c69B8C67688522B38B064CCec6; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tBCTotal = _tBCTotal.add(address(this).balance);
        TransferCharityETH(BCWallet, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_BCFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _BCFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousBCFee = _BCFee;
    ...
    function setBCFeePercent(uint256 BCFee) external onlyOwner() {
        _BCFee = BCFee;
    }
