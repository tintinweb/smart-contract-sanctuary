...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ggrainReward = multiplier.mul(ggrainPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ggrain.mint(feeAddDev, ggrainReward.mul(devCommissionRate).div(10000));
    ...
    function updateFeeAddBb(address _newFeeAddBb) public onlyOwner {
        require(_newFeeAddBb != address(0), "update: fee address BuyBack is zero");
        feeAddBb = _newFeeAddBb;
    }

    function updateReferralRate(uint16 _newReferralRate) public onlyOwner {
        require(_newReferralRate <= 1500, "update: referral rate is over 15%");
        referralCommissionRate = _newReferralRate;
    }

    function updateDevRate(uint16 _newDevRate) public onlyOwner {
        require(devCommissionRate <= 1500, "update: dev commission rate is over 15%");
