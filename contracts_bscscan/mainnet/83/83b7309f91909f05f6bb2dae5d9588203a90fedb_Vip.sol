...
interface IStakePool {
    ...
    // 获取当前最高id
    function currentId() external returns (uint);

    // 获取当前质押（订单）总数
    function totalSupply() external view returns (uint);

    // 获取用户当前质押（订单）总数 -- 传0地址为全局总数
    ...
    // 分页获取用户质押（订单） -- 传0地址为全局订单
    function getStakes(address _account, uint _index, uint _offset) external view returns (StakeSet.Item[] memory items);

    // 获取取消质押（订单）数量
    function withdrawLen() external view returns (uint);

    // 分页获取取消质押（订单）
    function getWithdrawIds(uint _index, uint _offset) external view returns (uint[] memory res);

    // 获取swap价格
    function getPrice(address _lp) external view returns (uint);

    /**
     * @dev stake
     * @param _payToken  payToken contract address eg. WETH/WBNB/... contract address
    ...
    // 设置组合类型开关
    function setTypeStatus(uint _index, bool _value) external;

    // 设置指定组合类型指定用户开关
    function setIncludeTypeAccount(uint _index, address _index1, bool _value) external;
...
contract Vip is Ownable {
    ...
    constructor (address _token, address _stakePool) {
        token = _token;
    ...
    function setStakePool(address _value) external onlyOwner {
        stakePool = _value;
    }

    function getVip(address _account) public view returns (uint) {
        uint staticPower = IStakePool(stakePool).weightOf(_account);
    ...
    function burnToken(uint _amount) external {
        ...
        IERC20(token).transferFrom(msg.sender, BURN_ADDRESS, _amount);
        // update burnAmountOf
        burnAmountOf[msg.sender] += _amount;
    }
}