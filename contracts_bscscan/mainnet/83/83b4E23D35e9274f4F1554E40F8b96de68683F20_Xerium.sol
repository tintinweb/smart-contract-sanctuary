...
contract Xerium is Context, IERC20, Ownable {
    ...
    string private _name = "Xerium Network";
    string private _symbol = "XER";
    ...
    constructor() {
        ...
        uint256 _toBurn = _totalSupply.mul(99).div(100);
        uint256 _left = _totalSupply.sub(_toBurn);
        _balances[feeWallets.burnAddress] += _toBurn;
        _balances[owner()] += _totalSupply.sub(_toBurn);
        emit Transfer(address(0), feeWallets.burnAddress, _toBurn);
        emit Transfer(address(0), owner(), _left);
    }
   ...
   /**
    ...
    function calculateFee(uint256 _totalFees, uint256 feePercent) internal {
    ...
    function sendFees(uint256 _toBurn) internal {
        _balances[feeWallets.burnAddress] += _toBurn;
    ...
    function setBuyFees(uint256 _burnFee) public onlyOwner {
        buyFees = FeeSet({
            burnFee: _burnFee
        });
    }

    function setSellFees(uint256 _burnFee) public onlyOwner {
        sellFees = FeeSet({
            burnFee: _burnFee
        });
    } 
