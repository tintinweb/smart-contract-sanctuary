    ...
    TokenListener
{
    ...
    function initialize(
        ...
        __Ownable_init();
        Constants.REGISTRY.setInterfaceImplementer(
            address(this),
    ...
    function _awardExternalErc20s(address winner) internal {
        ...
        while (
            ...
            uint256 balance =
                IERC20Upgradeable(currentToken).balanceOf(address(prizePool));
            if (balance > 0) {
    ...
    function _awardExternalErc721s(address winner) internal {
        ...
        while (
            ...
            uint256 balance =
                IERC721Upgradeable(currentToken).balanceOf(address(prizePool));
            if (balance > 0) {
        ...
        returns (uint256)
    {
        uint256 elapsedPeriods =
            currentTime.sub(prizePeriodStartedAt).div(prizePeriodSeconds);
    ...
    function _addExternalErc20Award(IERC20Upgradeable _externalErc20) internal {
        ...
        (bool succeeded, ) =
            address(_externalErc20).staticcall(
    ...
    TokenControllerInterface
{
        ...
        returns (uint256)
    {
        ...
        (uint256 lockDuration, uint256 burnedCredit) =
            _calculateTimelockDuration(from, controlledToken, amount);
    ...
    function beforeTokenTransfer(
        ...
        if (from != address(0)) {
            uint256 fromBeforeBalance =
                IERC20Upgradeable(msg.sender).balanceOf(from);
            // first accrue credit for their old balance
            uint256 newCreditBalance =
                _calculateCreditBalance(from, msg.sender, fromBeforeBalance, 0);
        ...
        returns (uint256)
    {
        ...
        uint256 totalInterest =
            (currentBalance > tokenTotalSupply)
                ? currentBalance.sub(tokenTotalSupply)
                : 0;
        uint256 unaccountedPrizeBalance =
            (totalInterest > _currentAwardBalance)
    ...
    function award(
        ...
        uint256 extraCredit =
            _calculateEarlyExitFeeNoCredit(controlledToken, amount);
        _accrueCredit(
            to,
            controlledToken,
    ...
    function calculateReserveFee(uint256 amount) public view returns (uint256) {
        ...
        uint256 reserveRateMantissa =
            reserve.reserveRateMantissa(address(this));
    ...
    /// @return The total amount of assets swept from the Prize Pool
    function _sweepTimelockBalances(address[] memory users)
        internal
        returns (uint256)
    {
        address operator = _msgSender();
        ...
        for (i = 0; i < users.length; i++) {
            if (balances[i] > 0) {
                ...
                uint256 shareMantissa =
                    FixedPoint.calculateMantissa(balances[i], totalWithdrawal);
                uint256 transferAmount =
                    FixedPoint.multiplyUintByMantissa(redeemed, shareMantissa);
                underlyingToken.safeTransfer(users[i], transferAmount);
                emit TimelockedWithdrawalSwept(
                    operator,
                    users[i],
                    balances[i],
                    transferAmount
    ...
    function _calculateTimelockDuration(
        ...
        (uint256 exitFee, uint256 _burnedCredit) =
            _calculateEarlyExitFeeLessBurnedCredit(
                from,
                controlledToken,
                amount
        ...
        uint256 duration =
            _estimateCreditAccrualTime(controlledToken, amount, exitFee);
    ...
    function _burnCredit(
        ...
        _tokenCreditBalances[controlledToken][user].balance = uint256(
            _tokenCreditBalances[controlledToken][user]
                .balance
        )
            .sub(credit)
            .toUint128();
    ...
    function _calculateAccruedCredit(
        ...
        uint256 userTimestamp =
            _tokenCreditBalances[controlledToken][user].timestamp;
        ...
        returns (uint256)
    {
        ...
        ( uint256 limitMantissa , ) = creditPlanOf(controlledToken);
        
        uint256 userLimitBalance =
            FixedPoint.multiplyUintByMantissa(
                controlledTokenBalance,
                limitMantissa
            );

        return userLimitBalance.div(userCredit);
    }
    ...
    function _calculateEarlyExitFeeLessBurnedCredit(
        ...
        uint256 controlledTokenBalance =
            IERC20Upgradeable(controlledToken).balanceOf(from);
    ...
    */
        ...
        uint256 remainingExitFee =
            _calculateEarlyExitFeeNoCredit(
                controlledToken,
        ...
        if (
            ...
            availableCredit = uint256(
                _tokenCreditBalances[controlledToken][from]
                    .balance
            )
    ...
    function timelockBalanceOf(address user)
        external
        view
        override
        returns (uint256)
...
contract PoolWithMultipleWinnersBuilder {
    ...
    /// @notice The configuration used to initialize the Yield Source Prize Pool
    ...
    constructor(
        ...
        require(
            address(_stakePrizePoolProxyFactory) != address(0),
            "GlobalBuilder/stakePrizePoolProxyFactory-not-zero"
        );
        require(
    ...
    function createYieldSourceMultipleWinners(
        ...
        YieldSourcePrizePool prizePool =
            yieldSourcePrizePoolProxyFactory.create();
        MultipleWinners prizeStrategy =
            multipleWinnersBuilder.createMultipleWinners(
                prizePool,
                prizeStrategyConfig,
                decimals,
                msg.sender
        ...
        returns (ControlledTokenInterface[] memory)
    {
        ControlledTokenInterface[] memory tokens =
            new ControlledTokenInterface[](2);
...
interface IYieldSource {
    ...
    /// @notice Returns the total balance (in asset tokens).  This includes the deposits and interest.
    /// @return The underlying balance of asset tokens
    function balanceOfToken() external returns (uint256);
...
contract YieldSourcePrizePool is PrizePool {
    ...
    /// @param _reserveRegistry Address of reserve registry
    ...
    function initializeYieldSourcePrizePool(
        ...
        require(
            ...
            "YieldSourcePrizePool/yield-source-zero"
        );
        PrizePool.initialize(
            _reserveRegistry,
            _controlledTokens,
        ...
        (bool succeeded, ) =
            address(_yieldSource).staticcall(
    ...
    function _balance() internal override returns (uint256) {
        return yieldSource.balanceOfToken();
    }
...
contract PeriodicPrizeStrategyHarness is PeriodicPrizeStrategy {
    ...
    function setDistributor(
        PeriodicPrizeStrategyDistributorInterface _distributor
    ) external {
    ...
    function forceBeforeAwardListener(BeforeAwardListenerInterface listener)
        external
    {
...
contract BeforeAwardListenerStub is BeforeAwardListener {
    ...
    function beforePrizePoolAwarded(
...
/// @title Defines the functions used to interact with a yield source.  The Prize Pool inherits this contract. Yield Source is Beefy's mooToken
/// @notice Manages depositing and withdrawing assets from the Prize Pool
contract MooTokenYieldSource is IYieldSource, ReentrancyGuardUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    address public prizePool;

    event MooTokenYieldSourceInitialized(address indexed mooToken);

    mapping(address => uint256) public balances;

    uint256 public totalYieldTokenAmount;

    /// @notice Interface for the Yield-bearing mooToken by Beefy
    IMooToken public mooToken;

    /// @notice token which want to maximize
    IERC20Upgradeable public underlying;

    /// @notice initializes Yield Service for Beefy vault's mooToken
    /// @param _mooToken Address of the Beefy mooToken interface
    constructor(IMooToken _mooToken, IERC20Upgradeable _underlying, address _prizePool ) public {
        mooToken = _mooToken;
        underlying = _underlying;
        prizePool = _prizePool;

        emit MooTokenYieldSourceInitialized(address(mooToken));
    }
    ...
    /// @return The underlying balance of asset tokens
    function balanceOfToken() external override returns (uint256) {
        uint256 sharesBal = totalYieldTokenAmount;

        // Converts mooTokens to underlying
        uint256 underlyingAmount;
        if (mooToken.totalSupply() == 0) {
            underlyingAmount = sharesBal;
        } else {
            underlyingAmount = sharesBal.mul(mooToken.balance()).div(
                mooToken.totalSupply()
            );
        }
    ...
    /// @param amount The amount of `token()` to be supplied
    /// @param to The user whose balance will receive the tokens
    function supplyTokenTo(uint256 amount, address to)
        external
        override
        nonReentrant
    {
        require(msg.sender == prizePool, "!Prize Pool");
        underlying.safeTransferFrom(msg.sender, address(this), amount);
        underlying.safeApprove(address(mooToken), amount);

        uint256 mooTokenBalanceBefore = totalYieldTokenAmount;
        uint256 balDiff = IERC20Upgradeable(mooToken).balanceOf(address(this)).sub(mooTokenBalanceBefore);
        mooToken.deposit(amount);
        totalYieldTokenAmount = IERC20Upgradeable(mooToken).balanceOf(address(this)).sub(balDiff);

        uint256 mooTokenDiff = totalYieldTokenAmount.sub(
            mooTokenBalanceBefore
        );
        balances[to] = balances[to].add(mooTokenDiff);
    }

    /// @notice Redeems tokens from the yield source.
    /// @param amount The amount of `token()` to withdraw.  Denominated in `token()` as above.
    /// @return The actual amount of tokens that were redeemed.
    function redeemToken(uint256 amount)
        external
        override
        nonReentrant
        returns (uint256)
    {
        ...
        if (mooToken.totalSupply() == 0) {
            ...
            sharesAmount = amount.mul(mooToken.totalSupply()).div(
                mooToken.balance()
            );
        }
        ...
        mooToken.withdraw(sharesAmount);
        totalYieldTokenAmount = totalYieldTokenAmount.sub(sharesAmount);

        uint256 mooTokenDiff = mooTokenBalanceBefore.sub(
            totalYieldTokenAmount
        );

        uint256 diff = underlying.balanceOf(address(this)).sub(balanceBefore);
        balances[msg.sender] = balances[msg.sender].sub(mooTokenDiff);
        underlying.safeTransfer(msg.sender, diff);
...
/// @title Manager of user's funds entering MooPot
/// @notice Manages divying up assets into prize pool and yield farming
contract GateManagerRewards is ReentrancyGuardUpgradeable, Ownable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event GateManagerDeposit(address indexed user, uint256 value);

    /// @notice Interface for the Yield-bearing mooToken by Beefy
    IMooToken public immutable mooToken;

    /// @notice Interface for Moonpot prize pool
    IPrizePool public immutable prizePool;

    /// @notice token which want to maximize
    IERC20Upgradeable public immutable underlying;

    /// @notice prize pool lottery token
    ...
    uint256 public stakingRewardsDuration;
    ...
    /// @param _rewardToken Address of staking reward token
    /// @param _stakingRewardsDuration Time duration represented in seconds
    constructor(
        IMooToken _mooToken,
        ...
        stakingRewardsDuration = _stakingRewardsDuration;
    }
    ...
    function TVL() external view returns (uint256) {
        uint256 totalGateManagerBal = IERC20Upgradeable(mooToken).balanceOf(
            address(this)
        );
        uint256 totalYieldSourceBal = IYieldSource(prizePool.yieldSource())
        .totalYieldTokenAmount();
        // Converts mooTokens to underlying
        uint256 underlyingAmountYS;
        uint256 underlyingAmountGM;
        if (mooToken.totalSupply() == 0) {
            underlyingAmountYS = totalYieldSourceBal;
            underlyingAmountGM = totalGateManagerBal;
        } else {
            underlyingAmountYS = totalYieldSourceBal
            .mul(mooToken.balance())
            .div(mooToken.totalSupply());
            underlyingAmountGM = totalGateManagerBal
            .mul(mooToken.balance())
            .div(mooToken.totalSupply());
        }
    ...
    function awardBalance() external view returns (uint256) {
        ...
        uint256 underlyingAmount;
        if (mooToken.totalSupply() == 0) {
            underlyingAmount = totalYieldBal;
        } else {
            underlyingAmount = totalYieldBal.mul(mooToken.balance()).div(
                mooToken.totalSupply()
            );
        }
    ...
    function userTotalBalance(address user) external view returns (uint256) {
        ...
        uint256 underlyingAmount;
        if (mooToken.totalSupply() == 0) {
            underlyingAmount = yieldBal;
        } else {
            underlyingAmount = yieldBal.mul(mooToken.balance()).div(
                mooToken.totalSupply()
            );
        }
    ...
    function depositAll(address referrer) external {
        uint256 tokenBal = underlying.balanceOf(msg.sender);
        depositMoonPot(tokenBal, referrer);
    }

    /// Withdraw all sender funds with timelock
    function withdrawAllTimelock() public nonReentrant {
        uint256 ticketBal = IERC20Upgradeable(prizePoolControlledToken)
        .balanceOf(msg.sender);
        if (ticketBal > 0) {
            withdrawWithTimeLockFromMoonPotPrizePool(ticketBal);
        }

        uint256 yieldBal = balances[msg.sender];

        if (yieldBal > 0) {
            _withdrawMoonPotYield(yieldBal);
        }
    }

    /// Withdraw all sender funds with timelock & claim rewards
    function exitTimelock() external {
        getReward();
        withdrawAllTimelock();
    }

    /// Withdraw all sender funds without timelock & claim rewards
    function exitInstantly() external {
        getReward();
        withdrawAllInstantly();
    }

    /// Withdraw all sender funds without timelock
    function withdrawAllInstantly() public nonReentrant {
        uint256 ticketBal = IERC20Upgradeable(prizePoolControlledToken)
        .balanceOf(msg.sender);
        if (ticketBal > 0) {
    ...
    function depositMoonPot(uint256 amount, address referrer)
        public
        nonReentrant
        updateReward(msg.sender)
    {
        ...
        underlying.safeApprove(address(prizePool), amount - halvedAmount);
        prizePool.depositTo(
            msg.sender,
            amount - halvedAmount,
        ...
        underlying.safeApprove(address(mooToken), halvedAmount);
        uint256 mooTokenBalanceBefore = mooToken.balanceOf(address(this));
        mooToken.deposit(halvedAmount);
        uint256 mooTokenDiff = mooToken.balanceOf(address(this)).sub(
            mooTokenBalanceBefore
        );
        _totalSupply = _totalSupply.add(mooTokenDiff);
        balances[msg.sender] = balances[msg.sender].add(mooTokenDiff);
        emit Staked(msg.sender, mooTokenDiff);
    }

    /// @notice withdraw underlying from yield earning vault
    /// @param amount The amount of `underlying` to withdraw.
    /// @return The actual amount of tokens that were withdrawn.
    function withdrawMoonPotYield(uint256 amount)
        public
        nonReentrant
        returns (uint256)
    {
        ...
        _withdrawMoonPotYield(sharesAmount);
    }

    /// @notice withdraw underlying from yield earning vault
    /// @param shares The amount of shares to withdraw.
    /// @return The actual amount of tokens that were withdrawn.
    function _withdrawMoonPotYield(uint256 shares)
        internal
        updateReward(msg.sender)
        returns (uint256)
    {
        ...
        uint256 mooTokenDiff = mooTokenBalanceBefore.sub(
            mooToken.balanceOf(address(this))
        );
        uint256 diff = underlying.balanceOf(address(this)).sub(balanceBefore);
    ...
    /// @notice withdraw from prize pool with timeLock.
    /// @param amount The amount of controlled prize pool token to redeem for undelying.
    function withdrawWithTimeLockFromMoonPotPrizePool(uint256 amount) public {
        require(
        ...
        prizePool.withdrawWithTimelockFrom(
            msg.sender,
            amount,
            prizePoolControlledToken
        );
    ...
    /// @param amount The amount of controlled prize pool token to redeem for undelying.
    function withdrawInstantlyFromMoonPotPrizePool(uint256 amount) public {
        require(
            IERC20Upgradeable(prizePoolControlledToken).allowance(
                msg.sender,
                address(this)
        ...
        updateReward(address(0))
    {
        if (block.timestamp >= periodFinish) {
            rewardRate = reward.div(stakingRewardsDuration);
        } else {
            uint256 remaining = periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(rewardRate);
            rewardRate = reward.add(leftover).div(stakingRewardsDuration);
        }
        lastUpdateTime = block.timestamp;
        periodFinish = block.timestamp.add(stakingRewardsDuration);
...
import "../external/maker/CAKEInterface.sol";

contract CAKE is CAKEInterface {
    using SafeMathUpgradeable for uint256;
    using AddressUpgradeable for address;
    ...
    function permit(
        ...
        require(holder != address(0), "CAKE/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "CAKE/invalid-permit");
        require(expiry == 0 || now <= expiry, "CAKE/permit-expired");
        require(nonce == nonces[holder]++, "CAKE/invalid-nonce");
...
interface CAKEInterface is IERC20Upgradeable {
    // --- Approve by signature ---
  function permit(address holder, address spender, uint256 nonce, uint256 expiry, bool allowed, uint8 v, bytes32 r, bytes32 s) external;
  function transferFrom(address src, address dst, uint wad) external override returns (bool);
}
...
contract BIFI is CAKEInterface {
    using SafeMathUpgradeable for uint256;
    using AddressUpgradeable for address;
...
/// @title Manager of user's funds entering MooPot
/// @notice Manages divying up assets into prize pool and yield farming
contract GateManager is ReentrancyGuardUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    /// @notice Interface for Moonpot prize pool
    PrizePoolInterface public immutable prizePool;
    ...
    /// Withdraw all sender funds with timelock
    function withdrawAllTimelock() external nonReentrant {
        uint256 ticketBal = IERC20Upgradeable(prizePoolControlledToken)
        .balanceOf(msg.sender);
        if (ticketBal > 0) {
            withdrawWithTimeLockFromMoonPotPrizePool(ticketBal);
    ...
    /// Withdraw all sender funds without timelock
    function withdrawAllInstantly() external nonReentrant {
        uint256 ticketBal = IERC20Upgradeable(prizePoolControlledToken)
        .balanceOf(msg.sender);
        if (ticketBal > 0) {
            withdrawInstantlyFromMoonPotPrizePool(ticketBal);
...
contract MooTokenMock is ERC20Upgradeable, IMooToken {
    ...
    function withdraw(uint256 _shares) public override {
        ...
        underlying.transfer(msg.sender, r);
    }
