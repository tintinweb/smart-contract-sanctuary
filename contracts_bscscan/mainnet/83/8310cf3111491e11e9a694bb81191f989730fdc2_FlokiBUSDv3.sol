...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable DividendsToken = address(0x55d398326f99059fF775485246999027B3197955);
...
contract FlokiBUSDv3 is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   FlokiBUSDv3DividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("FlokiBUSDv3", "FlokiBUSDv3") {
        _ETHRewardsFee = 12;
        _liquidityFee = 6;
		_marketingFee = 0;
    	...
    	dividendTracker = new FlokiBUSDv3DividendTracker();
    	antiBotEnabled = true;

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x1B6c3A15524aeF97a7c3356E15d11f41A1FeDa81);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FlokiBUSDv3: The dividend tracker already has that address");

        FlokiBUSDv3DividendTracker newDividendTracker = FlokiBUSDv3DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FlokiBUSDv3: The new dividend tracker must be owned by the FlokiBUSDv3 token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FlokiBUSDv3: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FlokiBUSDv3: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FlokiBUSDv3: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FlokiBUSDv3: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FlokiBUSDv3: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FlokiBUSDv3: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FlokiBUSDv3: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForCAKE(uint256 tokenAmount, address recipient) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of FlokiBUSDv3
            path,
            recipient,
            block.timestamp
        );
...
contract FlokiBUSDv3DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FlokiBUSDv3_Dividend_Tracker", "FlokiBUSDv3_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 50000000 * (10**18); //must hold 10000+ tokens
    }



    function _transfer(address, address, uint256) internal pure override {
        require(false, "FlokiBUSDv3_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "FlokiBUSDv3_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiBUSDv3 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "FlokiBUSDv3_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FlokiBUSDv3_Dividend_Tracker: Cannot update claimWait to same value");
