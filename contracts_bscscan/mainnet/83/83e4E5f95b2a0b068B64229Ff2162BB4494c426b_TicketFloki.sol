...
contract TicketFloki is ERC721URIStorage, Ownable, Tickable {
    ...
    uint8 private _maxMintPerUser = 5;
    ...
    event SafeTicket(uint indexed id, string indexed uri);
    ...
    function mint(uint nbMint) public payable {
        require(nbMint > 0, 'must be greater than 0');
        require(block.timestamp >= _startMint && _mintEnable, 'TICKETFLOKI: the mint dont start yet');
        require(msg.value >= _priceMint * nbMint, 'TICKETFLOKI: Value below price');
        require(_tokenIds.current() < (_maxSupply - _reserveMint), 'TICKETFLOKI: The mint is over');
        require(balanceOf(msg.sender) <= _maxMintPerUser, 'you can not mint more than 5 tickets');
        _transferFee(msg.value);
        for(uint8 i = 0; i < nbMint; i++) {
            Type typeTicket = _randomType();
            _mintTicket(typeTicket, _msgSender());
        }
    }
    ...
    function setMaxMintPerUser(uint8 maxMintPerUser) external onlyOwner{
        _maxMintPerUser = maxMintPerUser;
    }

    function getMaxMintPerUser() public view returns(uint8) {
