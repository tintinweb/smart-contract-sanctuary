...
Telegaram : https://t.me/XrpRoyale
Website : https://xrproyale.site/
...
contract XrpRoyale is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("XrpRoyale", "XrpRoyale") {
    	cakeDividendTracker = new CakeDividendTracker();

    	
    	marketingWallet = 0x12f8179Ab4ab8d3688BcE2A486C6dB0d469482E2;

    	cakeDividendToken = 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE ; //
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "XrpRoyale: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "XrpRoyale: The new dividend tracker must be owned by the XrpRoyale token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("XrpRoyale_Dividend_Tracker", "XrpRoyale_Dividend_Tracker", 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "XrpRoyale_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "XrpRoyale_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "XrpRoyale_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "XrpRoyale_Dividend_Tracker: Cannot update claimWait to same value");
