...
/**
 * BondDepository2 Contract - FOR OHMPLUS 
 * 
 * OHMPLUS Token [ ohm fork ]
 * Website: https://ohmplus.finance/
 * Telegram: https://t.me/ohmplus 
*/
...
abstract contract ERC20Permit is ERC20, IERC2612Permit {
    ...
    bytes32 public constant PERMIT_TYPEHASH = 0x8cad95687ba82c2ce50e74f7b754645e5117c3a5bec8151c0726d5857980a866;
...
contract OhmpBondDepository is Ownable {
    ...
    address public immutable OHMP; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints OHMP when receives principle
    ...
    struct Bond {
        uint payout; // OHMP remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 OHMP ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) OHMP
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( OHMP ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( OHMP ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( OHMP ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( OHMP ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( OHMP ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to OHMP supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( OHMP ).totalSupply();
    ...
    /**
     *  @notice calculate amount of OHMP available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or OHMP) to the DAO
     *  @return bool
     */
