...
/**
   ...
   2% fee auto add to Marketing pool
   1% fee auto add to marketing/ dev pool
...
contract FFF is Context, IERC20, Ownable {
    ...
    uint256 public _DevBudget = 1;
    uint256 private _previousDevBudget = _DevBudget;
    ...
    constructor () public  {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //Pancake V2 Swap's address
    ...
    function setDevAddress(address payable marketing) public onlyOwner {
        _DevAddress = marketing;
    }
    ...
    function setDevFeePercent(uint256 DevBudget) external onlyOwner {
        _DevBudget = 0;
        if(DevBudget < 6) {
	        _DevBudget = DevBudget;
	    }  
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusMarketingFee(tAmount);
    ...
    function calculateLiquidityPlusMarketingFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _MarketingFee + _DevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _DevBudget == 0 && _MarketingFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousMarketingFee = _MarketingFee;
        _previousDevBudget = _DevBudget;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _DevBudget + _liquidityFee + _MarketingFee;
        ...
        // calculate the portions of the liquidity to add to Marketing
        ...
        uint256 marketingBalance = newBalance.div(totalLiqFee).mul(_DevBudget);
        uint256 marketingPortion = otherHalf.div(totalLiqFee).mul(_DevBudget);
        
        uint256 finalBalance = newBalance.sub(MarketingBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(MarketingPortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _MarketingAddress.call{value: MarketingBalance}("");
        if(sent){
        ...
        (sent, data) = _DevAddress.call{value: marketingBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, marketingPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, marketingPortion);
        } else {
            addLiquidity(marketingPortion, marketingBalance, _DevAddress);
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address Marketing) private {
