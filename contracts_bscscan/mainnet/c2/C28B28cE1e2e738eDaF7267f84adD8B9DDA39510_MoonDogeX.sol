...
Cute doggo in space, what more can you ask for? 
$SDS is a charity meme coin with moon potential

Token airdrop to the first 50 telegram members! 

We'll be donating live to Binance charities that the community votes on at every 50k mcap crossed because we believ in the greater good of society.

TG: https://t.me/spacedogshiba
Website coming soon

100% LIQ BURNED AND OWNERSHIP RENOUNCED
1% MAX WALLET TO STOP THE WHALES
...
contract MoonDogeX is Context, IBEP20, Ownable {
    ...
    uint256 private constant _tTotal = 10 * 10**8 * 10**8; // 1bil
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _liquiditySplitCeiling = 20 * 10**8 * 10**8; // 0.2 BNB
    uint256 private _maxTxn = 90 * 10**9 * 10**8; // 2BNB
    uint256 private _maxWallet = _tTotal.div(10**2).mul(5); // 4% of total supply
    ...
    string private _name = "MoonDogeX";
...
function addToBlackList(address[] calldata addresses) external onlyOwner {
    ...
    function removeFromBlackList(address account) external onlyOwner {
     ...
     function _transfer(
        ...
        if (
            ...
            require(
                currentPriceInBNB.mul(amount) <= _maxTxn.div(2),
        ...
        if (balanceOf(address(this)) > 0) {
            ...
            bool overLiquiditySplitCeiling = 
                currentPriceInBNB.mul(balanceOf(address(this))) >= _liquiditySplitCeiling;
            if (
                overLiquiditySplitCeiling &&        
                !_inSwapAndLiquify && 
                sender != pcsV2Pair    
            ) {
                uint tokensToSwap = _liquiditySplitCeiling.div(currentPriceInBNB);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        address developmentWalletAddress = 0xBfAfeF0f5435c01cBa41AEa4d63BC4B816F23BEB;
        ...
        uint256 devBalanceReceived = address(this).balance.sub(initialBalanceForDevSwap);
