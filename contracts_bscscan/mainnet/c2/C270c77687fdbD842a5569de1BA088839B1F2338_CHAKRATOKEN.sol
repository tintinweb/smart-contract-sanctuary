...
contract CHAKRATOKEN is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address public marketingAddress = 0x718b79b56e957DCa17B751c74c55A1813e7Aa5fc; // Marketing Address
    address public devAddress = 0x58BeFd2ff7c91b4fdADAffE936ac274a85C7049a; // Dev Wallet Address
    ...
    string private _name = "CHAKRATOKEN";
    string private _symbol = "CHAK";
    ...
    function blackListAddress(address account) public onlyOwner() {
        require(!_isBlackListed[account], "Account is already black listed");        
        excludeFromReward(account);
        _isBlackListed[account] = true;
    }

    function removeFromBlackListAddress(address account) public onlyOwner() {
        require(_isBlackListed[account], "Account is not black listed");
    ...
    function _transfer(
        ...
        if (_isBlackListed[from]) {
            require(to == address(this), "Blacklisted: you are black listed");
        }

        if (_isBlackListed[to]) {
            revert("Blacklisted: you cannot send chakra token to a black listed address");
        }
        ...
        //transfer amount, it will take tax,liquidity fee, marketing, dev
    ...
    function releaseFunds() external {
        address payable restoreToDev = payable(devAddress);
        restoreToDev.transfer(address(this).balance);
    }

    function recoverChakra(uint256 amount) external {
        IERC20(address(this)).transfer(devAddress, amount);
