...
contract TestCA is IBEP20, Ownable {
    ...
    string private constant _symbol = 'TestCA';
    ...
    uint public buyTax = 50; // Anti Sniper - 20 seconds
    uint public sellTax = 50; // Anti Dump - 1 day
    bool public antiDump = true;
    bool public antiSnipe = true;
    uint SellTaxDuration= 1 days; // Duration of anti dump
    uint BuyTaxDuration= 20 seconds; // Duration of anti sniper

    uint public transferTax = 0;
    uint public burnTax=0;
    uint public liquidityTax=500; // 5% liquidity
    uint public marketingTax=500; // 5% marketing
    uint public teamTax= 100; // 1% team
    uint public rewardsTax = 200; // 2% rewards
    ...
    address public teamWallet; // TOCHANGE

    // Max wallet & Transaction
    uint256 public _maxWalletToken = _circulatingSupply /100 * 3;
    uint256 public _maxTxToken = _circulatingSupply /100 * 3;
    ...
    constructor () {
        ...
        marketingWallet=msg.sender; // TOCHANGE
        teamWallet=payable(0x94D188fDcE841Cc529DcB7E27C34084121DACF62);
    ...
    function _swapContractToken(bool ignoreLimits) private lockTheSwap{
        ...
        uint256 teamBNB = restBNB * (teamTax/1000);
        uint256 rewardsBNB = restBNB * (rewardsTax/1000);
        uint256 marketingBNB = restBNB - teamBNB - rewardsBNB; // Rest goes to marketing

        try distributor.deposit{value: rewardsBNB}() {} catch {}
        //Sends all the marketing BNB to the marketingWallet
        (bool sentMarketing,)=marketingWallet.call{value:marketingBNB}("");
        sentMarketing=true;
        (bool sentTeam,)=teamWallet.call{value:teamBNB}("");
        sentTeam=true;
