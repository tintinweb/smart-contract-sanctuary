...
contract BTCBIN is ERC20, Ownable {
    ...
    uint256 public coolDownInterval = 0 seconds;
    ...
    BTCBINDividendTracker public dividendTracker;
    ...
    uint256 public maxWalletDivisor = 50;
    uint256 public maxBuyDivisor = 1;
    uint256 public maxSellDivisor = 1;
    ...
    uint256 public totalFees = BTCBRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
    ...
    constructor(address _marketingWalletAddress) ERC20("BitFloki", "BTCFLOKI") {

    	dividendTracker = new BTCBINDividendTracker();
    ...
    /// @notice Withdraw tokens sent by mistake. Impossible to withdraw BTCBIN tokens
    /// @param tokenAddress The address of the token to withdraw
    function rescueBEP20Tokens(address tokenAddress) external onlyOwner{
        require(tokenAddress != address(this), "Impossible to withdraw BTCBIN tokens");
   ...
   /////////////////////////////////
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BTCBIN: Account is already the value of 'excluded'");
   ...
   ///////////////////////
    ...
    function setBTCBRewardsFee(uint256 value) external onlyOwner{
        ...
        totalFees = BTCBRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
    }
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "BTCBIN: Automated market maker pair is already set to that value");
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BTCBIN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BTCBIN: Cannot update gasForProcessing to same value");
   ...
   /**
    ...
    function _transfer(address from, address to, uint256 amount) internal override{
        ...
        if(!_isExcludedFromFees[to] && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[to]){
        ...
        if(!_isExcludedFromFees[from] && automatedMarketMakerPairs[to] && !swapping){
        ...
        if(canSwap && !swapping && swapEnabled && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[from]) {
            ...
            if(BTCBRewardsFee > 0){
                uint256 sellTokens = contractTokenBalance.mul(BTCBRewardsFee).div(totalFees);
...
contract BTCBINDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("BTCBIN_Dividen_Tracker", "BTCBIN_Dividend_Tracker") {
    	claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BTCBIN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BTCBIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BTCBIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BTCBIN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BTCBIN_Dividend_Tracker: Cannot update claimWait to same value");
