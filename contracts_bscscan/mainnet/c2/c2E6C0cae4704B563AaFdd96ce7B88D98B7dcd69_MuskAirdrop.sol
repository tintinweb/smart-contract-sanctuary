...
import {IMuskAirdrop} from './interfaces/IMuskAirdrop.sol';
...
contract MuskAirdrop is IMuskAirdrop, Initializable {
  ...
  function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override {
      ...
      uint256 tokenRate = SellToken(IDO_CONTRACT).tokenRate();
      uint256 busdAmount = availableClaim * tokenRate / 1e18;
      SellToken(IDO_CONTRACT).buyIDO(account, busdAmount, address(0));
  ...
  function updateIDO(address _idoContract) public onlyAdmin {
    IDO_CONTRACT = _idoContract;
  }

  function updateRewardToken(IERC20 _rewardToken) public onlyAdmin {
  ...
  function approveIDO(address token) public onlyAdmin {
    IERC20(token).approve(IDO_CONTRACT, 115792089237316195423570985008687907853269984665640564039457584007913129639935);
  }
}
