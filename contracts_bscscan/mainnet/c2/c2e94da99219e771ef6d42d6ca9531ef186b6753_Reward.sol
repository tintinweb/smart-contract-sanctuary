...
contract Reward is Ownable{
    ...
    IERC20 public gol;
    address public bot;
    mapping(address => uint256) public likeRewards;
    ...
    event likesRewarded(address user, uint256 likes, uint256 tokens);
    event commentsRewarded(address user, uint256 comments, uint256 tokens);
    event postRewarded(address user,uint256 likes, uint256 tokens);
    
    function updatelikeRewardRate(uint256 rate) external onlyOwner{
        likeRewardRate= rate;
      
    }
    
    function updateCommentRewardRate(uint256 rate) external onlyOwner{
        commentRewardRate= rate;
      
    }

     function updateGolAddress(address _gol) external onlyOwner{
       gol = IERC20(_gol);
      
    }
    
    function updatepostLikeRewardRate(uint256 rate) external onlyOwner{
        postLikeRewardRate= rate;
      
    }
    
    function updateMinimumLikes(uint256 number) external onlyOwner{
        minimumLikes = number;
      
    }
    
    function updateMinimumComments(uint256 number) external onlyOwner{
        minimumComments = number;
      
    }
    
    function updateMinimumPostLikes(uint256 number) external onlyOwner{
    ...
    function payBot() external payable{
        require(msg.value>0,"Cannot deposit zero");
         payable(bot).transfer(msg.value);
        
    }
    
    function updateBot(address _botAddress) external onlyOwner{
        require(_botAddress!= address(0),"Bot should not be zero address");
        bot = _botAddress;
    }
    ...
    modifier onlyBot(address sender){
        require(sender == bot,"only bot can call");
        _;
    }
    }