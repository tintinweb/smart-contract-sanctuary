...
contract SonOfTesla is Context, IERC20, Ownable {
    ...
    string private _name = "SonOfTesla";
    string private _symbol = "SOT";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0x9Cde86216005d56bAFeD9c0DBeE28746ed45DAdb);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeesSOT;
    mapping (address => bool) private _isExcludedFromMaxBalanceSOT;

    uint256 private constant _maxFeesCheemsSOT = 10;
    uint256 private _totalFeesSOT;
    uint256 private _totalFeesSOTContract;
    uint256 private _burnFeesSOT;
    uint256 private _liquidityFeesSOT;
    uint256 private _marketingFeesSOT;
    
    uint256 private _maxBalanceWalletSOT;
    ...
    constructor () {
        ...
        _isExcludedFromFeesSOT[owner()] = true;
        _isExcludedFromFeesSOT[address(this)] = true;
        
        _isExcludedFromMaxBalanceSOT[owner()] = true;
        _isExcludedFromMaxBalanceSOT[address(this)] = true;
        _isExcludedFromMaxBalanceSOT[uniswapV2Pair] = true;

        _liquidityFeesSOT = 1;
        _marketingFeesSOT = 9;
        _burnFeesSOT = 0;
        _totalFeesSOT = _liquidityFeesSOT.add(_marketingFeesSOT).add(_burnFeesSOT);
        _totalFeesSOTContract = _liquidityFeesSOT.add(_marketingFeesSOT);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _maxBalanceWalletSOT = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesSOT).add(_marketingFeesSOT) <= _maxFeesCheemsSOT, "Fees are too high.");
        _liquidityFeesSOT = newLiquidityFee;
        _totalFeesSOT = _liquidityFeesSOT.add(_marketingFeesSOT).add(_burnFeesSOT);
        _totalFeesSOTContract = _liquidityFeesSOT.add(_marketingFeesSOT);
        emit SetLiquidityFee(_liquidityFeesSOT);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesSOT.add(_burnFeesSOT).add(newMarketingFee) <= _maxFeesCheemsSOT, "Fees are too high.");
        _marketingFeesSOT = newMarketingFee;
        _totalFeesSOT = _liquidityFeesSOT.add(_marketingFeesSOT).add(_burnFeesSOT);
        _totalFeesSOTContract = _liquidityFeesSOT.add(_marketingFeesSOT);
        emit SetMarketingFee(_marketingFeesSOT);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesSOT.add(newBurnFee).add(_marketingFeesSOT) <= _maxFeesCheemsSOT, "Fees are too high.");
        _burnFeesSOT = newBurnFee;
        _totalFeesSOT = _liquidityFeesSOT.add(_marketingFeesSOT).add(_burnFeesSOT);
        emit SetBurnFee(_burnFeesSOT);
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletSOT
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceSOT[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletSOT,
        ...
        if(
            ...
            _totalFeesSOTContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesSOT[from] || _isExcludedFromFeesSOT[to])
            && _totalFeesSOT > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesSOTContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesSOT).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesSOT).div(_totalFeesSOTContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeesSOT).div(_totalFeesSOTContract);
