...
contract EmergencyLove is Ownable, ReentrancyGuard {

    uint256 public constant interactPrice = 0.015 ether;
    address public immutable LoveToken;
    address public immutable CatnipToken;
    address public immutable Carl;
    address public immutable NFTContract;
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;

constructor(address _LoveToken, address _Carl, address _CatnipToken, address _MyRevengeNFT) Ownable() {
    Carl = _Carl;
...
function emergencyPlay() public {
    uint256 boredom = MyRevengeCat(Carl).getBoredom();
    require(boredom < 11, "I'm not bored enough!");
    MyRevengeCat(Carl).play{value: interactPrice}();
    }

function emergencySleep() public {
    uint256 sleepiness = MyRevengeCat(Carl).getSleepiness();
    require(sleepiness < 11, "I'm not sleepy enough!");
    MyRevengeCat(Carl).sleep{value: interactPrice}();
    }

function emergencyClean() public {
    uint256 cleanliness = MyRevengeCat(Carl).getUncleanliness();
    require(cleanliness < 11, "I'm not dirty enough!");
    MyRevengeCat(Carl).clean{value: interactPrice}();
    }

function emergencyFeed() public {
    uint256 hunger = MyRevengeCat(Carl).getHunger();
    require(hunger < 11, "I'm not hungry enough!");
    MyRevengeCat(Carl).feed{value: interactPrice}();
    }

function available() public view returns (uint256) {
    return Love(LoveToken).balanceOf(address(this));
    }

function claim() public nonReentrant {
    uint256 currentLove = available();
    require(MyRevengeNFT(NFTContract).balanceOf(msg.sender) > 0, "Must own the NFT"); //Need to own an NFT to call this function.
    require(currentLove > 0, "wut? You can't claim 0 LOVE lmao"); //lmao
    Catnip(CatnipToken).transferFrom(msg.sender, deadAddress, 30000e18); //Burns 30000 Catnip tokens. Oh no!!
    Love(LoveToken).transferFrom(address(this), msg.sender, currentLove); //take the LOVE!
    }
}
