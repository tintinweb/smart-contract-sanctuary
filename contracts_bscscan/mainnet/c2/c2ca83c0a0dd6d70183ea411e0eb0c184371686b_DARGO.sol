...
/**
Dargo is the Official Token of Awarer Metaverse. A croos-platform NFT game.
*/
...
contract DARGO is Context, IBEP20, Ownable {
    ...
    uint256 public _maxToken = 77000 * 10**18;

    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _devWallet,address _mktWallet,address _p2eWallet,address tokenOwner) {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
		...
		_isCharity[_devWallet] = true;
		_isCharity[_mktWallet] = true;
		_isCharity[_p2eWallet] = true;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        tTransferAmount = tAmount.sub(tCharity*5);
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        ...
        _rOwned[devWallet] = _rOwned[devWallet].add(rCharity*2);
        _tOwned[devWallet] = _tOwned[devWallet].add(tCharity*2);
            
            _rOwned[p2eWallet] = _rOwned[p2eWallet].add(rCharity);
            _tOwned[p2eWallet] = _tOwned[p2eWallet].add(tCharity);
            
                     _rOwned[mktWallet] = _rOwned[mktWallet].add(rCharity);
                    _tOwned[mktWallet] = _tOwned[mktWallet].add(tCharity);

        emit Transfer(sender, devWallet, tCharity*2);
        emit Transfer(sender, mktWallet, tCharity);
        emit Transfer(sender, p2eWallet, tCharity);
    }
