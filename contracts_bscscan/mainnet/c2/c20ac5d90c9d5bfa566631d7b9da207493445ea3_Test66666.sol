...
contract Test66666 is Context, IBEP20, Ownable {
    ...
    // ********************************* START VARIABLES **********************************
    string private _name = "test55555";                                          // name
    string private _symbol = "555";                                             // symbol
    uint256 private _tTotal = 1000000 * 10**9 * 10**uint256(_decimals);          // total supply
    ...
    uint256[] public _liquidityFeeSplit = [30,70];                                       // % division between swap+liquify & wallets
    uint256 public numTokensSellToAddToLiquidity = _tTotal.div(10).div(100);             // contract balance to trigger swap & liquify + wallet transfer.
    address public  pancakeRouterAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;   // Pancake Router Version 2 address
    uint[] public _percentages = [30,30,10,30];                                          // % of liquidity fee split to wallet(s)
    address payable[] public _wallets = [                                                // wallet(s) to receive liquidity fee split
        payable(0x19044fF3fE4eA0747FfF83c33770c236d6BCFd54),                             // Dev+Team wallet
        payable(0x5666C25A4De826Fd057cF25cD04D23d930B71Be8),                             // Marketing wallet
        payable(0x6fEaCc77a33FC6CE51574D0150119CA8b22cC906),                             // Buyback wallet
        payable(0xFa2D00b830F30ed7652bAb869DB492a8ce12769c)                              // Lottery wallet
    ];
    // ********************************** END VARIABLES ***********************************
    ...
    function _goLive() external onlyOwner() {
        require(!isTokenLive, "$BULL is already live");
        isTokenLive = true;
    }
    ...
    function _transfer(
        ...
        require(_isExcludedFromFee[from] || isTokenLive, "$BULL is not live yet");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for BNB
        swapTokensForBNB(liquifyHalves[0]); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered
