...
/**
   #HHST features:
   0.5% fee auto add to the liquidity pool to locked forever when selling
   0.5% fee auto add to Team wallet
...
contract Ownable is Context {
    ...
    modifier onlyTeam() {
        require(_Team == _msgSender(), "Caller is not the Team address");
        _;
    }
    ...
    function setTeamAddress(address payable TeamAddress) public virtual onlyOwner
    {
        require(_Team == address(0), "Team address cannot be changed once set");
        _Team = TeamAddress;
    }
...
contract MyTest is Context, IERC20, Ownable {
    ...
    uint256 private _TeamPercentageOfLiquidity = 50;
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 private _totalTeamCollected = 0;
    ...
    event TeamCollected(uint256 ethCollected);
    ...
    function TeamPercentageOfLiquidity() public view returns (uint256)
    {
        return _TeamPercentageOfLiquidity;
    }
    
    function totalTeamCollected() public view returns (uint256)
    {
        return _totalTeamCollected;
    }
    ...
    function _transfer(
        ...
        require(to != Team(), "The Team address cannot receive tokens");
        require(from != Team(), "The Team address cannot send tokens");
    ...
    function collectTeam() public onlyTeam
    {
        _totalTeamCollected = _totalTeamCollected.add(address(this).balance);
        emit TeamCollected(address(this).balance);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // this also ignores any ETH that was reserved for Team last time
        // this was called
        uint256 initialBalance = address(this).balance;

        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> MYT swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        
        
        //Now reserve some of that eth for Team to collect
        uint256 balanceToTeam = (newBalance.mul(TeamPercentageOfLiquidity())).div(10**2);
        uint256 tokensExtra = (otherHalf.mul(TeamPercentageOfLiquidity())).div(10**2);
         
        //How much eth is left for liquidity?
        newBalance = newBalance.sub(balanceToTeam);
