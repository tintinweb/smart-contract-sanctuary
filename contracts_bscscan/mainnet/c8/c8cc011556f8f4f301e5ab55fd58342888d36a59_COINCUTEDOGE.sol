...
/**
 *Youngest son of Doge,The only doge with auto-claim feature of BNB,With practical use cases and utilities.
 *Official website:https://cutedogecoin.finance
 *Twitter:https://twitter.com/CuteDOGE_Coin
 *Telegram:https://t.me/CuteDOGE_Coin
*/
...
contract COINCUTEDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CUTEDOGEDividendTracker public dividendTracker;
    ...
    uint256 public fixedSaleStartTimestamp = 1625925000; //July 10, 2:00 UTC, 2021
    ...
    uint256 public tradingEnabledTimestamp = 1625925600; //July 10, 2:00 UTC, 2021
    ...
    constructor() public ERC20("CUTEDOGE", "CUTEDOGE") {
    	...
    	dividendTracker = new CUTEDOGEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CUTEDOGE: The dividend tracker already has that address");

        CUTEDOGEDividendTracker newDividendTracker = CUTEDOGEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CUTEDOGE: The new dividend tracker must be owned by the CUTEDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CUTEDOGE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CUTEDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CUTEDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CUTEDOGE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CUTEDOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CUTEDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CUTEDOGE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "CUTEDOGE: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "CUTEDOGE: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "CUTEDOGE: The fixed-sale is only available to certain participants at the start");
            }
...
contract CUTEDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("CUTEDOGE_Dividend_Tracker", "CUTEDOGE_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "CUTEDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "CUTEDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CUTEDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CUTEDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CUTEDOGE_Dividend_Tracker: Cannot update claimWait to same value");
