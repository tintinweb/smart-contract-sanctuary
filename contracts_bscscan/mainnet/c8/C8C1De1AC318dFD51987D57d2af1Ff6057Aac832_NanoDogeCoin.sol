...
contract DividendTracker is DividendPayingToken {
  ...
  constructor(
    ...
    minimumTokenBalanceForDividends = 1_000_000_000 * 10**18; // must hold 10 billion tokens which equates to 0.0001% of the total NanoDogeCoin supply
  }
  ...
  ReentrancyGuard
{
  ...
  function addBotToList(address account) public onlyRole(DEFAULT_ADMIN_ROLE) { // Developers have the option to pinpoint and exclude bots from trading on launch.
  ...
  function excludeDividends(address exclude)
    external
    override
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
