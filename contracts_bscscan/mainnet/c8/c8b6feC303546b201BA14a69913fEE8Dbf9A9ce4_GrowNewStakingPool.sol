...
// File contracts/grow/GrowNewStakingPool.sol
...
contract GrowNewStakingPool is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        rawRewardTokenSwapThreshold = 1e17;
    ...
    /// @dev unclaimed reward collector
    ...
    function _swap(address tokenA, address tokenB, uint256 amount) internal returns (uint256) {
        ...
        uint256 tokenReceived = ISwapUtils(SWAP_UTILS).swap(tokenA, tokenB, amount);
    ...
    function updateSwapUtils(address _swapUtilsAddress) external onlyRole(CONFIGURATOR_ROLE)  {
    ...
    function totalBalance() public view returns(uint256) {
        return stakingToken.balanceOf(address(this)).sub(remainingRewardTokenAmount);
    }
    ...
    function checkIfNeedSwapRewardAndSwap() public {
        if (rawRewardToken.balanceOf(address(this)) > rawRewardTokenSwapThreshold) {
            // 1. update remaining reward
            updateReward();

            // 2. swap reward
            uint256 reward = _swap(address(rawRewardToken), address(stakingToken), rawRewardToken.balanceOf(address(this)));
    ...
    function _deposit(uint256 amount, address userAddress) private {
        ...
        // 2. check if need swap reward then Swap
        checkIfNeedSwapRewardAndSwap();

        // 3. shares
        uint256 sharesToAdd = amount.mul(_DECIMAL).div(pricePreShare());

        // 4. transfer
        stakingToken.safeTransferFrom(userAddress, address(this), amount);
    ...
    function pump(uint256 amount) external {
        rawRewardToken.safeTransferFrom(msg.sender, address(this), amount);

        // Nobody share the rewards, take it to dev :P
        if (totalSupply == 0) {
            rawRewardToken.safeTransfer(devAddress, amount);
        }
