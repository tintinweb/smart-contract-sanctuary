...
contract MYSTIC_StakePool is Ownable{
    ...
    uint256 public Pool = 7000000;
    uint256 public PoolBalance;
    uint256 public tokenDecimal;
    uint256 public poolMinStake = 10000;

    uint256 public poolNumber = 1;
    uint256 public poolRewardPercent = 150;
    uint256 public fullMaturityTime = 60 days; 
    uint256 public earlyPenalty = 10;
    uint256 public minLock = 15 days;
    ...
    function claimPool() public returns(bool){
        ...
        require(block.timestamp > users[msg.sender].pool_deposit_time + minLock,"Unstake time not reached");

        uint256 amount = users[msg.sender].poolBal;

        if(block.timestamp < users[msg.sender].pool_deposit_time + fullMaturityTime){
            uint256 afterPenalty = amount - ((amount*earlyPenalty)/100);
            require(Token(tokenAddr).transfer(msg.sender, afterPenalty),"Cannot Transfer Principal Funds");
            require(Token(tokenAddr).transfer(owner(), ((amount*earlyPenalty)/100)));
