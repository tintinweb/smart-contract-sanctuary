...
/**
*
___  ___    ______      _____  _   _  _____ ______   ___  
|  \/  |    |  ___|    /  ___|| | | ||_   _|| ___ \ / _ \ 
| .  . |    | |_       \ `--. | |_| |  | |  | |_/ // /_\ \
| |\/| |    |  _|       `--. \|  _  |  | |  | ___ \|  _  |
| |  | | _  | |     _  /\__/ /| | | | _| |_ | |_/ /| | | |
\_|  |_/(_) \_|    (_) \____/ \_| |_/ \___/ \____/ \_| |_/

// https://www.miniflokishiba.com/
// https://twitter.com/miniflokishiba

Making #SHIBA great again!
A project with PancakeSwap locked liquidity, 4% auto-staking, 2% auto LP and 2% to 
the marketing and donation wallet.
We aim to reduce the pollution by donating to organizations involved in reducing
the Mini Floki Shiba footprint. The organizations will be voted by the community.

   #BEE
   
   #LIQ+#RFI+#SHIB+#DOGE = #Mini Floki Shiba
...
contract MFLOKIS is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    address BNBReward = 0xFf3Ce1bce88338146127436ac49a6ECF5b77de5B;
    ...
    string private _name = "Mini Floki Shiba";
    string private _symbol = "MFLOKIS";
    ...
    uint256 public _maxTxAmount = 250000000000 * 10**18;
    ...
    constructor() {
        ...
        _isExcludedFromFee[BNBReward] = true;
        _isExcludedFromReward[BNBReward] = true;
    ...
    function totalBNBreward() public view returns (uint256) {
        return balanceOf(BNBReward);
    }
    ...
    function _transferBurn(uint256 tBurn) private {
        uint256 currentRate = _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        _rOwned[BNBReward] = _rOwned[BNBReward].add(rBurn);
        if (_isExcludedFromReward[BNBReward]) _tOwned[BNBReward] = _tOwned[BNBReward].add(tBurn);
    }
    ...
    function _transferFromExcluded(
        ...
        emit TransferBurn(sender, BNBReward, tBurn);
