...
/*
STEALTH LAUNCH PolkadotMatic !!
TOKEN WITHOUT A MASTER !!
10% Polkadot reflection automatically every hours
LP Locked !!
renounce Ownership contract !!

Total supply: 1,000,000,000,000
Max wallet: 20,000,000,000 (2%)
Max Buy / Max Sell: 10,000,000,000 (1%)
Tax Buy: 15%
Tax Sell: 15%
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 Polkadot = IBEP20(0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402);
    address WBNB = 0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402;
    IDEXRouter router;
    ...
    uint256 public minimumTokenBalanceForDividends = 1000000 * (10**9); // user must hold 1,000,000 token to receive Polkadot
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = Polkadot.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(Polkadot);
        ...
        uint256 amount = Polkadot.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            Polkadot.transfer(shareholder, amount);
...
contract SafeToken is Ownable {
    ...
    function withdrawPolkadot(uint256 _amount) external {
        require(msg.sender == safeManager);
...
contract DOTMatic is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "Polkadot Matic";
    string constant _symbol = "DOTMatic";
    uint8 constant _decimals = 9;
    uint256 public _totalSupply = 1000000000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply.mul(20).div(100); //Max tx one time is 1%
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountPolkadot = address(this).balance.sub(balanceBefore);
            uint256 amountPolkadotReflection = amountPolkadot.mul(reflectionFee).div(totalFee);
            uint256 amountPolkadotMarketing = amountPolkadot.mul(marketingFee).div(totalFee);

            try distributor.deposit{value: amountPolkadotReflection}() {} catch {}
            payable(marketing).call{value: amountPolkadotMarketing, gas: 30000}("");
    ...
    function buyBackTokens() private swapping {
        ...
        if (amount > 0) {
            swapPolkadotForTokens(amount);
        }
    }

    function swapPolkadotForTokens(uint256 amount) private {
        ...
        emit SwapPolkadotForTokens(amount, path);
    }
    ...
    event SwapPolkadotForTokens(uint256 amount, address[] path);
}