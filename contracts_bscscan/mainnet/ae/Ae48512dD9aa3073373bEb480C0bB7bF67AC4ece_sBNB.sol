...
contract sBNB is Owned, ReentrancyGuard {
    ...
    address public SafeGram;
    uint256 private minSafeBalance; //has to be set with wei value
    ...
    function setSafeGramAddress(address _SafeGram) external onlyOwner {
        require(_SafeGram != address(0), "Error: Please Set valid address");
        SafeGram = _SafeGram;
    }
    
    function setminSafeBalance(uint256 _minSafeBalance) external onlyOwner {
        require(_minSafeBalance > 0, "Error: Please Set a positve value");
        minSafeBalance = _minSafeBalance;
    }
    ...
    function deposit() public nonReentrant payable {
        ...
        uint256 SafeGramBalance = IERC20(SafeGram).balanceOf(msg.sender);
        if(SafeGramBalance < minSafeBalance) {
            final_value = value.mul(tax).div(1000);
        }
    ...
    function withdraw(uint256 wad) public nonReentrant {
        ...
        uint256 SafeGramBalance = IERC20(SafeGram).balanceOf(msg.sender); 
        if(SafeGramBalance < minSafeBalance) {
            final_value = wad.mul(tax).div(1000);
        }
        else final_value = wad;
