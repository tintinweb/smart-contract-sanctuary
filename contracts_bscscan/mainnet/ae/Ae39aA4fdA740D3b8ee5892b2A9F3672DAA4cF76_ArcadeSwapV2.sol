...
contract ArcadeSwapV2 is AbstractArcadeUpgradeable {
    ...
    // time duration between tx
    uint256 public txDuration;

    // <user address, <game id => timestamp>>
    mapping(address => mapping(uint256 => uint256)) internal _lastTxTime; // last transaction time per user
    ...
    */
     ...
     */
    function buyGamePoint(uint256 _id, uint256 _amount) public virtual nonReentrant {
        require(_id != 0, "game id can't be zero");
        require(_amount != 0, "amount can't be zero");
        require(_gameKeys[_id] !=  0, "Not registered game key.");
        require(gamePointPrice[_id] != 0, "Not registered game point price.");
        require(
            block.timestamp - _lastTxTime[msg.sender][_id] >= txDuration,
        ...
        _lastTxTime[msg.sender][_id] = block.timestamp;
     ...
     * @param _snapshot timestamp to accept request
    ...
    function sellGamePoint(
        ...
        require(
            _lastTxTime[msg.sender][_id] < _snapshot,
    ...
    /**
     * @notice Return last transaction time per user
     * @param _user user address
     * @param _id game id
     * @return last transaction time
     */
    function lastTxTime(address _user, uint256 _id) external view returns (uint256) {
        return _lastTxTime[_user][_id];
    }
...
abstract contract AbstractArcadeUpgradeable is
	Initializable,
	OwnableUpgradeable,
