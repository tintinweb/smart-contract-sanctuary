...
contract GremlinStaking is Ownable {
    ...
    address public ecoSystemWallet = 0x25C607652cCb73059a8999cEC37aAe695829Ac77;
    address public marketingWallet = 0x2AF2C9381318077F1068d2bFBd346C55E40863E3;
    
    uint256 whaleFee = 2000; // 2000/10000 * 100 = 20% 
    uint256 whaleFeeNumber = 1000000 * (10**18);
    ...
    uint256 public GREMPerBlock = 5 * (10**18);
    ...
    constructor () {
        ...
        GREM = IERC20(0x8c458EE3ebc5DaDFE5E0A3Daae47e3C2Aa62FF1c);
    ...
    function addRewardToPool (uint256 _amount) public  {
        require(GREM.balanceOf(msg.sender) >= _amount, "Insufficient GREM tokens for transfer");
        totalPool = totalPool.add(_amount);
        GREM.transferFrom(msg.sender, address(this), _amount);
        emit RewardPoolUpdated(totalPool);
    }

    
    function setGREMPerBlock (uint256 _amount) public onlyOwner {
        require(_amount >= 0, "GREM per Block can not be negative" );
        GREMPerBlock = _amount;
    }
    ...
    function enterStaking (uint256 _amount) public  {
        ...
        if (_amount >= whaleFeeNumber || stakings[msg.sender].stakedAmount >= whaleFeeNumber ){
            ...
            // tax whales % 
            uint256 whaleFeeAmount = _amount.mul(whaleFee).div(divisor);
            _amount = _amount.sub(whaleFeeAmount);
            GREM.transfer(ecoSystemWallet, whaleFeeAmount);
            ...
            GREM.transfer(msg.sender, getReward(msg.sender));
    ...
    function leaveStaking (uint256 _amount) public  {
        ...
        if(block.number < stakings[msg.sender].lastBlockStaked.add(earlyWithdrawFeeTime)){
            ...
            GREM.transfer(ecoSystemWallet, withdrawalFee);
        }
        _amount = _amount.add(reward);
        GREM.transfer(msg.sender, _amount);
        //remove from array
        if(stakings[msg.sender].stakedAmount == 0){
    ...
    function getReward(address _address) internal view returns (uint256) {
        if(block.number <=  stakings[_address].lastBlockCompounded){
            ...
            if(totalPool == 0 || totalStaked ==0 ){
                ...
                uint256 totalReward = blocks.mul(GREMPerBlock);
    ...
    /// @dev Calculates potential reward based on GREM per block
    ...
    /// @dev calculates potential reward for the address provided based on GREM per block
    ...
    function claim () public  {
        ...
        GREM.transfer(msg.sender, reward);
        stakings[msg.sender].lastBlockCompounded = block.number;
    ...
    function singleCompound () public  {
        require(stakings[msg.sender].stakedAmount > 0, "Please Stake GREM to compound");
        
        uint256 reward = getReward(msg.sender);
        if(stakings[msg.sender].stakedAmount >= whaleFeeNumber)
        {
            uint256 whaleFeeAmount = reward.mul(whaleFee).div(divisor);
    ...
    function emergencyWithdraw() public {
        GREM.transfer( msg.sender, stakings[msg.sender].stakedAmount);
    ...
    // sends collected btc dividents to marketing wallet
    function sendBtcDividents() public onlyOwner {
       BTC.transferFrom(address(this), marketingWallet, BTC.balanceOf(address(this)));
    }
    ...
    function emergencyTotalPoolWithdraw () public onlyOwner {
        ...
        GREM.transfer(msg.sender, totalPool);
        totalPool = 0;
    }
