...
contract PrizePool is ERC721, Ownable {
    ...
    constructor() public ERC721("Prize Pool", "PrizePool ") {}
    
    
    uint256 public thePrice = 25000000000000000;

    function newTokenItem(address player, uint8[4] memory nftNumb, uint256 _amount, uint256 _issueIndex) internal 
        returns (uint256)
    {
        _tokenIds.increment();
    ...
    function reveal() public {
        require(hasRevealed[msg.sender] != true, 'has revealed');
    ...
    function buyToken() public payable {
        require(msg.value == thePrice, "must send at least 0.025 bnb");
        for(uint i=0; i<1; i++){
          newTokenItem(msg.sender,[1,2,3,4],1,1);
        }
    }
    
    function mintTokes(address _receiver, uint256 amount) public onlyOwner {
        for(uint256 i=0; i<amount; i++){
            newTokenItem(_receiver,[1,2,3,4],1,1);
        }
    }
    ...
    function getNumbers(uint256 tokenId) external view returns (uint8[4] memory) {
        return lotteryInfo[tokenId];
    }
    function getAmount(uint256 tokenId) external view returns (uint256) {
        return lotteryAmount[tokenId];
    }
    function getIssueIndex(uint256 tokenId) external view returns (uint256) {
