...
contract ART{
    ...
    uint256 ACTIVATION_TIME = 1619692333;
    ...
    event onReinvestment(
        address indexed customerAddress,
        uint256 ARTReinvested,
        uint256 tokensMinted
    );

    event onWithdraw(
        address indexed customerAddress,
        uint256 ARTWithdrawn
    );
    ...
    string public name = "STABLE Artonline";
    ...
    constructor() public {
        ...
        erc20 = TOKEN(address(0x0f931615BC672514F4a5B0370bC33B416ef352b3));
    }

    function checkAndTransferART(uint256 _amount) private {
    ...
    function sell(uint256 _amountOfTokens) onlyTokenHolders public {
        ...
        uint256 _taxedART= SafeMath.sub(_amountOfTokens, _undividedDividends);
        ...
        int256 _updatedPayouts = (int256) (profitPerShare_ * _amountOfTokens + (_taxedART* magnitude));
        ...
        emit onTokenSell(_customerAddress, _amountOfTokens, _taxedART, now);
    }
    ...
    function totalARTBalance() public view returns (uint256) {
    ...
    function sellPrice() public view returns (uint256) {
        uint256 _ART= 1e18;
        uint256 _dividends = SafeMath.div(SafeMath.mul(_ART, exitFee_), 100);
        uint256 _taxedART= SafeMath.sub(_ART, _dividends);
    ...
    function buyPrice() public view returns (uint256) {
        uint256 _ART= 1e18;
        uint256 _dividends = SafeMath.div(SafeMath.mul(_ART, entryFee_), 100);
        uint256 _taxedART= SafeMath.add(_ART, _dividends);

        return _taxedART;
    }

    function calculateTokensReceived(uint256 _ARTToSpend) public view returns (uint256) {
        uint256 _dividends = SafeMath.div(SafeMath.mul(_ARTToSpend, entryFee_), 100);
        uint256 _amountOfTokens = SafeMath.sub(_ARTToSpend, _dividends);

        return _amountOfTokens;
    }

    function calculateARTReceived(uint256 _tokensToSell) public view returns (uint256) {
        require(_tokensToSell <= tokenSupply_);
        uint256 _dividends = SafeMath.div(SafeMath.mul(_tokensToSell, exitFee_), 100);
        uint256 _taxedART= SafeMath.sub(_tokensToSell, _dividends);
    ...
    function purchaseTokens(address _referredBy, address _customerAddress, uint256 _incomingART) internal isActivated returns (uint256) {
        if (getInvested() == 0) {
          totalHolder++;
        }

        invested_[msg.sender] += _incomingART;

        uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingART, entryFee_), 100);
        ...
        uint256 _amountOfTokens = SafeMath.sub(_incomingART, _undividedDividends);
        ...
        emit onTokenPurchase(_customerAddress, _incomingART, _amountOfTokens, _referredBy, now);
