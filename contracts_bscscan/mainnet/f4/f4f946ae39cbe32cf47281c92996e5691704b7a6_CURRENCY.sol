    ...
    function get_currency(address _currencyAddress) public view returns(uint256 wbnb_rate,uint256 wbnb_token,uint256 busd_rate,uint256 busd_token,address token0, address token1){
        ...
        if(_currencyAddress==c.token0) {
            if(c.token1==wbnb){
                ...
                rate = 1e6 * uint(_reserve0)/uint(_reserve1) * uint(wbnb_reserve)/uint(busd_reserve);
                token = 1e6 * uint(_reserve1)/uint(_reserve0) * uint(busd_reserve)/uint(wbnb_reserve);
            } else { //token1==busd
            ...
            if(c.token0==wbnb){
                (uint112 wbnb_reserve, uint112 busd_reserve,)=Token(wbnb_busd_pair).getReserves(); 
                rate = 1e6 * uint(_reserve1)/uint(_reserve0) * uint(wbnb_reserve)/uint(busd_reserve);
                token = 1e6 * uint(_reserve0)/uint(_reserve1) * uint(busd_reserve)/uint(wbnb_reserve);
            } else { //token0==busd
