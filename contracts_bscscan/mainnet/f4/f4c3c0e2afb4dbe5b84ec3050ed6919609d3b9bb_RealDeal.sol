...
.
. 
. World's first fully automated decentralized trading competitions. 
. Every step of the competition is fully controlled by smart contracts.
. Unique fair tax system. Pay tax only if you make a profit.
. 
...
contract RealDeal is IBEP20, Context, Ownable {
    ...
    uint256 private constant _MAX_TAX_LIMIT = 5;
    uint256 private constant _minTokensForFeeConversion = 200 * _DECIMALFACTOR;
    ...
    uint256 private _maxTokensPerAccount = 5000 * _DECIMALFACTOR;
    uint256 private _minTokensToPurchase = 50 * _DECIMALFACTOR;
    ...
    constructor(address coreAddress, address marketingAddress, address developmentAddress) {
        ...
        _isSystemAddress[developmentAddress] = true;
     ...
     * Only used in case the current fee percentages are not sustainable and needs to be tuned.
     */
    function setProjectTaxPercentage(uint256 newDevelopmentFeePercentage) external onlyOwner {
        require(newDevelopmentFeePercentage <= _MAX_TAX_LIMIT, "REALDEAL: EXCEEDS_LIMIT");
        developmentFeePercentage = newDevelopmentFeePercentage;
        emit DevelopmentFeeChanged(newDevelopmentFeePercentage);
    }
    ...
    function setMaxTokensPerAccount(uint256 amount) external onlyOwner {
        _maxTokensPerAccount = amount.mul(_DECIMALFACTOR);
    ...
    function setMinTokensToPurchase(uint256 amount) external onlyOwner {
        _minTokensToPurchase = amount.mul(_DECIMALFACTOR);
    ...
    function aramEmergencyRecovery(bool onlyCore) external onlyOwner {
        ...
        if (!onlyCore) {
            (bool success, ) = _marketingAddress.call{value: address(this).balance}("");
            require(success, "REALDEAL: BNB_TX_FAILED");
        }
    ...
    function _handlePurchaseTransfer(address buyer, uint256 amount) private {
        if (_automaticAirdropInProgress) {
            if (_core.getRewardCount(competitionIndex - 1) == 0) {
                ...
                (bool success, ) = payable(address(_core)).call{value: rewardPoolSize}("");
                
                if (!success || !_core.aramStart(competitionIndex - 1)) {
                    _automaticAirdropInProgress = false;
        ...
        if (!_isSystemAddress[buyer]) {
            ...
            require(
                amount >= _minTokensToPurchase,
                "REALDEAL: AMOUNT_BELOW_MIN_LIMIT"
            );

            require(
                amount <= _maxTokensPerAccount,
    ...
    function _handleSellTransfer(address seller, uint256 amount) private {
        ...
        if (shouldTax) {
            ...
            uint256 marketingPoolAmount = amount.mul(marketingPoolPercentage).div(100);
            uint256 developmentAmount = amount.mul(developmentFeePercentage).div(100);

            uint256 feeAmount = rewardPoolAmount.add(marketingPoolAmount).add(developmentAmount);
    ...
    function _tryFeeConversion() private lockFeeConversion {
        ...
        uint256 totalFeePercentage = rewardPoolPercentage.add(marketingPoolPercentage).add(developmentFeePercentage);
        ...
        if (transferredBalance > 0) {
            if (marketingPoolPercentage > 0) {
                ...
                (bool success, ) = _marketingAddress.call{value: marketingSlice}("");
                require(success, "REALDEAL: BNB_TX_FAILED");
            }
                
            if (developmentFeePercentage > 0) {
                developmentSlice = transferredBalance.mul(developmentFeePercentage).div(totalFeePercentage);
                (bool success, ) = _developmentAddress.call{value: developmentSlice}("");
                require(success, "REALDEAL: BNB_TX_FAILED");
            }
        }
        
        emit FeeConversionTriggered(transferredBalance.sub(marketingSlice).sub(developmentSlice), marketingSlice, developmentSlice);
    }
}