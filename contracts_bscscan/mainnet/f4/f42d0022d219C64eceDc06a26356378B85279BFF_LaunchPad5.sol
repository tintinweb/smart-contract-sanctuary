...
contract LaunchPad5 is ERC20, Ownable {
    // CONFIG START
    
    // TOKEN
    string tokenName = "LaunchPad5";
    string tokenSymbol = "LPAD5";
    uint256 tokenTotalSupply = 505_050_505 * (10**18);
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcluded[account], "LPAD5: Account already excluded");

        isExcluded[account] = true;
    }

    function unExclude(address account) public onlyOwner {
        require(isExcluded[account], "LPAD5: Account is not excluded");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        ...
        if(!isExcluded[recipient] && sender == address(_UniswapV2Pair)) {
            if(nextBuyBlock[recipient] > block.number) {
                revert("LPAD5: Cooldown");
            }
            ...
            if(block.number <= startBlock + 3) {
                tax = 90;
            } else if(block.number <= startBlock + 6) {
                nextBuyBlock[recipient] = block.number + 5;

                if(amount > tokenTotalSupply / 1000 * 5) {
                    revert();
                }
            } else if(block.number <= startBlock + 20) {
                if(amount > tokenTotalSupply / 1000 * 20) {
                    revert();
                }
            }
        }
