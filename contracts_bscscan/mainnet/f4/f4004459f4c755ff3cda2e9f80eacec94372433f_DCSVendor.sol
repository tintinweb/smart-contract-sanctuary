...
contract DCSVendor
{
    ...
    uint256 public immutable forSale; //max unit for sale
    uint256 public sold; //sold units
    address payable public immutable receiver; //addres to send BNB too    
    ...
    function getUserInfo(uint256 _index) external view returns (address, uint256)
    {
        address buyer = buyers[_index];
        UserInfo storage user = userMap[buyer];

        return (buyer, user.bought);
    }
