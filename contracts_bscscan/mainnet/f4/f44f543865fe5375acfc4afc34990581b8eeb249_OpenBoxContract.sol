...
library UintArrayLib {
    function remove(uint256[] storage self, uint256 element) internal returns (bool) {
    ...
    function removeAt(uint256[] storage self, uint256 index) internal {
        self[index] = self[self.length - 1];
        self.pop();
    }

    function exists(uint256[] storage self, uint256 element) internal view returns (bool) {
        for (uint256 i = 0; i < self.length; i++) {
            if (self[i] == element) {
                return true;
            }
    ...
    function shuffle(uint256[] storage self) internal {
        uint256 salt = uint256(keccak256(abi.encodePacked(block.timestamp)));
        for (uint256 i = 0; i < self.length; i++) {
            uint256 n = i + (salt % (self.length - i));
            uint256 temp = self[n];
            self[n] = self[i];
            self[i] = temp;
        }
...
contract OpenBoxContract is ERC1155Receiver, AccessControl {
    using UintArrayLib for uint256[];

    address public boxAddress;
    address public rewardAddress;
    address public rewardWallet;
    uint256 public unboxDay;
    ...
    function setBoxAddress(address _boxAddress) external onlyAdmin {
        boxAddress = _boxAddress;
    }

    function setUnboxDay(uint256 _unboxDay) external onlyAdmin {
    ...
    function shuffleRewards() external onlyAdmin {
        _rewards.shuffle();
    }
    ...
    function onERC1155Received(
        ...
        require(_msgSender() == boxAddress && id == 1, "Error: Not accept");
        require(value == 1, "Error: Amount must equal 1");
        require(unboxDay <= block.timestamp, "Error: The unboxing date hasn't come yet");
        require(_rewards.length >= 1, "Error: Not enough reward");
    ...
    function onERC1155BatchReceived(
        ...
        require(ids.length == 0, "Error: Not accept");
