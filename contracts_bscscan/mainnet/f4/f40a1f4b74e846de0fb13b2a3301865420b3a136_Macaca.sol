...
//SPDX-License-Identifier: MIT
	 ...
	 * This allows applications to reconstruct the allowance for all acMacaca just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also Macacaed.
	...
	// File: contracts/MacacaDividendPayingToken.sol
	...
	// File: contracts/Macaca.sol

	// 

	pragma solidity ^0.7.0;

	contract Macaca is ERC20, Ownable {
		using SafeMath for uint256;

		IuniswapV2Router02 private uniswapV2Router;
		address private immutable uniswapV2Pair;

		address payable public _MarketingAddress = payable(0x16326917ACd678948f0545eeb43727a0D62dBE02); 
	
		bool private swapping;

		MacacaDividendTracker private dividendTracker;
		...
		event ExcludeMultipleAcMacacaFromFees(address[] acMacaca, bool isExcluded);
		...
		constructor() ERC20("Macaca Coin", "MACACA"){
			...
			dividendTracker = new MacacaDividendTracker();
		...
		function excludeFromFees(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "Macaca: Account is already the value of 'excluded'");
		...
		function excludeMultipleAcMacacaFromFees(address[] calldata acMacaca, bool excluded) public onlyOwner {
			for(uint256 i = 0; i < acMacaca.length; i++) {
				_isExcludedFromFees[acMacaca[i]] = excluded;
			}

			emit ExcludeMultipleAcMacacaFromFees(acMacaca, excluded);
		}

		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != uniswapV2Pair, "Macaca: The uniswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "Macaca: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "Macaca: The liquidity wallet is already this address");
		...
		function updateGasForProcessing(uint256 newValue) private onlyOwner {
			require(newValue >= 200000 && newValue <= 500000, "Macaca: gasForProcessing must be between 200,000 and 500,000");
			require(newValue != gasForProcessing, "Macaca: Cannot update gasForProcessing to same value");
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!hasLaunched) {
				require(canTransferBeforehasLaunched[from], "Macaca: This account cannot send tokens until trading is enabled");
			}
	...
	contract MacacaDividendTracker is DividendPayingToken, Ownable {
		using SafeMath for uint256;
		using SafeMathInt for int256;
		...
		constructor() public DividendPayingToken("Macaca_Dividend_Tracker", "Macaca_Dividend_Tracker") {
			claimWait = 3600;
			minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
		}

		function _transfer(address, address, uint256) internal override {
			require(true, "Macaca_Dividend_Tracker: No transfers allowed");
		}
		...
		function updateClaimWait(uint256 newClaimWait) external onlyOwner {
			require(newClaimWait >= 1 && newClaimWait <= 86400, "Macaca_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
			require(newClaimWait != claimWait, "Macaca_Dividend_Tracker: Cannot update claimWait to same value");
