...
/**
 * Developed By: DeFi Speculate ==> https://t.me/defispeculate
 * CREATED BY: SAFEMOON MARK
 * Next Gen DeFi Token, Neglecting the need for Liquidity Pools
 * No need for LPs, Private Sales, or Presales
 * We welcome all forks, this has some serious potential
 ...
 * Next Gen DeFi Token - Pegged to the price of KVLR
 * 
 */
contract SurgeKVLR is IERC20, Context, Ownable {
    ...
    // Address for KVLR
    address public _kvlr = 0x77d78F28b67AcB7534c2B67C677628767155548e;
    // Address for Pancakeswap Router V2
    IUniswapV2Router02 router;
    // token data
    string constant _name = "SurgeKVLR";
    string constant _symbol = "SKVLR";
    uint8 constant _decimals = 0;
    // 100 Billion Total Supply
    uint256 _totalSupply = 100 * 10**9;
    // balances
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) _allowances;
    // fees
    uint256 public sellFee = 94;
    uint256 public spreadDivisor = 97;
    ...
    function purchase(address buyer, uint256 bnbAmount) internal returns (bool) {
        ...
        // previous amount of KVLR before we received any
        uint256 prevKvlrAmount = IERC20(_kvlr).balanceOf(address(this));
        // buy KVLR with the BNB we received
        buyKvlr(bnbAmount);
        // if this is the first purchase, use current balance
        uint256 currentKvlrAmount = IERC20(_kvlr).balanceOf(address(this));
        // number of KVLR we have purchased
        uint256 difference = currentKvlrAmount.sub(prevKvlrAmount);
        // if this is the first purchase, use new amount
        prevKvlrAmount = prevKvlrAmount == 0 ? currentKvlrAmount : prevKvlrAmount;
        ...
        uint256 nShouldPurchase = calculatedTotalSupply.mul(difference).div(prevKvlrAmount);
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        // how much KVLR are these tokens worth?
        uint256 amountKVLR = tokensToSwap.mul(calculatePrice());
        
        // send KVLR to Seller
        bool successful = IERC20(_kvlr).transfer(msg.sender, amountKVLR);
        if (successful) {
    ...
    /**
     * Buys KVLR with BNB in the contract, storing in the contract
     */ 
    function buyKvlr(uint256 amount) internal {
    ...
    function calculatePrice() public view returns (uint256) {
        uint256 kvlrBalance = IERC20(_kvlr).balanceOf(address(this));
        return kvlrBalance.div(_totalSupply);
    }
    ...
    function getKvlrQuantityInContract() public view returns(uint256){
        return IERC20(_kvlr).balanceOf(address(this));
    }
