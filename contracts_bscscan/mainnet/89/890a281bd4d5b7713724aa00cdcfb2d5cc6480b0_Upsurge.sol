...
contract Upsurge is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'UpSurge Token';
    string private constant _symbol = 'SURGE';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 100000 * 10**6 * 10**_decimals; //equals 100.000.000.000 token

    //Divider for the MaxBalance based on circulating Supply (1.5%)
    uint8 public constant BalanceLimitDivider=150;
    //Divider for the Whitelist MaxBalance based on initial Supply(0.3%)
    ...
    address public constant CharityWallet = 0xEa1580f5C6269fc54cd3465B9A72558124D8c5CD;
    //MainNet
    address private constant PancakeRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;//0x10ED43C718714eb63d5aA57B78B54704E256024E;
    
    address private DividentTokenAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;  // Binance-Peg BUSD Token
    ...
    function _isTeam(address addr) private view returns (bool){
        return addr==owner()||addr==CharityWallet;
    }
    ...
    constructor () {
        ...
        //contract gets 10% of the token to generate LP token and Charity Budget fase
        ...
        //be converted to LP and CharityBNB
        _liquidityTax=2;
        _stakingTax=1;
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if(amount<=10**(_decimals)) ClaimDivident(sender);
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            require(recipientBalance+amount<=balanceLimit,"whale protection");
        ...
        //Swapping AutoLP and CharityBNB is only possible if sender is not pancake pair, 
    ...
    //charity share starts at 80% to push initial charity, after start
    //its capped to 50% max, the percentage of the staking that gets used for
    //charity/paying the team
    uint8 public charityShare=80;
    ...
    //distributes bnb between charity share and dividents 
    function _distributeStake(uint256 BNBamount) private {
        // Deduct charity Tax
        uint256 charitySplit = (BNBamount * charityShare) / 100;
        uint256 amount = BNBamount - charitySplit;
        ...
        if (amount > 0) {
            ...
            //when there are 0 shares, add everything to charity budget
            if (totalShares == 0) {
    ...
    event OnWithdrawLINK(uint256 amount, address recipient);
    
    //withdraws all dividents of address
    function ClaimDivident(address addr) private{
        ...
        emit OnWithdrawLINK(amount, addr);
        _isWithdrawing=false;
    }
    ...
    //swaps the token on the contract for Charity BNB and LP Token.
    ...
    function _swapContractToken() private lockTheSwap{
        ...
        //splits the token in TokenForLiquidity and tokenForCharity
        uint256 tokenForLiquidity=(tokenToSwap*_liquidityTax)/totalTax;
        uint256 tokenForCharity= tokenToSwap-tokenForLiquidity;
        ...
        uint256 swapToken=liqBNBToken+tokenForCharity;
        ...
        //distributes remaining BNB between stakers and Charity
    ...
    function getTaxes() public view returns(uint256 burnTax,uint256 liquidityTax,uint256 charityTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
    ...
    // function TeamGetToBePaidAccounts() public onlyTeam view returns(mapping(address => uint256)){
    //      return toBePaid;
    // }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////
    //Settings//////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function TeamWithdrawCharityBNB(uint256 amount) public onlyTeam{
        require(amount<=charityBalance);
        charityBalance-=amount;
        (bool sent,) = CharityWallet.call{value: (amount)}("");
        require(sent,"withdraw failed");
    }

    //switches autoLiquidity and charity BNB generation during transfers
    ...
    function TeamSetTaxes(uint8 burnTaxes, uint8 liquidityTaxes, uint8 stakingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public onlyTeam{
        ...
        require(totalTax==100, "burn+liq+charity needs to equal 100%");
    ...
    //How much of the staking tax should be allocated for charity
    function TeamChangeCharityShare(uint8 newShare) public onlyTeam{
        require(newShare<=50);
        charityShare=newShare;
    }
    ...
    function TeamSetDividentTokenAddress(address dividentTokenAddress) public onlyTeam{
        DividentTokenAddress = dividentTokenAddress;
    }
    ...
    function TeamReleaseLiquidity() public onlyTeam {
        ...
        if(liquidityRelease20Percent)
        {
            ...
            amount=amount*2/10;
            liquidityToken.transfer(CharityWallet, amount);
        }
        else
        {
    ...
    function TeamRemoveRemainingBNB() public onlyTeam{
        ...
        _liquidityUnlockTime=block.timestamp+DefaultLiquidityLockTime;
        (bool sent,) =CharityWallet.call{value: (address(this).balance)}("");
        require(sent);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////
    //external//////////////////////////////////////////////////////////////////////////////////////////////
