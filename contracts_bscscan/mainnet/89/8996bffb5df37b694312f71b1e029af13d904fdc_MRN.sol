...
contract MRCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MRC_Dividend_Tracker", "MRC_Dividend_Tracker") {
        claimWait = 30;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MRC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MRC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MRC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 30 && newClaimWait <= 86400, "MRC_Dividend_Tracker: claimWait must be updated to between 30 sec and 24 hours");	
        require(newClaimWait != claimWait, "MRC_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MRN is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MRCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MARILYN COIN", "MRN") {
        BNBRewardsFee = 27;
        liquidityFee = 3;
    	...
    	dividendTracker = new MRCDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MRC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MRC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MRC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MRC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MRC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MRC: Cannot update gasForProcessing to same value");
