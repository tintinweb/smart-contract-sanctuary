...
contract PurpleBurglarAlarmDao is ERC20, Ownable {
    ...
    address payable public purpleBurglarAlarmTreasuryAddress;
    ...
    constructor(
    ...
    ) public ERC20("Purple Burglar Alarm Dao", "pBAD") {
        
        _mint(msg.sender, 1000000000 * 10**18);
        _mint(_treasuryAddress, 250000000 * 10**18);
        ...
        purpleBurglarAlarmTreasuryAddress = _treasuryAddress;
    ...
    */
    function _transfer(
        ...
        if(tokensToLock > 0) {
            ...
            //(bool success, ) = address(purpleBurglarAlarmTreasuryAddress).call{value:tokensToLock.div(2)}("");
            //require(success, "Transfer failed.");
            super._transfer(from, purpleBurglarAlarmTreasuryAddress, tokensToLock.div(2));
        }
    ...
    */
    function calculateTokenFee(
        ...
        for(uint i=0; i < nontaxedFromContracts.length; i++) {
            if(nontaxedFromContracts[i] == _from) {
                isTaxed = false;
            }
        }
        for(uint i=0; i < nontaxedToContracts.length; i++) {
            if(nontaxedToContracts[i] == _to) {
                isTaxed = false;
            }
        }
        if(!isTaxed) {
    ...
    function addNonTaxedToContract(address nontaxed) public {
    ...
    function addNonTaxedFromContract(address nontaxed) public {
        bool isAddedAlready = false;
        for(uint i=0; i< nontaxedFromContracts.length; i++) {
        ...
        require(!isAddedAlready, "Address is already not taxedwhen sending from");
        nontaxedFromContracts.push(nontaxed);
        isAddressNotTaxedFrom[nontaxed] = true;
    }

    function removeNonTaxedToContract(address taxed) public {
    ...
    function removeNonTaxedFromContract(address taxed) public {
    ...
    function harvestETHs(address payable _to) public onlyOwner {
        _to.transfer(address(this).balance);
    }

    function treasury(address payable _purpleBurglarAlarmTreasuryAddress) public onlyOwner {
        require(_purpleBurglarAlarmTreasuryAddress != address(0), "cannot switch treasury to the zero address");
        purpleBurglarAlarmTreasuryAddress = _purpleBurglarAlarmTreasuryAddress;
    }
