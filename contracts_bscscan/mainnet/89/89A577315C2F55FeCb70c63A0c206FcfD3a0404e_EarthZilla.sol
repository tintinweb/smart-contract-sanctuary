...
// SPDX-License-Identifier: NoLLICENCE
...
contract EarthZillaReward is RewardHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() RewardHolders("EarthZilla_Reward_Tracker", "EarthZilla_Reward_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "EarthZilla_Reward_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "EarthZilla_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EarthZilla contract.");
    }
...
contract EarthZilla is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    EarthZillaReward public Reward;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public marketFeeReciever= 0xEBcD2cea937d100e81c0f26dE12ABAC8FD7CF7b3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Earth Zilla", "ERZ") {
...
emit RebaseUpdate(epoch, _totalSupply);
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "EarthZilla: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "EarthZilla: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "EarthZilla: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EarthZilla: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTEarthZilla = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTEarthZilla);
...
addLiquidity(tokensTEarthZilla, bnbForLiquidity);
