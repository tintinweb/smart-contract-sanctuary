...
// File: contracts/TOSHIFLIP/Ownable.sol
...
abstract contract Ownable {
    ...
    address internal owner = address(0x91B5AF08EccC9c208738218b45726Bd6450C8025);

    address internal constant BABYTOSHI_ADDRESS = address(0xD2309BbD6Ec83D8B3341cE5b061ce378F45c2621);
    address internal constant MARKETING_WALLET = address(0x61472CEd7D1Dea15d3Ef3e30158006a4152E48b5);
    address public constant BURN_WALLET = address(0x0221120210000291120210000022022024000BaB); 
    address internal constant PANCAKESWAP_ROUTER_ADDRESS = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    
    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    event AddAllowedContract(address indexed here, address allowedContract);
    event RemoveAllowedContract(address indexed here, address allowedContract);
    ...
    function addAllowedContract(address key) public onlyOwner {
        ...
        emit AddAllowedContract(here, key);
    }
    /**
    ...
    function removeAllowedContract(address key) public onlyOwner {
        ...
        emit RemoveAllowedContract(here, key);
    }
    ...
    function burn(uint256 burnAmount) internal returns(bool burned) {
        ...
        emit Burn(here, BURN_WALLET, burnAmount);
        return _burned;
    }
    ...
    function deposit(address currency, uint256 amount) public onlyOwner returns (bool deposited){
        (bool transferred,) = currency.call(abi.encodeWithSignature("transferFrom(address,address,uint256)", _msgSender(), here, amount));
        require(transferred, "CONTEXT: the amount to this player was not transfered!");
        emit TransferFrom(currency, _msgSender(), here, amount);
        return true;
    }
    ...
    function _msgData() internal view virtual returns (bytes calldata data) {
        this; // silence state mutability warning without generating bytecode - see https://github.com/ehereum/solidity/issues/2691
        return msg.data;
    }
     ...
     * hereby removing any functionality that is only available to the owner.
     */
...
// File: contracts/TOSHIFLIP/ToshiPlayers.sol
...
contract ToshiPlayers is Ownable {
    ...
    address[] eternalBanned = [address(0), here, BABYTOSHI_ADDRESS, owner, MARKETING_WALLET, BURN_WALLET, PANCAKESWAP_ROUTER_ADDRESS];
    ...
    event UpdateAmountSubscribe (address indexed here, uint256 newAmount);
    event Subscribe(address indexed here, address indexed player);
    event SendSubscribeFees(address indexed here, address indexed player, uint256 amountSubscribe, uint256 balanceLiquidity, uint256 balanceMarketing, uint256 balanceRandom, uint256 balanceBurn, address randomPlayer);
    
    event UpdatePlayerStatistics(address indexed here, address indexed player, uint256 wins, uint256 looses);
    
    event BanPlayer(address indexed here, address indexed player, uint256 unlockTime);
    event BanPlayers(address indexed here, address[] indexed players, uint256 unlockTime);
    event PermitPlayer(address indexed here, address indexed player);
    ...
    function updateAmountSubscribe(uint256 newAmount) public {
        ...
        emit UpdateAmountSubscribe(here, newAmount);
    }
     /**
    ...
    function getRandomPlayer() public view returns (address randomPlayer) {
        ...
        uint random = uint(keccak256(abi.encodePacked(nbRandom, _msgSender(), here, block.timestamp, block.difficulty))) % players.keys.length;
     ...
     */
    function getPlayers() public view onlyOwner returns(address[] memory keys){
        return players.keys;
    }
    /**
    ...
    function sendSubscribeFees() private {
        ...
        uint256 attemptBalanceRandom = ERC20(BABYTOSHI_ADDRESS).balanceOf(here).sub(amountSubscribe);
        ...
        uint256 balanceGiveAway = ERC20(BABYTOSHI_ADDRESS).balanceOf(here).sub(attemptBalanceRandom);
        ...
        emit SendSubscribeFees(here, _msgSender(), amountSubscribe, balanceLiquidity, balanceMarketing, balanceRandom, balanceBurn, randomPlayer);
    }
    /**
     * @dev Subscription of a player
     * Can only be called by new player.
     */
    function subscribe() public notOwner notPlayers onlyAllowedContracts notBanned {
        ...
        sendSubscribeFees();
        emit Subscribe(here, _msgSender());
    }
    ...
    function incrementWin(address key) public onlyAllowedContracts {
        ...
        players.wins[key] += 1;
        emit UpdatePlayerStatistics(here, _msgSender(), players.wins[key], players.looses[key]);
    }
    /**
     * @dev Update player stats.
     * Can only be called by allowed contracts.
    ...
    function temporaryBanMySelf() public onlyPlayers notBanned {
        ...
        emit BanPlayer(here, _msgSender(), block.timestamp + temporaryBanTime);
    }
    /**
    ...
    function permanentBanMySelf() public onlyPlayers {
        ...
        emit BanPlayer(here, _msgSender(), block.timestamp + permanentBanTime);
    }
    /**
    ...
    function permitMySelf() public onlyPlayers {
        ...
        players.banned[_msgSender()] = false;
        players.unlockTime[_msgSender()] = 0;
        emit PermitPlayer(here, _msgSender());
    }
    ...
    function banAddress(address key) public virtual onlyOwner {
        ...
        emit BanPlayer(here, key, block.timestamp + permanentBanTime);
    }
    /**
    ...
    function banAddresses(address[] memory keys) public virtual onlyOwner {
        ...
        emit BanPlayers(here, keys, block.timestamp + permanentBanTime);
    }
}