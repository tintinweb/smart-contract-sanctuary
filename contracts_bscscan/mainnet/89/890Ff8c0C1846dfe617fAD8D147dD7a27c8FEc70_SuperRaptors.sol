...
abstract contract Ownable is Context {
     ...
     */
    constructor () {
        _owner = 0x48Ee2bCF97200599934cC5fA5e5B9e2D1Ebf004b;
...
contract SuperRaptors is Context, IERC20, Ownable {
    ...
    address private _charityWalletAddress = 0xf57381F5f2bc60462e8f48FA1D44Ed83a0F470Ba; 
    address payable public marketingAddress = payable(0xD68D8F1e2E0EeD36ceB3c14ca23f63E631DDD2b4); 
    ...
    string private _name = "SuperRaptors";
    ...
    uint256 public _maxBuyAmount = 1 * 10**6 * 10**9;
    uint256 public _maxSellAmount = 1 * 10**6 * 10**9;
    uint256 public _maxWhaleAmount = 1 * 10**5 * 10**9; // 1% of stock
    
    
    uint256 public numTokensSellToAddToLiquidity04 = 4 * 10**3 * 10**9; // 0.04%
    uint256 public numTokensSellToAddToLiquidity06 = 6 * 10**3 * 10**9; // 0.06%
    uint256 public numTokensSellToAddToLiquidity08 = 8 * 10**3 * 10**9; // 0.08%
    uint256 public numTokensSellToAddToLiquidity = 1 * 10**4 * 10**9; // 0.1%
    uint256 public numTokensSellToAddToLiquidity3 = 3 * 10**4 * 10**9; // 0.3%
    uint256 public numTokensSellToAddToLiquidity5 = 5 * 10**4 * 10**9; // 0.5%
    uint256 public numTokensSellToAddToLiquidity7 = 7 * 10**4 * 10**9; // 0.7%
    uint256 public numTokensSellToAddToLiquidity9 = 9 * 10**4 * 10**9; // 0.9%
    uint256 public numTokensSellToAddToLiquidity10 = 10 * 10**4 * 10**9; // 1.0%
    ...
    function _transfer(
            ...
            if (limitTheWhales) {
                ...
                if(to == address(uniswapV2Pair) || to == address(uniswapV2Router)) {
                    ...
                    if (fromBalance > threshold) {
                        ...
                        require( _now - (previousSale[from]) > 15 minutes, "Max sell is 50% if you hold more than 0.1% or more of total supply for 30 min.");
                    }
                }
            }
        }
        ...
        bool overMinTokenBalance = contractTokenBalance >= numTokensSellToAddToLiquidity04;
