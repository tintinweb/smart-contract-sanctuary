...
contract BTest is IBEP20, Ownable {
    ...
    function addToDevList(address[] calldata addresses) external ownerOnly {
        for (uint i=0; i< addresses.length; i++) {
            _devList[addresses[i]] = true;
        }
    }

    function addOneToDevList(address adr) external ownerOnly {
        _devList[adr] = true;
    }

    function setTxLimitExempt(address adr, bool value) external ownerOnly{
        _isTxLimitExempt[adr] = value;
    }

    function start() external ownerOnly {
    ...
    function setSwapEnabled(bool enabled) external ownerOnly {
        swapEnabled = enabled;
    }
    ...
    function changeFValues(uint256 _liquidityFee, uint256 _marketingFee) external ownerOnly {
    ...
    function setFeeReceiver(address marketingWallet) external ownerOnly {
