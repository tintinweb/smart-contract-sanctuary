...
interface IFarm{
    ...
    function harvest(address[] memory _pools) external;
    function masterBarista() external view returns (address);
}

interface IBarista{
    function userInfo(address _pair, address _user) external view returns (uint256, uint256, uint256, address);
}
...
contract Strategy_LATTEv2_BUSD is Ownable {
    // This strategy is for BSC network. LATTEv2 - BUSD pair. RewardToken = "LATTEv2"
    ...
    uint256 public pendingBUSDFee;

    address public constant USDT = 0x55d398326f99059fF775485246999027B3197955;
    address public constant YELtoken = 0xD3b71117E6C1558c1553305b44988cd944e97300;
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

    address public token0; // LATTEv2
    address public token1; // BUSD
    address public vault;

    // ------------> Important constants <--------------
    bool public constant hasPid = false;
    address public routerLP = 0x4Ec82EcD19529ff50a467F522029f69459bf071d;
    // uses for swapping WBNB to USDT and WBNB to BUSD
    address public routerMain = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public yelLiquidityRouter = 0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7;
    address public constant lpToken = 0x1524C3380257eF5D556AFeB6056c35DeFA9db8b6;
    address public constant farm = 0xfE663C8C580832A916F8D9e9fF8e13Cb814Ec14d;
    // ------------>        END          <--------------
    ...
    function _withdrawFromFarm(uint256 _amount) internal {
        IFarm(farm).unstake(lpToken, _amount);
    }
    
    function _getRewardsFromFarm() internal {
        address[] memory pools = new address[](1);
        pools[0] = lpToken;
        IFarm(farm).harvest(pools);
    }

    function getAmountLPFromFarm() public view returns (uint256 amount) {
        address masterBarista = IFarm(farm).masterBarista();
        (amount, , , ) = IBarista(masterBarista).userInfo(lpToken, address(this));
    }

    function withdrawUSDTFee(address _owner) public onlyVault {
        if(getBalanceOfToken(USDT) > 0) {
            IERC20(USDT).transfer(_owner, getBalanceOfToken(USDT));
        }
    }
    ...
    function earn() public returns(uint256 _balance){
        ...
        if(_balance > 0) {
            ...
            IFarm(farm).stake(lpToken, _balance);
    ...
    function autoCompound() external {
        ...
        _approveToken(rewardToken, routerLP);
        _approveToken(WBNB, routerLP);
        _approveToken(WBNB, routerMain);
        _approveToken(USDT, routerLP);
        _approveToken(token1, lpToken);
        _approveToken(token1, routerLP);
        _approveToken(token1, routerMain);

        if (getAmountLPFromFarm() > 0) {
            // rewards and LP
            _getRewardsFromFarm();
            // as reward is LATTEv2, it needs to swap it through BUSD
            uint256 _balance = getBalanceOfToken(rewardToken);
            if(_balance > 0){
                path[0] = rewardToken;
                path[1] = token1;
                uint256 amount = _getAmountsOut(routerLP, _balance+pendingRewardsFee, path);
                if(amount > 100) {
                    uint256 _oldBalanceBUSD =  getBalanceOfToken(token1);
                    _swapExactTokensForTokens(routerLP, _balance+pendingRewardsFee, amount, path);
                    uint256 _newBalanceBUSD = getBalanceOfToken(token1);
                    path[0] = token1;
                    path[1] = WBNB;
                    amount = _getAmountsOut(routerMain, _newBalanceBUSD-_oldBalanceBUSD, path);
                    if(amount > 100) {
                        _swapExactTokensForTokens(routerMain, _newBalanceBUSD-_oldBalanceBUSD, amount, path);
                        pendingRewardsFee = 0;
                        _takeFee();
                    } else {
                        pendingBUSDFee += _newBalanceBUSD-_oldBalanceBUSD;
                    }
                } else{
    ...
    function setRouter(address _address) public onlyVaultOrOwner {
        ...
        routerLP = _address;
        _approveToken(token0, _address);
        _approveToken(token1, _address);
    }

    function setRouterMain(address _address) public onlyOwner {
        require(_address != address(0), "The address can not be zero address");
        routerMain = _address;
        _approveToken(WBNB, _address);
        _approveToken(USDT, _address);
    }
    ...
    function _takeFee() internal {
        ...
        if(rewardsFeeInWBNB > 0) {
            uint256 amount = _getAmountsOut(routerMain, rewardsFeeInWBNB+pendingFee, path);
            if(amount > 100) {
                _swapExactTokensForTokens(routerMain, rewardsFeeInWBNB + pendingFee, amount, path);
                pendingFee = 0;
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapWBNBToToken(routerMain, token1);
        _swapToken1ToToken0();
    ...
    function _swapExactETHForBUSD(uint256 _amountETH) internal {
        // to swap BNB to token1 it needs to have wBNB token in the path
        // BNB -> wBNB -> BUSD
        if(_amountETH > 0){
            address[] memory path = new address[](2);
            path[0] = WBNB;
            path[1] = token1;
            uint256 amount = _getAmountsOut(routerMain, _amountETH, path);
            if(amount > 100){
                IRouter(routerMain).swapExactETHForTokens{value:_amountETH}(
    ...
    function _swapToken1ToToken0() internal {
        // a half of token1 swap to token0
        uint256 _balance = getBalanceOfToken(token1) / 2;
        if(_balance > 0){
            address[] memory path = new address[](2);
            path[0] = token1;
            path[1] = token0;
            uint256 amount = _getAmountsOut(routerLP, _balance, path);
            if(amount > 100){
                _swapExactTokensForTokens(routerLP, _balance, amount, path);
            }
        }
    }

    function _swapToken0ToToken1() internal {
        // a half of token swap to token1
    ...
    function _addLiquidityFromETH(uint256 _amountETH) internal {
        // swap all wrapped tokens to BUSD
        _swapExactETHForBUSD(_amountETH);
        _swapToken1ToToken0();
    ...
    function _swapLPtoWBNB(uint256 _percent) internal {
        ...
        if (_totalLP > 0) {
            ...
            _swapToken0ToToken1();
            _swapTokenToWBNB(routerMain, token1);
        }
    }

    function _swapTokenToWBNB(address _router, address _token) internal {
        address[] memory path = new address[](2);
        path[0] = _token;
        path[1] = WBNB;
        uint256 _balance = getBalanceOfToken(_token);
        if(_balance > 0) {
            uint256[] memory amounts = IRouter(_router).getAmountsOut(_balance, path);
            if(amounts[1] > 100) {
                _swapExactTokensForTokens(_router, _balance, amounts[1], path);
            }
        }
    }
    ...
    function _getToken1Price() internal view returns (uint256) {
        ...
        path[0] = token0;
        path[1] = token1;
        return 1e36 / _getAmountsOut(routerLP, 1e18, path);
    }

    function _getToken0Price() internal view returns (uint256 result) {
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token0;
        result = 1e36 / _getAmountsOut(routerLP, 1e18, path);
    }

    function _getPriceWBNBToToken1() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = token1;
        return 1e36 / _getAmountsOut(routerMain, 1e18, path);
    }

    function _getToken1FromToken0(uint256 _token0Amount) internal view returns (uint256 token1Amount) {
        token1Amount = _getToken0Price() * _token0Amount / 1e18;
    }
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        ...
        if(_amount > 0) {
            ...
            tCI = (t1V + _getToken1FromToken0(t0V)) * _getPriceWBNBToToken1() / d;
        }

        uint256 _balance0 = getBalanceOfToken(token0);
        uint256 _balance1 = getBalanceOfToken(token1);
        if(_balance1 != 0) {
            tCI += (_getToken1FromToken0(_balance0) + _balance1) * _getPriceWBNBToToken1() / d;
        }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t0V != 0) {
                path[0] = token0;
                path[1] = token1;
                token1Amount = _getAmountsOut(routerLP, t0V, path);
                path[0] = token1;
                path[1] = WBNB;
                TC = _getAmountsOut(routerLP, token1Amount + t1V, path);
            }
        }
        ...
        if(_balance0 != 0) {
            path[0] = token0;
            path[1] = token1;
            token1Amount = _getAmountsOut(routerLP, _balance0, path);
            path[0] = token1;
            path[1] = WBNB;
            TC += _getAmountsOut(routerLP, token1Amount + _balance1, path);
        }
