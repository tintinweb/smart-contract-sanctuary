...
contract EquipNFT is ERC721URIStorageUpgradeable,ERC721EnumerableUpgradeable,Governable {
 ...
 /*   uint private constant DAOF = 0;
    ...
    function costDur(uint id) public {
        require((msg.sender == governor||admins[msg.sender]),"You do not have permission");
        curPropertys[id].curDurability -= property.costDurability;
    }
...
contract DaoFarmer is Governable,IERC721ReceiverUpgradeable {
    uint private constant DAOF = 0;
    uint private constant FOOD = 1;
    uint private constant WOOD = 2;
    uint private constant GOLD = 3;
    ...
    function forge(uint propId) public returns (uint tokenId) {
        ...
        //prop.curPropertys[tokenId].curDurability = totalDurability;
    ...
    function farm(uint propId,uint tokenId) public  {
        if (!powerInits[msg.sender]){
            powerInits[msg.sender] = true;
            powers[msg.sender] = initPower;
        }
        EquipNFT prop = EquipNFT(nftList[propId]);
        ...
        //curPropertys[tokenId].curDurability -=  costDurability;
        prop.costDur(tokenId);
        ...
        emit Farm(nftList[propId],tokenId,erc1,realValue,block.timestamp);  
    }
    ...
    function costFoodForFull() public view returns(uint foodValue) {
        foodValue = (maxPower-powers[msg.sender])*1e18/5;        
    }    

   
       
    function eat(uint foodValue) public{
        uint realFoodValue = costFoodForFull();
    ...
    function costGoldForRepairFull(uint propId,uint tokenId) public view returns (uint goldValue){
        ...
        goldValue =(totalDurability - curPropertys[tokenId].curDurability)*1e18/5;
    }
    ...
    function repair(uint propId,uint tokenId,uint goldValue) public {
        ...
        curPropertys[tokenId].curDurability +=  realGoldValue/5/1e18;
        curPropertys[tokenId].lastTime =  block.timestamp;    
    }
