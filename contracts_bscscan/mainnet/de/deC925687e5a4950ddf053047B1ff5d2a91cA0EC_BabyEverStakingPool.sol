...
contract BabyEverStakingPool is Ownable {
    ...
    function deposit(uint256 pid, uint256 amount) external {
        ...
        if (amount > 0) {
            ...
            if (txFee > 0) amount = amount.sub(amount.mul(txFee).div(MAX_FEE));
    ...
    function withdraw(uint256 pid, uint256 amount) public {
        ...
        if (amount > 0) {
            uint256 withdrawable = amount;
            if (withdrawalFee > 0) withdrawable = amount.sub(amount.mul(withdrawalFee).div(MAX_FEE));
            stakingToken.safeTransfer(address(msg.sender), withdrawable);
