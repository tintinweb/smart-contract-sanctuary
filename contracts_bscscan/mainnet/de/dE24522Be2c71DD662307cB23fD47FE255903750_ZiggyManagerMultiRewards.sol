...
contract ZiggyManagerMultiRewards is ReentrancyGuardUpgradeable, Ownable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event GateManagerDeposit(address indexed user, uint256 value);

    /// @notice Interface for the stakeToken
    ...
    modifier onlyNotifier() {
        require(msg.sender == owner() || msg.sender == potsConverter, "!notifier");
        _;
    }
    ...
    function TVL() external view returns (uint256) {
        uint256 totalGateManagerBal = totalSupply();
        address yieldSource = IPrizePool(prizePool).yieldSource();
        uint256 totalYieldSourceBal = IYieldSource(yieldSource).totalYieldTokenAmount();

        return totalGateManagerBal.add(totalYieldSourceBal);
    }
    ...
    function depositAll(address referrer) external {
        uint256 tokenBal = IERC20Upgradeable(stakeToken).balanceOf(msg.sender);
        depositMoonPot(tokenBal, referrer);
    }

    /// Withdraw all sender funds with timelock
    function withdrawAllTimelock() public {
        uint256 ticketBal = IERC20Upgradeable(prizePoolControlledToken)
        ...
        updateReward(msg.sender)
    {
        ...
        uint256 balBefore = IERC20Upgradeable(stakeToken).balanceOf(address(this));
        ...
        IERC20Upgradeable(stakeToken).safeApprove(address(prizePool), amount - halvedAmount);
        IPrizePool(prizePool).depositTo(
            msg.sender,
            amount - halvedAmount,
        ...
        uint256 balAfter = IERC20Upgradeable(stakeToken).balanceOf(address(this));
    ...
    /// @param amount The amount of stake token to withdraw.
    ...
    function withdrawWithTimeLockFromMoonPotPrizePool(uint256 amount) public nonReentrant {
        require(
        ...
        IPrizePool(prizePool).withdrawWithTimelockFrom(
            msg.sender,
            amount,
            prizePoolControlledToken
        );
    ...
    function withdrawInstantlyFromMoonPotPrizePool(uint256 amount) public nonReentrant {
        ...
        (uint256 exitFee, ) = IPrizePool(prizePool).calculateEarlyExitFee(
            msg.sender,
            prizePoolControlledToken,
            amount
        );

        uint256 actualFee = IPrizePool(prizePool).withdrawInstantlyFrom(
            msg.sender,
            amount,
            prizePoolControlledToken,
            exitFee
    ...
    // Compound users pots, this will extend users fair play
    ...
    // Sets pots converter
    function setPotsConverter(address newConverter) external onlyOwner {
            potsConverter = newConverter;
    }

    // Tells gate manager the reward amount per each reward token
    function notifyStakeTokenRewardAmount(uint256 reward)
        external
        onlyNotifier
        updateReward(address(0))
    {
        uint256 balance = IERC20Upgradeable(stakeToken).balanceOf(address(this));
        require(reward <= balance.sub(_totalSupply), "!too many pots");
        RewardInfo storage info = rewardInfo[0];     
    ...
    {
        RewardInfo storage info = rewardInfo[id];
        require(info.rewardToken != address(stakeToken), "!Can't notify stake token here");      
        if (block.timestamp >= info.periodFinish) {
            info.rewardRate = reward.div(info.stakingRewardsDuration);
        } else {
            uint256 remaining = info.periodFinish.sub(block.timestamp);
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        require(_token != address(stakeToken), "!staked");
