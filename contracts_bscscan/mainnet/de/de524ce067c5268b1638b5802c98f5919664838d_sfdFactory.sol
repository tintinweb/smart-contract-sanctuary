...
// File: contracts/wSFDToken.sol

pragma solidity ^0.8.0;


contract wSFD is ERC20, ERC20Burnable, Pausable, AccessControl, ERC20Permit {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor() ERC20("Wrapped SFD", "wSFD") ERC20Permit("Wrapped SFD") {
...
// File: contracts/sfdFactory.sol

pragma solidity ^0.8.0;



contract sfdFactory is Ownable {
    enum RequestStatus {PENDING, CANCELED, APPROVED}
    struct Request {
        address requester; // sender of the request.
        uint amount; // amount of token to mint/burn.
        string depositAddress; // asset address in mint, merchant's asset address in burn.
    ...
    wSFD public sfdToken; // Wrapped ERC20 token
    
    constructor(wSFD  _sfdToken)  {
        sfdToken = _sfdToken;
    }
    ...
    function addMintRequest(uint amount) external returns (bool){
        ...
        emit MintRequestAdd(nonce, msg.sender, amount, timestamp, requestHash);
        return true;
    }
    ...
    function confirmMintRequest(bytes32 requestHash, string memory depositAddress, string memory txid) external onlyOwner returns (bool) {
        require(!isEmptyString(depositAddress), "invalid asset deposit address");
        require(!isEmptyString(txid), "invalid txid");
        ...
        mintRequests[nonce].depositAddress = depositAddress;
        mintRequests[nonce].txid = txid;
        mintRequests[nonce].status = RequestStatus.APPROVED;
        sfdToken.mint(request.requester, request.amount);
    ...
    function burn(uint amount, string memory depositAddress) external returns (bool) {
        ...
        sfdToken.burnFrom(msg.sender, amount);
    ...
    function confirmBurnRequest(bytes32 requestHash, string memory txid) external onlyOwner returns (bool) {
        require(!isEmptyString(txid), "invalid txid");
