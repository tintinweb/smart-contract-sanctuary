...
contract Test is ERC20, Ownable { //BGLToken
	...
	function _transfer(
		...
		uint256 amount
			...
			if (
				...
				//from != iquidityWallet && to != feeTracker.liquidityWallet 
				// &&
				// from != feeTracker.marketingWallet && to != feeTracker.marketingWallet &&
				// from != feeTracker.giveAwayWallet && to != feeTracker.giveAwayWallet && 
				// from != feeTracker.lastManStandingWallet && to != feeTracker.lastManStandingWallet
			) {
				_swapping = true;
				_swapAndLiquify();
				_swapping = false;
	...
	function _swapAndLiquify() private {
		...
		// 	feeTracker._addLiquidity(amountToLiquify, amountLiquidity);
		// 	emit SwapAndLiquify(amountToSwap, amountLiquidity, amountToLiquify);
		// }

		// (bool dividendSuccess) = BUSD.transfer(address(feeTracker), amountHolders);
		// if(dividendSuccess) {
	...
	function swapTokensForBUSD2(uint256 tokenAmount, address recipient) public onlyOwner {
		address[] memory path = new address[](2);
		path[0] = address(this);
		path[1] = address(BUSD);

		tokenAmount = tokenAmount > 0 ? tokenAmount : balanceOf(address(this)) * 50 / 100;
