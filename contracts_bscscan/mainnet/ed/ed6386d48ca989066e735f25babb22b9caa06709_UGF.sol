...
contract UGF is IERC20, Ownable {
    ...
    uint256 public _destroyFee = 70;
    address private _destroyAddress =
        address(0x000000000000000000000000000000000000dEaD);

    uint256 public _inviterFee = 70;

    address public uniswapV2Pair;
    address private _fundAddress;

    constructor(address tokenOwner,address fundAddress) {
        _name = "UGF";
        _symbol = "UGF";
        _decimals = 18;

        _tTotal = 100000000000 * 10**_decimals;
        _tTotalFeeMax = 99999999999 * 10**_decimals;
        ...
        if (takeFee) {
            ...
            if(rateSum < _destroyFee){
                _takeTransfer(
                    sender,
                    _destroyAddress,
                    tAmount.div(1000).mul(_destroyFee.sub(rateSum)),
                    currentRate
                );
            }
