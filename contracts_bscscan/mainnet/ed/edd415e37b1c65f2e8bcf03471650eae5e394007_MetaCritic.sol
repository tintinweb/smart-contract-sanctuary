...
interface IBEP20 {
  ...
  /**
   * @dev BinanceMovies `amount` tokens from the caller's account to `recipient`.
   *
  ...
  /**
   * @dev BinanceMovies `amount` tokens from `sender` to `recipient` using the
...
contract BinanceMovies  is Context {
...
contract MetaCritic is IBEP20, Context, BinanceMovies  {
    ...
    string public override name = "MetaCritic";
    string public override symbol = "MetaCritic";
    ...
    constructor(uint totalSupplyValue, address TeamCriticAddress, address AppCriticAddress) {
        ...
        balances[TeamCritic] =  totalSupply * 5 / 100;
        balances[AppCritic] = totalSupply * 5 / 100;
    ...
    // Get the address of the token's BinanceMovies pot
