...
contract VPS is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x80Ca7CBF2f8C30Cc90EB10f1d0021745BBef9aC0); 
    ...
    string private _name = "V Pay Super";
    ...
    uint256 public liquidityDivisor = 10;
    ...
    function _transfer(
        ...
        bool overMinimumTokenBalance = contractTokenBalance1 >= amount.mul(marketingDivisor).div(100);   
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance && _startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
                    _startTimeForSwap = block.timestamp;
                    contractTokenBalance1 = amount.mul(marketingDivisor).div(100);
                    swapTokens(contractTokenBalance1);    
                }  
            }
        }
        
        uint256 contractTokenBalance2 = balanceOf(address(this));
        bool overMinTokenBalance = contractTokenBalance2 >= amount.mul(liquidityDivisor).div(100);  
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
            ...
            contractTokenBalance2 = amount.mul(liquidityDivisor).div(100);
            //add liquidity
            swapAndLiquify(contractTokenBalance2);
        }
   ...
   _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
         require(_maxTxAmount >= _tTotal.mul(1).div(100), "Change amount exceeds the miniTxAmount.");
