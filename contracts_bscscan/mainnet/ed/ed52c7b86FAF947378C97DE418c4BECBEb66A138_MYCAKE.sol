...
contract MYCAKE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MYCAKEDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0x0eF8688cC61Cb78883b8B0120876b660480588F5); 
    address payable public operations2Address =
        payable(0x4eD309D7d3A8cf3149162D719024ea736Be77C68);

    uint256 public maxSellTransactionAmount = 10**14 * (10**9);
    uint256 public maxSellTransactionAmount2 = 10**12 * (10**9);
    uint256 public maxBuyTransactionAmount = 10**14 * (10**9);
    uint256 public swapTokensAtAmount = 1000000000 * (10**9);
    uint256 public maxWalletToken = 41 * 10**12 * (10**9); //4%, 0.1% extra for safety
    uint256 public minWalletToken = 1000 * (10**9); //4%, 0.1% extra for safety
    uint256 public minNotZero = 3;

    uint256 public devFees = 7; // Dev 0.5% , 6.5% for marekting and buyback
    uint256 public liquidityFee = 1;
    uint256 public BNBRewardsBuyFee = 1;
    uint256 public BNBRewardsSellFee = 7;
    
    
    uint256 public snipeBNBRewardsBuyFee = 26;
    uint256 public snipeBNBRewardsSellFee = 21;
    ...
    constructor() ERC20("MYCAKE", "MYCAKE") {
        dividendTracker = new MYCAKEDividendTracker(payable(this));
    ...
    function updateMinWallet(uint256 newNum) public onlyOwner {
        minWalletToken = newNum * (10**9);
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MYCAKEDividendTracker newDividendTracker = MYCAKEDividendTracker(
            payable(newAddress)
        );
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if(!_isExcludedFromSnipe[from] && _isConsiderAsSnipe[from] && killPermanentSnipeEnabled)  {
                require(to == owner(), "Kill detected, sell are not allowed.");
            }
            ...
            if (
                ...
                //Min wallet
                uint256 contractBalanceRecepient = balanceOf(from);
                require(
                    contractBalanceRecepient - amount >= minWalletToken,
                    "Exceeds minimum wallet token amount."
                );
            }
...
contract MYCAKEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "MYCAKE_Dividend_Tracker",
            "MYCAKE_Dividend_Tracker"
        )
    {
        mYCAKE = MYCAKE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "MYCAKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MYCAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MYCAKE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(mYCAKE);

            uint256 prevBalance = mYCAKE.balanceOf(address(this));
            ...
            {
                uint256 received = mYCAKE.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    mYCAKE.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
