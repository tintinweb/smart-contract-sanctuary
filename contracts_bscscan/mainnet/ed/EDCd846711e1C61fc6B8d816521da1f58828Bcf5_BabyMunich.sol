...
Max Wallet:     3,000,000,000 (3%)*
Max Buy:        1,000,000,000 (1%)


https://t.me/BabyMunich
...
contract Dividend2Distributor is IDividendDistributor {
    using SafeMath for uint256;
...
contract BabyMunich is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "Baby Munich";
    string constant _symbol = "BabyMunich";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = (_totalSupply * 1) / 100; //1% of total supply
    uint256 public maxWalletToken = (_totalSupply * 3) / 100; // 3% of total supply
    ...
    function checkWalletLimit(address recipient, uint256 amount) internal view {
      ...
      require(
          contractBalanceRecepient + amount <= maxWalletToken || isWalletLimitExempt[recipient],
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBUSDReflection = amountBNB.mul(BUSDReflectionFee).div(totalBNBFee);
      


        try distributor.deposit{value: amountBUSDReflection}() {} catch {}
        payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("0x452C12571b7F86A969CEbcB2c6a2FAdF7837eB32");
    ...
    function prepareForPreSale() external onlyOwner {
      ...
      maxWalletToken = _totalSupply; // 100% of total supply
    }

    function afterPreSale() external onlyOwner {
      _maxTxAmount = (_totalSupply * 1) / 100; //1% of total supply
      maxWalletToken = (_totalSupply * 1) / 100; // 1% of total supply
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _BUSDReflectionFee, uint256 _busdReflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        BUSDReflectionFee = _BUSDReflectionFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_BUSDReflectionFee).add(_busdReflectionFee).add(_marketingFee);
