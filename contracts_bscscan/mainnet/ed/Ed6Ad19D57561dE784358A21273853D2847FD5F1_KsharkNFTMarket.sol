...
contract KsharkNFTMarket is IERC721Receiver, ReentrancyGuard {
    ...
    checkTime(index)
  {
    ...
    if (currencyAddr == address(0x0)) {
      ...
      require(amount >= this.getSalesPrice(index), "Your price is too low");
      uint256 returnBack = amount.sub(price);
      if (returnBack > 0) {
        // msg.sender.transfer(returnBack);
        IERC20(currencyAddr).transfer(msg.sender, returnBack);
      }
      if (tipsFee > 0) {
        // _tipsFeeWallet.transfer(tipsFee);
        IERC20(currencyAddr).transfer(_tipsFeeWallet, tipsFee);
      }
