...
contract DummyContract is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 30000000000 * (10**9);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if( canSwap && !swapping && automatedMarketMakerPairs[to]) {
			...
			uint256 tokenToLiqudityHalf = tokenToLiqudity.div(2);
			uint256 tokenToLiqudityOtherHalf = tokenToLiqudity.sub(tokenToLiqudityHalf);
			...
			if(liqudityPart > 0) {
			   addLiquidity(tokenToLiqudityOtherHalf, liqudityPart);
			}
			...
			_marketingFeeTotal = _marketingFeeTotal.sub(tokenToMarketing);
		    _charityFeeTotal = _charityFeeTotal.sub(tokenToCharity);
		    _buybackFeeTotal = _buybackFeeTotal.sub(tokenToBuyback);
		    _liquidityFeeTotal = _liquidityFeeTotal.sub(tokenToLiqudity);
		    _rewardFeeTotal = _rewardFeeTotal.sub(tokenToReward);
            swapping = false;
        }
