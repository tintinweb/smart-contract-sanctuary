...
/* REMOOT GAME
https://remoot.app
https://twitter.com/remootapp
https://discord.gg/kKHXQGfh9w
*/
...
contract Code is ERC20("Code", "CODE"), Ownable {
    ...
    function setOfficeAddress(address _officeAddress) external onlyOwner {
    ...
    function mint(address _to, uint256 _amount) external {
        require(_msgSender() == officeAddress, "Only the Office contract can mint");
        _mint(_to, _amount);
    }

    function burn(address _from, uint256 _amount) external {
        require(_msgSender() == developerAddress, "Only the Developer contract can burn");
        _burn(_from, _amount);
    }

    function transferToRepo(address _from, uint256 _amount) external {
        require(_msgSender() == repoAddress, "Only the Repo contract can call transferToRepo");
        _transfer(_from, repoAddress, _amount);
    }
}

// File contracts/Developer.sol

pragma solidity ^0.8.4;

contract Developer is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant MAX_PRESALE_SUPPLY = 1000;
    uint256 public constant BASE_MINT_PRICE = 0.15 ether; 
    uint256 public constant PRESALE_MINT_PRICE = 0.1 ether;  
    uint256 public constant BASE_MID_PERCENTAGE = 20;
    uint256 public constant BASE_SENIOR_PERCENTAGE = 5;
    ...
    string public constant BASE_URI = "https://remoot.app/nft/data/";

    constructor() ERC721("Developer", "DEVELOPER") {
        // supply and price are ignored for the base levels
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 1 })); // junior
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 5 })); // mid
        levels.push(Level({ supply: 0, maxSupply: 0, price: 0, yield: 10  })); // senior
    }

    /* Minting of developer */
    ...
    function _mintBaseTokens(uint16 _numTokens, address _for) internal {
        ...
        for (uint i = 0; i < _numTokens; i++) {
            ...
            if (_rand(totalSupply()) % 100 < BASE_SENIOR_PERCENTAGE) {
                tokenLevel[tokenId] = 2; // senior
                seniorSupply++;
            } 
            else if (_rand(totalSupply()) % 100 < BASE_MID_PERCENTAGE) {
                tokenLevel[tokenId] = 1; // mid
                midSupply++;
            }else {
                tokenLevel[tokenId] = 0; // junior
            }
        }
    }

    /* Minting of upgrade developer */
    ...
    function mintUpgrade(uint256 _level, uint16 _numTokens) external {
        ...
        require(code.balanceOf(msg.sender) >= totalCost, "Insufficient CODE balance");
        code.burn(msg.sender, totalCost);
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (isUpgrade(_tokenId)) {
            ...
            return string(abi.encodePacked(BASE_URI, _tokenId.toString(), "-senior.json"));
        } else if (revealedTokenLevel(_tokenId) == 1) {
            return string(abi.encodePacked(BASE_URI, _tokenId.toString(), "-mid.json"));
        } else {
            return string(abi.encodePacked(BASE_URI, _tokenId.toString(), "-junior.json"));
        }
    }
    ...
    function batchedDevelopersOfOwner(
        address _owner,
        uint256 _offset,
        uint256 _maxSize
    ) public view returns (DeveloperInfo[] memory) {
        if (_offset >= balanceOf(_owner)) {
            return new DeveloperInfo[](0);
        }
        ...
        DeveloperInfo[] memory developers = new DeveloperInfo[](outputSize);
    ...
    function setCode(Code _code) external onlyOwner {
        code = _code;
    }
    ...
    function rewards(uint16 _numTokens) external onlyOwner {
        _mintBaseTokens(_numTokens, _msgSender());
    }
