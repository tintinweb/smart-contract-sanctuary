...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Deposit Fee address / Dev Address and BuyBack Wallet / Treasury Wallet 
    address public feeAddDev;
    address public feeAddBb;
    address public feeAddTrea;
    ...
    uint16 public feeDevRate = 1000;
    uint16 public feeTreaRate = 500;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ufxReward = multiplier.mul(ufxPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        safeUfxTransfer(feeAddDev, ufxReward.mul(feeDevRate).div(10000));
        safeUfxTransfer(feeAddTrea, ufxReward.mul(feeTreaRate).div(10000));
    ...
    function updateFeeDevRate(uint16 _newFeeDevRate) public onlyOperator {
        require(_newFeeDevRate < 2000, "update: fee dev rate is over 20%");
        feeDevRate = _newFeeDevRate;
    }

    // update dev treasury rate
    function updateFeeTreaRate(uint16 _newFeeTreaRate) public onlyOperator {
        require(_newFeeTreaRate < 2000, "update: fee treasury rate is over 20%");
        feeTreaRate = _newFeeTreaRate;
    }
    ...
    // update fee address Treasury
    function updateFeeAddTrea(address _newFeeAddTrea) public onlyOperator {
        require(_newFeeAddTrea != address(0), "update: fee address Treasury is zero");
        feeAddTrea = _newFeeAddTrea;
    }
