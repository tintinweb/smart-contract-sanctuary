...
// File: contracts/MLAToken.sol

pragma solidity 0.6.12;

// MLAToken with Governance.
contract MLAToken is ERC20("MLAToken", "MLA"), Ownable {
   ...
   modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "MLA::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "MLA::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
...
// File: contracts/MLAFarm.sol

pragma solidity ^0.6.0;

contract MLAFarm is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        MLAToken rewardToken;        // Address of reward token contract.
    ...
    function add(uint256 _allocPoint, MLAToken _rewardToken, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate,uint256 _startBlock,uint256 _lockFromBlock,uint256 _lockInterval) public onlyOwner nonDuplicated(_lpToken){
    ...
    function setreferralToken(MLAToken _referralToken) public onlyOwner {
        mla = _referralToken;
    }
    ...
    function totalLock(MLAToken _rewardToken) public view returns (uint256) {
