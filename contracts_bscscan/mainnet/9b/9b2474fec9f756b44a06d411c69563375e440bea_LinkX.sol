...
/**
    ...
    Missed FegX?? Don't miss LinkX!! Get rewarded in $LINK for HODLing!!
	
	Telegram: https://t.me/LinkXbsc
...
contract LinkX is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LINKDividendTracker public dividendTracker;
    ...
    uint256 public LINKRewardsSplitValue = 7;
    ...
    uint256 public buyingFeesWithDecimals = (LINKRewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    
    uint256 public sellingFeesWithDecimals = (LINKRewardsSplitValue.add(liquiditySplitValue).add(marketingSplitValue).add(bananaRewardSplitValue)) * feeDecimalFactor;
    ...
    constructor() public ERC20("LinkX", "LinkX") {

    	dividendTracker = new LINKDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LINK: The dividend tracker already has that address");

        LINKDividendTracker newDividendTracker = LINKDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LINK: The new dividend tracker must be owned by the LINK token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LINK: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LINK: Account is already the value of 'excluded'");
    ...
    function setLINKRewardsSplitValue(uint256 value) external onlyOwner{
        require(value >= 1 && value <= 1000);
        LINKRewardsSplitValue = value;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LINK: The PanLINKSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LINK: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LINK: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LINK: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 marketingTokens = contractTokenBalance.mul(marketingSplitValue).div(LINKRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
            swapAndSendToFee(marketingTokens);

            uint256 swapTokens = contractTokenBalance.mul(liquiditySplitValue).div(LINKRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
            swapAndLiquify(swapTokens);

            totalBananas += contractTokenBalance.mul(bananaRewardSplitValue).div(LINKRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);

            uint256 sellTokens = contractTokenBalance.mul(LINKRewardsSplitValue).div(LINKRewardsSplitValue + liquiditySplitValue + marketingSplitValue + bananaRewardSplitValue);
    ...
    function swapAndSendDividends(uint256 tokens) private{
        uint256 initialPanLINKBalance = IERC20(LINK).balanceOf(address(this));
        swapTokensForLINK(tokens);
        uint256 dividends = (IERC20(LINK).balanceOf(address(this))).sub(initialPanLINKBalance);
...
contract LINKDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("LINK_Dividend_Tracker", "LINK_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "LINK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "LINK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LINK contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LINK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LINK_Dividend_Tracker: Cannot update claimWait to same value");
