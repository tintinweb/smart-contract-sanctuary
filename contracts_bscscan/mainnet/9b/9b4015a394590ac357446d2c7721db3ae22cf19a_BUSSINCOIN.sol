...
/**
   ...
   5% fee auto burnt to dead wallet
   2% Charity fee
...
contract BUSSINCOIN is Context, IERC20, Ownable {
	...
	address payable private _deadWallet;
	address payable private _charityWallet;

    string private _name = "BLACKLIVESMATTERCOIN";
    string private _symbol = "BLMC";
	...
	function setDeadAddress(address payable dead) public onlyOwner() {
        _deadWallet = dead;
	}
	function setCharityAddress(address payable charity) public onlyOwner() {
        _charityWallet = charity;
	}	
	function _getcharityWallet() public view virtual returns (address) {
        return _charityWallet;
	}
	
	function _getdeadWallet() public view virtual returns (address) {
        return _deadWallet;
	}
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tDead) = _getValues(tAmount);
	...
	function setDeadFeePercent(uint256 deadFee) external onlyOwner() {
        _deadFee = deadFee;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tCharity, data.tDead);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tCharity).sub(tDead);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tCharity, tDead, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rCharity = _data.tCharity.mul(_data.currentRate);
		uint256 rDead = _data.tDead.mul(_data.currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rCharity).sub(rDead);
	...
	function _takeDead(uint256 tDead) private {
        uint256 currentRate =  _getRate();
        uint256 rDead = tDead.mul(currentRate);

        if (_shouldSwapToBNB) {
            swapTokensForEth(rDead);
            _deadWallet.transfer(address(this).balance);
        } else {
            _rOwned[_deadWallet] = _rOwned[_deadWallet].add(rDead);
            if(_isExcluded[_deadWallet])
                _tOwned[_deadWallet] = _tOwned[_deadWallet].add(tDead);
        }
    }
