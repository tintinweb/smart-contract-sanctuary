...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("HexDex Lottery Ticket", "BPLT") {}
...
// File: contracts\HEDXLottery.sol
...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        hedx.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        hedx.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safehedxTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safehedxTransfer(address(msg.sender), totalReward);
        }
    ...
    // Safe hedx transfer function, just in case if rounding error causes pool to not have enough HEDXs.
    function safehedxTransfer(address _to, uint256 _amount) internal {
        uint256 hedxBal = hedx.balanceOf(address(this));
        if (_amount > hedxBal) {
            hedx.transfer(_to, hedxBal);
        } else {
            hedx.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        hedx.safeTransfer(address(msg.sender), _amount);
