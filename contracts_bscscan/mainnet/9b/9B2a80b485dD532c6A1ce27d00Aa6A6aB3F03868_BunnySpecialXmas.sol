...
contract BunnySpecialXmas is Ownable {
     ...
     */
    function canClaim(address _userAddress) public returns (bool) {
        (, uint256 numberUserPoints, , , , bool active) = pancakeProfile.getUserProfile(msg.sender);
        // If user is able to mint this NFT
        if (!hasClaimed[_userAddress] && block.number < endBlock && active && numberUserPoints >= pancakeProfileThresholdPoints) {
            return true;
        }
        return false;
    }
