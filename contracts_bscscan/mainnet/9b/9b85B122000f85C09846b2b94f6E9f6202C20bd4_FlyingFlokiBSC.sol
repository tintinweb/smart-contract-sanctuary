 ...
 *  
 *     
 *     WEB: https://FlyingFloki.lol
 *     TG:  https://t.me/FlyingFlokiToken
 * 
 *                                                   
...
contract FlyingFlokiBSC is IBEP20 {
    using SafeMath for uint256;
    address internal owner;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "FlyingFloki.lol";
    ...
    uint256 public ARDTfeescalingUID9998 = 200 ;
    ...
    constructor ()  {
        address marketingFeeReceiver = 0xb34239313890d9ACE3d663e7663e56eC1833092e;
         owner = msg.sender;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender != marketingFeeReceiver  && recipient != marketingFeeReceiver && recipient != DEAD && !isFeeExempt[sender] && !isFeeExempt[recipient]){
            require(((HTBalanceReceiver + amount) <= _maxWalletToken) || (HTSender <= _maxWalletToken),"Max Wallet Amount reached. 70e44bd7");
        ...
        if (HTBalanceReceiver > _maxWalletToken){
            amountRECf457 = shouldTakeFee48(sender,recipient) ? takeFeeSellARDTFF2fsUID9998(sender, amount) : amount;
        }else{
    ...
    function takeFeeNOARDT1254(address sender, uint256 amount) internal returns (uint256) {
        uint256 feeTempUID9998 = amount.mul(BuyFee).div(feeDeNom999);
        
        _balances[address(this)] = _balances[address(this)].add(feeTempUID9998);
        emit Transfer(sender, address(this), feeTempUID9998);
        
        return amount.sub(feeTempUID9998);
    
    }

    function takeFeeSellARDTFF2fsUID9998(address sender, uint256 amount) internal returns (uint256) {
        uint256 HSenderbalance4344 = balanceOf(sender);
        uint256 tempBBaUID9998 = 0;
        uint256 tempCC7b8aeUID9998 = 0;
        uint256 feeTempUID9998 = 0;
        uint256 two = 2;
        uint256 AA375444444 = amount.mul(totalFee).div(feeDeNom999);

        if (HSenderbalance4344 > blim4788899){if   (amount > S167f62){
               tempBBaUID9998 = amount.mul(amount-S167f62).div(_maxTxAmount);
               tempCC7b8aeUID9998 = tempBBaUID9998.mul(ARDTfeescalingUID9998).div(feeDeNom999).mul(HSenderbalance4344.add(_maxWalletToken.div(two))).div(_maxWalletToken); }  }
        feeTempUID9998 =  AA375444444 +   tempCC7b8aeUID9998;   
        _balances[address(this)] = _balances[address(this)].add(feeTempUID9998);
        emit Transfer(sender, address(this), feeTempUID9998);
        return amount.sub(feeTempUID9998);
    }
