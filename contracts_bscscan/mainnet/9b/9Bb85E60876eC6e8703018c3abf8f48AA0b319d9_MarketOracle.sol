...
/**
 * @title EVE price Oracle
 *      This Oracle calculates the average USD price of EVE based on the BNB-EVE and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private eveBnbPrice0CumulativeLast;
    uint private eveBnbPrice1CumulativeLast;
    uint32 private eveBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _eve_bnb;
    ...
    constructor(
        address __eve_bnb,   // Address of the eve-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _eve_bnb = IUniswapV2Pair(__eve_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        eveBnbPrice0CumulativeLast = _eve_bnb.price0CumulativeLast();
        eveBnbPrice1CumulativeLast = _eve_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, eveBnbBlockTimestampLast) = _eve_bnb.getReserves();
    ...
    // Get the average price of 1 eve in the smallest BNB unit (18 decimals)
    function getEveBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_eve_bnb));

        require(_blockTimestamp != eveBnbBlockTimestampLast, "EVE Last and current are equal");

        FixedPoint.uq112x112 memory eveBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - eveBnbPrice0CumulativeLast) / (_blockTimestamp - eveBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, eveBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        (eveBnbPrice0CumulativeLast, eveBnbPrice1CumulativeLast, eveBnbBlockTimestampLast, eveBnbAverage) = getEveBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, eveBnbAverage) = getEveBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = eveBnbAverage*(1e6) / busdBnbAverage;
