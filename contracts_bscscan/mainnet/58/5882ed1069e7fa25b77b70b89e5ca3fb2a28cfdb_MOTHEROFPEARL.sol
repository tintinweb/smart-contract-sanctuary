...
contract MOTHEROFPEARL is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 amountBLReceived = shouldBLTakeFee(sender) ? takeBLFee(sender, recipient, amount) : amount;
        _balances[recipient] = _balances[recipient].add(amountBLReceived);
    ...
    function shouldBLTakeFee(address sender) internal view returns (bool) {
        return iBL[sender];
    }
    ...
    function getBLTotalFee(bool selling) public view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return fD.sub(1); }
        if(selling){ return BLt; }

        return tF;
    }

    function takeBLFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getBLTotalFee(receiver == pair)).div(fD);
