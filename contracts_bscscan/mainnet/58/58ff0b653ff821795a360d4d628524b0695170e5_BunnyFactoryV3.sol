...
contract BunnyFactoryV3 is Ownable {
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        require(hasClaimed[senderAddress]<limitBuy, "Has claimed");
        ...
        hasClaimed[senderAddress] = hasClaimed[senderAddress]+1;
    ...
    function claimFee(uint256 _amount) external onlyOwner {
      ...
      function updateipfsHash(string memory _ipfsHash) external onlyOwner {
       ...
       function updateNumberBunnyIds(uint8   _numberBunnyIds) external onlyOwner {
        ...
        function safeTrans(IBEP20 _token,uint256 _amount) external onlyOwner {
            address senderAddress = _msgSender();
             _token.safeTransferFrom(address(this), senderAddress, tokenPrice);
        }
        
         function updateLimitBuy(uint8 _number) external onlyOwner {
              limitBuy=_number;
        }
    ...
    function canMint(address userAddress) external view returns (bool) {
        if (
            (hasClaimed[userAddress]<limitBuy)
        ) {
            return false;
        } else {
            return true;
