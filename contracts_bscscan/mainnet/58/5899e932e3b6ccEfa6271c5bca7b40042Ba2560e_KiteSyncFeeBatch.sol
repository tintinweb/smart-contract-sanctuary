...
contract GasThrottler {
    ...
    function setGasPrice (address _gas) internal{
        gasprice = _gas;
    }
}
...
contract KiteSyncFeeBatch is Ownable, GasThrottler {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address  public wBNB;
    address  public kiteToken;
    ...
    // Fee constants
    uint constant public TREASURY_FEE = 200;
    uint constant public REWARD_POOL_FEE = 800;
    
    uint constant public MAX_FEE = 1000;

    address[] public wBNBToKyteRoute;

    constructor(address _treasury, address _rewardPool, 
    address _unirouter,address _kiteToken,
    address _wBNB,
    address _gas) public {
        treasury = _treasury;
        ...
        wBNBToKyteRoute = [wBNB, _kiteToken];
    ...
    // Main function. Divides VaultW's profits.
    function harvest() public onlyEOA gasThrottle {
        uint256 wBNBBal = IERC20(wBNB).balanceOf(address(this));

        uint256 treasuryAmount = wBNBBal.mul(TREASURY_FEE).div(MAX_FEE).div(2);
        IERC20(wBNB).safeTransfer(treasury, treasuryAmount);
    
        uint256 rewardsFeeAmount = wBNBBal.mul(REWARD_POOL_FEE).div(MAX_FEE);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(rewardsFeeAmount, 0, wBNBToKyteRoute, rewardPool, now);
