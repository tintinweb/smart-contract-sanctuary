...
*/
/*
BAIme is built upon the fundamentals as utility token for BAI marketplaces. Apart from this it adds liquidity automatically function to keep the investor's value.
These additional features are:
1) sell/buy transactions have tax percent and distributed to holders for HODLing 
2) % tax for improving liquidity and marketing is collected
3) maximum token of one holder is set   
*/
...
contract BAIme is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x7CFb46DC2Cc625065AF4d5421C646266D790F3BB); // Marketing Address
    ...
    string private _name = "BAIme Token";
    string private _symbol = "BAIME";
    ...
    uint256 public _maxTokenHolder = 50 * 10**6 * 10**9;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Calc liquidity Fee  
        uint256 addLiquidPercent = _liquidityFee.sub(marketingDivisor);
        

        uint256 addLiquidPart = contractTokenBalance.div(_liquidityFee).mul(addLiquidPercent);
        ...
        uint256 mktBalance = newBalance.div(addLiquidPercent.div(2).add(marketingDivisor)).mul(marketingDivisor);

        // add liquidity to uniswap
        addLiquidity(otherHalf, newBalance.sub(mktBalance));

        //Send to Marketing address
        transferToAddressETH(marketingAddress,mktBalance );
