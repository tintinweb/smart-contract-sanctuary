...
/**
    Join Lord Pepe Community. Upcoming Meme Contest !!!!
    Telegram : https://t.me/LordPepeBSC
*/
...
contract LordPepe is Context, IERC20, Ownable {
    ...
    string private _name = "Lord Pepe";
    string private _symbol = "LORDPEPE";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0xf7f3a61902d39a3408422e3171b6e41c0F606e4B);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeesLP;
    mapping (address => bool) private _isExcludedFromMaxBalanceLP;

    uint256 private constant _maxFeesLP = 12;
    uint256 private _totalFeesLP;
    uint256 private _totalFeesLPContract;
    uint256 private _burnFeesLP;
    uint256 private _liquidityFeesLP;
    uint256 private _marketingFeeLP;
    
    uint256 private _maxBalanceWalletLP;
    ...
    constructor () {
        ...
        _isExcludedFromFeesLP[owner()] = true;
        _isExcludedFromFeesLP[address(this)] = true;
        
        _isExcludedFromMaxBalanceLP[owner()] = true;
        _isExcludedFromMaxBalanceLP[address(this)] = true;
        _isExcludedFromMaxBalanceLP[uniswapV2Pair] = true;

        _liquidityFeesLP = 2;
        _marketingFeeLP = 10;
        _burnFeesLP = 0;
        _totalFeesLP = _liquidityFeesLP.add(_marketingFeeLP).add(_burnFeesLP);
        _totalFeesLPContract = _liquidityFeesLP.add(_marketingFeeLP);

        _liquidityThreshhold = 3000 * 10**9 * 10**_decimals;
        _maxBalanceWalletLP = 1000000 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesLP).add(_marketingFeeLP) <= _maxFeesLP, "Fees are too high.");
        _liquidityFeesLP = newLiquidityFee;
        _totalFeesLP = _liquidityFeesLP.add(_marketingFeeLP).add(_burnFeesLP);
        _totalFeesLPContract = _liquidityFeesLP.add(_marketingFeeLP);
        emit SetLiquidityFee(_liquidityFeesLP);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesLP.add(_burnFeesLP).add(newMarketingFee) <= _maxFeesLP, "Fees are too high.");
        _marketingFeeLP = newMarketingFee;
        _totalFeesLP = _liquidityFeesLP.add(_marketingFeeLP).add(_burnFeesLP);
        _totalFeesLPContract = _liquidityFeesLP.add(_marketingFeeLP);
        emit SetMarketingFee(_marketingFeeLP);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesLP.add(newBurnFee).add(_marketingFeeLP) <= _maxFeesLP, "Fees are too high.");
        _burnFeesLP = newBurnFee;
        _totalFeesLP = _liquidityFeesLP.add(_marketingFeeLP).add(_burnFeesLP);
        emit SetBurnFee(_burnFeesLP);
    }
    ...
    function setMaxBalance(uint256 newMaxBalance) external onlyOwner(){
        // Minimum _maxBalanceWalletLP is 0.5% of _totalSupply 
        require(newMaxBalance >= _totalSupply.mul(5).div(1000));
        _maxBalanceWalletLP = newMaxBalance;
    }

    function isExcludedFromFees(address account) public view returns(bool) {
        return _isExcludedFromFeesLP[account];
    }
    
    function excludeFromFees(address account) public onlyOwner {
        _isExcludedFromFeesLP[account] = true;
    }
    
    function includeInFees(address account) public onlyOwner {
        _isExcludedFromFeesLP[account] = false;
    }

    function isExcludedFromMaxBalance(address account) public view returns(bool) {
        return _isExcludedFromMaxBalanceLP[account];
    }
    
    function excludeFromMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalanceLP[account] = true;
    }
    
    function includeInMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalanceLP[account] = false;
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletLP
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludedFromMaxBalanceLP[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletLP,
        ...
        if(
            ...
            _totalFeesLPContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesLP[from] || _isExcludedFromFeesLP[to])
            && _totalFeesLP > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesLPContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesLP).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesLP).div(_totalFeesLPContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeLP).div(_totalFeesLPContract);
