...
contract BiswapByalanLP is ByalanIsland, Sailor, GasThrottler, IByalan, ReentrancyGuard {
        ...
        )
    {
        ...
        if (lpToken0 != BSW) {
            ...
            require(
                IUniswapV2Router02(unirouter).getAmountsOut(1e6, _bswToLp0Route)[_bswToLp0Route.length - 1] > 0,
                "invalid lp 0 route"
            );
        ...
        if (lpToken1 != BSW) {
            ...
            require(
                IUniswapV2Router02(unirouter).getAmountsOut(1e6, _bswToLp1Route)[_bswToLp1Route.length - 1] > 0,
                "invalid lp 1 route"
            );
    ...
    function chargeFees() internal nonReentrant {
        ...
        IUniswapV2Router02(unirouter).swapExactTokensForETH(toBnb, 0, bswToWbnbRoute, address(this), block.timestamp);
    ...
    function pendingReward() external view override returns (uint256) {
        return IMasterChef(MASTERCHEF).pendingBSW(pid, address(this)) + IERC20(BSW).balanceOf(address(this));
    }
...
abstract contract ByalanIsland is Ownable, Pausable, IByalanIsland {
    ...
    function setHydra(address _hydra) external onlyManager {
        hydra = _hydra;
    }
...
abstract contract Sailor is ByalanIsland, ISailor {
    ...
    function setTotalFee(uint256 _totalFee) external onlyManager {
    ...
    function setKSWFee(uint256 _fee) external onlyManager {
        kswFee = _fee;
        feeSum = callFee + treasuryFee + kswFee;
    }
}
...
abstract contract GasThrottler is Ownable {
