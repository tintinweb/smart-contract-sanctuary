...
/**
 * @dev Collection of functions related to the address type @KartingToken
 */
library Address {
    /**
...
contract KartingToken is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        _symbol = "KART";
        _decimals = 9;
        // 100 trillion tokens total
        _tTotal = 1000 * 10 ** 6 * 10 ** _decimals;
        ...
        // 5 trillion
        numTokensSellToAddToLiquidity = ((_tTotal / 10 ** _decimals) * 5 / 100) * 10 ** _decimals;
        ...
        //Include owner to dev wallet
        includeDevWallet(_msgSender());
    ...
    function balanceOf(address wallet) public view override returns (uint256) {
        if (_isExcluded[wallet]) return _tOwned[wallet];
    ...
    function excludeFromReward(address wallet) public onlyOwner() {
        require(!_isExcluded[wallet], "Wallet is already excluded");
        if (_rOwned[wallet] > 0)
            _tOwned[wallet] = tokenFromReflection(_rOwned[wallet]);
        _isExcluded[wallet] = true;
    ...
    function includeDevWallet(address wallet) public onlyOwner() {
        require(!_isDevWallet[wallet], "Wallet is already included");
        _isDevWallet[wallet] = true;
        _devWallets.push(wallet);
    }

    function excludeDevWallet(address wallet) external onlyOwner() {
        require(_isDevWallet[wallet], "Wallet is already excluded");

        for (uint256 i = 0; i < _devWallets.length; i++) {
            if (_devWallets[i] == wallet) {
                _devWallets[i] = _devWallets[_devWallets.length - 1];
                _isDevWallet[wallet] = false;
                _devWallets.pop();
                break;
            }
        }
    ...
    function isDevWallet(address _address) public onlyOwner() view returns (bool) {
        return _isDevWallet[_address];
    }
    ...
    function _takeDev(uint256 tDev) private {
        if (_devWallets.length == 0) return;

        uint256 currentRate = _getRate();
        uint256 rDev = tDev.mul(currentRate) / _devWallets.length;

        for (uint256 i = 0; i < _devWallets.length; i++) {
            address devWallet = _devWallets[i];
    ...
    function _transfer(
        ...
        //if any wallet belongs to _isExcludedFromFee wallet then remove the fee
