 ...
 */
contract Unitroller is UnitrollerAdminStorage, EsgtrollerErrorReporter {

    /**
      * @notice Emitted when pendingEsgtrollerImplementation is changed
      */
    ...
    function _setPendingImplementation(address newPendingImplementation) public returns (uint) {
        ...
        address oldPendingImplementation = pendingEsgtrollerImplementation;

        pendingEsgtrollerImplementation = newPendingImplementation;

        emit NewPendingImplementation(oldPendingImplementation, pendingEsgtrollerImplementation);

        return uint(Error.NO_ERROR);
    }

    /**
    * @notice Accepts new implementation of esgtroller. msg.sender must be pendingImplementation
    * @dev Admin function for new implementation to accept it's role as implementation
    * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    */
    function _acceptImplementation() public returns (uint) {
        // Check caller is pendingImplementation and pendingImplementation ≠ address(0)
        if (msg.sender != pendingEsgtrollerImplementation || pendingEsgtrollerImplementation == address(0)) {
        ...
        esgtrollerImplementation = pendingEsgtrollerImplementation;

        pendingEsgtrollerImplementation = address(0);
...
abstract contract PriceOracle {
    ...
    /**
      ...
      */
    function getUnderlyingPrice(EToken eToken) public virtual returns (uint);
}
...
abstract contract InterestRateModel {
    ...
    /**
      ...
      */
    function getSupplyRate(uint cash, uint borrows, uint reserves, uint reserveFactorMantissa) public virtual returns (uint);
...
contract UnitrollerAdminStorage {
    ...
    */
    address public pendingEsgtrollerImplementation;
}
...
contract EsgtrollerV3Storage is EsgtrollerV2Storage {
    struct EsgMarketState {
        /// @notice The market's last updated esgBorrowIndex or esgSupplyIndex
        uint224 index;
...
abstract contract EsgtrollerInterface {
    /// @notice Indicator that this is a Esgtroller contract (for inspection)
    bool public constant isEsgtroller = true;
...
contract EsgtrollerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        ESGTROLLER_MISMATCH,
...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        BORROW_ESGTROLLER_REJECTION,
        LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        LIQUIDATE_ESGTROLLER_REJECTION,
        LIQUIDATE_ESGTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
...
contract ETokenStorage {
     ...
     */
    EsgtrollerInterface public esgtroller;
...
abstract contract ETokenInterface is ETokenStorage {
    ...
    /**
     * @notice Event emitted when esgtroller is changed
     */
    event NewEsgtroller(EsgtrollerInterface oldEsgtroller, EsgtrollerInterface newEsgtroller);
    ...
    function allowance(address owner, address spender) external virtual returns (uint);
    function balanceOf(address owner) external virtual returns (uint);
    function balanceOfUnderlying(address owner) external virtual returns (uint);
    function getAccountSnapshot(address account) external virtual returns (uint, uint, uint, uint);
    function borrowRatePerBlock() external virtual returns (uint);
    function supplyRatePerBlock() external virtual returns (uint);
    function totalBorrowsCurrent() external virtual returns (uint);
    function borrowBalanceCurrent(address account) external virtual returns (uint);
    function borrowBalanceStored(address account) public virtual returns (uint);
    function exchangeRateCurrent() internal virtual returns (uint);
    function exchangeRateStored() public virtual returns (uint);
    function getCash() external virtual returns (uint);
    ...
    function _setEsgtroller(EsgtrollerInterface newEsgtroller) internal virtual returns (uint);
...
abstract contract CDelegatorInterface is CDelegationStorage {
     ...
     */
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData) internal virtual;
}
...
abstract contract EToken is ETokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    function balanceOf(address owner) external override returns (uint256) {
     ...
     */
    function getAccountSnapshot(address account) external override returns (uint, uint, uint, uint) {
        ...
        return (uint(Error.NO_ERROR), eTokenBalance, borrowBalance, exchangeRateMantissa);
    }
     ...
     * @return The calculated balance
     */
    function borrowBalanceStored(address account) public override returns (uint) {
    ...
    /**
     * @notice Calculates the exchange rate from the underlying to the EToken
     * @dev This function does not accrue interest before calculating the exchange rate
     * @return Calculated exchange rate scaled by 1e18
     */
    function exchangeRateStored() public override returns (uint) {
     ...
     */
    function getCash() external override returns (uint) {
        return getCashPrior();
    }
    ...
    /**
     * @notice Sender supplies assets into the market and receives eTokens in exchange
    ...
    /**
     * @notice User supplies assets into the market and receives eTokens in exchange
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
         ...
         *  Note: The eToken must handle variations between ERC-20 and ETH underlying.
         *  `doTransferIn` reverts if anything goes wrong, since we can't be sure if
         *  side-effects occurred. The function returns the amount actually transferred,
         *  in case of a fee. On success, the eToken holds an additional `actualMintAmount`
         *  of cash.
         */
        vars.actualMintAmount = doTransferIn(minter, mintAmount);

        /*
         * We get the current exchange rate and calculate the number of eTokens to be minted:
        ...
        /*
         * We calculate the new total supply of eTokens and minter token balance, checking for overflow:
    ...
    /**
     * @notice Sender redeems eTokens in exchange for the underlying asset
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
    ...
    /**
     * @notice Sender redeems eTokens in exchange for a specified amount of underlying asset
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemAmount The amount of underlying to receive from redeeming eTokens
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    ...
    /**
     * @notice User redeems eTokens in exchange for the underlying asset
     * @dev Assumes interest has already been accrued up to the current block
     * @param redeemer The address of the account which is redeeming the tokens
     * @param redeemTokensIn The number of eTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @param redeemAmountIn The number of underlying tokens to receive from redeeming eTokens (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
         ...
         *  Note: The eToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the eToken has redeemAmount less of cash.
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
         ...
         *  Note: The eToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the eToken borrowAmount less of cash.
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
         ...
         *  Note: The eToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the eToken holds an additional repayAmount of cash.
     ...
     * @param borrower The borrower of this eToken to be liquidated
     * @param eTokenCollateral The market in which to seize collateral from the borrower
     * @param repayAmount The amount of the underlying borrowed asset to repay
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, ETokenInterface eTokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        error = eTokenCollateral.accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
        ...
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, eTokenCollateral);
    }
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, ETokenInterface eTokenCollateral) internal returns (uint, uint) {
        ...
        /* Verify eTokenCollateral market's block number equals current block number */
        if (eTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
        ...
        uint seizeError;
        if (address(eTokenCollateral) == address(this)) {
            seizeError = seizeInternal(address(this), liquidator, borrower, seizeTokens);
        } else {
            seizeError = eTokenCollateral.seize(liquidator, borrower, seizeTokens);
        }
        ...
        emit LiquidateBorrow(liquidator, borrower, actualRepayAmount, address(eTokenCollateral), seizeTokens);
     ...
     * @dev Will fail unless called by another eToken during the process of liquidation.
     *  Its absolutely critical to use msg.sender as the borrowed eToken and not a parameter.
    ...
    /**
     * @notice Transfers collateral tokens (this market) to the liquidator.
     * @dev Called only during an in-kind liquidation, or by liquidateBorrow during the liquidation of another EToken.
     *  Its absolutely critical to use msg.sender as the seizer eToken and not a parameter.
     * @param seizerToken The contract seizing the collateral (i.e. borrowed eToken)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of eTokens to seize
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setEsgtroller(EsgtrollerInterface newEsgtroller) internal override returns (uint) {
    ...
    function _addReservesFresh(uint addAmount) internal returns (uint, uint) {
         ...
         *  Note: The eToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the eToken holds an additional addAmount of cash.
         *  doTransferIn reverts if anything goes wrong, since we can't be sure if side effects occurred.
         *  it returns the amount actually transferred, in case of a fee.
         */
     ...
     */
    function getCashPrior() public view virtual returns (uint);
