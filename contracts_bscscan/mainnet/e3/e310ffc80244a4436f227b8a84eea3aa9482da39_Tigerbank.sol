...
/*
 ...
 /This is dedicated to all holders, WAGMIERers the degen hodlers,
 TigerBank is bringing non existestential banking to DeFi. 
 Hodl and lock your assets on the TigerBank and HODL your assets like a king hodler.
 Vest Assets 
 Lock LP
 TimeLock on Team Tokens
 Presales and IDO 
 All are the feaures of TigerBank
 Tokenomics /
     8% marketing
     1% Liquidity
     4% BankDev/buyback
     15% Tax
     15% Buy and sell tax
Telegram : t.me/tiGerBankBSC
Twitter: twitter.com/TigerBankBSC
Website: www.tigerbank.site
...
contract Tigerbank is IERC20, Ownable {
    ...
    string _name = "TigerBank";
    string _symbol = "TB";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);    // 100,000,000,000
    uint256 public _tsxnCap = (_totalSupply * 20) / 1000;       // 2% of total supply
    ...
    uint256 BankDevFee = 40;   // 4%
    uint256 charityFee = 1;     // 1%
    uint256 devFee = 0;         // 0%
    uint256 totalFee = 120;     // 13%
    uint256 feeDenominator = 1000;  // 100%
    
    address autoLiquidityReceiver;
    address marketingFeeReceiver;
    address BankDevReceiver;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (marketingFee, liquidityFee, BankDevFee, devFee, charityFee, totalFee, feeDenominator);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBBankDev = amountBNB.mul(BankDevFee).div(totalBNBFee);

        (bool success,)  = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool success2,) = payable(BankDevReceiver).call{value: amountBNBBankDev, gas: 30000}("");
    ...
    function adjustFees(uint256 _liquidityFee, uint256 _BankDevFee, uint256 _marketingFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        BankDevFee = _BankDevFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_BankDevFee).add(_marketingFee);
    ...
    function disableAntiSnipe() external onlyOwner {
        ...
        BankDevFee = 40;
        marketingFee = 80;
        totalFee = liquidityFee.add(BankDevFee).add(marketingFee);
        feeDenominator = 1000;
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _BankDevReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        BankDevReceiver = _BankDevReceiver;
    }
