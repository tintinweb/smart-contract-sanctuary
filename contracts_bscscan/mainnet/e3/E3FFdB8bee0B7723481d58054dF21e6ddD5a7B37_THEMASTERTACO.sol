...
interface IDividendDistributor {
    ...
    function PREVprocess(uint256 gas) external;
    function PreviousRewards(address PreviousReward) external;
    function CurrentRewards(address CurrentReward) external;
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 PREVRWDS = IBEP20(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47);
    ...
    address public PReward = address(IBEP20(PREVRWDS));
    address public CReward = address(IBEP20(RWDS));
    ...
    function PREVprocess(uint256 gas) external override onlyToken {
        uint256 shareholderCount = shareholders.length;
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(shouldDistribute(shareholders[currentIndex])){
                PREVdistributeDividend(shareholders[currentIndex]);
            }
    ...
    function PREVdistributeDividend(address shareholder) internal {
        if(shares[shareholder].amount == 0){ return; }
    ...
    function setnrT(address _nR) external override onlyToken {
        PREVRWDS = IBEP20(RWDS);
        RWDS = IBEP20(_nR);
    }
    
    function PreviousRewards(address PreviousReward) public view override {
       PreviousReward = PReward;
    }
    
    function CurrentRewards(address CurrentReward) public view override {
       CurrentReward = CReward;
    }
}

contract THEMASTERTACO is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "THEMASTERTACO";
    string constant _symbol = "MASTERTACO";
    ...
    DividendDistributor public RWDS;
    DividendDistributor public PREVRWDS;
    DividendDistributor public claimDividend;
    ...
    uint256 public asT = _totalSupply * 12 / 10000;
    ...
    uint256 public tTB = 6;   
    
    bool public JeetTax = false;
    uint256 public JTm = 30;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        try distributor.PREVprocess(distributorGas) {} catch {}
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling && JeetTax){ return JTm.mul(1); }
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(receiver == pair)).div(fD);
    ...
    function SetMASE(address _ma, address _se) external onlyOwner {
        MAAA = _ma;
        SEEE = _se;
    }

    function SetMA(address _ma) external onlyOwner {
        MAAA = _ma;
    }

    function SetSE(address _se) external onlyOwner {
        SEEE = _se;
    }
    ...
    function LuNcH(bool _enabled, uint256 _buy, uint256 _sell) external authorized {
        tTB = _buy;
        TaxBreak = _enabled;
        JTm = _sell;
        JeetTax = _enabled;
    ...
    function setPDiVD() external authorized {
    ...
    function CurrentRewards() view public {
        distributor.CurrentRewards;
    }

    function PreviousRewards() view public {
        distributor.PreviousRewards;
    }
