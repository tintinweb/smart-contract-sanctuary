...
contract Pessy is Context, IBEP20, Ownable {
    ...
    address payable public _marketingWallet = payable(0x8D4aB2425b494C7a0A9a513915BA601f5b5B03e1);
    ...
    string private constant _name     = "P. Ness Monster";
    string private constant _symbol   = "PESSY";
    uint8  private constant _decimals = 9;
    
    uint256 public  _taxFee       = 1; 
    uint256 public  _liquidityFee = 2; 
    uint256 public  _marketingFees  = 6; 
    uint256 public  _burnFee      = 1; 
    uint256 public  _totalFees      = _taxFee.add(_liquidityFee).add(_marketingFees).add(_burnFee);
    
    uint256 public  _maxTxAmount     = 500 * 10**6 * 10**9;
    uint256 private _minimumTokenBalance = 50 * 10**6 * 10**9;
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        _taxFee = taxFee;
        _totalFees      = _taxFee.add(_liquidityFee).add(_marketingFees).add(_burnFee);
    }
    ...
    function setMarketingAndDevelopmentFeePercent(uint256 marketingAndDevelopmentFee) external onlyOwner {
        _marketingFees = marketingAndDevelopmentFee;
        _totalFees      = _taxFee.add(_liquidityFee).add(_marketingFees).add(_burnFee);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 _totalFeesForBNB = _liquidityFee.add(_marketingFees);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private ltsTheSwap {
        ...
        if( marketingAndDevelopmentPart > 0)
        {
            ...
            transferToAddressBNB(_marketingWallet,newBalance);
        }
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        uint256 previousMarketingAndDevelopmentFee  = _marketingFees;
        ...
        if (!takeFee) {
            ...
            _marketingFees  = previousMarketingAndDevelopmentFee;
