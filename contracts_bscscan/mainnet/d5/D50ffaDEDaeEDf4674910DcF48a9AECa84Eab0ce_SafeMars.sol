...
contract SafeMars is Context, IERC20, Ownable {
    ...
    uint private _holders_n; // record holders
    ...
    string private _name = "TEST version 2";
...
// -----------------------------------------------------------------
    ...
    function _burn(address account, uint256 tAmount) internal {
        ...
        require(tAmount <= _rOwned[account].div(currentRate));

        if(_isExcluded[account]){
            require(tAmount <= _tOwned[account], "insufficient tOwned");
    ...
    function _reflectBurnFee(uint256 rBurn, uint256 tBurn) private{

        _rTotal = _rTotal.sub(rBurn);
        _tTotal = _tTotal.sub(tBurn);
        _burnTotal = _burnTotal.add(tBurn);
    }
    ...
    function _tokenTransfer(
        ...
        require(_isHolder[sender],"sender is not holder");
        if(!_isHolder[recipient]){
            // if new holder, increment
            _isHolder[recipient] = true;
            _holders_n = _holders_n.add(1);
        }
        ...
        // check if sender still has balance, if not then earse it from holders
        if(balanceOf(sender)==0){
            _isHolder[sender] = false;
            _holders_n = _holders_n.sub(1);
        }
    }
    ...
    function _transferStandard(
        ...
        uint256 tBurn = _burnFee.mul(tFee).div(_taxFee);
        uint256 rBurn = tBurn.mul(_getRate());
        _reflectBurnFee(rBurn, tBurn);
        emit Transfer(sender, recipient, tTransferAmount);
    }
