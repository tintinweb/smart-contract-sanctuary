...
contract WANDTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SWAND }
    ...
    address public sWAND;
    uint public sWANDQueue; // Delays change to sWAND address
    ...
    /**
        @notice allow approved address to deposit an asset for WAND
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint WAND needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( WAND ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn WAND for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IWANDERC20( WAND ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sWAND ).balanceOf( msg.sender ); // Can only borrow against sWAND held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with WAND
        @param _amount uint
     */
    function repayDebtWithWAND( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IWANDERC20( WAND ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, WAND, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( WAND ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( WAND ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns WAND valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match WAND decimals
            value_ = _amount.mul( 10 ** IERC20( WAND ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SWAND ) { // 9
            sWANDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
