...
contract BeDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BeDogeDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public marketingAddress = payable(0x0654E9F0fE5b4563E11010eA03BB7f7EED87afe8);

    uint256 public maxSellTransactionAmount = 100000000 * (10**9); // 0.1 %
    uint256 public swapTokensAtAmount = 1000000 * (10**9); //0.001%
    uint256 public swapTokensAtAmountMax = 1000000000 * (10**9); // 1%
    ...
    constructor() ERC20("BezosDoge", "$BeDoge") {
        dividendTracker = new BeDogeDividendTracker(payable(this));

        liquidityWallet = owner();

        uniswapV2Router = IUniswapV2Router02(
            // 0x1Ed675D5e63314B760162A3D1Cae1803DCFC87C7 // Test
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BeDogeDividendTracker newDividendTracker = BeDogeDividendTracker(
            payable(newAddress)
        );
    ...
    function updatePayoutToken(address token) public {
        require(balanceOf(msg.sender) > 0, "You must own more than zero $BeDoge tokens to switch your payout token!");
...
contract BeDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "BeDoge_Dividend_Tracker",
            "BeDoge_Dividend_Tracker"
        )
    {
        beDogeDealer = BeDoge(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BeDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BeDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BeDoge contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(beDogeDealer);

            uint256 prevBalance = beDogeDealer.balanceOf(address(this));
            ...
            {
                uint256 received = beDogeDealer.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    beDogeDealer.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
