...
//
// Copyright of The $Borneo Team
// There is a pot of gold at the end of every Borneo
...
/**
 * @dev The official BorneoToken smart contract
 */
contract BorneoCarbonProject  is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // General Info
    string private _name     = "BORNEO CARBON";
    string private _symbol   = "EMMI";
    uint8  private _decimals = 4;
    
    // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router; // The address of the PancakeSwap V2 Router
    address public _pancakeswapV2LiquidityPair;     // The address of the PancakeSwap V2 liquidity pairing for Borneo/WBNB
    ...
    // Addresses
    address payable public _marketingAddress = payable(0x22c9A0ac0de890e13b1F748E7c722301Ff88cFb5); // Marketing address used to pay for marketing
    ...
    uint256 public  _sellTaxFee       = 5; // 5% tax 
    uint256 public  _whaleSellTaxFee  = 15; // 15% tax 
    uint256 private _previousTaxFee;
    
    // Token Limits
    uint256 public _maxTxAmount        = 21 * 10**15 * 10**4; // ===========
    uint256 public _tokenSwapThreshold = 20 * 10**15 * 10**4; // ===========
    
    // Timer Constants 
    uint private constant DAY = 60 * 60 * 24; // How many seconds in a day
    
    // Anti-Whale Settings 
    uint256 public _whaleSellThreshold = 5 * 10**9 * 10**4; // ============
    ...
    uint256 public _buybackUpperLimit    = 1 * 10**9 * 10**4; // 1 billion $EMMI will be used to buy at maximum in any one trade
    ...
    uint256 public  _lotteryThreshold    = 10 * 10**6 * 10**4; // 10 million tokens required to be in the pool before lottery can be triggered
    uint256 public  _lotteryMinimumSpend = 10 * 10**6 * 10**4; // 10 million tokens required to buy before before lottery can be triggered
    ...
    /**
     * @notice Withdraws non-Borneo tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // Gets the contracts Borneo balance for buybacks, charity, liquidity and marketing
    ...
    /**
     * @notice Burns Borneo tokens straight to the burn address
     */
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // Swap 7/8ths of Borneo tokens for BNB
        ...
        // We now have 6/7ths left of BNB (converted from Borneo)
