...
APELoungeToken
Welcome to the Parent token of @DetectiveBSCApeLounge

APELOUNGETOKEN (ALT) is the parent token of DetectiveBSCApeLounge. This is where it all starts.
$ALT will be the first of its kind to be fueled by daily degen projects provided by the lounge in order to
create an everlasting buying pressure through the rewards contract and fees. The theory is simple.
You hold $ALT, get additional benefits and perks such as discount taxes on all upcoming degen plays, reflections  + lotteries.
Our daily stealth launches will always be allocated 5% of all buy/sell transactions to purchase $ALT.
...
/* Token contract */
contract APELoungeToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    // Name and symbol
    string constant _name = "APELoungeToken";
    ...
    uint256 public earlybuyBlocks = 100;  // for $APL holders to buy early
    uint256 public holdingThreshold = 50;  // 1 basis point, i.e., 1 is 0.01%
    ...
    function takeFee(address sender, uint256 amount, bool isSell, bool GREEDMode) internal returns (uint256) {
        ...
        uint256 den = _hasDiscount(sender) ? discountFactor : 1;
