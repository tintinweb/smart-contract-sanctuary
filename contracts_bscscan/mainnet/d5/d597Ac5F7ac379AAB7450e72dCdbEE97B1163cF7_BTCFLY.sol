...
contract BTCFLY {
    ...
    address public owner;
     address public projectFee;
    uint256[] public GI_PERCENT = [200,100,100,100, 100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100];
    uint256[] public level_condition = [1,1,1,2,2,2,3,3,3,4,4,4,5,5,5,6,6,6,7,7];
    ...
    constructor(IBEP20 tokenAdd,address _projectFee) public {
        owner = msg.sender;
   ...
   function deposit(uint8 _tarif, address _upline, uint256 token_quantity) external payable {
      ...
      token.transfer(projectFee,token_quantity.mul(10).div(100));
    ...
    function _drawPool() private {
        ...
        for(uint8 i = 0; i < pool_bonuses.length; i++) {
            ...
            players[pool_top[i]].pool_bonus += win;
             players[pool_top[i]].total_pool_bonus += win;
            pool_balance -= win;
...
function withdraw() payable external {
    ...
    /*
        Only external call
    */
    function userInfo(address _addr) view external returns(uint256[] memory structure,uint256[] memory structurebusiness,address[] memory addrs, uint256[] memory deps) {
        Player storage player = players[_addr];
    ...
    function reward_business_info(address addr) view external returns(uint256)
    {
        Player storage player = players[addr];
        return player.levelBusiness[0];
    }
