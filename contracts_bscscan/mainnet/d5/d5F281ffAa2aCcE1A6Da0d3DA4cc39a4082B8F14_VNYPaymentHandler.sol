...
// #Features:
// When an user makes a pruchase on the $VNY Marketplace, the payment is processed by the VNYPaymentHandler Contract,
// and a record is written in the VNYStorage Contract Database.
// The payment method is hybrid, using both VNY as a static amount per character and BNB with a pricefeed.
// All the payments are managed as the following
// VNY: 100% of $VNY Tokens used for purchase are immediatly burned, reducing the circulating supply.
// BNB: 15% of the BNB are used to purchase $VNY and another 15% is used to deposit the pair VNY/BNB inside the LP;
// 30% of the BNB are used to purchase $VNY and stored, in order to pay the Royalty System's Users.
// 20% of the BNB are stored in the contract for The Hydra, to buyback and burn the $VNY Tokens, when decided.
// The remaining 20% of the BNB is sent to the Dev Wallet as project revenue.
...
interface VNYNFT {
    ...
    function distributeDivs(string calldata needle, uint256 tokenAmount) external;
}
...
contract VNYPaymentHandler is Ownable, ReentrancyGuard {
    ...
    IERC20 public bnbBusdLP;
    VNYStorage public vnyStorage;
    VNYNFT public vnyNft;

    uint256 public tokenCostPerCharacterWrapped = 30000 * 1e18;
    uint256 public tokenCostPerCharacterUnwrapped = 30000 * 1e18;
    ...
    constructor( ) {
        ...
        feeToken = IERC20(0xAbc69f2025bDb12efcdB8fd048d240fFf943ca82);
    ...
    function payUnwrapped(
        ...
        string memory txnhash) external payable {
        ...
        vnyNft.distributeDivs(needle, royaltyAmount);

        vnyStorage.createNewRecord(msg.sender, name,
        network,
        needle,
        vnyFeeAmount,
        priceUSD,
    ...
    function payWrapped(uint256 feeAmount, string calldata needle) external payable {
    ...
    function unwrapNFT(uint256 tokenId,
        ...
        vnyStorage.updateRecord(recordId, "","Unwrapped", "");
