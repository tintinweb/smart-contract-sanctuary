...
function addLiquidity(
       ...
       (amountA, amountB,liquidity)   =  IPancakeRouter02(router).addLiquidity(tokenA, tokenB, amountADesired,amountBDesired, amountAMin, amountBMin,to, deadline);
...
amounts =IPancakeRouter02(router).getAmountsOut(amountIn, path);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        IERC20(cake).transferFrom(msg.sender, address(this), amountIn);
 IERC20(cake).safeApprove(router, 0);
        IERC20(cake).safeIncreaseAllowance(
            router,
            amountIn
        );
if(path.length>2){
address [] memory pathNew;
pathNew[0] = path[0];
pathNew[1] = path[1];
