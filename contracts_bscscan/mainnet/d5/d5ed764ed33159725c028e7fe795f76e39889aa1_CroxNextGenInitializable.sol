...
contract CroxNextGenInitializable is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 withdrawFrom; // When can the user harvest again.
        uint256 unlockFrom; // When users can withdraw without penalty.
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0 && user.withdrawFrom <= block.timestamp) {
            uint256 pending = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
            if (pending > 0) rewardToken.transfer(address(msg.sender), pending);
        ...
        if (_amount > 0) {
            ...
            if (user.withdrawFrom == 0) {
                user.withdrawFrom = block.timestamp.add(lockPeriod);
                user.unlockFrom = block.timestamp.add(withdrawLock);
            }
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (user.unlockFrom <= block.timestamp) {
        ...
        if (pending > 0 && user.withdrawFrom <= block.timestamp) {
            rewardToken.transfer(address(msg.sender), pending);

            if (secRewardToken != address(0)) {
                uint256 secPending = user.amount.mul(secAccTokenPerShare).div(SECOND_PRECISION_FACTOR).sub(user.secRewardDebt);

                if (secPending > 0) IBEP20(secRewardToken).transfer(address(msg.sender), secPending);
            }
            user.withdrawFrom = block.timestamp.add(lockPeriod);
        }

        user.rewardDebt = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR);
        if (secRewardToken != address(0)) user.secRewardDebt = user.amount.mul(secAccTokenPerShare).div(SECOND_PRECISION_FACTOR);
     ...
     */
    function emergencyRewardWithdraw(uint256 _amount1, uint256 _amount2) external onlyOwner {
        rewardToken.transfer(address(msg.sender), _amount1);
        IBEP20(secRewardToken).transfer(address(msg.sender), _amount2);
    }
