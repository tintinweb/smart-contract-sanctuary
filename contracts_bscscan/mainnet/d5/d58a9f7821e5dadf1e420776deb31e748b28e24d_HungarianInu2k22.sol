...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all HungarianInu2k22 just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also HungarianInu2k22ed.
	...
	// File: contracts/HungarianInu2k22BEP20PayingToken.sol
	...
	// File: contracts/HungarianInu2k22.sol

	// 

	pragma solidity ^0.8.0;

	contract HungarianInu2k22 is ERC20, Ownable {
		using SafeMath for uint256;
		...
		event excludeMultipleAccountFromRewards(address[] HungarianInu2k22, bool isExcluded);
		...
		constructor() ERC20("HungarianInu2k22", "HungarianInu2k22"){
			uint256 _reflectionFee = 1;
			...
			totalFee = _reflectionFee + _liquidityFee + _marketingFee;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "HungarianInu2k22: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "HungarianInu2k22: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "HungarianInu2k22: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "HungarianInu2k22: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "HungarianInu2k22: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "HungarianInu2k22: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "HungarianInu2k22: Cannot update distributorGas to same value");
        ...
        function setFees (uint256 _liquidityFee, uint256 _marketingFee, uint256 _reflectionFee, uint256 _feeDenominator) external onlyOwner {
            liquidityFee = _liquidityFee;
            marketingFee = _marketingFee;
            reflectionFee = _reflectionFee;
            feeDenominator = _feeDenominator;
        ...
        function setDeadBlock (uint256 _deadBlock) external onlyOwner {
		...
		function _transfer(
		   ...
		   if(from != owner()){
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "HungarianInu2k22: This account cannot send tokens until trading is enabled");
			}
