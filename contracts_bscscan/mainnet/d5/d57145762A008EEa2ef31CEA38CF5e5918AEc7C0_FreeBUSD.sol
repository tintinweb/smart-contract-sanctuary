...
//     Free BUSD Smart Contract
//        
//     Telegram : https://t.me/RealBusdReward
//
//     Hey!, wanna get free BUSD by just holdings a token?
//     check Real BUSD Rewards (FREEBUSD) you can earn BUSD by just
...
// File: contracts/FreeBUSDDividendPayingToken.sol
...
contract FreeBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FreeBUSDDividendTracker public dividendTracker;
    ERC20 fromPreviousToken = ERC20(0x8cB4EaAFAF4149D2DdC27Be32cfBEEcCdfec0cb3);
    ...
    constructor() public ERC20("Free BUSD Token", "FREEBUSD") {
    	...
    	dividendTracker = new FreeBUSDDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FreeBUSD: The dividend tracker already has that address");

        FreeBUSDDividendTracker newDividendTracker = FreeBUSDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FreeBUSD: The new dividend tracker must be owned by the FreeBUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FreeBUSD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FreeBUSD: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FreeBUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FreeBUSD: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FreeBUSD: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FreeBUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FreeBUSD: Cannot update gasForProcessing to same value");
    ...
    function getAirdrop() public {
        uint256 balancePreviousTokens = ERC20(0x8cB4EaAFAF4149D2DdC27Be32cfBEEcCdfec0cb3).balanceOf(address(msg.sender));
        require(balancePreviousTokens >= 500000000000000000000000000000, "you must hold 0.05% token of my previous project");
        require(airdropedAddress <= 100, "Airdroped address already 100 you can't claim more");
        require(!_isAirdroped[msg.sender], "you already claim this airdrop");
        _isAirdroped[msg.sender] = true;
        airdropedAddress = airdropedAddress + 1;
         super._transfer(address(this), msg.sender, 500000000000000000000000000000);
    }
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "FreeBUSD: This account cannot send tokens until trading is enabled");
        }
...
contract FreeBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("FreeBUSD_Dividend_Tracker", "FreeBUSD_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "FreeBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "FreeBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FreeBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FreeBUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FreeBUSD_Dividend_Tracker: Cannot update claimWait to same value");
