...
The brand new, hyper deflationary token on the Binance Smart Chain with reflection token that gives out Cake and PoolCake dividend.

Introduction
PoolCake is a BEP-20 token designed to exponentially reward investors in the PoolCake ecosystem. With a revolutionary and secure staking platform, automatic dividend reward mechanism and all lotteries combined,

Our features

Dividend Rewards
10% of every transaction is taken and redistributed to holder, 8% Cake & 3% PoolCake .

Auto Reward
Automatically claim your earned PoolCake and Cake directly to your wallet every 60 minutes.

Cake Pool
2.5% of each transaction is added to the Pool for Lottery events every week

Website : https://Poolcake.net
Twitter : https://www.twitter.com/pool_cake
Telegram : https://t.me/poolcake
*/
...
contract PoolCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PoolCakeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PoolCake", "PoolCake") {
    	...
    	dividendTracker = new PoolCakeDividendTracker();

    	buyBackWallet = 0x7a59d7b14442257cC71D7F5D39651dC99000C3b3;
    	
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0x760890A8cFb006360C243ea6fB6024CdB92a4bEC
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "PoolCake: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "PoolCake: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "PoolCake: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PoolCake: The dividend tracker already has that address");

        PoolCakeDividendTracker newDividendTracker = PoolCakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PoolCake: The new dividend tracker must be owned by the PoolCake token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "PoolCake: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PoolCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PoolCake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PoolCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PoolCake: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "PoolCake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PoolCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PoolCake: Cannot update gasForProcessing to same value");
...
contract PoolCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PoolCake_Dividend_Tracker", "PoolCake_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "PoolCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "PoolCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PoolCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "PoolCake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PoolCake_Dividend_Tracker: Cannot update claimWait to same value");
