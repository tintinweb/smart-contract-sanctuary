...
contract SmartChef is Ownable {
    ...
    function pendingReward(uint256 _pid,address _user) public view returns (uint256) {
        ...
        if(pool.lastTimeReward==0){
            rew = ((user.amountUSD6*(block.timestamp - user.lastClaim)*user.dailyReward6)+(user.amountUSD12*(block.timestamp - user.lastClaim)*user.dailyReward12))/8640000;
        }else{
            rew = ((user.amountUSD6*(pool.lastTimeReward - user.lastClaim)*user.dailyReward6)+(user.amountUSD12*(pool.lastTimeReward - user.lastClaim)*user.dailyReward12))/8640000;
        }
        return rew*1000;
    }
