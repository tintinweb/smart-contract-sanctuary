    ...
    function executeTrade(uint tradeId) external {
        ...
        if (totalFee > 0) {
            ...
            if(contractTokenBalance >= swapTokensAtAmount){
                uint tokenFee = (contractTokenBalance * tradeFee) / (10000 * (tradeFee + wbnbReflectRewardsFee));

                if (tokenFee > 0) {
                    require(token.transferFrom(address(this), feeWallet, tokenFee), "Fee transfer failed.");
                }
