...
/**
 * @title FLOKIUSA price Oracle
 *      This Oracle calculates the average USD price of FLOKIUSA based on the BNB-FLOKIUSA and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private FLOKIUSABnbPrice0CumulativeLast;
    uint private FLOKIUSABnbPrice1CumulativeLast;
    uint32 private FLOKIUSABnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _FLOKIUSA_bnb;
    ...
    constructor(
        address __FLOKIUSA_bnb,   // Address of the FLOKIUSA-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _FLOKIUSA_bnb = IUniswapV2Pair(__FLOKIUSA_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        FLOKIUSABnbPrice0CumulativeLast = _FLOKIUSA_bnb.price0CumulativeLast();
        FLOKIUSABnbPrice1CumulativeLast = _FLOKIUSA_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, FLOKIUSABnbBlockTimestampLast) = _FLOKIUSA_bnb.getReserves();
    ...
    // Get the average price of 1 FLOKIUSA in the smallest BNB unit (18 decimals)
    function getFLOKIUSABnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_FLOKIUSA_bnb));

        require(_blockTimestamp != FLOKIUSABnbBlockTimestampLast, "FLOKIUSA Last and current are equal");

        FixedPoint.uq112x112 memory FLOKIUSABnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - FLOKIUSABnbPrice0CumulativeLast) / (_blockTimestamp - FLOKIUSABnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, FLOKIUSABnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint FLOKIUSABnbAverage;
        uint busdBnbAverage;

        (FLOKIUSABnbPrice0CumulativeLast, FLOKIUSABnbPrice1CumulativeLast, FLOKIUSABnbBlockTimestampLast, FLOKIUSABnbAverage) = getFLOKIUSABnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, FLOKIUSABnbAverage) = getFLOKIUSABnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = (FLOKIUSABnbAverage)*1e6 / busdBnbAverage;
