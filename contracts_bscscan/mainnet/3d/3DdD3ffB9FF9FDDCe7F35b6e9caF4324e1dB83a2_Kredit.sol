...
contract Kredit is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public marketingWallet = payable(0xf250dAC44c9CD1C9A29af939DAd6E0c65ae69375);
    address payable public investmentWallet = payable(0x8e60c16281723A096e6b4D9A712D8a39805444d4);


    bool private swapping;

    KRDTDividendTracker public dividendTracker;
    ...
    uint256 public investmentFee;
    uint256 public totalFees;

    // sells have fees of 13.2 and 2.2 (12 * 1.1 and 2 * 1.1)
    ...
    constructor() ERC20("Kredit", "KRDT") {
        uint256 _BNBRewardsFee = 4;
        uint256 _liquidityFee = 4;
        uint256 _marketingFee = 4;
        uint256 _investmentFee = 4;

        BNBRewardsFee = _BNBRewardsFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        investmentFee = _investmentFee;
        totalFees = _BNBRewardsFee.add(_liquidityFee).add(_marketingFee).add(_investmentFee);
        
    	dividendTracker = new KRDTDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "KRDT: The dividend tracker already has that address");

        KRDTDividendTracker newDividendTracker = KRDTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "KRDT: The new dividend tracker must be owned by the KRDT token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "KRDT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "KRDT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "KRDT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "KRDT: Automated market maker pair is already set to that value");
    ...
    function setFees(uint256 _liquidityFee, uint256 _investmentFee, uint256 _BNBRewardsFee, uint256 _marketingFee) public onlyOwner {
        liquidityFee = _liquidityFee;
        investmentFee = _investmentFee;
        BNBRewardsFee = _BNBRewardsFee;
        marketingFee = _marketingFee;
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "KRDT: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "KRDT: The marketing wallet is already this address");
        excludeFromFees(newMarketingWallet, true);
        emit marketingWalletUpdated(newMarketingWallet, marketingWallet);
        marketingWallet = payable(newMarketingWallet);
    }
    
    function updateInvestmentWallet(address newInvestmentWallet) public onlyOwner {
        require(newInvestmentWallet != investmentWallet, "KRDT: The investment wallet is already this address");
        excludeFromFees(newInvestmentWallet, true);
        emit investmentWalletUpdated(newInvestmentWallet, investmentWallet);
        investmentWallet = payable(newInvestmentWallet);
    }
    
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "KRDT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KRDT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "KRDT: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "KRDT: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "KRDT: The fixed-sale is only available to certain participants at the start");
            }
        ...
        if(
            ...
            uint256 marketFee = investmentFee.add(marketingFee);
    ...
    function swapAndSendToMarketing(uint256 tokens) private{
        uint256 initialBalance = address(this).balance;
        uint256 totFee = marketingFee.add(investmentFee);
        swapTokensForEth(tokens);
        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        marketingWallet.transfer(newBalance.div(totFee).mul(marketingFee)); 
        investmentWallet.transfer(newBalance.div(totFee).mul(investmentFee));
    }
...
contract KRDTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("KRDT_Dividend_Tracker", "KRDT_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "KRDT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "KRDT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KRDT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "KRDT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "KRDT_Dividend_Tracker: Cannot update claimWait to same value");
