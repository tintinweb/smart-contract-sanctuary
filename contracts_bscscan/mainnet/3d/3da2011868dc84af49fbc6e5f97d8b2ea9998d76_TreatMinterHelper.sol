         ...
         "MBRRP RMMBR'j.   '$$          .$$'                ,J
          ...
          `YMMBBR00RRBNMMN'         YXMBRRRRRRRBNMMNBP
           ...
           'NMBBR00BNNMNl            XMBRRRRRBNMMMNBS
            ...
            iMMBBR0BNMY               XBMRRRRBMMMNBS    my kind a treat
...
contract TreatMinterHelper is Ownable {
    using SafeMath for uint256;
    TreatMinterV2 public treatMinterV2;
    bool public pausedTreats;

    event TotwNftsCreated(uint256[] nftIds, uint256[] maxSupplys, address creatorAddress);

    constructor(TreatMinterV2 _TreatMinterV2Address) public {
        treatMinterV2 = _TreatMinterV2Address;
        pausedTreats = false;
    }
    
    function createTreats(uint256[] memory _maxSupplys, address _creatorAddress) public onlyOwner returns (uint256[] memory nftIds) {
        require(pausedTreats == false, "Contract Paused");
        uint256[] memory newNftIds = new uint256[](_maxSupplys.length);
        for(uint256 i = 0; i < _maxSupplys.length; i++) {
            newNftIds[i] = treatMinterV2.create(_maxSupplys[i],0,"","0x69",_creatorAddress);
        }
        emit TotwNftsCreated(newNftIds,_maxSupplys,_creatorAddress);
        return newNftIds;
    }
