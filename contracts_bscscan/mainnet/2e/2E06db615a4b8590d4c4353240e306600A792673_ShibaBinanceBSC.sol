...
/**
   #shibabinance
   tax: 10% (to shibabinance team wallet)
   
   https://t.me/shibabinancebsc
...
contract ShibaBinanceBSC is Context, IERC20, Ownable {
    ...
    uint256 private _tshibabinanceTotal;

    string private _name = "ShibaBinanceBSC";
    string private _symbol = "SHIBIN";
    ...
    uint256 public _shibabinanceFee = 10;
    uint256 private _previousshibabinanceFee = _shibabinanceFee;
    
    uint256 public _maxTxAmount = 1000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 1 * 10**6 * 10**9; 
    
    address payable public shibabinanceWallet = 0x00bc2aa1281BBBd3B9Af8A39b69eB39d0F6AE501; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tshibabinanceTotal = _tshibabinanceTotal.add(address(this).balance);
        TransferCharityETH(shibabinanceWallet, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_shibabinanceFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _shibabinanceFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousshibabinanceFee = _shibabinanceFee;
    ...
    function restoreAllFee() private {
        ...
        _shibabinanceFee = _previousshibabinanceFee;
    }
    ...
    function setshibabinanceFeePercent(uint256 shibabinanceFee) external onlyOwner() {
        _shibabinanceFee = shibabinanceFee;
    }
