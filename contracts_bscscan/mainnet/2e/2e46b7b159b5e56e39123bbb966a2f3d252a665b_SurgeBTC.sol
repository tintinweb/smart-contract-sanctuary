...
/**
 * Contract Created by DeFi Speculate
 ...
 * This is the new DeFi Standard moving forward
 */
...
/** 
 * Contract: BTC Surge Token
 * Developed By: DeFi Speculate
 * 
 ...
 * Next Gen DeFi Token - Pegged to the price of BTC
 * 
 */
contract SurgeBTC is IERC20, Context, Ownable {
    ...
    // Address for BTCB
    address public _btcb = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;
    // Address for Pancakeswap Router V2
    IUniswapV2Router02 router;
    // token data
    string constant _name = "SurgeBTC";
    string constant _symbol = "SBTC";
    ...
    uint256 public spreadDivisor = 95;
    ...
    function purchase(address buyer, uint256 bnbAmount) internal returns (bool) {
        ...
        // previous amount of BTCB before we received any
        uint256 prevBtcbAmount = IERC20(_btcb).balanceOf(address(this));
        // buy BTCB with the BNB we received
        buyBtcb(bnbAmount);
        // if this is the first purchase, use current balance
        uint256 currentBtcbAmount = IERC20(_btcb).balanceOf(address(this));
        // number of BTCB we have purchased
        uint256 difference = currentBtcbAmount.sub(prevBtcbAmount);
        // if this is the first purchase, use new amount
        prevBtcbAmount = prevBtcbAmount == 0 ? currentBtcbAmount : prevBtcbAmount;
        ...
        uint256 nShouldPurchase = calculatedTotalSupply.mul(difference).div(prevBtcbAmount);
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        // how much BTCB are these tokens worth?
        uint256 amountBTCB = tokensToSwap.mul(calculatePrice());
        
        // send BTCB to Seller
        bool successful = IERC20(_btcb).transfer(msg.sender, amountBTCB);
        if (successful) {
    ...
    /**
     * Buys BTCB with BNB in the contract, storing in the contract
     */ 
    function buyBtcb(uint256 amount) internal {
    ...
    function calculatePrice() public view returns (uint256) {
        uint256 btcbBalance = IERC20(_btcb).balanceOf(address(this));
        return btcbBalance.div(_totalSupply);
    }
    ...
    function getBtcbQuantityInContract() public view returns(uint256){
        return IERC20(_btcb).balanceOf(address(this));
    }
