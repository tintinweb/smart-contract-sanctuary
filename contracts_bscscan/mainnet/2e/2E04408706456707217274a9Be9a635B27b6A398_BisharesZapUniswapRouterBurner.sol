...
interface IZapUniswapV2 {
    ...
    ) external payable returns (uint256 vaultAmount);
    ...
    function zapInEstimate(
        address router,
        address vault,
        uint256 value
    ) external view returns (uint256 vaultAmount);

    function zapOutEstimate(
        address router,
        address vault,
        uint256 withdrawAmount,
        address desiredToken
    ) external view returns (uint256 desiredTokenAmount);
}
...
contract BisharesZapUniswapRouterBurner is BMath {
  ...
  function _burnForAllTokensAndSwap(
    ...
    require(minAmountsOut.length == vaults.length, "BisharesUniswapRouterBurner: BAD_ARRAY_LENGTH");
