...
contract FISTDAO is Context, IERC20, Ownable {
    ...
    uint256 private _previousDevFee = _DAOFee;
    
    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);
    address public ownerAddres = address(0xd00748Bd5FC62Fe8817D1f3e02c08a4f27Ffcc50);
    address public devAddress = address(0xd00748Bd5FC62Fe8817D1f3e02c08a4f27Ffcc50);
    ...
    uint256 public _maxTxAmount = 60000000000 * 10 **18;
    uint256 private numTokensSellToAddToLiquidity = 60000000 * 10 ** 18;
    ...
    function setDevFeePercent(uint256 DAOFee) external onlyOwner() {
        _DAOFee = DAOFee;
    }
    ...
    function calculateDevFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_DAOFee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _DAOFee ==0) return;

        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousBurnFee = _burnFee;
        _previousDevFee = _DAOFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        ...
        uint256 addHl = uint256(100).mul(_liquidityFee).div(_liquidityFee.add(_DAOFee));
        ...
        uint256 addusdtNumber = newBalance.div(7);
        addLiquidity(addNumber,addusdtNumber);
        emit SwapAndLiquify(addNumber, addusdtNumber, addNumber);
    }
