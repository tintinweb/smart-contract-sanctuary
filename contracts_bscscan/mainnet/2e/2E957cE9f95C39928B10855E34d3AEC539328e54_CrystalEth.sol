 ...
 * CrystalEth!
 * 
 * Featuring BUSD AND Eth rewards
 * 
 * 10% go to rewards
 * 3% to liquidity
 * 3% to marketing
...
contract CrystalEth is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("CrystalEth", "CRETH") {
    ...
    function updateethDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "CrystalEth: The dividend tracker already has that address");
 
        ETHDividendTracker newethDividendTracker = ETHDividendTracker(payable(newAddress));
 
        require(newethDividendTracker.owner() == address(this), "CrystalEth: The new dividend tracker must be owned by the CrystalEth token contract");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "CrystalEth: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "CrystalEth: The new dividend tracker must be owned by the CrystalEth token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CrystalEth: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "CrystalEth: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CrystalEth: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "CrystalEth: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "CrystalEth: Cannot update gasForProcessing to same value");
...
contract ETHDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("CrystalEth_Eth_Dividend_Tracker", "CrystalEth_Eth_Dividend_Tracker", 0x2170Ed0880ac9A755fd29B2688956BD959F933F8) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "CrystalEth_Eth_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "CrystalEth_Eth_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CrystalEth contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CrystalEth_Eth_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CrystalEth_Eth_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BUSDDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("CrystalEth_Busd_Dividend_Tracker", "DualRewards_Busd_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "CrystalEth_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "CrystalEth_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CrystalEth contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "CrystalEth_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CrystalEth_Busd_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
