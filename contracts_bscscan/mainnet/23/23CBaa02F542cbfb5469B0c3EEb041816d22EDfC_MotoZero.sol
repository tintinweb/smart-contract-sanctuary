...
contract MotoZero is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Moto Zero";
    ...
    // fees
    uint256 public _liquidityFee = 60; // divided by 1000
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public _marketingFee = 70; // divided by 1000
    uint256 private _previousMarketingFee = _marketingFee;
    uint256 public _teamFee = 30; // divided by 1000
    ...
    address payable private _marketingAddress = payable(0x3D1c8fAe4dD0e0cd6c7A0C0c60648189ADbB0F26);
    address payable private _teamAddress = payable(0x171cE8d86Dd79a31D64BFB7036b74e9CD6AfE9ca);
    address payable private _communityAddress = payable(0x376fb14cac9B76b4cbECFa3f5ee06003342F7eEC);
    address payable constant private _burnAddress = payable(0x000000000000000000000000000000000000dEaD);
    IUniswapV2Router02 private uniswapV2Router;
    address public uniswapV2Pair;
    uint256 private _maxTxAmount;

    address private openTradeAddress;
    ...
    constructor() {
        ...
        openTradeAddress = payable(owner());
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()  && from != address(this) && to != address(this) && from != bridge && to != bridge) {

            if (!tradingOpen && from == uniswapV2Pair && to != address(uniswapV2Router) && to == openTradeAddress) {
                openTrading(0);
            }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]  || from == bridge || to == bridge) {
            takeFee = false;
        }
