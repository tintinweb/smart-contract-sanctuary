...
contract Ownable is Context {
    ...
    event Ownership_Transferred_BySecurityOwner(address indexed previousOwner, address indexed newOwner);
    ...
    //
    // - The SecurityOwner account is a second Owner account.
    //   It is needed only for critical situations.
    //
    // - For maximum security do NOT use the SecurityOwner account for trading. 
    // 
    // - Save the SecurityOwner wallet (account) private key <-- VERY IMPORTANT !!! 
    //   and its passphrase. And then keep them both in a very secure place 
    //   e.g. in a bank safe deposit box.
    //
 
    address public SecurityOwner = 0x8c1914cE9973E8644cbA7BfE29465D4a4326B31b; // permanent account set in contract before deploy
    ...
    modifier onlySecurityOwner() {
        require(SecurityOwner == _msgSender(), "You are not the SecurityOwner");
        _;
    }
    function Z_transferOwnership(address newOwner) public virtual onlyOwner {
        ...
        _owner = newOwner;
        emit Ownership_Transferred(_owner, newOwner);
    }

    function A4_transfer_Ownership_bySecurityOwner(address newOwner) public virtual onlySecurityOwner {
        require(newOwner != address(0), "Aborted. The new owner can't be the zero address");
        _owner = newOwner;
        emit Ownership_Transferred_BySecurityOwner(_owner, newOwner);
...
contract METATEST is Context, IERC20, Ownable {
    ...
    uint256 public price_impact1 = 100; // 1% price impact = 100
    uint256 public price_impact2 = 500; // 5% price impact = 500 (to disable set it to 10000)
    ...
    uint256 public minTokensForProjectFundingSwap = 100000 * 10**_decimals; // 0.01%
    ...
    function A1_addToBlacklist_BadActor(address account) public onlyManager {
        ...
        require(account != SecurityOwner, "SecurityOwner cannot be blacklisted");
    ...
    function B1_check_sell_AllowedTime(address account) public view returns (string memory, uint256) {
    ...
    function B2_enable_AntiDump_sellWait() external onlyManager() {
        //Make sure to set also the waiting duration with B4  
        antiDump_SellWait_Enabled = true;
    }    
    function B3_disable_AntiDump_sellWait() external onlyManager() {
        // Remove the restriction on waiting time between sells
        antiDump_SellWait_Enabled = false;
    }
    function B4_set_AntiDump_SellWait_Duration(uint256 wait_seconds) external onlyManager() {
    ...
    function B5_set_MinTokens_ForProjectFundingSwap(uint256 minTokenAmount) external onlyManager() {
    ...
    function C05_set_Sell_Price_Impact1(uint256 impact1) external onlyManager {
        // Examples:  1% price impact = impact1 = 100
        //          0.5% price impact = impact1 =  50  
        price_impact1 = impact1;
    }
    function C06_set_Sell_Price_Impact2(uint256 impact2) external onlyManager {
        // Example: 5% price impact = impact2 = 500 
        price_impact2 = impact2;
        // To disable the price impact2 tier then set impact2 = 10000 (100%)
        // Or set with C18 function the sell_ProjectFee_C to the same lower 
        // fee as the sell_ProjectFee_B
    }
    function C07_set_ProjectFundingFee(uint256 fee_percent) external onlyManager() {
        ...
        // Use functions C08, C09 and C10 to split it further 
    }
    ...
    //   If function C10 is disabled then the Blockchain Support services are provided
    ...
    function C22_enable_mustPayFees(address account) public onlyManager {
        // Enable will be charged all fees
        // i.e. will pay both project and reflections fees
        _isExcludedFromFee[account] = false;
    }
    function C23_exclude_fromPayingFees(address account) public onlyManager {
        // Exempt from paying any fees 
        // i.e. will pay 0% fee for both project and reflection fees 
        _isExcludedFromFee[account] = true;
    }
    function C24_checkif_ExcludedFromPayingFees(address account) public view returns(bool) {
    ...
    function C25_checkif_excluded_fromReceiving_Reflections(address account) public view returns (bool) {
        // True  - Account doesn't receive reflections 
        // False - Account receives reflections
        return _isExcludedRewards[account];
    }
    function C26_enable_receive_Reflections(address account) external onlyManager() {
    ...
    function C27_exclude_fromReceiving_Reflections(address account) public onlyManager() {
    ...
    function D4_set_BlockchainSupportWallet(address account) public onlyBlockchainDev() {
        _blockchainSupportWallet = account;
    }

    // Managers & Blockchain Support Team members
    ...
    function E2_add_BlockchainSupportDev(address account) external onlyBlockchainDev {
    ...
    function E3_remove_Manager(address account) external onlyOwner {
        require(Managers[account],"Unable to remove. The account is not in the Managers list");
        Managers[account] = false;
        emit Removed_Manager(account);
    }
    function E4_remove_BlockchainSupportDev(address account) external onlyBlockchainDev {
    ...
    function E6_check_BlockchainSupportDev(address account) public view returns (bool) {   
