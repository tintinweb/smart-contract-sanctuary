...
contract MarketMoveToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0x696936D804E1CA5CE43eb8010cF186D1b281B1BE); // Marketing Address
    address payable public featurePaymentAddress =
        payable(0xa49474Ae72c660697301356fdB82b56b26E62c0b); // Address for paying for product features.
    address payable public constant airDropAddress =
        payable(0x24B498529e3b1EcDb5BD52D17Eb7DF6e435C516C); // AirDrop Address
    ...
    string private constant _name = "MarketMove";
    ...
    uint256 public _sellLiquidityFee = 14;
    ...
    uint256 public marketingDivisor = 80; // = 4*2 because we halve the amount, one half to Swap&Liquify, another to Marketing. *10 because we divide first by 10!
    ...
    bool public constant _isEnabledBuyBackAndBurn = true;

    uint256 private _preSaleEndTime;
    bool private _preSaleFeesEnded = false;
    uint256 public constant _intervalMinutesForEndPreSaleRate = 1 * 30 seconds;
    ...
    constructor() {
        ...
        _isExcludedFromFee[featurePaymentAddress] = true;
    ...
    function _transfer(
                    ...
                    if (_isAutoBuyBack) {
                        ...
                        for (uint256 i = 0; i < _sellHistories.length; i++) {
                            if (_sellHistories[i].time >= startTime) {
                                sumBnbAmount = sumBnbAmount.add(
                                    _sellHistories[i].bnbAmount
                                );
                                cnt = cnt + 1;
                            }
                        }
                        ...
                        if (cnt > 0 && _buyBackDivisor > 0) {
                            _bBSLimitMax = sumBnbAmount.div(cnt).div(
                                _buyBackDivisor
                            );
                        }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (!_preSaleFeesEnded && _preSaleEndTime > 0) {
                if (
                    _preSaleEndTime + _intervalMinutesForEndPreSaleRate <=
                    block.timestamp
                ) {
                    _preSaleFeesEnded = true;
                    removeAllFee();
                    _taxFee = 2;
                    _liquidityFee = 10;
                } else {
    ...
    function setFeaturePaymentAddress(address _featurePaymentAddress)
        external
        onlyOwner
    {
        featurePaymentAddress = payable(_featurePaymentAddress);
        _isExcludedFromFee[featurePaymentAddress] = true;
    }
