...
/**
 DogeThug Smart Contract

Telegram:     https://t.me/DogeThugBSC

Earn BUSD while holding DogeThug!
DogeThug is all about passive income :D

10% BUSD reflection
5% liquidity rereflection

Supply: 100,000,000,000
Max wallet size of 1.5% (1,500,000,000)
Max transaction size of 0.3% of supply (300,000,000)

LP will be locked for one month, and can be extended
...
// File: contracts/DogeThugDividendPayingToken.sol
...
contract DogeThug is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DogeThugDividendTracker public dividendTracker;
    ...
    uint256 public immutable tradingEnabledTimestamp = 1625674972;
    ...
    function _isTeam(address addr) private view returns (bool){
        return addr==owner()||addr==liquidityWallet;
    }

    constructor() public ERC20("DogeThug", "DogeThug") {
    	...
    	dividendTracker = new DogeThugDividendTracker();
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) public onlyTeam {
    ...
    function updateDividendTracker(address newAddress) public onlyTeam {
        require(newAddress != address(dividendTracker), "DogeThug: The dividend tracker already has that address");

        DogeThugDividendTracker newDividendTracker = DogeThugDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DogeThug: The new dividend tracker must be owned by the DogeThug token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyTeam {
        require(newAddress != address(uniswapV2Router), "DogeThug: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyTeam {
        require(_isExcludedFromFees[account] != excluded, "DogeThug: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }

    function excludeMultipleAccountsFromFees(address[] calldata accounts, bool excluded) public onlyTeam {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyTeam {
        require(pair != uniswapV2Pair, "DogeThug: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DogeThug: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyTeam {
        require(newLiquidityWallet != liquidityWallet, "DogeThug: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyTeam {
        require(newValue >= 200000 && newValue <= 500000, "DogeThug: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DogeThug: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateClaimWait(uint256 claimWait) external onlyTeam {
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DogeThug: This account cannot send tokens until trading is enabled");
        }
...
contract DogeThugDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DogeThug_Dividend_Tracker", "DogeThug_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DogeThug_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DogeThug_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogeThug contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DogeThug_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DogeThug_Dividend_Tracker: Cannot update claimWait to same value");
