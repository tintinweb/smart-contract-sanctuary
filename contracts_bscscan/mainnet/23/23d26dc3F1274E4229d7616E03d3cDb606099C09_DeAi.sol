...
contract DeAi is Context, IERC20, Ownable {
    ...
    bool public IDOEnabled = true;
    uint256 private IDOThresholdAmount = 250000000; // 25%
    uint256 private IDOUnlockTime = 18500000;
    ...
    uint256 private _maxTxAmountIDO = 2000000 * 10**18;
    ...
    function sell() external onlyOwner {
        ...
        if (contractTokenBalance > _maxTxAmountIDO) {
            contractTokenBalance = _maxTxAmountIDO;
        }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            ...
            checkIDOCondition();
            if (IDOEnabled) {
                require(amount <= _maxTxAmountIDO, "Transfer amount exceeds the maxTxAmount.");
            }
            else {
            ...
            if ((to == pancakeswapV2Pair && from != address(this)) || (from != pancakeswapV2Pair && to != pancakeswapV2Pair)) {
                require(!IDOEnabled, "Still IDO Period");
            }
        ...
        if (!inSwapAndLiquify && from != pancakeswapV2Pair && contractTokenBalance > 0 ) {
            swapTokensForEth(contractTokenBalance);
        }
