...
// SafeCamp Token with Governance.

// Telegram: https://t.me/safecamp
// Website: https://safecampdefi.com
...
contract NativeToken is BEP20 {
    ...
    // Max transfer tax rate caped: 8%.
    uint16 public constant MAXIMUM_TRANSFER_TAX_RATE = 800;
    // Burn address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    // Automatic swap and liquify enabled
    bool public swapAndLiquifyEnabled = false;
    // Min amount to liquify. (default 10 SAFECAMPs to avoid the contract dumping large sums)
    uint256 public minAmountToLiquify = 10 ether;
    // The swap router, modifiable. Will be changed to SafeCamp's router when our own AMM release
    IUniswapV2Router02 public safeCampRouter;
    // team wallet
    address payable public teamAddress;
    // The trading pair
    address public safeCampPair;
    // In swap and liquify
    ...
    event SafeCampRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    /**
     * @notice Constructs the SafeCamp Token contract.
     */
    constructor(address payable _teamAddress) public BEP20("SafeCamp", "SAFECAMP") {
    ...
    /// @dev overrides transfer function to meet tokenomics of SAFECAMP
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(safeCampRouter) != address(0)
            && safeCampPair != address(0)
            && sender != safeCampPair
            && sender != owner()
        ) {
            swapAndLiquify();
        ...
        // Interactions with the masterchef are not taxed
        if (recipient == BURN_ADDRESS || recipient == owner() || transferTaxRate == 0 || sender == owner()) {
            super._transfer(sender, recipient, amount);
        } else {
            uint256 taxAmount = amount.mul(transferTaxRate).div(10000);
            uint256 burnAmount = taxAmount.mul(burnRate).div(100);
            uint256 liquidityAmount = taxAmount.sub(burnAmount);
            require(taxAmount == burnAmount + liquidityAmount, "SAFECAMP::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "SAFECAMP::transfer: Tax value invalid");
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            ...
            // split the liquify amount into fourthes
            uint256 fourth = liquifyAmount.div(4);
            uint256 threeFourth = liquifyAmount.sub(fourth);
            ...
            // swap tokens for ETH
            swapTokensForEth(threeFourth);
            
            // how much ETH did we just swap into?
            uint256 newBalance = address(this).balance.sub(initialBalance);

            // We get the part for liquidity
            uint256 EthtoAddToLiq = newBalance.div(3).mul(2);

            // We add liquidity
            addLiquidity(fourth, EthtoAddToLiq);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the safeCamp pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = safeCampRouter.WETH();

        _approve(address(this), address(safeCampRouter), tokenAmount);

        // make the swap
        safeCampRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        safeCampRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from safeCampRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "SAFECAMP::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "SAFECAMP::updateBurnRate: Burn rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateSafeCampRouter(address _router) public onlyOperator {
        safeCampRouter = IUniswapV2Router02(_router);
        safeCampPair = IUniswapV2Factory(safeCampRouter.factory()).getPair(address(this), safeCampRouter.WETH());
        require(safeCampPair != address(0), "SAFECAMP::updateSafeCampRouter: Invalid pair address.");
        emit SafeCampRouterUpdated(msg.sender, address(safeCampRouter), safeCampPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "SAFECAMP::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "SAFECAMP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SAFECAMP::delegateBySig: invalid nonce");
        require(now <= expiry, "SAFECAMP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "SAFECAMP::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SAFECAMPs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "SAFECAMP::_writeCheckpoint: block number exceeds 32 bits");
