...
//https://t.me/MiniBabyBUSD

// 13% BUSD reflection, 5% LP, 2% marketing
...
// File: contracts/minibabyBUSDDividendPayingToken.sol
...
// File: contracts/minibabyBUSD.sol
...
contract minibabyBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address private _marketingWallet = address(0xD5f5Db324c335B3cF9d7cC222407533409ddD43A);
    address private _devWallet = address(0xA9171eD312CbC8c878c9d66a340cFf16021E07A7);

    bool private swapping;
    bool public airdrop;

    minibabyBUSDDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet;
    address public devWallet = _devWallet;

    uint256 public maxBuyTranscationAmount = 100000000 * (10**18); // No limit
    uint256 public maxSellTransactionAmount = 100000000 * (10**18); // No limit
    uint256 public swapTokensAtAmount = 20000 * (10**18);
    uint256 public _maxWalletToken = 100000000 * (10**18); // No limit
    ...
    constructor() public ERC20("minibabyBUSD", "minibabyBUSD") {
    	...
    	dividendTracker = new minibabyBUSDDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "minibabyBUSD: The dividend tracker already has that address");

        minibabyBUSDDividendTracker newDividendTracker = minibabyBUSDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "minibabyBUSD: The new dividend tracker must be owned by the minibabyBUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "minibabyBUSD: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "minibabyBUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "minibabyBUSD: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "minibabyBUSD: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "minibabyBUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "minibabyBUSD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "minibabyBUSD: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            uint256 marketingTokens = contractTokenBalance.mul(9).div(totalFees);
            swapTokensForBNB(marketingTokens);
            
            uint256 devTokens = contractTokenBalance.mul(4).div(totalFees);
            swapTokensForBNB2(devTokens);
            
            uint256 swapTokens = contractTokenBalance.mul(4).div(totalFees);
...
contract minibabyBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("minibabyBUSD_Dividend_Tracker", "minibabyBUSD_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 1+ tokens
    }



    function _transfer(address, address, uint256) internal override {
        require(false, "minibabyBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "minibabyBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main minibabyBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "minibabyBUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "minibabyBUSD_Dividend_Tracker: Cannot update claimWait to same value");
