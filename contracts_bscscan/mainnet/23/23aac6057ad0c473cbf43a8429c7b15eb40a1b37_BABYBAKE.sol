...
FAIR LAUNCH FRIDAY 23rd JULY 
6:00 PM UTC⏰

WEBSITE : www.babybake.com 

TWITTER: twitter.com/RealBabyBake

5 member team.
Over 15k$  invested prior to launch.

TOKENOMICS

100B supply

5% team wallet locked for 1 year

15% Tax :
7% Reflection in BAKE 
...
contract BABYBAKE is Context, IERC20, Ownable {
    ...
    uint256 private _previousTaxFee = _taxFee; //7% Reflection in BAKE 
    
    uint256 public _burnFee = 2; //2% to Marketing/Buyback
    uint256 private _previousBurnFee = _burnFee;
    
    uint256 public _liquidFee = 6;
    uint256 private _previousLiquidFee = _liquidFee; //6% Added to liquidity
    ...
    private 
    liquidAddress 
    = 0x0fCB36330D79Be306e745737A82EcCfbcf64d7c6; //5% team wallet locked for 1 year
    ...
    function blacklistAddress(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function excludeFromFees(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,,,) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function setMarketingFees(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
