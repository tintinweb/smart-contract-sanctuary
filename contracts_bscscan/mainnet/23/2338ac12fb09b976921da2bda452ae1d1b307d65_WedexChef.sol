...
contract WedexChef is Ownable, ReentrancyGuard {
    ...
    function noLoop(address _user, address _referrer) internal returns(bool){
        if(_referrer==address(0))
            return true;
        if(address(wedexReferral)!=address(0)){
            address upper = wedexReferral.getReferrer(_referrer);
            if(upper==_user)
            {
                return false;
            }
        return noLoop(_user,upper);
        }
        return false;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(wedexReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender && noLoop(msg.sender, _referrer)) {
