...
contract SousChef is Ownable {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many CHERRY tokens the user has provided.
        ...
        //
        // Whenever a user deposits or withdraws CHERRY tokens to a pool. Here's what happens:
    ...
    // The CHERRY TOKEN!
    IBEP20 public cherry;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes CHERRY tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = cherry.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 cherrySupply = cherry.balanceOf(address(this));
        if (cherrySupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(cherrySupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit CHERRY tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        cherry.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw CHERRY tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        cherry.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        cherry.safeTransfer(address(msg.sender), user.amount);
