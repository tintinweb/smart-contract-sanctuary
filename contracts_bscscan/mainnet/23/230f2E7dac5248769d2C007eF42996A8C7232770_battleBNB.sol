...
contract battleBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    battleBNBDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0xcBcE9204B6D174B7Ba52D01943026d00810e980D);
    address payable public operations2Address =
        payable(0xcb733a4C249455455f2470523F2c313d5B149D18);
    ...
    constructor() ERC20("battleBNB", "BBNB") {
        dividendTracker = new battleBNBDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        battleBNBDividendTracker newDividendTracker = battleBNBDividendTracker(
            payable(newAddress)
        );
...
contract battleBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "battleBNB_Dividend_Tracker",
            "battleBNB_Dividend_Tracker"
        )
    {
        dbattleBNB = battleBNB(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DragonBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DragonBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DragonBNB contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(dbattleBNB);

            uint256 prevBalance = dbattleBNB.balanceOf(address(this));
            ...
            {
                uint256 received = dbattleBNB.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    dbattleBNB.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
