// SPDX-License-Identifier: IMM-Global
...
contract ImmFarm is Ownable {
	...
	function harvest(uint256 _pid) public verifyPoolId(_pid) returns (uint256) {
		updatePool(_pid);
	...
	function removeOtherToken(address _tokenAddress, address _to) external onlyOwner {
		for (uint256 pid = 0; pid < poolInfo.length; ++pid) {
			// we don't allow remove LPToken
			require(_tokenAddress != address(poolInfo[pid].lpToken), "Should not remove LP token");
		}
