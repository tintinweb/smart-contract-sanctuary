...
contract Coinflect is Context, ERC20, Ownable {
    ...
    modifier whiteListed() {
        require(minters[msg.sender] || msg.sender == owner(), "!minter");
        _;
    }

    constructor() ERC20("Coinflectx", "CFLTx") {
    ...
    function setMinter(address _minter, bool _flag) external onlyOwner {
        ...
        isExcludedFromFee[_minter] = _flag;
    }
    ...
    function burn(uint256 _amount) external whiteListed {
        uint256 before = balanceOf(address(this));
        IERC20(address(this)).safeTransferFrom(msg.sender, address(this), _amount);
        _burn(address(this), balanceOf(address(this)).sub(before));
    }
