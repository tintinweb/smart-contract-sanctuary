...
// File: contracts/CornToken.sol

pragma solidity 0.6.12;

// CornToken with Governance.
contract CornToken is BEP20 {
    ...
    // Min amount to liquify. (default 10 CORNs)
    uint256 public minAmountToLiquify = 10 ether;
    // The swap router, modifiable. Will be changed to Corn's router when our own AMM release
    IPancakeRouter02 public cornRouter;
    // The trading pair
    address public cornPair;
    // In swap and liquify
    ...
    event CornRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "CORN::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(swapEnabled == true, "CORN::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the CornToken contract.
     */
    constructor() public BEP20("CornDeFi Token", "CORN") {
    ...
    /// @dev overrides transfer function to meet tokenomics of CORN
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(cornRouter) != address(0)
            && cornPair != address(0)
            && sender != cornPair
            && sender != owner()
        ) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "CORN::transfer: Burn value invalid");

            // default 100% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "CORN::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the corn pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = cornRouter.WETH();

        _approve(address(this), address(cornRouter), tokenAmount);

        // make the swap
        cornRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        cornRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from cornRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "CORN::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "CORN::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "CORN::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateCornRouter(address _router) public onlyOperator {
        cornRouter = IPancakeRouter02(_router);
        cornPair = IPancakeV2Factory(cornRouter.factory()).getPair(address(this), cornRouter.WETH());
        require(cornPair != address(0), "CORN::updateCornRouter: Invalid pair address.");
        emit CornRouterUpdated(msg.sender, address(cornRouter), cornPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "CORN::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CORN::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CORN::delegateBySig: invalid nonce");
        require(now <= expiry, "CORN::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CORN::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CORNs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CORN::_writeCheckpoint: block number exceeds 32 bits");
...
// CornDeFi is the master of Corn Token (CORN). He can make CORN and he is a fair guy.
//
...
// contract once $CORN is sufficiently distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract CornDeFi is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accCornPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accCornPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Initial emission rate: 1 CORN per block.
    ...
    constructor(
        ...
        cornPerBlock = INITIAL_EMISSION_RATE;
    ...
    // Update the given pool's CORN allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function pendingCorn(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cornReward = multiplier.mul(cornPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accCornPerShare = accCornPerShare.add(
                cornReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accCornPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cornReward =
            multiplier.mul(cornPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
        corn.mint(devAddr, cornReward.div(10));
        corn.mint(address(this), cornReward);
        pool.accCornPerShare = pool.accCornPerShare.add(
            cornReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to CornDeFi for CORN allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);		
		payOrLockupPendingCorn(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(corn)) {
                uint256 transferTax = _amount.mul(corn.transferTaxRate()).div(10000);
    ...
    // Deposit LP tokens to CornDeFi for CORN allocation with referral.
    ...
    // Withdraw LP tokens from CornDeFi.
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
	    ...
	    // Pay or lockup pending CORNs.
    function payOrLockupPendingCorn(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeCornTransfer(msg.sender, totalRewards);
    ...
    function safeCornTransfer(address _to, uint256 _amount) internal {
        uint256 cornBal = corn.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > cornBal) {
            transferSuccess = corn.transfer(_to, cornBal);
        } else {
            transferSuccess = corn.transfer(_to, _amount);
        }
        require(transferSuccess, "safeCornTransfer: transfer failed.");
    }
    ...
    // updateEmissionRate
    function updateEmissionRate(uint256 _cornPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, cornPerBlock, _cornPerBlock);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            corn.mint(referrer, refBonusEarned);
