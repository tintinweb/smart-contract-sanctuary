...
function swapBNBForCake(uint256 bnbAmount) private {
    ...
    // function testSwapBNBForCake(uint256 bnbAmount) public {
    //     WETH.deposit{value: bnbAmount}();
    //     address[] memory path = new address[](2);
    ...
    function manualDeposit(uint256 tokenAmount) public {
        MRCVault(MRCVaultAddress).deposit(tokenAmount);
    }
    ...
    function emergencyWithdraw() public {
        payable(msg.sender).transfer(address(this).balance); // Need this for testing
    }
    ...
    constructor(address _CAKE) {
        ...
        address CAKEAddress = _CAKE;
        require(WETH.approve(address(uniswapV2Router), type(uint256).max), 'approve failed.');
        require(IERC20(CAKEAddress).approve(MRCVaultAddress, type(uint256).max), 'approve failed.');
    }
}
