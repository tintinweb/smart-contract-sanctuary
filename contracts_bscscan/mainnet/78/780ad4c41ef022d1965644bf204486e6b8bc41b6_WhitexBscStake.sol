...
contract WhitexBscStake is Owned {
...
_;
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(lead).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient  balance in pool');
...
function rewardPool() external view onlyOwner() returns(uint claimable) {
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        require((IERC20(lead).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient  balance in pool');
