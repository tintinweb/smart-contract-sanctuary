...
contract Strategy_ZOO_BUSD {
    // This strategy is for BSC network. ZOO - BUSD pair. RewardToken = "CAKE"
    ...
    address public token0; // ZOO
    address public token1; // BUSD
    address public vault;

    // ------------> Important constants <--------------
    uint256 public constant pid = 469;
    bool public constant hasPid = true;
    address public router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public constant lpToken = 0xd5D00B0aD40FF6d8C1a6F7e72F185B6fB3c3fd1F;
    ...
    function _takeFee() internal {
        ...
        if(rewardsFeeInWBNB > 0) {
            uint256 amount = _getAmountsOut(router, rewardsFeeInWBNB+pendingFee, path);
            if(amount > 100) {
    ...
    function _addLiquidityFromAllTokens() internal {
        _swapWBNBToToken(router, token1);
        _swapToken1ToToken0();
    ...
    function _swapExactETHForBUSD(uint256 _amountETH) internal {
        ...
        if(_amountETH > 0){
            ...
            path[0] = WBNB;
            path[1] = token1;
            uint256 amount = _getAmountsOut(router, _amountETH, path);
            if(amount > 100){
    ...
    function _getPriceWBNBToBUSD() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = token1;
        return 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
    ...
    function _getPriceWBNBToZOO() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = token0;
        return 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            if(t1V != 0) {
                path[0] = token1;
                path[1] = WBNB;
                TC = _getAmountsOut(router, t1V, path);
            }
            if(t0V != 0) {
                path[0] = token0;
                path[1] = WBNB;
