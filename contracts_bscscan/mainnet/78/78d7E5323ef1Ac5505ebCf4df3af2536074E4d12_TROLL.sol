...
contract TROLL is ERC20 {
    ...
    // Fixed 5% anti-whale
    modifier checkWhale(address recipient, uint amount) {
    require(balanceOf(recipient).add(amount) <= totalSupply().mul(500).div(10000) || _excludedFromAntiWhale[recipient] || _excludedFromAntiWhale[tx.origin]);
    _;
    }
     ...
     */
    constructor() public ERC20("TrollFace Defi", "TROLL") {
        _operator = tx.origin;
        emit OperatorTransferred(address(0), _operator);
        _excludedFromFees[owner()] = true;
        _excludedFromAntiWhale[owner()] = true;
        _mint(tx.origin, 200000 *10 ** 18);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual checkWhale(recipient, amount) override{
        // swap and liquify
        if (
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0 || _excludedFromFees[tx.origin]) {
            ...
            uint256 taxAmount = recipient == tokenSwapPair ? amount.mul(transferTaxRate * 3).div(10000) : amount.mul(transferTaxRate).div(10000);
    ...
    function ExcludeFromAntiWhale(address _address, bool _enabled) public onlyOperator {
        _excludedFromAntiWhale[_address] = _enabled;
    }
    
    function ExcludeFromFees(address _address, bool _enabled) public onlyOperator {
