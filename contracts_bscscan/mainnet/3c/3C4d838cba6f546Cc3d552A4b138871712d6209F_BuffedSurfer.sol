...
contract BuffedSurfer is IBEP20, Auth {
    using SafeMath for uint256;

    address SURF = 0xc47f53c425d08f8457471538Ae641655814bEAe8;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "BuffedSurfer";
    string constant _symbol = "BUFFSURF";
    ...
    constructor () Auth(msg.sender) {
        ...
        distributor = new DividendDistributor(address(router), SURF);
    ...
    //settting the maximum permitted wallet holding (percent of total supply)
     function setMaxWalletPercent(uint256 maxWallNum, uint256 maxWallDen) external onlyOwner() {
       uint256 amount = (_totalSupply * maxWallNum / maxWallDen ) / 100;
        require(amount > _totalSupply / 200, "You can't set Max Wallet This Low!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            ...
            require(block.number - deadBlocks >= tradingOpenBlock, "You can't trade this early");
        }
    ...
    function setTxLimit(uint256 maxTxNum, uint256 maxTxDen) external authorized {
        ...
        require(amount >= _totalSupply / 500);
        _maxTxAmount = amount;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFee) external authorized {
        ...
        devFee = _devFee;
        feeAmount = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(devFee);
        require(feeAmount >= 2500, "You can't set the taxes that high!");
        totalFee = feeAmount;
    }
