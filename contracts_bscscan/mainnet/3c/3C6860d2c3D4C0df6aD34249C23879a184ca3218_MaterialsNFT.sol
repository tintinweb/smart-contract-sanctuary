    ...
    OwnableUpgradeable
{
    ...
    function calculateInitialWolfPackLife(uint256 _wolfPackId) public view returns (uint256) {
        uint256 initialLife;
        for(uint256 i = 0; i < wolfsInWolfPack[_wolfPackId].length; i++) {
            initialLife = initialLife + WolfsNFT(WolfsNFTContractAddress).getWolfProperties(wolfsInWolfPack[_wolfPackId][i])[3] +
            WolfsNFT(WolfsNFTContractAddress).getWolfProperties(wolfsInWolfPack[_wolfPackId][i])[4];
        }
        return initialLife;
    }
        ...
        returns (bool)
    {
        ...
        uint256 limitTimestamp = DateTimeLibrary.addDays(wolfPackLinkDate[_wolfPackId], wolfPackLinkDays[_wolfPackId]);
        require(block.timestamp >= limitTimestamp || wolfPackLinkDays[_wolfPackId] == 0, "Bond is greater than 0");
        
        
        uint256 initialLife = calculateInitialWolfPackLife(_wolfPackId);
        require(wolfPackLife[_wolfPackId] >= initialLife, "Life is less than initial");
    ...
    function activateDeactivatePromo() external onlyOwner returns (bool) {
        ...
        dateLastPromoActivation = dateLastPromoActivation == 0 ? block.timestamp : 0;
        return true;
    }
    ...
    function changeLinkAndEnergyCommission(uint256 _newLinkCommission, uint256 _newEnergyCommission)
        external
        onlyOwner
        returns (bool)
    {
    ...
    OwnableUpgradeable
{
    ...
    event ClaimReward(
        ...
        uint256 indexed amountClaimedCWOLF,
        uint256 indexed amountClaimedDOllars,
