...
// File contracts/interfaces/IBrewFarm.sol

interface IBrewFarm {
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);

    function enterStaking(uint256 _wantAmt) external;

    function leaveStaking(uint256 _wantAmt) external;
...
// File contracts/BrewVault.sol

contract BrewVault is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable token; // Brew token
    IBrewFarm public immutable masterchef;
    ...
    uint256 public buyBackRate = 3000; // 30%
    ...
    /**
     * @notice Constructor
     * @param _token: Brew token contract
    ...
    /**
     * @notice Deposits funds into the Brew Vault
     * @param _amount: number of tokens to deposit (in BREW)
     */
    function deposit(uint256 _amount) external nonReentrant {
        require(_amount > 0, "BrewVault: Nothing to deposit");
    ...
    /**
     * @notice Reinvests BREW tokens into MasterChef
     */
    function harvest() external {
        uint256 balance = available();
        
        masterchef.leaveStaking(0);
        _buyback(balance, available());
    ...
    function setTreasury(address _treasury) external onlyOwner {
        require(_treasury != address(0), "BrewVault: Cannot be zero address");
    ...
    function setWithdrawFee(uint256 _withdrawFee) external onlyOwner {
        require(
            ...
            "BrewVault: withdrawFee cannot be more than MAX_WITHDRAW_FEE"
        );
     ...
     * @return Returns total pending Brew rewards
     */
    function calculateTotalPendingBrewRewards() external view returns (uint256) {
        uint256 amount = masterchef.pendingBREW(0, address(this));
    ...
    /**
     * @notice Withdraws from funds from the Brew Vault
    ...
    function withdraw(uint256 _shares) public nonReentrant {
        ...
        require(
            _shares > 0,
            "BrewVault: Nothing to withdraw"
        );
        require(
            _shares <= user.shares,
            "BrewVault: Withdraw amount exceeds balance"
        );
        ...
        if (bal < currentAmount) {
            ...
            masterchef.leaveStaking(balWithdraw);
            ...
            if(available() > currentAmount){
                _buyback(currentAmount, available());
            }
        }
    ...
    function _earn() internal {
        ...
        if (balance > 0) {
            ...
            _buyback(0, available());
        }
    }

    function _buyback(uint256 _prev, uint256 _after) internal{
        if(buyBackRate > 0){
            uint256 diff = _after.sub(_prev);
            if(diff > 0){
                token.safeTransfer(burnerAddress, diff.mul(buyBackRate).div(10000));
            }
        }
    }
