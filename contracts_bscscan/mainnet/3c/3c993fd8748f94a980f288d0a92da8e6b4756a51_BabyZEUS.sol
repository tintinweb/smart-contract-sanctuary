...
contract BabyZEUS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BABYZEUSDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address private immutable BNB = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); //BNB
    ...
    address private _marketingWalletAddress = address(0x4C4807e42d7529ef06EFBA471a454fB0193D7e8a); // marketing wallet
    address private _devWalletAddress = address(0x5DcF4f2f012089D15D223B0d47aC49422133f8c4); // fee for contract development
    ...
    constructor() ERC20("BabyZEUS", "BZEUS") {

    	dividendTracker = new BABYZEUSDividendTracker();
    ...
    uint256 public maxBuy = 10_000_000 * (10**18);
    uint256 public maxSell = 10_000_000 * (10**18);
    uint256 public maxWallet = 10_000_000 * (10**18);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BABYZEUS: The dividend tracker already has that address");

        BABYZEUSDividendTracker newDividendTracker = BABYZEUSDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BABYZEUS: The new dividend tracker must be owned by the BABYZEUS token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BABYZEUS: The router already has that address");
    ...
    function setBNBRewardsFee(uint256 value) external onlyOwner{
        ...
        BNBRewardsFee = value;
        require(BNBRewardsFee.add(liquidityFee).add(marketingFee) <= 15, "May not set total fees above 15%");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BABYZEUS: The PanBNBSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BABYZEUS: Automated market maker pair is already set to that value");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "BABYZEUS: airdropToWallets:: Arrays must be the same length");
        require(airdropWallets.length < 40, "BABYZEUS: airdropToWallets:: Cannot airdrop more than 40 wallets due to gas limit restrictions");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BABYZEUS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BABYZEUS: Cannot update gasForProcessing to same value");
...
contract BABYZEUSDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BABYZEUS_Dividend_Tracker", "BABYZEUS_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BABYZEUS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BABYZEUS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BABYZEUS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "BABYZEUS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BABYZEUS_Dividend_Tracker: Cannot update claimWait to same value");
