...
contract RewardToken is ERC20, Ownable {
    ...
    constructor() ERC20("MiniShit", "MSHIT") {
    	...
    	devWallet = 0x6BdC3Ae5b36BB2FD62F47bFd36492FBc93ae23Ac;
    	adaDividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        trxDividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "MiniShit: The marketing wallet is already this address");
    ...
    function updateadaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(adaDividendTracker), "MiniShit: The dividend tracker already has that address");
 
        ADADividendTracker newadaDividendTracker = ADADividendTracker(payable(newAddress));
 
        require(newadaDividendTracker.owner() == address(this), "MiniShit: The new dividend tracker must be owned by the MiniShit token contract");
    ...
    function updatetrxDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(trxDividendTracker), "MiniShit: The dividend tracker already has that address");
 
        TRXDividendTracker newtrxDividendTracker = TRXDividendTracker(payable(newAddress));
 
        require(newtrxDividendTracker.owner() == address(this), "MiniShit: The new dividend tracker must be owned by the MiniShit token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MiniShit: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "MiniShit: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MiniShit: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "MiniShit: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "MiniShit: Cannot update gasForProcessing to same value");
...
contract ADADividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("MiniShit_Ada_Dividend_Tracker", "MiniShit_Ada_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "MiniShit_Ada_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "MiniShit_Ada_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MiniShit contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "MiniShit_Ada_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MiniShit_Ada_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TRXDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("MiniShit_Trx_Dividend_Tracker", "MiniShit_Trx_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "MiniShit_Trx_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "MiniShit_Trx_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MiniShit contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "MiniShit_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MiniShit_Trx_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
