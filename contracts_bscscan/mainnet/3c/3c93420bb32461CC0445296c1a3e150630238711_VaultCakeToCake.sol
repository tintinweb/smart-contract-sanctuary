...
contract VaultCakeToCake is VaultController, IStrategy {
    ...
    uint256 public harvestedAt;     // Latest harvest timestamp 
    /* ========== INITIALIZER ========== */

    function initialize(address _circle, address _minter) external initializer {
        __VaultController_init(CAKE, _circle);
...
interface IStrategy is IVaultController {
    ...
    event CirclePaid(address indexed user, uint256 profit, uint256 performanceFee);
...
abstract contract VaultController is IVaultController, OwnerPausableUpgradeable, WhitelistUpgradeable {
    using SafeBEP20 for IBEP20;

    /* ========== CONSTANT VARIABLES ========== */
    BEP20 private CIRCLE;

    /* ========== STATE VARIABLES ========== */

    address public keeper;
    IBEP20 internal _stakingToken;
    ICircleMinter internal _minter;
    ...
    function __VaultController_init(IBEP20 token, address _circle) internal initializer {
        __OwnerPausable_init();
        __WhitelistUpgradeable_init();

        keeper = 0x1b0C88A7D1DC2B7bc4A892408D144761425F380D;
        _stakingToken = token;
        CIRCLE = BEP20(_circle);
    }
    ...
    function setMinter(address newMinter) virtual public onlyOwner {
        // can zero
        _minter = ICircleMinter(newMinter);
...
library PoolConstant {
    ...
    enum PoolTypes {
        CirclePool, CircleStake, CakeStake,
        FlipToFlip, FlipToCake, FlipToCakeMaximizer,
        Circle, CircleBNB
    }
