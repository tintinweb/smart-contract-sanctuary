...
contract BabyTCG2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BabyTCG2", "BabyTCG2") {
    	cakeDividendTracker = new CakeDividendTracker();


    	marketingWallet = 0xa5DEC9EcE15772fFcb928FEfde084A16FE74c0a4;

    	cakeDividendToken = 0xF73d8276C15Ce56b2f4AeE5920e62F767A7f3aEA; //
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "BabyTCG2: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "BabyTCG2: The new dividend tracker must be owned by the BabyTCG2 token contract");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BabyTCG2_Dividend_Tracker", "BabyTCG2_Dividend_Tracker", 0xF73d8276C15Ce56b2f4AeE5920e62F767A7f3aEA) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50,000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyTCG2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyTCG2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "BabyTCG2_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "BabyTCG2_Dividend_Tracker: Cannot update claimWait to same value");
