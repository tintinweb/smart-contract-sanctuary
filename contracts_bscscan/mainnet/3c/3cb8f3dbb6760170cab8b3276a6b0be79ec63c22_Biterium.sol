...
contract Biterium is Context, IERC20 {
    ...
    string constant private _name = "Biterium";
    string constant private _symbol = "BTM";
    ...
    address payable private _marketingWallet = payable(0xd949d9f349bF09B2b32706e7deB445c6E3a28a24);
    address payable private _buybackWallet = payable(0x46678CD9f062b73f30fBDD6E71215aa220358045);
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 reflectFee, uint16 liquidityFee, uint16 marketingFee, uint16 buybackFee) external onlyOwner {
        require(reflectFee <= staticVals.maxReflectFee
                ...
                && buybackFee <= staticVals.maxBuybackFee);
        require(liquidityFee + reflectFee + marketingFee + buybackFee <= 3450);
        _buyTaxes.liquidityFee = liquidityFee;
        _buyTaxes.reflectFee = reflectFee;
        _buyTaxes.marketingFee = marketingFee;
        _buyTaxes.buybackFee = buybackFee;
    }

    function setTaxesSell(uint16 reflectFee, uint16 liquidityFee, uint16 marketingFee, uint16 buybackFee) external onlyOwner {
        require(reflectFee <= staticVals.maxReflectFee
                && liquidityFee <= staticVals.maxLiquidityFee
                && marketingFee <= staticVals.maxMarketingFee
                && buybackFee <= staticVals.maxBuybackFee);
        ...
        _sellTaxes.buybackFee = buybackFee;
    }

    function setTaxesTransfer(uint16 reflectFee, uint16 liquidityFee, uint16 marketingFee, uint16 buybackFee) external onlyOwner {
        require(reflectFee <= staticVals.maxReflectFee
                && liquidityFee <= staticVals.maxLiquidityFee
                && marketingFee <= staticVals.maxMarketingFee
                && buybackFee <= staticVals.maxBuybackFee);
        ...
        _transferTaxes.buybackFee = buybackFee;
    }
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _marketingWallet.transfer(address(this).balance * _ratios.marketing / (_ratios.marketing + _ratios.buyback));
    ...
    function enableTrading() public onlyOwner {
        ...
        setExcludedFromReward(address(0), true);
