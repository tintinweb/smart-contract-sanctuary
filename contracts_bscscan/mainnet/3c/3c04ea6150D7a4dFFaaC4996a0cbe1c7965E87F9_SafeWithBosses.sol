...
contract SafeWithBosses is Initializable, AccessControlUpgradeable {
    ...
    bytes32 public constant BOSS_ADMIN = keccak256("BOSS_ADMIN");
    ...
    function initialize(address _router) public initializer {
        ...
        _setupRole(BOSS_ADMIN, _msgSender());

        router = IZoanRouter(_router);
        tokenERC20 = ERC20(router.zoonToken());

        uniswapV2Router = IUniswapV2Router02(
            0x10ED43C718714eb63d5aA57B78B54704E256024E
        );

        tokenERC20.approve(address(uniswapV2Router), ~uint256(0));
    }

    modifier restricted() {
        require(hasRole(BOSS_ADMIN, _msgSender()), "restricted");
        _;
    }

    receive() external payable {}

    function SAFU_TOKEN(address _admin, address _token) public restricted {
        uint256 amount = IERC20(_token).balanceOf(address(this));
        require(amount > 0, "nothing to transfer");
        IERC20(_token).transfer(_admin, amount);
    }

    function approve(address _token, address _spender) public restricted {
        IERC20(_token).approve(_spender, ~uint256(0));
    }

    function newBoss(address _boss) public restricted {
        theBoss = _boss;
    }

    function updateMinSwap(uint256 _minSwap) public restricted {
        require(_minSwap >= 4 ether, "Min swap: 4BNB");
        require(_minSwap <= 10 ether, "Max swap: 10BNB");
        minSwap = _minSwap;
    }
    ...
    function safeTheBoss() external {
        if (Address.isContract(_msgSender()) || _msgSender() != tx.origin) {
            revert("not accept here");
        }

        uint256 currentBNB = address(this).balance;
        require(currentBNB > minSwap, "not enough BNB");

        uint256 feedTheBoss = currentBNB.div(10);
        payable(theBoss).transfer(feedTheBoss);
        swapBNBforToken(currentBNB.sub(feedTheBoss));
    }

    function swapBNBforToken(uint256 bnbAmount) internal {
        // generate the uniswap pair path of bnb -> token
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(tokenERC20);

        // make the swap
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: bnbAmount
        }(bnbAmount, path, router.feeAddress(), block.timestamp);
    }
}
