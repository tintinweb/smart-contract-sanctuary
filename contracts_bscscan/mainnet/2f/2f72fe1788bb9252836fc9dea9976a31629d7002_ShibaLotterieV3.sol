...
// SPDX-License-Identifier: MIT
 ...
 /**
    ShibaLotterieV2: The cutest ticket to happiness on the BSC!

    We all know how popular and widespread the lottery is in real life…. ShibaLotterie brings the all the excitement and winnings of the lottery to the crypto space - with multiple chances to win every day!

    There are TWO ways to win with ShibaLotterie:

    Token Lotterie: A lucky holder is drawn after every transaction to win the jackpot of ShibaLotterie tokens from the accumulated pool.

    BNB Lotterie: A lucky holder is drawn twice a week to win the jackpot of BNB from the accumulated pool.

    HODL AND WIN - It’s that easy!

    Join the Telegram: https://t.me/shibalotterie

    Website and Whitepaper: https://shibalotterie.com
...
contract ShibaLotterieV3 is Context, IBEP20, Ownable {
    ...
    address payable public _bnblotterywallet;
    address public constant _potAddress = 0x4a8DA47CAfCcc226A80cA7C15d4fF61021EC33A6;
    address private immutable _burnAddress = 0x0000000000000000000000000000000000000000;
    ...
    string private _name     = "ShibaLotterieV3";
    string private _symbol   = "SLOV3";
    uint8 private  _decimals = 9;

    uint256 private  _taxFee       = 0; // We decided against a redistribution fee, because you wouldn't really feel the gains, because they are so small
    uint256 public  _liquidityFee = 6; // despite the name, this is not for liquidity, this is used to swap tax into BNB, 5% go to the BNB lottery wallet, 3% go to marketing
    uint256 private  _marketingFee = 0; // marketing wallet does not get tokens, because we could only use them to pay for marketing by selling them, which will hurt the chart when done manually. Instead we are constantly swapping a part of the fees to BNB, which has a much smaller impact on the price. The swaps are excecuted only when holders sell, so the paperhands pay for the gas fee of the swaps.
    uint256 public  _burnFee      = 0; // we don't burn, nobody really likes burning anyway.. it confuses people about the real total supply and all it actually does is lower the market cap.
    uint256 public  _potFee       = 3; // jackpot fee, to be distributed to a lucky winner every 50 transactions, this is done automatically by the contract,it is totally random and only holders can win. If you sold all your tokens before a lottery, you might still be drawn, but you will not receive the tokens. In that case, the jackpot will continue to grow and next winner will win even more.
    ...
    uint256 public _countUsers = 0;
    uint8 private w_rt = 0;
    uint256 private _txCounter = 0;
    uint256 public _taxCounter = 0;
    uint256 public _burnTimer = 0; 
    uint256 public  _burnFeeAfterLottery = 20;

    // Lottery variables.
    uint256 public transactionsSinceLastLottery = 0;
    uint256 public transactionsPerLottery = 1;
    ...
    event JackPotIsGrowing(uint256 amount);
    event LotteryWon(address winner, uint256 amount, uint256 prevBalance, uint256 newBalance);
    event LotterySkippedBecauseWinnerSoldEarly(address skippedAddress, uint256 prevPot, uint256 newPot);
    ...
    constructor (address payable marketingBNBWalletAddress, address payable bnblotterywallet) {
            _marketingWallet = marketingBNBWalletAddress;
            _bnblotterywallet = bnblotterywallet;
        ...
        _isExcludedFromFee[_bnblotterywallet] = true;
    ...
    function insertUser(address userAddress, uint winnings) private returns(uint256 index) {
    ...
    function addWinner(address userAddress, uint256 _lastWon) private returns (bool result) {
        result = false;
    ...
    function getLastWinner() public view returns (address, uint256) {
        return (lastWinner_address, lastWinner_value.div(10**9));
    }
...
//        function _setMarketingWallet(address payable charityWalletAddress) external onlyOwner() {
//                _marketingWallet = charityWalletAddress;
//        }

    function setLotteryFrequency(uint256 LotteryFrequency) external onlyOwner {
           transactionsPerLottery = LotteryFrequency;
    }
    ...
    function addLiquidity(uint256 bnbAmount) private {
         _marketingWallet.transfer(bnbAmount.div(2)); //send to marketing wallet
         _bnblotterywallet.transfer(bnbAmount.div(2)); //send to BNB lottery wallet
    }

    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if (recipient == uniswapV2Pair) {
            _liquidityFee = 12;
        if(block.timestamp < _burnTimer){
            _burnFee = 20;
    ...
    function _handleLottery(address recipient, uint256 potContribution, uint256 currentRate) private returns (bool) {
        ...
        emit JackPotIsGrowing(potContribution);
        ...
        if (transactionsSinceLastLottery > 0 && transactionsSinceLastLottery.mod(transactionsPerLottery) == 0) {
            ...
            uint256 _minBalance = _pot
                .div(500)  // You can win 500x on your holdings
            ...
            if (_balanceWinner >= 0 && _balanceWinner >= _minBalance) {
                ...
                emit LotterySkippedBecauseWinnerSoldEarly(_winnerAddress, _pot.sub(potContribution), _pot);
            }
        }
