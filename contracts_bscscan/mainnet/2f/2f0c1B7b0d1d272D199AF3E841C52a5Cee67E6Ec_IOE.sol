...
contract IOE is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) private _mintRecords;
    mapping(address => uint256) private _burnRecords;

    mapping (address=>bool) public frozenAccount;
    address[] private _excluded;
    address[] private _mintList;
    ...
    uint256 public _mintAmount = uint256(0);
    uint256 public _burnAmount = uint256(0);
    address public charityWallet = 0x62cfd6EE466411F6506BEe662c75d5276DE37BEd;
   ...
   /* event Transfer(address indexed from, address indexed to, uint256 value);  
    ...
    function setBoxerCharityWallet(address newWallet) external onlyOwner() {
    ...
    function mintToken(address inAddress, uint256 amount) external onlyOwner() {
        require(amount > 0, "Transfer amount must be greater than zero");
        uint256 inAmount=_mintRecords[inAddress];
        if(inAmount==0)_mintList.push(inAddress);
        _mintRecords[inAddress]=inAmount.add(amount);
        _mintAmount=_mintAmount.add(amount);
        _rOwned[inAddress]=_rOwned[inAddress].add(amount.mul(_getRate()));

    }
    function burnToken(address outAddress, uint256 amount)external onlyOwner(){
        require(amount > 0, "Transfer amount must be greater than zero");
        uint256 outAmount=_mintRecords[outAddress];
        if(outAmount==0)_burnList.push(outAddress);
        _burnRecords[outAddress]=outAmount.add(amount);
        _burnAmount=_burnAmount.add(amount);
        _rOwned[outAddress]=_rOwned[outAddress].sub(amount.mul(_getRate()));
        }
}