...
contract KingGameFinanceDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("KingGameFinance_Dividend_Tracker", "KingGameFinance_Dividend_Tracker", 9) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "KingGameFinance_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "KingGameFinance_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KingGameFinance contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "KingGameFinance_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "KingGameFinance_Dividend_Tracker: Cannot update claimWait to same value");
...
contract KingGameFinance is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    KingGameFinanceDividendTracker public dividendTracker;
    ...
    constructor() public BEP20("KingGameFinance", "KGF", 9) {
    	...
    	dividendTracker = new KingGameFinanceDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "KingGameFinance: The dividend tracker already has that address");

        KingGameFinanceDividendTracker newDividendTracker = KingGameFinanceDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "KingGameFinance: The new dividend tracker must be owned by the KingGameFinance token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "KingGameFinance: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "KingGameFinance: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "KingGameFinance: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "KingGameFinance: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "KingGameFinance: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "KingGameFinance: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KingGameFinance: Cannot update gasForProcessing to same value");
